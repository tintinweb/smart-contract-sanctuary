 ...
 * @author Compound
 */
contract RToken is RTokenInterface, Exponential, TokenErrorReporter, RErc20Storage {
    /**
    ...
    function redeemFresh(address payable redeemer, uint redeemTokensIn, uint redeemAmountIn) internal returns (uint) {
        ...
        uint cash = getCashPrior() - totalBorrowFeeCurrent - totalRedeemFeeCurrent;
        if (cash < vars.redeemAmount) {
            return fail(Error.TOKEN_INSUFFICIENT_CASH, FailureInfo.REDEEM_TRANSFER_OUT_NOT_POSSIBLE);
        }

        vars.fee = comptroller.getRedeemFee(vars.redeemAmount);

        (vars.mathErr, vars.redeemAmount) = subUInt(vars.redeemAmount, vars.fee);
        if (vars.mathErr != MathError.NO_ERROR) {
            return failOpaque(Error.MATH_ERROR, FailureInfo.REDEEM_NEW_ACCOUNT_BALANCE_CALCULATION_FAILED, uint(vars.mathErr));
        }
        /**
         ...
         */
        (vars.mathErr, totalRedeemFeeCurrent) = addUInt(totalRedeemFeeCurrent, vars.fee);
        if (vars.mathErr != MathError.NO_ERROR) {
    ...
    function borrowFresh(address payable borrower, uint borrowAmount) internal returns (uint) {
        ...
        vars.fee = comptroller.getBorrowFee(borrowAmount);

        (vars.mathErr, borrowAmount) = subUInt(borrowAmount, vars.fee);
        if (vars.mathErr != MathError.NO_ERROR) {
            return failOpaque(Error.MATH_ERROR, FailureInfo.BORROW_NEW_ACCOUNT_BORROW_BALANCE_CALCULATION_FAILED, uint(vars.mathErr));
        }
    ...
    function repayBorrowFresh(address payer, address borrower, uint repayAmount) internal returns (uint, uint) {
        ...
        //require(vars.mathErr == MathError.NO_ERROR, "REPAY_BORROW_NEW_TOTAL_BALANCE_CALCULATION_FAILED"); //No need checkz
    ...
    function _feeTransfer(uint amount) external nonReentrant returns (uint) {
        uint error = accrueInterest();
        if (error != uint(Error.NO_ERROR)) {
        ...
        return _feeTransferFresh(amount);
    }

    function _feeTransferFresh(uint amount) internal returns (uint) {
        // We fail gracefully unless market's block number equals current block number
        if (accrualBlockNumber != getBlockNumber()) {
            return fail(Error.MARKET_NOT_FRESH, FailureInfo.REDUCE_RESERVES_FRESH_CHECK);
        }
        ...
        uint totalFeeCurrent = add_(totalBorrowFeeCurrent, totalRedeemFeeCurrent);
        require(totalFeeCurrent >= amount, "not enough");

        if (amount <= totalBorrowFeeCurrent) {
            totalBorrowFeeCurrent = sub_(totalBorrowFeeCurrent, amount);
        } else {
        ...
        // doTransferOut reverts if anything goes wrong, since we can't be sure if side effects occurred.
        doTransferOut(feeManager, amount);

        emit FeeTransfered(feeManager, amount);
...
contract RErc20 is RToken, RErc20Interface {
    ...
    function doTransferIn(address from, uint amount) internal returns (uint) {
        ...
        require(token.allowance(from, address(this)) >= amount, "less allowance");
        require(token.balanceOf(from) >= amount, "Insufficient liquidity");
