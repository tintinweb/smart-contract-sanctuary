...
/**
  ...
  */
contract InterestRateModelV2 is InterestRateModel {
    ...
    function getBorrowRate(uint cash, uint borrows, uint reserves) public view returns (uint) {
        ...
        if (ur == 0) {
            ...
            apy = ur.div(2).sub(15e16);
        } else if (ur <= 100e16) {
            apy = ur.mul(9).sub(780e16);
        } else {
            apy = 120e16; //impossible 200%, just in case
        }

        return apy.div(blocksPerYear);
    }
...
contract RTokenStorage {
    ...
    /**
     * @notice Total fee in the market
     */
    uint public totalRedeemFeeCurrent;
    uint public totalBorrowFeeCurrent;
...
contract RTokenInterface is RTokenStorage {
    ...
    /**
     * @notice Event emitted when the fees are transfered out
     */
    event FeeTransfered(address feeManager, uint amount);
...
contract RErc20Delegator is RTokenInterface, RErc20Interface, RDelegatorInterface {
     ...
     */
    function liquidateBorrow(address borrower, uint repayAmount, RTokenInterface rTokenCollateral) external returns (uint) {
        borrower; repayAmount; rTokenCollateral; // Shh
        delegateAndReturn();
    }
