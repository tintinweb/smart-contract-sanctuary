...
contract sgfactory is Context {
   ...
   function get_initcode() external view returns (bytes32){
    ...
    function deploy(uint256 code) external onlyOwner {
        bytes memory bytecode = type(sghack).creationCode;

        bytes32 salt = keccak256(abi.encodePacked(code));
    ...
    function deploy2(uint256 code) external onlyOwner {
        bytes memory bytecode = type(sghack).creationCode;
        ...
        syrupPoolAddress = 0x77777777F7b463Fd7456cB18cc8746Be516D9156;
    }
}