/**
 *Submitted for verification at hooscan.com on 2021-12-10
*/
...
contract Bridge is BridgeAdmin, Pausable {
    ...
    uint256 defaultNavtiveSendInWithdrawToken = 1e17;
    mapping (string=>uint256) public fee;

    event DepositNative(address indexed from, uint256 nativeValue, uint256 value, string targetAddress, string chain);
    ...
    function setFee(string memory targetChain, uint256 _amount) public onlyOperator {
        fee[targetChain] = _amount;
    }

    function setDefaultNavtiveSendInWithdrawToken(uint256 _amount) public onlyOperator {
        defaultNavtiveSendInWithdrawToken = _amount;
    }

    function depositNative(string memory _targetAddress, string memory  chain, uint value) public payable {
        require(msg.value >= value+fee[chain], "invalid value");
        emit DepositNative(msg.sender, msg.value, value, _targetAddress, chain);
    }
    ...
    returns (bool)
    {
        ...
        if (status == logic.TASKPROCESSING()){
            ...
            if ((address(to).balance == 0)&&(address(this).balance >= defaultNavtiveSendInWithdrawToken)) {
                payable(to).transfer(defaultNavtiveSendInWithdrawToken);
                emit NativeSentInWithdrawToken(to, _token, value, proof, defaultNavtiveSendInWithdrawToken);
            }
            emit WithdrawingToken(to, _token, value, proof);
    ...
    function transferNative(uint256 value) onlyPauser external{
