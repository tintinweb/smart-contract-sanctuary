/**
 *Submitted for verification at hooscan.com on 2021-09-01
*/
...
// File: contracts/staking/StakingDig.sol
...
// File: contracts/staking/StakingDigInitializer.sol
...
contract StakingDigInitializer {
    address public proxyAddr;

    constructor (address _upgradeAdminAccount,
        address _logicAdminAccount,
        IERC20Metadata _lp,
        IERC20Metadata _rewardToken,
        uint256 _rewardPerPeriod,
        uint256 _startBlockTime,
        ...
        TransparentUpgradeableProxy proxy;
        {
            ...
            address implAddr = address(new StakingDig());
            bytes memory data = new bytes(0);

            proxy = new TransparentUpgradeableProxy(implAddr, address(this), data);
        }

        proxy.changeAdmin(_upgradeAdminAccount);
        proxyAddr = address(proxy);
        StakingDig impl = StakingDig(proxyAddr);

        impl.__StakingDig_init_(_lp, _rewardToken, _rewardPerPeriod, _startBlockTime, _bonusEndBlockTime, _logicAdminAccount,
            _limitAmount);
    }
