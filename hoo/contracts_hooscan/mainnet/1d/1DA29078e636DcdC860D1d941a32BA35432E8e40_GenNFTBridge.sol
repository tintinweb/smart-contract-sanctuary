...
contract GenNFTBridge is Trustable, ERC721Holder {
    using EnumerableSet for EnumerableSet.UintSet;
    ...
    constructor (IERC20 _gen, GenNFT _genNft) {
        gen = _gen;
        genNft = _genNft;
    }
    ...
    function create(uint nftId, uint8 destination, address target) external {
        require(genNft.ownerOf(nftId) == msg.sender, "you must be owner of nft");

        gen.safeTransferFrom(msg.sender, 0x000000000000000000000000000000000000dEaD, cost);
        genNft.safeTransferFrom(msg.sender, address(this), nftId);

        orders[nextOrderId] = Order(
            nextOrderId,
            msg.sender,
            target,
            genNft.getTypeByTokenId(nftId).id,
            destination
        );
        orderIds.add(nextOrderId);

        emit OrderCreated(orders[nextOrderId]);
        nextOrderId++;
    }
    ...
    function completeOrder(uint orderId, uint8 dstFrom, address to, uint typeId) external onlyTrusted {
        bytes32 orderHash = keccak256(abi.encodePacked(orderId, dstFrom));
        require (completed[orderHash] == false, "already transfered");

        genNft.createToken(to, typeId);
