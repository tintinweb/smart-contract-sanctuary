/**
 *Submitted for verification at hooscan.com on 2022-02-07
*/
...
contract WoxTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SWOX }
    ...
    address public sWOX;
    uint public sWOXQueue; // Delays change to sWOX address
    ...
    /**
        @notice allow approved address to deposit an asset for WOX
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint WOX needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( WOX ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn WOX for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IWOXERC20( WOX ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sWOX ).balanceOf( msg.sender ); // Can only borrow against sWOX held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with WOX
        @param _amount uint
     */
    function repayDebtWithWOX( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IWOXERC20( WOX ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, WOX, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( WOX ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( WOX ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns WOX valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match WOX decimals
            value_ = _amount.mul( 10 ** IERC20( WOX ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SWOX ) { // 9
            sWOXQueue = block.number.add( blocksNeededForQueue );
        } else return false;
