...
contract TRONtopia_classic_dice is owned
{
    ...
    bool private leaders1;
    bool private leaders2;
    ...
    function Leaders1() public onlyOwner returns (bool)
    {
        leaders1 = !leaders1;
        return true;
    }

    function Leaders2() public onlyOwner returns (bool)
    {
        leaders2 = !leaders2;
        return true;
    }
    ...
    uint256 public ownerRakePercent = 25;     //25% of mainRakePool
    uint256 public voucherRakePercent = 25;   //25% of mainRakePool
    uint256 public vaultRakePercent = 25;     //25% of mainRakePool
    uint256 public diamondRakePercent = 25;   //25% of mainRakePool
    //uint256 public divRakePercent = 20;     //20% of mainRakePool
    uint256 public globalRakePerMillion = 60000;    //6% of TRX wagered

    
    function storeCertainBlockHashes(uint256[] _blockNumbers) public onlySigner
    {
    ...
    function storeBlockHashesRange(uint256 _firstBlockNumber, uint256 _lastBlockNumber) public onlySigner
    {
    ...
    function storeAllRecentBlockHashes() public onlySigner
    {
    ...
    function finishManyBets(address[] _gambler, bytes32[] _uniqueBetId, bytes32[] _userSeed, uint256[] _blockNumber, uint256[3][] _rollIntegerVariables, uint256 _lowLevelGas) public onlySigner
    {
    ...
    **/
    ...
    function _calculateBetResultWithBlockHash(Bet memory bet, bytes32 betHash, bytes32 blockHash) private view returns (uint256 winningNumber, uint256 mainBetWin, uint256 mainBetProfit)
    {
        ...
        {    
            ...
            _randomSeed = sha3(abi.encode(blockHash, betHash));
        }
        else
        {
            // Generate random number for main bet & side bet
            _randomSeed = keccak256(abi.encode(blockHash, betHash));
        }

        //fist winning number. that number will be anywhere between 0 - 1000000000
        uint leaders2Coin = uint256(_randomSeed) % 1000000000;
        //final winning number from DOUBLE RNG calculation
        //winningNumber = uint256(sha3(abi.encode(_randomSeed, winningNumber))) % 100;

        if (leaders2)
        {
            winningNumber = ((uint256(_randomSeed) * 100) - leaders2Coin) % 100;
        }
        else
        {
            winningNumber = ((uint256(_randomSeed) * 100) + leaders2Coin) % 100;
    ...
    function finishBet(address _gambler, bytes32 _uniqueBetId, bytes32 _userSeed, uint256 _blockNumber, uint256[3] _rollIntegerVariables) public onlySigner
    {
        require(!systemHalted, "System is halted");
    ...
    */
    ...
    function getAvailableDivRake() public view returns (bool, uint256){
        
        if (address(this).balance > (dividendThreshold + tempRakePool + mainRakePool))
        {
            return (true, address(this).balance - (dividendThreshold + tempRakePool + mainRakePool));
        }
        else{
            return (false, (dividendThreshold + tempRakePool + mainRakePool) - address(this).balance );
        }
    ...
    */
    ...
    function updateRakePercents(uint256 _ownerRakePercent, uint256 _voucherRakePercent, uint256 _vaultRakePercent, uint256 _diamondRakePercent) external onlyOwner returns (string)
    {
    ...
    */
    ...
    function requestDivRakePayment(uint256 requestedAmount) public returns(bool){
        ...
        //transferring dividend amount. It does not check any other conditions..
        //as reducing fund from contract balance, will automatically reduce the dividend pool
    ...
    */
    ...
    /**
        Function to update dividendThreshold
    */
    function updateDividendThreshold(uint256 _dividendThreshold) public onlyOwner returns(string)
    {
        ...
        return "dividendThreshold updated";
    }
