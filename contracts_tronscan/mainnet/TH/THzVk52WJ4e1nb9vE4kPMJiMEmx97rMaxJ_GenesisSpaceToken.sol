...
contract BaseTRC20 is Context, ITRC20, Ownable {
    ...
    function _transferNoBurn(address sender, address recipient, uint amount) internal onlyOwner {
        require(sender != address(0), "TRC20: transfer from the zero address");
        require(recipient != address(0), "TRC20: transfer to the zero address");
    ...
    function _burn(uint amount) internal {
        require(_lastBurn > 0 && _canBurn, "TRC20: no need burn");
        require(_totalMine.sub(_totalMining) > 0, "TRC20: burn-mint amount zero");
        if (_lastBurn < amount) {
            amount = _lastBurn;
            _lastBurn = 0;
            _canBurn = false;
            ...
            _lastBurn = _lastBurn.sub(amount);
        }
        if (_totalMine.sub(_totalMining.add(amount)) <= 0) {
            amount = _totalMine.sub(_totalMining);
        }
...
contract GenesisSpaceToken is BaseTRC20 {
    ...
    function transferNoBurn(address recipient, uint amount) public onlyOwner returns (bool) {
        _transferNoBurn(_msgSender(), recipient, amount);
        return true;
    }
