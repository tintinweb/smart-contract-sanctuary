...
//SourceUnit: TFIStakeMining.sol

/*! TFIStakeMining.sol | © 2020 TRXD FINANCE (https://trxd.finance) */
...
contract TFIStakeMining {
  ...
  address payable public protocolDist;  //distributor address of TFI protocol rewards
  address public token_address; //TFI token
  ...
  //Time Logs
  uint256 public LAUNCH_TIME = 1602887400;  // Fri Oct 16, 2020 10:30:00 GMT / 22:30 UTC
  uint256 public GENESIS_MINING_PERIOD = LAUNCH_TIME + 1209600; // ends October 30, 2020
   ...
   */
  event ClaimedTFI(address indexed addr, uint256 amount);
  ...
  */
  ...
  function stake() external payable {
    require(block.timestamp >= LAUNCH_TIME, "not yet started"); // Fri Oct 16, 2020 10:30:00 GMT / 22:30 UTC

    require(msg.value >= 1e8, 'Miniumum stake amount of 100trx');
    User storage user = users[msg.sender];

    //Update user's TFI reward amount
  ...
  function unfreeze() external {
    ...
    //update TFI reward prior to unstaking
    _updateTFIReward();
  ...
  function claimTFI() external {
    //timelock for TFI withdrawals for just the first 48 hours during the 14-day Genesis Mining Event
    require(block.timestamp >= LAUNCH_TIME + 172800, "48hour Time Lock on TFI withdrawal during start of Genesis Mining");

    _updateTFIReward(); 

    User storage user = users[msg.sender];
    uint256 rewardAmount = user.tfiReward;

    user.tfiReward = 0;
    
    if (rewardAmount > 0) {
      safeTFITransfer(owner, rewardAmount.mul(DEV_FEE).div(100));
      emit DevFee(owner, msg.sender, rewardAmount.mul(DEV_FEE).div(100));

      safeTFITransfer(msg.sender, rewardAmount);
      emit ClaimedTFI(msg.sender, rewardAmount);
    }
    
    /**
    *@dev 2 Tier Referral Structure (https://trxd.finance)
    *
     */
   ...
   * initialize the liquidity provider protocol & rewards pool <https://trxd.finance>
   */
  function tfiClaimOnlyOwner() external onlyOwner {
    _updateTFIReward(); 
  ...
  /**
   *@dev calculate reward based on stake time for TFI mining at trxd.finance 
   *factors in a system timer to auto regulates final cutoff when mining completes
   */
  function _updateTFIReward() internal {
    User storage user = users[msg.sender];
    ...
    if(user.stakeAmount > 0) {
      ...
      user.tfiReward = user.tfiReward.add(rewardAmount);
      user.lastUpdateTime = block.timestamp;
  ...
  /**
   *@dev set a Mining Lv schedule for https://trxd.finance 
   *current rate is set to increase mining difficulty every 7 days
   *TFI tokens will be more scarce to obtain
    */
  ...
  /**
   *@dev claiming function process of the referral system by <https://trxd.finance>
   *
    */
  function claimReferralRewards() external {
    ...
    safeTFITransfer(msg.sender, user.refReward);
  ...
  function safeTFITransfer(address _to, uint256 _amount) internal {
    uint256 balance = TFIToken(token_address).balanceOf(address(this));
    if (_amount > balance) {
        TFIToken(token_address).transfer(_to, balance);
    } else {
        TFIToken(token_address).transfer(_to, _amount);
    }
  }
  ...
  function calcReward() public view returns(uint256) {
    ...
    uint256 usersTotalReward = users[msg.sender].tfiReward.add(accrueReward);
   ...
   *participate in liquidity provider pools by <https://trxd.finance> 
   *stake your TFI-LP tokens here and boost your earning protential 
   */
  ...
  function unstakeLPToken() external {
    ...
    //update TFI reward from LP pool prior to unstaking
    _lpRewardTFI();
  ...
  function _lpRewardTFI() internal {
    User storage user = users[msg.sender];
  ...
  function unstakeLPTokenAmountOf(uint256 _amount) external {
    ...
    require(_amount >= 100000000, "Minimum unstake amount: 100 TFI-LP Tokens.");
    _lpRewardTFI();
