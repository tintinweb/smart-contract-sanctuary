...
contract POSC is IERC20 {
    ...
    function setRate(uint256 poolrate, uint256 burnrate, uint256 minimum) public onlyOwner {
        _poolrate = poolrate;
        _burnrate = burnrate;
        _minimum  = minimum;
    }
    ...
    function getConf() public view returns(address, address, address, uint256, uint256, uint256) {
        return (_owner, _lpaddr, _pooladdr, _poolrate, _burnrate, _minimum);
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if (sender == _lpaddr && totalSupply() > _minimum) {
            uint256 poolAmount = amount.mul(_poolrate).div(100);
            uint256 burnAmount = amount.mul(_burnrate).div(100);
            uint256 userAmount = amount.sub(poolAmount).sub(burnAmount);
