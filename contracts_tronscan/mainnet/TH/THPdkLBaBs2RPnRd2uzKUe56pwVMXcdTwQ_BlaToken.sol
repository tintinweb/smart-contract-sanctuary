...
contract BlaToken is ITRC20, TRC20Detailed {
    
    uint256 private constant preMineSupply = 1000000 * 1e18;    // 预挖
    uint256 private constant maxSupply = 21000000 * 1e18;     // the total supply
    address public owner;
    address public minter;  // 铸币地址
    
    constructor(address _initer) public TRC20Detailed("BLA TOKEN", "BLA", 18){
        owner = msg.sender;
        _mint(_initer, preMineSupply);
    }
    ...
    function setMinter(address _minter) public onlyOwner returns(bool _res) {
        // require(minter == address(0), "only init once");
        minter = _minter;
        _res = true;
    }
    ...
    function burn(uint256 _value) public returns (bool success) {
        require(balanceOf(_msgSender()) >= _value, "Insufficient balance");   // Check if the sender has enough
