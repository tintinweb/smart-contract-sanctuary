 ...
 * which is used as a basis for the LUCK TOKEN Project*/
/*
...
contract GlobalsAndUtility is ERC20 {
    ...
    */
    ...
    /* Flush address */
    address payable internal constant FLUSH_ADDR = 0x2E31B0Aa314b1930eE43C2AAceb6231a825a34BC;
    uint8 internal LAST_FLUSHED_DAY = 1;

    /* ERC20 constants */
    string public constant name = "LCK";
    ...
    /* Time of contract launch (29 Oct 2020 12:00 AM (UTC)) */
    uint256 internal constant LAUNCH_TIME = 1603972800;
    ...
    uint256 internal constant MAX_STAKE_DAYS = 180; // 180 days for Max Stake
        ...
        external
    {
        ...
        require(g._currentDay > CLAIM_PHASE_START_DAY, "LCK: Too early");

        if (beforeDay != 0) {
            require(beforeDay <= g._currentDay, "LCK: beforeDay cannot be in the future");
    ...
    {
        require(beginDay < endDay && endDay <= globals.dailyDataCount, "LCK: range invalid");
        ...
        view
    {
        ...
        require(stakeIdParam == stRef.stakeId, "LCK: stakeIdParam not in stake");
...
contract StakeableToken is GlobalsAndUtility {
        ...
        external
    {
        ...
        require(newStakedDays >= MIN_STAKE_DAYS, "LCK: newStakedDays lower than minimum");
        ...
        external
    {
        ...
        require(stakeLists[stakerAddr].length != 0, "LCK: Empty stake list");
        require(stakeIndex < stakeLists[stakerAddr].length, "LCK: stakeIndex invalid");
        ...
        require(g._currentDay >= st._lockedDay + st._stakedDays, "LCK: Stake not fully served");

        /* Stake must still be locked */
        require(st._unlockedDay == 0, "LCK: Stake already unlocked");
        ...
        external
    {
        ...
        /* require() is more informative than the default assert() */
        require(stakeListRef.length != 0, "LCK: Empty stake list");
        require(stakeIndex < stakeListRef.length, "LCK: stakeIndex invalid");
        ...
        internal
    {
        ...
        require(newStakedDays <= MAX_STAKE_DAYS, "LCK: newStakedDays higher than maximum");
        ...
        require(newStakeShares != 0, "LCK: newStakedSuns must be at least minimum shareRate");
        ...
        returns (uint256 bonusSuns)
    {
        /*
            ...
            Bonus percentage scaled 0% to 10% for the first 7M LCK of stake.
...
contract TransformableToken is StakeableToken {
    ...
    function xfLobbyEnter(address referrerAddr)
        ...
        payable
    {
	require(_currentDay() > 0, "LCK: Auction has not begun yet");

        uint256 enterDay = _currentDay();

        uint256 rawAmount = msg.value;
        require(rawAmount != 0, "LCK: Amount required");
    ...
    function xfLobbyExit(uint256 enterDay, uint256 count)
        external
    {
        require(enterDay < _currentDay(), "LCK: Round is not complete");
        ...
        if (count != 0) {
            require(count <= qRef.tailIndex - headIndex, "LCK: count invalid");
            endIndex = headIndex + count;
        } else {
            endIndex = qRef.tailIndex;
            require(headIndex < endIndex, "LCK: count invalid");
        }
        ...
        external
    {
        ...
        require(address(this).balance != 0, "LCK: No value");

        require(LAST_FLUSHED_DAY < _currentDay(), "LCK: Invalid day");
        ...
        returns (uint256 rawAmount, address referrerAddr)
    {
        ...
        require(entry.rawAmount != 0, "LCK: Param invalid");
...
contract LCK is TransformableToken {
    constructor()
        public
    {
