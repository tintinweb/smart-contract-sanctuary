...
contract HKT_721 is Ownable, ERC721Enumerable, ERC721URIStorage {
    // for inherit
    ...
    function newCard(string calldata name_, uint cardId_, uint maxAmount_, string calldata tokenURI_) public onlyOwner {
        require(cardId_ != 0 && cardInfoes[cardId_].cardId == 0, "wrong cardId");
    ...
    function editCard(string calldata name_, uint cardId_, uint maxAmount_, string calldata tokenURI_) public onlyOwner {
        require(cardId_ != 0 && cardInfoes[cardId_].cardId == cardId_, "wrong cardId");
    ...
    function editURIType(uint id_, bool type_) public onlyOwner returns (bool) {
        if (type_) {
            _setTokenURI(id_, id_.toHexString(32));
        } else {
            _setTokenURI(id_, "");
        }
        return true;
    ...
    function editURITypeBatch(uint[] calldata ids_, bool[] calldata types_) public onlyOwner returns (bool) {
        ...
        for (uint i = 0; i < ids_.length; ++i) {
            if (types_[i]) {
                _setTokenURI(ids_[i], ids_[i].toHexString(32));
            } else {
                _setTokenURI(ids_[i], "");
            }
        }
    ...
    function mint(address player_, uint cardId_, bool uriInTokenId_) public returns (uint256) {
        ...
        if (superMinter != _msgSender()) {
            require(minters[_msgSender()][cardId_] > 0, " not minter");
    ...
    function mintWithId(address player_, uint id_, uint tokenId_, bool uriInTokenId_) public returns (bool) {
        require(id_ != 0 && cardInfoes[id_].cardId != 0, "wrong cardId");
    ...
    function mintMulti(address player_, uint cardId_, uint amount_, bool uriInTokenId_) public returns (uint256) {
        require(amount_ > 0, "missing amount");
        require(cardId_ != 0 && cardInfoes[cardId_].cardId != 0, "wrong cardId");

        if (superMinter != _msgSender()) {
            require(minters[_msgSender()][cardId_] >= amount_, "not minter");
            minters[_msgSender()][cardId_] -= amount_;
        }

        require(cardInfoes[cardId_].maxAmount - cardInfoes[cardId_].currentAmount >= amount_, "amount out of limit");
    ...
    function mintBatchWithId(address player_, uint[] calldata ids_, uint[] calldata tokenIds_, bool uriInTokenId_) public returns (bool) {
        require(ids_.length > 0 && ids_.length == tokenIds_.length, "length mismatch");
        for (uint i = 0; i < ids_.length; ++i) {
            mintWithId(player_, ids_[i], tokenIds_[i], uriInTokenId_);
        }
        return true;
    }
