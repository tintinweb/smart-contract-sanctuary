...
contract Tronify{
    ...
    constructor(address creatorAddress) public {
        levelPrice[1] = 50 * 1e6;
        uint8 i;
        ...
        users[creatorAddress] = user;
        idToAddress[1] = creatorAddress;
        
        for (i = 1; i <= LAST_LEVEL; i++) {
            users[creatorAddress].activeX3Levels[i] = true;
            users[creatorAddress].activeX6Levels[i] = true;
        }

        userIds[1] = creatorAddress;
        
        contractDeployTime = now;
        
        emit Registration(creatorAddress, address(0), 1, 0, 0);
    }
    
    function() external payable {
        if(msg.data.length == 0) {
            return registration(msg.sender, creator);
        }
    ...
    function updateX3Referrer(address userAddress, address referrerAddress, uint8 level) private {
        ...
        if (referrerAddress != creator) {
            ...
            sendETHDividends(creator, userAddress, 1, level);
            users[creator].x3Matrix[level].reinvestCount++;
            emit Reinvest(creator, address(0), userAddress, 1, level);
        }
    }
    ...
    function updateX6ReferrerSecondLevel(address userAddress, address referrerAddress, uint8 level) private {
        ...
        if (referrerAddress != creator) {
            ...
            emit Reinvest(creator, address(0), userAddress, 2, level);
            sendETHDividends(creator, userAddress, 2, level);
        }
    }
