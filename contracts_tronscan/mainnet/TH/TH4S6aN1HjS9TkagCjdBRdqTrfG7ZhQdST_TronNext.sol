//SourceUnit: TronNext.sol
    ...
    contract TronNext_TNT is ERC20 {
        using SafeMath for uint256;
        address public owner;
        //1 token = 0.01 trx
        uint256 public tokenCost = 0.1 trx;

        string public name;
        string public symbol;
        uint8 public decimals;
        uint256 public totalSupply = 5e9* 10**6; //500 cr
        ...
        function changeTokeCost(uint256 _tokenCost) public returns (bool) {
            tokenCost = _tokenCost;
            return true;
        }

        function getAmountOfToken(uint256 amount) public view returns (uint256) {
            uint256 tokenValue = (amount.mul(10 ** 6)).div(tokenCost);
            return tokenValue;
        }
    ...
    contract TronNext {
        ...
        struct UserStruct {
            ...
            uint256 totalEarningTrx;
            //persons referred
            address[] referral;
            //time for every level
            uint256 levelExpiresAt;
            //check if users is registered or not
        ...
        modifier userRegistered() {
            require(users[msg.sender].isNotRegistered == true, "User is not registered");
            _;
        }
        ...
        modifier userNotRegistered() {
            require(users[msg.sender].isNotRegistered == false, "User is already registered");
            _;
        }
      ...
      event BuyLevelEvent(address indexed user, uint256 indexed level, uint256 time);
        ...
        constructor(address _ownerAddress1, address _ownerAddress2, address _tokenAddr) public {
            ...
            objTNT = TronNext_TNT(_tokenAddr);
            last_uid = last_uid + 1;
            ...
            //20% = 2000
            LEVEL_INCENTIVE[2].directNumerator = 2000;
            //10% = 1000
            LEVEL_INCENTIVE[2].inDirectNumerator = 1000;
            //10% = 1000
            LEVEL_INCENTIVE[3].directNumerator = 1000;
            //5% = 500
            LEVEL_INCENTIVE[3].inDirectNumerator = 500;
            //5% = 500
            LEVEL_INCENTIVE[4].directNumerator = 500;
            //2.5% = 250
            LEVEL_INCENTIVE[4].inDirectNumerator = 250;
            //3.34% = 334
            LEVEL_INCENTIVE[5].directNumerator = 334;
            //1.7% = 170
            LEVEL_INCENTIVE[5].inDirectNumerator = 170;
        }
        ...
        function registerUser(uint256 _referrerUniqueID) public payable userNotRegistered() validReferrerID(_referrerUniqueID) {
            require(msg.value > 0, "trx value is 0");
            ...
            require(objTNT.transferFrom(owner, msg.sender, tokenAmount), "token transfer failed");
        ...
        function transferLevelPayment(address _userUpline, uint256 _levelForIncentive) internal {
            ...
            uint256 uplineIncentive;

            uint256 count = 1;

            while(uplineUID > 0 && count <= 7) {
                if(count == 1) {
                    uplineIncentive = (trxValue.mul(LEVEL_INCENTIVE[_levelForIncentive].directNumerator)).div(percentageDenominator);
                    if(now <= uplineUserLevelExpiry && users[uplineAddress].isExist) {
                        uplineAddress.transfer(uplineIncentive);
                        users[uplineAddress].totalEarningTrx = users[uplineAddress].totalEarningTrx.add(uplineIncentive);
                    } else {
                    ...
                    uplineIncentive = (trxValue.mul(LEVEL_INCENTIVE[_levelForIncentive].inDirectNumerator)).div(percentageDenominator);
                    if(now <= uplineUserLevelExpiry && users[uplineAddress].isExist) {
                        uplineAddress.transfer(uplineIncentive);
                        users[uplineAddress].totalEarningTrx = users[uplineAddress].totalEarningTrx.add(uplineIncentive);
                    } else {
                ...
                uplineUserLevelExpiry = users[uplineAddress].levelExpiresAt;
        ...
        function buyLevel(uint256 _level) public payable userRegistered() validLevel(_level){
            ...
            require((_level == userCurrentLevel.add(1)) || (userCurrentLevel == 5 && _level == 5), "Invalid level upgrade value");
        ...
        function changeIncentiveForLevel(uint256 _level, uint256 _directNumerator, uint256 _inDirectNumerator) public returns (bool) {
            require(_directNumerator > 0, "Direct Numerator is 0");
            require(_inDirectNumerator > 0, "Indirect Numerator is 0");
            LEVEL_INCENTIVE[_level].directNumerator = _directNumerator;
            LEVEL_INCENTIVE[_level].inDirectNumerator = _inDirectNumerator;
            return true;
        }
         ...
         */
        function getTotalEarnedTRX() public view returns (uint256) {
            uint256 totalTrx;
            for (uint256 i = 1; i <= last_uid; i++) {
                totalTrx = totalTrx.add(users[userAddresses[i]].totalEarningTrx);
            }
            return totalTrx;
        }
         ...
         */
        function getTokenAmountByLevel(uint256 _level) public pure returns (uint256) {
            uint256 amount;
