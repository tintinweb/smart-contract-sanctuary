...
contract GalaxyTron {
    ...
    uint private rate = 60;      
    uint private releaseTime=1607695200;
            ...
            if(players[msg.sender].deposits[pa].amount > 0){
                ...
                for(uint i=0 ; i< pos;i++){
                    ...
                    if(players[msg.sender].deposits[r].amount ==0){
                        players[msg.sender].deposits[r] = Deposit(msg.value,0,0,0,0);
                        getPer(msg.sender,r);
                        return true;
                    }
                    t++;
                ...
                players[msg.sender].deposits[pa] = Deposit(msg.value,0,0,0,0);
                getPer(msg.sender,pa);
                return true;
            }
        }else{
            players[msg.sender].deposits[p] = Deposit(msg.value,0,0,0,0);
            getPer(msg.sender,p);
            return true;
        }
    }
    function getPer(address pla, uint pos) private {
        Player storage player = players[pla];
        if(now >= releaseTime){
            player.deposits[pos].time = now;
            player.deposits[pos].tmp = now;
            player.deposits[pos].per = 95;
        }else{
            player.deposits[pos].time = releaseTime;
            player.deposits[pos].tmp = releaseTime;
            player.deposits[pos].per = 100;
        }
    }
    function withdraw(uint pos) public returns(uint256){
        ...
        uint rate2=player.deposits[pos].per;
