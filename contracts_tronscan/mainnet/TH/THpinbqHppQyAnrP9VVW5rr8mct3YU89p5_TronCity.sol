...
contract TronCity {
    ...
    uint internal startime = 1600362000000;
    ...
    function Signup(address payable _sponsor) public payable{
        ...
        require(msg.value == levelPrice[1] && now >= startime, 'Req 300 trx');
    ...
    function userInfo(address _userId) public view returns(uint _refs, uint _earnings,  uint _dre, uint _mxe, uint _highestStage, uint aC, uint _teamVolume){
        ...
        return(_refs,  _earnings, _dre, _mxe, _highestStage,  aC,  _teamVolume);
    ...
    function stats() public view returns(uint lId, uint rT, uint rTl, uint dE, uint lE, uint mI, uint mCI, uint jT, uint pB, uint cB){
