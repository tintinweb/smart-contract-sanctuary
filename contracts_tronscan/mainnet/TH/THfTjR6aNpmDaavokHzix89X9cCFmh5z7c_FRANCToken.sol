...
contract FRANCToken is Pausable, StandardTokenWithFees, BlackList {
    ...
    function FRANCToken(uint _initialSupply, string _name, string _symbol, uint8 _decimals) public {
    ...
    function decreaseApproval(address _spender, uint _sValue) public whenNotPaused returns (bool) {
        if (deprecated) {
            return UpgradedStandardToken(upgradedAddress).decreaseApprovalByLegacy(msg.sender, _spender, _sValue);
        } else {
            return super.decreaseApproval(_spender, _sValue);
        }
    }
