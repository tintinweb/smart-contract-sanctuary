...
contract TRONLegend is Creator,queue{
    ...
    uint constant public FOMO_MIN_AMOUNT = 5000 trx;
    uint constant public FOMO_INCREASE_AMOUNT = 1000 trx;
    uint constant public FOMO_MAX_AMOUNT = 20000 trx;
    uint constant public ACTIVE_MIN_AMOUNT = 1000 trx;
    ...
    uint constant public FOMO_BALANCE_STEP = 100000 trx;

    uint constant public TIME_STEP = 1 days;
    uint constant private rndMax_ = 1 hours;                // max length a round timer can be
 ...
 //   TDCCoinInterface public tdcCoin;
	...
	function invost(uint amount) public payable isActivated {
    ...
    function withdrawSafe() public isActivated{
        ...
        for(uint j=1;j<=safeIndex;j++){
          ...
          }else if(user.checkpoint>safe[safeIndex].start){
