...
contract nft is Ownable {
    ...
    uint256 public outAmountDay = 1;   // 产出天数
    ...
    function getAddressAll() public view returns (uint256 _length) {
        _length = users.length;
    }

    function setSwitchSwap(uint256 _switchSwap) public onlyOwner returns (bool) {
        switchSwap = _switchSwap;
        return true;
    }

    function setFoundationAddress(address _foundationAddress) public onlyOwner returns (bool) {
        foundationAddress = _foundationAddress;
        return true;
    }

    function setTechnicalTeamAddress(address _technicalTeamAddress) public onlyOwner returns (bool) {
        technicalTeamAddress = _technicalTeamAddress;
        return true;
    }
    
    function setOutAmountDay(uint256 _outAmountDay) public onlyOwner returns (bool)  {
        outAmountDay = _outAmountDay;
        return true;
    }
    
    function setNftToken(IERC20 _nftToken) public onlyOwner returns (bool) {
        nftToken = _nftToken;
        return true;
    }
    
    
    function setNftcToken(IERC20 _nftcToken) public onlyOwner returns (bool) {
    ...
    function setUsdtSwapContract(IJustswapExchange _usdtSwapContract) public onlyOwner returns (bool) {
        usdtSwapContract = _usdtSwapContract;
        return true;
    }
    
    function setNftcSwapContract(IJustswapExchange _nftcSwapContract) public onlyOwner returns (bool) {
        nftcSwapContract = _nftcSwapContract;
        return true;
    }
    
    function getNFTDestoryAmount() public view returns (uint256 _nftDestroyAmount) {
        for (uint256 i = 0; i < users.length; i++) {
            UserInfo memory user = userInfos[address(users[i])];
            _nftDestroyAmount += user.nftDestroyAmount;
        }
    }
    
    function getNFTCDestoryAmount() public view returns (uint256 _nftcDestroyAmount) {
        for (uint256 i = 0; i < users.length; i++) {
            UserInfo memory user = userInfos[address(users[i])];
            _nftcDestroyAmount += user.nftcDestroyAmount;
        }
    }
    ...
    function swap(uint256 _amount) external {
        require(switchSwap == 1, "swap:  close");
        ...
        calculatedForceValueAll += ((_amount * 15)/1500);
    ...
    function swap1(uint256 _amount) external {
        ...
        if (user.calculatedForceValue == 0) {
            users.push(address(msg.sender));
            user.calculatedForceValue = (_amount * getNFTCPrice(1000000000000000000))/150000000;
            user.nftcDestroyAmount = _amount;
        } else {
            user = userInfos[address(msg.sender)];
            user.calculatedForceValue += ((_amount * getNFTCPrice(1000000000000000000))/1500000000);
            user.nftcDestroyAmount += _amount;
        }
        
        calculatedForceValueAll +=  ((_amount * getNFTCPrice(1000000000000000000))/1500000000);
        user.usdtDestroyAmount += ((_amount * getNFTCPrice(1000000000000000000))/1000000);
        user.usdtOutMaxAmount += (user.usdtDestroyAmount * 3);
    ...
    function participateLPReward() public {
        ...
        if (_lpAmount > 0) {
            ...
            lpToken.transferFrom(address(msg.sender), address(this), _lpAmount);
        }
    }
     ...
     * LP授权额度
     **/
    function lpTokenAllowance(address _from) public view returns (uint256 _currentAllowance) {
        _currentAllowance = lpToken.allowance(_from, address(this));
    }
     ...
     * 查询可提取NFTC
     **/
    function getNFTAmount() public view returns (uint256 _nftcAmount) {
        UserInfo memory user = userInfos[address(msg.sender)];
        _nftcAmount = user.rewardRemain;
    }
    
    function transferNFTCAll() public onlyOwner {
     ...
     * 获取算力值、销毁数量
     **/
    function getUserInfo() public view returns (uint256 _calculatedForceValue,uint256 _lpAmount,uint256 _nftDestroyAmount, uint256 _nftcDestroyAmount,uint256 _nftcAmount,uint256 _rewardDebt,uint256 _rewardRemain,uint256 _lpRewardRemain,uint256 _isoverflow,uint256 _usdtOutAmount,uint256 _usdtOutMaxAmount) {
        
        UserInfo memory user = userInfos[address(msg.sender)];
        _calculatedForceValue = user.calculatedForceValue;
        _lpAmount = user.lpAmount;                      // LP 数量
        ...
        _isoverflow = user.isoverflow;
        _usdtOutAmount = user.usdtOutAmount;
        _usdtOutMaxAmount = user.usdtOutMaxAmount;
    }
     ...
     * 产量递减
     **/
    function setToDayOutAmount() public onlyOwner {
     ...
     * 计算收益
     **/
    function setBatchRewardRemain() public onlyOwner {
        ...
        for (uint256 i = 0; i < users.length; i++) {
            ...
            if (user.calculatedForceValue > 0 && calculatedForceValueAll > 0) {
                outAmount = ((user.calculatedForceValue * toDayOutAmount * 47 )/calculatedForceValueAll) / 100;
            }
            
            uint256 _lpAmount = lpToken.balanceOf(users[i]);
            if (lpAmountAll > 0 && _lpAmount > 0) {
                uint256 _outLPAmount = ((_lpAmount * toDayOutAmount * 40 ) / lpAmountAll) /100;
                user.nftcAmount += _outLPAmount;
                user.rewardRemain += outAmount;
                user.lpRewardRemain += _outLPAmount;
                userInfos[address(users[i])] = user;
            }
            
            if (user.isoverflow == 0 && outAmount > 0) {
                uint256 _usdtOutAmount = (outAmount * nftcPrice)/1000000;
