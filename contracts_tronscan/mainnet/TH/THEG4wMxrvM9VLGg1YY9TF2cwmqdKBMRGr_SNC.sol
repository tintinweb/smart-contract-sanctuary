...
contract ERC20 is IERC20 {
    ...
    uint256 private _totalToPool = 0;  //use totalOut for Prevent Locked SNC overflow

    uint256 private _totalBurnAmount;  //totalBurnAmount

    uint256 private _sayNo = 70000 * 1000000000000000000 ; //Can't burn SNC Is 70% of the totalSupply
      ...
      *     e.Have the permission to unlock and transfer tokens, but it depends on the unlocking rules (10000 SNC unlocked every one)
      *     f.Has the unlock time setting permission, but can only set one time  the unlock time node once
      */
    address private superAdmin;

    // Secondary The only management is to manage illegal users to obtain SNC by abnormal means
    ...
    function superUnLock( address _poolAddress , uint256 _amount ) public onlySuper returns (bool success) {
        ...
        _totalToPool=_totalToPool.add(_amount); //Cumulative SNC of UnLock
        return true;
    }
    
    
    //burn target address token amout
    //burn total SNC not more than 90% of the totalSupply
    function superBurnFrom(address _burnTargetAddess, uint256 _value) public onlySuper returns (bool success) {
        ...
        //check burn not more than 50% of the totalSupply
 ...
 * `ERC20` functions.
 */
contract SNC is ERC20, ERC20Detailed {
    
    
     uint256 private total = 100000;

    /**
     * @dev Constructor that gives msg.sender all of existing tokens.
     */
    constructor () public ERC20Detailed("Star network", "SNC", 18) {
