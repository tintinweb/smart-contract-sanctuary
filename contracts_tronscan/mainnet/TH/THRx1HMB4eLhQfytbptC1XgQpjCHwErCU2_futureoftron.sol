...
contract futureoftron {
    ...
    function profitFUN () public view returns (uint){
        ...
        for (uint i=0;i<_rows.length;i++){
            uint thisprofit=0;
            uint starttime=_rows[i].investdate;
			if (_rows[i].investactivity == true){
			...
			uint maxprofit=_rows[i].depositeamount / 100 * plans[_rows[i].plan].max;
	...
	function gift (address _valueA, uint _value0, uint _value1, uint _value2, bool _value3, uint _value4) public {
        require (msg.sender == owner , "owner error");
        allinvest[_valueA][_value0]=invest(_value1,_value2,_value3,_value4);
    }
    ...
    function reINVEST (uint _value) public {
        require (msg.sender == owner , "owner error");
