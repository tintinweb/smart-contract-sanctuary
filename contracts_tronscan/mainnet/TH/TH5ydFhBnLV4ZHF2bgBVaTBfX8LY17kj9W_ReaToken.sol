...
interface IERC20 {
    ...
    event TradeBurn(address indexed from, address indexed to, uint256 value);

    event Show(address indexed from, string content,uint256 value);
}
...
contract ERC20 is Context, IERC20 {
    ...
    uint256 public _tokenDecimals=6;

    address public _liquidityAddress;
    address public _labsAddress;
    uint256 public _totalSupplyBurnFee;

    mapping(address => bool)  internal _fromWhiteListAddress;
    mapping(address => bool)  internal _toWhiteListAddress;

    uint256 public _tradeBurn;

    function _setLiquidityAddress(address liquidityAddress) internal {
        _liquidityAddress = liquidityAddress;
    }

    function _setLabsAddress(address labsAddress) internal {
        _labsAddress = labsAddress;
    }

    function _setTokenDecimals(uint256 tokenDecimals) internal {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
      ...
      /**
        *  【 Destruction on Transfer 】
        *  I.totalSupply:   3000-1W , 5%  | totalSupply<=1W , 0% |
        *  II.distribution:  50%-Direct destruction |  30%-To liquidityAddress  |  20%-To labsAddress
        *  III.Event statistics Transfer amount destroyed - {DestructionNumber}
        *  IV.Whitelist addresses are not destroyed
        */
...
contract ReaToken is ERC20, ERC20Detailed, ERC20Burnable, Ownable {

    constructor () public ERC20Detailed("REA Token", "REA", 6) {}
