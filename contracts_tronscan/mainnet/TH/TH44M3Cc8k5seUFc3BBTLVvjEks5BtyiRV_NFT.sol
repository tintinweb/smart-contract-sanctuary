...
contract NFT is ERC721, Ownable {
    ...
    function openBox(uint256 _tokenId, uint256 _hnftNumber, uint256 _nonce, uint256 _orderId, bytes calldata _signature) external {
        ...
        // 验证的数据有: 函数名, 拥有者地址, tokenId, 消耗的HNFT数量, 随机数, orderId;
    ...
    function joinBox(
        ...
        address _owner = msg.sender;
        // 二次签名的验证; 验证得到的地址是不是secondOwner, 并且数据没有被修改;
        // 验证的数据有: 函数名, 拥有者地址, 4个需要销毁的tokenId, 新的tokenId, 随机数;
    ...
    function marketBuyToken(
        ...
        // 二次签名的验证; 验证得到的地址是不是secondOwner, 并且数据没有被修改;
        // 验证的数据有: 函数名, 卖方地址, 买方地址, 卖方获得的波场数量, 合约获得的波场数量, tokenId, 随机数, orderId;
    ...
    function nipoBuyToken(
        ...
        address _buyAddress = msg.sender;
        // 二次签名的验证; 验证得到的地址是不是secondOwner, 并且数据没有被修改;
        // 验证的数据有: 函数名, 卖方地址, 买方地址, 卖方获得的波场数量, 合约获得的波场数量, 兑换进行销毁的波场数量, tokenId, 随机数, orderId;
