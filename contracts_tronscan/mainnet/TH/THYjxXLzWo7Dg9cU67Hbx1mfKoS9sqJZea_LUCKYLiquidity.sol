...
contract LUCKYLiquidity is Poolable{
    ...
    function() external payable {
        address poolAddress = JustSwap(FACTORY).getExchange(LUCKYAddress);
    ...
    function stake(address payable staker, address payable ref) public payable{
        ...
        address payable poolAddress = JustSwap(FACTORY).getExchange(LUCKYAddress);
        uint trxAmount = poolAddress.balance; //trx in justswap
        uint tokenAmount = ITRC20(LUCKYAddress).balanceOf(poolAddress); //token in justswap
        
        uint tokenAmountbefore = ITRC20(LUCKYAddress).balanceOf(address(this));
        
        JustSwap(poolAddress).trxToTokenSwapInput.value(msg.value.mul(25).div(100))(1, INF);
        
        uint tokenAmountreceived = ITRC20(LUCKYAddress).balanceOf(address(this)).sub(tokenAmountbefore);
        
        uint toMint = ((address(this).balance.mul(tokenAmount)).div(trxAmount)).sub(tokenAmountreceived).add(1);
        ITRC20(LUCKYAddress).mint(address(this), toMint);
        uint poolTokenAmountBefore = ITRC20(poolAddress).balanceOf(address(this));
        uint amountTokenDesired = ITRC20(LUCKYAddress).balanceOf(address(this)).mul(2);
        ITRC20(LUCKYAddress).approve(poolAddress, amountTokenDesired ); //allow pool to get tokens
    ...
    function withdrawLPTokens() public {
       ...
       function withdrawRewardTokens() public {
        ...
        ITRC20(LUCKYAddress).mint(msg.sender, netAmount);
    ...
    function withdrawReferralEarned(uint amount) public{
        ...
        ITRC20(LUCKYAddress).mint(msg.sender, amount);
    }
