...
contract Tronreservebank {
    ...
    string public name = "Tron Reserve Bank";
    ...
    uint256 constant internal tokenPriceInitial_ = 1000000000;
    ...
    address public JSPOOL;

    address public owner;

    constructor(address owner_, address jsPoolAddress) public {
        owner = owner_;
        JSPOOL = jsPoolAddress;
    }
    ...
    function purchaseTokens(uint256 _incomingTron, address _referredBy) internal returns (uint256) {
        uint256 devFund = SafeMath.div(SafeMath.mul(_incomingTron, 5), 100);
        uint256 _incomingTron2 =  SafeMath.sub(_incomingTron, devFund);
        address _customerAddress = msg.sender;
        uint256 _undividedDividends = SafeMath.div(SafeMath.mul(_incomingTron2, entryFee_), 100);
        uint256 _referralBonus = SafeMath.div(SafeMath.mul(_undividedDividends, refferalFee_), 100);
        uint256 _dividends = SafeMath.sub(_undividedDividends, _referralBonus);
        uint256 _taxedTron = SafeMath.sub(_incomingTron2, _undividedDividends);
        uint256 _amountOfTokens = tronToTokens_(_taxedTron);
    ...
    function feeSplit(uint value) internal {
        ...
        uint b = value / 2;
        owner.send(a);
        JustSwap(JSPOOL).trxToTokenTransferInput.value(b)(1, 33136721748, address(this));
