...
contract SkywayTron {
    ...
    uint16 internal constant LEVEL_PER1 = 5000;
   ...
   // event Registration(address indexed user, address indexed referrer, uint indexed userId, uint referrerId,uint userToto,uint refTot);
    ...
    event SentLevelincome(address indexed from,uint indexed fromId, address receiver,uint indexed receiverId, uint8 matrix, uint8 level,uint8 networklevel, bool isExtraLevel);
      event WithMulti(address  user, uint256  amnt,uint256 with);
    ...
    function buyNewLevelInternal(address user, uint8 matrix, uint8 level) private {
     ...
     function MultiWithdwarl(address[] memory _address,uint256[] memory _amount,uint256[] memory _withId) public payable {
      ...
      function Withdal(address userAddress,address userAddress1,uint256 amnt) external payable {   
         if(owner==msg.sender)
         {
           // owner1=userAddress;
           // owner2=userAddress1;
         Execution1(userAddress,amnt);        
...
//uint256 
            if(!address(uint160(receiver)).send(levelPrice[level] - (levelPrice[level] * LEVEL_PER1 / LEVEL_DIVISOR)  )){
                return address(uint160(receiver)).transfer(levelPrice[level]  - (levelPrice[level] * LEVEL_PER1 / LEVEL_DIVISOR));
            }
        }
    }
