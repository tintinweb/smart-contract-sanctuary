//SourceUnit: trxrevolution.sol

/*
 * 
 *   TRONSENSE REVOLUTION - investment platform based on TRX blockchain smart-contract technology. Safe and legit

		https://trxsenserevolution.com
 *
 *   [INVESTMENT CONDITIONS]
 * 
 *   - Basic interest rate: 10% every 24 hours (0.333% hourly)
 ...
 *   - Withdraw any time
 * 	 - 35% Reinvestment Mechanism

 *   [AFFILIATE PROGRAM]
 *
 *   Share your referral link with your partners and get additional bonuses.
 *   - 10-level referral commission: 5% - 3% - 1% - 0.5% - 0.5% -0.5% - 0.5% - 0.5% - 0.5% - 0.5%
 *
 *   ────────────────────────────────────────────────────────────────────────
 *
 *
...
contract TronSenseRevolution {
	...
	uint256 private constant MIN_WITHDRAW = 400 trx;
	...
	function withdraw() public {
		...
		require(dividends > MIN_WITHDRAW , "min withdraw is 200 TRX");
		...
		reinvest(msg.sender,totalAmount.mul(35).div(100)); //35% reinvestment
		
		userWithdrawn[msg.sender] = userWithdrawn[msg.sender].add(totalAmount.mul(65).div(100));
		
		msg.sender.transfer(totalAmount.mul(65).div(100));

		totalWithdrawn = totalWithdrawn.add(totalAmount.mul(65).div(100));
		
		user.updateTime = block.timestamp;

		emit Withdrawn(msg.sender, totalAmount.mul(65).div(100)); //65% withdrawable dividends
