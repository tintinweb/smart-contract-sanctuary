...
contract GlobalsAndUtility is ERC20 {
    ...
    */
    ...
    /* ERC20 constants */
    string public constant name = "T2X";
    string public constant symbol = "T2X";
    uint8 public constant decimals = 8;

    /* Suns per Satoshi = 10,000 * 1e8 / 1e8 = 1e4 */
    uint256 private constant SUNS_PER_T2X = 10 ** uint256(decimals); // 1e8
    ...
    uint256 private constant BPB_MAX_T2X = 7 * 1e6;
    uint256 internal constant BPB_MAX_SUNS = BPB_MAX_T2X * SUNS_PER_T2X;
        ...
        external
    {
        ...
        require(g._currentDay > CLAIM_PHASE_START_DAY, "T2X: Too early");

        if (beforeDay != 0) {
            require(beforeDay <= g._currentDay, "T2X: beforeDay cannot be in the future");
    ...
    {
        require(beginDay < endDay && endDay <= globals.dailyDataCount, "T2X: range invalid");
        ...
        view
    {
        ...
        require(stakeIdParam == stRef.stakeId, "T2X: stakeIdParam not in stake");
...
contract StakeableToken is GlobalsAndUtility {
        ...
        external
    {
        ...
        require(newStakedDays >= MIN_STAKE_DAYS, "T2X: newStakedDays lower than minimum");
        ...
        external
    {
        ...
        require(stakeLists[stakerAddr].length != 0, "T2X: Empty stake list");
        require(stakeIndex < stakeLists[stakerAddr].length, "T2X: stakeIndex invalid");
        ...
        require(g._currentDay >= st._lockedDay + st._stakedDays, "T2X: Stake not fully served");

        /* Stake must still be locked */
        require(st._unlockedDay == 0, "T2X: Stake already unlocked");
        ...
        external
    {
        ...
        /* require() is more informative than the default assert() */
        require(stakeListRef.length != 0, "T2X: Empty stake list");
        require(stakeIndex < stakeListRef.length, "T2X: stakeIndex invalid");
        ...
        internal
    {
        ...
        require(newStakedDays <= MAX_STAKE_DAYS, "T2X: newStakedDays higher than maximum");
        ...
        require(newStakeShares != 0, "T2X: newStakedSuns must be at least minimum shareRate");
        ...
        returns (uint256 bonusSuns)
    {
        /*
            ...
            Bonus percentage scaled 0% to 10% for the first 7M T2X of stake.
...
contract TransformableToken is StakeableToken {
        ...
        payable
    {
        ...
        require(rawAmount != 0, "T2X: Amount required");
    ...
    function xfLobbyExit(uint256 enterDay, uint256 count)
        external
    {
        require(enterDay < _currentDay(), "T2X: Round is not complete");
        ...
        if (count != 0) {
            require(count <= qRef.tailIndex - headIndex, "T2X: count invalid");
            endIndex = headIndex + count;
        } else {
            endIndex = qRef.tailIndex;
            require(headIndex < endIndex, "T2X: count invalid");
        }
    ...
    function xfFlush()
        external
    {
        require(address(this).balance != 0, "T2X: No value");

        require(LAST_FLUSHED_DAY < _currentDay(), "T2X: Invalid day");
        ...
        returns (uint256 rawAmount, address referrerAddr)
    {
        ...
        require(entry.rawAmount != 0, "T2X: Param invalid");
    ...
    {
        /* 1095890410958 = ~ 4000000 * SUNS_PER_T2X / 365 */
...
contract T2X is TransformableToken {
    constructor()
        public
    {
