...
contract TronSpot {
  ...
  function getReferralIncome(address userAddress) public view returns(uint256[] referrals){
    ...
    function deposit(address _affAddr) public payable {
        ...
        players[msg.sender].totalInvested = players[msg.sender].totalInvested + msg.value;
  ...
  function _drawPool() private {
        ...
        pool_cycle++;
         ...
         if(pool_top == address(0)){
             ...
             players[pool_top].pool_Bonus += pool_balance;
    ...
    function transferPayout(address _receiver, uint _amount) internal {
        if (_receiver != address(0)) {
            ...
            player.pool_Bonus= player.pool_Bonus.sub(player.pool_Bonus); // Subtract pool bonus Received
