...
contract ERC20 is IERC20 {
    ...
    bool public isSellAndRemoveFee;
    bool public isBuyAndAddFee;
    ...
    function exchangeAddress() public view returns (address) {
        return _exchangeAddress;
    }

    function setExchangeAddress(address spender) public returns (bool) {
        require(msg.sender == devAddress, "Operator must be developer!");
        _exchangeAddress = spender;
        return true;
    }

    function setIsSellAndRemoveFee(bool _isSellAndRemoveFee) public {
        require(msg.sender == devAddress, "Operator must be developer!");
        isSellAndRemoveFee = _isSellAndRemoveFee;
    }

    function setIsBuyAndAddFee(bool _isBuyAndAddFee) public {
        require(msg.sender == devAddress, "Operator must be developer!");
        isBuyAndAddFee = _isBuyAndAddFee;
    }
    ...
    function _transfer(
        ...
        if (msg.sender == _exchangeAddress) {
            if (sender == _exchangeAddress && isSellAndRemoveFee) {
                require(
                ...
                emit Transfer(sender, lpAddress, rLp);
            } else if (
                _exchangeAddress == recipient &&
                isBuyAndAddFee &&
                totalBurn < MAX_STOP_FEE_TOTAL
            ) {
                uint256 rAmount = amount.mul(totalRate).div(
                    totalRate.sub(bBurnFee)
                );
                require(
                    rAmount <= _balances[sender],
                    "ERC20: transfer amount must be less than account balance！"
                );
                uint256 rBurn = rAmount.sub(amount).add(totalBurn) <=
                    MAX_STOP_FEE_TOTAL
                    ? rAmount.sub(amount)
                    : MAX_STOP_FEE_TOTAL.sub(totalBurn);
...
contract Token is ERC20, ERC20Detailed {
    ...
    constructor(
        ...
        address _devAddress,
        bool _isSellAndRemoveFee,
        bool _isBuyAndAddFee
    ) public ERC20Detailed(_name, _symbol, _decimals) {
        _mint(msg.sender, 100000 * (10**uint256(_decimals)));
        MAX_STOP_FEE_TOTAL = 1000 * (10**uint256(_decimals));
        totalRate = 1000;
        sFundFee = 5;
        sLpFee = 45;
        bBurnFee = 450;
