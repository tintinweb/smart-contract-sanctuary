...
//SourceUnit: FxBridgeLogicV2.sol
...
contract FxBridgeLogicV2 is ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable {
    ...
    uint256 public state_laseOracleSetHeight;
    mapping(address => uint256) public state_lastBatchNonces;
    address[] public bridgeTokens;
    mapping(address => TokenStatus) public tokenStatus;
    /**
     * @dev Update params FxBridgeLogicV2
     *  Add notStandardToken
     *  The Transfer method used to adapt TRON USDT is not standard TRC20
     */
    mapping(address => bool) public notStandardToken;
    ...
    function init(bytes32 _fxBridgeId, uint256 _powerThreshold, address[] memory _oracles, uint256[] memory _powers) public initializer {
        ...
        state_laseOracleSetHeight = block.number;
    ...
    function submitBatch(address[] memory _currentOracles, uint256[] memory _currentPowers,
    ...
    ) public nonReentrant {
        {
            ...
            {
                bool notStandard = notStandardToken[_tokenContract];
                uint256 totalFee;
                for (uint256 i = 0; i < _amounts.length; i++)
                {
                    ...
                    if (notStandard == true) {
                        IERC20MetadataUpgradeable(_tokenContract).transfer(_destinations[i], _amounts[i]);
                    } else {
                ...
                if (notStandard == true) {
                    IERC20MetadataUpgradeable(_tokenContract).transfer(_feeReceive, totalFee);
                } else {
    ...
    function updateNotStandardToken(address _token, bool _isNotStandard) public onlyOwner returns (bool) {
        require(tokenStatus[_token].isActive == true, "Unsupported Token");
        notStandardToken[_token] = _isNotStandard;
        return true;
    }
    ...
    function verifySig(address _signer, bytes32 _theHash, uint8 _v, bytes32 _r, bytes32 _s) private pure returns (bool) {
        bytes32 messageDigest = keccak256(abi.encodePacked("\x19TRON Signed Message:\n32", _theHash));
...
//SourceUnit: IERC20MetadataUpgradeable.sol
...
//SourceUnit: IERC20Upgradeable.sol
...
//SourceUnit: PausableUpgradeable.sol
...
//SourceUnit: ReentrancyGuardUpgradeable.sol
...
//SourceUnit: SafeERC20Upgradeable.sol
...
//SourceUnit: SafeMathUpgradeable.sol
