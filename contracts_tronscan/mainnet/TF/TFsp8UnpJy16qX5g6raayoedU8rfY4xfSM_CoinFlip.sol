...
contract CoinFlip is Ownable {
    ...
    event Won(address winner, uint256 bet, uint256 award);
    event Lose(address loser, uint256 bet);    

    function _getBit() view internal returns (bool){
        if( ((uint256(keccak256(abi.encodePacked(block.coinbase)))) / (address(msg.sender).tokenBalance(_tokenid) + address(msg.sender).balance)) % 2 == 0 )
            return false;
        else
            return true;
    }

    function BitBet() payable public returns (bool){
        return _BitBet();
    }

    function _BitBet() internal returns (bool){
        ...
        if ( _getBit() ) {
            ...
            emit Won(msg.sender, ((msg.tokenvalue)/1000000), ((msg.tokenvalue * _awardRate)/1000000));
            return true;
        } else {
            emit Lose(msg.sender, ((msg.tokenvalue)/1000000));
            return false;
        }
    }  
