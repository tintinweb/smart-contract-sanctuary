...
contract TronWin82 {
    ...
    address payable public insuranceFee;
	...
	constructor(address payable _feeOne, address payable _feeTwo, address payable _feeThree, address payable _insuranceFee, uint256 _startDate) public {
        feeOne = _feeOne;
        feeTwo = _feeTwo;
        feeThree = _feeThree;
	...
	function withdraw(uint8 _plan) public {
		...
		uint256 withdrawAmount = totalAmount.sub(reInvestAmount).sub(insuranceAmount);
		insuranceFee.transfer(insuranceAmount);
		user.totalWithdrawn = user.totalWithdrawn.add(withdrawAmount);
	...
	function getHighestDeposit() public view returns(uint256) {
	...
	function getPercent(uint8 plan) public view returns (uint256) {
		if (block.timestamp > startUNIX) {
			...
			uint256 percent = (plans[plan].percent.add(dayPercent)).mod(255);
			if(percent < plans[plan].percent){
				percent = percent.add(plans[plan].percent);
			}
			return percent;
		} else {
