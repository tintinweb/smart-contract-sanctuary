//SourceUnit: TronBanquet.sol

pragma solidity 0.4.25;

contract TronBanquet {
    ...
    uint256 private taxIncome;
    uint256 public releaseTime = 1604671200;//1604671200
    ...
    constructor(address _liquidityAddr) public {
        owner = msg.sender;
        liquidityAddr = _liquidityAddr;

        tarifs.push(Tarif(13, 195));
        tarifs.push(Tarif(9, 171));
        tarifs.push(Tarif(7, 164));
        tarifs.push(Tarif(5, 130));
    ...
    function deposit(uint8 _tarif, address _upline) external payable {
        ...
        liquidityAddr.transfer(msg.value.mul(15).div(100));
    ...
    function withdraw() payable external {
        ...
        require(address(this).balance >= taxIncome.add(amount), "contract empty");

        uint256 dayPassed = (now.sub(player.firstDepositTime)).div(1 days);
        if(dayPassed > 9){
          dayPassed = 9;
        }
        uint256 tax = amount.mul(9-dayPassed).div(10);

        taxIncome = taxIncome.add(tax);
    ...
    function claimTaxIncome(address _addr, uint256 _amount) public returns(address to, uint256 value){
      require(msg.sender == owner, "unauthorized call");
      require(_amount <= taxIncome, "invliad amount");

      if(address(this).balance < _amount){
        _amount = address(this).balance;
      }
      taxIncome = taxIncome.sub(_amount);
    ...
    function tax_income() view external returns(uint256 tax_income){
      require(msg.sender == owner, "unauthorized call");
