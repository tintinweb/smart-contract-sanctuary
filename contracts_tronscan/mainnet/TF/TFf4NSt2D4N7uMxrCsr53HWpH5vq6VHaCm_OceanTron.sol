...
contract OceanTron {
    ...
    constructor(address _commissionAddress) public {
        ...
        levelPrice[6] = 3000 trx;
        levelPrice[7] = 4000 trx;
        levelPrice[8] = 5000 trx;
        levelPrice[9] = 10000 trx;
        levelPrice[10] = 20000 trx;
        levelPrice[11] = 30000 trx;
        levelPrice[12] = 40000 trx;
        levelPrice[13] = 50000 trx;
        levelPrice[14] = 100000 trx;
        levelPrice[15] = 200000 trx;
        levelPrice[16] = 300000 trx;
        levelPrice[17] = 400000 trx;
        levelPrice[18] = 500000 trx;
        levelPrice[19] = 600000 trx;
        levelPrice[20] = 700000 trx;
        levelPrice[21] = 800000 trx;
        levelPrice[22] = 900000 trx;
        levelPrice[23] = 1000000 trx;
        levelPrice[24] = 5000000 trx;
    ...
    function pair(uint _referrerID) isLock public payable {
        ...
        payForLevel(1, msg.sender,adminCommission,0);
    ...
    function liquidity(uint8 _level) isLock external payable {
        ...
        payForLevel(_level, msg.sender, adminCommission,0);
        emit buyLevelEvent(msg.sender, _level, now);
    }
    
    
    function payForLevel(uint8 _level, address _userAddress, uint _adminPrice, uint8 _uplineLimit) internal {
        ...
        if (loopCheck[msg.sender] > 6 || _uplineLimit >= 6) 
            referer = userList[1];
        ...
        if (users[referer].levelStatus[_level] == true) {
            ...
            if (loopCheck[msg.sender] <= 6) {
                if(referer == ownerAddress) {
                    ...
                    payForLevel(_level, referer, _adminPrice, _uplineLimit+1);
                }
            }
        } else {
            if (loopCheck[msg.sender] <= 6 ) {
