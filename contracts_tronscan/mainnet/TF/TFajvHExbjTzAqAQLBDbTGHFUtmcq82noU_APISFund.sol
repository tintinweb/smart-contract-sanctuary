...
contract APISFund is ITokenRecipient, Ownable {

  using SafeMath for uint256;

    uint256 public TOKEN_DECIMALS = 6;
    uint256 public MIN_DEPOSIT = 100;
    
    uint256 public PROFIT_DAILY_PERCENT = 100;
    ...
    event TransferAdminReward(address indexed _from, uint256 _ownerReward);
    event NewDeposit(address indexed _from, address indexed _referrer, uint256 _value);
    ...
    constructor() public {
      ...
      for(uint8 i = 0; i < LEADER_BONUS_TRIGGERS.length; i++){
        LEADER_BONUS_TRIGGERS[i] = LEADER_BONUS_TRIGGERS[i].mul(10 ** TOKEN_DECIMALS);
        LEADER_BONUS_REWARDS[i]= LEADER_BONUS_REWARDS[i].mul(10 ** TOKEN_DECIMALS);
      }
    }
    ...
    function tokenFallback(address _from, uint256 _value, bytes memory _data) public returns (bool) {
      ...
      if (msg.sender != TOKEN_CONTRACT_ADDRESS) { 
        return false;
      }

      require(_value >= MIN_DEPOSIT.mul(10 ** TOKEN_DECIMALS), "Minimum deposit is 100 tokens");
      ...
      address _referrer;
      assembly {
	      _referrer := mload(add(_data, 32))
      }

      Player storage player = players[_from];
      if (_value >= player.deposit.mul(PROFITABILITY_TRIGGER_PERCENT).div(100)) {
      ...
      if (player.time == 0) {
        ...
        if (_referrer != address(0x0) && players[_referrer].deposit > 0){
          register(_from, _referrer);
        } else{
          register(_from, owner());
        }
        player.dailyPercent = dailyPercent;
      }

      player.deposit = player.deposit.add(_value);

      distributeRef(_value, player.referrer);
      distributeBonuses(_value, player.referrer);

      totalInvested = totalInvested.add(_value);

      uint256 ownerReward = _value.mul(ADMIN_FEE_DEPOSIT_PERCENT).div(100);
      TOKEN.transfer(owner(), ownerReward);
      
      emit NewDeposit(_from, _referrer, _value);
      emit TransferAdminReward(_from, ownerReward);
    ...
    function reinvest() public {
      ...
      require(TOKEN.balanceOf(address(this)) >= player.profit);
    ...
    function collect(address _addr) internal {
      ...
      if (player.time > 0 && timeDiff > 0) {
        uint256 collectProfit = player.deposit.mul(timeDiff).mul(player.dailyPercent).div(100).div(100).div(1 days);
    ...
    function transferPayout(address _receiver, uint256 _amount) internal {
      if (_amount > 0 && _receiver != address(0)) {
        ...
        if (contractBalance > 0) {
          ...
          TOKEN.transfer(_receiver, payout);
    ...
    function distributeRef(uint256 _amount, address _referrer) private {
      ...
      for (uint8 i = 0; i < REFERRAL_FEE_PERCENTS.length; i++) {
        ...
        TOKEN.transfer(ref, refReward);

        ref = players[ref].referrer;
      }

      if (totalReward > 0) {
        TOKEN.transfer(owner(), totalReward);
      }
    }
        ...
        for (uint8 j = 0; j < LEADER_BONUS_TRIGGERS.length; j++) {
          if (players[ref].leadTurnover >= LEADER_BONUS_TRIGGERS[j]) {
            if (!players[ref].receivedBonuses[j]) {
              ...
              TOKEN.transfer(ref, LEADER_BONUS_REWARDS[j]);
            }
          } else {
            break;
          }
    ...
    function changeDailyPercent() internal {
      ...
      if (dailyPercentTime != 0) {
        if (now > dailyPercentTime.add(24 hours)) {
          dailyPercent = dailyPercent.add(20);
