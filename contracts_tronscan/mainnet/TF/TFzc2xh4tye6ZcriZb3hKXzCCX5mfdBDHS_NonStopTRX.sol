...
contract NonStopTRX {
    ...
    function withdraw(uint _amount) public returns(bool){
        ...
        require(_amount <= _player.finance[0].balance, 'NotAllowed');
    ...
    // Import User's Info
    function updateInfo() public returns(bool){
        address _userId = msg.sender;
        (uint _id, , address payable _refBy) = NonStopTRXOld(oldContract).players(_userId);
        require(_id >= 1, 'NotAllowed');
        DataStructs.Players storage _player = players[_userId];
    ...
    function setInfo(address _userId) internal{
        ...
        uint _withdrawan, uint _balance, uint _refs, uint _teamVolume, uint _cycles, uint _teamSize) = NonStopTRX(dataMigration).userInfo(_userId);
