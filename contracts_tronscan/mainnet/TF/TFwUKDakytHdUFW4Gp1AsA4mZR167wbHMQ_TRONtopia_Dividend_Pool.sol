...
contract TRONtopia_Dividend_Pool is owned{
    ...
    uint256 public dividendAccumulated; //this will hold all the fund requested from all the game contract, which will be emptied while distribution
    

    uint256 private confirmedDividendForFreeze;
    uint256 public dividendPaidLastTime;

    uint256 public mainDiviendPaidAllTime;      //this tracks all the distributions done by owner all time
    uint256 public topiaFrozenAtDivDistribution;    //this tracks current total frozen topia when dividend is distributed 
    ...
    */
    ...
    constructor() public {
        ...
        mainDiviendPaidAllTime = 1;
        topiaFrozenAtDivDistribution = 1;
        totalDividendsPaidNumber = 1;
    }
    ...
    */
    function displayAvailableDividendALL() public view returns(bool, uint256){
        ...
        if(totalDividend > 0){
            ...
            uint256 finalAmount = totalDividend + dividendAccumulated;
    ...
    function distributeMainDividend() public  returns(uint256){
        ...
        for(uint i=0; i < totalGameContracts; i++){
            ...
            if(amount > 0){
                ...
                uint256 bronzrTierAmount = finalAmount  * 75 / 100 * bronzeTopiaAllUsers / (bronzeTopiaAllUsers + silverTopiaAllUsers + goldTopiaAllUsers);
                //silver tier
                uint256 silverTierAmount = finalAmount  * 85 / 100 * silverTopiaAllUsers / (bronzeTopiaAllUsers + silverTopiaAllUsers + goldTopiaAllUsers);
                //gold tier
                uint256 goldTierAmount = finalAmount  * goldTopiaAllUsers / (bronzeTopiaAllUsers + silverTopiaAllUsers + goldTopiaAllUsers);
        ...
        if(totalDividend > 0){
            ...
            finalDividendAmount = totalDividend + dividendAccumulated;

            //now lets empty the dividendAccumulated
            dividendAccumulated = 0;

        }

        else if(dividendAccumulated > (totalDividend) ){
            finalDividendAmount = dividendAccumulated - (totalDividend);
            dividendAccumulated = 0;
        }
        ...
        if(finalDividendAmount > 0){
            ...
            mainDiviendPaidAllTime += finalDividendAmount * (100000000 - leaderboardsPercentSUN - refPoolPercentSUN - sharesPoolPercentSUN ) / 100000000;
            topiaFrozenAtDivDistribution += bronzeTopiaAllUsers + silverTopiaAllUsers + goldTopiaAllUsers;
    ...
    */
    function availableMainDividends(address user) public view returns(uint256){
        ...
        (uint256 freezeTierLevel, uint256 freezeAmount) = TRONtopiaInterface(topiaTokenContractAddress).userFreezeTierAndAmount(user);
            
        if(mainDiviendPaidAllTime > previousDivAmountWithdrawal[user] && freezeAmount > 0){
            ...
            uint256 newDividendPoolAmount = mainDiviendPaidAllTime - previousDivAmountWithdrawal[user];
            uint256 totalFrozenTopia = topiaFrozenAtDivDistribution - totalFrozenTopiaTracker[user];
            ...
            if(freezeTierLevel == 0){
                ...
                sharePercentage = freezeAmount * 100 * tronDecimals / (totalFrozenTopia / totalNoOfDivPaid) ;
            }
            //silver tier = freezeTierLevel = 1
            else if(freezeTierLevel == 1){
            ...
            //gold tier = freezeTierLevel = 2
            else if(freezeTierLevel == 2){
            ...
            if(newDivPool * sharePercentage > 0){
    ...
    */
    function withdrawDividendTRXandTopia() public {
        ...
        //processing unfreeze topia - this will return true or false. We dont want to make any condition, 
        //as regardless of transaction return true/false, we want this execution to continue (except this call throws revert)
        //following call only revert if topia token contract does not have enough tokens to transfer to user upon withdrawal
        TRONtopiaInterface(topiaTokenContractAddress).withdrawUnfrozenTopia();

        //processing main dividend
        uint256 availableMainDividend = availableMainDividends(user);
        if(availableMainDividend > 0){

            //update div trackers
            updateDivTrackers(user);
    ...
    */
    function outstandingDivWithdrawFreeze(address user) public returns(bool){
        ...
        uint256 outstandingDiv = availableMainDividends(user);
        

        //update div trackers regardless user has outstanding div or not
        updateDivTrackers(user);
        ...
        if(outstandingDiv + availableMainDividendDivRake > 0){
            ...
            user.transfer(outstandingDiv + availableMainDividendDivRake);
    ...
    */
    function updateDivTrackers(address user) internal {
        ...
        previousDivAmountWithdrawal[user] =  mainDiviendPaidAllTime;
        ...
        totalFrozenTopiaTracker[user] = topiaFrozenAtDivDistribution;
    ...
    */
    ...
    /*============================================
    =      DIV RAKE POOL DIVIDEND FUNCTIONS      =
    =============================================*/
    ...
    uint256 public divPercentageDivRakeSUN = 100000000;  //100% of dividend distributed by default

    uint256 public divPaidAllTimeDivRake = 1;
    ...
    //events
    event DividendPaidDivRake(uint256 indexed totalDividend, uint256 indexed dividendAccumulated, uint256 indexed payableAmount);

    function getDividendPotentialDivRake() public view returns(uint256){
        //we will check dividends of all the game contract individually
        uint256 totalGameContracts = whitelistCallerArray.length;
        uint256 totalDividend;
        for(uint i=0; i < totalGameContracts; i++){
        ...
        if(totalDividend > 0 || dividendAccumulatedDivRake > 0){
            ...
            uint256 newAmount = totalDividend * divPercentageDivRakeSUN / 100 / 1000000; 

            return newAmount + dividendAccumulatedDivRake;
    ...
    function distributeDividendDivRake() public  onlySigner returns(string, uint256, uint256, uint256){

        //checking freeze topia for all tiers individually
        uint256 frozenTopiaGlobal = TRONtopiaInterface(topiaTokenContractAddress).frozenTopiaGlobal();


        require(frozenTopiaGlobal > 0, 'none has frozen any topia');
        require(!globalHalt, 'Global Halt is on');
        ...
        for(uint i=0; i < totalGameContracts; i++){
            ...
            uint256 amount = InterfaceGAMES(gameAddress).getAvailableDivRake();
            if(amount > 0){
                //if amount is positive, which means particular game has positive dividend available
                totalDividend += amount;

                //we will request that dividend TRX from game contract to this token contract
                require(InterfaceGAMES(gameAddress).requestDivRakePayment(amount), 'could not transfer trx');
            }
        }

        uint256 dividendAccumulatedLocal = dividendAccumulatedDivRake;
        if(totalDividend > 0 || dividendAccumulatedLocal > 0){            

            //admin can set % of dividend to be distributed.
            //reason for 1000000 is that divPercentageDivRakeSUN was in SUN
            uint256 newAmount =  totalDividend * divPercentageDivRakeSUN / 100 / 1000000; 
            
            //if dividend % is less than 100%, then track that remainder in another variable
            if(divPercentageDivRakeSUN < 100000000){
                dividendRemainderDivRake +=  totalDividend * (100000000 - divPercentageDivRakeSUN) / 100 / 1000000;
            }
            ...
            divPaidAllTimeDivRake += payableAmount;  
            topiaFrozenAtDistributionDivRake += frozenTopiaGlobal;
            totalDividendsPaidNumberDivRake++;   
            
            dividendPaidLastTimeDivRake = now;
            dividendAccumulatedDivRake = 0;
            
            emit DividendPaidDivRake(totalDividend, dividendAccumulatedLocal, payableAmount);
        ...
        return ("There is nothing to transfer: ", 0,0,0);
    ...
    //to updated divPercentageDivRakeSUN variable by owner
    function updateDivPercentageDivRakeSUN(uint256 _divPercentageDivRakeSUN) public onlyOwner returns(string){
        divPercentageDivRakeSUN = _divPercentageDivRakeSUN;
        return "divPercentageDivRakeSUN updated successfully";
    }
