...
contract AGRITECH_EXCHANGE is Ownable {
     using SafeMath for uint256;
   
    uint public token_price = 270;
	...
	uint public  admin_rate=5;
	
	
    address public owner;
    
    event TokenDistribution(address sender, address receiver,string user_wallet, uint total_token, uint live_rate,string tr_type,uint admin_rate);
   ...
   event onBuy(address buyer , uint256 amount);
    ...
    function withdrawLostTRXFromBalance() public 
    {
	 ...
	 function buyToken(string memory _user,uint tokenQty) public payable {
	           ...
	           require(tokenQty>=MINIMUM_BUY,"Invalid minimum quatity");
	            ...
	            uint trx_amt=((tokenQty+(tokenQty*admin_rate)/100)*(token_price)/1000)*1000000;
	            
	            require(msg.value>=trx_amt,"Invalid buy amount");
				AGRITECH.transfer(msg.sender , (tokenQty*100000000));
				emit TokenDistribution(address(this), msg.sender,_user, tokenQty*100000000, token_price,'BUY',admin_rate);	
	...
	function sellToken(string memory _user,address userAddress,uint tokenQty) public payable 
	{
			...
			uint trx_amt=((tokenQty-(tokenQty*admin_rate)/100)*(token_price)/100)*100000;
	        
			AGRITECH.approve(userAddress,(tokenQty*1000000));
			AGRITECH.transferFrom(userAddress ,address(this), (tokenQty*100000000));
			address(uint160(msg.sender)).send(trx_amt);
			emit TokenDistribution(userAddress,address(this),_user, tokenQty*100000000, token_price,'SELL',admin_rate);
			
	 }
	 
        function token_setting(uint min_buy, uint max_buy, uint min_sale, uint max_sale,uint256 adminrate,uint256 token_price) public payable
        {
