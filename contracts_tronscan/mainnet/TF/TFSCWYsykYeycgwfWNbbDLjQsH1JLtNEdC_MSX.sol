...
contract GlobalsAndUtility is ERC20 {
    ...
    */
    ...
    /* Flush address */
    address payable internal constant FLUSH_ADDR = 0xd58F6c516443D3927b270cf302C1b89f47917F45;
    uint8 internal LAST_FLUSHED_DAY = 1;
    
    
    /* ERC20 constants */
    string public constant name = "MSX";
        ...
        external
    {
        ...
        require(g._currentDay > CLAIM_PHASE_START_DAY, "MSX: Too early");

        if (beforeDay != 0) {
            require(beforeDay <= g._currentDay, "MSX: beforeDay cannot be in the future");
    ...
    {
        require(beginDay < endDay && endDay <= globals.dailyDataCount, "MSX: range invalid");
        ...
        view
    {
        ...
        require(stakeIdParam == stRef.stakeId, "MSX: stakeIdParam not in stake");
...
contract StakeableToken is GlobalsAndUtility {
        ...
        external
    {
        ...
        require(newStakedDays >= MIN_STAKE_DAYS, "MSX: newStakedDays lower than minimum");
        ...
        external
    {
        ...
        require(stakeLists[stakerAddr].length != 0, "MSX: Empty stake list");
        require(stakeIndex < stakeLists[stakerAddr].length, "MSX: stakeIndex invalid");
        ...
        require(g._currentDay >= st._lockedDay + st._stakedDays, "MSX: Stake not fully served");

        /* Stake must still be locked */
        require(st._unlockedDay == 0, "MSX: Stake already unlocked");
        ...
        external
    {
        ...
        /* require() is more informative than the default assert() */
        require(stakeListRef.length != 0, "MSX: Empty stake list");
        require(stakeIndex < stakeListRef.length, "MSX: stakeIndex invalid");
        ...
        internal
    {
        ...
        require(newStakedDays <= MAX_STAKE_DAYS, "MSX: newStakedDays higher than maximum");
        ...
        require(newStakeShares != 0, "MSX: newStakedSuns must be at least minimum shareRate");
        ...
        returns (uint256 bonusSuns)
    {
        /*
            ...
            Bonus percentage scaled 0% to 10% for the first 7M MSX of stake.
...
contract TransformableToken is StakeableToken {
    ...
    function xfLobbyEnter(address referrerAddr)
        ...
        payable
    {
	require(_currentDay() > 0, "MSX: Auction has not begun yet");

        uint256 enterDay = _currentDay();

        uint256 rawAmount = msg.value;
        require(rawAmount != 0, "MSX: Amount required");
    ...
    function xfLobbyExit(uint256 enterDay, uint256 count)
        external
    {
        require(enterDay < _currentDay(), "MSX: Round is not complete");
        ...
        if (count != 0) {
            require(count <= qRef.tailIndex - headIndex, "MSX: count invalid");
            endIndex = headIndex + count;
        } else {
            endIndex = qRef.tailIndex;
            require(headIndex < endIndex, "MSX: count invalid");
        }
        ...
        external
    {
        ...
        require(address(this).balance != 0, "MSX: No value");

        require(LAST_FLUSHED_DAY < _currentDay(), "MSX: Invalid day");
        ...
        returns (uint256 rawAmount, address referrerAddr)
    {
        ...
        require(entry.rawAmount != 0, "MSX: Param invalid");
...
contract MSX is TransformableToken {
    constructor()
        public
    {
