...
contract SynUSwap is Ownable{
    ...
    constructor(address synu_token_address, address swap_token_address, uint token_swap_amount_in, uint trade_token_decimal ,address banker_address, uint token_swap_amount_out) public{
        owner = msg.sender;
    ...
    function Sell(uint synu_amount) public{
        ...
        //Validate transfer amount
        if (token_trade_amount > ERC20Interface.allowance(BankerAddress, address(this))) {
            emit TransferFailed(from_, Keeper, token_trade_amount);
            revert();
        }
        
        ERC20Interface.transferFrom(BankerAddress, address(this), token_trade_amount);
        emit TransferSuccessful(BankerAddress, address(this), token_trade_amount);
    ...
    function SetBankerAddress(address address_)
        public
        onlyOwner
        returns (bool)
    {
        require(address_ != address(0));
        BankerAddress = address_;
        return true;
    }
