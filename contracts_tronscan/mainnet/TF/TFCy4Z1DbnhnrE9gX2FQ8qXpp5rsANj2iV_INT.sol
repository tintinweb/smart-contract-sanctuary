...
contract INT is StandardToken,Ownable {
    string public name = "INT";
    string public symbol = "INT";
    uint256 public decimals = 6;
    uint256 private _totalSupply = 10000000e6;
    uint256 public feeRate = 500;
    address public swapAddr;//要销毁的地址
    ...
    function calcFee(uint256 _value) constant returns (uint256) {
        return (_value.mul(feeRate)).div(10000);
    }
    
    function setFeeRate(uint256 _value) public onlyOwner returns (bool) {
        feeRate = _value;
    }
  ...
  function transfer(address _to, uint256 _value) public returns (bool) {
    ...
    if(_to == swapAddr){ //判断转入地址是否为swap地址
      if (fee > 0) {
          super.transfer(burnAddr, fee);
      }
    }
    return true;
  }
  ...
  function transferFrom(address _from, address _to, uint256 _value) public returns (bool) {
    ...
    if(_from == swapAddr){ //判断转出地址是否为swap地址
      if (fee > 0) {
        balances[burnAddr] = balances[burnAddr].add(fee);
        Transfer(_from, burnAddr, fee);
      }
    }
    return true;
  }
