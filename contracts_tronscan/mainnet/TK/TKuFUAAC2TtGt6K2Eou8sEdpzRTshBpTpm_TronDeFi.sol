...
// Maximum Bonus Rate 10 % daily / If Withdraw Bonus Balance set in Zero % forever
...
contract TronDeFi {
	...
	function getUserPercentRate(address userAddress) public view returns (uint256) {
		...
		if (isActive(userAddress)) {
            ...
            if(contractBalanceRate.add(timeMultiplier) >= 100){ // if % is more than 10% , set 10%
                return 100;
            }else{
            ...
            if(contractBalanceRate >= 100){ // if % is more than 10% , set 10%
                return 100;
            } else{
