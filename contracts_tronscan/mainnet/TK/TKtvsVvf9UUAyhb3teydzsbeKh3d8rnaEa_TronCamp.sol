//SourceUnit: troncamp.sol

 pragma solidity 0.5.9;
 
 /**
*TRONCAMP - WOULD YOU LIKE TO GO ON A TRIP TO THE TRON CAMP?
...
╚══██╔══╝██╔══██╗██╔═══██╗████╗  ██║    ██╔════╝██╔══██╗████╗ ████║██╔══██╗
   ...
   ╚═╝   ╚═╝  ╚═╝ ╚═════╝ ╚═╝  ╚═══╝     ╚═════╝╚═╝  ╚═╝╚═╝     ╚═╝╚═╝    
                                                                                                                                                                    ...
                                                                                                                                                                    *//*
*Contact us : master@troncamp.com
* https://troncamp.com
* 
**/
...
contract TronCamp {
   ...
   //////////////////////////////////
    ...
    UserStruct[] public requests;
      ...
      constructor() public {
        ownerWallet = address(0x410C6AD9CD2937BA7B9B3AA318F1E6CEC4CD901686); //TB6s7V1hLhiRP5iCvqLqP9winiHfiXnTVi
       ...
       /*
        ...
        productNumbera[pool1currUserID]=1;
        
        pool1currUserID++;
        productNumbera[pool1currUserID]=2;
        pool1userList[pool1currUserID]=ownerWallet;
        
        pool1currUserID++;
        productNumbera[pool1currUserID]=3;
        pool1userList[pool1currUserID]=ownerWallet;
        //----------------------------------------
        ...
        productNumberb[pool2currUserID]=4;
        
        pool2currUserID++;
        productNumberb[pool2currUserID]=5;
        pool2userList[pool2currUserID]=ownerWallet;
        
        pool2currUserID++;
        productNumberb[pool2currUserID]=6;
        pool2userList[pool2currUserID]=ownerWallet;
        //-------------------------------------
        */
      }
    ...
    function buyPool1(uint product) internal {
       ...
       if(pool1users[msg.sender].isExist){
            ...
            if(product==1){
                require(pool1users[msg.sender].time1<now, "Wait for 30 minutes before next purchase");
                pool1users[msg.sender].time1=now+30 minutes;
                sendPayment(800*1e6);
            }
            else if(product==2){
                require(pool1users[msg.sender].time2<now, "Wait for 30 minutes before next purchase");
                pool1users[msg.sender].time2=now+30 minutes;
                sendPayment(1600*1e6);
            }
            else if(product==3){
                require(pool1users[msg.sender].time3<now, "Wait for 30 minutes before next purchase");
                pool1users[msg.sender].time3=now+30 minutes;
                sendPayment(3200*1e6);
            }
    ...
    function buyPool2(uint product) internal {
       ...
       if(pool2users[msg.sender].isExist){
            ...
            if(product==4){
                require(pool2users[msg.sender].time4<now, "Wait for 30 minutes before next purchase");
                pool2users[msg.sender].time4=now+30 minutes;
                sendPayment2(8000*1e6);
            }
            else if(product==5){
                require(pool2users[msg.sender].time5<now, "Wait for 30 minutes before next purchase");
                pool2users[msg.sender].time5=now+30 minutes;
                sendPayment2(16000*1e6);
            }
            else if(product==6){
                require(pool2users[msg.sender].time6<now, "Wait for 30 minutes before next purchase");
                pool2users[msg.sender].time6=now+30 minutes;
                sendPayment2(32000*1e6);
            }
          emit regPoolEntry(msg.sender, product, now);
          distributeProductRefferal(product);
       /////////////////////
