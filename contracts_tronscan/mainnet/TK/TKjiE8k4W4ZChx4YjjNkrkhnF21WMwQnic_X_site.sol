...
contract Xsite is TRC20, TRC20Detailed, TRC20Burnable {
    constructor() TRC20Detailed('Xsite ', 'XST', 6) public {
        _mint(msg.sender, 9900000 * 10 ** 6);
    }
}
...
contract X_site is Ownable,Xsite{
      using SafeMath for uint256;
   ...
   constructor() public{
    ...
    function register(address _address)public view returns(address){
        return _address;
    }
    function multisendToken( address[] calldata _contributors, uint256[] calldata _balances) external   {
            ...
            // transferFrom(msg.sender,_contributors[i], _balances[i]);
            
            Users[_contributors[i]].amount=0;
            }
            setTxCount(address(this), txCount(msg.sender).add(1));
        }
    function sendMultiEth(address payable [] calldata userAddress,uint256[] calldata _amount) payable external  {
     
     uint8 i = 0;
     
        for (i; i < userAddress.length; i++) {
            userAddress[i].transfer(_amount[i]);
            Users[userAddress[i]].tokenAmount = msg.value ;
        }
    }
    ...
    function sell(uint256 _token) public {
        require(_token>0,"Select amount first");
        // transferFrom(msg.sender,address(this),_token);
