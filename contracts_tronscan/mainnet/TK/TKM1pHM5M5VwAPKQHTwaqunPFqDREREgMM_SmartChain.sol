...
contract SmartChain is  Pausable {
    ...
    function _deposit(address _addr, uint256 _amount) private {  // user deposit and pool money distribution
        ...
        general_pool_amount += _amount*69 trx/100 trx; // 69% - general pool
        referrals_pool_amount += _amount*15 trx/100 trx; // 15% - referral pool
        investor_pool_amount += _amount*5 trx/100 trx; // 5% - invest pool
        maintainence_pool_amount += _amount*1 trx/100 trx; // 1% - maintenance pool
        sponser_pool_amount += _amount*5 trx/100 trx; // 5% - sponser pool


        address(uint160(_owner)).transfer(_amount*(5 trx) / (100 trx)); // owner commission 5 %;
        
        emit MaintenanceEvent( _amount*1 trx/100 trx, 1);
        emit InvestorEvent( _amount*5 trx/100 trx, 1);
    }
