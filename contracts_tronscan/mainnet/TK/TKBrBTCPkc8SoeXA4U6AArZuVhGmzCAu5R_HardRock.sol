//SourceUnit: SmartTronHardRock.sol
...
contract HardRock {
    ...
    // Const
    uint256 private constant DEV_FEE              = 6;
    uint256 private constant MARKET_FEE           = 4;
    uint256 private constant DAILY_HOLDBONUS      = 0.3E9;
    uint256 private constant UNI_R_BONUS          = 0.4E9;
    uint256 private constant DEFAULT_PLANBONUS    = 10E9;
    uint256 private constant MIN_INVEST           = 50E6; //100E6
    uint256 private constant MIN_WITHDRAW         = 1E6;
    ...
    function setUpline(address _addr, address _upline, uint256 _amount) private {
        if(players[_addr].upline == address(0) && _addr != developer) {
             if(players[_upline].depositCount == 0) {
                 ...
                 players[_addr].cback_bonus += _amount * random() / 100;
    ...
    function deposit(address _upline) external payable {
       ...
       //require(now >= contract_StartTime, "Deposits are not available yet");
        require(msg.value >= MIN_INVEST, "Minimum deposit is 30 TRX");
    ...
    function withdraw() external {
        ...
        require(MIN_WITHDRAW <= player.dividends + player.refer_bonus + player.cback_bonus, "Minimum to withdraw is 100 TRX");
    ...
    function payTrxOnTopFee(uint256 val) private {
        uint256 dev_amount = (val * DEV_FEE) / 100;
        uint256 market_amount = (val * MARKET_FEE) / 100;
        developer.transfer(dev_amount);
        marketing.transfer(market_amount);
    }

    function random() view private returns (uint256) {
        uint256 number = uint8(uint256(keccak256(abi.encode(block.timestamp, block.difficulty)))%21) + 10;
        return number;
    }
   ...
   function contractInfo() view external returns(uint256 _invested, uint256 _investors, uint256 _referBonus, uint256 _withdrawn, uint256 _depositCount,
    ...
    function userGeneralInfo(address _addr) view external returns(uint256 _referBonus, uint256 _cbackBonus, uint256 _totalDeposited, uint256 _totalWithdrawn,
    ...
    function userPlanInfo(address _addr) view external returns(uint256 _depositCount, uint256 _activeDeposit, uint256 _reinvestCount, uint256 _dividends, uint256 _tarifN, uint256 _tarifB) {
        ...
        uint256 dividends = getTotal_InterestProfit(_addr) + player.refer_bonus + player.cback_bonus;
