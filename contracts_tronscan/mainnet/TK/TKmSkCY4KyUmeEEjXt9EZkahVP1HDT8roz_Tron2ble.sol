...
contract Tron2ble {
    ...
    uint private releaseTime = 1604469600;
            ...
            if(players[msg.sender].deposits[pa].amount > 0){
                ...
                for(uint i=0 ; i< pos;i++){
                    ...
                    if(players[msg.sender].deposits[r].amount ==0){
                        players[msg.sender].deposits[r] = Deposit(msg.value,now,now+7 days,0);
                        return true;
                    }
                    t++;
                }
            }else{
                players[msg.sender].deposits[pa] = Deposit(msg.value,now,now+7 days,0);
                return true;
            }
        }else{
            players[msg.sender].deposits[p] = Deposit(msg.value,now,now+7 days,0);
            return true;
        }
    }
    function withdraw(uint pos) public returns(uint256){
        ...
        uint rate2=0;
        if( (player.deposits[pos].time+1 days)<= now ){
            rate2 = 20+rate;
        }else{
            rate2 = rate;
        }
        ...
        uint256 total = getPercent(player.deposits[pos].amount,rate2)*7;
