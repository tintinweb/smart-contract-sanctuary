...
contract TRONZ is TRC20 {
    ...
    /**
     * @dev To change tronzAddress 
     * @param _newTronzAddress New Tronz address
     */ 
    function changeTronzAddress(address _newTronzAddress) public onlyOwner returns(bool) {
        require(_newTronzAddress != address(0), "Invalid Address");
        tronzAddress = _newTronzAddress;
        return true;
    }
    
    /**
     * @dev To Share Tronz Tokens by Admin
    ...
    /**
     * @dev To Distribute Tronz Tokens by tronz
     * @param _receiver Reciever address
     * @param _amount Amount to mint
     */ 
    function distribute(address _receiver, uint256 _amount) public returns (bool) {
        ...
        require(msg.sender == tronzAddress,"Only From Tronz");
        require(balances[address(this)] >= _amount, "Insufficient Token");
        balances[address(this)] = balances[address(this)].sub(_amount);
        balances[_receiver] = balances[_receiver].add(_amount);
        emit Transfer(address(this), _receiver, _amount);
