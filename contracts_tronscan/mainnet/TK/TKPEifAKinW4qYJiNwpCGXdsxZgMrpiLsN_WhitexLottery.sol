...
contract WhitexLottery is Ownable, Initializable, Testable {
    // Libraries 
    // Safe math
    using SafeMath for uint256;
    ...
    // State variables 
    // Instance of WHX token (collateral currency for lotto)
    IERC20 internal whx_;
    // Storing of the NFT
    ...
    struct LottoInfo {
        ...
        uint256 prizePoolInWHX;    // The amount of WHX for prize money
        uint256 costPerTicket;      // Cost per ticket in $WHX
    ...
    function getBasicLottoInfo(uint256 _lotteryId) external view returns(uint256 lotteryID, 
        uint256 prizePoolInWHX, uint256 costPerTicket, uint256 startingTimestamp, uint256 closingTimestamp, uint8[] memory prizeDistribution,  uint16[] memory winningNumbers){
        return(
            allLotteries_[_lotteryId].lotteryID,
            allLotteries_[_lotteryId].prizePoolInWHX,
            allLotteries_[_lotteryId].costPerTicket,
            allLotteries_[_lotteryId].startingTimestamp,
            allLotteries_[_lotteryId].closingTimestamp,
            allLotteries_[_lotteryId].prizeDistribution,
    ...
    function updateNFTContract(address _nft) external onlyOwner() {
       require(
            _nft != address(0),
            "Contracts cannot be 0 address"
        );
        nft_ = ILotteryNFT(_nft);
    }
    ...
    function RandomGenerator() internal view returns(uint){
        return uint(keccak256(abi.encodePacked(now,block.difficulty,maxValidRange_)));
    }
        ...
        onlyOwner() 
    {
        ...
        uint256 _randomNumber = RandomGenerator();
        
        numbersDrawn(_lotteryId, _randomNumber);
     ...
     * @param   _prizePoolInWHX The amount of WHX available to win in this 
     *          lottery.
        ...
        returns(uint256 lotteryId)
    {
        ...
        require(
            _prizePoolInWHX != 0 && _costPerTicket != 0,
    ...
    function withdrawWHX(uint256 _amount) external onlyOwner() {
        whx_.transfer(
            msg.sender, 
            _amount
        );
        ...
        notContract()
    {
        ...
        // Transfers the required WHX to this contract
        whx_.transferFrom(
            msg.sender, 
            address(this), 
            costWithDiscount
    ...
    function claimReward(uint256 _lotteryId, uint256 _tokenId) external notContract() {
        ...
        allLotteries_[_lotteryId].prizePoolInWHX = allLotteries_[_lotteryId].prizePoolInWHX.sub(prizeAmount);
        // Transfering the user their winnings
        whx_.safeTransfer(address(msg.sender), prizeAmount);
    }
        ...
        notContract()
    {
        ...
        whx_.safeTransfer(address(msg.sender), totalPrize);
    }
     ...
     * @return  uint256: The prize amount in WHX the user is entitled to 
     */
        ...
        returns(uint256) 
    {
        ...
        prize = allLotteries_[_lotteryId].prizePoolInWHX.mul(perOfPool);
