...
TET is the new experimental platform of the TRON DeFi. 
TET is the first of a series of new services, incentives and platforms. 
Together with the others DeFi's projects, it enriches the whole TRON ecosystem. 
TET Money currency can be used in the same way as any other cryptocurrency. 
Can be sent free of charge to third parties, used as payment for goods and 
services, as a protection against market volatility or in other services 
and DeFi's platforms. 

 TET mining pools are mined every second. 
 The reward is fixed according to the stake amount.

 Total Supply is 20,000,000
...
contract TETMoney is Election,Pools,ByPass { 
    event Staked(
        address indexed owner,
        address indexed ref,
        uint256 poolID,
    ...
    function unstake(uint _poolID, uint _percent) external usePercent returns (bool){ 
        ...
        if(pools[_poolID].native) {
            if(holder.stake[_poolID].time+penalty_time <= block.timestamp) {
            ...
            if(holder.stake[_poolID].time+penalty_time <= block.timestamp) {
                pools[_poolID].token.safeTransfer(msg.sender, _value);
            } else {
    ...
    function settings(uint256 _refcomm, uint256 _penalty_time, uint256 _penalty_fee) external onlyCollegiate {
        refcomm = _refcomm;
        penalty_time = _penalty_time;
