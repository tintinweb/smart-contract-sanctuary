//SourceUnit: TeleTronAuto50.sol

pragma solidity 0.5.9;

contract TeleTronAuto50 {
    ...
    function deposit(address _referral) external payable {
        ...
        if(player.milestones[1] == false && player.total_invested >= 10000000000){
            player.milestones[1] = true;
            player.bonus_percent += 2;
        }
        if(player.milestones[2] == false && player.total_invested >= 100000000000){
            player.milestones[2] = true;
            player.bonus_percent += 5;
        }
    ...
    function _setReferral(address _addr, address _referral) private {
        if(players[_addr].referral == address(0)) {
            ...
            for(uint8 i = 0; i < referral_bonuses.length; i++) {
                ...
                if(i == 0){
                    if(players[_addr].milestones[4] == false && players[_referral].referrals_per_level[i] >= 20){
                        players[_addr].milestones[4] = true;
                        players[_addr].bonus_percent += 2;
                    }
                    if(players[_addr].milestones[5] == false && players[_referral].referrals_per_level[i] >= 50){
                        players[_addr].milestones[5] = true;
                        players[_addr].bonus_percent += 5;
                    }
                }
    ...
    function _referralPayout(address _addr, uint256 _amount) private {
        ...
        for(uint8 i = 0; i < referral_bonuses.length; i++) {
            ...
            if(i == 0){
                ...
                if(players[ref].last_payout != 0 && players[ref].last_payout < (block.timestamp-86400)){ _bonus_perc += 2; }
