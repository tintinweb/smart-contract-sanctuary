...
contract Box is RandomGenerator, Ownable {
    ...
    uint public walletLimit = 1e16;
    uint public valueLimit = 200e18;
    Mining public mine;
    ...
    function setValueLimit(uint limit_) external onlyOwner {
        valueLimit = limit_;
    }

    function setWalletLimit(uint limit_) external onlyOwner {
        walletLimit = limit_;
    }
    ...
    function setAddress(address burnAddress_, address mining_) external onlyOwner {
    ...
    function buyBox(uint amount_) public {
        require(userInfo[msg.sender].buyAmount + amount_ <= limit, 'over limit');
        require(mine.checkUserValue(msg.sender) >= valueLimit, 'not enough value');
    ...
    function withdraw(address token_, address wallet, uint amount_) public onlyOwner {
        IERC20(token_).transfer(wallet, amount_);
    }

    function getPrice() public view returns (uint){
        return mine.getHktPrice();
    }

    function openBox() public {
        require(userInfo[msg.sender].box > 0, 'no box');
        require(IERC20(HKT).balanceOf(_msgSender()) >= walletLimit, 'not enough HKT');
