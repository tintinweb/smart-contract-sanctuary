...
contract ERC20 is Context, TRC20 {
    ...
    // allocating 20 million tokens for promotion, airdrop, liquidity and dev share
...
contract GlobalsAndUtility is ERC20 {
    ...
    */
    ...
    /* Flush address */
    address payable internal constant FLUSH_ADDR = 0x293AFee30818E65547971644f2fD1Fad6493B279;
    uint8 internal LAST_FLUSHED_DAY = 1;

    /* ERC20 constants */
    string public constant name = "RK";
    string public constant symbol = "A2B";
    uint8 public constant decimals = 8;

    /* Suns per Satoshi = 10,000 * 1e8 / 1e8 = 1e4 */
    uint256 private constant SUNS_PER_DIV = 10 ** uint256(decimals); // 1e8

    /* Time of contract launch (2020-11-12T23:10:00Z) */
    uint256 internal constant LAUNCH_TIME = 1605202800;
    ...
    uint256 internal constant MAX_STAKE_DAYS = 120; // Approx 4 months
        ...
        external
    {
        ...
        require(g._currentDay > CLAIM_PHASE_START_DAY, "A2B: Too early");

        if (beforeDay != 0) {
            require(beforeDay <= g._currentDay, "A2B: beforeDay cannot be in the future");
    ...
    {
        require(beginDay < endDay && endDay <= globals.dailyDataCount, "A2B: range invalid");
        ...
        returns (uint256)
    {
        if (block.timestamp < LAUNCH_TIME){
             ...
             // for every 10 mins its counts as 1 day
        }
    }
        ...
        view
    {
        ...
        require(stakeIdParam == stRef.stakeId, "A2B: stakeIdParam not in stake");
...
contract StakeableToken is GlobalsAndUtility {
        ...
        external
    {
        ...
        require(newStakedDays >= MIN_STAKE_DAYS, "A2B: newStakedDays lower than minimum");
        ...
        external
    {
        ...
        require(stakeLists[stakerAddr].length != 0, "A2B: Empty stake list");
        require(stakeIndex < stakeLists[stakerAddr].length, "A2B: stakeIndex invalid");
        ...
        require(g._currentDay >= st._lockedDay + st._stakedDays, "A2B: Stake not fully served");

        /* Stake must still be locked */
        require(st._unlockedDay == 0, "A2B: Stake already unlocked");
        ...
        external
    {
        ...
        /* require() is more informative than the default assert() */
        require(stakeListRef.length != 0, "A2B: Empty stake list");
        require(stakeIndex < stakeListRef.length, "A2B: stakeIndex invalid");
        ...
        internal
    {
        ...
        require(newStakedDays <= MAX_STAKE_DAYS, "A2B: newStakedDays higher than maximum");
        ...
        require(newStakeShares != 0, "A2B: newStakedSuns must be at least minimum shareRate");
        ...
        returns (uint256 payout)
    {
        ...
        for (uint256 day = beginDay; day < endDay; day++) {
            ...
            /* user's share of 98% of the day's dividends */
        ...
        returns (uint256 bonusSuns)
    {
        /*
            ...
            Bonus percentage scaled 0% to 10% for the first 7M A2B of stake.
...
contract TransformableToken is StakeableToken {
    ...
    function xfLobbyEnter(address referrerAddr)
        ...
        payable
    {
	require(_currentDay() > 0, "A2B: Auction has not begun yet");

        uint256 enterDay = _currentDay();

        uint256 rawAmount = msg.value;
        require(rawAmount != 0, "A2B: Amount required");
    ...
    function xfLobbyExit(uint256 enterDay, uint256 count)
        external
    {
        require(enterDay < _currentDay(), "A2B: Round is not complete");
        ...
        if (count != 0) {
            require(count <= qRef.tailIndex - headIndex, "A2B: count invalid");
            endIndex = headIndex + count;
        } else {
            endIndex = qRef.tailIndex;
            require(headIndex < endIndex, "A2B: count invalid");
        }
    ...
    /**
     * @dev PUBLIC FACING: Release 2% dev share from daily dividends
     */
    function xfFlush()
        external
    {
        ...
        require(address(this).balance != 0, "A2B: No value");

        require(LAST_FLUSHED_DAY < _currentDay(), "A2B: Invalid day");
        ...
        returns (uint256 rawAmount, address referrerAddr)
    {
        ...
        require(entry.rawAmount != 0, "A2B: Param invalid");
...
contract A2B is TransformableToken {
    constructor()
        public
    {
