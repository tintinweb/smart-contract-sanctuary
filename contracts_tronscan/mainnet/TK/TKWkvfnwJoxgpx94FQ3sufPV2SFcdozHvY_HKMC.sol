 ...
 *
 */
contract HkmcFeePool is Ownable ,ITRC20, MinterRole {
    ...
    constructor(address _hkmcContract) public {
        hkmcContract = HKMC(_hkmcContract);
    }
    ...
    function notifyTx(address _payer, uint256 _value) external onlySender(address(hkmcContract)) returns (bool) {
    ...
    function transfer(address _to, uint256 _value) public returns (bool) {
        ...
        if (_to == address(this)) {
            uint256 currentPool = hkmcContract.balanceOf(address(this));
            uint256 currentPZ = currentMined();
            uint256 result = currentPool.mul(_value).div(currentPZ);

            _transfer(msg.sender, _to, _value);

            hkmcContract.feePoolTransfer(msg.sender, result);
 ...
 *
 */
contract HKMC is Ownable, ITRC20, MinterRole {
    using SafeMath for uint256;
   ...
   */
    ...
    */
    ...
    function changeFeePool(address _newFeePool) external onlyOwner returns (bool) {
        ...
        feePool = HkmcFeePool(_newFeePool);
