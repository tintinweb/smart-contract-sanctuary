...
contract NFT is ERC721, Ownable {
    ...
    // just swap; GDCV-TRX配对合约地址; TBu8mRm3CD6EhUSnh7QxMJAoLM3KvwYxHf=0x152B0d70C0fEE3B471f02dA25Ea4B176BC33cdE7;
    ...
    function openBlindBox(uint256 _seriesId) external {
        ...
        userTokenBoxStructBlock[_tokenId] = block.number;
    }
    ...
    function userBuyOrder(uint256 _orderId) external payable {
        ...
        if(o_.seller == Owner) {
            ...
            (uint256 _tokenNumber) = IJustswapExchange(justSwapPair).trxToTokenSwapInput.value(_value20)(0 , block.timestamp + 300);
            (bool success, ) = hnftAddress.call(
    ...
    /* // 上级; 用户地址=>上级地址
     ...
     // 设置上级
     function setSuper(address _address) public {
         require(superAddress[msg.sender] == address(0), "NFT: There is already a superior");
         superAddress[msg.sender] = _address;
     }*/
    ...
    function userTakeRed2(address _address) private {
        ...
        if(_amount > 1000) {
            ...
            (address(uint160(_super1))).transfer(_amount.mul(3).div(100));
            (address(uint160(_super2))).transfer(_amount.mul(2).div(100));
            (address(uint160(msg.sender))).transfer(_amount.mul(95).div(100));*/
