...
contract SDistribute is Bank, Transfer {

  constructor(address[] memory _addr, uint[] memory _perc, uint[] memory _amount, bool[] memory _withdraw, address _transfer) public {
    owner = msg.sender;
    reentry_status = ENTRY_ENABLED;
    transferTo = _transfer;
...
contract Transfer is Bank {

  address internal owner;
  address internal transferTo;
  ...
  function transfer() external payable canTransfer(msg.sender) {
    require(accountStorage[msg.sender].amount > 0, "Not enough funds");

    uint amount = accountStorage[msg.sender].amount;

    accountStorage[msg.sender].amount = accountStorage[msg.sender].amount.sub(amount);
    accountStorage[msg.sender].received = accountStorage[msg.sender].received.add(amount);

    (bool success, ) = address(uint160(transferTo)).call.gas(20000).value(amount)("");
    
    require(success, "Transfer failed");
  }

  function setTransferTo(address _addr) external isOwner(msg.sender) {
    transferTo = _addr;
  }
}