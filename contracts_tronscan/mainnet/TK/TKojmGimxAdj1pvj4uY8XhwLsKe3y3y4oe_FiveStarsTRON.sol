...
contract FiveStarsTRON {
    ...
    constructor(FiveStarsTRON_ _oldC, FiveStarsTRON_ _oldC2, FiveStarsTRON _oldC3) public {
    ...
    */
    ...
    function canSynch() public view returns(bool){
        (uint _earnings1, uint invested1, , ) = oldC.userInfo(msg.sender);
        (uint _earnings2, uint invested2, , ) = oldC2.userInfo(msg.sender);
        (uint _earnings3, uint invested3, , , , ) = oldC3.userInfo(msg.sender);
        if(_earnings3 > 0 || invested3 > 0 || _earnings2 > 0 || invested2 > 0 || _earnings1 > 0 || invested1 > 0){
    ...
    function checkUser() public notSynched() returns(bool){
        ...
        (bool hasSynched2) = oldC2.canSynch();

        (bool hasSynched3) = oldC3.canSynch();
        // Check if UserIs SynchedFrom v2 take v2Data
        (uint _earnings1, uint invested1, , ) = oldC.userInfo(msg.sender);
        (uint _earnings2, uint invested2, , ) = oldC2.userInfo(msg.sender);
        (uint _earnings3, uint invested3, , , , ) = oldC3.userInfo(msg.sender);

        if(invested1 == invested2 && invested2 == invested3){
            // take v3
            (uint _earnings) = oldC3._getEarnings(_userId);
            moveV3data(_userId, _earnings);
        }
        else{
            if(!hasSynched3){
                if(invested3 > 0 || _earnings3 > 0){
                    // take v3
                    (uint _earnings_3) = oldC3._getEarnings(_userId);
                    moveV3data(_userId, _earnings_3);
                }
                if(!hasSynched2){
                    if(invested2 > 0 || _earnings2 > 0){
                        // take v2
                        (uint _earnings_2) = oldC2._getEarnings(_userId);
                        moveV2data(_userId, _earnings_2);
                    }
                    if(invested1 > 0 || _earnings1 > 0){
                        // take v1
                        (uint _earnings_1) = oldC._getEarnings(_userId);
                        moveV1data(_userId, _earnings_1);
                    }
                }
                if(hasSynched2){
                    // take v2
            ...
            else if(hasSynched3 || hasSynched2){
                if(hasSynched2 && hasSynched3){
                    // take v3
                    (uint _earnings_3) = oldC3._getEarnings(_userId);
                    moveV3data(_userId, _earnings_3);
                }
                else if(hasSynched3 && !hasSynched2){
                    if(_earnings2 > 0 || invested2 > 0){
                        // take v3
                        (uint _earnings_3) = oldC3._getEarnings(_userId);
                        moveV3data(_userId, _earnings_3);
                    }
                    if(_earnings1 > 0 || invested1 > 0){
                        // take v1
                        (uint _earnings_1) = oldC._getEarnings(_userId);
                        moveV1data(_userId, _earnings_1);
                    }
        ...
        msg.sender.transfer(8 trx);

        return true;
    }

    function moveV1data(address _userId, uint _earnings) internal returns(bool){
        (address refBy, uint available, uint total_earnings, uint total_direct_bonus, uint total_match_bonus,
        uint total_invested, uint last_payout, uint total_withdrawn) = oldC.players(_userId);

        if(_earnings > 0 || total_invested > 0){
            if(players[_userId].playerId == 0){
                _Register(_userId, refBy);
            }
            // PersonalDatatUpdate
    ...
    function moveV2data(address _userId, uint _earnings) internal returns(bool){
        ...
        (address refBy, uint available, uint total_earnings, uint total_direct_bonus, uint total_match_bonus,
        uint total_invested, uint last_payout, uint total_withdrawn) = oldC2.players(_userId);
    ...
    function moveV3data(address _userId, uint _earnings) internal returns(bool){
        (, address refBy) = oldC3.players(_userId);
        (uint available, uint total_invested, uint total_withdrawn,
        uint total_match_bonus, uint total_cashback, uint aff1sum) = oldC3.userInfo(_userId);
        if(_earnings > 0 || total_invested > 0){
            if(players[_userId].playerId == 0){
                _Register(_userId, refBy);
            }
            // PersonalDatatUpdate
            players[_userId].finances[0].available += available;
            players[_userId].finances[0].total_earnings += total_withdrawn.add(available);
            players[_userId].finances[0].total_direct_bonus += total_withdrawn.add(available).sub(total_match_bonus);
            players[_userId].finances[0].total_match_bonus += total_match_bonus;
            players[_userId].finances[0].total_invested += total_invested;
            players[_userId].finances[0].total_withdrawn += total_withdrawn;
            players[_userId].finances[0].total_cashback += total_cashback;
            players[_userId].refscount[0].aff1sum += aff1sum;
            players[_userId].finances[0].last_payout = now.sub(2 days);
            // Update Global Data
            invested += total_invested;
            earnings += total_withdrawn.add(available);
            withdrawn += total_withdrawn;
            direct_bonus += total_withdrawn.add(available).sub(total_match_bonus);
            match_bonus += total_match_bonus;
            // Add My Deposits
            players[_userId].deposits.push(DataStructs.Deposit({
                planId: _getPack(total_invested),
    ...
    function _syncingToggle() external onlyContract() returns(bool){
        syncing_ = !syncing_ ? true:false;
        return syncing_;
    }
