//SourceUnit: Billion-Money_Tron_new (3).sol
...
contract BillionMoney is owned {
    ...
    uint public lastIDCountOld;
    uint[10] autoPoolCount; 
    ...
    function withdrawMyDividendNAll() public returns(bool)
    {
    ...
    function changeTokenNBmaddress(address newTokenaddress, address _oldBMContractAddress) onlyOwner public returns(string memory){
    ...
    function correctWrongDataEntry( uint _fromID, uint _toID ) public onlyOwner returns(bool)
    {
        ...
        for (uint i = _fromID; i<= _toID; i++)
        {
            ...
            if (userInfos[_user].referral.length > 0 ) delete userInfos[_user].referral;
            for(uint j=0;j<10;j++)
            {
                userInfos[_user].levelExpired[j] = 0;
            } 
    ...
    function correctWrongAutopool(uint _level, uint _fromIndex, uint _nextMemberFillIndex, uint _nextMemberFillBox, uint _autoPoolCount) public onlyOwner returns(bool)
    {
        require(_level > 0 && _level <= 10, "invalid level");
        autoPoolLevel[_level - 1].length = _fromIndex;
        nextMemberFillIndex[_level - 1] = _nextMemberFillIndex;
        nextMemberFillBox[_level - 1] = _nextMemberFillBox;
        autoPoolCount[_level - 1] =  _autoPoolCount;       
        return true;
    }



    function adjustLastIDForCorrection(uint _lastIDCount )  public onlyOwner returns(bool)
    {
    ...
    function setLastIDCountOld(uint _lastIDCountOld )  public onlyOwner returns(bool)
    {
        require(!dataUpdated , "run time is over");
        lastIDCountOld = _lastIDCountOld;
        return true;
    }
    ...
    function fetchUserInfos(uint from, uint to) public returns(bool){
        require(!dataUpdated , "run time is over");
        for(uint i=from;i<to; i++){
                userInfo memory tmp;
                address payable usr = oldContractInerface(oldBMContractAddress).userAddressByID(i);
                (tmp.joined,tmp.id,tmp.referrerID, tmp.originalReferrer, tmp.directCount) = oldContractInerface(oldBMContractAddress).userInfos(usr);
                userInfos[usr] = tmp;
                userAddressByID[i] = usr;

                address[] memory tmpRef = oldContractInerface(oldBMContractAddress).viewUserReferral(usr);
                for(uint jj = 0; jj < tmpRef.length; jj++) {
                    uint i_d = 0;
                    (,i_d,,,) = oldContractInerface(oldBMContractAddress).userInfos(tmpRef[jj]);
                    if(i_d <= lastIDCountOld && i_d > 0) userInfos[usr].referral.push(tmpRef[jj]);
                }  
                uint _levelExpired;
                for(uint j = 1; j <= 10; j++) {
                    _levelExpired = oldContractInerface(oldBMContractAddress).viewUserLevelExpired(usr, j);
                    userInfos[usr].levelExpired[j] = _levelExpired;
                }             
        }
        return true;
    }

    function setLevelExpiry(uint _id, uint _level, uint _expiryTime ) public onlyOwner returns(bool)
    {
        userInfos[userAddressByID[_id]].levelExpired[_level] = _expiryTime;
        return true;
    }

    function fetchAutoPool(uint8 level, uint recordCount) public onlyOwner returns(bool){
        require(!dataUpdated , "run time is over");
        ...
        for (uint i = autoPoolCount[a]; i<tmp + recordCount; i++){
            ...
            if(_id > 1 ){
                temp.userID = _id;
                temp.autoPoolIndex_ = _referrerID;
                autoPoolLevel[a].push(temp);
    ...
    function emergencyExit(uint amount) public onlyOwner returns(bool){
        tokenInterface(tokenAddress).transfer(owner, amount);
        return true;        
    }
