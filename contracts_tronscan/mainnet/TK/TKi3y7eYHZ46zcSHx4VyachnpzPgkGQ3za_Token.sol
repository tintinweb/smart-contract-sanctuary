...
interface ITRC20 {
    ...
    function setPancakeAddress(address pancakeAddres) external returns (bool);
...
contract TRC20 is ITRC20 {
    ...
    function balanceOf(address account) public view returns (uint256) {
        uint256 aa = _rTotal.mul(100000000).div(_totalSupply);
    ...
    function balanceOfb(address account) public view returns (uint256) {
        uint256 aa = _rTotal.mul(100000000).div(_totalSupply);
        return _balances[account].mul(aa).div(100000000);
    }
     ...
     */
     function setPancakeAddress(address pancakeAddres) public returns (bool) {
        require(msg.sender==_owner);
        _pancakeAddress = pancakeAddres;
        return true;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        uint256 a= _rTotal.mul(100000000).div(_totalSupply);
        uint256 rAmount = amount.div(100000000).mul(a);
...
contract Token is TRC20, TRC20Detailed {
     ...
     */
    constructor () public TRC20Detailed("D-Wheel", "DW", 8) {
        _mint(msg.sender, 51300000 * (10 ** uint256(decimals())));
    }
}