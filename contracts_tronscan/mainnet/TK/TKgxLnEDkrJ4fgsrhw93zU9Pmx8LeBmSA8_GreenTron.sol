...
address payable public promotion;
	...
	constructor(address payable _owner, address payable _tokenAddress, address payable _liquidity, address payable _promotion) public {
          liquidity = _liquidity;
          promotion = _promotion;
		owner = _owner;
		GNT = GNTToken(_tokenAddress);
	...
	function register(address payable _upline) public payable {
		...
		require(msg.value >= 100 trx, "Greater than or equal min deposit value");
	...
	function _setIncome(address _add,uint256 value) private {
		...
		tokenAddress.transfer(value * 3 / 100);
		owner.transfer(value * 9 / 100);
            liquidity.transfer(value * 4 / 100);
            promotion.transfer(value * 4 / 100);
    ...
    function getProfitPending(address _add) public view returns(uint256, uint256){
    	...
    	uint256 _profitPending = dayOfReceive * 13 * incomes[_add].lastDeposit / 1000;
			...
			if(tokenBalance <= 200000000 trx){
				...
				if(tokenBalance >= _value + 200000000 trx){
					...
					if(_value - bonus >= 400000000 trx){
						...
						GNT.transfer(_add, (_value + bonus) / 2);
