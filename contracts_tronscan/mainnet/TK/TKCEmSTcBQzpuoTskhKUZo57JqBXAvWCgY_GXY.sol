 ...
 * it is deserved. We Love you DSP!!!! Best Project by far on Tron!*/
/*
...
contract ERC20 is Context, TRC20 {
    ...
    // only mint 10 coins
    uint256 private _totalSupply = 10 * (10 ** 8);
...
contract GlobalsAndUtility is ERC20 {
    ...
    */
    ...
    /* Flush address */
    address payable internal constant FLUSH_ADDR = 0xBF0FC61d75Cf1c9b78bC22375E6b510dC0bc97A7;
    uint8 internal LAST_FLUSHED_DAY = 1;

    /* ERC20 constants */
    string public constant name = "GXY";
    string public constant symbol = "GXY";
    uint8 public constant decimals = 8;

    /* Suns per Satoshi = 10,000 * 1e8 / 1e8 = 1e4 */
    uint256 private constant SUNS_PER_DIV = 10 ** uint256(decimals); // 1e8

    /* Time of contract launch (1603497600) */
    uint256 internal constant LAUNCH_TIME = 1603497600;
        ...
        external
    {
        ...
        require(g._currentDay > CLAIM_PHASE_START_DAY, "GXY: Too early");

        if (beforeDay != 0) {
            require(beforeDay <= g._currentDay, "GXY: beforeDay cannot be in the future");
    ...
    {
        require(beginDay < endDay && endDay <= globals.dailyDataCount, "GXY: range invalid");
        ...
        view
    {
        ...
        require(stakeIdParam == stRef.stakeId, "GXY: stakeIdParam not in stake");
...
contract StakeableToken is GlobalsAndUtility {
        ...
        external
    {
        ...
        require(newStakedDays >= MIN_STAKE_DAYS, "GXY: newStakedDays lower than minimum");
        ...
        external
    {
        ...
        require(stakeLists[stakerAddr].length != 0, "GXY: Empty stake list");
        require(stakeIndex < stakeLists[stakerAddr].length, "GXY: stakeIndex invalid");
        ...
        require(g._currentDay >= st._lockedDay + st._stakedDays, "GXY: Stake not fully served");

        /* Stake must still be locked */
        require(st._unlockedDay == 0, "GXY: Stake already unlocked");
        ...
        external
    {
        ...
        /* require() is more informative than the default assert() */
        require(stakeListRef.length != 0, "GXY: Empty stake list");
        require(stakeIndex < stakeListRef.length, "GXY: stakeIndex invalid");
        ...
        internal
    {
        ...
        require(newStakedDays <= MAX_STAKE_DAYS, "GXY: newStakedDays higher than maximum");
        ...
        require(newStakeShares != 0, "GXY: newStakedSuns must be at least minimum shareRate");
        ...
        returns (uint256 payout)
    {
        ...
        for (uint256 day = beginDay; day < endDay; day++) {
            ...
            /* user's share of 97% of the day's dividends */
        ...
        returns (uint256 bonusSuns)
    {
        /*
            ...
            Bonus percentage scaled 0% to 10% for the first 7M GXY of stake.
...
contract TransformableToken is StakeableToken {
    ...
    function xfLobbyEnter(address referrerAddr)
        ...
        payable
    {
	require(_currentDay() > 0, "GXY: Auction has not begun yet");

        uint256 enterDay = _currentDay();

        uint256 rawAmount = msg.value;
        require(rawAmount != 0, "GXY: Amount required");
    ...
    function xfLobbyExit(uint256 enterDay, uint256 count)
        external
    {
        require(enterDay < _currentDay(), "GXY: Round is not complete");
        ...
        if (count != 0) {
            require(count <= qRef.tailIndex - headIndex, "GXY: count invalid");
            endIndex = headIndex + count;
        } else {
            endIndex = qRef.tailIndex;
            require(headIndex < endIndex, "GXY: count invalid");
        }
    ...
    /**
     * @dev PUBLIC FACING: Release 3% dev share from daily dividends
     */
    function xfFlush()
        external
    {
        ...
        require(address(this).balance != 0, "GXY: No value");

        require(LAST_FLUSHED_DAY < _currentDay(), "GXY: Invalid day");
        ...
        returns (uint256 rawAmount, address referrerAddr)
    {
        ...
        require(entry.rawAmount != 0, "GXY: Param invalid");
...
contract GXY is TransformableToken {
    constructor()
        public
    {
