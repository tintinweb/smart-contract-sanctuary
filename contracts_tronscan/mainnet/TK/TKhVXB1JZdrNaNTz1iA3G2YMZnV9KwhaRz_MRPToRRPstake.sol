...
contract MRPToRRPstake {
    ...
    uint256 public investLevel1Amount = 5000 * 10**uint256(tokenDecimals);
    uint256 public investLevel2Amount = 10000 * 10**uint256(tokenDecimals);

    uint256 public referrerLevel1Amount = 125 * 10**uint256(2);
    uint256 public referrerLevel2Amount = 250 * 10**uint256(2);

    uint256 public withdrawLevel1Amount = 125 * 10**uint256(3);
    uint256 public withdrawLevel2Amount = 250 * 10**uint256(3);
    address private _burnPool;
    address public whitelist;
    address private owner;

    mapping(address => UserInfo) public userInfo;
    uint256 public constant intervalTime = 1 days;
    uint256 public maxDay = 40;
    ...
    constructor(address _tokenAddress, address rewardTokenAddress, address burnPool, address _whitelist) {
    ...
    function bindReferrer(address referrer) public returns(address) {
        ...
        // require(whitelist != msg.sender, "Can't bind whitelist");
        // require(userInfo[referrer].depositLevel1.start != 0 || userInfo[referrer].depositLevel2.start != 0, "Invalid account");
        UserInfo storage user = userInfo[msg.sender];
        require(userInfo[referrer].referrer != address(0) || referrer == whitelist);
        // require(msg.sender != whitelist && user.referrer != address(0), "error");
    ...
    function investLevel1(uint256 _amount) public payable {
        ...
        if (user.depositLevel1.start != 0) {
            require(user.depositLevel1.miningDay == 40, "Mining");
        }
    ...
    function investLevel2(uint256 _amount) public payable {
        ...
        if (user.depositLevel2.start != 0) {
            require(user.depositLevel2.miningDay == 40, "Mining");
        }
    ...
    function withdrawLevel1() public {
        ...
        require(user.depositLevel1.miningDay < 40, "Reward has been received");
        uint256 claimDay = getDayByTime(user.depositLevel1.lastaction);
        if (user.depositLevel1.miningDay.add(claimDay) > 40) {
        ...
        IERC20(_rewardTokenAddress).transfer(msg.sender, claimDay.mul(withdrawLevel1Amount));
        if (user.referrer != address(0)) {
            IERC20(_rewardTokenAddress).transfer(user.referrer, claimDay.mul(referrerLevel1Amount));
    ...
    function getWithdrawLevel2(address _address) public view returns(
        ...
        UserInfo storage user = userInfo[_address];
        require(user.depositLevel2.miningDay < 40, "Reward has been received");
        uint256 claimDay = getDayByTime(user.depositLevel2.lastaction);
        if (user.depositLevel2.miningDay.add(claimDay) > 40) {
    ...
    function withdrawLevel2() public {
        ...
        IERC20(_rewardTokenAddress).transfer(msg.sender, claimDay.mul(withdrawLevel2Amount));
        if (user.referrer != address(0)) {
            IERC20(_rewardTokenAddress).transfer(user.referrer, claimDay.mul(referrerLevel2Amount));
    ...
    function getContractInfo() public view returns(uint256 balance, uint256 totalwithdraw) {
        balance = IERC20(_rewardTokenAddress).balanceOf(address(this));
