...
contract Gold is TRC20 {
    ...
    uint256 public time1 = 3;
    uint256 public time2 = 3;
    ...
    function tweakDistributionTime(uint256 newDistributionTime1, uint256 newDistributionTime2) external {
        require(msg.sender == owner);
        require(newDistributionTime2 > 1 && newDistributionTime2 <= 1000000000);
        require(newDistributionTime1 > 1 && newDistributionTime1 <= 1000000000);
        
        time2 = newDistributionTime2;
        time1 = newDistributionTime1;
    }
    
    // Incase community prefers gold deposit payments over production %, can be tweaked for balance
    function tweakDailyDividends(uint256 newDistributionTime2, uint256 newDistributionTime1) external {
        require(msg.sender == owner);
        require(newDistributionTime2 > 0 && newDistributionTime2 <= 10000);
        require(newDistributionTime1 > 0 && newDistributionTime1 <= 10000);
        
        researchDivPercent = newDistributionTime2;
        goldDepositDivPercent = newDistributionTime1;
    }
    ...
    function buyTronUnit(uint256 unitId, uint256 amount) external payable SnapshotCheck {
        ...
        uint256 devFund = SafeMath.div(SafeMath.mul(tronCost, 8), 100);
    ...
    function snapshotDailyGoldResearchFunding() public {
        ...
        nextSnapshotTime = block.timestamp + 5 minutes;
    }
