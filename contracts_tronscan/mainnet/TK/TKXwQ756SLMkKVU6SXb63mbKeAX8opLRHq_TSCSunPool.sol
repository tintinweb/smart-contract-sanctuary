//SourceUnit: TSC_miner_token_relation.sol
...
contract TSCTokenPool is LPTokenWrapper, IRewardDistributionRecipient {
    ITRC20 public rewardToken = ITRC20(0x417d0a446d2465887b5a92728e44dded9148ce6f81);
    uint256 public constant DURATION = 5 days;

    IRelation public relationCtx = IRelation(0x41da1f36c8974daba7c323dfb4b511d917784e48a8); // ok
    ...
    function setRelationAddr(address addr) public onlyOwner returns (bool) {
        relationCtx = IRelation(addr);
        return true;
    }
    ...
    function getReward() public updateReward(msg.sender) checkStart {
        ...
        if (trueReward > 0) {
            ...
            if (address(0) != p) {
                ...
                relationCtx.incTsc(p, trueReward.mul(3).div(100));
                rate = rate.sub(3);
            }
            if (address(0) != gp) {
                rewardToken.safeTransfer(gp, trueReward.mul(2).div(100));
                relationCtx.incTsc(gp, trueReward.mul(2).div(100));
                rate = rate.sub(2);
            }
    ...
    {
        require(address(0) != address(rewardToken), "reward token not set");
...
contract TSCSunPool is TSCTokenPool {
    constructor() public{
        rewardDistribution = _msgSender();
        periodFinish = starttime.add(DURATION);
        rewardRate = 12500 * 1e18 / DURATION;
        tokenAddr = ITRC20(0x416b5151320359ec18b08607c70a3b7439af626aa3); // TKkeiboTkxXKJpbmVFbv4a8ov5rAfRDMf9
    }
}


contract TSCUsdtPool is TSCTokenPool {
    constructor() public{
        rewardDistribution = _msgSender();
        periodFinish = starttime.add(DURATION);
        rewardRate = 15000 * 1e18 / DURATION;
        tokenAddr = ITRC20(0x41a614f803b6fd780986a42c78ec9c7f77e6ded13c); // TR7NHqjeKQxGTCi8q8ZY4pL8otSzgjLj6t
    }
}


contract TSCBtcPool is TSCTokenPool {
    constructor() public{
        rewardDistribution = _msgSender();
        periodFinish = starttime.add(DURATION);
        rewardRate = 7500 * 1e18 / DURATION;
        tokenAddr = ITRC20(0x4184716914c0fdf7110a44030d04d0c4923504d9cc); // TN3W4H6rK2ce4vX9YnFQHwKENnHjoxb3m9
    }
}