...
contract Trethos {
    ...
    // divident
    uint public CONTRACT_BONUS_REACH = 1000000e6;
    uint public currentTotalDiv = 0;
    ...
    mapping(address => bool) public depositBonusReceived;
    ...
    function withdraw() external  {
        ...
        // activeDepositBonus
        if((users[msg.sender].payouts[users[msg.sender].currentCycle] < max_payout) && ((users[msg.sender].activeDepositBonus > 0) || (getUserDividends( msg.sender) > 0 ))) {
            
            if((users[msg.sender].activeDepositBonus == 0) && (getUserDividends( msg.sender) > 0 )) {
                users[msg.sender].activeDepositBonus = getUserDividends( msg.sender);
                depositBonusReceived[msg.sender] = true;
                emit activeDepositEvent(msg.sender, users[msg.sender].activeDepositBonus, now);
             }
    ...
    function getUserDividends( address _user) public view returns (uint divident){
        bool _isActive = isDepoistActive(_user);
        
        if((_isActive == false) || (currentTotalDiv < CONTRACT_BONUS_REACH) || (depositBonusReceived[_user])) { return 0; }
        
        divident = CONTRACT_BONUS_REACH.div(100);
    }
    
    function isDepoistActive( address _user) public view returns (bool){
        if(users[_user].cycle < users[_user].currentCycle) { return false; }
    ...
    function _deposit(address _addr, uint256 _amount) private {  // user deposit and pool money distribution
        ...
        if(currentTotalDiv < CONTRACT_BONUS_REACH) { 
            currentTotalDiv = currentTotalDiv.add(_amount);
        }
