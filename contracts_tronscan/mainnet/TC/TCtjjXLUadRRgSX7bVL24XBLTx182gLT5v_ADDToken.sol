...
contract ADDToken is Basic, SafeMath {
    string public name = "Main";         //  token name
    string public symbol = "Main";       //  token symbol
...
contract ADDTokenCrowdFund is Basic, SafeMath {
    IADDToken public addToken;
    ...
    constructor (address _addTokenAddress) public {
        owner = msg.sender;
        addToken = IADDToken(_addTokenAddress);
    }
    ...
    function tokenSale() saleNotFinished public payable {
        ...
        addToken.transferAndFreeze(msg.sender, newSale);
        if (saled >= saleGoal) {
    ...
    function withdraw(uint256 value, uint256 tokenValue) public isOwner {
        ...
        if (tokenValue > 0) {
            addToken.transfer(msg.sender, tokenValue);
        }
    }
