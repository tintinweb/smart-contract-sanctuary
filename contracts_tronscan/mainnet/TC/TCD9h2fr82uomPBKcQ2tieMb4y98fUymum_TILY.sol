...
library DataStructs {
    ...
    struct GlobalsCache {
        uint _lockedSunsTotal;
        uint _nextStakeSharesTotal;
        uint _shareRate;
        uint _stakePenaltyTotal;
        uint _dailyDataCount;
        uint _stakeSharesTotal;
    ...
    struct XfLobbyQueueStore {
        uint40 headIndex;
        uint40 tailIndex;
        mapping(uint => XfLobbyEntryStore) entries;
    }
}
...
contract TRC20 is Context, ITRC20 {
    ...
    uint private _totalSupply = 125000000 * (10 ** 8);
...
contract GlobalsAndUtility is TRC20 {
    /*  XfLobbyEnter
    */
    event XfLobbyEnter(
        uint timestamp,
    ...
    */
    ...
    /* Flush address */
    address payable internal FLUSH_ADDR;
    uint8 internal LAST_FLUSHED_DAY = 1;

    /* ERC20 constants */
    string public constant name = "TILY";
    ...
    /* Time of contract launch (2020-9-3T3:55:43Z) */
    uint internal constant LAUNCH_TIME = 1599148543;

    /* Start of claim phase */
    uint internal constant PRE_CLAIM_DAYS = 1;
    uint internal constant CLAIM_STARTING_AMOUNT = 5000000 * (10 ** 8);
    uint internal constant CLAIM_LOWEST_AMOUNT = 1000000 * (10 ** 8);
    uint internal constant CLAIM_PHASE_START_DAY = PRE_CLAIM_DAYS;

    /* Number of words to hold 1 bit for each transform lobby day */
    uint internal constant XF_LOBBY_DAY_WORDS = ((1 + (50 * 7)) + 255) >> 8;

    /* Stake timing parameters */
    uint internal constant MIN_STAKE_DAYS = 5;

    uint internal constant MAX_STAKE_DAYS = 5555; // Approx 15 years

    uint internal constant EARLY_PENALTY_MIN_DAYS = 90;

    uint private constant LATE_PENALTY_GRACE_WEEKS = 2;
    uint internal constant LATE_PENALTY_GRACE_DAYS = LATE_PENALTY_GRACE_WEEKS * 7;

    uint private constant LATE_PENALTY_SCALE_WEEKS = 100;
    uint internal constant LATE_PENALTY_SCALE_DAYS = LATE_PENALTY_SCALE_WEEKS * 7;

    /* Stake shares Longer Pays Better bonus constants used by _stakeStartBonusSuns() */
    uint private constant LPB_BONUS_PERCENT = 20;
    uint private constant LPB_BONUS_MAX_PERCENT = 200;
    uint internal constant LPB = 364 * 100 / LPB_BONUS_PERCENT;
    uint internal constant LPB_MAX_DAYS = LPB * LPB_BONUS_MAX_PERCENT / 100;

    /* Stake shares Bigger Pays Better bonus constants used by _stakeStartBonusSuns() */
    uint private constant BPB_BONUS_PERCENT = 10;
    uint private constant BPB_MAX_TILY = 7 * 1e6;
    uint internal constant BPB_MAX_SUNS = BPB_MAX_TILY * SUNS_PER_TILY;
    uint internal constant BPB = BPB_MAX_SUNS * 100 / BPB_BONUS_PERCENT;

    /* Share rate is scaled to increase precision */
    uint internal constant SHARE_RATE_SCALE = 1e5;

    /* Share rate max (after scaling) */
    uint internal constant SHARE_RATE_UINT_SIZE = 40;
    uint internal constant SHARE_RATE_MAX = (1 << SHARE_RATE_UINT_SIZE) - 1;

    /* weekly staking bonus */
    uint8 internal constant BONUS_DAY_SCALE = 2;

    DataStructs.GlobalsStore public globals;

    mapping(uint => DataStructs.DailyDataStore) public dailyData;

    mapping(address => DataStructs.StakeStore[]) public stakeLists;

    mapping(uint => uint) public xfLobby;
    mapping(uint => mapping(address => DataStructs.XfLobbyQueueStore)) public xfLobbyMembers;
     ...
     */
    function dailyDataUpdate(uint beforeDay)
    external
    {
        DataStructs.GlobalsCache memory g;
        DataStructs.GlobalsCache memory gSnapshot;
        _globalsLoad(g, gSnapshot);

        /* Skip pre-claim period */
        require(g._currentDay > CLAIM_PHASE_START_DAY, "TILY: Too early");

        if (beforeDay != 0) {
            require(beforeDay <= g._currentDay, "TILY: beforeDay cannot be in the future");
     ...
     */
    function dailyDataRange(uint beginDay, uint endDay)
    external
    view
    returns (uint[] memory _dayStakeSharesTotal, uint[] memory _dayPayoutTotal, uint[] memory _dayDividends)
    {
        require(beginDay < endDay && endDay <= globals.dailyDataCount, "TILY: range invalid");

        _dayStakeSharesTotal = new uint[](endDay - beginDay);
        _dayPayoutTotal = new uint[](endDay - beginDay);
        _dayDividends = new uint[](endDay - beginDay);

        uint src = beginDay;
        uint dst = 0;
        do {
            _dayStakeSharesTotal[dst] = uint(dailyData[src].dayStakeSharesTotal);
            _dayPayoutTotal[dst++] = uint(dailyData[src].dayPayoutTotal);
    ...
    function _dailyDataUpdateAuto(DataStructs.GlobalsCache memory g)
    internal
    {
        _dailyDataUpdate(g, g._currentDay);
    }

    function _globalsLoad(DataStructs.GlobalsCache memory g, DataStructs.GlobalsCache memory gSnapshot)
    internal
    view
    {
    ...
    function _globalsCacheSnapshot(DataStructs.GlobalsCache memory g, DataStructs.GlobalsCache memory gSnapshot)
    internal
    pure
    {
    ...
    function _globalsSync(DataStructs.GlobalsCache memory g, DataStructs.GlobalsCache memory gSnapshot)
    internal
    {
    ...
    function _stakeLoad(DataStructs.StakeStore storage stRef, uint40 stakeIdParam, DataStructs.StakeCache memory st)
    internal
    view
    {
        /* Ensure caller's stakeIndex is still current */
        require(stakeIdParam == stRef.stakeId, "TILY: stakeIdParam not in stake");
    ...
    function _stakeUpdate(DataStructs.StakeStore storage stRef, DataStructs.StakeCache memory st)
    internal
    {
    ...
    function _stakeAdd(
        DataStructs.StakeStore[] storage stakeListRef,
        uint40 newStakeId,
        uint newStakedSuns,
        uint newStakeShares,
        uint newLockedDay,
     ...
     */
    function _stakeRemove(DataStructs.StakeStore[] storage stakeListRef, uint stakeIndex)
    internal
    {
        uint lastIndex = stakeListRef.length - 1;
     ...
     */
    function _estimatePayoutRewardsDay(DataStructs.GlobalsCache memory g, uint stakeSharesParam)
    internal
    view
    returns (uint payout)
    {
        /* Prevent updating state for this estimation */
        DataStructs.GlobalsCache memory gTmp;
        _globalsCacheSnapshot(g, gTmp);

        DataStructs.DailyRoundState memory rs;
    ...
    function _dailyRoundCalc(DataStructs.GlobalsCache memory g, DataStructs.DailyRoundState memory rs)
    private
    pure
    {
    ...
    function _dailyRoundCalcAndStore(DataStructs.GlobalsCache memory g, DataStructs.DailyRoundState memory rs, uint day)
    private
    {
    ...
    function _dailyDataUpdate(DataStructs.GlobalsCache memory g, uint beforeDay)
    private
    {
...
contract StakeableToken is GlobalsAndUtility {
     ...
     */
    function stakeStart(uint newStakedSuns, uint newStakedDays)
    external
    {
        DataStructs.GlobalsCache memory g;
        DataStructs.GlobalsCache memory gSnapshot;
        ...
        require(newStakedDays >= MIN_STAKE_DAYS, "TILY: newStakedDays lower than minimum");
     ...
     */
    function stakeGoodAccounting(address stakerAddr, uint stakeIndex, uint40 stakeIdParam)
    external
    {
        DataStructs.GlobalsCache memory g;
        DataStructs.GlobalsCache memory gSnapshot;
        ...
        require(stakeLists[stakerAddr].length != 0, "TILY: Empty stake list");
        require(stakeIndex < stakeLists[stakerAddr].length, "TILY: stakeIndex invalid");

        DataStructs.StakeStore storage stRef = stakeLists[stakerAddr][stakeIndex];

        /* Get stake copy */
        DataStructs.StakeCache memory st;
        _stakeLoad(stRef, stakeIdParam, st);

        /* Stake must have served full term */
        require(g._currentDay >= st._lockedDay + st._stakedDays, "TILY: Stake not fully served");

        /* Stake must still be locked */
        require(st._unlockedDay == 0, "TILY: Stake already unlocked");
        ...
        (, uint payout, , uint penalty, uint cappedPenalty) = _stakePerformance(
            g,
            st,
            st._stakedDays
        );
     ...
     * @param stakeIdParam The stake's id
     */
    function stakeEnd(uint stakeIndex, uint40 stakeIdParam)
    external
    {
        DataStructs.GlobalsCache memory g;
        DataStructs.GlobalsCache memory gSnapshot;
        ...
        DataStructs.StakeStore[] storage stakeListRef = stakeLists[msg.sender];

        /* require() is more informative than the default assert() */
        require(stakeListRef.length != 0, "TILY: Empty stake list");
        require(stakeIndex < stakeListRef.length, "TILY: stakeIndex invalid");
    ...
    internal
    {
        ...
        require(newStakedDays <= MAX_STAKE_DAYS, "TILY: newStakedDays higher than maximum");

        uint bonusSuns = _stakeStartBonusSuns(newStakedSuns, newStakedDays);
        uint newStakeShares = (newStakedSuns + bonusSuns) * SHARE_RATE_SCALE / g._shareRate;

        /* Ensure newStakedSuns is enough for at least one stake share */
        require(newStakeShares != 0, "TILY: newStakedSuns must be at least minimum shareRate");
        ...
        */
        uint newLockedDay = g._currentDay + 1;
    ...
    returns (uint payout)
    {
        ...
        for (uint day = beginDay; day < endDay; day++) {
            uint dayPayout;
     ...
     * @param newStakedDays Number of days to stake
     */
    function _stakeStartBonusSuns(uint newStakedSuns, uint newStakedDays)
    private
    pure
    returns (uint bonusSuns)
    {

        uint cappedExtraDays = 0;
        ...
        uint cappedStakedSuns = newStakedSuns <= BPB_MAX_SUNS
        ? newStakedSuns
        : BPB_MAX_SUNS;
    ...
    function _stakeUnlock(DataStructs.GlobalsCache memory g, DataStructs.StakeCache memory st)
    private
    pure
    {
    ...
    function _stakePerformance(DataStructs.GlobalsCache memory g, DataStructs.StakeCache memory st, uint servedDays)
    private
    view
    returns (uint stakeReturn, uint payout, uint dividends, uint penalty, uint cappedPenalty)
    {
        if (servedDays < st._stakedDays) {
    ...
    function _calcPayoutAndEarlyPenalty(
        DataStructs.GlobalsCache memory g,
        uint lockedDayParam,
        uint stakedDaysParam,
        uint servedDays,
        uint stakeSharesParam
    )
    private
    view
    returns (uint payout, uint penalty)
    {
        uint servedEndDay = lockedDayParam + servedDays;

        /* 50% of stakedDays (rounded up) with a minimum applied */
        uint penaltyDays = (stakedDaysParam + 1) / 2;
        ...
        if (servedDays == 0) {
            ...
            uint expected = _estimatePayoutRewardsDay(g, stakeSharesParam);
        ...
        if (penaltyDays < servedDays) {
            ...
            */
            uint penaltyEndDay = lockedDayParam + penaltyDays;
            penalty = _calcPayoutRewards(stakeSharesParam, lockedDayParam, penaltyEndDay);

            uint delta = _calcPayoutRewards(stakeSharesParam, penaltyEndDay, servedEndDay);
    ...
    returns (uint)
    {
        ...
        uint maxUnlockedDay = lockedDayParam + stakedDaysParam + LATE_PENALTY_GRACE_DAYS;
    ...
    function _shareRateUpdate(DataStructs.GlobalsCache memory g, DataStructs.StakeCache memory st, uint stakeReturn)
    private
    {
        if (stakeReturn > st._stakedSuns) {
            ...
            */
            uint bonusSuns = _stakeStartBonusSuns(stakeReturn, st._stakedDays);
            uint newShareRate = (stakeReturn + bonusSuns) * SHARE_RATE_SCALE / st._stakeShares;
...
contract TransformableToken is StakeableToken {
    ...
    payable
    {
        uint enterDay = _currentDay();

        uint rawAmount = msg.value;
        require(rawAmount != 0, "TILY: Amount required");

        DataStructs.XfLobbyQueueStore storage qRef = xfLobbyMembers[enterDay][msg.sender];

        uint entryIndex = qRef.tailIndex++;

        qRef.entries[entryIndex] = DataStructs.XfLobbyEntryStore(uint96(rawAmount), referrerAddr);
     ...
     */
    function xfLobbyExit(uint enterDay, uint count)
    external
    {
        require(enterDay < _currentDay(), "TILY: Round is not complete");

        DataStructs.XfLobbyQueueStore storage qRef = xfLobbyMembers[enterDay][msg.sender];

        uint headIndex = qRef.headIndex;
        uint endIndex;

        if (count != 0) {
            require(count <= qRef.tailIndex - headIndex, "TILY: count invalid");
            endIndex = headIndex + count;
        } else {
            endIndex = qRef.tailIndex;
            require(headIndex < endIndex, "TILY: count invalid");
        }

        uint waasLobby = _waasLobby(enterDay);
        uint _xfLobby = xfLobby[enterDay];
        uint totalXfAmount = 0;

        do {
            uint rawAmount = qRef.entries[headIndex].rawAmount;
            address referrerAddr = qRef.entries[headIndex].referrerAddr;

            delete qRef.entries[headIndex];

            uint xfAmount = waasLobby * rawAmount / _xfLobby;
     ...
     * @return Fixed array of values
     */
    function xfLobbyRange(uint beginDay, uint endDay)
    external
    view
    returns (uint[] memory list)
    {
        require(
        ...
        list = new uint[](endDay - beginDay);

        uint src = beginDay;
        uint dst = 0;
        do {
            list[dst++] = uint(xfLobby[src++]);
        } while (src < endDay);
    ...
    external
    {
        require(address(this).balance != 0, "TILY: No value");

        require(LAST_FLUSHED_DAY < _currentDay(), "TILY: Invalid day");
     ...
     * @param enterDay ssss
     * @param entryIndex ssss
     * @return 1: Raw amount that was entered with; 2: Referring TRX addr (optional; 0x0 for no referrer)
     */
    function xfLobbyEntry(address memberAddr, uint enterDay, uint entryIndex)
    external
    view
    returns (uint rawAmount, address referrerAddr)
    {
        DataStructs.XfLobbyEntryStore storage entry = xfLobbyMembers[enterDay][memberAddr].entries[entryIndex];

        require(entry.rawAmount != 0, "TILY: Param invalid");
    ...
    external
    view
    returns (uint[XF_LOBBY_DAY_WORDS] memory words)
    {
        uint day = _currentDay() + 1;
    ...
    function _waasLobby(uint enterDay)
    private pure
    returns (uint waasLobby)
    {
...
contract TILY is TransformableToken {
    constructor()
    public
    {
