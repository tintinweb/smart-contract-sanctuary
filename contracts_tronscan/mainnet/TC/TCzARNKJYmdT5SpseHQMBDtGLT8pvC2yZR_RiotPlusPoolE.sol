//SourceUnit: RiotPoolE.sol

pragma solidity 0.5.12;

contract RiotPlusPoolE {
	...
	function withdraw() public {
		...
		for (uint256 i = 0; i < user.deposits.length; i++) {
			...
			if (user.deposits[i].withdrawn < user.deposits[i].amount.mul(1260).div(1000)) {
				...
				if (user.deposits[i].withdrawn.add(dividends) > user.deposits[i].amount.mul(1260).div(1000)) {
					dividends = (user.deposits[i].amount.mul(1260).div(1000)).sub(user.deposits[i].withdrawn);
				}
	...
	function getUserPercentRate(address userAddress) public view returns (uint256) {
		...
		if (isActive(userAddress)) {
			uint256 timeMultiplier = (now.sub(user.checkpoint)).div(TIME_STEP).mul(100);
	...
	function getUserPercentRateByStartedAt(address userAddress,uint256 startedAt) public view returns (uint256) {
		...
		if (isActive(userAddress)) {
			...
			uint256 timeMultiplier = (now.sub(startedAt)).div(TIME_STEP).mul(100);
			return contractBalanceRate.add(timeMultiplier);
		} else {
			return contractBalanceRate;
		}
	...
	function getUserDepositAvgRate(address userAddress) public view returns (uint256) {
		...
		if (percentDividend > 1600) {
			percentDividend = 1600;
		}
	...
	function isActive(address userAddress) public view returns (bool) {
		...
		if (user.deposits.length > 0) {
			if (user.deposits[user.deposits.length-1].withdrawn < user.deposits[user.deposits.length-1].amount.mul(1260).div(1000)) {
				return true;
			}
		}
	}
