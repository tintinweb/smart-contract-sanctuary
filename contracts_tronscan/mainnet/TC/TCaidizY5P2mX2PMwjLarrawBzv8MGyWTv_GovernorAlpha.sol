...
contract GovernorAlpha {
    ...
    string public constant name = "APENFT Governor Alpha";

    /// @notice The number of votes in support of a proposal required in order for a quorum to be reached and for a vote to succeed
    function quorumVotes() public pure returns (uint) {return 19999800000000e6;} // 19,999,800,000,000 = 2% of NFT(999,990,000,000,000)

    /// @notice The number of votes required in order for a voter to become a proposer
    function proposalThreshold() public pure returns (uint) {return 4999950000000e6;} // 4,999,950,000,000 = 0.5% of NFT(999,990,000,000,000)
    ...
    constructor(address wnft_, address guardian_) public {
        wnft = WNFTInterface(wnft_);
        guardian = guardian_;
    }
...
//    }
    ...
    function propose(address[] memory targets, uint[] memory values, string[] memory signatures, bytes[] memory calldatas, string memory description) public returns (uint) {
        require(wnft.getPriorVotes(msg.sender, sub256(block.number, 1)) >= proposalThreshold(), "GovernorAlpha::propose: proposer votes below proposal threshold");
    ...
    function cancel(uint proposalId) public {
        ...
        Proposal storage proposal = proposals[proposalId];
        require(msg.sender == guardian || wnft.getPriorVotes(proposal.proposer, sub256(block.number, 1)) < proposalThreshold(), "GovernorAlpha::cancel: proposer above threshold");
    ...
    function _castVote(address voter, uint proposalId, uint votes, bool support) internal {
        ...
        //uint96 votes = wnft.getPriorVotes(voter, proposal.startBlock);
        require(votes <= wnft.getPriorVotes(voter, proposal.startBlock), "GovernorAlpha::_castVote: short of vote power");
        if (support) {
        ...
        // burn 
        require(wnft.voteFresh(voter, proposalId, support, votes), "wnft vote exception");
