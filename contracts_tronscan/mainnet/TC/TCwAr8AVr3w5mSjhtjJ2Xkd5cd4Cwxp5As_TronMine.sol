...
contract TronMine {
    ...
    modifier onlyOwner() {
        require(ownerWallet == msg.sender, "Only Owner");
        _;
    }
    ...
    function updatePackagePrice(uint _package, uint _price) public onlyOwner returns(bool) {
        require(_package > 0, "_package must not be zero");
        require(_price > 0, "_price must not be zero");
        packages[_package].packagePrice =  _price;
        
        return true;
    }
    
    function updateCeilingPrice(uint _package, uint _price) public onlyOwner returns(bool) {
        require(_package > 0, "_package must not be zero");
        require(_price > 0, "_price must not be zero");
        packages[_package].celling =  _price;
        
        return true;
    }
    
    function updateMinimumTopUp( uint _minimumTopUp) public onlyOwner returns(bool){
        require(_minimumTopUp > 0, "_minimumTopUp must not be zero");
        minimumTopUp = _minimumTopUp;
        
        return true;
    }
    
    function updateRewardValue( uint _rewardValue) public onlyOwner returns(bool){
        require(_rewardValue > 0, "_rewardValue must not be zero");
