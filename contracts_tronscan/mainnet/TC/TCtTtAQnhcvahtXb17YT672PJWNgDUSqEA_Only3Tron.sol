//SourceUnit: Only3Tron.sol

pragma solidity 0.5.10;

interface IOnly3Tron {
    enum Overflow {
        DOWN,
        DIRECT,
        UP,
...
contract Only3Tron is IOnly3Tron {
    struct Level {
        bool active;
 ...
 levels.push(i > 0 ? (levels[i - 1] * (i > 6 ? 2 : 2)) : 3e6);
    ...
    function levelPriceWithComm(uint8 _level) view external returns(uint256) {
        return levels[_level] + (levels[_level] / 100 * 10);
    }
