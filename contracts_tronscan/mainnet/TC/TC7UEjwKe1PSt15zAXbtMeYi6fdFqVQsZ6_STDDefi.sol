...
contract STDDefi {
    ...
    address payable public          stdchain_fund;
    ...
    constructor(address payable _owner) public {
        ...
        stdchain_fund = address(0x4102646988408635DFC6960E65C5193DD280B4C24A);
    ...
    function _deposit(address _addr, uint256 _amount) private {
        ...
        stdchain_fund.transfer(_amount * 1 / 100);  // 1%
    ...
    function _tokenRewards(address _addr, uint256 _amount, uint256 _total_deposited) private {
        ...
        if(_total_deposited > 0 && _total_deposited <= 1e15) {//10 y
            _transferTokenSTD(_addr, _amount/10 * 1);// * 1e6
        } else if(_total_deposited > 1e15 && _total_deposited <= 2e15) {
            _transferTokenSTD(_addr, _amount/20 * 2);
        } else if(_total_deposited > 2e15 && _total_deposited <= 3e15) {
            _transferTokenSTD(_addr, _amount/40 * 3);
        } else if(_total_deposited > 3e15 && _total_deposited <= 4e15) {
            _transferTokenSTD(_addr, _amount/80 * 4);
        } else if(_total_deposited > 4e15 && _total_deposited <= 5e15) {
            _transferTokenSTD(_addr, _amount/160 * 5);
        } else if(_total_deposited > 5e15 && _total_deposited <= 6e15) {
            _transferTokenSTD(_addr, _amount/320 * 6);
        } else if(_total_deposited > 6e15 && _total_deposited <= 7e15) {
             _transferTokenSTD(_addr, _amount/640 * 7);
        } else if(_total_deposited > 7e15 && _total_deposited <= 8e15) {
            _transferTokenSTD(_addr, _amount/128 * 8);
        } else if(_total_deposited > 8e15 && _total_deposited <= 9e15) {
            _transferTokenSTD(_addr, _amount/256 * 9);
        } else if(_total_deposited > 9e15 && _total_deposited <= 1e16) {
            _transferTokenSTD(_addr, _amount/512 * 10);
        }
    }
    
    function _transferTokenSTD(address _addr,uint256 _amount)  private {
        require(_amount > 0, "STD Less Zero");

        address STDAddress = address(0x41DB488D58240A5B491AF749683C26AA407752B964);
        TokenSTD token = TokenSTD(STDAddress); //STD
