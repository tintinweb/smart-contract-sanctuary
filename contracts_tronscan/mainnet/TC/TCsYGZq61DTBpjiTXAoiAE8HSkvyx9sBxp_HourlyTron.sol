...
contract HourlyTron {
            ...
            if(players[msg.sender].deposits[pa].amount > 0){
                ...
                for(uint i=0 ; i< pos;i++){
                    ...
                    if(players[msg.sender].deposits[r].amount ==0){
                        players[msg.sender].deposits[r] = Deposit(msg.value,now,now+150 hours,0);
                        return true;
                    }
                    t++;
                }
            }else{
                players[msg.sender].deposits[pa] = Deposit(msg.value,now,now+150 hours,0);
                return true;
            }
        }else{
            players[msg.sender].deposits[p] = Deposit(msg.value,now,now+150 hours,0);
            return true;
        }
    }
    function withdraw(uint pos) public returns(uint256){
        ...
        uint256 total = getPercent(player.deposits[pos].amount,75)*2;
