...
contract TronLian is Ownership {
    using SafeMath
    for uint256;
    ...
    constructor(address toper, address payable drawaddress) public {
        ...
        freerate = [300, 100, 100, 100, 100, 80, 80, 80, 80, 80, 50, 50, 50, 50, 50];
    ...
    {
        require(users[uaddr].level > 0, "user is not register");
    ...
    function setInvestRate(uint64 rate) external onlyOwner returns(uint64) {
        investrate = rate;
        return investrate;
    }    
    ...
    function invest() public payable isOn returns(uint256) {
        ...
        require(_amount.mod(500 trx) == 0, "500 integral times");
        ...
        if (upaddr != address(0)) {
            ...
            bool isout = false;
            UData storage upInfo  = users[user];

            if (upInfo.valid == 1) {
                if (upInfo.amount < upInfo.reward) {
                    freereward = 0;
                    isout = true;
                } else {
                    freereward = _amount.mul(investrate).div(1000);
                    if (freereward > upInfo.amount.sub(upInfo.reward)) {
                        freereward = upInfo.amount.sub(upInfo.reward);
                        isout = true;
                    }
                }

                freeUpdate(upaddr, freereward, 2, isout);
