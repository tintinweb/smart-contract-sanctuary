...
contract Troncraft {
    ...
    constructor(address payable _owner,address payable _fee) public {
        owner = _owner;
        admin_fee = _fee;
    ...
    function _setUpline(address _addr, address _upline) private {
        if(users[_addr].upline == address(0) && _upline != _addr && _addr != owner && (users[_upline].deposit_time > 0 || _upline == owner || _upline == admin_fee )) {
    ...
    function _deposit(address _addr, uint256 _amount) private {
        require(users[_addr].upline != address(0) || _addr == owner || _addr == admin_fee, "No upline");
    ...
    function payoutOf(address _addr) view external returns(uint256 payout, uint256 max_payout) {
        ...
        if(users[_addr].deposit_payouts < max_payout) {
            payout = (users[_addr].deposit_amount * ((block.timestamp - users[_addr].deposit_time) / 12 hours) / 100) - users[_addr].deposit_payouts;
