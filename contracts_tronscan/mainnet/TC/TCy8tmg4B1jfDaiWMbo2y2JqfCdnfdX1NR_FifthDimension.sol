...
contract FifthDimension is Util, WhitelistAdminRole {

    using SafeMath for *;

    string constant private name = "Fifth Dimension";
        ...
        payable
    {
        ...
        //
        uint limitAmount = value.mul(23).div(10);
    ...
    function statisticOfChampion(address _sender,uint _value) private {
        //pk 3%
        bigRound[bigRid].gameAmount += _value.mul(3).div(100);
    ...
    function unbalancedStaticProfit(address userAddr) public view returns(uint)
    {
        ...
        for (uint i = user.staticFlag; i < user.invests.length; i++) {
            ...
            if (earnAmount >= invest.limitAmount) {
                uint xiuzheng = earnAmount - invest.limitAmount;
                if(xiuzheng > 0){
                    allStatic = allStatic.sub(xiuzheng);
