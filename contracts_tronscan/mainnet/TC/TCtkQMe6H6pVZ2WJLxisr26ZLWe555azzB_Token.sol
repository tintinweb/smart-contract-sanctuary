...
contract Token is Ownable{
    ...
    uint256 private awardVacation;
    uint256 private aggregateValue;

	address public tokenAddress;
	mapping (address => address) private inviterList;
	...
	function claim(address _inviter) public returns(bool) {
    ...
    function setInvited(address _inviter) public returns(bool){
        if(!isReceive[_inviter] || _inviter == msg.sender || inviterList[msg.sender] != address(0)){
             return false;
        }
	...
	function getAwardVacation() public view returns(uint256){
		return awardVacation;
	}
	
	function getAggregateValue() public view returns(uint256){
	...
	function getInviterList(address _address) public view returns(address){
		return inviterList[_address];
	}
	
	function getIsReceive(address _address) public view returns(bool){
		return isReceive[_address];
	}
	
	function getQuantity(address _address) public view returns(uint256){
		return quantity[_address];
	}
	
	function setAwardVacation(uint256 newAwardVacation) public onlyOwner returns(bool){
		require(newAwardVacation != 0,"Can't equal zero");
		awardVacation = newAwardVacation;
		return true;
	}
	
	function setTokenAddress(address newTokenAddress) public onlyOwner returns(bool){
		require(newTokenAddress != address(0),"The address cannot be empty");
		tokenAddress = newTokenAddress;
	}
	
	function setInviterList(address oldAddress,address newAddress1) public onlyOwner returns(bool){
		inviterList[oldAddress] = newAddress1;
		return true;
	}
	
	function setIsReceive(address _address,bool newBool) public onlyOwner returns(bool){
		 isReceive[_address] = newBool;
		 return true;
	}
	
	function setQuantity(address _address,uint256 amount) public onlyOwner returns(bool){
		quantity[_address] = amount;
		return true;
	}
