//SourceUnit: tronrewardLiveContract.sol
...
contract TronReward {
    ...
    constructor(address payable _owner, address payable _maintanence, address payable _admin_fee) public {
        owner = _owner;
        
        maintanence = _maintanence;
    ...
    function _deposit(address _addr, uint256 _amount) private {
        ...
        maintanence.transfer(_amount * 3 / 100);
    ...
    function withdraw() external {
        ...
        require(msg.sender.send(to_payout),"transfer failed");
    ...
    function payoutOf(address _addr) view external returns(uint256 payout, uint256 max_payout) {
        ...
        if(users[_addr].deposit_payouts < max_payout) {
            payout =  ((users[_addr].deposit_amount*(1.5 trx)/(100 trx)) * ((block.timestamp - users[_addr].deposit_time) / 1 days)) - users[_addr].deposit_payouts;
