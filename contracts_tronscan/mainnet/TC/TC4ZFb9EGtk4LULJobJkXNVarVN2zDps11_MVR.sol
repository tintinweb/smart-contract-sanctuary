...
contract MVR is EIP20Interface,Ownable {
    ...
    function _transfer(address _from, address _to, uint256 _value) private {
        ...
        if(stopFlag){
            ...
            //deduction fee
            uint256 _fee = _value.div(1000).mul(25);
            ...
            uint256 _feeReward = _fee.div(100).mul(50);
            ...
            if(lpContractAddress!=address(0)){
                balances[lpContractAddress] = balances[lpContractAddress].add(_feeBurn);
            }else{
            ...
            balances[bonusAddress] = balances[bonusAddress].add(_feeReward);
            emit Transfer(_from, burnAddress, _feeBurn);
            emit Transfer(_from, bonusAddress, _feeReward);
        }
    }
    function balanceOf(address _owner) public view returns (uint256 balance) {
    ...
    function setBonusAddress(address _address) public onlyOwner returns (bool success) {
