//SourceUnit: B001_all_in_one_v3.sol
...
contract YMIR_2 is TRC20Detailed, TRC20 {
    using Address for address;
    using SafeMath for uint;
    ...
    // offer logic
    uint public BuyStartTime = 1608681600; // 12/23/2020 @ 12:00am (UTC)
    ...
    function setDev(address addr, address addr2) public onlyOwner returns (bool) {
    ...
    function buy(uint inviterID, uint amount) public payable returns (bool) {
        ...
        uint devAmount = amount.mul(87).div(100);
        usdtToken.transferFrom(msg.sender, dev_, devAmount);
        usdtToken.transferFrom(msg.sender, partner_, amount.sub(devAmount));
    ...
    function setGameFlag(bool flag) public onlyOwner returns (bool) {
        GameStart = flag;
        return true;
    }
    ...
    function getPieces(uint amount, uint seed) public gameStart returns (bool) {
    ...
    function getCard(uint cardID, uint cardCnt) public gameStart returns (bool) {
        playCnt_++;
        uint uid = getUID(msg.sender);
        require(uid > 0, "invalid user");
    ...
    function upgradeCard(uint cardID, uint cardCnt) public gameStart returns (bool) {
        playCnt_++;
        uint uid = getUID(msg.sender);
        require(uid > 0, "invalid user");
    ...
    function sellCard(uint cardID, uint cardCnt) public gameStart returns (bool) {
        playCnt_++;
        uint uid = getUID(msg.sender);
        require(uid > 0, "invalid user");
    ...
    function setStakeFlag(bool flag) public onlyOwner returns (bool) {
        StakeStart = flag;
        if (StakeStart) {
    ...
    modifier stakeStart {
        require(StakeStart, "stake not start");
        _;
    }
    ...
    function stake(uint inviterID, uint amount) public stakeStart returns (bool) {
    ...
    function unstake(uint amount) public stakeStart returns (bool) {
        uint uid = getUID(msg.sender);
        require(uid > 0, "invalid user");
    ...
    uint public GetRewardInterval = 3 days;

    function getReward() public stakeStart returns (bool) {
        uint uid = getUID(msg.sender);
        require(uid > 0, "invalid user");
        
        Objects.User storage user = uid2User_[uid];
        require(block.timestamp >= user.lastWithdrawTime.add(GetRewardInterval), "invalid getReward time");
    ...
    uint public ProfitCycle = 1 days;
    function calcProfit(uint amount, uint startTime) internal view returns (uint) {
        ...
        return amount.mul(price).mul(duration).div(ProfitCycle).div(1000);
    }
