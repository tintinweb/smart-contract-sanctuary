//SourceUnit: autorider_bulleto1.sol
...
contract EcoBull_9x10x is owned {
    ...
    event regLevelEv(uint128 _userid, uint128 indexed _userID, uint128 indexed _referrerID, uint _time, address _refererWallet, address _userWallet);
    ...
    constructor(address payable ownerAddress, address payable ID1address) public {
        ...
        emit regLevelEv(userInfos[ownerWallet].id, 1, 0, now, address(this), ownerAddress);
    ...
    //update variables
        ...
        emit regLevelEv(userInfos[msgSender].id, lastIDCount, _referrerID, now,userAddressByID[_referrerID], msgSender);
