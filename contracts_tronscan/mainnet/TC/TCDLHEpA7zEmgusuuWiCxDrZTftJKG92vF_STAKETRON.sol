...
contract STAKETRON {
	...
	uint256 constant public INVEST_MIN_AMOUNT = 100e6; // 100TRX
	uint256 constant public BASE_PERCENT = 1000; //10 PERCENT BASE
	uint256[] public REFERRAL_PERCENTS = [500, 500, 500, 500 ,500]; //5LEVELS
	uint256 constant public PERCENTS_DIVIDER = 10000;
	uint256 constant public CONTRACT_BALANCE_STEP = 50000e6 trx; //50K
	...
	function invest(address referrer) public payable {
        ...
        owner.transfer(msg.value.mul(5).div(100));    //5 PERCENT
	...
	function withdraw() public {
		...
		for (uint256 i = 0; i < user.deposits.length; i++) {
			...
			if (user.deposits[i].withdrawn < user.deposits[i].amount.mul(5).div(2)) {  //DEV5%
				...
				if (user.deposits[i].withdrawn.add(dividends) > user.deposits[i].amount.mul(5).div(2)) {   //dev5%
...
function Reinvest(uint256 _value) internal {
		owner.transfer(_value.mul(5).div(100));
		User storage user = users[msg.sender];
    ...
    function getContractBonus() public view returns (uint256) {
		uint256 contractBalancePercent = maxBalance.div(CONTRACT_BALANCE_STEP).mul(1000);
		if (contractBalancePercent > 1000) {
			contractBalancePercent = 1000;
		}
    ...
    function getUserHoldBonus(address userAddress) public view returns (uint256) {
		...
		if (isActive(userAddress)) {
			uint256 holdBonus = (now.sub(user.checkpoint)).div(TIME_STEP).mul(200);
			if (holdBonus > 200) {
				holdBonus = 200;
			}
			return holdBonus;
