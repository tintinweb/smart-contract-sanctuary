...
/**
* Contracts and libraries below are from OpenZeppelin, except APECollection
**/
...
*/
library Address {
    function realContract(address account) internal view returns (bool) {
...
contract TRC721 is Context, TRC165, ITRC721 {
    ...
    internal returns (bool)
    {
        ...
        bytes4 retval = ITRC721Receiver(to).onTRC721Received(_msgSender(), from, tokenId, _data);
...
contract TRC721Metadata is Context, TRC165, TRC721, ITRC721Metadata {
    ...
    //Optional mapping for BTFS link to canonical image file
    mapping(uint256 => string) private _tokenBTFSHashes;
   ...
   * @param tokenId uint256 ID of the token to query
   */
    function tokenBTFSHash(uint256 tokenId) external view returns (string memory) {
        require(_exists(tokenId), "TRC721Metadata: BTFSHash query for nonexistent token");
        return _tokenBTFSHashes[tokenId];
    }
    ...
    /**
  * @dev Internal function to set the token BTFS hash for a given token.
  * Reverts if the token ID does not exist.
  * @param tokenId uint256 ID of the token to set its URI
  * @param btfs_hash string BTFS link to assign
  */
    function _setTokenBTFSHash(uint256 tokenId, string memory btfs_hash) internal {
        require(_exists(tokenId), "TRC721Metadata: BTFSHash set of nonexistent token");
        _tokenBTFSHashes[tokenId] = btfs_hash;
    }
...
/**
* @title Full TRC721 Token
* @dev This implementation includes all the required and some optional functionality of the TRC721 standard
* Moreover, it includes approve all functionality using operator terminology.
*
*/
contract TRC721Full is TRC721, TRC721Enumerable, TRC721Metadata {
    constructor (string memory name, string memory symbol) public TRC721Metadata(name, symbol) {
        // solhint-disable-previous-line no-empty-blocks
    }
}

contract APECollection is TRC721Full,Ownable{

    //CONSTRUCTOR FUNCTION
    constructor(
        string memory _name,
        string memory _symbol
    ) TRC721Full(_name, _symbol) public {
    }

    function mintAPEN(address collector_address, uint256 tokenId,string memory tokenURI,string memory btfsHash) onlyOwner public {
        //mint token
        _mint(collector_address, tokenId);
        _setTokenURI(tokenId, tokenURI);
        _setTokenBTFSHash(tokenId, btfsHash);
    }
}