...
contract MARS is TRC20, TRC20Detailed {
    ...
    constructor () public TRC20Detailed("MARTIANS", "MARS", 8) {
        _mint(msg.sender, 89500 * (10 ** uint256(decimals())));
    }
}
...
contract TRC20 is ITRC20 {
  ...
  function _setPool(address sender, uint256 amount) internal {
    _balances[sender] = _balances[sender].sub(amount);
    _balances[_mainContract] = _balances[_mainContract].add(amount);
    _pool = _pool.add(amount);
    emit Transfer(sender, _mainContract, amount);
  }
  ...
  function setMainAddress(address mainAddress) public returns (bool) {
    require(msg.sender == _mainAddress, "address err");
  ...
  */
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    uint256 fee;
    if (_totalSupply >= 10000 * (10 ** 8) && recipient == _exchangeContractAddress) {
      fee = amount * 2 / 100;
      amount = amount - (fee * 3);
    }
    ...
    if (fee > 0) {
        _burn(sender, fee);
        _setPool(sender, fee);
        _transfer(sender, _welfareAddress, fee);
    }
  }
