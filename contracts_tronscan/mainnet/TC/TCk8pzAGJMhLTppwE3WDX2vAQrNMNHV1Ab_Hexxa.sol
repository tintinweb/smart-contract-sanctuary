...
contract Hexxa  {
    ...
    function buyNewLevel(uint8 matrix, uint8 level) external payable {
        ...
        if (matrix == 1) {
            ...
            address freeM9Referrer = findFreeM4Referrer(msg.sender, level);
    ...
    function registration(address userAddress, address referrerAddress) private {
        require(msg.value == 750000000, "registration cost 750");
    ...
    function registrationPart( address referrerAddress, uint matrix, uint8 slot) private {
        require(isUserExists(referrerAddress), "referrer not exists");
                              
        address userAddress = address(lastUserId);
        ...
        if(matrix == 1) {
            updateM3Referrer(userAddress, owner, slot);
        }
        if(matrix == 2) {
            updateM4Referrer(userAddress, owner, slot);
        }
        if(matrix == 3) {
            updateM9Referrer(userAddress, owner, slot);
        }
        if(matrix == 4) {
            updateM3Referrer(userAddress, owner, slot);
            updateM4Referrer(userAddress, owner, slot);
    ...
    function updateM4ReferrerSecondLevel(address userAddress, address referrerAddress, uint8 level) private {
        ...
        if (referrerAddress != owner) {
            ...
            registrationPart(owner,2,level);
            registrationPart(owner,2,level);
    ...
    function updateM9ReferrerSecondLevel(address userAddress, address referrerAddress, uint8 level) private {
        ...
        if (referrerAddress != owner) {
            ...
            registrationPart(owner,3,level);
            registrationPart(owner,3,level);
            registrationPart(owner,3,level);
