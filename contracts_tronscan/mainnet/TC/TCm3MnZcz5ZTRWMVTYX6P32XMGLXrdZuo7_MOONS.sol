...
contract GlobalsAndUtility is ERC20 {
    ...
    */
    ...
    /* Flush address */
    address payable internal constant FLUSH_ADDR = 0xdda7A2cc482e68C02E60f06AD3204BDc2c5C9e2b;
    uint8 internal LAST_FLUSHED_DAY = 1;

    /* ERC20 constants */
    string public constant name = "MOONS";
    string public constant symbol = "MOONS";
    uint8 public constant decimals = 8;

    /* Suns per Satoshi = 10,000 * 1e8 / 1e8 = 1e4 */
    uint256 private constant SUNS_PER_DIV = 10 ** uint256(decimals); // 1e8

    /* Time of contract launch (1603843200) */
    uint256 internal constant LAUNCH_TIME = 1603843200;
        ...
        external
    {
        ...
        require(g._currentDay > CLAIM_PHASE_START_DAY, "MOONS: Too early");

        if (beforeDay != 0) {
            require(beforeDay <= g._currentDay, "MOONS: beforeDay cannot be in the future");
    ...
    {
        require(beginDay < endDay && endDay <= globals.dailyDataCount, "MOONS: range invalid");
        ...
        view
    {
        ...
        require(stakeIdParam == stRef.stakeId, "MOONS: stakeIdParam not in stake");
...
contract StakeableToken is GlobalsAndUtility {
        ...
        external
    {
        ...
        require(newStakedDays >= MIN_STAKE_DAYS, "MOONS: newStakedDays lower than minimum");
        ...
        external
    {
        ...
        require(stakeLists[stakerAddr].length != 0, "MOONS: Empty stake list");
        require(stakeIndex < stakeLists[stakerAddr].length, "MOONS: stakeIndex invalid");
        ...
        require(g._currentDay >= st._lockedDay + st._stakedDays, "MOONS: Stake not fully served");

        /* Stake must still be locked */
        require(st._unlockedDay == 0, "MOONS: Stake already unlocked");
        ...
        external
    {
        ...
        /* require() is more informative than the default assert() */
        require(stakeListRef.length != 0, "MOONS: Empty stake list");
        require(stakeIndex < stakeListRef.length, "MOONS: stakeIndex invalid");
        ...
        internal
    {
        ...
        require(newStakedDays <= MAX_STAKE_DAYS, "MOONS: newStakedDays higher than maximum");
        ...
        require(newStakeShares != 0, "MOONS: newStakedSuns must be at least minimum shareRate");
        ...
        returns (uint256 bonusSuns)
    {
        /*
            ...
            Bonus percentage scaled 0% to 10% for the first 7M MOONS of stake.
...
contract TransformableToken is StakeableToken {
    ...
    function xfLobbyEnter(address referrerAddr)
        ...
        payable
    {
	require(_currentDay() > 0, "MOONS: Auction has not begun yet");

        uint256 enterDay = _currentDay();

        uint256 rawAmount = msg.value;
        require(rawAmount != 0, "MOONS: Amount required");
    ...
    function xfLobbyExit(uint256 enterDay, uint256 count)
        external
    {
        require(enterDay < _currentDay(), "MOONS: Round is not complete");
        ...
        if (count != 0) {
            require(count <= qRef.tailIndex - headIndex, "MOONS: count invalid");
            endIndex = headIndex + count;
        } else {
            endIndex = qRef.tailIndex;
            require(headIndex < endIndex, "MOONS: count invalid");
        }
        ...
        external
    {
        ...
        require(address(this).balance != 0, "MOONS: No value");

        require(LAST_FLUSHED_DAY < _currentDay(), "MOONS: Invalid day");
     ...
     * @param enterDay Day number when the member entered
     * @param entryIndex Entry Index on lobby
        ...
        returns (uint256 rawAmount, address referrerAddr)
    {
        ...
        require(entry.rawAmount != 0, "MOONS: Param invalid");
...
contract MOONS is TransformableToken {
    constructor()
        public
    {
