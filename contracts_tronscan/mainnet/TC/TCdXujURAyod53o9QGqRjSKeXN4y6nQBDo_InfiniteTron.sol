...
contract InfiniteTron {
    ...
    function withdraw(uint pos) public returns(uint256){
        ...
        if( (player.deposits[pos].time+1 days)<= now){
            uint n =  (now-player.deposits[pos].time)/24/60/60;
            rate2 = n+rate;
        }else{
            rate2 = rate;
        }
    ...
    function datas() public view returns(uint256[] memory, uint256[] memory, uint256[] memory, uint256[] memory, uint[] memory,uint[] memory){
        ...
        for (uint i=0; i<players[msg.sender].deposits.length ;i++){
            if(players[msg.sender].deposits[i].amount > 0){
                ...
                pos[p] = i;
                if( (players[msg.sender].deposits[i].time+1 days)<= now){
                    n =  (now-players[msg.sender].deposits[i].time)/24/60/60;
                    rate2 = n+players[msg.sender].deposits[i].per;
                }else{
                    rate2 = players[msg.sender].deposits[i].per;
                }
                per[p] = rate2;
                p++;
            }
