//SourceUnit: speederXTrc-0917V3.sol
...
contract SpeederXContract {
    ...
    event SentTrx(address indexed receiver, uint8 matrix, uint8 level);
    
    constructor(address ownerAddress) public {
        levelPrice[1] = 100 trx;
        for (uint8 i = 2; i <= LAST_LEVEL - 3; i++) {
            levelPrice[i] = levelPrice[i-1] * 2;
        }

        for (uint8 i = 13; i <= LAST_LEVEL; i++) {
            levelPrice[i] = levelPrice[i-1] * 7 / 4 ;
        }
    ...
    //"2-5","0-5"
    function buyNewLevel(uint8 matrix, uint8 startLevel, uint8 endLevel) external payable {
    ...
    function sendETHDividends(address userAddress, address _from, uint8 matrix, uint8 level) private {
        ...
        emit SentTrx(receiver, matrix, level);
