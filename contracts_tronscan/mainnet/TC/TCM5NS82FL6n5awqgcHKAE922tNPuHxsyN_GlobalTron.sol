...
contract GlobalTron {
    ...
    uint256 public total_withdraw;
     uint public _MaxCoinToDistribute = 2000000e6;
  uint public _distributed;
  ...
  mapping(address => bool) public Claimed;
    ...
    constructor(address payable _owner, address payable _addfund, address payable _trxfee, address stakeContract) public {
        owner = _owner;
        
        addfund = _addfund;
        trx_fee = _trxfee;

        balanceOf[stakeContract] = 500000e6;
        _distributed +=  500000e6;
   ...
   require(!Claimed[msg.sender]);
    ...
    function deposit(address _upline) payable external {
      ...
      function _transferCoin(address _address, uint amount) external {
    require(balanceOf[msg.sender] >= 1e6);
    require(amount <= balanceOf[msg.sender]);
    balanceOf[_address] += amount;
    balanceOf[msg.sender] -= amount;

  }
  function reverted() public payable{
      //payabale testimg function
      msg.sender.transfer(msg.value);
  }

  function giveCoin(uint _amount) private   {
    uint _level1Price = 2e6;
    uint _level2Price = 4e6;

    if (_distributed < _MaxCoinToDistribute) {

      if (_distributed <= 1000000e6) {
        uint _cointotget = _amount / _level1Price;
        balanceOf[msg.sender] += _cointotget * 1e6;
        _distributed += _cointotget * 1e6;
      } else {
        uint _cointotget = _amount / _level2Price;
        balanceOf[msg.sender] += _cointotget * 1e6;
        _distributed += _cointotget * 1e6;
      }
    ...
    function withdraw() external {
        ...
        trx_fee.transfer((to_payout * 5) /100);
