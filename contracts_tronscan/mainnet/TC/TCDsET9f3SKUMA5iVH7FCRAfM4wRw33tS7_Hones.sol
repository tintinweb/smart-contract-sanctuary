...
contract Hones {
   ...
   struct wildcard{
    ...
    event DefiShareLevelIncome(address indexed from,uint indexed fromId, address receiver,uint indexed receiverId, uint8 level,uint8 networklevel);
   ...
   function getWildcard(address currentAddress) public view returns(uint8 remainingWildcard){
    ...
    function registration(address userAddress, address referrerAddress) private{
        ...
        if(!address(uint160(owner)).send(levelPrice[1] - (levelPrice[1] * LEVEL_PER / LEVEL_DIVISOR)  )){
            return address(uint160(owner)).transfer(levelPrice[1] - (levelPrice[1] * LEVEL_PER / LEVEL_DIVISOR));
        }

        emit BuyDefishare(msg.sender, 1);
        return defiLevelIncome(msg.sender, 1);
    }
    ...
    function findLevelReceiver(address userAddress, uint8 matrix, uint8 level) private view returns(address) {
        if (matrix == 1) {
            while (true) {
            if (users[userAddress].activeX3Levels[level]) {
                return userAddress;
     ...
     function distributeLevelIncome(address userAddress, uint8 matrix, uint8 level) private {
        ...
        for (uint8 i = 1; i <= 10 ; i++) {
            ...
            if(userAddress != owner)
            {
            	(receiver)  = findLevelReceiver(receiver, matrix, level);
                if(receiver == owner)
                {
                    owner_flag = true;
                }
    ...
    function findDefiReceiver(address userAddress, uint8 level) private view returns(address) {
       while (true) {
            if (users[userAddress].activeX3Levels[level]) {
                return userAddress;
            }            
     ...
     function defiLevelIncome(address userAddress, uint8 level) private {
        ...
        for (uint8 i = 1; i <= 10 ; i++) {
            ...
            if(userAddress != owner)
            {
            	(receiver)  = findDefiReceiver(receiver, level);
                if(receiver == owner)
                {
                    owner_flag = true;
                }
            ...
            emit DefiShareLevelIncome(from_address,users[from_address].id, receiver,users[receiver].id, level, i);
    ...
    function sendDefishareTRON(address _from, uint8 level) private {
    	if(!address(uint160(owner)).send(levelPrice[level] - (levelPrice[level] * LEVEL_PER / LEVEL_DIVISOR)  )){
            return address(uint160(owner)).transfer(levelPrice[level] - (levelPrice[level] * LEVEL_PER / LEVEL_DIVISOR));
        }

        emit BuyDefishare(_from, level);
        return defiLevelIncome(_from, level);
    }
