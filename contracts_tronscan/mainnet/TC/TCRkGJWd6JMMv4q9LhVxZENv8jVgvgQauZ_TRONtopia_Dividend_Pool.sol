...
interface InterfaceGAMES {
    ...
    function getAvailableDivRake() external returns (bool, uint256);
...
contract TRONtopia_Dividend_Pool is owned{
    ...
    uint256 public divPercentageSUN = 100000000; // 100% of current dividend amount by default
    ...
    //events
    event DividendPaidDivRake(uint256 indexed totalDividend, uint256 totalNegativeDividend, uint256 indexed payableAmount, uint256 dividendAccumulatedDivRake);
...
/*
    ...
    */
    ...
    /**
        Update divPercentageSUN
    */
    function updateDivPercentageSUN( uint256 _divPercentageSUN ) public onlyOwner returns(string){
        
        divPercentageSUN = _divPercentageSUN;

        return "divPercentageSUN updated successfully";
    }
    ...
    */
    function requestFundFromGameContracts() public onlySigner returns(bool){
        ...
        for(uint i=0; i < totalGameContracts; i++){
            (bool result, uint256 amount) = InterfaceGAMES(whitelistCallerArray[i]).getAvailableDivRake();
            if(result){
    ...
    */
    function displayAvailableDividendALL() public view returns(bool, uint256){
        ...
        uint256 totalNegativeDividend;
        uint256 leaderBoardShare;
        uint256 refPoolShare;
        uint256 sharesPoolShare;
        for(uint i=0; i < totalGameContracts; i++){
            (bool result, uint256 amount) = InterfaceGAMES(whitelistCallerArray[i]).getAvailableDivRake();
            if(result){
                ...
                totalNegativeDividend += amount;
            }
        }


        //when totalDividend is higher than totalNegativeDividend.. It also considers positive or zero value of dividendAccumulatedDivRake
        if(totalDividend > totalNegativeDividend){
            

            totalDividend = (totalDividend - totalNegativeDividend) + dividendAccumulatedDivRake ;
            
            //we deduct 1% of finalAmount from itself for Leaderboard distribution
            leaderBoardShare = totalDividend * leaderboardsPercentSUN / 100000000;
            //we deduct RefPool share as well
            refPoolShare = totalDividend * refPoolPercentSUN / 100000000;  //refPoolPercentSUN is in SUN
            // we deduct shares pool %
            sharesPoolShare = totalDividend * sharesPoolPercentSUN / 100000000;  

            return (true, (totalDividend  - (leaderBoardShare + refPoolShare + sharesPoolShare )));
            
        }
        
        
        //when totalNegativeDividend is higher than totalDividend. Also considers dividendAccumulatedDivRake
        else if(totalNegativeDividend > totalDividend){
            
            
            //if dividendAccumulatedDivRake is higher than the difference, then it will return positive dividend
            if(dividendAccumulatedDivRake > (totalNegativeDividend - totalDividend) ){
                
                totalDividend = dividendAccumulatedDivRake - (totalNegativeDividend - totalDividend);
                ...
                return (false, (totalNegativeDividend - totalDividend) - dividendAccumulatedDivRake );
            }
        }
        
        
        //this is situation where both dividends are same, but dividendAccumulatedDivRake is positive.
        else if(dividendAccumulatedDivRake > 0){
            
            totalDividend = dividendAccumulatedDivRake;
    ...
    */
    function distributeMainDividend() public  returns(uint256){
        ...
        uint256 leftover;
        uint256 totalDividendPositive;
        
        (uint256 totalDividend, uint256 totalNegativeDividend) = requestDividendPayment();
        
        
        //when totalDividend is higher than totalNegativeDividend.. It also considers positive or zero value of dividendAccumulatedDivRake
        if(totalDividend > totalNegativeDividend){
            totalDividendPositive = totalDividend;
            totalDividend = totalDividend + dividendAccumulatedDivRake - totalNegativeDividend;
            totalDividend = totalDividend * divPercentageSUN / 100000000;
            (finalDividendAmount, leftover) = distributionProecssing(totalDividend);
            
            dividendAccumulatedDivRake  = leftover + dividendAccumulatedDivRake + (totalDividendPositive * divPercentageSUN / 100000000) - totalDividend;
        ...
        //when totalNegativeDividend is higher than totalDividend. Also considers dividendAccumulatedDivRake
            ...
            if(dividendAccumulatedDivRake > (totalNegativeDividend - totalDividend) ){
                ...
                dividendAccumulatedDivRake += totalDividend*divPercentageSUN/100000000;
                return 0;   //nothing to distribute if everything is in negative
            }
        ...
        //this is situation where both dividends are same, but dividendAccumulatedDivRake is positive.
            ...
            totalDividend = dividendAccumulatedDivRake*divPercentageSUN/100000000;
            (finalDividendAmount, leftover) = distributionProecssing(totalDividend);
            
            dividendAccumulatedDivRake = leftover + (dividendAccumulatedDivRake - totalDividend);
        }
        
        
        
        
        emit DividendPaidDivRake(totalDividend, totalNegativeDividend, finalDividendAmount, dividendAccumulatedDivRake);
        //default return zero
    }
    
    function requestDividendPayment() internal returns(uint256, uint256) {
        //we will check dividends of all the game contract individually
        uint256 totalGameContracts = whitelistCallerArray.length;
        uint256 totalDividend;
        uint256 totalNegativeDividend;
        for(uint i=0; i < totalGameContracts; i++){
            (bool status, uint256 amount) = InterfaceGAMES(whitelistCallerArray[i]).getAvailableDivRake();
            if(status){
                //if status is true, which means particular game has positive dividend available
                totalDividend += amount;
                amount = amount * divPercentageSUN / 100000000;
        ...
        return (totalDividend, totalNegativeDividend);
    }
    
    
    function distributionProecssing(uint256 totalDividend) internal returns (uint256, uint256){
            ...
            uint256 finalDividendAmount = (totalDividend *  (100000000 - leaderboardsPercentSUN - refPoolPercentSUN - sharesPoolPercentSUN ) / 100000000 );
            ...
            return (finalDividendAmount, finalDividendAmount - (bronzrTierAmount + silverTierAmount + goldTierAmount));
    }
