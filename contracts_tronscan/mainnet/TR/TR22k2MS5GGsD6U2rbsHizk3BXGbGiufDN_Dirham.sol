...
contract Dirham is AccessControl, ERC20Detailed, Ownable {
    ...
    constructor(address multiSig) public ERC20Detailed("Dirham", "DHS") {
        _setupRole(DEFAULT_ADMIN_ROLE, multiSig);
        _setupRole(MINTER_ROLE, multiSig);
        _setupRole(REBASER_ROLE, multiSig);
        _currentFactor = 1e18;
    ...
    function burn(uint256 amount) external {         
        ...
        if (amount.mul(_accuracy) % _currentFactor != 0) {
            _balances[_msgSender()] = _balances[_msgSender()].sub(scaledAmount + 1, "ERC20: burn amount exceeds balance");
            _totalSupply = _totalSupply - _currentFactor.div(_accuracy);
        }
