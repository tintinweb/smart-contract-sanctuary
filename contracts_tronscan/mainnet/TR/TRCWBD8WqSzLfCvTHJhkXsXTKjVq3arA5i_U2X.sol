...
contract _ERC20 is Context, TRC20 {
    ...
    /* allocating 25 million tokens for promotion, airdrop, liquidity, token buy back and burn, dev share and to fund development of next project "Razor Bridge"*/
...
contract GlobalsAndUtility is _ERC20 {
    /*  XfLobbyEnter
    */
    event XfLobbyEnter(
        uint256 timestamp,
    ...
    */
    ...
    /* Flush address */
    //address payable internal constant FLUSH_ADDR = 0x6716e0c823929b4c2FCa92ec31962941Cd7F2576;
    address public  FLUSH_ADDR;
    address public  USDT_ADDRESS; 
    uint8 internal LAST_FLUSHED_DAY = 1;
   
    /* ERC20 constants */
    string public constant name = "U2X";
    string public constant symbol = "U2X";
    uint8 public constant decimals = 8;

    /* Suns per Satoshi = 10,000 * 1e8 / 1e8 = 1e4 */
    uint256 private constant SUNS_PER_U2X = 10 ** uint256(decimals); // 1e8

    /* Time of contract launch (2020-7-11T00:00:00Z) */
    uint256 public LAUNCH_TIME = 1603360641;

    /* Start of claim phase */
    uint256 internal constant PRE_CLAIM_DAYS = 1;
    uint256 internal constant CLAIM_STARTING_AMOUNT = 500000 * (10 ** 8);
    ...
    uint256 internal constant MAX_STAKE_DAYS = 100; //  100 days
    ...
    uint256 private constant BPB_MAX_U2X = 7 * 1e6;
    uint256 internal constant BPB_MAX_SUNS = BPB_MAX_U2X * SUNS_PER_U2X;
        ...
        external
    {
        ...
        require(g._currentDay > CLAIM_PHASE_START_DAY, "U2X: Too early");

        if (beforeDay != 0) {
            require(beforeDay <= g._currentDay, "U2X: beforeDay cannot be in the future");
    ...
    {
        require(beginDay < endDay && endDay <= globals.dailyDataCount, "U2X: range invalid");
        ...
        view
    {
        ...
        require(stakeIdParam == stRef.stakeId, "U2X: stakeIdParam not in stake");
...
contract StakeableToken is GlobalsAndUtility {
        ...
        external
    {
        ...
        require(newStakedDays >= MIN_STAKE_DAYS, "U2X: newStakedDays lower than minimum");
        ...
        external
    {
        ...
        require(stakeLists[stakerAddr].length != 0, "U2X: Empty stake list");
        require(stakeIndex < stakeLists[stakerAddr].length, "U2X: stakeIndex invalid");
        ...
        require(g._currentDay >= st._lockedDay + st._stakedDays, "U2X: Stake not fully served");

        /* Stake must still be locked */
        require(st._unlockedDay == 0, "U2X: Stake already unlocked");
        ...
        external
    {
        ...
        /* require() is more informative than the default assert() */
        require(stakeListRef.length != 0, "U2X: Empty stake list");
        require(stakeIndex < stakeListRef.length, "U2X: stakeIndex invalid");
        ...
        if (g._currentDay >= st._lockedDay) {
            ...
            //msg.sender.transfer(dividends);ERC20(USDT_ADDRESS).transfer(msg.sender,1000);
            ERC20(USDT_ADDRESS).transfer(msg.sender,dividends);
        } else {
        ...
        internal
    {
        ...
        require(newStakedDays <= MAX_STAKE_DAYS, "U2X: newStakedDays higher than maximum");
        ...
        require(newStakeShares != 0, "U2X: newStakedSuns must be at least minimum shareRate");
        ...
        returns (uint256 bonusSuns)
    {
        /*
            ...
            Bonus percentage scaled 0% to 10% for the first 7M U2X of stake.
...
contract TransformableToken is StakeableToken {
     ...
     * @param referrerAddr USDT address of referring user (optional; 0x0 for no referrer)
     * Before calling this please approve the spenders amount.
     */
    function xfLobbyEnter(address referrerAddr,uint256 cUSDTAmount)
        external
        
    {
        require(_currentDay() > 0, "U2X: Auction has not begun yet");

        uint256 EnterDay = _currentDay();

        //uint256 rawAmount = msg.value;

        require(cUSDTAmount < ERC20Basic(USDT_ADDRESS).balanceOf(address(msg.sender)),"Incomplete Balance");

        //ERC20(address(USDT_ADDRESS)).approve(address((this)),cUSDTAmount);
        ERC20(address(USDT_ADDRESS)).transferFrom(msg.sender,address(this),cUSDTAmount);
        
        uint256 rawAmount = cUSDTAmount;
        require(rawAmount != 0, "U2X: Amount required");
    ...
    function xfLobbyExit(uint256 EnterDay, uint256 count)
        external
    {
        require(EnterDay < _currentDay(), "U2X: Round is not complete");
        ...
        if (count != 0) {
            require(count <= qRef.tailIndex - headIndex, "U2X: count invalid");
            endIndex = headIndex + count;
        } else {
            endIndex = qRef.tailIndex;
            require(headIndex < endIndex, "U2X: count invalid");
        }
    ...
    function xfFlush() external 
    {
        ...
        require(ERC20Basic(USDT_ADDRESS).balanceOf(address(this)) != 0, "U2X: No value");

        require(LAST_FLUSHED_DAY < _currentDay(), "U2X: Invalid day");
         _dailyDataUpdateAuto(g);
        //FLUSH_ADDR.transfer((dailyData[LAST_FLUSHED_DAY].dayDividends * 5) / 100);
        ERC20Basic(address(USDT_ADDRESS)).transfer(address(FLUSH_ADDR),(dailyData[LAST_FLUSHED_DAY].dayDividends * 5) / 100);
        LAST_FLUSHED_DAY++;
        _globalsSync(g, gSnapshot);
    }
     ...
     * @param memberAddr USDT address of the lobby member
     * @param EnterDay 
     * @param entryIndex 
     * @return 1: Raw amount that was entered with; 2: Referring USDT addr (optional; 0x0 for no referrer)
     */
        ...
        returns (uint256 rawAmount, address referrerAddr)
    {
        ...
        require(entry.rawAmount != 0, "U2X: Param invalid");
     ...
     * @param memberAddr USDT address of the user
    ...
    // Can be Used at Front End to Calculate the U2X pool.
    function _waasLobby(uint256 EnterDay)
        private
        returns (uint256 waasLobby)
    {
        /* 134246575342  = ~ 4890000 * SUNS_PER_DIV /365  */
        if (EnterDay > 0 && EnterDay <= 365) {                                     
            waasLobby = CLAIM_STARTING_AMOUNT - ((EnterDay - 1) * 134246575342);
        } else {
...
contract U2X is TransformableToken {

     constructor(address USDTaddress,uint256 _timestamp,address _address)
        public
    {
