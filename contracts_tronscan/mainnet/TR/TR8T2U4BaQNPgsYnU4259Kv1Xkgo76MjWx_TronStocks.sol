...
contract TronStocks {
   ...
   uint256 public TimeStart = 1594638000;  // Launch time in UNIX 
   ...
   constructor() public {		
	...
	uint8 constant internal refferalTier0 = 40; // 40% from 50% = 20% referral reward
    uint8 constant internal refferalTier1 = 50; // 50% from 50% = 25% referral reward
    uint8 constant internal refferalTier2 = 60; // 60% from 50% = 30% referral reward
    ...
    function purchaseTokens(uint256 _incomingTron, address _referredBy) internal returns (uint256) {
		...
		uint256 FeeRateCalculator = (_incomingTron / 100) * FeeRate;
