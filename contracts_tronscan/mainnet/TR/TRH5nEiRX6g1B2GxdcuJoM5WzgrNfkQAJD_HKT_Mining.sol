...
contract HKT_Mining is Ownable, ERC721Holder {
    ...
    uint public food;//猫粮卡的卡牌ID
    uint public fristRewDays = 3;//每日头矿查看往期的最多天数
    uint[] public rewardLimit = [5000e18 ,10000e18];//每日头矿分界线
    ...
    uint[] public renewPower = [2000e18, 10000e18, 50000e18];//维修卡槽的算力界限
    ...
    function renew(uint slot_) public {
        ...
        require(slotInfo[_msgSender()][slot_].status, 'no use');
        require(slotInfo[_msgSender()][slot_].endTime > block.timestamp + cycle[2], 'overdue');
        uint need = 1;
