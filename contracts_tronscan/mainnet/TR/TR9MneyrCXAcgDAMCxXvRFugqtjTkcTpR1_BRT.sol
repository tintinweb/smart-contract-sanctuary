...
/*! btr.token.sol | SPDX-License-Identifier: MIT License */
...
contract BRT is TRC20 {
    address public owner;
    bool public stopmint;

    modifier onlyOnwer() {
        require(msg.sender == owner, "BRT: ACCESS_DENIED");
        _;
    }
    ...
    function burn(uint256 value) external onlyOnwer {
        require(balanceOf[msg.sender] >= value, "BRT: INSUFFICEINT_FUNDS");

        _burn(msg.sender, value);
    }

    function stopMint() external onlyOnwer {
        require(!stopmint, "BRT: MINT_ALREADY_STOPED");
