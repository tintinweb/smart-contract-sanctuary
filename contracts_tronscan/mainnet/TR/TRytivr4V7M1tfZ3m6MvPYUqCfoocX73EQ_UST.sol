...
contract UST {
  ...
  uint public TOKEN_ID = 1003698;
  uint DAY = 1 days;
  uint START_AT = 1611954000;
  ...
  function getBuyTokenPrice() public view returns (uint) {
    ...
    return 20000 + (block.timestamp <= START_AT ? 0 : (800 * ( (block.timestamp - START_AT) / DAY) * totalPurchased) / 80000000);
  }
  ...
  function claim() external playtime {
    ...
    support.transfer(investor.stakeProfit / 10);
  ...
  function buy(address referrer) external payable playtime {
    ...
    totalPurchased += (msg.value * 1000000 / getBuyTokenPrice() / 1000000);
  }
  ...
  function sell() external payable playtime {
    ...
    totalPurchased -= ( msg.tokenvalue / 1000000 );
  }
}