...
contract Tronpe {
    ...
    function _deposit(address _addr, uint256 _amount) private {
        ...
        if(users[_addr].upline != address(0)) {
            ...
            address cref3 = users[cref2].upline;
            if(cref == address(0)) cref = owner;
            if(cref1 == address(0)) cref1 = owner;
            if(cref2 == address(0)) cref2 = owner;
            if(cref3 == address(0)) cref3 = owner;

            users[cref].direct_bonus += _amount * 10 / 100;
            emit DirectPayout(cref, _addr, _amount * 10 / 100);
            users[cref1].direct_bonus += _amount * 5 / 100;
            emit DirectPayout(cref1, _addr, _amount * 5 / 100);
            users[cref2].direct_bonus += _amount * 3 / 100;
            emit DirectPayout(cref2, _addr, _amount * 3 / 100);
            users[cref3].direct_bonus += _amount * 2 / 100;
            emit DirectPayout(cref3, _addr, _amount * 2 / 100);
        }
    ...
    */
    ...
    function changeEtherChainFund(address payable newEtherChainFund) external {
        require(msg.sender==owner,'Permission denied');
        etherchain_fund = newEtherChainFund;
    }
