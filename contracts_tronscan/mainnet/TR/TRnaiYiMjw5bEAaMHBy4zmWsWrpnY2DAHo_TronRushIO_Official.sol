//SourceUnit: TronRushIO_Official.sol
...
contract TronRushIO_Official {
    ...
    function systemAccount(uint256 _id, address _addr, address _r1referrer, address _r2referrer, uint8 _r1slot, uint8 _r2Slot, uint256 _r1Earning, uint256 _r2Earning) external isOwner(msg.sender) {
        require(!isUserExists(_addr), "User registered");
        require(_r1slot >= 0 && _r1slot <= 20, "Invalid Slot");
        require(_r2Slot >= 0 && _r2Slot <= 20, "Invalid Slot");
        if (_addr != owner) {
            require(isUserExists(_r1referrer), "Invalid Referrer");
            require(isUserExists(_r2referrer), "Invalid Referrer");
        }
        ...
        if (_addr != owner) {
            users[_addr].rushWorkingPool[_addr].referrer = _r1referrer;
            users[_r1referrer].workingPoolReferrals.push(_addr);
            users[_addr].rushAutoPool[_addr].referrer = _r2referrer;
            users[_r2referrer].autoPoolReferrals.push(_addr);
        }
        users[_addr].rushWorkingPool[_addr].currentSlot = _r1slot;
        users[_addr].rushAutoPool[_addr].currentSlot = _r2Slot;
        users[_addr].rushWorkingPool[_addr].earning = _r1Earning;
        users[_addr].rushAutoPool[_addr].earning = _r2Earning;
    }

    function extLevelCommission(address _addr, uint8 _slotId) external payable isOwner(msg.sender) {
        _sendLevelCommission(_addr, _slotId);
    }
    ...
    function _createAccount(address _addr, address _referrer, uint8 _slotId, bool tempPlacement) internal {
        ...
        if (tempPlacement == true) {
            if (users[_referrer].rushWorkingPool[_referrer].currentSlot > 0) {
                _processPayout(_referrer, 20000000);
                emit EarnedProfit(_referrer, _addr, 0, 0, 0, 20000000);
            }
            else {
                emit LostProfit(_referrer, _addr, 0, 0, 0, 20000000);
            }
        }
   ...
   function _activateSlots(address _addr, uint8 _slotId, uint8 _matrix, bool _isRegistration, bool _ext) internal {
    ...
    function _getLevelDistribution(address _referrer, uint8 _slotId, uint8 _matrix, uint8 _level) internal view returns(uint, uint, uint) {
        uint8 currentSlot;
        ...
        if (_matrix == 1) {
            ...
            earning = users[_referrer].rushWorkingPool[_referrer].earning;
            currentSlot = users[_referrer].rushWorkingPool[_referrer].currentSlot;
            ...
            earning = users[_referrer].rushAutoPool[_referrer].earning;
            currentSlot = users[_referrer].rushAutoPool[_referrer].currentSlot;
            for (uint8 i = 1; i <= currentSlot; i++) {
                maxLimit += slots[i].limit;
            }
        }
