...
contract TRONtopia_classic_dice is owned
{
    ...
    **/
    ...
    function startBet(uint256[3] _rollIntegerVariables, address _referrer, bytes32 _userSeed, bytes32 _uniqueBetId) payable public returns (bytes32 _newUnfinishedBetHash)
    {
        ...
        if(topiaTokenContractAddress != address(0)){
            interfaceTOKEN(topiaTokenContractAddress).mintToken(player, trxSent);
        }
    ...
    */
    function updateContractAddresses(address topiaContract, address voucherContract, address dividendContract, address vaultContract, address diamondContract, address refPoolContract) public onlyOwner returns(string)
    {
