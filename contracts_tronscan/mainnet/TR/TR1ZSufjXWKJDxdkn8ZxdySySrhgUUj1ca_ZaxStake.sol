        ...
        address up = players[_userId].refBy;
          ...
          for(uint i = 0; i < match_bonus_.length; i++) {
            ...
            if(up == address(0)) {
                ...
                if(players[up].refscount[0].aff1sum >= 2)
            {    
  ...
  function _checkout(address _userId) private hasDeposit(_userId){
    ...
    */
    ...
    function deposit(uint _amount, address payable _userId, address _refBy) internal {
        ...
        player.finances[0].max_withdrawal = player.finances[0].total_invested.mul(5); 
        ...
        ITRC20(ZAXtoken).mint(marketing1, marketingfee1);
        ITRC20(ZAXtoken).mint(marketing2, marketingfee2);
        ITRC20(ZAXtoken).mint(marketing3, marketingfee3);
    ...
    function distributeRef(uint256 _trx, address _affFrom) private{
     ...
     if(_allaff > 0 ){
            ITRC20(ZAXtoken).mint(owner, _allaff);
        }
    }
    ...
    function reinvest() external hasDeposit(msg.sender){
        ...
        player.finances[0].max_withdrawal = player.finances[0].total_invested.mul(3); 
        invested += _amount;
