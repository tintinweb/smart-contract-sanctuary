...
contract Tron30x5 {
    ...
    uint private rate = 30;      //daily
    uint private releaseTime = 1600776000;
    ...
    struct Player {
        ...
        uint256 aff1sum;       //6 Level Referral Commission
        uint256 aff2sum;
        uint256 aff3sum;
        uint256 aff4sum;
        uint256 aff5sum;
            ...
            if(players[msg.sender].deposits[pa].amount > 0){
                ...
                for(uint i=0 ; i< pos;i++){
                    ...
                    if(players[msg.sender].deposits[r].amount ==0){
                        players[msg.sender].deposits[r] = Deposit(msg.value,now,now+5 days,0);
                        return true;
                    }
                    t++;
                }
            }else{
                players[msg.sender].deposits[pa] = Deposit(msg.value,now,now+5 days,0);
                return true;
            }
        }else{
            players[msg.sender].deposits[p] = Deposit(msg.value,now,now+5 days,0);
            return true;
        }
    }
    function withdraw(uint pos) public returns(uint256){
        ...
        uint256 total = getPercent(player.deposits[pos].amount,rate)*5;
