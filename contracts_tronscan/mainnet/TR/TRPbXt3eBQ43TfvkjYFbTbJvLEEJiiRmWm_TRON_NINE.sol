...
contract TRON_NINE {
   ...
   // mapping(uint => address) public vIdToAddress;
       ...
       require(isUserExists(msg.sender), "user is not exists. Register first.");
        ...
        if (matrix == 1) 
        {
           ...
           uint256 defd = users[users[msg.sender].referrer].globalRferalCount[level];
            ...
            if(uyi == 0)
            {
                if(users[users[msg.sender].referrer].activeX3Levels[level] == true)
                {
                ...
                if (referrerAddress != owner) {
                    //check referrer active level
                        ...
                        address freeReferrerAddress = findFreeX3Referrer2(referrerAddress,level);
                            ...
                            updateX3Referrer(referrerAddress, findFreeX3Referrer2(referrerAddress,level), level);
    ...
    function findFreeX3Referrer(uint8 level) public view returns(address) {
       ...
       function updateFreeX3Referrer(uint8 level) private
    {
        
       while (true) {
           uint256 id=currentvId[level];
        if (users[vId_number[level][id]].x3Matrix[level].reinvestCount==0) {
            return ;//
        }
        else
        {
