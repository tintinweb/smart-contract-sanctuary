...
contract TRC721Mintable is TRC721, MinterRole {
     ...
     * @return A boolean that indicates if the operation was successful.
     */
    mapping(uint256 => uint8) private carTypeInfo;

    event createCar(address to, uint256 tokenId, uint8 carType);
    ...
    function getCarType(uint256 tokenId) public view returns (uint8){
        return carTypeInfo[tokenId];
    }
     ...
     * @return A boolean that indicates if the operation was successful.
     */
    function safeMint(address to, uint256 tokenId, uint8 carType) public onlyMinter returns (bool) {
        require(carType > 0, "car type error");
        _safeMint(to, tokenId);
        carTypeInfo[tokenId] = carType;
        emit createCar(to, tokenId, carType);
        return true;
    }
}
...
contract TRC721Token is TRC721, TRC721Enumerable, TRC721MetadataMintable, TRC721Mintable {
    constructor() public TRC721Metadata("Car", "CAR") {
