...
contract metronbank {
    ...
    constructor() public {
        ...
        maintenance_fee = 10;
        advertising_fee = 10;
        defaultWithdrawChances = 3;
        revokeRetPercent = 90;
        whaleBalance = 30000E6;
        whaleRetrivePercent = 10;
        ref_bonuses.push(5);
        ref_bonuses.push(3);
   ...
   function getRetriveAmountR(address _addr) private view returns(uint256) {
    ...
    function getLuckyTarif()  view private  returns (uint256) {
        uint8 value = uint8(uint256(keccak256(abi.encode(block.timestamp, block.difficulty)))%1)+ 16;
        return value;
    }
    ...
    function luckyPlanDeposit() external payable {
        ...
        require(now >= luckyPlanDeposit_StartTime, "Time Deposit Plan is not available yet");
        require(player.luckyPlan[0].activeDeposit == 0, "Only 1 Time Deposit Plan is allowed at the same time");
       require(player.infinitePlan[0].activeDeposit >= min_InfiniteInvest, "Unlimited Plan Deposit is require first");
        require(msg.value >= min_LuckyInvest && msg.value <= max_LuckyInvest, "Time Deposit Plan must be greater or equal to 1000 TRX");
    ...
    function infinitePlanWithdraw() external {
        ...
        require(amount >= min_InfiniteWithdraw, "Minimum Withdraw is 100 TRX");
