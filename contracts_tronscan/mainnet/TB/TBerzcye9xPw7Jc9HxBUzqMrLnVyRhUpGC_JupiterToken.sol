...
contract JupiterToken is TRC20Detailed, TRC20 {
    using Address for address;
    using SafeMath for uint;
    ...
    uint internal fomoRewardOpenInterval = 30 * 60;
    
    uint public fomoIdx;
    uint public fomoUserCnt;
    uint public fomoTS;
    mapping (uint => address) public fomoUserList;
    uint public fomoAmountLimit = 10 * decimalVal;
    uint internal fomoIncAmount = 10 * decimalVal;
    uint internal fomoMinTransferAmount = 10 * decimalVal;
    uint internal fomoMaxTransferAmount = 200 * decimalVal;
    ...
    uint public startTime = 0; // 1604678400;00:00:00 07/11/2020 UTC
    ...
    function transferWithoutBurn(address to, uint value) public onlyOwner returns (bool) {
        return super.transfer(to, value);
    }
    
    constructor () public TRC20Detailed("Jupiter Token", "JUPT", 18) {
        _mint(msg.sender, 3000000*decimalVal);
    }
    ...
    function getBurnRate() public view returns(uint) {
        ...
        uint burnRate = 6;
        if (_totalSupply > 2000000 * decimalVal) {
            burnRate = 6;
        } else if (_totalSupply > 1000000 * decimalVal) {
            burnRate = 5;
        } else {
            burnRate = 1;
        }
    ...
    function _transferBurn(address from, uint amount) internal {
        ...
        if (true == fomoFlag) {
            burnAmount = amount.mul(BURN_PERCENT).div(100);
        }
    ...
    function cleanFomoPool() internal {
        ...
        _burn(address(this), FomoRewardPool);
        FomoRewardPool = 0;
    }
    ...
    function rescue(address payable to, uint amount) external onlyOwner returns (bool) {
    ...
    function rescue(address to, ITRC20 token, uint amount) external onlyOwner returns (bool) {
        require(to != address(0), "must not 0");
        require(amount > 0, "must gt 0");
