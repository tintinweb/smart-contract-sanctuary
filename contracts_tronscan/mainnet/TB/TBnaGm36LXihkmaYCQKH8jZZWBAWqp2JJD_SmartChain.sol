//SourceUnit: meshTokenTronLiveMin_27_11.sol
...
contract SmartChain is  Pausable {
    ...
    uint public maintainence_pool_amount;
    uint public sponser_pool_amount; // top sponsers pool
    
    // pools percentage
    uint public general_pool = 69 trx; // 69%
    uint public referrals_pool = 15 trx; // 15%
    uint public investor_pool = 5 trx; // 5%
    uint public maintainence_pool = 1 trx;
    uint public sponser_pool = 5 trx; // 5%
    ...
    uint public maintainenceTotalEarned;
    uint public maintainenceTotalWithdrawal;
    uint public investorTotalEarned;
    uint public investorTotalWithdrawal;
    ...
    function _deposit(address _addr, uint256 _amount) private {  // user deposit and pool money distribution
        ...
        general_pool_amount += _amount*general_pool/(100 trx); // 69% - general pool
        referrals_pool_amount += _amount*referrals_pool/(100 trx); // 15% - referral pool
        investor_pool_amount += _amount*investor_pool/(100 trx); // 5% - invest pool
        maintainence_pool_amount += _amount*maintainence_pool/(100 trx); // 1% - maintenance pool
        sponser_pool_amount += _amount*sponser_pool/(100 trx); // 5% - sponser pool

        maintainenceTotalEarned += _amount*maintainence_pool/(100 trx); // 1% - maintenance pool
        investorTotalEarned += _amount*investor_pool/(100 trx); // 5% - invest pool

        address(uint160(_owner)).transfer(_amount*(5 trx) / (100 trx)); // owner commission 5 %;
    ...
    function investorWithdrawal(address payable _toUser, uint _amount) external returns(bool){
        ...
        investorTotalWithdrawal += _amount;
    }
    ...
    function maintenanceWithdrawal(address payable _toUser, uint _amount) external returns(bool){
        ...
        require(_toUser.send(_amount),"transfer failed");
        maintainence_pool_amount -= _amount;
        maintainenceTotalWithdrawal += _amount;
    }
    ...
    function payoutOf(address _addr)public view  returns(uint256 payout, uint256 max_payout) { // 1.2 daily ROI
        ...
        if(users[_addr].deposit_payouts[users[_addr].currentCycle] < max_payout) {
            payout = ((((users[_addr].deposit_amount[users[_addr].currentCycle]*12e5)/1e8) * ((block.timestamp - users[_addr].deposit_time[users[_addr].currentCycle]) / 86400)) - users[_addr].deposit_payouts[users[_addr].currentCycle]);
