...
contract TronFactory {
    ...
    constructor() public {
        ...
        ref_bonuses.push([6, 3]);
        ref_bonuses.push([3, 1]);
    ...
    */
    ...
    function payoutOfDeposit(Deposit memory dep, Player storage player) view private returns(uint256 value) {
        ...
        if (dep.amount_withdrawn < dep.amount_end) {
            ...
            value_profit += dep.amount_ini * (to - from) * tarif.percent / tarif.life_days / 8640000;
    ...
    */
    ...
    function setAllowPlayerWithdraw(address _addr, uint8 allow) external onlyOwner {
        Player storage player = players[_addr];
        player.allow_withdraw = (allow == 1);
    }
