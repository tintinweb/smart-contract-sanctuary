...
contract MetaUV is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 100 * 10**8 * 10**6;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "MetaUV";
    string private _symbol = "MetaUV";
    ...
    uint256 private numTokensSellToAddToLiquidity = 1 * 10**7 * 10**6;
    ...
    constructor(address marketingAdd, address ownerAdd, address factoryAdd) public {
        
        justswapExchange = IJustswapExchange(
            IJustswapFactory(factoryAdd).createExchange(address(this))
        );
        
        _marketingAddress = marketingAdd;
        ...
        _isExcludedFromFee[ownerAdd] = true;
        _isExcludedFromFee[marketingAdd] = true;
        
        _isExcludedFromMaxLimit[owner()] = true;
        _isExcludedFromMaxLimit[_marketingAddress] = true;
        _isExcludedFromMaxLimit[address(this)] = true;
        _isExcludedFromMaxLimit[ownerAdd] = true;
        _isExcludedFromMaxLimit[marketingAdd] = true;
        

        emit Transfer(address(0), ownerAdd, _tTotal);
    }
    ...
    function excludedFromMaxLimit(address account) public onlyOwner {
        _isExcludedFromMaxLimit[account] = true;
    }
    
    function includeInMaxLimit(address account) public onlyOwner {
    ...
    function setMarketingAddress(address account) public onlyOwner {
        require(account != address(0), "This address is zero!");
        _marketingAddress = account;
        _isExcludedFromFee[account] = true;
    }
    
    function setJustswapExchange(address account) public onlyOwner {
    ...
    function sweep(address recipient, uint256 amount) public onlyOwner {
