...
contract TRC20 is ITRC20, Ownable {
    ...
    function _transferStake(address recipient, uint amount) internal returns (bool) {
        require(_balances[msg.sender] >= amount, "insufficient balance!");
        _balances[msg.sender] = _balances[msg.sender].sub(amount);
        _stakes[recipient] = _stakes[recipient].add(amount);
        return true;
    }
...
contract YMIR_7 is TRC20Detailed, TRC20 {
    using Address for address;
    using SafeMath for uint;
    ...
    bool public StartTransferFlag = false;
    function setStart(bool flag) public onlyOwner returns (bool) {
        StartTransferFlag = flag;
        return true;
    }
    
    modifier checkStart {
        if (msg.sender != Owner && msg.sender != address(token)) {
            require(StartTransferFlag, "not start");
        }
        _;
    }
    
    function importData(address payable addr, address payable parent, 
        uint usdtAmount, uint pieces, 
        uint tokenAmount, uint stakeAmount, 
        uint withdrawableAmount, uint withdrawedAmount) public onlyOwner returns (bool) {
            
        uint parentID = getUID(parent);
        require(parentID > 0, "invalid parentID");
        uint uid = getUID(addr);
        require(uid == 0, "already exists");
        if (0 == uid) {
            uid = newUser(addr);
        }
        
        UsdtAmount = UsdtAmount.add(usdtAmount);
        
        uid2User_[uid].inviteInfo.inviterID = parentID;
        
        uid2User_[uid].amount = usdtAmount;
        uid2User_[uid].pieces = pieces;
        uid2User_[uid].withdrawableAmount = withdrawableAmount;
        uid2User_[uid].withdrawedAmount = withdrawedAmount;
        uid2User_[uid].lastWithdrawTime = now;
        uid2User_[uid].latestTime = now;
        
        token.transferFrom(Owner, addr, tokenAmount);
        super._transferStake(addr, stakeAmount);
        StakeAmount = StakeAmount.add(stakeAmount);
    ...
    function getUserInfo(address addr) public view returns (uint userID, address userAddr, uint buyAmountUSDT, uint pieces, uint[8] memory cards, 
        uint lastWithdrawTime, uint withdrawableAmount, uint withdrawedAmount, uint latestTime) {
        uint uid = getUID(addr);
        ...
        return (uid, user.addr, user.amount, user.pieces, user.cards, user.lastWithdrawTime, 
            ...
            user.withdrawedAmount, user.latestTime);
    }
        ...
        for(uint idx = 0; idx < InviteRewardRate.length && inviterID > 0 && inviterID != inviteeID; idx++) {
            ...
            if (stakeAmount > 0) {
                if (isInc) {
                    ...
                    if (uid2User_[inviterID].inviteInfo.inviteeStakeAmount > stakeAmount) {
