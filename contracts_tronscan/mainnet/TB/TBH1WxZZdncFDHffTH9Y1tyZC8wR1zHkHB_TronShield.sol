...
contract TronShield {
    ...
    //base variables
    uint256[] public referralreward = [10, 3, 2, 1, 1];
    ...
    function MakeInvest(address referrer) public payable returns (bool) {
        ...
        if (users[msg.sender].deposits.length == 0) {
            ...
            user.deposits.push(
                deposit(msg.value, block.timestamp, 0, block.timestamp)
            );
    ...
    function GetUserData(address uaddr)
        public
        view
        returns (
            address wallet,
    ...
    function GetUserDeposits(address uaddr)
        public
        view
    ...
    //shows CheckWithdrawable ammount.`
    function CheckWithdrawable(address _address)
        public
        view
        returns (uint256 total, bool status)
    {
        ...
        uint256 _ROI = CheckCurrentROI();
        for (uint256 i = 0; i < noofdeposits; i++) {
            if (
                block.timestamp > (user.deposits[i].lastaction + minimumwtime)
            ) {
                withdrawablebalance =
                    ((user.deposits[i].amount * _ROI) *
                        (block.timestamp - user.deposits[i].start)) /
                    (86400 * 10000);
                if (
                    withdrawablebalance >
                    (user.deposits[i].amount * maxearnings)
                ) {
                    withdrawablebalance = (user.deposits[i].amount *
                        maxearnings);
                }
                withdrawablebalance =
                    withdrawablebalance -
    ...
    //shows eaarnings.`
    function GetEarnings(address _address) public view returns (uint256 total) {
        userst storage user = users[_address];
        uint256 noofdeposits = user.deposits.length;
        total = 0;
        uint256 withdrawablebalance = 0;
    ...
    function CheckCurrentROI() public view returns (uint256) {
        uint256 _ROI;
        uint256 contractbalance = address(this).balance;
        if (contractbalance > 500000000000) {
            return _ROI = 450;
        } else {
            uint256 additional = contractbalance / 100000000000;
    ...
    function withdraw() public payable returns (bool) {
        ...
        if (withdrawablecondition == true) {
            ...
            for (uint256 i = 0; i < noofdeposits; i++) {
                ...
                if (
                    block.timestamp >
                    (user.deposits[i].lastaction + minimumwtime)
                ) {
                    withdrawablebalance =
                        ((user.deposits[i].amount * _ROI) *
                            (block.timestamp - user.deposits[i].start)) /
