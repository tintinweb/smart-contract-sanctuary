//SourceUnit: trxcode.sol

/**
* https://www.trxcode.gold [Best 2021 TRX Fund Raising Program]
...
contract TRXcode {
    ...
    address payable internal team1; //
    address payable internal team2; //
    ...
    // Enter Cycle Pool
    function enterCycle(address _userId, uint _planId, uint _enteryId, uint _amount, bool _isCycle) private{
        if(_isCycle){
        ...
        uint _positions = nonstopPool.length.sub(1);

        if(_positions > 1 && _positions.mod(2) == 0){
            ...
            // Cycle [Re-enters]
            enterCycle(_cycler, planId_, entryId_, _amount, true);
        }
    }
    ...
    function activatePath(address payable _userId, uint _amount, uint _planId) private{
        ...
        for(uint i = 1; i <= _entries; i++){
            // Update Team Volume
            enterCycle(_userId, _planId, _activation.entries.length, packs[0].cost, false);
        }
    }
    ...
    function joinPath(address payable _refBy) public payable{
        ...
        if(preLaunch){
            require(_player.summary[0].spent.add(_amount) <= 3000 trx && _amount <= 2500 trx, 'NotAllowed');
            _player.finance[0].prepositioned = true;
        }

        else{
            require(systemInit, 'WaitingSystemLaunnch');

            if(!_player.finance[0].isPaid){
                _player.finance[0].isPaid = true;
                players[_player.refBy].finance[0].paidMembers++;
            }
        }
    ...
    function withdraw(uint _amount) public {
        ...
        require(_player.summary[0].activations >= 1, 'MustbeActive');

        if(_player.finance[0].prepositioned && _player.id > 4){
            require(_player.finance[0].paidMembers >= 10, 'Required10Referrals');
        }
    ...
    function awardLeadership() public {
        require(systemInit, 'WaitingSystemLaunnch');
    ...
    function vipSharesAward() public {
        require(systemInit, 'WaitingSystemLaunnch');
        // address payable _userId = msg.sender;
    ...
    function purgeCycle() public {
        ...
        for(uint i = 1; i <= 5; i++){
            // Update Team Volume
            enterCycle(address(0), 2, 1, 500 trx, false);
        }
    ...
    function globalData() public view returns(uint _players, uint _joinedToday, uint _earnings, uint _payouts,  uint _vipPool, uint _vipPoolPaid,
        uint _leadersPool, uint _leadersPoolPaid, uint _purgeBalance, uint _balance, uint _lastVIP, uint _lastLead, uint _gobalBalance){
    ...
    function setTeam(address payable _admin, address payable _team1, address payable _team2) public onlyContract returns(bool){
        ...
        team1 = _team1;
        team2 = _team2;
        Players storage _player = players[admin];
        _player.refBy = contract_;
        _player.id = lastUid.add(1);
        Players storage _player2 = players[team1];
        _player2.refBy = contract_;
        _player2.id = lastUid.add(2);
        Players storage _player3 = players[team2];
        _player3.refBy = contract_;
        _player3.id = lastUid.add(3);
        lastUid = lastUid.add(3);
        return true;
    }
    ...
    function togglePrelaunch() public onlyContract returns(bool){
        preLaunch = !preLaunch ? true:false;
        return preLaunch;
    }
