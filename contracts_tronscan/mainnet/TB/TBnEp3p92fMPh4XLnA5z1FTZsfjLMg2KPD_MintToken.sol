...
contract StakeBase is TRC20 {
    ...
    mapping (address => mapping (uint8 => StakeInfo)) private _stakes;

    event NewStake(address indexed account, uint8 index, uint256 amount);
    event UnStake(address indexed account, uint8 index);
    // event StakeProfit(address indexed account, uint8 index, uint256 amount);
    ...
    constructor () public  {
        ...
        _usdPrice = SUN.div(100); // $0.01 init price
    }
    ...
    function accountStakeInfo(address account, uint8 idx) public view 
    returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        return (
    ...
    function stakeAccount(address account, uint256 amount, address referrer) public returns (bool) {
    ...
    function unstakeAccount(address account, uint8 idx) external onlyOwner returns (bool) {
        _unstake(account, idx);
    ...
    function withdrawAccountCommissions(address account) public returns (uint256) {
        ...
        require(commissions > 0, 'Account commissions is zero');

        _accounts[account].commissions = 0; // reset
        _mint2(account, commissions);
        // emit AccountCommissions(account, commissions);

        return commissions;
    }

    function withdrawAccountStakeProfit(address account, uint8 idx) public returns (uint256) {
        require(_stakes[account][idx].balance > 0, 'Stake balance is zero');
        ...
        if (profit > 0) {
            ...
            // Affiliate commissions/profit
            _proCommissions(account, profit);
            _mint2(account, profit);
    ...
    function withdrawAccountStakeCapital(address account, uint8 idx) public returns (uint256) {
        require(_stakes[account][idx].balance > 0, 'Stake balance is zero');
    ...
    */
    function mintingRewardMasterNodes() external returns (uint256, uint256) {
    ...
    * 2% of profit divide to commissions of all master nodes
    */
    function unstakeRewardMasterNodes() external returns (uint256) {
    ...
    */
    function _stake(address account, uint256 amount, address referrer) internal {
        ...
        require(_accounts[account].stakeCount < 255, 'Account reaches max 255 stakes');

        bool isMasterNode = false;        
        uint8 idx = _accounts[account].stakeCount;
        ...
        if (_accounts[account].stakeCount > 0) {
            ...
            // referrer once save cannot change
    ...
    * - 20% fee: 10% burn, 10% + unstake capital
    * - 80% can withdraw in next 10 months
    */
    function _unstake(address account, uint8 idx) internal {
        require(_stakes[account][idx].capital > 0, 'Stake does not exist');
        require(_stakes[account][idx].capital == _stakes[account][idx].balance, 'Stake has been unstaked');
    ...
    */
    function _capital(address account, uint8 idx, uint256 time) internal view returns (uint256) {
    ...
    * 4% later
    */
    function _profit(address account, uint8 idx, uint256 time) internal view returns (uint256) {
        StakeInfo memory info = _stakes[account][idx];
    ...
    */
     ...
     */
    function _mint2(address account, uint256 value) internal {
