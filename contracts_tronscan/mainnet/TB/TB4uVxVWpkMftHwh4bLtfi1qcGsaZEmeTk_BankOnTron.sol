...
contract Managable {
    ...
    function setViewAccount(address _newViewAccount, uint _status) public onlyOwner {
        require(_newViewAccount != address(0));
        admins[_newViewAccount] = _status;
    }
...
contract BankOnTron is Managable {
    using SafeMath for uint;
    
    uint constant DEVELOPER_RATE = 13;
    uint constant REFER_REWARD_LEVEL1 = 12;
    uint constant REFER_REWARD_LEVEL2 = 5;
    uint constant REFER_REWARD_LEVEL3 = 3;

    uint constant DIVIDE_BASE = 100;
    uint constant INVEST_DAILY_BASE_RATE = 20;
    uint constant INVEST_MAX_DAY = 365;

    uint constant MINIMUM = 10000000; //minimum investment needed, 10 trx
    uint constant REFERRER_CODE = 2020;
    ...
    function getInvestorReferInfo(uint _uid) public view returns (uint, address, uint, uint, uint, uint, uint, uint) {
        if (msg.sender != owner && admins[msg.sender] != 1) {
            require(address2UID[msg.sender] == _uid || 0 == _uid, "only owner or self can check the investor info.");
        }
        if (0 == _uid) {
            _uid = address2UID[msg.sender];
        }
        require(_uid >= REFERRER_CODE && _uid <= latestReferrerCode);
    ...
    function getInvestmentPlanByUID(uint _uid) public view returns (uint[] memory, uint[] memory, uint[] memory, uint[] memory, bool[] memory) {
        if (msg.sender != owner && admins[msg.sender] != 1) {
            require(address2UID[msg.sender] == _uid || 0 == _uid, "only owner or self can check the investment plan info.");
        }
        if (0 == _uid) {
            _uid = address2UID[msg.sender];
        }
    ...
    function investStatistic() external view returns (uint, uint, uint, uint) {
        return (
            latestReferrerCode - REFERRER_CODE + 1, // invetor count
            totalInvestments_,
            totalInvestCnt_,
    ...
    function _invest(address _addr, uint _planId, uint _referID, uint _amount) private returns (bool) {
