...
contract LPTokenWrapper {
    ...
    function withdraw(uint256 amount) public {
        ...
        for (uint256 i = 0; i < 3; i++) {
            if (refTree[i] != address(0x0)) {
                uint256 refAmount = Math.min(
                    amount.mul(refPercent[i]).div(100),
                    _refBalances[refTree[i]]
                );
                totalRef = totalRef.add(refAmount);
...
contract CherryLPRewards is LPTokenWrapper, IRewardDistributionRecipient {
    ...
    uint256 public timeUnlock = block.timestamp + 100 days;
    struct LockedFund {
        uint256 remaining;
        uint256 timeUpdate;
    }
    mapping(address => LockedFund) public locked;
    uint256 allRewards;
    ...
    function getReward() public updateReward(msg.sender) {
        ...
        if (reward > 0) {
            ...
            cherry.mintTo(msg.sender, reward / 4);
            locked[msg.sender].remaining = locked[msg.sender].remaining.add(
                (reward * 3) / 4
            );
    ...
    function getUnlockAmount(address account) public view returns (uint256) {
        LockedFund memory lock = locked[account];
        if (block.timestamp < periodFinish) return 0;
        if (block.timestamp >= periodFinish + 75 days) return lock.remaining;
        uint256 time = block.timestamp -
            (lock.timeUpdate == 0 ? periodFinish : lock.timeUpdate);
        uint256 timeLeft = periodFinish + 75 days - block.timestamp;
        return lock.remaining.mul(time).div(timeLeft);
    }

    function getUnlock() public {
        uint256 amount = Math.min(
            getUnlockAmount(msg.sender),
            locked[msg.sender].remaining
        );
        cherry.mintTo(msg.sender, amount);
        locked[msg.sender].remaining = locked[msg.sender].remaining.sub(amount);
        locked[msg.sender].timeUpdate = block.timestamp;
    }
    ...
    function farmClaim() public onlyRewardDistribution {
        require(block.timestamp > periodFinish + 15 days, "Pool ended 15 day");
