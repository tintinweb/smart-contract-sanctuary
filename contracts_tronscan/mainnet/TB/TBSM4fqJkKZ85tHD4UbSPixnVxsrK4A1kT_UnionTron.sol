...
contract UnionTron {
    ...
    function _deposit(address _addr, uint256 _amount) private {
        ...
        if(users[_addr].upline != address(0)) {
            users[users[_addr].upline].direct_bonus += _amount * 12 / 100;

            emit DirectPayout(users[_addr].upline, _addr, _amount * 12 / 100);
        }
        ...
        admin_fee.transfer(_amount * 7 / 100);
