//SourceUnit: OpalX - Live Contract.sol
...
contract OPALX {
    ...
    string public name = "OPAL X";
    string public symbol = "OPLX";
    uint8 constant public decimals = 6;
    uint8 constant public dailyallowedSell_ = 5;
    uint8 constant internal referralFee_ = 50;
    uint8 constant internal dividendBuyFee_ = 10;
    uint8 constant internal dividendSellFee_ = 15;
    uint8 constant internal dividendTransferFee_ = 5;
    ...
    uint public launchTime = 1604505600;
    
    uint public SellTimeLimit = 86400;
    uint public SellTenure = 7776000; 
    
    mapping(address => uint256) public tokenBalanceLedger_;
    mapping(address => uint256) public referralBalance_;
    
    mapping(address => uint) public depositTime_;
    mapping(address => uint) public lastwithdrawalTime_;
    ...
    function buy(address _referredBy, uint256 amount) public {
    ...
    function exit() public {
        ...
        uint256 _timepassed = now - depositTime_[_customerAddress];
        require(_timepassed >= SellTenure);
    ...
    function withdraw() onlyStronghands public {
        ...
        tokencontract.transfer(msg.sender,_dividends);
    ...
    function sell(uint256 _amountOfTokens) onlyBagholders public {
        ...
        uint256 _withdrawaltimepassed = now - lastwithdrawalTime_[_customerAddress];
        
        uint256 _timepassed = now - depositTime_[_customerAddress];
        
        uint256 _max_daily_sell = SafeMath.div(SafeMath.mul(tokenBalanceLedger_[_customerAddress], dailyallowedSell_), 100);
        
        uint256 _tokens = 0;
        
        
        if(_timepassed >= SellTenure){
        _tokens = _amountOfTokens;
        }else{
        require(_withdrawaltimepassed >= SellTimeLimit);
            if(_amountOfTokens <= _max_daily_sell){
        _tokens = _amountOfTokens;
        }else{
        _tokens = _max_daily_sell;
        }
        }
        
        
        uint256 _tron = tokensToTron_(_tokens);
        uint256 _dividends = SafeMath.div(SafeMath.mul(_tron, dividendSellFee_), 100);
        ...
        lastwithdrawalTime_[_customerAddress] = now;
    ...
    function transfer(address _toAddress, uint256 _amountOfTokens) onlyBagholders public returns (bool) {
        ...
        uint256 _tokenFee = SafeMath.div(SafeMath.mul(_tokens, dividendTransferFee_), 100);
        uint256 _taxedTokens = SafeMath.sub(_tokens, _tokenFee);
        uint256 _dividends = tokensToTron_(_tokenFee);

        tokenSupply_ = SafeMath.sub(tokenSupply_, _tokenFee);
        tokenBalanceLedger_[_customerAddress] = SafeMath.sub(tokenBalanceLedger_[_customerAddress], _tokens);
        tokenBalanceLedger_[_toAddress] = SafeMath.add(tokenBalanceLedger_[_toAddress], _taxedTokens);
        payoutsTo_[_customerAddress] -= (int256) (profitPerShare_ * _tokens);
    ...
    function myTokens() public view returns (uint256) {
        ...
        function myDepositTime() public view returns (uint256) {
        address _customerAddress = msg.sender;
        return depositTime_[_customerAddress];
    }
    
        function myLastWithdrawalTime() public view returns (uint256) {
        address _customerAddress = msg.sender;
        return lastwithdrawalTime_[_customerAddress];
    }
    ...
    function buyPrice() public view returns (uint256) {
        if (tokenSupply_ == 0) {
            ...
            uint256 _tron = tokensToTron_(1e6);
            uint256 _dividends = SafeMath.div(SafeMath.mul(_tron, dividendBuyFee_), 100);
    ...
    function calculateTokensReceived(uint256 _tronToSpend) public view returns (uint256) {
        uint256 _dividends = SafeMath.div(SafeMath.mul(_tronToSpend, dividendSellFee_), 100);
    ...
    function purchaseTokens(uint256 _incomingTron, address _referredBy) internal returns (uint256) {
        address _customerAddress = msg.sender;
        uint256 _undividedDividends = SafeMath.div(SafeMath.mul(_incomingTron, dividendBuyFee_), 100);
