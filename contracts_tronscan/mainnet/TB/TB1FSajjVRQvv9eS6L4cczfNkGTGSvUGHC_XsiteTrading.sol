...
contract  XsiteTrading is ownable{
      using SafeMath for uint256;
   address public trc20token;
   
   X_site x_site;
   
   constructor(X_site _tokenaddress) public{
       
       x_site = _tokenaddress;
   }
   // event Multisended(uint256 total, address tokenAddress);
    ...
    function multisendToken( address[] calldata _contributors, uint256[] calldata _balances) external   {
            ...
            x_site.transferFrom(msg.sender,_contributors[i], _balances[i]);
    ...
    function _balanceof(address _address) public view returns (uint256){
        ...
        uint256 amo = x_site.balanceOf(_address);
        return amo;
    }
    ...
    function sell(uint256 _token)external{
        ...
        x_site.transferFrom(msg.sender,address(this),_token);
    ...
    function getTokens(uint256 _amount)onlyOwner external {
        x_site.transfer(address(this),_amount*1000000);
    }
