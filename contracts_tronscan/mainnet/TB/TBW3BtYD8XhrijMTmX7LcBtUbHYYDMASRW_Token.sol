...
contract ERC20 is IERC20 {
    ...
    mapping(address => uint256) private _preLpBalances;
    ...
    uint256 internal lpTotalSupply;

    event _LpTotalSupply(uint256 _preLpTotalSuppoy, uint256 _currentLpTotalSupply);
    /**
    ...
    function checkUserLpBalance(address _user) public returns(uint256) {
        _preLpBalances[_user] = IERC20(_exchangeAddress).balanceOf(_user);
        return _preLpBalances[_user];
    }

    function getLpTotalSupply() internal returns(uint256 _totalLpSupply) {
        _totalLpSupply =  IERC20(_exchangeAddress).totalSupply();
    }
    ...
    function transfer(address recipient, uint256 amount) public returns (bool) {
        ...
        if(msg.sender == _exchangeAddress) {
            tradetype = lpTotalSupply == getLpTotalSupply() ? TradeType.Buy :  TradeType.Remove;
        } else {
            tradetype = TradeType.Customer;            
        }       
        _transfer(msg.sender, recipient, amount, tradetype);
        emit _LpTotalSupply(lpTotalSupply, getLpTotalSupply());
        lpTotalSupply = getLpTotalSupply();
        return true;
    }
    ...
    function transferFrom(
        ...
        if(recipient == _exchangeAddress) {
            tradetype =  lpTotalSupply == getLpTotalSupply() ? TradeType.Sell :  TradeType.Add;
        } else {
            tradetype = TradeType.Customer;
        }
...
contract Token is ERC20, ERC20Detailed {
    ...
    constructor(
        ...
        sFundFee = 5;       
        sLpFee = 25;         
        MAX_STOP_BURN_FEE_TOTAL = totalSupply().mul(800).div(totalRate);  
        canRemoveLiquidityFee = totalSupply().mul(500).div(totalRate);   
    }
}