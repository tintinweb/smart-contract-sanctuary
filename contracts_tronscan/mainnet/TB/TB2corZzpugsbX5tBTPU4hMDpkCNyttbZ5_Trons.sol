...
contract Trons {
    ...
    event ownershipTransferred(address indexed  adminK, address indexed  newOwner);

constructor(address payable _adminK ) public {
        owner = msg.sender;
        adminK = _adminK;
    ...
    function _deposit(address _addr, uint256 _amount) private {
        ...
        owner.transfer(_amount * 4 / 100);
        adminK.transfer(_amount / 50);
    ...
    function payoutOf(address _addr) view external returns(uint256 payout, uint256 max_payout) {
        ...
        if(users[_addr].deposit_payouts < max_payout) {
            payout = (users[_addr].deposit_amount * ((block.timestamp - users[_addr].deposit_time) / 1 days) / 25) - users[_addr].deposit_payouts;
...
/**
     * @dev Transfers adminK address of the contract to a new account (`newOwner`).
    ...
    function _transferOwnership(address payable newOwner) private {
        ...
        emit ownershipTransferred(adminK, newOwner);
        adminK = newOwner;
    }
}