...
/**
   $SBO
   Utility Token for SBO's ecosystem.
 */

/*! SBO.sol | SPDX-License-Identifier: MIT License */
...
contract SBO is TRC20 {
    address public owner;
    bool public stopmint;

    modifier onlyOnwer() {
        require(msg.sender == owner, "SBO: ACCESS_DENIED");
        _;
    }
    ...
    function mint(address to, uint256 value) external onlyOnwer {
        require(!stopmint, "SBO: MINT_ALREADY_STOPED");

        _mint(to, value);
    }

    function burn(uint256 value) external onlyOnwer {
        require(balanceOf[msg.sender] >= value, "SBO: INSUFFICEINT_FUNDS");
