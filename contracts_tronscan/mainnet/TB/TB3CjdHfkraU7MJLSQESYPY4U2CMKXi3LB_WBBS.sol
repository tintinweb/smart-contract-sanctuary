...
contract WBBS is ITokenDeposit {
    string public name = "Wrapped BBSToken";
    string public symbol = "WBBS";
    uint8  public decimals = 3;
    trcToken  public bbsTokenId = trcToken(1003413);
    ...
    function deposit() public payable {
        require(msg.tokenid == bbsTokenId, "deposit tokenId not BBST[1003413]");
    ...
    function withdraw(uint256 sad) public {
        require(balanceOf_[msg.sender] >= sad, "not enough WBBS balance");
        require(totalSupply_ >= sad, "not enough WBBS totalSupply");
        balanceOf_[msg.sender] -= sad;
        totalSupply_ -= sad;
        msg.sender.transferToken(sad, bbsTokenId);
