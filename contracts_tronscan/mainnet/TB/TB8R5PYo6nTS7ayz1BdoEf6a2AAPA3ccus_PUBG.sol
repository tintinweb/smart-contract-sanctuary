...
contract PUBG is Context, IERC20, Ownable {
  ...
  constructor() public {
    ...
    function showPrice( ) public view returns (uint256 Bp,uint256 Sp,uint256 Bt,uint256 St)
    {
            return (BuyPrice,BuyTkn,SellPrice,SellTkn);
        
    }
    
    function currentcirculation( ) public
    view returns (uint256 total)
    {
        uint256 getamount = 0;
        if(totalbuy > totalsell){
            getamount = totalbuy - totalsell;
        }
            return (getamount);
    ...
    function getPayment() public payable returns (bool) {
     ...
     function transferForBuy() public payable returns (bool) {
        ...
        totalbuy = totalbuy + ((msg.value*BuyTkn)/BuyPrice);
        return true;
    }
