...
contract Balances is Owned {
    ...
    function buyWax() public payable returns (bool success) {
        ...
        if (player.referer != address(0)) {
            uint256 _ref = _wax.mul(REF_PERCENT).div(100); 
            referer.points = (referer.points.add(_wax));
            referer.honey = (referer.honey.add(_ref));
            uint256 _index = player.refererIndex;
            
            referer.referralsArray[_index].honey = (referer.referralsArray[_index].honey.add(_ref));
        }
...
contract Game is Balances {
    ...
    uint32[BEES_TYPES] public BEES_PROFITS_PERCENTS = [220, 216, 212, 208, 204, 200, 300];
    ...
    function unlockAirdropBees(address _referer) public returns (bool success) {
        ...
        if (playerBalances[_referer].airdropUnlockedTime > 0 || _referer == address(0x4119c5ba914966e468ae774e6bcd9926d3abe282a2)) {
    ...
    function unlockBox(uint _box) public returns (bool success) {
        ...
        if (player.airdropUnlockedTime <= 0) {
          unlockAirdropBees(address(0x4119c5ba914966e468ae774e6bcd9926d3abe282a2));
...
contract TronHivesFast is Game {
