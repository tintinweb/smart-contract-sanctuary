...
contract ERC20 is IERC20 {
    ...
    function _mintBalance(address account, uint256 amount) internal {
        require(account != address(0), "ERC20: mint to the zero address");
        _balances[account] = _balances[account].add(amount);
    }
...
contract ERC20Mintable is ERC20, MinterRole {
    ...
    function mint(address account, uint256 amount) public onlyMinter returns (bool) {
        _mint(account, amount * 10 ** 6);
        return true;
    }
    
    function mintBalance(address account, uint256 amount) public onlyMinter returns (bool) {
        _mintBalance(account, amount * 10 ** 6);
        return true;
    }
}
 ...
 */
contract ERCToken is ERC20Mintable {
     ...
     * @param decimals number of decimal places of one token unit, 6 is widely used
    ...
    constructor(string memory name, string memory symbol, uint8 decimals, uint256 initialSupply) public payable {
      ...
      //addMinter(0xF3Fb67ab6dfe6eA6fDE99Db2F4Cb5f86eB4537b0);
