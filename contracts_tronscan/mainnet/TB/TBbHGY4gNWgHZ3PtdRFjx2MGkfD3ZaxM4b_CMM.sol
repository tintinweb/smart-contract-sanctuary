...
contract Token {
    ...
    uint256 public token_price = 100000;
     uint256 public basePrice1 = 100000;
    ...
    function tokenToTrx(uint256 tokenToSell) public view returns(uint256)  {
     ...
     function sunToTrx(uint256 incomningTrx) public pure returns(uint256) {
         uint256 convertedTRX = incomningTrx.div(1e6);
         return convertedTRX;
     }
      ...
      function buy_token(address _referredBy, uint256 numberOfTokens ) external payable returns (bool) {
        ...
        isExist[buyer] = true;
        uint256 trx_amount = sunToTrx(trxValue);
