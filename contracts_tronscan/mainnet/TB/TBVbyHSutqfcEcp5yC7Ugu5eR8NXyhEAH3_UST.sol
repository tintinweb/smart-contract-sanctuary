...
contract UST {
  ...
  uint public TOKEN_ID = 1003687;
  uint DAY = 1 days;
  uint START_AT = 1611608400;
  ...
  function getBuyTokenPrice() public view returns (uint) {
    return 10000 + ( (totalPurchased / 10000000) < 0 ? 0 : (totalPurchased / 10000000));
  }
  ...
  function unstake() external playtime {
    Investor storage investor = investors[msg.sender];
    require(block.timestamp - investor.stakedAt >= 3 days, 'Wait 3 days for unstaking');
  ...
  function buy(address referrer) external payable playtime {
    ...
    totalPurchased += (msg.value * 1000000 / getBuyTokenPrice());
  }
  ...
  function sell() external payable playtime {
    ...
    support.transfer((msg.tokenvalue * getSellTokenPrice() / 1000000) / 10);
    totalPurchased -= msg.tokenvalue;
  }
}