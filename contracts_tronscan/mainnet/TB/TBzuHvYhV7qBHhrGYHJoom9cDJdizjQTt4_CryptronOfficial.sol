//SourceUnit: CryptronOfficial.sol

pragma solidity >=0.4.25;

contract CryptronOfficial {
    ...
    function buyNewLevel(uint8 matrix, uint8 level) external payable {
        require(isUserExists(msg.sender), "Register first as user does not exist");
        require(matrix == 1 || matrix == 2, "Invalid Matrix");
        require(msg.value == levelPrice[level], "Invalid Price");
        require(level > 1 && level <= LAST_LEVEL, "Invalid Level");

        if (matrix == 1) {
            require(!users[msg.sender].activeX3Levels[level], "Level has already been activated");
            ...
            require(!users[msg.sender].activeX6Levels[level], "Level has already been activated"); 
    ...
    function registration(address userAddress, address referrerAddress) private {
        require(msg.value == 50 trx, "Registration Cost is 50");
