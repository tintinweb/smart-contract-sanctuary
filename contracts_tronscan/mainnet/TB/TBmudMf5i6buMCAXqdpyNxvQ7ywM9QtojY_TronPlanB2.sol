...
contract TronPlanB2 is Ownable {
    ...
    uint256 public constant DIRECT_BONUS_DIVIDER = 1e9;
    uint256 public constant DIRECT_BONUS_STEP = 20;
    uint256 public constant DIRECT_BONUS_MAX = 100;
    uint256 public constant INSURANCE_RATE_V2 = 100;
    uint256 public constant INSURANCE_RATE_V1 = 50;
    uint256 public constant INSURANCE_INTEREST = 50;
    ...
    address payable private TronPlanBV1Account;
    ...
    function setTronPlanBV1Account(address payable _newAccount) public onlyOwner {
        require(_newAccount != address(0));
        TronPlanBV1Account = _newAccount;
    }

    function getTronPlanBV1Account() public view onlyOwner returns (address) {
        return TronPlanBV1Account;
    }
    ...
    function getInvestorDividends(uint256 _uid) public view returns (uint256) {
        ...
        newDividends += investor.remainDividends;
        return newDividends ;
    }
    
    function getInvestorTotalStats(uint256 _uid) public view returns (uint256,uint256,uint256,uint256) {
        Investor storage investor = uid2Investor[_uid];
        return (
    ...
    function _invest(address _addr, uint256 _referrerCode, uint256 _amount) private returns (bool) {
        ...
        insurance = insurance.add(_amount.mul(INSURANCE_RATE_V2).div(DIVIDER));
        TronPlanBV1Account.transfer(_amount.mul(INSURANCE_RATE_V1).div(DIVIDER));
    ...
    function withdraw() public {
        ...
        Investor storage investor = uid2Investor[uid];
		require(block.timestamp > investor.checkpoint + TIME_STEP , "Only once a day");
        ...
        if(insStatus==false){
            ...
            for (uint256 i = 0; i < investor.planCount; i++) {
                if(investor.plans[i].withdrawn < investor.plans[i].investment.mul(ROI).div(DIVIDER)){
                    ...
                    withdrawalAmount += amount.div(2);
                    investor.plans[i].lastWithdrawalDate = withdrawalDate;
                    investor.plans[i].withdrawn += amount;
                }
            }

            withdrawalAmount += investor.remainDividends.div(2);
            investor.remainDividends=0;

            if(withdrawalAmount>0){
                investor.remainDividends += withdrawalAmount;
                uint256 currentBalance = getBalance();
                currentBalance = currentBalance.subz(insurance);
                if(withdrawalAmount >= currentBalance){
                    investor.remainDividends += (withdrawalAmount-currentBalance);
    ...
    function getInterestRate(uint256 uid) public view returns(uint256){
        ...
        if(insStatus){
            ...
            interest = uint256(uid2Investor[uid].turnover.div(DIRECT_BONUS_DIVIDER)).mul(DIRECT_BONUS_STEP);
    ...
    function _calculateReferrerReward(uint256 _investment, uint256 _referrerCode) private {
        ...
        if (_referrerCode != 0) {
            ...
            if (_ref1 != 0) {
                ...
                uid2Investor[_ref1].turnover = uid2Investor[_ref1].turnover.add(_investment);
