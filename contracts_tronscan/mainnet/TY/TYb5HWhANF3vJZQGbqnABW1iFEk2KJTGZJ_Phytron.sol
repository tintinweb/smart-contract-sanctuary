...
contract Phytron {
    ...
    function sendETHDividends(address userAddress, address _from, uint8 matrix, uint8 level) private {
        ...
        if (!address(uint160(receiver)).send(levelPrice[level] - SHARE)) { // ??? What is this check?
            return address(uint160(receiver)).transfer(address(this).balance - SHARE);
        }
