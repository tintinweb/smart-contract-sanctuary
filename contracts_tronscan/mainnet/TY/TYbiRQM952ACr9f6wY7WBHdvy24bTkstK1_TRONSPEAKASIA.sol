//SourceUnit: tronspeakasiax.sol

pragma solidity 0.5.10;

contract TRONSPEAKASIA {
	using SafeMath for uint256;

	uint256 constant public INVEST_MIN_AMOUNT = 200 trx;
	uint256 constant public BASE_PERCENT = 40;
	uint256 constant public DIRECT_PERCENT = 50;
	uint256[] public REFERRAL_PERCENTS = [70, 40, 20,10,10,10,10,10,10,10];
	...
	function invest(address referrer) public payable {
		...
		if (user.referrer != address(0)) {
			...
			for (uint256 i = 0; i < 10; i++) 
			{
    				...
    				if(i==0)
			        {
			         uint256 amt=msg.value.mul(DIRECT_PERCENT).div(PERCENTS_DIVIDER);
		      		 users[upline].direct = users[upline].direct.add(amt);
			 		 emit DirectBonus(upline, msg.sender,amt);
