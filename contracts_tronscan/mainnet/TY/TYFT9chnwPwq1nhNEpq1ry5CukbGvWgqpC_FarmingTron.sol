//SourceUnit: FarmingTron.sol

pragma solidity >=0.4.25 <0.6.0;

contract FarmingTron {
    ...
    uint private releaseTime = 1607436000;
    ...
    constructor() public {
      owner = msg.sender;
      planes[0]=Plans("Plan 1",65,0);
      planes[1]=Plans("Plan 2",70,0);
      planes[2]=Plans("Plan 3",80,0);
    }
    ...
    function deposit(address payable _affAddr, uint posplan) public payable  returns(bool){
        ...
        if(posplan==1 && depositAmount <= 1999000000){
            return false;
        }else if(posplan==2 && depositAmount <= 9999000000){
            return false;
        }
            ...
            if(players[msg.sender].deposits[pa].amount > 0){
                ...
                for(uint i=0 ; i< pos;i++){
                    ...
                    if(players[msg.sender].deposits[r].amount ==0){
                        players[msg.sender].deposits[r] = Deposit(msg.value,now,now,0,posplan);
                        return true;
                    }
                    t++;
                }
            }else{
                players[msg.sender].deposits[pa] = Deposit(msg.value,now,now ,0,posplan);
                return true;
            }
        }else{
            players[msg.sender].deposits[p] = Deposit(msg.value,now,now,0,posplan);
            return true;
        }
    }
