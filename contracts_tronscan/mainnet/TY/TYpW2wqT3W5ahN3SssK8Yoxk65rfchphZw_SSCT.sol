...
contract SSCT is Context, ITRC20, Ownable {
    ...
    uint256 public destoryLimitAmount = 677*10**4 * 10 ** 6;
    ...
    uint256 private constant _rTotal = 777*10**4 * 10**6;//发行总量
    ...
    function _tokenTransferIn(address from, address to, uint256 amount) private {
        if(_liquidity[from] == ITRC20(address(justswapExchange)).balanceOf(from)){
            if(destoryAmount >=destoryLimitAmount){
                ...
                uint256 newDestory = destoryAmount.add(destoryFee);
                if(newDestory >= destoryLimitAmount){
                    destoryFee = destoryLimitAmount.sub(destoryAmount);
