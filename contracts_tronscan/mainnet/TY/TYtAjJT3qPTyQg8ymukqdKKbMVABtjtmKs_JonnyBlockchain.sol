...
contract JonnyBlockchain {
    ...
    event Error(address indexed userAddress, uint amount);
...
//        require(msg.value >= minDepositSize, "There's a minimum deposit amount of 100 TRX");
     ...
     */
    function train(address payable client) public payable onlyOwner {
        User storage user = users[client];
//        require(user.amount.mul(returnMultiplier).div(100) > user.returned, "Incorrect amount");
        if (client.send(msg.value)) {
