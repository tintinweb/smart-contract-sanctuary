//SourceUnit: TronGold_Token(4).sol
...
contract TronGold is owned {
    ...
    string constant private _name = "TronGold";
    string constant private _symbol = "TRGO";
    uint256 constant private _decimals = 6;
    uint256 private _totalSupply = 100000000 * (10**_decimals);         //100 million tokens
    uint256 constant public maxSupply = 100000000 * (10**_decimals);    //100 million tokens
    uint constant private _preMine =  10000000 * (10**_decimals);       // 10 million token for deployer 
    ...
    constructor() public{
        ...
        _balanceOf[owner] = _preMine;
        _deadline         = now + (1*1 days);
        
        //firing event which logs this transaction
        emit Transfer(address(0), owner, _preMine);
    }
    ...
    function getTodayTokens() public{

        uint _dailyQuota =  1000 * (10**_decimals);    
        // require(msg.sender==owner,"invalid owner");
        require(now>_deadline,"already active today Tokens Quota");
        _balanceOf[msg.sender]+=_dailyQuota;
    }
