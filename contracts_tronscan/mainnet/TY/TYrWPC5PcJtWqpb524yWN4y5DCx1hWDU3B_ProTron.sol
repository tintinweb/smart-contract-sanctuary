...
contract ProTron {
	...
	mapping(uint => uint256) public users_chunk_size;
	...
	constructor(address payable _owner, address payable _ensurance, uint _pool_last_cycle_time, uint256 _pool_cycle, uint256 _pool_balance, uint256 _total_deposited, uint256 _total_withdraw ) public {
		...
		users_chunk_size[0] = 0;
		users_chunk_size[1] = 3;
		users_chunk_size[2] = 0;
	}
	...
	function withdraw() external {
		...
		if(pool_last_cycle_time + 1 days < now) {
			_process_cycle_withdraw();
		}
	}

	function reinvest() external {
		
		require(users[msg.sender].balance >= 100, "Balance less then 100 TRX");

		uint _deposit_index = 0;	
		uint256 _daily_payout = 0;	
		uint256 _max_payout = users[msg.sender].balance * 4;	

		if (users[msg.sender].balance >= 100 trx && users[msg.sender].balance <= 100000 trx) {
			
			_daily_payout = users[msg.sender].balance.div(100);
			
		} else if (users[msg.sender].balance >= 100001 trx && users[msg.sender].balance <= 200000 trx) {
			
			_daily_payout = users[msg.sender].balance.mul(11).div(1000);

			_deposit_index = 1;
		
		} else if (users[msg.sender].balance >= 200001 trx && users[msg.sender].balance <= 300000 trx) {
			
			_daily_payout = users[msg.sender].balance.mul(12).div(1000);

			_deposit_index = 2;

		} else if (users[msg.sender].balance >= 300001 trx && users[msg.sender].balance <= 400000 trx) {
			
			_daily_payout = users[msg.sender].balance.mul(13).div(1000);

			_deposit_index = 3;
		
		} else if (users[msg.sender].balance >= 400001 trx && users[msg.sender].balance <= 500000 trx) {
			
			_daily_payout = users[msg.sender].balance.mul(14).div(1000);

			_deposit_index = 4;
		
		} else if (users[msg.sender].balance >= 500001 trx) {
			
			_daily_payout = users[msg.sender].balance.mul(15).div(1000);

			_deposit_index = 5;
		
		}

		users[msg.sender].deposits[_deposit_index].deposit_amount += users[msg.sender].balance;
		users[msg.sender].deposits[_deposit_index].max_payout += _max_payout;
		users[msg.sender].deposits[_deposit_index].daily_payout += _daily_payout;

		
		users[msg.sender].total_deposits += users[msg.sender].balance;
		users[msg.sender].max_payout += _max_payout;

		total_deposited += users[msg.sender].balance;

		users[msg.sender].balance = 0;

		if(pool_last_cycle_time + 1 days < now) {
			_process_cycle_reinvest();
		}
	}
	...
	function _set_users_chunk_size(uint256 _users_chunk_size_deposit, uint256 _users_chunk_size_withdraw, uint256 _users_chunk_size_reinvest) external onlyOwner { 

		users_chunk_size[0] = _users_chunk_size_deposit;
		users_chunk_size[1] = _users_chunk_size_withdraw;
		users_chunk_size[2] = _users_chunk_size_reinvest;
	...
	function _process_cycle(uint256 _users_chunk_size) private {
		...
		if (_users_chunk_size > 0) {
			...
			uint256 _end_user_index = current_cycle_user_id + _users_chunk_size;
			...
			if (current_cycle_user_id > users_addresses.length - 1) {
				pool_last_cycle_time = pool_last_cycle_time + 1 days;
				pool_cycle++;
	...
	function _process_cycle_deposit() private {

		_process_cycle(users_chunk_size[0]);

	}

	function _process_cycle_withdraw() private {

		_process_cycle(users_chunk_size[1]);

	}

	function _process_cycle_reinvest() private {

		_process_cycle(users_chunk_size[2]);
