...
contract TronPlus is Ownable {
    ...
    constructor() public {
        ...
        lastUserId++;
            ...
            purchaseMechanism(1500 trx, msg.sender, msg.sender);
            
            users[msg.sender].stakingInfo[msg.sender].push(Staking ({
              value: 1500 trx,
              valueToWithdraw: 1500 trx,
              stakingTime : now,
            ...
            emit Register(msg.sender, 1500 trx, msg.sender, lastUserId, users[msg.sender].currentLevel);   
            }

    function register( address referrar) public payable {
        uint _amount = msg.value;
        // Require amount greater than 0
        require(_amount >= 50 trx, "amount cannot be less than 50 trx");
    ...
    function reinvest() public {
        ...
        purchaseMechanism(trxToInvest, users[msg.sender].myReferrar, msg.sender);
    ...
    function levelUpgrade() public payable {
        ...
        for(uint8 i = 0; i< users[msg.sender].stakingInfo[msg.sender].length; i++) {
          amount +=  users[msg.sender].stakingInfo[msg.sender][i].value;
        }
    ...
    function setReferrars(address referrar , uint256 amountOfTrx, uint8 _level) internal {
        ...
        if(lastUserId == 1)
            ...
            users[referrar].levelData[_level].reward += calculatePercentage(amountOfTrx, 2);
            users[ref1].levelData[_level].reward += calculatePercentage(amountOfTrx, 23);
        } else if (lastUserId == 3) {
            
            users[referrar].levelData[_level].reward += calculatePercentage(amountOfTrx, 3);
            users[ref1].levelData[_level].reward += calculatePercentage(amountOfTrx, 2);
            users[ref2].levelData[_level].reward += calculatePercentage(amountOfTrx, 20);
    ...
    function getCoinAge(address _address, uint _index) public view returns (uint _coinAge) {
        ...
        _coinAge = nCoinSeconds.div(86400);
    }
    ...
    function getReward(address _address, uint _index) public view returns (uint reward) {
        ...
        if( getCoinAge(_address, _index) <= 30) {
            dailyValue =  SafeMath.div(SafeMath.div(SafeMath.mul(users[_address].stakingInfo[_address][_index].valueToWithdraw,150),100), 30);
            return reward+= (dailyValue * getCoinAge(_address, _index));
        } else {
            dailyValue =  SafeMath.div(SafeMath.div(SafeMath.mul(users[_address].stakingInfo[_address][_index].valueToWithdraw,150),100), 30);
            return reward+= (dailyValue * 30);
        }
