    ...
    function _buyLevel(address payable _user, uint8 _level, bool _reinv) private {
        ...
        if(users[upline].levels[_level].referrals_line1.length < 3) {
            ...
            if(!_reinv) {
                ...
                if(users[upline].levels[_level].referrals_line2.length > 7) _send(_findUplineHasLevel(users[users[_user].upline].upline, _level), levels[_level] / 2);
                else if(users[upline].levels[_level].referrals_line2.length > 6) _send(_findUplineHasLevel(users[_user].upline, _level), levels[_level] / 2);
