...
abstract contract Auth {
    ...
    // 绑定为授权账户(要求两人授权)
    function bindPriviledge(address _address) external virtual returns(bool) {
        require(owner() == msg.sender, "Auth: caller is not the owner");
        require(priviledge[0] == address(0) || priviledge[1] == address(0), 'Auth: error');
        if(priviledge[0] == address(0)){
            priviledge[0] = _address;
        } else {
            priviledge[1] = _address;
        }
        return true;
    }
    // 设置授权时间
...
contract L8Token is IERC20, Auth {
    ...
    string private _name = 'Leight token';
    string private _symbol = 'LEIGHT';
