...
contract NTCLock {
    ...
    function addLock(uint256 amount) public {
        address sender = msg.sender;
        // 转移
        require(IERC20(lpAddress).transferFrom(sender, address(this), amount));

        uint256 index = addressLockIndex[sender];
        if (index == 0) {
            lockList.push(Lock({user: sender, amount: 0}));
            index = lockList.length;
            addressLockIndex[sender] = index;
        } else {
    ...
    function releaseLock(uint256 amount) public {
        address sender = msg.sender;
        ...
        require(IERC20(lpAddress).transfer(sender, amount));
