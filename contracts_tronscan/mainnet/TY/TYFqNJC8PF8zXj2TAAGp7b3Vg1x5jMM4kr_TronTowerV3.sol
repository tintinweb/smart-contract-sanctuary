 ...
 *
 *  - Developer &  project manager  : Dr.Blockchain 
 *    
 *	----------------------------*** ENJOY IT ***-----------------------------
***/
...
contract TronTowerV3 is Pausable {
    ...
    mapping (address => bool) public isTabaredUser;
    ...
    function reinvest(uint256 amount, uint256 boxId, string memory predict) public whenNotPaused {
        ...
        address addr = msg.sender;
	    require(!isTabaredUser[msg.sender]);
    ...
    function getPlayerStat(address addr) public view returns
        ...
        function setisTabaredUser(address _addr) public onlyOwner {

        isTabaredUser[_addr] = true;
    }

    function removeisTabaredUser(address _addr) public onlyOwner {

        isTabaredUser[_addr] = false;
    }
