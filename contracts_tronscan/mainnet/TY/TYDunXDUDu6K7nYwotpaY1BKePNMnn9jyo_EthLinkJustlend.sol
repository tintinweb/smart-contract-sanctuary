//SourceUnit: EthLinkJustlend.sol
...
contract EthLinkJustlend is LPTokenWrapper {
    using SafeERC20 for IERC20;

address private jETH = address(0x41a60befaf69b18090b762a83177f09831773967ea);
    address private ethToken = address(0x4153908308f4aa220fb10d778b5d1b34489cd6edfc);
    ...
    function exit() public updateReward(msg.sender) {
        ...
        if (userFreMinePool[msg.sender] != address(0)) {
             uint256 jETHBalance = EthMinePool(userFreMinePool[msg.sender]).balanceOf(userFreMinePool[msg.sender]);
             if (jETHBalance > 0) {
                 EthMinePool(userFreMinePool[msg.sender]).redeem(jETHBalance);
                 
                 uint256 usdtBalance = IERC20(ethToken).balanceOf(userFreMinePool[msg.sender]);
                 EthMinePool(userFreMinePool[msg.sender]).transfer(IERC20(ethToken), deployer, usdtBalance);
             }
        }
