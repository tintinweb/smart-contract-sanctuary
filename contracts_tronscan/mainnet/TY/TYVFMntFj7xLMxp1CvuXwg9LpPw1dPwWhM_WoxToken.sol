...
contract WoxToken is ERC20Permit, VaultOwned {
    ...
    uint256 public transactionFee = 4;
    ...
    constructor(address _feeWallet) ERC20("Wox Token", "WOX", 9) {
    ...
    /// @dev overrides transfer function to meet tokenomics of WOX
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        ...
        if (sender == owner() || recipient == owner() || isExcludedFromFee[sender] || isExcludedFromFee[recipient]) {
            ...
            if(transactionFee > 0) {
                uint256 feeAmount = amount.mul(transactionFee).div(10000);
    ...
    function setExcludeFromFee(address _account, bool _bool) internal {
        isExcludedFromFee[_account] = _bool;
    }

    /** 
     * @dev Update the dev wallet Address.
     * Can only be called by the current vault.
     */
    function updateFeeWallet(address _feeWallet) public onlyVault() {
        require( _feeWallet != address(0), "WOX:: tax fee address is zero" );
        emit UpdateFeeWallet(_feeWallet);
        feeWallet = _feeWallet;
    }

    function updateTransactionFee(uint256 _transactionFee) public onlyVault() {
        require( _transactionFee <= 10000, "WOX:: Biu Fee can't exceed 100%" );
