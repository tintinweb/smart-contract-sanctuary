...
contract goldiTron {
    ...
    uint256 public dev_comission = 4;
    uint256 public sm_commission = 3;
    uint256 public market_com = 4;
    ...
    constructor(address payable _sm, address payable _marketing) public {
        owner = msg.sender;
        sm = _sm;
        ...
        tarifs.push(Tarif(1000, 8000));
        tarifs.push(Tarif(8, 144));
        tarifs.push(Tarif(12, 180));
        tarifs.push(Tarif(18, 216));
    ...
    function deposit(uint8 _tarif, address payable _upline) external payable {
        ...
        uint256  sm_share=(msg.value* sm_commission)/100;
         uint256  marketing_share=(msg.value* market_com)/100;
        
        

        owner.transfer(dev_share);
        sm.transfer(sm_share);
        marketing.transfer(marketing_share);
   ...
   // address[] memory addrs = new address[](indexes.length);
    ...
    /*
        Only external call
    */
    function userInfo(address _addr) view external returns(uint256 for_withdraw, uint256 total_invested, uint256 total_withdrawn, uint256 total_match_bonus, uint256[6] memory structure) {
        Player storage player = players[_addr];
