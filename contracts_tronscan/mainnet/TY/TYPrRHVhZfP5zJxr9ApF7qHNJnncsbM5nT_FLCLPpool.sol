...
contract FLCLPpool is LPTokenWrapper {
    ...
    event StartPool(uint256 initreward,uint256 starttime);   
    ...
    constructor (address _y, address _token,uint256 _initreward, uint256 _initRewardRate) public {
        deployer = msg.sender;
        ...
        rewardRate = (_initRewardRate * (precision)).div(86400);
        emit StartPool(initreward,starttime);
    }


    function setPool(uint256 _initreward, uint256 _starttime) public {
        require(msg.sender == deployer, "sender must deployer");
    ...
    function getReward() public updateReward(msg.sender)  checkStart{
        ...
        if (reward > 0) {
            ...
            if (parent != address(0) && parent != address(this)) {
              ...
              token.safeTransfer(parent, projectfee);
    ...
    function getPoolInfo() public view returns (uint256,uint256,uint256) {
        ...
        return (starttime,totalSupply(),left);
    }
