...
contract D1VSLottery {
    ...
    HourglassInterface D1VS;
    address public developer;
    
    //////////////////////////////////////////////////////////////////////////////////////////////////
    
    uint256 constant public PRICE_PER_TICKET = 5 trx;
    ...
    constructor(address _hourglassAddress) public {
        ...
        D1VS = HourglassInterface(_hourglassAddress);
    ...
    function potentialWinner() external view returns(address) {
        if(isRoundOver() && rounds[currentRoundNumber].blockNumber != 0 && block.number - 256 <= rounds[currentRoundNumber].blockNumber && rounds[currentRoundNumber].blockNumber != block.number) {
    ...
    function jackpotSize() external view returns(uint256) {return rounds[currentRoundNumber].pot.add(D1VS.myDividends(true)).mul(97) / 100;}
    function validatorReward() external view returns(uint256) {return rounds[currentRoundNumber].pot.add(D1VS.myDividends(true)) / 100;}
    ...
    // 1B: Buy tickets from your vault
    ...
    // 2: Validate a round - requires the round to be over.
    function validate() external {
        require(isRoundOver());
        onRoundEnd();
    }
    
    // 3: Withdraw TRX from your vault.
    ...
    function onRoundEnd() private returns (bool newRound) {
        ...
        uint256 dividends = D1VS.myDividends(true);
        if(dividends > 0) {
            D1VS.withdraw();
            totalWinnings = totalWinnings.add(dividends);
        }
        
        uint256 winnings = totalWinnings.mul(97) / 100; //winner reward
        vault[winner] = vault[winner].add(winnings);
        
        emit LotteryWinner(winner, winnings, winningTicket);
        vault[msg.sender] = vault[msg.sender].add(totalWinnings / 100); // Validator Reward
        
        emit Validator(msg.sender, totalWinnings / 100);
        vault[developer] = vault[developer].add(totalWinnings.mul(2) / 100); // Dev Cut
    ...
    function acceptPurchase(uint256 _howMany, uint256 value) private {
        ...
        for(uint256 i = ticketsSold; i < boundary; i++) {rounds[currentRoundNumber].tickets[i] = msg.sender;}
        ...
        D1VS.buy.value(value.mul(40) / 100)(developer);
    }
}