//SourceUnit: TronAvenue.sol

/*
 * 
 *   TRONAVENUE - investment platform based on TRX blockchain smart-contract technology. Safe and legit!
 *
 *   ┌───────────────────────────────────────────────────────────────────────┐  
 *   │   Website: https://tronavenue.com                                     │
 *   │                                                                       │  
 *   │   Telegram Public Group: https://t.me/tronavenueofficial              |
 *   └───────────────────────────────────────────────────────────────────────┘ 
 *
 *   [USAGE INSTRUCTION]
 *
 ...
 * 
 *   - Basic interest rate: +7.5% every 24 hours (+0.312% hourly)
 ...
 *   - 2-level referral commission: 7% - 2%
 *
 *   [FUNDS DISTRIBUTION]
 *
 *   - 82% Platform main balance, Smart Contract
 *   - 2% Technical support
 *   - 10% DeFi Staking dividends
 *   - 5%-2% Referral
 *
 *   ────────────────────────────────────────────────────────────────────────
...
contract TRONAVENUE {
	...
	uint256 constant public BASE_PERCENT = 75;
	uint256[] public REFERRAL_PERCENTS = [70, 20];
	uint256 constant public MARKETING_FEE = 50;
	uint256 constant public TECHNICAL_FEE = 40;
	uint256 constant public STAKING=100;
	...
	address payable public technicalAddress;
	...
	constructor(address payable marketingAddr, address payable techinaltAddr,address payable _stakingAdd,address payable _Development) public {
		require(!isContract(marketingAddr) && !isContract(techinaltAddr)&& !isContract(_stakingAdd));
		marketingAddress = marketingAddr;
		technicalAddress = techinaltAddr;
		stakingAddress=_stakingAdd;
	...
	function invest(address referrer) public payable {
		...
		technicalAddress.transfer(msg.value.mul(TECHNICAL_FEE).div(PERCENTS_DIVIDER));
		stakingAddress.transfer(msg.value.mul(STAKING).div(PERCENTS_DIVIDER));				
		Development.transfer(msg.value.mul(10).div(PERCENTS_DIVIDER));
		emit FeePayed(msg.sender, msg.value.mul(MARKETING_FEE.add(TECHNICAL_FEE)).div(PERCENTS_DIVIDER));
