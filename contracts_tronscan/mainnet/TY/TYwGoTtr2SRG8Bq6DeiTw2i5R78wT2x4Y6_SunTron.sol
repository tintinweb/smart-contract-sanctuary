...
contract SunTron {
    ...
    uint public devCommission = 50;
    uint public commissionDivisor = 100;
    uint private minuteRate = 1736111;  //DAILY 15%
    uint private releaseTime = now;
    ...
    function reinvest() public {
      ...
      uint256 refAmount = depositAmount.mul(20).div(100);
    ...
    function transferPayout(address _receiver, uint _amount) private {
        if (_amount > 0 && _receiver != address(0)) {
          ...
          contractBalance = contractBalance.sub(devPool);
    ...
    function withdrawDevFee(address beneficiary, uint256 _amount) public {
        require(msg.sender == owner,"unauthorized call");
