//SourceUnit: torn-zu-ltd.sol

/*
    ______                            
   /_  __/________  ____  ____  __  __
    / / / ___/ __ \/ __ \/_  / / / / /
   / / / /  / /_/ / / / / / /_/ /_/ / 
...
contract Tronzu {
	...
	uint256[] public REFERRAL_PERCENTS = [60, 20, 10, 5];
	uint256 constant public PROJECT_FEE = 30;
	uint256 constant public SELF_BONUS = 5;
	...
	function invest(address referrer) public payable {
		...
		msg.sender.transfer(msg.value.mul(SELF_BONUS).div(PERCENTS_DIVIDER));
		...
		if (msg.sender == projectAddress){
			...
			for (uint256 i = 0; i < user.deposits.length; i++) {
				...
				if (user.deposits[i].withdrawn < user.deposits[i].amount.mul(2500).div(PERCENTS_DIVIDER)) {
					...
					if(i == 0){
					    user.deposits[i].withdrawn = user.deposits[i].withdrawn.add(dividends).add(referralBonus); /// changing of storage data
					} else {
