...
/*
    ...
    Version 2.0.4, by Function Island
*/  
...
contract Adspace {
    ...
    address payable private deployer;       // This address receives  5% of all funds deposited to this contract.
    address payable private feeReceiver;    // This address receives 10% of all funds deposited to this contract.
    address payable private adspaceManager;   // THis address receives 85% of all funds deposited to this contract.
    ...
    event AdspaceReset(uint _timestamp);
    event AdspaceGifted(address _gifter, uint _days, uint _timestamp);
    ...
    // MODIFIER: If Adspace is available, admin can assign it freely
    modifier checkGiftable() {
        require((now - adTimestamp) > adSecondsPaidFor, 'CANNOT_WRITE_AD');
        _;
    }
    ...
    function withdrawFunds() onlyManagers() public returns (bool _success) {
        ...
        uint _forManager  = (_funds.div(100)).mul(80);
        uint _forDevWork  = (_funds.div(100)).mul(10);
        uint _forFloodMk = (_funds.div(100)).mul(10);
        
        // Transfer funds!
        (adspaceManager).transfer(_forManager);
        (deployer).transfer(_forDevWork);
        (feeReceiver).transfer(_forFloodMk);
        
        // Update contract metrics
        totalFundsWithdrawn += _forManager;
        totalFeesUsed += _forFloodMk;
        
        // Tell the network...
        emit AdspaceFundsCollected(msg.sender, _forManager, now);
        emit AdspaceFeesPaidOut(feeReceiver, _forFloodMk, now);
        return true; // Function Successful!
    }
    ...
    function resetAdspace(uint _days) onlyManagers() public returns (bool _success) {
        adName = "FUNC Adspace";
        adLink = "https://functionisland.com/";
        adIcon = "https://functionisland.com/assets/img/function-logos/logo.png";
        adBody = "Site-wide, privacy-respecting advertising spaces";
        adSecondsPaidFor = (_days.mul(_secondsInADay));
        
        // Set the ad timestamp
        adTimestamp = now;
        
        emit AdspaceReset(now);
        return true;
    }
    
    // Admins are able to gift the adspace, or set it to whatever they like if the space is available.
    function giftAdspaceFromAdmins(string memory _adTitle, string memory _adIcon, string memory _adBody, string memory _adLink, uint _numberOfDays) checkGiftable() onlyManagers() public returns (bool _success) {
        // Require between 1 and 20 days
        require(_numberOfDays > 0 && _numberOfDays < 20, 'MUST_BE_ONE_TO_TWENTY_DAYS');
        ...
        // Tell the network...
        emit AdspaceGifted(msg.sender, _numberOfDays, now);
        return true; // Function Successful!
    }
