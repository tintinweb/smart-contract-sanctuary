...
contract TopiaFreeze is owned 
{
    ...
    */
    ...
    event unfreezeTopiaEv(uint256 thisTime,address caller,uint256 unFreezingAmount, uint256 freezingTier);
    ...
    function unfreezeTopia(uint256 amountToUnFreeze, bool forceUnfreeze) public returns (bool)
    {
        ...
        if (userFrozens[caller].freezingAmount == 0)
        {
            userFrozens[caller].freezingTiming = 0;
        }
        ...
        emit unfreezeTopiaEv(now,caller,amountToUnFreeze,freezingTier);
        return true;
    }
    ...
    {
        if(userFrozens[caller].unfreezed)
        {
    ...
    function releaseTopia() public returns (bool)
    {
        ...
        if (timeRemains == 0 )
        {
            ...
            if (frzAmount > 0 && userFrozens[caller].unfreezed == false)
            {
        ...
        userFrozens[caller].preMatureUnFreezed = false;
        emit releaseTopiaEv( now, caller, amountToRelease, panelty);
        return true;
    }


    function ownerChangeUserFreezeData(address user, uint256 freezingTier_, uint256 freezingAmount_, bool unfreezed_, uint256 unFreezingAmount_, uint256 unFreezingTime_, uint256 freezingTiming_, bool preMatureUnFreezed_ ) public onlySigner returns(bool){


        userFrozens[user].freezingTier = freezingTier_;
        userFrozens[user].freezingAmount = freezingAmount_;
        userFrozens[user].unfreezed = unfreezed_;
        userFrozens[user].unFreezingAmount = unFreezingAmount_;
        userFrozens[user].unFreezingTime = unFreezingTime_;
        userFrozens[user].freezingTiming = freezingTiming_;
        userFrozens[user].preMatureUnFreezed = preMatureUnFreezed_;
    ...
    function finalWithdrawableTopia(address user) public view returns(uint256){
        
        
        if(userFrozens[user].unfreezed == true){
            
            if(userFrozens[user].preMatureUnFreezed == true && (now - userFrozens[user].unFreezingTime) < releaseLock){
                return 0;
            }
            
            return userFrozens[user].unFreezingAmount;
        }
