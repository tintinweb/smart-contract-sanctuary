//SourceUnit: BtcLinkJustlend.sol
...
contract BtcMinePool {
    ...
    JustlendPool public justlendPool =
        JustlendPool(0x417513102bc947f138b88f4bcc6acf73acb8d4d087);
...
contract BtcLinkJustlend is LPTokenWrapper {
    using SafeERC20 for IERC20;

    address private jBTC =
        address(0x417513102bc947f138b88f4bcc6acf73acb8d4d087);
    address private btcToken =
        address(0x4184716914c0fdf7110a44030d04d0c4923504d9cc);
    ...
    function stake(uint256 amount) public updateReward(msg.sender) checkStop {
        ...
        if (userFreMinePool[msg.sender] == address(0)) {
            BtcMinePool _freMinePool = new BtcMinePool();
            userFreMinePool[msg.sender] = address(_freMinePool);
        }

        super.stake(amount, 0, userFreMinePool[msg.sender]);

        uint256 allowances = IERC20(btcToken).allowance(
            userFreMinePool[msg.sender],
            address(jBTC)
        );
        if (allowances <= 0) {
            BtcMinePool(userFreMinePool[msg.sender]).approve(
                IERC20(btcToken),
                address(jBTC),
                115792089237316195423570985008687907853269984665640564039457584007913129639935
            );
        }
        BtcMinePool(userFreMinePool[msg.sender]).mint(amount);
    ...
    function exit() public updateReward(msg.sender) {
        ...
        if (userFreMinePool[msg.sender] != address(0)) {
            uint256 jBTCBalance = BtcMinePool(userFreMinePool[msg.sender])
                .balanceOf(userFreMinePool[msg.sender]);
            if (jBTCBalance > 0) {
                BtcMinePool(userFreMinePool[msg.sender]).redeem(jBTCBalance);

                uint256 usdtBalance = IERC20(btcToken).balanceOf(
                    userFreMinePool[msg.sender]
                );
                uint256 fee = 0;
                if (usdtBalance >= amount) {
                    ...
                    BtcMinePool(userFreMinePool[msg.sender]).transfer(
                        IERC20(btcToken),
                        deployer,
                        fee
                    );
