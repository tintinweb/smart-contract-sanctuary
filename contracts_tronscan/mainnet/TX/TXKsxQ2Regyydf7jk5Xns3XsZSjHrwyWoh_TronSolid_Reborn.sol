//SourceUnit: TronSolid_Reborn.sol

pragma solidity 0.5.8;

/* ---> (www.tronsolid.com)  | (c) 2020 Developed by TRX-ON-TOP TEAM TronSolid_Reborn.sol | La Habana - Cuba <------ */

contract TronSolid_Reborn {
    ...
    // Const
    uint256 private constant TRXONTOP_FEE         = 10; 
    ...
    uint256 private constant MAX_PLANPROFIT       = 2;
    uint256 private constant CONTRACT_TIMETOSTART = 36 * 60 * 60;
    ...
    function getPlan_InterestProfit(address _addr, uint256 plan) view private returns(uint256) {
        ...
        if(fr < to) {
           div = minVal(
                   minZero(player.deposits[plan].amount * MAX_PLANPROFIT, player.deposits[plan].payout), 
    ...
    function payTrxOnTopFee(uint256 val) private {
        uint256 amount = (val * TRXONTOP_FEE) / 100;
