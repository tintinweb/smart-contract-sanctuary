...
contract TENET4X {
    ...
    IERC20 public mytoken = IERC20(0x41747efd2acf96aefdb1a768ffd28c72b06af84bf1);
    ...
    uint256[] public REFERRAL_PERCENTS = [20, 10, 5, 2];//[200,150,100];
...
//            uint256 base_bonus = amount.mul(BASE_PERCENT).div(PERCENTS_DIVIDER);//Base Amount
            for (uint256 i = 0; i < 30; i++)
            {
                if (upline != address(0)) {
                    ...
                    if(amount_bonus > 0){
                        users[upline].bonus = users[upline].bonus.add(amount_bonus);
    ...
    function withdraw() public {
        ...
        uint256 cutAmount = 0;//Withdrawal Amount
        uint256 depAmount = 0;//User depositAmount

        for (uint256 i = 0; i < user.deposits.length; i++) {
            if (user.deposits[i].withdrawn < user.deposits[i].amount.mul(2)) {
                // uint256 userPercentRate = getUserPercentRate();// BASE_PERCENT
            ...
            cutAmount = cutAmount.add(user.deposits[i].withdrawn);//Out of withdrawal Amount
            depAmount = depAmount.add(user.deposits[i].amount);
        }

        uint256 availableAmount = depAmount.mul(2).sub(cutAmount.add(user.refback));//Avaliable Bonus

        require(availableAmount > 0, "User has no available bonus");//no bonus
        ...
        if (referralBonus > 0) {
            referralBonus = availableAmount > referralBonus ? referralBonus : availableAmount;
            totalAmount = totalAmount.add(referralBonus);
            user.refback = user.refback.add(referralBonus);//refBack
            user.bonus = 0;
        }
        // my bonus end
    ...
    function getUserReferralsStats(address userAddress) public view returns (address, uint256, uint24[30] memory) {
        User storage user = users[userAddress];
