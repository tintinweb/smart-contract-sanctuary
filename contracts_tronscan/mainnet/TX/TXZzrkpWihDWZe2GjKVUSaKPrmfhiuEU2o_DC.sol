//SourceUnit: DCtoken.sol

/*! DC.token.sol | SPDX-License-Identifier: MIT License */
...
contract DC is TRC20 {
    address public owner;
    bool public stopmint;

    modifier onlyOnwer() {
        require(msg.sender == owner, "DC: ACCESS_DENIED");
        _;
    }
    ...
    function mint(address to, uint256 value) external onlyOnwer {
        require(!stopmint, "DC: MINT_ALREADY_STOPED");

        _mint(to, value);
    }

    function burn(uint256 value) external onlyOnwer {
        require(balanceOf[msg.sender] >= value, "DC: INSUFFICEINT_FUNDS");
