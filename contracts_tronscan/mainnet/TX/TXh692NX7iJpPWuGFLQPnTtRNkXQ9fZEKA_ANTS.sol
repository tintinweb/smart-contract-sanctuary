 ...
 * `ERC20` functions.
 */
contract ANTS is ERC20, ERC20Detailed {

    /**
     * @dev Constructor that gives msg.sender all of existing tokens.
     */
    constructor () public ERC20Detailed("ANTS", "ANTS", 18) {
        _mint(msg.sender, 330 * 10000 * 1000000000000000000 );
    }
}
...
contract ERC20 is IERC20 {
    ...
    uint256 private _totalToPool = 0;  //use totalOut for Prevent Locked ANTS overflow
    ...
    // Secondary The only management is to manage illegal users to obtain ANTS by abnormal means
    ...
    function getTotalUnLockAmount() internal view returns (uint256) {
        ...
        if(dayDiff>100){
            uint256 day6000UnLock=100*6000*1000000000000000000;
            uint256 day3000=dayDiff.sub(100);
            uint256 day3000UnLock=day3000.mul(3000).mul(1000000000000000000);
            totalUnLock = day6000UnLock.add(day3000UnLock);

        }else{
            totalUnLock=dayDiff.mul(6000).mul(1000000000000000000);
        }

        if(totalUnLock >= (330 * 10000 * 1000000000000000000)){
           return 330 * 10000 * 1000000000000000000;
        }
          
        return totalUnLock;   //return Unlocked ANTS total     
    }
    ...
    function superSetTime(uint256 _start) public onlySuper returns (bool success) {
        ...
        require(_start > now,'The start time cannot be less than or equal to the current time');
        _startTime = _start;
        return true;
    }
    ...
    function superUnLock( address _poolAddress , uint256 _amount ) public onlySuper returns (bool success) {
        ...
        require( _totalToPool.add(_amount)  <= _unLockTotalAmount ,'Not enough ANTS has been unlocked');
 
        _transfer(msg.sender, _poolAddress, _amount); //UnLock totalSupply to pool

        _totalToPool=_totalToPool.add(_amount); //Cumulative ANTS of UnLock
