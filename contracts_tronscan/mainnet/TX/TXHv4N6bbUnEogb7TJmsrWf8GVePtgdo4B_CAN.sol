 ...
 * `ERC20` functions.
 */
contract CAN is ERC20, ERC20Detailed {

    uint256 public _locked;
    uint256 public _remain;
    uint256 public _basic;
    ...
    constructor () public ERC20Detailed("Calluna", "CAN", 10) {
        _lastTime = 1619711999;
        _totalSupply = 660000000 * (10 ** uint256(decimals()));
        _locked = 660000000 * (10 ** uint256(decimals()));
        _remain = 660000000 * (10 ** uint256(decimals()));
        _sourceTotalSupply = 660000000 * (10 ** uint256(decimals()));
        _turn = 0;
        _days =0;
        _basic = 99000;
    ...
    function issue() public returns (bool){
        ...
        uint256 amount = _remain / 3600 ;
        _locked = _locked.sub(amount);
        uint256 amount1 = amount /10;
        uint256 amount2 = amount /10;
    ...
    function bindNode(address addressNode) public returns (bool){
        if(msg.sender == _owner){
            ...
            require(_node == address(0), "binded");
            _node = addressNode;
        }
    ...
    function bindSuanli(address addressSuanli) public returns (bool){
        if(msg.sender == _owner){
            ...
            require(_suanli == address(0), "binded");
            _suanli = addressSuanli;
        }
    ...
    function bindProductor(address addressProductor) public returns (bool){
        if(msg.sender == _owner){
            ...
            require(_productor == address(0), "binded");
            _productor = addressProductor;
        }
    ...
    function bindOwner(address addressOwner) public returns (bool){
        require(_owner == address(0), "binded");
        _owner = addressOwner;
        return true;
    }
}
