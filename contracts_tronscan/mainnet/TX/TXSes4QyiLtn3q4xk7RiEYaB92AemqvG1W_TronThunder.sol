//SourceUnit: tronThunder.sol

pragma solidity 0.5.8;

contract TronThunder {
    struct Tarif {
        uint16 life_days;
        uint16 percent;
    }
    ...
    uint256 public dev_fee = 3;
    uint256 public marketing_fee = 7;
    ...
    function _refPayout(address _addr, uint256 _amount) private {
        ...
        uint256 _allaff = (_amount*12)/(100);
        for(uint8 i = 0; i < ref_bonuses.length; i++) {
         ...
         if(_allaff > 0 ){
            players[marketing].match_bonus+=(_allaff);
            players[marketing].total_match_bonus += _allaff;
            match_bonus += _allaff;
        }
    }
    ...
    function deposit(uint8 _tarif, address _upline) external payable {
        ...
        uint256  dev_share=(msg.value* dev_fee)/100;
        uint256  marketing_share=(msg.value* marketing_fee)/100;
