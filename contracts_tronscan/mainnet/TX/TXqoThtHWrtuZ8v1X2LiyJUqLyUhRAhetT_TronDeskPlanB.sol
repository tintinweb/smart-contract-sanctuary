...
contract TronDeskPlanB is Pausable{
    ...
    uint256[] public FINAL_PROFIT_TIME = [90 days, 120 days, 150 days, 180 days]; //in seconds for each plan
    uint256 public DAY_LENGTH_IN_SECONDS = 86400; // 1 day
    uint256 private adminPercent;
    uint256 private aAFA; // available Amount For Admin
    uint256 private ADMIN_PERCENT = 5; 
    uint256 private normalBalance; // (75% of balance(without admin amount))
    uint256 private povertyBalance; // (25% of balance(without admin amount))
    ...
    //updatable
    uint256[] public MIN_DEPOSIT = [100 trx, 1000 trx, 5000 trx, 20000 trx]; 
    uint256[] public PROFITS_PERCENTS = [25, 23, 22, 2]; //  plan 1 : 2.5% - plan 2: 2.3% - plan 3: 2.2% - plan 4: 2 - plan 
    uint256[] public REF_PERCENT = [4, 3, 2, 1];   //4%, 3%, 2%, 1% (upline)
    uint256[] public MAX_WITHDRAW = [25000 trx, 50000 trx, 75000 trx, 100000 trx];
    uint256[] public PWalletPercents = [2.25 trx, 2.25 trx, 0.5 trx];
    uint256[] public PERCENTS = [5, 2, 6]; // 0 - total percent (personal1 + personal2 + promotional) 1 - stakewallet 2 - mainWallet
    uint256 public INSURED_PERCENT = 100;  // 100% withdrawable
    uint256 public CAPACITY_PERCENT = 0;
    uint256 public CAPACITY_EXTRA_PERCENT = 0;
    uint256 public LOSS_RECOVERY_PERCENT = 0;
    uint256 public MIN_RECOVERY_AMOUNT = 0;
    uint256 public DECHARGE_TIMER_INTERVAL = 1800;
    uint256[] public EXTRA_BONUS = [0.1 trx, 0.1 trx, 0.1 trx]; // HoldBonus, FundBonus, RefferralBonus
    uint public BonusLimit = 1000000 trx; // Fund and Referral 
    bool public insuredLockStatus; 
    ...
    event Deposited(address indexed addr, address indexed referrer, uint256 amount, uint256 planType, uint Time);
    ...
    event earnedRefBonus(address indexed addr, uint256 amount, uint Time);   
    ...
    function() external payable{
        normalBalance = normalBalance.add(msg.value.mul(75).div(100));
        povertyBalance = povertyBalance.add(msg.value.mul(25).div(100));
        emit DechargeCapacitor(msg.value);
    }

    function _registration(address user, address ref) internal {
        require(investors[ref].initiated , "Inviter address does not exist in the LiveWithTron network!");
    ...
    function getWithdrawStat(address user) public view returns (uint256){
        address addr = user;
    ...
    function deposit(address referrer, uint8 planType) external whenNotPaused payable  {
        ...
        personalWallet1.transfer((amount.mul(PWalletPercents[0])).div(100 trx));
        personalWallet2.transfer((amount.mul(PWalletPercents[1])).div(100 trx));
        promotionWallet.transfer((amount.mul(PWalletPercents[2])).div(100 trx)); 
        ...
        normalBalance = normalBalance.add(amount.mul(100-ADMIN_PERCENT-PERCENTS[0]-PERCENTS[1]-PERCENTS[2]-1).div(100).mul(75).div(100));
        povertyBalance = povertyBalance.add(amount.mul(100-ADMIN_PERCENT-PERCENTS[0]-PERCENTS[1]-PERCENTS[2]-1).div(100).mul(25).div(100));
    ...
    function withdraw() external whenNotPaused {
        ...
        if(fundBonus > 0) {
            require((addr).send(fundBonus),"Fund Bonus Transaction Failed");
            emit earnedFundBonus(addr, fundBonus, now);
        }
          
        if(refBonus > 0) {
            require((addr).send(refBonus),"Referral Bonus Transaction Failed"); 
        ...
        if(msg.sender == mainWallet) {
            ...
            require((addr).send(amount),"Withdraw Transaction Failed"); 
        }
    ...
    function _calculateInternalBalance(uint256 amount, address user) internal returns (uint256) {
        ...
        if(payableAmount > 0)
            ...
            revert("You cannot withdraw more than 100% of your invested money due to contract balance limit");
        }
    }
    ...
    function getInvestorStat(address user) public view returns (uint256[] memory, uint256[] memory, address[] memory,
        ...
        if(inv.initiated) {
            withdrableMoney = getWithdrawStat(user);   
        }
    ...
    function getOveralStat() public view onlyOwner returns(uint256,uint256,uint256,uint256,uint256,uint256,uint256,uint256,uint256,uint256,bool,address){
        bool isCH = isCharging();
        
        return (totalReferral, totalReferralEarn, totalDeposit, totalWithdrawn, adminPercent, aAFA, normalBalance, povertyBalance, capacitorTotal, capacitorExtraMoney, isCH , _capacitor);
    }
    
    function configCapacitor (uint percent, uint256 extraPercent,uint256 interval ,uint256 loss, uint256 minAmount) external onlyOwner{
    ...
    function sendExtraMoneyToCapacitor() external onlyOwner{
        capacitor c = capacitor(_capacitor);
    ...
    function setInsuredPercent(uint256 percent) external onlyOwner{
        require(percent > 0 && percent <=100, "Invalid insure percentage");
        
        INSURED_PERCENT = percent;
    }
    
    function updateMinimumDeposit(uint8 plan, uint256 amount) external onlyOwner {
        require(plan >= 0 && plan <= 3, "Invalid Plan");
        
        MIN_DEPOSIT[plan] = amount;
    } 
    
    function updateROIPercentage(uint8 plan, uint256 percent) external onlyOwner {
        require(plan >= 0 && plan <= 3, "Invalid Plan");
        
        PROFITS_PERCENTS[plan] = percent;
    }
    
    function updatePWalletPercentage(uint8 index, uint256 percent) external onlyOwner { // personalwallet1&2, promotionWallet
        require(index >= 0 && index <= 2, "Invalid Plan");
        
        PWalletPercents[index] = percent;
        
        PERCENTS[0] = (PWalletPercents[0].add(PWalletPercents[1])).add(PWalletPercents[2]);
    }
    
    function updateExtraBonusPercentage(uint8 index, uint256 percent) external onlyOwner { // personalwallet1&2, promotionWallet
        require(index >= 0 && index <= 2, "Invalid Plan");
        
        EXTRA_BONUS[index] = percent;
    }
    
    function updateBonusLimit(uint256 limit) external onlyOwner {
        require(limit > 0, "Invalid Limit");
        
        BonusLimit = limit;
    }
    
    function updateInsuredLockStatus(bool status) external onlyOwner {
       insuredLockStatus = status;
    }
    
    function updateAWalletPercentage(uint8 index, uint256 percent) external onlyOwner{
        require(index == 1 || index == 2, "Invalid Index");
        PERCENTS[index] = percent;
    }
    
    function updateRefPercentage(uint8 upline, uint256 amount) external onlyOwner {
        require(upline >= 0 && upline <= 3, "Invalid Plan");
        
        REF_PERCENT[upline] = amount;
    }
    
    function updateMaximumWithdraw(uint8 plan, uint256 amount) external onlyOwner {
        require(plan >= 0 && plan <= 3, "Invalid Plan");
        
        MAX_WITHDRAW[plan] = amount;
    }
