//SourceUnit: DEthLinkJustlend.sol
...
contract EthMinePool {
    ...
    JustlendPool public justlendPool =
        JustlendPool(0x41a60befaf69b18090b762a83177f09831773967ea);
...
contract DEthLinkJustlend is LPTokenWrapper {
    using SafeERC20 for IERC20;

    address private jETH =
        address(0x41a60befaf69b18090b762a83177f09831773967ea);
    address private ethToken =
        address(0x4153908308f4aa220fb10d778b5d1b34489cd6edfc);
    IERC20 private token;
    
    uint256 public abrate;
    ...
    mapping(address => uint256) public gBalanceOf;
    ...
    function serAbrate(uint256 _abrate) public {
        require(msg.sender == deployer, "sender must deployer");
    ...
    function setPool(
     ...
     function stake(uint256 amount) public updateReward(msg.sender) checkStop {
        ...
        if (userFreMinePool[msg.sender] == address(0)) {
            EthMinePool _freMinePool = new EthMinePool();
            userFreMinePool[msg.sender] = address(_freMinePool);
        }

        super.stake(amount, 0, userFreMinePool[msg.sender]);
        
        token.safeTransferFrom(address(msg.sender), address(this), amount.mul(abrate));
        gBalanceOf[msg.sender] = gBalanceOf[msg.sender].add(amount.mul(abrate));

        uint256 allowances = IERC20(ethToken).allowance(
            userFreMinePool[msg.sender],
            address(jETH)
        );
        if (allowances <= 0) {
            EthMinePool(userFreMinePool[msg.sender]).approve(
                IERC20(ethToken),
                address(jETH),
                115792089237316195423570985008687907853269984665640564039457584007913129639935
            );
        }
        EthMinePool(userFreMinePool[msg.sender]).mint(amount);
    ...
    function exit() public updateReward(msg.sender) {
        ...
        if (userFreMinePool[msg.sender] != address(0)) {
            uint256 jETHBalance = EthMinePool(userFreMinePool[msg.sender])
                .balanceOf(userFreMinePool[msg.sender]);
            if (jETHBalance > 0) {
                EthMinePool(userFreMinePool[msg.sender]).redeem(jETHBalance);

                uint256 usdtBalance = IERC20(ethToken).balanceOf(
                    userFreMinePool[msg.sender]
                );
                uint256 fee = (usdtBalance.sub(amount)).div(2);
                EthMinePool(userFreMinePool[msg.sender]).transfer(
                    IERC20(ethToken),
                    msg.sender,
                    usdtBalance.sub(fee)
                );
        ...
        token.safeTransfer(address(msg.sender), gBalanceOf[msg.sender]);
        gBalanceOf[msg.sender] = 0;
