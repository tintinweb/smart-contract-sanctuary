...
//SourceUnit: SWBStake.sol

pragma solidity ^0.5.0;
import "./Manageable.sol";
import "./TRC20.sol";

contract SWBStake is Manageable {
    ...
    TRC20 SWB;
    uint256 profit1Week = 231;
    uint256 profit6Month = 278;
    uint256 profit10Month = 463;
    uint256 profit12Month = 556;
    uint256 profit15Month = 694;
    uint256 profit20Month = 926;
    uint256 profit24Month = 1111;
    uint256 blockPerDay = 720;
    uint256 dayOfWeek = 10;

    
	constructor(address payable _owner, address payable _tokenAddress) public {
		owner = _owner;
		SWB = TRC20(_tokenAddress);
	...
	function stake(uint256 _amount) public returns (bool) {
		...
		require(
            SWB.allowance(msg.sender, address(this)) >= _amount,
	...
	function unStake(uint256 _timeStake) public returns (bool) {
		...
		users[msg.sender].totalHarvest += profit;
	    SWB.transfer(incomes[_timeStake].staker, incomes[_timeStake].stakeAmount);
		users[msg.sender].totalUnStake += incomes[_timeStake].stakeAmount;
		require(totalHarvest <= maxTotalHarvest, "Total Harvest was maximum");
	    if(profit > 0) {
		   	SWB.transfer(incomes[_timeStake].staker, profit);
	    }
	...
	function calculateProfit(uint256 _timeStake) public returns (uint256) {
	    ...
	    if(subBlock > (blockPerDay*dayOfWeek) && subBlock <= (blockPerDay*60)) {
	        profit = (blockPerDay * dayOfWeek * profit1Week);
	        profit = profit + ((subBlock - (blockPerDay * dayOfWeek)) * profit6Month);
	    }
	    if(subBlock > (blockPerDay*60) && subBlock <= (blockPerDay*120)) {
	        profit = (blockPerDay*60 * profit6Month);
	        profit = profit + ((subBlock - (blockPerDay * 30)) * profit10Month);
	    }
	    if(subBlock > (blockPerDay*120) && subBlock <= (blockPerDay*210)) {
	        profit = (blockPerDay*120 * profit10Month);
	        profit = profit + ((subBlock - (blockPerDay * 120)) * profit12Month);
	    }
	    if(subBlock > (blockPerDay*210) && subBlock <= (blockPerDay*360)) {
	        profit = (blockPerDay*210 * profit12Month);
	        profit = profit + ((subBlock - (blockPerDay * 210)) * profit15Month);
	    }
	    if(subBlock > (blockPerDay*360) && subBlock <= (blockPerDay*540)) {
	        profit = (blockPerDay*360 * profit15Month);
	        profit = profit + ((subBlock - (blockPerDay * 360)) * profit20Month);
	    }
	    if( subBlock > (blockPerDay*540) && subBlock <= (blockPerDay*720)) {
	        profit = (blockPerDay*540 * profit20Month);
	        profit = profit + ((subBlock - (blockPerDay * 540)) * profit24Month);
	    }
	...
	function _safeTransferFrom(address _sender, address _recipient, uint _amount) private {
        bool sent = SWB.transferFrom(_sender, _recipient, _amount);
	...
	function withdrawSWB(address _recipient, uint256 _amount) public onlyAdmins returns (bool)  {
		require(_amount <= SWB.balanceOf(address(this)), "The amount must be lower than balance");
		SWB.transfer(_recipient, _amount);
		emit WithdrawSWB(_recipient, _amount);
		return true;
	}
