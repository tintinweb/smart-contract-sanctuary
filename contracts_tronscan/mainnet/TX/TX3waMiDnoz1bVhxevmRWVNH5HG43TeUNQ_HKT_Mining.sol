...
contract HKT_Mining is Ownable, ERC721Holder {
    ...
    function claimRewards() external {
        if (block.timestamp >= deadLine) {
            deadLine = (86400 - block.timestamp % 86400) + block.timestamp + 57600;
            //刷新截止时间
    ...
    function coutingFristReward(address addr_) external view returns (uint){
        ...
        return (rew *  (100 - toClaimRate) / 100);
    }
    ...
    function sliverBullet(address player, uint amount, uint slot) public onlyOwner {
        uint uAmount;
        uint decimal = IERC20(coinID[0]).decimals();
        if (!userInfo[player].frist) {
            userInfo[player].frist = true;
            userAmount += 1;
        }
        uint tempPow = coutingPower(amount, coinID[0]);
        uAmount = amount * (10 ** (18 - decimal));
        slotInfo[player][slot].status = true;
        slotInfo[player][slot].power = tempPow;
        slotInfo[player][slot].stakeTime = block.timestamp;
        slotInfo[player][slot].endTime = 1640318400 + 15 days;
        slotInfo[player][slot].debt = coutingDebt();
        slotInfo[player][slot].claimTime = block.timestamp;
        slotInfo[player][slot].deadTime = 1640318400 + 130 days;
        userInfo[player].total += tempPow;
        debt = coutingDebt();
        totalPower += tempPow;
        lastTime = block.timestamp;
