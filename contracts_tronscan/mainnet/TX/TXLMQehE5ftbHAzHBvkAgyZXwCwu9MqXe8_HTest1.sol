...
contract HTest1 is Managable {
	...
	struct User {
		...
		uint16 referees;//直推数量
        uint256 teams;//团队数量
		uint256 refBonus;//十代奖金
		uint256 poolBonus;//全球前四名最佳推荐人奖励
		uint256 teamsBonus;//团队收益奖励
		uint256 investPayouts;//当前可提本金总数
		uint256 payouts;//当前投资已提利息总数
		uint256 depositAmount;//最近一次投资总额
        uint256 depositTop;//最高一次投资总额
		uint256 depositPayouts;//最近一次投资总额已出金金额
		uint40 depositTime;//最近一次投资时间
	}

	struct Uext {
		uint256 directBonus;//直推奖励
	...
	address payable private adminAddr;//项目方收款地址
	...
	uint8[] private refBonusScale; // 直推奖比例
	uint8[] private teamsBonusScale; // 团队奖比例
	uint8[] private poolBonuses; // 前4名分红比例
	uint40 private poolLastDraw = uint40(block.timestamp);
	uint256 public poolCycle; //第几轮发放前4名基金池
	uint256 private poolBalance;//前4名基金池
	mapping(uint256 => mapping(address => uint256)) private poolUsersRefsDepositsSum;//所有用户推荐总额
	mapping(uint8 => address) private poolTop;//前4名地址
