...
contract SFBToken is Context, ITRC20, Ownable {
    ...
    uint256 private constant _tTotal = 21 *10**7 * 10**8;
    ...
    uint256 public numTokensSellToAddToLiquidity = 105* 10**2 * 10**8;

    string private constant _name = 'SFB TOKEN'; 
    string private constant _symbol = 'SFB';
    ...
    function reflectionFromToken(uint256 tAmount, bool deductTransferFee) public view returns(uint256) {
        ...
        if (!deductTransferFee) {
            ...
            (,uint256 rTransferAmount,,,,,,) = _getValues(tAmount, _taxFee, _liquidityFee,_burnFee);
    ...
    function _getValues(uint256 tAmount, uint256 taxFee, uint256 liqFee, uint256 burnFee) private view 
    returns (
        uint256 rAmount, 
        ...
        tFee = tAmount.mul(taxFee).div(1000);
        tLiquidity = tAmount.mul(liqFee).div(1000);
        tBurn = tAmount.mul(burnFee).div(1000);
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        ...
        uint256 tBurn,uint256 rate) = _getValues(amount, takeFee ? _taxFee : 0, takeFee ? _liquidityFee : 0, takeFee ? _burnFee : 0);
