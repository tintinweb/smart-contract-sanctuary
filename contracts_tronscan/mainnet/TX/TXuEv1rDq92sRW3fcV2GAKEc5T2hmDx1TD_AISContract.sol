...
contract Model {
  ...
  uint256[7] internal MODEL_AB_DEPOSIT_LIMIT =[100*decimals,500*decimals,1000*decimals,5000*decimals,10000*decimals,50000*decimals,100000*decimals];
...
contract AISContract is Ownable,Model{
    ...
    uint256 public constant MINIMAL_DEPOSIT = 100*decimals; 
	//Maximum recharge amount
    uint256 public constant MAXIMAL_DEPOSIT = 100000*decimals; 
	
    uint256 public constant DESTORY_LIMIT = 100*decimals; 
    ...
    //Contract start time
    uint256 private constant CREATE_TIME = 1606780800;
    //Activity start time
    uint256 private constant START_TIME = 1606780800;
    ...
    function makeDeposit(address payable ref, uint8 modelType,uint8 payType,uint256 depositAmount) external _checkPoolInit _checkPlayerInit(msg.sender) _updateA3Time _updateA3Status {
        ...
        AISImpl.safeTransferFrom(msg.sender,address(this),depositAmount);
    ...
    function _withdraw(address payable _wallet, uint256 _amount) private {
        require(getBalance() >= _amount, "TRX not enougth");
        AISImpl.safeTransfer(_wallet,_amount);
    }
    //Out this operation
    function takeAwayDeposit(uint256 depositId) external _checkPoolDestory _checkPoolInit _checkPlayerInit(msg.sender) _updateA3Time _updateA3Status returns (uint256) {
        ...
        require(getBalance() >= deposit.amount, "TRX not enought");
        ...
        AISImpl.safeTransfer(msg.sender,deposit.amount);
    ...
    function withdrawReferReward() external _checkPoolDestory _checkPoolInit _checkPlayerInit(msg.sender) _updateA3Time _updateA3Status returns (uint256){
        ...
        AISImpl.safeTransfer(msg.sender,refReward);
    ...
    function checktest() public view returns(bool){
        return getBalance()<DESTORY_LIMIT;
    }
