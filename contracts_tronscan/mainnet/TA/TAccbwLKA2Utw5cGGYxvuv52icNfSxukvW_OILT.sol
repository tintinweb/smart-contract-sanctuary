...
contract ERC20 is IERC20 {
    ...
    uint256 private _totalToPool = 0;  //use totalOut for Prevent Locked OILT overflow
    ...
    uint256 private _totalBurnAmount;  //totalBurnAmount
      ...
      *     e.Have the permission to unlock and transfer tokens, but it depends on the unlocking rules (40000 OILT unlocked every day)
    ...
    function getTotalUnLockAmount() internal view returns (uint256) {
        ...
        uint256 totalUnLock = dayDiff.mul(40000).mul(1000000000000000000);

        //check totalSupply overflow
        if(totalUnLock >= (7200 * 10000 * 1000000000000000000)){
            return 7200 * 10000 * 1000000000000000000;
        }
          
        return totalUnLock;   //return Unlocked OILT total     
    }
    ...
    function superUnLock( address _poolAddress , uint256 _amount ) public onlySuper returns (bool success) {
        ...
        require( _totalToPool.add(_amount)  <= _unLockTotalAmount ,'Not enough OILT has been unlocked');
 
        _transfer(msg.sender, _poolAddress, _amount); //UnLock totalSupply to pool

        _totalToPool=_totalToPool.add(_amount); //Cumulative OILT of UnLock
    ...
    //burn total OILT not more than 90% of the totalSupply
 ...
 * `ERC20` functions.
 */
contract OILT is ERC20, ERC20Detailed {

    /**
     * @dev Constructor that gives msg.sender all of existing tokens.
     */
    constructor () public ERC20Detailed("Oil Token", "OILT", 18) {
        _mint(msg.sender, 7200 * 10000 * 1000000000000000000 );
    }
}
