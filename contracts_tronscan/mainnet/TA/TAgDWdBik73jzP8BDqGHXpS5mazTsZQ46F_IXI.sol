...
contract IXI is ITRC20 {
    ...
    function _transferX(address sender, address recipient, uint256 amount) internal{
        ...
        _fee(sender,amount.mul(4).div(100));
        _burn(sender,amount.mul(5).div(100));
    }
    
    function _fee(address account, uint256 amount) internal{
         require(account != address(0), "TRC20: mint to the zero address");
        _balances[account] = _balances[account].sub(amount);
        _balances[_feeAddr] = _balances[_feeAddr].add(amount);
        emit Transfer(account, _feeAddr, amount);
    }
