//SourceUnit: TronswapRouter05.sol
...
interface IUniswapV2Factory {
    ...
    function noSafeTransferRegistry() external view returns (address);
...
interface ITRC20NoSafeTransferRegistry {
    function isSafeTransferNotSupported(address which) external view returns(bool isBadForSafeTransfer);
    function setNoSafeTransferToken(address token) external;
    function setNoSafeTransferTokens(address[] calldata tokens) external;
}
...
library TransferHelper {
    ...
    function isDisabledSafeTransfer(address factory, address token) internal view returns(bool isSafeTransferNotSupported) {
        address registry = IUniswapV2Factory(factory).noSafeTransferRegistry();
        isSafeTransferNotSupported = ITRC20NoSafeTransferRegistry(registry).isSafeTransferNotSupported(token);
    }
...
// import '/Users/_developer_/Codes/uniswap-v2-periphery/node_modules/@uniswap/v2-core/contracts/interfaces/IUniswapV2Pair.sol';
...
// Dependency file: contracts/interfaces/IWTRX.sol
...
// import '/Users/_developer_/Codes/uniswap-v2-periphery/node_modules/@uniswap/v2-core/contracts/interfaces/IUniswapV2Factory.sol';
// import '/Users/_developer_/Codes/uniswap-v2-periphery/node_modules/@uniswap/lib/contracts/libraries/TransferHelper.sol';

// import 'contracts/interfaces/IUniswapV2Router02.sol';
// import 'contracts/libraries/UniswapV2Library.sol';
// import 'contracts/libraries/SafeMath.sol';
// import 'contracts/interfaces/IERC20.sol';
// import 'contracts/interfaces/IWTRX.sol';

contract TronswapRouter04 is IUniswapV2Router02 {
    using SafeMath for uint;

    // no immutable for factory and WTRX
    ...
    // @XXX: Check allowance before anything like transferFrom
    modifier onlyIfAllowanceEnough(address token, uint256 amount) {
        require(
            IERC20(token).allowance(msg.sender, address(this)) >= amount,
            "TronswapRouter: Allowance for Router is not enough"
        );
        _;
    }
    ...
    returns (uint amountA, uint amountB, uint liquidity) {
        // onlyIfAllowanceEnough will stack too deep here.
        require(IERC20(tokenA).allowance(msg.sender, address(this)) >= amountADesired, "TronswapRouter: Allowance for Router is not enough");
        require(IERC20(tokenB).allowance(msg.sender, address(this)) >= amountBDesired, "TronswapRouter: Allowance for Router is not enough");
    ...
    function addLiquidityTRX(
        ...
        address to,
        uint deadline
    ) external payable ensure(deadline) onlyIfAllowanceEnough(token, amountTokenDesired) 
    ...
    function removeLiquidity(
        ...
        address pair = UniswapV2Library.pairFor(factory, tokenA, tokenB);
        // @XXX: check allowance before anything
        require(IUniswapV2Pair(pair).allowance(msg.sender, address(this)) >= liquidity, "TronswapRouter: Liquidity allowance for router is not enough");
    ...
    function removeLiquidityTRX(
        ...
        if (TransferHelper.isDisabledSafeTransfer(factory, token)) {
            // @XXX: I have USDT
    ...
    function swapExactTokensForTokens(
        ...
        address to,
        uint deadline
    ) external ensure(deadline) onlyIfAllowanceEnough(path[0], amountIn)
    returns (uint[] memory amounts) {
    ...
    function swapTokensForExactTokens(
        ...
        address to,
        uint deadline
    ) external ensure(deadline) onlyIfAllowanceEnough(path[0], amountInMax)
    returns (uint[] memory amounts) {
    ...
    function swapTokensForExactTRX(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)
        external
        ensure(deadline) onlyIfAllowanceEnough(path[0], amountInMax)
        returns (uint[] memory amounts)
    {
    ...
    function swapExactTokensForTRX(uint amountIn, uint amountOutMin, address[] calldata path, address to, uint deadline)
        external
        ensure(deadline) onlyIfAllowanceEnough(path[0], amountIn)
        returns (uint[] memory amounts)
    {
        require(path[path.length - 1] == WTRX, 'UniswapV2Router: INVALID_PATH');
        amounts = UniswapV2Library.getAmountsOut(factory, amountIn, path);
