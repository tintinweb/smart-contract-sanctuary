...
//SourceUnit: TDFMiningPool.sol

/*! TDFMiningPool.sol | © 2020 TRXD FINANCE (trxd.finance) */
...
contract TDFMiningPool {
  using SafeMath for uint256;

  struct User {
    uint256 tdf_userBalance;
    uint256 staked;
    uint256 dividends;
    uint256 entries;
    uint256 unfreezing;
  ...
  event WithdrawTDF(address indexed addr, uint256 amount);

  event DepositTDF(address indexed addr, uint256 amount);
  ...
  */
  ...
  function deposit() external payable {
    ...
    if (msg.value / roundRate >= roundRemaining) {
      ...
      TDFToken(token_address).mint(address(this), (msg.value).div(roundRate));
      user.tdf_userBalance = user.tdf_userBalance.add((msg.value).div(roundRate));
  ...
  function _updateMiningRate(address _user, uint256 _depAmount) internal {
    ...
    TDFToken(token_address).mint(address(this), addToBalance);
    user.tdf_userBalance = user.tdf_userBalance.add(addToBalance);
  ...
  function freeze(uint256 _amount) public {
    ...
    require(user.tdf_userBalance >= _amount, "Token balance not enough / Invalid token amount to freeze");
    ...
    user.tdf_userBalance = user.tdf_userBalance.sub(_amount);
  ...
  function claimUnfreeze() public {
    ...
    user.unfreezing = 0;
    user.tdf_userBalance = user.tdf_userBalance.add(amountUnfreezing);
  ...
  function withdrawTDF(uint256 _amount) external {
    User storage user = users[msg.sender];
    require(user.tdf_userBalance >= _amount, "Unavailable amount / Invalid balance for withdrawal");

    user.tdf_userBalance = user.tdf_userBalance.sub(_amount);
    TDFToken(token_address).transfer(msg.sender, _amount);

    emit WithdrawTDF(msg.sender, _amount);
  }

  function depositTDF(uint256 _amount) external {
   
    TDFToken(token_address).transferFrom(msg.sender, address(this), _amount);

    User storage user = users[msg.sender];
    user.tdf_userBalance = user.tdf_userBalance.add(_amount);

    emit DepositTDF(msg.sender, _amount);
  }
  ...
  function exchangeReserves(uint256 _amount) external {
    User storage user = users[msg.sender];
    require(user.tdf_userBalance >= _amount, "Invalid exchange amount or unavailable balance");

    require(_amount >= 1e7, "minimum exchange amount of 10 TDF");
    
    uint256 supplyCoverage = TDFToken(token_address).totalSupply().mul(coveragePercentage).div(1000);
    uint256 payoutExchangeRate = reserveBalance.div(supplyCoverage);

    user.tdf_userBalance = user.tdf_userBalance.sub(_amount);

    TDFToken(token_address).burn(_amount.mul(burnPortion).div(100));
  ...
  function buyLotteryEntries(uint256 _numberOfEntries) external {
    ...
    User storage user = users[msg.sender];
    require(user.tdf_userBalance >= entryPurchase, "Not enough available TDF tokens for purchase");

    user.tdf_userBalance = user.tdf_userBalance.sub(entryPurchase);
  ...
  function promoLotteryBuy(uint256 _numberOfEntries) external {
    ...
    User storage user = users[msg.sender];
    require(user.tdf_userBalance >= promoPurchase, "Not enough available TDF tokens for purchase");

    user.tdf_userBalance = user.tdf_userBalance.sub(promoPurchase);
