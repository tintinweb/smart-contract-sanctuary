...
/*
 *	TRON CITY - investment platform based on TRX blockchain smart-contract technology. Safe and legit!
 *
 *   ┌───────────────────────────────────────────────────────────────────────┐  
 *   │   Website: https://TRON-CITY.com                                      │
 *   │                                                                       │  
 *   │   Telegram Live Support: https://t.me/TRONCITYsupport                 |
 *   │   Telegram Public Group: https://t.me/TRONCITYgroup                   |
 *   │   Telegram News Channel: https://t.me/TRONCITYnews                    |
 *   |                                                                       |
 *   |   Twitter: https://twitter.com/TronCity2020                           |
 *   |                                                                       |
 *   |   E-mail: admin@TRON-CITY.com       partner trontower                 |
 *   └───────────────────────────────────────────────────────────────────────┘ 
 *
 *   [USAGE INSTRUCTION]
 *   ────────────────────────────────────────────────────────────────────────
 ...
 *
 *   - Officially registered company name: TRON CITY LIMITED (#16590649)
 *   - Company status: https://beta.companieshouse.gov.uk/company/120245
 *   - Certificate of incorporation: https://TRON-CITY.com/img/certificate.pdf
 *
 *
 *   ────────────────────────────────────────────────────────────────────────
 *
...
contract TRONCITY is Pausable{
    ...
    uint256[] public REF_PERCENT = [30, 15, 5, 5];   
    uint256[] public BOX_RATE_DIVIDER = [4160000, 2160000, 1728000, 1440000];
    uint256[] public BOX_PERIOD = [0, 50 days, 28 days, 2 days];
    ...
    function register(address referrer) public {
        require(users[msg.sender].parent == address(0), "Inviter address was set in the TRON CITY network!");
        require(users[referrer].parent != address(0) || referrer == owner(), "Inviter address does not exist in the TRON CITY network!");
    ...
    function reinvest(uint256 amount, uint256 boxId, string memory predict) public whenNotPaused {
        ...
        address addr = msg.sender;
        require(users[addr].parent != address(0), "the address does not exist in the TRON CITY network!");
