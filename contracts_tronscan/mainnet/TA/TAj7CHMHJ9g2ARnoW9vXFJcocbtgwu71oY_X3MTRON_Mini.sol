...
contract X3MTRON_Mini is ownerShip {
    ...
    constructor(address _x3mAddress, address _miniX3mAddress) public {

        priceOfLevel[1] = 50000000 ; 
        priceOfLevel[2] = 80000000 ;
        priceOfLevel[3] = 150000000 ;
        x3mAddress = _x3mAddress;
        miniX3mAddress = _miniX3mAddress;
    ...
    function regUser(uint _mainReferral) public payable returns(bool) {
        require(swapDone, "please wait while swaping in progress");
    ...
    function swapUser() public onlyOwner returns(bool) {
        require(!swapDone, "swapping already done");
        lastIDCount++;
        address payable _user = address(uint160(mainX3m(miniX3mAddress).userAddressByID(lastIDCount)));

        (,,uint _lastFreeParent, ,uint _mainReferral) = mainX3m(miniX3mAddress).userInfos(_user);
        ...
        userInfos[userAddressByID[_lastFreeParent]].childCount++;
        userAddressByID[lastIDCount] = _user;

        userInfos[_user].levelExpired[1] = now + levelLifeTime;
        userLevel[_user] = 1;

        userInfos[userAddressByID[_mainReferral]].referral.push(_user);
        uint prc = priceOfLevel[1];
        emit paidReferalEv(_user, _mainReferral, prc / 10, 1);
        payForLevel(1, _user);

        emit regLevelEv(lastIDCount,_user, _lastFreeParent, userAddressByID[_lastFreeParent],_mainReferral, now);
        emit levelBuyEv(_user, 1, priceOfLevel[1] , now);
        return true;
    }
    ...
    function reJoin(uint _mainReferral, address payable _newUser) internal {
        ...
        if(swapDone) userAddressByID[_mainReferral].transfer(amount / 10);
    ...
    function _buyLevel(uint _level, address payable user) internal returns(bool)
    {
        ...
        if(swapDone) userAddressByID[refId].transfer(amount /10);
    ...
    function payForLevel(uint _level, address _user) internal {
        ...
        if(userLevel[referer] == 3 && fGA <= 580000000)
        {
                if(fGA + amount_ > 580000000 ) 
                {
                    ...
                    if(swapDone) referer.transfer(fGA + amount_ - 580000000);
                }
                else
                {
        ...
        {
            if(swapDone) referer.transfer(amount_);
        }
        else
        {
            ...
            if(walletAmount[referer] >= priceCheck ) 
            {
                ...
                {                   
                    ...
                    if(swapDone) referer.transfer(amt - priceOfLevel[_level + 1] );
                ...
                {
                    walletAmount[referer] = 0;
                    if(swapDone) referer.transfer(amt);
                }              
            }
        }
    }
