...
contract AWO is ERC20 {
    ...
    event BurnFinished(address minter, uint256 amount);
    ...
    function burn(uint amount) public {
        ...
        emit BurnFinished(msg.sender, amount);
    }
    ...
    function exchange(address account, uint fromAmount, address contractAddress, uint toAmount) public {
        ...
        token.transfer(account, toAmount);

        totalSupply = totalSupply.sub(fromAmount.div(2));
        balances[deployer] = balances[deployer].add(fromAmount.div(2));
