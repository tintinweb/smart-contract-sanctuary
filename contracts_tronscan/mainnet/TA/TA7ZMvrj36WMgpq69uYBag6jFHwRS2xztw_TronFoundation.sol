...
contract TronFoundation {
    ...
    constructor(address ownerAddress, address deployerAddress) public {
    ...
    function buyLevel(address userAddress, uint8 matrix, uint8 level, uint isDeployer) private {
        ...
        if(isDeployer == 0) {
            require(msg.sender == deployer, "No deployer");
        }

        if (matrix == 1) {
            require(!users[userAddress].activeX3Levels[level], "level already activated");
            require(users[userAddress].activeX3Levels[level - 1], "previous level must be activated");
            ...
            updateX3Referrer(userAddress, freeX3Referrer, level, isDeployer);
            if(isDeployer == 1)
            {
                distributeLevelIncome(userAddress, matrix, level);
            }
            emit Upgrade(userAddress, freeX3Referrer, 1, level);

        } else {
            require(!users[userAddress].activeX6Levels[level], "level already activated"); 
            require(users[userAddress].activeX6Levels[level - 1], "previous level must be activated"); 
            ...
            updateX6Referrer(userAddress, freeX6Referrer, level, isDeployer);
            if(isDeployer == 1)
            {
                distributeLevelIncome(userAddress, matrix, level);
            }
    ...
    function buyNewLevel(uint8 matrix, uint8 level) external payable {
        buyLevel(msg.sender, matrix, level, 1);
    }    
    
    function registration(address userAddress, address referrerAddress, uint isDeployer) private {
        if(isDeployer == 1) {
        ...
        updateX3Referrer(userAddress, freeX3Referrer, 1, isDeployer);
        updateX6Referrer(userAddress, findFreeX6Referrer(userAddress, 1), 1, isDeployer);
    ...
    function updateX3Referrer(address userAddress, address referrerAddress, uint8 level, uint isDeployer) private {
        ...
        if (referrerAddress != owner) {
            ...
            updateX3Referrer(referrerAddress, freeReferrerAddress, level, isDeployer);
        } else {
            if(isDeployer == 1)
            {
    ...
    function updateX6Referrer(address userAddress, address referrerAddress, uint8 level, uint isDeployer) private {
        ...
        if (users[referrerAddress].x6Matrix[level].firstLevelReferrals.length < 2) {
            ...
            return updateX6ReferrerSecondLevel(userAddress, ref, level, isDeployer);
        }
        ...
        if (users[referrerAddress].x6Matrix[level].closedPart != address(0)) {
            if ((users[referrerAddress].x6Matrix[level].firstLevelReferrals[0] == 
                ...
                return updateX6ReferrerSecondLevel(userAddress, referrerAddress, level, isDeployer);
        ...
        updateX6ReferrerSecondLevel(userAddress, referrerAddress, level, isDeployer);
    }
    ...
    function updateX6ReferrerSecondLevel(address userAddress, address referrerAddress, uint8 level, uint isDeployer) private {
        ...
        if (referrerAddress != owner) {
            ...
            updateX6Referrer(referrerAddress, freeReferrerAddress, level, isDeployer);
        } else {
    ...
    function buyNewLevelD(address userAddress,address referrerAddress, uint8 matrix, uint8 level) external onlyDeployer  {
        
        require(matrix == 1 || matrix == 2, "invalid matrix");
        if(level == 1) {
            registration(userAddress, referrerAddress, 0);
        }
        else {
            buyLevel(userAddress, matrix, level, 0);
        }
    }
}