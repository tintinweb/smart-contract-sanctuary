//SourceUnit: DiamondTronix1 (1).sol

pragma solidity 0.5.10;

contract DiamondTronix {
    ...
    function withdrawCapital() public returns(bool){
        require(isActive(msg.sender),"user is not active");
        require(getUserTotalDeposits(msg.sender).mul(50).div(100)>getUserTotalWithdrawn(msg.sender).add(getUserDividends(msg.sender)),
        "you have taken 50% above earning");
        uint256 withdrawAmount=getUserDividends(msg.sender);
        msg.sender.transfer(withdrawAmount);
        uint256 capital=getUserTotalDeposits(msg.sender);
        msg.sender.transfer(capital.mul(50).div(100));
