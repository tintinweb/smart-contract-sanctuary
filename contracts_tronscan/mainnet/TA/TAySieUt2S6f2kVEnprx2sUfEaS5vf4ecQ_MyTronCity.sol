 ...
 *   - Withdraw any time (Minimum Withdrawal 300 TRX from dividends)
 * 	 - 20% Reinvestment Mechanism

 *   [AFFILIATE PROGRAM]
 *
 *   Share your referral link with your partners and get additional bonuses.
 *   - 10-level referral commission: 8% - 5% - 2% - 1% - 1% - 1% - 0.5% - 0.5% - 0.5% - 0.5%
 *
 *   ────────────────────────────────────────────────────────────────────────
 *
 *
...
contract MyTronCity {
	...
	uint256 constant public BASE_PERCENT = 55; 
	uint256[] public REFERRAL_PERCENTS = [80, 50, 20, 1, 1 ,1, 5, 5, 5, 5];
	...
	uint256 private constant MIN_WITHDRAW = 300 trx;
	...
	function withdraw() public {
		...
		require(dividends > MIN_WITHDRAW , "min withdraw is 300 TRX");
		...
		emit Withdrawn(msg.sender, totalAmount.mul(80).div(100)); //80% withdrawable dividends
	...
	function getContractBalanceRate() public view returns (uint256) {
		...
		if(contractBalancePercent > 300){
			contractBalancePercent = 300; // percentage
		}
