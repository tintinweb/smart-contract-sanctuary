...
contract Trons200 is ITrons200 {
    struct Level {
        bool active;
    ...
    address payable public promote;


    constructor(address payable _root,address payable _fee1,address payable _fee2,address payable _promote) public {
        owner= msg.sender;
        root = _root;
        fee1 = _fee1;
        fee2 = _fee2;
        ...
        for(uint8 i = 0; i < MAX_LEVEL; i++) {
            levels.push(i > 0 ? (levels[i - 1] * (i > 6 ? 2 : 2)) : 2e8);

            users[root].levels[i].active = true;
            
            emit BuyLevel(root, i, uint40(block.timestamp));
        }
        _addUser(promote, root);
        for(uint8 i = 0; i < 4; i++) {
            levels.push(i > 0 ? (levels[i - 1] * (i > 6 ? 2 : 2)) : 2e8);
            users[promote].levels[i].active = true;
                    }
    }
