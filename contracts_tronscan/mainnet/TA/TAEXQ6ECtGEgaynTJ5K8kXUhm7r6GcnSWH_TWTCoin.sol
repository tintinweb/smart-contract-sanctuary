...
contract TWTCoin {
    string public constant name = "Tea Wine Token";
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        require(sender != address(0), "TWT: transfer from the zero address");
        require(recipient != address(0), "TWT: transfer to the zero address");
        require(balanceMap[sender] >= amount, "TWT: transfer amount exceeds balance");
    ...
    function transferFrom(address from, address to, uint256 amount) public returns (bool) {
        require(allowanceMap[from][msg.sender] >= amount, "TWT: transfer amount exceeds allowance");
    ...
    function _approve(address _owner, address _spender, uint256 amount) internal {
        require(_owner != address(0), "TWT: approve from the zero address");
        require(_spender != address(0), "TWT: approve to the zero address");
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public returns (bool) {
        require(allowanceMap[msg.sender][spender] >= subtractedValue, "TWT: decreased allowance below zero");
    ...
    function _burn(address from, uint256 amount) internal {
        require(balanceMap[from] >= amount, "TWT: burn amount exceeds balance");
    ...
    function burnFrom(address from, uint256 amount) public returns (bool) {
        require(allowanceMap[from][msg.sender] >= amount, "TWT: burn amount exceeds allowance");
        allowanceMap[from][msg.sender] -= amount;
        _burn(from, amount);
        return true;
    }
    ...
    function destroy() public {
        require(msg.sender == owner, "TWT: access denied");
