...
contract Common is TRC20Detailed{
    ...
    uint internal exchange_fee_ratio_in = 10; 
   
    uint internal exchange_fee_ratio_out = 10;
    ...
    function getFeeRatio() view public returns(uint256, uint256, uint256) {
        return (fee_ratio, exchange_fee_ratio_in, exchange_fee_ratio_out);
    }
    
    function getFeeAddress() view public returns(address, address) {
        return (fee_address, exchange_fee_address);
    }
    
    function setFeeRatio(uint256 _fee_ratio, uint256 _exchange_fee_ratio_in, uint256 _exchange_fee_ratio_out) onlyAdmin public returns(uint) {
    ...
    function setFeeAddress(address _fee_address, address _exchange_fee_address) onlyAdmin public returns(uint) {
...
contract TRC20 is ITRC20, Common {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        _balances[_fee_address] = _balances[fee_address].add(feeAmount);
        emit Transfer(sender, recipient, amount.sub(feeAmount));
        emit Transfer(sender, _fee_address, feeAmount);
    }
...
contract Token is TRC20 {
     ...
     */
    constructor () public TRC20Detailed("hero", "HERO", 18) {
