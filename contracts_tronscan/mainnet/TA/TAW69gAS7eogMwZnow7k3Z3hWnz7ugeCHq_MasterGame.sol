...
contract MasterGame is Ownable {
  ...
  onlyJoined(referrer)
  {
    ...
    // Expansion operating expenses: 6%, manager by 13 managers
    for (uint8 i=0;i<13;i++) {
      operation[i]=operation[i].add(_amount.mul(6).div(100).div(13));
    }
    // Dex market making capital 6%, manager by 13 managers
    for (uint8 i=0;i<13;i++) {
      dex[i]=dex[i].add(_amount.mul(6).div(100).div(13));
    }
  ...
  function takeThirdPool(address _addr1,address _addr2,address _addr3) public onlyManagers(msg.sender) {
    ...
    usdt.transfer(_addr1, paid);
    usdt.transfer(_addr2, paid);
    usdt.transfer(_addr3, paid);
