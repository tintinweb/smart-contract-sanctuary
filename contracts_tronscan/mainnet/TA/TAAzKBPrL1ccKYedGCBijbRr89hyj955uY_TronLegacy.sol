...
contract TronLegacy {
    ...
    uint private releaseTime=1626318000;
    ...
    function deposit(address payable _affAddr) public payable  returns(bool){
        require(block.timestamp >= releaseTime, "not time yet!");
        ...
        players[msg.sender].time = block.timestamp;
            ...
            if(players[msg.sender].deposits[pa].amount > 0){
                ...
                for(uint i=0 ; i< pos;i++){
                    ...
                    if(players[msg.sender].deposits[r].amount ==0){
                        players[msg.sender].deposits[r] = Deposit(msg.value,block.timestamp,block.timestamp+ 4 days,0,rate);
                        return true;
                    }
                    t++;
                }
            }else{
                players[msg.sender].deposits[pa] = Deposit(msg.value,block.timestamp,block.timestamp + 4 days,0,rate);
                return true;
            }
        }else{
            players[msg.sender].deposits[p] = Deposit(msg.value,block.timestamp,block.timestamp+ 4 days,0,rate);
            return true;
        }
    }
    function withdraw(uint pos) public returns(uint256){
        ...
        if( player.deposits[pos].tmp < block.timestamp){
            ...
            uint secPassed = block.timestamp-player.deposits[pos].time;
            pft = (secPassed*(por/24/60/60));
            require(pft< address(this).balance, 'not balance system');
            player.deposits[pos].time = block.timestamp;
