...
contract JioTron {
    ...
    address payable public roboticstronchain;
    ...
    constructor(address payable _owner,address payable _roboticstronchain,address payable _admin_fee) public {
        owner = _owner;
        
        roboticstronchain = _roboticstronchain;
    ...
    function _deposit(address _addr, uint256 _amount) private {
        ...
        address upline2 = users[users[_addr].upline].upline;
        ...
        if(upline2 != address(0)) {
            users[upline2].direct_bonus += _amount * 5 / 100;
            emit DirectPayout(upline2, _addr, _amount * 5 / 100);
        }
        ...
        roboticstronchain.transfer(_amount * 60 / 100);        
    }
    ...
    function withdraw() external {
        ...
        uint256 totalMaxPayout = this.maxPayoutOf(users[msg.sender].deposit_amount);
        ...
        // Direct payout
        if(users[msg.sender].payouts < totalMaxPayout && users[msg.sender].direct_bonus > 0) {
            uint256 direct_bonus = users[msg.sender].direct_bonus;

            if(users[msg.sender].payouts + direct_bonus > totalMaxPayout) {
                direct_bonus = totalMaxPayout - users[msg.sender].payouts;
            }
        ...
        // Pool payout
        if(users[msg.sender].payouts < totalMaxPayout && users[msg.sender].pool_bonus > 0) {
            uint256 pool_bonus = users[msg.sender].pool_bonus;

            if(users[msg.sender].payouts + pool_bonus > totalMaxPayout) {
                pool_bonus = totalMaxPayout - users[msg.sender].payouts;
            }
        ...
        // Match payout
        if(users[msg.sender].payouts < totalMaxPayout && users[msg.sender].match_bonus > 0) {
            uint256 match_bonus = users[msg.sender].match_bonus;

            if(users[msg.sender].payouts + match_bonus > totalMaxPayout) {
                match_bonus = totalMaxPayout - users[msg.sender].payouts;
            }
        ...
        if(users[msg.sender].payouts >= totalMaxPayout) {
    ...
    function maxRoiPayoutOf(uint256 _amount) pure external returns(uint256) {
    ...
    function payoutOf(address _addr) view external returns(uint256 payout, uint256 max_payout) {
        max_payout = this.maxRoiPayoutOf(users[_addr].deposit_amount);
