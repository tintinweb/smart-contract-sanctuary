...
contract B7test is ERC20, ERC20Detailed {
    ...
    constructor () public ERC20Detailed("B7Token", "B7test", 10) {
         _owner = msg.sender;
         _productor= msg.sender;
         _price=1.0;
        // _suanliprice=100* (10 ** uint256(decimals()));
        _lastTime = 1619711999;
        _totalSupply = 200000000 * (10 ** uint256(decimals()));
        _locked = 200000000 * (10 ** uint256(decimals()));
        _release=166666 * (10 ** uint256(decimals()));
    ...
    function issue() public returns (bool){
        require(block.timestamp.sub(_lastTime) > 3600, "It's not time yet");
        require(_locked>_release);
        _locked = _locked.sub(_release);
        uint256 amount1 = _release.div(10).div(24);
        uint256 amount2 = _release .div(24).sub(amount1)  ;
        _balances[_productor] = _balances[_productor].add(amount1);
        _balances[_suanli] = _balances[_suanli].add(amount2);
        uint256 nowturn = _hours/360/24;
        if(nowturn > _turn){
            _turn = nowturn;
            _release = _release*9/10;
        }
        _lastTime = _lastTime.add(3600);
        _hours = _hours.add(1);
    ...
    //usdt->A7
    function buyA7(uint256 usdtAmount) public returns (bool){
        uint256 a7Amount=usdtAmount.div(_price);
        uint256 nodeAmount=usdtAmount.div(10);
        require(_tradeflag==true);
        require(_locked>a7Amount, "It's not enough A7 Token");
        
        require( tokenInterface(USDTToken).transferFrom(msg.sender, address(this), usdtAmount),"token transfer failed");
        ...
        require(tokenInterface(USDTToken).transfer(_productor, nodeAmount),"token transfer failed");
        return true;
    }
     //A7->usdt
    function sellA7(uint256 a7Amount) public returns (bool){
        uint256 usdtAmount=a7Amount.mul(_price).div(10).mul(9);
        uint256 burnAmount=a7Amount.div(10);
        uint256 addAmount=a7Amount.sub(burnAmount);
        require(_tradeflag==true);
        
        require(tokenInterface(USDTToken).balanceOf(address(this)) > usdtAmount,"It's not enough usdt Token");
        _balances[msg.sender]=_balances[msg.sender].sub(a7Amount);
        emit Transfer( msg.sender, address(this),addAmount);
        tokenInterface(USDTToken).transfer(msg.sender, usdtAmount);
        _totalSupply = _totalSupply.sub(burnAmount);
        _locked = _locked.add(addAmount);
        burns.push(Burn(msg.sender,burnAmount));
    ...
    function setA7price(uint a7Amount) public returns (bool){
 ...
 function settradeflag(bool flag) public returns (bool){
        if(msg.sender == _owner){
            _tradeflag=flag;
        }
        return true;
