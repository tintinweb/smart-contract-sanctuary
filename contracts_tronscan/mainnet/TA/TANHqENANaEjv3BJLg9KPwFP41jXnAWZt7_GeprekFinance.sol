...
contract GFIAuthority {
    function canCall(
        address src, address dst, bytes4 sig
    ) public view returns (bool);
}

contract GFIAuthEvents {
    event LogSetAuthority (address indexed authority);
    event LogSetOwner     (address indexed owner);
}

contract GFIAuth is GFIAuthEvents {
    GFIAuthority  public  authority;
    ...
    function setAuthority(GFIAuthority authority_)
    public
    auth
    returns (bool result)
    {
    ...
    function isAuthorized(address src, bytes4 sig) internal view returns (bool) {
        if (src == address(this)) {
            ...
            return true;
        } else if (authority == GFIAuthority(0)) {
            return false;
        } else {
...
contract GFIStop is GFINote, GFIAuth {
    bool public stopped;
...
contract GeprekFinance is GFITokenBase(0), GFIStop {
...
contract GFITokenBase is TRC20, GFIMath {
    uint256                                            _supply;
