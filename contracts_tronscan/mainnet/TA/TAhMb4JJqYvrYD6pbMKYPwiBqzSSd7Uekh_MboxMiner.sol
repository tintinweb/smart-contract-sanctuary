...
contract LpWallet //EMPTY CONTRACT TO HOLD THE USERS assetS
{
    ...
    function TakeBack(address to, address feeOwner,uint256 amounta,uint256 amountb, uint256 feerate) public 
    {
        require(_MainContract==msg.sender);//Only lizmain can do this
        ...
        {
            uint256 mainfee= amounta.mul(feerate).div(100);
            IBEP20(lptoken).transfer(to, amounta.sub(mainfee));
            IBEP20(lptoken).transfer(feeOwner, mainfee);
...
contract MboxMiner is ReentrancyGuard {
    ...
    event TakedBack(address indexed tokenaddress, uint256 amount, uint256 feeamount);
    ...
    function getVipBuyPrice() public view returns (uint256[8] memory) {
        return _vipbuyprice;
    }
    ...
    function TakeBack(address tokenAddress, uint256 pct) public nonReentrant returns (bool) {
        ...
        emit TakedBack(tokenAddress, amounta.mul(99).div(100), amounta.div(100));
        return true;
    }
