...
contract TokexICO {
  ...
  uint public price; //Starts at 2M trx per tokex
  ...
  constructor(address tokenAddress, address _recycle) public {
    ...
    startTime = now;
    remainingBalance = 1000000000000;
    recycle = _recycle;
  }
  ...
  function getTokexForTRX(address referrer) payable public returns (bool) {
    if(startTime + 7 days > now) {
      calculatePrice();
  ...
  function calculatePrice() internal {
    ...
    } else if(startTime +6 days < now) {
      price = 2680191;
    } else if(startTime + 5 days < now) {
      price = 2552563;
    } else if(startTime + 4 days < now) {
      price = 2431012;
