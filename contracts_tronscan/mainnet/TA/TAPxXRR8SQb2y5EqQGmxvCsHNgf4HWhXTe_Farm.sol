...
contract Farm is LPTokenWrapper, IRewardDistributionRecipient {
    ...
    function _updateReferralRelationship(address user, address referrer) internal {
        ...
        if (IterableMapping.contains(referralRelationships, user)) {//referrer has been set
            return;
        }
    ...
    function getReferrer(address account) public view returns (address) {
        return referralRelationships.data[account].value;
    }

    function getReferralRewards(address account) public view returns (uint256) {
        uint256 availableReferralRewards = 0;
        for (uint i = IterableMapping.iterate_start(referralRelationships); IterableMapping.iterate_valid(referralRelationships, i); i = IterableMapping.iterate_next(referralRelationships, i))
        {
            address user;
            address referrer;
            (user, referrer) = IterableMapping.iterate_get(referralRelationships, i);
            if (referrer == account) {
                uint256 reward = earned(user);
                if (reward > 0) {
                    availableReferralRewards = availableReferralRewards.add(reward.mul(10).div(100));
                }
            }
        }
    ...
    function getReward() public updateReward(msg.sender) checkStart {
        ...
        if (reward > 0) {
            ...
            if (IterableMapping.contains(referralRelationships, msg.sender))
            {
                address referrer = referralRelationships.data[msg.sender].value;
                uint256 referrerFee = reward.mul(25).div(100);
    ...
    updateReward(address(0))
    {
        if (block.timestamp >= periodFinish) {
            ...
            poolInfo.totalReward = reward.add(reward.mul(25).div(100));
