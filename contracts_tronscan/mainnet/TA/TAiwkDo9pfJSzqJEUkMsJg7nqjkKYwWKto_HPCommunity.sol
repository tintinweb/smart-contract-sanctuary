...
contract HPCommunityCore {
    ...
    function _deposit(address _addr, uint256 _amount) private {
        ...
        hpcommunityInsurance.transfer((_amount * insuranceFee) / 100);
        admin.transfer((_amount * adminFee) / 100);
    }
    ...
    function toggleWitdraw(bool enable) external {
        require(msg.sender == owner || msg.sender == initializer, "not authorized");
        isWithdraw = enable;
    }
    
    function toggleDeposit(bool enable) external {
        require(msg.sender == owner || msg.sender == initializer, "not authorized");
        isDeposit = enable;
    }
    ...
    function setAdmin(uint8 _percent) external {
        require(msg.sender == owner || msg.sender == initializer, "not authorized");
        adminFee = _percent;
    }
    ...
    function withdraw() payable external {
        require(isWithdraw, "no_withdraw");
...
contract HPCommunity is HPCommunityCore {
    ...
    function syncInfo(uint256 totalUsers, uint256 totalDeposited, uint256 totalWithdraw, uint256 totalTopSponsor, uint256 totalDailyIncome, uint256 totalNetwork) external {
