...
contract Doges is ITRC721, ITRC165, ITRC721Metadata, ITRC721Enumerable {
    ...
    function mint() external payable reentrancyGuard mintingEnabled returns (uint256[MAXIMUM_MINT_LIMIT] memory) {
        ...
        uint256 amountOfDogesToMint = Utils.min(MAXIMUM_MINT_LIMIT, Utils.min(msg.value/mintingPrice, (MINTING_LIMIT - totalMinted)));
        uint256 totalMintPrice = mintingPrice * amountOfDogesToMint;
        
        if (msg.value > totalMintPrice) {
            payable(msg.sender).transfer(msg.value - totalMintPrice);
        }

        for (uint dogeIndex = 0; dogeIndex < amountOfDogesToMint; dogeIndex++) {
            mintedIndexes[dogeIndex] = _mint(msg.sender);
            if (totalMinted % DENOMINATOR_FOR_FREE_DOGE == 0) {
    ...
    function bidOnLot(uint256 _tokenId) external payable marketEnabled {
        ...
        emit BidOnDoge(_tokenId, mb.bidder, mb.seller, msg.value);
    }
    ...
    event BidOnDoge(uint256 indexed _tokenId, address indexed _bidder, address indexed _seller, uint256 _bidValue);
