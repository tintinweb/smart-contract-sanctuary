...
contract TrxMoltenStore is Pool {
    ...
    function deposit(address _referrer) external payable {
        ...
        if (referrer != address(0x0)) {
            ...
            if (joinAmount(referrer) >= MIN_AMOUNT_COMM_JOIN_AMOUNT && allInviteeJoinAmount(referrer) >= MIN_AMOUNT_COMM_AMOUNT) {
    ...
    function updateAllInviteeReward(address account, uint256 reward) private  {
        ...
        while(parent != address(0x0) && dis <= 11) {
            dis = users[account].deep - users[parent].deep;

            if (dis == 1) {
                users[parent].allInviteeReward = users[parent].allInviteeReward
                    .add(reward.mul(1e20).mul(30).div(1e2));
            } else if (dis == 2) {
                users[parent].allInviteeReward = users[parent].allInviteeReward
                    .add(reward.mul(1e20).mul(3).div(1e2));
            } else if (dis == 3) {
                users[parent].allInviteeReward = users[parent].allInviteeReward
                    .add(reward.mul(1e20).mul(15).div(1e4));
            } else if (dis == 4) {
                users[parent].allInviteeReward = users[parent].allInviteeReward
                    .add(reward.mul(1e20).mul(45).div(1e6));
            } else if (dis == 5) {
                users[parent].allInviteeReward = users[parent].allInviteeReward
                    .add(reward.mul(1e20).mul(135).div(1e8));
            } else if (dis == 6) {
                users[parent].allInviteeReward = users[parent].allInviteeReward
                    .add(reward.mul(1e20).mul(405).div(1e10));
            } else if (dis == 7) {
                users[parent].allInviteeReward = users[parent].allInviteeReward
                    .add(reward.mul(1e20).mul(1215).div(1e12));
            } else if (dis == 8) {
                users[parent].allInviteeReward = users[parent].allInviteeReward
                    .add(reward.mul(1e20).mul(3645).div(1e14));
            } else if (dis == 9) {
                users[parent].allInviteeReward = users[parent].allInviteeReward
                    .add(reward.mul(1e20).mul(10935).div(1e16));
            } else if (dis == 10) {
                users[parent].allInviteeReward = users[parent].allInviteeReward
                    .add(reward.mul(1e20).mul(32805).div(1e18));
            } else if (dis == 11) {
                users[parent].allInviteeReward = users[parent].allInviteeReward
                    .add(reward.mul(1e20).mul(98415).div(1e20));
            }
    ...
    function allInviteeList(address account) private view returns(address[] memory, uint256) {
        address[] memory stack = new address[](allUser.length);
        uint256 len;

        address[] memory all = new address[](10000);
        uint256 len1;
        ...
        while (len > 0) {
            ...
            if (len1 > 9998 && users[temp].deep.sub(users[account].deep) >= 11) {
                break;
            }
    ...
    function allInviteeJoinAmount(address account) private view returns(uint256) {
        address[] memory invitee = users[account].invitee;

        uint256 amount;
        for (uint256 i = 0; i < invitee.length; i++) {
            amount = amount.add(joinAmount(invitee[i]));
        }
    ...
    function allInviteReward(address account) public view returns(uint256) {
        return users[account].allInviteeReward.div(1e20).add(users[account].systemReward);
    }
    ...
    function userRank(address account) external view returns(uint256) {
        address[] memory _allUser = allUser;
        uint256 len = _allUser.length;
        for (uint256 i = 0; i < len; i++) {
            for (uint256 j = 0; j < len - i - 1; j++) {
                if (joinAmount(_allUser[j]) < joinAmount(_allUser[j + 1])) {
                    address temp = _allUser[j + 1];
                    _allUser[j + 1] = _allUser[j];
                    _allUser[j] = temp;
                }
            }

            if (_allUser[len - i - 1] == account) {
                return len - i;
            }
        }
    }
