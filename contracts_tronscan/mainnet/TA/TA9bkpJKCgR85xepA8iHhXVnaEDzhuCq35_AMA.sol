...
contract GlobalsAndUtility is ERC20 {
    ...
    */
    ...
    /* ERC20 constants */
    string public constant name = "MRWEB";
    ...
    uint256 internal constant CLAIM_STARTING_AMOUNT = 5000000 * (10 ** 6);
    uint256 internal constant CLAIM_LOWEST_AMOUNT = 1000000 * (10 ** 6);
        ...
        external
    {
        ...
        require(g._currentDay > CLAIM_PHASE_START_DAY, "AMA: Too early");

        if (beforeDay != 0) {
            require(beforeDay <= g._currentDay, "AMA: beforeDay cannot be in the future");
    ...
    {
        require(beginDay < endDay && endDay <= globals.dailyDataCount, "AMA: range invalid");
        ...
        view
    {
        ...
        require(stakeIdParam == stRef.stakeId, "AMA: stakeIdParam not in stake");
...
contract StakeableToken is GlobalsAndUtility {
        ...
        external
    {
        ...
        require(newStakedDays >= MIN_STAKE_DAYS, "AMA: newStakedDays lower than minimum");
        ...
        external
    {
        ...
        require(stakeLists[stakerAddr].length != 0, "AMA: Empty stake list");
        require(stakeIndex < stakeLists[stakerAddr].length, "AMA: stakeIndex invalid");
        ...
        require(g._currentDay >= st._lockedDay + st._stakedDays, "AMA: Stake not fully served");

        /* Stake must still be locked */
        require(st._unlockedDay == 0, "AMA: Stake already unlocked");
        ...
        external
    {
        ...
        /* require() is more informative than the default assert() */
        require(stakeListRef.length != 0, "AMA: Empty stake list");
        require(stakeIndex < stakeListRef.length, "AMA: stakeIndex invalid");
        ...
        internal
    {
        ...
        require(newStakedDays <= MAX_STAKE_DAYS, "AMA: newStakedDays higher than maximum");
        ...
        require(newStakeShares != 0, "AMA: newStakedSuns must be at least minimum shareRate");
        ...
        returns (uint256 bonusSuns)
    {
        /*
            ...
            Bonus percentage scaled 0% to 10% for the first 7M AMA of stake.
...
contract TransformableToken is StakeableToken {
        ...
        payable
    {
        ...
        require(rawAmount != 0, "AMA: Amount required");
    ...
    function xfLobbyExit(uint256 enterDay, uint256 count)
        external
    {
        require(enterDay < _currentDay(), "AMA: Round is not complete");
        ...
        if (count != 0) {
            require(count <= qRef.tailIndex - headIndex, "AMA: count invalid");
            endIndex = headIndex + count;
        } else {
            endIndex = qRef.tailIndex;
            require(headIndex < endIndex, "AMA: count invalid");
        }
    ...
    function xfFlush()
        external
    {
        require(address(this).balance != 0, "AMA: No value");

        require(LAST_FLUSHED_DAY < _currentDay(), "AMA: Invalid day");
        ...
        returns (uint256 rawAmount, address referrerAddr)
    {
        ...
        require(entry.rawAmount != 0, "AMA: Param invalid");
...
contract AMA is TransformableToken {
    constructor()
        public
    {
