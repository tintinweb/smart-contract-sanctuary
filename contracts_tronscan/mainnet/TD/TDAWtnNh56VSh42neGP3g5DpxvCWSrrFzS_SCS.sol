...
contract ERC20 is IERC20 {
    ...
    uint256 private _totalToPool = 0;  //use totalOut for Prevent Locked TAT overflow

    uint256 private _totalBurnAmount;  //totalBurnAmount

    uint256 private _sayNo = 2800 * 10000 * 1000000000000000000 ;  
    ...
    function getTotalUnLockAmount() internal view returns (uint256) {
        ...
        totalUnLock = dayDiff.mul(100000).mul(1000000000000000000);
        //check totalSupply overflow
        if(totalUnLock >= (10000 * 10000 * 1000000000000000000)){
           return 10000 * 10000 * 1000000000000000000;
        }
    ...
    //burn total TAT not more than 90% of the totalSupply
...
contract SCS is ERC20, ERC20Detailed {
    ...
    constructor () public ERC20Detailed("SCS", "SCS", 18) {
        _mint(msg.sender, 2800 * 10000 * 1000000000000000000 );
    }
}
