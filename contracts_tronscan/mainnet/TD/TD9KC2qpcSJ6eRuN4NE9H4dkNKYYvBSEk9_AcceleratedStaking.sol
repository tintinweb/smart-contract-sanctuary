//SourceUnit: AcceleratedStaking.sol
...
contract AcceleratedStaking is Ownable, TokenInfo {
  ...
  uint private _minPrice;
  uint private _maxPrice;
  uint private _period;
  uint private _yield;

  uint private constant _percentBase = 100;

  uint[] private _referralSpeedups;
  ...
  mapping(address => StakeStats) public _stats;

  constructor(
    address token,
    address referralTree//,
    // uint minPrice,
    // uint maxPrice,
    // uint yield,
    // uint period//Dsgbkbk,
    // uint[] memory referralSpeedups
  ) public {
    _token = ITRC20(token);
    ...
    // _yield = yield;
    // _period = period;
    // _referralSpeedups = referralSpeedups;
  }
  ...
  function setParameters (
    ...
    uint yield,
    uint period,
    uint[] calldata referralSpeedups
  ) external onlyOwner {
    _minPrice = minPrice;
    _maxPrice = maxPrice;
    _yield = yield;
  ...
  function referralSpeedups() public view returns (uint[] memory) { return _referralSpeedups; }
  ...
  function push(uint amount) external registeredUser {
    require(amount >= _minPrice, "Token amount too low");
    require(amount <= _maxPrice, "Token amount too high");
    _token.transferFrom(msg.sender, address(this), amount);

    StakeStats storage stats = _stats[msg.sender];
    uint leftToMine = amount.mul(_yield).div(_percentBase);
    _stakes[msg.sender][++stats.total] = Stake(
      uint32(block.timestamp),
      uint32(block.timestamp + _period),
      uint192(leftToMine)
    );

    emit StakeCreated(msg.sender, stats.total, block.timestamp, block.timestamp + _period, amount, leftToMine);

    address parent = _referralTree.parentOf(msg.sender);
    for (uint256 i = 0; i < _referralSpeedups.length; i++) {
      if(_referralTree.isRegistered(parent)) {
        uint speedupAmount = amount.mul(_referralSpeedups[i]).div(_percentBase);
        if(speedupAmount > 0) {
          _boostFor(parent, speedupAmount);
  ...
  function takeProfit() external {
    ...
    for (uint256 i = stats.completed + 1; i <= stats.total; i++) {
      profit = profit.add( _mine(msg.sender, i) );
    }
  ...
  function availableFor(address user) public view returns (uint available) {
    StakeStats storage stats = _stats[user];
    available = 0;
    for (uint256 i = stats.completed + 1; i <= stats.total; i++) {
      available = available.add(
        _availableInStake(_stakes[user][i])
      );
    }
  }

  function _availableInStake(Stake storage stake) private view returns (uint amount) {
    uint256 start = stake.startTime;
    uint256 end = stake.endTime;
    amount = stake.leftToMine;
  ...
  function _mine(address user, uint key) private returns (uint amount) {
    Stake storage stake = _stakes[user][key];
    uint256 start = stake.startTime;
    uint256 end = stake.endTime;
    amount = stake.leftToMine;
    if(block.timestamp > start) {
      if(block.timestamp < end) {
        amount = amount.mul(block.timestamp - start).div(end - start);
        stake.leftToMine = uint192( uint256(stake.leftToMine).sub(amount) );
        stake.startTime = uint32(block.timestamp);
        emit StakeUpdated(user, key, stake.startTime, stake.endTime, stake.leftToMine, amount);
      } else {
        delete _stakes[user][key];
        _stats[user].completed++;
        emit StakeCompleted(user, key);
      }
    } else
      amount = 0;
  }

  event Boosted(address user, uint amount);

  function _boostFor(address user, uint amount) private {
    StakeStats storage stats = _stats[user];
    uint boostedTotal = 0;
    for (uint256 i = stats.completed + 1; i <= stats.total; i++) {
      uint boosted;
      Stake storage stake = _stakes[user][i];
      (amount, boosted) = _boost(stake, amount);
      if(boosted > 0) emit StakeUpdated(user, i, stake.startTime, stake.endTime, stake.leftToMine, 0);
      boostedTotal += boosted;
      if(amount == 0) break;
    }
    if(boostedTotal > 0) emit Boosted(user, boostedTotal);
  }

  function _boost(Stake storage stake, uint amount) private returns (uint residue, uint boosted) {
    uint256 start = stake.startTime;
    uint256 end = stake.endTime;
    uint256 leftToMine = stake.leftToMine;
    if(block.timestamp < end) {
      uint timeLeft = end - block.timestamp;
      // amountLeft/timeLeft = leftToMine/(end-start)
      uint amountLeft = leftToMine.mul(timeLeft).div(end - start);
      if(amountLeft < amount) {
        boosted = amountLeft;
        residue = amount - amountLeft;
        stake.endTime = uint32( block.timestamp );
      } else {
        boosted = amount;
        residue = 0;
        // amount/timeShift = leftToMine/(end-start)
        uint32 timeShift = uint32( amount.mul(end - start).div(leftToMine) );
        stake.startTime -= timeShift;
        stake.endTime -= timeShift;
      }
    } else {
      residue = amount;
      boosted = 0;
