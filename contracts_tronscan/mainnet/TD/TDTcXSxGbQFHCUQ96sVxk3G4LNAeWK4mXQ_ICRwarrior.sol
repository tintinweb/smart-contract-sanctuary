...
contract  ICRwarrior is ITRC721, ITRC165, ITRC721Metadata, ITRC721Enumerable {
    using EnumerableSet for EnumerableSet.UintSet;
    using EnumerableSetMarket for EnumerableSetMarket.MarketLotSet;

    string internal _name = "ICRwarrior";
    string internal _symbol = "ICRW";

    uint256 internal totalMinted = 0;

    uint256 public constant MINTING_LIMIT = 1000;

    uint256 public mintingPrice = 1500000000;
    uint256 public minimumMarketPrice = 1500000000;

    uint256 internal devComissionNom = 1;
    uint256 internal devComissionDenom = 100;

    uint256 internal DENOMINATOR_FOR_FREE_ICRW = 1001;
    ...
    function tokenURI(uint256 _tokenId) external pure override returns (string memory) {
        return string(abi.encodePacked("https://icrwarrior-api.intercroneswap.com/api/getjson/", Utils.uintToString(_tokenId)));
    }
    ...
    function mint() external payable reentrancyGuard mintingEnabled returns (uint256[MAXIMUM_MINT_LIMIT] memory) {
        ...
        uint256 amountOfICRwarriorToMint = Utils.min(MAXIMUM_MINT_LIMIT, Utils.min(msg.value/mintingPrice, (MINTING_LIMIT - totalMinted)));
        uint256 totalMintPrice = mintingPrice * amountOfICRwarriorToMint;
        
        if (msg.value > totalMintPrice) {
            payable(msg.sender).transfer(msg.value - totalMintPrice);
        }

        for (uint icrwIndex = 0; icrwIndex < amountOfICRwarriorToMint; icrwIndex++) {
            mintedIndexes[icrwIndex] = _mint(msg.sender);
            if (totalMinted % DENOMINATOR_FOR_FREE_ICRW == 0) {
    ...
    function bidOnLot(uint256 _tokenId) external payable marketEnabled {
        ...
        emit BidOnicrw(_tokenId, mb.bidder, mb.seller, msg.value);
    }
    ...
    event BidOnicrw(uint256 indexed _tokenId, address indexed _bidder, address indexed _seller, uint256 _bidValue);
