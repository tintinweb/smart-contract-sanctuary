...
contract WorldBurnerGame is Owned {
  ...
  uint public gameLastsFor = 8640; // Initially the game will last for 0.1 days to test.
  uint public round;
  uint public buyIn;
  ...
  function register(address referralAddress) external payable {
      require(!paused && (gameStartedAt == 0 || (now - gameStartedAt) < gameLastsFor));
      ...
      if (streamTo != 0x0) {
        if (div != 0)
          streamTo.transfer(buyIn * 1000000 / div);
      }
