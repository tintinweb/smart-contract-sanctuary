...
contract FastShark {
    ...
    mapping(address=>StakeHolder)public stakeHolderUsers; 
    address payable [] STakeHolderArray;
    ...
    function _deposit(address payable _addr, uint256 _amount) private {
        ...
        require(!stakeHolderUsers[_addr].isExist,"you are stakeHolder and you are already registered");
        ...
        admin_fee.transfer(_amount.mul(5).div(100));
        
        }
        
        function setUpStakeHolder(address payable _add)internal returns(bool){
            // require(stakeHolderUsers[_add].stakeHolderAmount==_amount);
            users[_add]=User({cycle:0,upline:owner,referrals:users[_add].referrals,direct_bonus:users[_add].direct_bonus,payouts:0,pool_bonus:0,
                match_bonus:0,deposit_amount:0,deposit_payouts:0,deposit_time:block.timestamp,total_deposits:0,total_payouts:0,total_structure:0
            });
        }
    ...
    function withdraw() external {
        if(stakeHolderUsers[msg.sender].isExist){
            
            uint256 direct_bonus = users[msg.sender].direct_bonus;
            users[msg.sender].direct_bonus -= direct_bonus;
            msg.sender.transfer(direct_bonus);
    ...
    */
    ...
    function putStakeHolder(address payable _newStakeHolder)public returns(bool){
        require(msg.sender==owner,"Access denied");
        stakeHolderUsers[_newStakeHolder]=StakeHolder({isExist:true});
        setUpStakeHolder(_newStakeHolder);
     return true;
    }
