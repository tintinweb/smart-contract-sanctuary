...
contract TronPeak {
    ...
    function deposit(address _referral) external payable {
        require(msg.value >= 200 trx, "Minimal deposit: 200 TRX");
       ...
       if(player.deposits.length == 0){
        ...
        addr1.transfer(30 trx);
        addr2.transfer(5 trx);
        addr3.transfer(5 trx);
        addr4.transfer(10 trx);
    ...
    function getUserReferralPercent(address userAddress, uint256 level) public view returns (uint256) {
		...
		if (player.total_ref_invested >= 25000000 trx) {
			return referral_bonuses[2][level];
		} else if (player.total_ref_invested >= 10000000 trx) {
