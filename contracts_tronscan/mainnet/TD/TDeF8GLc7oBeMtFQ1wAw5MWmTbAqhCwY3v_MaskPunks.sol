...
contract MaskPunks is Ownable, TRC165, ITRC721Enumerable, IERC721Metadata, ReentrancyGuard {
    ...
    */
    function mintNormal(uint256 number) nonReentrant public payable {
        ...
        for (uint i = 0; i < number; i++) {
            mintIndex = getNextPunkIndex(mintIndex);
    ...
    */
    ...
    function getNextPunkIndex(uint256 seed) private returns(uint256) {

        uint256 n = 0;
        if (punks_index_exists_length > 1){
            n = uint256(keccak256(abi.encodePacked(blockhash(block.number - 1), seed + block.timestamp))) % punks_index_exists_length;
        }
