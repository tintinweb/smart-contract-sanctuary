...
/**
 * @title SimpleToken
 * @dev Very simple ERC20 BTRAVAL example, where all tokens are pre-assigned to the creator.
 * Note they can later distribute these tokens as they wish using `transfer` and other
 * `ERC20` functions.
 */
contract Btraval is ERC20, ERC20Detailed {

    /**
     * @dev Constructor that gives msg.sender all of existing tokens.
     */
    constructor () public ERC20Detailed("BTRAVAL", "BTV", 6) {
        _mint(msg.sender, 10000000000 * (10 ** uint256(decimals())));
    }
}

//SourceUnit: Creater.sol
...
library Creater {
    ...
    function add(uint256 a, uint256 b) internal pure returns (uint256) {
        uint256 c = a + b;
        require(c >= a, "Creater: addition overflow");
    ...
    function sub(uint256 a, uint256 b) internal pure returns (uint256) {
        return sub(a, b, "Creater: subtraction overflow");
    }
    ...
    function mul(uint256 a, uint256 b) internal pure returns (uint256) {
        ...
        uint256 c = a * b;
        require(c / a == b, "Creater: multiplication overflow");
    ...
    function div(uint256 a, uint256 b) internal pure returns (uint256) {
        return div(a, b, "Creater: division by zero");
    }
    ...
    function mod(uint256 a, uint256 b) internal pure returns (uint256) {
        return mod(a, b, "Creater: modulo by zero");
    }
 ...
 * Since it is not possible to overflow a 256 bit integer with increments of one, `increment` can skip the {Creater}
 * overflow check, thereby saving gas. This does assume however correct usage, in that the underlying `_value` is never
 * directly accessed.
 */
library Counters {
    using Creater for uint256;
    ...
    function increment(Counter storage counter) internal {
        // The {Creater} overflow check can be skipped here, see the comment at the top
        counter._value += 1;
    }
