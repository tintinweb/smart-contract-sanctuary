...
contract TronShowLogic is TronShowStorage  {
    using SafeMath for uint256;
    ...
    payFeePool.transfer(SafeMath.mul(SafeMath.div( msg.value, 100), 5));
        address payable payTechPool = address(uint160(techPool));
    payTechPool.transfer(SafeMath.mul(SafeMath.div( msg.value, 100), 3));
    racePool = SafeMath.add(racePool, SafeMath.mul(SafeMath.div(msg.value, 100), 10));
    }
...
//1:每日分红  2:竞赛激励  3:重启发奖  4:邀请奖励
    ...
    function setFoundings(address _add5, address _add3, address _add2)public isOwner {
        feePool = _add5;
        techPool = _add3;
        foundingPool = _add2;
    }
