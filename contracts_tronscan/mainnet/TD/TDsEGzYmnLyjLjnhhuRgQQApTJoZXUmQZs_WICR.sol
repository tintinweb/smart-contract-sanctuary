...
contract WICR is ITokenDeposit {
    string public name = "Wrapped InterCrone";
    string public symbol = "WICR";
    uint8  public decimals = 6;
    trcToken  public icrTokenId = trcToken(1003009);
    ...
    function deposit() public payable {
        require(msg.tokenid == icrTokenId, "deposit tokenId not ICR[1003009]");
    ...
    function withdraw(uint256 sad) public {
        require(balanceOf_[msg.sender] >= sad, "not enough WICR balance");
        require(totalSupply_ >= sad, "not enough WICR totalSupply");
        balanceOf_[msg.sender] -= sad;
        totalSupply_ -= sad;
        msg.sender.transferToken(sad, icrTokenId);
