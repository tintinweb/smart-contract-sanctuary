 ...
 * which is used as a basis for the CSN Project
...
contract ERC20 is Context, TRC20 {
    ...
    // allocating 5 million tokens for promotion, airdrop, liquidity and dev share
...
contract GlobalsAndUtility is ERC20 {
    ...
    */
    ...
    /* ERC20 constants */
    string public constant name = "Community Staking Network";
    ...
    /* Time of contract launch (Nov 19th 2020 00:00:00 UTC) */
    uint256 internal constant LAUNCH_TIME = 1605744000;
        ...
        external
    {
        ...
        require(g._currentDay > CLAIM_PHASE_START_DAY, "CSN: Too early");

        if (beforeDay != 0) {
            require(beforeDay <= g._currentDay, "CSN: beforeDay cannot be in the future");
    ...
    {
        require(beginDay < endDay && endDay <= globals.dailyDataCount, "CSN: range invalid");
        ...
        view
    {
        ...
        require(stakeIdParam == stRef.stakeId, "CSN: stakeIdParam not in stake");
...
contract StakeableToken is GlobalsAndUtility {
        ...
        external
    {
        ...
        require(newStakedDays >= MIN_STAKE_DAYS, "CSN: newStakedDays lower than minimum");
        ...
        external
    {
        ...
        require(stakeLists[stakerAddr].length != 0, "CSN: Empty stake list");
        require(stakeIndex < stakeLists[stakerAddr].length, "CSN: stakeIndex invalid");
        ...
        require(g._currentDay >= st._lockedDay + st._stakedDays, "CSN: Stake not fully served");

        /* Stake must still be locked */
        require(st._unlockedDay == 0, "CSN: Stake already unlocked");
        ...
        external
    {
        ...
        /* require() is more informative than the default assert() */
        require(stakeListRef.length != 0, "CSN: Empty stake list");
        require(stakeIndex < stakeListRef.length, "CSN: stakeIndex invalid");
        ...
        internal
    {
        ...
        require(newStakedDays <= MAX_STAKE_DAYS, "CSN: newStakedDays higher than maximum");
        ...
        require(newStakeShares != 0, "CSN: newStakedSuns must be at least minimum shareRate");
        ...
        returns (uint256 payout)
    {
        ...
        for (uint256 day = beginDay; day < endDay; day++) {
            ...
            dayPayout += ((dailyData[day].dayDividends * 100) / 100) * stakeSharesParam
            / dailyData[day].dayStakeSharesTotal;
        ...
        returns (uint256 bonusSuns)
    {
        /*
            ...
            Bonus percentage scaled 0% to 10% for the first 7M CSN of stake.
...
contract TransformableToken is StakeableToken {
    ...
    function xfLobbyEnter(address referrerAddr)
        ...
        payable
    {
	require(_currentDay() > 0, "CSN: Auction has not begun yet");

        uint256 enterDay = _currentDay();

        uint256 rawAmount = msg.value;
        require(rawAmount != 0, "CSN: Amount required");
    ...
    function xfLobbyExit(uint256 enterDay, uint256 count)
        external
    {
        require(enterDay < _currentDay(), "CSN: Round is not complete");
        ...
        if (count != 0) {
            require(count <= qRef.tailIndex - headIndex, "CSN: count invalid");
            endIndex = headIndex + count;
        } else {
            endIndex = qRef.tailIndex;
            require(headIndex < endIndex, "CSN: count invalid");
        }
    ...
    //     require(address(this).balance != 0, "CSN: No value");

    //     require(LAST_FLUSHED_DAY < _currentDay(), "CSN: Invalid day");

    //     _dailyDataUpdateAuto(g);

    //     FLUSH_ADDR.transfer((dailyData[LAST_FLUSHED_DAY].dayDividends * 0) / 100);
        ...
        returns (uint256 rawAmount, address referrerAddr)
    {
        ...
        require(entry.rawAmount != 0, "CSN: Param invalid");
...
contract CSN is TransformableToken {
    constructor()
        public
    {
