...
contract TronLike5 {
	...
	uint256 constant public  INTENVAL = 24 hours;
	...
	function invest(address referrer) public payable {
		...
		if (user.referrer != address(0)) {
			...
			for (uint256 i = 0; i < 10; i++) {
				if (upline != address(0)) {
					...
					users[upline].referralReward[i] = users[upline].referralReward[i].add(10);
		...
		totalDeposits = totalDeposits.add(10);
