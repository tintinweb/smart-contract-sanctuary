//SourceUnit: oracletxdefi2021.sol
...
contract ERC20 is Context, TRC20 {
    ...
    // allocating 25 million tokens for promotion, airdrop, liquidity and dev share
...
contract GlobalsAndUtility is ERC20 {
    ...
    */
    ...
    /* Flush address */
    address payable internal constant FLUSH_ADDR = 0xf0BFa373e110Fedc17B8464CAb3F974aAb6a5c9B;
    uint8 internal LAST_FLUSHED_DAY = 1;

    /* ERC20 constants */
    string public constant name = "OCTX";
    string public constant symbol = "OCTX";
    uint8 public constant decimals = 8;

    /* Suns per Satoshi = 10,000 * 1e8 / 1e8 = 1e4 */
    uint256 private constant SUNS_PER_OCTX = 10 ** uint256(decimals); // 1e8

    /* Time of contract launch (2021-01-15T00:00:00Z) */
    uint256 internal constant LAUNCH_TIME = 1610668800;
				
    /* Start of claim phase */
    uint256 internal constant PRE_CLAIM_DAYS = 1;
											
    uint256 internal constant CLAIM_STARTING_AMOUNT = 2000000 * (10 ** 8); //2m
											
    uint256 internal constant CLAIM_LOWEST_AMOUNT = 1000000 * (10 ** 8);	//1m
    ...
    uint256 private constant BPB_MAX_OCTX = 7 * 1e6;
    uint256 internal constant BPB_MAX_SUNS = BPB_MAX_OCTX * SUNS_PER_OCTX;
        ...
        external
    {
        ...
        require(g._currentDay > CLAIM_PHASE_START_DAY, "OCTX: Too early");

        if (beforeDay != 0) {
            require(beforeDay <= g._currentDay, "OCTX: beforeDay cannot be in the future");
    ...
    {
        require(beginDay < endDay && endDay <= globals.dailyDataCount, "OCTX: range invalid");
        ...
        view
    {
        ...
        require(stakeIdParam == stRef.stakeId, "OCTX: stakeIdParam not in stake");
...
contract StakeableToken is GlobalsAndUtility {
        ...
        external
    {
        ...
        require(newStakedDays >= MIN_STAKE_DAYS, "OCTX: newStakedDays lower than minimum");
        ...
        external
    {
        ...
        require(stakeLists[stakerAddr].length != 0, "OCTX: Empty stake list");
        require(stakeIndex < stakeLists[stakerAddr].length, "OCTX: stakeIndex invalid");
        ...
        require(g._currentDay >= st._lockedDay + st._stakedDays, "OCTX: Stake not fully served");

        /* Stake must still be locked */
        require(st._unlockedDay == 0, "OCTX: Stake already unlocked");
        ...
        external
    {
        ...
        /* require() is more informative than the default assert() */
        require(stakeListRef.length != 0, "OCTX: Empty stake list");
        require(stakeIndex < stakeListRef.length, "OCTX: stakeIndex invalid");
        ...
        internal
    {
        ...
        require(newStakedDays <= MAX_STAKE_DAYS, "OCTX: newStakedDays higher than maximum");
        ...
        require(newStakeShares != 0, "OCTX: newStakedSuns must be at least minimum shareRate");
        ...
        returns (uint256 bonusSuns)
    {
        /*
            ...
            Bonus percentage scaled 0% to 10% for the first 7M OCTX of stake.
...
contract TransformableToken is StakeableToken {
        ...
        payable
    {
        ...
        require(rawAmount != 0, "OCTX: Amount required");
    ...
    function xfLobbyExit(uint256 enterDay, uint256 count)
        external
    {
        require(enterDay < _currentDay(), "OCTX: Round is not complete");
        ...
        if (count != 0) {
            require(count <= qRef.tailIndex - headIndex, "OCTX: count invalid");
            endIndex = headIndex + count;
        } else {
            endIndex = qRef.tailIndex;
            require(headIndex < endIndex, "OCTX: count invalid");
        }
    ...
    function xfFlush()
        external
    {
        require(address(this).balance != 0, "OCTX: No value");
									  
								   

        require(LAST_FLUSHED_DAY < _currentDay(), "OCTX: Invalid day");
        ...
        returns (uint256 rawAmount, address referrerAddr)
    {
        ...
        require(entry.rawAmount != 0, "OCTX: Param invalid");
    ...
    {
        /* 1095890410958 = ~ 4000000 * SUNS_PER_OCTX / 365 */
...
contract OCTX is TransformableToken {
    constructor()
        public
    {
