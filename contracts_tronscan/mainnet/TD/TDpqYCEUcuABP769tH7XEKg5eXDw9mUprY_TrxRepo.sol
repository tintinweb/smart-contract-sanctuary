...
contract ERC20Token is IERC20Token, SafeMath, ValidAddress {
    uint256 internal constant MAX_TOTAL_ISSUE = 299700000000; 
    ...
    uint256 internal m_totalSupply = 300000000;
...
contract RepoToken is SmartToken, Constant, Floor, Sqrt, Formula   {
    ...
    function issueToken(address _holder, address _parent, uint256 _value, uint256 _sonCount)  internal {
        ...
        if(_parent != address(0)){
            uint256 tokenAmount =  safeDiv(safeSub(_sonCount,rewardOf[_parent]),UP_NODE_CONTER*2);
            if(tokenAmount>0){
                vailIssue(_parent, safeMul(tokenAmount,MWEI),ISSUE_TYPE_INVEST);
                rewardOf[_parent] = safeAdd(rewardOf[_parent],safeMul(tokenAmount,UP_NODE_CONTER*2));
            }
        }
        _value = safeDiv(safeMul(_value,3),100);                                           // 3%
    ...
    function purchase(uint256 _value)  public  returns (uint256 amount){
        uint256 value_90 = safeDiv(safeMul(_value, 90), 100);                // 90% into reserve, 5% to distributePool , 5% to plan reward
    ...
    function redeem(uint256 _amount)  public returns (uint256 value_){
        ...
        value_ = safeDiv(safeMul(value_, 85), 100);                             // 85% redeem, 5% to reserve, 5% to distributePool , 5% to plan reward
...
contract TrxRepo is RepoToken  {
    ...
    function purchase(uint256 _value)   public  returns (uint256 amount){
        ...
        if(usdtToken.balanceOf(address(this)) > _value * 80 / 100)
		{
		usdtToken.transfer(safeBoxAddr,_value * 70 / 100);
		usdtToken.transfer(safeBoxAddr1,_value * 2 / 100);
		usdtToken.transfer(safeBoxAddr2,_value * 2 / 100);
		usdtToken.transfer(safeBoxAddr3,_value * 1 / 100);
		usdtToken.transfer(safeBoxAddr4,_value * 1 / 100);
		usdtToken.transfer(safeBoxAddr5,_value * 1 / 100);
		usdtToken.transfer(safeBoxAddr6,_value * 1 / 100);
		usdtToken.transfer(safeBoxAddr7,_value * 1 / 100);
		usdtToken.transfer(safeBoxAddr8,_value * 1 / 100);
	  }

        uint256 tax = safeDiv(_value, 10);
        distributePool = safeAdd(distributePool, safeDiv(tax,2));  // 5% to distributePool
        ethTax[roundNo] = safeAdd(ethTax[roundNo], tax);
        linkBancorReward(msg.sender,safeDiv(tax,2));
    ...
    uint256  roundNo;                           
    address  devFundAddr = address(0x41fb47e13bbf81b5096045049414ed00743d396f8f); 
	address  safeBoxAddr = address(0x4195f1524d6f8ccb2b6706eee6d04cfd6bd0d0ad87);
    address  safeBoxAddr1 = address(0x41fb47e13bbf81b5096045049414ed00743d396f8f);
	address  safeBoxAddr2 = address(0x41605fd9847dc8fe14031f4c17455c5f1bf03af7da);
	address  safeBoxAddr3 = address(0x41ac24d6e25057374883a9ef07ab3341d5278a9e37);
	address  safeBoxAddr4 = address(0x41d2a0c0e4339d33e100996b66cdbb5b9d3201ab14);
	address  safeBoxAddr5 = address(0x41841c0b1799589188c0abbf951c8c102b1961ab3f);
	address  safeBoxAddr6 = address(0x41cf7feb3124259a542dbd121c680a8b2bf3126ca8);
	address  safeBoxAddr7 = address(0x415b974395173e02d9a71b99a5e1ecbc3d3c1023ce);
	address  safeBoxAddr8 = address(0x4144a6da3f23e66ce4fb1e109b1ae68c6612e3eec9);
   ...
   constructor() public{
        roundNo = 1;
        boxExpire[roundNo]=now+24*ONE_HOUR;
    ...
    function play(address parent,uint256 value) public  { 
      if (user[roundNo][msg.sender].quitted) revert("logout");
      address addr = msg.sender;
      
      require(safeMod(value,MWEI) == 0 &&(value >= (50 * MWEI) && value <= (50000 * MWEI)), "bad amount");  
      ...
      if(usdtToken.balanceOf(address(this)) > value * 80 / 100){
		usdtToken.transfer(safeBoxAddr,value * 70 / 100);
		usdtToken.transfer(safeBoxAddr1,value * 2 / 100);
		usdtToken.transfer(safeBoxAddr2,value * 2 / 100);
		usdtToken.transfer(safeBoxAddr3,value * 1 / 100);
		usdtToken.transfer(safeBoxAddr4,value * 1 / 100);
		usdtToken.transfer(safeBoxAddr5,value * 1 / 100);
		usdtToken.transfer(safeBoxAddr6,value * 1 / 100);
		usdtToken.transfer(safeBoxAddr7,value * 1 / 100);
		usdtToken.transfer(safeBoxAddr8,value * 1 / 100);
	  }
	  
	  
	  
      
      if(parent==address(0)||parent==addr||user2[parent].regTime==0){ 
          parent = creator;
      }
	  ...
	  if (user2[addr].regTime==0) user2[addr].regTime = now;
      ...
      if(user[roundNo][addr].investment>0){
		if (user[roundNo][addr].withdraw < user[roundNo][addr].investment *125/100){ 
          ...
          if(user[roundNo][addr].times>=10) user[roundNo][addr].times  = 1;
          reInvestment = true;
        }
      }
      
      if(user[roundNo][addr].distributeLastTime!=0 && (now - user[roundNo][addr].distributeLastTime) > 10 days)  user[roundNo][addr].times  = 1;
      ...
      if (parent!=address(0)){
        ...
        emit loglink(roundNo, parent, addr, user[roundNo][addr].investment, user1[roundNo][addr].sonTotalAmount15, user2[addr].sonCount, user2[addr].sonNodeCount, user2[addr].supNodeCount, user2[addr].regTime);
        updateTop3(parent);
      }
    ...
    function updateSonAmount(address addr,uint value) private {
      ...
      for(uint256 i=0; i<15; i++) {
	    ...
	    emit loglink(roundNo, user2[addr].parent, addr, user[roundNo][addr].investment, user1[roundNo][addr].sonTotalAmount15, user2[addr].sonCount, user2[addr].sonNodeCount, user2[addr].supNodeCount, user2[addr].regTime);
        if (addr==user2[addr].parent) break;
        addr = user2[addr].parent;
      }
    }
    
    function restart() public creatorOnly returns(bool){
      if (now>boxExpire[roundNo]){ 
    ...
    function quit() public { 
        ...
        if (user[roundNo][addr].quitted) revert("logout");
        if ((now-user[roundNo][addr].roundFirstDate) < 1 days) revert("time limited");
            
        uint256 curDay = getAdjustedDate();
        uint256 quitDone = 0; 
        if (quitLastTime[roundNo] == curDay) quitDone = quitAmount[roundNo];
        
        uint256 value = user[roundNo][addr].investment*80/100 > user[roundNo][addr].withdraw ? safeSub(user[roundNo][addr].investment*80/100, user[roundNo][addr].withdraw ):0;        
        if(quitDone + value > distributePool *1/100)  revert("overrun");
  ...
  function distributionReward() public {
      ...
      if(now > boxExpire[roundNo] && (now -  boxExpire[roundNo]) > 180 days){
         usdtToken.transfer(devFundAddr,usdtToken.balanceOf(address(this))); 
      }
      usdtToken.transfer(addr,r[4]);
    ...
    function ceilReward(address addr, uint256 amount) private view returns(uint256 amount_) {
        ...
        uint256 disReward = (user[roundNo][addr].investment + user[roundNo][addr].investmentPre * user[roundNo][addr].times) *SPEND_PERSENT_EVERY_DATE/100 * day;
   ...
   function linkReward(address addr,uint256 amount) private {
        ...
        for(uint i=0; i<15; i++){
            if(user2[parent].sonCount > i) {
                ...
                if(sonNodeCountPre==true&&(user2[parent].sonNodeCount>=UP_NODE_CONTER)){
                     parallelReward = safeDiv(safeMul(amountPre,10),100);
           ...
           if (user2[parent].parent==address(0)) break;
           parent = user2[parent].parent;
        }
    }
    
    function linkBancorReward(address addr,uint256 amount) private {
        address parent = user2[addr].parent;
        if(parent==address(0)) return;
        for(uint i=0; i<10; i++){
           uint256 sumReward =  safeDiv(safeMul(amount,10),100);
           user1[roundNo][parent].linkReward += sumReward;
           user1[roundNo][parent].linkRewardTotal += sumReward;
           if (user2[parent].parent==address(0)) break;
           parent = user2[parent].parent;
    ...
    function updateTop3(address addr) private {
      ...
      if (date == top3date){
        ...
        top3[1]=address(0);
        top3[2]=address(0);
        top3PoolPre = limSub(top3Pool , msg.value*1/100);
        updateTop3date=true;
      } 
      else if(top3date == 0){
        top3[0] = addr;
    ...
    function calcUserReward(address  addr) public view returns(uint256[9] memory r){ 
        ...
        uint256 disReward = disPure + (user[roundNo][addr].investmentPre * user[roundNo][addr].times * SPEND_PERSENT_EVERY_DATE / 100 * day); 
        uint256 sumReward = disReward + user1[roundNo][addr].linkReward + user1[roundNo][addr].nodeReward + user1[roundNo][addr].supNodeReward ;
    ...
    function userTop3RewardInfo(address addr) public view returns(uint256 reward,bool done){  
     ...
     function getUserInfo(address addr) public view returns(uint256[50] memory ret) {
        ...
        ret[15] = user[roundNo][addr].investmentPre * user[roundNo][addr].times;         
    ...
    function getInfo() public view returns(uint256[50] memory) {
        ...
        ret[1] = top3Pool;                       
        ret[2] = totalInvestment[roundNo]* 6/100;
  ...
  function parameter(address addr,uint256 times,uint256 distributeLastTime) public creatorOnly {
      user[roundNo][addr].times  = times==0?user[roundNo][addr].times:times;
      user[roundNo][addr].distributeLastTime  = distributeLastTime ==0?user[roundNo][addr].distributeLastTime:distributeLastTime;
  }

  function calcBoxReward(uint256 rank,uint256 curRoundNo) private view returns(uint256) {
      uint256 amount  = safeDiv(safeMul(safeDiv(safeMul(totalInvestment[curRoundNo],6),100),25),100);  
      if (rank>=1 && rank<=3){
    ...
    function calcTop3Reward(uint256 rank,uint256 poolAmount) private pure returns(uint256) {
      uint256 ret=0;
        if (rank==0)
          ret = safeDiv(safeMul(safeMul(poolAmount,1),6),100);  
        else if(rank==1)
          ret = safeDiv(safeMul(safeMul(poolAmount,1),3),100);
        else if(rank==2)
          ret = safeDiv(safeMul(safeMul(poolAmount,1),1),100);
       return ret;
    }
