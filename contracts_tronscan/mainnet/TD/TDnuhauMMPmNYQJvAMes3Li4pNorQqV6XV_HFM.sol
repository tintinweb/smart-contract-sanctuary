...
contract ERC20 is Context, TRC20 {
    ...
    // Only 250k of tokens for the liquidity, promotions and collection of the foundation days.
    
    uint256 private _totalSupply = 250000 * (10 ** 8);
...
contract GlobalsAndUtility is ERC20 {
    ...
    */
    ...
    /* Flush address */
    address payable internal constant FLUSH_ADDR = 0x5392a62e790e6c5ba50C48a15Bf9831238Dd8eDf;
    uint8 internal LAST_FLUSHED_DAY = 1;

    /* ERC20 constants */
    string public constant name = "HASH FENIX MINING";
    string public constant symbol = "HFM";
    uint8 public constant decimals = 8;

    /* Suns per Satoshi = 10,000 * 1e8 / 1e8 = 1e4 */
    uint256 private constant SUNS_PER_DIV = 10 ** uint256(decimals); // 1e8

    /* Time of contract launch 8 FEB 2021 UTC (2021-02-08T00:00:00Z) */
    uint256 internal constant LAUNCH_TIME = 1612742400;

    /* Start of claim phase */
    uint256 internal constant PRE_CLAIM_DAYS = 1;
    /* reduce amount of tokens to 250000 */
    uint256 internal constant CLAIM_STARTING_AMOUNT = 250000 * (10 ** 8);
    ...
    uint256 internal constant MAX_STAKE_DAYS = 90; // 3 months
        ...
        external
    {
        ...
        require(g._currentDay > CLAIM_PHASE_START_DAY, "HFM: Too early");

        if (beforeDay != 0) {
            require(beforeDay <= g._currentDay, "HFM: beforeDay cannot be in the future");
    ...
    {
        require(beginDay < endDay && endDay <= globals.dailyDataCount, "HFM: range invalid");
        ...
        view
    {
        ...
        require(stakeIdParam == stRef.stakeId, "HFM: stakeIdParam not in stake");
...
contract StakeableToken is GlobalsAndUtility {
        ...
        external
    {
        ...
        require(newStakedDays >= MIN_STAKE_DAYS, "HFM: newStakedDays lower than minimum");
        ...
        external
    {
        ...
        require(stakeLists[stakerAddr].length != 0, "HFM: Empty stake list");
        require(stakeIndex < stakeLists[stakerAddr].length, "HFM: stakeIndex invalid");
        ...
        require(g._currentDay >= st._lockedDay + st._stakedDays, "HFM: Stake not fully served");

        /* Stake must still be locked */
        require(st._unlockedDay == 0, "HFM: Stake already unlocked");
        ...
        external
    {
        ...
        /* require() is more informative than the default assert() */
        require(stakeListRef.length != 0, "HFM: Empty stake list");
        require(stakeIndex < stakeListRef.length, "HFM: stakeIndex invalid");
        ...
        internal
    {
        ...
        require(newStakedDays <= MAX_STAKE_DAYS, "HFM: newStakedDays higher than maximum");
        ...
        require(newStakeShares != 0, "HFM: newStakedSuns must be at least minimum shareRate");
        ...
        returns (uint256 bonusSuns)
    {
        /*
            ...
            Bonus percentage scaled 0% to 10% for the first 700k HFM of stake.
...
contract TransformableToken is StakeableToken {
    ...
    function xfLobbyEnter(address referrerAddr)
        ...
        payable
    {
	require(_currentDay() > 0, "HFM: Auction has not begun yet");

        uint256 enterDay = _currentDay();

        uint256 rawAmount = msg.value;
        require(rawAmount != 0, "HFM: Amount required");
    ...
    function xfLobbyExit(uint256 enterDay, uint256 count)
        external
    {
        require(enterDay < _currentDay(), "HFM: Round is not complete");
        ...
        if (count != 0) {
            require(count <= qRef.tailIndex - headIndex, "HFM: count invalid");
            endIndex = headIndex + count;
        } else {
            endIndex = qRef.tailIndex;
            require(headIndex < endIndex, "HFM: count invalid");
        }
    ...
    /**
     * @dev PUBLIC FACING: Release 10% liquidity (JustSwap) share from daily dividends
     */
    function xfFlush()
        external
    {
        ...
        require(address(this).balance != 0, "HFM: No value");

        require(LAST_FLUSHED_DAY < _currentDay(), "HFM: Invalid day");

        _dailyDataUpdateAuto(g);

        FLUSH_ADDR.transfer((dailyData[LAST_FLUSHED_DAY].dayDividends * 10) / 100);
        ...
        returns (uint256 rawAmount, address referrerAddr)
    {
        ...
        require(entry.rawAmount != 0, "HFM: Param invalid");
    ...
    {
        if (enterDay > 0 && enterDay <= 60) {                                     
            waasLobby = CLAIM_STARTING_AMOUNT - ( (enterDay - 1) * 2500 * ( 10 ** 8) );
        } else {
...
contract HFM is TransformableToken {
    constructor()
        public
    {
