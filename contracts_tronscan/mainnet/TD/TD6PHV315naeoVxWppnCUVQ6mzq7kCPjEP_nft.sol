...
contract nft is Ownable {
    ...
    function setOutAmountDay(uint256 _outAmountDay) public returns (bool) {
        outAmountDay = _outAmountDay;
        return true;
    }
    
    function setNftToken(IERC20 _nftToken) public returns (bool) {
        nftToken = _nftToken;
        return true;
    }
    
    function setNftcToken(IERC20 _nftcToken) public returns (bool) {
        nftcToken = _nftcToken;
        return true;
    }
    
    function setLPToken(IERC20 _lpToken) public returns (bool) {
        lpToken = _lpToken;
        return true;
    }
    
    function setDestroyAddress(address _destroyAddress) public returns (bool) {
        destroyAddress = _destroyAddress;
        return true;
    }
    
    function setUsdtSwapContract(IJustswapExchange _usdtSwapContract) public returns (bool) {
        usdtSwapContract = _usdtSwapContract;
        return true;
    }
    
    function setNftcSwapContract(IJustswapExchange _nftcSwapContract) public returns (bool) {
     ...
     * 获取算力值、销毁数量
     **/
    function getUserInfo() public view returns (uint256 _calculatedForceValue,uint256 _lpAmount,uint256 _nftDestroyAmount, uint256 _nftcDestroyAmount,uint256 _nftcAmount,uint256 _rewardDebt,uint256 _rewardRemain,uint256 _lpRewardRemain) {
