...
contract FASTTRON {
    ...
    constructor() public {
        ...
        tarifs.push(Tarif(30, 450, 1e7)); // 15
        tarifs.push(Tarif(15, 360, 5e8)); // 24
        tarifs.push(Tarif(10, 300, 1e9)); // 30
        tarifs.push(Tarif(7, 280, 25e8)); // 40
        tarifs.push(Tarif(5, 250, 5e9)); // 50
    ...
    function _refPayout(address _addr, uint256 _amount) private {
        ...
        for(uint8 i = 0; i < ref_bonuses.length; i++) {
            ...
            if(now < launch_date) {
if(i==0){bonus = _amount * (ref_bonuses[i] + 10) / 100;}
else{ bonus = _amount * (ref_bonuses[i] + 5) / 100;}
            }
...
up.transfer(bonus);
    ...
    function _setUpline(address _addr, address payable _upline, uint256 _amount) private {
    ...
    function deposit(uint8 _tarif, address payable _upline) external payable {
        ...
        require(player.deposits.length < 10, "Max 100 deposits per address");
    ...
    function userInfo(address _addr) view external returns(uint256 for_withdraw, uint256 total_invested, uint256 total_withdrawn, uint256 total_match_bonus, uint256[3] memory structure, uint[4][100] memory deposits) {
        ...
        return (
            payout + player.dividends + player.direct_bonus,
