...
contract vaultGame is  owned
{
    ...
    bool public oddEven;
    uint256 public totalVoucherBurntPrev;
    uint256 public toDividendPrev;
    mapping (address => uint256) public usersVaultBurnAmount;
    ...
    function pushMyBurn(address token, uint amountToBurn) internal returns(bool)
    {
        ...
        // calculating how much second need to increase with the given burn amount
    ...
    function distributeReward(address token) onlyOwner public returns(bool)
    {
        ...
        if (totalVoucherBurnt > 0)    
        {
            ...
            burnerAddress = owner;
            userTrxBalance[burnerAddress] = userTrxBalance[burnerAddress].add(toOwner);


            // for few days record so then claimMyPart can work
