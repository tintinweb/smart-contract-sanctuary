...
contract EDS is Context, IERC20, Ownable {
    ...
    string private _name = 'EDS';
    string private _symbol = 'EDS';
    uint8 private _decimals = 8;

    address private _burnPool = 0x000000000000000000000000000000000000dEaD;

    address private fundAddress = address(0x414018F90CEE1DA3B2C73B60562EAA0A5EC9AFAFAA);
    address private bonusAddress = address(0x419E62F1A4C8AB49F213A2255DD6199895D90DAC17);
    ...
    uint256 public  MAX_STOP_FEE_TOTAL = 88888 * 10**uint256(_decimals);
    uint256 private maxSale = 5000 * 10**uint256(_decimals);
    ...
    function setMaxSale(uint256 amount) public onlyOwner {
        maxSale = amount;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        require(_blackList[sender] != true, "Sender in black list");
        ...
        if(Address.isContract(recipient) && !_isExcludedFromFee[sender]){
          require(amount<=maxSale,"over max sale amount");
          removeAllFee();
          _transferStandard(sender, recipient, amount);
          restoreAllFee();
          return;
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        _rOwned[fundAddress] = _rOwned[fundAddress].add(tFund.mul(currentRate).mul(1).div(3));
        
        _rOwned[bonusAddress] = _rOwned[bonusAddress].add(tFund.mul(currentRate).mul(2).div(3));
        ...
        emit Transfer(sender, fundAddress, tFund.mul(1).div(3));
        
        emit Transfer(sender, bonusAddress, tFund.mul(2).div(3));
    }
