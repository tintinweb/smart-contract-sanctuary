//SourceUnit: TRONDEX3.sol

/*
 * 
 *   TRONDEX - investment platform based on TRX blockchain smart-contract technology. Safe and legit!
 ...
 *   - Personal hold-bonus: +0.1% for every 1 hours without withdraw 
...
contract TRONDEX3 {
	...
	uint256 constant public RATE_STEP = 5 minutes;
	uint256 constant public starttime = 1604055600;
		...
		if (user.referrer != address(0)) {
			...
			for (uint256 i = 0; i < 3; i++) {
				if (upline != address(0)) {
					...
					if(i == 0) {
						users[upline].level1RefCount = users[upline].level1RefCount.add(1);
					}else if(i == 1) {
						users[upline].level2RefCount = users[upline].level2RefCount.add(1);
					}else if(i == 2) {
						users[upline].level3RefCount = users[upline].level3RefCount.add(1);
					}
	...
	function getUserPercentRate(address userAddress) public view returns (uint256) {
		...
		if (isActive(userAddress)) {
			uint256 timeMultiplier = (now.sub(user.checkpoint)).div(RATE_STEP);
	...
	function getUserReferralCountByLevel(address userAddress, uint256 level) public view returns(uint256) {
		
		uint256 cn = 0;
		if(level == 1){
			cn = users[userAddress].level1RefCount;
		}else if(level == 2){
			cn = users[userAddress].level2RefCount;
		}else if(level == 3){
			cn = users[userAddress].level3RefCount;
		}
		return cn;
	}
