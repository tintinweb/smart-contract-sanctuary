...
contract UKing{
	...
	constructor(address payable adminWallet, address payable zoneWallet, address payable retailWallet, address USDTAddress, address UKCTokenAddress, uint256 startDate) public {
	...
	function invest(address referrer, uint256 deposit_amount, uint8 plan) public payable {
		...
		//deposit USDT to contract address
		usdt.transferFrom(msg.sender, address(this), deposit_amount);
		...
		usdt.transfer(zonePoolWallet, zonePoolAmount);

		if(referrer == retailReferWallet)
		{
			usdt.transfer(retailReferWallet, zonePoolAmount);
		}
	...
	function withdraw() public {
		...
		uint256 contractUKCBalance = ukc.balanceOf(address(this));
		if (contractUKCBalance < totalAirdrop) {
			totalAirdrop = contractUKCBalance;
		}
		user.totalAirdropWithdrawn = user.totalAirdropWithdrawn.add(totalAirdrop);

		user.checkpoint = block.timestamp;

		ukc.transfer(msg.sender, totalAirdrop);
	...
	function getZoneRewardPoolBalance() public view returns (uint256) {
		return usdt.balanceOf(zonePoolWallet);
	}
