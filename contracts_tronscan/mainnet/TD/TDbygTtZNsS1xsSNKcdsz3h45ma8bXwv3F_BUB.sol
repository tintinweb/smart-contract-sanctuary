...
contract ERC20 is Context, TRC20 {
    ...
    // allocating 50 million tokens for promotion, airdrop, liquidity and dev share
...
contract GlobalsAndUtility is ERC20 {
    ...
    */
    ...
    /* Flush address */
    address payable internal constant FLUSH_ADDR = 0x167d86A32E0829b9C7B03d44557CD43724bDCa3B;
    uint8 internal LAST_FLUSHED_DAY = 1;

    /* ERC20 constants */
    string public constant name = "Bubble";
    string public constant symbol = "BUB";
    uint8 public constant decimals = 8;

    /* Suns per Satoshi = 10,000 * 1e8 / 1e8 = 1e4 */
    uint256 private constant SUNS_PER_DIV = 10 ** uint256(decimals); // 1e8

    /* Time of contract launch (2020-31-10T00:00:00Z) */
    uint256 internal constant LAUNCH_TIME = 1607990400;
    ...
    uint256 internal constant MAX_STAKE_DAYS = 365; // One year
        ...
        external
    {
        ...
        require(g._currentDay > CLAIM_PHASE_START_DAY, "BUB: Too early");

        if (beforeDay != 0) {
            require(beforeDay <= g._currentDay, "BUB: beforeDay cannot be in the future");
    ...
    {
        require(beginDay < endDay && endDay <= globals.dailyDataCount, "BUB: range invalid");
        ...
        view
    {
        ...
        require(stakeIdParam == stRef.stakeId, "BUB: stakeIdParam not in stake");
...
contract StakeableToken is GlobalsAndUtility {
        ...
        external
    {
        ...
        require(newStakedDays >= MIN_STAKE_DAYS, "BUB: newStakedDays lower than minimum");
        ...
        external
    {
        ...
        require(stakeLists[stakerAddr].length != 0, "BUB: Empty stake list");
        require(stakeIndex < stakeLists[stakerAddr].length, "BUB: stakeIndex invalid");
        ...
        require(g._currentDay >= st._lockedDay + st._stakedDays, "BUB: Stake not fully served");

        /* Stake must still be locked */
        require(st._unlockedDay == 0, "BUB: Stake already unlocked");
        ...
        external
    {
        ...
        /* require() is more informative than the default assert() */
        require(stakeListRef.length != 0, "BUB: Empty stake list");
        require(stakeIndex < stakeListRef.length, "BUB: stakeIndex invalid");
        ...
        internal
    {
        ...
        require(newStakedDays <= MAX_STAKE_DAYS, "BUB: newStakedDays higher than maximum");
        ...
        require(newStakeShares != 0, "BUB: newStakedSuns must be at least minimum shareRate");
        ...
        returns (uint256 bonusSuns)
    {
        /*
            ...
            Bonus percentage scaled 0% to 10% for the first 7M BUB of stake.
...
contract TransformableToken is StakeableToken {
    ...
    function xfLobbyEnter(address referrerAddr)
        ...
        payable
    {
	require(_currentDay() > 0, "BUB: Auction has not begun yet");

        uint256 enterDay = _currentDay();

        uint256 rawAmount = msg.value;
        require(rawAmount != 0, "BUB: Amount required");
    ...
    function xfLobbyExit(uint256 enterDay, uint256 count)
        external
    {
        require(enterDay < _currentDay(), "BUB: Round is not complete");
        ...
        if (count != 0) {
            require(count <= qRef.tailIndex - headIndex, "BUB: count invalid");
            endIndex = headIndex + count;
        } else {
            endIndex = qRef.tailIndex;
            require(headIndex < endIndex, "BUB: count invalid");
        }
    ...
    /**
     * @dev PUBLIC FACING: Release 4% dev share from daily dividends
     */
    function xfFlush()
        external
    {
        ...
        require(address(this).balance != 0, "BUB: No value");

        require(LAST_FLUSHED_DAY < _currentDay(), "BUB: Invalid day");
     ...
     * @param enterDay Enter day
     * @param entryIndex Beginning of entry
        ...
        returns (uint256 rawAmount, address referrerAddr)
    {
        ...
        require(entry.rawAmount != 0, "BUB: Param invalid");
...
contract BUB is TransformableToken {
    constructor()
        public
    {
