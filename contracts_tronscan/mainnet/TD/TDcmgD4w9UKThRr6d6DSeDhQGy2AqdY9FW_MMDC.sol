//SourceUnit: MMDC source code.sol
...
contract MMDC is owned {
    ...
    string constant private _name = "MULTIMEDIA DIGITAL COIN";
    string constant private _symbol = "MMDC";
    uint256 constant private _decimals = 6;
    uint256 private _totalSupply;
    uint256 constant public maxSupply = 1000000000 * (10**_decimals);    //1 billion tokens
    uint constant private _preMine =  1000000000 * (10**_decimals);       // 1 billion token for deployer 
    ...
    constructor() public{
        //sending all premine tokens to Owner
        _balanceOf[owner] = _preMine;
        _deadline         = now + (1 days);
        _totalSupply = _preMine;
	...
	function waitingTimeToClaimTokens() public view returns(uint256){
		if(now > _deadline){
			return 0;
		}
		else{
    ...
    function claimTodayTokens() public onlyOwner returns(string memory){

        uint _dailyQuota =  1 * (10**_decimals);    
        require(now>=_deadline,"already active today Tokens Quota");
        require((_totalSupply + _dailyQuota) <= maxSupply, "Max supply is reached");
        _balanceOf[owner]+=_dailyQuota;
        _totalSupply +=_dailyQuota;
		_deadline = now + (1 days);
		emit Transfer(address(0), owner, _dailyQuota);
