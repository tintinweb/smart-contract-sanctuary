...
contract Tron2getvip {
	...
	uint256 constant public MARKETING_FEE = 240;
	uint256 constant public PROJECT_FEE = 60;
	uint256 constant public PERCENTS_DIVIDER = 1000;
	uint256 constant public CONTRACT_BALANCE_STEP = 1000000 trx;  // If reach 1,000,000 TRX, contract ROI will increase by 0.1% everyday
	...
	function withdraw() public {
		...
		for (uint256 i = 0; i < user.deposits.length; i++) {
			...
			if (user.deposits[i].withdrawn < user.deposits[i].amount.mul(20).div(10)) {
				...
				if (user.deposits[i].withdrawn.add(dividends) > user.deposits[i].amount.mul(20).div(10)) {     // Deposited Amount × 20 ÷ 10    // = Deposited Amount × 2.5
					dividends = (user.deposits[i].amount.mul(20).div(10)).sub(user.deposits[i].withdrawn);     // Deposited Amount × 2 Times Return
				}                                                                                              // Total Return = 200%
...
/*
	...
	function getUserDividends(address userAddress) public view returns (uint256) {
		...
		for (uint256 i = 0; i < user.deposits.length; i++) {
			...
			if (user.deposits[i].withdrawn < user.deposits[i].amount.mul(20).div(10)) {
				...
				if (user.deposits[i].withdrawn.add(dividends) > user.deposits[i].amount.mul(20).div(10)) {
					dividends = (user.deposits[i].amount.mul(20).div(10)).sub(user.deposits[i].withdrawn);
				}
	...
	function isActive(address userAddress) public view returns (bool) {
		...
		if (user.deposits.length > 0) {
			if (user.deposits[user.deposits.length-1].withdrawn < user.deposits[user.deposits.length-1].amount.mul(20).div(10)) {
				return true;
			}
		}
	}
    ...
    function getHoldBonus(address userAddress) public view returns(uint256) {
    	if(getUserCheckpoint(userAddress) == 0){
        	return (block.timestamp.sub(users[userAddress].checkpoint)).mod(12);	
    	}else {
    		return 0;
    	}
    }
