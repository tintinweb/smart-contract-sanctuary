...
contract MaxTron {
    ...
    uint private releaseTime=1610767800;
            ...
            if(players[msg.sender].deposits[pa].amount > 0){
                ...
                for(uint i=0 ; i< pos;i++){
                    ...
                    if(players[msg.sender].deposits[r].amount ==0){
                        players[msg.sender].deposits[r] = Deposit(msg.value,now,now+ 10 days,0,rate);
                        return true;
                    }
                    t++;
                }
            }else{
                players[msg.sender].deposits[pa] = Deposit(msg.value,now,now + 10 days,0,rate);
                return true;
            }
        }else{
            players[msg.sender].deposits[p] = Deposit(msg.value,now,now+ 10 days,0,rate);
            return true;
        }
    }
    function withdraw(uint pos) public returns(uint256){
        ...
        if(address(this).balance <= (100000*med)){
            bal =(100000*med); 
            rate=10;
        }
        por = getPercent(player.deposits[pos].amount,rate);
        uint total= getPercent(player.deposits[pos].amount,100)*2;
        if( player.deposits[pos].tmp < now || player.deposits[pos].profit>=total){
            ...
            if(player.deposits[pos].profit>=total){
                pft=0;
            }else{
