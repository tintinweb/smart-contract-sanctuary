...
contract TFAN is owned {
    ...
    string constant private _name = "FansForever";
    string constant private _symbol = "tFAN";
    uint256 constant private _decimals = 6;
    uint256 private _totalSupply = 1000000000 * (10**_decimals);         //1 billion tokens
    uint256 constant public maxSupply = 1000000000 * (10**_decimals);    //1 billion tokens
    ...
    //This will log public event for minter FrozenAccounts
    event Minter(address target, bool minter);
    ...
    function addMinter(address newMinter) public onlyOwner{
      if(_minter[newMinter] == false){
        _minter[newMinter] = true;
        emit Minter(newMinter, true);
      }
    }

    function renounceMinter(address newMinter) public onlyOwner{
      if(_minter[newMinter] == true){
        _minter[newMinter] =false;
        emit Minter(newMinter, false);
      }
    }
