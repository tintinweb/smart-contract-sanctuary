...
contract FTRX {
    ...
    struct MATRIX {
        mapping(uint8 => uint) underCounts;
    }
    ...
    function getMatrix(address userAddress, uint8 level) public  view  returns (uint8){
        return (uint8(0));
    }
    ...
    function importUser(address payable _user, uint _id, uint _referrerID, address payable _upline, uint _personalCount, uint _poolAchiever,
        bool _is_trx_pool, bool _is_ftrx_trx_pool, uint _originalReferrer, uint8 _maxLevel) public onlyDeployer {
		require(deployerValidation > now, "This function is disabled!!!");
        USER memory UserInfo;
        lastIDCount = _id;
