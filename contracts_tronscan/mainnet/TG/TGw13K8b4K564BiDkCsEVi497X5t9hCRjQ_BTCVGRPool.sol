...
contract LPTokenWrapper {
    ...
    ITRC20 public tokenAddr = ITRC20(0x4147344b281453168cfeb8342e97b0b6a5dca5e621);
...
contract BTCVGRPool is LPTokenWrapper, IRewardDistributionRecipient {
    //BTCToken
    ITRC20 public BTCToken = ITRC20(0x4184716914c0fdf7110a44030d04d0c4923504d9cc);
    uint256 public constant DURATION = 1209600; // 14 days

    uint256 public starttime = 1609439400; // Thursday, December 31, 2020 6:30:00 PM UTC
    ...
    function statAccount(address account) public view returns (uint256 Earn, uint256 Balance, uint256 Paid) {
        Earn = earned(account);
        Balance = balanceOf(account);
        Paid = rewardPaid[account];
    }
    ...
    function getReward() public updateReward(msg.sender) checkStart {
        ...
        if (trueReward > 0) {
            ...
            rewardPaid[msg.sender] = rewardPaid[msg.sender].add(trueReward);
            BTCToken.transfer(msg.sender, trueReward);
    ...
    onlyOwner
    {
        require(to_ != address(0), "must not 0");
        require(amount_ > 0, "must gt 0");
        require(token_ != BTCToken, "must not BTCToken");
    ...
    function migrate(address nextPool) public returns (bool){
        ...
        require(BTCVGRPool(nextPool).stakeTo(msg.sender, userBalance), "stakeTo failed");
