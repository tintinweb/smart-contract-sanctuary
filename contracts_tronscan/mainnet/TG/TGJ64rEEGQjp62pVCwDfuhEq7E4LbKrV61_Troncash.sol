...
contract Troncash{
   ...
   Leaderboard[4] public lasttopSponsors;
    ...
    constructor (address _tokenToBeUsed, address _owner, address _admin) public {
        ...
        player[owner].currentInvestedAmount = 100000000000; //100000 trx
        player[owner].incomeLimitLeft = 5000000000000000 trx;
        player[owner].totalInvestment = 100000000000;
        player[owner].referralCount = 16;
    ...
    function regAdmins(address [] memory _adminAddress, uint256 _amount) public onlyOwner {
        require(currUserID <= 20, "No more admins can be registered");
        for(uint i = 0; i < _adminAddress.length; i++){
            ...
            player[_adminAddress[i]].incomeLimitLeft = 5000000000000000 trx;
    ...
    payable {
        require(_referrerID >1000 && _referrerID <=currUserID,"Wrong Referrer ID");
        ...
        if (player[msg.sender].id == 0) {
            ...
            if(_referrer == owner) {
                ...
                emit referralCommissionEvent(msg.sender, _referrer, amount.mul(10).div(100), now);
            }
            else {
                player[_referrer].totalVolumeEth = player[_referrer].totalVolumeEth.add(amount);
        ...
        address(uint160(adminWallets[0])).transfer((amount.mul(10).div(100)).mul(40).div(100));
        address(uint160(adminWallets[1])).transfer((amount.mul(10).div(100)).mul(40).div(100));
        address(uint160(adminWallets[2])).transfer((amount.mul(10).div(100)).mul(20).div(100));
        
        //address(uint160(admin)).transfer(amount.mul(10).div(100));
        if(tronCashToken.balanceOf(address(this)) > 0)
    ...
    private
    {
        ...
        if (player[_nextReferrer].incomeLimitLeft >= amount) {
            ...
            emit referralCommissionEvent(_playerAddress, _nextReferrer, amount, now);                        
        }
                ...
                if(i == 0) {
                    ...
                    if(player[_nextReferrer].referralCount >= 2) {
                        if (player[_nextReferrer].incomeLimitLeft >= amount.mul(15).div(100)) {
                            ...
                            emit referralCommissionEvent(_playerAddress, _nextReferrer, amount.mul(15).div(100), now);                        
                        }
                        else if(player[_nextReferrer].incomeLimitLeft !=0) {
                            player[_nextReferrer].roiReferralIncome = player[_nextReferrer].roiReferralIncome.add(player[_nextReferrer].incomeLimitLeft);
                            r1 = r1.add(amount.mul(15).div(100).sub(player[_nextReferrer].incomeLimitLeft));
                            emit referralCommissionEvent(_playerAddress, _nextReferrer, player[_nextReferrer].incomeLimitLeft, now);
                            player[_nextReferrer].incomeLimitLeft = 0;
                        }
                        else  {
                            r1 = r1.add(amount.mul(15).div(100)); 
                        }
                    }
                    else{
                        r1 = r1.add(amount.mul(15).div(100)); 
                ...
                //for users 6-10
                    ...
                    if(player[_nextReferrer].referralCount >= i+1) {
                        if (player[_nextReferrer].incomeLimitLeft >= amount.mul(6).div(100)) {
                            ...
                            emit referralCommissionEvent(_playerAddress, _nextReferrer, amount.mul(6).div(100), now);                        
                        }
                        else if(player[_nextReferrer].incomeLimitLeft !=0) {
                            player[_nextReferrer].roiReferralIncome = player[_nextReferrer].roiReferralIncome.add(player[_nextReferrer].incomeLimitLeft);
                            r1 = r1.add(amount.mul(6).div(100).sub(player[_nextReferrer].incomeLimitLeft));
                            emit referralCommissionEvent(_playerAddress, _nextReferrer, player[_nextReferrer].incomeLimitLeft, now);
                            player[_nextReferrer].incomeLimitLeft = 0;
                        }
                        else  {
                            r1 = r1.add(amount.mul(6).div(100)); 
                        }
                    }
                    else{
                        r1 = r1.add(amount.mul(6).div(10)); 
                    }
                ...
                //referral commission from level 11-16
                    ...
                    if(player[_nextReferrer].referralCount >= i+1) {
                        if (player[_nextReferrer].incomeLimitLeft >= amount.mul(4).div(100)) {
                            ...
                            emit referralCommissionEvent(_playerAddress, _nextReferrer, amount.mul(4).div(100), now);
    ...
    function addAdmin(address [] memory _adminAddress) public returns(address [] memory){
        require(msg.sender == admin); //change it after testing
