...
pragma solidity ^0.4.18;
 ...
 contract CanReclaimToken is Ownable {
   using TFX20 for TFX20B;

   function reclaimToken(TFX20B token) external onlyOwner {
 ...
 contract TFC is Claimable {
     string public name;
     mapping (address => bool) public onList;

     function TFC(string _name, bool nullValue) public {
         name = _name;
 ...
 contract NamableTFC is TFC {
     function NamableTFC(string _name, bool nullValue)
         TFC(_name, nullValue) public {}
 ...
 contract TimeLockedController is HasNoEther, HasNoTokens, Claimable {
     ...
     uint public constant blocksDelay = 24*60*6015;
     ...
     event ChangeTFXGEvent(TFXG newContract);
     ...
     function changeStakingFees(uint80 _transferFeeNumerator,
         ...
         ChangeStakingFeesEvent(_transferFeeNumerator,
                                           ...
                                           _burnFeeDenominator,
                                           _burnFeeFlat);
         TFXC.changeStakingFees(_transferFeeNumerator,
                                   _transferFeeDenominator,
                                   _mintFeeNumerator,
                                   _mintFeeDenominator,
                                   _mintFeeFlat,
     ...
     function setLists(TFC _canReceiveMintWhiteList, TFC _canBurnWhiteList, TFC _blackList, TFC _noFeesList) onlyOwner public {
         TFXC.setLists(_canReceiveMintWhiteList, _canBurnWhiteList, _blackList, _noFeesList);
     }

     function updateList(address list, address entry, bool flag) public onlyAdminOrOwner {
         TFC(list).changeList(entry, flag);
     }

     function renameList(address list, string name) public onlyAdminOrOwner {
         NamableTFC(list).changeName(name);
     }
 ...
 contract BasicToken is TFX20B, Claimable {
   using SafeMath for uint256;
 ...
 library TFX20 {
   function safeTransfer(TFX20B token, address to, uint256 value) internal {
 ...
 contract TFXG is StandardDelegate, PausableToken, BurnableToken, NoOwner, CanDelegate {
     string public name = "TFXG";
     string public symbol = "TFX";
     uint8 public constant decimals = 6;

     TFC public canReceiveMintWhiteList;
     TFC public canBurnWhiteList;
     TFC public blackList;
     TFC public noFeesList;
     uint256 public burnMin = 1 * 10**uint256(decimals);
     uint256 public burnMax = 1000 * 10**uint256(decimals);
     ...
     function TFXG() public {
         totalSupply_ = 3333000;
         staker = msg.sender;
     }
