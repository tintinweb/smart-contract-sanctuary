...
contract TRC20 is ITRC20, Common {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if (whiteAddressList[sender]) {
			...
			if (fund_income_ratio >= _tranfer_fee_ratio){
				feeAmount = fundIncome;
			}
			if (feeAmount != 0 && _tranfer_fee_ratio > fund_income_ratio){
				_balances[_fee_address] = _balances[_fee_address].add(feeAmount.sub(fundIncome));
...
contract Token is TRC20 {
     ...
     */
    constructor () public TRC20Detailed("BTxq", "BTxq", 18) {
