//SourceUnit: GEMTRX_Investment01_06.sol
...
contract GEMTRX is Ownable{
   ...
   // address payable public etherchain_fund;
    ...
    uint256 public constant DEVELOPER_RATE = 40;            // 4% Community Backup Fund
    uint256 public constant MARKETING_RATE = 50;            // 5% Community Backup Fund
    ...
    event pendingWithdraw(address indexed addr, uint256 amount);
    ...
    function _deposit(address _addr, uint256 _amount) private {
        ...
        //pending withdrwal amount transfer
        if(users[_addr].total_pendingpayouts != 0)
        {
            msg.sender.transfer(users[_addr].total_pendingpayouts);
            users[_addr].total_pendingpayouts = 0;
            emit pendingWithdraw(msg.sender, users[_addr].total_pendingpayouts);
        }
      ...
      //  _pollDeposits(_addr, _amount);
       ...
       // }
        ...
        uint256 marketingPercentage = (_amount.mul(50)).div(1000); // 5% Admin 
    ...
    function withdraw() external {
        ...
        users[msg.sender].total_pendingpayouts += to_payout.mul(36).div(100);

        msg.sender.transfer(to_payout.mul(54).div(100));

       // uint256 marketingPercentage = (to_payout.mul(MARKETING_RATE)).div(1000);
      //  marketingAccount_.transfer(marketingPercentage);
    ...
    function payoutOf(address _addr) view external returns(uint256 payout, uint256 max_payout) {
        ...
        if(users[_addr].deposit_payouts < max_payout) {
           ...
           payout = (users[_addr].deposit_amount * ((block.timestamp - users[_addr].deposit_time) / 1 days) * 2 / 100) - users[_addr].deposit_payouts;
           // payout = ((users[_addr].deposit_amount * 20 / 1000 * (block.timestamp - users[_addr].deposit_time)) / (60*60*24)) - users[_addr].deposit_payouts; //2% daily
    ...
    */
    ...
    function userInfoTotals(address _addr) view external returns(uint256 referrals, uint256 total_deposits, uint256 total_payouts, uint256 total_structure , uint256 total_pendingpayouts) {
        return (users[_addr].referrals, users[_addr].total_deposits, users[_addr].total_payouts, users[_addr].total_structure, users[_addr].total_pendingpayouts);
    }
