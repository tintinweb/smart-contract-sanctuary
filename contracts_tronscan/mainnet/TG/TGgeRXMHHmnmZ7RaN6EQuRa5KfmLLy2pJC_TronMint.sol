...
contract TronMint {
    ...
    struct Player {
        ...
        mapping(uint8 => uint256) downline;
    }
    ...
    function deposit(address _referral, uint8 _is_assured) external payable {
        ...
        if(total_income==0 || total_income>=player.total_invested*investment_max/100){
            ...
            require(depositAmt >= 500000000, "Invalid Amount");
            player.is_assured_withdrawl=false;
    ...
    function availamount(address ref,uint256 chkamount) view private returns(uint256) {
        uint256 total_income = players[ref].total_withdrawn + players[ref].referral_bonus + players[ref].cto_income + players[ref].level_roi;
    ...
    function _referralPayout(address _addr, uint256 _amount) private {
        ...
        if((players[ref].is_assured_withdrawl == false) || ref == admin){
           uint256 bonus = _amount/20;
            ...
            if(players[ref].referral_business>= 7500 && players[ref].is_cto==false || ref == admin){
                cto_users.push(ref);
    ...
    function roi_to_levels(address _addr, uint256 _amount) private {
        ...
        for(uint8 i = 0; i < level_roi_bonus.length; i++) {
            if(ref == address(0)) break;
            if(players[ref].referrals_per_level[0] >= (i+1)  && players[ref].total_invested*investment_max/100>total_income && players[ref].is_assured_withdrawl==false){
    ...
    function payoutOf(address _addr) view external returns(uint256 value) {
        ...
        uint256 total_income = players[_addr].total_withdrawn + players[_addr].referral_bonus + players[_addr].cto_income + players[_addr].level_roi;
       
        for(uint256 i = 0; i < player.deposits.length; i++) {
            PlayerDeposit storage dep = player.deposits[i];
            if(player.is_assured_withdrawl==false && players[_addr].total_invested*investment_max/100>total_income){   
                uint256 time_end = dep.time + 8640000;
    ...
    function userInfo(address _addr) view external returns(uint256 for_withdraw, uint256[1] memory referrals, uint256[1] memory refperlevel, uint256[15] memory my_roi_income, uint256[15] memory my_downline) {
        Player storage player = players[_addr];
        uint256 payout = this.payoutOf(_addr);
        for(uint8 i = 0; i < level_roi_bonus.length; i++) {
            my_roi_income[i] = player.total_level_roi[i];
            my_downline[i] = player.downline[i];
        }
    ...
    function isUserInvestable(address _addr) view external returns(bool) {
        Player storage player = players[_addr];
        uint256 total_income = player.total_withdrawn + player.referral_bonus + player.cto_income + player.level_roi;
        bool is_reinvestable = (total_income>=player.total_invested*investment_max/100)?true:false;
