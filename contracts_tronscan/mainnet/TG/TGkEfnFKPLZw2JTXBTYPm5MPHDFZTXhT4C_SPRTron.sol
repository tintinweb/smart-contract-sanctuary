...
contract SPRTron {
    ...
    function buyNewLevels(address userAddress, uint8 matrix, uint8 level) external onlyOwner {
        ...
        if (matrix == 1) {
            require(!users[userAddress].activeO3Levels[level], "level already activated");
            require(users[userAddress].activeO3Levels[level - 1], "previous level should be activated");

            if (users[userAddress].O3Matrix[level-1].blocked) {
                users[userAddress].O3Matrix[level-1].blocked = false;
            }
    
            address freeO3Referrer = findFreeO3Referrer(userAddress, level);
            users[userAddress].O3MaxLevel = level;
            users[userAddress].O3Matrix[level].currentReferrer = freeO3Referrer;
            users[userAddress].activeO3Levels[level] = true;
            updateO3Referrer(userAddress, freeO3Referrer, level);
             totalearnedtrx = totalearnedtrx+levelPrice[level];
            emit Upgrade(userAddress, freeO3Referrer, 1, level);

        } else {
            require(!users[userAddress].activeO6Levels[level], "level already activated"); 
            require(users[userAddress].activeO6Levels[level - 1], "previous level should be activated"); 

            if (users[userAddress].O6Matrix[level-1].blocked) {
                users[userAddress].O6Matrix[level-1].blocked = false;
            }

            address freeO6Referrer = findFreeO6Referrer(userAddress, level);
            users[userAddress].O6MaxLevel = level;
            users[userAddress].activeO6Levels[level] = true;
            updateO6Referrer(userAddress, freeO6Referrer, level);
            
        
          totalearnedtrx = totalearnedtrx+levelPrice[level];
            emit Upgrade(userAddress, freeO6Referrer, 2, level);
        }
