...
contract HashPool {
    ...
    uint256[5] internal VIP_SHARE = [0,20,30,45,55];
    ...
    function stake(address uaddress,uint256 amount) external onlyStakePool checkRegister(uaddress) returns (bool) {
        ...
        updateRateGlobals();
        users[uaddress].deposited+=amount;
    ...
    function reap(uint256 amount,address uaddress) public onlyStakePool returns(uint256){
        ...
        if(f_!=address(0)){
            earns[f_][0]+=actualGive(amount*F_SHARE/100,0);
            address s_ = users[f_].referrer;
            uint256 sc_ = users[f_].refsCount;
            if(s_!=address(0)&&sc_>2){
                earns[s_][0]+=actualGive(amount*S_SHARE/100,0);
            }
        }
        ...
        while(true){
            ...
            earns[lup][1]+=actualGive(amount*VIP_SHARE[uvip]/100,1);
            up = lup;
        }
    ...
    function actualGive(uint256 amount,uint256 kind) internal returns(uint256 actualOut){
        ...
        uint256 supply = _output*ABS_RATE[kind]/100;
        if(gaven[kind]+amount>supply){
            actualOut = gaven[kind]>supply?0:supply-gaven[kind];
        }else{
            actualOut = amount;
        }
        gaven[kind]+=actualOut;
    }
    ...
    function withdraw() public returns(uint256 uamount){
        ...
        uamount+=rateGlobals[users[msg.sender].vip].sub(users[msg.sender].rateUsed);
        require(uamount>0,"No income");
        manager.withdraw(uamount);
        delete earns[msg.sender];
        users[msg.sender].withdrew+=uamount;
            ...
            if(users[uaddress].deposited>=VIP_SILL[vip]){
                if(vip==1){
                    if(users[uaddress].teamMax>=VIP1_MASS){
                        if(users[uaddress].teamMass.sub(users[uaddress].teamMax)>=VIP1_MASS){
                            return true;
                        }
                    }
                }else{
                    if(users[uaddress].vipCounts[vip-1]>=2){
                        return true;
                    }
                }
            }
    ...
    function updateTeamMass(address uaddress,uint256 uamount) internal {
        ...
        users[uaddress].teamMass += uamount;
        uint256 down = users[uaddress].teamMass;
        address up = users[uaddress].referrer;
        bool[5] memory vipUp;
        ...
        while(true){
            ...
            for(uint8 i=1;i<5;i++){
                if(users[up].blocked[i]) vipUp[i] = false;
    ...
    function upgrade(address uaddress,uint256 vip) internal returns(uint256 reward) {
        ...
        earns[uaddress][2]+=actualGive(reward,2);
    ...
    function getStats(address uaddress) public view returns(uint256[8] memory stats){
        ...
        stats[4] = earns[uaddress][2]+rateGlobals[users[uaddress].vip].sub(users[uaddress].rateUsed);
...
interface IPoolManager {
    ...
    function refundFrom(address owner,uint256 uamount,uint256 tamount) external returns(bool);
