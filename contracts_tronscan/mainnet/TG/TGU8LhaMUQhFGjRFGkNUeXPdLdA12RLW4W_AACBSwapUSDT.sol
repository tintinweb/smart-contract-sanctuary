//SourceUnit: AACBTokenSwapUSDT.sol
...
interface ICoinBossReward{
    function AACBRate() external view returns (uint); 
    function usdtRate() external view returns (uint);
}

//USDT => AACB
contract AACBSwapUSDT is Ownable{
    ...
    constructor(address AACB_token_address, address usdt_token_address, address coin_boss_reward_address, address _fee) public{
        owner = msg.sender;
        AACBToken = AACB_token_address;
        ERC20ACInterface = IERC20(AACBToken);
    ...
    function SwapUSDTToAACB(
        ...
        //Calculation with ratio 100,000 ** 18 = 100,000 ** 6
        uint token_received_amount = GetSwapAACBAmount(tradeTokenAmount);
    ...
    function GetSwapUSDTAmount(uint AACBAmount)
        public
        view
        returns (uint)
    {
        //Calculation with ratio 100,000 ** 18 = 100,000 ** 6
        uint AACBToUsdt = ICoinBossReward(CoinBossRewardAddress).usdtRate();
        uint usdt_amount = AACBAmount * AACBToUsdt / 10 ** 6;
        return usdt_amount;
    }
    
    function GetSwapAACBAmount(uint USDTAmount)
        public
        view
        returns (uint)
    {
        //Calculation with ratio 100,000 ** 18 = 100,000 ** 6
        uint USDTToAACB = USDTAmount * 10 ** 7 / (ICoinBossReward(CoinBossRewardAddress).usdtRate()) ;
        USDTToAACB = USDTToAACB / 10;
        return USDTToAACB;
    }
