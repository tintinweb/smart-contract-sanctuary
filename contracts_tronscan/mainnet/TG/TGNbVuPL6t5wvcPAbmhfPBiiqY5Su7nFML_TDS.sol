...
contract TDS is Ownable {
    ...
    uint256 public totalCrazyFundReceived = 0;
    uint256 public totalCrazyFundCollected = 0;
    uint256 public totalMinedTewken = 0;

    uint256 private tokenSupply = 0;
    uint256 private profitPerShare = 0;

    address payable public crazy;
    EXCH stable;
    TOKEN trc20;

    constructor() public {
        stable = EXCH(address(0x41b5ab45b9cd03ae6d13693209388ae95e9017fcde));
        crazy = address(0x41a07018dd223c200f1b074af704211435132b5281);
    ...
    function payFund(bytes32 exchange) public {
        if (exchange == "stable") {
          uint256 _tronToPay = totalStableFundCollected.sub(totalStableFundReceived);
          require(_tronToPay > 0);
          totalStableFundReceived = totalStableFundReceived.add(_tronToPay);
          stable.distribute.value(_tronToPay)();
        } else if (exchange == "crazy") {
          uint256 _tronToPay = totalCrazyFundCollected.sub(totalCrazyFundReceived);
          require(_tronToPay > 0);
          totalCrazyFundReceived = totalCrazyFundReceived.add(_tronToPay);
          crazy.transfer(_tronToPay);
        }
    }
    ...
    function purchaseTokens(address _referredBy, address _customerAddress, uint256 _incomingTron) isActivated private returns (uint256) {
        ...
        totalStableFundCollected = totalStableFundCollected.add(_exchangeFee);
        totalCrazyFundCollected = totalCrazyFundCollected.add(_exchangeFee);

        if (tronToSendFund("stable") >= 1000e6) {
            payFund("stable");
        }

        if (tronToSendFund("crazy") >= 1000e6) {
            payFund("crazy");
        }
    ...
    function distributeTRC20(address _customerAddress, uint256 _tron) private {
        ...
        if (_scaledToken > 0 && trc20.isMinter(address(this)) && trc20.totalSupply().add(_scaledToken) <= trc20.cap()) {
            ...
            totalMinedTewken += _scaledToken;
            playerStats[_customerAddress].minedTewken += _scaledToken;
        }
    }
    ...
    function tronToSendFund(bytes32 exchange) public view returns(uint256) {
        if (exchange == "stable") {
          ...
          return totalCrazyFundCollected.sub(totalCrazyFundReceived);
        }
    }
}