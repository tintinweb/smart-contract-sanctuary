...
contract DigitalShareTrading is Context, IERC20, Ownable {
  ...
  constructor() public {
    _name = "Digital Share Trading";
    _symbol = "DST";
    _decimals = 18;
    _totalSupply = 2500000 * 10 ** 18;
    _balances[msg.sender] = _totalSupply;

    emit Transfer(address(0), msg.sender, _totalSupply);
  }
    
    uint256 public incremental = 5;
    uint256 public helpamount = 20;
    ...
    function getPrice(uint256 currentTime) internal {
        uint256 difference = currentTime - lastTime;
        uint256 dayss = 0;
        if(difference >= 86400 ){
            dayss = (difference / 86400) ; 
        }
        if(dayss >= 1){
            BuyPrice = BuyPrice + (dayss * (BuyPrice * incremental)/1000);
            lastTime = currentTime;
        }
    }
    
    function setBlocktimestamp(uint256 settime)  public onlyOwner returns(uint256){
        lastTime = settime;
        return lastTime;
    }
    ...
    function transferForBuy() public payable returns (bool) {
        ...
        uint256 netamount = value - ((value * helpamount)/1000) ; 
        getPrice(block.timestamp);
        _transfer(address(this), userAddress, (netamount*BuyTkn)/BuyPrice);
        return true;
    }
    ...
    function transferForSell(uint256 amount) public returns (bool) {
        ...
        _balances[address(this)] = _balances[address(this)].add(amount);
         uint256 value = (amount*BuyPrice)/BuyTkn;
        uint256 netamount = value - ((value * helpamount)/1000) ; 
        sender.transfer(netamount);
