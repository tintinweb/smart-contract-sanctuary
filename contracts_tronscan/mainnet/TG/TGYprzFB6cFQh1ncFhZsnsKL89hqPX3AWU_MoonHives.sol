...
contract Balances is Owned {
    ...
    uint256 public constant INITIAL_AUCTION_DEPOSIT = 100000; 
    ...
    uint256[5] public REF_PERCENTS = [10, 5, 3, 1, 1];
    ...
    function buyWax() public payable returns (bool success) {
        ...
        if (player.referer != address(0)) {
            ...
            for (uint8 i = 0; i < REF_PERCENTS.length; i++) {
              ...
              uint256 referrerReward = _wax.mul(REF_PERCENTS[i]).div(100);
...
contract Game is Balances {
    ...
    uint32[BEES_TYPES] public BEES_PROFITS_PERCENTS = [700, 710, 720, 730, 740, 750, 1000];

    
    uint32[BEES_LEVELS] public BEES_LEVELS_HONEY_PERCENTS = [80, 82, 84, 86, 88, 90];
