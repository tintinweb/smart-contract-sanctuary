...
*/
    ...
    contract Tronspeed{
        ...
        uint256 private incomeTimes = 20;
       ...
       //Round constants 
        uint256 constant private rndInc = 2 seconds;  // every tron purchased adds this much to the timer
        uint256 constant private rndMax =  6 hours;
       
        address public current_winner1;
        address public current_winner2;
        address public current_winner3;
        
        address public last_winner1;
        address public last_winner2;
        address public last_winner3;
        ...
        mapping (address => uint256) public withdrawnByUser;
        ...
        constructor (address _insuranceFunds) public {
            
            owner = msg.sender;
            insuranceFunds = _insuranceFunds;
            roundID = 1;
           ...
           ++currUserID;
            ...
            player[msg.sender].incomeLimitLeft = 20000000000000000 trx;
        ...
        modifier isWithinLimits(uint256 _trx) {
            require(_trx >= 500000000, "Minimum contribution amount is 500 TRX");
            _;
        }
    ...
    isallowedValue(msg.value)
    {
            ...
            if ((now > round[roundID].endTime && round[roundID].ended == false) && current_winner1 != address(0x0)) {
                startNewRound();
            }
            
            updateTimer(amount, roundID);
            
            if(current_winner1 != msg.sender){
              
                address tempPlayer1 = current_winner1;
                address tempPlayer2 = current_winner2;
                current_winner1 = msg.sender;
                current_winner2 = tempPlayer1;
                current_winner3 = tempPlayer2;
            }
    ...
    private 
    {
        ...
        if(now > player[_playerAddress].lastSettledTime + payoutPeriod) {
            ...
            _dailyIncome = currInvestedAmount.div(1000);
            uint256 currentExpirationCounter = playerInvestmentTracking[_playerAddress].expirationCounter;
            uint256 investmentCounter = playerInvestmentTracking[_playerAddress].investmentCounter;
            
            if(currentExpirationCounter < investmentCounter){
    ...
    returns (uint256)
    {
        ...
        if(now > player[_playerAddress].lastSettledTime + payoutPeriod) {
            ...
            if(currentExpirationCounter == playerInvestmentTracking[_playerAddress].investmentCounter){
                return 0;
            }
    ...
    public 
    {
            ...
            if(_earnings > 0) {
                ...
                withdrawnByUser[_playerAddress] = withdrawnByUser[_playerAddress].add(_earnings);
                address(uint160(_playerAddress)).transfer(_earnings.div(2));
                
                compoundInvestment(_playerAddress, _earnings.div(2));
    ...
    public {
            ...
            if(_earnings > 0) {
                ...
                compoundInvestment(_playerAddress, _earnings);
                
                emit withdrawEvent(_playerAddress, _earnings, now);
            }
        } 
        
    function compoundInvestment(address _playerAddress, uint256 _earnings) 
    private {
    
            uint256 amount = _earnings;
            address _referrer;
                
            _referrer = player[_playerAddress].referrer;
            
            if(player[_playerAddress].incomeLimitLeft == 0){
                player[_playerAddress].lastSettledTime = now;
                player[_playerAddress].currentInvestedAmount = amount;
            }
            else {
                player[_playerAddress].currentInvestedAmount = player[_playerAddress].currentInvestedAmount.add(amount);
            }
                    
                
            player[_playerAddress].incomeLimitLeft = player[_playerAddress].incomeLimitLeft.add(amount.mul(incomeTimes).div(incomeDivide));
            player[_playerAddress].totalInvestment = player[_playerAddress].totalInvestment.add(amount);
                
            playerInvestmentTracking[_playerAddress].investmentCounter += 1;
            playerInvestmentTracking[_playerAddress].investments.push(amount);
            playerInvestmentTracking[_playerAddress].earningsLimit.push(amount.mul(incomeTimes).div(incomeDivide));
                    
            player[_referrer].totalVolumeEth = player[_referrer].totalVolumeEth.add(amount);
            referralBonusTransferDirect(_playerAddress, amount);
            ...
            totalAmountInvested = totalAmountInvested.add(amount);
            emit investmentEvent (_playerAddress, amount);
    ...
    private
    {
        ...
        uint256 _newTime;
        if (_now > round[_rID].endTime && current_winner1 == address(0x0))
    ...
    private
    {
            ...
            if (now > round[_roundID].endTime && round[_roundID].ended == false) {
                    ...
                    address _winner1 = current_winner1;
                    address _winner2 = current_winner2;
                    address _winner3 = current_winner3;
                   
                    address(uint160(_winner1)).transfer(_poolAmount.mul(5).div(100));
                    address(uint160(_winner2)).transfer(_poolAmount.mul(2).div(100));
                    address(uint160(_winner3)).transfer(_poolAmount.mul(1).div(100));
                    address(uint160(owner)).transfer(_poolAmount.mul(2).div(100));
                    address(uint160(insuranceFunds)).transfer(_poolAmount.mul(5).div(100));
                 
                    
                    last_winner1 = _winner1;
                    last_winner2 = _winner2;
                    last_winner3 = _winner3;
                    current_winner1 = address(0x0);
                    current_winner2 = address(0x0);
                    current_winner3 = address(0x0);
                    _roundID++;
                    roundID++;
                    round[_roundID].startTime = now;
                    round[_roundID].endTime = now.add(rndMax);
                    round[_roundID].pool = _poolAmount.mul(85).div(100);
