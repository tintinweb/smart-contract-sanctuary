 ...
 */
contract TRC20 is ITRC20, Pausable, BlackList {
...
contract VLGPToken is TRC20, TRC20Detailed{
    
    constructor () public TRC20Detailed("VLG Plus Token", "VLGP", 8) {
        _mint(msg.sender, 5000000000 * (10 ** uint256(decimals())));
		transferOwnership(msg.sender);
    }
	
    function issue( uint256 _amount) public onlyOwner {
        require(_amount > 0,"Must be large than 0");
         _mint(owner, _amount * (10 ** uint256(decimals())));
    }

    function burn( uint256 _amount) public onlyOwner {
        uint256 token = _amount * (10 ** uint256(decimals()));
        require(_amount > 0,"Must be large than 0");
        require(balanceOf(msg.sender) >= token,"Insufficient balance");
