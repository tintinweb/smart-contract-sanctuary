//SourceUnit: clientWalletFactory.sol
...
contract clientWallet is Context, Ownable {
  ...
  event TRC20transfered(address indexed token, address indexed client, uint256 amount);
  event TRCtransfered(address clientWallet, uint256 amount);
  ...
  function transferTRC() public onlyOwner returns(bool){
    _transferTRC();
    return true;
  }

  function transferTRC20(address tokenAddress) public onlyOwner returns(bool){
    uint256 balanceContract = ITRC20(tokenAddress).balanceOf(address(this));
    _transferTRC2O(tokenAddress, balanceContract);
    emit TRC20transfered(tokenAddress, destination, balanceContract);
    return true;
  }

  function _transferTRC2O(address tokenAddress, uint256 _amount) internal {
    ITRC20(tokenAddress).transfer(destination, _amount);
  }

  function _transferTRC() internal {
    uint256 balanceThisContract = address(this).balance;
    payable(destination).transfer(balanceThisContract);
    emit TRCtransfered(address(this), balanceThisContract);
  }
