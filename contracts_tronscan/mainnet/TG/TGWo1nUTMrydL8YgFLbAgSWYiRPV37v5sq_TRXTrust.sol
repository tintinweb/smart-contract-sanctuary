...
contract TRXTrust {
    ...
    uint256[] plans = [2,4,6]; //%
    uint256[] plansDay = [250,50,25];//days 
    uint256[] levelsAffiliate = [2,2,2];
    ...
    uint private releaseTime = 1598544000;
    ...
    function investPlan(uint256 idRefer,uint8 idPlan) public payable returns (string memory) {
        ...
        ownerBalance += (msg.value * 5) / 100;
        developerBalance += (msg.value * 5) / 100;
        marketingBalance += (msg.value * 2) / 100;
        
        owner.transfer((msg.value * 5) / 100);
        developer.transfer((msg.value * 5) / 100);
        marketing.transfer((msg.value * 2) / 100);
