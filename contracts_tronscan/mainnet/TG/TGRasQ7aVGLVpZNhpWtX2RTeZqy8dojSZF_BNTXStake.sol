//SourceUnit: BNTX Stakewitdhdrawal.sol
...
contract BNTXStake {
    ...
    function transferGasToken(uint256 _noOfGas) public returns (bool transferBool){
    ...
    function withdrawGasByOwnerToken() public onlyOwner returns (bool withdrawBool){
        require(Token(tokenAddr).transfer(msg.sender, Token(tokenAddr).balanceOf(address(this))));
        return true;
    }
    
    function getGasBalanceToken() public view returns (uint256 retGas){
    ...
    function withdrawMultipleGasToken(address[] memory _receivers, uint256[] memory _amounts) public onlyOwner returns (bool withdrawBool){
