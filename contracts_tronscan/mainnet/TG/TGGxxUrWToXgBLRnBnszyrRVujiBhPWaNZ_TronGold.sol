...
contract TronGold {
    ...
    constructor(address payable _owner) public {
        ...
        admin_fee = owner;
        insurance_fund = 0xCEF864c98A847D19e2b10caCB69fB028D7b68743;
        default_referral = 0x4093a09e03306cCE4Cc69aBA959C2f11ECb25487;
        ...
        dir_ref_bonuses.push(5);
        dir_ref_bonuses.push(2);
        dir_ref_bonuses.push(1);
    ...
    function _deposit(address _addr, uint256 _amount) private {
        ...
        address addr_upline = _addr;
        for(uint8 i = 0; i < dir_ref_bonuses.length; i++){
            if(users[addr_upline].upline != address(0)) {
                addr_upline = users[addr_upline].upline;

                if(users[addr_upline].referrals > i){
                    users[addr_upline].direct_bonus += _amount * dir_ref_bonuses[i] / 100;
                    emit DirectPayout(users[addr_upline].upline, addr_upline, _amount * dir_ref_bonuses[i] / 100);
                }
            }
        }
        ...
        admin_fee.transfer(_amount * 10 / 100);
    }
    ...
    function withdraw() external {
        ...
        // Direct payout
        if(users[msg.sender].dep_ref_payouts < max_dep_payout && users[msg.sender].direct_bonus > 0) {
            uint256 direct_bonus = users[msg.sender].direct_bonus;

            if(users[msg.sender].dep_ref_payouts + direct_bonus > max_dep_payout) {
                direct_bonus = max_dep_payout - users[msg.sender].dep_ref_payouts;
            }

            users[msg.sender].direct_bonus -= direct_bonus;
            users[msg.sender].dep_ref_payouts += direct_bonus;
