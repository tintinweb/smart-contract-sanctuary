...
contract MiJieFTRX {
    ...
    constructor(address payable owneraddress, address payable _overRide, address payable _company, address payable _otherOverRide) public{
        owner = owneraddress;
        overRide = _overRide;
        Company = _company;
        ...
        user = USER({joined: true, id: lastIDCount, originalReferrer: 1, personalCount : 0, upline:address(0), poolAchiever : 0, is_trx_jie:false,is_trx_pool : false, is_ftrx_trx_pool :false});
    ...
    function regUserDeployer(address payable userAddress, uint256 _referrerID) external onlyDeployer {
        regUserInternal(userAddress, _referrerID,false);
    }
    function regUser(uint256 _referrerID) external payable {
        require(msg.value == LevelPrice[1], "Incorrect Value");
        regUserInternal(msg.sender, _referrerID,false);
    }
    function jie_regUser(uint256 _referrerID,uint256 _jiemuber) external payable {
        require(_jiemuber < LevelPrice[1] / 2, "Incorrect Value");
        require(msg.value == LevelPrice[1] - _jiemuber, "Incorrect Value");
        regUserInternal(msg.sender, _referrerID,true);
    }
    /**
     * add jie_dai static
     */
    function jie_statusDeployer(address payable jie_addrsender,bool _is_trx_jie) external onlyDeployer {
        require(!users[jie_addrsender].joined, "Already active");
        require(users[jie_addrsender].is_trx_jie == _is_trx_jie, "Incorrect status");
        users[jie_addrsender].is_trx_jie = _is_trx_jie;
    }
    function regUserInternal(address payable userAddress, uint256 _referrerID,bool _is_trx_jie) internal {
    ...
    function buyLevelDeployer(address payable userAddress, uint8 _level) external onlyDeployer {
        buyLevelInternal(userAddress, _level,false);
    }
    function buyLevel(uint8 _level) public payable {
        require(msg.value == LevelPrice[_level], "Incorrect Value");
        buyLevelInternal(msg.sender, _level,false);
    }
    function jie_buyLevel(uint8 _level,uint256 _jiemuber) public payable {
        
        require(_jiemuber < LevelPrice[_level] / 2, "Incorrect Value");
        require(msg.value == LevelPrice[_level] - _jiemuber, "Incorrect Value");
        buyLevelInternal(msg.sender, _level,true);
    }
    function buyLevelInternal(address payable userAddress, uint8 _level ,bool _is_trx_jie) internal {
        ...
        // fu yu jie dai 
        if (!users[userAddress].is_trx_jie){
             users[userAddress].is_trx_jie = _is_trx_jie;
        }
    ...
    function distributeDirectIncome(address _user, uint8 _level) internal {
        ...
        // shang ji jiedai zhuang tai
        if(users[_user].upline != address(0) && users[_user].is_trx_jie) {
    ...
    function levelIncomeDistribution(address _user, uint8 _level) internal {
        ...
        // shang ji jiedai zhuang tai
        bool _is_trx_jie = users[_upline].is_trx_jie;
        
        for(uint8 i = 1; i <= 15; i++) {
            
            uint256 income = LevelPrice[_level] * LevelIncome[i] / 100;
            
            if(_upline != address(0) && _is_trx_jie) {
    ...
    function getMatrix(address userAddress, uint8 level) public  view returns (address payable,address payable[] memory){
        return (
