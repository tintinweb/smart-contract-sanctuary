...
//SourceUnit: LetsBTTRefund.sol

pragma solidity ^0.5.4;

contract LetsBTTRefund {
    ...
    mapping(address => uint256) public usersRefunds;
    
    function() payable external {}
    
    trcToken public token = 1002000;
    LetsBTT public letsBTT;
    address owner;
    
    uint256 public MAX_REFUND = 75;//75%
    bool public refundEnabled;
    
    constructor(address _addr) public{
        require(isContract(_addr), "Only contract");
        letsBTT = LetsBTT(_addr);
        owner = msg.sender;
    }
    
    function withdraw() external {
        require(refundEnabled, "Refund disabled");
        require(getBalance()>0, 'Balance empty');
        uint256 refundable = getRefundable(msg.sender);
        require(refundable > 0,'Nothing to refund');
        if(refundable > getBalance()) refundable = getBalance();
        usersRefunds[msg.sender] = usersRefunds[msg.sender] + refundable;
        msg.sender.transferToken(refundable, token);
        emit Refund(msg.sender, refundable);
    }
    
    function getRefundable(address _addr) public view returns (uint256){
        (uint256 totalInvestment, uint256 totalWithdrawn) = letsBTT.getUserBalance(_addr);
        if(totalInvestment == 0) return 0;
        if(totalWithdrawn >= MAX_REFUND * totalInvestment / 100) return 0;
        uint256 refundableVal = MAX_REFUND * totalInvestment / 100 - totalWithdrawn;
        if(refundableVal <= usersRefunds[_addr]) return 0;
        return refundableVal - usersRefunds[_addr];
    }
    
    function getUserBalance(address _addr) external view returns (uint256, uint256){
        return letsBTT.getUserBalance(_addr);
    }
    ...
    function setRefundEnabled(bool _val) external onlyOwner {
        refundEnabled=_val;
    }
