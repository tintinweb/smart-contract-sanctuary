...
// File: contracts/LibertyMaster.sol

contract LibertyMaster {
    using SafeMath for uint256;

    LBTToken public rewardToken;

    uint256 public lastMintTime;
    uint256 public totalStaked = 0;
    uint256 public mintedCurrentLevel = 0;
    ...
    constructor(LBTToken _rewardToken) public {
        owner = msg.sender;
    ...
    function _refund(address _user, uint256 _amount) private {
        _mintTokens();
    ...
    function _stake(address _address, address _referrer, uint256 _amount) private {
        require(msg.value >= 100 trx, 'Too low value');
        require(_address != owner, "Owner can't stake");
        _mintTokens();

        address referrer = _referrer == address(0x0) ? owner : _referrer;

        require(userInfo[referrer].exists, 'Invalid Referrer');
        ...
        if (!user.exists) {
            ...
            userInfo[referrer].totalReferrals = userInfo[referrer]
                .totalReferrals
                .add(1);
    ...
    function unstake(uint256 _amount) public {
        _mintTokens();
        require(!paused, 'Please retry later');

        UserInfo storage user = userInfo[msg.sender];

        require(user.exists, 'Invalid User');

        uint256 amount = _amount == 0 ? user.investment : _amount;

        _claimStaked();

        emit Operation('unstake', user.addr, user.referrer, amount);

        if (msg.sender != owner) {
            totalStaked = totalStaked.sub(amount);
            user.investment = user.investment.sub(amount, 'Insufficient funds');
    ...
    function _mintTokens() private {
        ...
        if (totalStaked != 0) {
            uint256 toMint = totalStaked
                .mul(timePassed)
                .mul(LEVEL_YIELD[currentLevel])
                .div(REWARD_INTERVAL);
            // Add ref percent
            toMint = toMint.add(toMint.mul(REF_REWARD_PERCENT).div(100));

            mintedCurrentLevel = mintedCurrentLevel.add(toMint);

            // If cap is reached mint as much as possible
            if (rewardToken.totalSupply().add(toMint) > rewardToken.cap()) {
                toMint = rewardToken.cap().sub(rewardToken.totalSupply());
            }
            rewardToken.mint(address(this), toMint);

            if (
                mintedCurrentLevel >= LEVEL_LIMIT[currentLevel] &&
                currentLevel <= LEVEL_LIMIT.length
            ) {
                levelChangeTime[currentLevel] = block.timestamp;
                currentLevel++;
                mintedCurrentLevel = 0;
    ...
    function _claimStaked() internal {
        ...
        if (pendingReward > rewardToken.balanceOf(address(this))) {
            pendingReward = rewardToken.balanceOf(address(this));
        }

        rewardToken.transfer(user.addr, pendingReward);

        uint256 referralReward = pendingReward.mul(REF_REWARD_PERCENT).div(100);

        userInfo[user.referrer].referralReward = userInfo[user.referrer]
            .referralReward
            .add(referralReward);

        emit ClaimStaked(user.addr, user.referrer, pendingReward);
