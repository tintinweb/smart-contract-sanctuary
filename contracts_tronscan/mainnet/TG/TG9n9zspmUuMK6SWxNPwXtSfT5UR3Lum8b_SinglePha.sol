...
contract SinglePha is LPTokenWrapper {
    ...
    //gha单币质押每天1%收益
    uint256 private rewardRatePerCoin = 11574074;            
    ...
    constructor (address _y, address _token, uint256 _initreward) public {
        deployer = msg.sender;
    ...
    function stake(uint256 amount) public updateReward(msg.sender) checkRewardToken {
    ...
    function rewardPerToken(address account) internal view returns (uint256) {
        ...
        uint256 lastTime = 0 ;
        if(flag){
            lastTime = lastUpdateTime[account];
        }else{
            lastTime = starttime;
        }
        ...
        rewardPerTokenStored.add(
            ...
            .sub(lastTime)
            .mul(rewardRatePerCoin)
            .div(1e8)
        );
    }

    modifier checkRewardToken() {
        require(totalRewards < initreward);
        _;
    }
