...
contract Autumn is TRC20Token {
    constructor () public TRC20Token(100000000, 6) {}
}
...
import "./TRC20TokenBase.sol";

contract TRC20Token is TRC20TokenBase {
    constructor(uint supply_, uint256 decimals_) TRC20TokenBase(supply_, decimals_) public {}
...
contract TRC20TokenBase is ITRC20, Rescuable {
    using SafeMath for uint256;
    ...
    constructor(uint256 supply, uint256 decimals) public {
        _balances[msg.sender] = _supply = supply.mul(10 ** (_decimals = decimals));
    }
    ...
    function transferFrom(address src, address dst, uint wad) public returns (bool) {
        require(src != dst, "token-base-self-transfer");
        
        if (src != msg.sender && _approvals[src][msg.sender] != uint256(-1)) {
            require(_approvals[src][msg.sender] >= wad, "token-base-insufficient-approval");
            _approvals[src][msg.sender] = _approvals[src][msg.sender].sub(wad);
        }

        require(_balances[src] >= wad, "token-base-insufficient-balance");
