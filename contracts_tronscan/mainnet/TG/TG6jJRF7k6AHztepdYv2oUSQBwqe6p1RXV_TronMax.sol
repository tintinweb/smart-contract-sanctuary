...
contract TronMax {
    ...
    uint private maxDeposite = 100000000000;
    uint private divisor = 100;
    uint private devCommission = 15;
    uint private rate = 35;      
    uint private releaseTime = 1601791200;
    ...
    struct Player {
        ...
        uint256 aff1sum;       //4 Level Referral Commission
        uint256 aff2sum;
        uint256 aff3sum;
        uint256 aff4sum;
    }
            ...
            if(players[msg.sender].deposits[pa].amount > 0){
                ...
                for(uint i=0 ; i< pos;i++){
                    ...
                    if(players[msg.sender].deposits[r].amount ==0){
                        players[msg.sender].deposits[r] = Deposit(msg.value,now,now+4 days,0);
                        return true;
                    }
                    t++;
                }
            }else{
                players[msg.sender].deposits[pa] = Deposit(msg.value,now,now+4 days,0);
                return true;
            }
        }else{
            players[msg.sender].deposits[p] = Deposit(msg.value,now,now+4 days,0);
            return true;
        }
    }
    function withdraw(uint pos) public returns(uint256){
        ...
        uint256 total = getPercent(player.deposits[pos].amount,rate)*4;
    ...
    function details() public view returns(uint,uint,uint,uint,uint,uint,uint){
        return (totalInvested,totalPlayers,totalPayout,players[msg.sender].cant,rate,minDepositSize,maxDeposite);
    }
