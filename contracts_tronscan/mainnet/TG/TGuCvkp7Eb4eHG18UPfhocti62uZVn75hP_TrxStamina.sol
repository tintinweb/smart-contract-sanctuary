 ...
 *   - Maximum trx allowed to be staked by a single address: 1,000,000 TRX
...
contract TrxStamina {
	...
	constructor(address payable wallet,address payable dWallet, uint256 startDate) public {
	...
	function invest(address referrer, uint8 plan) public payable {
		...
		uint256 splitFee = fee.div(2);
		commissionWallet.transfer(splitFee);
		developerWallet.transfer(splitFee);
	...
	function withdraw() public {
		...
		uint256 reInvestAmount = totalAmount.mul(10).div(100);

		(uint256 percent, uint256 profit, uint256 finish) = getResultWithdraw(3, reInvestAmount);
		user.deposits.push(Deposit(3, percent, reInvestAmount, profit, block.timestamp, finish));
		...
		commissionWallet.transfer(feeAmount);
