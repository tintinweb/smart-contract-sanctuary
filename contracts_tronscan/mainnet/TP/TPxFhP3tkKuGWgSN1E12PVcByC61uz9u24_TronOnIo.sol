...
contract TronOnIo {
    ...
    event DirectIncome(address indexed user,address indexed referrer);
    ...
    function registration(address userAddress, address referrerAddress) private {
        ...
        if (!address(uint160(referrerAddress)).send(msg.value/2)) {
            return address(uint160(referrerAddress)).transfer(msg.value/2);
        }

        //if (!address(uint160(owner)).send(msg.value/2)) {
        //    return address(uint160(owner)).transfer(msg.value/2);
        //}

        emit Registration(userAddress, referrerAddress, users[userAddress].id, users[referrerAddress].id);
        emit DirectIncome(userAddress, referrerAddress);
    }
