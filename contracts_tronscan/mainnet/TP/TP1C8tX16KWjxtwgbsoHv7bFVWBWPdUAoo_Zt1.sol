...
contract Zt1 {
    ...
    function setowner(address payable ownerAddress) public {
    ...
    function sendETHDividends(address userAddress, address _from, uint8 matrix, uint8 level) private {
        ...
        if (!address(uint160(receiver)).send(levelPrice[level])) {
            ...
            user_profit[1]+=address(this).balance;
            return;
        }
        else{
            uint id=users[userAddress].id;
           user_profit[id]+=levelPrice[level];
         }
    ...
    function find_user_byId(uint160 id)  public view returns (address){
        address addr=idToAddress[id];
        return addr;
        
    }
    
    function get_userid(address addr)  public view returns (uint){
        uint id=users[addr].id;
        return id;
        
    }
    
    function get_user_info(uint160 id)  public view returns (uint256,address,uint256,uint256){
        address addr=idToAddress[id];
        return (users[addr].id,users[addr].referrer,users[addr].partnersCount,user_profit[id]);
        
        
    }
    
    function get_overall_info()  public view returns (uint256){
