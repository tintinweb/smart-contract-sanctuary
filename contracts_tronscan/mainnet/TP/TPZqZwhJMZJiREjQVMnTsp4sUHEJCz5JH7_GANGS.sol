...
contract GANGS {
    ...
    uint256 public constant AIRDROP_COLLECT_TIME_PAUSE = 24 hours;
    ...
    function collectAirdrop() public returns (bool success) {
        ...
        require(player.airdropCollectedTime == 0 || now - player.airdropCollectedTime >= AIRDROP_COLLECT_TIME_PAUSE, "should be never collected before or more then 24 hours from last collect");
        uint256 _rep = (10 ** REP_AND_CASH_DECIMALS).mul(50);
        player.airdropCollectedTime = now;
        player.rep = (player.rep.add(_rep));
    ...
    function getIncome(address addr) public view returns (uint256, uint256) {
        ...
        if (player.airdropUnlockedTime > 0) {
            _rep = _rep.add((10 ** REP_AND_CASH_DECIMALS).mul(50).div(24));
        }
