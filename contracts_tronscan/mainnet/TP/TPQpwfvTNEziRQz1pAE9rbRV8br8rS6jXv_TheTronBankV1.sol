...
contract TheTronBankV1{
    ...
    uint256[] public BOX_RATE_DIVIDER = [8640000, 4320000, 2880000, 1440000]; //1%, 2%, 3%, 4% daily
    uint256[] public BOX_PERIOD = [0, 100 days, 46 days, 30 days];
    ...
    address payable constant public ADMIN_WALLET = address(0x41660073BF449FA23584AE5D72EA8B42F7F43363C2);
	address payable constant public creator = address(0x41F3A3864DB5E1B94B381D287E16D66B3AE2B78BFB);
    ...
    function register(address referrer) public {
        require(users[msg.sender].parent == address(0), "Inviter address was set in the TheTronBank network!");
        require(users[referrer].parent != address(0) || referrer == creator, "Inviter address does not exist in the TheTronBank network!");
        _registration(msg.sender, referrer);
    }

    function deposit(address referrer, uint256 boxId, string memory predict) public payable {
        require(now > 1600344000, "Investment time not reached!");
    ...
    function reinvest(uint256 amount, uint256 boxId, string memory predict) public  {
        ...
        address addr = msg.sender;
        require(users[addr].parent != address(0), "the address does not exist in the TheTronBank network!");
