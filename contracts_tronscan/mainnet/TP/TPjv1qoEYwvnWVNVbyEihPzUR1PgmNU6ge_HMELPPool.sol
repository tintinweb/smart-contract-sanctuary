...
contract DateWrapper is Ownable {
    ...
    function currentHours() public view returns (uint) {
        return now.add(extraTime).div(1 hours);
    }
...
contract HMELPPool is LPTokenWrapper, DateWrapper {

    InviteContract public inviteContract;
    IERC20 public token;
    uint256 public initReward;
    uint256 private inviterRate = 6;
    ...
    uint256 public DEA = 72 hours;

    constructor( uint256 _startTime, address _token, address _lp) public {
        token = IERC20(_token);
        lpToken = IERC20(_lp);
    ...
    function getReward() public updateReward(msg.sender) checkEnd(msg.sender) checkHalve checkStart {
    ...
    modifier checkHalve(){
        if (block.timestamp.add(extraTime) >= periodFinish) {
            ...
            if (level >= 36) {
                ...
                rewardRate = uint256(47000000 * 10 ** 12).div(30 * DURATION);
            } else {
                if(level == 5) {
                    rewardRate = uint256(80).add(5 * 10).mul(10000).mul(10 ** 12).div(DURATION);
                } else {
                    rewardRate = uint256(80).add(level * 10).mul(10000).mul(10 ** 12).div(DURATION);
                }
            }
            level++;
