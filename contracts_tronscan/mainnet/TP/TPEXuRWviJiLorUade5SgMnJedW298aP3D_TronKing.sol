...
contract TronKing {
    ...
    uint[] public REFERRAL_PERCENTS = [600, 300, 150, 150, 100, 100, 40, 30, 20, 10];
    uint constant public ADMIN_MARKETING_FEE = 900; 
	uint constant public DEV_FEE = 100;
    uint constant public MAX_HOLD_PERCENT = 100;
    uint constant public MAX_COMMUNITY_PERCENT = 20;
    uint constant public COMMUNITY_BONUS_STEP = 25000;
    uint constant public PERCENTS_DIVIDER = 10000;
    uint constant public CONTRACT_BALANCE_STEP = 25000000e6;
    uint constant public MAX_CONTRACT_PERCENT = 20;
    ...
    constructor(address payable amAddr, address payable devAddr) public {
        require(!isContract(amAddr) && !isContract(devAddr));
        amAddress = amAddr;
		devAddress = devAddr;
    ...
    function invest(address referrer) public payable {
        ...
        uint amFee = msgValue.mul(ADMIN_MARKETING_FEE).div(PERCENTS_DIVIDER);
		uint devFee = msgValue.mul(DEV_FEE).div(PERCENTS_DIVIDER);

        amAddress.transfer(amFee);
		devAddress.transfer(devFee);

        emit FeePayed(msg.sender, amFee.add(devFee));
    ...
    function getUserReferralsStats(address userAddress) public view returns (address, uint64, uint24[10] memory) {
        User storage user = users[userAddress];
