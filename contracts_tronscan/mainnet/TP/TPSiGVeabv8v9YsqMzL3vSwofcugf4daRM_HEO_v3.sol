...
contract HEO_v3 is Managable {
	...
	function dataMigrate(address _addr, address _upline, uint256 _value) public onlyOwner {
		_setUpline(_addr, _upline);
		_initDeposit(_addr, _value);
    }

	function recharge(uint256 _amount) public payable onlyOwner {
		require(_amount >= 0, "Bad amount");

		users[msg.sender].payouts = 0;
		users[msg.sender].depositTop = _amount;
        ...
        uexts[msg.sender].totalDeposits += _amount;

		users[msg.sender].refBonus += _amount;
	...
	function _initDeposit(address _addr, uint256 _amount) private {
		...
		if (users[_addr].upline != address(0)) {
			...
			users[__addr].refBonus += _amount;
		}
        
		users[owesFund].refBonus += _amount / 200;
		users[supplyFund].refBonus += _amount / 200;
		users[safeFund].refBonus += _amount / 200;
		users[encourageFund].refBonus += _amount / 200;

		// uint256 adminScale = _amount / 100;
		// adminWithdraw += adminScale;
		users[adminAddr].refBonus += _amount / 100;
	}
	...
	function maxPayoutOf(address _addr) view private returns(uint256 payout) {
		uint256 amount = payoutOfInterest(_addr) + users[_addr].investLast + users[_addr].investPayouts + users[_addr].poolBonus + users[_addr].refBonus + uexts[_addr].directBonus;
		return amount;
	}
	...
	function contractInfo() view external returns(uint256 rtotalUsers, uint256 rtotalDeposited, uint256 rtotalWithdraw, uint40 rpoolLastDraw, uint256 rpoolBalance, uint256 radminWithdraw, uint256 toketBalance, uint256 safeFunds) {
		uint256 refBonus = users[safeFund].refBonus;
		return (totalUsers, totalDeposited, totalWithdraw, poolLastDraw, poolBalance, adminWithdraw, usdtToken.balanceOf(address(this)), refBonus);
	}
