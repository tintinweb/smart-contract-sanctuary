...
//SourceUnit: WGRNPC.sol

pragma solidity ^0.5.8;

import "./ITokenDeposit.sol";

contract WGRNPC is ITokenDeposit {
    string public name = "Wrapped Elon Green";
    string public symbol = "WGRNPC";
    uint8  public decimals = 6;
    trcToken  public grnpcTokenId = trcToken(1004051);
    ...
    function deposit() public payable {
        require(msg.tokenid == grnpcTokenId, "deposit tokenId not GRNPC[1002000]");
    ...
    function withdraw(uint256 sad) public {
        require(balanceOf_[msg.sender] >= sad, "not enough WGRNPC balance");
        require(totalSupply_ >= sad, "not enough WGRNPC totalSupply");
        balanceOf_[msg.sender] -= sad;
        totalSupply_ -= sad;
        msg.sender.transferToken(sad, grnpcTokenId);
