...
contract LPTokenWrapper is IPool,Governance {
    ...
    IERC20 public _TokenOne = IERC20(0xEfB94dE7eB42f46D3080b83E8517167a5BE595D3); //todo
    IERC20 public _TokenTwo = IERC20(0x3dbB29ace9e850360773de63226AFc037EA580C0); //todo

    uint256 public _oneRate = 1;
    uint256 public _twoRate = 1;

    uint256 private _totalSupplyone;
    uint256 private _totalSupplytwo;
    mapping(address => uint256) private _balancesOne;
    mapping(address => uint256) private _balancesTwo;

    function totalSupply() public view returns (uint256) {
        return _totalSupplyone.mul(_oneRate).add(_totalSupplytwo.mul(_twoRate));
    }
    
    function setOne_token(address one_token) public  onlyGovernance {
        //return _totalSupply;
        _TokenOne = IERC20(one_token);
    }

    function settwo_token(address two_token) public  onlyGovernance {
        //return _totalSupply;
        _TokenTwo = IERC20(two_token);
    }

    function setTwotokensRate(uint newonerate, uint newtworate) public  onlyGovernance {
        _oneRate = newonerate;
        _twoRate = newtworate;
    }

    function balanceOf(address account) public view returns (uint256) {
        return _balancesOne[account].mul(_oneRate).add(_balancesTwo[account].mul(_twoRate));
    }

    function stake(uint256 amountone, uint256 amounttwo) public {
        _totalSupplyone = _totalSupplyone.add(amountone);
        _totalSupplytwo = _totalSupplytwo.add(amounttwo);
        _balancesOne[msg.sender] = _balancesOne[msg.sender].add(amountone);
        _balancesTwo[msg.sender] = _balancesTwo[msg.sender].add(amounttwo);

        _TokenOne.safeTransferFrom(msg.sender, address(this), amountone);
        _TokenTwo.safeTransferFrom(msg.sender, address(this), amounttwo);

    }
    // address admin_address = 0xb07c1B3Cd13245e5351FD29CbB3995eB7D083044;     
    function withdraw(uint256 amountone, uint256 amounttwo) public {
        require(amountone > 0, "amout > 0");
        require(amounttwo > 0, "amout > 0");

         _totalSupplyone = _totalSupplyone.sub(amountone);
        _totalSupplytwo = _totalSupplytwo.sub(amounttwo);
         _balancesOne[msg.sender] = _balancesOne[msg.sender].sub(amountone);
        _balancesTwo[msg.sender] = _balancesTwo[msg.sender].sub(amounttwo);
        

        _TokenOne.safeTransfer( msg.sender, amountone);
       _TokenTwo.safeTransfer( msg.sender, amounttwo);
    }
...
contract UniswapReward is LPTokenWrapper{
    ...
    function seize(IERC20 token, uint256 amount) external onlyGovernance{
        ...
        require(token != _TokenOne, "stake");
        require(token != _TokenTwo, "stake");
    ...
    function stake(uint256 amountone, uint256 amounttwo)
        public
        updateReward(msg.sender)
        checkHalve
        checkStart
    {
        require(amountone > 0, "Cannot stake 0");
        require(amounttwo > 0, "Cannot stake 0");
        super.stake(amountone,amounttwo);

        _lastStakedTime[msg.sender] = now;

    }

    function withdraw(uint256 amountone, uint256 amounttwo)
        public
        updateReward(msg.sender)
        checkHalve
        checkStart
    {
        require(amountone > 0, "Cannot withdraw 0");
        require(amounttwo > 0, "Cannot withdraw 0");
        super.withdraw(amountone, amounttwo);
