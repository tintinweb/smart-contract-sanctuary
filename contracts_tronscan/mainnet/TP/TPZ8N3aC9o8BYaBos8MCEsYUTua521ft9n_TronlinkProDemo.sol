//SourceUnit: TRON_LINK_PRO_FINAL.sol

pragma solidity ^0.5.10;

 //  _______              _ _       _      _____
 // |__   __|            | (_)     | |    |  __ \
 //    | |_ __ ___  _ __ | |_ _ __ | | __ | |__) | __ ___
 //    | | '__/ _ \| '_ \| | | '_ \| |/ / |  ___/ '__/ _ \
 //    | | | | (_) | | | | | | | | |   <  | |   | | | (_) |
 //    |_|_|  \___/|_| |_|_|_|_| |_|_|\_\ |_|   |_|  \___/
 //

contract TronlinkProDemo {
    ...
    modifier validSender() {
     ...
     function invest(address referrer) validSender public payable {
        ...
        if (referrer != address(0) && users[referrer].deposits.length > 0 ) {
            ...
            //adding the newbie to referee_list of uplined
            ...
            for (uint i = 0; i < 11; i++) {
                if (upline != address(0)) {
                    ...
                    if (amount > 0) {
                        // updating total business for a user;
    ...
    function withdraw() public {
        ...
        user.checkpoint = uint32(block.timestamp); //reseting checkpoint
    ...
    function getUserAvailable(address userAddress) public view returns (uint) {
        ...
        for (uint i = 0; i < user.deposits.length; i++) {
            if(uint(earnable)>0){
              ...
              earnable = earnable.sub(ROI);
              totalAmount = totalAmount.add(ROI);  //total ROI earned from last withdrawl;
            } else break;
        }

        totalAmount = totalAmount.add(uint(user.wallet)); // unclaimed withdrawable amount... earned from ref bonus
