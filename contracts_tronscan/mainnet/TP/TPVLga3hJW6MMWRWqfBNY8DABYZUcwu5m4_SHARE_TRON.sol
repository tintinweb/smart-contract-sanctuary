...
contract SHARE_TRON{
	...
	event LevelUpgrade(string  member_name, string  current_level,string promoter,address indexed sender,uint Extend);
	...
	function buyLevel(string memory member_name, string memory current_level,string memory promoter,address payable[]  memory  _contributors, uint256[] memory _balances,uint _Extend) public payable
	{
		multisendTRX(_contributors,_balances);
		emit LevelUpgrade(member_name, current_level,promoter,msg.sender,_Extend);
	}
