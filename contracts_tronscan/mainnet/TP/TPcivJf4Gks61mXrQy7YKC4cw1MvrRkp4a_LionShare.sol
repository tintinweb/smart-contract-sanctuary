...
contract LionShare {
  ...
  event Upgrade(address member, address sponsor, uint matrix, uint level);
  event PlacementL1(address member, address sponsor, uint level, uint8 placement, bool passup);
  event PlacementL2(address member, address sponsor, uint level, address placedUnder, uint8 placementSide, bool passup);
  event Cycle(address indexed member, address fromPosition, uint matrix, uint level);
  event PlacementReEntry(address indexed member, address reEntryFrom, uint matrix, uint level);
  event FundsPayout(address indexed member, address payoutFrom, uint matrix, uint level);
  event FundsPassup(address indexed member, address passupFrom, uint matrix, uint level);
  ...
  function purchaseLevel(uint _matrix, uint _level) external payable isMember(msg.sender) blockReEntry() {
    ...
    emit Upgrade(msg.sender, activeSponsor, _matrix, _level);
  ...
  function sponsorPlaceL2(address _addr, address _sponsor, uint _level, bool passup) internal {
    ...
    emit PlacementL2(_addr, _sponsor, _level, member.placedUnder, member.placementSide, passup); 
  ...
  function compLevel(address _addr, uint _matrix, uint _level) public isOwner(msg.sender) isMember(_addr) {
    ...
    for (uint num = (activeLevel + 1);num <= _level;num++) {
      ...
      emit Upgrade(_addr, activeSponsor, _matrix, num);
