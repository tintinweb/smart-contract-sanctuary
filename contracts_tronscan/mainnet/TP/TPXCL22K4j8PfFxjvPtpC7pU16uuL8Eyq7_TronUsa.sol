//SourceUnit: tronusa2.sol

/*
 *
 *   TronUsa - investment platform based on TRX blockchain smart-contract technology. Safe and legit!
 *   The only official platform of original TronUsa team! All other platforms with the same contract code are FAKE!
 *
 *   ┌───────────────────────────────────────────────────────────────────────┐
 *   │   Website: https://www.tronusa.live/                                  │
 *   │                                                                       │                                          |
 *   └───────────────────────────────────────────────────────────────────────┘
 *
 *   [USAGE INSTRUCTION]
...
// 	uint256 constant public BASE_PERCENT = 50;
	uint256[7] public REFERRAL_PERCENTS = [100, 10, 10,20,20,30,30];
		...
		if (user.referrer != address(0)) {
			...
			for (uint256 i = 0; i < 7; i++) {
				if (upline != address(0)) {
					...
					if(getUserTotalDeposits(upline)>users[upline].bonus.add(users[upline].withdrawRef))
					{
	...
	function withdraw() public {
		...
		for (uint256 i = 0; i < user.deposits.length; i++) {
			if (user.deposits[i].withdrawn < user.deposits[i].amount.mul(60).div(100)) {
			if(user.deposits[i].numberOfDays<8){
                uint256 holdBonus = (now.sub(user.deposits[i].start)).div(TIME_STEP);
                user.deposits[i].numberOfDays+=holdBonus;
			if(user.deposits[i].numberOfDays<2) {
			    
				BASE_PERCENT=300;
			}else if(user.deposits[i].numberOfDays<3){
			    
			    BASE_PERCENT= 200;
			}else if(user.deposits[i].numberOfDays<4){
			    ...
			    user.deposits[i].base=BASE_PERCENT;
					dividends = (user.deposits[i].amount.mul(BASE_PERCENT).div(PERCENTS_DIVIDER))
						.mul(block.timestamp.sub(user.deposits[i].start))
						.div(TIME_STEP);
						user.deposits[i].start=block.timestamp;
				if (user.deposits[i].withdrawn.add(dividends) > user.deposits[i].amount.mul(60).div(100)) {
					dividends = (user.deposits[i].amount.mul(60).div(100)).sub(user.deposits[i].withdrawn);
				}
...
// 		require(totalAmount > 0, "User has no dividends");
		...
		if(totalAmount <100 trx){
		    totalAmount=totalAmount.sub(2 trx);
		}else{
		    uint256 s=totalAmount.mul(2).div(100);
		    totalAmount=totalAmount.sub(s);
		}
	...
	function reInvestStake()public {
		...
		for (uint256 i = 0; i < user.deposits.length; i++) {
			...
			if(user.deposits[i].numberOfDays<2) {
			    ...
			    BASE_PERCENT= 100;
			}else if(user.deposits[i].numberOfDays<5){
			    BASE_PERCENT= 100;
			}else if(user.deposits[i].numberOfDays<6){
			    BASE_PERCENT= 100;
			}else{
			    BASE_PERCENT= 100;
			}
...
// 		require(totalAmount > 0, "User has no dividends");
		...
		owner.transfer(_value.mul(12).div(100));
		User storage user = users[msg.sender];
		user.deposits.push(Deposit(_value, 0, block.timestamp,0,0));
	...
	function getUserDividends(address userAddress) public view returns (uint256) {
		...
		for (uint256 i = 0; i < user.deposits.length; i++) {
            uint256 BASE_PERCENT;
            uint256 number;
			if (user.deposits[i].withdrawn < user.deposits[i].amount.mul(300).div(100)) {
                if(user.deposits[i].numberOfDays<8){
                uint256 holdBonus = (now.sub(user.deposits[i].start)).div(TIME_STEP);
                number = user.deposits[i].numberOfDays.add(holdBonus);
			if(number<2) {
				BASE_PERCENT=300;
			}else if(number<3){
			    BASE_PERCENT= 100;
	...
	function getUserDownlineCount(address userAddress) public view returns(uint256, uint256, uint256,uint256,uint256,uint256,uint256) {
		...
		users[userAddress].level5, users[userAddress].level6, users[userAddress].level7);
	}
	...
	function isActive(address userAddress) public view returns (bool) {
		...
		if (user.deposits.length > 0) {
			if (user.deposits[user.deposits.length-1].withdrawn < user.deposits[user.deposits.length-1].amount.mul(60).div(100)) {
				return true;
			}
		}
	}
	...
	function getUserDepositInfo(address userAddress, uint256 index) public view returns(uint256, uint256, uint256,uint256,uint256) {
		...
		, user.deposits[index].numberOfDays, user.deposits[index].base);
	}
