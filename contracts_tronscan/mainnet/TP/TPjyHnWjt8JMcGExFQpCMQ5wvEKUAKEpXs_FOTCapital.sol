//SourceUnit: FOTCapital-Contract-Live.sol
...
contract FOTCapital {
    ...
    string public name = "FOT Capital";
    ...
    function tokensToTron_(uint256 _tokens) internal view returns (uint256) {
        ...
        uint256 _tronReceived = (SafeMath.sub((((tokenPriceInitial_ + (tokenPriceIncremental_ * (_tokenSupply / 1e6))
            ) - tokenPriceIncremental_) * (tokens_ - 1e6)
            ), (tokenPriceIncremental_ * ((tokens_ ** 2 - tokens_) / 1e6)) / 2)/ 1e6);
