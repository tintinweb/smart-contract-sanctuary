//SourceUnit: TriumphDefi.sol

pragma solidity ^0.4.25;

contract TriumphDefi {
  struct Investor {
    bool registered;
    address referrer;
    uint staked;
  ...
  uint public TOKEN_ID = 1003655;
  uint DAY = 1 days;
  uint START_AT = 26853409;
  ...
  function stake() external payable playtime {
    ...
    require(msg.tokenvalue >= 47 trx, 'too low amount');
  ...
  function unstake() external playtime {
    Investor storage investor = investors[msg.sender];
    require(block.timestamp - investor.stakedAt >= 1 days, 'Wait 1 days for unstaking');
  ...
  function sell() external payable playtime {
    ...
    support.transfer((msg.tokenvalue * getSellTokenPrice() / 1000000) / 47);
