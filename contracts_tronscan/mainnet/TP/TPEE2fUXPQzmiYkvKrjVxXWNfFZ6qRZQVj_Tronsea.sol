...
contract Tronsea is Ownable {
    using SafeMath for uint256;
    uint256 private constant INTEREST_CYCLE = 1 days;
    uint256 private constant DEVELOPER_ENTRY_RATE = 100; //per thousand
    ...
    function _init() private {
		...
		REFERENCE_LEVEL_RATE[2]=30;
		REFERENCE_LEVEL_RATE[3]=20;
		REFERENCE_LEVEL_RATE[4]=2;
		REFERENCE_LEVEL_RATE[5]=2;
		REFERENCE_LEVEL_RATE[6]=2;
		REFERENCE_LEVEL_RATE[7]=2;
		REFERENCE_LEVEL_RATE[8]=2;
		REFERENCE_LEVEL_RATE[9]=2;
		REFERENCE_LEVEL_RATE[10]=2;
		REFERENCE_LEVEL_RATE[11]=2;
		REFERENCE_LEVEL_RATE[12]=2;
		REFERENCE_LEVEL_RATE[13]=2;
    ...
    function getInvestorInfoByUID(uint256 _uid) public view returns (uint256, uint256, uint256, uint256[] memory, uint256,  uint256[] memory, uint256[] memory) {
        ...
        uint256[] memory currentDividends = new  uint256[](investor.planCount);
         uint256[] memory RefCount = new uint256[](13);
    ...
    function _addInvestor(address _addr, uint256 _referrerCode) private returns (uint256) {
		...
		while (_referrerCode >= REFERRER_CODE && ln<13) 
		{
