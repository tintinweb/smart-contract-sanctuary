...
contract TronBank {
    ...
    function getProfit() public returns (uint256) {
        ...
        require(avl,'is disabled get profit');
        Users storage us = users[msg.sender];
        us.timeprofit = now + 1 minutes ;
        uint256 pft=0;
        require(now>= us.timetmp,'not your time');
        uint256 p = now-us.timetmp;
        uint256 t = getPercent(us.invest,percent);
        pft =p*(t/24/60/60);
        us.timetmp = now;
        us.profit += pft;
    ...
    function setAvl(bool _avl) public onlyOwner returns(bool){
        avl = _avl;
        return avl;
    }
