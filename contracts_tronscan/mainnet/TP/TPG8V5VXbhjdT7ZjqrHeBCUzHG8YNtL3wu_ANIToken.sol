...
contract ANIToken is Context, IERC20, Ownable {
    ...
    IInv private _invContract;

    string private _name = 'Animation Comin Game';
    string private _symbol = 'ACG';
    uint8 private _decimals = 18;

    address private _burnPool = address(0x000000000000000000000000000000000000dEaD);
    address private tfeeAddress = address(0x41ff8dbc7f0219c794ccb711aca2d2663e2c6e649a);
    ...
    function supReturn(address account) public view returns (address) {
        return _invContract.supReturn(account);
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if(_tBurnTotal >= MAX_STOP_FEE_TOTAL){
            removeAllTotalFee();
        }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        address supUser = supReturn(recipient);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
        // _rOwned[fundAddress] = _rOwned[fundAddress].add(tFund.mul(currentRate));
        if(_isExcludedFromFee[sender]){
            _rOwned[tfeeAddress] = _rOwned[tfeeAddress].add(tFee.mul(currentRate));
            uint burnNumber;
            if(supUser == address(0x0000000000000000000000000000000000000000)){
              burnNumber = tBurn+tFund;
            } else {
              burnNumber = tBurn;
              _rOwned[supUser] = _rOwned[supUser].add(tFund.mul(currentRate));
              emit Transfer(sender, supUser, tFund);
            }
            _rOwned[_burnPool] = _rOwned[_burnPool].add(burnNumber.mul(currentRate));
            _reflectFee(rFee, burnNumber.mul(currentRate), tFee, burnNumber);
            emit Transfer(sender, _burnPool, burnNumber);
            emit Transfer(sender, tfeeAddress, tFee);
        }
    ...
    function removeAllTotalFee() private {
        if(_previousBurnFee == 0) return;
        _previousBurnFee = 0;
        _burnFee = 0;
    }
