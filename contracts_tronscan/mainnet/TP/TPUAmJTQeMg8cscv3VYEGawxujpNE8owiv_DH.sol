...
contract DH is Context, IERC20, Ownable {
    ...
    uint256 public eachBlock=2400;

    uint256 public buyRate = 5;

    uint256 public minRate =5;
    ...
    function setMinRate(uint256 _minRate) public onlyOwner returns (bool) {
    ...
    function setFeeRate(uint256 _feeRate) public onlyOwner returns (bool) {
    ...
    function setSubRate(uint256 _subRate) public onlyOwner returns (bool) {
    ...
    function setEachBlock(uint256 _eachBlock) public onlyOwner returns (bool) {

        eachBlock=_eachBlock;

        return true;

    }

    function setBuyRate(uint256 _buyRate) public onlyOwner returns (bool) {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if(_lpAddr==sender && sender!=owner())	{
            ...
            uint256 buyNumber = amount.mul(100-buyRate).div(100);

            uint256 feeNumber = amount.mul(buyRate).div(100);

            _rOwned[recipient] = _rOwned[recipient].add(buyNumber.mul(getRate));

            _rOwned[_lpAddr] = _rOwned[_lpAddr].add(feeNumber.mul(getRate));

            

            emit Transfer(sender, recipient, buyNumber);

            

            emit Transfer(sender, _lpAddr,feeNumber);

            

			userAmounts[recipient].add(buyNumber);
		    ...
		    uint256 num = b.div(eachBlock);

		    uint256 rate = feeRate.sub(num.mul(subRate));
		    ...
		    uint256 fee = rate.mul(amount).div(100);
