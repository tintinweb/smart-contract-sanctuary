...
contract Tron2tron {
    ...
    uint256 private minuteRate =  1388888;  //DAILY 12%
    ...
    function collect(address _addr) internal {
        ...
        if (secPassed >  3456000  && player.time > 0) {
            secPassed = 3456000; // 100 days
    ...
    function distributeRef(uint256 _trx, address _affFrom) private {
        ...
        if (_affAddr2 != address(0)) {
            _affRewards = (_trx.mul(13)).div(100);
            totalRefDistributed = totalRefDistributed.add(_affRewards);
        ...
        if (_affAddr3 != address(0)) {
         ...
         if (_affAddr7 != address(0)) {
            ...
            totalRefDistributed = totalRefDistributed.add(_affRewards);
            players[_affAddr7].affRewards = players[_affAddr7].affRewards.add(
                _affRewards
            );
