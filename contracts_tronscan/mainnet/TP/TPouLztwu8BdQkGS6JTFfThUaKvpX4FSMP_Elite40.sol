...
contract Elite40 is Ownable {
    ...
    uint256 public constant PLAN_TERM = 6 days;
    uint256 public constant CONTRACT_LIMIT = 300;
    uint256 internal constant LAUNCH = 1609416000;
    
    uint256 public  contract_balance;
    uint256 public  contract_unholdBalance;
    ...
    function getUnholdBalance() public view returns (uint256) {
        return contract_unholdBalance;
    }
    ...
    function getInvestorInfoByUID(uint256 _uid) public view returns (uint256,uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256[] memory) {
        ...
        investor.planCount,
        investor.checkpoint,
        investor.reinvestpoint,
        investor.withdrawcount,
        newDividends
        );
    }

    function getInvestmentPlanByUID(uint256 _uid) public view returns (uint256[] memory, uint256[] memory, uint256[] memory, bool[] memory, uint256) {
        if (msg.sender != owner) {
    ...
    function invest(uint256 _referrerCode) public payable {
        require(block.timestamp >= LAUNCH, 'Try again later');
    ...
    function withdraw() public {
        ...
        require(block.timestamp > uid2Investor[uid].checkpoint + (1 days * uid2Investor[uid].withdrawcount) , "Only once a day x withdrawal count");
        uid2Investor[uid].checkpoint = block.timestamp;
        uid2Investor[uid].withdrawcount =  uid2Investor[uid].withdrawcount + 1;
        ...
        if(withdrawalAmount>0){
            ...
            require(withdrawalAmount <= contract_unholdBalance, "Not enough Daily TRX available to pay");
            uint256 reinvestAmount = withdrawalAmount.div(2);
            if(withdrawalAmount > 20e9 ){
                reinvestAmount = withdrawalAmount.sub(10e9);
            }
            //refresh contract_unholdBalance
            contract_unholdBalance = contract_unholdBalance.sub(withdrawalAmount.sub(reinvestAmount));
            //reinvest
    ...
    function reinvest() public {
        ...
        uint256 uid = address2UID[msg.sender];
        require(uid != 0, "Can not reinvest without any investments");
        require(block.timestamp > uid2Investor[uid].reinvestpoint + (1 days * uid2Investor[uid].reinvestcount) , "Only once a day x reinvest count");
        uid2Investor[uid].reinvestcount =  uid2Investor[uid].reinvestcount + 1;
        uid2Investor[uid].reinvestpoint = block.timestamp;
    ...
    function updateBalance() public {
        ...
        contract_unholdBalance = contract_balance.mul(CONTRACT_LIMIT).div(1000);
    }
