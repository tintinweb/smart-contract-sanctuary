...
contract MEOX is ITokenDeposit {
    string public name = "Metronix";
    string public symbol = "MEOX";
    uint8  public decimals = 18;
    trcToken  public meoxTokenId = trcToken(1002877);
    ...
    function deposit() public payable {
        require(msg.tokenid == meoxTokenId, "deposit tokenId not MEOX[1002877]");
        // tokenvalue is long value
        balanceOf_[msg.sender] += msg.tokenvalue * 10 ** 12;
        totalSupply_ += msg.tokenvalue * 10 ** 12;
        emit Transfer(address(0x00), msg.sender, msg.tokenvalue * 10 ** 12);
        emit Deposit(msg.sender, msg.tokenvalue * 10 ** 12);
    }

    function withdraw(uint256 sad) public {
        require(balanceOf_[msg.sender] >= sad, "not enough MEOX balance");
        require(totalSupply_ >= sad, "not enough MEOX totalSupply");
        balanceOf_[msg.sender] -= sad;
        totalSupply_ -= sad;
        msg.sender.transferToken(sad/10 ** 12, meoxTokenId);

        emit Transfer(msg.sender, address(0x00), sad/10 ** 12);
        emit Withdrawal(msg.sender, sad/10 ** 12);
    }
