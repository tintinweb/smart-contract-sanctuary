...
contract Finimo {
    ...
    event NewDeposit(address indexed addr, uint256 amount, uint256 plan);
    event MatchPayout(address indexed addr, address indexed from, uint256 amount);
    event Withdraw(address indexed addr, uint256 amount);

    constructor(address _owner, address __address, address _tokenContractAddress) public {
        tokenContractAddress = _tokenContractAddress;
        owner = _owner;
        withdrawFee = 0;
        _address = __address;
        
        plans.push(Plan(500, 500));
        plans.push(Plan(100, 200));
        plans.push(Plan(50, 150));
    ...
    function deposit(uint256 _plan, address _upline) external payable {
        require(plans[_plan].life_days > 0, "Plan not found");
        require(msg.value >= 5e8, "Zero amount");
        if(_plan==0){
            require(msg.value >= 5e8, "Zero amount");    
        }
        else if(_plan==1){
            ...
            require(msg.value >= 15e8, "Zero amount");
        }
        else if(_plan==3){
            require(msg.value >= 2e9, "Zero amount");
        }
    ...
    function withdraw() payable external {
        ...
        tokencontract.transfer(msg.sender,player.total_invested*1/100);
...
//     }
    ...
    */
    ...
    function investmentsInfo(address _addr) view external returns(uint256[] memory ids, uint256[] memory endTimes, uint256[] memory amounts, uint256[] memory totalWithdraws) {
        Player storage player = players[_addr];
