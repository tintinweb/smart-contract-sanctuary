//SourceUnit: IRecehExchange.sol

pragma solidity ^0.5.8;

interface IRecehExchange {
...
//SourceUnit: IRecehExchangeFactory.sol

pragma solidity ^0.5.8;

interface IRecehExchangeFactory {
...
//SourceUnit: RecehExchange.sol

pragma solidity ^0.5.8;
import "./TRC20.sol";
import "./ITRC20.sol";
import "./IRecehExchangeFactory.sol";
import "./IRecehExchange.sol";
import "./ReentrancyGuard.sol";
import "./TransferHelper.sol";



contract RecehExchange is TRC20,ReentrancyGuard {
  ...
  // Variables
  string public name;         // RecehExchange
  string public symbol;       // RX
  uint256 public decimals;     // 6
  ITRC20 token;                // address of the TRC20 token traded on this contract
  IRecehExchangeFactory factory;     // interface for the factory that created this contract
  ...
  function setup(address token_addr) public {
    ...
    factory = IRecehExchangeFactory(msg.sender);
  ...
  private returns (uint256)
  {
    ...
    uint256 tokens_bought = IRecehExchange(exchange_addr).trxToTokenTransferInput.value(wei_bought)(min_tokens_bought, deadline, recipient);
    emit TrxPurchase(buyer, tokens_sold, wei_bought);
    emit Snapshot(buyer,address(this).balance,token.balanceOf(address(this)));
    return tokens_bought;
  }
  ...
  private returns (uint256)
  {
    ...
    require(exchange_addr != address(this) && exchange_addr != address(0), "illegal exchange addr");
    uint256 trx_bought = IRecehExchange(exchange_addr).getTrxToTokenOutputPrice(tokens_bought);
    uint256 token_reserve = token.balanceOf(address(this));
    uint256 tokens_sold = getOutputPrice(trx_bought, token_reserve, address(this).balance);
    // tokens sold is always > 0
    ...
    require(address(token).safeTransferFrom(buyer, address(this), tokens_sold), "transfer failed");
    uint256 trx_sold = IRecehExchange(exchange_addr).trxToTokenTransferOutput.value(trx_bought)(tokens_bought, deadline, recipient);
    emit TrxPurchase(buyer, tokens_sold, trx_bought);
    emit Snapshot(buyer,address(this).balance,token.balanceOf(address(this)));
    return tokens_sold;
  }
  ...
  /**
   * @notice Deposit TRX && Tokens (token) at current ratio to mint RX tokens.
   * @dev min_liquidity does nothing when total JUSTSWAP supply is 0.
   * @param min_liquidity Minimum number of RX sender will mint if total JUSTSWAP supply is greater than 0.
   * @param max_tokens Maximum number of tokens deposited. Deposits max amount if total JUSTSWAP supply is 0.
   * @param deadline Time after which this transaction can no longer be executed.
   * @return The amount of RX minted.
   */
  ...
  /**
   * @dev Burn RX tokens to withdraw TRX && Tokens at current ratio.
   * @param amount Amount of RX burned.
   * @param min_trx Minimum TRX withdrawn.
   * @param min_tokens Minimum Tokens withdrawn.
   * @param deadline Time after which this transaction can no longer be executed.
   * @return The amount of TRX && Tokens withdrawn.
