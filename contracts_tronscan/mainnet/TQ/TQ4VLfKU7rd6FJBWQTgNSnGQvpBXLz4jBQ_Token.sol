...
contract ERC20 is IERC20, Ownable {
	...
	function freeze(address account, uint256 value) onlyOwner public returns (bool success) {
        require(value > 0); 
		require(account != address(0), "ERC20: freeze from the zero address"); 
        require(_balances[account] >= value);
		
		_balances[account] = _balances[account].sub(value);
        _freeze[account] = _freeze[account].add(value);
        emit Freeze(account, value);
        return true;
    }
	
	function unfreeze(address account, uint256 value) onlyOwner public returns (bool success) {
        require(value > 0); 
		require(account != address(0), "ERC20: unfreeze from the zero address"); 
        require(_freeze[account] >= value);
		
        _freeze[account] = _freeze[account].sub(value);
...
contract Token is ERC20, ERC20Detailed {
    constructor () public ERC20Detailed("AVEXTOKEN", "AVEX", 18) {
