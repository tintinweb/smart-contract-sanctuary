...
contract SuperWave {
    ...
    address payable private _holdNode;
    ...
    constructor(address payable superNode,address payable creatorNode,address payable holdNode,address payable feeNode) public {
        _owner = msg.sender;
    ...
    function take(uint256 amount) public {
        ...
        _aReJoin = _aReJoin.add(amount.sub(takeAmount));

        order.mTake = order.mTake.add(amount);
        order.mReJoin = order.mReJoin.add(amount.sub(takeAmount));
