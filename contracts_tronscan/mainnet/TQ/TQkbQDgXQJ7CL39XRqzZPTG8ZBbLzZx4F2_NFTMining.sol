...
contract NFTMining is Ownable {
    ...
    function _depositStorage(address _tokenAddress, uint256 _value, uint256 _tokenType, tokenStruct storage t_, userStruct storage u_) private {
        ...
        if(u_.amount == 0) {
            ...
            uint256 _totalRed = (block.number.sub(t_.blocknum)).mul(everyNumber[0]).mul(everyNumber[_tokenType]).div(100).div(_length);
            // 最新的accSushi
            // uint256 _nowSushi = t_.accSushi + _totalRed / t_.totalDeposit;
            uint256 _nowSushi = t_.accSushi.add(_totalRed.mul(1e18).div(t_.totalDeposit));
        ...
        uint256 _userRed = u_.amount.mul(_nowSushi.sub(u_.accSushi)).div(1e18).mul(block.number.sub(u_.blocknum)).div(block.number.sub(t_.blocknum));
    ...
    function _getRed(uint256 _tokenType, tokenStruct memory _t, userStruct memory _u) private view returns (uint256 _userRed) {
        ...
        uint256 _totalRed = (block.number.sub(_t.blocknum)).mul(everyNumber[0]).mul(everyNumber[_tokenType]).div(100).div(_length);
        // 最新的accSushi
        // uint256 _nowSushi = _t.accSushi + _totalRed / _t.totalDeposit;
        uint256 _nowSushi = _t.accSushi.add(_totalRed.mul(1e18).div(_t.totalDeposit));
        // 计算用户收益
        // _userRed = _u.amount * (_nowSushi - _u.accSushi);
        _userRed = _u.amount.mul(_nowSushi.sub(_u.accSushi)).div(1e18).mul(block.number.sub(_u.blocknum)).div(block.number.sub(_t.blocknum));
    }
