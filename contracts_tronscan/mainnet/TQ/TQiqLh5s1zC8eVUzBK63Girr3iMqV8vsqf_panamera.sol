...
contract panamera is Context, ITRC20, Ownable {
    ...
    string private _name = 'panamera';
    string private _symbol = 'panamera';
    ...
    uint256 public  MAX_STOP_FEE_TOTAL = 4200 * 10**uint256(_decimals);
    ...
    function setfundWallet(address fundWallet) public onlyOwner {
        _fundAddress = fundWallet;
    }

    function getfundWallet() public view onlyOwner returns (address) {
        return _fundAddress;
    }
    ...
    function setAllFee(uint256 liquidityFee_, uint256 burnFee_, uint256 fundFee_) public onlyOwner {
        _liquidityFee = liquidityFee_;
        _burnFee = burnFee_;
        _fundFee = fundFee_;
    }

    function setPreviousFee(uint256 liquidityFee_, uint256 burnFee_, uint256 fundFee_) public onlyOwner {
        _previousLiquidityFee = liquidityFee_;
        _previousBurnFee = burnFee_;
        _previousFundFee = fundFee_;
    }
}