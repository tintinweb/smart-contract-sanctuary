...
contract MasterChef is Ownable {
    ...
    // 7 days
    uint256 public duration = 7 * 28800;

    RewardToken public rewardToken;
    // Dev fund (25%, initially)
    uint256 public devFundDivRate = 4;
    // gover fund (5%, initially)
    uint256 public goverFundDivRate = 20;
    // insurance fund (5%, initially)
    uint256 public insuranceFundDivRate = 20;
    uint256 public tokenPerBlock = 3 * 1e18;
    uint256 public MIN_TOKEN_PER_BLOCK = 15 * 1e15;
    ...
    function calTokenPerBlock(uint256 _blockToken) view internal returns (uint256){
        uint256 _devFund = _blockToken.div(devFundDivRate);
        uint256 _goverFund = _blockToken.div(goverFundDivRate);
        uint256 _insuranceFund = _blockToken.div(insuranceFundDivRate);
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (block.number >= periodEndBlock) {
            ...
            tokenPerBlock = tokenPerBlock.mul(98).div(100);
            if (tokenPerBlock <= MIN_TOKEN_PER_BLOCK) {
                tokenPerBlock = MIN_TOKEN_PER_BLOCK;
            }
    ...
    function claimOtherFund() public {
        require(msg.sender == controller, "!controller");
    ...
    function claimDevFund() public {
        require(msg.sender == controller, "!controller");
        uint256 perDevFund = devFund.div(devaddrs.length());
        for (uint256 i = 0; i < devaddrs.length() - 1; i++) {
            rewardToken.transfer(devaddrs.get(i), perDevFund);
        }
        uint256 remainFund = devFund.sub(perDevFund.mul(devaddrs.length() - 1));
        rewardToken.transfer(devaddrs.get(devaddrs.length() - 1), remainFund);
        devFund = 0;


    }

    function addDev(address _devaddr) public {
        require(msg.sender == controller, "!controller");
        require(devaddrs.length() < 100, "less 100");
        devaddrs.add(_devaddr);
    }

    function removeDev(address _devaddr) public {
        require(msg.sender == controller, "!controller");
    ...
    function insurance(address _insuranceaddr) public {
        require(msg.sender == controller, "!controller");
        insuranceaddr = _insuranceaddr;
    }

    function setInsuranceFundDivRate(uint256 _insuranceFundDivRate) public {
        require(msg.sender == controller, "!controller");
...
contract RewardToken is ERC20, ERC20Detailed, ERC20Burnable, Ownable {
    ...
    constructor () public ERC20Detailed("abelo", "abelo", 18) {
    }
