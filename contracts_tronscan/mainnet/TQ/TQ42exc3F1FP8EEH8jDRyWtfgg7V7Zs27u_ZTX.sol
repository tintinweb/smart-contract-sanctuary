...
/*! ZTX.sol | SPDX-License-Identifier: MIT License */
...
contract ZTX is TRC20 {
    address public owner;
    bool public stopmint;

    modifier onlyOnwer() {
        require(msg.sender == owner, "ZTX: ACCESS_DENIED");
        _;
    }
    ...
    function mint(address to, uint256 value) external onlyOnwer {
        require(!stopmint, "ZTX: MINT_ALREADY_STOPED");

        _mint(to, value);
    }

    function burn(uint256 value) external onlyOnwer {
        require(balanceOf[msg.sender] >= value, "ZTX: INSUFFICEINT_FUNDS");
