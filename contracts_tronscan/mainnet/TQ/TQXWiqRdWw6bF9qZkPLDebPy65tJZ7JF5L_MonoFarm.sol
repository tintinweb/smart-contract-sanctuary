...
contract MonoFarm is Ownable{
    ...
    constructor(ITRC20 _KeyBiz) public {
        KeyBiz = _KeyBiz;
        allocation[1] = 5;
        allocation[3] = 20;
        allocation[6] = 45;
    ...
    function farm(uint256 _amount, uint256 _lockableDays) public 
    {
        ...
        KeyBiz.transferFrom(msg.sender, address(this), _amount);
        ...
        function pendindRewards() public view returns(uint256 reward)
    {
        ...
        for(uint256 z=0 ; z< deposit.length;z++){
        userInfo storage user = users[msg.sender][z];
        uint256 lockTime = user.depositeTime+(user.lockableDays*time);
        if(now > lockTime ){
    ...
    function harvest() public {
        userInfo storage user = Users[msg.sender];
            ...
            KeyBiz.transfer(msg.sender,  totalwithdrawAmount);
    ...
    function emergencyWithdraw(uint256 WORTHWHILEAmount) public onlyOwner {
         KeyBiz.transfer(msg.sender, WORTHWHILEAmount);
    }
