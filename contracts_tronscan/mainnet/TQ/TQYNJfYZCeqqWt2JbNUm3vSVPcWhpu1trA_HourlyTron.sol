...
contract HourlyTron {
    ...
    uint private rate = 49;      
    uint private releaseTime = 1604296800;
    ...
    function deposit(address payable _affAddr) public payable  returns(bool){
        ...
        players[msg.sender].time = now+1 hours;
            ...
            if(players[msg.sender].deposits[pa].amount > 0){
                ...
                for(uint i=0 ; i< pos;i++){
                    ...
                    if(players[msg.sender].deposits[r].amount ==0){
                        players[msg.sender].deposits[r] = Deposit(msg.value,now,now+100 hours,0);
                        return true;
                    }
                    t++;
                }
            }else{
                players[msg.sender].deposits[pa] = Deposit(msg.value,now,now+100 hours,0);
                return true;
            }
        }else{
            players[msg.sender].deposits[p] = Deposit(msg.value,now,now+100 hours,0);
            return true;
        }
    }
    function withdraw(uint pos) public returns(uint256){
        ...
        uint256 total = getPercent(player.deposits[pos].amount,100)*2;
    ...
    function distributeRef(uint256 _trx, address payable _affFrom) private{
        ...
        if (_affAddr1 != address(0)) {
            _affRewards = getPercent(_trx,12);
    ...
    function details() public view returns(uint,uint,uint,uint,uint){
        return (totalInvested,totalPlayers,totalPayout,players[msg.sender].cant,rate);
    }
    ...
    function getHistory() public view returns(uint256,uint,uint,uint){
        Player storage player = players[msg.sender];
        return (player.trxDeposit,player.interestProfit,player.affRewards,player.reinvest);
    }
