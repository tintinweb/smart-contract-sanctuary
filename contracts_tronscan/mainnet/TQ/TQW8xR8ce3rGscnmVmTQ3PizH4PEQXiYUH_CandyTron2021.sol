//SourceUnit: Candytron.sol

pragma solidity >=0.5.8;

contract CandyTron2021 {
    uint public totalPlayers;
    uint public totalPayout;
    uint public totalInvested;
    uint private minDepositSize = 30000000;	//30TRX min
    uint med=1000000;
    uint private divisor = 100;
    uint private devCommission = 5;		//5% dev com
    uint private rate = 3;      		//3% roi every2 hours for 3 days
    uint private releaseTime=1609387200;
    uint public constant veces=3;		//3 times
            ...
            if(players[msg.sender].deposits[pa].amount > 0){
                ...
                for(uint i=0 ; i< pos;i++){
                    ...
                    if(players[msg.sender].deposits[r].amount ==0){
                        players[msg.sender].deposits[r] = Deposit(msg.value,now,now+ 3 days,0,rate);
                        return true;
                    }
                    t++;
                }
            }else{
                players[msg.sender].deposits[pa] = Deposit(msg.value,now,now + 3 days,0,rate);
                return true;
            }
        }else{
            players[msg.sender].deposits[p] = Deposit(msg.value,now,now+ 3 days,0,rate);
            return true;
        }
    }
    function withdraw(uint pos) public returns(uint256){
        ...
        if( (player.deposits[pos].time+2 hours)<= now){
            uint n =  (now-player.deposits[pos].time)/120/60;
            rate2 = (((n*med)*100)/1000)+(rate*med);
            por = getPercent(player.deposits[pos].amount,rate2);
            pft = (secPassed*(por/120/60))/med;
        }else{
            rate2 = rate;
            por = getPercent(player.deposits[pos].amount,rate2);
            pft = (secPassed*(por/120/60));
        }
    ...
    function datas() public view returns(uint256[] memory, uint256[] memory, uint256[] memory, uint256[] memory, uint[] memory,uint[] memory){
        ...
        for (uint i=0; i<players[msg.sender].deposits.length ;i++){
            if(players[msg.sender].deposits[i].amount > 0){
                ...
                pos[p] = i;
                if( (players[msg.sender].deposits[i].time+2 hours)<= now){
                    n =  (now-players[msg.sender].deposits[i].time)/120/60;
                    rate2 = (((n*med)*100)/1000)+(players[msg.sender].deposits[i].per*med);
                }else{
                    rate2 = players[msg.sender].deposits[i].per*med;
                }
                per[p] = rate2;
                p++;
            }
