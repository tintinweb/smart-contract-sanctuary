...
/*
    ...
    */
    ...
    function divSet(address user, uint256 _value) internal returns (bool)
    {
        ...
        if(topiaTokenContractAddress != address(0)){
            ...
            if(usersDiamondFrozen >= 1000000 && usersDiamondFrozen <= 1000000000){
                extraMint = usersDiamondFrozen * 1000 / 1000000 ;
            }	
            else if (usersDiamondFrozen > 1000000000)
            {
                extraMint = 1000000;
            }	    
            uint256 extraMintValue = _value * extraMint / 1000000;
            extraMintValue -= calculatePercentage(extraMintValue,reduceExtraMinting);
            _value -= calculatePercentage(_value,reduceMainMinting);
	...
	function settleBet(address user, uint64 ticket) public onlyAgent {
        ...
        if (refPoolContractAddress != address(0x0) && InterfaceREFERRAL(refPoolContractAddress).referrers(user) != address(0x0))
        {           
            // Processing referral system fund distribution
            // [✓] 0.003% trx to referral if any.
            InterfaceREFERRAL(refPoolContractAddress).payReferrerBonusOnly(user, bet.amount/10);
        }
		emit BetSettled(user, ticket );        
