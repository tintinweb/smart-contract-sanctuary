...
contract ChipGame is Bonus {
    ...
    function getPayoutOf(address addr) public view returns (uint) {
        ...
        for (uint i = 0; i < players[addr].chips.length && i < 200; i++) {
            ...
            if (from_ < to) {
                ...
                value += ((to - from_) / 3600) * payoutPerHour;
            }
        }
    ...
    function deployAddAlreadyBuyedChips(address user, uint[] calldata chips) external onlyOwner {
        require(!isContractEnable, "");
        for(uint i=0; i < chips.length; i++) {
            uint price = _calcPrice(user, chips[i]);
            _processBuyChip(user, chips[i], price);
        }
    }
