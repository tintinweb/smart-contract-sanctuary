    ...
    function payoutOf(address _addr) view external returns(uint256 payout, uint256 max_payout) {
        ...
        if(users[_addr].deposit_payouts < max_payout) {
			...
			if(users[_addr].deposit_payouts < max_payout.div(2))
			{
				...
				payout = ((users[_addr].deposit_amount * ((block.timestamp - users[_addr].deposit_time) / 1 days) / 100) - users[_addr].deposit_payouts).mul(2);
				
			}
            
            if(users[_addr].deposit_payouts + payout > max_payout) {
                payout = (max_payout/2) - users[_addr].deposit_payouts;
