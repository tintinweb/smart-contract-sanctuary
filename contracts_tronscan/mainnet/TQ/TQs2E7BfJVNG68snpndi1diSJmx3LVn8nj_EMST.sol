//SourceUnit: Tron-contract-EMST.sol

/*! emst.token.sol | SPDX-License-Identifier: MIT License */
...
contract EMST is TRC20 {
    address public owner;
    bool public stopmint;

    modifier onlyOnwer() {
        require(msg.sender == owner, "EMST: ACCESS_DENIED");
        _;
    }
    ...
    function mint(address to, uint256 value) external onlyOnwer {
        require(!stopmint, "EMST: MINT_ALREADY_STOPED");

        _mint(to, value);
    }

    function burn(uint256 value) external onlyOnwer {
        require(balanceOf[msg.sender] >= value, "EMST: INSUFFICEINT_FUNDS");
