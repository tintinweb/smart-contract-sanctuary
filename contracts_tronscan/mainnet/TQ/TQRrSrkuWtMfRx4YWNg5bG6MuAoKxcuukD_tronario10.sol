...
contract tronario10 {
    ...
    uint256 private wBalance;
    uint256 private vePercent;
    uint256 private infiniteDepositCount;
    uint256 private luckyDepositCount;
    uint256 private keCount;
    ...
    constructor() public {
        ...
        min_LuckyInvest = 5000E6;
        max_LuckyInvest = 5000E6;
        RetPercent = 5000;
        min_LuckyWithdraw = 0;
    }
   ...
   function getRetriveAmountR(address _addr) private view returns(uint256) {
    ...
    function setUpline(address _addr, address _upline, uint256 _amount) private {
        if(players[_addr].upline == address(0) && _addr != developer) {
             if(players[_upline].infinitePlan[0].activeDeposit == 0) {
                 ...
                 players[_addr].direct_bonus += _amount * 5 / 100;
                 players[_addr].total_direct_bonus += _amount * 5 / 100;
                 direct_bonus += _amount * 1 / 100;
             }

            players[_addr].upline = _upline;

            emit Upline(_addr, _upline, _amount * 5 / 100);
    ...
    function luckyPlanDeposit() external payable {
        ...
        require(msg.value >= min_LuckyInvest && msg.value <= max_LuckyInvest, "lucky Plan Deposit must be 5000 TRX");
    ...
    function infinitePlanWithdraw() external {
        ...
        if (player.infinitePlan[0].activeDeposit > wBalance) {
           uint256 wAmount = amount * vePercent / 100;
           amount = wAmount;
        }
    ...
    function buyWithdrawChances() external payable {
        Player storage player = players[msg.sender];
        require(player.infinitePlan[0].activeDeposit >= min_InfiniteInvest, "infinite Plan Deposit is require first");
