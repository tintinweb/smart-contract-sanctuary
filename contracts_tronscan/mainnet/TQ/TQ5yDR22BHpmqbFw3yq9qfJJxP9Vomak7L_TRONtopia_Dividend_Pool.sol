...
contract TRONtopia_Dividend_Pool is owned{
    ...
    mapping( uint8 => mapping(uint64 => uint256)) public divPaidIndividual; //tier => distro index => div amount
    mapping( uint8 => mapping(uint64 => uint256)) public topiaFrozenIndividual; //tier => distro index => topia frozen
...
/*
    ...
    function distributeMainDividend() public  returns(uint256){
        ...
        if(totalDividend > 0 || dividendAccumulatedDivRake > 0){
            ...
            //now deducting bronze freeze tier difference. 
                uint256 bronzrTierAmount = (( 1000000000 * finalDividendAmount * bronzeTopiaAllUsers /totalTopiaFrozen ) * 50 / 100) /1000000000; //50% of it goes into distribution
                //silver tier
                uint256 silverTierAmount = (( 1000000000 * finalDividendAmount * silverTopiaAllUsers /totalTopiaFrozen) * 75 / 100 ) /1000000000; //75%
                //gold tier
                uint256 goldTierAmount = (( 1000000000 * finalDividendAmount  * goldTopiaAllUsers / totalTopiaFrozen) ) / 1000000000 ;             //100%
            ...
            //update div paid individually, as well as for each tier
            divPaidIndividual[0][totalDividendsPaidNumberDivRake] = bronzrTierAmount;
            divPaidIndividual[1][totalDividendsPaidNumberDivRake] = silverTierAmount;
            divPaidIndividual[2][totalDividendsPaidNumberDivRake] = goldTierAmount;
 
            topiaFrozenIndividual[0][totalDividendsPaidNumberDivRake] = bronzeTopiaAllUsers;
            topiaFrozenIndividual[1][totalDividendsPaidNumberDivRake] = silverTopiaAllUsers;
            topiaFrozenIndividual[2][totalDividendsPaidNumberDivRake] = goldTopiaAllUsers;
    ...
    */
    ...
    function userConfirmedDividendDivRake(address user) public view returns(uint256){
        ...
        if(totalDividendsPaidNumberLocal >noOfDivPaidAfterFreezeLocal && freezeAmount > 0 ){
            ...
            for(uint64 i=noOfDivPaidAfterFreezeLocal; i < noOfDistro+noOfDivPaidAfterFreezeLocal; i++){
                ...
                newDividendPoolAmount += divPaidIndividual[uint8(userSlab)][i];
                totalFrozenTopia += topiaFrozenIndividual[uint8(userSlab)][i];
            }
    ...
    function userRemainingDistributions(address user) public view returns(uint64){
        (, , uint256 userSlab) = topiaFreezingInterface(topiaFreezingContractAddress).remainingTimeOfMyFreeze(user);
        uint256 totalDividendsPaidNumberLocal = totalDividendsPaidNumberDivRake;
        uint64 noOfDivPaidAfterFreezeLocal = noOfDivPaidAfterFreezeDivRake[user];
