...
contract vaultGame is  owned
{
    ...
    mapping(address => uint256) public burnInBlock;
    ...
    function startVaultPlay(address token, uint amountToBurn) internal returns(bool)
    {
        ...
        if (amountToBurn >= minimumVoucherToBurn)
        {
            ...
            burnInBlock[starter] = block.number;
            if(! oddEven)
            {
    ...
    function pushMyBurn(address token, uint amountToBurn) internal returns(bool)
    {
        ...
        require(amountToBurn<= maxBurnLimit,"In one call can't burn beyond limit");
        require(burnInBlock[callingUser] != block.number , "already burn in this block");
        burnInBlock[callingUser] = block.number;
        require(globalToken[token], "invalid token address");
        ...
        uint256 thisAmount = usersVaultBurnAmount[callingUser];

        if (thisAmount >= usersVaultBurnAmount[firstHighestBurner])
        {
            if(callingUser != firstHighestBurner && callingUser != secondHighestBurner && callingUser != thirdHighestBurner)
            {
                thirdHighestBurner = secondHighestBurner;
                secondHighestBurner = firstHighestBurner;
            }
            else if (callingUser == secondHighestBurner)
            {
                secondHighestBurner = firstHighestBurner;
            }
            else if (callingUser == thirdHighestBurner)
            {
                thirdHighestBurner = secondHighestBurner;
                secondHighestBurner = firstHighestBurner;
            }
        ...
        if (thisAmount >= usersVaultBurnAmount[secondHighestBurner] && thisAmount < usersVaultBurnAmount[firstHighestBurner] )
        {
            thirdHighestBurner = secondHighestBurner;
            secondHighestBurner = callingUser;
        }  

        if (thisAmount >= usersVaultBurnAmount[thirdHighestBurner]  && thisAmount < usersVaultBurnAmount[secondHighestBurner] )
        {
