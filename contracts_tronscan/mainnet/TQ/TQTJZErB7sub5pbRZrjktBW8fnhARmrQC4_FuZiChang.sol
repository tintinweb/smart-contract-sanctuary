...
contract FuZiChang is Ownership {
    ...
    function syncUidmap(address token, uint64 offset) public payable onlyOwner  returns(uint64)
    {
        FuZiChang old = FuZiChang(token);

        uint64 i = 0;
        address addr = address(0);
        while(old.uidmap(freetimeid) != address(0) && i < offset){
            if(freetime.length <= freetimeid){
                freetime.push(old.freetime(freetimeid));
            }else{
                freetime[freetimeid] = old.freetime(freetimeid);
            }

            addr = old.uidmap(freetimeid);
            uidmap[freetimeid] = addr;
            freetimeid++;
            i++;
        }
    ...
    function syncAddrmap(address token, uint64 offset) public payable onlyOwner  returns(uint64)
    {
        FuZiChang old = FuZiChang(token);
        uint64 i = 0;
        address addr = address(0);
        while(old.addrmap(currentid) != address(0) && i <= offset){
            addr = old.addrmap(currentid);
            addrmap[currentid] = addr;
            currentid++;
            i++;
        }
    ...
    function syncUData(address uaddr, address uper,uint32 id,uint32 valid,uint32 level,uint32 ucount,uint32 vcount,uint256 validinvest,uint256 amount,uint256 reward,uint256 validmoney) public payable onlyOwner  returns(address)
    {
    ...
    function setUData(address uaddr, uint256 validinvest,uint256 amount,uint256 reward,uint256 validmoney) public payable onlyOwner  returns(address)
    {
