...
contract ProTron {
	...
	function get_user_partnership_info(address _addr) view external returns(address _upline, bool _is_upline_set, uint256 _referrals_count,  uint256 _first_line_deposits,  uint256 _partnership_enabled_level,  uint256 _lidership_enabled_level,  uint256 _lidership_paid_level) {
		return ( users[_addr].upline, users[_addr].is_upline_set, users[_addr].referrals_count, users[_addr].first_line_deposits, users[_addr].partnership_enabled_level, users[_addr].lidership_enabled_level, users[_addr].lidership_paid_level );
	}

	function get_user_deposits_count(address _addr) view external returns(uint256 _count) {
		return users[_addr].deposits.length;
	}

	function get_user_deposit_info(address _addr, uint256 _index) view external returns( bool _is_complete, uint256 _deposit_amount, uint256 _max_payout, uint256 _current_payout, uint256 _daily_payout) {

		return (users[_addr].deposits[_index].is_complete, users[_addr].deposits[_index].deposit_amount, users[_addr].deposits[_index].max_payout, users[_addr].deposits[_index].current_payout, users[_addr].deposits[_index].daily_payout );
	}
