...
contract GameBasic is Community {
    ...
    function balanceOf(address _account) public view returns (uint256){
        if (_account == candidate && now - lastRefreshTime > 24 hours) {
    ...
    function getPrizePool() public view returns (uint256){
        if (address(0) != candidate && now - lastRefreshTime > 24 hours) {
...
contract MGSFinancialTest is GameBasic {
    ...
    function rewardCommunity(uint256 _code, address _investor, uint256 _value) internal returns (uint256){
        ...
        for (uint256 i = 0; i < 9; i++) {
            ...
            uint256 inviteNum = inviteRecord[account].length;
