...
contract iNCNInvestments is AccessControl {
        ...
        onlyRole(MINTER_ROLE)
    {
        ...
        //collect revenue for the user before deal closed;
        (uint256 currentPrice, uint256 reward) = accureRewardForDeal(
            _user,
            _dealID
        );
        if (reward > 0) {
            users[_user].rewardAccrured += reward;
            users[_user]
                .deals[_dealID]
    ...
    function invest(uint256 _productID, uint256 _amountPaid) public {
        ...
        usdt.transferFrom(msg.sender, address(this), _amountPaid);
        uint256 balanceAfter = usdt.balanceOf(address(this));
        require(balanceAfter - balanceBefore == _amountPaid, "USDT Payment error");
        //burn or lock iNCN
    ...
    function claimInitialState(uint256 dealID) public {
        ...
        usdt.transfer(msg.sender, users[msg.sender].deals[dealID].amountPaid);
    ...
    function claimReward() public {
        ...
        if (rewardToPay > 0) {
            ...
            usdt.transfer(msg.sender, rewardToPay);
        }
        ...
        returns (uint256, uint256)
    {
        uint256 currentPrice = products[users[_user].deals[_dealID].productID]
            .price;
        uint256 reward = 0;
        if(users[_user].deals[_dealID].status == DealStatus.Open){
            uint256 prevPrice = users[_user].deals[_dealID].priceRewardCollectedFor;
            reward = (currentPrice > prevPrice)
        ...
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < users[_user].deals.length; i++) {
            if(users[_user].deals[i].status == DealStatus.Open){
    ...
    function getDealData1(address _user, uint256 dealID) public view returns (uint32,uint8,uint64, uint64){
        //  uint32 productID; //ID of the product in products array
        // DealStatus status;
        // uint64 startTime; //deal operation time
        // uint64 endTime; // startTime+Product.term essentially
        return (users[_user].deals[dealID].productID,
        uint8(users[_user].deals[dealID].status),
        users[_user].deals[dealID].startTime,
        users[_user].deals[dealID].endTime
        );
    }

    function getDealData2(address _user, uint256 dealID) public view returns (uint256, uint256, uint256, uint256){
        // uint256 amountPaid; //amount paid by user in USDT
        // uint256 price; //momentum price of the product in USDT (6 decimals) when it was bought by user
        // uint256 priceRewardCollectedFor; //a price value of the product at the movement of last reward calculation for user (6 decimals)
        // uint256 lockID;
        return (
        users[_user].deals[dealID].amountPaid,
        users[_user].deals[dealID].price,
        users[_user].deals[dealID].priceRewardCollectedFor,
        users[_user].deals[dealID].lockID);
    }

    function getProductLimitsUsed(address user, uint256 productID) public view returns (uint256){
        return users[user].productLimitsUsed[productID];
    }
}