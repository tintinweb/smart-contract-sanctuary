...
uint256 private PricePerVyper = 10;
uint256 private VyperRegisters = 0;
uint256 private VyperDepositedTRX = 0;
uint256 private VyperDepositedVPR = 0;
uint256 private VyperWithdrawnTRX = 0;
uint256 private VyperWithdrawnVPR = 0;
uint256 private VyperTokenSale = 0;
uint256 private VyperOnFreezed = 0;
uint256 private VyperFrozens = 10;
uint256 private VyperMinBuys = 1000000;
...
mapping (address => uint256[4]) private UserOnLowest;
...
function readVyperFrozens() public view returns(uint256) {
return VyperFrozens;
}
function readVyperMinBuys() public view returns(uint256) {
return VyperMinBuys;
}
...
function writeVyperFrozens(uint256 NewLimit) public returns(uint256) {
if (msg.sender == VyperCreator) {
VyperFrozens = NewLimit;
return VyperFrozens;
} else {
return VyperFrozens;
}
}
function writeVyperMinBuys(uint256 NewLimit) public returns(uint256) {
if (msg.sender == VyperCreator) {
VyperMinBuys = NewLimit;
return VyperMinBuys;
} else {
return VyperMinBuys;
}
...
function readUserOnLowest(uint256 Param) public view returns(uint256) {
return UserOnLowest[msg.sender][Param];
}
...
if (TokenSale >= VyperMinBuys && Referrer != 0x0) {
VyperRegisters += 1;
...
uint256 UserBuys = UserTokenBuys[msg.sender];
if (Amount >= 1000000 && UserBuys >= VyperMinBuys) {
...
if (Amount >= 1000000 && Amount <= UserBalances[msg.sender][2]) {
...
uint256 MyPortion = uint256(1000000 * UserOnLowest[msg.sender][1] / VyperOnFreezed);
uint256 EstProfit = uint256(EstGlobal * MyPortion / 1000000);
return EstProfit;
} else {
return 0;
}
...
uint256 MyPortion = uint256(1000000 * UserOnLowest[msg.sender][1] / VyperOnFreezed);
uint256 EstProfit = uint256(EstGlobal * MyPortion / 1000000);
uint256 StartDate = readPrevMidnight(UserDatelock[msg.sender][2]);
...
uint256 MyPortion = uint256(1000000 * UserOnLowest[msg.sender][1] / VyperOnFreezed);
uint256 EstProfit = uint256(EstGlobal * MyPortion / 1000000);
uint256 StartDate = readPrevMidnight(UserDatelock[msg.sender][3]);
uint256 UntilDate = readPrevMidnight(block.timestamp);
uint256 EstClaims = 0;
if (UntilDate > StartDate) {
uint256 StakeDays = uint256((UntilDate - StartDate) / 86400);
...
uint256 UserBuys = UserTokenBuys[msg.sender];
uint256 UserMaxs = UserBuys * VyperFrozens;
if (Amount >= 1000000 && UserBuys >= VyperMinBuys && UserMaxs >= (UserOnFreezed[msg.sender]+Amount)) {
require(VyperToken.allowance(msg.sender, address(this)) >= Amount, "Allowance too low");
safeTransferFrom(VyperToken, msg.sender, address(this), Amount);
VyperOnFreezed += Amount;
UserOnFreezed[msg.sender] += Amount;
if (UserOnLowest[msg.sender][2] == 0) {
UserOnLowest[msg.sender][1] = UserOnFreezed[msg.sender];
UserOnLowest[msg.sender][2] = block.timestamp;
}
if (readPrevMidnight(block.timestamp) > readPrevMidnight(UserOnLowest[msg.sender][2])) {
UserOnLowest[msg.sender][1] = UserOnFreezed[msg.sender];
UserOnLowest[msg.sender][2] = block.timestamp;
}
...
if (Amount >= 1000000 && Amount <= UserOnFreezed[msg.sender]) {
VyperOnFreezed -= Amount;
UserOnFreezed[msg.sender] -= Amount;
if (readPrevMidnight(block.timestamp) > readPrevMidnight(UserOnLowest[msg.sender][2])) {
UserOnLowest[msg.sender][1] = UserOnFreezed[msg.sender];
...
if (UserOnFreezed[msg.sender] < UserOnLowest[msg.sender][1]) {
UserOnLowest[msg.sender][1] = UserOnFreezed[msg.sender];
}
VyperToken.transfer(msg.sender, Amount);
return Amount;
