...
contract VoteContract is Ownable {
    ...
    IERC20 token = IERC20(0x41a495ae6d77fcaa6a8de909ccf695e6468435f78c);
    ...
    function toVote(uint256 _index, bool yesOrNo) external {
        ...
        require(getVoteCount() >= 1, "already vote");
        if (yesOrNo) {
            vote.yes = vote.yes + 1;
        } else {
            vote.no = vote.no + 1;
    ...
    function getVoteCount() public view returns (uint256) {
        uint256 balan = token.balanceOf(msg.sender) / (10 ** 8);
        uint256 count = userVote[msg.sender];
        if(balan <= count) {
            return 0;
        }
        return balan - count;
