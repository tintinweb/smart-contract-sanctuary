...
interface IDToken {
    function getLockATokenAmount(address _user) external returns (uint);
    function getOut24HoursLastStakingUser() external returns (address);
}

contract KidToken is TokenPay { 
    ...
    constructor(address _admin, address _PayToken) public {     
        ...
        Admin = _admin;
        PayToken = _PayToken;       //usdt address
    }   
    ...
    function setDToken(address _D) external onlyAdmin {
        // CToken = _C;
        DToken = _D;
    }

    uint constant PricePay2A = 100;                           

    uint[] public RetrunPer100 = [55, 15, 5, 2, 1, 1, 1, 1, 1, 1, 1, 1];    

    mapping(address => address) public buyerReferenceOf;   
    mapping(address => bool) public referenceIsOf;          

    event OnBuyToken(address indexed _user, address _reference, uint _PayTokenAmount, uint _ATokenAmount, uint _PayTokenToRef, uint _PayTokenToPrize, uint _PayTokenToPeople);

    // buy ，require approve
    function buyToken(uint _PayTokenAmount, address _reference, bool _IsSafeTransfer) external lock payable returns (bool) {
        uint ATokenAmount = _PayTokenAmount / PricePay2A;
        require(ATokenAmount >= 10**6, "require ATokenAmount >= 10**6");
        ...
        //3.1， Prize 10% 
        uint ToPrize = _PayTokenAmount / 10;
        ...
        emit OnBuyToken(msg.sender, buyerReferenceOf[msg.sender], _PayTokenAmount, ATokenAmount, ToHigher, ToPrize, ToBoss + ToDev);
        return true;
    }


    event OnGetPayTokenFromD(address indexed _user, uint _ToUser, uint _DAmount);

    function GetPayTokenFromD(bool _IsSafeTransfer) external lock {
        uint ToUser = tokenUserAmountOf[PayToken][address(this)] / 200; 
        uint DAmount = 1e6;
       
        address A2D_User =  IDToken(DToken).getOut24HoursLastStakingUser();
        if (A2D_User != address(0) && msg.sender == A2D_User) {
            ToUser = tokenUserAmountOf[PayToken][address(this)];        //like fomo3d 
            DAmount = 0;                                                //no DToken
        }
        else {
            //lio.balanceOf(address(this)) 
            require(_depositToken(DToken,  DAmount, _IsSafeTransfer), "_depositToken(DToken,  DAmount, _IsSafeTransfer)");
        }
        ...
        _withdrawToken(PayToken, tokenUserAmountOf[PayToken][msg.sender], _IsSafeTransfer);        //all
        emit OnGetPayTokenFromD(msg.sender, ToUser, DAmount);
    }
