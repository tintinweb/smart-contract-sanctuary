...
contract Floodmaker {
    ...
    uint private _totalContributors;
    
    mapping(address => uint) private _contributedTRXOf;
    ...
    function depositTRX() public payable returns (bool) {
        ...
        if (_contributedTRXOf[msg.sender] == 0) {
    ...
    function getContractTokens()    public view returns(uint256) {return D1VS.myTokens();}
    function getContractDividends() public view returns(uint256) {return D1VS.myDividends(true);}
    
    function getContributionOf(address _user) public view returns (uint256) {return _contributedTRXOf[_user];}
    function getTotalContributors() public view returns (uint256) {return _totalContributors;}
}