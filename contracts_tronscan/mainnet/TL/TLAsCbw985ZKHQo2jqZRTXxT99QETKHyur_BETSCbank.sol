...
contract BETSCbank {
    ...
    uint private constant release_time = 1608501600; // 2020/12/20 22:00 UTC
    ...
    function invest(address referrer) public payable {
        require(block.timestamp >= release_time, "Release date is 2020/12/20 22:00 UTC.");
        require(msg.tokenid == token_id, "Send only BETSC.");
        
        uint msgValue = msg.tokenvalue;
        
        require(msgValue >= minimum_deposit, "The minimum deposit is 1000 BETSC");
    ...
    function withdraw() public {
        ...
        for(uint i = 0; i < investor.deposit_list.length; i++) {
            if(investor.deposit_list[i].withdrawn < uint(investor.deposit_list[i].amount).div(4).mul(3)) {
                ...
                if(uint(investor.deposit_list[i].withdrawn).add(dividends) > uint(investor.deposit_list[i].amount).div(4).mul(3)) {
                    dividends = (uint(investor.deposit_list[i].amount).div(4).mul(3)).sub(investor.deposit_list[i].withdrawn);
                }
