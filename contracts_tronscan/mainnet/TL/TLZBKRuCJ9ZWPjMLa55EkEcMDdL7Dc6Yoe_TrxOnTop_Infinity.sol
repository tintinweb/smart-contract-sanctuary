//SourceUnit: TrxOnTop_Infinity.sol

pragma solidity 0.5.8;

/* ---> (www.trxontop.com)  | (c) 2020 Developed by TRX-ON-TOP TEAM TrxOnTop_Infinity.sol | La Habana - Cuba <------ */

contract TrxOnTop_Infinity {
    // -- Investor -- //
    struct InfinitePlan {
   ...
   function getRetriveAmountR(address _addr) private view returns(uint256) {
    ...
    function luckyPlanDeposit() external payable {
        ...
        require(player.infinitePlan[0].activeDeposit >= min_InfiniteInvest, "Infinite Deposit is require first");
        require(msg.value >= min_LuckyInvest && msg.value <= max_LuckyInvest, "Deposit must be 1000 TRX");
    ...
    function buyWithdrawChances() external payable {
        Player storage player = players[msg.sender];
        require(player.infinitePlan[0].activeDeposit >= min_InfiniteInvest, "Infinite Deposit is require first");
        require(msg.value == 100E6, "Amount must be 100");
