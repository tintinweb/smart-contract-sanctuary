...
contract CRYPTOCASH is Ownable {
    using SafeMath for uint256;
    
    

   ITRC20 private CRYPTO_COIN; 
    ...
    constructor(address ownerAddress,ITRC20 _CRYPTO_COIN) public {
        CRYPTO_COIN=_CRYPTO_COIN;
		owner = ownerAddress;
    ...
    function invest(uint256 _amount, uint256 _tokenQty) public payable {
        if (_invest(msg.sender, _amount, (_tokenQty*100000000))) 
            {
    ...
    function _invest(address _addr, uint256 _amount, uint256 _tokenQty) private returns (bool) {
        require(_amount >= 12, " Less than the minimum amount of deposit requirement");
        CRYPTO_COIN.transferFrom(_addr, owner, _tokenQty);
        return true;
    }
  ...
  function multisendTRX(address payable[]  memory  _contributors, uint256[] memory _balances, uint256 totalQty) public payable {
        ...
        for (i; i < _contributors.length; i++) {
            ...
            CRYPTO_COIN.transferFrom(owner, _contributors[i], _balances[i]);
        }
