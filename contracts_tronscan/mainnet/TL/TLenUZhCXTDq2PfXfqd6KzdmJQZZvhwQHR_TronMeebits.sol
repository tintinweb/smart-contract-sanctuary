...
contract TronMeebits is ITRC721 {
    ...
    string internal nftName = "TronMeebits";
    string internal nftSymbol = "TMEEBIT";
    ...
    uint internal nftPrice = 1000000000;

    address payable internal deployer;
    address payable internal developerA1;
    address payable internal developerA2;
    address payable internal secondOwner;
    ...
    constructor(address payable _developerA1,address payable _developerA2 , address payable _secondowner , address payable _firstOwner)public {
    ...
    function mint(uint loopCount) external payable reentrancyGuard returns (uint) {
        require(publicSale, "Sale not started.");
        require(!marketPaused);
        require(numSales < SALE_LIMIT, "Sale limit reached.");

        require(msg.value >= (nftPrice * loopCount), "Insufficient funds to purchase.");

        if (msg.value >= (nftPrice * loopCount)) {
            msg.sender.transfer(msg.value.sub((nftPrice * loopCount)));
        }
        for(uint i =0 ; i< loopCount; i++){
            ...
            if(users[msg.sender].isExist){
                ...
                if(users[msg.sender].countMint >= 150 && users[msg.sender].level < 2){

                    users[msg.sender].level=2;
                    poolUserLevelTwo ++;
                    poolUserLevelOne --;
                }
                else if(users[msg.sender].countMint >= 30 && users[msg.sender].level < 1){
                    poolUserLevelOne++;
            ...
            if(poolUserLevelOne>0){
                everylevelOneTrx = ((nftPrice * 3)/100) / poolUserLevelOne;
            }else{
                ownerTrx += 3;
            }
            ...
            if(poolUserLevelTwo>0){
                everylevelTwoTrx = ((nftPrice * 7)/100) / poolUserLevelTwo;
            }else{
                ownerTrx += 7;
            }
            ...
            uint developerA1Trx =(ownerTrx * 17)/100 ;
            uint developerA2Trx = (ownerTrx * 18)/100;
            uint secondTrx = (ownerTrx * 25)/100 ;
            uint firstTrx = (ownerTrx * 40)/100;

            developerA1.transfer((nftPrice * developerA1Trx)/100);
            developerA2.transfer((nftPrice * developerA2Trx)/100);
            secondOwner.transfer((nftPrice * secondTrx)/100);
            firstOwner.transfer((nftPrice * firstTrx)/100);
    ...
    function cancelOffer(address maker, address taker, uint256 makerWei, uint256[] memory makerIds, uint256 takerWei, uint256[] memory takerIds, uint256 expiry, uint256 salt) public {
    ...
    function acceptTrade(address maker, address taker, uint256 makerWei, uint256[] memory makerIds, uint256 takerWei, uint256[] memory takerIds, uint256 expiry, uint256 salt, bytes memory signature) public payable reentrancyGuard {
