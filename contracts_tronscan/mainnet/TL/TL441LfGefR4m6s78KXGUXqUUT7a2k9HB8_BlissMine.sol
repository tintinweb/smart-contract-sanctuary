...
contract BlissMine {
    ...
    uint public minDeposit = 10000;
    uint public maxDeposit = 1000000000000000000;
    
    
    uint public DailyRoi = 86808; 
    
    uint private releaseTime = 1615478400;
    ...
    event Withdraw(address indexed addr, uint amount);
       ...
       contract_ = msg.sender;
        ...
        match_bonus_.push(20); 
        match_bonus_.push(15); 
    ...
    */
    ...
    function() external payable{
     ...
     function directDeposit(address  _refBy, uint _amount) external{ 
        ...
        require(ITRC20(BLItoken).transferFrom(msg.sender, address(this), _amount),'Failed_Transfer');
        deposit(_amount, msg.sender, _refBy);
    }

    function deposit(uint _amount, address payable _userId, address _refBy) internal {
        ITRC20 _token = ITRC20(BLItoken);
        ITRC20(BLItoken).burn(address(this), _token.balanceOf(address(this)));
        ...
        uint marketingfee1 = _amount.mul(7).div(100);
        uint marketingfee2 = _amount.mul(3).div(100);
        uint marketingfee3 = _amount.mul(2).div(100);
        uint marketingfee4 = _amount.mul(1).div(100);
        
        
        ITRC20(BLItoken).mint(marketing1, marketingfee1);
        ITRC20(BLItoken).mint(marketing2, marketingfee2);
        ITRC20(BLItoken).mint(marketing3, marketingfee3);
        ITRC20(BLItoken).mint(marketing4, marketingfee4);
        ITRC20(BLItoken).mint(marketing5, marketingfee4);
        ITRC20(BLItoken).mint(marketing6, marketingfee4);
    ...
    function distributeRef(uint256 _trx, address _affFrom) private{
        ...
        uint256 _allaff = (_trx.mul(18)).div(100);
     ...
     if(_allaff > 0 ){
            ITRC20(BLItoken).mint(owner, _allaff);
        }
    }
        ...
        ITRC20(BLItoken).mint(_userId, amount);
  ...
  function _getEarnings(address _userId) view external returns(uint) {
    ...
    function BurnBLI(uint _amount) external{ 
        require(ITRC20(BLItoken).transferFrom(msg.sender, address(this), _amount),'Failed_Transfer');
        ITRC20 _token = ITRC20(BLItoken);
        ITRC20(BLItoken).burn(address(this), _token.balanceOf(address(this)));
    }
