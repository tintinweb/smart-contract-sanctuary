//SourceUnit: Billion-Money_Tron_old (2).sol
...
contract billionMoney is owned {
    ...
    function correctWrongDataEntry( uint _fromID, uint _toID ) public onlyOwner returns(bool)
    {
        require(!dataUpdated, "can not change now");
        require(_toID - _fromID <=  100, "can not update more then 100");
        for (uint i = _fromID; i<= _toID; i++)
        {
            address _user = userAddressByID[i];
            userInfos[_user].id = 0;
            userInfos[_user].joined = false;
            userInfos[_user].referrerID = 0;
            userInfos[_user].originalReferrer = 0;
            userInfos[_user].directCount = 0;
            delete userInfos[_user].referral;
            for(uint i=0;i<10;i++)
            {
                userInfos[_user].levelExpired[i] = 0;
            } 
            userAddressByID[i] = address(0);            
            autoPoolLevel[0][i-1].userID = 0;
            autoPoolLevel[0][i-1].autoPoolIndex_ = 0;
            autoPoolIndex[_user][0] = 0;
        }
        autoPoolLevel[0].length = _fromID - 1 ;
        return true;
    }


    function adjustIndexNLastIDForCorrection(uint _lastIDCount, uint _nextMemberFillIndex, uint _nextMemberFillBox)  public onlyOwner returns(bool)
    {
        lastIDCount = _lastIDCount;
        nextMemberFillIndex[0] = _nextMemberFillIndex;
        nextMemberFillBox[0] = _nextMemberFillBox;
        return true;
    }

    function changeAutoPoolRecord(address _user, uint _level,uint _idx, uint _id,uint _autoPoolIndex_) public onlyOwner returns(bool)
    {
        require(!dataUpdated, "can not change now");
        autoPoolLevel[_level - 1 ][_idx].userID = _id;
        autoPoolLevel[_level - 1 ][_idx].autoPoolIndex_ = _autoPoolIndex_;
        autoPoolIndex[_user][_level - 1] = _idx; 
        return true;
    }

    function setAutoPool( uint _level, address[] memory _user,uint[] memory _idx, uint[] memory _id,uint[] memory _autoPoolIndex_) public onlyOwner returns(bool)
    {
        require(_level <= 10 && _level >0, "invalid level");
        require(!dataUpdated , "run time is over");
        for(uint i=0;i<_user.length;i++)
        {
            autoPoolLevel[_level - 1 ][_idx[i]].userID = _id[i];
            autoPoolLevel[_level - 1 ][_idx[i]].autoPoolIndex_ = _autoPoolIndex_[i];
            autoPoolIndex[_user[i]][_level - 1] = _idx[i];            
        }
        return true;
    }
    ...
    function lastIDView(uint value) external view returns (uint lastID){
        lastID = lastIDCount;
    }
