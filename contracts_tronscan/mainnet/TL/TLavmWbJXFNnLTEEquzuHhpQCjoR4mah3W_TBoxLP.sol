...
contract TBoxLP {
    ...
    function _calPayout(uint256 idx) private view returns(uint256 payout) {

        uint256 idxDepositAmount = users[msg.sender].depositAmounts[idx];
        uint256 idxDepositTime = users[msg.sender].depositTimes[idx];
