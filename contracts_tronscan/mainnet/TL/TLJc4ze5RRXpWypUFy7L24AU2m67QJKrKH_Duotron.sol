...
contract Duotron is Ownable {
    ...
    uint256 public constant PLAN_INTEREST = 40;           
    uint256 public constant PLAN_TERM =75 days;            
    uint256 public constant marketing_FEE = 70;
    uint256 public constant DEVELOPER_FEE = 30;
   ...
   address payable public marktAddress;
    address payable public developerAddress;

    constructor(address payable marktAddr,address payable devAddr) public {
        require(!isContract(marktAddr) && !isContract(devAddr));
        marktAddress=marktAddr;
    ...
    function _invest(address _addr, uint256 _referrerCode, uint256 _amount) private returns (bool) {
        ...
        uint256 marktFee = _amount.mul(marketing_FEE).div(1000);
        uint256 devloperFee = _amount.mul(DEVELOPER_FEE).div(1000);
        
         marktAddress.transfer(marktFee);
    ...
    function withdraw() public {
        ...
        if(withdrawalAmount>0){
            ...
            uint256 reinvestAmount = withdrawalAmount.mul(1).div(100);
