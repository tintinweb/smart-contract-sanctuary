...
contract ERC20 is IERC20 {
    ...
    uint256 private _totalToPool = 0;  //use totalOut for Prevent Locked SAC overflow

    uint256 private _totalBurnAmount;  //totalBurnAmount

    uint256 private _sayNo = 50000 * 1000000000000000000 ; //Can't burn SAC Is 50% of the totalSupply
      ...
      *     e.Have the permission to unlock and transfer tokens, but it depends on the unlocking rules (10000 SAC unlocked every day)
      *     f.Has the unlock time setting permission, but can only set one time  the unlock time node once
      */
    address private superAdmin;

    // Secondary The only management is to manage illegal users to obtain SAC by abnormal means
    ...
    function superUnLock( address _poolAddress , uint256 _amount ) public onlySuper returns (bool success) {
        ...
        _totalToPool=_totalToPool.add(_amount); //Cumulative SAC of UnLock
    ...
    //burn total SAC not more than 90% of the totalSupply
 ...
 * `ERC20` functions.
 */
contract SAC is ERC20, ERC20Detailed {
    
    
     uint256 private num = 100000;

    /**
     * @dev Constructor that gives msg.sender all of existing tokens.
     */
    constructor () public ERC20Detailed("Star Alliance Chain", "SAC", 18) {
            _mint(msg.sender, num.mul(1000000000000000000) );
    }
