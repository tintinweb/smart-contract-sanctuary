...
contract TronGoldenInvestV1 {
  ...
  function getUserDividends(address userAddress) public view returns (uint256) {
    ...
    for (uint256 i = 0; i < user.deposits.length; i++) {
      uint256 maxIncome = (user.deposits[i].amount.mul(2));
      if (user.deposits[i].withdrawn < maxIncome) {
        if (user.deposits[i].start > user.checkpoint) {
          dividends = (user.deposits[i].amount.mul(userPercentRate).div(DIVIDER))
        ...
        if (user.deposits[i].withdrawn.add(dividends) >maxIncome) {
          dividends = maxIncome.sub(user.deposits[i].withdrawn);
        }
  ...
  function getUserDepositInfo(address userAddress, uint256 index) public view returns(uint256, uint256, uint256,uint256) {
      User storage user = users[userAddress];
    return (user.deposits[index].amount, user.deposits[index].withdrawn, user.deposits[index].start,getUserDividends(userAddress));
  }
