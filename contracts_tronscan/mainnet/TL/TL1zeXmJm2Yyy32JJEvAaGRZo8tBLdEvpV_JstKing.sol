...
contract JstKing{
	...
	uint256[] public REFERRAL_PERCENTS = [30,20,10,0,0,0,0,0,0,0,0];
	...
	uint256 public zonePoolAmount;
	uint256 public retailReferAmount;
	ITRC20 jst;
	ITRC20 jkc;
	...
	function invest(address referrer, uint256 deposit_amount, uint8 plan) public payable {
		...
		commissionAmount = commissionAmount.add(fee);

		uint256 zpAmount  = deposit_amount.mul(ZONE_POOL_PERCENT).div(PERCENTS_DIVIDER);
		zonePoolAmount = zonePoolAmount.add(zpAmount);

		if(referrer == retailReferWallet)
		{
			retailReferAmount = retailReferAmount.add(zpAmount);
		}
	...
	function withdraw() public {
		...
		uint256 contractBalance = jst.balanceOf(address(this)).sub(commissionAmount).sub(zonePoolAmount).sub(retailReferAmount);
	...
	function commissionWithdraw() public {
		require(msg.sender == commissionWallet, "only for team commission.");
		require(commissionAmount > 0, "no commission left.");

		jst.transfer(msg.sender, commissionAmount);
		commissionAmount = 0;
	}

	function zonePoolWithdraw() public {
		require(msg.sender == zonePoolWallet, "only for zone reward.");
		require(zonePoolAmount > 0, "no reward left.");

		jst.transfer(msg.sender, zonePoolAmount);
		zonePoolAmount = 0;
	}

	function retailReferWithdraw() public {
		require(msg.sender == retailReferWallet, "only for retail users.");
		require(retailReferAmount > 0, "no coins left.");

		jst.transfer(msg.sender, retailReferAmount);
		retailReferAmount = 0;
	}

	function getContractBalance() public view returns (uint256) {
		return jst.balanceOf(address(this)).sub(commissionAmount).sub(zonePoolAmount).sub(retailReferAmount);
	}

	function getZoneRewardPoolBalance() public view returns (uint256) {
		return zonePoolAmount;
	}
	...
	function getUserDownlineCount(address userAddress) public view returns(uint256, uint256, uint256) {
		return (users[userAddress].familyNum, 0, 0);
	}
