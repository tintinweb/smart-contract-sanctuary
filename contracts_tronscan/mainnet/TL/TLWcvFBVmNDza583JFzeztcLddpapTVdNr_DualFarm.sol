...
contract DualFarm is Ownable {
    ...
    uint256 public constant minDepositWYZ = 100E18;
    uint256 public constant minDepositTRX = 100E6;
    ...
    function farmWYZTH(uint256 _amount, uint256 _lockableDays) public {
        ...
        require(_amount >= minDepositWYZ, "Invalid amount");
    ...
    function farmTRX(uint256 _lockableDays) public payable {
        userInfo storage user = users[msg.sender];
        require(user.farmedToken == 0, "Muliple farm not allowed");
        require(msg.value >= minDepositTRX, "Invalid amount");
        require(allocation[_lockableDays] > 0, "Invalid day selection");
        ...
        returns (uint256 wyzthReward, uint256 uleReward)
    {
        ...
        if (users[msg.sender].lockableDays > toDays) {
            ...
            if (
                bytes(users[msg.sender].farmType).length ==
                bytes("WYZTH").length
            ) {
    ...
    function harvest() public {
        ...
        if (bytes(users[msg.sender].farmType).length == bytes("TRX").length) {
    ...
    // EMERGENCY ONLY.
    function emergencyWithdraw(uint256 wyzthAmount, uint256 uleAmount)
        public
        onlyOwner
    {
