//SourceUnit: tronOxigenUltimate.sol
...
contract TronOxigenUltimate {
    ...
    function _refPayout(address _addr, uint256 _amount) private {
        ...
        uint256 amount_to_distribute = _amount * ref_bonuses[0] / 100;
            ...
            if (dep.time_finish > uint40(block.timestamp)) {
                ...
                if (deposit_profit + dep.amount_withdrawn < dep.amount_end) {
                    ...
                    if ( diff_amount <= amount_to_distribute ) {
                        ...
                        deposits_ref[dep.id].push(DepositRef(_addr, diff_amount, uint40(block.timestamp)));
                    }
                    else {
                        ...
                        deposits_ref[dep.id].push(DepositRef(_addr, amount_to_distribute, uint40(block.timestamp)));
        ...
        if (amount_to_distribute > 0) {
            ...
            if (!with_deposits) {
                if (up == owner) {
                    amount_to_distribute = _amount * ref_bonuses[0] / 100;
                    owner.transfer(amount_to_distribute);
                }
                else {
                    amount_to_distribute = _amount * ref_bonuses[1] / 100;
                    pending_deposits_low_ref[up].push(DepositRef(_addr, amount_to_distribute, uint40(block.timestamp)));
                ...
                if (up == owner) {
                    ...
                    pending_deposits_ref[up].push(DepositRef(_addr, amount_to_distribute, uint40(block.timestamp)));
                    player.total_match_bonus += amount_to_distribute;
                }
            }
            ...
            if (pending_deposits[i].time + (expiration_days_pending_deposits * 86400) > uint40(block.timestamp)) {
                if ( dep.amount_ref + dep.amount_low_ref < dep.amount_end ) {
                    ...
                    if (diff > referral_amount) {
                        ...
                        deposits_ref[dep.id].push( DepositRef(pending_deposits[i].referred, referral_amount, pending_deposits[i].time) );
                        pending_deposits[i].amount = 0;
                        player.match_bonus_hight += referral_amount;
                    }
                    else {
                        dep.amount_ref += diff;
                        deposits_ref[dep.id].push( DepositRef(pending_deposits[i].referred, diff, pending_deposits[i].time) );
            ...
            if (pending_deposits_low[i].time + (expiration_days_pending_deposits * 86400) > uint40(block.timestamp)) {
                if ( dep.amount_ref + dep.amount_low_ref < dep.amount_end ) {
                    ...
                    if (diff > referral_amount) {
                        ...
                        deposits_ref_low[dep.id].push( DepositRef(pending_deposits_low[i].referred, referral_amount, pending_deposits_low[i].time) );
                        pending_deposits_low[i].amount = 0;
                        player.match_bonus_low += referral_amount;
                    }
                    else {
                        dep.amount_low_ref += diff;
                        deposits_ref_low[dep.id].push( DepositRef(pending_deposits_low[i].referred, diff, pending_deposits_low[i].time) );
    ...
    */
    ...
    function getReferralsInvestmentPlayer(uint256 id) view external returns (address[] memory, uint256[] memory, uint40[] memory) {
    ...
    */
    ...
    function getPendingDepositsLowRef(address _addr) external view returns (address[] memory referred, uint256[] memory amount, uint40[] memory time) {
    ...
    function getPendingDepositsRef(address _addr) external view returns (address[] memory referred, uint256[] memory amount, uint40[] memory time) {
    ...
    function getDepositsRef(uint256 id) external view returns (address[] memory referred, uint256[] memory amount, uint40[] memory time) {
    ...
    function getDepositsRefLow(uint256 id) external view returns (address[] memory referred, uint256[] memory amount, uint40[] memory time) {
