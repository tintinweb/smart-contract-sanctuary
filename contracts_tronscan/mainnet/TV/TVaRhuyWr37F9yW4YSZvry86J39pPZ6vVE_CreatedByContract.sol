...
contract JustswapExchange is IJustswapExchange, TRC20, ReentrancyGuard {
    /***********************************|
    |        Variables && Events        |
    |__________________________________*/
    ...
    function trxToTokenInput(
        ...
        uint256 tokens_bought = getInputPrice(
            trx_sold,
            address(this).balance.sub(trx_sold),
            token_reserve
        );
    ...
    function trxToTokenOutput(
        ...
        require(
            deadline >= block.timestamp && tokens_bought > 0 && max_trx > 0
        );
        uint256 token_reserve = token.balanceOf(address(this));
        uint256 trx_sold = getOutputPrice(
            tokens_bought,
            address(this).balance.sub(max_trx),
            token_reserve
        );
    ...
    function tokenToTrxInput(
        ...
        uint256 token_reserve = token.balanceOf(address(this));
        uint256 trx_bought = getInputPrice(
            tokens_sold,
            token_reserve,
            address(this).balance
        );
    ...
    function tokenToTokenInput(
        ...
        require(
            exchange_addr != address(this) && exchange_addr != address(0),
            "illegal exchange addr"
        );
        uint256 token_reserve = token.balanceOf(address(this));
        uint256 trx_bought = getInputPrice(
            tokens_sold,
        ...
        uint256 tokens_bought = IJustswapExchange(exchange_addr)
    ...
    function tokenToTokenOutput(
        ...
        require(
            deadline >= block.timestamp &&
                (tokens_bought > 0 && max_trx_sold > 0),
            "illegal input parameters"
        );
        require(
            exchange_addr != address(this) && exchange_addr != address(0),
        ...
        uint256 trx_bought = IJustswapExchange(exchange_addr)
            .getTrxToTokenOutputPrice(tokens_bought);
        uint256 token_reserve = token.balanceOf(address(this));
        uint256 tokens_sold = getOutputPrice(
            trx_bought,
            token_reserve,
        ...
        // tokens sold is always > 0
        require(
            max_tokens_sold >= tokens_sold && max_trx_sold >= trx_bought,
            "max token sold not matched"
        );
        ...
        uint256 trx_sold = IJustswapExchange(exchange_addr)
            .trxToTokenTransferOutput
            .value(trx_bought)(tokens_bought, deadline, recipient);
        emit TrxPurchase(buyer, tokens_sold, trx_bought);
        emit Snapshot(
            buyer,
            address(this).balance,
    ...
    function addLiquidity(
        ...
        require(
            ...
            "JustExchange#addLiquidity: INVALID_ARGUMENT"
        );
        ...
        if (total_liquidity > 0) {
            ...
            uint256 token_amount = (
                msg.value.mul(token_reserve).div(trx_reserve)
            )
                .add(1);
            uint256 liquidity_minted = msg.value.mul(total_liquidity).div(
                trx_reserve
            );

            require(
                max_tokens >= token_amount && liquidity_minted >= min_liquidity,
                "max tokens not meet or liquidity_minted not meet min_liquidity"
            );
            ...
            require(
                address(factory) != address(0) &&
                    address(token) != address(0) &&
                    msg.value >= 10_000_000,
                "INVALID_VALUE"
            );
            require(
                factory.getExchange(address(token)) == address(this),
                "token address not meet exchange"
            );
    ...
    function removeLiquidity(
        ...
        uint256 token_reserve = token.balanceOf(address(this));
        uint256 trx_amount = amount.mul(address(this).balance) /
            total_liquidity;
        uint256 token_amount = amount.mul(token_reserve) / total_liquidity;
        require(
            trx_amount >= min_trx && token_amount >= min_tokens,
            "min_token or min_trx not meet"
        );
        ...
        require(
            address(token).safeTransfer(msg.sender, token_amount),
            "transfer failed"
        );
