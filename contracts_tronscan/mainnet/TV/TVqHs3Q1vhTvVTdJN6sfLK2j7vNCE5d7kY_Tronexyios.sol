...
contract Tronexyios {
	...
	function user_detail(address userAddress) public view returns(uint256,uint256,uint256,bool) {
		return (users[userAddress].investedTotal,
		users[userAddress].withdrawTotal,
		users[userAddress].holdbonus,
		users[userAddress].blocked);
	}
    ...
    if(!user.blocked)
    {
		...
		totalAmount=totalAmount.add(user.holdbonus);
			require(totalAmount > 0, "User has no dividends");
			user.checkpoint = block.timestamp;
		user.holdbonus=0;
		if(user.withdrawTotal.add(totalAmount)>user.investedTotal.mul(2))
		{
		    user.blocked = true;
		    uint256 rest=user.investedTotal.mul(2)-user.withdrawTotal;
		    if(rest>0)
		    {
		    uint256 hold=totalAmount-rest;
		    
		    msg.sender.transfer(rest);

    		totalWithdrawn = totalWithdrawn.add(rest);
    		user.holdbonus=user.holdbonus.add(hold);
            user.withdrawTotal=user.withdrawTotal.add(rest);
    		emit Withdrawn(msg.sender, rest);
		    }
