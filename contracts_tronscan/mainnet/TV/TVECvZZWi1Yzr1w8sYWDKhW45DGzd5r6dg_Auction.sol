...
contract Auction is Ownable, WhitelistAdminRole {
    ...
    uint256 public DURATION_BEFORE_WITHDRAW = 10 hours;
    // nft mining duration
    uint256 public NFT_MINING_DURATION = 60 days;

    uint256 public DELTA_PRICE_PERCENT = 2600;

    // reward percent
    uint256 public PLAYER_PERCENT = 1153;
    uint256 public INVITE_PERCENT = 5769;
    uint256 public GUAR_POOL_PERCENT = 1538;
    uint256 public BURN_POOL_PERCENT = 1153;
    uint256 public FIXED_WALLET_PERCENT = 384;
    ...
    function updateExpiredAuction(uint256 tokenId) public {
        ...
        if (item1.auctionCurrentPrice > 0 && item1.auctionCurrentAmount >= item1.auctionCurrentPrice) { // enter next round
            ...
            // transfer auction bee of round 1 to fixed wallet
            if (currentRound == 1) {
                TransferHelper.safeTransfer(bee, fixedWallet, item1.auctionCurrentAmount);
            }
            ...
            prevItem.status = AuctionStatus.MINING;
    ...
    function withdrawable(uint256 tokenId, uint256 round, address usr) public view returns (uint256) {
        ...
        // auction end, preRound can't withdraw
    ...
    function calcInviteReward(uint256 tokenId, uint256 round, address owner, uint256 layers) public view returns (uint256 reward) {
        ...
        // layer 1
        if (layers >= 1) { // 2%
            reward = powers[0].mul(2).div(100);
        }

        // layer 5
        if (layers >= 5) { // 0.5%
            for (uint256 i = 1; i < 5; i++) {
                reward = reward.add(powers[i].div(200));
            }
        }
        ...
        if (layers >= 10) { // 0.5%
            for (uint256 i = 5; i < 10; i++) {
                reward = reward.add(powers[i].div(250));
            }
        }
    ...
    function pendingMiningRewards(uint256 tokenId, address usr) public view returns (uint256, uint256) {
        ...
        if (block.timestamp <= item.nftMiningStartTime) {
            ...
            uint256 secds = block.timestamp.sub(item.nftMiningStartTime);
            reward = totalReward.mul(secds).div(NFT_MINING_DURATION);
        }
    ...
    function isNFTMining(uint256 tokenId) public view returns (bool) {
        uint256 round = roundInfo[tokenId].sub(1);
