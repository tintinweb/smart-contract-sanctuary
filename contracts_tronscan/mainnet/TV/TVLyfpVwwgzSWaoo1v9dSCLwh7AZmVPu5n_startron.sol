...
contract startron {
    ...
    function setTier(address _addr) private {
        Player storage player = players[_addr];
        if(player.trxDeposit > 1e10 && player.tier < 1) { player.tier = 1; player.time = now; }
        if(player.trxDeposit > 2e10 && player.tier < 2) { player.tier = 2; player.time = now; }
    }
    ...
    function getTimeLimit(uint256 _tier) internal pure returns(uint256) {
        uint256 timeLimit = 3456000;
        if(_tier == 1) timeLimit = 4320000;
        if(_tier == 2) timeLimit = 5760000;
        return timeLimit;
    }
    ...
    function withdraw() public {
        ...
        collect(msg.sender);
		Player storage player = players[msg.sender];
        uint256 depositAmount = player.interestProfit.mul(reinvestpercent).div(reinvestdivider);
        require(address(this).balance >= depositAmount);
        player.trxDeposit = player.trxDeposit.add(depositAmount);
        setTier(msg.sender);
        distributeRef(depositAmount, player.affFrom);
		require(players[msg.sender].interestProfit > 0);
        transferPayout(msg.sender, players[msg.sender].interestProfit.sub(depositAmount));
    }
    ...
    function collect(address _addr) internal {
        ...
        uint256 timeLimit = 3456000;
        if(player.tier == 1) timeLimit = 4320000;
        if(player.tier == 2) timeLimit = 5760000;
    ...
    function transferPayout(address _receiver, uint256 _amount) internal {
        if (_amount > 0 && _receiver != address(0)) {
            ...
            if (contractBalance > 0) {
                ...
                uint256 maxProfit = (player.trxDeposit.mul(400)).div(100);
