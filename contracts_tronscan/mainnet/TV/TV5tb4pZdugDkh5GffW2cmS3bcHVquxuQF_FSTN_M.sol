...
contract FSTN_M {
    ...
    uint256 constant public BASE1 = 1;
    uint256 constant public BASE2 = 5;
    uint256 constant public BASE3 = 10;
...
uint256 BaseROI;
uint256 totalDepAmount= getUserTotalDeposits(msg.sender);
...
contractBalanceRate = contractBalanceRate.sub(BASE_PERCENT).add(UserBase_ROI);
...
getContractBalanceRate());
    ...
    function getUserReferralsStats(address userAddress) public view returns (address, uint256, uint24[3] memory) {
        User storage user = users[userAddress];
