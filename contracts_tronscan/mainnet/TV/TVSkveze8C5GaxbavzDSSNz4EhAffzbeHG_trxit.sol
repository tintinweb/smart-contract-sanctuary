...
contract trxit {
    ...
    constructor(address payable _owner) public {
        ...
        dev_fee = 0xB1A9A88fFc26B46758c5b83c1CA09C309D1d6a44;
    ...
    function _deposit(address _addr, uint256 _amount) private {
        ...
        else require(_amount >= 5 * 1e8, "Bad amount"); 
    ...
    function _drawPool() private {
        uint256 diff = (block.timestamp - pool_last_draw) / 1 days;
        pool_last_draw = pool_last_draw + (uint40(diff) * 1 days);               
	...
	function calc_dividend_bonus(uint deposit_amount, uint40 deposit_time) public view returns(uint) {
	    uint noofdays =(block.timestamp - deposit_time) / 1 days;
        uint dividend_bonus=0;
        for(uint i = 1; i <= noofdays; i++) {
            dividend_bonus = dividend_bonus + (( (deposit_amount / 10000 * 150) * 40) / 100);
            deposit_amount = deposit_amount + (( (deposit_amount / 10000 * 150) * 60) / 100);
        }
		return dividend_bonus;
    }
    
    function _doDrawPool() private {
