...
contract LuckyOne is Owned
{
    ...
    // Contract settings
    address public topiaTokenContractAddress = address(0x0);
    address public blockHashStorageContractAddress = address(0x0);
    address public dividendContractAddress = address(0x0);
    address public voucherContractAddress = address(0x0);
    ...
    ) payable public returns (bytes32 _newUnfinishedBetHash)
    {
        ...
        if (topiaTokenContractAddress != address(0x0))
        {
            TRONtopiaInterface(topiaTokenContractAddress).mintToken(player, msg.value);
        }
