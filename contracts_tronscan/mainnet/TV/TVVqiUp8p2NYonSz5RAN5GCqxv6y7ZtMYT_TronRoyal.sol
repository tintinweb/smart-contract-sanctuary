...
contract TronRoyal {
  ...
  // flexible rates
  function setDivider(uint _rateDivider) public restricted {
    // can only increase dividends rate
    if (_rateDivider < rateDivider) {
      rateDivider = _rateDivider;
    }
  }

  // flexible rates
  function setRefReward(uint[] memory _refReward) public restricted {
  ...
  function deposit(uint64 refLink) public payable {
    ...
    // to test before release. Withdraws cannot be paused
