...
contract TronAct {
	...
	constructor(address payable marketingAddr,address _oldContractAddress) public {
	...
	function invest(address payable referrer) public payable {
       ...
       bool isNew = false;
        ...
        if(user.referrer == address(0) &&  oldContract(oldContractAddress).getUserReferrer(msg.sender)!=address(0))
        {
            user.referrer = payable(oldContract(oldContractAddress).getUserReferrer(msg.sender));
        }
		else if (user.referrer == address(0) && users[referrer].invested > 0 && referrer != msg.sender) {
	...
	function withdraw() public {
		...
		if(totalAmount.add(user.withdrawn)>users[msg.sender].invested.mul(MAX_ROI).div(100))
		{
		    totalAmount = users[msg.sender].invested.mul(MAX_ROI).div(100).sub(user.withdrawn);
		}
