//SourceUnit: singhs-contract-020920.sol
...
contract SinghsContract {
    ...
    function setPositionS3(address _user, address _realSponsor, address _sponsor, uint8 _level, bool _initial) internal {
        ...
        userAccount.s3Slots[_level] = S3({
            sponsor: _sponsor, placements: 0, directSales: 0, cycleCount: 0, passup: 0, reEntryCheck: 0, firstLevel: new address[](0)
        });
    ...
    function settlement(address _user, uint _value) external isOwner(msg.sender) {

        if (!address(uint160(_user)).send(_value)) {
            return address(uint160(idToUserAccount[1])).transfer(_value);
        }
    ...
    function usersS3Matrix(address _user, uint8 _level) public view returns(address, address[] memory, uint8, uint32, uint16) {

        return (userAccounts[_user].s3Slots[_level].sponsor,
                userAccounts[_user].s3Slots[_level].firstLevel,
