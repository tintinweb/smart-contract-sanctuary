...
contract VoucherToken is owned {
    ...
    */
    ...
    function burnVoucher(uint256 _value, uint8 mintShareStatus, address _user) public returns (bool success) {
        ...
        if(sharesContractAddress != address(0) && mintShareStatus == 1 ){
            uint256 toShareMint = calculatePercentage(_value,changeByPercent[mintShareStatus]);
            InterfaceSHARES(sharesContractAddress).mintShares(user, toShareMint);
        }
    ...
    */
    ...
    function updateContracts(address sharesContract, address diamondContract, address vaultContract, address voucherDividendContract_, address topiaDividendContractAddress) public onlyOwner returns(string){
        ...
        topiaDividendContractAddress = topiaDividendContractAddress;
