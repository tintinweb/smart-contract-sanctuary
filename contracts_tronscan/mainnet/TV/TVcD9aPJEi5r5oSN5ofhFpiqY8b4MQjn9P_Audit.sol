...
contract Audit is TRC20, TRC20Detailed {
  ...
  uint256 maxSupply = 1000e6;
  uint private tokenPrice = 500e6;
  address public tokenContractAddress;
  address public governance;
  address public defiAuditsToken;
  uint public launchTime = 1610668800;
  mapping (address => bool) public minters;
    address payable public ownerC;
    uint256 public auditCost = 10e6;
    ...
    constructor (address _tokenContractAddress) public TRC20Detailed("Audit", "AD", 6) {
    ...
    function acceptAuditRequest(bool isAccepted, uint256 contractId) public{
     ...
     function verifyAudit(bool isVerified, uint256 contractId) public{
        require(whitelisted[msg.sender],"Not Authorized to verify");
         AuditRequest storage audit = currentAuditRequest[contractId];
         require(!audit.isAccepted,"Contract is not Accepted By Admin");
...
function removeDataFromWhiteList(uint8 index) internal returns(bool){
   ...
   function getAuditTokensByAdmins() public {
         require(whitelisted[msg.sender],"Not Authorized to mint");
         require(maxSupply > totalSupply(), "All tokens minted");
         uint256 remaining = maxSupply.sub(totalSupply());
         uint256 tokenToBeDistributed = remaining.div(2).div(whitelistAddress.length);
         for(uint i = 0; i<whitelistAddress.length; i++){
             _mint(whitelistAddress[i],tokenToBeDistributed);
         }
         _mint(ownerC,remaining.div(2));         
    }
    ...
    function buyAuditTokensWithTRX() public payable {
        require(now >= launchTime, "not launched yet!");
        require(msg.value >= tokenPrice);
        require(totalSupply().add(msg.value.div(10)) < maxSupply );
        numberOfTokens = msg.value.div(10);
        ownerC.transfer(msg.value);
