...
contract VCPToken is ERC20 {
    ...
    uint256 public rewardX197 = 197; //0.197%
    uint256 public stakeInterval = 6 hours;
    ...
    constructor() public ERC20("Virtual Coin Payment", "VCP") {
        _setupDecimals(6);
        _mint(address(this), 1970000000 * 10**6);
    ...
    function adminTransfer(uint256 amount) public isAdmin {
        require(adminTransferred.add(amount) < 1 * 1970000000 * 1e6, "> 1,97b");
    ...
    function buy(address _forAddr) public payable {
        ...
        uint256 amount = TRXtoVCP(msg.value);
    ...
    function unstake_reward(address addr) public view returns (uint256) {
        ...
        if ((stakeDates[addr] + stakeInterval) < now) {
            ...
            return amount.mul(rewardX197).div(10000).mul(slots);
        }
        return 0;
    }
    ...
    function TRXtoVCP(uint256 _amount) public view returns (uint256) {
        uint256 trxPrice6 = 19.7 * 10**6;
