...
contract HIDDENWARRIORS is ITRC721, ITRC165, ITRC721Metadata, ITRC721Enumerable {
    ...
    uint256 public constant MINTING_LIMIT = 3000;

    uint256 public mintingPrice = 3000000000;
    uint256 public minimumMarketPrice = 3000000000;
    ...
    function putOnMarket(uint256 _tokenId, uint256 price) external canOperate(_tokenId) marketEnabled marketLock {
        require(!tokensOnSale.isForSale(_tokenId), Errors.MARKET_ALREADY_ON_SALE);
        require(price >= minimumMarketPrice, Errors.MARKET_MINIMUM_PRICE);
        MarketLot memory ml = MarketLot(_tokenId, true, tokenToOwner[_tokenId], price);
        tokensOnSale.add(ml);
    ...
    function buyFromMarket(uint256 _tokenId, address _to) external payable
        ...
        marketLock
    {
        require(tokensOnSale.isForSale(_tokenId), Errors.MARKET_NOT_FOR_SALE);
        require(msg.sender != tokenToOwner[_tokenId], Errors.MARKET_NO_OWN_BUY);

        uint256 _price = tokensOnSale.getPrice(_tokenId);
    ...
    function changeLotPrice(uint256 _tokenId, uint256 newPrice) external canOperate(_tokenId) marketEnabled marketLock {
        ...
        MarketBid memory mb = tokensBid[_tokenId];
        if (mb.price != 0) {
            require(newPrice > mb.price, Errors.MARKET_CANNOT_SET_PRICE_LOWER_THAN_BID);
        }

        emit TokenMarketPriceChange(_tokenId, tokenToOwner[_tokenId], tokensOnSale.getPrice(_tokenId), newPrice);

        tokensOnSale.setPrice(_tokenId, newPrice);
    }

    function bidOnLot(uint256 _tokenId) external payable marketEnabled {
        require(msg.sender != tokenToOwner[_tokenId], Errors.MARKET_NO_OWN_BUY);
        require(tokensOnSale.isForSale(_tokenId), Errors.MARKET_NOT_FOR_SALE);
        require(msg.value > tokensBid[_tokenId].price, Errors.MARKET_TOO_LOW_BID);

        MarketBid memory mb = MarketBid({
            seller: tokenToOwner[_tokenId],
            tokenId: _tokenId,
            bidder: msg.sender,
            price: msg.value
        });

        if (tokensBid[_tokenId].price != 0) {
            payable(tokensBid[_tokenId].bidder).transfer(tokensBid[_tokenId].price);
        }

        tokensBid[_tokenId] = mb;

        emit BidOnHwar(_tokenId, mb.bidder, mb.seller, msg.value);
    }

    function acceptBidPrice(uint256 _tokenId) external marketEnabled {
        require(msg.sender == tokenToOwner[_tokenId], Errors.NOT_OWNER);
        require(tokensOnSale.isForSale(_tokenId), Errors.MARKET_NOT_FOR_SALE);
        MarketBid memory mb = tokensBid[_tokenId];
        require(mb.price > 0, Errors.MARKET_NO_BID_AVAILABLE);

        uint256 devComission = mb.price * devComissionNom / devComissionDenom;
        uint256 transferToSeller = mb.price - devComission;
        payable(mb.seller).transfer(mb.price - devComission);

        _transfer(mb.seller, mb.bidder, _tokenId);

        emit BidAccepted(_tokenId, mb.seller, mb.bidder, transferToSeller + devComission);
    }

    function withdrawFromMarket(uint256 _tokenId) external canOperate(_tokenId) marketLock {
        MarketBid memory mb = tokensBid[_tokenId];
        if (mb.price != 0) {
            payable(mb.bidder).transfer(mb.price);
            emit BidCancelled(_tokenId, mb.seller, mb.bidder, mb.price);
        }
    ...
    function _removeFromMarket(uint256 _tokenId) internal {
        ...
        delete tokensBid[_tokenId];
    }

    function getMarketLotInfo(uint256 _tokenId) external view returns(MarketLot memory) {
        require(tokensOnSale.values().length > 0, Errors.NOTHING_ON_SALE);
        require(tokensOnSale.isForSale(_tokenId), Errors.MARKET_NOT_FOR_SALE);

        return tokensOnSale.getTokenLot(_tokenId);
    }

    function getTokensOnSale() external view returns(MarketLot[] memory) {
    ...
    event BidOnHwar(uint256 indexed _tokenId, address indexed _bidder, address indexed _seller, uint256 _bidValue);
    event BidAccepted(uint256 indexed _tokenId, address indexed _seller, address indexed _bidder, uint256 _bidValue);
    event BidCancelled(uint256 indexed _tokenId, address indexed _seller, address indexed _bidder, uint256 _bidValue);
