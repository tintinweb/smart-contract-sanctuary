//SourceUnit: MonoFarm_ETAGHY.sol
...
contract MonoFarm_ETAGHY is Ownable{
    ...
    function pendindRewards() public view returns (uint256 ETAGHYReward) {
        userInfo storage user = users[msg.sender];
        ...
        if (users[msg.sender].lockableDays > toDays) return (0);
            ...
            ETAGHYReward = allocation[users[msg.sender].lockableDays].mul(users[msg.sender].farmedToken).div(100);
            ETAGHYReward = ETAGHYReward.add(users[msg.sender].farmedToken);    
            return (ETAGHYReward);
    }
    ...
    function harvest() public {
        ...
        (uint256 ETAGHYReward) = pendindRewards();
        if(ETAGHYReward > 0 ) {
            uint256 amounttoTransferred = user.farmedToken.add(ETAGHYReward);
    ...
    function safeTransferWYZ(address _farmer, uint256 _amount) internal {
        uint256 tokenBal = ETAGHY.balanceOf(address(this));
        if (_amount > tokenBal) {
            ETAGHY.transfer(_farmer, tokenBal);
        } else {
            ETAGHY.transfer(_farmer, _amount);
        }
    }
   
    
    // EMERGENCY ONLY.
    function emergencyWithdraw(uint256 ETAGHYAmount) public onlyOwner {
        safeTransferWYZ(msg.sender, ETAGHYAmount);
    }
}