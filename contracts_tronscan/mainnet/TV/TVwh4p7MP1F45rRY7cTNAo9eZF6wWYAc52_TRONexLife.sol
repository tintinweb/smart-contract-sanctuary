//SourceUnit: TRONexLife.sol

/*
 *
 *   TRONex Life - investment platform based on TRX blockchain smart-contract technology. Safe and legit!
 *   The only official platform of original TRONex team! All other platforms with the same contract code are FAKE!
 *
 *   ┌───────────────────────────────────────────────────────────────────────┐
 *   │   Website: https://tronex.life                                        │
 *   │                                                                       │
 *   │   Telegram Group: https://t.me/tronexlife                             |
 *   └───────────────────────────────────────────────────────────────────────┘
 *
 *   [USAGE INSTRUCTION]
 *
 ...
 *   - Certificate of incorporation: https://tronex.life/files/certificate.pdf
 *
 *   [SMART-CONTRACT AUDITION AND SAFETY]
 *
 *   - Audited by independent company Telescr In (Webiste: https://telescr.in)
 *   - Audition certificate: https://tronex.life/files/tronexlife_audit_en.pdf
 *   - Audited by independent company GROX Solutions (Webiste: https://grox.solutions)
 *   - Audition certificate: https://tronex.life/files/tronexlife_audit_en2.pdf
 *
 */
...
contract TRONexLife {
    ...
    function invest(address referrer) public payable {
        ...
        uint availableLimit = getCurrentQuarterDayAvailable();
        ...
        uint quarterDayTurnover = turnover[getCurrentQuarterDay()];
        uint quarterDayLimit = getCurrentDayLimit();

        if (INVEST_MIN_AMOUNT.add(msgValue).add(quarterDayTurnover) < quarterDayLimit) {
            turnover[getCurrentQuarterDay()] = quarterDayTurnover.add(msgValue);
        } else {
            turnover[getCurrentQuarterDay()] = quarterDayLimit;
        }
    ...
    function getCurrentQuarterDay() public view returns (uint) {
        return (block.timestamp.sub(contractCreation)).div(TIME_STEP.div(4));
    }
    ...
    function getCurrentQuarterDayTurnover() public view returns (uint) {
        return turnover[getCurrentQuarterDay()];
    }

    function getCurrentQuarterDayAvailable() public view returns (uint) {
        return getCurrentDayLimit().sub(getCurrentQuarterDayTurnover());
    }
    ...
    function getSiteStats() public view returns (uint, uint, uint, uint, uint) {
        return (totalInvested, totalDeposits, address(this).balance, contractPercent, getCurrentQuarterDayAvailable());
    }
