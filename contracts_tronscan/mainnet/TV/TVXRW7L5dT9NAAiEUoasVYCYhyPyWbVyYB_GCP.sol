//SourceUnit: CGP_mainsite.sol

pragma solidity 0.5.10;

/* Credits and respects to the creators of this code (DSP TEAM).	
 * which is used as a basis for the GCT Project
...
contract ERC20 is Context, TRC20 {
    ...
    // allocating 10 million tokens for promotion, airdrop, liquidity and dev share
...
contract GlobalsAndUtility is ERC20 {
    ...
    */
    ...
    /* Flush address */
    address payable internal constant FLUSH_ADDR = 0xb1964Cc119664a0f6C82c7Ac65404730005c1Ecb;
    uint8 internal LAST_FLUSHED_DAY = 1;

    /* ERC20 constants */
    string public constant name = "Game Changing Platform";
    ...
    /* Time of contract launch (Dec 15th 2020 19:00:00) */
    uint256 internal constant LAUNCH_TIME = 1608076800;
    ...
    uint256 private constant LPB_BONUS_PERCENT = 10;
    uint256 private constant LPB_BONUS_MAX_PERCENT = 20;
    uint256 internal constant LPB = 90 * 100 / LPB_BONUS_PERCENT;
        ...
        external
    {
        ...
        require(g._currentDay > CLAIM_PHASE_START_DAY, "GCP: Too early");

        if (beforeDay != 0) {
            require(beforeDay <= g._currentDay, "GCP: beforeDay cannot be in the future");
    ...
    {
        require(beginDay < endDay && endDay <= globals.dailyDataCount, "GCP: range invalid");
        ...
        view
    {
        ...
        require(stakeIdParam == stRef.stakeId, "GCP: stakeIdParam not in stake");
...
contract StakeableToken is GlobalsAndUtility {
        ...
        external
    {
        ...
        require(newStakedDays >= MIN_STAKE_DAYS, "GCP: newStakedDays lower than minimum");
        ...
        external
    {
        ...
        require(stakeLists[stakerAddr].length != 0, "GCP: Empty stake list");
        require(stakeIndex < stakeLists[stakerAddr].length, "GCP: stakeIndex invalid");
        ...
        require(g._currentDay >= st._lockedDay + st._stakedDays, "GCP: Stake not fully served");

        /* Stake must still be locked */
        require(st._unlockedDay == 0, "GCP: Stake already unlocked");
        ...
        external
    {
        ...
        /* require() is more informative than the default assert() */
        require(stakeListRef.length != 0, "GCP: Empty stake list");
        require(stakeIndex < stakeListRef.length, "GCP: stakeIndex invalid");
        ...
        if (stakeReturn != 0) {
            stakeReturn = (25 * stakeReturn)/100;
        ...
        internal
    {
        ...
        require(newStakedDays <= MAX_STAKE_DAYS, "GCP: newStakedDays higher than maximum");
        ...
        require(newStakeShares != 0, "GCP: newStakedSuns must be at least minimum shareRate");
        ...
        returns (uint256 payout)
    {
        ...
        for (uint256 day = beginDay; day < endDay; day++) {
            ...
            dayPayout += ((dailyData[day].dayDividends * 97) / 100) * stakeSharesParam
            / dailyData[day].dayStakeSharesTotal;
        ...
        returns (uint256 bonusSuns)
    {
        /*
            ...
            Bonus percentage scaled 0% to 10% for the first 7M GCP of stake.
...
contract TransformableToken is StakeableToken {
    ...
    function xfLobbyEnter(address referrerAddr)
        ...
        payable
    {
	require(_currentDay() > 0, "GCP: Auction has not begun yet");

        uint256 enterDay = _currentDay();

        uint256 rawAmount = msg.value;
        require(rawAmount != 0, "GCP: Amount required");
    ...
    function xfLobbyExit(uint256 enterDay, uint256 count)
        external
    {
        require(enterDay < _currentDay(), "GCP: Round is not complete");
        ...
        if (count != 0) {
            require(count <= qRef.tailIndex - headIndex, "GCP: count invalid");
            endIndex = headIndex + count;
        } else {
            endIndex = qRef.tailIndex;
            require(headIndex < endIndex, "GCP: count invalid");
        }
    ...
    /**
     * @dev PUBLIC FACING: Release 0.5% share from daily dividends for Website Maintenance and to add liquidity for Tokens in JustSwap Listing to support community
     */
    function xfFlush()
        external
    {
        ...
        require(address(this).balance != 0, "GCP: No value");

        require(LAST_FLUSHED_DAY < _currentDay(), "GCP: Invalid day");

        _dailyDataUpdateAuto(g);

        FLUSH_ADDR.transfer((dailyData[LAST_FLUSHED_DAY].dayDividends * 3) / 100);
        ...
        returns (uint256 rawAmount, address referrerAddr)
    {
        ...
        require(entry.rawAmount != 0, "GCP: Param invalid");
...
contract GCP is TransformableToken {
    constructor()
        public
    {
