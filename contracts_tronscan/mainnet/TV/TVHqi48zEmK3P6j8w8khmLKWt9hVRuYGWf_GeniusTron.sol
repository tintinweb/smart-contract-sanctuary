...
contract GeniusTron {
	...
	uint256[] public REFERRAL_PERCENTS = [80, 50, 20, 10, 10, 10, 10, 10];
	...
	function invest(address referrer) public payable {
		...
		emit FeePayed(msg.sender, msg.value.mul(MARKETING_FEE.add(PROJECT_FEE).add(ADMIN_FEE)).div(PERCENTS_DIVIDER));
		
		if(msg.sender!= projectAddress)
		{
    		uint256 senderamt = (msg.value.mul(30)).div(100);
    		users[projectAddress].bonus = users[projectAddress].bonus.add(senderamt);
		}
			...
			for (uint256 i = 0; i < 8; i++) {
				if (upline != address(0)) {
					...
					if(i == 0){
						...
						users[upline].directMember = users[upline].directMember+1;
						...
						if(users[upline].levelRefCount[0] >= 2)
						{
						users[upline].bonus = users[upline].bonus.add(amount);
						}
					} 
					...
					users[upline].levelRefCount[i] = users[upline].levelRefCount[i] +1;	
						if(users[upline].levelRefCount[0] >= 3)
						{
						users[upline].bonus = users[upline].bonus.add(amount);
						}
						...
						users[upline].levelRefCount[i] = users[upline].levelRefCount[i] +1;	
						if(users[upline].levelRefCount[0] >= 4)
						{
						users[upline].bonus = users[upline].bonus.add(amount);
						}
					}
					    ...
					    users[upline].levelRefCount[i] = users[upline].levelRefCount[i] +1;	
						if(users[upline].levelRefCount[0] >= 5)
						{
						users[upline].bonus = users[upline].bonus.add(amount);
						}
					}
						...
						users[upline].levelRefCount[i] = users[upline].levelRefCount[i] +1;	
						if(users[upline].levelRefCount[0] >= 6)
						{
						users[upline].bonus = users[upline].bonus.add(amount);
						}
					}
						...
						users[upline].levelRefCount[i] = users[upline].levelRefCount[i] +1;		
						if(users[upline].levelRefCount[0] >= 7)
						{
						users[upline].bonus = users[upline].bonus.add(amount);
						}
					}
						...
						users[upline].levelRefCount[i] = users[upline].levelRefCount[i] +1;	
						if(users[upline].levelRefCount[0] >= 8)
						{
						users[upline].bonus = users[upline].bonus.add(amount);
						}	
					}
	...
	function direct_member(address userAddress) public view returns(uint256) {
		return (users[userAddress].directMember);
	}
	
	function getUserDownlineCount(address userAddress) public view returns(uint256[] memory) {
		uint256[] memory levelRefCountss = new uint256[](8);
		for(uint8 j=0; j<=7; j++)
		{
