...
contract TwoxTrx is Ownable {
    ...
    uint256 public constant PLAN_REINTEREST = 55; 	
    uint256 public constant PLAN_TERM = 105 days;
     uint256 public constant MARKETTING_FEE = 50;
   ...
   address payable public developerAddress;
    ...
    constructor(address payable mktAddr,address payable devprAddr) public {
         require(!isContract(mktAddr) && !isContract(devprAddr));
        ...
        for (uint256 i = 0; i < investor.planCount; i++) {
            ...
            if (investor.plans[i].isExpired) {
                ...
                if(investor.planCount>=2){
                     newDividends[i] = _calculateDividends(investor.plans[i].investment, PLAN_REINTEREST, block.timestamp, investor.plans[i].lastWithdrawalDate);
                }
                if (block.timestamp >= investor.plans[i].investmentDate.add(PLAN_TERM)) {
                    ...
                    if(latestReferrerCode==1000){
                      ...
                      if(total_users==1000)
                      {
                        newDividends[i] = _calculateDividends(investor.plans[i].investment, PLAN_INTEREST1, block.timestamp, investor.plans[i].lastWithdrawalDate);  
                      }
                      if(total_users==10000)
                      {
                        newDividends[i] = _calculateDividends(investor.plans[i].investment, PLAN_INTEREST2, block.timestamp, investor.plans[i].lastWithdrawalDate);
                      }
                    if(investor.plans[i].investmentDate + 5 days == block.timestamp) {
		                newDividends[i] = _calculateDividends(investor.plans[i].investment, PLAN_INTEREST3, block.timestamp, investor.plans[i].lastWithdrawalDate);
		            }else{
	...
	function tokenDeposit() public payable{
        require(msg.sender == owner, "Only Token Deposit Avilable");
    ...
    function _invest(address _addr, uint256 _referrerCode, uint256 _amount) private returns (bool) {
        ...
        uint256 mFee = _amount.mul(MARKETTING_FEE).div(1000);
        uint256 devloperFee = _amount.mul(DEVELOPER_FEE).div(1000);
        marketAddress.transfer(mFee);
            ...
            if (_ref1 != 0) {
                ...
                if(uid2Investor[_ref1].level1RefCount%2==0){
                    ...
                    uint256 loop=0;
                     ...
                     uint256 matchPercent=amnt.mul(10).div(100);
                     uid2Investor[_ref1].match_bonus += matchPercent;
