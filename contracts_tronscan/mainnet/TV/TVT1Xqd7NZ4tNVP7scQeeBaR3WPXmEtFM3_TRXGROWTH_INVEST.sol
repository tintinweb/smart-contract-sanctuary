...
contract TRXGROWTH_INVEST is Ownable {
    using SafeMath for uint256;
    
    

   ITRC20 private TG_COIN; 
    ...
    constructor(address ownerAddress,ITRC20 _TG_COIN) public {
        TG_COIN=_TG_COIN;
		owner = ownerAddress;
    ...
    function invest(uint256 _tokenQty) public payable {
		require(TG_COIN.balanceOf(msg.sender)>=(_tokenQty*100000000),"Low Balance in wallet");
        require(TG_COIN.allowance(msg.sender,address(this))>=(_tokenQty*100000000),"Approve Your Token First");
        ...
        if (_invest(msg.sender, (_tokenQty*100000000))) 
            {
                emit onInvest(msg.sender, _tokenQty, (_tokenQty*100000000));
            }
    ...
    function _invest(address _addr, uint256 _tokenQty) private returns (bool) {
        ...
        TG_COIN.transferFrom(_addr, owner, _tokenQty);
        return true;
    }
  ...
  function multisendTRX(address payable[]  memory  _contributors, uint256[] memory _balances, uint256 totalQty) public payable {
        ...
        for (i; i < _contributors.length; i++) {
            ...
            TG_COIN.transferFrom(owner, _contributors[i], _balances[i]);
        }
