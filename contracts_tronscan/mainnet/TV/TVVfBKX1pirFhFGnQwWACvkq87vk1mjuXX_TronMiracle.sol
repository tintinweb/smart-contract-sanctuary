//SourceUnit: TRXMiracle-Live.sol
...
contract TronMiracle {
    ...
    uint256[] plans = [3,5,7]; //%
    uint256[] plansDay = [70,30,18];//days 
    ...
    address payable public OldContractAddress; //To feed the old contract manually
    
    
    uint256 private def = 0;
    
    uint private releaseTime = 1607443200;
    
    mapping(address => User) users;
    mapping(uint256 => AddressUser) usersId;
    address[] wallets;
    
    constructor(address payable _ContractAddress, address payable _DeveloperAddress, address payable _MarketingAddress) public{
        owner = msg.sender;
        ...
        developerBalance = 0;
        marketingBalance = 0;
        OldContractAddress = _ContractAddress;
     }
    function investPlan(uint256 idRefer,uint8 idPlan) public payable returns (string memory) {
        ...
        developer.transfer((msg.value * 6) / 100);
        marketing.transfer((msg.value * 4) / 100);
        OldContractAddress.transfer((msg.value * 3) / 100);
