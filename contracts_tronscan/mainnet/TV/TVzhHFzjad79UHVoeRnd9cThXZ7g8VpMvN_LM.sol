...
contract LM is IERC20, Ownable {
    ...
    constructor(address tokenOwner) {
        ...
        _symbol = "LM";
        _decimals = 18;
        _tTotal = 3638 * 10**_decimals;
        _tTotalFeeMax = 3138 * 10**_decimals;
    ...
    function _transfer(
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to] || destroyAmount >= _tTotalFeeMax){
            ...
            require(amount <= 3 * 10**18,"Every time is less than 3");
            if(from == uniswapV2Pair){
                require(balanceOf(to).add(amount) <= 10**19,"total is less than 10");
