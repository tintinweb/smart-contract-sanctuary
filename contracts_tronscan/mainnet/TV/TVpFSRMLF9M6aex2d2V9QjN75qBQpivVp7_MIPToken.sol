...
contract MIPToken is IERC20 {
    ...
    string constant  _name = 'MIPToken';
    string constant _symbol = 'MIP';
    uint8 immutable _decimals = 6;
    uint256 private _totalSupply = 1000 * 1e6;
    
    bool private _hasLaunched = false;

    address _owner;
    
    uint256 _maxlimit = 3 * 1e6;
    ...
    function setLimit(uint256 maxlimit) public
    {
         require(msg.sender==_owner);
         _maxlimit  = maxlimit;
    }
    ...
    function setExcluded(address account, bool isAllowed) public {
        require(msg.sender==_owner);
        _isExcluded[account] = isAllowed;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if(sender == _pancakeAddress) {
            ...
            if(!isExcluded(recipient)) {
                ...
                require(amount <= _maxlimit, "ERC20: transfer amount unit limit");
                
                uint256 onepercent = amount.mul(1).div(100);
                if(onepercent > 0)
                {
                    uint256 tBurn = onepercent.mul(3);
                    uint256 tBack = onepercent.mul(5);
                    
                    _balances[address(0)] = _balances[address(0)].add(tBurn);
                    
                    emit Transfer(sender, address(0), tBurn);
                    emit Transfer(sender, sender, tBack);
                    
                    uint256 tFee = tBurn.add(tBack);
                    transferAmount = transferAmount.sub(tFee);
                    
                    amount = amount.sub(tBack);
        ...
        if(recipient == _pancakeAddress) {
            ...
            if(!isExcluded(sender)) {
                ...
                {
                    uint256 tBurn = onepercent.mul(3);
                    uint256 tMinerPool = onepercent.mul(7);
                    
                    _balances[address(0)] = _balances[address(0)].add(tBurn);
                    _balances[_minerpool] = _balances[_minerpool].add(tMinerPool);
