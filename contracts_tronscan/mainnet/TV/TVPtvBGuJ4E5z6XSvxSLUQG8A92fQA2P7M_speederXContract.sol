//SourceUnit: speederXTrxLevel15Test-v1.sol
...
contract speederXContract {
    ...
    function registration(address userAddress, address referrerAddress) private {
        require(msg.value == 2 trx, "registration cost 200");
    ...
    function updateF1Referrer(address userAddress, address referrerAddress, uint8 level) private {
        ...
        if (users[referrerAddress].f1Matrix[level].referrals.length < 3) {
            emit NewUserPlace(userAddress, referrerAddress, 1, level, uint8(users[referrerAddress].f1Matrix[level].referrals.length), users[referrerAddress].f2Matrix[level].reinvestCount);
            return sendETHDividends(referrerAddress, userAddress, 1, level);
        }
        
        emit NewUserPlace(userAddress, referrerAddress, 1, level, 3, users[referrerAddress].f1Matrix[level].reinvestCount);
        //close matrix
    ...
    function updateF2Referrer(address userAddress, address referrerAddress, uint8 level) private {
        ...
        if (users[referrerAddress].f2Matrix[level].firstLevelReferrals.length < 2) {
            ...
            emit NewUserPlace(userAddress, referrerAddress, 2, level, uint8(users[referrerAddress].f2Matrix[level].firstLevelReferrals.length), users[referrerAddress].f2Matrix[level].reinvestCount);
            ...
            if ((len == 2) && 
                ...
                if (users[referrerAddress].f2Matrix[level].firstLevelReferrals.length == 1) {
                    emit NewUserPlace(userAddress, ref, 2, level, 5, users[referrerAddress].f2Matrix[level].reinvestCount);
                } else {
                    emit NewUserPlace(userAddress, ref, 2, level, 6, users[referrerAddress].f2Matrix[level].reinvestCount);
                }
            }  else if ((len == 1 || len == 2) &&
                    users[ref].f2Matrix[level].firstLevelReferrals[0] == referrerAddress) {
                if (users[referrerAddress].f2Matrix[level].firstLevelReferrals.length == 1) {
                    emit NewUserPlace(userAddress, ref, 2, level, 3, users[referrerAddress].f2Matrix[level].reinvestCount);
                } else {
                    emit NewUserPlace(userAddress, ref, 2, level, 4, users[referrerAddress].f2Matrix[level].reinvestCount);
                }
    ...
    function updateF2(address userAddress, address referrerAddress, uint8 level, bool x2) private {
        if (!x2) {
            ...
            emit NewUserPlace(userAddress, users[referrerAddress].f2Matrix[level].firstLevelReferrals[0], 2, level, uint8(users[users[referrerAddress].f2Matrix[level].firstLevelReferrals[0]].f2Matrix[level].firstLevelReferrals.length), users[referrerAddress].f2Matrix[level].reinvestCount);
            emit NewUserPlace(userAddress, referrerAddress, 2, level, 2 + uint8(users[users[referrerAddress].f2Matrix[level].firstLevelReferrals[0]].f2Matrix[level].firstLevelReferrals.length), users[referrerAddress].f2Matrix[level].reinvestCount);
            //set current level
            ...
            emit NewUserPlace(userAddress, users[referrerAddress].f2Matrix[level].firstLevelReferrals[1], 2, level, uint8(users[users[referrerAddress].f2Matrix[level].firstLevelReferrals[1]].f2Matrix[level].firstLevelReferrals.length), users[referrerAddress].f2Matrix[level].reinvestCount);
            emit NewUserPlace(userAddress, referrerAddress, 2, level, 4 + uint8(users[users[referrerAddress].f2Matrix[level].firstLevelReferrals[1]].f2Matrix[level].firstLevelReferrals.length), users[referrerAddress].f2Matrix[level].reinvestCount);
            //set current level
