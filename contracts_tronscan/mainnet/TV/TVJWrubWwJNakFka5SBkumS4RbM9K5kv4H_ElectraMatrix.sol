//SourceUnit: Eletra2newfinal.sol
...
contract ElectraMatrix {
    ...
    function withdraw(
        ...
        emit withdrawl(level,userAddress,amount);
    ...
    function investmentbyadmin(address user, uint8 level, uint256 amount) private {
        ...
        users[user].activeEECLevels[level] = true;
        emit EECupgrade(user, level, (amount * 1) / 2);
    }
        ...
        returns (string memory)
    {
        if (EECc[level].totalservicefund > 0) {
            ...
            emit withdrawnetworkfees(level,msg.sender,EECc[level].totalservicefund);
            return "withdraw successfull";
        }
    }
}