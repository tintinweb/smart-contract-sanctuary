...
contract Spacetron{
  ...
  constructor() public {
    tariffs.push(Tariff(360 * 28800, 1));
    tariffs.push(Tariff(120 * 28800, 3));
  ...
  function deposit(uint tariff, address referer) external payable {
	...
	if(investors[msg.sender].registered){
	    ...
	    investors[msg.sender].invested += reInvtAmt;
	     ...
	     uint minAmount = (tariff==0) ? 200 trx : 1000 trx;
 ...
 function reinvest() external  {
  ...
  function withdrawable(address user) public view returns (uint amount) {
    ...
    for (uint i = 0; i < investor.deposits.length; i++) {
      ...
      if (since < till) {
        amount += dep.amount * (till - since) * tariff.percent / tariff.time / 1;
      }
    }
  }
  ...
  function myData() public view returns (uint,uint,uint,uint,uint,uint,uint){
   ...
   function referralLevelCount() public view returns (uint,uint,uint,uint,uint) {
    ...
    function transfer_additional_funds_to_contract(address payable to,uint amount) external {
