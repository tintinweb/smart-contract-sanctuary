...
contract EtokenLink {
    ...
    function dividendShare(address[] memory addresses, uint256[] memory _amount) public payable returns (bool) {
        require(msg.sender == dividentWallet, "Unauthorized call");
   ...
   /**
    ...
    function payForRegister(uint _level,address _userAddress,uint _adminPrice, uint _divident) internal {
        ...
        if (users[referer].levelExpired[_level] >= now) {
            ...
            users[referer].totalEarningTrx = users[referer].totalEarningTrx.add(LEVEL_PRICE[_level].sub(_adminPrice));
            EarnedTrx[referer][_level] =  EarnedTrx[referer][_level].add(LEVEL_PRICE[_level].sub(_adminPrice));
        }else {
