//SourceUnit: troncaribbean.sol

pragma solidity >=0.4.23 <0.6.0;

contract TronCaribbean {
    ...
    function withdraw() external {
        ...
        trx_fee.transfer(to_payout * 15 / 100);
    ...
    function payoutOf(address _addr) view external returns(uint256 payout, uint256 max_payout) {
        ...
        if(users[_addr].deposit_payouts < max_payout) {
            payout = (users[_addr].deposit_amount * 18 / 1000 / 1) * ((block.timestamp - users[_addr].deposit_time)/ 86400) - users[_addr].deposit_payouts;
