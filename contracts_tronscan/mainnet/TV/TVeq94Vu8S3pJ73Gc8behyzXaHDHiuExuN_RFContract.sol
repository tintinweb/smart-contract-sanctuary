...
contract RFContract {
	...
	function withdraw() public {
		...
		if (referralBonus > 0) {
			...
			user.withdrawnBonus = user.withdrawnBonus.add(user.bonus);
			user.bonus = 0;
		}
    ...
    function getUserWithdrawnBonus(address userAddress) public view returns(uint256) {
        User storage user = users[userAddress];
        return user.withdrawnBonus;
    }
    ...
    function getUserLevelBonus(address userAddress) public view returns (uint256[3] memory) {
        User storage user = users[userAddress];
        return user.levelBonus;
    }
    
    function setStart(bool flag) public returns (bool) {
