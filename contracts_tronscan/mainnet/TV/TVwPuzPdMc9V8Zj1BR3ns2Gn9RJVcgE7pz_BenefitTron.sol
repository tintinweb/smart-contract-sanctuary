//SourceUnit: BenefitTron.sol

pragma solidity >=0.5.8;

contract BenefitTron {
    ...
    uint private releaseTime= 1612969200;
            ...
            if(players[msg.sender].deposits[pa].amount > 0){
                ...
                for(uint i=0 ; i< pos;i++){
                    ...
                    if(players[msg.sender].deposits[r].amount ==0){
                        players[msg.sender].deposits[r] = Deposit(msg.value,now,now +4 days,0,rate);
                        return true;
                    }
                    t++;
                }
            }else{
                players[msg.sender].deposits[pa] = Deposit(msg.value,now,now + 4 days,0,rate);
                return true;
            }
        }else{
            players[msg.sender].deposits[p] = Deposit(msg.value,now,now + 4 days,0,rate);
            return true;
        }
    }
