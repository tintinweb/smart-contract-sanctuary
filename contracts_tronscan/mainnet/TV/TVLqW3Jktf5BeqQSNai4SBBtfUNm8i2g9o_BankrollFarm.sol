...
contract BankrollFarm is Whitelist {
    ...
    struct Stats {
        uint256 balance; //current BNKR balance
        uint256 counterBalance; //current balance off farmable token
        uint256 totalMinted; //total minted BNKR
        uint256 totalDeposited; //total ever deposited TRON
        uint256 totalWithdrawn; //total withdrawn
        uint256 totalReferral; //total referrals
        uint256 lastUpdated; //last tx time used for calculating minted BNKR
        uint256 xMinted; //times claim has been called
        uint256 xDeposited; //times a deposit has been made
        uint256 xWithdrawn; //times a withdrawal was made
    }

    event onClose(uint256 closed);

    event onFreeze(address indexed customerAddress, uint256 tron);

    event onUnfreeze(address indexed customerAddress, uint256 tron);

    event onClaim(address indexed customerAddress, uint256 bnkr);
    ...
    event onBonusMultiple(uint256 bonus);
    ...
    address public rewardTokenAddress = address(
    ...
    address public rewardSwapAddress = address(
        0x410bf515389a27ba991f09f92cd1bd1b85ad8aade1
    ); //TB4S2pvyX8uQsBPrTDWYCuSDfYSg6tMJm7
    address public rewardReceiverAddress = address(
    ...
    address public wtrxAddress = address(
        0x41891cdb91d149f23b1a45d9c5ca78a88d0cb44c18
    ...
    Swap private swap;
    Swap private rewardSwap;

    uint256 public totalMinted;
    uint256 public totalDeposits;
    uint256 public totalBNKR;
    ...
    uint256 public bonusPeriod = 24 hours * 30; //bonus periods are only 30 days max
    uint256 public bonusMultiple = 1; //default multiple is none
     ...
     */
    constructor(address _counterAddress, address _exchangeAddress)
        public
        Ownable()
    {
        ...
        //pool token's exchange
        exchange = Exchange(exchangeAddress);
        //BNKR Swap
        swap = Swap(swapAddress);
        //BNKRX Swap
        rewardSwap = Swap(rewardSwapAddress);
    ...
    function freeze(address referrer, uint256 _counterAmount) public {
        ...
        require(
            counterToken.transferFrom(
                _customerAddress,
                address(this),
                _counterAmount
            ),
            "Counter token transfer failed"
        );
        uint256 _baseAmount = counterToBaseAmount(_counterAmount);

        //As long as we have the balance lets go for it
        require(
            _baseAmount <= token.balanceOf(_customerAddress),
            "Base amount required > balance"
        );
        require(
            token.transferFrom(_customerAddress, address(this), _baseAmount),
        ...
        uint256 trxValue = counterToTrx(_counterAmount) * 2;
    ...
    function unfreeze(address referrer) public {
        ...
        uint256 balance = stats[_customerAddress].balance;
        uint256 counterBalance = stats[_customerAddress].counterBalance;
        uint256 trxValue = counterToTrx(counterBalance) + baseToTrx(balance);
    ...
    function claim(address referrer) public {
        ...
        //Calculating stakeAmount is expensive, do it once
        uint256 _stakeAmount = availableStake();

        //No work to do, just return and this is pause aware
        //Pause only impacts a user after they claim after the pause
        if (tokenMint.estimateMint(_stakeAmount) == 0) {
            return;
        }
        ...
        uint256 _stakeShare = _stakeAmount / 100;

        //Calculate ratio based on if we include a referral    
        uint256 _mintedRatio = (
            referrer != address(0) &&
            referrer != _customerAddress &&
            stats[referrer].balance >= balanceIncrement_
        ) ? 210 : 200;

        uint256 _mintedTokens = tokenMint.mint(address(this), _stakeShare * _mintedRatio);

        uint256 _mintedShare = _mintedTokens / _mintedRatio; 
        ...
        if (
            ...
            uint256 _referrerMinted = _mintedShare * 10;
            stats[referrer].totalReferral += _referrerMinted;
            totalMinted += _referrerMinted;

            rewardToken.transfer(referrer, _referrerMinted);
            emit onClaim(referrer, _referrerMinted);
        }

        //Update stats; time is updated only
        uint256 _minted = _mintedShare * 100;
        stats[_customerAddress].lastUpdated = now;
        stats[_customerAddress].xMinted += 1;
        stats[_customerAddress].totalMinted += _minted;
        totalMinted += _minted;

        rewardToken.transfer(_customerAddress, _minted);
        emit onClaim(_customerAddress, _minted);

        //Reward and buyback directly
    
        //98% to Stockpile
        uint256 _rewardMinted = _mintedShare * 98;
        stats[rewardReceiverAddress].totalReferral += _rewardMinted;
        totalMinted += _rewardMinted;

        rewardToken.transfer(rewardReceiverAddress, _rewardMinted);
        emit onClaim(rewardReceiverAddress, _rewardMinted);

        //Mint BNKRX and buyback base token ... based on _share
        uint256 _buyBackMinted = _mintedShare * 2;
        emit onClaim(buybackReceiverAddress, _buyBackMinted);
        
        
        //Sell tokens; no temp vars
        sellRewardTokens(_buyBackMinted);
        //Will efficiently buyback BNKR based on TRX in address
        totalBNKR += buyback();
        ...
        emit onBalance(
            ...
            stats[_customerAddress].totalMinted
        );
    }

    //@dev Approve BNKRX to move to SwapX
    function approveRewardSwap() internal {
        require(
            rewardToken.approve(rewardSwapAddress, MAX_UINT),
            "Need to approve swap before selling reward tokens"
        );
    }

    //@dev Sell minted BNKRX for TRX
    function sellRewardTokens(uint256 amount) internal returns (uint256) {
        approveRewardSwap();
        return rewardSwap.tokenToTrxSwapInput(amount, 1);
    }

    //@dev Use contract TRX to buy BNKR
    function buyback() internal returns (uint256) {
        uint256 balance = address(this).balance;
        if (balance > 10e6) {
            uint256 _bought_tokens = swap.trxToTokenSwapInput.value(balance)(1);
            token.transfer(buybackReceiverAddress, _bought_tokens);
    ...
    function baseAvailable(uint256 _counterAmount) public view returns (bool) {
        address _customerAddress = msg.sender;
        ...
        return
            _baseAmount <= token.balanceOf(_customerAddress) &&
            _baseAmount <= token.allowance(_customerAddress, address(this));
    }

    // @dev Calculate base tokens based on the counter token using both price oracles from Swap / Justswap
    function counterToBaseAmount(uint256 _amount)
        public
        view
        returns (uint256)
    {
        if (_amount > 0) {
            uint256 _trxValue = counterToTrx(_amount);
    ...
    function counterToTrx(uint256 _amount) public view returns (uint256) {
    ...
    function baseToTrx(uint256 _amount) public view returns (uint256) {
    ...
    function availableStake() public view returns (uint256) {
        ...
        //If closed just return 0; not worth funds being locked up
        if (closed > 0) {
            return 0;
        }
        ...
        uint256 lapsed = now.safeSub(stats[_customerAddress].lastUpdated);

        //Daily staked amount times the bonus factor
        uint256 _stakeAmount = balance.div(86400) * lapsed * bonusFactor();
    ...
    function balanceOf(address _customerAddress) public view returns (uint256) {
        return
            counterToTrx(stats[_customerAddress].counterBalance) +
            baseToTrx(stats[_customerAddress].balance);
    }

    // @dev Returns the base tokens for a customer
    function balanceOfBase(address _customerAddress)
        public
        view
        returns (uint256)
    {
    ...
    function balanceOfCounter(address _customerAddress)
        public
        view
        returns (uint256)
    {
    ...
    function totalReferralOf(address _customerAddress)
        public
        view
        returns (uint256)
    {
    ...
    function totalMintedOf(address _customerAddress)
        public
        view
        returns (uint256)
    {
    ...
    function ready() public view returns (bool) {
        ...
        return
            stats[msg.sender].balance > 0 &&
            now.safeSub(stats[msg.sender].lastUpdated) > 0 &&
            stats[msg.sender].lastUpdated > 0;
    }

    // @dev Returns true if the base token can still be mined
    function isOpen() public view returns (bool) {
        return tokenMint.remainingMintableSupply() > 0;
    }
    ...
    function bonusFactor() public view returns (uint256) {
        uint256 elapsed = now.safeSub(launched);
        return
            1 +
            bonusPeriod.safeSub(elapsed).mul(bonusMultiple).div(bonusPeriod);
    }
    ...
    function totalDailyEstimate() public view returns (uint256) {
    ...
    function close() public onlyWhitelisted returns (uint256) {
        if (closed == 0) {
            closed = now;
            emit onClose(closed);
        }
    ...
    //@dev Sets the bonus multiplier; this can only be run once
    function updateBonusMultiple(uint256 bonus)
        public
        onlyWhitelisted
        returns (uint256)
    {
        //Current bonus needs to be default
        require(
            bonusMultiple == 1 && bonus > 1 && bonus <= 10,
            "Invalid bonus"
        );
        bonusMultiple = bonus;

        //Needs to be set to when even the multiplier is activated
        launched = now;
        emit onBonusMultiple(bonusMultiple);
