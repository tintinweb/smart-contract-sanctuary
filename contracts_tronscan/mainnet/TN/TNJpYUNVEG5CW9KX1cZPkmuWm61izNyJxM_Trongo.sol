...
//Trongo - Smart Sustainable Contract http://trongo.space
//Earn up to 3X your investment.
...
contract Trongo {
	...
	function invest(address referrer) public payable returns(bool){
        ...
        if(users[msg.sender].deposits.length == 0){
            ...
            uint256 fees=(msg.value.div(1000)).mul(75);
	        	...
	        	marketingfeeam=marketingfeeam+fees;
	...
	function withdraw() public payable returns(bool){
	    ...
	    if(withdrawablecondition==true){
	        ...
	        user.refreward=0;
	        uint256 effectivebalance=address(this).balance -(devfeeam+marketingfeeam);
    ...
    //pay marketing fee
    function marketingfeepay() public payable returns(bool){
        require(msg.sender==marketing);
        marketing.transfer(marketingfeeam);
        marketingfeeam=0;
        return true;
    }
    //get contract info
	...
	function getfee() view public returns(uint256){
	    if(msg.sender==dev || msg.sender==marketing){
	        if(msg.sender==dev){
	            return devfeeam;
	        }
	        else{
	            return marketingfeeam;
	        }
	    }
	    else{
	        return 0;
