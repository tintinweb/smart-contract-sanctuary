...
contract GreenTron {
  ...
  constructor() public {
    tariffs.push(Tariff(7 * 28800, 105));
    tariffs.push(Tariff(7 * 28800, 112));
    tariffs.push(Tariff(7 * 28800, 119));
    tariffs.push(Tariff(7 * 28800, 126));
  ...
  function deposit(address referer) external payable {
    ...
    require(investors[msg.sender].depositCount < 200);
    
    uint tariff;
    if        (msg.value < 500000000) { // 500 TRX
      tariff = 0;
    } else if (msg.value < 5000000000) { // 5 000 TRX
      tariff = 1;
    } else if (msg.value < 50000000000) { // 50 000 TRX
      tariff = 2;
    } else if (msg.value < 500000000000) { // 500 000 TRX
      tariff = 3;
    } else {
      tariff = 4;
    }
