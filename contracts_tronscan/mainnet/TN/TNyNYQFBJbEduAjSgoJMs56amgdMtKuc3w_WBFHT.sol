...
/**
 * @dev Contract for wrapping TRC10 version of BFHT into TRC20 version
 */
contract WBFHT is Ownable, TRC10Wrapper {
    /**
     * @dev Emitted the owner recovers TRX sent by mistake
     */
    event RecoverTRX(
        address indexed owner,
    ...
    /**
     * @dev Emitted the owner recovers tokens sent by mistake
     */
    event RecoverTokens(
        address indexed owner,
        address indexed recipient,
    ...
    constructor()
        TRC20("Wrapped BeFaster Holder Token", "WBFHT")
        TRC10Wrapper(1003360)
    {}
    ...
    function recoverMistakenTokens(address payable account, trcToken tokenId)
        external
        onlyOwner
        returns (uint256)
    {
        uint256 amount = address(this).tokenBalance(tokenId);
        if (amount > 0) {
            account.transferToken(amount, tokenId);
            emit RecoverTokens(_msgSender(), account, tokenId, amount);
        }
        return amount;
    }

    function recoverMistakenTRX(address payable account)
        external
        onlyOwner
        returns (uint256)
    {
        ...
        if (amount > 0) {
            ...
            emit RecoverTRX(_msgSender(), account, amount);
        }
        return amount;
    }
}