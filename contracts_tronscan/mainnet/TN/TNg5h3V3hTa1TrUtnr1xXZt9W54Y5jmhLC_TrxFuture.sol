...
contract TrxFuture {
    ...
    function withdraw() payable external {
        ...
        require(this.withdrawalsOf(msg.sender) == 0,"1 Withdrawal per month only");
    ...
    function reinvest2() payable external {
        _payout(msg.sender);
        Player storage player = players[msg.sender];
        require(player.dividends > 0 || player.referral_bonus > 0, "Zero amount");
        uint256 amount = player.dividends + player.referral_bonus + player.withdrawal_excess;
        player.dividends = 0;
        player.referral_bonus = 0;
        player.withdrawal_excess=0;
        uint256 amount_withdrawable = 0;
        amount_withdrawable = amount;
        reinvest(msg.sender,amount_withdrawable);
        emit Reinvest(msg.sender, amount_withdrawable);
    }
