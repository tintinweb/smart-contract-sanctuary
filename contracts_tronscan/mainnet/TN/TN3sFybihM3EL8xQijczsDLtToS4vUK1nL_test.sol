...
contract test {
    ...
    uint private minDepositSize = 1000000; // min deposit 1
  ...
  //  uint private minuteRate = 0;  
   ...
   function register(address _addr, address _affAddr) private{
    ...
    function deposit(uint _depAmount, address _affAddr ) public {
        ...
        require(maxtime < 260 minutes, "your deposit has finished");
       ...
       //   uint256 actualCycle = getActualCycle(); //seguramente esta parte no está bien, pues tiene que tener en cuenta su propio ciclo.
        ...
        //  player.lastCycle = actualCycle + 26;
          ...
          checkPlus(msg.sender);
           ...
           player.activedeptime = player.activation - 10 minutes;  //REVISAR EN GETPROFIT EN QUE MOMENTO TENGO QUE PONER ESE VALOR PARA CALCULAR
    ...
    function newactivedep(address _addr) private {
        ...
        uint256 timepassed = getActualActivationCycle();
          timepassed = timepassed - 10 minutes;
          timepassed = now - timepassed;
          uint256 cycleinterest = ((((player.activeDeposit*timepassed) / 10 minutes)*(player.plus-100)) / 100);
     ...
     newvalue2 = newvalue2 / 10 minutes;
    ...
    function finalwithdraw() public {
      ...
      require(totaltime > 260 minutes && player.totalDeposit >0);
      
      // uint256 check = getActualActivationCycle(); //evitar solicitar retiros si hay retiros solicitados y estamos en los 5 días de desbloqueo final. Está mal...pues 
      //si es así nunca now será mayor a getactualactivationcycle.
      
      if (player.withdrawtime > 0){
      check = player.withdrawtime - 30 minutes;
      }else {
        check = 0;
      }
      
      uint256 check2 =  check + 30 minutes;
     ...
     player.withdrawtime = player.withdrawtime.add(30 minutes);
    ...
    function partialwithdraw(uint _withdrawAmount) public{
     ...
     if (player.withdrawtime > 0){
      ...
      player.activedeptime = player.activedeptime - 10 minutes;
     ...
     //newvalue2 = (newvalue2 *10000) / 10 minutes;
    ...
    function getUserCycle(uint256 _playertime, uint256 _deptime) public view returns (uint256){  
      ...
      if (now>= _playertime){
      cycle = (now.sub(_playertime) / 10 minutes); 
      } else { cycle =0;}
      
      
      if (now>= _deptime){
      depcycle = (now.sub(_deptime) / 10 minutes); 
      
            if(cycle > 25){
                
                uint256 max = ((_playertime+260 minutes) - _deptime);  
                depcycle = (max /10 minutes);  //ESTOY AQUÍ SI SE CONSIGUE LA N CORRECTA, LUEGO HAY QUE MODIFICAR EN GET PROFIT SI N>25 LOS RESULTADOS 
   ...
   /* function getActualCycle() public view returns (uint256){  
    ...
    function getActualActivationCycle() public view  returns (uint256){
      ...
      uint256 cycle = (now.sub(releaseTime) / 10 minutes) + 1;
      uint256 nextactivation = releaseTime + (cycle * 10 minutes);
 ...
 /*  if (n > 25){
    ...
    Player storage player = players[msg.sender];
    require (now > player.time && player.time > 0);
    if (now - player.time > 260 minutes){
   ...
   if (player.activeDeposit > 0){   //zona activos
      ...
      if (now - player.time < 260 minutes){
      passtime = now - player.activedeptime - (a* 10 minutes); 
      } else { passtime = 0;}
    ...
    interestactive = max + ((((passtime *max)/10 minutes)*(player.plus-100)) /100);
   ...
   if (now > player.activation){   //zona pending
     ...
     if (now - player.time < 260 minutes){
    passtime = now - player.activation - (p* 10 minutes);
     } else { passtime = 0;}
    ...
    interest= max + ((((passtime *max)/10 minutes)*(player.plus-100)) /100);
  ...
  de pending...teniendo como referencia el n final.
   ...
   // function realProfit(address _addr) public view returns (uint){
