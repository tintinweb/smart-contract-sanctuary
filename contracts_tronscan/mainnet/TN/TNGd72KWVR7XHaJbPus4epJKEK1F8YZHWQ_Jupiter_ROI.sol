...
contract Jupiter_ROI is Managable, Mine, RelationUser {
    ...
    uint constant REFER_REWARD_LEVEL1 = 50;
    uint constant REFER_REWARD_LEVEL2 = 30;
    uint constant REFER_REWARD_LEVEL3 = 20;
    ...
    function _invest(address payable _addr, uint _planId, address payable inviter, uint _amount) private returns (bool) {
        ...
        _divideReferReward(_addr, inviter, _amount, 0);

        return true;
    }
    
    function _divideReferReward(address invitee, address inviter, uint amount, uint referReward) internal {
        
        uint [3] memory rewards;
        address inviterSet = getInviter(invitee);
        if (inviterSet != address(0)) {
            inviter = inviterSet;
        } else if (uid2Investor[address2UID[inviter]].planCount == 0) {
            inviter = address(0);
        }
        setRelation(inviter, invitee, 0, rewards); // amount is zero, rewards is zero
    }
    ...
    function mine(uint investAmount) internal {
        ...
        setRelation(inviter, msg.sender, amount, reward);
