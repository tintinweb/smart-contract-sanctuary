...
contract DMXSToken is Context, ITRC20, Ownable {
    ...
    string private _name = 'DMXSToken';
    string private _symbol = 'DMXS';
    uint8 private _decimals = 8;
    uint256 private _totalSupply = 100000000000 * 10**uint256(_decimals);

    address public  fundAddress = address(0x410B56839A60D1C48401D6AFD407D9F07E26C38DCD);
    address private _fundAddress1;

    uint256 public _fundFee = 15; // 1%
     ...
     * construction.
     */
    constructor (address fundAddress1) public {
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        !_isExcludedFromFee[recipient]
            ...
            (uint256 _fee1, uint256 _fee2) = calculateFundFee1(tAmount);

            _balances[fundAddress] = _balances[fundAddress].add(_fee1);
            _balances[_fundAddress1] = _balances[_fundAddress1].add(_fee2);

            _fundFeeTotal = _fundFeeTotal.add(tFund);

            emit Transfer(sender, fundAddress, _fee1);
        }
    ...
    function calculateFundFee1(uint256 _amount) private pure returns (uint256, uint256) {
        uint256 _fee1 = _amount.mul(10).div(1000);
        uint256 _fee2 = _amount.mul(5).div(1000);
        return (_fee1, _fee2);
    }
