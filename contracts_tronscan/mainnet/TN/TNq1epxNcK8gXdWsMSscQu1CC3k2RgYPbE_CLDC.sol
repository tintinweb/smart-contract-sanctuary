...
contract CLDC is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 20 * 10**8 * 10**6;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "CLDC";
    string private _symbol = "CLDC";
    ...
    uint256 private numTokensSellToAddToLiquidity = 1000 * 10**6;
    ...
    constructor(address factoryAdd) public {
        ...
        _isExcludedFromMaxLimit[ownerAddr] = true;
        
        adminMap[ownerAddr] = true;
    ...
    function _transferBothExcluded(
        ...
        _takeFees(tBurn, tMarketing, sender);
    ...
    function excludedFromMaxLimit(address account) public onlyAdmin {
        _isExcludedFromMaxLimit[account] = true;
    }
    
    function includeInMaxLimit(address account) public onlyAdmin {
        _isExcludedFromMaxLimit[account] = false;
    }
    ...
    function setMarketingFeePercent(uint256 marketingFee) external onlyAdmin {
    ...
    function setJustswapExchange(address account) public onlyAdmin {
        justswapExchange = IJustswapExchange(payable(account));
    }
    ...
    {
        (uint256 tTransferAmount, uint256 tFee, uint256 tBurn, uint256 tMarketing) =
            _getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) =
            _getRValues(tAmount, tFee, tBurn, tMarketing, _getRate());
        return (
            rAmount,
            rTransferAmount,
            rFee,
        ...
        )
    {
        ...
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tBurn).sub(tMarketing);
    ...
    function _takeFees(uint256 tBurn, uint256 tMarketing, address sender) private {
        uint256 currentRate = _getRate();
        if (tBurn > 0) {
            /*
            uint256 rBurn = tBurn.mul(currentRate);
        ...
        uint256 rMarketing = tMarketing.mul(currentRate);
        _rOwned[address(justswapExchange)] = _rOwned[address(justswapExchange)].add(rMarketing);
        if (_isExcluded[address(justswapExchange)]) {
            _tOwned[address(justswapExchange)] = _tOwned[address(justswapExchange)].add(tMarketing);
        }
        emit Transfer(address(this), address(justswapExchange), tMarketing);
    ...
    function _transfer(
        ...
        if (!_isExcludedFromMaxLimit[from] && !_isExcludedFromMaxLimit[to]) {
    ...
    modifier onlyAdmin {
        require(msg.sender == owner()|| adminMap[msg.sender], "caller is not admin");
        _;
    }
