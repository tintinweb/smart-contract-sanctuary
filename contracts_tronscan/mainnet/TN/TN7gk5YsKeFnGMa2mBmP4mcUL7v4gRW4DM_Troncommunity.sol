//SourceUnit: Troncommunity.sol

pragma solidity 0.5.10;

contract Troncommunity {
    struct User {
        uint256 cycle;
        address upline;
        uint256 referrals;
    ...
    function withdrawCapital (address _addr) external
    {
        (uint256 to_payout, uint256 max_payout) = this.payoutOf(msg.sender);
        require(users[msg.sender].payouts < max_payout, "Full payouts");
        
    
           uint256 withdrawable_capital =  users[msg.sender].deposit_amount * 80 /100;
        if(users[_addr].payouts + withdrawable_capital > max_payout) {
                withdrawable_capital = max_payout - users[_addr].payouts;
            }
     
          msg.sender.transfer(withdrawable_capital);
       
        users[msg.sender].payouts += withdrawable_capital;
        users[msg.sender].total_payouts += withdrawable_capital;
        total_withdraw += withdrawable_capital;
