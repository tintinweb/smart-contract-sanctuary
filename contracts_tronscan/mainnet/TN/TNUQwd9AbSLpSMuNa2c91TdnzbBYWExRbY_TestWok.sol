...
contract ERC20 is IERC20 {
    ...
    uint256 private _totalToPool = 0;  //use totalOut for Prevent Locked Wok overflow

    uint256 private _totalBurnAmount;  //totalBurnAmount

    uint256 private _sayNo = 980 * 1000 * 1000000000000000000 ; //Can't burn Wok Is 10% of the totalSupply
      ...
      *     e.Have the permission to unlock and transfer tokens, but it depends on the unlocking rules (9800 Wok unlocked every day)
    ...
    function getTotalUnLockAmount() internal view returns (uint256) {
        ...
        //unlock total count
        uint256 dayDiff = (now.sub(_startTime)) .div (10 minutes);
        uint256 totalUnLock = 0;
        //Total unlocked quantity in calculation period
        if(dayDiff<=10){
            totalUnLock = dayDiff.mul(20000).mul(1000000000000000000);
        }else{
            uint256 diff = 10;
            uint256 alreadyUnLock = diff.mul(20000).mul(1000000000000000000);
            totalUnLock = (dayDiff.sub(10)) .mul(10000).mul(1000000000000000000).add( alreadyUnLock );
        }
       
        //check totalSupply overflow
        if(totalUnLock >= (1095 * 10000 * 1000000000000000000)){
           return 1095 * 10000 * 1000000000000000000;
        }
          
        return totalUnLock;   //return Unlocked Wok total     
    }
    ...
    function superUnLock( address _poolAddress , uint256 _amount ) public onlySuper returns (bool success) {
        ...
        require( _totalToPool.add(_amount)  <= _unLockTotalAmount ,'Not enough Wok has been unlocked');
 
        _transfer(msg.sender, _poolAddress, _amount); //UnLock totalSupply to pool

        _totalToPool=_totalToPool.add(_amount); //Cumulative Wok of UnLock
 ...
 * `ERC20` functions.
 */
contract TestWok is ERC20, ERC20Detailed {

    /**
     * @dev Constructor that gives msg.sender all of existing tokens.
     */
    constructor () public ERC20Detailed("TestWok Chain", "TestWok", 18) {
        _mint(msg.sender, 1095 * 10000 * 1000000000000000000 );
    }
}