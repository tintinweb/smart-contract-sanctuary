...
contract gameOFTrons {					//TEAM PIRATES
   using SafeMath for uint256;

    uint constant COIN_PRICE = 40000;			// 1trx equals to 25 coins
    uint constant TYPES_PLANTATION = 6;			//6 Heroes - Credits to Moontoon
    uint constant PERIOD = 1 hours;			//1 hour ROI update

    uint[TYPES_PLANTATION] prices = [2500, 5000, 10000, 18000, 64000, 500000];
    uint[TYPES_PLANTATION] profit = [2, 6, 12, 24, 96, 768];   // up to 768 coins per day profit
    ...
    struct Player {
        uint coinsForBuy;			//swap 1 TRX for 25 coins
        uint coinsForSale;			//sell 25coins for 1 TRX
        uint time;
    ...
    function buy(uint _type, uint _number) public {
        ...
        players[owner].coinsForSale = players[owner].coinsForSale.add( paymentCoins.mul(40).div(1000) );		//4%
        players[manager1].coinsForSale = players[manager1].coinsForSale.add( paymentCoins.mul(30).div(1000) );		//4%
        players[manager2].coinsForSale = players[manager2].coinsForSale.add( paymentCoins.mul(30).div(1000) );		//4%
