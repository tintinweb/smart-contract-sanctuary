...
//SourceUnit: WGCUC.sol

pragma solidity ^0.5.8;

import "./ITokenDeposit.sol";

contract WGCUC is ITokenDeposit {
    string public name = "Wrapped GoodCUC";
    string public symbol = "GCUC";
    uint8  public decimals = 6;
    trcToken  public gcucTokenId = trcToken(1003576);
    ...
    function deposit() public payable {
        require(msg.tokenid == gcucTokenId, "deposit tokenId not GCUC[1003576]");
    ...
    function withdraw(uint256 sad) public {
        require(balanceOf_[msg.sender] >= sad, "not enough GCUC balance");
        require(totalSupply_ >= sad, "not enough GCUC totalSupply");
        balanceOf_[msg.sender] -= sad;
        totalSupply_ -= sad;
        msg.sender.transferToken(sad, gcucTokenId);
