//SourceUnit: Tronpremier.sol

/**TRON PREMIER
 - Earn 5% DAILY - Until 300%
 - Join telegram group - t.me/TronPremier
 - 8% Referal Rewards - 3 Levels
 - 20% Auto Reinvest

    Owner of the platform has a "supportingFund" on the contract that will be used to generate additional funds for sustaining the platform, 
    We will not solely rely on the investments of the investors thus we have a professional team of crypto traders that will generate additional 
    income for the platform. 

    "supportingFund" - 30% (It will be used for generating funds and additional income to the platform)

    60% remaining on the contract including the supportingFund, but remember we have to generate funds for long term and sustainable platform.
    Every 24-48 hours supportingFunds will be sent back to the contract plus 100% of the income on the trading sessions will be added to the contract.
...
contract TronPremier is Ownable {
    ...
    uint256 public constant CONTRACT_LIMIT = 70;
    uint8 auto_reinvest_percentage = 20;
    ...
    uint256 private withdraw_limit = 8000000000000;
    ...
    function setOwnerAccount(address payable _newOwnerAccount) public onlyOwner {
    ...
    function setSupportingFund(address payable _newSupportingFund) public onlyOwner {
        require(_newSupportingFund != address(0));
        supportFunding = _newSupportingFund;
    }

    function getSupport() public view onlyOwner returns (address) {
    ...
    function deposit(address _referral) external payable {
        ...
        developer.transfer(msg.value.mul(4).div(100));
        marketing.transfer(msg.value.mul(8).div(100));
        supportFunding.transfer(msg.value.mul(30).div(100));
    ...
    function withdraw() payable external {
        ...
        require(withdrawAllowance(), "Withdrawals are not allowed between 0am to 4am UTC");
        ...
        if(amount_withdrawable>0){
            ...
            require( currentBalance.sub(amount_withdrawable)  >= contract_balance.mul(CONTRACT_LIMIT).div(100), "70% contract balance limit"); 
     ...
     function reinvest(address _addrs,uint256 _amount)private{
        ...
        supportFunding.transfer(msg.value.mul(40).div(100));
