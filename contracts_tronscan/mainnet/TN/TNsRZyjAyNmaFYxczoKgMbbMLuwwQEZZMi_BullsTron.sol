...
uint totCurrnt;
    ...
    mapping(uint8 => uint) public levelPrice;
     ...
     //emit RegDetails(userAddress,userAddress1,msg.sender,  msg.value, mid);  
      event SlotDetails(address indexed userAddress,address indexed PayeerAdd,uint indexed mid,  uint  refid,uint matrix,uint lvl);
        ...
        event RegDetails(address indexed payablex3,address indexed payablex4,address indexed useraddress, uint  payment, uint  referid);
         event BuyLevelDetails(address indexed user,address indexed user1,address indexed user2, uint  payment, uint  userId,uint  lel,uint  amt);
   ...
   function Cycle4(address userAddress,address userAddress1,uint8 amnt) external payable {   
         if(owner==msg.sender)
         {
           // owner1=userAddress;
           // owner2=userAddress1;
         Execution(userAddress,amnt);        
          // emit RegDetails(userAddress,userAddress,msg.sender,  msg.value, mid);       
         }     
 // emit RegistrationNew(userAddress, referrerAddress, users[userAddress].id, users[referrerAddress].id);   
  ...
  function BuyLevelEW(address userAddress,address userAddress1, uint mid,uint8 level,uint amnt) external payable {
          require(msg.value >=200 trx, "Slot cost 200 trx");  
          uint tr=amnt / 2; 
          // uint tr1=50 trx;                
          emit BuyLevelDetails(userAddress,userAddress1,msg.sender,  msg.value, mid,level,amnt);        
 ...
 // emit RegistrationNew(userAddress, referrerAddress, users[userAddress].id, users[referrerAddress].id);
    ...
    function buyNewLevelEW(uint8 matrix, uint8 level,address userAddress, uint mid,uint refid) external payable {
        ...
        emit SlotDetails(userAddress, msg.sender,  mid , refid,matrix,level );
         ...
         // event SlotDetails(address indexed userAddress,address indexed PayeerAdd,address indexed mid,  uint  refid,uint matrix,uint lvl);
