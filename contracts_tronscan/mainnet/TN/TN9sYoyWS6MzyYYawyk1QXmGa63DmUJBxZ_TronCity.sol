...
contract TronCity {
    ...
    function userInfo(address _userId) public view returns(uint _id, address _refBy, uint _refs, uint _earnings,
        uint _withdrawn, uint _pool_bonus, uint _dre, uint _mxe, uint _mce, uint _highestStage, uint _teamVolume){
        uint p = 12;
        for(p - 1; p > 0; p--){
            if(users[_userId].CityMatrix[p].active){
                break;
        ...
        user.refBy,
        user.refsCount,
        user.finances[0].earnings,
        user.finances[0].withdrawn,
        user.finances[0].pool_bonus,
        user.finances[0].dreEarnings,
        user.finances[0].matrixEarnings,
        user.finances[0].matchingBonus,
        p,
        user.finances[0].teamvolume
        );
    ...
    function stats() public view returns(uint lId, uint rT, uint rTl, uint dE, uint lE, uint mI, uint mCI, uint jT, uint pB){
        return (lastUserId, raisedToday, raisedTotal, distEarning, lostEarnings, matrixIncomes, matchIncomes, joinedToday, weeklyPool);
    }
