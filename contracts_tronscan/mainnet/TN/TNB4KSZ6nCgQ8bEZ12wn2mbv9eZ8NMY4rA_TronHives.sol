...
contract TronHives is Claimable, UserBonus {
    ...
    uint256[BEES_COUNT] public BEES_PRICES = [0e6, 1500e6, 7500e6, 30000e6, 75000e6, 250000e6, 750000e6, 150000e6];
    uint256[BEES_COUNT] public BEES_LEVELS_PRICES = [0e6, 0e6, 11250e6, 45000e6, 112500e6, 375000e6, 1125000e6, 0];
    uint256[BEES_COUNT] public BEES_MONTHLY_PERCENTS = [0, 300, 310, 320, 330, 340, 350, 500];
    uint256[MEDALS_COUNT] public MEDALS_POINTS = [0e6, 50000e6, 190000e6, 510000e6, 1350000e6, 3225000e6, 5725000e6, 8850000e6, 12725000e6, 23500000e6];
    uint256[MEDALS_COUNT] public MEDALS_REWARDS = [0e6, 3500e6, 10500e6, 24000e6, 65000e6, 140000e6, 185000e6, 235000e6, 290000e6, 800000e6];
    uint256[QUALITIES_COUNT] public QUALITY_HONEY_PERCENT = [50, 52, 54, 56, 58, 60];
    uint256[QUALITIES_COUNT] public QUALITY_PRICE = [0e6, 15000e6, 50000e6, 120000e6, 250000e6, 400000e6];

    uint256 public constant COINS_PER_TRX = 25;
    uint256 public constant MAX_BEES_PER_TARIFF = 32;
    uint256 public constant FIRST_BEE_AIRDROP_AMOUNT = 500e6;
    ...
    uint chanceX025 = 23260;
    uint chanceX05 = 46520;
    uint chanceX075 = 69780;
    uint chanceX2 = 84070;
    uint chanceX3 = 90740;
    uint chanceJackpot = 91180;
    uint256 public winnerChance;
    uint256 jackpot = 555;
    uint256 public balanceOfLottery;
    bool lotteryPaused = true;
    ...
    uint256 public leaderReferrals;
    ...
    event LotteryPlayed(address indexed user, uint256 amountWax, uint256 amountHoney, uint256 chance, uint256 winnings);
    ...
    function changeSuperBeeStatus() public returns(bool) {
      ...
      if (address(this).balance >= maxBalance.mul(100 + SUPERBEE_PERCENT_LOCK).div(100)) {
    ...
    function deposit(address ref) public payable payRepBonusIfNeeded {
        ...
        uint256 wax = msg.value.mul(COINS_PER_TRX);
        ...
        uint256 _maxBalance = Math.max(maxBalance, address(this).balance.add(adminWithdrawed));
        if (_maxBalance >= maxBalance.mul(100 + SUPERBEE_PERCENT_LOCK).div(100)) {
          isSuperBeeUnlocked = false;
        }
    ...
    function withdraw(uint256 amount) public {
        ...
        uint256 value = amount.div(COINS_PER_TRX);
    ...
    function _distributeFees(address user, uint256 wax, uint256 deposited, address refAddress) internal {
        ...
        address(uint160(owner())).transfer(wax * ADMIN_PERCENT / 100 / COINS_PER_TRX);
    ...
    function _register(address user, address refAddress) internal {
        ...
        if (refAddress != address(0)) {
            ...
            if (players[refAddress].referrals.length > leaderReferrals) {
                leaderReferrals++;
            }
    ...
    function _addToBonusIfNeeded(address user) internal {
        if (user != address(0) && !bonus.userRegistered[user]) {
            ...
            if (player.totalDeposited >= 25000 trx &&
                player.referrals.length >= 10 &&
                player.referralsTotalDeposited >= 250000 trx)
            {
    ...
    function _payWithWaxAndHoneyForLottery(address user, uint256 amount) internal returns(uint256) {
      Player storage player = players[user];

      uint256 wax = Math.min(amount, player.balanceWax);
      uint256 honey = amount.sub(wax);

      _payWithWaxOnly(user, wax);
      _payWithHoneyOnly(user, honey);

      return honey;
    }

    function lottery(uint256 wax) public returns(uint256) {
        require(!lotteryPaused, "Lottery is paused");
        require(wax >= 500 trx, "Minimum rate 500 wax");

        Player storage player = players[msg.sender];

        uint256 honey = _payWithWaxAndHoneyForLottery(msg.sender, wax);
        uint random = uint(keccak256(abi.encodePacked(now, msg.sender, randNonce))) % _modulus;
        randNonce++;
        uint256 chanceWon;
        chanceWon = (random >= 0 && random < chanceX025) ? 25 : (random >= chanceX025 && random < chanceX05) ? chanceWon = 5 : (random >= chanceX05 && random < chanceX075) ? 75 : (random >= chanceX075 && random < chanceX2) ? 2 : (random >= chanceX2 && random < chanceX3) ? 3 : (random >= chanceX3 && random < chanceJackpot) ? jackpot : 0;
        winnerChance = chanceWon;
        collect();
        if (chanceWon != jackpot) {
          uint256 prize = (chanceWon == 25 || chanceWon == 75) ? wax.mul(chanceWon).div(100) : chanceWon == 5 ? wax.mul(chanceWon).div(10) : wax.mul(chanceWon);
          if (prize < wax) {
            uint256 remainder = wax.sub(prize);
            balanceOfLottery = balanceOfLottery.add(remainder);
          }
          player.balanceWax = player.balanceWax.add(prize);

          emit LotteryPlayed(msg.sender, wax.sub(honey), honey, prize, chanceWon);
        } else {
          player.balanceWax = player.balanceWax.add(balanceOfLottery);
          emit LotteryPlayed(msg.sender, wax.sub(honey), honey, balanceOfLottery, chanceWon);
          balanceOfLottery = wax;
        }
