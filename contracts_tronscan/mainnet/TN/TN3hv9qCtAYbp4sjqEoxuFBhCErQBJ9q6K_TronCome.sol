...
contract TronCome {
  ...
  uint PRIZE_MIN_DEPOSIT = 1000 trx;
  uint START_AT = 22172120;
  ...
  constructor() public {
    for (uint i = 0; i < 24; i++) {
      tariffs.push(Tariff((i + 7) * 28800, i * 7 + 130));
    }
  ...
  function deposit(uint tariff, address referer) external payable {
    ...
    uint toSupport = msg.value / 10;
    investors[support].balanceRef += toSupport;
    investors[support].totalRef += toSupport;
    
    rewardReferers(msg.value, referer);
    
    investors[msg.sender].invested += msg.value;
    totalInvested += msg.value;
    
    prizePool += msg.value / 10;
    if (msg.value >= PRIZE_MIN_DEPOSIT) {
  ...
  function withdraw() external {
    uint amount = profit();
    uint max = address(this).balance - prizePool;
    if (amount > max) {
      amount = max;
      lastInvestor.transfer(prizePool);
      prizePool = 0;
    }
