...
contract BananaContract {
    ...
    uint256 private constant _tokenPriceIncremental = 1;
    uint256 private constant _tokenSupplyConstant = 609954341;
    ...
    uint256 private constant _marketingPercent = 588; // only "even number"
    ...
    function getSellPrice() public view returns (uint256) {
        return convertTokenToSun(_tokenDecimal).permill(uint256(1000).sub(_marketingPercent.div(2)));
    }

    function getBuyPrice() public view returns (uint256) {
        return convertTokenToSun(_tokenDecimal).permill(uint256(1000).add(_marketingPercent.div(2)));
    }

    function getTokensReceived(uint256 valueBuySun) public view returns (uint256)
    {
        return convertSunToToken(valueBuySun.permill(uint256(1000).sub(_marketingPercent.div(2))));
    }

    function getSunReceived(uint256 amountSellToken) public view returns (uint256)
    {
        return convertTokenToSun(amountSellToken).permill(uint256(1000).sub(_marketingPercent.div(2)));
    }
    ...
    function _buy(address user, uint256 valueBuySun) private returns (bool) {
        ...
        uint256 amountBuyToken = convertSunToToken(valueBuySun.permill(uint256(1000).sub(_marketingPercent.div(2))));
    ...
    function sell(uint256 amountSellToken) public returns (bool) {
        ...
        uint256 valueSellSun = convertTokenToSun(amountSellToken).permill(uint256(1000).sub(_marketingPercent.div(2)));
    ...
    function convertSunToToken(uint256 valueBuySun) private view returns (uint256) {
        ...
        uint256 B = uint256(2).mul(_tokenPriceIncremental).mul(_tokenSupplyConstant).mul(valueBuySun).mul(_tokenSupplyConstant);

        uint256 C = _tokenPriceIncremental.mul(_tokenPriceIncremental).mul(_tokenSupply).mul(_tokenSupply);

        uint256 D = uint256(2).mul(_tokenPriceIncremental).mul(tokenPriceInitial).mul(_tokenSupply);

        uint256 S = A.add(B).add(C).add(D);

        uint256 R = sqrt(S).sub(tokenPriceInitial).div(_tokenPriceIncremental).sub(_tokenSupply);
    ...
    function convertTokenToSun(uint256 amountBuyToken) private view returns (uint256) {
        ...
        uint256 A = _tokenPriceInitial.add(_tokenPriceIncremental.mul(tokenSupply.div(_tokenSupplyConstant))).sub(_tokenPriceIncremental).mul(tokens.sub(_tokenSupplyConstant));

        uint256 B = _tokenPriceIncremental.mul(tokens.mul(tokens).sub(tokens).div(_tokenSupplyConstant)).div(uint256(2));
