...
contract TronLoop {
  ...
  uint256 public startTime = 1603720800;
  ...
  function register(address forAddress) internal {
    lastUserId ++;
  ...
  function join(uint8 _pool) payable public{
    ...
    require(poolCosts[_pool-1] == msg.value || msg.sender==operator, "Invalid TRX amount");
  ...
  function _poolPay(uint8 _pool) internal{
    ...
    if(PoolMembers[_pool][indx].paymentsCount >= 2){
      ...
      users[PoolMembers[_pool][indx].userAddress].pools[_pool].earned += poolCosts[_pool-1];
