...
contract MonoFarm is Ownable{
    ...
    ITRC20 public wyzthTOKEN;
    ITRC20 public ULETOKEN;
    ...
    uint256 constant public minimumDeposit = 100E18;
    ...
    function farm(uint256 _amount, uint256 _lockableDays) public {
        ...
        wyzthTOKEN.transferFrom(msg.sender, address(this), _amount);
        user.farmedToken = _amount;
        user.lastUpdated = now;
        user.lockableDays = _lockableDays;
    }

    function pendindRewards() public view returns (uint256 wyzthReward, uint256 uleReward) {
        userInfo storage user = users[msg.sender];
        uint256 leftout = now.sub(user.lastUpdated);
        uint256 toDays = leftout.div(1 days);
        if (users[msg.sender].lockableDays > toDays) return (0,0);
        else
            wyzthReward = allocation[users[msg.sender].lockableDays]
                .mul(users[msg.sender].farmedToken)
                .div(10000).mul(user.lockableDays);
    ...
    function harvest() public {
        ...
        if(wyzthReward > 0 && uleReward > 0) {
            uint256 amounttoTransferred = user.farmedToken.add(wyzthReward);
    ...
    function safeTransferWYZ(address _farmer, uint256 _amount) internal {
        uint256 tokenBal = wyzthTOKEN.balanceOf(address(this));
        if (_amount > tokenBal) {
            wyzthTOKEN.transfer(_farmer, tokenBal);
        } else {
            wyzthTOKEN.transfer(_farmer, _amount);
        }
    }
    
    function safeTransferULE(address _farmer, uint256 _amount) internal {
        uint256 tokenBal = ULETOKEN.balanceOf(address(this));
        if (_amount > tokenBal) {
            ULETOKEN.transfer(_farmer, tokenBal);
        } else {
            ULETOKEN.transfer(_farmer, _amount);
        }
    }
    
    // EMERGENCY ONLY.
    function emergencyWithdraw(uint256 wyzthAmount, uint256 uleAmount) public onlyOwner {
        safeTransferWYZ(msg.sender, wyzthAmount);
        safeTransferULE(msg.sender, uleAmount);
    }
}