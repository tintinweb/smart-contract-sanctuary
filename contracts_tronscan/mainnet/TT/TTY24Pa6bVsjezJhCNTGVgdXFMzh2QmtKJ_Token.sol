//SourceUnit: ERC20 - 0.6.0通缩合约 - 2 - 3发推荐.sol
...
contract Token is Context, IERC20, Ownable {
    ...
    string private _name = "DYFT";
    string private _symbol = "DYFT";
    uint8 private _decimals = 6;

    address private _burnPool = 0x000000000000000000000000000000000000dEaD;

    address private fundAddress =
        address(0xFDEe2802ed2A74f5b7a25E0a26c09490FD02D14b);
    ...
    function _transfer(
        ...
        if (_tTotal <= MAX_STOP_FEE_TOTAL) {
            ...
            bool exclude = _isExcludedFromFee[sender] ||
                _isExcludedFromFee[recipient];

            if (exclude) removeAllFee();

            _transferStandard(sender, recipient, amount);

            if (exclude) restoreAllFee();
        }
    }
