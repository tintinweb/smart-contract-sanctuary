...
contract Gold is TRC20 {
    ...
    function beginGame(uint256 firstDivsTime) external payable {
        ...
        nextGoldDepositSnapshotTime = firstDivsTime + 2 minutes;
    ...
    function buyTronUnit(uint256 unitId, uint256 amount) external payable SnapshotCheck {
        ...
        uint256 devFund = SafeMath.div(SafeMath.mul(tronCost, 2), 100); // % for developer
    ...
    function fundGoldResearch(uint256 amount) external SnapshotCheck {
        //require(msg.value >= 1000000); // Min. 1M Micrograms or 1 GOLD
    ...
    function snapshotDailyGoldDepositFunding() public {
        //require(msg.sender == owner);
	//require(totalGoldDepositSnapshots >0);
        ...
        nextGoldDepositSnapshotTime = block.timestamp + 5 minutes;
	//resetGame(0);
