...
interface IStakingRewards {
    ...
    function withdraw(uint256 amount, uint256 amount2) external;
...
contract RewardsDistributionRecipient {
    ...
    function notifyRewardAmount(uint256 reward, uint256 noOFdays) external;
    ...
    ReentrancyGuard
{
    ...
    struct stakeInfo {
        ...
        uint256 totalunstaked2;
        uint256 gainreward;
        uint256 referalreward;
        uint256 earnreferalreward;
        bool stake;
        address ref;
        bool rewardlock;
        bool referallock;
    ...
    function calcRate(uint256 totalSupply) external view returns (uint256) {
        return
            rewardPerTokenStored.add(
                lastTimeRewardApplicable()
                    .sub(lastUpdateTime)
    ...
    function stake(
        ...
        //amount
        uint256 deposit_fee = amount.mul(fee) / 100;
        ...
        stakingToken.safeTransferFrom(
            msg.sender,
            address(this),
            amount.add(deposit_fee)
        );

        //amount2
        if (amount2 > 0) {
            uint256 deposit_fee = amount2.mul(fee) / 100;
            depositFee[rewardsDistribution] += deposit_fee;
            amount2 = amount2.sub(deposit_fee);
            _totalSupply2 = _totalSupply2.add(amount2);
            _balances2[msg.sender] = _balances2[msg.sender].add(amount2);
            rewardsToken.safeTransferFrom(
                msg.sender,
                address(this),
                amount2.add(deposit_fee)
            );
        } else {
            amount2 = 0;
        }
        ...
        if (stakeInfos[msg.sender].stake == true) {
            ...
            stakeInfos[msg.sender].totalStaked2 += amount2;
            totalstakeuser++;
        } else {
            stakeInfo memory StakeInfo;
            StakeInfo = stakeInfo({
                ...
                totalStaked2: stakeInfos[msg.sender].totalStaked2.add(amount2),
                totalunstaked: 0,
                totalunstaked2: 0,
                gainreward: 0,
                referalreward: 0,
        ...
        emit Staked(msg.sender, amount, amount2);
        emit DepositFee(address(0), deposit_fee);
    }

    function withdraw(uint256 amount, uint256 amount2)
        public
        nonReentrant
        updateReward(msg.sender)
    {
        ...
        if (amount2 != 0) {
            _totalSupply2 = _totalSupply2.sub(amount2);
            _balances2[msg.sender] = _balances2[msg.sender].sub(amount2);
            rewardsToken.safeTransfer(msg.sender, amount2);
            stakeInfos[msg.sender].totalunstaked2 += amount2;
        } else {
            amount2 = 0;
        }
        emit Withdrawn(msg.sender, amount, amount2);
    }
    ...
    function getReward() public nonReentrant updateReward(msg.sender) {
        ...
        if (reward > 0 && stakeInfos[msg.sender].rewardlock == false) {
            ...
            uint256 rewardamt = reward.mul(rewardpercentage) / 100;
            uint256 amount = reward.sub(rewardamt);
    ...
    function exit() external {
        withdraw(_balances[msg.sender], _balances2[msg.sender]);
        getReward();
    }

    function lockunlockreward(address lockaddress, bool lock)
        external
    ...
    function lockunlockreferal(address lockaddress, bool lock)
        external
        onlyRewardsDistribution
    {
    ...
    function notifyRewardAmount(uint256 reward, uint256 noOFdays)
        external
        onlyRewardsDistribution
        updateReward(address(0))
    {
        rewardsDuration = noOFdays * 86400;
    ...
    function getRewardamount(address owneraddr, uint256 amount)
        external
        onlyRewardsDistribution
        updateReward(address(0))
    {
    ...
    function getFeeWithdrawal(address _owner) external onlyRewardsDistribution {
        uint256 feeamount = depositFee[msg.sender];
    ...
    event Staked(address indexed user, uint256 amount, uint256 amount2);
    event Withdrawn(address indexed user, uint256 amount, uint256 amount2);
