...
contract GlobalsAndUtility is ERC20 {
    ...
    */
    ...
    /* ERC20 constants */
    string public constant name = "PMA";
    string public constant symbol = "PMA";
    uint8 public constant decimals = 8;

    /* Suns per Satoshi = 10,000 * 1e8 / 1e8 = 1e4 */
    uint256 private constant SUNS_PER_PMA = 10 ** uint256(decimals); // 1e8
    ...
    uint256 private constant BPB_MAX_PMA = 7 * 1e6;
    uint256 internal constant BPB_MAX_SUNS = BPB_MAX_PMA * SUNS_PER_PMA;
        ...
        external
    {
        ...
        require(g._currentDay > CLAIM_PHASE_START_DAY, "PMA: Too early");

        if (beforeDay != 0) {
            require(beforeDay <= g._currentDay, "PMA: beforeDay cannot be in the future");
    ...
    {
        require(beginDay < endDay && endDay <= globals.dailyDataCount, "PMA: range invalid");
        ...
        view
    {
        ...
        require(stakeIdParam == stRef.stakeId, "PMA: stakeIdParam not in stake");
...
contract StakeableToken is GlobalsAndUtility {
        ...
        external
    {
        ...
        require(newStakedDays >= MIN_STAKE_DAYS, "PMA: newStakedDays lower than minimum");
        ...
        external
    {
        ...
        require(stakeLists[stakerAddr].length != 0, "PMA: Empty stake list");
        require(stakeIndex < stakeLists[stakerAddr].length, "PMA: stakeIndex invalid");
        ...
        require(g._currentDay >= st._lockedDay + st._stakedDays, "PMA: Stake not fully served");

        /* Stake must still be locked */
        require(st._unlockedDay == 0, "PMA: Stake already unlocked");
        ...
        external
    {
        ...
        /* require() is more informative than the default assert() */
        require(stakeListRef.length != 0, "PMA: Empty stake list");
        require(stakeIndex < stakeListRef.length, "PMA: stakeIndex invalid");
        ...
        internal
    {
        ...
        require(newStakedDays <= MAX_STAKE_DAYS, "PMA: newStakedDays higher than maximum");
        ...
        require(newStakeShares != 0, "PMA: newStakedSuns must be at least minimum shareRate");
        ...
        returns (uint256 bonusSuns)
    {
        /*
            ...
            Bonus percentage scaled 0% to 10% for the first 7M PMA of stake.
...
contract TransformableToken is StakeableToken {
        ...
        payable
    {
        ...
        require(rawAmount != 0, "PMA: Amount required");
    ...
    function xfLobbyExit(uint256 enterDay, uint256 count)
        external
    {
        require(enterDay < _currentDay(), "PMA: Round is not complete");
        ...
        if (count != 0) {
            require(count <= qRef.tailIndex - headIndex, "PMA: count invalid");
            endIndex = headIndex + count;
        } else {
            endIndex = qRef.tailIndex;
            require(headIndex < endIndex, "PMA: count invalid");
        }
    ...
    function xfFlush()
        external
    {
        require(address(this).balance != 0, "PMA: No value");

        require(LAST_FLUSHED_DAY < _currentDay(), "PMA: Invalid day");
        ...
        returns (uint256 rawAmount, address referrerAddr)
    {
        ...
        require(entry.rawAmount != 0, "PMA: Param invalid");
    ...
    {
        /* 1095890410958 = ~ 4000000 * SUNS_PER_PMA / 365 */
...
contract PMA is TransformableToken {
    constructor()
        public
    {
