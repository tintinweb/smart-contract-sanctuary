...
contract Token is Context, IERC20, Ownable {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if (_totalSupply <= MAX_STOP_FEE_TOTAL) {
            ...
            _transferStopBurn(sender, recipient, amount);
            restoreAllFee();
        } else {
            if(
                _isExcludedFromFee[sender] || 
    ...
    function _transferStopBurn(address sender, address recipient, uint256 tAmount) private {

        (uint256 tTransferAmount, uint256 tLiquidity, uint256 tFund) = _getStopBurnValues(tAmount);
    ...
    function _getStopBurnValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tLiquidity, uint256 tFund) = _getStopBurnTValues(tAmount);
    ...
    function _getStopBurnTValues(uint256 tAmount) private view returns (uint256, uint256,uint256) {
        //uint256 tBurn = calculateBurnFee(tAmount);
        uint256 tLiquidity = calculateLiquidityFee(tAmount);
        uint256 tFund = calculateFundFee(tAmount);
    ...
    function removeBurnFee() private {
        if(_liquidityFee == 4 && _burnFee == 0 && _fundFee == 2) return;
        _previousLiquidityFee = _liquidityFee;
        _previousBurnFee = _burnFee;
        _previousFundFee = _fundFee;
        _liquidityFee = 4;
