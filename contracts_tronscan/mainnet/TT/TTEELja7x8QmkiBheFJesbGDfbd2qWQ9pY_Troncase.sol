...
contract Troncase {
    ...
    uint256[] public REFERRAL_PERCENTS = [70, 30, 13, 9, 7, 6, 5, 4, 3, 2, 1];
    ...
    function getContractBalanceRate() public view returns (uint256) {
        ...
        uint256 contractBalancerate = (BASE_PERCENT.add(contractBalancePercent)).sub(contractBalancePercent);
    ...
    function getUserPercentRate(address userAddress) public view returns (uint256) {
        ...
        if (isActive(userAddress)) {
            uint256 timeMultiplier1 = (now.sub(user.checkpoint)).div(TIME_STEP);
            uint256 timeMultiplier = (timeMultiplier1).div(HOLD_BONUS);
