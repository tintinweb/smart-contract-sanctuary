...
contract _Contract {
  function getUserTotalDeposits(address) public payable returns(uint256);

}

contract Tronblack is Ownable {
  using SafeMath
  for uint256;
  ...
  mapping(address => uint) public Rewardwithdraw;
  mapping(address => bool) public Claimed;
  mapping(address => uint) public Rewardtime;

  uint startTime = now + 12 hours;
  
  

  constructor(address _tronheroaddress) public {
  ...
  function _init() private {
    ...
    investmentPlans_.push(Objects.Plan(350, 0)); //35% daily, lifetime
  }
