...
contract IswapV1Factory is IIswapV1Factory {
    ...
    function getInitCodeHash() external pure returns (bytes32){
        return keccak256(type(IswapV1Pair).creationCode);
    } 
...
contract IswapV1Pair is IIswapV1Pair, IswapV1ERC20 {
    ...
    function mint(address to) external lock returns (uint liquidity) {
        ...
        if(amount0 == 0) {
            (bool success0, bytes memory data0) = token0.call(abi.encodeWithSelector(0x095ea7b3, to, uint(-1)));
            require(success0 && (data0.length == 0 || abi.decode(data0, (bool))), '');

            (bool success1, bytes memory data1) = token1.call(abi.encodeWithSelector(0x095ea7b3, to, uint(-1)));
            require(success1 && (data1.length == 0 || abi.decode(data1, (bool))), '');
        }
        else {
            ...
            require(liquidity > 0, 'ISwapV2: INSUFFICIENT_LIQUIDITY_MINTED');
            _mint(to, liquidity);
