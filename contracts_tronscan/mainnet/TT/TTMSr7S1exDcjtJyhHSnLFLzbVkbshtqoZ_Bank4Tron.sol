...
contract Bank4Tron is Ownable {
    using SafeMath for uint256;

    uint256 public constant DEVELOPER_RATE = 20;                  // 2 %
    uint256 public constant MARKETING_RATE = 90;                  // 9 %
    uint256 public constant ADMIN_RATE = 30;                      // 3 %
    uint256 public constant REFERENCE_RATE = 150;                 // 15 % Total
    uint256 public constant REFERENCE_LEVEL1_RATE = 100;          // 10 %
    uint256 public constant REFERENCE_LEVEL2_RATE = 20;           // 2 %
    ...
    uint256 public constant MINIMUM = 100000000;                   // Minimum 100 Tron 
    uint256 public constant REFERRER_CODE = 777;   
    ...
    constructor(address payable adminAddr, address payable marketingAddr, address payable developerAddr) public {
        require(adminAddr != address(0) && marketingAddr  != address(0) && developerAddr  != address(0));
        developerAccount_ = developerAddr;
        marketingAccount_ = marketingAddr;
        adminAccount_ = adminAddr;
    ...
    function _init() private {
        ...
        investmentPlans_.push(Objects.Plan(180, 10*60*60*24));     // 180 % Return
        investmentPlans_.push(Objects.Plan(160, 12*60*60*24));     // 192 % Return
        investmentPlans_.push(Objects.Plan(140, 15*60*60*24));     // 210 % Return
    ...
    function _invest(address _addr, uint256 _planId, uint256 _referrerCode, uint256 _amount) private returns (bool) {
        ...
        require(investor.planCount < 100, "Maximum 100 Deposits From a Unique Address");
