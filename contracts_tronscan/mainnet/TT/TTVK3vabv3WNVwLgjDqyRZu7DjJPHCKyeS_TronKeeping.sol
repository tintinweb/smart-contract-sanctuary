//SourceUnit: tronkeeping.sol

pragma solidity 0.7.6;

contract TronKeeping {
    using SafeMath for uint;
	uint256 constant public PERCENTS_DIVIDER = 1000;
    uint8[] public REFERRAL_LEVEL = [5, 3, 2, 1, 1];
    uint256 public constant MIN_INVEST = 100 trx;
    uint8 public constant PARTICIPATION_DAYS = 200;
    uint256 public constant PARTICIPATION_PERCENT = 200;
    ...
    function userInfo(address _addr) view external returns(uint256 for_withdraw, uint256 withdrawable_referral_bonus, uint256 invested, uint256 withdrawn, uint256 referral_bonus, uint256[8] memory referrals, uint256 _last_withdrawal) {
        ...
        for(uint256 i = 0; i < user.invests.length; i++) {
            ...
            if(from < to) {
                payout += dep.amount * (to - from) * PARTICIPATION_PERCENT / PARTICIPATION_DAYS / 17280000;
            }
        }
    ...
    function withdraw() public {
        ...
        require(user.invests.length < 150000, "Max 150000 deposits per address");
        ...
        if(payout > 0) {
            ...
            if(from < to) {
                user.invests[i].total_withdraw += dep.amount * (to - from) * PARTICIPATION_PERCENT / PARTICIPATION_DAYS / 17280000;
            }
        }
