...
contract BetGame is Ownable {
    ...
    // Info of each user.
    struct UserInfo {
        uint256 amount;     // How many tokens the user has bet.
        uint256 totalAmount; // total tokens user bet;
    ...
    mapping(address => UserInfo) userBets;

    event UserBet(address indexed user, uint256 amount);
    event Withdraw(address indexed user, uint256 amount);


    function bet(uint256 amount) public payable returns (uint256 ){

        require(amount>0,'bet amount must large than 1');

        UserInfo storage userBet = userBets[msg.sender];

        require(address(token).safeTransferFrom(msg.sender,address(this),amount));
        userBet.amount = userBet.amount.add(amount);
        emit UserBet(msg.sender,amount);
        return amount;
    }

    function withdrawWin(uint256 amount) public  returns (uint256 ){

        require(amount>0,'bet amount must large than 1');

        UserInfo storage userBet = userBets[msg.sender];
        require(userBet.amount>=amount,'not enough money for user');

        userBet.amount = userBet.amount.sub(amount);
