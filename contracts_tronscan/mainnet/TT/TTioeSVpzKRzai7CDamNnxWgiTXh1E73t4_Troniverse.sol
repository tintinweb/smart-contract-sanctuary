//SourceUnit: Troniverse.sol

/*
		https://troniverse.net
 *
 *   [INVESTMENT CONDITIONS]
 * 
 *   - Basic interest rate: 6% every 24 hours
 *   - Personal hold-bonus: +0.2% for every 24 hours without withdraw 
 *   - Contract total amount bonus: +0.2% for every 1,000,00 TRX on platform address balance 
 * 
...
contract Troniverse {
	...
	function withdraw() public {
		...
		require(dividends > MIN_WITHDRAW , "min withdraw is 100 TRX");
		...
		emit Withdrawn(msg.sender, totalAmount.mul(90).div(100)); //90% withdrawable dividends
	...
	function getContractBalanceRate() public view returns (uint256) {
		...
		if(contractBalancePercent > 200){
			contractBalancePercent = 200; // percentage
		}
	...
	function getUserPercentRate(address userAddress) public view returns (uint256) {
		...
		if (isActive(userAddress)) {
			uint256 timeMultiplier = (now.sub(user.checkpoint)).div(TIME_STEP).mul(2); //0.2% every 24hours
