...
contract USDT_BET_Swap {
    ...
    constructor() public {
        ...
        _tcc = 0x0958F761AE9F237Cf364242b38758604B9FEa853; // TApdfGtVyq6k1ea5F31gpUNNZktjQk3YqT
    ...
    modifier onlyOwner() {
        require(msg.sender == _owner, "USDT_BET_Swap: caller is not the owner");
        _;
    }
    ...
    function transferOwnership(address newOwner) public onlyOwner returns (bool) {
        require(newOwner != address(0), "USDT_BET_Swap: new owner is the zero address");
        _newOwner = newOwner;
    ...
    function acceptOwnership() public returns (bool) {
        require(msg.sender == _newOwner, "USDT_BET_Swap: caller is not the new owner");
        _owner = _newOwner;
        _newOwner = address(0);
    ...
    function withdrawUsdt(uint256 _usdtAmount) public onlyOwner returns (bool) {
        require(usdtBalance() >= _usdtAmount, "USDT_BET_Swap: withdraw amount exceeds balance");
    ...
    function withdrawTcc(uint256 _tccAmount) public onlyOwner returns (bool) {
        require(tccBalance() >= _tccAmount, "USDT_BET_Swap: withdraw amount exceeds balance");
        require(IERC20(_tcc).transfer(_owner, _tccAmount), "USDT_BET_Swap: withdraw BET fail");
    ...
    function setFeePermille(uint256 newFeePermille) public onlyOwner returns (bool) {
        require(newFeePermille >= 0, "USDT_BET_Swap: new fee permille should be greater than or equal to zero");
        require(newFeePermille <= 1000, "USDT_BET_Swap: new fee permille should be less than or equal to 1000");
    ...
    function setTccSwapUsdtRate(uint256 _newRate) public onlyOwner returns (bool) {
        require(_newRate > 0, "USDT_BET_Swap: new rate should be greater than zero");
    ...
    function tccSwapUsdt(uint256 _tccAmount) public returns (bool) {
        ...
        require(IERC20(_tcc).transferFrom(caller, address(this), _tccAmount), "USDT_BET_Swap: transfer BET from caller fail");
    ...
    function usdtSwapTcc(uint256 _usdtAmount) public returns (bool) {
        address caller = address(msg.sender);
        require(IERC20(_usdt).transferFrom(caller, address(this), _usdtAmount), "USDT_BET_Swap: transfer USDT from caller fail");
        uint256 _tccAmount = usdtSwapTccCalc(_usdtAmount);
        require(IERC20(_tcc).transfer(caller, _tccAmount), "USDT_BET_Swap: transfer BET to caller fail");
