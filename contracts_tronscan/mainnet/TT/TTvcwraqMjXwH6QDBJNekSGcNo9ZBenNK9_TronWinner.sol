...
contract TronWinner {
    ...
    uint private minuteRate = 231481;
    uint public releaseTime = 1595952000;
    ...
    function getCurrentMinuteRate() internal view returns (uint256) {
        if (now > releaseTime) {
            uint256 dayPassed = now.sub(releaseTime).div(60 * 60 * 24);
            return minuteRate + dayPassed * minuteRate / 10;
        } else {
            return minuteRate;
        }
    }
    ...
    function collect(address _addr) internal {
        ...
        if (secPassed > 0 && player.time > 0) {
            uint collectProfit = (player.trxDeposit.mul(secPassed.mul(getCurrentMinuteRate()))).div(interestRateDivisor);
