...
//    mapping(address=>uint256)private levelOne;
  //  mapping(address=>uint256)private levelTwo;
    ...
    /*
        Only external call
    */
    function getuserDetail() public view returns(address referralid,uint256 userids,uint256 lastpakage,string memory  _position){
        
        return (users[msg.sender].upline,users[msg.sender].id,levelAmount[msg.sender][users[msg.sender].id],users[msg.sender].position);
    }
