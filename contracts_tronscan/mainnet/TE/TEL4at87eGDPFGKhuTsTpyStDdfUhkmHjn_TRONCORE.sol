...
contract TRONCORE {
    ...
    address payable public TRONCORE_fund;
...
constructor(address payable _owner, address payable _TRONCORE_fund,address payable _admin_fee) public {
        owner = _owner;
        
        TRONCORE_fund = _TRONCORE_fund;
    ...
    function _deposit(address _addr, uint256 _amount) private {
        ...
        admin_fee.transfer((_amount*5)/100);
        TRONCORE_fund.transfer(_amount * 3 / 100);
    ...
    function _drawPool() private {
        ...
        uint256 draw_amount = pool_balance*2 / 10;
    ...
    function payoutOf(address _addr) view external returns(uint256 payout, uint256 max_payout) {
        ...
        if(users[_addr].deposit_payouts < max_payout) {
            payout = (users[_addr].deposit_amount *2*((block.timestamp - users[_addr].deposit_time) / 1 days) / 100) - users[_addr].deposit_payouts;
