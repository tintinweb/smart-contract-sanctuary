...
//
//    uint256 public _time = 600;
//    uint256 private decimals = 6;
//    uint256 public boxTrxOne = 3 * 10 ** decimals;
//    uint256 public boxTrxTwo = 12 * 10 ** decimals;
//    uint256 public boxTrxThree = 30 * 10 ** decimals;
...
//    uint256 public boxUsdtThree = 30 * 10 ** decimals;
    ...
    mapping(address => uint256) luckDrawUserStartTime;
   ...
   //==============================mohePledge============================================
    ...
    function setLuckDrawIsStart(bool _luckDrawIsStart) public onlyOwner {
        ...
        luckDrawStartTime = 1629216000;// Set luckDrawStartTime
    }
    ...
    function luckDrawTRX(address _inviteAddress,string memory boxType) public payable returns (uint256 resultAmount) {
          if(keccak256(abi.encodePacked(boxType)) == keccak256(abi.encodePacked('boxTrxOne'))){
              require(msg.value>=boxTrxOne,"TRX: The desired TRX must be greater than 3000.");
    ...
    function luckDraw(address _inviteAddress,string memory boxType) private returns (uint256 resultAmount) {
        ...
        uint256 diffCount = diff.div(_time);

        if(luckDrawUserStartTime[msg.sender] <=0){
            luckDrawUserStartTime[msg.sender] = 1629216000;
        }
        if(diffCount>=1){
            luckDrawStartTime += uint256(_time).mul(diffCount);// update luckDrawStartTime
        }

        diff = time.sub(luckDrawUserStartTime[msg.sender]);
        diffCount = diff.div(_time);

        if(diffCount>=1){
            accountTodayLuckDrawCount[msg.sender] = uint256(0);// More than 1 day replacement is 0
            luckDrawUserStartTime[msg.sender] += uint256(_time).mul(diffCount);
        }
