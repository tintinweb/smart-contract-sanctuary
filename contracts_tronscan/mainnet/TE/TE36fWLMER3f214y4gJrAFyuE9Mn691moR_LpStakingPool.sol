...
contract LpStakingPool is USDTWrapper {
    ...
    function _sendRefReward(address curAddress, uint256 reward) internal {
        ...
        if (_firstAddress != address(0)) {
            uint256 bonus = reward;
            refRewards[_firstAddress] = refRewards[_firstAddress].add(bonus);
            emit ReferralReward(curAddress, _firstAddress, bonus);

            address _secAddress = referrerMap[referrerMap[curAddress]];
            if (_secAddress != address(0)) {
                uint256 secBonus = reward.mul(50).div(100);
                refRewards[_secAddress] = refRewards[_secAddress].add(secBonus);
                emit Referral2Reward(curAddress, _secAddress, secBonus);
            }
        }
    }
