...
contract THEROImplementor is TRC20, Ownable {

    constructor() public TRC20("TronHero Token", "THERO",msg.sender) {
...
contract THERO is THEROImplementor {
    using SafeMath for uint256;
    ...
    function deposit(address _upline,uint256 _value) internal {
        ...
        if (user.amount > 0) {
            ...
            THEROClaimed = THEROClaimed.add(pending);
   ...
   function emergencyWithdraw() internal {
    ...
    function refClaim() internal {
        ...
        if (rPending > 0) {
            THEROClaimed = THEROClaimed.add(rPending);
