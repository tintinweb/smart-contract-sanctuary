...
contract GlobalsAndUtility is ERC20 {
    ...
    */
    ...
    /* ERC20 constants */
    string public constant name = "MX2";
    string public constant symbol = "MX2";
    uint8 public constant decimals = 8;

    /* Suns per Satoshi = 10,000 * 1e8 / 1e8 = 1e4 */
    uint256 private constant SUNS_PER_MX2 = 10 ** uint256(decimals); // 1e8
    ...
    uint256 private constant BPB_MAX_MX2 = 7 * 1e6;
    uint256 internal constant BPB_MAX_SUNS = BPB_MAX_MX2 * SUNS_PER_MX2;
        ...
        external
    {
        ...
        require(g._currentDay > CLAIM_PHASE_START_DAY, "MX2: Too early");

        if (beforeDay != 0) {
            require(beforeDay <= g._currentDay, "MX2: beforeDay cannot be in the future");
    ...
    {
        require(beginDay < endDay && endDay <= globals.dailyDataCount, "MX2: range invalid");
        ...
        view
    {
        ...
        require(stakeIdParam == stRef.stakeId, "MX2: stakeIdParam not in stake");
...
contract StakeableToken is GlobalsAndUtility {
        ...
        external
    {
        ...
        require(newStakedDays >= MIN_STAKE_DAYS, "MX2: newStakedDays lower than minimum");
        ...
        external
    {
        ...
        require(stakeLists[stakerAddr].length != 0, "MX2: Empty stake list");
        require(stakeIndex < stakeLists[stakerAddr].length, "MX2: stakeIndex invalid");
        ...
        require(g._currentDay >= st._lockedDay + st._stakedDays, "MX2: Stake not fully served");

        /* Stake must still be locked */
        require(st._unlockedDay == 0, "MX2: Stake already unlocked");
        ...
        external
    {
        ...
        /* require() is more informative than the default assert() */
        require(stakeListRef.length != 0, "MX2: Empty stake list");
        require(stakeIndex < stakeListRef.length, "MX2: stakeIndex invalid");
        ...
        internal
    {
        ...
        require(newStakedDays <= MAX_STAKE_DAYS, "MX2: newStakedDays higher than maximum");
        ...
        require(newStakeShares != 0, "MX2: newStakedSuns must be at least minimum shareRate");
        ...
        returns (uint256 bonusSuns)
    {
        /*
            ...
            Bonus percentage scaled 0% to 10% for the first 7M MX2 of stake.
...
contract TransformableToken is StakeableToken {
        ...
        payable
    {
        ...
        require(rawAmount != 0, "MX2: Amount required");
    ...
    function xfLobbyExit(uint256 enterDay, uint256 count)
        external
    {
        require(enterDay < _currentDay(), "MX2: Round is not complete");
        ...
        if (count != 0) {
            require(count <= qRef.tailIndex - headIndex, "MX2: count invalid");
            endIndex = headIndex + count;
        } else {
            endIndex = qRef.tailIndex;
            require(headIndex < endIndex, "MX2: count invalid");
        }
    ...
    function xfFlush()
        external
    {
        require(address(this).balance != 0, "MX2: No value");

        require(LAST_FLUSHED_DAY < _currentDay(), "MX2: Invalid day");
        ...
        returns (uint256 rawAmount, address referrerAddr)
    {
        ...
        require(entry.rawAmount != 0, "MX2: Param invalid");
    ...
    {
        /* 1095890410958 = ~ 4000000 * SUNS_PER_MX2 / 365 */
...
contract MX2 is TransformableToken {
    constructor()
        public
    {
