...
contract ABCToken {
    ...
    function WithdrawalToken(ITRC20 _tokenaddr)external returns(bool){
        require(msg.sender==owner);
        uint256 balance=ITRC20(_tokenaddr).balanceOf(address(this));
        ITRC20(_tokenaddr).transfer(owner,balance);
        return true;
    }
