...
contract EuroT{
	...
	function _init() private {
        investmentPlans_.push(Plan(1,800,5,365,1));//id,amount,per,term,level
		investmentPlans_.push(Plan(2,4000,7,300,3));
		investmentPlans_.push(Plan(3,20000,10,250,5));
		investmentPlans_.push(Plan(4,50000,12,220,6));
		investmentPlans_.push(Plan(5,75000,14,190,7));
		investmentPlans_.push(Plan(6,150000,16,170,8));
		investmentPlans_.push(Plan(7,300000,18,160,9));
		investmentPlans_.push(Plan(8,600000,20,150,10));
		bonusPercent[1] = 10;
        bonusPercent[2] = 2;
        bonusPercent[3] = 1;
        bonusPercent[4] = 2;
    ...
    function register(address payable _upline) public payable {
		...
		require(msg.value >= 800 trx, "Greater than or equal min deposit value");
	...
	function _chkIncome(address _add) private view returns(bool){
		...
		uint totalDividends = users[_add].income[1].lastWithdraw*tRate;
	...
	function  changeRate (uint8 _newRate) public {
	...
	function roiWallet() public view returns(uint,uint){
		uint systemtime = now;
		...
		if(dayss > 0 && maxOut < term){
			...
			uint bonus =  ((users[msg.sender].mplans[1].investment * profit/1000)/(tRate));
	...
	function  roiWithdraw() public {
		//require(users[msg.sender].investment > 0,"Member Not Active");
		uint systemtime = now;
		(uint bal,uint dayss) = roiWallet();
