...
contract MasterChef is Ownable {
    ...
    mapping(address => uint256) public amount;// user amount
    ...
    function setLpToken(IERC20 _lpToken) public onlyOwner returns (bool) {
        lpToken = _lpToken;
        return true;
    }
    ...
    function deposit(uint256 _amount) public returns (bool){
        require(address(lpToken) != address(0),"please set lpToken");
        require(address(lpToken).safeTransferFrom(address(msg.sender), address(this), _amount),"transfer failed");
    ...
    function withdraw(uint256 _amount) public returns (bool){
        require(address(lpToken) != address(0),"please set lpToken");
        require(amount[msg.sender]>=_amount,"no balance");
        require(address(lpToken).safeTransfer(address(msg.sender), _amount),"transfer failed");
        amount[msg.sender] -= _amount;
        totalAmount -= _amount;
