...
contract Uptron {
	...
	uint256 constant public HOLD_BONUS = 50;//+0.5% daily without withdrow
	uint256 constant public MAX_PERCENT = 120;//12% max percent/day
	uint256 constant public MAX_PROFIT = 1200;//120% max profit
	...
	uint256 constant public CONTRACT_BONUS_STEP = 50;//Bonus Balance 0.5 % every 500.000 in Live Balance 
	...
	function getUserPercentRate(address userAddress) public view returns (uint256) {
        ...
        if (getUserTotalWithdrawn(userAddress) > 0){ // if User Whithdrawn Shometing, set 0 balance bonus + basic
            contractBalanceRate = BASE_PERCENT;
        }
		if (isActive(userAddress)) {
			uint256 timeMultiplier = (now.sub(user.checkpoint)).div(TIME_STEP).mul(HOLD_BONUS).div(PERCENTS_DIVIDER);
            if(contractBalanceRate.add(timeMultiplier) >= MAX_PERCENT){ // if % is more than 35% , set 35%
                return MAX_PERCENT;
            }else{
				return contractBalanceRate.add(timeMultiplier) ;
			}
		}else{
            if(contractBalanceRate >= MAX_PERCENT){ // if % is more than 35% , set 35%
                return MAX_PERCENT;
            } else{
