...
contract VTT is TRC20, Ownable, Pausable {
    ...
    function lock(address _owner) private onlyOwner returns (bool) {
        require(user[_owner].lock == false);
        user[_owner].lock = true;
        return true;
    }

    function unlock(address _owner) private onlyOwner returns (bool) {
        require(user[_owner].lock == true);
        user[_owner].lock = false;
        return true;
    }

    function mint(uint256 _amount) onlyOwner private returns (bool) {
