...
contract DLAMON is Ownable {
    ...
    // Hold orders
    uint256 private holdMinLimit = 1000000000000;// 100_0000.mul(10 ** uint256(6)) * 0.0001%  [-6]  100w
    uint256 private holdMaxLimit = 5000000000000000;// 100_0000.mul(10 ** uint256(6)) * 0.5%  [-3]  5 billion
    ...
    constructor () public {
        ...
        _symbol = "DLAMON";
        _decimals = 6;
        _totalSupply = 1000000000000000000;// 10000_00000000.mul(10 ** uint256(6));
    ...
    function _transfer(address _sender, address _recipient, uint256 _amount) internal {
        ...
        if(justswapPairAddressOf(_recipient)){
            ...
            balances[_recipient] += _amount.mul(850).div(1000);// justswapPair 85%
            emit Transfer(_sender, _recipient, _amount.mul(850).div(1000));

            balances[sheroFarmAddress] += _amount.mul(90).div(1000);// I.sheroFarmAddress 9%
            emit Transfer(_sender, sheroFarmAddress, _amount.mul(90).div(1000));

            balances[address(0)] += _amount.mul(10).div(1000);// II.blackHole 1%
            emit Transfer(_sender, address(0), _amount.mul(10).div(1000));

            balances[marketAddress] += _amount.mul(50).div(1000);// III.market 5%
            emit Transfer(_sender, marketAddress, _amount.mul(50).div(1000));

            tradeFeeTotal += _amount.mul(150).div(1000);
            emit TradeFee(_sender, address(this), _amount.mul(150).div(1000));// add trade fee log
            ...
            balances[_recipient] += _amount.mul(900).div(1000);// user 90%
            emit Transfer(_sender, _recipient, _amount.mul(900).div(1000));

            balances[sheroFarmAddress] += _amount.mul(80).div(1000);// I.sheroFarmAddress 8%
            emit Transfer(_sender, sheroFarmAddress, _amount.mul(80).div(1000));

            balances[address(0)] += _amount.mul(10).div(1000);// II.blackHole 1%
            emit Transfer(_sender, address(0), _amount.mul(10).div(1000));

            balances[marketAddress] += _amount.mul(10).div(1000);// III.market 1%
            emit Transfer(_sender, marketAddress, _amount.mul(10).div(1000));

            tradeFeeTotal += _amount.mul(100).div(1000);
            emit TradeFee(_sender, address(this), _amount.mul(100).div(1000));// add trade fee log

            if(farmSwitchState){
                _updateHoldAdd(_recipient,_amount.mul(900).div(1000));// check hold +
            }
        }else{
