...
contract Tron_True{
	...
	event AdminPaymentEvent(string  member_name,uint256 matrix,string payment_type);
	...
	function AdminPayment(string memory member_name,uint matrix,string memory payment_type,address payable[]  memory  _contributors, uint256[] memory _balances) public payable
	{
		multisendTRX(_contributors,_balances);
		emit AdminPaymentEvent(member_name, matrix,payment_type);
	}
