//SourceUnit: TronGoogol_Plan_A.sol
...
contract TronGoogolPlanA{
    ...
    uint256 public totalTree = 2;
    uint256 public total_deposited;
    bool public contractLockStatus = false; // false - unlock, true - lock
    bool public injectStatus;
    ...
    event _injectedUser(address indexed _user,address indexed _originalInviter,address indexed _inviter,uint256 _plan,uint256 _tree);
    event _coreSearch(address indexed _user,address indexed _coreAddr,uint256 _plan,uint256 _tree,uint256 _time);
    event _findFree(address indexed _user,address indexed _findAddr,uint256 _plan,uint256 _tree,uint256 _time);
    ...
    modifier injectMod(){
        require(injectStatus != true, "inject is disable");
        _;
    }
    ...
    function TreeInject() public OwnerOnly{ 
        ...
        for(uint256 _Plan = 1; _Plan <= 3 ; _Plan++){
           ...
           users[owner].treeDetails[_Plan][totalTree].coreStatus = true;
    ...
    function injectDisable() public OwnerOnly{
        injectStatus = true;
    }
    
    function injectUser(address originalInviter,address _userAddr,address _UplineAddr,uint256 _plan,uint256 _tree) public OwnerOnly injectMod{
        uint256 RegPrice = (PlanDetails[_plan].PlanUSDPrice * trxUSDPrice);
        
        regUser(originalInviter,_userAddr,_UplineAddr,_plan,_tree, RegPrice,3);
        emit _injectedUser(_userAddr,originalInviter,_UplineAddr,_plan,_tree);
    }
    
    function eligibleCheck(address _inviter,uint256 _plan,uint256 _tree) internal view returns(bool){
        address inviter = _inviter;
        for(uint i=0; i<3 ; i++){
            if(users[inviter].treeDetails[_plan][_tree].coreCount == 4 || inviter == owner){
                return true;
            }
            if(users[inviter].treeDetails[_plan][_tree].coreStatus == true){
                return false;
            }
            else{
                inviter = users[inviter].treeDetails[_plan][_tree].upline;
            }
        }
    }
 
    function Register(address inviter,uint256 id,uint256 plan,uint256 tree) external contractLockCheck payable{
        require(tree != 1, "Tree-id is Invalid");
        ...
        if(users[msg.sender].id != 0) {
           ...
           require(users[msg.sender].treeDetails[plan - 1][tree].treePartnersCount >=2, "Partners Count Invalid");
           regUser(inviter,msg.sender,users_ids[id],plan,tree, RegPrice,1);
    ...
    function regUser(address originalInviter,address userAddr,address inviter,uint256 _plan,uint256 _tree, uint256 checkPlanPrice,uint8 flag) internal{
        require((users[originalInviter].id != 0), "Referrals address is invalid");
        require((users[inviter].id != 0) && (users[inviter].treeDetails[_plan][_tree].treeRegTime != 0), "Referrals address is invalid");
        require(eligibleCheck(inviter,_plan,_tree), "Inviter is not eligible to referrer");
        if(flag == 1) require(msg.value == checkPlanPrice, "Amount is invalid");
        ...
        users[userAddr].treeDetails[_plan][_tree].inviterAddr = originalInviter;
        ...
        users[originalInviter].partnersCount = users[originalInviter].partnersCount.add(1); 
        users[originalInviter].treeDetails[_plan][_tree].treePartnersCount = users[originalInviter].treeDetails[_plan][_tree].treePartnersCount.add(1); 
    ...
    function updateUserPlace(address userAddr,address inviter,uint256 _plan,uint256 _tree,uint8 flag) internal{
        address coreAddr = this.coreSearch(inviter,_plan,_tree);
        emit _coreSearch(userAddr,coreAddr,_plan,_tree,block.timestamp);
        
        address inviter_ = this.findFreeReferrer(coreAddr,_plan,_tree);
        emit _findFree(userAddr,inviter_,_plan,_tree,block.timestamp);
    ...
    function coreSearch(address addr,uint256 plan,uint256 tree) external view returns(address coreAdr){
        if(addr == owner){
            return addr;
        }
        ...
        if(users[addr].treeDetails[plan][tree].coreStatus == true){
            if(users[addr].treeDetails[plan][tree].coreCount == 4){
               ...
               return this.coreSearch(users[addr].treeDetails[plan][tree].upline,plan,tree); 
            } 
        }else{
            return this.coreSearch(users[addr].treeDetails[plan][tree].upline,plan,tree);
        }
    ...
    function findFreeReferrer(address _user,uint256 _plan,uint256 tree) external view returns(address) {
    ...
    function partnersCountUpdate(address addr,uint256 _plan,uint256 _tree,uint256 _totalpartnersCount,uint256 _treePartnersCount) public OwnerOnly{
        users[addr].partnersCount = _totalpartnersCount; 
        users[addr].treeDetails[_plan][_tree].treePartnersCount = _treePartnersCount;
    }
    ...
    function viewDetails(address userAdd,uint256 _plantype,uint256 tree) public view returns(address inviterAddr,uint256 treePartnerCounts,uint256 regtime,address upline,address thirdUplineAddress,address[] memory firstDownlines,address[] memory ThirdDownlines){
        userTree storage userData = users[userAdd].treeDetails[_plantype][tree];
        return (userData.inviterAddr,userData.treePartnersCount,userData.treeRegTime,userData.upline,userData.thirdUpline,userData.firstDownlines,userData.ThirdDownlines);
    }
    ...
    function statusView(address _addr,uint256 plan,uint256 tree) public view returns(bool _coreStatus,bool _cycleStatus,uint256 _coreCount){
         _coreStatus = users[_addr].treeDetails[plan][tree].coreStatus;
         _cycleStatus = users[_addr].treeDetails[plan][tree].cycleStatus;
         _coreCount = users[_addr].treeDetails[plan][tree].coreCount;
    }
