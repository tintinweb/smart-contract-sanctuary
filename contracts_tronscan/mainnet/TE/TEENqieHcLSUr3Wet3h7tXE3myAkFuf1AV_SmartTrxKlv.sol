...
contract SmartTrxKlv {
	using SafeMath for uint256;

	uint256[2] public INVEST_MIN_AMOUNT = [100 trx, 200 trx]; // 100 trx, 200 klv
	uint256[2] public CONTRACT_BALANCE_STEP = [1000000 trx, 2000000 trx]; // 1000000 trx, 2000000 klv
	uint256[3] public REFERRAL_PERCENTS = [100, 10, 10];
	...
	uint256[2] public totalInvested; // 0-->TRX, 1-->KLV
	...
	constructor(ITRC20 _klv, address payable _projectAddress) public {
	...
	function invest(uint8 tokenType, uint256 amountIn, address referrer) public payable {
		...
		if(tokenType == 1){
			klv.transferFrom(msg.sender, projectAddress, fee);
			klv.transferFrom(msg.sender, address(this), val.sub(fee));
		}else{
	...
	function withdraw(uint8 tokenType) public {
		...
		if(tokenType == 1){
			contractBalance = klv.balanceOf(address(this));
		}
		...
		if(tokenType == 1){
			klv.transfer(msg.sender, totalAmount);
		}else{
	...
	function getContractBalance(uint8 tokenType) public view returns (uint256) {
		if(tokenType == 1){
			return klv.balanceOf(address(this));
		} 
