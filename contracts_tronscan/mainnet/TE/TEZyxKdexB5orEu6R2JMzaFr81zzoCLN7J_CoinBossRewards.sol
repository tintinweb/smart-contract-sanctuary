//SourceUnit: CoinBossRewards.sol
...
contract CoinBossRewards is LPTokenWrapper, Ownable {
    ...
    IERC20 public AACB;
    uint public AACBRate = 50000000;
    uint public cptRate = 1000000;
    uint public usdtRate = 5000000; 
    ...
    uint256 public TotalBurnedAACB;
    mapping(address => uint256) public userRewardPerTokenPaid;
    mapping(address => uint256) public rewards;
    
    uint public IdCount = 1;
    bool public MiningStart = false;
    uint public AACBTokenIncreamentValue;
    uint8 public CurrentPhase;
    uint public ComputingPowerValue = 0;
    uint public USDTIncreamentRate;
    address public PlatformAccount;
    address public TAccount;
    ...
    event AACBRateIncrement(address indexed caller, uint indexed _amount, uint indexed _current_AACB_price);
    event AACBBurned(address indexed caller, uint indexed _amount);
    event RewardAllocated(address indexed caller, uint256 reward);
    ...
    constructor(address _AACB, address _platform, address _burn, address _taccount) public{ //Include start time here
    ...
    function Activate() public updateReward(msg.sender) checkPhase checkStart checkUser{ 
        require(MiningStart,"Notify Reward not activated");
        ...
        value = value + amount;
        ComputingPowerValue = value.mod(4);
        value = value - ComputingPowerValue;
        uint fees = value.div(4);
        ComputingPowerBalance[TAccount] = ComputingPowerBalance[TAccount] + fees; 
        
        
        //Increase rate for every computing power
        usdtRate = usdtRate.add(USDTIncreamentRate.mul(amount)); 
        emit AACBRateIncrement(msg.sender, amount ,usdtRate);
    ...
    function claim() public updateReward(msg.sender) checkPhase checkStart{
        ...
        if (reward > 0) {
            ...
            while(count <= 10){
                ...
                emit RewardAllocated(referral, currentPaid);
                paid = paid.add(currentPaid);
                if(referral == PlatformAccount){
                    if(reward.sub(paid) > 0){
                        TotalBurnedAACB = TotalBurnedAACB.add(reward.sub(paid));
                        AACB.transfer(BurnAccount, reward.sub(paid));
                        emit AACBBurned(msg.sender, reward.sub(paid));
                    }
                    break;
                }
            ...
            AACB.transfer(msg.sender, reward);
    ...
    function notifyRewardAmountAdmin(uint reward)
        external
        onlyOwner
        updateReward(address(0))
    {
    ...
    function Buy(uint _amount, address _referral) checkUser public{
        ...
        uint token_deduct_amount = _amount * (150000000 * 10 ** 7)  / usdtRate;
        token_deduct_amount = token_deduct_amount / 10;
        
        //Trade AACB to CPT
        require(token_deduct_amount <= AACB.balanceOf(msg.sender), "Insufficient AACB token balance.");
        AACB.transferFrom(msg.sender, BurnAccount, token_deduct_amount);
        ComputingPowerBalance[from_] = ComputingPowerBalance[from_] + _amount; 
        
        //Record burn token
        TotalBurnedAACB = TotalBurnedAACB.add(token_deduct_amount);
        emit AACBBurned(msg.sender, token_deduct_amount);
        
        emit BuyCompleted(msg.sender, _amount, token_deduct_amount);
    }
    
    function MintAACB() external onlyOwner{
        AACB.mint(address(this), 900000 * (10 ** uint256(6)));
    }
    ...
    function SetUSDTIncreamentRate(uint rate) onlyOwner external{
        USDTIncreamentRate = rate;
    }
    ...
    function SetUSDTRate(uint _usdtRate) public onlyOwner{
        usdtRate = _usdtRate;
    }
    
    function SetTAccount(address _taccount) public onlyOwner{
        TAccount = _taccount;
    }
}