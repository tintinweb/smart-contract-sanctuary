//SourceUnit: TronFlame.sol

pragma solidity 0.5.8;

/* ---> (www.tronflame.com)  | (c) 2020 Developed by TRX-ON-TOP TEAM TronFlame.sol | La Habana - Cuba <------ */
...
contract TronFlame {
    ...
    // Const
    uint256 private constant ADVERTISING_FEE      = 5;
    ...
    uint256 private constant UNI_POPBONUS         = 0.001E9;
    uint256 private constant MAX_POPBONUS         = 10E9;
    uint256 private constant UNI_REINVESTBONUS    = 0.1E9;
    uint256 private constant DAILY_BALACEBONUS    = 0.1E9;
    ...
    uint256 private constant MIN_INVEST           = 30E6;  
    ...
    uint256 private constant CONTRACT_TIMETOSTART = 164 * 60 * 60;
    ...
    function updateBalanceBonus() internal {
        if (now > last_read_balance + WAIT_FOR_BALANCE) {
           ...
           last_read_balance += WAIT_FOR_BALANCE;
    ...
    function userPlanInfo(address _addr) view external returns(uint256 _depositCount, uint256 _activeDeposit, uint256 _reinvestCount, uint256 _dividends, uint256 _nextReinvest, uint256 _nextResetWithdraw) {
        ...
        if (now < contract_StartTime) {
            ...
            nrw = minZero(last_read_balance + WAIT_FOR_BALANCE, now);
        }
