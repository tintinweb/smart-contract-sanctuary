...
contract MasterChef is ChefStorage(address(0),address(0)){
    ...
    function pendingCake(uint256 _pid, address _user) external view returns (uint256,uint256) {
        ...
        return (user.amount.mul(accCakePerShare).div(1e12).sub(user.rewardDebt),user.shareAward);
    }
    ...
    function draw(uint256 _pid) external returns(uint award,uint share) {
        ...
        award = user.award;
        share = user.shareAward;

        uint total = award + share;

        if( total > 0 ){
            user.award = 0;
            user.shareAward = 0;

            mineToken.transfer(msg.sender,total);
