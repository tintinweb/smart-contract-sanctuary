...
contract TronDynasty {
    ...
    user.referrer = referrer;
		...
		uint256 max;
		if(msg.value<=10000000 trx && msg.value>1000000 trx){
		    max=225;
		}
		...
		else if(msg.value<=10000 trx && msg.value>100 trx){
		    max=135;
		}
        ...
        if (user.referrer != address(0)) {
			...
			for (uint256 i = 0; i < level; i++) {
				if (upline != address(0)) {
					...
					users[upline].uniLVL[i] = users[upline].uniLVL[i].add(amount);
					upline = users[upline].referrer;
				} else break;
			}
	...
	function getUserDepositInfo(address userAddress, uint256 index) public view returns(uint256, uint256, uint256,uint256) {
		...
		return (user.deposits[index].amount, user.deposits[index].withdrawn, user.deposits[index].start,user.deposits[index].max);
	}
