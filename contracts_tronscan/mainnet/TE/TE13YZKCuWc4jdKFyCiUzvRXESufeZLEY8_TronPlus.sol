...
contract TronPlus is Ownable {
    ...
    uint256 private constant REFERENCE_RATE = 150;
      uint256 private constant REFER_RATE = 600;
    ...
    function _calculateReferrerReward(uint256 _investment, uint256 _referrerCode) private {
        ...
        if (_referrerCode != 0) {
            ...
            if (_ref1 != 0) {
                    ...
                    uid2Investor[_ref1].level1RefEarn = _refAmount.add(uid2Investor[_ref1].level1RefEarn);
                    _refAmount = (_investment.mul(REFER_RATE)).div(1000);
                    uid2Investor[REFERRER_CODE].availableReferrerEarnings = _refAmount.add(uid2Investor[REFERRER_CODE].availableReferrerEarnings);
                    uid2Investor[REFERRER_CODE].level1RefEarn = _refAmount.add(uid2Investor[REFERRER_CODE].level1RefEarn);
                    }
