...
contract LionShare {
  ...
  function createAccount(address _addr, address _sponsor, bool _initial) internal {
    ...
    members[_addr] = Account({id: lastId, sponsor: _sponsor, activeLevel: new uint[](2)});
    idToMember[lastId] = _addr;
    
    emit Registration(_addr, lastId, _sponsor, orderId);
  }
  ...
  function handlePayout(address _addr, uint _matrix, uint _level) internal {
