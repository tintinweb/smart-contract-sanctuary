...
/**
 * @title TFC Token Contract
 * @dev Responsible for receiving the token's details at deployment
    and creating the token with the TRC20 token standard. TFC token
    also has the burn feature
 * @author @wafflemakr
 */

contract TFC is TRC20Burnable, TRC20Mintable{
    ...
    (
        ...
        address initialOwner, uint256 initTime
    )

        public TRC20Detailed(name, symbol, decimals, supply * (10 ** uint256(decimals)), 5 * (10 ** uint256(decimals)), initTime)
...
contract TRC20Detailed is TRC20 {
    ...
    constructor (string name, string symbol, uint8 decimals, uint256 supply, uint256 minUnit, uint256 initTime) public {
        _name = name;
        _symbol = symbol;
        _decimals = decimals;
        _supply = supply;
     ...
     /**
     * @return the time of the token.
     */
    function initTime() public view returns (uint256) {
        return _initTime;
    }
    ...
    /**
     * @return the minUnit of the token.
     */
    function minUnit() public view returns (uint256) {
        return _minUnit;
    }
...
contract TRC20Mintable is MinterRole,TRC20Detailed  {
    ...
    function mint(address to) public onlyMinter returns (bool) {
        uint256 value = 0;
        uint256 interval = now.sub(super.initTime());
        uint256 year = interval.div(4 years);
        uint256 temp = 0;
        for (uint256 i = 0; i <= year; i++) {
            if (interval >= 4 years) {
                interval = interval.sub(4 years);
                temp = 4 years;
            } else {
                temp = interval;
            }
            value = value.add( temp.div(10 minutes) * super.minUnit().div(2 ** i));
        }
        
        if (super.supply() < value) {
            value = super.supply().sub(super.totalSupply());
        } else {
            value = value.sub(super.totalSupply());
        }
