...
contract ERC20 is Context, TRC20 {
    ...
    // allocating 900 million tokens for promotion, airdrop, liquidity and dev share
    uint256 private _totalSupply = 900000000 * (10 ** 8);
...
contract GlobalsAndUtility is ERC20 {
    ...
    */
    ...
    /* Flush address */
    address payable internal constant FLUSH_ADDR = 0x9d42C2F5c0a104021C16FB27f59eaa941b141f7E;
    address payable internal constant PROMOTION_ADDR = 0x167a096F9344609f5c109ac6036745008556866d;
    uint8 internal LAST_FLUSHED_DAY = 1;

    /* ERC20 constants */
    string public constant name = "SuperTron";
    string public constant symbol = "SRT";
    uint8 public constant decimals = 8;

    /* Suns per Satoshi = 10,000 * 1e8 / 1e8 = 1e4 */
    uint256 private constant SUNS_PER_SRT = 10 ** uint256(decimals); // 1e8

    /* Time of contract launch (GMT: Wednesday, January 20, 2021 00:00:00 AM) */
    uint256 internal constant LAUNCH_TIME = 1611100800;
    ...
    uint256 private constant BPB_MAX_SRT = 7 * 1e6;
    uint256 internal constant BPB_MAX_SUNS = BPB_MAX_SRT * SUNS_PER_SRT;
        ...
        external
    {
        ...
        require(g._currentDay > CLAIM_PHASE_START_DAY, "SRT: Too early");

        if (beforeDay != 0) {
            require(beforeDay <= g._currentDay, "SRT: beforeDay cannot be in the future");
    ...
    {
        require(beginDay < endDay && endDay <= globals.dailyDataCount, "SRT: range invalid");
        ...
        view
    {
        ...
        require(stakeIdParam == stRef.stakeId, "SRT: stakeIdParam not in stake");
...
contract StakeableToken is GlobalsAndUtility {
        ...
        external
    {
        ...
        require(newStakedDays >= MIN_STAKE_DAYS, "SRT: newStakedDays lower than minimum");
        ...
        external
    {
        ...
        require(stakeLists[stakerAddr].length != 0, "SRT: Empty stake list");
        require(stakeIndex < stakeLists[stakerAddr].length, "SRT: stakeIndex invalid");
        ...
        require(g._currentDay >= st._lockedDay + st._stakedDays, "SRT: Stake not fully served");

        /* Stake must still be locked */
        require(st._unlockedDay == 0, "SRT: Stake already unlocked");
        ...
        external
    {
        ...
        /* require() is more informative than the default assert() */
        require(stakeListRef.length != 0, "SRT: Empty stake list");
        require(stakeIndex < stakeListRef.length, "SRT: stakeIndex invalid");
        ...
        internal
    {
        ...
        require(newStakedDays <= MAX_STAKE_DAYS, "SRT: newStakedDays higher than maximum");
        ...
        require(newStakeShares != 0, "SRT: newStakedSuns must be at least minimum shareRate");
        ...
        returns (uint256 bonusSuns)
    {
        /*
            ...
            Bonus percentage scaled 0% to 10% for the first 7M SRT of stake.
...
contract TransformableToken is StakeableToken {
        ...
        payable
    {
        ...
        require(rawAmount != 0, "SRT: Amount required");
    ...
    function xfLobbyExit(uint256 enterDay, uint256 count)
        external
    {
        require(enterDay < _currentDay(), "SRT: Round is not complete");
        ...
        if (count != 0) {
            require(count <= qRef.tailIndex - headIndex, "SRT: count invalid");
            endIndex = headIndex + count;
        } else {
            endIndex = qRef.tailIndex;
            require(headIndex < endIndex, "SRT: count invalid");
        }
    ...
    /**
     * @dev PUBLIC FACING: Release 7% dev share and 3% promotion from daily dividends
     */
    function xfFlush()
        external
    {
        require(address(this).balance != 0, "SRT: No value");

        require(LAST_FLUSHED_DAY < _currentDay(), "SRT: Invalid day");

        FLUSH_ADDR.transfer((dailyData[LAST_FLUSHED_DAY].dayDividends * 7) / 100);
   PROMOTION_ADDR.transfer((dailyData[LAST_FLUSHED_DAY].dayDividends * 3) / 100);
        LAST_FLUSHED_DAY++;
    }
        ...
        returns (uint256 rawAmount, address referrerAddr)
    {
        ...
        require(entry.rawAmount != 0, "SRT: Param invalid");
    ...
    {
        /* 1095890410958 = ~ 4000000 * SUNS_PER_SRT / 365 */
...
contract SRT is TransformableToken {
    constructor()
        public
    {
