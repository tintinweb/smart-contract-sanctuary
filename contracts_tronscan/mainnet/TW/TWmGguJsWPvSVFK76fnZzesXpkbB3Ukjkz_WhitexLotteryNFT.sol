//SourceUnit: WhitexLotteryNFT.sol
...
contract WhitexLotteryNFT is ERC1155, Ownable, Testable {
    // Libraries 
    // Safe math
    using SafeMath for uint256;
    ...
    // State variables 
    address public lotteryContract_;
    ...
    function updateURI(string memory _newURI) external onlyOwner returns(bool) {
        _setURI(_newURI);
        return true;
    }
    
    function updateLotteryContract(address _newLotto) external onlyOwner returns(bool) {
        lotteryContract_ = _newLotto;
        return true;
    }
