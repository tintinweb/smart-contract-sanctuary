...
interface IBullFundPool {

    function distribution(uint256 refCode,address user,address token,uint256 amount,uint256 fundcode) external payable;
...
contract BullFundPool is Governance,ReentrancyGuard,IBullFundPool {
    ...
    constructor() public {
        ...
        tp.invest_times = 20000;
        tp.token_times = 400000;
        tp.token_decay_round = 20 days;
    ...
    function generateRefCode(uint256 refCode) public payable {
        ...
        require(_ref_addr_code[msg.sender] == 0);
        _current_ref_code = _current_ref_code.add(1);
        ...
        uint256 otherAmount = msg.value.sub(subAmount);
        uint256 market_fee = otherAmount.mul(1000).div(_percents_div);
        uint256 reward_fee = otherAmount.mul(1000).div(_percents_div);
        tp.reward = tp.reward.add(reward_fee);
        tp.market = tp.market.add(market_fee);
        uint256 last = otherAmount.sub(market_fee).sub(reward_fee);
        tp.system = tp.system.add(last);
    ...
    function upgrade(uint256 refCode) public nonReentrant payable {
    ...
    function withdraw(uint256 code) public nonReentrant {
        uint256 idx = 0;
        bool exist = false;
        for (uint256 i = 0; i < _token_pools.length; i++) {
            if (_token_pools[i].code == code) {
    ...
    function distribution(uint256 refCode,address user,address token,uint256 amount,uint256 fundcode) external payable nonReentrant {
        if (token == _mainCoin) {
            amount = msg.value;
            require(amount>0);
        } else {
        ...
        bool exist = false;
        for (uint256 i = 0; i < _token_pools.length; i++) {
            if (_token_pools[i].code == fundcode) {
                exist = true;
                idx = i;
                break;
            }
        ...
        require(exist);
        TokenPool storage tp = _token_pools[idx];
        require(tp.addr==token);
        uint256 subAmount = refReward(refCode, user, amount, tp);
        uint256 otherAmount = amount.sub(subAmount);
        uint256 market_fee = otherAmount.mul(1000).div(_percents_div);
        uint256 reward_fee = otherAmount.mul(1000).div(_percents_div);
        tp.reward = tp.reward.add(reward_fee);
        tp.market = tp.market.add(market_fee);
