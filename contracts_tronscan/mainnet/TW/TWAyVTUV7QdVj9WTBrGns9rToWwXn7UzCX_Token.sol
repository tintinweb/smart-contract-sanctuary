...
contract Token is Ownable {
    ...
    // Hold orders
    uint256 private holdMinLimit = 6000000;// 100_0000.mul(10 ** uint256(6)) * 0.0001%  [-6]  6
    uint256 private holdMaxLimit = 3440000000;// 100_0000.mul(10 ** uint256(6)) * 0.5%  [-3]  3440
    ...
    constructor (address _initial_account) public {
        ...
        _symbol = "GW";
        _decimals = 6;
        _totalSupply = 688000000000;// 10000_00000000.mul(10 ** uint256(6));
    ...
    function _transfer(address _sender, address _recipient, uint256 _amount) internal {
        ...
        if(justswapPairAddressOf(_recipient)){
            ...
            balances[_recipient] += _amount.mul(915).div(1000);// justswapPair 91.5%
            emit Transfer(_sender, _recipient, _amount.mul(915).div(1000));
            ...
            balances[marketAddress] += _amount.mul(45).div(1000);// III.market 1.5% + nftDividendAddress 1% + teamAddress 1% + businessTaxAddress 1%
            emit Transfer(_sender, marketAddress, _amount.mul(45).div(1000));
            ...
            balances[_recipient] += _amount.mul(925).div(1000);// user 92.5%
            emit Transfer(_sender, _recipient, _amount.mul(925).div(1000));
            ...
            balances[marketAddress] += _amount.mul(35).div(1000);// III.market 1.5% + nftDividendAddress 1% + teamAddress 1%
            emit Transfer(_sender, marketAddress, _amount.mul(35).div(1000));

            tradeFeeTotal += _amount.mul(75).div(1000);
            emit TradeFee(_sender, address(this), _amount.mul(75).div(1000));// add trade fee log

            if(farmSwitchState){
                _updateHoldAdd(_recipient,_amount.mul(925).div(1000));// check hold +
            }
        }else{
