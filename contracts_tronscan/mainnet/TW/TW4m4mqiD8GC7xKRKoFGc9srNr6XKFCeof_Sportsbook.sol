...
contract Sportsbook is owned
{
    ...
    */
	...
	function placeBet(uint64 ticket, string memory title, bool parlay) payable public {
	...
	function placeBets(uint64[] tickets, string memory titles, uint256[] amounts, bool parlay) payable public {
	...
	function eraseBet(address user, uint64 ticket) public onlyAgent {
		...
		//if (bet.status == $(BETSTATUS_ACCEPTED) || bet.status == $(BETSTATUS_GRADED))
		//	totalPotentialPayoutSUN = totalPotentialPayoutSUN.sub(bet.amount).sub(bet.to_win);
	...
	function uniquebetid (address user, uint64 ticket) private pure returns (uint256) {
		return (uint256(user) << 128) | uint128(ticket);
	}
