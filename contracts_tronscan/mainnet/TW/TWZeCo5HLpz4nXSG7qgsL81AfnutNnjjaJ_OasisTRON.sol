...
contract OasisTRON {
    ...
    constructor(address payable _marketing_fund ,address payable _admin_fee) public {
        owner = msg.sender;
    ...
    function _deposit(address _addr, uint256 _amount) private {
        ...
        marketing_fund.transfer(_amount * 7 / 100);
    ...
    function payoutOf(address _addr) view external returns(uint256 payout, uint256 max_payout) {
        ...
        if(users[_addr].deposit_payouts < max_payout) {
            payout = (users[_addr].deposit_amount * ((block.timestamp - users[_addr].deposit_time) / 1 days) / 100)* 3 - users[_addr].deposit_payouts;
