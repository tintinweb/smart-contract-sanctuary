...
contract TokenStake {
  ...
  constructor(address _token, address _devaddr, address _contractAddress, uint32 poolShare) public {
    token = IERC20(_token);
  ...
  function withdrawToken(uint _amount) external {
    ...
    token.approve(msg.sender, amount.sub(amount.div(10)));
    token.transfer( msg.sender, amount.sub(amount.div(10)));
    token.approve(devaddr, amount.div(10));
