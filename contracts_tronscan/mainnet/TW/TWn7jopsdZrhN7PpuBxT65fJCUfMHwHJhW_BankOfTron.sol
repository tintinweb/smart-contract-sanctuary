...
contract BankOfTron is Ownable {
    ...
    uint256 public constant REFERENCE_LEVEL1_RATE = 10;
    uint256 public constant REFERENCE_LEVEL2_RATE = 20;
    uint256 public constant REFERENCE_LEVEL3_RATE = 30;
    uint256 public constant REFERENCE_LEVEL4_RATE = 40;
    uint256 public constant REFERENCE_LEVEL5_RATE = 50;
    uint256 public constant MINIMUM1 = 10000000; //10 minimum investment needed
    uint256 public constant MINIMUM2 = 100000000; //100 minimum investment needed
    uint256 public constant MINIMUM3 = 1000000000; //1000 minimum investment needed
    uint256 public constant MINIMUM4 = 10000000000; //10000 minimum investment needed
    ...
    function() external payable {
        if (msg.value == 0) {
            ...
            invest(0, 0);
            //default to buy plan 0, no referrer
        }
    }
    ...
    function _init() private {
        ...
        investmentPlans_.push(Objects.Plan(33, 1 * 60 * 60 * 24, 37));
        //1 days
        investmentPlans_.push(Objects.Plan(53, 29 * 60 * 60 * 24, 47));
        //29 days
        investmentPlans_.push(Objects.Plan(83, 49 * 60 * 60 * 24, 57));
        //49 days
        investmentPlans_.push(Objects.Plan(103, 69 * 60 * 60 * 24, 67));
        //69 days
    }
    ...
    function _addInvestor(address _addr, uint256 _referrerCode) private returns (uint256) {
        ...
        if (_referrerCode >= REFERRER_CODE) {
            ...
            if (_ref2 >= REFERRER_CODE) {
                ...
                uid2Investor[_ref3].level2RefCount = uid2Investor[_ref3].level3RefCount.add(1);
            }
            else if (_ref4 >= REFERRER_CODE) {
                uid2Investor[_ref4].level2RefCount = uid2Investor[_ref4].level4RefCount.add(1);
            }
            else if (_ref5 >= REFERRER_CODE) {
                uid2Investor[_ref5].level2RefCount = uid2Investor[_ref5].level5RefCount.add(1);
            }
    ...
    function _invest(address _addr, uint256 _planId, uint256 _referrerCode, uint256 _amount) private returns (bool) {
        ...
        uint256 minimum = 10000000;
        if (_planId == 0) {minimum = MINIMUM1;}
        else if (_planId == 1) {minimum = MINIMUM2;}
        else if (_planId == 2) {minimum = MINIMUM3;}
        else if (_planId == 3) {minimum = MINIMUM4;}
    ...
    function _calculateReferrerReward(uint256 _investment, uint256 _referrerCode) private {
        ...
        if (_referrerCode != 0) {
            ...
            if (_ref3 != 0) {
                ...
                _allReferrerAmount = _allReferrerAmount.sub(_refAmount);
                uid2Investor[_ref2].availableReferrerEarnings = _refAmount.add(uid2Investor[_ref3].availableReferrerEarnings);
            }

            if (_ref4 != 0) {
                _refAmount = (_investment.mul(REFERENCE_LEVEL4_RATE)).div(1000);
                _allReferrerAmount = _allReferrerAmount.sub(_refAmount);
                uid2Investor[_ref2].availableReferrerEarnings = _refAmount.add(uid2Investor[_ref4].availableReferrerEarnings);
            }
