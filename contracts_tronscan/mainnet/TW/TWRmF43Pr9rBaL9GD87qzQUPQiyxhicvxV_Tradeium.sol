...
contract Tradeium {
    ...
    mapping(uint8 => uint) public tradePoolPrice;

    event Registration(address indexed user, address indexed referrer, uint indexed userId, uint referrerId);
    event Reinvest(uint indexed sender, uint indexed userId, uint indexed currentReferrerId, uint callerId, uint8 matrix, uint8 level, uint reInvestCount, bool isTradeSpill);
    event Upgrade(uint indexed sender, uint indexed userId, uint indexed referrerId, uint8 matrix, uint8 level);
    event NewUserPlace(uint indexed sender, uint indexed userId, uint indexed referrerId, uint8 matrix, uint8 level, uint8 place, uint reInvestCount, uint userReInvestCount, bool isTradeSpill);
    event TradePoolDistribution(uint indexed sender, uint receiverId, uint8 level, uint amount);
    ...
    event MissedTrxReceive(uint indexed sender, uint indexed receiverId, uint indexed fromId, uint8 matrix, uint8 level, uint amount);
    ...
    constructor(address payable uplineAddress) public {
        ...
        tradePoolPrice[1] = 100 * 1e6;
        tradePoolPrice[2] = 200 * 1e6;
        tradePoolPrice[3] = 300 * 1e6;
        tradePoolPrice[4] = 400 * 1e6;
        tradePoolPrice[5] = 500 * 1e6;
        tradePoolPrice[6] = 1000 * 1e6;
        tradePoolPrice[7] = 2000 * 1e6;
        tradePoolPrice[8] = 3000 * 1e6;
        tradePoolPrice[9] = 4000 * 1e6;
        tradePoolPrice[10] = 5000 * 1e6;
        tradePoolPrice[11] = 6000 * 1e6;
        tradePoolPrice[12] = 7000 * 1e6;
        tradePoolPrice[13] = 8000 * 1e6;
        tradePoolPrice[14] = 9000 * 1e6;
        tradePoolPrice[15] = 10000 * 1e6;
    ...
    function upgradeLevel(uint8 matrix, uint8 level) external payable {
        ...
        if (matrix == 1) {
            ...
            require(msg.value == tradePoolPrice[level], "invalid price");
            require(!users[msg.sender].activeT3XLevels[level], "level already activated");
    
            address freeX2Referrer = findT2Referrer(msg.sender, level);
            users[msg.sender].t3XMatrix[level].currentReferrer = freeX2Referrer;
            users[msg.sender].activeT3XLevels[level] = true;
            users[msg.sender].T3XLastLevel++;

            uint amount = tradePoolPrice[level];
            updateT2XReferrer(msg.sender, freeX2Referrer, level, amount);
            
            emit Upgrade(getIdByAddress(msg.sender), getIdByAddress(msg.sender), getIdByAddress(freeX2Referrer), 3, level);
        }
    }

    function registration(address userAddress, address referrerAddress) private {
        require(msg.value == (price[1]*2), "Invalid registration amount");
        ...
        uint amount = price[1] * 2;
        uint uplineBonus = amount * 10 / 100;
        amount = price[1] - (uplineBonus / 2);
    ...
    function updateT3Referrer(address userAddress, address referrerAddress, uint8 level, uint amount) private {
        ...
        if (users[referrerAddress].t3Matrix[level].referrals.length < 4) {
        ...
        emit NewUserPlace(getIdByAddress(msg.sender), getIdByAddress(userAddress), getIdByAddress(referrerAddress), 1, level, 4, users[referrerAddress].t3Matrix[level].reinvestCount, 0, false);
    ...
    function updateT2XReferrer(address userAddress, address referrerAddress, uint8 level, uint amount) private {
        ...
        if (users[referrerAddress].t3XMatrix[level].referrals.length < 2) {
            uint reinvestCountOfUser = users[userAddress].t3XMatrix[level].reinvestCount;
            emit NewUserPlace(getIdByAddress(msg.sender), getIdByAddress(userAddress), 
            getIdByAddress(referrerAddress), 3, level, 1, users[referrerAddress].t3XMatrix[level].reinvestCount, 
            reinvestCountOfUser, false);
        ...
        uint reinvestCountOfUser = users[userAddress].t3XMatrix[level].reinvestCount;
        emit NewUserPlace(getIdByAddress(msg.sender), getIdByAddress(userAddress), 
        getIdByAddress(referrerAddress), 3, level, 2, 
        users[referrerAddress].t3XMatrix[level].reinvestCount, 
        reinvestCountOfUser, true);
        
        payNext10T2XUsers(level, amount, userAddress);

        //close matrix
        users[referrerAddress].t3XMatrix[level].referrals = new address[](0);
        users[referrerAddress].t3XMatrix[level].reinvestCount++;
        uint reinvestCount = users[referrerAddress].t3XMatrix[level].reinvestCount;

        if(referrerAddress != uplineUser) {
            address nextUserAddress = findT2Referrer(referrerAddress, level);
            ...
            emit Reinvest(getIdByAddress(msg.sender), getIdByAddress(referrerAddress), 
                getIdByAddress(nextUserAddress), getIdByAddress(userAddress), 3, level, 
                reinvestCount, true);
        } else {
            emit Reinvest(getIdByAddress(msg.sender), getIdByAddress(referrerAddress), 
                getIdByAddress(referrerAddress), getIdByAddress(userAddress), 3, level, 
                reinvestCount, true);
        }        
    ...
    function findT2Referrer(address userAddress, uint8 level) internal returns(address) {
        ...
        return getAddressById(users[uplineUser].id);
    }
        ...
        if (matrix == 1) {
            while (true) {
                if (users[receiver].t3Matrix[level].blocked) {
                    emit MissedTrxReceive(getIdByAddress(msg.sender), getIdByAddress(receiver), getIdByAddress(_from), 1, level, amount);
            ...
            while (true) {
                if (users[receiver].t4Matrix[level].blocked) {
                    emit MissedTrxReceive(getIdByAddress(msg.sender), getIdByAddress(receiver), getIdByAddress(_from), 2, level, amount);
                    hasBlockedLevelMissings = true;
    ...
    function payNext10T2XUsers(uint8 level, uint amount, address spiller) private {
        uint percentAmount = amount * 10 / 100;
        for(uint8 i = 0; i < 10; i++) {
            address nextAddress = T3XDynamicQueueOrder[level][currentPointingQueueNo[level]].userAddress;
            if (nextAddress == spiller) {
                address(uint160(uplineUser)).transfer(address(this).balance);
                break;
            }
            address(uint160(nextAddress)).transfer(percentAmount);
            ...
            users[nextAddress].queuePositions[level].push(lastAvailableQueueNo[level]);
            T3XDynamicQueueOrder[level][lastAvailableQueueNo[level]] = dynamicQueue;
            lastAvailableQueueNo[level]++;
            
            emit TradePoolDistribution(getIdByAddress(spiller), getIdByAddress(nextAddress), level, percentAmount);
        }
    }
    ...
    function findUplineLeader(address userAddress) private view returns(address) {
        ...
        while (true) {
            if (users[users[userAddress].referrer].partnersCount >= 10 || users[userAddress].referrer == uplineTop) {
                return users[userAddress].referrer;
            }
            userAddress = users[userAddress].referrer;
        }
    ...
    function getCurrentQueueUserId(uint8 level, uint8 nextMember) public view returns(uint id) {
        address userAddress = T3XDynamicQueueOrder[level][currentPointingQueueNo[level] + nextMember].userAddress;
        return users[userAddress].id;
    }
    
    function withDraw() public {
        require(msg.sender == uplineUser, "Only owner will call the method");
        address(uint160(uplineUser)).transfer(address(this).balance);
    }
}