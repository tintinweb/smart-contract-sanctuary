...
contract TrxBanker {
    ...
    constructor() public {
        ...
        tarifs.push(Tarif(8, 50));  // 8 Days 
        tarifs.push(Tarif(8, 50));  // 50*4 = 200 ; 200/8 = 25%
        tarifs.push(Tarif(8, 50));
    ...
    function deposit(uint8 _tarif, address _upline) external payable {
        ...
        require(player.deposits.length < 500000,   "Max 50000 deposits per address");
        ...
        for(uint256 i = 0; i < player.deposits.length; i++) {
            ...
            if(from < to) {
                    ...
                    if(player.total_invested<1000e7){
                        value += dep.amount * (to - from) * 50 * 4 / tarif.life_days / 8640000;
                    }
                    if(player.total_invested>=1000e7){
                        value += dep.amount * (to - from) * 50 * 4 / tarif.life_days / 8640000;
                    }
                    if(player.total_invested>=10000e7){
                        value += dep.amount * (to - from) * 50 * 4 / tarif.life_days / 8640000; //   50*4 = 200  ; 200/8 = 25%
                    }
            }
        }
