...
contract GlobalsAndUtility is ERC20 {
    ...
    */
    ...
    /* Flush address */
    address payable internal constant FLUSH_ADDR = 0xF2B0f4CEbD33A13592cC05Ee0D6c773847fa17c1;
    uint8 internal LAST_FLUSHED_DAY = 1;

    /* ERC20 constants */
    string public constant name = "XM";
    string public constant symbol = "XM";
    uint8 public constant decimals = 8;

    /* Suns per Satoshi = 10,000 * 1e8 / 1e8 = 1e4 */
    uint256 private constant SUNS_PER_XM = 10 ** uint256(decimals); // 1e8

    /* Time of contract launch (2020-11-01T00:00:00Z) */
    uint256 internal constant LAUNCH_TIME = 1604188800;
    ...
    uint256 private constant BPB_MAX_XM = 7 * 1e6;
    uint256 internal constant BPB_MAX_SUNS = BPB_MAX_XM * SUNS_PER_XM;
        ...
        external
    {
        ...
        require(g._currentDay > CLAIM_PHASE_START_DAY, "XM: Too early");

        if (beforeDay != 0) {
            require(beforeDay <= g._currentDay, "XM: beforeDay cannot be in the future");
    ...
    {
        require(beginDay < endDay && endDay <= globals.dailyDataCount, "XM: range invalid");
        ...
        view
    {
        ...
        require(stakeIdParam == stRef.stakeId, "XM: stakeIdParam not in stake");
...
contract StakeableToken is GlobalsAndUtility {
        ...
        external
    {
        ...
        require(newStakedDays >= MIN_STAKE_DAYS, "XM: newStakedDays lower than minimum");
        ...
        external
    {
        ...
        require(stakeLists[stakerAddr].length != 0, "XM: Empty stake list");
        require(stakeIndex < stakeLists[stakerAddr].length, "XM: stakeIndex invalid");
        ...
        require(g._currentDay >= st._lockedDay + st._stakedDays, "XM: Stake not fully served");

        /* Stake must still be locked */
        require(st._unlockedDay == 0, "XM: Stake already unlocked");
        ...
        external
    {
        ...
        /* require() is more informative than the default assert() */
        require(stakeListRef.length != 0, "XM: Empty stake list");
        require(stakeIndex < stakeListRef.length, "XM: stakeIndex invalid");
        ...
        internal
    {
        ...
        require(newStakedDays <= MAX_STAKE_DAYS, "XM: newStakedDays higher than maximum");
        ...
        require(newStakeShares != 0, "XM: newStakedSuns must be at least minimum shareRate");
        ...
        returns (uint256 payout)
    {
        ...
        for (uint256 day = beginDay; day < endDay; day++) {
            ...
            /* user's share of 50% of the day's dividends */
            dayPayout += ((dailyData[day].dayDividends * 50) / 100) * stakeSharesParam
            / dailyData[day].dayStakeSharesTotal;
        ...
        returns (uint256 bonusSuns)
    {
        /*
            ...
            Bonus percentage scaled 0% to 10% for the first 7M XM of stake.
...
contract TransformableToken is StakeableToken {
        ...
        payable
    {
        ...
        require(rawAmount != 0, "XM: Amount required");
    ...
    function xfLobbyExit(uint256 enterDay, uint256 count)
        external
    {
        require(enterDay < _currentDay(), "XM: Round is not complete");
        ...
        if (count != 0) {
            require(count <= qRef.tailIndex - headIndex, "XM: count invalid");
            endIndex = headIndex + count;
        } else {
            endIndex = qRef.tailIndex;
            require(headIndex < endIndex, "XM: count invalid");
        }
        ...
        do {
            ...
            if (referrerAddr == address(0) || referrerAddr == msg.sender) {
                ...
                /* Referral bonus of 1% of xfAmount to member */
                uint256 referralBonusSuns = xfAmount / 100;

                xfAmount += referralBonusSuns;

                /* Then a cumulative referrer bonus of 1% to referrer */
                uint256 referrerBonusSuns = xfAmount / 100;
    ...
    /**
     * @dev PUBLIC FACING: Release 50% for Yield Farming Stakers from daily dividends
     */
    function xfFlush()
        external
    {
        require(address(this).balance != 0, "XM: No value");
									  
								   

        require(LAST_FLUSHED_DAY < _currentDay(), "XM: Invalid day");
        ...
        returns (uint256 rawAmount, address referrerAddr)
    {
        ...
        require(entry.rawAmount != 0, "XM: Param invalid");
    ...
    {
        /* 1095890410958 = ~ 4000000 * SUNS_PER_XM / 365 */
        if (enterDay > 0 && enterDay <= 365) {                                     
            waasLobby = CLAIM_STARTING_AMOUNT - ((enterDay - 1) * 1095890410958);
        } else {
...
contract XM is TransformableToken {
    constructor()
        public
    {
