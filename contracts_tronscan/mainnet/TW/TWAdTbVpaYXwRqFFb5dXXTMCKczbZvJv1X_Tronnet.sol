  ...
  function checkTRXContractBalance( uint _amount) external {
    ...
    function _deposit(address _addr, uint256 _amount) private {
        ...
        if(users[_addr].upline != address(0)) {
            users[users[_addr].upline].direct_bonus += _amount * 20 / 100;

            emit DirectPayout(users[_addr].upline, _addr, _amount * 20 / 100);
        }
    ...
    function getUnWithdrawAvailableIncome() view external returns(uint256 amount){
        (uint256 to_payout, uint256 max_payout) = this.payoutOf(msg.sender);
        if(to_payout > 0) {
            if(users[msg.sender].payouts + to_payout > max_payout) {
                to_payout = max_payout - users[msg.sender].payouts;
