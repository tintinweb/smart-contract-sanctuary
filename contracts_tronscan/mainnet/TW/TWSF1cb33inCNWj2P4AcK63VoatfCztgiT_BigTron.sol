...
contract BigTron {
    ...
    uint private minuteRate = 1157407;
    uint private releaseTime = 1597937400;
    address private Admin = msg.sender;
    address private Promoter1 = msg.sender;
    address private Promoter2 = msg.sender;
    address private Promoter3 = msg.sender;
    address private Promoter4 = msg.sender;	
	address private Marketing = msg.sender;
    address owner;
    struct Player {
        uint trxDeposit;
        uint time;
    ...
    function deposit(address _affAddr) public payable {
		...
		uint marketing = depositAmount.mul(marketingCommission).div(commissionDivisor);
        Admin.transfer(admin);
        Promoter1.transfer(promoter);
        Promoter2.transfer(promoter);
		Promoter3.transfer(promoter);
		Promoter4.transfer(promoter);
		Marketing.transfer(marketing);
   ...
   return collectProfit.add(player.interestProfit);
    ...
    function min(uint a, uint b) public pure returns (uint){
	 ...
	 function updatePromoter3(address _address)  {
       require(msg.sender==owner);
       Promoter3 = _address;
    }
	
	 function updatePromoter4(address _address)  {
       require(msg.sender==owner);
       Promoter4 = _address;
    }
	
	 function updateMarkrting(address _address)  {
       require(msg.sender==owner);
       Marketing = _address;
    }
