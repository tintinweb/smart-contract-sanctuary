...
contract TronTrain {
    ...
    function userInfo(address _addr) view external returns(address, uint256, uint256, uint256, uint256, uint256, uint256);
    function userInfoTotals(address _addr) view external returns(uint256, uint256, uint256, uint256);
...
contract TronTrains {
    ...
    TronTrain public trontrain = TronTrain(0x41cfe0b2857b215fbded3fd5002e7c37859ca7a21f);
    ...
    uint256 public total_users = 59;
    uint256 public total_deposited = 206700000000;
    uint256 public total_withdraw = 162427576173;
    ...
    function getDataFromOld(address _addr) external onlyDeployer returns(address)
    {        
        (address upline, uint256 time, uint256 deposit_amount, uint256 payouts, uint256 direct_bonus, uint256 pool_bonus, uint256 match_bonus) = trontrain.userInfo(_addr);

        (uint256 referrals, uint256 total_deposits, uint256 total_payouts, uint256 total_structure) = trontrain.userInfoTotals(_addr);
   
    
        users[_addr].upline = upline;
        users[_addr].referrals = referrals;
        users[_addr].payouts = payouts;
        ...
        users[_addr].time = time;
        users[_addr].total_deposits = total_deposits;
        users[_addr].total_payouts = total_payouts;
        users[_addr].total_structure = total_structure;
    }
    function getDataFromOld1(address _addr, uint256 deposit_payouts, uint256 deposit_time, uint256 pending_payout) external onlyDeployer returns(address)
    {        
        users[_addr].deposit_payouts = deposit_payouts;
        users[_addr].deposit_time = deposit_time;
        users[_addr].pending_payout = pending_payout;
    }
