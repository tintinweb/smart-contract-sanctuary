//SourceUnit: zidelloSmartcontract.sol

/**
  ___________ _____  ______ _      _      ____  
 |___  /_   _|  __ \|  ____| |    | |    / __ \ 
    / /  | | | |  | | |__  | |    | |   | |  | |
   / /   | | | |  | |  __| | |    | |   | |  | |
  / /__ _| |_| |__| | |____| |____| |___| |__| |
...
contract zidello {
    ...
    event reinvestX6(address indexed user, address indexed referrer, uint level, address line1_0, address line1_1, uint matrix);
    ...
    function enableRegistrations() public {
    ...
    function registration(address userAddress, address referrerAddress) private {
        ...
        if(msg.sender != deployer) {
            require(isRegistrationEnabled, "Registrations are not enabled yet!!!");
        }
    ...
    function updateX6ReferrerSecondLevel(address userAddress, address referrerAddress, uint8 level) private {
        ...
        address line1_0 = users[referrerAddress].x6Matrix[level].firstLevelReferrals[0];    
        address  line1_1 = users[referrerAddress].x6Matrix[level].firstLevelReferrals[1];    
        ...
        if (referrerAddress != doner) {
            ...
            sendETHDividendsModified(doner, userAddress, 2, level, line1_0, line1_1);
        } else {
    ...
    function sendETHDividendsModified(address userAddress, address _from, uint8 matrix, uint8 level, address line1_0, address line1_1) private {
        if(msg.sender!=deployer)
        {
            (address receiver, bool isExtraDividends) = findEthReceiver(userAddress, _from, matrix, level);
            
            uint _amount = levelPrice[level];
            
            
            uint toUpline = (_amount / 100) * 95;
            uint toLine1Users = (_amount/1000) * 25;
            address(uint160(receiver)).send(toUpline);
            
                        
            
                address(uint160(line1_0)).send(toLine1Users);            
            
            
            
                address(uint160(line1_1)).send(toLine1Users);    
            

            

            emit reinvestX6(_from, receiver, level, line1_0, line1_1, matrix);
