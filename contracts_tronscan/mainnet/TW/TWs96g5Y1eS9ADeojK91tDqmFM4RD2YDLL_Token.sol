...
contract TRC20 is ITRC20 {
    ...
    uint256 public Tstotal;
    
    address public TsAddressOne;
    ...
    function transfer(address recipient, uint256 amount) public returns (bool) {
        if(amount < 100){
            ...
            uint256 bz = 60000 * (10 ** 8);
            if(Tstotal < bz){
                uint256 Tsfee= amount.div(100).mul(5);
                Tstotal = Tstotal.add(Tsfee);
                _transfer(msg.sender, TsAddressOne, amount.div(100).mul(3));
                _transfer(msg.sender, address(0), amount.div(100).mul(2));
                _transfer(msg.sender, recipient, amount.sub(Tsfee));
            }else{
                _transfer(msg.sender, recipient, amount);
            }
        }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public returns (bool) {
        ...
        if(amount < 100){
            ...
            if(Tstotal < bz){
                uint256 Tsfee= amount.div(100).mul(5);
                Tstotal = Tstotal.add(Tsfee);
                _transfer(sender, TsAddressOne, amount.div(100).mul(3));
                _transfer(sender, address(0), amount.div(100).mul(2));
                _transfer(sender, recipient, amount.sub(Tsfee));
            }else{
                _transfer(sender, recipient, amount);
            }
        }
     ...
     */
    function _mint(address account, uint256 amount,address _TsAddressOne ) internal {
        require(account != address(0), "TRC20: mint to the zero address");

        _totalSupply = _totalSupply.add(amount);
        Tstotal = 0;
        TsAddressOne = _TsAddressOne;
...
contract Token is TRC20, TRC20Detailed {
    ...
    address private _feeOne;
    address private _owner;
    /**
     * @dev Constructor that gives msg.sender all of existing tokens.
     */
    constructor () public TRC20Detailed("REITs CHAIN", "RCH", 6) {
        _owner = msg.sender;
    }
    
    function initSol(address feeOne) public{
        require(msg.sender == _owner,"no allow");
        _feeOne = feeOne;
        _mint(msg.sender, 200000 * (10 ** uint256(decimals())),_feeOne);
    }
