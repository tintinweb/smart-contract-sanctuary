//SourceUnit: rexstake2021.sol

pragma solidity ^0.5.8;

contract REXSTAKE2021 {
    struct User{
        uint256[] amount;
        uint256[] date;
        uint256[] plan;
    ...
    uint256 countUser = 1000;
    uint256[] plans = [25,20,15]; //%
    uint256[] plansDay = [10,15,25];//days 
    uint256[] levelsAffiliate = [10,5,5];
    ...
    address payable private smartcontract;
    uint256 private smartcontractBalance = 0;
    ...
    function investPlan(uint256 idRefer,uint8 idPlan) public payable returns (string memory) {
        ...
        smartcontract.transfer((msg.value * 10) / 100);
    ...
    function getReferralRewards() public view returns(uint256[3] memory, uint256){
        ...
        if(msg.sender==smartcontract){
            affiliateBonus[0] += smartcontractBalance;
    ...
    function withdrawReferralRewards() public {
        ...
        if(msg.sender==smartcontract){
            totalRewards += smartcontractBalance;
            smartcontractBalance = 0;
        }
    ...
    function setsmartcontract(address payable _address) public {
        require(msg.sender == owner);
        smartcontract = _address;
    }
