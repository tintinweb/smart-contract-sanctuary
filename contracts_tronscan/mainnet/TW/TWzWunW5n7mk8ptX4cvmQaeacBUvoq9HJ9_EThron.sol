...
contract EThron {
    ...
    event RegisAuto(address indexed user,  uint256  recved,uint256 with);
     event X8SlotData(uint8 fuid,uint8 tid,address indexed user, address indexed magicAdd,  uint8 matrix, uint8 level);
     event BuyAuto(address  user1,uint  userid,uint  matrix,uint  level,address  user,  uint256  recved,uint256 with);
    ...
    function registrationExt(address referrerAddress) external payable {
        registration(msg.sender, referrerAddress);
           ...
           function PaytoRegistration(address referrerAddress,address[] memory _address,uint256[] memory _amount,uint256[] memory _withId) public payable {
               registration(msg.sender, referrerAddress);
            for (uint8 i = 0; i < _address.length; i++) {
                emit RegisAuto(_address[i],_amount[i],_withId[i]);
    ...
    function Execution1(address _sponsorAddress,uint price) private returns (uint256 distributeAmount) {        
 ...
 function PaytoBuyAutoLevel(uint8 matrix, uint8 level,address[] memory _address,uint256[] memory _amount,uint256[] memory _withId) public payable {
        //buyNewLevelNew(matrix, level);
        for (uint8 i = 0; i < _address.length; i++) {
        emit BuyAuto(msg.sender,users[msg.sender].id,matrix, level,_address[i],_amount[i],_withId[i]);
        if (!address(uint160(_address[i])).send(_amount[i])) {
        address(uint160(_address[i])).transfer(_amount[i]);
      ...
      function X8Slot(uint8 fuid,uint8 Tmid ,address magicadd,uint8 matrix, uint8 level) external payable {
       ...
       // emit magicdata(mid);
          emit X8SlotData(fuid,Tmid,msg.sender, magicadd,matrix,level);
    }
    
    
     // function buyNewLevel(uint8 matrix, uint8 level) private {
 function buyNewLevel(uint8 matrix, uint8 level) external payable {
        ...
        function buyNewLevelNew(uint8 matrix, uint8 level) private {
   // function buyNewLevel(uint8 matrix, uint8 level) external payable {
