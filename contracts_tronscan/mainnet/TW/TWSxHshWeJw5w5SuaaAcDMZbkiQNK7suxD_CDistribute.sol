...
contract Bank {
  ...
  function closeAccount(address payable _addr, address _to) internal hasAccount(_addr) {

    if (accountStorage[_addr].amount > 0) {
      uint amount = accountStorage[_addr].amount;

      accountStorage[_addr].amount = accountStorage[_addr].amount.sub(amount);
      accountStorage[_addr].received = accountStorage[_addr].received.add(amount);
...
contract CDistribute is Bank {
  ...
  function reviewAgreement(uint _account) external hasAccount(msg.sender) {
    ...
    if (accountStorage[accountLookup[_account]].received > agreementAmount[_account]) {
      ...
      subPercentage(accountLookup[_account], 11);
      deleteAccount(accountLookup[2], accountLookup[_account]);
      addPercentage(accountLookup[3], 13);
    }
  }
