...
contract TBoxLP {
    ...
    uint40 public startTime = uint40(block.timestamp);
    uint8[] public year_bonus;
    uint8[] public ref_bonus;
    ...
    function _deposit(uint256 value) private {
        ...
        users[msg.sender].depositTimes.push(uint40(block.timestamp));
    ...
    function Redeem() external {      
        ...
        for(uint256 i = users[msg.sender].validFlag; i < users[msg.sender].depositTimes.length; i++) {
            ...
            users[msg.sender].depositTimes[i] = uint40(block.timestamp);
            payouts += payout;
    ...
    function _calPayout(uint256 idx) private view returns(uint256 payout) {
        ...
        uint40 idxDepositTime = users[msg.sender].depositTimes[idx];
