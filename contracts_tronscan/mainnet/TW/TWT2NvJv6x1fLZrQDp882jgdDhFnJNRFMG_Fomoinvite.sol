...
contract Fomoinvite {
    ...
    constructor(address ownerAddress,address trc20Contract) public {
    ...
    function buyNewLevel(uint8 matrix, uint8 level) public {
        require(isUserExists(msg.sender), "user is not exists. Register first.");
        require(matrix == 1 || matrix == 2, "invalid matrix");
        require(level > 1 && level <= LAST_LEVEL, "invalid level");
        require(check(msg.sender, levelPrice[level]), "transferFrom error");

        TRC20Token(trc20).transferFrom(msg.sender, address(this), levelPrice[level]);
    ...
    function check(address userAddress,uint256 amount) public view returns(bool) {
       uint256 value = TRC20Token(trc20).allowance(userAddress, address(this));
    ...
    function registration(address userAddress, address referrerAddress) private {
        require(check(msg.sender, 50 * 1e18), "transferFrom error");
        require(!isUserExists(userAddress), "user exists");
        require(isUserExists(referrerAddress), "referrer not exists");

        TRC20Token(trc20).transferFrom(msg.sender, address(this), 50 * 1e18);
    ...
    function usersX6MatrixFirstLevelLength(address userAddress,uint8 level) public view returns(uint) {
        return users[userAddress].x6Matrix[level].firstLevelReferrals.length;
    }

    function usersX6MatrixSecondLevelLength(address userAddress,uint8 level) public view returns(uint) {
        return users[userAddress].x6Matrix[level].firstLevelReferrals.length;
    }
    ...
    function sendTronDividends(address userAddress, address _from, uint8 matrix, uint8 level) private {
        ...
        TRC20Token(trc20).transfer(address(receiver), levelPrice[level]);
