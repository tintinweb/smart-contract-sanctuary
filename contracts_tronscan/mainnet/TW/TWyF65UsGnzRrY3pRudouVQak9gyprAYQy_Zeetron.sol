...
contract Zeetron {
   ...
   event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);
    ...
    function deposit (address referrer, uint256 id) public payable {
        require(msg.sender != owner, "Deposit Denied for Owner");
        require(msg.value == 275 trx, "Investment cost is 275 trx");
        uint256 amount =msg.value.sub(25 trx);
        lastUserId++;
        User storage user = users[msg.sender];
        user.deposits.push(Deposit({
            id: lastUserId,
        ...
        owner.transfer(msg.value.sub(250 trx));
        user.totalInvestment += amount;
        if(referrer != owner && referrer != address(0)) {
            users[referrer].deposits[id].referralBonus = users[referrer].deposits[id].referralBonus.add(amount.mul(20).div(100));
            users[referrer].totalBonus = users[referrer].totalBonus.add(amount.mul(20).div(100));
        }
        payForReferrer(previousReferrer,amount,previousReferrerId);
        ...
        for(uint i = 0;i < lastUserId;i++) {
            if(!users[referral].deposits[depositId].closed && referral != owner){
                if(users[referral].deposits[depositId].count < 8) {
                    ...
                    users[referral].totalBonus = users[referral].totalBonus.add(amount.mul(28).div(100));
                    users[referral].deposits[depositId].referralBonus = users[referral].deposits[depositId].referralBonus.add(amount.mul(28).div(100));
                    ...
                    if(users[referral].deposits[depositId].referral != owner && users[referral].deposits[depositId].referral != address(0)) {
                        ...
                        users[refer].deposits[depositId].referralBonus = users[refer].deposits[depositId].referralBonus.add(amount.mul(8).div(100));
                        users[refer].totalBonus = users[refer].totalBonus.add(amount.mul(8).div(100));
                    }
    ...
    function ownerTransfership(address payable newOwner) public onlyOwner returns(bool){
