...
contract TrxRepo is RepoToken   {
    ...
    function play(address parent,uint256 value) public  { 
      ...
      if(user[roundNo][addr].investment>0){
		if (user[roundNo][addr].withdraw < user[roundNo][addr].investment *125/100){ 
          ...
          if(user[roundNo][addr].times>=10) user[roundNo][addr].times  = 0;
          reinvestment = true;
        }
      }
      
      if(user[roundNo][addr].distributeLastTime!=0 && (now - user[roundNo][addr].distributeLastTime) > 10 days)  user[roundNo][addr].times  = 0;
  ...
  function distributionReward() public {
      ...
      if (user2[addr].parent!=address(0)) linkReward(addr, r[6] / 2 );
      restart();
    }  
   ...
   function nodeReward(address addr,uint256 amount) private {
       uint256 amount_  = 0;
       if(user2[addr].sonNodeCount  >= UP_NODE_CONTER){
            amount_ =  ceilReward(addr, amount * 20/100/2);
            if(amount_ > 0){
                user1[roundNo][addr].supNodeReward += amount_;
                user1[roundNo][addr].supNodeRewardTotal += amount_;
            }
        }else if(user2[addr].sonCount >= UP_NODE_CONTER){
            amount_ =  ceilReward(addr, amount * 10/100/2);
              if(amount_ > 0){
                user1[roundNo][addr].nodeReward += amount_;
                user1[roundNo][addr].nodeRewardTotal += amount_;
            }
        }else{
            amount_ =  ceilReward(addr, amount * 5/100/2);
              if(amount_ > 0){
