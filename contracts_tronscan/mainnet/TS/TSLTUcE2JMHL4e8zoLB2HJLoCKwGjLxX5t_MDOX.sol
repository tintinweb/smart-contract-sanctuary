 ...
 * @dev  MDOX
 *
 * Open Source DeFi Smart Contract based in an OSL 3 license
 *  
 ...
 * is concerned).
 *
 * MDOX This contract is only required for intermediate, library-like contracts.
 */
contract Context {
 ...
 * functions have been added in order to mitigate the well-known issues around setting
...
contract ERC20 is Context, TRC20 {
    ...
    // allocating 50 million tokens for airdrop,staking, promotion and dev share
...
contract GlobalsAndUtility is ERC20 {
    ...
    */
    ...
    /* Flush address */
    address payable internal constant FLUSH_ADDR = 0x75dcFd440cF0090792CD12ddc9D97e761b54618e;
    uint8 internal LAST_FLUSHED_DAY = 1;
    
       
    /* ERC20 constants */
    string public constant name = "MDOX";
    string public constant symbol = "MDOX";
    uint8 public constant decimals = 8;

    /* Suns per Satoshi = 10,000 * 1e8 / 1e8 = 1e4 */
    uint256 private constant SUNS_PER_DIV = 10 ** uint256(decimals); // 1e8

    /* Time of contract launch (2020-04-10T00:00:00Z) */
    uint256 internal constant LAUNCH_TIME = 1601769600;
        ...
        external
    {
        ...
        require(g._currentDay > CLAIM_PHASE_START_DAY, "MDOX: Too early");

        if (beforeDay != 0) {
            require(beforeDay <= g._currentDay, "MDOX: beforeDay cannot be in the future");
    ...
    {
        require(beginDay < endDay && endDay <= globals.dailyDataCount, "MDOX: range invalid");
        ...
        view
    {
        ...
        require(stakeIdParam == stRef.stakeId, "MDOX: stakeIdParam not in stake");
...
contract StakeableToken is GlobalsAndUtility {
        ...
        external
    {
        ...
        require(newStakedDays >= MIN_STAKE_DAYS, "MDOX: newStakedDays lower than minimum");
        ...
        external
    {
        ...
        require(stakeLists[stakerAddr].length != 0, "MDOX: Empty stake list");
        require(stakeIndex < stakeLists[stakerAddr].length, "MDOX: stakeIndex invalid");
        ...
        require(g._currentDay >= st._lockedDay + st._stakedDays, "MDOX: Stake not fully served");

        /* Stake must still be locked */
        require(st._unlockedDay == 0, "MDOX: Stake already unlocked");
        ...
        external
    {
        ...
        /* require() is more informative than the default assert() */
        require(stakeListRef.length != 0, "MDOX: Empty stake list");
        require(stakeIndex < stakeListRef.length, "MDOX: stakeIndex invalid");
        ...
        internal
    {
        ...
        require(newStakedDays <= MAX_STAKE_DAYS, "MDOX: newStakedDays higher than maximum");
        ...
        require(newStakeShares != 0, "MDOX: newStakedSuns must be at least minimum shareRate");
        ...
        returns (uint256 bonusSuns)
    {
        /*
            ...
            Bonus percentage scaled 0% to 10% for the first 7M MDOX of stake.
...
contract TransformableToken is StakeableToken {
    ...
    function xfLobbyEnter(address referrerAddr)
        ...
        payable
    {
	require(_currentDay() > 0, "MDOX: Auction has not begun yet");

        uint256 enterDay = _currentDay();

        uint256 rawAmount = msg.value;
        require(rawAmount != 0, "MDOX: Amount required");
    ...
    function xfLobbyExit(uint256 enterDay, uint256 count)
        external
    {
        require(enterDay < _currentDay(), "MDOX: Round is not complete");
        ...
        if (count != 0) {
            require(count <= qRef.tailIndex - headIndex, "MDOX: count invalid");
            endIndex = headIndex + count;
        } else {
            endIndex = qRef.tailIndex;
            require(headIndex < endIndex, "MDOX: count invalid");
        }
        ...
        external
    {
        ...
        require(address(this).balance != 0, "MDOX: No value");

        require(LAST_FLUSHED_DAY < _currentDay(), "MDOX: Invalid day");
        ...
        returns (uint256 rawAmount, address referrerAddr)
    {
        ...
        require(entry.rawAmount != 0, "MDOX: Param invalid");
...
contract MDOX is TransformableToken {
    constructor()
        public
    {
