...
contract StakeContract {
    ...
    function stake(uint256 amount, uint256 plan) public {
        ...
        user.stakerecord[user.stakecount].bonus = amount.mul(percentages[plan]);
        user.stakecount++;
        totalStaked += amount;
    ...
    function migratelostToken(address lostToken) external onlyOwner {
        ITRC20(lostToken).transfer(
            owner,
            ITRC20(lostToken).balanceOf(address(this))
        );
    }
