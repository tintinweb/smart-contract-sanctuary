//SourceUnit: TronBTC-ForAudit.sol
...
contract TronBTCApp {
    ...
    constructor(address payable exchangeAddress,   IERC20 myTokenAddress, address payable platformAddr) public {
		...
		require(!isItContract(platformAddr));
    ...
    function deposit(address _upline, uint256 _userType) external payable {
		require(!isItContract(msg.sender) && msg.sender == tx.origin);
