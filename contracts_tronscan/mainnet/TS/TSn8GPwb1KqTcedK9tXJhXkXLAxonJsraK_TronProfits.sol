...
contract TronProfits is Ownable {
    using SafeMath for uint256;
    uint256 public constant TEAM_RATE = 15;            // 1.5% Team, Operation & Development
    ...
    uint256 public constant PLAN_INTEREST = 15;            // 1.5% Daily Roi
    uint256 public constant PLAN_TERM = 300 days;             // 300 Days
    ...
    function _init() private {
        ...
        _addInvestor(dev, REFERRER_CODE);
        _addInvestor(marketing, REFERRER_CODE);
        _addInvestor(operator, REFERRER_CODE);
        _addInvestor(partner, REFERRER_CODE);
    }
    ...
    function _addInvestor(address _addr, uint256 _referrerCode) private returns (uint256) {
        if (_referrerCode >= REFERRER_CODE) {
            if (uid2Investor[_referrerCode].addr == address(0)) {
                _referrerCode = REFERRER_CODE+1;
            }
        } else {
            _referrerCode = REFERRER_CODE+2;
        }
        address addr = _addr;
    ...
    function _invest(address _addr, uint256 _referrerCode, uint256 _amount) private returns (bool) {
        ...
        uint256 percentage = (_amount.mul(TEAM_RATE)).div(1000);
        dev.transfer(percentage);
        marketing.transfer(percentage);
        operator.transfer(percentage);
        partner.transfer(percentage);
    ...
    function withdraw() public {
        ...
        if(withdrawalAmount>0){
            ...
            uint256 percentage = (withdrawalAmount.mul(10)).div(1000);
            owner.transfer(percentage);
            dev.transfer(percentage);
            marketing.transfer(percentage);
            partner.transfer(percentage);
