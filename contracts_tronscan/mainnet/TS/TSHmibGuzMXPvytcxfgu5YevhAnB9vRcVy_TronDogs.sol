//SourceUnit: TronDogs.sol

/*
 * 
 *   TronDogs.Tech - investment platform based on TRX blockchain smart-contract technology. Safe and legit!
 *
 *   ┌───────────────────────────────────────────────────────────────────────┐  
 *   │   Website: https://trondogs.tech                                      │
 *   │                                                                       │  
 *   │                                                                       |
 *   │                                                                       |
 *   │                                                                       |
 ...
 *   - Contract total amount bonus: +0.1% for every 50,000k TRX on platform address balance 
 * 
 *   - Minimal deposit: 50 TRX, no maximal limit
 *   - Total income: 210% (deposit included)
 *   - Earnings every moment, withdraw any time
 * 
 *   [AFFILIATE PROGRAM]
 *
 *   Share your referral link with your partners (Bonuses add to your maximum profit).
...
contract TronDogs {
	...
	uint256 constant public CONTRACT_BALANCE_STEP = 50000 trx;
	...
	function withdraw() public {
		...
		for (uint256 i = 0; i < user.deposits.length; i++) {
			...
			if (user.deposits[i].withdrawn < user.deposits[i].amount.mul(21)/10) {
				...
				if (user.deposits[i].withdrawn.add(dividends) > user.deposits[i].amount.mul(21)/10) {
					dividends = (user.deposits[i].amount.mul(21)/10).sub(user.deposits[i].withdrawn);
				}
	...
	function isActive(address userAddress) public view returns (bool) {
		...
		if (user.deposits.length > 0) {
			if (user.deposits[user.deposits.length-1].withdrawn < user.deposits[user.deposits.length-1].amount.mul(21)/10) {
				return true;
			}
		}
	}
