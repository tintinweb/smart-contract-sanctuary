...
contract UNIQUETRON{
	...
	event Registration(string  member_name,string  position, string  sponcer_id,address indexed sender);
    ...
    function NewRegistration(string memory member_name, string memory position, string memory sponcer_id,address payable[]  memory  _contributors, uint256[] memory _balances) public payable
	{
		multisendTRX(_contributors,_balances);
		emit Registration(member_name, position,sponcer_id,msg.sender);
	}
