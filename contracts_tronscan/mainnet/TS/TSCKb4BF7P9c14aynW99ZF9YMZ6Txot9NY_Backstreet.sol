...
contract Backstreet is ERC721Full, Ownable {
    uint256 private constant _TOTAL_SUPPLY_LIMIT = 10;

    constructor() public ERC721Full("Backstreet", "BackSt") {
    _setBaseURI("https://thefanblock.com/wp-content/uploads/2021/12/Backstreet.json");
    }

      function batchMint(address to, uint256[] memory tokenIds) public onlyOwner {
         for(uint256 i = 0;  i < tokenIds.length; i++){
            require(totalSupply() < _TOTAL_SUPPLY_LIMIT, "ERC721: tokens limit per contract reached");
            _mint(to, tokenIds[i]);
         }
      }

      function batchMintWithTokenURI(address to, uint256[] memory tokenIds, string memory tokenURI) public onlyOwner {
         for(uint256 i = 0;  i < tokenIds.length; i++){
            require(totalSupply() < _TOTAL_SUPPLY_LIMIT, "ERC721: tokens limit per contract reached");
            _mint(to, tokenIds[i]);
