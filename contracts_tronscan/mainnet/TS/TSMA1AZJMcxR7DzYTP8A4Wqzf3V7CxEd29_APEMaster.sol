//SourceUnit: APEMaster.sol

pragma solidity 0.5.12;

import "./APEToken.sol";

contract APEMaster is Ownable {
    using SafeMath for uint256;

    APE public rewardToken;
    ...
    constructor(APE _rewardToken) public {
    ...
    function unstake(uint256 _amount) public {
        ...
        user.investment = user.investment.sub(
            _amount,
            'APEMaster::unstake: Insufficient funds'
        );
    ...
    function stats() view public returns (
        ...
        totalAPE = rewardToken.totalSupply();
...
contract APE is Ownable {
    ...
    uint256 private _maxSupply = 27500000 trx; // 27.5 million APE
    ...
    function _mint(address _to, uint256 _amount) internal {
        require(_to != address(0), 'APE::_mint: mint to the zero address');
    ...
    function mint(address _to, uint256 amount) public onlyOwner returns (bool) {
        require(_totalSupply.add(amount, 'APE::mint: mint amount overflows') <= _maxSupply, 'APE::mint: max supply exceeded');
        _mint(_to, amount);
        return true;
    }
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            'APE::delegateBySig: invalid signature'
        );
        require(
            nonce == nonces[signatory]++,
            'APE::delegateBySig: invalid nonce'
        );
        require(block.timestamp <= expiry, 'APE::delegateBySig: signature expired');
        ...
        returns (uint256)
    {
        require(
            ...
            'APE::getPriorVotes: not yet determined'
        );
    ...
    function _transferTokens(
        ...
        require(
            src != address(0),
            'APE::_transferTokens: cannot transfer from the zero address'
        );
        require(
            dst != address(0),
            'APE::_transferTokens: cannot transfer to the zero address'
        );

        balances[src] = balances[src].sub(amount, 'APE::_transferTokens: transfer amount exceeds balance');
        balances[dst] = balances[dst].add(amount, 'APE::_transferTokens: transfer amount overflows');
        ...
        if (srcRep != dstRep && amount > 0) {
            if (srcRep != address(0)) {
                ...
                uint256 srcRepOld = srcRepNum > 0
                    ...
                    : 0;
                uint256 srcRepNew = srcRepOld.add(amount, 'APE::_moveVotes: vote amount underflows');
            ...
            if (dstRep != address(0)) {
                ...
                uint256 dstRepOld = dstRepNum > 0
                    ...
                    : 0;
                uint256 dstRepNew = dstRepOld.add(amount, 'APE::_moveVotes: vote amount overflows');
