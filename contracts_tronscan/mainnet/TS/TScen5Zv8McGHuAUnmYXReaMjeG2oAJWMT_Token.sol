...
contract Token is Context, ITRC20, Ownable {
    ...
    string private _name = 'FTB';
    string private _symbol = 'FTB';
    uint8 private _decimals = 18;
    uint256 private _totalSupply = 9999 * 10**uint256(_decimals);
    ...
    uint256 private maxSale= 10 * 10**uint256(_decimals);
    uint256 private maxBuy= 10 * 10**uint256(_decimals);
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if(Address.isContract(sender)&&!_isExcludedFromFee[sender]&&!_isExcludedFromFee[recipient] ){
          require((balanceOf(recipient)+amount) <= 20*10**uint256(_decimals));
