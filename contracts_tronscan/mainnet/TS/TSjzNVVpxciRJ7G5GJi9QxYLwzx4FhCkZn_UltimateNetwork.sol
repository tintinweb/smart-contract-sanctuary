//SourceUnit: ultimateNetwork.sol
...
contract UltimateNetwork {
   ...
   struct UserStruct {
    ...
    constructor(address _owner, address _wallet1, address _wallet2) public {
        ...
        LEVEL_PRICE[2] = 1250*1e6;
        LEVEL_PRICE[3] = 2500*1e6;
        LEVEL_PRICE[4] = 9000*1e6;
        LEVEL_PRICE[5] = 50000*1e6;
        LEVEL_PRICE[6] = 500000*1e6;
        LEVEL_PRICE[7] = 5000000*1e6;
       
        
        HOLD_LEVEL_PRICE[2] = 834*1e6;
        HOLD_LEVEL_PRICE[3] = 1500*1e6;
        HOLD_LEVEL_PRICE[4] = 4167*1e6;
        HOLD_LEVEL_PRICE[5] = 27778*1e6;
        HOLD_LEVEL_PRICE[6] = 200000*1e6;
    ...
    function regUser(uint _referrerID,uint _userId) public payable {
        ...
        require(msg.tokenvalue == LEVEL_PRICE[1]+LEVEL_PRICE[2], 'Incorrect Value RU');
    ...
    function buyLevel(uint _level,address _user) public payable {
        ...
        require(users[_user].holdamount >= LEVEL_PRICE[_level], 'Incorrect Token Value');
        require(users[_user].activeLevel[_level-1],"Buy Previous Level");
    ...
    function payForLevel(uint _level, address _user, address _sponsor) internal {
        ...
        if(_level == 1) {
                ...
                address(uint160(actualReferer)).transferToken(LEVEL_PRICE[_level],tokenId);
                sent = true;
                if (sent) {
            ...
            if(users[actualReferer].activeLevel[_level]) {
                ...
                {
                address(uint160(actualReferer)).transferToken(LEVEL_PRICE[_level].mul(earnings).div(100),tokenId); //  70% for level income
                sent = true;
                }
                else
                {
                    ...
                    address(uint160(actualReferer)).transferToken(left_amt,tokenId); //  70% for level income
                    sent = true;
                ...
                if (sent) {
                        ...
                        address(uint160(_sponsor)).transferToken(LEVEL_PRICE [_level].mul(directSponsorFees).div(100),tokenId); // direct income on level purchase
                        emit getSponsorBonusEvent(_sponsor, _user, _level, now);
                      
                    address(uint160(wallet1)).transferToken(LEVEL_PRICE[_level].mul(adminFees).div(100),tokenId); // admin fee
