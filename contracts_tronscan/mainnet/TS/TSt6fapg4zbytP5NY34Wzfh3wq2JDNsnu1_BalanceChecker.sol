...
function allowances(address owner, address spender, address[] tokens) external view returns (uint[]) {
    uint[] memory _allowances = new uint[](tokens.length);
    
    for (uint i = 0; i < tokens.length; i++) {
        if (tokens[i] != address(0x0)) { 
            _allowances[i] = tokenAllowance(owner, spender, tokens[i]);
        } else {
            _allowances[i] = Token(0x0).allowance(owner,spender); 
        }
    }  
    return _allowances;
  }
