...
contract Tron120 {
    ...
    uint private maxDeposite = 20000000000;
    uint private divisor = 100;
    uint private devCommission = 5;
    uint private rate = 20;      
    uint private releaseTime = 1599760800;
    ...
    struct Player {
        ...
        uint256 aff1sum;       //5 Level Referral Commission
        uint256 aff2sum;
        uint256 aff3sum;
        uint256 aff4sum;
        uint256 aff5sum;
            ...
            if(players[msg.sender].deposits[pa].amount > 0){
                ...
                for(uint i=0 ; i< pos;i++){
                    ...
                    if(players[msg.sender].deposits[r].amount ==0){
                        players[msg.sender].deposits[r] = Deposit(msg.value,now,now+1 days,0);
                        return true;
                    }
                    t++;
                }
            }else{
                players[msg.sender].deposits[pa] = Deposit(msg.value,now,now+1 days,0);
                return true;
            }
        }else{
            players[msg.sender].deposits[p] = Deposit(msg.value,now,now+1 days,0);
            return true;
        }
    }
    function withdraw(uint pos) public returns(uint256){
        ...
        uint256 total = getPercent(player.deposits[pos].amount,rate)*6;
        uint256 pft = secPassed*(total/24/60/60);
