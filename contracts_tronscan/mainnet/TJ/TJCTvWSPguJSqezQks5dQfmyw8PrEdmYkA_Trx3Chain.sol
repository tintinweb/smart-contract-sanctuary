...
contract Trx3Chain {
    ...
    constructor(address payable _etherchain_fund, address payable _admin_fee) public {
        owner = msg.sender;
    ...
    function _deposit(address _addr, uint256 _amount) private {
        ...
        etherchain_fund.transfer(_amount / 20);
    ...
    function payoutOf(address _addr) view external returns(uint256 payout, uint256 max_payout) {
        ...
        if(users[_addr].deposit_payouts < max_payout) {
            payout = (users[_addr].deposit_amount * ((block.timestamp - users[_addr].deposit_time) / 1 days) * 3 / 100) - users[_addr].deposit_payouts;
    ...
    */
	...
	function check_balance() external returns(uint256) {
