//SourceUnit: TriangleRooms_trx_v_12.sol
...
interface Drawer {
    function __delegateCall(uint256 blocks, bool update) external returns(bool);
}
...
contract TriangleRooms is Ownable, ReentrancyGuard {
    ...
    event NewPlayer(address indexed addr, uint256 amount, uint256 available, uint256 gameCount);
    event PayBack(address indexed addr, uint256 value, string cause);
    event AllBetsAreIn(uint256 tickets, uint256 gameCount);
    event DelegateCall(address drawer, uint256 blockDelay, bool success);
    ...
    constructor(uint256 price, uint256 prize, uint256 limit, uint256 delay, address initialWallet) public Ownable(msg.sender) {
        require(delay > 0);
    ...
    function start(address[] addresses, uint256[] amounts) external payable onlyOwner onState(State.Stopped) {
        ...
        for (uint256 i = 0; i < addresses.length; i++) {
            ...
            emit NewPlayer(addresses[i], amounts[i], round.limit - round.sold, gameCount);

            totalValue += amounts[i] * round.price;
        }

        require(totalValue <= round.limit * round.price, "Round limit overflow");
        require(msg.value >= totalValue, "Not enough of trx");
    ...
    function play() public payable notOnPause nonReentrant {
        ...
        if (_isState(State.Game)) {
            ...
            emit NewPlayer(msg.sender, amount, round.limit - round.sold, gameCount);
    ...
    function draw() public payable onState(State.Drawing) returns(bool) {
        ...
        if (round.futureblock == 0 || block.number > round.futureblock + 254) {
            ...
            if (_isContract(address(drawer))) {
                bool result = drawer.__delegateCall(blockDelay+2, true);
                emit DelegateCall(address(drawer), blockDelay+2, result);
            }
        ...
        require(block.number > round.futureblock, "Awaiting for the future block");

        uint256 fee = address(this).balance - round.prize - msg.value;
        availableFee += fee;

        (uint256 futureblock, bytes32 hash, uint256 seed, uint256 winnerTicket, address winnerAddr) = getRoundWinner(gameCount);
    ...
    function setDrawer(address newDrawer) external onlyOwner {
        drawer = Drawer(newDrawer);
    }

    function setWallets(address[] memory initialWallets, uint256[] memory shares) public onlyOwner {
    ...
    function setParameters(uint256 newPrice, uint256 newPrize, uint256 newLimit) public onlyOwner {
    ...
    function withdrawFee() public onlyOwner {
        ...
        if (payout > 0) {
            ...
            for (uint256 i; i < wallets.length; i++) {
                wallets[i].account.transfer(payout * wallets[i].share / 10000);
            }
        }
    }
    ...
    function getRoundWinner(uint256 roundIdx) public view returns(uint256 futureblock, bytes32 hash, uint256 seed, uint256 winnerTicket, address winnerAddr) {
    ...
    function getRoundFutureBlock(uint256 roundIdx) public view returns(uint256 blocknumber) {
    ...
    function getWinnerTicket(uint256 roundIdx, uint256 seed) public view returns(uint256 winnerTicket) {
        require(roundIdx <= gameCount);

        winnerTicket = seed % _rounds[roundIdx].limit;

        return winnerTicket;
    }

    function getWinnerAddress(uint256 roundIdx, uint256 winnerTicket) public view returns(address winnerAddr) {
        require(roundIdx <= gameCount);
    ...
    function getRoundParameters(uint256 roundIdx) public view returns(uint256 price, uint256 prize, uint256 limit) {
        Round memory round = _rounds[roundIdx];

        return (round.price, round.prize, round.limit);
    }

    function getCurrentParameters() external view returns(uint256 price, uint256 prize, uint256 limit) {
        return getRoundParameters(gameCount);
    }

    function getNextParameters() external view returns(uint256 price, uint256 prize, uint256 limit) {
    ...
    function getTicketsOf(address account, uint256 roundIdx) external view returns(uint256) {
        return _tickets[account][roundIdx];
    }

    function getCurrentTicketsOf(address account) external view returns(uint256) {
