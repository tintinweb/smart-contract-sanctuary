...
/**
 * @title The RichesToken contract.
 */
contract RichesToken is Token {
    ...
    uint256 public contractPercent = 12;
    ...
    constructor() public {
        wallet = msg.sender;
        address[] memory _rec = new address[](1);
        _rec[0] = msg.sender;
        uint256[] memory _am = new uint256[](1);
        _am[0] = 10;
        distribution(_rec, _am, 0);
        restart();
    }
    ...
    function buy(address referrer) public payable {
        ...
        (wallet.send(msg.value.mul(contractPercent).div(100)));
