...
contract HMETokenPool is LPTokenWrapper, DateWrapper {
    ...
    modifier checkHalve(){
        if (block.timestamp.add(extraTime) >= periodFinish) {
            ...
            if (level >= 36) {
                ...
                rewardRate = uint256(5920000 * 10 ** 12).div(30 * DURATION);
            } else {
                if(level == 5) {
                    rewardRate = uint256(12).add(6).mul(10000).mul(10 ** 12).div(DURATION);
                } else {
                    rewardRate = uint256(12).add(level).mul(10000).mul(10 ** 12).div(DURATION);
                }
            }
            level++;
