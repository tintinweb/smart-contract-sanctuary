...
contract TRONGLOBAL {
	...
	uint256[] public REFERRAL_PERCENTS = [70, 50,40,30,20];
	...
	uint256 constant public DAY_STEP = 10 days;
    address owner;
	...
	function withdraw() public {
		...
		require(block.timestamp > user.withdrawcheckpoint + 1 days , "Only once a day");
	    ...
	    msg.sender.transfer(totalAmount.mul(600).div(PERCENTS_DIVIDER));  
	    
        user.withdraw_bonus=user.withdraw_bonus.add(totalAmount);
	    totalWithdrawn = totalWithdrawn.add(totalAmount.mul(600).div(PERCENTS_DIVIDER));
	    user.recived_bonus=user.recived_bonus.add(totalAmount.mul(600).div(PERCENTS_DIVIDER));
		user.reinvest_wallet=user.reinvest_wallet.add(totalAmount.mul(400).div(PERCENTS_DIVIDER));
	...
	function getRemainingTime(address _userAddress)public view returns (uint256 x) 
	{
	    uint256 remaintime=0;
	    User storage user = users[msg.sender];
	    remaintime=(user.withdrawcheckpoint + 1 days).sub(block.timestamp); 
	    if (remaintime<0)
	    {
	        remaintime=0;
	    }
   ...
   function getUserFundDtl(address userAddress) public view returns(uint256 teamx,uint256 directfund, uint256 withdrawal,uint256 received,uint256 reinvestWallet) {
	...
	function getUserLevelBonus(address userAddress) public view returns(uint256 l1,uint256 l2,uint256 l3,uint256 l4, uint256 l5,uint256 l6) {
	   uint256 level_1=0; 
	   uint256 level_2=0; 
	   uint256 level_3=0;
	   uint256 level_4=0;
	   ...
	   for(uint256 m=0;m<users[userAddress].levelcomm.length;m++)
	    {
	        if(m==0)
	        {
	            level_1=users[userAddress].levelcomm[m];
	            
	        }
	        
	        if(m==1)
	        {
	            level_2=users[userAddress].levelcomm[m];
	           
	        }
	        if(m==2)
	        {
	            level_3=users[userAddress].levelcomm[m];
	           
	        }
	        if(m==3)
	        {
	            level_4=users[userAddress].levelcomm[m];
	         
	        }
	        if(m==4)
	        {
	            level_5=users[userAddress].levelcomm[m];
	            
	        }
	        if(m==5)
	        {
	            level_6=users[userAddress].levelcomm[m];
	...
	function getUserTeamBV(address userAddress) public view returns(uint256 b1,uint256 b2,uint256 b3,uint256 b4, uint256 b5,uint256 b6) {
	   ...
	   for(uint256 m=0;m<users[userAddress].levelcomm.length;m++)
	    {
	        if(m==0)
	        {
	            ...
	            bv_1=users[userAddress].teambv[m];
	        }
	        
	        if(m==1)
	        {
	           
	            bv_2=users[userAddress].teambv[m];
	        }
	        if(m==2)
	        {
	           
	            bv_3=users[userAddress].teambv[m];
	        }
	        if(m==3)
	        {
	            
	            bv_4=users[userAddress].teambv[m];
	        }
	        if(m==4)
	        {
	           
	            bv_5=users[userAddress].teambv[m];
	        }
	        if(m==5)
	        {
	            
	            bv_6=users[userAddress].teambv[m];
	        }
	    }
	    
	   
		return (bv_1,bv_2,bv_3,bv_4,bv_5,bv_6);
	}
