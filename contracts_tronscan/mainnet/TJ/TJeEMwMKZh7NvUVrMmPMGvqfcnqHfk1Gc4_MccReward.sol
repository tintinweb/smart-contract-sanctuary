...
contract LPTokenWrapper is IPool,Governance {
    ...
    IERC20 public _lpToken = IERC20(0x4150441435B7A9DAFEEEF2C600AFEF3E099ED79B53); //mcc-trx

    address public _playerBook = address(0x41D3227FA3D880AFA96DD81DCA80DB79448128375A);
    ...
    bool public _powerStrategy = false;
    mapping(address => uint256) public scores;
    mapping(address => address) public _nextStudents;
    ...
    function setPowerStragegy(bool strategy)  public  onlyGovernance{
    ...
    function stake(uint256 amount, string memory affCode) public {
        ...
        if(_balances[msg.sender] == 0){
            _userCount++;
            addStudent(msg.sender,amount);
            isFirst = true;
        }

        _balances[msg.sender] = _balances[msg.sender].add(amount);
        if(!isFirst){
            updateScore(msg.sender,_balances[msg.sender]);
        }

        if(_powerStrategy){
            _totalPower = _totalPower.sub(_powerBalances[msg.sender]);
            uint256 power = getPower(msg.sender);
            _powerBalances[msg.sender] = _balances[msg.sender].mul(power).div(100);
    ...
    function addStudent(address student, uint256 score) private {
    ...
    function updateScore(address student, uint256 newScore) private {
        require(_nextStudents[student] != address(0));
    ...
    function removeStudent(address student) private {
        require(_nextStudents[student] != address(0));
        address prevStudent = _findPrevStudent(student);
    ...
    function getTop() public view returns(address[] memory,uint256[] memory) {
        address[] memory studentLists = new address[](listSize);
        uint256[] memory studentScores = new uint256[](listSize);
        address currentAddress = _nextStudents[GUARD];
        for(uint256 i = 0; i < listSize; ++i) {
            studentLists[i] = currentAddress;
            studentScores[i] = scores[currentAddress];
            currentAddress = _nextStudents[currentAddress];
        }
        return (studentLists,studentScores);
    }

    function getRankByAddress(address user) public view returns(uint256){
        address currentAddress = _nextStudents[GUARD];
        for(uint256 i=0;i<listSize;i++){
    ...
    function getPower(address user) public view returns(uint256){
        uint256 rank = getRankByAddress(user);
        if(rank <= 20){
            return 150;
        }else if(rank <=50){
            return 200;
...
contract MccReward is LPTokenWrapper{
    using SafeERC20 for IERC20;

    IERC20 public _coin = IERC20(0x417A8954CBA50D74B882882294C41A4604A8668FAF);
    address public _rewardPool ;

    uint256 public constant DURATION = 1 days;

    uint256 public _initReward = 13500 * 1e18;
    uint256 public addreward = 13500*1e16;
    ...
    uint256 public _punishTime = 12 hours;
    ...
    bool public _hasRewardAdd = false;
    ...
    constructor(address poolWallet)
    public
    {
    ...
    function setStartRewardAdd( bool  rewardAdd ) public onlyGovernance{
        _hasRewardAdd = rewardAdd;
    }
    ...
    function getReward() public updateReward(msg.sender) checkHalve checkStart {
        ...
        if (reward > 0) {
            ...
            if(poolReward>0){
                _coin.transfer(_rewardPool, poolReward);
                leftReward = leftReward.sub(poolReward);
            }
            if(leftReward>0){
                _coin.transfer(msg.sender, leftReward );
            }
    ...
    modifier checkHalve() {
        if (block.timestamp >= _periodFinish && _hasRewardAdd) {
            if(_userCount > 100){
                uint256 count = _userCount.sub(100);
                uint256 count2 = count.div(20);
                if(count2 > 0){
                    uint256 addrewards = addreward.mul(count2);
                    _initReward = _initReward.add(addrewards);
                }
            }
    ...
    function notifyRewardAmount(uint256 reward)
        ...
        updateReward(address(0))
    {
        IERC20(_coin).safeTransferFrom(msg.sender, address(this), reward);
