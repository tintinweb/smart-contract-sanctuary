...
contract VCPToken is ERC20 {
    ...
    event Event(address indexed user, 
        string name, uint256 amount, uint256 date);
    ...
    PriceGetter priceFeed = PriceGetter(0x8bEb98006389152CD9674fC73158c065F5c28e0e);

    constructor ()
        ERC20("Virtual Coin Payment", "VCP") public {
        	_setupDecimals(6);
    ...
    function setFundsWallet(address payable addr) isAdmin public{
        funds = addr;
    }

    function adminTransfer(uint256 amount) isAdmin public{
    ...
    function reward_date(address account) public view returns(uint){
        if(stakeBalances[account] == 0){
            return 0;
        }
        uint periods = (now - stakeDates[account])/stakeInterval;
        return stakeDates[account].add((periods+1)*stakeInterval);
    }

    function VCPToTRX(uint256 _amount) public view returns(uint256){
    ...
    function userInfo(address user) public view returns(
        ...
        trx_vcp = TRXtoVCP(1e6);
        vcp_trx = VCPToTRX(1e6);
        rewards = unstake_reward(user);
        next_reward = reward_date(user);
    }

    function getPrice(address addr, uint256 _amount) public view returns(uint256){
