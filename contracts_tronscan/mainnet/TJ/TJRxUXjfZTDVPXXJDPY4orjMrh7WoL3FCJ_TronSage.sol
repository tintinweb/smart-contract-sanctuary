...
uint totCurrnt;
    ...
    mapping(uint8 => uint) public levelPrice;
     ...
     //emit RegDetails(userAddress,userAddress1,msg.sender,  msg.value, mid);  
      event SlotDetails(address indexed userAddress,address indexed PayeerAdd,uint indexed mid,  uint  refid,uint matrix,uint lvl);
       event SlotDetails4(address indexed userAddress,address indexed userAddress1,address indexed PayeerAdd,uint  mid,  uint  refid,uint matrix,uint lvl);
     ...
     // event SlotDetails(uint8 matrix indexed, uint8 level indexed,address userAddress indexed,address PayeerAdd indexed, uint mid indexed,uint refid indexed);
      //event SlotDetails(address indexed user,address indexed user1,address indexed user2, uint  payment, uint  userId);
        event RegDetails(address indexed user1,address indexed user21,address indexed user22,address  Payuser, uint  payment, uint  userId);
    ...
    function registrationnEW(address user1,address user21,address user22, uint mid,address referrerAddress) external payable {
          require(msg.value == 500 trx, "registration cost 500 trx");  
          uint tr=250 trx; 
           uint tr1=187.5 trx;   
             uint tr2=62.5 trx;    
    
//event RegDetails(address indexed user1,address indexed user21,address indexed user22,address indexed Payuser, uint  payment, uint  userId);
        
         Execution1(user1,tr);
         Execution1(user21,tr1);
         Execution1(user22,tr2);
           registration(msg.sender, msg.sender);
            emit RegDetails(user1,user21, user22,msg.sender,  msg.value, mid);  
 // emit RegistrationNew(userAddress, referrerAddress, users[userAddress].id, users[referrerAddress].id);
       ...
       function buyNewLevelEW4(uint8 matrix, uint8 level,address userAddress,address userAddress1, uint mid,uint refid) external payable {
         require(matrix == 1 || matrix == 2, "invalid matrix");
        require(msg.value == levelPrice[level], "invalid price");
        require(level > 1 && level <= LAST_LEVEL, "invalid level");

      uint tr1=msg.value * 25 /100;
      uint tr2=msg.value * 75 /100;
        Execution(userAddress,tr1);
        Execution(userAddress1,tr2);
        emit SlotDetails4(userAddress,userAddress1, msg.sender,  mid , refid,matrix,level );
    ...
    function registration(address userAddress, address referrerAddress) private {
      require(msg.value == 500 trx, "registration cost 850");
