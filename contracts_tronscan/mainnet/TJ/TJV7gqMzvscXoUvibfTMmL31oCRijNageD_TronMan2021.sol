...
contract  TronMan2021{
	uint256 constant public FIX_PERCENT = 100;
        uint256 constant public MAX_DAILY_WITHDRAWAL = 15000 trx;
        uint256 constant public MAX_DAILY_Ref_WITHDRAWAL = 5000 trx;
	uint256 constant public INVEST_MIN_AMOUNT = 1 trx;		
	uint256[5] public REFERRAL_PERCENTS = [40, 30, 10, 10, 10];
	uint256[5] public REINVEST_REFERRAL_PERCENTS = [5, 4, 1, 1, 2];		
	uint256 constant public MARKETING_FEE = 60;
	uint256 constant public SUPPORTING_FEE = 60;
    ...
    function pauseWithdrawal() public view returns(bool){
        uint8 hour = uint8((block.timestamp / 60 / 60) % 24);		//hold wdraw  4am - 8am UTC
        if(hour >= 4 && hour <= 7){
            return true;
        }
        else{
	...
	function withdraw(uint _plan) public {
		...
		require(!pauseWithdrawal(), "Withdrawal paused between 4-8 am utc");
