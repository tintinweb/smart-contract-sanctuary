...
contract Tronreboot
{
    ...
    address payable first_investor;

    uint8 investment_minutes;
    ...
    function _updateTotalPayout(address _addr) private{
        ...
        for(uint256 i = 0; i < player.deposits.length; i++) {
            ...
            uint256 time_end = dep.time + investment_minutes * 60;
            uint256 from = player.last_payout > dep.time ? player.last_payout : dep.time;
            uint256 to = block.timestamp > time_end ? time_end : uint256(block.timestamp);

            if(from < to) {
                uint256 depositDividendAmount = dep.amount * (to - from) * investment_perc / investment_minutes / 6000;
    ...
    function investmentsInfo(address _addr) view public returns(uint256[] memory endTimes, uint256[] memory amounts, uint256[] memory totalWithdraws) {
        ...
        for(uint256 i = 0; i < player.deposits.length; i++) {
          ...
          _endTimes[i] = dep.time + investment_minutes * 60;
        }
        return (
          _endTimes,
          _amounts,
    ...
    function _checkInvestmentStatus(address _addr) private {
        ...
        for(uint256 i = 0; i < player.deposits.length; i++) {
            PlayerDeposit storage dep = player.deposits[i];
            if(dep.isActive) {
                uint endTime = dep.time + investment_minutes * 60;
