...
contract ERC20 is IERC20 {
    ...
    uint256 private _totalToPool = 0;  //use totalOut for Prevent Locked GCC overflow

    uint256 private _totalBurnAmount;  //totalBurnAmount

    uint256 private _sayNo = 219 * 1000000 ; //Can't burn GCC Is 10% of the totalSupply
      ...
      *     e.Have the permission to unlock and transfer tokens, but it depends on the unlocking rules (10000 GCC unlocked every day)
      *     f.Has the unlock time setting permission, but can only set one time  the unlock time node once
      */
    address private superAdmin;

    // Secondary The only management is to manage illegal users to obtain GCC by abnormal means
    ...
    function getTotalUnLockAmount() internal view returns (uint256) {
        ...
        if(totalUnLock >= (8888 * 10000 * 1000000)){
           return 8888 * 10000 * 1000000;
        }
 ...
 * `ERC20` functions.
 */
contract GCC is ERC20, ERC20Detailed {

    /**
     * @dev Constructor that gives msg.sender all of existing tokens.
     */
    constructor () public ERC20Detailed("GCC", "GCC", 6) {
        _mint(msg.sender, 21900000 * 1000000 );
    }
}
