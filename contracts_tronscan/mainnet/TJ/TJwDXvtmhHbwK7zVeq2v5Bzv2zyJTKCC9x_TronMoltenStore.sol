...
interface OldUser {
    ...
    function setNew(address _account) external;
}
...
contract TronMoltenStore is Ownable, StoreDomain {
    ...
    constructor(ITronPools _pools, ITronState _state, IPrizeState _prizeState, OldUser _oldUser) {
        pools = _pools;
        state = _state;
    ...
    function migrate(ITronPools _pools, ITronState _state, IPrizeState _prizeState, OldUser _oldUser) external onlyOwner {
        pools = _pools;
        state = _state;
        prizeState = _prizeState;
        oldUser = _oldUser;
    ...
    function _withdraw(address payable _account, uint256 _liquid, uint256 _invitee, uint256 _group, uint256 _prize, uint256 _store) private {
        ...
        if (_user.liquid != _user.lastLiquid && oldUser.isOld(_account)) {
            oldUser.setNew(_account);
        }

        if (oldUser.isOld(_account)) {
    ...
    function updateGroup(address _account, uint256 _amount) private  {
        ...
        while(parent != address(0x0) && dis <= 21) {
            ...
            if (dis <= user.inviteeCount && user.liquid > 0) {
                if (dis == 1) {
