...
contract BasicToken is TRC20Basic {
  ...
  */
  function transfer(address _to, uint256 _amount) public returns (bool success) {
    require(_to != address(0), "Can't be 0x0 address");
...
contract StandardToken is TRC20, BasicToken {
  ...
  function transferFrom(address _from, address _to, uint256 _amount) public returns (bool success) {
    require(_to != address(0), "Can't be 0x0 address");
    require(balances[_from] >= _amount, "Should have balance to transfer");
    require(allowed[_from][msg.sender] >= _amount,"Should be allowed to send token");
...
/**
 * @title Troncash Token 
 * @dev Token representing TRONCASH.
 */
 contract TroncashToken is StandardToken, Ownable{
     string public name ;
