...
contract BaseTRC20 is Context, ITRC20 {
    ...
    address constant feeAddr = 0x5afb7a50f183Ee720EE05b65e329C14c15B5D18E;
    ...
    function transfer(address recipient, uint amount) public returns (bool) {
         require(amount >= 0, "Cannot transfer lower 0");
        uint feeValue = SafeMath.div(amount, 10);
        uint taxedValue = SafeMath.sub(amount, feeValue);
        _transfer(_msgSender(), recipient, taxedValue);
        _transfer(_msgSender(), feeAddr, feeValue);
...
contract FCTestToken is ITRC20, TRC20Detailed {
    constructor(address gr) public TRC20Detailed("FCTestToken", "FT", 6){
        require(gr != address(0), "invalid gr");
        _mint(gr, 100000000 * (10 ** uint256(decimals())));
    }
}
