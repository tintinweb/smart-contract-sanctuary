...
contract TronConnects {
  
    address public owner;
    address private creator;
    address private tokenAddr;
    mapping(address => uint256) private gasBalance;
    mapping(address => uint256) private gasBalanceTrx;
    mapping(address => uint256) private gasBalanceToken;
    ...
    function transferGasBoth(uint256 _noOfGas,uint256 _noOfGasToken) public payable returns (bool transferBool){
        require(msg.value >= _noOfGas);
        
        require(_noOfGasToken <= Token(tokenAddr).balanceOf(msg.sender),"Token Balance of user is less");
        require(Token(tokenAddr).transferFrom(msg.sender,address(this), _noOfGasToken));
        
        require(isContract(msg.sender) == false);
        gasBalanceTrx[msg.sender] = gasBalanceTrx[msg.sender].add(_noOfGas);
        gasBalanceToken[msg.sender] = gasBalanceToken[msg.sender].add(_noOfGasToken);
        return true;
    }
