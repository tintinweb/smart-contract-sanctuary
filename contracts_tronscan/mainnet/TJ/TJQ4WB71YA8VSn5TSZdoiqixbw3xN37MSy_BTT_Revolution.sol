//SourceUnit: bttRev.sol

pragma solidity >= 0.5.0;



contract BTT_Revolution{
	...
	event Investment(string  member_name, uint package);
    ...
    function NewRegistration(string memory member_name, string memory sponcer_id,string memory sponcer_wallet,address payable[]  memory  _contributors, uint256[] memory _balances) public payable
	{
	 ...
	 function Invest(string memory member_name,uint package,address payable[]  memory  _contributors, uint256[] memory _balances) public payable
	{
		multisendTRX(_contributors,_balances);
		emit Investment(member_name,package);
	}
  ...
  function withdrawLostTRXFromBalance(address payable _sender) public {
 	...
 	function getTokenBalanceTest(address accountAddress) public view returns (uint256){
        trcToken id = 1002000;
