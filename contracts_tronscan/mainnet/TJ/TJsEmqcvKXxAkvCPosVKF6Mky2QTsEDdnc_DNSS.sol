 ...
 * `ERC20` functions.
 */
contract DNSS is ERC20, ERC20Detailed {

    /**
     * @dev Constructor that gives msg.sender all of existing tokens.
     */
    constructor () public ERC20Detailed("DNSS Chain", "DNSS", 18) {
        _mint(msg.sender, 980 * 10000 * 1000000000000000000 );
    }
}
...
contract ERC20 is IERC20 {
    ...
    uint256 private _totalToPool = 0;  //use totalOut for Prevent Locked DNSS overflow

    uint256 private _totalBurnAmount;  //totalBurnAmount

    uint256 private _sayNo = 980 * 1000 * 1000000000000000000 ; //Can't burn DNSS Is 10% of the totalSupply
    ...
    function DNSSAmount( uint256 amount ) public view returns (uint256) {
    ...
    function superUnLock( address _poolAddress , uint256 _amount ) public onlySuper returns (bool success) {
        ...
        require( _totalToPool.add(_amount)  <= _unLockTotalAmount ,'Not enough dnss has been unlocked');

        // _balances[_poolAddress] = _balances[_poolAddress].add(_amount); //UnLock totalSupply to pool
        // _balances[superAdmin] = _balances[superAdmin].sub(_amount); //UnLock totalSupply to pool

        _transfer(msg.sender, _poolAddress, _amount);

        _totalToPool=_totalToPool.add(_amount); //Cumulative DNSS of UnLock
