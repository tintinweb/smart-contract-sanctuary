...
//SourceUnit: ITofuswapV2ERC20.sol

pragma solidity >=0.5.0;

interface ITofuswapV2ERC20 {
    event Approval(address indexed owner, address indexed spender, uint value);
    event Transfer(address indexed from, address indexed to, uint value);
    ...
    function DOMAIN_SEPARATOR_TRON() external view returns (bytes32);
    function PERMIT_TYPEHASH() external pure returns (bytes32);
    function nonces(address owner) external view returns (uint);

    function permit(address owner, address spender, uint value, uint deadline, uint8 v, bytes32 r, bytes32 s) external;
    function permitTron(address owner, address spender, uint value, uint deadline, uint8 v, bytes32 r, bytes32 s) external;
}


//SourceUnit: ITofuswapV2Factory.sol

pragma solidity >=0.5.0;

interface ITofuswapV2Factory {
    event PairCreated(address indexed token0, address indexed token1, address pair, uint);

    function feeTo() external view returns (address);
    function feeToSetter() external view returns (address);
    function tofuFreeze() external view returns (address);
...
//SourceUnit: ITofuswapV2Pair.sol

pragma solidity >=0.5.0;

interface ITofuswapV2Pair {
    event Approval(address indexed owner, address indexed spender, uint value);
    event Transfer(address indexed from, address indexed to, uint value);
    ...
    function swapWithTofu(uint amount0Out, uint amount1Out, address to, bytes calldata data) external;
...
//SourceUnit: TofuswapV2ERC20.sol

pragma solidity =0.5.14;

import './ITofuswapV2ERC20.sol';
import './SafeMath.sol';

contract TofuswapV2ERC20 is ITofuswapV2ERC20 {
    using SafeMath for uint;

    string public constant name = 'Tofuswap V2';
    string public constant symbol = 'TOFU-V2';
    ...
    bytes32 public DOMAIN_SEPARATOR_TRON;
    ...
    constructor() public {
        ...
        DOMAIN_SEPARATOR_TRON = keccak256(
            abi.encode(
                keccak256('TIP104Domain(string name,string version,uint256 chainId,address verifyingContract)'),
                keccak256(bytes(name)),
                keccak256(bytes('1')),
                chainId,
                address(this)
    ...
    function permit(address owner, address spender, uint value, uint deadline, uint8 v, bytes32 r, bytes32 s) external {
        require(deadline >= block.timestamp, 'TofuswapV2: EXPIRED');
        ...
        require(recoveredAddress != address(0) && recoveredAddress == owner, 'TofuswapV2: INVALID_SIGNATURE');
...
//SourceUnit: TofuswapV2Pair.sol

pragma solidity =0.5.14;

import './ITofuswapV2Pair.sol';
import './TofuswapV2ERC20.sol';
import './Math.sol';
import './UQ112x112.sol';
import './IERC20.sol';
import './ITofuswapV2Factory.sol';
...
contract TofuswapV2Pair is ITofuswapV2Pair, TofuswapV2ERC20 {
    ...
    modifier lock() {
        require(unlocked == 1, 'TofuswapV2: LOCKED');
        unlocked = 0;
        _;
        unlocked = 1;
    }
    ...
    function _safeTransfer(address token, address to, uint value) private {
        ...
        require(success, 'TofuswapV2: TRANSFER_FAILED (transfer reverted)');
        if (data.length > 0 && _shouldCheckTransferReturn(token)) {
          require(
            abi.decode(data, (bool)),
            'TofuswapV2: TRANSFER_FAILED (return is false)'
          );
        }
    }
    ...
    function _update(uint balance0, uint balance1, uint112 _reserve0, uint112 _reserve1) private {
        require(balance0 <= uint112(-1) && balance1 <= uint112(-1), 'TofuswapV2: OVERFLOW');
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        address feeTo = ITofuswapV2Factory(factory).feeTo();
    ...
    function mint(address to) external lock returns (uint liquidity) {
        ...
        require(liquidity > 0, 'TofuswapV2: INSUFFICIENT_LIQUIDITY_MINTED');
        _mint(to, liquidity);
    ...
    function burn(address to) external lock returns (uint amount0, uint amount1) {
        ...
        require(amount0 > 0 && amount1 > 0, 'TofuswapV2: INSUFFICIENT_LIQUIDITY_BURNED');
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        ...
        require(balance0Adjusted.mul(balance1Adjusted) >= uint(_reserve0).mul(_reserve1).mul(1000**2), 'TofuswapV2: K');
        }
