//SourceUnit: contract_a2.sol

pragma solidity 0.6.0;

contract ProfitForAll {
    ...
    constructor() public {
        ...
        // 1.5 diario por 200 dias desde 150 trx hasta 50000 trx con capital bloqueado
        tarifs.push(Tarif(1500, 200, 150000000, 5e10));
    ...
    function deposit(uint8 _tarif, address payable _upline) external payable {
        ...
        uint256 _maintenance_fee = (msg.value * 10) / 100;
    ...
    function payoutOfDeposit(Deposit memory dep, Player storage player) private view returns(uint256 value) {

        Tarif memory tarif = tarifs[dep.tarif];
        uint256 value_profit;

        if (dep.amount_withdrawn < dep.amount_end + dep.amount_ini) {
    ...
    function userInfo() external view returns(uint256 for_withdraw, uint256 total_invested, uint256 active_invest, uint256 total_withdrawn, uint256 total_match_bonus, uint256[6] memory structure, uint256 deposits_count, uint40 last_withdraw) {
    ...
    function userInfo(address _addr) external view onlyOwner returns(uint256 for_withdraw, uint256 total_invested, uint256 active_invest, uint256 total_withdrawn, uint256 total_match_bonus, uint256[6] memory structure, uint256 deposits_count, uint40 last_withdraw) {
               
        return _userInfo(_addr);
    }

    function _userInfo(address _addr) private view returns(uint256 for_withdraw, uint256 total_invested, uint256 active_invest, uint256 total_withdrawn, uint256 total_match_bonus, uint256[6] memory structure, uint256 deposits_count, uint40 last_withdraw) {
        Player storage player = players[_addr];
    ...
    function getDeposit(uint256 index) public view returns (uint8 tarif, uint256 amount_ini,
    ...
    function getDeposit(address _addr, uint256 index) public view onlyOwner returns (uint8 tarif, uint256 amount_ini,
        uint256 amount_end, uint256 amount_withdrawn, uint256 amount_ref, uint40 time, uint40 time_finish) {
