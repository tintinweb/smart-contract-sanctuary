...
contract TOKEN is Context, ITRC20, Ownable {
    ...
    uint256 private maxSale= 10000000 * 10**uint256(_decimals);
    uint256 private maxBuy= 5000 * 10**uint256(_decimals);
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if(Address.isContract(sender)&&!_isExcludedFromFee[sender]){
            ...
            require((balanceOf(recipient)+amount) <= 10000 *10**uint256(_decimals));
        }
        if(Address.isContract(recipient)&&!_isExcludedFromFee[recipient]){
