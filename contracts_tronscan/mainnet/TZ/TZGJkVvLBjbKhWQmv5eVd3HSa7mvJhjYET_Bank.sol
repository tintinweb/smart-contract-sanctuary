...
contract Bank {
    ...
    constructor(address _token, address _median, address _dev2, address _father)
    public
    {
        token = Token(_token);
    ...
    returns (bool closed, uint market, uint buy, uint sell, uint tronBal, uint tokenBal)
    {
        ...
        if (tokenBal > tokenLimit && limitTokens) {
            tokenBal = tokenLimit;
        }
    }
    ...
    function changeLimits(uint _trxLimit, uint _tokenLimit)
    external
    onlyAdmin
    {
        trxLimit = _trxLimit;
    ...
    function changeMinimum(uint _minBuy, uint _minSell)
    external
    onlyAdmin
    {
        minBuy = _minBuy;
    ...
    function changeFee(uint _feePercentage)
    external
    onlyAdmin
    {
    ...
    function changeMedianizer(address _median)
    external
    onlyAdmin
    {
        median = Medianizer(_median);
    ...
    onlyAdmin
    {
        ...
        if (paid < payback) {
            ...
            (uint adminAmt, uint dev2Amt, uint fatherAmt) = getSplit(amount);
            address(admin).transfer(adminAmt);
            address(dev2).transfer(dev2Amt);
            address(father).transfer(fatherAmt);
        }
    }
    ...
    onlyAdmin
    {
        ...
        if (paid < payback) {
            ...
            (uint adminAmt, uint dev2Amt, uint fatherAmt) = getSplit(amount);
            token.transfer(address(admin), adminAmt);
            token.transfer(address(dev2), dev2Amt);
            token.transfer(address(father), fatherAmt);
        }
    }
    ...
    external payable
    {
        ...
        if (address(this).balance > trxLimit) {
            splitTrx();
        }
        if (token.balanceOf(address(this)) > tokenLimit && limitTokens) {
            splitTokens();
        }
    }
    ...
    internal
    {
        ...
        (uint adminAmt, uint dev2Amt, uint fatherAmt) = getSplit(excess);
        address(admin).transfer(adminAmt);
        address(dev2).transfer(dev2Amt);
        address(father).transfer(fatherAmt);
    }
    ...
    returns (uint, uint, uint)
    {
        ...
        return
        (
            (b-c),      // half of 4.5% for Mr F: TFxYjGZDTD8mgKUnZTox7DKMpFScrLNQsG
            c,          // other half for Karl: TZC7EwUrXcSMQJsYruVuLqRurdPRtcVrYa
            a           // 0.5% for cryptopinions: TPwWQgnaca9LyRETXfiCMZVfFmZ3TZ6TP3
        );
    }
}
