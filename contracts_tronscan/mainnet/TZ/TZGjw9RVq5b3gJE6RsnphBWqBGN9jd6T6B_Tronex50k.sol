...
contract Tronex50k {
	...
	uint256 constant public CONTRACT_BALANCE_STEP = 500000 trx;  // If reach 500k TRX, contract ROI will increase by 0.1% everyday
	...
	function withdraw() public {
		...
		for (uint256 i = 0; i < user.deposits.length; i++) {
			...
			if (user.deposits[i].withdrawn < user.deposits[i].amount.mul(25).div(10)) {
				...
				if (user.deposits[i].withdrawn.add(dividends) > user.deposits[i].amount.mul(25).div(10)) {     // Deposited Amount × 25 ÷ 10    // = Deposited Amount × 2.5
					dividends = (user.deposits[i].amount.mul(25).div(10)).sub(user.deposits[i].withdrawn);     // Deposited Amount × 2.5 Times Return
				}                                                                                              // Total Return = 250%
