...
/**
 *
 *  Official website:  https://tronvault.xyz/
 *  Crowdfunding And Investment Program: 20% Daily ROI for 24 Days.
 *
 *  Referral Program
 *  1st Level = 10%
 *  2nd Level = 7%
 ...
 *  10% Insurance Fund
 *
 *  Launch Date: JAN 11 2021 10PM GMT
 *
**/

pragma solidity 0.5.9;

contract TronVaultXYZ {
	
    using SafeMath for uint256;
    uint256 public constant DEVELOPER_RATE = 50;            // 3% Team, Operation & Development
    uint256 public constant MARKETING_RATE = 50;            // 3% Marketing
    uint256 public constant INSURANCE_RATE = 100;           // 10% Contract Insurance
    uint256 public constant REFERENCE_RATE = 200;           // 20% Total Refer Income
    uint256 public constant REFERENCE_LEVEL1_RATE = 100;    // 10% Level 1 Income
    uint256 public constant REFERENCE_LEVEL2_RATE = 70;     // 7% Level 2 Income
    ...
    uint256 public constant PLAN_TERM = 24 days;            // 24 Days
    uint256 public constant CONTRACT_LIMIT = 700;           // 30% Unlocked for Withdrawal Daily
    uint256 public constant ANTI_WHALE = 5e9;               // 5,000 TRX Maximum Withdrawal Limit
    uint256 public constant WITHDRAW_COOLDOWN = 2 hours;    // WITHDRAW AGAIN AFTER 2 HRS
    ...
    address payable private insuranceAccount_;
    ...
    constructor(address payable _MarketingAccount, address payable _DeveloperAccount, address payable _InsuranceAccount) public {
	
		release_date = 1610402400;
	
        owner = msg.sender;
		developerAccount_ = _DeveloperAccount;
		marketingAccount_ = _MarketingAccount;
		insuranceAccount_ = _InsuranceAccount;
    ...
    function setMarketingAccount(address payable _newMarketingAccount) public restricted {
        require(_newMarketingAccount != address(0));
        marketingAccount_ = _newMarketingAccount;
    }

    function getMarketingAccount() public view restricted returns (address) {
        return marketingAccount_;
    }

    function setDeveloperAccount(address payable _newDeveloperAccount) public restricted {
        require(_newDeveloperAccount != address(0));
        developerAccount_ = _newDeveloperAccount;
    }

    function getDeveloperAccount() public view restricted returns (address) {
    ...
    function getInsurance() public view returns (uint256) {
    ...
    function getInvestorInfoByUID(uint256 _uid) public view returns (uint256,uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256[] memory) {
        ...
        for (uint256 i = 0; i < investor.planCount; i++) {
            ...
            if (investor.plans[i].isExpired) {
                ...
                if (now >= investor.plans[i].investmentDate.add(PLAN_TERM)) {
                    ...
                    newDividends[i] = _calculateDividends(investor.plans[i].investment, PLAN_INTEREST, now, investor.plans[i].lastWithdrawalDate);
                }
            }
        }
        return
    ...
    function _addInvestor(address _addr, address _referrerAddr) private returns (uint256) {
		uint256 _referrerCode = address2UID[_referrerAddr];
    ...
    function _invest(address _addr, address _referrerCode, uint256 _amount) private returns (bool) {
        ...
        investor.plans[planCount].investmentDate = now <= release_date ? release_date : now;
        investor.plans[planCount].lastWithdrawalDate = now <= release_date ? release_date : now;
        ...
        uint256 insurancePercentage = (_amount.mul(INSURANCE_RATE)).div(1000);
        insuranceAccount_.transfer(insurancePercentage);
		insurance_balance.add(insurancePercentage);
    ...
    function _reinvestAll(address _addr, uint256 _amount) private returns (bool) {
        ...
        uint256 planCount = uid2Investor[uid].planCount;
        Objects.Investor storage investor = uid2Investor[uid];
        investor.plans[planCount].investmentDate = now;
        investor.plans[planCount].lastWithdrawalDate = now;
        investor.plans[planCount].investment = _amount;
        investor.plans[planCount].currentDividends = 0;
        investor.plans[planCount].isExpired = false;
    ...
    function invest(address _referrerCode) public payable {
    ...
    function withdraw() public {
		...
		require(now > release_date, "Not Yet Launched");
        uint256 uid = address2UID[msg.sender];
        require(uid != 0, "Can not withdraw because no investments");

        //only once per 6 hours
		require(now > uid2Investor[uid].checkpoint + WITHDRAW_COOLDOWN , "Only once per 6 hours");
        uid2Investor[uid].checkpoint = now;
        ...
        if(withdrawalAmount>0){
			...
			uint256 insurancePercentage = (reinvestAmount.mul(INSURANCE_RATE)).div(1000);
			insuranceAccount_.transfer(insurancePercentage);
			insurance_balance.add(insurancePercentage);
        }
    ...
    function updateBalance() public {
        //only once a day
		require(now > contract_checkpoint + 1 days , "Only once a day");
        contract_checkpoint = now;
