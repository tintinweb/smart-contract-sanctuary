...
contract Cryptanes is TRC721, TRC721Enumerable, TRC721MetadataMintable {
    constructor() public TRC721Metadata("The Cryptanes", "CRYPTANES") {
       
    }

    uint256 public MAX_NFT_SUPPLY = 7500;     // max allowed to mint payable amount (not 0->..., but ... <-15000 !!!)
    uint256 public PRICE_COEFFICIENT = 67;   // coefficient for price adoption according to cource change
    uint256 public constant UPPER_BORDER = 15000;   // constant upper collection border

    
    uint256 public SpecialAmount = 0;   // special NFTs amount (minted in GOD-MODE on TRON)
    
    bool public SaleAllowed = true;
    
    function getNFTPrice() public view returns (uint256) {
        require(totalSupply().sub(SpecialAmount) < MAX_NFT_SUPPLY, "Lower collection border reached");

        uint currentSupply = totalSupply().sub(SpecialAmount);

        if (currentSupply >= 7000) {
            return PRICE_COEFFICIENT.mul(50000000); // 8008 - 7501	 eq. 500usdt
        } else if (currentSupply >= 6000) {
            return PRICE_COEFFICIENT.mul(40000000); // 9000 - 8001	 eq. 400usdt
        } else if (currentSupply >= 5000) {
            return PRICE_COEFFICIENT.mul(30000000); // 10000  - 9001	 eq. 300usdt
        } else if (currentSupply >= 4000) {
            return PRICE_COEFFICIENT.mul(25000000); // 11000 - 10001     eq. 250usdt
        } else if (currentSupply >= 3000) {
            return PRICE_COEFFICIENT.mul(20000000); // 12000 - 11001	 eq. 200usdt
        } else if (currentSupply >= 2000) {
            return PRICE_COEFFICIENT.mul(15000000); //  13000 - 12001    eq. 150usdt
        } else {
            return PRICE_COEFFICIENT.mul(10000000); //  15000 - 13001 	 eq. 100usdt 
        }
    }
    ...
    */
    function mintNFT(uint256 numberOfNfts) public payable {
        ...
        require(totalSupply().sub(SpecialAmount) < MAX_NFT_SUPPLY, "Lower collection border is reached. Cryptanes amount on TRON may be increased, pls wait");
        ...
        for (uint i = 0; i < numberOfNfts; i++) {
            uint mintIndex = UPPER_BORDER.sub(totalSupply()).add(SpecialAmount);
    ...
    */
    function FreeMintNFT(uint256 index) onlyMinter public {
    ...
    */
    function FreeMintToSelectedAddress(address Addr) public onlyMinter {
    ...
    * @dev Owner mints Cryptane to selected Address
    */
    function FreeMintParticularToSelectedAddress(address Addr, uint256 index) public onlyMinter {
    ...
    */
    function FreeMintSpecialNFT(uint256 index) public onlyMinter {
        _safeMint(msg.sender, index);
    ...
    */
    function setPriceCoefficient(uint256 value) public onlyMinter {
        PRICE_COEFFICIENT = value;
    }

    /**
    * @dev set collection part size  - not max index!!!
    */
    function setMaxSupplyAmount(uint256 value) public onlyMinter { 
    ...
    */
    function setSpecialAmount(uint256 value) public onlyMinter {
        SpecialAmount = value;
    }
    ...
    */    
    function AllowSale() public onlyMinter {
        SaleAllowed = true;
    }
    ...
    */    
    function DenySale() public onlyMinter {
        SaleAllowed = false;
    }
    ...
    */
    function withdraw() onlyMinter public {
