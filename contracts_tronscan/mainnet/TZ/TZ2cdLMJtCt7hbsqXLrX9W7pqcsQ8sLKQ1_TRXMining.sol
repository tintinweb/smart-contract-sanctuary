...
contract TRXMining {
    ...
    uint private minDepositSize = 30000000;
    uint private divisor = 100;
    uint private devCommission = 10;
    uint private rate = 30;
    uint private releaseTime=1611457200;
    uint private constant veces=3;
    ...
    function deposit(address payable _affAddr) public payable  returns(bool){
        ...
        destReferr1(msg.value, players[msg.sender].affFrom);
            ...
            if(players[msg.sender].deposits[pa].amount > 0){
                ...
                for(uint i=0 ; i< pos;i++){
                    ...
                    if(players[msg.sender].deposits[r].amount ==0){
                        players[msg.sender].deposits[r] = Deposit(msg.value,now,now,0,rate);
                        return true;
                    }
                    t++;
                }
            }else{
                players[msg.sender].deposits[pa] = Deposit(msg.value,now,now ,0,rate);
                return true;
            }
        }else{
            players[msg.sender].deposits[p] = Deposit(msg.value,now,now,0,rate);
            return true;
        }
    }
    function withdraw(uint pos) public returns(uint256){
        ...
        distributeRef(pft, players[msg.sender].affFrom);
    ...
    function reinvest(uint pos) public returns(uint256){
        require(now >= releaseTime, "not time yet!");
        Player storage player = players[msg.sender];
        require(player.deposits[pos].amount > 0 ,'you have already withdraw everything.');
        uint por;
        ...
        pft = (secPassed*(por/24/60/60));
        require(pft< address(this).balance, 'not balance system');
        uint devEarn = getPercent(pft,5);
        owner.transfer(devEarn);
        player.deposits[pos].time =now;
        player.interestProfit += pft;
        player.deposits[pos].profit += pft;
        player.deposits[pos].amount += pft;
        return pft;
    }
    function payRWD()public returns(uint){
        require(now >= releaseTime, "not time yet!");
        Player storage player = players[msg.sender];
        require(player.payrwd > 0 ,'you have already rewards everything.');
        msg.sender.transfer(player.payrwd);
        player.payrwd =0;
        return player.payrwd;
    }
    function distributeRef(uint256 _trx, address payable _affFrom) private{
        ...
        if (_affAddr1 != address(0)) {
            ...
            players[_affAddr1].payrwd += _affRewards;
            totalPayout += _affRewards; 
        }
        if (_affAddr2 != address(0)) {
            _affRewards = getPercent(_trx,5);
            players[_affAddr2].affRewards += _affRewards;
            players[_affAddr2].payrwd += _affRewards;
            totalPayout += _affRewards;
        }
        if (_affAddr3 != address(0)) {
            _affRewards = getPercent(_trx,5);
            players[_affAddr3].affRewards += _affRewards;
            players[_affAddr3].payrwd += _affRewards;
            totalPayout += _affRewards;
        }
        
    }
    function destReferr1(uint256 _trx, address payable _affFrom)private{
        address payable _affAddr1 = _affFrom;
        uint256 _affRewards = 0;
        if (_affAddr1 != address(0)) {
    ...
    function getHistory() public view returns(uint256,uint,uint,uint){
        Player storage player = players[msg.sender];
        return (player.trxDeposit,player.interestProfit,player.affRewards,player.payrwd);
    }
