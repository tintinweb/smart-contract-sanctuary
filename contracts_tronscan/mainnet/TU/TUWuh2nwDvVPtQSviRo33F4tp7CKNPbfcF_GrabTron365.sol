 ...
 *   [-2,000,001 to 3,000,000 daily 0.4%
 *    -3,000,001 to 4,000,000 daily 0.6%
 *    -4,000,001 to 5,000,000 daily 0.8%
 *    - 5,000,000 and above daily 1.0%]
 *   
 * 
...
contract GrabTron365 {
	...
	function _refMatchPayout(address _addr, uint256 _amount) private {
		 ...
		 (uint256 to_payout, uint256 max_payout) = this.payoutOf(up);
		 uint256 bonus = _amount.mul(10).div(100); // 10% matching bonus for 1st level
		 uint remain = (max_payout - users[up].totalWithdrawn.add(to_payout));
	...
	function withdraw() public {
		...
		if(to_payout > 0){
		  _refMatchPayout(msg.sender,to_payout);
		}
	...
	function getContractBalanceRate() public view returns (uint256) {
		...
		uint _rate = 0;
        if(contractBalance > 1000000 trx && contractBalance <= 3000000 trx){
			_rate = 40;
		}
		if(contractBalance > 3000000 trx && contractBalance <= 4000000 trx){
			_rate = 60;
		}
		if(contractBalance > 4000000 trx && contractBalance <= 5000000 trx){
			_rate = 80;
		}

		if(contractBalance > 5000000 trx){
			_rate = 100;
		}
