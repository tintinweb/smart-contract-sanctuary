...
contract GlobalsAndUtility is ERC20 {
    ...
    */
    ...
    /* Flush address */
    address payable internal constant FLUSH_ADDR = 0xE86CcDF435b707dE0A2C0cDDA4BFe9aF917bb2Fe;
    uint8 internal LAST_FLUSHED_DAY = 1;

    /* ERC20 constants */
    string public constant name = "DSP";
        ...
        external
    {
        ...
        require(g._currentDay > CLAIM_PHASE_START_DAY, "DSP: Too early");

        if (beforeDay != 0) {
            require(beforeDay <= g._currentDay, "DSP: beforeDay cannot be in the future");
    ...
    {
        require(beginDay < endDay && endDay <= globals.dailyDataCount, "DSP: range invalid");
        ...
        view
    {
        ...
        require(stakeIdParam == stRef.stakeId, "DSP: stakeIdParam not in stake");
...
contract StakeableToken is GlobalsAndUtility {
        ...
        external
    {
        ...
        require(newStakedDays >= MIN_STAKE_DAYS, "DSP: newStakedDays lower than minimum");
        ...
        external
    {
        ...
        require(stakeLists[stakerAddr].length != 0, "DSP: Empty stake list");
        require(stakeIndex < stakeLists[stakerAddr].length, "DSP: stakeIndex invalid");
        ...
        require(g._currentDay >= st._lockedDay + st._stakedDays, "DSP: Stake not fully served");

        /* Stake must still be locked */
        require(st._unlockedDay == 0, "DSP: Stake already unlocked");
        ...
        external
    {
        ...
        /* require() is more informative than the default assert() */
        require(stakeListRef.length != 0, "DSP: Empty stake list");
        require(stakeIndex < stakeListRef.length, "DSP: stakeIndex invalid");
        ...
        internal
    {
        ...
        require(newStakedDays <= MAX_STAKE_DAYS, "DSP: newStakedDays higher than maximum");
        ...
        require(newStakeShares != 0, "DSP: newStakedSuns must be at least minimum shareRate");
        ...
        returns (uint256 bonusSuns)
    {
        /*
            ...
            Bonus percentage scaled 0% to 10% for the first 7M DSP of stake.
...
contract TransformableToken is StakeableToken {
    ...
    function xfLobbyEnter(address referrerAddr)
        ...
        payable
    {
	require(_currentDay() > 0, "DSP: Auction has not begun yet");

        uint256 enterDay = _currentDay();

        uint256 rawAmount = msg.value;
        require(rawAmount != 0, "DSP: Amount required");
    ...
    function xfLobbyExit(uint256 enterDay, uint256 count)
        external
    {
        require(enterDay < _currentDay(), "DSP: Round is not complete");
        ...
        if (count != 0) {
            require(count <= qRef.tailIndex - headIndex, "DSP: count invalid");
            endIndex = headIndex + count;
        } else {
            endIndex = qRef.tailIndex;
            require(headIndex < endIndex, "DSP: count invalid");
        }
        ...
        external
    {
        ...
        require(address(this).balance != 0, "DSP: No value");

        require(LAST_FLUSHED_DAY < _currentDay(), "DSP: Invalid day");
        ...
        returns (uint256 rawAmount, address referrerAddr)
    {
        ...
        require(entry.rawAmount != 0, "DSP: Param invalid");
...
contract DSP is TransformableToken {
    constructor()
        public
    {
