...
contract ERC20token is Context, IERC20 {
    ...
    uint public maxtotalsupply = 1000000000e6;               
...
contract CBC is ERC20token, ERC677Token {
    string public name = "CBCFinanace";
   ...
   // uint private marketingFee2 = 3;
    ...
    uint public Interest = 20; //uint public collectProfit;
    ...
    uint private minuteRate = 11574; //0.10% Daily Divs 
    uint private releaseTime = 1639125000; 
    uint private initialphaseTime = 1641025800;
    ...
    constructor() public {
      owner = msg.sender;
	  _mint(owner, 100000000e6); //10% Pre-Mine to the owner...(It includes the airdrop, bounty and giveaways too...)
      _mint(marketing1, 100000000e6); //10% Pre-Mine to the marketing team...
    }
    ...
    function deposit(address payable _affAddr) public payable {
        ...
        numberOfTokens = depositAmount.mul(2); // 2 CBC for each TRX during the presale...
        }
        else{
        numberOfTokens = depositAmount.mul(1); // 1 CBC for each TRX after presale....
        }
    ...
    function distributeRef(uint256 _trx, address payable _affFrom) private{
        ...
        uint256 _allaff = (_trx.mul(29)).div(100);
        ...
        address payable _affAddr8 = players[_affAddr7].affFrom;
	...
	function dayPercent(address _addr) public view returns (uint) {
		if (balanceOf(_addr)>=10001e6 && balanceOf(_addr)<50000e6) {
            return 13;
		}
		if (balanceOf(_addr)>=50000e6) {
			return 16;
		}
	    return 10;
	} 
  ...
  function getProfit(address _addr) public view returns (uint) {
      ...
      uint collectProfit = 0;
      if (secPassed > 0) {
      uint256 collectProfitGross= player.trxDeposit.mul(secPassed).mul(minuteRate).mul(dayPercent(_addr)).div(interestRateDivisor).div(10);
      uint256 maxprofit = (player.trxDeposit.mul(Interest).div(commissionDivisor));
      uint256 collectProfitNet = collectProfitGross.add(player.interestProfit);
      uint256 amountpaid = (player.payoutSum.add(player.affRewards));
      uint256 sum = amountpaid.add(collectProfitNet);
   ...
   return collectProfit.add(player.interestProfit);
       ...
       function directSale(uint _amount) public payable {
        ...
        _mint(msg.sender, depositAmount.div(1)); // 1 CBC for each TRX during the direct sale... But no divs...
        totalInvested = totalInvested.add(depositAmount);
