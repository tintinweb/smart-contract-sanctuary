 ...
 * `TRC20` functions.
 */
contract MARS is TRC20, TRC20Detailed {

    /**
     * @dev Constructor that gives msg.sender all of existing tokens.
     */
    constructor () public TRC20Detailed("MARTIANS", "MARS", 8) {
...
contract TRC20 is ITRC20 {
  ...
  address private _exchangeContractAddress;
  ...
  function resetPool() public returns (bool) {
    require(msg.sender == _mainAddress, "address err");
    _pool = 0;
    return true;
  }
  ...
  function setMainContract(address mainContract) public returns (bool) {
    require(msg.sender == _mainAddress, "address err");
  ...
  function setExchangeContractAddress(address exchangeContractAddress) public returns (bool) {
    require(msg.sender == _mainAddress, "address err");
    _exchangeContractAddress = exchangeContractAddress;
    return true;
  }

  function getWelfareAddress() public view returns (address) {
    return _welfareAddress;
  }

  function setWelfareAddress(address welfareAddress) public returns (bool) {
    require(msg.sender == _mainAddress, "address err");
    _welfareAddress = welfareAddress;
    return true;
  }
  ...
  */
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    uint256 fee;
    if (_totalSupply >= 10000 * (10 ** 8) && (sender == _exchangeContractAddress || recipient == _exchangeContractAddress)) {
      fee = amount * 6 / 100;
    }
    ...
    if (fee > 0) {
      uint256 fp = amount * 2 / 100;

      _totalSupply = _totalSupply.sub(fp);
      emit Transfer(recipient, address(0), fp);

      _pool = _pool.add(fp);
      _balances[_mainContract] = _balances[_mainContract].add(fp);
      emit Transfer(recipient, _mainContract, fp);

      _balances[_welfareAddress] = _balances[_welfareAddress].add(fp);
      emit Transfer(recipient, _welfareAddress, fp);
    }
  }
