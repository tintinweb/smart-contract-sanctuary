...
contract TRONQUEEN {
	...
	uint256 constant public MAX_HOLD_BONUS = 200; // 2%
	...
	function getContractUsersRate(address userAddress) public view returns (uint256) { // +0.1% per 100 users
	...
	function getUserPercentRate(address userAddress) public view returns (uint256) {
		...
		if (isActive(userAddress)) { 
			uint256 timeMultiplier = (now.sub(user.checkpoint)).div(TIME_STEP).mul(5); // +0.05% per day
