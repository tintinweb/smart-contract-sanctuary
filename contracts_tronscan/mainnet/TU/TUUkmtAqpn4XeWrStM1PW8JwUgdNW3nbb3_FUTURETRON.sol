...
contract FUTURETRON {
    ...
    uint256 public devCommission = 150; /* 15% platform fee */
    uint256 public commissionDivisor = 1000;
    address private devAddress = msg.sender;
    uint private releaseTime = 	1616526000; // march 24 2021
    ...
    function getRate(uint256 _tier) internal pure returns (uint256) {
        uint256 _rate = 2314814;              //20%
        if(_tier == 1) { _rate = 1814940; }    //15%
        if(_tier == 2) { _rate = 1313640; }    //10%
        if(_tier == 3) { _rate = 777780; }    //5%
        if(_tier == 4) { _rate = 423432; }    //3%
        return _rate;
    }
    
    function getTimeLimit(uint256 _tier) internal pure returns(uint256) {
        uint256 timeLimit = 864000;         //10 days
        if(_tier == 1) timeLimit = 1382400;  //16 days
        if(_tier == 2) timeLimit = 2419200;  //28 days
        if(_tier == 3) timeLimit = 4838400;  //56 days
        if(_tier == 4) timeLimit = 10368000;  //120 days
        return timeLimit;
    }
    ...
    function collect(address _addr) internal {
        ...
        uint256 timeLimit = 864000;         //10 days
        if(player.tier == 1) timeLimit = 1382400;  //16 days
        if(player.tier == 2) timeLimit = 2419200;  //28 days
        if(player.tier == 3) timeLimit = 4838400;  //56 days
        if(player.tier == 4) timeLimit = 10368000;  //120 days
