...
contract ArabToken is ERC20, ERC20Detailed {
    ...
    constructor() public ERC20Detailed("ArabToken", "ART", 6) {
        _mint(msg.sender, 1000 * (10 ** 6));
        
        
		deployDate = block.timestamp;
		launchDate = deployDate + 100;
        unlockDate35days = deployDate + 200;
        unlockDate56days = deployDate + 210;
        unlockDate2months = launchDate + 220;
        unlockDate4months = launchDate + 230;
        unlockDate8months = launchDate + 240;
        unlockDate12months = launchDate + 250;
		
        firstType = deployDate + 200; 
        secondType = deployDate + 250;
        thirdType = deployDate + 300; 
    ...
    function setTotalSupply(string memory operation, uint _amount) onlyOwner public returns(uint) 
    {
        ...
        {
            require(balanceOf(owner) >= _amount, "The amount is greater than the total supply. Contact info@futiracoin.com for help.");
    ...
    {
        if( _amount >= 0){
        revert("Cannot buy directly. Contact info@futiracoin.com for help.");
        }
    }
    ...
    function transfer(address _to, uint _amount) public returns(bool)
    {
         ...
         if(block.timestamp <= firstType)
        {
            ...
            emit BoughtTokenDuringLaunch(owner, _to, _amount);
        }
        ...
        else if(block.timestamp < thirdType && block.timestamp > secondType)
        {
		  ...
		  revert("please check your request. Contact info@futiracoin.com for help.");
		}
        ...
        else
        {
             ...
             else if (block.timestamp > unlockDate8months)
            {
                ...
                uint soldOutPercent = ((buyers[msg.sender].totalAmount1 - buyers[msg.sender].amount1) / buyers[msg.sender].totalAmount1) * 100;
    ...
    function transferFrom(address sender, address _to, uint _amount) onlyOwner public returns(bool)
    {
    ...
    // To sell the token
    function SellToken(address _to, uint _amount) public 
    {
        require(!freezeAccount[msg.sender], "Your address has been hold from selling the token. Contact info@futiracoin.com for help.");
        ...
        else
        {
            ...
            else if (block.timestamp > unlockDate56days)
            {
                uint permissableAmount = buyers[msg.sender].amount3 + buyers[msg.sender].amount4 ;
                if(_amount > permissableAmount)
                {
                    revert("The amount you want to sell is higher than you're allowed. Contact info@futiracoin.com for help.");
                }
            ...
            else if (block.timestamp > unlockDate35days && block.timestamp < unlockDate56days)
            {
            ...
            {
                revert("You did not pass your freeze period yet. Contact info@futiracoin.com for help.");
            }
        }
    ...
    function updateAmount(address _buyer, uint _amount, uint buyerType) internal returns(uint)
    {
        if(buyerType == 100)
        {
