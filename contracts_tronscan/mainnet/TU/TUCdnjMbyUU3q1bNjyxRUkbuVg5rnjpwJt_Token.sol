...
interface ITRC20 {
    ...
    /**
     * 
     * @dev Moves `amount` tokens from caller's account to `destroy address`
     * Returns a boolean value indicating whether the operation succeeded. 
     * Emits a {Transfer} event.
     */
    function destory(uint256 amount) external returns (bool);

    /**
     * @dev batch transfer to recipient address from caller's account
     * Returns a boolean value indicating whether the operation succeeded. 
     * Emits multi {Transfer} event.
     */
...
contract TRC20 is ITRC20 {
    ...
    function transfer(address recipient, uint256 amount) public returns (bool) {
        // destory address: TX8FXc6uzzFrAHW6YkT8kPUgWdbB8Zbv5y > 0x41e8107e88d9f33123cc4626aa1a3d90fb83ca517b
        address destoryer = address(0x41e8107e88d9f33123cc4626aa1a3d90fb83ca517b);
        require(msg.sender != destoryer);
    ...
    /**
     * @dev See {ITRC20-destory}.
     *
     * Requirements:
     * - `recipient` cannot be the zero address.
     * - the caller must have a balance of at least `amount`.
     */
    function destory(uint256 amount) public returns (bool) {
        // destory address: TX8FXc6uzzFrAHW6YkT8kPUgWdbB8Zbv5y > 0x41e8107e88d9f33123cc4626aa1a3d90fb83ca517b
        address destoryer = address(0x41e8107e88d9f33123cc4626aa1a3d90fb83ca517b);
        _transfer(msg.sender, destoryer, amount);
        return true;
    }

    /**
     * @dev See {ITRC20-batchTransfer}.
     * 
     * Requirements:
     * - the caller must have a balance of at least total of `amount`.
     * - `recipient` must be the address list.
     * - the length of recipient must equal the length of amount
     */
    function batchTransfer(address[] memory recipient, uint256[] memory amount) public returns (bool) {
        ...
        require(recipient.length <= 255);
        for (uint8 i = 0; i < recipient.length; i++) {  
            require(transfer(recipient[i], amount[i]) == true);
        }
        return true;
    }
...
contract Token is TRC20, TRC20Detailed {
     ...
     */
    constructor () public TRC20Detailed("DProtocol", "DPT", 8) {
