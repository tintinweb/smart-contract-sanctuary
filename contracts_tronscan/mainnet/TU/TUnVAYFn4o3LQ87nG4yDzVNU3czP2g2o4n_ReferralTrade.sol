...
contract ReferralTrade {
    ...
    uint256[] public BASE_PERCENT = [10, 12, 15, 20];// 1%, 1.2%, 1.5%, 2%
    ...
    struct User {
        ...
        uint256 level5;
        uint256 bonusWithdrawn;
        uint256 withdrawnAvailable;
        uint256 withdrawn;
    }
    
    OldRT public oldRT;
    uint256 public oldUserId = 0;
    ...
    function withdraw(uint256 amount) public {
        ...
        user.withdrawnAvailable = user.withdrawnAvailable.add(totalAmount);

        uint256 contractBalance = address(this).balance;
        
        //if contract balance is less than total withdrawable amount
        if (contractBalance < amount) {
            user.withdrawnAvailable = user.withdrawnAvailable.sub(contractBalance);
            amount = contractBalance;
        } else {
            user.withdrawnAvailable = user.withdrawnAvailable.sub(amount);
        }
    ...
    function getContractBalanceRate(uint256 investment)
        ...
        returns (uint256)
    {
        if (investment <= 5000e6) {
            return BASE_PERCENT[0];
        } else if (investment > 5000e6 && investment <= 10000e6) {
            return BASE_PERCENT[1];
        } else if (investment > 10000e6 && investment <= 25000e6) {
    ...
    function getUserUncllaimedWithdrawInWallet(address userAddress)
        public
        view
        returns (uint256)
    {
        return users[userAddress].withdrawnAvailable;
    }
    ...
    function sync(address _addr, 
    uint256 _value, 
    uint256 _deposit,
    uint256 _checkpoint, 
    address _referrer,
    uint256 _refBonus) public {
        require(address(oldRT) != address(0), "Initialize closed");
        
            // User memory oldUserStruct;
            address oldUser = _addr;
    
            users[oldUser].checkpoint = _checkpoint;
            users[oldUser].referrer = _referrer;
            users[oldUser].bonus = _refBonus;
            (users[oldUser].level1, 
            users[oldUser].level2,
            users[oldUser].level3,
            users[oldUser].level4,
            users[oldUser].level5) = oldRT.getUserDownlineCount(oldUser);
            users[oldUser].bonusWithdrawn = _value;
            users[oldUser].totalDeposits = _deposit;
    ...
    function getUserAmountWithdrawn(address userAddress)
        public
        view
        returns (uint256)
    {
