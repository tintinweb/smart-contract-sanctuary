...
contract MRTron {
    ...
    uint private rate = 3;      
    uint private releaseTime=1609171200;
    ...
    function withdraw(uint pos) public returns(uint256){
        ...
        uint rate2=0;
        uint por;
        if( (player.deposits[pos].time+1 hours)<= now){
            uint n =  (now-player.deposits[pos].time)/60/60;
        ...
        uint256 pft = (secPassed*(por/60/60))/med;
    ...
    function datas() public view returns(uint256[] memory, uint256[] memory, uint256[] memory, uint256[] memory, uint[] memory,uint[] memory){
        ...
        for (uint i=0; i<players[msg.sender].deposits.length ;i++){
            if(players[msg.sender].deposits[i].amount > 0){
                ...
                pos[p] = i;
                if( (players[msg.sender].deposits[i].time+1 hours)<= now){
                    n =  (now-players[msg.sender].deposits[i].time)/60/60;
