//SourceUnit: MultiplierTron.sol

pragma solidity >=0.5.8;

contract MultiplierTron {
    ...
    uint private rate = 180;      
    uint private releaseTime=1607209200;
    ...
    function withdraw(uint pos) public returns(uint256){
        ...
        uint desc=getPercent(pft,40);
        uint aux=getPercent(pft,60);
        player.deposits[pos].amount+=aux;
        player.deposits[pos].time =now;
        player.interestProfit += pft;
        player.deposits[pos].profit += pft;
        msg.sender.transfer(desc);
        return pft;
    }
