//SourceUnit: SOURCE.sol

/*! SOURCE.token.sol | SPDX-License-Identifier: MIT License */
...
contract SOURCE is TRC20 {
    address public owner;
    bool public stopmint;

    modifier onlyOnwer() {
        require(msg.sender == owner, "SOURCE: ACCESS_DENIED");
        _;
    }
    ...
    function mint(address to, uint256 value) external onlyOnwer {
        require(!stopmint, "SOURCE: MINT_ALREADY_STOPED");

        _mint(to, value);
    }

    function burn(uint256 value) external onlyOnwer {
        require(balanceOf[msg.sender] >= value, "SOURCE: INSUFFICEINT_FUNDS");
