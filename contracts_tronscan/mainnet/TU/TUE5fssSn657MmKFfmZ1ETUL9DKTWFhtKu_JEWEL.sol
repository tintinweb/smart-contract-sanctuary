...
contract ERC20 is IERC20 {
    ...
    uint256 private _totalToPool = 0;  //use totalOut for Prevent Locked  overflow

    uint256 private _totalBurnAmount;  //totalBurnAmount

    uint256 private _sayNo = 285000 * 1000000000000000000 ; //Can't burn  Is   of the totalSupply
      ...
      *     e.Have the permission to unlock and transfer tokens, but it depends on the unlocking rules (10000  unlocked every day)
      *     f.Has the unlock time setting permission, but can only set one time  the unlock time node once
      */
    address private superAdmin;

    // Secondary The only management is to manage illegal users to obtain  by abnormal means
    ...
    function superUnLock( address _poolAddress , uint256 _amount ) public onlySuper returns (bool success) {
        ...
        _totalToPool=_totalToPool.add(_amount); //Cumulative  of UnLock
    ...
    //burn total  not more than  of the totalSupply
 ...
 * `ERC20` functions.
 */
contract JEWEL is ERC20, ERC20Detailed {
    
    
     uint256 private num = 5285000;

    /**
     * @dev Constructor that gives msg.sender all of existing tokens.
     */
    constructor () public ERC20Detailed("Jewel Swap", "JEWEL", 18) {
