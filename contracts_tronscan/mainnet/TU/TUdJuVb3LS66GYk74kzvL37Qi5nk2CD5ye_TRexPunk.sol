 ...
 */
contract TRexPunk is Context, Ownable, TRC165, ITpunks, IERC721Metadata {
    ...
    function setAdminFees(uint256 _adminFees) onlyOwner public  {
        adminFees=_adminFees;
    }
    
    // Function for set the Artist Fees 
    function setArtistFees(uint256 _artistFees)onlyOwner public {
    ...
    * @dev Mints Tpunk
    */
    function mintNFT(uint256 [] memory tokenId) public payable {
        uint256 total = totalSupply();
        address payable adminAddress=address(uint160(owner()));
        require(start_sale, 'sale is not start');
        require(totalSupply() < MAX_NFT_SUPPLY, "Sale has already ended");
        require(tokenId.length > 0, "numberOfNfts cannot be 0");
        require(tokenId.length <= 20, "You may not buy more than 20 NFTs at once");
        require(totalSupply().add(tokenId.length) <= MAX_NFT_SUPPLY, "Exceeds MAX_NFT_SUPPLY");
        require(getNFTPrice().mul(tokenId.length) == msg.value, "Trx value sent is not correct");
        adminAddress.transfer(msg.value);
        for (uint256 i=0; i < tokenId.length; i++) {
            _safeMint(msg.sender, tokenId[i]);
            artist[tokenId[i]]=msg.sender;
           
    }
    } 
    function buyNFT(uint256 tokenId) public payable{
           
            require(msg.value>=getNFTPrice()," Low BNB Amount :(");
            uint256 owneFees=((uint256(100).sub(artistFees)).sub(adminFees));      
            address payable  ownerAddress= (address(uint160(ownerOf(tokenId))));
            address payable adminAddress=address(uint160(owner()));
