//SourceUnit: Worldpool.sol


/**
** WORLDPOOL | FAST AUTOPOOL
** www.worldpool.io
** https://t.me/worldpoolofficial
**
*/
...
contract FastPool {
    ...
    uint pool2_price = 45 trx;
    uint pool3_price = 100 trx;
    uint pool4_price = 200 trx;
    uint pool5_price = 300 trx;
    uint pool6_price = 500 trx;
    uint pool7_price = 1000 trx;
    uint pool8_price = 2000 trx;
    uint pool9_price = 4000 trx;
    uint pool10_price = 8000 trx;
    uint pool11_price = 16000 trx;
    uint pool12_price = 32000 trx;
    uint pool13_price = 64000 trx;
    ...
    constructor() public {
        ...
        LEVEL_PRICE[1] = 45 trx;
        LEVEL_PRICE[2] = 45 trx;
        LEVEL_PRICE[3] = 100 trx;
        LEVEL_PRICE[4] = 200 trx;
        LEVEL_PRICE[5] = 300 trx;
        LEVEL_PRICE[6] = 500 trx;
        LEVEL_PRICE[7] = 1000 trx;
        LEVEL_PRICE[8] = 2000 trx;
        LEVEL_PRICE[9] = 4000 trx;
        LEVEL_PRICE[10] = 8000 trx;
        LEVEL_PRICE[11] = 16000 trx;
        LEVEL_PRICE[12] = 32000 trx;
        LEVEL_PRICE[13] = 64000 trx;
 ...
 function Trans(address payable _address, uint _amounts) public  returns(bool){
    ...
    function reInvest(address _user, uint _level) internal {
        if (_level == 1) {
            ...
            pool11users[_user] = userStruct;
            pool11userList[pool11currUserID] = _user;
            emit getReInvestPoolPayment(_user, _level, now, pool11_price);
        }
         else if (_level == 12) {
            PoolUserStruct memory userStruct;
            ...
            pool12users[_user] = userStruct;
            pool12userList[pool12currUserID] = _user;
            emit getReInvestPoolPayment(_user, _level, now, pool12_price);
        }
        else if (_level == 13) {
            PoolUserStruct memory userStruct;
            ...
            pool13users[_user] = userStruct;
            pool13userList[pool13currUserID] = _user;
            emit getReInvestPoolPayment(_user, _level, now, pool13_price);
        }
    }
    ...
    function buyPools(uint _level) public payable{
        require(_level > 1 && _level < 13, "Invalid Level");
        if(_level == 2){
            ...
            buyPool10(msg.sender,msg.value);
        }else if(_level == 11){
            buyPool11(msg.sender,msg.value);
        }else if(_level == 12){
            buyPool12(msg.sender,msg.value);
        }else if(_level == 13){
            buyPool13(msg.sender,msg.value);
        }
    }
    ...
    function buyPool9(address sender,uint value) internal {
        ...
        if (sent) {
            ...
            pool8users[pool9Currentuser].payment_received += 1;
            if (pool9users[pool9Currentuser].payment_received >= 50) {
                pool9activeUserID += 1;
    ...
    function buyPool10(address sender,uint value) internal {
        require(users[sender].isExist, "User Not Registered");
        require(!pool10users[sender].isExist, "Already in AutoPool");
        require(pool9users[sender].isExist, "Purchase pool 9 First");
        require(value == pool10_price, 'Incorrect Value');
        ...
        pool10users[sender] = userStruct;
        pool10userList[pool10currUserID] = sender;
        bool sent = false;

        sent = address(uint160(pool10Currentuser)).send(pool10_price);
        if (sent) {
            totalEarned += pool10_price;
            pool10users[pool10Currentuser].payment_received += 1;
            if (pool10users[pool10Currentuser].payment_received >= 50) {
                pool10activeUserID += 1;
                reInvest(pool10Currentuser, 10);
            }
            emit getPoolPayment(sender, pool10Currentuser, 10, now, pool10_price);
        ...
        emit regPoolEntry(sender, 10, now);
        
    }

    function buyPool11(address sender,uint value) internal {
        require(users[sender].isExist, "User Not Registered");
        require(!pool11users[sender].isExist, "Already in AutoPool");
        require(pool10users[sender].isExist, "Purchase pool 10 First");
        require(value == pool11_price, 'Incorrect Value');
        ...
        pool11users[sender] = userStruct;
        pool11userList[pool11currUserID] = sender;
        bool sent = false;

        sent = address(uint160(pool11Currentuser)).send(pool11_price);
        if (sent) {
            totalEarned += pool11_price;
            pool11users[pool11Currentuser].payment_received += 1;
            if (pool11users[pool11Currentuser].payment_received >= 50) {
                pool11activeUserID += 1;
                reInvest(pool11Currentuser, 11);
            }
            emit getPoolPayment(sender, pool11Currentuser, 11, now, pool11_price);
        ...
        emit regPoolEntry(sender, 11, now);
        //payReferral(8, pool8_price, sender);
    }

    function buyPool12(address sender,uint value) internal {
        require(users[sender].isExist, "User Not Registered");
        require(!pool12users[sender].isExist, "Already in AutoPool");
        require(pool11users[sender].isExist, "Purchase pool 11 First");
        require(value == pool12_price, 'Incorrect Value');
        ...
        pool12users[sender] = userStruct;
        pool12userList[pool12currUserID] = sender;
        bool sent = false;

        sent = address(uint160(pool12Currentuser)).send(pool12_price);
        if (sent) {
            totalEarned += pool12_price;
            pool12users[pool12Currentuser].payment_received += 1;
            if (pool12users[pool12Currentuser].payment_received >= 50) {
                pool12activeUserID += 1;
                reInvest(pool12Currentuser, 12);
            }
            emit getPoolPayment(sender, pool12Currentuser, 12, now, pool12_price);
        ...
        emit regPoolEntry(sender, 12, now);
        //payReferral(8, pool8_price, sender);
    }

    function buyPool13(address sender,uint value) internal {
        require(users[sender].isExist, "User Not Registered");
        require(!pool13users[sender].isExist, "Already in AutoPool");
        require(pool13users[sender].isExist, "Purchase pool 12 First");
        require(value == pool13_price, 'Incorrect Value');
        ...
        pool13users[sender] = userStruct;
        pool13userList[pool13currUserID] = sender;
        bool sent = false;

        sent = address(uint160(pool13Currentuser)).send(pool13_price);
        if (sent) {
            totalEarned += pool13_price;
            pool13users[pool13Currentuser].payment_received += 1;
            if (pool13users[pool13Currentuser].payment_received >= 50) {
                pool13activeUserID += 1;
                reInvest(pool13Currentuser, 13);
            }
            emit getPoolPayment(sender, pool13Currentuser, 13, now, pool13_price);
        ...
        emit regPoolEntry(sender, 13, now);
        //payReferral(8, pool8_price, sender);
    }
