...
contract RiccoTron {
    ...
    function sendETHDividends(address userAddress, address _from, uint8 matrix, uint8 level) private {
        ...
        if (!address(uint160(receiver)).send(levelPrice[level])) {
             emit  UserIncome(receiver,_from,address(this).balance, matrix,level);
             if (matrix == 1) {
            users[receiver].X3LevelIncome[level] += address(this).balance;
            users[receiver].X3Income += address(this).balance;
            } else {
                users[receiver].X4LevelIncome[level] += address(this).balance;
                users[receiver].X4Income += address(this).balance;
            }
            return address(uint160(receiver)).transfer(address(this).balance);
        }
        if (matrix == 1) {
            users[receiver].X3LevelIncome[level] += levelPrice[level];
            users[receiver].X3Income += levelPrice[level];
        } else {
            users[receiver].X4LevelIncome[level] += levelPrice[level];
            users[receiver].X4Income += levelPrice[level];
        }
    
         emit  UserIncome(receiver,_from,levelPrice[level],matrix,level);
        if (isExtraDividends) {
