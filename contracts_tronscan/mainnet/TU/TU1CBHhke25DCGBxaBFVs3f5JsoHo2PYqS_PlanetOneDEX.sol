...
contract PlanetOneDEX {
    ...
    function changeTradeAddress(address changeTradeaddress) onlyOwner public returns(bool){
        tradeAddress = changeTradeaddress;
        return true;
    }
    ...
    function updateToken(address tokenAddress_,string memory tokenSymbol,uint128 decimal_,uint120 withdrawFee_) onlyTradeAddress public returns(bool){
        require(tokendetails[tokenAddress_].status==1);
        tokendetails[tokenAddress_] = tokens(tokenAddress_,tokenSymbol,decimal_,withdrawFee_,1);
        return true;
    }
    ...
    function deposit() dexstatuscheck notOwner public payable returns(bool) { 
        ...
        userDetails[msg.sender]._balance[address(0)] = userDetails[msg.sender]._balance[address(0)].add(msg.value);
    ...
    function tokenDeposit(address tokenaddr,uint256 tokenAmount) dexstatuscheck notOwner public returns(bool)
    {
        ...
        require(ITRC20(tokenaddr).balanceOf(msg.sender) > tokenAmount); 
        require(ITRC20(tokenaddr).allowance(msg.sender,address(this)) > 0); 
        
        if(userDetails[msg.sender].ref == address(0)) { userDetails[msg.sender].ref = admin; emit referredBy( msg.sender, admin);}
        
        userDetails[msg.sender]._balance[tokenaddr] = userDetails[msg.sender]._balance[tokenaddr].add(tokenAmount);
    ...
    function withdraw(uint8 type_,address tokenaddr,uint256 amount) dexstatuscheck notOwner public returns(bool) {
        ...
        if(type_==0){
            ...
            require(amount>0 && amount <= userDetails[msg.sender]._balance[address(0)] && withdrawfee[address(0)]<amount);
            ...
            require(amount>0 && amount <= userDetails[msg.sender]._balance[tokenaddr] && withdrawfee[tokenaddr]<amount);
            ITRC20(tokenaddr).transfer(msg.sender, (amount.sub(withdrawfee[tokenaddr])));  
        }
        
        userDetails[msg.sender]._balance[tokenaddr] = userDetails[msg.sender]._balance[tokenaddr].sub(amount);  
    ...
    function adminProfitWithdraw(uint8 type_,address tokenAddr)public onlyOwner returns(bool){ 
       require(type_ ==0 || type_ == 1);
         if(type_==0){
            ...
            require(tokenAddr != address(0)) ;
            ITRC20(tokenAddr).transfer(admin, adminProfit[admin][tokenAddr]);
    ...
    function makeOrder(uint128[8] memory makerOrder, uint128[8] memory takerOrder,address[] memory traderAddress , bytes32[2] memory msgHash,uint8[2] memory  v,bytes32[4] memory rs) onlyTradeAddress public returns(bool){
        ...
        if(makerOrder[3] == 1){ 
            amount__m =amount__m.add(div128(mul128(tradeAmount,makerOrder[2]),tokendetails[traderAddress[0]].decimals)) ; // maker buy trade amount
            ...
            amount__t = amount__t.add(div128(mul128(tradeAmount,makerOrder[2]), tokendetails[traderAddress[2]].decimals)); // taker sell trade amount
        }
    ...
    function feeDistribution( address trader, uint amount) internal returns (bool) {
        ...
        uint sevemtyPercent = amount.mul(70).div(100);
        
        for(uint i=0;i<6;i++){
            if(i ==0) {
                address(uint160(stakerWallets[i])).transfer(sevemtyPercent);
                emit stakerFeeTransfer( stakerWallets[i], sevemtyPercent);
            }
            else{
                address(uint160(stakerWallets[i])).transfer(fivePercent);
                emit stakerFeeTransfer( stakerWallets[i], fivePercent);
            }
        }
