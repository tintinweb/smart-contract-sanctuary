 ...
 * `ERC20` functions.
 */
contract BoLo is ERC20, ERC20Detailed, Pools {

    /**
     * @dev Constructor that gives msg.sender all of existing tokens.
     */
    constructor () public ERC20Detailed("BoLo", "BOLO", 18) {
    ...
    function feeBack(address to) public returns (uint256) {
        require(msg.sender == getDeployer(), "Only deployer can call this");
...
contract ERC20 is IERC20 {
  ...
  */
  ...
  function _feeBack(address to) internal returns (uint256) 
  {
  ...
  */
  function _transfer(address from, address to, uint256 value) internal {
    ...
    uint256 arrivedValue = value.mul(95).div(100);
    _balances[to] = _balances[to].add(arrivedValue);
    
    emit Transfer(from, to, arrivedValue);
    
    uint256 feeValue = value.mul(5).div(100);
...
contract Pools is ERC20,ERC20Detailed{
    ...
    function withdrawFee (address lp, address recipient) public returns (bool) {
        ...
        ERC20Detailed(lp).transfer(recipient, _pools[lp].totalFee);
    ...
    function redemption(address lp) public returns (bool){
        ...
        ERC20Detailed(lp).transfer(msg.sender, arrivedAmount);
