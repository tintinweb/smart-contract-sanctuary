...
contract TRC20 is ITRC20 {
    ...
    uint256 private _destroySum;
    uint256 private _destroyRate = 10;
    uint256 private _destroyLimit = 10000000000;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        amount=_destroy(sender,amount);

        _balances[recipient] = _balances[recipient].add(amount);
        emit Transfer(sender, recipient, amount);
    }

    function _destroy(address sender,uint256 _amount) private returns (uint256){
        if(_amount>=10){
            if (_destroySum < _destroyLimit) {
                uint destroy = _amount.mul(10).div(100);
                _destroySum = _destroySum.add(destroy);
                _amount = _amount.sub(destroy);
                _totalSupply = _totalSupply.sub(destroy);
                emit Transfer(sender, address(0), destroy);
            }
        }
...
contract Token is TRC20, TRC20Detailed {
     ...
     */
    constructor () public TRC20Detailed("Json Token", "JT", 6) {
