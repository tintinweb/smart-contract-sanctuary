...
contract ReferralTrade {
    ...
    function invest(address referrer) public payable {
        ...
        user.deposits.push(Deposit(userDeposit, 0, block.timestamp));
        //update user total deposits
        user.totalDeposits = user.totalDeposits.add(userDeposit);

        totalInvested = totalInvested.add(userDeposit);
        totalDeposits = totalDeposits.add(1);

        emit NewDeposit(msg.sender, userDeposit);
    }
