...
contract TronCore {
    ...
    uint256 constant public ROI_RATE=2;
    ...
    constructor(address payable _owner,address payable _etherchainFund,address payable _adminFee) public {
        owner = _owner;
        
        etherchain_fund = _etherchainFund;
        admin_fee = _adminFee;
    ...
    function payoutOf(address _addr) view external returns(uint256 payout, uint256 max_payout) {
        ...
        if(users[_addr].deposit_payouts < max_payout) {
            payout = (users[_addr].deposit_amount * ROI_RATE * ((block.timestamp - users[_addr].deposit_time) / 1 days) / 100) - users[_addr].deposit_payouts;
