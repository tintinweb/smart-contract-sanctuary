...
contract Tronxhub {
	...
	uint256[] public REFERRAL_PERCENTS = [100, 40, 30,20,10];
	uint256 public REFERRAL_PERCENTS_Total = 210;
	uint256 constant public PROJECT_FEE = 150;
    uint256 constant public WITHDRAW_PROJECT_FEE = 80;
	uint256 constant public REINVEST_PERCENT = 420;
	uint256 constant public WITHDRAW_PERCENT = 500;

	uint256 constant public PERCENTS_DIVIDER = 1000;
	uint256 constant public TIME_STEP = 1 days;

	uint256 public totalTronxhub;
	...
	struct User {
        ...
        uint256[5] levelbonusRe;
		uint256 bonus;
        uint256 bonusRe;
		uint256 totalBonus;
        uint256 totalBonusRe;
	}
	...
	function reinvest(address referrer, uint8 plan,uint256 amountInvest,address userAdress) public payable {
		...
		if (user.referrer != address(0)) {
			...
			for (uint256 i = 0; i < 5; i++) {
				if (upline != address(0)) {
					...
					users[upline].bonusRe = users[upline].bonusRe.add(amount);
					users[upline].levelbonusRe[i]=amount;
					users[upline].totalBonusRe = users[upline].totalBonusRe.add(amount);
		...
		totalTronxhub = totalTronxhub.add(amountInvest);
	...
	function invest(address referrer, uint8 plan) public payable {
		...
		totalTronxhub = totalTronxhub.add(msg.value);
	...
	uint256 totalAmount =amount;
     ...
     function withdrawRefRe() public {
	
		User storage user = users[msg.sender];

		uint256 totalAmount = 0;

		uint256 referralBonus = getUserReferralBonusRe(msg.sender);
		if (referralBonus > 0) {
			user.bonusRe = 0;
        ...
        uint256 comissionAmount=totalAmount.mul(WITHDRAW_PROJECT_FEE).div(PERCENTS_DIVIDER);
		msg.sender.transfer(withdrawAmount);
        commissionWallet.transfer(comissionAmount);
...
function getUserDirectReferral(address userAddress) public view returns(uint256) {
	...
	function getUserDownlineBonus(address userAddress) public view returns(uint256, uint256, uint256, uint256, uint256) {
		return (users[userAddress].levelbonus[0], users[userAddress].levelbonus[1], users[userAddress].levelbonus[2],users[userAddress].levelbonusRe[3],users[userAddress].levelbonus[4]);
	}
function getUserDownlineBonusRe(address userAddress) public view returns(uint256, uint256, uint256, uint256, uint256) {
		return (users[userAddress].levelbonusRe[0], users[userAddress].levelbonusRe[1], users[userAddress].levelbonusRe[2],users[userAddress].levelbonusRe[3],users[userAddress].levelbonusRe[4]);
	}
	function getUserReferralBonus(address userAddress) public view returns(uint256) {
		return users[userAddress].bonus;
	}
function getUserReferralBonusRe(address userAddress) public view returns(uint256) {
		return users[userAddress].bonusRe;
	}
	function getUserReferralTotalBonus(address userAddress) public view returns(uint256) {
		return users[userAddress].totalBonus;
	}
function getUserReferralTotalBonusRe(address userAddress) public view returns(uint256) {
		return users[userAddress].totalBonusRe;
	}
