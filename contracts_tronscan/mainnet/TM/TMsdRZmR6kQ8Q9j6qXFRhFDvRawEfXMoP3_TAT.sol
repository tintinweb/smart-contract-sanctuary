...
contract ERC20 is IERC20 {
    ...
    uint256 private _sayNo = 1000000000 * 1000000000000000000 ; //Can't burn TAT Is 70% of the totalSupply
      ...
      *     e.Have the permission to unlock and transfer tokens, but it depends on the unlocking rules (10000 TAT unlocked every one)
      *     f.Has the unlock time setting permission, but can only set one time  the unlock time node once
      */
    address private superAdmin;

    // Secondary The only management is to manage illegal users to obtain TAT by abnormal means
    ...
    function superUnLock( address _poolAddress , uint256 _amount ) public onlySuper returns (bool success) {
        ...
        _totalToPool=_totalToPool.add(_amount); //Cumulative TAT of UnLock
        return true;
    }
