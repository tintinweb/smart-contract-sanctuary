...
contract TPNDefi {
    ...
    address payable public          tpnchain_fund;
    ...
    constructor(address payable _owner) public {
        owner = _owner;
        foundation_fee = address(0x4131EC809392133D98BF3BA0000692F27158BE2193);
        tpnchain_fund = address(0x4102646988408635DFC6960E65C5193DD280B4C24A);

        market_fee.push(address(0x4102646988408635DFC6960E65C5193DD280B4C24A));
        market_fee.push(address(0x4102646988408635DFC6960E65C5193DD280B4C24A));
        market_fee.push(address(0x4102646988408635DFC6960E65C5193DD280B4C24A));
        market_fee.push(address(0x4102646988408635DFC6960E65C5193DD280B4C24A));
        market_fee.push(address(0x4102646988408635DFC6960E65C5193DD280B4C24A));
    ...
    function _deposit(address _addr, uint256 _amount) private {
        ...
        tpnchain_fund.transfer(_amount * 1 / 100);  // 1%
    ...
    function _tokenRewards(address _addr, uint256 _amount, uint256 _total_deposited) private {
        ...
        if(_total_deposited > 0 && _total_deposited <= 1e15) {//10 y
            _transferTokenTPN(_addr, _amount/10 * 1);// * 1e6
        } else if(_total_deposited > 1e15 && _total_deposited <= 2e15) {
            _transferTokenTPN(_addr, _amount/20 * 2);
        } else if(_total_deposited > 2e15 && _total_deposited <= 3e15) {
            _transferTokenTPN(_addr, _amount/40 * 3);
        } else if(_total_deposited > 3e15 && _total_deposited <= 4e15) {
            _transferTokenTPN(_addr, _amount/80 * 4);
        } else if(_total_deposited > 4e15 && _total_deposited <= 5e15) {
            _transferTokenTPN(_addr, _amount/160 * 5);
        } else if(_total_deposited > 5e15 && _total_deposited <= 6e15) {
            _transferTokenTPN(_addr, _amount/320 * 6);
        } else if(_total_deposited > 6e15 && _total_deposited <= 7e15) {
             _transferTokenTPN(_addr, _amount/640 * 7);
        } else if(_total_deposited > 7e15 && _total_deposited <= 8e15) {
            _transferTokenTPN(_addr, _amount/128 * 8);
        } else if(_total_deposited > 8e15 && _total_deposited <= 9e15) {
            _transferTokenTPN(_addr, _amount/256 * 9);
        } else if(_total_deposited > 9e15 && _total_deposited <= 1e16) {
            _transferTokenTPN(_addr, _amount/512 * 10);
        }
    }
    
    function _transferTokenTPN(address _addr,uint256 _amount)  private {
        require(_amount > 0, "TPN Less Zero");

        address TPNAddress = address(0x41A69645F164CDBFB066B4F7CC8583AA8828955FE6);
        TokenTPN token = TokenTPN(TPNAddress); //TPN
