...
contract WPAL is ITokenDeposit {
    string public name = "Wrapped PlayAndLike";
    string public symbol = "WPAL";
    uint8  public decimals = 2;
    trcToken  public palTokenId = trcToken(1003533);
    ...
    function deposit() public payable {
        require(msg.tokenid == palTokenId, "deposit tokenId not PAL[1003533]");
    ...
    function withdraw(uint256 sad) public {
        require(balanceOf_[msg.sender] >= sad, "not enough WPAL balance");
        require(totalSupply_ >= sad, "not enough WPAL totalSupply");
        balanceOf_[msg.sender] -= sad;
        totalSupply_ -= sad;
        msg.sender.transferToken(sad, palTokenId);
