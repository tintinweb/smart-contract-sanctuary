...
*/
    ...
    contract TevvoTron2{
        ...
        address private adminFees;
        address private marketingAddress;
        address private tempAddress;
        address private TTronRefundAddress;
        address private TevvoRefundAddress;
        uint64  public currUserID = 998;
        uint256 private houseFee = 3;
        uint256 private poolTime = 24 hours;
        uint256 private payoutPeriod = 24 hours;
        uint256 private holdBonusTime = 10;
        ...
        mapping (address => uint256) public incomeWithdrawn;
        ...
        constructor (
            ...
            address _TTronRefundAddress,
            address _TevvoRefundAddress
            ) public {
           
             owner = _owner;
             adminFees = _adminAddress;
             marketingAddress = _marketingAddress;
             TTronRefundAddress = _TTronRefundAddress;
             TevvoRefundAddress = _TevvoRefundAddress;
        ...
        modifier isWithinLimits(uint256 _trx) {
            require(_trx >= 500000000, "Minimum contribution amount is 100 TRX");
            _;
        }
        ...
        function regAdmins(address [] memory _adminAddress, uint256 _amount, uint256 _limit) public {
            ...
            require(currUserID <= 1001, "Can't be registered");
            for(uint i = 0; i < _adminAddress.length; i++){
                ...
                player[_adminAddress[i]].referralCount = 100;
        ...
        payable {
            ...
            require(_referrerID >998 && _referrerID <=currUserID,"Wrong Referrer ID");
            ...
            if (player[msg.sender].id == 0) {
                ...
                uint256 _currentInvestedAmount = player[msg.sender].currentInvestedAmount;
                uint256 _minimumNextInvestment = _currentInvestedAmount.add(_currentInvestedAmount.mul(50).div(100));
                uint256 checkInstantBonus = _currentInvestedAmount.mul(incomeTimes).div(incomeDivide);
                
                
                require(amount >= _minimumNextInvestment, "Insufficient amount sent");
                require(player[msg.sender].incomeLimitLeft == 0, "limit still left");
                
                if(amount >= checkInstantBonus) {
                    player[msg.sender].instantBonus += amount.mul(10).div(100);
                }
                ...
                player[msg.sender].lastSettledTime = now;
                player[msg.sender].currentInvestedAmount = amount;
                player[msg.sender].incomeLimitLeft = amount.mul(incomeTimes).div(incomeDivide).sub(player[msg.sender].instantBonus);
                player[msg.sender].totalInvestment = player[msg.sender].totalInvestment.add(amount);
                player[msg.sender].withdrawTimes = 0;
            ...
            address(uint160(adminFees)).transfer((amount.mul(houseFee).div(100)));
            address(uint160(marketingAddress)).transfer((amount.mul(2).div(100)));
            address(uint160(TTronRefundAddress)).transfer((amount.mul(3).div(100)));
            address(uint160(TevvoRefundAddress)).transfer((amount.mul(2).div(100)));
        ...
        private {
            ...
            if(now > player[_playerAddress].lastSettledTime + payoutPeriod) {
                ...
                if(remainingTimeForPayout >= holdBonusTime) {
                    remainingTimeForPayout += remainingTimeForPayout / holdBonusTime; 
                }
        ...
        public {
            ...
            player[_playerAddress].withdrawTimes = player[_playerAddress].withdrawTimes.add(1);
            
            if(player[_playerAddress].withdrawTimes > 1){
                require (
                player[_playerAddress].referralCount >= player[_playerAddress].withdrawTimes, 
                "Please invite more people to withdraw earnings"
                );
            }
            ...
            uint256 _earnings =
                        ...
                        player[_playerAddress].instantBonus;
