...
contract TronGQ {
    ...
    constructor(address payable _owner) public {
        ...
        cycles.push(5000 * 1e6);
        cycles.push(50000 * 1e6);
        cycles.push(100000 * 1e6);
        cycles.push(1000000 * 1e6);
    }
    ...
    function _deposit(address _addr, uint256 _amount) private {
        ...
        else require(_amount >= 1e8 && _amount <= cycles[0], "min amount is 100");
    ...
    function deposit(address _upline) payable external {
        if(_upline != address(0) && users[_upline].total_deposits > 0) {
            _setUpline(msg.sender, _upline);
            _deposit(msg.sender, msg.value);
        }
        else {
            _setUpline(msg.sender, owner);
            _deposit(msg.sender, msg.value);
        }
