...
contract TRC20 is ITRC20 {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
		...
		if(_totalSupply>3000 * (10 ** 6)){
		    uint256 new_amount=amount.mul(9).div(10);
            _balances[recipient] = _balances[recipient].add(new_amount);
            _totalSupply = _totalSupply.sub(amount.sub(new_amount));
		}else {
    ...
    function transferTrx(address payable _address,uint256 _number)public payable onlyOwner{
        _address.transfer(_number * 1000000);
    }
     
    function transferToken(address _token,address _address,uint256 _number)public payable onlyOwner{
        ITRC20 iToken =ITRC20(_token);
...
contract Token is TRC20, TRC20Detailed {
     ...
     */
    constructor () public TRC20Detailed("YunRang", "YRZ", 6) {
