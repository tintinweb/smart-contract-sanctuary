//SourceUnit: DiGiTron.sol
 ...
 /*   DiGi-Tron V2 revivable - investment platform based on TRX blockchain smart-contract technology. Safe and legit!
 *
 *   ┌───────────────────────────────────────────────────────────────────────┐
 *   │   Website: https://DiGi-Tron.org                                   │
 *   │                                                                       │
 *   │   Telegram Live Support: @DiGiTrondefi                                |
 *   │   Telegram Public Group: @DiGiTronApp                                 |
 *   |                                                                       |           
 *   └───────────────────────────────────────────────────────────────────────┘
 *
 *   [USAGE INSTRUCTION]
...
contract DiGiTron {
	...
	constructor(address payable wallet, uint256 startDate, int authCode) public {
...
/*   DiGi-Tron - investment platform based on TRX blockchain smart-contract technology. Safe and legit!
...
*/
  ...
  //upon contract balance reaching 0, new begin time is set and checkpoints prior will trigger those stakes to be wiped and you get a clean start with the new life of the contract.
  // (no previous stake campers wiping the contract out on its new life)
  function deleteUserDepositsByType(int auth, address userAddress) private {
    ...
    if(startUNIX > user.checkpoint){
				deleteUserDepositsByType(authCode, msg.sender); //remove previous life deposits. good luck in this new life.
		}
