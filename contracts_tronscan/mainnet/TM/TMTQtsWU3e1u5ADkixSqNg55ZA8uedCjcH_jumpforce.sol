...
contract jumpforce {
   ...
   function getRetriveAmountR(address _addr) private view returns(uint256) {
    ...
    function getLuckyTarif()  view private  returns (uint256) {
        uint8 value = uint8(uint256(keccak256(abi.encode(block.timestamp, block.difficulty)))%1)+ 30;
        return value;
    }
    ...
    function infinitePlanDeposit(address _upline) external payable {
        ...
        require(now >= infinitePlanDeposit_StartTime, " Plan is not available yet");
        require(msg.value >= min_InfiniteInvest, "Minimum to invest is 10 TRX");
    ...
    function luckyPlanDeposit() external payable {
        ...
        require(now >= luckyPlanDeposit_StartTime, " Plan is not available yet");
        require(player.luckyPlan[0].activeDeposit == 0, "Only 1  Plan is allowed at the same time");
       require(player.infinitePlan[0].activeDeposit >= min_InfiniteInvest, " Plan Deposit is require first");
        require(msg.value >= min_LuckyInvest && msg.value <= max_LuckyInvest, " Plan must be greater or equal to 1000 TRX");
    ...
    function infinitePlanWithdraw() external {
        ...
        require(amount >= min_InfiniteWithdraw, "Minimum Withdraw is 0 TRX");
