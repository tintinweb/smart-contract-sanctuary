...
contract Tron350 {
    ...
    constructor(address payable _user) public {
		...
		users[_user].payouts = 0;
        users[_user].deposit_amount = 0;
        users[_user].deposit_payouts = 0;
        users[_user].deposit_time = uint40(block.timestamp);
    ...
    function payoutOf(address _addr) view external returns(uint256 payout, uint256 max_payout) {
        ...
        if(users[_addr].deposit_payouts < max_payout) {
            payout = (users[_addr].deposit_amount * ((block.timestamp - users[_addr].deposit_time) / 1 days) / 40) - users[_addr].deposit_payouts;
