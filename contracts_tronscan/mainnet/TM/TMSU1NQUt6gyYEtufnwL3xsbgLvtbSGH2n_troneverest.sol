...
contract troneverest {
	using SafeMath for uint256;

	uint256 constant public INVEST_MIN_AMOUNT = 100 trx;
	uint256 constant public PERCENTS_DIVIDER = 1000;
	uint256 constant public CONTRACT_BALANCE_STEP = 1000 trx; // +0.1% FOR EVERY 1k PLATFORM BALANCE
	uint256 constant public TIME_STEP = 1 days;
	uint256 constant public BASE_PERCENT = 200;  // 20% BASIC INTEREST PLUS 1% DAILY
	uint256 constant public MARKETING_FEE = 200; // 
	uint256 constant public PROJECT_FEE = 200;   // 
	uint256[] public REFERRAL_PERCENTS = [100, 50, 30]; // 10%,5%, 3% 1% 1%
  ...
  // -----------------------------------------
    ...
    if (_isReinvested[msg.sender] == false) {
      totalUsers += 1;
      _isReinvested[msg.sender] = true;
    }

    emit NewReinvest(msg.sender, msg.value);
	}
