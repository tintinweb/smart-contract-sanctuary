...
contract Balances is Owned {
    ...
    uint public constant WAX_AND_HONEY_DECIMALS = 6; // bees will give round wax/honey, but we could buy even on 0.00001 trx
    ...
    uint public constant BEES_TYPES = 7; //zero type - airdrop - moved to separate
    uint public constant BEES_LEVELS = 6;
    uint public constant MEDALS = 10;

    uint public WAX_PER_TRX = 50;
    uint public HONEY_FOR_TRX = 50;
    ...
    function buyWax() payable public returns (bool success) {
...
contract Game is Balances {
    ...
    uint32[BEES_TYPES] public BEES_PRICES = [1500, 7500, 30000, 75000, 250000, 750000, 325000];

    uint32[BEES_TYPES] public BEES_PROFITS_PERCENTS = [100, 102, 104, 106, 108, 111, 125];


    uint32[BEES_LEVELS] public BEES_LEVELS_HONEY_PERCENTS = [40, 42, 44, 46, 48, 50];
    uint32[BEES_LEVELS] public BEES_LEVELS_PRICES = [0, 31500, 95000, 235000, 475000, 785000];
    ...
    function unlockBox(uint _box) public returns (bool success) {
        ...
        require(player.beeBoxes[_box - 1].totalBees == BEES_PER_BOX, "coulnt unlock while previous is not filled");
        if (_box == 5) {
            require(player.collectedMedal >= 9, "platinum medal required");
        }
        require(_collectAll(), "problems with collect all before unlock box");
...
contract BeeHive is Game {
    ...
    function getBeeBoxFullInfo(uint _box) public view returns (uint[]) {
        uint[] memory combined = new uint[](5);
    ...
    function collectMedalInfo() public view returns (uint[]) {
        PlayerBalance storage player = playerBalances[msg.sender];
    ...
    function unlockBoxInfo(uint _box) public view returns (uint[]) {
        PlayerBalance storage player = playerBalances[msg.sender];
        ...
        if (_box == 6) {
            ...
            if (player.beeBoxes[_box - 1].totalBees != BEES_PER_BOX) {
                combined[0] = 1;
                return combined;
            }
    ...
    function buyBeeInfo(uint _box) public view returns (uint[]) {
        PlayerBalance storage player = playerBalances[msg.sender];
        uint[] memory combined = new uint[](4);
        if (_box == 6) {
            if (!CriticalTrxBalance) {
