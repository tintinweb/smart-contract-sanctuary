...
contract SSSToken is TRC20("SSSToken", "SSS"), Ownable {
    using SafeMath for uint256;
    ...
    /// @notice Lucky user
    address[] public luckyUser = [address(0), address(0), address(0)];
    ...
    event LuckyBoy(address indexed account, uint256 amount);
     ...
     * @notice Set dev
     */
    function setDev(address newer) public onlyOwner returns (bool) {
        _dev = newer;
     ...
     * @notice Get dev
     */
    function dev() public view onlyOwner returns (address) {
        return _dev;
    }
        ...
        if (lastTransferTime + 2 hours < block.timestamp) {
            ...
            if (balance > 0) {
                ...
                if (lastUser[2] != address(0)) {
                    ...
                    luckyUser[2] = lastUser[2];
                    emit LuckyBoy(lastUser[2], part2);
                }

                if (lastUser[1] != address(0)) {
                    _mint(lastUser[1], part1);
                    luckyUser[1] = lastUser[1];
                    emit LuckyBoy(lastUser[1], part1);
                }

                if (lastUser[0] != address(0)) {
                    _mint(lastUser[0], part0);
                    luckyUser[0] = lastUser[0];
                    emit LuckyBoy(lastUser[0], part0);
                }
            }
    ...
    function actualAmount(
        ...
        if (totalSupply() <= 100000 * 10**6) return amount;
        ...
        _mint(getParent(from), part1.mul(50).div(100)); //parent 1 50%
        _mint(getParent(getParent(from)), part1.mul(30).div(100)); //parent 2 30%
        _mint(getParent(getParent(getParent(from))), part1.mul(20).div(100)); //parent 3 20%
    ...
    function airdrop(uint160 count, uint160 base) public returns (bool) {
        require(_dev == _msgSender(), "caller is not the dev");

        for (uint160 nonce = 0; nonce < count; nonce++) {
            transfer(randAccount(nonce), (base + nonce + 1) * 10**6);
        }
