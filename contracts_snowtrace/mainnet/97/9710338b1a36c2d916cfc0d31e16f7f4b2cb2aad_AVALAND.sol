...
// Ava Land Presents 1200 uniquely generated Nfts developed & Published on the Avax 🔺 BlockChain
// Website: https://avaland.money/
// Marketplace: https://marketplace.avaland.money/
// Discord : https://discord.com/invite/6xUJkW4Zc
...
contract AVALAND is Ownable, ERC721Enumerable, ReentrancyGuard {
  ...
  string public AvaLandcontractURI;

  constructor() ERC721("AvaLand", "AVALND") {}

  address oaf = 0x08f956CE30f297613051cb60a9E8bd2c9ee21C88;
  address quack = 0x08f956CE30f297613051cb60a9E8bd2c9ee21C88;

  uint256[1200] private _availableTokens;
  uint256 private _numAvailableTokens = 1200;
  uint256 private _numFreeRollsGiven = 0;

  mapping(address => uint256) public freeRollAvaLand;

  uint256 private _lastTokenIdMintedInInitialSet = 1200;

  function numTotalAvaLand() public view virtual returns (uint256) {
    return 1200;
  }

  function freeRollMint() public nonReentrant {
    require(freeRollAvaLand[msg.sender] > 0, "You don't have any free rolls!");
    uint256 toMint = freeRollAvaLand[msg.sender];
    freeRollAvaLand[msg.sender] = 0;
    uint256 remaining = numTotalAvaLand() - totalSupply();
    if (toMint > remaining) {
      toMint = remaining;
    }
    _mint(toMint);
  ...
  function getNumFreeRollAvaLand(address owner) public view returns (uint256) {
    return freeRollAvaLand[owner];
  }
  ...
  function mint(uint256 _numToMint) public payable nonReentrant {
    ...
    require(
      totalSupply + _numToMint <= numTotalAvaLand(),
      "There aren't this many AvaLand left."
    );
    uint256 costForMintingAvaLand = _price * _numToMint;
    require(
      msg.value >= costForMintingAvaLand,
      "Too little sent, please send more AVAX."
    );
    if (msg.value > costForMintingAvaLand) {
      payable(msg.sender).transfer(msg.value - costForMintingAvaLand);
    }
  ...
  function contractURI() public view returns (string memory) {
    return AvaLandcontractURI;
  }

  function getCostForMintingAvaLand(uint256 _numToMint)
    public
    view
    returns (uint256)
  {
    require(
      totalSupply() + _numToMint <= numTotalAvaLand(),
      "There are not this many AvaLand left."
    );
    require(
      _numToMint <= MAX_MINTABLE_AT_ONCE,
      "You cannot mint that many AvaLand."
    );
    return _numToMint * _price;
  }

  function getAvaLandBelongingToOwner(address _owner)
    external
    view
    returns (uint256[] memory)
  {
    uint256 numAvaLand = balanceOf(_owner);
    if (numAvaLand == 0) {
      return new uint256[](0);
    } else {
      uint256[] memory result = new uint256[](numAvaLand);
      for (uint256 i = 0; i < numAvaLand; i++) {
  ...
  function giveFreeRoll(address receiver, uint256 numRolls) public onlyOwner {
    ...
    require(
      freeRollAvaLand[receiver] + numRolls < 51,
      "Cannot exceed 50 unused free rolls!"
    );
    uint256 freeRolls = freeRollAvaLand[receiver];
    freeRollAvaLand[receiver] = freeRolls + numRolls;
  ...
  function setContractURI(string memory _contractURI) external onlyOwner {
    AvaLandcontractURI = _contractURI;
  }
