...
contract RagnarokTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SRGK }
    ...
    address public sRGK;
    uint public sRGKQueue; // Delays change to sRGK address
    ...
    constructor (
        ...
        isReserveToken[ _RGK ] = true;
        reserveTokens.push( _RGK );

        isReserveToken[ _MIM ] = true;
        reserveTokens.push( _MIM );

        isReserveToken[ _RGK_MIM_LP] = true;
        reserveTokens.push( _RGK_MIM_LP );

        blocksNeededForQueue = _blocksNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for RGK
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint RGK needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( RGK ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn RGK for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IRGKERC20( RGK ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sRGK ).balanceOf( msg.sender ); // Can only borrow against sRGK held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with RGK
        @param _amount uint
     */
    function repayDebtWithRGK( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IRGKERC20( RGK ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, RGK, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( RGK ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( RGK ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns RGK valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match RGK decimals
            value_ = _amount.mul( 10 ** IERC20( RGK ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SRGK ) { // 9
            sRGKQueue = block.number.add( blocksNeededForQueue );
        } else return false;
