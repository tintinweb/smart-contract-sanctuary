...
contract TimeTreasury is Ownable {
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( Time ).totalSupply().sub( totalDebt ) ).mul( uimultiplier );
    }
	
	/**
        @notice returns EC multiplier rate
        @return uint
     */
    function ecMultiplierRate() public view returns ( uint ) {
        return uimultiplier;
    }

    /**
        @notice sets EC multiplier rate
     */
    function setEcMultiplierRate(uint _multiplier) external onlyManager() {
