...
contract ChiknNameService is Ownable {
    ...
    // CNS variables
    uint256 public CNS_BASE_PRICE = 999999000000000000000000; // number of egg required to change the name
    ...
    uint256 public PERSONALITY_BASE_PRICE = 999999000000000000000000; // number of egg required to change the personality
    uint256 public LORE_MAX_LENGTH = 140;
    uint256 public FLAVOUR_MAX_LENGTH = 70;

    // RNS vairable
    uint256 public RNS_BASE_PRICE = 999999000000000000000000; // number of egg required to change the Roost name
    ...
    // update cns and personality in one function
    // this function work out if cns is empty or personality is empty so calculate the right price
    function nameAndPersonalityChange(
        uint256 tokenId,
        string memory name,
        string memory lore,
        string memory flavour
        ...
        ChickenRunV5 x = ChickenRunV5(CHIKN_CONTRACT);
        require(x.ownerOf(tokenId) == msg.sender, "Not owner");
        bytes32 hashedName = keccak256(abi.encodePacked(_toLower(name)));
        if (keccak256(abi.encodePacked("")) != hashedName) {
            price += CNS_BASE_PRICE;
            // validate name
            isCNSNameAvailable(name);
            setName = true;
        }
        bytes32 hashedPersonality = keccak256(abi.encodePacked(_toLower(lore)));
        if (keccak256(abi.encodePacked("")) != hashedPersonality) {
            price += PERSONALITY_BASE_PRICE;
            // validate lore
            isPersonalityFit(lore, flavour);
            setPersonality = true;
        }
        EggV2 eggContract = EggV2(EGG_CONTRACT);
        uint256 available = eggContract.balanceOf(msg.sender);
        require(available >= price, "Not enough egg");
        // burn egg
        eggContract.burnEggs(msg.sender, price);
        if (setName) {
            string memory currentChiknName = x.cns(tokenId);
            bytes memory byteName = bytes(currentChiknName);
            // remove previous name so other can use
            if (byteName.length > 0) {
                // assume chikn have already been name
                bytes32 oldName = keccak256(
                    abi.encodePacked(_toLower(currentChiknName))
            ...
            cnsAlreadyRegisted[hashedName] = true;
            // and update the chikn contract
            x.changeName(tokenId, name);
            emit CNSChange(tokenId, currentChiknName, name);
        }
    ...
    function updateNameLength(
        uint256 _rnsMaxLength,
        uint256 _cnsMaxLength,
        uint256 _loreMaxLength,
        uint256 _flavourMaxLength
    ) external onlyOwner {
