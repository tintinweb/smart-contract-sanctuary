...
contract KamiWorld is Ownable, ERC721Enumerable, ERC721Burnable {
    ...
    constructor(address[] memory whiteList, uint256 whiteListSize, uint256 _maxTokens, uint256[] memory _tokenId) ERC721("KamiWorld", "KAMI") {
      uint256 mintLength = _tokenId.length;
      for(uint i = 0; i < mintLength; i++){
        _safeMint(msg.sender, _tokenId[i]);
        emit Mint(msg.sender, _tokenId[i]);
        ...
        bool whitelistMintEnabled = true;
    }
    ...
    function whitelistMint(address account, uint256 _count) public payable {
