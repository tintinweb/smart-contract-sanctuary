...
contract CypherTreasury is Ownable {
    ...
    ITIMEERC20 public immutable CYPHER;
    ...
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        CYPHER.burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = ORACLE.balanceOf( msg.sender ); // Can only borrow against sOHM held
    ...
    function repayDebtWithTime( uint _amount ) external {
        ...
        CYPHER.burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, address(CYPHER), _amount, _amount );
    }
...
//TimeTreasury
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        CYPHER.mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( CYPHER.totalSupply().sub( totalDebt ) );
    }
    ...
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            ...
            value_ = _amount.mul( 10 ** CYPHER.decimals() ).div( 10 ** IERC20( _token ).decimals() );
