...
contract AtpadPresale is ReentrancyGuard, Ownable, Pausable {
    ...
    uint initialVestPercentage= 10;

    uint startVest = 1642881300;        // Saturday 22 January 2022 19:55:00
    uint endVest  = 1673985300;         // Tuesday 17 January 2023 19:55:00
    ...
    uint public allocateTotal;  
    uint public swapTotal;      
    uint public pendingTotal;  
    uint public claimedTotal; 
    ...
    function swap(uint _amount) external payable nonReentrant swapEnabled onProgress { // 
    ...
    function swapClaim() external payable nonReentrant claimEnabled whenFinished {   //
        
        uint _free = getReleasedPercentageFromTable();
    ...
    function getReleasedPercentageFromTable() view public returns (uint) {
        ...
        //          timestamp   vest                time            30days  release totalRelease
        if (_time > startVest) _perc = 10;  // 1  22-01-2022 19:55  2592000 20-time
        if (_time > startVest.add(_month.mul(1))) _perc = 18;  // 2  21-02-2022 19:55  2592000 8%  18%
        if (_time > startVest.add(_month.mul(2))) _perc = 26;  // 3  23-03-2022 19:55  2592000 8%  26%
        if (_time > startVest.add(_month.mul(3))) _perc = 34;  // 4  22-04-2022 19:55  2592000 8%  34%
        if (_time > startVest.add(_month.mul(4))) _perc = 42;  // 5  22-05-2022 19:55  2592000 8%  42%
        if (_time > startVest.add(_month.mul(5))) _perc = 50;  // 6  21-06-2022 19:55  2592000 8%  50%
        if (_time > startVest.add(_month.mul(6))) _perc = 58;  // 7  21-07-2022 19:55  2592000 8%  58%
        if (_time > startVest.add(_month.mul(7))) _perc = 66;  // 8  20-08-2022 19:55  2592000 8%  66%
        if (_time > startVest.add(_month.mul(8))) _perc = 74;  // 9  19-09-2022 19:55  2592000 8%  74%
        if (_time > startVest.add(_month.mul(9))) _perc = 82;  //10  19-10-2022 19:55  2592000 8%  82%
        if (_time > startVest.add(_month.mul(10))) _perc = 90;  //11  18-11-2022 19:55  2592000 8%  90%
        if (_time > startVest.add(_month.mul(11))) _perc = 95;  //12  18-12-2022 19:55  2592000 5% 100%
        if (_time > startVest.add(_month.mul(12))) _perc = 100; //12  17-1-2023 19:55  2592000 5% 100%

        return _perc;
    }

    /** better routine, not unit tested though.....
    function getReleasedPercentageFromCalculation() view public returns (uint) {

        uint _time = block.timestamp;
        uint _perc = 0 ;
        uint _month = 2592000;  

        if (_time <=  startVest) return 0;
        for(uint i = 0 ; i < 11 ; i++) {
            if (_time > startVest.add(i.mul(_month)))_perc=(20).add(i.mul(7));  
        }
        if (_time > endVest) return 100;
        return _perc;
    ...
    function getUserAllocated(address _wallet) view external returns (uint) {
        ...
        uint _perc = getReleasedPercentageFromTable();
