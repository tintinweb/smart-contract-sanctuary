...
interface IStableSwap{
    ...
    function get_dy(int128 i,int128 j,uint dx) external view returns(uint256);
}
...
contract CurveNormalNoMeta is Ownable, BigfootConfig {
  ...
  function checkBalanced(uint _a, uint _b, address _curveswap, uint _maxPriceDiff) public view returns(bool){
      address coinA = IStableSwap(_curveswap).coins(_a);
      address coinB = IStableSwap(_curveswap).coins(_b);

      uint aDecimals = uint(IDecimals(coinA).decimals());
      uint bDecimals = uint(IDecimals(coinB).decimals());

      uint priceAB = IStableSwap(_curveswap).get_dy(int128(_a),int128(_b), 10**aDecimals);

      require(priceAB < 10**bDecimals*_maxPriceDiff/10000, "pool unbalanced 0");
      require(priceAB > 10**bDecimals*(20000-_maxPriceDiff)/10000, "pool unbalanced 1");
      return true;
  }

  /// @dev Return whether the given pool is stable, presumably not under manipulation.
  function isStable(address bigfoot) public view returns (bool) {
    address cvswap;
    {
        cvswap = IBorrow(bigfoot).curveswap();
    }
    // 2. Check that price is in the acceptable range

    uint maxPriceDiff = bigfoots[bigfoot].maxPriceDiff;
    
    checkBalanced(0,1,cvswap,maxPriceDiff);
    checkBalanced(0,2,cvswap,maxPriceDiff);
    checkBalanced(2,1,cvswap,maxPriceDiff);
