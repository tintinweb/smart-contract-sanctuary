...
contract StrategyAddCurve is Ownable, ReentrancyGuard, Strategy {
  ...
  function execute(
    ...
    _amounts[getTokenIndex(fToken,curveswap)] = fToken.myBalance();
    _amounts[getTokenIndex(banktoken,curveswap)] = banktoken.myBalance();

    ICurveSwap(curveswap).add_liquidity(_amounts,0);

    uint have = curveswap.myBalance();

    require(have >= minLPAmount, 'insufficient LP tokens received');
    curveswap.safeApprove(vault, uint(-1));
