...
contract SITHBondingCalculator is IBondingCalculator {

    using FixedPoint for *;
    using LowGasSafeMath for uint;
    using LowGasSafeMath for uint112;

    IERC20 public immutable SITH;

    constructor( address _SITH ) {
        require( _SITH != address(0) );
        SITH = IERC20(_SITH);
    }
    ...
    function markdown( address _pair ) external view returns ( uint ) {
        ...
        uint reserve;
        if ( IUniswapV2Pair( _pair ).token0() == address(SITH) ) {
            reserve = reserve1;
        } else {
            require(IUniswapV2Pair( _pair ).token1() == address(SITH), "not a SITH lp pair");
            reserve = reserve0;
        }
        return reserve.mul( 2 * ( 10 ** SITH.decimals() ) ).div( getTotalValue( _pair ) );
    }
}