...
contract VerseTreasury is Ownable {
    ...
    address public sVerse;
    uint public sVERSEQueue; // Delays change to sVERSE address
    ...
    //    isLiquidityToken[ _VERSEDAI ] = true;
    //    liquidityTokens.push( _VERSEDAI );

        secondsNeededForQueue = _secondsNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for VERSE
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint VERSE needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( Verse ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn VERSE for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IVERSE( Verse ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sVerse ).balanceOf( msg.sender ); // Can only borrow against sVERSE held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with VERSE
        @param _amount uint
     */
    function repayDebtWithVERSE( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IVERSE( Verse ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, Verse, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( Verse ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( Verse ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns VERSE valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match VERSE decimals
            value_ = _amount.mul( 10 ** IERC20( Verse ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SVERSE ) { // 9
            sVERSEQueue = uint32(block.timestamp).add32( secondsNeededForQueue );
        } else return false;
