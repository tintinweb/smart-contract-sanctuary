...
// Dependency file: contracts/baby/LootPrinterDividendTracker.sol
    ...
    DividendPayingTokenOptionalInterface
{
    ...
    function distributeLOOTDividends(uint256 amount) public onlyOwner {
...
contract LootPrinterDividendTracker is OwnableUpgradeable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
...
// Root file: contracts/baby/LootPrinter.sol

pragma solidity =0.8.4;


contract LootPrinter is ERC20, Ownable, BaseToken {
    using SafeMath for uint256;
    ...
    LootPrinterDividendTracker public dividendTracker;
    ...
    constructor(
        ...
        dividendTracker = LootPrinterDividendTracker(
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        LootPrinterDividendTracker newDividendTracker = LootPrinterDividendTracker(
            payable(newAddress)
        );

        require(
            newDividendTracker.owner() == address(this),
            "LootPrinter: The new dividend tracker must be owned by the LootPrinter token contract"
        );
    ...
    function updateJoeRouter(address newAddress) public onlyOwner {
        require(
            ...
            "LootPrinter: The router already has that address"
        );
    ...
    function swapAndSendToFee(uint256 tokens) private {
        uint256 initialLOOTBalance = IERC20(rewardToken).balanceOf(
            address(this)
        );

        swapTokensForLOOT(tokens);
    ...
    function swapTokensForLOOT(uint256 tokenAmount) private {
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        if (success) {
            dividendTracker.distributeLOOTDividends(dividends);
