...
contract BEP20 is Context, IBEP20, Ownable {
    uint256 constant MAX_SUPPLY = 80000 * (10 ** 18);
...
contract SnowToken is BEP20 {
    // antiBot, default to true. True allowed to cut the maximum transfer tax rate in order to prevent bot to buy at the launch.
    // If antiBot is set to 0, is it not possible to re set at 1, only one time.
    uint256 public antiBot = 1;
    // Max transfer tax rate: 10%.
    uint16 public constant MAXIMUM_TRANSFER_TAX_RATE = 1000;
    // Transfer tax rate in basis points. (default 5%)
    uint16 public transferTaxRate = 500;
    // Fees rate % of transfer tax. (default 50% x 5% = 2,5% of total amount).
    uint16 public feesRate = 50;
    ...
    // Min amount to liquify. (default 1 Snow)
    uint256 public minAmountToLiquify = 1 ether;
    // The swap router, modifiable. Will be changed to SnowSwap's router when our own AMM release
    IPangolinRouter public snowSwapRouter;
    // The trading pair
    address public snowSwapPair;
    // In swap and liquify
...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 accSnowPerShare;   // Accumulated SNOWs per share, times 1e12. See below.
    ...
    // Bonus multiplier for early snow makers.
    ...
    // Max deposit fee
    uint16 public maxDepositFeeBP = 400;
    ...
    event UpdateEmissionRate(address indexed user, uint256 snowPerBlock);
    ...
    function pendingSnow(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 snowReward = multiplier.mul(snowPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accSnowPerShare = accSnowPerShare.add(snowReward.mul(1e12).div(lpSupply));
        }

        return user.amount.mul(accSnowPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 snowReward = multiplier.mul(snowPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        snow.mint(devAddress, snowReward.div(10));
        snow.mint(address(this), snowReward);
        pool.accSnowPerShare = pool.accSnowPerShare.add(snowReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accSnowPerShare).div(1e12).sub(user.rewardDebt);
    ...
    function safeSnowTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > snowBal) {
            transferSuccess = snow.transfer(_to, snowBal);
        } else {
            transferSuccess = snow.transfer(_to, _amount);
        }

        require(transferSuccess, "safeSnowTransfer: transfer failed");
    }
    ...
    // Update fees address by the previous fees address.
    ...
    function updateEmissionRate(uint256 _snowPerBlock) public onlyOwner {
        massUpdatePools();
        snowPerBlock = _snowPerBlock;
        emit UpdateEmissionRate(msg.sender, _snowPerBlock);
    }
