...
Levine Capital Aggregation Fund: $LCAF
- Deflationary DeFi-as-a-Service (DaaS) Token, with 60% supply burned to 0x0dEaD
- Our advisors will bootstrap the investments in our initial phase. We invest on multiple chains and return the profits to $LCAF holders.

Tokenomics:
- Buy Tax / Sell Tax: 14%
    - 5% of each buy goes to ETH reflections. We will have dashboard to claim ETH soon
    - 5% of each sell to our team wallet
...
Telegram:
http://t.me/Levine_Capital

Twitter:
https://twitter.com/Levine_Capital

Medium:
http://levine-capital.medium.com
...
contract LevineCapitalAggregationFundDividendTracker is IDividendDistributor {
...
contract Test is Ownable, IERC20 {
    ...
    address marketing = address(0x92314868984B92e780B19abdA64B948b82943Dee);
    address liquidity = address(0xdCc8591c5C766B64414956CF1132C103404eDFbd);

    IUniswapV2Router02 public router;
    address pair;

    LevineCapitalAggregationFundDividendTracker dividendTracker;
    ...
    modifier open(address from, address to, bool isBot) {
        require(isOpen || from == owner() || to == owner() || !isBot, "Not Open");
        _;
    }

    constructor (address[] memory botAddressBlacklist) {
        blacklistBotAddress(botAddressBlacklist, true);
        ...
        dividendTracker = new LevineCapitalAggregationFundDividendTracker(msg.sender);
    ...
    function blacklistBotAddress(address[] memory _botAddress, bool exempt) public onlyOwner {
