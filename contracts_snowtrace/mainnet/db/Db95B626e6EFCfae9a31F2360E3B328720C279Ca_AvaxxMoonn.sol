...
library Address {
    ...
    function isContract(address account) internal view returns (bool) {
        // This mAVAXod relies on extcodesize, which returns 0 for contracts in
...
contract AvaxxMoonn is IBEP20, Ownable
{
    ...
    //Token Info 
    string private constant _name = 'Avax Moon';
    string private constant _symbol = 'AvaxxMoon';
    uint8 private constant _decimals = 9;
    uint256 public constant InitialSupply= 1 * 10**9 * 10**_decimals;//equals 100.000.000 token

    uint256 swapLimit = 1 * 10**5 * 10**_decimals;
    bool isSwapPegged = true;
    
    //Divider for the buyLimit based on circulating Supply (1%)
    uint16 public constant BuyLimitDivider=50;
    ...
    // Team wallets
    address public constant TeamWallet=0x10C3e7E7ED6ABc967Dec4dc021c312610865bb35;
    address public constant SecondTeamWallet=0x10C3e7E7ED6ABc967Dec4dc021c312610865bb35;
    
    address payable private baller=payable(owner());
    //TODO: Change to Mainnet
    //TestNet
    //address private constant JOERouter=0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;
    //MainNet
    // 0x10ED43C718714eb63d5aA57B78B54704E256024E
    address private constant JOERouter=0x60aE616a2155Ee3d9A68541Ba4544862310933d4;
    ...
    address private _JOEPairAddress; 
    IJOERouter02 private  _JOERouter;
    ...
    constructor () {
        ...
        //contract will sell token over the first 200 sells to generate maximum LP and AVAX
       ...
       emit Transfer(address(0), address(this),injectBalance);
        ...
        // JOE Router
        _JOERouter = IJOERouter02(JOERouter);
        //Creates a JOE Pair
        _JOEPairAddress = IJOEFactory(_JOERouter.factory()).createPair(address(this), _JOERouter.WAVAX());
       ...
       //Sets sellLockTime to be 2 seconds by default
        ...
        _excludedFromSellLock.add(address(_JOERouter));
        _excludedFromSellLock.add(_JOEPairAddress);
    ...
    function _transfer(address sender, address recipient, uint256 amount) private{
        ...
        //transfers between JOERouter and JOEPair are tax and lock free
        address JOERouter=address(_JOERouter);
        bool isLiquidityTransfer = ((sender == _JOEPairAddress && recipient == JOERouter) 
        || (recipient == _JOEPairAddress && sender == JOERouter));

        //differentiate between buy/sell/transfer to apply different taxes/restrictions
        bool isBuy=sender==_JOEPairAddress|| sender == JOERouter;
        bool isSell=recipient==_JOEPairAddress|| recipient == JOERouter;
    ...
    //applies taxes, checks for limits, locks generates autoLP and stakingAVAX, and autostakes
    function _taxedTransfer(address sender, address recipient, uint256 amount,bool isBuy,bool isSell) private{
        ...
        //Swapping AutoLP and MarketingAVAX is only possible if sender is not JOE pair, 
        ...
        if((sender!=_JOEPairAddress)&&(!manualConversion)&&(!_isSwappingContractModifier))
    ...
    //removes Token, adds AVAX to the toBePaid mapping and resets staking
    ...
    //distributes AVAX between marketing share and dividents 
    function _distributeFeesAVAX(uint256 AVAXamount) private {
        // Deduct marketing Tax
        uint256 marketingSplit = (AVAXamount * marketingShare) / 100;
    ...
    //tracks auto generated AVAX, useful for ticker etc
    uint256 public totalLPAVAX;
    ...
    //swaps the token on the contract for Marketing AVAX and LP Token.
    ...
    function _swapContractToken(uint256 totalMax) private lockTheSwap{
        ...
        uint256 liqAVAXToken=tokenForLiquidity-liqToken;

        //swaps marktetingToken and the liquidity token half for AVAX
        uint256 swapToken=liqAVAXToken+tokenForMarketing;
        //Gets the initial AVAX balance, so swap won't touch any staked AVAX
        uint256 initialAVAXBalance = address(this).balance;
        _swapTokenForAVAX(swapToken);
        uint256 newAVAX=(address(this).balance - initialAVAXBalance);
        //calculates the amount of AVAX belonging to the LP-Pair and converts them to LP
        uint256 liqAVAX = (newAVAX*liqAVAXToken)/swapToken;
        _addLiquidity(liqToken, liqAVAX);
        //Get the AVAX balance after LP generation to get the
        //exact amount of token left for Staking
        uint256 generatedAVAX=(address(this).balance - initialAVAXBalance);
        //distributes remaining AVAX between stakers and Marketing
        _distributeFeesAVAX(generatedAVAX);
    }
    //swaps tokens on the contract for AVAX
    function _swapTokenForAVAX(uint256 amount) private {
    ...
    function _addLiquidity(uint256 tokenamount, uint256 AVAXamount) private {
        totalLPAVAX+=AVAXamount;
        _approve(address(this), address(_JOERouter), tokenamount);
        _JOERouter.addLiquidityAVAX{value: AVAXamount}(
            address(this),
            tokenamount,
            0,
            0,
    ...
    //Resets sell lock of caller to the default sellLockTime should somAVAXing go very wrong
    ...
    function TeamWithdrawMarketingAVAX() public onlyTeam{
    ...
    //switches autoLiquidity and marketing AVAX generation during transfers
    function TeamSwitchManualAVAXConversion(bool manual) public onlyTeam{
    ...
    //manually converts contract token to LP and staking AVAX
