...
contract ParrotAvaxBondDepository is Ownable {
    ...
    /* ======== STATE VARIABLES ======== */
    IERC20 public immutable Rug; // token given as payment for bond
    IWAVAX9 public immutable principle; // token used to create bond
    ITreasury public immutable treasury; // mints Rug when receives principle
    ...
    function deposit( 
        ...
        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 Rug ( underflow protection )
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            Rug.transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                Rug.approve( address(stakingHelper), _amount );
                stakingHelper.stake( _amount, _recipient );
            } else {
                Rug.approve( address(staking), _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return Rug.totalSupply().mul( terms.maxPayout )/ 100000;
    }
