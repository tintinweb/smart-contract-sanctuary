...
contract NovaTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SNOVA }
    ...
    uint public sNOVAQueue; // Delays change to sNOVA address
    ...
    /**
        @notice allow approved address to deposit an asset for NOVA
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint NOVA needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( NOVA ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn NOVA for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        INOVAERC20( NOVA ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sNOVA ).balanceOf( msg.sender ); // Can only borrow against sNOVA held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with NOVA
        @param _amount uint
     */
    function repayDebtWithNOVA( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        INOVAERC20( NOVA ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, NOVA, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( NOVA ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( NOVA ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns NOVA valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match NOVA decimals
            value_ = _amount.mul( 10 ** IERC20( NOVA ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SNOVA ) { // 9
            sNOVAQueue = block.number.add( blocksNeededForQueue );
        } else return false;
    ...
    /**
        @notice cNOVAks requirements and returns altered structs
    ...
    /**
        @notice cNOVAks array to ensure against duplicate
        @param _list address[]
        @param _token address
        @return bool
     */
