...
contract IShareRewardPool {
    ...
    uint256 public iSharePerSecond = 0.00187687 ether; // 60000 ISHARE / (370 days * 24h * 60min * 60s)
    ...
    // Update the given pool's ISHARE allocation point. Can only be called by the owner.
    ...
    function governanceRecoverUnsupported(IERC20 _token, uint256 amount, address to) external onlyOperator {
        if (block.timestamp < poolEndTime + 90 days) {
            // do not allow to drain core token (ISHARE or lps) if less than 90 days after pool ends
            require(_token != ishare, "ishare");
            uint256 length = poolInfo.length;
            for (uint256 pid = 0; pid < length; ++pid) {
