...
contract SOV is Context, IERC20, Ownable {
    ...
    address buybackAddress = 0xFa4deeA3a4e0119C6275c0C0fc8e96dc607E24A7;
    ...
    uint256 private _tokenTotal = 500000000 * _decimalFactor ; 
    ...
    constructor () {
        ...
        _isExcludedFromReward[buybackAddress] = true;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        _reflectBalances[buybackAddress]+=burnToken*currentRate;
        ...
        if(_isExcludedFromReward[buybackAddress]) _balances[buybackAddress] += burnToken;
        _reflectedTotal -= reflectToken*currentRate;
        emit Transfer(sender, recipient, amount-burnToken-reflectToken);
        if (burnToken!=uint256(0)) emit Transfer(sender, buybackAddress, burnToken);
    }
    ...
    function getTransferFee(address sender, address recipient) private view returns (uint256, uint256){
        ...
        uint256 burnFee = 400;
        uint256 reflectFee = 400;
        uint256 burnt = balanceOf(buybackAddress);
    ...
    function _getSupply() public view returns(uint256){
        return _tokenTotal-balanceOf(buybackAddress);
    }
    ...
    function getFeeInfo() public view returns(uint256,uint256){
        return (getTransferFee(buybackAddress, buybackAddress));
    }
