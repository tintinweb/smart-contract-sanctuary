...
contract POSTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SPOS }
    ...
    address public sPOS;
    uint public sPOSQueue; // Delays change to sPOS address
    ...
    /**
        @notice allow approved address to deposit an asset for POS
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint POS needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( POS ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn POS for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IPOSERC20( POS ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sPOS ).balanceOf( msg.sender ); // Can only borrow against sPOS held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with POS
        @param _amount uint
     */
    function repayDebtWithPOS( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IPOSERC20( POS ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, POS, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( POS ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( POS ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns POS valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match POS decimals
            value_ = _amount.mul( 10 ** IERC20( POS ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SPOS ) { // 9
            sPOSQueue = block.number.add( blocksNeededForQueue );
        } else return false;
