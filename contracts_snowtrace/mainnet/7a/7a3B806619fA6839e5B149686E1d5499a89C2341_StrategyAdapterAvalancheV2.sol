...
contract StrategyAdapterAvalancheV2 is AlphaStrategyAvalanche {
    ...
    function initialize(
        ...
        AlphaStrategyAvalanche.initializeAlphaStrategy(
            ...
            address(0xd6a4F121CA35509aF06A0Be99093d08462f53052),  // Masterchef #1
            _poolId,
            24
        );
    }
...
contract AlphaStrategyAvalanche is OwnableUpgradeable {
    ...
    */
    ...
    function pendingXJoe() public view returns (uint256) {
        ...
        return masterChefBalance(joeMasterChef, poolIdSecond).add(xJoeBalance);
    }
    ...
    function withdrawReward(address user) public onlyVault {
        ...
        if(_xJoeBalance < _pendingXJoe){
            ...
            uint256 toWithdraw = Math.min(masterChefBalance(joeMasterChef, poolIdSecond), needToWithdraw);

            IMasterChef(joeMasterChef).withdraw(poolIdSecond, toWithdraw);
    ...
    function deposit(address chef, uint256 pid, uint256 bal) internal {
        IMasterChef(chef).deposit(pid, bal);
    }
    ...
    function withdrawToVault(uint256 amount) public onlyVault {
        ...
        if(amount > entireBalance){
            ...
            uint256 toWithdraw = Math.min(masterChefBalance(masterChef, poolId), needToWithdraw);
    ...
    function investedUnderlyingBalance() external view returns (uint256) {
        ...
        return masterChefBalance(masterChef, poolId).add(IERC20(underlying).balanceOf(address(this)));
    }

    function masterChefBalance(address chef, uint256 pid) internal view returns (uint256 amount) {
        (amount,) = IMasterChef(chef).userInfo(pid, address(this));
    }

    function exitJoeRewardPool() internal {
        uint256 bal = masterChefBalance(masterChef, poolId);
        if (bal != 0) {
    ...
    function claimJoeRewardPool() internal {
        uint256 balance = masterChefBalance(joeMasterChef, poolIdSecond);

        if (balance != 0) {
            deposit(joeMasterChef, poolIdSecond, 0);
        }
    }
    ...
    function enterJoeRewardPool() internal {
        ...
        deposit(masterChef, poolId, entireBalance);
    }
    ...
    function enterXJoeRewardPool() internal {
        ...
        if(entireBalance > 0) {
            IERC20(xJoe).safeApprove(joeMasterChef, 0);
            IERC20(xJoe).safeApprove(joeMasterChef, entireBalance);
        }

        deposit(joeMasterChef, poolIdSecond, entireBalance);
    }
    ...
    function setJoeMasterchef(address _address) public onlyMultisigOrOwner {
        joeMasterChef = _address;
    }
}
