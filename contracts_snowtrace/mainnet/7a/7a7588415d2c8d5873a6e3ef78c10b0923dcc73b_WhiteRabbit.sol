...
contract WhiteRabbit is ERC20, Ownable {
	...
	constructor(address payable _marketingWalletAddress) public ERC20("WhiteRabbit", "RABBIT") {
	...
	function setBuyFees(uint256 _dividendsFee, uint256 _marketingFee, uint256 _liquidityFee) public onlyOwner {
		require((_dividendsFee + _marketingFee + _liquidityFee) <= 30, "Sum of buy fees may not be over 30%");
	...
	function setSellFees(uint256 _dividendsFee, uint256 _marketingFee, uint256 _liquidityFee) public onlyOwner {
		require((_dividendsFee + _marketingFee + _liquidityFee) <= 30, "Sum of sell fees may not be over 30%");
	...
	function _transfer(address from, address to, uint256 amount) internal override {
		...
		if (hasLaunched && canSwap && !swapping && !automatedMarketMakerPairs[from] && from != owner() && to != owner()) {
			swapping = true;
		...
		if (hasLaunched && !swapping && !isExcludedFromFees[from] && !isExcludedFromFees[to]) {
			uint256 feePercent = automatedMarketMakerPairs[to] ? getSumOfSellFees() : getSumOfBuyFees();
...
contract WhiteRabbitDividendTracker is Ownable, DividendPayingToken {
	...
	constructor(address rewardsTokenAddress) public DividendPayingToken("WhiteRabbit_Dividend_Tracker", "RABBIT_Dividend_Tracker", rewardsTokenAddress) {
