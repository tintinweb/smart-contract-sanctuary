...
contract SohmoyedToken is ERC20Permit {
    ...
    modifier onlyBowl() {
        require(bowl[msg.sender], "bark");
        _;
    }
    
    uint256 public immutable maxSupply = 3_300_000_000_000 * 1e18;

    address private initializer; // its a hard job, but someone's gotta do it
    mapping(address => bool) public bowl; // bonds
    ...
    function lock(address _fetch, address _gohm) external {
        require(msg.sender == initializer, "only init");
        require(_fetch != address(0), "fetch lp: 0");
        require(_gohm != address(0), "gohm lp: 0");
        bowl[_fetch] = true;
        bowl[_gohm] = true;
