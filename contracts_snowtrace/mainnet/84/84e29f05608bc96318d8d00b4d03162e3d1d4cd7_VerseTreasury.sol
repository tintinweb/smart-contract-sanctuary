...
contract VerseTreasury is Ownable {
    ...
    uint public sAMMOQueue; // Delays change to sAMMO address
    ...
    //    isLiquidityToken[ _AMMODAI ] = true;
    //    liquidityTokens.push( _AMMODAI );

        secondsNeededForQueue = _secondsNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for AMMO
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint AMMO needed and store amount of rewards for distribution
    ...
    /**
        @notice allow approved address to burn AMMO for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IAMMO( Verse ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( MEMOries ).balanceOf( msg.sender ); // Can only borrow against sAMMO held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with AMMO
        @param _amount uint
     */
    function repayDebtWithAMMO( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IAMMO( Verse ).burnFrom( msg.sender, _amount );
    ...
    /**
        @notice returns AMMO valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match AMMO decimals
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SAMMO ) { // 9
            sAMMOQueue = uint32(block.timestamp).add32( secondsNeededForQueue );
        } else return false;
