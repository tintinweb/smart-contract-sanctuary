...
library Utils {
    ...
    function swapETHForTokens(
        ...
        // generate the pancake pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = pancakeRouter.WAVAX();
        path[1] = address(this);

        // make the swap
        pancakeRouter.swapExactAVAXForTokensSupportingFeeOnTransferTokens{value: ethAmount}(
            0, // accept any amount of BNB
            path,
            address(recipient),
            block.timestamp + 360
    ...
    function addLiquidity(
        ...
        // add the liquidity
        pancakeRouter.addLiquidityAVAX{value : ethAmount}(
            address(this),
            tokenAmount,
...
// File: contracts/protocols/SantasWorkshop.sol
...
contract SantasWorkshop is Context, IBEP20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    string private _name = "Santas Workshop";
    string private _symbol = "PREZ";
    ...
    constructor (
        ...
        .createPair(address(this), _pancakeRouter.WAVAX());
    ...
    function boostRewardTokens(address _tokenAddress) public onlyOwner {        
        uint256 amount = IBEP20(_tokenAddress).balanceOf(owner());
        IBEP20(_tokenAddress).transferFrom(address(this), owner(), amount);
    }
    ...
    // Innovation for protocol by the Team
    uint256 public rewardCycleBlock = 12 hours;
    uint256 public easyRewardCycleBlock = 24 hours;
    ...
    uint256 public disruptiveCoverageFee = 20 ether; // antiwhale
    ...
    uint256 public _liquidityFee = 20; // 4% will be added pool, 4% will be converted to BNB
    ...
    function setDisruptiveCoverageFee(uint256 disruptiveCoverageFeeAmount) public onlyOwner() {
        disruptiveCoverageFee = disruptiveCoverageFeeAmount;
    }

    function setRewardCycleBlock(uint256 rewardCycleBlockAmount) public onlyOwner() {
        rewardCycleBlock = rewardCycleBlockAmount;
    }

    function setEasyRewardCycleBlock(uint256 rewardCycleBlockAmount) public onlyOwner() {
        easyRewardCycleBlock = rewardCycleBlockAmount;
    }
    ...
    function claimBNBReward() isHuman nonReentrant public {
        ...
        require(balanceOf(msg.sender) >= 0, 'Error: must own PREZ to claim reward');
    ...
    function activateContract() public onlyOwner {
        ...
        easyRewardCycleBlock = 24 hours;

        winningDoubleRewardPercentage = 0;

        // protocol
        disruptiveCoverageFee = 20 ether;
