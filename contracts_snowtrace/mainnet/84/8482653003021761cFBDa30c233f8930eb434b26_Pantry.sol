...
// File contracts/Pantry.sol

pragma solidity ^0.8.4;



contract Pantry is ERC20("Staked Cookie", "sCOOKIE"), Ownable {
    ...
    uint256 public frozenCookie; // $COOKIE pending withdraw
    ...
    function setCookie(Cookie _cookie) external onlyOwner {
        require(address(cookie) == address(0), "Cookie contract already set");
    ...
    function stake(uint256 _amount) public {
        ...
        // If no sCOOKIE exists, mint it 1:1 to the amount put in
        if (totalShares == 0 || cookieBalance() == 0) {
            _mint(msg.sender, _amount);
        } else {
            uint256 what = _amount.mul(totalShares).div(cookieBalance());
            _mint(msg.sender, what);
        }

        cookie.transferToPantry(msg.sender, _amount);
    }
    ...
    function quickUnstake(uint256 _share) public {
        ...
        cookie.transfer(msg.sender, output);
    }

    // argument specified in sCOOKIE
    ...
    // argument specified in COOKIE, not sCOOKIE
    function claimDelayedUnstake(uint256 _amount) public {
        require(block.timestamp >= unlockTimestamps[msg.sender], "COOKIE not yet unlocked");
        require(_amount <= unlockAmounts[msg.sender], "Insufficient locked balance");

        unlockAmounts[msg.sender] -= _amount;
        cookie.transfer(msg.sender, _amount);
        frozenCookie -= _amount;
    }

    function cookieBalance() public view returns (uint256 balance) {
        balance = cookie.balanceOf(address(this)) - frozenCookie;
    }
