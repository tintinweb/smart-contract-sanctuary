...
contract VaporNodes is ERC20, Ownable, PaymentSplitter {
    ...
    uint256 private timeDelay = 300;

    mapping(address => bool) public isBlacklisted;
    mapping(address => bool) public automatedMarketMakerPairs;
    mapping(address => uint256) private timeLimitList; 
        ...
        PaymentSplitter(payees, shares)
    {
        ...
        require(totalSupply() == 1000000000e18, "CONSTR:6");
    ...
    function updateIsSlowMode(bool newVal) external onlyOwner {
        isSlowMode = newVal;
    }

    function updateMaxTx(uint8 newVal) external onlyOwner {
        maxTx = newVal;
    }

    function updateTimeDelay(uint8 newVal) external onlyOwner {
        timeDelay = newVal; 
    }
    ...
    function _transfer(
        ...
        if (from != owner() && to != joePair && to != address(joeRouter) && to != address(this) && from != address(this)) {
            ...
            // anti-whale/bot
            if (to != rewardsPool && to != teamPool && from != rewardsPool && from != teamPool) {
                if (isSlowMode) {
                    require(timeLimitList[to] + timeDelay > block.timestamp, "HIGH_F");
                    timeLimitList[to] = block.timestamp;
                }

                uint256 totalSupply = totalSupply();
                uint256 walletBalance = balanceOf(address(to));
                require(amount.add(walletBalance) <= totalSupply.mul(maxTx).div(10000), "MAX_TX");
            }
        }
