...
// File contracts/CrossChainHub.sol
...
contract CrossChainHub is Ownable {

    using SafeMath for uint256;
    using SafeERC20 for ERC20Detailed;

    ERC20Detailed public asset;

    constructor(address _asset) public {
         asset = ERC20Detailed(_asset);
     }
    ...
    function addAsset(address _token) public onlyOwner() {
      ...
      require(ERC20Detailed(_token).decimals() == asset.decimals(), "addAsset: decimal mismatch.");
    ...
    function swapIn(address _bridgedAsset, uint256 _amount) public able(_bridgedAsset) {
        ERC20Detailed bridgedAsset = ERC20Detailed(_bridgedAsset);
        require(_able[_bridgedAsset].limit > 0 && (bridgedAsset.balanceOf(address(this))).add(_amount) <= _able[_bridgedAsset].limit, "swapIn: bridge limit has been reached for now.");
        bridgedAsset.safeTransferFrom(msg.sender, address(this), _amount);
        asset.safeTransfer(msg.sender, _amount);
    }

    function swapOut(address _bridgedAsset, uint256 _amount) public able(_bridgedAsset) {
        ERC20Detailed bridgedAsset = ERC20Detailed(_bridgedAsset);
        asset.safeTransferFrom(msg.sender, address(this), _amount);
        bridgedAsset.safeTransfer(msg.sender, _amount);
    }
}