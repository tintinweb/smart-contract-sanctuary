...
contract ERC20Vesting is IERC20, IERC20Vesting, ReentrancyGuard{
	...
	*/
	 ...
	 */
	function revoke(address beneficiary, uint256 ID) external nonReentrant override returns(uint256){
		...
		emit VestingRevoked(vesting.beneficiary, vesting.ID, collected);
	...
	function abandon(uint256 ID) external nonReentrant override returns(uint256){
		...
		// emit event
		emit VestingAbandoned(vesting.beneficiary, vesting.ID, collected);
	...
	*/
	function disburse(uint256 amount) external nonReentrant override returns(uint256){
		require(msg.sender == Ownable(_token).owner(), "Vesting: E30: Authorization denied.");
