...
contract RaceXRaceNFCar is ERC721Enumerable, ERC2981RaceXNFCarRoyalties, Ownable {
  using Strings for uint256;
  ...
  constructor(
  ) ERC721("RaceX Race Car Series 1", "RCXNFCAR") {
    setBaseURI("https://racex.s3.us-east-2.amazonaws.com/");
    _baseTokenURI = "";
  ...
  function mint(address _to, uint256 _mintAmount) public payable {
    ...
    require((supply + _mintAmount) - devIssued <= maxSupply - maxDevSupply, "There are not enough NFT's to fulfil your order");
    ...
    if(supply > 0){
        uint256 reflectAmount = ((msg.value) * royaltyValue) / 10000;
        uint256 devReflect = (reflectAmount/7)*4;
        (bool sent, ) = payable(royaltyContractAddress).call{value: devReflect}("");
        require(sent);
        splitBalance((reflectAmount/7)*3);
    }
...
//only owner
 ...
 function setPayments(address _payments) public onlyOwner {
  ...
  function splitReflections(uint256 amount) private onlyOwner{
      reflectDividend(amount);
  }
