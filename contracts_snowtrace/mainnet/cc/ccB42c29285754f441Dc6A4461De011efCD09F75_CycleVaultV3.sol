...
contract CycleVaultV3 is ERC20, ReentrancyGuard, Ownable, Pausable {
    ...
    constructor(
        ...
        IERC20(WAVAX).safeApprove(Router, uint256(-1));
        IERC20(LPtoken).safeApprove(Router, 0);
        IERC20(LPtoken).safeApprove(Router, uint256(-1));
    ...
    function balanceLPinSystem() public view returns (uint256) {
        return balanceLPinVault().add(balanceLPinStrategy());
    }
    ...
    function getAccountLP(address account) public view returns (uint256) {
        return totalSupply() == 0 ? 0 : balanceLPinSystem().mul(accountShareBalance(account)).div(totalSupply());
    }
    ...
    function getLPamountForShares(uint256 shares) public view returns (uint256) {
        return totalSupply() == 0 ? 1e18 : balanceLPinSystem().mul(shares).div(totalSupply());
    }
    ...
    function getAVAXamountForLPamount(uint256 amountLP) public view returns (uint256) {
        ...
        uint256 amountToken0 = reservesToken0.mul(amountLP).div(totalSupplyLP);
        uint256 amountToken1 = reservesToken1.mul(amountLP).div(totalSupplyLP);

        if (Token0 == WAVAX) {
            uint256 Token1toWAVAX = AMMLibrary.getAmountOut(amountToken1, reservesToken1, reservesToken0);
            return amountToken0.add(Token1toWAVAX);
        } else if (Token1 == WAVAX) {
            uint256 Token0toWAVAX = AMMLibrary.getAmountOut(amountToken0, reservesToken0, reservesToken1);
            return amountToken1.add(Token0toWAVAX);
        } else {
            ...
            return Token0toWAVAX.add(Token1toWAVAX);
        }
    }
    ...
    function depositAVAX() external payable nonReentrant whenNotPaused {
        ...
        uint256 halfAmountWAVAX = amountAVAX.div(2);

        if (Token0 == WAVAX) {
            IRouter(Router).swapExactTokensForTokens(halfAmountWAVAX, 0, WAVAXtoToken1path, address(this), block.timestamp.add(120));    
        } else if (Token1 == WAVAX) {
            IRouter(Router).swapExactTokensForTokens(halfAmountWAVAX, 0, WAVAXtoToken0path, address(this), block.timestamp.add(120));
        } else {
            IRouter(Router).swapExactTokensForTokens(halfAmountWAVAX, 0, WAVAXtoToken0path, address(this), block.timestamp.add(120));
            IRouter(Router).swapExactTokensForTokens(halfAmountWAVAX, 0, WAVAXtoToken1path, address(this), block.timestamp.add(120));
        }
        ...
        (,, uint256 amountLP) = IRouter(Router).addLiquidity(Token0, Token1, balanceToken0, balanceToken1, 0, 0, address(this), block.timestamp.add(120));
    ...
    function withdrawAVAX(uint256 sharesToWithdraw) external nonReentrant {
        ...
        IRouter(Router).removeLiquidity(Token0, Token1, amountLPforWithdraw, 0, 0, address(this), block.timestamp.add(120));

        uint256 balanceToken0 = IERC20(Token0).balanceOf(address(this));
        uint256 balanceToken1 = IERC20(Token1).balanceOf(address(this));

        if (Token0 == WAVAX) {
            IRouter(Router).swapExactTokensForTokens(balanceToken1, 0, Token1toWAVAXpath, address(this), block.timestamp.add(120));
        } else if (Token1 == WAVAX) {
            IRouter(Router).swapExactTokensForTokens(balanceToken0, 0, Token0toWAVAXpath, address(this), block.timestamp.add(120));
        } else {
            IRouter(Router).swapExactTokensForTokens(balanceToken0, 0, Token0toWAVAXpath, address(this), block.timestamp.add(120));
            IRouter(Router).swapExactTokensForTokens(balanceToken1, 0, Token1toWAVAXpath, address(this), block.timestamp.add(120));
        }
    ...
    function _withdraw(uint256 shares) internal returns (uint256 amountLPforWithdraw) {
        ...
        if (balanceLPinVaultBefore < amountLPforWithdraw) {
            uint256 amountLPToWithdrawFromStrategy = amountLPforWithdraw.sub(balanceLPinVaultBefore);
            ...
            uint256 difference = balanceLPinVaultAfter.sub(balanceLPinVaultBefore);
            if (difference < amountLPToWithdrawFromStrategy) {
                amountLPforWithdraw = balanceLPinVaultBefore.add(difference);
            }
        }
    }
