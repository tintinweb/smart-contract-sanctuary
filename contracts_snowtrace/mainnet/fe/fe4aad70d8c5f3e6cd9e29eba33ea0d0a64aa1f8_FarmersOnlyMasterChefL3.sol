...
contract FarmersOnlyMasterChefL3 is Ownable, ReentrancyGuard {
    ...
    // USDT.e Token.
    ERC20 private constant usdt = ERC20(0xc7198437980c041c805A1EDcbA50c1Ce5db95118);
    // USDC.e Token.
    ERC20 private constant usdc = ERC20(0xA7D7079b0FEaD91F3e65f86E8915Cb59c1a4C664);
    // CORN-USDC Pair for CORN price fetching.
    address private constant cornusdc = 0xc6A9dc8569ada7626b77E04445e93227d0790478;
    // TMT-USDT Pair for TMT price fetching.
    address private constant tmtusdt = 0x2b2793d3ED4640db784e6a438a5bc25b6FE7C169;
    ...
    constructor(
        ...
        onionusdt = address(0);
        kingTimer = _startTime + INITIAL_KING_ROT;
    ...
    function addPool(uint256 _allocPoint, IERC20 _lpToken, uint16 _depositFeeBP, uint16 _withdrawFeeBP, uint256 _harvestInterval, bool _kingRot, bool _isKing, bool _hybridHarvest, bool _withUpdate) external onlyOwner nonDuplicated(_lpToken){
        ...
        // Check for pools added with lpToken = address(0), for example native LP pools (that can be added only after token liquidity is added)
    ...
    // Fetch USD price of token multiplied by 1000 (if _quote has 6 decimals), needed for the King Rotation logic.
    function price(ERC20 _token, address _pair, ERC20 _quote) private view returns (uint256) {
        return ((_quote.balanceOf(_pair).mul(10**15)).div(_token.balanceOf(_pair)));
    }
    
    // Check if $ in onion vote pool is higher than $ in tmt + $ in corn vote pools.
    function onionSupremacy() private view returns (bool) {
        return onion.balanceOf(onionVote).mul(price(onion, onionusdt, usdt)) >
            (corn.balanceOf(cornVote).mul(price(corn, cornusdc, usdc)))
            .add(tmt.balanceOf(tmtVote).mul(price(tmt, tmtusdt, usdt)));
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (onionusdt != address(0) && isKingRotationActive)
        {
            rotateKing();
        }
    ...
    // Set the ONION-USDT LP address for ONION price fetching in the price() function (to use after ONION liquidity is provided).
    // Since this is likely better to be done right after providing liquidity for the token, we removed onlyOwner and set it to be done by the first owner address, so we can add the Timelock even before launching the token and still be able to execute this anytime.
    function setOnionUsdt(address _onionusdt) external {
        require(msg.sender == firstOwnerAddress, "setOnionUsdt: sender is not the owner");
        require(onionusdt == address(0), "setOnionUsdt: the pair is already set, updating it is forbidden");
        require(_onionusdt != address(0), "setOnionUsdt: setting the pair address to the zero address is forbidden");
        onionusdt = _onionusdt;
        massUpdatePools();
        emit SetOnionUsdt(msg.sender, _onionusdt);
    }
    ...
    // Since this is an experimental feature and we may want to disable it anytime (no harm to users if we do that), it will be doable without waiting for the timelock delay.
    function setHybridHarvest(uint256 _pid, bool _hybridHarvest) external {
        require(msg.sender == firstOwnerAddress, "setHybridHarvest: sender is not the owner");
    ...
    // Set the staking token address of pools that have been added with "lpToken = address(0)", such as native LPs (cause they can be added only after token liquidity is provided).
    // Since this is likely better to be done right after providing liquidity for the token, we removed onlyOwner and set it to be done by the first owner address, so we can add the Timelock even before launching the token and still be able to execute this anytime.
    // KEEP IN MIND that this can be executed only ONE TIME per pool, and only on pools where lpToken is not set
    function setPoolLp(uint256 _pid, IERC20 _lpToken) external {
        require(msg.sender == firstOwnerAddress, "setPoolLp: sender is not the owner");
        require(address(poolInfo[_pid].lpToken) == address(0), "setPoolLp: changing an already set LP is forbidden");
        _lpToken.balanceOf(address(this));
        poolExistence[_lpToken] = true;
        poolInfo[_pid].lpToken = _lpToken;
        emit SetPoolLp(msg.sender, _pid, _lpToken);
    }
    
    // Set the multiplier of the King pool.
    function setKingPoolMul(uint256 _allocPoint) external onlyOwner {
        require(isKingRotationActive, "setKingPoolMul: king rotation feature has been disabled");
