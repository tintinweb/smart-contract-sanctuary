...
import {IAvaxDexRouter as IPangolinRouter} from "../interfaces/dex/IAvaxDexRouter.sol";

/// @notice Pangolin Handler used to execute an order
contract PangolinHandler is IHandler {
     ...
     * @param _router - Address of the Pangolin router contract
     * @param _wavax - Address of WAVAX contract
     * @param _codeHash - Bytes32 of the Pangolin v2 pair contract unit code hash
    ...
    modifier onlyYolo() {
        require(
            msg.sender == yolo,
            "PangolinHandler: only yolo contract can invoke this function"
        );
        _;
    }
        ...
        returns (uint256)
    {
        ...
        IPangolinRouter _pangolinRouter = pangolinRouter;
        ...
        if (order.outputToken == WAVAX) {
            ...
            returns (uint256[] memory avaxAmount) {
                ...
                returnAmount = _pangolinRouter.swapExactTokensForAVAX(
                    order.inputAmount,
                    amountOutMin,
                    path,
                    order.recipient,
            ...
            returns (uint256[] memory tokenAmount) {
                ...
                returnAmount = _pangolinRouter.swapExactTokensForTokens(
                    order.inputAmount,
                    amountOutMin,
                    path,
                    order.recipient,
