...
contract Bank2 is Ownable, IERC721Receiver, Pauseable {
   ...
   */
    ...
    function setOldTokenInfo(uint256 _tokenId) external {
        require(msg.sender == swapper, "only swpr");
        (bool thief, , , , , , , , uint8 alphaIndex) = game.tokenTraits(_tokenId);
        (, uint80 value, address addr) = oldBank.bank(_tokenId);
        if (thief) {
            _addThiefToBankWithTime(addr, _tokenId, value);
        }
        else {
            uint256 packIndex = oldBank.packIndices(_tokenId);
            Bank.Stake memory s = oldBank.pack(8 - alphaIndex, packIndex);
            require(s.tokenId == _tokenId, "token id is not same");
            _addPoliceToPack(s.owner, _tokenId);
        }
    }
