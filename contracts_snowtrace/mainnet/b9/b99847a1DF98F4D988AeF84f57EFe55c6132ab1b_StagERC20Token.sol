...
contract StagERC20Token is ERC20Permit, VaultOwned {
    ...
    constructor() ERC20("StagDAO", "STG", 9) {
    ...
    function setDaoAddress(address _dao_address) public {
        require(msg.sender == tax_operator, "Stag::updateTaxOperator: Only Tax Operator can set tax collector.");
        dao_address = _dao_address;
    }

    function setExcludedFromTax(address _account, bool _excluded) public {
        require(msg.sender == tax_operator, "Stag::updateTaxOperator: Only Tax Operator can set tax free status.");
    ...
    function updateTaxOperator(address _taxOperator) public {
      require(msg.sender == tax_operator, "Stag::updateTaxOperator: Only Tax Operator can update next tax operator.");
      tax_operator = _taxOperator;
    }

    function updateTransferTaxRate(uint16 _transferTaxRate) public {
        require(_transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "Stag::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
        require(msg.sender == tax_operator, "Stag::updateTransferTaxRate: Only Tax Operator can update Tax rate.");
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
      if (transferTaxRate == 0 || dao_address == address(0) || isExcludedFromTax(sender)) {
        ...
        require(amount == sendAmount + taxAmount, "Stag::transfer: Tax value invalid");
