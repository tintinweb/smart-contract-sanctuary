...
interface IHunter {
  ...
  function getTokenTraits(uint256 tokenId) external view returns (AvtHtr memory);
}
...
contract Hunter is IHunter, ERC721Enumerable, Ownable, Pausable {
  ...
  // mint price 1.5AVAX
  uint256 public constant MINT_PRICE = .375 ether;
   ...
   */
  constructor(address _gem, address _traits, uint256 _maxTokens) ERC721("Yield Hunt", 'HGAME') {
    ...
    aliases[0] = [0];
    // jacket
    rarities[1] = [255, 170, 12, 64, 123, 140, 46, 63, 215, 26, 80, 4, 222, 194, 196, 4, 70, 121, 141, 37, 7, 63, 140,
       127, 122, 51, 122, 102, 249, 229];
    aliases[1] = [24, 16, 10, 18, 7, 30, 14, 10, 12, 21, 18, 17, 22, 9, 18, 16, 11, 27, 21, 20, 22, 11, 16, 30, 1, 3, 28, 4, 10, 4];
    // hair
    rarities[2] =  [198, 3, 65, 136, 119, 94, 211, 179, 216, 83, 173,71, 55, 116, 192, 194, 49, 211, 101, 252, 117, 185, 139, 176, 214, 156, 22, 243, 221, 191];
    aliases[2] = [28, 4, 22, 30, 23, 13, 21, 24, 5, 20, 7, 19, 21, 29, 2, 22, 3, 18, 5, 12, 24, 8, 0, 18, 24, 2, 21, 30, 2, 21];
    // backpack
    rarities[3] = [149, 186, 229, 228, 64, 182, 189, 103, 63, 180, 214, 140, 30, 74, 230, 17, 234, 46, 239, 61, 186, 69, 69, 73, 47, 184, 207, 120, 82, 174];
    aliases[3] = [23, 27, 0, 15, 2, 21, 26, 14, 7, 5, 26, 0, 17, 23, 7, 16, 13, 28, 23, 12, 30, 4, 1, 29, 3, 0, 29, 5, 17, 18];
    ...
    aliases[4] = [0];
    // clothes
    rarities[5] = [163, 87, 68, 38, 142, 108, 30, 206, 122, 167, 231, 109, 253, 190, 234, 134, 255, 59, 215, 240];
    aliases[5] = [4, 3, 1, 7, 14, 16, 17, 14, 8, 6, 3, 2, 15, 17, 5, 9, 13, 12, 19, 10];
    // mask
    rarities[6] = [213, 64, 82, 199, 59, 153, 131, 230, 62, 234, 173, 122, 177, 201, 134, 58, 138, 105, 193, 60];
    aliases[6] = [1, 11, 5, 3, 0, 10, 11, 17, 18, 12, 10, 4, 8, 2, 6, 18, 4, 18, 4, 7];
    // weapon
    rarities[7] = [94, 26, 89, 146, 206, 251, 89, 42, 216, 147, 185, 155, 64, 229, 160, 143, 215, 241, 56, 24];
    aliases[7] = [4, 12, 8, 3, 0, 16, 3, 0, 13, 11, 4, 17, 9, 7, 15, 13, 15, 3, 15, 20];
    // arm
    rarities[8] = [255];
    aliases[8] = [0];
    // alphaIndex
    rarities[9] = [8, 160, 73, 255];
    aliases[9] = [2, 3, 3, 3];
  }

  /** EXTERNAL */
  function setTimeforPremint(uint256 _startTimestamp, uint256 _endTimestamp) external onlyOwner {
  ...
  function mint_process(uint256 amount, bool stake) private {
    require(msg.sender == _msgSender(), "Only EOA");
    if (minted < PAID_TOKENS) {
      require(minted + amount <= PAID_TOKENS, "All tokens on-sale already sold");
      // require(amount * MINT_PRICE <= msg.value, "Invalid payment amount");
    } else {
      require(msg.value == 0, "Add this");
    }
    ...
    for (uint i = 0; i < amount; i++) {
      ...
      totalGemCost += mintCost(minted);
    }
    if (minted < PAID_TOKENS) {
      withdrawMoneyTo(Multisig);
    } else {
      withdrawMoneyTo(AdminWallet);
    }
   ...
   */
  function generate(uint256 tokenId, uint256 seed) internal returns (AvtHtr memory t) {
    t = selectTraits(seed);
   ...
   */
  function selectTraits(uint256 _seed) internal view returns (AvtHtr memory t) {
    uint256 seed = _seed;
    t.isAdventurer = (seed & 0xFFFF) % 4 != 0;
    uint8 shift = t.isAdventurer ? 0 : 4;
    if(t.isAdventurer) {
      seed >>= 16;
      t.avt_body = selectTrait(uint16(seed & 0xFFFF), 0 + shift);
      seed >>= 16;
      t.jacket = selectTrait(uint16(seed & 0xFFFF), 1 + shift);
      seed >>= 16;
      t.hair = selectTrait(uint16(seed & 0xFFFF), 2 + shift);
      seed >>= 16;
      t.backpack = selectTrait(uint16(seed & 0xFFFF), 3 + shift);
    } else {
      seed >>= 16;
      t.htr_body = selectTrait(uint16(seed & 0xFFFF), 0 + shift);
      seed >>= 16;
      t.mask = selectTrait(uint16(seed & 0xFFFF), 1 + shift);
      seed >>= 16;
      t.clothes = selectTrait(uint16(seed & 0xFFFF), 2 + shift);
      seed >>= 16;
      t.weapon = selectTrait(uint16(seed & 0xFFFF), 3 + shift);
      seed >>= 16;
      t.arm = selectTrait(uint16(seed & 0xFFFF), 4 + shift);
      seed >>= 16;
   ...
   */
  function structToHash(AvtHtr memory s) internal pure returns (uint256) {
  ...
  function getTokenTraits(uint256 tokenId) external view override returns (AvtHtr memory) {
...
contract Barn is Ownable, IERC721Receiver, Pausable {
  ...
  // there will only ever be (roughly) 1.8 billion $GEM earned through staking
  uint256 public constant MAXIMUM_GLOBAL_GEM = 1800000000 ether;
   ...
   */
  function addManyToBarnAndPack(address account, uint16[] memory tokenIds) external {
    require(account == _msgSender() || _msgSender() == address(hunter), "DONT GIVE YOUR TOKENS AWAY");
    for (uint i = 0; i < tokenIds.length; i++) {
      if (_msgSender() != address(hunter)) { // dont do this step if its a mint + stake
        require(hunter.ownerOf(tokenIds[i]) == _msgSender(), "AINT YO TOKEN");
      ...
      if (isAdventurer(tokenIds[i]))
        ...
        _addHunterToPack(account, tokenIds[i]);
    }
  }
   ...
   */
  function _addAdventurerToBarn(address account, uint256 tokenId) internal whenNotPaused _updateEarnings {
    barn[tokenId] = Stake({
      owner: account,
   ...
   */
  function claimManyFromBarnAndPack(uint16[] memory tokenIds, bool unstake) external whenNotPaused _updateEarnings {
    uint256 owed = 0;
    for (uint i = 0; i < tokenIds.length; i++) {
      if (isAdventurer(tokenIds[i]))
        owed += _claimAdventurerFromBarn(tokenIds[i], unstake);
      else
        owed += _claimHunterFromPack(tokenIds[i], unstake);
    }
    if (owed == 0) return;
    IERC20(gem).approve(address(this), owed);
    IERC20(gem).transferFrom(address(this), _msgSender(), owed);
  }
  ...
  function calculateRewards(uint256 tokenId) external view returns (uint256 owed) {
    ...
    if(hunter.getTokenTraits(tokenId).isAdventurer) {
      if(totalGemEarned < MAXIMUM_GLOBAL_GEM) {
        ...
        owed = (lastClaimTimestamp - stake.value) * DAILY_GEM_RATE / 1 days; // stop earning additional $GEM if it's all been earned
      }
    }
    else {
      uint256 alpha = _alphaForHunter(tokenId);
   ...
   */ 
  function _claimAdventurerFromBarn(uint256 tokenId, bool unstake) internal returns (uint256 owed) {
    Stake memory stake = barn[tokenId];
    require(stake.owner == _msgSender(), "SWIPER, NO SWIPING");
    require(!(unstake && block.timestamp - stake.value < MINIMUM_TO_EXIT), "GONNA BE COLD WITHOUT TWO DAY'S GEM");
    if (totalGemEarned < MAXIMUM_GLOBAL_GEM) {
      owed = (block.timestamp - stake.value) * DAILY_GEM_RATE / 1 days;
    } else if (stake.value > lastClaimTimestamp) {
      owed = 0; // $GEM production stopped already
    ...
    if (unstake) {
      if (random(tokenId) & 1 == 1) { // 50% chance of all $GEM stolen
        _payHunterTax(owed);
        owed = 0;
      }
   ...
   * @return owed - the amount of $GEM earned
   */
  function _claimHunterFromPack(uint256 tokenId, bool unstake) internal returns (uint256 owed) {
    require(hunter.ownerOf(tokenId) == address(this), "AINT A PART OF THE PACK");
    uint256 alpha = _alphaForHunter(tokenId);
   ...
   * @param tokenIds the IDs of the tokens to claim earnings from
   */
  function rescue(uint256[] memory tokenIds) external {
  ...
  function _payHunterTax(uint256 amount) internal {
    if (totalAlphaStaked == 0) { // if there's no staked hunters
  ...
  modifier _updateEarnings() {
    if (totalGemEarned < MAXIMUM_GLOBAL_GEM) {
      totalGemEarned += (block.timestamp - lastClaimTimestamp) * totalAdventurerStaked * DAILY_GEM_RATE / 1 days;
   ...
   */
  function isAdventurer(uint256 tokenId) public view returns (bool adventurer) {
