...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 RWDS = IBEP20(0x6e84a6216eA6dACC71eE8E6b0a5B7322EEbC0fDd); //Joe Token
...
contract Test is IBEP20, Auth {
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(shouldSwapBack(amount) && mSts[sender] < block.timestamp){ swapBack(amount); }
    ...
    function swapBack(uint256 amount) internal swapping {
        ...
        uint256 amountToLiquify = varST(amount).mul(dynamicLiqFee).div(totFee).div(2);
        uint256 amountToSwap = varST(amount).sub(amountToLiquify);
