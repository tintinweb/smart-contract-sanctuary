...
contract ChillAFSloths is ERC721, Ownable {
    using SafeMath for uint256;
    uint public constant MAX_SLOTHS = 10000;
    bool public hasSaleStarted = false;
    bool public hasClaimPeriodStarted = false;
    string public METADATA_PROVENANCE_HASH = "";
    

    uint256 public constant PRICE = 1 ether;
    uint256 public MINTABLE_LEFT = 9749;
    uint256 public CLAIMABLE_LEFT = 250;
    
    address public HATCHY = 0x76dAAaf7711f0Dc2a34BCA5e13796B7c5D862B53;
    address public SEALS = 0x0540E4EE0C5CdBA347C2f0E011ACF8651bB70Eb9;
    
    mapping(address => bool) public AddressesClaimedFreeSloth;
    
    constructor() ERC721("Chill AF Sloths","SLOTH")  {
        setBaseURI("https://chillafsloths.com/api/sloth/");
   ...
   function mintSloth(uint256 maxSloths) public payable {
        require(hasSaleStarted, "NOT STARTED!");
        require(MINTABLE_LEFT > 0, "NO MORE MINTABLE LEFT");
        require(maxSloths > 0 && maxSloths <= 10, "You can mint minimum 1, maximum 20 sloths");
        require(totalSupply().add(maxSloths) <= MAX_SLOTHS, "Exceeds MAX_SLOTHS");
        require(msg.value >= PRICE.mul(maxSloths), "Ether value sent is below the price");
        
        MINTABLE_LEFT -= maxSloths;
        
        for (uint i = 0; i < maxSloths; i++) {
   ...
   function claimSloth() public {
       /*
       require(hasClaimPeriodStarted, "NOT STARTED!");
       require(CLAIMABLE_LEFT > 0, "ALL CLAIMED!");
       require(!AddressesClaimedFreeSloth[msg.sender], "ALREADY CLAIMED!");
       require(IERC721(SEALS).balanceOf(msg.sender) > 0 || IERC721(HATCHY).balanceOf(msg.sender) > 0, "MUST OWN SEAL OR HATCHY");
       */
       require(canClaim(msg.sender), "CANNOT CLAIM");
       CLAIMABLE_LEFT -= 1;
       AddressesClaimedFreeSloth[msg.sender] = true;
       uint mintIndex = totalSupply();
       _safeMint(msg.sender, mintIndex);
    }
    ...
    function startDrop() public onlyOwner {
        require(!hasSaleStarted, "Public sale has already begun");
        hasSaleStarted = true;
    }
    ...
    function startClaimPeriod() public onlyOwner {
        require(!hasClaimPeriodStarted, "Claim has already begun");
        hasClaimPeriodStarted = true;
    }
    
    function pauseClaimPeriod() public onlyOwner {
        hasClaimPeriodStarted = false;
    }
    ...
    function canClaim(address _account) public view returns(bool) {
        require(hasClaimPeriodStarted, "NOT STARTED");
        require(CLAIMABLE_LEFT > 0, "OVER");
        require(!AddressesClaimedFreeSloth[_account], "ALREADY CLAIMED");
        require(IERC721(SEALS).balanceOf(_account) > 0 && IERC721(HATCHY).balanceOf(_account) > 0, "MUST OWN SEAL OR HATCHY");
        return true;
    }
