...
contract SimpleSplitter is Ownable {
    ...
    IERC20 public immutable avaone; // The AvaOne token
    IERC20 public immutable dummyDepositToken;
    IMasterAvao public immutable masterAvao;
   ...
   constructor(
        IERC20 _avaone,
        IERC20 _dummyDepositToken,
        IMasterAvao _masterAvao,
        uint256 _poolID,
        address _okinaPrime,
        address _satoku,
        address _hashercat,
        ...
        avaone = _avaone;
        dummyDepositToken = _dummyDepositToken;
        masterAvao = _masterAvao;
        poolID = _poolID;
    ...
    function splitBalanceBetweenAddress() external {
        // Get reward from underlying dummyPool
        masterAvao.withdraw(poolID, 0);
    ...
    function depositDummyToken() external onlyOwner {
        require(dummyDepositToken.balanceOf(address(this)) > 0, "DEPOSIT: Cannot deposit balance of 0");
        dummyDepositToken.approve(address(masterAvao), type(uint256).max);
        masterAvao.deposit(poolID, dummyDepositToken.balanceOf(address(this)));
    }
}