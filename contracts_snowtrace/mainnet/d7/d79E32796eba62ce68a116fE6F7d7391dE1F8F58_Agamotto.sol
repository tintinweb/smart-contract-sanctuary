...
contract Agamotto is ERC20, Ownable {
    using SafeMath for uint256;

    NODERewardManagement[3] public nodeRewardManager;

    IJoeRouter02 public uniswapV2Router;

    address public uniswapV2Pair;
    address public futurUsePool = 0x74E78044Cfb4c9beE6650bA929724260fD6aB69B;
    address public distributionPool = 0xd488180a5a9093d701b9FeA9F79a426e4066E986;
    address public marketingWallet = 0xd1eD81B74D740755F248Ff8d1589bE139B1B2213;
    ...
    function setNodeManagement(address nodeManagement, uint256 id) external onlyOwner {
        nodeRewardManager[id] = NODERewardManagement(nodeManagement);
    }
    ...
    function createNodeWithTokens(string memory name, uint256 id) public {
        ...
        require(_nodeNumber[sender] < maxNodeNumber, "cannot create node more than 100");
        require(
            sender != address(0),
...
//            swapTokensForEth(balanceOf(address(this)));
            swapAndSendToFee(marketingWallet, balanceOf(address(this)));
    ...
    function cashoutReward(uint256 blocktime) public {
        ...
        require(
            sender != futurUsePool && sender != distributionPool,
            "CSHT: future and rewardsPool cannot cashout rewards"
        );

        uint256 rewardAmount = 0;
        for(uint256 id = 0; id < 3; id ++) {
            rewardAmount = rewardAmount.add(nodeRewardManager[id]._getRewardAmountOf(sender, blocktime));
        }
    ...
    function cashoutAll() public {
        ...
        for(uint256 id = 0; id < 3; id ++) {
            ...
            rewardAmount = rewardAmount.add(nodeRewardManager[id]._getRewardAmountOf(sender));
        }
        ...
        for(uint256 id = 0; id < 3; id ++) {
            if(getNodeNumberOf(sender, id) == 0) continue;
            nodeRewardManager[id]._cashoutAllNodesReward(sender);
        }
    }
    ...
    function changeRewardPerNode(uint256 newPrice, uint256 id) public onlyOwner {
        nodeRewardManager[id]._changeRewardPerNode(newPrice);
    }

    function getRewardPerNode(uint256 id) public view returns (uint256) {
        return nodeRewardManager[id].rewardPerNode();
    }

    function changeClaimTime(uint256 newTime, uint256 id) public onlyOwner {
        nodeRewardManager[id]._changeClaimTime(newTime);
    }

    function getClaimTime(uint256 id) public view returns (uint256) {
        return nodeRewardManager[id].claimTime();
    }

    function changeAutoDistri(bool newMode, uint256 id) public onlyOwner {
        nodeRewardManager[id]._changeAutoDistri(newMode);
    }

    function getAutoDistri(uint256 id) public view returns (bool) {
        return nodeRewardManager[id].autoDistri();
    }

    function changeGasDistri(uint256 newGasDistri, uint256 id) public onlyOwner {
        nodeRewardManager[id]._changeGasDistri(newGasDistri);
    }

    function getGasDistri(uint256 id) public view returns (uint256) {
        return nodeRewardManager[id].gasForDistribution();
    }

    function getDistriCount(uint256 id) public view returns (uint256) {
        return nodeRewardManager[id].lastDistributionCount();
    }

    function getNodesNames(uint256 id) public view returns (string memory) {
        require(_msgSender() != address(0), "SENDER CAN'T BE ZERO");
        require(
            nodeRewardManager[id]._isNodeOwner(_msgSender()),
            "NO NODE OWNER"
        );
        return nodeRewardManager[id]._getNodesNames(_msgSender());
    }

    function getNodesCreatime(uint256 id) public view returns (string memory) {
        require(_msgSender() != address(0), "SENDER CAN'T BE ZERO");
        require(
            nodeRewardManager[id]._isNodeOwner(_msgSender()),
            "NO NODE OWNER"
        );
        return nodeRewardManager[id]._getNodesCreationTime(_msgSender());
    }

    function getNodesRewards(uint256 id) public view returns (string memory) {
        require(_msgSender() != address(0), "SENDER CAN'T BE ZERO");
        require(
            nodeRewardManager[id]._isNodeOwner(_msgSender()),
            "NO NODE OWNER"
        );
        return nodeRewardManager[id]._getNodesRewardAvailable(_msgSender());
    }

    function getNodesLastClaims(uint256 id) public view returns (string memory) {
        require(_msgSender() != address(0), "SENDER CAN'T BE ZERO");
        require(
            nodeRewardManager[id]._isNodeOwner(_msgSender()),
            "NO NODE OWNER"
        );
        return nodeRewardManager[id]._getNodesLastClaimTime(_msgSender());
    }

    function distributeRewards(uint256 id) public onlyOwner returns (uint256, uint256, uint256) {
        return nodeRewardManager[id]._distributeRewards();
    }

    function publiDistriRewards() public {
        for(uint256 id = 0; id < 3; id ++) {
            nodeRewardManager[id]._distributeRewards();
        }
    }
    ...
    function getTotalStakedReward() public view returns (uint256) {
        uint256 res = 0;
        for(uint256 id = 0; id < 3; id ++) {
            res = res.add(nodeRewardManager[id].totalRewardStaked());
        }
        return res;
    }
