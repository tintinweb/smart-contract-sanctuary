...
contract Implementation is State, Bonding, Market, Regulator, Govern {
    ...
    function initializeAddresses(address _dollar, address _oracle, address _pool) public {
        require(address(_state.provider.dollar) == address(0), "!dollar");
        require(address(_state.provider.oracle) == address(0), "!oracle");
        require(address(_state.provider.pool) == address(0), "!pool");

        _state.provider.dollar = IDollar(_dollar);
        _state.provider.oracle = IOracle(address(_oracle));
        _state.provider.pool = _pool;

        // Note: we're incentivizing here as the addresses are only saved in this function
        // For the next implementation, the incentivize should be in the initialize address
        // as the state has been saved. We're separating out contract deployments as
        // we kept hitting contract size too big

        // Market maker
        incentivize(Constants.getMarketMaker(), 5_000_000e18);
    }

    function initialize() initializer public {
        // Fixes the epoch period
        uint256 epochPeriod = Constants.getEpochPeriod();
        _state.epoch.current = 0;
        // Sun, 05 Dec 2021 00:00:00 GMT
        _state.epoch.start = 1638662400; // (block.timestamp / epochPeriod + 1) * epochPeriod; // Round to the next UTC-0
        _state.epoch.period = epochPeriod;
    }

    function advance() external {
        // only allows humans to call advance
        // don't want any flashloan attacks to manipulate
        // curve price pool
...
contract Market is Comptroller, Curve {
    ...
    event CouponRedemption(address indexed account, uint256 couponAmount);
    ...
    function purchaseCoupons(uint256 dollarAmount) external returns (uint256) {
        ...
        incrementBalanceOfCoupons(epoch, msg.sender, couponAmount);
    ...
    function redeemCoupons(uint256 couponAmount) external {
        uint256 lastPurchasedEpoch = userLastPurchasedCouponEpoch(msg.sender);
        require(epoch().sub(lastPurchasedEpoch) >= 2, "Market: Too early to redeem");
        decrementBalanceOfCoupons(msg.sender, couponAmount, "Market: Insufficient coupon balance");
        redeemToAccount(msg.sender, couponAmount);

        emit CouponRedemption(msg.sender, couponAmount);
    }
}
...
contract Setters is State, Getters {
    ...
    function incrementBalanceOfCoupons(uint256 epoch, address account, uint256 amount) internal {
        _state.accounts[account].coupons = _state.accounts[account].coupons.add(amount);
        _state.balance.coupons = _state.balance.coupons.add(amount);

        // User last bought COUPONS at <X> epoch
        _state.accounts[account].lastCouponPurchasedEpoch = epoch;
    }

    function decrementBalanceOfCoupons(address account, uint256 amount, string memory reason) internal {
        _state.accounts[account].coupons = _state.accounts[account].coupons.sub(amount, reason);
...
contract Account {
    ...
    struct State {
        ...
        uint256 balance;
        uint256 coupons;
        uint256 lastCouponPurchasedEpoch;
        uint256 fluidUntil;
        uint256 lockedUntil;
    }
}
...
contract Getters is State {
    ...
    function curvePool() public view returns (address) {
        return address(_state.provider.oracle.curvePool());
    }
    ...
    function balanceOfCoupons(address account) public view returns (uint256) {
        if (outstandingCoupons() == 0) {
            return 0;
        }
        return _state.accounts[account].coupons;
    }
    ...
    function outstandingCoupons() public view returns (uint256) {
        return _state.balance.coupons;
    }

    function userLastPurchasedCouponEpoch(address user) public view returns (uint256) {
        return _state.accounts[user].lastCouponPurchasedEpoch;
    }
