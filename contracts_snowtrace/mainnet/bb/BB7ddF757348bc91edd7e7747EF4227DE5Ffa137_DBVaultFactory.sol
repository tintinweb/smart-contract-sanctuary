...
contract DBVaultFactory is UpgradeableBeacon, IBondingHQ {
    ...
    function getDepositoryPath(address _depository)
        external
        view
        override
        returns (address[] memory)
    {
        return depositoryInfo[_depository].path;
    }
    ...
    {
        usingWrapped = depositoryInfo[_depository].usingWrapped;
        isLpToken = depositoryInfo[_depository].isLpToken;
        active = depositoryInfo[_depository].active;
        addresses[0] = depositoryInfo[_depository].principle;
        addresses[1] = depositoryInfo[_depository].router;
        addresses[2] = depositoryInfo[_depository].tokenA;
        addresses[3] = depositoryInfo[_depository].tokenB;
        for (uint256 i = 0; i < depositoryInfo[_depository].path.length; i++) {
            addresses[4 + i] = depositoryInfo[_depository].path[i];
        }
    }
...
contract DBPersonalVault is Initializable, Ownable, IPersonalVault {
    ...
    function bond(
        ...
        address[] memory path = bondingHQ.getDepositoryPath(_depository);
        require(
            info[0] != address(0) && active,
    ...
    function redeem(address _depository) external override {
        (, , , address[] memory info) = bondingHQ.getDepositoryInfo(
            _depository
        );
        require(info[0] != address(0));
