...
interface IDexRouter {
    ...
    function removeLiquidityAVAXWithPermit(
        ...
        uint deadline,
        bool approveMax, uint8 v, bytes32 r, bytes32 s
    ) external returns (uint amountToken, uint amounAVAX);
...
contract SnowGoose is IERC20, Auth {

	address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;

	string constant _name = "Snow Goose";
	...
	event AutoLiquify(uint256 amountAVAX, uint256 autoBuybackAmount);


	constructor() Auth(msg.sender) {
		router = IDexRouter(0x60aE616a2155Ee3d9A68541Ba4544862310933d4);
        avaxPair = IDexFactory(router.factory()).createPair(router.WAVAX(), address(this));
	...
	function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!launched() && recipient == avaxPair) {
	...
	function checkTxLimit(address sender, address recipient, uint256 amount) internal {
        require(amount <= _maxTxAmountBuy || isTxLimitExempt[sender] || isTxLimitExempt[recipient] && sender == avaxPair, "TX Limit Exceeded");
		// Max wallet check.
		if (sender != owner
            && recipient != owner
