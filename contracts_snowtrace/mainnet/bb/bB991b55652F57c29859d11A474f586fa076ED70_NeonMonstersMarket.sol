...
contract NeonMonstersMarket is Pausable, Ownable, ReentrancyGuard {
    event Sale(
    ...
    NeonMonstersInterface private _masterContract;
    ...
    constructor(
        ...
        _masterContract = NeonMonstersInterface(masterContractAddress);
    ...
    modifier isTokenOwner(uint256 tokenId) {
        ...
        require(
            caller == _msgSender(),
            "NeonMonstersMarket: caller is not the owner of the token"
        );
        _;
    }
    ...
    function createPrivateListing(
        ...
        require(
            ...
            "NeonMonstersMarket: market must be approved to transfer token"
        );
        ...
        isTokenOwner(tokenId)
    {
        ...
        require(trade.exists == true, "NeonMonstersMarket: token not for sale");
    ...
    function buy(uint256 tokenId) external payable whenNotPaused nonReentrant {
        ...
        require(
            ...
            "NeonMonstersMarket: must send correct amount to buy"
        );
        require(
            _masterContract.ownerOf(tokenId) == trade.seller,
            "NeonMonstersMarket: seller must equal current token owner"
        );
        if (trade.buyer != address(0)) {
            require(
                trade.buyer == _msgSender(),
                "NeonMonstersMarket: listing is not available to the caller"
            );
        }
...
abstract contract NeonMonstersInterface {
