...
import "../interfaces/ITimeStake.sol";
import "../interfaces/ITraderJoe.sol";


contract StrategyWonderland is IStrategyAlpaca, ReentrancyGuard, Ownable, CoreRef {
    ...
    // temporary method
    function setTimeMaxPrice(uint256 _maxPrice) public {
        maxPrice = _maxPrice;
    }

    function stakeTime() public nonReentrant whenNotPaused {
        ITimeBondDepository(timeBondDepositoryAddress).redeem(address(this), true);
    }
    ...
    function _deposit(uint256 _wantAmt) internal {
        ...
        ITimeBondDepository(timeBondDepositoryAddress).deposit(_wantAmt, maxPrice, address(this));
        // ITimeBondDepository(timeBondDepositoryAddress).deposit(_wantAmt, IOracle(oracle).getLatestPrice(timeAddress), address(this));
    }
    ...
    function withdraw() public override onlyMultistrategy nonReentrant {
        ...
        ITimeStaking(timeStakingAddress).unstake(withdrawAmt, true); // set true to trigger rebase to update memo balance but user can only withdraw updated amount after the call

        IERC20(timeAddress).safeApprove(traderJoeRouterAddress, withdrawAmt);
        // uint256 minReturnWant = _calculateMinReturn(withdrawAmt);
        ITraderJoeRouter(traderJoeRouterAddress).swapExactTokensForTokens(
            withdrawAmt,
            1, // minReturnWant
            earnedToWantPath,
            address(this),
...
interface IJCollateralCapErc20Delegate {
