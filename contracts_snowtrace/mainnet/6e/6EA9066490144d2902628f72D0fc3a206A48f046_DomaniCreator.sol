...
contract Domani is ERC20 {
  ...
  /*
        The PositionState is the status of the Position, whether it is Default (held on the Domani token)
  ...
  /**
   * Throws if the sender is not the Domani token's manager
   */
  ...
  /**
   * Throws if Domani is locked and called by any account other than the locker.
   */
  ...
  // List of initialized Modules; Modules extend the functionality of Domani tokens
  ...
  /**
   * When a new Domani token is created, initializes Positions in default state and adds modules into pending state.
   * All parameter validations are on the DomaniCreator contract. Validations are performed already on the
   * DomaniCreator. Initiates the positionMultiplier as 1e18 (no adjustments).
   *
   * @param _components             List of addresses of components for initial Positions
   * @param _units                  List of units. Each unit is the # of components per 10^18 of a Domani token
   * @param _modules                List of modules to enable. All modules must be approved by the Controller
   * @param _controller             Address of the controller
   * @param _manager                Address of the manager
   * @param _name                   Name of the Domani
   * @param _symbol                 Symbol of the Domani
   */
  constructor(
  ...
  /**
   * PRIVELEGED MODULE FUNCTION. When a Domani token is locked, only the locker can call privileged functions.
   */
  ...
  /**
   * PRIVELEGED MODULE FUNCTION. Unlocks the Domani token and clears the locker
   */
  ...
  /**
   * MANAGER ONLY. Removes a module from the Domani token. Domani calls removeModule on module itself to confirm
  ...
  /**
   * MANAGER ONLY. Removes a pending module from the Domani token.
   */
  ...
  /**
   * MANAGER ONLY. Changes manager; We allow null addresses in case the manager wishes to wind down the Domani token.
   ...
   *
   * Module must be initialized on the Domani token and enabled by the controller
   */
 ...
 * @author Domani Protocol
 *
 * DomaniCreator is a smart contract used to deploy new Domani contracts. The DomaniCreator
 * is a Factory contract that is enabled by the controller to create and register new Domani tokens.
 */
contract DomaniCreator {
  using AddressArrayUtils for address[];

  /* ============ Events ============ */

  event DomaniCreated(address indexed _domani, address _manager, string _name, string _symbol);
  ...
  /**
   * Creates a Domani smart contract and registers the Domani with the controller. The Domani tokens are composed
   ...
   * @param _modules                List of modules to enable. All modules must be approved by the Controller
   * @param _manager                Address of the manager
   * @param _name                   Name of the Domani token
   * @param _symbol                 Symbol of the Domani token
   * @return address                Address of the newly created Domani token
   */
  function create(
    address[] memory _components,
    int256[] memory _units,
    ...
    // Creates a new Domani instance
    Domani domani = new Domani(_components, _units, _modules, controller, _manager, _name, _symbol);

    // Registers Domani with controller
    controller.addDomani(address(domani));

    emit DomaniCreated(address(domani), _manager, _name, _symbol);

    return address(domani);
  }
}
