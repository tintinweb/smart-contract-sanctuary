...
// File: contracts/farms/olive/common/IERC20Metadata.sol
...
// File: contracts/farms/olive/OlivePool.sol
...
contract OlivePool is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
...
//        uint256 _poolLimitPerUser,
    ...
    /* Olive Admin functions to setup pool */
    ...
    function oliveStartIn(uint256 _blocks) public onlyManager {
        oliveSetStart(block.number.add(_blocks));
    }

    function oliveSetStart(uint256 _startBlock) public onlyManager {
        require(block.number < startBlock, "Pool has started");
        require(block.number < _startBlock, "Start block must be in future");
    ...
    function oliveSetDuration(uint256 _durationBlocks) public onlyManager {
        require(block.number < startBlock, "Pool has started");
        bonusEndBlock = startBlock.add(_durationBlocks);
        oliveCalcRewardPerBlock();
    }

    function oliveSetStartAndDuration(uint256 _startBlock, uint256 _durationBlocks) public onlyManager {
        oliveSetStart(_startBlock);
        oliveSetDuration(_durationBlocks);
    }

    function oliveCalcRewardPerBlock() public onlyManager {
        require(block.number < startBlock, "Pool has started");
        uint256 rewardBal = rewardToken.balanceOf(address(this));
        rewardPerBlock = rewardBal.div(rewardDuration());
    }

    function oliveSetRewardMaxTxAmount(uint256 _maxTxAmount) public onlyManager {
        // 0 = no limit
