...
contract AvaxMetaverse is Context, IERC20, Ownable {
    ...
    mapping (address => bool) private _isExcludedFromFeesAVM;
    mapping (address => bool) private _isExcludedFromMaxBalanceAVM;

    uint256 private constant _maxFeesAVM = 10;
    uint256 private _totalFeesAVM;
    uint256 private _totalFeesAVMContract;
    uint256 private _burnFeesAVM;
    uint256 private _liquidityFeesAVM;
    uint256 private _marketingFeeAVM;
    
    uint256 private _maxBalanceWalletAVM;
    ...
    constructor () {
       IJoeRouter02 _joeRouter = IJoeRouter02(0x60aE616a2155Ee3d9A68541Ba4544862310933d4);
        ...
        _isExcludedFromFeesAVM[owner()] = true;
        _isExcludedFromFeesAVM[address(this)] = true;
        
        _isExcludedFromMaxBalanceAVM[owner()] = true;
        _isExcludedFromMaxBalanceAVM[address(this)] = true;
        _isExcludedFromMaxBalanceAVM[joePair] = true;

        _liquidityFeesAVM = 4;
        _marketingFeeAVM = 2;
        _burnFeesAVM = 1;
        _totalFeesAVM = _liquidityFeesAVM.add(_marketingFeeAVM).add(_burnFeesAVM);
        _totalFeesAVMContract = _liquidityFeesAVM.add(_marketingFeeAVM);

        _liquidityThreshhold = 5000 * 10**9 * 10**_decimals;
        _maxBalanceWalletAVM = 1000000 * 10**9 * 10**_decimals;
    ...
    function setLiquidityFeePercent(uint256 newLiquidityFee) external onlyOwner() {
        ...
        require(newLiquidityFee.add(_burnFeesAVM).add(_marketingFeeAVM) <= _maxFeesAVM, "Fees are too high.");
        _liquidityFeesAVM = newLiquidityFee;
        _totalFeesAVM = _liquidityFeesAVM.add(_marketingFeeAVM).add(_burnFeesAVM);
        _totalFeesAVMContract = _liquidityFeesAVM.add(_marketingFeeAVM);
        emit SetLiquidityFee(_liquidityFeesAVM);
    }

    function setMarketingFeePercent(uint256 newMarketingFee) external onlyOwner() {
        require(!inSwapAndLiquify, "inSwapAndLiquify");
        require(_liquidityFeesAVM.add(_burnFeesAVM).add(newMarketingFee) <= _maxFeesAVM, "Fees are too high.");
        _marketingFeeAVM = newMarketingFee;
        _totalFeesAVM = _liquidityFeesAVM.add(_marketingFeeAVM).add(_burnFeesAVM);
        _totalFeesAVMContract = _liquidityFeesAVM.add(_marketingFeeAVM);
        emit SetMarketingFee(_marketingFeeAVM);
    }

    function setBurnFeePercent(uint256 newBurnFee) external onlyOwner() {
        require(_liquidityFeesAVM.add(newBurnFee).add(_marketingFeeAVM) <= _maxFeesAVM, "Fees are too high.");
        _burnFeesAVM = newBurnFee;
        _totalFeesAVM = _liquidityFeesAVM.add(_marketingFeeAVM).add(_burnFeesAVM);
        emit SetBurnFee(_burnFeesAVM);
    }
    ...
    function _transfer(
        ...
        // Make sure that: Balance + Buy Amount <= _maxBalanceWalletAVM
        if(
            from != owner() &&              // Not from Owner
            to != owner() &&                // Not to Owner
            !_isExcludedFromMaxBalanceAVM[to]  // is excludedFromMaxBalance
        ){
            require(
                balanceOf(to).add(amount) <= _maxBalanceWalletAVM,
        ...
        if(
            ...
            _totalFeesAVMContract > 0 &&                         // LiquidityFee + MarketingFee > 0
            from != owner() &&                                  // Not from Owner
            to != owner()                                       // Not to Owner
        ) {
            collectFees();
        ...
        // Take Fees 
        if(
            !(_isExcludedFromFeesAVM[from] || _isExcludedFromFeesAVM[to])
            && _totalFeesAVM > 0
        ) {
            
        	uint256 feesToContract = amount.mul(_totalFeesAVMContract).div(100);
            uint256 toBurnAmount = amount.mul(_burnFeesAVM).div(100);
    ...
    function collectFees() private lockTheSwap {
        ...
        uint256 liquidityTokensToSell = balanceOf(address(this)).mul(_liquidityFeesAVM).div(_totalFeesAVMContract);
        uint256 marketingTokensToSell = balanceOf(address(this)).mul(_marketingFeeAVM).div(_totalFeesAVMContract);
