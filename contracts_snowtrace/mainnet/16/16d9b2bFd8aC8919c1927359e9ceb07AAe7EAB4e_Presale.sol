...
contract Presale is Ownable {
    ...
    uint256 public purchaseMIMAmount; // This is 200 MIM
    uint256 public allocatedaPRXPerUser; //  This is hardcoded to be 1 aPRX

    mapping(address => bool) public boughtPRX;
    ...
    function initialize(
        ...
        allocatedaPRXPerUser = _allocatedaPRXPerUser;
    ...
    function purchaseaPRXWithwsOHM() external returns (bool) {
        require(saleStarted() == true, "Not started");
        require(whiteListed[msg.sender] == true, "Not whitelisted");
        require(boughtPRX[msg.sender] == false, "Already participated");
        require(block.timestamp < endOfSale, "Sale over");

        boughtPRX[msg.sender] = true;

        IERC20(wsOHM).safeTransferFrom(msg.sender, addressToSendwsOHM, purchasewsOHMAmount);
        IERC20(aPRX).safeTransfer(msg.sender, allocatedaPRXPerUser);

        return true;
    }

    function purchaseaPRXWithMIM() external returns (bool) {
        require(saleStarted() == true, "Not started");
        require(whiteListed[msg.sender] == true, "Not whitelisted");
        require(boughtPRX[msg.sender] == false, "Already participated");
        require(block.timestamp < endOfSale, "Sale over");
        ...
        IERC20(MIM).safeTransferFrom(msg.sender, addressToSendwsOHM, purchaseMIMAmount);
        IERC20(aPRX).safeTransfer(msg.sender, allocatedaPRXPerUser);
    ...
    /**
     *  @notice Burn the remaining aPRX 
     *  @return true if it works
    */
    function burnRemainingaPRX()
        external
        onlyOwner()
        returns (bool)
    {
        ...
        IERC20Burnable(aPRX).burn(IERC20(aPRX).balanceOf(address(this)));
