...
contract StakingRewards is ReentrancyGuard, Ownable {
    ...
    function stakeWithPermit(uint256 amount, uint deadline, uint8 v, bytes32 r, bytes32 s) external nonReentrant updateReward(msg.sender) {
        ...
        ICanaryERC20(address(stakingToken)).permit(msg.sender, address(this), amount, deadline, v, r, s);
...
// File contracts/LiquidityPoolManagerV4.sol




pragma solidity ^0.8.9;
contract LiquidityPoolManagerV4 is Ownable, ReentrancyGuard {
    using EnumerableSet for EnumerableSet.AddressSet;
    using SafeMath for uint;
    using SafeERC20 for IERC20;

    EnumerableSet.AddressSet private pools;
    mapping(address => uint) public rewardWeight;
    ...
    uint public unallocatedCnr = 0;

    constructor(address cnr_,address treasuryVester_,address charger_) {
    ...
    function addWhitelistedPool(address stakeContract, uint _rewardWeight) public onlyOwner {
        ...
        require(_rewardWeight > 0, 'LiquidityPoolManager::addWhitelistedPool: rewardWeight cannot be zero');

        pools.add(stakeContract);
        rewardWeight[stakeContract] = _rewardWeight;
    ...
    function removeWhitelistedPool(address stakeContract) public onlyOwner {
        ...
        rewardWeight[stakeContract] = 0;

        numPools = numPools.sub(1);
    }

    function updateWeight(address stakeContract, uint weight) public onlyOwner {
        require(isWhitelisted(stakeContract), 'LiquidityPoolManager::updateWeight: Pool not whitelisted');
        require(weight > 0, 'LiquidityPoolManager::updateWeight: Amount must be bigger than 0');

        rewardWeight[stakeContract] = weight;
    }
    ...
    function distributeTokens() public  {
        ...
        for (uint i = 0; i < pools.length(); i++) {
            totalWeight = totalWeight.add(rewardWeight[pools.at(i)]);
        }

        mulValue = unallocatedCnr.div(totalWeight);

        for (uint i = 0; i < pools.length(); i++) {
            stakeContract = pools.at(i);
           
            rewardTokens = rewardWeight[stakeContract].mul(mulValue);
            if (rewardTokens > 0) {
    ...
    function distributeTokensSinglePool(uint pairIndex) public {
        ...
        uint rewardTokens = rewardWeight[stakeContract].mul(mulValue);
        if (rewardTokens > 0) {
            require(ICNR(cnr).transfer(stakeContract, rewardTokens), 'LiquidityPoolManager::distributeTokens: Transfer failed');
            StakingRewards(stakeContract).notifyRewardAmount(rewardTokens);
        }
    ...
    function vestAllocation() public  {
        require(unallocatedCnr == 0, 'LiquidityPoolManager::vestAllocation: Old CNR is unallocated. Call distributeTokens().');
        require(msg.sender == charger, "LiquidityPoolManager::vestAllocation: charger can call this.");
        unallocatedCnr = ITreasuryVester(treasuryVester).claim();
        require(unallocatedCnr > 0, 'LiquidityPoolManager::vestAllocation: No CNR to claim. Try after 1 hour');

        uint actualBalance = ICNR(cnr).balanceOf(address(this));
        require(actualBalance >= unallocatedCnr, "LiquidityPoolManager::vestAllocation: Insufficient CNR transferred");
        unallocatedCnr = actualBalance;
    }
