...
import {PermissionsAvalanche} from "./PermissionsAvalanche.sol";


contract PendleDistributeRewards is PermissionsAvalanche {
    using SafeERC20 for IERC20;

    IERC20 internal constant AVAX_TOKEN_ADDRESS = IERC20(
        0xaAaAaAaaAaAaAaaAaAAAAAAAAaaaAaAaAaaAaaAa
    );

    constructor(address _admin) PermissionsAvalanche(_admin) {}
    ...
    function distributeToOne(
        ...
        if (token == AVAX_TOKEN_ADDRESS) {
            require(address(this).balance >= amount, "avax amount required > balance");
...
abstract contract PermissionsAvalanche {
    using SafeERC20 for IERC20;
    ...
    event AvaxWithdraw(uint256 amount, address sendTo);
    ...
    function withdrawAvax(uint256 amount, address payable sendTo) external onlyAdmin {
        (bool success, ) = sendTo.call{value: amount}("");
        require(success, "withdraw failed");
        emit AvaxWithdraw(amount, sendTo);
    }
