...
contract NADOSale {
    ...
    constructor(address _baseToken, address _daoToken, address _dev, uint256 _depositTimestamp, uint256 _closingTimestamp) {
    ...
    function deposit() public onlyEOA {
        require(depositTimestamp <= block.timestamp && block.timestamp <= closingTimestamp, "Can't deposit yet, ?LBE in discord");
        require(!investor[msg.sender],"Already deposited, ?LBE in discord");
    ...
    function claim() public onlyEOA {
        require(initClaimBlock, "Can't claim yet, ?LBE in discord");
        require(claimTimestamp <= block.timestamp, "Can't claim yet, ?LBE in discord");
        require(investor[msg.sender], "Didn't deposit, ?LBE in discord");
        require(!claimed[msg.sender], "Already claimed, ?LBE in discord");
        claimed[msg.sender] = true;
        IERC20(daoToken).transfer(msg.sender, MAX_BY_MINT * 10 ** (ERC20(daoToken).decimals()));
        emit Claim();
    }
