...
contract StrategyWonderland is ReentrancyGuard, Ownable, CoreRef {
    ...
    address public timeBondDepositoryAddress;
    address public timeStakingAddress;
    address public timeStakingHelperAddress;
    address public dexRouterAddress;
    address[] public inputTokenToTimePath;
    ...
    constructor(
        ...
        address _timeAddress,
        address _memoAddress,
        address _timeBondDepositoryAddress,
        address _timeStakingAddress,
        address _timeStakingHelperAddress,
        address _dexRouterAddress,
        address[] memory _inputTokenToTimePath,
        address[] memory _earnedToInputTokenPath,
        uint256 _swapSlippage
    ) public CoreRef(_core) {
        inputTokenAddress = _inputTokenAddress;
        timeAddress = _timeAddress;
        memoAddress = _memoAddress;
        timeBondDepositoryAddress = _timeBondDepositoryAddress;
        timeStakingAddress = _timeStakingAddress;
        timeStakingHelperAddress = _timeStakingHelperAddress;
        dexRouterAddress = _dexRouterAddress;
        inputTokenToTimePath = _inputTokenToTimePath;
    ...
    function deposit(uint256 _depositAmt) public nonReentrant whenNotPaused {
        ...
        if(isMint == true && ITimeBondDepository(timeBondDepositoryAddress).bondPrice() < timePrice) {
            _mint(_depositAmt);
        } else {
    ...
    function _mint(uint256 _depositAmt) internal {
        IERC20(inputTokenAddress).safeApprove(timeBondDepositoryAddress, _depositAmt);
        ITimeBondDepository(timeBondDepositoryAddress).deposit(_depositAmt, timePrice, address(this));
    }

    function _buyAndStake(uint256 _depositAmt) internal {
        uint256 minReturnWant = _calculateMinTimeReturn(_depositAmt);
        IERC20(inputTokenAddress).safeApprove(dexRouterAddress, _depositAmt);
        IJoeRouter(dexRouterAddress).swapExactTokensForTokens(
            _depositAmt,
            minReturnWant,
            inputTokenToTimePath,
            address(this),
            block.timestamp.add(600)
        );
        uint256 timeBalance = IERC20(timeAddress).balanceOf(address(this));
        IERC20(timeAddress).safeApprove(timeStakingHelperAddress, timeBalance);
        IStakingHelper(timeStakingHelperAddress).stake(timeBalance, address(this));
    }
    ...
    function withdraw() public onlyMultistrategy nonReentrant {
        ...
        IERC20(timeAddress).safeApprove(dexRouterAddress, withdrawAmt);
        uint256 minReturnWant = _calculateMinMimReturn(withdrawAmt);
        IJoeRouter(dexRouterAddress).swapExactTokensForTokens(
            withdrawAmt,
            minReturnWant,
    ...
    function _calculateMinMimReturn(uint256 amount) internal view returns (uint256 minReturn) {
        uint256 total = amount.mul(timePrice).mul(1e7); // amount * 10^9 * timePrice * 10^16 / 10^18
        minReturn = total.mul(100 - swapSlippage).div(100);
    }

    function _calculateMinTimeReturn(uint256 amount) internal view returns (uint256 minReturn) {
        uint256 total = amount.div(timePrice.mul(1e7));
        minReturn = total.mul(100 - swapSlippage).div(100);
    }
    ...
    function setTimePrice(uint256 _timePrice) public onlyGovernor {
        // 6 digit number
    ...
    function setInputTokenToTimePath(address[] calldata _inputTokenToTimePath) public onlyGovernor {
        inputTokenToTimePath = _inputTokenToTimePath;
    }
    ...
    function inCaseTokensGetStuck(
        ...
        require(_token != timeAddress, "!safe");
        require(_token != memoAddress, "!safe");
