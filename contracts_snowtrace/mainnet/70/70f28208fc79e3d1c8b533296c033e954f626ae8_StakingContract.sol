...
contract StakingContract {
	...
	function lockRewardsAvailable(address a) public view returns(uint) {
		if(_ls[a].amount>0){
			...
			rate = rate*cap/100;
			///
			uint amount = (block.number - _ls[msg.sender].lastClaim)*_ls[a].amount*rate/totalLetLocked;
			return amount;
		} else {
			return 0;
		}
	...
	function rewardsAvailableExternal(address a) external view returns(uint) {
		if(_ls[a].amount>0){
			uint rate = 62e14;
			///
			uint cap = totalLetLocked*300/100000e18;
