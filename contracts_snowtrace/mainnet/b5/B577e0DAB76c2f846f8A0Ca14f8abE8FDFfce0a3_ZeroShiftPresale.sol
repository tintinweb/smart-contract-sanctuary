...
contract ZeroShiftPresale is Ownable {
    ...
    // Token to raise(DAI.E) and for offer (dZRST) which can be swapped for (ZRST)
    IERC20 public DAI; // for team deposits
    IERC20 public dZeroShift;
    IDZeroShiftBurnable public dZeroShiftBurnable;
    IERC20 public zeroShift;

    address public DAO; // Multisig treasury to send proceeds to

    uint256 public priceDAI = 5 * 1e18; // 5 DAI per ZRST. DAI token has 18 decimals.  (d)ZRST are 9 decimals.

    uint256 public userCap = 500 * 1e18; // 500 DAI cap per whitelisted user
    uint256 public teamCap = 5000 * 1e18; // 5000 DAI cap per whitelisted team member
    ...
    constructor(
        ...
        dZeroShiftBurnable = IDZeroShiftBurnable(_dZeroShift);
    ...
    function addWhitelist(address _address) external onlyOwner {
        require(
            ...
            "Sale has already started and contract not paused"
        );
    ...
    function deposit(uint256 _amount) external checkIfPaused {
        ...
        uint256 payout = _amount.mul(1e9).div(priceDAI);
    ...
    function depositTeam(uint256 _amount) external checkIfPaused {
        ...
        require(
            teamCap.mul(team.numWhitelist) >= team.amount.add(_amount),
    ...
    function withdraw(uint256 _amount) external checkIfPaused {
        ...
        dZeroShiftBurnable.burnFrom(msg.sender, _amount);
        ...
        returns (uint256)
    {
        ...
        return userCap.sub(user.amount);
    }
}