...
contract ACMasterChef is AutoCompoundStrategy {
    ...
    address immutable time = 0xb54f16fB19478766A268F172C9480f8da1a7c9C3;
    address immutable timeToWmemo = 0x0F703D893F684e64784436C6329e0D79b8c86A77;
    ...
    function _safeSwap(
        ...
        if (_path.length == 2 && _path[0] == address(earnToken) && _path[1] == address(stakeToken)) {
            ...
            swapPath[0] = _path[0];
            swapPath[1] = 0xA7D7079b0FEaD91F3e65f86E8915Cb59c1a4C664;
            swapPath[2] = 0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7;
            swapPath[3] = time;
            ...
            uint256 timeBalance = IERC20(time).balanceOf(address(this));
            IERC20(time).safeIncreaseAllowance(timeToWmemo, timeBalance);
            ITimeToWmemo(timeToWmemo).swap(timeBalance);
        } else {
            IUniswapV2Router02 router = swapRouterEnabled ? swapRouter : liquidityRouter;
            IERC20(_path[0]).safeIncreaseAllowance(address(router), _amountIn);
            if (_ignoreErrors) {
