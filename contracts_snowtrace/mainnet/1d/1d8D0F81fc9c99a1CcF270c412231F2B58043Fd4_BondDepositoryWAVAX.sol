...
contract BondDepositoryWAVAX is Ownable {
    ...
    address public immutable PULSE; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints PULSE when receives principle
    ...
    struct Bond {
        uint256 payout; // PULSE remaining to be paid
    ...
    function deposit(
        ...
        require(payout >= 10000000, "Bond too small"); // must be > 0.01 PULSE ( underflow protection )
    ...
    function stakeOrSend(
        ...
        if (!_stake) {
            ...
            IERC20(PULSE).transfer(_recipient, _amount); // send payout
        } else {
            // if user wants to stake
            if (useHelper) {
                // use if staking warmup is 0
                IERC20(PULSE).approve(stakingHelper, _amount);
    ...
    function maxPayout() public view returns (uint256) {
        return IERC20(PULSE).totalSupply().mul(terms.maxPayout).div(100000);
    }
    ...
    /**
     *  @notice calculate current ratio of debt to PULSE supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns (uint256 debtRatio_) {
        uint256 supply = IERC20(PULSE).totalSupply();
    ...
    /**
     *  @notice calculate amount of PULSE available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or PULSE) to the DAO
     *  @return bool
     */
    function recoverLostToken(address _token) external returns (bool) {
        require(_token != PULSE);
