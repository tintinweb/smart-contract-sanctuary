...
https://t.me/silcoJinx
https://www.silcojinx.live
...
contract Contract is IERC20, Ownable {
    ...
    constructor(
        ...
        // Create a joe lp for this new token
        router = IJoeRouter02(routerAddress);
        lp = IJoeFactory(router.factory()).createPair(address(this), router.WAVAX());
    ...
    function _transfer(
        ...
        if (!inSwapAndLiquify && from != lp && from != address(router) && !_isExcludedFromFee[from]) {
        ...
        // also, don't swap & liquify if sender is joe lp.
        ...
        if (overMinTokenBalance && !inSwapAndLiquify && from != lp && swapAndLiquifyEnabled) swapAndLiquify(contractTokenBalance);
    ...
    function swapTokensForEth(uint256 tokenAmount, address to) private {
        // generate the joe lp path of token -> weth
