...
abstract contract RewardsDistributionRecipientUpgradeable is OwnableUpgradeable {
    ...
    event RewardsDistributionSet(address indexed _rewardsDistribution);

    modifier onlyRewardsDistribution() {
        require(msg.sender == rewardsDistribution, "RewardsDistributionRecipientUpgradeable: caller is not the rewardsDistribution");
        _;
    }
    ...
    function setRewardsDistribution(address _rewardsDistribution) external onlyOwner {
        ...
        emit RewardsDistributionSet(_rewardsDistribution);
    }
...
// File: contracts/vaults/VaultFlipToJoe.sol
...
contract VaultFlipToJoe is VaultController, IStrategy, RewardsDistributionRecipientUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function initialize(uint _pid, address _token) external initializer {
        ...
        setMinter(0x1f0FAB7d12B17d7754AaD88f5386A9C6193e8c6f);
        setRewardsToken(0xD980B42bBF9229e61c0EBCe4928a46C04d0636BF);
    }
    ...
    function withdraw(uint amount) override public nonReentrant updateReward(msg.sender) {
        require(amount > 0, "VaultFlipToJoe: amount must be greater than zero");
        _totalSupply = _totalSupply.sub(amount);
        _balances[msg.sender] = _balances[msg.sender].sub(amount);
        uint joeHarvested = _withdrawStakingToken(amount);
        uint withdrawalFee;
        uint performanceFee;
        if (canMint()) {
            ...
            if (withdrawalFee > 0) {
                performanceFee = withdrawalFee.div(100);
        ...
        emit Withdrawn(msg.sender, amount, withdrawalFee.sub(performanceFee));

        _harvest(joeHarvested);
    }
    ...
    function getReward() public override nonReentrant updateReward(msg.sender) {
        ...
        if (reward > 0) {
            ...
            uint before = IERC20(JOE).balanceOf(address(this));
            _rewardsToken.withdraw(reward);
            uint joeBalance = IERC20(JOE).balanceOf(address(this)).sub(before);
            uint performanceFee;

            if (canMint()) {
                performanceFee = _minter.performanceFee(joeBalance);
                _minter.mintForV2(JOE, 0, performanceFee, msg.sender, _depositedAt[msg.sender]);
            }

            IERC20(JOE).safeTransfer(msg.sender, joeBalance.sub(performanceFee));
            emit ProfitPaid(msg.sender, joeBalance, performanceFee);
        }
    }

    function harvest() public override {
        uint joeHarvested = _withdrawStakingToken(0);
        _harvest(joeHarvested);
    }
    ...
    function setMinter(address newMinter) override public onlyOwner {
        ...
        if (newMinter != address(0)) {
            IERC20(JOE).safeApprove(newMinter, 0);
            IERC20(JOE).safeApprove(newMinter, uint(- 1));
        }
    }

    function setRewardsToken(address newRewardsToken) public onlyOwner {
        require(address(_rewardsToken) == address(0), "VaultFlipToJoe: rewards token already set");

        _rewardsToken = IStrategy(newRewardsToken);
        IERC20(JOE).safeApprove(newRewardsToken, 0);
        IERC20(JOE).safeApprove(newRewardsToken, uint(- 1));
    }
    ...
    function setRewardsDuration(uint _rewardsDuration) external onlyOwner {
        require(periodFinish == 0 || block.timestamp > periodFinish, "VaultFlipToJoe: reward duration can only be updated after the period ends");
    ...
    function _deposit(uint amount, address _to) private nonReentrant notPaused updateReward(_to) {
        ...
        uint joeHarvested = _depositStakingToken(amount);
        emit Deposited(_to, amount);

        _harvest(joeHarvested);
    }

    function _depositStakingToken(uint amount) private returns (uint joeHarvested) {
        uint before = IERC20(JOE).balanceOf(address(this));
        JOE_MASTER_CHEF.deposit(pid, amount);
        joeHarvested = IERC20(JOE).balanceOf(address(this)).sub(before);
    }

    function _withdrawStakingToken(uint amount) private returns (uint joeHarvested) {
        uint before = IERC20(JOE).balanceOf(address(this));
        JOE_MASTER_CHEF.withdraw(pid, amount);
        joeHarvested = IERC20(JOE).balanceOf(address(this)).sub(before);
    }

    function _harvest(uint joeAmount) private {
        uint _before = _rewardsToken.sharesOf(address(this));
        _rewardsToken.deposit(joeAmount);
    ...
    function _notifyRewardAmount(uint reward) private updateReward(address(0)) {
        ...
        require(rewardRate <= _balance.div(rewardsDuration), "VaultFlipToJoe: reward rate must be in the right range");
    ...
    // @dev rewardToken(JOE) must not remain balance in this contract. So dev should be able to salvage reward token transferred by mistake.
    function recoverToken(address tokenAddress, uint tokenAmount) external override onlyOwner {
        require(tokenAddress != address(_stakingToken), "VaultFlipToJoe: cannot recover underlying token");

        IERC20(tokenAddress).safeTransfer(owner(), tokenAmount);
        emit Recovered(tokenAddress, tokenAmount);
    }

    /* ========== MIGRATE JOE V1 to V2 ========== */
    function setPidToken(uint _pid, address token) external onlyOwner {
        require(_totalSupply == 0);
        pid = _pid;
        _stakingToken = IERC20(token);

        _stakingToken.safeApprove(address(JOE_MASTER_CHEF), 0);
        _stakingToken.safeApprove(address(JOE_MASTER_CHEF), uint(- 1));
