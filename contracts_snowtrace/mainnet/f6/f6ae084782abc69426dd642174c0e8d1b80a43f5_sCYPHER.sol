...
//Time
    IERC20 public immutable CYPHER;
    IMemo public immutable ORACLE;
    ...
    function stake( uint _amount, address _recipient ) external returns ( bool ) {
        ...
        CYPHER.safeTransferFrom( msg.sender, address(this), _amount );
        ...
        warmupInfo[ _recipient ] = Claim ({
            ...
            gons: info.gons.add( ORACLE.gonsForBalance( _amount ) ),
            expiry: epoch.number.add( warmupPeriod ),
            lock: false
        });
        
        ORACLE.safeTransfer( address(warmupContract), _amount );
    ...
    function claim ( address _recipient ) external {
        ...
        if ( epoch.number >= info.expiry && info.expiry != 0 ) {
            ...
            uint256 amount = ORACLE.balanceForGons( info.gons );
    ...
    function forfeit() external {
        ...
        uint memoBalance = ORACLE.balanceForGons( info.gons );
        warmupContract.retrieve( address(this),  memoBalance);
        CYPHER.safeTransfer( msg.sender, info.deposit);
    ...
    function unstake( uint _amount, bool _trigger ) external {
        ...
        ORACLE.safeTransferFrom( msg.sender, address(this), _amount );
        CYPHER.safeTransfer( msg.sender, _amount );
    ...
    function rebase() public {
        if( epoch.endTime <= uint32(block.timestamp) ) {
            ...
            ORACLE.rebase( epoch.distribute, epoch.number );
            ...
            uint staked = ORACLE.circulatingSupply();
    ...
    function contractBalance() public view returns ( uint ) {
        return CYPHER.balanceOf( address(this) ).add( totalBonus );
    }
