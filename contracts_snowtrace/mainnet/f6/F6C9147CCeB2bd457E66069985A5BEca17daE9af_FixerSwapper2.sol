...
interface NewGame is IPoliceAndThief {
...
interface IBank2 is IBank {
    function setOldBankStats() external;
}

contract FixerSwapper2 is Ownable, Pauseable {
    NewGame public OLD_GAME;
    IBank public OLD_BANK;
    IBank public FIRST_BANK;
    Bank3 public NEW_BANK;
    ...
    function setAddrs(NewGame _newgame, NewGame _oldgame, IBank _firstbank, IBank2 _oldbank, Bank3 _newbank) public onlyOwner {
        OLD_GAME = _oldgame;
        newGame = _newgame;
        FIRST_BANK = _firstbank;
        NEW_BANK = _newbank;
        OLD_BANK = _oldbank;
    }

    uint16 public mylatest = 0;

    function transferFrom(address _from, address _to, uint256 _id) public onlyOwner {
        newGame.transferFrom(_from, _to, _id);
    }

    function setDataToNewBank(TokenData[] memory _data) public onlyOwner {
        for (uint256 i = 0; i < _data.length; i++) {
            TokenData memory t = _data[i];
            newGame.transferFrom(address(OLD_BANK), address(NEW_BANK), t.id);
            NEW_BANK.setOldTokenInfo(t.id, t.isThief, t.owner, t.value);
            emit Registered(t.id, t.owner, t.isThief, t.value);
        }
    }

    function setMyLatest(uint16 my) public onlyOwner {
        mylatest = my;
    }
}
...
contract PoliceAndThief is IPoliceAndThief, ERC721Enumerable, Ownable, Pauseable {
     ...
     */
    constructor(ILOOT _loot, ITraits _traits, uint256 _maxTokens) ERC721("Police Game", 'WGAME') {
        loot = _loot;
        traits = _traits;
...
contract Bank3 is Ownable, IERC721Receiver, Pauseable {
   ...
   */
    constructor(PoliceAndThief _game, LOOT _loot, IBank _oldBank, address _swapper) {
        game = _game;
        loot = _loot;
