...
contract AutoBondPersonalVault {
    ...
    // M0
    function bondWithMim(uint256 _amount, uint256 _maxPrice)
        public
        onlyManager
    {
        require(mim.balanceOf(address(this)) >= _amount, "Not enough tokens");
        // M1 add check for price
        mim.approve(address(mimBondDepository), _amount);
        mimBondDepository.deposit(_amount, _maxPrice, address(this));
        // M0b have MEMO -> unstake, sell for MIM, bond with MIM
    }

    // M0a
    function redeemBond(bool _stake) public onlyManager {
        uint256 amount = mimBondDepository.redeem(address(this), _stake);
        uint256 fee = (amount * fee) / 10000;
        if (_stake) {
            stakedAsset.safeTransfer(feeHarvester, fee);
        } else {
            asset.safeTransfer(feeHarvester, fee);
        }
    }

    // M0a
    function withdraw(uint256 _amount, bool _staked) public onlyDepositor {
        if (_staked) {
            require(
    ...
    // M0b
    // function openClose() external {}

    // // M1
    // function stakeAssets(uint256 _amount) public {}

    // // M1
    // function unstakeAssets(uint256 _amount) public {}
    ...
    // Test function
    function bondPrice() external view returns (uint256 _price) {
        return mimBondDepository.bondPrice();
    }
