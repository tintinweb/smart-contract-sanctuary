...
contract StakingHelper {
    ...
    IERC20 public immutable Dirac;

    constructor ( address _staking, address _Dirac ) {
        require( _staking != address(0) );
        staking = IStaking(_staking);
        require( _Dirac != address(0) );
        Dirac = IERC20(_Dirac);
    }

    function stake( uint _amount, address recipient ) external {
        Dirac.transferFrom( msg.sender, address(this), _amount );
        Dirac.approve( address(staking), _amount );
