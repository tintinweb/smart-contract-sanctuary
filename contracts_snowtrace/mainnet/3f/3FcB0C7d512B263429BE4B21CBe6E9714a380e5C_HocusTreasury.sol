...
contract HocusTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SHocus }
    ...
    address public sHocus;
    uint public sHocusQueue; // Delays change to sHocus address
    ...
    /**
        @notice allow approved address to deposit an asset for Hocus
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint Hocus needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( Hocus ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn Hocus for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IHocusERC20( Hocus ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sHocus ).balanceOf( msg.sender ); // Can only borrow against sHocus held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with Hocus
        @param _amount uint
     */
    function repayDebtWithHocus( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IHocusERC20( Hocus ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, Hocus, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( Hocus ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( Hocus ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns Hocus valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match Hocus decimals
            value_ = _amount.mul( 10 ** IERC20( Hocus ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SHocus ) { // 9
            sHocusQueue = block.number.add( blocksNeededForQueue );
        } else return false;
