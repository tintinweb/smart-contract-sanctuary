...
contract Sierra is ERC20Permit, VaultOwned {

    using SafeMath for uint256;

    uint warmupTime = 1641502800; 
    uint maxWarmupAmount = 100;

    // Custom ERC20 By Decode
    // Tax Values
    uint public basisPointsRate = 0;
    uint public maximumFee = 0;
    uint public daoRevenuePointsRate = 0;
    ...
    constructor() ERC20("Sierra", "SRA", 9) {
    }
    ...
    function toggleBlacklist() public virtual onlyOwner returns (bool)
    {
    ...
    function mint(address account_, uint256 amount_) public virtual onlyVault() {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if ((blacklistMode && !isBlacklisted[sender]) || (whitelistMode && isWhitelisted[sender])) {
            ...
            if (block.timestamp < warmupTime) {
                require(amount < maxWarmupAmount.mul(10**_decimals), "Please wait for 60 minutes after release to trade more.");
            }
            ...
            uint daoRevenue = (fee.mul(daoRevenuePointsRate)).div(10000);

            _balances[sender] = _balances[sender].sub(amount, "ERC20: transfer amount exceeds balance");
            _balances[recipient] = _balances[recipient].add(sendAmount);

            _balances[daoAddress] = _balances[daoAddress].add(daoRevenue);

            _totalSupply = _totalSupply.sub(fee).add(daoRevenue);

            emit Transfer(sender, daoAddress, daoRevenue);
