...
contract Registry is SYSTEM, ERC721("PR0XY Registry", "PR0XY ID") {
  ...
  function assignId(address wallet_) external returns (bytes2) {

    // 1. Take the first two bytes (4 hex characters) of a hash of the wallet
    bytes32 addressHash = keccak256(abi.encode(wallet_));
    bytes2 generatedId = bytes2(addressHash);

    // 2. Keep hashing until a unique unregistered Id is found (due to potential collisions)
    while (generatedId == bytes2(0) && walletOf[generatedId] != address(0)) {
      addressHash = keccak256(abi.encode(addressHash));
      generatedId = bytes2(addressHash);
    }

    // 3. Save the userId in the DAO
    walletOf[generatedId] = wallet_;
