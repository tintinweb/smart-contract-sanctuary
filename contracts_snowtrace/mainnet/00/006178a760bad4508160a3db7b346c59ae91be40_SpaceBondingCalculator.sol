...
contract SpaceBondingCalculator is IBondingCalculator {
    ...
    function markdown( address _pair ) external view returns ( uint _value ) {
        ( uint reserve0, uint reserve1, ) = IUniswapV2Pair( _pair ).getReserves();

        uint reserve;
        if ( IUniswapV2Pair( _pair ).token0() == Space ) {
            reserve = reserve1;
        } else {
            reserve = reserve0;
        }
        _value = reserve.mul( 2 * ( 10 ** IERC20( Space ).decimals() ) ).div( getTotalValue( _pair ) );
    }
}