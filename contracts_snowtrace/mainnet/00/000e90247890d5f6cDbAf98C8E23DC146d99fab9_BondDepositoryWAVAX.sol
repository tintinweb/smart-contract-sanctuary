...
contract BondDepositoryWAVAX is Ownable {
    ...
    address public immutable CDAO; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints CDAO when receives principle
    ...
    struct Bond {
        uint256 payout; // CDAO remaining to be paid
    ...
    function deposit(
        ...
        require(payout >= 10000000, "Bond too small"); // must be > 0.01 CDAO ( underflow protection )
    ...
    function stakeOrSend(
        ...
        if (!_stake) {
            ...
            IERC20(CDAO).transfer(_recipient, _amount); // send payout
        } else {
            // if user wants to stake
            if (useHelper) {
                // use if staking warmup is 0
                IERC20(CDAO).approve(stakingHelper, _amount);
                IStakingHelper(stakingHelper).stake(_amount, _recipient);
            } else {
                IERC20(CDAO).approve(staking, _amount);
    ...
    function maxPayout() public view returns (uint256) {
        return IERC20(CDAO).totalSupply().mul(terms.maxPayout).div(100000);
    }
    ...
    /**
     *  @notice calculate current ratio of debt to CDAO supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns (uint256 debtRatio_) {
        uint256 supply = IERC20(CDAO).totalSupply();
    ...
    /**
     *  @notice calculate amount of CDAO available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or CDAO) to the DAO
     *  @return bool
     */
