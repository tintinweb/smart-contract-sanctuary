...
contract Traits is Ownable, ITraits {
  ...
  function setWnD(address _awndNFT) external onlyOwner {
  ...
  function drawSVG(uint256 tokenId) internal view returns (string memory) {
    ...
    return string(abi.encodePacked(
      '<svg id="awndNFT" width="100%" height="100%" version="1.1" viewBox="0 0 40 40" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">',
      svgString,
      "</svg>"
    ));
  }
  ...
  function compileAttributes(uint256 tokenId) internal view returns (string memory) {
    ...
    return string(abi.encodePacked(
      ...
      tokenId <= awndNFT.getPaidTokens() ? '"Gen 0"' : '"Gen 1"',
  ...
  function tokenURI(uint256 tokenId) public view override returns (string memory) {
    require(_msgSender() == address(awndNFT), "hmmmm what doing?");
    IWnD.WizardDragon memory s = awndNFT.getTokenTraits(tokenId);
    ...
    string memory metadata = string(abi.encodePacked(
      ...
      s.isWizard ? 'Wizard #' : 'Dragon #',
      tokenId.toString(),
      '", "description": "Thousands of Wizards and Dragons compete in a tower in the metaverse. A tempting prize of $aGP awaits, with deadly high stakes. All the metadata and images are generated and stored 100% on-chain. No IPFS. NO API. Just the Avalanche C-chain.", "image": "data:image/svg+xml;base64,',
