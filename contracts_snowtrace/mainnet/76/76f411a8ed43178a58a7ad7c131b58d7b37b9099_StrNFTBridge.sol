...
// File contracts/StrNFTBridge.sol



pragma solidity ^0.7.6;
contract StrNFTBridge is ReentrancyGuard, Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    ...
    function notifyRewardAmount(uint256 reward) external {
        require(msg.sender == owner() || msg.sender == liqPoolManager, "Liqpool Manager or Owner can call this.");
        uint balance = rewardsToken.balanceOf(address(this));
        require(ICNR(cnr).transfer(stakingRewardsAddress, balance), 'LiquidityPoolManager::distributeTokens: Transfer failed');
        StakingRewards(stakingRewardsAddress).notifyRewardAmount(balance);
        emit Bridged(stakingRewardsAddress, balance);
    }

    function setStrAddress(address _newStrAddress) external onlyOwner {
        stakingRewardsAddress = _newStrAddress;
    }
    ...
    event Bridged(address StrAddress, uint256 balance);
    event Recovered(address token, uint256 amount);
}
