...
interface PrezaleLockerInterface {
    function changeLockOwner(uint256 lockId, address newowner)
        external
        returns (address newLockOwner);
    ...
    function extendLock(uint256 lockId, uint256 unlockTimestamp) external;
...
contract LOCK {
    ...
    constructor(
        ...
        (
            ...
            uint256 unlockTimestamp,
            address pair
        ) = PrezaleLockerInterface(parentContract).getLockInfo(lockid);
        ...
        )
    {
        ...
        (owner, , , , , , , unlockTimestamp, pair) = PrezaleLockerInterface(
            parentContract
        ).getLockInfo(lockid);
        if (pair != address(0)) {
            isLpLock = true;
    ...
    function isWithdrawn() public view returns (bool withdrawn) {
        (, , , , , , uint256 amountRemaining, , ) = PrezaleLockerInterface(
            parentContract
        ).getLockInfo(lockid);
    ...
    function extendLock(uint256 newLockTimestamp) public {
        PrezaleLockerInterface(parentContract).extendLock(
            lockid,
            newLockTimestamp
        );
    }
    ...
    function changeLockOwner(address newowner) public {
        PrezaleLockerInterface(parentContract).changeLockOwner(
            lockid,
            newowner
        );
    }
