...
contract BountyTreasury is Ownable {
    ...
    address public sBOUNTY;
    uint public sBOUNTYQueue; // Delays change to sBOUNTY address
    ...
    //    isLiquidityToken[ _BOUNTYDAI ] = true;
    //    liquidityTokens.push( _BOUNTYDAI );

        secondsNeededForQueue = _secondsNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for BOUNTY
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint BOUNTY needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( Bounty ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn BOUNTY for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IBOUNTY( Bounty ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sBOUNTY ).balanceOf( msg.sender ); // Can only borrow against sBOUNTY held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with BOUNTY
        @param _amount uint
     */
    function repayDebtWithBOUNTY( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IBOUNTY( Bounty ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, Bounty, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( Bounty ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( Bounty ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns BOUNTY valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match BOUNTY decimals
            value_ = _amount.mul( 10 ** IERC20( Bounty ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SBOUNTY ) { // 9
            sBOUNTYQueue = uint32(block.timestamp).add32( secondsNeededForQueue );
        } else return false;
