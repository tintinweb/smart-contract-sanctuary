...
contract HocusToken is ERC20Permit, VaultOwned {
    ...
    constructor() ERC20("HocusDao", "$Hocus", 9) {
      operator = msg.sender;
    }
    ...
    function _transfer(
        ...
        if(sender == owner() || sender == operator || recipient == owner() || recipient == operator || recipient == vault() || sender == vault()) {
          super._transfer(sender, recipient, amount);
        } else {
          require(openTrading == true, "trading is not opened");
          super._transfer(sender, recipient, amount);
        }
    }
    ...
    function setOperator(address _operator) public {
      ...
      require(_operator != address(0), "_operator is zero addresss");

      operator = _operator;
    }

    function setOpenTrading(bool _trueOrfalse) public {
      require(msg.sender == operator, "msg.sender is not operator");

      openTrading = _trueOrfalse;
    }
}