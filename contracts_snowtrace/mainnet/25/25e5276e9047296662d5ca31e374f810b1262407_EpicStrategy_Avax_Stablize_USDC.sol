...
// File: interfaces/stabilize/IStabilize.sol

pragma solidity >=0.6.0 <0.9.0;

interface IStabilizeChef {
...
contract EpicStrategy_Avax_Stablize_USDC{
    ...
    uint256 public constant TRADERJOE_FEE = 30;
    ...
    function calcUnderlyingFinalDeposit(uint256 _amount) external view returns (uint256) {
        return _amount.mul(TRADERJOE_FEE).div(MAX_BPS);
    }

    function calcUSDCperJLP() public view returns (uint256) {
        (uint256 reserveUSDC, , ) = ITraderJoeLP(JOE_LP_TOKEN).getReserves();
        uint256 totalSupplyJLP = ITraderJoeLP(JOE_LP_TOKEN).totalSupply().div(1e12);
    ...
    function balanceOfPool() public view returns (uint256) {
        //Balance of Curve LP
        uint256 amountJLP = IStabilizeChef(CHEF).balanceOf(address(this));

        if (amountJLP > 0) {

            uint256 amountUSDC = amountJLP.mul(calcUSDCperJLP()).div(1e12);
    ...
    function deposit() public {
        ...
        IStabilizeChef(CHEF).stake(IERC20(JOE_LP_TOKEN).balanceOf(address(this)));
    ...
    function _withdrawSome(uint256 _amount) internal returns (uint256) {
        ...
        uint256 _amountJLP = IStabilizeChef(CHEF).balanceOf(address(this));
        if (_amountNeededJLP > _amountJLP) {
            _amountNeededJLP = _amountJLP;
        }

        IStabilizeChef(CHEF).withdraw(_amountNeededJLP);
    ...
    function _withdrawAll() internal {
        ...
        if(_amountJLP > 0) {
            IStabilizeChef(CHEF).withdraw(_amountJLP);
    ...
    function harvest() public {
        ...
        pathA[0] = reward;
        pathA[1] = USDC;
        ITraderJoeRouter(TRADERJOE_ROUTER).swapExactTokensForTokensSupportingFeeOnTransferTokens(
            rewardsToReinvest1,
            0,
            pathA,
            address(this),
