...
// File contracts/ERC20Distributor.sol
...
contract ERC20Distributor {
    ...
    function splitERC20(address token) public {
        ...
        if (tokenBalance > 0) {
            for (uint256 i = 0; i < numberBeneficiaries; i++) {
            IERC20(token).safeTransfer(beneficiaries[i], (tokenBalance * bips[i]) / MAX_BIPS);
            }
        }
    }
...
// File contracts/ERC20DistributorAdjustableBips.sol
pragma solidity ^0.8.6;
...
// File contracts/BOOFI_Distributor.sol
pragma solidity ^0.8.6;
