...
contract OlympusTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SGAIA }
    ...
    address public sGAIA;
    uint public sGAIAQueue; // Delays change to sGAIA address
    ...
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IGAIAERC20( GAIA ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sGAIA ).balanceOf( msg.sender ); // Can only borrow against sGAIA held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with GAIA
        @param _amount uint
     */
    function repayDebtWithGAIA( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IGAIAERC20( GAIA ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, GAIA, _amount, _amount );
    }

    function updateTimeNeededForQueue( uint _timeNeededForQueue ) external {
        require(isReserveManager[ msg.sender ], "Not approved" );
        timeNeededForQueue = _timeNeededForQueue;
    }

    function renounceBootstrappingAllowance( uint _timeNeededForQueue ) external {
        require(isReserveManager[ msg.sender ], "Not approved" );
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        require( _amount <= excessReserves() || bootstrappingReservesAllowed, "Insufficient reserves" );

        IERC20Mintable( GAIA ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( GAIA ).totalSupply().sub( totalDebt ) );
    }
    ...
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            ...
            value_ = _amount.mul( 10 ** IERC20( GAIA ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SGAIA ) { // 9
            sGAIAQueue = block.timestamp.add( timeNeededForQueue );
        } else return false;
