...
contract CoinClawVesting is IERC20, IERC20Vesting, ReentrancyGuard{
	...
	constructor(address _token) {
		require(_token != address(0), "Vesting: E00: token address cannot be zero.");
		token = _token;
	}
	...
	function _collectTokens(Schedule storage vesting, uint256 amount) private returns(uint256){
		...
		bool ok = IERC20(token).transfer(vesting.beneficiary, tobeTransfered);
		if(ok == false){
			return 0;
		}
	...
	function _refundTokens(address grantor, uint256 amount) internal returns (uint256){
		...
		bool ok = IERC20(token).transfer(grantor, amount);
		if( ok == false){
			return 0;
		}
	...
	function _addVestingSchedule(Schedule memory vesting) internal{
		...
		require(IERC20(token).balanceOf(address(this)) >= _pendingTotal, "Vesting: E04: Insufficent balance. Please deposit ERC20 tokens to vesting contract.");
	...
	*/
	...
	function allowGrantor(address grantor) external nonReentrant override returns(bool){
		require(msg.sender == Ownable(token).owner(), "Vesting: E05: Authorization denied.");
	...
	function disallowGrantor(address grantor) external nonReentrant override returns(bool){
		require(msg.sender == Ownable(token).owner(), "Vesting: E07: Authorization denied.");
	...
	function getFreeTokens() external view override returns(uint256){
		...
		return IERC20(token).balanceOf(address(this)) - _pendingTotal;
	}
	...
	*/
	...
	function disburse(uint256 amount) external override returns(uint256){
		...
		uint256 refundable = min(amount, IERC20(token).balanceOf(address(this)) - _pendingTotal);

		IERC20(token).transfer(msg.sender, refundable);
