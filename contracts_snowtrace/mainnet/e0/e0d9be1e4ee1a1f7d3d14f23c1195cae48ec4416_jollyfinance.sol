...
// File contracts/jollyfinance.sol

pragma experimental ABIEncoderV2;

/**
 * @title jollyfinance
 * @dev The governance token for jollyfinance
 * ERC-20 with fixed supply + offchain signing
 */
contract jollyfinance {
    using SafeMath for uint256;

    /// @notice EIP-20 token name for this token
    string public name = "jollyfinance";
    ...
    uint256 public totalSupply = 1_000_000_000_000e18; // 1,000,000,000,000
    ...
    function permit(address owner, address spender, uint256 value, uint256 deadline, uint8 v, bytes32 r, bytes32 s) external {
        require(deadline >= block.timestamp, "jollyfinance::permit: signature expired");
    ...
    function transferFrom(address src, address dst, uint256 amount) external returns (bool) {
        ...
        if (spender != src && spenderAllowance != uint256(-1)) {
            uint256 newAllowance = spenderAllowance.sub(
                amount,
                "jollyfinance::transferFrom: transfer amount exceeds allowance"
            );
    ...
    function _validateSignedData(address signer, bytes32 encodeData, uint8 v, bytes32 r, bytes32 s) internal view {
        ...
        require(recoveredAddress != address(0) && recoveredAddress == signer, "jollyfinance::validateSig: invalid signature");
    }
    ...
    function _approve(address owner, address spender, uint256 amount) internal {
        require(owner != address(0), "jollyfinance::_approve: approve from the zero address");
        require(spender != address(0), "jollyfinance::_approve: approve to the zero address");
    ...
    function _decreaseAllowance(
        ...
        _approve(
            ...
            allowances[owner][spender].sub(
                subtractedValue,
                "jollyfinance::_decreaseAllowance: decreased allowance below zero"
            )
        );
    }
    ...
    function _transferTokens(address from, address to, uint256 value) internal {
        require(to != address(0), "jollyfinance::_transferTokens: cannot transfer to the zero address");

        balances[from] = balances[from].sub(
            value,
            "jollyfinance::_transferTokens: transfer exceeds from balance"
        );
