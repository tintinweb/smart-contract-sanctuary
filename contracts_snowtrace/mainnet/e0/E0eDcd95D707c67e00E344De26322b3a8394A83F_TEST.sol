...
contract DividendDistributor is IDividendDistributor {
    ...
    IERC20 EP = IERC20(0x49D5c2BdFfac6CE2BFdB6640F4F80f226bc10bAB);
...
contract TEST is IERC20, Auth {
    ...
    constructor () Auth(msg.sender) {
        ...
        pair = IFactory(router.factory()).createPair(WAVAX, address(this));
    ...
    function swapBack(uint256 saleAmount) internal swapping {
        uint256 swapThresholdClamped = swapThreshold;
        if (swapThresholdClamped > saleAmount) {
            swapThresholdClamped = saleAmount;
        }
        uint256 dynamicLiquidityFee = isOverLiquified(targetLiquidity, targetLiquidityDenominator) ? 0 : liquidityFee;
        uint256 amountToLiquify = swapThresholdClamped.mul(dynamicLiquidityFee).div(totalFee).div(2);
        uint256 amountToSwap = swapThresholdClamped.sub(amountToLiquify);
    ...
    //     uint256 amountToSwap = swapThreshold.sub(amountToLiquify);
        ...
        uint256 amountAVAXReflection = amountAVAX.sub(amountAVAXLiquidity).sub(amountAVAXMarketing);
