...
contract IQCoinMasterChef is Ownable {
    using SafeMath for uint256;
    ...
    // Percentage of pool rewards that goes to the burn.
    ...
    constructor(
        ...
        require(
            0 <= _burnPercent && _burnPercent <= 1000,
            "constructor: invalid burn percent value"
        );
        require(
        ...
        require(
            _devPercent + _burnPercent + _investorPercent <= 1000,
    ...
    function init(uint _initialSupply) public onlyOwner {
        require(initiated == false, "Already initiated");
        
        joe.mint(devAddr, _initialSupply.mul(devPercent).div(1000));
        joe.mint(burnAddr, _initialSupply.mul(burnPercent).div(1000));
        joe.mint(investorAddr, _initialSupply.mul(investorPercent).div(1000));
        joe.mint(msg.sender, _initialSupply.mul(1000-devPercent-burnPercent-investorPercent).div(1000));
...
//        uint256 joeReward2 = multiplier.mul(joePerSec);
        ...
        joe.mint(burnAddr, joeReward.mul(burnPercent).div(1000));
    ...
    function setDevPercent(uint256 _newDevPercent) public onlyOwner {
        ...
        require(
            burnPercent + _newDevPercent + investorPercent <= 1000,
            "setDevPercent: total percent over max"
        );
        devPercent = _newDevPercent;
    }

    // Update burn address by the previous burn.
    function setBurnAddr(address _burnAddr) public {
        require(msg.sender == burnAddr, "setBurnAddr: wut?");
        burnAddr = _burnAddr;
    }

    function setBurnPercent(uint256 _newBurnPercent) public onlyOwner {
        require(
            0 <= _newBurnPercent && _newBurnPercent <= 1000,
            "setBurnPercent: invalid percent value"
        );
        require(
            devPercent + _newBurnPercent + investorPercent <= 1000,
            "setBurnPercent: total percent over max"
        );
    ...
    function setInvestorPercent(uint256 _newInvestorPercent) public onlyOwner {
        ...
        require(
            devPercent + _newInvestorPercent + burnPercent <= 1000,
...
contract IQCoin is JoeToken {
    constructor() ERC20("IQCoin", "IQC") {}
}

interface IQC_Interface {
    function balanceOf(address account) external view returns (uint256);
    function totalSupply() external view returns (uint256);
}
...
contract Data {
    ...
    uint ethwei = 10**18;
    address IQC = 0x85Ad6DA4f531715bb1915a9AF077CffAde78A4ef;
    address FARM = 0xe01Dc2183735F2EFe1b4804e0934A297Ba6A0Ce6;
    address LP = 0xaDAdD709b75445ed5D7c58F1FE43871005ecc04D;
    
    IQC_Interface public iqc = IQC_Interface(IQC);
    ...
    function getRewardShare_18(address _user) public view returns (uint) {
        uint reward = farm.userAmount(0, _user) * 100 * ethwei / lp.balanceOf(FARM);
        return reward;
    }
    
    function getData() public view returns (uint, uint, uint, uint, uint, uint) {
        (uint r0, uint r1, ) = lp.getReserves();
        uint price_18 = (r1 * ethwei) / r0;
        
        uint liquidityPerLP = (iqc.balanceOf(LP)*price_18*2) / lp.totalSupply();
        uint tvl_36 = liquidityPerLP*lp.balanceOf(FARM);
        
        uint apr = farm.getEmission() * price_18 * 18921600 * 100 / tvl_36;
        
        return (iqc.totalSupply(), tvl_36, price_18, farm.getEmission()*6/10, apr, iqc.balanceOf(0x000000000000000000000000000000000000dEaD));
    }
}