...
/**
 * @title ETHPSwap
 * @dev Swap ETHPv1 for ETHPv2
 */
contract ETHPSwap is Ownable {
  IERC20 private ethpV1;
  IERC20 private ethpV2;
  ...
  function swap() external {
    ...
    uint256 _v2Amount = ethpV1.balanceOf(msg.sender);
    require(_v2Amount > 0, 'you do not have any V1 tokens');
    require(
      ethpV2.balanceOf(address(this)) >= _v2Amount,
      'not enough V2 liquidity to complete swap'
    );
    swapped[msg.sender] = true;
    ethpV1.transferFrom(msg.sender, address(this), _v2Amount);
    ethpV2.transfer(msg.sender, _v2Amount);
  }
  ...
  function v1() external view returns (address) {
    return address(ethpV1);
  }

  function v2() external view returns (address) {
    return address(ethpV2);
  }
