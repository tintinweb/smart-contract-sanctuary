...
contract PolarNFT is Context, ERC721Enumerable, Ownable, ReentrancyGuard {
  using Strings for uint256;
  ...
  constructor(string memory name, string memory symbol, uint MAX_SUPPLY_, string memory _initBaseURI) ERC721(name, symbol) {
    setBaseURI(_initBaseURI);
    MAX_SUPPLY = MAX_SUPPLY_;
  }

  function directMint(address dest_wallet) external onlyOwner{
    uint256 supply = totalSupply();
    require(!paused, "The contract is paused");
    require(totalSupply() < MAX_SUPPLY, "No more mints");

    _safeMint(dest_wallet, supply + 1);
    _setTokenURI(supply + 1, baseTokenURI);
  }

  function allMint(address[] calldata whitelist) external onlyOwner{
    require(!paused, "The contract is paused");
    require(totalSupply() < MAX_SUPPLY, "No more mints");

    for (uint i=0; i<whitelist.length; i++) {
      if (getTokenIds(whitelist[i]).length == 0) {
      uint256 supply = totalSupply();
      _safeMint(whitelist[i], supply + 1);
      _setTokenURI(supply + 1, baseTokenURI);
      }
    }
  }
  ...
  function setSupply(uint _baseSupply) public onlyOwner {
    MAX_SUPPLY = _baseSupply;
  }
...
contract PolarWLsale is Crowdsale, TimedCrowdsale
{
    using SafeERC20 for IERC20;

    uint public hardCap;
    uint public allocationCap;
    PolarNFT public polarnft;
    ...
    constructor(uint allocationCap_, uint hardCap_, uint numerator_, uint denominator_, address subject_, address token_, address polarNFT_, address MultisigWallet_, uint openingTime, uint closingTime)
    Crowdsale(numerator_, denominator_, address(this), IERC20(subject_), IERC20(token_))
    TimedCrowdsale(openingTime, closingTime)
    {
        allocationCap = allocationCap_ * 10 ** 18;
        hardCap = hardCap_ * 10 ** 18;
        claimOpen = false;
        endSale = false;
        polarnft = PolarNFT(polarNFT_);
        MultisigWallet = MultisigWallet_;
        walletContract = token_;
    }
    function getPurchasable(address wallet) public view returns (uint)
    {
        return purchasedAddresses[wallet];
    }
    ...
    function endOfSale() external onlyOwner
    {
        endSale = true;
    }
// onlyWhileOpen
    ...
    {
        require(polarnft.getTokenIds(msg.sender).length != 0, "You dont have the NFT Key in your wallet");
        uint256 supply;
        for (uint256 i=0; i<polarnft.totalSupply(); i++) {
            supply = supply+ 1;
        }
        ...
        IERC20(subject).transfer(MultisigWallet, amount);
    ...
    function retreiveExcessToken() external onlyOwner {
        require(endSale, "Presale is not over yet");
        token.safeTransfer(walletContract, checkbalances());
    }
}