...
contract MythicTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SMYTC }
    ...
    address public sMYTC;
    uint public sMYTCQueue; // Delays change to sMYTC address
    ...
    constructor (
        ...
        isReserveToken[ _MYTC ] = true;
        reserveTokens.push( _MYTC );
    ...
    /**
        @notice allow approved address to deposit an asset for MYTC
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint MYTC needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( MYTC ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn MYTC for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IMYTCERC20( MYTC ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sMYTC ).balanceOf( msg.sender ); // Can only borrow against sMYTC held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with MYTC
        @param _amount uint
     */
    function repayDebtWithMYTC( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IMYTCERC20( MYTC ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, MYTC, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( MYTC ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( MYTC ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns MYTC valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match MYTC decimals
            value_ = _amount.mul( 10 ** IERC20( MYTC ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SMYTC ) { // 9
            sMYTCQueue = block.number.add( blocksNeededForQueue );
        } else return false;
