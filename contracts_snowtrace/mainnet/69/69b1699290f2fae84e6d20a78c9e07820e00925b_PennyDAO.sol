...
contract PennyDAO is ERC20 {
  ...
  function reset(uint256 _price) external {
    onlyOwner();
    require((_price > 0) && (_price < price));
    if (totalSupply > 0) {
      uint256 threshold = (balanceOf[owner] * 10500) / 10000;
      require(threshold > totalSupply);
      uint256 offset = address(this).balance - (totalSupply * _price);
      payable(treasury).transfer(offset);
      updatePrice();
    } else {
      price = _price;
    }
