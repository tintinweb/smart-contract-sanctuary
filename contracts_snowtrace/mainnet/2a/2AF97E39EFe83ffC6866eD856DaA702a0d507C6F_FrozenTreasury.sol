...
contract FrozenTreasury is Ownable {
    using SafeMath for uint256;
    ...
    address public immutable Frozen;
    ...
    address public sFrozen;
    uint256 public sFrozenQueue; // Delays change to sFrozen address
    ...
    function deposit(
        ...
        IERC20Mintable(Frozen).mint(msg.sender, send_);
    ...
    function withdraw(uint256 _amount, address _token) external {
        ...
        uint256 value = valueOf(_token, _amount);
        IFrozenERC20(Frozen).burnFrom(msg.sender, value);
    ...
    function incurDebt(uint256 _amount, address _token) external {
        ...
        uint256 maximumDebt = IERC20(sFrozen).balanceOf(msg.sender); // Can only borrow against sFrozen held
    ...
    function repayDebtWithOHM(uint256 _amount) external {
        ...
        IFrozenERC20(Frozen).burnFrom(msg.sender, _amount);

        debtorBalance[msg.sender] = debtorBalance[msg.sender].sub(_amount);
        totalDebt = totalDebt.sub(_amount);

        emit RepayDebt(msg.sender, Frozen, _amount, _amount);
    }
    ...
    function mintRewards(address _recipient, uint256 _amount) external {
        ...
        IERC20Mintable(Frozen).mint(_recipient, _amount);
    ...
    function excessReserves() public view returns (uint256) {
        return totalReserves.sub(IERC20(Frozen).totalSupply().sub(totalDebt));
    }
        ...
        returns (uint256 value_)
    {
        if (isReserveToken[_token]) {
            ...
            value_ = _amount.mul(10**IERC20(Frozen).decimals()).div(
        ...
        returns (bool)
    {
        ...
        } else if (_managing == MANAGING.SFrozen) {
            // 9
            sFrozenQueue = uint32(block.timestamp).add32(secondsNeededForQueue);
        } else return false;
