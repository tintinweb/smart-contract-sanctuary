...
// File: contracts\interfaces\IStakingRewards06.sol
...
// File: contracts\StakedLPToken06.sol
...
contract StakedLPToken06 is IUniswapV2Pair, PoolToken {
    using SafeToken for address;
	
	bool public constant isStakedLPToken = true;
	string public constant stakedLPTokenType = "06";
	...
	function _initialize(
		...
		require(factory == address(0), "StakedLPToken06: FACTORY_ALREADY_SET"); // sufficient check
		factory = msg.sender;
	...
	function _update() internal {
		totalBalance = IStakingRewards06(stakingRewards).supplyAmount(address(this));
		emit Sync(totalBalance);
	}
	...
	function mint(address minter) external nonReentrant update returns (uint mintTokens) {
		...
		IStakingRewards06(stakingRewards).deposit(mintAmount);
		mintTokens = mintAmount.mul(1e18).div(exchangeRate());
		...
		require(mintTokens > 0, "StakedLPToken06: MINT_AMOUNT_ZERO");
		_mint(minter, mintTokens);
		emit Mint(msg.sender, minter, mintAmount, mintTokens);
	}
	...
	function redeem(address redeemer) external nonReentrant update returns (uint redeemAmount) {
		...
		require(redeemAmount > 0, "StakedLPToken06: REDEEM_AMOUNT_ZERO");
		require(redeemAmount <= totalBalance, "StakedLPToken06: INSUFFICIENT_CASH");
		_burn(address(this), redeemTokens);
		IStakingRewards06(stakingRewards).redeem(redeemAmount);
		_safeTransfer(redeemer, redeemAmount);
		emit Redeem(msg.sender, redeemer, redeemAmount, redeemTokens);		
	}
	...
	function _getReward() internal returns (uint256) {
		IStakingRewards06(stakingRewards).claimRewards();
		uint256 wethBalance = WETH.myBalance();
		if (wethBalance > 0) {
			swapExactTokensForTokens(WETH, rewardsToken, wethBalance);
		}
	...
	function reinvest() external nonReentrant update {
		...
		IStakingRewards06(stakingRewards).deposit(liquidity);
	...
	function getReserves() external view returns (uint112 reserve0, uint112 reserve1, uint32 blockTimestampLast) {
		...
		require(reserve0 > 100 && reserve1 > 100, "StakedLPToken06: INSUFFICIENT_RESERVES");
	}
    ...
    function safe112(uint n) internal pure returns (uint112) {
        require(n < 2**112, "StakedLPToken06: SAFE112");
        return uint112(n);
    }
}

// File: contracts\StakedLPTokenFactory06.sol
