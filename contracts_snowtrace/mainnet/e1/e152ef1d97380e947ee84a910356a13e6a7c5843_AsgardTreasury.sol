...
contract AsgardTreasury is Ownable {
    ...
    /**
        ...
        @param _amount uint
     */
    function repayDebtWithVolt( uint _amount ) external {
    ...
    function emergencyWithdrawl(address token_, uint amount_, bool all_) external onlyOwner() {
        if(all_){
            uint total = IERC20(token_).balanceOf(address(this));
            IERC20(token_).transfer(msg.sender, total);
        }else{
