 ...
 * @author Domani Protocol
 *
 * Module that enables issuance and redemption functionality on a Domani. This is a module that is
 * required to bring the totalSupply of a Domani above 0.
 */
contract BasicIssuanceModule is ModuleBase, ReentrancyGuard {
  ...
  event DomaniIssued(
    address indexed _domani,
    address indexed _issuer,
    address indexed _to,
    address _hookContract,
    uint256 _quantity
  );
  event DomaniRedeemed(
    address indexed _domani,
  ...
  // Mapping of Domani to Issuance hook configurations
  mapping(IDomani => IManagerIssuanceHook) public managerIssuanceHook;

  /* ============ Constructor ============ */

  /**
   * Domani state controller state variable
   *
   * @param _controller             Address of controller contract
   */
  ...
  /**
   * Deposits the Domani's position components into the Domani and mints the Domani of the given quantity
   * to the specified _to address. This function only handles Default Positions (positionState = 0).
   *
   * @param _domani             Instance of the Domani contract
   * @param _quantity             Quantity of the Domani to mint
   * @param _to                   Address to mint Domani to
   */
  function issue(
    IDomani _domani,
    uint256 _quantity,
    address _to
  ) external nonReentrant onlyValidAndInitializedDomani(_domani) {
    require(_quantity > 0, "Issue quantity must be > 0");

    address hookContract = _callPreIssueHooks(_domani, _quantity, msg.sender, _to);

    (
      address[] memory components,
      uint256[] memory componentQuantities
    ) = getRequiredComponentUnitsForIssue(_domani, _quantity);

    // For each position, transfer the required underlying to the Domani
    for (uint256 i = 0; i < components.length; i++) {
      // Transfer the component to the Domani
      transferFrom(IERC20(components[i]), msg.sender, address(_domani), componentQuantities[i]);
    }

    // Mint the Domani
    _domani.mint(_to, _quantity);

    emit DomaniIssued(address(_domani), msg.sender, _to, hookContract, _quantity);
  }

  /**
   * Redeems the Domani's positions and sends the components of the given
   * quantity to the caller. This function only handles Default Positions (positionState = 0).
   *
   * @param _domani             Instance of the Domani contract
   * @param _quantity             Quantity of the Domani to redeem
  ...
  function redeem(
    ...
    // Burn the Domani - ERC20's internal burn already checks that the user has enough balance
    _domani.burn(msg.sender, _quantity);

    // For each position, invoke the Domani to transfer the tokens to the user
    address[] memory components = _domani.getComponents();
    for (uint256 i = 0; i < components.length; i++) {
      address component = components[i];
      require(!_domani.hasExternalPosition(component), "Only default positions are supported");

      uint256 unit = _domani.getDefaultPositionRealUnit(component).toUint256();
      ...
      // Instruct the Domani to transfer the component to the user
      _domani.strictInvokeTransfer(component, _to, componentQuantity);
    }

    emit DomaniRedeemed(address(_domani), msg.sender, _to, _quantity);
  }

  /**
   * Initializes this module to the Domani with issuance-related hooks. Only callable by the Domani's manager.
   * Hook addresses are optional. Address(0) means that no hook will be called
   *
   * @param _domani             Instance of the Domani to issue
   * @param _preIssueHook         Instance of the Manager Contract with the Pre-Issuance Hook function
   */
  function initialize(IDomani _domani, IManagerIssuanceHook _preIssueHook)
    external
    onlyDomaniManager(_domani, msg.sender)
    onlyValidAndPendingDomani(_domani)
  {
    managerIssuanceHook[_domani] = _preIssueHook;
   ...
   * have a way to redeem their Domani tokens
   */
  ...
  /**
   * Retrieves the addresses and units required to mint a particular quantity of Domani.
   *
   * @param _domani             Instance of the Domani to issue
   * @param _quantity             Quantity of Domani to issue
   * @return address[]            List of component addresses
   * @return uint256[]            List of component units required to issue the quantity of Domani tokens
   */
  function getRequiredComponentUnitsForIssue(IDomani _domani, uint256 _quantity)
    public
    view
    ...
    returns (address[] memory, uint256[] memory)
  {
    ...
    for (uint256 i = 0; i < components.length; i++) {
      require(!_domani.hasExternalPosition(components[i]), "Only default positions are supported");

      notionalUnits[i] = _domani
  ...
  function _callPreIssueHooks(
    ...
    IManagerIssuanceHook preIssueHook = managerIssuanceHook[_domani];
    if (address(preIssueHook) != address(0)) {
      preIssueHook.invokePreIssueHook(_domani, _quantity, _caller, _to);
