...
contract TesterStaking is Ownable {

    using LowGasSafeMath for uint256;
    using LowGasSafeMath for uint32;
    using SafeERC20 for IERC20;
    using SafeERC20 for IsTEST;

    IERC20 public immutable TEST;
    IsTEST public immutable stakedTester;
    ...
    constructor ( 
        address _Tester, 
        address _stakedTester, 
        uint32 _epochLength,
        uint _firstEpochNumber,
        uint32 _firstEpochTime
    ) {
        require( _Tester != address(0) );
        TEST = IERC20(_Tester);
        require( _stakedTester != address(0) );
        stakedTester = IsTEST(_stakedTester);
    ...
    function stake( uint _amount, address _recipient ) external returns ( bool ) {
        ...
        TEST.safeTransferFrom( msg.sender, address(this), _amount );
        ...
        warmupInfo[ _recipient ] = Claim ({
            ...
            gons: info.gons.add( stakedTester.gonsForBalance( _amount ) ),
            expiry: epoch.number.add( warmupPeriod ),
            lock: false
        });
        
        stakedTester.safeTransfer( address(warmupContract), _amount );
    ...
    function claim ( address _recipient ) external {
        ...
        if ( epoch.number >= info.expiry && info.expiry != 0 ) {
            ...
            uint256 amount = stakedTester.balanceForGons( info.gons );
    ...
    function forfeit() external {
        ...
        uint memoBalance = stakedTester.balanceForGons( info.gons );
        warmupContract.retrieve( address(this),  memoBalance);
        TEST.safeTransfer( msg.sender, info.deposit);
    ...
    function unstake( uint _amount, bool _trigger ) external {
        ...
        stakedTester.safeTransferFrom( msg.sender, address(this), _amount );
        TEST.safeTransfer( msg.sender, _amount );
    ...
    function rebase() public {
        if( epoch.endTime <= uint32(block.timestamp) ) {
            ...
            stakedTester.rebase( epoch.distribute, epoch.number );
            ...
            uint staked = stakedTester.circulatingSupply();
    ...
    function contractBalance() public view returns ( uint ) {
        return TEST.balanceOf( address(this) ).add( totalBonus );
    }
