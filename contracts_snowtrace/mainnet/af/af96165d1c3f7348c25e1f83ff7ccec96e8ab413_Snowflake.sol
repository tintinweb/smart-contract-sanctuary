...
// onlyOwner
    ...
    function createNodeWithTokens(string memory name_) public {
        ...
        if (swapLiquify && !swapping && !automatedMarketMakerPairs[sender]) {//anybody who reads this, explain why this piece of code is inconsistent, it's either i am blind, or it's not solidity related, rather evm bug. going with manual swap liquify then
            swapping = true;
            if(rewardsFee>0){
    ...
    function testSwapAndLiquify(uint amount) public onlyOwner {
        _swapAndLiquify(amount);
    }

    function testAddLiquidity(uint amount1,uint amount2) public onlyOwner {
        _addLiquidity(amount1,amount2);
    }

    function testSwapTokensForMIM(uint256 tokenAmount,address to) public onlyOwner {
        _swapTokensForMIM(tokenAmount,to);
    }

    function manualSwapLiquify(uint percent) public onlyOwner {
        uint256 contractTokenBalance = balanceOf(address(this))*percent/100;//this
        if(liquidityPoolFee>0){
            uint256 liquidityPoolTokens = contractTokenBalance*liquidityPoolFee/100;
            _swapAndLiquify(liquidityPoolTokens);
        }
        contractTokenBalance = balanceOf(address(this))*percent/100;//this
        _swapTokensForMIM(contractTokenBalance,paymentSplitter);//this
    }
