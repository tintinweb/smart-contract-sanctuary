...
// File contracts/pool/BoringChef.sol
...
contract BoringChef is Ownable {
    ...
    returns (uint256)
  {
    ...
    if (block.number > pool.lastRewardBlock && lpSupply != 0) {
      ...
      uint256 boringReward = multiplier
        .mul(boringPerBlock)
        .mul(pool.allocPoint)
        .div(totalAllocPoint);
    ...
    return user.amount.mul(accBoringPerShare).div(1e12).sub(user.rewardDebt);
  }
  ...
  function deposit(uint256 _pid, uint256 _amount) public {
    ...
    updatePool(_pid);
    if (user.amount > 0) {
      uint256 pending = user.amount.mul(pool.accBoringPerShare).div(1e12).sub(
        user.rewardDebt
      );
