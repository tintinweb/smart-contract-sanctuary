...
contract AirdropModule is ModuleBase, ReentrancyGuard {
   ...
   */
  modifier onlyValidCaller(IDomani _setToken) {
   ...
   */
  function batchAbsorb(IDomani _setToken, address[] memory _tokens)
    external
    nonReentrant
   ...
   */
  function absorb(IDomani _setToken, address _token)
    external
    nonReentrant
    onlyValidCaller(_setToken)
    onlyValidAndInitializedSet(_setToken)
   ...
   */
  function addAirdrop(IDomani _setToken, address _airdrop)
    external
   ...
   */
  function removeAirdrop(IDomani _setToken, address _airdrop)
    external
    onlyManagerAndValidSet(_setToken)
  {
   ...
   * @param _setToken                 Address of SetToken
   */
  function updateAnyoneAbsorb(IDomani _setToken) external onlyManagerAndValidSet(_setToken) {
   ...
   */
  function updateAirdropFee(IDomani _setToken, uint256 _newFee)
    external
    onlySetManager(_setToken, msg.sender)
    onlyValidAndInitializedSet(_setToken)
  {
   ...
   */
  function initialize(IDomani _setToken, AirdropSettings memory _airdropSettings)
    external
    onlySetManager(_setToken, msg.sender)
   ...
   */
  function getAirdrops(IDomani _setToken) external view returns (address[] memory) {
   ...
   */
  function isAirdropToken(IDomani _setToken, address _token) public view returns (bool) {
   ...
   */
  function _absorb(IDomani _setToken, address _token) internal {
    require(isAirdropToken(_setToken, _token), "Must be approved token.");

    _handleAirdropPosition(_setToken, _token);
  }

  function _batchAbsorb(IDomani _setToken, address[] memory _tokens) internal {
   ...
   */
  function _handleAirdropPosition(IDomani _setToken, address _token) internal {
   ...
   */
  function _isValidCaller(IDomani _setToken) internal view returns (bool) {
    return airdropSettings[_setToken].anyoneAbsorb || isSetManager(_setToken, msg.sender);
  }

  function _airdrops(IDomani _setToken) internal view returns (address[] memory) {
