...
contract TimeTreasury is Ownable {
    ...
    //    liquidityTokens.push( _OHMDAI );
        ...
        _autoliquify = __autoliquify;
    }

    function is_reserve_token( address _token ) external view returns ( bool ) {
        return isReserveToken[ _token ];
    }

    function is_liquidity_token( address _token ) external view returns ( bool ) {
        return isLiquidityToken[ _token ];
    }

    function transferFromAutoLiquidify( uint _amount, address _token ) external {
        require(msg.sender == address(_autoliquify), "only contract");
