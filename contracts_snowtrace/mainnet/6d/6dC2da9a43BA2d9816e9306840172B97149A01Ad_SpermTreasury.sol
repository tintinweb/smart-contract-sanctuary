...
contract SpermTreasury is Ownable {
    ...
    address public Fetus;
    uint public FetusQueue; // Delays change to Fetus address
    ...
    constructor (
        ...
        isReserveToken[ _wAVAX ] = true;
        reserveTokens.push( _wAVAX );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( Fetus ).balanceOf( msg.sender ); // Can only borrow against Fetus held
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        if ( _managing == MANAGING.RESERVEDEPOSITOR ) { // 0
            ...
            FetusQueue = uint32(block.timestamp).add32( secondsNeededForQueue );
        } else return false;
