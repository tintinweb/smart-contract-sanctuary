...
// File contracts/BIFI/interfaces/lendhub/IComptroller.sol
...
// File contracts/BIFI/interfaces/traderjoe/IXPool.sol
...
// File contracts/BIFI/strategies/TraderJoe/StrategyBankerJoeXJoe.sol



pragma solidity ^0.6.12;
//Lending Strategy
contract StrategyBankerJoeXJoe is StratManager, FeeManager {
    ...
    address constant public comptroller = 0xdc13687554205E5b89Ac783db14bb5bba4A1eDaC;
    ...
    constructor(
        ...
        require(_nativeToWantRoute[0] == native, "nativeToWantRoute[0] != native");
        require(_nativeToWantRoute[_nativeToWantRoute.length - 1] == output, "nativeToWantRoute[last] != output");
        nativeToWantRoute = _nativeToWantRoute;

        _giveAllowances();

        super.setWithdrawalFee(1);
    ...
    function chargeFees(address callFeeRecipient) internal {
        uint256 beforeNativeBal = IERC20(native).balanceOf(address(this));
        uint256 toNative = IERC20(output).balanceOf(address(this)).mul(45).div(1000);
        if (toNative > 0) {
            IUniswapRouter(unirouter).swapExactTokensForTokens(toNative, 0, outputToNativeRoute, address(this), now);
        }

        uint256 feeOutputBal = IERC20(native).balanceOf(address(this)).sub(beforeNativeBal);
        uint256 feeNativeBal = beforeNativeBal.mul(45).div(1000);
        uint256 feeBal = feeOutputBal.add(feeNativeBal);
    ...
    function rewardsAvailable() public payable returns (uint256, uint256) {
        IComptroller(comptroller).claimReward(0, address(this));
        IComptroller(comptroller).claimReward(1, address(this));
        uint256 _toWrap = address(this).balance;
        IWrappedNative(native).deposit{value: _toWrap}();
