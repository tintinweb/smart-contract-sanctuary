...
// Dependency file: contracts/interfaces/IJoeRouter02.sol
...
// Dependency file: contracts/baby/JewelPrinterDividendTracker.sol
...
// import "contracts/interfaces/IJoeRouter02.sol";
    ...
    DividendPayingTokenOptionalInterface
{
    ...
    function distributeJEWELDividends(uint256 amount) public onlyOwner {
...
contract JewelPrinterDividendTracker is OwnableUpgradeable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main JewelPrinter contract."
        );
    }
...
// Root file: contracts/baby/JewelPrinter.sol

pragma solidity =0.8.4;


contract JewelPrinter is ERC20, Ownable, BaseToken {
    using SafeMath for uint256;
    ...
    JewelPrinterDividendTracker public dividendTracker;
    ...
    constructor(
        ...
        dividendTracker = JewelPrinterDividendTracker(
        ...
        IJoeRouter02 _JoeRouter = IJoeRouter02(addrs[1]);
        ...
        dividendTracker.excludeFromDividends(address(_JoeRouter));
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "JewelPrinter: The dividend tracker already has that address"
        );

        JewelPrinterDividendTracker newDividendTracker = JewelPrinterDividendTracker(
            payable(newAddress)
        );

        require(
            newDividendTracker.owner() == address(this),
            "JewelPrinter: The new dividend tracker must be owned by the JewelPrinter token contract"
        );

        newDividendTracker.excludeFromDividends(address(newDividendTracker));
        newDividendTracker.excludeFromDividends(address(this));
        newDividendTracker.excludeFromDividends(owner());
        newDividendTracker.excludeFromDividends(address(JoeRouter));
    ...
    function updateJoeRouter(address newAddress) public onlyOwner {
        require(
            newAddress != address(JoeRouter),
            "JewelPrinter: The router already has that address"
        );
        emit UpdateJoeRouter(newAddress, address(JoeRouter));
        JoeRouter = IJoeRouter02(newAddress);
        address _JoePair = IJoeFactory(JoeRouter.factory())
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "JewelPrinter: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
        ...
        onlyOwner
    {
        require(
            pair != JoePair,
            "JewelPrinter: The PanJEWELSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "JewelPrinter: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "JewelPrinter: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "JewelPrinter: Cannot update gasForProcessing to same value"
        );
    ...
    function swapAndSendToFee(uint256 tokens) private {
        uint256 initialJEWELBalance = IERC20(rewardToken).balanceOf(
            address(this)
        );

        swapTokensForJEWEL(tokens);
    ...
    function swapAndLiquify(uint256 tokens) private {
        ...
        // swap tokens for ETH
        swapTokensForAVAX(half); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered
    ...
    function swapTokensForJEWEL(uint256 tokenAmount) private {
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        if (success) {
            dividendTracker.distributeJEWELDividends(dividends);
