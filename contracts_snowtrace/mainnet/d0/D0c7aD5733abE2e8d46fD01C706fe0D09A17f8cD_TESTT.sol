...
contract TESTT is Context, IERC20, Ownable {
    ...
    function cashoutReward(uint256 blocktime) public {
        ...
        if (cashoutFee > 0) {
            ...
            uint256 feeTokens = rewardAmount.mul(cashoutFee).div(10000);
            uint256 treasuryTokens = feeTokens.mul(75).div(10000);
            uint256 operationsTokens = feeTokens.mul(25).div(10000);
    ...
    function createNodeWithTokens(uint256 numberOfNodes) public {
        ...
        if (rewardsPoolFee > 0 || treasuryFee > 0 || smoothingReserveFee > 0 || operationPoolFee > 0) {
            ...
            rewardsPoolTokens = nodePrice.mul(rewardsPoolFee).div(10000);
            treasureTokens = nodePrice.mul(treasuryFee).div(10000);
            smoothingTokens = nodePrice.mul(smoothingReserveFee).div(10000);
            operationPoolTokens = nodePrice.mul(operationPoolFee).div(10000);
