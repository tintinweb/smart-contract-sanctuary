...
contract CakeVault is Ownable, Pausable {
    ...
    function calculateHarvestCakeRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingStorm(0, address(this));
    ...
    function withdraw(uint256 _shares) public notContract {
        ...
        if (bal < currentAmount) {
            ...
            IMasterChef(masterchef).withdraw(5,balWithdraw);
    ...
    function _earn() internal {
        uint256 bal = available();
        if (bal > 0) {
            IMasterChef(masterchef).deposit(5,bal,burnadd);
        }
    }
