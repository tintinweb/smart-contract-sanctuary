...
contract Trea is Ownable {
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( Time ).totalSupply().sub( totalDebt ) ).add( 1 ).mul( uimultiplier );
    }
	
	/**
        @notice returns multiplier rate
        @return uint
     */
    function eMultiplierRate() public view returns ( uint ) {
        return uimultiplier;
    }

    /**
        @notice sets multiplier rate
        @notice always consolidate to recognized reserves before audit
     */
    function seteMultiplierRate(uint _multiplier) external onlyManager() {
		uimultiplier = _multiplier;
	}
