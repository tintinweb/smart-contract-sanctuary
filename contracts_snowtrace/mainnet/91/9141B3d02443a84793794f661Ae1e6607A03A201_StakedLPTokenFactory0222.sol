...
// File: contracts\interfaces\IMasterChef0222.sol
...
// File: contracts\StakedLPToken0222.sol
...
contract StakedLPToken0222 is IUniswapV2Pair, PoolToken {
	using SafeToken for address;
	
	bool public constant isStakedLPToken = true;
	string public constant stakedLPTokenType = "0222";
	...
	function _update() internal {
		(uint256 _totalBalance,) = IMasterChef0222(masterChef).userInfo(pid, address(this));
	...
	function mint(address minter) external nonReentrant update returns (uint mintTokens) {
		...
		(uint256 _totalBalanceBefore,) = IMasterChef0222(masterChef).userInfo(pid, address(this));
		IMasterChef0222(masterChef).deposit(pid, mintAmount, false);
		(uint256 _totalBalanceAfter,) = IMasterChef0222(masterChef).userInfo(pid, address(this));
	...
	function redeem(address redeemer) external nonReentrant update returns (uint redeemAmount) {
		...
		_burn(address(this), redeemTokens);
		IMasterChef0222(masterChef).withdraw(pid, redeemAmount, false);
		_safeTransfer(redeemer, redeemAmount);
		emit Redeem(msg.sender, redeemer, redeemAmount, redeemTokens);		
	}
	...
	function _getReward() internal returns (uint256) {
		IMasterChef0222(masterChef).claim(pid);
	...
	function reinvest() external nonReentrant update {
		...
		IMasterChef0222(masterChef).deposit(pid, liquidity, false);
...
// File: contracts\StakedLPTokenFactory0222.sol
