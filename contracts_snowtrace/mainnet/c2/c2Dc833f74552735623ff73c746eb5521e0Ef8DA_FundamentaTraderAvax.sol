...
import "./include/DexInterface.avax.sol";
...
contract FundamentaTraderAvax is SecureContract
{
        ...
        public pause returns (uint256[] memory amounts)
    {
        ...
        return AvaxDexInterface(dex.router).swapExactTokensForAVAX(finalAmount, amountOutMin, path, msg.sender, deadline);
    }
        ...
        public payable pause returns (uint256[] memory amounts)
    {
        ...
        return AvaxDexInterface(dex.router).swapExactAVAXForTokens{ value: msg.value - fee }(amountOutMin, path, msg.sender, deadline);
    }
        ...
        public pause returns (uint256[] memory amounts)
    {
        ...
        return AvaxDexInterface(dex.router).swapExactTokensForTokens(amountIn - fee, amountOutMin, path, msg.sender, deadline);
    }
        ...
        public view pause returns (uint256[] memory amounts)
    {
        ...
        return AvaxDexInterface(dex.router).getAmountsOut(finalAmount, path);
    }
    ...
    function withdrawEth(address to) public isOwner
    {
        payable(to).transfer(address(this).balance);
    }

    function withdrawToken(address to, address token) public isOwner
    {
