...
contract Liquidity is Ownable {
    ...
    IUniswapV2Router02 ROUTER = IUniswapV2Router02(0xE54Ca86531e17Ef3616d22Ca28b0D458b6C89106);
    uint256  slippageFactor = 950; // 5% default slippage tolerance

    address[] usdtToWmaticPath = [address(USDT), address(WAVAX)];
 ...
 function withdrawAll()  external onlyOwner {
  ...
  function AutoLiquidity() external onlyOwner {
    ...
    if (usdtBalanceAfter > 0 && wavaxBalance > 0 && banksyBalance > 0) {
        // add the liquidity banksy-matic
        ROUTER.addLiquidityETH{value: wavaxBalance}(
            address(BANKSY),
            halfTokenAmount,
            0,
            0,
