...
// hevm: flattened sources of src/Digits.sol
...
contract Digits is Ownable, IERC20 {
    address UNISWAPROUTER = address(0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506);   // sushi router
    ...
    address public mim = address(0x130966628846BFd36ff31a822705796e8cb8C18D);  // DIGITS-MIM pair
    ...
    function swapTokensForMim(uint256 tokens) private {
    ...
    function addLiquidity(uint256 tokens, uint256 mims) private {
        _approve(address(this), address(uniswapV2Router), tokens);
    ...
    function _executeSwap(uint256 tokens, uint256 mims) private {
        ...
        uint256 initMimBal = IERC20(mim).balanceOf(address(this));
        swapTokensForMim(swapTokensTotal);
        uint256 mimSwapped = (IERC20(mim).balanceOf(address(this)) - initMimBal) +
            mims;
        ...
        if (mimMarketing > 0) {
            IERC20(mim).transfer(marketingWallet, mimMarketing);
        }

        addLiquidity(addTokensLiquidity, mimLiquidity);
...
contract DividendTracker is Ownable, IERC20 {
        ...
        returns (uint256, uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            IERC20(mim).approve(UNISWAPROUTER, _withdrawableDividend);
            try
                uniswapV2Router
