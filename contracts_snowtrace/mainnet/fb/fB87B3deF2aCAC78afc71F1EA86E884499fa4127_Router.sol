...
contract Router {
	...
	mapping(address => uint256) private poolShare; // currency (avax, usdc, etc.) => bps
	mapping(address => uint256) private oavaxShare; // currency => bps

	mapping(address => address) poolRewards; // currency => contract
	mapping(address => address) oavaxRewards; // currency => contract
	...
	function getOAVAXShare(address currency) external view returns(uint256) {
		return oavaxShare[currency];
	}
	...
	function getOAVAXRewards(address currency) external view returns(address) {
		return oavaxRewards[currency];
	}
	...
	function setOAVAXShare(address currency, uint256 share) external onlyOwner {
		oavaxShare[currency] = share;
	}
	...
	function setOAVAXRewards(address currency, address _contract) external onlyOwner {
		oavaxRewards[currency] = _contract;
	}
