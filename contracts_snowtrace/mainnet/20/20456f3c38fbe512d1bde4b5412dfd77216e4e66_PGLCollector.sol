...
// File: interfaces/Pangolin/IPangolinRouter.sol
...
// File: interfaces/Pangolin/IPangolinPair.sol
...
// File: contracts/PGLCollector.sol
...
contract PGLCollector is Ownable {

    using SafeERC20 for IERC20;

    address public constant pangolinRouterAddress =
    ...
    function _pullLiquidity(IPangolinPair pair, uint256 balance) internal {
        pair.approve(pangolinRouterAddress, balance);
        IPangolinRouter(pangolinRouterAddress).removeLiquidity(
            pair.token0(),
            pair.token1(),
            balance,
            0,
    ...
    internal {
        ...
        IERC20(token).safeApprove(pangolinRouterAddress, 0);
        IERC20(token).safeApprove(pangolinRouterAddress, amount);
        IPangolinRouter(pangolinRouterAddress).swapExactTokensForTokens(
            amount,
            0,
            path,
            address(this),
    ...
    function collectFees(address[] memory liquidityPairs,
        address outputToken) public {
        require(liquidityPairs.length > 0, "No pairs specified");
        ...
        for (uint256 i = 0; i < liquidityPairs.length; i++) {
            ...
            uint256 pglBalance = currentPair.balanceOf(msg.sender);
            if (pglBalance > 0) {
                currentPair.transferFrom(msg.sender, address(this), pglBalance);
        ...
        if (finalBalance > 0) {
            IERC20(outputToken).safeTransfer(msg.sender, finalBalance);
        }
    }

    /// @notice Allow to withdraw ERC-20 tokens received / stuck by mistake
    /// @param _asset - address of the token to retrieve
