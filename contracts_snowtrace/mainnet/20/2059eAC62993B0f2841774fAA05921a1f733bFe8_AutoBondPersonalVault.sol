...
contract AutoBondPersonalVault {
        ...
        returns (uint256)
    {
        ...
        // bond with MIM
        require(mim.balanceOf(address(this)) <= 0, "Not enough tokens");
        mim.approve(address(mimBondDepository), results[1]);
        // hardcoded 0.5% slippage
        uint256 maxPrice = (mimBondDepository.bondPrice() * 9950) / 10000;
    ...
    /**
     *  @notice Anybody can top up the vault, but only depositor will benefit
     */
    function deposit(uint256 _amount) external {
        ...
        stakedAsset.safeTransferFrom(depositor, address(this), _amount);
    }
    ...
    function stakeAssets(uint256 _amount) public only(vaultManager) {
        require(asset.balanceOf(msg.sender) >= _amount, "Not enough tokens");
