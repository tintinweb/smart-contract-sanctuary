...
contract zVault is ERC20, ERC20Detailed, Ownable {
    ...
    address public platformAddress = 0x254e34FD8DC5ca1752944DF0D89261809C225F9D;
    uint public fee = 2000000;
    
    address public governance;
    address public controller;
    
    constructor (address _token,uint _earnLowerlimit ,address _controller) public ERC20Detailed(
        string(abi.encodePacked("time ", ERC20Detailed(_token).name())),
    ...
    function setPlatformAddress(address _platform) onlyOwner external{
        platformAddress = _platform;
    }
    
    function balance() public view returns (uint) {
        return token.balanceOf(address(this))
                .add(Controller(controller).balanceOf(address(token), msg.sender));
    }
    ...
    function withdraw(uint _shares) public {
        ...
        uint feeamount = r.mul(fee).div(10 ** 9);
        uint balance = r.sub(feeamount);
        token.safeTransfer(msg.sender, balance);
        token.safeTransfer(platformAddress, feeamount);
    }
