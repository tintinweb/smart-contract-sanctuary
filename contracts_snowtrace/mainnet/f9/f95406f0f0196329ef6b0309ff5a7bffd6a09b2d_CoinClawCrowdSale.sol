...
contract CoinClawCrowdSale is ICrowdSale, Ownable, ReentrancyGuard{
	 ...
	 */
	function setStarts(uint32 starts) external override{
	 ...
	 */
	function setEnds(uint32 ends) external override{
	 ...
	 */
	function setPrice(uint256 price) external override{
	 ...
	 */
	function setLimit(uint256 limit) public override{
		require(msg.sender == owner(), "Sales: E07: Authorization denied.");
	...
	*/
	...
	/*
	* Sales contract can withdraw unscheduled amount of redundantly deposited balances from vesting contract.
	*/
	function disburse(uint256 amount) external onlyOwner returns(uint256){
		return _vesting.disburse(amount);
	}
	/*---------------------------------------- VESTING -----------------------------------------*/
}