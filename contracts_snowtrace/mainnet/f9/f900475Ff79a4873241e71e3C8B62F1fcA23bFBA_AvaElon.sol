...
contract AvaElon is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("AvaElon", "AvaElon") {
        ...
        uint256 totalSupply = 300 * 1e6 * 1e18;

        maxTransactionAmount = totalSupply * 5 / 1000; // 0.5% maxTransactionAmount
        ...
        marketingWallet = address(0x62d16C07183c94fdbAe0CfBBD59af67c085a9973); // set as marketing wallet
        buyBackWallet = address(0x9Ed2d0Fb6a8174F80575f9178e839B338644A418); // set as buyback wallet
    ...
    function swapBack() private {
        ...
        uint256 avaxForMarketing = avaxBalance.mul(tokensForMarketing).div(totalTokensToSwap);
        uint256 avaxForBuyBack = avaxBalance.mul(tokensForBuyBack).div(totalTokensToSwap);


        uint256 avaxForLiquidity = avaxBalance - avaxForMarketing - avaxForBuyBack;
        ...
        (bool success,) = address(marketingWallet).call{value: avaxForMarketing}("");
    ...
    // useful for buybacks or to reclaim any AVAX on the contract in a way that helps holders.
    function buyBackTokens(uint256 bnbAmountInWei) external onlyOwner {
        // generate the pair path of wavax -> avax
        address[] memory path = new address[](2);
        path[0] = router.WAVAX();
        path[1] = address(this);

        // make the swap
        router.swapExactAVAXForTokensSupportingFeeOnTransferTokens{value: bnbAmountInWei}(
