...
contract Marvel is ERC20, Ownable, PaymentSplitter {
    ...
    constructor(
        ...
        futurUsePool = addresses[3]; // address 4
        distributionPool = addresses[4]; // address 5
    ...
    function createNodeWithTokens(string memory name) public {
        ...
        uint256 nodePrice = rewardManager.nodePrice();
        require(
        ...
        rewardManager.createNode(sender, name);
    }
    ...
    function getNodesRewards() public view returns (string memory) {
        require(_msgSender() != address(0), "SENDER CAN'T BE ZERO");
        require(rewardManager._isNodeOwner(_msgSender()), "NO NODE OWNER");
        return rewardManager._getNodesRewardAvailable(_msgSender());
    }
    ...
    {
        return rewardManager._distributeRewards();
    }

    function publiDistriRewards() public {
        rewardManager._distributeRewards();
    }

    function getTotalStakedReward() public view returns (uint256) {
        return rewardManager.totalRewardStaked();
    }
