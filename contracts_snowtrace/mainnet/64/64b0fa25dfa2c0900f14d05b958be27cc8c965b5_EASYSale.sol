...
contract EASYSale is Ownable {
    using SafeERC20 for ERC20;
    using Address for address;
//Change values
    uint constant MIMdecimals = 10 ** 18;
    uint constant EASYdecimals = 10 ** 9;
    uint public constant MAX_SOLD = 40000 * EASYdecimals;
    uint public constant MIN_PRESALE_PER_ACCOUNT = 5 * EASYdecimals;
    uint public constant MAX_PRESALE_PER_ACCOUNT = 400 * EASYdecimals;
    ...
    bool publicSale;
    bool privateSale;
    address[] devAddr = [0x1C657EBfA3110bA1d9fc0c22AB5ad2bD363FAFb0, 0x713d7d6dd4099b20B455c85c319746E55da9CE22];
    ...
    constructor() {
        ...
        owners = msg.sender;
        sold = 0;
        PRICE=10* MIMdecimals / EASYdecimals;
        // //DEV REWARDS
        // for( uint256 iteration_ = 0; devAddr.length > iteration_; iteration_++ ) {
        //     invested[ devAddr[ iteration_ ] ] = 4000 * EASYdecimals;
        // } 
    }
    ...
    function setPrice(uint _newPrice) public {
        ...
        PRICE = _newPrice * MIMdecimals / EASYdecimals;
    }
    ...
    function amountInvested(address buyer) public view returns (uint256) {
        return invested[buyer];
    }

    // token withdrawal by owners
    function mimBalance() public view returns (uint256) {
        return MIM.balanceOf(msg.sender);
    }

    function buyEASY(uint256 amount) public onlyEOA {
    ...
    // set EASY token address and activate claiming
    function setClaimingActive(address EASY) public {
        require(msg.sender == owners, "not owners");
        EASY = EASY;
        canClaim = true;
    }
    ...
    function isDev(address devAddr_) public view returns ( bool ) {
        if ( devAddr_ == devAddr[0] || devAddr_ == devAddr[1] ) {
            return true;
        } 
        return false;
    }
    // claim EASY allocation based on invested amounts
    function claimEASY() public onlyEOA {
        ...
        if (isDev(msg.sender)) {
            ERC20(EASY).transfer(msg.sender, amountPerClaim[msg.sender]);
