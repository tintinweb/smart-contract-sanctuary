...
contract Treasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SROCKET }
    ...
    uint public sROCKETQueue; // Delays change to sROCKET address
    ...
    /**
        @notice allow approved address to deposit an asset for ROCKET
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint ROCKET needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( ROCKET ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn ROCKET for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IROCKETERC20( ROCKET ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sROCKET ).balanceOf( msg.sender ); // Can only borrow against sROCKET held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with ROCKET
        @param _amount uint
     */
    function repayDebtWithROCKET( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IROCKETERC20( ROCKET ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, ROCKET, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( ROCKET ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( ROCKET ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns ROCKET valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match ROCKET decimals
            value_ = _amount.mul( 10 ** IERC20( ROCKET ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SROCKET ) { // 9
            sROCKETQueue = block.number.add( blocksNeededForQueue );
        } else return false;
