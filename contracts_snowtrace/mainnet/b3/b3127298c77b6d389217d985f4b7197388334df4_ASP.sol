...
contract GlobalsAndUtility is ERC20 {
    ...
    */
    ...
    /* ASP allocation share address */
    address payable internal constant ASP_SHARE_ADDR = payable(0x5F951B74e671a675320a60ebCE8cb970c326B47a);

    uint8 internal LAST_FLUSHED_DAY = 1;

    /* ERC20 constants */
    string public constant name = "ASP";
    ...
    uint256 internal constant LAUNCH_TIME = 1645315200;
    ...
    uint256 internal constant BPB_MAX_SUNS = BPB_MAX_E2X * SUNS_PER_ASP;
        ...
        external
    {
        ...
        require(g._currentDay > CLAIM_PHASE_START_DAY, "ASP: Too early");

        if (beforeDay != 0) {
            require(beforeDay <= g._currentDay, "ASP: beforeDay cannot be in the future");
     ...
     * @return _dayStakeSharesTotal array of day stake shares total
     * @return _dayPayoutTotal array of day payout total
     * @return _dayDividends array of day dividends
     */
    ...
    {
        require(beginDay < endDay && endDay <= globals.dailyDataCount, "ASP: range invalid");
        ...
        view
    {
        ...
        require(stakeIdParam == stRef.stakeId, "ASP: stakeIdParam not in stake");
...
contract StakeableToken is GlobalsAndUtility {
        ...
        external
    {
        ...
        require(newStakedDays >= MIN_STAKE_DAYS, "ASP: newStakedDays lower than minimum");
        ...
        external
    {
        ...
        require(stakeLists[stakerAddr].length != 0, "ASP: Empty stake list");
        require(stakeIndex < stakeLists[stakerAddr].length, "ASP: stakeIndex invalid");
        ...
        require(g._currentDay >= st._lockedDay + st._stakedDays, "ASP: Stake not fully served");

        /* Stake must still be locked */
        require(st._unlockedDay == 0, "ASP: Stake already unlocked");
        ...
        external
    {
        ...
        /* require() is more informative than the default assert() */
        require(stakeListRef.length != 0, "ASP: Empty stake list");
        require(stakeIndex < stakeListRef.length, "ASP: stakeIndex invalid");
        ...
        internal
    {
        ...
        require(newStakedDays <= MAX_STAKE_DAYS, "ASP: newStakedDays higher than maximum");
        ...
        require(newStakeShares != 0, "ASP: newStakedSuns must be at least minimum shareRate");
        ...
        returns (uint256 bonusSuns)
    {
        /*
            ...
            extraDaysMax    =  longerBonusMax% * 1820
                            ...
                            =  LPB_MAX_DAYS
            BIGGER PAYS BETTER:
            Bonus percentage scaled 0% to 10% for the first 7M ASP of stake.
...
contract TransformableToken is StakeableToken {
        ...
        payable
    {
        ...
        require(rawAmount != 0, "ASP: Amount required");
    ...
    function xfLobbyExit(uint256 enterDay, uint256 count)
        external
    {
        require(enterDay < _currentDay(), "ASP: Round is not complete");
        ...
        if (count != 0) {
            require(count <= qRef.tailIndex - headIndex, "ASP: count invalid");
            endIndex = headIndex + count;
        } else {
            endIndex = qRef.tailIndex;
            require(headIndex < endIndex, "ASP: count invalid");
        }
        ...
        external
    {
        ...
        require(address(this).balance != 0, "ASP: No value");

        require(LAST_FLUSHED_DAY < _currentDay(), "ASP: Invalid day");

        _dailyDataUpdateAuto(g);

        ASP_SHARE_ADDR.transfer((dailyData[LAST_FLUSHED_DAY].dayDividends * 100) / 100);
     ...
     * @param enterDay day entered
     * @param entryIndex index of entry
     * @return rawAmount uint Raw amount that was entered with
     * @return referrerAddr Referring TRX addr (optional; 0x0 for no referrer)
     */
        ...
        returns (uint256 rawAmount, address referrerAddr)
    {
        ...
        require(entry.rawAmount != 0, "ASP: Param invalid");
    ...
    {
        /* 1342465753424 = ~ 4900000 * SUNS_PER_ASP / 365 */
...
contract ASP is TransformableToken {
    constructor()
    {
