...
/*
    ...
    E5: USDT amount must be greater than 0 
    E6: There is not enough USDT
...
contract SaleNFT is ReentrancyGuard, Ownable, IERC721Receiver, ERC721Holder {
    ...
    IERC721Enumerable public banksyNFT = IERC721Enumerable(0x99B3A7886C177020e8C4698fC9494f8Dbf64A3A7); 

    uint256 public nftCardPerAccountMaxTotal = 9; // 9 cards
    
    uint256 public salePrice = 15 * (10 ** 6); // 15 usdt
    ...
    event BuyNftCard(address sender, uint256 usdtSpent, uint256 nftCardReceived);
    ...
    function buyNftCard(uint256 _amountNFT) external nonReentrant {
        ...
        uint256 _usdtSpend = (_amountNFT * salePrice);

        require(_usdtSpend > 0, "E5");
        require(_usdtSpend <= USDT.balanceOf(_msgSender()), "E6");
        ...
        if (_balanceOfNFT > 0) {
              ...
              // send usdt to sale address
              USDT.safeTransferFrom(_msgSender(), SALE_ADDRESS, _usdtSpend);
        }
        
        emit BuyNftCard(_msgSender(), _amountNFT, _usdtSpend);
