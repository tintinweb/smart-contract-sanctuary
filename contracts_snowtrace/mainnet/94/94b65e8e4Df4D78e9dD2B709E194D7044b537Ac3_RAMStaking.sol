...
contract RAMStaking is Ownable {
    using LowGasSafeMath for uint256;
        ...
        returns (bool)
    {
        ...
        RAM.safeTransferFrom(msg.sender, address(this), _amount);
        ...
        warmupInfo[_recipient] = Claim({
            ...
            gons: info.gons.add(ROM.gonsForBalance(_amount)),
            expiry: epoch.number.add(warmupPeriod),
            lock: false
        });

        ROM.safeTransfer(address(warmupContract), _amount);
    ...
    function claim(address _recipient) external {
        ...
        if (epoch.number >= info.expiry && info.expiry != 0) {
            ...
            uint256 amount = ROM.balanceForGons(info.gons);
    ...
    function forfeit() external {
        ...
        uint256 romBalance = ROM.balanceForGons(info.gons);
        warmupContract.retrieve(address(this), romBalance);
        RAM.safeTransfer(msg.sender, info.deposit);
        emit LogForfeit(msg.sender, romBalance, info.deposit);
    }
    ...
    function unstake(uint256 _amount, bool _trigger) external {
        ...
        RAM.safeTransfer(msg.sender, _amount);
    ...
    function rebase() public {
        if (epoch.endTime <= uint32(block.timestamp)) {
            ROM.rebase(epoch.distribute, epoch.number);
            ...
            uint256 staked = ROM.circulatingSupply();
    ...
    function contractBalance() public view returns (uint256) {
        return RAM.balanceOf(address(this)).add(totalBonus);
    }
