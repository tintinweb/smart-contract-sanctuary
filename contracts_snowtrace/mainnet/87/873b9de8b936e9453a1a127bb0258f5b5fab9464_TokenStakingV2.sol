...
interface ITokenStaking {
    ...
    event totalTokenLockedUpdated(uint totalTokenLocked);
    event totalTokenUnlockedUpdated(uint totalTokenUnlocked);
    event LockChanged(address indexed staker, uint newLocked);
    event UnlockChanged(address indexed staker, uint newUnlocked);
    event UnlockTimeChanged(address indexed staker, uint newUnlockTime);
    event BonusMultiplierChanged(uint newBonusMultiplier);
    event UnlockPeriodChanged(uint newUnlockPeriod);
    event VETokenChanged(address newVeTokenAddress);
...
// File: contracts/protocol/global/TokenStakingV2.sol
