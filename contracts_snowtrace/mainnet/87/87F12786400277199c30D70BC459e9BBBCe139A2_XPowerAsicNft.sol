 ...
 */
contract XPowerNft is ERC1155, ERC1155Burnable, ERC1155Supply, URIMalleable {
    ...
    constructor(string memory uri, address xpower) ERC1155(uri) {
        _xpower = ERC20Burnable(xpower);
    }

    /** create `amount` tokens of token type `kind`, and assign` them to `to` */
    function mint(
        address to,
        uint256 kind,
        uint256 amount
        ...
        _mint(to, idBy(year(), kind), amount, "");
    }
    ...
    function mintBatch(
        ...
        for (uint256 i = 0; i < kinds.length; i++) {
            require(kinds[i] % 3 == 0, "non-ternary kinds");
            uint256 delta = amounts[i] * (10**kinds[i]);
            require(delta > 0, "non-positive amounts");
            xpow += delta;
        }
        _xpower.burnFrom(msg.sender, xpow);
        _mintBatch(to, idsBy(year(), kinds), amounts, "");
    }

    /** @return current number of years since anno domini */
    function year() public view returns (uint256) {
        uint256 y = block.timestamp / (365 days);
...
contract XPowerCpuNft is XPowerNft {
    constructor(string memory uri, address xpower) XPowerNft(uri, xpower) {}
}
