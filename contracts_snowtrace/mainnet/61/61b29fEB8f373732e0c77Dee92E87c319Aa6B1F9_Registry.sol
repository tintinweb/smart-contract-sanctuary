...
contract Registry is SYSTEM, ERC721("PR0XY Registry", "PR0XY ID") {
  ...
  constructor(Admin pr0xyAdmin_) SYSTEM(pr0xyAdmin_) {}
  ...
  mapping(bytes2 => address) public walletOf;
  mapping(address => bytes2) public memberIdFor;
  mapping(address => uint256) public repBudgetOf;
  mapping(address => uint256) public repScoreFor;
  mapping(address => uint256) public uniqueRepsFor;
  ...
  event IdAssigned(address, bytes2);

  // @@@ Check that the address hash cannot be bytes2(0)
  function assignId(address wallet_) external pure returns (bytes2) {

    // // 1. Take the first two bytes (4 hex characters) of a hash of the wallet
    // bytes2 userId = bytes2(keccak256(abi.encode(wallet_ + block.timestamp)));

    // // 2. Keep hashing until a unique Id is found (due to potential collisions)
    // while (walletOf[userId] != address(0)) {
    //   userId = keccak256(abi.encode(addressHash));
    //   userId = bytes2(addressHash);
    // }

    // // 3. Save the userId in the DAO
    // walletOfId[userId] = wallet_;
    ...
    // return userId;
    return bytes2(keccak256(abi.encode(wallet_)));
  }


  //
  function increaseRepBudget(address member_, uint256 amount_) external onlyPolicy {

    //
    repBudgetOf[member_] += amount_;
  }


  //
  function incrementUniqueReps(address member_) external onlyPolicy {
    
    //
    uniqueRepsFor[member_]++;
  }
  

  //
  function giveReputation(address from_, address to_, uint256 amount_) external onlyPolicy {
    require (repBudgetOf[from_] > amount_, "cannot giveReputation(): giver does not have enough in their budget");

    //
    repBudgetOf[from_] -= amount_;
    repScoreFor[to_] += amount_;
  }
}
