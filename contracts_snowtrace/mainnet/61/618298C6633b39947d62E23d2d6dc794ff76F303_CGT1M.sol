...
contract CGT1M is ERC721Enumerable, AccessControl, EIP712Base, ERC2981ContractWideRoyalties {

    string _baseTokenURI = "https://api.chainguardians.io/api/opensea/";
   ...
   function supportsInterface(bytes4 interfaceId) public view virtual override(ERC721Enumerable, AccessControl, ERC2981Base) returns (bool) {
    ...
    function setBaseURI(string memory baseURI) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "CGT1M: Caller is not a admin");

        _baseTokenURI = baseURI;
    }

    function setTokenRange(uint256 _tokenRangeStart, uint256 _tokenRangeEnd) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "CGT1M: Caller is not a admin");
 
        tokenRangeStart = _tokenRangeStart;
        tokenRangeEnd = _tokenRangeEnd;
    }
    ...
    function updateAttributes(uint256 _tokenId, uint256 _attributes) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "CGT1M: Caller is not a owner");
        require(_exists(_tokenId), "CGT1M: Token does not exist");

        tokenIds_attributes[_tokenId] = _attributes;
    }

    function mint(address _user, uint256 _tokenId, uint256 _attributes) public {
        require(hasRole(MINTER_ROLE, msg.sender), "CGT1M: Caller is not a minter");
        require(_user != address(0), "CGT1M: mint to the zero address");
        require(!_exists(_tokenId), "CGT1M: ERC721: token already minted");
        require(_tokenId>=tokenRangeStart && _tokenId<=tokenRangeEnd, "CGT1M: Token id must be in range");
