...
contract CypherBondDepository is Ownable {
    ...
    /* ======== STATE VARIABLES ======== */
    IERC20 public immutable CYPHER; // token given as payment for bond
    ...
    constructor ( 
        ...
        require( _CYPHER != address(0) );
        CYPHER = IERC20(_CYPHER);
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            CYPHER.transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                CYPHER.approve( address(stakingHelper), _amount );
                stakingHelper.stake( _amount, _recipient );
            } else {
                CYPHER.approve( address(staking), _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return CYPHER.totalSupply().mul( terms.maxPayout )/ 100000;
    }
    ...
    function debtRatio() public view returns ( uint debtRatio_ ) {   
        uint supply = CYPHER.totalSupply();
