...
interface IPriceCalculator {
    ...
    function priceOfAVAX() view external returns (uint);
    function priceOfMaximus() view external returns (uint);
...
// File: contracts/interfaces/ILockedStrategy.sol
...
// File: contracts/library/SafeDecimal.sol
...
// File: contracts/dashboard/Dashboard.sol
...
contract Dashboard is OwnableUpgradeable {
    ...
    IPriceCalculator public constant priceCalculator = IPriceCalculator(0xeABe3A541058f6894C97Ac11c257C7A3755f1741);

    address public constant WAVAX = 0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7;
    address public constant MAXIMUS = 0x885d748C00A279B67A7749EC6b03301700dd0455;

    IMaximusChef private constant maximusChef = IMaximusChef(0x6a2709889544Fa7a19d099b5F3D07684aF490920);
    ...
    function calculateProfit(address pool, address account) public view returns (uint profit, uint profitInAVAX) {
        PoolConstant.PoolTypes poolType = poolTypes[pool];
        profit = 0;
        profitInAVAX = 0;

        if (poolType == PoolConstant.PoolTypes.Maximus) {
            // profit as avax
            IStrategy strategy = IStrategy(pool);
            profit = strategy.earned(account);
            (profitInAVAX,) = priceCalculator.valueOfAsset(strategy.rewardsToken(), profit);
        }
        if (poolType == PoolConstant.PoolTypes.MaximusAVAX) {
            // profit as maximus
            profit = maximusChef.pendingMaximus(pool, account);
            (profitInAVAX,) = priceCalculator.valueOfAsset(MAXIMUS, profit);
        }
        else if (poolType == PoolConstant.PoolTypes.FlipToFlip || poolType == PoolConstant.PoolTypes.MaximusToMaximus || poolType == PoolConstant.PoolTypes.TokenStake) {
            // profit as underlying
            IStrategy strategy = IStrategy(pool);
            profit = strategy.earned(account);
            (profitInAVAX,) = priceCalculator.valueOfAsset(strategy.stakingToken(), profit);
        }
        else if (poolType == PoolConstant.PoolTypes.FlipToToken) {
            // maximizer
            IStrategy strategy = IStrategy(pool);
            profit = strategy.earned(account).mul(IStrategy(strategy.rewardsToken()).priceShare()).div(1e18);
            (profitInAVAX,) = priceCalculator.valueOfAsset(IStrategy(strategy.rewardsToken()).stakingToken(), profit);
        }

    }

    function profitOfPool(address pool, address account) public view returns (uint profit, uint maximus) {
        (uint profitCalculated, uint profitInAVAX) = calculateProfit(pool, account);
        ...
        if (!perfExemptions[pool]) {
            ...
            if (strategy.minter() != address(0)) {
                ...
                maximus = IMaximusMinter(strategy.minter()).amountMaximusToMint(profitInAVAX.mul(30).div(100));
            }

            if (strategy.maximusChef() != address(0)) {
                maximus = maximus.add(maximusChef.pendingMaximus(pool, account));
            }
        }
    }
    ...
    function infoOfPool(address pool, address account) public view returns (PoolConstant.PoolInfo memory) {
        ...
        IStrategy strategy = IStrategy(pool);
        (uint pBASE, uint pMAXIMUS) = profitOfPool(pool, account);
        ...
        poolInfo.pMAXIMUS = pMAXIMUS;

        PoolConstant.PoolTypes poolType = poolTypeOf(pool);
        if (poolType != PoolConstant.PoolTypes.Maximus && strategy.minter() != address(0)) {
            IMaximusMinter minter = IMaximusMinter(strategy.minter());
            poolInfo.depositedAt = strategy.depositedAt(account);
            poolInfo.feeDuration = minter.WITHDRAWAL_FEE_PERIOD();
    ...
    function withdrawableOf(address pool, address account) public view returns (uint) {
        ...
        if (poolType == PoolConstant.PoolTypes.MaximusToMaximus
            || poolType == PoolConstant.PoolTypes.MaximusAVAX) {
    ...
    function portfolioOfPoolInUSD(address pool, address account) internal view returns (uint) {
        ...
        (, uint profitInAVAX) = calculateProfit(pool, account);
        uint profitInMAXIMUS = 0;

        if (!perfExemptions[pool]) {
            IStrategy strategy = IStrategy(pool);
            if (strategy.minter() != address(0)) {
                profitInAVAX = profitInAVAX.mul(70).div(100);
                profitInMAXIMUS = IMaximusMinter(strategy.minter()).amountMaximusToMint(profitInAVAX.mul(30).div(100));
            }

            if ((poolTypes[pool] == PoolConstant.PoolTypes.MaximusAVAX || poolTypes[pool] == PoolConstant.PoolTypes.FlipToFlip)
                && strategy.maximusChef() != address(0)) {
                profitInMAXIMUS = profitInMAXIMUS.add(maximusChef.pendingMaximus(pool, account));
            }
        }

        (, uint profitAVAXInUSD) = priceCalculator.valueOfAsset(WAVAX, profitInAVAX);
        (, uint profitMAXIMUSInUSD) = priceCalculator.valueOfAsset(MAXIMUS, profitInMAXIMUS);
        return tokenInUSD.add(profitAVAXInUSD).add(profitMAXIMUSInUSD);
    }
