...
contract TWAPOracleUpdater is ERC20Permit, VaultOwned {
    ...
    function addTWAPSource(address newTWAPSourceDexPool_) external onlyOwner {
        require(_dexPoolsTWAPSources.add(newTWAPSourceDexPool_), "PULSEToken: TWAP Source already stored.");
        emit TWAPSourceAdded(newTWAPSourceDexPool_);
    }

    function removeTWAPSource(address twapSourceToRemove_) external onlyOwner {
        require(_dexPoolsTWAPSources.remove(twapSourceToRemove_), "PULSEToken: TWAP source not present.");
        emit TWAPSourceRemoved(twapSourceToRemove_);
    }

    function updateTWAPOracle(address dexPoolToUpdateFrom_, uint256 twapEpochPeriodToUpdate_) internal {
    ...
    function _beforeTokenTransfer(
        ...
        if (_dexPoolsTWAPSources.contains(from_)) {
            updateTWAPOracle(from_, twapEpochPeriod);
        } else {
            if (_dexPoolsTWAPSources.contains(to_)) {
                updateTWAPOracle(to_, twapEpochPeriod);
            }
        }
    }
}
...
contract PULSEToken is Divine {
    using SafeMath for uint256;

    constructor() Divine("Pulse", "PULSE", 9) {}
