...
contract TokenConverter {
    ...
    constructor(IRouter _defaultRouter) {
        DEFAULT_ROUTER = _defaultRouter; // 0x10ED43C718714eb63d5aA57B78B54704E256024E
        DEFAULT_FACTORY = IFactory(DEFAULT_ROUTER.factory());
        WETH = DEFAULT_ROUTER.WAVAX();
    }
    ...
    function convertChainedUniversal(address[] memory _tokens, uint256 _amount)
        public
        view
        returns (uint256 amt)
    {
    ...
    function convertChainedByFactory(
        ...
        for (uint256 i = 0; i < _tokens.length - 1; i++)
            amt = convertTwoByFactory(_factory, _tokens[i], _tokens[i + 1], amt);
    }
    ...
    function convertChainedUniversalByFactory(
        ...
        if (_tokens.length < 2) return amt;
        for (uint256 i = 0; i < _tokens.length - 1; i++)
            amt = convertTwoUniversalByFactory(_factory, _tokens[i], _tokens[i + 1], amt);
    }
