...
contract HcToken is HcSwapAvaxERC20 {
    ...
    modifier onlyOwner(){
        require(msg.sender == owner, string(abi.encodePacked("HcToken a", name, ":ONLY_OWNER")));
        _;
    }

    modifier onlyMinter(){
        require(msg.sender == owner || minter[msg.sender] == true, string(abi.encodePacked("HcToken a", name, ":ONLY_MINTER")));
        _;
    }
