...
contract Treasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SPAPA }
    ...
    address public sPAPA;
    uint public sPAPAQueue; // Delays change to sPAPA address
    ...
    /**
        @notice allow approved address to deposit an asset for PAPA
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint PAPA needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( PAPA ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn PAPA for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IPAPAERC20( PAPA ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sPAPA ).balanceOf( msg.sender ); // Can only borrow against sPAPA held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with PAPA
        @param _amount uint
     */
    function repayDebtWithPAPA( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IPAPAERC20( PAPA ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, PAPA, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( PAPA ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( PAPA ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns PAPA valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match PAPA decimals
            value_ = _amount.mul( 10 ** IERC20( PAPA ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SPAPA ) { // 9
            sPAPAQueue = block.number.add( blocksNeededForQueue );
        } else return false;
