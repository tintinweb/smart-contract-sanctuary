...
contract MagikFaucet is Context{
    ...
    address public Usdc;
    uint256 public MaxAmount = 5.55 *10**18;
...
constructor (){
        ...
        ownerOfContract = _msgSender();
 
    
}
    modifier onlyOwner() {
        require(ownerOfContract == _msgSender(), "Requires owner");
        _;      
    }
    ...
    onlyOwner()
{
    ...
    function setMaxAmount(uint256 amount)
    public
    onlyOwner()
{
   MaxAmount=amount;
    ...
    function magikFaucet(address _token, uint256 amount) public {
        uint256 x2 = amount*2;
        require(MaxAmount >= amount,"Exceeds maxAmount" );
        ...
        if(IERC20(_token).balanceOf(address(this))>=x2){
        IERC20(_token).safeTransfer(msg.sender,x2 );}
        else{
    ...
    /** Gets the address to which tokens can be recovered from the LuxRune contract - the owner wallet. */
