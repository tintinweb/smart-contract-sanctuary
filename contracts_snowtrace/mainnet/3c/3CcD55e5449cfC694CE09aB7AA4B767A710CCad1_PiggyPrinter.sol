 ...
 * instead returning `false` on failure. This behavior is nonAVAXeless
...
contract ERC20 is Context, IERC20, IERC20Metadata {
     ...
     * AVAXer and Wei. This is the value {ERC20} uses, unless this function is
     * overridden;
     *
...
/**
 * @dev https://eips.AVAXereum.org/EIPS/eip-1167[EIP 1167] is a standard for
...
// Dependency file: contracts/baby/PiggyPrinterDividendTracker.sol
...
interface DividendPayingTokenInterface {
    ...
    /// @notice Withdraws the AVAXer distributed to the sender.
    /// @dev SHOULD transfer `dividendOf(msg.sender)` wei to `msg.sender`, and `dividendOf(msg.sender)` SHOULD be 0 after the transfer.
    ///  MUST emit a `DividendWithdrawn` event if the amount of AVAXer transferred is greater than 0.
    function withdrawDividend() external;

    /// @dev This event MUST emit when AVAXer is distributed to token holders.
    /// @param from The address which sends AVAXer to this contract.
    /// @param weiAmount The amount of distributed AVAXer in wei.
    event DividendsDistributed(address indexed from, uint256 weiAmount);

    /// @dev This event MUST emit when an address withdraws their dividend.
    /// @param to The address which withdraws AVAXer from this contract.
    /// @param weiAmount The amount of withdrawn AVAXer in wei.
...
/// @author Roger Wu (https://github.com/roger-wu)
/// @dev A mintable ERC20 token that allows anyone to pay and distribute AVAXer
///  to token holders as dividends and allows token holders to withdraw their dividends.
///  Reference: the source code of PoWH3D: https://AVAXerscan.io/address/0xB3775fB83F7D12A36E0475aBdD1FCA35c091efBe#code
    ...
    DividendPayingTokenOptionalInterface
{
    ...
    // With `magnitude`, we can properly distribute dividends even if the amount of received AVAXer is small.
    // For more discussion about choosing the value of `magnitude`,
    //  see https://github.com/AVAXereum/EIPs/issues/1726#issuecomment-472352728
    ...
    function distributePIGGYDividends(uint256 amount) public onlyOwner {
    ...
    /// @notice Withdraws the AVAXer distributed to the sender.
    /// @dev It emits a `DividendWithdrawn` event if the amount of withdrawn AVAXer is greater than 0.
...
contract PiggyPrinterDividendTracker is OwnableUpgradeable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main PiggyPrinter contract."
        );
    }
...
// Root file: contracts/baby/PiggyPrinter.sol
...
// import "contracts/interfaces/IUniswapV2Factory.sol";
// import "contracts/interfaces/IUniswapV2Router02.sol";
// import "contracts/baby/PiggyPrinterDividendTracker.sol";
// import "contracts/BaseToken.sol";

contract PiggyPrinter is ERC20, Ownable, BaseToken {
    using SafeMath for uint256;
    ...
    PiggyPrinterDividendTracker public dividendTracker;
    ...
    constructor(
        ...
        dividendTracker = PiggyPrinterDividendTracker(
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "PiggyPrinter: The dividend tracker already has that address"
        );

        PiggyPrinterDividendTracker newDividendTracker = PiggyPrinterDividendTracker(
            payable(newAddress)
        );

        require(
            newDividendTracker.owner() == address(this),
            "PiggyPrinter: The new dividend tracker must be owned by the PiggyPrinter token contract"
        );
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "PiggyPrinter: The router already has that address"
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "PiggyPrinter: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "PiggyPrinter: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "PiggyPrinter: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "PiggyPrinter: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "PiggyPrinter: Cannot update gasForProcessing to same value"
        );
    ...
    function swapAndSendToFee(uint256 tokens) private {
        uint256 initialPIGGYBalance = IERC20(rewardToken).balanceOf(
            address(this)
        );

        swapTokensForPIGGY(tokens);
    ...
    function swapTokensForPIGGY(uint256 tokenAmount) private {
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        if (success) {
            dividendTracker.distributePIGGYDividends(dividends);
