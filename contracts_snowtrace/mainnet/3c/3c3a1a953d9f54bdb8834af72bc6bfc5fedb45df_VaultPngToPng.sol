...
// File: contracts/interfaces/IPangolinStakingRewards.sol
...
// File: contracts/vaults/VaultPngToPng.sol
...
contract VaultPngToPng is VaultController, IStrategy {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;

    /* ========== CONSTANTS ============= */

    IERC20 private constant PNG = IERC20(0x60781C2586D68229fde47564546784ab3fACA982);
    ...
    IPangolinStakingRewards private stakingReward;

    /* ========== INITIALIZER ========== */

    //_token => 0x7c05d54fc5cb6e4ad87c6f5db3b807c94bb89c52 _stakingReward => 0x830A966B9B447c9B15aB24c0369c4018E75F31C9
    function initialize(address _stakingReward) external initializer {
        __VaultController_init(IERC20(PNG));

        stakingReward = IPangolinStakingRewards(_stakingReward);
        _stakingToken.safeApprove(address(_stakingReward), uint(- 1));
    }
    ...
    function balance() public view override returns (uint amount) {
        amount = stakingReward.balanceOf(address(this));
    }
    ...
    function withdrawAll() external override {
        ...
        bool _canMint = canMint();
        uint withdrawalFee = _canMint ? _minter.withdrawalFee(principal, depositTimestamp) : 0;
        uint performanceFee = _canMint ? _minter.performanceFee(profit) : 0;
        if (_canMint) {
    ...
    function harvest() external override onlyKeeper {
        uint pngHarvested = _harvest();

        stakingReward.stake(pngHarvested);
        emit Harvested(pngHarvested);
    }

    function _harvest() private returns(uint) {
        uint before = PNG.balanceOf(address(this));
        stakingReward.getReward();
        return PNG.balanceOf(address(this)).sub(before);
    }
    ...
    function withdrawUnderlying(uint _amount) external {
        ...
        uint depositTimestamp = _depositedAt[msg.sender];
        bool _canMint = canMint();
        uint withdrawalFee = _canMint ? _minter.withdrawalFee(amount, depositTimestamp) : 0;
        if (_canMint){
    ...
    // @dev profits only (underlying + maxi) + no withdraw fee + perf fee
    function getReward() external override {
        ...
        uint depositTimestamp = _depositedAt[msg.sender];
        bool _canMint = canMint();
        uint performanceFee = _canMint ? _minter.performanceFee(amount) : 0;
        if (_canMint) {
