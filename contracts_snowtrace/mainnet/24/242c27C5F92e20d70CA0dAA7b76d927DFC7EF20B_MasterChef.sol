...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    uint256 public swiftPerBlock = 0 ether;
    ...
    function payOrBurnPendingSwift(uint256 _pid, address _user) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        ...
        if (canHarvest(_pid, _user)) {
            if (pending > 0) {
                ...
                // send rewards
                safeSwiftTransfer(_user, pending.sub(devReward));
    ...
    function deposit(
        ...
        payOrBurnPendingSwift(_pid, msg.sender);
        if (_amount > 0) {
    ...
    function _withdraw(
        ...
        updatePool(_pid);
        payOrBurnPendingSwift(_pid, _user);
        if (_amount > 0) {
