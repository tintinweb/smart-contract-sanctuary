...
// File: Dependencies/CheckContract.sol
...
contract LQTYToken is CheckContract, ILQTYToken {
    ...
    string constant internal _NAME = "lPiggyBank";
    string constant internal _SYMBOL = "lPB";
    ...
    address public immutable sPBSwapAddress;

    // --- Events ---
    event LQTYStakingAddressSet(address _lqtyStakingAddress);
    event sPBSwapIn(address swapper, uint256 amount);
    event sPBSwapOut(address swapper, uint256 amount);
    ...
    (
        ...
        address _sPBSwapAddress
    ) 
        public 
    {
        checkContract(_lqtyStakingAddress);
        CheckContract(_sPBSwapAddress);

        deploymentStartTime  = block.timestamp;
        
        lqtyStakingAddress = _lqtyStakingAddress;
        sPBSwapAddress = _sPBSwapAddress;
    ...
    // --- sPB Swap functions ---
    function swapIn(uint256 amount) public returns(bool) {
        require(IERC20(sPBSwapAddress).allowance(msg.sender, address(this)) >= amount);
        IERC20(sPBSwapAddress).transferFrom(msg.sender, address(this), amount);
        _mint(msg.sender, amount);

        emit sPBSwapIn(msg.sender, amount);
        return true;
    }

    function swapOut(uint256 amount) public returns(bool) {
        require(this.balanceOf(msg.sender) >= amount);
        _burn(msg.sender, amount);
        IERC20(sPBSwapAddress).transfer(msg.sender, amount);
        
        emit sPBSwapOut(msg.sender, amount);
        return true;
    }
