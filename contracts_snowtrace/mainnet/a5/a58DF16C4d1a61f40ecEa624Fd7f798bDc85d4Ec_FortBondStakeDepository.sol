...
contract FortBondStakeDepository is Ownable {
    ...
    address public immutable FORT; // intermediate reward token from treasury
    address public immutable sFORT; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints FORT when receives principle
    ...
    struct Bond {
        uint gonsPayout; // sFORT gons remaining to be paid
        uint fortPayout; //fort amount at the moment of bond
    ...
    function deposit(
        ...
        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 FORT ( underflow protection )
        ...
        /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) FORT
         */
        ...
        IERC20( FORT ).approve( stakingManager, payout );

        IStakingManager(stakingManager).stake( payout, address(this) );
        /* ---------------------------------------------------------- */

        uint stakeGons=ISFORT(sFORT).gonsForBalance(payout);

        // depositor info is stored
        _bondInfo[ _depositor ] = Bond({
            gonsPayout: _bondInfo[ _depositor ].gonsPayout.add( stakeGons ),
            fortPayout: _bondInfo[ _depositor ].fortPayout.add( payout ),
    ...
    function redeem( address _recipient, bool _stake) external returns ( uint ) {
        ...
        uint _amount = ISFORT(sFORT).balanceForGons(info.gonsPayout);
        emit BondRedeemed( _recipient, _amount, 0 ); // emit bond data
        IERC20( sFORT ).transfer( _recipient, _amount ); // pay user everything due
        return _amount;
    }
    ...
    /**
     *  @notice return bond info with latest sFORT balance calculated from gons
     *  @param _depositor address
     *  @return payout uint
     *  @return vesting uint
    ...
    function bondInfo(address _depositor) public view returns ( uint payout,uint vesting,uint lastBlock,uint pricePaid ) {
        ...
        payout=ISFORT(sFORT).balanceForGons(info.gonsPayout);
    ...
    /**
     *  @notice calculate current ratio of debt to FORT supply
    ...
    /**
     *  @notice calculate amount of FORT available for claim by depositor
     *  @param _depositor address
     *  @return pendingPayout_ uint
     */
    function pendingPayoutFor( address _depositor ) external view returns ( uint pendingPayout_ ) {
        uint percentVested = percentVestedFor( _depositor );
        uint payout = ISFORT(sFORT).balanceForGons(_bondInfo[ _depositor ].gonsPayout);
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or FORT) to the DAO
     *  @return bool
     */
    function recoverLostToken( address _token ) external returns ( bool ) {
        require( _token != FORT );
        require( _token != sFORT );
