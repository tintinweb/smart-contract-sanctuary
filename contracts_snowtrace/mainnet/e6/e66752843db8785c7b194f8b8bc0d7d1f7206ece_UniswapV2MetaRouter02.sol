...
contract UniswapV2MetaRouter02 is Ownable, IUniswapV2Router02 {
    ...
    modifier notComplex(address[] memory path){
        require(path.length == 2, "complex paths not supported yet");
        _;
    }
        ...
        returns (uint[] memory amounts)
    {
        ...
        amountOut = haveBank(path[1]) ? convertToBank(newPath[1], amountOut) : amountOut;

        amounts = IUniswapV2Router02(router).getAmountsIn(amountOut, actualPath(path));
        
        amounts[0] = haveBank(path[0]) ? convertToUnderlying(newPath[0], amounts[0]) : amounts[0];
        amounts[1] = haveBank(path[1]) ? convertToUnderlying(newPath[1], amounts[1]) : amounts[1];
    }
        ...
        returns (uint[] memory amounts)
    {
        ...
        uint send = withdrawIfTheresBank(path[1]);
    ...
    function swapExactTokensForTokens(
        ...
        returns (uint[] memory amounts) {
        amounts = getAmountsOut(amountIn, path);
        require(amounts[1] >= amountOutMin, 'UniswapV2Router: SMALL_OUTPUT_AMOUNT');
        ...
        IERC20(path[1]).transfer(to, send);
    }    
    ...
    function swapTokensForExactTokens(
        ...
        notComplex(path) returns (uint[] memory amounts) {
        amounts = getAmountsIn(amountOut, path);
        require(amounts[0] <= amountInMax, 'UniswapV2Router: EXCESSIVE_INPUT_AMOUNT');
    ...
    function swapExactETHForTokens(uint amountOutMin, address[] calldata path, address to, uint deadline)
        ...
        returns (uint[] memory amounts)
    {
        require(path[0] == WETH, "no weth");
        uint amountETHIn = msg.value;
        IWETH(WETH).deposit{value:amountETHIn}();

        //WAVAX DONT REQUIRE APPROVAL

        amounts = IUniswapV2Router02(address(this)).swapExactTokensForTokens(amountETHIn, amountOutMin, path, to, deadline);
    }
}
