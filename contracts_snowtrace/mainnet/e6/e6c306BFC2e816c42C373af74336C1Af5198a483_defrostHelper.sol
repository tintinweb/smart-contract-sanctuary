...
contract defrostHelper is defrostHelperData {
    ...
    function getCollateralInfo()external view returns(address payable[] memory,address[] memory,uint8[] memory,uint256[] memory){
        address payable[] memory vaults = defrostFactory.getAllVaults();
