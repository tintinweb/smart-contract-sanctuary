...
contract HolyCows is ERC721Enumerable, Ownable {
    ...
    function setMintTime(uint256 newMintTime) external onlyOwner {
        mintTime = newMintTime;
    }
    ...
    function setDev(address payable newAddress) external onlyOwner {
        dev = newAddress;
    }

    function setDevFee(uint256 newFee) external onlyOwner {
        devFeePercent = newFee;
    }

    function mintNFTs(uint256 quantity) external payable {
        require(mintTime <= block.timestamp, "It not mint time yet");
    ...
    function withdraw() external onlyOwner {
        ...
        uint amount = balance;
        if(dev != address(0) && devFeePercent > 0){
            uint devFee = balance.mul(devFeePercent).div(100);
            amount = balance.sub(devFee);
            dev.transfer(devFee);
        }
