...
contract Treasury {
	...
	function notifyFeeReceived(
		...
		address avasRewards = IRouter(router).getAVASRewards(currency);
		...
		// Send avasPoolShare to avas-currency rewards contract
		uint256 avasReward = IRouter(router).getAVASShare(currency) * amount / 10**4;
		_transferOut(currency, avasRewards, avasReward);
		IRewards(avasRewards).notifyRewardReceived(avasReward);
