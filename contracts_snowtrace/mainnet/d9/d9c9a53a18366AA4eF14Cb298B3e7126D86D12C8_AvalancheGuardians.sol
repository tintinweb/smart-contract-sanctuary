...
// File contracts/AvalancheGuardians.sol
...
|  _  |  |  |  _  |  |  |  |   __|_ _ ___ ___ _| |_|___ ___ ___
|     |  |  |     |-   -|  |  |  | | | .'|  _| . | | .'|   |_ -|
|__|__|\___/|__|__|__|__|  |_____|___|__,|_| |___|_|__,|_|_|___|

*/

contract AvalancheGuardians is ERC721Enumerable, IERC2981, Ownable {
    using SafeMath for uint256;
    using Counters for Counters.Counter;

    string private baseURI = 'https://avaxguardians.com/api/metadata/';
    uint256 constant public MAX_SUPPLY = 5000;
    uint256 public mintPrice = 1.5e18;
    address private royaltyBeneficiary;
    uint256 public royaltyPercentage = 7;
    ...
    event UpdateMintPrice(uint256 value);

    mapping(uint256 => address) public minter;
    mapping (uint256 => uint256) public lastDividendAt;

    constructor() ERC721("Avalanche Guardians", "AVAXG") {
    ...
    function mintNFTs(uint256 mintTotal) external payable {
        ...
        require(msg.value >= mintPrice * mintTotal, "Not enough AVAX sent to cover sale price");
        require(mintTotal + totalSupply() < MAX_SUPPLY, "Exceeds Supply");
    ...
    function royaltyInfo(uint256 /*_tokenId*/, uint256 salePrice_) external view override returns (address, uint256 royaltyAmount) {
        royaltyAmount = (salePrice_ * royaltyPercentage) / 100;
    ...
    function withdrawContractAvax(address payable recipient, uint256 amount) external onlyOwner {
    ...
    function presaleMint(address[] memory presaleMinters) external onlyOwner {
        for(uint256 i = 0; i < presaleMinters.length; i++) {
            uint mintIndex = totalSupply();
            minter[mintIndex] = presaleMinters[i];
            lastDividendAt[mintIndex] = totalDividend;
            _safeMint(presaleMinters[i], mintIndex);
        }
    }
}