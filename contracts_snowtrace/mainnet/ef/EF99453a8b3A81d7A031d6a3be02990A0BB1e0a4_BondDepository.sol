...
contract BondDepository is Ownable {
    ...
    address public immutable KARMA; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints KARMA when receives principle
    ...
    struct Bond {
        uint256 payout; // KARMA remaining to be paid
    ...
    function deposit(
        ...
        // must be > 0.0001 KARMA ( underflow protection )
        ...
        /**
                ...
                approved and
                deposited into the treasury, returning (_amount - profit) KARMA
             */
        ...
        if (fee != 0) {
            ...
            IERC20(KARMA).safeTransfer(DAO, fee);
        }
    ...
    function stakeOrSend(
        ...
        if (!_stake) {
            ...
            IERC20(KARMA).transfer(_recipient, _amount);
            // send payout
        } else {
            // if user wants to stake
            if (useHelper) {
                // use if staking warmup is 0
                IERC20(KARMA).approve(stakingHelper, _amount);
                IStakingHelper(stakingHelper).stake(_amount, _recipient);
            } else {
                IERC20(KARMA).approve(staking, _amount);
    ...
    function maxPayout() public view returns (uint256) {
        return IERC20(KARMA).totalSupply().mul(terms.maxPayout).div(100000);
    }
    ...
    /**
     *  @notice calculate current ratio of debt to KARMA supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns (uint256 debtRatio_) {
        uint256 supply = IERC20(KARMA).totalSupply();
    ...
    /**
     *  @notice calculate amount of KARMA available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or KARMA) to the DAO
     *  @return bool
     */
    function recoverLostToken(address _token) external returns (bool) {
        require(_token != KARMA);
