...
contract AvaxArb
{
	...
	function rebalance() external
	{
		...
		(uint256 _originalReserveA0, uint256 _originalReserveA1,) = _pairA.getReserves();
		(uint256 _originalReserveB0, uint256 _originalReserveB1,) = _pairB.getReserves();
		uint256 _reserveA0 = _originalReserveA0;
		uint256 _reserveA1 = _originalReserveA1;
		uint256 _reserveB0 = _originalReserveB0;
		uint256 _reserveB1 = _originalReserveB1;
		_reserveB0 *= 1e12;
		...
		if (_reserveB0 > _mean && _mean > _reserveA0) {
			...
			_usdcAmt /= 1e12;
			uint256 _avaxAmt = (_morAmt * _originalReserveA1 * 997) / (_originalReserveA0 * 1000 + _morAmt * 997);
		...
		if (_reserveA0 > _mean && _mean > _reserveB0) {
			...
			_usdcAmt /= 1e12;
			uint256 _avaxAmt = (_usdcAmt * _originalReserveB1 * 997) / (_originalReserveB0 * 1000 + _usdcAmt * 997);
