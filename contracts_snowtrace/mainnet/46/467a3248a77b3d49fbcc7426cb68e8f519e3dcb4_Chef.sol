...
/* BURGR GAME
Website: https://idleburgr.game/
Discord: https://discord.gg/445mAMdzhm
Whitepaper: https://whitepaper.idleburgr.game/
Twitter: https://twitter.com/idleburgrtoken
*/
...
// File contracts/Burgr.sol

pragma solidity ^0.8.4;


contract Burgr is ERC20("Burgr", "BURGR"), Ownable {
    address shopAddress;
    address chefAddress;
    ...
    function setShopAddress(address _shopAddress) external onlyOwner {
        require(address(shopAddress) == address(0), "Shop address already set");
    ...
    function mint(address _to, uint256 _amount) external {
        require(_msgSender() == shopAddress, "Only the Shop contract can mint");
        _mint(_to, _amount);
    }

    function burn(address _from, uint256 _amount) external {
        require(_msgSender() == chefAddress, "Only the Chef contract can burn");
        _burn(_from, _amount);
    }
...
contract Chef is ERC721Enumerable, Ownable {
    ...
    uint256 public robotSupply;
    uint256 public alienSupply;
    ...
    constructor() ERC721("Burgr Chef", "BURGR-CHEF") {
    ...
    function _mintBaseTokens(uint16 _numTokens, address _for) internal {
        ...
        for (uint i = 0; i < _numTokens; i++) {
            ...
            if (_rand(totalSupply()) % 100 < BASE_ALIEN_PERCENTAGE) {
                ...
                tokenLevel[tokenId] = 0; // normal chef
            }
        }
    }

    /* Minting of upgrade chef */
    ...
    function mintUpgrade(uint256 _level, uint16 _numTokens) external {
        ...
        require(burgr.balanceOf(msg.sender) >= totalCost, "Insufficient BURGR balance");
        burgr.burn(msg.sender, totalCost);
    ...
    function setShopAddress(address _shopAddress) external onlyOwner {
        require(shopAddress == address(0), "Shop address already set");
        shopAddress = _shopAddress;
    }

    function setBurgr(Burgr _burgr) external onlyOwner {
        burgr = _burgr;
    }
