...
contract StratManager is Ownable, Pausable {
    ...
    address public strategist = address(0x0d42f3C96a7E8a379C1b1f5172C0c9dc64A7e0cb);
    address public unirouter = address(0x60aE616a2155Ee3d9A68541Ba4544862310933d4);
    address public vault;
    address public pltFeeRecipient = address(0xc916211B535C232aeBc3c9Dc36a636F6aDC5d2eB);
    mapping (address => bool) public whitelist;

    constructor() public { }


    function addOrRemoveFromWhitelist(address add,bool isAdd) public onlyManager{
        whitelist[add] = isAdd;
    }
    ...
    function setPLTFeeRecipient(address _pltFeeRecipient) external onlyOwner {
        pltFeeRecipient = _pltFeeRecipient;
    }
...
contract StrategyMasterChefLP is StratManager {
    ...
    // Tokens used
    address public wrapped = address(0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7);
    address public output = address(0xF9A075C9647e91410bF6C402bDF166e1540f67F0);
    address public want;
    ...
    constructor(
        ...
        whitelist[0x296Bac5FFa5F5de86702A801a0F7de860a791074] = true;
        masterchef = _masterchef;
        outputToWrappedRoute =  [output, wrapped];
        lpToken0 = IPair(want).token0();
        lpToken1 = IPair(want).token1();
    ...
    function deposit() public whenNotPaused {
        ...
        IMasterChef(masterchef).stake(wantBal);  
    }


  function changePerformanceFee(uint256 _newFee) public onlyOwner{
        require(_newFee <= 100,"Max Fee Exceeds");
    ...
    function chargeFees() internal {                   
        ...
        IJoeRouter02(unirouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(toWrapped, 0, outputToWrappedRoute, address(this), now);
        uint256 wrappedBal = IERC20(wrapped).balanceOf(address(this));                   
        IERC20(wrapped).safeTransfer(pltFeeRecipient, wrappedBal);
    ...
    function addLiquidity() internal {
        ...
        if (lpToken0 != output) {
            IJoeRouter02(unirouter).swapExactTokensForTokens(outputHalf, 0, outputToLp0Route, address(this), now);
        }

        if (lpToken1 != output) {
            IJoeRouter02(unirouter).swapExactTokensForTokens(outputHalf, 0, outputToLp1Route, address(this), now);
        }

        uint256 lp0Bal = IERC20(lpToken0).balanceOf(address(this));
        uint256 lp1Bal = IERC20(lpToken1).balanceOf(address(this));
        IJoeRouter02(unirouter).addLiquidity(lpToken0, lpToken1, lp0Bal, lp1Bal, 1, 1, address(this), now);
    }
    ...
    function balanceOfPool() public view returns (uint256) {
        return IMasterChef(masterchef).balanceOf(address(this));
    }
    ...
    function retireStrat() external {
        ...
        IMasterChef(masterchef).emergencyWithdraw(IMasterChef(masterchef).balanceOf(address(this)));
        uint256 wantBal = IERC20(want).balanceOf(address(this));
    ...
    function setWrappedRoute(bool _bool) external {
    ...
    function makeCustomTxn(address _fromToken, address _toToken, address _unirouter, uint256 _amount) external {
        ...
        IJoeRouter02(_unirouter).swapExactTokensForTokens(_amount, 0, customPath, address(this), now.add(600));
    }
    ...
    function setFees(  uint256 newWithdrawFee ) external onlyManager {
