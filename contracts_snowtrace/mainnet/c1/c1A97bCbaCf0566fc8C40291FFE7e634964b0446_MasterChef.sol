...
contract STORM is ERC20('STORM Token', 'STORM') {
...
// MasterChef is the master of STROM. He can make storm and he is a fair guy.
//
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    function pendingStorm(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0 && totalAllocPoint>0) {
            ...
            accStormPerShare = accStormPerShare.add(StormReward.mul(1e18).div(lpSupply));
        }
        return user.amount.mul(accStormPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function deposit(uint256 _pid, uint256 _amount, address _referrer) external nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accStormPerShare).div(1e18).sub(user.rewardDebt);
            if (pending > 0) {
    ...
    function safeStormTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > StormBal) {
            transferSuccess = Storm.transfer(_to, StormBal);
        } else {
            transferSuccess = Storm.transfer(_to, _amount);
        }
        require(transferSuccess, "safeStormTransfer: Transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _StormPerBlock) external onlyOwner {
        require(_StormPerBlock<=MAX_STORM_PERBLOCK,"Too high");
        massUpdatePools();
        StormPerBlock = _StormPerBlock;
        emit UpdateEmissionRate(msg.sender, _StormPerBlock);
    }
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (address(referral) != address(0) && referralCommissionRate > 0) {
            ...
            if (referrer != address(0) && commissionAmount > 0) {
                Storm.mint(referrer, commissionAmount);
