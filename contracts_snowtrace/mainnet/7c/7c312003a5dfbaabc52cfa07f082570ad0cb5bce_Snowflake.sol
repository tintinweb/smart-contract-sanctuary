...
// onlyOwner
    ...
    function manualSwapLiquify(uint percent) public {
        uint256 contractTokenBalance = balanceOf(address(this))*percent/100;
        require(!swapping); // just basically copied it with all the strange solutions
        swapping = true;
        if(rewardsFee>0){
            uint256 rewardsPoolTokens = contractTokenBalance*rewardsFee/100;
            _transfer(address(this), distributionPool, rewardsPoolTokens);
        ...
        contractTokenBalance = balanceOf(address(this))*percent/100;
        _swapTokensForMIM(contractTokenBalance,paymentSplitter);
        swapping = false;
    }
