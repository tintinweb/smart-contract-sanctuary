...
contract SwapCenterAvalanche is ISwap, ReentrancyGuard, Ownable {
  ...
  uint256 constant WAVAX_ID = type(uint256).max;
  ...
  uint256 constant TRADERJOE_ID = 0;
  ...
  uint256 constant TRADERJOE_ZAP_IN = 9;

  uint256 constant SUSHISWAP_ZAP_IN = 10;

  uint256 constant TRADERJOE_RM_LIQ = 11;

  uint256 constant SUSHISWAP_RM_LIQ = 12;

  uint256 constant PANGOLIN_ID = 13;
  uint256 constant PANGOLIN_ZAP_IN = 14;
  uint256 constant PANGOLIN_RM_LIQ = 15;


  address constant TRADERJOE_ROUTER = 0x57319d41F71E81F3c65F2a47CA4e001EbAFd4F33;
  ...
  function swapExactTokenIn(address tokenIn, address tokenOut, uint256 amountIn, uint256 minAmountOut) external override payable nonReentrant returns (uint256) {
    ...
    for(uint256 i = 0; i < exchangeLen; i++) {
      ...
      if(exchangeId == TRADERJOE_ID) {
        _swapExactTokenIn_uniswapType(TRADERJOE_ROUTER, curSellPath);
      } else if (exchangeId == SUSHISWAP_ID) {
        _swapExactTokenIn_uniswapType(SUSHI_ROUTER, curSellPath);
      ...
      } else if (exchangeId == TRADERJOE_ZAP_IN) {
        _swapZapIn_uniswapType(TRADERJOE_ROUTER, curSellPath);
      } else if (exchangeId == SUSHISWAP_ZAP_IN) {
        _swapZapIn_uniswapType(SUSHI_ROUTER, curSellPath);
      } else if(exchangeId == TRADERJOE_RM_LIQ) {
        _swapRmLiq_uniswapType(TRADERJOE_ROUTER, curSellPath);
      } else if(exchangeId == SUSHISWAP_RM_LIQ) {
        _swapRmLiq_uniswapType(SUSHI_ROUTER, curSellPath);
      } else if (exchangeId == PANGOLIN_ID) {
        _swapExactTokenIn_uniswapType(PANGOLIN_ROUTER, curSellPath);
      } else if (exchangeId == PANGOLIN_ZAP_IN) {
        _swapZapIn_uniswapType(PANGOLIN_ROUTER, curSellPath);
      } else if (exchangeId == PANGOLIN_RM_LIQ) {
        _swapRmLiq_uniswapType(PANGOLIN_ROUTER, curSellPath);
      } else if (exchangeId == VAULT_DEPOSIT_ID) {
        _swapDepositVault(curSellPath);
      } else if (exchangeId == WAVAX_ID) {
        _swapWAVAX(curSellPath);
      } else {
  ...
  function _swapWAVAX(address[] memory path) internal{
    require(path.length == 2, "Unsupported path");
    ...
    if (curInputToken == ETH_ADDRESS && curOutputToken == wavax_address) {
      uint256 amountIn = address(this).balance;
      IWETH(wavax_address).deposit{value:amountIn}();
    } else if (curInputToken == wavax_address && curOutputToken == ETH_ADDRESS) {
      uint256 amountIn = IERC20(curInputToken).balanceOf(address(this));
      IERC20(curInputToken).safeApprove(wavax_address, 0);
      IERC20(curInputToken).safeApprove(wavax_address, amountIn);
      IWETH(wavax_address).withdraw(amountIn);
    } else {
  ...
  function _swapZapIn_uniswapType(address router, address[] memory path) internal {
    require(path.length == 2, "Unsupported path");
    ...
    (uint256 res0, uint256 res1, ) = IUniswapV2Pair(curOutputToken).getReserves();
    
    address token0 = IUniswapV2Pair(curOutputToken).token0();
    address token1 = IUniswapV2Pair(curOutputToken).token1();
    
    address[] memory sellPath = new address[](2);
    uint256 sellAmount;

    if(curInputToken == token0){
      sellPath[0] = token0;
      sellPath[1] = token1;
      sellAmount = _calculateSwapInAmount(res0, amountIn);
    } else if(curInputToken == token1) {
      sellPath[0] = token1;
      sellPath[1] = token0;
      sellAmount = _calculateSwapInAmount(res1, amountIn);
    }

    IERC20(curInputToken).approve(router, amountIn);
    
    IUniswapV2Router02(router).swapExactTokensForTokens(
      sellAmount, 0, sellPath, address(this), block.timestamp);

    // Amount of the intermediate token. (The other token to form the pair.)
    uint256 otherAmount = IERC20(sellPath[1]).balanceOf(address(this));
    
    // Amount of the remaining input token
    uint256 afterAmount = amountIn - sellAmount; 
    require(afterAmount <=  amountIn, "sub: overflow");

    IERC20(sellPath[1]).approve(router, otherAmount);
  ...
  // https://snowtrace.io/address/0x8308a1A404dB3cB3075618B1651671bC4E15F9d5/transactions#code
  // ln 1239
