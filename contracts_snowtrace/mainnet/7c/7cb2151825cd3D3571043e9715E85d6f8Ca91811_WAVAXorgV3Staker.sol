...
// File: contracts/WAVAXorgV3Staker.sol
...
contract WAVAXorgV3Staker is IUniswapV3Staker, Multicall {
    /// @notice Represents a staking incentive
    struct Incentive {
        uint256 totalRewardUnclaimed;
        uint160 totalSecondsClaimedX128;
    ...
    function createIncentive(IncentiveKey memory key, uint256 reward) external override {
        require(reward > 0, 'WAVAXorgV3Staker::createIncentive: reward must be positive');
        require(
            block.timestamp <= key.startTime,
            'WAVAXorgV3Staker::createIncentive: start time must be now or in the future'
        );
        require(
            key.startTime - block.timestamp <= maxIncentiveStartLeadTime,
            'WAVAXorgV3Staker::createIncentive: start time too far into future'
        );
        require(key.startTime < key.endTime, 'WAVAXorgV3Staker::createIncentive: start time must be before end time');
        require(
            key.endTime - key.startTime <= maxIncentiveDuration,
            'WAVAXorgV3Staker::createIncentive: incentive duration is too long'
        );
    ...
    function endIncentive(IncentiveKey memory key) external override returns (uint256 refund) {
        require(block.timestamp >= key.endTime, 'WAVAXorgV3Staker::endIncentive: cannot end incentive before end time');
        ...
        require(refund > 0, 'WAVAXorgV3Staker::endIncentive: no refund available');
        require(
            incentive.numberOfStakes == 0,
            'WAVAXorgV3Staker::endIncentive: cannot end incentive while deposits are staked'
        );
    ...
    function onERC721Received(
        ...
        require(
            ...
            'WAVAXorgV3Staker::onERC721Received: not a univ3 nft'
        );
    ...
    function transferDeposit(uint256 tokenId, address to) external override {
        require(to != address(0), 'WAVAXorgV3Staker::transferDeposit: invalid transfer recipient');
        address owner = deposits[tokenId].owner;
        require(owner == msg.sender, 'WAVAXorgV3Staker::transferDeposit: can only be called by deposit owner');
    ...
    function withdrawToken(
        ...
        require(to != address(this), 'WAVAXorgV3Staker::withdrawToken: cannot withdraw to staker');
        Deposit memory deposit = deposits[tokenId];
        require(deposit.numberOfStakes == 0, 'WAVAXorgV3Staker::withdrawToken: cannot withdraw token while staked');
        require(deposit.owner == msg.sender, 'WAVAXorgV3Staker::withdrawToken: only owner can withdraw token');
    ...
    function stakeToken(IncentiveKey memory key, uint256 tokenId) external override {
        require(deposits[tokenId].owner == msg.sender, 'WAVAXorgV3Staker::stakeToken: only owner can stake token');
    ...
    function unstakeToken(IncentiveKey memory key, uint256 tokenId) external override {
        ...
        if (block.timestamp < key.endTime) {
            require(
                ...
                'WAVAXorgV3Staker::unstakeToken: only owner can withdraw token before incentive end time'
            );
        }
        ...
        require(liquidity != 0, 'WAVAXorgV3Staker::unstakeToken: stake does not exist');
        ...
        returns (uint256 reward, uint160 secondsInsideX128)
    {
        ...
        (uint160 secondsPerLiquidityInsideInitialX128, uint128 liquidity) = stakes(tokenId, incentiveId);
        require(liquidity > 0, 'WAVAXorgV3Staker::getRewardInfo: stake does not exist');
    ...
    function _stakeToken(IncentiveKey memory key, uint256 tokenId) private {
        require(block.timestamp >= key.startTime, 'WAVAXorgV3Staker::stakeToken: incentive not started');
        require(block.timestamp < key.endTime, 'WAVAXorgV3Staker::stakeToken: incentive ended');

        bytes32 incentiveId = IncentiveId.compute(key);

        require(
            incentives[incentiveId].totalRewardUnclaimed > 0,
            'WAVAXorgV3Staker::stakeToken: non-existent incentive'
        );
        require(
            _stakes[tokenId][incentiveId].liquidityNoOverflow == 0,
            'WAVAXorgV3Staker::stakeToken: token already staked'
        );

        (IUniswapV3Pool pool, int24 tickLower, int24 tickUpper, uint128 liquidity) =
            NFTPositionInfo.getPositionInfo(factory, nonfungiblePositionManager, tokenId);

        require(pool == key.pool, 'WAVAXorgV3Staker::stakeToken: token pool is not the incentive pool');
        require(liquidity > 0, 'WAVAXorgV3Staker::stakeToken: cannot stake token with 0 liquidity');
