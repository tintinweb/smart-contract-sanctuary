...
contract StrategyTraderJoe is IStrategyAlpaca, ReentrancyGuard, Ownable, CoreRef {
    ...
    function _calculateMinReturn(uint256 amount) internal view returns (uint256 minReturn) {
        ...
        uint256 oraclePriceUsdPerMim = IOracle(oracle).getLatestPrice(wantAddress);
        uint256 priceMimPerJoe = oraclePriceUsdPerJoe.mul(1e18).div(oraclePriceUsdPerMim);
        uint256 total = amount.mul(priceMimPerJoe).div(1e18);
        minReturn = total.mul(100 - swapSlippage).div(100);
    }

    function setEarnedToWantPath(address[] calldata _earnedToWantPath) public onlyGovernor {
        earnedToWantPath = _earnedToWantPath;
    }
