...
interface IPawBondingCalculator {
...
contract PawBondingCalculator is IPawBondingCalculator {
    ...
    function getKValue( address _pair ) public view returns( uint k_ ) {
        uint token0 = IERC20( IJoePair( _pair ).token0() ).decimals();
        uint token1 = IERC20( IJoePair( _pair ).token1() ).decimals();
    ...
    function valuation( address _pair, uint amount_ ) external view override returns ( uint _value ) {
        ...
        uint totalSupply = IJoePair( _pair ).totalSupply();
    ...
    function markdown( address _pair ) external view override returns ( uint ) {
        ...
        uint reserve;
        if ( IJoePair( _pair ).token0() == PAW ) {
            reserve = reserve1;
        } else {
            reserve = reserve0;
        }
        return reserve.mul( 2 * ( 10 ** IERC20( PAW ).decimals() ) ).div( getTotalValue( _pair ) );
    }
}