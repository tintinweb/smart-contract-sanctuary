...
interface IDIRACERC20 is IERC20Mintable, IERC20 {
...
contract DiracTreasury is Ownable {
    ...
    IDIRACERC20 public immutable Dirac;
    ...
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        Dirac.burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = Dp.balanceOf( msg.sender ); // Can only borrow against sOHM held
    ...
    function repayDebtWithTime( uint _amount ) external {
        ...
        Dirac.burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, address(Dirac), _amount, _amount );
    }
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( Dirac.totalSupply().sub( totalDebt ) );
    }
    ...
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            ...
            value_ = _amount.mul( 10 ** Dirac.decimals() ).div( 10 ** IERC20( _token ).decimals() );
