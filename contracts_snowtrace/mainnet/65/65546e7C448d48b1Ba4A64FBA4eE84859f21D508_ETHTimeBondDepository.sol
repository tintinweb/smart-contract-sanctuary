...
contract ETHTimeBondDepository is Ownable {
    ...
    address public immutable stablePair; // principle pair with a stablecoin
    address public immutable quote; // stablecoin paired with the principle
    address public immutable dexFactory; // factory that made the pair
    ...
    constructor ( 
        ...
        DAO = _DAO;
        require( _stablePair != address(0) );
        stablePair = _stablePair;
        require( _quote != address(0) );
        quote = _quote;
        require( _dexFactory != address(0) );
    ...
    // Fetches USD price of token by accessing its pair reserves using Uniswap as Oracle.
    function price(address _factory, address _token, address _quote) private view returns (uint256) {
        ...
        return UniswapLibrary.quote(1e20, reserve0, reserve1);
    }
    ...
    function assetPrice() public view returns (int) {
        return int(price(dexFactory, principle, quote));
    }
