...
interface ICurveFi_2 {
    ...
    function remove_liquidity_one_coin(uint256 _token_amount, int128 i, uint256 min_amount, bool _use_underlying) external;
...
abstract contract StrategyBase {
    ...
    // Dex
    address public univ2Router2 = 0xE54Ca86531e17Ef3616d22Ca28b0D458b6C89106;//pangolin

    //Sushi
    address public sushiRouter = 0x60aE616a2155Ee3d9A68541Ba4544862310933d4;//traderjoe
...
contract StrategyAvaxCrv3cryptoV1 is StrategyBase {
    address public constant crv3cypto = 0x1daB6560494B04473A0BE3E7D83CF3Fdf3a51828;
    address public constant USDC = 0xA7D7079b0FEaD91F3e65f86E8915Cb59c1a4C664;
    address public constant CRV = 0x47536F17F4fF30e64A96a7555826b8f9e66ec468;

    // staking constants
    address public constant rewardGauge = 0x445FE580eF8d70FF569aB36e80c647af338db351;
    address public constant stakingGauge = 0x58e57cA18B7A47112b877E31929798Cd3D703b0f;
    ...
    public StrategyBase(crv3cypto, _governance, _strategist, _controller, _timelock)
    {
        IERC20(crv3cypto).safeApprove(rewardGauge, uint256(-1));
        IERC20(crv3cypto).safeApprove(stakingGauge, uint256(-1));
        IERC20(USDC).safeApprove(stakingGauge, uint256(-1));
    }
    ...
    function balanceOfPool() public override view returns (uint256){
        return IERC20(rewardGauge).balanceOf(address(this));
    }

    // **** State Mutations ****

    function deposit() public override {
        uint256 _lpToken = IERC20(crv3cypto).balanceOf(address(this));
        if (_lpToken > 0) {
            LpTokenStaker(rewardGauge).deposit(_lpToken);
        }
    }
    ...
    function _withdrawSome(uint256 _amount) internal override returns (uint256) {
        ...
        LpTokenStaker(rewardGauge).withdraw(_amount);
    ...
    function _convertWantToBuyback(uint256 _lpAmount) internal override returns (address, uint256){
        ...
        //remove liquidity
        ICurveFi_5(stakingGauge).remove_liquidity_one_coin(_lpAmount, 1, 0);
        uint256 _USDCBal = IERC20(USDC).balanceOf(address(this));
        //swap USDC to WAVAX
        if(_USDCBal > 0){
            _swapUniswap(USDC, WAVAX, _USDCBal);
        }
        return (WAVAX, IERC20(WAVAX).balanceOf(address(this)));
    }
    ...
    function harvest() public override onlyBenignCallers {
        ...
        // if include CRV rewards
        uint256 _rewardsCRV = IERC20(CRV).balanceOf(address(this));
        if (_rewardsCRV > 0) {
            address[] memory path = new address[](2);
            path[0] = CRV;
            path[1] = WAVAX;
            _swapUniswapWithDetailConfig(CRV, WAVAX, _rewardsCRV, 1, path, sushiRouter);
        }

        uint256 _wavax = IERC20(WAVAX).balanceOf(address(this));
        uint256 _buybackLpAmount = _wavax.mul(performanceFee).div(performanceMax);
        if (_wavax > 0) {
            if (buybackEnabled == true && _buybackLpAmount > 0){
                buybackAndNotify(WAVAX, _buybackLpAmount);
            }

            _swapUniswap(WAVAX, USDC, IERC20(WAVAX).balanceOf(address(this)));
            uint256 _USDCBal = IERC20(USDC).balanceOf(address(this));

            if (_USDCBal > 0){
                //add liquidity
                uint256[5] memory liquidity;
                liquidity[1] = _USDCBal;
                ICurveFi_5(stakingGauge).add_liquidity(liquidity, 0);
            }
    ...
    function withdraw(IERC20 _asset) external override returns (uint256 balance) {
        require(address(_asset) != USDC, '!USDC');
        require(address(_asset) != crv3cypto, '!lpToken');
    ...
    function _claimRewards() internal{
        LpTokenStaker(rewardGauge).claim_rewards(address(this));//claim WAVAX/CRV
    }

    function getHarvestable() public view returns (uint256) {
        return LpTokenStaker(rewardGauge).claimable_reward_write(address(this), CRV);
    }

    function getHarvestableWAVAX() public view returns (uint256) {
        return LpTokenStaker(rewardGauge).claimable_reward_write(address(this), WAVAX);
    }

    function getName() external override pure returns (string memory) {
        return "StrategyAvaxCrv3cryptoV1";
    }
}