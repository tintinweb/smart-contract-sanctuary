...
interface ISniper {
	...
	function calculateTaxes(address sender, uint256 amount) external view returns (uint256);
...
contract AVAXBBToken is IERC20 {
	...
	function tokenTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
		...
		bool isLiquidityTransfer = sender == lpPair || recipient == lpPair;

		if (isExcludedTransfer || isContractTransfer) {
			return feelessTransfer(sender, recipient, amount);
		} else if (tradingEnabled && isLiquidityTransfer) {
			...
			uint256 contractShares = antiSnipe.calculateTaxes(sender, amount);
	...
	function buyAndLiquify(uint256 amount) external onlyOperator {
		require(address(this).balance / 2 >= amount, "Amount > Balance");

		uint256 currentAmount = _balances[address(this)];
		...
		uint256 newAmount = _balances[address(this)];
		uint256 liquidityAmount = newAmount - currentAmount;

		dexRouter.addLiquidityAVAX{value: amount}(
			address(this),
			liquidityAmount,
			0,
			0,
	...
	function buyAndSend(uint256 amount) external onlyOperator {
		require(address(this).balance >= amount, "Amount > Balance");
