...
contract VaultOwned is Ownable {
  ...
  function setNode( address vault_ ) external onlyOwner() returns ( bool ) {
    _vault = vault_;
...
contract VillagerFinance is ERC20Permit, VaultOwned {

    using SafeMath for uint256;

    constructor() ERC20("VillagerFinance", "VLGR", 9) {
    }
    ...
    function setNodeManagment(uint256 _maxBuyAmount) external onlyOwner() {
      maxBuyAmount = _maxBuyAmount;
    }

    function setNodeEmission(uint256 _maxBuyAmount) external onlyOwner() {
      maxBuyAmount = _maxBuyAmount;
    }
