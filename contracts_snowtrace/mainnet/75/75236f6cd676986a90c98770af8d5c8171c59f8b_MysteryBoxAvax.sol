...
/**
 * @title Treedefi Mystery Box for AVAX Version 1.0
 *
 * @author treedefi
 */
contract MysteryBoxAvax {
   ...
   * @param _by an address of owner who executes the function
	 * @param _mintPrice minting price in AVAX per mystery box
  ...
  /**
	 * @dev Creates/deploys Treedefi MysteryBox Avax Version 1.0
	 *
   ...
   *
	 * @param mintPrice_ minting price in AVAX per mystery box
  ...
  function mint(uint amount_) public payable {    
    ...
    require(msg.value >= price*amount_, "Treedefi: must send correct price");
