...
contract HolyCows is ERC721Enumerable, Ownable {
    ...
    function mintNFTs(uint256 quantity) external payable {
        ...
        require(price.mul(quantity) == msg.value, "Incorrect AVAX value");
        ...
        if(dev != address(0) && devFeePercent > 0){
            uint devFee = msg.value.mul(devFeePercent).div(100);
            dev.transfer(devFee);
        }
    }

    function oldContractMigrate(address[] calldata mintAddress, uint256[] calldata tokenID) external onlyOwner {
        for (uint256 i = 0; i < mintAddress.length; i++){
            require(_exists(tokenID[i]) == false, "Token already exists");
        }
        for (uint256 i = 0; i < mintAddress.length; i++){
            _safeMint(mintAddress[i], tokenID[i]);
        }
    }
