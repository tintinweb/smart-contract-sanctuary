...
contract ChestSale is Ownable, ERC1155Holder {
    ...
    uint256 constant public TOTAL_CHESTS = 270000;  // must be a multiple of TOTAL_WEEKS * 4
...
contract Resource is ERC1155, Ownable {
    ...
    function initialMint(ChestSale chest) external onlyOwner {
        ...
        _mint(address(chest), 1, 67500, "");
        _mint(address(chest), 2, 67500, "");
        _mint(address(chest), 3, 67500, "");
        _mint(address(chest), 4, 67500, "");
    ...
    function premint(address to, uint256[] calldata tokenId, uint256[] calldata amount) external onlyOwner {
        require(!premintFinished, "premint is finished");
        require(tokenId.length == amount.length, "invalid args");
