...
contract SuperNovaSaleFantom is ReentrancyGuard {
    using SafeMath for uint256;
    ...
    // Price of sale SPN, 1 DAI
    uint256 DAIPerToken = 5 ;
    // Amount of DAI received in sale
    ...
    constructor (address _SPN, address _DAI, uint256 _startingTimestamp) public {
        SPN = IBEP20(_SPN);
        DAI = IBEP20(_DAI);
        isSaleActive = true;
        owner = msg.sender;
    ...
    function buy (uint256 _amount, address beneficiary) public nonReentrant {
        ...
        require (daiReceived +  _amount <= 10000 ether, "sale hardcap reached");
        require(block.timestamp >= startingTimeStamp, "sale has not started");

        DAI.safeTransferFrom(beneficiary, address(this), _amount);
