...
contract VYBEStaking is Ownable {
    ...
    using SafeERC20 for IsVYBE;

    IERC20 public immutable VYBE;
    IsVYBE public immutable sVYBE;
    ...
    event LogForfeit(address indexed recipient, uint256 memoAmount, uint256 VYBEAmount);
    ...
    constructor ( 
        ...
        uint32 _firstEpochVYBE
    ) {
        require( _VYBE != address(0) );
        VYBE = IERC20(_VYBE);
        require( _sVYBE != address(0) );
        sVYBE = IsVYBE(_sVYBE);
        
        epoch = Epoch({
            length: _epochLength,
            number: _firstEpochNumber,
            endVYBE: _firstEpochVYBE,
            distribute: 0
        });
    }
    ...
    function stake( uint _amount, address _recipient ) external returns ( bool ) {
        ...
        warmupInfo[ _recipient ] = Claim ({
            ...
            gons: info.gons.add( sVYBE.gonsForBalance( _amount ) ),
            expiry: epoch.number.add( warmupPeriod ),
            lock: false
        });
        
        sVYBE.safeTransfer( address(warmupContract), _amount );
    ...
    function claim ( address _recipient ) external {
        ...
        if ( epoch.number >= info.expiry && info.expiry != 0 ) {
            ...
            uint256 amount = sVYBE.balanceForGons( info.gons );
    ...
    function forfeit() external {
        ...
        uint sVYBEBalance = sVYBE.balanceForGons( info.gons );
        warmupContract.retrieve( address(this),  sVYBEBalance);
        VYBE.safeTransfer( msg.sender, info.deposit);
        emit LogForfeit(msg.sender, sVYBEBalance, info.deposit);
    }
    ...
    function unstake( uint _amount, bool _trigger ) external {
        ...
        sVYBE.safeTransferFrom( msg.sender, address(this), _amount );
    ...
    function rebase() public {
        if( epoch.endVYBE <= uint32(block.timestamp) ) {

            sVYBE.rebase( epoch.distribute, epoch.number );

            epoch.endVYBE = epoch.endVYBE.add32( epoch.length );
            epoch.number++;
            ...
            uint staked = sVYBE.circulatingSupply();
