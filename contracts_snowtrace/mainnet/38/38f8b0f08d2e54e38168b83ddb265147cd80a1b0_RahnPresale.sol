...
contract RahnPresale is ReentrancyGuard {
	using SafeMath for uint256;
	...
	// Price of presale RAHN, 20 USDC
	uint256 USDCPerToken = 25;
	...
	function buy(uint256 _amount, address beneficiary) public nonReentrant {
		...
		uint256 tokens = (_amount * 10 ** 12).div(USDCPerToken/1000);
    ...
    function setRAHN(address _rahn) external onlyOwner {
		RAHN = IBEP20(_rahn);
	}
	...
	function getRAHNTokensLeft() external view returns (uint256) {
		return RAHN.balanceOf(address(this));
	}

	function claimTokens(address _claimer) external nonReentrant {
		require(isClaimActive, "Claim is not allowed yet");
		require(tokensOwned[_claimer] > 0, "User should own some RAHN tokens");
		require(tokensUnclaimed[_claimer] > 0, "User should have unclaimed RAHN tokens");
		require(RAHN.balanceOf(address(this)) >= tokensUnclaimed[_claimer], "There are not enough RAHN tokens to transfer.");

		uint256 callerTokensUnclaimed = tokensUnclaimed[_claimer];
		tokensUnclaimed[_claimer] = 0;
		RAHN.safeTransfer(_claimer, callerTokensUnclaimed);
	...
	function withdrawUnsoldRAHN() external onlyOwner {
		uint256 amount = RAHN.balanceOf(address(this)) - totalTokensSold;
		RAHN.safeTransfer(msg.sender, amount);
	}

	function withdrawAllRAHN() external onlyOwner {
		RAHN.safeTransfer(msg.sender, RAHN.balanceOf(address(this)));
	}
}