...
contract AVAXPlay is IBEP20, Ownable {

    // Events
    event AutoLiquify(uint256 amountAVAX, uint256 amountTokens);
    event SetFees(uint256 liquidityFee, uint256 marketingFee, uint256 reflectionFee, uint256 teamFee, uint256 devFee, uint256 totalFee);
    event SetSwapBackSettings(bool enabled, uint256 swapThreshold);
    event SetIsFeeExempt(address holder, bool enabled);
    event SetFeeReceivers(address marketingReceiver, address teamFeeReceiver, address devFeeReceiver);
    ...
    event devFunds(uint256 amountAVAX, address recipient);
    ...
    uint256 public _maxTXafter1H = 10000000 * 10 **_decimals;

    // Basic Contract Info
    string constant _name = 'AVAXPlay';
    ...
    uint256 private totalFees = _marketingFee + _reflectionFee + _liquidityFee + _teamFee + _devFee;
    uint256 private contractFees = _marketingFee + _liquidityFee + _teamFee + _devFee;
    ...
    // Fee Receiver
    address private marketingFeeReceiver = 0xeb745f8Ca9c5004cAce018E72dFE3cA8359489e5;
    address private teamFeeReceiver = 0xFF82598087E2b56d149f3290668D72f916F37978;
    address private devFeeReceiver = 0xfD41c1D0313985795f11827D4161B921Df88785D;
    ...
    uint256 public swapThreshold = _tTotal / 1000 * 2; // 0.2% 
    ...
    function removeAllFee() private {
        ...
        _teamFee = 0;
        _devFee = 0;
        contractFees = _marketingFee + _liquidityFee + _teamFee + _devFee;
    }
    ...
    function swapBack() internal swapping {       
        ...
        uint256 amountAVAXdev = amountAVAX - amountAVAXMarketing - amountAVAXTeam - amountAVAXLiquidity;
        ...
        if(amountAVAXdev > 0) {payable(devFeeReceiver).transfer(amountAVAXdev);
            emit devFunds(amountAVAXdev, devFeeReceiver);
        }
    ...
    function setFees(uint256 liquidityFee, uint256 marketingFee, uint256 reflectionFee, uint256 teamFee, uint256 devFee) external onlyOwner {
        require(liquidityFee + marketingFee + reflectionFee < 60, "Total fees must be below 60%");
        _liquidityFee = liquidityFee;
        _marketingFee = marketingFee;
        _reflectionFee = reflectionFee;
        _teamFee = teamFee;
        _devFee = devFee;
        totalFees = _liquidityFee + _marketingFee + _reflectionFee + _teamFee + _devFee;
        contractFees = _liquidityFee + _marketingFee + _devFee + _teamFee;
        emit SetFees(_liquidityFee, _marketingFee, _reflectionFee, _teamFee, _devFee, totalFees);
    }

    function setFeeReceiver(address _marketingFeeReceiver, address _teamFeeReceiver, address _devFeeReceiver) external onlyOwner {
        marketingFeeReceiver = _marketingFeeReceiver;
        devFeeReceiver = _devFeeReceiver;
        teamFeeReceiver = _teamFeeReceiver;
        emit SetFeeReceivers(marketingFeeReceiver, teamFeeReceiver, devFeeReceiver);
    }
