...
contract ValhallaBondStakeDepository is Ownable {
    ...
    address public immutable VAL; // intermediate reward token from treasury
    address public immutable sVAL; // token given as payment for bond
    IWAVAX9 public immutable principle; // token used to create bond
    address public immutable treasury; // mints VAL when receives principle
    ...
    struct Terms {
        ...
        uint8 pricingType; // 0 min 1 hardmin 2 fixed
    }


    // Info for bond holder with gons
    struct Bond {
        uint gonsPayout; // sVAL gons remaining to be paid
        uint valPayout; //VAL amount at the moment of bond
    ...
    constructor ( 
        ...
        require( _VAL != address(0) );
        VAL = _VAL;
        require( _sVAL != address(0) );
        sVAL = _sVAL;
    ...
    enum PARAMETER { VESTING, PAYOUT, FEE, DEBT, MINPRICE, PRICINGTYPE, CONTROLVARIABLE }
    /**
    ...
    function setBondTerms ( PARAMETER _parameter, uint _input ) external onlyPolicy() {
        ...
        } else if ( _parameter == PARAMETER.PRICINGTYPE ) { // 5
            terms.pricingType = uint8(_input);
        } else if ( _parameter == PARAMETER.CONTROLVARIABLE ) { // 6
    ...
    function deposit( 
        ...
        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 VAL ( underflow protection )
        ...
        /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) VAL
         */
        ...
        if ( fee != 0 ) { // fee is transferred to dao 
            IERC20( VAL ).safeTransfer( DAO, fee ); 
        }
        
        // total debt is increased
        totalDebt = totalDebt.add( value ); 
        IERC20( VAL ).approve( staking, payout );
        IStakingHelper( staking ).stake( payout, address(this) );
        uint stakeGons = ISVAL(sVAL).gonsForBalance(payout);

        // depositor info is stored
        _bondInfo[ _depositor ] = Bond({ 
            gonsPayout: _bondInfo[ _depositor ].gonsPayout.add( stakeGons ),
            valPayout: _bondInfo[ _depositor ].valPayout.add( payout ),
    ...
    function redeem( address _recipient, bool ) external returns ( uint ) {        
        ...
        uint _amount = ISVAL(sVAL).balanceForGons(info.gonsPayout);
        emit BondRedeemed( _recipient, _amount, 0 ); // emit bond data
        IERC20( sVAL ).transfer( _recipient, _amount ); // pay user everything due
        return _amount;
    }
    ...
    function maxPayout() public view returns ( uint ) {
        return IERC20( VAL ).totalSupply().mul( terms.maxPayout ).div( 100000 );
    }
    ...
    function _bondPrice() internal returns ( uint price_ ) {
        if(terms.pricingType == 2)
            ...
            if ( price_ < terms.minimumPrice ) {
                price_ = terms.minimumPrice;        
                if ( terms.minimumPrice != 0 && terms.pricingType == 0) 
    ...
    /**
     *  @notice converts bond price to MIM value
     *  @return price_ uint
     */
    function bondPriceInUSD() public view returns ( uint price_ ) {
        if( isLiquidityBond ) {
            price_ = bondPrice().mul( IBondCalculator( bondCalculator ).markdown( address(principle) ) ).div( 10000 );
        } else {
            price_ = bondPrice().mul( assetPrice() ).div( 10000 );
        }
    }
    ...
    function assetPrice() public view returns (uint) {
        address lpPair = IJoeFactory(joeFactory).getPair(address(principle), MIM);
        uint den = principle.balanceOf(lpPair);
    ...
    /**
     *  @notice return bond info with latest sVAL balance calculated from gons
     *  @param _depositor address
     *  @return payout uint
     *  @return vesting uint32
     *  @return lastTime uint32
     *  @return pricePaid uint
     */
    function bondInfo(address _depositor) public view returns ( uint payout, uint32 vesting, uint32 lastTime, uint pricePaid ) {
        Bond memory info = _bondInfo[ _depositor ];
        payout = ISVAL(sVAL).balanceForGons(info.gonsPayout);
    ...
    /**
     *  @notice calculate current ratio of debt to VAL supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns ( uint debtRatio_ ) {   
        uint supply = IERC20( VAL ).totalSupply();
    ...
    function standardizedDebtRatio() external view returns ( uint ) {
        if ( isLiquidityBond ) {
            return debtRatio().mul( IBondCalculator( bondCalculator ).markdown( address(principle) ) ).div( 1e9 );
        } else {
            return debtRatio();
        }
    }
    ...
    /**
     *  @notice calculate amount of VAL available for claim by depositor
     *  @param _depositor address
     *  @return pendingPayout_ uint
     */
    function pendingPayoutFor( address _depositor ) external view returns ( uint pendingPayout_ ) {
        uint percentVested = percentVestedFor( _depositor );
        uint payout = ISVAL(sVAL).balanceForGons(_bondInfo[ _depositor ].gonsPayout);
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or VAL) to the DAO
     *  @return bool
     */
