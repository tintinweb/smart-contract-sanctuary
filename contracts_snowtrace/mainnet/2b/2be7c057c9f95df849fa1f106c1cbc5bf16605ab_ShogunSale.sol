...
contract ShogunSale is Ownable {
    using SafeERC20 for ERC20;
    using Address for address;
//Presale Data
    uint constant MIMdecimals = 10 ** 18;
    uint constant Shogundecimals = 10 ** 9;
    uint public constant MAX_SOLD = 110000 * Shogundecimals;
    uint public constant PRICE = 4 * MIMdecimals / Shogundecimals ;
    uint public constant MIN_PRESALE_PER_ACCOUNT = 25 * Shogundecimals;
    uint public constant MAX_PRESALE_PER_ACCOUNT = 2500000 * Shogundecimals;
    address public owners;
    ERC20 MIM;

    uint public sold;
    address public Shogun;
    bool canClaim;
    bool publicSale;
    bool privateSale;
    address[] devAddr = [0x04d606777fCd56238922Df54A1b34ec4e395e1ce, 0x61Ea45517EFD2B0686626Bb0CFf15CaBD932656B];
    ...
    function buyShogun(uint256 amount) public onlyEOA {
    ...
    function setClaimingActive(address shogun) public {
    ...
    // claim Shogun allocation based on invested amounts
    function claimShogun() public onlyEOA {
        ...
        if (dailyClaimed[msg.sender] == 0) {
            ...
            amountPerClaim[msg.sender] = (isDev(msg.sender) ? invested[msg.sender] * 25 / 100 : invested[msg.sender] * 25 / 100);
        }
        
        if (isDev(msg.sender)) {
            ERC20(Shogun).transfer(msg.sender, amountPerClaim[msg.sender]);
            invested[msg.sender] -= amountPerClaim[msg.sender];
            dailyClaimed[msg.sender] += 2592000;
        } else {
            ERC20(Shogun).transfer(msg.sender, amountPerClaim[msg.sender]);
            invested[msg.sender] -= amountPerClaim[msg.sender];
    ...
    // ACTIVATION OF PUBLIC SALE
    function activatePublicSale() public {
        require(msg.sender == owners, "not owners");
        publicSale = true;
    }
    // DEACTIVATION OF PUBLIC SALE
    function deactivatePublicSale() public {
        require(msg.sender == owners, "not owners");
        publicSale = false;
    }

    // ACTIVATION OF PRIVATE SALE
    function activatePrivateSale() public {
        require(msg.sender == owners, "not owners");
        privateSale = true;
    }

    // DEACTIVATION OF PRIVATE SALE
