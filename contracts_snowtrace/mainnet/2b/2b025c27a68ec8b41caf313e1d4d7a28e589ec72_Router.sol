...
contract Router {
	...
	mapping(address => uint256) private avasShare; // currency => bps

	mapping(address => address) poolRewards; // currency => contract
	mapping(address => address) avasRewards; // currency => contract
	...
	function getAVASShare(address currency) external view returns(uint256) {
		return avasShare[currency];
	}
	...
	function getAVASRewards(address currency) external view returns(address) {
		return avasRewards[currency];
	}
	...
	function setAVASShare(address currency, uint256 share) external onlyOwner {
		avasShare[currency] = share;
	}
	...
	function setAVASRewards(address currency, address _contract) external onlyOwner {
		avasRewards[currency] = _contract;
	}
