...
contract PowerBombAvaxJoeStake is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    function withdrawAVAX(IERC20Upgradeable token, uint amountOutLpToken, uint amountOutMin) external {
        ...
        require(token == WAVAX, "Withdraw: only WAVAX");
    ...
    function harvest() public {
        ...
        // Special for gOHM
        uint gOHMAmt = token0.balanceOf(address(this));
        if (address(lpToken) == 0xB674f93952F02F2538214D4572Aa47F262e990Ff && gOHMAmt > 1e15) { // gOHM-AVAX, 0.001 gOHM
            swap3(address(token0), address(JOE), gOHMAmt, 0); // token0 == gOHM
        }
    ...
    function getPricePerFullShareInUSD() public view returns (uint) {
        ...
        return getLpTokenPriceInAVAX() * AVAXPriceInUSD / 1e20;
    }
