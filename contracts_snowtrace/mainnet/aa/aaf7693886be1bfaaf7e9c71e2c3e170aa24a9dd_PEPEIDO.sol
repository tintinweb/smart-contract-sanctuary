...
contract PEPEIDO is ERC20 {
    using SafeMath for uint256;
    event PEPEIDOSold(address indexed buyer, uint256 indexed amount);
    ...
    uint256 public maxPurchase = 24*10**9;
    ...
    function setWhitelists(address[] memory _users) public onlyCEO {
        for(uint256 i; i< _users.length; i++) {
            require(!whitelist[_users[i]]);
    ...
    function buyPEPEIDO(uint256 _amount) public whenNotPaused returns(uint256) {
        require(whitelist[msg.sender], "Not on whitelist!");
        require(BuyerQuota[msg.sender].add(_amount) <= maxPurchase, "Exceed MaxPurachse");
        require(mimERC20.allowance(msg.sender, address(this)) >= mimPrice.mul(_amount).div(10**9), "Insufficient approved MIM");
        ...
        emit PEPEIDOSold(msg.sender, _amount);
    }
