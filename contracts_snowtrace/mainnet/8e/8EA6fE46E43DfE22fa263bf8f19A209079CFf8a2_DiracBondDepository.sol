...
contract DiracBondDepository is Ownable {
    ...
    IERC20 public immutable Dirac; // token given as payment for bond
    ...
    function deposit( 
        ...
        uint balanceBefore = Dirac.balanceOf(address(this));
        /**
        ...
        if ( fee != 0 ) { // fee is transferred to dao 
            Dirac.safeTransfer( DAO, fee ); 
        }
        require(balanceBefore.add(profit) == Dirac.balanceOf(address(this)), "Not enough Time to cover profit");
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            Dirac.transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                Dirac.approve( address(stakingHelper), _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return Dirac.totalSupply().mul( terms.maxPayout ) / 100000 ;
    }
