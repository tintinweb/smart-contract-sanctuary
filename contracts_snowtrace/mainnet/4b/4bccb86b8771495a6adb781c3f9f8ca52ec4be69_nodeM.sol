...
contract nodeM {
    ...
    function setNodeManager(address managerAddress) public {
        nm = managerAddress;
        nodeRewardManagement = NODERewardManagement(managerAddress);
    }

    function massCreate(address creator, uint number, string[] memory namesNodes) public {
        for(uint i = 0; i < number; i++){
            nodeRewardManagement.createNode(creator, namesNodes[i]);
        }
    }
