...
contract ULTRANodeSale is Ownable {
    ...
    constructor(address _dev, address _CA, address nodeManagement) {
        CA = ERC20(_CA);
        dev = _dev;
        nodeRewardManager = NODERewardManagement(nodeManagement);
        sold = 0;
    ...
    function buyNODE_test(string memory name, uint256 _initValue) public onlyOwner {
        uint256 nodePrice = _initValue;
        address sender = _msgSender();

        CA.safeTransferFrom(sender, address(this), nodePrice.mul(1e18));
        nodeRewardManager.createNode(sender, name, _initValue.mul(1e18));
    ...
    function setCAaddress(address _CA) public {
