...
contract BOOFI is ERC20WithVotingAndPermit("TEST Boo Finance Token", "TEST_BOOFI"), Ownable {
...
contract zBOOFI_WithdrawalFeeCalculator is Ownable {
    ...
    uint256 public withdrawalFeeMultiplier = 20000;
    ...
    function withdrawalFee() public view returns(uint256) {
        ...
        uint256 timeSoFarToday = ZBOOFI.timeSinceLastDailyUpdate() <= SECONDS_PER_DAY ? ZBOOFI.timeSinceLastDailyUpdate() : SECONDS_PER_DAY;
        uint256 withdrawalsSoFarToday = ZBOOFI.totalWithdrawals() - ZBOOFI.rollingStartTotalWithdrawals();
...
contract ZombieBOOFI is ERC20WithVotingAndPermit("TEST ZombieBOOFI", "TEST_zBOOFI"), Ownable {
    ...
    uint256 public constant MAX_BIPS = 10000;
    uint256 public constant SECONDS_PER_DAY = 86400;
    //scaled up by 1e18. e.g. 100e18 is zBOOFI per BOOFI
    ...
    constructor(IERC20 _boofi) {
        ...
        historicExchangeRates.push(INIT_EXCHANGE_RATE);
    ...
    function withdrawalFee() public view returns(uint256) {
        if (!withdrawalFeeEnabled) {
            ...
            uint256 withdrawalFeeValue = zBOOFI_WithdrawalFeeCalculator(withdrawalFeeCalculator).withdrawalFee();
    ...
    function enter(uint256 _amount) external {
        ...
        uint256 totalShares = totalSupply();
        // If no zBOOFI exists, mint it 1:1 to the amount put in
        if (totalShares == 0 || totalBoofi == 0) {
            _mint(msg.sender, (_amount * (INIT_EXCHANGE_RATE) / 1e18));
        }
    ...
    function leave(uint256 _share) external {
        ...
        //send withdrawn BOOFI
        uint256 tax = (what * withdrawalFee()) / MAX_BIPS;
        uint256 toSend = what - tax;
        // Send the person's BOOFI to their address
        boofi.safeTransfer(msg.sender, toSend);
    ...
    function _transfer(
        ...
        deposits[recipient] += boofiValueOfShares;
        withdrawals[sender] += boofiValueOfShares;
