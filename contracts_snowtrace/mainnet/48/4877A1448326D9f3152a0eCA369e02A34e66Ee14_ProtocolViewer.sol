...
interface IStreamingFeeModule {
  function feeStates(IDomani _domani) external view returns (StreamingFeeModule.FeeState memory);

  function getFee(IDomani _domani) external view returns (uint256);
}
 ...
 * @author Domani Protocol
 *
 * DomaniViewer enables batch queries of Domani state.
 *
 * UPDATE:
 * - Added getDomaniDetails functions
 */
contract DomaniViewer {
  struct DomaniDetails {
    string name;
    string symbol;
    address manager;
    address[] modules;
    IDomani.ModuleState[] moduleStatuses;
    IDomani.Position[] positions;
    uint256 totalSupply;
  }

  function batchFetchManagers(IDomani[] memory _domanis) external view returns (address[] memory) {
    address[] memory managers = new address[](_domanis.length);

    for (uint256 i = 0; i < _domanis.length; i++) {
      managers[i] = _domanis[i].manager();
    }
    return managers;
  }

  function batchFetchModuleStates(IDomani[] memory _domanis, address[] calldata _modules)
    public
    view
    returns (IDomani.ModuleState[][] memory)
  {
    IDomani.ModuleState[][] memory states = new IDomani.ModuleState[][](_domanis.length);
    for (uint256 i = 0; i < _domanis.length; i++) {
      IDomani.ModuleState[] memory moduleStates = new IDomani.ModuleState[](_modules.length);
      for (uint256 j = 0; j < _modules.length; j++) {
        moduleStates[j] = _domanis[i].moduleStates(_modules[j]);
      }
  ...
  function batchFetchDetails(IDomani[] memory _domanis, address[] calldata _moduleList)
    public
    view
    returns (DomaniDetails[] memory)
  {
    IDomani.ModuleState[][] memory moduleStates = batchFetchModuleStates(_domanis, _moduleList);

    DomaniDetails[] memory details = new DomaniDetails[](_domanis.length);
    for (uint256 i = 0; i < _domanis.length; i++) {
      IDomani domani = _domanis[i];

      details[i] = DomaniDetails({
        name: ERC20(address(domani)).name(),
        symbol: ERC20(address(domani)).symbol(),
        manager: domani.manager(),
        modules: domani.getModules(),
        moduleStatuses: moduleStates[i],
        positions: domani.getPositions(),
        totalSupply: domani.totalSupply()
      });
    }
    return details;
  }

  function getDomaniDetails(IDomani _domani, address[] calldata _moduleList)
    external
    view
    returns (DomaniDetails memory)
  {
    IDomani[] memory setAddressForBatchFetch = new IDomani[](1);
    setAddressForBatchFetch[0] = _domani;
...
/*
    Copyright 2021 Domani Labs Inc.
    Licensed under the Apache License, Version 2.0 (the "License");
    you may not use this file except in compliance with the License.
    You may obtain a copy of the License at
    http://www.apache.org/licenses/LICENSE-2.0
...
import {DomaniViewer} from "./DomaniViewer.sol";
 ...
 */
contract ProtocolViewer is ERC20Viewer, DomaniViewer, StreamingFeeModuleViewer {
...
contract StreamingFeeModuleViewer {
  ...
  function batchFetchStreamingFeeInfo(
    ...
    IDomani[] memory _domanis
  ) external view returns (StreamingFeeInfo[] memory) {
    StreamingFeeInfo[] memory feeInfo = new StreamingFeeInfo[](_domanis.length);

    for (uint256 i = 0; i < _domanis.length; i++) {
      StreamingFeeModule.FeeState memory feeState = _streamingFeeModule.feeStates(_domanis[i]);
      uint256 unaccruedFees = _streamingFeeModule.getFee(_domanis[i]);
 ...
 * @author Domani Protocol
 *
 * Smart contract that accrues streaming fees for Domani managers. Streaming fees are denominated as percent
 * per year and realized as Domani inflation rewarded to the manager.
 */
contract StreamingFeeModule is ModuleBase, ReentrancyGuard {
  ...
  struct FeeState {
    ...
    uint256 streamingFeePercentage; // Percent of Domani accruing to manager annually (1% = 1e16, 100% = 1e18)
    uint256 lastStreamingFeeTimestamp; // Timestamp last streaming fee was accrued
  }

  /* ============ Events ============ */

  event FeeActualized(address indexed _domani, uint256 _managerFee, uint256 _protocolFee);
  event StreamingFeeUpdated(address indexed _domani, uint256 _newStreamingFee);
  event FeeRecipientUpdated(address indexed _domani, address _newFeeRecipient);
  ...
  /*
   * Calculates total inflation percentage then mints new Domani tokens to the fee recipient. Position units are
   ...
   */
  function accrueFee(IDomani _domani) public nonReentrant onlyValidAndInitializedDomani(_domani) {
    uint256 managerFee;
    uint256 protocolFee;

    if (_streamingFeePercentage(_domani) > 0) {
      uint256 inflationFeePercentage = _calculateStreamingFee(_domani);

      // Calculate incentiveFee inflation
      uint256 feeQuantity = _calculateStreamingFeeInflation(_domani, inflationFeePercentage);

      // Mint new Domani tokens to manager and protocol
      (managerFee, protocolFee) = _mintManagerAndProtocolFee(_domani, feeQuantity);

      _editPositionMultiplier(_domani, inflationFeePercentage);
    }

    // solhint-disable-next-line not-rely-on-time
    feeStates[_domani].lastStreamingFeeTimestamp = block.timestamp;

    emit FeeActualized(address(_domani), managerFee, protocolFee);
  }

  /**
   * SET MANAGER ONLY. Initialize module with Domani and set the fee state for the Domani. Passed
   * _settings will have lastStreamingFeeTimestamp over-written.
   *
   * @param _domani                 Address of Domani
   * @param _settings                 FeeState struct defining fee parameters
   */
  function initialize(IDomani _domani, FeeState memory _settings)
    external
    ...
    onlyValidAndPendingDomani(_domani)
  {
    ...
    feeStates[_domani] = _settings;
    _domani.initializeModule();
  }

  /**
   * Removes this module from the Domani, via call by the Domani. Manager's feeState is deleted. Fees
  ...
  /*
   * Domani new streaming fee. Fees accrue at current rate then new rate is set.
   ...
   */
  function updateStreamingFee(IDomani _domani, uint256 _newFee)
    external
    onlyDomaniManager(_domani, msg.sender)
    onlyValidAndInitializedDomani(_domani)
  {
    require(_newFee < _maxStreamingFeePercentage(_domani), "Fee must be less than max");
    accrueFee(_domani);

    feeStates[_domani].streamingFeePercentage = _newFee;

    emit StreamingFeeUpdated(address(_domani), _newFee);
  }

  /*
   * Domani new fee recipient.
   *
   * @param _domani             Address of Domani
    ...
    onlyValidAndInitializedDomani(_domani)
  {
    ...
    feeStates[_domani].feeRecipient = _newFeeRecipient;

    emit FeeRecipientUpdated(address(_domani), _newFeeRecipient);
  }
   ...
   */
  function getFee(IDomani _domani) external view returns (uint256) {
    return _calculateStreamingFee(_domani);
  }
   ...
   *
   * @param  _domani          Address of Domani to have feeState updated
   * @return uint256            Streaming fee denominated in percentage of totalSupply
   */
  function _calculateStreamingFee(IDomani _domani) internal view returns (uint256) {
    // solhint-disable-next-line not-rely-on-time
    uint256 timeSinceLastFee = block.timestamp.sub(_lastStreamingFeeTimestamp(_domani));

    // Streaming fee is streaming fee times years since last fee
    return timeSinceLastFee.mul(_streamingFeePercentage(_domani)).div(ONE_YEAR_IN_SECONDS);
  }

  /**
   * Returns the new incentive fee denominated in the number of Domani tokens to mint. The calculation for the fee involves
   * implying mint quantity so that the feeRecipient owns the fee percentage of the entire supply of the Domani.
   *
   ...
   *
   * @param   _domani               Domani instance
   * @param   _feePercentage          Fee levied to feeRecipient
   * @return  uint256                 New RebalancingDomani issue quantity
   */
  function _calculateStreamingFeeInflation(IDomani _domani, uint256 _feePercentage)
    internal
    view
    returns (uint256)
  {
  ...
  /**
   * Mints sets to both the manager and the protocol. Protocol takes a percentage fee of the total amount of Domani tokens
   * minted to manager.
   *
   * @param   _domani               Domani instance
   * @param   _feeQuantity            Amount of Domani tokens to be minted as fees
   * @return  uint256                 Amount of Domani tokens accrued to manager as fee
   * @return  uint256                 Amount of Domani tokens accrued to protocol as fee
   */
  function _mintManagerAndProtocolFee(IDomani _domani, uint256 _feeQuantity)
    internal
    returns (uint256, uint256)
  {
    ...
    _domani.mint(_feeRecipient(_domani), managerFeeAmount);

    if (protocolFeeAmount > 0) {
      _domani.mint(protocolFeeRecipient, protocolFeeAmount);
    }
   ...
   */
  function _editPositionMultiplier(IDomani _domani, uint256 _inflationFee) internal {
    int256 currentMultipler = _domani.positionMultiplier();
    int256 newMultiplier = currentMultipler.preciseMul(
      PreciseUnitMath.preciseUnit().sub(_inflationFee).toInt256()
    );

    _domani.editPositionMultiplier(newMultiplier);
  }
