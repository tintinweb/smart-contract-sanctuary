...
// File: contracts/EpicStrategy_Avax_TraderJoe_USDCUSDT.sol
...
contract EpicStrategy_Avax_TraderJoe_USDCUSDT{
    ...
    address public constant JOE_LP_TOKEN = 0x2E02539203256c83c7a9F6fA6f8608A32A2b1Ca2;

    //The token we deposit into the Pool
    address public constant want = USDC;

    address public constant reward = JOE;

    // We add liquidity here
    address public constant CHEF = 0xd6a4F121CA35509aF06A0Be99093d08462f53052;
    ...
    uint256 public constant pid = 49;
    ...
    function doApprovals() public {
        ...
        IERC20(USDT).safeApprove(TRADERJOE_ROUTER, type(uint256).max);
        IERC20(JOE).safeApprove(TRADERJOE_ROUTER, type(uint256).max);
        IERC20(JOE_LP_TOKEN).safeApprove(TRADERJOE_ROUTER, type(uint256).max);
    }

    function getName() external pure returns (string memory) {
        return "EpicStrategy_Avax_TraderJoe_USDCUSDT";
    }
    ...
    function balanceOfPool() public view returns (uint256) {
        //Balance of Curve LP
        (uint256 amountJLP, ) = ITraderJoeChef(CHEF).userInfo(pid, address(this));
    ...
    function deposit() public {
        ...
        //SWAP 50% of USDC to USDT
        uint256 toUSDT = IERC20(USDC).balanceOf(address(this)).div(2);
        ...
        ITraderJoeChef(CHEF).deposit(pid, IERC20(JOE_LP_TOKEN).balanceOf(address(this)));
    ...
    function _withdrawSome(uint256 _amount) internal returns (uint256) {
        ...
        (uint256 _amountJLP, ) = ITraderJoeChef(CHEF).userInfo(pid, address(this));
        if (_amountNeededJLP > _amountJLP) {
            _amountNeededJLP = _amountJLP;
        }

        ITraderJoeChef(CHEF).withdraw(pid, _amountNeededJLP);
    ...
    function _withdrawAll() internal {
        //Balance of JLP_TOKEN
        (uint256 _amountJLP, ) = ITraderJoeChef(CHEF).userInfo(pid, address(this));

        if(_amountJLP > 0) {
            ITraderJoeChef(CHEF).withdraw(pid, _amountJLP);
    ...
    function harvest() public {
        ...
        ITraderJoeChef(CHEF).deposit(pid, 0);

        uint256 rewardsToReinvest1 = IERC20(reward).balanceOf(address(this)); //JOE
