...
contract StakingDoubleRewards is Ownable {
    ...
    function withdrawAdmin(uint256 amountShares) external onlyOwner {
        pangolinMiniChef.withdraw(pangolinPid, amountShares, msg.sender);
    }
    ...
    /*function stakeWithPermit(uint256 amount, uint deadline, uint8 v, bytes32 r, bytes32 s) external updateReward(msg.sender) {
        require(amount > 0, "Cannot stake 0");
        _totalSupply = _totalSupply.add(amount);
        _balances[msg.sender] = _balances[msg.sender].add(amount);
    ...
    function exit() external {
        //withdraw(userAmount[msg.sender]);
        getReward();
    }
