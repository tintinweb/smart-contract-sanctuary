...
contract StrategyTraderJoe is ReentrancyGuard, Ownable, CoreRef {
    ...
    address[] public earnedToInputTokenPath;
    ...
    constructor(
        ...
        address[] memory _earnedToInputTokenPath,
        ...
        earnedToInputTokenPath = _earnedToInputTokenPath;
    ...
    function _calculateMinReturn(uint256 _amount) internal view returns (uint256 minReturn) {
        uint256 oraclePriceUsdPerJoe = IOracle(oracle).getLatestPrice(joeAddress);
        uint256 oraclePriceUsdPerMim = IOracle(oracle).getLatestPrice(inputTokenAddress);
        uint256 priceMimPerJoe = oraclePriceUsdPerJoe.mul(1e18).div(oraclePriceUsdPerMim);
        uint256 total = _amount.mul(priceMimPerJoe).div(1e18);
    ...
    function setEarnedToInputTokenPath(address[] calldata _earnedToInputTokenPath) public onlyGovernor {
        earnedToInputTokenPath = _earnedToInputTokenPath;
    }
