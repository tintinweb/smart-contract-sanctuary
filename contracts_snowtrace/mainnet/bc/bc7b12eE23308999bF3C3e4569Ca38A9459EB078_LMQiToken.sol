...
contract LMQiToken is ERC20,proxyOwner {
    ...
    function swapTraderJoe(address token,uint256 oldAvaxBalance)internal{
        ...
        if (token != address(0)){
            ...
            uint[] memory amountOut = IUniswapV2Router02(traderJoe).getAmountsOut(balance, path);
            uint256 minOut = amountOut[amountOut.length-1]*98/100;
            IUniswapV2Router02(traderJoe).swapExactTokensForTokens(balance,minOut,path,address(this),block.timestamp+30);
        }else{
            uint256 balance = address(this).balance;
            balance = balance.sub(oldAvaxBalance);
            address[] memory path = getSwapRouterPath(token);
            uint[] memory amountOut = IUniswapV2Router02(traderJoe).getAmountsOut(balance, path);
            uint256 minOut = amountOut[amountOut.length-1]*98/100;
            IUniswapV2Router02(traderJoe).swapExactETHForTokens{value : balance}(minOut,path,address(this),block.timestamp+30);
        }
    }
