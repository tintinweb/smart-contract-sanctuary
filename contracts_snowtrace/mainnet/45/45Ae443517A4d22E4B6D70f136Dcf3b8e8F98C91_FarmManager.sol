...
contract FarmManager is Ownable, Pausable {
    ...
    IOliveBurner public burner;
    mapping(address => bool) public managers;
    uint256 public allocLimit;
    bool public onlyLPs;

    constructor(IMasterChef _chef, IOliveBurner _burner) public {
        chef = _chef;
        burner = _burner;
    }
    ...
    function changeChefOwner(address _newOwner) public onlyOwner {
        Ownable(address(chef)).transferOwnership(_newOwner);
    }


    //// Olive Burner

    function burn() public onlyManager {
        burner.burn();
    }

    function changeBurnerOwner(address _newOwner) public onlyOwner {
        Ownable(address(burner)).transferOwnership(_newOwner);
    }
}