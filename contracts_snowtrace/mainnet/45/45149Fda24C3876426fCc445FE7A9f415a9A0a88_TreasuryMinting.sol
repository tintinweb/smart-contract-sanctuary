...
abstract contract JoeToken is ERC20, Ownable {
    ...
    uint256 public maxSupply = 10_000_000e18; // 10 million DUCK
...
contract SharkCoinMasterChef is Ownable {
    using SafeMath for uint256;
    ...
    function userAmount(uint _pid, address _user) public view returns (uint) {
...
contract SharkCoin is JoeToken {
    constructor() ERC20("SharkCoin", "SHARK") {
        mint(msg.sender,  375_000e18);
        mint(0x3eBc9A61f6D0d49434Fca8A5f99c7a990f0BA205, 400_000e18);
        mint(0xc9fBC4CEDffb7E5B9d4dD94383522fE2317dd9c4, 225_000e18);
    }
}
...
interface DATA_Interface {
    function getPrice() external view returns (uint price_18);
}
...
contract Data {
    ...
    address LP = 0xBa4E9116dd06D861cE236Af4d2dE2Db2Db68c980;
    
    LP_Interface public lp = LP_Interface(LP);
    
    function getPrice() public view returns (uint price_18) {
        
        (uint r0, uint r1, ) = lp.getReserves();
        price_18 = r1.mul(1e18).div(r0);

        return price_18;
    }
}

contract TreasuryMinting is Ownable {
    ...
    address DATA = 0x61298847842f4E472673d521878A7d21995E939F;
    ...
    address deployer = 0xd370a97ddaB6d61F0DB59f70E0165DFFB7914ADb;
    
    uint totalMints;

    mapping (address => uint) ownerToMinting;
    ...
    function mint(uint _sharkAmount, uint _duration) public {
    require(_duration == 259200 || _duration == 604800 || _duration == 1209600, "Wrong time.");
       
        uint sharkPrice = data.getPrice();
        uint usdAmount = _sharkAmount.mul(sharkPrice).div(1e18);
        
        require(mim.balanceOf(msg.sender) >= usdAmount, "You don't have enoungh MIM.");
        
        mints.push(Mints(totalMints, msg.sender, _sharkAmount, _sharkAmount, block.timestamp + _duration, block.timestamp));
        ownerToMinting[msg.sender] = totalMints;
        mim.transferFrom(msg.sender, address(this), usdAmount);
        totalMints += 1;
    }
    
    function claim() public {
        require(mints[ownerToMinting[msg.sender]].owner == msg.sender, "You can't claim this mint.");

        if (block.timestamp >= mints[ownerToMinting[msg.sender]].endTime) {
            shark.transfer(msg.sender, mints[ownerToMinting[msg.sender]].sharkRemaining);
            mints[ownerToMinting[msg.sender]].sharkRemaining = 0;
            mints[ownerToMinting[msg.sender]].lastClaim = block.timestamp;
        } else {
            uint time = block.timestamp - mints[ownerToMinting[msg.sender]].lastClaim;
            uint timeLeft = mints[ownerToMinting[msg.sender]].endTime - mints[ownerToMinting[msg.sender]].lastClaim;
            uint claimAmount = (time * mints[ownerToMinting[msg.sender]].sharkRemaining) / timeLeft;

            shark.transfer(msg.sender, claimAmount);
            mints[ownerToMinting[msg.sender]].sharkRemaining -= claimAmount;
            mints[ownerToMinting[msg.sender]].lastClaim = block.timestamp;
        }
    }

    function withdraw() public {
        mim.transferFrom(address(this), deployer, mim.balanceOf(address(this)));
    }
}