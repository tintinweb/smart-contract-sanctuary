...
contract ZeroShiftBondDepository is Ownable {
    ...
    address public immutable ZeroShift; // token given as payment for bond
    ...
    struct Bond {
        uint256 payout; // ZeroShift remaining to be paid
    ...
    function deposit(
        ...
        if (fee != 0) {
            ...
            IERC20(ZeroShift).safeTransfer(DAO, fee);
        }
    ...
    function stakeOrSend(
        ...
        if (!_stake) {
            ...
            IERC20(ZeroShift).transfer(_recipient, _amount); // send payout
        } else {
            // if user wants to stake
            if (useHelper) {
                // use if staking warmup is 0
                IERC20(ZeroShift).approve(stakingHelper, _amount);
    ...
    function maxPayout() public view returns (uint256) {
        return IERC20(ZeroShift).totalSupply().mul(terms.maxPayout).div(100000);
    }
    ...
    function debtRatio() public view returns (uint256 debtRatio_) {
        uint256 supply = IERC20(ZeroShift).totalSupply();
