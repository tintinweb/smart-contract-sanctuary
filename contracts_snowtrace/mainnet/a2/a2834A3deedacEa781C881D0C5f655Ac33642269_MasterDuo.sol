...
contract MasterDuo is Ownable, ReentrancyGuard{
    ...
    struct PoolInfo {
        ...
        uint allocPoint;       // How many allocation points assigned to this pool. SINGs to distribute per block.
        uint ethAlloc;
        uint lastRewardTime;  // Last block number that SINGs distribution occurs.
        uint accSingPerShare;   // Accumulated SINGs per share, times 1e12. See below.
        uint accEthPerShare;
    ...
    constructor(Duo _sing,address _devaddr,address _feeAddress,uint _singPerSec,uint _startTime) public { 
    ...
    function pendingTokens(uint _pid, address _user) external view returns (uint,uint) {//SING , ETH
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint accSingPerShare = pool.accSingPerShare;
        ...
        if (block.timestamp > pool.lastRewardTime && pool.totalcap != 0 && totalAllocPoint>0) {
            ...
            uint singReward = multiplier.mul(singPerSec).mul(pool.allocPoint).div(totalAllocPoint);
        ...
        return (user.amount.mul(accSingPerShare).div(1e12).sub(user.rewardDebt),user.amount.mul(accEthPerShare).div(1e12).sub(user.ethDebt));
    }
    ...
    function deposit(uint _pid, uint _amount) external onlyEOA nonReentrant{
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint pending = user.amount.mul(pool.accSingPerShare).div(1e12).sub(user.rewardDebt);
    ...
    function safeSingTransfer(address _to, uint _amount) internal {
        uint singBal = sing.balanceOf(address(this));
        if(_amount>0 || singBal>0){
            if (_amount > singBal) {
    ...
    function safeEthTransfer(address _to, uint _amount) internal {
        uint ethBal = sing.balanceOf(address(this));
    ...
    function updateEmissionRate(uint _singPerSec) external onlyOwner {
