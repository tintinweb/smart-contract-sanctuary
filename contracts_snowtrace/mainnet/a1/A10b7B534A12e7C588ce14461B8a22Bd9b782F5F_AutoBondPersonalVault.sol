...
contract AutoBondPersonalVault {
    ...
    constructor(
        ...
        address _timeMimLpBondDepository,
        address _mim,
        address _timeMimJLP,
        uint256 _fee,
        address _joeRouter,
        ...
        mim = IERC20(_mim);
        require(_timeMimJLP != address(0));
        timeMimJLP = IERC20(_timeMimJLP);
        ...
        require(_timeMimLpBondDepository != address(0));
        timeMimLpBondDepository = ITimeBondDepository(_timeMimLpBondDepository);
        ...
        returns (uint256)
    {
        ...
        uint256 received = _sellAssetFor(mim, _amount, _slippage);
        uint256 payout = _bondWith(mim, received, mimBondDepository);
        require(
            isBondProfitable(_amount, payout),
            "Bonding rate worse than staking"
        );
        emit BondCreated(_amount, address(mim), payout);
        return payout;
    }
    ...
    function stakeAssets(uint256 _amount) public only(manager) {
    ...
    function unstakeAssets(uint256 _amount) public only(manager) {
        require(
        ...
        returns (bool _profitable)
    {
        ...
        uint256 circualtingSupply = IMemories(address(stakedAsset))
            .circulatingSupply();
