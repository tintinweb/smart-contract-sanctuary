...
contract MIMStakeDepository is Ownable {
    ...
    address public immutable PAPA; // intermediate reward token from treasury
    address public immutable sPAPA; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints PAPA when receives principle
    ...
    struct Bond {
        uint gonsPayout; // sPAPA gons remaining to be paid
        uint papaPayout; //papa amount at the moment of bond
    ...
    function deposit( 
        ...
        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 PAPA ( underflow protection )
        ...
        /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) PAPA
         */
        ...
        if ( fee != 0 ) { // fee is transferred to dao 
            IERC20( PAPA ).safeTransfer( DAO, fee ); 
        }
        ...
        IERC20( PAPA ).approve( staking, payout );
        IStaking( staking ).stake( payout, address(this) );
        IStaking( staking ).claim( address(this) );
        uint stakeGons=ISPAPA(sPAPA).gonsForBalance(payout);

        // depositor info is stored
        _bondInfo[ _depositor ] = Bond({ 
            gonsPayout: _bondInfo[ _depositor ].gonsPayout.add( stakeGons ),
            papaPayout: _bondInfo[ _depositor ].papaPayout.add( payout ),
    ...
    function redeem( address _recipient, bool _stake) external returns ( uint ) {        
        ...
        uint _amount = ISPAPA(sPAPA).balanceForGons(info.gonsPayout);
        emit BondRedeemed( _recipient, _amount, 0 ); // emit bond data
        IERC20( sPAPA ).transfer( _recipient, _amount ); // pay user everything due
        return _amount;
    }
    ...
    function maxPayout() public view returns ( uint ) {
        return IERC20( PAPA ).totalSupply().mul( terms.maxPayout ).div( 100000 );
    }
    ...
    /**
     *  @notice return bond info with latest sPAPA balance calculated from gons
     *  @param _depositor address
     *  @return payout uint
     *  @return vesting uint
    ...
    function bondInfo(address _depositor) public view returns ( uint payout,uint vesting,uint lastBlock,uint pricePaid ) {
        ...
        payout=ISPAPA(sPAPA).balanceForGons(info.gonsPayout);
    ...
    /**
     *  @notice calculate current ratio of debt to PAPA supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns ( uint debtRatio_ ) {   
        uint supply = IERC20( PAPA ).totalSupply();
    ...
    /**
     *  @notice calculate amount of PAPA available for claim by depositor
     *  @param _depositor address
     *  @return pendingPayout_ uint
     */
    function pendingPayoutFor( address _depositor ) external view returns ( uint pendingPayout_ ) {
        uint percentVested = percentVestedFor( _depositor );
        uint payout = ISPAPA(sPAPA).balanceForGons(_bondInfo[ _depositor ].gonsPayout);
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or PAPA) to the DAO
     *  @return bool
     */
    function recoverLostToken( address _token ) external returns ( bool ) {
        require( _token != PAPA );
        require( _token != sPAPA );
