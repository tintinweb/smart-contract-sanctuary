...
contract ApeKing is Context, IERC20, Ownable {
    ...
    address payable public marketingAddress = payable(0xd790618636FE128c52c0A67d331385eCFc56C736);
    address public marketingWalletToken = 0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7;
    address public constant deadAddress = 0x000000000000000000000000000000000000dEaD;
    
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) private _isExcludedFromFeesApeking;
    mapping (address => bool) private _isExcludedFromMaxBalanceApeking;

    uint256 private constant _maxFeesApeking = 10;
    uint256 private _totalFeesApeking;
    uint256 private _totalFeesApekingContract;
    uint256 private _burnFeesApeking;
    uint256 private _liquidityFeesApeking;
    uint256 private _marketingFeeApeking;
    
    uint256 private _maxBalanceWalletApeking;
    ...
    constructor () {
       IJoeRouter02 _joeRouter = IJoeRouter02(0x60aE616a2155Ee3d9A68541Ba4544862310933d4);
        ...
        _isExcludedFromFeesApeking[owner()] = true;
        _isExcludedFromFeesApeking[address(this)] = true;
        
        _isExcludedFromMaxBalanceApeking[owner()] = true;
        _isExcludedFromMaxBalanceApeking[address(this)] = true;
        _isExcludedFromMaxBalanceApeking[joePair] = true;

        _liquidityFeesApeking = 4;
        _marketingFeeApeking = 2;
        _burnFeesApeking = 1;
        _totalFeesApeking = _liquidityFeesApeking.add(_marketingFeeApeking).add(_burnFeesApeking);
        _totalFeesApekingContract = _liquidityFeesApeking.add(_marketingFeeApeking);

        _liquidityThreshhold = 50 * 10**9 * 10**_decimals;
        _maxBalanceWalletApeking = 1000 * 10**9 * 10**_decimals;
    ...
    function setLiquidityFeePercent(uint256 newLiquidityFee) external onlyOwner() {
        ...
        require(newLiquidityFee.add(_burnFeesApeking).add(_marketingFeeApeking) <= _maxFeesApeking, "Fees are too high.");
        _liquidityFeesApeking = newLiquidityFee;
        _totalFeesApeking = _liquidityFeesApeking.add(_marketingFeeApeking).add(_burnFeesApeking);
        _totalFeesApekingContract = _liquidityFeesApeking.add(_marketingFeeApeking);
        emit SetLiquidityFee(_liquidityFeesApeking);
    }

    function setMarketingFeePercent(uint256 newMarketingFee) external onlyOwner() {
        require(!inSwapAndLiquify, "inSwapAndLiquify");
        require(_liquidityFeesApeking.add(_burnFeesApeking).add(newMarketingFee) <= _maxFeesApeking, "Fees are too high.");
        _marketingFeeApeking = newMarketingFee;
        _totalFeesApeking = _liquidityFeesApeking.add(_marketingFeeApeking).add(_burnFeesApeking);
        _totalFeesApekingContract = _liquidityFeesApeking.add(_marketingFeeApeking);
        emit SetMarketingFee(_marketingFeeApeking);
    }

    function setBurnFeePercent(uint256 newBurnFee) external onlyOwner() {
        require(_liquidityFeesApeking.add(newBurnFee).add(_marketingFeeApeking) <= _maxFeesApeking, "Fees are too high.");
        _burnFeesApeking = newBurnFee;
        _totalFeesApeking = _liquidityFeesApeking.add(_marketingFeeApeking).add(_burnFeesApeking);
        emit SetBurnFee(_burnFeesApeking);
    }
    ...
    function maxBalance() public view returns (uint256) {
        return _maxBalanceWalletApeking;
    }
    ...
    function _transfer(
        ...
        // Make sure that: Balance + Buy Amount <= _maxBalanceWalletApeking
        if(
            from != owner() &&              // Not from Owner
            to != owner() &&                // Not to Owner
            !_isExcludedFromMaxBalanceApeking[to]  // is excludedFromMaxBalance
        ){
            require(
                balanceOf(to).add(amount) <= _maxBalanceWalletApeking,
        ...
        if(
            ...
            _totalFeesApekingContract > 0 &&                         // LiquidityFee + MarketingFee > 0
            from != owner() &&                                  // Not from Owner
            to != owner()                                       // Not to Owner
        ) {
            collectFees();
        ...
        // Take Fees 
        if(
            !(_isExcludedFromFeesApeking[from] || _isExcludedFromFeesApeking[to])
            && _totalFeesApeking > 0
        ) {
            
        	uint256 feesToContract = amount.mul(_totalFeesApekingContract).div(100);
            uint256 toBurnAmount = amount.mul(_burnFeesApeking).div(100);
    ...
    function collectFees() private lockTheSwap {
        ...
        uint256 liquidityTokensToSell = balanceOf(address(this)).mul(_liquidityFeesApeking).div(_totalFeesApekingContract);
        uint256 marketingTokensToSell = balanceOf(address(this)).mul(_marketingFeeApeking).div(_totalFeesApekingContract);
