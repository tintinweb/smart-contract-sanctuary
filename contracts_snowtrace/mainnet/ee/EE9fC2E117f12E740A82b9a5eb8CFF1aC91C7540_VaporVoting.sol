...
contract VaporVoting is Ownable {
    address public nodeManagerAddress;
    INodeManager private nodeManager;

    constructor(address nodeManagerAddress_) {
        nodeManagerAddress = nodeManagerAddress_;
        nodeManager = INodeManager(nodeManagerAddress_);
    }

    function updateNodeManagerAddress(address newVal) external onlyOwner {
        nodeManagerAddress = newVal;
        nodeManager = INodeManager(newVal);
    }

    function getAllNodesAmount() external view returns (uint256) {
        INodeManager.NodeEntity[] memory nodes = nodeManager.getAllNodes(_msgSender());
