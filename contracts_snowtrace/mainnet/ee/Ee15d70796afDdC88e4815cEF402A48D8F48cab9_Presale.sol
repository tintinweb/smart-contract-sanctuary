...
interface PrezaleInterface {
    ...
    function getPaidFees(address tokenAddress) external view returns(uint256 websiteTokensFee , uint256 websiteBNBFee );
...
contract Presale {
    ...
    function getPaidFees() public view returns (uint256 feeAmount , uint256 tokenFeeAmount) {
        (uint256 websiteTokensFee , uint256 websiteBNBFee ) = PrezaleInterface(parentContract).getPaidFees(tokenAddress);
        feeAmount = websiteBNBFee;
        tokenFeeAmount = websiteTokensFee;
        return (websiteBNBFee, websiteTokensFee);
    }
