...
contract ZeroShiftPresale is Ownable {
    ...
    uint256 public priceDai = 5 * 1e18; // 5 DAI per ZRST. DAI token has 18 decimals.

    uint256 public cap = 500 * 1e6; // 500 USD cap per whitelisted user
    uint256 public capDai = 500 * 1e18; // 500 DAI cap per whitelisted team member
    ...
    function depositTeam(uint256 _amount) external checkIfPaused {
        ...
        require(
            capDai.mul(team.numWhitelist) >= team.amount.add(_amount),
        ...
        uint256 payout = _amount.mul(1e18).div(priceDai); // ZRST debt to claim
