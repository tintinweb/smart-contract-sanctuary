...
contract qiApyCal {
    ...
    address[] public qiTokens = [0x5C0401e81Bc07Ca70fAD469b451682c0d747Ef1c,//qiavax
                                ...
                                0x835866d37AFB8CB8F8334dCCdaf66cf01832Ff5D,//QI，decimal
...
function getDistributionApy()
        ...
        returns (uint256,uint256)
    {
        ...
        uint256 qiPrice = IOracle(oracleAddress).getPrice(qiAddress);
        uint256 avaxPrice = IOracle(oracleAddress).getPrice(address(0));

        for(uint256 i=0;i<qiTokens.length;i++) {
            uint256 qiTokenUnderLyingPrice =  IOracle(oracleAddress).getPrice(qiTokenUnderlyings[i]);
            uint256 total =  IQiToken(qiTokens[i]).totalSupply();

            uint256 qiRewardSpeed = IComptroller(ComptrollerAddress).rewardSpeeds(0,qiTokens[i]);
            qiRewardSpeed = qiRewardSpeed.mul(3600*24).mul(rayDecimals).mul(qiPrice).mul(underlyingDecimal[i]);
            qiRewardSpeed = (qiRewardSpeed.div(qiTokenUnderLyingPrice).div(total).div(10**(18+36))).add(rayDecimals);
            uint256 qiApy = rpower(qiRewardSpeed,365,rayDecimals).sub(rayDecimals);


            uint256 avaxRewardSpeed = IComptroller(ComptrollerAddress).rewardSpeeds(1,qiTokens[i]);
            avaxRewardSpeed = avaxRewardSpeed.mul(3600*24).mul(rayDecimals).mul(avaxPrice).mul(underlyingDecimal[i]);
            avaxRewardSpeed = (avaxRewardSpeed.div(qiTokenUnderLyingPrice).div(total).div(10**(18+36))).add(rayDecimals);
