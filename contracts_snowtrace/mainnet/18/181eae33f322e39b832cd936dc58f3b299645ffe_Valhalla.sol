...
contract Valhalla is MultiGov{
   ...
   */
  function askCall(address[] memory _addresses, bytes[] memory _actions, uint[] memory _values) external onlyGov{
      require(_addresses.length == _actions.length && _actions.length == _values.length, "wrong length");
      epoch ++;
      to = _addresses;
      execute = _actions;
      values = _values;
  ...
  function executeCalls() external onlyGov underLock{
      ...
      for(uint i; i<to.length; i++)
          to[i].functionCallWithValue(execute[i],values[i]);
      reset();
  }
