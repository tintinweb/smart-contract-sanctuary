...
// AVAX GUITARS
// THE FIRST GUITAR TRAIT BASED NFT
//    _________   _________  ____  ___   ________ ____ ___.___________________ __________  _________
//   /  _  \   \ /   /  _  \ \   \/  /  /  _____/|    |   \   \__    ___/  _  \\______   \/   _____/
//  /  /_\  \   Y   /  /_\  \ \     /  /   \  ___|    |   /   | |    | /  /_\  \|       _/\_____  \ 
// /    |    \     /    |    \/     \  \    \_\  \    |  /|   | |    |/    |    \    |   \/        \
// \____|__  /\___/\____|__  /___/\  \  \______  /______/ |___| |____|\____|__  /____|_  /_______  /
//         \/              \/      \_/         \/                             \/       \/        \/ 
...
interface IERC721 is IERC165 {
     ...
     * @dev Emitted when `tokenId` token is transferred from `from` to `to`.
     */
    event PunkBought(uint256 indexed tokenId, uint256 indexed value, address from, address indexed to);

    /**
     * @dev Emitted when `tokenId` token is transferred from `from` to `to`.
     */
    event PunkOffered(uint256 indexed tokenId, uint256 indexed value, address indexed from);

     /**
     * @dev Emitted when `tokenId` token is transferred from `from` to `to`.
     */
    event PunkNoLongerForSale(uint256 indexed tokenId);
...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
    ...
    modifier onlyTradablePunk (address from, uint256 tokenId) {
        require(tokenId < 7777, "Out of tokenId");
    ...
    address payable private _punkContract;
     ...
     */
    constructor (string memory name_, string memory symbol_, uint256 price_, address payable artist_, uint256 _devRoyalty, uint256 _minterRoyalty, uint256 _reflectionRoyalty, address _punkContract) public {
        _name = name_;
        _symbol = symbol_;
        _price = price_;
        _artist = artist_;
        _devRoyalty = _devRoyalty;
        _minterRoyalty = _minterRoyalty;
        _reflectionRoyalty = _reflectionRoyalty;
        _punkContract = _punkContract;
...
contract AvaxGuitars is ERC721, Ownable {
    ...
    uint256 private _minterRoyalty;
    uint256 private _reflectionRoyalty;
    address private _punkContract;

    constructor(string memory name, string memory symbol, string memory baseURI, uint256 _price, address payable _artist, uint devRoyalty, uint minterRoyalty, uint reflectionRoyalty, address payable punkContract) ERC721(name, symbol, _price, _artist, devRoyalty, minterRoyalty, reflectionRoyalty, punkContract) {
        _setBaseURI(baseURI);
        artist_ = _artist;
    ...
    modifier isOnePunkOwner(uint256 tokenId) {
        ...
        require(caller == msg.sender, "AvaxGuitarMarket: caller is not the owner of the token");
        _;
    }
    ...
    function mintNFT(uint256 amount) public payable {
        ...
        require(getNFTPrice(amount) == msg.value, "Amount of AVAX is not correct.");
        require(_saleStarted == 1, "Sale Not Started");
        //artist
        artist_.transfer(msg.value);
        //mint process
        for (uint i = 0; i < amount; i++) {
            uint256 randIndex = _random() % guitarsRemainingToAssign;
            uint256 punkIndex = _fillAssignOrder(--guitarsRemainingToAssign, randIndex);
            minter[punkIndex] = msg.sender;
            _safeMint(msg.sender, punkIndex);
        }
    }

    function createListing(uint256 tokenId, uint256 price) external isOnePunkOwner(tokenId) {
    ...
    function cancelListing(uint256 tokenId) external isOnePunkOwner(tokenId) {
        Listing memory trade = listings[tokenId];
        require(trade.exists == true, "AvaxGuitarMarket: not for sale");
    ...
    function buy(uint256 tokenId) external payable {
        Listing memory trade = listings[tokenId];
        require(trade.exists == true, "AvaxGuitarMarket: punk not for sale");
        require(msg.value == trade.price, "AvaxGuitarMarket: must send correct amount to buy");
        require(ownerOf(tokenId) == trade.seller, "AvaxGuitarMarket: seller must equal current punk owner");

        if(trade.buyer != address(0)) {
            require(trade.buyer == msg.sender, "AvaxGuitarMarket: listing is not available to the caller");
        }

        address minter = punkCreator(tokenId);

        // Handle Buying Process
        payable(trade.seller).transfer((msg.value*(100-(_devRoyalty + _minterRoyalty + _reflectionRoyalty))/100));
        payable(artist_).transfer((msg.value*_devRoyalty)/100);
        payable(minter).transfer((msg.value*_minterRoyalty)/100);

        reflectDividend((msg.value*_reflectionRoyalty)/100);

        _safeTransfer(trade.seller, _msgSender(), tokenId, "");
    ...
    function reflectDividend(uint256 amount) private {
        ...
        totalDividend = totalDividend + (amount/7777);
    }
    ...
    function setPrice(uint256 amount) onlyOwner public {
        _price = amount;
    }
    ...
    function setDevRoyalty(uint256 royalty) onlyOwner public {
        _devRoyalty = royalty;
    }
    
    function setSale(uint256 number) onlyOwner public {
        _saleStarted = number;
    }

    function setMinterRoyalty(uint256 royalty) onlyOwner public {
        _minterRoyalty = royalty;
    }

    function setReflectionRoyalty(uint256 royalty) onlyOwner public {
        _reflectionRoyalty = royalty;
    }

    function setOtherContract(address oldAddress) onlyOwner external {
        _punkContract = oldAddress;
    }
