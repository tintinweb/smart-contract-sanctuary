...
import {IAvaxDexRouter as IJoeRouter} from "../interfaces/dex/IAvaxDexRouter.sol";

/// @notice Joe Handler used to execute an order
contract JoeHandler is IHandler {
    using SafeERC20 for IERC20;

    address public immutable WAVAX;
    address public immutable FACTORY;
    bytes32 public immutable FACTORY_CODE_HASH;
    IJoeRouter internal immutable joeRouter;
    address public immutable yolo;

    /**
     * @notice Creates the handler
     * @param _router - Address of the Joe router contract
     * @param _wavax - Address of WAVAX contract
     * @param _codeHash - Bytes32 of the Joe v2 pair contract unit code hash
    ...
    modifier onlyYolo() {
        require(
            msg.sender == yolo,
            "JoeHandler: only yolo contract can invoke this function"
        );
        _;
    }
        ...
        returns (uint256)
    {
        ...
        IJoeRouter _joeRouter = joeRouter;

        if (order.outputToken == WAVAX) {
            try
                _joeRouter.swapExactTokensForAVAX(
                    order.inputAmount,
                    amountOutMin,
                    path,
                    order.recipient,
            ...
            returns (uint256[] memory avaxAmount) {
                returnAmount = avaxAmount;
            } catch {
                IERC20(order.inputToken).approve(
                    address(_joeRouter),
                    type(uint256).max
                );
                returnAmount = _joeRouter.swapExactTokensForAVAX(
                    order.inputAmount,
                    amountOutMin,
                    path,
                    order.recipient,
            ...
            try
                _joeRouter.swapExactTokensForTokens(
                    order.inputAmount,
                    amountOutMin,
                    path,
                    order.recipient,
            ...
            returns (uint256[] memory tokenAmount) {
                ...
                returnAmount = _joeRouter.swapExactTokensForTokens(
                    order.inputAmount,
                    amountOutMin,
                    path,
                    order.recipient,
