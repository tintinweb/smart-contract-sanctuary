...
contract CocaineCash is Context, IERC20, Ownable {
	using Address for address payable;

	string constant NAME = "Cocaine Cash";
	string constant SYMBOL = "COKE";
	uint8 constant DECIMALS = 9;

	uint256 constant MAX_UINT = 2 ** 256 - 1;
	address constant ROUTER_ADDRESS = address(0x60aE616a2155Ee3d9A68541Ba4544862310933d4);
	...
	uint256 public maxTxAmountBuy = tTotal / 100;
	uint256 public maxTxAmountSell = tTotal / 100;
	...
	function _addLiquidity(uint256 amount) private {
		...
		router.addLiquidityAVAX{value: ethForLiquidity}(self, tokensForLiquidity, 0, 0, DEAD_ADDRESS, block.timestamp);
	}
	...
	function _swapExactTokensForETH(uint256 amountIn) private returns (uint256) {
		...
		router.swapExactTokensForAVAXSupportingFeeOnTransferTokens(amountIn, 0, path, self, block.timestamp);
	...
	function openTrading(uint256 tokensForLiquidity) external payable onlyOwner {
		...
		//Create pair
		address pairAddress = ITraderJoeFactory(router.factory()).createPair(self, router.WAVAX());
