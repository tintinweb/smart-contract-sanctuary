...
contract Hermes is ERC20Burnable, Operator {
    ...
    uint256 public constant INITIAL_LAUNCH_DISTRIBUTION = 100 ether;
     ...
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("HERMES", "HERMES") {
    ...
    function _getHermesPrice() internal view returns (uint256 _hermesPrice) {
        try IOracle(hermesOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Hermes: failed to fetch HERMES price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _hermesPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_hermesPrice >= taxTiersTwaps[tierId]) {
    ...
    function setHermesOracle(address _hermesOracle) public onlyOperatorOrTaxOffice {
        require(_hermesOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints HERMES to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of HERMES to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentHermesPrice = _getHermesPrice();
            currentTaxRate = _updateTaxRate(currentHermesPrice);
            if (currentHermesPrice < burnThreshold) {
                burnTax = true;
            }
        }
