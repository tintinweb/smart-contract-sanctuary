...
contract StrategyAdapterAvalanche is AlphaStrategyAvalanche {
    ...
    function initialize(
        ...
        AlphaStrategyAvalanche.initializeAlphaStrategy(
            ...
            _poolId,
            address(0x2352b745561e7e6FCD03c093cE7220e3e126ace0), // Masterchef #2
            24
        );
    }
}
...
contract AlphaStrategyAvalanche is OwnableUpgradeable {
    ...
    address public secondMasterchef;
    ...
    function initializeAlphaStrategy(
        ...
        secondMasterchef = _masterchefSecond;
    ...
    */
    ...
    function withdrawReward(address user) public onlyVault {
        ...
        if(_xJoeBalance < _pendingXJoe){
            ...
            IMasterChef(secondMasterchef).withdraw(poolIdSecond, toWithdraw);
    ...
    function masterChefBalance() internal view returns (uint256 amount) {
    ...
    function claimJoeRewardPool() internal {
        uint256 balance = xJoeMasterChefBalance();

        if (balance != 0) {
            IMasterChef(secondMasterchef).deposit(poolIdSecond, 0);
        }
    }

    function xJoeMasterChefBalance() internal view returns (uint256 amount) {
        (amount,) = IMasterChef(secondMasterchef).userInfo(poolIdSecond, address(this));
    }
    ...
    function enterXJoeRewardPool() internal {
        ...
        IERC20(xJoe).safeApprove(secondMasterchef, 0);
        IERC20(xJoe).safeApprove(secondMasterchef, entireBalance);

        IMasterChef(secondMasterchef).deposit(poolIdSecond, entireBalance);
    }
