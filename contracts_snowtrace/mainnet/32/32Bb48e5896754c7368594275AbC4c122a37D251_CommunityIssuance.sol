...
contract CommunityIssuance is ICommunityIssuance, BaseMath, Initializable {
    ...
    *
    * Minutes in five year: 60*24*365*5 = 2628000
    *
    ...
    */
    ...
    function upgradeReleaseRate() external {
        require(upgradeTime == 0, "Upgrade has been completed!");

        upgradeTime = block.timestamp;
        issuedBeforeUpgrade = totalLQTYIssued;
    ...
    function issueLQTY() external override returns (uint) {
        ...
        uint latestTotalLQTYIssued = newSupplyCap.mul(_getCumulativeIssuanceFraction()).div(DECIMAL_PRECISION).add(issuedBeforeUpgrade);
    ...
    function _getCumulativeIssuanceFraction() internal view returns (uint) {
        ...
        uint timePassedInMinutes = block.timestamp.sub(upgradeTime).div(SECONDS_IN_ONE_MINUTE);
