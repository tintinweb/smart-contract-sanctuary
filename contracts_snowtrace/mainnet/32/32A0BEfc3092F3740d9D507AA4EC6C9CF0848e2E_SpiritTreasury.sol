...
contract SpiritTreasury is Ownable {
    ...
    address public TOMB;
    uint public TOMBQueue; // Delays change to TOMB address
    ...
    constructor (
       ...
       /* isLiquidityToken[ _GHOSTMIM ] = true;
       liquidityTokens.push( _GHOSTMIM ); */

        blocksNeededForQueue = _blocksNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for GHOST
        @param _amount uint
        @param _token address
        @param _profit uint
        @return send_ uint
     */
    function deposit( uint _amount, address _token, uint _profit ) public returns ( uint send_ ) {
        ...
        // mint GHOST needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( GHOST ).mint( msg.sender, send_ );
    ...
    function depositAndMintFixed (uint _amount, address _token, uint _mintAmount ) external returns ( uint profit ) {
            uint value = valueOf(_token, _amount);
            profit = value.sub(_mintAmount);
            deposit(_amount, _token, profit);

    }
    /**
        @notice allow approved address to burn GHOST for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IGHOSTERC20( GHOST ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( TOMB ).balanceOf( msg.sender ); // Can only borrow against TOMB held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with GHOST
        @param _amount uint
     */
    function repayDebtWithGHOST( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IGHOSTERC20( GHOST ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, GHOST, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( GHOST ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( GHOST ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns GHOST valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match GHOST decimals
            value_ = _amount.mul( 10 ** IERC20( GHOST ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.TOMB ) { // 9
            TOMBQueue = block.number.add( blocksNeededForQueue );
        } else return false;
