...
contract CrowdSale is ICrowdSale, Ownable, ReentrancyGuard{
	 ...
	 * Referral records
	 */
	mapping(uint256 => Referral) private _referral;
	mapping(address => uint256) private _referralAddr;


	/*
	 * Percentage of ethers to transfer to the referring party. 
	 */
	uint8 private _refRewardPercent;

	/*
	 * Percentage of price discount for the referral purchases.
	 * Simply used for calculating, discount_price = price * (1+refDiscountPercent)
	 */
	uint8 private _refDiscountPercent;
	 ...
	 * @param price: price in wei 
	 */
	function getOffer(uint256 amount, string memory referralCode) public view override returns(uint256, uint256){
		Referral storage referral = _referral[uint256(keccak256(abi.encode(referralCode)))];

		uint256 price = _price;

		if(referral.owner != address(0) && referral.owner != msg.sender){
			price = _price + (_price * _refDiscountPercent) / 100; 

			if(amount / _price > 100){
				return (price, (amount + amount / 100 * _refDiscountPercent) * _price);
			}
		}
	 ...
	 * Origin: Account, Contract
	 */
	function purchase(string memory referralCode) external nonReentrant payable override returns(uint256 vestingID){
		...
		(uint256 price, uint256 requested) = getOffer(msg.value, referralCode);
		...
		// referral reward 
		if(referral.owner != address(0) && funded >= 100 && referral.owner != msg.sender){
			uint256 reward = (funded / 100) * _refRewardPercent;
			payable(referral.owner).transfer(reward);
			referral.count += 1;
			referral.gross += reward;
			require(referral.gross >= reward, "Sales> E21: Integer overflow");

			emit ReferralReward(referral.code, referral.owner, msg.sender, reward);
		}
	...
	/*
	 * Users can register a fixed length 9 char referral code which can be used for collecting commission fees..
	 */
	function registerReferral(string memory code) external override{
		require(_referral[uint256(keccak256(abi.encode(code)))].owner == address(0), "Sales> E19: Duplicated code! Please try with another code.");
		require(bytes(code).length <= 9 && bytes(code).length >= 5,                  "Sales: E20: Invalid referral code. It should be an alphanumeric value with min:5 max:9 chars.");

		uint256 refID = uint256(keccak256(abi.encode(code)));
		_referral[refID] = Referral(
			code,
			msg.sender,
			0,
			0
		...
		_referralAddr[msg.sender] = refID;

		emit ReferralRegister(
			code,
			msg.sender
		);
	}

	function getReferral(string memory code) external view override returns(Referral memory){
		return _referral[uint256(keccak256(abi.encode(code)))];
	}

	function getReferralAddr(address account) external view override returns(Referral memory){
		return _referral[_referralAddr[account]];
	}

	function getRefRewardPercent() external view override returns(uint8){
		return _refRewardPercent;
	}
	function getRefDiscountPercent() external view override returns(uint8){
		return _refDiscountPercent;
	}
	function setRefDiscountPercent(uint8 discount) external override {
		require(msg.sender == owner(), "Sales> E22: Authorization denied.");
		_refDiscountPercent = discount;
	}
	function setRefRewardPercent(uint8 reward) external override{
		require(msg.sender == owner(), "Sales> E23: Authorization denied.");
		_refRewardPercent = reward;
	}
