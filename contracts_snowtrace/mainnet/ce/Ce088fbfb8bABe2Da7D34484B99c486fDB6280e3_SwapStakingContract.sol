...
contract SwapStakingContract is Initializable, ContextUpgradeSafe, AccessControlUpgradeSafe, PausableUpgradeSafe, ReentrancyGuardUpgradeSafe {
    ...
    function setMaxStakingAmount(uint256 _maxStakingAmount)
    external
    {
        require(hasRole(OWNER_ROLE, _msgSender()), "must have owner role to set max cap");
        require(_maxStakingAmount > 0, "max cap has to be larger than 0");
        maxStakingAmount = _maxStakingAmount;
    }

    function setUnstakingPeriod(uint256 _unstakingPeriodInDays)
    external
    {
        require(hasRole(OWNER_ROLE, _msgSender()), "must have owner role to set unstaking period");
        unstakingPeriod = _unstakingPeriodInDays;
    }
