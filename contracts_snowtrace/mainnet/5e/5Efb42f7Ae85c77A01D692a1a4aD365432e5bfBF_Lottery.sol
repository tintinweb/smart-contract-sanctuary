...
contract LotteryNFT is ERC721, Ownable {
    ...
    constructor() public ERC721("Hertz Lottery Ticket", "HTZLT") {}
...
contract Lottery is OwnableUpgradeable {
    ...
    function buy(uint256 _price, uint8[4] memory _numbers) external {
        ...
        htz.safeTransferFrom(address(msg.sender), address(this), _price);
    ...
    function multiBuy(uint256 _price, uint8[4][] memory _numbers) external {
        ...
        htz.safeTransferFrom(address(msg.sender), address(this), totalPrice);
    ...
    function claimReward(uint256 _tokenId) external {
        ...
        if(reward>0) {
            safeHtzTransfer(address(msg.sender), reward);
        }
    ...
    function multiClaim(uint256[] memory _tickets) external {
        ...
        if(totalReward>0) {
            safeHtzTransfer(address(msg.sender), totalReward);
        }
    ...
    // Safe HTZ transfer function, just in case if rounding error causes pool to not have enough HTZs.
    function safeHtzTransfer(address _to, uint256 _amount) internal {
        uint256 htzBal = htz.balanceOf(address(this));
        if (_amount > htzBal) {
            htz.transfer(_to, htzBal);
        } else {
            htz.transfer(_to, _amount);
        }
    }
    ...
    function adminWithdraw(uint256 _amount) external onlyAdmin {
        htz.safeTransfer(address(msg.sender), _amount);
