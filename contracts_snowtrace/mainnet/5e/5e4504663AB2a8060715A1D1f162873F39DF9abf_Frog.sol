...
contract Frog is ERC721URIStorage, Ownable, RoyaltiesV2Impl {
  ...
  constructor() ERC721("Amphibian Outlaws", "OUTLAW") {
    _customBaseURI = 'https://stark-journey-83649.herokuapp.com/nft/';
    _price = 0.42 ether;
    _maxMintable = 7777;
  ...
  function airdrop(address recipient, uint256 id) public onlyOwner {
    require(_tokenIds.current() <= _maxMintable, "Project is finished minting.");

    _tokenIds.increment();
    _mint(recipient, id);

    setRoyaltiesOnMint(id, payable(owner()), 750);
  }
  ...
  function mintForPurchase(address recipient) private {
    ...
    setRoyaltiesOnMint(_tokenIds.current(), payable(owner()), 750);
  }
