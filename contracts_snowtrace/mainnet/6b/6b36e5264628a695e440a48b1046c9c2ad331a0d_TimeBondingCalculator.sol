...
contract TimeBondingCalculator is IBondingCalculator {
    ...
    address public immutable eleMim;

    constructor( address _eleMim ) {
        require( _eleMim != address(0) );
        eleMim = _eleMim;
    }
    ...
    function markdown( address _pair ) external view returns ( uint ) {
        ...
        address token;
        uint pps = IEleBank(eleMim).getPricePerFullShare();
        uint reserve;
        if ( IUniswapV2Pair( _pair ).token0() == eleMim ) {
            reserve = reserve0;
            token = token1;
        } else {
            reserve = reserve0;
        ...
        return reserve.mul( 2 * ( 10 ** IERC20( token ).decimals() ) ).div( getTotalValue( _pair ) ).mul(pps).div(1 ether);
    }
}