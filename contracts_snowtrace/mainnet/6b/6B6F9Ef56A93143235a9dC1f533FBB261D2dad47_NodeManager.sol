...
contract NodeManager is Ownable {
    ...
    modifier onlyGuard() {
        require(owner() == _msgSender() || token == _msgSender(), "caller is not the guard");
        _;
    }

    modifier onlyNodeOwner(address account) {
        require(isNodeOwner(account), "caller is not node owner");
        _;
    }
    ...
    function createNode(address account, string memory nodeName, uint256 amount_) external onlyGuard {
        require(isEnabled, "Contract Paused");
        require(
    ...
    function updateIsEnabled(bool newVal) external onlyOwner {
        isEnabled = newVal;
    }
