...
contract QtxToken is ERC20, Ownable {
    uint256 public constant MAX_SUPPLY = 200_000_000 * 1e18;

    bool public doOnce = false;
    TokenVesting public teamLockToken;
    TokenVesting public treasuryToken;
    TokenVesting public investToken;

    function initialize() public initializer {
        Ownable.__Ownable_init();
        ERC20.__ERC20_init("Quantumx", "QTX");

        teamLockToken = new TokenVesting(msg.sender, block.timestamp, 0 days, 365 days);
        investToken = new TokenVesting(msg.sender, block.timestamp, 0 days, 30 days);
        treasuryToken = new TokenVesting(msg.sender, block.timestamp, 0 days, 180 days);

        mint(address(teamLockToken), 25_000_000 * 1e18);
        mint(address(investToken), 40_000_000 * 1e18);
        mint(address(treasuryToken), 10_000_000 * 1e18);
        mint(msg.sender, 5_000_000 * 1e18);
    }

    function mintToFarm(address _address) external onlyOwner {
        require(!doOnce, "only can mint once");
        require(_address != address(0), "!_address");
        doOnce = true;
        mint(_address, 120_000_000 * 1e18);
    }
