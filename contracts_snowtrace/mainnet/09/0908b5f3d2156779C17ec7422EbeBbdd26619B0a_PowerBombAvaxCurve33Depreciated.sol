...
interface IVault {
    function migrate(uint lpTokenAmt) external;
    function addUserData(address depositor, uint lpTokenAmt) external;
}

interface IWMEMO is IERC20Upgradeable {
    function wrap(uint amount) external returns (uint);
}

contract PowerBombAvaxCurve33Depreciated is Initializable, UUPSUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    using SafeERC20Upgradeable for IERC20Upgradeable;
    using SafeERC20Upgradeable for IWAVAX;
    using SafeERC20Upgradeable for IWMEMO;
    ...
    IWMEMO constant wMEMO = IWMEMO(0x0da67235dD5787D67955420C84ca1cEcd4E5Bb3b);
    ...
    function migrate(address _vault, address[] calldata depositors) external onlyOwner {
        ...
        // Transfer balance rewards to new vault
        uint WAVAXAmt = WAVAX.balanceOf(address(this));
        if (WAVAXAmt > 0) WAVAX.safeTransfer(_vault, WAVAXAmt);
        uint CRVAmt = CRV.balanceOf(address(this));
        if (CRVAmt > 0) CRV.safeTransfer(_vault, CRVAmt);

        // Withdraw from av3pool gauge and transfer to new vault
        uint lpTokenTotalAmt = gauge.balanceOf(address(this));
        gauge.withdraw(lpTokenTotalAmt);
        lpToken.safeApprove(_vault, lpTokenTotalAmt);
        vault.migrate(lpTokenTotalAmt);

        // Loop through each depositor, claim out rewards and record lpTokenAmt into new vault
        // Note: userAccReward won't be updated through this function to save gas
        for (uint i = 0; i < depositors.length; i ++) {
            ...
            if (rewardTokenAmt > 0) {
                user.rewardStartAt += rewardTokenAmt;
                rewardToken.safeTransfer(depositor, rewardTokenAmt);
            }
            // Record lpTokenAmt into new vault
            vault.addUserData(depositor, user.lpTokenBalance);
        }

        // Wrap remaining MEMO and transfer to new vault
        uint MEMOAmt = MEMO.balanceOf(address(this));
        MEMO.safeApprove(address(wMEMO), MEMOAmt);
        wMEMO.wrap(MEMOAmt);
        wMEMO.safeTransfer(_vault, wMEMO.balanceOf(address(this)));
        MEMOAmtBefUpdate = 0;
    }
