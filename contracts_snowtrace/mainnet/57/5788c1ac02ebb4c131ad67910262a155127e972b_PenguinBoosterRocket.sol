...
contract PenguinBoosterRocket is Ownable {
    ...
    //tokens to be used for payment
    IERC20[] public tokensToPay;
    uint256[] public tokenDecimals;
    //contract that can controls allocations
    address public allocationController;
    //amount of tokensToPay that buys an entire tokenForDistribution (assuming 18 decimals in payment token -- differences in decimals accounted for in 'findAmountToPay')
    ...
    //tracks sum of proceeds collected in tokensToPay from all token sales
    ...
    constructor(
        ...
        tokensToPay = new IERC20[](4);
        tokensToPay[0] = IERC20(0xA7D7079b0FEaD91F3e65f86E8915Cb59c1a4C664); //USDC.e (6 decimals)
        tokensToPay[1] = IERC20(0xd586E7F844cEa2F87f50152665BCbc2C279D8d70); //DAI.e (18 decimals)
        tokensToPay[2] = IERC20(0xc7198437980c041c805A1EDcbA50c1Ce5db95118); //USDT.e (6 decimals)
        tokensToPay[3] = IERC20(0x130966628846BFd36ff31a822705796e8cb8C18D); //MIM (18 decimals)
        tokenDecimals = new uint256[](4);
        tokenDecimals[0] = 6;
        tokenDecimals[1] = 18;
        tokenDecimals[2] = 6;
        tokenDecimals[3] = 18;
    ...
    //find amount of paymentTokenIndex'th token in tokensToPay that 'penguinAddress' needs to spend to buy 'amountToBuy' of tokenForDistribution
    function findAmountToPay(address penguinAddress, uint256 amountToBuy, uint256 paymentTokenIndex) public view returns(uint256) {
        ...
        uint256 addedDivisor = 10**(18 - tokenDecimals[paymentTokenIndex]);
        uint256 amountToPay = ((amountToBuy * exchangeRateWholeToken * (10000 - discount)) / 10000) / (exchangeRateDivisor * addedDivisor);
        return amountToPay;
    }
    ...
    //EXTERNAL FUNCTIONS
    function purchaseTokens(uint256 amountToBuy, uint256 paymentTokenIndex) external checkPurchase(msg.sender, amountToBuy) {
        agreeToTermsAndConditions();
        require(amountToBuy > 0);
        _processPurchase(msg.sender, amountToBuy, paymentTokenIndex);
    }
    ...
    function withdrawDistributionProceeds(address dest, uint256 paymentTokenIndex) external onlyOwner {
        uint256 toSend = tokensToPay[paymentTokenIndex].balanceOf(address(this));
        tokensToPay[paymentTokenIndex].safeTransfer(dest, toSend);
    }
    ...
    //INTERNAL FUNCTIONS
    function _processPurchase(address penguinAddress, uint256 amountToBuy, uint256 paymentTokenIndex) internal {
        uint256 amountToPay = findAmountToPay(penguinAddress, amountToBuy, paymentTokenIndex);
        totalProceeds[paymentTokenIndex] += amountToPay;
        ...
        tokensToPay[paymentTokenIndex].safeTransferFrom(penguinAddress, address(this), amountToPay);
    }
}