 ...
 */
 contract Presale_SPC is Owned{
    ...
    constructor(uint256 _rate, address _wallet, ERC20 _mim, ERC20 _pToken, uint _closingTime, uint _vestedTime) {
        require(_rate > 0);
    ...
    // allows buyers to put their mim to get some pToken once the presale will closes
    function buy(uint _amount) public {
        require(isPresaleOpen, "Presale is not open yet");
        require(whiteListed[msg.sender] == true);
        require(_amount > minMimLimit, "You need to sell at least some min amount");
        require(_paidTotal[msg.sender] + _amount <= maxMimLimit);
    ...
    function vipBuy(uint _amount) public {
        require(isPresaleOpen, "Presale is not open yet");
    ...
    function claim() public {
        ...
        uint totalpTokenToClaim = preBuys[msg.sender].mimAmount.mul(rate).div(RATE_DECIMALS).mul(percentReleased).div(VESTING_TIME_DECIMALS);
