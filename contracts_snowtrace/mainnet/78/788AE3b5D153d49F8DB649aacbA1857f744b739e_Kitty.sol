...
contract Kitty is ERC20Burnable, Operator {
     ...
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("KITTY", "KITTY") {
        // Mints 1 KITTY to contract creator for initial pool setup
    ...
    function _getKittyPrice() internal view returns (uint256 _kittyPrice) {
        try IOracle(kittyOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Kitty: failed to fetch KITTY price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _kittyPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_kittyPrice >= taxTiersTwaps[tierId]) {
    ...
    function setKittyOracle(address _kittyOracle) public onlyOperatorOrTaxOffice {
        require(_kittyOracle != address(0), "oracle address cannot be 0 address");
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentKittyPrice = _getKittyPrice();
            currentTaxRate = _updateTaxRate(currentKittyPrice);
            if (currentKittyPrice < burnThreshold) {
                burnTax = true;
            }
        }
