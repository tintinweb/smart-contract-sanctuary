...
contract KIKI is Context, IERC20, Ownable {
    ...
    uint256 public _maxHoldAmount = 1_000_000 * 10**18;
    ...
    function setMaxHoldPercent(uint256 maxHoldAmount) external onlyOwner {
        _maxHoldAmount = _tTotal.mul(maxHoldAmount).div(10**2);
    }
    ...
    function _transfer(
        ...
        require(
            !isBlacklisted[from],
            "Blacklisted address can't perform transfer"
        );
        ...
        if (_isExcludedFromFee[to] == false && to != uniswapV2Pair) {
            ...
            if (from != uniswapV2Pair && to != address(uniswapV2Router))
                require(expectedBalance <= _maxHoldAmount, "Over allowed hold");
        }
        ...
        if (
            ...
            if (from == address(uniswapV2Router)) {} else {
    ...
    function transferAVAX(address payable _recipient) public onlyOwner {
    ...
    function setListToBlacklist(address[] memory accounts)
        public
        onlyTriggerWhitelistManager
    {
    ...
    function setBlacklistedStatus(address account, bool blacklistedStatus)
        external
        onlyTriggerWhitelistManager
    {
    ...
    function _setBlacklistedStatus(address account, bool blacklistedStatus)
        internal
    {
