...
///@author Zapper
///@notice This contract removes liquidity from TraderJoe pools on Avalanche, receiving AVAX, ERC20 or both.
...
// File contracts/Avalanche/TraderJoe/TraderJoe_ZapOut_V1.sol
...
contract TraderJoe_ZapOut_V1_0_1 is ZapOutBaseV3_1 {
    using SafeERC20 for IERC20;
    ...
    */
    function ZapOut2PairToken(
        ...
        _approveToken(fromPoolAddress, address(joeRouter), incomingLP);

        if (token0 == wavaxTokenAddress || token1 == wavaxTokenAddress) {
            address _token = token0 == wavaxTokenAddress ? token1 : token0;
            (amountA, amountB) = joeRouter.removeLiquidityAVAX(
                _token,
                incomingLP,
                1,
                1,
            ...
            (amountA, amountB) = joeRouter.removeLiquidity(
                token0,
                token1,
                incomingLP,
                1,
        ...
        emit zapOut(msg.sender, fromPoolAddress, token0, amountA, affiliate);
        emit zapOut(msg.sender, fromPoolAddress, token1, amountB, affiliate);
    }
    ...
    */
    function ZapOut(
        ...
        (uint256 amount0, uint256 amount1) =
            _removeLiquidity(fromPoolAddress, incomingLP);
    ...
    */
    ...
    function _removeLiquidity(address fromPoolAddress, uint256 incomingLP)
        internal
        returns (uint256 amount0, uint256 amount1)
    {
        ...
        _pullTokens(fromPoolAddress, incomingLP);

        _approveToken(fromPoolAddress, address(joeRouter), incomingLP);

        (amount0, amount1) = joeRouter.removeLiquidity(
            token0,
            token1,
            incomingLP,
            1,
    ...
    function _fillQuote(
        ...
        if (fromTokenAddress == wavaxTokenAddress && toToken == address(0)) {
            IWETH(wavaxTokenAddress).withdraw(amount);
            return amount;
        }
