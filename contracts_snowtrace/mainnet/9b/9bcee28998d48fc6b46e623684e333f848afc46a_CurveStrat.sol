...
interface ICurve{
    function add_liquidity(uint256[3] memory,uint256) external returns (uint256);
 ...
 */
contract CurveStrat is Ownable, Pausable {
    ...
    address constant public masterchef = 0x06408571E0aD5e8F52eAd01450Bde74E5074dC74;
    address constant public spell = 0xCE1bFFBD5374Dac86a2893119683F4911a2F7814; 
     ...
     *
     * {WITHDRAWAL_FEE} - Fee taxed when a user withdraws funds. 3 === 0.03% fee.
    ...
    constructor(address _lpPair, uint _pid, address _vault) public {
            ...
            IERC20(ICurve(lpPair).coins(0)).approve(lpPair, uint(-1));
            IERC20(ICurve(lpPair).coins(1)).approve(lpPair, uint(-1));
            IERC20(ICurve(lpPair).coins(2)).approve(lpPair, uint(-1));
        }
        
    function setHarvester(address _add, bool _bool) public onlyOwner{
    ...
    function getBpsFee() public view returns(uint){
        return Buybackstrat(stor).defaultFeeBPS();
    }
    ...
    function harvest() public whenNotPaused {
        ...
        uint bps = Buybackstrat(stor).defaultFeeBps();
        token.transfer(stor, bal*bps/10000);        
        ...
        uint am0 = IERC20(ICurve(lpPair).coins(0)).balanceOf(address(this));
        uint am1 = IERC20(ICurve(lpPair).coins(1)).balanceOf(address(this));
        uint am2 = IERC20(ICurve(lpPair).coins(2)).balanceOf(address(this));
        ICurve(lpPair).add_liquidity([am0,am1,am2],0);
    ...
    function withdraw(uint256 _amount) external {
        ...
        IERC20(lpPair).safeTransfer(stor, withdrawalFee);
    }
    ...
    function pause() public onlyOwner {
        ...
        IERC20(ICurve(lpPair).coins(0)).approve(lpPair, 0);
        IERC20(ICurve(lpPair).coins(1)).approve(lpPair, 0);
        IERC20(ICurve(lpPair).coins(2)).approve(lpPair, 0);
    }
