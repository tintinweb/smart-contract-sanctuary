...
contract BabyAVAX is Context, IERC20, Ownable {
    ...
    string private _symbol = "BAVAX";
    uint8 private _decimals = 18;
    uint256 private _totalSupply = 1000000 * 10**9 * 10**_decimals;
    
    address payable public marketingAddress = payable(0x5C45835b8b926175B9b6475fe2cd3AAe8A7b392f);
    address public marketingWalletToken = 0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7;
    address public constant deadAddress = 0x000000000000000000000000000000000000dEaD;
    
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) private _isExcludedFromFeesBavax;
    mapping (address => bool) private _isExcludedFromMaxBalanceBavax;

    uint256 private constant _maxFeesBavax = 10;
    uint256 private _totalFeesBavax;
    uint256 private _totalFeesBavaxContract;
    uint256 private _burnFeesBavax;
    uint256 private _liquidityFeesBavax;
    uint256 private _marketingFeeBavax;
    
    uint256 private _maxBalanceWalletBavax;
    ...
    constructor () {
       IJoeRouter02 _joeRouter = IJoeRouter02(0x60aE616a2155Ee3d9A68541Ba4544862310933d4);
        ...
        _isExcludedFromFeesBavax[owner()] = true;
        _isExcludedFromFeesBavax[address(this)] = true;
        
        _isExcludedFromMaxBalanceBavax[owner()] = true;
        _isExcludedFromMaxBalanceBavax[address(this)] = true;
        _isExcludedFromMaxBalanceBavax[joePair] = true;

        _liquidityFeesBavax = 2;
        _marketingFeeBavax = 7;
        _burnFeesBavax = 1;
        _totalFeesBavax = _liquidityFeesBavax.add(_marketingFeeBavax).add(_burnFeesBavax);
        _totalFeesBavaxContract = _liquidityFeesBavax.add(_marketingFeeBavax);

        _liquidityThreshhold = 5000 * 10**9 * 10**_decimals;
        _maxBalanceWalletBavax = 50000 * 10**9 * 10**_decimals;
    ...
    function setLiquidityFeePercent(uint256 newLiquidityFee) external onlyOwner() {
        ...
        require(newLiquidityFee.add(_burnFeesBavax).add(_marketingFeeBavax) <= _maxFeesBavax, "Fees are too high.");
        _liquidityFeesBavax = newLiquidityFee;
        _totalFeesBavax = _liquidityFeesBavax.add(_marketingFeeBavax).add(_burnFeesBavax);
        _totalFeesBavaxContract = _liquidityFeesBavax.add(_marketingFeeBavax);
        emit SetLiquidityFee(_liquidityFeesBavax);
    }

    function setMarketingFeePercent(uint256 newMarketingFee) external onlyOwner() {
        require(!inSwapAndLiquify, "inSwapAndLiquify");
        require(_liquidityFeesBavax.add(_burnFeesBavax).add(newMarketingFee) <= _maxFeesBavax, "Fees are too high.");
        _marketingFeeBavax = newMarketingFee;
        _totalFeesBavax = _liquidityFeesBavax.add(_marketingFeeBavax).add(_burnFeesBavax);
        _totalFeesBavaxContract = _liquidityFeesBavax.add(_marketingFeeBavax);
        emit SetMarketingFee(_marketingFeeBavax);
    }

    function setBurnFeePercent(uint256 newBurnFee) external onlyOwner() {
        require(_liquidityFeesBavax.add(newBurnFee).add(_marketingFeeBavax) <= _maxFeesBavax, "Fees are too high.");
        _burnFeesBavax = newBurnFee;
        _totalFeesBavax = _liquidityFeesBavax.add(_marketingFeeBavax).add(_burnFeesBavax);
        emit SetBurnFee(_burnFeesBavax);
    }
    ...
    function _transfer(
        ...
        // Make sure that: Balance + Buy Amount <= _maxBalanceWalletBavax
        if(
            from != owner() &&              // Not from Owner
            to != owner() &&                // Not to Owner
            !_isExcludedFromMaxBalanceBavax[to]  // is excludedFromMaxBalance
        ){
            require(
                balanceOf(to).add(amount) <= _maxBalanceWalletBavax,
        ...
        if(
            ...
            _totalFeesBavaxContract > 0 &&                         // LiquidityFee + MarketingFee > 0
            from != owner() &&                                  // Not from Owner
            to != owner()                                       // Not to Owner
        ) {
            collectFees();
        ...
        // Take Fees 
        if(
            !(_isExcludedFromFeesBavax[from] || _isExcludedFromFeesBavax[to])
            && _totalFeesBavax > 0
        ) {
            
        	uint256 feesToContract = amount.mul(_totalFeesBavaxContract).div(100);
            uint256 toBurnAmount = amount.mul(_burnFeesBavax).div(100);
    ...
    function collectFees() private lockTheSwap {
        ...
        uint256 liquidityTokensToSell = balanceOf(address(this)).mul(_liquidityFeesBavax).div(_totalFeesBavaxContract);
        uint256 marketingTokensToSell = balanceOf(address(this)).mul(_marketingFeeBavax).div(_totalFeesBavaxContract);
