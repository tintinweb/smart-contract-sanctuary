...
interface IOldManager {
    function getClaimOf(uint64 _id) external view returns (uint64);
    function getMintOf(uint64 _id) external view returns (uint64);
    function getNameOf(uint64 _id) external view returns (string memory);
...
contract ArmyHelper is Ownable, PaymentSplitter {
    ...
    address public team = 0xAD92cBfab5d8ba84E8302268ACE41E76881388D7;
    Pool public pool;
    ...
    constructor(address[] memory _payees, uint256[] memory _shares, address _manager, address _token, address _oldManager) PaymentSplitter(_payees, _shares) {
        ...
        oldManager = IOldManager(_oldManager);
    }
    ...
    function migrate(uint64[] calldata _ids) external {
        for (uint256 i = 0; i < _ids.length; i++) {
            oldManager.transferFrom(msg.sender, address(this), _ids[i]);
            string memory _name = oldManager.getNameOf(_ids[i]);
            uint64 _mint = oldManager.getMintOf(_ids[i]);
            uint64 _claim = oldManager.getClaimOf(_ids[i]);
            manager.importArmy(msg.sender, _name, _mint, _claim);
        }
    }
}