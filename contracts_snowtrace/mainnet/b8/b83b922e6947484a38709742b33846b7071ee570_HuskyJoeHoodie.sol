...
contract HuskyJoeHoodie is ERC721, Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    uint256 public constant MAX_HUSKY = 100;
    bool public _hasSaleStarted = false;
    uint256 _nftPrice = 3 ether;

    // Index of a person is its ID.
    address[] public _persons;

    // number of nft of each holder
    mapping (address => uint256) public _nbNFTHolders;
    
    /// @notice Current owner of this contract
    address public _contractowner;

    event Mint(address nftMinter, uint nftId);

    constructor() ERC721("HuskyJoe Hoodie", "HUSKYJOEHOODIE") {
        setBaseURI("");
    ...
    function changeOwner(address newOwner) 
        external
    {
        require(msg.sender == _contractowner, "Claim::changeOwner: not owner");
        require(newOwner != address(0) && newOwner != address(this), "Claim::changeOwner: not valid address");
    ...
    function recoverERC20(address tokenAddress, uint256 tokenAmount) external {
        require(msg.sender == _contractowner, "Claim::recoverERC20: not owner");
        require(tokenAmount > 0, "Claim::recoverERC20: amount is 0");
        IERC20(tokenAddress).safeTransfer(_contractowner, tokenAmount);
    }
    ...
    function mintHusky(uint256 maxHusky, uint id) public payable {
        require(_hasSaleStarted == true, "sale hasn't started yet");
        require(maxHusky <= 3, "can't mint more than 3 nft");
        require(totalSupply().add(maxHusky) < MAX_HUSKY, "exceed NFT limit");
        require(msg.value >= _nftPrice.mul(maxHusky), "price too low");

        for (uint i = 0; i < maxHusky; i++) {
    ...
    function setTokenURI(uint id, string memory tokenURI) public onlyOwner {
        _setTokenURI(id, tokenURI);
    }

    function startDrop() public onlyOwner {
        _hasSaleStarted = true;
    }
    ...
    function withdraw(uint256 _value) public payable onlyOwner {
        require(payable(msg.sender).send(_value));
    }
