...
contract NeonMonsters is ERC721, ERC721Enumerable, Ownable, ReentrancyGuard {
    uint256 public constant MAX_SUPPLY = 10000;
    uint256 public constant MAX_TOKEN_MINT = 5;

    string public provenance;
    bool public saleIsActive;
    string private _baseURIextended;
    uint256 public tokensReserve; // for events and giveaways
    ...
    constructor(string memory baseURI_) ERC721("Neon Monsters", "NEMO") {
    ...
    function minterOf(uint256 tokenId) public view returns (address) {
        require(_exists(tokenId), "NeonMonsters: token does not exists");
    ...
    {
        require(
            numberOfTokens > 0 && numberOfTokens <= tokensReserve,
            "NeonMonsters: not enough reserve left"
        );
        uint256 supply = totalSupply();
        for (uint256 i = 0; numberOfTokens > i; i++) {
            _safeMint(to, supply + i);
        }
        tokensReserve -= numberOfTokens;
    }
    ...
    function _tokenPrice(uint256 tokenId) private pure returns (uint256) {
        return ((tokenId / 500) * 100000000000000000) + 100000000000000000;
    }

    function tokenPrice(uint256 tokenId) external pure returns (uint256) {
        require(tokenId >= 0 && tokenId < 10000, "NeonMonsters: invalid range");
        return _tokenPrice(tokenId);
    }

    function mintToken(uint256 numberOfTokens) public payable {
        require(saleIsActive, "NeonMonsters: sale is not active");
        require(
            numberOfTokens > 0 && numberOfTokens <= MAX_TOKEN_MINT,
            "NeonMonsters: purchase exceed max limit per transaction"
        );
        require(
            totalSupply() + numberOfTokens <= MAX_SUPPLY,
            "NeonMonsters: purchase exceed max supply of tokens"
        );
        uint256 amountToPay;
        for (uint256 i = 0; numberOfTokens > i; i++) {
            amountToPay += _tokenPrice(totalSupply() + i);
        }
        require(
            amountToPay <= msg.value,
            "NeonMonsters: ether value sent is not correct"
        );
        ...
        for (uint256 i = 0; numberOfTokens > i; i++) {
            ...
            _splitBalance(_tokenPrice(mintIndex));
        }
    }
