...
contract CypherBondingCalculator is IBondingCalculator {
    ...
    function markdown( address _pair ) external view returns ( uint ) {
        ...
        uint reserve;
        if ( IUniswapV2Pair( _pair ).token0() == address(CYPHER) ) {
            reserve = reserve1;
        } else {
            require(IUniswapV2Pair( _pair ).token1() == address(CYPHER), "not a CYPHER lp pair");
            reserve = reserve0;
        }
        return reserve.mul( 2 * ( 10 ** CYPHER.decimals() ) ).div( getTotalValue( _pair ) );
    }
}