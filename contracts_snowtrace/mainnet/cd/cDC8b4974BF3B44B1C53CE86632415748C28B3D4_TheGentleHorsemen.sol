...
// File: https://github.com/dievardump/EIP2981-implementation/blob/main/contracts/IERC2981Royalties.sol
...
// File: https://github.com/dievardump/EIP2981-implementation/blob/main/contracts/ERC2981Base.sol
...
// File: https://github.com/dievardump/EIP2981-implementation/blob/main/contracts/ERC2981PerTokenRoyalties.sol
...
{
    using Strings for uint256;
    string public INITIAL_EXTENSION = ".json";
    ...
    uint256 public constant INITIAL_CLAIM_PRICE = 2.5 ether;
    uint256 public constant POST_3888_PRICE = 2 ether;

    bool public canClaim = false;
    bool public canClaimAirdrop = true;
    bool public canOnlyPresale = true; 
    ...
    uint256 public airdropNumber = 10; 
    ...
    uint256 public post3888Extra = 0; 
    bool public revealed = false;

    // tokenID mapping
    mapping(uint256 => uint256) indexer; 
    uint256 indexerLength = TOTAL_SUPPLY; 
    ...
    constructor(
        ...
        unrevealUri = unrevealTokenURI;
        post3888Extra = 0.5 ether;
    }
    /* *************** */
    /* Minting Rewards */
    /* *************** */
    ...
    function getFirstRewardComponent(uint256 index)
        ...
        returns (uint256 r2)
    {
        uint256 reward = getPrice(index) * 500;
        reward = (reward * (TOTAL_SUPPLY - index));
        reward = reward / 1000 / TOTAL_SUPPLY;
        return reward; 
    }
    ...
    {
        if (index >= 3888) return 0;
        if (redeemedIDs[index]) return 0; 
        return getFirstRewardComponent(index) - post3888Extra;
    }
        ...
        returns (uint256 reward)
    {
        ...
        return
            (getProjectedReward(index) * (minted * 1000)) / TOTAL_SUPPLY / 1000;
    }

    function getPrice(uint256 index) public pure returns (uint256 price) {
        if (index >= 3888) return POST_3888_PRICE;
        return INITIAL_CLAIM_PRICE - ((index * 10**18) / TOTAL_SUPPLY);
    }
    ...
    function enoughRandom() internal view returns (uint256) {
        if (TOTAL_SUPPLY - minted == 0) return 0;
        return
            uint256(
                keccak256(
                    abi.encodePacked(
    ...
    function toggleWhitelistClaimability() external onlyOwner {
        canOnlyPresale = !canOnlyPresale;
    }

    function mintGHORSE(address receiver, uint256 nextTokenIndex) internal {
    ...
    function claim(uint256 n) public payable nonReentrant { 
        require(
            canClaim,
            "it's not possible to claim just yet."
        );
        require(
            TOTAL_SUPPLY - airdropNumber - minted > 0,
            "not enough GHORSE left to mint."
        );
        require(n > 0, "not enough GHORSE left to mint.");
        require(n <= MAX_PER_CLAIM);
        if (n > TOTAL_SUPPLY - airdropNumber - minted) {  
            n = TOTAL_SUPPLY - airdropNumber - minted;
        }

        if(canOnlyPresale) {
            require(whitelistedAddresses[msg.sender] == 1, "user not whitelisted");
        }
        ...
        for (uint256 i = 0; i < n; i++) {
            ...
            mintGHORSE(_msgSender(), nextIndex);
    ...
    function addHonoraries(address[] calldata honoraries) external onlyOwner {
        uint256 honoraryID = numHonoraries + 77;
 ...
 //set le nb de NFTs alloué à un airdrop et le mots de passe pour l'activer, 
    function setAirdropNumber(uint256 number)
        external
        onlyOwner
    {
        require(minted + number <= TOTAL_SUPPLY);
        airdropNumber = number;
    }

    //check le nb de NFT déjà claimed par l'adresse
    ...
    function sendAirdrop(address account, uint256 requesting) external onlyOwner 
    {
        require(canClaimAirdrop,  "You can't claim airdrop now, come back later.");
        require(minted + requesting <= TOTAL_SUPPLY, "Airdrop value exceeds total supply.");
        require(airdropNumber >= requesting, "No GHORSE left to airdrop.");        

       uint256 nextId = minted;
        for (uint256 i = 0; i < requesting; i++) {
            mintGHORSE(account, nextId);
        ...
        returns (string memory)
    {
        ...
        return
            bytes(baseURI).length > 0
                ? string(abi.encodePacked(baseURI, imageID.toString()/*, INITIAL_EXTENSION*/))
                : "";
    }
    ...
    function setUnrevealUri(string memory uri) external onlyOwner {
        unrevealUri = uri;
    }

    function setINITIAL_EXTENSION(string memory extension) external onlyOwner {
    ...
    function withdrawableBalance() public view returns (uint256 value) {
        return
            gross -
            ((totalProjectedRewards * (minted * 1000)) / TOTAL_SUPPLY / 1000) -
            withdrawn;
    }
