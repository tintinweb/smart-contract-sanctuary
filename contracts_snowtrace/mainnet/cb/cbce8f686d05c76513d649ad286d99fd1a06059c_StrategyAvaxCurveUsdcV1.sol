...
abstract contract StrategyAvaxCurveFarmBase is StrategyBase {
    address public constant CRV = 0x47536F17F4fF30e64A96a7555826b8f9e66ec468; //avax CRV
    ...
    function getHarvestableWavax() public view returns (uint256) {
        return LpTokenStaker(stakingPool).claimable_reward_write(address(this), WAVAX);
    }
    ...
    function _claimRewards() internal {
        LpTokenStaker(stakingPool).claim_rewards(address(this));
    }
...
contract StrategyAvaxCurveUsdcV1 is StrategyAvaxCurveFarmBase {
    address public constant usdc = 0xA7D7079b0FEaD91F3e65f86E8915Cb59c1a4C664; //want token
    ...
    // staking constants
    address public constant curveGauge = 0x5B5CFE992AdAC0C9D48E05854B2d91C73a003858;
    address public constant curvePool = 0x7f90122BF0700F9E7e1F688fe926940E8839F353;
    address public constant curveLpToken = 0x1337BedC9D22ecbe766dF105c9623922A27963EC;
    ...
    public StrategyAvaxCurveFarmBase(usdc, curveLpToken, curveGauge, _governance, _strategist, _controller, _timelock)
    {
    ...
    function _withdrawSome(uint256 _amount) internal override returns (uint256) {
        ...
        if (_lpAmt > 0){
            ...
            uint256 _min_out = _amount.mul(DENOMINATOR.sub(slippageProtectionOut)).div(DENOMINATOR);
            ICurveFi_3(curvePool).remove_liquidity_one_coin(requiredLpAmt, 1, _min_out, true);
        }
    ...
    function harvest() public override onlyBenignCallers {
        ...
        if (_rewards > 0) {
            ...
            path[0] = CRV;
            path[1] = WAVAX;
            _swapUniswapWithDetailConfig(CRV, WAVAX, _rewards, 1, path, sushiRouter);
        }

        uint256 _waavx = IERC20(WAVAX).balanceOf(address(this));
        uint256 _buybackLpAmount = _waavx.mul(performanceFee).div(performanceMax);
        if (_waavx > 0) {
            if (buybackEnabled == true && _buybackLpAmount > 0){
                buybackAndNotify(WAVAX, _buybackLpAmount);
            }

            _swapUniswap(WAVAX, want, IERC20(WAVAX).balanceOf(address(this)));
    ...
    function getName() external override pure returns (string memory) {
        return "StrategyAvaxCurveUsdcV1";
    }
}