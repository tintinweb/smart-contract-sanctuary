...
contract VaporNodes is ERC20, Ownable, PaymentSplitter {
    ...
    function updateMaxTx(uint256 newVal) external onlyOwner {
        maxTx = newVal;
    }
    ...
    function _transfer(
        ...
        if (from != owner() && to != joePair && to != address(joeRouter) && to != address(this) && from != address(this)) {
            require(isTradingEnabled, "TKN:5");
            ...
            if (to != rewardsPool && to != teamPool && from != rewardsPool && from != teamPool) {
                ...
                require(amount.add(walletBalance) <= totalSupply.mul(maxTx).div(10000), "TKN:6");
            }
        }
    ...
    function cashoutReward(uint256 blocktime) external {
        ...
        emit Cashout(_msgSender(), rewardAmount, blocktime);
    }
    ...
    function cashoutAll() external {
        ...
        emit Cashout(_msgSender(), rewardAmount, 0);
    }
}
