...
contract Emerald is ERC20, Ownable {
    ...
    struct Fees {
        ...
        uint8 space; // for SPACE holders / Alien Hands / ??
    }
    ...
    address public spaceHolderFeeReceiver;
    ...
    ) ERC20("Emerald Protocol V2", "EMRLD") {
        setFeeOptions(0, 0, 0, 0);

        setFeeReceivers(_initialWallet, _initialWallet, _initialWallet);
    ...
    function swapBack() internal swapping {
        ...
        uint256 totalAVAXFee = fees.total - (dynamicLiquidityFee / 2);

        uint256 amountAVAXLiquidity = (amountAVAX * dynamicLiquidityFee) /
            (totalAVAXFee / 2);
        uint256 amountAVAXReflection = (amountAVAX * fees.reflection) /
            totalAVAXFee;
        uint256 amountAVAXMarketing = (amountAVAX * fees.marketing) /
            totalAVAXFee;
        uint256 amtForSpaceHolders = (amountAVAX * fees.space) / totalAVAXFee;

        payable(marketingFeeReceiver).transfer(amountAVAXMarketing);

        payable(spaceHolderFeeReceiver).transfer(amtForSpaceHolders);

        if (dividendsEnabled) {
            totalDividends += amountAVAXReflection;
            dividendsPerShare +=
                (dividendsPerShareAccuracyFactor * amountAVAXReflection) /
                totalShares;
        }
    ...
    function setFeeReceivers(
        ...
        address _spaceHolderFeeReceiver
    ) public onlyOwner {
        ...
        spaceHolderFeeReceiver = _spaceHolderFeeReceiver;
        setWalletSizeExempt(spaceHolderFeeReceiver, true);
        setFeeExempt(spaceHolderFeeReceiver, true);
    }
    ...
    function setRewardPath(address[] memory _path) public onlyOwner {
        rewardPath = _path;
        rewardToken = (_path.length == 0 ? WAVAX : _path[_path.length - 1]);
    }
