// contracts/AutoBondPersonalVault.sol
...
contract AutoBondPersonalVault {
    ...
    address public depositor; // address allowed to withdraw funds
    address public feeHarvester; // address to send fees
    address public vaultManager; // address which can manage bonds
    uint256 public fee;
    ...
    constructor(
        ...
        require(_joeRouter != address(0));
        joeRouter = IJoeRouter01(_joeRouter);
        require(_staking != address(0));
        staking = IStaking(_staking);
        require(_mimBondDepository != address(0));
        mimBondDepository = ITimeBondDepository(_mimBondDepository);
    ...
    function bondWithMim(
        ...
        // bond with MIM
        require(
            mim.balanceOf(address(this)) >= results[1],
            "Not enough tokens"
        );
        mim.approve(address(mimBondDepository), results[1]);
        uint256 payout = mimBondDepository.deposit(
            results[1],
            _maxBondPrice,
            address(this)
        );
        emit BondCreated(_amount, path[1], payout);
        return payout;
    }

    // M0b
    // function bondWithTimeMimLP(uint256 _amount) public {}

    function redeemBond(ITimeBondDepository depository)
        external
        only(vaultManager)
        returns (uint256)
    {
        uint256 amount = depository.redeem(address(this), true);
        uint256 feeValue = (amount * fee) / 10000;
        stakedAsset.safeTransfer(feeHarvester, feeValue);
        uint256 redeemed = amount - feeValue;
        emit BondRedeemed(address(depository), redeemed);
        return redeemed;
    }

    function stakeAssets(uint256 _amount) public only(vaultManager) {
    ...
    function unstakeAssets(uint256 _amount) public only(vaultManager) {
        require(
    ...
    // M1 add check for price
    // M1 add maturing of harvested bonds
    // M1 add management of vault manager and fee harvester and fee itself

    // // M1
    // function reserveForWithdrawal() public {}

    // // M1
    // function decreaseReserved() public {}

    // // M1
    // function calculateBond5DayRoi() public view {}

    // // M1
    // function calculateStake5DayRoi() public view {}

    /* ======= AUXILLIARY ======= */

    /**
     *  @notice allow anyone to send lost tokens (excluding asset and stakedAsset) to the feeHarvester
     *  @return bool
     */
    function recoverLostToken(IERC20 _token) external returns (bool) {
        ...
        _token.safeTransfer(feeHarvester, balance);
        return true;
    }
}
