...
contract VYBEBondingCalculator is IBondingCalculator {
    ...
    function markdown( address _pair ) external view returns ( uint ) {
        ...
        uint reserve;
        if ( IUniswapV2Pair( _pair ).token0() == address(VYBE) ) {
            reserve = reserve1;
        } else {
            require(IUniswapV2Pair( _pair ).token1() == address(VYBE), "not a VYBE lp pair");
            reserve = reserve0;
        }
        return reserve.mul( 2 * ( 10 ** VYBE.decimals() ) ).div( getTotalValue( _pair ) );
    }
}