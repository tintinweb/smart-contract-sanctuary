...
/**
 * @title Guardian node comet storage
 */
contract comet_NodeSave {
    IBMapping mappingContract;                      
    IBcomet cometContract;                             
    ...
    */
    constructor (address map) public {
        ...
        cometContract = IBcomet(address(mappingContract.checkAddress("comet")));            
    }
    ...
    */
    function turnOut(uint256 amount, address to) public onlyMiningCalculation returns(uint256) {
        uint256 leftNum = cometContract.balanceOf(address(this));
        if (leftNum >= amount) {
            cometContract.transfer(to, amount);
            return amount;
        } else {
            return 0;
        }
 ...
 */
contract comet_NodeAssignmentData {
    ...
    */
    ...
    //  Add comet
    function addcomet(uint256 amount) public onlyNodeAssignment {
 ...
 */
contract comet_NodeAssignment {
    
    using SafeMath for uint256;
    IBMapping mappingContract;  
    IBcomet cometContract;                                   
    SuperMan supermanContract;                              
    comet_NodeSave nodeSave;
    comet_NodeAssignmentData nodeAssignmentData;
    ...
    */
    constructor (address map) public {
        mappingContract = IBMapping(map); 
        cometContract = IBcomet(address(mappingContract.checkAddress("comet")));
        supermanContract = SuperMan(address(mappingContract.checkAddress("cometNode")));
        nodeSave = comet_NodeSave(address(mappingContract.checkAddress("cometNodeSave")));
        nodeAssignmentData = comet_NodeAssignmentData(address(mappingContract.checkAddress("nodeAssignmentData")));
    }
    ...
    */
    ...
    /**
    * @dev Deposit in comet
    * @param amount Quantity deposited in comet
    */
    function bookKeeping(uint256 amount) public {
        require(amount > 0);
        require(cometContract.balanceOf(address(msg.sender)) >= amount);
        require(cometContract.allowance(address(msg.sender), address(this)) >= amount);
        require(cometContract.transferFrom(address(msg.sender), address(nodeSave), amount));
        nodeAssignmentData.addcomet(amount);
    }
    ...
    */
    function nodeGet() public {
        ...
        require(cometContract.balanceOf(address(nodeSave)) >= getAmount);
    ...
    */
    function nodeCount(address fromAdd, address toAdd) public {
        ...
        require(cometContract.balanceOf(address(nodeSave)) >= getAmountFrom);
        ...
        require(cometContract.balanceOf(address(nodeSave)) >= getAmountTo);
...
contract SuperMan is IERC20 {
    ...
    */
    function _transfer(address from, address to, uint256 value) internal {
        ...
        comet_NodeAssignment nodeAssignment = comet_NodeAssignment(address(mappingContract.checkAddress("nodeAssignment")));
