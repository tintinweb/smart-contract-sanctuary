...
contract NINJAVAX is IERC20, Authorized {
    ...
    string private _name = "NINJAVAX";
    string private _symbol = "NINJAVAX";
    ...
    uint256 private _totalLPFee = 10;
    ...
    uint256 public swapTokensAtAmount = 5 * _tTotal / 1000;  // 0.2% of total supply, 2e6 tokens   
    uint256 public maxTx = 5 * _tTotal / 1000;               // 0.5 of total supply, 10e6 tokens  
    uint256 public maxWallet = 40 * _tTotal / 1000;          // 2.0% of total supply, 20e6 tokens
    ...
    constructor () {
        ...
        buybackAddress = address(0x53843bc59b7Af1b698F3675fFB5862b0095579Ab);
        autoLiquidityReceiver = msg.sender;
            
        IUniswapV2Router _uniswapV2Router = IUniswapV2Router(0x60aE616a2155Ee3d9A68541Ba4544862310933d4);
    ...
    function calculateTaxFee(uint256 _amount, address from) private returns (uint256) {
        ...
        if(rushing){
            ...
            if(block.timestamp < halfTime){
                fee = isPair[from] ? 0 : 15; 
            }
            else if(block.timestamp < fullTime){
                fee = isPair[from] ? 2 : 10; 
            }
            else{
                fee = _taxFee;
                rushing = false;
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "NINJAVAX: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function buyback(uint256 slippage) payable public onlyAuthorized{
        ...
        uniswapV2Router.swapExactAVAXForTokensSupportingFeeOnTransferTokens{value: msg.value}(
