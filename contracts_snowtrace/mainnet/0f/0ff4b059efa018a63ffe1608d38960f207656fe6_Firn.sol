...
contract Firn {
    ...
    RedBlack public tree = new RedBlack(address(this));
    struct Info {
        uint256 epoch;
    ...
    constructor(address _transfer, address _withdrawal, uint32 _fee) {
        transferVerifier = TransferVerifier(_transfer);
        withdrawalVerifier = WithdrawalVerifier(_withdrawal);
        owner = msg.sender;
        fee = _fee;
    }
    ...
    function deposit(bytes32 Y, bytes32[2] calldata signature) external payable {
        ...
        scratch[0] = scratch[0].add(Utils.g().mul(msg.value / 1e16));
        require(address(this).balance <= 0xFFFFFFFF * 1e16, "Escrow pool now too large.");
        ...
        current.epoch = epoch;
        current.amount += uint32(msg.value / 1e16);
    ...
    function transfer(bytes32[N] calldata Y, bytes32[N] calldata C, bytes32 D, bytes32 u, uint256 epoch, uint32 tip, bytes calldata proof) external {
        require(epoch == block.timestamp / EPOCH_LENGTH, "Wrong epoch. Check your clock.");
    ...
    function withdraw(bytes32[N] calldata Y, bytes32[N] calldata C, bytes32 D, bytes32 u, uint256 epoch, uint32 amount, address recipient, uint32 tip, bytes calldata proof) external {
        require(epoch == block.timestamp / EPOCH_LENGTH, "Wrong epoch. Check your clock."); // this doesn't actually provide security; it's just a convenience method
        ...
        payable(owner).transfer(uint256(amount >> fee) * 1e16);
