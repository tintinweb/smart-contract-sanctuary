...
contract VaultService is PriceModel, Ownable {
    ...
    mapping(address => uint256) private _balancesLiquidity;
    uint256 private _balanceLiquidity;
    ...
    event RemoveLiquidity(address indexed sender, uint256 amount, uint256 apy);
    ...
    function addLiquidity(uint256 amount) external {
        require(amount < MAX_DEPOSIT || _admin == msg.sender, "NOT MORE in TESTS");
        ERC20(_underlyingTokenAddress).transferFrom(
            msg.sender,
            address(this),
            amount
        );
        _dieselToken.mint(msg.sender, amount.mul(S_1).div(_rate));
        _balanceLiquidity = _balanceLiquidity.add(amount);
        _balancesLiquidity[msg.sender] = _balancesLiquidity[msg.sender].add(amount);
    ...
    function removeLiquidity(uint256 amount) external {
        uint256 amountDieselToken = _dieselToken.balanceOf(msg.sender).mul(amount).div(_balancesLiquidity[msg.sender]);
        uint256 amountUnderlyingToken = amountDieselToken.mul(_rate).div(S_1);
        require(
            amountUnderlyingToken <= _availableLiquidity && amount <= _balancesLiquidity[msg.sender],
            "Not enough liquidity"
        );

        uint256 withdrawAmount = 0;
        _balanceLiquidity = _balanceLiquidity.sub(amount);
        _balancesLiquidity[msg.sender] = _balancesLiquidity[msg.sender].sub(amount);

        if (_balanceLiquidity == 0) {
            withdrawAmount = ERC20(_underlyingTokenAddress).balanceOf(address(this));
            _profit = 0;
            _totalLiquidity = 0;
            _availableLiquidity = 0;
            _rate = S_1;
        } else {
            withdrawAmount = amountUnderlyingToken;
            _profit = _profit.add(amount).sub(withdrawAmount);
            _totalLiquidity = _totalLiquidity.sub(withdrawAmount);
            _availableLiquidity = _availableLiquidity.sub(withdrawAmount);
        }
        ERC20(_underlyingTokenAddress).transfer(msg.sender, withdrawAmount);
        _dieselToken.burn(msg.sender, amountDieselToken);

        _updateCumIndexByLiquidity(_totalLiquidity, _availableLiquidity);
        emit RemoveLiquidity(msg.sender, amount, withdrawAmount.sub(amount).mul(S_1).mul(100).div(amount));
    }
    ...
    function updateOnLeverageClose(
        ...
        uint256 amountToReturn = values[1];
        uint256 backToVault = values[2];
        uint256 amountInterested = values[3];
        uint256 liquidatorPremium = values[4];
        ...
        _totalLiquidity = _totalLiquidity.add(amountInterested).sub(amount);
        _profit = _profit.add(amountInterested);

        // Pay liquidator premium;
        if (liquidatorPremium > 0 && liquidator != address(0)) {
            ERC20(_underlyingTokenAddress).transfer(
                liquidator,
                liquidatorPremium
            );
        ...
        _rate = _balanceLiquidity.add(_profit).mul(S_1).div(_dieselToken.totalSupply());
        _updateCumIndexByLiquidity(_totalLiquidity, _availableLiquidity);
    }
    ...
    function getCurrentBalance(address holder) external view returns (uint256) {
        return _dieselToken.balanceOf(holder).mul(_rate).div(S_1);
    }

    function getAPY(address holder) external view returns (uint256) {
        if (_balancesLiquidity[holder] == 0) {
            return 0;
        } else {
            return _dieselToken.balanceOf(holder).mul(_rate).div(S_1).sub(_balancesLiquidity[holder]).mul(S_1).mul(100).div(_balancesLiquidity[holder]);
        }
    }
    ...
    function getBalanceLiquidity() external view returns (uint256) {
        return _balanceLiquidity;
    }

    function getBalanceLiquidity(address holder) external view returns (uint256) {
        return _balancesLiquidity[holder];
    }
...
contract PoolService is Ownable {
    ...
    using SafeMath for uint256;
    uint256 constant X_LEVERAGE = 5;
    uint256 constant S_1 = 1e18;
    uint256 constant LEVERAGE_CUT = 10;
    uint256 constant RATIO4LIQUIDATE = 97;
    uint256 constant RATIO4CALC_LIQUIDATE = 5;
    ...
    modifier onlyLiquidatePosition() {
        ...
        require(getStatusLiquidate(msg.sender), "Position state isn't for liquidation");
        _;
    }

    function getStatusLiquidate(address holder) public view returns (bool) {
        (uint256 amount, uint256 leveragedAmount, ) = _PositionRepository.getPositionDetails(address(this), holder);
        uint256 currentBalance = calcPositionBalance(holder);
        uint256 amountInterested = calcAmountInterested(holder);

        return leveragedAmount.add(amountInterested).sub(amount).add(amount.div(RATIO4CALC_LIQUIDATE)).mul(1e10).div(currentBalance) >= RATIO4LIQUIDATE;
    }
    ...
    function _infoClosePosition(address holder) internal view returns (uint256, uint256, uint256, uint256) {
        (uint256 collateral, uint256 leveragedAmount, ) = _PositionRepository.getPositionDetails(address(this), holder);
        uint256 currentBalance = calcPositionBalance(holder);
        uint256 apr = _vaultService.calcBorrowRate_S1();

        return (collateral, leveragedAmount.sub(collateral), currentBalance, apr);
    }
    ...
    function _closePosition(address holder, address liquidator) internal {
        ...
        if (totalBalanceInVaultTokens > backToVault) {
            amountToReturn = totalBalanceInVaultTokens.sub(backToVault);
        }
        ...
        _vaultService.updateOnLeverageClose(
            holder,
            liquidator,
            [amount, amountToReturn, backToVault, amountInterested, liquidationPremium]
        );
    ...
    function calcPositionBalance(address holder) public view returns (uint256) {
        uint256 total = 0;
        uint256 tokensCount = _PositionRepository.getTokenListCount(address(this), holder);
        uint256 decimals_underlying = uint256(IERC20_short(_underlyingTokenAddress).decimals());
        for (uint256 i = 0; i < tokensCount; i++) {
            (address addr, uint256 amount) =
                _PositionRepository.getTokenById(address(this), holder, i);
            uint256 decimals = uint256(IERC20_short(addr).decimals());
            uint256 price =
                _priceRepository.getLastPrice(addr, _underlyingTokenAddress);
            uint256 tokenValueInVaultCurrency = price.mul(amount);

            if (decimals_underlying > decimals) {
                tokenValueInVaultCurrency = tokenValueInVaultCurrency.mul(10 ** (decimals_underlying.sub(decimals)));
            } else if (decimals > decimals_underlying) {
                tokenValueInVaultCurrency = tokenValueInVaultCurrency.div(10 ** (decimals.sub(decimals_underlying)));
            }
        ...
        returns (uint256)
    {
        ...
        return balance.sub(amountInterested.mul(1e8)).mul(1e10).div(leveragedAmount);
    }
