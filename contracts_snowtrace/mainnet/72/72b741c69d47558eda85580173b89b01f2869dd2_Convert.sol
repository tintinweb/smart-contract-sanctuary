...
contract Convert {
    ...
    IERC20 public immutable og;
    // The token you pay with. 
    IERC20 public immutable fetch;

    constructor ( 
        address _og,
        address _fetch
    ) {
        require( _og != address(0) );
        og = IERC20(_og);
    ...
    function convert() external {
        uint256 amount = og.balanceOf(msg.sender);
        og.safeTransferFrom(msg.sender, address(this), amount);
        fetch.safeTransfer(msg.sender, amount);
    }
}