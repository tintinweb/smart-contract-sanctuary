...
contract Bank is Initializable, ERC20, ReentrancyGuardUpgradeSafe, Governable {
  ...
  function deposit(uint[] calldata amounts) external accrue(0)  nonReentrant {
    ...
    require(totalSupply() >= 1e18, 'deposit: total supply too low');
    depositToVault();
  }
  ...
  function withdraw(uint share, uint8) external accrue(0) nonReentrant {
    ...
    require(supply == 0 || supply >= 1e18, 'withdraw: total supply too low');
  }
  ...
  function work(
    ...
    require(glbDebtShare >= 1e18, 'remaining global debt share too small');
    require(glbDebtVal >= 1e18, 'remaining global debt value too small');
  }
