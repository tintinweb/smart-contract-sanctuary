...
contract LoveBondingCalculator is IBondingCalculator {
    ...
    constructor(address _LoveBankERC20) {
    ...
    function markdown(address _pair) external view returns (uint) {
        ...
        uint reserve;
        if (IUniswapV2Pair(_pair).token0() == LoveBankERC20) {
            reserve = reserve1;
        } else {
            reserve = reserve0;
        }
        return reserve.mul(2 * (10 ** IERC20(LoveBankERC20).decimals())).div(getTotalValue(_pair));
    }
}