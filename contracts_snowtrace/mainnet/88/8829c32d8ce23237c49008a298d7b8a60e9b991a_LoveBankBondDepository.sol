...
contract LoveBankBondDepository is Ownable {
    ...
    address public immutable LoveBankERC20; // token given as payment for bond
    ...
    struct Bond {
        uint payout; // LOVE remaining to be paid
    ...
    function deposit(
        ...
        // must be > 0.01 OHM ( underflow protection )
        ...
        if (fee != 0) {// fee is transferred to dao
            IERC20(LoveBankERC20).safeTransfer(DAO, fee);
        }
    ...
    function stakeOrSend(address _recipient, bool _stake, uint _amount) internal returns (uint) {
        if (!_stake) {// if user does not want to stake
            IERC20(LoveBankERC20).transfer(_recipient, _amount);
            // send payout
        } else {// if user wants to stake
            if (useHelper) {// use if staking warmup is 0
                IERC20(LoveBankERC20).approve(stakingHelper, _amount);
                IStakingHelper(stakingHelper).stake(_amount, _recipient);
            } else {
                IERC20(LoveBankERC20).approve(staking, _amount);
    ...
    function maxPayout() public view returns (uint) {
        return IERC20(LoveBankERC20).totalSupply().mul(terms.maxPayout).div(100000);
    }
    ...
    function debtRatio() public view returns (uint debtRatio_) {
        uint supply = IERC20(LoveBankERC20).totalSupply();
