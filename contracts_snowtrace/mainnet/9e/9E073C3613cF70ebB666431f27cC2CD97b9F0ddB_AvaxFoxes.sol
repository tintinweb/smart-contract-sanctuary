...
contract AvaxFoxes is ERC721Metadata {
  using SafeMath for uint256;
  using Counters for Counters.Counter;

  // Max supply of NFTs
  uint256 public constant MAX_NFT_SUPPLY = 9600;
  ...
  constructor(
    string memory baseURI_,
    address admin_
  ) ERC721Metadata("AvaxFoxes", "AVAXFOX", baseURI_) {
    _admin = admin_;
  }
  ...
  function setGiveawayMax(uint256 _giveawayMax) external onlyOwner {
    require(
      ...
      "AvaxFoxes: invalid max value"
    );
    require(giveawayMax != _giveawayMax, "AvaxFoxes: already set");
    giveawayMax = _giveawayMax;
  }

  function randomGiveaway(address to, uint256 tokenId) external onlyOwner {
    require(to != address(0), "AvaxFoxes: zero address");
    require(
      _giveawayCounter.current() < giveawayMax,
      "AvaxFoxes: overflow giveaways"
    );
    _mint(to, tokenId);
  ...
  function mint(uint256 numberOfNfts) external payable {
    require(!paused, "AvaxFoxes: Minting is currently paused");
    require(pendingCount > 0, "AvaxFoxes: All minted");
    require(numberOfNfts > 0, "AvaxFoxes: numberOfNfts cannot be 0");
    require(
      numberOfNfts <= 20,
    ...
    require(
      ...
      "AvaxFoxes: sale already ended"
    );
    require(
      MINT_PRICE.mul(numberOfNfts) == msg.value,
      "AvaxFoxes: invalid ether value"
    );
  ...
  function claimReward(uint256 tokenId) public {
    ...
    require(
      ...
      "AvaxFoxes: can't claim for giveaways"
    );
  ...
  function _splitBalance(uint256 amount) internal {
    uint256 reflectionShare = (amount * 10) / 100;
