...
contract FAILD is Context, IERC20, Ownable {
    ...
    string private _name = "FAILD";
    string private _symbol = "FAILD";
    ...
    uint256 internal _tokenTotal = 100 *10**16; // 100B total supply
    ...
    // fees
    uint256 public _StakingFee = 8; // 100 = 1%
    uint256 public _marketingFee = 0; // 100 = 1%
    uint256 public _BurnFee = 2; // 100 = 1%
    //variables
 ...
 /**
    ...
    */
    ...
    function reflectionFromToken(uint256 tokenAmount, bool deductTransferFee) public view returns (uint256) {
        ...
        if (!deductTransferFee) {
            ...
            return tokenAmount.sub(tokenAmount.mul(_StakingFee).div(10000)).mul(_getReflectionRate());
        }
    }
    ...
    function excludeAccount(address account) external onlyOwner() {
        require(account != 0x10ED43C718714eb63d5aA57B78B54704E256024E,"FAILD: Uniswap router cannot be excluded.");
        require(account != address(this), 'FAILD: The contract it self cannot be excluded');
        require(!_isExcluded[account], "FAILD: Account is already excluded");
    ...
    function includeAccount(address account) external onlyOwner() {
        require(_isExcluded[account], "FAILD: Account is already included");
    ...
    */
    ...
    function delegateBySig(address delegatee, uint nonce, uint expiry, uint8 v, bytes32 r, bytes32 s) external {
        ...
        require(signatory != address(0), "FAILD::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "FAILD::delegateBySig: invalid nonce");
        require(block.timestamp <= expiry, "FAILD::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber) external view returns (uint256) {
        require(blockNumber < block.number, "FAILD::getPriorVotes: not yet determined");
    ...
    function _writeCheckpoint(address delegatee, uint32 nCheckpoints, uint256 oldVotes, uint256 newVotes) internal {
        uint32 blockNumber = safe32(block.number, "FAILD::_writeCheckpoint: block number exceeds 32 bits");
    ...
    function setStakingFee(uint256 fee) public onlyOwner {
        _StakingFee = fee;
    }
    // discrd bedoer#9275
    ...
    function setMarketingAddress(address _Address) public onlyOwner {
        require(_Address != marketingAddress);
