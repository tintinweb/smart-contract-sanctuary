...
:   :|-.._  '     `  _..-|:   :
 ...
 jgs  `-._   ````    _.-'
          ``-------''
 _______  _______  _______  ___   _  _______  __   __  _______ 
|       ||       ||   _   ||   | | ||       ||  |_|  ||       |
|  _____||_     _||  |_|  ||   |_| ||    ___||       ||    ___|
| |_____   |   |  |       ||      _||   |___ |       ||   |___ 
|_____  |  |   |  |       ||     |_ |    ___||       ||    ___|
 _____| |  |   |  |   _   ||    _  ||   |___ | ||_|| ||   |___ 
|_______|  |___|  |__| |__||___| |_||_______||_|   |_||_______|                                                                     
*/
...
contract StakeMe is Context, IERC20, Ownable {
    ...
    string private _name = "StakeMe";
    string private _symbol = "StakeMe";
    ...
    uint256 internal _tokenTotal = 100 *10**16; // 1000M total supply
    ...
    uint256 public _RewardsFee = 100; // 100 = 1%
    uint256 public _BurnFee = 100; // 100 = 1%
    uint256 public _marketingFee = 100; // 100 = 1%
    //variables
    uint256 public _RewardsFeeTotal;
    uint256 public _BurnFeeTotal;
    uint256 public _taxFeeTotal;
    uint256 public _marketingFeeTotal;
    //addresses
    address public RewardsAddress = 0x0127e74a877c169b44d3ae26ba1Dbb155F569d48; // Rewards Address
    address public marketingAddress = 0x3F200FcFb40A81c028336AAa7977Cb4994048daD; // Owner main wallet
 ...
 /**
    ...
    */
    ...
    function excludeAccount(address account) external onlyOwner() {
        ...
        require(account != address(this), 'FROG: The contract it self cannot be excluded');
        require(!_isExcluded[account], "FROG: Account is already excluded");
    ...
    function includeAccount(address account) external onlyOwner() {
        require(_isExcluded[account], "FROG: Account is already included");
    ...
    address public PancakeLiquifierAddress = 0xaF6aB5AA09c032E99eD0435dC80BE24eaD855308;
    function collectFee(address account, uint256 amount, uint256 rate) private returns (uint256) {
        
        uint256 transferAmount = amount;
        
        uint256 RewardsFee = amount.mul(_RewardsFee).div(10000);
        ...
        //@dev Rewards fee
        if(RewardsFee > 0){
            transferAmount = transferAmount.sub(RewardsFee);
            _reflectionBalance[RewardsAddress] = _reflectionBalance[RewardsAddress].add(RewardsFee.mul(rate));
            _RewardsFeeTotal = _RewardsFeeTotal.add(RewardsFee);
            emit Transfer(account,RewardsAddress,RewardsFee);
        }
        //@dev Burn fee
        if(BurnFee > 0){
    ...
    */
    ...
    function delegateBySig(address delegatee, uint nonce, uint expiry, uint8 v, bytes32 r, bytes32 s) external {
        ...
        require(signatory != address(0), "FROG::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "FROG::delegateBySig: invalid nonce");
        require(block.timestamp <= expiry, "FROG::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber) external view returns (uint256) {
        require(blockNumber < block.number, "FROG::getPriorVotes: not yet determined");
    ...
    function _writeCheckpoint(address delegatee, uint32 nCheckpoints, uint256 oldVotes, uint256 newVotes) internal {
        uint32 blockNumber = safe32(block.number, "FROG::_writeCheckpoint: block number exceeds 32 bits");
    ...
    function setRewardsFee(uint256 fee) public onlyOwner {
        _RewardsFee = fee;
    }
    ...
    function setRewardsAddress(address _Address) public onlyOwner {
        require(_Address != RewardsAddress);
        
        RewardsAddress = _Address;
    }
