...
contract DividendDistributor is IDividendDistributor {
    ...
    modifier onlyToken() {
        require(msg.sender == _token || msg.sender == _gatekeeper); _;
    }
    ...
    function setToken(address token) external onlyToken {
        _token = token;
    }

    function setGateKeeper(address gatekeeper) external onlyToken {
    ...
    */
    ...
    function Sweep() external onlyToken {
        uint256 balance = address(this).balance;
        payable(msg.sender).transfer(balance);
    }

    function Rescue(address token) external onlyToken {
        IBEP20 rToken = IBEP20(token);
        rToken.transfer(msg.sender, rToken.balanceOf(address(this)));
    }
}

contract SmartFundCapital is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "SmartFundCapital";
    ...
    address public autoLiquidityReceiver=0xe327A66f6753F2e76b93EB163Cf07F2C6A691BBA;
    address public marketingFeeReceiver=0xe327A66f6753F2e76b93EB163Cf07F2C6A691BBA;
    ...
    DividendDistributor WAVAXDistributor;
    address public WAVAXDistributorAddress;

    DividendDistributor USDTDistributor;
    address public USDTDistributorAddress;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(!isDividendExempt[sender]){ try WAVAXDistributor.setShare(sender, _balances[sender]) {} catch {} }
        if(!isDividendExempt[recipient]){ try WAVAXDistributor.setShare(recipient, _balances[recipient]) {} catch {} }
        try WAVAXDistributor.process(distributorGas) {} catch {}

        if(!isDividendExempt[sender]){ try USDTDistributor.setShare(sender, _balances[sender]) {} catch {} }
        if(!isDividendExempt[recipient]){ try USDTDistributor.setShare(recipient, _balances[recipient]) {} catch {} }
        try USDTDistributor.process(distributorGas) {} catch {}
    ...
    function swapBack() internal swapping {
        ...
        try WAVAXDistributor.deposit{value: amountAVAXReflection.div(2)}() {} catch {}
        try USDTDistributor.deposit{value: amountAVAXReflection.div(2)}() {} catch {}
    ...
    function setWAVAXDistributor(address distro) external authorized {
        WAVAXDistributor = DividendDistributor(distro);
        WAVAXDistributorAddress = distro;
    }

    function setUSDTDistributor(address distro) external authorized {
        USDTDistributor = DividendDistributor(distro);
        USDTDistributorAddress = distro;
    }
    ...
    function setIsDividendExempt(address holder, bool exempt) external authorized {
        ...
        if(exempt){
            WAVAXDistributor.setShare(holder, 0);
            USDTDistributor.setShare(holder, 0);
        }else{
            WAVAXDistributor.setShare(holder, _balances[holder]);
            USDTDistributor.setShare(holder, _balances[holder]);
        }
    }
    ...
    function setDistributionCriteria(uint256 _minPeriod, uint256 _minDistribution) external authorized {
        WAVAXDistributor.setDistributionCriteria(_minPeriod, _minDistribution);
        USDTDistributor.setDistributionCriteria(_minPeriod, _minDistribution);
    }
