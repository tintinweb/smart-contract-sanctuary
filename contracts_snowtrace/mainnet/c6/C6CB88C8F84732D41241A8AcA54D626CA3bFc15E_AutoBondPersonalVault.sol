...
contract AutoBondPersonalVault {
    ...
    function bondWithTimeMimLP(
        ...
        uint256[] memory amounts = joeRouter.getAmountsOut(half, path);
        require(_slippage <= 150, "Slippage greater than 1.5%");
        uint256 minOutput = (amounts[1] * (10000 - _slippage)) / 10000;
        asset.approve(address(joeRouter), _amount);
        uint256[] memory results = joeRouter.swapExactTokensForTokens(
        ...
        // LP tokens
        mim.approve(address(joeRouter), results[1]);
        (uint256 mimSent, uint256 assetSent, ) = joeRouter.addLiquidity(
            path[1], // MIM is first as we want to match that completely
            path[0],
            results[1],
            asset.balanceOf(address(this)),
            (results[1] * 995) / 1000,
            (asset.balanceOf(address(this)) * 995) / 1000,
            address(this),
            block.timestamp + 60
        );

        if (assetSent < half) {
            stakeAssets(asset.balanceOf(address(this)));
        }

        // bond with TIME-MIM LP tokens
        uint256 lpAmount = timeMimJLP.balanceOf(address(this));
        timeMimJLP.approve(address(timeMimLpBondDepository), lpAmount);
        uint256 payout = timeMimLpBondDepository.deposit(
            lpAmount,
            _maxBondPrice,
            address(this)
        );

        // add require(price check)
        emit BondCreated(_amount, address(timeMimJLP), payout);
        return payout;
    }
