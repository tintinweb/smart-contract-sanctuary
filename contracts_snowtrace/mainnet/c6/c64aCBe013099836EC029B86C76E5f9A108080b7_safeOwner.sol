...
import "./MasterDuo.sol";
import "./libs/IERC20.sol";
import "@openzeppelin/contracts/access/Ownable.sol";

contract safeOwner is Ownable{
  address public confirmer;//rugdoc address
  MasterDuo public master;
  address public queuedOwner;
  mapping(address=>bool) public whitelistStrat;
  mapping(address=>bool) public existingLP;
  constructor(address master_,address confirmer_) public {
    master=MasterDuo(master_);
    confirmer=confirmer_;
    for(uint i=0;i<master.poolLength();i++){
      (IERC20 lp,,,,,,,,,)=master.poolInfo(i);
      existingLP[address(lp)]=true;
    }
  }

  function setEthpool(uint rollrate,uint period) external onlyOwner{
    master.setEthpool(rollrate, period);
  }
  function add(uint _allocPoint,uint _ethalloc, IERC20 _lpToken, uint16 _depositFeeBP, address _strat,uint _stratId,bool _withUpdate) external onlyOwner {
    require(!existingLP[address(_lpToken)]);
    existingLP[address(_lpToken)]=true;
    master.add(_allocPoint, _ethalloc, _lpToken, _depositFeeBP, _strat, _stratId, _withUpdate);
  }
  function addwithPermit(uint _allocPoint,uint _ethalloc, IERC20 _lpToken, uint16 _depositFeeBP, address _strat,uint _stratId,bool _withUpdate) external onlyOwner {
    require(whitelistStrat[_strat]==true);
    existingLP[address(_lpToken)]=true;
    master.add(_allocPoint, _ethalloc, _lpToken, _depositFeeBP, _strat, _stratId, _withUpdate);
  }
  function set(uint _pid, uint _allocPoint, uint _ethAlloc, uint16 _depositFeeBP, bool _withUpdate) public onlyOwner {
    master.set(_pid, _allocPoint, _ethAlloc, _depositFeeBP, _withUpdate);
  }
  function setApprovedContract(address _contract, bool _status) external onlyOwner {
    master.setApprovedContract(_contract, _status);
  }
  function updateEmissionRate(uint _duoPerSec) external onlyOwner {
    master.updateEmissionRate(_duoPerSec);
  }

  function queueNewowner(address newSafeOwner) external {
    require(msg.sender==confirmer && queuedOwner!=address(0),"wrong caller");
    queuedOwner=newSafeOwner;
  }
  function executeTransferOwnership() external onlyOwner{
    master.transferOwnership(queuedOwner);
    queuedOwner=address(0);
  }
  function allowstrat(address _strat) external{
    require(msg.sender==confirmer,"wrong caller");
    whitelistStrat[_strat]=true;
  }
}
