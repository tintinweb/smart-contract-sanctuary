...
contract FarmersOnlyMasterChefL2 is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    event UpdateEmissionRate(address indexed user, uint256 indexed tomatoPerSecond);
    ...
    function pendingTomato(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 pending = user.amount.mul(accTomatoPerShare).div(1e18).sub(user.rewardDebt);
        return pending.add(user.rewardLockedUp);
    }


    // View function to see if user can harvest Tomatos's.
    ...
    // Pay or lockup pending tomato.
    function payOrLockupPendingTomato(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
    ...
    function safeTomatoTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > tomatoBal) {
            transferSuccess = tomato.transfer(_to, tomatoBal);
        } else {
            transferSuccess = tomato.transfer(_to, _amount);
        }
        require(transferSuccess, "safeTomatoTransfer: transfer failed");
    }
