...
/**
Website: https://ponzi.financial
Discord: https://discord.gg/PRcMew9e5r
Twitter: https://twitter.com/PonziNodes
*/
...
contract NODERewardManagement {
    ...
    uint256 public rewardsPerMinute = 69444444444444;
    ...
    function createNode(address account, string memory nodeName) external onlyOwner {
        require(
        ...
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < nodesCount; i++) {
            _node = nodes[i];
            uint reward = dividendsOwing(_node);
            rewardsTotal += reward;
            _node.dividendsPaid += reward;
        }
        return rewardsTotal;
    }
        ...
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < nodesCount; i++) {
            rewardCount += dividendsOwing(nodes[i]);
        }
    ...
    {
        return
            dividendsOwing(_getNodeWithCreatime(_nodesOfUser[account], creationTime));
    }
	...
	function dividendsOwing(NodeEntity memory node) private view returns (uint256 availableRewards) {
		uint256 minutesPassed = (block.timestamp).sub(node.creationTime).div(60);
		return minutesPassed.mul(rewardsPerMinute).add(0.1 * 10 ** 18).sub(node.dividendsPaid);
	}
    ...
    function _changeNodePrice(uint256 newNodePrice) external onlyOwner {
        nodePrice = newNodePrice;
    }

    function _changeAutoDistri(bool newMode) external onlyOwner {
        autoDistri = newMode;
    }

    function _changeGasDistri(uint256 newGasDistri) external onlyOwner {
...
contract PonziNodes is ERC20, Ownable, PaymentSplitter {
    using SafeMath for uint256;
    ...
    ) ERC20("PonziNodes", "PONZI") PaymentSplitter(payees, shares) {
    ...
    // function setProectSale(bool newProtectSale) external onlyOwner {
    //     protectSale = newProtectSale;
    // }
