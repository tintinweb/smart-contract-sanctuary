...
import {AVAX} from "../../constants/Tokens.sol";
...
function _swapExactAVAXForTokens(
    ...
    try
        _pangolinRouter.swapExactAVAXForTokens{value: _amountIn}(
            _amountOutMin,
            _path,
            _to,
            _deadline
...
function _swapExactTokensForAVAX(
    ...
    try
        _pangolinRouter.swapExactTokensForAVAX(
            _amountIn,
            _amountOutMin,
            _path,
            _to,
...
function _swapExactTokensForTokens(
    ...
    try
        _pangolinRouter.swapExactTokensForTokens(
            _amountIn,
            _amountOutMin,
            _path,
            _to,
...
function _swapTokensForExactAVAX(
    ...
    try
        _pangolinRouter.swapTokensForExactAVAX(
            _amountOut,
            _amountInMax,
            _path,
            _to,
...
} from "../../functions/pangolin/FPangolin.sol";

/// @notice Pangolin Handler used to execute an order via PangolinRouter02
/// @dev This does NOT implement the standard IHANDLER
contract PangolinHandler is IHandler {
    using TokenUtils for address;

    // solhint-disable var-name-mixedcase
    IPangolinRouter public PANGOLIN_ROUTER;
    address public immutable WAVAX;

    // solhint-enable var-name-mixedcase

    constructor(IPangolinRouter _pangolinRouter, address _wavax) {
        PANGOLIN_ROUTER = _pangolinRouter;
        WAVAX = _wavax;
    }

    /// @notice receive AVAX from JoeRouter02 during swapXForAvax
    receive() external payable override {
        require(
            msg.sender != tx.origin,
            "JoeRouter02Handler#receive: NO_SEND_AVAX_PLEASE"
        );
    }
    ...
    function handle(
        ...
        if (inToken == WAVAX || inToken == AVAX) {
            if (inToken == WAVAX) IWETH(WAVAX).withdraw(fee);
            bought = _swap(amountIn - fee, _amountOutMin, path, msg.sender);
        } else if (outToken == WAVAX || outToken == AVAX) {
            bought = _swap(amountIn, _amountOutMin + fee, path, address(this));
            if (outToken == WAVAX) IWETH(WAVAX).withdraw(fee);
    ...
    function _getAmountOut(uint256 _amountIn, address[] memory _path)
        ...
        returns (uint256 amountOut)
    {
        uint256[] memory amountsOut = PANGOLIN_ROUTER.getAmountsOut(
            _amountIn,
            _path
        );
    ...
    function _getAmountIn(uint256 _amountOut, address[] memory _path)
        ...
        returns (uint256 amountIn)
    {
        uint256[] memory amountsIn = PANGOLIN_ROUTER.getAmountsIn(_amountOut, _path);
