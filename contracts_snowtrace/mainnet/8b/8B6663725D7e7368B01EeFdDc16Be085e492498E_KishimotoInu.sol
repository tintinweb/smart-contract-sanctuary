...
contract KishimotoInu is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 60000000000 * 10**6 * 10**9;
    ...
    // limit
    uint256 public _maxTxAmount = 7500000000000 * 10**2 * 10**9; 
    ...
    uint256 public minTokensBeforeSwap = 8000 * 10**9;
    ...
    // @dev set Pair
    function setPair(address _joePair) external onlyOwner {
        joePair = _joePair;
    }

    // @dev set Router
    function setRouter(address _newJoeRouter) external onlyOwner {
        IJoeRouter02 _joeRouter = IJoeRouter02(_newJoeRouter);
    ...
    function addBotToBlackList(address account) external onlyOwner() {
        require(account != 0x60aE616a2155Ee3d9A68541Ba4544862310933d4, 'We can not blacklist joe router.');
    ...
    function expectedRewards(address _sender) external view returns(uint256){
        ...
        uint256 rewards = holdersBal.mul(_balance).div(_tTotal.sub(balanceOf(joePair)).sub(totalExcludedBal));
        return rewards;
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        swapTokensForAvax(half.add(devExp).add(forRewards)); // <- this breaks the AVAX -> HATE swap when swap+liquify is triggered
       ...
       // for(uint256 i = 0; i < numberOfTokenHolders; i++){
         //   uint256 share = (balanceOf(tokenHolder[i]).mul(avaxFees)).div(totalSupply());
