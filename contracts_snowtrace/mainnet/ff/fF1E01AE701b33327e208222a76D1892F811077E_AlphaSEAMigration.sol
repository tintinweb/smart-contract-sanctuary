...
contract AlphaSEAMigration is Ownable {
  ...
  function initialize(address _SEA, address _aSEA) public onlyOwner {
    SEA = IERC20(_SEA);
    aSEA = IERC20(_aSEA);
    isInitialized = true;
  }
  ...
  function check(address _user) public view returns (uint256) {
    return
      available(block.timestamp, aSEA.balanceOf(_user), lastMigration[_user]);
  }
  ...
  function migrate() external {
    ...
    require(
      aSEA.allowance(msg.sender, address(this)) >= availableTokens,
      "Base token allowance too low."
    );
    aSEA.transferFrom(msg.sender, address(this), availableTokens);
    lastMigration[msg.sender] = block.timestamp;
    SEA.transfer(msg.sender, availableTokens);
  }

  function withdrawAll() external onlyOwner {
    require(
      block.timestamp > steps[steps.length - 1],
      "Migration of aSEA has not ended"
    );
