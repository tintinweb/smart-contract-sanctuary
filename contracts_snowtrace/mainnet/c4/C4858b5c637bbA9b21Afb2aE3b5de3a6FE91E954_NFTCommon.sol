...
import "./IXIOVaultLight.sol";

contract NFTCommon is ERC721URIStorage, Ownable {
    using Counters for Counters.Counter;
    Counters.Counter private tokenIds;

    string constant ipfsURL = "ipfs://QmRQQyBLtHMkFHrniLqKPZYaDmX2pshiStsEnL6npRsUwM/111.json";
    uint256 constant hardCap = 1000;

    // Amount to transfer into NFT
    uint256 constant depositAmount = 2500 * 10**18;
    ...
    function contractURI() public view returns (string memory) {
        return "ipfs://QmdRbvXsueFwhMdES3UBEn8vvhDoCQ2TD3Zzb9bwnM1N75";
    }
    ...
    function approveSpending(address _erc20Address, uint256 _amount) public onlyOwner returns (bool) {
        IERC20(_erc20Address).approve(vaultContractAddress, _amount);
    ...
    function mint(address _recipientAddress) public virtual returns (uint256) {
        ...
        require(vaultContractAddress != address(0), "VAULT IS INVALID");
        ...
        IXIOVaultLight(vaultContractAddress).deposit(newUniqueId, depositAmount);

        return newUniqueId;
    }

    function bulkMint(address[] memory _recipientAddresses) public onlyOwner returns (uint256[] memory) {
...
interface IXIOVaultLight {
    function deposit(uint256 nftId, uint256 tokenAmount) external;
}
