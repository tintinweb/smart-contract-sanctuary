...
// File contracts/libs/IStrategy.sol
...
contract VaultChef is ReentrancyGuard, Ownable {
    ...
    function _deposit(uint256 _pid, uint256 _wantAmt, address _to) internal {
        ...
        if (_wantAmt > 0) {
            ...
            pool.want.safeTransferFrom(_msgSender(), address(this), _wantAmt);
    ...
    function _withdraw(uint256 _pid, uint256 _wantAmt, address _to) internal {
        ...
        if (_wantAmt > 0) {
            uint256 sharesRemoved = IStrategy(pool.strat).withdraw(_msgSender(), _wantAmt);
    ...
    function earnAll() external onlyOperator {
        for (uint256 i=0; i<poolInfo.length; i++) {
    ...
    function earnSome(uint256[] memory pids) external onlyOperator {
