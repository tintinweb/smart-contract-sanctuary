...
contract DefiYieldTokenVesting is AccessControl {
    ...
    uint256 public constant ALLOWED_VESTING_UPDATE_THRESHOLD = 360 days;
    ...
    modifier onlyAdmin() {
        require(
            ...
            "DefiYieldTokenVesting::onlyAdmin: admin role required"
        );
        _;
    }
    ...
    modifier onlyOperator() {
        require(
            ...
            "DefiYieldTokenVesting::onlyOperator: operator role required"
        );
        _;
    }
    ...
    function recoverErc20(address token_, uint256 amount_) external onlyAdmin {
        require(
            ...
            "DefiYieldTokenVesting::recoverErc20: cannot recover vesting tokens"
        );
    ...
    function recoverEther() external onlyAdmin {
        ...
        require(
            success,
            "DefiYieldTokenVesting::recoverEther: failed to send Ether"
        );
    ...
    function initVestingPool(
        ...
        require(
            ...
            "DefiYieldTokenVesting::initVestingPool: invalid pool start time"
        );
        require(
            startTime_ < endTime_,
            "DefiYieldTokenVesting::initVestingPool: invalid pool end time"
        );
    ...
    /**
     * @dev Updates parameters of the vesting pool.
     * @param pid_  pool id.
     * @param name_  name of pool.
     * @param startTime_  pool start time.
     * @param endTime_  pool end time.
     */
    function updateVestingPool(
        uint8 pid_,
        string calldata name_,
        uint256 startTime_,
        uint256 endTime_
        ...
        require(
            lockPools[pid_].startTime != 0,
            "DefiYieldTokenVesting::updateVestingPool: pool does not exist"
        );
        require(
            lockPools[pid_].startTime > block.timestamp,
            "DefiYieldTokenVesting::updateVestingPool: pool is already running"
        );
        require(
            startTime_ > block.timestamp,
            "DefiYieldTokenVesting::updateVestingPool: invalid pool start time"
        );
        require(
            startTime_ < endTime_,
            "DefiYieldTokenVesting::updateVestingPool: invalid pool end time"
        );
        require(
            lockPools[pid_].initialStartTime +
                ALLOWED_VESTING_UPDATE_THRESHOLD >=
                startTime_,
            "DefiYieldTokenVesting::updateVestingPool: new start date is to large"
        );
        require(
            lockPools[pid_].initialStartTime -
                ALLOWED_VESTING_UPDATE_THRESHOLD <=
                startTime_,
            "DefiYieldTokenVesting::updateVestingPool: new start date is to small"
        );
        require(
            lockPools[pid_].initialEndTime + ALLOWED_VESTING_UPDATE_THRESHOLD >=
                endTime_,
            "DefiYieldTokenVesting::updateVestingPool: new end date is to large"
        );
        require(
            lockPools[pid_].initialEndTime - ALLOWED_VESTING_UPDATE_THRESHOLD <=
                endTime_,
            "DefiYieldTokenVesting::updateVestingPool: new end date is to small"
        );
    ...
    function addBeneficiary(
        ...
        require(
            pid_ < lockPools.length,
            "DefiYieldTokenVesting::addBeneficiary: non existing pool"
        );

        token.safeTransferFrom(_msgSender(), address(this), lockedAmount_);
        userInfo[pid_][beneficiary_].lockedAmount =
            userInfo[pid_][beneficiary_].lockedAmount +
            lockedAmount_;
        lockPools[pid_].totalLocked =
            lockPools[pid_].totalLocked +
            lockedAmount_;
    ...
    function addBeneficiaryBatches(
        ...
        require(
            beneficiaries_.length == lockedAmounts_.length,
            "DefiYieldTokenVesting::addBeneficiaryBatches: params invalid length"
        );
        require(
            pid_ < lockPools.length,
            "DefiYieldTokenVesting::addBeneficiaryBatches: non existing pool"
        );
        ...
        for (uint8 i = 0; i < beneficiariesLength; i++) {
            ...
            userInfo[pid_][beneficiary].lockedAmount =
                userInfo[pid_][beneficiary].lockedAmount +
                lockedAmount;
            lockPools[pid_].totalLocked =
                lockPools[pid_].totalLocked +
     ...
     */
    function disableBeneficiary(uint8 pid_, address beneficiary_)
        external
        onlyAdmin
    {
        require(
            pid_ < lockPools.length,
            "DefiYieldTokenVesting::disableBeneficiary: non existing pool"
        );
        require(
            !userInfo[pid_][beneficiary_].disabled,
            "DefiYieldTokenVesting::disableBeneficiary: already disabled"
        );
        require(
            userInfo[pid_][beneficiary_].withdrawn <
                userInfo[pid_][beneficiary_].lockedAmount,
            "DefiYieldTokenVesting::disableBeneficiary: no funds to secure"
        );

        uint256 securedAmount = userInfo[pid_][beneficiary_].lockedAmount -
            userInfo[pid_][beneficiary_].withdrawn;
        userInfo[pid_][beneficiary_].withdrawn = userInfo[pid_][beneficiary_]
            .lockedAmount;
        userInfo[pid_][beneficiary_].disabled = true;

        token.safeTransfer(_msgSender(), securedAmount);

        emit BeneficiaryDisabled(pid_, beneficiary_, securedAmount);
    }
    ...
    function claim(uint8 pid_) external returns (uint256 amount) {
        require(
            !userInfo[pid_][_msgSender()].disabled,
            "DefiYieldTokenVesting::claim: beneficiary is disabled"
        );
        amount = getReleasableAmount(pid_, _msgSender());
        require(
            amount > 0,
            "DefiYieldTokenVesting::claim: can't claim 0 amount"
        );

        userInfo[pid_][_msgSender()].withdrawn =
            userInfo[pid_][_msgSender()].withdrawn +
            amount;
    ...
    {
        return
            getVestedAmount(pid_, beneficiary_, block.timestamp) -
            userInfo[pid_][beneficiary_].withdrawn;
    }
    ...
    function getVestedAmount(
        ...
        if (userInfo[pid_][beneficiary_].disabled) {
            return userInfo[pid_][beneficiary_].lockedAmount;
        }
        if (pid_ >= lockPools.length) {
            return 0;
        }
        if (time_ < lockPools[pid_].startTime) {
            return 0;
        }
        ...
        uint256 vestingDuration = lockPools[pid_].endTime -
            lockPools[pid_].startTime;
        ...
        )
    {
        ...
        if (pid_ < lockPools.length) {
            ...
            disabled = userInfo[pid_][beneficiary_].disabled;
