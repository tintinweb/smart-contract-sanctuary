...
contract ConeMasterChef is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of CONE
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accConePerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accConePerShare` (and `lastRewardSecond`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 lastRewardSecond;  // Last second that CONEs distribution occurs.
        uint256 accConePerShare;   // Accumulated CONEs per share, times 1e18. See below.
    ...
    // The CONE TOKEN!
    ConeCoin public immutable cone;
    // Dev address.
    address public devaddr;
    // Deposit Fee address
    address public feeAddress;
    // CONE tokens created per second.
    ...
    // The block timestamp when CONE farming starts.
    ...
    // Initial ConePerTime
    uint256 public EMISSION_RATE = 0.007 ether;
    ...
    event UpdateEmissionRate(address indexed user, uint256 conePerSecond);
    ...
    event ConeMintError(bytes reason);

    constructor(
        ConeCoin _cone,
        address _devaddr,
        address _feeAddress,
        // uint256 _ConePerTime,
        uint256 _startTime
    ) public {
        cone = _cone;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        conePerSecond = EMISSION_RATE;
    ...
    function pendingCone(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardSecond && pool.lpSupply != 0 && totalAllocPoint > 0) {
            ...
            uint256 coneReward = multiplier.mul(conePerSecond).mul(pool.allocPoint).div(totalAllocPoint);
            accConePerShare = accConePerShare.add(coneReward.mul(1e18).div(pool.lpSupply));
        }
        uint256 pending = user.amount.mul(accConePerShare).div(1e18).sub(user.rewardDebt);
    ...
    function updatePool(uint256 _pid) public {
        ...
        try cone.mint(devaddr, coneReward.div(10)) {
        } catch (bytes memory reason) {
            coneReward = 0;
            emit ConeMintError(reason);
        }

        try cone.mint(address(this), coneReward) {
        } catch (bytes memory reason) {
            coneReward = 0;
            emit ConeMintError(reason);
        }

        pool.accConePerShare = pool.accConePerShare.add(coneReward.mul(1e18).div(pool.lpSupply));
        pool.lastRewardSecond = block.timestamp;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
          uint256 pending =user.amount.mul(pool.accConePerShare).div(1e18).sub(user.rewardDebt);
        if (pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accConePerShare).div(1e18);
    ...
    function payOrLockupPending(uint256 _pid) internal {
        ...
        if (canHarvest(_pid, msg.sender)) {
            if (pending > 0 || user.rewardLockedUp > 0) {
                ...
                safeConeTransfer(msg.sender, totalRewards);
            }
        } else if (pending > 0) {
    ...
    // Safe test transfer function, just in case if rounding error causes pool to not have enough CONEs.
    function safeConeTransfer(address _to, uint256 _amount) internal {
        uint256 coneBal = cone.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > coneBal) {
            transferSuccess = cone.transfer(_to, coneBal);
        } else {
            transferSuccess = cone.transfer(_to, _amount);
        }
        require(transferSuccess, "safeConeTransfer: transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _conePerSecond) external onlyOwner {
        require(_conePerSecond <= MAX_EMISSION_RATE, "Too high");
        massUpdatePools();
        conePerSecond = _conePerSecond;
        emit UpdateEmissionRate(msg.sender, _conePerSecond);
    }
