 ...
 */
 contract Presale_SPC is Owned{
    ...
    function setPreBuys(address[] memory addresses, uint amounts) public onlyOwner{
        for (uint i = 0; i < addresses.length; i++) {
            preBuys[addresses[i]].mimAmount = amounts;
            mimRaised = mimRaised.add(amounts);
        }
    }
    ...
    function buy(uint _amount) public {
        ...
        require(paidTotal[msg.sender] + _amount <= maxMimLimit);
    ...
    function getClaimableAmount(address _account) public view returns (uint) {
        ...
        uint totalpTokenToClaim = preBuys[_account].mimAmount.mul(rate).div(RATE_DECIMALS).mul(percentReleased).div(VESTING_TIME_DECIMALS);
    ...
    function claim() public {
        require(!isPresaleOpen, "Presale is not over yet");
        require(vipWhiteListed[msg.sender] == true || whiteListed[msg.sender] == true);
        uint percentReleased = getPercentReleased();
