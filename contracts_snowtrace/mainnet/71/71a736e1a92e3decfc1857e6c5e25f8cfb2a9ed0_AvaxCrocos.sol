...
// File: contracts/AvaxCrocos.sol


// File: tests/AvaxCrocos.sol
 ...
 */
contract AvaxCrocos is Context,  AccessControlEnumerable, ERC721Enumerable, ERC721URIStorage{
  ...
  function setBaseURI(string memory baseURI) external {
    require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "AvaxCrocos: must have admin role to change base URI");
    _baseTokenURI = baseURI;
  }

  function setTokenURI(uint256 tokenId, string memory _tokenURI) external {
    require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "AvaxCrocos: must have admin role to change token URI");
    _setTokenURI(tokenId, _tokenURI);
  }

  function setPrice(uint mintPrice) external {
    require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "AvaxCrocos: must have admin role to change price");
    _price = mintPrice;
  }
  ...
  function mint(uint amount) public payable {
    require(msg.value == _price*amount, "AvaxCrocos: must send correct price");
    require(_tokenIdTracker.current() + amount <= _max, "AvaxCrocos: not enough avax croco left to mint amount");
  ...
  function claimReward(uint256 tokenId) public {
    require(ownerOf(tokenId) == _msgSender() || getApproved(tokenId) == _msgSender(), "AvaxCrocos: Only owner or approved can claim rewards");
