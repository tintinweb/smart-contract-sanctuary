 ...
 * @author Domani Protocol
 *
 * Contract that houses state for approvals and system contracts such as added Domani tokens,
...
contract Controller is Ownable {
  ...
  event DomaniAdded(address indexed _domani, address indexed _factory);
  event DomaniRemoved(address indexed _domani);
  ...
  // List of enabled Domani tokens
  address[] public sets;
  // List of enabled factories of Domani tokens
  address[] public factories;
  // List of enabled Modules; Modules extend the functionality of Domani tokens
  address[] public modules;
  // List of enabled Resources; Resources provide data, functionality, or
  // permissions that can be drawn upon from Module, Domani tokens or factories
  address[] public resources;

  // Mappings to check whether address is valid Domani, Factory, Module or Resource
  mapping(address => bool) public isDomani;
  ...
  function initialize(
    ...
    // Domani to true to only allow initialization once
    isInitialized = true;
  }

  /**
   * PRIVILEGED FACTORY FUNCTION. Adds a newly deployed Domani tokens as an enabled Domani.
   *
   * @param _domani               Address of the Domani contract to add
   */
  ...
  /**
   * PRIVILEGED GOVERNANCE FUNCTION. Allows governance to remove a Domani
   *
   * @param _domani               Address of the Domani contract to remove
   */
  function removeDomani(address _domani) external onlyInitialized onlyOwner {
    require(isDomani[_domani], "Domani does not exist");

    sets = sets.remove(_domani);

    isDomani[_domani] = false;

    emit DomaniRemoved(_domani);
  }
  ...
  function getDomanis() external view returns (address[] memory) {
    return sets;
  }

  /**
   * Check if a contract address is a module, Domani, resource, factory or controller
   *
   * @param  _contractAddress           The contract address to check
   */
  function isSystemContract(address _contractAddress) external view returns (bool) {
    return (isDomani[_contractAddress] ||
