...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. MINTs to distribute per second.
        uint256 lastRewardSecond; // Last second that MINTs distribution occurs.
        uint256 accMintPerShare; // Accumulated MINTs per share, times 1e18. See below.
        uint16 depositFeeBP; // Deposit fee in basis points
        uint256 lpSupply;
    }

    // The MINT TOKEN!
    LeafToken public mint;
    // Dev address.
    address public devaddr;
    // MINT tokens created per second.
    ...
    // The block timestamp when Mint mining starts.
    ...
    event UpdateEmissionRate(address indexed user, uint256 MintPerSecond);
    event addPool(
        uint256 indexed pid,
        address lpToken,
        uint256 allocPoint,
    ...
    // View function to see pending Mint on frontend.
        ...
        returns (uint256)
    {
        ...
        if (
            ...
            uint256 mintReward = multiplier
                .mul(MintPerSecond)
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
            accMintPerShare = accMintPerShare.add(
                mintReward.mul(1e18).div(pool.lpSupply)
            );
        }
        return user.amount.mul(accMintPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 devReward = mintReward.div(10);
        uint256 totalRewards = mint.totalSupply().add(devReward).add(
            mintReward
        );

        if (totalRewards <= mint.maxSupply()) {
            // mint as normal as not at maxSupply
            mint.mint(devaddr, mintReward.div(10));
        ...
        if (mintReward != 0) {
            // only calculate and update if mintReward is non 0
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        user.rewardDebt = user.amount.mul(pool.accMintPerShare).div(1e18);
    ...
    function withdraw(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accMintPerShare).div(1e18).sub(
            user.rewardDebt
        );
        if (pending > 0) {
            safeMintTransfer(msg.sender, pending);
    ...
    function updateEmissionRate(uint256 _MintPerSecond) external onlyOwner {
        require(_MintPerSecond <= MAX_EMISSION_RATE, "Emission too high");
        massUpdatePools();
        MintPerSecond = _MintPerSecond;
        emit UpdateEmissionRate(msg.sender, _MintPerSecond);
    }
