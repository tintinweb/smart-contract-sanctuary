...
contract StrategyXJoe is StratManager, FeeManager {
    ...
    ) StratManager(_keeper, _strategist, _unirouter, _vault, _platformFeeRecipient) public {
        want = _want;
        xWant = _xWant;
        chef = _chef;
        pid = _pid;
    ...
    function deposit() public whenNotPaused {
        ...
        if (wantBal > 0) {
            ...
            IMasterChef(chef).deposit(pid, xWantBal);
        }
    }
    ...
    function withdraw(uint256 _amount) external {
        ...
        uint256 wantBal = balanceOfWant();
        uint256 xWantBal = balanceOfXWant();
        uint256 xAmount = tokenForxToken(_amount);

        if (wantBal < _amount) {
            IMasterChef(chef).withdraw(pid, xAmount.sub(xWantBal));
    ...
    function balanceOfPool() public view returns (uint256) {
        return xTokenForToken(balanceOfXPool());
    }
    
    // it calculates how much 'xWant' the strategy has working in the farm.
    function balanceOfXPool() public view returns (uint256) {
        (uint256 xWantBal,) = IMasterChef(chef).userInfo(pid, address(this));
        return xWantBal;
    }
    
    function tokenForxToken(uint256 _amount) public view returns (uint256) {
        return _amount.mul(exchangeRate()).div(1e18);
    }
    
    function xTokenForToken(uint256 _amount) public view returns (uint256) {
        return _amount.mul(1e18).div(exchangeRate());
    }
    
    function exchangeRate() public view returns (uint256) {
        return IERC20(xWant).totalSupply().mul(1e18).div(IERC20(want).balanceOf(xWant));
    }
    ...
    function retireStrat() external {
        ...
        IMasterChef(chef).withdraw(pid, balanceOfXPool());
