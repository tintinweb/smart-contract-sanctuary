...
contract OPSYFarm is Ownable, Pausable {
    ...
    function deposit(uint256 _pid, uint256 _amount) external onlyAfterInit whenNotPaused{
        ...
        if (user.amount > 0) {
            ...
            uint256 feeWalletAmount = pendingAmount * pool.claimFee / 1000;
    ...
    function withdraw(uint256 _pid, uint256 _amount) external whenNotPaused {
        ...
        uint256 pendingAmount = user.amount.mul(pool.accERC20PerShare).div(1e36).sub(user.rewardDebt);
        uint256 feeRewardAmount = pendingAmount.mul(pool.claimFee).div(1000);
        ...
        user.rewardDebt = user.amount.mul(pool.accERC20PerShare).div(1e36);
        uint256 feeWalletAmount = _amount * pool.withdrawFee / 1000;
        if (feeWalletAmount > 0)
            pool.lpToken.safeTransfer(feeWallet, feeWalletAmount);
        pool.lpToken.safeTransfer(address(msg.sender), _amount - feeWalletAmount);
