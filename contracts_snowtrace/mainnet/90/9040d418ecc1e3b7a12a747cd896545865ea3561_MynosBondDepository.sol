...
contract MynosBondDepository is Ownable {
    ...
    address public immutable Mynos; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints MYNOS when receives principle
    ...
    struct Bond {
        uint payout; // MYNOS remaining to be paid
    ...
    function deposit(
        ...
        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 MYNOS ( underflow protection )
        ...
        /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) MYNOS
         */
        ...
        if ( fee != 0 ) { // fee is transferred to dao
            IERC20( Mynos ).safeTransfer( DAO, fee );
        }
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            IERC20( Mynos ).transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                IERC20( Mynos ).approve( stakingHelper, _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return IERC20( Mynos ).totalSupply().mul( terms.maxPayout ).div( 100000 );
    }
    ...
    /**
     *  @notice calculate current ratio of debt to MYNOS supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns ( uint debtRatio_ ) {
        uint supply = IERC20( Mynos ).totalSupply();
    ...
    /**
     *  @notice calculate amount of MYNOS available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or MYNOS) to the DAO
     *  @return bool
     */
    function recoverLostToken( address _token ) external returns ( bool ) {
        require( _token != Mynos );
