...
contract AWnDGame is IWnDGame, Ownable, ReentrancyGuard, Pausable {
  ...
  uint256 public salePrice = 2.5 ether;
  uint256 public treasureChestTypeId;
  
  // max $aGP cost 
  uint256 private maxAGpCost = 72000 ether;


  // reference to the Tower for choosing random Dragon thieves
  ITower public tower;
  // reference to $aGP for burning on mint
  IGP public agpToken;
  // reference to Traits
  ...
  modifier requireContractsSet() {
      require(address(agpToken) != address(0) && address(traits) != address(0) 
        && address(awndNFT) != address(0) && address(tower) != address(0) && address(alter) != address(0)
         && address(randomizer) != address(0)
        , "Contracts not set");
      _;
  }

  function setContracts(address _agp, address _traits, address _awnd, address _tower, address _alter, address _rand) external onlyOwner {
    agpToken = IGP(_agp);
   ...
   * The first 25% are claimed with eth, the remaining cost $aGP
   */
  function mint(uint256 amount, bool stake) external payable whenNotPaused nonReentrant {
    require(tx.origin == _msgSender(), "Only EOA");
    uint16 minted = awndNFT.minted();
    uint256 maxTokens = awndNFT.getMaxTokens();
    uint256 paidTokens = awndNFT.getPaidTokens();
    ...
    if (minted < paidTokens) {
      ...
      require(amount * salePrice <= msg.value, "Invalid payment amount"); 
    ...
    for (uint i = 0; i < amount; i++) {
      ...
      if (!stake || recipient != _msgSender()) {
        awndNFT.mint(recipient, seed);
      } else {
        awndNFT.mint(address(tower), seed);
      }
      totalAGpCost += mintCost(minted, maxTokens, paidTokens);
    }
    awndNFT.updateOriginAccess(tokenIds);
    if (totalAGpCost > 0) {
      agpToken.burn(_msgSender(), totalAGpCost);
  ...
  function mintCost(uint256 tokenId, uint256 maxTokens, uint256 paidTokens) public view returns (uint256) {
    ...
    if (tokenId > maxTokens * 17 / 20) return maxAGpCost;
  }

  function payTribute(uint256 agpAmt) external whenNotPaused nonReentrant {
    require(tx.origin == _msgSender(), "Only EOA");
    uint16 minted = awndNFT.minted();
    uint256 maxTokens = awndNFT.getMaxTokens();
    uint256 paidTokens = awndNFT.getPaidTokens();
    uint256 agpMintCost = mintCost(minted, maxTokens, paidTokens);
    require(agpMintCost > 0, "Sacrificial alter currently closed");
    require(agpAmt >= agpMintCost, "Not enough agp given");
    agpToken.burn(_msgSender(), agpAmt);
    if(agpAmt < agpMintCost * 2) {
  ...
  function makeTreasureChests(uint16 qty) external whenNotPaused {
    ...
    // $aGP exchange amount handled within alter contract
    // Will fail if sender doesn't have enough $aGP
  ...
  function sacrifice(uint256 tokenId, uint256 agpAmt) external whenNotPaused nonReentrant {
    require(tx.origin == _msgSender(), "Only EOA");
    ...
    IWnD.WizardDragon memory nft = awndNFT.getTokenTraits(tokenId);
    uint16 minted = awndNFT.minted();
    uint256 maxTokens = awndNFT.getMaxTokens();
    uint256 paidTokens = awndNFT.getPaidTokens();
    uint256 agpMintCost = mintCost(minted, maxTokens, paidTokens);
    require(agpMintCost > 0, "Sacrificial alter currently closed");
    if(nft.isWizard) {
      // Wizard sacrifice requires 3x $aGP curve
      require(agpAmt >= agpMintCost * 3, "not enough agp provided");
      agpToken.burn(_msgSender(), agpAmt);
      // This will check if origin is the owner of the token
      awndNFT.burn(tokenId);
      alter.mint(3, 1, _msgSender());
    }
    else {
      // Dragon sacrifice requires 4x $aGP curve
      require(agpAmt >= agpMintCost * 4, "not enough agp provided");
      agpToken.burn(_msgSender(), agpAmt);
      // This will check if origin is the owner of the token
      awndNFT.burn(tokenId);
  ...
  function setMaxAGpCost(uint256 _amount) external requireContractsSet onlyOwner {
    maxAGpCost = _amount;
  } 
