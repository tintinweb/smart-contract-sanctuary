...
contract FarmHelper {
    bytes4 public constant PoolInfoSel =
        bytes4(keccak256(bytes("poolInfo(uint256)")));
    bytes4 public constant UserInfoSel =
        bytes4(keccak256(bytes("userInfo(uint256,address)")));
    bytes4 public constant PendingSushiSel =
        bytes4(keccak256(bytes("pendingSushi(uint256,address)")));
    bytes4 public constant DecimalsSel = bytes4(keccak256(bytes("decimals()")));
    bytes4 public constant SupportsInterfaceSel =
    ...
    function _userInfo(
        ...
        for (uint256 i = 0; i < pids_.length; i++) {
            ...
            (p.userData0, p.userData1, p.userData2, p.userData) = _userInfo(
                masterchef_,
                userInfoSel_,
                pids_[i],
                account_
            );
            (p.pending0, p.pending1) = _pendingSushi(
                masterchef_,
                pendingSushiSel_,
                pids_[i],
                account_
            ...
            if (p.isErc721) {
                bool isApproveAll = IERC721(p.token).isApprovedForAll(
                    account_,
                    masterchef_
                );
    ...
    {
        (bool success, bytes memory data) = masterchef_.staticcall(
            abi.encodeWithSelector(sel_, pid_, account_)
        );
        if (success) {
    ...
    function _poolInfo(address masterchef_, uint256 pid_)
        public
        view
        returns (PoolData memory)
    {
        return
            _poolInfo(masterchef_, PoolInfoSel, pid_, 0, 32, 64, 96, 128, 160);
    }
    ...
    function _poolInfo(
        ...
        (bool success, bytes memory data) = masterchef_.staticcall(
            abi.encodeWithSelector(sel_, pid_)
        );
        p.pid = pid_;
        p.data = data;
        if (success) {
    ...
    function _decimals(address token_) public view returns (uint8) {
        (bool success, bytes memory data) = token_.staticcall(
            abi.encodeWithSelector(DecimalsSel)
        );
        if (success && data.length > 0) return abi.decode(data, (uint8));
        return 0;
    }

    function _pendingSushi(
        address masterchef_,
        bytes4 sel_,
        uint256 pid_,
        address account_
    ...
    function _isErc721(address _token) public view returns (bool) {
        (bool success, bytes memory data) = _token.staticcall(
            abi.encodeWithSelector(SupportsInterfaceSel, bytes4(0x80ac58cd))
        );
        if (success) {
