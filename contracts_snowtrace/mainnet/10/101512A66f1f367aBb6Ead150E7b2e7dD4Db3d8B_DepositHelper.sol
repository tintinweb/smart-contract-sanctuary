...
contract DepositHelper is Ownable {
    ...
    constructor (address _deposit, ISwapV2Router _router, address _principle, address _ICE, address _MIN, ITreasury _treasury) {
    ...
    function depositHelper(
        ...
        if (_tokenAddress == principle) {
            ...
            (,, uint256 moreLPAmount) = router.addLiquidity(MIN, ICE, MIN.myBalance(), ICE.myBalance(), 0, 0, address(this), block.timestamp);
            principle.safeApprove(address(deposit), moreLPAmount);
            if (MIN.myBalance() > 0) {
                MIN.safeTransfer(msg.sender, MIN.myBalance());
            }
            if (ICE.myBalance() > 0) {
                ICE.safeTransfer(msg.sender, ICE.myBalance());
            }
    ...
    function depositValue(uint256 _amount) public view returns (uint256 value_) {
        ISwapV2Pair lpToken = ISwapV2Pair(factory.getPair(MIN, ICE));
        ...
        if (swapAmt > 0) {
            ...
            (path[0], path[1]) = isReversed ? (ICE, MIN) : (MIN, ICE);
