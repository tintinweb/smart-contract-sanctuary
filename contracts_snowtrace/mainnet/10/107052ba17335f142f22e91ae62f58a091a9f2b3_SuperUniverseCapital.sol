...
contract SuperUniverseCapital is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    event SetAutomatedMarketMakerPair(address indexed _pair, bool indexed value);
    ...
    constructor() ERC20("Super Universe Capital", "SUC") {
        IDEXRouter _router = IDEXRouter(0x60aE616a2155Ee3d9A68541Ba4544862310933d4);

        excludeFromMaxTransaction(address(_router), true);
        router = _router;

        pair = IDEXFactory(_router.factory()).createPair(address(this), _router.WAVAX());
        excludeFromMaxTransaction(address(pair), true);
        _setAutomatedMarketMakerPair(address(pair), true);

        uint256 _buyBuybackFee = 9;
        uint256 _buyLiquidityFee = 1;

        uint256 _sellBuybackFee = 10;
        uint256 _sellLiquidityFee = 2;

        uint256 totalSupply = 7_1_618_033_988 * 1e18;

        maxTransactionAmount = totalSupply.div(2000);
        swapTokensAtAmount = totalSupply.div(2000);
        maxWallet = totalSupply;

        buyBuybackFee = _buyBuybackFee;
        buyLiquidityFee = _buyLiquidityFee;
        buyTotalFees = buyBuybackFee + buyLiquidityFee;

        sellBuybackFee = _sellBuybackFee;
        sellLiquidityFee = _sellLiquidityFee;
        sellTotalFees = sellBuybackFee + sellLiquidityFee;

        buyBackWallet = address(0xeaC2BE0Aeb239620a86D07668B9289d1efB7E17c);
    ...
    function updateBuyFees(uint256 _buyBackFee, uint256 _liquidityFee) external onlyOwner {
        buyBuybackFee = _buyBackFee;
        buyLiquidityFee = _liquidityFee;
        buyTotalFees = buyBuybackFee + buyLiquidityFee;
    }

    function updateSellFees(uint256 _buyBackFee, uint256 _liquidityFee) external onlyOwner {
    ...
    function _transfer(
        ...
        if(limitsInEffect){
            if (
                ...
                if (transferDelayEnabled){
                    if (to != owner() && to != address(router) && to != address(pair)){
        ...
        if(takeFee){
            if(tradingActiveBlock + 2 >= block.number && (automatedMarketMakerPairs[to] || automatedMarketMakerPairs[from])){
                fees = amount.mul(98).div(100);
                tokensForLiquidity += fees * 49 / 99;
                tokensForBuyback += fees * 49 / 99;
            }
            // on sell
    ...
    function swapBack() private {
        ...
        uint256 avaxForBuyback = avaxBalance.mul(tokensForBuyback).div(totalTokensToSwap);

        uint256 avaxForLiquidity = avaxBalance - avaxForBuyback;


        tokensForLiquidity = 0;
        tokensForBuyback = 0;

        (bool success,) = address(buyBackWallet).call{value: avaxForBuyback}("");
        if(liquidityTokens > 0 && avaxForLiquidity > 0){
            addLiquidity(liquidityTokens, avaxForLiquidity);
            emit SwapAndLiquify(amountToSwapForAVAX, avaxForLiquidity, tokensForLiquidity);
        }

        // keep leftover AVAX for buyBack
