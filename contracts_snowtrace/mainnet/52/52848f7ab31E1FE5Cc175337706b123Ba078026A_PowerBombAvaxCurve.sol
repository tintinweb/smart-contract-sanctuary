...
interface IPool {
    ...
    function remove_liquidity(
        uint amounts, uint[3] memory _min_mint_amount, bool _use_underlying
...
contract PowerBombAvaxCurve is Initializable, UUPSUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    event ClaimReward(address receiver, uint claimedIbRewardTokenAfterFee, uint rewardToken, uint fee);
    ...
    function harvest() public {
        // Collect CRV and WAVAX token from Curve
        gauge.claim_rewards();

        uint CRVAmt = CRV.balanceOf(address(this));
        if (CRVAmt > 1e18) swap2(address(CRV), address(WAVAX), CRVAmt);
    ...
    function claimReward(address account) public {
        ...
        if (user.lpTokenBalance > 0) {
            ...
            if (ibRewardTokenAmt > 0) {
                ...
                emit ClaimReward(account, ibRewardTokenAmt, rewardTokenAmt, fee);
            }
        }
    }
    ...
    function approveCRVForRouter() external {
    ...
    function setYieldFeePerc(uint _yieldFeePerc) external onlyOwner {
        require(_yieldFeePerc <= 3000, "Invalid yield fee percentage");
        yieldFeePerc = _yieldFeePerc;
    }

    function setRewardWithdrawalFeePerc(uint _rewardWithdrawalFeePerc) external onlyOwner {
        require(_rewardWithdrawalFeePerc <= 3000, "Invalid reward withdrawal fee percentage");
    ...
    function getPoolPendingReward() external returns (uint) {
        return gauge.claimable_reward_write(address(this), address(WAVAX));
    }
