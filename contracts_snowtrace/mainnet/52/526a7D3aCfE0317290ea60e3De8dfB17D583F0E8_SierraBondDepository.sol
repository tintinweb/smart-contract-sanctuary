...
contract SierraBondDepository is Ownable {
    ...
    function deposit( 
        ...
        /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) sra
         */
        ...
        // From here, we use the "fake" _amount, value, payout without fees to not take fees on users benefices (even if the sra minted are not 100% backed) :

        // "fake" amount of sra that will be minted (even if there aren't 100% backed)
        uint value = ITreasury( treasury ).valueOf( principle, _amount );
        uint payout = payoutFor( value ); // payout to bonder is computed

        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 sra ( underflow protection )
