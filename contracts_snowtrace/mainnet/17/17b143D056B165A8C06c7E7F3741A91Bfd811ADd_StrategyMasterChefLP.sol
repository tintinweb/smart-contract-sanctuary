...
abstract contract BaseStrategy is Ownable, ReentrancyGuard, Pausable {
    ...
    address public wNativeAdress = 0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7;

    address public rewardAddress = 0xA94aa23F43E9f00CE154F3C1c7f971ECb5318bE8;
    address public withdrawFeeAddress = 0xA94aa23F43E9f00CE154F3C1c7f971ECb5318bE8;
    ...
    uint256 public buyBackRate = 100; // 1%
    ...
    address[] public earnedToBanksyPath;
    ...
    function distributeRewards(uint256 _earnedAmt) internal returns (uint256) {
        if (rewardRate > 0) {
            ...
            if (fee > 0) {
                ...
                if (earnedAddress == banksyAddress) {
                    // Earn token is BANKSY - don't sell
    ...
    function _safeSwap(
        ...
        uint256[] memory amounts = IRouter(uniRouterAddress).getAmountsOut(_amountIn, _path);
        uint256 amountOut = amounts[amounts.length.sub(1)];

        IRouter(uniRouterAddress).swapExactTokensForTokens(
            _amountIn,
    ...
    function _safeSwapWnative(
        ...
        IRouter(uniRouterAddress).swapExactTokensForAVAX(
            _amountIn,
            amountOut.mul(slippageFactor).div(1000),
            _path,
            _to,
        ...
        if (earnedAmt > 0) {
            ...
            if (earnedAmt > 0) {
                ...
                if (token0Amt > 0 && token1Amt > 0) {
                    IRouter(uniRouterAddress).addLiquidity(
                        token0Address,
                        token1Address,
                        token0Amt,
                        token1Amt,
...
// File contracts/StrategyMasterchefLP.sol
...
contract StrategyMasterChefLP is BaseStrategyLPSingle {
    ...
    /**
        ...
        _configAddress[6]  _banksyAddress,
    */
    constructor(
        ...
        address[] memory _earnedToBanksyPath,
        ...
        banksyAddress = _configAddresses[6];

        earnedToWnativePath = _earnedToWnativePath;
        earnedToUsdPath = _earnedToUsdPath;
        earnedToBanksyPath = _earnedToBanksyPath;
        earnedToToken0Path = _earnedToToken0Path;
        earnedToToken1Path = _earnedToToken1Path;
        token0ToEarnedPath = _token0ToEarnedPath;
        token1ToEarnedPath = _token1ToEarnedPath;

        token0Address = token0ToEarnedPath[0];
        token1Address = token1ToEarnedPath[0];
