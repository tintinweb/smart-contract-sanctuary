...
contract Treasury is Ownable {
    ...
    address public immutable PULSE;
    ...
    constructor(
        ...
        require(_PULSE != address(0));
        PULSE = _PULSE;

        isReserveToken[_DAI] = true;
        reserveTokens.push(_DAI);

        isLiquidityToken[_PULSEDAI] = true;
        liquidityTokens.push(_PULSEDAI);
        bondCalculator[_PULSEDAI] = _bondCalculator;
    ...
    /**
        @notice allow approved address to deposit an asset for PULSE
        @param _amount uint
    ...
    function deposit(
        ...
        // mint PULSE needed and store amount of rewards for distribution
        send_ = value.sub(_profit);
        IERC20Mintable(PULSE).mint(msg.sender, send_);
    ...
    /**
        @notice allow approved address to burn PULSE for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw(uint256 _amount, address _token) external {
        ...
        uint256 value = valueOf(_token, _amount);
        IPULSEERC20(PULSE).burnFrom(msg.sender, value);
    ...
    /**
        @notice allow approved address to repay borrowed reserves with PULSE
        @param _amount uint
     */
    function repayDebtWithPULSE(uint256 _amount) external {
        require(isDebtor[msg.sender], "Not approved");

        IPULSEERC20(PULSE).burnFrom(msg.sender, _amount);

        debtorBalance[msg.sender] = debtorBalance[msg.sender].sub(_amount);
        totalDebt = totalDebt.sub(_amount);

        emit RepayDebt(msg.sender, PULSE, _amount, _amount);
    }
    ...
    function mintRewards(address _recipient, uint256 _amount) external {
        ...
        IERC20Mintable(PULSE).mint(_recipient, _amount);
    ...
    function excessReserves() public view returns (uint256) {
        return totalReserves.sub(IERC20(PULSE).totalSupply().sub(totalDebt));
    }
    ...
    /**
        @notice returns PULSE valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf(address _token, uint256 _amount) public view returns (uint256 value_) {
        if (isReserveToken[_token]) {
            // convert amount to match PULSE decimals
            value_ = _amount.mul(10**IERC20(PULSE).decimals()).div(10**IERC20(_token).decimals());
