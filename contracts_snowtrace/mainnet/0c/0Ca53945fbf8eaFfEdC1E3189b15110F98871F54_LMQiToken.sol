...
contract LMQiToken is ERC20,proxyOwner {
    ...
    constructor(address multiSignature,address origin0,address origin1,address _qiToken,address payable _FeePool)
        ...
        FeePool = _FeePool;
        _addReward(0,0x8729438EB15e2C8B576fCc6AeCdA6A148776C0F5);
        _addReward(1,address(0));
    ...
    function harvestAndSwap(uint256 i,uint256 _slipRate) public {
        slipRate = _slipRate;
        address[] memory qiTokens = new address[](1); 
        qiTokens[0] = address(qiToken);
        compounder.claimReward(rewardType[i],address(this),qiTokens);
        swapTraderJoe(rewardTokens[i]);
    }
    ...
    function swapTraderJoe(address token)internal{
        if(token == underlying){
            return;
        }
        if (token != address(0)){
    ...
    function addReward(uint8 _reward,address _rewardToken) external isAuthorized {
        _addReward(_reward,_rewardToken);
    }
    function _addReward(uint8 _reward,address _rewardToken) internal{
