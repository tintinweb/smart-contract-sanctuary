...
contract MagnetERC20Token is ERC20Permit, VaultOwned {

    using SafeMath for uint256;

    // Transfer tax rate in basis points. (default 0%)
    // Tax is set to prevent the sniping bots. Tax will be set to 0% after announcing the exact launch time to the public.
    ...
    constructor() ERC20("MagnetDAO", "MAG", 9) {
        tax_operator = msg.sender;
        _excludedFromTax[tax_operator] = true;
    }
    ...
    function setDaoAddress(address _dao_address) public {
        require(msg.sender == tax_operator, "Magnet::updateTaxOperator: Only Tax Operator can set tax collector.");
        dao_address = _dao_address;
    }

    function setExcludedFromTax(address _account, bool _excluded) public {
        require(msg.sender == tax_operator, "Magnet::updateTaxOperator: Only Tax Operator can set tax free status.");
    ...
    function updateTaxOperator(address _taxOperator) public {
      require(msg.sender == tax_operator, "Magnet::updateTaxOperator: Only Tax Operator can update next tax operator.");
      tax_operator = _taxOperator;
    }

    function updateTransferTaxRate(uint16 _transferTaxRate) public {
        require(_transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "Magnet::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
        require(msg.sender == tax_operator, "Magnet::updateTransferTaxRate: Only Tax Operator can update Tax rate.");
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
      if (transferTaxRate == 0 || dao_address == address(0) || isExcludedFromTax(sender)) {
        super._transfer(sender, recipient, amount);
      } else {
        uint256 taxAmount = amount.mul(transferTaxRate).div(10000);
        uint256 sendAmount = amount.sub(taxAmount);
        require(amount == sendAmount + taxAmount, "Magnet::transfer: Tax value invalid");

        super._transfer(sender, dao_address, taxAmount);
