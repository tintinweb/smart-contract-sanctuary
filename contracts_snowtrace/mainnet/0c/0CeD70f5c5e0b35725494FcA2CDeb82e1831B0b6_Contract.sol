...
contract Contract is IERC20, Ownable {
    ...
    address public immutable liquidPair;
    ...
    constructor(
        ...
        // Create a joe liquidPair for this new token
        router = IJoeRouter02(routerAddress);
        liquidPair = IJoeFactory(router.factory()).createPair(address(this), router.WAVAX());
    ...
    function _transfer(
        ...
        if (!inSwapAndLiquify && from != liquidPair && from != address(router) && !_isExcludedFromFee[from]) {
        ...
        // also, don't swap & liquify if sender is joe liquidPair.
        ...
        if (overMinTokenBalance && !inSwapAndLiquify && from != liquidPair && swapAndLiquifyEnabled) swapAndLiquify(contractTokenBalance);
    ...
    function swapTokensForEth(uint256 tokenAmount, address to) private {
        // generate the joe liquidPair path of token -> weth
