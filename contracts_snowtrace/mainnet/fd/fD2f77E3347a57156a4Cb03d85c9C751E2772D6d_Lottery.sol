...
contract Lottery is Ownable {
    ...
    function draw() private onlyOwner returns(address){
        ...
        require(LOTTO_LIVE);
        uint randomNum = uint(keccak256(abi.encode(block.difficulty, block.timestamp, TICKETBAG)));
        uint index = randomNum % TICKETBAG.length;
        WINNER = TICKETBAG[index];
    ...
    function withdrawTokens(address to) external onlyOwner {
        ...
        wStackAddress.transfer(to, tokenSupply);
    }
}
