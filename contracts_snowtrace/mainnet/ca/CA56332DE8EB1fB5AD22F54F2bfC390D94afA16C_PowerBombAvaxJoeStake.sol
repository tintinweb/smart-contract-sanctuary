...
interface IJoeBar is IERC20Upgradeable{
    function enter (uint amount) external;
    function leave (uint share) external;
}
...
contract PowerBombAvaxJoeStake is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    using SafeERC20Upgradeable for IERC20Upgradeable;
    using SafeERC20Upgradeable for IPair;
    using SafeERC20Upgradeable for IWAVAX;
    using SafeERC20Upgradeable for IJoeBar;
    ...
    IMasterChef constant masterChefV2 = IMasterChef(0xd6a4F121CA35509aF06A0Be99093d08462f53052);
    ...
    event Harvest(uint harvestedJOE, uint stakedXJOEAfterFee, uint fee);
    event ClaimReward(address receiver, uint claimedXJOE);
    ...
    function initialize(IMasterChef _masterChef, uint _poolId, address _treasury, address _bot) external initializer {
        __Ownable_init();
        ...
        JOE.safeApprove(address(xJOE), type(uint).max);
        xJOE.safeApprove(address(masterChefV2), type(uint).max);
    ...
    function harvest() public {
        ...
        // Collect JOE from xJOE farm
        masterChefV2.withdraw(24, 0);

        // Swap WAVAX to JOE (if any)
        uint WAVAXAmt = WAVAX.balanceOf(address(this));
        if (WAVAXAmt > 1e16) swap2(address(WAVAX), address(JOE), WAVAXAmt, 0);

        // Special for gOHM
        if (address(lpToken) == gOHMWAVAX) { 
            uint gOHMAmt = token0.balanceOf(address(this)); // token0 == gOHM
            if (gOHMAmt > 1e15) { // 0.001 gOHM
                swap3(address(token0), address(JOE), gOHMAmt, 0);
            }
        }

        uint JOEAmt = JOE.balanceOf(address(this));
        uint _WAVAXAmt = router.getAmountsOut(JOEAmt, getPath(address(JOE), address(WAVAX)))[1];
        if (_WAVAXAmt > 25e16) { // 0.25 WAVAX
            // Stake JOE to xJOE
            xJOE.enter(JOEAmt);

            // Collect fee
            uint xJOEAmt = xJOE.balanceOf(address(this));
            uint fee = xJOEAmt * yieldFeePerc / 10000;
            xJOE.safeTransfer(treasury, fee);
            xJOEAmt -= fee;

            // Stake xJOE to farm
            masterChefV2.deposit(24, xJOEAmt);

            // Update accRewardPerlpToken
            (uint currentPool,) = masterChef.userInfo(poolId, address(this));
            accRewardPerlpToken += (xJOEAmt * 1e36 / currentPool);

            emit Harvest(JOEAmt, xJOEAmt, fee);
        }
    }
    ...
    function claimReward(address account) public {
        ...
        if (user.lpTokenBalance > 0) {
            ...
            uint xJOEAmt = (user.lpTokenBalance * accRewardPerlpToken / 1e36) - user.rewardStartAt;
            if (xJOEAmt > 0) {
                user.rewardStartAt += xJOEAmt;

                // Unstake xJOE from xJOE farm
                masterChefV2.withdraw(24, xJOEAmt);
                
                // Transfer rewardToken to user
                xJOE.safeTransfer(account, xJOEAmt);
                userAccReward[account] += xJOEAmt;

                emit ClaimReward(account, xJOEAmt);
            }
        }
    }
    ...
    function swap3(address tokenIn, address tokenOut, uint amount, uint amountOutMin) private returns (uint) {
    ...
    /// @return xJOEAmt User pending reward (xJOE)
    function getUserPendingReward(address account) external view returns (uint xJOEAmt) {
        User storage user = userInfo[account];
        xJOEAmt = (user.lpTokenBalance * accRewardPerlpToken / 1e36) - user.rewardStartAt;
    }
