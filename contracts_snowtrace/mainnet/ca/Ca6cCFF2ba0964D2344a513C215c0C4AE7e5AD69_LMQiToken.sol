...
// File: contracts/LMBenqi/ERC20.sol
...
// File: contracts/LMBenqi/IBenqiCompound.sol
...
// File: contracts/LMBenqi/LMQiToken.sol
...
contract LMQiToken is ERC20,proxyOwner {
    ...
    address constant public CEther = 0x5C0401e81Bc07Ca70fAD469b451682c0d747Ef1c;
    ...
    constructor(address multiSignature,address origin0,address origin1,address _qiToken,address payable _FeePool)
        ...
        FeePool = _FeePool;
        rewardType = new uint8[](2);
        rewardType[0] = 0;
        rewardType[1] = 1;
        rewardTokens = new address[](2);
        rewardTokens[0] = 0x8729438EB15e2C8B576fCc6AeCdA6A148776C0F5;
        rewardTokens[1] = address(0);
        SafeERC20.safeApprove(IERC20(rewardTokens[0]), traderJoe, uint(-1));
        address joe = 0x8729438EB15e2C8B576fCc6AeCdA6A148776C0F5;
        swapRoutingPath[joe] = new address[](3);
        swapRoutingPath[joe][0] = rewardTokens[0];
        swapRoutingPath[joe][1] = WAVAX;
        swapRoutingPath[joe][2] = underlying;
    ...
    function harvest() public {
        //uint256 oldAvaxBalance = address(this).balance;
        uint nLen = rewardType.length;
        address[] memory qiTokens = new address[](1); 
        qiTokens[0] = address(qiToken);
        for (uint i=0;i<nLen;i++){
            compounder.claimReward(rewardType[i],address(this),qiTokens);
        }
    }
    function harvestAndSwap() public {
        uint256 oldAvaxBalance = address(this).balance;
        uint nLen = rewardType.length;
        address[] memory qiTokens = new address[](1); 
        qiTokens[0] = address(qiToken);
        ...
        nLen = rewardTokens.length;
        for (uint i=0;i<nLen;i++){
            swapTraderJoe(rewardTokens[i],oldAvaxBalance);
        }
    }
    function compound() public{
        ...
        if (underlying == address(0)){
            ...
            balance = balance.sub(oldAvaxBalance);
    ...
    function swapTraderJoe(address token,uint256 oldAvaxBalance)internal{
        if(token == underlying){
            return;
        }
        if (token != address(0)){
            uint256 balance = IERC20(token).balanceOf(address(this));
            SafeERC20.safeApprove(IERC20(token), traderJoe, balance);
            address[] memory path = getSwapRouterPath(token);
            uint[] memory amountOut = IJoeRouter01(traderJoe).getAmountsOut(balance, path);
            uint256 minOut = amountOut[amountOut.length-1]*98/100;
    ...
    function approveToken(address token,address contractAddr)external isAuthorized{
        SafeERC20.safeApprove(IERC20(token), contractAddr, uint(-1));
    }
    function setSwapRoutingPath(address token,address[] calldata swapPath) external isAuthorized {
        swapRoutingPath[token] = swapPath;
    }
    function addReward(uint8 _reward,address _rewardToken) external isAuthorized {
        rewardType.push(_reward);
