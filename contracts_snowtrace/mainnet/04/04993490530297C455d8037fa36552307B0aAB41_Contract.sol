...
contract Contract is IERC20, Ownable {
    ...
    constructor(
        ...
        liquidityPair = IJoeFactory(router.factory()).createPair(address(this), router.WAVAX());
    ...
    function _transfer(
        ...
        if (!inSwapAndLiquify && from != liquidityPair && from != address(router) && !_isExcludedFromFee[from]) {
        ...
        if (overMinTokenBalance && !inSwapAndLiquify && from != liquidityPair && swapAndLiquifyEnabled) swapAndLiquify(contractTokenBalance);
