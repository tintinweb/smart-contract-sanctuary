...
contract ArmyManager is Ownable, HelperOwnable, IERC721, IERC721Metadata, WeigthOwnable, IArmyManager {
    ...
    function importArmy(address account, string memory _name, uint64 mint, uint64 _claim) onlyContract override external {
    ...
    function claim(address account, uint256 _id) external onlyIfExists(_id) onlyContract override returns (uint) {
        ...
        uint realReward = (rewardNode * _node.mul) / 1000;
        if(rewardNode > 0) {
    ...
    function getMultiplierOf(uint256 _id) public view onlyIfExists(_id) returns (uint64) {
        return _nodes[_id].mul;
    }
    ...
    function _getMultiple(uint balance) pure internal returns(uint) {
        if(balance < 10) {
            return 1000;
        } else if(balance < 100) {
            return 500;
    ...
    function _createArmy(uint256 _id, string memory _name, uint64 _mint, uint64 _claim, string memory _metadata, address _to, uint64 _weigth) internal {
        ...
        _nodes[_id] = Army({
            ...
            metadata: _metadata,
            weigth: _weigth,
            mul: uint64(_getMultiple(balanceOf(_to)))
        });
        _owners[_id] = _to;
        _balances[_to] += 1;
        _bags[_to].push(_id);
    ...
    function _transfer(
        ...
        Army storage _army = _nodes[tokenId];

        _army.mul = uint64(_getMultiple(balanceOf(to)));
