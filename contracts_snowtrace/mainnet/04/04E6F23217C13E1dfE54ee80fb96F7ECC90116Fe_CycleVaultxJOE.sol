...
contract CycleVaultxJOE is ERC20, ReentrancyGuard, Ownable, Pausable {
    ...
    constructor() public ERC20("CycleVaultShares(TraderJoe-xJOE)", "cyVLT") {
        IERC20(WAVAX).safeApprove(Router, 0);
        IERC20(WAVAX).safeApprove(Router, uint256(-1));
        IERC20(JOE).safeApprove(Router, 0);
        IERC20(JOE).safeApprove(Router, uint256(-1));
        IERC20(JOE).safeApprove(xJOE, 0);
        IERC20(JOE).safeApprove(xJOE, uint256(-1));
        IERC20(xJOE).safeApprove(MasterChef, 0);
        IERC20(xJOE).safeApprove(MasterChef, uint256(-1));
    }
