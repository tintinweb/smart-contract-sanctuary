...
contract AVACO is ERC20('AVACO Token', 'AVACO') {
...
// MasterChef is the master of Avaco Pools. He can make avaco and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once avaco is sufficiently
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 accAvacoPerShare;   // Accumulated Yieldes per share, times 1e18. See below.
        uint16 withdrawFeeBP;      // withdraw fee in basis points
        uint256 lpSupply;
        
    }

    AVACO public immutable Avaco;
    address public devAddress;
	address public treasuryAddress;
    address public feeAddress;
    

    // Avaco tokens created per block, can update with setNewAvacoPerBlock
    uint256 public AvacoPerBlock = 0.00025 ether;
    ...
    // The block number when Avaco mining starts.
    ...
    event UpdateEmissionRate(address indexed user, uint256 AvacoPerBlock);
    ...
    // Update the given pool's Avaco allocation point and withdraw fee. Can only be called by the owner.
    ...
    // View function to see pending Avacoes on frontend.
    function pendingAvaco(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accAvacoPerShare = pool.accAvacoPerShare;
        uint256 lpSupply = pool.lpSupply;
        if (block.number > pool.lastRewardBlock && lpSupply != 0 && totalAllocPoint>0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 AvacoReward = multiplier.mul(AvacoPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accAvacoPerShare = accAvacoPerShare.add(AvacoReward.mul(1e18).div(lpSupply));
        }
        return user.amount.mul(accAvacoPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        Avaco.mint(BURN_ADDRESS, AvacoReward.mul(20).div(1000));
        pool.accAvacoPerShare = pool.accAvacoPerShare.add(AvacoReward.mul(1e18).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for Avaco allocation.
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accAvacoPerShare).div(1e18).sub(user.rewardDebt);
            if (pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accAvacoPerShare).div(1e18);
    ...
    // Safe Avaco transfer function, just in case if rounding error causes pool to not have enough Avaco.
    function safeAvacoTransfer(address _to, uint256 _amount) internal {
        uint256 AvacoBal = Avaco.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > AvacoBal) {
            transferSuccess = Avaco.transfer(_to, AvacoBal);
        } else {
            transferSuccess = Avaco.transfer(_to, _amount);
        }
        require(transferSuccess, "safeAvacoTransfer: Transfer failed");
    }
    ...
    // Dev can change every time AvacoPerBlock
    function setNewAvacoPerBlock(uint256 _AvacoPerBlock) external onlyOwner {
        require(msg.sender == devAddress, "MasterChef: only Dev!");
        AvacoPerBlock = _AvacoPerBlock;
    }

    function setTreasuryAddress(address _treasuryAddress) external onlyOwner {
        require(_treasuryAddress != address(0),"non-zero");
        treasuryAddress = _treasuryAddress;
