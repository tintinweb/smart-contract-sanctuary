...
contract TimeTreasury is Ownable {
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( Time ).totalSupply().sub( totalDebt ) ).mul( cvmultiplier );
    }
	
	/**
        @notice returns CV multiplier rate
        @return uint
     */
    function cvMultiplierRate() public view returns ( uint ) {
        return cvmultiplier;
    }

    /**
        @notice sets CV multiplier rate
     */
    function setcvMultiplierRate(uint _multiplier) external onlyManager() {
		cvmultiplier = _multiplier;
	}
