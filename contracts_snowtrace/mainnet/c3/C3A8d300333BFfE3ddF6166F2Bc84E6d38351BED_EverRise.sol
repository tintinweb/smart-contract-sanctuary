...
contract EverRise is Context, IERC20, Ownable {
    ...
    constructor(address _stakingAddress, address routerAddress) {
        ...
        authorizedSwapTokenAdd(uniswapV2Router.WAVAX());
    ...
    // Receives AVAX from uniswapV2Router when swapping
    ...
    function swapTokens(
        ...
        if (toToken == uniswapV2Router.WAVAX()) {
            swapTokensForAvax(fromToken, address(this), actualAmount);
        } else if (fromToken == uniswapV2Router.WAVAX()) {
    ...
    function authorizedSwapTokenRemove(address tokenAddress) public onlyOwner {
        ...
        require(tokenAddress != uniswapV2Router.WAVAX(), "cannot remove the WAVAX type contract from authorized tokens");
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        swapTokensForAvax(address(this), address(this), contractTokenBalance);
    ...
    function swapAVAXForTokensNoFee(
        ...
        // make the swap
        uint256[] memory amounts = uniswapV2Router.swapExactAVAXForTokens{
            value: amount
        }(
