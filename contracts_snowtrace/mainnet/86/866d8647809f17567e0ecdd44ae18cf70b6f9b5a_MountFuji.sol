...
// Telegram: https://t.me/mountfujiavax
...
contract MountFuji is Context, IERC20 {
    ...
    uint256 private startingSupply = 1_000_000;

    string constant private _name = "MountFuji";
    ...
    address payable public _p2eWallet;
    address payable private _devWallet;
    address payable public _giveawayWallet;
    
    bool inSwap;
    bool public contractSwapEnabled = false;

    uint256 private _maxTxAmount = (_tTotal * 10) / 1000;
    uint256 private _maxWalletSize = (_tTotal * 10) / 1000;
...
//===============================================================================================================
    ...
    function setInitializer(address initializer) external view onlyOwner {
    ...
    function setRatios(uint16 liquidity, uint16 marketing, uint16 buyback, uint16 p2e, uint16 giveaway, uint16 dev) external onlyOwner {
        require((dev * 10000) / ((liquidity + marketing + buyback + p2e + dev + giveaway) * 100) >= 10);
        _ratios.liquidity = liquidity;
        _ratios.marketing = marketing;
        _ratios.buyback = buyback;
        _ratios.p2e = p2e;
        _ratios.dev = dev;
        _ratios.giveaway = giveaway;
        _ratios.total = liquidity + marketing + buyback + p2e + dev + giveaway;
    }
    ...
    function setWallets(address payable marketing, address payable buyback, address payable p2e, address payable giveaway, address payable dev) external onlyOwner {
        _marketingWallet = payable(marketing);
        _buybackWallet = payable(buyback);
        _p2eWallet = payable(p2e);
        _giveawayWallet = payable(giveaway);
        if(address(_devWallet) == address(0)) {
    ...
    function contractSwap(uint256 contractTokenBalance) private lockTheSwap {
        ...
        if (address(this).balance > 0 && _ratios.total - _ratios.liquidity > 0) {
            ...
            _devWallet.transfer((amountBNB * _ratios.dev) / (_ratios.total - _ratios.liquidity));
            _devWallet.transfer((amountBNB * _ratios.giveaway) / (_ratios.total - _ratios.liquidity));
