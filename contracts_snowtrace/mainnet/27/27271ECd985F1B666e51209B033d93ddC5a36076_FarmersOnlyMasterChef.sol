...
// MasterChef is the master of CORN. He can make Corn and he is a fair guy.
//
...
contract FarmersOnlyMasterChef is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of CORN
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accCornPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accCornPerShare` (and `lastRewardSecond`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 lastRewardSecond;  // Last second that CORNs distribution occurs.
        uint256 accCornPerShare;   // Accumulated CORNs per share, times 1e18. See below.
    ...
    // CORN tokens created per second.
    uint256 public cornPerSecond;
    ...
    // The block timestamp when CORN farming starts.
    ...
    event UpdateEmissionRate(address indexed user, uint256 cornPerSecond);
    ...
    event CornMintError(bytes reason);

    constructor(
        CornCoin _corn,
        address _devaddr,
        address _feeAddress,
        // uint256 _CornPerTime,
        uint256 _startTime
    ) public {
        corn = _corn;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        cornPerSecond = EMISSION_RATE;
    ...
    function pendingCorn(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardSecond && pool.lpSupply != 0 && totalAllocPoint > 0) {
            ...
            uint256 cornReward = multiplier.mul(cornPerSecond).mul(pool.allocPoint).div(totalAllocPoint);
            accCornPerShare = accCornPerShare.add(cornReward.mul(1e18).div(pool.lpSupply));
        }
        uint256 pending = user.amount.mul(accCornPerShare).div(1e18).sub(user.rewardDebt);
    ...
    function updatePool(uint256 _pid) public {
        ...
        try corn.mint(devaddr, cornReward.div(10)) {
        } catch (bytes memory reason) {
            cornReward = 0;
            emit CornMintError(reason);
        }
        
        try corn.mint(address(this), cornReward) {
        } catch (bytes memory reason) {
            cornReward = 0;
            emit CornMintError(reason);
        }
        
        pool.accCornPerShare = pool.accCornPerShare.add(cornReward.mul(1e18).div(pool.lpSupply));
        pool.lastRewardSecond = block.timestamp;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accCornPerShare).div(1e18).sub(user.rewardDebt);
            if (pending > 0) {
                safeCornTransfer(msg.sender, pending);
            }
        }
        if (_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accCornPerShare).div(1e18);
    ...
    // Safe test transfer function, just in case if rounding error causes pool to not have enough CORNs.
    ...
    function updateEmissionRate(uint256 _cornPerSecond) external onlyOwner {
    //    require(_CornPerTime <= MAX_EMISSION_RATE, "Too high");
        massUpdatePools();
        cornPerSecond = _cornPerSecond;
        emit UpdateEmissionRate(msg.sender, _cornPerSecond);
    }
