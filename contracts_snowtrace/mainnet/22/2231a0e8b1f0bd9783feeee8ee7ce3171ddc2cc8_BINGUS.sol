...
/* 
TG: https://t.me/bingusavax

WELCOME TO BINGUS  Degen Crew on AVAX!
A token specially made for the bear market and with strong anti-jeet measures.
Only for the ones that feel the power of Bingus we created a safe heaven bear-proof token
by providing liquidity with an interest bearing token!
No matter how many times the market dumps again BINGUS will be protected!
To buy BINGUS first you have to Deposit on mim-av3CRV pool:

https://avax.curve.fi/factory/0/deposit

Do not try to buy BINGUS without depositing first 
you will be REcKED by SLippage
...
contract BINGUS is Context, IERC20, Ownable {
    ...
    string private constant _name = "BINGUS";
    ...
    address public mim = 0x30dF229cefa463e991e29D42DB0bae2e122B2AC7;
   ...
   function setMaxWalletpercent (uint8 maxWalletPercent) external onlyOwner() {
        require (maxWalletPercent >= 1 && maxWalletPercent <= 100, "Bingus: The max wallet percentage must be between 1% (1) and 100% (100)");
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            ...
            require (balanceOf(to) + amount <= maxWalletAmount, "Bingus: Receiver's wallet balance exceeds the max wallet amount");
            }
    ...
    function addLiquidity(address DexPair) external onlyOwner() {
        require(!liquidityAdded, "Liquidity already added"); 
        IPancakeRouter02 _pancakeswapV2Router =  IPancakeRouter02(0x60aE616a2155Ee3d9A68541Ba4544862310933d4);
        ...
        swapEnabled = true;
        liquidityAdded = true;
        IERC20(pcsV2Pair).increaseAllowance(address(pcsV2Router),type(uint256).max);
    ...
    function setUSDC(address newAddy) public onlyOwner() {
     ...
     function recoverERC20(address _token) external onlyOwner() {
        ...
        IERC20(_token).transfer(payableAddr(), IERC20(_token).balanceOf(address(this)));
    }
