...
interface IRewardManagement {
    function createNode(address account, string memory nodeName, uint256 _nodeInitialValue) external returns(address, string memory, uint256);
    function isNameAvailable(address account, string memory nodeName) external view returns(bool);
    function _cashoutAllNodesReward(address account) external returns (uint256);
    function _addNodeValue(address account, uint256 _creationTime) external returns (uint256);
    function _getNodeValueOf(address account) external view returns (uint256);
    function createNodeWithTokens(string memory name, uint256 _initValue) external;
    function nodeMinPrice() external view returns (uint256);
...
contract ULTRANodeSale is Ownable {
    ...
    constructor() {
        ...
        MGMT = IRewardManagement(0xf89908aDE15f26113748ab44a88BE15542199502);
...
//        uint256 nodePrice = _initValue;
       // require(nodePrice >= MinPrice,"NODE CREATION: Node Value set below MinPrice");
    ...
    function create(string memory name, uint256  _nodeInitialValue) public {
    MGMT.createNode(msg.sender, name, _nodeInitialValue * 10 ** 18);
    }
    ...
    function setnodeMinPrice(uint _setMinPrice) public {
        require(msg.sender == dev, "!dev");
        MinPrice = _setMinPrice;
    }
