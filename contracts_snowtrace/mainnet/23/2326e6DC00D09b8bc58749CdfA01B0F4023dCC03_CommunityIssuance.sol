...
contract CommunityIssuance is ICommunityIssuance, BaseMath, Initializable {
    ...
    */
    ...
    function issueLQTY() external override returns (uint) {
        ...
        if (upgradeTime == 0) {
            uint origin_issuance_factor = 999998681227695000;
            latestTotalLQTYIssued = LQTYSupplyCap.mul(_getCumulativeIssuanceFraction(deploymentTime, origin_issuance_factor)).div(DECIMAL_PRECISION);
            
            upgradeTime = block.timestamp;
            issuedBeforeUpgrade = latestTotalLQTYIssued;
            newSupplyCap = LQTYSupplyCap.sub(issuedBeforeUpgrade);
        } else {
            latestTotalLQTYIssued = newSupplyCap.mul(_getCumulativeIssuanceFraction(upgradeTime, ISSUANCE_FACTOR)).div(DECIMAL_PRECISION).add(issuedBeforeUpgrade);
        }
    ...
    function _getCumulativeIssuanceFraction(uint startTime, uint issuance_factor) internal view returns (uint) {
