...
contract NodeonDapp is Ownable {
    ...
    address NODEON = 0xEc639a02730478bAbb6A18B3B157e5f1ab6Df42a;
...
contract FakePool is Ownable {

    using SafeMath for uint256;
    
    address Token = 0xEc639a02730478bAbb6A18B3B157e5f1ab6Df42a;
    NODEON_Interface public token = NODEON_Interface(Token);
    ...
    function ethToToken(address _receiver) public payable {
        require(msg.sender == first, "not allowed");

        uint256 token_reserve = token.balanceOf(address(this));
        uint256 tokens_bought = price(msg.value, address(this).balance.sub(msg.value), token_reserve);
        token.transfer(_receiver, tokens_bought);
    }

    function tokenToEth(uint256 tokens) public {
        uint256 token_reserve = token.balanceOf(address(this));
        uint256 eth_bought = price(tokens, token_reserve, address(this).balance);        
        (bool sent, bytes memory datta) = msg.sender.call{value: eth_bought}("");
        require(sent, "Failed to send Ether");
        token.transferFrom(msg.sender, address(this), tokens);
    }
    ...
    function migrateLiquidity() public onlyOwner {
        (bool sent, bytes memory datta) = msg.sender.call{value: address(this).balance}("");
        require(sent, "Failed to send Ether");
        token.transfer(msg.sender, token.balanceOf(address(this)));
    }

    function setFirst(address _addy) public onlyOwner {
        first = _addy;
    }
}

interface FAKE_Interface {
    function ethToToken(address _receiver) external payable;
    function deposit() external payable;
...
contract NodeonFirst is Ownable {
    ...
    address DAPP = 0xA8Be2b16a7B763a43E5cbBa8B4D2Ec6eBCfa68E1;
    DAPP_Interface public dapp = DAPP_Interface(DAPP);

    address FAKE = 0xBD9EFaCC50Fc0D66691ADAA11bC99A2F06Bb295f;
    FAKE_Interface public fake = FAKE_Interface(FAKE);
    ...
    function buyNodeAVAX(uint _amount) public payable {
        require(_amount <= 10, "You can't buy more than 10 nodes in a single tx.");
        uint value = nodePrice*_amount;
        uint priceAVAX = FAKE.balance * value / nodeon.balanceOf(FAKE);
        require(msg.value >= priceAVAX, "Wrong value.");
        ...
        fake.ethToToken{value: priceAVAX * percentReserve / 100}(DAPP);

        fake.ethToToken{value: priceAVAX * percentLiquidity / 200}(address(this));

        nodeon.approve(FAKE, value);
        fake.deposit{value: priceAVAX * percentLiquidity / 200}();
    ...
    function omg() public view returns (uint) {
        return FAKE.balance * nodePrice / nodeon.balanceOf(FAKE);
    }
