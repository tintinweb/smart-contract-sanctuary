...
contract CougarBankInitializable is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    // The address of the cougar bank factory
    address public COUGAR_BANK_FACTORY;
    ...
    // The block number when COUGAR mining ends.
    ...
    constructor() public {
        COUGAR_BANK_FACTORY = msg.sender;
    }
    ...
    function initialize(
        ...
        require(msg.sender == COUGAR_BANK_FACTORY, "Not factory");
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        if (_amount > 0) {
            ...
            uint256 beforeDeposit = stakedToken.balanceOf(address(this));
            stakedToken.safeTransferFrom(msg.sender, address(this), _amount);
            uint256 afterDeposit = stakedToken.balanceOf(address(this));
            _amount = afterDeposit.sub(beforeDeposit);

            if (stakedTokenTransferFee > 0) {
                uint256 transferFee = _amount.mul(stakedTokenTransferFee).div(10000);
                // Burn this fee
                stakedToken.safeTransfer(BURN_ADDRESS, transferFee);
