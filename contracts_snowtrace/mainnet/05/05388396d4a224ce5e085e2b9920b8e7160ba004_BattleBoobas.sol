...
pragma solidity ^0.8.0;
 ...
 // Bogstandard mint contract
contract BattleBoobas is ERC721URIStorage, Ownable{

    event MintBooba (address indexed minter, uint256 startWith, uint256 times);
   
    
    uint256 public totalBoobas;
    uint256 public totalCount = 3003; //bruhTotal
    uint256 public presaleMax = 303; 
    ...
    function mintBoobas(uint256 _times) payable public mintEnabled {
       
        require(_times >0 && _times <= maxBatch, "Incorrect mint count");
        require(totalBoobas + _times <= totalCount, "too much");
        require(msg.value == _times * price, "value error");
        payable(owner()).transfer(msg.value);
        emit MintBooba(_msgSender(), totalBoobas+1, _times);
        for(uint256 i=0; i< _times; i++){
            _mint(_msgSender(), 1 + totalBoobas++);
        }
    } 
    ...
    function whitelistMint(uint _times) payable public canWhitelistMint {
        ...
        require(totalBoobas + _times <= presaleMax, "Mint amount will exceed total presale amount.");
    ...
    function adminMintGiveaways(address _addr) public onlyOwner {
        require(totalBoobas + 1 <= totalCount, "Mint amount will exceed total collection amount.");
        emit MintBooba(_addr, totalBoobas+1, 1);
        _mint(_addr, 1 + totalBoobas++);
    } 
