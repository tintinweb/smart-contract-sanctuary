...
contract TroveManager is LiquityBase, Initializable, ITroveManager {
    ...
    IUSDToken public override lusdToken;

    IToken public override lqtyToken;
    ...
    */
        ...
        returns (LiquidationValues memory singleLiquidation)
    {
        ...
        uint cappedDebtPortion = _entireTroveDebt.mul(MCR).div(DECIMAL_PRECISION);
        uint cappedCollPortion = debtToColl(cappedDebtPortion, _price);
    ...
    */
        ...
        internal returns (SingleRedemptionValues memory singleRedemption)
    {
        ...
        singleRedemption.ETHLot = debtToColl(singleRedemption.LUSDLot, _price);
    ...
    */
        ...
        override
    {
        ...
        contractsCache.lqtyStaking.increaseRedeemingFee(totals.ETHFee);
    ...
    function getCurrentICR(address _borrower, uint _price) public view override returns (uint) {
        ...
        uint ICR = _computeCR(currentETH, currentLUSDDebt, _price);
        return ICR;
    }
    ...
    function getPendingETHReward(address _borrower) public view override returns (uint) {
        ...
        uint pendingETHReward = stake.mul(rewardPerUnitStaked).div(10 ** getCollDecimals());
    ...
    function getPendingLUSDDebtReward(address _borrower) public view override returns (uint) {
        ...
        uint pendingLUSDDebtReward = stake.mul(rewardPerUnitStaked).div(10 ** getCollDecimals());
    ...
    function _redistributeDebtAndColl(IActivePool _activePool, IDefaultPool _defaultPool, uint _debt, uint _coll) internal {
        ...
        */
        uint ETHNumerator = _coll.mul(10 ** getCollDecimals()).add(lastETHError_Redistribution);
        uint LUSDDebtNumerator = _debt.mul(10 ** getCollDecimals()).add(lastLUSDDebtError_Redistribution);
    ...
    */
    function _updateBaseRateFromRedemption(uint _ETHDrawn,  uint _price, uint _totalLUSDSupply) internal returns (uint) {
        ...
        * the fraction of total supply that was redeemed at face value. */
        uint redeemedLUSDFraction = collToDebt(_ETHDrawn, _price).mul(DECIMAL_PRECISION).div(_totalLUSDSupply);
    ...
    function _requireLUSDBalanceCoversRedemption(IUSDToken _lusdToken, address _redeemer, uint _amount) internal view {
    ...
    function _requireAfterBootstrapPeriod() internal view {
        require(block.timestamp >= deploymentTime.add(BOOTSTRAP_PERIOD), "TroveManager: Redemptions are not allowed during bootstrap phase");
    }
...
*/
contract SortedTroves is ISortedTroves, Initializable{
...
// File: contracts/protocol/MultiTroveGetter.sol
