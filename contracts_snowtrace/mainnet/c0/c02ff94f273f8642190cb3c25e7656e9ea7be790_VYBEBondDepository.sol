...
contract VYBEBondDepository is Ownable {
    ...
    IERC20 public immutable VYBE; // token given as payment for bond
    IERC20 public immutable principle; // token used to create bond
    ITreasury public immutable treasury; // mints VYBE when receives principle
    ...
    uint32 public lastDecay; // reference VYBE for debt decay
    ...
    struct Bond {
        uint payout; // VYBE remaining to be paid
        uint pricePaid; // In DAI, for front end viewing
        uint32 lastVYBE; // Last interaction
    ...
    struct Adjust {
        ...
        uint32 lastVYBE; // VYBE when last adjustment made
    }
    ...
    function setAdjustment ( 
        ...
        adjustment = Adjust({
            ...
            target: _target,
            buffer: _buffer,
            lastVYBE: uint32(block.timestamp)
        });
    ...
    function deposit( 
        ...
        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 VYBE ( underflow protection )
        ...
        uint balanceBefore = VYBE.balanceOf(address(this));
        /**
            principle is transferred in
            approved and
            deposited into the treasury, returning (_amount - profit) VYBE
         */
        ...
        if ( fee != 0 ) { // fee is transferred to dao 
            VYBE.safeTransfer( DAO, fee ); 
        }
        require(balanceBefore.add(profit) == VYBE.balanceOf(address(this)), "Not enough VYBE to cover profit");
    ...
    function redeem( address _recipient, bool _stake ) external returns ( uint ) {
        ...
        if ( percentVested >= 10000 ) { // if fully vested
            ...
            bondInfo[ _recipient ] = Bond({
                ...
                vesting: info.vesting.sub32( uint32( block.timestamp ).sub32( info.lastVYBE ) ),
                lastVYBE: uint32(block.timestamp),
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            VYBE.transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                VYBE.approve( address(stakingHelper), _amount );
    ...
    function adjust() internal {
        uint timeCanAdjust = adjustment.lastVYBE.add32( adjustment.buffer );
        if( adjustment.rate != 0 && block.timestamp >= timeCanAdjust ) {
            ...
            adjustment.lastVYBE = uint32(block.timestamp);
    ...
    function maxPayout() public view returns ( uint ) {
        return VYBE.totalSupply().mul( terms.maxPayout ) / 100000 ;
    }
    ...
    /**
     *  @notice calculate current ratio of debt to VYBE supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns ( uint debtRatio_ ) {   
        uint supply = VYBE.totalSupply();
    ...
    function percentVestedFor( address _depositor ) public view returns ( uint percentVested_ ) {
        ...
        uint secondsSinceLast = uint32(block.timestamp).sub32( bond.lastVYBE );
    ...
    /**
     *  @notice calculate amount of available for claim by depositor
     *  @param _depositor address
