...
contract OhmArbitrage is Ownable, Pausable {
    ...
    constructor (
        ...
        require(IStaking(_staking).Time() == _token);
        require(IStakingHelper(_stakingHelper).staking() == _staking);
    ...
    function addDepositor(address depositor, bool isLiquidity) public onlyOwner {
        if (isLiquidity) {
            require(!isLiquidityDepositor[depositor], "existing");
            ...
            require(!isReserveDepositor[depositor], "existing");
    ...
    function deposit(uint256 amount) public onlyOwner {
        ...
        uint256 amountS = IERC20(sToken).balanceOf(address(this)).sub(sTokenBalance);
        require(amountS == amount);
    }

    function depositS(uint256 amount) public onlyOwner {
        IERC20(sToken).safeTransferFrom(msg.sender, address(this), amount);
    }

    function selectBestDepositor(bool allowPending) public view returns (address) {
        address selectDepositor;
        uint256 selectBondPrice = 2 ** 256 - 1;
        for (uint256 i; i < reserveDepositors.length; i++) {
            uint256 bondPrice = IBondDepository(reserveDepositors[i]).bondPriceInUSD();
            (uint256 pendingPayout, , ,) = IBondDepository(reserveDepositors[i]).bondInfo(address(this));
            if ((bondPrice < selectBondPrice) && (allowPending || pendingPayout == 0)) {
                selectBondPrice = bondPrice;
                selectDepositor = reserveDepositors[i];
            }
        }
        for (uint256 i; i < liquidityDepositors.length; i++) {
            uint256 bondPrice = IBondDepository(liquidityDepositors[i]).bondPriceInUSD();
            (uint256 pendingPayout, , ,) = IBondDepository(liquidityDepositors[i]).bondInfo(address(this));
            if ((bondPrice < selectBondPrice) && (allowPending || pendingPayout == 0)) {
                selectBondPrice = bondPrice;
                selectDepositor = liquidityDepositors[i];
            }
        }
        return selectDepositor;
    }

    function reBondingAuto(bool allowPending, uint256 minDiscount) public onlyOwner {
        address depositor = selectBestDepositor(allowPending);
        require(depositor != address(0), "no best depositor");
        reBonding(depositor, allowPending, minDiscount);
    }

    function reBonding(address _depositor, bool _allowPending, uint256 _minDiscount) public onlyOwner {
        bool isLiquidity = isLiquidityDepositor[_depositor];
        require(isLiquidity || isReserveDepositor[_depositor], "invalid depositor");

        uint256 fiveDayRate;

        {
            if (!_allowPending) {
                (uint256 pendingPayout, , ,) = IBondDepository(_depositor).bondInfo(address(this));
            ...
            console.log("bondPrice: %s, marketPrice: %s, bondDiscount: %s", bondPrice, marketPrice, bondDiscount);

            require(_minDiscount == 0 || bondDiscount >= _minDiscount, "discount is too little");
            ...
            console.log("profitRate: %s, fiveDayRate: %s", profitRate, fiveDayRate);
            require(profitRate >= fiveDayRate + 100, "extra profit < 1%");
        }

        uint256 amount = IERC20(sToken).balanceOf(address(this));
        uint256 previousAmount = IERC20(token).balanceOf(address(this));
        IStaking(staking).unstake(amount, true);
        // check amount
        require(IERC20(token).balanceOf(address(this)) == previousAmount + amount);
        amount = amount.add(previousAmount);
        ...
        if (isLiquidity) {
            ...
            uint amountB;
            {
                ...
                if (tokenB == wNativeToken) {
                    ...
                    // retain 0.1 native token for Gas fee
                    amountB = address(this).balance.sub(10 ** 17);
                } else {
                ...
                require(amountB >= amounts[amounts.length - 1]);
            }
            ...
            payout = IBondDepository(depositor).deposit(liquidity, IBondDepository(depositor).bondPrice(), address(this));
        } else {
            address reserveToken = IBondDepository(depositor).principle();
            ...
            payout = IBondDepository(depositor).deposit(amountReserve, IBondDepository(depositor).bondPrice(), address(this));
        }
    ...
    function redeemAndStake(address _depositor) public onlyOwner {
        uint256 payout = IBondDepository(_depositor).redeem(address(this), true);
    ...
    function approveOwner(address _token) public onlyOwner {
        IERC20(_token).approve(owner(), 2 ** 256 - 1);
    }
    ...
    function withdrawAll() public onlyOwner {
        ...
        // retain 0.01 native token for Gas fee
        Address.sendValue(payable(owner()), address(this).balance.sub(10 ** 16));
    }
