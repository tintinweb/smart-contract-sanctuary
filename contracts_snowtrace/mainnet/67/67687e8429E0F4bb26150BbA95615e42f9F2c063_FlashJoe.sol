...
contract FlashJoe {
    ...
    // https://ethereum.stackexchange.com/a/13057
    function uintToBytes(uint256 x)
        internal
        pure
    ...
    // https://ethereum.stackexchange.com/a/13663
    ...
    // https://ethereum.stackexchange.com/a/50528
    ...
    // Always start from a dex that offers a lower price
    // We call it dexA, while another one is dexB
    // tokenB is the token that we get from dexA
    // We'll return tokenA to dexA and tokenB to dexB
    function startSwap(
        address pairAddress,
        ...
        // get tokenB from dexA
        IJoePair(pairAddress).swap(
            amount0,
            amount1,
            // address(this),
            pair2Address,
            concat(addressToBytes(pair2Address), uintToBytes(amountReturn))
        );
    }
    ...
    function joeCall(
        ...
        // require(sender == owner, "Invalid sender");

        address pairAddress = bytesToAddress(data);
        // address takes 20 bytes, so we start from 20
        // amount of tokenB to return to dexA
        uint256 amountReturn = bytesToUint256(data, 20);

        // amount of tokenB from dexA
        address tokenAAddress = amount0 == 0 ? IJoePair(msg.sender).token1.address : IJoePair(pairAddress).token0.address;

        // get tokenA from dexB
        IJoePair(pairAddress).swap(
            0,
            amountReturn,
            msg.sender,
        ...
        IERC20Joe tokenA = IERC20Joe(tokenAAddress);
