...
// File: contracts/farm/Oracle.sol
...
library HakuswapOracleLibrary {
    ...
    function currentCumulativePrices(
        ...
        price0Cumulative = IHakuswapPair(pair).price0CumulativeLast();
        price1Cumulative = IHakuswapPair(pair).price1CumulativeLast();

        // if time has elapsed since the last update on the pair, mock the accumulated price values
        (uint112 reserve0, uint112 reserve1, uint32 blockTimestampLast) = IHakuswapPair(pair).getReserves();
...
contract Oracle {
    ...
    function update(address tokenA, address tokenB) external {
        ...
        address pair = IHakuswapFactory(factory).expectPairFor(tokenA, tokenB);

        Observation storage observation = pairObservations[pair];
        uint timeElapsed = block.timestamp - observation.timestamp;
        require(timeElapsed >= CYCLE, 'MDEXOracle: PERIOD_NOT_ELAPSED');
        (uint price0Cumulative, uint price1Cumulative,) = HakuswapOracleLibrary.currentCumulativePrices(pair);
    ...
    function consult(address tokenIn, uint amountIn, address tokenOut) external view returns (uint amountOut) {
        address pair = IHakuswapFactory(factory).expectPairFor(tokenIn, tokenOut);
        Observation storage observation = pairObservations[pair];
        uint timeElapsed = block.timestamp - observation.timestamp;
        (uint price0Cumulative, uint price1Cumulative,) = HakuswapOracleLibrary.currentCumulativePrices(pair);
        (address token0,) = HakuswapLibrary.sortTokens(tokenIn, tokenOut);
