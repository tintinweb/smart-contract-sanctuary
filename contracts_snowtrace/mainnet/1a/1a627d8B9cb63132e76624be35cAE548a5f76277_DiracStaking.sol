...
contract DiracStaking is Ownable {
    ...
    function stake( uint _amount, address _recipient ) external returns ( bool ) {
        ...
        Dirac.safeTransferFrom( msg.sender, address(this), _amount );
        ...
        warmupInfo[ _recipient ] = Claim ({
            ...
            gons: info.gons.add( DP.gonsForBalance( _amount ) ),
            expiry: epoch.number.add( warmupPeriod ),
            lock: false
        });
        
        DP.safeTransfer( address(warmupContract), _amount );
    ...
    function claim ( address _recipient ) external {
        ...
        if ( epoch.number >= info.expiry && info.expiry != 0 ) {
            ...
            uint256 amount = DP.balanceForGons( info.gons );
    ...
    function forfeit() external {
        ...
        uint memoBalance = DP.balanceForGons( info.gons );
        warmupContract.retrieve( address(this),  memoBalance);
        Dirac.safeTransfer( msg.sender, info.deposit);
    ...
    function unstake( uint _amount, bool _trigger ) external {
        ...
        DP.safeTransferFrom( msg.sender, address(this), _amount );
        Dirac.safeTransfer( msg.sender, _amount );
    ...
    function rebase() public {
        if( epoch.endTime <= uint32(block.timestamp) ) {
            ...
            DP.rebase( epoch.distribute, epoch.number );
            ...
            uint staked = DP.circulatingSupply();
    ...
    function contractBalance() public view returns ( uint ) {
        return Dirac.balanceOf( address(this) ).add( totalBonus );
    }
