...
contract FarmersOnlyMasterChefL3 is Ownable, ReentrancyGuard {
    ...
    // Hybrid Harvest feature status (true: active, false: disabled).
    bool private hybridHarvest;
    ...
    function isHybrid() external view returns (bool) {
        return hybridHarvest;
    }
    ...
    function addPool(uint256 _allocPoint, IERC20 _lpToken, uint16 _depositFeeBP, uint16 _withdrawFeeBP, uint256 _harvestInterval, bool _kingRot, bool _isKing, bool _withUpdate) external onlyOwner nonDuplicated(_lpToken){
    ...
    // Turn on/off the hybrid harvest function.
