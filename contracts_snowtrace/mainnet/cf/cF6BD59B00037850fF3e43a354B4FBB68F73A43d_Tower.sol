...
contract Tower is ITower, Ownable, ReentrancyGuard, IERC721Receiver, Pausable {
  ...
  // reference to the AWnD NFT contract
  IWnD public awndNFT;
  // reference to the WnD NFT contract
  IWnDGame public awndGame;
  // reference to the $aGP contract for minting $aGP earnings
  IGP public agpToken;
  ...
  // amount of $aGP due for each rank point staked
  uint256 private agpPerRank = 0;

  // wizards earn 12000 $aGP per day
  uint256 public constant DAILY_AGP_RATE = 12000 ether;
  // wizards must have 2 days worth of $aGP to unstake or else they're still guarding the tower
  uint256 public constant MINIMUM_TO_EXIT = 2 days;
  // dragons take a 20% tax on all $aGP claimed
  uint256 public constant AGP_CLAIM_TAX_PERCENTAGE = 20;
  // there will only ever be (roughly) 2.4 billion $aGP earned through staking
  uint256 public constant MAXIMUM_GLOBAL_AGP = 2880000000 ether;
  uint256 public treasureChestTypeId;

  // amount of $aGP earned so far
  uint256 public totalAGPEarned;
  // the last time $aGP was claimed
  uint256 private lastClaimTimestamp;

  // emergency rescue to allow unstaking without any checks but without $aGP
  ...
  modifier requireContractsSet() {
      require(address(awndNFT) != address(0) && address(agpToken) != address(0) 
        && address(awndGame) != address(0) && address(randomizer) != address(0), "Contracts not set");
      _;
  }

  function setContracts(address _awndNFT, address _agp, address _awndGame, address _rand) external onlyOwner {
    awndNFT = IWnD(_awndNFT);
    agpToken = IGP(_agp);
    awndGame = IWnDGame(_awndGame);
  ...
  function addManyToTowerAndFlight(address account, uint16[] calldata tokenIds) external override nonReentrant {
    require(tx.origin == _msgSender() || _msgSender() == address(awndGame), "Only EOA");
    require(account == tx.origin, "account to sender mismatch");
    for (uint i = 0; i < tokenIds.length; i++) {
      if (_msgSender() != address(awndGame)) { // dont do this step if its a mint + stake
        require(awndNFT.ownerOf(tokenIds[i]) == _msgSender(), "You don't own this token");
        awndNFT.transferFrom(_msgSender(), address(this), tokenIds[i]);
      } else if (tokenIds[i] == 0) {
        continue; // there may be gaps in the array for stolen tokens
      }

      if (awndNFT.isWizard(tokenIds[i])) 
  ...
  function _addDragonToFlight(address account, uint256 tokenId) internal {
    ...
    flight[rank].push(Stake({
      owner: account,
      tokenId: uint16(tokenId),
      value: uint80(agpPerRank)
    })); // Add the dragon to the Flight
    emit TokenStaked(account, tokenId, false, agpPerRank);
  }

  /** CLAIMING / UNSTAKING */

  /**
   * realize $aGP earnings and optionally unstake tokens from the Tower / Flight
   * to unstake a Wizard it will require it has 2 days worth of $aGP unclaimed
  ...
  function claimManyFromTowerAndFlight(uint16[] calldata tokenIds, bool unstake) external whenNotPaused _updateEarnings nonReentrant {
    ...
    agpToken.updateOriginAccess();
    if (owed == 0) {
      return;
    }
    agpToken.mint(_msgSender(), owed);
  }

  function calculateRewards(uint256 tokenId) external view returns (uint256 owed) {
    uint64 lastTokenWrite = awndNFT.getTokenWriteBlock(tokenId);
    ...
    if(awndNFT.isWizard(tokenId)) {
      if (totalAGPEarned < MAXIMUM_GLOBAL_AGP) {
        owed = (block.timestamp - stake.value) * DAILY_AGP_RATE / 1 days;
      } else if (stake.value > lastClaimTimestamp) {
        owed = 0; // $aGP production stopped already
      } else {
        owed = (lastClaimTimestamp - stake.value) * DAILY_AGP_RATE / 1 days; // stop earning additional $aGP if it's all been earned
      }
    }
    else {
      uint8 rank = _rankForDragon(tokenId);
      owed = (rank) * (agpPerRank - stake.value); // Calculate portion of tokens based on Rank
    }
  }

  /**
   * realize $aGP earnings for a single Wizard and optionally unstake it
   * if not unstaking, pay a 20% tax to the staked Dragons
   * if unstaking, there is a 50% chance all $aGP is stolen
   * @param tokenId the ID of the Wizards to claim earnings from
   * @param unstake whether or not to unstake the Wizards
   * @return owed - the amount of $aGP earned
   */
  function _claimWizardFromTower(uint256 tokenId, bool unstake) internal returns (uint256 owed) {
    ...
    if (unstake) {
      if (randomizer.random() & 1 == 1) { // 50% chance of all $aGP stolen
        _payDragonTax(owed);
        owed = 0;
      }
      ...
      awndNFT.safeTransferFrom(address(this), _msgSender(), tokenId, ""); // send back Wizard
    } else {
      _payDragonTax(owed * AGP_CLAIM_TAX_PERCENTAGE / 100); // percentage tax to staked dragons
      owed = owed * (100 - AGP_CLAIM_TAX_PERCENTAGE) / 100; // remainder goes to Wizard owner
      tower[tokenId] = Stake({
        owner: _msgSender(),
        tokenId: uint16(tokenId),
        value: uint80(block.timestamp)
  ...
  /**
   * realize $aGP earnings for a single Dragon and optionally unstake it
   * Dragons earn $aGP proportional to their rank
  ...
  function _claimDragonFromFlight(uint256 tokenId, bool unstake) internal returns (uint256 owed) {
    require(awndNFT.ownerOf(tokenId) == address(this), "Doesn't own token");
    uint8 rank = _rankForDragon(tokenId);
    ...
    if (unstake) {
      ...
      awndNFT.safeTransferFrom(address(this), _msgSender(), tokenId, ""); // Send back Dragon
    } else {
  ...
  function rescue(uint256[] calldata tokenIds) external nonReentrant {
    ...
    for (uint i = 0; i < tokenIds.length; i++) {
      ...
      if (awndNFT.isWizard(tokenId)) {
        ...
        delete tower[tokenId];
        numWizardsStaked -= 1;
        awndNFT.safeTransferFrom(address(this), _msgSender(), tokenId, ""); // send back Wizards
  ...
  /** 
   * add $aGP to claimable pot for the Flight
   * @param amount $aGP to add to the pot
   */
  function _payDragonTax(uint256 amount) internal {
    if (totalRankStaked == 0) { // if there's no staked dragons
      unaccountedRewards += amount; // keep track of $aGP due to dragons
      return;
    }
    // makes sure to include any unaccounted $aGP 
    agpPerRank += (amount + unaccountedRewards) / totalRankStaked;
    unaccountedRewards = 0;
  }

  /**
   * tracks $aGP earnings to ensure it stops once 2.4 billion is eclipsed
   */
  ...
  function _rankForDragon(uint256 tokenId) internal view returns (uint8) {
    IWnD.WizardDragon memory s = awndNFT.getTokenTraits(tokenId);
