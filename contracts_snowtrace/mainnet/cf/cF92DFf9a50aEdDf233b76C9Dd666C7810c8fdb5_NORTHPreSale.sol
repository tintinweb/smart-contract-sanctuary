...
contract NORTHPreSale is Ownable {
    using SafeMath for uint;
    using SafeERC20 for IERC20;

    address public alphaNORTH;
    ...
    mapping(address => bool) public boughtNORTH;
    ...
    function purchaseaNORTH(uint256 _val) external returns (bool) {
        ...
        require(boughtNORTH[msg.sender] == false, 'Already participated');
        require(startTimestamp < block.timestamp,"Not started yet");
        
        boughtNORTH[msg.sender] = true;
        ...
        IERC20(usdt).safeTransferFrom(msg.sender ,address(this), _val);
        IERC20(usdt).safeTransfer(DAOAddress, _val);
        uint _purchaseAmount = _calculateSaleQuote(_val);
        IERC20(alphaNORTH).safeTransfer(msg.sender, _purchaseAmount);
        return true;
    }
    ...
    function withdraw(address _erc20,address _addr,uint256 _val) external onlyOwner() returns (bool){
        IERC20(_erc20).safeTransfer(_addr, _val);
        return true;
    }
