...
contract test is ERC20Detailed, Ownable, MinterRole {
    ...
    ) ERC20Detailed("SmartCoin", "SMARTCOIN", uint8(DECIMALS)) {
    ...
    function coreRebase(uint256 epoch, int256 supplyDelta) private returns (uint256) {
        ...
        if (supplyDelta < 0) {
            _totalSupply = _totalSupply.sub(uint256(-supplyDelta)); // NOTE check it is logic to substract -suuplyDelta  supplyDelta can be
        } else {
