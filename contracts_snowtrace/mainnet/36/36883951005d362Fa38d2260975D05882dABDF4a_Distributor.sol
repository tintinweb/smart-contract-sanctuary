...
interface IERC20FVOR is IERC20 {

    function fixedMint(address account_) external;
...
contract Distributor is Ownable {
    ...
    IERC20FVOR public immutable FVOR;
    ...
    constructor( address _treasury, address _volt, address _fvor, uint32 _epochLength, uint32 _nextEpochTime ) {  
        ...
        require( _fvor != address(0) );
        FVOR = IERC20FVOR(_fvor);
    ...
    function adjust( uint _index ) internal {
        ...
        if ( adjustment.rate != 0 ) {
            ...
            if ( adjustment.add ) { // if rate should increase
                ...
                int subCheck = int(rate) - int( adjustment.rate ); // prevent underflow revert safeguard
                if(subCheck < 0){ 
                    rate = adjustment.target; 
                }else{
    ...
    function nextRewardAt( uint _rate ) public view returns ( uint ) {
        return VOLT.totalSupply().mul( _rate ).div( 1000000 );
    }
    ...
    function addRecipient( address _recipient, uint _rewardRate ) external onlyOwner {
        ...
        require(_rewardRate <= 50000, "Too high reward rate"); // previously 5000
    ...
    function setAdjustment( uint _index, bool _add, uint _rate, uint _target ) external onlyOwner {
        require(_target <= 50000, "Too high reward rate"); // previously 5000
