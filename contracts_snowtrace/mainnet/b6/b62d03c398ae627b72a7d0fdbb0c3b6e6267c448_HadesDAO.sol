...
contract HadesDAO is BEP20 {
    ...
    // Min amount to liquify. (default 1 HGP)
    ...
    constructor() public BEP20("HadesDAO", "HADES") {
    ...
    /// @dev overrides transfer function to meet tokenomics of HGP
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        ...
        if (transferTaxRate == 0 || sender == owner() || recipient == owner()) {
            ...
            require(taxAmount == feesAmount + liquidityAmount, "HGP::transfer: Fees value invalid");

            // default 95% of transfer sent to recipient
            uint256 sendAmount = amount.sub(taxAmount);
            require(amount == sendAmount + taxAmount, "HGP::transfer: Tax value invalid");
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "HGP::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateFeesRate(uint16 _feesRate) public onlyOperator {
        require(_feesRate <= 100, "HGP::updateFeesRate: Fees rate must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 10000, "HGP::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
        require(_maxTransferAmountRate >= 100, "HGP::updateMaxTransferAmountRate: Max transfer amount rate must exceed the minimum rate.");
    ...
    function updateSnowSwapRouter(address _router) public onlyOwner {
        ...
        require(snowSwapPair != address(0), "HGP::updateSnowSwapRouter: Invalid pair address.");
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "HGP::transferOperator: new operator is the zero address");
    ...
    function transferFeesAddress(address newFeesAddress) public onlyOperator {
        require(newFeesAddress != address(0), "HGP::transferFeesAddress: new fees address is the zero address");
    ...
    */
    ...
    external
    {
        ...
        require(signatory != address(0), "HGP::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "HGP::delegateBySig: invalid nonce");
        require(now <= expiry, "HGP::delegateBySig: signature expired");
    ...
    returns (uint256)
    {
        require(blockNumber < block.number, "HGP::getPriorVotes: not yet determined");
    ...
    internal
    {
        uint32 blockNumber = safe32(block.number, "HGP::_writeCheckpoint: block number exceeds 32 bits");
