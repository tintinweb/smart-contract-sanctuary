...
contract NFTCollection is ERC721URIStorage, Ownable {
    ...
    event MintNFT (address indexed minter, uint256 startWith, uint256 times);
   
    
    uint256 public totalItems;
    uint256 public totalCount = 3333; 
    uint256 public maxBatch = 25; 
    uint256 public price = 0.5 ether; // 0.5 avax
    ...
    function mintNFT(uint256 _times) payable public mintEnabled {
       
        require(_times >0 && _times <= maxBatch, "mint wrong number");
        require(totalItems + _times <= totalCount, "too much");
        require(msg.value == _times * price, "value error");
        payable(owner()).transfer(msg.value);
        emit MintNFT(_msgSender(), totalItems+1, _times);
        for(uint256 i=0; i< _times; i++){
            _mint(_msgSender(), 1 + totalItems++);
        }
    } 
    ...
    function adminMintGiveaways(address _addr) public onlyOwner {
        require(totalItems + 1 <= totalCount, "Mint amount will exceed total collection amount.");
        emit MintNFT(_addr, totalItems+1, 1);
        _mint(_addr, 1 + totalItems++);
    } 
