...
contract MIMWANDPresale is Ownable {
    ...
    // Tokens to raise (MIM) and for offer (pWAND) which can be swapped for (WAND)
    ...
    uint256 public tokenPrice = 20 * 1e18; // 20 MIM per pWAND

    uint256 public cap = 1000 * 1e18; // 1000 MIM cap per whitelisted user

    uint256 public minCap = 200 * 1e18; // 200 MIM cap per whitelisted user
    ...
    /**
     *  @notice set tokenPrice for pWAND
     *  @param _price: tokenPrice for pWAND
     */
    ...
    function addMultipleWhitelist(address[] calldata _addresses)
        ...
        onlyOwner
    {
        require(_addresses.length <= 330, "too many addresses");
     ...
     *  @param _address: address to remove from whitelisted
     */
    ...
    function deposit(uint256 _amount) external checkIfPaused {
        ...
        require(
            whitelisted[msg.sender] == true || isPublic,
        ...
        uint256 payout = _amount.mul(1e18).div(tokenPrice).div(1e9); // pWAND to mint for _amount
