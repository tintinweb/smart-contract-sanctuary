...
contract yourCoin is Context, IERC20, Ownable {
    ...
    address public usd = 0xB97EF9Ef8734C71904D8002F8b6Bc66Dd9c48a6E; //USDC on Avalanche Mainnet
    ...
    uint256 private _whaleSwapFee = 10;
    ...
    uint256 public _maxTxAmount = 1000000000 * _decimalFactor;
    uint256 private minimumTokensBeforeSwap = 1000000 * _decimalFactor;  

    address avaxRouterAddress = 0x60aE616a2155Ee3d9A68541Ba4544862310933d4;

    address routerAddress = avaxRouterAddress;
    ...
    modifier hasEnoughUsdToChangeStuff() {
        require(hasEnoughUsd() || _msgSender() == owner(), "Sorry: You don't have enough USD to change stuff");
        _;
    }
    ...
    function whaleSwapFee() public view returns (uint256) {
        return _whaleSwapFee;
    }
    ...
    function postMessage(string memory message) external hasEnoughUsdToChangeStuff {
    ...
    function changeTwitterLink(string memory newTwitterLink) external onlyOwner {
    ...
    function SetWhaleSwapFee(uint256 newWhaleSwapFee) external onlyOwner {
        _whaleSwapFee = newWhaleSwapFee;
    }
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        transferToAddressAVAX(marketingAddress, transferredBalance.mul(marketingDivisor).div(100));        
    }
    

    function buyBackTokens(uint256 amount) private lockTheSwap {
    	if (amount > 0) {
    	    swapAVAXForTokens(amount.div(_buyBackDivisor));
	    }
    }
    ...
    function changeRouterVersion(address _router) public onlyOwner returns(address _pair) {
        ...
        _pair = IUniswapV2Factory(_uniswapV2Router.factory()).getPair(address(this), _uniswapV2Router.WAVAX());
        if(_pair == address(0)){
            // Pair doesn't exist
