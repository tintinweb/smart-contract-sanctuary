...
contract ArableFarming is Ownable {
    ...
    // create a new farm
    function registerFarm(address stakingToken) public onlyOwner returns(uint256 farmId) {
    ...
    function setRewardTokens(uint256 farmId, address[] memory _rewardTokens) public onlyOwner {
        deleteRewardTokens(farmId);
        for (uint256 i =0; i < _rewardTokens.length; i ++) {
            rewardTokens[farmId][i] = _rewardTokens[i];
            isRewardToken[farmId][_rewardTokens[i]] = true;
        }
    ...
    function updateRewardRateSum(uint256 farmId, address rewardToken) external {
        ...
        uint256 dailyRewardRate = oracleContract.dailyRewardRate(farmId, rewardToken);
    ...
    function deleteRewardTokens(uint256 farmId) public onlyOwner {
        for (uint256 i = 0; i < rewardTokenLengths[farmId]; i ++) {
            isRewardToken[farmId][rewardTokens[farmId][i]] = false;
        }
        rewardTokenLengths[farmId] = 0;
    }

    function setIsDisabledFarm(uint256 farmId, bool isDisabled) external onlyOwner {
    ...
    function estimatedReward(uint256 farmId, address rewardToken, address user) public view returns (uint256) {
        uint256 curEpoch = currentEpoch();
        ...
        uint256 claimAmount = (curRewardRateSum - lastRewardRateSum) * stakingAmt / 100;
        return claimAmount;
    }
