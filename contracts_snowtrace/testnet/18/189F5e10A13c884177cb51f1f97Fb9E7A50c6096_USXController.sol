...
interface ComptrollerImplInterface {
    ...
    function distributeUSXMinterThermo(address usxMinter) external;
}
 ...
 */
contract USXController is USXControllerStorageG2, USXControllerErrorReporter, Exponential {
     ...
     */
    event RepayUSX(address payer, address borrower, uint repayUSXAmount);
     ...
     */
    event LiquidateUSX(address liquidator, address borrower, uint repayAmount, address sTokenCollateral, uint seizeTokens);
    ...
    /**
     * @notice Event emitted when USXs are minted and fee are transferred
     */
    ...
    function mintUSX(uint mintUSXAmount) external nonReentrant returns (uint) {
        if(address(comptroller) != address(0)) {
            require(mintUSXAmount > 0, "mintUSXAmount cannt be zero");
            ...
            ComptrollerImplInterface(address(comptroller)).distributeUSXMinterThermo(minter);

            (vars.oErr, vars.accountMintableUSX) = getMintableUSX(minter);
            if (vars.oErr != uint(Error.NO_ERROR)) {
                return uint(Error.REJECTION);
            }

            // check that user have sufficient mintableUSX balance
            if (mintUSXAmount > vars.accountMintableUSX) {
                return fail(Error.REJECTION, FailureInfo.USX_MINT_REJECTION);
            }

            (vars.mathErr, vars.accountMintUSXNew) = addUInt(ComptrollerImplInterface(address(comptroller)).mintedUSXs(minter), mintUSXAmount);
            require(vars.mathErr == MathError.NO_ERROR, "USX_MINT_AMOUNT_CALCULATION_FAILED");
            uint error = comptroller.setMintedUSXOf(minter, vars.accountMintUSXNew);
            if (error != 0 ) {
                return error;
            }
            ...
            if (treasuryPercent != 0) {
                ...
                USX(getUSXAddress()).mint(treasuryAddress, feeAmount);
            ...
            USX(getUSXAddress()).mint(minter, remainedAmount);

            emit MintUSX(minter, remainedAmount);
     ...
     * @notice Repay USX
     */
    function repayUSX(uint repayUSXAmount) external nonReentrant returns (uint, uint) {
        if(address(comptroller) != address(0)) {
            require(repayUSXAmount > 0, "repayUSXAmount cannt be zero");
            ...
            updateThermoUSXMintIndex();
            ComptrollerImplInterface(address(comptroller)).distributeUSXMinterThermo(payer);

            return repayUSXFresh(msg.sender, msg.sender, repayUSXAmount);
        }
    }

    /**
     * @notice Repay USX Internal
     * @notice Borrowed USXs are repaid by another user (possibly the borrower).
     * @param payer the account paying off the USX
     * @param borrower the account with the debt being payed off
     * @param repayAmount the amount of USX being returned
     * @return (uint, uint) An error code (0=success, otherwise a failure, see ErrorReporter.sol), and the actual repayment amount.
     */
    function repayUSXFresh(address payer, address borrower, uint repayAmount) internal returns (uint, uint) {
        uint actualBurnAmount;

        uint usxBalanceBorrower = ComptrollerImplInterface(address(comptroller)).mintedUSXs(borrower);

        if(usxBalanceBorrower > repayAmount) {
            actualBurnAmount = repayAmount;
        } else {
            actualBurnAmount = usxBalanceBorrower;
        }

        MathError mErr;
        uint accountUSXNew;

        USX(getUSXAddress()).burn(payer, actualBurnAmount);

        (mErr, accountUSXNew) = subUInt(usxBalanceBorrower, actualBurnAmount);
        require(mErr == MathError.NO_ERROR, "USX_BURN_AMOUNT_CALCULATION_FAILED");

        uint error = comptroller.setMintedUSXOf(borrower, accountUSXNew);
        if (error != 0) {
            return (error, 0);
        }
        emit RepayUSX(payer, borrower, actualBurnAmount);

        return (uint(Error.NO_ERROR), actualBurnAmount);
    }

    /**
     * @notice The sender liquidates the usx minters collateral.
     *  The collateral seized is transferred to the liquidator.
     * @param borrower The borrower of usx to be liquidated
     * @param repayAmount The amount of the underlying borrowed asset to repay
     * @return (uint, uint) An error code (0=success, otherwise a failure, see ErrorReporter.sol), and the actual repayment amount.
     */
    function liquidateUSX(address borrower, uint repayAmount, STokenInterface sTokenCollateral) external nonReentrant returns (uint, uint) {
        require(!ComptrollerImplInterface(address(comptroller)).protocolPaused(), "protocol is paused");

        uint error = sTokenCollateral.accrueInterest();
        if (error != uint(Error.NO_ERROR)) {
            // accrueInterest emits logs on errors, but we still want to log the fact that an attempted liquidation failed
            return (fail(Error(error), FailureInfo.USX_LIQUIDATE_ACCRUE_COLLATERAL_INTEREST_FAILED), 0);
        }

        // liquidateUSXFresh emits borrow-specific logs on errors, so we don't need to
        return liquidateUSXFresh(msg.sender, borrower, repayAmount, sTokenCollateral);
    }

    /**
     * @notice The liquidator liquidates the borrowers collateral by repay borrowers USX.
     *  The collateral seized is transferred to the liquidator.
     * @param liquidator The address repaying the USX and seizing collateral
     * @param borrower The borrower of this USX to be liquidated
     * @param sTokenCollateral The market in which to seize collateral from the borrower
     * @param repayAmount The amount of the USX to repay
     * @return (uint, uint) An error code (0=success, otherwise a failure, see ErrorReporter.sol), and the actual repayment USX.
     */
    function liquidateUSXFresh(address liquidator, address borrower, uint repayAmount, STokenInterface sTokenCollateral) internal returns (uint, uint) {
        if(address(comptroller) != address(0)) {
            ...
            if (allowed != 0) {
                return (failOpaque(Error.REJECTION, FailureInfo.USX_LIQUIDATE_COMPTROLLER_REJECTION, allowed), 0);
            }

            /* Verify sTokenCollateral market's block number equals current block number */
            //if (sTokenCollateral.accrualBlockNumber() != accrualBlockNumber) {
            if (sTokenCollateral.accrualBlockNumber() != getBlockNumber()) {
                return (fail(Error.REJECTION, FailureInfo.USX_LIQUIDATE_COLLATERAL_FRESHNESS_CHECK), 0);
            }

            /* Fail if borrower = liquidator */
            if (borrower == liquidator) {
                return (fail(Error.REJECTION, FailureInfo.USX_LIQUIDATE_LIQUIDATOR_IS_BORROWER), 0);
            }

            /* Fail if repayAmount = 0 */
            if (repayAmount == 0) {
                return (fail(Error.REJECTION, FailureInfo.USX_LIQUIDATE_CLOSE_AMOUNT_IS_ZERO), 0);
            }

            /* Fail if repayAmount = -1 */
            if (repayAmount == uint(-1)) {
                return (fail(Error.REJECTION, FailureInfo.USX_LIQUIDATE_CLOSE_AMOUNT_IS_UINT_MAX), 0);
            }


            /* Fail if repayUSX fails */
            (uint repayBorrowError, uint actualRepayAmount) = repayUSXFresh(liquidator, borrower, repayAmount);
            if (repayBorrowError != uint(Error.NO_ERROR)) {
                return (fail(Error(repayBorrowError), FailureInfo.USX_LIQUIDATE_REPAY_BORROW_FRESH_FAILED), 0);
            }
            ...
            (uint amountSeizeError, uint seizeTokens) = comptroller.liquidateUSXCalculateSeizeTokens(address(sTokenCollateral), actualRepayAmount);
            require(amountSeizeError == uint(Error.NO_ERROR), "USX_LIQUIDATE_COMPTROLLER_CALCULATE_AMOUNT_SEIZE_FAILED");

            /* Revert if borrower collateral token balance < seizeTokens */
            require(sTokenCollateral.balanceOf(borrower) >= seizeTokens, "USX_LIQUIDATE_SEIZE_TOO_MUCH");
            ...
            emit LiquidateUSX(liquidator, borrower, actualRepayAmount, address(sTokenCollateral), seizeTokens);
    ...
    function updateThermoUSXMintIndex() public returns (uint) {
        uint usxMinterSpeed = ComptrollerImplInterface(address(comptroller)).thermoUSXRate();
    ...
    function calcDistributeUSXMinterThermo(address usxMinter) public returns(uint, uint, uint, uint) {
        ...
        uint usxMinterAmount = ComptrollerImplInterface(address(comptroller)).mintedUSXs(usxMinter);
        uint usxMinterDelta = mul_(usxMinterAmount, deltaIndex);
        uint usxMinterAccrued = add_(ComptrollerImplInterface(address(comptroller)).thermoAccrued(usxMinter), usxMinterDelta);
    ...
    function _become(USXUnitroller unitroller) external {
    ...
    function getMintableUSX(address minter) public view returns (uint, uint) {
        ...
        SToken[] memory enteredMarkets = ComptrollerImplInterface(address(comptroller)).getAssetsIn(minter);
        ...
        for (i = 0; i < enteredMarkets.length; i++) {
            (vars.oErr, vars.sTokenBalance, vars.borrowBalance, vars.exchangeRateMantissa) = enteredMarkets[i].getAccountSnapshot(minter);
            ...
            (vars.mErr, vars.sumSupply) = mulScalarTruncateAddUInt(vars.tokensToDenom, vars.sTokenBalance, vars.sumSupply);
            if (vars.mErr != MathError.NO_ERROR) {
                return (uint(Error.MATH_ERROR), 0);
            }
        ...
        (vars.mErr, vars.sumBorrowPlusEffects) = addUInt(vars.sumBorrowPlusEffects, ComptrollerImplInterface(address(comptroller)).mintedUSXs(minter));
        if (vars.mErr != MathError.NO_ERROR) {
            return (uint(Error.MATH_ERROR), 0);
        }

        (vars.mErr, accountMintableUSX) = mulUInt(vars.sumSupply, ComptrollerImplInterface(address(comptroller)).usxMintRate());
        require(vars.mErr == MathError.NO_ERROR, "USX_MINT_AMOUNT_CALCULATION_FAILED");

        (vars.mErr, accountMintableUSX) = divUInt(accountMintableUSX, 10000);
        require(vars.mErr == MathError.NO_ERROR, "USX_MINT_AMOUNT_CALCULATION_FAILED");


        (vars.mErr, accountMintableUSX) = subUInt(accountMintableUSX, vars.sumBorrowPlusEffects);
        if (vars.mErr != MathError.NO_ERROR) {
