...
contract Noonercoin is ERC20{
   ...
   uint256 totalCycleLeft = 19;
    ...
    uint256 private _cycleTime = 72576000;//120 week secs
   ...
   function mintToken(address add) public returns (bool) {  //admin only
       ...
       uint256 nowTime = now;
       if(nowTime-weekStartTime>604800){//7 days sec
           popRandomVariable();
           weekStartTime=now;
       }
    ...
    function burnToken() internal returns(bool){
        ...
        for(uint256 index=1;index<=totalCycleLeft;index++){
            uint256 intValue = getIntegerValue(flag * 604800,  150 ** index, index);
            uint256 intDecimalValue = getDecimalValue(flag * 604800,  150 ** index, index);
  ...
  function checkMissingPops() public view returns(uint256){
      ...
      if(lastMintingTime != 0) {
          ...
          uint256 estimatedPicks = timeDifference / 604800;
