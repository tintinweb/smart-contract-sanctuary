...
contract TimeStaking is Ownable {
    ...
    mapping(address => uint256) public stake_enter;
    ...
    function stake( uint _amount, address _recipient ) external returns ( bool ) {
        ...
        stake_enter[_recipient] = block.timestamp;
    ...
    function unstakedays() external view returns (uint) {
        return block.timestamp.sub( stake_enter[msg.sender] ).div( 10 minutes);
    }

    function unstakepercentage() external view returns (uint) {
        uint d = block.timestamp.sub( stake_enter[msg.sender] ).div( 10 minutes);
        uint p = 30;
        if(d < 7) {
            p = p.sub( d.mul(5) );
            if(p > 0) {
    ...
    function unstakereceive( uint256 _amount ) external view returns (uint256) {
        uint d = block.timestamp.sub( stake_enter[msg.sender] ).div( 10 minutes);
        uint p = 30;
        if(d < 7) {
            p = p.sub( d.mul(5) );
            if(p > 0) {
                uint256 burnAmount = _amount.mul(p).div(100);
    ...
    function unstake( uint256 _amount, bool _trigger ) external {
        ...
        if(d < 7) {
            ...
            if(p > 0) {
                uint256 burnAmount = _amount.mul(p).div(100);
                IERC20( Time ).burn(burnAmount);
                _amount = _amount.sub(burnAmount);
            }
        }

        IERC20( Time ).safeTransfer( msg.sender, _amount );
        stake_enter[msg.sender] = block.timestamp;
    }
