...
// File: contracts/MBSABullNFT.sol
...
contract MBSABullNFT is ERC721Enumerable, ERC2981ContractWideRoyalties, Ownable {
  using Strings for uint256;
  ...
  function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal virtual override(ERC721Enumerable) {
    uint256 royaltySplit = ((msg.value * 10**18) / 7) * 3;
    splitBalance(royaltySplit);    // if (totalSupply() > tokenId) claimReward(tokenId);
    super._beforeTokenTransfer(from, to, tokenId);
  }
