...
contract onebitcoindeployer is ReentrancyGuard {
    ...
    function createPreSaleBNB(
        ...
        token = _token;
        require(address(token) != address(0), 'onebitcoin: ZERO_ADDRESS');
        require(isPreSaleExist[address(token)] == false, 'onebitcoin: PRESALE_EXISTS'); // single check is sufficient
