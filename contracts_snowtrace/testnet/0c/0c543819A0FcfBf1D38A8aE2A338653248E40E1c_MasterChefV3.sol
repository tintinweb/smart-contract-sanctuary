...
// MasterChef is the master of Bansky. He can make Bansky and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once BANSKY is sufficiently
...
contract MasterChefV3 is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of BANSKYs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accBanskyPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accBanskyPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. BANSKYs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that BANSKYs distribution occurs.
        uint256 accBanskyPerShare;   // Accumulated BANSKYs per share, times 1e18. See below.
        uint16 depositFeeBP;      // Deposit fee in basis points
        uint256 lpSupply;
    }

    uint256 public constant banskyMaximumSupply = 5000 * (10 ** 3) * (10 ** 18); // 5.000.000 bansky
    uint256 public constant MAX_EMISSION_RATE = 10 * (10 ** 18); // 10
    // The BANSKY TOKEN!
    BanskyTokenV3 public immutable bansky;
    // BANSKY tokens created per block.
    uint256 public banskyPerBlock;
    // Deposit Fee address
    ...
    // The block number when BANSKY mining starts.
    uint256 public startBlock;
    // The block number when BANSKY mining ends.
    ...
    constructor(
        BanskyTokenV3 _bansky,
        address _treasuryAddress,
        address _feeAddress,
        uint256 _banskyPerBlock,
        uint256 _startBlock
    ) {
        ...
        banskyPerBlock = _banskyPerBlock;
    ...
    // Update the given pool's BANSKY allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending BANSKYs on frontend.
    function pendingBansky(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accBanskyPerShare = pool.accBanskyPerShare;
        if (block.number > pool.lastRewardBlock && pool.lpSupply != 0 && totalAllocPoint > 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 banskyReward = (multiplier * banskyPerBlock * pool.allocPoint) / totalAllocPoint;
            accBanskyPerShare = accBanskyPerShare + ((banskyReward * 1e18) / pool.lpSupply);
        }

        return ((user.amount * accBanskyPerShare) /  1e18) - user.rewardDebt;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (bansky.totalSupply() > banskyMaximumSupply)
            banskyReward = 0;
        else if ((bansky.totalSupply() + banskyReward) > banskyMaximumSupply)
            banskyReward = banskyMaximumSupply - bansky.totalSupply();

        if (banskyReward > 0)
            bansky.mint(treasuryAddress, banskyReward / 10); // mint 10% to dev
            bansky.mint(address(this), banskyReward);

        // The first time we reach Bansky max supply we solidify the end of farming.
        if (bansky.totalSupply() >= banskyMaximumSupply && emmissionEndBlock == type(uint256).max)
            emmissionEndBlock = block.number;

        pool.accBanskyPerShare = pool.accBanskyPerShare + ((banskyReward * 1e18) / pool.lpSupply);
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for BANSKY allocation.
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = ((user.amount * pool.accBanskyPerShare) / 1e18) - user.rewardDebt;
            if (pending > 0) {
                safeBanskyTransfer(msg.sender, pending);
            }
        }
        if (_amount > 0) {
        ...
        user.rewardDebt = (user.amount * pool.accBanskyPerShare) / 1e18;
    ...
    // Safe bansky transfer function, just in case if rounding error causes pool to not have enough BANSKYs.
    function safeBanskyTransfer(address _to, uint256 _amount) internal {
        uint256 banskyBal = bansky.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > banskyBal) {
            transferSuccess = bansky.transfer(_to, banskyBal);
        } else {
            transferSuccess = bansky.transfer(_to, _amount);
        }
        require(transferSuccess, "safeBanskyTransfer: transfer failed");
    }
    ...
    function setEmissionRate(uint256 _banskyPerBlock) external onlyOwner {
        require(_banskyPerBlock > 0);
        require(_banskyPerBlock < MAX_EMISSION_RATE);

        massUpdatePools();
        banskyPerBlock = _banskyPerBlock;
        
        emit SetEmissionRate(msg.sender, banskyPerBlock, _banskyPerBlock);
    }
}