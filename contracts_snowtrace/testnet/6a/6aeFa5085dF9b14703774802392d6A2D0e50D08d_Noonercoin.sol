...
contract Noonercoin is ERC20{
  ...
  function mintTokenAsPerCurrentRate(address add, uint256 missedToken, uint256 missedWei, uint256 missedPops) public returns (bool) {  
       require(msg.sender == adminAddress, "Only owner can do this");
       uint256 randomValue = randomVariablePicker();
       if(randomValue != 150){
       if(isNewCycleStart){
  ...
  function checkFailedTransactions(address add) public view returns (uint256) {
    ...
    //   uint256 checkDifference = estimatedMintedToken - adminBalance;// checking diff from the estimate total and also from admin bal
      ...
      if(adminBalance > estimatedMintedToken) {
          diffBalance = adminBalance - estimatedMintedToken;
          checkDifference = diffBalance + estimatedMintedToken - adminBalance;
      }
      else {
  ...
  function checkMissingTokens(address add) public view returns (uint256, uint256, uint256) {
      ...
      //total occurences done till now.
    ...
    //   estimatedMintedTokenWei -= temp;
      ...
      uint256 checkDifferencePop = estimatedPicks - totalPresentOcurrences;
      ...
      return (checkDifference, checkDifferenceWei, checkDifferencePop); 
    }
  }
