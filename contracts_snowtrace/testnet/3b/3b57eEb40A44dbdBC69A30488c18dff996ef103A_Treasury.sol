...
contract Treasury is Ownable {
  ...
  uint256 public sTokenQueue; // Delays change to sToken address
  ...
  function incurDebt(uint256 _amount, address _token) external {
    ...
    uint256 maximumDebt = IERC20(sToken).balanceOf(msg.sender); // Can only borrow against sToken held
  ...
  */
    ...
    returns (uint256 value_)
  {
    if (isReserveToken[_token]) {
      ...
      value_ = _amount.mul(10**IERC20(Token).decimals()).div(
    ...
    returns (bool)
  {
    ...
    } else if (_managing == MANAGING.SToken) {
      // 9
      sTokenQueue = block.number.add(blocksNeededForQueue);
    } else return false;
