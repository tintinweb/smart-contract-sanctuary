...
/**
 * @title Thermo's STmxLikeDelegate Contract
 * @notice STokens which can 'delegate votes' of their underlying BEP-20
 * @author Venus (Modified by Source)
 */
contract STmxLikeDelegate is SBep20Delegate {
  /**
   * @notice Construct an empty delegate
   */
  constructor() public SBep20Delegate() {}

  /**
   * @notice Admin call to delegate the votes of the TMX-like underlying
   * @param tmxLikeDelegatee The address to delegate votes to
   */
  function _delegateTmxLikeTo(address tmxLikeDelegatee) external {
    require(msg.sender == admin, "only the admin may set the tmx-like delegate");
    TmxLike(underlying).delegate(tmxLikeDelegatee);
  }
}
