...
// checks if vesting schedule does not exists for specified address
    ...
    function claimTokensForAddress(address vestingAddress, uint256 amount) 
        ...
        uint256 claimedAmount = amount * 10 ** 2; // token has 2 decimals
        uint256 amountCanBeClaimed = (vestingSchedule.amount * currentPeriod.percentagesPerPeriod) / 100;
        require(amountCanBeClaimed >= claimedAmount, "Can't claim tokens, not enough vested tokens");
        ...
        _token.transfer(beneficiaryPayable, claimedAmount);
        emit Claimed(beneficiaryPayable, claimedAmount);
    }
