...
import "./USXUnitroller.sol";
import "./USX/USX.sol";

interface ComptrollerLensInterface {
    function protocolPaused() external view returns (bool);
    function mintedUSXs(address account) external view returns (uint);
    function usxMintRate() external view returns (uint);
    function thermoUSXRate() external view returns (uint);
    function thermoAccrued(address account) external view returns(uint);
    function getAssetsIn(address account) external view returns (SToken[] memory);
    function oracle() external view returns (PriceOracle);

    function distributeUSXMinterThermo(address usxMinter, bool distributeAll) external;
}

/**
 * @title Thermo's USX Comptroller Contract
 * @author Venus (Modified by Source)
 */
contract USXControllerG1 is USXControllerStorageG1, USXControllerErrorReporter, Exponential {

    /// @notice Emitted when Comptroller is changed
    event NewComptroller(ComptrollerInterface oldComptroller, ComptrollerInterface newComptroller);

    /**
     * @notice Event emitted when USX is minted
     */
    event MintUSX(address minter, uint mintUSXAmount);

    /**
     * @notice Event emitted when USX is repaid
     */
    event RepayUSX(address repayer, uint repayUSXAmount);

    /// @notice The initial Thermo index for a market
    uint224 public constant thermoInitialIndex = 1e36;

    /*** Main Actions ***/

    function mintUSX(uint mintUSXAmount) external returns (uint) {
        if(address(comptroller) != address(0)) {
            ...
            updateThermoUSXMintIndex();
            ComptrollerLensInterface(address(comptroller)).distributeUSXMinterThermo(minter, false);

            uint oErr;
            MathError mErr;
            uint accountMintUSXNew;
            uint accountMintableUSX;

            (oErr, accountMintableUSX) = getMintableUSX(minter);
            if (oErr != uint(Error.NO_ERROR)) {
                return uint(Error.REJECTION);
            }

            // check that user have sufficient mintableUSX balance
            if (mintUSXAmount > accountMintableUSX) {
                return fail(Error.REJECTION, FailureInfo.USX_MINT_REJECTION);
            }

            (mErr, accountMintUSXNew) = addUInt(ComptrollerLensInterface(address(comptroller)).mintedUSXs(minter), mintUSXAmount);
            require(mErr == MathError.NO_ERROR, "USX_MINT_AMOUNT_CALCULATION_FAILED");
            uint error = comptroller.setMintedUSXOf(minter, accountMintUSXNew);
            if (error != 0 ) {
                return error;
            }

            USX(getUSXAddress()).mint(minter, mintUSXAmount);
            emit MintUSX(minter, mintUSXAmount);
     ...
     * @notice Repay USX
     */
    function repayUSX(uint repayUSXAmount) external returns (uint) {
        if(address(comptroller) != address(0)) {
            require(!ComptrollerLensInterface(address(comptroller)).protocolPaused(), "protocol is paused");
            ...
            updateThermoUSXMintIndex();
            ComptrollerLensInterface(address(comptroller)).distributeUSXMinterThermo(repayer, false);

            uint actualBurnAmount;

            uint usxBalance = ComptrollerLensInterface(address(comptroller)).mintedUSXs(repayer);

            if(usxBalance > repayUSXAmount) {
                actualBurnAmount = repayUSXAmount;
            } else {
                actualBurnAmount = usxBalance;
            }

            uint error = comptroller.setMintedUSXOf(repayer, usxBalance - actualBurnAmount);
            if (error != 0) {
                return error;
            }

            USX(getUSXAddress()).burn(repayer, actualBurnAmount);
            emit RepayUSX(repayer, actualBurnAmount);
    ...
    /**
     * @notice Initialize the ThermoUSXState
     */
    function _initializeThermoUSXState(uint blockNumber) external returns (uint) {
        ...
        if (isThermoUSXInitialized == false) {
            isThermoUSXInitialized = true;
            uint usxBlockNumber = blockNumber == 0 ? getBlockNumber() : blockNumber;
            thermoUSXState = ThermoUSXState({
                index: thermoInitialIndex,
                block: safe32(usxBlockNumber, "block number overflows")
            });
        }
    }

    /**
     * @notice Accrue TMX to by updating the USX minter index
     */
    function updateThermoUSXMintIndex() public returns (uint) {
        uint usxMinterSpeed = ComptrollerLensInterface(address(comptroller)).thermoUSXRate();
        uint blockNumber = getBlockNumber();
        uint deltaBlocks = sub_(blockNumber, uint(thermoUSXState.block));
        if (deltaBlocks > 0 && usxMinterSpeed > 0) {
            uint usxAmount = USX(getUSXAddress()).totalSupply();
            uint thermoAccrued = mul_(deltaBlocks, usxMinterSpeed);
            Double memory ratio = usxAmount > 0 ? fraction(thermoAccrued, usxAmount) : Double({mantissa: 0});
            Double memory index = add_(Double({mantissa: thermoUSXState.index}), ratio);
            thermoUSXState = ThermoUSXState({
                index: safe224(index.mantissa, "new index overflows"),
                block: safe32(blockNumber, "block number overflows")
            });
        } else if (deltaBlocks > 0) {
            thermoUSXState.block = safe32(blockNumber, "block number overflows");
        }
    }

    /**
     * @notice Calculate TMX accrued by a USX minter
     * @param usxMinter The address of the USX minter to distribute TMX to
     */
    function calcDistributeUSXMinterThermo(address usxMinter) public returns(uint, uint, uint, uint) {
        // Check caller is comptroller
        if (msg.sender != address(comptroller)) {
            return (fail(Error.UNAUTHORIZED, FailureInfo.SET_COMPTROLLER_OWNER_CHECK), 0, 0, 0);
        }

        Double memory usxMintIndex = Double({mantissa: thermoUSXState.index});
        Double memory usxMinterIndex = Double({mantissa: thermoUSXMinterIndex[usxMinter]});
        thermoUSXMinterIndex[usxMinter] = usxMintIndex.mantissa;

        if (usxMinterIndex.mantissa == 0 && usxMintIndex.mantissa > 0) {
            usxMinterIndex.mantissa = thermoInitialIndex;
        }

        Double memory deltaIndex = sub_(usxMintIndex, usxMinterIndex);
        uint usxMinterAmount = ComptrollerLensInterface(address(comptroller)).mintedUSXs(usxMinter);
        uint usxMinterDelta = mul_(usxMinterAmount, deltaIndex);
        uint usxMinterAccrued = add_(ComptrollerLensInterface(address(comptroller)).thermoAccrued(usxMinter), usxMinterDelta);
        return (uint(Error.NO_ERROR), usxMinterAccrued, usxMinterDelta, usxMintIndex.mantissa);
    }
    ...
    function _become(USXUnitroller unitroller) public {
    ...
    function getMintableUSX(address minter) public view returns (uint, uint) {
        ...
        SToken[] memory enteredMarkets = ComptrollerLensInterface(address(comptroller)).getAssetsIn(minter);
        ...
        /**
         * We use this formula to calculate mintable USX amount.
         * totalSupplyAmount * USXMintRate - (totalBorrowAmount + mintedUSXOf)
         */
        for (i = 0; i < enteredMarkets.length; i++) {
            (oErr, vars.sTokenBalance, vars.borrowBalance, vars.exchangeRateMantissa) = enteredMarkets[i].getAccountSnapshot(minter);
            ...
            // sumSupply += tokensToDenom * sTokenBalance
            (mErr, vars.sumSupply) = mulScalarTruncateAddUInt(vars.tokensToDenom, vars.sTokenBalance, vars.sumSupply);
            if (mErr != MathError.NO_ERROR) {
                return (uint(Error.MATH_ERROR), 0);
            }
        ...
        (mErr, vars.sumBorrowPlusEffects) = addUInt(vars.sumBorrowPlusEffects, ComptrollerLensInterface(address(comptroller)).mintedUSXs(minter));
        if (mErr != MathError.NO_ERROR) {
            return (uint(Error.MATH_ERROR), 0);
        }

        (mErr, accountMintableUSX) = mulUInt(vars.sumSupply, ComptrollerLensInterface(address(comptroller)).usxMintRate());
        require(mErr == MathError.NO_ERROR, "USX_MINT_AMOUNT_CALCULATION_FAILED");

        (mErr, accountMintableUSX) = divUInt(accountMintableUSX, 10000);
        require(mErr == MathError.NO_ERROR, "USX_MINT_AMOUNT_CALCULATION_FAILED");


        (mErr, accountMintableUSX) = subUInt(accountMintableUSX, vars.sumBorrowPlusEffects);
        if (mErr != MathError.NO_ERROR) {
            return (uint(Error.REJECTION), 0);
        }

        return (uint(Error.NO_ERROR), accountMintableUSX);
    }
    ...
    /**
     * @notice Return the address of the USX token
     * @return The address of USX
     */
    function getUSXAddress() public view returns (address) {
        return 0x4BD17003473389A42DAF6a0a729f6Fdb328BbBd7;  //update after USX Launch!!
    }
}
...
contract USX is LibNote {
    // --- Auth ---
    mapping (address => uint) public wards;
    function rely(address guy) external note auth { wards[guy] = 1; }
    function deny(address guy) external note auth { wards[guy] = 0; }
    modifier auth {
        require(wards[msg.sender] == 1, "USX/not-authorized");
        _;
    }

    // --- BEP20 Data ---
    string  public constant name     = "USX Stablecoin";
    ...
    function add(uint x, uint y) internal pure returns (uint z) {
        require((z = x + y) >= x, "USX math error");
    }
    ...
    function transferFrom(address src, address dst, uint wad)
        public returns (bool)
    {
        require(balanceOf[src] >= wad, "USX/insufficient-balance");
        if (src != msg.sender && allowance[src][msg.sender] != uint(-1)) {
            require(allowance[src][msg.sender] >= wad, "USX/insufficient-allowance");
    ...
    function burn(address usr, uint wad) external {
        require(balanceOf[usr] >= wad, "USX/insufficient-balance");
        if (usr != msg.sender && allowance[usr][msg.sender] != uint(-1)) {
            require(allowance[usr][msg.sender] >= wad, "USX/insufficient-allowance");
                    ...
                    bool allowed, uint8 v, bytes32 r, bytes32 s) external
    {
        ...
        require(holder != address(0), "USX/invalid-address-0");
        require(holder == ecrecover(digest, v, r, s), "USX/invalid-permit");
        require(expiry == 0 || now <= expiry, "USX/permit-expired");
        require(nonce == nonces[holder]++, "USX/invalid-nonce");
