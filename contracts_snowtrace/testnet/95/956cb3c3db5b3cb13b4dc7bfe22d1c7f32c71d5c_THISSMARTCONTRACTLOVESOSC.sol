...
contract THISSMARTCONTRACTLOVESOSC is Ownable {
    ...
    function changeParameter(uint _parameter, uint value) public {   
        ...
        require(eventRunning,"AAA");
        require(_parameter < totalParams,"BBB");
        require(value >= param.minVal && value <= param.maxVal, "CCC");
        param.value = value;
    ...
    function changeParameterMulti(ParamChange[] memory _input) public {  
        require(eventRunning && _input.length <= _section.numParams, "AAA");
        for (uint i = 0; i < _input.length; i++) {
            require(_input[i].id < totalParams, "BBB");
            Param storage param = params[_input[i].id];     
            require(_input[i].value >= param.minVal && _input[i].value <= param.maxVal,"CCC");
