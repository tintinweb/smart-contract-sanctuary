...
contract GameLending is Ownable, ERC1155Holder {
    ...
    function getBorrowedListings(address borrower) public view returns (LendListing[] memory) {
        uint256 activeListingsLength = _activeListingsByBorrower[msg.sender].length();
        LendListing[] memory result = new LendListing[](activeListingsLength);
        uint256 resultIdx;
        for (uint256 i=0; i < activeListingsLength; i++) {
            LendListing memory l = _listings[_activeListingsByBorrower[msg.sender].at(i)];
...
contract Game2 is Ownable, ERC1155Holder, Pausable {
    ...
    function placeBorrowedCard(uint256 listingId) external {
        ...
        for (uint256 i=0; i < game_.round; i++)
            require(
                turn0 && game_.player[0].borrowedCards[i] != listingId ||
                !turn0 && game_.player[1].borrowedCards[i] != listingId, "this listing is already used");
        ...
        game_.player[game_.turn].placedCards[game_.round] = tokenId;
        game_.player[game_.turn].borrowedCards[game_.round] = listingId;
        _afterPlace(game_, tokenId);
    }
