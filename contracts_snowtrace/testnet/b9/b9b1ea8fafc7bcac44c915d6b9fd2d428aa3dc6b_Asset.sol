...
// File: contracts/pancake-swap/interfaces/IPangolinRouter02.sol
            ...
            if (isAllowedAutoXYRebalace == true) {
                ...
                if (xAmount >= sellAmounts[i] && xAmount - sellAmounts[i] >= xStopAmount) {
                    ...
                    if (xAmount > xAmountMax) {
                        ...
                        yVaultAmount[tokensInAsset[i]] = 0;
                    }
                }
            } else {
                require(
...
contract Asset is ERC20Upgradeable, ReentrancyGuard, IAsset {
    ...
    event DepositToAsset(address[] tokensInAsset, uint256[] tokenAmountsToY);
    event AssetsFromFeesConvertedToWeth(uint256 assetAmount, uint256 wethAmount);
    event TransferFeesInFeeAddress(uint256 feeAmount);
    ...
    function __Asset_init(
        ...
        require(_feeAddress != address(0), "Not valid address");
        require(
            // solhint-disable-next-line not-rely-on-time
    ...
    function depositToIndex(uint256[] memory tokenAmountsOfY)
        external
        payable
        nonReentrant
        onlyManagerOrAdmin
    {
        ...
        emit DepositToAsset(_tokensInAsset, tokenAmountsOfY);
    }
    ...
    function _autoTransferFee(bool isForce) private {
        ...
        if (totalAmountFee > 0) {
            feeAddress.transfer(totalAmountFee);
            // IStakingFactory(zVault).inputBnb{value: totalAmountFee}();
            emit TransferFeesInFeeAddress(totalAmountFee);
        }
    }
