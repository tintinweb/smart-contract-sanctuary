...
contract Shadow is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 11000 ether;
    // Initial distribution for the day 2-5 SHADOW-WFTM LP -> SHADOW pool
    uint256 public constant INITIAL_SHADOW_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the SHADOW ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("SHADOW", "SHADOW") {
        // Mints 1 SHADOW to contract creator for initial pool setup
    ...
    function _getShadowPrice() internal view returns (uint256 _shadowPrice) {
        try IOracle(shadowOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Shadow: failed to fetch SHADOW price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _shadowPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_shadowPrice >= taxTiersTwaps[tierId]) {
    ...
    function setShadowOracle(address _shadowOracle) public onlyOperatorOrTaxOffice {
        require(_shadowOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints SHADOW to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of SHADOW to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentShadowPrice = _getShadowPrice();
            currentTaxRate = _updateTaxRate(currentShadowPrice);
            if (currentShadowPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_shadowPool != address(0), "!_shadowPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_shadowPool, INITIAL_SHADOW_POOL_DISTRIBUTION);
