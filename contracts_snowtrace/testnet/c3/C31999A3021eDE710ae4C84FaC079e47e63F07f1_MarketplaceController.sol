...
// File: interfaces/IMultiwallet.sol
...
// File: interfaces/ICampaignContract.sol
...
// File: interfaces/IArkiusAttentionSeekerToken.sol
...
// File: interfaces/IArkiusCertifierToken.sol
...
// File: MarketPlaceController.sol
 ...
 */
contract MarketplaceController is Ownable, ReentrancyGuard, Pausable {
    ...
    */
    ...
    /// @dev Address which will call the batch functions.
    ...
    event UpdaterAddressChanged(address oldUpdater, address updater);
    ...
    constructor(IERC20                tokenContract,
                ...
                address               pauserAddress) Ownable (multisigAddress) {
    ...
    function adServedBatch(address[]   memory members,
                           ...
                           address[][] memory certifiers,
                           uint256[][] memory certifierWeights) onlyUpdater nonReentrant whenNotPaused external  {
     ...
     */
    function adClickedBatch(uint256[] memory ids) onlyUpdater nonReentrant whenNotPaused external {
     ...
     * @param cutPercentDenominator Denominator of percentage amount to be deducted.
     */
    function adExpiredBatch(uint256[] memory ids, uint256 cutPercentNumerator, uint256 cutPercentDenominator) onlyUpdater nonReentrant whenNotPaused external  {
        for (uint256 idx=0; idx<ids.length; idx++) {
 ...
 * @dev Throws if called by any account other than the updater.
    ...
    modifier onlyUpdater() {
        require(_msgSender()==updater, "caller is not the updater");
        _;
    }

    /**
     * @dev Update the address of updater.
     * Can only be called by the current owner.
     */
    function setUpdaterAddress(address newUpdater) external onlyOwner {
        address oldUpdater = updater;
        updater = newUpdater;
        emit UpdaterAddressChanged(oldUpdater, updater);
    }
