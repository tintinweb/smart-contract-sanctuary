/**
 *Submitted for verification at testnet.snowtrace.io on 2021-12-11
*/
...
interface IERC721 {
    //Returns the number of APAs in ``owner``'s wallet.
...
contract APAGovernance {
    ...
    function balanceOf(address owner) 
        ...
        view 
        returns (uint){
        return IERC721(apaToken).balanceOf(owner);
    }
    ...
    function vote(uint proposalId , Votes _vote) external {
        ...
        require(block.timestamp < proposals[proposalId].timeCreated + votingPeriod, "Proposal has Expired");
        uint currentAPA;
        uint8 ineligibleCount=0;
        uint[] memory UserAPAs = new uint[](voterBalance);
       
        //get user Apas map from wallet
       for(uint16 i=0; i < voterBalance; i++){
            currentAPA = IERC721Enumerable(apaToken).tokenOfOwnerByIndex(msg.sender, i);
            //check if APA has already voted and add to UserAPAs if eligible
           
            if(votedAPAs[proposalId][currentAPA] != true){
                //add to UserAPAs array
...
*/
    ...
    function certifyResults(uint proposalId) external onlyManager() returns(Status) {
            ...
            require(block.timestamp > proposals[proposalId].timeCreated + votingPeriod, "Proposal has not yet ended");
