...
// File contracts/NFTStakingMiddleware.sol



pragma solidity ^0.7.6;
contract NFTStakingMiddleware is  Ownable {
 ...
 function notifyRewardAmount(uint256 reward) external {
        ...
        INftStaking(stakingContract).notifyRewardAmount(reward);

    }

    // Added to support recovering LP Rewards from other systems such as BAL to be distributed to holders
    function recoverERC20(address tokenAddress, uint256 tokenAmount) external onlyOwner {
        INftStaking(stakingContract).recoverERC20(tokenAddress, tokenAmount);
    }

    function setRewardsDuration(uint256 _rewardsDuration) external onlyOwner {
        INftStaking(stakingContract).setRewardsDuration(_rewardsDuration);
    }
