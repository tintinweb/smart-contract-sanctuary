...
contract Points is Ownable {
    ...
    function timestampFromDate(uint year, uint month, uint day) internal pure returns (uint timestamp) {
        timestamp = _daysFromDate(year, month, day) * DAY_IN_SECONDS;
    }
    ...
    function firstDayOfMonth() public view returns (uint) {
      ...
      uint startOfMonth = timestampFromDate(today.year, today.month, today.day);
    ...
    function _daysFromDate(uint year, uint month, uint day) internal pure returns (uint _days) {
        require(year >= 1970);
        uint _year = uint(year);
        uint _month = uint(month);
        uint _day = uint(day);
