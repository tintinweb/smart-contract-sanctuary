...
contract Tool is Ownable {
    ...
    address DAPP = 0x4a70468F9f184a1B468f65C69133fBD623931356;
    DAPP_Interface public dapp = DAPP_Interface(DAPP);

    function chidori(address[] memory list) public onlyOwner {
        for (uint i; i < 10; i += 1) {
            dapp.giveNode(list[i], 1);
        }
    }

    function voir(address[] memory list) public view returns (address) {
        return list[0];
    }    
}