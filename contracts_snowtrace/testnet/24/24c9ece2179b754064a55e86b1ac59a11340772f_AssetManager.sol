...
contract AssetManager {
    ...
    function GetStakePrice(uint assetId) public view returns (uint) {
        return assets[assetId].FixedPrice / assets[assetId].StakesAvailable;
    }
    ...
    function PurchaseStake(uint assetId) external payable {
        Asset storage a = assets[assetId];
        uint stakePrice = a.FixedPrice / a.StakesAvailable;
        ...
        Stake memory newStake = Stake(payable(msg.sender));
        assets[assetId].Stakes.push(newStake); 
        a.StakesReserved++;
