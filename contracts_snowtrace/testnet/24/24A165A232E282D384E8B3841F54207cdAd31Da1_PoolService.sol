...
contract VaultService is PriceModel, Ownable {
    ...
    function approveOnUniswap(address token, address router) public onlyOwner {
        ERC20(token).approve(router, MAX_INT);
    }
...
contract PoolService is Ownable {
    ...
    function allowRouterForTrading(address router) external onlyOwner {
        require(router != address(0), "0x0 address is not allowed");
        for (uint256 i = 0; i < _allowedTokensList.length; i++) {
            if (IERC20_short(_allowedTokensList[i]).allowance(address(_vaultService), router) == 0) {
    ...
    function swapTokensForExactTokens(
        ...
        _swapTokensForExactTokens(amountOut,amountInMax, path, deadline, address(0));
    }
    ...
    function swapTokensForExactTokens(
        ...
        _swapTokensForExactTokens(amountOut,amountInMax, path, deadline, router);
    }
