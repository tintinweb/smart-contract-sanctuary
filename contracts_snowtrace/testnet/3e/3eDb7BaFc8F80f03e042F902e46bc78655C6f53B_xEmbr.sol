    ...
    Ownable
{
    ...
    /** Shared Events */
    event Staked( address indexed provider, uint256 value, uint256 locktime, uint256 start, LockAction indexed action, uint256 ts);
    ...
    /** Paper Hand settings */
    uint256 paperHandsIndex = 2**256 - 1;
    uint256 paperHandsFinish = 0;
    uint256 paperHandsRewardRate = 0;
    uint256 paperHandsLastUpdateTime = 0;
    uint256 paperHandsRewardsPerTokenStored = 0;
    ...
    ****************************************/
    ...
    function _depositFor(
        ...
        newLocked.start = block.timestamp;
        ...
        emit Staked(_addr, _value, newLocked.end, newLocked.start, _action, block.timestamp);
    }
        ...
        updateRewards(msg.sender)
    {
        ...
        require(unlock_time <= block.timestamp + MAXTIME, "Voting lock can be 1 year max (until recol)");
        ...
        updateRewards(_addr) 
    {
        ...
        if (block.timestamp < oldLock.end) { 
            uint256 _paperHandsFee = ((oldLock.end - block.timestamp) * 100 / (oldLock.end - oldLock.start) / 2 * value) / 100;
            value = value - _paperHandsFee;
            _notifyPaperHandsAmount(_paperHandsFee);
        }

        LockedBalance memory currentLock = LockedBalance({ end: 0, amount: 0, start: 0 });
    ...
    ****************************************/
    ...
    function _updatePH(address _account)  private { 
        uint256 newPaperHandsRewardPerToken = _rewardPerToken(paperHandsIndex);
        if (newPaperHandsRewardPerToken > 0) {
            paperHandsRewardsPerTokenStored = newPaperHandsRewardPerToken;
            paperHandsLastUpdateTime = lastTimeRewardApplicable(paperHandsIndex);
            // Setting of personal vars based on new globals
            if (_account != address(0)) {
                rewards[paperHandsIndex][_account] = _earned(paperHandsIndex, _account);
                userRewardPerTokenPaid[paperHandsIndex][_account] = newPaperHandsRewardPerToken;
            }
        }
    }
    ...
    {
        if(_tid == paperHandsIndex) {
            _claimPaperHandsRewards(msg.sender);
        } else {
            RewardInfo memory rInfo = RewardInfo({
                current: activeRewardInfo[_tid].current,
                last: activeRewardInfo[_tid].last,
    ...
    function _claimPaperHandsRewards(address addr) private { 
        uint256 paperHandReward = rewards[paperHandsIndex][addr];
        if (paperHandReward > 0) {
            rewards[paperHandsIndex][addr] = 0;
            stakingToken.safeTransfer(addr, paperHandReward);
            rewardsPaid[paperHandsIndex][addr] = rewardsPaid[paperHandsIndex][addr] + paperHandReward;

            emit RewardPaid(addr, address(stakingToken), paperHandReward);
        }
    }
    ...
    ****************************************/
    ...
    {
        if (_tid == paperHandsIndex) {
             return StableMath.min(block.timestamp, paperHandsFinish);
        }
        RewardInfo memory rInfo = RewardInfo({
            current: activeRewardInfo[_tid].current,
            last: activeRewardInfo[_tid].last,
        ...
        returns (uint256) 
    {
        ...
        if (totalStatic == 0) {
             if (_tid == paperHandsIndex) {
                return paperHandsRewardsPerTokenStored;
            }
            return rewardPerTokenStored[rInfo.current];
        }
        if (_tid == paperHandsIndex) { 
            // new reward units to distribute = rewardRate * timeSinceLastUpdate
            uint256 rewardUnitsToDistribute = paperHandsRewardRate *
                (_lastTimeRewardApplicable(paperHandsIndex) - paperHandsLastUpdateTime);
            // new reward units per token = (rewardUnitsToDistribute * 1e18) / totalTokens
            uint256 unitsToDistributePerToken = rewardUnitsToDistribute.divPrecisely(totalStatic);
            // return summed rate
            return paperHandsRewardsPerTokenStored + unitsToDistributePerToken;
        } else { 
            // new reward units to distribute = rewardRate * timeSinceLastUpdate
    ...
    {
        if (_tid == paperHandsIndex)  {
            uint256 userRewardDelta = _rewardPerToken(paperHandsIndex) - userRewardPerTokenPaid[paperHandsIndex][_addr];
            // new reward = staked tokens * difference in rate
            uint256 userNewReward = staticBalanceOf(_addr).mulTruncate(userRewardDelta);
            // add to previous rewards
            return rewards[paperHandsIndex][_addr] + userNewReward;
    ...
    ****************************************/
     ...
     */
    function _notifyPaperHandsAmount(uint256 _rewards)
        private
        contractNotExpired
    {
        uint256 currentTime = block.timestamp;
        // If previous period over, reset rewardRate
        if (currentTime >= paperHandsFinish) {
            paperHandsRewardRate = _rewards / WEEK;
        }
        // If additional reward to existing period, calc sum
        else {
            uint256 remaining = paperHandsFinish - currentTime;
            uint256 leftover = remaining * paperHandsRewardRate;
            paperHandsRewardRate = (_rewards + leftover) / WEEK;
        }

        paperHandsLastUpdateTime = currentTime;
        paperHandsRewardsPerTokenStored = currentTime + WEEK;
        
        address[] memory rewardAddresses = new address[](1);
        rewardAddresses[0] = address(stakingToken);

        uint256[] memory rewardResults = new uint256[](1);
        rewardResults[0] = _rewards;
        emit RewardsAdded(rewardResults, rewardAddresses);
    }
