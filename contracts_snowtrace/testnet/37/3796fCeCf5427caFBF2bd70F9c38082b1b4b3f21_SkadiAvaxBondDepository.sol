...
interface IWAVAX9 is IERC20 {
    /// @notice Deposit avax to get wrapped avax
    function deposit() external payable;
}

contract SkadiAvaxBondDepository is Ownable {
    ...
    /* ======== STATE VARIABLES ======== */
    address public immutable SKI; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints SKI when receives principle
    ...
    struct Bond {
        uint payout; // SKI remaining to be paid
    ...
    function deposit( 
        ...
        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 SKI ( underflow protection )
        ...
        if (address(this).balance >= _amount) {
            // pay with WAVAX9
            IWAVAX9(principle).deposit{value: _amount}(); // wrap only what is needed to pay
            IWAVAX9(principle).transfer(treasury, _amount);
        } else {
        ...
        refundAVAX(); //refund user if needed
        return payout; 
    }
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            IERC20( SKI ).transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                IERC20( SKI ).approve( stakingHelper, _amount );
                IStakingHelper( stakingHelper ).stake( _amount, _recipient );
            } else {
                IERC20( SKI ).approve( staking, _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return IERC20( SKI ).totalSupply().mul( terms.maxPayout ).div( 100000 );
    }
    ...
    /**
     *  @notice calculate current ratio of debt to SKI supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns ( uint debtRatio_ ) {   
        uint supply = IERC20( SKI ).totalSupply();
    ...
    function standardizedDebtRatio() external view returns ( uint ) {
        return debtRatio().mul( uint( assetPrice() ) ).div( 1e8 ); // AVAX feed is 8 decimals
    }
    ...
    /**
     *  @notice calculate amount of SKI available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or SKI) to the DAO
     *  @return bool
     */
    ...
    function refundAVAX() internal {
        if (address(this).balance > 0) safeTransferAVAX(DAO, address(this).balance);
    }

    /// @notice Transfers AVAX to the recipient address
    /// @dev Fails with `STE`
