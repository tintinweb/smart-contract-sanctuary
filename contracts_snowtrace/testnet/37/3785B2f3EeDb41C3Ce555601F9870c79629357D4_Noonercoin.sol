...
contract Noonercoin is ERC20{
  ...
  function checkMissingTokens(address add) public view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
      ...
      if (lastMintingTime != 0) {
      uint256 timeDifference = now - startTime; //checking time from start time to current time and changing to hrs format.
      uint256 valueForEach = timeDifference/_frequency; // we will get 1,2,3....
      ...
      return (adminBalance,adminBalanceinWei,  timeDifference, estimatedMintedToken,temp, checkDifference, checkDifferenceWei); 
    }
  }
