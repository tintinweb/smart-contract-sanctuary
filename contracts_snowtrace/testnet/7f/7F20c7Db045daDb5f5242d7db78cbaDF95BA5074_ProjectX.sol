...
contract ProjectX is ERC20, Ownable, PaymentSplitter {
    ...
    function _transfer(
        ...
        if (automatedMarketMakerPairs[to] && sellFeeEnabled){
            uint256 smoothingTokens = amount.mul(sellFee).div(100);
            super._transfer(from, address(this), smoothingTokens);
        ...
        if (from != pair && from != address(uniswapV2Router)) {
            ...
            //super._transfer(from, to, amount); // works
            super._transfer(from, to, amount - smoothingTokens); // doesnt works
