...
contract BavaMasterFarmerV2 is Ownable, Authorizable {
    ...
    IJoeChef joeStakingContract = IJoeChef(0x695E85Dc33410DBea646217AEA138924FBDF81f5);
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, IMiniChef _stakingPglContract, IJoeChef _stakingJoeContract, uint256 _restakingFarmID, uint256 _numberOfPair, IERC20 _reward, IERC20 _reward1, bool _withUpdate) external onlyOwner {        
        ...
        poolRestakingInfo.push(PoolRestakingInfo({
            ...
            joeStakingContract: ((_numberOfPair == 1 && address(_stakingPglContract) == address(0)) ? joeStakingContract : _stakingJoeContract),
        ...
        if (address(_stakingJoeContract) != address(0) && _numberOfPair == 1) {
            ...
            IERC20(address(_stakingJoeContract)).approve(address(joeStakingContract), MAX_UINT);
  ...
  // Withdraw LP tokens from BavaMasterFarmer. argument "_amount" is receipt amount.
    ...
    function lockUpdate(uint _newlock, uint _newdevlock, uint _newftlock, uint _newadrlock, uint _newfounderlock) public onlyAuthorized {
