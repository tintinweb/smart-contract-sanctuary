...
contract sEldaERC20Token is IsELDA, ERC20, ERC20Burnable, ERC20Permit {
    /* ========== DEPENDENCIES ========== */
    ...
    IgELDA public gELDA; // additional staked supply (governance token)
    ...
    constructor() ERC20("Staked ELDA", "sELDA") ERC20Permit("Staked ELDA") {
    ...
    function setgELDA(address _gELDA) external {
        require(msg.sender == initializer, "Initializer:  caller is not initializer");
        require(address(gELDA) == address(0), "gELDA:  gELDA already set");
        require(_gELDA != address(0), "gELDA:  gELDA is not a valid contract");
        gELDA = IgELDA(_gELDA);
    }
    ...
    /**
        @notice increases rELDA supply to increase staking balances relative to profit_
    ...
    // this function is called by the treasury, and informs sELDA of changes to debt.
    // note that addresses with debt balances cannot transfer collateralized sELDA
    ...
    function changeDebt(
        ...
        require(debtBalances[debtor] <= balanceOf(debtor), "sELDA: insufficient balance");
    }
    ...
    // toG converts an sELDA balance to gELDA terms.
    function toG(uint256 amount) external view override returns (uint256) {
        return gELDA.balanceTo(amount);
    }

    // fromG converts a gELDA balance to sELDA terms.
    function fromG(uint256 amount) external view override returns (uint256) {
        return gELDA.balanceFrom(amount);
    }

    // Staking contract holds excess sELDA
    function circulatingSupply() public view override returns (uint256) {
        return
            _totalSupply
                .sub(balanceOf(stakingContract))
                .add(gELDA.balanceFrom(IERC20(address(gELDA)).totalSupply()))
