...
contract Tirrek is Context, IERC20, Ownable {
    ...
    uint256 public _Total_Fees;

    // Track original fees to bypass fees for charity account
    uint256 private ORIG_TAX_FEE;
    uint256 private ORIG_BURN_FEE;
    uint256 private ORIG_Total_Fees;
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tFee, uint256 tBurn, uint256 tCharity) = _getTBasics(tAmount, _TAX_FEE, _BURN_FEE, _Total_Fees);
    ...
    function removeAllFee() private {
        if(_TAX_FEE == 0 && _BURN_FEE == 0 && _Total_Fees == 0) return;
