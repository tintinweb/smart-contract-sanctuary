...
contract AmpERC20 is ERC20Permit, Ownable {
    ...
    mapping (address => bool ) _AmpAllowed;
    ...
    constructor() ERC20("Asgardian Promissory", "AMP", 9) ERC20Permit() {
        initializer = msg.sender;
        _totalSupply = INITIAL_FRAGMENTS_SUPPLY;
        _gonsPerFragment = TOTAL_GONS.div(_totalSupply);
    }

    function initialize( address stakingContract_, address stakingHelper_, address stakingWarmup_ ) external returns ( bool ) {
        require( msg.sender == initializer, "NA" );
        require( stakingContract_ != address(0), "IA" );
        stakingWarmup = stakingWarmup_;
    ...
    /**
        @notice increases AMPere supply to increase staking balances relative to profit_
    ...
    // Staking contract holds excess AMPere
    ...
    function transfer( address to, uint256 value ) public override returns (bool) {
        ...
        if(transferLocked){
            require(msg.sender == owner || msg.sender == stakingContract || msg.sender == stakingHelper || msg.sender == stakingWarmup || _AmpAllowed[msg.sender], "AMP transfer not permitted" );
        }
    ...
    function addAmpAllowed(address approved_ ) external onlyOwner() {
        _AmpAllowed[approved_] = true;
    }

    function removeAmpAllowed(address remove_ ) external onlyOwner() {
        _AmpAllowed[remove_] = false;
    }

    function isAmpAllowed(address test_) external view returns(bool) {
        return _AmpAllowed[test_];
    }

    function setTranferLocked(bool transferLock_) external onlyOwner() {
        transferLocked = transferLock_;
    }
