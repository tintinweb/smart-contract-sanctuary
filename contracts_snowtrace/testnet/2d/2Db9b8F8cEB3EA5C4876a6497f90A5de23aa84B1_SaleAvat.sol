...
contract SaleAvat is Ownable, ReentrancyGuard {
    ...
    mapping(address => IERC20) public _tokens;

    mapping(uint256 => DistributionPeriod) private _distributionPeriods;

    mapping(address => mapping(uint256 => DistributionAddress)) private _distributionAddresses;

    constructor ( address[] memory tokens_)
    {
        uint256 tokensLength = tokens_.length;
        for(uint i = 0 ; i < tokensLength; i++) {
            _tokens[tokens_[i]] = IERC20(tokens_[i]);
        }
    }

    /**
     * @dev Public function. Creates distribution period
     */
    function createDistributionPeriod(DistributionPeriod[] memory distributionPeriods_, uint precision_)
    public
    onlyOwner
    {
        uint _precision = precision_;
        ...
        for(uint i = 0 ; i < periodsLength; i++) {
            ...
            _distributionPeriods[i] = distributionPeriods_[i];
        }

        require(amountPercent.div(_precision) == 100, "Total percentage should be 100%");
    }
