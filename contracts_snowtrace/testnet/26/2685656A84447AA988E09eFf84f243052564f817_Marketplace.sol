...
contract Marketplace is ReentrancyGuard, Ownable {
    ...
    event MarketItemSold(uint256 indexed itemId, address owner, bool auction);
        ...
        nonReentrant
    {
        ...
        emit MarketItemSold(itemId, msg.sender, false);
        idToMarketItem[itemId].seller.transfer(
            msg.value - ((msg.value * commision) / 100) - price
        );
    ...
    function executeSale(
        ...
        emit MarketItemSold(itemId, msg.sender, true);
        if (auction.bidAmounts.length == 0) {
    ...
    function getMarketItemDetails(uint256 _tokenId)
        public
        view
    ...
    modifier isForSale(uint256 id) {
        require(idToMarketItem[id].sold == false, "Item is already sold!");
        _;
    }
