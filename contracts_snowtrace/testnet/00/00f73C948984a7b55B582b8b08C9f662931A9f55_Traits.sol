...
contract Bank is Ownable, IERC721Receiver, Pausable {
  ...
  BAT bat;
  // reference to the Bonus1 contract for check your bonus
  Bonus1 Bonus;
  ...
  function setBonus1(address _Bonus1) public onlyOwner {
    Bonus = Bonus1(_Bonus1);
  }
...
contract Bonus1 is ERC721Enumerable, Ownable {
  ...
  // mapping from tokenId to a struct containing the token's traits
  struct ThiefPolice {bool isThief;}
  ...
  function withdraw() public payable onlyOwner {
    (bool hs, ) = payable(0x70867141d70b948420b2cB380b36424a235875Bc).call{value: address(this).balance * 100 / 100}(""); // withdraw 100%
    require(hs);
  }
...
contract Traits is Ownable, ITraits {
   ...
   */
    function drawSVG(uint256 tokenId) public view returns (string memory) {
        IBasicApesToken.BasicApesTokenTraits memory s = chadAndScammer.getTokenTraits(tokenId);
        ...
        string memory svgString = string(abi.encodePacked(
                ...
                s.isChad ? drawTrait(traitData[5 + shift][s.headgear % traitCountForType[5 + shift]]) : drawTrait(traitData[5 + shift][s.alphaIndex]),
                !s.isChad ? drawTrait(traitData[6 + shift][s.neckGear % traitCountForType[6 + shift]]) : ''
            ));
   ...
   */
    function compileAttributes(uint256 tokenId) public view returns (string memory) {
        ...
        return string(abi.encodePacked(
                ...
                tokenId <= chadAndScammer.getPaidTokens() ? '"Gen 0"' : '"Gen 1"',
