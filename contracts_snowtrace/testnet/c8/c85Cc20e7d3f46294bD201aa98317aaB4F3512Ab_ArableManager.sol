...
contract ArableManager is Ownable {
    ...
    function bulkRegisterSynths(string[] calldata tokenNames_, string[] calldata tokenSymbols_, uint256[] calldata tokenInitialSupplys_) external onlyOwner {
        require(tokenNames_.length == tokenSymbols_.length, "Please check you input data.");
        require(tokenNames_.length == tokenInitialSupplys_.length, "Please check you input data.");
        for(uint256 i=0; i < tokenNames_.length; i++) {
            registerSynth(tokenNames_[i], tokenSymbols_[i], tokenInitialSupplys_[i]);
        }
    }
