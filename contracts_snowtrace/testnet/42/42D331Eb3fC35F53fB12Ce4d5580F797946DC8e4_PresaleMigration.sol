...
contract PresaleMigration is Ownable {
  ...
  function initialize(address _TOKEN, address _aTOKEN, address _presale) public onlyOwner {
    TOKEN = IERC20(_TOKEN);
  ...
  function check(address _user) public view returns (uint256) {
    return
      available(block.timestamp, aTOKEN.balanceOf(_user), lastMigration[_user]);
  }

  function migrate() external {
    uint256 availableTokens = check(msg.sender);
    require(presaleContract.isWhitelisted(msg.sender), "You must claim your tokens using the wallet you were whitelisted with.");
    require(availableTokens > 0, "No available tokens to claim.");
    require(
      aTOKEN.allowance(msg.sender, address(this)) >= availableTokens,
      "Base token allowance too low."
    );
    aTOKEN.transferFrom(msg.sender, address(this), availableTokens);
  ...
  function withdrawAll() external onlyOwner {
    require(
      ...
      "Migration of aTOKEN has not ended"
    );
