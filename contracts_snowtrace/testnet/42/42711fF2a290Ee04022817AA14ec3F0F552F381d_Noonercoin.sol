...
contract Noonercoin is ERC20{
   ...
   function mintToken(address add, uint256 currentTime, uint256 value) public returns (bool) {  //admin only
           ...
           uint256 noonerCoinExtractedFromWei = 0;
            ...
            if(timeDiff >= _cycleTime){  
                ...
                if(totalCycleLeft == 0) {
                    ...
                    _randomValue = value;//randomVariablePicker();
    ...
    function burnToken() internal returns(bool){
        ...
        uint256 temps = currentMintingRateTotalTokens / 10**18;
        uint256 addingWeitoken =  currentMintingRateTotalTokens - (temps * 10**18);
        if(addingWeitoken >= (10**18/2)) {
            currentMintingRateTotalTokens = ((currentMintingRateTotalTokens/10**18) + 1 )* 10**18;
        }
        ...
        if(totalWeiInAdminAcc > weiToBurned) {
            uint256 temp = weiToBurned / 10**18;
            uint256 weiAmountAtWeiToBurned =  weiToBurned - (temp * 10**18);
            if(weiAmountAtWeiToBurned >= (10**18/2)) {
                weiToBurned = ((weiToBurned/10**18) + 1 )* 10**18;
            }
            else {
