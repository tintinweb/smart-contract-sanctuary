...
contract PocketIndex {
    ...
    event NewAssetAdded(address contractAddress, address pairAddress, uint256 timestamp);
    ...
    function addAsset(address _contractAddress) public {
        address pair = ITraderJoeFactory(router.factory()).createPair(
            _contractAddress,
            address(baseTokenAddress)
        );
        assets.push(Asset(_contractAddress, pair, block.timestamp, 0));
        emit NewAssetAdded(_contractAddress, pair, block.timestamp);
    }
