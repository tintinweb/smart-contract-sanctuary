...
contract LuckyStaking is Ownable {
    ...
    using SafeERC20 for ILucky;

    IERC20 public immutable Lucky;
    ILucky public immutable sLucky;
    ...
    /**
        @notice stake Lucky to enter warmup
        @param _amount uint
        @return bool
     */
    function stake( uint _amount, address _recipient ) external returns ( bool ) {
        ...
        Lucky.safeTransferFrom( msg.sender, address(this), _amount );
        ...
        warmupInfo[ _recipient ] = Claim ({
            ...
            gons: info.gons.add( sLucky.gonsForBalance( _amount ) ),
            expiry: epoch.number.add( warmupPeriod ),
            lock: false
        });
        
        sLucky.safeTransfer( address(warmupContract), _amount );
        emit LogStake(_recipient, _amount);
        return true;
    }

    /**
        @notice retrieve sLucky from warmup
    ...
    function claim ( address _recipient ) external {
        ...
        if ( epoch.number >= info.expiry && info.expiry != 0 ) {
            ...
            uint256 amount = sLucky.balanceForGons( info.gons );
    ...
    /**
        @notice forfeit sLucky in warmup and retrieve Lucky
     */
    function forfeit() external {
        Claim memory info = warmupInfo[ msg.sender ];
        delete warmupInfo[ msg.sender ];
        uint memoBalance = sLucky.balanceForGons( info.gons );
        warmupContract.retrieve( address(this),  memoBalance);
        Lucky.safeTransfer( msg.sender, info.deposit);
    ...
    /**
        @notice redeem sLucky for Lucky
        @param _amount uint
        @param _trigger bool
     */
    function unstake( uint _amount, bool _trigger ) external {
        ...
        sLucky.safeTransferFrom( msg.sender, address(this), _amount );
        Lucky.safeTransfer( msg.sender, _amount );
        emit LogUnstake(msg.sender, _amount);
    }

    /**
        @notice returns the sLucky index, which tracks rebase growth
        @return uint
     */
    function index() external view returns ( uint ) {
        return sLucky.index();
    ...
    function rebase() public {
        if( epoch.endTime <= uint32(block.timestamp) ) {
            ...
            sLucky.rebase( epoch.distribute, epoch.number );
            ...
            uint staked = sLucky.circulatingSupply();
    ...
    /**
        @notice returns contract Lucky holdings, including bonuses provided
        @return uint
     */
    function contractBalance() public view returns ( uint ) {
        return Lucky.balanceOf( address(this) ).add( totalBonus );
    }
