...
contract GenTestRoyalty is  ERC721URIStorage, ERC721Enumerable, Ownable, IERC2981 { 
    uint256  gen_c; 
    ...
    function generate () public payable {
        ...
        tokenIdToMinter[gen_c]=msg.sender;
        gen_c = gen_c + 1 ;
        r = uint256(keccak256(abi.encode(msg.sender, 777, block.timestamp)));  
    }

    function set_royalty(uint256 _percentage) public onlyOwner {
        royalty_percentage = _percentage ;
    }
    function royaltyInfo(uint256 _tokenId, uint256 _salePrice) external view returns(address receiver, uint256 royaltyAmount){
        uint256 royaltyFee = _salePrice * royalty_percentage / 100 ;
        address minter = tokenIdToMinter[_tokenId];
        return (minter, royaltyFee);
    }
    function info(uint256 _tokenId, uint256 _salePrice) public view returns(address receiver, uint256 royaltyAmount){
        uint256 royaltyFee = _salePrice * royalty_percentage / 100 ;
        address minter = tokenIdToMinter[_tokenId];
        return (minter, royaltyFee);
    }
