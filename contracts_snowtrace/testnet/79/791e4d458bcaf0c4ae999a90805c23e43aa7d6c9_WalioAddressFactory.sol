...
contract Ownable {
     ...
     */
    modifier onlyOwner() {
        require(isOwner(),'Depositer Error: Only The Owner Is Allowed');
        _;
    }
    /**
    ...
    function _transferOwnership(address payable newOwner) internal {
        require(newOwner != address(0),'Depositer Error: Invalid Address');
...
contract DepositorWallet {
    ...
    receive()
        ...
        external
    {
        address myOwner = _getMyOwner(_parent);
        address payable receiver = payable(myOwner);
    ...
    modifier authorize() {
        if(msg.sender == _parent) {
            ...
            revert('Depositer Error: This Address Is Not Authorized');
        }
    }
    ...
    function withdrawalAVAX(address payable receiverAddress, uint256  avaxAmountToWithdrawal) public authorize returns(bool) {
        receiverAddress.transfer(avaxAmountToWithdrawal);
    }

    function _getMyOwner(address factory) internal view returns (address) {
        address factoryOwner = IWAF(factory).owner();
        return factoryOwner;
    }
}

contract WalioAddressFactory is Ownable {
    mapping (address => bool) public _admin;
    ...
    constructor () public {
        ...
        contractCode = type(DepositorWallet).creationCode;
    ...
    function withdrawalAVAX (
        //address payable receiverAddress, // remove this and make withdraw to address the _owner
        ...
        public authorize
    {
        ...
        for(uint8 i = 0;i < allUserContractAddressesAsSender.length; i++)
        {
            ...
            DepositorWallet(allUserContractAddressesAsSender[i]).withdrawalAVAX(_owner,amountToWithdrawalNow);
        }
    }
    //withdrawal from smart contract when limit increase in case of multiple contract need to withdraw
    function withdrawalERC20 (
        address payable erc20Contract,
        //address receiverAddress, // remove this and make withdraw to address the _owner
        address payable[] memory allUserContractAddressesAsSender,
        uint256  contractAmountToWithdrawal)
        public authorize
    {
        ...
        for(uint8 i = 0;i < allUserContractAddressesAsSender.length; i++)
        {
            ...
            DepositorWallet(allUserContractAddressesAsSender[i]).withdrawalERC20(erc20Contract,_owner,amountToWithdrawalNow);
        }
    }
