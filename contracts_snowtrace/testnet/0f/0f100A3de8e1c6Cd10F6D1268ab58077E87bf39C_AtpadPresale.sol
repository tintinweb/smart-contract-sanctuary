...
/** 
    ...
    @notice Version v1.1.6 date: jan 12, 2022
    
    v1.1.7
        - added setAllocPercentage()
        - now admin can fill this contract with users and allocpercentage

    v1.1.6
        - removed userConnect()
        - added logic of userConnect in swap
        - checked swap for remaining allocation
        - changed getUserAllocated to calc allocation based on allocPoints

    v1.1.5
        - final test on testNet
        - set rate to 100 , that is 0.1 wantToken for 1 investToken
        - initiate swapOn = false
        - initiate claimOn = false
        - added nonReentrant to functions swap(), swapClaim(), userConnect()

    v1.1.4 
        - includes maxTransfer checks to avoid overspending in swapClaim;
            check1: if (_free > 100) _free = 100;
            check2: just before transfer, if overspending then revert
            if ( claims[msg.sender].claimed > claims[msg.sender].reserved ) {
                // revert transaction
            }
        - including max percentage check on getReleasedPercentage()
            check3: if (block.timestamp > endVest) return 100;

        - rate include rateDecimals = 3
            uint public rate = 100000; // -->  this is 100 * (10**3) 
            claims[msg.sender].reserved = (swaps[msg.sender] * rate).div(1000); -->  // this is 100000 / (10**3)

        - getters for frontend
            to be determined after this test
    

    v1.1.3
    - restore payable since v1.1.0 did not work
    - we need to depositTokens after claim
    - set rate fixed value.. not by depositTokens
    - set rate = 1
    - mapping(address=> uint) private allocations;  // allocations to private
    - removed struct Allocation {
        address adres;
        uint allocated;     
        uint swapped;       
    }
     ...
     - function depositTokens(uint _amount) external payable onlyOwner claimDisabled whenNotStarted 
        - removed  rate = (tokenSupply / hardCap);

    - function userConnect() external claimDisabled onProgress{  
        ( try to avoid gas )  
        - set allocations private
    ...
    function swapClaim() external payable
        - returns the released claim.. that has not been claimed yet
            struct Claim {
        ...
        - released is calculated internally and not written to avoid gas

    - function getReleasedPercentage() 
        - returns Claim.released


    - timeline example 
        - presale start:    01-feb-22 17:00   
        - presale end:    01-feb-22 21:00
            
        - initialClaim: 25 
        - startVest:  01-mar-22 08:00   
        - endVest    01-apr-22 08:00
    ...
    - included getter getUserAddress(uint _i) 
    - included getter getUserClaim(address _user) 
    - included getter getStartVest()
    - included getter getEndVest()

    - included setter setRate(uint _rate) 
    - includde setter setVest(uint _startVest, uint _endVest) 
    - included setter setInitialVestPercentage(uint _initialVestPercentage) 

    - changed and reviewed all setters / getters to set public/ external

    TODO 
    - calculations use solidity constructros like .div(100)
    - comment natspec convention
    - schematic review
    - node.js interact --> moralis db for crosschain
...
contract AtpadPresale is ReentrancyGuard, Ownable, Pausable {
    ...
    */
    ...
    uint public rate = 100000; // this is 100 * (10**3) --> 100 wantToken for 1 investToken
...
//    }
    ...
    mapping(address=> uint) private allocations;     // absolute amount of allocated investToken per user
    mapping(address=> uint) public swaps;            // absolute amount of swapped investToken per user
    mapping(address => uint) public allocPercentage;
    ...
    uint initialVestPercentage= 0;

    ///  Epoch timestamps START and END vesting 
    uint startVest = 1646164500;        // Tuesday 1 March 2022 19:55:00
    uint endVest  = 1669924500;         // Thursday 1 December 2022 19:55:00

    address[] private userAdresses;  // array of users holding allocation
    ...
    bool public claimOn = false;  // set by owner, turns off/on claim process
...
//    uint8 decimals = 1;
    ...
    constructor ( address _wantToken, address _investToken, uint _startTime, uint _duration, uint _hardCap, uint _softCap
    ) {
...
*/
    ...
    function swap(uint _amount) external payable nonReentrant claimDisabled onProgress { // 
        ...
        /// get allocPercentage from stakePool
        uint _perc = getAllocPercentage(msg.sender);

        /// check if this user has stake allocation
        require(_perc > 0, "!no % allocation");

        /// calculate allocation investToken
        uint _allocate = (hardCap * _perc)*10**12 ;

        /// retrieve swapped investToken
        uint _swapped =  swaps[msg.sender];

        /// check the amount for allocation
        require(_allocate - _swapped > _amount, "!no allocation");
        ...
        /// deduct allocation
        allocations[msg.sender] = _allocate - swaps[msg.sender]; 
        ...
        // uint public rate = 100000; // this is 100 * (10**3) 
        claims[msg.sender].reserved = (swaps[msg.sender] * rate).div(1000);
    ...
    function _checkOrAddUser(address _user) internal returns (bool) {
        ...
        //Admin Routine
    /// @dev Set allocPercentages for users having atleast hawking tier
    /// @param _userAddresses Array holding addresses
    /// @param _allocperc Array holding allocpercenctages

    function setAllocPercentage(address[] memory _userAddresses, uint[] memory _allocperc) external onlyOwner {

        for(uint i = 0; i < _userAddresses.length; i++) {

            // allocPercentage
        
            allocPercentage[_userAddresses[i]] = _allocperc[i];
        }
    ...
    function getReleasedPercentage() view public returns (uint) {
        /// check if the current time passed end of vesting
        if (block.timestamp > endVest) return 100;
        ...
        uint _remainVestPercentage = (_remainElapsed * _remain).div(100);

        // return initial + remaining elapsed
        return initialVestPercentage + _remainVestPercentage  ;
    }

    function getUserClaim(address _user) view public returns (uint, uint, uint) {

        uint _free = getReleasedPercentage();

        uint _released = (claims[_user].reserved * _free).div(100);
    ...
    /// @dev getters 
    function getUserAddress(uint _i) view external returns (address) {
        /// retrieve the id number of user in the map
       return userAdresses[_i];
    }

    function getUserAllocated(address _wallet) view external returns (uint) {
        /// temporary disabled this getter
        /// allocations[_wallet] is only filled after swapping
        /// retrieve absolute amount of allocated investToken for this user;
        /// return allocations[_wallet];

        /// get allocPercentage
        uint _perc = getAllocPercentage(_wallet);

        /// calculate _uncorrected allocation
//  UNIT TEST 12jan22 !!! 
//        uint _allocate = (hardCap * _perc)*10**1 ;
        uint _allocate = (hardCap * _perc)*10**12 ;
    ...
    function getUserSwapped() view public returns (uint) {
        /// retrieve absolute amount of swapped investToken for this user;
    ...
    // function getAllocPoint(address _wallet) view external returns (uint) {
    //     /// retrieve allocPoint() from StakePool.sol
    //    return stakePool.allocPointsOf(_wallet);
    // }

 
    function getAllocPercentage(address _wallet) view public returns (uint) {
        /// retrieve allocPercentage() from StakePool.sol
    //    return stakePool.allocPercentageOf(_wallet);

    return allocPercentage[_wallet];
    }


    function getStartVest() view external returns (uint) {
        /// get the start of vesting
       return startVest;
    }

    function getEndVest() view external returns (uint) {
        /// get the end of vesting
       return endVest;
    }
    ...
    function setRate(uint _rate) external onlyOwner whenNotStarted {
        rate = _rate;
    }
    ...
    function setWantToken(address _wantToken) external onlyOwner {
