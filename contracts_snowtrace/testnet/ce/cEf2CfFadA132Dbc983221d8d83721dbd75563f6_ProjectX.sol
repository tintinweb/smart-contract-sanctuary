...
contract ProjectX is ERC20, Ownable, PaymentSplitter {
    ...
    function _transfer(
        ...
        bool sellingOrAddingLiquidity = (msg.sender == address(uniswapV2Router) && to == address(uniswapV2Pair));
        bool removingLiquidity = (msg.sender == address(uniswapV2Router) && from == address(uniswapV2Router));
        bool buyingToken = msg.sender == address(uniswapV2Pair) && from == address(uniswapV2Pair);
        ...
        if (swapAmountOk &&
            ...
            if (from == uniswapV2Pair) {
            ...
            else if (to == uniswapV2Pair && sellFeeEnabled) {




                bool takeFee = !swapping && sellFeeEnabled;
                ...
                if (takeFee) {
                    uint256 smoothingTokens = amount.mul(sellFee).div(100);
                    amount = amount.sub(smoothingTokens);
                    super._transfer(address(this), smoothingReserveAddress, smoothingTokens);
                }
