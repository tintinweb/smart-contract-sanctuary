...
contract NODERewardManagement {
    ...
    function getNodeInfo(
        ...
        bool found = false;
        int256 index = binary_search(_nodesOfUser[account], 0, _nodesOfUser[account].length, _creationTime);
        uint256 validIndex;
        if (index >= 0) {
            found = true;
    ...
    function _getNodeIndexWithCreatime(
    ...
    external onlyManager
    {
        require(isNodeOwner(account), "GET REWARD OF: NO NODE OWNER");
        require(_creationTime > 0, "NODE: CREATIME must be higher than zero");
        uint256 nodeIndex = _getNodeIndexWithCreatime(_nodesOfUser[account], _creationTime);
