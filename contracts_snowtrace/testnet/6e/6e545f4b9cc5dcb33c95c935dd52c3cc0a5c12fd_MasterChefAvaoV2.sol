...
contract AvaOne is ERC20("AvaOne", "AVAO"), Ownable {
    ...
    uint256 public maxSupply = 100_000_000e18; // 100 million Avao
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "AVAO::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "AVAO::delegateBySig: invalid nonce"
        );
        require(now <= expiry, "AVAO::delegateBySig: signature expired");
        ...
        returns (uint256)
    {
        require(
            ...
            "AVAO::getPriorVotes: not yet determined"
        );
    ...
    function _delegate(address delegator, address delegatee) internal {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying AVAOs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            "AVAO::_writeCheckpoint: block number exceeds 32 bits"
        );
...
interface IRewarder {
    ...
    function onAvaoReward(address user, uint256 newLpAmount) external;
...
// MasterChefAvao is a boss. He says "go f your blocks lego boy, I'm gonna use timestamp instead".
...
// will be transferred to a governance smart contract once AVAO is sufficiently
...
contract MasterChefAvaoV2 is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of AVAOs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accAvaoPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accAvaoPerShare` (and `lastRewardTimestamp`) gets updated.
    ...
    // The timestamp when AVAO mining starts.
    ...
    // Update the given pool's AVAO allocation point. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        IRewarder _rewarder,
    ...
    // View function to see pending AVAOs on frontend.
        ...
        )
    {
        ...
        if (block.timestamp > pool.lastRewardTimestamp && lpSupply != 0) {
            ...
            accAvaoPerShare = accAvaoPerShare.add(
                avaoReward.mul(1e12).div(lpSupply)
            );
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accAvaoPerShare = pool.accAvaoPerShare.add(
            avaoReward.mul(1e12).div(lpSupply).mul(lpPercent).div(1000)
        );
        pool.lastRewardTimestamp = block.timestamp;
        emit UpdatePool(
            _pid,
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (address(rewarder) != address(0)) {
            rewarder.onAvaoReward(msg.sender, user.amount);
        }
    ...
    function safeAvaoTransfer(address _to, uint256 _amount) internal {
        uint256 avaoBal = avao.balanceOf(address(this));
        if (_amount > avaoBal) {
            avao.transfer(_to, avaoBal);
        } else {
            avao.transfer(_to, _amount);
        }
    }
