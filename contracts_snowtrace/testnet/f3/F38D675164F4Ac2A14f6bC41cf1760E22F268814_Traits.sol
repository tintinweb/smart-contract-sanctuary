...
contract Traits is Ownable, ITraits {
    ...
    function setGame(address _hunter) external onlyOwner {
        hunter = IHunter(_hunter);
    }

    function uploadBodies(string calldata _hunter, string calldata _adventurer)
        external
        onlyOwner
    {
        hunterBody = _hunter;
        adventurerBody = _adventurer;
    }
    ...
    function drawSVG(uint256 tokenId) public view returns (string memory) {
        IHunter.AvtHtr memory s = hunter.getTokenTraits(tokenId);
        ...
        return
            string(
                abi.encodePacked(
                    ...
                    tokenId <= hunter.getPaidTokens() ? '"Gen 0"' : '"Gen 1"',
                    '},{"trait_type":"Type","value":',
                    s.isAdventurer ? '"Adventurer"' : '"Hunter"',
                    "}]"
                )
            );
    }
        ...
        returns (string memory)
    {
        ...
        string memory metadata = string(
            abi.encodePacked(
                ...
                s.isAdventurer ? "Adventurer #" : "Hunter #",
                tokenId.toString(),
                '", "description": "Thousands of Adventurers and Hunters compete on a forest in the metaverse. A tempting prize of $GEM awaits, with deadly high stakes. All the metadata and images are generated and stored 100% on-chain. No IPFS. NO API. Just the Avalanche blockchain.", "image": "data:image/svg+xml;base64,',
