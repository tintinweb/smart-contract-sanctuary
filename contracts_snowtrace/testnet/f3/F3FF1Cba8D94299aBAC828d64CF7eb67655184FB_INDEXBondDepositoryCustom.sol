...
contract INDEXBondDepositoryCustom is Ownable {
    ...
    /* ======== STATE VARIABLES ======== */
    address public immutable INDEX; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable dexPair; // the DEX pair for the principle
    address public immutable stable; // the stable coin in the DEX pair

    address public immutable treasury; // mints INDEX when receives principle
    ...
    struct Bond {
        uint payout; // INDEX remaining to be paid
    ...
    function deposit(
        ...
        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 INDEX ( underflow protection )
        ...
        if ( fee != 0 ) { // fee is transferred to dao
            IERC20( INDEX ).safeTransfer( DAO, fee );
        }
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            IERC20( INDEX ).transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                IERC20( INDEX ).approve( stakingHelper, _amount );
                IStakingHelper( stakingHelper ).stake( _amount, _recipient );
            } else {
                IERC20( INDEX ).approve( staking, _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return IERC20( INDEX ).totalSupply().mul( terms.maxPayout ).div( 100000 );
    }
    ...
    /**
     *  @notice get asset price from liquidity pools
     */
    function assetPrice() public view returns (int) {
      uint256 pairStableBalance = IERC20( stable ).balanceOf( dexPair ) / ( 10 ** IERC20( stable ).decimals() );
      uint256 pairPrincipleBalance = IERC20( principle ).balanceOf( dexPair ) / ( 10 ** IERC20( principle ).decimals() );
      uint256 price = pairStableBalance / pairPrincipleBalance;
    ...
    /**
     *  @notice calculate current ratio of debt to INDEX supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns ( uint debtRatio_ ) {
        uint supply = IERC20( INDEX ).totalSupply();
    ...
    function standardizedDebtRatio() external view returns ( uint ) {
        return debtRatio().mul( uint( assetPrice() ) ).div( 1e8 );
    }
    ...
    /**
     *  @notice calculate amount of INDEX available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or INDEX) to the DAO
     *  @return bool
     */
    function recoverLostToken( address _token ) external returns ( bool ) {
        require( _token != INDEX );
