...
contract Enigm is ERC20Permit, VaultOwned {
  using SafeMath for uint256;
  ...
  address public immutable feesWallet =
    0xCe0955588193fD8659f88acaf0d3364Ec2642619;

  // 5000 Tokens
  uint256 public txLimit = 5000000000000;
  mapping(address => bool) public limitWhitelisted;

  bool public _tradingOpen = false;

  // Exempted from fees
  mapping(address => bool) public feesWhitelisted;

  // Bypass trading opening
  mapping(address => bool) public bypassWhitelisted;

  constructor() ERC20("ENIGM", "GMA", 9) {
    limitWhitelisted[address(this)] = true;
    feesWhitelisted[address(this)] = true;
    bypassWhitelisted[address(this)] = true;
  }

  function setFeesWhitelisted(address _user, bool _whitelisted)
    external
    onlyOwner
    returns (bool)
  {
    feesWhitelisted[_user] = _whitelisted;
    return true;
  }

  function setLimitWhitelisted(address _user, bool _whitelisted)
    external
    onlyOwner
    returns (bool)
  {
    limitWhitelisted[_user] = _whitelisted;
    return true;
  }

  function setBypassWhitelisted(address _user, bool _whitelisted)
    external
    onlyOwner
    returns (bool)
  {
    bypassWhitelisted[_user] = _whitelisted;
    return true;
  }

  function setFees(uint256 _fees) external onlyOwner returns (bool) {
    fees = _fees;
    return true;
  }
  ...
  function transferFrom(
    ...
    require(
      bypassWhitelisted[sender] || bypassWhitelisted[recipient] || _tradingOpen,
      "Trading not enabled yet"
    );
    require(
      limitWhitelisted[sender] || amount <= txLimit || txLimit == 0,
      "Transaction amount over limit"
    );
    if (feesWhitelisted[sender] == false) {
      tAmount = amount.mul(fees).div(100);
      _transfer(sender, feesWallet, tAmount);
    ...
    returns (bool)
  {
    uint256 tAmount = amount;
    require(
      bypassWhitelisted[msg.sender] ||
        bypassWhitelisted[recipient] ||
        _tradingOpen,
      "Trading not enabled yet"
    );
    require(
      limitWhitelisted[msg.sender] || amount <= txLimit || txLimit == 0,
      "Transaction amount over limit"
    );
    if (feesWhitelisted[msg.sender] == false) {
      tAmount = amount.mul(fees).div(100);
      _transfer(msg.sender, feesWallet, tAmount);
      tAmount = amount.sub(tAmount);
    }
