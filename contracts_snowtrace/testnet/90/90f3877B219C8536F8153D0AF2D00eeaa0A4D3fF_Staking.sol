...
contract Staking is Ownable {
    ...
    uint256 public minimumDays = 5 minutes;
    uint256 public lockTime = 6 minutes;
    ...
    function withdraw(uint256 _stakeCount) public {
        ...
        if (
            user.stakerecord[_stakeCount].totalWithdrawnBonus + totalBonus >
            user.stakerecord[_stakeCount].bonus
        ) {
            totalBonus = user.stakerecord[_stakeCount].bonus.sub(
                user.stakerecord[_stakeCount].totalWithdrawnBonus
            );
            erc20Token.transfer(msg.sender, totalBonus);
    ...
    function unstake(uint256 _stakeCount) public {
        ...
        erc20Token.transfer(msg.sender, user.stakerecord[_stakeCount].amount);
        ...
        )
    {
        ...
        for (uint256 i = 0; i <= user.stakecount; i++) {
            if (user.stakerecord[i].lastWithdrawTime != 0) {
                uint256 dividends = block.timestamp.sub(
                    user.stakerecord[i].time
                );
                availableToClaim = (
                    (
                ...
                // If exceed claimable balance
                if (
                    user.stakerecord[i].totalWithdrawnBonus + availableToClaim >
                    user.stakerecord[i].bonus
                ) {
                    availableToClaim = user.stakerecord[i].bonus.sub(
                        user.stakerecord[i].totalWithdrawnBonus
                    );
                }
            }
