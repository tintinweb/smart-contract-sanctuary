...
contract ProjectX is ERC20, Ownable, PaymentSplitter {
    ...
    function _transfer(
        ...
        bool sellingOrAddingLiquidity = (msg.sender == address(uniswapV2Router) &&
            to == address(uniswapV2Pair));
        bool removingLiquidity = (msg.sender == address(uniswapV2Router) &&
            from == address(uniswapV2Router));
        bool buyingToken = msg.sender == address(uniswapV2Pair) &&
        ...
        if( swapAmountOk &&
            ...
            uint256 smoothingTokens = toAmount.mul(sellFee).div(100);
            toAmount = toAmount.sub(smoothingTokens);
            //super._transfer(address(this), address(this), smoothingTokens);
        ...
        else if(to == uniswapV2Pair && sellFeeEnabled) // burn 10% for selling
        {
            uint256 smoothingTokens = toAmount.mul(sellFee).div(100);
            toAmount = toAmount.sub(smoothingTokens);
        ...
        */
        ...
        /*
        //sale
        //if (msg.sender == address(uniswapV2Router) && from == address(this) && to == uniswapV2Pair && sellFeeEnabled) {
        //swapping = true;
        if (sellingOrAddingLiquidity && sellFeeEnabled) {
            uint256 smoothingTokens = toAmount.mul(sellFee).div(100);
            toAmount = toAmount.sub(smoothingTokens);
            super._transfer(
                address(this),
