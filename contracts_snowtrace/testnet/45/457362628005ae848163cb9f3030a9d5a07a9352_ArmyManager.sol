...
interface IArmyManager is IERC721 {
    ...
    function claim(address account, uint64 _id) external returns (uint);
...
contract ArmyManager is Ownable, HelperOwnable, IERC721, IERC721Metadata, WeigthOwnable, IArmyManager {
    ...
    mapping(uint64 => Army) private _nodes;
    ...
    function importArmy(address account, string calldata _name, uint64 mint, uint64 _claim) onlyContract override external {
        uint64 nodeId = uint64(nodeCounter);
    ...
    function claim(address account, uint64 _id) external onlyIfExists(_id) onlyContract override returns (uint) {
    ...
    function updateArmy(uint64 id, uint64 weigth, string calldata metadata) onlyWeigth external {
        Army storage army = _nodes[id];
        army.weigth = weigth;
        army.metadata = metadata;
    }

    function getArmy(uint64 _id) public view returns (Army memory) {
        return _nodes[_id];
    }
    ...
    function getNameOf(uint64 _id) public view override onlyIfExists(_id) returns (string memory) {
        return _nodes[_id].name;
    }

    function getMintOf(uint64 _id) public view override onlyIfExists(_id) returns (uint64) {
        return _nodes[_id].mint;
    }

    function getClaimOf(uint64 _id) public view override onlyIfExists(_id) returns (uint64) {
        return _nodes[_id].claim;
    }

    function getArmiesOf(address _account) public view returns (uint64[] memory) {
    ...
    function _addArmy(uint64 _id, string calldata _name, uint64 _mint, uint64 _claim, string calldata _metadata, address _to, uint64 _weigth) onlyOwner external {
        _createArmy(_id, _name, _mint, _claim, _metadata, _to, _weigth);
    }

    function _deleteArmy(uint64 _id) onlyOwner external {
    ...
    function _deleteMultipleArmy(uint64[] calldata _ids) onlyOwner external {
    ...
    function _createArmy(uint64 _id, string memory _name, uint64 _mint, uint64 _claim, string memory _metadata, address _to, uint64 _weigth) internal {
    ...
    function _modifyArmy(uint64 _id, string memory _name, uint64 _mint, uint64 _claim, string memory _metadata, uint64 _weigth) internal {
        _nodes[_id] = Army({
            name: _name,
            mint: _mint,
            claim: _claim,
    ...
    function _remove(uint64 _id, address _account) internal {
