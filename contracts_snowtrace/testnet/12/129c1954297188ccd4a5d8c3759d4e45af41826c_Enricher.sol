...
// File: contracts/operations/interfaces/IPair.sol
...
// File: contracts/operations/interfaces/IFactory.sol
...
// This contract "enriches" the Moneybags contract by transferring a portion
// of the trading fees in the form of GTR tokens.

// T1 - T4: OK
contract Enricher is BoringOwnable {
    ...
    address public immutable moneyBags;
    ...
    // F6: There is an exploit to add lots of GTR to moneyBags, run convert, then remove the GTR again.
    //     As the size of Moneybags has grown, this requires large amounts of funds and isn't super profitable anymore
    ...
    modifier onlyEOA() {
        ...
        require(msg.sender == tx.origin, "Enricher: must use EOA");
        _;
    }
    ...
    function _convert(address token0, address token1) internal {
        ...
        require(address(pair) != address(0), "Enricher: Invalid pair");
        // balanceOf: S1 - S4: OK
        // transfer: X1 - X5: OK
        ...
        // We get the amount0 and amount1 by their respective balance of the Enricher.
    ...
    function _convertStep(
        ...
        if (token0 == token1) {
            ...
            if (token0 == gtr) {
                IERC20(gtr).safeTransfer(moneyBags, amount);
                gtrOut = amount;
            ...
            // eg. GTR - AVAX
            IERC20(gtr).safeTransfer(moneyBags, amount0);
            gtrOut = _toGTR(token1, amount1).add(amount0);
        } else if (token1 == gtr) {
            // eg. USDT - GTR
            IERC20(gtr).safeTransfer(moneyBags, amount1);
    ...
    function _swap(
        ...
        require(address(pair) != address(0), "Enricher: Cannot convert");
    ...
    function _toGTR(address token, uint256 amountIn) internal returns (uint256 amountOut) {
        // X1 - X5: OK
        amountOut = _swap(token, gtr, amountIn, moneyBags);
    }
    ...
    function setBridge(address token, address bridge) external onlyOwner {
        // Checks
        require(token != gtr && token != wavax && token != bridge, "Enricher: Invalid bridge");
