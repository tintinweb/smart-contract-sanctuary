...
contract AlphaENIGMMigration is Ownable {
  ...
  function available(address owner) public view returns (uint256) {
    ...
    for (uint256 i = steps.length - 1; i > 0; i--) {
      ...
      if (lastMigration[owner] < step && timestamp > step) {
        claimable += (startBalance[owner] * percentPerStep[i]) / 100;
      }
    }
    return claimable;
  }

  function check(address owner) public returns (uint256) {
    if (startBalance[owner] == 0) {
      // startBalance[owner] = aENIGM.balanceOf(msg.sender) ;
      startBalance[owner] = 400;
    }
    return available(msg.sender);
  }
