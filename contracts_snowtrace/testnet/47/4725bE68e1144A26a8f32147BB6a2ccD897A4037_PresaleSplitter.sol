...
contract PresaleSplitter is Ownable {
    ...
    uint256 public maxClaimTimes = 24;
        ...
        returns (uint256 claimAmount)
    {
        ...
        uint diff = (endDate - presaleStartDate) / claimInterval;
        //calc if the address has claim before, or has times left to claim
        if (diff > 0 && timesClaimed[msg.sender] != 0) {
            uint calc = (diff - timesClaimed[msg.sender]) == 0 ? 1 : (diff - timesClaimed[msg.sender]) +1;
            totalClaimableValue = calc * claimValueByDay;
