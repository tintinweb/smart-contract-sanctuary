...
interface ICreditERC20 is IERC20Mintable, IERC20 {
...
contract CreditTreasury is Ownable {
    ...
    ICreditERC20 public immutable Credit;
    ...
    /**
        @notice allow approved address to deposit an asset for Credit
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint Credit needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        limitRequirements(msg.sender, send_);
        Credit.mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn Credit for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        Credit.burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = sCredit.balanceOf( msg.sender ); // Can only borrow against sOHM held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with Credit
        @param _amount uint
     */
    function repayDebtWithCredit( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved as debtor" );
        require( isReserveSpender[ msg.sender ], "Not approved as spender" );

        Credit.burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, address(Credit), _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        Credit.mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( Credit.totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns Credit valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match Credit decimals
            value_ = _amount.mul( 10 ** Credit.decimals() ).div( 10 ** IERC20( _token ).decimals() );
