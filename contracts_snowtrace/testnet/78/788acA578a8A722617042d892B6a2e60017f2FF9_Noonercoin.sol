...
contract Noonercoin is ERC20{
   ...
   uint256 private _cycleTime = 72576000; //given one day sec
   ...
   function mintToken(address add) public returns (bool) {  //admin only
    ...
    function burnToken() internal returns(bool){
        ...
        for(uint256 index=1;index<=totalCycleLeft;index++){
            uint256 intValue = getIntegerValue(flag * 604800, 150**index, index);//604800
            uint256 intDecimalValue = getDecimalValue(flag * 604800, 150**index, index);//604800
        ...
        uint256 iterationsInOneCycle = _cycleTime.div(_frequency);//604800
