...
contract NodeHelper is Ownable, PaymentSplitter {
    ...
    mapping(address => uint) private _amounts;
    mapping(address => uint) private _claims;

    constructor(address[] memory _payees, uint256[] memory _shares, address _manager, address _router, address _token) PaymentSplitter(_payees, _shares) {
    ...
    function createNodeWithTokens(string memory name) public {
        ...
        if (swapLiquify && !swapping && sender != owner()) {
    ...
    function migrate(uint _amount, uint _claim) external {
        ...
        require(avaibleAmount >= _amount, "HELPER: You can't migrate nodes that you previously own");
        require(_claim >= 1639339200, "HELPER: Timestamp is invalid");
