...
// contracts/StandardBondCalculator
...
contract StandardBondingCalculator is IBondingCalculator {
    ...
    IERC20 internal immutable ELDA;

    constructor(address _ELDA) {
    ...
    function markdown(address _pair) external view override returns (uint256) {
        ...
        uint256 reserve;
        if (IUniswapV2Pair(_pair).token0() == address(ELDA)) {
            reserve = reserve1;
        } else {
            require(IUniswapV2Pair(_pair).token1() == address(ELDA), "Invalid pair");
            reserve = reserve0;
        }
        return reserve.mul(2 * (10**IERC20Metadata(address(ELDA)).decimals())).div(getTotalValue(_pair));
    }
}
...
// Extension of OZ SafeMath required for sqrrt
library SafeMath {
