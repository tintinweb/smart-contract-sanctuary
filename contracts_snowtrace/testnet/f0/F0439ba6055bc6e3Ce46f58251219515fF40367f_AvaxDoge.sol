...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
    ...
    modifier onlyTradableDoge (address from, uint256 tokenId) {
    ...
    mapping (uint256 => Offer) public dogesOfferedForSale;
    mapping (uint256 => Bid) public dogeBids;
...
contract AvaxDoge is ERC721, Ownable {
    ...
    uint256 public SALE_START_TIMESTAMP = 1632355200; 

    uint256 public constant MAX_SUPPLY = 10000;
    uint256 public dogesRemainingToAssign = 10000;
    ...
    function mintNFT(uint256 amount) public payable {
        ...
        for (uint i = 0; i < amount; i++) {
            uint256 randIndex = _random() % dogesRemainingToAssign;
            uint256 dogeIndex = _fillAssignOrder(--dogesRemainingToAssign, randIndex);
            _safeMint(msg.sender, dogeIndex);
        }
    }

    function buyDoge(uint256 tokenId) payable public {
        Offer memory offer = dogesOfferedForSale[tokenId];
        require(tokenId < 10000, "Out of tokenId");
        ...
        require(success);
        // handle
        // emit DogeBought(tokenId, msg.value, seller, _msgSender());
    }

    function dogeNoLongerForSale(uint256 tokenId) public {
        _dogeNoLongerForSale(_msgSender(), tokenId);
    }

    function _dogeNoLongerForSale(address from, uint256 tokenId) internal onlyTradableDoge(from, tokenId) {
        dogesOfferedForSale[tokenId] = Offer(false, tokenId, from, 0, address(0));
        // emit DogeNoLongerForSale(tokenId);
    }

    function offerDogeForSale(uint256 tokenId, uint256 minSalePriceInWei) public onlyTradableDoge(_msgSender(), tokenId) {
        dogesOfferedForSale[tokenId] = Offer(true, tokenId, _msgSender(), minSalePriceInWei, address(0));
        // emit DogeOffered(tokenId, minSalePriceInWei, address(0));
    }

    function enterBidForDoge(uint256 tokenId) public payable {
        require(tokenId < 10000, "Out");
        require(ownerOf(tokenId) != _msgSender(), "Invalid");
        require(msg.value > dogeBids[tokenId].value, "Invalid");
        Bid memory existing = dogeBids[tokenId];
        if (existing.value > 0) {
            // Refund the failing bid
            pendingWithdrawals[existing.bidder] += existing.value;
        }
        dogeBids[tokenId] = Bid(true, tokenId, _msgSender(), msg.value);
        // emit DogeBidEntered(tokenId, msg.value, _msgSender());
    }

    function acceptBidForDoge(uint256 tokenId, uint256 minPrice) public onlyTradableDoge(_msgSender(), tokenId) {
        require(dogeBids[tokenId].value >= minPrice, "Invalid");
        Bid memory bid = dogeBids[tokenId];

        dogeBids[tokenId] = Bid(false, tokenId, address(0), 0);
        ...
        // emit DogeBought(tokenId, bid.value, _msgSender(), bid.bidder);
    }

    function withdrawBidForDoge(uint256 tokenId) public {
        require(tokenId < 10000, "Out");
        require(ownerOf(tokenId) != _msgSender(), "Invalid");
        require(dogeBids[tokenId].bidder == _msgSender(), "Invalid");
        dogeBids[tokenId] = Bid(false, tokenId, address(0), 0);
        // Refund the bid money
        (bool success,) = _msgSender().call{value: dogeBids[tokenId].value}("");
        require(success);
        // emit DogeBidWithdrawn(tokenId, dogeBids[tokenId].value, _msgSender());
    }
