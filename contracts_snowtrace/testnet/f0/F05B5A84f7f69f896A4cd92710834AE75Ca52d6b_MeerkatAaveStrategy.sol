...
contract MeerkatAaveStrategy is StratManager, FeeManager {
    ...
    constructor(
        ...
        (aToken, , varDebtToken) = IDataProvider(dataProvider)
     ...
     */
    function deleverageOnce(uint256 _borrowRate) external onlyManager {
    ...
    function _harvestGainedInterest(address callFeeRecipient) internal whenNotPaused {
        ...
        uint256 harvestedBal = afterBal.sub(reserveToMaintain);
        if (harvestedBal > 0) {
            chargeFees(harvestedBal, callFeeRecipient);
            //send the harvested balance to staking contract
            uint256 bal = IERC20(want).balanceOf(address(this));
            uint256 stakingAmount = bal.sub(reserveToMaintain);
            IERC20(want).safeTransfer(stakingContract, stakingAmount);
            deposit();
            lastHarvest = block.timestamp;
            emit StratHarvest(msg.sender);
        }
        ...
        internal
    {
        ...
        IERC20(want).safeTransfer(devFeeRecipient, devFeeAmount);
    }
    ...
    function rewardsAvailable() public view returns (uint256) {
        ...
        assets[1] = varDebtToken;
        return
            IIncentivesController(incentivesController).getRewardsBalance(
                assets,
                address(this)
            );
    }
    ...
    function setStakingContract(address _implementation) public onlyOwner {
        stakingContract = _implementation;
    }

    function setMinLeverage(uint256 _amount) public onlyManager {
        minLeverage = _amount;
    }
