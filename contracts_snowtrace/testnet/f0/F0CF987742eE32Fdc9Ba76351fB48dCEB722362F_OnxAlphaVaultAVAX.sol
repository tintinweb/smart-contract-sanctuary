...
interface IStrategy {
    ...
    function pendingXJoe() external view returns (uint256);
    function pendingXJoeOfUser(address user) external view returns (uint256);
...
contract OnxAlphaVaultAVAX is ERC20Upgradeable, ControllableInit, VaultStorage {
    ...
    //strategy this vault is using see alpha strategy and strategy adapter
    ...
    //ensures we have strategy defined before calling fxns
    ...
    //transfer tokens in vault contract(this) to strategy
    function stakeJoeFarm() whenStrategyDefined onlyControllerOrGovernance external {
        invest();
        IStrategy(strategy()).stakeJoeFarm();
    }
    //stake xJoe
    function stakeXJoe() whenStrategyDefined onlyControllerOrGovernance external {
        IStrategy(strategy()).stakeXJoe();
    }
    //stake xJOE for more JOE
    ...
    //transfers everything and stake everything
    ...
    //how much is staked by users with us(vault+strategy)
    ...
    //deposit for someone else for some reaosn
    function depositFor(uint256 amount, address holder) public defense {
        _deposit(amount, msg.sender, holder);
    }
    //withdraw to from masterchef to strat to pool
