...
contract StreamingFeeModule is ModuleBase, ReentrancyGuard {
   ...
   */
  function accrueFee(IDomani _setToken) public nonReentrant onlyValidAndInitializedSet(_setToken) {
    uint256 managerFee;
    uint256 protocolFee;
   ...
   */
  function initialize(IDomani _setToken, FeeState memory _settings)
    external
  ...
  function removeModule() external override {
    delete feeStates[IDomani(msg.sender)];
  }
   ...
   */
  function updateStreamingFee(IDomani _setToken, uint256 _newFee)
    external
    onlySetManager(_setToken, msg.sender)
   ...
   */
  function getFee(IDomani _setToken) external view returns (uint256) {
   ...
   */
  function _calculateStreamingFee(IDomani _setToken) internal view returns (uint256) {
    // solhint-disable-next-line not-rely-on-time
   ...
   */
  function _calculateStreamingFeeInflation(IDomani _setToken, uint256 _feePercentage)
    internal
    view
    returns (uint256)
  {
   ...
   */
  function _mintManagerAndProtocolFee(IDomani _setToken, uint256 _feeQuantity)
    internal
   ...
   */
  function _editPositionMultiplier(IDomani _setToken, uint256 _inflationFee) internal {
  ...
  function _lastStreamingFeeTimestamp(IDomani _set) internal view returns (uint256) {
    return feeStates[_set].lastStreamingFeeTimestamp;
  }

  function _maxStreamingFeePercentage(IDomani _set) internal view returns (uint256) {
    return feeStates[_set].maxStreamingFeePercentage;
  }

  function _streamingFeePercentage(IDomani _set) internal view returns (uint256) {
