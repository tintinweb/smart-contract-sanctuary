...
contract Shamanz is ERC721EnumerableUpgradeable, OwnableUpgradeable, IERC2981Upgradeable {
    using StringsUpgradeable for uint256;
    ...
    uint256 public maxMintAmount; //max amount of nfts mintable after presale is turned off
    uint256 public nftPerAddressLimit; //max amount of nfts mintable for one address during presale mint phase
    bool public paused; //pause minting functionality
    bool public onlyWhitelisted; //whitelist minting ON/OFF
    address payable public payments; //public for now remove later
    ...
    function mint(uint256 _mintAmount) external payable {
        ...
        if (msg.sender != owner()) {
            if (onlyWhitelisted == true) {
                ...
                require (ownerMintedCount + _mintAmount <= nftPerAddressLimit, "max nft per address reached");
            }
    ...
    function royaltyInfo(
        ...
        receiver = address(payments);  // address of royalty fees receiver
        royaltyAmount = 500;  // precent of sale that goes to reciever
    }
}
