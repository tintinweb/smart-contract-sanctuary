...
contract knivesRoyalties {
    ...
    mapping(address => uint) internal lastClaimed;
    ...
    function defContractAddress(address _contractAddress) public onlyManager {
    ...
    function rewardsOf(address _user) public view returns(uint256){
        IERC721Enumerable contrat = IERC721Enumerable(_contract);
        uint reward;
        for (uint i=0; i<deposits.length; i++) {
            if (lastClaimed[_user]<deposits[i]) {
                reward += rewards[deposits[i]]*contrat.balanceOf(_user).div(contrat.totalSupply());
            }
        }
        return reward;
    }
    ...
    function claim() public {
        require(rewardsOf(msg.sender)>0, "You have nothing to claim");
        msg.sender.transfer(rewardsOf(msg.sender));
