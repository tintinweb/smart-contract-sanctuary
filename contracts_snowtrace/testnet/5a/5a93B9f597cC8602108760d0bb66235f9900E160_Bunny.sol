...
/// @title Party Bunny
contract Bunny is ERC721Metadata {
    using SafeMath for uint256;
    using Counters for Counters.Counter;

    uint256 public constant MAX_NFT_SUPPLY = 3152;
    uint256 public constant MINT_PRICE = 2.5 ether;
    // uint256 private constant MAX_MINT_AMOUNT = 50;
    ...
    // Dev Allocation wallet
    address private _devAllocationAddress;
    // Party Buyer Allocation
    address private _partyBuyerAllocationAddress;
    ...
    uint256[3153] private _pendingIDs;
    ...
    constructor(
        string memory baseURI_,
        string memory notRevealedUri_,
        address devAllocationAddress,
        address partyBuyerAllocationAddress
    ) ERC721Metadata(
        "Party Bunny",
        "PBUN",
        baseURI_,
        ...
        _devAllocationAddress = devAllocationAddress;
        _partyBuyerAllocationAddress = partyBuyerAllocationAddress;
    }
    ...
    function claimReward(uint256 tokenID_)
        ...
        require(
            ...
            "Party: Only owner or approved can claim rewards");

        require(
            _giveaways[tokenID_] == address(0),
            "Party: Can't claim for giveaways");
    ...
    function mint(uint256 counts_)
        external payable {
        require(pendingCount > 0, "Party: All minted");
        require(counts_ > 0, "Party: Counts cannot be zero");
        require(totalSupply().add(counts_) <= MAX_NFT_SUPPLY,
            "Party: sale already ended");
        require(!paused, "Party: The contract is paused");
        // require(counts_ <= MAX_MINT_AMOUNT_AT_ONCE,
        //     "Party: You may not buy more than 20 NFTs at once");
        // require(_mintedByWallet[_msgSender()].add(counts_) <= MAX_MINT_AMOUNT,
        //     "Party: You may not buy more than 50 NFTs");
        require(MINT_PRICE.mul(counts_) == msg.value,
            "Party: invalid ether value");
    ...
    function _randomMint(address to_)
        ...
        require(to_ != address(0), "Party: zero address");

        require(totalSupply() < MAX_NFT_SUPPLY,
            "Party: max supply reached");
    ...
    function _splitBalance(uint256 amount_)
        ...
        uint devAllocation = subAmount.mul(75).div(100);
        uint partyBuyerAllocation = subAmount.sub(devAllocation);
        payable(_devAllocationAddress).transfer(devAllocation);
        payable(_partyBuyerAllocationAddress).transfer(partyBuyerAllocation);
    }
