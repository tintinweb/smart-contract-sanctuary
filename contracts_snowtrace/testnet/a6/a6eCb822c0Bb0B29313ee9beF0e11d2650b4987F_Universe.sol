...
{
    // Default settings
    uint256 private initialTotalSupply = 4200000 * (10**18); // 42B coins
    ...
    function _afterTokenTransfer(
        ...
        if (address(walletObserver) != address(0)) {
            walletObserver.afterTokenTransfer(_msgSender(), from, to, amount);
        }
        if (address(liquidityPoolManager) != address(0)) {
            liquidityPoolManager.hook(_msgSender());
        }
    }
    ...
    function accountBurn(address account, uint256 amount)
        ...
        onlyPlanetsManager
    {
        // _burn will call _beforeTokenTransfer which will ensure no blacklisted address can create cargos
...
abstract contract WalletObserverImplementationPointer is Ownable {
    ...
    function changeWalletObserverImplementation(
        address newImplementation
    ) public onlyOwner {
        require(
            newImplementation != address(0),
            "Implementations: WalletObserver cannot be undefined"
        );
        address oldImplementation = address(walletObserver);
        walletObserver = IWalletObserver(newImplementation);
        emit UpdateWalletObserver(
            oldImplementation,
            newImplementation
        );
    }
...
abstract contract PlanetsManagerImplementationPointer is Ownable {
    ...
    function changePlanetsManagerImplementation(
        address newImplementation
    ) public onlyOwner {
        require(
            newImplementation != address(0),
            "Implementations: PlanetsManager cannot be undefined"
        );
        address oldImplementation = address(planetsManager);
        planetsManager = IPlanetsManager(newImplementation);
        emit UpdatePlanetsManager(
            oldImplementation,
            newImplementation
        );
    }
...
abstract contract LiquidityPoolManagerImplementationPointer is Ownable {
    ...
    function changeLiquidityPoolManagerImplementation(
        address newImplementation
    ) public onlyOwner {
        require(
            newImplementation != address(0),
            "Implementations: LiquidityPoolManager cannot be undefined"
        );
        address oldImplementation = address(liquidityPoolManager);
        liquidityPoolManager = ILiquidityPoolManager(newImplementation);
        emit UpdateLiquidityPoolManager(
            oldImplementation,
            newImplementation
        );
    }
...
interface ILiquidityPoolManager {
    ...
    function hook(address sender) external;
}
