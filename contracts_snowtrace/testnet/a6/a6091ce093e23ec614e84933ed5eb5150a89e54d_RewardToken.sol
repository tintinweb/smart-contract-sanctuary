...
contract RewardToken is ERC20("LP Reward Token", "LPR", 18), Ownable {
    ...
    // fund reward token with rewards
    // note input as 18 decimal number
    function mint(uint256 amount) external {
        for(uint256 i = 0; i < tokens.length; i++) {
            tokens[i].safeTransferFrom(msg.sender, address(this), _getAmount(i, amount));
        }
        _mint(msg.sender, amount);
    }

    // burn reward token and get underlyings
    function unwrap(address to, uint256 amount) external {
        for(uint256 i = 0; i < tokens.length; i++) {
            tokens[i].safeTransfer(to, _getAmount(i, amount));
        }
    ...
    function _getAmount(uint256 i, uint256 amount) internal view returns (uint256) {
        return tokens[i].balanceOf(address(this)) * amount / totalSupply();
    }
}