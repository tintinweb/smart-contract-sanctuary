...
contract PreNoahArkERC20Sales is Ownable {
    ...
    function initialize(
        ...
        pNRK = IERC20( _pNRK );
        dai = IERC20( dai_ );
        pNRKPrice = pNRKPrice_;
        _saleProceedsAddress = saleProceedsAddress_;
        initialized = true;
    }

    function setpNRKPrice( uint256 newpNRKPrice_ ) external onlyOwner() returns ( uint256 ) {
        pNRKPrice = newpNRKPrice_;
        return pNRKPrice;
    }
    ...
    function _calculateAmountPurchased( uint256 amountPaid_ ) internal returns ( uint256 ) {
        return amountPaid_.mul( pNRKPrice );
    }

    function buyPNRK( uint256 amountPaid_ ) external returns ( bool ) {
        require( approvedBuyers[msg.sender], "Buyer not approved." );
        uint256 pNRKAmountPurchased_ = _calculateAmountPurchased( amountPaid_ );
        dai.safeTransferFrom( msg.sender, _saleProceedsAddress, amountPaid_ );
        pNRK.safeTransfer( msg.sender, pNRKAmountPurchased_ );
        return true;
    }
