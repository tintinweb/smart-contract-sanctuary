...
contract ArableOracle is Ownable {
    ...
    function registerPrice(address token_, uint256 price_) public onlyOwner {
    ...
    function bulkPriceSet(address[] calldata tokens_, uint256[] calldata prices_) external onlyOwner {
    ...
    function bulkRegisterRewardRate(uint256 farmId_, address[] calldata tokens_, uint256[] calldata dailyRewardRates_) external onlyOwner {
        require(tokens_.length == dailyRewardRates_.length, "Please check you input data.");
        for(uint256 i=0; i < tokens_.length; i++) {
            registerRewardRate(farmId_, tokens_[i], dailyRewardRates_[i]);
        }
    }
}
