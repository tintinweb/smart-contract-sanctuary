...
contract MapOfSouls is ERC721, Ownable {
    using SafeMath for uint256;
    uint public constant MAX_SOULS = 500;
    bool public hasSaleStarted = false;
    
    string public METADATA_PROVENANCE_HASH = "";
     
    constructor() ERC721("MapOfSouls","SOUL")  {
        setBaseURI("http://mapofsouls.com/api/");
    ...
    function currentPrice() public view returns (uint256) {
        ...
        require(totalSupply() < MAX_SOULS, "Sale has already ended");
   ...
   function mintSoul(uint256 maxSouls) public payable {
        require(totalSupply() < MAX_SOULS, "Sale has already ended");
        require(maxSouls > 0);
        require(totalSupply().add(maxSouls) <= MAX_SOULS, "Exceeds MAX_SOULS");
        require(msg.value >= currentPrice().mul(maxSouls), "Ether value sent is below the price");
