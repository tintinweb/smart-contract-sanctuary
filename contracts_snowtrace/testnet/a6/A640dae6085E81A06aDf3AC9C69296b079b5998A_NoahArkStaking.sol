...
contract NoahArkStaking is Ownable {
    ...
    function claim ( address _recipient ) public {
        ...
        if ( epoch.number >= info.expiry && info.expiry != 0 ) {
            ...
            IWarmup( warmupContract ).retrieve( _recipient, IsNRK( sNRK ).balanceForGons( info.gons ) );
        }
    }
    ...
    function forfeit() external {
        ...
        IWarmup( warmupContract ).retrieve( address(this), IsNRK( sNRK ).balanceForGons( info.gons ) );
        IERC20( NRK ).safeTransfer( msg.sender, info.deposit );
    }
