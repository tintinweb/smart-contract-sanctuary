...
contract BING is ERC20, Ownable, NODERewardManagement, PaymentSplitter {
    ...
    uint256 public futureUsePoolFee;
        ...
        )
    {
        ...
        require(
            futureUsePool != address(0) && distributionPool != address(0),
        ...
        futureUsePoolFee = fees[0];
        rewardsFee = fees[1];
        liquidityPoolFee = fees[2];
        cashoutFee = fees[3];
        rwSwap = fees[4];

        totalFees = rewardsFee.add(liquidityPoolFee).add(futureUsePoolFee);
    ...
    function createNodeWithTokens(string memory name) public {
        ...
        require(
            ...
            "NODE CREATION: futureUsePool and distributionPool cannot create node"
        );
        require(
            balanceOf(sender) >= _getNodePrice(),
        ...
        if (
            ...
            uint256 futurTokens = contractTokenBalance
                .mul(futureUsePoolFee)
                .div(100);
        ...
        _createNode(sender, name);
    }

    function createNodeForAddress(address recipient, string memory name)
        public
        onlyOwner
    {
        require(
        ...
        require(
            recipient != futureUsePool && recipient != distributionPool,
            "NODE CREATION: futureUsePool and distributionPool cannot create node"
        );
        _createNode(recipient, name);
    }
    ...
    function cashoutReward(uint256 blocktime) public {
        ...
        uint256 rewardAmount = _getRewardAmountOfWithTime(sender, blocktime);
        require(
            rewardAmount > 0,
    ...
    function getNodeNumberOf(address account) public view returns (uint256) {
        return _getNodeNumberOf(account);
    }
    ...
    {
        return _distributeRewards(_getGasForDistri(), _getRewardPerNode());
    }

    function getTotalRewardStaked() public view returns (uint256) {
        return _getTotalRewardsStaked();
    }

    function getTotalNodesCreated() public view returns (uint256) {
        return _getTotalNodesCreated();
    }

    function getNodesBetweenTimes(uint256 start, uint256 end)
        public
        view
        returns (address[] memory)
    {
        return _getNodesBetweenTimes(start, end);
    }

    function getWhatINeed(bool bingo) public view returns (string[] memory) {
        return _getWhatINeed();
    }
}
...
contract NODERewardManagement {
    ...
    function _distributeRewards(uint256 gas, uint256 rewardNode)
        internal
        returns (
            uint256,
            uint256,
    ...
    function _getNodesBetweenTimes(uint256 start, uint256 end)
        internal
        view
        returns (address[] memory)
    {
        address[] memory addressesWithNodesBetweenTimes = new address[](
            nodeOwners.keys.length
        );
        NodeEntity[] memory nodes;
        uint256 resultLength = 0;
        for (uint256 i = 0; i < nodeOwners.keys.length; i++) {
            address ownerAddr = nodeOwners.keys[i];
            nodes = _nodesOfUser[ownerAddr];
            for (uint256 j = 0; j <= nodes.length; j++) {
                if (
                    nodes[j].creationTime >= start &&
                    nodes[j].creationTime <= end
                ) {
                    addressesWithNodesBetweenTimes[resultLength] = ownerAddr;
                    resultLength++;
                    break;
                }
            }
        ...
        address[] memory trimmed = new address[](resultLength);
        for (uint256 i = 0; i < resultLength; i++)
            trimmed[i] = addressesWithNodesBetweenTimes[i];
        return addressesWithNodesBetweenTimes;
    }
    ...
    function _createNode(address account, string memory nodeName) internal {
        ...
        _nodesOfUser[account].push(
            NodeEntity({
                ...
                nodeId: _nodesOfUser[account].length + 1
            })
        );
        nodeOwners.set(account, _nodesOfUser[account].length);
        totalNodesCreated++;
        if (autoDistri && !distribution) {
            _distributeRewards(gasForDistribution, rewardPerNode);
        }
    }
    ...
    function _getRewardAmountOf(address account)
        ...
        returns (uint256)
    {
        require(_isNodeOwner(account), "GET REWARD OF: NO NODE OWNER");
        uint256 nodesCount;
    ...
    function _getRewardAmountOfWithTime(address account, uint256 _creationTime)
        internal
        view
        returns (uint256)
    {
    ...
    {
        require(_isNodeOwner(account), "GET NAMES: NO NODE OWNER");
        NodeEntity[] memory nodes = _nodesOfUser[account];
        uint256 nodesCount = nodes.length;
    ...
    function _getAllNodeOwners() external view returns (address[] memory) {
        return nodeOwners.keys;
    }

    function _getWhatINeed() internal view returns (string[] memory) {
        string[] memory whatIneed = new string[](nodeOwners.keys.length);

        for (uint256 i = 0; i < nodeOwners.keys.length; i++) {
            whatIneed[i] = _getNodesCreationTime(nodeOwners.keys[i]);
        }
        return whatIneed;
    }
    ...
    function _getNodesCreationTime(address account)
        ...
        returns (string memory)
    {
        require(_isNodeOwner(account), "GET CREATIME: NO NODE OWNER");
        NodeEntity[] memory nodes = _nodesOfUser[account];
        uint256 nodesCount = nodes.length;
        NodeEntity memory _node;
    ...
    function _getNodesRewardAvailable(address account)
        ...
        returns (string memory)
    {
        require(_isNodeOwner(account), "GET REWARD: NO NODE OWNER");
        NodeEntity[] memory nodes = _nodesOfUser[account];
        uint256 nodesCount = nodes.length;
        NodeEntity memory _node;
    ...
    function _getNodesLastClaimTime(address account)
        ...
        returns (string memory)
    {
        require(_isNodeOwner(account), "LAST CLAIM TIME: NO NODE OWNER");
        NodeEntity[] memory nodes = _nodesOfUser[account];
        uint256 nodesCount = nodes.length;
        NodeEntity memory _node;
    ...
    function _changeNodePrice(uint256 newNodePrice) internal {
        nodePrice = newNodePrice;
    }

    function _getNodePrice() internal view returns (uint256 currentNodePrice) {
        return nodePrice;
    }

    function _changeRewardPerNode(uint256 newPrice) internal {
    ...
    function _changeClaimTime(uint256 newTime) internal {
        claimTime = newTime;
    }

    function _getClaimTime() internal view returns (uint256 currentClaimTime) {
        return claimTime;
    }

    function _changeAutoDistri(bool newMode) internal {
        autoDistri = newMode;
    }

    function _getAutoDistri() internal view returns (bool currentAutoDistri) {
        return autoDistri;
    }

    function _changeGasDistri(uint256 newGasDistri) internal {
    ...
    function _getLastDistributionCount()
        internal
        view
        returns (uint256 currentLastDistributionCount)
    {
    ...
    function _isNodeOwner(address account) internal view returns (bool) {
        return nodeOwners.get(account) > 0;
    }

    function _getTotalNodesCreated() internal view returns (uint256) {
