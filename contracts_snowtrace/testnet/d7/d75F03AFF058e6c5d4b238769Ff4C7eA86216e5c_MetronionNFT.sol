...
contract MetronionNFT is ERC721, ERC721Enumerable, PermissionGroup, AcceptedAssets, IMetronionNFT {
    ...
    function mintMetronion(
        ...
        require(
            version.currentSupply.add(amount) <= version.maxSupply,
            "MetronionNFT: amount of Metronions left is not enough"
        );

        version.currentSupply = version.currentSupply.add(amount);
    ...
    function addNewVersion(uint256 maxSupply, string memory provenance) external onlyOwner {
        ...
        emit NewVersionAdded(latestVersionId.add(1));
    }
    ...
    function finalizeStartingIndex(uint256 versionId) external override onlyOwner {
        ...
        uint256 startingIndex = uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp))).mod(
            version.maxSupply
        );
