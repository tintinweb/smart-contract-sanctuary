...
contract Noonercoin is ERC20{
   ...
   uint8[] __randomVariable = [150, 175, 200, 225];
   uint8[] __remainingRandomVariable = [150, 175, 200, 225];
   ...
   function mintToken(address add) public returns (bool) {  //admin only
       ...
       if(now-weekStartTime>86400){//changed 3600 to 86400
           popRandomVariable();
           weekStartTime=0;
       }
     ...
     if(timeDiff > fromTime){    //120weeks - 120seconds // 
         if(timeDiff == _cycleTime){//120 weeks // change  '<' to '=='
    ...
    function popRandomVariable() public  returns(bool){
        ...
        if(occuranceOfRandonNumber[randomNumber]>=6){
            //remove variable
            uint256 _index;
  ...
  function getLastMintingTimeAndStartTimeDifferencev() public view returns (uint256){
