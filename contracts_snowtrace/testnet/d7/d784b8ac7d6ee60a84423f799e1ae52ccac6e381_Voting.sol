...
/*
----------------------------------------------------------------------------------------------------
Tomorrow Tree Project Voting - Test 4
----------------------------------------------------------------------------------------------------
*/
...
// Maybe we add a function that sets the length of timer ? Added
    ...
    modifier oneActiveProposal(address proposerAddress){
        require(proposerRegister[proposerAddress].activeProposal == false,"This address is already running a proposal" );
        _;
    }


    event OwnershipTransferred(address indexed _oldOwner, address indexed _newOwner);
    event voterRegistered(uint64 indexed voterId, address indexed voterAddress);
    ...
    function newProposal(string memory _projectName, uint _tokensToBeMinted) public isProposerModifier(msg.sender) oneActiveProposal(msg.sender) {
        ...
        //adds proposalID to the mapping so we can find what proposals did proposers create
    ...
    function voteOnProposal(uint _proposalId, bool _vote, uint voteLengthInSeconds) public timer(proposalList[_proposalId].proposalStartTime,voteLengthInSeconds){
