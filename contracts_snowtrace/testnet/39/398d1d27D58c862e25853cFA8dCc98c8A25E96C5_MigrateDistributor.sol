...
contract MigrateDistributor is Policy {
    ...
    constructor( address _treasury, address _AOM, uint _epochLength, uint _nextEpochBlock, address _Staking, address _sAOM ) {
    ...
    function nextRewardAt( uint _rate ) public view returns ( uint ) {
        return IERC20( AOM ).totalSupply().mul( _rate ).div( 1000000 );
    }
    ...
    function setStaking(address _Staking, address _sAOM) external onlyPolicy() {
        require( _Staking != address(0) );
        Staking = _Staking;
        require( _sAOM != address(0) );
        sAOM = _sAOM;
