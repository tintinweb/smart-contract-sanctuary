...
contract Traits is Ownable {
    ...
    uint public bearTraitsNumber = 5;
    uint public bullTraitsNumber = 5;
    ...
    function getIndexFromTokenId(uint _tokenId) public view returns (uint index) {
        index = getIndexFromTraits(BullAndBear.getTokenTraits(_tokenId));
    }

    function getIndexFromTraits(IBullAndBear.Trait memory _trait) public view returns (uint index) {
        if (_trait.isBear) {
            for (uint i = 0; i < _trait.traits.length; i++) {
                index += i * multiplierIndexForBearTraitType[i];
            }
        } else {
            index = firstBullIndex;

            for (uint i = 0; i < _trait.traits.length; i++) {
                index += i * multiplierIndexForBullTraitType[i];
            }
        }
    }

    function getTraitsFromIndex(uint _index) public view returns (IBullAndBear.Trait memory _trait) {
        uint id;

        if (_index < firstBullIndex) {
            _trait.isBear = true;
            for (uint i = 0; i < _trait.traits.length; i++) {
                id = _trait.traits.length - i - 1; // get the id from the end
                _trait.traits[id] = uint(_index / multiplierIndexForBearTraitType[i]);
            ...
            for (uint i = 0; i < _trait.traits.length; i++) {
                id = _trait.traits.length - i - 1; // get the id from the end
                _trait.traits[id] = uint(_index / multiplierIndexForBullTraitType[i]);
