...
uint8 constant WALLET_LIQUIDITY = 3;
...
uint8 constant FEE_LIQUIDITY = 4;
...
contract RetroVisionToken is IERC20, Context, ReentrancyGuard, Ownable {
    ...
    // TODO: Management contract configurations
    ...
    constructor(
        ...
        // Set initial fees
        setFee(FEE_REFLECTION, 500);
        setFee(FEE_DEV_MARKET, 200);
        setFee(FEE_CHARITY, 100);
        setFee(FEE_RESOURCE_LIQUIDITY, 200);
        setFee(FEE_LIQUIDITY, 100);

        // Calculate seed amounts
        uint256 _devMarketSeed = Finance.calculateFeeAmount(_tokenSupply, 100); // 1%
        uint256 _charitySeed = Finance.calculateFeeAmount(_tokenSupply, 100); // 1%
        uint256 _liquiditySeed = Finance.calculateFeeAmount(_tokenSupply, 3000); // 30%
        uint256 _remainingSeed = _tokenSupply.sub(
            _devMarketSeed.add(_charitySeed).add(_liquiditySeed)
        );

        // Seed distributions
        _accounts[_devMarketWallet].balance = _devMarketSeed;
        _accounts[_charityWallet].balance = _charitySeed;
        _accounts[address(this)].balance = _liquiditySeed;
        ...
        emit Transfer(address(0), address(this), _liquiditySeed);
    ...
    function _transfer(
        ...
        if (_sender != owner()) {
            if (!_accounts[_sender].excluded) {
                require(_tokenGoLive, "ERC20: token not yet live on-chain");
        ...
        returns (uint256)
    {
        ...
        uint256 _liquidityCollected = _accounts[address(this)].balance;
        ...
        // Resource Liquidity
        // TODO: Tokens past threshold to reflection pool?
        ...
        // DEX Liquidity
        _accounts[address(this)].balance = _liquidityCollected.add(
            _liquidityFee
        );
        // TODO: Liquidity is pooled within contract to threshold then flush to LP

        emit LiquidityFeeApplied(_liquidityFee);
        ...
        )
    {
        ...
        _liquidityFee = Finance.calculateFeeAmount(
            _amount,
            _feeSchedule[FEE_LIQUIDITY]
        );
        ...
        require(
            (
                _feeSchedule[FEE_REFLECTION]
                    ...
                    .add(_feeSchedule[FEE_LIQUIDITY])
                    .add(_newFee)
    ...
    function getInternalWalletAddress(uint8 _walletType)
        ...
        returns (address)
    {
        return _internalWallets[_walletType];
    }
    ...
    event LiquidityFeeApplied(uint256 _amount);

    // Emmitted when LP internal pool has hit it's reserve and is flushed to LP wallet
    event LiquidityPoolFlushed(address indexed _address, uint256 _amount);
...
library Finance {
    ...
    function calculateBP(uint256 _dividend, uint256 _divisor)
        ...
        returns (uint256)
    {
        uint256 _bp = _dividend.mul(10000).div(_divisor);
        return _bp;
    }
}
