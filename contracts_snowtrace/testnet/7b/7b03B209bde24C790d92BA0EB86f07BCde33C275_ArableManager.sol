...
contract ArableManager is Ownable {
    ...
    event RegisterSynth(address synthAddress, string tokenName, string tokenSymbol, uint256 tokenInitialSupply);
    ...
    function registerSynth(
        ...
        uint256 tokenInitialSupply) public onlyOwner {
        // TODO: farming and exchange contracts should be set before synths register
        // TODO: think of decimal initialization
        ...
        ArableSynth synth = new ArableSynth(tokenName, tokenSymbol, tokenInitialSupply, owner(), farming, exchange);
        synths.push(address(synth));
        isSynth[address(synth)] = true;

        emit RegisterSynth(address(synth), tokenName, tokenSymbol, tokenInitialSupply);
    }

    function bulkRegisterSynths(
        string[] calldata tokenNames_, string[] calldata tokenSymbols_, 
        uint256 tokenInitialSupply) external onlyOwner {
        require(tokenNames_.length == tokenSymbols_.length, "Please check you input data.");
        for(uint256 i=0; i < tokenNames_.length; i++) {
            registerSynth(tokenNames_[i], tokenSymbols_[i], tokenInitialSupply);
        }
    }
...
// Tokens are mintable by admins, staking or exchange contract
contract ArableSynth is ERC20, AccessControl {
    // Define roles
    ...
    constructor(
        ...
        address owner, address farming_, address exchange_) ERC20(name_, symbol_) {
        // TODO: think of decimal initialization
        ...
        // provide minter role for admin, staking and exchange contract
