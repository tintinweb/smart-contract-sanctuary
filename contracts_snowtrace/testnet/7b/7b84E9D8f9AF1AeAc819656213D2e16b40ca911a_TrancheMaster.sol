...
contract IbBusdFake is ERC20 {
    ...
    event TokenBalance(uint256 id, address account, uint256 amount);
...
contract StrategyVenus is IStrategyVenus, ReentrancyGuard, Ownable, CoreRef {
    ...
    function earn(uint256 minReturnWant) public override whenNotPaused onlyTimelock {
    ...
    function withdraw(uint256 minReturnWant) public override onlyMultistrategy nonReentrant {
        require(minReturnWant > 0, "Min return is zero");
...
interface IStrategy {
    ...
    function withdraw(uint256 minReturnWant) external;
    ...
    function earn(uint256 minReturnWant) external;
...
contract StrategyAlpaca is IStrategyAlpaca, ReentrancyGuard, Ownable, CoreRef {
    ...
    function _deposit(uint256 _wantAmt) internal {
        ...
        FairLaunch(fairLaunchAddress).deposit(Vault(vaultAddress).balanceOf(address(this)));
    ...
    function withdraw(uint256 minReturnWant) public override onlyMultistrategy nonReentrant {
        ...
        FairLaunch(fairLaunchAddress).withdraw(FairLaunch(fairLaunchAddress).balanceOf(address(this)));
...
contract StrategyAave is IStrategyAlpaca, ReentrancyGuard, Ownable, CoreRef {
    ...
    // address public constant lendingPoolAddressesProviderAddress = 0x88757f2f99175387aB4C6a4b3067c77A695b0349;
    address public constant lendingPoolAddressesProviderAddress = 0x7fdC1FdF79BE3309bf82f4abdAD9f111A6590C0f;
...
contract MultiStrategyToken is IMultiStrategyToken, ReentrancyGuard, CoreRef {
    ...
    function withdraw(uint256[] memory minReturns) public override onlyRole(MASTER_ROLE) nonReentrant {
        updateAllStrategies();
        for (uint256 i = 0; i < strategies.length; i++) {
            IStrategy(strategies[i]).withdraw(minReturns[i]);
        }
...
interface IStrategyToken {
    ...
    function withdraw(uint256[] memory minReturns) external;
...
contract TrancheMaster is ITrancheMaster, CoreRef, ReentrancyGuard {
    ...
    function _stopCycle(uint256[] memory minReturns) internal {
        require(block.timestamp >= actualStartAt + duration, "cycle not expired");
        _processExit(minReturns);
        active = false;
        cycle++;
    ...
    function _processExit(uint256[] memory minReturns) internal {
        uint256 before = IERC20(currency).balanceOf(address(this));
        IStrategyToken(strategy).withdraw(minReturns);
    ...
    function stop(uint256[] memory minReturns) public override checkActive nonReentrant onlyGovernor {
        _stopCycle(minReturns);
    }
...
interface ITrancheMaster {
    ...
    function stop(uint256[] memory minReturns) external;
...
contract TimelockController is CoreRef, ReentrancyGuard {
    ...
    function earn(address _strategy, uint256 minReturnWant) public onlyRole(EXECUTOR_ROLE) {
        IStrategy(_strategy).earn(minReturnWant);
    }
...
contract UniswapV2Router02 {
    using SafeERC20 for IERC20;
    address public currencyIn;
    address public currencyOut;

    constructor(address _currencyIn, address _currencyOut) public {
        ...
        )
    {
        ...
        IERC20(currencyOut).safeTransfer(to, amountOut);
        amounts[0] = amountIn;
        amounts[1] = amountOut;
        return amounts;
    }
...
contract AlpacaFake is ERC20 {
    ...
    function withdraw(uint256 redeemTokens) public {
        ...
        uint256 amount = (redeemTokens * factor) / 100;
        // IERC20(underlying).safeTransfer(msg.sender, amount);
        IERC20(underlying).safeTransfer(msg.sender, redeemTokens);
        if (rewardToken != address(0)) {
            IERC20(rewardToken).safeTransfer(msg.sender, amount);
        }
        _burn(msg.sender, redeemTokens);
