...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //   1. The pool's `accMegaPerShare` (and `lastRewardSecond`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 lastRewardSecond;  // Last second that MEGAs distribution occurs.
    ...
    // MEGA tokens created per second.
    uint256 public megaPerSecond;
    ...
    // The block timestamp when MEGA farming starts.
    ...
    event UpdateEmissionRate(address indexed user, uint256 megaPerSecond);
    ...
    constructor(
        ...
        address _devaddr,
        address _feeAddress,
        // uint256 _MegaPerTime,
        uint256 _startTime
    ) public {
        mega = _mega;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        megaPerSecond = EMISSION_RATE;
    ...
    function pendingMega(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardSecond && pool.lpSupply != 0 && totalAllocPoint > 0) {
            ...
            uint256 megaReward = multiplier.mul(megaPerSecond).mul(pool.allocPoint).div(totalAllocPoint);
            accMegaPerShare = accMegaPerShare.add(megaReward.mul(1e18).div(pool.lpSupply));
        }
        uint256 pending = user.amount.mul(accMegaPerShare).div(1e18).sub(user.rewardDebt);
    ...
    function payOrLockupPending(uint256 _pid) internal {
        ...
        if (canHarvest(_pid, msg.sender)) {
            if (pending > 0 || user.rewardLockedUp > 0) {
                ...
                safeMegaTransfer(msg.sender, totalRewards);
            }
        } else if (pending > 0) {
    ...
    // Safe test transfer function, just in case if rounding error causes pool to not have enough MEGAs.
    ...
    function updateEmissionRate(uint256 _megaPerSecond) external onlyOwner {
    //    require(_MegaPerTime <= MAX_EMISSION_RATE, "Too high");
        massUpdatePools();
        megaPerSecond = _megaPerSecond;
        emit UpdateEmissionRate(msg.sender, _megaPerSecond);
    }
