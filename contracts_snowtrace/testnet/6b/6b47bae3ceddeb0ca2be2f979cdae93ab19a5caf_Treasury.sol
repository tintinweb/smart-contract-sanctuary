...
interface ICESTA is IERC20Upgradeable{
...
contract Treasury is Initializable, OwnableUpgradeable {
    ...
    using SafeERC20Upgradeable for ICESTA;
    ...
    uint public cestaPrice; //should not be used as oracle
    ...
    function initialize(address _cesta,         
        address _DAI,
        address owner_, uint cestaPrice_) external initializer {
        __Ownable_init();

        cestaPrice = cestaPrice_;
        CESTA = ICESTA(_cesta);
    ...
    function updateCestaPrice(uint cestaPrice_) external onlyOwner{
        cestaPrice = cestaPrice_;
    }
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        send_ = (value -  _profit) * 1e18 / cestaPrice;
        CESTA.mint( msg.sender, send_ );
    ...
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        CESTA.burnFrom( msg.sender, value );
    ...
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        value_ = _amount * ( 10 ** CESTA.decimals() ) / ( 10 ** IToken( _token ).decimals() );
    }

    function excessReserves() public view returns ( uint ) {
        return totalReserves - (CESTA.totalSupply() * cestaPrice / 1e18);
    }
