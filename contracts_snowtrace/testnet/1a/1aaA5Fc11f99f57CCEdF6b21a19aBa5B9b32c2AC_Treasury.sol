...
contract Treasury is Ownable {
    ...
    function buyERC20(IERC20 _spendingToken, uint256 _amount) public view returns(uint256){
        // alırken her hangi bir fee yok. Sadece exclude edilmez ise transfer fee olacak.
        require(msg.sender != address(0), "sender zero");
        ...
        uint256 costToken = spendingPrice.mul(cost);
    ...
    function pathMaker(IERC20 _f, IERC20 _s) internal view returns(address[] memory) {
        address[] memory path;
        path = new address[](2);
        path[0] = address(_f);
        path[1] = address(_s);
