...
contract Storage is Owner {
    ...
    mapping (address => FileHash[] ) private _hashFiles;

    function check_file (address account) public returns (FileHash[] memory){
        return _hashFiles[account];
    }
    ...
    function saveHash(string memory hash, string memory key) external isAllowed{
