...
contract Counterfeit is ERC20, Ownable {
  // a mapping from an address to whether or not it can mint / burn
  mapping(address => bool) controllers;
  // Swap tax / gold => Counterfeit
  uint256 public SwapTax = 20;
  uint256 public Ratio = 15;
  // Gold address
  address gold_address = 0x9065c2aC793FfBc5010Fac8Ad949EB4C64F0C414;
  ...
  constructor() ERC20("Counterfeit", "COUNTERFEIT") {}
  ...
  /**
   * swap $GOLD for Counterfeit
   * @param amount the amount of $GOLD to swap
   */
  function swapGold(uint256 amount) external {
    uint goldBalance = GoldCall.balanceOf(msg.sender);
    require(goldBalance >= amount, "not enough funds");
    GoldCall.burn(msg.sender, amount);

    uint taxTokenUnstaked = getAllTokenUnstaked(msg.sender);
    uint taxTokenStaked = getAllTokenStaked(msg.sender);
    uint TAX = (taxTokenUnstaked + taxTokenStaked) / 2;
    uint part = 100 - TAX;

    _mint(msg.sender, amount * part / 100);
  }
  ...
  function setSwapTax(uint _newTax) external onlyOwner {
    SwapTax = _newTax;
  }
  ...
  */
  ...
  function getAllTokenUnstaked(address sender) public view returns (uint) {
    uint256 allIndians;
    uint256 allCowboys;
    ...
    if (allCowboys == 0) allCowboys = 1;
    uint tax = Ratio / (allIndians / allCowboys);
    if (tax > 100) tax = 80;
    return tax;
  }

  function getAllTokenStaked(address sender) public view returns (uint) {
    uint256 allIndians;
    uint256 allCowboys;
