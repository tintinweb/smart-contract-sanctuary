...
interface IUniswapV2Pair is IUniswapV2ERC20 {
    function getReserves() external view returns (uint112 reserve0, uint112 reserve1, uint32 blockVoltstampLast);
...
contract VoltBondingCalculator is IBondingCalculator {
    ...
    function markdown( address _pair ) external view returns ( uint ) {
        ...
        uint reserve;
        if ( IUniswapV2Pair( _pair ).token0() == address(Volt) ) {
            reserve = reserve1;
        } else {
            require(IUniswapV2Pair( _pair ).token1() == address(Volt), "not a Volt lp pair");
            reserve = reserve0;
        }
        return reserve.mul( 2 * ( 10 ** Volt.decimals() ) ).div( getTotalValue( _pair ) );
    }
}