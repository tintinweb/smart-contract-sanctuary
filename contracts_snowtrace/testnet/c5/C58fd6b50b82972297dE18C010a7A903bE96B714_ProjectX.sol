...
contract ProjectX is ERC20, Ownable, PaymentSplitter {
    ...
    function _transfer(
        ...
        _isExcluded[from] ||
        _isExcluded[to] ||
        (from == pair && to == address(uniswapV2Router))
        ) {
            super._transfer(from, to, amount);
            return;
        }

        if (from != pair && from != address(uniswapV2Router)) {
            uint256 smoothingTokens = amount.mul(sellFee).div(100);
            super._transfer(from, smoothingReserveAddress, smoothingTokens);
            amount = amount.sub(smoothingTokens);
            super._transfer(from, to, amount);
        }
        ...
        /*
        uint256 transferFeeRate = (to == uniswapV2Pair) ? sellFee : 0;

        if (
            transferFeeRate > 0 &&
            from != address(this) &&
            to != address(this) &&
            !_isExcluded[from] && from != address(uniswapV2Router)
        ) {
            uint256 smoothingTokens = tempAmount.mul(sellFee).div(100);
            super._transfer(from, smoothingReserveAddress, smoothingTokens);
            tempAmount = tempAmount.sub(smoothingTokens);
        }
...
//sell
if (!swapping && to == uniswapV2Pair && sellFeeEnabled) {
 ...
 if (swapAmountOk) {
     ...
     uint256 smoothingTokens = amount.mul(sellFee).div(100);
     //amount = amount.sub(smoothingTokens);
     //swapAndSendToFee(smoothingReserveAddress, smoothingTokens);
     //swapTokensForEth(1000000000000000000);
     super._transfer(from, smoothingReserveAddress, smoothingTokens);
...
=======
                swapping = true;
>>>>>>> 1c4f58282ff016637b06dbb4b610480627b5bffb
        ...
        if ( (from==uniswapV2Pair || to==uniswapV2Pair) && !(_isExcluded[from] || _isExcluded[to]) ) {
            //takeFee = true;
    ...
    function withdrawStakingPosition(uint256 index) public {
        ...
        require(
            ...
            "You are not eligible to claim this position yet"
        );
        require(
            position.balance > 0,
            "There is nothing to claim from this position"
        );
    ...
    function createNodeWithStakePosition(string memory name, uint256 stakeDays)
    public
    {
        require(stakingEnabled, "Staking disabled");
        ...
        require(
            stakeDays >= 1 && stakeDays <= 30,
            "Stake time must be between 1 and 30 days"
        );
        ...
        uint256 nodePrice = nodeRewardManagement.nodePrice();
        require(
            balanceOf(sender) >= nodePrice + nodeFee,
            "NODE CREATION: Balance too low for creation."
        );
        uint256 contractTokenBalance = balanceOf(address(this));
    ...
    function changeSellFeeEnabled(bool newVal) public onlyOwner {
        sellFeeEnabled = newVal;
    }
