   ...
   function mintToken(address add) public returns (bool) {  //admin only
       ...
       if(stopTheMinting) {
           ...
           uint256 noonerCoinExtractedFromWei = 0;
            ...
            if(timeDiff >= _cycleTime){  
                ...
                uint256 estimatedTokenWei = 840 * mintingRateNoonerWei;
                uint256 temp = estimatedTokenWei/10**18;
                uint256 weiVariance = estimatedTokenWei - (temp * 10**18);
                uint256 tokens = 0;
                if(totalCycleLeft != 19){
                    tokens =  temp + previousCyclesTotalTokens + 840 * mintingRateNoonerCoin;
                }
                if(totalCycleLeft == 19){
                    tokens =  _fundersAmount + 840 * mintingRateNoonerCoin;
                }
                ...
                if(totalCycleLeft == 0) {
                    stopTheMinting = true;
                    totalMintedTokens = (noonercoin[adminAddress] -_fundersAmount)*10**18 + noonerwei[adminAddress] + totalWeiBurned;
                }
                else {
    ...
    function changeConfigVariable() public returns (bool){
        ...
        if(stopTheMinting) {
            ...
            //a checking conditions if all tokens minted are equal or not
            uint256 estimatedTokenWei = 604800 * mintingRateNoonerWei;
            uint256 temp = estimatedTokenWei/10**18;
            uint256 weiVariance = estimatedTokenWei - (temp * 10**18);
            uint256 tokens = 0;
            if(totalCycleLeft != 19){
                tokens =  temp + previousCyclesTotalTokens + 604800 * mintingRateNoonerCoin;
            }
            if(totalCycleLeft == 19){
                tokens =  _fundersAmount + 604800 * mintingRateNoonerCoin;
            }
            if(noonercoin[adminAddress] != tokens) {
                noonercoin[adminAddress] = tokens;
            } 
