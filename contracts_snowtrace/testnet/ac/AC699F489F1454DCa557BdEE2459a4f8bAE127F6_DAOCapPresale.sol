...
contract DAOCapPresale is Ownable {
    ...
    function buyTokens(address addr, uint256 _amount) private {
        uint256 tokens = usdToTokens(avaxToUsd(_amount));
        
        users[addr].tokenBalance = users[addr].tokenBalance.add(tokens);
        require(users[addr].tokenBalance <= maxLimit, "you have exceeded max limit");

        users[addr].amountInvestedInPreSale = users[addr].amountInvestedInPreSale.add(_amount);
        users[addr].isInvestedInPreslae = true;
    ...
    function withdrawTokenByOwner()external onlyOwner returns(bool){
        token.transfer(owner(), token.balanceOf(address(this)));
        return true;
    }
    
    function withdrawFundByOwner()external onlyOwner returns(bool){
