...
contract SimplePriceOracle is PriceOracle {
    ...
    function getUnderlyingPrice(SToken sToken) public view returns (uint) {
        if (compareStrings(sToken.symbol(), "sBNB")) {
            return 1e18;
        } else {
            return prices[address(SBep20(address(sToken)).underlying())];
        }
    }

    function setUnderlyingPrice(SToken sToken, uint underlyingPriceMantissa) public {
        address asset = address(SBep20(address(sToken)).underlying());
...
/**
 * @title Thermo's SBep20 Contract
 * @notice STokens which wrap an EIP-20 underlying
 * @author Venus (Modified by Source)
 */
contract SBep20 is SToken, SBep20Interface {
    /**
    ...
    /**
     * @notice Sender supplies assets into the market and receiver receives sTokens in exchange
     * @dev Accrues interest whether or not the operation succeeds, unless reverted
     * @param receiver the account which is receiving the sTokens
     * @param mintAmount The amount of the underlying asset to supply
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
...
/**
 * @title Thermo's SToken Contract
...
contract SToken is STokenInterface, Exponential, TokenErrorReporter {
    ...
    function mintFresh(address minter, uint mintAmount) internal returns (uint, uint) {
         ...
         *  Note: The sToken must handle variations between BEP-20 and BNB underlying.
     ...
     * @notice Sender supplies assets into the market and receiver receives sTokens in exchange
     * @dev Accrues interest whether or not the operation succeeds, unless reverted
     * @param receiver The address of the account which is receiving the sTokens
     * @param mintAmount The amount of the underlying asset to supply
     * @return (uint, uint) An error code (0=success, otherwise a failure, see ErrorReporter.sol), and the actual mint amount.
     */
    ...
    /**
     * @notice Payer supplies assets into the market and receiver receives sTokens in exchange
     * @dev Assumes interest has already been accrued up to the current block
     * @param payer The address of the account which is paying the underlying token
     * @param receiver The address of the account which is receiving sToken
     * @param mintAmount The amount of the underlying asset to supply
     * @return (uint, uint) An error code (0=success, otherwise a failure, see ErrorReporter.sol), and the actual mint amount.
     */
    function mintBehalfFresh(address payer, address receiver, uint mintAmount) internal returns (uint, uint) {
        ...
        /*
         * We calculate the new total supply of sTokens and receiver token balance, checking for overflow:
         *  totalSupplyNew = totalSupply + mintTokens
...
contract SBep20Interface is SBep20Storage {
...
/**
  * @title Thermo's InterestRateModel Interface
  * @author Venus (Modified by Source)
  */
