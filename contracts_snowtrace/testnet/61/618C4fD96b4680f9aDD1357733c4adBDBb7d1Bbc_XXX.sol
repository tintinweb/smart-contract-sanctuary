...
contract XXX is Context, IERC20, Ownable {
  ...
  address public assBurnAddress = 0x000000000000000000000000000000000000dEaD; 
  ...
  constructor(
    ...
    uint256 presale = bunghole.mul(25).div(100);

    // 59% to deployer for LP addition
    uint256 lp = bunghole.mul(45).div(100);

    // 10% to the DAO
    uint256 dao = bunghole.div(10);

    // 1% assBurn
    uint256 assBurn = bunghole.div(100);
    ...
    _reserveTokenBalance[assBurnAddress] = assBurn.mul(rate);
  ...
  function applyExternalTransactionTax(
    ...
    _reserveTokenBalance[assBurnAddress] = _reserveTokenBalance[assBurnAddress].add(reserveValues.reserveTransferAmountbungassBurn);
    emit Transfer(
      sender,
      assBurnAddress,
      transactionValues.bungassBurn
    );     
  }
  ...
  function _getTValues(
    ...
    // 0.5% assBurn.
    transactionValues.bungassBurn = transferAmount.div(200);
    // Net transfer amount to recipient
    transactionValues.netTransferAmount = transferAmount.sub(transactionValues.transactionFee).sub(transactionValues.daoTax).sub(transactionValues.bungassBurn);
  ...
  function _getRValues(
    ...
    reserveValues.reserveTransferAmountbungassBurn = transactionValues.bungassBurn.mul(currentRate);
    reserveValues.reserveTransferAmount = reserveValues.reserveAmount.sub(
      reserveValues.reserveFee
      ).sub(
        reserveValues.reserveTransferAmountDao
        ).sub(
          reserveValues.reserveTransferAmountbungassBurn
        );
