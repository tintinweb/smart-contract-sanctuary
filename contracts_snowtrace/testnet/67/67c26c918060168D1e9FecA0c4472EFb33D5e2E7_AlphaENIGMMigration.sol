...
contract AlphaENIGMMigration is Ownable {
  ...
  function available(address _user, uint256 _fakeTimestamp) public view returns (uint256) {
    uint256 timestamp = _fakeTimestamp;
    if (timestamp < steps[0]) {
      return 0;
    }
    uint256 claimed = 20;
    uint256 claimable = 80;
    uint256 originalBalance = aENIGM.balanceOf(_user) / (100 - claimed) * 100;
    return originalBalance * claimable / 100;
  }

  function migrate(uint256 amount) external {
    require(
      aENIGM.balanceOf(msg.sender) >= amount,
      "amount above user balance"
    );

    aENIGM.transferFrom(msg.sender, address(this), amount);
    senderInfo[msg.sender] = senderInfo[msg.sender].add(amount);
    ENIGM.transfer(msg.sender, amount);
  }

  function reclaim() external {
    require(senderInfo[msg.sender] > 0, "user has no aENIGM to withdraw");

    uint256 amount = senderInfo[msg.sender];
    senderInfo[msg.sender] = 0;
    aENIGM.transfer(msg.sender, amount);
  }

  function withdraw() external onlyOwner {
    require(block.number > swapEndBlock, "swapping of aENIGM has not ended");
