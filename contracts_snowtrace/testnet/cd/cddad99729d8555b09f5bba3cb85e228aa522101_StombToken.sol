...
contract StombToken is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 20000 ether;
    // Initial distribution for the day 2-5 Stomb-WETH LP -> Stomb pool
    uint256 public constant INITIAL_Stomb_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the Stomb ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Snowtomb Token", "STOMB") {
        // Mints 1 Stomb to contract creator for initial pool setup
    ...
    function _getStombPrice() internal view returns (uint256 _StombPrice) {
        try IOracle(StombOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Stomb: failed to fetch Stomb price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _StombPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_StombPrice >= taxTiersTwaps[tierId]) {
    ...
    function setStombOracle(address _StombOracle) public onlyOperatorOrTaxOffice {
        require(_StombOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints Stomb to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of Stomb to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentStombPrice = _getStombPrice();
            currentTaxRate = _updateTaxRate(currentStombPrice);
            if (currentStombPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_StombPool != address(0), "!_StombPool");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_StombPool, INITIAL_Stomb_POOL_DISTRIBUTION);
    }
