...
contract SaleAvat is Ownable, ReentrancyGuard {
    ...
    constructor (address[] memory depositTokens_, uint[] memory rates_, address withdrawToken_)
    {
     ...
     */
    function createDistributionPeriod(DistributionPeriod[] memory distributionPeriods_, uint endSale_, uint precision_)
    external
    onlyOwner
    {
        ...
        for(uint i = 0 ; i < periodsLength; i++) {
            ...
            distributionDates[i] = distributionPeriods_[i].timestamp;
        }

        require(amountPercent.div(_precision) == 100, "SaleAvat::createDistributionPeriod: Total percentage should be 100%");

        periods = periodsLength;
    }

    /**
    * @notice Buy token.
    */
    function buyToken(address addressTransferFrom, uint amountTransferFrom, uint nonce, uint deadline, uint8 v, bytes32 r, bytes32 s)
    external
    {
        ...
        uint amount = _rateDepositTokens[addressTransferFrom].mul(amountTransferFrom).div(100);
    ...
    function permitBySig(address participant, uint rawAmount, uint nonce, uint deadline, uint8 v, bytes32 r, bytes32 s) internal returns (bool) {
        ...
        require(signatory != address(0), "SaleAvat::permitBySig: invalid signature");
        require(nonce > nonces[signatory], "SaleAvat::permitBySig: invalid nonce");
        require(signatory == this.owner(), "SaleAvat::permitBySig: unauthorized");
        require(block.timestamp <= deadline, "SaleAvat::permitBySig: signature expired");
        nonces[signatory] = nonce;
    ...
    */
    function createDistributionAddress(uint amount_, address participant_)
    internal
    {
        ...
        for(uint i = 0 ; i < periods; i++) {
            _amountPercent = amount_.mul(_distributionPeriods[i].percent).div(100).div(_precision);
            DistributionAddress memory distributionAddress = DistributionAddress({amount: _amountPercent, isClaimed: false });
            _distributionAddresses[participant_][i] = distributionAddress;
        }
    }
    ...
    */
    function claimToken(uint periodId_)
    external
    {
        ...
        require(periodId_ < periods, "SaleAvat::claimToken: PeriodId should be lesser then total periods");
    ...
    */
    function collectToken(address token_, uint amount_, address ownerAddress_)
    external
    onlyOwner
    {
    ...
    * @param ownerAddress_ Address owner
    */
    function leftover(uint amount_, address ownerAddress_)
    external
    onlyOwner
    {
        address payable addressPayable = payable(ownerAddress_);
    ...
    function isPeriodUnlocked(uint periodId_)
    public
    view
    returns (bool)
    {
        return block.timestamp >= distributionDates[periodId_];
    }
