...
contract AlphaENIGMMigration is Ownable {
  ...
  function available() external returns (uint256) {
    ...
    if (startBalance[msg.sender] == 0) {
      // startBalance[msg.sender] = aENIGM.balanceOf(msg.sender) ;
      startBalance[msg.sender] = 400;
    }
    ...
    for (uint256 i = steps.length - 1; i > 0; i--) {
      ...
      if (lastMigration[msg.sender] < step && timestamp > step) {
        claimable += (startBalance[msg.sender] * percentPerStep[i]) / 100;
      }
    }
    return claimable;
  }
  ...
  function setVestingInfos(uint256[] calldata _steps, uint256[] calldata _percentPerStep) public onlyOwner {
    delete steps;
