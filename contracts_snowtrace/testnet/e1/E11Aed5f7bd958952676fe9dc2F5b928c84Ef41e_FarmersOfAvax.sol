/**
 *Submitted for verification at testnet.snowtrace.io on 2021-12-02
*/
...
contract FarmersOfAvax is ERC721Enumerable, Ownable {
  ...
  uint256 public farmersRemainingToAssign = 2500;
  ...
  function mintNFT(uint256 amount) public payable {
    ...
    for (uint i = 0; i < amount; i++) {
        ...
        if(supply > 0) {
            giftAddress = payable(ownerOf(mintedTokens[randomNum(currentSupply, block.timestamp, currentSupply + 1)]));
            giftValue = SafeMath.div(cost, 10); // cost * 10 / 100;
        }
        ...
        mintedTokens[currentSupply] = farmerIndex;
    }
