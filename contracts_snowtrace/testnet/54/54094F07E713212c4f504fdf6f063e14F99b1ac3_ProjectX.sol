...
contract ProjectX is ERC20, Ownable, PaymentSplitter {
    ...
    address public distributionPoolAddress;
    ...
    constructor(
        ...
        require(addresses[0] != address(0) && addresses[1] != address(0), "FUTURE & REWARD ADDRESS CANNOT BE ZERO");
        distributionPoolAddress = addresses[0];
        operationPoolAddress = addresses[1];
        ...
        totalFees = distributionPoolFee.add(liquidityPoolFee).add(futureFee);
    ...
    function updateRewardsFee(uint256 value) external onlyOwner {
        distributionPoolFee = value;
        totalFees = distributionPoolFee.add(liquidityPoolFee).add(operationPoolFee);
    }
    ...
    function createNodeWithTokens(uint numberOfNodes) public {
        for (uint256 i = 0; i < numberOfNodes; i++) {
            ...
            require(sender != operationPoolAddress && sender != distributionPoolAddress, "NODE CREATION: futur and rewardsPool cannot create node");
            ...
            if (
                ...
                // reward pool
                uint256 rewardsPoolTokens = contractTokenBalance.mul(distributionPoolFee).div(100);
                super._transfer(address(this), distributionPoolAddress, rewardsPoolTokens);
    ...
    function createNodeWithStakePosition(string memory name, uint256 stakeDays) public {
        ...
        if (
            ...
            swapAndSendToFee(distributionPoolAddress, balanceOf(address(this)));
    ...
    function cashoutReward(uint256 blocktime) public {
        ...
        require(
            sender != operationPoolAddress && sender != distributionPoolAddress,
        ...
        if (swapLiquify && cashoutFee > 0) {
            super._transfer(distributionPoolAddress, address(this), feeAmount);
            swapAndSendToFee(operationPoolAddress, feeAmount);
        }
        super._transfer(distributionPoolAddress, sender, rewardAmount);
