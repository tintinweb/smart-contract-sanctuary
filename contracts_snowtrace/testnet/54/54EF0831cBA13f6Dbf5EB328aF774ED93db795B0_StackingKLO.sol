...
contract StackingKLO is Ownable,ReentrancyGuard {
    ...
    /// @param tokenPerSec Reward tokens per block.timestamp.
    /// @param lpSupply Total staked amount.
    /// @param accRewardsPerShare Total rewards accumulated per staked token.
    /// @param lastRewardTs Last time rewards were updated for the pool.
    /// @param endOfEpochTs End of epoc block.timestamp for compute and to avoid deposits.
    struct PoolInfo {
        uint256 tokenPerSec;
        uint256 lpSupply;
        uint256 accRewardsPerShare;
        uint256 lastRewardTs;
        uint256 endOfEpochTs;
    }
    ...
    event LogUpdatePool(uint256 indexed pid, uint256 lastRewardTs, uint256 lpSupply, uint256 accRewardsPerShare);
    event RewardsAdded(uint256 amount);
    event ExtendPool(uint256 indexed pid, uint256 rewardBlock, uint256 endOfEpochTs);
    ...
    /// @param _kloPerBlock Rewards per block.
    /// @param _endOfEpochTs Epocs end block.timestamp.
    function add(IERC20 _lpToken, uint256 _kloPerBlock, uint256 _endOfEpochTs) public onlyOwner {
        ...
        require(_endOfEpochTs > block.timestamp, "Cannot create the pool for past time.");
        ...
        tokenPerSec: _kloPerBlock,
        endOfEpochTs:_endOfEpochTs,
    ...
    /// @param _kloPerBlock Rewards per block.
    /// @param _endOfEpochTs Epocs end block.timestamp.
    function extendPool(uint256 _pid, uint256 _kloPerBlock, uint256 _endOfEpochTs) public onlyOwner {

        require(_endOfEpochTs > block.timestamp && _endOfEpochTs > poolInfo[_pid].endOfEpochTs, "Cannot extend the pool for past time.");

        // Update the accumulated rewards
        PoolInfo memory pool = updatePool(_pid);

        pool.tokenPerSec = _kloPerBlock;
        pool.endOfEpochTs = _endOfEpochTs;
        pool.lastRewardTs = block.timestamp;

        // Update the Pool Storage
        poolInfo[_pid] = pool;

        emit ExtendPool(_pid, _kloPerBlock, _endOfEpochTs);
    }

    /// @dev To get the rewards per block.
    function kloPerBlock(uint256 _pid) public view returns (uint256 amount) {
        PoolInfo memory pool = poolInfo[_pid];
        amount = pool.tokenPerSec;
    }
    ...
    function updatePool(uint256 _pid) private returns (PoolInfo memory pool) {
        ...
        if (block.timestamp > pool.lastRewardTs && pool.lastRewardTs < pool.endOfEpochTs) {

            if(lpSupply > 0){

                uint256 blocks;
                if(block.timestamp < pool.endOfEpochTs) {
                    blocks = block.timestamp.sub(pool.lastRewardTs);
                } else {
                    blocks = pool.endOfEpochTs.sub(pool.lastRewardTs);
                }

                uint256 kloReward = blocks.mul(kloPerBlock(_pid));
                pool.accRewardsPerShare = pool.accRewardsPerShare.add((kloReward.mul(ACC_REWARDS_PRECISION) / lpSupply));

            }

            pool.lastRewardTs = block.timestamp;
            poolInfo[_pid] = pool;
            emit LogUpdatePool(_pid, pool.lastRewardTs, lpSupply, pool.accRewardsPerShare);
        }
    }
    ...
    function pendingRewards(uint256 _pid, address _user) external view returns (uint256 pending) {
        ...
        if (block.timestamp > pool.lastRewardTs && pool.lastRewardTs < pool.endOfEpochTs) {
            ...
            if(lpSupply > 0){
                ...
                uint256 kloReward = blocks.mul(kloPerBlock(_pid));
                accRewardsPerShare = accRewardsPerShare.add(kloReward.mul(ACC_REWARDS_PRECISION) / lpSupply);
    ...
    function deposit(uint256 _pid, uint256 _amount, address _to) external nonReentrant {
        ...
        require (pool.endOfEpochTs > block.timestamp,"This pool epoch has ended. Please join staking new session.");
