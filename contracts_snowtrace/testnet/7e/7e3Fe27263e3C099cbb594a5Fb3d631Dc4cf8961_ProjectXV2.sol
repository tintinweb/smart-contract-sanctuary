...
// INIT
contract ProjectXV2 is Context, IERC20, Ownable {
    ...
    string private constant _name = "ProjectXV2";
    string private constant _symbol = "PXTv2";
    ...
    function doBuyBack() private lockTheSwap {
        ...
        uint256 operationsAvaxBalance = avaxBalance.mul(_operationsTokensToSwap).div(totalTokensToSwap);

        _liquidityTokensToSwap = 0;
        _operationsTokensToSwap = 0;

        payable(operationPoolAddress).transfer(operationsAvaxBalance);
    ...
    function _transferStandard(
        ...
        uint256 tFee,
        uint256 _tOperations
        ) = _getValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
        _takeLiquidity(_tOperations);
    ...
    )
    {
        ...
        uint256 _tOperations = calculateLiquidityFee(tAmount);
        uint256 tTransferAmount = tAmount.sub(tFee).sub(_tOperations);
        return (tTransferAmount, tFee, _tOperations);
    }
    ...
    )
    {
        ...
        uint256 rLiquidity = _tOperations.mul(currentRate);
    ...
    onlyOwner
    {
        ...
        _buyOperationsFee = buySmoothingFee;
    ...
    onlyOwner
    {
        ...
        _sellOperationsFee = sellSmoothingFee;
