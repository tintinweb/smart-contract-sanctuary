...
// File contracts/StakingDoubleRewards.sol
...
contract StakingDoubleRewards is Ownable {
    ...
    address public rewardTokenPNG;
    uint256 public totalShares; 
    uint256 public lpPerShare = 1e18;
    uint256 public pangolinPid; // 7 = avax usdte
    ...
    constructor(
        ...
        //stakingToken.safeApprove(address(pangolinMiniChef), MAX_UINT);
        rewardTokenPNG = 0x60781C2586D68229fde47564546784ab3fACA982; //PNG token
    ...
    function stake(uint256 amount) external onlyApprovedContractOrEOA updateReward(msg.sender) {
        ...
        if (amount > 0) {
            pangolinMiniChef.deposit(pangolinPid, amount, address(this));
        }
        if (newShares > 0) {
            increaseRewardDebt(msg.sender, rewardTokenPNG, newShares);
        }
    ...
    function withdraw(uint256 amountShares) external onlyApprovedContractOrEOA updateReward(msg.sender) {
        ...
        if (amountShares > 0) {
            ...
            withdrawals[pangolinPid][msg.sender] += lpFromShares;
        ...
        if (lpFromShares > 0) {
            pangolinMiniChef.withdraw(pangolinPid, lpFromShares, msg.sender);
        }
        if (amountShares > 0) {
            decreaseRewardDebt(msg.sender, rewardTokenPNG, amountShares);
        }
    ...
    function _claimRewards() internal {
        ...
        if (updateAndClaim && totalShares > 0) {
            ...
            balancesBefore = _checkBalance(rewardTokenPNG);

            pangolinMiniChef.harvest(pangolinPid, address(this));

            uint256 balanceDiff = _checkBalance(rewardTokenPNG) - balancesBefore;
            if (balanceDiff > 0) {
                increaseRewardTokensPerShare(rewardTokenPNG, (balanceDiff * ACC_PRECISION) / totalShares);
            }  
        }
    }
    ...
    function _harvest() internal {
        ...
        address rewardToken = rewardTokenPNG;
    ...
    function pendingRewards(address user) public view returns (uint256, uint256) {
        return (earned(user), pendingRewardPng(user));
    }

    function pendingRewardPng(address user) public view returns (uint256) {
