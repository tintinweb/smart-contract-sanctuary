...
library ECDSA {
    ...
    event DomainSeparator(bytes32 domainSeparator,bytes32 structHash);
    ...
    function toTypedDataHash(bytes32 domainSeparator, bytes32 structHash) internal returns (bytes32) {
        emit DomainSeparator(domainSeparator, structHash);
...
abstract contract EIP712 {
    ...
    event ChaindIds(uint256 chainId,uint256 cachedId);
    ...
    function _domainSeparatorV4() internal returns (bytes32) {
        emit ChaindIds(block.chainid, _CACHED_CHAIN_ID);
