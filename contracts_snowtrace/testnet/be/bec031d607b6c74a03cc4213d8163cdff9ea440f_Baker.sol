...
// File contracts/Cookie.sol

pragma solidity ^0.8.4;


contract Cookie is ERC20("Cookie", "COOKIE"), Ownable {
...
// File contracts/Baker.sol

pragma solidity ^0.8.4;

contract Baker is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    uint256 public constant BASE_MINT_PRICE = 0.6 ether; // 1.5 AVAX
    uint256 public constant PRESALE_MINT_PRICE = 0.4 ether; // 1.25 AVAX
    uint256 public constant BASE_ROBOT_PERCENTAGE = 20;
    uint256 public constant BASE_ALIEN_PERCENTAGE = 6;
    ...
    string public constant BASE_URI = "https://burger-game.com/tokens/";

    constructor() ERC721("Cookie Baker", "COOKIE-BAKER") {
        // supply and price are ignored for the base levels
        levels.push(Level({ supply: 0, maxSupply: 0, price: 0, yield: 1 })); // regular chef
        levels.push(Level({ supply: 0, maxSupply: 0, price: 0, yield: 3 })); // robot
        levels.push(Level({ supply: 0, maxSupply: 0, price: 0, yield: 6 })); // alien
    }

    /* Minting of base bakers */
    ...
    function _mintBaseTokens(uint16 _numTokens, address _for) internal {
        ...
        for (uint i = 0; i < _numTokens; i++) {
            ...
            if (_rand(totalSupply()) % 100 < BASE_ALIEN_PERCENTAGE) {
                tokenLevel[tokenId] = 2; // alien
            } 
            else if (_rand(totalSupply()) % 100 < BASE_ROBOT_PERCENTAGE) {
                tokenLevel[tokenId] = 1; // robot
            }else {
                tokenLevel[tokenId] = 0; // normal baker
            }
        }
    }

    /* Minting of upgrade bakers */
    ...
    function mintUpgrade(uint256 _level, uint16 _numTokens) external {
        ...
        require(cookie.balanceOf(msg.sender) >= totalCost, "Insufficient COOKIE balance");
        cookie.burn(msg.sender, totalCost);
    ...
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
        ...
        } else if (revealedTokenLevel(_tokenId) == 2) {
            return string(abi.encodePacked(BASE_URI, _tokenId.toString(), "-alien.json"));
        } else if (revealedTokenLevel(_tokenId) == 1) {
            return string(abi.encodePacked(BASE_URI, _tokenId.toString(), "-robot.json"));
        } else {
            return string(abi.encodePacked(BASE_URI, _tokenId.toString(), "-baker.json"));
        }
    }
    ...
    function batchedBakersOfOwner(
        address _owner,
        uint256 _offset,
        uint256 _maxSize
    ) public view returns (BakerInfo[] memory) {
        if (_offset >= balanceOf(_owner)) {
            return new BakerInfo[](0);
        }
        ...
        BakerInfo[] memory bakers = new BakerInfo[](outputSize);
    ...
    function setBakeryAddress(address _bakeryAddress) external onlyOwner {
        require(bakeryAddress == address(0), "Bakery address already set");
        bakeryAddress = _bakeryAddress;
    }

    function setCookie(Cookie _cookie) external onlyOwner {
        cookie = _cookie;
    }
