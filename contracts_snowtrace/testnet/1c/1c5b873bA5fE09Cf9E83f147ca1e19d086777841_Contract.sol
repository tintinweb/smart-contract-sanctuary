...
contract Contract is IERC20, Ownable {
    ...
    function getPair() private view returns (address) {
        return IJoeFactory(router.factory()).getPair(address(this), router.WAVAX());
    }
    ...
    function _transfer(
        ...
        if (!inSwapAndLiquify && from != getPair() && from != address(router) && !_isExcludedFromFee[from]) {
        ...
        if (overMinTokenBalance && !inSwapAndLiquify && from != getPair() && swapAndLiquifyEnabled) swapAndLiquify(contractTokenBalance);
