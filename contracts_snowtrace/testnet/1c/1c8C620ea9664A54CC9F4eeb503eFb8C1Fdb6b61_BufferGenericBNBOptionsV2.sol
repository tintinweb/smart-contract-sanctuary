...
// Part: IBufferOptionsV2

interface IBufferOptionsV2 {
    event Create(
        uint256 indexed id,
...
// File: BufferGenericBNBOptionsV2.sol
 ...
 */
contract BufferGenericBNBOptionsV2 is
    IBufferOptionsV2,
    Ownable,
    ERC721URIStorage,
    ERC721Enumerable,
    ...
    ReentrancyGuard
{
    ...
    mapping(uint256 => uint256) public optionAssetAmounts;
    ...
    constructor(
        ...
        AggregatorV3Interface bnbPP,
        IBufferStakingBNB staking,
        BufferBNBPool _pool
    ) ERC721("Buffer", "BFR") {
        pool = _pool;
        priceProvider = pp;
        bnbPriceProvider = bnbPP;
     ...
     * @param amount Option amount in BNB
     * @param optionAssetAmount Option amount in currency the option is being sold for
    ...
    function create(
        ...
        require(IsInOptionCreationWindow(), "Creation isn't allowed in this time window");
        ...
        uint256 lockedAmount = ((strikeAmount * optionCollateralizationRatio) /
            100) + strikeFee;
        ...
        optionAssetAmounts[optionID] = optionAssetAmount;
    ...
    function payProfit(uint256 optionID) internal returns (uint256 profit) {
        ...
        uint256 _optionAssetAmount = optionAssetAmounts[optionID];

        (, int256 latestPrice, , , ) = priceProvider.latestRoundData();
        uint256 currentPrice = uint256(latestPrice);

        (, int256 bnbLatestPrice, , , ) = bnbPriceProvider.latestRoundData();
        uint256 bnbCurrentPrice = uint256(bnbLatestPrice);

        if (option.optionType == OptionType.Call) {
            require(option.strike <= currentPrice, "Current price is too low");
            profit =
                ((currentPrice - option.strike) * _optionAssetAmount) /
                bnbCurrentPrice;
        } else {
            require(option.strike >= currentPrice, "Current price is too high");
            profit =
                ((option.strike - currentPrice) * _optionAssetAmount) /
                bnbCurrentPrice;
        }
        ...
        returns (uint256 stakingAmount)
    {
        ...
        if (adminFee > 0) {
            if (
                ...
                uint256 referralReward = (adminFee * referralRewardPercentage) /
                    100;
