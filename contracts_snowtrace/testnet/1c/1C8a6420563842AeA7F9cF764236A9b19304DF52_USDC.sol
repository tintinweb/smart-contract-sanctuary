...
contract USDC is LibNote {
  ...
  event Transfer(address indexed src, address indexed dst, uint wad);
    ...
    constructor(uint256 chainId_) {
        ...
        usdclyUSDCLimit = 10000000000000000000000;
    }
    ...
    function adjustUsdclyUSDCLimit(uint _limit) external auth {
        usdclyUSDCLimit = _limit;
    }

    function mint(address usr, uint wad) external {

      if(wards[msg.sender] == 0) {
        require(add(wad, usdcMintedToday[msg.sender]) <= usdclyUSDCLimit || sub(block.number, lastMintRestart[msg.sender]) >= 6500 && wad <= usdclyUSDCLimit, "Over usdcly USDC Limit");
