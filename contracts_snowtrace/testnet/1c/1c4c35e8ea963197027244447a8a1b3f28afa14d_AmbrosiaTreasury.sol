...
contract AmbrosiaTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SAMB }
    ...
    address public sAMB;
    uint public sAMBQueue; // Delays change to sAMB address
    ...
    /**
        @notice allow approved address to deposit an asset for AMB
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint AMB needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( AMB ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn AMB for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IAMBERC20( AMB ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sAMB ).balanceOf( msg.sender ); // Can only borrow against sAMB held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with AMB
        @param _amount uint
     */
    function repayDebtWithAMB( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );
        ...
        emit RepayDebt( msg.sender, AMB, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( AMB ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( AMB ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns AMB valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match AMB decimals
            value_ = _amount.mul( 10 ** IERC20( AMB ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SAMB ) { // 9
            sAMBQueue = uint32(block.timestamp).add32( secondsNeededForQueue );
        } else return false;
    ...
    /**
        ...
        @param queue_ mapping( address => uint32 )
