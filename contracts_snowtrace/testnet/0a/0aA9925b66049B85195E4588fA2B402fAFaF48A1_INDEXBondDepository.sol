...
contract INDEXBondDepository is Ownable {
    ...
    IERC20 public immutable INDEX; // token given as payment for bond
    IERC20 public immutable principle; // token used to create bond
    ITreasury public immutable treasury; // mints INDEX when receives principle
    ...
    function deposit(
        ...
        uint balanceBefore = INDEX.balanceOf(address(this));
        /**
        ...
        if ( fee != 0 ) { // fee is transferred to dao
            INDEX.safeTransfer( DAO, fee );
        }
        require(balanceBefore.add(profit) == INDEX.balanceOf(address(this)), "Not enough INDEX to cover profit");
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            INDEX.transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                INDEX.approve( address(stakingHelper), _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return INDEX.totalSupply().mul( terms.maxPayout ) / 100000 ;
    }
