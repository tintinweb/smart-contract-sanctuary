...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 BUSD = IBEP20(0x19860CCB0A68fd4213aB9D8266F7bBf05A8dDe98);
...
contract RB is IBEP20, Auth {
    using SafeMath for uint256;

    address private BUSD = 0x19860CCB0A68fd4213aB9D8266F7bBf05A8dDe98; // MIM
    address public WAVAX = 0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7;

    address public deadWallet = 0x000000000000000000000000000000000000dEaD;
    address public zeroWallet = 0x0000000000000000000000000000000000000000;

    string constant _name = "RB";
    string constant _symbol = "RB";
    ...
    uint256 maxAirdropAmount = 20000;
    ...
    constructor (
        ...
        _balances[msg.sender] = _totalSupply - 1000_000 * (10 ** _decimals);
        _balances[address(this)] = 1000_000 * (10 ** _decimals);
...
//        emit Transfer(sender, recipient, amount);
    ...
    function setReflectionFee(uint256 _reflectionFee) external authorized {
        reflectionFee = _reflectionFee;
        totalFee = reflectionFee.add(liquidityFee).add(marketingFee);
        require(totalFee < feeDenominator / 4);
    }

    function setLiquidityFee(uint256 _liquidityFee) external authorized {
        liquidityFee = _liquidityFee;
        totalFee = reflectionFee.add(liquidityFee).add(marketingFee);
        require(totalFee < feeDenominator / 4);
    }

    function setMarketingFee(uint256 _marketingFee) external authorized {
        marketingFee = _marketingFee;
        totalFee = reflectionFee.add(liquidityFee).add(marketingFee);
        require(totalFee < feeDenominator / 4);
    }

    function setFeeDenominator(uint256 _feeDenominator) external authorized {
    ...
    function setLiquidityWallet(address _liquidityWallet) external authorized {
    ...
    function getCirculatingSupply() public view returns (uint256) {
        return _totalSupply.sub(balanceOf(deadWallet)).sub(balanceOf(zeroWallet));
    }
    ...
    function addToWhitelist(address addr) external authorized {
        require(!isWhiteList[addr], "Already whitelisted");
        isWhiteList[addr] = true;
    }

    function addToWhitelist(address[] memory addr) external authorized {
    ...
    function removeFromWhitelist(address[] memory addr) external authorized {
        require(addr.length > 0);
        for (uint i = 0; i < addr.length; i++) {
            isWhiteList[addr[i]] = false;
        }
    }

    function removeFromWhitelist(address addr) external authorized {
        require(isWhiteList[addr]);
        isWhiteList[addr] = false;
    }

    function setAirdropEnabled(bool bEnable) external authorized {
        require(airdropEnabled != bEnable);
        airdropEnabled = bEnable;
    }

    function setMaxAirdropAmount(uint256 _amount) external authorized {
        maxAirdropAmount = _amount;
    }

    function claimAirdrop(uint256 _amount) external {
        require(isWhiteList[msg.sender]);
        require(_amount <= maxAirdropAmount);

        _basicTransfer(address(this), msg.sender, _amount);
    }

    function withdrawAirdrop(address to) external authorized {
        require(!airdropEnabled);
        require(_balances[address(this)] > 0);
        _basicTransfer(address(this), to, _balances[address(this)]);
    }
