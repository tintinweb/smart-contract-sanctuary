...
contract PiggyIDO is ERC20 {
    using SafeMath for uint256;
    event PiggyIDOSold(address indexed buyer, uint256 indexed amount);
    
    uint256 public totalPreSale = 3000*10**9;
    uint256 public totalPreSaleLeft = 3000*10**9;
    
    uint256 public daiPrice = 20*10**18;
    
    uint256 public maxPurchase = 100*10**9;
    mapping(address=>uint256) public BuyerQuota;
    
    constructor(string memory _name, string memory _symbol, address _DAIaddress) public payable{
        ...
        DAIERC20 candidateContract = DAIERC20(_DAIaddress);
        daiERC20 = candidateContract;
    }
    
    function setDAIAddress(address _address) external onlyCEO {
        DAIERC20 candidateContract = DAIERC20(_address);
        daiERC20 = candidateContract;
    }
    
    function setDAIPrice(uint256 _amount) public onlyCEO {
        require(_amount > 0);
        daiPrice = _amount;
    }
    ...
    function buyPiggyIDO(uint256 _amount) public whenNotPaused returns(uint256) {
        require(BuyerQuota[msg.sender].add(_amount) <= maxPurchase, "Exceed MaxPurachse");
        require(daiERC20.allowance(msg.sender, address(this)) >= daiPrice.mul(_amount).div(10**9), "Insuffcient approved DAI");
        daiERC20.transferFrom(msg.sender, address(this), daiPrice.mul(_amount).div(10**9));
        
        _mint(msg.sender, _amount);
        BuyerQuota[msg.sender] = BuyerQuota[msg.sender].add(_amount);
        
        emit PiggyIDOSold(msg.sender, _amount);
    }
