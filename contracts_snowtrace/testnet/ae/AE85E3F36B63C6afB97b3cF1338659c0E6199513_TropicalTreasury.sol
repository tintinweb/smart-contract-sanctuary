...
contract TropicalTreasury is Ownable {
    ...
    address public Coco;
    uint public COCOQueue; // Delays change to COCO address
    ...
    constructor (
        ...
        require( _Pts != address(0) );
        Pts = _Pts;

        isReserveToken[ _MIM ] = true;
        reserveTokens.push( _MIM );

        // isLiquidityToken[ _PTSMIM ] = true;
        // liquidityTokens.push( _PTSMIM );

        secondsNeededForQueue = _secondsNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for PTS
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint PTS needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( Pts ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn PTS for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IPTSERC20( Pts ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( Coco ).balanceOf( msg.sender ); // Can only borrow against COCO held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with PTS
        @param _amount uint
     */
    function repayDebtWithPTS( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IPTSERC20( Pts ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, Pts, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( Pts ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( Pts ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns PTS valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match PTS decimals
            value_ = _amount.mul( 10 ** IERC20( Pts ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.COCO ) { // 9
            COCOQueue = uint32(block.timestamp).add32( secondsNeededForQueue );
        } else return false;
