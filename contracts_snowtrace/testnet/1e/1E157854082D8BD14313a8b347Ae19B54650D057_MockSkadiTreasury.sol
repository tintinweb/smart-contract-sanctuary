...
contract MockSkadiTreasury is Ownable {
  using SafeMath for uint256;
  ...
  address public sSKI;
  uint256 public sSKIQueue; // Delays change to sSKI address
  ...
  function incurDebt(uint256 _amount, address _token) external {
    ...
    uint256 maximumDebt = IERC20(sSKI).balanceOf(msg.sender); // Can only borrow against sSKI held
  ...
  /**
        ...
        @param _amount uint
     */
  function repayDebtWithSKI(uint256 _amount) external {
    require(isDebtor[msg.sender], "Not approved");
    ...
    returns (uint256 value_)
  {
    if (isReserveToken[_token]) {
      ...
      value_ = _amount.mul(10**IERC20(SKI).decimals()).div(
    ...
    returns (bool)
  {
    ...
    } else if (_managing == MANAGING.SSKI) {
      // 9
      sSKIQueue = block.number.add(blocksNeededForQueue);
    } else return false;
