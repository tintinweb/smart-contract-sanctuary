...
contract SuperBondDepository is Ownable {
    ...
    address public immutable Super; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints Super when receives principle
    ...
    struct Bond {
        uint payout; // Super remaining to be paid
    ...
    function deposit( 
        ...
        uint value = ITreasury( treasury ).valueOf( principle, _amount );// amount of principle token without decimal
        uint payout = payoutFor( value ); // payout to bonder is computed (super)

        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 Super ( underflow protection )
        ...
        /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) Super
         */
        ...
        if ( fee != 0 ) { // fee is transferred to dao 
            IERC20( Super ).safeTransfer( DAO, fee ); 
        }
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            IERC20( Super ).transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                IERC20( Super ).approve( stakingHelper, _amount );
                IStakingHelper( stakingHelper ).stake( _amount, _recipient );
            } else {
                IERC20( Super ).approve( staking, _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return IERC20( Super ).totalSupply().mul( terms.maxPayout ).div( 100000 );
    }
    ...
    /**
     *  @notice calculate current ratio of debt to Super supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns ( uint debtRatio_ ) {   
        uint supply = IERC20( Super ).totalSupply();
    ...
    /**
     *  @notice calculate amount of Super available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or Super) to the DAO
     *  @return bool
     */
    function recoverLostToken( address _token ) external returns ( bool ) {
        require( _token != Super );
