    ...
    )
    {
        ...
        while (gasUsed < gas && iterations < numberOfnodeOwners) {
            ...
            for (uint256 i = 0; i < _nodesOfUser[account].length; i++) {
                ...
                _nodesOfUser[account][nodeIndex].rewardNotClaimed += claimableAmount(_nodesOfUser[account][i].lastClaimTime) + _nodesOfUser[account][i].rewardNotClaimed + _pendingClaimableAmount(_nodesOfUser[account][i].lastClaimTime);
                _nodesOfUser[account][nodeIndex].lastClaimTime = block.timestamp;
                claims++;
            }
            iterations++;
