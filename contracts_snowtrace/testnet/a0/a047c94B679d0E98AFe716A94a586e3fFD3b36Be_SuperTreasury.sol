...
contract SuperTreasury is Ownable {
    ...
    address public Mega;
    uint public MegaQueue; // Delays change to Mega address
    ...
    //    isLiquidityToken[ _SPRDAI ] = true;
    //    liquidityTokens.push( _SPRDAI );

        secondsNeededForQueue = _secondsNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for Super
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint Super needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( Super ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn Super for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        ISuperERC20( Super ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( Mega ).balanceOf( msg.sender ); // Can only borrow against Mega held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with Super
        @param _amount uint
     */
    function repayDebtWithSuper( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        ISuperERC20( Super ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, Super, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( Super ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( Super ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns Super valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match Super decimals
            value_ = _amount.mul( 10 ** IERC20( Super ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        if ( _managing == MANAGING.RESERVEDEPOSITOR ) { // 0
            ...
            MegaQueue = uint32(block.timestamp).add32( secondsNeededForQueue );
        } else return false;
