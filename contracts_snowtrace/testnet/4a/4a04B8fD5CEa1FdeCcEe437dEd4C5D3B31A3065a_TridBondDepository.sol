...
contract TridBondDepository is Ownable {
    ...
    address public immutable Trid; // token given as payment for bond
    ...
    function deposit(
        ...
        if (fee != 0) {
            ...
            IERC20(Trid).safeTransfer(DAO, fee);
        }
    ...
    function stakeOrSend(
        ...
        if (!_stake) {
            ...
            IERC20(Trid).transfer(_recipient, _amount); // send payout
        } else {
            // if user wants to stake
            if (useHelper) {
                // use if staking warmup is 0
                IERC20(Trid).approve(stakingHelper, _amount);
                IStakingHelper(stakingHelper).stake(_amount, _recipient);
            } else {
                IERC20(Trid).approve(staking, _amount);
    ...
    function maxPayout() public view returns (uint256) {
        return IERC20(Trid).totalSupply().mul(terms.maxPayout).div(100000);
    }
    ...
    function debtRatio() public view returns (uint256 debtRatio_) {
        uint256 supply = IERC20(Trid).totalSupply();
