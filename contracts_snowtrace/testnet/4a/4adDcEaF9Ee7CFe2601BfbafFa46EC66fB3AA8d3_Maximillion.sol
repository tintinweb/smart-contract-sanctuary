...
/**
 * @title Thermo's Maximillion Contract
 * @author Venus (modified by Source)
 */
contract Maximillion {
    /**
     * @notice The default sBnb market to repay in
     */
    SBNB public sBnb;

    /**
     * @notice Construct a Maximillion to repay max in a SBNB market
     */
    constructor(SBNB sBnb_) public {
        sBnb = sBnb_;
    }

    /**
     * @notice msg.sender sends BNB to repay an account's borrow in the sBnb market
     * @dev The provided BNB is applied towards the borrow balance, any excess is refunded
     * @param borrower The address of the borrower account to repay on behalf of
     */
    function repayBehalf(address borrower) public payable {
        repayBehalfExplicit(borrower, sBnb);
    }

    /**
     * @notice msg.sender sends BNB to repay an account's borrow in a sBnb market
     * @dev The provided BNB is applied towards the borrow balance, any excess is refunded
     * @param borrower The address of the borrower account to repay on behalf of
     * @param sBnb_ The address of the sBnb contract to repay in
     */
    function repayBehalfExplicit(address borrower, SBNB sBnb_) public payable {
        uint received = msg.value;
        uint borrows = sBnb_.borrowBalanceCurrent(borrower);
        if (received > borrows) {
            sBnb_.repayBorrowBehalf.value(borrows)(borrower);
            msg.sender.transfer(received - borrows);
        } else {
            sBnb_.repayBorrowBehalf.value(received)(borrower);
        }
    }
}
