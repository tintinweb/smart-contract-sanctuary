...
contract SaleAvat is Ownable, ReentrancyGuard {
    ...
    */
    function buyToken(address addressTransferFrom, uint amountTransferFrom, uint value, uint nonce, uint deadline, uint8 v, bytes32 r, bytes32 s) external {

        require(_createDistribution[address(msg.sender)] == false, "SaleAvat::buyToken: Participant already create distribution");

        bool accept = permitBySig(msg.sender, value, nonce, deadline, v, r, s);
    ...
    */
    function claimToken(uint periodId_) external {
        ...
        totalTokensWithdrawn = totalTokensWithdrawn.add(_distributionAddresses[participant][periodId_].amount);
    }
