...
contract WONDERLAND is Ownable, ERC721Enumerable, ReentrancyGuard {
  ...
  uint256 public constant MAX_MINTABLE_AT_ONCE = 1;

  uint256 private _price = 0.1 ether;

  string public catcontractURI;

  constructor() ERC721("Wonderland", "WONDR") {}

  address oaf = 0x72CB6f400382Ff6F5545f0d3756aec8C68c4ddB0;
  address quack = 0x72CB6f400382Ff6F5545f0d3756aec8C68c4ddB0;

  uint256[10000] private _availableTokens;
  uint256 private _numAvailableTokens = 5;
  uint256 private _numFreeRollsGiven = 0;

  mapping(address => uint256) public freeRollWONDR;

  uint256 private _lastTokenIdMintedInInitialSet = 10000;

  function numTotalWONDR() public view virtual returns (uint256) {
    return 5;
  }

  function freeRollMint() public nonReentrant {
    require(freeRollWONDR[msg.sender] > 0, "You don't have any free rolls!");
    uint256 toMint = freeRollWONDR[msg.sender];
    freeRollWONDR[msg.sender] = 0;
    uint256 remaining = numTotalWONDR() - totalSupply();
    if (toMint > remaining) {
      toMint = remaining;
    }
    _mint(toMint);
  ...
  function getNumFreeRollWONDR(address owner) public view returns (uint256) {
    return freeRollWONDR[owner];
  }
  ...
  function mint(uint256 _numToMint) public payable nonReentrant {
    ...
    require(
      totalSupply + _numToMint <= numTotalWONDR(),
      "There aren't this many WONDR left."
    );
    uint256 costForMintingWONDR = _price * _numToMint;
    require(
      msg.value >= costForMintingWONDR,
      "Too little sent, please send more AVAX."
    );
    if (msg.value > costForMintingWONDR) {
      payable(msg.sender).transfer(msg.value - costForMintingWONDR);
    }
  ...
  function contractURI() public view returns (string memory) {
    return catcontractURI;
  }

  function getCostForMintingWONDR(uint256 _numToMint)
    public
    view
    returns (uint256)
  {
    require(
      totalSupply() + _numToMint <= numTotalWONDR(),
      "There are not this many WONDR left."
    );
    require(
      _numToMint <= MAX_MINTABLE_AT_ONCE,
      "You cannot mint that many WONDR."
    );
    return _numToMint * _price;
  }

  function getWONDRBelongingToOwner(address _owner)
    external
    view
    returns (uint256[] memory)
  {
    uint256 numWONDR = balanceOf(_owner);
    if (numWONDR == 0) {
      return new uint256[](0);
    } else {
      uint256[] memory result = new uint256[](numWONDR);
      for (uint256 i = 0; i < numWONDR; i++) {
  ...
  function giveFreeRoll(address receiver, uint256 numRolls) public onlyOwner {
    ...
    require(_numFreeRollsGiven < 100, "Already given max number of free rolls");
    require(
      freeRollWONDR[receiver] + numRolls < 51,
      "Cannot exceed 50 unused free rolls!"
    );
    uint256 freeRolls = freeRollWONDR[receiver];
    freeRollWONDR[receiver] = freeRolls + numRolls;
