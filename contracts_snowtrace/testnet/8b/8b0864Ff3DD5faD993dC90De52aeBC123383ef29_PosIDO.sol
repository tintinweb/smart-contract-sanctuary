...
// File: contracts/presale/PosIDO.sol
...
contract PosIDO is Ownable {
    ...
    mapping(address => bool) public boughtPOS;
    ...
    function purchasePOS(uint256 _amountMIM) external returns (bool) {
        require(saleStarted() == true, 'Not started');
        require(
            !whiteListEnabled || whiteListed[msg.sender] == true,
            'Not whitelisted'
        );
        require(boughtPOS[msg.sender] == false, 'Already participated');

        boughtPOS[msg.sender] = true;
    ...
    function finalize(address _receipt) external onlyOwner {
        require(totalAmount == 0, 'need all POS to be sold');

        uint256 maiInTreasure = 250000 * 1e18;

        IERC20(MIM).approve(treasury, maiInTreasure);
        uint256 posMinted = ITreasury(treasury).deposit(maiInTreasure, MIM, 0);

        require(posMinted == 250000 * 1e9);

        // dev: create lp with 30 MIM per POS
        IERC20(MIM).transfer(mimPosLP, 300000 * 1e18);
        IERC20(POS).transfer(mimPosLP, 10000 * 1e9);
        uint256 lpBalance = IUniswapV2Pair(mimPosLP).mint(address(this));
        uint256 valueOfToken = ITreasury(treasury).valueOfToken(
            mimPosLP,
            lpBalance
        );

        IUniswapV2Pair(mimPosLP).approve(treasury, lpBalance);
        uint256 zeroMinted = ITreasury(treasury).deposit(
            lpBalance,
            mimPosLP,
            valueOfToken
        );
        require(zeroMinted == 0, 'should not mint any POS');
        IERC20(POS).approve(staking, posMinted);
