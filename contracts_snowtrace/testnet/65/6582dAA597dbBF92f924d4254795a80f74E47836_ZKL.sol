...
/// @author zk.link
contract ZKL is ERC20Capped, ERC20Permit, ILayerZeroReceiver {
    ...
    /// @notice to prevent duplicate receive event
    mapping(uint16 => mapping(bytes => uint64)) receiveNonce;
    ...
    function bridge(uint16 _lzChainId, bytes calldata _receiver, uint _amount) public payable {
    ...
    function lzReceive(uint16 _srcChainId, bytes calldata _srcAddress, uint64 _nonce, bytes calldata _payload) override external onlyLZEndpoint {
        require(bridgeable, 'ZKL: bridge disabled');

        // reject invalid src contract address
        require(keccak256(destination[_srcChainId]) == keccak256(_srcAddress), 'ZKL: invalid lz chain id and zkl address');

        // reject duplicate nonce
        require(receiveNonce[_srcChainId][_srcAddress] != _nonce, 'ZKL: duplicate receive');
        receiveNonce[_srcChainId][_srcAddress] = _nonce;
