...
contract Traits is Ownable, ITraits {
    ...
    'pants', // Global 2
    'hats', // Global 3
    'facial hair', // Hunters 4
    'backpack', // Hunters 5
    'accessories', // Monkeys 6
    'weapon', // Hunter 7
    'alpha' // Hunter 8
    ];
    ...
    IBasicApesToken public chadAndScammer;
    ...
    function setGame(address _chadAndScammer) external onlyOwner {
        chadAndScammer = IBasicApesToken(_chadAndScammer);
    }
    ...
    function drawSVG(uint256 tokenId) public view returns (string memory) {
        IBasicApesToken.BasicApesTokenTraits memory s = chadAndScammer
            .getTokenTraits(tokenId);
        uint8 shift = s.isChad ? 0 : 10;

        string memory svgString = string(
            abi.encodePacked(
                s.isChad ? draw(thiefBody) : draw(policeBody),
                drawTrait(
                    traitData[0 + shift][
                        s.base % traitCountForType[0 + shift]
                    ]
                ),
                drawTrait(
                ...
                drawTrait(
                    traitData[3 + shift][s.hats % traitCountForType[3 + shift]]
                ),
                s.isChad
                    ? drawTrait(
                        traitData[6 + shift][
                            s.accessories % traitCountForType[6 + shift]
                        ]
                    )
                    : drawTrait(
                    traitData[4 + shift][s.facialHair % traitCountForType[4 + shift]]
                  )
            )
        );
        ...
        return
            string(
                abi.encodePacked(
                    '<svg id="chadAndScammer" width="100%" height="100%" version="1.1" viewBox="0 0 40 40" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">',
                    svgString,
                    "</svg>"
                )
            );
        ...
        if (s.isChad) { // Monkeys
            traits = string(
                abi.encodePacked(
                    attributeForTypeAndValue(
                        _traitTypes[0],
                        traitData[0][s.base % traitCountForType[0]].name
                    ),
                    ",",
                    attributeForTypeAndValue(
                        _traitTypes[1],
                    ...
                    attributeForTypeAndValue(
                        _traitTypes[3],
                        traitData[3][s.hats % traitCountForType[3]].name
                    ),
                    ",",
                    attributeForTypeAndValue(
                        _traitTypes[6],
                        traitData[4][s.accessories % traitCountForType[4]].name
                    ),
                    ","
                )
            );
            ...
            traits = string(
                abi.encodePacked(
                    attributeForTypeAndValue(
                        _traitTypes[0],
                        traitData[10][s.base % traitCountForType[10]].name
                    ),
                    ",",
                    attributeForTypeAndValue(
                        _traitTypes[1],
                    ...
                    attributeForTypeAndValue(
                        _traitTypes[3],
                        traitData[13][s.hats % traitCountForType[13]].name
                    ),
                    ",",
                    attributeForTypeAndValue(
                        _traitTypes[4],
                        traitData[14][s.facialHair % traitCountForType[14]].name
                    ),
                    ",",
                   attributeForTypeAndValue(
                        _traitTypes[5],
                        traitData[15][s.backpack % traitCountForType[15]].name
                    ),
                    ",",
                    attributeForTypeAndValue(
                        _traitTypes[6],
                        traitData[16][s.weapon % traitCountForType[16]].name
                    ),
                    ",",
                    attributeForTypeAndValue(
                        "Alpha Score",
        ...
        return
            string(
                abi.encodePacked(
                    ...
                    tokenId <= chadAndScammer.getPaidTokens()
                        ? '"Gen 0"'
                        : '"Gen 1"',
                    '},{"trait_type":"Type","value":',
                    s.isChad ? '"Chad"' : '"Scammer"',
                    "}]"
                )
            );
    }
        ...
        returns (string memory)
    {
        ...
        string memory metadata = string(
            abi.encodePacked(
                ...
                s.isChad ? "Chad #" : "Scammer #",
                tokenId.toString(),
                '", "description": "Chad & Scammer Game is a new generation play-to-earn NFT game on FTM that incorporates probability-based derivatives alongside NFTs. Through a vast array of choices and decision-making possibilities, Chad & Scammer Game promises to instil excitement and curiosity amongst the community as every individual adopts different strategies to do better than one another and to come out on top. The real question is, are you #TeamChad or #TeamScammer? Choose wisely or watch the other get rich!", "image": "data:image/svg+xml;base64,',
...
interface IBasicApesToken {
  // struct to store each token's traits
  struct BasicApesTokenTraits {
    bool isChad;
    uint8 base;
    uint8 top;
    uint8 pants;
    ...
    Pausable
{
     ...
     */
    constructor(address _bat, address _traits)
        ...
        whenNotPaused
    {
        ...
        if (minted < PAID_TOKENS) {
            ...
            require(
                amount * mintCost(minted) == msg.value,
    ...
    function mintCost(uint256 tokenId) public view returns (uint256) {
        // FTM
        return 0.1 ether;
        /* if (presaleOnly) return 30 ether;
        if (tokenId <= 4000) return 40 ether; 
        if (tokenId <= 6000) return 50 ether; 
        if (tokenId <= 8000) return 60 ether; 
        if (tokenId <= 10000) return 70 ether; 
        // $BANANA
        ...
        returns (BasicApesTokenTraits memory t)
    {
        ...
        seed >>= 16;
        t.hats = selectTrait(uint16(seed & 0xFFFF), 3 + shift);

        seed >>= 16;
        if (!t.isChad) { // Hunters
            t.facialHair = selectTrait(uint16(seed & 0xFFFF), 4 + shift);
            t.backpack = selectTrait(uint16(seed & 0xFFFF), 5 + shift);
            t.weapon = selectTrait(uint16(seed & 0xFFFF), 7 + shift);
        } else {
            t.accessories = selectTrait(uint16(seed & 0xFFFF), 6 + shift);
        }
    }
     ...
     */
    function structToHash(BasicApesTokenTraits memory s)
        internal
        pure
        returns (uint256)
    {
...
contract Bank is Ownable, IERC721Receiver, Pausable {
     ...
     */
    constructor(address _game, address _bat) {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            if (isThief(tokenIds[i])) {
                ...
                if (totalbatEarned < MAXIMUM_GLOBAL_bat) {
                    newOwed =
                        ((block.timestamp - stake.value) * DAILY_bat_RATE) /
                        1 days;
                } else if (stake.value > lastClaimTimestamp) {
                    newOwed = 0;
                } else {
                    newOwed =
                        ((lastClaimTimestamp - stake.value) * DAILY_bat_RATE) /
                        1 days;
                }
                owed[i] = (newOwed * (100 - bat_CLAIM_TAX_PERCENTAGE)) / 100;
            } else {
                uint256 alpha = _alphaForPolice(tokenIds[i]);
                Stake memory stake = pack[alpha][packIndices[tokenIds[i]]];
                owed[i] = (alpha) * (batPerAlpha - stake.value);
            }
        }
        return owed;
    }
