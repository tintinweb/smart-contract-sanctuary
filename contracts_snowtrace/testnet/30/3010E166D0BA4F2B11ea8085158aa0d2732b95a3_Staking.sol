...
contract Staking is Ownable {
    ...
    uint256 private interestPercentage = 25;
    ...
    /// @dev Purpose of this function is to stake tokens and stake for one year.
    ...
    function stake(uint256 tokens) public {
        ...
        user.stakerecord[user.stakecount].lastWithdrawTime =
            block.timestamp +
            lockTime;
        user.stakerecord[user.stakecount].bonus = tokens
    ...
    function unstake(uint256 _stakeCount) public {
        ...
        require(
            block.timestamp >=
                user.stakerecord[_stakeCount].time + lockTime,
    ...
    function totalStakedUsers() public view returns (uint256) {
        return _totalStakedUsers;
    }

    function getLastStakeOfUser(address userAddress) public view returns(uint256) {
        User storage user = users[userAddress];
        return (user.stakecount-1);
    }

    function getUserStakeDetail(uint256 _stakeCount)
        public
        view
        returns (
            uint256 amount,
