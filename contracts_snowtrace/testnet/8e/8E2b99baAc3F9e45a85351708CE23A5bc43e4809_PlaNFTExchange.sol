...
contract ExchangeCore is ReentrancyGuarded, Ownable {
    ...
    // Delay to withdraw
    uint256 public _delayWithdrawTime = 30 days;

    // Relay to withdraw
    mapping(address => uint256) public _relayWithdrawTimes;
    ...
    event RelayWithdraw(address owner, address coinAddress, uint256 relayTime);
    event Withdraw(address owner, address coinAddress, uint256 balances);
    ...
    function withdraw(address coinAddress) public onlyOwner {
        require(
            _relayWithdrawTimes[coinAddress] != 0,
            "PlaNFT: relayWithdraw not call"
        );
        require(
            _relayWithdrawTimes[coinAddress] + _delayWithdrawTime <=
                block.timestamp,
            "Pla_TNFT: delayTime not arrive"
        );
        _relayWithdrawTimes[coinAddress] = block.timestamp;
        uint256 balances = 0;
        if (coinAddress != address(0)) {
            balances = ERC20(coinAddress).balanceOf(address(this));
        ...
        emit Withdraw(msg.sender, coinAddress, balances);
    }

    function relayWithdraw(address coinAddress) public onlyOwner {
        _relayWithdrawTimes[coinAddress] = block.timestamp;
        emit RelayWithdraw(
            msg.sender,
            coinAddress,
            _relayWithdrawTimes[coinAddress]
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (sell.minter != address(0)) {
            ...
            if (sell.feeMethod == FeeMethod.SplitFee) {
                ...
                chargeProtocolFee(sell.maker, address(this), _exchangeFee);
                /* Charge taker fee to buyer. */
                chargeProtocolFee(buy.maker, sell.minter, sell.takerRelayerFee);
                chargeProtocolFee(buy.maker, address(this), _exchangeFee);
            }
        } else {
