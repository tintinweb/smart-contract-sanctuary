    ...
    ERC2981PerTokenRoyalties
{
    ...
    uint256 public maxMintForTx = 4;
    uint256 public maxMintsForAddress = 4;
  ...
  */
    function mint(uint256 _mintAmount) public payable {
        ...
        require(
            ...
            "ERROR: Sorry Fren, you have reached your Frogs Frens mint limit."
        );
        require(!paused, "ERROR: Contract paused. Please check discord.");
        require(
            _mintAmount <= maxMintForTx,
            "ERROR: Sorry Fren, you have breached the max number of mints per transaction."
        );
        require(
            supply + _mintAmount <= maxSupply,
            "ERROR: Sorry Fren, all our Frog Frens have now been minted!"
        );

        if (!whiteListAddresses[msg.sender]) {
            require(
                msg.value >= cost * _mintAmount,
                "ERROR: Sorry Fren, you need more AVAX."
            );
        }
