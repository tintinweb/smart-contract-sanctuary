...
contract DeathIDO is Ownable, ReentrancyGuard {
    ...
    uint256 public constant DEATH_ALLOCATION = 30000000000000000000000; // hardcap 50k DEATH
    ...
    function addLiquidity() external onlyOwner {
        ...
        uint256 _BuyLiquidity = _BuyBalance * 7 / 10; //1 * 7 / 10 = 0.7, 70% of the balance
        uint256 _DeathLiquidity = _BuyLiquidity / 20; // 20 BUY = 1 DEATH : listing price
        uint256 _TeamFunds = _BuyBalance - _BuyLiquidity; // the other 30%
        ...
        uniswapV2Router.addLiquidity(
            ...
            0, // slippage is unavoidable
            address(0), //Sends the LP token to the address 0, which is the burn address. Locked liquidity straight away
            block.timestamp
        );

        //Sends the other 30% to the dev wallet
