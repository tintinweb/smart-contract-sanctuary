...
contract UUSD is ERC20BurnableUpgradeable, AccessControlUpgradeable {
...
contract UUSDPool is AccessControlUpgradeable {
    ...
    // Token
    IWrappedNative public share; // Wrapped Native Token
    ICustomToken public synth; // Synthetic UUSD Token
    IERC20 public collateralToken; // Collateral Token
    ...
    function mintAlgorithmicSynth(uint256 synthOutMin)
        external
        payable
        notMintPaused
        onlyUserOrWhitelistedContracts
    {
        ...
        share.deposit{value: msg.value}();
        mintAlgorithmicSynth(msg.value, synthOutMin);
    }
        ...
        private
    {
        require(block.number >= lastAction[msg.sender].add(actionDelay));
        require(
            share.balanceOf(address(this)) >= shareAmount,
            "No enough Share"
        );
        require(
        ...
        // transfer WAVAX from this contract to collateralReserve
        share.transfer(address(collateralReserve), shareAmount);
        synth.mint(msg.sender, _synthAmountReceive);
        synth.mint(address(this), _fee);
    }
    ...
    ) external payable notMintPaused onlyUserOrWhitelistedContracts {
        share.deposit{value: msg.value}();
        mintFractionalSynth(_collateralAmount, msg.value, _synthOutMin);
    }
    ...
    function mintFractionalSynth(
        ...
        require(
            share.balanceOf(address(this)) >= _shareAmount,
            "No enough Share"
        );
        ...
        // transfer WAVAX from this contract to collateralReserve
        share.transfer(address(collateralReserve), _shareNeeded);
        collateralToken.safeTransferFrom(
            msg.sender,
            address(collateralReserve),
            _collateralAmount
        ...
        onlyUserOrWhitelistedContracts
    {
        ...
        share.withdraw(_shareReceived);

        // Transfer AVAX to msg.sender
        msg.sender.transfer(_shareReceived);
    }
    ...
    function withdrawFee() external {
        ...
        uint256 shareAmount = share.balanceOf(address(this));
        share.withdraw(shareAmount);
...
contract StableCollateralReserve is AccessControlUpgradeable {
    ...
    function totalGlobalSynthValue() public view returns (uint256 _tgsv) {
        for (uint256 i = 0; i < synthArray.length; i++) {
            if (synthArray[i] != address(0)) {
                ...
                uint256 _price = UUSD(synthArray[i]).getSynthPrice();
                _tgsv = _tgsv.add(
    ...
    function buyBackShare(uint256 _collateralOutMin, address _collateralToken)
        external
        payable
    {
        share.deposit{value: msg.value}();
        buyBackShare(msg.value, _collateralOutMin, _collateralToken);
    }
    ...
    function recollateralizeShare(
        ...
        share.withdraw(_sharePaidBackReceived.add(_fee));
        msg.sender.transfer(_sharePaidBackReceived);
