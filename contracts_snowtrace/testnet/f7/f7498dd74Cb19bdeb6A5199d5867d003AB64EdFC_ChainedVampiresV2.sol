...
contract ChainedVampiresV2 is ERC721, ERC721Enumerable, Ownable {
    ...
    uint256 private RESERVED = 10;   // Reserved = 10
    uint256 private RESTORED = 298;  // Restored Assets
    address payable private feeCollector;

    string public baseURI;
    string public baseExtension = ".json";
    bool public paused = false;
    bool public restorePeriod = true;
    ...
    function getHolderAssets(address _prevcontract, address _holder) public view returns(uint256[] memory){
        uint256[] memory tokenIds = IChainedVampires(_prevcontract).walletOfOwner(_holder);
        return tokenIds;
    }

    function restoreAssets(address _prevcontract, address _holder) public onlyOwner {
        uint256[] memory tokenIds = getHolderAssets(_prevcontract, _holder);

        for (uint256 i = 0; i < tokenIds.length; i++) {
            _safeMint(_holder, tokenIds[i]);
            minter[tokenIds[i]] = _holder; // Full ownership is given to rewarded address
            lastDividendAt[tokenIds[i]] = currentDividendPerHolder;
            _tokenIdCounter.increment();
        }
    }

    function SetRestorePeriod(bool _state) public onlyOwner {
        restorePeriod = _state;
    }

    function summonVampire(uint256 _amount) public payable {
        require(!restorePeriod, "Restore period must be finished!");
