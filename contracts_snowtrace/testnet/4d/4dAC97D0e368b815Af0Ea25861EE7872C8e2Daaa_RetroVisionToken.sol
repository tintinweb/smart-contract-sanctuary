...
contract RetroVisionToken is IERC20, Context, ReentrancyGuard, Ownable {
    ...
    constructor(
        ...
        uint256 _devMarketSeed = Finance.calculateFeeAmount(_tokenParams.totalSupply, 500); // 5%
        uint256 _charitySeed = Finance.calculateFeeAmount(_tokenParams.totalSupply, 200); // 2%
     ...
     */
    function _applyFees(uint256 _amount)
        private
        returns (uint256)
    {
        ...
        _totalReflectionTicks.increment();
        _totalFeesCollected[FEE_REFLECTION] = _totalFeesCollected[FEE_REFLECTION].add(
            _reflectionFee
        );
        ...
        _totalFeesCollected[FEE_DEV_MARKET] = _totalFeesCollected[FEE_DEV_MARKET].add(
            _devMarketFee
        );
        ...
        _totalFeesCollected[FEE_RESOURCE_LIQUIDITY] = _totalFeesCollected[FEE_RESOURCE_LIQUIDITY]
        ...
        returns (uint256)
    {
        ...
        if (block.timestamp.sub(_accounts[_account].firstTX) < FIRST_TX_DELAY) {
            uint256 _initialWait = block.timestamp.sub(_accounts[_account].firstTX);
            return _initialWait > FIRST_TX_DELAY ? 0 : FIRST_TX_DELAY.sub(_initialWait);
        }
        // Arbitrage delay, we are already past initial delay
        else if (block.timestamp.sub(_accounts[_account].lastTX) < TX_DELAY) {
            uint256 _arbitrageWait = block.timestamp.sub(_accounts[_account].lastTX);
