...
interface IVOLTERC20 is IERC20Mintable, IERC20 {
...
contract AsgardTreasury is Ownable {
    ...
    IVOLTERC20 public immutable Volt;
    ...
    IERC20 public AMPeres;
    uint public AmpQueue; // Delays change to sOHM address
    ...
    /**
        @notice allow approved address to deposit an asset for Volt
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint Volt needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        limitRequirements(msg.sender, send_);
        Volt.mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn Volt for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        Volt.burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = AMPeres.balanceOf( msg.sender ); // Can only borrow against sOHM held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with Volt
        @param _amount uint
     */
    function repayDebtWithTime( uint _amount ) external {
        ...
        Volt.burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, address(Volt), _amount, _amount );
    }
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( Volt.totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns Volt valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match Volt decimals
            value_ = _amount.mul( 10 ** Volt.decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyOwner returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.AMP ) { // 9
            AmpQueue = uint32(block.timestamp).add32( secondsNeededForQueue );
        } else return false;
