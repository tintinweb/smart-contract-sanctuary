...
// File contracts/StakingRewardsNFT.sol



pragma solidity ^0.7.6;
contract StakingRewardsNFT is ReentrancyGuard, Ownable {
    using SafeMath for uint256;
    ...
    function stake(uint256[] calldata tokenId) external nonReentrant updateReward(msg.sender) {
        for (uint i = 0; i < tokenId.length; i++) {
            IERC721(stakingToken).transferFrom(msg.sender, address(this), tokenId[i]);
            _stakings[tokenId[i]] = msg.sender;
        }
        _totalSupply = _totalSupply.add(tokenId.length);
        _balances[msg.sender] = _balances[msg.sender].add(tokenId.length);
		emit Staked(msg.sender, tokenId);
	}

    function withdraw(uint256[] memory tokenId) public nonReentrant updateReward(msg.sender) {
        for (uint i = 0; i < tokenId.length; i++) {
            require(_stakings[tokenId[i]] == msg.sender, "Permission denied");
            IERC721(stakingToken).transferFrom(address(this), msg.sender, tokenId[i]);
        }
		_totalSupply = _totalSupply.sub(tokenId.length);
		_balances[msg.sender] = _balances[msg.sender].sub(tokenId.length);
