...
/**
 * @title Source's Comptroller Contract
...
contract Comptroller is ComptrollerV5Storage, ComptrollerInterfaceG2, ComptrollerErrorReporter, ExponentialNoError {
    ...
    /// @notice Emitted when USX Vault info is changed
    event NewUSXVaultInfo(address vault_, uint releaseStartBlock_, uint releaseInterval_);
    ...
    /// @notice Emitted when Thermo USX rate is changed
    event NewThermoUSXRate(uint oldThermoUSXRate, uint newThermoUSXRate);

    /// @notice Emitted when Thermo USX Vault rate is changed
    event NewThermoUSXVaultRate(uint oldThermoUSXVaultRate, uint newThermoUSXVaultRate);

    /// @notice Emitted when a new Thermo speed is calculated for a market
    event ThermoSpeedUpdated(SToken indexed sToken, uint newSpeed);

    /// @notice Emitted when TMX is distributed to a supplier
    event DistributedSupplierThermo(SToken indexed sToken, address indexed supplier, uint thermoDelta, uint thermoSupplyIndex);

    /// @notice Emitted when TMX is distributed to a borrower
    event DistributedBorrowerThermo(SToken indexed sToken, address indexed borrower, uint thermoDelta, uint thermoBorrowIndex);

    /// @notice Emitted when TMX is distributed to a USX minter
    event DistributedUSXMinterThermo(address indexed usxMinter, uint thermoDelta, uint thermoUSXMintIndex);

    /// @notice Emitted when TMX is distributed to USX Vault
    event DistributedUSXVaultThermo(uint amount);

    /// @notice Emitted when USXController is changed
    event NewUSXController(USXControllerInterface oldUSXController, USXControllerInterface newUSXController);

    /// @notice Emitted when USX mint rate is changed by admin
    event NewUSXMintRate(uint oldUSXMintRate, uint newUSXMintRate);
    ...
    /// @notice Emitted when Thermo is granted by admin
    event ThermoGranted(address recipient, uint amount);
    ...
    modifier onlyListedMarket(SToken sToken) {
        require(markets[address(sToken)].isListed, "thermo market is not listed");
        _;
    }
    ...
    function enterMarkets(address[] calldata sTokens) external returns (uint[] memory) {
        ...
        for (uint i = 0; i < len; i++) {
            results[i] = uint(addToMarketInternal(SToken(sTokens[i]), msg.sender));
        }
    ...
    function exitMarket(address sTokenAddress) external returns (uint) {
        ...
        // In order to delete sToken, copy last item in list to location of item to be removed, reduce length by 1
        SToken[] storage userAssetList = accountAssets[msg.sender];
    ...
    function mintAllowed(address sToken, address minter, uint mintAmount) external onlyProtocolAllowed returns (uint) {
        ...
        updateThermoSupplyIndex(sToken);
        distributeSupplierThermo(sToken, minter);
    ...
    function redeemAllowed(address sToken, address redeemer, uint redeemTokens) external onlyProtocolAllowed returns (uint) {
        ...
        // Keep the flywheel moving
        updateThermoSupplyIndex(sToken);
        distributeSupplierThermo(sToken, redeemer);
    ...
    function borrowAllowed(address sToken, address borrower, uint borrowAmount) external onlyProtocolAllowed returns (uint) {
        ...
        if (!markets[sToken].accountMembership[borrower]) {
            ...
            Error err = addToMarketInternal(SToken(sToken), borrower);
            if (err != Error.NO_ERROR) {
                return uint(err);
            }
        }
        ...
        updateThermoBorrowIndex(sToken, borrowIndex);
        distributeBorrowerThermo(sToken, borrower, borrowIndex);
    ...
    function liquidateBorrowAllowed(
        ...
        if (!(markets[sTokenBorrowed].isListed || address(sTokenBorrowed) == address(usxController)) || !markets[sTokenCollateral].isListed) {
            return uint(Error.MARKET_NOT_LISTED);
        }

        /* The borrower must have shortfall in order to be liquidatable */
        (Error err, , uint shortfall) = getHypotheticalAccountLiquidityInternal(borrower, SToken(0), 0, 0);
        if (err != Error.NO_ERROR) {
            return uint(err);
        }
        if (shortfall == 0) {
        ...
        uint borrowBalance;
        if (address(sTokenBorrowed) != address(usxController)) {
            borrowBalance = SToken(sTokenBorrowed).borrowBalanceStored(borrower);
        } else {
            borrowBalance = mintedUSXs[borrower];
        }
    ...
    function seizeAllowed(
        ...
        // We've added USXController as a borrowed token list check for seize
        if (!markets[sTokenCollateral].isListed || !(markets[sTokenBorrowed].isListed || address(sTokenBorrowed) == address(usxController))) {
            return uint(Error.MARKET_NOT_LISTED);
        }
        ...
        // Keep the flywheel moving
        updateThermoSupplyIndex(sTokenCollateral);
        distributeSupplierThermo(sTokenCollateral, borrower);
        distributeSupplierThermo(sTokenCollateral, liquidator);
    ...
    function transferAllowed(address sToken, address src, address dst, uint transferTokens) external onlyProtocolAllowed returns (uint) {
        ...
        // Keep the flywheel moving
        updateThermoSupplyIndex(sToken);
        distributeSupplierThermo(sToken, src);
        distributeSupplierThermo(sToken, dst);
    ...
    function getHypotheticalAccountLiquidityInternal(
        ...
        for (uint i = 0; i < assets.length; i++) {
            ...
            vars.sumCollateral = mul_ScalarTruncateAddUInt(vars.tokensToDenom, vars.sTokenBalance, vars.sumCollateral);
        ...
        vars.sumBorrowPlusEffects = add_(vars.sumBorrowPlusEffects, mintedUSXs[account]);
     ...
     * @return (errorCode, number of sTokenCollateral tokens to be seized in a liquidation)
     */
    function liquidateUSXCalculateSeizeTokens(address sTokenCollateral, uint actualRepayAmount) external view returns (uint, uint) {
        /* Read oracle prices for borrowed and collateral markets */
        uint priceBorrowedMantissa = 1e18;  // Note: this is USX
        uint priceCollateralMantissa = oracle.getUnderlyingPrice(SToken(sTokenCollateral));
    ...
    function _supportMarket(SToken sToken) external returns (uint) {
        ...
        // Note that isThermo is not in active use anymore
        markets[address(sToken)] = Market({isListed: true, isThermo: false, collateralFactorMantissa: 0});

        _addMarketInternal(sToken);
    ...
    function _addMarketInternal(SToken sToken) internal {
        for (uint i = 0; i < allMarkets.length; i ++) {
            require(allMarkets[i] != sToken, "market already added");
        }
        allMarkets.push(sToken);
    }
    ...
    /**
      * @notice Sets a new USX controller
      * @dev Admin function to set a new USX controller
      * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
      */
    function _setUSXController(USXControllerInterface usxController_) external returns (uint) {
        // Check caller is admin
        if (msg.sender != admin) {
            return fail(Error.UNAUTHORIZED, FailureInfo.SET_USXCONTROLLER_OWNER_CHECK);
        }

        USXControllerInterface oldRate = usxController;
        usxController = usxController_;
        emit NewUSXController(oldRate, usxController_);
    }

    function _setUSXMintRate(uint newUSXMintRate) external returns (uint) {
        // Check caller is admin
        if (msg.sender != admin) {
            return fail(Error.UNAUTHORIZED, FailureInfo.SET_USX_MINT_RATE_CHECK);
        }

        uint oldUSXMintRate = usxMintRate;
        usxMintRate = newUSXMintRate;
        emit NewUSXMintRate(oldUSXMintRate, newUSXMintRate);
    ...
    function setThermoSpeedInternal(SToken sToken, uint thermoSpeed) internal {
        uint currentThermoSpeed = thermoSpeeds[address(sToken)];
        if (currentThermoSpeed != 0) {
            // note that TMX speed could be set to 0 to halt liquidity rewards for a market
            Exp memory borrowIndex = Exp({mantissa: sToken.borrowIndex()});
            updateThermoSupplyIndex(address(sToken));
            updateThermoBorrowIndex(address(sToken), borrowIndex);
        } else if (thermoSpeed != 0) {
            // Add the TMX market
            Market storage market = markets[address(sToken)];
            require(market.isListed == true, "thermo market is not listed");

            if (thermoSupplyState[address(sToken)].index == 0 && thermoSupplyState[address(sToken)].block == 0) {
                thermoSupplyState[address(sToken)] = ThermoMarketState({
        ...
        if (thermoBorrowState[address(sToken)].index == 0 && thermoBorrowState[address(sToken)].block == 0) {
                thermoBorrowState[address(sToken)] = ThermoMarketState({
                    index: thermoInitialIndex,
                    block: safe32(getBlockNumber(), "block number exceeds 32 bits")
                });
            }
        ...
        if (currentThermoSpeed != thermoSpeed) {
            thermoSpeeds[address(sToken)] = thermoSpeed;
            emit ThermoSpeedUpdated(sToken, thermoSpeed);
        }
    }

    /**
     * @notice Accrue TMX to the market by updating the supply index
     * @param sToken The market whose supply index to update
     */
    function updateThermoSupplyIndex(address sToken) internal {
        ThermoMarketState storage supplyState = thermoSupplyState[sToken];
        uint supplySpeed = thermoSpeeds[sToken];
        uint blockNumber = getBlockNumber();
        uint deltaBlocks = sub_(blockNumber, uint(supplyState.block));
        if (deltaBlocks > 0 && supplySpeed > 0) {
            uint supplyTokens = SToken(sToken).totalSupply();
            uint thermoAccrued = mul_(deltaBlocks, supplySpeed);
            Double memory ratio = supplyTokens > 0 ? fraction(thermoAccrued, supplyTokens) : Double({mantissa: 0});
            Double memory index = add_(Double({mantissa: supplyState.index}), ratio);
            thermoSupplyState[sToken] = ThermoMarketState({
    ...
    /**
     * @notice Accrue TMX to the market by updating the borrow index
     * @param sToken The market whose borrow index to update
     */
    function updateThermoBorrowIndex(address sToken, Exp memory marketBorrowIndex) internal {
        ThermoMarketState storage borrowState = thermoBorrowState[sToken];
        uint borrowSpeed = thermoSpeeds[sToken];
        uint blockNumber = getBlockNumber();
        uint deltaBlocks = sub_(blockNumber, uint(borrowState.block));
        if (deltaBlocks > 0 && borrowSpeed > 0) {
            uint borrowAmount = div_(SToken(sToken).totalBorrows(), marketBorrowIndex);
            uint thermoAccrued = mul_(deltaBlocks, borrowSpeed);
            Double memory ratio = borrowAmount > 0 ? fraction(thermoAccrued, borrowAmount) : Double({mantissa: 0});
            Double memory index = add_(Double({mantissa: borrowState.index}), ratio);
            thermoBorrowState[sToken] = ThermoMarketState({
                index: safe224(index.mantissa, "new index overflows"),
                block: safe32(blockNumber, "block number overflows")
            });
        } else if (deltaBlocks > 0) {
    ...
    /**
     * @notice Calculate TMX accrued by a supplier and possibly transfer it to them
     * @param sToken The market in which the supplier is interacting
     * @param supplier The address of the supplier to distribute TMX to
     */
    function distributeSupplierThermo(address sToken, address supplier) internal {
        if (address(usxVaultAddress) != address(0)) {
            releaseToVault();
        }

        ThermoMarketState storage supplyState = thermoSupplyState[sToken];
        Double memory supplyIndex = Double({mantissa: supplyState.index});
        Double memory supplierIndex = Double({mantissa: thermoSupplierIndex[sToken][supplier]});
        thermoSupplierIndex[sToken][supplier] = supplyIndex.mantissa;

        if (supplierIndex.mantissa == 0 && supplyIndex.mantissa > 0) {
            supplierIndex.mantissa = thermoInitialIndex;
        }

        Double memory deltaIndex = sub_(supplyIndex, supplierIndex);
        uint supplierTokens = SToken(sToken).balanceOf(supplier);
        uint supplierDelta = mul_(supplierTokens, deltaIndex);
        uint supplierAccrued = add_(thermoAccrued[supplier], supplierDelta);
        thermoAccrued[supplier] = supplierAccrued;
        emit DistributedSupplierThermo(SToken(sToken), supplier, supplierDelta, supplyIndex.mantissa);
    }

    /**
     * @notice Calculate TMX accrued by a borrower and possibly transfer it to them
     * @dev Borrowers will not begin to accrue until after the first interaction with the protocol.
     * @param sToken The market in which the borrower is interacting
     * @param borrower The address of the borrower to distribute TMX to
     */
    function distributeBorrowerThermo(address sToken, address borrower, Exp memory marketBorrowIndex) internal {
        if (address(usxVaultAddress) != address(0)) {
            releaseToVault();
        }
        ...
        Double memory borrowerIndex = Double({mantissa:thermoBorrowerIndex[sToken][borrower]});
        thermoBorrowerIndex[sToken][borrower] = borrowIndex.mantissa;

        if (borrowerIndex.mantissa > 0) {
            Double memory deltaIndex = sub_(borrowIndex, borrowerIndex);
            uint borrowerAmount = div_(SToken(sToken).borrowBalanceStored(borrower), marketBorrowIndex);
            uint borrowerDelta = mul_(borrowerAmount, deltaIndex);
            uint borrowerAccrued = add_(thermoAccrued[borrower], borrowerDelta);
            thermoAccrued[borrower] = borrowerAccrued;
            emit DistributedBorrowerThermo(SToken(sToken), borrower, borrowerDelta, borrowIndex.mantissa);
        }
    }

    /**
     * @notice Calculate TMX accrued by a USX minter and possibly transfer it to them
     * @dev USX minters will not begin to accrue until after the first interaction with the protocol.
     * @param usxMinter The address of the USX minter to distribute TMX to
     */
    function distributeUSXMinterThermo(address usxMinter) public {
        if (address(usxVaultAddress) != address(0)) {
            releaseToVault();
        }

        if (address(usxController) != address(0)) {
            uint usxMinterAccrued;
            uint usxMinterDelta;
            uint usxMintIndexMantissa;
            uint err;
            (err, usxMinterAccrued, usxMinterDelta, usxMintIndexMantissa) = usxController.calcDistributeUSXMinterThermo(usxMinter);
            if (err == uint(Error.NO_ERROR)) {
                thermoAccrued[usxMinter] = usxMinterAccrued;
                emit DistributedUSXMinterThermo(usxMinter, usxMinterDelta, usxMintIndexMantissa);
            }
        }
    }

    /**
     * @notice Claim all the tmx accrued by holder in all markets and USX
     * @param holder The address to claim TMX for
     */
    function claimThermo(address holder) public {
        return claimThermo(holder, allMarkets);
    }

    /**
     * @notice Claim all the tmx accrued by holder in the specified markets
     * @param holder The address to claim TMX for
     * @param sTokens The list of markets to claim TMX in
     */
    function claimThermo(address holder, SToken[] memory sTokens) public {
        address[] memory holders = new address[](1);
        holders[0] = holder;
        claimThermo(holders, sTokens, true, true);
    }

    /**
     * @notice Claim all tmx accrued by the holders
     * @param holders The addresses to claim TMX for
     * @param sTokens The list of markets to claim TMX in
     * @param borrowers Whether or not to claim TMX earned by borrowing
     * @param suppliers Whether or not to claim TMX earned by supplying
     */
    function claimThermo(address[] memory holders, SToken[] memory sTokens, bool borrowers, bool suppliers) public {
        uint j;
        if(address(usxController) != address(0)) {
            usxController.updateThermoUSXMintIndex();
        }
        for (j = 0; j < holders.length; j++) {
            distributeUSXMinterThermo(holders[j]);
            thermoAccrued[holders[j]] = grantTMXInternal(holders[j], thermoAccrued[holders[j]]);
        }
        for (uint i = 0; i < sTokens.length; i++) {
            SToken sToken = sTokens[i];
            require(markets[address(sToken)].isListed, "not listed market");
            if (borrowers) {
                Exp memory borrowIndex = Exp({mantissa: sToken.borrowIndex()});
                updateThermoBorrowIndex(address(sToken), borrowIndex);
                for (j = 0; j < holders.length; j++) {
                    distributeBorrowerThermo(address(sToken), holders[j], borrowIndex);
                   thermoAccrued[holders[j]] = grantTMXInternal(holders[j], thermoAccrued[holders[j]]);
                }
            }
            if (suppliers) {
                updateThermoSupplyIndex(address(sToken));
                for (j = 0; j < holders.length; j++) {
                    distributeSupplierThermo(address(sToken), holders[j]);
                    thermoAccrued[holders[j]] = grantTMXInternal(holders[j], thermoAccrued[holders[j]]);
                }
            }
        }
    ...
    /**
     * @notice Transfer TMX to the user
     * @dev Note: If there is not enough TMX, we do not perform the transfer all.
     * @param user The address of the user to transfer TMX to
     * @param amount The amount of TMX to (possibly) transfer
     * @return The amount of TMX which was NOT transferred to the user
     */
    function grantTMXInternal(address user, uint amount) internal returns (uint) {
        TMX tmx = TMX(getTMXAddress());
        uint thermoRemaining = tmx.balanceOf(address(this));
        if (amount > 0 && amount <= thermoRemaining) {
            tmx.transfer(user, amount);
            return 0;
        }
        return amount;
    }

    /*** Thermo Distribution Admin ***/

    /**
     * @notice Transfer TMX to the recipient
     * @dev Note: If there is not enough TMX, we do not perform the transfer all.
     * @param recipient The address of the recipient to transfer TMX to
     * @param amount The amount of TMX to (possibly) transfer
     */
    function _grantTMX(address recipient, uint amount) public {
        require(adminOrInitializing(), "only admin can grant tmx");
        uint amountLeft = grantTMXInternal(recipient, amount);
        require(amountLeft == 0, "insufficient tmx for grant");
        emit ThermoGranted(recipient, amount);
    }

    /**
     * @notice Set the amount of TMX distributed per block to USX Mint
     * @param thermoUSXRate_ The amount of TMX wei per block to distribute to USX Mint
     */
    function _setThermoUSXRate(uint thermoUSXRate_) public onlyAdmin {
        uint oldUSXRate = thermoUSXRate;
        thermoUSXRate = thermoUSXRate_;
        emit NewThermoUSXRate(oldUSXRate, thermoUSXRate_);
    }

    /**
     * @notice Set the amount of TMX distributed per block to USX Vault
     * @param thermoUSXVaultRate_ The amount of TMX wei per block to distribute to USX Vault
     */
    function _setThermoUSXVaultRate(uint thermoUSXVaultRate_) public onlyAdmin {
        uint oldThermoUSXVaultRate = thermoUSXVaultRate;
        thermoUSXVaultRate = thermoUSXVaultRate_;
        emit NewThermoUSXVaultRate(oldThermoUSXVaultRate, thermoUSXVaultRate_);
    }

    /**
     * @notice Set the USX Vault infos
     * @param vault_ The address of the USX Vault
     * @param releaseStartBlock_ The start block of release to USX Vault
     * @param minReleaseAmount_ The minimum release amount to USX Vault
     */
    function _setUSXVaultInfo(address vault_, uint256 releaseStartBlock_, uint256 minReleaseAmount_) public onlyAdmin {
        usxVaultAddress = vault_;
        releaseStartBlock = releaseStartBlock_;
        minReleaseAmount = minReleaseAmount_;
        emit NewUSXVaultInfo(vault_, releaseStartBlock_, minReleaseAmount_);
    }

    /**
     * @notice Set TMX speed for a single market
     * @param sToken The market whose TMX speed to update
     * @param thermoSpeed New TMX speed for market
     */
    function _setThermoSpeed(SToken sToken, uint thermoSpeed) public {
        require(adminOrInitializing(), "only admin can set thermo speed");
        setThermoSpeedInternal(sToken, thermoSpeed);
    }
    ...
    /**
     * @notice Return the address of the TMX token
     * @return The address of TMX
     */
    function getTMXAddress() public view returns (address) {
    ...
    /**
     * @notice Set the minted USX amount of the `owner`
     * @param owner The address of the account to set
     * @param amount The amount of USX to set to the account
     * @return The number of minted USX by `owner`
     */
    function setMintedUSXOf(address owner, uint amount) external onlyProtocolAllowed returns (uint) {
        // Pausing is a very serious situation - we revert to sound the alarms
        require(!mintUSXGuardianPaused && !repayUSXGuardianPaused, "USX is paused");
        // Check caller is usxController
        if (msg.sender != address(usxController)) {
            return fail(Error.REJECTION, FailureInfo.SET_MINTED_USX_REJECTION);
        }
        mintedUSXs[owner] = amount;

        return uint(Error.NO_ERROR);
    }

    /**
     * @notice Transfer TMX to USX Vault
     */
    function releaseToVault() public {
        ...
        uint256 tmxBalance = tmx.balanceOf(address(this));
        if(tmxBalance == 0) {
            return;
        }
        ...
        // releaseAmount = thermoUSXVaultRate * deltaBlocks
        uint256 _releaseAmount = mul_(thermoUSXVaultRate, deltaBlocks);

        if (_releaseAmount < minReleaseAmount) {
            return;
        }

        if (tmxBalance >= _releaseAmount) {
            actualAmount = _releaseAmount;
        } else {
            actualAmount = tmxBalance;
        }

        releaseStartBlock = getBlockNumber();

        tmx.transfer(usxVaultAddress, actualAmount);
        emit DistributedUSXVaultThermo(actualAmount);

        IUSXVault(usxVaultAddress).updatePendingRewards();
    }
}
