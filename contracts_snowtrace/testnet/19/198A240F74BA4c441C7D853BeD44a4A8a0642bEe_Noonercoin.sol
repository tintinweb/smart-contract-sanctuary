...
contract Noonercoin is ERC20{
   ...
   uint256 weiAmountAdded = 0;
   uint256 weiAmountAddedAtBurn = 0;
   ...
   function mintToken(address add) public returns (bool) {  //admin only
       ...
       if(stopTheMinting) {
           ...
           uint256 noonerCoinExtractedFromWei = 0;
            ...
            if(totalOccurences != 120) {
                if(nowTime-weekStartTime >= 604800){
                    popRandomVariable();
                    weekStartTime=now;
                }
            }
            ...
            if(totalPicks != 120 && lastMintingTime != 0) {
                ...
                if(diff > _frequency) {
                    estimatePicks = (diff-_frequency)/604800;
    ...
    function burnToken() internal returns(bool){
        ...
        for(uint256 index=1;index<=totalCycleLeft;index++){
            uint256 intValue = getIntegerValue(flag/720*604800, 150**index, index);//604800
            uint256 intDecimalValue = getDecimalValue(flag/720*604800, 150**index, index);//604800
        ...
        uint256 iterationsInOneCycle = _cycleTime/_frequency;//604800
        ...
        uint256 totalMintedTokens =  (adminBalance-_fundersAmount - weiAmountAdded)*10**18 + noonerwei[adminAddress] + totalWeiBurned; //before adding totalWeiBurned.
        uint256 weiToBurned = _totalSupply*10**18 - signmaValueWei - totalMintedTokens - currentMintingRateTotalTokens - totalWeiBurned;
        uint256 totalWeiInAdminAcc = (adminBalance-_fundersAmount - weiAmountAdded) * 10**18 + noonerwei[adminAddress];
        ...
        if(totalWeiInAdminAcc > weiToBurned) {
            ...
            for(indexs=1;indexs<=1;indexs++) {
                previousCyclesTotalTokens = _fundersAmount + weiAmountAdded +weiAmountAddedAtBurn + (remainingWei/10**18);
