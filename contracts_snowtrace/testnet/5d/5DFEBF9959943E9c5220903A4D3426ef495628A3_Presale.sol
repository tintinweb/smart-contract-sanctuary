...
contract Presale is Ownable {
    ...
    uint256 public purchaseMIMAmount; // This is 250 MIM
    ...
    function purchaseaPRXWithwsOHM() external returns (bool) {
        ...
        IERC20(wsOHM).safeTransferFrom(msg.sender, addressToSendMIM, purchasewsOHMAmount);
        IERC20(aPOS).safeTransfer(msg.sender, allocatedaPOSPerUser);

        return true;
    }

    function purchaseaPOSWithMIM() external returns (bool) {
        require(saleStarted() == true, "Not started");
        require(whiteListed[msg.sender] == true, "Not whitelisted");
        require(boughtPOS[msg.sender] == false, "Already participated");
        require(block.timestamp < endOfSale, "Sale over");
