...
// File: contracts/presale/PrePosSales.sol
...
contract PrePosSales is Ownable {
  ...
  function initialize(
    ...
    pPos = IERC20( pPos_ );
    mim = IERC20( mim_ );
    pPosPrice = pPosPrice_;
    _saleProceedsAddress = saleProceedsAddress_;
    initialized = true;
  }

  function setPrePosPrice( uint256 newPrePosPrice_ ) external onlyOwner() returns ( uint256 ) {
    pPosPrice = newPrePosPrice_;
    return pPosPrice; 
  }
  ...
  function _calculateAmountPurchased( uint256 amountPaid_ ) internal returns ( uint256 ) {
    return amountPaid_.mul( pPosPrice );
  }

  function buyPrePos( uint256 amountPaid_ ) external returns ( bool ) {
    require( approvedBuyers[msg.sender], "Buyer not approved." );
    uint256 pPosAmountPurchased_ = _calculateAmountPurchased( amountPaid_ );
    mim.safeTransferFrom( msg.sender, _saleProceedsAddress, amountPaid_ );
    pPos.safeTransfer( msg.sender, pPosAmountPurchased_ );
    return true;
  }
