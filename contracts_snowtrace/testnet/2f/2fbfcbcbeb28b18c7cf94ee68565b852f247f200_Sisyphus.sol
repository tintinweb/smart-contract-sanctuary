...
contract Sisyphus is ERC20, Ownable, PaymentSplitter {
    using SafeMath for uint256;
    ...
    constructor(
        ...
        uint256 swapAmount,
        address uniV2Router
    ) ERC20("Sisyphus", "SISYPHUS") PaymentSplitter(payees, shares) {
...
*/
    ...
    function _transfer(
        ...
        if (from != owner() && to != uniswapV2Pair && to != address(uniswapV2Router) && to != address(this) && from != address(this)) {
            ...
            // anti whale
            if (to != futurUsePool && to != distributionPool && from != futurUsePool && from != distributionPool) {
