...
interface IStreamingFeeModule {
  function feeStates(IDomani _setToken) external view returns (StreamingFeeModule.FeeState memory);
...
contract DomaniViewer {
  ...
  function batchFetchManagers(IDomani[] memory _setTokens)
    external
    view
  ...
  function batchFetchModuleStates(IDomani[] memory _setTokens, address[] calldata _modules)
    public
    view
    returns (IDomani.ModuleState[][] memory)
  {
    IDomani.ModuleState[][] memory states = new IDomani.ModuleState[][](_setTokens.length);
    for (uint256 i = 0; i < _setTokens.length; i++) {
  ...
  function batchFetchDetails(IDomani[] memory _setTokens, address[] calldata _moduleList)
    public
    view
    returns (SetDetails[] memory)
  {
    IDomani.ModuleState[][] memory moduleStates = batchFetchModuleStates(_setTokens, _moduleList);

    SetDetails[] memory details = new SetDetails[](_setTokens.length);
    for (uint256 i = 0; i < _setTokens.length; i++) {
      IDomani setToken = _setTokens[i];
  ...
  function getSetDetails(IDomani _setToken, address[] calldata _moduleList)
    external
    view
    returns (SetDetails memory)
  {
