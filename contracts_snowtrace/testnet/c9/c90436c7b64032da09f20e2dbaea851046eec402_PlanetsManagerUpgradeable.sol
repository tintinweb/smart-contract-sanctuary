    ...
    LiquidityPoolManagerImplementationPointerUpgradeable
{
    ...
    function initialize() external initializer {
        ...
        changeNodeMinPrice(4_200 * (10**18)); // 4200 UNIV
        changeCompoundDelay(14400); // 4h
        changeProcessingFee(20); // 20%
        changeTierSystem(
        ...
        whenNotPaused
    {
        ...
        uint256 planetReward = _getPlanetCompoundRewards(_planetId);
        require(
            planetReward > 0,
            "Planets: You must wait until you can compound again"
        );
        _compoundReward(planetReward);

        emit Compound(account, _planetId, planetReward);
    }
    ...
    function compoundAll() external nonReentrant onlyPlanetOwner whenNotPaused {
        ...
        for (uint256 i = 0; i < planetsOwned.length; i++) {
            uint256 planetReward = _getPlanetCompoundRewards(planetsOwned[i]);
            if (planetReward > 0) {
                planetsAffected[i] = planetsOwned[i];
                rewardsTotal += planetReward;
            } else {
    ...
    function rewardPerDayFor(PlanetEntity memory planet)
        ...
        returns (uint256)
    {
        return _calculateRewardsFromValue(planet, ONE_DAY, rewardPerDay);
    }

    function rewardPerYearFor(PlanetEntity memory planet)
        private
        view
        returns (uint256)
    {
        return _calculateRewardsFromValue(planet, ONE_YEAR, rewardPerDay);
    }

    function _calculateRewardsFromValue(
        PlanetEntity memory _planet,
        uint256 _timeRewards,
        uint256 _rewardPerDay
    ) private pure returns (uint256) {
        uint256 rewards = (_timeRewards * _rewardPerDay) / 1000000;
        uint256 rewardsMultiplicated = (rewards * _planet.rewardMult) / 100000;
        return (rewardsMultiplicated * _planet.planetValue) / 100000;
    }
    ...
    function changeCompoundDelay(uint256 _compoundDelay) public onlyOwner {
        require(
            _compoundDelay > 0,
            "Planets: compoundDelay must be superior to 0"
        );
    ...
    function changeRewardPerDay(uint256 _rewardPerDay) public onlyOwner {
        require(
            _rewardPerDay > 0,
            "Planets: rewardPerDay must be superior to 0"
        );
