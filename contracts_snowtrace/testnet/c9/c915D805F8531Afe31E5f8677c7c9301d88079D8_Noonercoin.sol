...
contract Noonercoin is ERC20{
  ...
  function checkFailedTransactions(address add) public view returns (uint256) {
      ...
      uint256 timeDifference = now - startTime; 
      uint256 valueForEach = timeDifference/_frequency;
      uint256 estimatedMintedToken = valueForEach * currMintingRate;
  ...
  function checkMissingTokens(address add) public view returns (uint256, uint256,uint256, uint256,uint256, uint256) {
      ...
      if(lastMintingTime != 0) {
          uint256 timeDifference = now-startTime;
          uint256 valueForEach = timeDifference/_frequency;
          uint256 estimatedMintedToken = valueForEach*mintingRateNoonerCoin;
          ...
          uint256 checkDifferenceWei = estimatedMintedTokenWei-adminBalanceinWei;
          uint256 checkDifference = estimatedMintedToken - adminBalance;
          return(timeDifference,valueForEach, estimatedMintedToken,temp, checkDifference, checkDifferenceWei);
    }
  }
