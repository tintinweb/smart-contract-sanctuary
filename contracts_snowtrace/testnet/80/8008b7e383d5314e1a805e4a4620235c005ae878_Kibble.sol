...
contract Kibble {
    ...
    uint256 public immutable fetchPerSecond = 100000 * 1e18;
    ...
    constructor(
        ...
        fetch = _fetch;
        require(address(_gOHM) != address(0));
        poolInfo[0] = PoolInfo({
            token: _gOHM,
            allocPoint: 100,
        ...
        poolInfo[1] = PoolInfo({
            token: _lp,
            allocPoint: 0,
            lastRewardTime: (_startTime + 300), // 3.5 day delay
            accFetchPerShare: 0
        });
        totalAlloc = 100;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (block.timestamp > (lastWeightChange + interval) && poolInfo[0].allocPoint > 0) {
