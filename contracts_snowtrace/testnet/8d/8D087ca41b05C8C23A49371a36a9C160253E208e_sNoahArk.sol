...
contract sNoahArk is ERC20Permit, Ownable {
    ...
    function circulatingSupply() public view returns ( uint ) {
        ...
        return balanceForGons(gonsForBalance(_totalSupply).sub(_gonBalances[stakingContract]));
    }
    ...
    function transfer( address to, uint256 value ) public override returns (bool) {
        if (msg.sender != stakingContract) {
            IStaking( stakingContract ).settleWarmupInfo(msg.sender, value);
        }
    ...
    function transferFrom( address from, address to, uint256 value ) public override returns ( bool ) {
        if (from != stakingContract) {
            IStaking( stakingContract ).settleWarmupInfo(from, value);
        }
//        IStaking( stakingContract ).settleWarmupInfo(from, value);
