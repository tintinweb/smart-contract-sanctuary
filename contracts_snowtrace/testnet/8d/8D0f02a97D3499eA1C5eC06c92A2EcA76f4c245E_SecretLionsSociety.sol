 ...
 */
contract SecretLionsSociety is Context,  AccessControlEnumerable, ERC721Enumerable, ERC721URIStorage, ERC2981PerTokenRoyalties{
  using Counters for Counters.Counter;
  Counters.Counter public _tokenIdTracker;

  string private _baseTokenURI;
  uint256 private _price = 1000000000000000000;//2500000000000000000 = 2.5 AVAX
  uint _max = 10000;
  uint _giveAwayMax = 100;
  ...
  event Mint(address cchainAddress);

  constructor() ERC721("Secret Lions Society", "SLC") {
      _baseTokenURI = "temporary";

  }

  function supportsInterface(bytes4 interfaceId) public view virtual override(ERC721, AccessControlEnumerable, ERC721Enumerable, ERC2981PerTokenRoyalties) returns (bool) {
        return ERC721.supportsInterface(interfaceId);
  }

  function setDepositAddress(address depositAddress) external {
  ...
  function setTokenURI(uint256 tokenId, string memory _tokenURI) external {
    ...
    function setRoyaltyAddress(address royaltyAddress) external {
    require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "Only admin can change royalty address");
    _royaltyAddress = royaltyAddress;
  }

  function setPrice(uint mintPrice) external {
    require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "Only admin can change price");
    _price = mintPrice;
  }
  ...
  function mint(uint amount) public payable {
    require(_tokenIdTracker.current() + amount <= _max, "Not enough lions left to mint amount");
    for(uint i=0; i < amount; i++){
      ...
      _setTokenRoyalty(_tokenIdTracker.current(), _royaltyAddress, 500);
      emit Mint(msg.sender);
    }
  }

  function giveawayMint(uint amount, address mintedFor) public {
    require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "Only admin can mint for giveaway");
    require(_tokenIdTracker.current() < _giveAwayMax, "giveaway limit exceeded");
    for(uint i=0; i < amount; i++){
      _mint(mintedFor, _tokenIdTracker.current());
      minter[_tokenIdTracker.current()] = mintedFor;
      lastDividendAt[_tokenIdTracker.current()] = totalDividend;
      _tokenIdTracker.increment();
      _setTokenRoyalty(_tokenIdTracker.current(), _royaltyAddress, 500);
    }
  ...
  function splitBalance(uint256 amount) private {
      uint256 reflectionShare = amount/12;
      uint256 mintingShare  = (amount - reflectionShare);
      reflectDividend(reflectionShare);
      payable(_depositAddress).transfer(mintingShare);
  }
