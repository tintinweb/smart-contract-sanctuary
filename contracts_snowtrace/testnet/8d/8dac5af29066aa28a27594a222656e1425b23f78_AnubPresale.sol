...
contract AnubPresale is ERC20 {

    uint constant MIM_units = 10 ** 18;
    uint constant ANB_units = 10 ** 18;

    uint public constant DEFAULT_ANB_PRICE = 10 * MIM_units / ANB_units ;
    uint public constant MIN_PER_ACCOUNT = 50 * ANB_units;
    uint public constant MAX_PER_ACCOUNT = 250 * ANB_units;
    ...
    mapping( address => Claim ) public dailyClaimed;

    constructor(address[] memory whiteListed_) ERC20("TEST Token", "TST"){
        //0x130966628846BFd36ff31a822705796e8cb8C18D
        MIM = ERC20(0x0A844A8E1e4A80B86098d0E901e01F8874A7cbF0);
        owner = msg.sender;

        // Initialized whiteListed
        for(uint256 i = 0; whiteListed_.length > i; i++ ) {
            whiteListedMap[whiteListed_[i]] = true;
        }

        // Initialized presale
        presale_sold = 0;
        presale_enable = false;
        presale_claim_enable = false;
    }
    ...
    function setMaxPresaleToken(uint256 _max) isOwner external {
        presale_max_to_sold = _max * ANB_units;
    }
    ...
    function amountInMax(address buyer) public view returns (uint256) {
        uint256 maxBuyable = MAX_PER_ACCOUNT;
        return maxBuyable - soldListed[buyer];
    }

    function buyAnubToken(uint256 amountIn) external isFromContract {
        // All requirements
        require(isWhiteListed(msg.sender), "Not whitelised");
        require(presale_sold + amountIn < presale_max_to_sold, "No more token available (limit reached)");
        require(amountIn >= MIN_PER_ACCOUNT, "Amount is not sufficient");
        require(amountIn + soldListed[msg.sender] >= MAX_PER_ACCOUNT, "Amount buyable reached");
        require(amountIn <= amountInMax(msg.sender), "Max token buyable reached");
        ...
        soldListed[msg.sender] += amountIn;
        presale_sold += amountIn;
    }

    function claimAnubToken() external isFromContract {
        // All requirements
        require(soldListed[msg.sender] < dailyClaimed[msg.sender].totalClaimed, "No tokens to claim");
        ...
        if(dailyClaimed[msg.sender].lastClaimed == 0) {
            dailyClaimed[msg.sender].amountClaimable = soldListed[msg.sender] * 20/100; //20% per day
        ...
        if(dailyClaimed[msg.sender].totalClaimed + amountOut > soldListed[msg.sender]) {
            amountOut = soldListed[msg.sender] - dailyClaimed[msg.sender].totalClaimed;
        }
