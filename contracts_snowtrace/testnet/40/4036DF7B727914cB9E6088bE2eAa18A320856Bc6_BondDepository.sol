...
/* ======== CONSTRUCTOR ======== */
  ...
  constructor(
    IEldoradoAuthority _authority,
    IERC20 _elda,
    IgELDA _gelda,
    IStaking _staking,
    ITreasury _treasury
  ) NoteKeeper(_authority, _elda, _gelda, _staking, _treasury) {
    // save gas for users by bulk approving stake() transactions
    _elda.approve(address(_staking), 1e45);
  }
...
/* ======== DEPOSIT ======== */
   ...
   * @return payout_     the amount of gELDA due
  ...
  function deposit(
     ...
     * price = quote tokens : elda (i.e. 42069 DAI : ELDA)
     *
     * 1e27 = ELDA decimals (18) + price decimals (9)
     */
     ...
     *
     * this is either the number of ELDA that the market can sell
    ...
    // purchased, and how much ELDA has been sold
  ...
  function _tune(uint256 _id, uint48 _time) internal {
    ...
    if (_time >= meta.lastTune + meta.tuneInterval) {
      ...
      // elda decimals (18) + price decimals (9)
       ...
       * is 10,000 ELDA. max payout would be 1,000 ELDA (10,000 * 1 / 10).
       */  
...
/* ======== CREATE ======== */
   ...
   * @param _market      [capacity (in ELDA or quote), initial price / ELDA (18 decimals), debt buffer (6 decimals)]
  ...
  function create(
     ...
     *
     * 1e27 = elda decimals (18) + initial price decimals (9)
     */
    uint128 targetDebt = uint128(_booleans[0]
     ...
     * interval. for example, if capacity is 1,000 ELDA, there are 10 days to conclusion, 
     * and the preferred deposit interval is 1 day, max payout would be 100 ELDA.
     */
    ...
    emit CreateMarket(id_, address(elda), address(_quoteToken), _market[1]);
  }
...
/* ======== EXTERNAL VIEW ======== */
   ...
   * @param _id          ID of market
   * @return             price for market in ELDA decimals
   *
   ...
   * @param _id          ID of market
   * @return             amount of ELDA to be paid in ELDA decimals
   *
   * @dev 1e27 = elda decimals (18) + market price decimals (9)
   */
   ...
   * @param _id          ID of market
   * @return             current debt for market in ELDA decimals
   */
   ...
   * @param _id          ID of market
   * @return             control variable for market in ELDA decimals
   */
...
interface IBondDepository {
  ...
  struct Market {
    ...
    bool capacityInQuote; // capacity limit is in payment token (true) or in ELDA (false, default)
  ...
  struct Terms {
    ...
    uint128 maxDebt; // 9 decimal debt maximum in ELDA
  }
...
interface INoteKeeper {
  ...
  struct Note {
    uint256 payout; // gELDA remaining to be paid
  ...
  function redeem(address _user, uint256[] memory _indexes, bool _sendgELDA) external returns (uint256);
  function redeemAll(address _user, bool _sendgELDA) external returns (uint256);
...
abstract contract FrontEndRewarder is EldoradoAccessControlled {
  ...
  IERC20 internal immutable elda; // reward token

  constructor(
    IEldoradoAuthority _authority, 
    IERC20 _elda
  ) EldoradoAccessControlled(_authority) {
    elda = _elda;
  }
  ...
  function getReward() external {
    ...
    elda.transfer(msg.sender, reward);
  }
...
abstract contract NoteKeeper is INoteKeeper, FrontEndRewarder {
  ...
  IgELDA internal immutable gELDA;
  ...
  constructor (
    ...
    IStaking _staking,
    ITreasury _treasury
  ) FrontEndRewarder(_authority, _elda) {
    gELDA = _gelda;
    staking = _staking;
    treasury = _treasury;
  }
...
/* ========== ADD ========== */
   ...
   * @param _payout      the amount of ELDA due to the user
  ...
  function addNote(
    ...
    notes[_user].push(
      Note({
        payout: gELDA.balanceTo(_payout),
    ...
    // note that only the payout gets staked (front end rewards are in ELDA)
...
/* ========== REDEEM ========== */
   ...
   * @param _sendgELDA    send payout as gELDA or sELDA
   * @return payout_     sum of payout sent, in gELDA
   */
  function redeem(address _user, uint256[] memory _indexes, bool _sendgELDA) public override returns (uint256 payout_) {
    ...
    if (_sendgELDA) {
      gELDA.transfer(_user, payout_); // send payout as gELDA
    } else {
      staking.unwrap(_user, payout_); // unwrap and send payout as sELDA
    }
  }
   ...
   * @param _sendgELDA   send payout as gELDA or sELDA
   * @return             sum of payout sent, in gELDA
   */ 
  function redeemAll(address _user, bool _sendgELDA) external override returns (uint256) {
    return redeem(_user, indexesFor(_user), _sendgELDA);
  }
...
/* ========== VIEW ========== */
   ...
   * @return payout_     the payout due, in gELDA
