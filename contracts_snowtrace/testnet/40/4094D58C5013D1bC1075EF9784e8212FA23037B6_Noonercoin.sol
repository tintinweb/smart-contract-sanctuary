       ...
       if(stopTheMinting) {
           ...
           uint256 noonerCoinExtractedFromWei = 0;
            ...
            if(timeDiff >= _cycleTime){  
                uint256 valueForEach = timeDiff/_frequency;    
                uint256 tokenWei = valueForEach * mintingRateNoonerWei;
                uint256 tokens = 0;
                if(totalCycleLeft != 19){
                     tokens =  tokenWei/10**18 + previousCyclesTotalTokens + weiAmountAdded + weiAmountAddedAtBurn + valueForEach * mintingRateNoonerCoin;
                }
                if(totalCycleLeft == 19){
                     tokens =  _fundersAmount + valueForEach * mintingRateNoonerCoin;
                }
