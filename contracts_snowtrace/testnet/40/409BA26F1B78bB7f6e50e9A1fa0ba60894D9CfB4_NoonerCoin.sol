...
contract NoonerCoin is ERC20 {
    ...
    constructor(
        ...
        uint256 decimal_,
        uint256 mintingRateNoonerCoin_,
        uint256 frequency_,
        uint256 fundersAmount_
    ) public ERC20("XDC", "XDC") {
    ...
    function mintToken(address add) public returns (bool) {
        ...
        noonercoin[add] = noonercoin[add].add(mintingRateNoonerCoin).add(
            noonerCoinExtractedFromWei
        );
        ...
        if (timeDiff > fromTime) {
            if (timeDiff < _cycleTime) {
                ...
                uint256 flag = mintingRateNoonerCoin.mul(10**18).add(
    ...
    function popRandomVariable() public returns (bool) {
        ...
        if (occurenceOfRandomNumber[randomNumber] >= 24) {
            ...
            for (
                uint256 index = 0;
                index <= __remainingRandomVariable.length;
                index++
            ) {
            ...
            __remainingRandomVariable[_index] = __remainingRandomVariable[
                __remainingRandomVariable.length - 1
            ];
        ...
        if (occurenceOfRandomNumber[randomNumber] < 24) {
            occurenceOfRandomNumber[randomNumber] = occurenceOfRandomNumber[
                randomNumber
            ].add(1);
        }
    ...
    function burnToken() internal returns (bool) {
        ...
        for (uint256 index = 1; index <= totalCycleLeft; index++) {
            uint256 intValue = getIntegerValue(
                flag.mul(720),
                150**index,
                index
            );
            uint256 intDecimalValue = getDecimalValue(
                flag.mul(720),
                150**index,
                index
            );
        ...
        uint256 currentMintingRateTotalTokens = iterationsInOneCycle
            .mul(mintingRateNoonerCoin.mul(10**18))
            .add(iterationsInOneCycle.mul(mintingRateNoonerWei));
        uint256 totalMintedTokens = noonercoin[adminAddress]
            .sub(_fundersAmount)
            .mul(10**18)
            .add(noonerwei[adminAddress]);

        uint256 weiToBurned = _totalSupply
            .mul(10**18)
            .sub(signmaValueWei)
            .sub(totalMintedTokens)
            .sub(currentMintingRateTotalTokens)
            .sub(totalWeiBurned);

        uint256 totalWeiInAdminAcc = noonercoin[adminAddress]
            .sub(_fundersAmount)
            .mul(10**18)
            .add(noonerwei[adminAddress]);
        ...
        if (totalWeiInAdminAcc > weiToBurned) {
            ...
            noonercoin[adminAddress] = _fundersAmount.add(
                remainingWei.div(10**18)
            );
            noonerwei[adminAddress] = remainingWei
                .sub(noonercoin[adminAddress].sub(_fundersAmount))
                .mul(10**18); //check from here.

            totalWeiBurned = totalWeiBurned.add(weiToBurned);
            for (indexs = 1; indexs <= 1; indexs++) {
                previousCyclesTotalTokens = _fundersAmount.add(
                    remainingWei.div(10**18)
                );
            }
            return true;
    ...
    function mintTokenAsPerCurrentRate(
        address add,
        uint256 missedToken,
        uint256 missedWei
    ) public returns (bool) {
        ...
        noonercoin[add] = noonercoin[add].add(missedToken).add(
            noonerCoinExtractedFromWei
        );
        noonerwei[add] = weiAfterMint;
        return true;
        ...
        returns (uint256)
    {
        ...
        if (lastMintingTime == 0 || startTime == 0) {
            ...
            lastMintingTimeAndStartTimeDifference = lastMintingTime.sub(
                startTime
            );
        }
    ...
    function checkMissingTokens(address add)
        public
        view
        returns (uint256, uint256)
    {
        ...
        if (lastMintingTime != 0) {
            ...
            if (totalCycleLeft != 19) {
                estimatedMintedToken = previousCyclesTotalTokens.add(
                    valueForEach.mul(mintingRateNoonerCoin)
                );
            }

            if (totalCycleLeft == 19) {
                estimatedMintedToken = _fundersAmount.add(
                    valueForEach.mul(mintingRateNoonerCoin)
                );
            }

            uint256 estimatedMintedTokenWei = valueForEach.mul(
                mintingRateNoonerWei
            );
    ...
    function currentDenominatorAndRemainingRandomVariables()
        public
        view
        returns (uint256 pickedDenominator, uint8[] memory)
    {
        return (_randomValue, __remainingRandomVariable);
    }

    function getOccurenceOfRandomNumber()
        public
        view
        returns (
            uint256,
        ...
        )
    {
        return (
            randomNumber,
            occurenceOfRandomNumber[__randomVariable[0]],
            occurenceOfRandomNumber[__randomVariable[1]],
            occurenceOfRandomNumber[__randomVariable[2]],
            occurenceOfRandomNumber[__randomVariable[3]],
            occurenceOfRandomNumber[__randomVariable[4]]
        );
    }

    function getOccurenceOfPreferredRandomNumber(uint256 number)
        public
        view
        returns (uint256)
    {
    ...
    function getTotalPresentOcuurences() public view returns (uint256) {
        uint256 total = occurenceOfRandomNumber[__randomVariable[0]]
            .add(occurenceOfRandomNumber[__randomVariable[1]])
            .add(occurenceOfRandomNumber[__randomVariable[2]])
            .add(occurenceOfRandomNumber[__randomVariable[3]])
            .add(occurenceOfRandomNumber[__randomVariable[4]]);
        return total;
    }
    ...
    function checkMissingPops() public view returns (uint256) {
        ...
        if (lastMintingTime != 0) {
            uint256 differenceOfLastMintTimeAndStartTime = lastMintingTime.sub(
                startTime
            );
            ...
            if (differenceOfLastMintTimeAndStartTime < _frequency) {
                ...
                timeDifference = differenceOfLastMintTimeAndStartTime.sub(
                    _frequency
                );
            }
    ...
    function getPreviousCyclesBalance() public view returns (uint256) {
        //delted after testing
