...
// File: contracts/SQUIDScombined.sol
...
contract SimpleNftLowerGas is ERC721, Ownable {
    ...
    constructor() ERC721("SQUIDS", "SSG") {
        setHiddenMetadataUri(
            "ipfs://QmRfhSntGWoayUk5zMBG4He8WaqSnwwirx1aZ1vdUpKq89"
        );
        mint(20);
    }
        ...
        mintCompliance(_mintAmount)
    {
        ...
        if (supply.current() > 0) {
            giftAddress = payable(
                ownerOf(randomNum(supply.current(), block.timestamp, supply.current() + 1) + 1)
            );
        ...
        if (msg.sender != owner()) {
            if (whitelisted[msg.sender] != true) {
                require(msg.value >= cost * _mintAmount, "Not enough money!");
            }
        }
