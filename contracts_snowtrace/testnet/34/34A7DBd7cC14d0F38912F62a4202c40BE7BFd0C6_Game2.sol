...
contract Game2 is Ownable, ERC1155Holder, Pausable {
    ...
    function joinGame() external whenNotPaused {
        ...
        require(lastGameTimestamps[msg.sender] - block.timestamp >= epoch, "wait for join timeout");
        uint256 gameId = _gameIds.current();
        GameInfo storage game_ = _games[gameId];
        ...
        if (game_.player[0].addr == address(0)) {
            ...
            game_.started = true;
            emit GameStarted(game_.gameId);
            _createGame();
        }
        emit PlayerEntered(game_.gameId, msg.sender);
    ...
    function _finishGame(GameInfo storage game_) private {
        ...
        if (balance != 0) {
            _playerWins.set(game_.winner, _playerWins.get(game_.winner) + 1);
            _phi.safeTransfer(game_.winner, game_.bank * (1e6 - fee));
        } else {
    ...
    function _abort(GameInfo storage game_, address winner) private {
        ...
        if (winner == address(0)) {
            ...
            _phi.safeTransfer(winner, game_.bank * (1e6 - fee));
            _playerWins.set(winner, _playerWins.get(winner) + 1);
        }
