...
contract NODERewardManagement {
    ...
    bool public createSingleNodeEnabled = true;
    bool public createMultiNodeEnabled = true;
    ...
    function getNodeInfo(
        ...
        uint256 validIndex = getNodeIndexByCreationTime(_nodesOfUser[account], _creationTime);
     
        require(validIndex > 0, "NODE SEARCH: No NODE Found with this blocktime");
        return _nodesOfUser[account][validIndex];
    }
    ...
    function _getNodeWithCreatime(
        ...
        require(_creationTime > 0, "NODE: CREATIME must be higher than zero");
        uint256 validIndex = getNodeIndexByCreationTime(nodes, _creationTime);
    ...
    external onlyManager
    {
        ...
        uint256 nodeIndex = getNodeIndexByCreationTime(_nodesOfUser[account], _creationTime);
        _nodesOfUser[account][nodeIndex].rewardNotClaimed += amount;
    }
    ...
    */
    ...
    returns (uint256)
    {
        ...
        _nodesOfUser[account][nodeIndex].rewardNotClaimed = 0;
        _nodesOfUser[account][nodeIndex].lastClaimTime = block.timestamp;
    ...
    )
    {
        ...
        while (gasUsed < gas && iterations < numberOfnodeOwners) {
            ...
            for (uint256 i = 0; i < _nodesOfUser[account].length; i++) {
                uint256 nodeIndex = getNodeIndexByCreationTime(_nodesOfUser[account], _nodesOfUser[account][i].creationTime);
                _nodesOfUser[account][nodeIndex].rewardNotClaimed += claimableAmount(_nodesOfUser[account][i].lastClaimTime) + _nodesOfUser[account][i].rewardNotClaimed;
                _nodesOfUser[account][nodeIndex].lastClaimTime = block.timestamp;
                claims++;
            }
            iterations++;
    ...
    )
    {
        ...
        while (gasUsed < gas && iterations < numberOfnodeOwners) {
            ...
            for (uint256 i = 0; i < _nodesOfUser[account].length; i++) {
                ...
                //_nodesOfUser[account][i].rewardAvailable += rewardAmount;
                _nodesOfUser[account][nodeIndex].rewardNotClaimed += rewardAmount;
                _nodesOfUser[account][nodeIndex].lastClaimTime = block.timestamp;
