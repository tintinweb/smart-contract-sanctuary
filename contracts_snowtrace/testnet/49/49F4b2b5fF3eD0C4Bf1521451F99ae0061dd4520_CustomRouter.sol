...
contract CustomRouter is Ownable {
    ...
    function route(uint256[] memory amounts, address[] memory path, address _to) private {
    ...
    function pairFor_(address factory_, address tokenA_, address tokenB_, bytes32 creationCode_) public pure returns (address pair_) {
        ...
        pair_ = address(uint256(keccak256(abi.encodePacked(
                hex'ff',
                factory_,
    ...
    function getReserves_(address factory_, address tokenA_, address tokenB_, bytes32 creationCode_) public view returns (uint256 reserveA_, uint256 reserveB_) {
        (address token0_,) = sortTokens_(tokenA_, tokenB_);
        pairFor_(factory_, tokenA_, tokenB_, creationCode_);
        (uint256 reserve0_, uint256 reserve1_,) = IUniswapV2Pair(pairFor_(factory_, tokenA_, tokenB_, creationCode_)).getReserves();
        (reserveA_, reserveB_) = tokenA_ == token0_ ? (reserve0_, reserve1_) : (reserve1_, reserve0_);
    }


        // performs chained getAmountOut calculations on any number of pairs
    function getAmountsOut_(address factory_, uint256 amountIn_, address[] memory path_, bytes32 creationCode_) public view returns (uint256[] memory amounts_) {
        ...
        for (uint256 i; i < path_.length - 1; i++) {
            (uint256 reserveIn_, uint256 reserveOut_) = getReserves_(factory_, path_[i], path_[i + 1], creationCode_);
            amounts_[i + 1] = getAmountOut_(amounts_[i], reserveIn_, reserveOut_);
        }
    }

        // given an input amount of an asset and pair reserves, returns the maximum output amount of the other asset
    function getAmountOut_(uint256 amountIn_, uint256 reserveIn_, uint256 reserveOut_) public pure returns (uint256 amountOut_) {
        ...
        uint256 amountInWithFee_ = amountIn_.mul(997);
        uint256 numerator_ = amountInWithFee_.mul(reserveOut_);
        uint256 denominator_ = reserveIn_.mul(1000).add(amountInWithFee_);
...
library UniSwapV2Library {
    ...
    // fetches and sorts the reserves for a pair
    function getReserves(address factory, address tokenA, address tokenB, bytes32 creationCode) internal view returns (uint256 reserveA, uint256 reserveB) {
        (address token0,) = sortTokens(tokenA, tokenB);
        pairFor(factory, tokenA, tokenB, creationCode);
        (uint256 reserve0, uint256 reserve1,) = IUniswapV2Pair(pairFor(factory, tokenA, tokenB, creationCode)).getReserves();
    ...
    // performs chained getAmountOut calculations on any number of pairs
    function getAmountsOut(address factory, uint256 amountIn, address[] memory path, bytes32 creationCode) internal view returns (uint256[] memory amounts) {
        require(path.length >= 2, 'UniSwapV2Library: INVALID_PATH');
        amounts = new uint256[](path.length);
        amounts[0] = amountIn;
        for (uint256 i; i < path.length - 1; i++) {
            (uint256 reserveIn, uint256 reserveOut) = getReserves(factory, path[i], path[i + 1], creationCode);
            amounts[i + 1] = getAmountOut(amounts[i], reserveIn, reserveOut);
        }
    }

    // performs chained getAmountIn calculations on any number of pairs
    function getAmountsIn(address factory, uint256 amountOut, address[] memory path, bytes32 creationCode) internal view returns (uint256[] memory amounts) {
        require(path.length >= 2, 'UniSwapV2Library: INVALID_PATH');
        amounts = new uint256[](path.length);
        amounts[amounts.length - 1] = amountOut;
        for (uint256 i = path.length - 1; i > 0; i--) {
            (uint256 reserveIn, uint256 reserveOut) = getReserves(factory, path[i - 1], path[i], creationCode);
