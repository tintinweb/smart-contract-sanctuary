...
contract PresaleSplitterV2 is Ownable {
   using SafeMath for uint256;
    address[] PRESALE_WALLETS = [
        ...
        0x78B443BC4766D4265FD989a21fb248b2D1D9e207,
        0xb6e76628BeB7872D2ade6AE9641bb390401c18ef,
        0xf69089B8e61544151eA33ABF369123D345302Cc4
    ];
    ...
    constructor() {
        ...
        for (uint256 i = 0; i<PRESALE_WALLETS.length;i++) {
            tokenAmount[PRESALE_WALLETS[i]] = random();
        }
    }
    ...
    function random() private view returns (uint) {
        uint randomHash = uint(keccak256("rockandroll"));
        ...
        returns (uint256 claimAmount)
    {
        if (timesClaimed[msg.sender] == 0) {
            return tokenAmount[msg.sender].div(100).mul(30);
        }
        if (
            firstClaimInterval < block.timestamp &&
            timesClaimed[msg.sender] == 1
        ) {
            return tokenAmount[msg.sender].div(100).mul(35);
        }
        if (
