...
contract Game2 is Ownable, ERC1155Holder, Pausable {
    ...
    Resource internal immutable _resource;
    IERC20 internal immutable _phi;
    GameLending internal immutable _gameLending;
    ...
    function _roundWinner(GameInfo storage game_, uint256 round) private view returns (int8) {
        ...
        for (uint8 p=0; p < 2; p++) {
            ...
            else if (opponentPower.used && opponentPower.powerType == PowerType.Water)
                multiplier = 75;
            else
                multiplier = 100;
            weights[p] = _resource.getResourceWeight(game_.player[p].placedCards[round]) * multiplier / 100;
