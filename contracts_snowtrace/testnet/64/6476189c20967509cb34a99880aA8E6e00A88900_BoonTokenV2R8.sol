...
contract BoonTokenV2R8 is ERC20, AccessControl {
    using SafeMath for uint256;

    bytes32 public constant BURNER_ROLE = keccak256("BURNER");
    address public constant BURN_ADDRESS = address(uint160(uint256(keccak256(abi.encodePacked("KIEN")))));
    address public constant ZERO_ADDRESS = 0x0000000000000000000000000000000000000000;
    uint256 totalLiquidity = uint256(700_000_028).mul(1e18);
    ...
    constructor() ERC20("BoonTokenV2R8", "BOON") {
        ...
        angelInvestor = VestingType(15 seconds, 4 hours, 40, 0);
        privateSale = VestingType(10 seconds, 1 hours, 30, 0);
    ...
    function grantTokenToInvestor(address _to, uint256 _amount) public onlyAdmin(_msgSender()) {
        require(
            _amount.add(angelInvestor.totalTransfer) <
                totalLiquidity.mul(angelInvestor.amount).div(BASE_RATE),
            "VestingToken: Exceed allowed amount"
        );
        require(
            address(VestingWalletInfo[_to]) == ZERO_ADDRESS,
            "VestingToken: Token has already granted to this address"
        );
        require(
            _to != ZERO_ADDRESS,
            "VestingToken: Can't send to zero address"
        );
    ...
    function grantTokenToPrivateSale(address _to, uint256 _amount) public onlyAdmin(_msgSender()) {
        require(
            _amount.add(privateSale.totalTransfer) < totalLiquidity.mul(privateSale.amount).div(BASE_RATE),
            "VestingToken: Exceed allowed amount"
        );
        require(
            address(VestingWalletInfo[_to]) == ZERO_ADDRESS,
    ...
    function grantTokenToPublicSale(address _to, uint256 _amount) public onlyAdmin(_msgSender()) {
        require(
            _amount.add(publicSale.totalTransfer) <
                totalLiquidity.mul(publicSale.amount).div(BASE_RATE),
            "VestingToken: Exceed allowed amount"
        );
        require(
            address(VestingWalletInfo[_to]) == ZERO_ADDRESS,
        ...
        uint256 vestingStartDate = block.timestamp.add(
            publicSale.cliffDuration
        );
        VestingWallet vestingWalletInfo = new VestingWallet(
            _to,
            uint64(vestingStartDate),
            uint64(publicSale.vestingDuration)
        );
        VestingWalletInfo[_to] = vestingWalletInfo;
        userGrantedAmount[_to] = userGrantedAmount[_to].add(_amount);
    ...
    function getLockedAmount(address _user) public view returns (uint256) {
        return
            userGrantedAmount[_user].sub(
                VestingWalletInfo[_user].vestedAmount(
                    address(this),
    ...
    function pendingUnlockedTokenToClaim(address _user) public view returns (uint256)  {
        return VestingWalletInfo[_user]
            .vestedAmount(address(this), uint64(block.timestamp))
            .sub(VestingWalletInfo[_user].released(address(this)));
    }
