...
contract NFT_Minter is ERC721Enumerable, Ownable {
    ...
    function minter_balanceOf(address owner) public view virtual returns (uint256) {
        require(owner != address(0), "ERC721: balance query for the zero address");
        return minter_balances[owner];
    }

    function minter_tokenByIndex(address owner, uint256 index) public view virtual returns (uint256) {
        require(index < minter_balanceOf(owner), "ERC721Enumerable: owner index out of bounds");
        return minter_tokens[owner][index];
    }

    function claimRewards() public {
        uint count = minter_balanceOf(msg.sender);
        uint256 balance = 0;
        for (uint i = 0; i < count; i++) {
            uint tokenId = minter_tokenByIndex(msg.sender, i);
    ...
    function claimReward(uint256 tokenId) public {
        require(tokenMinter(tokenId) == _msgSender() || getApproved(tokenId) == _msgSender(), "Only owner or approved can claim rewards");
