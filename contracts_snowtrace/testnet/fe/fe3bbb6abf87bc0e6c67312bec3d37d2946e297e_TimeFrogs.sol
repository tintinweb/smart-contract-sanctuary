...
/**
    ...
    DeFi Degen will not be liable in any way if for the use 
...
contract Multisig is Ownable {
  ...
  address[] private _signatoryAddresses;

  uint private _signatureCount = 0;
  uint private _noRequiredSignatures = 1;
  ...
  function signTx() public {
    require (msg.sender == _primarySignatory || msg.sender == _signatory_2 || msg.sender == _signatory_3 || msg.sender == _signatory_4, 'ERROR: You are not a signatory');
...
contract TimeFrogs is ERC721Enumerable, Ownable, Multisig(msg.sender) {
  using Strings for uint256;
  ...
  mapping(uint256 => address) private winnerLog;
  ...
  */
  function mint(uint256 _mintAmount) public payable {
    ...
    require(ownerCanMintCount >= _mintAmount, 'ERROR: You cant mint that many frogs');
    ...
    require(supply + _mintAmount <= maxSupply, 'ERROR: Not enough Frogs left to mint!');

    if (!noFeeAddresses[msg.sender]) {
  ...
  */
  function selectWinner() public onlySignatories returns(address) {
    ...
    uint winningToken = uint(keccak256(abi.encodePacked(epochNow, block.difficulty, msg.sender))) % (maxSupply + 1);
    address winnerAddress = _tokenToAddressMap[winningToken];
    winnerLog[epochNow] = winnerAddress;
  ...
  /*
    ...
    @description - Gets the last lottery winner
    @returns <address>
  */
  function getLastWinner() public view returns(address) {
      require(lotteryActive, 'ERROR: Lottery not active yet');
      require(winners.length > 0, 'ERROR: No winners yet');
      return winnerLog[winners.length - 1];
  }

  /*
    @function getAllWinners()
    @description - Gets the list of previous winners
    @returns <address[]>
   */
  ...
  */
  ...
  /*
    @function getLotteryDates(value)
    @description - Gets the lottery dates
    @param <uint256[]> memory - The list of lotto dates
  */
  function getLotteryDates() public view returns(uint256[] memory) {
      return lotteryDates;
  }

  function castVote(string memory option) public {
      require(!paused, 'ERROR: Contract paused');
      require(pollOpen, 'ERROR: No poll to vote on right now');
      require(pollOptionsMap[option], 'ERROR: Invalid voting option');
      require(keccak256(abi.encodePacked(votedAddresses[msg.sender])) != keccak256(abi.encodePacked(pollName)), 'ERROR: You have already voted in this poll!');
      
      uint256 noVotes = balanceOf(msg.sender);

      require(noVotes > 0, 'ERROR: You have no voting rights. Get yourself a frog my fren!');
  ...
  function togglePollVoting(bool value) public onlySignatories {
      pollOpen = value;
  }
