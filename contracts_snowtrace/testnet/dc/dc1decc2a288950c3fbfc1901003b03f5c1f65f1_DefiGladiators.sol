...
contract DefiGladiators is Ownable, ERC721Enumerable {

    mapping(address => bool) public isMintPassHolder;



    uint256 constant MAXSUPPLY = 5550;
    ...
    // address constant wallet1 = address(0);

    // address constant wallet2 = address(0);



    uint256 public price_mintpass = 0.1 ether;
    ...
    function buy(uint256 _count) external payable {
        ...
        require(_count > 0, "Mint at least one NFT");

        require(_count <= MAX_MINT_PER_TX, "Max 3 mints allowed");

        require(msg.value == PRICE * _count, "Incorrect AVAX amount");
