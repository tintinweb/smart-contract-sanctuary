...
contract Vault is ERC20("Staked Taco", "sTACO"), Ownable {
    using SafeMath for uint256;
    Taco public taco;

    uint256 public constant DELAYED_UNSTAKE_LOCKUP_PERIOD = 2 days;
    uint256 public constant QUICK_UNSTAKE_PENALTY_PERCENT = 60;
    uint256 public constant QUICK_UNSTAKE_VAULT_PERCENT = 30;

    uint256 public frozenTaco; // $TACO pending withdraw
    ...
    function setTaco(Taco _taco) external onlyOwner {
        require(address(taco) == address(0), "Taco contract already set");
    ...
    function stake(uint256 _amount) public {
        ...
        // If no sTACO exists, mint it 1:1 to the amount put in
        if (totalShares == 0 || tacoBalance() == 0) {
            _mint(msg.sender, _amount);
        } else {
            uint256 what = _amount.mul(totalShares).div(tacoBalance());
            _mint(msg.sender, what);
        }

        taco.transferToVault(msg.sender, _amount);
    }

    function quickUnstake(uint256 _share) public {
        uint256 output = _unstakeOutput(_share).mul(QUICK_UNSTAKE_PENALTY_PERCENT).div(100);
        uint256 output_burn = _unstakeOutput(_share).mul(100-QUICK_UNSTAKE_PENALTY_PERCENT-QUICK_UNSTAKE_VAULT_PERCENT).div(100);
        _burn(msg.sender, _share);
        taco.transfer(msg.sender, output);
        taco.burn(address(this), output_burn);
    }

    // argument specified in sTACO
    ...
    // argument specified in TACO, not sTACO
    function claimDelayedUnstake(uint256 _amount) public {
        require(block.timestamp >= unlockTimestamps[msg.sender], "TACO not yet unlocked");
        require(_amount <= unlockAmounts[msg.sender], "Insufficient locked balance");

        unlockAmounts[msg.sender] -= _amount;
        taco.transfer(msg.sender, _amount);
        frozenTaco -= _amount;
    }

    function tacoBalance() public view returns (uint256 balance) {
        balance = taco.balanceOf(address(this)) - frozenTaco;
    }
