...
contract ATNPreSale is Ownable {
  ...
  constructor (
    ...
    sellAmount = 0;
    limit1 = 800000000000000000000;
    limit2 = 1200000000000000000000;
  }
  ...
  function redeemATN() public onlyWhileClose onlyWhitelisted {
    require(preBuys[msg.sender].mimAmount > 0, "Already redeemed");

    uint256 percentReleased = getPercentReleased();

    uint256 totalATNToClaim = preBuys[msg.sender]
      .mimAmount
      .mul(rate)
      .div(RATE_DECIMALS)
      .mul(percentReleased)
    ...
    uint256 atnToClaim = totalATNToClaim.sub(
      preBuys[msg.sender].atnClaimedAmount
    ).div(1e9); // MIM decimals: 18, ATN decimals: 9

    preBuys[msg.sender].mimAmount = 0;
    preBuys[msg.sender].atnClaimedAmount = preBuys[msg.sender]
      .atnClaimedAmount
      .add(atnToClaim);

    IERC20Mintable( alphaATN ).mint( msg.sender, atnToClaim );
  }
}