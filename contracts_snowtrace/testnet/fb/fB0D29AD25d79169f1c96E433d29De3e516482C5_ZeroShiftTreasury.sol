...
contract ZeroShiftTreasury is Ownable {
    using SafeMath for uint256;
    ...
    function deposit(
        ...
        IERC20Mintable(ZeroShift).mint(msg.sender, send_);
    ...
    function withdraw(uint256 _amount, address _token) external {
        ...
        uint256 value = valueOf(_token, _amount);
        IOHMERC20(ZeroShift).burnFrom(msg.sender, value);
    ...
    function incurDebt(uint256 _amount, address _token) external {
        ...
        uint256 maximumDebt = IERC20(sZeroShift).balanceOf(msg.sender); // Can only borrow against sOHM held
    ...
    function repayDebtWithOHM(uint256 _amount) external {
        ...
        IOHMERC20(ZeroShift).burnFrom(msg.sender, _amount);

        debtorBalance[msg.sender] = debtorBalance[msg.sender].sub(_amount);
        totalDebt = totalDebt.sub(_amount);

        emit RepayDebt(msg.sender, ZeroShift, _amount, _amount);
    }
    ...
    function mintRewards(address _recipient, uint256 _amount) external {
        ...
        IERC20Mintable(ZeroShift).mint(_recipient, _amount);
    ...
    function excessReserves() public view returns (uint256) {
        return
            totalReserves.sub(IERC20(ZeroShift).totalSupply().sub(totalDebt));
    }
        ...
        returns (uint256 value_)
    {
        if (isReserveToken[_token]) {
            ...
            value_ = _amount.mul(10**IERC20(ZeroShift).decimals()).div(
