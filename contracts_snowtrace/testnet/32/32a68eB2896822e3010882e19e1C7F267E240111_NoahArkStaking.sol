...
contract NoahArkStaking is Ownable {
     ...
     */
    function canClaimBalance ( address _recipient ) public view returns ( uint ) {
    ...
    function settleWarmupInfo(address _user, uint _amount) external {
        ...
        Claim[] storage info = warmupInfo[ _user ];
        uint _infoLength = info.length;
        uint _tempGons = IsNRK( sNRK ).gonsForBalance( _amount );
        for (uint i = 0 ; i < _infoLength ; i ++) {
            Claim storage claim = info[i];
            if ( epoch.number >= claim.expiry && claim.expiry != 0 ) {
                if (claim.gons <= _tempGons) {
                    delete info[ i ];
                    _infoLength --;
                    i --;
                    _tempGons = _tempGons - claim.gons;
                }else {
                    claim.gons = claim.gons - _tempGons;
                    claim.deposit = claim.deposit - IsNRK( sNRK ).balanceForGons( _tempGons );
                    _tempGons = 0;
                    break;
                }
            }
        }
        require(_tempGons == 0, 'snrk balance not enough' );
    }
