...
contract NFT_Minter is ERC721Enumerable, Ownable {
  ...
  uint256 public preMintAmount = 5;
  ...
  function mint(address _to, uint256 _mintAmount) public payable {
    ...
    for (uint256 i = 1; i <= _mintAmount; i++) {
      uint256 currentId = supply + i;
      _safeMint(_to, currentId);
      minter[currentId] = msg.sender;
      if (currentId > preMintAmount) {
          lastDividendAt[currentId] = totalDividend;
  ...
  function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal virtual override {
    if (totalSupply() > tokenId && tokenId > preMintAmount) claimReward(tokenId);
    super._beforeTokenTransfer(from, to, tokenId);
  }

  function currentRate() public view returns (uint256){
      if(totalSupply() == 0) return 0;
      return reflectionBalance/(totalSupply() - preMintAmount);
  }
 ...
 function whitelistUser(address _user) public onlyOwner {
  ...
  function claimReward(uint256 tokenId) public {
    ...
    require(tokenId > preMintAmount, "Cannot claim rewards on pre-minted items");
  ...
  function reflectDividend(uint256 amount) private {
    ...
    totalDividend = totalDividend + (amount / (totalSupply() - preMintAmount));
  }
