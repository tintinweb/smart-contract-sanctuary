...
library ECDSA {
     ...
     * See {recover}.
     */
    function toTypedDataHash(bytes32 domainSeparator, bytes32 structHash) internal returns (bytes32) {
        emit DomainSeparator(domainSeparator);
...
abstract contract SigCheckable is EIP712 {
    ...
    */
    function signer(
        bytes32 message,
        bytes memory signature) internal  returns (bytes32 digest, address _signer) {
...
// File contracts/bridge/BridgePool.sol

 pragma solidity ^0.8.0;
contract BridgePoolAvax is SigCheckable, WithAdmin {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    ...
    event BridgeSwap(address from,
        ...
        uint256 amount);
    event WithDrawSignedVerifyParameters(
        address token,
        address payee,
        uint256 amount,
        bytes32 salt,
    ...
    event WithDrawSignedVerifyOutput(
        bytes32 digest,
        address signer
    );
    event WithDrawSigner(
        address signer
    );
    event withdrawSignedMethodHash(
        bytes32 message,
        bytes32 signedMethod
    );
    ...
    external returns(uint256) {
		...
		require(salt != 0, "BP: bad salt");
		require(amount != 0, "BP: bad amount");
        //Structure Hash *message*
        bytes32 message = withdrawSignedMessage(token, payee, amount, salt);
        address _signer = signerUnique(message, signature);
        emit withdrawSignedMethodHash(message,WITHDRAW_SIGNED_METHOD);
    ...
    external  returns (bytes32, address) {
        emit WithDrawSignedVerifyParameters(token, payee,amount,salt,signature);
        bytes32 message = withdrawSignedMessage(token, payee, amount, salt);
        (bytes32 digest, address _signer) = signer(message, signature);
        emit WithDrawSignedVerifyOutput(digest,_signer);
