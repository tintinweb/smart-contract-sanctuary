...
contract Barn is Ownable, IERC721Receiver, Pauseable {
   ...
   */
    function addManyToBarnAndPack(address account, uint16[] calldata tokenIds) external nonReentrant {
   ...
   */
    function claimManyFromBarnAndPack(uint16[] calldata tokenIds, bool unstake) external nonReentrant whenNotPaused _updateEarnings {
        uint256 owed = 0;
        for (uint i = 0; i < tokenIds.length; i++) {
            if (isSheep(tokenIds[i]))
   ...
   */
    function _claimSheepFromBarn(uint256 tokenId, bool unstake) internal returns (uint256 owed) {
        ...
        if (unstake) {
            ...
            woolf.transferFrom(address(this), _msgSender(), tokenId); // send back Sheep
            delete barn[tokenId];
            totalSheepStaked -= 1;
        } else {
   ...
   */
    function _claimWolfFromPack(uint256 tokenId, bool unstake) internal returns (uint256 owed) {
        ...
        if (unstake) {
            ...
            woolf.transferFrom(address(this), _msgSender(), tokenId); // Send back Wolf
