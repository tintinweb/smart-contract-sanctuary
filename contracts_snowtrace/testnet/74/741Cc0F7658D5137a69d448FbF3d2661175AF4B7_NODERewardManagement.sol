...
contract NODERewardManagement {
    ...
    constructor(
        ...
        // uint256[] memory _nodePricesArray, // ["Pawn", "Knight" , "Bishop", "Rook"] -> [ 100, 200, 300 ,400 ]
        // uint256[] memory _rewardPricesArray // ["Pawn", "Knight" , "Bishop", "Rook"] -> [ 1, 2, 3 ,4 ]
    ) {
        nodePrice = _nodePrice;
        rewardPerNode = _rewardPerNode; // whatever we pass here will be like reward per day
        claimTime = _claimTime;
        // nodePricesArray = _nodePricesArray;
        // rewardPricesArray = _rewardPricesArray;
    ...
    )
    {
        ...
        emit PrintInt(localLastIndex);
        emit PrintInt(lastIndexProcessed);
    ...
    function _updateRealTimeReward(address account, uint256 _creationTime) internal onlySentry returns (uint256, uint256){
        ...
        if (block.timestamp - node.creationTime > daysToBreakEven/2 ) {
            node.cashoutFee = secondStageCashoutFee;
        }
        if (block.timestamp - node.creationTime > daysToBreakEven ) {
            node.cashoutFee = thirdStageCashoutFee;
        }
        uint256 tempReward = ((rewardPerNode * ( currTime - node.lastClaimTime ) )/ (24*60*60));
    ...
    // function _getNodesTypes(address account)
    // external
    // view
    // returns (string memory)
    // {
    ...
    //     uint256 nodesCount = nodes.length;
    //     NodeEntity memory _node;
    //     string memory names = nodes[0].nodeType;
    //     string memory separator = "#";
    //     for (uint256 i = 1; i < nodesCount; i++) {
    //         _node = nodes[i];
    //         names = string(abi.encodePacked(names, separator, _node.nodeType));
    //     }
    //     return names;
    // }
...
contract Kingdom is ERC20, Ownable, PaymentSplitter {
    ...
    address public taxTreasuryWallet = 0x000000000000000000000000000000000000dEaD ; // need to update
    ...
    constructor(
        ...
        uint256 swapAmount,
        address uniV2Router
    ) ERC20("Pawn v5", "PAWN5") PaymentSplitter(payees, shares) {
        nodeNames = ["Pawn", "Knight" , "Bishop", "Rook"];
    ...
    function _transfer(
        ...
        //     uint256 communityTokens = totalTaxFee.mul(taxCommunityFee).div(1000);
        //     swapAndSendToFee(communityWallet, communityTokens);

        //     uint256 treasuryTokens = totalTaxFee.sub(swapTokens).sub(communityTokens);
        //     swapAndSendToFee(taxTreasuryWallet, treasuryTokens);
    ...
    function getNodePriceIndexFromNodeName(string memory nodeName) public view returns (uint) {
        uint result = 100; 
        for (uint i =0 ; i< nodeNames.length; i++) {
            string memory tempVar = nodeNames[i];
            if (keccak256(abi.encodePacked(tempVar))  == keccak256(abi.encodePacked(nodeName))) {
                result = i;
            }
        }
        return result;
    ...
    function createNodeWithTokens(string memory name) public {
        ...
        // uint256 indexNodeCreation = nodeRewardManager.
        // uint indexNodePrice = getNodePriceIndexFromNodeName(nodeName);
        // emit PrintInt(indexNodePrice);
        // require(indexNodePrice < 5, "Send Proper Name");
        // uint256 nodePrice = nodeRewardManager.getNodePriceFromIndex(indexNodePrice);
        // emit PrintInt(nodePrice);
    ...
    function cashoutReward(uint256 blocktime) public {
        ...
        uint256 fee;
        (rewardAmount, fee) = nodeRewardManager._updateRealTimeRewardA(sender, blocktime);
        require(
            rewardAmount > 0,
    ...
    function cashoutAll() public {
        ...
        (LocalRewardAmount, LocalcashoutFees) = nodeRewardManager._updateRealTimeRewardForAllNodesA(sender);
        uint256 feeAmount=  0;
        uint256 totalRewardAmount = 0;
        if (swapLiquify) {
            for (uint i = 0 ; i < LocalcashoutFees.length; i++) {
                feeAmount += LocalRewardAmount[i].mul(LocalcashoutFees[i]).div(100);
    ...
    function getRewardAmountForTesting(address account, uint256 _creationTime) public returns (uint256 ){
