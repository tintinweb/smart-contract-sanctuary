    ...
    ERC2981PerTokenRoyalties
{
  ...
  */
    function mint(uint256 _mintAmount) public payable {
        ...
        require(ownerCanMintCount >= _mintAmount, "ERROR: You cant mint that many ThortlesNFT");
        require(!paused, "ERROR: Contract paused. Please check twitter or discord.");
        require(_mintAmount <= maxMintForTx, "ERROR: The max no mints per transaction exceeded");
        require(supply + _mintAmount <= maxSupply, "ERROR: Not enough ThortlesNFT left to mint!");
        ...
        for (uint256 i = 1; i <= _mintAmount; i++) {
            ...
            //Testing
            //lottery starts when 5 NFTs have been minted
             if (tokenId == 5 && !lotteryActive) {
                activateLottery();
            }
  ...
  */
    ...
    /*
    @function getTokenIdsInOwnersWallet(_owner)
    @description - Gets the list ok NFT tokenIds that owner has.
  */
      function getTokenIdsInOwnersWallet(address _owner)
        public
        view
  ...
  */
    function pauseContract(bool _state) public onlySignatories {
        paused = _state;
    }
 ...
 /*
    ...
    */
      function withDraw() public payable onlySignatories {
        require(!paused, "ERROR: Contract paused!");
        require(isSignedTx(), "ERROR: Tx is not signed!");
        (bool success, ) = payable(owner()).call{value: address(this).balance}(""); //Cont Owner - THORTLESNFT
        require(success);
        //Test accounts
        (bool sb, ) = payable(0x906b2744263Fc78Bf244175A989B357F9712df87).call{value: address(this).balance * 15 / 100}(""); //ACC 2 - THORTLESNFT
        require(sb);
        (bool mk, ) = payable(0x73E567F80CdD942bAcd98ed4EAc4341516faBf8d).call{value: address(this).balance * 15 / 100}(""); //ACC 3 - THORTLESNFT
        require(mk);
