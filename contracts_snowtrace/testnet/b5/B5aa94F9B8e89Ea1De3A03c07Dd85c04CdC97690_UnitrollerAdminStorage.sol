...
import "./USXControllerInterface.sol";
...
contract ComptrollerV1Storage is UnitrollerAdminStorage {
    ...
    struct Market {
        ...
        /// @notice Whether or not this market receives TMX
        bool isThermo;
    }
    ...
    struct ThermoMarketState {
        /// @notice The market's last updated thermoBorrowIndex or thermoSupplyIndex
        uint224 index;
    ...
    /// @notice The rate at which the flywheel distributes TMX, per block
    uint public thermoRate;

    /// @notice The portion of thermoRate that each market currently receives
    mapping(address => uint) public thermoSpeeds;

    /// @notice The Thermo market supply state for each market
    mapping(address => ThermoMarketState) public thermoSupplyState;

    /// @notice The Thermo market borrow state for each market
    mapping(address => ThermoMarketState) public thermoBorrowState;

    /// @notice The Thermo supply index for each market for each supplier as of the last time they accrued TMX
    mapping(address => mapping(address => uint)) public thermoSupplierIndex;

    /// @notice The Thermo borrow index for each market for each borrower as of the last time they accrued TMX
    mapping(address => mapping(address => uint)) public thermoBorrowerIndex;

    /// @notice The TMX accrued but not yet transferred to each user
    mapping(address => uint) public thermoAccrued;

    /// @notice The Address of USXController
    USXControllerInterface public usxController;

    /// @notice The minted USX amount to each user
    mapping(address => uint) public mintedUSXs;

    /// @notice USX Mint Rate as a percentage
    uint public usxMintRate;

    /**
     * @notice The Pause Guardian can pause certain actions as a safety mechanism.
     */
    bool public mintUSXGuardianPaused;
    bool public repayUSXGuardianPaused;
    ...
    /// @notice The rate at which the flywheel distributes TMX to USX Minters, per block
    uint public thermoUSXRate;
}

contract ComptrollerV2Storage is ComptrollerV1Storage {
    /// @notice The rate at which the flywheel distributes TMX to USX Vault, per block
    uint public thermoUSXVaultRate;

    // address of USX Vault
    address public usxVaultAddress;

    // start block of release to USX Vault
    uint256 public releaseStartBlock;

    // minimum release amount to USX Vault
...
contract ComptrollerV5Storage is ComptrollerV4Storage {
    /// @notice The portion of TMX that each contributor receives per block
    mapping(address => uint) public thermoContributorSpeeds;

    /// @notice Last block at which a contributor's TMX rewards have been allocated
...
contract USXControllerInterface {
    function getUSXAddress() public view returns (address);
    function getMintableUSX(address minter) public view returns (uint, uint);
    function mintUSX(address minter, uint mintUSXAmount) external returns (uint);
    function repayUSX(address repayer, uint repayUSXAmount) external returns (uint);

    function _initializeThermoUSXState(uint blockNumber) external returns (uint);
    function updateThermoUSXMintIndex() external returns (uint);
    function calcDistributeUSXMinterThermo(address usxMinter) external returns(uint, uint, uint, uint);
}
