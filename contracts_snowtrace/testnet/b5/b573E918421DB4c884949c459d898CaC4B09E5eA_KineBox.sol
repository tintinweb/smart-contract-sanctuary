...
contract KineBox is AccessControl, Pausable, ERC1155Holder {
    ...
    uint256 private _range = 1000000;

    event AddedMaxSupply(uint256 oldMaxSupply,uint256 newMaxSupply);
    event RateUpdated(uint256 level);
    event RewardUpdated(uint256 indexed datetime, uint256 amount);
    event Opened(address indexed account, uint256 level, uint256 tokenId,uint256 time);
    ...
    function today() public view returns (uint256) {
        return (block.timestamp - opening) / 600 + 1;
    }
    ...
    // 更新指定等级盲盒的奖励比例
    function updateRate(uint256 level, uint256[] memory tokens, uint256[] memory values) external onlyRole(MANAGER_ROLE) {
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            ...
            rates[level].push(rate);

            totalRates += values[i];
        }

        require(totalRates == _range, "Invalid values");

        emit RateUpdated(level);
    }
    

    // 更新今日奖励
    function updateTodayReward(address[] memory accounts, uint256[] memory levels) external onlyRole(MANAGER_ROLE) {
    ...
    function open(uint256 seed) external whenNotPaused {
        ...
        require(level > 0,"today reward must be greater than zero");

        uint256 tokenId = 0;

        uint256 randomNumber = _randomNumber(seed, _range);
        
        Rate[] memory data = rates[level];
        uint256 lastValue;
    ...
    function _randomNumber(uint256 seed, uint256 modulus) internal returns (uint256) {
