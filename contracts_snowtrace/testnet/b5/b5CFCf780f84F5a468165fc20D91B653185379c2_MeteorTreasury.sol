...
contract MeteorTreasury is Ownable {
    ...
    address public sMtr;
    uint public sMTRQueue; // Delays change to sMTR address
    ...
    constructor (
        ...
        // isLiquidityToken[ _MTRMIM ] = true;
        // liquidityTokens.push( _MTRMIM );

        secondsNeededForQueue = _secondsNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for MTR
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint MTR needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( Mtr ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn MTR for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IMTRERC20( Mtr ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sMtr ).balanceOf( msg.sender ); // Can only borrow against sMTR held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with MTR
        @param _amount uint
     */
    function repayDebtWithMTR( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IMTRERC20( Mtr ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, Mtr, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( Mtr ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( Mtr ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns MTR valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match MTR decimals
            value_ = _amount.mul( 10 ** IERC20( Mtr ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SMTR ) { // 9
            sMTRQueue = uint32(block.timestamp).add32( secondsNeededForQueue );
        } else return false;
