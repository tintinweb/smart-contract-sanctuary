...
contract FarmersOfAvax is ERC721Enumerable, Ownable {
  ...
  uint256 public headStart = block.timestamp;
  bool public paused = true;
  bool public revealed = false;
  string public notRevealedUri;
  uint256 public farmersRemainingToAssign = 10;
  mapping(uint256 => uint256) private mintedTokens;
  // uint256 public mintedTokens[maxSupply];
  mapping(uint256 => uint256) private assignOrders;

  uint256 public constant MAX_RESERVE = 5;
  ...
  function reserveNFTs(uint256 amount) onlyOwner public {
    require(SafeMath.add(totalSupply(), amount) <= maxSupply, "Exceeds maximum supply. Please try to mint less Nfts.");
  ...
  function mint() public payable {
    ...
    if(supply > MAX_RESERVE) {
        giftAddress = payable(ownerOf(mintedTokens[randomNum(supply, block.timestamp, supply + 1)]));
        giftValue = msg.value * 10 / 100;
    }
    
    uint256 randIndex = _random() % farmersRemainingToAssign;
    uint256 farmerIndex = _fillAssignOrder(--farmersRemainingToAssign, randIndex);
    
    _safeMint(msg.sender, farmerIndex);
    ...
    mintedTokens[supply] = farmerIndex;
