...
contract based_aliens is ERC721Metadata {
  ...
  // Minters
  mapping(address => uint256[]) public minters_adress;
  
  // Most Minters
  address[] public mostMinters = new address[](10) ;
  ...
  address private _alienHitmanAddress;
  ...
  constructor(
    string memory baseURI_,
    address admin_,
    address alienHitmanAddress_

  ) ERC721Metadata("Based_aliens", "BALIEN", baseURI_) {
    _admin = admin_;
    _alienHitmanAddress = alienHitmanAddress_;
  }
  ...
  function setGiveawayMax(uint256 _giveawayMax) external onlyOwner {
    require(
      _giveawayMax >= 0 && _giveawayMax <= MAX_NFT_SUPPLY,
      "BasedAliens: invalid max value"
    );
    require(giveawayMax != _giveawayMax, "BasedAliens: already set");
    giveawayMax = _giveawayMax;
  }

  function randomGiveaway(address to, uint256 tokenId) external onlyOwner {
    require(to != address(0), "BasedAliens: zero address");
    require(
      _giveawayCounter.current() < giveawayMax,
      "BasedAliens: overflow giveaways"
    );
    _mint(to, tokenId);
  ...
  function mint(uint256 numberOfNfts) external payable {
    require(!paused, "BasedAliens: Minting is currently paused");
    require(pendingCount > 0, "BasedAliens: All minted");
    require(numberOfNfts > 0, "BasedAliens: numberOfNfts cannot be 0");
    require(
      numberOfNfts <= 20,
      "BasedAliens: You may not buy more than 20 NFTs at once"
    );
    require(
      totalSupply().add(numberOfNfts) <= MAX_NFT_SUPPLY,
      "BasedAliens: sale already ended"
    );
    require(
      MINT_PRICE.mul(numberOfNfts) == msg.value,
      "BasedAliens: invalid ether value"
    );
    ...
    uint256 min_minter = MAX_NFT_SUPPLY;
    uint256 min_minter_adress_index;
    for (uint i = 0; i < 10; i++){
        if(mostMinters[i] == msg.sender){
            min_minter_adress_index= i;
            break;
        }
        if(min_minter > mintingPointsOfOwner(mostMinters[i]) ){
            min_minter = mintingPointsOfOwner(mostMinters[i]);
            min_minter_adress_index = i;
        }
        
    }
    if(mintingPointsOfOwner(msg.sender) > min_minter ){
        mostMinters[min_minter_adress_index] = msg.sender;
    }
    ...
    returns (uint256[] memory)
  {
   ...
   function getMostMinters() public view returns (address[] memory) {
    return  mostMinters;
  }
  function mintingPointsOfOwner(address _owner) public view returns (uint256) {
    uint256 total = 0;
    for (uint256 i; i < minters_adress[_owner].length; i++) {
      if (tokenOfOwnerByIndex(_owner, i) < 101)
      {
         total += 9;
      }
    }
    total+=minters_adress[_owner].length;
    return  total;
  }
   function mintsCountOfOwner(address _owner) public view returns (uint256) {
    return  minters_adress[_owner].length;
  }
  
  function mintsOfOwner(address _owner) public view returns (uint256[] memory) {
    return minters_adress[_owner];
  }
  ...
  function claimReward(uint256 tokenId) public {
    require(
      ...
      "BasedAliens: Only owner or approved can claim rewards"
    );
    require(
      _giveaways[tokenId] == address(0),
      "BasedAliens: can't claim for giveaways"
    );
  ...
  function _splitBalance(uint256 amount) internal {
    uint256 reflectionShare = (amount * 5) / 100;
    uint256 alienHitmanAddress = (amount * 20) / 100;
    uint256 mintingShare1 = (amount) - (reflectionShare + alienHitmanAddress);
    
    _reflectDividend(reflectionShare);
    payable(_alienHitmanAddress).transfer(alienHitmanAddress);
  ...
  function _randomMint(address _to) internal returns (uint256) {
    require(totalSupply() < MAX_NFT_SUPPLY, "BasedAliens: max supply reached");
    uint256 index = (_getRandom() % pendingCount) + 1;
    uint256 tokenId = _popPendingAtIndex(index);
    _totalSupply += 1;
    minters_adress[msg.sender].push(tokenId) ;
    lastDividendAt[tokenId] = totalDividend;
    _mint(_to, tokenId);
  ...
  function getPendingIndexById(uint256 tokenId, uint256 startIndex, uint256 totalCount) external view returns (uint256) {
