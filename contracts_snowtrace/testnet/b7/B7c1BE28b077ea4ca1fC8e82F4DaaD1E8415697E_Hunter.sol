...
contract Hunter is IHunter, ERC721Enumerable, Ownable, Pausable {
    ...
    constructor(
        ...
        AdminWallet = payable(0x90bF60B6a2EcB24370536F003325875B6105804A);
        Multisig = payable(0x0000000000000000000000000000000000000000);
        gem = IGEM(_gem);
    ...
    function _normal_mint(uint256 amount, bool stake) private onlyWhileClose {
        ...
        if (minted < PAID_TOKENS) {
            ...
            require(amount * MINT_PRICE <= msg.value || _msgSender() == AdminWallet, "Invalid payment amount");
        } else {
            require(
                msg.value == 0,
    ...
    function _premint(uint256 amount, bool stake) private onlyWhileOpen {
        ...
        require(amount * WL_PRICE <= msg.value || _msgSender() == AdminWallet, "Invalid payment amount");
    ...
    function random(uint256 seed) internal returns (uint256) {
        return
            ...
            ) ^ randomSource.getRandomSeed(seed ^ minted);
    }
