...
////// /nix/store/8xb41r4qd0cjb63wcrxf1qmfg88p0961-dss-6fd7de0/src/gOHM.sol
...
contract MockgOHM is LibNote {
  ...
  event Transfer(address indexed src, address indexed dst, uint wad);
    ...
    modifier auth {
        require(wards[msg.sender] == 1, "gOHM/not-authorized");
        _;
    }

    // --- ERC20 Data ---
    string  public constant name     = "Mock Governance OHM";
    ...
    function transferFrom(address src, address dst, uint wad) public returns (bool) {
      ...
      require(balanceOf[src] >= wad, "gOHM/insufficient-balance");
        if (src != msg.sender && _allowance( src, msg.sender ) != uint(-1)) {
            require(_allowance( src, msg.sender ) >= wad, "gOHM/insufficient-allowance");
    ...
    function burn(address usr, uint wad) external {
        require(balanceOf[usr] >= wad, "gOHM/insufficient-balance");
        if (usr != msg.sender && _allowance( usr, msg.sender ) != uint(-1)) {
            require(_allowance( usr, msg.sender ) >= wad, "gOHM/insufficient-allowance");
                    ...
                    bool allowed, uint8 v, bytes32 r, bytes32 s) external
    {
        ...
        require(holder != address(0), "gOHM/invalid-address-0");
        require(holder == ecrecover(digest, v, r, s), "gOHM/invalid-permit");
        require(expiry == 0 || block.timestamp <= expiry, "gOHM/permit-expired");
        require(nonce == nonces[holder]++, "gOHM/invalid-nonce");
