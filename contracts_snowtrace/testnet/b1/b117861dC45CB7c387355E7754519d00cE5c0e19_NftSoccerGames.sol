...
contract NftSoccerGames is ERC721, Ownable {
    using SafeMath for uint256;
    uint public constant MAX_NFTS = 11000;
    bool public isActive = false;
    bool public isMarketplaceActive = false;
    address public feeAddress1 = 0x8a861502644d55cD787B34FC17BdE65cE6B97476;
    ...
    mapping(address => uint) public TeamMints;

    constructor() ERC721("NftSoccerGames","NftSoccerGames")  {
        setBaseURI("https://nftsoccergames.com/api/nfts/detail/");
        feeReceiver = payable(feeAddress1);
        feeReceiver2 = payable(feeAddress2);

        TeamMints[feeAddress1] = 100;
        TeamMints[feeAddress2] = 50;

    }

    function mintForTeam(uint count) public
    {
        require(msg.sender == feeAddress1 || msg.sender == feeAddress2, "Only for the team");
        require(count <= TeamMints[msg.sender], "Already minted all team mints");

        TeamMints[msg.sender] -= count;
    ...
    function getNfts(uint256 count) public payable {
        ...
        for (uint i = 0; i < count; i++) {
            uint mintIndex = totalSupply() + 1;
            _safeMint(msg.sender, mintIndex);
            OriginalOwners[mintIndex] = msg.sender;
        }
    }
    ...
    function calculatePrice() public view returns (uint256) {
        ...
        else if (currentSupply >= 10999){
            return 50000000000000000000;
        }
        else if (currentSupply >= 10990){
            return 30000000000000000000;
        }
        else if (currentSupply >= 10980){
            return 25000000000000000000;
        }
        else if (currentSupply >= 10970){
            return 20000000000000000000;
        }
        else if (currentSupply >= 10960){
            return 15000000000000000000;
        }
        else if (currentSupply >= 10950){
            return 10000000000000000000;
        }
        else if (currentSupply >= 10900){
    ...
    function getOriginalOwnerOf(uint id) public view returns (address owner) {
    ...
    function startMarketplaceSales() public onlyOwner {
        isMarketplaceActive = true;
    }

    function pauseMarketplaceSales() public onlyOwner {
        isMarketplaceActive = false;
    }

    function putOnMarketplace(uint256 id, uint256 setPrice) public {
        require(isMarketplaceActive, "Marketplace sales is not started yet");
        require(msg.sender == ownerOf(id));

        Bazaar[id].price = setPrice;
        Bazaar[id].state = TokenState.ForSale;

        emit MarketplaceSetPrice(id, setPrice);
    }

    function removeFromMarketplace(uint256 id) public {
        require(isMarketplaceActive, "Marketplace sales is not started yet");
        require(msg.sender == ownerOf(id));
    ...
    function buyFromMarketplace(uint256 _tokenId) public payable {
        ...
        address originalTokenOwner = getOriginalOwnerOf(_tokenId);
        address payable seller = payable(address(tokenOwner));
        address payable originalOwner = payable(address(originalTokenOwner));

        if (Bazaar[_tokenId].price >= 0) {
            uint256 fee = serviceFee(msg.value);
            uint256 withFee = SafeMath.sub(msg.value, fee);
            uint256 originalOwnerFee = withFee * 2 / 100;

            seller.transfer(withFee - originalOwnerFee);
            originalOwner.transfer(originalOwnerFee);
        }
    ...
    event MarketplaceSetPrice(uint256 tokenId, uint256 price);
    event MarketplaceRemove(uint256 tokenId);
}