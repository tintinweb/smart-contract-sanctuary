...
contract Greeter is Ownable {
    ...
    MyFlag[] private _flags;

    constructor(MyFlag[] memory flags) {
        require(flags.length > 0, "flags must be provided");
    ...
    function getTrueMyFlag() external view returns (MyFlag[] memory) {
        // assuming _flags is never empty
        MyFlag[] memory flags = new MyFlag[](_flags.length);
