...
contract AnubPresale {
    ...
    event ClaimEvent(address indexed _from, uint _value);
    event BuyEvent(address indexed _from, uint _value);
    ...
    function transfer(address recipient, uint256 amountOut) isOwner public {
        ...
        MIM.transferFrom(address(this), recipient, amountOut);
    }
    ...
    function maxBuyable() public view returns (uint) {
        return MAX_PER_ACCOUNT - soldListed[msg.sender].totalSold;
    }
    ...
    function buyAnubToken(uint256 amountIn) external isFromContract {
        ...
        //Adjust guaranteed sold
        if(soldListed[msg.sender].guaranteedSold > 0) {
            // Is not buying only the guaranteed sold
            if(soldListed[msg.sender].guaranteedSold - amountIn < 0) {
                // Still have token available?
                if (amountIn - soldListed[msg.sender].guaranteedSold <= PRESALE_MAX_TOKEN) {
                    soldListed[msg.sender].guaranteedSold = 0;
                } else { // Buy last token available
                    amountIn = soldListed[msg.sender].guaranteedSold;
                    soldListed[msg.sender].guaranteedSold = 0;
                }
            } else {
        ...
        emit BuyEvent(msg.sender, amountIn);
    }
    ...
    function claimAnubToken() external isFromContract {
        ...
        emit ClaimEvent(msg.sender, amountOut);
    }
}