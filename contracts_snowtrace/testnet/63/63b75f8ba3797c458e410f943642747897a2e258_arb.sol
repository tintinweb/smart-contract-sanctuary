...
contract arb {
    ...
    function trade(address[] memory path , address[] memory pairs ,  uint256 amount, uint256 amount_out ) public {
        ...
        for (uint i = 0; i < pairs.length ; i++) {
            ...
            if(path[i] == WAVAX &&  i == 0){
                ...
                assert(IWAVAX(WAVAX).transfer(pairs[i], amount));
            }
            ...
            (uint amount0Out, uint amount1Out) = path[i] == token0 ? (uint(0), amount) : (amount, uint(0));
            ...
            if(path[i+1] == WAVAX ){
                IWAVAX(WAVAX).withdraw(IAVAX20(WAVAX).balanceOf(address(this)));
            }
        }
    ...
    function getAmountOut(address[] memory path , address[] memory pairs ,  uint256 amount) internal view returns (uint amountOut) {
        amountOut = amount;
