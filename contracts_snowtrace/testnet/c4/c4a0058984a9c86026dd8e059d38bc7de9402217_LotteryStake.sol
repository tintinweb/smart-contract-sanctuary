...
contract LotteryStake is Ownable {
    ...
    function getLotteryPoints(uint16 year, uint8 month) public view returns (Points[] memory) {
        ...
        Points[] memory results = new Points[](stakedTokens);
        
        for (uint x = 0; x < stakedTokens; x++) {
            address dep = deposits[x];
            
            uint addressIndex = nftSupply;
            for (uint y = 0; y < results.length; y++) {
                if (results[y].owner == dep) {
                    addressIndex = y;
                    break;
                }
            }
            ...
            if (addressIndex < nftSupply) {
                results[addressIndex].points += lt.logs[dep];
            } else {
                results[results.length].owner = dep;
                results[results.length].points = lt.logs[dep];
            }
        }
    ...
    function randomDraw(uint8 month, uint16 year) external onlyOwner returns (address) {
        ...
        for (uint x = 0; x < nftSupply; x++) {
            if (deposits[x] != address(0)) {
                uint points = lt.logs[deposits[x]];
                for (uint y = 0; y < points; y++) {
                    tickets.push(deposits[x]);
                }
            }
        }
