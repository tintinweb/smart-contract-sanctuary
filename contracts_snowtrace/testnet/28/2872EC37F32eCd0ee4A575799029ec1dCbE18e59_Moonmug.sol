/**
 *Submitted for verification at testnet.snowtrace.io on 2021-11-26
*/
...
// File contracts/lib/IERC2981Royalties.sol
...
// File contracts/lib/ERC2981Base.sol
...
abstract contract ERC2981PerTokenRoyalties is ERC2981Base {
        ...
        returns (address receiver, uint256 royaltyAmount)
    {
        ...
        uint256 royaltyValue = (value * royalties.amount) / 10000;
        return (receiver, royaltyValue);
    }
}
...
{
    using Strings for uint256;
    uint256 public constant ROYALTY_VALUE = 2000; // 20% using 2 decimals - 10000 = 100, 0 = 0
    ...
    function getFirstRewardComponent(uint256 index)
        ...
        returns (uint256 r2)
    {
        uint256 reward = (getPrice() * ROYALTY_VALUE);
        return reward;
    }
        ...
        returns (uint256 reward)
    {
        ...
        return getFirstRewardComponent(index);
    }
    ...
    function getPrice() internal pure returns (uint256 price) {
        return INITIAL_CLAIM_PRICE;
    }
    ...
    function claim(uint256 n) public payable nonReentrant {
        require(
            canClaim,
            "Moonmug: it's not possible to claim just yet."
        );
        require(
            MAX_MINTABLE - airdropNumber - minted > 0,
            "Moonmug: not enough Moonmug left to mint."
        );
        require(n > 0, "Moonmug: not enough Moonmug left to mint.");
        ...
        for (uint256 i = 0; i < n; i++) {
            ...
            _safeMint(_msgSender(), nextIndex);
    ...
    function redeem(
        ...
        require(
            canClaimAirdrop,
            "Moonmug: it's not possible to claim the airdrop at the moment."
        );
