...
contract Distributor is Initializable, OwnableUpgradeable{
    ...
    function initialize(address cesta_, address treasury_, uint epochLength_, uint nextEpochTimestamp_, address admin_) external initializer {
        __Ownable_init();

        Treasury = ITreasury(treasury_);
        CESTA = IERC20Upgradeable(cesta_);
    ...
    function distributePenaltyRewards() internal {
        ...
        if(balance > 0) {
            ...
            for(uint i; i< totalReceivers; i++){
                CESTA.safeTransfer(penaltyRewardReceivers[i], amtToDistribute);
            }
    ...
    function nextRewardAt(uint rate_) public view returns (uint) {
        return CESTA.totalSupply() * rate_ / 1000000;
    }
    ...
    /**
        ...
        @param stakingToken_ stakingToken of stakingContract (sCESTA etc).
