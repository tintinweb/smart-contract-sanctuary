...
contract Treasury is Ownable, ReentrancyGuard {
    ...
    event Remainder(address indexed receiver, uint256 amount);
    ...
    // Function will not transfers you the exact token equivalent of avax sent. It just checks the amount of MUZIK you want and compare with the DAI based avg price you have to pay. 
    // If you paid enough or higher, you will receive tokens, else reverts.

    function buyWithAVAX(uint256 _amount) public payable nonReentrant{
        ...
        emit Console("cost-netamount",cost,netAmount,0,0,0); 
        
        require(cost <= netAmount, "Not enough AVAX paid.");

        uint256 extraCharge = netAmount.sub(cost); // amount of extra paid DAIs

        if(extraCharge > 0) {
            address[] memory changePath = pathMaker(stableCoin,IERC20(WAVAX));
            router.swapExactTokensForAVAX(extraCharge, 0, changePath, msg.sender, block.timestamp);

            emit Remainder(msg.sender,extraCharge); // extraCharge is a DAI
        }
