...
contract TeamDistributor is Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        members.set(treasuryAccount, DistributeIterableMapping.DistributeMember(treasuryAccount, 0, 0, 0));
    ...
    function getReleasableAmount(address member) public view returns (uint256) {
        ...
        if (member == treasuryAccount) {
            personalPending =
                ...
                (pendingAmount * (totalAllocation - allocationSum)) /
                totalAllocation;
        } else {
            personalPending =
                members.get(member).pending +
    ...
    function _release(address _member) private nonReentrant {
        ...
        if (releasableAmount > 0) {
            member.totalReleased = member.totalReleased + releasableAmount;
            totalReleased = totalReleased + releasableAmount;
    ...
    function releaseToMemberAll() external onlyOwner isCliffStarted {
        updatePendingAmounts();
