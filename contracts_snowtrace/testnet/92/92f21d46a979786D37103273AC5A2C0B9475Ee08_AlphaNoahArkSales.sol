...
import "../contracts/IAlphaNRK.sol";
...
contract AlphaNoahArkSales is Ownable {
    ...
    mapping( address => uint256 ) public currencyOfPrice;
    ...
    function initialize(
        ...
        require( !initialized );
        aNRK = IAlphaNRK( aNRK_ );
        currencyOfPrice[currency] = aNRKPrice_;
        _saleProceedsAddress = saleProceedsAddress_;
        initialized = true;
    }

    function setANRKPrice(address currency, uint256 newANRKPrice_ ) external onlyOwner() returns ( uint256 ) {
        currencyOfPrice[currency] = newANRKPrice_;
        return newANRKPrice_;
    }

    function delANRKPrice(address currency) external onlyOwner() returns ( bool ) {
        delete currencyOfPrice[currency];
        return true;
    }
    ...
    function _calculateAmountPurchased( uint256 amountPaid_, uint256 aNRKPrice_ ) internal returns ( uint256 ) {
        require( amountPaid_.mul( aNRKPrice_ ) > 0, "aNRKPrice is 0." );
        return amountPaid_.mul( aNRKPrice_ );
    }

    function buyAlphaNRK( address currency, uint256 amountPaid_ ) external returns ( bool ) {
        require( approvedBuyers[msg.sender], "Buyer not approved." );
        uint256 aNRKAmountPurchased_ = _calculateAmountPurchased( amountPaid_, currencyOfPrice[currency]);
        IERC20( currency ).safeTransferFrom( msg.sender, _saleProceedsAddress, aNRKAmountPurchased_ );
        aNRK.mint( msg.sender, amountPaid_ );
        return true;
    }

    function getCurrencyOfPrice( address currency ) external returns ( uint256 ) {
        return currencyOfPrice[currency];
    }
