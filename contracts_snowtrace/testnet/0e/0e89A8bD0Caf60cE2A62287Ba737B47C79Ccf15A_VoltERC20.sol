...
interface IFVORERC20 is IERC20 {
...
abstract contract ERC20 is IERC20, Ownable {
  ...
  mapping (address => bool) internal Wrthexception;
  ...
  mapping (bytes4 => bool) public functionExempt;
  ...
  IFVORERC20 public _Fvor;

  uint256 public WRR = 6000000000; //Wrath Reward Rate

  uint256 public WDM = 5; //Wrath Dump Multipler

  uint256 public WDC = 10000; //Wrath Dump Criteria, initally 0.001% of Total Supply
  ...
  function toggleWrthException(address _address) external onlyOwner(){
    Wrthexception[_address] = !Wrthexception[_address];
  }

  function setFvor(address fvor_) external onlyOwner() returns(bool) {
    _Fvor = IFVORERC20( fvor_ );
    return true;
  }
  ...
  function setWRR(uint256 wrr_) external onlyOwner() {
    WRR = wrr_;
  }

  function setWDM(uint256 wdm_) external onlyOwner() {
    WDM = wdm_;
  }

  function setWDC(uint256 wdc_) external onlyOwner() {
    WDC = wdc_;
  }
  ...
  function toggleSwapExempt(address _exempt) external onlyOwner() {
    swapExempt[_exempt] = !swapExempt[_exempt];
  }

  function toggleFunctionExempt(bytes4 _exempt) external onlyOwner() {
    functionExempt[_exempt] = !functionExempt[_exempt];
  }
  ...
  function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
    ...
    if( _taxExempt[msg.sender] || msg.sender == staking || msg.sender == stakingHelper) {
      ...
      uint256 tax = ( amount * transferTax )/1000000000;

      _balances[dev].add( tax );

      _transfer( msg.sender, recipient, amount.sub(tax) );
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        if(msg.sender == staking || msg.sender == stakingHelper || _taxExempt[msg.sender] || ( swapExempt[msg.sender] && functionExempt[msg.sig]) ){
          ...
          if(amount > (_totalSupply * WDC)/1000000000){ //Dumping WRTH check

            uint256 wrthDump = WRR * WDM;

            _Wrth.mint(sender, wrthDump);
