...
contract Distribution is Ownable, ReentrancyGuard {
    ...
    mapping(address => DistributionAddress) private distributions;
     ...
     */
    function setAddress(SetDistribution[] memory distributionAddresses)
        public
        onlyOwner
    {
        ...
        for (
            ...
            if (distributions[distribution].isActive) {
                updateAddress(distributionAddresses[addressIndex]);
            } else {
                createAddress(distributionAddresses[addressIndex]);
            }
        }
    }
     ...
     */
    function createAddress(SetDistribution memory newAddress) private {
        address distributionAddress = newAddress.beneficiaryAddress;

        distributions[distributionAddress].isActive = true;
        delete distributions[distributionAddress].periods;
        ...
        for (uint256 i = 0; i < periodsLength; i += 1) {
            SetDistributionPeriod memory period = newAddress.periods[i];
            ...
            distributions[distributionAddress].periods.push(
     ...
     */
    function updateAddress(SetDistribution memory editedAddress) private {
        address distributionAddress = editedAddress.beneficiaryAddress;

        uint256 periodsLength = distributions[distributionAddress]
            .periods
            .length;
        ...
        for (uint256 i = 0; i < periodsLength; i += 1) {
            SetDistributionPeriod memory newPeriod = editedAddress.periods[i];
            DistributionPeriod memory oldPeriod = distributions[
                distributionAddress
            ].periods[i];
            ...
            distributions[distributionAddress].periods[i] = DistributionPeriod(
                newPeriod.timestamp,
                newPeriod.amount,
                false
            );
        ...
        for (uint256 i = periodsLength - 1; i >= newPeriodsLength; i -= 1) {
            ...
            distributions[distributionAddress].periods.pop();
        }
    }
     ...
     */ 
    function removeAddresses(address[] memory distributionAddresses)
        public
        onlyOwner
    {
        ...
        for (uint256 i = 0; i < length; i += 1) {
            require(
                distributions[distributionAddresses[i]].isActive,
                "Address is not exist"
            );

            distributions[distributionAddresses[i]].isActive = false;
        }
    }
     ...
     */ 
    function claimTokensForAddress(address to) public nonReentrant {
        require(distributions[msg.sender].isActive, "No claiming periods");

        DistributionAddress storage distribution = distributions[msg.sender];
    ...
    function getAmountToBeClaimed(address distributionAddress)
        ...
        returns (uint256)
    {
        if (!distributions[distributionAddress].isActive) {
            return 0;
        }

        DistributionAddress storage distribution = distributions[
            distributionAddress
        ];
     ...
     */ 
    function getAddress(address distributionAddress)
        public
        view
        returns (DistributionAddress memory)
    {
        DistributionAddress memory distribution = distributions[
            distributionAddress
        ];
        return distribution;
    }
