            ...
            uint256(
                keccak256(
                    abi.encodePacked(
                        ...
                        hex"7904edaf966de985b302ab98d987269561c27f4db6ec0743f1e9a98519d18cc2" // init code fuji
                    )
                )
            )
        );
...
contract Router is IRouter {
    ...
    function _swap(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            IPair(AlligatorLibrary.pairFor(factory, input, output)).swap(amount0Out, amount1Out, to, new bytes(0));
        }
    }
    ...
    function swapExactAVAXForTokens(
        ...
        require(path[0] == WAVAX, "Router: INVALID_PATH");
    ...
    function swapTokensForExactAVAX(
        ...
        require(path[path.length - 1] == WAVAX, "Router: INVALID_PATH");
        amounts = AlligatorLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= amountInMax, "Router: EXCESSIVE_INPUT_AMOUNT");
        TransferHelper.safeTransferFrom(
            path[0],
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint256 i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0, ) = AlligatorLibrary.sortTokens(input, output);
            IPair pair = IPair(AlligatorLibrary.pairFor(factory, input, output));
            uint256 amountInput;
