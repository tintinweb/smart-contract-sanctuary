...
contract MockAvax is ERC20Permit, VaultOwned {

    using SafeMath for uint256;
    address _admin;

    constructor() ERC20("Wrapped AVAX-mock", "wAVAX", 18) {
    }

    modifier onlyAdmin() {
      require(msg.sender == _vault 
        || msg.sender == _owner 
        || msg.sender == _admin, "OnlyAdmin: the caller is not the admin");
      _;
    }
    ...
    function mint(address account_, uint256 amount_) external onlyAdmin() {
