...
contract CustomMasterChef is WhitelistGuard
{
	...
	struct PoolInfo {
		...
		uint256 lastRewardSec;    // Last second that CAKEs distribution occurs.
	...
	// The second when CAKE mining starts.
	uint256 public immutable startSec;

	event Deposit(address indexed user, uint256 indexed pid, uint256 amount);
	event Withdraw(address indexed user, uint256 indexed pid, uint256 amount);
	event EmergencyWithdraw(address indexed user, uint256 indexed pid, uint256 amount);

	constructor (address _cake, address _syrup, uint256 _cakePerSec, uint256 _startSec)
		public
	{
		cake = MintableToken(_cake);
		syrup = MintableStakeToken(_syrup);
		cakePerSec = _cakePerSec;
		startSec = _startSec;
	...
	function updateCakePerSec(uint256 _cakePerSec) external onlyOwner
	{
		cakePerSec = _cakePerSec;
	}
	...
	function add(uint256 _allocPoint, IERC20 _lpToken, bool _withUpdate) external onlyOwner
	{
		...
		uint256 lastRewardSec = block.timestamp > startSec ? block.timestamp : startSec;
	...
	function pendingCake(uint256 _pid, address _user) external view returns (uint256)
	{
		...
		if (block.timestamp > pool.lastRewardSec && lpSupply != 0) {
			uint256 multiplier = getMultiplier(pool.lastRewardSec, block.timestamp);
			uint256 cakeReward = multiplier.mul(cakePerSec).mul(pool.allocPoint).div(totalAllocPoint);
	...
	{
		PoolInfo storage pool = poolInfo[_pid];
		if (block.timestamp <= pool.lastRewardSec) {
			return;
		}
		uint256 lpSupply = pool.lpToken.balanceOf(address(this));
		if (lpSupply == 0) {
...
contract MasterChefAdmin is Ownable, ReentrancyGuard
{
	...
	function updateCakePerSec(uint256 _cakePerSec) external onlyOwner nonReentrant
	{
		CustomMasterChef(masterChef).updateCakePerSec(_cakePerSec);
	}
