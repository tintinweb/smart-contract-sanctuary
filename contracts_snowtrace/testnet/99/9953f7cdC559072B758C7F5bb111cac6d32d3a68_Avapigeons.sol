...
contract Avapigeons is
    ...
    uint256 public _price = 1.1 ether;
    uint256 public _max = 2222;
    uint256 public maxMintForTx = 40;
    uint256 private noFreeAddresses = 0;
    uint256 private contractRoyalties = 600;
    ...
    constructor(
        ...
        setNotRevealedURI(_initNotRevealedUri);
         ...
         _owner2 = owner2;
         _owner3 = owner3;
         royaltyContractAddress = royaltyContract;
    ...
    function setRoyaltyContractAddress(address _royaltyContractAddress) public onlyOwner(){
      royaltyContractAddress = _royaltyContractAddress;
        ...
        require(_tokenIdTracker.current() + amount <= _max, "Avapigeons: not enough avax pigeons left to mint amount");
        require(amount <= maxMintForTx, "max mint amount per session excceded");
        ...
        _setTokenRoyalty(_tokenIdTracker.current(), royaltyContractAddress, contractRoyalties);
    ...
    function claimReward(uint256 tokenId) public {
        require(ownerOf(tokenId) == _msgSender() || getApproved(tokenId) == _msgSender(), "Avapigeons: Only owner or approved can claim rewards");
    ...
    function splitBalance(uint256 amount) private {
        ...
        uint256 mintingShare1  = (amount - reflectionShare)/3;
        uint256 mintingShare2  = (amount - reflectionShare)/3;
        uint256 mintingShare3  = (amount - reflectionShare)/3;

        reflectDividend(reflectionShare);
        payable(_owner1).transfer(mintingShare1);
        payable(_owner2).transfer(mintingShare2);
        payable(_owner3).transfer(mintingShare3);
    }
