...
contract Sierra is ERC20Permit, VaultOwned {
    ...
    uint public daoRevenuePoints = 0;

    address public daoAddress = address(0);

    event DaoAddressTransferred(address indexed previousDaoAddress, address indexed newDaoAddress);

    constructor() ERC20("Sierra", "SRA", 9) {
    }

    function setTaxParams(uint newBasisPoints, uint newMaxFee, uint newDaoRevenuePoints) public onlyOwner {
        // HardCoded Fee Limit ( 1000 = 10%)
        require(newBasisPoints < 1000);

        // HardCoded Dao Revenue Fee ( 10000 = 100%)
        require(newDaoRevenuePoints < 10000);

        basisPointsRate = newBasisPoints;
        maximumFee = newMaxFee.mul(10**_decimals);

        daoRevenuePoints = newDaoRevenuePoints;

    }

    function transferDaoAddress( address newDaoAddress ) public virtual onlyOwner() {
        emit DaoAddressTransferred( daoAddress, newDaoAddress );
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        uint daoRevenue = (fee.mul(daoRevenuePoints)).div(10000);
