...
contract Summit is Ownable {
    ...
    modifier onlyWhitelisted(){
        //TEST require(whitelisted[msg.sender]==true);
        _;
    }
    ...
    constructor(){
        ...
        price = 5;
        MIM = 0x3EcF3E826a5AfEA443807571B51043597655C1CC;//test mim 
    }
    ...
    function claimSnow() public {
        ...
        require(!blacklisted[msg.sender], "blacklisted");
        IERC20(snow).transfer(msg.sender, invested[msg.sender]);
    ...
    function setClaimingActive(address a) public onlyOwner {
        snow = a;
        claimingActive = true;
    }
    ...
    function amountBuyable(address a) public view returns (uint) {
        uint256 max;
        if (whitelisted[a] && !presalePublic) {
            max = maxWLsale;
        }
        if (presalePublic) {
            max = maxPublicSale;
    ...
    //test
    function setSold(uint amount) external {
        sold = amount;
    }
}