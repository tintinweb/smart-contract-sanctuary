...
contract Bank is Ownable, IERC721Receiver, Pausable {
        ...
        _updateEarnings
    {
        ...
        uint256[] memory tokenIdsBonus = game.walletOfOwner(_msgSender());
        for (uint256 i = 0; i < tokenIdsBonus.length; i++) {
            if (BonusCall.getBonusType(tokenIdsBonus[i]) == 1) {
                ...
                owed = owed * 10 / 100;
            } else if (tokenIdsBonus.length == i) {
                require(owed >= 20000 ether, "A minimum of 20k GOLD is required to claim");
            }
        }
        //fee for sheriff
