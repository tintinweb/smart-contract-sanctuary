...
contract SkadiStaking is Ownable {
    ...
    function stake( uint _amount, address _recipient ) external returns ( bool ) {
        ...
        warmupInfo[ _recipient ] = Claim ({
            ...
            gons: info.gons.add( ISki( sSki ).gonsForBalance( _amount ) ),
    ...
    function claim ( address _recipient ) public {
        ...
        if ( epoch.number >= info.expiry && info.expiry != 0 ) {
            ...
            IWarmup( warmupContract ).retrieve( _recipient, ISki( sSki ).balanceForGons( info.gons ) );
        }
    }
    ...
    function forfeit() external {
        ...
        IWarmup( warmupContract ).retrieve( address(this), ISki( sSki ).balanceForGons( info.gons ) );
    ...
    function rebase() public {
        if( epoch.endTime <= uint32(block.timestamp) ) {
            ...
            ISki( sSki ).rebase( epoch.distribute, epoch.number );
            ...
            uint staked = ISki( sSki ).circulatingSupply();
