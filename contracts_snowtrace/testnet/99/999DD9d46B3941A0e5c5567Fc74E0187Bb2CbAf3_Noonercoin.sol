...
contract Noonercoin is ERC20{
   ...
   function mintToken(address add, uint256 currentTime) public returns (bool) {  //admin only
      ...
      if(isNewCycleStart) {
       ...
       if(stopTheMinting) {
           ...
           uint256 noonerCoinExtractedFromWei = 0;
            ...
            uint256 nowTime = now;
            uint256 totalOccurences = getTotalPresentOcuurencers();
    ...
    function burnToken() internal returns(bool){
        ...
        signmaValueWei = forAddingAnToken(signmaValueWei);
        ...
        currentMintingRateTotalTokens = forAddingAnToken(currentMintingRateTotalTokens);
        ...
        if(totalWeiInAdminAcc > weiToBurned) {
            weiToBurned = forAddingAnToken(weiToBurned);
            remainingWei = totalWeiInAdminAcc - weiToBurned;
            noonercoin[adminAddress] =  _fundersAmount  + (remainingWei/10**18);
            uint256 weiAddedInAdminAccount  = remainingWei -  (noonercoin[adminAddress] - _fundersAmount) * 10**18;
  ...
  function getLastMintingTimeAndStartTimeDifference() public view returns (uint256) {
    ...
    function getTotalPresentOcuurencers() public view returns(uint256){
    ...
    function forAddingAnToken(uint256 value) internal pure returns(uint256) {
