...
contract AlphaENIGMMigration is Ownable {
  ...
  function available(address _user, uint256 _fakeTimestamp, uint256 _fakeBalance, uint256 _fakeLastMigration) public view returns (uint256) {
    ...
    for (uint256 i = steps.length; i > 0; i--) {
      ...
      if (_fakeLastMigration < step && timestamp >= step) {
        claimable += percentPerStep[i - 1];
      } else if (_fakeLastMigration > step) {
