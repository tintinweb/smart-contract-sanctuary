...
contract MIM is LibNote {
  ...
  event Transfer(address indexed src, address indexed dst, uint wad);
    ...
    constructor(uint256 chainId_) {
        ...
        MIMlyMIMLimit = 10000000000000000000000;
    }
    ...
    function adjustMIMlyMIMLimit(uint _limit) external auth {
        MIMlyMIMLimit = _limit;
    }

    function mint(address usr, uint wad) external {

      if(wards[msg.sender] == 0) {
        require(add(wad, MIMMintedToday[msg.sender]) <= MIMlyMIMLimit || sub(block.number, lastMintRestart[msg.sender]) >= 6500 && wad <= MIMlyMIMLimit, "Over MIMly MIM Limit");
