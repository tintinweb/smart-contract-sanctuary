...
contract DiamondbankERC20Token is ERC20Permit, VaultOwned {
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        uint256 burnAmount = taxAmount.mul(burnRate).div(taxFee);

        super._transfer(from, to, amount.sub(taxAmount));
        super._transfer(from, DeadAddress, burnAmount);
        super._transfer(from, treasury, taxAmount.sub(burnAmount));
    ...
    function updateTaxFee(uint256 _newTax) public onlyOwner {
        require(_newTax >= burnRate, "Invalid Tax Fee");
        taxFee = _newTax;
    }

    function updateBurnRate(uint256 _newRate) public view onlyOwner {
        require(_newRate <= taxFee, "Invalid Burn Rate");
    }
}