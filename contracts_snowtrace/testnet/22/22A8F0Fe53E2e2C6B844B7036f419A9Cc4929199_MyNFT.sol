...
using Strings for uint256;
    ...
    string private _nftName = "MyNFT";
    string private _nftSymbol = "MMM";
    uint256 private mintCost = 1*10**17;
    uint256 private mintCostForWhitelistInPresale = 5*10**17;

    string private __baseURI = "baseurl";
    string private _uriExtention = ".json";

    uint256 public maxSupply = 10000;

    uint256 private publicSaleStartDate = 1638010871;
    uint256 private preSaleStartDate = publicSaleStartDate - 6*60*60; // 6 hours

    address[] private whitelistedAddresses;
    mapping(address => uint256) private addressMintedBalance;
    uint256 private maxMintPerUser = 20;
    ...
    function mint(uint256 _mintAmount) public payable {
        ...
        require(_mintAmount <= maxMintPerUser, "max mint amount per session exceeded");
        ...
        if (block.timestamp < publicSaleStartDate ) {
            if(isWhitelisted(msg.sender)) {
                ...
                require(userMintedCount + _mintAmount <= maxMintPerUser, "max NFT per address exceeded");
            } else {
