...
contract ERC721LazyMintWith712 is ERC721, EIP712, AccessControl {
  ...
  constructor(string memory name, string memory symbol) ERC721(name, symbol) EIP712(name, '1.0.0') {
  ...
  function _hash(address account, uint256 tokenId) internal view returns (bytes32) {
    return
      _hashTypedDataV4(
        keccak256(abi.encode(keccak256('NFT(uint256 tokenId,address account)'), tokenId, account))
      );
  }
