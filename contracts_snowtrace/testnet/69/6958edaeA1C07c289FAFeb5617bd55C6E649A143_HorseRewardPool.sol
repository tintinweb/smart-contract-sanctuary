...
contract HorseRewardPool {
	...
	function buyHorse(address owner) external{
		IVeeERC20(rewardToken).transferFrom(msg.sender, address(this), hoursePrice);
		IVeeHorse(veeHorse).mint(owner);
		uint cost = hoursePrice * feeRatio / 1e18;
		uint fee = hoursePrice - cost;
		totalReward += cost;
		serviceFee += fee;
	}

	function settlementAccount(address[] memory accounts,uint[] memory amounts) external{
		require(msg.sender == admin,"permission deny");
