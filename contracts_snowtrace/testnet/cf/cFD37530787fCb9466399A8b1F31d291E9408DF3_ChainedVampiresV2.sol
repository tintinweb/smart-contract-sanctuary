...
interface IChainedVampires {
    function walletOfOwner(address _owner) external view returns (uint256[] memory);
}

contract ChainedVampiresV2 is ERC721, ERC721Enumerable, Ownable {
    ...
    uint256 private RESERVED = 821; // Reserved = 10 , Early Adopters = 811
    ...
    uint256 public totalMarketItemCount = 0;
    ...
    function getHolderAssets(address _prevcontract, address _holders) public view returns(uint256[] memory){
        uint256[] memory tokenIds = IChainedVampires(_prevcontract).walletOfOwner(_holders);
        return tokenIds;
    }
    ...
    uint256[] memory tokenIds = new uint256[](totalMarketItemCount);
    uint256 currentIdx = 0;
    for (uint256 i=0; i < _tokenIdCounter.current(); i++) {
        if(MarketItem[i].state == SaleState.ForSale){
            tokenIds[currentIdx] = i;
            currentIdx++;
        }
    }
    return tokenIds;
    ...
    // Only Owner
    function getCurrentReserved () public view onlyOwner returns(uint256) {
        return RESERVED;
    }
