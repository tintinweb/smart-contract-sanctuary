...
contract ArmyHelper is Ownable, PaymentSplitter {
    IArmyManager public oldManager;
    ...
    constructor(address[] memory _payees, uint256[] memory _shares, address _manager, address _token, address _oldManager) PaymentSplitter(_payees, _shares) {
        manager = IArmyManager(_manager);
        token = IERC20(_token);
        pool = new Pool(_token);
        oldManager = IArmyManager(_oldManager);
    }
    ...
    function createNodeWithTokens(string memory name) public {
        ...
        uint256 nodePrice = manager.price() * 10 ** 18;
    ...
    function createMultipleNodeWithTokens(string memory name, uint amount) public {
        ...
        require(sender != team, "HELPER: Team cannot create node");
        uint256 nodePrice = manager.price() * 10 ** 18;
        require(token.balanceOf(sender) >= nodePrice * amount, "HELPER: Balance too low for creation.");
        token.transferFrom(_msgSender(), address(this),  nodePrice * amount);
        uint256 contractTokenBalance = token.balanceOf(address(this));
        _transferIt(contractTokenBalance);
    ...
    function createMultipleNodeWithTokensAndName(string[] memory names, uint amount) public {
        ...
        address sender = _msgSender();
        require(sender != address(0), "HELPER:  creation from the zero address");
        require(sender != team, "HELPER: futur and rewardsPool cannot create node");
        uint256 nodePrice = manager.price() * 10 ** 18;
        require(token.balanceOf(sender) >= nodePrice * amount, "HELPER: Balance too low for creation.");
        token.transferFrom(_msgSender(), address(this), nodePrice * amount);
        uint256 contractTokenBalance = token.balanceOf(address(this));
    ...
    function claimAll(uint[] calldata _nodes) public {
        address sender = _msgSender();
        require(sender != address(0), "HELPER: creation from the zero address");
    ...
    function migrate(uint[] calldata _ids) external {
        for (uint256 i = 0; i < _ids.length; i++) {
            oldManager.transferFrom(msg.sender, address(this), _ids[i]);
            manager.importArmy(msg.sender, oldManager.getNameOf(_ids[i]), oldManager.getMintOf(_ids[i]), oldManager.getClaimOf(_ids[i]));
        }
    }
}