...
// File: contracts/BananaRepublic.sol
...
/**
 * The Glorious Banana Republic NFT Contract.
 */
contract BananaRepublicNFT is Ownable, ERC721, ERC721Enumerable {
    ...
    uint private _mintSharesReflect = 90;
    uint private _mintSharesLottery = 10;
    ...
    // --- owners call --- //
    /**
     * @dev stricly for emergency migration purposes
     */
    ...
    function teamPayout() public onlyOwner {
        uint adminPayout = _balanceTeam * 25/100;
        uint admin2Payout = _balanceTeam * 26/100;
        uint admin3Payout = _balanceTeam * 25/100;
        uint admin4Payout = _balanceTeam - (adminPayout + admin2Payout + admin3Payout);

        payable(_admin).transfer(adminPayout);
        payable(_admin2).transfer(admin2Payout);
        payable(_admin3).transfer(admin3Payout);
        payable(_admin4).transfer(admin4Payout);
        _balanceTeam = 0;
    }
