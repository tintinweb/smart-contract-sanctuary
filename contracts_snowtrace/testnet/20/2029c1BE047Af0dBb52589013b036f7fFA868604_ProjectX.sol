...
contract ProjectX is ERC20, Ownable, PaymentSplitter {
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        ...
        _approve(address(this), address(uniswapV2Pair), balanceOf(address(this)));
    ...
    function _transfer(
        ...
        if (recipient == uniswapV2Pair && sellFeeEnabled && !swapping) {
        ...
        */
        ...
        super._transfer(sender, recipient, amount - feeTokens);
    ...
    function createNodeWithTokens(uint256 numberOfNodes) public {
        ...
        for (uint256 i = 0; i < numberOfNodes; i++) {
            ...
            /*
            for (uint256 j = 0; j < PRESALE_WALLETS.length; j++) {
                if (sender == address(PRESALE_WALLETS[j])) {
                    exist = int(j);
                }
            }
            */

            uint256 _totalNodeAllowed = exist != - 1 ? 250 : totalNodeAllowed;

            require(
                _totalNodeAllowed <=
                totalNodesCreatedFromAccount + numberOfNodes,
                "NODE CREATION: MAX NODES CREATED"
            );
