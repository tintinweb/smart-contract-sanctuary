...
contract Staking is Initializable, OwnableUpgradeable {
    ...
    function initialzeStaking(IERC20Upgradeable _CESTA, IStakingToken _sCESTA, address distributor_, address _stakingWarmUp, 
        ...
        uint _firstEpochTimestamp, uint warmUpPeriod_, bool _isLockup) external onlyOwner {
        ...
        epoch = Epoch({
            ...
            endTimestamp: _firstEpochTimestamp + _epochLength,
            distribute: 0
        });
    ...
    function claim(address _receiver) public {
        ...
        if ( epoch.timestamp >= info.expiry && info.expiry != 0) { 
            delete warmupInfo[_receiver];
            uint amount = stakingToken.balanceForGons(info.gons);
            
            stakingWarmUp.retrieve(_receiver, amount);
        }
    }
    ...
    function claimRewards(address _receiver) external {
        ...
        CESTA.safeTransfer(_receiver, rewards);
    }

    ///@notice Function to withdraw the staked CESTA before lockup period. Penalty amount is subtracted from rewards
    function forfeit() external {
        ...
        if(reward > 0) {
            ...
            stakingToken.transfer(distributor, penalty);

        } else {
            amtToWithdraw = info.deposit;
        }

        CESTA.safeTransfer( msg.sender, amtToWithdraw );        
    }
    ...
    function unStake(uint _amount, bool _trigger) external {
        ...
        CESTA.safeTransfer( msg.sender, _amount );
    }
