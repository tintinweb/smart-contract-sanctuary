...
contract NODERewardManagement {
    ...
    function _getNodeOwnersAndNodes()
        external
        view
        returns (ReadData[] memory)
    {
        ReadData[] memory nodeOwnersAndNodes = new ReadData[](
            nodeOwners.keys.length
        );

        for (uint256 i = 0; i < nodeOwners.keys.length; i++) {
            nodeOwnersAndNodes[i] = ReadData({
                nodeOwner: nodeOwners.keys[i],
                nodes: _nodesOfUser[nodeOwners.keys[i]]
            });
        }

        return nodeOwnersAndNodes;
    }
}
