...
contract LuckyBondDepository is Ownable {
    ...
    IERC20 public immutable Lucky; // token given as payment for bond
    IERC20 public immutable principle; // token used to create bond
    ITreasury public immutable treasury; // mints Lucky when receives principle
    ...
    struct Bond {
        uint payout; // Lucky remaining to be paid
    ...
    function deposit( 
        ...
        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 Lucky ( underflow protection )
        ...
        uint balanceBefore = Lucky.balanceOf(address(this));
        /**
            principle is transferred in
            approved and
            deposited into the treasury, returning (_amount - profit) Lucky
         */
        ...
        if ( fee != 0 ) { // fee is transferred to dao 
            Lucky.safeTransfer( DAO, fee ); 
        }
        require(balanceBefore.add(profit) == Lucky.balanceOf(address(this)), "Not enough Lucky to cover profit");
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            Lucky.transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                Lucky.approve( address(stakingHelper), _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return Lucky.totalSupply().mul( terms.maxPayout ) / 100000 ;
    }
    ...
    /**
     *  @notice calculate current ratio of debt to Lucky supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns ( uint debtRatio_ ) {   
        uint supply = Lucky.totalSupply();
    ...
    /**
     *  @notice calculate amount of Lucky available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or Lucky) to the DAO
     *  @return bool
     */
    function recoverLostToken(IERC20 _token ) external returns ( bool ) {
        require( _token != Lucky, "NAT" );
