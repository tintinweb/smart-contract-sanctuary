...
contract Codenamepetrol  is ERC721, ERC721Enumerable, Ownable, ReentrancyGuard {
  using SafeMath for uint8;
  using SafeMath for uint256;
  // total tokens
  uint256 public constant MAX_TOKENS = 4444;
  // maximum tokens for giveaways
  uint256 public constant MAX_GIVEAWAY_TOKENS = 100;
  ...
  constructor(string memory baseURI) ERC721("CodenamePetrol", "Test") {
    setBaseURI(baseURI);
  }
  ...
  function mint(uint256 numTokens) external payable nonReentrant {
    ...
    require(numTokens > 0 && numTokens <= 50, "The number of tokens must be between 1 and 10");
