...
contract PROBE is Context, IERC20, Ownable {
    ...
    address public royaltyAddress = 0xeB2B7dbf1D37B1495f855aCb2d251Fa68e1202ce;
    ...
    // address snatchWallet = 0x4B20993Bc481177ec7E8f571ceCaE8A9e22C02db;

    // uint256 bunghole = _totalSupply.div(1000);
    ...
    string private _symbol = "PROBEZ";
    string private _name = "ANAL Space Probe!";
    ...
    constructor() 
    {
        ...
        uint256 presale = blackHole.mul(2).div(10);

        // 59% to deployer for LP addition
        uint256 lp = blackHole.mul(45).div(100);
        ...
        _reserveTokenBalance[treasuryWallet] = presale.mul(rate);
        _reserveTokenBalance[_msgSender()] = lp.mul(rate);
        _reserveTokenBalance[dexWallet] = dex.mul(rate);
        _reserveTokenBalance[dev1] = devs.mul(rate);
        _reserveTokenBalance[dev2] = devs.mul(rate);
        _reserveTokenBalance[dev3] = devs.mul(rate);

        emit Transfer(address(0), burn, blackHole);
        emit Transfer(address(0), treasuryWallet, presale);
        emit Transfer(address(0), _msgSender(), lp);
        emit Transfer(address(0), dexWallet, dex);
        emit Transfer(address(0), dev1, devs);
        emit Transfer(address(0), dev2, devs);
        emit Transfer(address(0), dev3, devs);
    }
  ...
  function reserveSupply() public view returns (uint256) {
    ...
    function applyExternalTransactionTax(
        ...
        _reserveTokenBalance[royaltyAddress] = _reserveTokenBalance[
            royaltyAddress
        ].add(reserveValues.reserveTransferAmountMooncakeRoyalty);
        emit Transfer(
            sender,
            royaltyAddress,
