...
interface IAUXLERC20 is IERC20Mintable, IERC20 {
...
contract AuxlTreasury is Ownable {
    ...
    /**
        @notice allow approved address to deposit an asset for Auxl
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint Auxl needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        limitRequirements(msg.sender, send_);
        Auxl.mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn Auxl for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        Auxl.burnFrom( msg.sender, value );
    ...
    /**
        @notice allow approved address to repay borrowed reserves with Auxl
        @param _amount uint
     */
    function repayDebtWithAuxl( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved as debtor" );
        require( isReserveSpender[ msg.sender ], "Not approved as spender" );

        Auxl.burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, address(Auxl), _amount, _amount );
    }
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( Auxl.totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns Auxl valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match Auxl decimals
            value_ = _amount.mul( 10 ** Auxl.decimals() ).div( 10 ** IERC20( _token ).decimals() );
