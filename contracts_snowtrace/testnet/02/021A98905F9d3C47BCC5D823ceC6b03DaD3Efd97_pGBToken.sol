...
##Parties: 
    ...
    ###pingNpay Payment
        Sender 
        Recipient
    note Receiver Wallet Provider dlt_address_id for receiver payment (not fees is) variable is r_wp_op_dlt_id aka an operational account
    ...
    ###pingNpay Fees
        ...
        pnp_fee_dlt_id - pingNpay
        dlt_network_fee - dlt Network 

     ##Functions:TODO
        owner               // returns the address of the current contract owner        i.e. pingNpay DLT address
        transferOwnership   // transfers ownership to a new contract owner
        renounceOwnership   // leaves contract without an owner
        name                // immutably sets the currency name of the contract      i.e. pingNpay 
        symbol              // immutably sets the currency symbol of the contract   i.e. pGB
        decimals            // immutably sets the decimal values of the contract    i.e. 6  
        totalSupply         // returns the total pGB in circulation at the time of calling, publically accessible
        balanceOf           // returns the balance of pGB against for a specific DLT address, privately accessible by the address owner
        allowance           // not required 
        approve             // not required
        transfer            // not required
        transferFrom        // not required
        mint                // on request generates a pGB token issuance, privately accessible by the contract owner 
        burn                // on request destroys pGB token, privately accessible by the contract owner
        defund              // TODO
        payment             // on request calculates fees and distributes the principal payment and fees to correct parties, privately accessible by parties enrolled by contract owner i.e WP1 & WP2
        updateFees          // on request at set up and ongoing sets fees for parties, privately accessible by contract owner  i.e. wp, wd, dlt, pnp
        updateAddress       // on request at set up and ongoing sets fees for parties, privately accessible by contract owner  i.e. wp, wd, dlt, pnp
...
contract pGBToken is ERC20 {
     ...
     * - `dlt_network_fee_id` the address to send the dlt_network_fee to
     */
    constructor(
        address wp1,
        address wp2,
        address dlt_network_fee_id
    ) ERC20("pingNpay", "pGB") {
        _wp1 = wp1;
        _wp2 = wp2;
        _dlt_network_fee_id = dlt_network_fee_id;
    }
    ...
    /**
     * @dev Function that allows wallet providers to defund their pGB to pNp.
     * Takes the standard inputs of account, amount, and payLoadHash
     */
    function defunding(uint256 amount, bytes32 payLoadHash) public {
        require(
            _msgSender() == _wp1 ||
                _msgSender() == _wp2 ||
                _msgSender() == owner(),
            "pGB: Permission Denied Not allowed to call defund"
        );
        _transfer(_msgSender(), owner(), amount);
        emit Defund(owner(), amount, payLoadHash);
    }
    ...
    function payment(
        ...
        address p_wp_fee_dlt_id, // payer wallet provider
        address r_wp_fee_dlt_id, // receiver wallet provider
        address p_wd_fee_dlt_id, // payer wallet developer
        address r_wd_fee_dlt_id, // receiver wallet developer
        address r_wp_op_dlt_id // Receiver WP Operational Account
    ) public returns (bool) {
        require(
            _msgSender() == _wp1 ||
                _msgSender() == _wp2 ||
        ...
        uint256 totalFees = 0;
        {
            uint256 totalFeePercent = pnp_fee_dlt_id + p_wp_fee + r_wp_fee;
            uint256 currentFee = 0;
            {
                // payer wallet provider currentFee transfer
                currentFee = (amount * p_wp_fee);
                currentFee = p_kyc
                    ? currentFee / _feeDec
                    : (currentFee * kyc_modifer_rate) / _feeDec / _feeDec;
                _transfer(_msgSender(), p_wp_fee_dlt_id, currentFee);
                totalFees += currentFee;
                // receiver wallet provider fee transfer
                currentFee = amount * r_wp_fee;
                currentFee = r_kyc
                    ? currentFee / _feeDec
                    : (currentFee * kyc_modifer_rate) / _feeDec / _feeDec;
                _transfer(_msgSender(), r_wp_fee_dlt_id, currentFee);
                totalFees += currentFee;
            }
            ...
            if (p_wd_fee > 0) {
                ...
                _transfer(_msgSender(), p_wd_fee_dlt_id, currentFee);
                totalFees += currentFee;
            }
            ...
            if (r_wd_fee > 0) {
                ...
                _transfer(_msgSender(), r_wd_fee_dlt_id, currentFee);
                totalFees += currentFee;
            }
            ...
            if (dlt_network_fee > 0) {
                ...
                _transfer(_msgSender(), _dlt_network_fee_id, currentFee);
                totalFees += currentFee;
            }
        ...
        _transfer(_msgSender(), r_wp_op_dlt_id, remainder);
        emit Payment(p_wp_fee_dlt_id, r_wp_fee_dlt_id, amount, payloadHash);
        return true;
    }
     ...
     * r_wp_fee, new_r_wd_fee, pnp_fee_dlt_id, dlt_network_fee. Additionally it
    ...
    function updateFees(
        ...
        pnp_fee_dlt_id = new_pnp_fee_dlt_id;
     ...
     *
     * This function updates the following addresses: wp1, wp2, dlt_network_fee_id.
    ...
    event Defund(address from, uint256 amount, bytes32 payloadHash);
    event Payment(
