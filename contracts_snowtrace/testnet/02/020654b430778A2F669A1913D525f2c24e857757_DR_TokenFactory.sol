...
import "../DR_Token/DR_Token.sol";
import "../DR_Registry/IDR_Registry.sol";


contract DR_TokenFactory {

    IDR_Registry public proxyregistry;
    address      public sale_address;

    DR_Token[]   public projects;

    error notAdminError(address notAdmin);

    modifier onlyAdmin() {
        if (! proxyregistry.isAdmin(msg.sender)){
            revert notAdminError({ notAdmin: msg.sender});
        }
        _;
    }

    constructor(IDR_Registry _reggie, address _sale_address) {
        sale_address = _sale_address;
        proxyregistry = _reggie;
    }
    ...
    function newToken(
        ...
        dr.transferOwnership(msg.sender);
        
        //proxyregistry.
    }

    function number_of_tokens() external view returns (uint256) {
...
import "../DR_Registry/IDR_Registry.sol";
import "./IDR_Token.sol";

//import "hardhat/console.sol";

contract DR_Token  is ContextMixin, ERC721Enumerable, NativeMetaTransaction, Ownable {
    ...
    function isApprovedForAll(
        ...
        if (registry.authorised(_owner,_operator)) {
            return true;
        }
...
interface IDR_Token is IERC721 {

    function mint(address _owner, address _artist, uint tokenId, string memory _tokenURI) external;
