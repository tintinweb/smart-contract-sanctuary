/**
 *Submitted for verification at testnet.snowtrace.io on 2022-01-01
*/
...
contract NoonerCoin  is ERC20{
    ...
    function mintToken(address add) public returns(bool) {
        ...
        //uint256 fromTime = _cycleTime.sub(_frequency);//86280
    ...
    function burnToken() internal returns(bool) {
        ...
        uint256 totalMintedTokens = noonercoin[adminAddress].sub(_fundersAmount).mul(10**18).add(noonerwei[adminAddress]).add(totalWeiBurned);
    ...
    function checkMissingPops() public view returns(uint256){
        ...
        if(lastMintingTime != 0) {
            ...
            uint256 secondFrequency = _frequency.mul(2);
            if(differenceOfLastMintTimeAndStartTime <= _frequency || differenceOfLastMintTimeAndStartTime <= secondFrequency) {
                timeDifference = 0;
            }
            else {
                timeDifference = differenceOfLastMintTimeAndStartTime.sub(secondFrequency);
            }
