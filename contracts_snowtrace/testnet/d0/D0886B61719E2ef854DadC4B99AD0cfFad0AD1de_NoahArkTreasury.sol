...
contract NoahArkTreasury is Ownable {
  using SafeMath for uint256;
  ...
  address public sNRK;
  uint256 public sNRKQueue; // Delays change to sNRK address
  ...
  /**
        @notice allow approved address to deposit an asset for NRK
        @param _amount uint
  ...
  function deposit(
    ...
    (_token, _amount);
    // mint NRK needed and store amount of rewards for distribution
    send_ = value.sub(_profit);
    IERC20Mintable(NRK).mint(msg.sender, send_);
  ...
  /**
        @notice allow approved address to burn NRK for reserves
        @param _amount uint
        @param _token address
     */
  function withdraw(uint256 _amount, address _token) external {
    ...
    uint256 value = valueOfToken(_token, _amount);
    INRKERC20(NRK).burnFrom(msg.sender, value);
  ...
  function incurDebt(uint256 _amount, address _token) external {
    ...
    uint256 maximumDebt = IERC20(sNRK).balanceOf(msg.sender); // Can only borrow against sNRK held
  ...
  /**
        @notice allow approved address to repay borrowed reserves with NRK
        @param _amount uint
     */
  function repayDebtWithNRK(uint256 _amount) external {
    require(isDebtor[msg.sender], "Not approved");

    INRKERC20(NRK).burnFrom(msg.sender, _amount);

    debtorBalance[msg.sender] = debtorBalance[msg.sender].sub(_amount);
    totalDebt = totalDebt.sub(_amount);

    emit RepayDebt(msg.sender, NRK, _amount, _amount);
  }
  ...
  function mintRewards(address _recipient, uint256 _amount) external {
    ...
    IERC20Mintable(NRK).mint(_recipient, _amount);
  ...
  function excessReserves() public view returns (uint256) {
    return totalReserves.sub(IERC20(NRK).totalSupply().sub(totalDebt));
  }
  ...
  /**
        @notice returns NRK valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
  ...
  {
    if (isReserveToken[_token]) {
      // convert amount to match NRK decimals
      value_ = _amount.mul(10**IERC20(NRK).decimals()).div(
    ...
    returns (bool)
  {
    ...
    } else if (_managing == MANAGING.SNRK) {
      // 9
      sNRKQueue = block.number.add(blocksNeededForQueue);
    } else return false;
