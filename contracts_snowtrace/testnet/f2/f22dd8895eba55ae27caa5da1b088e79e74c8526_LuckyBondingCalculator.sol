...
contract LuckyBondingCalculator is IBondingCalculator {
    ...
    function markdown( address _pair ) external view returns ( uint ) {
        ...
        uint reserve;
        if ( IUniswapV2Pair( _pair ).token0() == address(Lucky) ) {
            reserve = reserve1;
        } else {
            require(IUniswapV2Pair( _pair ).token1() == address(Lucky), "not a Lucky lp pair");
            reserve = reserve0;
        }
        return reserve.mul( 2 * ( 10 ** Lucky.decimals() ) ).div( getTotalValue( _pair ) );
    }
}