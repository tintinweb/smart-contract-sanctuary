...
// modifiers
    modifier onlyStaker() {
        require(msg.sender == stakes[msg.sender].staker, "Only stakers can call this function");
        _;
    }
    ...
    modifier onlyActivator() {
        require(msg.sender == activator, "Only activator can call this function");
        _;
    }
// constructor
    constructor(uint256 _maxPayout, address _rewardToken){
...
// functions
    ...
    function unstake() external onlyStaker{
        ...
        require(stakes[msg.sender].earnings >= 0, "You can't claim RXG with negative earnings");
        ...
        require(token.allowance(activator, address(this)) >= stakes[msg.sender].earnings, "Contract doesn't have enough allowance to send earnings");
        bool success = token.transferFrom(activator, msg.sender, stakes[msg.sender].earnings);
    ...
    function claimRXG(uint256 _amount) external onlyStaker{ // make sure activator has approved the contract to spend the rxg
        require(stakes[msg.sender].earnings > _amount);
        require(token.allowance(activator, address(this)) >= stakes[msg.sender].earnings, "Contract doesn't have enough allowance to send earnings");
        bool success = token.transferFrom(activator, payable(msg.sender), _amount);
        require(success, "Failed to send RXG to the staker");
    ...
    function setPaychart(uint256[] memory _payChart) onlyOwner() external {
        payChart = _payChart;
    }
    ...
    function autoEmissions(address _staker) internal { //in wei
    ...
    function activate() public onlyActivator() {
        ...
        for (uint256 i = 0; i < stakers.length; i++) {
            if (time >= stakes[stakers[i]].timestamp + 30) { // 30 seconds test days
                stakes[msg.sender].timestamp = time;
