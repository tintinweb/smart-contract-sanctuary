...
contract BlockBondDepository is Ownable {
    ...
    IERC20 public immutable Block; // token given as payment for bond
    IERC20 public immutable principle; // token used to create bond
    ITreasury public immutable treasury; // mints Block when receives principle
    ...
    event LogTrace(string text, uint256 value);
    ...
    function deposit( 
        ...
        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 Block ( underflow protection )
        require( payout <= maxPayout(), "Bond too large"); // size protection because there is no slippage

        emit LogTrace("bondprice", nativePrice);
        return 0;
        ...
        // uint balanceBefore = Block.balanceOf(address(this));
        // /**
        //     principle is transferred in
        //     approved and
        //     deposited into the treasury, returning (_amount - profit) Block
        //  */
        ...
        //     Block.safeTransfer( DAO, fee ); 
        // }
        // require(balanceBefore.add(profit) == Block.balanceOf(address(this)), "Not enough Block to cover profit");
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            Block.transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                Block.approve( address(stakingHelper), _amount );
                stakingHelper.stake( _amount, _recipient );
            } else {
                Block.approve( address(staking), _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return Block.totalSupply().mul( terms.maxPayout ) / 100000 ;
    }
    ...
    /**
     *  @notice calculate current ratio of debt to Block supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns ( uint debtRatio_ ) {   
        uint supply = Block.totalSupply();
    ...
    /**
     *  @notice calculate amount of Block available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or Block) to the DAO
     *  @return bool
     */
    function recoverLostToken(IERC20 _token ) external returns ( bool ) {
        require( _token != Block, "NAT" );
