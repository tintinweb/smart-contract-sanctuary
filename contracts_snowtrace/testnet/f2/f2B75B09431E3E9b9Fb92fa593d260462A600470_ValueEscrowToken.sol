...
contract ValueEscrowToken is IERC20Historical, Ownable, ReentrancyGuard
{
	...
	uint256 public constant UNLOCK_BASIS = 1 seconds;
	...
	function checkpoint() external override nonEmergency
	{
		...
		if (block.timestamp >= _point.time + UNLOCK_BASIS) {
			_checkpoint(0, 0, criticalGas_);
		}
	}

	function updateCriticalGas() external nonEmergency
	{
		Point[] storage _points = points_;
		Point storage _point = _points[_points.length - 1];
		require(block.timestamp >= _point.time + UNLOCK_BASIS, "invalid state");
		uint256 _start = gasleft();
		_checkpoint(0, 0, type(uint256).max);
		uint256 _end = gasleft();
		criticalGas_ = _start - _end;
	}
	...
	function _checkpoint(uint256 _incBias, uint256 _incSlope, uint256 _criticalGas) internal
	{
	...
	function _checkpoint(address _account, uint256 _oldAmount, uint256 _oldUnlock, uint256 _newAmount, uint256 _newUnlock) internal
	{
		...
		_checkpoint(_newBias - _oldBias, _newSlope - _oldSlope, 0);
