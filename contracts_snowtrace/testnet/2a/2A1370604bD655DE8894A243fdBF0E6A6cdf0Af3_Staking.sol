...
contract Staking is Ownable {
    ...
    event Withdrawn(address indexed _user, uint256 indexed _amount, uint256 _time);
   
    event Unstake(address indexed _user, uint256 indexed _amount, uint256 _time);
    ...
    function stake(uint256 tokens) public {
        ...
        user.stakerecord[user.stakecount].lastWithdrawTime = block.timestamp + 30 days;
        user.stakerecord[user.stakecount].bonus = tokens.mul(interestPercentage).div(divider);
    ...
    function withdraw (uint256 _stakeCount) public {
        ...
        require(block.timestamp >= user.stakerecord[_stakeCount].lastWithdrawTime + minimumDays, "Withdraw cant be done before time.");
    
        uint256 totalAvailableWithdraw = block.timestamp.sub(user.stakerecord[_stakeCount].lastWithdrawTime).div(minimumDays);
        uint256 totalBonus = (user.stakerecord[_stakeCount].bonus.mul(monthlyPercentage).div(divider)).mul(totalAvailableWithdraw);
        
        uint256 totalWithdrawn = (monthlyPercentage
                                    .mul(user.stakerecord[_stakeCount].totalWithdrawn)
                                    .mul(user.stakerecord[_stakeCount].bonus))
                                    .div(divider);

        if(user.stakerecord[_stakeCount].totalWithdrawn + totalAvailableWithdraw > withDrawalTimes){
            erc20Token.transferFrom(owner(), msg.sender, user.stakerecord[_stakeCount].bonus.sub(totalWithdrawn));
            ...
            user.stakerecord[_stakeCount].totalWithdrawn = user.stakerecord[_stakeCount].totalWithdrawn.add(totalAvailableWithdraw);
        }
    ...
    function unstake(uint256 _stakeCount) public {
        User storage user = users[msg.sender];   
        require(block.timestamp >= user.stakerecord[_stakeCount].lastWithdrawTime + 30 days, "Stake: Tokens not unlocked yet.");
        require (!user.stakerecord[_stakeCount].unstaked, "Already unstaked.");
        
        erc20Token.transferFrom(owner(), msg.sender, user.stakerecord[_stakeCount].amount);
        user.stakerecord[_stakeCount].unstaked = true;       
     
        emit Unstake(msg.sender, user.stakerecord[_stakeCount].amount, block.timestamp);
    }
