...
contract AlphaENIGMMigration is Ownable {
  ...
  function available(address _user, uint256 _fakeTimestamp, uint256 _fakeBalance, uint256 _fakeClaimed) public view returns (uint256) {
    ...
    uint256 claimed = 0;
    uint256 claimable = 0;
    for (uint256 i = steps.length - 1; i >= 0; i--) {
      uint256 step = steps[i];
      if (lastMigration[_user] < step && timestamp >= step) {
        // claimable += percentPerStep[i];
      } else if (lastMigration[_user] > timestamp) {
        // claimed += percentPerStep[i];
      }
    }
    uint256 balance = aENIGM.balanceOf(_user);
    uint256 originalBalance = _fakeBalance / (100 - _fakeClaimed) * 100;
