...
contract Shop is Ownable {
    ...
    uint256 public constant YIELD_CPS = 16666666666666667; // burgrs baked per second per unit of yield
    uint256 public constant CLAIM_BURGR_TAX_PERCENTAGE = 20;
    ...
    constructor(Chef _chef, Burgr _burgr, address _pantryAddress) {
        chef = _chef;
        burgr = _burgr;
        pantryAddress = _pantryAddress;
    }
    ...
    function getBurgrsAccruedForMany(uint256[] calldata _tokenIds) external view returns (uint256[] memory) {
        uint256[] memory burgrAmounts = new uint256[](_tokenIds.length);
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            burgrAmounts[i] = _getBurgrsAccruedFor(_tokenIds[i], false);
        }
        return burgrAmounts;
    }

    function _getBurgrsAccruedFor(uint256 _tokenId, bool checkOwnership) internal view returns (uint256) {
        ...
        return (block.timestamp - stake.startTimestamp) * chef.tokenYield(_tokenId) * YIELD_CPS;
    }
    ...
    function stakeMany(uint256[] calldata _tokenIds) external {
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            _addChefToShop(tokenId, _msgSender());
        }
    }

    function claimBurgrsAndMaybeUnstake(uint256[] calldata _tokenIds, bool unstake) external {
        uint256 totalClaimed = 0;
        uint256 totalTaxed = 0;

        for (uint256 i = 0; i < _tokenIds.length; i++) {
            uint256 tokenId = _tokenIds[i];
            uint256 burgrs = _getBurgrsAccruedFor(tokenId, true); // also checks that msg.sender owns this token
            uint256 taxAmount = (burgrs * CLAIM_BURGR_TAX_PERCENTAGE + 99) / 100; // +99 to round the division up

            totalClaimed += burgrs - taxAmount;
        ...
        burgr.mint(_msgSender(), totalClaimed);
        burgr.mint(pantryAddress, totalTaxed);
    }
    ...
    function _addChefToShop(uint256 _tokenId, address _owner) internal {
    ...
    function batchedStakesOfOwner(
        ...
        for (uint256 i = 0; i < outputSize; i++) {
            ...
            outputs[i] = OwnedStakeInfo({
                ...
                accrual: _getBurgrsAccruedFor(tokenId, false)
            });
        }
