...
abstract contract ERC20 is IERC20, Ownable {
  ...
  function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
    ...
    function _priceImpactCalc(uint amount_) internal view returns(uint impact) {
      ...
      return ((price - newPrice) * 1e9)/price; // priceImpact dec9
    ...
    uint public aggWGF = 25000000;  //aggWFG initial 1 wrth/0.025% PriceImpact
    ...
    event testWrth(uint indexed priceImpact_, uint indexed aggDump_, uint indexed wrthGen_);
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        if(msg.sender == staking || msg.sender == stakingHelper || _taxExempt[msg.sender] || _taxExempt[sender] ){
          ...
          if(priceImpact >= aggThreshold){
            ...
            uint aggDump = priceImpact - aggThreshold;

            wrthGen = ( (aggThreshold/modWGF) + (aggDump/aggWGF) ) * 1e9 ;

            emit testWrth(priceImpact, aggDump, wrthGen);

            _Wrth.mint(sender, wrthGen);

          }else{

            wrthGen = ( (priceImpact/modWGF) + 1 ) * 1e9;
