...
contract MarketplaceController is Ownable, ReentrancyGuard, Pausable {
    ...
    */
    ...
    event AdClicked(uint256 indexed adId);

    /// Emitted when a single ad is expired.
    event AdExpired(uint256 indexed adId);

    /// Emitted when a single ad-serve fails.
    event AdServeFailed(address indexed member, uint256 indexed campaignId, string error);

    /// Emitted when a single ad-click fails.
    event AdClickFailed(uint256 indexed adId, string error);

    /// Emitted when a single ad-expire fails.
    event AdExpireFailed(uint256 indexed adId, string error);
    ...
    function adServed(address          member,
        ...
        if(!(TIMESTAMP <= timestamp && timestamp <= block.timestamp)){
            emit AdServeFailed(member, campaignID, "Invalid timestamp.");
            return;
        }
        ...
        if(certifiers.length != certifierWeights.length){
            emit AdServeFailed(member, campaignID, "Invalid certifier data.");
            return;
        }
        ...
        if (campaign.paused) {
            emit AdServeFailed(member, campaignID, "Campaign is paused.");
            return;
        }

        if (campaign.multiwalletBalance < campaign.amountPerAd) {
            emit AdServeFailed(member, campaignID, "Not enough funds.");
            return;
        }
        ...
        if(multiwallet.multiwalletBalance(IMultiwalletContract.Recipients.attentionSeeker, campaign.campaignOwner) < campaign.amountPerAd){
            emit AdServeFailed(member, campaignID, "Not enough multiwallet.");
            return;
        }
        ...
        newAd.member        = member;
        newAd.campaignOwner = campaign.campaignOwner;
        newAd.amountPerAd   = campaign.amountPerAd;
        ...
        adServedUpdate(campaign.amountPerAd, adId, certifiers, certifierWeights, memberRewards);
    ...
    function adServedUpdate(uint256           amountPerAd,
                            uint256           adId,
                            address[]  memory certifiers,
        ...
        uint256 certifierRewards = (amountPerAd * rewardShare[Recipients.certifier]) / totalShare;
        ...
        rewardsLeftovers = rewardsLeftovers + (amountPerAd - (memberRewards + certifierRewards));

        uint256 recipientRewards = (amountPerAd * rewardShare[Recipients.treasury]) / totalShare;
        rewards[adId][Recipients.treasury][treasuryAddress] = recipientRewards;
        rewardsLeftovers = rewardsLeftovers - recipientRewards;

        recipientRewards = (amountPerAd * rewardShare[Recipients.devfund]) / totalShare;
        rewards[adId][Recipients.devfund][devfundAddress] = recipientRewards;
        rewardsLeftovers = rewardsLeftovers - recipientRewards;

        recipientRewards = (amountPerAd * rewardShare[Recipients.arkiusPBCLicense]) / totalShare;
     ...
     *
     * @param adObjs  No. of calls made to each certifier.
     */
    function adServedBatch(AdServe[] memory adObjs) onlyUpdater nonReentrant whenNotPaused external  {

        for (uint256 idx=0; idx<adObjs.length; idx++) {
            adServed(adObjs[idx].member, adObjs[idx].timestamp, adObjs[idx].campaignId, adObjs[idx].certifiers, adObjs[idx].certifierWeights);
        }
    }
    ...
    function adClicked(uint256 adId) internal {
        ...
        if(ad.status != AdStatus.served){
            emit AdClickFailed(adId,"Ad not served.");
            return;
        }

        multiwallet.multiwalletTransfer(ad.campaignOwner,
    ...
    function adExpired(uint256 adId, uint256 cutPercentNumerator, uint256 cutPercentDenominator) internal {
        Ad memory ad = ads[adId];
        if(ad.status != AdStatus.served){
            emit AdExpireFailed(adId, "Ad not served.");
            return;
        }
        // require(ad._timestamp.add(m_expirationTime) <= block.timestamp, "Ad not expired.");

        uint256 cutAmount = (ad.amountPerAd * cutPercentNumerator * PRECISION) / (cutPercentDenominator * PRECISION);

        multiwallet.multiwalletTransfer(ad.campaignOwner, IMultiwalletContract.Recipients.attentionSeeker, treasuryAddress, IMultiwalletContract.Recipients.treasury, cutAmount);
        multiwallet.deMultiwallet(ad.campaignOwner, IMultiwalletContract.Recipients.attentionSeeker, ad.amountPerAd - cutAmount);
