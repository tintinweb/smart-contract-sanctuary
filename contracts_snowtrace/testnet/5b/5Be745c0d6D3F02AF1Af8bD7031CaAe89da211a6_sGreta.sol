...
contract sGreta is IsGRT, ERC20Permit {
    /* ========== DEPENDENCIES ========== */
    ...
    IgGRT public gGRT; // additional staked supply (governance token)
    ...
    constructor() ERC20("Staked GRT", "sGRT", 9) ERC20Permit("Staked GRT") {
    ...
    function setgGRT(address _gGRT) external {
        require(msg.sender == initializer, "Initializer:  caller is not initializer");
        require(address(gGRT) == address(0), "gGRT:  gGRT already set");
        require(_gGRT != address(0), "gGRT:  gGRT is not a valid contract");
        gGRT = IgGRT(_gGRT);
    }
    ...
    /**
        @notice increases rGRT supply to increase staking balances relative to profit_
    ...
    // this function is called by the treasury, and informs sGRT of changes to debt.
    // note that addresses with debt balances cannot transfer collateralized sGRT
    ...
    function changeDebt(
        ...
        require(debtBalances[debtor] <= balanceOf(debtor), "sGRT: insufficient balance");
    }
    ...
    // toG converts an sGRT balance to gGRT terms. gGRT is an 18 decimal token. balance given is in 18 decimal format.
    function toG(uint256 amount) external view override returns (uint256) {
        return gGRT.balanceTo(amount);
    }

    // fromG converts a gGRT balance to sGRT terms. sGRT is a 9 decimal token. balance given is in 9 decimal format.
    function fromG(uint256 amount) external view override returns (uint256) {
        return gGRT.balanceFrom(amount);
    }

    // Staking contract holds excess sGRT
    function circulatingSupply() public view override returns (uint256) {
        return
            _totalSupply.sub(balanceOf(stakingContract)).add(gGRT.balanceFrom(IERC20(address(gGRT)).totalSupply())).add(
