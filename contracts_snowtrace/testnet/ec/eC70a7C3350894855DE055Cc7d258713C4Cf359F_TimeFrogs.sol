...
contract Multisig is Ownable {
    ...
    address[] internal _signatoryAddresses;
...
contract TimeFrogs is ERC721Enumerable, Ownable, Multisig(msg.sender), ERC2981PerTokenRoyalties {
    using Strings for uint256;
  ...
  */
    function mint(uint256 _mintAmount) public payable {
        ...
        require(_mintAmount > 0, "ERROR: Nothing to mint.");
        require(
        ...
        for (uint256 i = 1; i <= _mintAmount; i++) {
            ...
            if (tokenId == maxSupply) {
                lotteryActive = true;
                lotteryDates.push(
                    block.timestamp + (epochDay * lotteryIntervalDays)
                );
            }
  ...
  */
    function sta__100_perc() public payable onlySignatories {
  ...
  */
    function selectWinner() public onlySignatories returns (address) {
        ...
        // increment draw to start
        drawNumber++;

        // Generate random id between 1 - 5000 (corresponds to NFT id)
        uint256 winningToken = uint256(
            keccak256(abi.encodePacked(epochNow, block.difficulty, msg.sender))
        ) % (maxSupply + 1);
        address winnerAddress = ownerOf(winningToken);
        
         Winner memory winnerEntry = Winner({
          date: epochNow,
          winner: winnerAddress,
          tokenId: winningToken
        });

        winnerLog[drawNumber] = winnerEntry;

        // Add the next lottery date
  ...
  */
    function removeFeesForAddress(address addr) public onlySignatories {
        require(!paused, "ERROR: Contract paused!");
        require(
            noFreeAddresses < 10,
  ...
  */
    ...
    function isSignatory() public view returns(bool) {
        bool isSigner = false;
