...
contract EldoradoTreasury is ITreasury, EldoradoAccessControlled {
    /* ========== DEPENDENCIES ========== */
    ...
    /**
     * @notice allow approved address to deposit an asset for ELDA
    ...
    function deposit(
        ...
        // mint ELDA needed and store amount of rewards for distribution
        send_ = value.sub(_profit);
        ELDA.mint(msg.sender, send_);
    ...
    /**
     * @notice allow approved address to burn ELDA for reserves
     * @param _amount uint256
     * @param _token address
     */
    function withdraw(uint256 _amount, address _token) external override {
        ...
        uint256 value = tokenValue(_token, _amount);
        ELDA.burnFrom(msg.sender, value);
    ...
    /**
     * @notice mint new ELDA using excess reserves
    ...
    function mint(address _recipient, uint256 _amount) external override {
        ...
        ELDA.mint(_recipient, _amount);
        emit Minted(msg.sender, _recipient, _amount);
    }

    /**
     * DEBT: The debt functions allow approved addresses to borrow treasury assets
     * or ELDA from the treasury, using sELDA as collateral. This might allow an
     * sELDA holder to provide ELDA liquidity without taking on the opportunity cost
    ...
    function incurDebt(uint256 _amount, address _token) external override {
        uint256 value;
        if (_token == address(ELDA)) {
            require(permissions[STATUS.ELDADEBTOR][msg.sender], notApproved);
            value = _amount;
        } else {
        ...
        sELDA.changeDebt(value, msg.sender, true);
        require(sELDA.debtBalances(msg.sender) <= debtLimit[msg.sender], "Treasury: exceeds limit");
        totalDebt = totalDebt.add(value);

        if (_token == address(ELDA)) {
            ELDA.mint(msg.sender, value);
            ELDADebt = ELDADebt.add(value);
        } else {
            totalReserves = totalReserves.sub(value);
            IERC20(_token).safeTransfer(msg.sender, _amount);
        }
    ...
    function repayDebtWithReserve(uint256 _amount, address _token) external override {
        ...
        IERC20(_token).safeTransferFrom(msg.sender, address(this), _amount);
        uint256 value = tokenValue(_token, _amount);
        sELDA.changeDebt(value, msg.sender, false);
    ...
    /**
     * @notice allow approved address to repay borrowed reserves with ELDA
     * @param _amount uint256
     */
    function repayDebtWithELDA(uint256 _amount) external {
        require(
            permissions[STATUS.RESERVEDEBTOR][msg.sender] || permissions[STATUS.ELDADEBTOR][msg.sender],
            notApproved
        );
        ELDA.burnFrom(msg.sender, _amount);
        sELDA.changeDebt(_amount, msg.sender, false);
        totalDebt = totalDebt.sub(_amount);
        ELDADebt = ELDADebt.sub(_amount);
        emit RepayDebt(msg.sender, address(ELDA), _amount, _amount);
    }
    ...
    function enable(
        ...
        if (_status == STATUS.SELDA) {
            sELDA = IsELDA(_address);
        } else {
    ...
    function execute(uint256 _index) external {
        ...
        if (info.managing == STATUS.SELDA) {
            // 9
            sELDA = IsELDA(info.toPermit);
        } else {
    ...
    function excessReserves() public view override returns (uint256) {
        return totalReserves.sub(ELDA.totalSupply().sub(totalDebt));
    }

    /**
     * @notice returns ELDA valuation of asset
     * @param _token address
     * @param _amount uint256
     * @return value_ uint256
     */
    function tokenValue(address _token, uint256 _amount) public view override returns (uint256 value_) {
        value_ = _amount.mul(10**IERC20Metadata(address(ELDA)).decimals()).div(10**IERC20Metadata(_token).decimals());
    ...
    function baseSupply() external view override returns (uint256) {
        return ELDA.totalSupply() - ELDADebt;
    }
}
