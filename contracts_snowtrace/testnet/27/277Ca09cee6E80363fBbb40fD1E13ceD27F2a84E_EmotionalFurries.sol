...
contract EmotionalFurries is ERC721Enumerable, Ownable {
    // Using statements
    using SafeMath for uint256;
    using Counters for Counters.Counter;
    using Strings for uint256;
    
    // BaseURI State Variable
    string internal baseURI;
    
    // OpenZeppelin "Standard" Opener
    constructor(string memory baseURI_) ERC721("EmotionalFurries", "EFXA") {
        baseURI = baseURI_;
    }
    ...
    // Set new Mint price. The input is in Wei. 1 Ether/AVAX is 10^18 Wei
    function setMintPrice(uint256 mintPriceWei) external onlyOwner {
        require(mintPriceWei > 0, "EmotionalFurries: invalid mintPriceWei");
    ...
    uint256 public startTime = 1640995200;  // 1640995200 = 2022-01-01T00:00:00Z ; 1609455600 = 2021-01-01
    
    // Modifier to make a function start only after a specific time
    modifier periodStarted() {
        require(block.timestamp >= startTime, "EmotionalFurries: Period not started");
        _;
    }
    
    // Set the time when purchase will be available
    function setStartTime(uint256 _startTime) external onlyOwner {
        require(_startTime > 0, "EmotionalFurries: invalid _startTime");
        require(_startTime > block.timestamp, "EmotionalFurries: new start time already passed");
        startTime = _startTime;
    }
    
    // Public Purchase Function
    function purchase(uint256 numberOfNfts) external payable periodStarted {
        require(pendingCount > 0, "EmotionalFurries: All minted");
        require(numberOfNfts > 0, "EmotionalFurries: numberOfNfts cannot be 0");
        require(
          numberOfNfts <= 20,
          "EmotionalFurries: You may not buy more than 20 NFTs at once"
        );
        require(
          totalSupply().add(numberOfNfts) <= MAX_NFT_SUPPLY,
          "EmotionalFurries: sale already ended"
        );
        require(
          mintPrice.mul(numberOfNfts) == msg.value,
          "EmotionalFurries: invalid ether value"
        );
    ...
    // Internal used to do a randomized mint
    function _randomMint(address _to) internal returns (uint256) {
        require(totalSupply() < MAX_NFT_SUPPLY, "EmotionalFurries: max supply reached");
    ...
    // Get index from the [0, 1, 2, 3, ...] pristine list
    function _getPendingAtIndex(uint256 _index) internal view returns (uint256) {
        return _pendingIds[_index] + _index;
    }
    
    // Pop the index, doing a replace to have a [0, 1, 2, 534, 4, ..., totalSupply, extractedIds...] list
    ...
    // Generate a "pseudo" random number from chain stats
    ...
    // Allow to burn a token if you're authorized from, or the owner of the token
    ...
    // Withdraw from program account
    function withdraw(address _to, uint256 amount) external onlyOwner {
        require(amount <= address(this).balance, "The amount required exceed balance");
