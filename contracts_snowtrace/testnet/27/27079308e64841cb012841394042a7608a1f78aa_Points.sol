...
contract Points is Ownable {
    ...
    function firstDayOfMonth() public view returns (uint) {
      ...
      uint subtract = timeNow.sub((today.day - 1).mul(DAY_IN_SECONDS)); //subtract days
      subtract = subtract.sub((today.hour).mul(3600));  //subtract hours
      // subtract = subtract.sub((today.minute).mul(60)); //subtract minutes
      // subtract = subtract.sub(today.second); //subtract seconds
