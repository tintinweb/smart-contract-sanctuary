...
// Dependency file: contracts/interfaces/ITraderJoeFactory.sol
...
// Dependency file: contracts/interfaces/ITraderJoeRouter02.sol
...
// import "contracts/interfaces/ITraderJoeRouter02.sol";
...
contract DividendDistributor is IDividendDistributor {
    ...
    ITraderJoeRouter02 public router;
    ...
    constructor(address rewardToken_, address router_) {
        ...
        router = ITraderJoeRouter02(router_);
...
// import "contracts/interfaces/ITraderJoeFactory.sol";
// import "contracts/interfaces/ITraderJoeRouter02.sol";
// import "contracts/interfaces/IERC20Extended.sol";
...
contract BuybackBabyToken is IERC20Extended, Auth, BaseToken {
    ...
    event AutoLiquify(uint256 amountUSDT, uint256 amountBOG);
    ...
    constructor() payable Auth(msg.sender) {
        _name = "Spongebob Do Not Invest";
        _symbol = "DNI";
        _totalSupply = 7777777777777777 * 10**9;

        rewardToken = 0x08a978a0399465621e667C49CD54CC874DC064Eb;
        router = ITraderJoeRouter02(0x7E3411B04766089cFaa52DB688855356A12f05D1);
        ...
        uint256[5] memory feeSettings;
        feeSettings = [uint256(500), 100, 400, 500, 100000];
        _initializeFees(feeSettings);
        _initializeLiquidityBuyBack();

        distributorGas = 500000;
        swapEnabled = true;
        swapThreshold = _totalSupply / 20000; // 0.005%
        maxWalletSize = _totalSupply / 200; // 0.5%
        maxTxSize = _totalSupply / 500; // 0.2%
    ...
    function setAutomatedMarketMakerPair(address newPair, bool value) public onlyOwner {
        require(pair != newPair, "TVC: The TraderJoe pair cannot be removed from automatedMarketMakerPairs");
 
        _setAutomatedMarketMakerPair(newPair, value);
    }
 
    function _setAutomatedMarketMakerPair(address newPair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[newPair] != value, "TVC: Automated market maker pair is already set to that value");
        automatedMarketMakerPairs[newPair] = value;
        isDividendExempt[newPair] = value;
        isFeeExempt[newPair] = value;
    ...
    function _transferFrom(
        ...
        if(!automatedMarketMakerPairs[recipient] && !excludedAccount){
            require(balanceOf(recipient).add(amount) <= maxWalletSize, "Wallet balance is exceeding maxWalletSize");
        }
        ...
        if (
            ...
            require(amount <= maxTxSize, "Transfer amount exceeds the maxTxAmount.");
 	    
        } else if (
            automatedMarketMakerPairs[recipient] &&
            !excludedAccount
        ) {
            require(amount <= maxTxSize, "Sell transfer amount exceeds the maxTxAmount.");
        }
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountUSDT = address(this).balance.sub(balanceBefore);

        uint256 totalUSDTFee = totalFee.sub(dynamicLiquidityFee.div(2));

        uint256 amountUSDTLiquidity = amountUSDT
            .mul(dynamicLiquidityFee)
            .div(totalUSDTFee)
            .div(2);
        uint256 amountUSDTReflection = amountUSDT.mul(reflectionFee).div(
            totalUSDTFee
        );
        uint256 amountUSDTMarketing = amountUSDT.mul(marketingFee).div(
            totalUSDTFee
        );

        try distributor.deposit{value: amountUSDTReflection}() {} catch {}
        payable(marketingFeeReceiver).transfer(amountUSDTMarketing);

        if (amountToLiquify > 0) {
            router.addLiquidityAVAX{value: amountUSDTLiquidity}(
                address(this),
                amountToLiquify,
                0,
                0,
    ...
    function setMaxTxLimit(uint256 _amount)
        external
        authorized
    {
        maxTxSize = _amount;
