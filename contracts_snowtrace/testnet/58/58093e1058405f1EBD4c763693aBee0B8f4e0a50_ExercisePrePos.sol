...
// File: contracts/presale/ExercisePrePos.sol
...
interface ICirculatingPOS {
    function POSCirculatingSupply() external view returns ( uint );
}

contract ExercisePrePos is Ownable {
    using SafeMath for uint;
    using SafeERC20 for IERC20;

    address public immutable pPOS;
    address public immutable POS;
    address public immutable MIM;
    address public immutable treasury;
    address public immutable circulatingPOSContract;
    ...
    constructor( address _pPOS, address _pos, address _mim, address _treasury, address _circulatingPOSContract ) {
        require( _pPOS != address(0) );
        pPOS = _pPOS;
        ...
        treasury = _treasury;
        require( _circulatingPOSContract != address(0) );
        circulatingPOSContract = _circulatingPOSContract;
    }
    ...
    // Allows wallet to redeem pPOS for POS
    function exercise( uint _amount ) external returns ( bool ) {
        ...
        IERC20( MIM ).safeTransferFrom( msg.sender, address( this ), _amount );
        IPrePos( pPOS ).burnFrom( msg.sender, _amount );

        IERC20( MIM ).approve( treasury, _amount );
        uint POSToSend = ITreasury( treasury ).deposit( _amount, MIM, 0 );

        terms[ msg.sender ].claimed = info.claimed.add( _amount );

        IERC20( POS ).safeTransfer( msg.sender, POSToSend );
    ...
    function redeemable( Term memory _info ) internal view returns ( uint ) {
        return ( ICirculatingPOS( circulatingPOSContract ).POSCirculatingSupply().mul( _info.percent ).mul( 1000 ) ).sub( _info.claimed );
    }
