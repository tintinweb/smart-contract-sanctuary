...
contract AtnBondDepository is Ownable {
    ...
    /* ======== STATE VARIABLES ======== */
    address public immutable ATN; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints ATN when receives principle
    ...
    struct Bond {
        uint payout; // ATN remaining to be paid
    ...
    function deposit( 
        ...
        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 ATN ( underflow protection )
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            IERC20( ATN ).transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                IERC20( ATN ).approve( stakingHelper, _amount );
                IStakingHelper( stakingHelper ).stake( _amount, _recipient );
            } else {
                IERC20( ATN ).approve( staking, _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return IERC20( ATN ).totalSupply().mul( terms.maxPayout ).div( 100000 );
    }
    ...
    /**
     *  @notice calculate current ratio of debt to ATN supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns ( uint debtRatio_ ) {   
        uint supply = IERC20( ATN ).totalSupply();
    ...
    /**
     *  @notice calculate amount of ATN available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or ATN) to the DAO
     *  @return bool
     */
