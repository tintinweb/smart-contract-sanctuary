...
contract EncodeCoin is ERC20, Ownable {
    ...
    /**
     * @notice Transfer to multiple recipients. Completes transfers in order until balance is depleted
     * @param _recipients Address array of recipients
     * @param _amounts uint array of amounts
     */
    function transferMulti(address[] memory _recipients, uint256[] memory _amounts) public {
        for (uint256 i = 0; i < _recipients.length; i++) {
            _transfer(_msgSender(), _recipients[i], _amounts[i]);
        }
  }
