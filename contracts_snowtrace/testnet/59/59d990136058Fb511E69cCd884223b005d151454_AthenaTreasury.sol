...
contract AthenaTreasury is Ownable {
    ...
    address public sAthena;
    uint public sATNQueue; // Delays change to sATN address
    ...
    constructor (
        ...
        // isLiquidityToken[ _ATNMIM ] = true;
        // liquidityTokens.push( _ATNMIM );

        secondsNeededForQueue = _secondsNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for ATN
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint ATN needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( Atn ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn ATN for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IATNERC20( Atn ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sAthena ).balanceOf( msg.sender ); // Can only borrow against sATN held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with ATN
        @param _amount uint
     */
    function repayDebtWithATN( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IATNERC20( Atn ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, Atn, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( Atn ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( Atn ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns ATN valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match ATN decimals
            value_ = _amount.mul( 10 ** IERC20( Atn ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SATN ) { // 9
            sATNQueue = uint32(block.timestamp).add32( secondsNeededForQueue );
        } else return false;
