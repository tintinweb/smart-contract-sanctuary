...
contract Points is Ownable {
    ...
    function firstDayOfMonth() public view returns (uint) {
      ...
      uint subtract = timeNow.sub((today.day).mul(DAY_IN_SECONDS));
      subtract = subtract.sub((today.hour).mul(3600));
      subtract = subtract.sub((today.minute).mul(60));
      subtract = subtract.sub(today.second);
