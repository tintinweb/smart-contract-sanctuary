...
contract ProjectX is Context, IERC20, Ownable {
    ...
    uint256 accumulatedRewardsTokensAmount = 0;
    ...
    function createNodeWithTokens(uint256 numberOfNodes) public {
        ...
        if (rewardsPoolFee > 0 || treasuryFee > 0 || smoothingReserveFee > 0 || operationPoolFee > 0) {
            ...
            if (rewardsPoolFee > 0 && treasuryFee > 0 && smoothingReserveFee > 0 && operationPoolFee > 0) {
                ...
                // add to
                accumulatedRewardsTokensAmount += rewardsPoolTokens.mul(numberOfNodes);
                accumulatedTreasureTokensAmount += treasureTokens.mul(numberOfNodes);
                accumulatedSmoothingTokensAmount += smoothingTokens.div(2).mul(numberOfNodes);
                accumulatedOperationPoolTokensAmount += operationPoolTokens.mul(numberOfNodes);
                ...
                // transfer token reward pool smoothing pool
                _tokenTransfer(
                    address(this),
            ...
            if (accumulatedTreasureTokensAmount >= swapTreasureTokensThreshold) {
                //swapAndSendToAddress(destination, tokens);(tokenToSwap);
            }

            if (accumulatedTreasureTokensAmount >= swapTreasureTokensThreshold) {
                //swapAndSendToAddress(tokenToSwap);
            }
    ...
    function launch(address routerAddress, uint256 avaxAmount, uint tokenAmount) external onlyOwner {
        ...
        require(avaxAmount > 0, "Initial avax for add liquidity should be bigger than zero");
        require(tokenAmount > 0, "Initial tokens for add liquidity be bigger than zero");
        require(address(this).balance > 0, "Must have AVAX on contract to launch");
        addLiquidity(tokenAmount, avaxAmount);
        restoreAllFee();
        //enableTrading();
