...
contract SNFT is ERC1155, Ownable, ReentrancyGuard, VRFConsumerBase {
    using SafeERC20 for IERC20;
    IERC20 public GRBtoken;
    IStaker public staker;
    AggregatorV3Interface avaxOracle;
    ...
    uint256 public initialInventorySlot = 3;
    ...
    uint256 public priceGRBtoUSD = 1;
    uint256 public totalSupplyGRB = 1000000;
    ...
    uint256 constant LAND_START = AVATAR_START + AVATAR_SKIN_COUNT;
    uint256 constant UPGRADE_START = LAND_START + LAND_SKIN_COUNT;
    uint256 constant NFT_START = UPGRADE_START + UPGRADE_TYPE_COUNT;

    uint256 public lastId = NFT_START;

    constructor() 
            VRFConsumerBase(address(0), address(0))
            ERC1155("https://9jwlufwrttxr.usemoralis.com:2053/server/functions/metadata?_ApplicationId=dYs1HPZBZwkTGBMw4ksfWnvpE5BZ6nT13LfPmHuU&id={id}") {
        ...
        avaxOracle = AggregatorV3Interface(0x5498BB86BC934c8D34FDA08E81D444153d0D06aD);
    ...
    //temporary random function until chainlink VRF
    function random() private returns (uint) {
        nonce++;
        return uint(keccak256(abi.encodePacked(block.timestamp, msg.sender, nonce)));
    } 

    function randomFromSeed(uint256 _seed) private returns (uint) {
        nonce++;
        return uint(keccak256(abi.encodePacked(_seed, nonce)));
    }
    ...
    function upgradeShip(uint256 _tokenId, uint256 statNo, uint256 upgradeCount) external nonReentrant {
        require(balanceOf(msg.sender, _tokenId) == 1, 'ship doesnt belong to the user');
        require(balanceOf(msg.sender, CRYSTAL) >= upgradeShipStatCrystalCost * upgradeCount, 'you dont have enough crystal');
        
        _burn(msg.sender, CRYSTAL, upgradeShipStatCrystalCost * upgradeCount);

        if(statNo == 0){
            spaceshipData[_tokenId].hp +=  upgradeCount;
            emit ShipUpgraded(msg.sender, 0, _tokenId, block.timestamp, upgradeCount, 0, 0, 0);
        }
        else if(statNo == 1){
            spaceshipData[_tokenId].attack +=  upgradeCount;
            emit ShipUpgraded(msg.sender, 0, _tokenId, block.timestamp, 0, upgradeCount, 0, 0);
        }
        else if(statNo == 2){
            require(spaceshipData[_tokenId].miningSpeed != 0, 'you cant upgrade 0 stat');
            spaceshipData[_tokenId].miningSpeed +=  upgradeCount;
            emit ShipUpgraded(msg.sender, 0, _tokenId, block.timestamp, 0, 0, upgradeCount, 0);
        }
        else if(statNo == 3){
            spaceshipData[_tokenId].travelSpeed +=  upgradeCount;
            emit ShipUpgraded(msg.sender, 0, _tokenId, block.timestamp, 0, 0, 0, upgradeCount);
        }
        else{
            require(statNo < 4, 'there is no such stat to upgrade');
        }
    }
    ...
    function fulfillRandomness(bytes32, uint256 randomNumber) internal override {
        fullfillExplore(randomNumber);
    }

    function fullfillExplore(uint256 _random) internal {
    ...
    function claimExploration() external nonReentrant {
        ...
        ExplorationType explorationType = explorationStatus.currentExplorationType;
        uint distance = explorationStatus.currentExplorationDistance;

        if(explorationType == ExplorationType.MINERAL){
            mintMineral(3**distance);
        }
        else if(explorationType == ExplorationType.ENCOUNTER){
            if(explorationStatus.currentEncounterType == EncounterType.ABONDONED){
                mintMineral(2**distance);
            }
            else if(explorationStatus.currentEncounterType == EncounterType.AMBUSH){
                if(explorationStatus.currentMissionFailed){
                    repairDamage(explorationStatus.damageTaken);
                }
                else{
                    repairDamage(explorationStatus.damageTaken);
                    mintMineral(4**distance);
                }
            }
            else if(explorationStatus.currentEncounterType == EncounterType.STORM){
                repairDamage(explorationStatus.damageTaken);
            }
        }
    }
    ...
    modifier updateRefineryData {
        ...
        refinery.waitingToClaim += mineralSpenditure * refinery.productionPerSecond;
        _;
    }
    ...
    function calculateRefinery() external view returns(uint, uint) {
        ...
        return (mineralSpenditure, mineralSpenditure * refinery.productionPerSecond);
    }
    ...
    function removeShipFromFleet(uint _tokenId) external {
        ...
        userFleet[msg.sender][_tokenId] = userFleet[msg.sender][userFleet[msg.sender].length-1];
    ...
    function createShip(address user, uint256 randomNumber, ShipType shiptype) internal {
        ...
        uint256 hp = ((randomNumber % ((stats.hpMax - stats.hpMin) * 100)) / 10 ) + stats.hpMax;
        uint256 attack = (randomNumber % ((stats.attackMax - stats.attackMin) * 10000) / 1000) + stats.attackMin;
        uint256 miningSpeed = ((randomNumber % ((stats.miningSpeedMax - stats.miningSpeedMin) * 1000000)) / 100000 ) + stats.miningSpeedMin;
        uint256 travelSpeed = ((randomNumber % ((stats.travelSpeedMax - stats.travelSpeedMin) * 100000000)) / 10000000 ) + stats.travelSpeedMin;
        uint256 skin = (randomNumber % (shipTypeSkinCount[shiptype] * 10000000000)) / 1000000000;
    ...
    function buyBoosterPackGRB() external nonReentrant {
        GRBtoken.safeTransferFrom(msg.sender, address(this), boosterPackPriceGRB);
        _mint(msg.sender, BOOSTER_PACK, 1, "");
    }

    function buyBoosterPackAVAX() external payable nonReentrant {
        uint price = priceGRBtoUSD * boosterPackPriceGRB / uint(_getAvaxPriceFromOracle());
    ...
    function useBoosterPack() external nonReentrant {
        ...
        uint256 randomNumber = random();
        fullfillBoosterPack(randomNumber);
    }

    function fullfillBoosterPack(uint256 _random) internal {
    ...
    function rewardUpgrade(address _user, uint _randomNumber) internal {
        uint randomNumber = _randomNumber % UPGRADE_TYPE_COUNT;
        _mint(msg.sender, UPGRADE_START + randomNumber, 1, "");
    ...
    function _getAvaxPriceFromOracle() internal view returns (int256) {
        int256 price = avaxOracle.lastAnswer();
        return price;
    }

    function buyGRB(uint256 _amountGRB) external payable nonReentrant {
        uint price = priceGRBtoUSD * _amountGRB / uint(_getAvaxPriceFromOracle());
        require(msg.value >= price, 'you need to send correct pack value');
