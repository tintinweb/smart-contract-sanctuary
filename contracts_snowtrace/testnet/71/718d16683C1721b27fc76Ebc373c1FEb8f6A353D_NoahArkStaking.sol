...
contract NoahArkStaking is Ownable {
    ...
    function stake( uint _amount, address _recipient ) external returns ( bool ) {
        ...
        warmupInfo[ _recipient ].push ( Claim ({
            deposit: _amount ,
            gons: IsNRK( sNRK ).gonsForBalance( _amount ) ,
            expiry: epoch.number.add( warmupPeriod ),
            stakeEpochNumber: epoch.number
        }));
        
        IERC20( sNRK ).safeTransfer( warmupContract, _amount );
        emit StakeEvent(_amount, _recipient);
        return true;
    }

    /**
        @notice retrieve sNRK from warmup
        @param _recipient address
     */
    function claim ( address _recipient ) public {
        Claim[] memory info = warmupInfo[ _recipient ];
        uint _infoLength = info.length;
        uint _totalClaimGons = 0;
        for (uint i = 0 ; i < _infoLength ; i ++) {
            Claim memory claim = info[i];
            if ( epoch.number >= claim.expiry && claim.expiry != 0 ) {
                delete info[ i ];
                _infoLength --;
                i --;
                _totalClaimGons = _totalClaimGons + claim.gons;
            }
        }
        if (_totalClaimGons > 0) {
            IWarmup( warmupContract ).retrieve( _recipient, IsNRK( sNRK ).balanceForGons( _totalClaimGons ) );
        }
    }
    ...
    /**
        ...
        @param _recipient address
     */
    function canClaim ( address _recipient ) public returns ( uint ) {
        Claim[] memory info = warmupInfo[ _recipient ];
        uint _infoLength = info.length;
        uint _totalClaimGons = 0;
        for (uint i = 0 ; i < _infoLength ; i ++) {
        ...
        return IsNRK( sNRK ).balanceForGons( _totalClaimGons );
    }

    /**
        @notice forfeit sNRK in warmup and retrieve NRK
     */
    function forfeit() external {
        ...
        for (uint i = 0 ; i < _infoLength ; i ++) {
            Claim memory claim = info[i];
            _totalClaimGons = _totalClaimGons + claim.gons;
            _totalDeposit = _totalDeposit + claim.deposit;
        }
        if (_totalClaimGons > 0) {
            IWarmup( warmupContract ).retrieve( address(this), IsNRK( sNRK ).balanceForGons( _totalClaimGons ) );
        }
        if (_totalDeposit > 0) {
            IERC20( NRK ).safeTransfer( msg.sender, _totalDeposit );
        }
    }
