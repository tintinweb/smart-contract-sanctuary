...
contract SaleAvat is Ownable, ReentrancyGuard {
    ...
    mapping(address => bool) private _createDistribution;
     ...
     */
    function createDistributionPeriod(DistributionPeriod[] memory distributionPeriods_, uint endSale_, uint precision_) external onlyOwner {

        delete _distributionPeriods;

        _precisionCurse = precision_;
        endSale = endSale_;
        ...
        require(amountPercent.div(_precisionCurse) == 100, "SaleAvat::createDistributionPeriod: Total percentage should be 100%");
    }
    ...
    */
    function buyToken(address addressTransferFrom, uint amountTransferFrom, uint nonce, uint deadline, uint8 v, bytes32 r, bytes32 s) external {

        require(_createDistribution[address(msg.sender)] == false, "SaleAvat::buyToken: Participant already create distribution");

        bool accept = permitBySig(msg.sender, amountTransferFrom, nonce, deadline, v, r, s);
        ...
        uint amount = (amountTransferFrom).mul(_precisionCurse).div(_rateDepositTokens[addressTransferFrom]);
    ...
    function permitBySig(address participant, uint rawAmount, uint nonce, uint deadline, uint8 v, bytes32 r, bytes32 s) internal returns (bool) {
        ...
        require(nonce > nonces[_signatory], "SaleAvat::permitBySig: invalid nonce");
    ...
    */
    function createDistributionAddress(uint amount_, address participant_) internal {
        ...
        delete _distributionAddresses[participant_];

        for(uint i = 0 ; i < _distributionPeriods.length; i++) {
            _amountPercent = amount_.mul(_distributionPeriods[i].percent).div(100).div(_precisionCurse);
         ...
         _createDistribution[participant_] = true;
    }
