...
contract RewardDistributor is Ownable, ReentrancyGuard
{
	...
	uint256 public constant CLAIM_BASIS = 15 minutes;
	uint256 public constant MIN_ALLOC_TIME = 5 minutes;

	uint256 constant DEFAULT_PENALTY_RATE = 50e16; // 50%
	uint256 constant DEFAULT_PENALTY_PERIODS = 2; // 30 minutes
	uint256 constant MAXIMUM_PENALTY_PERIODS = 5 * 52 * 7 * 24 * 4; // ~5 years
	...
	constructor(address _escrowToken, address _rewardToken, address _boostToken, address _treasury)
	{
	...
	function allocate() public returns (uint256 _amount)
	{
		...
		uint256 _newBalance = IERC20(rewardToken).balanceOf(address(this));
		uint256 _balance = _newBalance - _oldBalance;
		uint256 _maxBalance = type(uint256).max / _time;
...
contract StakingToken is IERC20Historical, Ownable, ReentrancyGuard, ERC20
{
	using SafeERC20 for IERC20;
...
contract ValueEscrowToken is IERC20Historical, Ownable, ReentrancyGuard
{
	...
	uint256 public constant UNLOCK_BASIS = 15 minutes;
