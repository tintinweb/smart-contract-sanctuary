...
import "./StaticPriceFeedConsumer.sol";


contract StaticPFWrapper is AggregatorV3Interface, StaticPriceFeedConsumer {

    string internal sDescription;

    constructor(address _link, address _oracle, string memory _fromAsset, string memory _toAsset, string memory _sDescription, uint8 _precision)
    StaticPriceFeedConsumer(_link, _oracle, _fromAsset, _toAsset, _precision) {
    ...
    function getRoundData(uint80 _roundId) external view override returns (
      ...
      uint80 answeredInRound
        ...
        require(_roundId <= lastRoundId);
        roundId = _roundId;
        answeredInRound = roundId;

        answer = roundData[roundId];
...
contract StaticPriceFeedConsumer is ChainlinkClient, ConfirmedOwner {
  using Chainlink for Chainlink.Request;

  uint256 constant private ORACLE_PAYMENT = 1 * ( LINK_DIVISIBILITY / 10 );

  uint8  internal immutable precision;
  string internal fromAsset;
  string internal toAsset;

  bytes32 internal jobId;
  uint80 internal lastRoundId;

  mapping (uint80 => int256)  internal roundData;

  Chainlink.Request private request;
  ...
  constructor(address _link, address _oracle, string memory _fromAsset, string memory _toAsset, uint8 _precision)
  ...
  function update()
    ...
    onlyOwner
  {
    sendChainlinkRequestTo(chainlinkOracleAddress(), request, ORACLE_PAYMENT);
  }

  function fulfillUpdate(bytes32 _requestId, int256 _price)
    public
    recordChainlinkFulfillment(_requestId)
  {
    uint80 _roundId = ++lastRoundId;

    roundData[_roundId] = _price;
    emit PriceUpdated(_requestId, _roundId, _price);
  }

  function setJobId(string memory source) external onlyOwner {
    bytes memory tempString = bytes(source);
    require(tempString.length != 0, "ENTPY_STRING_NOT_ALLOWED");
    ...
    _rebuildRequest(result);
  }

  function _rebuildRequest(bytes32 _jobId) internal {
    Chainlink.Request memory _request = buildChainlinkRequest(_jobId, address(this), this.fulfillUpdate.selector);
    _request.add("from", fromAsset);
    _request.add("to", toAsset);
