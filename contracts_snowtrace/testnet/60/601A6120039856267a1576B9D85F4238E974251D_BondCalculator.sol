...
// File: contracts/libraries/SafeMathExtended.sol
...
library SafeMathExtended {
    function sqrrt(uint256 a) internal pure returns (uint c) {
        if (a > 3) {
            c = a;
            uint b = SafeMath.add( SafeMath.div( a, 2), 1 );
            while (b < c) {
                c = b;
...
contract BondCalculator is IBondCalculator {
    ...
    function markdown( address _pair ) external view override returns ( uint ) {
        ...
        return reserve.mul( 2 * ( 10 ** IERC20Metadata( MAXI ).decimals() ) ).div( getTotalValue( _pair ) );
    }
}