...
contract PartyMansion is
    ERC721Enumerable,
    ReentrancyGuard,
    Ownable
{
    using Strings for uint256;
    uint256 public constant MAX_MINTABLE = 432;

    bool public canClaimAirdrop = false;
    uint256 public minted = 0;

    bytes32 merkleRoot;
    mapping(address => bool) redeemTracker; // checks if addressed has already redeemed

    //to be replaced with the actual imageID
    string baseUri = "data:application/json;base64,e2Rlc2NyaXB0aW9uIjogIkF2YWxhbmNoZSBQYXJ0eSBBbmltYWxzIiwgImltYWdlIjogImh0dHBzOi8vaXBmcy5pby9pcGZzL1FtWkQ0Q1VIaWllNEt3Z01CQ3JmZ0d1aWs4bjFkUFVSMVNDVWZNdTZVeUJyTEQvMjk5LnBuZyJ9";
    ...
    function redeem(
        ...
        require(redeemTracker[account] == false);
        require(
            _verify(_leaf(account, totalGiven), proof),
            "Invalid merkle proof"
        );

        mintAPA(account, minted);
        emit Redeem(minted);
        minted += 1;
        redeemTracker[account] = true;
    }
