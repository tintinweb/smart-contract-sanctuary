...
contract LSTTokenSALE is Ownable {
    using SafeERC20 for ERC20;
    using Address for address;

    uint constant MIMdecimals = 10 ** 18;
    uint constant LSTdecimals = 10 ** 9;
    //200k $LST Allocated for Whitelist Sale
    uint public constant MAX_SOLD = 200000 * LSTdecimals;
    //$5 Minimum per $LST
    uint public constant PRICE = 5 * MIMdecimals / LSTdecimals ;
    // $250 minimum sale
    uint public constant MIN_PRESALE_PER_ACCOUNT = 50 * LSTdecimals;
    // $2k maximum sale
    uint public constant MAX_PRESALE_PER_ACCOUNT = 400 * LSTdecimals;
    ERC20 MIM;
    ...
    bool privateSale;
    bool giveLSTBool;
    address[] founderAddr = [0x55e872922A1a0206A694F45584bbe5a49343108f];
    ...
    constructor() {
        ...
        for( uint256 iteration_ = 0; founderAddr.length > iteration_; iteration_++ ) {
            invested[ founderAddr[ iteration_ ] ] = 5000 * LSTdecimals;
        } 
    }
    ...
    function buyLST(uint256 amount) public onlyEOA {
    ...
    // set LST token address and activate claiming
    function setClaimingActive(address LST) external onlyOwner() {
        LST = LST;
        canClaim = true;
    }
    ...
    // claim LST allocation based on invested amounts
    function claimLST() public onlyEOA {
        ...
        ERC20(LST).transfer(msg.sender, amountPerClaim[msg.sender]);
    ...
    function giveLSTStatus() public view returns ( bool ) {
        return giveLSTBool;
    }

    // Disable the giveLST and giveLSTs function permanently 
    function disableGiveLST() external onlyOwner() {
        giveLSTBool = false;
    }

    // Give LST to address without purchase
    // function giveLST(address buyer_, uint256 amount) external onlyOwner() {
    //     require(sold < MAX_SOLD, "sold out");
    //     require(sold + amount < MAX_SOLD, "not enough remaining");
    ...
    //     require(giveLSTBool, "Period to add address over");

    //     invested[buyer_] += amount;
    //     sold += amount;
    // }

    // Remove LST from function
    function removeLST(address buyer_, uint256 amount) external onlyOwner() { 
    ...
    // Give LST to addresses without purchases
    function giveLST( address[] calldata buyers_, uint256[] calldata amounts_  ) external onlyOwner() returns ( uint256 ) {
        require(giveLSTBool, "Period to add address over");
        require(sold < MAX_SOLD, "sold out");
