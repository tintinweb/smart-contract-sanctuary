...
contract Domani is ERC20 {
  ...
  mapping(address => IDomani.ModuleState) public moduleStates;
  ...
  mapping(address => IDomani.ComponentPosition) private componentPositions;
  ...
  constructor(
    ...
    for (uint256 i = 0; i < _modules.length; i++) {
      moduleStates[_modules[i]] = IDomani.ModuleState.PENDING;
    }
  ...
  function addModule(address _module) external onlyManager {
    require(moduleStates[_module] == IDomani.ModuleState.NONE, "Module must not be added");
    require(controller.isModule(_module), "Must be enabled on Controller");

    moduleStates[_module] = IDomani.ModuleState.PENDING;
  ...
  function removeModule(address _module) external onlyManager {
    ...
    require(moduleStates[_module] == IDomani.ModuleState.INITIALIZED, "Module must be added");

    IModule(_module).removeModule();

    moduleStates[_module] = IDomani.ModuleState.NONE;
  ...
  function removePendingModule(address _module) external onlyManager {
    require(!isLocked, "Only when unlocked");
    require(moduleStates[_module] == IDomani.ModuleState.PENDING, "Module must be pending");
  ...
  function initializeModule() external {
    require(!isLocked, "Only when unlocked");
    require(moduleStates[msg.sender] == IDomani.ModuleState.PENDING, "Module must be pending");

    moduleStates[msg.sender] = IDomani.ModuleState.INITIALIZED;
  ...
  function isInitializedModule(address _module) external view returns (bool) {
    return moduleStates[_module] == IDomani.ModuleState.INITIALIZED;
  }
  ...
  function isPendingModule(address _module) external view returns (bool) {
    return moduleStates[_module] == IDomani.ModuleState.PENDING;
  }
   ...
   */
  function getPositions() external view returns (IDomani.Position[] memory) {
    IDomani.Position[] memory positions = new IDomani.Position[](_getPositionCount());
    ...
    for (uint256 i = 0; i < components.length; i++) {
      ...
      if (_defaultPositionVirtualUnit(component) > 0) {
        positions[positionCount] = IDomani.Position({
          component: component,
          module: address(0),
