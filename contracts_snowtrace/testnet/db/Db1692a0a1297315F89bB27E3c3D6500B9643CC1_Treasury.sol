...
import "../shared/interfaces/IBondingCalculator.sol";
...
contract Treasury is ITreasury, MetaVaultAC {
    ...
    constructor(
        ...
        initialized = false;
        secondsNeededForQueue = _timelock;
    }
    ...
    /**
     * @notice mint new MVD using excess reserves
     * @param _recipient address
     * @param _amount uint256
     */
    function mint(address _recipient, uint256 _amount) external override {
        //console.log("excessReserves");
        //console.log(excessReserves());
        //console.log("_amount");
        //console.log(_amount);
        ...
        MVD.mint(_recipient, _amount);
        emit Minted(msg.sender, _recipient, _amount);
    }

    /**
     * DEBT: The debt functions allow approved addresses to borrow treasury assets
     * or MVD from the treasury, using sMVD as collateral. This might allow an
     * sMVD holder to provide MVD liquidity without taking on the opportunity cost
    ...
    function incurDebt(uint256 _amount, address _token) external override {
        uint256 value;
        if (_token == address(MVD)) {
            require(permissions[STATUS.MVDDEBTOR][msg.sender], notApproved);
            value = _amount;
        } else {
        ...
        sMVD.changeDebt(value, msg.sender, true);
        require(sMVD.debtBalances(msg.sender) <= debtLimit[msg.sender], "Treasury: exceeds limit");
        totalDebt = totalDebt.add(value);

        if (_token == address(MVD)) {
            MVD.mint(msg.sender, value);
            mvdDebt = mvdDebt.add(value);
        } else {
            totalReserves = totalReserves.sub(value);
            IERC20(_token).safeTransfer(msg.sender, _amount);
        }
    ...
    function repayDebtWithReserve(uint256 _amount, address _token) external override {
        ...
        IERC20(_token).safeTransferFrom(msg.sender, address(this), _amount);
        uint256 value = tokenValue(_token, _amount);
        sMVD.changeDebt(value, msg.sender, false);
    ...
    function repayDebtWithMVD(uint256 _amount) external {
        require(permissions[STATUS.RESERVEDEBTOR][msg.sender] || permissions[STATUS.MVDDEBTOR][msg.sender], notApproved);
        MVD.burnFrom(msg.sender, _amount);
        sMVD.changeDebt(_amount, msg.sender, false);
        totalDebt = totalDebt.sub(_amount);
        mvdDebt = mvdDebt.sub(_amount);
        emit RepayDebt(msg.sender, address(MVD), _amount, _amount);
    }
    ...
    function queueTimelock(
        ...
        uint32 timelock = uint32(block.timestamp).add32(secondsNeededForQueue);
        if (_status == STATUS.RESERVEMANAGER || _status == STATUS.LIQUIDITYMANAGER) {
            timelock = uint32(block.timestamp).add32(secondsNeededForQueue.mul32(2));
        }
    ...
    function execute(uint256 _index) external {
        ...
        if (info.managing == STATUS.SMVD) {
            // 9
            sMVD = IsMVD(info.toPermit);
        } else {
    ...
    function disableTimelock() external onlyGovernor {
        ...
        if (onChainGovernanceTimelock != 0 && onChainGovernanceTimelock <= block.timestamp) {
            ...
            onChainGovernanceTimelock = uint32(block.timestamp).add32(secondsNeededForQueue).mul32(7); // 7-day timelock
        }
    }
    ...
    function excessReserves() public view override returns (uint256) {
        return totalReserves.sub(MVD.totalSupply().sub(totalDebt));
    }
    ...
    function tokenValue(address _token, uint256 _amount) public view override returns (uint256 value_) {
        value_ = _amount.mul(10**IERC20(address(MVD)).decimals()).div(10**IERC20(_token).decimals());
    ...
    function baseSupply() external view override returns (uint256) {
        return MVD.totalSupply() - mvdDebt;
    }
}
