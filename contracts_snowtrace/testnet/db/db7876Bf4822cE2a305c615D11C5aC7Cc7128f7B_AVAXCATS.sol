...
contract AVAXCATS is Ownable, ERC721Enumerable, ReentrancyGuard {
  ...
  uint256 private _price = 1.5 ether;

  string public catcontractURI;

  constructor() ERC721("AVAXCATS", "CAT") {}

  address oaf = 0x85a23DF1E86Bf070B1cB30Aa990888BCB1e981Ae;
  address quack = 0x969278E8aAC52C46371A88F6f288E3605FC84daA;

  uint256[10000] private _availableTokens;
  uint256 private _numAvailableTokens = 10000;
  uint256 private _numFreeRollsGiven = 0;

  mapping(address => uint256) public freeRollCats;

  uint256 private _lastTokenIdMintedInInitialSet = 10000;

  function numTotalCats() public view virtual returns (uint256) {
    return 10000;
  }

  function freeRollMint() public nonReentrant {
    require(freeRollCats[msg.sender] > 0, "You don't have any free rolls!");
    uint256 toMint = freeRollCats[msg.sender];
    freeRollCats[msg.sender] = 0;
    uint256 remaining = numTotalCats() - totalSupply();
    if (toMint > remaining) {
      toMint = remaining;
    }
    _mint(toMint);
  ...
  function getNumFreeRollCats(address owner) public view returns (uint256) {
    return freeRollCats[owner];
  }
  ...
  function mint(uint256 _numToMint) public payable nonReentrant {
    ...
    require(
      totalSupply + _numToMint <= numTotalCats(),
      "There aren't this many cats left."
    );
    uint256 costForMintingCats = _price * _numToMint;
    require(
      msg.value >= costForMintingCats,
      "Too little sent, please send more AVAX."
    );
    if (msg.value > costForMintingCats) {
      payable(msg.sender).transfer(msg.value - costForMintingCats);
    }
  ...
  function getCostForMintingCats(uint256 _numToMint)
    public
    view
    returns (uint256)
  {
    require(
      totalSupply() + _numToMint <= numTotalCats(),
      "There are not this many cats left."
    );
    require(
      _numToMint <= MAX_MINTABLE_AT_ONCE,
      "You cannot mint that many cats."
    );
    return _numToMint * _price;
  }

  function getCatsBelongingToOwner(address _owner)
    external
    view
    returns (uint256[] memory)
  {
    uint256 numCats = balanceOf(_owner);
    if (numCats == 0) {
      return new uint256[](0);
    } else {
      uint256[] memory result = new uint256[](numCats);
  ...
  function giveFreeRoll(address receiver, uint256 numRolls) public onlyOwner {
    ...
    require(_numFreeRollsGiven < 500, "Already given max number of free rolls");
    require(
      freeRollCats[receiver] + numRolls < 51,
      "Cannot exceed 50 unused free rolls!"
    );
    uint256 freeRolls = freeRollCats[receiver];
    freeRollCats[receiver] = freeRolls + numRolls;
