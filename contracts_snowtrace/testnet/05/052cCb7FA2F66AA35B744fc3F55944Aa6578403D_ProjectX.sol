...
contract ProjectX is ERC20, Ownable, PaymentSplitter {
    ...
    function createNodeWithTokens(uint numberOfNodes) public {
        for (uint256 i = 0; i < numberOfNodes; i++) {
            ...
            if (
                ...
                // reward pool
                uint256 rewardsPoolTokens = contractTokenBalance.mul(rewardsPoolFee).div(100);
                super._transfer(address(this), rewardsPoolAddress, rewardsPoolTokens);

                // treasury
                uint256 treasureTokens = contractTokenBalance.mul(treasuryFee).div(100);
                uint256 treasureEthAmount = getEstimatedETHForTokens(address(this), treasureTokens);
                swapTokensForEth(treasureTokens);
                payable(treasuryAddress).transfer(treasureEthAmount);

                // smoothing
                uint256 smoothingTokens = contractTokenBalance.mul(smoothingReserveFee).div(100);
                uint256 smoothingEthAmount = getEstimatedETHForTokens(address(this), smoothingTokens.div(2));
                swapTokensForEth(smoothingTokens.div(2));
                payable(smoothingReserveAddress).transfer(smoothingEthAmount);
    ...
    function getEstimatedETHForTokens(address tokenAddress, uint amount) public view returns (uint256) {
        return uniswapV2Router.getAmountsOut(amount, getPathEthForTokens(tokenAddress))[0];
    }

    function getPathEthForTokens(address tokenAddress) private view returns (address[] memory) {
        address[] memory path = new address[](2);
    ...
    function cashoutReward(uint256 blocktime) public {
        ...
        if (swapLiquify && cashoutFee > 0) {
            super._transfer(rewardsPoolAddress, address(this), feeAmount);
            swapAndSendToFee(operationPoolAddress, feeAmount);
        }
