...
contract Noonercoin is ERC20{
 ...
 function name() public view  returns (string memory) {
  ...
  function check2() public view returns (uint256) {
      return lastMintingTime - startTime;
  }
  ...
  function checkFailedTransactions(address add) public view returns (uint256) {
     uint256 balAdmin = noonercoin[add]; //admin bal 
      uint256 currMintingRate = getLastMintingRate(); 
      uint256 timeRefer = lastMintingTime - startTime; //checking time from start time to current time and changing to hrs format.
      
      
      //lets say we got value from timeRefer and we can also get token amount ie.., if value = 24 then divide 24/24 = 1
      
      uint256 valueForEach = timeRefer/120; // we will get 1,2,3....
      
      uint256 estTotal = valueForEach * currMintingRate;// gets the total coins for that total hours
       
      uint256 checkDiff = estTotal - balAdmin;// checking diff from the estimate total and also from admin bal
      uint256 missTokens = checkDiff / mintingRateNoonerCoin; //finding missing tokens.
  ...
  function checkMissingTokens(address add) public view returns (uint256) {
      uint256 balAdmin = noonercoin[add]; //admin bal 
      uint256 currMintingRate = getLastMintingRate(); 
      uint256 timeRefer = lastMintingTime - startTime; //checking time from start time to current time and changing to hrs format.
  ...
  //   uint256 missSec = missTokens * 86400;
    //   _cycleTime += missSec;
