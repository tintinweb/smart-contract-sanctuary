...
// - Properly isolated the contracts
    ...
    LiquidityPoolManagerImplementationPointerUpgradeable
{
    ...
    modifier onlyPlanetOwner() {
        ...
        require(
            ...
            "Planets: Fee receivers cannot access to Cargos"
        );
        _;
    }
    ...
    function initialize(
        ...
        changeNodeMinPrice(_creationMinPrice);
        changeClaimTime(_claimTime);
        changeCashoutFee(_cashoutFee);
        changeTierSystem(
    ...
    function createPlanetWithTokens(
        ...
        require(
            planetPrice >= creationMinPrice,
            "Planets: Node value set below creationMinPrice"
        );
        require(
        ...
        require(
            ...
            "Planets: Balance too low for creation. Use lower initValue"
        );

        // ToDo: Transfer cashout fee to liquidity pool

        // Burn the tokens used to mint the Planet
        universe.accountBurn(sender, planetPrice);
        ...
        //string memory tokenURI = "wtf";
        //_setTokenURI(newPlanetId, tokenURI);
        emit Create(sender, newPlanetId, planetPrice);
        ...
        checkPermissions(_planetId)
    {
        ...
        planet.rewardAvailable = 0;
        planet.rewardMult = tierLevel[0];
        planet.addValueCount = 0;
        ...
        checkPermissions(_planetId)
    {
        ...
        PlanetEntity storage planet = _planets[_planetId];
        require(
            isCompoundable(planet),
            "Planets: You must wait until you can compound again"
        );
        uint256 reward = calculateReward(planet);
        planet.planetValue += reward;
        planet.lastClaimTime = block.timestamp;
        uint256 prevMult = planet.rewardMult;
        if (reward > 0) {
            if (prevMult >= tierLevel[5]) {
                planet.rewardMult += tierSlope[5];
            } else if (prevMult >= tierLevel[4]) {
                planet.rewardMult += tierSlope[4];
            } else if (prevMult >= tierLevel[3]) {
                planet.rewardMult += tierSlope[2];
            } else if (prevMult >= tierLevel[2]) {
                planet.rewardMult += tierSlope[2];
            } else if (prevMult >= tierLevel[1]) {
                planet.rewardMult += tierSlope[1];
            } else {
                planet.rewardMult += tierSlope[0];
            }
            planet.rewardAvailable = 0;
            planet.addValueCount += 1;
        }
        _compoundReward(reward);

        emit Compound(account, _planetId, reward);
    }

    function compoundAll() external nonReentrant onlyPlanetOwner {
        address account = _msgSender();
        ...
        for (uint256 i = 0; i < planetsOwned.length; i++) {
            uint256 planetId = planetsOwned[i];
            PlanetEntity storage planet = _planets[planetId];
            if (!isCompoundable(planet)) {
                delete planetsAffected[i];
                continue;
            }
            planetsAffected[i] = planetId;
            uint256 reward = calculateReward(planet);
            rewardsTotal += reward;
            planet.planetValue += reward;
            planet.lastClaimTime = block.timestamp;
            uint256 prevMult = planet.rewardMult;
            if (planet.rewardAvailable > 0) {
                if (prevMult >= tierLevel[5]) {
                    planet.rewardMult += tierSlope[5];
                } else if (prevMult >= tierLevel[4]) {
                    planet.rewardMult += tierSlope[4];
        ...
        _compoundReward(rewardsTotal);

        emit CompoundAll(account, planetsAffected, rewardsTotal);
    }

    function cashoutAll() external nonReentrant onlyPlanetOwner {
        address account = _msgSender();
        uint256 rewardsTotal = 0;

        uint256[] memory planetsOwned = getPlanetIdsOf(account);
        for (uint256 i = 0; i < planetsOwned.length; i++) {
            PlanetEntity storage planet = _planets[planetsOwned[i]];
            uint256 reward = calculateReward(planet);
            rewardsTotal += reward;
            planet.totalClaimed += reward;
            planet.rewardAvailable = 0;
    ...
    function _cashoutReward(uint256 amount) private {
        ...
        (uint256 amountToReward, uint256 feeAmount) = getCashoutFee(amount);
    ...
    function _compoundReward(uint256 amount) private {
        require(amount > 0, "Planets: No rewards to compound");
        universe.liquidityReward(amount);
    }

    // Private view functions

    function getCashoutFee(uint256 rewardAmount)
        private
        view
    ...
    function isCompoundable(PlanetEntity memory planet)
        ...
        returns (bool)
    {
        return planet.lastClaimTime + claimTime <= block.timestamp;
    }
    ...
    {
        return
            (((planet.rewardAvailable * planet.rewardMult) *
                planet.planetValue) / 100000) / 1e18;
    }
    ...
    function getCreationMinPrice() public view returns (uint256) {
    ...
    function planetExists(uint256 _planetId) public view returns (bool) {
    ...
    function changeNodeMinPrice(uint256 _creationMinPrice) public onlyOwner {
        require(
            _creationMinPrice > 0,
            "Planets: Minimum price to create a Planet must be above 1"
        );
        creationMinPrice = _creationMinPrice;
    }

    function changeClaimTime(uint256 _claimTime) public onlyOwner {
        require(_claimTime > 0, "Planets: claimTime must be superior to 0");
    ...
    function changeCashoutFee(uint8 _cashoutFee) public onlyOwner {
        require(_cashoutFee <= 30, "Cashout fee can never exceed 30%");
