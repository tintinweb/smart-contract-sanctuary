...
interface IBLOCKERC20 is IERC20Mintable, IERC20 {
...
contract BlockTreasury is Ownable {
    ...
    IBLOCKERC20 public immutable Block;
    ...
    /**
        @notice allow approved address to deposit an asset for Block
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint Block needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        limitRequirements(msg.sender, send_);
        Block.mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn Block for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        Block.burnFrom( msg.sender, value );
    ...
    /**
        @notice allow approved address to repay borrowed reserves with Block
        @param _amount uint
     */
    function repayDebtWithBlock( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved as debtor" );
        require( isReserveSpender[ msg.sender ], "Not approved as spender" );

        Block.burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, address(Block), _amount, _amount );
    }
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( Block.totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns Block valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match Block decimals
            value_ = _amount.mul( 10 ** Block.decimals() ).div( 10 ** IERC20( _token ).decimals() );
