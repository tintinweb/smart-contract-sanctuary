...
contract SkadiTreasury is Ownable {
    ...
    uint public sSKIQueue; // Delays change to sSKI address
    ...
    constructor (
        ...
        // isLiquidityToken[ _SKIMIM ] = true;
        // liquidityTokens.push( _SKIMIM );

        secondsNeededForQueue = _secondsNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for SKI
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint SKI needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( Ski ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn SKI for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        ISKIERC20( Ski ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( MEMOries ).balanceOf( msg.sender ); // Can only borrow against sSKI held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with SKI
        @param _amount uint
     */
    function repayDebtWithSKI( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        ISKIERC20( Ski ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, Ski, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( Ski ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( Ski ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns SKI valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match SKI decimals
            value_ = _amount.mul( 10 ** IERC20( Ski ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SSKI ) { // 9
            sSKIQueue = uint32(block.timestamp).add32( secondsNeededForQueue );
        } else return false;
