...
contract DiceSale is Ownable {
    using SafeERC20 for ERC20;
    using Address for address;
//Change values
    uint constant MIMdecimals = 10 ** 18;
    uint constant Dicedecimals = 10 ** 9;
    // TU WOLNO ZMIENIAC, decimals maja zostac!
    uint public constant MAX_SOLD = 75000 * Dicedecimals;
    uint public constant PRICE = 4 * MIMdecimals / Dicedecimals ;
    uint public constant MIN_PRESALE_PER_ACCOUNT = 50 * Dicedecimals;
    uint public constant MAX_PRESALE_PER_ACCOUNT = 250 * Dicedecimals;
    // TODO
//    address[] devAddr = [""];
    // KONIEC
    ...
    // TODO change from testtoken to MIM
    constructor(address adice, address testToken) {
        //MIM CONTRACT ADDRESS
        //MIM = ERC20(0x130966628846BFd36ff31a822705796e8cb8C18D);
        MIM = ERC20(testToken);
        ADice = adice;
        ADICE = ERC20Burnable(ADice);
        owners = msg.sender;
        sold = 0;
        //DEV REWARDS
//        for( uint256 iteration_ = 0; devAddr.length > iteration_; iteration_++ ) {
//            invested[ devAddr[ iteration_ ] ] = 4000 * Dicedecimals;
//        }
    }
    ...
    function buyDice(uint256 amount) public onlyEOA {
        ...
        MIM.transferFrom(msg.sender, address(this), amount * PRICE );
        invested[msg.sender] += amount;
        ADICE.transfer(msg.sender, amount);
        sold += amount;
    }

    // set Dice token address and activate claiming
    function setClaimingActive(address dice) public {
...
//        return false;
//    }
    // claim Dice allocation based on invested amounts
    function claimDice() public onlyEOA {
        require(canClaim, "Cannot claim now");
        uint256 amount = ADICE.balanceOf(msg.sender);
        require(amount > 0, "no claim avalaible");
        require(ADICE.allowance(msg.sender, address(this)) == amount, "Allowance not set!");
        ADICE.burnFrom(msg.sender, amount);
        ERC20(Dice).transfer(msg.sender, amount);
    }
