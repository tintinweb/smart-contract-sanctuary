...
contract PublicSale is Ownable, ReentrancyGuard {
    ...
    struct Sale {
        ...
        uint256 remainingAmount; // Remaining amount of tokens
        uint256 nextTimeRelease; // Next time release
        bool isTge; // 30% on TGE
    }
    ...
    // Total amount sold
    uint256 public totalTokenAmountSold;
    // Release Period
    ...
    constructor(
        ...
        require(_whitelist != address(0), "Zero token address");
    ...
    checkPoolWhiteList(msg.sender)
    {
        ...
        require(
            amount <= getBeneficiaryAlloc(msg.sender) ,
            "Maximum allocation size"
        );
        require(currency.approve(address(this), amount));
        currency.safeTransferFrom(msg.sender, address(this), amount);
        uint256 tokenAmount = amount.div(price).mul(1e18); // Convert amount to wei
        sales[msg.sender] = Sale(tokenAmount, tokenAmount,releaseTime,true);
        numberParticipants += 1;
        totalTokenAmountSold += tokenAmount;
        emit Buy(msg.sender, amount, tokenAmount);
    }

    // Withdraw purchased tokens after release time
    function release() external canClaim(msg.sender) nonReentrant {
        Sale memory user = sales[msg.sender];
        uint256 amount = 0;
        uint256 eachReleaseAmount = 0;

        if(user.isTge) {
            user.isTge = false;
            eachReleaseAmount = (user.amount * 30) / 100;
        }else{
            eachReleaseAmount = (user.amount * 14) / 100;
        }

        if (eachReleaseAmount <= user.remainingAmount) {
            amount = eachReleaseAmount;
        } else {
            amount = user.remainingAmount;
        }
        user.remainingAmount = user.remainingAmount - amount;
        user.nextTimeRelease = user.nextTimeRelease + releasePeriod;
        sales[msg.sender] = user;
    ...
    function getBeneficiaryAlloc(address _address)
    public
    view
    returns (uint256 alloc)
    {
        ...
        alloc = IWhitelist(whitelistContract).whitelistedAmountFor(
            _address
        );
    }

    function getSaleIndexOf(address _address)
    public
    view
    returns (Sale memory)
    {
    ...
    function isPoolWhiteListed(address _address) public view returns (bool whitelisted_) {
        require(
            _address != address(0),
            "Zero account"
        );

        whitelisted_ = IWhitelist(whitelistContract).isWhitelisted(
            _address
        );
    }
    ...
    modifier checkPoolWhiteList(address _address) {
        require(
            IWhitelist(whitelistContract).isWhitelisted(_address),
    ...
    modifier canClaim(address _address) {
        Sale memory user = sales[msg.sender];
        require(user.remainingAmount > 0,"All tokens were released");
        require(
            block.timestamp >= user.nextTimeRelease,
