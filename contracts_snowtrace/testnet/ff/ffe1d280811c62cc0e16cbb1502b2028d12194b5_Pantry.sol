...
contract Pantry is ERC20("Staked Burgr", "sBURGR"), Ownable {
    ...
    uint256 public frozenBurgr; // $BURGR pending withdraw
    ...
    function setBurgr(Burgr _burgr) external onlyOwner {
        require(address(burgr) == address(0), "Burgr contract already set");
    ...
    function stake(uint256 _amount) public {
        ...
        // If no sBURGR exists, mint it 1:1 to the amount put in
        if (totalShares == 0 || burgrBalance() == 0) {
            _mint(msg.sender, _amount);
        } else {
            uint256 what = _amount.mul(totalShares).div(burgrBalance());
            _mint(msg.sender, what);
        }

        burgr.transferToPantry(msg.sender, _amount);
    }
    ...
    function quickUnstake(uint256 _share) public {
        ...
        burgr.transfer(msg.sender, output);
    }

    // argument specified in sBURGR
    ...
    // argument specified in BURGR, not sBURGR
    function claimDelayedUnstake(uint256 _amount) public {
        require(block.timestamp >= unlockTimestamps[msg.sender], "BURGR not yet unlocked");
        require(_amount <= unlockAmounts[msg.sender], "Insufficient locked balance");

        unlockAmounts[msg.sender] -= _amount;
        burgr.transfer(msg.sender, _amount);
        frozenBurgr -= _amount;
    }

    function burgrBalance() public view returns (uint256 balance) {
        balance = burgr.balanceOf(address(this)) - frozenBurgr;
    }
