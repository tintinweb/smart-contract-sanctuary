...
interface ComptrollerLensInterface {
    ...
    function claimThermo(address) external;
    function thermoAccrued(address) external view returns (uint);
}
...
contract ThermoLens {
    ...
    function sTokenMetadata(SToken sToken) public returns (STokenMetadata memory) {
        ...
        if (compareStrings(sToken.symbol(), "sBNB")) {
            ...
            SBep20 sBep20 = SBep20(address(sToken));
            underlyingAssetAddress = sBep20.underlying();
            underlyingDecimals = EIP20Interface(sBep20.underlying()).decimals();
        }
    ...
    function sTokenBalances(SToken sToken, address payable account) public returns (STokenBalances memory) {
        ...
        if (compareStrings(sToken.symbol(), "sBNB")) {
            ...
            SBep20 sBep20 = SBep20(address(sToken));
            EIP20Interface underlying = EIP20Interface(sBep20.underlying());
    ...
    function sTokenUnderlyingPrice(SToken sToken) public view returns (STokenUnderlyingPrice memory) {
        ComptrollerLensInterface comptroller = ComptrollerLensInterface(address(sToken.comptroller()));
    ...
    function sTokenUnderlyingPriceAll(SToken[] calldata sTokens) external view returns (STokenUnderlyingPrice[] memory) {
        uint sTokenCount = sTokens.length;
    ...
    struct TMXBalanceMetadata {
        uint balance;
        uint votes;
        address delegate;
    }

    function getTMXBalanceMetadata(TMX tmx, address account) external view returns (TMXBalanceMetadata memory) {
        return TMXBalanceMetadata({
            balance: tmx.balanceOf(account),
            votes: uint256(tmx.getCurrentVotes(account)),
            delegate: tmx.delegates(account)
        });
    }

    struct TMXBalanceMetadataExt {
        uint balance;
        uint votes;
        address delegate;
        uint allocated;
    ...
    function getTMXBalanceMetadataExt(TMX tmx, ComptrollerLensInterface comptroller, address account) external returns (TMXBalanceMetadataExt memory) {
        uint balance = tmx.balanceOf(account);
        comptroller.claimThermo(account);
        uint newBalance = tmx.balanceOf(account);
        uint accrued = comptroller.thermoAccrued(account);
        uint total = add(accrued, newBalance, "sum tmx total");
        uint allocated = sub(total, balance, "sub allocated");

        return TMXBalanceMetadataExt({
            balance: balance,
            votes: uint256(tmx.getCurrentVotes(account)),
            delegate: tmx.delegates(account),
            allocated: allocated
    ...
    function getThermoVotes(TMX tmx, address account, uint32[] calldata blockNumbers) external view returns (ThermoVotes[] memory) {
        ThermoVotes[] memory res = new ThermoVotes[](blockNumbers.length);
        for (uint i = 0; i < blockNumbers.length; i++) {
            res[i] = ThermoVotes({
                blockNumber: uint256(blockNumbers[i]),
                votes: uint256(tmx.getPriorVotes(account, blockNumbers[i]))
            });
        }
        return res;
    }
...
contract GovernorAlpha {
    ...
    string public constant name = "Thermo Governor Alpha";

    /// @notice The number of votes in support of a proposal required in order for a quorum to be reached and for a vote to succeed
    function quorumVotes() public pure returns (uint) { return 600000e18; } // 600,000 = 2% of TMX

    /// @notice The number of votes required in order for a voter to become a proposer
    function proposalThreshold() public pure returns (uint) { return 300000e18; } // 300,000 = 1% of TMX
    ...
    /// @notice The address of the Thermo Protocol Timelock
    TimelockInterface public timelock;

    /// @notice The address of the Thermo governance token
    TMXInterface public tmx;
    ...
    constructor(address timelock_, address tmx_, address guardian_) public {
        timelock = TimelockInterface(timelock_);
        tmx = TMXInterface(tmx_);
        guardian = guardian_;
    }

    function propose(address[] memory targets, uint[] memory values, string[] memory signatures, bytes[] memory calldatas, string memory description) public returns (uint) {
        require(tmx.getPriorVotes(msg.sender, sub256(block.number, 1)) > proposalThreshold(), "GovernorAlpha::propose: proposer votes below proposal threshold");
    ...
    function cancel(uint proposalId) public {
        ...
        Proposal storage proposal = proposals[proposalId];
        require(msg.sender == guardian || tmx.getPriorVotes(proposal.proposer, sub256(block.number, 1)) < proposalThreshold(), "GovernorAlpha::cancel: proposer above threshold");
    ...
    function _castVote(address voter, uint proposalId, bool support) internal {
        ...
        uint96 votes = tmx.getPriorVotes(voter, proposal.startBlock);
...
contract TMX is Tokenlock {
    /// @notice BEP-20 token name for this token
    string public constant name = "Thermo";
    ...
    uint public constant totalSupply = 30000000e18; // 30 million TMX
    ...
    /**
     * @notice Construct a new TMX token
    ...
    function approve(address spender, uint rawAmount) external validLock returns (bool) {
        ...
        if (rawAmount == uint(-1)) {
            ...
            amount = safe96(rawAmount, "TMX::approve: amount exceeds 96 bits");
        }
    ...
    function transfer(address dst, uint rawAmount) external validLock returns (bool) {
        uint96 amount = safe96(rawAmount, "TMX::transfer: amount exceeds 96 bits");
    ...
    function transferFrom(address src, address dst, uint rawAmount) external validLock returns (bool) {
        ...
        uint96 amount = safe96(rawAmount, "TMX::approve: amount exceeds 96 bits");

        if (spender != src && spenderAllowance != uint96(-1)) {
            uint96 newAllowance = sub96(spenderAllowance, amount, "TMX::transferFrom: transfer amount exceeds spender allowance");
    ...
    function delegateBySig(address delegatee, uint nonce, uint expiry, uint8 v, bytes32 r, bytes32 s) public validLock {
        ...
        bytes32 digest = keccak256(abi.encodePacked("\x19\x01", domainSeparator, structHash));
        address signatory = ecrecover(digest, v, r, s);
        require(signatory != address(0), "TMX::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "TMX::delegateBySig: invalid nonce");
        require(now <= expiry, "TMX::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber) public view returns (uint96) {
        require(blockNumber < block.number, "TMX::getPriorVotes: not yet determined");
    ...
    function _transferTokens(address src, address dst, uint96 amount) internal {
        require(src != address(0), "TMX::_transferTokens: cannot transfer from the zero address");
        require(dst != address(0), "TMX::_transferTokens: cannot transfer to the zero address");

        balances[src] = sub96(balances[src], amount, "TMX::_transferTokens: transfer amount exceeds balance");
        balances[dst] = add96(balances[dst], amount, "TMX::_transferTokens: transfer amount overflows");
    ...
    function _moveDelegates(address srcRep, address dstRep, uint96 amount) internal {
        if (srcRep != dstRep && amount > 0) {
            if (srcRep != address(0)) {
                ...
                uint96 srcRepNew = sub96(srcRepOld, amount, "TMX::_moveVotes: vote amount underflows");
            ...
            if (dstRep != address(0)) {
                ...
                uint96 dstRepNew = add96(dstRepOld, amount, "TMX::_moveVotes: vote amount overflows");
    ...
    function _writeCheckpoint(address delegatee, uint32 nCheckpoints, uint96 oldVotes, uint96 newVotes) internal {
      uint32 blockNumber = safe32(block.number, "TMX::_writeCheckpoint: block number exceeds 32 bits");
