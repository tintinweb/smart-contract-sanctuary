...
contract IglooxVesting is Ownable {
    ...
    function claimTeamShare() external returns(uint256){
        require(intializedTime.add(356 days) < block.timestamp,"Token is locked");
        require(_msgSender() == teamWallet,"caller is not a teamWallet");
        if(teamLastClaim == 0){
           teamLastClaim = block.timestamp; 
        } else{
            require(teamLastClaim.add(30 days) < block.timestamp,"");
            teamLastClaim = teamLastClaim.add(30 days);
        }
        
        uint256 currentPercentage = teamShare.mul(teamMonthlyPercent).div(1e3);
    ...
    function claimAdvisorShare() external returns(uint256){
        ...
        require(intializedTime.add(356 days).add(356 days) < block.timestamp,"Token is locked");

        if(advisorlastClaim == 0){
           teamLastClaim = block.timestamp; 
        } else{
            require(advisorlastClaim.add(30 days) < block.timestamp,"");
            advisorlastClaim = advisorlastClaim.add(30 days);
        }
        uint256 currentPercentage = teamShare.mul(advisorMonthlyPercent).div(1e3);
    ...
    function launchAvalaunch() external onlyOwner {
        require(launchTime.add(90 days) < block.timestamp,"Token is locked");
        launchTime = block.timestamp;
        uint256 tokens = saleShare.mul(AvalaunchSalePercentage).div(1e3);
