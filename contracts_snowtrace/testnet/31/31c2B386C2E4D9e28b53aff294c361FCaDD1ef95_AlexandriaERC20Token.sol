...
contract TWAPOracleUpdater is ERC20Permit, VaultOwned {
  ...
  function addTWAPSource( address newTWAPSourceDexPool_ ) external onlyOwner() {
    require( _dexPoolsTWAPSources.add( newTWAPSourceDexPool_ ), "AlexandriaERC20TOken: TWAP Source already stored." );
    emit TWAPSourceAdded( newTWAPSourceDexPool_ );
  }

  function removeTWAPSource( address twapSourceToRemove_ ) external onlyOwner() {
    require( _dexPoolsTWAPSources.remove( twapSourceToRemove_ ), "AlexandriaERC20TOken: TWAP source not present." );
...
contract AlexandriaERC20Token is Divine {

  using SafeMath for uint256;

    constructor() Divine("Alexandria", "ALX", 9) {
    }

    function deposit(address account_, uint256 amount_) external onlyVault() {
