...
interface ICirculatingNRK {
    function NRKCirculatingSupply() external view returns (uint);
}

contract pNRKExchange {
    ...
    address public immutable pNRK;
    address public immutable NRK;
    address public immutable treasury;
    address public immutable circulatingNRKContract;
    ...
    constructor(address _pNRK, address _nrk, address _treasury, address _circulatingNRKContract) {
        owner = msg.sender;
        require(_pNRK != address(0));
        pNRK = _pNRK;
        require(_nrk != address(0));
        NRK = _nrk;
        require(_treasury != address(0));
        treasury = _treasury;
        require(_circulatingNRKContract != address(0));
        circulatingNRKContract = _circulatingNRKContract;
    }
    ...
    // Allows wallet to redeem pNRK for NRK
    function exerciseWithReserves(address token, uint _amount) external returns (bool) {
        require(supportTokens[token], 'Token not support');
        ...
        IPNRK(pNRK).burnFrom(msg.sender, _amount);

        IERC20(token).approve(treasury, _amount);
        uint NRKToSend = ITreasury(treasury).deposit(_amount, token, 0);

        terms[msg.sender].claimed = info.claimed.add(_amount);

        IERC20(NRK).safeTransfer(msg.sender, NRKToSend);
    ...
    function redeemable(Term memory _info) internal view returns (uint) {
        return (ICirculatingNRK(circulatingNRKContract).NRKCirculatingSupply().mul(_info.percent).mul(1000)).sub(_info.claimed);
    }
    ...
    //manage support token
    function manageSupportToken(address token) external {
        require(msg.sender == owner, "access deny");
        supportTokens[token] = !supportTokens[token];
    }
}