...
contract ThermoChainlinkOracle is PriceOracle {
    using SafeMath for uint;
    uint public constant USX_VALUE = 1e18;
    ...
    function getUnderlyingPrice(SToken sToken) public view returns (uint) {
        ...
        } else if (compareStrings(symbol, "USX")) {
            return USX_VALUE;
        } else if (compareStrings(symbol, "TMX")) {
    ...
    function getPrice(SToken sToken) internal view returns (uint price) {
        BEP20Interface token = BEP20Interface(SBep20(address(sToken)).underlying());
