...
contract MetronionSale is IMetronionSale, Whitelist, TokenWithdrawable {
    ...
    uint256 public constant SALE_PRICE = 1 * 10**18; // 1 AVAX
    ...
    constructor(
        ...
        require(_publicTime > _privateTime, "invalid public and private time");
        require(_endTime > _publicTime, "invalid public and end time");
    ...
    function buy(uint64 amount) external payable override {
        ...
        require(!buyer.isContract(), "MetronionSale: only EOA");
    ...
    function _validateAndUpdateBuy(address buyer, uint64 amount) internal {
        uint256 maxSupply = nftContract.versionById(versionId).maxSupply;
        // buy amount cannot exceed max supply

        _saleRecords.totalSold += amount;
        require(_saleRecords.totalSold <= maxSupply, "MetronionSale: exceed buy amount");
        ...
        if (msg.sender == owner()) {
            ...
            if (timestamp < _saleConfig.endTime) {
                require(_saleRecords.ownerBought <= CAP_OWNER_INITIAL_MINT, "MetronionSale: exceed owner cap");
            }
            emit OwnerBought(buyer, versionId, totalPaid);
            return;
        }

        UserRecord storage userRecord = _userRecords[buyer];
        ...
        if (timestamp >= _saleConfig.privateTime && timestamp < _saleConfig.publicTime) {
            ...
            userRecord.privateBought += amount;
        ...
        if (timestamp >= _saleConfig.publicTime && timestamp < _saleConfig.endTime) {
            ...
            userRecord.publicBought += amount;
