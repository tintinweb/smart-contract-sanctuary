...
contract RouterFacet is IRouter, ITeleportDApp {
    /**
     *  @notice Initialize the router with proper data
     */
    ...
    // TODO isn't uint256 more efficient when not packing
    // TODO I don't think we need to pass transmissionReceiver_, we already know it from the whitelist
    // IF it is 1:1 chainId => routerAddress that is. If we want to receive from multiple
    // contracts in one chain, we'd need a separate mapping for the actual receiver, or leave it to the FE.

    /**
     *  @notice Send tokens to another chain via Teleport.
     */
    function egress(uint8 targetChain_, bytes calldata transmissionReceiver_,
        CollectionWithTokens[] calldata assets, bytes calldata receiver_)
        external override payable
    {
        LibRouter.Storage storage rs = LibRouter.routerStorage();
        bytes[] memory actions = new bytes[](assets.length);

        for (uint256 i = 0; i < assets.length; i++) {
            LibRouter.NativeCollectionWithChainId memory nativeCollection = wrappedToNativeCollection(assets[i].collection);

            if (nativeCollection.chainId == 0) {
                emit LockMint(targetChain_, assets[i].collection, assets[i].tokenIds, receiver_);

                actions[i] = abi.encode(
                        IRouter.TargetAction.Mint, _addressToBytes(assets[i].collection),
                        _lockMint(assets[i].collection, assets[i].tokenIds)
                    );
            }
            else {
                if (nativeCollection.chainId == targetChain_) {
                    emit BurnUnlock(targetChain_, assets[i].collection, assets[i].tokenIds, receiver_);

                    actions[i] = abi.encode(
                            IRouter.TargetAction.Unlock, nativeCollection.contractAddress,
                            _burnUnlock(assets[i].collection, assets[i].tokenIds)
                        );
                }
                else {
                    emit BurnMint(targetChain_, assets[i].collection, assets[i].tokenIds, receiver_);

                    actions[i] = abi.encode(
                            IRouter.TargetAction.Mint, nativeCollection.contractAddress,
                            _burnMint(nativeCollection.chainId, assets[i].collection, assets[i].tokenIds)
                        );
                }
            }
        }
        ...
        // TODO structs are more expensive, but how much? payload is ugly
        uint256 feeOwed = ITeleport(LibRouter.routerStorage().teleportAddress).serviceFee();
        require(msg.value >= feeOwed, "Router: insufficient fee");
        ITeleport(rs.teleportAddress).transmit{value: msg.value}(
                targetChain_, transmissionReceiver_, rs.dAppId,
                payload
            );
    }

    /**
     *  @return Payload for sending a native token to a non-native chain
     */
    ...
    /**
     *  @return Payload for sending a non-native token to its native chain
     */
    ...
    /**
     *  @return Payload for sending a non-native token to a non-native chain
     */
        ...
        external override
    {
        ...
        require(rs.allowedSender[sourceChainId_][transmissionSender_], "Router: unknown sender");
    ...
    {
        WrappedERC721 deployedCollection = new WrappedERC721(collectionName_, collectionSymbol_);
        LibRouter.setCollectionMappings(sourceChain_, nativeCollection_, address(deployedCollection));
        emit WrappedCollectionDeployed(sourceChain_, nativeCollection_, address(deployedCollection));

        return address(deployedCollection);
    }

    /**
     *  @return The address of the wrapped counterpart of `nativeToken` in the current chain
     */
    function nativeToWrappedCollection(uint8 _chainId, bytes memory _nativeCollection) public view override returns (address) {
        return LibRouter.routerStorage().nativeToWrappedCollection[_chainId][_nativeCollection];
    }

    /**
     *  @return The chainId and address of the original token
     */
    function wrappedToNativeCollection(address _wrappedCollection) public view override returns (LibRouter.NativeCollectionWithChainId memory) {
        return LibRouter.routerStorage().wrappedToNativeCollection[_wrappedCollection];
    }

    function _bytesToAddress(bytes memory bys) internal pure returns (address addr) {
        require(bys.length == 20, "Router: wrong address length");
        assembly {
...
contract WrappedERC721 is ERC721Enumerable, Pausable, Ownable {
    ...
    // TODO do we really want to do this optional metadata stuff, since we can't guarantee parity anyway?
    // TODO can't the mapping be public and shadow the inherited supportsInterface?
    ...
    constructor(
        ...
        // _supportedInterfaces[type(IERC721Metadata).interfaceId] = supportsMetadata;
    }

    // TODO name, symbol setters are also nice to have for when AB wants to change naming schema
    // but they are private, so some storage layout hacks are needed
        ...
        external onlyOwner
    {
        ...
        for (uint256 i = 0; i < tokenIds_.length; i++) {
            // mints tokens to unknown user supplied address, so we are being safe
...
interface IRouter {
    ...
    function egress(
        uint8 targetChain_, bytes calldata transmissionReceiver_,
...
interface ITeleport {
    ...
    event Transmission(address transmissionSender, uint8 targetChainId, bytes transmissionReceiver, bytes32 dAppId, bytes payload);
...
library LibRouter {
    ...
    struct Storage {
        ...
        // Maps who is allowed to send us teleport messages
        // source chainId => (source address => isAllowed)
        mapping(uint8 => mapping(bytes => bool)) allowedSender;
