...
pragma solidity >=0.7.0 <0.9.0;
contract NFT_Minter is ERC721Enumerable, Ownable, ERC2981 {
    using Strings for uint256;
    ...
    function mint(uint256 _mintAmount) public payable {
        ...
        if (saleState == 1) {
            ...
            require(
                minter_balances[msg.sender] + _mintAmount <=
        ...
        for (uint256 i = 1; i <= _mintAmount; i++) {
            ...
            if (currentId > presale + premint) {
                totalCost = totalCost + cost;
            } else {
                totalCost = totalCost + presaleCost;
            }
            _safeMint(msg.sender, currentId);
            minter[currentId] = msg.sender;
            minter_tokens[msg.sender][minter_balanceOf(msg.sender)] = currentId;
    ...
    function _beforeTokenTransfer(
        ...
        if (totalSupply() > tokenId && tokenId > premint) claimReward(tokenId);
        super._beforeTokenTransfer(from, to, tokenId);
    }

    function currentRate() public view returns (uint256) {
        if (totalSupply() == 0) return 0;
        return reflectionBalance / (totalSupply() - premint);
    }
    ...
    function claimReward(uint256 tokenId) public {
        ...
        require(tokenId > premint, "Cannot claim rewards on pre-minted items");
    ...
    function reflectDividend(uint256 amount) private {
        ...
        totalDividend = totalDividend + (amount / (totalSupply() - premint));
    }
