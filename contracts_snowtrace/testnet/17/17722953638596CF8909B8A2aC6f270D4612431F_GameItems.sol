...
contract GameItems is ERC1155  {
  ...
  event createGamePropeEvent(address indexed  player,uint256 tokenId,string  name,uint64 propeType,uint64 level,string  tokenURI);
    ...
    */
      function createGamePrope(address player,string memory name,string memory tokenUrl,uint64 propeType,uint64 level)
        onlyOwner
        public
        ...
        uint256 newItemId = _tokenIds.current();
         ...
         emit createGamePropeEvent(player,newItemId,name,propeType,level,tokenUrl);
        return newItemId;
    }
    ...
    */
    function batchCreateGamePrope(address player,string[] memory name,string[] memory tokensUrl,uint64 length,uint64[] memory propeType,uint64[] memory level)
     public
     onlyOwner
     returns(uint256[] memory tokendIds) {

        require(length == tokensUrl.length  && propeType.length == tokensUrl.length && name.length == propeType.length && propeType.length == level.length,"Array lengths do not match");
        ...
        for(uint i =0;i< length;i++){
          ...
          emit createGamePropeEvent(player,newItemId,name[i],propeType[i],level[i],tokensUrl[i]);
        }
