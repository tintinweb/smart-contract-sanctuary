...
contract IndexTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SINDEX }
    ...
    address public sINDEX;
    uint public sINDEXQueue; // Delays change to sINDEX address
    ...
    /**
        @notice allow approved address to deposit an asset for INDEX
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint INDEX needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( INDEX ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn INDEX for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IINDEXERC20( INDEX ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sINDEX ).balanceOf( msg.sender ); // Can only borrow against sINDEX held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with INDEX
        @param _amount uint
     */
    function repayDebtWithINDEX( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IINDEXERC20( INDEX ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, INDEX, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( INDEX ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( INDEX ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns INDEX valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match INDEX decimals
            value_ = _amount.mul( 10 ** IERC20( INDEX ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SINDEX ) { // 9
            sINDEXQueue = block.number.add( blocksNeededForQueue );
        } else return false;
