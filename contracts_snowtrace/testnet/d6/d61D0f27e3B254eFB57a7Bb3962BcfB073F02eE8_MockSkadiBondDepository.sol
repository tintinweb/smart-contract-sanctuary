...
contract MockSkadiBondDepository is Ownable {
  ...
  struct Bond {
    uint256 payout; // SKI remaining to be paid
    uint256 vesting; // Blocks left to vest
    uint256 lastBlock; // Last interaction
    uint256 pricePaid; // In MIM, for front end viewing
  }
  ...
  function deposit(
    ...
    /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) SKI
         */
    ...
    if (fee != 0) {
      ...
      IERC20(SKI).safeTransfer(DAO, fee);
    }
  ...
  function debtRatio() public view returns (uint256 debtRatio_) {
    uint256 supply = IERC20(SKI).totalSupply();
