...
contract SVECore is ERC721, Ownable, ReentrancyGuard {
    ...
    mapping(uint256 => SpaceMarvel) spaceMarvels;
    uint256 breedMax;
    ...
    function born(address _toAddress, uint256 _gene) public nonReentrant {
        ...
        spaceMarvels[_nftId].gene=_gene;
        spaceMarvels[_nftId].breedCount=0;
        spaceMarvels[_nftId].bornAt=block.timestamp;
    ...
    function evolve(uint256[] memory _nftIds, uint256 _newGene) external nonReentrant {
        ...
        spaceMarvels[_newNftId] = SpaceMarvel(
            _nftIds,
            _newGene,
            0,
            block.timestamp
    ...
    function breed(
        ...
        if (breedMax != 0) {
            require(
                spaceMarvels[_nftId1].breedCount < breedMax,
                "NFT 1 breed max"
            );
            require(
                spaceMarvels[_nftId2].breedCount < breedMax,
                "NFT 2 breed max"
            );
        }
        ...
        _mint(_toAddress, _nftId);
        // spaceMarvels[_nftId] = SpaceMarvel(
        //     [_nftId1,_nftId2],
        //     _gene,
        //     0,
        //     block.timestamp
        // );
        spaceMarvels[_nftId].parents.push(_nftId1);
        spaceMarvels[_nftId].parents.push(_nftId2);
        spaceMarvels[_nftId].gene=_gene;
        spaceMarvels[_nftId].breedCount=0;
        spaceMarvels[_nftId].bornAt=block.timestamp;

        spaceMarvels[_nftId1].breedCount += 1;
        spaceMarvels[_nftId2].breedCount += 1;
        ...
        )
    {
        return (
            ownerOf(_nftId),
            spaceMarvels[_nftId].parents,
            spaceMarvels[_nftId].gene,
            spaceMarvels[_nftId].breedCount,
            spaceMarvels[_nftId].bornAt
        );
    }
}
