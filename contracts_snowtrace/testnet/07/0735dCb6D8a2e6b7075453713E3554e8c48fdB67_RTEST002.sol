...
// File: contracts/IRACEXFENFT.sol
...
abstract contract ERC2981RaceXNFKeyRoyalties is ERC165, IERC2981Royalties {
...
contract RTEST002 is ERC721Enumerable, ERC2981RaceXNFKeyRoyalties, IRACEXNFKEYNFT, Ownable {
  using Strings for uint256;
  ...
  uint256 public maxDevSupply = 100;
  ...
  address private payments = 0x8a6a4205F1e5caf27e74f89A30733d01D80b2231;
  ...
  constructor(
  ) ERC721("RTEST002", "RTEST002") {
    setBaseURI("https://racextest.s3.us-east-2.amazonaws.com/");
    _baseTokenURI = "";
    payments = payable(msg.sender);
    _ipfsBaseURI = "https://racextest.s3.us-east-2.amazonaws.com/";
  ...
  // public
  function mint(address _to, uint256 _mintAmount, uint256 _discount) public payable {
    ...
    require(_discount < (cost * _mintAmount) / 4, "Your discount applied is too high");
    ...
    (bool sent, ) = payable(royaltyContractAddress).call{value: reflectAmount}("");
    require(sent);
    splitBalance(reflectAmount);
  ...
  function devMint(address _to, uint256 _mintAmount) public onlyOwner{
    ...
    require(msg.sender==owner(), "You are not approved for this transaction");
    for (uint256 i = 1; i <= _mintAmount; i++) {
        _tokenIds.increment();
        uint256 newTokenId = _tokenIds.current();
      _safeMint(_to, newTokenId);
  ...
  function tokenMinter(uint256 tokenId) public view returns(address){
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(ERC721Enumerable, ERC2981RaceXNFKeyRoyalties)
        returns (bool)
    {
        return
            ERC721.supportsInterface(interfaceId) ||
            ERC2981RaceXNFKeyRoyalties.supportsInterface(interfaceId);
    }
...
//only owner
 ...
 function setPayments(address _payments) public onlyOwner {
  ...
  function splitBalance(uint256 amount) public override returns(bool success){
