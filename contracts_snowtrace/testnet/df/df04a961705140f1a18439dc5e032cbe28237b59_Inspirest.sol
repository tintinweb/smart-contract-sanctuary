...
contract Inspirest is Context, IERC20, Ownable {
    ...
    event PresaleWalletStatus(address user);
    ...
    constructor ()  {
        ...
        tokenOwner = 0xF77A5EAeEB5b31434811459Ce3896a41301B4925;
		App_Fees_Address = 0x4eF9A651F8656DEf8454178406eEae16FB7Ca458;
		_owner = tokenOwner;
    ...
    function setPresaleWallet(address user) external onlyOwner {
        if (!preSaleUsers[user]) {
            preSaleUsers[user] = true;
           emit PresaleWalletStatus(user);
            }
    
    }

    function isPresaleWallet(address user) private  returns(bool) {
        if (preSaleUsers[user]) {
                if(block.timestamp <= 1649924449){ // 14 april 2022 unlocked 1649924449
                    return true;
            }else{
            delete preSaleUsers[user];  
            return false;              
            }
    }else{
        return false;
    ...
    function getPresaleWalletStatus(address user) public view returns(bool) {
        return preSaleUsers[user];
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        require(isPresaleWallet(sender) == false, "PRESALE:Transfer lock will be Auto unlocked on April 14, 2022.");
