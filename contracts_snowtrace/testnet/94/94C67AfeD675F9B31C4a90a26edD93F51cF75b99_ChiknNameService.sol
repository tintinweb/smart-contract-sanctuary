...
import "./ChickenRunV5.sol";
import "./EggV2.sol";

import "hardhat/console.sol";

contract ChiknNameService is Ownable {
    address public EGG_CONTRACT; // egg contract
    address public CHIKN_CONTRACT; // chikn nft contract

    // CNS variables
    uint256 public CNS_BASE_PRICE = 30000000000000000000; // number of egg required to change the name
    mapping(bytes32 => bool) public cnsAlreadyRegisted;
    uint256 public CNS_MAX_LENGTH = 32;
    ...
    // tokenId to personality
    mapping(uint256 => Personality) public personalityInfo;
    uint256 public PERSONALITY_BASE_PRICE = 50000000000000000000; // number of egg required to change the personality
    uint256 public LORE_MAX_LENGTH = 140;
    uint256 public FLAVOUR_MAX_LENGTH = 70;

    // RNS vairable
    uint256 public RNS_BASE_PRICE = 70000000000000000000; // number of egg required to change the Roost name
    mapping(address => string) public rnsInfo;
    mapping(bytes32 => bool) public rnsAlreadyRegisted;
    uint256 public RNS_MAX_LENGTH = 32;

    // address tracker for RNS
    address[] public _allRNSName;
    mapping(address => uint256) private _allRNSNameIndex;

    // events
    event CNSChange(uint256 tokenId, string previousName, string name);
    event RNSChange(address holder, string previousName, string name);
    event PersonalityChange(uint256 tokenId, string lore, string flavour);

    constructor(address _eggContract, address _chiknContract) {
    ...
    function cnsNameChange(uint256 tokenId, string memory name) external {
        // lower case all the name so people cant have similar name
        bytes32 hashedInput = isCNSNameAvailable(name);
        ChickenRunV5 x = ChickenRunV5(CHIKN_CONTRACT);
        ...
        require(available >= CNS_BASE_PRICE, "Not enough egg");

        string memory currentChiknName = x.cns(tokenId);
        bytes memory byteName = bytes(currentChiknName);
        console.log("Current chikn name", currentChiknName);
        if (byteName.length > 0) {
            console.log("Chikn have name before", currentChiknName);
            // assume chikn have already been name
            bytes32 oldName = keccak256(
                abi.encodePacked(_toLower(currentChiknName))
            );
            delete cnsAlreadyRegisted[oldName];
        }
        // register the unique chikn name
        cnsAlreadyRegisted[hashedInput] = true;
        // burn the egg
        eggContract.burnEggs(msg.sender, CNS_BASE_PRICE);
        // and update the chikn contract
        x.changeName(tokenId, name);
        emit CNSChange(tokenId, currentChiknName, name);
    }

    // changing the Chikn Personality
    function personalityChange(
        uint256 tokenId,
        string memory lore,
        string memory flavour
    ) external {
        require(isPersonalityFit(lore, flavour), "Not valid input");
        ChickenRunV5 x = ChickenRunV5(CHIKN_CONTRACT);
        require(x.ownerOf(tokenId) == msg.sender, "Not owner");
        EggV2 eggContract = EggV2(EGG_CONTRACT);
        uint256 available = eggContract.balanceOf(msg.sender);
        require(available >= PERSONALITY_BASE_PRICE, "Not enough egg");
        // get personality
        Personality memory p = personalityInfo[tokenId];
        p.lore = lore;
        p.flavour = flavour;
        // register the personality to chikkn id
        personalityInfo[tokenId] = p;
        // burn the egg
        eggContract.burnEggs(msg.sender, PERSONALITY_BASE_PRICE);
        emit PersonalityChange(tokenId, lore, flavour);
    }

    // roost naming service

    function rnsNameChange(string memory name) external {
        // lower case all the name so people cant have similar name
        bytes32 hashedInput = isRNSNameAvailable(name);
        EggV2 eggContract = EggV2(EGG_CONTRACT);
        uint256 available = eggContract.balanceOf(msg.sender);
        require(available >= RNS_BASE_PRICE, "Not enough egg");
        string memory currentRoostName = rnsInfo[msg.sender];
        bytes memory byteName = bytes(currentRoostName);

        uint256 addressExist = _allRNSNameIndex[msg.sender];
        if (addressExist == 0) {
            // new RNS user to tracker
            _allRNSNameIndex[msg.sender] = _allRNSName.length;
            _allRNSName.push(msg.sender);
        }
        console.log("Current Roost name", currentRoostName);
        if (byteName.length > 0) {
            // assume there is a previous name, remove them from the duplicate list
            bytes32 oldName = keccak256(
                abi.encodePacked(_toLower(currentRoostName))
            );
            delete rnsAlreadyRegisted[oldName];
        }
        // register the unique roost name
        rnsAlreadyRegisted[hashedInput] = true;
        // burn the egg
        eggContract.burnEggs(msg.sender, RNS_BASE_PRICE);
        // update the roost name
        rnsInfo[msg.sender] = name;
        emit RNSChange(msg.sender, currentRoostName, name);
    }

    // check name avaiable (cns)
    function isCNSNameAvailable(string memory _name)
        public
        view
        returns (bytes32)
    {
        bytes32 hashedInput = keccak256(abi.encodePacked(_toLower(_name)));
        require(keccak256(abi.encodePacked("")) != hashedInput, "Empty name!");
        require(
            cnsAlreadyRegisted[hashedInput] == false,
            "Name already registered"
        );
        // bytes memory inputBytes = bytes(_name);
        require(utfStringLength(_name) <= CNS_MAX_LENGTH, "Name too long");
        return hashedInput;
    }

    function isPersonalityFit(string memory _lore, string memory _flavour)
        public
        view
        returns (bool)
    {
        bytes32 loreHashedInput = keccak256(abi.encodePacked(_toLower(_lore)));
        require(
            keccak256(abi.encodePacked("")) != loreHashedInput,
            "Empty name!"
        );
        // flavour can be empty
        require(utfStringLength(_lore) <= LORE_MAX_LENGTH, "Lore too long");
        require(
            utfStringLength(_flavour) <= FLAVOUR_MAX_LENGTH,
            "Flavour too long"
        );
        return true;
    }

    // check name available (rns)
    function isRNSNameAvailable(string memory _name)
        public
        view
        returns (bytes32)
    {
        ...
        require(keccak256(abi.encodePacked("")) != hashedInput, "Empty name!");
        require(
            rnsAlreadyRegisted[hashedInput] == false,
            "Name already registered"
        );
        // bytes memory inputBytes = bytes(_name);
        require(utfStringLength(_name) <= RNS_MAX_LENGTH, "Name too long");
        return hashedInput;
    }

    /**
     * Admin : util get total RNS addresses
     */
    function totalRNS() public view returns (uint256) {
        return _allRNSName.length;
    }

    // Admin function to change the cost of the rns
    function updateBasePrice(uint256 _RNSvalue, uint256 _CNSValue, uint256 _PersonalityValue)
        external
        onlyOwner
    {
        RNS_BASE_PRICE = _RNSvalue;
        CNS_BASE_PRICE = _CNSValue;
        PERSONALITY_BASE_PRICE = _PersonalityValue;
    }

    function updateNameLength(uint256 _rnsMaxLength, uint256 _cnsMaxLength, uint256 _loreMaxLength, uint256 _flavourMaxLength)
        external
        onlyOwner
    {
        RNS_MAX_LENGTH = _rnsMaxLength;
        CNS_MAX_LENGTH = _cnsMaxLength;
        LORE_MAX_LENGTH = _loreMaxLength;
        FLAVOUR_MAX_LENGTH = _flavourMaxLength;
    }
