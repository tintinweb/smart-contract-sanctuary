...
contract ASS is Context, IERC20, Ownable {
  ...
  string private _name = "Alien Space Ship";
  ...
  constructor(
    ...
    uint256 lp = blackHole.mul(70).div(100);

    // 10% to the DAO
    uint256 dao = blackHole.div(10);

    // 1% royalty
    uint256 royalty = blackHole.div(400);
        
    // ratio of reserve to total supply
    uint256 rate = getRate();
    
    _reserveTokenBalance[burn] = blackHole.mul(rate);
    _reserveTokenBalance[_marketingWallet] = presale.mul(rate);
  ...
  /// @notice Transfers 4.5% of every transaction to the ASS DAO
  ...
  function applyExternalTransactionTax(
    ...
    _reserveTokenBalance[royaltyAddress] = _reserveTokenBalance[royaltyAddress].add(reserveValues.reserveTransferAmountburnburnRoyalty);
    emit Transfer(
      sender,
      royaltyAddress,
  ...
  function _getTValues(
    ...
    // 4.5% fee to the ASS DAO contract.
    transactionValues.daoTax = transferAmount.mul(9).div(200);
    // 0.25% burn.
    transactionValues.burnburnRoyalty = transferAmount.div(400);
    // Net transfer amount to recipient
    transactionValues.netTransferAmount = transferAmount.sub(transactionValues.transactionFee).sub(transactionValues.daoTax).sub(transactionValues.burnburnRoyalty);
  ...
  function _getRValues(
    ...
    reserveValues.reserveTransferAmountburnburnRoyalty = transactionValues.burnburnRoyalty.mul(currentRate);
    reserveValues.reserveTransferAmount = reserveValues.reserveAmount.sub(
      reserveValues.reserveFee
      ).sub(
        reserveValues.reserveTransferAmountDao
        ).sub(
          reserveValues.reserveTransferAmountburnburnRoyalty
        );
