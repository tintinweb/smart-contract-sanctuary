...
interface ILottery {
    function checkpoints(uint256) external returns (uint256);

    function deposits(uint256) external returns (address);
...
contract Points is Ownable {
    ...
    function getRedeemable(uint256 tokenId) public returns (uint256) {
    ...
    function firstDayOfMonth() internal view returns (uint) {
      uint timeNow = block.timestamp;
      _DateTime memory today = parseTimestamp(timeNow);
      return timeNow.sub((today.day).mul(DAY_IN_SECONDS)).sub((today.hour).mul(3600)).sub((today.minute).mul(60)).sub(today.second);
    }
