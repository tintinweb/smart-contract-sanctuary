...
contract GretaTreasury is GretaAccessControlled, ITreasury {
    /* ========== DEPENDENCIES ========== */
    ...
    /**
     * @notice allow approved address to deposit an asset for GRT
    ...
    function deposit(
        ...
        // mint GRT needed and store amount of rewards for distribution
        send_ = value.sub(_profit);
        GRT.mint(msg.sender, send_);
    ...
    /**
     * @notice allow approved address to burn GRT for reserves
     * @param _amount uint256
     * @param _token address
     */
    ...
    /**
     * @notice mint new GRT using excess reserves
    ...
    function mint(address _recipient, uint256 _amount) external override {
        ...
        GRT.mint(_recipient, _amount);
        emit Minted(msg.sender, _recipient, _amount);
    }

    /**
     * DEBT: The debt functions allow approved addresses to borrow treasury assets
     * or GRT from the treasury, using sGRT as collateral. This might allow an
     * sGRT holder to provide GRT liquidity without taking on the opportunity cost
    ...
    function incurDebt(uint256 _amount, address _token) external override {
        uint256 value;
        if (_token == address(GRT)) {
            require(permissions[STATUS.GRTDEBTOR][msg.sender], notApproved);
            value = _amount;
        } else {
        ...
        sGRT.changeDebt(value, msg.sender, true);
        require(sGRT.debtBalances(msg.sender) <= debtLimit[msg.sender], "limit");
        totalDebt = totalDebt.add(value);

        if (_token == address(GRT)) {
            GRT.mint(msg.sender, value);
            grtDebt = grtDebt.add(value);
        } else {
            totalReserves = totalReserves.sub(value);
            IERC20(_token).safeTransfer(msg.sender, _amount);
        }
    ...
    function repayDebtWithReserve(uint256 _amount, address _token) external override {
        ...
        IERC20(_token).safeTransferFrom(msg.sender, address(this), _amount);
        uint256 value = tokenValue(_token, _amount);
        sGRT.changeDebt(value, msg.sender, false);
    ...
    /**
     * @notice allow approved address to repay borrowed reserves with GRT
     * @param _amount uint256
     */
    function repayDebtWithGRT(uint256 _amount) external {
        require(permissions[STATUS.RESERVEDEBTOR][msg.sender] || permissions[STATUS.GRTDEBTOR][msg.sender], notApproved);
        GRT.burnFrom(msg.sender, _amount);
        sGRT.changeDebt(_amount, msg.sender, false);
        totalDebt = totalDebt.sub(_amount);
        grtDebt = grtDebt.sub(_amount);
        emit RepayDebt(msg.sender, address(GRT), _amount, _amount);
    }
    ...
    function enable(
        ...
        require(timelockEnabled == false, "UqT");
        if (_status == STATUS.SGRT) {
            sGRT = IsGRT(_address);
        } else {
    ...
    function disable(STATUS _status, address _toDisable) external {
        require(msg.sender == authority.governor() || msg.sender == authority.guardian(), "gvrnr-grdn");
    ...
    function queueTimelock(
        ...
        require(timelockEnabled == true, "Tl enable");
    ...
    function execute(uint256 _index) external {
        ...
        require(!info.nullify, "nullified");
        require(!info.executed, "executed");
        require(block.number >= info.timelockEnd, "Tl incomplete");

        if (info.managing == STATUS.SGRT) {
            // 9
            sGRT = IsGRT(info.toPermit);
        } else {
    ...
    function disableTimelock() external onlyGovernor {
        require(timelockEnabled == true, "tl disabled");
    ...
    function excessReserves() public view override returns (uint256) {
        return totalReserves.sub(GRT.totalSupply().sub(totalDebt));
    }

    /**
     * @notice returns GRT valuation of asset
     * @param _token address
     * @param _amount uint256
     * @return value_ uint256
     */
    function tokenValue(address _token, uint256 _amount) public view override returns (uint256 value_) {
        value_ = _amount.mul(10**IERC20Metadata(address(GRT)).decimals()).div(10**IERC20Metadata(_token).decimals());
    ...
    function baseSupply() external view override returns (uint256) {
        return GRT.totalSupply() - grtDebt;
    }
}
...
interface IsGRT is IERC20 {
    function rebase( uint256 grtProfit_, uint epoch_) external returns (uint256);
...
abstract contract GretaAccessControlled {

    /* ========== EVENTS ========== */

    event AuthorityUpdated(IGretaAuthority indexed authority);

    string UNAUTHORIZED = "UNAUTHORIZED"; // save gas

    /* ========== STATE VARIABLES ========== */

    IGretaAuthority public authority;


    /* ========== Constructor ========== */

    constructor(IGretaAuthority _authority) {
    ...
    function setAuthority(IGretaAuthority _newAuthority) external onlyGovernor {
