...
// File contracts/airdrop/ArableAirDrop.sol
...
contract ArableAirDrop is Ownable, ReentrancyGuard {
    ...
    event AirDropRequested(address indexed addr);
    ...
    function requestAirDrop() external {
        emit AirDropRequested(msg.sender);
    }
    ...
    function _handleAirDrop(
        ...
        // require(!status[addr][mode], "Already airdropped!");
        // require(mode < 4, "Invalid mode");

        if (!status[addr][mode] && mode < 4 && addr != address(0)) {
            token.safeTransfer(addr, amount);

            status[addr][mode] = true;

            emit AirDropped(addr, amount, mode);
        }
    }
    ...
    function handleAirDrop(
        ...
        _handleAirDrop(addr, amount, mode);
    }

    function handleAirDrops(
        ...
        for (uint256 index = 0; index < addrs.length; index++) {
            _handleAirDrop(addrs[index], amounts[index], mode);
        }
    }
