...
contract Marketplace is ReentrancyGuard, Ownable {
    ...
    function createMarketItem(
        ...
        require(
            activeItems[_nftContract][_tokenId] == false,
        ...
        activeItems[_nftContract][_tokenId] = true;
        ...
        nonReentrant
    {
        ...
        require(
            idToMarketItem[_itemId].auction == false,
            "This item is only for sell on auction"
        );
        require(
            msg.value == price,
            "Please submit the asking price in order to complete the purchase"
        );
        idToMarketItem[_itemId].seller.transfer(
            msg.value - ((msg.value * commision) / 100)
        );
        IERC721(_nftContract).transferFrom(address(this), msg.sender, tokenId);
        _itemsSold.increment();
        activeItems[_nftContract][tokenId] = false;
    ...
    function cancelMarketSale(address _nftContract, uint256 _itemId)
        public
        nonReentrant
    {
        uint256 tokenId = idToMarketItem[_itemId].tokenId;
    ...
    function bid(
        ...
        MarketItem storage auction = idToMarketItem[_itemId];
        require(!paused);
        require(
            msg.sender != idToMarketItem[_itemId].seller,
            "This item belongs to you."
        ...
        require(
            idToMarketItem[_itemId].auction == true,
            "This item is only for sell on auction"
        );
        if (bids[_nftContract][_tokenId][msg.sender] > 0) {
            (bool success, ) = msg.sender.call{
                value: bids[_nftContract][_tokenId][msg.sender]
            }("");
            require(success);
        }
    ...
    function executeSale(
        ...
        MarketItem storage auction = idToMarketItem[_tokenId];
        require(!paused);
        require(
        ...
        require(
            idToMarketItem[_itemId].auction == true,
            "This item is only for sell on market"
        );
        activeItems[_nftContract][_tokenId] = false;
    ...
    function getMarketItemDetails(uint256 _itemId)
        public
        view
    ...
    function setCommision(uint256 _newCommision) public onlyOwner {
