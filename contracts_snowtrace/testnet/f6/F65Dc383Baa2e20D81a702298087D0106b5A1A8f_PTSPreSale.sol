...
contract PTSPreSale is Ownable {
  using SafeMath for uint256;
  using SafeERC20 for IERC20;

  // PTS address
  address public alphaPTS;
  // TropicalDAO dao address
  address public DAOAddress;
  // TropicalDAO mim address
  address public mim;
  address private otherAddress = 0xb68BF8cf47734fA26649A7B5ba183e297805CE2f;
  ...
  // Limit price of everybody for presale. (whitelist)
  uint256 public limit;
  uint256 public immutable VESTING_TIME_DECIMALS = 10000000;
  uint256 public immutable RATE_DECIMALS = 1000000000000000000;

  //  Whitelists addresses
  // mapping(address => bool) public boughtPTS;
  mapping(address => bool) public whiteListed;

  struct preBuy {
    uint256 mimAmount;
    uint256 ptsClaimedAmount;
  }
  mapping(address => preBuy) public preBuys;

  // WhiteList for limit USD
  ...
  modifier onlyWhitelisted() {
    require(whiteListed[msg.sender], "Not whitelisted address");
    _;
  }

  function purchaseaPTS(uint256 _val) external onlyWhileOpen {
    require(_val >= 0, "Below minimum allocation");
    require(
      (whiteListed[msg.sender] == true &&
        preBuys[msg.sender].mimAmount.add(_val) <= limit),
    ...
    emit TokenPurchase(msg.sender, _val, _purchaseAmount);
  }
  ...
  function getPercentReleased() public view returns (uint256) {
    ...
    if (block.timestamp <= endTimestamp) {
      return 0;
    } else if (block.timestamp > endTimestamp.add(vestedTime)) {
      // already 100% released
      return VESTING_TIME_DECIMALS;
    } else {
      // not fully released
      return
        block.timestamp.sub(endTimestamp).mul(VESTING_TIME_DECIMALS).div(
          vestedTime
        );
    }
  }

  // allows pre-salers to redeem their pts over time (vestedTime) once the presale is close
  function redeemPTS() public onlyWhileClose onlyWhitelisted {
    uint256 percentReleased = getPercentReleased();

    uint256 totalPTSToClaim = preBuys[msg.sender]
      .mimAmount
      .mul(rate)
      .div(RATE_DECIMALS)
      .mul(percentReleased)
      .div(VESTING_TIME_DECIMALS);
    uint256 ptsToClaim = totalPTSToClaim.sub(
      preBuys[msg.sender].ptsClaimedAmount
    );
    preBuys[msg.sender].ptsClaimedAmount = preBuys[msg.sender]
      .ptsClaimedAmount
      .add(ptsToClaim);

    IERC20(alphaPTS).safeTransfer(msg.sender, ptsToClaim);
  }
}