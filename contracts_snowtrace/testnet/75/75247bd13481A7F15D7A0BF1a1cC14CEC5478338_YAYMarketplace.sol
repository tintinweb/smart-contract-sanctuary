...
// File contracts/interface/IYAYMarketplace.sol



pragma solidity 0.8.10;

interface IYAYMarketplace {
...
// File contracts/YAYMarketplace.sol
...
contract YAYMarketplace is IYAYMarketplace, Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, ERC1155HolderUpgradeable, ERC721HolderUpgradeable {
    ...
    function createOrder(
        ...
        require(nftStandart != NFTStandart.NULL, "YAYMarketplace: wrong nft standart");
        require(tokenContract != address(0), "YAYMarketplace: zero contract address");
        if (nftStandart == NFTStandart.ERC721) {
            require(_whitelistErc721.contains(tokenContract), "YAYMarketplace: nft not registered");
            require(tokenAmount == 1, "YAYMarketplace: wrong token amount");
        } else if (nftStandart == NFTStandart.ERC1155) {
            require(_whitelistErc1155.contains(tokenContract), "YAYMarketplace: nft not registered");
            require(tokenAmount > 1, "YAYMarketplace: wrong token amount");
        }
        require(settlementToken != address(0), "YAYMarketplace: zero trade token address");
        require(_settlementTokens.contains(settlementToken), "YAYMarketplace: settlement token not registered");
        require(price > 0.0001 ether, "YAYMarketplace: wrong price");
        ...
        if (nftStandart == NFTStandart.ERC721) {
            IERC721Upgradeable(tokenContract).safeTransferFrom(msg.sender, address(this), tokenId);
        } else if (nftStandart == NFTStandart.ERC1155) {
            IERC1155Upgradeable(tokenContract).safeTransferFrom(msg.sender, address(this), tokenId, tokenAmount, "");
        }
    ...
    function cancelOrder(uint256 orderId) external override nonReentrant returns(bool success) {
        require(orderId < _totalOrders.current(), "YAYMarketplace: order does not exist");

        Order memory order = _orders[orderId];

        require(order.status == OrderStatus.OPEN, "YAYMarketplace: only for open orders");
        require(order.seller == msg.sender, "YAYMarketplace: sender is not the seller");
    ...
    function executeOrder(uint256 orderId) external override nonReentrant returns(bool success) {
        ...
        require(order.status == OrderStatus.OPEN, "YAYMarketplace: only for open orders");
        require(order.seller != msg.sender, "YAYMarketplace: not for seller");
    ...
    function fetchOrdersBy(
        ...
        return(
            _fetchWithPagination(_ordersByUserByStatus[byUser][byStatus], cursor, howMany)
        );
    }
    ...
    function setTradeFeePercent(uint256 newTradeFeePercent) external override nonReentrant onlyOwner returns(bool success) {
        _setTradeFeePercent(newTradeFeePercent);
        return(true);
    }
    ...
    function addSettlementToken(address erc20) external override nonReentrant onlyOwner returns(bool success) {
        require(IERC20MetadataUpgradeable(erc20).decimals() == 18, "YAYMarketplace: wrong erc20 decimals");
        _settlementTokens.add(erc20);
        return(true);
    }

    function removeSettlementToken(address erc20) external override nonReentrant onlyOwner returns(bool success) {
    ...
    function getSettlementTokens() external view override returns(address[] memory) {
        return(_settlementTokens.values());
    }

    // nft whitelist

    function addToWhitelistErc721(address erc721) external override nonReentrant onlyOwner returns(bool success) {
        _whitelistErc721.add(erc721);
        return(true);
    }

    function addToWhitelistErc1155(address erc1155) external override nonReentrant onlyOwner returns(bool success) {
        _whitelistErc1155.add(erc1155);
        return(true);
    }

    function removeFromWhitelistErc721(address erc721) external override nonReentrant onlyOwner returns(bool success) {
        _whitelistErc1155.remove(erc721);
        return(true);
    }

    function removeFromWhitelistErc1155(address erc1155) external override nonReentrant onlyOwner returns(bool success) {
    ...
    function withdrawERC20(address erc20, uint256 amount)
        external
        override
        nonReentrant
        onlyOwner
    ...
    function _setTradeFeePercent(uint256 newTradeFeePercent) internal {
        require(newTradeFeePercent >= 0 && newTradeFeePercent <= 100, "YAYMarketplace: wrong percent value");
