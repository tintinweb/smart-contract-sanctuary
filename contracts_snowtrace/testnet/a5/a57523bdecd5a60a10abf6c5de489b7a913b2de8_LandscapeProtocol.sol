...
contract LandscapeProtocol is ERC20 {
    ...
    uint[] private nodeRewards = [1, 2, 4];  // 2 times the daily rewards for a little node, a medium node, a big node
    uint public nodeCounter = 0;  // Number of Nodes 
    Node[] public SodaNodes; // List of the existing nodes
    ...
    uint public saleTokens = uint(totalSupply())/2;  // Amount of tokens to be sold during 1st phasis 
    

    constructor() ERC20("SodaCan", "SDCN") {
    ...
    function rewardsOf(uint _nodeId) public view returns(uint) {
        ...
        Node memory _node = SodaNodes[_nodeId];
        uint roundedTime = (block.timestamp- _node.creationTime) - (block.timestamp - _node.creationTime)%900;
        return roundedTime / 96 * nodeRewards[_node.size-1] / 2 * decimals();
    }
