...
contract ASS is Context, IERC20, Ownable {
    ...
    address public royaltyAddress = 0x000000000000000000000000000000000000dEaD;
    ...
    struct ReserveValues {
        ...
        uint256 reserveTransferAmountburnburnRoyalty;
    }
    ...
    struct TransactionValues {
        ...
        uint256 daoTax;
        uint256 burnburnRoyalty;
    }

    constructor(address _daoContractAddress, address _marketingWallet) {
        daoContractAddress = _daoContractAddress;

        // 50% backhole burn.
        uint256 blackHole = _totalSupply.div(2);
        // All tokens goto gnosis wallet
        uint256 lp = blackHole.mul(100).div(100);

        uint256 rate = getRate();

        _reserveTokenBalance[burn] = blackHole.mul(rate);

        _reserveTokenBalance[_marketingWallet] = lp.mul(rate);

        emit Transfer(address(0), burn, blackHole);

        emit Transfer(address(0), _marketingWallet, lp);
    }

    /// @notice Applies anti-bot sell tax.
    ...
    /// @notice Transfers 9% of every transaction to the ASS DAO
    /// @notice burns  0.5% of every transaction
    function applyExternalTransactionTax(
        ...
        _reserveTokenBalance[royaltyAddress] = _reserveTokenBalance[
            royaltyAddress
        ].add(reserveValues.reserveTransferAmountburnburnRoyalty);
        emit Transfer(
            sender,
            royaltyAddress,
            transactionValues.burnburnRoyalty
        );
    }
    ...
    {
        TransactionValues memory transactionValues;
        // .5% fee to all ASS Token holders.
        transactionValues.transactionFee = transferAmount.div(200);
        // 9% fee to the ASS DAO contract.
        transactionValues.daoTax = transferAmount.mul(9).div(100);
        // 0.5% burn.
        transactionValues.burnburnRoyalty = transferAmount.div(200);
        ...
        transactionValues.netTransferAmount = transferAmount
            ...
            .sub(transactionValues.burnburnRoyalty);
    ...
    function _getRValues(
        ...
        reserveValues.reserveTransferAmountburnburnRoyalty = transactionValues
            .burnburnRoyalty
            .mul(currentRate);
        reserveValues.reserveTransferAmount = reserveValues
            ...
            .sub(reserveValues.reserveTransferAmountburnburnRoyalty);
