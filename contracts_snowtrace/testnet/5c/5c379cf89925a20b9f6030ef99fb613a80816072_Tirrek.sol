...
contract Tirrek is Context, IERC20, Ownable {
    ...
    constructor ()  {
		...
		AddressofFees = 0x4eF9A651F8656DEf8454178406eEae16FB7Ca458;
		_owner = tokenOwner;
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        bool takeFee = true;
        if (AddressofFees == sender || AddressofFees == recipient || _isExcluded[recipient]) {
            takeFee = false;
        }
    ...
    function _sendToCharity(uint256 tCharity, address sender) private {
        uint256 currentRate = _getRate();
        uint256 rCharity = tCharity.mul(currentRate);
        _rOwned[AddressofFees] = _rOwned[AddressofFees].add(rCharity);
        _tOwned[AddressofFees] = _tOwned[AddressofFees].add(tCharity);
        emit Transfer(sender, AddressofFees, tCharity);
    }
