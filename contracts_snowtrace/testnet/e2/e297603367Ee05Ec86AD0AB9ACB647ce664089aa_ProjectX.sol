...
contract ProjectX is ERC20, Ownable, PaymentSplitter {
    ...
    function cashoutAllAndCreateNode(uint256 number) public {
        address sender = msg.sender;
        uint256 pendingClaimAmount = nodeRewardManagement.getPendingCashoutAllNodesReward(sender);

        uint256 requiredClaimAmount = nodeRewardManagement.nodePrice() * number;
        require(
            pendingClaimAmount >= requiredClaimAmount,
            "MANIA CSHT: low pending claim amount"
        );

        cashoutAll();
        createNodeWithTokens(number);
    }
