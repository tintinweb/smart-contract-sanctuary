...
contract TropicalBondDepository is Ownable {
    ...
    address public immutable Pts; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints PTS when receives principle
    ...
    struct Bond {
        uint payout; // PTS remaining to be paid
    ...
    function deposit( 
        ...
        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 PTS ( underflow protection )
        ...
        /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) PTS
         */
        ...
        if ( fee != 0 ) { // fee is transferred to dao 
            IERC20( Pts ).safeTransfer( DAO, fee ); 
        }
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            IERC20( Pts ).transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                IERC20( Pts ).approve( stakingHelper, _amount );
                IStakingHelper( stakingHelper ).stake( _amount, _recipient );
            } else {
                IERC20( Pts ).approve( staking, _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return IERC20( Pts ).totalSupply().mul( terms.maxPayout ).div( 100000 );
    }
    ...
    /**
     *  @notice calculate current ratio of debt to PTS supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns ( uint debtRatio_ ) {   
        uint supply = IERC20( Pts ).totalSupply();
    ...
    /**
     *  @notice calculate amount of PTS available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or PTS) to the DAO
     *  @return bool
     */
