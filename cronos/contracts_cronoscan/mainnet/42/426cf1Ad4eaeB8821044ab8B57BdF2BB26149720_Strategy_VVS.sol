...
abstract contract Strategy is Ownable, ReentrancyGuard, Pausable {
    ...
    function _unfarm(uint256 _wantAmt) internal virtual {
        if (isFarmReferral) {
            IPancakeswapFarm(farmContractAddress).deposit(pid, 0, rewardsAddress);
        }
        else {
...
// File: contracts/Strategy_VVS.sol
...
contract Strategy_VVS is Strategy {
    constructor(
    ...
    function calculateHarvestRewards() override external view returns (uint256) {
        uint256 amount = IPancakeswapFarm(farmContractAddress).pendingVVS(pid, address(this));
