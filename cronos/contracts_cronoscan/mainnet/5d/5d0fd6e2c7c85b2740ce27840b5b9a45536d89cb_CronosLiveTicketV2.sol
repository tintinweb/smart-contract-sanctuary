...
contract CronosLiveTicketV2 is ERC721r, Ownable, ReentrancyGuard {
    ...
    constructor(string memory _initBaseURI, string memory _hiddenMetadataUri, uint _cost, uint _maxMintAmountPerTx) ERC721r("Cronos Live Ticket V2", "tCLLV2", 20_0) {
  ...
  modifier mintCompliance(uint quantity) {
    require(quantity > 0 && quantity <= maxMintAmountPerTx, "Invalid mint amount!");
    require(totalSupply() + quantity <= 20_0, "Max supply exceeded!");
    _;
  }

  modifier mintPriceCompliance(uint quantity) {
    require(msg.value >= cost * quantity, "Insufficient funds!");
    _;
  }

  function whitelistMint(uint quantity, bytes32[] calldata _merkleProof) public payable mintCompliance(quantity) mintPriceCompliance(quantity) {
    ...
    _mintRandom(_msgSender(), quantity);
  }

  function mint(uint quantity) public payable mintCompliance(quantity) mintPriceCompliance(quantity) {
    require(!paused, "The contract is paused!");

    _mintRandom(_msgSender(), quantity);
  }
  
  function mintForAddress(uint quantity, address _receiver) public mintCompliance(quantity) onlyOwner {
    _mintRandom(_receiver, quantity);
  }
