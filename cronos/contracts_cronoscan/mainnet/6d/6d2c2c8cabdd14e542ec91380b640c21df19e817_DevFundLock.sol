...
contract DevFundLock is Ownable {
    using SafeMath for uint;

    // the HERD token
    IERC20 public herd;
    // dev address to receive HERD
    ...
    constructor(IERC20 _herd, address _devaddr, uint _withdraw_interval, uint _withdraw_half_threshold) public {
        require(address(_herd) != address(0) && _devaddr != address(0), "invalid address");
        herd = _herd;
        devaddr = _devaddr;
    ...
    function withdraw() public {
        ...
        require(block.number >= unlockBlock, "HERD locked");
        uint _amount = herd.balanceOf(address(this));
        require(_amount > 0, "zero HERD amount");
        uint amountReal = _amount;
        if (_amount > WITHDRAW_HALF_THRESHOLD) amountReal = _amount.div(2);
        lastWithdrawBlock = block.number;
        herd.transfer(devaddr, amountReal);
    }
    ...
    function setHerd(IERC20 _herd) public onlyOwner {
        require(address(_herd) != address(0), "can not be address(0)");
        herd = _herd;
    }
