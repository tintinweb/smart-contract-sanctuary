...
contract GenesisMultiRewardPool is IMultiRewardPool, ReentrancyGuard {
    ...
    modifier onlyOperator() {
        require(
            operator == msg.sender,
            "DarkoRewardPool: caller is not the operator"
        );
        _;
    }
    ...
    function getRewardPerSecond() external view override returns (uint256) {
        for (uint8 epochId = 0; epochId <= 3; ++epochId) {
            if (block.timestamp <= epochEndTimes[epochId])
                return epochDarkoPerSecond[epochId];
        }
        return 0;
    }
        ...
        for (uint8 epochId = 0; epochId <= 3; ++epochId) {
            if (block.timestamp <= epochEndTimes[epochId]) {
                ...
                _rewardPerSecondArr[1] = _rewardPerSecondArr[0]
                    .mul(REWARD_RATE_DBTC)
                    .div(REWARD_RATE_DENOMINATION);
                _rewardPerSecondArr[2] = _rewardPerSecondArr[0]
                    .mul(REWARD_RATE_DETH)
                    .div(REWARD_RATE_DENOMINATION);
                _rewardPerSecondArr[3] = _rewardPerSecondArr[0]
                    .mul(REWARD_RATE_DSGD)
                    .div(REWARD_RATE_DENOMINATION);
            }
        }
    }
        ...
        returns (uint256)
    {
        ...
        if (block.timestamp > pool.lastRewardTime && tokenSupply != 0) {
            ...
            uint256 _lightReward = _generatedReward.mul(pool.allocPoint).div(
                totalAllocPoint_
            );
            accDarkoPerShare = accDarkoPerShare.add(
                _lightReward.mul(1e18).div(tokenSupply)
            );
        }
        ...
        returns (uint256[] memory _pendingMultiRewardArr)
    {
        ...
        _pendingMultiRewardArr[1] = _lightReward.mul(REWARD_RATE_DBTC).div(
            REWARD_RATE_DENOMINATION
        );
        _pendingMultiRewardArr[2] = _lightReward.mul(REWARD_RATE_DETH).div(
            REWARD_RATE_DENOMINATION
        );
        _pendingMultiRewardArr[3] = _lightReward.mul(REWARD_RATE_DSGD).div(
            REWARD_RATE_DENOMINATION
        );
    }
        ...
        returns (uint256[] memory _totalMultiRewardArr)
    {
        ...
        _totalMultiRewardArr[1] = _lightTotalReward.mul(REWARD_RATE_DBTC).div(
            REWARD_RATE_DENOMINATION
        );
        _totalMultiRewardArr[2] = _lightTotalReward.mul(REWARD_RATE_DETH).div(
            REWARD_RATE_DENOMINATION
        );
        _totalMultiRewardArr[3] = _lightTotalReward.mul(REWARD_RATE_DSGD).div(
            REWARD_RATE_DENOMINATION
        );
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalAllocPoint_ > 0) {
            ...
            pool.accDarkoPerShare = pool.accDarkoPerShare.add(
                _lightReward.mul(1e18).div(tokenSupply)
            );
        }
        pool.lastRewardTime = block.timestamp;
    ...
    function _withdraw(
        ...
        if (_amount > 0) {
            ...
            require(
                ...
                "DarkoRewardPool: locked"
            );
        }
        updatePool(_pid);
        uint256 _pending = user.amount.mul(pool.accDarkoPerShare).div(1e18).sub(
            user.rewardDebt
        );
        if (_pending > 0) {
    ...
    function _claimReward(address _account, uint256 _lightAmt) internal {
        uint256 _dbtcAmt = _lightAmt.mul(REWARD_RATE_DBTC).div(
            REWARD_RATE_DENOMINATION
        );
        uint256 _dethAmt = _lightAmt.mul(REWARD_RATE_DETH).div(
            REWARD_RATE_DENOMINATION
        );
        uint256 _dsgdAmt = _lightAmt.mul(REWARD_RATE_DSGD).div(
            REWARD_RATE_DENOMINATION
        );
    ...
    function governanceRecoverUnsupported(
        ...
        if (block.timestamp < epochEndTimes[3] + 90 days) {
            ...
            require(
                _token != light && _token != dbtc && _token != deth,
                "reward"
            );
            uint256 length = poolInfo.length;
            for (uint256 pid = 0; pid < length; ++pid) {
