...
interface IStrategy {
    ...
    // Transfer want tokens CGOFarm -> strategy
    function deposit(address _userAddress, uint256 _wantAmt)
        external
        returns (uint256);

    // Transfer want tokens strategy -> CGOFarm
...
// File: contracts/TimelockController_RewardsDistributor.sol
...
/**
 * @dev CGOFarm functions that do not require less than the min timelock
 */
interface ICGOFarm {
    function add(
        uint256 _allocPoint,
    ...
    ReentrancyGuard
{
    ...
    uint256 [] public beneficiaryRewardFactors;
    ...
    event DistributeRewards(uint256 rewardsAmt);
    ...
    function setBeneficiaries(address[] memory _beneficiaryAddresses, uint256[] memory _beneficiaryRewardFactors) external virtual {
        require(
            msg.sender == address(this),
            "TimelockController: caller must be timelock"
        );
    ...
    function executeSet(
        ...
        _beforeCall(id, predecessor);
        ICGOFarm(_CGOfarmAddress).set(_pid, _allocPoint, _withUpdate);
        _afterCall(id);
    }
    ...
    function add(
        ...
        ICGOFarm(_CGOfarmAddress).add(0, _want, _withUpdate, _strat); // allocPoint = 0. Schedule set (timelocked) to increase allocPoint.
    }
    ...
    function convertTokensToWBNB( Swap [] memory _swaps ) public onlyRole(EXECUTOR_ROLE) {
        for (uint8 i = 0; i < _swaps.length; i++) {
            _convertTokenToWBNB(_swaps[i].uniRouterAddress, _swaps[i].path, _swaps[i].slippageFactor);
        }
    }
