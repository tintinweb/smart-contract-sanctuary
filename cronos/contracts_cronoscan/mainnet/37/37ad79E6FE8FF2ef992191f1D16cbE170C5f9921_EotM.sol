...
// File contracts/drops/EotM.sol

// 
pragma solidity ^0.8.4;
contract EotM is IDrop, BaseDrop {
    using Counters for Counters.Counter;
    using SafePct for uint256;
    using SafeMathLite for uint256;
    Counters.Counter public _tokenIdCounter;

    uint256 private memberCost = 250 ether;
    uint256 private regularCost = 350 ether;
    uint64 constant MAX_TOKENS = 62;
    uint8 constant MAX_MINTAMOUNT = 3;

    uint256 public mintStartTime;

    string baseURI = "ipfs://QmXRbpsJfxfcasGnHoFMcruGMWCrt4zxHCbKzfUGEhexTw";
    
    constructor(address _marketContract, address _artist, uint16[] memory _order) ERC721("Employee of the Month", "EotM") {
    ...
    function mint(uint256 _amount) external override payable whenNotPaused {
        require(mintStartTime != 0 && mintStartTime <= block.timestamp, "not started");
   ...
   function safeMint(address _to) private {
    ...
    function setMintStartTime(uint256 _time) external onlyOwner {
        mintStartTime = _time;
    }
}