...
// File contracts/treasury/Strat_ReduceReserveLP_MUSD.sol
...
contract StratReduceReserveLPMUSD is Ownable {
    using SafeERC20 for IERC20;

    ISaharaDaoTreasury public treasury;
    ...
    function reduceReserve(uint256 _amount, uint256 _minYTokenAmount) external onlyOwner {
        ...
        swapRouter.removeLiquidity(address(yToken), address(WethUtils.MUSD), _amount, 0, 0, address(this), block.timestamp);

        // 2. swap Weth -> YToken
        swap(WethUtils.MUSD.balanceOf(address(this)), _minYTokenAmount);
    ...
    function swap(uint256 _wethToSwap, uint256 _minYTokenOut) internal {
        WethUtils.MUSD.safeIncreaseAllowance(address(swapRouter), _wethToSwap);
