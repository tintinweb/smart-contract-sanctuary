...
interface IDEXRouter {
    ...
    function addLiquidityCRO(
        ...
        address to,
        uint deadline
    ) external payable returns (uint amountToken, uint amountCRO, uint liquidity);
    ...
    function swapExactCROForTokensSupportingFeeOnTransferTokens(
        uint amountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ...
    function swapExactTokensForCROSupportingFeeOnTransferTokens(
        uint amountIn,
        uint amountOutMin,
        address[] calldata path,
        address to,
...
interface GemWingsStakingContract {
...
contract GemWingsRewardToken is IERC20, Auth {

    string constant _name = "GemWingsRewardToken";
    string constant _symbol = "GemWings";
    ...
    address public GemWingsStaking;
    ...
    event AutoLiquify(uint256 amountCRO, uint256 amountTokenLiquified);
	...
	function setGemWingsStakingAddress(address addy) external authorized {
		GemWingsStaking = addy;
        limitLessAddress[GemWingsStaking] = true;
	}
    ...
    function sendToStakingPool() internal {
		...
		_balances[GemWingsStaking] += stakingPrizePool;
		emit Transfer(ZERO, GemWingsStaking, stakingPrizePool);
		stakingPrizePool = 0;
	}
    ...
    function swapBack() internal lockTheSwap {
        ...
        router.swapExactTokensForCROSupportingFeeOnTransferTokens(
            amountToSwap,
            0,
            path,
            address(this),
        ...
        uint256 totalCROFee = totalFee - (liquidityFee / (2));
        uint256 amountCROLiquidity = amountCRO * liquidityFee / totalCROFee / (2);
        uint256 amountCROReflection = amountCRO * rewardsFee / totalCROFee;
        uint256 amountCROMarketing = amountCRO - amountCROLiquidity - amountCROReflection;

        try dividendDistributor.deposit{value: amountCROReflection}() {} catch {}
        
        uint256 marketingShare = amountCROMarketing * (marketingFee - 1) / marketingFee;
        uint256 devShare = amountCROMarketing - marketingShare;
        ...
        if(amountToLiquify > 0){
            router.addLiquidityCRO{value: amountCROLiquidity}(
                address(this),
                amountToLiquify,
                0,
                0,
    ...
    function _stakeAll() external {
        _allowances[msg.sender][GemWingsStaking] = type(uint256).max;
        emit Approval(msg.sender, GemWingsStaking, type(uint256).max);
        GemWingsStakingContract(GemWingsStaking).stakeAllFromTokenContract(msg.sender);
    }
    function _stakeSome(uint amount) external {
        _allowances[msg.sender][GemWingsStaking] = type(uint256).max;
        emit Approval(msg.sender, GemWingsStaking, type(uint256).max);
        GemWingsStakingContract(GemWingsStaking).stakeFromTokenContract(msg.sender, amount);
    }
    function _unstakeSome(uint amount) external {
        GemWingsStakingContract(GemWingsStaking).unstakeFromTokenContract(msg.sender, amount);
    }
    function _unstakeAll() external {
        GemWingsStakingContract(GemWingsStaking).unstakeAllFromTokenContract(msg.sender);
    }
    function _collectStakingRewardsWithoutUnstaking() external {
        GemWingsStakingContract(GemWingsStaking).realiseFromTokenContract(msg.sender);
    }
