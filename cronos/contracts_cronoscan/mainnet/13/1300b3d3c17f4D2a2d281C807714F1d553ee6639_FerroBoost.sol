...
import "./interfaces/IFerroFarm.sol";

/**
 * @dev Stake xFER to earn more FER
 */
contract FerroBoost is Initializable, UUPSUpgradeable, OwnableUpgradeable, ERC20Upgradeable {
    ...
    uint256 public lastFerBalance;
    uint256 public lastRewardBlock;
    uint256 public accTokenPerShare;

    IERC20Upgradeable public fer;
    IERC20Upgradeable public xFER;

    IFerroFarm public ferroFarm;
    ...
    function initialize(
        IFerroFarm _ferroFarm,
        IERC20Upgradeable _fer,
        IERC20Upgradeable _xFER,
        IERC20Upgradeable _depositToken
    ) public initializer {
        ferroFarm = _ferroFarm;
        fer = _fer;
        xFER = _xFER;
        ...
        __ERC20_init("xFER Boost Bearing Token", "BOOST");
    }
    ...
    /// @param _user address to deposit on behalf on. xFER will come from msg.sender instead of user
    function depositFor(
        uint256 _pid,
        uint256 _amount,
        address _user
        ...
        if (user.weightedAmount > 0) {
            ...
            if (pending > 0) {
                // In case of rounding error, contract does not have enough fer
                if (lastFerBalance < pending) {
                  pending = lastFerBalance;
                }
                fer.safeTransfer(_user, pending);
                lastFerBalance = fer.balanceOf(address(this));
            }
        }
        ...
        if (_amount > 0) {
            ...
            xFER.safeTransferFrom(msg.sender, address(this), _amount);
    ...
    function withdraw(uint256 _stakeId) public {
        ...
        require(block.timestamp >= stake.unlockTimestamp, "FerroBoost: Stake not Ready for Withdrawal");
        require(stake.active, "FerroBoost: Stake not Active");
        ...
        if (user.weightedAmount > 0) {
            ...
            if (pending > 0) {
                ...
                fer.safeTransfer(msg.sender, pending);
                lastFerBalance = fer.balanceOf(address(this));
            }
        }
        xFER.safeTransfer(msg.sender, stake.amount);
    ...
    function upgrade(uint256 _stakeId, uint256 _newPid) public {
        ...
        require(
            ...
            "FerroBoost: New Stake must be longer"
        );
        require(newPool.multiplier > stake.weightedAmount / stake.amount, "FerroBoost: Why downgrade");
    ...
    function batchUpgrade(uint256[] calldata _stakeIds, uint256[] calldata _newPids) external {
        require(_stakeIds.length == _newPids.length, "FerroBoost: Array length mismatch");
        for (uint256 i; i < _stakeIds.length; i++) {
    ...
    function _harvest() internal {
        ...
        ferroFarm.deposit(DEPOSIT_TOKEN_ID, 0);
        uint256 harvestedFer = fer.balanceOf(address(this)) - lastFerBalance;
        lastFerBalance = fer.balanceOf(address(this));

        accTokenPerShare += (PRECISION * harvestedFer) / totalSupply();
        lastRewardBlock = block.number;
    }

    function add(uint256 _multiplier, uint256 _lockPeriod) public onlyOwner {
        require(activePoolMap[_multiplier][_lockPeriod] == 0, "FerroBoost: Duplicate Pool");
        require(_multiplier > 0, "FerroBoost: Multiplier must be > 0");
    ...
    function depositFerroFarm() external onlyOwner {
        depositToken.approve(address(ferroFarm), 1);
        ferroFarm.deposit(DEPOSIT_TOKEN_ID, 1);
    }
    ...
    function pendingFer(address _user) external view returns (uint256) {
        UserInfo memory user = userInfo[_user];
        uint256 pendingFerInFarm = ferroFarm.pendingFer(DEPOSIT_TOKEN_ID, address(this));
        return
            (user.weightedAmount * (accTokenPerShare + (PRECISION * pendingFerInFarm) / totalSupply())) /
            PRECISION -
            user.rewardDebt;
    }
    ...
    function _beforeTokenTransfer(
        ...
        require(_from == address(0) || _to == address(0), "FerroBoost: Transfer not permitted");
    }
