...
interface IBoardroom {
    ...
    function getDarkoPrice() external view returns (uint256);
...
contract Treasury is ITreasury, ContractGuard, ReentrancyGuard {
    ...
    // 28 first epochs (1 week) with 4.5% expansion regardless of dark price
    uint256 public bootstrapEpochs;
    uint256 public bootstrapSupplyExpansionPercent;

    uint256 public override previousEpochDarkPrice;
    ...
    address[] public darkLockedAccounts;
    ...
    modifier checkEpoch() {
        ...
        epoch_ = epoch_.add(1);
        epochSupplyContractionLeft = (getDarkPrice() > darkPriceCeiling)
            ? 0
            : IERC20(dark).totalSupply().mul(maxSupplyContractionPercent).div(
                10000
            );
    }
    ...
    function getPegPrice() external view override returns (int256) {
        return IOracle(darkOracle).getPegPrice();
    }

    function getPegPriceUpdated() external view override returns (int256) {
        return IOracle(darkOracle).getPegPriceUpdated();
    }

    // oracle
    function getDarkPrice() public view override returns (uint256 darkPrice) {
    ...
    function getDarkUpdatedPrice()
        public
        view
        override
        returns (uint256 _darkPrice)
    {
    ...
    function getBurnabledarkLeft()
        external
        view
        returns (uint256 _burnabledarkLeft)
    {
        uint256 _darkPrice = getDarkPrice();
        if (_darkPrice <= darkPriceOne) {
            uint256 _bondMaxSupply = IERC20(dark)
                .totalSupply()
            ...
            if (_bondMaxSupply > _bondSupply) {
                ...
                uint256 _maxBurnabledark = _maxMintableBond
                    .mul(getBondDiscountRate())
                    .div(1e18);
                _burnabledarkLeft = Math.min(
    ...
    function getBondPremiumRate() public view override returns (uint256 _rate) {
        ...
        if (_darkPrice > darkPriceCeiling) {
            if (premiumPercent == 0) {
                ...
                uint256 _premiumAmount = _darkPrice
                    .sub(darkPriceOne)
                    .mul(premiumPercent)
                    .div(10000);
    ...
    function getDarkCirculatingSupply() public view override returns (uint256) {
        return IERC20(dark).totalSupply().sub(getDarkLockedBalance());
    }

    function getDarkLockedBalance()
        public
        view
        override
        returns (uint256 _lockedBalance)
    {
        uint256 _length = darkLockedAccounts.length;
        IERC20 _dark = IERC20(dark);
        for (uint256 i = 0; i < _length; i++) {
            _lockedBalance = _lockedBalance.add(
                _dark.balanceOf(darkLockedAccounts[i])
            );
        }
    }
    ...
    function getNextExpansionRate() public view override returns (uint256) {
        if (epoch_ < bootstrapEpochs) {
            ...
            return getPegTokenExpansionRate(dark);
        }
    }

    function getNextExpansionAmount() external view override returns (uint256) {
        return getDarkCirculatingSupply().mul(getNextExpansionRate()).div(1e6);
    }
    ...
    function initialize(
        ...
        epochLength_ = 8 hours;
        lastEpochTime = _startTime.sub(8 hours);

        darkPriceOne = 10**18; // This is to allow a PEG of 1 dark per VVS
        darkPriceCeiling = darkPriceOne.mul(1001).div(1000);

        maxSupplyExpansionPercent = 150; // Upto 1.5% supply for expansion

        bondDepletionFloorPercent = 10000; // 100% of Bond supply for depletion floor
        seigniorageExpansionFloorPercent = 3500; // At least 35% of expansion reserved for boardroom
        maxSupplyContractionPercent = 750; // Upto 7.5% supply for contraction (to burn dark and mint light)
        maxDebtRatioPercent = 15000; // Upto 150% supply of light to purchase
        ...
        nextSupplyTarget = 200000000 ether; // 200 million supply is the next target to reduce expansion rate
        multiPegEnabled = true;
        allocateSeigniorageSalary = 1 ether; // 1 DARKO for allocateSeigniorage() calling
    ...
    function setDarkPriceCeiling(uint256 _darkPriceCeiling)
        external
        onlyOperator
    {
        require(
            _darkPriceCeiling >= darkPriceOne &&
                _darkPriceCeiling <= darkPriceOne.mul(120).div(100),
            "out of range"
        ); // [$1.0, $1.2]
    ...
    {
        require(
            _maxDebtRatioPercent >= 1000 && _maxDebtRatioPercent <= 20000,
            "out of range"
        ); // [10%, 200%]
    ...
    function setNextSupplyTarget(uint256 _target) external onlyOperator {
        require(_target > IERC20(dark).totalSupply(), "too small");
        nextSupplyTarget = _target;
    }

    function setDarkLockedAccounts(address[] memory _darkLockedAccounts)
        external
        onlyOperator
    {
        delete darkLockedAccounts;
        uint256 _length = _darkLockedAccounts.length;
        for (uint256 i = 0; i < _length; i++) {
            darkLockedAccounts.push(_darkLockedAccounts[i]);
        }
    }
    ...
    function buyBonds(uint256 _darkAmount, uint256 targetPrice)
        external
        override
        onlyOneBlock
        checkOperator
        nonReentrant
    {
        require(_darkAmount > 0, "zero amount");

        uint256 darkPrice = getDarkPrice();
        require(darkPrice == targetPrice, "price moved");
        require(
            darkPrice < darkPriceOne, // price < $1
            "darkPrice not eligible for bond purchase"
        );

        require(
            _darkAmount <= epochSupplyContractionLeft,
        ...
        uint256 _darkSupply = IERC20(dark).totalSupply();
        uint256 newBondSupply = IERC20(light).totalSupply().add(_bondAmount);
        require(
            newBondSupply <= _darkSupply.mul(maxDebtRatioPercent).div(10000),
            "over max debt ratio"
        );

        IBasisAsset(_dark).burnFrom(msg.sender, _darkAmount);
        ...
        nonReentrant
    {
        ...
        require(
            IERC20(dark).balanceOf(address(this)) >= _darkAmount,
            "treasury has no more budget"
        );

        seigniorageSaved = seigniorageSaved.sub(
            Math.min(seigniorageSaved, _darkAmount)
        );
        allocateSeigniorageSalary = 1000 ether; // 1000 dark salary for calling allocateSeigniorage()
        ...
        _updatedarkPrice();
        if (regulationStats != address(0))
            IRegulationStats(regulationStats).addRedeemed(epoch_, _darkAmount);
    ...
    function _sendToBoardroom(uint256 _amount, uint256 _expanded) internal {
        address _dark = dark;
        IBasisAsset(_dark).mint(address(this), _amount);

        uint256 _daoFundSharedAmount = 0;
        if (daoFundSharedPercent > 0) {
            _daoFundSharedAmount = _amount.mul(daoFundSharedPercent).div(10000);
            IERC20(_dark).transfer(daoFund, _daoFundSharedAmount);
        }
        ...
        if (marketingFundSharedPercent > 0) {
            _marketingFundSharedAmount = _amount
                ...
                .div(10000);
            IERC20(_dark).transfer(marketingFund, _marketingFundSharedAmount);
        }
        ...
        if (insuranceFundSharedPercent > 0) {
            _insuranceFundSharedAmount = _amount
                ...
                .div(10000);
            IERC20(_dark).transfer(insuranceFund, _insuranceFundSharedAmount);
        }
        ...
        IERC20(_dark).safeIncreaseAllowance(boardroom, _amount);
        IBoardroom(boardroom).allocateSeigniorage(_dark, _amount);

        if (regulationStats != address(0))
            IRegulationStats(regulationStats).addEpochInfo(
                epoch_.add(1),
                previousEpochDarkPrice,
                _expanded,
                _amount,
        ...
        nonReentrant
    {
        ...
        address _dark = dark;
        uint256 _supply = getDarkCirculatingSupply();
        ...
        if (epoch_ < bootstrapEpochs) {
            // 28 first epochs with 4.5% expansion
            if (epoch_ == 0) _supply = IERC20(_dark).totalSupply();
            ...
            if (previousEpochDarkPrice > darkPriceCeiling) {
                // Expansion ($dark Price > 1 $ETH): there is some seigniorage to be allocated
                ...
                if (_savedForBond > 0) {
                    ...
                    IBasisAsset(_dark).mint(address(this), _savedForBond);
                    emit TreasuryFunded(block.timestamp, _savedForBond);
                }
            } else if (previousEpochDarkPrice < darkPriceOne) {
                if (regulationStats != address(0))
                    IRegulationStats(regulationStats).addEpochInfo(
                        epoch_.add(1),
                        previousEpochDarkPrice,
        ...
        internal
    {
        ...
        if (_pegTokenTwap > darkPriceCeiling) {
            uint256 _percentage = _pegTokenTwap.sub(darkPriceOne); // 1% = 1e16
    ...
    function governanceRecoverUnsupported(
        ...
        require(address(_token) != address(light), "bond");
