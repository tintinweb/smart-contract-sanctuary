...
contract AutoCompoundingToken is ERC20Detailed, Ownable {
    ...
    bool public cooldownsEnabled;
    uint256 public constant MAX_BUY_SIZE = 10;
    uint256 public constant MAX_SELL_SIZE = 5;
    mapping(address => uint256) public buyCooldowns;
    mapping(address => uint256) public sellCooldowns;
    ...
    constructor() public ERC20Detailed("RaOfCRO", "RA", 18) Ownable() {
        uint256 initialFragments = uint256(1000000000000).mul(10**uint256(18));
        INITIAL_FRAGMENTS_SUPPLY = initialFragments;

        uint256 totalGons = MAX_UINT256 - (MAX_UINT256 % initialFragments);
        TOTAL_GONS = totalGons;

        MAX_SUPPLY = uint256(1000000000000).mul(10**uint256(18)).mul(10**4);
        ...
        sellFee = 0;
        rebaseInterval = 60;
        rebasePercentage = 7900;
        lpInterval = 60;
        totalFee = 130;

        router = IUniswapV2Router(0x145677FC4d9b8F19B5D56d1820c48e0443049a30);
        pairContract = IUniswapV2Pair(
            IUniswapV2Factory(
                IUniswapV2Router(0x145677FC4d9b8F19B5D56d1820c48e0443049a30)
                    .factory()
            ).createPair(
                    IUniswapV2Router(0x145677FC4d9b8F19B5D56d1820c48e0443049a30)
                        .WETH(),
        ...
        autoLiquidityReceiver = 0xa631dbbff0eA638467656418Ae7EBD97670CbBDB;
        treasuryReceiver = 0x4f8A034ca4D5cE799e4F9c17298FF7b3c14fe952;
        insuranceFundReceiver = 0x43AF51BBd3182c20424A0F7EAeb9636a3cbe4C9d;
        burnReceiver = 0xEDBD360d45599e55c61E2Ece8ED34E7AB653f89D;
        ...
        cooldownExempt[0xa631dbbff0eA638467656418Ae7EBD97670CbBDB] = true;
        cooldownExempt[0x4f8A034ca4D5cE799e4F9c17298FF7b3c14fe952] = true;
        cooldownExempt[0x43AF51BBd3182c20424A0F7EAeb9636a3cbe4C9d] = true;
        cooldownExempt[0xEDBD360d45599e55c61E2Ece8ED34E7AB653f89D] = true;
        cooldownExempt[0x145677FC4d9b8F19B5D56d1820c48e0443049a30] = true;
    ...
    function updateCooldowns(bool _flag) external onlyOwner {
        cooldownsEnabled = _flag;
    }
    ...
    function _transferFrom(
        ...
        if (cooldownsEnabled) {
            if (recipient == address(pairContract) && !cooldownExempt[sender]) {
                // sell
                require(
                    sellCooldowns[sender] <= block.timestamp,
                    "You cannot sell more until your cooldown ends"
                );
                require(
                    gonAmountReceived.div(gonsPerFragment) <=
                        MAX_SELL_SIZE.mul(totalFragmentSupply).div(1000),
                    "Exceeds max txn size"
                );
                sellCooldowns[sender] = block.timestamp + 10 minutes;
            } else if (
                sender == address(pairContract) && !cooldownExempt[recipient]
            ) {
                // buy
                require(
                    buyCooldowns[recipient] <= block.timestamp,
                    "You cannot purchase more until your cooldown ends"
                );
                require(
                    gonAmountReceived.div(gonsPerFragment) <=
                        MAX_BUY_SIZE.mul(totalFragmentSupply).div(1000),
                    "Exceeds max txn size"
                );
                buyCooldowns[sender] = block.timestamp + 1 minutes;
            }
        }
