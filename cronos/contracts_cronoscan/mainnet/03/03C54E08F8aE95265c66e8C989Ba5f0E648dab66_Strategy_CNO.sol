...
interface IPancakeswapFarm {
    ...
    function pendingCyborg(uint256 _pid, address _user)
        external
        view
        returns (uint256);
...
// File: contracts/Strategy_CNO.sol
...
contract Strategy_CNO is Strategy {
    constructor(
        ...
        CGOFarmAddress = _addresses[2]; // Vault Farming
        CGOAddress = _addresses[3]; // Token
    ...
    /**
     * @notice Calculates the expected harvest reward from third party
     * @return Expected reward to collect in Cyborg
     */
    function calculateHarvestRewards() override external view returns (uint256) {
        uint256 amount = IPancakeswapFarm(farmContractAddress).pendingCyborg(pid, address(this));
        amount = amount.add(available());
        uint256 currentCallFee = amount.mul(callFee).div(callFeeMax);
