...
// File contracts/interfaces/ISaharaDaoTreasury.sol


pragma solidity 0.8.4;

interface ISaharaDaoTreasury {
...
// File contracts/treasury/Strat_Recollateralize_MUSD.sol
...
contract StratRecollateralizeMUSD is Ownable {
    using SafeERC20 for IERC20;

    IPool public immutable pool;
    ISaharaDaoTreasury public immutable treasury;

    constructor(ISaharaDaoTreasury _treasury, IPool _pool) {
        treasury = _treasury;
        pool = _pool;
    }
    ...
    function recollateralize(uint256 _amount) external onlyOwner {
        ...
        treasury.requestFund(address(WethUtils.MUSD), _amount);
        WethUtils.MUSD.safeTransferFrom(address(treasury), address(this), _amount);

        WethUtils.MUSD.safeIncreaseAllowance(address(pool), _amount);
        pool.recollateralize(_amount);
