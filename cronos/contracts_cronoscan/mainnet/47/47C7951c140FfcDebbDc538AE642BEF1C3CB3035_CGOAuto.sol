...
contract CGOAuto is Ownable, Pausable {
    ...
    uint256 public withdrawFeePeriod = 0; // 3 days
    ...
    function harvest() external notContract whenNotPaused {
        IPancakeswapFarm(masterchef).withdraw(0, 0);
     ...
     */
    function calculateHarvestCGORewards() external view returns (uint256) {
        uint256 amount = IPancakeswapFarm(masterchef).pendingCGO(0, address(this));
     ...
     */
    function calculateTotalPendingCGORewards() external view returns (uint256) {
        uint256 amount = IPancakeswapFarm(masterchef).pendingCGO(0, address(this));
        amount = amount.add(available());
    ...
    function withdraw(uint256 _shares) public notContract {
        ...
        if (bal < currentAmount) {
            ...
            IPancakeswapFarm(masterchef).withdraw(0, balWithdraw);
    ...
    function balanceOf() public view returns (uint256) {
        (uint256 amount) = IPancakeswapFarm(masterchef).stakedWantTokens(0, address(this));
    ...
    function _earn() internal {
        uint256 bal = available();
        if (bal > 0) {
            IPancakeswapFarm(masterchef).deposit(0, bal);
        }
    }
