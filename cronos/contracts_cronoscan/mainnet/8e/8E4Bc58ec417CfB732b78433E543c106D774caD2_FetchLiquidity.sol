...
/** 
 *  SourceUnit: /Users/ganesh/repos/dfyn/path-finder-api/contracts/FetchLiquidity.sol
*/
...
contract FetchLiquidity {
    ...
    function getLiquidityReserves(IERC20[2][] calldata tokensIn, uint256[] calldata exchangeIds)
        external
        view
        returns (dexResponse[] memory response)
    {
        return _getReserves(exchangeIds, tokensIn);
    }

    function _getReserves(uint256[] memory exchangeIds, IERC20[2][] memory tokensIn) internal view returns(dexResponse[] memory) {
        dexResponse[] memory response = new dexResponse[](tokensIn.length * exchangeIds.length);
        uint256 actualLength = 0;
        for (uint256 index = 0; index < exchangeIds.length; index++) {
            uint256 exchangeId = exchangeIds[index];
            (IUniswapV2Factory exchangeInstance, uint256 exchangeCode) = _getExchangeInstance(exchangeId);
            dexResponse[] memory data = _getLiquidityMulti(tokensIn, exchangeInstance, exchangeCode);
    ...
    function _getLiquidityMulti(IERC20[2][] memory tokensIn, IUniswapV2Factory exchangeInstance, uint256 exchangeCode)
        internal
        view
        returns (dexResponse[] memory response)
    {
        ...
        for (uint256 i = 0; i < tokensIn.length; i++) {
            (IERC20 _fromToken, IERC20 _destToken, uint112 _reserve0, uint112 _reserve1,) = _calculateExchange(tokensIn[i][0], tokensIn[i][1], exchangeInstance);
            response[i] = dexResponse(_fromToken, _destToken, _reserve0, _reserve1, uint112(exchangeCode));
        }
        return response;
    }

        function _calculateExchange(IERC20 fromToken, IERC20 destToken, IUniswapV2Factory exchangeInstance)
        internal
        view
        returns (
            IERC20 _fromToken,
    ...
    {
        IUniswapV2Exchange exchange = exchangeInstance.getPair(fromToken, destToken);
        if (exchange != IUniswapV2Exchange(address(0))) {
        (_reserve0, _reserve1,) = exchange.getReserves();
        return (fromToken, destToken, _reserve0, _reserve1, _blockTimestampLast);
        }
        return (fromToken, destToken, 0, 0, 0);
    }
    
    function _getExchangeInstance(uint256 flags)
        internal
        pure
        returns(IUniswapV2Factory, uint256)
    {
