...
contract GShareRewardPool {
    ...
    struct PoolInfo {
        ...
        uint256 accGSharePerShare; // Accumulated tSHAREs per share, times 1e18. See below.
    ...
    uint256 public tSharePerSecond = 0.00186123 ether; // 59500 gshare / (370 days * 24h * 60min * 60s)
    ...
    function set(uint256 _pid, uint256 _allocPoint) external onlyOperator returns (bool) {
        massUpdatePools();
    ...
    function emergencyWithdraw(uint256 _pid) external returns (bool) {
        PoolInfo storage pool = poolInfo[_pid];
    ...
    // Safe gshare transfer function, just in case if rounding error causes pool to not have enough tSHAREs.
