...
interface IPancakeswapFarm {
    ...
    // View function to see pending CGOs on frontend.
    function pendingCGO(uint256 _pid, address _user)
        external
        view
        returns (uint256);

    // View function to see pending CGOs on frontend.
    function pendingCGX(uint256 _pid, address _user)
        external
        view
        returns (uint256);
    ...
    function pendingMeerkat(uint256 _pid, address _user)
        external
        view
        returns (uint256);
...
abstract contract Strategy is Ownable, ReentrancyGuard, Pausable {
    ...
    bool public isCGOComp; // this vault is purely for staking. eg. WBNB-CGO staking vault.

    address public farmContractAddress; // address of farm, eg, PCS, Thugs etc.
    uint256 public pid; // pid of pool in farmContractAddress
    address public wantAddress; // staked address
     ...
     * @return Expected reward to collect in CGX
     */
    function calculateHarvestRewards() virtual external view returns (uint256) {
     ...
     * @dev Only govAddress by the contract admin.
     */
    function setCallFee(uint256 _callFee) external onlyAllowGov {
        require(_callFee <= 200, "callFee cannot be more than MAX_CALL_FEE");
        callFee = _callFee;
...
// File: contracts/Strategy_CGO.sol
...
contract Strategy_CGO is Strategy {
    ...
    uint256 public burnRate = 5000; // (50% earn amount) ~ 1% of rewards
    uint256 public MAX_BURN_RATE = 10000;
    ...
    event SetBurnRate(uint256 _rate);
    ...
    constructor(
        ...
        CGOFarmAddress = _addresses[2];
        CGOAddress = _addresses[3];
        ...
        returns (uint256)
    {
        ...
        wantLockedTotal = IERC20(CGOAddress).balanceOf(address(this));
    ...
    // Only gov execute buyback and burn CGO
    function earn() public override nonReentrant whenNotPaused {
        ...
        uint256 burnAmt = earnedAmt.mul(burnRate).div(1e4);

        earnedAmt = earnedAmt.sub(burnAmt);
        // Buyback
        IERC20(earnedAddress).safeApprove(uniRouterAddress, 0);
        IERC20(earnedAddress).safeIncreaseAllowance(
            uniRouterAddress,
    ...
    function setBurnRate(uint256 _rate) external onlyAllowGov {
        require(_rate <= MAX_BURN_RATE, "burn rate cannot be more than MAX_BURN_RATE");
        burnRate = _rate;
        emit SetBurnRate(_rate);
    }
