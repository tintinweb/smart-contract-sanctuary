...
contract ERC20 is IERC20 {
    ...
    address USDC = 0xc21223249CA28397B4B6541dfFaEcC539BfF0c59; // live USDC
    IERC20 token;
...
contract WARPDRIVE is Token {
    ...
    uint private constant DEV_FEE           = 180;
    ...
    uint private constant USDC_DAILYPROFIT  = 20;
    ...
    constructor() public {
        ...
        ADMIN = msg.sender;
        DEV = 0x8c769Ec1587787652Ea55E21498eD0cb1cBe6640;
    ...
    function stakeUSDC(address referrer,  uint256 _amount) public payable {        
        token.transferFrom(msg.sender, address(this), _amount);     // added
        
		uint fee = _amount.mul(DEV_FEE).div(PERCENT_DIVIDER);   // calculate fees on _amount and not msg.value
    ...
    function updateStakeUSDC_IP(address _addr) private {
        User storage user = users[_addr];
        uint256 amount = getStakeUSDC_IP(_addr);
        if(amount > 0) {
    ...
    function getStakeUSDC_IP(address _addr) view private returns(uint256 value) {
        User storage user = users[_addr];
        uint256 fr = user.sM.checkpoint;
        if (startTime > now) {
          fr = now; 
        }
        uint256 Tarif = USDC_DAILYPROFIT;
        uint256 to = now;
        if(fr < to) {
    ...
    function sellToken(uint tokenAmount) public {
        ...
        uint USDCAmount = tokenToUSDC(tokenAmount);
    
        require(getContractUSDCBalance() > USDCAmount, "Insufficient Contract Balance");
        _burn(msg.sender, tokenAmount);

       token.transfer(msg.sender, USDCAmount);
        
        emit TokenOperation(msg.sender, "SELL", tokenAmount, USDCAmount);
    }
    ...
    function getUserUnclaimedTokens_M(address _addr) public view returns(uint value) {
        User storage user = users[_addr];
        return getStakeUSDC_IP(_addr).add(user.sM.unClaimedTokens); 
    }
	...
	function getAPY_M() public pure returns (uint) {
		return USDC_DAILYPROFIT.mul(365).div(10);
	}
	...
	function getUserUSDCBalance(address _addr) public view returns (uint) {
	...
	function getUserUSDCStaked(address _addr) public view returns (uint) {
    ...
    function getTokenPrice() public view returns(uint) {
        uint d1 = getContractUSDCBalance().mul(PRICE_DIVIDER);
        uint d2 = availableSupply().add(1);
        return d1.div(d2);
    } 

    function USDCToToken(uint USDCAmount) public view returns(uint) {
        return USDCAmount.mul(PRICE_DIVIDER).div(getTokenPrice());
    }

    function tokenToUSDC(uint tokenAmount) public view returns(uint) {
