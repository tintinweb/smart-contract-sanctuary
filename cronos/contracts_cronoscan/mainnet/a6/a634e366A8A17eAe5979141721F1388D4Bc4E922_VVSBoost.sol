...
/**
 * @dev Stake xVVS to earn more VVS
 */
contract VVSBoost is Initializable, UUPSUpgradeable, OwnableUpgradeable, ERC20Upgradeable {
    ...
    uint256 public lastVVSBalance;
    uint256 public lastRewardBlock;
    uint256 public accTokenPerShare;

    IERC20Upgradeable public vvs;
    IERC20Upgradeable public xvvs;
    ...
    function initialize(
        ...
        IERC20Upgradeable _vvs,
        IERC20Upgradeable _xvvs,
        ...
        __ERC20_init("xVVS Boost Bearing Token", "BOOST");
    }

    function deposit(uint256 _pid, uint256 _amount) external {
        PoolInfo storage pool = poolInfo[_pid];
        require(pool.multiplier > 0, "VVSBoost: Invalid Pool ID");
        ...
        if (user.weightedAmount > 0) {
            ...
            if (pending > 0) {
                vvs.safeTransfer(msg.sender, pending);
                lastVVSBalance = vvs.balanceOf(address(this));
            }
        }
        ...
        if (_amount > 0) {
            ...
            xvvs.safeTransferFrom(msg.sender, address(this), _amount);
    ...
    function withdraw(uint256 _stakeId) public {
        ...
        require(block.timestamp >= stake.unlockTimestamp, "VVSBoost: Stake not Ready for Withdrawal");
        require(stake.active, "VVSBoost: Stake not Active");
        ...
        xvvs.safeTransfer(msg.sender, stake.amount);
    ...
    function upgrade(uint256 _stakeId, uint256 _newPid) public {
        ...
        require(
            ...
            "VVSBoost: New Stake must be longer"
        );
        require(newPool.multiplier > stake.weightedAmount / stake.amount, "VVSBoost: Why downgrade");
    ...
    function batchUpgrade(uint256[] calldata _stakeIds, uint256[] calldata _newPids) external {
        require(_stakeIds.length == _newPids.length, "VVSBoost: Array length mismatch");
        for (uint256 i; i < _stakeIds.length; i++) {
    ...
    function _harvest() internal {
        ...
        uint256 harvestedVVS = vvs.balanceOf(address(this)) - lastVVSBalance;
        lastVVSBalance = vvs.balanceOf(address(this));

        accTokenPerShare += (PRECISION * harvestedVVS) / totalSupply();
        lastRewardBlock = block.number;
    }

    function add(uint256 _multiplier, uint256 _lockPeriod) public onlyOwner {
        require(activePoolMap[_multiplier][_lockPeriod] == 0, "VVSBoost: Duplicate Pool");
    ...
    function pendingVVS(address _user) external view returns (uint256) {
        UserInfo memory user = userInfo[_user];
        uint256 pendingVVSInCraftsman = craftsman.pendingVVS(depositTokenPid, address(this));
        return
            (user.weightedAmount * (accTokenPerShare + (PRECISION * pendingVVSInCraftsman) / totalSupply())) /
            PRECISION -
            user.rewardDebt;
    }
    ...
    function _beforeTokenTransfer(
        ...
        require(_from == address(0) || _to == address(0), "VVSBoost: Transfer not permitted");
    }
