...
//        require(canClaimReward(), "contraction");
    ...
    function _claimReward(bool _useLightCollateral) internal updateReward(msg.sender) {
        ...
        if (_useLightCollateral) {
            ...
            uint256 _colAmt = getTotalLightCollateralForClaimAllPegTokens(msg.sender);
            if (_colAmt > 0) {
                light.safeTransferFrom(msg.sender, insuranceFund, _colAmt);
            }
        } else {
            uint256 _colAmt = getTotalCollateralForClaimAllPegTokens(msg.sender);
            if (_colAmt > 0) {
                wcro.safeTransferFrom(msg.sender, insuranceFund, _colAmt);
            }
        }
