...
abstract contract ERC20WithDecimalsUpgradeable is ERC20Upgradeable {
    uint8 private _decimals;

    function _ERC20WithDecimal_init (
...
interface IVault {
  ...
  // function bigBangPoolId() external view returns (uint256);
 ...
 * Lending pool
 */
contract Vault is IVault, ERC20WithDecimalsUpgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
  /// @notice Libraries
  using SafeToken for address;
  using SafeMath for uint256;

  /// @notice Events
  event Deposit(uint256 amountToken);
  event Borrow(uint256 debtVal);
  event Withdraw(uint256 amountToken);
  event Repay(uint256 debtVal);
  ...
  uint256 public override vaultDebtShare;
  uint256 public vaultDebtVal;
  uint256 public lastAccrueBlockTime;
  ...
  modifier accrue(uint256 value) {
    if (block.timestamp > lastAccrueBlockTime) {
      ...
      lastAccrueBlockTime = block.timestamp ;
    }
    _;
  }
  ...
  function initialize(
    ...
    ERC20WithDecimalsUpgradeable._ERC20WithDecimal_init(_name, _symbol,_decimals);
  ...
  function pendingInterest(uint256 value) public override view returns (uint256) {
   
      uint256 timePast = block.timestamp.sub(lastAccrueBlockTime);
  ...
  function debtShareToVal(uint256 debtShare) public override view returns (uint256) {
  ...
  function debtValToShare(uint256 debtVal) public override view returns (uint256) {
  ...
  function _deposit(uint256 amountToken) internal {
    ...
    require(totalSupply() > 10**(decimals()), "no tiny shares");
  }
  ...
  function borrow(uint256 debtVal) external override onlyBank accrue(0) nonReentrant returns(uint256) {
    require(debtVal <= token.balanceOf(address(this)).sub(reservePool), "insufficient funds in the vault");
    require(debtVal >= config.minDebtSize(), "too small debt size");

    uint256 debtShare = debtValToShare(debtVal);
    vaultDebtShare = vaultDebtShare.add(debtShare);
    vaultDebtVal = vaultDebtVal.add(debtVal);
    
    SafeToken.safeTransfer(token, address(bank), debtVal);
    IDebtToken(debtToken).mint(address(bank), debtShare);
    emit Borrow(debtVal);
    
    //to indicate function succeeded.
    return 0;
  } 

  function repay(uint256 debtVal) external override onlyBank nonReentrant returns (uint256){
    if (debtVal > 0) {
      uint256 debtShare = debtValToShare(debtVal);
      //if paid too much debtVal
      if (debtVal > vaultDebtVal) {
        debtVal = vaultDebtVal;
        debtShare = vaultDebtShare;
      }
      vaultDebtShare = vaultDebtShare.sub(debtShare);
      vaultDebtVal = vaultDebtVal.sub(debtVal);
      
      SafeToken.safeTransferFrom(token, bank, address(this), debtVal);
      IDebtToken(debtToken).burn(bank, Math.min(IERC20(debtToken).balanceOf(bank),debtShare));
      emit Repay(debtVal);
    }
    return 0;
  }

  function currentDebtVal() accrue(0) external override returns (uint256){
    return vaultDebtVal;
  } 
