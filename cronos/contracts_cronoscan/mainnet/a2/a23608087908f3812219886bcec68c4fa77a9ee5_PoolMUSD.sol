...
contract PoolMUSD is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;
    ...
    uint256 public priceTarget = 1e18; // = 1; 1 XToken pegged to the value of 1 MUSD
    uint256 public priceBand = 4e15; // = 0.004; CR will be adjusted if XToken > 1.004 MUSD or XToken < 0.996 MUSD
    ...
    function usableCollateralBalance() public view returns (uint256) {
        uint256 _balance = WethUtils.MUSD.balanceOf(address(this));
    ...
    function mint(uint256 _minXTokenOut, uint256 _ethIn) external nonReentrant {
        ...
        WethUtils.MUSD.safeTransferFrom(_sender, address(this), _ethIn);

        if (_yTokenOutTwap > 0 && _wethSwapIn > 0) {
            WethUtils.MUSD.safeIncreaseAllowance(address(swapStrategy), _wethSwapIn);
    ...
    function collect() external nonReentrant {
        ...
        if (_sendEth) {
            WethUtils.MUSD.safeTransfer(_sender, _ethAmount);
        }
    }
    ...
    function recollateralize(uint256 _amount) external {
        ...
        WethUtils.MUSD.safeTransferFrom(msg.sender, address(this), _amount);
    ...
    function transferToTreasury(uint256 _amount) internal {
        ...
        if (_amount > 0) {
            WethUtils.MUSD.safeTransfer(treasury, _amount);
        }
    }
