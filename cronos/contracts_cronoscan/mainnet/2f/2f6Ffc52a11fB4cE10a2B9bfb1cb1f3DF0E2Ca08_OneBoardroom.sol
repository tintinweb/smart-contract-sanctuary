...
contract OneBoardroom is ShareWrapper, ContractGuard {
    ...
    function canUseLightForCollateral() public view returns (bool) {
        return getDarkUpdatedPrice() < 1e18;
    }
    ...
    function getDarkUpdatedPrice() public view returns (uint256) {
        return treasury.getDarkUpdatedPrice();
    }

    function getLightOverWcroPrice() public view returns (uint256) {
        return (lightOracle == address(0)) ? 1e18 : uint256(IEpoch(lightOracle).getPegPriceUpdated());
    }

    function getPegOriginalTokenOverWcroPrice(address _token) public view returns (uint256) {
    ...
    function getCollateralForClaimAllPegTokens(address _token, address _member) public view returns (uint256) {
        ...
        uint256 _price = getPegOriginalTokenOverWcroPrice(_token);
        uint256 _collateralPercent = getCollateralPercent();
        return _earned.mul(_price).mul(_collateralPercent).div(1e22); // 1e18 * 10000
    }

    function getCollateralForClaimAllPegTokens(address _member) public view returns (uint256 _numOfPegTokens, address[] memory _pegTokenAddresses, uint256[] memory _earnedPegTokens, uint256[] memory _collateralForPegTokens, uint256 _totalCollateral, uint256 _totalLightCollateral) {
        _numOfPegTokens = numOfPegTokens();
        _pegTokenAddresses = new address[](_numOfPegTokens);
        _earnedPegTokens = new uint256[](_numOfPegTokens);
        ...
        _totalLightCollateral = _totalCollateral.mul(1e18).div(getLightOverWcroPrice());
        _totalLightCollateral = _totalLightCollateral.sub(_totalLightCollateral.mul(lightCollateralDiscountPercent).div(10000)); // sub discount
    }
    ...
    function getTotalLightCollateralForClaimAllPegTokens(address _member) public view returns (uint256 _totalLightCollateral) {
...
//        require(canClaimReward(), "contraction");
    ...
    function _claimReward(bool _useLightCollateral) internal updateReward(msg.sender) {
        ...
        if (_reward > 0) {
            if (_useLightCollateral) {
                require(canUseLightForCollateral(), "Use LIGHT only during DARK contraction");
                light.safeTransferFrom(msg.sender, insuranceFund, getTotalLightCollateralForClaimAllPegTokens(msg.sender));
            } else {
