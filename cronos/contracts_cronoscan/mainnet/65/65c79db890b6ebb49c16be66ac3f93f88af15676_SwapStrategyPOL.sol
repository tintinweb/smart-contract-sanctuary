...
contract SwapStrategyPOL is ISwapStrategy, Ownable {
    ...
    function execute(uint256 _wethIn, uint256 _yTokenOut) external override {
        WethUtils.MMF.safeTransferFrom(msg.sender, address(this), _wethIn);

        // 1. swap 50% of WETH to YToken
        IUniswapV2Pair _pair = IUniswapV2Pair(address(lp));
    ...
    function swap(uint256 _wethToSwap, uint256 _minYTokenOut) internal returns (uint256) {
        WethUtils.MMF.safeIncreaseAllowance(address(swapRouter), _wethToSwap);
        uint256[] memory _amounts = swapRouter.swapExactTokensForTokens(_wethToSwap, _minYTokenOut, swapPaths, address(this), block.timestamp);
    ...
    function addLiquidity(
        ...
        if (yTokenAmt > 0 && wethAmt > 0) {
            uint256 _minYTokenOut = (yTokenAmt * (SLIPPAGE_PRECISION - slippage)) / SLIPPAGE_PRECISION;
            ...
            WethUtils.MMF.safeIncreaseAllowance(address(swapRouter), wethAmt);
            (uint256 _amountA, uint256 _amountB, uint256 _liquidity) = swapRouter.addLiquidity(
                address(yToken),
                address(WethUtils.MMF),
                yTokenAmt,
                wethAmt,
                _minYTokenOut,
                _minWethOut,
    ...
    function cleanDust() external onlyOwner {
        ...
        WethUtils.MMF.safeTransfer(treasury, WethUtils.MMF.balanceOf(address(this)));
    }

    function changeSlippage(uint256 _newSlippage) external onlyOwner {
        require(_newSlippage <= SLIPPAGE_PRECISION, "SwapStrategyPOL::changeSlippage: Invalid slippage");
