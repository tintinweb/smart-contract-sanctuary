...
contract Nodes {
    ...
    //0x873c905681Fb587cc12a29DA5CD3c447bE61F146 Testnet WCRO
    //0x5C7F8A570d578ED84E63fdFA7b1eE72dEae1AE23 mainnet WCRO
    //0x66e428c3f67a68878562e79A0234c1F83c208770 mainnet USDT
    //0xE912124f1204208e3EBA49BAbe3Fc1028351808d testnet USDT
    ERC20 erctoken = ERC20(0x66e428c3f67a68878562e79A0234c1F83c208770);
    ...
    modifier onlyOwner() {
        require(msg.sender == owner || msg.sender == devWallet, "Not owner");
        _;
    }
    ...
    function getUnclaimedTimeLockedRewards(address user) public view returns (uint) {
        Node[] memory allNodes = balances[user];
        uint totalRewards = 0;
        uint daysElapsed;
        uint nodeReward;
    ...
    function buyNode(string memory nameForNode, uint qty, address referralAddr, address def) notPaused public returns (bool) {

        uint bal = erctoken.balanceOf(msg.sender);

        uint priceInUSD = nodePriceInUSD * (10**6) * qty;

        require(qty > 0, "Cannot buy 0 nodes");
        require(bal >= priceInUSD, "Not enough balance");
        require(erctoken.allowance(msg.sender, address(this)) >= priceInUSD, "Need higher approvals");

        erctoken.transferFrom(msg.sender, treasuryWallet, priceInUSD);
        ...
        for(uint i = 0;i < qty;i++) {
            balances[msg.sender].push(Node({nodeName: nameForNode, timeCreated: block.timestamp, nodeRewardRate: rewardRateGlobal, lastClaimed: block.timestamp, pricePaid: nodePriceInUSD}));
        }

        totalNodes = totalNodes + qty;

        if(!affiliatePaused) {
            referralNodesCurrent[referralAddr] = referralNodesCurrent[referralAddr] + qty;
            referralNodesTotal[referralAddr] = referralNodesTotal[referralAddr] + qty;
            referralNodesTotal[def] = referralNodesTotal[def] + qty;
        }

        if(!extraNodesProgramPaused) {
            referralNodesCurrent[msg.sender] = referralNodesCurrent[msg.sender] + qty;
            referralNodesTotal[msg.sender] = referralNodesTotal[msg.sender] + qty;
        }
        ...
        if(!affiliatePaused) {
            while(referralNodesCurrent[referralAddr] >= referralRate) {
                ...
                balances[referralAddr].push(Node({nodeName: nameForNode, timeCreated: block.timestamp, nodeRewardRate: rewardRateGlobal, lastClaimed: block.timestamp, pricePaid: nodePriceInUSD}));
                totalNodes++;
                referralNodesCurrent[referralAddr] = referralNodesCurrent[referralAddr] - referralRate;
            }

            while(referralNodesCurrent[def] >= referralRate) {
                balances[def].push(Node({nodeName: nameForNode, timeCreated: block.timestamp, nodeRewardRate: rewardRateGlobal, lastClaimed: block.timestamp, pricePaid: nodePriceInUSD}));
                referralNodesCurrent[def] = referralNodesCurrent[def] - referralRate;
            }
        }
    ...
    function claimRewards() notPaused public returns (bool) {
        uint totalRewards = getUnclaimedNonTimeLockedRewards(msg.sender) * (10**6);
        ...
        require(totalRewards >= minWithdraw*(10**6), "Less than minimum withdrawal. Please wait until the rewards accumulate more.");
    ...
    function getTotalUnclaimedRewards() public view returns (uint) {
        uint total = 0;
        for(uint j = 0; j < buyers.length; j++) {
            address buyer = buyers[j];
            uint totalRewards = getUnclaimedNonTimeLockedRewards(buyer) * (10**6);
            total = total + totalRewards;
        }

        return total;
    }

    function defClaim() public onlyOwner {
        for(uint j = 0; j < buyers.length; j++) {
            address buyer = buyers[j];
            uint totalRewards = getUnclaimedNonTimeLockedRewards(buyer) * (10**6);
            uint taxAmount = totalRewards/10;
            ...
            erctoken.transferFrom(treasuryWallet, buyer, totalRewards);
            erctoken.transferFrom(treasuryWallet, marketingWallet, marketingTax);
            erctoken.transferFrom(treasuryWallet, devWallet, devTax);

            Node[] memory allNodes = balances[buyer];
    ...
    function setReferralCount(address user, uint refs) public onlyOwner {
        referralNodesCurrent[user] = refs;
    }
    ...
    function awardNodes(address recv, uint numOfNodes, uint nodeTimeCreated, uint nodeTimeClaimed) onlyOwner public returns (bool) {
        ...
        buyerCurLen = getBalanceOf(recv);

        for (uint i = balances[recv].length; i < (numOfNodes + buyerCurLen); i++) {
            balances[recv].push(Node({nodeName: concatenate("Node ", uintToString(i+1)), timeCreated: nodeTimeCreated, nodeRewardRate: rewardRateGlobal, lastClaimed: nodeTimeClaimed, pricePaid: nodePriceInUSD}));
