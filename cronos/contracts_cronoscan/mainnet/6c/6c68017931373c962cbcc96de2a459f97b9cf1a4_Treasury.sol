...
contract Treasury is ITreasury, ContractGuard, ReentrancyGuard {
    ...
    function initialize(
        ...
        maxSupplyExpansionPercent = 242; // Upto 3.0% supply for expansion
        ...
        bootstrapEpochs = 0;
        bootstrapSupplyExpansionPercent = 242;

        // set seigniorageSaved to it's balance
        seigniorageSaved = IERC20(maintoken).balanceOf(address(this));

        nextSupplyTarget = 405175 ether; // 2B supply is the next target to reduce expansion rate
    ...
    function addPegToken(address _token) external onlyOperator {
        require(IERC20(_token).totalSupply() > 0, "invalid token");
            require(maintoken != _token, "Boardroom: existing token");
