...
contract MasterChefForErc20 is Ownable, ReentrancyGuard {
    ...
    function getTokensReserves(address[] memory _pairList) public view returns (pairReservesItem[] memory pairReservesList) {
        ...
        for (uint256 i = 0; i < _pairList.length; i++)
        {
            ...
            pairReservesList[i] = pairReservesItem(pair(_pair).factory(), token0, token1, reserve0, reserve1, IERC20(token0).decimals(), IERC20(token1).decimals(), pair(_pair).decimals(), pair(_pair).totalSupply(), IERC20(token0).symbol(), IERC20(token1).symbol(), IERC20(token0).name(), IERC20(token1).name());
        }
    }
