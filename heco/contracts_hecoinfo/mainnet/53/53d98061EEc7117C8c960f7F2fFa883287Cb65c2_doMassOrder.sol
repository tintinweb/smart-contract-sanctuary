...
contract doMassOrder is Ownable, ReentrancyGuard,structK {
    ...
    function doAction(uint256[] memory orderIdList) external payable {
        ...
        for (uint256 i =0;i<orderIdList.length;i++) {
            ...
            orderItem memory lop = NftMarket(marketAddress).orderItemInfo(orderid);
            uint256 orderId = lop.orderId;
            IERC721Enumerable nftToken = lop.nftToken;
            uint256 tokenId = lop.tokenId;
            address erc20Token = lop.erc20Token;
            uint256 price = lop.price;
