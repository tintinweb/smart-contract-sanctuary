...
contract RT is Ownable {
    ...
    event create(string name, string symbol, address _marketingAddress, uint256 _preSupply, uint256 _maxSupply);

    constructor (string memory name, string memory symbol, address _marketingAddress, uint256 _preSupply, uint256 _maxSupply) public {
        _name = name;
        _symbol = symbol;
        _decimals = 18;
        ...
        emit create(name, symbol, _marketingAddress, _preSupply, _maxSupply);
    ...
    function setFee(uint256 _burnFee, uint256 _marketingFee) external onlyOwner {
        burnFee = _burnFee;
