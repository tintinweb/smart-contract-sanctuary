...
contract RT is Ownable {
    ...
    constructor (string memory name, string memory symbol, address _lpAddress, address _marketingAddress, uint256 _preSupply, uint256 _maxSupply) public {
        _name = name;
        _symbol = symbol;
        _decimals = 18;
        ...
        takeNoFeeList[lpAddress] = true;
        takeNoFeeList[marketingAddress] = true;
        takeNoFeeList[address(1)] = true;
        emit Transfer(address(0), _msgSender(), _totalSupply);
    }

    function setLpAddress(address _lpAddress) external onlyOwner {
        require(_lpAddress != address(0), "e001");
        if (lpAddress != _lpAddress && lpAddress != address(0)) {
            takeNoFeeList[lpAddress] = false;
        }
        lpAddress = _lpAddress;
        takeNoFeeList[lpAddress] = true;
    }
    ...
    function setMarketingAddress(address _marketingAddress) external onlyOwner {
        require(_marketingAddress != address(0), "e001");
        if (marketingAddress != _marketingAddress && marketingAddress != address(0)) {
            takeNoFeeList[marketingAddress] = false;
        }
        marketingAddress = _marketingAddress;
        takeNoFeeList[marketingAddress] = true;
    }
    ...
    function setFee(uint256 _burnFee, uint256 _lpFee, uint256 _marketingFee) external onlyOwner {
        burnFee = _burnFee;
        lpFee = _lpFee;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if (takeNoFeeList[sender] || takeNoFeeList[recipient]) {
            ...
            uint256 transferAmount = amount.sub(burnAmount).sub(lpAmount).sub(marketingAmount);
