...
interface NftMarket is structK {
    ...
    function orderItemInfo(uint256 _orderId) external view returns(uint256, address,IERC721Enumerable,uint256,address,uint256);
...
contract doMassOrder is Ownable, ReentrancyGuard,structK {
    ...
    function doAction(uint256[] memory orderIdList) external payable  {
        ...
        for (uint256 i =0;i<orderIdList.length;i++) {
            ...
            (,,IERC721Enumerable nftToken,uint256 tokenId,address erc20Token,uint256 price) = NftMarket(marketAddress).orderItemInfo(orderId);
