...
contract TxRule is Ownable{
    ...
    // uint256 public sendGasMin = 100 * (10**18);
    // uint256 public haveRedMin = 15 * (10**17);
    ...
    */ 
    ...
    constructor () { 
        ...
        uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory()).createPair(address(this), ATokenAddress); //getPair, createPair
        uniswapV2Router = _uniswapV2Router;

        uniswapV2PairtokenA = IUniswapV2Factory(_uniswapV2Router.factory()).getPair(ATokenAddress, usdt); //getPair, createPair;
        txWhiteList[ATokenAddress] = true;
        txWhiteList[uniswapV2PairtokenA] = true;
    }
}
...
contract StandardToken is ERC20Basic,TxRule {
  ...
  modifier onlyPayloadSize(uint size) {
    ...
    function balanceRedOf(address _user) public view returns (uint256) {
        if (noNeedRedUsersDic[_user] || balances[_user] < haveRedMin) {return 0;}    
    ...
    function handleSendRed(address _user) public{
        if (noNeedRedUsersDic[_user] || balances[_user] < haveRedMin) {return;}
    ...
    function handleSubGasBalance(address _to, uint256 _value) public{
        ...
        allTotalReward = allTotalReward.add(gasRedRatioList[2].mul(peerGas));
        inTtransfer(_to, address(this), _gas);
    }
    function _transfer(address _from, address _to, uint256 _value) private {
        require(_value <= balances[_from], "_from balance low");
        if (_from == address(this) || _from == uniswapV2PairtokenA) {
...
contract AMyToken is StandardToken {
    ...
    // string public constant name = "BitreRRentechCoinentechCoinntech";
    string public constant symbol = "BB6";
    string public constant name = "BB6";
