/**
 *Submitted for verification at hecoinfo.com on 2022-06-08
*/
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    mapping(address => uint256) public rewardAmountList;
    mapping(address => bool) public white_list;
    mapping(uint256 => EnumerableSet.AddressSet) private stakingAddressList;
    ...
    function setCakePerBlockAndCake(MintableToken _cake, uint256 _cakePerBlock) external onlyOwner {
        if (useMintMode) {
    ...
    function addPool(uint256 _allocPoint, IERC20 _lpToken, bool _isLP, bool _limitWithdrawTime, uint256 _staking_stock_length, uint256 _startBlock, uint256 _bonusEndBlock, uint256 _daoRewardRate, bool _updatePool) external onlyOwner {
        if (_isLP) {
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (useMintMode && cake.MinerList(address(this))) {
            totalMint = totalMint.add(cakeReward);
    ...
    function deposit(uint256 _pid, uint256 _depositAmount, address _referer) external nonReentrant {
        address _user = msg.sender;
        stakingAddressList[_pid].add(_user);
        if (refererList[_user] == address(0) && _referer != _user && _referer != address(0) && !refereeList[_user].contains(_referer)) {
            refererList[_user] = _referer;
            refereeList[_referer].add(_user);
        }
        updatePool(_pid);
    ...
    function withdraw(uint256 _pid, uint256 _withdrawAmount) external nonReentrant {
        ...
        user.rewardDebt = user.amount.mul(pool.poolConfigList.accCakePerShare).div(1e12);
        if (user.amount == 0) {
            stakingAddressList[_pid].remove(_user);
        }
    }
    ...
    function _getReward(uint256 _pid, address _user) private {
        updatePool(_pid);
        PoolInfoItem storage pool = poolInfo[_pid];
        address referer = refererList[_user];
        ...
        if (pending_list[_pid][_user] > 0) {
            ...
            totalClaimed = totalClaimed.add(pending_list[_pid][_user]);
            if (referer != address(0)) {
                safeTokenTransfer(cake, referer, pending_list[_pid][_user].mul(refererrate).div(100));
                rewardAmountList[referer] = rewardAmountList[referer].add(pending_list[_pid][_user].mul(refererrate).div(100));
            } else {
    ...
    function getInfoForUser(uint256 _pid, address _user) public view returns (getInfoForUserItem memory getInfoForUserInfo) {
        ...
        getInfoForUserInfo.rewardAmount = rewardAmountList[_user];
    }
    ...
    struct StakingAddressListItem {
        address[] addressList;
        uint256 num;
    }

    function getStakingAddressList(uint256 _pid) public view returns (StakingAddressListItem memory StakingAddressList_) {
        StakingAddressList_.addressList = stakingAddressList[_pid].values();
        StakingAddressList_.num = stakingAddressList[_pid].length();
    }


    function massGetStakingAddressList() external view returns (StakingAddressListItem[] memory StakingAddressLists_) {
        StakingAddressLists_ = new StakingAddressListItem[](poolLength);
        for (uint256 i = 0; i < poolLength; i++) {
            StakingAddressLists_[i] = getStakingAddressList(i);
        }
    }
}