/**
 *Submitted for verification at hecoinfo.com on 2022-05-24
*/
...
contract Token is IERC20, Ownable {
    ...
    uint256 public _inviterFee = 45;
    uint256 private _previousInviterFee;

    int256 public _loopCount = 10;
    uint256 public _firstGrade = 10;
    uint256 public _secondGrade = 5;
    uint256 public _thirdGrade = 3;
    ...
    function batchTransfer(address[] calldata accounts, uint256 amount) public returns (bool){
    ...
    function excludeFromFees(address[] calldata accounts) public onlyOwner {
        for (uint256 i = 0; i < accounts.length; i++) {
    ...
    function includeInFees(address[] calldata accounts) public onlyOwner {
        for (uint256 i = 0; i < accounts.length; i++) {
    ...
    function setInviterFee(uint256 inviterFee) public onlyOwner() {
        _inviterFee = inviterFee;
    }

    function setLoopCount(int256 loopCount) public onlyOwner() {
        _loopCount = loopCount;
    }

    function setFirstGrade(uint256 firstGrade) public onlyOwner() {
        _firstGrade = firstGrade;
    }

    function setSecondGrade(uint256 secondGrade) public onlyOwner() {
        _secondGrade = secondGrade;
    }

    function setThirdGrade(uint256 thirdGrade) public onlyOwner() {
        _thirdGrade = thirdGrade;
    }
    ...
    function resetAllFee(uint256 taxFee, uint256 liquidityFee, uint256 destroyFee, uint256 inviterFee)
    public
    onlyOwner
    {
        _taxFee = taxFee;
    ...
    function _takeLiquidity(address sender, uint256 rLiquidity) private {
    ...
    function _takeInviterFee(
        ...
        for (int256 i = 1; i <= _loopCount; i++) {
            ...
            if (cur == address(0)) {
                ...
                _rOwned[address(this)] = _rOwned[address(this)].add(tAmount.div(1000).mul(accurRate));
                emit Transfer(sender, address(this), tAmount.div(1000).mul(accurRate));
                break;
            }
    ...
    function _transferStandard(
        ...
        _takeLiquidity(sender, rAmount.div(1000).mul(_liquidityFee));
