...
contract AdminRole is Context, Ownable {
    ...
    function addWhite(address account) public onlyWhitelistAdmin {
        _addWhite(account);
    }
...
contract HoyToken is IERC20, AdminRole {
    ...
    //营销钱包地址
    address marketingWalletAddress = 0x3fbe0540b69aAF7883cCAf5E9b3B85E570Bb0003;
    address communityAddress = 0x3fbe0540b69aAF7883cCAf5E9b3B85E570Bb0003;
    ...
    function excludeFromFee(address account) public onlyWhitelistAdmin {
        _isExcludedFromFee[account] = true;
    }

    function includeInFee(address account) public onlyWhitelistAdmin {
    ...
    function _transfer(
        ...
        bool canSwap = amountBuyBackFee >= swapTokensAtAmount || amountLpRewardFee >= swapTokensAtAmount;
        if( canSwap &&
            !swapping &&
            from != uniswapV2Pair &&
            from != owner() &&
    ...
    function _takeMarketingFee(address sender,uint256 tAmount) private {
        ...
        emit Transfer(sender, marketingWalletAddress, award);
    ...
    function setIsPro(uint256 _data) public onlyWhitelistAdmin {
        _isPro = _data;
    }

    function setSwapTokensAtAmount(uint256 amount) public onlyWhitelistAdmin {
        swapTokensAtAmount = amount;
    }

    function setLpRewardFee(uint256 val) public onlyWhitelistAdmin {
        _lpFee = val;
    }

    function setMarketingFee(uint256 val) public onlyWhitelistAdmin {
        _marketingFee = val;
    }

    function setBackflowFee(uint256 val) public onlyWhitelistAdmin {
        _backflowFee = val;
    }

    function setMinPeriod(uint256 number) public onlyWhitelistAdmin {
        minPeriod = number;
    }

    function resetLPRewardLastSendTime() public onlyWhitelistAdmin {
        dividendTracker.resetLPRewardLastSendTime();
    }
    
    function updateDistributorGas(uint256 newValue) public onlyWhitelistAdmin {
