...
contract SwapPoolForAirdrop is Ownable {
    using SafeMath for uint256;
    using Address for address;
    ...
    uint256 public feePrice = 0.01*10**18;
    ...
    function setFeeInfo(IERC20 _feeToken, uint256 _feePrice, address _feeAddress) external onlyOwner {
        feeToken = _feeToken;
        feePrice = _feePrice;
    ...
    function SwapToken(uint256 _tokenId) external {
        feeToken.transferFrom(msg.sender,feeAddress,feePrice);
        require(canSwap,"e0");
    ...
    function SwapTokenList(uint256[] memory _tokenIdList) external {
         feeToken.transferFrom(msg.sender,feeAddress,feePrice.mul(_tokenIdList.length));
        require(canSwap,"e1");
