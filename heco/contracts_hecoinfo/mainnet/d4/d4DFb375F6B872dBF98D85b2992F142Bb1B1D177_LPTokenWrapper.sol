...
contract LPTokenWrapper is Ownable {
    ...
    mapping(address => uint256) public _lastLpHSB;
    mapping(address => uint256) public _isLpHSB;
    mapping(address => uint256) public _myLpHSB;         
    uint256 public _totalLpHSB;
    uint256 public _isTotalLpHSB;       
    ...
    constructor(IERC20 htLPToken, IERC20 usdtLPToken, IERC20 hsbToken) {
    ...
    function myFund() public view returns (uint256) {
        uint256 hsbBalances = _hsbToken.balanceOf(address(this));             
        uint256 totalLpHSB = hsbBalances + _isTotalLpHSB;
        if(_lastLpHSB[msg.sender] > 0 && totalLpHSB >= _lastLpHSB[msg.sender] && _totalPower>0){
            uint256 myLpHSB =  totalLpHSB - _lastLpHSB[msg.sender];
            return _power[msg.sender].mul(myLpHSB).div(_totalPower);                                
        }
        else{
            return 0;
        }
    ...
    function _deposit(uint256 amount, uint256 typeNum, uint256 typeCoin) private {
        ...
        //HSB  LP分红
        uint256 hsbBalances = _hsbToken.balanceOf(address(this));             
        _totalLpHSB = hsbBalances + _isTotalLpHSB;
        if(_lastLpHSB[msg.sender] > 0 && _totalLpHSB >= _lastLpHSB[msg.sender] && _totalPower>0){
            uint256 totalLpHSB =  _totalLpHSB - _lastLpHSB[msg.sender];
            _myLpHSB[msg.sender] += _power[msg.sender].mul(totalLpHSB).div(_totalPower);                                
        }
        _lastLpHSB[msg.sender] = _totalLpHSB;
    ...
    function _withdrawTran(uint256 amount, uint256 typeNum, uint256 listNum) private {
        ...
        if(typeCoin == 1) amount = amount.mul(_htLpPct).div(100);
        if(typeCoin == 2) amount = amount.mul(_usdtLpPct).div(100);        
        if(_power[msg.sender]>= amount)  _power[msg.sender] -= amount;
        if(_totalPower>= amount)  _totalPower -= amount;
        _tokenListStatus[msg.sender][listNum] = 0;
        if(_power[msg.sender] == 0)_lastLpHSB[msg.sender] = 0;         
    }   
    ...
    function _withdrawFund() private{
        require(_myLpHSB[msg.sender] > 1*10**12, "less than withdraw Min");
        require(_myLpHSB[msg.sender]<=_hsbToken.balanceOf(address(this)), "balance not enough");
        require(_openFund, "not open");                
        _hsbToken.safeTransfer(msg.sender, _myLpHSB[msg.sender]);
        _isLpHSB[msg.sender] += _myLpHSB[msg.sender];
        _isTotalLpHSB += _myLpHSB[msg.sender];
        _myLpHSB[msg.sender] = 0;
        emit WithdrawFund(msg.sender, _myLpHSB[msg.sender]);            
    }         
