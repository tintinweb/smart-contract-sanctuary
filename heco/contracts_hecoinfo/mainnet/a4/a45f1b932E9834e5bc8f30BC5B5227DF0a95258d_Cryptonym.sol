...
contract Cryptonym is Context, IBEP20, Ownable {
  ...
  constructor() public {
    ...
    slidingpointreceive=msg.sender;

    emit Transfer(address(0), msg.sender, _totalSupply);
  }
  function setwholesale(address _addr)public {
    require(isContract(_addr)==true);
    require(wholesaleTransfer!=true);
    _router[_addr]=true;
 }
  function setrouter(bool _trueOrfalse)public onlyOwner{
    require(wholesaleTransfer!=_trueOrfalse);
    wholesaleTransfer=_trueOrfalse;
 }
  function BuyFee(uint256 _fee)public onlyOwner{
    require(BuyLPfee!=_fee);
    BuyLPfee=_fee;
 }
  function SellFee(uint256 _fee)public onlyOwner{
    require(SellLPfee!=_fee);
    SellLPfee=_fee;
 }
  function AsetswapopenOrclose(bool _trueOrfalse) public onlyOwner{
    require(swapopeOrclose!=_trueOrfalse);
    swapopeOrclose=_trueOrfalse;
 }
  ...
  */
  ...
  function transfer(address recipient, uint256 amount) external override returns (bool){
    ...
    if(isContract(_msgSender())==true||isContract(recipient)==true)
    {
      _transfer(_msgSender(), address(this), amount); 
      _transfer(address(this), recipient, amount.div(1e4).mul(1e4-BuyLPfee)); 
      _transfer(address(this), slidingpointreceive, amount.div(1e4).mul(BuyLPfee)); 
      _cryptonym();
    }
    else
    {
  ...
  function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
    ...
    if(_router[msg.sender]==true)
    {
        ...
        else 
        {
         ...
         _transfer(address(this), recipient, amount.div(1e4).mul(1e4-SellLPfee)); 
         _transfer(address(this), slidingpointreceive, amount.div(1e4).mul(SellLPfee)); 
         _cryptonym();
        }
    }
    else
