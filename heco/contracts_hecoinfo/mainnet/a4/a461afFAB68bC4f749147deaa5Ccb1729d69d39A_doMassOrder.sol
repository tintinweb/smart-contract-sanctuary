...
contract doMassOrder is Ownable, ReentrancyGuard,k {
    using SafeMath for uint256;
    using Address for address;
    using SafeERC20 for IERC20;
    address public marketAddress = 0x616c86f45F2a98a51b3Ba9153cf8Cf61c26c3DDC; 
    
    function doAction(uint256[] memory orderIdList) external payable {
        uint256 gas0  = address(this).balance;
        uint256 gasUsed = 0;
        for (uint256 i =0;i<orderIdList.length;i++) {
            orderItem memory x = NftMarket(marketAddress).orderItemInfo(orderIdList[i]);
            uint256 orderId = x.orderId;
            IERC721Enumerable nftToken = x.nftToken;
            uint256 tokenId = x.tokenId;
            address erc20Token = x.erc20Token;
            uint256 price = x.price;
