...
contract Token is IERC20, Ownable {
    ...
    constructor(address tokenOwner) {
        ...
        _wy = 210000000000000 * 10 ** _decimals;
    ...
    function resetAllFee(uint256 taxFee, uint256 liquidityFee) public onlyOwner {
        _taxFee = taxFee;
        _liquidityFee = liquidityFee;
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner() && !_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
            if (from == uniswapV2Pair) {
                require(balanceOf(to) + amount <= _tTotal.div(1).mul(_maximum));
            }
        }
    ...
    function _takeDestroyFee(address sender, uint256 tAmount, uint256 currentRate) private {
    ...
    function _takeInviterFee(address sender, address recipient, uint256 tAmount, uint256 currentRate) private {
