...
contract RouterHelper is Ownable {
    ...
    function getout(router[] memory _routerList,address fromToken,address toToken,uint256 fromAmount) external view returns(uint256[] memory outList,address[] memory pairList) {
        outList = new uint256[](_routerList.length);
        pairList = new address[](_routerList.length);
        uint256 out;
        for (uint256 i=0;i<_routerList.length;i++) {
            router x = _routerList[i];
            address Fatory = x.factory();
            address pair_ = fatory(Fatory).getPair(fromToken,toToken);
             if (pair_ != address(0)) {
                 ...
                 if (fromToken == token0) {
                      out = x.getAmountOut(fromAmount,reserve0,reserve1);
                 } else {
                      out = x.getAmountOut(fromAmount,reserve1,reserve0);
                 }
             } else {
                  out = 0;
             }
