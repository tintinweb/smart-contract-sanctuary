...
contract MasterChefForErc20 is Ownable, ReentrancyGuard {
    ...
    mapping(address => uint256) public daoTokenIdForUser;
    ...
    function setDao(dao _daoAddress, address _daoNftToken, bool _useDaoReward) external onlyOwner {
        daoAddress = _daoAddress;
        daoNftToken = _daoNftToken;
        useDaoReward = _useDaoReward;
    }
    ...
    function addPool(uint256 _allocPoint, IERC20 _lpToken, bool _limitWithdrawTime, uint256 _staking_stock_length, uint256 _startBlock, uint256 _bonusEndBlock, uint256 _daoRewardRate, IERC20 _rewardToken, bool _updatePool, bool _onlyRewardMode) external onlyOwner {
        if (_onlyRewardMode) {
    ...
    function setPool(uint256 _pid, IERC20 _lpToken, IERC20 _rewardToken, bool _limitWithdrawTime, bool _updatePool, uint256 _allocPoint, uint256 _staking_stock_length, uint256 _startBlock, uint256 _bonusEndBlock, uint256 _daoRewardRate, bool _onlyRewardMode) external onlyOwner {
        if (_onlyRewardMode) {
            require(_rewardToken != cake, "e001");
        }
        if (_updatePool) {
        ...
        totalAllocPoint = totalAllocPoint.sub(poolItem.poolConfigList.allocPoint).add(_allocPoint);
        poolItem.poolConfigList.allocPoint = _allocPoint;
        poolItem.tokensList.lpToken = _lpToken;
        poolItem.statusList.limitWithdrawTime = _limitWithdrawTime;
        poolItem.statusList.updatePool = _updatePool;
        ...
        poolItem.poolConfigList.refererrate = _daoRewardRate;
        if (!rewardTokenList[_rewardToken].contains(_pid) && _rewardToken != cake) {
            rewardTokenList[_rewardToken].add(_pid);
            rewardTokenList[poolItem.tokensList.rewardToken].remove(_pid);
        }
        poolItem.tokensList.rewardToken = _rewardToken;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (!pool.statusList.onlyRewardMode) {
            ...
            if (!pool.statusList.onlyRewardMode && useMintMode && cake.MinerList(address(this))) {
    ...
    function deposit(uint256 _pid, uint256 _depositAmount, uint256 _tokenId) external nonReentrant {
        updatePool(_pid);
        require(poolInfo[_pid].statusList.pool_status, "e4");
        address _user = msg.sender;
        if (daoTokenIdForUser[_user] == 0 && _tokenId > 0) {
            daoTokenIdForUser[_user] = _tokenId;
        }
        PoolInfoItem storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        if (user.amount > 0) {
        ...
        if (_depositAmount > 0) {
            ...
            pool.tokensList.lpToken.transferFrom(_user, address(this), _depositAmount);
    ...
    function withdraw(uint256 _pid, uint256 _withdrawAmount) external nonReentrant {
        ...
        if (poolInfo[_pid].statusList.limitWithdrawTime) {
            if (!useWhiteList) {
                require(block.timestamp > unlock_time[_pid][_user], "e10");
            } else {
                if (!white_list[_user]) {
                    require(block.timestamp > unlock_time[_pid][_user], "e11");
                }
            }
        }
        PoolInfoItem storage pool = poolInfo[_pid];
        ...
        //     }
        // }
        pool.tokensList.lpToken.transferFrom(address(this), _user, _withdrawAmount);
    ...
    function _getReward(uint256 _pid, address _user) private {
        updatePool(_pid);
        PoolInfoItem storage pool = poolInfo[_pid];
        address referer = daoAddress.stakingNftOlderOwnerList(daoNftToken, daoTokenIdForUser[_user]);
        ...
        if (pending_list[_pid][_user] > 0 && !pool.statusList.onlyRewardMode) {
            ...
            safeTokenTransfer(cake, _user, pending_list[_pid][_user].mul(leftrate).div(100));
            if (referer != address(0) && useDaoReward) {
                safeTokenTransfer(cake, referer, pending_list[_pid][_user].mul(refererrate).div(100));
            } else {
                safeTokenTransfer(cake, address(1), pending_list[_pid][_user].mul(refererrate).div(100));
            }
        ...
        if (pool.tokensList.rewardToken != cake && pending_list2[_pid][_user] > 0) {
            ...
            uint256 refererrate = pool.poolConfigList.refererrate;
            leftrate = uint256(100).sub(refererrate);
            allrewardList2[_pid][_user] = allrewardList2[_pid][_user].add(pending_list2[_pid][_user].mul(leftrate).div(100));
            safeTokenTransfer(pool.tokensList.rewardToken, _user, pending_list2[_pid][_user].mul(leftrate).div(100));
            if (referer != address(0) && useDaoReward) {
                safeTokenTransfer(pool.tokensList.rewardToken, referer, pending_list2[_pid][_user].mul(refererrate).div(100));
            } else {
                safeTokenTransfer(pool.tokensList.rewardToken, address(1), pending_list2[_pid][_user].mul(refererrate).div(100));
            }
