...
contract MasterChefForErc20 is Ownable, ReentrancyGuard {
    ...
    function addPool(uint256 _allocPoint, IERC20 _lpToken, bool _limitWithdrawTime, uint256 _staking_stock_length, uint256 _startBlock, uint256 _bonusEndBlock, uint256 _daoRewardRate, IERC20 _rewardToken, bool _updatePool) external onlyOwner {
        if (_updatePool) {
            massUpdatePools();
        }
    ...
    function setPool(uint256 _pid, IERC20 _lpToken, IERC20 _rewardToken, bool _limitWithdrawTime, bool _updatePool, uint256 _allocPoint, uint256 _staking_stock_length, uint256 _startBlock, uint256 _bonusEndBlock, uint256 _daoRewardRate) external onlyOwner {
        if (_updatePool) {
            massUpdatePools();
        }
