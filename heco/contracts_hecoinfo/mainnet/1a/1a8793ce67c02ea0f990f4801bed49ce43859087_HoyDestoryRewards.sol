...
contract HoyDestoryRewards is Util, WhitelistAdminRole,CoinTokenWrapper {
    ...
    function destroyHoy() public updateReward(msg.sender) checkStart {
        ...
        if (reward > 0) {
            uint staticReward = reward;
            //累计用户静态挖矿收益
            user.hisHoyAward = user.hisHoyAward.add(staticReward);
            
            rewards[msg.sender] = 0;
            //erc.transfer(destroyAddr, staticReward);
            
            uint256 tokenValueUsdt = getTokenPrice(address(erc),usdtAddr,staticReward);
    ...
    function getMiningInfo(address _user) public view returns(uint[17] memory ct,string memory inviteCode, string memory referrer) {
    ...
    function getTokenInfo(address _token) public view returns (uint[3] memory ct,bool _support)
    {
        User memory userInfo = userRoundMapping[rid][msg.sender];
