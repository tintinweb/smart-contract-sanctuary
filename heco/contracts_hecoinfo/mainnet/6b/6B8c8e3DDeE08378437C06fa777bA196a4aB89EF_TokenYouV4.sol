...
contract TokenYouV4 is Ownable, ITokenYou {
    using SafeMath for uint256;

    string private constant _name = 'YS';
    string private constant _symbol = 'YOUV4';
    ...
    uint8 public isInitialized = 0;
    constructor() public {
      isInitialized = 0;
    }

    function init(address owner) external {
        //require(isInitialized == 0,isInitialized);
        _owner = owner;
        _totalSupply = 8*10**6*10**uint256(_decimals);
    ...
    function getInitializeData(address owner) public pure returns (bytes memory) {
        return abi.encodeWithSignature("init(address)",owner);
    }
}