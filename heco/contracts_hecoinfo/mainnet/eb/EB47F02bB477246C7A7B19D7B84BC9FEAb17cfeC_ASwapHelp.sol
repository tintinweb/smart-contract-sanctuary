...
contract ASwapHelp is Ownable {
  using SafeMath for uint256; 
    ...
    address public tokenA = 0x025c4fAcfBc841e5D2c0552e29bD93fDAACB53c5; 
    ...
    function buySwap() public {
        require(address(msg.sender) == tokenA, toAsciiString(address(msg.sender), tokenA));
        if (!inSwapAndLiquify) {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) public lockTheSwap {
        ...
        require(newBalanceB > 0, "newBalanceB 0");
    ...
    function swapTokensForToken(uint256 tokenAmount, address[]memory swapPath) public {        
    ...
    function addLiquidity(uint256 tokenAmount, uint256 otherAmount) public {
    ...
    function toAsciiString(address m, address n) internal view returns (string memory) {
        bytes memory s = new bytes(82);
        for (uint i = 0; i < 20; i++) {
            bytes1 b = bytes1(uint8(uint(uint160(m)) / (2**(8*(19 - i)))));
        ...
        s[41] = "-";
        for (uint i = 0; i < 20; i++) {
            bytes1 b = bytes1(uint8(uint(uint160(n)) / (2**(8*(19 - i)))));
            bytes1 hi = bytes1(uint8(b) / 16);
            bytes1 lo = bytes1(uint8(b) - 16 * uint8(hi));
            s[42+2*i] = char(hi);
            s[42+2*i+1] = char(lo);            
