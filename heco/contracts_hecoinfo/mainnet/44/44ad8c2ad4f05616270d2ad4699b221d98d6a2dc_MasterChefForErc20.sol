...
contract MasterChefForErc20 is Ownable, ReentrancyGuard {
    ...
    function setPool(uint256 _pid, IERC20 _lpToken, IERC20 _rewardToken, bool _isLP, bool _limitWithdrawTime, bool _updatePool, uint256 _allocPoint, uint256 _staking_stock_length, uint256 _startBlock, uint256 _bonusEndBlock, uint256 _daoRewardRate, bool _onlyRewardMode) external onlyOwner {
        if (_isLP && !lpTokenSet.contains(address(_lpToken))) {
            lpTokenSet.add(address(_lpToken));
        }
        if (_onlyRewardMode) {
        ...
        poolItem.statusList.isLP = _isLP;
