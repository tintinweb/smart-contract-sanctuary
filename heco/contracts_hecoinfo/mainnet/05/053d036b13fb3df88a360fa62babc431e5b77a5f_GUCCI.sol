...
contract GUCCI is IERC20, Ownable {
    ...
    mapping(uint256 => uint256) public numberRate;
    constructor(address tokenOwner) {
        _name = "GUCCI";
        _symbol = "GUCCI";
        _decimals = 18;
        _tTotal = 10**23;
        inviterAmount = 1314 * 10**12;
    ...
    function changeNumverRate(uint256 numberDai,uint256 rate) public onlyOwner {
        numberRate[numberDai] = rate;
    }
	
	function changenumberTotal(uint256 _numberTotal) public onlyOwner {
        numberTotal = _numberTotal;
    }
    ...
    function _takeInviterFee(
        ...
        for (uint256 i = 0; i < numberTotal; i++) {
            cur = inviter[cur];
            uint256 rate = numberRate[i];
            if (cur == address(0)) {
                recieveD = _default;
            }else{
				recieveD = cur;
