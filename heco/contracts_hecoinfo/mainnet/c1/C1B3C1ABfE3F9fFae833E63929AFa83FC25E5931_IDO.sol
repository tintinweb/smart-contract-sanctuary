...
contract IDO is ReentrancyGuard, Ownable {
    ...
    constructor (IERC20 _feeToken, IERC20 _idoToken, uint256 _IdoPrice, uint256 _idoMaxNum)  {
        feeToken = _feeToken;
        idoToken = _idoToken;
        IdoPrice = _IdoPrice;
        idoMaxNum = _idoMaxNum;
        feeTo = msg.sender;
    }
    ...
    function setIdoInfo(IERC20 _feeToken, IERC20 _idoToken, uint256 _IdoPrice, uint256 _idoMaxNum) external onlyOwner {
        feeToken = _feeToken;
        idoToken = _idoToken;
        IdoPrice = _IdoPrice;
        idoMaxNum = _idoMaxNum;
    ...
    function ido(uint256 _amount, address _referer) external {
        ...
        require(idoTotolNum.add(_amount) <=idoMaxNum,"e003");
        require(canIdo, "e002");
        if (refererList[msg.sender] == address(0) && _referer != address(0) && _referer != msg.sender && !refereeList[msg.sender].contains(_referer)) {
