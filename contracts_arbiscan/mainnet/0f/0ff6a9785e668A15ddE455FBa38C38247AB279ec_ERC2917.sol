...
contract ERC2917 is IERC2917, Initializable {
    ...
    struct UserStakeInfo {
        uint256 amount; // LP tokens the user has provided.
        ...
        )
    {
        ...
        if (userStakeInfo.amount > 0) {
            pending = userStakeInfo.amount.mul(accAmountPerShare).div(1e12).sub(
        ...
        )
    {
        ...
        _update();
        uint256 pending =
            userStakeInfo.amount.mul(accAmountPerShare).div(1e12).sub(userStakeInfo.rewardDebt);
        totalInterestPaid = totalInterestPaid.add(pending);
    ...
    function take() public view override returns (uint256) {
        return takeWithAddress(msg.sender);
    }

    // Returns how much a user could earn plus the giving block number.
    function takeWithBlock() public view override returns (uint256, uint256) {
    ...
    function getProductivity(address user) external view override returns (uint256, uint256) {
