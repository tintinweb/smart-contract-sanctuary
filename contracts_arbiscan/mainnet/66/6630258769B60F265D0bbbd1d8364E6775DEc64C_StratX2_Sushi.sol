...
abstract contract StratX2 is Ownable, ReentrancyGuard, Pausable {
    ...
    address public uniRouterAddress; // uniswap, pancakeswap, sushiswap etc

    address public wbnbAddress;
    address public pieFarmAddress;
        ...
        returns (uint256)
    {
        ...
        IERC20(wantAddress).safeTransfer(pieFarmAddress, _wantAmt);
    ...
    function buyBack(uint256 _earnedAmt) internal virtual returns (uint256) {
        if (buyBackRate <= 0 || _earnedAmt == 0) {
            return _earnedAmt;
        }

        uint256 buyBackAmt = _earnedAmt.mul(buyBackRate).div(buyBackRateMax);

        if (earnedAddress == PIEAddress) {
...
// File contracts/StratX2_Sushi.sol

// SPDX-License-Identifier: MIT

pragma solidity 0.6.12;

interface IMiniChefFarm {
    function deposit(uint256 pid, uint256 amount, address to) external;
    function withdrawAndHarvest(uint256 pid, uint256 amount, address to) external;
}

contract StratX2_Sushi is StratX2 {
    address[2] public earnedRewardAddresses;
    address[] public earned1ToEarnedPath;
    address[] public earned2ToEarnedPath;
    bool public isDoubleReward = false;
    ...
    function init (
        ...
        pieFarmAddress = _addresses[2];
        PIEAddress = _addresses[3];
        ...
        transferOwnership(pieFarmAddress);
    }

    function initEarnedRewards(bool _isDoubleReward,
        address[2] memory _earnedRewardAddresses,
        address[] memory _earned1ToEarnedPath,
        address[] memory _earned2ToEarnedPath) public {

        isDoubleReward = _isDoubleReward;
        earnedRewardAddresses = _earnedRewardAddresses;
        earned1ToEarnedPath = _earned1ToEarnedPath;
        earned2ToEarnedPath = _earned2ToEarnedPath;
    }

    function _isDoubleReward(bool _isDoubleReward) public {
        isDoubleReward = _isDoubleReward;
    }
    ...
    function _farm() internal override {
        ...
        IMiniChefFarm(farmContractAddress).deposit(pid, wantAmt, address(this));
    }

    function _unfarm(uint256 _wantAmt) internal override {
        IMiniChefFarm(farmContractAddress).withdrawAndHarvest(pid, _wantAmt, address(this));
        if (_wantAmt == 0) {
            // only harvest
            _before_earn();
        }
    ...
    function _before_earn() internal {
        if (onlyGov) {
            require(msg.sender == govAddress, "!gov");
        }

        if (isDoubleReward) {
            // Harvest earnedRewards
            _unfarm(0);

            // Converts farm 1 tokens to earnedAddress
            address earned1Address = earnedRewardAddresses[0];
            uint256 earned1Amt = IERC20(earned1Address).balanceOf(address(this));
            if (earned1Amt > 0) {
                IERC20(earned1Address).safeApprove(uniRouterAddress, 0);
                IERC20(earned1Address).safeIncreaseAllowance(
                    uniRouterAddress,
                    earned1Amt
                );
                _safeSwap(
            ...
            // Converts farm 2 tokens to earnedAddress
            address earned2Address = earnedRewardAddresses[1];
