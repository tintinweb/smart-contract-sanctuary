/**
 *Submitted for verification at arbiscan.io on 2022-01-03
*/
...
contract PublicKeeper {
    Arb constant arb = Arb(0xcEAF62Ba209e2FB7990D29c5f5157377D54FC7b2);

    receive() external payable {}

    function findSmallestQty(address[] memory bamms, uint maxEthQty, uint minEthQty, uint minProfitInBps) public returns(uint, address, uint) {
        for(uint j = 0 ; j < 2 ; j++)
        {
            ...
            for(uint i = 0 ; i < bamms.length ; i++) {
                ...
                for(uint qty = maxEthQty ; qty > minEthQty ; qty = qty / 2) {
    ...
    function doArb(uint qty, address bamm, uint uniFee) public payable {
        arb.swap{value: msg.value}(qty, bamm, uniFee);
