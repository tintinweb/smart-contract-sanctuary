...
contract EggToken is BEP20("Dragon Egg", "DRAGON") {
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "DRAGON::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "DRAGON::delegateBySig: invalid nonce"
        );
        require(now <= expiry, "DRAGON::delegateBySig: signature expired");
        ...
        returns (uint256)
    {
        require(
            ...
            "DRAGON::getPriorVotes: not yet determined"
        );
    ...
    function _delegate(address delegator, address delegatee) internal {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying DRAGONs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            "DRAGON::_writeCheckpoint: block number exceeds 32 bits"
        );
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    event UpdateEmissionRate(address indexed user, uint256 EggPerBlock);
    event addPool(
        uint256 indexed pid,
        address lpToken,
        uint256 allocPoint,
        ...
        returns (uint256)
    {
        ...
        if (
            ...
            accEggPerShare = accEggPerShare.add(
                eggReward.mul(1e18).div(pool.lpSupply)
            );
        }
        return
            user.amount.mul(accEggPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function withdraw(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accEggPerShare).div(1e18).sub(
            user.rewardDebt
        );
        if (pending > 0) {
            safeEggTransfer(msg.sender, pending);
    ...
    function updateEmissionRate(uint256 _EggPerBlock) external onlyOwner {
        require(_EggPerBlock <= MAX_EMISSION_RATE, "Emission too high");
        massUpdatePools();
        EggPerBlock = _EggPerBlock;
