...
interface StabilizeStrategy {
    function updateExchangeRates() external; // Unique to the Impermax vaults
...
contract zsToken is ERC20("Stabilize Arbitrum Strategy Impermax USDC", "zs-imUSDC"), Ownable, ReentrancyGuard {
    ...
    function pricePerTokenLast() public view returns (uint256) {
        if(totalSupply() == 0){
    ...
    function pricePerToken() public returns (uint256) {
        if(totalSupply() == 0){
            return 1e18; // Shown in Wei units
        }else{
            if(currentStrategy != StabilizeStrategy(address(0))){
                StabilizeStrategy(currentStrategy).updateExchangeRates();
            }
            return uint256(1e18).mul(valueOfVaultAndStrategy()).div(totalSupply());      
        }
    }
