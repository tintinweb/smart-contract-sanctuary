...
contract GetMakerts is Ownable {
    ...
    address public COMPTROLLER=0xD5B649c7d27C13a2b80425daEe8Cb6023015Dc6B;
    address public ORACLE=0x87746c74A0f2143f16872d0D563c9136576f0D31;
    ...
    function getUnderlyingTokenAddress(address _pToken) public view returns (address){
        if (_pToken == 0xBC4a19345c598D73939b62371cF9891128ecCB8B) {
            return address(0);
        }else{
    ...
    function getSymbol(address _tokenAddress) public view returns (string memory) {
        return _tokenAddress == address(0)? 'ETH' : UERC20(_tokenAddress).symbol();
    }
    ...
    function getMarketDetail(address _account) public view returns (PoolInfo[] memory){
        address[] memory _PTokens = getAllPTokens();
        uint256 len = _PTokens.length;
        PoolInfo[] memory results = new PoolInfo[](len);
        for(uint256 i = 0 ; i < len; i++){
            results[i] = PoolInfo({
                pTokenAddress: _PTokens[i],
                underlyingAddress: getUnderlyingTokenAddress(_PTokens[i]),
                symbol: getSymbol(getUnderlyingTokenAddress(_PTokens[i])),
                supplyApy: getSupplyApy(_PTokens[i]),
                borrowApy: getBorrowApy(_PTokens[i]),
                underlyingAllowance:getAllowance(getUnderlyingTokenAddress(_PTokens[i]), _account, _PTokens[i]),
                walletBalance: getBalanceOf(getUnderlyingTokenAddress(_PTokens[i]), _account),
                marketTotalSupply: getMarketTotalSupplyInTokenUnit(_PTokens[i]),
                marketTotalBorrowInTokenUnit: getMarketTotalBorrowInTokenUnit(_PTokens[i]),
                underlyingAmount: getUnderlyingAmount(_PTokens[i]),
                underlyingPrice: getUnderlyingPrice(_PTokens[i]),
                collateralFactor: getCollateralFactor(_PTokens[i]),
                pctSpeed: getPctSpeed(_PTokens[i]),
                decimals: getDecimals(getUnderlyingTokenAddress(_PTokens[i])),
                cTokenBalance: getCTokenBalance(_PTokens[i],_account),
                borrowBalance: getBorrowBalance(_PTokens[i],_account),
                exchangeRateMantissa: getExchangeRateMantissa(_PTokens[i],_account),
