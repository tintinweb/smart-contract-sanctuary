...
// File: contracts/strategies/StabilizeStrategyMIMV1.sol

pragma solidity =0.6.6;

// This is a strategy that takes advantage of arb opportunities for mim, usdc and usdt
// Strat will constantly trade between tokens using Curve
...
contract StabilizeStrategyMIMV1 is Ownable {
    using SafeMath for uint256;
    ...
    */
    ...
    function getCheaperToken() internal view returns (uint256) {
        ...
        for(uint256 i = 1; i < tokenList.length; i++){
            estimate = simulateExchange(0,i,mainAmount);
    ...
    function estimateMaxSellAmount(uint256 originID, uint256 targetID, uint256 _tokenBalance) internal view returns (uint256) {
        ...
        for(uint256 i = 0; i < length; i++){
            if(i != targetID){
                ...
                if(_bal <= _minTradeTarget){
                    ...
                    sellBalance = estimateMaxSellAmount(i, targetID, _bal);
                }
    ...
    function executorSwapTokens(address _executor, uint256 _minSecSinceLastTrade, uint256 _deadlineTime, uint256 _buyID) external {
