...
contract BlueParrotEgg is Ownable, ERC20("Blue Parrot Egg", "BPEGG") {
    ...
    function incubator(uint256 _amount) public {
        require (block.number >= startBlock || msg.sender == owner(), 'BlueParrotEgg: wait to startBlock or become the owner');
        require (_amount >= 2000000000000000000, 'BlueParrotEgg: you have to use at least 2 PPEGG');
        require (_amount.mod(2) == 0, 'BlueParrotEgg: please, only even numbers');
    ...
    function updateStartBlock(uint256 _newStartBlock) external onlyOwner {
        require(block.number < startBlock, "cannot change start block if minting has already started");
