...
contract DOGUE_MAGAZINE is ERC721Enumerable, Ownable {
    ...
    uint256 private maxMagazines = 20; //Initial Batch
    uint256 private minDogueTokens = 10*(10**18); //10
    ...
    constructor() ERC721("Dogue Magazine", "DogueNFT")  {
    ...
    function addDogueBatch(uint256 batchSize, bool andPublish) public onlyOwner(){
        maxMagazines += batchSize;
        if (andPublish)
            publishMagazine(batchSize);
    }

    function setDogueTokenAddress(address dogueTokenContract) external onlyOwner() { 
        dogueTokenAddress = address(dogueTokenContract);
    }

    function setMinDogueTokens(uint256 newLimit) public onlyOwner() {
        minDogueTokens = newLimit*(10**18); //$DOGUE Decimals
    }
    ...
    function publishMagazine(uint256 copies) public payable {
        ...
        require(supply + copies <= maxMagazines,                                        "Exceeds current maximum Dogue supply");
        ...
        if (allowPublicMint && msg.sender != owner() && msg.sender != communityFund) {
            ...
            require(msg.sender == owner() || msg.sender == communityFund,               "Not allowed. Public minting is not enabled yet");
        }
    ...
    function setWalletsBackup(address marketing, address newFund) external onlyOwner() { 
        marketingWallet = address(marketing);
        communityFund = address(newFund);
    }
