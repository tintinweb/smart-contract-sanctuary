...
// LGE to bootstrap minimum viable liquidity
contract LiquidityGenerationEvent is Deployer, UniswapV2AddLiquidityHelperV1 {

    mapping (address => bool) public isWhitelisted;
    mapping (address => uint256) public userContribution;
    uint256 internal _LGEStartTime;
    uint256 internal _LGEFinishTime;
    bool internal _LGEInitiated;

    uint256 public maxContribution = 20 ether;
    uint256 public totalContribution;

    address public arbiStakeAddress;
    bool private _tokenAddressGiven = false;

    function LGEActive() public view returns(bool) {
        if (block.timestamp > _LGEStartTime && _LGEFinishTime > block.timestamp) {
            return true;
        }
        return false;
    }
    ...
    function whitelistUsers(address[] memory _addresses) public onlyDeployer {
        for (uint i=0; i< _addresses.length; i++) {
            isWhitelisted[_addresses[i]] = true;
        }
    }

    function getLGEStartTime() external view returns(uint256) {
        return _LGEStartTime;
    }

    function getLGEFinishTime() external view returns(uint256) {
        return _LGEFinishTime;
    }

    function isLGEInitiated() external view returns(bool) {
        return _LGEInitiated;
    }

    function startLGE() public payable onlyDeployer {
        require(!_LGEInitiated, "You can only start the LGE once.");
        
        require(address(this).balance > 0);
        uint toMint = address(this).balance;
        IERC20(arbiStakeAddress).mint(address(this), toMint);
        IERC20(arbiStakeAddress).approve(UNIROUTER, toMint);
        Uniswap(UNIROUTER).addLiquidityETH{ value: address(this).balance }(arbiStakeAddress, toMint, 1, 1, arbiStakeAddress, 33136721748); //seeds small liquidity and sets the price

        _LGEStartTime = block.timestamp;
        _LGEFinishTime = block.timestamp + 2 hours; // LGE lasts for two hours;
...
contract Staker is LiquidityGenerationEvent {
    ...
    uint private _tokenRewardValue = 25*10**20; //2.5x that of lp staking
    ...
    function stake() public payable {
        ...
        require(_LGEInitiated, "LGE has not been initialized yet.");
        if (LGEActive()) {
            require(isWhitelisted[msg.sender], "User is not whitelisted to prestake in LGE.");
            userContribution[staker] = userContribution[staker].add(msg.value); 
            require(userContribution[staker] <= 1 ether, "Maximum contribution during LGE is 1 eth.");
            require(msg.value.add(totalContribution) <= maxContribution, "Contribution exceeds limit of LGE.");
        ...
        if (LGEActive()) { // prestakers begin earning rewards immediately once LGE finishes. 
            timePooled1[staker] = _LGEFinishTime;
            internalTime1[staker] = _LGEFinishTime;
        }
        else {
    ...
    function stakeToken(uint256 amount) public {
        require(_LGEFinishTime < block.timestamp, "LGE has not finished.");
    ...
    function viewRecentRewardTokenAmountFromETH(address who) internal view returns (uint){
        if (internalTime1[who] > block.timestamp) { // the only instance where internalTime is greater than current time is LGE. 
            return 0;
        }
    ...
    function earnCalc(uint LPTime) internal view returns(uint){
        return ( rewardValue().mul(LPTime)  ) / ( 31557600 * DECIMAL );
    }

    function earnCalc2(uint LPTime) internal view returns(uint){
