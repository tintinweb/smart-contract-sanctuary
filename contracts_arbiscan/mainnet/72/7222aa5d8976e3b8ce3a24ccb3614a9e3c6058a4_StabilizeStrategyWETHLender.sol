...
// File: contracts/strategies/StabilizeStrategyWETHLender.sol

pragma solidity =0.6.6;

// This is a strategy is a bank of WETH that can be used for all purpose flash loans
// Contract creators simply call the flashloan function to borrow a certain amount that must be paid back before transaction end
// Flash loan fee is split between buyback and depositors
...
contract StabilizeStrategyWETHLender is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    */
    ...
    function flashLoan(
        ...
        // 4. take amount + fee from receiver, then check balance
        ...
        uint256 buyAmount = totalFee.mul(DIVISION_FACTOR.sub(percentDepositor)).div(DIVISION_FACTOR);
        if(buybackActive == true){
            doSTBZBuyback(buyAmount); // Buy STBZ with the WETH
        }else{
            tokenList[0].token.safeTransfer(treasuryAddress, buyAmount); // Send to the dev treasury 
        }
    ...
    function doSTBZBuyback(uint256 _amount) internal {
        ...
        tokenList[0].token.safeApprove(address(router), _amount);
