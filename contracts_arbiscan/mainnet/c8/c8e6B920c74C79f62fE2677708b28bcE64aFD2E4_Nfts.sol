...
contract Nfts is NFTokenEnumerableMetadata, Ownable {
	...
	uint256 public priceStep = 0.03 ether;
	uint256 public maxSupply = 100;
	string public baseUri = 'https://arbi.rocks/tokens.php?id=';
	uint16 public refShare = 1000; // in basis points, so it's 10%
	uint256 public startTime = 1632765600;
	...
	constructor() {
		...
		super._setTokenUri(
			0,
			string(abi.encodePacked(baseUri, toString(0)))
		);
		
		super._mint(msg.sender, 1);
		super._setTokenUri(
			1,
			string(abi.encodePacked(baseUri, toString(1)))
		);
	}
	
	function getNextPrice() internal returns (uint) {
	    return price + priceStep * this.totalSupply();
	}
