...
///
/// This contract was inspired by Chef Nomi's 'MasterChef' contract & Alchemix updates which can be found in this
/// repository: https://github.com/alchemix-finance/alchemix-protocol
...
contract StakingPools is ReentrancyGuard, Ownable {
  ...
  /// @dev The token which will be awarded as a reward for staking.
  ...
  constructor(
    ...
    require(address(IERC20(_reward)) != address(0), "StakingPools: rewards address cannot be 0x0");
    reward = _reward;
  }
  ...
  function createPool(IERC20 _token) external onlyOwner returns (uint256) {
  ...
  function setRewardWeights(uint256[] calldata _rewardWeights) external onlyOwner {
  ...
  function _claim(uint256 _poolId) internal {
    ...
    // Added to support for recovering Rewards
    function recoverRewards() external onlyOwner {
        uint256 amount = IERC20(reward).balanceOf(address(this));
        IERC20(reward).safeTransfer(owner(), amount);
    }
