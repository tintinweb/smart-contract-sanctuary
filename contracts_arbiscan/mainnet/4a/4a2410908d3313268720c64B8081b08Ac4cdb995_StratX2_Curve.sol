...
// File contracts/StratX2_Curve.sol

// SPDX-License-Identifier: MIT

pragma solidity 0.6.12;

interface ICurveFarm {
    function deposit(uint256 _amount) external;
    function withdraw(uint256 _value, bool claim) external;
...
contract StratX2_Curve is StratX2 {
    ...
    function _farm() internal override {
        ...
        ICurveFarm(farmContractAddress).deposit(wantAmt);
    }

    function _unfarm(uint256 _wantAmt) internal override {
        ICurveFarm(farmContractAddress).withdraw(_wantAmt, false);
    }

    function _claimRewards() internal {
        ICurveFarm(farmContractAddress).claim_rewards(address(this)); 
    }

    function _addLiquidity() internal {
        uint256 balOfToken0 = IERC20(token0Address).balanceOf(address(this));
        ICurvePool(curvePool).add_liquidity([balOfToken0, 0], 0);
    }
