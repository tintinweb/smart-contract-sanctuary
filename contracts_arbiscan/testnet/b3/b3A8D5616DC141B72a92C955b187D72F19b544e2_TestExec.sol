...
contract TestExec {
    ...
    constructor(IRelayHub _hub) {
        setRelayHub(_hub);
    }

    function setRelayHub(IRelayHub _hub) public {
        hub = _hub;
    }

    function exec(
        address target,
        bytes calldata callData) public {

        (bool success, bytes memory res) = target.call(callData);
        emit Exec(success, false, res);
    }
    ...
    fallback() external {
        ...
        // out and outsize are 0 because we don't know the size yet.
            let result := call(gas(), hub1, 0, 0, calldatasize(), 0, 0)
