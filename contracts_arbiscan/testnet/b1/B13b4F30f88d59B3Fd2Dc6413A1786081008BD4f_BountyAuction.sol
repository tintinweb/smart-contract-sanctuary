...
contract PricingSession is ReentrancyGuard {
    ...
    /* ======== UINT ======== */
    /// @notice this value determines the risk current multiplier 
    /// @dev A users stakes gets harvested at a rate of (margin of error - 5%) * riskFactor
        ...
        for(uint a = 0; a < defender; a++) { 
            ...
            for(uint i = 0; i < rounds; i++) { 
                for(uint j = 0; j < 20; j++) {
                    ...
                    if(sessionMap.nftVotes[voters[voterCount]].appraisal > finalAppraisal) { 
            ...
            uint stdev = sqrtLibrary.sqrt(totalValue/sessionCore.uniqueVoters);
            voterCount = 0;
            uint upper = PostSessionLibrary.calculateBound(finalAppraisal, stdev, defender);
            uint weight;
            voterCount = 0;
            for(uint i = 0; i < rounds; i++) {
                for(uint j = 0; j < 20; j++) {
    ...
    */
    ...
    function _harvest(address _user, address nftAddress, uint tokenid) internal {
        ...
        // Treasury.updateTotalProfitsGenerated(sessionMap.amountHarvested[_user]);
    ...
    */
    ...
    function _claim(address _user, address nftAddress, uint tokenid) internal {
        ...
        if(sessionCheck.correct * 100 / (sessionCheck.correct + sessionCheck.incorrect) >= (95-spread)) {
