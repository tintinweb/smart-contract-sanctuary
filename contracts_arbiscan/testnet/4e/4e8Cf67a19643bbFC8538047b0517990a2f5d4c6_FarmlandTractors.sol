...
contract FarmlandTractors is ERC721, Ownable, ReentrancyGuard, FarmlandCollectible {
    ...
    uint256 public priceWithFarmer;
        ...
        nonReentrant 
    {
        ...
        if ( farmers.balanceOf(msg.sender)>0 || farm.getFarmCollectibleTotalOfType(msg.sender, CollectibleType.Farmer)>0 ) {
            require( numTokens < 21,                                                           "With a farmer, you can mint a maximum of 20" );
            require( msg.value >= priceWithFarmer * numTokens,                                 "Ether sent is not correct" );
        } else {
            require( numTokens < 11,                                                           "Without a farmer, you can mint a maximum of 10" );
        ...
        override 
    {
        ...
        collectibleTrait.trait1 = randMod(300).add(100);    // power from 1 to 4 (100-400 so div by 100)
        collectibleTrait.trait2 = randMod(35).add(65);      // weight from 1 to 0.65 (65 to 100 div by 100)
        collectibleTrait.trait3 = randMod(70).add(80);      // grip from 0.8 to 1.5 (road tires to race tires) (80 to 150 div by 100)

        uint256 collectibleExpires = randMod(100);
        if (collectibleExpires > 50) { // 50% of the collectibles expire
            collectibleTrait.expiryDate = now.add(90 days).add(randMod(640 days)); // pseudo random between 90 days & 2 years
        }
    }

    function initialize(string memory uri_, uint256 reserved_, uint256 maxSupply_, uint256 price_, address payable farmersAddress_, address cornAddress_) 
        external
        override
        onlyOwner
        initializer
    {
        require(cornAddress_ != address(0),    "Invalid Corn Address");
        require(farmersAddress_ != address(0), "Invalid Farmers NFT Address");
        setBaseURI(uri_);
        reserved = reserved_;
        maxSupply = maxSupply_;
        price = price_;
        priceWithFarmer = price_.div(2);
        farm = ICropV2(cornAddress_);
        farmers = FarmlandCollectible(farmersAddress_);
    }
}
