...
contract CrazyCarl is ERC1155, Ownable {
	...
	bytes32 public root = 0x96c4f03a11be35af3b16b3316c4d2ebcbc0ea75856070110e3761d333d6df067;

	string public _baseURI = "https://xxxxxxxxxxx/metadata/";
	string public _contractURI = "https://xxxxxxxxx/xxxxxx.json";
	...
	bool public publicSaleEnabled = false; //enable or disable buying a token without being in a whitelist
	uint256 public saleStartTime = 1643666400; //1st Feb 2022, 0AM UTC

	constructor() ERC1155(_baseURI) {
		_mint(msg.sender, 1, 1, ""); //replace this with whatever
	}
	...
	//enable or disable buying a token without being in a whitelist
	function setPublicSale(bool _isEnabled) external onlyOwner {
		publicSaleEnabled = _isEnabled;
	}

	//can add a new tier
	function setMaxTiers(uint256 _maxTires) external onlyOwner {
		maxTiers = _maxTires;
	}

	//starts a new unlock period
	function setNewUnlockPeriod(
		uint256 _qty,
		uint256 _startTime,
		uint256 _price
	) external onlyOwner {
