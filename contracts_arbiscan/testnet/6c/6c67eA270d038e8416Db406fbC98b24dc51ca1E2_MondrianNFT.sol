...
contract MondrianNFT is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    // address constant bonusPoolAddress=0xd830eCAe92EA6Cbc2F2411A1c1c92C17c01eAf8A;
    ...
    address constant bonusPoolAddress=0xC37f3a8c511329C9028b8D82f491847256B56dE7;
    address constant devPoolAddress=0xC37f3a8c511329C9028b8D82f491847256B56dE7;  
    address constant bidToken=0x5F43740a1adE98414284Ddf3cF58C49801Aa1Df5; 
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        for(uint256  i=1;;i++){
            ...
            for(uint j=0;j<2;j++){
                ...
                for(x=0;x<4;x++){
                    ...
                    output = string(abi.encodePacked(output, p1,px.toString(), p2,py.toString(),p3,w.toString(),p4,h.toString(),p5,uint24tohexstr(c),p6));
                }
            }
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "',string(bytesStringTrimmed),' #', tokenId.toString(), '", "description": "MondrianNFT from topbidder", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
    ...
    function bid(address inviterAddress, uint256 artid) nonReentrant public payable
    {
        ...
        require(msg.value>=payprice, "ERR_NOT_ENOUGH_MONEY");
         ...
         if(parent==0){
             payable(devPoolAddress).send(smoney/10);
         }else{
             payable(ownerOf(parent)).send(smoney/10);
         }
    ...
    constructor() ERC721("Mondrian Minimized Version1", "NFT") Ownable() {
