...
contract PriceOracle is IPriceOracle {
    ...
    function getRealInterval(address pool) external view returns (uint32) {
        return UniswapV3TwapGetter.getRealTwapInterval(pool, 120);
    }
...
library UniswapV3TwapGetter {
    ...
    function getRealTwapInterval(address uniswapV3Pool, uint32 twapInterval) internal view returns (uint32) {
        IUniswapV3Pool pool = IUniswapV3Pool(uniswapV3Pool);
        uint160 sqrtPriceX96;
        uint16 observationCardinality;
        (sqrtPriceX96, , , observationCardinality, , , ) = pool.slot0();
