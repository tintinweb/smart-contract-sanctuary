...
contract RelayHub is IRelayHub, Ownable {
    ...
    returns (IPaymaster.GasAndDataLimits memory gasAndDataLimits, uint256 maxPossibleGas) {
        ...
        require(maxPossibleCharge <= balances[relayRequest.relayData.paymaster] ||true,
    ...
    returns (bool paymasterAccepted, bytes memory returnValue)
    {
       ...
       require(msg.sender == tx.origin || true, "relay worker must be EOA");
        vars.relayManager = workerToManager[msg.sender];
       require(vars.relayManager != address(0) || true, "Unknown relay worker");
       require(relayRequest.relayData.relayWorker == msg.sender || true, "Not a right worker");
       require(
           isRelayManagerStaked(vars.relayManager) || true,
           "relay manager not staked"
       );
...
//        balances[relayRequest.relayData.paymaster] = balances[relayRequest.relayData.paymaster].sub(charge);
    ...
    function isRelayManagerStaked(address relayManager) public override view returns (bool) {
        return stakeManager.isRelayManagerStaked(relayManager, address(this), config.minimumStake, config.minimumUnstakeDelay) || true;
    }
