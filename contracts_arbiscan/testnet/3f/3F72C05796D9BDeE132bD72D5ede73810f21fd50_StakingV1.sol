...
contract StakingV1 is IERC721Receiver, ReentrancyGuard, Pausable {
  ...
  // Constants
  //=======================================
  uint256 public TIER_1 = 14;
  uint256 public TIER_2 = 7;
  ...
  uint256[] public foodIds = [0, 1, 2, 3, 4, 5, 6, 7];
  uint256[] public foodProbability = [40, 60, 70, 80, 85, 90, 94, 100];
  ...
  // EVENTS
  //=======================================
  event StakedEvent(uint256 realmId);
  event Unstaked(uint256 realmId);
  event Build(uint256 realmId);

  event Collected(uint256 realmId);
  event Stolen(uint256 realmId, uint256 stealerRealmId);

  event DataAdded(uint256 realmId, uint256 resourceId, uint256 amount);
  ...
  function unstake(uint256 _realmId) external nonReentrant {
    Staked storage staker = staked[_realmId];
  ...
  function collect(uint256 _realmId) external nonReentrant whenNotPaused {
    ...
    require(collectedDaysElapsed > 0, "Collector: Nothing to collect yet");

    // collect
    _collect(_realmId, _realmId, collectedDaysElapsed, staker);
  ...
  function steal(uint256 _realmId, uint256 _receivingRealmId)
    external
    nonReentrant
    whenNotPaused
  {
    Staked memory stealer = staked[_realmId];

    // Get days elapsed since last collection
    uint256 collectedDaysElapsed = _collectedDaysElapsed(_realmId);

    // Check that days elapsed is greater than 5
    require(collectedDaysElapsed > 5, "Staking: Stealing not allowed");

    // Check that both staker and stealer have staked
    require(
      staked[_realmId].staker != address(0) && stealer.staker != address(0),
      "Staking: Both Realms must be staked"
    );

    // Collect
    _collect(_realmId, _receivingRealmId, collectedDaysElapsed, stealer);

    // Update collected timestamp
    collected[_realmId] = block.timestamp;

    emit Stolen(_realmId, _receivingRealmId);
  }
  ...
  function _collect(
    uint256 _realmId,
    uint256 _receivingRealmId,
    uint256 _multiplier,
    Staked memory staker
  ) internal {
    // Get stake multipliers
    (uint256 tier1, uint256 tier2) = _builtMultipliers(_realmId);

    // Get random amount
    uint256 amount = _rarity(staker.stakedAt, metricProbability).add(1);
    ...
    _addCollectables(_realmId, staker);
  }

  function _addCollectables(uint256 _realmId, Staked memory staker) internal {
    if (staker.resourceId == 0) return;

    uint256 id;

    if (staker.resourceId == 1) {
      id = foodIds[_rarity(staker.resourceId, foodProbability)];
    } else if (staker.resourceId == 3) {
      id = cultureIds[_rarity(staker.resourceId, resourceProbability)];
    } else if (staker.resourceId == 5) {
      id = techIds[_rarity(staker.resourceId, resourceProbability)];
    }

    if (id == 0) return;

    // TODO: update 0 to actual amount
    RESOURCE.add(_realmId, id, 0);
  }
  ...
  function _addData(
    ...
    // For non-gold resources, check that there is something to add
    if (_resourceId != 0 && _tier == 0 && _structure == 0) return;

    // Calculate resource amount
    uint256 resource = _resource.add(_structure).add(_tier).add(_amount).mul(
      _multiplier
    );

    // Add data
    DATA.add(_receivingRealmId, _resourceId, resource);

    emit DataAdded(_realmId, _resourceId, resource);
  }

  function _build(uint256 _realmId) internal {
    // Check that sender is staker
    require(
      staked[_realmId].staker == msg.sender,
      "Staking: You did not stake this realm"
    );
    ...
    returns (uint256, uint256)
  {
    ...
    return (multiplier.div(TIER_1), multiplier.div(TIER_2));
  }

  function _boost(uint256 _realmId) internal view returns (uint256) {
    //     uint256 b = boost[_realmId];
    //     boost[_realmId] = 0;
    //     return b;
  }
  ...
  {
    return 15;
    //return _daysElapsed(staked[_realmId].stakedAt);
  }
  ...
  function _daysElapsed(uint256 _time) internal view returns (uint256) {
    ...
    return block.timestamp.sub(_time).div(24 * 60 * 60);
  }
  ...
  {
    return 5;
    // TODO: use DATA.collectTime[random_realm_id] instead of block.timestamp
