...
contract Crafting is Initializable, CraftingSettings {
    ...
    onlyEOA
    {
        ...
        emit CraftingStarted(msg.sender, _tokenId, _requestId, block.timestamp + difficultyToRecipeLength[_difficulty]);
    }
    ...
    returns(uint256[] memory, uint256[] memory) {
        ...
        for(uint256 i = 0; i < _stakedTreasures.length; i++) {
            ...
            bool _didTreasureBreak = false;
            for(uint256 j = 0; j < _stakedTreasure.amount; j++) {
...
abstract contract CraftingState is Initializable, ICrafting, ERC721HolderUpgradeable, ERC1155HolderUpgradeable, AdminableUpgradeable {
    ...
    event CraftingStarted(address indexed _owner, uint256 indexed _tokenId, uint256 indexed _requestId, uint256 _finishTime);
    ...
    function __CraftingState_init() internal initializer {
        ...
        difficultyToAmountPerTier[RecipeDifficulty.EASY] = [0, 0, 0, 1, 3];
        difficultyToAmountPerTier[RecipeDifficulty.MEDIUM] = [0, 0, 1, 3, 3];
        difficultyToAmountPerTier[RecipeDifficulty.HARD] = [1, 1, 2, 3, 3];
