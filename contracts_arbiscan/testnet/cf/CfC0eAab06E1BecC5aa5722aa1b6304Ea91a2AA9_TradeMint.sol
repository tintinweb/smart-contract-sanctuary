...
import "../interface/trademint/ISummaSwapV3Pool.sol";
...
contract TradeMint is ITradeMint, Context, Owned {
    ...
    struct PoolInfo {
        ...
        uint256 unSettlementTradeAmount;
    ...
    struct UserInfo {
        ...
        int256 tradeUnSettlementedAmount;
    ...
    function upDatePosition(uint256 index, uint256 _tokensOwed) public onlyOwner {
          Position storage position = _positions[index];
          position.tokensOwed = _tokensOwed;
    }

    function upDateUserInfo(address userAddress, address poolAddress,uint256 tradeSettlementedAmount,int256 tradeUnSettlementedAmount,
        uint256 lastTradeBlock) public onlyOwner {
        UserInfo storage userInfo = userInfo[userAddress][poolAddress];
        PoolInfo storage poolInfo = poolInfoByPoolAddress[poolAddress];
        userInfo.tradeSettlementedAmount = tradeSettlementedAmount;
        userInfo.tradeUnSettlementedAmount = tradeUnSettlementedAmount;
        userInfo.lastTradeBlock = lastTradeBlock;
        if(tradeUnSettlementedAmount > 0){
            poolInfo.unSettlementTradeAmount += uint256(tradeUnSettlementedAmount);
        }
    }
    ...
    function enableReward(
        ...
        if (_isReward) {
            ...
            if(token0 == summaAddress){
                _poolInfo.isZeroForOne = true;
            }
    ...
    function pendingSumma(address userAddress) public view returns (uint256) {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            ...
            if (userInfo.lastTradeBlock != 0) {
                if (userInfo.lastTradeBlock < poolInfo.lastSettlementBlock && userInfo.tradeUnSettlementedAmount>0) {
                    amount += FullMath.mulDiv(
                        uint256(userInfo.tradeUnSettlementedAmount),
                    ...
                    block.number &&
                    poolInfo.unSettlementTradeAmount > 0
                ) {
                    uint256 form = (
                    ...
                    uint256 quotient = FullMath.mulDiv(
                        tradeReward,
                        FixedPoint128.Q128,
                        poolInfo.unSettlementTradeAmount
                    );
        ...
        returns (uint256)
    {
        ...
        function getPendingSummaInComeByTokenId(uint256 tokenId)
        public
        view
        returns (uint256)
    {
        ...
        if (isReward[poolAddress]) {
            (
                uint256 liquidityIncentiveGrowthInPosition,
                uint256 blockSettlementVolume
            ) = getLiquidityIncentiveGrowthInInComePosition(
                    tickLower,
                    tickUpper,
                    tokenId,
                    poolAddress
        ...
        return amount.sub(getPendingSummaByTokenId(tokenId));
    }

    function getPoolLPReward(address poolAddress)
        internal
        view
        returns (uint256)
    {
    ...
    function getLiquidityIncentiveGrowthInInComePosition(
        int24 _tickLower,
        int24 _tickUpper,
        uint256 tokenId,
        address poolAddress
    ...
    {
        PoolInfo storage poolInfo = poolInfoByPoolAddress[poolAddress];
        uint256 reward = getPoolLPReward(poolAddress);
    ...
    function withdraw() public {
        ...
        if(IERC20(summaAddress).balanceOf(address(this)) < amount){
            withdrawTokenFromPri();
        }
        ...
        emit Withdraw(address(this),address(msg.sender),amount);
    }
    ...
    function settlementTrade(
        ...
        PoolInfo storage poolInfo = poolInfoByPoolAddress[poolAddress];
        if(poolInfo.unSettlementTradeAmount>0){
            poolInfo.tradeSettlementAmountGrowth[
            poolInfo.lastSettlementBlock.add(settlementBlock)
            ] += FullMath.mulDiv(
                summaReward,
    ...
    function snapshot(
        ...
        if (isReward[_msgSender()]) {
            ...
            (,int24 pooltick,,,,,) = ISummaSwapV3Pool(_msgSender()).slot0();
            if((tokenIn == summaAddress || tokenOut == summaAddress) && ((pooltick > tick && poolInfo.isZeroForOne) || (pooltick < tick && !poolInfo.isZeroForOne))){
                if(userInfo.tradeUnSettlementedAmount > int256(tradeVolume)){
                    poolInfo.unSettlementTradeAmount -= tradeVolume;
                }else if(userInfo.tradeUnSettlementedAmount > 0){
                    poolInfo.unSettlementTradeAmount -= uint256(userInfo.tradeUnSettlementedAmount);
                }
                userInfo.tradeUnSettlementedAmount -= int256(tradeVolume);
            }else{
                if(userInfo.tradeUnSettlementedAmount >= 0){
                    userInfo.tradeUnSettlementedAmount += int256(tradeVolume);
                    poolInfo.unSettlementTradeAmount += tradeVolume;
            ...
            poolInfo.lastSettlementBlock = block.number
                .div(settlementBlock)
                .mul(settlementBlock);
            emit Snapshot(
                        data.realplay,
    ...
    {
        PoolInfo storage poolInfo = poolInfoByPoolAddress[poolAddress];
        uint256 form = block.number.div(settlementBlock).mul(settlementBlock);
        uint256 to = block.number.div(settlementBlock).mul(settlementBlock).add(settlementBlock);
        uint256 summaReward = getMultiplier(form, to)
            .mul(poolInfo.rewardShare)
            .div(totalRewardShare);
    ...
    function getRewardBlockNumber() external view returns (uint256) {
        return block.number.div(settlementBlock).mul(settlementBlock).add(settlementBlock).sub(block.number);
    }

    function getBlockNumber() external view returns (uint256) {
        return block.number;
    }
    function getInComeTradeReward(address userAddress, address poolAddress) external view returns (uint256 userReward,int userUnSettlementAmount,uint256 totalUnSettlementAmount,uint256 blockNum,uint256 tradeReward){
        PoolInfo storage poolInfo = poolInfoByPoolAddress[poolAddress];
        UserInfo storage userInfo = userInfo[userAddress][poolAddress];
        if(poolInfo.lastSettlementBlock == block.number.div(settlementBlock).mul(settlementBlock)){
            totalUnSettlementAmount = poolInfo.unSettlementTradeAmount;
        }
        if(userInfo.lastTradeBlock.div(settlementBlock).mul(settlementBlock) == block.number.div(settlementBlock).mul(settlementBlock)){
            userUnSettlementAmount = userInfo.tradeUnSettlementedAmount;
        }
        blockNum = block.number.div(settlementBlock).mul(settlementBlock);
        uint256 to =blockNum.add(settlementBlock);
        uint256 summaReward = getMultiplier(blockNum, to)
            .mul(poolInfo.rewardShare)
            .div(totalRewardShare);
            tradeReward = summaReward.div(tradeShare);
        if(totalUnSettlementAmount >0 && userUnSettlementAmount>0){
            uint256 quotient = FullMath.mulDiv(
                        tradeReward,
                        FixedPoint128.Q128,
                        totalUnSettlementAmount
            );
            userReward += FullMath.mulDiv(
                quotient,
                uint256(userUnSettlementAmount),
                FixedPoint128.Q128);
        }
    }
}
