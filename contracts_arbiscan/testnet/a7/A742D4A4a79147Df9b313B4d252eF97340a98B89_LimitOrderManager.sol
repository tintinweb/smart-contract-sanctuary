...
contract LimitOrderManager is
    ...
    function placeLimitOrderRelayed (
    ...
    external payable virtual returns (
        ...
        bytes32 dataHash = keccak256(abi.encodePacked(
                params._token0, params._token1, params._fee, params._sqrtPriceX96,  params._amount0, params._amount1, _owner
            )
        );
        _checkSignature(dataHash, _owner, signature);

        _tokenId = _placeLimitOrder(params, _owner);
    ...
    function placeLimitOrder(LimitOrderParams calldata params)
        ...
        _tokenId = _placeLimitOrder(params, msg.sender);
    }

    function _placeLimitOrder(LimitOrderParams calldata params, address _owner)
    internal returns (
        uint256 _tokenId
    ) {
        ...
        (_tickLower, _tickUpper, _liquidity,) = utils.calculateLimitTicks(
            _pool,
    ...
    function cancelLimitOrderRelayed (
        uint256 _tokenId,
        address _owner,
        bytes calldata signature
    ) external returns (
        ...
        (_amount0, _amount1) = _cancelLimitOrder(_tokenId, _owner);
    ...
    function cancelLimitOrder(uint256 _tokenId) external returns (
        ...
        return _cancelLimitOrder(_tokenId, msg.sender);
    }

    function _cancelLimitOrder(uint256 _tokenId, address _owner) internal returns (
        uint256 _amount0, uint256 _amount1
    ) {

        isAuthorizedForToken(_tokenId, _owner);
    ...
    function burn(uint256 _tokenId) external {
        ...
        isAuthorizedForToken(_tokenId, msg.sender);
    ...
    function _checkSignature(bytes32 dataHash, address _owner, bytes memory _signature) internal pure {
        ...
        require(recoveredAddress == _owner, "ERR_NO_RELAY");
    }
    ...
    function isAuthorizedForToken(uint256 tokenId, address _owner) internal view {
