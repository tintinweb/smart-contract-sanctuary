...
contract BizzSwap is IBizzSwap, Ownable {
    ...
    event InvoiceCreated(uint256 indexed invoiceId, string indexed ipfsCid, address desiredTokenAddress, address recipient, uint256 exactAmountOut);
    ...
    event PaymentCompleted(uint256 indexed invoiceId);
    ...
    /// @inheritdoc IBizzSwap
    function createInvoice(address _desiredTokenAddress, uint _exactAmountOut, string memory _ipfsCid) 
    public 
    override
    ...
    returns(uint256 invoiceId) 
    {
        ...
        invoices[invoiceId] = Invoice(_exactAmountOut, _desiredTokenAddress, msg.sender, false, _ipfsCid);
        _invoiceId.increment();

        emit InvoiceCreated(invoiceId, _ipfsCid, _desiredTokenAddress, msg.sender, _exactAmountOut);
    }
    ...
    override
    {
       ...
       _pay(_params, invoice.recipient, _inputTokenAddress, invoice.desiredTokenAddress, invoice.exactAmountOut, _maximumAmountIn);

       invoice.isPaid = true;
        
       emit PaymentCompleted(invoiceId);
    }
    ...
    returns(uint256 _amountIn)
    {
        ...
        emit Swap(_inputTokenAddress, _outputTokenAddress, _exactAmountOut, msg.sender, _recipient);
    }
    ...
    validAddress(_inputTokenAddress)
    {
        if(_inputTokenAddress == _outputTokenAddress) {
            IERC20(_inputTokenAddress).safeTransferFrom(msg.sender, _recipient, _exactAmountOut);
            emit Transfer(_inputTokenAddress, _exactAmountOut, msg.sender, _recipient);
        } else {
...
interface IBizzSwap {
   ...
   */
  function createInvoice(address _desiredTokenAddress, uint _exactAmountOut, string memory _ipfsCid) external returns(uint256 invoiceId);
   ...
   * No return, reverts on error
   */
  function payOneForOne(uint256 invoiceId, address _inputTokenAddress, uint256 _maximumAmountIn, SwapParameters memory _params) external;
}
