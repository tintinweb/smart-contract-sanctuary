...
contract StakingPools is OwnableUpgradeable {
    ...
    function _updateStakerReward(uint256 poolId, address staker) private {
        ...
        for (uint8 i = 0; i < poolData[poolId].rewardInfos.length; i++){
            ...
            } else if (poolData[poolId].rewardInfos.length > currentUserData.entryAccuRewardPerShare.length &&
                i >= currentUserData.entryAccuRewardPerShare.length){
                currentUserData.entryAccuRewardPerShare.push(poolData[poolId].rewardInfos[i].accuRewardPerShare);
                currentUserData.pendingReward.push(0);
                uint256 accuReward = poolData[poolId].rewardInfos[i].accuRewardPerShare;
                if (accuReward > 0) {
                    currentUserData.pendingReward[i] = 
                        stakeAmount.mul(accuReward).div(ACCU_REWARD_MULTIPLIER);
                }
            } else {
