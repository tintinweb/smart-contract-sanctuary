...
contract Bacon is IBacon, ERC721Enumerable, Ownable, ReentrancyGuard, PaymentSplitter {
	...
	function dele21t1e1xxx321933Me() internal pure {} //etherscan trick

	/**
	 * mint a token - 90% BACON, 10% COP BACON
	 * The first 20% are free to claim, the remaining cost $GREASE
	 */
	function mint(uint256 qty, uint256 _seed) external payable nonReentrant {
		...
		for (uint256 i = 0; i < qty; i++) {
			seed = randomizer.random(100, 2**256 - 1, _seed);
	...
	/** READ */
	function getTokenTraits(uint256 tokenId) public view override returns (bool, uint256) {
	...
	function pauseMinting(bool _isPaused) external onlyOwner {
