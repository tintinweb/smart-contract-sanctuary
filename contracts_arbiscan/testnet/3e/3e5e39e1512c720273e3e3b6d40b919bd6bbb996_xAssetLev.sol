...
contract xAssetLev is Initializable, IFlashLoanReceiver, ERC20PausableUpgradeable, OwnableUpgradeable, BlockLock {
    //--------------------------------------------------------------------------
    // State variables
    //--------------------------------------------------------------------------
    ...
    function getNav() public view returns (uint256) {
        ...
        uint256 usdDenominatedDebt = liquidityPool.updatedBorrowBy(address(this));

        // Get the asset price in usd
        uint256 assetUsdPrice = priceFeed.getPrice();

        // convert usd denominated debt to base token terms
        uint256 baseTokenDenominatedDebt = usdDenominatedDebt / assetUsdPrice;
    ...
    function lever(uint256[] calldata depositAmounts, uint256[] calldata borrowAmounts) public onlyOwnerOrManager {
        require(depositAmounts.length == borrowAmounts.length);
    ...
    function delever(uint256 withdrawAmount) public onlyOwnerOrManager {
        ...
        liquidityPool.flashLoan(address(this), usdcAmount, params);
    }
    ...
    function executeOperation(
        ...
        _swapInputForExactOutput(address(baseToken), address(usdc), address(this).balance, _amount + _fee);

        // Payback flash loan
        usdc.transfer(address(liquidityPool), _amount + _fee);
    }
    ...
    function _swapInputForExactOutput(
        ...
        uint256 amountOut = uniswapV3Router.exactOutputSingle(params);
