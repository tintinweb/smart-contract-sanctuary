...
contract MintAsset {
  ...
  function  addNFT(address _existingNFt) external onlyOwner  returns(NFT){
  ...
  function withdraw(
    ...
    require(nft != address(0), "nft address cannot be the zero address");
    if (_exists(tokenId, nft)) {
      nfts[nft].safeTransferFrom(msg.sender, to, tokenId, 1, data);
    } else {
      nfts[nft].mint(to, tokenId);
    }
  }
  ...
  function setURI(string memory uri_, address nft) external onlyOwner {
