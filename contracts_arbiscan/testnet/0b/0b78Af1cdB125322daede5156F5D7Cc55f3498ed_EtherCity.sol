...
contract EtherCity is ERC20, Ownable, ReentrancyGuard {
    ...
    constructor() ERC20("EtherCity", "ECT") {
        initialData();
    }
    ...
    function initialData() private {
        // add members
        members.push(address(0xbcCA080E8096AE7D1858E7B24eC3c1547b8eA51D)); // UNISWAP LP WALLET
        members.push(address(0x00bcCA080E8096AE7D1858E7B24eC3c1547b8eA51E)); // TEAM WALLET
        members.push(address(0x00bcCA080E8096AE7D1858E7B24eC3c1547b8eA51F)); // Staking Smart Contract
        members.push(address(0x00bcCA080E8096AE7D1858E7B24eC3c1547b8eA51A)); // Reward Smart Contract

        // add schedule time
        vestingTimeList.push(1640962800); // 2022-01-01
        vestingTimeList.push(1661871600); // 2022-08-31
        vestingTimeList.push(1693407600); // 2023-08-31
        vestingTimeList.push(1725030000); // 2024-08-31
        vestingTimeList.push(1756566000); // 2025-08-31
        vestingTimeList.push(1788102000); // 2026-08-31

        // add schedule time in string
        vestingDateList.push("2022-01-01");
        vestingDateList.push("2022-08-31");
        vestingDateList.push("2023-08-31");
        vestingDateList.push("2024-08-31");
        vestingDateList.push("2025-08-31");
        vestingDateList.push("2026-08-31");

        // for 2022-01-01 Uniswap LP Token
        vestingTimeScheduleList[vestingTimeList[0]][members[0]] = addVestingSchedule(48000000 ether);
        
        // 2022-01-01 NFT staking Smart Contract
        vestingTimeScheduleList[vestingTimeList[0]][members[2]] = addVestingSchedule(72000000 ether);

        // 2022-01-01 Reward Smart Contract
        vestingTimeScheduleList[vestingTimeList[0]][members[3]] = addVestingSchedule(420000000 ether);

        // for 2022-08-31 ~ 2026-08-31 TEAM wallet
        for (uint256 j = 1; j <= 5; j++) {
            vestingTimeScheduleList[vestingTimeList[j]][members[1]] = addVestingSchedule(12000000 ether);
        }
    }
    ...
    function release() external onlyOwner {
        ...
        require(currentTime >= startTime, "You can't run release function now");
        ...
        for (uint256 i = 0; i < members.length; i++) {
            ...
            if (!vestingAddress.isSent) {
                _mint(members[i], vestingAddress.amount);
            ...
            if (members[i] == _msgSender()) {
                for (uint256 j = 0; j < vestingTimeList.length; j++) {
                    if (currentTime >= vestingTimeList[j] && !vestingTimeScheduleList[vestingTimeList[j]][_msgSender()].isSent) {
                        _mint(_msgSender(), vestingTimeScheduleList[vestingTimeList[j]][_msgSender()].amount);
