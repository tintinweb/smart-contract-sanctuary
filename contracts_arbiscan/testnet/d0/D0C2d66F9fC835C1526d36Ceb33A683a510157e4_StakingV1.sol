...
contract StakingV1 {
  ...
  function stake(uint256 _realmId) external {
    ...
    staked[_realmId] = block.timestamp;

    emit Staked(_realmId);
  }
  ...
  function _days(uint256 _realmId) internal view returns (uint256) {
    if (staked[_realmId] == 0) {
      return 1;
    }

    if (block.timestamp <= staked[_realmId]) {
      return 0;
    }

    return block.timestamp.sub(staked[_realmId]).div(24 * 60 * 60);
  }
