...
contract OlympusBondDepository is Ownable {
    ...
    address payable public devAddr; // Dev address
    ...
    function deposit( 
        ...
        if ( devFee != 0 ) { // dev fee is transferred
            uint devAmount = payout.mul(devFee).div( 100 );
            ITreasury( treasury ).mintRewards( devAddr, devAmount );
        }
     ...
     *  @param _devAddr address
     */
    function setDev(address payable _devAddr) external onlyPolicy() {
        devAddr = _devAddr;
    }
     ...
     *  @param _devFee uint
     */
    function setDevFee(uint _devFee) external onlyPolicy() {
        devFee = _devFee;
    }
}