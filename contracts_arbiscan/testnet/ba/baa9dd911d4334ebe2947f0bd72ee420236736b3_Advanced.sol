/**
 *Submitted for verification at arbiscan.io on 2021-11-21
*/
...
contract Basic is GIDS, Manager{
    ...
    function create(address to, _propInput[] memory prop, _inforInput[] memory infor, uint256 quantity) public onlyManager{
        ...
        properties[nextId]["totalSupply"]=_prop(count+1, "totalSupply", quantity);
        key[nextId][count+1]="totalSupply";
        
        currentProp[nextId] = count+1;
        //add extended infor
...
contract Advanced is Basic{
    ...
    function getUserItem(address owner) public view returns(_userItem[] memory){
        ...
        for(i=0;i<itemId;i++){
            if(balanceOf(owner,i+1)>0){
                all[k]=_userItem(i+1,getItem(i+1).prop, getItem(i+1).infor, balanceOf(owner,i+1));
                k++;
            }
        }
        return all;
