...
contract GutterComics is ERC721A, Ownable, ReentrancyGuard {
	...
	uint256 public maxTotalSupply = 12000;
	uint256 public maxPresale = 9000;
	uint256 public maxPublicSale = 9000;
	uint256 public maxFreeSale = 3000;
	uint256 public pricePerComic = 0.07 ether;

	bool public locked; //metadata lock

	mapping(uint256 => uint256) private version; //nft id -> version
	...
	function publicSale(uint256 qty, uint256 key) external payable {
		...
		require(qty < 21, "max 20 per tx");
		require(totalSupply() + qty <= maxTotalSupply, "out of stock");
		require(totalSupply() + qty <= maxPublicSale, "out of stock");
		require(pricePerComic * qty == msg.value, "exact amount needed");
	...
	function freeSale(
		...
		require(qty < 21, "max 20 per tx");
		require(totalSupply() + qty <= maxTotalSupply, "out of stock");
		require(totalSupply() + qty <= maxFreeSale, "out of stock");
		require(pricePerComic * qty == msg.value, "exact amount needed");
	...
	function setKey(uint256 keyType, uint256 keyValue) external onlyOwner {
		if (keyType == 0) {
			presaleKey = keyValue;
			presaleTimer = block.timestamp + 2 hours;
		}
		if (keyType == 1) {
