...
contract AtlasMine is Initializable, AccessControlEnumerableUpgradeable {
    ...
    // user => nft => tokenIds
    mapping (address => mapping(address => EnumerableSetUpgradeable.UintSet)) private nftStaked;
    // user => nft addresses
    mapping (address => EnumerableSetUpgradeable.AddressSet) private userNfts;
    // user => boost
    ...
    function deposit(uint256 _amount, Lock _lock) public updateRewards {
        ...
        emit Deposit(msg.sender, depositId, _amount, uint8(_lock));
    }
    ...
    function stake(address _nft, uint256 _tokenId) external updateRewards {
        ...
        IERC721Upgradeable(_nft).transferFrom(msg.sender, address(this), _tokenId);
    ...
    function unstake(address _nft, uint256 _tokenId) external updateRewards {
        ...
        IERC721Upgradeable(_nft).transferFrom(address(this), msg.sender, _tokenId);
