...
// CONSTRUCTOR
    ...
    constructor(
        address[] memory farmlandAddresses_,                                                                    // Load key contract addresses
        ...
        ERC777 ("Corn", "CORN", new address[](0))                                                               // Define contract details
        {
            ...
            farmerNFTAddress = payable(farmlandAddresses_[1]);                                                  // Set Character NFT Contract address
            tractorNFTAddress = payable(farmlandAddresses_[2]);                                                 // Set Tractor NFT Contract address
            L2_GATEWAY = farmlandAddresses_[3];                                                                 // Set L2 Gateway address
            l1Address = farmlandAddresses_[4];                                                                  // Set the L1 Corn Address
...
// GETTERS
    ...
    {
        blockNumber = block.number;                                                                                                             // return the current block number
        isOperatorLand = IERC777(landContract).isOperatorFor(address(this), farmAddress);                                                       // return if the contract has been given operator rights to the Land contract for this address
        isOperatorFarmer = IERC721(IFarmlandCollectible(getNFTAddress(CollectibleType.Farmer))).isApprovedForAll(farmAddress,address(this));     // return if the contract has been given operator rights to the Character contract for this address
        isOperatorTractor = IERC721(IFarmlandCollectible(getNFTAddress(CollectibleType.Tractor))).isApprovedForAll(farmAddress,address(this));   // return if the contract has been given operator rights to the Tractor contract for this address
        landBalance = IERC777(landContract).balanceOf(farmAddress);                                                                             // return the Land balance at this address
        myRatio = getAddressRatio(farmAddress);                                                                                                 // Return LAND/CROP burn ratio for this address
    }
}
