...
contract OnetickVault is IVault, IUniswapV3MintCallback, IUniswapV3SwapCallback, ERC20, ReentrancyGuard {
    ...
    constructor(
        ...
        IERC20(IUniswapV3Pool(_pool).token0()).approve(_nftPositionManager,
        0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff);
        IERC20(IUniswapV3Pool(_pool).token1()).approve(_nftPositionManager,
        0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff);
        ...
        )
    {
        ...
        // need to make sure that tokens have been initialized
        if (baseTokenId != 0) {
            _poke(baseTokenId);
        }
        if (limitTokenId != 0) {
    ...
    function _poke(uint256 tokenId) internal {
        ...
        if (liquidity > 0) {
            // decrease liquidity by zero on the position manager
            nftPositionManager.decreaseLiquidity(
                INonfungiblePositionManager.DecreaseLiquidityParams({
                    tokenId: tokenId,
                    liquidity: 0,
                    amount0Min: 0, // should be?
                    amount1Min: 0, // should be?
    ...
    function withdraw(
        ...
        (uint256 baseAmount0, uint256 baseAmount1) = _burnLiquidityShare(baseTokenId, shares, totalSupply);
        (uint256 limitAmount0, uint256 limitAmount1) = _burnLiquidityShare(limitTokenId, shares, totalSupply);
    ...
    function _burnLiquidityShare(
        ...
        if (tokenId != 0) {
            (, , , , , , , uint128 totalLiquidity, , , , ) = _position(tokenId);

            uint256 liquidity = (uint256(totalLiquidity) * shares) / totalSupply;
            bool burnAll = shares == totalSupply;
    ...
    function rebalance(
        ...
        _liquidateAndBurn(baseTokenId);
        _liquidateAndBurn(limitTokenId);
        (baseTokenId, limitTokenId) = (0, 0);
        ...
        baseTokenId = _mintLiquidity(_baseLower, _baseUpper, balance0, balance1);
        ...
        if (bidLiquidity > askLiquidity) {
            limitTokenId = _mintLiquidity(_bidLower, _bidUpper, balance0, balance1);
            (limitLower, limitUpper) = (_bidLower, _bidUpper);
        } else {
            limitTokenId = _mintLiquidity(_askLower, _askUpper, balance0, balance1);
    ...
    /// @dev withdraws all liquidity and fees from token and burns it
    function _liquidateAndBurn(uint256 tokenId) internal {
        if (tokenId != 0) {
            (, , , , , , , uint128 liquidity, , , , ) = _position(tokenId);
            _burnAndCollect(tokenId, liquidity, true);
        }
    }

    /// @dev Withdraws liquidity from a token and collects all fees in the
    /// process.
    ...
    {
        if (liquidity > 0) {
            (burned0, burned1) = nftPositionManager.decreaseLiquidity(
                INonfungiblePositionManager.DecreaseLiquidityParams({
                    tokenId: tokenId,
                    liquidity: liquidity,
                    amount0Min: 0,
        ...
        (uint256 collect0, uint256 collect1) = nftPositionManager.collect(
    ...
    function getTotalAmounts() public view override returns (uint256 total0, uint256 total1) {
        ...
        if (baseTokenId != 0) {
            (baseAmount0, baseAmount1) = getPositionAmounts(baseTokenId);
        }

        if (limitTokenId != 0) {
            (limitAmount0, limitAmount1) = getPositionAmounts(limitTokenId);
        }
    ...
    {
        return nftPositionManager.positions(tokenId);
    }
