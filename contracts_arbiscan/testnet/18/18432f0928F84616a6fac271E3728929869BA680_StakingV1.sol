...
contract StakingV1 is IERC721Receiver {
  ...
  // EVENTS
  //=======================================
  event StakedEvent(uint256 _realmId);
  event Unstaked(uint256 _realmId);
  ...
  // External
  //=======================================
  function stake(uint256 _realmId) external {
    ...
    emit StakedEvent(_realmId);
  }

  function unstake(uint256 _realmId) external {
    Staked memory staker = staked[_realmId];
    ...
    uint256 daysStaked = _days(_realmId);
    uint256 tier1 = daysStaked.div(14);
    uint256 tier2 = daysStaked.div(7);
    ...
    // TODO: Maybe do a loop so that you can add more structures as you go.

    uint256 metric = _rarity(_realmId, metricProbability).add(1);

    DATA.addGoldSupply(_realmId, tier1.mul(metric));
    DATA.addReligionSupply(_realmId, tier1.mul(metric));

    uint256 m = tier2.mul(metric);

    DATA.add(_realmId, 2, m);
    DATA.add(_realmId, 3, m);
    DATA.add(_realmId, 5, m);
  ...
  function _days(uint256 _realmId) internal view returns (uint256) {
    ...
    if (staker.stakedAt == 0) {
      return 1;
    }

    if (block.timestamp <= staker.stakedAt) {
      return 0;
    }

    return block.timestamp.sub(staker.stakedAt).div(24 * 60 * 60);
  }
