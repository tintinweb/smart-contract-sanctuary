...
abstract contract CraftingSettings is Initializable, CraftingContracts {
    ...
    function setCraftingLevelSettings(
        ...
        uint256[] calldata _levelToCPGainedPerRecipe)
    external
    onlyAdminOrOwner
    {
        require(_maxCraftingLevel > 0, "Bad max level");
        require(_levelToCPNeeded.length == _maxCraftingLevel - 1, "Not enough CP steps");
        require(_levelToCPGainedPerRecipe.length == _maxCraftingLevel - 1, "Not enough CP gained steps");
        ...
        for(uint256 i = 0; i < _maxCraftingLevel - 1; i++) {
            ...
            levelToCPGainedPerRecipe.push(_levelToCPGainedPerRecipe[i]);
        }
    }
}
...
abstract contract CraftingContracts is Initializable, CraftingState {
    ...
    modifier contractsAreSet() {
        require(address(randomizer) != address(0)
            ...
            && address(magic) != address(0), "Contracts aren't set");
...
abstract contract CraftingState is Initializable, ICrafting, ERC721HolderUpgradeable, ERC1155HolderUpgradeable, AdminableUpgradeable {
    ...
    function __CraftingState_init() internal initializer {
        ...
        levelToCPNeeded.push(600);
        levelToCPNeeded.push(1500);
        levelToCPNeeded.push(6000);
        ...
        percentReturnedOnFailure = 80;

        craftingFee = 10 ether;
    }
}
