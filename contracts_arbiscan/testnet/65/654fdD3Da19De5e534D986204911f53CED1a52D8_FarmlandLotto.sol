...
/**
    1. The farmer buys a ticket(s) with CORN
    2. The tickets are entered into the draw
    3. All the CORN is combined into the prize pool.
    4. The prize pool is split between: the winner 68%, burn 30%, 2% draw tx
    5. The UI should display prize pool split
    6. Once the lottery ends (? days) a button appears on the UI anyone can click the button & claim the 2% draw tx
    7. The button triggers a function that ends the draw and picks 3 winners (split 50,30,20)
    8. The draw finishes and the next one starts.
*/
...
// PUBLIC FUNCTIONS
    ...
    {
        Draw storage draw = draws[getCurrentDraw()];                              // Shortcut accessor for the Draw
        require(draw.drawEnded == false,                                          "The lottery has ended. Please check if you won the price!");
        uint256 burnAmount;                                                       // Instantiate the burn amount
        uint256 closersFee;                                                       // Instantiate the closers fee
        ...
        IERC777Burn(address(paymentContract)).burn(burnAmount,"");                // The Burn
    }
...
// GETTERS
    ...
    /**
     * @dev PUBLIC: Return array of draws
     */
    function getDraws()
        external
        view
    ...
    /**
     * @dev PUBLIC: Return array of players for a draw
     */
    ...
    /**
     * @dev PUBLIC: Return winners addresses by draw
     */
    ...
    {
        Draw memory draw = draws[drawIndex];                // Shortcut accessor for the Draw
        return draw.players[draw.winnerPlayerIndex];
    }
