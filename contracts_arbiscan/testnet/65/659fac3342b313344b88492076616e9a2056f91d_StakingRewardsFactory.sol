...
contract StakingRewardsFactory is Ownable, ReentrancyGuard {
    ...
    function deploy(
        ...
        require(rewardAmountJONES > 0, "Invalid JONES reward amount");
        ...
        info.rewardAmountJONES = rewardAmountJONES;
        info.id = id;
        stakingID.push(id);
    }

    // Withdraw tokens in case functions exceed gas cost
    function withdrawRewardToken(uint256 amountJONES)
        public
        onlyOwner
        returns (uint256)
    {
        ...
        if (OwnerAddress == msg.sender) {
            IERC20(rewardsTokenJones).transfer(OwnerAddress, amountJONES);
        }
        return amountJONES;
    }

    function withdrawRewardTokensFromContract(uint256 amountJONES, uint256 id)
        public
        onlyOwner
    {
        address OwnerAddress = owner();
    ...
    function addToContractWhitelist(address _contract, uint256 id)
        external
        onlyOwner
    {
        StakingRewardsInfo storage info = stakingRewardsInfoByStakingToken[id];
    ...
    function removeFromContractWhitelist(address _contract, uint256 id)
        external
        onlyOwner
    {
        StakingRewardsInfo storage info = stakingRewardsInfoByStakingToken[id];
    ...
    Ownable
{
        ...
        returns (uint256 JONEStokensEarned)
    {
        ...
        JONEStokensEarned = _balances[account]
            .mul(perTokenRateJONES.sub(userJONESRewardPerTokenPaid[account]))
            .div(1e18)
            .add(rewardsJONES[account]);
    }
    ...
    function withdrawRewardTokens(uint256 amountJONES)
        public
        onlyOwner
        returns (uint256)
    {
        address OwnerAddress = owner();
        if (OwnerAddress == msg.sender) {
            IERC20(rewardsTokenJONES).safeTransfer(OwnerAddress, amountJONES);
        }
        return amountJONES;
    }
    ...
    function notifyRewardAmount(uint256 rewardJONES)
        external
        override
    ...
    {
        if (periodFinish == 0) {
            rewardRateJONES = rewardJONES.div(
            ...
            rewardRateJONES = rewardJONES.add(leftoverJONES).div(
                rewardsDuration
            );
            lastUpdateTime = block.timestamp;
            periodFinish = block.timestamp.add(rewardsDuration);
        }
        emit RewardAdded(rewardJONES);
    }
    ...
    modifier setReward(address account) {
        ...
        if (account != address(0)) {
            rewardsJONES[account] = earned(account);
            userJONESRewardPerTokenPaid[account] = rewardPerTokenStoredJONES;
        }
        _;
    }
