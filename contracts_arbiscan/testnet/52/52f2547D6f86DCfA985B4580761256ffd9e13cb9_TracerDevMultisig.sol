...
import "./interfaces/ITracerDevMultisig.sol";
...
contract TracerDevMultisig is ITracerDevMultisig {
    ...
    constructor() {
        proposalDuration = 40000;
        maxProposalTargets = 20;

        multisig[0xc18fcFFD8c9173faB1684Ec1EEE32976f780B13E] = true;
        multisig[0xdcf10C54dD59663410235BEcF937868645eEF466] = true;
    }
     ...
     */
    function propose(address[] memory targets, bytes[] memory proposalData) external override onlyMultisig {
        require(targets.length != 0, "DAO: 0 targets");
        require(targets.length < maxProposalTargets, "DAO: Too many targets");
        require(targets.length == proposalData.length, "DAO: Argument length mismatch");
    ...
    function execute(uint256 proposalId) external override onlySelf {
        require(proposals[proposalId].state == ProposalState.PASSED, "DAO: Proposal state != PASSED");
        proposals[proposalId].state = ProposalState.EXECUTED;
        for (uint256 i = 0; i < proposals[proposalId].targets.length; i++) {
            (bool success, bytes memory data) = proposals[proposalId].targets[i].call(
                proposals[proposalId].proposalData[i]
            );
            require(success, "DAO: Failed target execution");
            emit TargetExecuted(proposalId, proposals[proposalId].targets[i], data);
        }
     ...
     */
    function vote(uint256 proposalId, bool userVote) external override onlyMultisig {
        require(proposals[proposalId].state == ProposalState.PROPOSED, "DAO: Proposal not voteable");
        require(proposals[proposalId].expiryTime > block.timestamp, "DAO: Proposal Expired");
        require(!proposals[proposalId].userVoted[msg.sender], "Multisig member must have not voted");
