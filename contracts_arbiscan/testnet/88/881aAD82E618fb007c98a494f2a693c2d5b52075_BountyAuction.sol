/**
 *Submitted for verification at arbiscan.io on 2021-11-12
*/
...
contract PricingSession is ReentrancyGuard {
    ...
    event voteWeighed(address user_, uint nonce, address nftAddress_, uint tokenid_, uint appraisal, uint weight);
    ...
    */
    ...
    function setVote(
        ...
        emit newAppraisalAdded(msg.sender, nftNonce[nftAddress][tokenid], nftAddress, tokenid, msg.value, concealedAppraisal);
    }
    ...
    */
    ...
    function weightVote(address nftAddress, uint tokenid, uint appraisal, uint seedNum) checkParticipation(nftAddress, tokenid) nonReentrant external {
        ...
        emit voteWeighed(msg.sender, currentNonce, nftAddress, tokenid, appraisal, weight);
        if(sessionCheck.calls == sessionCore.uniqueVoters || sessionCore.endTime + sessionCore.votingTime < block.timestamp) {
            ...
            setFinalAppraisal(nftAddress, tokenid);
        }
    }
    ...
    */
    ...
    function harvest(address nftAddress, uint tokenid) checkParticipation(nftAddress, tokenid) nonReentrant external {
       ...
       sessionMap.amountHarvested[msg.sender] = PostSessionLibrary.harvest( 
        ...
        require(sent);
        emit userHarvested(msg.sender, nftNonce[nftAddress][tokenid], nftAddress, tokenid, sessionMap.amountHarvested[msg.sender]);
    ...
    */
    ...
    function endSession(address nftAddress, uint tokenid) public {
        ...
        if (sessionCheck.timeFinalAppraisalSet == 0) {
            require(
            sessionCheck.sessionProgression == 4
            || block.timestamp > (sessionCore.endTime + sessionCore.votingTime * 3)
            );
        }
        else{
            require(
    ...
    function addToAppraisal(address nftAddress, uint tokenid, address user) payable external {
        ...
        sessionCore.totalSessionStake += msg.value;
        emit appraisalIncreased(msg.sender, nftNonce[nftAddress][tokenid], nftAddress, tokenid, msg.value);
    }
...
contract BountyAuction is ReentrancyGuard {
    ...
    function addToBid() nonReentrant payable external {
        require(
