  ...
  IERC165
{
  ...
  ChainlinkOracle public chainlink_altsOracle;

  CustomOracleV1 public custom_altsOracleV1;

  CustomOracleV2 public custom_altsOracleV2;
  ...
  uint256 public ALTS_ORACLE_TYPE; 
  ...
  event OracleTypeChanged(address indexed oracleSetter, uint256 altsOracleType);
  ...
  constructor(
    ...
    ALTS_ORACLE_TYPE = _ALTS_ORACLE_TYPE;

    if (ALTS_ORACLE_TYPE == 1) {
      custom_altsOracleV1 = CustomOracleV1(_altsOracle);
    } else if (ALTS_ORACLE_TYPE == 2) {
      custom_altsOracleV2 = CustomOracleV2(_altsOracle);
    } else if (ALTS_ORACLE_TYPE == 3) {
      chainlink_altsOracle = ChainlinkOracle(_altsOracle);
    } else {
  ...
  function setOracleType(uint256 _ALTS_ORACLE_TYPE) public virtual {
    require(msg.sender == oracleSetter, "VaultHandler::setOracleType: not a valid oracle setter address");

    ALTS_ORACLE_TYPE = _ALTS_ORACLE_TYPE;

    emit OracleTypeChanged(oracleSetter, _ALTS_ORACLE_TYPE);
  }
  ...
  function changeOracleAddress(address _newOracleAddress) public virtual {
    ...
    if (ALTS_ORACLE_TYPE == 1) { 
      custom_altsOracleV1 = CustomOracleV1(_newOracleAddress);
    } else if (ALTS_ORACLE_TYPE == 2) {
      custom_altsOracleV2 = CustomOracleV2(_newOracleAddress);
    } else if (ALTS_ORACLE_TYPE == 3) {  
      chainlink_altsOracle = ChainlinkOracle(_newOracleAddress);
    } else  {
  ...
  function getOraclePriceCustomV1(CustomOracleV1 _oracle) public view virtual returns (uint256 price) {
    price = _oracle.getLatestAnswer().mul(oracleDigits);
  } 

  function getOraclePriceCustomV2(CustomOracleV2 _oracle) public view virtual returns (uint256 price) {
    price = _oracle.getLatestAnswer().mul(oracleDigits);
  } 
  ...
  function ALTSPrice() internal virtual returns (uint256 price) {
    uint256 altsPrice;

    if (ALTS_ORACLE_TYPE == 1) {
      altsPrice = getOraclePriceCustomV1(custom_altsOracleV1);
    } else if (ALTS_ORACLE_TYPE == 2) {
      altsPrice = getOraclePriceCustomV2(custom_altsOracleV2);
    } else if (ALTS_ORACLE_TYPE == 3) {
      altsPrice = getOraclePrice(chainlink_altsOracle);
    } else {
      revert("VaultHandler::ALTSPrice: not a valid oracle type");
    }
