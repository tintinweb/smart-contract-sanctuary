...
contract Summoning is Initializable, SummoningLPStakable {
    ...
    onlyEOA
    {
        ...
        for(uint256 i = 0; i < _tokenIds.length; i++) {
            _lpNeeded += _startSummonSingle(_tokenIds[i], _crystalIds[i]);
        }

        uint256 _magicCost = _tokenIds.length * magicCost;
        if(_magicCost > 0) {
            // Send magic straight to the treasury.
            bool _magicSuccess = magic.transferFrom(msg.sender, address(treasury), _magicCost);
            require(_magicSuccess, "Magic failed to transfer");

            treasury.forwardCoinsToMine(_magicCost);
        }
    ...
    function _startSummonSingle(uint256 _tokenId, uint256 _crystalId) private returns(uint256) {
