...
contract StakingV1 {
  ...
  function unstake(uint256 _realmId) external {
    ...
    uint256 tier1 = daysStaked / 14;
    uint256 tier2 = daysStaked / 7;
    ...
    uint256 metric = _rarity(_realmId, metricProbability) + 1;

    DATA.addGoldSupply(_realmId, tier1 * metric);
    DATA.addReligionSupply(_realmId, tier1 * metric);

    uint256 m = tier2 * metric;
