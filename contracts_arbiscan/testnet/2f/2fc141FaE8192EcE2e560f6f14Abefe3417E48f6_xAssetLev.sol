    ...
    BlockLock
{
     ...
     * @param _market The xtoken lending market contract for the base token
    ...
    function getFundBalances() public view returns (uint256 marketBalance, uint256 bufferBalance) {
        uint256 collateral = market.collateral(address(this));
        ...
        uint256 assetUsdPriceAdjusted = assetUsdPrice * 10**(baseToken.decimals() - 12); // baseToken decimals

        // convert usd denominated debt to base token terms
        uint256 baseTokenMultiplier = 10**baseToken.decimals();
        uint256 baseTokenDenominatedDebt = (usdDenominatedDebt * baseTokenMultiplier) / assetUsdPriceAdjusted; // baseToken decimals

        marketBalance = collateral - baseTokenDenominatedDebt;
    ...
    function delever(
        ...
        uint256 priceFeedDivisor = 10**12; // 12 decimals
        uint256 baseTokenToUSDCDivisor = 10**(baseToken.decimals() - usdc.decimals()); // todo corner case base dec < usdc dec
        uint256 usdcAmountAdjusted = usdcAmount / priceFeedDivisor / baseTokenToUSDCDivisor; // usdc decimals (6)
    ...
    function executeOperation(
        ...
        uint256 feeBaseToken = (_fee * 10**(usdc.decimals() + baseToken.decimals())) / baseTokenUsdValue; // baseToken decimals
