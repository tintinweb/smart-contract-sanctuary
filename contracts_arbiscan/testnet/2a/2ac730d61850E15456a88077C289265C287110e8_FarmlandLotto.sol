...
struct Draw { uint256 winnerPlayerIndex; bool prizeClaimed; address[] players; uint256 prizePool; bool drawEnded; uint256 deadline; }
error DeadlineNotReached(uint256 blocknumber, uint256 deadline);
...
// PUBLIC FUNCTIONS
        ...
        isADraw(drawIndex)
    {
        ...
        uint256 amount = draw.prizePool * winnerPercent / 100;                        // Calculate the prize for this Draw
...
// GETTERS
    ...
    /**
     * @dev PUBLIC: Return list of draws
     */
    function getDraws()
        external
        view
        returns (
            Draw[] memory allDraws                  // Define the array of draws to be returned
        )
    {
        return draws;                               // Return the array of collectibles on the farm
    }
    ...
    {
        if ( drawIndex > draws.length ) { return players; }
        Draw memory draw = draws[drawIndex];                   // Shortcut accessor for the Draw
