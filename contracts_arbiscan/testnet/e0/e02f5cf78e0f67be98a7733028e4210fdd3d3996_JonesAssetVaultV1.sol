...
contract JonesAssetVaultV1 is Ownable {
    ...
    function depositAsset(uint256 _amount) public payable {
        ...
        assetToken.transferFrom(
            msg.sender,
            address(this),
            _amount.mul(jonesAssetPerAsset())
        );
        jonesAssetToken.mint(msg.sender, _amount);
        emit Deposited(msg.sender, _amount.mul(jonesAssetPerAsset()), _amount);
    }

    /// Burn jAsset and redeem Asset from the vault.
    /// @dev Assumes both tokens have same decimal places.
    /// @param _amount Amount of jAsset to burn.
    function claimAsset(uint256 _amount) public payable {
        ...
        require(
            assetToken.balanceOf(address(this)) >=
                _amount.div(jonesAssetPerAsset()),
            "Not enough Assset in the contract."
        );
        jonesAssetToken.burnFrom(msg.sender, _amount);
        assetToken.transfer(msg.sender, _amount.div(jonesAssetPerAsset()));
        emit Claimed(msg.sender, _amount.div(jonesAssetPerAsset()), _amount);
    }
    ...
    /// Calculates how much jAsset one Asset can buy.
    /// e.g 100 jgOHM, 200 gOHM, function returns 0.5 because 1 gOHM = 0.5 jgOHM.
    /// @return jAsset per Asset.
    function jonesAssetPerAsset() public view returns (uint256) {
        return snapshotJonesAssetSupply.div(snapshotVaultBalance);
    }

    /// Withdraws all Asset from the vault to the aumMultisigAddr address.
    function withdrawAll() public payable onlyOwner {
        require(!DEPOSIT_WINDOW_OPEN, "Mint window is open");
        require(!CLAIM_WINDOW_OPEN, "Burn window is open");
