...
contract Summoning is Initializable, SummoningLPStakable {
    ...
    function _startSummonSingle(uint256 _tokenId, uint256 _crystalId) private returns(uint256) {
        ...
        emit SummoningStarted(msg.sender, _tokenId, _requestId);
    ...
    function _finishSummon(uint256[] memory _inProgressSummons) private {
        ...
        for(uint256 i = 0; i < _inProgressSummons.length; i++) {
            ...
            if(_newTokenId != 0) {
                ...
                emit SummoningFinished(msg.sender, _tokenId, _newTokenId);
            }
        }
    ...
    returns(uint256, uint256)
    {
        ...
        legionMetadataStore.setInitialMetadataForLegion(_newTokenId, LegionGeneration.AUXILIARY, _newClass, _newRarity);
...
abstract contract SummoningState is Initializable, ISummoning, ERC721HolderUpgradeable, ERC1155HolderUpgradeable, AdminableUpgradeable {
    ...
    event SummoningStarted(address indexed _user, uint256 indexed _tokenId, uint256 indexed _requestId);
    event NoSummoningToFinish(address indexed _user);
    event SummoningFinished(address indexed _user, uint256 indexed _returnedId, uint256 indexed _newId);
