...
contract Margin is ERC20 {
  ...
  constructor(address _dopex) ERC20('Margin Pool USDT', 'mpUSDT') {
    Dopex dopexContract = Dopex(_dopex);
    dopex = dopexContract;
    // Caches USDT contract address on deploy to save gas
    usdt = IERC20(dopexContract.getAddress('USDT'));
    ...
    address usdtAddress = dopexContract.getAddress('USDT');
    collateralAssets[usdtAddress] = Collateral({
      collateralizationRatio: 100,
      liquidationRatio: 100
    });
    collaterals.push(usdtAddress);
    emit UpdateCollateral(usdtAddress, 100, 100);
  }
