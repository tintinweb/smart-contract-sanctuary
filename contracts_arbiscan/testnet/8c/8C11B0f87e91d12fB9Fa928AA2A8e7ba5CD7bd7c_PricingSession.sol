...
contract PricingSession is ReentrancyGuard {
    ...
    mapping(bytes32 => VotingSessionMapping) NftSessionMap;
    ...
    mapping(bytes32 => VotingSessionChecks) public NftSessionCheck;
    ...
    mapping(bytes32 => VotingSessionCore) public NftSessionCore;

    /// @notice maps each NFT pricing session (nonce dependent) to its final appraisal value output
    mapping(bytes32 => uint) public finalAppraisalValue;
    ...
    */ 
    ...
    function createNewSession(
        ...
        bytes32 sessionHash = keccak256(abi.encodePacked(nftNonce[nftAddress][tokenid], nftAddress, tokenid));
        require(_votingTime <= 1 days && (!auctionStatus || msg.sender == auction));
        VotingSessionCore storage sessionCore = NftSessionCore[sessionHash];
        ...
        sessionHash = keccak256(abi.encodePacked(nftNonce[nftAddress][tokenid], nftAddress, tokenid));
        VotingSessionCore storage sessionCoreNew = NftSessionCore[sessionHash];
    ...
    */
    ...
    function setVote(
        ...
        bytes32 sessionHash = keccak256(abi.encodePacked(nftNonce[nftAddress][tokenid], nftAddress, tokenid));
        VotingSessionCore storage sessionCore = NftSessionCore[sessionHash];
        VotingSessionMapping storage sessionMap = NftSessionMap[sessionHash];
        require(sessionCore.endTime > block.timestamp);
        sessionMap.voterCheck[msg.sender] = 1;
        if(msg.value <= principalStored[msg.sender]) {
    ...
    */
    ...
    function weightVote(address nftAddress, uint tokenid, uint appraisal, uint seedNum) checkParticipation(nftAddress, tokenid) nonReentrant external {
        ...
        VotingSessionCore storage sessionCore = NftSessionCore[sessionHash];
        VotingSessionMapping storage sessionMap = NftSessionMap[sessionHash];
        VotingSessionChecks storage sessionCheck = NftSessionCheck[sessionHash];
        ...
        emit voteWeighed(msg.sender, nftNonce[nftAddress][tokenid], nftAddress, tokenid, appraisal);
    ...
    function setFinalAppraisal(address nftAddress, uint tokenid) public {
        ...
        finalAppraisalValue[sessionHash] = (sessionCore.totalAppraisalValue)/(sessionCore.totalVotes);
        sessionCore.totalVotes = 0;
        sessionCore.totalAppraisalValue = 0;
        sessionCore.maxAppraisal = 0;
        sessionCheck.sessionProgression = 3;
        emit finalAppraisalDetermined(nftNonce[nftAddress][tokenid], nftAddress, tokenid, finalAppraisalValue[sessionHash], sessionCore.uniqueVoters, sessionCore.totalSessionStake);
    }
    ...
    */
    ...
    function harvest(address nftAddress, uint tokenid) checkParticipation(nftAddress, tokenid) nonReentrant external {
        ...
        bytes32 sessionHash = keccak256(abi.encodePacked(nonce, nftAddress, tokenid));
        VotingSessionCore storage sessionCore = NftSessionCore[sessionHash];
        VotingSessionChecks storage sessionCheck = NftSessionCheck[sessionHash];
        VotingSessionMapping storage sessionMap = NftSessionMap[sessionHash];
        require(
    ...
    */
    ...
    function _harvest(address nftAddress, uint tokenid) internal {
        ...
        sessionMap.nftVotes[msg.sender].base = 
            PostSessionLibrary.calculateBase(
                finalAppraisalValue[sessionHash], 
    ...
    function addToAppraisal(address nftAddress, uint tokenid, address user) payable external {
        ...
        VotingSessionCore storage sessionCore = NftSessionCore[sessionHash];
        require(
            NftSessionMap[sessionHash].voterCheck[user] == 1
            && sessionCore.endTime > block.timestamp
        );
        NftSessionMap[sessionHash].nftVotes[user].stake += msg.value;
        sessionCore.totalSessionStake += msg.value;
    ...
    function getStatus(address nftAddress, uint tokenid) view public returns(uint) {
        bytes32 sessionHash = keccak256(abi.encodePacked(nftNonce[nftAddress][tokenid], nftAddress, tokenid));
        return NftSessionCheck[sessionHash].sessionProgression;
    }
    ...
    function getEthPayout(address nftAddress, uint tokenid) view external returns(uint) {
        ...
        return sessionCore.totalSessionStake * NftSessionMap[sessionHash].winnerPoints[msg.sender] / sessionCore.totalWinnerPoints;
    }
    ...
    function getVoterCheck(address nftAddress, uint tokenid, address _user) view external returns(uint) {
        bytes32 sessionHash = keccak256(abi.encodePacked(nftNonce[nftAddress][tokenid], nftAddress, tokenid));
        return NftSessionMap[sessionHash].voterCheck[_user];
    }
    ...
    modifier stopOverwrite(
        ...
        require(
            ...
            || block.timestamp > NftSessionCore[sessionHash].endTime + NftSessionCore[sessionHash].votingTime * 3
        );
        _;
    }
    ...
    modifier properVote(
        ...
        bytes32 sessionHash = keccak256(abi.encodePacked(nftNonce[nftAddress][tokenid], nftAddress, tokenid));
        require(
            NftSessionMap[sessionHash].voterCheck[msg.sender] == 0
    ...
    modifier checkParticipation(
        ...
        bytes32 sessionHash = keccak256(abi.encodePacked(nftNonce[nftAddress][tokenid], nftAddress, tokenid));
        require(NftSessionMap[sessionHash].voterCheck[msg.sender] > 0);
        _;
    }
}