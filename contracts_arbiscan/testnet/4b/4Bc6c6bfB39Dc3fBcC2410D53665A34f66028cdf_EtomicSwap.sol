...
contract EtomicSwap {
    ...
    function ethPayment(
        ...
        bytes20 paymentHash = bytes20(uint160(uint256(keccak256(abi.encodePacked(
                _receiver,
                msg.sender,
                _secretHash,
                address(0),
    ...
    function receiverSpend(
        ...
        bytes20 paymentHash = bytes20(uint160(uint256(keccak256(abi.encodePacked(
                msg.sender,
                _sender,
                bytes20(uint160(uint256(keccak256(abi.encodePacked(sha256(abi.encodePacked(_secret))))))),
                _tokenAddress,
                _amount
            )))));
