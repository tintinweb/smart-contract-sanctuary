...
contract Pilgrimage is Initializable, PilgrimageTimeKeeper {
    ...
    function _embarkOnPilgrimage(uint256 _id, LegionGeneration _generation) private {
    ...
    whenNotPaused
    {
        for(uint256 i = 0; i < _tokenIds.length; i++) {
            require(userToPilgrimagesInProgress[msg.sender].contains(_tokenIds[i]), "Token does not belong to user");
        }

        _returnFromPilgrimages(_tokenIds);
    }
    ...
    function _returnFromPilgrimages(uint256[] memory _inProgressPilgrimages) private {
        ...
        if(_minted721sIndex > 0) {
            emit PilgrimagesFinished(msg.sender, _minted721s);
        } else {
    ...
    function _returnFromPilgrimage(uint256 _pilgrimageID) private returns(uint256) {
        ...
        legionMetadataStore.setInitialMetadataForLegion(_tokenId, pilgrimageIdToGeneration[_pilgrimageID], _class, _rarity);
...
abstract contract PilgrimageState is Initializable, IPilgrimage, AdminableUpgradeable {
    ...
    event PilgrimagesStarted(address indexed _user, uint256 indexed _numberStarted);
    event NoPilgrimagesToFinish(address indexed _user);
    event PilgrimagesFinished(address indexed _user, uint256[] _tokenIds);
...
interface ILegionMetadataStore {
    ...
    // Admin only.
    function setInitialMetadataForLegion(uint256 _tokenId, LegionGeneration _generation, LegionClass _class, LegionRarity _rarity) external;
