...
contract MaviaTokenL2 is ERC20Token, IArbToken {
  address public l2CustomGateway;
  address public override l1Address;

  modifier onlyGateway() {
    require(msg.sender == l2CustomGateway, "ONLY_l2CUSTOMGATEWAY");
    _;
  }
   ...
   */
  function __MaviaTokenL2_init(address _l2CustomGateway, address _l1Counterpart) public initializer {
    __ERC20Token_init("MaviaToken", "MAVIA", 18, 0);
   ...
   * @dev Bridge burn
   */
  function updateArbConfig(address _l2CustomGateway, address _l1Counterpart) external onlyOwner {
    l2CustomGateway = _l2CustomGateway;
    l1Address = _l1Counterpart;
  }
}
...
contract Blacklist is OwnableUpgradeable {
  ...
  function fSetBlacklist(address _pAccount, bool _pStatus) external onlyOwner {
    require(isInBlacklist[_pAccount] != _pStatus, "Added");
...
contract ERC20Token is ERC20Upgradeable, OwnableUpgradeable, IBurnable, Blacklist {
  ...
  function fTransferAnyERC20Token(
    ...
    address _pTo,
    uint _pAmount
  ) external onlyOwner returns (bool _rSuccess) {
