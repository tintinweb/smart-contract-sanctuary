...
struct Draw { uint256 winnerPlayerIndex; bool prizeClaimed; address[] players; uint256 prizePool; bool drawEnded; uint256 deadline; uint256 winnerPrize; uint256 numberOfTickets; }
...
// STATE VARIABLES
    ...
    /**
     * @dev Prize Pool % of total
     */
    uint256 public winnerPercent = 68;
...
// EVENTS
    ...
    event DrawClosed(address closedBy, uint256 blocknumber, uint256 burnedAmount, uint256 closerFee);
    event PrizeClaimed(address claimedBy, uint256 blocknumber, uint256 amountClaimed);
    event PayoutStructureChanged(address updatedBy, uint256 winnerPercent, uint256 burnPercent, uint256 closerFeePercent);
...
// MODIFIERS
    ...
    modifier withinDeadline() {
        Draw memory draw = draws[getCurrentDraw()];                   // Shortcut accessor for the Draw
        require(block.number <= draw.deadline,                        "This draw has reached it's deadline");
        _;
    }
    ...
    modifier drawComplete(uint256 drawIndex) {
        ...
        require(draw.drawEnded,                                       "This draw is still ongoing");
        _;
    }
    ...
    modifier isWinner(uint256 drawIndex) {
        ...
        require(_msgSender() == draw.players[draw.winnerPlayerIndex], "Sorry, you didn't win this time!");
        _;
    }
...
// PUBLIC FUNCTIONS
    ...
    /**
     * @dev PUBLIC: Anyone can buy a ticket
     */
    ...
    /**
     * @dev PUBLIC: Winner can claim a prize
     */
        ...
        isADraw(drawIndex)
    {
        ...
        require (paymentContract.balanceOf(address(this)) >= amount,                  "Contract balance isn't enough to cover the winner");
    ...
    /**
     * @dev PUBLIC: Press the big red button & choose the winner, pay the closer & complete the burn
     */
    function closeDraw() 
        external
        nonReentrant
    ...
    {
        Draw storage draw = draws[getCurrentDraw()];                                  // Shortcut accessor for the Draw
        require(!draw.drawEnded,                                                      "The lottery has ended. Please check if you won the price!");
        ...
        if (numberOfPlayers > 0 ) {
            draw.winnerPlayerIndex = random(numberOfPlayers, _msgSender());           // Assign winner
            draw.winnerPrize = draw.prizePool * winnerPercent / 100;                  // Assign prize
            closersFee = draw.prizePool * closerFeePercent / 100;                     // Calculate the closers fee for this Draw
            burnAmount = draw.prizePool * burnPercent / 100;                          // Calculate the burn for this Draw
        }
        draw.drawEnded = true;                                                        // Flag draw as ended
        ...
        emit DrawClosed(_msgSender(), block.number, burnAmount, closersFee);          // Write an event to the chain
        if (closersFee > 0) {
...
// ADMIN FUNCTIONS
    ...
    /**
     * @dev ADMIN: Allow change to the ticket prices
     */
    ...
    /**
     * @dev ADMIN: Allow change in the draw length
     */
    ...
    /**
     * @dev ADMIN: Start or pause the contract
     */
        ...
        onlyOwner
    {
        if ( paymentContractAddress != address(0) && 
             paymentContractAddress != address(IERC20(paymentContract))
        ) {
    ...
    /**
     * @dev ADMIN: Enable changes to draw payout structure
     */
    function setDrawPayoutStructure(uint256 winner, uint256 burn, uint256 closerFee)
        external
        onlyOwner
    {
        require ( winner + burn + closerFee == 100, "Total should equal 100");
        winnerPercent = winner;
        burnPercent = burn;
        closerFeePercent = closerFee;
        emit PayoutStructureChanged(_msgSender(), winner, burn, closerFee);
    }

// GETTERS

    /**
     * @dev PUBLIC: Return list of players for a draw
     */
    function getPlayersByDraw(uint256 drawIndex)
        ...
        isADraw(drawIndex)
        returns (
            address[] memory players                // Define the array of addresses / players to be returned. Requires ABIEncoderV2.
        )
    {
        Draw memory draw = draws[drawIndex];        // Shortcut accessor for the Draw
        return draw.players;                        // Return the array of collectibles on the farm
    }
    ...
    /**
     * @dev PUBLIC: Return unclaimed wins by address
     */
        ...
        returns (uint256[] memory winners)
    {
        ...
        for(uint256 drawIndex = 0; drawIndex < total; drawIndex++){        // Loop through the draws
            if ( !draws[drawIndex].prizeClaimed &&                         // Check if win is unclaimed
                ...
                {
                    unclaimedTotal++;                                      // Add drawIndex to _winners array
                }
        }
