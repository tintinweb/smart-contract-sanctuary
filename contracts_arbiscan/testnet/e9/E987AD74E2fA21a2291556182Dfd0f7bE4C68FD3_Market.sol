...
contract Market is Initializable, OwnableUpgradeable, BlockLock, PausableUpgradeable, IMarket {
    ...
    event Collateralize(uint256 amount);
    ...
    function collateral(address _borrower) public view override returns (uint256) {
        return convertFrom18(collaterals[_borrower]);
    }
    ...
    function sendCollateralToLiquidator(
        ...
        IERC20(underlyingAssetAddress).safeTransfer(_liquidator, convertFrom18(tokens));
    }
    ...
    function withdraw(uint256 _amount) external notLocked(msg.sender) whenNotPaused {
        ...
        require(
            healthRatioAfterWithdraw == FACTOR || healthRatioAfterWithdraw >= 100,
        ...
        IERC20(underlyingAssetAddress).safeTransfer(msg.sender, _amount);
    }
...
contract BlockLock {
    ...
    function lock(address lockAddress) internal {
