...
contract ResearchLab {
  ...
  function build(uint256 _realmId, uint256 _queueSlot) external {
    ...
    uint256 buildTime = _buildTime(_realmId);
    DATA.addToBuildQueue(_realmId, _queueSlot, buildTime);

    _updateData(_realmId, buildTime, 0);
  }
  ...
  function build(
    ...
    require(owner == msg.sender || REALM.isApprovedForAll(owner, msg.sender));
    require(_workforce <= 24, "Must be less than 24");

    DATA.remove(_realmId, 2, _workforce);

    uint256 buildTime = _buildTime(_realmId);

    if (_workforce <= buildTime) {
      buildTime = buildTime - _workforce;
    } else {
      buildTime = 0;
    }
  ...
  function buildWithGold(
    ...
    _updateData(_realmId, buildTime, _gold);
  }
  ...
  function _updateData(
    ...
    if (rarity > 0) {
      ...
      RESOURCE.add(_realmId, resourceIds[rarity], metricAdded);
      resourceAdded = metricAdded;
    }

    if (_buildTimeBonus > 11) {
      metricAdded = metricAdded + (_buildTimeBonus - 12) + _bonus;
    }

    DATA.add(_realmId, 5, metricAdded);
  ...
  function _buildTime(uint256 _salt) internal view returns (uint256) {
