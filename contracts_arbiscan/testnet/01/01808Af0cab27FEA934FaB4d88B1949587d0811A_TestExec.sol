 ...
 */
contract TestExec {
    event Exec(bool success, bool pmAccepted, bytes res);
    ...
    function relayCall(
        ...
        try hub.relayCall(maxAcceptanceBudget, relayRequest, signature, approvalData)
            returns (bool paymasterAccepted, bytes memory returnValue) {

            emit Exec(true, paymasterAccepted, returnValue);
        } catch Error(string memory revertReason) {
            emit Exec(false, false, bytes(revertReason));
        } catch (bytes memory rawError){
            emit Exec(false, false, rawError);
        }
    }
}
