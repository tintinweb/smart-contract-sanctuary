...
contract DuneSpices is Initializable, ERC20Upgradeable, ERC20BurnableUpgradeable, OwnableUpgradeable, ERC20PermitUpgradeable, ERC20VotesUpgradeable, UUPSUpgradeable {
    ...
    // avatarID => AVATARINFO
    mapping(uint256=>AVATARINFO) _avatarInfo;

    // start a risk
    function open(uint256 avatarID, uint256 landID, uint256 exitBlock) public {
        // check the avatarID

        // check the landID

        // record this into AVATARINFO and LANDINFO

    }

    // start liquidation
    function close(uint256 avatarID) public {
        // check the avatarID whether belongs to singer

        // check the avatar info by AVATARINFO

        // Is it alive

        // Whether to come out in advance

        // Calculated production of spices

        // mint Spices to singer address
