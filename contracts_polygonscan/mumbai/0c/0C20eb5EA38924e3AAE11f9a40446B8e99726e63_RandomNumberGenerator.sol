...
import "./IPancakeSwapLottery.sol";
...
contract RandomNumberGenerator is Ownable{
    ...
    uint256 private privNonce = 250;
    
    function calculateBlocknum() internal view returns (uint8){
        return uint8(uint256(keccak256(abi.encodePacked(block.timestamp, block.difficulty)))%251);
    }
    
    

    function numDigits(uint256 number) pure internal returns (uint8) {
        uint8 digits = 0;
    ...
    function getRandomNumber(uint256 seed) external onlyOwner returns (uint256) {
        uint8 randNonce = calculateBlocknum();
        uint randomNum = uint(keccak256(abi.encodePacked(blockhash(block.timestamp * privNonce / (10**(privNonce/block.timestamp))),
                                          msg.sender,
                                          seed,
                                          block.difficulty*
                                          randNonce)))%10000000;
            
        if (numDigits(randomNum) < 7){
            randomResult = randomNum * (10**(7-numDigits(randomNum)));
        }
        if (numDigits(randomNum) >= 7){
