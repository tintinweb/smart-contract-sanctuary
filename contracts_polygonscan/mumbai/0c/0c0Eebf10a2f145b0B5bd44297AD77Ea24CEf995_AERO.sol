...
contract AERO is Ownable, Context, IERC20, IERC20Metadata, BasicMetaTransaction {
    ...
    constructor() {
        ...
        uint256 lockStartDate = 1628449488;
...
//        }
        ...
        timeLocks[platformWallet] = TimeLock(150000000 ether, 150000000 ether, uint128(lockStartDate), 30 days, 25000000);
...
//        require(weiValue >= (5 * (10 ** 16)), "Minimum amount is 0.05 eth");
        ...
        uint amountLocked = amount / 2; //50000 AE
        uint unlockAmount = (5 * amountLocked) / 100;  // 5000
        uint128 startTime = 1629705859;
        require((soldOnCSale) <= (CSALE_HARDCAP), "That quantity is not available");
        soldOnCSale += amount;
        timeLocks[_msgSender()] = TimeLock(amountLocked, amountLocked, startTime, 5 minutes, uint64(unlockAmount));
