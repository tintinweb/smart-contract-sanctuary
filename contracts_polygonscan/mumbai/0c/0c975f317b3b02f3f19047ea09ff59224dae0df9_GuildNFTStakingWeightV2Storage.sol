...
contract GuildNFTStakingWeightV2Storage is IGuildNFTStakingWeightStorage, BaseRelayRecipient, Initializable {
    ...
    function updateStakingWeightContract(address _weightContract) external {
        require(
            accessControls.hasAdminRole(_msgSender()),
            "GuildNFTStaking.updateStakingWeightContract: Sender must be admin"
        );
        require(address(_weightContract) != address(0), "GuildNFTStakingWeightV2.updateAccessControls: Zero Address");
        weightContract = _weightContract;
    }
