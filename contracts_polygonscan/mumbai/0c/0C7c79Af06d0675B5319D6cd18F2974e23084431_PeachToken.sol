...
contract PeachToken is ERC20 {
    ...
    event Mint(address request, uint256 amount);

    constructor() ERC20("Peach Token", "PEA") {
        owner = msg.sender;
        _mint(msg.sender, 1000000000e18);
    }

    function setOperator(address _operator) external returns (bool) {
        require(msg.sender == owner, "shuold be owner");
        operator = _operator;
        return true;
    }

    function burn(uint256 _amount) external returns (bool) {
        require(msg.sender == owner, "should be operator");
