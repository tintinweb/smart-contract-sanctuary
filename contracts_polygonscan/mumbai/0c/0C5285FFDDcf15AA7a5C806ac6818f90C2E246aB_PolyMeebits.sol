...
contract PolyMeebits is IERC721 {
    ...
    function mint() external payable reentrancyGuard returns (uint) {
        ...
        // uint ownerAmount = 0;
            ...
            uint feeAmount = ownerTrx * nftPrice;
            uint feeOne = (feeAmount *40 )/100;
            uint feeTwo = (feeAmount *60)/100;
            
            payFee(feeOne ,developerA1);
            payFee(feeTwo,firstOwner);
    ...
    function payFee(uint _amount,address _to) internal  {
        
        // uint feeOne = (_amount *40 )/100;
        // uint feeTwo = (_amount *60)/100;
        // developerA1.transfer(feeOne);
        // firstOwner.transfer(feeTwo);
    ...
    function tokenURI(uint256 _tokenId) external view validNFToken(_tokenId) returns (string memory) {
        return string(abi.encodePacked("https://polymeebits.com/api/meebits/", toString(_tokenId)));
    }
