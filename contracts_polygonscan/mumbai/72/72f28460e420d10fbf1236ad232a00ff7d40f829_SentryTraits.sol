...
// TypeError: Member "length" not found or not visible after argument-dependent lookup in mapping(uint8 => struct Traits.Layer storage ref).
    ...
    function assembleTraits(uint16 tokenId) external onlyController returns (Traits memory) {
        Sentry memory sentry = factory.getSentryTraits(tokenId);
        Traits memory newTraits = Traits(
            layerCount[sentry.species][generateRarity(layerCount[sentry.species][0])],
            layerCount[sentry.species][generateRarity(layerCount[sentry.species][1])],
            sentry.generation == 0 ? 0 : 1,
            layerCount[sentry.species][generateRarity(layerCount[sentry.species][4])],
            layerCount[sentry.species][generateRarity(layerCount[sentry.species][5])],
            layerCount[sentry.species][generateRarity(layerCount[sentry.species][6])],
            layerCount[sentry.species][generateRarity(layerCount[sentry.species][7])]
        );
        return newTraits;
    }
