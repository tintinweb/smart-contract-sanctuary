...
contract DPSCartographer is Ownable, ReentrancyGuard {
    ...
    mapping(address => uint256) private lastCreatedVoyage;
        ...
        returns (VoyageConfig memory voyageConfig)
    {
        ...
        if (causalityParams.signature.length > 0) {
            ...
            randomInteractionsConfig.randomNoOfStorms = causality.getRandom(
                causalityParams.userAddress,
                causalityParams.blockNumber[1],
                causalityParams.hash1[1],
                causalityParams.hash2[1],
                causalityParams.timestamp[1],
                causalityParams.signature[1],
                "NOOFSTORMS",
        ...
        for (uint256 i; i < configForThisInteraction.totalInteractions; i++) {
            ...
            if (randomInteractionsConfig.positionsForGeneratingInteractions.length == 1) {
                randomPosition = 0;
            } else if (causalityParams.signature.length > 0) {
                randomPosition = causality.getRandom(
                    causalityParams.userAddress,
                    causalityParams.blockNumber[i + 2],
                    causalityParams.hash1[i + 2],
                    causalityParams.hash2[i + 2],
                    causalityParams.timestamp[i + 2],
                    causalityParams.signature[i + 2],
