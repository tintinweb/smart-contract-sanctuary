...
contract Registry is Ownable {
    ...
    /// @dev Mapping from moduleType to factory address
    mapping(uint256 => address) public moduleFactories;
    ...
    /// @dev Emitted when a new factory address is added
    event NewFactory(uint256 moduleType, address factoryAddress);
    ...
    /// @dev Sets module factory address.
    function setModuleFactory(uint256 _moduleType, address _factory) external onlyOwner {
        moduleFactories[_moduleType] = _factory;

        emit NewFactory(_moduleType, _factory);
    }
...
// Factories
import { LazyNFTFactory } from "./LazyNFT/LazyNFTFactory.sol";
...
contract ProtocolControl is AccessControlEnumerable {
    ...
    /// @dev Events.
    event ModuleCreated(uint256 indexed moduleType, address indexed module, bytes32 moduleId);
    ...
    function createLazyMint(
        ...
        uint256 _maxSupply
    ) external onlyProtocolAdmin returns (bytes32 moduleId) {
        uint256 _moduleType = 7;

        address moduleFactory = Registry(registry).moduleFactories(_moduleType);

        address _newModuleAddress = address(
            LazyNFTFactory(moduleFactory).deploy(_name, _symbol, _baseTokenUri, _maxSupply)
        );
        ...
        emit ModuleCreated(_moduleType, _newModuleAddress, moduleId);
    }
...
contract LazyNFTFactory is Ownable {
    address public forwarder;
    ...
    function deploy(
        ...
        string memory _baseTokenUri,
        uint256 _maxSupply
    ) external returns (LazyNFT) {
        address _admin = _msgSender();

        return new LazyNFT(payable(_admin), _name, _symbol, forwarder, "", _baseTokenUri, _maxSupply, 0, 0, _admin);
    }
    ...
    Multicall
{
    ...
    constructor(
        ...
        _setupRole(DEFAULT_ADMIN_ROLE, _saleRecipient);
        _setupRole(MINTER_ROLE, _saleRecipient);
        _setupRole(TRANSFER_ROLE, _saleRecipient);
