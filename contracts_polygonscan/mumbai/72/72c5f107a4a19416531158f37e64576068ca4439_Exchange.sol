...
contract Exchange is Ownable{
    ...
    function placeBid(uint256 aucId, uint256 amount) public {
        ...
        require( approveValue >= amountTot, "oops! Insufficient Balance");
    ...
    function claimWinner(uint256 aucId, uint256 bidId) public {
        address userAddr = msg.sender;
        require(action[aucId].bidId == 0, "Auction already completed");
        require(bidData[aucId][userAddr].amount > 0, "oops! Bid is not valid");
        require(bidData[aucId][userAddr].status == 1, "oops! Accept bid not allowed to you");

        string memory tokenName = action[aucId].tokenName;
        address tokenAddr = tokenType[tokenName].addr;
        uint256 amount = bidData[aucId][userAddr].amount;
        address conaddress = action[aucId].nftContAddr;
        uint256 Id = action[aucId].nftId;

        _claimWinner(tokenAddr, userAddr, owner(), amount, Id, conaddress);
        IERC721(conaddress).safeTransferFrom(msg.sender, userAddr, Id);
        bidData[aucId][userAddr].status = 2;
