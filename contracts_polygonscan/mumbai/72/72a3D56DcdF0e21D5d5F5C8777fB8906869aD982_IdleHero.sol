...
contract IdleHero is IIdleHero, ERC721URIStorage, ERC721Enumerable, SecurityBase
{
    ...
    function preMintHero(address to, uint256 _tokenId, uint256 dna) public whenNotPaused onlyMinter returns (uint256) {
    ...
    function setBaseCounter(uint256 _value) public whenNotPaused onlyMinter {
    ...
    function addParentsChilds(uint256 parentIDA, uint256 parentIDB, uint256 tokenId) public whenNotPaused onlyMinter override {
        ...
        emit BornUpdate(tokenId, _heroMap[tokenId].parents, _heroMap[parentIDA].childs, _heroMap[parentIDB].bornCount);
        emit BornUpdate(parentIDA, _heroMap[parentIDA].parents, _heroMap[parentIDA].childs, _heroMap[parentIDA].bornCount);
        emit BornUpdate(parentIDB, _heroMap[parentIDB].parents, _heroMap[parentIDB].childs, _heroMap[parentIDB].bornCount);
    }
}
