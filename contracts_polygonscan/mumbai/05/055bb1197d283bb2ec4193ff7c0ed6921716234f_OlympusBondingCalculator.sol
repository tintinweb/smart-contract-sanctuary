...
contract OlympusBondingCalculator is IBondCalculator {
    ...
    constructor(address _oxygen) {
        require(_oxygen != address(0));
        oxygen = _oxygen;
    }

    function getKValue(address _pair) public view returns (uint256 k_) {
        uint256 token0 = IOxygen(IUniswapV2Pair(_pair).token0()).decimals();
        uint256 token1 = IOxygen(IUniswapV2Pair(_pair).token1()).decimals();
        uint256 decimals = token0.add(token1).sub(IOxygen(_pair).decimals());
    ...
    function markdown(address _pair) external view override returns (uint256) {
        ...
        uint256 reserve;
        if (IUniswapV2Pair(_pair).token0() == oxygen) {
            reserve = reserve1;
        } else {
            reserve = reserve0;
        }
        return reserve.mul(2 * (10**IOxygen(oxygen).decimals())).div(getTotalValue(_pair));
    }
}