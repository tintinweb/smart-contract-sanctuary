...
contract FuelToGRVX is Ownable {
    ...
    function burnGRVX(uint256 amount) external {
        require(burnStatus == true, "Contract not active at the moment");
        User storage user = addressToUser[msg.sender];
        if(block.timestamp - user.lastBurned>=DAY){
            user.burnedThisPeriod=0;
            user.lastBurned=block.timestamp;
        }
        user.burnedThisPeriod+=amount;
        uint256 rate;
        if(user.burnedThisPeriod<31){
            rate=500;
        }else if(user.burnedThisPeriod<76){
            rate=600;
        }else if(user.burnedThisPeriod<151){
            rate=700;
        }else if(user.burnedThisPeriod<301){
            rate=800;
        }else{
            rate=900;
        }
        uint256 amountBurnt = amount * 10**18;
        uint256 fuelMinted = amountBurnt / rate;
        grvx.safeTransferFrom(msg.sender, grvxOutlet, amountBurnt);
        fuel.mint(msg.sender, fuelMinted);
        emit grvxBurnt(msg.sender, amountBurnt, fuelMinted);
    }
