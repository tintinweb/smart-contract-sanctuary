...
contract NFTTrade is Ownable, IERC721Receiver, ReentrancyGuard, EIP712Base, BaseRelayRecipient {
    ...
    constructor(
        ...
        treasuryAmount = _treasuryAmount.mul(1e18); // expected to be passed as a normal number, and converted to decimal-adjusted value
    }
    ...
    function setTreasuryAmount(uint256 _treasuryAmount) external onlyAdmin {
        ...
        treasuryAmount = _treasuryAmount.mul(1e18);
    }

    function transferToTreasury(address _traderOne, address _traderTwo) internal {
        uint256 perHeadFee = treasuryAmount.div(2);
    ...
    function acceptOffer(
        ...
        for (uint256 i = 0; i < _nftIdsOffered.length; i++) {
            ...
            IERC721(_nftAddressesOffered[i]).transferFrom(offer.offerMaker, bundle1Owner, _nftIdsOffered[i]);
        }
        for (uint256 i = 0; i < _nftIdsWanted.length; i++) {
            _nftToBundleId[_nftAddressesWanted[i]][_nftIdsWanted[i]] = 0;
            IERC721(_nftAddressesWanted[i]).transferFrom(bundle1Owner, offer.offerMaker, _nftIdsWanted[i]);
        }
