...
contract drop1 {
    ...
    function getAxie(uint256 _axieID) external view returns (address,uint256,uint256,uint256) {
         return Origin(core).getAxie(_axieID);
    }

    function setAxie(address _to, uint256 _dna, uint256 _bornAt) external returns (uint256) {
        return Origin(core).birthAxie(_to,_dna,_bornAt);
    }
