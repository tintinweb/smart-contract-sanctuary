...
abstract contract NFTInterface {
    ...
    function addNftPurchaser(uint __tokenId, address  __buyer) public virtual ;//, uint prezzo, string memory __email
    function deleteNftPurchaser(uint __tokenId, address  seller  )virtual public ;
  ...
  ERC721Metadata, Ownable
{
   ...
   */
   uint256 constant public tokenPrice = 0.00001 ether;//1000000000000000
   uint256 public nftTokenIdLast;
   ...
   // address  public NftContractSaler;
    ...
    uint[] nftTokenIds;
    uint256 constant public TOTAL_SUPPLY = 3333;
  ...
  constructor()
  {
    ...
    supportedInterfaces[0x780e9d63] = true; // ERC721Enumarable
    nftName = "Super Mario Collection";
    nftSymbol = "SuperMario";
    artistAddress = payable(0x4938800209c36CfFcDEeC7836DdECfd58D6F5574);
    feeArtclub = 20;
    owner = msg.sender;
  ...
  {
    require(NftContractSaler[msg.sender] == true,"Only approved contract can resell NFT");
 ...
 */
    ...
    function addNftPurchaser(uint __tokenId, address  __buyer) public {//, uint prezzo
    require(NftContractSaler[msg.sender] == true,"Only approved contract can act here");
    tokenIdOwners[__tokenId].purchaserArray.push(__buyer);
   // nftTokenIds.push(__tokenId);
  //  emit newTokenMinted(__tokenId);
    }
    
    function deleteNftPurchaser(uint __tokenId, address  exOwner  ) public  {
    require(NftContractSaler[msg.sender] == true,"Only approved contract can act here");
    for(uint index = 0;index < tokenIdOwners[__tokenId].purchaserArray.length;index++){
        if(tokenIdOwners[__tokenId].purchaserArray[index]==exOwner){
            delete tokenIdOwners[__tokenId].purchaserArray[index];
    ...
    function getNftTokenIds() public view returns (uint [] memory) {
  		return nftTokenIds;
	}
...
function mint(
  ...
  function buyTokenNFT (string memory _uri)  external payable returns (bool success){//override
        uint256 amount = msg.value;
        require(nftTokenIds.length <= TOTAL_SUPPLY,"Collection is not more available");
        require(amount >= tokenPrice,"Not enought amount to buy NFT");
        
        
         nftTokenIdLast += 1;
        
         mint(msg.sender,nftTokenIdLast,_uri);
                     
         //addNftPurchaser(nftTokenIdLast,msg.sender);
    ...
    function claimPurchases() public  {//onlyOwner
    require(NftContractSaler[msg.sender],"Only Admin can act this operation");
        uint amount = address(this).balance;
        uint commissione = amount * feeArtclub / 100;
        uint quotaArtista = amount - commissione ;
        payable(address(owner)).transfer(commissione);
       
        artistAddress.transfer(quotaArtista);
...
/* ENUMERABLE FUNCTIONS
    function tokenOfOwnerByIndex(address owner, uint256 index) public view virtual returns (uint256) {
        ...
        // ...
        return artIdOwners[__artId].artworksOwnedBy[owner].artworkArray[index];//artworksOwnedBy[owner].[index];
    }*/
