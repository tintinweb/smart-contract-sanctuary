...
contract AddLiquidityTest is KiBridgeV1Factory{
  ...
  function removeLiquidity(
    IKiBridgeV1Liquidity _liquidity, 
    uint256 _amount
  ) 
    external
  {
    require(_amount > 0,"Amount must be greater than zero");
    _liquidity.burnFrom(msg.sender, _amount);
    IERC20 token = IERC20(IKiBridgeV1Liquidity(_liquidity).token());
    liquidityInfo[token].toAllChainableBalance -= _amount;
