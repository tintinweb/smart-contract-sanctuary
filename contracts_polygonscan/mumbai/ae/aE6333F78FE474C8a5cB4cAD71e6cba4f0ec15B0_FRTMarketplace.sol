...
contract FRTMarketplace is Ownable{
    ...
    function mintNFT(string memory tokenHash) public {

        
        uint256 newItemId = baseNFT.mintNFT( msg.sender, tokenHash);
        listedNFTs[newItemId] = true; // we need to list and approve one by one because nfts must be avaliable for sell immidiatly  
        if(!mintedURIs[tokenHash]){
            baseToken.transfer(msg.sender, baseTokenReward);        
            mintedURIs[tokenHash] = true;
        }
    }

    // function isApprovalForAllNFT() public view returns (bool){
    //     return baseNFT.isApprovedForAll(msg.sender, address(this));
    // }

    function checkAlowance() public view returns (uint256){
        return baseToken.allowance(msg.sender, address(this));
    }
    ...
    function buyNFT(uint256 tokenId) public {
        require(listedNFTs[tokenId], "Token not listed for sale");
        require(checkAlowance() >= baseNFTPrice, "FRTMarketplace: transfer amount exceeds allowance");
        // baseToken.approve(spender, amount);
        baseToken.transferFrom(msg.sender, baseNFT.ownerOf(tokenId), baseNFTPrice);
        baseNFT.transferFrom(baseNFT.ownerOf(tokenId), msg.sender, tokenId);
    }

    // function listForSelling(uint256 tokenId) public {
    //     baseNFT.approve(address(this), tokenId);
    // }
    /////asdasdas
