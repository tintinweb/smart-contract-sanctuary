...
import "./Rift721Receiver.sol";
import "./RiftDragonStakable.sol";
import "./RiftContracts.sol";

contract Rift is Initializable, RiftDragonStakable, Rift721Receiver {

    function initialize() public initializer {
        RiftDragonStakable.__RiftDragonStakable_init();
    }

    function setChanceWizardStolen(uint8 _chanceWizardStolen) external onlyAdminOrOwner {
    ...
    function wizardAndDragonsReceivedFromL1(
        address _recipient,
        uint256[] calldata _wndTokenIds)
        external
        override
        nonZeroAddress(_recipient)
        nonZeroLength(_wndTokenIds)
        onlyAdminOrOwner
        returns(bytes32)
    {
    ...
    function withdrawWizards() external contractsAreSet whenNotPaused onlyEOA {
    ...
    function _decideFateOfWizard(uint256 _tokenId, uint256 _random) private {
        ...
        if(_didWizardGetStolen) {
            // Sad, send the wizard to a dragon owner
            ...
            _transferToken(_tokenId, _randomDragonOwner);

            emit WizardStolen(msg.sender, _randomDragonOwner, _tokenId);
        } else {
            // User keeps token.
            _transferToken(_tokenId, msg.sender);
            emit TokenWithdrawn(msg.sender, _tokenId);
        }
    }

    function _transferToken(uint256 _tokenId, address _recipient) private {
        wnd.transferFrom(address(this), _recipient, _tokenId);
    }
}
...
contract Rift721Receiver is IERC721Receiver {
...
abstract contract RiftContracts is Initializable, IRift, RiftTier {
    ...
    function setContracts(address _worldAddress, address _wndAddress, address _trainingProficiencyAddress, address _randomizerAddress, address _bridgeAddress) external onlyAdminOrOwner {
        require(_worldAddress != address(0)
            && _wndAddress != address(0)
            && _randomizerAddress != address(0)
            && _trainingProficiencyAddress != address(0)
            && _bridgeAddress != address(0), "Bad address.");
    ...
    modifier contractsAreSet() {
        require(address(world) != address(0)
            ...
            && address(bridge) != address(0), "Contracts not set");
...
interface IRift is IDragonStakable {
    function wizardAndDragonsReceivedFromL1(address _recipient, uint256[] calldata _wndTokenIds) external returns(bytes32);
...
abstract contract RiftTier is Initializable, IRift, RiftState {
    ...
    function getRiftTier(address _address) external view override returns(uint256) {
        uint256 _amt = bridge.getStakedGPAmt(_address);
...
contract RiftState is Initializable, AdminableUpgradeable {
...
contract AdminableUpgradeable is UtilitiesUpgradeable {
    ...
    modifier onlyAdminOrOwner() {
        console.log("aaaaaaaaaaaaaaaaaaaaaa", msg.sender);
        console.log("bbbbbbbbbbbbbbbbbbbbbb", owner());
...
interface IBridge {
    ...
    function getStakedGPAmt(address _address) external view returns(uint256);
}
...
interface IWorldReadOnly {
    ...
    // Returns the number of dragons that are staked at the given location.
    function numberOfDragonsStakedAtLocation(Location _location) external view returns(uint256);
    ...
    function dragonAtLocationAtIndex(Location _location, uint256 _index) external view returns(uint256);
