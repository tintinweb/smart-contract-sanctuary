...
// File contracts/MiniRewarderTimeFactory.sol

pragma solidity 0.6.12;


contract MiniRewarderTimeFactory {
  address public defaultMiniChef;
  address public defaultOwner;
  using BoringERC20 for IERC20;

  event DeployedRewarderContract(address indexed rewarder, address indexed rewardToken, uint256 indexed tokenPerBlock);

  constructor (address _defaultMiniChef, address _defaultOwner) public {
    defaultOwner = _defaultOwner;
    defaultMiniChef = _defaultMiniChef;
  }

  function deployDefaultRewarder(IERC20 _rewardToken, uint256 _tokenPerBlock) public {
    deployRewarder(defaultMiniChef, defaultOwner, _rewardToken, _tokenPerBlock);
  }

  function deployRewarder (address _miniChef, address _owner, IERC20 _rewardToken, uint256 _tokenPerBlock) public {
    MiniRewarderTime rewarder = new MiniRewarderTime();

    rewarder.initialize(_rewardToken, _tokenPerBlock, _miniChef);

    rewarder.transferOwnership(_owner, true, false);

    emit DeployedRewarderContract(address(rewarder), address(_rewardToken), _tokenPerBlock);
  }
