...
contract FuzzyswapStaker is IFuzzyswapStaker, Multicall {
    ...
    function stakes(uint256 tokenId, bytes32 incentiveId)
        ...
        view
        override
        returns (uint160 secondsPerLiquidityInsideInitialX128, uint128 liquidity, uint256 stakedTime)
    {
    ...
    function unstakeToken(IncentiveKey memory key, uint256 tokenId) external override {
        ...
        (uint160 secondsPerLiquidityInsideInitialX128, uint128 liquidity, uint256 stakedTime) = stakes(tokenId, incentiveId);
        ...
        if(block.timestamp < key.endTime){
            incentive.totalLiquidity = incentive.totalLiquidity - liquidity;
        }
        ...
        if(block.timestamp > key.endTime && stake.stakedTime < key.startTime){
    ...
    function _stakeToken(IncentiveKey memory key, uint256 tokenId) private {
        ...
        if (block.timestamp < key.startTime){
            (, uint128 stakeLiquidity,) = stakes(tokenId, incentiveId);
            incentives[incentiveId].totalLiquidity += stakeLiquidity;
        }
...
library RewardMath {
    ...
    function computeRewardAmount(
        ...
        secondsInsideX128 = totalSecondsClaimedX128;
        reward = FullMath.mulDiv(totalRewardUnclaimed, liquidity, totalLiquidity);
    }
}
