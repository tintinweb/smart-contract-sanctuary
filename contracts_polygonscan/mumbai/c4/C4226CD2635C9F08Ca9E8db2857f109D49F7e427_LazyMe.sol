...
contract LazyMe is ERC721Tradable, ReentrancyGuard, Factory {
    ...
    function getProxyOwner() public view returns (address){
        ProxyRegistry proxyRegistry = ProxyRegistry(creatureProxyRegistryAddress);
        return address(proxyRegistry.proxies(owner()));
    }
    ...
    function _transfer(address from, address to, uint256 tokenId) internal override {
        if (_isBox[tokenId]) {
            ProxyRegistry proxyRegistry = ProxyRegistry(creatureProxyRegistryAddress);
            assert( address(proxyRegistry.proxies(owner())) == _msgSender() || owner() == _msgSender() || ERC721.ownerOf(tokenId) == from);
