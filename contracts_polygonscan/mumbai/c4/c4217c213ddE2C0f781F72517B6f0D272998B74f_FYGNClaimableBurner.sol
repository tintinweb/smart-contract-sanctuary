...
contract FYGNClaimableBurner {
    ...
    function burnAndStake(uint256 _fYGNShare) external {
        uint256 beforeBalance = ygnToken.balanceOf(address(this));
        leaveInternal(address(this), _fYGNShare);
        
        uint256 afterBalance = ygnToken.balanceOf(address(this));
        uint256 diff = afterBalance.sub(beforeBalance);
        ygnStaker.approve(address(ygnStaker), diff);
        ygnStaker.enterFor(msg.sender, diff);
    }
