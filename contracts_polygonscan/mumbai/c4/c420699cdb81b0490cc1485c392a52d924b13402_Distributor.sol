...
contract Distributor is IDistributor, OwnableUpgradeable {
    ...
    IApeRouter02 private constant router = IApeRouter02(0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff);
    IApeFactory private constant factory = IApeFactory(0x5757371414417b8C6CAad45bAeF941aBc7d3Ab32);
    ...
    address public spaceETHPool;  
    address public stakingRewards; // SPACE Pool
    ...
    function notifyEngage(uint256 auctionId, address user, uint256 amount) override external onlyRocket {
        users.push(user);
        amount = ISpaceLauncher(launcher).swapTokenAmount(auctionId, amount);
    ...
    // 150% mint -> 50% LP user staking, 50% LP SpacePool rewards, 50% SPACE user staking
    ...
    function setspaceETHPool(address _masterChef) external onlyOwner {
        spaceETHPool = _masterChef;
    }
    ...
    function setSpacePool(address _spacePool) external onlyOwner {
    ...
    function distributeTokens(uint index, uint length) external onlyOwner {
        ...
        require(spaceETHPool != address(0), 'not set spaceETHPool');
        require(stakingRewards != address(0), 'not set stakingRewards');

        IERC20(lpToken).safeApprove(spaceETHPool, 0);
        IERC20(lpToken).safeApprove(spaceETHPool, totalFlipBalance);
    ...
    function sendToRewards() external onlyOwner {
        ...
        IERC20(flipToken()).safeTransfer(spacePool, lpBalance);
        IStakingRewards(spacePool).notifyRewardAmount(lpBalance);
    }
    ...
    function _distributeFlip(address user, uint share) private {
        ...
        IStrategy(spaceETHPool).stakeTo(amount, user);
    }
...
interface ISpaceLauncher {
    ...
    struct AuctionInfo {
        ...
        uint capacity;                  // total value of token in ETH or BNB (calculated by SpaceLauncher)
