...
import "../interfaces/IOVRLandFolder.sol";
    ...
    PausableUpgradeable
{
    ...
    IOVRLandFolder public OVRFolder;
    ...
    mapping(uint256 => bool) public folderOnSelling;
    ...
    function initialize(
        ...
        OVRFolder = IOVRLandFolder(_OVRFolder);
        __Ownable_init();
        __Pausable_init();
    ...
    event OfferPlaced(address indexed sender, uint256 indexed _nftId, uint256 _value, uint256 _timestamp);
    event OfferCanceled(uint256 indexed _nftId, address indexed _sender, uint256 _timestamp);
    event OfferAccepted(address indexed owner, uint256 indexed _nftId, uint256 _value, uint256 _timestamp);
    event Sold(address indexed seller, uint256 indexed _nftId, uint256 _value, uint256 _timestamp);
    event FolderSold(address indexed seller, uint256 indexed folderId, uint256 _value, uint256 _timestamp);
    event FolderBought(
        uint256 indexed _idFolder,
        uint256 _value,
        address indexed _sender,
        uint256 _timestamp
    );
    event SellCanceled(uint256 indexed _nftId, address indexed _sender, uint256 _timestamp);
    event SellFolderCanceled(
        uint256 indexed _idFolder,
        address indexed _sender,
        uint256 _timestamp
    );
    ...
    event PriceFolderChanged(
        uint256 indexed _folderId,
        uint256 _newPrice,
        uint256 _timestamp
    );
    ...
    modifier isFolderOwner(uint256 _idFolder) {
        require(
            OVRFolder.ownerOf(_idFolder) == _msgSender(),
            "Not the owner of folder"
        );
        _;
    }
    ...
    modifier onSellingFolder(uint256 _idFolder) {
        require(
            sellingFolder[_idFolder].from == OVRFolder.ownerOf(_idFolder),
            "Not for sale"
        ); //L'owner attuale non è chi l'ha messo in vendita
        _;
    }
    ...
    function folderIsOnSelling(uint256 _folderId)
        ...
        returns (bool _onSelling)
    {
        return folderOnSelling[_folderId];
    }
        ...
        whenNotPaused
    {
        ...
        emit OfferAccepted(_msgSender(), _nftId, bestOffers[_nftId].value, currentTimestamp);
    }
    ...
    /**
     * @notice this function can be called by the owner of a folder to sell a it
     *
     **/
    function sellFolder(uint256 _folderId, uint256 _value)
        public
        isFolderOwner(_folderId)
        whenNotPaused
    {
        uint256 fees = _value.mul(feePerc).div(1e4);
        sellingFolder[_folderId].fee = fees;
        sellingFolder[_folderId].landId = OVRFolder.childsOfParent(_folderId);
        sellingFolder[_folderId].from = _msgSender();
        sellingFolder[_folderId].value = _value;
        sellingFolder[_folderId].timestamp = _now();
        folderOnSelling[_folderId] = true;
        emit FolderSold(_msgSender(), _folderId, _value, _now());
    }
     ...
     **/
    function buyFolder(uint256 _idFolder)
        public
        whenNotPaused
        onSellingFolder(_idFolder)
    {
        uint256 minBalance = sellingFolder[_idFolder].value.add(
            sellingFolder[_idFolder].fee
        );
        require(
        ...
        require(
            token.transferFrom(
                _msgSender(),
                sellingFolder[_idFolder].from,
                sellingFolder[_idFolder].value
            ),
            "Insufficient allowance"
        );
        require(
        ...
        OVRFolder.transferFrom(
            sellingFolder[_idFolder].from,
            _msgSender(),
            _idFolder
        );

        folderOnSelling[_idFolder] = false;
        uint256 currentTimestamp = _now();
    ...
    /**
     * @notice delete folder on selling, can be called only by the person that own the folder
     **/
    function cancelSellFolder(uint256 _idFolder)
        public
        isFolderOwner(_idFolder)
    {
        ...
        folderOnSelling[_idFolder] = false;
        delete sellingFolder[_idFolder];
        emit SellFolderCanceled(_idFolder, _msgSender(), currentTimestamp);
    }

    /**
     * @notice change price folder on selling, can be called only by the person that own the folder
     **/
    function updatePriceFolder(uint256 _idFolder, uint256 _price)
        public
        isFolderOwner(_idFolder)
    {
        require(folderOnSelling[_idFolder] == true, "folder is not on selling");
        uint256 fees = _price.mul(feePerc).div(1e4);
        uint256 currentTimestamp = _now();
        sellingFolder[_idFolder].value = _price;
        sellingFolder[_idFolder].fee = fees;
        emit PriceFolderChanged(_idFolder, _price, currentTimestamp);
    }
    ...
    function updatePriceLand(uint256 _nftId, uint256 _price)
        ...
        isLandOwner(_nftId)
    {
        require(landOnSelling[_nftId] == true, "folder is not on selling");
        uint256 fees = _price.mul(feePerc).div(1e4);
        uint256 currentTimestamp = _now();
    ...
    function buy(uint256 _nftId) public whenNotPaused onSelling(_nftId) {
        ...
        delete selling[_nftId];
        landOnSelling[_nftId] = false;
        emit Bought(_nftId, selling[_nftId].value, _msgSender(), currentTimestamp);
    }
...
interface IOVRLandFolder is IERC721 {
