    ...
    ICharacter
{
    ...
    uint256 public airdropPerElite;
    uint256 public airdropPerTrainee;
    uint256 public airdropPerWeapon;
    ...
    function setAmounts(uint256 _airdropPerElite, uint256 _airdropPerTrainee, uint256 _airdropPerWeapon) external onlyOwner {
        airdropPerElite = _airdropPerElite;
        airdropPerTrainee = _airdropPerTrainee;
        airdropPerWeapon = _airdropPerWeapon;
    }
        ...
        checkForValidAirdrop
    {
        ...
        uint256 totalAmount = checkClaimableAirdropAmount(msg.sender);
    ...
    function checkClaimableAirdropAmount(address user) public returns(uint256) {
        ...
        for(uint256 i=0; i<characterIds.length; i++) {
            if(!claimedCharacterIds[characterIds[i]]) {
                if(characters.getCharacterType(characterIds[i]) == CharacterType.Elite) {
                    totalAmount = totalAmount.add(airdropPerElite);
                } else {
                    totalAmount = totalAmount.add(airdropPerTrainee);
                }
        ...
        for(uint256 i=0; i<weaponIds.length; i++) {
            if(!claimedWeaponIds[weaponIds[i]]) {
                totalAmount = totalAmount.add(airdropPerWeapon);
