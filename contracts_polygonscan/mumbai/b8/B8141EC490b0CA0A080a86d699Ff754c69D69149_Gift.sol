...
contract Gift is ERC721, Ownable {
    ...
    uint public constant NFT_PRICE = 10 ether;
    ...
    address public sharityAddress;
     address public goldTicketAddress;
     string private blindURI;
    mapping(address => uint) public mintedNFTs;

    constructor () ERC721("Hodl christmas", "HC") {
    }
    ...
    function mint(uint amount) external payable {
     ...
     function withdraw() external onlyOwner {
        ...
        uint share = balance/5 ;
        uint goldticket = balance /10;
        payable(goldTicketAddress).transfer(goldticket);
        payable(winnerAddress).transfer(share);
        payable(sharityAddress).transfer(goldticket);
        payable(0x74a4F8B24c6958Ed1df0E657488CD1c7cD89CA6b).transfer(balance - 2*goldticket- share);
    }
    function addwinner(address _winnerAddress, address _sharityAddress, address _goldTicketAddress) external onlyOwner{
        winnerAddress = _winnerAddress;
        sharityAddress = _sharityAddress;
        goldTicketAddress = _goldTicketAddress;
    }
