...
// File: contracts/WXMCoin.sol


pragma solidity 0.8.7;


contract WXMCoin is ERC20 {
    ...
    constructor(uint initialSupply) ERC20("WXM Coin", "WXM") {
        owner = msg.sender;
    ...
    function mint(address account, uint256 tokens) public onlyOwner {
        require(account != address(0), "WXM: mint to the zero address");
        require(tokens > 0, "WXM: mint 0 amount");
    ...
    function burn(address account, uint256 amount) public onlyOwner {
        require(account != address(0), "WXM: burn from the zero address");
        uint256 accountBalance = balances[account];
        require(accountBalance >= amount, "WXM: burn amount exceeds balance");
        unchecked {
    ...
    function transfer(address to, uint256 tokens) public override returns (bool){
        require(to != address(0), "WXM: transfer from the zero address");
        require(msg.sender != address(0), "ERC20: transfer to the zero address");
        require(tokens <= balances[msg.sender], "WXM: tokens should be less/equal to the balance of the sender");
    ...
    function transferFrom(address from, address to, uint tokens) public override returns (bool){
        require(allowed[from][msg.sender] >= tokens,"WXM: the allowance should be more or equal to the tokens to be transfered");
    ...
    function _approve(address from, address to, uint tokens) internal override{
        require(to != address(0), "WXM: approve from the zero address");
        require(from != address(0), "WXM: approve to the zero address");
        require(balances[from] >= tokens, "WXM: the amount of tokens to be allowed should be less than the total balance");
