...
// File contracts/solc-0.6/GameeTransfer.sol
...
contract GameeTransfer is ITokenRecipient {
    using SafeMath for uint256; 
    ...
    function transfer(uint256 amount, uint32 userId) public {
        require(amount > 0, "Cannot transfer 0");
        transferToken.safeTransferFrom(msg.sender, tokenRecipient, amount);

        emit Transferred(msg.sender, amount, userId);
    }

    function withdraw(uint256 amount) external onlyOwner {
        require(amount != 0, "Redeemer: invalid amount");
        transferToken.safeTransfer(msg.sender, amount);
