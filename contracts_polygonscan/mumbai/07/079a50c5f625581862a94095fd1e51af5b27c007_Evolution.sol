...
contract Evolution is Initializable, ERC721HolderUpgradeable, AccessControlUpgradeable, PausableUpgradeable  {
    ...
    function initialize(address _KRL, address _EOC, address _RACE, address _TRACE) public initializer {
        EOC = IERC20(_EOC);
        KRL = IERC20(_KRL);
        RACE = IERC721(_RACE);
    ...
    function setLevelPriceforEOC(uint8 _levelEOC, uint _priceEOC) public validate {
        levels[_levelEOC] = _priceEOC;
    }

    function setLevelPriceforKRL(uint8 _levelKRL, uint _priceKRL) public validate {
        levels[_levelKRL] = _priceKRL;
    }

    function getLevelPriceforEOC(uint8 _levelEOC) public view returns (uint256) {
        return levels[_levelEOC];
    }
    function getLevelPriceforKRL(uint8 _levelKRL) public view returns (uint256) {
        return levels[_levelKRL];
    }

    function setNewLevelDiffForEOC(uint _newLevelDiffEOC) public validate {
        newLevelDiff = _newLevelDiffEOC;
    }

    function setNewLevelDiffForKRL(uint _newLevelDiffKRL) public validate {
        newLevelDiff = _newLevelDiffKRL;
    }
    ...
    function depositUsingEOC(uint256 _tokenId) public {
    ...
    function depositUsingKRL(uint256 _tokenId) public {
        require(_tokenId < newLevelDiff * 3, "Evolution: Max upgrade limit reached for this Racer");
        require(RACE.ownerOf(_tokenId) == msg.sender, "Evolution: Not authorized to deposit this Racer");
        uint256 _levelPrice = _tokenId / newLevelDiff + 1;
        require(KRL.balanceOf(msg.sender) >= _levelPrice, "Evolution: Insufficient funds");
        
        // transfer 5% to _safe & burn rest
        uint256 _fivePercent = _levelPrice * 5 / 100;
        KRL.transferFrom(msg.sender, address(this), _levelPrice);
        KRL.transfer(_safe, _fivePercent);
        KRL.burn(address(this), _levelPrice - _fivePercent);
        // deposit NFT once in the contract by the user.
        RACE.safeTransferFrom(msg.sender, address(this), _tokenId);
        // mint dummy NFT (reciept) in the depositers wallet. ERC721.
        TRACE.safeMint(msg.sender, _tokenId);
