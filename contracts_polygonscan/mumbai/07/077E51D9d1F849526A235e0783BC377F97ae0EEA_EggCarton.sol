...
contract EggCarton is FxBaseChildTunnel, VRFConsumerBase, Ownable {
    ...
    function sendEggToCaller(uint baby) external{
        ...
        _findEggBonus(baby, parents[0], parents[1]);
    ...
    function _findEggBonus(uint baby, uint _mom, uint _dad) internal{
        uint momSum = 0;
        uint dadSum = 0;
        uint32[8] memory mom = Egg.getCrowMetaData(_mom);
        uint32[8] memory dad = Egg.getCrowMetaData(_dad); 
        ...
        bonus = (baseMultiplier * (Egg.eventTraitSum() - avgRarity)) / (Egg.eventTraitSum());
    ...
    function findTraits(uint tokenID) external {
        ...
        _calculateMadHouseTraits(tokenID, Egg.crowRNG(tokenID));
    ...
    function sendHatchedEggToMainnet(uint eggID) external {
        ...
        _calculateEggBreedingTraits(eggID, Egg.crowRNG(eggID));
    ...
    function _calculateMadHouseTraits(uint tokenID, uint randomNumber) internal{
        ...
        uint32[8] memory newCrow = Egg.getCrowMetaData(tokenID);
        for (uint8 i=0; i<8; i++){
    ...
    function _calculateEggBreedingTraits(uint tokenID, uint randomNumber) internal{
        ...
        for (uint8 i=0; i<8; i++){
            if(newCrow[i] == 0){//means this trait is not set
                amountToDiminishBy = (maturity - minMaturity) / (Egg.eventTraitStartingIndex(0, i) - Egg.eventTraitStartingIndex(1, i));
                sum = 0;
                uint rng = expandedRandomness[i] % Egg.eventTraitSum();
                for(uint32 j=Egg.eventTraitStartingIndex(0, i); j<Egg.eventTraitStartingIndex(1, i); j++){
        ...
        if(Egg.eggInfo(tokenID).EventID != 0){
            maturity = Egg.eggInfo(tokenID).Maturity;
            if(maturity > maxMaturity){
                maturity = maxMaturity;
            }
            uint eventID = Egg.eggInfo(tokenID).EventID;
            for (uint8 i=8; i<16; i++){
                amountToDiminishBy = (maturity - minMaturity) / (Egg.eventTraitStartingIndex(eventID, i) - Egg.eventTraitStartingIndex(eventID + 1, i));
                sum = 0;
                uint rng = expandedRandomness[i] % Egg.eventTraitSum();
