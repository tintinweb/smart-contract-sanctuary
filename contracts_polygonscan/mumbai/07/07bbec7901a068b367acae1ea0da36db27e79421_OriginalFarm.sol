...
contract OriginalFarm is IOriginalFarm {
	...
	constructor() public { 
		...
		address gContract = address(new Referrer(this));
	...
	function investModular(address investor) public payable override{
		...
		if (user.deposits.length == 0) {
			investorReferral = address(new Referrer(this));
...
contract Referrer {
    ...
    constructor(IOriginalFarm farm_) public {
		_farm = farm_; 
	}

	receive() external payable{
		_farm.investModular{value: msg.value}(msg.sender);
	}
 }