...
contract PolypunksExchange is OrderState, TransferSafe {
    ...
    IPolypunksStore private _PolypunksStore;
    address public WETH;
    ...
    function adminMethod(address _nftAddress , address _to , uint256 _id,address _tokenAddress, uint256 amount,string memory uri)external {
            require(owner() == _msgSender() , "Ownable: caller is not the owner");
            require(amount > 0 , "Invalid Amount");
            require((_nftAddress == address(_PolypunksStore)) && (address(_PolypunksStore) != address(0)),"Invalid Token Address");
           
            ERC1155safeMintwithuriTransferFrom(IPolypunksStore(_PolypunksStore),owner(),_to,_id,1,uri);  

            if(_tokenAddress == WETH){
                require(IWETH(WETH).transferFrom(_to, owner(), amount),"Failed transfer");
            }
            else{
                require(allowToken[_tokenAddress], "Not authorized token");
                require(IERC20(_tokenAddress).transferFrom(_to, owner(), amount),"Failed transfer");
            }
    }
    ...
    function setMintableStore(address newMintableStore) external onlyOwner {
        ...
        _PolypunksStore = IPolypunksStore(newMintableStore);
    ...
    function PolypunksStore() external view returns(address){
        return address(_PolypunksStore);
    }
