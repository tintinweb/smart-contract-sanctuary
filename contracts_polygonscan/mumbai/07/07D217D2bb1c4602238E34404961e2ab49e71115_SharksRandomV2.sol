...
contract SharksRandomV2 is VRFConsumerBase, Ownable {
	...
	//shuffling process
	uint256 public nftMaxSupply = 10000;
	mapping(uint256 => uint256) public shuffledNFTsTracker;
	uint256[] public shuffledNFTs; //limit?
	...
	function fillRandomArray(uint256 limit, uint256 drawId) external onlyOwner {
		require(shuffledNFTs.length < nftMaxSupply, "shuffling complete");
		for (uint256 i = 0; i < limit; i++) {
			uint256 winner = (uint256(keccak256(abi.encode(randomResults[drawId], i))) % nftMaxSupply) +
				1;
			if (shuffledNFTsTracker[winner] == 0 && shuffledNFTs.length < nftMaxSupply) {
	...
	//reads the shuffled NFTs in batch. needed?
	function readShuffledNFTs(uint256 cursor, uint256 length) public view returns (uint256[] memory) {
		uint256[] memory array = new uint256[](length);
		for (uint256 i = cursor; i < cursor + length; i++) {
			array[i] = shuffledNFTs[i];
		}
		return array;
	}
	...
	/**
	 * Requests 20 random numbers. Don't care about requestID
	 */
	function get20RandomNumber() external onlyOwner {
