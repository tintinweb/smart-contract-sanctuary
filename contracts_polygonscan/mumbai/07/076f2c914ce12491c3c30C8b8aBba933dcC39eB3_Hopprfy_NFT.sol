...
contract Hopprfy_NFT is ERC721, Governance {
    uint256 _last_ID = 0;

    constructor() ERC721("Hopprfy Finance", "HOPPNFT") {
        _setBaseURI("https://api.hopprfy.io/asset/");
    }
     ...
     */
    function mint(address to) external onlyGovernance returns (bool) {
        _last_ID++;
        _mint(to, _last_ID);
        return true;
    }
     ...
     * @return A boolean that indicates if the operation was successful.
     */
    function safeMint(address to) public onlyGovernance returns (bool) {
        _last_ID++;
        _safeMint(to, _last_ID);
        return true;
    }
