    ...
    ContextMixin
{
    ...
    /**
     * @notice called when tokens need to be transferred to multiple recipients with different amounts
     * @dev Two arg arrays, reccipients and amounts, should have same lengths
     * recipients should not include a zero address
     * amounts should not include a non-positive amount
     * sender should have balance more than sum of amounts
     * @param recipients user addresses who receive tokens
     * @param amounts amounts to be transferred to recipients (1-1 mapping)
     */
    function transferBatch(address[] calldata recipients, uint[] calldata amounts) external  {

        require(recipients.length == amounts.length, "LeedoERC20Matic: two arguments have different lengths");

        address sender = _msgSender();

        for (uint256 i; i < recipients.length; i++) {
            require(amounts[i] > uint(0), "LeedoERC20Matic: there is a non-positive value in amounts");
            bool success = transfer(recipients[i], amounts[i]);
            require(success, "LeedoERC20Matic: transfer failed");
