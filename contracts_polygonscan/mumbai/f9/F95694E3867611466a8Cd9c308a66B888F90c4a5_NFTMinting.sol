...
contract NFTAuction is ReentrancyGuard, Ownable {
  ...
  Counters.Counter private _auctionsSold;
 ...
 address payable treasury;
  ...
  function createAuction(
   address nftContract,
    uint256 tokenId,
    uint256 price, uint256 duration, uint256 startTime
  ...
  function endAuction(uint256 auctionId) public isValidId(auctionId) {
      ...
      require (idToAuction[auctionId].startTime <= block.timestamp,"Auction has not begun");
       IERC721(idToAuction[auctionId].nftContract).transferFrom(address(this),idToAuction[auctionId].bidder, idToAuction[auctionId].tokenId);
  ...
  function fetchUnsoldAuctions() public view returns (Auction[] memory) {
    ...
    uint unsoldauctionCount = _auctionIds.current() - _auctionsSold.current();
    uint currentIndex = 0;
  ...
  function transferFunds(uint256 auctionId) public payable isValidId(auctionId) {
        require(msg.sender == address(this));
        uint256 royalty = idToAuction[auctionId].royalty;
    uint256 amountTominter = (idToAuction[auctionId].amount)*(royalty)/(100);
    uint256 amountToSeller = (idToAuction[auctionId].amount)-(amountTominter);
     payable(idToAuction[auctionId].minter).transfer(amountTominter);
    payable(idToAuction[auctionId].seller).transfer(amountToSeller);
    }
  
   function transferFundsToLastBidder(uint256 auctionId) public payable isValidId(auctionId) {
         require(msg.sender == address(this));
       payable( idToAuction[auctionId].bidder).transfer(idToAuction[auctionId].amount);
    }

  /* Returns onlyl auctions that a user has created */
  ...
  function fetchauctionsBid(address user) public view returns (Auction[] memory) {
    uint totalauctionCount = _auctionIds.current();
    uint auctionCount = 0;
    uint currentIndex = 0;
