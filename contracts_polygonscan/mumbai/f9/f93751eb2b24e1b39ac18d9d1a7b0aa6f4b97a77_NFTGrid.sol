...
contract NFTGrid is ERC721, Ownable {
    using Strings for uint256;
    ...
    constructor() ERC721("NFTGrid", "GRID") {}
    ...
    function mintSingleNFT(uint256 tokenId) public payable {
        require(msg.value >= mintPrice, "Insufficient payment");
        require(mintedCount() + 1 <= maxSupply, "All NFTs have been minted");
        require(balanceOf(msg.sender) + 1 <= ownershipMintLimit, "Reached current ownership limit");
        _mint(msg.sender, tokenId);
        _mintedTokenIds.push(tokenId);       
    }
    function mintMultiNFT(uint256 tokenId, uint8 width, uint8 height) public payable {
        uint256 quantity = (width * height);
        require(msg.value >= (mintPrice * quantity), "Insufficient payment");
        require(mintedCount() + quantity <= maxSupply, "All NFTs have been minted");
        require(balanceOf(msg.sender) + quantity <= ownershipMintLimit, "Reached current ownership limit");
        for (uint8 y = 0; y < height; y++) {
            for (uint8 x = 0; x < width; x++) {
                uint256 innerTokenId = tokenId + (100 * y) + x;
                _mint(msg.sender, innerTokenId);
                _mintedTokenIds.push(innerTokenId);
            }
        }
    } 
