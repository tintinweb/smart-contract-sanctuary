...
contract EGGVERSEAuction is Initializable, ERC721EnumerableUpgradeable, OwnableUpgradeable {
    ...
    event AuctionFinalized(uint256 _tokenId, uint256 _certificateTokenId);
    ...
    function mint(
        ...
        emit AuctionCreated(newId, _auctionType, _tokenURI, msg.sender);
        return newId;
    }
    ...
    function setAuctionCategory(uint256 _auctionType, uint256 _categoryId)
        external
        onlyOwner
    {
        auctionCategory[_auctionType] = _categoryId;
    }

    function getAuctionCategory(uint256 _auctionType) external view returns (uint256) {
        return auctionCategory[_auctionType];
    }
    /**
     * @dev Destroys `tokenId`.
     * The approval is cleared when the token is burned.
     ...
     * @dev make event for subgraph
     */
    function finalizeOrder(uint256 _tokenId, uint256 _certificateTokenId) external onlyBid {
        auctionStatus[_tokenId] = Status.SUCCESS;
        emit AuctionFinalized(_tokenId, _certificateTokenId);
    }
