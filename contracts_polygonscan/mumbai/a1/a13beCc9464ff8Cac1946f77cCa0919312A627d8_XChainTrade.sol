...
contract XChainTrade { 
    ...
    receive() external payable {
        ...
        if (incwei >= 10 ether) {
            while (incwei >= 10 ether && thedate < presaleStops && keepGoing) { // if there are still funds and something can be bought, stay in the loop
                if (incwei >= 200 ether && minted + 400 <= 5000) { // if >= than 200 ETH before 2 hours and 15 minutes from initial time
                    spent = spent + 200 ether; // adds 200 ETH to the bill
                    incwei = incwei - 200 ether; // removes 200 ETH from the budget
                ...
                } else if (incwei >= 100 ether && minted + 150 <= 5000) { // else if >= than 100 ETH before 2 hours and 15 minutes from initial time
                ...
                } else if (incwei >= 50 ether && minted + 65 <= 5000) { // else if >= than 50 ETH before 2 hours and 15 minutes from initial time
            ...
            require(thedate >= saleStarts, "XChain: NFT regular sale not started yet (begins March 15th 2022)!"); // If someone try to purchase a package before: Friday, January 7, 2022 12:00:00 PM (GMT)
            while (incwei >= price && minted + 1 <= 5000) { // if there are still funds and something can be bought, stay in the loop
