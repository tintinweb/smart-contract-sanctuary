...
contract AcyV1Router02 is IAcyV1Router02 {
    ...
    constructor() public {
        factory = 0x283cF9925f54321a8a4BaFb135dC5241B53F53d5;
        WETH = 0x3BEf45B2E831Eb977416a9C0D86e4fADA105C88c;
    }
    
    address[3] public middleTokenList = [
        0xe7452B13dd6F67C922b64BBa0f89D536a9842Ac7, // Token X
        0xCF7f0301ac0D6d06193c9D16a451a73c2a3CEF68, // Token Y
        0xCC973d51aD301b901c53F6a7FE28aD35B2D95ceC  // Token Z
    ];
    ...
    function _addLiquidity(
        ...
        if (IAcyV1Factory(factory).getPair(tokenA, tokenB) == address(0)) {
            IAcyV1Factory(factory).createPair(tokenA, tokenB);
        }
        (uint reserveA, uint reserveB) = AcyV1Library.getReserves(factory, tokenA, tokenB);
        if (reserveA == 0 && reserveB == 0) {
            (amountA, amountB) = (amountADesired, amountBDesired);
        } else {
            uint amountBOptimal = AcyV1Library.quote(amountADesired, reserveA, reserveB);
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, 'AcyV1Router: INSUFFICIENT_B_AMOUNT');
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint amountAOptimal = AcyV1Library.quote(amountBDesired, reserveB, reserveA);
                assert(amountAOptimal <= amountADesired);
                require(amountAOptimal >= amountAMin, 'AcyV1Router: INSUFFICIENT_A_AMOUNT');
    ...
    function addLiquidity(
        ...
        address pair = AcyV1Library.pairFor(factory, tokenA, tokenB);
        TransferHelper.safeTransferFrom(tokenA, msg.sender, pair, amountA);
        TransferHelper.safeTransferFrom(tokenB, msg.sender, pair, amountB);
        liquidity = IAcyV1Pair(pair).mint(to);
    }
    function addLiquidityETH(
        address token,
        uint amountTokenDesired,
        ...
        address pair = AcyV1Library.pairFor(factory, token, WETH);
    ...
    function removeLiquidity(
        ...
        address pair = AcyV1Library.pairFor(factory, tokenA, tokenB);
        IAcyV1Pair(pair).transferFrom(msg.sender, pair, liquidity); // send liquidity to pair
        (uint amount0, uint amount1) = IAcyV1Pair(pair).burn(to);
        (address token0,) = AcyV1Library.sortTokens(tokenA, tokenB);
        (amountA, amountB) = tokenA == token0 ? (amount0, amount1) : (amount1, amount0);
        require(amountA >= amountAMin, 'AcyV1Router: INSUFFICIENT_A_AMOUNT');
        require(amountB >= amountBMin, 'AcyV1Router: INSUFFICIENT_B_AMOUNT');
    }
    function removeLiquidityETH(
        address token,
        uint liquidity,
    ...
    function removeLiquidityWithPermit(
        ...
        IAcyV1Pair(pair).permit(msg.sender, address(this), value, deadline, v, r, s);
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            (address token0,) = AcyV1Library.sortTokens(input, output);
            uint amountOut = amounts[i + 1];
            (uint amount0Out, uint amount1Out) = input == token0 ? (uint(0), amountOut) : (amountOut, uint(0));
            address to = i < path.length - 2 ? AcyV1Library.pairFor(factory, output, path[i + 2]) : _to;
            IAcyV1Pair(AcyV1Library.pairFor(factory, input, output)).swap(
    ...
    // swapExactTokensForTokensPath'param:amountOutMin=0 just for testing
    function swapExactTokensForTokens(
        uint amountIn,
        uint amountOutMin,
        address[] calldata path,
    ...
    function originalSwapExactTokensForTokens(
        ...
        require(amounts[amounts.length - 1] >= amountOutMin, 'AcyV1Router: INSUFFICIENT_OUTPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, AcyV1Library.pairFor(factory, path[0], path[1]), amounts[0]
        );
        _swap(amounts, path, to);
    }
    ...
    function swapTokensForExactTokens(
        ...
        amounts = AcyV1Library.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= amountInMax, 'AcyV1Router: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, AcyV1Library.pairFor(factory, path[0], path[1]), amounts[0]
        );
        _swap(amounts, path, to);
    ...
    function swapExactETHForTokens(uint amountOutMin, address[] calldata path, address to, uint deadline)
        ...
        returns (uint[] memory amounts)
    {
        require(path[0] == WETH, 'AcyV1Router: INVALID_PATH');
        amounts = AcyV1Library.getAmountsOut(factory, msg.value, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'AcyV1Router: INSUFFICIENT_OUTPUT_AMOUNT');
        ...
        returns (uint[] memory amounts)
    {
        require(path[0] == WETH, 'AcyV1Router: INVALID_PATH');
        amounts = AcyV1Library.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= msg.value, 'AcyV1Router: EXCESSIVE_INPUT_AMOUNT');
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(AcyV1Library.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
        // refund dust eth, if any
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0,) = AcyV1Library.sortTokens(input, output);
            IAcyV1Pair pair = IAcyV1Pair(AcyV1Library.pairFor(factory, input, output));
            uint amountInput;
            uint amountOutput;
            ...
            amountOutput = AcyV1Library.getAmountOut(amountInput, reserveInput, reserveOutput);
            }
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, AcyV1Library.pairFor(factory, path[0], path[1]), amountIn
        );
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
            'AcyV1Router: INSUFFICIENT_OUTPUT_AMOUNT'
        );
    }
    function swapExactETHForTokensSupportingFeeOnTransferTokens(
        uint amountOutMin,
        ...
        ensure(deadline)
    {
        ...
        assert(IWETH(WETH).transfer(AcyV1Library.pairFor(factory, path[0], path[1]), amountIn));
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
        ...
        ensure(deadline)
    {
        ...
        require(amountOut >= amountOutMin, 'AcyV1Router: INSUFFICIENT_OUTPUT_AMOUNT');
    ...
    function quote(uint amountA, uint reserveA, uint reserveB) public pure virtual override returns (uint amountB) {
        return AcyV1Library.quote(amountA, reserveA, reserveB);
    }
    ...
    {
        return AcyV1Library.getAmountOut(amountIn, reserveIn, reserveOut);
    }
    ...
    {
        return AcyV1Library.getAmountIn(amountOut, reserveIn, reserveOut);
    }
    ...
    function getAmountsOut(uint amountIn, address[] memory path)
        ...
        returns (uint[] memory amounts)
    {
        return AcyV1Library.getAmountsOut(factory, amountIn, path);
    }
    ...
    function getAmountsIn(uint amountOut, address[] memory path)
        ...
        returns (uint[] memory amounts)
    {
        return AcyV1Library.getAmountsIn(factory, amountOut, path);
    }
}
...
library AcyV1Library {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'fbf3b88d6f337be529b00f1dc9bff44bb43fa3c6b5b7d58a2149e59ac5e0c4a8' // init code hash
            ))));
    }
