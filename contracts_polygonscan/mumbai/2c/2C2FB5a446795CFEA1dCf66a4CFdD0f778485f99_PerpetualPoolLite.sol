...
contract PerpetualPoolLite is IPerpetualPoolLite, Migratable {
    ...
    function _removeLiquidity(address account, uint256 lShares) internal _lock_ {
        ...
        require(
            totalAbsCost == 0 ||
                ((totalDynamicEquity - bAmount.utoi()) * ONE) / totalAbsCost >= _minPoolMarginRatio,
    ...
    function _removeMargin(address account, uint256 bAmount) internal _lock_ {
        ...
        require(
            _getTraderMarginRatio(symbolIds, positions, margin) >= _minInitialMarginRatio,
    ...
    function _trade(
        ...
        require(
            totalAbsCost == 0 || (totalDynamicEquity * ONE) / totalAbsCost >= _minPoolMarginRatio,
    ...
    function _liquidate(address liquidator, address account) internal _lock_ {
        ...
        require(
            _getTraderMarginRatio(symbolIds, positions, margin) < _minMaintenanceMarginRatio,
        ...
        if (netEquity <= _minLiquidationReward) {
            ...
            reward =
                ((netEquity - _minLiquidationReward) * _liquidationCutRatio) /
                ONE +
                _minLiquidationReward;
        }
