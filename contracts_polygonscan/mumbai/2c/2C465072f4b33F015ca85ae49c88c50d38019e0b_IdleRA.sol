...
contract IdleRA is IProtocol, Ownable {
        ...
        returns (uint256)
    {
        ...
        IERC20(address(idle)).transfer(user, minted);
        return minted;
    }
    ...
    function redeemProtocolToken(address user, uint256 amount)
        ...
        returns (uint256)
    {
        IERC20(address(idle)).approve(address(idle), amount);
        uint256 redeemed = idle.redeemIdleToken(amount);
        usdc.transfer(user, redeemed);
        return redeemed;
    }
}
