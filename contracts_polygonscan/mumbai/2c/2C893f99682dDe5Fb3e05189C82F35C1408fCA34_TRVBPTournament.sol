...
contract TRVBPTournament is Ownable {
    ...
    function setChampionToken(address token) external onlyOwner {
        TRVBP_ADDRESS = token;
    }
    ...
    function cancelTournament(uint256 _tournamentId) external {
        ...
        for (uint256 i = 0; i < warriorsCount; i++) {
            ...
            IWETH(WETH_ADDRESS).transferFrom(address(this), ownerOfWarrior, amount);
    ...
    function setWinner(uint256 _tournamentId, uint16 _firstBest, uint16 _secondBest) external {
        ...
        IWETH(WETH_ADDRESS).transferFrom(address(this), ownerOfFistBest, bestPrize); // 70%
        IWETH(WETH_ADDRESS).transferFrom(address(this), ownerOfSecondBest, secondPrize); // 27.5%
    ...
    function withdrawFees() external onlyOwner { // withdraw WETH
        IWETH(WETH_ADDRESS).transferFrom(address(this), msg.sender, platformShare);
    }
