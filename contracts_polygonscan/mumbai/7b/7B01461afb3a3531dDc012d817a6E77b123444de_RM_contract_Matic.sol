...
// File: contracts/TRM_Contract.sol
...
contract RM_contract_Matic is ERC721, ERC721Enumerable, Ownable {
  using Strings for uint256;
 ...
 function setBaseURI(string memory _newBaseURI) public onlyOwner {
  ...
  // presale condition
  function mintPresale (uint256 _numberOfMints) public payable {
    require (!paused, "the contract is paused");
    uint256 supply = totalSupply();
    uint256 reserved = preSaleWhiteList[msg.sender];
    require (preSaleActive, "no presale active");
    require (_numberOfMints > 0, "Invalid purchase amount");
    require (_numberOfMints < maxMintAmount, "max mint per session");
    require (supply + _numberOfMints <= maxSupply, "Max NFT Limit");
    
    
    if (msg.sender != owner()) {
      require (reserved > 0, "not on the whitelist");
      uint256 ownerMintedCount = addressMintedBalance[msg.sender];
      require (ownerMintedCount + _numberOfMints <= nftPerAddressLimit, "NFT per Adress limit");
      require (_numberOfMints * preSalePrice <= msg.value, "not enough ether");
    }
  ...
  // Sale condition
  function mintSale (uint256 _numberOfMints) public payable {
    require (!paused, "the contract is paused");
    uint256 supply = totalSupply();
    require (saleActive, "no sale active");
    require (_numberOfMints > 0, "Invalid purchase amount");
    require (_numberOfMints < maxMintAmount, "max mint per session");
    require (supply + _numberOfMints <= maxSupply, "Max NFT Limit");
    ...
    if (msg.sender != owner()) {
      uint256 ownerMintedCount = addressMintedBalance[msg.sender];
      require (ownerMintedCount + _numberOfMints <= nftPerAddressLimit, "NFT per Adress limit");
      require (_numberOfMints * salePrice <= msg.value, "not enough ether");
    }
  ...
  // set Price for each stage
  function setPreSalePrice (uint256 _newPreSelaPrice) external onlyOwner {
    preSalePrice = _newPreSelaPrice;
  }
