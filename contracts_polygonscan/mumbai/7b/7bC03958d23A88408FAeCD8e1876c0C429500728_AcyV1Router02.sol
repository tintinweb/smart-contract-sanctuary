...
contract AcyV1Router02 is IAcyV1Router02 {
    ...
    function getFenZi(uint256 amountT,address XToken,address YToken,address ZToken) view public returns (uint256 fenzi) {
        (uint Xy, uint Yx) = AcyV1Library.getReserves(factory, XToken, YToken);
        (uint Xz, uint Zx) = AcyV1Library.getReserves(factory, XToken, ZToken);
        (uint Yz, uint Zy) = AcyV1Library.getReserves(factory, YToken, ZToken);
        return Zx.mul(Yz) + Yx.mul(Zy) + Yx.mul(Zx);
    }
    
    function getFenMu(uint256 amountT,address XToken,address YToken,address ZToken) view public returns (uint256 fenmu) {
        (uint Xy, uint Yx) = AcyV1Library.getReserves(factory, XToken, YToken);
        (uint Xz, uint Zx) = AcyV1Library.getReserves(factory, XToken, ZToken);
        (uint Yz, uint Zy) = AcyV1Library.getReserves(factory, YToken, ZToken);
        return Yx.mul(Zy).mul(Xz) + Yx.mul(Zy).mul(amountT) + (Yx.mul(amountT) - Xy.mul(Yz)).mul(Zx);
    }
