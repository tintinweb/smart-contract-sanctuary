...
contract CDHAccessControl is AccessControl {
    ...
    bytes32 public constant MINTER_ROLE_ADMIN = keccak256(abi.encodePacked("MINTER_ROLE_ADMIN"));
...
contract CDHInventory is Pausable, ERC1155, CDHAccessControl,
EIP712MetaTransaction("Crazy Defence Heroes Inventory", "1.0") {

    event PoolSet(bytes1 id, address pool);
    ...
    mapping(address => uint256[]) public allTokens; // user address => list of available token ids. Unordered Array.
    ...
    function info(uint256 id) public view returns (string memory name, bytes16 cardType, bytes1 rarity, uint256 rank, uint256 level, string memory uri, uint256 blockNum) {
        uint256 cardId = card(id);
        CardPool pool = CardPool(tokenPools[id]);
        (name,cardType,rarity,rank,level,uri) = pool.getCard(cardId);
        uri = uris[id];
    ...
    function setPool(bytes1 poolId, address poolAddress) public onlyAdmin(DEFAULT_ADMIN_ROLE) {
        ...
        emit PoolSet(poolId, poolAddress);
    }
    ...
    function _generateTokenId(uint256 _cardId) internal returns (uint256) {
        ...
        address pool = availablePools[poolId];
        CardPool cardPool = CardPool(pool);
        require(_count <= count(_cardId) && cardPool.exists(_cardId), "Card does not exist");
        ...
        tokenPools[uint256(tokenId)] = pool;
        (,,,,, string memory uri) = cardPool.getCard(_cardId);
        uris[uint256(tokenId)] = uri;
    ...
    /* TODO: not sure the burn functions are required or not or who uses this. need to see
             how f1 delta handle nft burn functionality
    */
    ...
    function parse(uint256 id) public pure returns (bytes1 token, bytes4 cdhId, bytes1 poolId, bytes32 cardType, bytes1 rarity, uint256 counter_){
        ...
        temp = id;
        counter_ = ((temp >> 16) << 176) >> 192;

        temp = id;
        token = bytes1(bytes32(temp << 248));
    }

    function count(uint256 _cardId) public view returns (uint256) {
...
contract ChestOperator is Ownable, Pausable {
    CDHInventory private target;

    constructor(address _targetInventory)
    Ownable()
    Pausable() {
        target = CDHInventory(_targetInventory);
    }
    ...
    function _open(address _to, uint256 _id, uint256 _amount, bytes memory _data) internal whenNotPaused {
        target.mint(_to, _id, _amount, _data);
    }

    function _open(address _to, uint256[] memory _ids, uint256[] memory _amounts, bytes memory _data) public whenNotPaused {
        target.mintBatch(_to, _ids, _amounts, _data);
    }
}
...
abstract contract Chest is ChestOperator, EIP712MetaTransaction {
    ...
    string private constant NAME = "CRAZY DEFENCE HEROES CHEST";
...
// File contracts/chest/GoldChest.sol


pragma solidity >=0.6.6;

contract GoldChest is Chest {
    string private constant NAME = "CRAZY DEFENCE HEROES GOLD CHEST";

    constructor(
        string memory _name,
        string memory _version,
        address _goldChestToken,
        address _randomNumberSender,
        address _inventory,
        address _equipmentPool,
        address _heroPool,
        ...
        address _towerPool,
        address _blackHoleContract
    ) Chest(_name, _version, _goldChestToken, _randomNumberSender, _inventory,
    ...
    function _getProbabilisticRarity(uint256 rarityProb, uint256 counter) internal override pure returns (bytes1) {
        if (counter == 1) {
            ...
            } else if (rarityProb > 20 && rarityProb <= 55) {
                return Constants.RARITY_RARE;
            } else if (rarityProb > 55 && rarityProb <= 100) {
                return Constants.RARITY_EPIC;
            }
        } else if (counter == 2) {
            if (rarityProb <= 10) {
                return Constants.RARITY_COMMON;
            } else if (rarityProb > 10 && rarityProb <= 25) {
                return Constants.RARITY_RARE;
            } else if (rarityProb > 25 && rarityProb <= 70) {
                return Constants.RARITY_EPIC;
            } else if (rarityProb > 70 && rarityProb <= 100) {
