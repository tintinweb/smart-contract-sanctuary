...
contract BridgeHandler is IBridgeHandler {
    ...
    modifier onlyBridge() {
        require(msg.sender == bridge, "sender must be bridge contract");
        _;
    }
    // destId => depositNonce => Deposit Record
    mapping(uint256 => mapping(uint64 => DepositRecord)) public depositRecords;

    /**
        @param _bridge Contract address of previously deployed Bridge.
        @param _tokenAddress Contract address of token.
     */
    constructor(address _bridge, address _tokenAddress) {
        bridge = _bridge;
        token = _tokenAddress;
    }
    ...
    */
    function getDepositRecord(uint64 _depositNonce, uint256 _destId)
        external
        view
     ...
     */
    function _mintERC20(address _recipient, uint256 _amount) internal {
     ...
     */
    function _burnERC20(address _owner, uint256 _amount) internal {
