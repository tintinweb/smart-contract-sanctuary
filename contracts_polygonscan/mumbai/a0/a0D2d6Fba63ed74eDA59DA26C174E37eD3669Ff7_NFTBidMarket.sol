...
contract NFTBidMarket is ReentrancyGuard, Ownable {
    ...
    function createMarketAccept(uint256 itemId, address erc20Address, uint256 amount) public payable nonReentrant {
        ...
        IERC20 token = IERC20(item.erc20Address);
        require(token.balanceOf(item.bidder) >= priceTotal, "Insufficient bidder ERC20 balance for paying for the asset.");
        if (priceTotal > token.allowance(item.bidder, address(this))) {
        ...
        token.transferFrom(item.bidder, msg.sender, priceTotal.sub(fee));
        if (msg.value > 0) {
