...
contract IdentityProxyFactory {
    ...
    constructor(address identityImpl) {
        require(
            ...
            "IdentityProxyFactory: identity implementation must not be the zero address"
        );
    ...
    function reserveProxy(bytes32 saltHash) external {
        require(
            ...
            "IdentityProxyFactory: reserved salt hash"
        );
        ...
        returns (address)
    {
        ...
        require(
            reserver == address(0) || msg.sender == reserver,
            "IdentityProxyFactory: caller must be the reserver"
        );
