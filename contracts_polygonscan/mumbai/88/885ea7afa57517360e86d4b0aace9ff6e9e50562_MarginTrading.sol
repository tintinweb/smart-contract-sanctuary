...
contract MarginTrading {
    ...
    function withdrawAfterTrade(uint256 _positionID) public payable {
        ...
        if(amount >= depositInfo.assetAmount) {
            ...
            withdrawAmount = positionInfo.targetAmount.sub(_interest);
            depositInfo.loanAmount = 0;
            depositInfo.leverage = 0;
        }
        else {
        ...
        if(positionInfo.targetAsset == address(0)) 
            ...
            IERC20(positionInfo.targetAsset).transfer(msg.sender, withdrawAmount);

        userbalances[msg.sender][positionInfo.targetAsset] =  userbalances[msg.sender][positionInfo.targetAsset].sub(withdrawAmount);
        depositInfo.assetAmount =  depositInfo.assetAmount.sub(withdrawAmount);
        depositInfo.usedMargin =  depositInfo.usedMargin.sub(withdrawAmount);
        depositInfo.timestamp = block.number; 
