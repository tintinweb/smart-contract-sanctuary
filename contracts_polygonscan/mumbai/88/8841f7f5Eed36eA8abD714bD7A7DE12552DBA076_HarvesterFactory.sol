...
contract HarvesterFactory is Ownable, ReentrancyGuard, IFactory {
   ...
   // 合约部署者增加操作员
    ...
    function geneMetaData(address owner, uint256 _id, string memory _imageUrl) external override returns (string memory){
        ...
        string memory output = string(abi.encodePacked('"image":"',_imageUrl,'","external_url":"https://api.mglobe.net/token/', toString(_id),'"}', json));
    ...
    function upgradeMetaData(address owner, uint256 oldId, uint256 newId, string memory _imageUrl) external override returns (string memory) {
        ...
        string memory json = string(abi.encodePacked('{"name":', nftNameHarvester, '","description":"Harvester for meta globe game", "attributes":[{"trait_type":"Energy Efficiency","value":', toString(energy_efficiency),'}, {"trait_type":"Uru Efficiency","value":',toString(uru_efficiency),'}, {"trait_type":"Max Durability","value":', toString(max_durability),'}],'));
        string memory output = string(abi.encodePacked('"image":"',_imageUrl,'","external_url":"https://api.mglobe.net/token/', toString(newId),'"}', json));
            ...
            returns (bool)
    {
        ...
        emit OnAllowUpgrade(owner, tokenId, _energy_efficiency, _uru_efficiency, _max_durability);
        return true;
    }
