...
contract EtheremonWorldNFT is BasicAccessControl, NativeMetaTransaction {
    ...
    function getPrice(
        ...
        external
        view
        returns(bool catchable, uint price)
    {
        EtheremonDataBase data = EtheremonDataBase(dataContract);
        MonsterClassAcc memory class;
        (class.classId, class.price, class.returnPrice, class.total, class.catchable) = data.getMonsterClass(_classId);
    ...
    //MATIC: CHANGES FOR SUPPORTING ETH
        ...
        isActive
    {
        ...
        uint8 value;
        uint seed = getRandom(msgSender(), block.number-1, objId);
        // generate base stat for the previous one
        for (uint i=0; i < STAT_COUNT; i+= 1) {
            seed /= 100;
            value = uint8(seed % STAT_MAX) + data.getElementInArrayType(EthermonEnum.ArrayType.STAT_START, uint64(_classId), i);
        ...
        EtheremonMonsterNFTInterface(monsterNFT).triggerTransferEvent(address(0), msgSender(), objId);
        // refund extra
        if (value > price) {
