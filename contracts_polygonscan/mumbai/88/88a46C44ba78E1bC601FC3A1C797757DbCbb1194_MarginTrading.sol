...
contract MarginTrading {
    ...
    function createPositionandTrade(address tokenAddress, address targetToken, uint256 amount) external {
        Deposit storage depositInfo = deposits[msg.sender][tokenAddress];
        require(depositInfo.assetAmount >= amount, "Create Position call : Insufficient Deposit");
    ...
    function closePosition(uint256 _positionID) external {
        withdrawAfterTrade(_positionID);
    ...
    function withdrawAfterTrade(uint256 _positionID) public payable {
        ...
        uint256 targetWithdrawAmount; uint256 depositWithdrawAmount;
        if(amount >= depositInfo.assetAmount) {
            depositInfo.interestAccumulated = 0;
            if(depositInfo.assetAmount < _interest) {
                uint256 amountTopay = _interest.sub(depositInfo.assetAmount);
                if(IERC20(positionInfo.targetAsset).decimals() == 6) {
                    amountTopay = amountTopay.div(1E6);
                    _interest = _interest.div(1E6);
                }
            ...
            if(positionInfo.asset == uniswapV2Router.WETH()) depositWithdrawAmount = positionInfo.targetAmount.sub(_interest.mul(1E6));
            else {
        ...
        depositInfo.assetAmount =  depositInfo.assetAmount.sub(depositWithdrawAmount);
        depositInfo.usedMargin =  depositInfo.usedMargin.sub(depositWithdrawAmount);
        depositInfo.timestamp = block.number; 
