...
contract Mars is Context, Ownable, ERC20 {
    ...
    address public lootContractAddress =
        0xd30c23d19187B2f24b6bC7dD41AB2d6C8bC3Db56;
    ...
    uint256 public MarsPerTokenId = 10000 * (10**decimals());
    ...
    */
    ...
    constructor() Ownable() ERC20("Mars", "MARS") {
        ...
        transferOwnership(0x626103C5333673da4f7F4289bEbbA9cF9993d7f6);
...
//        require(totalSupply() < MAX_NFT_SUPPLY, "Sale has already ended");
        ...
        if (currentSupply >= 2) {
            return tokenId.mul(_ec); // 8000 - 1000 0.3 ETH
        } else if (currentSupply >= 1) {
            return tokenId.mul(_eb); // 3000 - 7999 0.2 ETH
        } else {
            return tokenId.mul(_ea); // 0 - 2999 0.1 ETH
        }
    }
...
//    /// @param tokenId The tokenId of the Loot NFT
    function claim(uint256 mine) payable external {
        // Follow the Checks-Effects-Interactions pattern to prevent reentrancy
        // attacks
        require(getNFTPriceForETH(mine) <= msg.value, "Polygon value sent is not correct");


        // Checks

        // Check that the msgSender owns the token that is being claimed
//        require(msg.value >= 0.001 ether, "Must more than 1 polygon !");
        require(
            _msgSender() == lootContract.ownerOf(mine),
            "MUST_OWN_TOKEN_ID"
        );
...
//        claimed[msg.sender] += 1;
    ...
    function _claim(uint256 tokenId, address tokenOwner) internal {
        ...
        _mint(tokenOwner, MarsPerTokenId);
    }
    ...
    function daoSetMarsPerTokenId(uint256 marsDisplayValue)
        public
        onlyOwner
    {
        MarsPerTokenId = marsDisplayValue * (10**decimals());
    }

    function daoSetSeasonAndMarsPerTokenID(
        uint256 season_,
        uint256 marsDisplayValue
    ) external onlyOwner {
        daoSetSeason(season_);
        daoSetMarsPerTokenId(marsDisplayValue);
    }
}