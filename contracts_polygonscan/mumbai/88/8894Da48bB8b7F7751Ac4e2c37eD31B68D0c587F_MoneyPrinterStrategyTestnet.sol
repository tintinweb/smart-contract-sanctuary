...
/*         DAI.approve(address(WexPolyRouter), type(uint).max);
    ...
    function deposit(uint _amount, IERC20 _token) external onlyVault{
        ...
        _depositTestnet(_amount, _token);

    }

    function _depositTestnet(uint _amount, IERC20 _token) internal {
        if(_token == DAI) {
            daiInPool = daiInPool.add(_amount);
        }
        if(_token == USDC) {
            usdcInPool = usdcInPool.add(_amount.mul(1e12));
        }
        if(_token == USDT) {
            usdtInPool = usdtInPool.add(_amount.mul(1e12));
        }
    }
    ...
    function withdrawTestnet(uint _amount, IERC20 _token) external onlyVault{
        if(_token == DAI) {
            daiInPool = daiInPool.sub(_amount);
        }
        if(_token == USDC) {
            usdcInPool = usdcInPool.sub(_amount);
            _amount = _amount.div(1e12);
        }
        if(_token == USDT) {
            usdtInPool = usdtInPool.sub(_amount);
            _amount = _amount.div(1e12);
        }

        _token.safeTransfer(vault, _amount);
    }
