...
// File contracts/GalaxyLottery.sol
...
contract GalaxyLottery is ReentrancyGuard, Ownable {
    ...
    // function to get tickets of an address in current 
    ...
    function _randModulus(uint256 _mod) internal returns(uint256) {
        uint256 rand = uint256(keccak256(abi.encodePacked(nonce, oracle.rand(), block.timestamp, block.difficulty, 
                                msg.sender))) % _mod;
        nonce++;
        return rand;
    }
