...
contract AngleNFT is ERC721Tradable {
    ...
    // Count of tokenID
    Counters.Counter private gTokenIdCount;
    Counters.Counter private sTokenIdCount;
    Counters.Counter private bTokenIdCount;

    // Count of tokenID
    // TODO fix these parameters
    uint256 public gTokenSupply = 500;
    uint256 public sTokenSupply = 1000;
    ...
    modifier onlyOwner() {
        require(owners[_msgSender()], "Caller is not owner");
        _;
    }
    ...
    ) ERC721Tradable("Angle Knowledge NFT", "ANGLEKNOWLEDGE") {
        merkleRoot = _merkleRoot;
        baseTokenURI = _baseTokenURI;

        sTokenIdCount._value = gTokenSupply;
        bTokenIdCount._value = gTokenSupply + sTokenSupply;
    ...
    function mint(
        ...
        _setClaimed(index);
        if (tokenType == 0) {
            require(gTokenIdCount._value < gTokenSupply, "max supply reached");
            gTokenIdCount.increment();
            _mint(to, gTokenIdCount.current());
        } else if (tokenType == 1) {
            require(
                sTokenIdCount._value < sTokenSupply + gTokenSupply,
                "max supply reached"
            );
            sTokenIdCount.increment();
            _mint(to, sTokenIdCount.current());
        } else {
            bTokenIdCount.increment();
            _mint(to, bTokenIdCount.current());
        }
    }
