...
contract ScrapsMerchantMumbai is Ownable, VRFConsumerBase {
  ...
  ERC20 internal weth;
  AggregatorInterface internal maticFeed;
  AggregatorInterface internal ethFeed;
  ...
  mapping(bytes32 => Minting) internal mintings;
    ...
    )
  {
    ...
    weth = ERC20(0x326C977E6efc84E512bB9C30f76E30c160eD06FB);
    maticFeed = AggregatorInterface(0xd0D5e3DB44DE05E9F294BB0a3bEEaF030DE24Ada);
    ethFeed = AggregatorInterface(0x0715A7794a1dc8e42615F059dD6e406A6594651A);
    jys = JunkyardScraps(address(0)); // TODO: fill address
  ...
  function mint(uint256 amount) internal {
    ...
    Minting storage m = mintings[requestId];
    m.amount = amount;
    m.minter = msg.sender;
  }
  ...
  function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
    Minting storage m = mintings[requestId];
    uint256[] memory amounts = expand(randomness, m.amount);
    for (uint256 i = 1; i <= 3; i++) {
      uint256 amount = amounts[i - 1];
      if (amount > 0) {
        jys.mint(m.minter, i, amount);
      }
    }
  }

  function expand(uint256 randomValue, uint256 n) internal view returns (uint256[] memory) {
    uint256[] memory amounts = new uint256[](3);
    for (uint256 i = 0; i < n; i++) {
      uint256 scale = uint256(keccak256(abi.encode(randomValue, i))) % 100;
      for (uint256 j = 1; j <= 3; j++) {
        uint256 chance = chances[j];
        if (scale <= chance) {
          amounts[j-1]++;
          break;
        } else {
  ...
  function MATICPrice() public view returns (uint256) {
    uint256 matic = uint256(maticFeed.latestAnswer());
    uint256 eth = uint256(ethFeed.latestAnswer());
    return eth * ETHprice / matic;
  }

  modifier purchaseConditions(uint256 amount) {
    require(remaining >= amount, "There aren't that many scraps remaining this time");
    require(amount > 0, "Amount must be at least 1");
    require(amount <= maxPerTransaction, "You can't buy that amount on 1 transaction");
    _;
  }

  function purchaseWithMATIC(uint256 amount) public payable purchaseConditions(amount) {
    uint256 maticPrice = MATICPrice();
    uint256 totalPrice = maticPrice * amount;
    require(msg.value >= totalPrice, "You must send enough MATIC to fulfill your order");
    remaining -= amount;
    mint(amount);
  }

  function purchaseWithETH(uint256 amount) public purchaseConditions(amount) {
    uint256 allowance = 1 ether * weth.allowance(msg.sender, address(this));
    uint256 totalPrice = ETHprice * amount;
    require(allowance >= totalPrice, "You must send enough WETH to fulfill your order");
  ...
  function purchase(uint256 amount) public payable {
    if (msg.value > 0) {
      purchaseWithMATIC(amount);
    } else {
      purchaseWithETH(amount);
    }
  }

  function addChance(uint256 level, uint256 chance) public onlyOwner {
    require(level >= 1 && level <= 3, "Incorrect level");
    require(chance >= 1 && chance <= 100, "Incorrect chance");
    chances[level] = chance;
  }
  ...
  function withdraw() public onlyOwner {
    ...
    uint256 wethBalance = 1 ether * weth.balanceOf(address(this));
...
contract JunkyardScraps is Ownable, ERC1155, Whitelist, JunkyardDroppable1155 {
    ...
    function mintOfLevel(address account, uint256 level, uint256 amount) internal {
      ...
      for (uint256 i = 0; i < amount; i++) {
        ...
        uint256 tokenId = 10 * level + 1 + random;
    ...
    function levelUp(uint256 level, uint256[] memory amounts) public {
      ...
      for (uint256 i = 0; i < amounts.length; i++) {
        require(balanceOf(msg.sender, 10 * level + i + 1) > 0, "You must have enough of that type of token");
