...
contract PrimodialEnergy is ERC721Enumerable, ReentrancyGuard {
    ...
    constructor(address _oracleAddress)
        ERC721("Primodial Crypto Energy", "PCE")
        ReentrancyGuard()
    {
...
contract WorkerNFT is ERC721Enumerable, ReentrancyGuard {
    ...
    constructor(
        address payable primodialContractInitAddress,
        address _oracleAddress
    ) ERC721("Primodial Crypto Worker", "PCW") ReentrancyGuard() {
        // gameContract = gameContractInitAddress;
        primodialContractAddress = primodialContractInitAddress;
        primodial = PrimodialEnergy(primodialContractInitAddress);
        oracle = Oracle(_oracleAddress);
    }
    ...
    // function useWorker(uint256 workerId, uint8 durability, uint8 stamina) external {
    //     require(msg.sender == gameContract);
    //     require(workers[workerId].stamina != 0 && workers[workerId].durability != 0, "it's not gonna work" );
    //     workers[workerId].stamina -= stamina;
    //     workers[workerId].durability -= durability;
    // }
        ...
        returns (uint256 newWorkerId)
    {
        ...
        // workers[currentWorkerIds].durability = 100;
        // workers[currentWorkerIds].stamina = 100;
        // workers[currentWorkerIds].hp = 5000;
