...
contract JACK_IN_THE_BLOCKS is ERC721, ERC721Enumerable, Ownable {
  ...
  string internal hiddenCID; // hash to hide metadata for reveal
    ...
    returns (string memory)
  {
    ...
    return
      bytes(baseCID).length > 0
        ...
        : string(abi.encodePacked('ipfs://', hiddenCID, '/'));
  }
  ...
  function pre_mint() external payable {
    ...
    require(mintedFromSupply + 1 <= mintingSupply - _reserved, 'EXCEED_SUPPLY');
    _tokenIds.increment();
  ...
  function mint(uint8 amount) external payable {
    ...
    require(msg.value * amount == getDiscountPrice(msg.sender), 'ETH sent not correct!');
    require(
  ...
  function setHiddenCID(string memory cid) public onlyOwner {
    hiddenCID = cid;
  }
