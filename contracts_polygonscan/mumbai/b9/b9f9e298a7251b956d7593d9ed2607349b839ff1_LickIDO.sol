...
contract LickIDO is Ownable {
    ...
    mapping(address => bool) public boughtLICK;
    ...
    constructor(
        ...
        require(_maiLickLP != address(0));

        LICK = _LICK;
        MAI = _MAI;
        treasury = _treasury;
        maiLickLP = _maiLickLP;
        staking = _staking;
        cancelled = false;
        finalized = false;
    }
    ...
    function purchaseLICK(uint256 _amountMAI) external returns (bool) {
        require(saleStarted() == true, 'Not started');
        require(
            !whiteListEnabled || whiteListed[msg.sender] == true,
            'Not whitelisted'
        );
        require(boughtLICK[msg.sender] == false, 'Already participated');

        boughtLICK[msg.sender] = true;
    ...
    function finalize(address _receipt) external onlyOwner {
        require(totalAmount == 0, 'need all licks to be sold');

        uint256 maiInTreasure = 250000 * 1e18;

        IERC20(MAI).approve(treasury, maiInTreasure);
        uint256 lickMinted = ITreasury(treasury).deposit(maiInTreasure, MAI, 0);

        require(lickMinted == 250000 * 1e9);

        // dev: create lp with 15 MAI per LICK
        IERC20(MAI).transfer(maiLickLP, 750000 * 1e18);
        IERC20(LICK).transfer(maiLickLP, 50000 * 1e9);
        uint256 lpBalance = IUniswapV2Pair(maiLickLP).mint(address(this));
        uint256 valueOfToken = ITreasury(treasury).valueOfToken(
            maiLickLP,
            lpBalance
        );

        IUniswapV2Pair(maiLickLP).approve(treasury, lpBalance);
        uint256 zeroMinted = ITreasury(treasury).deposit(
            lpBalance,
            maiLickLP,
            valueOfToken
        );
        require(zeroMinted == 0, 'should not mint any LICK');
        IERC20(LICK).approve(staking, lickMinted);
