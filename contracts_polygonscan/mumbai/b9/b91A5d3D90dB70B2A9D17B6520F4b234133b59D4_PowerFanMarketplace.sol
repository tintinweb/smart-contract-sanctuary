...
contract PowerFanMarketplace is ReentrancyGuard, Ownable {
    ...
    function _makeOrder(
        ...
        IERC20 tokenA = IERC20(pairA);

        // validate maker's deposit
        if (pairAisEther && (weiSend <= 0 || weiSend < amount)) return false;
        else if (
            !pairAisEther &&
            (tokenA.allowance(maker, address(this)) < amount ||
                tokenA.balanceOf(maker) < amount)
        ) return false;
        ...
        if (pairAisEther) {
            ...
            // pull tokens
            tokenA.safeTransferFrom(maker, address(this), amount);
        }
        isActive[id] = true;
    ...
    function _takeOrder(
        ...
        // token interfaces
        IERC20 tokenA = IERC20(order.pairA);
        IERC20 tokenB = IERC20(order.pairB);

        // validate taker's deposit
        require(!pairBisEther && (weiSend >= 0 || weiSend > totalB));

        require(
            (!pairBisEther &&
                (tokenB.allowance(taker, address(this)) > totalB ||
                    tokenB.balanceOf(taker) > amount))
        );
        ...
        if (pairBisEther) {
            ...
            payable(order.maker).transfer(totalB);
        } else {
