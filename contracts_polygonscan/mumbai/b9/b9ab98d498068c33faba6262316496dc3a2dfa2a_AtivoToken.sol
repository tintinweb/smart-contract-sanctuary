...
// File: contracts/AtivoTokenMatic.sol
    ...
    ContextMixin
{
     ...
     */
    function pause() public virtual only(DEFAULT_ADMIN_ROLE) {
        _pause();
    }
     ...
     * - the caller must be owner
     */
    function unpause() public virtual only(DEFAULT_ADMIN_ROLE) {
        _unpause();
    }
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "ATIVO::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "ATIVO::delegateBySig: invalid nonce"
        );
        require(now <= expiry, "ATIVO::delegateBySig: signature expired");
        ...
        returns (uint256)
    {
        require(
            ...
            "ATIVO::getPriorVotes: not yet determined"
        );
    ...
    function _delegate(address delegator, address delegatee) internal {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying ATIVOs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            "ATIVO::_writeCheckpoint: block number exceeds 32 bits"
        );
