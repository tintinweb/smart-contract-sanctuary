...
contract Offers is Ownable, Pausable {
    ...
    mapping(uint256 => OfferItemFinalOffer) private offersFinalOffers;
    ...
    // check that final offer offer's state fits selected pricing table item
    function finalOfferFitsPricingTableItem(
        OfferItemFinalOffer memory offer,
        PricingTableItem memory pricing
    ) internal pure returns (bool) {
        return
            isInMinMaxRange(
    ...
    function updateOfferStatus(uint256 _offerId, uint256 _newStatus) internal {
        require(
    ...
    {
        require(
            _offer.amount > 0 && _offer.status != 1,
    ...
    // new state for offer - final offer
    function offerFinalOffer(uint256 _id, OfferItemFinalOffer memory _offer)
        public
        whenNotPaused
        onlyOwner
        returns (bool)
    {
        ...
        offersFinalOffers[_id] = _offer;
        updateOfferStatus(_id, 2);
        emit OfferFinalOffer(_id, _offer);
        return true;
    }
    ...
    {
        return (
            offersFinalOffers[_id].tokenAddress,
    ...
    event OfferFinalOffer(uint256 _id, OfferItemFinalOffer _offer);
