...
// File: original_contracts/Global/AutoBetSettings.sol
...
contract AutoBetSettings is EIP712MetaTransaction {
    using SafeMath for uint256;
    ...
    mapping(address=>mapping(address=>AutoBet)) public userVsGameVsAutobet;

    IBadBitSettings public settings;

    event AutoBetStarted(
    ...
    {
        require(
            settings.isGameAddress(game),
            "Invalid game address"
        );
        require(
            GameInterface(game).validatePayload(payload),
            "Invalid payload"
        );

        AutoBet storage autoBet = userVsGameVsAutobet[_msgSender()][game];

        require(autoBet.numberOfBets == 0, "Autobet already running for user");

        autoBet.numberOfBets = numberOfBets;
        autoBet.betAmount = betAmount;
        autoBet.payload = payload;
    ...
    {
        require(
            settings.isOperatorAddress(_msgSender()) || settings.isGameAddress(_msgSender()),
            "Unauthorized access"
        );

        AutoBet storage autoBet = userVsGameVsAutobet[_msgSender()][game];
        uint256 remBets = autoBet.numberOfBets;
        autoBet.numberOfBets = 0;
        emit AutoBetStopped(
            user,
            game,
            remBets
    ...
    {
        AutoBet memory autoBet = userVsGameVsAutobet[user][game];
        return(
           autoBet.numberOfBets,
           autoBet.betAmount,
           autoBet.payload
        );
    }

    function updateAutoBetForUser(address user) external {
        require(
            settings.isGameAddress(_msgSender()),
            "Unauthorized access"
        );

        AutoBet storage autoBet = userVsGameVsAutobet[user][_msgSender()];

        require(autoBet.numberOfBets > 0, "Autobet disabled for the user");

        autoBet.numberOfBets = autoBet.numberOfBets.sub(1);
        if (autoBet.numberOfBets == 0) {
            emit AutoBetStopped(
                user,
                _msgSender(),
                0
