    ...
    ContextMixin
{
    ...
    {
        _setupContractId("ChildNFTMetaKey");
    ...
    */
    ...
    function maxSupply(uint256 _id) public pure returns (uint256 _maxSupply) {
        if (_id == 1) {
          _maxSupply = uint256(500);
        } else if (_id == 2)  {
          _maxSupply = uint256(750);
        } else if (_id == 3)  {
          _maxSupply = uint256(1000);
        } else if (_id == 4)  {
          _maxSupply = uint256(7500);
        } else if (_id == 5)  {
          _maxSupply = uint256(255);
        } else {
          _maxSupply = uint256(10000);
        }
    }
        ...
        only(DEPOSITOR_ROLE)
    {
        ...
        require(
            user != address(0),
            "ChildNFTMetaKey: INVALID_DEPOSIT_USER"
        );
    ...
    function mint(
        ...
        require( (id > uint(0)), "ChildNFTMetaKey: INVALID_TOKEN_ID");
        require(((amount > uint(0)) && (amount == restTotalSupply(id, amount))) , "ChildNFTMetaKey: INVALID_TOKEN_AMOUNT");
    ...
    function mintBatch(
        ...
        for (uint i = 0; i < ids.length; i++) {
            require( (ids[i] > uint(0)), "ChildNFTMetaKey: INVALID_TOKEN_ID");
            require(((amounts[i] > uint(0)) && (amounts[i] == restTotalSupply(ids[i], amounts[i]))) , "ChildNFTMetaKey: INVALID_TOKEN_AMOUNT");
        }
    ...
    function burn(
        ...
        require(
            ...
            "ChildNFTMetaKey: caller is not owner nor approved"
        );
    ...
    function restTotalSupply(uint256 _id, uint256 _amount) public view returns (uint256) {
      ...
      require(( subtotal <= maxSupply(_id)), "ChildNFTMetaKey: EXCEED MAX_AMOUNT");
      return _amount;
    }
    ...
    function _beforeTokenTransfer(
        ...
        require(!paused(), "ChildNFTMetaKey: token transfer while paused");
