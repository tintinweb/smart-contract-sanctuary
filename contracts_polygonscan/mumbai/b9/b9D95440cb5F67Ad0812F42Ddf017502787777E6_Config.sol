...
contract Config is AccessControl {
    ...
    function setLatestAddress(
        ...
        if (isUpgradable[_contractName]) {
            contractsActive[contractAddresses[_contractName]] = false;
            contractsActive[_contractAddress] = true;
        }
        contractAddresses[_contractName] = _contractAddress;
    }
    ...
    function getCurrencyDecimal(uint8 _currencyType)
        ...
        returns (uint8)
    {
        return currencyDecimals[CurrencyType(_currencyType)];
    }
    ...
    {
        return currencyName[CurrencyType(_currencyType)];
    }
     ...
     */
    function setInfiTokenAddr(address _newAddr) external {
        // Check Permission
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "ERR_AUTH_1");

        infiTokenAddr = _newAddr;
    }

    function setMaxDevaluation(uint256 _newValue) external {
        // Check Permission
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "ERR_AUTH_1");
        maxDevaluation = _newValue;
    }

    function setMonitoringPeriod(uint256 _newValue) external {
        // Check Permission
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "ERR_AUTH_1");
        monitoringPeriod = _newValue;
    }

    function setMaxPayoutPeriod(uint256 _newValue) external {
        // Check Permission
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "ERR_AUTH_1");
        maxPayoutPeriod = _newValue;
    }

    function setValidationPreviousPeriod(uint256 _newValue) external {
        // Check Permission
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "ERR_AUTH_1");
        validationPreviousPeriod = _newValue;
    }
}
