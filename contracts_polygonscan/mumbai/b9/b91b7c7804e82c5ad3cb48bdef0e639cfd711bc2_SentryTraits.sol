...
contract SentryTraits is Ownable, IStructs {
    ...
    function generateStat(uint8 min) private returns (uint8) {
...
// TypeError: Member "length" not found or not visible after argument-dependent lookup in mapping(uint8 => struct Traits.Layer storage ref).
    ...
    function assembleTraits(uint8 species, uint16 tokenId) external onlyController returns (Traits memory) {
        Sentry memory sentry = factory.getSentryTraits(tokenId);
        Traits memory newTraits = Traits(
            layerCount[species][generateRarity(layerCount[species][0])],
            layerCount[species][generateRarity(layerCount[species][1])],
            sentry.generation == 0 ? 0 : 1,
            layerCount[species][generateRarity(layerCount[species][4])],
            layerCount[species][generateRarity(layerCount[species][5])],
            layerCount[species][generateRarity(layerCount[species][6])],
            layerCount[species][generateRarity(layerCount[species][7])]
        );
        return newTraits;
    }
    ...
    function buildImage(uint16 _tokenId) public view returns(string memory) {
        ...
        svg = string(abi.encodePacked(
            ...
            _drawLayer(layers[sentry.species][0][1])
        ));
        return string(abi.encodePacked(
    ...
    function _compileAttributes(uint16 tokenId) public view returns (string memory) {
        ...
        string memory traits = string(abi.encodePacked(
            _sintleAttributeCompilation("Background",   layers[sentry.species][0][sentryTraits.body].name), ",", 
            _sintleAttributeCompilation("Body",  layers[sentry.species][0][sentryTraits.body].name), ",",
            _sintleAttributeCompilation("Clothing Top", layers[sentry.species][0][sentryTraits.body].name), ",",
            _sintleAttributeCompilation("Clothing Bottom", layers[sentry.species][0][sentryTraits.body].name), ",",
            _sintleAttributeCompilation("Suit",  layers[sentry.species][0][sentryTraits.body].name), ",",
            _sintleAttributeCompilation("Backpack",  layers[sentry.species][0][sentryTraits.body].name), ",",
            _sintleAttributeCompilation("Ears",  layers[sentry.species][0][sentryTraits.body].name), ",",
            _sintleAttributeCompilation("Head",  layers[sentry.species][0][sentryTraits.body].name), ","
        ));
        return string(abi.encodePacked(
        '[',
            traits,
    ...
    function uploadTraits(uint8 species,uint8 layerCategory, uint8[] calldata layerIds, string[][] calldata newLayers) external onlyOwner {
        ...
        layerCount[species][layerCategory] = uint8(layerIds.length);
    }
