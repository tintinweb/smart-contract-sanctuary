...
contract Prediction is AccessProtected, Feed, KeeperCompatible {
    ...
    function placeBet(
        ...
        emit BetPlaced(sender, newbetId, team, bets[newbetId].amount, pool);
    }

    function totalBetsBySport(uint8 _sportId)
        public
        view
        returns (uint256 counter)
    {
        ...
        for (uint256 i = 0; i < totalBets; i++) {
            uint256 matchId = bets[i].matchId;
            if (matches[matchId].sportId == _sportId) {
                counter++;
            }
        }
    }

    function allBetsBySportId(uint8 _sportId)
        external
        view
        returns (uint256[] memory)
    {
        uint256[] memory result = new uint256[](totalBetsBySport(_sportId));
        uint256 totalBets = _betIds.current();
        uint256 counter = 0;
        for (uint256 i = 0; i < totalBets; i++) {
            uint256 matchId = bets[i].matchId;
    ...
    function totalBetsByMatchId(uint256 _matchId)
        public
        view
        returns (uint256 counter)
    {
    ...
    function allBetsByMatchId(uint256 _matchId)
        ...
        returns (uint256[] memory)
    {
        uint256[] memory result = new uint256[](totalBetsByMatchId(_matchId));
        uint256 totalBets = _betIds.current();
        uint256 counter = 0;
        for (uint256 i = 0; i < totalBets; i++) {
            if (bets[i].matchId == _matchId) {
