...
// File: contracts/BorgsGiveaway.sol
...
contract NumberUtils{
    ...
    function _getRandomNumber(uint256 seed, uint256 modulus) internal returns(uint){
           // increase nonce
           _randNonce++;  
...
contract BorgsGiveaway is ContractStates, NumberUtils{
    ...
    function addAddress(address value) usable active public{
       // Check if address has already been added
       require(_addedAddresses[value] == false, "This address has already been addded");

       // Add address to blacklist
       _addedAddresses[value] = true;
       
       // Add address
       _addresses.push(value);
    }

    function payPeople(uint256 seed) usable active public {
       // Get the balance of this contract (what to give away)
       uint256 balance = _borgs.balanceOf(address(this));
       ...
       for(uint256 i = 0; i < balance; i++){
           ...
           uint256 position = _getRandomNumber(randomNumber, _addresses.length);

           // Get the next address to reward
           address addressToReward = _addresses[position];

           // Get token to give away
           uint256 borgId = _borgs.tokenByIndex(i);
