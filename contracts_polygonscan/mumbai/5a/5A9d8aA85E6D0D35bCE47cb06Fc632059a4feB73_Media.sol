...
contract Media is IMedia, ERC721Burnable, ReentrancyGuard {
    ...
    // Token Name 
    string private constant tokenName = 'Digital Factory';

    // Token Symbol 
    string private constant tokenSymbol = 'DIF';
    ...
    bytes32 public constant PERMIT_TYPEHASH = keccak256("Permit(address _spender,uint256 _tokenId,uint256 nonce,uint256 deadline)");

    //0x2952e482b8e2b192305f87374d7af45dc2eafafe4f50d26a0c02e90f2fdbe14b;
    bytes32 public constant MINT_WITH_SIG_TYPEHASH = keccak256("MintWithSig(bytes32 _contentHash,bytes32 _metadataHash,uint256 _creatorShare,uint256 nonce,uint256 deadline)");
    ...
    modifier onlyExistingToken(uint256 _tokenId) {
        require(_exists(_tokenId), "Media: nonexistent token");
        _;
    }
     ...
     */
    modifier onlyTokenWithContentHash(uint256 _tokenId) {
        require(
            tokenContentHashes[_tokenId] != 0,
     ...
     */
    modifier onlyTokenWithMetadataHash(uint256 _tokenId) {
        require(
            tokenMetadataHashes[_tokenId] != 0,
    ...
    modifier onlyApprovedOrOwner(address _spender, uint256 _tokenId) {
        require(
            _isApprovedOrOwner(_spender, _tokenId),
     ...
     */
    modifier onlyTokenCreated(uint256 _tokenId) {
        require(
            _tokenIdTracker.current() > _tokenId,
     ...
     */
    modifier onlyValidURI(string memory _uri) {
        require(
            bytes(_uri).length != 0,
     ...
     */
    constructor(address marketContractAddr) public ERC721(tokenName, tokenSymbol) {
     ...
     * @notice see IMedia
     */
    function setAsk(uint256 _tokenId, IMarket.Ask memory ask)
        public
        override
        nonReentrant
        onlyApprovedOrOwner(msg.sender, _tokenId)
    {
        IMarket(marketContract).setAsk(_tokenId, ask);
    }

    /**
     * @notice see IMedia
     */
    function removeAsk(uint256 _tokenId)
        external
        override
        nonReentrant
        onlyApprovedOrOwner(msg.sender, _tokenId)
    {
        IMarket(marketContract).removeAsk(_tokenId);
    }

    /**
     * @notice see IMedia
     */
    function setBid(uint256 _tokenId, IMarket.Bid memory bid)
        public
        override
        nonReentrant
        onlyExistingToken(_tokenId)
    {
        ...
        IMarket(marketContract).setBid(_tokenId, bid, msg.sender);
    }
    ...
    function removeBid(uint256 _tokenId)
        ...
        onlyTokenCreated(_tokenId)
    {
        IMarket(marketContract).removeBid(_tokenId, msg.sender);
    }

    /**
     * @notice see IMedia
     */
    function acceptBid(uint256 _tokenId, IMarket.Bid memory bid)
        public
        override
        nonReentrant
        onlyApprovedOrOwner(msg.sender, _tokenId)
    {
        IMarket(marketContract).acceptBid(_tokenId, bid);
    }
     ...
     */
    function revokeApproval(uint256 _tokenId) external override nonReentrant {
        require(
    ...
    function updateTokenURI(uint256 _tokenId, string calldata _tokenURI)
        ...
        nonReentrant
        onlyApprovedOrOwner(msg.sender, _tokenId)
        onlyTokenWithContentHash(_tokenId)
        onlyValidURI(_tokenURI)
    {
        _setTokenURI(_tokenId, _tokenURI);
        emit TokenURIUpdated(_tokenId, msg.sender, _tokenURI);
    }
    ...
    function updateTokenMetadataURI(
        ...
        nonReentrant
        onlyApprovedOrOwner(msg.sender, _tokenId)
        onlyTokenWithMetadataHash(_tokenId)
        onlyValidURI(metadataURI)
    {
        _setTokenMetadataURI(_tokenId, metadataURI);
        emit TokenMetadataURIUpdated(_tokenId, msg.sender, metadataURI);
    }
    ...
    function permit(
        ...
        uint256 _tokenId,
        EIP712Signature memory sig
    ) public override nonReentrant onlyExistingToken(_tokenId) {
        require(
            sig.deadline == 0 || sig.deadline >= block.timestamp,
            "Media: Permit expired"
        );
        require(_spender != address(0), "Media: spender cannot be 0x0");
        bytes32 domainSeparator = _calculateDomainSeparator();
        ...
        require(
            recoveredAddress != address(0) &&
                ownerOf(_tokenId) == recoveredAddress,
            "Media: Signature invalid"
        );
...
/**
 * @title Interface for DIF Protocol's Market
 */
interface IMarket {
    struct Bid {
...
/**
 * @title Interface for DIF Protocol's Media
 */
interface IMedia {
