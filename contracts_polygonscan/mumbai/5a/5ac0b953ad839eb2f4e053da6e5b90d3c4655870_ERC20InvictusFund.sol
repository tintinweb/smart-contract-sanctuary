...
contract ERC20InvictusFund is OwnableUpgradeable, ERC20PresetMinterPauserUpgradeable, ERC2771ContextUpgradeable {
     ...
     */
    function addCurrency(string memory ticker, address contractAddress, uint256 numerator, uint256 decimals) external onlyPricingOracle {
     ...
     */
    function decreaseTokenAmendment(address participant, uint256 amount, string memory reason, string memory originalTxHash) external onlyAdjustable {
        require(amount > 0, 'Invalid amount');
    ...
    function dealRedemption(uint256 eventId, address participant, uint256 redemptionCurrencyAmount, uint256 tokenAmount, string memory redemptionCurrency) public onlyPricingOracle {
        ...
        // Checks that we have enough lequidity to process the redmption. 
        if(ERC20Upgradeable(stableContract).balanceOf(address(this)) < redemptionCurrencyAmount){
            ...
            emit RedemptionFailed(eventId, tokenAmount, redemptionCurrencyAmount, redemptionCurrency, participant, dealingNonce);
            return;
        } else {
            ERC20Upgradeable(stableContract).safeTransfer(participant, redemptionCurrencyAmount);
    ...
    function _handleRedemptionsDealing() internal returns (bool) {
        ...
        for(uint i = redemptions.length; i > endingIndex; i--){
    ...
    function _handleInvestmentsDealing() internal returns (bool) {
        uint256 endingIndex = investments.length.sub(MathUpgradeable.min(investments.length, maxInvestmentsPerTx));
        Investment memory investment;
        
        for(uint i = investments.length; i > endingIndex; i --){
    ...
    modifier onlyBurner(){
        _roleRequire(BURNER_ROLE, 'Only burner', _msgSender());
        _;
    }
    ...
    modifier onlyAdjustable() {
        _roleRequire(ADJUSTABLE_ROLE, 'Only adjustable', _msgSender());
        _;
    }
