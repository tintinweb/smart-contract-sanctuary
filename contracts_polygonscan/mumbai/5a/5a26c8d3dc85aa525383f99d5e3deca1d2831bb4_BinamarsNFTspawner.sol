...
//0x550a59a60eB73bE8A3F3b35507f72805512863D9
...
interface BinamarsNFT is IERC721 {
    function spawn(address to) external;

    function multiSpawn(address to, uint256 amount) external;
...
contract BinamarsNFTspawner is Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    IBinamarsRouter public binamarsRouter;
    BinamarsNFT public binamarsToken;
    ...
    constructor(
        address _binamarsRouter,
        address _binaToken,
        address _binamarsToken
    ) {
        binamarsRouter = IBinamarsRouter(_binamarsRouter);
        binaToken = IERC20(_binaToken);
        binamarsToken = BinamarsNFT(_binamarsToken);
    ...
    function setManager(address _config) public onlyOwner {
        binamarsRouter = IBinamarsRouter(_config);
    }

    function setNFT(address _binamarsToken) public onlyOwner {
        binamarsToken = BinamarsNFT(_binamarsToken);
    }
    ...
    function spawnMultiple(uint256 _amount) internal {
        uint256 totalFee = binamarsRouter.priceEgg().mul(_amount);
        if (totalFee > 0) {
            binaToken.transferFrom(
                _msgSender(),
                binamarsRouter.feeAddress(),
                totalFee
            );
        }
        if (_amount == 1) {
            binamarsToken.spawn(_msgSender());
        } else {
            binamarsToken.multiSpawn(_msgSender(), _amount);
        }
        _generateKey(bytes32(block.timestamp));
    }
    ...
    function buy(uint256 _amount) public {
        ...
        spawnMultiple(_amount);
    }

    function evolve(uint256 _tokenId) external {
        if (
            _msgSender() != tx.origin ||
            binamarsToken.ownerOf(_tokenId) != tx.origin ||
            binamarsToken.ownerOf(_tokenId) != _msgSender() ||
            Address.isContract(_msgSender()) ||
            Address.isContract(binamarsToken.ownerOf(_tokenId))
        ) {
            revert("reject");
        }

        uint256 fee = binamarsRouter.feeSpawn();

        if (fee > 0) {
            binaToken.transferFrom(_msgSender(), binamarsRouter.feeAddress(), fee);
        }
        uint256 rnd = random(privateKey, 4);
        uint8 level = uint8(parserLevel(rnd));
        binamarsToken.evolve(_tokenId, _msgSender(), level);
    ...
    function parserLevel(uint256 _random) internal pure returns (uint256) {
        ...
        if (_random > 9999) {
            ...
            return 4;
        } else if (_random > 6499) {
            return 3;
        } else if (_random > 3499) {
            return 2;
        } else {
            return 1;
        }
    ...
    function adminEvolve(uint256 _tokenId, uint8 _level) public onlyOwner {
        binamarsToken.evolve(_tokenId, binamarsToken.ownerOf(_tokenId), _level);
    }
