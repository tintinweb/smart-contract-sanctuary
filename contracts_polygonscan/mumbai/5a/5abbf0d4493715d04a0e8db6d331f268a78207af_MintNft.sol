...
contract MintNft is ERC721, Ownable, ReentrancyGuard{
    ...
    address public blockticketsAdmin;
   ...
   event TicketBought();
    ...
    function batchMint( string[] memory _uri, uint256[] memory _royalty) public nonReentrant returns (bool) {
    ...
    // blocktickets
    function buyTicket(string[] memory _uri, uint256[] memory _royalty) external payable nonReentrant returns(uint256[] memory ids){
    payable(blockticketsAdmin).transfer(msg.value);
            require(_uri.length == _royalty.length,"Length of Uri and Royalty should be same");
        uint256 _totalNft = _uri.length;  
        uint256[] memory tokenIds;
        for(uint i = 0; i< _totalNft; i++) {
            require(_royalty[i]< maximumRoyalty,"Royalty cannnot be 100 or more");
         uint256 id =  _mintNft(msg.sender, _uri[i], _royalty[i]);
         tokenIds[i] = id;
        }
    ...
    function setFeeTo(address _admin) external onlyOwner{
     blockticketsAdmin = _admin;
    }
...
function getOwnedBy(address user) external view returns(uint256[] memory _ids){
uint256[] memory ids;
uint256 counter = 0;
