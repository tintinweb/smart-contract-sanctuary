...
contract Evolution is Initializable, ERC721HolderUpgradeable, AccessControlUpgradeable, PausableUpgradeable  {
    ...
    uint256 private minEOCBalance;
    uint256 private daysForWithdraw;
    ...
    mapping(uint256 => uint256) public tracers; // tracerId => deposit timestamp 
    event RacerDepositEvent(bool _event, address from, address to, uint tokenId, uint timestamp);
    ...
    function initialize(address _EOC, address _RACE, address _TRACE) public initializer {
        ...
        newLevelDiff = 100000;
        minEOCBalance = 22500000;
        daysForWithdraw = 5 * 1 days;
    }
    modifier validate() {
        require(
    ...
    function setMinEOCBalance(uint _minEOCBalance) public validate {
        minEOCBalance = _minEOCBalance;
    }

    function getMinEOCBalance() public view returns (uint256) {
        return minEOCBalance;
    }

    function setDaysForWithdraw(uint _daysForWithdraw) public validate {
        daysForWithdraw = _daysForWithdraw;
    }

    function getDaysForWithdraw() public view returns (uint256) {
        return daysForWithdraw;
    }

    function deposit(uint256 _tokenId) public {
        require(RACE.ownerOf(_tokenId) == msg.sender, "Not authorized to deposit this Racer");
        // require eoc > 2500*9000, "Insufficiant funds"
        require(EOC.balanceOf(msg.sender) >= minEOCBalance, "Insufficient funds");
        
        // transfer 5% to _safe & burn rest
        uint256 fivePercent = minEOCBalance * 5 / 100;
        EOC.transfer(_safe, fivePercent);
        EOC.burn(msg.sender, minEOCBalance - fivePercent);
        ...
        tracers[_tokenId] = block.timestamp;
        // logs update.
        emit RacerDepositEvent(true, msg.sender, address(this), _tokenId, block.timestamp);
    }

    function withdraw(uint256 _tokenId) public {
        require(TRACE.ownerOf(_tokenId) == msg.sender, "Not authorized to withdraw this TRacer");
        require(block.timestamp >= tracers[_tokenId] + daysForWithdraw, "Tracer can not be withdrawn currently");
        // logs check for deposited NFT. !!
        ...
        RACE.safeTransferFrom(address(this), RACE.ownerOf(_tokenId), _tokenId); 

        // 108274
        // mint a new racer - Evolved character 100k+
        RACE.safeMint(msg.sender, _tokenId + newLevelDiff);
        emit RacerDepositEvent(false, msg.sender, RACE.ownerOf(_tokenId), _tokenId, block.timestamp);
        delete tracers[_tokenId];
    }
}
