...
contract ChessBettor is Ownable {
    ...
    function createGame(string memory _gameId, uint256 _amount) internal {
        ...
        newGame.voteList[newGame.aI].push(Vote(0, 0, block.timestamp, true)); // This implies that player has not placed bet yet.
    ...
    function placeBet(string memory _gameId, uint8 _vote, uint256 _amount) public payable {
        ...
        allPlayerData[msg.sender].gamesWithUnclaimedBalance.push(_gameId);  // TODO : Add AutoClaim System.
    ...
    function endGame(uint256 _gameIndex, string memory _gameId, uint8 _winner, uint256 _gameEndTimestamp) public onlyOwner() {
        ...
        if (_winner >= 3) {
            remainingFeeBalance += game.totalAmount;
            ...
            if (_winner == 1 && game.validAmountBlack == 0) {
                remainingFeeBalance += game.validAmountWhite;
            } else if (_winner == 2 && game.validAmountWhite == 0) {
                remainingFeeBalance += game.validAmountBlack;
            }
        } else if (_winner == 0) {
