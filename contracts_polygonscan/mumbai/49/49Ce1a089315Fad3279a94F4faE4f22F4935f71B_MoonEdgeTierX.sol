...
contract MoonEdgeTierX is VRFConsumerBase, AccessControl {
    ...
    function inArray(address who) public view returns (bool) {
        ...
        if (who != address(0x0) && _idoWinners[currentDraw_idoID][who] > 0) { 
            return true;
        }
        return false;
    }
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        ...
        _idoWinnersToDraw[currentDraw_idoID] = winnersToDraw;

        address currAddress = address(0);

        //TODO: change to 100
        for (uint16 i = 0; i < _idoWinnersToDraw[currentDraw_idoID]; i++) {

            currAddress = address(0);

            // random value
            _idoDrawValuesRaw[currentDraw_idoID][i] = uint256(keccak256(abi.encode(randomValue, i)));

            // winner index in participants array
            _idoWinnersParticipantsIndex[currentDraw_idoID][i] = (_idoDrawValuesRaw[currentDraw_idoID][i] % _idoNumberOfParticipants[currentDraw_idoID] );//zero based
            _idoWinnersParticipantsIndexOriginal[currentDraw_idoID][i] = _idoWinnersParticipantsIndex[currentDraw_idoID][i];

            currAddress = _idoParticipantsIndex[currentDraw_idoID][ _idoWinnersParticipantsIndex[currentDraw_idoID][i] ];
            
            // save that the original address was already a winner
            _idoWinnersPreBool[currentDraw_idoID][ currAddress ] = _idoWinnersBool[currentDraw_idoID][ currAddress ];

            // try to find a new winner
            if(_idoWinnersBool[currentDraw_idoID][ currAddress ]){
                for(uint16 e=0; e<100; e++){
                    _idoWinnersParticipantsIndex[currentDraw_idoID][i]++;
                    //if we reached the end, restart at 0
                    if(_idoWinnersParticipantsIndex[currentDraw_idoID][i] >= _idoNumberOfParticipants[currentDraw_idoID]){
                        _idoWinnersParticipantsIndex[currentDraw_idoID][i] = 0;
                    }
                    currAddress = _idoParticipantsIndex[currentDraw_idoID][ _idoWinnersParticipantsIndex[currentDraw_idoID][i] ];
            ...
            _idoWinnersList[currentDraw_idoID][i] = currAddress;
            
            // add participant address to winners list by address
            _idoWinners[currentDraw_idoID][ currAddress ] = i;
            
            _idoWinnersBool[currentDraw_idoID][ currAddress ] = true;

        }

        //emit RandomnessResult(randomResult);

    }


    function getInfo() public view returns (bytes32, uint256) { 
        return (lastRequestId, randomResult);
    }
