...
// File: contracts/interfaces/IPolyBetaLottery.sol

pragma solidity ^0.8.4;

interface IPolyBetaLottery {
    /**
    ...
    /**
     * @notice Draw the final number, calculate reward in BETA per group, and make lottery claimable
     * @param _lotteryId: lottery id
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in BETA token
     * @dev Callable by operator
     */
     ...
     * @param _priceTicketInBeta: price of a ticket in BETA
...
// File: contracts/PolyBetaLottery.sol

pragma solidity ^0.8.4;
pragma abicoder v2;

/** @title PolyBeta Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract PolyBetaLottery is ReentrancyGuard, IPolyBetaLottery, Ownable {
    ...
    uint256 public maxPriceTicketInBeta = 50 ether;
    uint256 public minPriceTicketInBeta = 0.005 ether;

    uint256 public pendingInjectionNextLottery;

    uint256 public constant MIN_DISCOUNT_DIVISOR = 300; // 3%
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInBeta;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] betaPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInBeta;
        uint32 finalNumber;
    }
     ...
     * @param _betaTokenAddress: address of the BETA token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _betaTokenAddress, address _randomGeneratorAddress) {
        betaToken = IERC20(_betaTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of BETA to this contract
        uint256 amountBetaToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInBeta,
            _ticketNumbers.length
        );

        // Transfer beta tokens to this contract
        betaToken.safeTransferFrom(address(msg.sender), address(this), amountBetaToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInBeta += amountBetaToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInBetaToTransfer
        uint256 rewardInBetaToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInBetaToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        betaToken.safeTransfer(msg.sender, rewardInBetaToTransfer);

        emit TicketsClaim(msg.sender, rewardInBetaToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInBeta) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in BETA for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].betaPerBracket[j] =
                ...
                // A. No BETA to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].betaPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInBeta - amountToShareToWinners);

        // Transfer BETA to treasury address
        betaToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        betaToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInBeta += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInBeta >= minPriceTicketInBeta) && (_priceTicketInBeta <= maxPriceTicketInBeta),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInBeta: _priceTicketInBeta,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            betaPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInBeta: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(betaToken), "Cannot be BETA token");
    ...
    /**
     * @notice Set BETA price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInBeta: minimum price of a ticket in BETA
     * @param _maxPriceTicketInBeta: maximum price of a ticket in BETA
     */
    function setMinAndMaxTicketPriceInBeta(uint256 _minPriceTicketInBeta, uint256 _maxPriceTicketInBeta)
        external
        onlyOwner
    {
        require(_minPriceTicketInBeta <= _maxPriceTicketInBeta, "minPrice must be < maxPrice");

        minPriceTicketInBeta = _minPriceTicketInBeta;
        maxPriceTicketInBeta = _maxPriceTicketInBeta;
    }
     ...
     * @param _priceTicket price of a ticket (in BETA)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].betaPerBracket[_bracket];
        } else {
            return 0;
        }
    }
