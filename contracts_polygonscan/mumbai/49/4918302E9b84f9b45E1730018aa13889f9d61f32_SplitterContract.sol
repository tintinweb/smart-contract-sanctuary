...
contract SplitterContract is OwnableUpgradeable {
    ...
    function setDistribution(
        ...
        require(_getArraySum(primary_shares) <= 100 * shareDecimals, "wrong primary share distribution");
        require(_getArraySum(secondary_shares) <= 10 * shareDecimals, "wrong secondary share distribution");
        ...
        onlyMarket
    {
        ...
        for (uint256 i = 0; i < currentArtist.addresses.length; i++) {
            uint256 targetShare = (usdAmountForDistribution *
                currentArtist.shares[i]) / 100 / shareDecimals;
    ...
    function secondaryDistribution(
        ...
        for (uint256 i = 0; i < currentArtist.addresses.length; i++) {
            uint256 targetShare = (usdAmountForDistribution *
                currentArtist.shares[i]) / 10 / shareDecimals;
            IERC20Upgradeable(usdc).safeTransfer(
                currentArtist.addresses[i],
                targetShare
            );
     ...
     */
    function setFeeDecimals (uint _decimals) external onlyOwner {
