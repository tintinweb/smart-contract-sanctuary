...
contract TradeRacer is Initializable, AccessControlUpgradeable, ERC721HolderUpgradeable, PausableUpgradeable  {
    IERC721 public RACE;
    IERC20 public KRL;
    ...
    struct Trade {
        uint8 status; // 1  - listed, 2 - sold, 3 - cancelled
        address seller;
        address buyer;
        uint price;
        uint racerId;
    ...
    function cancelRacerListing(uint _tokenId) public {
        ...
        trades[_tokenId].timestamp = block.timestamp;
        emit RacerTradeEvent(CANCELLED, msg.sender, address(0) , _tokenId, trades[_tokenId].price, block.timestamp);
    }
