...
contract Artifact is ERC721Enumerable, Ownable{
    using Strings for uint256;
    ...
    address private collectionContract;
    ...
    // uint256 constant ADVANCED = 250;
    // uint256 constant INTERMEDIATE = 500;
    uint256 constant ADVANCED = 2;
    uint256 constant INTERMEDIATE = 5;
    ...
    constructor(
        ...
        string memory _initSpecialURI) 
        ERC721("Artifact", "ART") 
    {
        setBaseURI(_initBaseURI);
        setDepletedURI(_initDepletedURI);
        setSpecialURI(_initSpecialURI);
        mint(5);
    }
    
    ///@notice Each address can only have 2 NFTs 
    modifier maxNftPerAddress(uint256 _amount){
        if(msg.sender != owner()){
            require(totalMintedByAddress[msg.sender]<NFTPERADDRESS && _amount<=NFTPERADDRESS, "Exceeded max amount");
        }
        _;
    }
    ...
    function _depletedURI() internal view returns (string memory) {
        return depletedURI;
    }
    
    function _specialURI() internal view returns (string memory) {
        return specialURI;
    }

    //public
    function mint(uint256 _amount)public payable maxNftPerAddress(_amount){
        require(canMint==true, "The sale period is over."); 
        require(paused==false, "Contract is paused");
        uint256 supply = totalSupply();
        require(_amount > 0, "Required to mint at least 1 NFT.");
        require(supply + _amount <= maxSupply, "Collection is sold out.");
        ...
        for(uint256 i=1; i<= _amount;i++){
            ...
            if(ADVANCED >= supply + i){
                freeMintsPerArtifact[supply + i] = 3;
            }else if(INTERMEDIATE >= supply + i){
                freeMintsPerArtifact[supply + i] = 2;
            }else{
                freeMintsPerArtifact[supply + i] = 1;
            }
            artifactURI[supply + i] = baseURI;
        }
    }
    ...
    returns (string memory){
        ...
        return bytes(artifactURI[_tokenId]).length >0 ? string(abi.encodePacked(artifactURI[_tokenId], _tokenId.toString(), ".json")) : "";
    }
    
    function freeMintsLeft(uint256 _tokenId) public view returns (uint256){
    ...
    function useFreeMint(uint256 tokenId, address _senderAddress) external {
        ...
        require(freeMintsPerArtifact[tokenId]>0,"This token has used all the free mint available");
    ...
    //@dev burns the artifact after it is used
    function burnArtifact(uint256 tokenId) external{
         require(msg.sender == enhancementContract, "Calling from invalid address");
        _burn(tokenId);
    }
    
    //@dev If collection 2 is sold out. Remove free mints from all artifacts
    function wipeFreeMints() external{
        require(msg.sender == collectionContract, "Calling from invalid address");
    ...
    function setCollectionContract(address _contractAddress) external onlyOwner{
        collectionContract = _contractAddress;
    }
   ...
   function setDepletedURI(string memory _newDepletedURI) public onlyOwner {
    ...
    function setSpecialURI(string memory _newSpecialURI) public onlyOwner {
