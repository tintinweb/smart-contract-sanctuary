...
contract WAGMIpet is Ownable, ERC721, ERC721Enumerable {
    ...
    constructor() ERC721("WagmiPet", "PET") {
        birth("Genesis");
    }

    function birth(string memory name) public returns (uint256) {
        _tokenIds.increment();
    ...
    function getHunger(uint256 tokenId) public view returns (uint256) {
        return _hunger[tokenId] + ((block.number - _lastFeedBlock[tokenId]) / 50);
    }

    function getUncleanliness(uint256 tokenId) public view returns (uint256) {
        return _uncleanliness[tokenId] + ((block.number - _lastCleanBlock[tokenId]) / 50);
    }

    function getBoredom(uint256 tokenId) public view returns (uint256) {
        return _boredom[tokenId] + ((block.number - _lastPlayBlock[tokenId]) / 50);
    }

    function getSleepiness(uint256 tokenId) public view returns (uint256) {
        return _sleepiness[tokenId] + ((block.number - _lastSleepBlock[tokenId]) / 50);
    }
