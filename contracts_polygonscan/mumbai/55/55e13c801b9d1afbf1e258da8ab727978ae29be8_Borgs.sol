...
contract CommonObjects{
    ...
    struct Layer{
        bool exists;
        string name;
        mapping(uint => BorgAttribute) attributes;
        uint256 borgAttributeSize;
        uint256 borgAttributesCumulativeTotal;
    }
...
contract Borgs is ERC721Enumerable, ERC721Holder, CommonObjects, NumberUtils{
    ...
    mapping (string => BorgAttribute) public _borgAttributes;
    ...
    // The colors used
    mapping(uint8 => string) private _colors;
    ...
    uint256 private _supplyLimit = 20000;
    ...
    function getBorgAttributeBytes(string memory attributeName) public view returns (bytes[] memory){
        return _borgAttributes[attributeName].colors;
    }
    
    function getBorgAttributeBytesLength(string memory attributeName) public view returns (uint256){
        return _borgAttributes[attributeName].colors.length;
    }
    ...
    function getMarketPlaceListingCommission(uint256 price) public view returns(uint256){
    ...
    function getListingByTokenId(uint256 tokenId) public view returns(uint256 borgId, uint256 price, address seller, address buyer, uint256 timestamp, string[] memory image){
        // Get listing
    ...
    function getListingByIndex(uint256 index) public view returns(uint256 tokenId, uint256 price, address seller, address buyer, uint256 timestamp, string[] memory image){
        // Get tokenId
    ...
    function getOwnersListingByIndex(address owner, uint256 index) public view returns(uint256 tokenId, uint256 price, address seller, address buyer, uint256 timestamp, string[] memory image){
        // Get tokenId
    ...
    function addBorgAttributeToLayer(string calldata layerName, uint256 chance, string memory borgAttributeName, bytes[] memory colours) external onlyOwner editable{
        // Get the layer
        ...
        BorgAttribute memory borgAttribute = BorgAttribute(true, colours, chance, borgAttributeName);
        
        // Get current items size
        uint256 position = layer.borgAttributeSize;
        
        // Add onto end
        layer.attributes[position] = borgAttribute;
        
        // Update the item size
        layer.borgAttributeSize = (position + 1);
        
        // Update the cumulative total of chances for the layer
        layer.borgAttributesCumulativeTotal = (layer.borgAttributesCumulativeTotal + chance);
        
        // Set in global attributes
        _borgAttributes[borgAttributeName] = borgAttribute;
    }
    ...
    function generateBorg() external payable usable returns(uint256){
        ...
        else
        {
            ...
            require(_supplyLimit >= (_currentGeneratedCount + FREE_GENERATED_COUNT), "Borg generation limit has been reached");
        }
    ...
    function previewBreedBorgs(uint256 borgId1, uint256 borgId2) external view usable returns(string[] memory image, string[] memory attributes){
        // Get the first borg to breed
        Borg storage borg1 = _borgs[borgId1];
        require(borg1.exists, 'Borg 1 doesnt exist');
        require(borg1.descendantId < 1, 'Borg1 already has a descendant');
        ...
        // Filter out the rareset of the 2 sets of items (layer by layer)
        (uint256[] memory borgAttributeLayerPositions, string[] memory borgAttributeNames) = _filterRarestBorgPeicesForPreview(layerItemPosition1, layerItemPosition2);

        // TODO REMOVE THIS
        borgAttributeLayerPositions[0];
        
        // Create image
        image = _combineBorgAttributes(borgAttributeNames);
        attributes = borgAttributeNames;
    }

    function _filterRarestBorgPeices(uint256[] memory borg1sAttributes, uint256[] memory borg2sAttributes) internal returns(uint256[] memory rarestLayerPositions, string[] memory borgPeiceNames){
        // Check the peices size is the same
        require(borg1sAttributes.length == borg2sAttributes.length, 'Borg layer counts do not match');
        
        // Create the new item arrays
        rarestLayerPositions = new uint256[](borg1sAttributes.length);
        string[] memory rarestBorgAttributeNames = new string[](borg1sAttributes.length);
        ...
        for(uint256 i=0;i<_layerMetaData.length;i++){
            ...
            BorgAttribute storage borg1sAttribute = layer.attributes[borg1sAttributes[i]];
            BorgAttribute storage borg2sAttribute = layer.attributes[borg2sAttributes[i]];
            
            // Compare and take the item with the lowst chance
            if(borg1sAttribute.chance <= borg2sAttribute.chance){
                rarestLayerPositions[i] = borg1sAttributes[i];
                rarestBorgAttributeNames[i] = borg1sAttribute.name;
            }
            else{
                rarestLayerPositions[i] = borg2sAttributes[i];
                rarestBorgAttributeNames[i] = borg2sAttribute.name;
            }
    ...
    function _filterRarestBorgPeicesForPreview(uint256[] memory borg1sAttribute, uint256[] memory borg2sAttribute) internal view returns(uint256[] memory rarestLayerPositions, string[] memory borgPeiceNames){
        // Create the new item arrays
        rarestLayerPositions = new uint256[](borg1sAttribute.length);
        string[] memory rarestBorgAttributeNames = new string[](borg1sAttribute.length);
        ...
        for(uint256 i=0;i<_layerMetaData.length;i++){
            ...
            // Get items from both parents
            BorgAttribute storage borg1Attribute = layer.attributes[borg1sAttribute[i]];
            BorgAttribute storage borg2Attribute = layer.attributes[borg2sAttribute[i]];
            
            // Compare and take the item with the lowst chance
            if(borg1Attribute.chance <= borg2Attribute.chance){
                rarestLayerPositions[i] = borg1sAttribute[i];
                rarestBorgAttributeNames[i] = borg1Attribute.name;
            }
            else{
                rarestLayerPositions[i] = borg2sAttribute[i];
                rarestBorgAttributeNames[i] = borg2Attribute.name;
            }
        }
    ...
    function _combineBorgAttributes(string[] memory borgAttributeNames) internal view returns (string[] memory hexPixals){
        ...
        for(uint256 i=1;i<borgAttributeNames.length;i++){
             BorgAttribute storage borgAttribute = _borgAttributes[borgAttributeNames[i]];
 
             bytes memory colorBytes = borgAttribute.colors[0];
             
             uint32 incrementCounter = 2;
              
             for(uint256 j = 1;j<borgAttribute.colors.length;j+=incrementCounter){
                if(uint16(convertBytesToBytes2(abi.encodePacked(borgAttribute.colors[j], borgAttribute.colors[j + 1]))) == 0){
                    colorBytes = borgAttribute.colors[j+2];
                    incrementCounter = 3;
                }
                else {
                    bytes2 colorStartPositionBytes = convertBytesToBytes2(abi.encodePacked(borgAttribute.colors[j + 1],borgAttribute.colors[j + 2]));
                    bytes memory colorLengthBytes = borgAttribute.colors[j + 3];
                    incrementCounter = 3;
                             
                    uint8 color = uint8(colorBytes[0]);
                    uint16 colorStartPosition = uint16(colorStartPositionBytes);
                    uint8 colorLength = uint8(colorLengthBytes[0]);
                    
                    for(uint32 k = colorStartPosition;k<colorLength;k++){
                        hexPixals[k] = _colors[color];
                    }
                }
            }
        }
    ...
    function convertBytesToBytes2(bytes memory inBytes) public pure returns (bytes2 outBytes8){
        if (inBytes.length == 0) {
            return 0x0;
        }
    ...
    function a(bytes memory value, bytes memory value2) public pure returns(uint16 returnValue){
        bytes memory encodedValues = abi.encodePacked(value[0], value2[0]);
        
        return uint16(convertBytesToBytes2(encodedValues));
    }
    ...
    function _getRandomLayerItemName(string memory layerItemName) internal returns(string memory borgAttributeName, uint256 position){
        ...
        require(layer.borgAttributeSize > 0, "No attributes were found");
        
        // Get a random number from 0-the cumulative total of all layer item chances
        uint256 randomChance = _getRandomNumber(layer.borgAttributesCumulativeTotal);
        ...
        for(uint256 i=0;i<layer.borgAttributeSize;i++){
            // Get the layer item
            BorgAttribute memory attribute = layer.attributes[i];
            
            // add the chance for the item to the running total
            cumulativeChance += attribute.chance;
            
            // If the running total is greater than the random number, we have a winner
            if(cumulativeChance >= randomChance){
                // Set return values
                borgAttributeName = attribute.name;
                position = i + 0;
                
                // Leave loop
                i = layer.borgAttributeSize;
            }
        }
    }
