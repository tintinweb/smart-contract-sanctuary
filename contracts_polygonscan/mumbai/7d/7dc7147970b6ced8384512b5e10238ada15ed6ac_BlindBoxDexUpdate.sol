...
contract BlindBoxDexUpdate is Ownable, DexStorage {
...
function mintWithCollection(address collection, address to, string memory tokenURI, uint256 royalty, string memory ownerId, address from ) isValid(collection) public returns(uint256) {
    ...
    _tokenAuthors[tokenId][address(collection)]._address = msg.sender;
    }
    emit MintWithTokenURI(collection, tokenId, _tokenAuthors[tokenId][address(collection)]._address, tokenURI);
    return tokenId;
  }
