...
contract Governance is AccessControl, ReentrancyGuard {
    ...
    function balanceOf(address user) external view returns (uint256) {
        ...
        return votePower[user].mul(genesis.balanceOf(address(this))).div(totalVotePower);
    }
    ...
    function deposit(uint256 amount) public {
        ...
        uint256 newVotePower = amount;
        if (totalVotePower == 0 || genesisBalance == 0) {
        ...
        balances[voter] = balances[voter].add(amount);
    ...
    function withdraw(uint256 amount) public nonReentrant() {
        ...
        balances[voter] = balances[voter].sub(amount);
    ...
    function voteFor(uint256 proposalId) public {
        ...
        uint256 balance = balances[voter];
        proposal.totalFor = proposal.totalFor.add(balance);
        proposal.votesFor[voter] = balance;
        if (proposal.votesAgainst[voter] > 0) {
            proposal.totalAgainst = proposal.totalAgainst.sub(balance);
    ...
    function voteAgainst(uint256 proposalId) public {
        ...
        if (proposal.votesFor[voter] > 0) {
            proposal.totalFor = proposal.totalFor.sub(balance);
