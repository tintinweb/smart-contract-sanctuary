    ...
    RoyaltiesV1Luxy
{
    ...
    function __ERC1155Luxy_init_unchained(string memory _name, string memory _symbol, string memory baseURI) public initializer {
        name = _name;
        symbol = _symbol;
        __ERC1155Luxy_init_unchained();
        _setBaseURI(baseURI);
    }    
    ...
    function mint(address account, uint256 amount, bytes memory data, Royalties[] memory royalties) public 
    returns (Royalties[] memory){
        uint256 itemId = _tokenIds.current();
        uint256 id =  uint256(uint160(address(this))) + (itemId >> 200); 
        _mint(account, id, amount, data);
        _tokenIds.increment();
        Royalties[] memory royalties = _setRoyalties(id,royalties);
        return royalties;
        //royalties.setRoyalties(20, [{ account: creator1, value: 10 },{ account: creator2, value: 30 },{ account: creator3, value: 20 }]);
    }
