...
// STOOPID SNAKE SOCIETY 2021-v1POLYGON MATIC TEST- 622 NFTs on POLYGON- SSSSSSSSSSSSSSSSSuper Secret SSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSS
...
//Who let these motha fookin snakes on the motha fookin blockchain?!
...
contract StoopidSnakeSocietyTestV1 is ERC721Enumerable, Ownable {
  using Strings for uint256;

  string baseURI;
  string public baseExtension = ".json";
  // Reserved for the team, customs, giveaways, collabs and so on.
  uint256 public reserved = 22;
  uint256 public cost = .005 ether;
  uint256 public maxSupply = 622;
...
for(uint i = 0; i < _mintAmount; i++) {
            uint mintIndex = uint(keccak256(abi.encodePacked(block.timestamp, block.difficulty, msg.sender))) % 600;
            mintIndex = mintIndex + 22;
  ...
  // Limited to a publicly set amount
   ...
   function setAddresses(address[] memory _a) public onlyOwner {
    ...
    function withdrawTeam(uint256 amount) public payable onlyOwner {
        ...
        // 50% Split evenly among the team- 25% for the Snakes on a Chain Game
        require(payable(a1).send(percent * 75/2));
        require(payable(a2).send(percent * 75/2));
