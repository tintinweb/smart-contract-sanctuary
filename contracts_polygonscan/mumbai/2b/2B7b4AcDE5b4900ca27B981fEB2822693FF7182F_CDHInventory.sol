...
EIP712MetaTransaction("Crazy Defence Heroes Inventory", "1.0") {
    ...
    event InventoryPoolInvalid(uint256 cardId, bytes1 poolId, uint256 count);
    ...
    mapping(uint256 => uint256) private tokenBlockNumber; // nft address => minted block number
    ...
    function _generateTokenId(uint256 _cardId) internal returns (uint256) {
        ...
        console.log("Count: %s", _count);
        require(_count == 0, "Card ID Invalid");

        address _pool = availablePools[poolId];
        if(_pool == address(0)){
            emit InventoryPoolInvalid(_cardId, poolId, _count);
            return 0;
        }
        ...
        require(tokenBlockNumber[uint256(tokenId)] == 0, "Token already minted");

        tokenPools[uint256(tokenId)] = _pool;
        (,,,,, string memory _uri) = cardPool.getCard(_cardId);
        uris[uint256(tokenId)] = _uri;
        console.log("Uri: %s", _uri);

        tokenBlockNumber[uint256(tokenId)] = block.number;
    ...
    whenNotPaused {
        ...
        for (uint256 i; i < ids.length; i++) {
            ...
            console.log("All token Ids: %s",ids[i]);
