...
contract TheDropSale is TheDropBase, ITheDropSale {
    using SafeMath for uint256;
    ...
    modifier saleExist(uint256 tokenId) {
        require(tokenCurrentSale[tokenId].owner != address(0), "TheDropSale: Sale does not exist");
        _;
    }

     modifier onlyTokenOwner(uint256 tokenId) {
        require(tokenCurrentSale[tokenId].owner == _msgSender(), "TheDropSale: caller is not the toke owner");
        _;
    }

    modifier saleIsOpen(uint256 tokenId) {
        require(tokenCurrentSale[tokenId].status == SaleStatus.FORSALE, "TheDropSale: Sale is not open");
        _;
    }
    ...
    function listArtwork(uint256 _tokenId, string memory _uri, address payable _creator, uint256 _price, uint256 _editions) public override onlyOwner {
        require(!_isTokenMinted(_tokenId) && !_exists(_tokenId), "TheDropSale: token already minted or listed");
    ...
    function resellArtwork(uint256 _tokenId, uint256 _price) public override{
        require(_isTokenMinted(_tokenId), "TheDropSale: token not minted");
        require(TheDropNFT(tokenContract).getApproved(_tokenId) == address(this), "TheDropSale: token not approved for sale");
        ...
        require(tokenOwner == _msgSender(), "TheDropSale: caller not the token owner");
    ...
    function _createSale(uint256 _tokenId, string memory _uri, address payable _owner, address payable _creator,  uint256 _price, SaleType _type, uint256 _editions) private {
        ...
        require(_price > 0, "TheDropSale: price cannot set to 0");
        require(_editions > 0 && _editions < 10000, "TheDropSale: number of editions should between 1 and 9999");
    ...
    function purchase(uint256 _tokenId) public override payable saleExist(_tokenId) saleIsOpen(_tokenId) {
        ...
        require(msg.value >= tmpSale.price, "TheDropSale: insuficient ether value sent");
        ...
        if(tmpSale.saleType == SaleType.INITIAL) {
            ...
            TheDropNFT(tokenContract).transferFrom(tmpSale.owner, _msgSender(), _tokenId);
        }
    ...
    function _mintToken(uint256 _artworkId, uint256 _tokenEditionId, bool _isOriginal) private {
        ...
        TheDropNFT(tokenContract).createTokenForEdition(_tokenEditionId, tokenUri[_artworkId], tmpSale.owner, tmpSale.buyer, _isOriginal, creatorRate);
    }

    function closeSale(uint256 _tokenId) public override saleExist(_tokenId) saleIsOpen(_tokenId){
        require( tokenCurrentSale[_tokenId].owner == _msgSender() || owner() == _msgSender(), "TheDropSale: caller is not the owner.");
