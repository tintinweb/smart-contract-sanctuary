...
contract VelhallaCardRedeem is ERC1155Supply, Ownable {
    ...
    string public constant name = "Velh1";
...
// public
    ...
    function claimSilverCardBySeries(uint256 _book, uint256 _amount) public returns (bool) {
        require((!silverPaused)&&(publicSilverRedeemStart <= block.timestamp), "the redeem for Silver Card is paused");
    ...
    function checkSilverCardRedeemAvailable (uint256 _book, uint256 _chapter) external view returns (uint256) {
    ...
    function claimGoldCard(uint256 _book, uint256 _amount) public returns (bool) {
        require((!goldPaused)&&(publicGoldRedeemStart <= block.timestamp), "the redeem for Gold Card is paused");
        require(_book <= ((idGoldCardNFTIndex+TOTAL_GOLD_CARD_COPY-ID_GOLD_CARD_START)/TOTAL_GOLD_CARD_COPY), "need to redeem under revealed series");
        ...
        for (uint32 k = 0; k < amount; k++) {
		    idGoldCardNFTIndex = idGoldCardNFTIndex * _book + 1;
    ...
    function checkGoldCardRedeemAvailable (uint256 _book) external view returns (uint256) {
