...
// File: contracts/ROETokenSale.sol
...
contract DBSTokenSale is Pausable, AccessControl{

    IERC20 public DBSToken;
    ...
    event Raised(address buyer, uint amount, uint timestamp);

    constructor(address _0x93caa14b407181c3ba2ba1731ae34040d0954780) {
        _setupRole(DEFAULT_ADMIN_ROLE, _safe);

        minInvestment = 20 * 1e18;
        maxInvestment = 1000 * 1e18;
        DBSToken = IERC20(_0x93caa14b407181c3ba2ba1731ae34040d0954780 );
        tokenRate = 20;
    }
    ...
    function setCapSize(uint _min, uint _max) public validate {
        minInvestment = _min;
        maxInvestment = _max;
    }

    function InvestInBMC() public payable{
        address sender = msg.sender;
        uint value = msg.value;

        require(value >= minInvestment, "Investment Too Low");
        require(value <= maxInvestment, "Whale Shock Safety: Enter a smaller amount");
        require((BMCSold + value * tokenRate) < 150000000 * 1e18, "SALE END: All tokens sold out.");

        payable(_safe).transfer(address(this).balance);
        MaticRaised += value;
    
        DBSToken.mint(sender, value * tokenRate);
        BMCSold += value * tokenRate;

        Sale storage sale = sales[++soldCount];
        sale.buyer = sender;
        sale.amount = value;
