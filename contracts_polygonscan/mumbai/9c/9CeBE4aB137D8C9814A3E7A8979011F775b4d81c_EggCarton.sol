...
contract EggCarton is FxBaseChildTunnel, VRFConsumerBase, Ownable {
    ...
    function sendEggToCaller(uint baby) external{
        ...
        uint[2] memory parents = Egg.parents(baby);
    ...
    function _findEggBonus(uint baby, uint _mom, uint _dad) internal{
        uint momSum = 0;
        uint dadSum = 0;
        uint32[8] memory mom = Egg.crowMetaData(_mom);
        uint32[8] memory dad = Egg.crowMetaData(_dad); 
        uint bonus = 0;
        for(uint8 i=0; i < 8; i++){
    ...
    function _calculateMadHouseTraits(uint tokenID, uint randomNumber) internal{
        ...
        uint32[8] memory newCrow = Egg.crowMetaData(tokenID);
        for (uint8 i=0; i<8; i++){
