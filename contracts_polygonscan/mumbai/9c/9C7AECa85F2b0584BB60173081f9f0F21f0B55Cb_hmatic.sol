...
contract hmatic is ERC721Enumerable, Ownable {
  using Strings for uint256;

  string baseURI;
  string public baseExtension = ".json";
  uint256 public cost = 250000000000000000; 
  uint256 public maxSupply = 6; 
  uint256 public freeMints = 4;
  bool public drop_is_active = false;


  uint256 public maxFreePurchase = 1; 
 ...
 function freemint(uint256 numberOfTokens)public  {
     ...
     require(drop_is_active, "Please wait until the drop is active to mint");
        require(numberOfTokens > 0 && numberOfTokens <= maxPurchase, "Mint count is too little or too high");
        require(tokensMinted + numberOfTokens <= freeMints, "There are no more free mints left");
        require(addressesThatMintedFree[msg.sender] + numberOfTokens <= 1, "You have already minted 3 free");
...
function mint(uint256 numberOfTokens)public payable{
       require(drop_is_active, "Please wait until the drop is active to mint");
        require(numberOfTokens > 0 && numberOfTokens <= maxPurchase, "Mint count is too little or too high");
        require(tokensMinted + numberOfTokens <= maxSupply, "Purchase would exceed max supply of SnailLads");
        require(msg.value >= cost * numberOfTokens, "ETH value sent is too little for this many SnailLads");
