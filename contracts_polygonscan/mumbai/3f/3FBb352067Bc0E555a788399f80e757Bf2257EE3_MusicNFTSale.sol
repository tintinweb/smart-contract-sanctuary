...
//TODO: all payments should be claimable (pull system) - check with client (gas efficiency)
//TODO: see if it's worth dropping saleId and just use the UUID
...
contract MusicNFTSale is ISale, ERC1155Receiver, Ownable {
  ...
  // event SaleStatusChanged(uint256 saleId, Status status);
  // event Purchase(
  //   uint256 saleId, 
  //   address purchaser, 
  //   address recipient, 
  ...
  // event FundsClaimed(uint256 claimant, address token, uint256 amount);
  ...
  function getSaleDetails(uint256 saleId) external view returns(Sale memory) {
    // requires to not return nonexistent
    return sales[saleId];
  }

  function getSaleStatus(uint256 saleId) external view returns(Status) {
    // requires to not return nonexistent
  ...
  // do we need whitelisting for creating a sale/auction?
  // at least the contract should be validated in the registry
  // times need to be validated
  function createSale(
    address nftContract,
    uint256 tokenId,
    uint256 amount,
    ...
    // check that NFT is 2981 compliant?
    ...
    sales[saleId].id = saleId;
    sales[saleId].owner = msg.sender;
    sales[saleId].nftContract = nftContract;
    ...
    sales[saleId].maxBuyAmount = maxBuyAmount;
    sales[saleId].currency = currency;
    sales[saleId].status = currentStatus;
    sales[saleId].dbId = dbId;
  ...
  // need checks that enough NFTs remain to fill purchase
  function buy(uint256 saleId, address recipient, uint256 amount) external payable returns(bool) {
    require(block.timestamp >= sales[saleId].startTime, "sale is not yet active");
    require(block.timestamp < sales[saleId].endTime, "sale has ended");
    require(sales[saleId].status != Status.CANCELLED, "sale is cancelled");
    if(sales[saleId].status == Status.PENDING) sales[saleId].status = Status.ACTIVE;
    require(amount <= sales[saleId].maxBuyAmount, "buy quantity too high");
    // should the next check revert or buy the rest of the stock?
    require(amount <= sales[saleId].amount - sales[saleId].purchased,
      "not enough stock for purchase");

    uint256 fundsToPay = sales[saleId].price * amount;
    IMusicNFT Nft = IMusicNFT(sales[saleId].nftContract);
    (address artistAddress, uint256 royalties) = Nft.royaltyInfo(sales[saleId].tokenId, fundsToPay);

    if(sales[saleId].currency != ETH) {
      IERC20 Token = IERC20(sales[saleId].currency);

      Token.safeTransferFrom(msg.sender, address(this), fundsToPay);
    } else {
      require(msg.value >= sales[saleId].price * amount, "insufficient ETH sent");
    }

    // system fee
    (address systemWallet, uint256 fee) = Registry.feeInfo(fundsToPay);
    fundsToPay -= fee;
    claimableFunds[systemWallet][sales[saleId].currency] += fee;

    // artist royalty if artist isn't the seller
    if(sales[saleId].owner != artistAddress){
      fundsToPay -= royalties;
      claimableFunds[artistAddress][sales[saleId].currency] += royalties;
    }

    // seller gains
    claimableFunds[sales[saleId].owner][sales[saleId].currency] += fundsToPay;
    ...
    // if stock sells out, end sale
    if(sales[saleId].purchased == sales[saleId].amount) {
      sales[saleId].status = Status.ENDED;
    }

    emit Purchase(saleId, msg.sender, recipient, amount);

    return true;
  }

  function claimNfts(uint256 saleId) external {
    require(sales[saleId].status == Status.CANCELLED ||
      sales[saleId].status == Status.ENDED,
  ...
  //TODO: I think this could be refactored out
  function updateSaleStatus(uint256 saleId) external {
    require(sales[saleId].status != Status.CANCELLED,
      "sale is cancelled");
    require(sales[saleId].status != Status.ENDED,
        "sale has already ended");
    if(
      block.timestamp >= sales[saleId].startTime &&
      block.timestamp < sales[saleId].endTime
    ) {
      require(sales[saleId].status == Status.PENDING,
        "sale is already active or ended");
      sales[saleId].status = Status.ACTIVE;
    }
    if(block.timestamp >= sales[saleId].endTime) {
      sales[saleId].status = Status.ENDED;
    }

    emit SaleStatusChanged(saleId, sales[saleId].status);
  }
...
pragma solidity ^0.8.9;
 ...
 //TODO: artist can cancel sales in middle, reclaim unsold inventory
...
interface ISale {
  ...
  event NFTsReclaimed(uint256 indexed id, address indexed owner, uint256 indexed amount);
  event FundsClaimed(address claimant, address token, uint256 amount);
