...
contract BorgsGiveaway is ContractStates, NumberUtils{
    ...
    function payPeople(uint256 seed) onlyOwner active public {
       ...
       // Get a random number
       uint256 randomNumber = _getRandomNumber(seed, 1000000);

       for(uint256 i = 0; i < balance; i++){
           // Get the a random position to find who to reward
           randomNumber = _getRandomNumber(randomNumber, 1000000);
    ...
    function payPeopleTest(uint256 seed) active public view returns (uint256[] memory response, address[] memory addresses){
       // Check there are addresses to pay
       require(_addresses.length > 0, "There is noone to payout to");
       ...
       response = new uint256[](balance);
       addresses = new address[](balance);
