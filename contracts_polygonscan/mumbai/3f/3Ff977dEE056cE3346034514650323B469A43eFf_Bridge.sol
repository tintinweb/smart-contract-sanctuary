...
contract Bridge is Ownable, ReentrancyGuard {
    ...
    function createOrder(uint256 amount, uint256 chainId, uint256 destination, uint256 srcToken, uint256 srcVia, uint256 destToken, uint256 destVia) external {
        address[] memory path;
        ...
        orders[nextOrderId] = Order(nextOrderId, msg.sender, amount, chainId, destination, destToken, destVia);
    ...
    function completeOrder(uint256 orderId, uint256 chainId, address payable destination, uint256 amount, uint256 destToken, uint256 destVia) internal {
        address[] memory path;
    ...
    function completeOrders(CompleteParams[] calldata params) external onlyOperator {
        for (uint i = 0; i < params.length; i++) {
            completeOrder(params[i].orderId, params[i].chainId, params[i].destination, params[i].amount, params[i].destToken, params[i].destVia);
        }
    }
