...
/// @title LennyTimelock
/// @notice  This contract is used for Liquidity Security
contract LennyTimelock is Context, Democratic {
    ...
    /// @param _beneficiary_ public address of the beneficiary
    /// @param _releaseTime_ timestamp at which token locked will be unlocked
    /// @notice All instantiation are immutable.
    constructor(
        address _doscAddress,
        address _beneficiary_,
        uint256 _releaseTime_
    ) Democratic(_doscAddress) {
        require(_releaseTime_ > block.timestamp, "TokenTimelock: release time is before current time");
        require(_doscAddress != address(0), 'not a zero Address');
        require(_beneficiary_ != address(0), 'not a zero Address');

        _beneficiary = _beneficiary_;
        _releaseTime = _releaseTime_;
    }

    /// @param newAddress address of the token locked
    /// @notice This function is restricted to democratic process
    function changeTokenAddress(address newAddress) external demokratia() {
        require(newAddress != address(0), 'not a zero address');
        tokenAdd = newAddress;
    }
    ...
    /// @return the address of the release timestamp
    function releaseTime() public view virtual returns (uint256) {
        return _releaseTime;
    }

    /// @dev this function send the token locked in the smart contract to the beneficiary
    function release() external virtual {
        require(block.timestamp >= releaseTime(), "TokenTimelock: current time is before release time");

        IERC20 token = IERC20(getTokenAddress());
