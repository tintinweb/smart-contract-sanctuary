...
contract CanvasManager is Context, AccessControl, Ownable, ICanvas, ReentrancyGuard {
    ...
    uint256 public maxPix = 1200; // max consumed tokens
    ...
    modifier checkNftId(uint256 nftId) {
        require(nftId <= mNftId && nftId > 0, "wrong nftId");
        _;
    }
    ...
    function setDefaultAuctionCurrency(address currencyAddress) external onlyAdmin {
        // TODO: verify currencyAddress is ERC20
        _defaultAuctionCurrency = currencyAddress;
    }

    /// @return auctionAddress address of auction contract which tokenId is nftId
    function artAuctionAddress(uint256 nftId) external view checkNftId(nftId) returns (address auctionAddress) {
        auctionAddress = address(artAuctions[nftId]);

        require(auctionAddress != address(0), "art is not end");
    }
    ...
    function contribution(uint256 nftId, address contributor) external view checkNftId(nftId) returns (uint256) {
    ...
    function withdrawAdmin(
        ...
        require(tokenIndex < mTokens.length, "tokenIndex is too big");
        ...
        if (amount == 0) {
            ...
            require(amount <= balance, "amount is too big");
        }
    ...
    function artAuctionId(uint256 nftId) external view checkNftId(nftId) returns (uint256) {
        IArtAuction artAuction = _getArtAuction(nftId);
        return artAuction.artAuctionId();
    }

    function auctionCurrency(uint256 nftId) external view checkNftId(nftId) returns (address) {
        IArtAuction artAuction = _getArtAuction(nftId);
        return artAuction.auctionCurrency();
    }

    function setCurrency(uint256 nftId, address newCurrency) external onlyAdmin checkNftId(nftId) {
        IArtAuction artAuction = _getArtAuction(nftId);
    ...
    ) external onlyAdmin checkNftId(nftId) returns (uint256) {
        IArtAuction artAuction = _getArtAuction(nftId);
    ...
    function createArtAuctionAndApproval(
        ...
        uint256 reservePrice,
        uint8 curatorFeePercentage
    ) external onlyAdmin checkNftId(nftId) returns (uint256 auctionId) {
        IArtAuction artAuction = _getArtAuction(nftId);
    ...
    function artAuctionApproval(uint256 nftId) external view checkNftId(nftId) returns (bool) {
        IArtAuction artAuction = _getArtAuction(nftId);
        return artAuction.artAuctionApproval();
    }

    function setArtAuctionApproval(uint256 nftId, bool approved) external onlyAdmin checkNftId(nftId) {
        IArtAuction artAuction = _getArtAuction(nftId);
    ...
    function artAuctionReservePrice(uint256 nftId) external view checkNftId(nftId) returns (uint256) {
        IArtAuction artAuction = _getArtAuction(nftId);
        return artAuction.artAuctionReservePrice();
    }

    function setArtAuctionReservePrice(uint256 nftId, uint256 reservePrice) external onlyAdmin checkNftId(nftId) {
        IArtAuction artAuction = _getArtAuction(nftId);
        artAuction.setArtAuctionReservePrice(reservePrice);
    }

    function cancelArtAuction(uint256 nftId) external onlyAdmin checkNftId(nftId) {
        IArtAuction artAuction = _getArtAuction(nftId);
        artAuction.cancelArtAuction();
    }

    function endArtAuction(uint256 nftId) external onlyAdmin checkNftId(nftId) {
        IArtAuction artAuction = _getArtAuction(nftId);
     ...
     */
    function claimeArtAuction(uint256 nftId, address contributor) external nonReentrant checkNftId(nftId) {
        IArtAuction artAuction = _getArtAuction(nftId);
        ...
        require(share > 0, "contributor has no share");
    ...
    function addConsumeToken(IERC20 addr) public onlyAdmin {
        mTokens.push(addr);
    }

    function setMaxPix(uint256 i) public onlyAdmin {
        maxPix = i;
    }

    function startNewArt(uint256 endTime) public onlyAdmin returns (uint256 nftId) {
        require(mNftId < MaxArt, "can't create art now");
        require(block.timestamp < endTime, "wrong endTime");
    ...
    function setEndTime(uint256 nftId, uint256 endTime) public onlyAdmin checkNftId(nftId) {
        Art storage art = mArts[nftId];
        require(art.state == 1, "art is not open");
        require(art.startTime < endTime, "endTime is too small");
        art.endTime = endTime;
    ...
    function endArt(uint256 nftId) public checkNftId(nftId) {
        Art storage art = mArts[nftId];
        require(art.state == 1, "art is not open");
        require(block.timestamp >= art.endTime || art.consumToken > maxPix, "art can't end now");
    ...
    function getOpenArts() public view returns (uint256[] memory nftIds) {
        uint256 index = 0;
    ...
    function getEndArts() public view returns (uint256[] memory nftIds) {
        uint256 index = 0;
        for (uint256 i = 1; i <= mNftId; ++i) {
            if (mArts[i].state == 2) {
                index++;
    ...
    function getAllArts() public view returns (uint256[] memory nftIds) {
        uint256 index = 0;
        for (uint256 i = 1; i <= mNftId; ++i) {
            if (mArts[i].state > 0) {
                index++;
    ...
    function getDrawPrice(uint256 nftId, uint256 index) public view checkNftId(nftId) returns (uint256 price) {
        require(index < PixCount, "must less 1024");
    ...
    function drawPixel(
        ...
        uint24 color,
        uint256 bidPrice
    ) public checkNftId(nftId) {
        require(index < PixCount, "must less 1024");
        require(tokenIndex < mTokens.length);

        Art storage art = mArts[nftId];
        require(art.state == 1, "art is not open");
        require(block.timestamp <= art.endTime, "art exceeds end time");

        uint256 price = _newPrice(art.price[index]);
        IERC20 token = mTokens[tokenIndex];
        require(token.balanceOf(_msgSender()) >= price, "insufficient balance");
        require(bidPrice >= price, "bidPrice is too small");
    ...
    function _contribution(uint256 nftId, address contributor) internal view returns (uint256) {
        Art storage art = mArts[nftId];
        require(art.state == 2, "art is not end");
    ...
    function _getArtAuction(uint256 nftId) internal view returns (IArtAuction) {
        require(address(artAuctions[nftId]) != address(0), "artAuctions is not exist");
...
contract ZoraAuction is IArtAuction, IERC721Receiver, Ownable {
    ...
    constructor(
        ...
        require(nftTokenId != 0, "nftTokenId is zero");
        require(totalShares != 0, "totalShares is zero");
        require(nftAddress != address(0), "nftAddress is zero");
        require(marketAddress != address(0), "marketAddress is zero");
    ...
    modifier auctionNotStarted() {
        ...
        require(tokenOwner == address(this), "auction is started");
        _;
    }
    ...
    modifier auctionInProcess() {
        address tokenOwner = IERC721(_nftContract).ownerOf(_nftId);
        require(tokenOwner == _marketAddress, "auction is not in process");
        _;
    }
    ...
    function setCurrency(address newCurrency) external onlyOwner auctionNotStarted {
        require(newCurrency != address(0), "newCurrency is zero");
    ...
    function createArtAuction(
        ...
        require(reservePrice > 0, "reservePrice is zero");
    ...
    function setArtAuctionApproval(bool approved) external onlyOwner auctionInProcess returns (bool) {
        require(_auctionId > 0, "auctionId is zero");
    ...
    function cancelArtAuction() external onlyOwner {
        address tokenOwner = IERC721(_nftContract).ownerOf(_nftId);
        require(tokenOwner == _marketAddress, "token is not in market");
    ...
    function claimeArtAuction(address contributor, uint256 share) external onlyOwner {
        address tokenOwner = IERC721(_nftContract).ownerOf(_nftId);
        require(tokenOwner != address(this) && tokenOwner != _marketAddress, "auction is not end");
        ...
        require(paymentSum > alreadyPaid, "no more payment");
...
abstract contract ERC721Tradable is ContextMixin, ERC721Enumerable, NativeMetaTransaction, Ownable, ERC721Burnable, AccessControl {
    ...
    modifier onlyPix() {
        require(hasRole(PIX_ROLE, _msgSender()), "only pixel");
        _;
    }
