...
contract ERC1155Tradable is ERC1155, ERC1155Burnable, Ownable {
  ...
  event PunkComposed(address indexed account);
  ...
  function balancesOf (
    ...
    uint candy1 = balanceOf(account, 1);
    uint candy2 = balanceOf(account, 2);
    uint candy3 = balanceOf(account, 3);
    uint candy4 = balanceOf(account, 4);
    return [candy1, candy2, candy3, candy4];
  }

  function compose () public {
    for (uint i=0; i<4; i++) {
      require(balanceOf(msg.sender, i)>0, "lack some candy");
      _burn(msg.sender, i, 1);
      tokenSupply[i] --;
    }
    composedPunk[msg.sender] ++;
    emit PunkComposed(msg.sender);
  }
