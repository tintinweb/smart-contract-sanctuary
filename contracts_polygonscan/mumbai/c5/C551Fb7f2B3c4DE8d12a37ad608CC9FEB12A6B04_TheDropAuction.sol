...
contract TheDropNFT is ITheDropNFT, ERC721Enumerable, Ownable, CreatorRole {
    ...
    constructor() ERC721("NFTMUATAug", "NFTMUATAUG") {
    }
...
abstract contract TheDropBase  is ITheDropBase, Ownable {
    ...
    function _isTokenMinted(uint256 tokenId) internal view returns (bool) {
        return TheDropNFT(tokenContract).exists(tokenId);
    }
    ...
    function _getTokenResellRate(uint256 _tokenId) internal virtual view returns (uint256) {
        return TheDropNFT(tokenContract).tokenSellRate(_tokenId);
    }
...
contract TheDropAuction is TheDropBase, ITheDropAuction {
    ...
    constructor(address _tokenContract, address _platformwallet)  TheDropBase(_tokenContract, _platformwallet) {
    }

    modifier onlyTokenOwner(uint256 tokenId) {
        require(tokenCurrentAuction[tokenId].owner == _msgSender(), "TheDropAuction: caller is not the toke owner");
        _;
    }

    modifier auctionExist(uint256 tokenId) {
        require(tokenCurrentAuction[tokenId].owner != address(0), "TheDropAuction: Auction does not exist");
        _;
    }

    modifier auctionIsOpen(uint256 tokenId) {
        require(tokenCurrentAuction[tokenId].status == AuctionStatus.FORAUCTION, "TheDropAuction: Auction is not open");
        _;
    }
    ...
    function setPrice(uint256 _tokenId, uint256 _price) public override auctionExist(_tokenId) auctionIsOpen(_tokenId) onlyTokenOwner(_tokenId) {
        require(tokenCurrentAuction[_tokenId].currentBuyer == address(0), "TheDropAuction: Bid started, price cannot be changed");
    ...
    function listArtwork(uint256 _tokenId, string memory _uri, address payable _creator, uint256 _price) public override onlyOwner {
        require(!_isTokenMinted(_tokenId) && !_exists(_tokenId), "TheDropAuction: token already minted or listed");
    ...
    function resellArtwork(uint256 _tokenId, uint256 _price) public override{
        require(_isTokenMinted(_tokenId), "TheDropAuction: token not minted");
        require(TheDropNFT(tokenContract).getApproved(_tokenId) == address(this), "TheDropAuction: token not approved for auction");
        address payable creator = payable(TheDropNFT(tokenContract).creatorOf(_tokenId));
        address payable tokenOwner = payable(TheDropNFT(tokenContract).ownerOf(_tokenId));
        require(tokenOwner == _msgSender(), "TheDropAuction: caller not the token owner");
        string memory uri = TheDropNFT(tokenContract).tokenURI(_tokenId);
    ...
    function _createAuction(uint256 _tokenId, string memory _uri, address payable _creator, address payable _owner, uint256 _price, SaleType _type) private {
        require(_creator != address(0), "TheDropAuction: token owner address cannot be 0");
        require(_price > 0, "TheDropSale: price cannot set to 0");
    ...
    function acceptBid(uint256 _tokenId) public override auctionExist(_tokenId) auctionIsOpen(_tokenId) onlyOwner{
        ...
        if(tmpAuction.auctionType == SaleType.INITIAL) {
            ...
            TheDropNFT(tokenContract).transferFrom(tmpAuction.owner, tmpAuction.currentBuyer, _tokenId);
        }
    ...
    function _mintToken(uint256 _tokenId) private {
        Auction storage tmpAuction = tokenCurrentAuction[_tokenId];
        TheDropNFT(tokenContract).createTokenFor(_tokenId, tokenUri[_tokenId], tmpAuction.owner, tmpAuction.currentBuyer, creatorRate);
