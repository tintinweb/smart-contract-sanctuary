...
contract MaticPoxs is Ownable, ERC721Enumerable, ReentrancyGuard {
  ...
  uint256 private _price = 0.000003 ether; // 30000000000000000

  event punkNameChange(address _by, uint _tokenId, string _name);

  
  
  constructor() ERC721("MaticPoxs", "MTPX") {}


  uint256[200] private _availableTokens;
  uint256 private _numAvailableTokens = 200;
  uint256 private _numFreeRollsGiven = 0;

  mapping(address => uint256) public freeRollPunks;

  uint256 private _lastTokenIdMintedInInitialSet = 200;
