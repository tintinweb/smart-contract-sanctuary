...
contract SlumStaking is ERC721Holder, ERC1155Holder, Ownable {
	...
	event BurnSlumbos(address indexed sender, uint256[] slumboIds, uint256 reward);
	event BurnSlumdoges(address indexed sender, uint256[] slumboIds, uint256 reward);
	...
	// verified
	function burnSlumbos(uint256[] calldata _slumboIds) external {
		require(burnts[msg.sender][0] + _slumboIds.length <= MAX_BURNED_TICKETS, 'Burn: Can not burn more than 3 Slumboginis');

		uint256 reward = SLUMBO_BURN_REWARD * _slumboIds.length;
		for (uint256 i = 0; i < _slumboIds.length; i++) {
			ISBOToken.safeTransferFrom(msg.sender, BURN_ADDRESS, _slumboIds[i]);
			burnts[msg.sender][0] += 1;
		}
		ISLCToken.transfer(msg.sender, reward);
		emit BurnSlumbos(msg.sender, _slumboIds, reward);
	}

	// verified
	function burnSlumdoges(uint256[] calldata _slumdogeIds) external {
		require(burnts[msg.sender][1] + _slumdogeIds.length <= MAX_BURNED_TICKETS, 'Burn: Can not burn more than 3 SlumdogeBillionaires');

		uint256 reward = SLUMDOGE_BURN_REWARD * _slumdogeIds.length;
		for (uint256 i = 0; i < _slumdogeIds.length; i++) {
			ISDBToken.safeTransferFrom(msg.sender, BURN_ADDRESS, _slumdogeIds[i]);
			burnts[msg.sender][1] += 1;
		}
		ISLCToken.transfer(msg.sender, reward);
		emit BurnSlumdoges(msg.sender, _slumdogeIds, reward);
	}
