...
contract DaysWTFToken is IDaysWTFToken, Ownable, ERC721Checkpointable {
    // address of founders DAO
    address payable public foundersDAO;

    // The Days.WTF token URI descriptor
    IDaysWTFDescriptor public descriptor;

    // Store data for days tokens
    mapping(uint256 => DayData) public daysData;

    // Current count of days at certain levels
    uint256[7] public levelSupply;

    // An address who has permissions to mint Days
    ...
    // The internal day ID tracker
    uint256 private _currentDayId;
    ...
    modifier onlyDayController(uint256 dayId) {
        require(_isApprovedOrOwner(_msgSender(), dayId), 'Only the owner can change the name.');
        _;
    }
    ...
    constructor(
        ...
        address _minter,
        IDaysWTFDescriptor _descriptor,
        IProxyRegistry _proxyRegistry
    ) ERC721('Days.WTF', 'DAY') {
        require(_foundersDAO != address(0), 'Founders DAO address cannot be 0');
        require(_minter != address(0), 'Minter address cannot be 0');

        foundersDAO = payable(_foundersDAO);
        minter = _minter;
    ...
    /**
        @notice Return scarcity level of days.
     */
    function getLevelScarcity(DayLevel level) public pure returns (uint256) {
        if (level == DayLevel.MILLENNIUM) return 4;
        if (level == DayLevel.CENTURY) return 20;
        if (level == DayLevel.DECADE) return 80;
        if (level == DayLevel.LEAP) return 200;
        if (level == DayLevel.YEAR) return 400;
        if (level == DayLevel.MONTH) return 800;
        return 8496;
    }

    /**
     *  @notice Returns the founders DAO address.
        @return address - The address of the founders DAO.
     */
    function getFoundersDAO() external view override returns (address) {
        return foundersDAO;
    }

    /**
        @notice Returns the descriptor.
     */
    function getDescriptor() external view override returns (IDaysWTFDescriptor) {
        return descriptor;
    }
    ...
    /**
        @notice Convert timestamp to dayId
     */
    function timestampToDayId(uint256 timestamp) public view override returns (uint256) {
        return descriptor.timestampToDayId(timestamp);
    }

    function stringToDayId(string memory dayString) public view override returns (uint256) {
        return descriptor.stringToDayId(dayString);
    }

    /**
        @notice Get the level for a dayId
     */
    function getDayLevel(uint256 dayId) public view override returns (uint256) {
        return uint256(descriptor.getDayLevel(dayId));
    }

    /**
     * @notice Mint a Day to the minter.
     * @dev Call _mintTo with the to address(es).
     */
    function mint(uint256 _dayId, IDaysWTFAuctionHouse.MintType _type) public override onlyMinter returns (uint256) {
        return _mintTo(minter, _dayId, _type);
    }

    /**
     * @notice Burn a day.
     */
    function burn(uint256 dayId) public override onlyMinter {
        // burn the token
        _burn(dayId);

        // decrease the level supply
        levelSupply[uint256(descriptor.getDayLevel(dayId))]++;

        emit DayBurned(dayId);
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        require(_exists(tokenId), 'DaysWTFToken: URI query for nonexistent token');
        return descriptor.tokenURI(tokenId, daysData[tokenId]);
        //return string(abi.encodePacked("https://days.wtf/day/", Strings.toString(tokenId)));
    }
    ...
    function dataURI(uint256 tokenId) public view override returns (string memory) {
        require(_exists(tokenId), 'DaysWTFToken: URI query for nonexistent token');
        return descriptor.dataURI(tokenId, daysData[tokenId]);
        //return string(abi.encodePacked("https://days.wtf/day/", Strings.toString(tokenId)));
    }
     ...
     * @dev Only callable by the owner when not locked.
     */
    function setDescriptor(IDaysWTFDescriptor _descriptor) external override onlyOwner whenDescriptorNotLocked {
        require(address(_descriptor) != address(0), 'Descriptor must not be null');
    ...
    /**
     * @notice Mint a Day with `dayId` to the provided `to` address.
     */
    function _mintTo(
        address to,
        uint256 dayId,
        IDaysWTFAuctionHouse.MintType _type
    ) internal returns (uint256) {
        // check level scarcity
        uint256 level = uint256(descriptor.getDayLevel(dayId));
        require(
            _type == IDaysWTFAuctionHouse.MintType.TODAY ||
                getLevelScarcity(DayLevel(level)) > (10000 * levelSupply[level]) / totalSupply(),
            "Can't mint over the scarcity limit"
        );
        levelSupply[level]++;

        // mint
        _mint(owner(), to, dayId);

        // save seed information
        daysData[dayId].seed = descriptor.getSeed(dayId, _type);

        emit DayMinted(dayId);

        return dayId;
    }

    function getDayName(uint256 dayId) public view override returns (string memory) {
        return string(abi.encodePacked(daysData[dayId].name));
    }

    function getDayUrl(uint256 dayId) public view override returns (string memory) {
        return daysData[dayId].url;
    }

    function getDayHash(uint256 dayId) public view override returns (string memory) {
        return daysData[dayId].hash;
    }

    function getDaySeed(uint256 dayId) public view override returns (uint256) {
        return daysData[dayId].seed;
    }
    ...
    {
        return (
            string(abi.encodePacked(daysData[dayId].name)),
            daysData[dayId].url,
            daysData[dayId].hash,
            daysData[dayId].seed
        );
    ...
    function changeDayName(uint256 dayId, string memory _newName) external override onlyDayController(dayId) {
        bytes memory name = bytes(_newName);
        require(name.length <= 32, 'Name must be 32 bytes or less.');

        daysData[dayId].name = bytes32(name);

        emit DayDataChanged(dayId, 'name', _newName);
    }

    function changeDayUrl(uint256 dayId, string memory newUrl) external override onlyDayController(dayId) {
        require(bytes(newUrl).length <= 128, 'Url must be 128 bytes or less.');

        daysData[dayId].url = newUrl;

        emit DayDataChanged(dayId, 'url', newUrl);
    }

    function changeDayHash(uint256 dayId, string memory newHash) external override onlyDayController(dayId) {
        require(bytes(newHash).length <= 128, 'Hash must be 128 bytes or less.');

        daysData[dayId].hash = newHash;

        emit DayDataChanged(dayId, 'hash', newHash);
    }
