...
contract Jackpot is Ownable,ERC721 {
    ...
    function mint(address to) external payable returns(uint){
        require(ids < maxSupply, "cap reached");
        require(msg.value >= PRICE, "not enough funds supplied");
        uint newid = ids;
        _safeMint(to, newid);
        string memory baseURI = _baseURI();
        string memory newuri = string(abi.encodePacked(baseURI,metadataURI,"/", Strings.toString(newid),".json"));
        emit PermanentURI(newuri,newid);
        ids += 1;
        return newid;
    }

    function getPrice(uint price_) public returns (uint) {
        return PRICE;
    }
    ...
    function setMaxSupply(uint amount_) public onlyOwner {
        maxSupply = amount_;
    }
