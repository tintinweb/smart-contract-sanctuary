...
contract DEMONft721 is ERC721 {
    ...
    function mintToken() public {
        ...
        tokenIndex = randomGen.getRandomNumber();
        tokenIndex = indexExists(tokenIndex);
        _safeMint(msg.sender, tokenCounter);
        tokens[tokenCounter] = tokenIndex;
        tokenCounter += 1;
        emit minted(address(0), msg.sender, tokenId, tokenIndex);
    }

    function indexExists(uint256 _tokenIndex) internal virtual returns(uint256){
        if(existIndex[_tokenIndex]){
            _tokenIndex = randomGen.getRandomNumber();
            indexExists(_tokenIndex);
        }
    ...
    function getAssignedIndex() public view returns (uint256[] memory){
        ...
        for(uint256 i = 0; i < tokenCounter; i++){
            assginedTokenToIndex[i] = tokens[i];
        }
