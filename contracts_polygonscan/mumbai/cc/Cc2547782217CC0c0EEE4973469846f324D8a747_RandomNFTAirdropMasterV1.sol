...
// File: RandomNFTAirdropMasterV1_flat.sol
...
interface IRandomNFTAirdropMasterV1 {
     ...
     */
    event DrawEntrantAdded(bytes32 drawUUID, address entrant);
    /**
...
interface IRandomNFTAirdropDrawV1 {
     ...
     */
    function getERC721RewardInfo(uint256)
        external
        view
     ...
     */
    function assignERC721Reward(uint256, uint256) external returns (address);

    /**
     * Sends an ERC721 reward to a winner
     * Reward must be assigned to the caller and status assigned (not claimed)
     */
    function claimERC721Reward(uint256) external returns (address);
     ...
     */
    function addEntrant(address) external;
    ...
    IRandomNFTAirdropMasterV1
{
     ...
     */
    function addEntrantToDraw(bytes32 drawUUID, address entrant)
        public
        onlyRole(WORKER_ROLE)
    {
        IRandomNFTAirdropDrawV1(getDrawAddress(drawUUID)).addEntrant(entrant);
        emit DrawEntrantAdded(drawUUID, entrant);
    }
    ...
    function _assignERC721Winner(
        ...
        emit DrawERC721RewardAssigned(
            ...
            IRandomNFTAirdropDrawV1(getDrawAddress(drawUUID))
                .assignERC721Reward(index, winnerIndex)
        );
    }
