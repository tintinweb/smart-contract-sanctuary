...
contract Exchange is ReentrancyGuard, Ownable {
	...
	****************************************/
	...
	{
		uint256 _borrowFee = liquidityPool.borrowFee();
		uint256 _outAmountNorm = outAmount_.mul(liquidityPool.TOKENS_MUL(outTokenIndex_));
		uint256 _inAmountNorm = _outAmountNorm.mul(pctPrecision.add(_borrowFee)).div(pctPrecision);
		return _inAmountNorm.div(liquidityPool.TOKENS_MUL(inTokenIndex_));
	}
