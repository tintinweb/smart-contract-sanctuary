...
interface ILexitNFTVault {
    ...
    function getIsBlackListed(address) external view returns (bool);

    function setIsBlackListed(address _user, bool status) external;
...
contract LexitNFTVault is ERC721, Ownable, ILexitNFTVault {
    ...
    modifier _validateTokenOwner(uint256 _tokenId, address _sender) {
        require(isBlackListed[_sender] != true, "YOU ARE BLACKLISTED!");
        require(
    ...
    modifier _validateOfferer(
        ...
        require(status[_tokenId]==2 || status[_tokenId]==3 , "This token is not on auction");
        require(
            contractForAccess[_msgSender()],
            "This contract does not have access"
        );
    ...
    constructor(string memory _baseURI) public ERC721("LEXITNFT", "LEXINFT") {
    ...
    function setStatus(uint256 _id, uint8 _status) external override _hasAccess {
       status[_id]=_status;
    }
    ...
    function getIsBlackListed(address _user)
        external
        view
        override
        returns (bool)
    ...
    function setIsBlackListed(address _user, bool _status)
        external
        override
        onlyOwner
    {
