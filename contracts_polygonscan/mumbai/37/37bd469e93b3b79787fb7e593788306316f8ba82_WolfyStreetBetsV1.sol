...
contract WolfyStreetBetsV1 is Ownable, ReentrancyGuard, BaseRelayRecipient {
    ...
    uint256 public HRWinFactor;
    uint256 public LRWinFactor; 
    ...
    */
    function setLRWinFactor(uint256 _LRWinFactor) public onlyOwner {
        require(poolStarted == false, "Pool has already been started!");
        LRWinFactor =  _LRWinFactor;
    }

    /**
    * @dev Sets win requirement for high risk (HR) predictions as a multiplier.
    */
    function setHRWinFactor(uint256 _HRWinFactor) public onlyOwner {
        require(poolStarted == false, "Pool has already been started!");
        HRWinFactor = _HRWinFactor;
    }
    ...
    * @param _champ win or loss (bool)
    */
    function checkOwnerBalance(uint256 _factor, bool _champ) internal {
        uint256 totalWinLowRisk;
        uint256 totalWinHighRisk;

        if (_factor >= LRWinFactor && _champ == true) {
            // LR | PROFIT => +12%
        ...
        if (_factor >= HRWinFactor && _champ == true) {     
            // HR | PROFIT => +30%
    ...
    */
    function rewardManager(uint256 _factor, bool _champ) internal {      
        checkOwnerBalance(_factor,_champ);
        // LR wl
        if (_factor >= LRWinFactor && _champ == true) {
            // LR | PROFIT => +12%
            for (uint i = 0 ; i < _lowRiskUsers.length ; i++) {           
    ...
    * @param _inputPredictionAsset1 current price of asset 1
    * @param _inputpredictionAsset2 current price of asset 2
    ...
    */
    function getTotalStaked() external view returns (uint256) {
        return depositLowRisk.add(depositHighRisk); 
    }
