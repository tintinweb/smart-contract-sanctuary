...
pragma solidity ^0.8.0;
contract DomNFT is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    function isWhitelisted(address _sender) public view returns(bool) {
        return whitelist.isWhitelisted(_sender);
    }
    function viewAPR() public view returns(uint256) {
        return apr;
    }
    function viewROIRate() public view returns(uint256) {
        return roiRate;
    }
    //only owner
    ...
    function uncoverWhitelistOnly() public onlyOwner {
        whitelistOnly = false;
    }
    function setWhitelistPointer(address _whitelist) public onlyOwner {
        whitelist = Whitelist(_whitelist);
    }
    function setAPR(uint256 _apr) public onlyOwner {
        apr = _apr;
    }
    function setROIRate(uint256 _roiRate) public onlyOwner {
        roiRate = _roiRate;
    }
