...
contract MockUtxoTreasury is Ownable {
  using SafeMath for uint256;
  ...
  address public sUTXO;
  uint256 public sUTXOQueue; // Delays change to sUTXO address
  ...
  constructor(
    ...
    isReserveToken[_WBTC] = true;
    reserveTokens.push(_WBTC);

    // isLiquidityToken[ _UTXOWBTC ] = true;
    // liquidityTokens.push( _UTXOWBTC );

    blocksNeededForQueue = _blocksNeededForQueue;
  }

  /**
        @notice allow approved address to deposit an asset for UTXO
        @param _amount uint
  ...
  function deposit(
    ...
    (_token, _amount);
    // mint UTXO needed and store amount of rewards for distribution
    send_ = value.sub(_profit);
    IERC20Mintable(UTXO).mint(msg.sender, send_);
  ...
  /**
        @notice allow approved address to burn UTXO for reserves
        @param _amount uint
        @param _token address
     */
  function withdraw(uint256 _amount, address _token) external {
    ...
    uint256 value = valueOfToken(_token, _amount);
    IUTXOERC20(UTXO).burnFrom(msg.sender, value);
  ...
  function incurDebt(uint256 _amount, address _token) external {
    ...
    uint256 maximumDebt = IERC20(sUTXO).balanceOf(msg.sender); // Can only borrow against sUTXO held
  ...
  /**
        @notice allow approved address to repay borrowed reserves with UTXO
        @param _amount uint
     */
  function repayDebtWithUTXO(uint256 _amount) external {
    require(isDebtor[msg.sender], "Not approved");

    IUTXOERC20(UTXO).burnFrom(msg.sender, _amount);

    debtorBalance[msg.sender] = debtorBalance[msg.sender].sub(_amount);
    totalDebt = totalDebt.sub(_amount);

    emit RepayDebt(msg.sender, UTXO, _amount, _amount);
  }
  ...
  function mintRewards(address _recipient, uint256 _amount) external {
    ...
    IERC20Mintable(UTXO).mint(_recipient, _amount);
  ...
  function excessReserves() public view returns (uint256) {
    return totalReserves.sub(IERC20(UTXO).totalSupply().sub(totalDebt));
  }
  ...
  /**
        @notice returns UTXO valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
  ...
  {
    if (isReserveToken[_token]) {
      // convert amount to match UTXO decimals
      value_ = _amount.mul(10**IERC20(UTXO).decimals()).div(
    ...
    returns (bool)
  {
    ...
    } else if (_managing == MANAGING.SUTXO) {
      // 9
      sUTXOQueue = block.number.add(blocksNeededForQueue);
    } else return false;
