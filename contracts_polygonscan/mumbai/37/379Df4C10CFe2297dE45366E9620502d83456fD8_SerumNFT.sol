    ...
    ERC721URIStorageUpgradeable
{
    ...
    struct TokenMeta {
        ...
        uint256 price;
        string uri;
        bool status;  // full or empty
        bool generic;
        bool card_specific;
        bool collection_specific;
    }
    ...
    function _setCollectionMeta(uint256 _tokenId, CollectionMeta memory _collection) private {
        require(_exists(_tokenId));
        require(ownerOf(_tokenId) == _msgSender() || owner() == _msgSender());
        _tokenCollection[_tokenId].push(_collection);
    }
    ...
    function _setCardMeta(uint256 _tokenId, CardMeta memory _card) private {
        require(_exists(_tokenId));
        require(ownerOf(_tokenId) == _msgSender() || owner() == _msgSender());
        _tokenCard[_tokenId].push(_card);
    }
     ...
     * @param _status bool for token status 
     * @param _generic bool for token generic
     * @param _card_specific bool for token card
     * @param _collection_specific bool for token collectiom
     * Requirements:
