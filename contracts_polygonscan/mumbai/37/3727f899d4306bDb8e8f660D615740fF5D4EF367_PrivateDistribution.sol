...
contract PrivateDistribution is Ownable {
    ...
    {   
        uint256 noOfDays = 533;
        uint256 currentTimeStamp = block.timestamp;
        uint256 noOfDaysPassed = BokkyPooBahsDateTimeLibrary.diffDays(_initialTimestamp, currentTimeStamp);
        ...
        uint256 tokensAvailable = ((noOfTokens.sub(investor.withdrawnTokens)).div(noOfDays)).mul(noOfDaysPassed);
        ...
        returns (uint256 availablePercentage)
    {
        ...
        uint256 vestingDuration = _initialTimestamp + 533 days;
        uint256 noOfRemaingDays = 532;
        ...
        if (currentTimeStamp > _initialTimestamp) {
            if (allocationType == AllocationType.PRIVATE) {
                ...
                uint256 initialReleasePercentage = uint256(506481).mul(1e18).div(10000);
                uint256 remainingDistroPercentage = uint256(493518).div(10000);
