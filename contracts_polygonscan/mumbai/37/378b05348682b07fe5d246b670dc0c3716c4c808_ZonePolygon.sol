...
// File contracts/Token/ZonePolygon.sol
...
contract CompBase is Ownable {
     ...
     */
    uint256 private constant _cap = 28000000 * (10 ** uint256(decimals)); // 28M
    ...
    bytes32 public immutable DELEGATION_DOMAIN_SEPARATOR; // This is needed for compatibility with Ethereum token
    ...
    constructor(address ownerAddress) Ownable(ownerAddress) public {
        ...
        DELEGATION_DOMAIN_SEPARATOR = keccak256(abi.encode(
     ...
     */
    function _checkPermit(address owner, address spender, uint256 amount, uint256 deadline, uint8 v, bytes32 r, bytes32 s) internal {
        require(deadline >= block.timestamp, "ZONE: expired permit deadline");
    ...
    function transferWithPermit(address src, address dst, uint256 amount, uint256 deadline, uint8 v, bytes32 r, bytes32 s) external returns (bool) {
        _checkPermit(src, dst, amount, deadline, v, r, s);
        _transferTokens(src, dst, amount);
        return true;
    }
    ...
    function delegateBySig(address delegatee, uint nonce, uint expiry, uint8 v, bytes32 r, bytes32 s) public {
        ...
        bytes32 digest = keccak256(abi.encodePacked("\x19\x01", DELEGATION_DOMAIN_SEPARATOR, structHash));
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) view internal {
        if (from == address(0)) { // When minting tokens
            require(totalSupply().add(amount) <= _cap, "ZONE: cap exceeded");
        }
    }
}

contract ZonePolygon is CompBase, AccessControl {
    using SafeMath for uint256;

    bytes32 public constant DEPOSITOR_ROLE = keccak256("DEPOSITOR_ROLE");

    constructor(address _ownerAddress, address _childChainManager) CompBase(_ownerAddress) public {
        _setupRole(DEFAULT_ADMIN_ROLE, _ownerAddress);
        _setupRole(DEPOSITOR_ROLE, _childChainManager);
    }

    modifier only(bytes32 role) {
        require(hasRole(role, _msgSender()), "ZONE: No allowed with permission");
        _;
    }
    ...
    receive() external payable {
        require(false, "ZONE: We will not accept ETH");
    }
}