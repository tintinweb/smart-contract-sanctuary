...
contract BinamarsItemMarket {
    ...
    uint256 public feeMarketRate = 50; //5%
    IERC721 public bieqNFT;

    constructor(address _binaToken, address _bieqNFT) {
        binaToken = IERC20(_binaToken);
        bieqNFT = IERC721(_bieqNFT);
    }

    function placeOrder(uint256 _tokenId, uint256 _price) public {
        require(bieqNFT.ownerOf(_tokenId) == msg.sender, "Only owner can sell");
        require(_price > 0, "Cannot sell without fee");
        bieqNFT.transferFrom(msg.sender, address(this), _tokenId);
    ...
    function cancelOrder(uint256 _tokenId) public {
        ...
        bieqNFT.transferFrom(address(this), msg.sender, _tokenId);
    ...
    function fillOrder(uint256 _tokenId) public {
        require(tokenSales.contains(_tokenId), "not sale");
        ItemSale storage itemSale = markets[_tokenId];
        uint256 feeMarket = itemSale.price.mul(feeMarketRate).div(divPercent);

        binaToken.transferFrom(msg.sender, feeAddress, feeMarket);
