...
// File: contracts/LiveNFTDexPolygon.sol
...
contract NFTDexV8Polygon is ERC721Full, ERC721MetadataMintable, Ownable {
  IERC20 ALIA;
...
function init5() public{
  ...
  function buyNFT(address nft_a,uint256 tokenId, string memory ownerId) public{
        ...
        emit BuyNFTNonCrypto( msg.sender, address(this), tokenId, ownerId);     
         ...
         uint256 mainPrice = (price  / 1000) * mainPerecentage;
        uint256 _artistPrice=(price  / 1000) * authorPercentage;
        if(isArtistAuthor && _tokenAuthors[tokenId] == nonCryptoNFTVault){
          ...
          if(revenueShare.checkArtistOwner(_nonCryptoOwners[tokenId], tokenId)){
           ...
           ALIA.transferFrom(msg.sender, temp.seller, mainPrice);
          ...
          ALIA.transferFrom(msg.sender, _tokenAuthors[tokenId], _artistPrice);
        }
        _transferFrom(address(this), msg.sender, tokenId);
         if(temp.seller == nonCryptoNFTVault && !isArtistAuthor) {
  ...
  require(msg.sender == 0x773E9CF672c7c4B50C39856A057f5E6581418e3D);
  _;
  }
