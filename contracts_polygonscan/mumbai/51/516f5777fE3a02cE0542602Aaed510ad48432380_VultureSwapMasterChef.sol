...
contract CRC20 is Context, ICRC20, Ownable {
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public returns (bool) {
        _approve(
            _msgSender(),
            spender,
            _allowances[_msgSender()][spender].sub(subtractedValue, "CRC20: decreased allowance below zero")
        );
        return true;
    }
    ...
    function _burnFrom(address account, uint256 amount) internal {
        ...
        _approve(
            account,
            _msgSender(),
            _allowances[account][_msgSender()].sub(amount, "CRC20: burn amount exceeds allowance")
        );
    }
}

// VultureSwapToken.
contract VultureSwapToken is CRC20('VultureSwap Finance', 'VULTR') {
...
// MasterChef is the master of VULTR. He can make VultureSwap and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once VULTR is sufficiently
// distributed and the community can show to govern itself.
//
// Have fun reading it. Hopefully it's bug-free. God bless.
contract VultureSwapMasterChef is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of VULTR
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accVulturePerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accVulturePerShare` (and `lastRewardSecond`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. VULTRs to distribute per second.
        uint256 lastRewardSecond;  // Last timestamp that VULTRs distribution occurs.
        uint256 accVulturePerShare;   // Accumulated VULTRs per share, times 1e18. See below.
        uint256 lpSupply;
    }

    // The VULTR TOKEN!
    VultureSwapToken public immutable vulture;
    // Dev address.
    address public devaddr;
    // Treasury address.
    address public treasuryaddr;
    // VULTR tokens created per second.
    uint256 public VulturePerSecond;
    ...
    // The timestamp when VultureSwap mining starts.
    uint256 public startTime;

    // Maximum VulturePerSecond
    ...
    event UpdateEmissionRate(address indexed user, uint256 VulturePerSecond);
    ...
    event VultureMintError(bytes reason);

    constructor(
        VultureSwapToken _vulture,
        address _devaddr,
    ...
    mapping(ICRC20 => bool) public poolExistence;
    modifier nonDuplicated(ICRC20 _lpToken) {
        require(poolExistence[_lpToken] == false, "nonDuplicated: duplicated");
        _;
    }

    // Add a new lp to the pool. Can only be called by the owner.
    function add(uint256 _allocPoint, ICRC20 _lpToken, bool _withUpdate) external onlyOwner nonDuplicated(_lpToken) {
        // valid ERC20 token
    ...
    // View function to see pending VultureSwaps on frontend.
    function pendingVulture(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accVulturePerShare = pool.accVulturePerShare;
        if (block.timestamp > pool.lastRewardSecond && pool.lpSupply != 0 && totalAllocPoint > 0) {
            ...
            accVulturePerShare = accVulturePerShare.add(vultureReward.mul(1e18).div(pool.lpSupply));
        }
        return user.amount.mul(accVulturePerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    // Safe vulture transfer function, just in case if rounding error causes pool to not have enough VULTRs.
    function safeVultureTransfer(address _to, uint256 _amount) internal {
        uint256 vultureBal = vulture.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > vultureBal) {
            transferSuccess = vulture.transfer(_to, vultureBal);
        } else {
            transferSuccess = vulture.transfer(_to, _amount);
        }
        require(transferSuccess, "safeVultureTransfer: transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _VulturePerSecond) external onlyOwner {
        require(_VulturePerSecond <= MAX_EMISSION_RATE, "Too high");
        massUpdatePools();
        VulturePerSecond = _VulturePerSecond;
        emit UpdateEmissionRate(msg.sender, _VulturePerSecond);
    }
