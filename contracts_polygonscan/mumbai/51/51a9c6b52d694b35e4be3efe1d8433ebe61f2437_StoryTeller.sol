...
contract StoryTeller is ERC721Enumerable, Ownable {
  using Strings for uint256;
...
struct Book { 
  ...
  mapping (uint256 => Book) public pages;
...
function claim() public {
    ...
    require(ownedAmount >= requiredAmount, "You can't tell your story if you don't own your dinosaur!");
    uint256 supply = totalSupply();
    require(supply + 1 <= 12500, "Max supply reached");
     Book memory newText = Book("");
    pages[supply + 1] = newText;
...
function setMessage(uint256 _tokenId, string memory _newMessage) public {
    ...
    Book storage currentBook = pages[_tokenId];
    currentBook.message = _newMessage;
  }

   function readMessage(uint256 _tokenId) public view returns (string memory message){
    Book memory currentBook = pages[_tokenId];
    return currentBook.message;
  }
