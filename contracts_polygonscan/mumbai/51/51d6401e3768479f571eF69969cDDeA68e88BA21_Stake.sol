...
contract Stake {
    ...
    function _updateReward(address account) internal {
        Stakes storage thisStake = stakes[msg.sender];

        thisStake.lastUpdateTime = block.timestamp;
        thisStake.rewards = earned(account);
    }
    ...
    function stake(uint _amount, uint _numberOfDays) external {
    ...
    function rewardPerToken(address account) public view returns (uint) {
        return  stakes[account].amount * rate / 10000;
    }

    function earned(address account) public view returns (uint256) {
        return
            ((block.timestamp - stakes[account].lastUpdateTime) * rewardPerToken(account) / 1e18) +
    ...
    function getReward() external {
        ...
        uint256 reward = thisStake.rewards;
        thisStake.rewards = 0;
