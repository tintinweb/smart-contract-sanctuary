...
contract TokenVestor is Initializable, AccessControlEnumerableUpgradeable {
    ...
    bool public vestingActive;  // still need?
    ...
    function initialize(
        ...
        grantRole(GRANTOR, admin);
        grantRole(CLOSER, admin);
    }
    ...
    function launchVesting(address[] calldata recipientAddresses) public onlyRole(MANAGER) {
        for(uint i = 0; i < recipientAddresses.length; i++) {
            ...
            FlowRecipient memory recipient = _recipients[addr];
            if (block.timestamp > recipient.cliffEnd) {
                openStream(recipientAddresses[i]);
            }
        }

        if(!vestingActive) {
            // vestingActive is true when >1 address is vesting
    ...
    function openStream(address recipient) internal {
        ...
        if (_recipients[recipient].starttime == 0) {
            _recipients[recipient].starttime = block.timestamp;
        }
    }
    ...
    function closeVesting(address[] calldata recipientAddresses) public onlyRole(CLOSER) {
        ...
        for(uint i = 0; i < recipientAddresses.length; i++) {
            address addr = recipientAddresses[i];
            FlowRecipient memory recipient = _recipients[addr];
            if (elapsedTime(recipientAddresses[i]) > recipient.vestingDuration) {
                closeStream(recipientAddresses[i]);
            }
        }
    }

    function closeStream(address recipient) public onlyRole(CLOSER) {
        require(_recipients[recipient].state == RecipientState.Flowing, "Stream inactive");

        if(elapsedTime(recipient) < _recipients[recipient].vestingDuration) {
    ...
    function elapsedTime(address recipient) public view returns (uint256) {
        require(_recipients[recipient].starttime > 0, "Vesting has not yet started.");
        return block.timestamp.sub(_recipients[recipient].starttime);
    }

    function estimateElapsedTokens(address recipient) public view onlyRole(MANAGER) returns (uint256) {
        require(vestingActive, "Vesting inactive");
        uint256 durationEstimate = block.timestamp
                                    .sub(_recipients[recipient].starttime)
    ...
    function estimateTotalTokens(address recipient) public onlyRole(MANAGER) view returns (uint256) {
        return _recipients[recipient].vestingDuration.mul(_recipients[recipient].flowRate);
    }

    function estimateRemainingTokens(address recipient) public onlyRole(MANAGER)  view returns (uint256) {
        return estimateTotalTokens(recipient).sub(estimateElapsedTokens(recipient));
    }

    function getFlowRecipient(address adr) public onlyRole(MANAGER) view returns (FlowRecipient memory) {
        return _recipients[adr];
    }

    function registerRecipient(address adr, uint256 flowRate, bool isPermanent, uint256 cliffEnd, uint256 vestingDuration) public onlyRole(GRANTOR) notRegistered(adr) returns (FlowRecipient memory) {
        FlowRecipient memory newRecipient = FlowRecipient(adr, flowRate, isPermanent, RecipientState.Registered, cliffEnd, vestingDuration, 0);
...
contract VestingFactory {
    ...
    function createVestor(address _token) external returns (address) {
        ...
        TokenVestor(clone).initialize(_token);
