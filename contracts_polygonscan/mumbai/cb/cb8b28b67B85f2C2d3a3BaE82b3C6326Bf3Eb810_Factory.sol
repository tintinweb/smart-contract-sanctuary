...
import "./libraries/NativeMetaTransaction/OwnableUpgradeable.sol";
import "@openzeppelin/contracts-upgradeable/proxy/ClonesUpgradeable.sol";
import "@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol";
import "@openzeppelin/contracts-upgradeable/proxy/utils/UUPSUpgradeable.sol";

import "./GameQuestion.sol";

contract Factory is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    mapping(bytes32 => address[]) public questionAddressArray;
    mapping(bytes32 => mapping(bytes32 => address)) public questionAddressMap;
    mapping(bytes32 => bool) public isGameEnded;
    mapping(bytes32 => bool) public isGameClaimed;
    ...
    function addQuestionType(bytes32 _id, address _questionAddress) external onlyOwner {
        require(questionType[_id] == address(0));
    ...
    function deployQuestions(bytes32 _id, QuestionEncodedData[] memory questionData) external onlyOperator {
        require(
            questionAddressMap[questionData[0].gameID][questionData[0].questionID] == address(0),
            "QuestionSet: already deployed"
        );
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            bytes4 funcSig = bytes4(keccak256(bytes("initialize(bytes,address,address)")));
            bytes memory payload = abi.encodeWithSelector(funcSig, questionData[i].data, adminAddress, operatorAddress);
            ...
            questionAddressMap[questionData[i].gameID][questionData[i].questionID] = questionClone;
    ...
    function setAnswers(bytes32 gameID, uint8[] memory options) external onlyOperator {
        require(isGameEnded[gameID] == false, "Answers have already been set");
        uint256 len = questionAddressArray[gameID].length;
        for (uint256 i = 0; i < len; i++) {
            GameQuestion(questionAddressArray[gameID][i]).executeQuestion(options[i]);
        }
        isGameEnded[gameID] = true;
    }

    //Claim rewards from all questions
    function claimAll(bytes32 gameID, address userAddress) external {
        require(isGameClaimed[gameID] == false, "All Rewards Have been claimed");
        uint256 len = questionAddressArray[gameID].length;
        for (uint256 i = 0; i < len; i++) {
            GameQuestion(questionAddressArray[gameID][i]).claim(userAddress);
        }
        isGameClaimed[gameID] = true;
    }
    ...
    function getGameQuestionAddress(bytes32 gameID, bytes32 questionID) external view returns (address gameAddress) {
        gameAddress = questionAddressMap[gameID][questionID];
    }
}
    ...
    NativeMetaTransaction
{
    ...
    event Pause(bytes32 questionID);
    event Unpause(bytes32 questionID);
    ...
    modifier onlyAdminOrOperator() {
        require(_msgSender() == adminAddress || _msgSender() == operatorAddress, "admin | operator: wut?");
        _;
    }
    ...
    function placeBet(uint256 amount, uint8 option) external whenNotPaused nonReentrant {
        //Option to be A or B
        ...
        //can only bet once
        require(ledger[_msgSender()].amount == 0, "only bet once");

        question.totalAmount += amount;
        BetInfo storage betInfo = ledger[_msgSender()];

        if (option == 0) {
            question.betAmountA += amount;
            betInfo.position = Position.A;
            emit PlaceBetA(_msgSender(), questionID, amount);
        } else {
            question.betAmountB += amount;
            betInfo.position = Position.B;
            emit PlaceBetB(_msgSender(), questionID, amount);
        }

        betInfo.amount = amount;

        gameToken.safeTransferFrom(_msgSender(), address(this), amount);
    }

    function claim(address userAddress) external whenNotPaused nonReentrant {
        require(question.isGameEnded, "Game has not ended");
        require(ledger[userAddress].amount != 0, "Bet amount 0");
        //require(!ledger[_msgSender()].claimed, "Rewards claimed");

        uint256 reward;

        if (!ledger[userAddress].claimed) {
    ...
    function executeQuestion(uint8 option) external onlyOwner whenNotPaused {
        _setAnswer(option);
        _calculateRewards();
    }
