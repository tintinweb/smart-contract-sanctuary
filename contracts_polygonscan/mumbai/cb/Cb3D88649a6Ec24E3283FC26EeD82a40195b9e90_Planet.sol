    ...
    ERC721Data
{
    ...
    mapping(uint256 => uint256) internal planetState;
    ...
    event StateUpdate(uint256 indexed tokenId, uint256 indexed state);
    ...
    function setPlanetPassItemsContract(
        PlanetPassItems _planetPassItemsContract
    ...
    function setPlanetState(uint256 tokenId, uint256 state)
        external
        onlyRole(STATE_UPDATER_ROLE)
    {
        planetState[tokenId] = state;
    ...
    /// @param state the state to update to
    function setPlanetState(uint256[] calldata tokenId, uint256 state)
        external
        onlyRole(STATE_UPDATER_ROLE)
    {
        for (uint256 i = 0; i < tokenId.length; i++) {
            planetState[tokenId[i]] = state;
            emit StateUpdate(tokenId[i], state);
        }
    }
    ...
    function getPlanetState(uint256 tokenId) public view returns (uint256) {
        ...
        return planetState[tokenId];
    }

    /// Allows owner of planet to renamer their planet for a Stardust fee
    /// @param tokenId the token ID of the planet
    ...
    /// Allows owner of planet to renamer their planets for a Stardust fee
...
abstract contract ERC721Data is ERC721, AccessControl, Pausable {
    ...
    /// Event emitted when an update occurs.
    event SetData(
    ...
    /// Event emitted when a token is unlocked (cannot be updated).
