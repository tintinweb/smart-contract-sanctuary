...
abstract contract PrizePool is BaseRelayRecipient, PrizePoolInterface, OwnableUpgradeable, ReentrancyGuardUpgradeable, TokenControllerInterface {
  using SafeMathUpgradeable for uint256;
    ...
    returns (uint256)
  {
    ...
    // burn the tickets
    ControlledToken(controlledToken).controllerBurnFrom(msgSender(), from, amount);
    ...
    emit InstantWithdrawal(msgSender(), from, controlledToken, amount, redeemed, exitFee);
    ...
    returns (uint256)
  {
    ...
    emit TimelockedWithdrawal(msgSender(), from, controlledToken, amount, unlockTimestamp);
  ...
  function beforeTokenTransfer(address from, address to, uint256 amount) external override onlyControlledToken(msgSender()) {
    if (from != address(0)) {
      uint256 fromBeforeBalance = IERC20Upgradeable(msgSender()).balanceOf(from);
      // first accrue credit for their old balance
      uint256 newCreditBalance = _calculateCreditBalance(from, msgSender(), fromBeforeBalance, 0);

      if (from != to) {
        // if they are sending funds to someone else, we need to limit their accrued credit to their new balance
        newCreditBalance = _applyCreditLimit(msgSender(), fromBeforeBalance.sub(amount), newCreditBalance);
      }

      _updateCreditBalance(from, msgSender(), newCreditBalance);
    }
    if (to != address(0) && to != from) {
      _accrueCredit(to, msgSender(), IERC20Upgradeable(msgSender()).balanceOf(to), 0);
    }
    // if we aren't minting
    if (from != address(0) && address(prizeStrategy) != address(0)) {
      prizeStrategy.beforeTokenTransfer(from, to, amount, msgSender());
    }
  }
  ...
  modifier onlyPrizeStrategy() {
    require(msgSender() == address(prizeStrategy), "PrizePool/only-prizeStrategy");
    _;
  }
  ...
  modifier onlyReserve() {
    ReserveInterface reserve = ReserveInterface(reserveRegistry.lookup());
    require(address(reserve) == msgSender(), "PrizePool/only-reserve");
    _;
  }
}
...
import "./RelayRecipientInterface.sol";

/**
 * A base contract to be inherited by any contract that want to receive relayed transactions
 * A subclass must use "_msgSender()" instead of "msg.sender"
 */
abstract contract BaseRelayRecipient is RelayRecipientInterface {
     ...
     */
    function msgSender() internal override virtual view returns (address payable ret) {
 ...
 */
abstract contract RelayRecipientInterface {
