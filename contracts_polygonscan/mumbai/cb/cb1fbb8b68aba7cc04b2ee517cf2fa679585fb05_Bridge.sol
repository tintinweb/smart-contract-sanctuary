...
contract Bridge is Initializable, OwnableUpgradeable, UUPSUpgradeable {
	struct TokenInfo {
		address otherToken; // Token on the other chain
		uint256 originChainId; // Chain id of the chain where the origin token is
	}
	...
	uint256 public minRequiredValidators; // Minimum number of validators that signed a claim message
	mapping(bytes32 => bool) public claimed; // Stores claimIds which have been used
	uint256 public fee; // Teleport fee. 1000 => 100%, 500 => 50%, 1 => 0.1%
	address public feeReceiver; // The fee receiver gets teleport fees if there are any
	mapping(address => bool) public noFeeAddress; // Addresses which don't have to pay the teleport fee
	mapping(uint256 => bool) public whitelistedChainId; // Chains which are cleared to be teleported to and claimed from
	uint256 claimIdCounter; // Counter for creating unique claim ids
	...
	/**
	 * @dev Locks or burns the sender's tokens to transfer them to the destination chain.
	 */
	function teleport(
		uint256 destChainId,
		address srcToken,
		...
		if (destTokenInfo.originChainId == getChainId()) {
			// In order to support reflection tokens the transfered amount is measured
	...
	/**
	 * @dev Accepts an validator signed message to transfer/mint tokens to the destination address. A message cannot be used more than once. Signatures beyond signatures[minRequiredValidators - 1] are discarded.
	 */
	...
	/**
	 * @dev Sets the minimum number of validators that signed a claim message.
	 */
	...
	/**
	 * @dev Adds a chains which is cleared to be teleported to and claimed from.
	 */
	...
	/**
	 * @dev Connects a token from this chain with a token from another chain. This function needs to be called on the other chain too to connect the tokens with each other.
	 */
	function addToken(
		address token,
	...
	/**
	 * @dev Sets the teleport fee. Valid values are >=0 and <1000.
	 */
	...
	/**
	 * @dev Sets the teleport fee receiver.
	 */
	...
	/**
	 * @dev Adds an address which doesn't have to pay the teleport fee.
	 */
	...
	/**
	 * @dev Removes an address from the no teleport fee list.
	 */
	...
	/**
	 * @dev Returns an unique claim id.
	 */
	...
	/**
	 * @dev Verifies the signatures. The called functions will revert if the signatures aren't valid.
	 */
	...
	/**
	 * @dev Recovers the validator addresses from the signatures and the message hash.
	 */
	...
	/**
	 * @dev Splits a signature into its parts r, s and v.
	 */
	...
	/**
	 * @dev Checks if the recovered addresses from the signarues are valid validator addresses.
	 */
