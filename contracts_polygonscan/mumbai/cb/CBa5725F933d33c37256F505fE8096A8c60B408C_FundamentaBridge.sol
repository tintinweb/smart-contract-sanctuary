...
contract FundamentaBridge is SecureContract
{
    ...
    function queryAccumulatedFees(uint256 tokenId) public view returns (uint256, uint256) { 
        return (_tokens[tokenId].accumulatedFee, _tokens[tokenId].totalBurn); }
        ...
        returns(address, uint256, uint256, uint256, uint256)
    {
        ...
        require(signer == msg.sender, "Bridge: Not sent by client");
        require(signer == sender, "Bridge: Not signed by client");
        ...
        return (sender, nonce, destNetwork, tokenId, amount);
    }
        ...
        returns(address, uint256, uint256, uint256, uint256)
    {
        ...
        return (sender, nonce, srcNetwork, tokenId, amount);
    }
    ...
    function deposit(bytes memory clientSignature, bytes[] memory serverSignatures, bytes memory transactionData) public pause
    {
        ...
        uint256 tokenId;
        uint256 amount;
        (sender, nonce, destNetwork, tokenId, amount) = verifyDeposit(clientSignature, serverSignatures, transactionData);
        ...
        {
            require(TokenInterface(_tokens[0].info.token).balanceOf(sender) >= token.fmtaDepositFee, "Bridge: Insufficient FMTA balance");
            TokenInterface(_tokens[0].info.token).burnFrom(sender, token.fmtaDepositFee);
            token.totalBurn += token.fmtaDepositFee;
        }

        token.depositNonces[nonce] = true;
        TokenInterface(token.info.token).mintTo(sender, amount - fee);
        ...
        emit Deposit(nonce, sender, srcNetwork, destNetwork, tokenId, amount, token.fmtaDepositFee, fee);
    }
    ...
    function withdraw(bytes memory clientSignature, bytes memory transactionData) public pause
    {
        ...
        uint256 tokenId;
        uint256 amount;
        (sender, nonce, srcNetwork, tokenId, amount) = verifyWithdraw(clientSignature, transactionData);
        ...
        {
            require(TokenInterface(_tokens[0].info.token).balanceOf(sender) >= token.fmtaWithdrawFee, "Bridge: Insufficient FMTA balance");
            TokenInterface(_tokens[0].info.token).burnFrom(sender, token.fmtaWithdrawFee);
            token.totalBurn += token.fmtaWithdrawFee;
        }

        require(TokenInterface(token.info.token).balanceOf(sender) >= (amount + fee), "Bridge: Insufficient balance");

        token.withdrawNonces[nonce] = true;
        TokenInterface(token.info.token).burnFrom(sender, amount + fee);
        token.accumulatedFee += fee;
        ...
        emit Withdraw(nonce, sender, srcNetwork, destNetwork, tokenId, amount, token.fmtaWithdrawFee, fee);
    }
    ...
    function depositAndUnwrap(bytes memory clientSignature, bytes[] memory serverSignatures, bytes memory transactionData) public pause
    {
        ...
        token.depositNonces[nonce] = true;
        WrappedTokenInterface(token.info.token).crossChainUnwrap(sender, amount, fee);
    ...
    function wrapAndWithdraw(bytes memory clientSignature, bytes memory transactionData) public pause
    {
        ...
        if (token.fmtaWithdrawFee != 0)
        {
            require(TokenInterface(_tokens[0].info.token).balanceOf(sender) >= token.fmtaWithdrawFee, "Bridge: Insufficient FMTA amount to pay fee");
            TokenInterface(_tokens[0].info.token).burnFrom(sender, token.fmtaWithdrawFee);
            token.totalBurn += token.fmtaDepositFee;
        }

        token.withdrawNonces[nonce] = true;
        WrappedTokenInterface(token.info.token).crossChainWrap(sender, amount, fee);
