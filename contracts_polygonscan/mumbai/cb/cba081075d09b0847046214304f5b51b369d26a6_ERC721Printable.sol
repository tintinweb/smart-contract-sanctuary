...
/** 
 *  SourceUnit: c:\Users\anike\Work\Mintable_v2.0\src\constants\contracts\PrintablePublic.sol
*/
    ...
    IERC721Enumerable
{
    ...
    {
        require(exchange != _msgSender(), "ERC721: approve to caller");

        _operatorApprovals[_msgSender()][exchange] = approved;
        emit ApprovalForAll(_msgSender(), exchange, approved);
    }
    /**
    ...
    function _batchMint(address to, uint256 count) internal virtual {
        ...
        _batchMintOwnersMap[_totalBatches].owner = _msgSender();
        _batchMax[_totalBatches].add(_totalSupply);
        _batchMax[_totalBatches].add(count.add(_totalSupply));
        _ownersBatches[_msgSender()].add(count);
        _totalBatches = _totalBatches.add(1);
        BalancesMap[_msgSender()] = BalancesMap[_msgSender()].add(count);
  ...
  event TransferFee(address indexed to, uint256 indexed fee); 
   ...
   function _createPrintSeries(uint256 _totalAmount, uint256 _price, string memory _url) internal returns (bool){
        totalSeries = totalSeries.add(1);
          ...
          PreMintData[totalSeries].amount_of_tokens_left = _totalAmount;
              PreMintData[totalSeries].price = _price;
                ...
                PreMintData[totalSeries].creator = _msgSender();
                
                emit SeriesMade(_msgSender(), _price, _totalAmount);
                return true;
 }
    ...
    function mintSeries(uint256 _seriesID, address _to) public payable returns (bool){
     ...
     emit SeriesPurchased( _msgSender(), super.totalSupply().add(1), msg.value); 
