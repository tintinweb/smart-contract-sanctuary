 ...
 *
 */
contract InvictusWhitelist is Initializable, OwnableUpgradeable, AccessControlUpgradeable, ERC2771ContextUpgradeable {
...
contract ERC20InvictusFund is OwnableUpgradeable, ERC20PresetMinterPauserUpgradeable, ERC2771ContextUpgradeable {
  ...
  address public constant dealAddress = 0x000000000000000000000000000000000000deA1;
  ...
  /**
   * @dev Purges the supported currencies array.
   */
  ...
  function calculateExitFee(uint256 redemptionAmount) private view returns (uint256) {
    require(redemptionAmount.mul(10000).div(10000) == redemptionAmount, "too small");
    return redemptionAmount.mul(exitFeePercentage).div(10000);
  }

  /**
   * @dev Request redemtion in a supported currencies to be processed on the next dealing.
   * @param withdrawAmount The amount of fund tokens to redeem.
   * @param ticker The currency you would like to recieved your redemption in.
   */
  function requestRedemption(uint256 withdrawAmount, string memory ticker)
    external
    onlyWhitelisted
    onlyRedemptionCurrencies(ticker)
  {
    require(balanceOf(_msgSender()) >= withdrawAmount, "Balance to low");
    require(withdrawAmount >= minTokenRedemption, "Too few tokens");

    transfer(dealAddress, withdrawAmount);
  ...
  /**
   * @dev Queues investment in a supported currencies to be processed on the next dealing.
   * @param amount The amount of fund tokens to redeem.
   * @param investmentCurrency The currency you would like to recieved your redemption in.
   * @param transactionHash The currency you would like to recieved your redemption in.
   * @param investor The currency you would like to recieved your redemption in.
   * @param chainId The currency you would like to recieved your redemption in.
  ...
  function queueInvestment(
    ...
    require(currencies[investmentCurrency].price.numerator != 0, "Invalid currency");
    require(amount > 0, "Invalid amount");
    require(!investmentTxIndex[transactionHash][index], "Index not unique");

    investmentTxIndex[transactionHash][index] = true;
    investments.push(
      Investment(amount, block.timestamp, chainId, index, investmentCurrency, transactionHash, investor)
    );
    emit InvestmentRequest(amount, block.timestamp, investmentCurrency, transactionHash, investor, chainId, index);
  }

  /**
   * @dev Initiate dealing. Processes queued up investments and redeptions.
   * @param _currencies List of currencies to process.
   * @param _rates List of rates for the currencies passed in.
   */
  function dealing(string[] calldata _currencies, uint256[] calldata _rates) external onlyPricingOracle {
    ...
    require(_currencies.length == supportedCurrencies.length, "Invalid currency list");
    for (uint256 i = 0; i < supportedCurrencies.length; i++) {
      require(keccak256(bytes(supportedCurrencies[i])) == keccak256(bytes(_currencies[i])), "Currencies do not match");
    }
   ...
   * @param ticker The currency ticker that you want remove liquidity for.
   */
  ...
  /**
   * @dev Add a stable currency, all redemption currencies should have zero address. (Usually currency used for investments)
   * @param ticker The ticker of the currency. eg. ETH
   * @param numerator The the current USD price of the currency.
   * @param decimals The amount of decimals in the currency contract.
   */
  function addCurrency(
    ...
    currencies[ticker] = Currency(Price(numerator, denominator), contractAddress, decimals);
    supportedCurrencies.push(ticker);
  }
   ...
   * @param investor The ticker of the currency. eg. ETH
   * @param amount The the current USD price of the currency.
   * @param reason The amount of decimals in the currency contract.
   * @param txHash The amount of decimals in the currency contract.
   */
  ...
  /**
   * @dev Update the rates for the supported currencies.
   * @param _currencies The list of supported currencies tickers.
   * @param rates The prices in USD, mapped to the _currencies list.
   */
  function updateCurrenciesPrices(string[] memory _currencies, uint256[] memory rates) public onlyPricingOracle {
    for (uint256 i = 0; i < _currencies.length; i++) {
      ...
      emit AssetPriceUpdated(rates[i], _currencies[i]);
    }
  }
   ...
   * @param ticker The currency requested to recive your redemtion in.
   */
  function dealRedemption(
    ...
    require(
      ERC20Upgradeable(stableContract).balanceOf(address(this)) > redemptionCurrencyAmount,
      "not enough liquidity"
    );
    uint256 exitFee = calculateExitFee(redemptionCurrencyAmount);

    _burn(dealAddress, tokenAmount);
    ERC20Upgradeable(stableContract).safeTransfer(feeAddress, exitFee);
    ERC20Upgradeable(stableContract).safeTransfer(investor, (redemptionCurrencyAmount.sub(exitFee)));
   ...
   * @param investmentCurrency The investment currency.
   ...
   * @param investmentCurrency The currency the investment was made in.
   * @param investmentCurrencyAmount The ammount of tokens sent for investment.
   */
  function calculateTokenAmount(string memory investmentCurrency, uint256 investmentCurrencyAmount)
    private
    view
    returns (uint256)
  {
    ...
    return investmentCurrencyAmount.mul(currency.price.numerator).div(contractToken.price.numerator);
  }

  /**
   * @dev Calculates the amount of tokens a investor will recieved for their redemption.
   * @param ticker The requested currency the investor wants to recieve the redemption in.
   * @param tokenAmount The ammount of fund tokens the investor wants to redeem.
   */
  function calculateRedemptionCurrencyAmount(string memory ticker, uint256 tokenAmount) private view returns (uint256) {
    Currency memory currency = currencies[ticker];
    Currency memory contractToken = currencies[symbol()];
    uint256 tokens = ceil(
      tokenAmount.mul(contractToken.price.numerator).div(currency.price.numerator),
      10**(contractToken.decimals - currency.decimals)
    );
    if (currency.decimals > contractToken.decimals) {
      return tokens.mul(10**(currency.decimals - contractToken.decimals));
    } else {
      return tokens.div(10**(contractToken.decimals - currency.decimals));
    }
  }

  /**
   * @dev Rounding function used for the redeptions in currencies with differnet decimal points.
   * @param value The value to be rounded.
   * @param roundLimit The points to round to.
   */
  ...
  function _handleRedemptionsDealing() private returns (bool) {
    ...
    uint256 endingIndex = redemptionsLength().sub(numberOfRedemptions);
    ...
    for (uint256 i = redemptionsLength(); i > endingIndex; i--) {
      ...
      if (!InvictusWhitelist(whitelistContract).isWhitelisted(redemption.investor)) {
        ...
        dealRedemption(
          i - 1,
          redemption.investor,
          calculateRedemptionCurrencyAmount(redemption.ticker, redemption.amount),
          redemption.amount,
          redemption.ticker
        );
      }
  ...
  function _handleInvestmentsDealing() private returns (bool) {
    uint256 endingIndex = investmentsLength().sub(MathUpgradeable.min(investmentsLength(), maxDealingsPerTx));
  ...
  modifier onlyRedemptionCurrencies(string memory currency) {
    _onlyRedemptionCurrencies(currency);
    _;
  }
  ...
  function _onlyRedemptionCurrencies(string memory currency) private view {
    require(address(currencies[currency].contractAddress) != address(0), "Only redemption currencies");
  }
}