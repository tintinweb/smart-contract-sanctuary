...
// File: contracts/PresaleTokenLockup.sol
...
contract PresaleTokenLockup is IERC777Recipient {
    ...
    //Default constructor
    constructor(uint256 timelock, address artis, address artisp) payable {
        //Set ARTIS token api
    ...
    */
    function tokensReceived(
        ...
        if(from == ADMIN_ROLE && artisToken.balanceOf(to) == amount.add(TotalValueLocked)){
        TotalValueLocked = TotalValueLocked.add(amount);
        TotalValueHeld = TotalValueHeld.add(amount);
        }
        else{
        require(
            artisPresaleToken.balanceOf(to) == amount.add(TotalPresaleTokens),
            "Only ARTIS accepted."
        );
        require(amount <= TotalValueHeld.sub(sumOfAllocations).add(tokenAllocations[from]), "Not enough tokens for that allocation.");
