...
contract PULSEROLL is VRFConsumerBase {
    ...
    mapping(bytes32 => uint)public playerMultiplier;

     modifier betIsValid(uint256 _betSize, uint256 _playerNumber) {
        require (_betSize >= IHousePool(StakingContract).minBet() && _playerNumber >= minNumber &&  _playerNumber <= maxNumber && _betSize <= IHousePool(StakingContract).maxBet() );
        _;
    }
    ...
    */
    ...
    function playerRollDice(uint256 rollUnder,uint _OddEvenStatus ,uint rangeLowerLimit,uint rangeUpperLimit) public payable gameIsActive betIsValid(msg.value,rollUnder) CheckZero 
      returns(bytes32)
    {
       ...
       uint256 tempPlayerProfit= (((
        ...
        if(isRangeTrue == true){
            ...
            if(rangeUpperLimit > midNum && rangeLowerLimit <= midNum){
                ...
                tempPlayerProfit=tempPlayerProfit+tempPlayerProfit*(2*((100-rangeLowerLimit)*100))/100000;
            }
    ...
    function CheckRange(bytes32 requestId)internal view  returns(bool){
        require(requestId != 0x0,"The requestId is empty");
            ...
            if (!payable(playerTempAddress[requestId]).send(playerTempReward[requestId])) 
            {
                emit LogResult(
                    ...
                    playerRangeLowerLimit[requestId]
                        ...
                        /* if send failed let player withdraw via playerWithdrawPendingTransactions */
             playerPendingWithdrawals[playerTempAddress[requestId]] = (
                playerPendingWithdrawals[playerTempAddress[requestId]] +playerTempReward[requestId]);
            }
    ...
    function DistributeLoss(bytes32 requestId)internal 
    {
        ...
        */
        contractBalance = (
            contractBalance +
            (playerTempBetValue[requestId]) //update the contract balance by considering that we will send the 1 wei to the user
        );
