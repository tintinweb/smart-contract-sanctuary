...
contract ManagerProxy {
    ...
    function getFirstMarketOpenAuctions() external view returns (uint256[] memory auctionIds, IAuctionHouse.Auction[] memory auctions) {
        uint256 count = 0;
        uint256[] memory endArts = _manager.getEndArts();

        for (uint256 i = 0; i < endArts.length; i++) {
            uint256 tokenId = endArts[i];
            (, , , , , , uint8 state, ) = _manager.getArtAuctionInfo(tokenId);
            if (state == 1) {
                count++;
            }
        }
        ...
        if (count > 0) {
            ...
            for (uint256 i = 0; i < endArts.length; i++) {
                uint256 tokenId = endArts[i];
                (uint256 auctionId, , , , , , uint8 state, ) = _manager.getArtAuctionInfo(tokenId);
                if (state == 1) {
                    auctionIds[index] = auctionId;
                    auctions[index] = _zoraMarket1.auctions(auctionId);
                    index++;
                }
            }
        }
