     ...
     function mintLogic(uint256 _amount) private {
        ...
        for(uint256 i=1; i<_amount+1;i++){
            if(amountMinted < startFrom ){
                ...
                (_hp, _strength,_agility,_intelligence,_dexterity,_luck) = generateStats(6, randomResult[msg.sender]); //change back to VRF randomResult[msg.sender]
                ...
                //@dev assign stats, generateStats takes in the number of stats, the ID and a random number
                (_hp, _strength,_agility,_intelligence,_dexterity,_luck) = generateStats(6, randomResult[msg.sender]); //change back to VRF
...
library EnemyLib {
    ...
    function getEnemyStats(Enemy storage enemy) view internal returns (int256 hp, uint256 str){
