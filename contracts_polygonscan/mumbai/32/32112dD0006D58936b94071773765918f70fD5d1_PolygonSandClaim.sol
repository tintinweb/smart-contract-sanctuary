...
contract PolygonSandClaim is Ownable {
    ...
    constructor(
        ...
        childChainManagerProxy = childChainManagerProxyAddress;
    ...
    /**
     * @notice mint new sand if more fake sand is minted on Layer 2 during the remapping process
     */
    function mintClaimable() external onlyOwner {
        uint256 fakeSandTotalSupply = _fakePolygonSand.totalSupply();
        uint256 claimableTotalSupply = unclaimedSand() + claimedSand();
        require(fakeSandTotalSupply > claimableTotalSupply, "Claimable SAND already minted");
        _polygonSand.updateChildChainManager(address(this));
        _polygonSand.deposit(address(this), abi.encode(fakeSandTotalSupply - claimableTotalSupply));
        _polygonSand.updateChildChainManager(childChainManagerProxy);
        _polygonSand.transferOwnership(msg.sender);
    }
     ...
     */
    function claimedSand() public returns (uint256) {
...
interface IPolygonSand {
    ...
    /// @dev Transfers ownership of the contract to a new account (`newOwner`).
    ///      Can only be called by the current owner.
    /// @param newOwner address to which ownership will be transferred to.
