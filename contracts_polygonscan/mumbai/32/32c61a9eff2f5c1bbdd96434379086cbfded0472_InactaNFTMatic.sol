...
contract InactaNFT is Initializable, ReentrancyGuardUpgradeable, AccessControlEnumerableUpgradeable, ERC1155Upgradeable {
  ...
  function distributeNFTs(
    ...
    require(_amountETH * _recipients.length == msg.value, "inaccturate value supplied");
    for (uint256 i = 0; i < _recipients.length; i++) {
      address recipient = _recipients[i];
      if (_amountETH > 0) {
        (bool success, ) = recipient.call{value: _amountETH}("");
        require(success, "transfer failed");
      }
      _safeTransferFrom(_from, recipient, _tokenId, 1, "");
    }
  }
   ...
   */
  function updateURI(string calldata _newUri) external onlyRole(DEFAULT_ADMIN_ROLE) {
    _setURI(_newUri);
  }

  /**
   * @dev https://forum.openzeppelin.com/t/derived-contract-must-override-function-supportsinterface/6315
   */
