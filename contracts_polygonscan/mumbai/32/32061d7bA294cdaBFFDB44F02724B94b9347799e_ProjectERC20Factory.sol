    ...
    UUPSUpgradeable
{
    ...
    function initialize() public initializer {
        __Context_init_unchained();
        __ERC721_init_unchained('CO2KEN Project Collection', 'CO2KEN-PNFT');
        __Ownable_init_unchained();
        __Pausable_init_unchained();
    }
    ...
    function emergencyStop() public virtual onlyOwner {
        _pause();
    }
    ...
    function mapStorageMethod(
        string[] memory methodology,
        string[] memory storageMethod
    ) external onlyOwner whenNotPaused {
        uint256 methodologyLen = methodology.length;
        uint256 storageMethodLen = storageMethod.length;
        require(
            methodologyLen == storageMethodLen,
            'Error: Length of arrays do not match'
        );

        for (uint256 i = 0; i < methodologyLen; i++) {
            methodologyToStorageMethod[methodology[i]] = storageMethod[i];
        }
    }
    ...
    function addNewProject(
        ...
        require(
            metaDataHashes[metaDataHash] == false,
            'Error: metaDataHash already exists'
        );
    ...
    function updateProject(
        ...
        require(
            metaDataHashes[newMetaDataHash] == false,
            'Error: metaDataHash already exists'
        );
    ...
    function removeProject(uint256 tokenId) external onlyOwner whenNotPaused {
        ...
        )
    {
        return (
            ...
            methodologyToStorageMethod[projects[tokenId].methodology]
        );
    }

    // Retrieve data from ProjectNFT relevant for pools
    function getProjectDataByProjectId(string memory projectId)
        external
        view
        returns (
            string memory,
...
contract ProjectCollectionV2 is ProjectCollection {
    struct Data {
        bool var1;
        uint256 var2;
    }
...
contract ProjectCollectionV3 is ProjectCollection {
    struct Data {
        bool var1;
        uint256 var2;
        string text;
...
contract ProjectsStorageV2 is ProjectsStorage {
    string public dummyVar;
}

// Test for upgrade according to data separation pattern
contract ProjectCollectionV4 is ProjectCollectionV3, ProjectsStorageV2 {
...
contract ProjectERC20 is Context, ERC20, IERC721Receiver {
    // // A variable to cap the supply for a specific project-vintage-ERC20
    // uint256 public maxSupply;
    string public projectId;
    uint16 public vintage;
    // Project-vintage specific data
    bool public isCorsiaCompliant;
    bool public isCCPcompliant;
    string public coBenefits;
    string public correspAdjustment;
        ...
        )
    {
        address c = IContractRegistry(contractRegistry)
            .projectCollectionAddress();
        string memory standard;
        string memory methodology;
        string memory region;
        string memory storageMethod;
        (standard, methodology, region, storageMethod) = ProjectCollection(c)
            .getProjectDataByProjectId(projectId);
        return (vintage, standard, methodology, region);
    }
    ...
    function onERC721Received(
        ...
        (
            ...
            uint256 quantity,
            bool confirmed
        ) = IBatchCollection(msg.sender).getBatchNFTData(tokenId);
        require(
            checkMatchingAttributes(_projectId, _vintage),
    ...
    function checkWhiteListed(address collection) internal view returns (bool) {
        if (
            collection ==
            IContractRegistry(contractRegistry).batchCollectionAddress()
        ) {
            return true;
        } else {
            return false;
     ...
     **/
    function checkMatchingAttributes(string memory _projectId, uint16 _vintage)
        internal
        view
        returns (bool)
    {
    ...
    function mintBadgeNFT(address to, uint256 amount) public {
        address badgeAddr = IContractRegistry(contractRegistry)
            .retirementBadgeCollectionAddress();
        require(
            retiredAmount[msg.sender] >= amount,
            'Error: Cannot mint more than user has retired'
        );

        IRetirementBadgeCollection(badgeAddr).mintBadge(
            to,
            projectId,
            vintage,
            amount
...
contract ProjectERC20Factory is Ownable, Pausable {
    ...
    function deployFromTemplate(uint256 tokenId) public whenNotPaused {
        address collection = IContractRegistry(contractRegistry)
            .batchCollectionAddress();
        (string memory pid, uint16 vintage, , , ) = IBatchCollection(collection)
...
// Storage contract for ProjectCollection
...
contract ProjectsStorage {
    ...
    // Attribute-derived data, methodoloy is the key, storage method is value
    mapping(string => string) public methodologyToStorageMethod;
