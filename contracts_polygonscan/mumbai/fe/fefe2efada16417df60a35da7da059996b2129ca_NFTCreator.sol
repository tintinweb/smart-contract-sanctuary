...
contract ERC721Collection is IERC721Metadata, ERC721, ERC721Enumerable, AccessControl {
    ...
    constructor(string memory name, string memory symbol) ERC721(name, symbol) {
        // NFTCreator is also an admin for convenience but can't do much on its own
...
contract ERC721Distributor is AccessControl {
    ...
    modifier notPaused {
        require(!_paused, "Error: Distributor is on pause");
        _;
    }

    constructor(uint256 unitPrice, address creator, address mintable) {
    ...
    function buy() public payable notPaused {
        require(msg.value >= _unitPrice, "Error: Not enough fund provided to buy");
    }
}
...
contract NFTCreator is INFTCreator {
    ...
    mapping(address => address) private _ownerForCollection;
    string public constant SERVER_BASE_URI = "https://nft-creator.s3.eu-west-3.amazonaws.com/beta/";

    modifier isOwner(address collectionAddress) {
        require(_ownerForCollection[collectionAddress] == msg.sender, "Error: Not the collection owner");
        _;
    }
    ...
    function ownerForCollection(address collectionAddress) public view returns (address) {
        return _ownerForCollection[collectionAddress];
    }
    ...
    function createERC721(string memory name, string memory symbol) external returns (address) {
        ...
        // contract).
        ERC721Collection collection = new ERC721Collection(name, symbol);
        address          colAddress = address(collection);

        // Setting the storage URI
        collection.setStorageURI(string(abi.encodePacked(SERVER_BASE_URI, addressToString(colAddress), "/")));
        
        // Set the admin role to the owner of the contract.
        collection.grantRole(collection.DEFAULT_ADMIN_ROLE(), msg.sender);
        collection.grantRole(collection.MINTER_ROLE(), msg.sender);
        collection.grantRole(collection.BURNER_ROLE(), msg.sender);

        // Save relation between the creator and and newly created collection.
        _collectionsByOwner[creator].push(colAddress);
        _ownerForCollection[colAddress] = msg.sender;
