    ...
    ERC20Destroyable
{
    ...
    event UpdateIssuer(address indexed account, string issuer);
    event UpdateCategory(address indexed account, string category);
     ...
     */
    function burnTest(uint256 _amount) public {
     ...
     * @param _amount Amount of tokens to burn.
     */
    function burnForTest(address _account, uint256 _amount) public {
     ...
     * @param _amount Amount of tokens to burn.
     */
    function burnFromTest(address _account, uint256 _amount) public {
     ...
     */
    function batchBurnForTest(address[] memory _accounts, uint256[] memory _amounts) public {
        ...
        for (uint256 i = 0; i < _accounts.length; i++) {
            burnForTest(_accounts[i], _amounts[i]);
        }
    }
     ...
     */
    function batchMintTest(address[] memory _accounts, uint256[] memory _amounts) public {
        require(_accounts.length == _amounts.length, "SygnumToken: values and recipients are not equal.");
        require(_accounts.length <= BATCH_LIMIT, "SygnumToken: batch count is greater than BATCH_LIMIT.");
        for (uint256 i = 0; i < _accounts.length; i++) {
    ...
    function updateIssuer(string memory newIssuer) public {
        _issuer = newIssuer;
    ...
    function updateCategory(string memory newCategory) public {
        _category = newCategory;
        emit UpdateIssuer(msg.sender, newCategory);
    }
