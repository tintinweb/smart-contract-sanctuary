...
contract BogosBinted is ERC721, ERC721Enumerable, ERC721Burnable, Ownable {
    ...
    uint256 public constant MAX_MINTS_PER_TXN = 2;

    uint256 public mintPrice = 100000000 gwei; // 0.1 MATIC
    ...
    constructor(string memory name, string memory symbol, uint256 maxBogosSupply) ERC721(name, symbol) {
        maxTokenSupply = maxBogosSupply;

        _shareholders[0] = 0x7166e696E93d67B8B2962C85b3725cc93e14A4AA;

        _shares[0] = 10000;
    }

    function setMaxTokenSupply(uint256 maxBogosSupply) public onlyOwner {
        maxTokenSupply = maxBogosSupply;
    }
    ...
    * Bint the bogos
    */
    function mintBogoss(uint256 numberOfTokens) public payable {
        require(saleIsActive, "Sale must be active to mint Bogoss");
        require(numberOfTokens <= MAX_MINTS_PER_TXN, "You can only mint 16 Bogoss at a time");
        require(totalSupply() + numberOfTokens <= maxTokenSupply, "Purchase would exceed max available Bogoss");
