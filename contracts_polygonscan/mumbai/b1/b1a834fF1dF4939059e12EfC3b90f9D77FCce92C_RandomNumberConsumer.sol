...
contract RandomNumberConsumer is VRFConsumerBase {
    ...
    mapping(bytes32 => mapping (uint256 => uint256)) public randomindex;
    ...
    function viewrandomN() public view returns(uint256){
        return randomResult;
    }
    
    function getRandomResult(uint256 _tokenId) external view returns(uint256){
        return randoms[_tokenId];
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        ...
        randoms[_tokenId] = randomResult;
        randomindex[requestId][_tokenId] = randomResult;
        ERC721._setTokenURI(tokenIds[requestId], randoms[_tokenId]);
