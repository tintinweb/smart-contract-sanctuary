    ...
    ITangibleNFT
{
    ...
    function _produceTNFTtoStock(
        ...
        require(_brandApproved(brandName), "BNA");
    ...
    function setBrand(uint256 tokenId, string calldata brand)
        ...
        onlyFactoryOrFactoryAdmin
    {
        require(_brandApproved(brand), "BNA");
        tokenBrand[tokenId] = brand;
    }

    function _brandApproved(string calldata brand) internal returns (bool) {
        uint256 length = approvedBrands.length;
        for (uint256 i = 0; i < length; i++) {
            if (
                keccak256(abi.encodePacked(approvedBrands[i])) ==
                keccak256(abi.encodePacked(brand))
            ) {
                return true;
            }
        }
    ...
    function addApprovedBrand(string calldata brand)
        ...
        onlyFactoryOrFactoryAdmin
    {
        require(!_brandApproved(brand), "BAA");
