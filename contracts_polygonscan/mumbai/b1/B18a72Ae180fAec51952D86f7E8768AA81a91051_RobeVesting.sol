...
contract RobeVesting is Ownable, ReentrancyGuard {
        ...
        onlyIfVestingScheduleNotRevoked(vestingScheduleId)
    {
        ...
        uint256 claimableAmount = _computeVestableAmount(vestingSchedule);
        require(
    ...
    {
        uint256 currentTime = getCurrentTime();
        uint256 endDate = currentTime;

        if (vestingSchedule.end != 0) {
            endDate = vestingSchedule.end;
        }

        uint256 vestedDate = endDate.sub(vestingSchedule.start);
        uint256 vestedAmount = vestedDate.mul(emmissionRatePerSecond);
