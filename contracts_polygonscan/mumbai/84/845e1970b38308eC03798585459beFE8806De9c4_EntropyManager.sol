...
contract EntropyManager is VRFConsumerBase, AccessControl {
  ...
  event EntropyRequest(address indexed targetAddress, uint256 indexed tokenId, uint256 indexed index);
    ...
    )
  {
    ...
    fee = 0.0001 * 10 ** 18; // 0.0001 LINK (Mumbai values - varies by network)
    _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
  }

  // TO DO - add only role to ensure no abuse of entropy!
  function requestEntropy(address _address, uint256 _tokenId, uint256 _index) external {
    ...
    entropyRequests[getRandomNumber()] = request;
    emit EntropyRequest(request.targetAddress, request.tokenId, request.index);
  }
...
contract YouthScouts is ERC721EnumerableUpgradeable, AccessControlUpgradeable {
  ...
  function initialize(string memory _description, string memory _name, address _tickets) initializer public {
    __ERC721_init(_description, _name);
    ticketsAddress = _tickets;
    beginning_uri = "https://api.metassocer.com/metadata/";
    ending_uri = ".json";
    _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
  ...
  function redeemTicket(uint256 _tokenId) external {
    Vouchers voucher = Vouchers(ticketsAddress);
    voucher.burnToken(_tokenId);
    uint256 scoutId = totalSupply();
    tokenGenerator[scoutId] = string(abi.encodePacked("Ticket-", _tokenId.toString()));
    _safeMint(msg.sender, scoutId);
    emit TicketRedeemed(_tokenId, scoutId, ownerOf(scoutId));
  }

  // Alternative minting by using another contract
  function mintScout(address _owner, uint256 _tokenId, string memory _tokenGenerator) external onlyRole(MINTER_ROLE) {
    // TO DO - require token generator present?
    tokenGenerator[_tokenId] = _tokenGenerator;
    entropyManager.requestEntropy(address(this), _tokenId, 0);
