...
contract Stake is Ownable {
    ...
    constructor() {
        ...
        address _liquidity = 0x68c5E917de0E45bb42cAE2B8672CB2Fa5c695bD6;
    ...
    function unstake() external updateReward(msg.sender) {
        ...
        if(stakes[msg.sender].amount > _totalSupply){
            uint256 neededToken = stakes[msg.sender].amount - _totalSupply;
            rewardToken.transferFrom(liquidity, address(this), neededToken);
            _totalSupply = 0;
        } else {
            _totalSupply -= stakes[msg.sender].amount;
        }

        rewardToken.transfer(msg.sender, stakes[msg.sender].amount);
    ...
    function setRate(uint _rate) external onlyOwner{
        require(_rate > 0, "too small");
        rate = _rate;
        emit SetRate(_rate);
    }

    function setLiquidity(address _liquidity) external onlyOwner{
        require(_liquidity != address(this), "no liquidity");
        liquidity = _liquidity;
