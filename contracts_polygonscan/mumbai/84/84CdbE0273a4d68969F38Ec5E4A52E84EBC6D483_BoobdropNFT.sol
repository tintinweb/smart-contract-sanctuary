...
// File contracts/BoobdropNFT.sol
...
contract BoobdropNFT is ERC721URIStorage, Ownable, AccessControl, IChildToken {
  string public constant BOOBGEN_SOURCE_HASH = "TODO";
  string public constant BOOBGEN_SOURCE_URI = "TODO";
  string public constant BOOBGEN_SOURCE_IPFS = "TODO";

  string public constant PLACEHOLDER_URI = "TODO";
  ...
  mapping (uint256 => string) private _tokenURIs;
  mapping(uint256 => string) private _tokenSeeds;
  ...
  /**
   * Will be minted with some generic default metadata
   * admin must then update with generated metadata from the token hash
   * Should verify if token is withdrawn by checking `withdrawnTokens` mapping
   * @param user user for whom tokens are being minted
   */
  function mint(address user) public payable {
    require(_tokenIds.current() <= MAX_TOKENS, "All tokens have been minted already");
    require(msg.value >= 15 ether, "Not enough MATIC sent; mint price is 15 MATIC");

    _tokenIds.increment();
    uint256 newItemId = _tokenIds.current();
    _mint(user, newItemId);
    _setTokenURI(newItemId, PLACEHOLDER_URI);
  }

  /**
   * Set the final generated metadata URI for the token. Only callable by admin
   * It then freezes the metadata permanently
   * @param tokenId Token for which URI to be set
   * @param seed string the seed used to generate the image. Derived from the mint tx hash and mint block hash
   * @param uri metadata uri
   */
  function setFinalUri(uint256 tokenId, string memory seed, string memory uri) public onlyRole(DEFAULT_ADMIN_ROLE) {
  ...
  function _setTokenURI(uint256 tokenId, string memory uri) internal virtual override {
    require(_frozenTokens[tokenId] != true, "Cannot set token URI: Token URI is frozen");
