...
library SVGShapes {
    ...
    function generateSVG(uint256 _randomNumber) internal pure returns (string memory finalSVG) {
        ...
        for(uint i=0; i < numberOfShapes; i++){
            ...
            string memory pathSvg = generateShape(newRNG); //this is what needs to be replaced first with a shape choosing function
    ...
    function generateShape(uint256 _randomNumber) internal pure returns(string memory pathSvg) {
        ...
        if (_randomNumber % 2 == 0) {
            pathSvg = string(abi.encodePacked(pathSvg, "<circle cx='36' cy='68' r='43' stroke=rgb(225,158,65) stroke-width='46' fill=transparent />"));
            //draw a circle
            ...
            //pathSvg = string(abi.encodePacked(pathSvg, "<circle cx='", string(Strings.toString(circleVars[0] % 500)), "' cy='", string(Strings.toString(circleVars[1] % 500)), "' r='", string(Strings.toString(circleVars[2] % 250)), "' stroke=rgb(", string(Strings.toString(circleVars[3] % 255)), ",", string(Strings.toString(circleVars[4] % 255)), ",", string(Strings.toString(circleVars[5] % 255)), ") stroke-width='", string(Strings.toString(circleVars[6] % 100)), "' fill=transparent />"));
                ...
                pathSvg = string(abi.encodePacked(pathSvg, "<rect x='429' y='215' width='89' height='60' fill=rgb(110,216,124) />"));
            //draw a square
...
//contract RandomSVG is ERC721URIStorage, VRFConsumerBase, Strings {
contract RandomSVG is ChainlinkClient, ERC721URIStorage, VRFConsumerBase {
    // Library declaration for strings
    using Strings for string;
    ...
    mapping(bytes32 => address) public requestIdToSender; //this can be internal for live implemintation
    ...
    event CreatedRandomSVG(uint256 indexed tokenId, string tokenURI);
     ...
     constructor(address _VFRCoordinator, address _LinkToken, bytes32 _keyHash, uint256 _fee, address _oracle)
     ...
     ERC721 ("RandomSVG", "rsNFT") {
        ...
        tokenCounter = 0;
        oracle = _oracle;
        jobId = '54bd0a3d8fb54da6bd8da32b17912ed3';
    ...
    function finishMint (uint256 _tokenId) public {
        ...
        //require(tokenIdToRandomWord[_tokenId] > 0, "Need to wait for External Adapter"); - remove this line
