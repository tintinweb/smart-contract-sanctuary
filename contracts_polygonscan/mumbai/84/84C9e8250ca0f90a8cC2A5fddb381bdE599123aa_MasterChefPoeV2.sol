...
// File: contracts/MasterChefPoeV2.sol
...
interface IRewarder {
    ...
    function onPoeReward(address user, uint256 newLpAmount) external;
...
// MasterChefPoe is a boss. He says "go f your blocks lego boy, I'm gonna use timestamp instead".
...
// will be transferred to a governance smart contract once POE is sufficiently
// distributed and the community can show to govern itself.
//
// With thanks to the Lydia Finance team.
//
// Godspeed and may the 10x be with you.
contract MasterChefPoeV2 is Ownable {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of POEs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accPoePerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accPoePerShare` (and `lastRewardTimestamp`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. POEs to distribute per second.
        uint256 lastRewardTimestamp; // Last timestamp that POEs distribution occurs.
        uint256 accPoePerShare; // Accumulated POEs per share, times 1e12. See below.
        IRewarder rewarder;
    }
    ...
    // POE tokens created per second.
    uint256 public poePerSec;
    ...
    // The timestamp when POE mining starts.
    ...
    event UpdateEmissionRate(address indexed user, uint256 _poePerSec);
    ...
    constructor(
        ...
        _devAddr = 0x8AFEB0EFB6c3245D59d367a8595C23B4f068517a;
        _treasuryAddr = 0x8AFEB0EFB6c3245D59d367a8595C23B4f068517a;
        _investorAddr = 0x8AFEB0EFB6c3245D59d367a8595C23B4f068517a;
        _poePerSec = 1;
        _startTimestamp = 1640579593;
        _devPercent = 20;
        _treasuryPercent = 10;
        _investorPercent = 10;
        require(
    ...
    // Update the given pool's POE allocation point. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        IRewarder _rewarder,
    ...
    // View function to see pending POEs on frontend.
        ...
        )
    {
        ...
        uint256 accPoePerShare = pool.accPoePerShare;
        ...
        if (block.timestamp > pool.lastRewardTimestamp && lpSupply != 0) {
            ...
            uint256 lpPercent = 1000 -
                ...
                treasuryPercent -
                investorPercent;
            uint256 poeReward = multiplier
                .mul(poePerSec)
                .mul(pool.allocPoint)
                .div(totalAllocPoint)
                .mul(lpPercent)
                .div(1000);
            accPoePerShare = accPoePerShare.add(
                poeReward.mul(1e12).div(lpSupply)
            );
        }
        pendingPoe = user.amount.mul(accPoePerShare).div(1e12).sub(
            user.rewardDebt
        );
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = block.timestamp.sub(pool.lastRewardTimestamp);
        uint256 poeReward = multiplier.mul(poePerSec).mul(pool.allocPoint).div(
            totalAllocPoint
        );
        uint256 lpPercent = 1000 -
            devPercent -
            treasuryPercent -
            investorPercent;
        poe.mint(devAddr, poeReward.mul(devPercent).div(1000));
        poe.mint(treasuryAddr, poeReward.mul(treasuryPercent).div(1000));
        poe.mint(investorAddr, poeReward.mul(investorPercent).div(1000));
        poe.mint(address(this), poeReward.mul(lpPercent).div(1000));
        pool.accPoePerShare = pool.accPoePerShare.add(
            poeReward.mul(1e12).div(lpSupply).mul(lpPercent).div(1000)
        );
        pool.lastRewardTimestamp = block.timestamp;
        emit UpdatePool(
            _pid,
    ...
    // Deposit LP tokens to MasterChef for POE allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            ...
            uint256 pending = user
                .amount
                .mul(pool.accPoePerShare)
                .div(1e12)
                .sub(user.rewardDebt);
            safePoeTransfer(msg.sender, pending);
            emit Harvest(msg.sender, _pid, pending);
        }
        user.amount = user.amount.add(_amount);
        user.rewardDebt = user.amount.mul(pool.accPoePerShare).div(1e12);

        IRewarder rewarder = poolInfo[_pid].rewarder;
        if (address(rewarder) != address(0)) {
            rewarder.onPoeReward(msg.sender, user.amount);
        }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        // Harvest POE
        uint256 pending = user.amount.mul(pool.accPoePerShare).div(1e12).sub(
            user.rewardDebt
        );
        safePoeTransfer(msg.sender, pending);
        emit Harvest(msg.sender, _pid, pending);
    ...
    // Safe poe transfer function, just in case if rounding error causes pool to not have enough POEs.
    function safePoeTransfer(address _to, uint256 _amount) internal {
        uint256 poeBal = poe.balanceOf(address(this));
        if (_amount > poeBal) {
            poe.transfer(_to, poeBal);
        } else {
            poe.transfer(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _poePerSec) public onlyOwner {
        massUpdatePools();
        poePerSec = _poePerSec;
        emit UpdateEmissionRate(msg.sender, _poePerSec);
    }
}