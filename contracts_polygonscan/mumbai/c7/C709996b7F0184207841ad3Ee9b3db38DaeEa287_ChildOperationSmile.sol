...
contract ChildOperationSmile is ERC721, Ownable, ContextMixin {
    ...
    function airdrop(address[] calldata to) external onlyOwner {
        for (uint256 i = 0; i < to.length; i++) {
            uint256 tokenId = _tokenIds + OG_FLOWERS;

            _safeMint(to[i], tokenId);
            _plantedAt[tokenId] = block.timestamp;
    ...
    function deposit(address user, bytes calldata depositData) external {
        require(_msgSender() == _childChainManager, "Caller is not child chain manager");
    ...
    function encodeTokenMetadata(uint256 tokenId) external view virtual returns (bytes memory) {
        return abi.encode(tokenURI(tokenId), _plantedAt[tokenId]);
    }
