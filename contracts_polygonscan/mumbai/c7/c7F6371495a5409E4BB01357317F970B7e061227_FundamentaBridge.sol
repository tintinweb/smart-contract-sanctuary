...
contract FundamentaBridge is SecureContract
{
    ...
    event Deposit(uint256 indexed nonce, address sender, uint256 indexed sourceNetwork, uint256 destinationNetwork, uint256 indexed sourceToken, uint256 destinationToken, uint256 amount, uint256 fmtaFee, uint256 tokenFee);
    event Withdraw(uint256 indexed nonce, address sender, uint256 sourceNetwork, uint256 indexed destinationNetwork, uint256 sourceToken, uint256 indexed destinationToken, uint256 amount, uint256 fmtaFee, uint256 tokenFee);
        ...
        returns(address, uint256, uint256, uint256, uint256)
    {
        ...
        TokenData storage token = _tokens[destToken];
        ...
        returns(address, uint256, uint256, uint256, uint256)
    {
        ...
        TokenData storage token = _tokens[srcToken];
        require(token.info.token != address(0), "Bridge: Token not registered");
    ...
    function deposit(bytes memory clientSignature, bytes[] memory serverSignatures, bytes memory transactionData) public pause
    {
        ...
        uint256 fee = calculateDepositFee(destToken, amount);
        ...
        emit Deposit(nonce, sender, srcNetwork, destNetwork, srcToken, destToken, amount, token.fmtaDepositFee, fee);
    }
    ...
    function withdraw(bytes memory clientSignature, bytes memory transactionData) public pause
    {
        ...
        uint256 fee = calculateWithdrawFee(srcToken, amount);
        ...
        emit Withdraw(nonce, sender, srcNetwork, destNetwork, srcToken, destToken, amount, token.fmtaWithdrawFee, fee);
    }
