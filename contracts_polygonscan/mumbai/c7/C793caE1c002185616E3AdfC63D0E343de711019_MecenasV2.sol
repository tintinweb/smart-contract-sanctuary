...
contract MecenasV2 is ERC20m, ReentrancyGuard {
    ...
    uint[] public jackpotcounter;
    uint[] public jackpotresults;
    ...
    function pickawinner() external nonReentrant {
        ...
        if (block.number - blockNumber > 250) {
            ...
            jackpotcounter.push(lotterycounter);
            jackpotwinners.push(EMPTY_ADDRESS);
            jackpotwinnersamount.push(0);
            jackpotwinnersdate.push(end);
            jackpotresults.push(2);
    ...
    function getjackpotcounter() external view returns(uint[] memory) {
        return jackpotcounter;
    }
    
    
    function getjackpotresults() external view returns(uint[] memory) {
