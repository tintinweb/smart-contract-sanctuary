...
contract L2VaultConfig is VaultConfigBase, IL2VaultConfig {
    ...
    */
    ...
    function getSupportedAMM(uint256 ammID)
        external
        view
        override
        returns (address)
    ...
    function changeRemoteTokenRatio(
        ...
        override
        onlyOwner
        validAmount(remoteTokenRatio[remoteNetworkID][tokenAddress])
    {
        remoteTokenRatio[remoteNetworkID][tokenAddress] = _remoteTokenRatio;
    }
        ...
        validAddress(tokenAddress)
    {
        ...
        require(whitelistedTokens[tokenAddress].underlyingReceiptAddress == address(0), "Token already whitelisted");
    ...
    {
        require(_remoteNetworkID > 0, "Invalid network ID");
        require(whitelistedTokens[_tokenAddress].underlyingReceiptAddress != address(0), "Token not whitelisted");

        delete remoteTokenAddress[_remoteNetworkID][_tokenAddress];
        delete remoteTokenRatio[_remoteNetworkID][_tokenAddress];

        emit RemoteTokenRemoved(
            _tokenAddress,
            _remoteNetworkID
        );
    }
    ...
    function removeWhitelistedToken(
        ...
        override
        onlyOwner
        onlySupportedRemoteTokens(_remoteNetworkID, _tokenAddress)
    {
        require(_remoteNetworkID > 0, "Invalid network ID");
        emit TokenWhitelistRemoved(
            _tokenAddress,
            _remoteNetworkID
        );
...
interface IL2VaultConfig is IVaultConfigBase {
    ...
    function removeWhitelistedToken(address _token, uint256 _remoteNetworkID)
        external;
