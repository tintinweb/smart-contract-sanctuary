  ...
  Pausable
{
  ...
  mapping(address => uint256[]) ownedShips;
  // mapping of ship ID to index in ownedShips array
  mapping(uint256 => uint256) ownedShipIndex;
  uint256[] _allPlanets;

  mapping(uint256 => Planet) planets;
  mapping(address => uint256[]) ownedPlanets;
  mapping(uint256 => uint256) ownedPlanetIndex;
  mapping(uint256 => uint256[]) shipsOnPlanet;
