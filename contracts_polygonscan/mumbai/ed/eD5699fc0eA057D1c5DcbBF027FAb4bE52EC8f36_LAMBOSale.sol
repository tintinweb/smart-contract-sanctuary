...
contract LAMBOSale is Ownable {

    using SafeERC20 for ERC20;
    using Address for address;

    uint constant DAIdecimals = 10 ** 9;
    uint constant LAMBOdecimals = 10 ** 9;
    uint public constant MAX_SOLD = 92000 * LAMBOdecimals;
    uint public constant PRICE = 10 * DAIdecimals / LAMBOdecimals ;
    uint public constant MAX_SALE_PER_ACCOUNT = 200 * LAMBOdecimals;
    uint public constant MAX_PRESALE_PER_ACCOUNT = 200 * LAMBOdecimals;
    ...
    constructor( address _dao, uint _presaleTimestamp, address dai_address) {
        dao = _dao;
    ...
    function isPublicSale() public view returns (bool) {
      if (presaleTimestamp + 1 * 60 <= block.timestamp) {
        return true;
      }
      return false;
    ...
    function buyLAMBO(uint256 amount) public onlyEOA {
    ...
    function claimLAMBO() public onlyEOA {
      ERC20(LAMBO).transfer(msg.sender, invested[msg.sender]);
      owed -= invested[msg.sender];
      invested[msg.sender] = 0;

    }
    function setClaimingActive(address lambo) public {
