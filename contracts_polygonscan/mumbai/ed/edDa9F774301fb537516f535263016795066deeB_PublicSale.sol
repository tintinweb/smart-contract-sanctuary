...
contract PublicSale is Ownable{
    ...
    **/
    ...
    function ownerWithdraw() public onlyOwner {
        ...
        require(token.transfer(_msgSender(), maxVolume - currentVolume),"Transfer from contract to sender");
    ...
    function addToWhiteList(address _address, uint256 _value) public onlyOwner {
        ...
        whitelist[_address].isClaimed = false;
    ...
    function claim() public returns (bool){
        ...
        require(!whitelist[_msgSender()].isClaimed, "Claimed!");
        require(whitelist[_msgSender()].amount > 0, "Nothing to claim");

        require(token.transfer(_msgSender(), whitelist[_msgSender()].amount),"Transfer Failed");
        whitelist[_msgSender()].isClaimed = true;

        emit Claim(_msgSender(),whitelist[_msgSender()].amount);
        return true;
    }
    ...
    function getLockingAddress(address _address) public view returns (uint256 amount, bool isClaimed) {
        return(whitelist[_address].amount, whitelist[_address].isClaimed);
    }
}