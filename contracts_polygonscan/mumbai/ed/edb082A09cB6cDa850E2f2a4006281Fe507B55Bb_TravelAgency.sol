...
import "./Planet.sol";
import "./Passport.sol";
import "./agency_action/AgencyAction.sol";
import "./Stardust.sol";
...
contract TravelAgency is AccessControl, Pausable, ReentrancyGuard {
    /// Configuration for a Planet.
    struct Config {
        /// Fee in WETH to charge per visit.
        uint256 fee;
        /// Whether the Planet is open for visits.
        bool available;
    }
    ...
    Planet public planetContract;

    /// Location of Passport contract.
    Passport public passportContract;
    ...
    /// Ratio of Stardust tokens rewarded to WETH charge for each agencyVisitPlanet, in basis points.
    ...
    /// Mapping of Planet IDs to their status, namespaced by planet owners.
    mapping(address => mapping(uint256 => Config)) private planetConfig;
    ...
    mapping(bytes32 => AgencyAction) public magicToActions;

    /// One action to be performed after a Visit.
    struct Action {
        bytes32 magic;
        bytes32[] proof;
        bytes data;
    ...
    /// Mapping of Planet States to bonus Stardust rate.
    /// This is ADDED to the reward basis points.
    mapping(uint256 => uint256) public planetStateBonusBp;

    /// Toggles Stardust rewards on and off (initially false).
    /// When this is set to `false`, no Stardust will be minted for any visit.
    bool public rewardsEnabled;

    /// Event emitted when the Travel Agency is used.
    /// @param from the address that initiated the Visit
    /// @param planetOwner the owner of the Planet that was visited
    ...
    /// Event emitted when a Planet's config is updated.
    /// @param from the address that updated the config
    /// @param planetId the token ID of the Planet
    /// @param fee the new fee charged per visit
    /// @param available whether the planet is available for visit
    event UpdatePlanetConfig(
        address indexed from,
        uint256 indexed planetId,
        uint256 fee,
        bool available
    ...
    /// Event emitted when a Magic mapping is set.
    /// @param magic the 32-byte magic identifier
    /// @param action the destination action address
    event SetAction(bytes32 indexed magic, address indexed action);

    /// Event emitted when an action is triggered.
    /// @param magic the 32-byte magic identifier
    /// @param action the destination action address
    event TriggerAction(bytes32 indexed magic, address indexed action);

    constructor(
        uint256 _operatorFeeBp,
        Planet _planetContract,
        Passport _passportContract,
    ...
    /// Updates the ratio of Stardust tokens rewarded to WETH spent for each agencyVisitPlanet.
    /// @param _stardustRewardBp the new reward ratio
    function setStardustRewardBp(uint256 _stardustRewardBp)
        external
    ...
    /// Sets a planetState to stardustReward pairing.
    /// This is "extra" basis points that will be rewarded for WETH spent.
    /// @param _planetState the target planet state
    /// @param _stardustReward the new Stardust reward
    function setPlanetStateBonusBp(
        uint256 _planetState,
        uint256 _stardustReward
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        planetStateBonusBp[_planetState] = _stardustReward;
    }

    /// Enable or disable Stardust rewards.
    /// @param _rewardsEnabled new state of rewards (on or off)
    function setRewardsEnabled(bool _rewardsEnabled)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    function setOperatorFeeBp(uint256 _operatorFeeBp)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        operatorFeeBp = _operatorFeeBp;
    ...
    function setActionForMagic(bytes32 magic, AgencyAction action)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        magicToActions[magic] = action;
        emit SetAction(magic, address(action));
    }

    /// Visit a Planet onto a Passport.
    /// Note: this requires delegationApproval from Passport for this contract, otherwise it will revert.
    /// Note: this requires delegationApproval from Planet for this contract, otherwise it will revert.
    /// Note: this requires ERC-20 spending approval for WETH, otherwise it will revert.
    /// @param planetId token ID of the planet to visit.
    /// @param passportId token ID of the Passport that will used for visit.
    ...
    function agencyVisitPlanet(
        ...
        address planetOwner = planetContract.ownerOf(planetId);

        require(
            planetConfig[planetOwner][planetId].available,
            "Planet not available"
        );
        uint256 fee = planetConfig[planetOwner][planetId].fee;
        ...
        passportContract.visitPlanet(passportId, planetId, stampId);

        // Perform actions
        Action memory action;
        for (uint256 i = 0; i < actions.length; i++) {
            action = actions[i];
            AgencyAction dest = magicToActions[action.magic];
            ...
            emit TriggerAction(action.magic, address(dest));
        }

        // Stardust will only be dispensed if ALL of these rules are met:
        // 1) The Travel Agency contract has MINTER_ROLE for Stardust
        // 2) Rewards are enabled
        // 3) WETH has been spent
        if (
            stardustContract.hasRole(keccak256("MINTER_ROLE"), address(this)) &&
            rewardsEnabled &&
            spent > 0
        ) {
            // Counter for rewards
            ...
            // If WETH-scaling rewards are enabled
            if (stardustRewardBp != 0) {
                reward += (spent * stardustRewardBp) / BASIS_POINTS_DIVISOR;
            }

            uint256 bonusBp = planetStateBonusBp[
                planetContract.getPlanetState(planetId)
            ];
            if (bonusBp != 0) {
                reward += (spent * bonusBp) / BASIS_POINTS_DIVISOR;
            }

            stardustContract.mint(msg.sender, reward);
        }

        emit UseAgency(msg.sender, planetOwner, planetId, passportId);
    }
    ...
    /// Updates the configuration of the Planet.
    /// @param id the token ID of the planet
    /// @param config the new Config of the planet
    function setPlanetConfig(uint256 id, Config calldata config) external {
        planetConfig[msg.sender][id] = config;
        emit UpdatePlanetConfig(msg.sender, id, config.fee, config.available);
    }

    /// Get the status of a Planet.
    /// @param id the token ID of the planet
    /// @param owner the owner of the planet
    function getPlanetConfig(uint256 id, address owner)
        public
        view
        returns (Config memory)
    {
        return planetConfig[owner][id];
    }

    /// Get the current status of a Planet, which depends on its current owner.
    /// @param id the token ID of the planet
    function getPlanetConfigCurrent(uint256 id)
        public
        view
        returns (Config memory)
    {
        address owner = planetContract.ownerOf(id);
        return planetConfig[owner][id];
    }

    /// Withdraws the accured operator fees to a designated address.
    /// @param to the address to send fees to
    function withdrawOperatorFees(address to)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    Pausable
{
    ...
    /// Override for planet state.
    struct Override {
        /// The state to override to.
        uint256 state;
        /// Whether the override should be applied.
        bool enabled;
    }

    /// Minter role. An address with this role is allowed to mint Planets.
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");

    /// State update role. An address with this role is allowed to update the state of Planets.
    ...
    /// Mapping of whether planet ID has been claimed.
    mapping(uint256 => bool) public claimed;

    /// The default state for a Planet.
    uint256 private defaultState;

    /// An override for a Planet state.
    mapping(uint256 => Override) private overrideState;

    /// Location of Items contract.
    PlanetPassItems public planetPassItemsContract;

    /// Token ID for the customisation coupon.
    uint256 public customizationItemId;

    /// Event emitted when the override state of a Planet is updated.
    ...
    /// Event emitted when the default state is updated.
    ...
    /// Updates the token ID for the customisation coupon.
    /// @param _customizationItemId the new location of the Planet contract
    ...
    /// @param tokenId the token ID of the Planet being claimed
    ...
    function claim(
        ...
        // Make sure planet has not been claimed already
        require(!claimed[tokenId], "Already claimed");

        // Make sure merkle proof is valid
        require(_verify(_leaf(tokenId, msg.sender), proof), "Bad merkle proof");
    ...
    /// @param tokenId the token ID of the leaf
    ...
    /// @param to the address to send the Planet to
    /// @param tokenId the token ID of the Planet to mint
    function safeMint(
        address to,
        uint256 tokenId,
        Data calldata data
    ...
    /// @param to the address to send the Planet to
    /// @param tokenId an array of token IDs of Planets to mint
    /// @param data an array of data for tokens
    function safeMint(
        address to,
    ...
    /// Update the default state of Planets.
    /// @param _defaultState the new default state
    ...
    /// Update the override for Planet states.
    /// @param tokenId the token IDs of Planets
    /// @param _overrideState the override state to update to
    ...
    /// Allows owner to change a Planet's data. For every data update, one customization item is spent.
    /// @param tokenId the token ID of the planet
    /// @param data the new name
    ...
    /// Allows owner to change Planets' data. For every data update, one customization item is spent.
    /// @param tokenId array of token IDs of planets
    /// @param data an array of new data of tokens
    ...
    Pausable
{
    ...
    /// Minter role. An address with this role is allowed to mint Passports without expending an item.
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
    ...
    bytes32 public constant PASS_TEMPLATE_IDENT = keccak256("PASS_TEMPLATE");
    ...
    /// Mapping of Passports to array of visits.
    mapping(uint256 => Visit[]) private visits;

    /// Mapping of Passports to the Template they are.
    mapping(uint256 => uint256) private passTemplates;
    ...
    /// @param passportId Passport that was stamped
    /// @param planetId Planet that was stamped
    /// @param planetState state of the Planet at the time of stamping
    /// @param stampId token ID of the stamp used for the visit
    event Stamp(
        address indexed from,
    ...
    /// Event emitted when Passport style is updated.
    /// @param passportId token ID of the Passport
    /// @param templateId new token ID of the Template
    event SetTemplate(uint256 indexed passportId, uint256 indexed templateId);

    /// Only allow the owner of a token to call method
    modifier onlyTokenOwner(uint256 tokenId) {
        require(ownerOf(tokenId) == msg.sender, "Not owner of token");
        _;
    }

    /// Only allow owner of a token or approved delegator to call method.
    /// @param tokenId the token ID
    modifier onlyTokenOwnerOrDelegate(uint256 tokenId) {
        require(
            isOwnerOrDelegate(tokenId, msg.sender),
            "Not owner of token or approved"
        );
        _;
    }
    ...
    /// Set the template used on a Passport.
    /// @param tokenId the token ID of the Passport
    /// @param templateId the token ID of the template used
    function _setPassTemplate(uint256 tokenId, uint256 templateId) internal {
        passTemplates[tokenId] = templateId;
        emit SetTemplate(tokenId, templateId);
    }

    /// Get the template of a Passport.
    /// @param tokenId the token ID of the Passport
    function templateOf(uint256 tokenId) public view returns (uint256) {
        require(_exists(tokenId), "Query for nonexistent token");
        return passTemplates[tokenId];
    }

    /// Override upgrade the Template of a Passport. This does not consume any items.
    /// Note: This method does not check whether the new template is the same as the old template.
    /// Note: This method does not check whether the new template is of a higher rarity than the old template.
    /// Note: This does not check whether the the templateId is a PASS_TEMPLATE.
    /// @param tokenId the token ID of the Passport
    /// @param templateId the new token ID of the template
    function overrideUpgradePass(uint256 tokenId, uint256 templateId)
        external
        onlyRole(MINTER_ROLE)
        onlyTokenOwner(tokenId)
        tokenExists(tokenId)
    {
        _setPassTemplate(tokenId, templateId);
    }

    /// Upgrade the Template of a Passport.
    /// Note: This method does not check whether the new template is the same as the old template.
    /// Note: This method does not check whether the new template is of a higher rarity than the old template.
    /// @param tokenId the token ID of the Passport
    /// @param templateId the new token ID of the template
    function upgradePass(uint256 tokenId, uint256 templateId)
        external
        whenNotPaused
        onlyTokenOwner(tokenId)
        tokenExists(tokenId)
    {
        // Make sure it's a PASS_TEMPLATE
        require(
            planetPassItemsContract.tokenIsType(
                templateId,
                PASS_TEMPLATE_IDENT
            ),
            "Item not a Template"
        ...
        // Make sure sender owns the Template
        require(
    ...
    /// Override mint a new Passport. This does not consume any items.
    /// Note: This does not check whether the the templateId is a PASS_TEMPLATE.
    /// @param to the address to send the Passport to
    /// @param data the data for the token
    /// @param templateId the token ID of the template to use
    function overrideSafeMint(
        address to,
        Data calldata data,
        uint256 templateId
    ) external onlyRole(MINTER_ROLE) {
        _safeMint(to, _tokenIdCounter.current());
        _setData(_tokenIdCounter.current(), data);
        _setPassTemplate(_tokenIdCounter.current(), templateId);
    ...
    /// Gets all visits of a Passport.
    /// @param tokenId the token ID of the Passport
    /// @return an array of all visits
    function getVisits(uint256 tokenId) external view returns (Visit[] memory) {
        require(_exists(tokenId), "Query for nonexistent token");
        return visits[tokenId];
    }

    /// Get the visit of a Passport at a given index.
    /// @param tokenId the token ID of the Passport
    /// @param index the index of the element
    function getVisitAtIndex(uint256 tokenId, uint256 index)
        external
        view
        returns (Visit memory)
    {
        require(_exists(tokenId), "Query for nonexistent token");
        return visits[tokenId][index];
    }
    ...
    {
        return
            Visit(planetId, planetContract.getPlanetState(planetId), stampId);
    }
    ...
    function _visitPlanet(
        ...
        visits[tokenId].push(_makeVisit(planetId, stampId));
    ...
    /// Visit a planet.
    /// Note: This requires approval for the contract to burn an item from Passport owner. Otherwise it will revert.
    /// Note: An item of token Id `stampId` will be burned from the Passport owner.
    /// Note: This requires the Passport to be either owned by `msg.sender` or Delegation approved.
    /// Note: This requires the Planet to be either owned by `msg.sender` or Delegation.
    /// @param tokenId the token ID of the Passport
    /// @param planetId the token ID of the Planet to visit
    /// @param stampId the token ID of the stamp to use
        ...
        tokenExists(tokenId)
    {
        ...
        // Make sure Planet can be visited
        require(
            planetContract.isOwnerOrDelegate(planetId, msg.sender),
            "Not owner of Planet nor approved"
        );

        // Make sure stamp item is a stamp
        require(
            planetPassItemsContract.tokenIsType(stampId, STAMP_IDENT),
            "Item not a Stamp"
        );
        // Make sure sender has the stamp type
        require(
            planetPassItemsContract.balanceOf(passportOwner, stampId) > 0,
            "Item not owned"
        );

        // Burn the stamp
        planetPassItemsContract.burn(passportOwner, stampId, 1);

        _visitPlanet(tokenId, planetId, stampId);
    }

    /// Allows owner to change a Passport's data. For every data update, one customization item is spent.
    /// @param tokenId the token ID of the Passport
    /// @param data the new data of the token
    function setData(uint256 tokenId, Data calldata data)
        public
        virtual
        override
    ...
    /// Allows owner to change Passports' data. For every data update, one customization item is spent.
    /// @param tokenId array of token IDs of Passport
    /// @param data an array of new data of tokens
    function setData(uint256[] calldata tokenId, Data[] calldata data)
        public
        virtual
...
abstract contract AgencyAction is AccessControl {
    /// Role for managing parameters related to Action.
    bytes32 public constant ACTION_MANAGER_ROLE =
        keccak256("ACTION_MANAGER_ROLE");

    /// Location of the Travel Agency contract
    TravelAgency public travelAgencyContract;

    /// Merkle root for the list of planets
    ...
    constructor(TravelAgency _travelAgencyContract, bytes32 _root) {
        ...
        _setupRole(ACTION_MANAGER_ROLE, msg.sender);
    }
    ...
    function setTravelAgencyContract(TravelAgency _travelAgencyContract)
        external
        onlyRole(ACTION_MANAGER_ROLE)
    {
        travelAgencyContract = _travelAgencyContract;
    }
    ...
    function setRoot(bytes32 _root) external onlyRole(ACTION_MANAGER_ROLE) {
        root = _root;
    }
    ...
    modifier containsPlanet(uint256 planetId, bytes32[] memory proof) {
        require(
            _verify(_leaf(planetId), proof),
            "Planet not eligible for action"
        );
        _;
    }

    /// @dev override method for what action should be performed when a planet is visited.
    /// @dev this method will be called by TravelAgency whenever a matching visit occurs.
    ...
    /// @param planetId the token ID of the planet
    /// @return hash of the leaf
    function _leaf(uint256 planetId) internal pure returns (bytes32) {
...
abstract contract ERC721Data is ERC721, AccessControl {
    /// Data moderator role. An address with this role is allowed to set data of any token + lock it.
    ...
    /// Data of ERC-721 token.
    mapping(uint256 => Data) internal tokenData;

    /// Mapping to ERC-721 token to whether they are locked.
    /// Locked tokens cannot have their data updated (unless by someone who has DATA_MODERATOR_ROLE using `forceUpdateAndLock`)
    mapping(uint256 => bool) internal locked;

    /// Event emitted when a data update occurs.
    /// @param from the address which initiated the data update
    /// @param id the token ID updated
    /// @param data keccak256 digest of the updated data
    event SetData(address indexed from, uint256 indexed id, bytes32 data);

    /// Event emitted when a token is locked (cannot be updated).
    /// @param from the address which locked the token
    /// @param id the token ID
    event DataLock(address indexed from, uint256 indexed id);

    /// Event emitted when a token is unlocked (can be updated).
    /// @param from the address which unlocked the token
    /// @param id the token ID
    event DataUnlock(address indexed from, uint256 indexed id);

    /// Only allow method call if token is not locked
    /// @param id the token ID
    ...
    /// @param tokenId an array token IDs
    /// @param data an array of new data of tokens
    function setData(uint256[] calldata tokenId, Data[] calldata data)
        public
        virtual
...
contract PlanetPassItems is ERC1155, ERC1155Burnable, AccessControl {
    ...
    /// Returns whether a token ID is of a particular type.
    /// @param id the token ID
    /// @param _type the desired type of the token
    function tokenIsType(uint256 id, bytes32 _type) public view returns (bool) {
        return itemType[id] == _type;
    }

    /// Returns the item type of a token.
    /// @param id the token ID
...
abstract contract Delegation is ERC721 {
    /// Mapping of owners to delegated addresses.
    /// This is essentially the same as `setApprovalForAll` but does not confer the right to transfer.
    /// For Planets, this allows the delegatee to use planets as if they owned them.
    /// For Passports, this allows the delegatee to update existing passports.
    mapping(address => mapping(address => bool)) private delegationApproval;

    /// Event emitted when delegation approval of an address is updated.
    /// @param owner address of the delegator
    /// @param operator address of the delegatee
    ...
    /// Return delegationApproval mapping.
    ...
    /// Helper function to check whether the token is owned by `ownerOrd` or has been approved.
    /// @param tokenId the token ID of the Passport
    /// @param ownerOrd the address that may be the owner or approved
        ...
        returns (bool)
    {
        ...
        if (owner != ownerOrd) {
            // If not owner, check if approved
