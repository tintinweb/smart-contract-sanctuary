...
contract KawaiiDAO is Ownable, ERC721 {
    using SafeMath for uint256;
    using Counters for Counters.Counter;

    Counters.Counter private _tokenSupply;
    
    uint256 public tokenPrice = 20 ether; //20 MATIC
    uint256 public budgetDEV = 0 ether; 
    uint256 public budgetAiChan = 0 ether; 
    uint256 public budgetCommunityWallet = 0 ether; 
    uint256 public budgetTCLC = 0 ether; 
    uint256 public constant MAX_TOKENS=15000;
    uint public constant MAX_PURCHASE = 40; // set 1 to high to avoid some gas
    uint public constant MAX_RESERVE = 300; // Devs, mods, admins and Massive Giveaways!
    ...
    //Food NFT Whitelist Presale
    address public ChumbiValley = 0x5492Ef6aEebA1A3896357359eF039a8B11621b45; 
    address public LABO = 0xAF7688CB6Ad6648CF4940A428067aa5955f47751; 
    //NFT Projects with Presale
    address public ZedRun = 0xA5F1Ea7DF861952863dF2e8d1312f7305dabf215;
    address public CryptoUnicornsMarket = 0xdC0479CC5BbA033B3e7De9F178607150B3AbCe1f; //fa
    address public TrollGameOfficial = 0x953e74EEdEAd562f1Ef58AabE0Cf8e6C7eC0a00A; //vi
	address public CryptoRaiders = 0x2aD64e95EFF639fbd0273BCd4d0eeB72A60C8586; //la
	address public BoodleBears = 0x8c466Ea16B0E211250816a83795Cb25C75f2B5C0;	//no
    address public proxyRegistryAddress = 0xa5409ec958C83C3f309868babACA7c86DCB077c1;
    address public FATBOIS = 0x04932F507BE386D5bc8951f5187736aA259A36Db; 
    
    address private constant DEV = 0x0E4464e9C2Fa753a9447CbD8005ba44Be2d3060b;
    address private constant AiChan = 0x92e76168ea89905B162e37452259031B18aF29D6;
    address private constant CommunityWallet = 0x799De5439516853880ad3cEf253479Ff40f49b0E;
    address private constant TCLC = 0xC3911190953eF42338173774f4EFD03d14F4B83f;
    
    event priceChange(address _by, uint256 price);
    event PaymentReleased(address to, uint256 amount);
    
    constructor() ERC721("KawaiiDAO NFT", "KawaiiDAO") {
        _baseTokenURI = ""; 
    ...
    */
    ...
    function reserveTokens(address to,uint numberOfTokens) public onlyOwner {    
        ...
        require(numberOfTokens < MAX_RESERVE, "Can only mint 40 tokens at a time");
    ...
    */
    function setChumbiValleyPass(address newAddress) external onlyOwner {
         ChumbiValley = newAddress;
    }
    /**    
    * Set mintPass contract address
    */
    function setLABOPass(address newAddress) external onlyOwner {
         LABO = newAddress;
    }
    /**    
    * Set mintPass contract address
    */
    function setZedRunPass(address newAddress) external onlyOwner {
         ZedRun = newAddress;
    }
    /**    
    * Set mintPass contract address
    */
    function setCryptoUnicornsMarketPass(address newAddress) external onlyOwner {
         CryptoUnicornsMarket = newAddress;
    }
    /**    
    * Set mintPass contract address
    */
    function setTrollGameOfficialPass(address newAddress) external onlyOwner {
         TrollGameOfficial = newAddress;
    }
	/**    
    * Set mintPass contract address
    */
    function setCryptoRaidersPass(address newAddress) external onlyOwner {
         CryptoRaiders = newAddress;
    }
    /**
    * Set mintPass contract address
    */
    function setBoodleBearsPass(address newAddress) external onlyOwner {
         BoodleBears = newAddress;
    }
	
    /**    
    * Set mintPass contract address
    */
    function setFATBOISPass(address newAddress) external onlyOwner {
         FATBOIS = newAddress;
    }
    ...
    */
    ...
    function hasMintPass(address sender) public view returns (bool){
        if(sender==address(0)){
            return false;
        } else if (IERC1155(FATBOIS).balanceOf(sender,1)>0){
            return true;
        } else if (IERC721(ChumbiValley).balanceOf(sender)>0){
            return true;
        } else if (IERC721(LABO).balanceOf(sender)>0){
            return true;
        } else if (IERC721(ZedRun).balanceOf(sender)>0){
            return true;
        } else if (IERC721(CryptoUnicornsMarket).balanceOf(sender)>0){
            return true;
		} else if (IERC721(TrollGameOfficial).balanceOf(sender)>0){
            return true;
		} else if (IERC721(CryptoRaiders).balanceOf(sender)>0){
            return true;
        } else if (IERC721(BoodleBears).balanceOf(sender)>0){
            return true;
        }
        return false;
    }
    ...
    function iternalMint(uint256 numberOfTokens) private{
        ...
        require(numberOfTokens < MAX_PURCHASE, "Can only mint 40 tokens at a time");
    ...
    function withdrawAll() public onlyOwner {
      ...
      _withdraw(DEV, ((balance * 25) / 100));
      _withdraw(AiChan, ((balance * 40) / 100));
      _withdraw(CommunityWallet, ((balance * 25) / 100));
      _withdraw(TCLC, ((balance * 10) / 100));
      _withdraw(DEV, address(this).balance);
    }
