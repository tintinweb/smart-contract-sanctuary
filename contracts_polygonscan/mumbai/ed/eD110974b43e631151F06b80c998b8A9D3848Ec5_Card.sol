...
contract Card is ERC1155, Ownable {
    ...
    event Mint(address indexed account, uint256 id, uint256 quantity);
    ...
    function add(uint256[] calldata ids) external onlyOwner {
    ...
    function mint(uint256 id, uint256 quantity) external onlyOwner pausable {
        require(exists[id], "Invalid ID");
        require(tokenSupply[id] + quantity <= MAX_TOKENS, "Purchase would exceed max supply of tokens");
        _mint(msg.sender, id, quantity, new bytes(0));
        emit Mint(msg.sender, id, quantity);
    }
