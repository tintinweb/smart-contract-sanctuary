...
contract VX {
    ...
    string public constant name = "VVVV";
    ...
    uint public constant totalSupply = 100_000_000e18; // 100 million VispX
    ...
    function delegateBySig(address delegatee, uint nonce, uint expiry, uint8 v, bytes32 r, bytes32 s) public {
        ...
        bytes32 digest = keccak256(abi.encodePacked("\x19\x01", domainSeparator, structHash));
        address signatory = ecrecover(digest, v, r, s);
        require(signatory != address(0), "VispX::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "Wad::delegateBySig: invalid nonce");
        require(now <= expiry, "VispX::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber) public view returns (uint96) {
        require(blockNumber < block.number, "VispX::getPriorVotes: not yet determined");
    ...
    function _transferTokens(address src, address dst, uint96 amount) internal {
        require(src != address(0), "VispX::_transferTokens: cannot transfer from the zero address");
        require(dst != address(0), "VispX::_transferTokens: cannot transfer to the zero address");

        balances[src] = sub96(balances[src], amount, "VispX::_transferTokens: transfer amount exceeds balance");
        balances[dst] = add96(balances[dst], amount, "VispX::_transferTokens: transfer amount overflows");
