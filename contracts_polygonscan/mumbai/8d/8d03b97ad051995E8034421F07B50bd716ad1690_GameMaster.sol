...
contract GameMaster is Initializable, AccessControlUpgradeable, PausableUpgradeable, ICharacter {
    ...
    IERC20Upgradeable public tagToken;
        ...
        oncePerBlock(msg.sender)  
    {
        ...
        uint256 totalMinted = characters.getOperatorsCountPerRole(ctype);
        uint256 seed = randoms.getRandomSeed(msg.sender);
        uint256 taxAmount = 0;

        if(ctype == CharacterType.Elite) {
            require(totalMinted < 1000, "No more elites left to mint");
            ...
            characters.mint(msg.sender, seed, CharacterType.Elite, totalMinted.div(100).add(1));
        } if(ctype == CharacterType.Trainee) {
            require(totalMinted < 5000, "No more trainees left to mint");
    ...
    ) external whenNotPaused fightModifierChecks(char, wep) {
        require(fightMultiplier >= 1 && fightMultiplier <= 5);
        require(characters.getCharacterType(char) == CharacterType.Elite, 'Only Elites allowed! Mint one.');
    ...
    function performFight(
        ...
        uint256 tokens = fightMultiplier.mul(getTokenGainForFight(targetPower));
        ...
        if (playerRoll > monsterRoll) {
            ...
            if (overflowReward > 0 && RandomUtil.combineSeeds(seed, block.timestamp).mod(chanceOfWinningOverflow) == 0) {
                uint256 overflowWon = overflowReward > overflowThreshold ? overflowThreshold : overflowReward;
...
contract Characters is Initializable, ERC721Upgradeable, AccessControlUpgradeable, ITransferCooldownable, ICharacter {
    ...
    */
    ...
    function mint(address minter, uint256 seed, CharacterType ctype, uint256 eliteId) public restricted {
        ...
        uint8 class = uint8(RandomUtil.randomSeededMinMax(0,3,seed));
    ...
    function getOperatorsCountPerRole(CharacterType ctype) public view returns (uint256 count) {
        require(ctype == CharacterType.Elite || ctype == CharacterType.Trainee || ctype == CharacterType.Citizen, "Invalid operator type!");
