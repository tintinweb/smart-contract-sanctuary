...
contract Prediction is AccessProtected, Feed, KeeperCompatible {
    ...
    uint256 miniPoolPrice = 1e16;

    uint256 smallPoolPrice = 1e17;

    uint256 mediumPoolPrice = 3 * 1e17;

    uint256 largePoolPrice = 1e18;
    ...
    mapping(uint8 => Sport) public sports;
    ...
    mapping(uint8 => uint256[]) private _sportToPendingMatchIds;

    // Mapping from match id to position in the pendingmatches array
    mapping(uint256 => uint256) private _pendingMatchesIndex;
    ...
    function addSport(uint8 _id, Sport calldata _sport) external onlyAdmin {
        require(
    ...
    function updatePoolPrice(PoolType _poolType, uint256 _price)
        external
        onlyAdmin
    {
        if (_poolType == PoolType.Mini) {
            miniPoolPrice = _price;
        } else if (_poolType == PoolType.Small) {
            smallPoolPrice = _price;
        } else if (_poolType == PoolType.Medium) {
            mediumPoolPrice = _price;
        } else {
            mediumPoolPrice = _price;
        }
    }
    ...
    function updateTeams(uint8 teamId, Team calldata team) external onlyAdmin {
        require(bytes(teams[teamId].name).length != 0, "Invalid team Id");
        teams[teamId] = team;
        emit TeamUpdated(teamId, teams[teamId].name);
    }

    function _addMatch(uint256 matchId, Match calldata matchData) private {
        require(matches[matchId].startTime == 0, "Match already added");
    ...
    function placeBet(
        ...
        if (pool == PoolType.Mini) {
            bets[newbetId].amount = miniPoolPrice;
        } else if (pool == PoolType.Small) {
            bets[newbetId].amount = smallPoolPrice;
        } else if (pool == PoolType.Medium) {
            bets[newbetId].amount = mediumPoolPrice;
        } else {
            bets[newbetId].amount = largePoolPrice;
        }
        bets[newbetId].placerAddress = sender;
        bets[newbetId].matchId = matchId;
        bets[newbetId].placedOn = team;
        bets[newbetId].pool = pool;
        bund.safeTransferFrom(sender, address(this), bets[newbetId].amount);
        emit BetPlaced(newbetId, team, bets[newbetId].amount);
    }

    function totalBetsByOwner(address _owner)
        public
        view
        returns (uint256 counter)
    {
        uint256 totalBets = _betIds.current();
        for (uint256 i = 0; i < totalBets; i++) {
            if (bets[i].placerAddress == _owner) {
                counter++;
            }
        }
    }

    function allBetsByOwner(address _owner) external view returns (uint256[] memory) {
        uint256[] memory result = new uint256[](totalBetsByOwner(_owner));
        uint256 totalBets = _betIds.current();
        uint256 counter = 0;
        for (uint256 i = 0; i < totalBets; i++) {
            if (bets[i].placerAddress == _owner) {
    ...
    function totalBetssByMatchId(uint256 _matchId)
        public
        view
        returns (uint256 counter)
    {
    ...
    function totalBetsOnMatchByPool(uint256 _matchId, PoolType _poolType)
        public
        view
        returns (uint256 counter)
    {
        uint256 totalBets = _betIds.current();
        for (uint256 i = 0; i < totalBets; i++) {
            if (bets[i].matchId == _matchId && bets[i].pool == _poolType) {
                counter++;
            }
        }
    }

    function allBetsByMatchId(uint256 _matchId)
        external
        view
        returns (uint256[] memory)
    {
        uint256[] memory result = new uint256[](totalBetssByMatchId(_matchId));
        uint256 totalBets = _betIds.current();
        uint256 counter = 0;
        for (uint256 i = 0; i < totalBets; i++) {
            if (bets[i].matchId == _matchId) {
    ...
    function allBetsByMatchIdAndPool(uint256 _matchId, PoolType _pool)
        external
        view
        returns (uint256[] memory)
    {
        uint256[] memory result = new uint256[](
            totalBetsOnMatchByPool(_matchId,_pool)
        );
        uint256 totalBets = _betIds.current();
        uint256 counter = 0;
        for (uint256 i = 0; i < totalBets; i++) {
            if (bets[i].matchId == _matchId && bets[i].pool == _pool) {
                result[counter] = i;
                counter++;
            }
        }
        ...
        returns (bool upkeepNeeded, bytes memory performData)
    {
        upkeepNeeded = false;
        for (uint8 i = 0; i < 3; i++) {
            for (uint256 j = 0; i < _sportToPendingMatchIds[i].length; i++) {
