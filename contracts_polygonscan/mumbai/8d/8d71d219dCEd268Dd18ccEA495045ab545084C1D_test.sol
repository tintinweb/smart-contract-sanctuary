...
//最后领取轮次 tokenId => rounds
    ...
    function showAward(uint256 _tokenId) public view returns(uint256){
        ...
        for(uint256 i = 1; i < 5001 ; i++ ){
            quantity = quantity+(1 * roundsDetails[1].assetsNumber/roundsDetails[1].sumPower);
        }
        return quantity;
    }
    
    function claim(uint256 _tokenId) public virtual returns(uint256){
       
        uint256 award = showAward(_tokenId);
