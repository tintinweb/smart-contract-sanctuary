...
/**
CHILD Contract to receive from main, and also for bridging back and forth
 */
...
contract NFTMarketplaceChild is IERC721Receiver, Ownable, ILayerZeroReceiver {
    ...
    /// @notice Bridge NFT to main chain
    /// @param _tokenId Token ID
    function bridge(uint _tokenId, uint16 _chainId, bytes memory _destination) public payable {
        require(nft.ownerOf(_tokenId) == msg.sender, "Sender does not own this token ID");
        ...
        // payload bytes
        payload memory _payload = payload(_tokenId, msg.sender, true);
        bytes memory _payloadBytes = abi.encode(_payload);

        // calculate relayer fees (TEMP - this will be done in frontend)
        uint _relayerFee = endpoint.estimateNativeFees(
            _chainId, mainChainAddress, _payloadBytes, false, bytes("")
        );
        require(msg.value >= _relayerFee, "Not enough gas for relayer");

        endpoint.send{value:_relayerFee}(
            _chainId, 
            _destination,
            _payloadBytes,
            payable(msg.sender),
    ...
    function setMainChainAddress(address _address) public onlyOwner {
        mainChainAddress = _address;
    }
    ...
    function lzReceive(uint16 , bytes memory _srcCounterMockAddress, uint64 _nonce, bytes memory _payload) override external {
        ...
        if (_inPayload.mint) {
            ...
            // Lock
            // require(tokenMap[_inPayload.tokenId] == "0x0", "Token already taken");
            tokenMap[_inPayload.tokenId] = _inPayload.owner;
        }
    }
