...
// hevm: flattened sources of /nix/store/b748ii3m6qrlwq7qk8p0lfpl3f36awcf-geb-proxy-actions/dapp/geb-proxy-actions/src/GebProxySaviourActions.sol
...
////// /nix/store/b748ii3m6qrlwq7qk8p0lfpl3f36awcf-geb-proxy-actions/dapp/geb-proxy-actions/src/GebProxySaviourActions.sol
...
abstract contract SaviourCRatioSetterLike_1 {
...
contract GebProxySaviourActions {
    ...
    */
    function transferTokenFromAndApprove(address token, address target, uint256 amount) internal {
        DSTokenLike_2(token).transferFrom(msg.sender, address(this), amount);
        DSTokenLike_2(token).approve(target, 0);
        DSTokenLike_2(token).approve(target, amount);
    }
    ...
    */
    function transferTokensToCaller(address[] memory tokens) public {
        for (uint i = 0; i < tokens.length; i++) {
            uint256 selfBalance = DSTokenLike_2(tokens[i]).balanceOf(address(this));
            if (selfBalance > 0) {
              DSTokenLike_2(tokens[i]).transfer(msg.sender, selfBalance);
            }
        }
    }
    /*
    ...
    */
    function setDesiredCollateralizationRatio(
        ...
        SaviourCRatioSetterLike_1(cRatioSetter).setDesiredCollateralizationRatio(collateralType, safe, cRatio);
    }
    /*
