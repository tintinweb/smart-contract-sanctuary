...
contract LOTTPublicNFT is ERC721, ERC721URIStorage, TokenMetaData {
...
abstract contract TokenMetaData is ERC721{
    ...
    mapping(uint256 => mapping(string => string)) stringToString;
    ...
    function _setMetaData(
        ...
        stringToString[tokenId]["publicInfo"] = _publicInfo;
        stringToString[tokenId]["privateInfo"] = _privateInfo;
        stringToString[tokenId]["publicFileHash"] = _publicFileHash;
        stringToString[tokenId]["privateFileHash"] = _privateFileHash;
    }
    ...
    function publicInfo(uint256 tokenId) external view returns(string memory _publicInfo) {
        ...
        return stringToString[tokenId]["publicInfo"];
    }

    function privateInfo(uint256 tokenId) external view returns(string memory _privateInfo) {
        require(_isApprovedOrOwner(_msgSender(), tokenId), "ERC721: you are not owner nor approved");
        return stringToString[tokenId]["privateInfo"];
    }

    function publicFileHash(uint256 tokenId) external view returns(string memory _publicFileHash) {
        require(_exists(tokenId), "TokenMetaData: nonexistent token");
        return stringToString[tokenId]["publicFileHash"];
    }

    function privateFileHash(uint256 tokenId) external view returns(string memory _privateFileHash) {
        require(_isApprovedOrOwner(_msgSender(), tokenId), "ERC721: you are not owner nor approved");
        return stringToString[tokenId]["privateFileHash"];
    }
}
