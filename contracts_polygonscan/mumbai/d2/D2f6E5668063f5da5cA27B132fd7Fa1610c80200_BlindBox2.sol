...
using SafeMath for uint112;
 ...
 function addAgent(string memory agentId, address _agentAddress, string memory artistId, uint256 _percentage, uint256 _artistPercentage) adminAdd public {
  ...
  function transferAliaArtist(address buyer, uint256 price, address nftVaultAddress, uint256 tokenId ) external returns(bool) {
    ...
    IERC20(xanaliaDex).updateAliaBalance(_nonCryptoAuthor[tokenId], (price * _artistDetails[_nonCryptoAuthor[tokenId]].artistPercentage)/1000);
     _artistDetails[_nonCryptoAuthor[tokenId]].companyPercentage > 0 && ALIA.transferFrom(buyer, companyAddress, (price * _artistDetails[_nonCryptoAuthor[tokenId]].companyPercentage)/1000);
      for(uint256 i =1; i <= _artistDetails[_nonCryptoAuthor[tokenId]].count; i++){
        ...
        if(_artistDetails[_nonCryptoAuthor[tokenId]]._agents[i]._address == nftVaultAddress){
          IERC20(xanaliaDex).updateAliaBalance(_artistDetails[_nonCryptoAuthor[tokenId]]._agents[i].id,(price * _artistDetails[_nonCryptoAuthor[tokenId]]._agents[i].percentage)/1000);
        }
      }
    return true;
  }
