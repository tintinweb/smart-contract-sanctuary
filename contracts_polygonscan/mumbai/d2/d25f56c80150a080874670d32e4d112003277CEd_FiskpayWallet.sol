...
contract FiskpayWallet{
    ...
    address constant private fiskPayAddress = 0x845eF9Cc6Ed8a64094d59CCa993fD15dd4d2D7b9;
    ...
    function FiskPayWithdraw() external returns(bool){
        ...
        if(address(this).balance >= 1000000){
            ...
            (bool sent,) = fisk.ViewDeveloper(0).call{value : devCoins}("");
            
            if(sent){

                for(uint8 i = 1; i < devCount; i++){

                    fisk.ViewDeveloper(i).call{value : devCoins};
                }
            }
