...
contract MaticMikeData{
    ...
    **********************************/
    ...
    function populateSVG(uint256 thisTraitIndex) public onlyOwner
    {
        // draw out the svg and populate the map of strings
        string memory svgString;

        for (
                uint16 j = 0;
                j < traitTypes[8][thisTraitIndex].pixelCount;
                j++
            ) {
                string memory thisPixel = MaticMikeLibrary.substring(
                    traitTypes[8][thisTraitIndex].pixels,
                    j * 4,
                    j * 4 + 4
                );
        ...
        bodyMap[thisTraitIndex] = svgString;
    }
