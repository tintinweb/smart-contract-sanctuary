...
*/
    ...
    // Contract Maps
    mapping(uint256 => uint256[]) public bossLoot;
    ...
    * @param val is true or false
    */
    function activateReroll(bool val) public onlyOwner {
        burnRerollActive = val;
    }
    ...
    */
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        ...
        if(thisTraitIndex > 0){
            ...
            while(true){
                // create new hash
                if(_traitRoll == 8){
                    newHash = string(abi.encodePacked(MaticMikeLibrary.substring(tokenHash, 0, 7), MaticMikeLibrary.toString(startIndex)));
                }
                else{
                    newHash = string(
                        abi.encodePacked(
                            MaticMikeLibrary.substring(tokenHash, 0, _traitRoll-1), 
                            MaticMikeLibrary.toString(startIndex), 
                            MaticMikeLibrary.substring(tokenHash, _traitRoll+1, 8)
                        )
                    );
                }
                ...
                if (!hashToMinted[newHash]){
                    // successful mint. Update token hash and increase HGH cost to upgrade by 5
    ...
    /*
    Read Functions
    Some of this is taken from Anonymice contract, and modified significantly to suit
    our MVP scope.
    */
                        ...
                        bytes(
                            string(
                                abi.encodePacked(
                                    ...
                                    '", "description": "Matic Mike is a collection of 10k NFTs with unbelievable utility, all on-chain.", "image": "data:image/svg+xml;base64,',
    ...
    */
    ...
    function currentHghCost() public view returns (uint256) {
        if (totalSupply() >= 2000 && totalSupply() <= 4000)
            return 1000000000000000000;
        if (totalSupply() > 4000 && totalSupply() <= 6000)
            return 2000000000000000000;
        if (totalSupply() > 6000 && totalSupply() <= 8000)
            return 3000000000000000000;
        if (totalSupply() > 8000 && totalSupply() <= 10000)
    ...
    function mintInternal() internal {
        ...
        tokenIdToHash[_totalSupply] = hash(_totalSupply, msg.sender, 0);

        hashToMinted[tokenIdToHash[_totalSupply]] = true;
    ...
    function mintMike() public {
        ...
        if (totalSupply() < 2000) return mintInternal();
    ...
    function donationMint() public payable {
        ...
        require(purchased[msg.sender] + 1 <= MAX_PER_WALLET, "Only 10 mints per wallet allowed");
        require(totalSupply() + 1 < 10000, "Total supply exceeded.");
        require(1000000000000000000 == msg.value, "Insuffient amount sent.");
    ...
    function burnForMint(uint256 _tokenId) public {
        ...
        require(totalSupply() < 10000, "No mints left.");
