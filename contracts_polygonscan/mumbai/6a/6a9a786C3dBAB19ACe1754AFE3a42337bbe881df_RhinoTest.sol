...
contract RhinoTest is ERC721Enumerable, Ownable {
    ...
    function isWhitelistOpen () public returns(bool){
      if(finalTimestamp >= block.timestamp && block.timestamp >= startWlTimestamp) {
        return true;
      } else {
        return false;
      }
    ...
    function isPublicOpen () public returns(bool){
      if(finalTimestamp >= block.timestamp && block.timestamp >= startPublicTimestamp) {
        return true;
      } else {
        return false;
      }
