...
interface IPolyAlphaLottery {
     ...
     * @param _priceTicketInAlpha: price of a ticket in ALPHA
...
// File: contracts/PolyAlphaLottery.sol

pragma solidity ^0.8.4;
pragma abicoder v2;

/** @title PolyAlpha Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract PolyAlphaLottery is ReentrancyGuard, IPolyAlphaLottery, Ownable {
    ...
    uint256 public maxPriceTicketInAlpha = 50 ether;
    uint256 public minPriceTicketInAlpha = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInAlpha;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] alphaPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInAlpha;
        uint32 finalNumber;
    }
     ...
     * @param _alphaTokenAddress: address of the ALPHA token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _alphaTokenAddress, address _randomGeneratorAddress) {
        alphaToken = IERC20(_alphaTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of ALPHA to this contract
        uint256 amountAlphaToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInAlpha,
            _ticketNumbers.length
        );

        // Transfer alpha tokens to this contract
        alphaToken.safeTransferFrom(address(msg.sender), address(this), amountAlphaToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInAlpha += amountAlphaToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInAlphaToTransfer
        uint256 rewardInAlphaToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInAlphaToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        alphaToken.safeTransfer(msg.sender, rewardInAlphaToTransfer);

        emit TicketsClaim(msg.sender, rewardInAlphaToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInAlpha) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in ALPHA for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].alphaPerBracket[j] =
                ...
                // A. No ALPHA to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].alphaPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInAlpha - amountToShareToWinners);

        // Transfer ALPHA to treasury address
        alphaToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        alphaToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInAlpha += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInAlpha >= minPriceTicketInAlpha) && (_priceTicketInAlpha <= maxPriceTicketInAlpha),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInAlpha: _priceTicketInAlpha,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            alphaPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInAlpha: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(alphaToken), "Cannot be ALPHA token");
    ...
    /**
     * @notice Set ALPHA price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInAlpha: minimum price of a ticket in ALPHA
     * @param _maxPriceTicketInAlpha: maximum price of a ticket in ALPHA
     */
    function setMinAndMaxTicketPriceInAlpha(uint256 _minPriceTicketInAlpha, uint256 _maxPriceTicketInAlpha)
        external
        onlyOwner
    {
        require(_minPriceTicketInAlpha <= _maxPriceTicketInAlpha, "minPrice must be < maxPrice");

        minPriceTicketInAlpha = _minPriceTicketInAlpha;
        maxPriceTicketInAlpha = _maxPriceTicketInAlpha;
    }
     ...
     * @param _priceTicket price of a ticket (in ALPHA)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].alphaPerBracket[_bracket];
        } else {
            return 0;
        }
    }
