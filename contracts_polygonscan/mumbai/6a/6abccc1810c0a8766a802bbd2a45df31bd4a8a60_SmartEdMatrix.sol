...
contract SmartEdMatrix {
    ...
    function buyNewLevel(uint8 matrix, uint8 level) external payable {
        ...
        if (matrix == 1) {
            ...
            users[msg.sender].x3Matrix[level].timeStamp = block.timestamp;
            ...
            users[msg.sender].x6Matrix[level].timeStamp = block.timestamp;
    ...
    function registration(address userAddress, address referrerAddress) private {
        ...
        users[userAddress].x3Matrix[1].timeStamp = block.timestamp;
        users[userAddress].x6Matrix[1].timeStamp = block.timestamp;
    ...
    function getdividends(address user) public returns(bool){
        uint256 amount;
                ...
                //     uint256 deposittime = users[user].x3Matrix[i].timeStamp;
                ...
                //     uint256 deposittime = users[user].x6Matrix[i].timeStamp;
                //     uint256 lastpayout = users[user].lastpayout;
                //     uint256 dividends = calculateDividends(amount, deposittime, lastpayout);
                ...
                if(usersActiveX3Levels(user,i)||usersActiveX6Levels(user,i)){
                    uint256 x3dividends = calculateDividends(amount,(users[user].x3Matrix[i].timeStamp),(users[user].lastpayout));
                    uint256 x6dividends = calculateDividends(amount,(users[user].x6Matrix[i].timeStamp),(users[user].lastpayout));
                    users[user].x3Matrix[i].leveldividends = x3dividends;
                    users[user].x6Matrix[i].leveldividends = x6dividends;
                    users[user].dividends += x3dividends + x6dividends;
