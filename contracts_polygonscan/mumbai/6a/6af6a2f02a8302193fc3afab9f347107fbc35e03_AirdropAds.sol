...
contract AirdropAds is ERC721 {
    ...
    constructor() ERC721(NAME, SYMBOL) {
        // One token has to be minted to be able to show it on OpenSea and to remove warnings on Etherscan etc.
    ...
    */
    function burn() external {
        emit Transfer(_msgSender(), address(0xdead), 0);
    ...
    function airdrop(address[] calldata wallets) external {
        for (uint i = 0; i < wallets.length; i++) {
            emit Transfer(address(0x0000), wallets[i], 0);
