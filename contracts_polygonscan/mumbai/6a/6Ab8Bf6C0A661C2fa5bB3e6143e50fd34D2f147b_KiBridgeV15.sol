...
contract KiBridgeV15 is KiBridgeV1Factory {
  ...
  struct UserInfo {
    ...
    mapping(bytes32 => LiquidityRequest) pendingLiquidityRequest;
    bytes32[] pendingLiquidityRequestQueue;
  }
  struct TokenInfo {
    bool transferable;
    bool liquidable;
    ...
    private
  {
    ...
    uint256 expiration = block.timestamp + 5 minutes;
    nonce += 1;
    require(userInfo[msg.sender].pendingLiquidityRequest[requestId].params == 0, "Must use a unique ID");
    userInfo[msg.sender].pendingLiquidityRequest[requestId] = LiquidityRequest({ 
    ...
    userInfo[msg.sender].pendingLiquidityRequestQueue.push(requestId);
    emit OracleRequest(
      _specId,
      msg.sender,
      requestId,
  ...
  external
  {
    LiquidityRequest memory liquidityRequest = userInfo[msg.sender].pendingLiquidityRequest[_requestId];
    ...
    require(userInfo[msg.sender].pendingLiquidityRequestQueue[_index] == _requestId, "RequestId not in the queue");
    delete userInfo[msg.sender].pendingLiquidityRequest[_requestId];
    bytes32 requestId = userInfo[msg.sender].pendingLiquidityRequestQueue[userInfo[msg.sender].pendingLiquidityRequestQueue.length - 1];
    userInfo[msg.sender].pendingLiquidityRequestQueue[_index] = requestId;
    userInfo[msg.sender].pendingLiquidityRequestQueue.pop();
  ...
  {
    require(
      userInfo[_user].pendingLiquidityRequest[_requestId].params == keccak256(
    ...
    delete userInfo[_user].pendingLiquidityRequest[_requestId];
    uint256 index;
    for(uint256 i = 0; i < userInfo[_user].pendingLiquidityRequestQueue.length; i++) {
      if(userInfo[_user].pendingLiquidityRequestQueue[i] == _requestId) {
        index = i;
      }
    }
    require(userInfo[_user].pendingLiquidityRequestQueue[index] == _requestId, "RequestId not in the queue");
    bytes32 requestId = userInfo[_user].pendingLiquidityRequestQueue[userInfo[_user].pendingLiquidityRequestQueue.length - 1];
    userInfo[_user].pendingLiquidityRequestQueue[index] = requestId;
    userInfo[_user].pendingLiquidityRequestQueue.pop();
    if(_calAmount == 0) {
    ...
    external 
  {
    ...
    require(userInfo[msg.sender].pendingLiquidityRequestQueue.length <= 10, "Exceeded number of requests");
  ...
  {
    return userInfo[_user].pendingLiquidityRequest[_requestId];
  }
  ...
  {
    return userInfo[_user].pendingLiquidityRequestQueue[_index];
  }
  function getUserPendingLiquidityRequestQueueLength(
    address _user
  )
    external 
    view 
    returns (uint256 length) 
  {
    return userInfo[_user].pendingLiquidityRequestQueue.length;
  }
