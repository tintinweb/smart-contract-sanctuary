...
/*
 ...
 |\/| |__   |   /\  /__`  /\  |  | |__) /__`
 |  | |___  |  /~~\ .__/ /~~\ \__/ |  \ .__/
 115 105 124 101 123 101 125 122 123
*/
contract Metasaurs is ERC721Enumerable, Ownable, ReentrancyGuard {
	...
	address private _signerAddress = 0xB0be9291F7CFF63BDf2c858ce25fdA59e0530fFc;

	uint256 public pricePerToken = 90000000000000000; //0.09 ETH

	uint256 public publicAmountMinted;
	bool public saleLive = true; //TODO: MAKE THEM FALSE WHEN PUBLISH
	...
	//thsi function shouldt be public... but it is!
	...
	//thsi function shouldt be public... but it is!
	function matchAddresSigner2(bytes32 hash, bytes memory signature) public view returns (bool) {
		bytes32 ethHash = hash.toEthSignedMessageHash();
		return _signerAddress == ethHash.recover(signature);
	}

	//howmever can buy if teh sale si live
	...
	//presale via teh signatures. maxssimum 5 jpegs ;)
	function presaleBuy(
		bytes32 hash,
		bytes memory sig,
		uint256 qty,
		...
		for (uint256 i = 0; i < qty; i++) {
			presaleCounter++;
			require(presaleCounter < maxPresale, "presale - out of stock");
			_safeMint(msg.sender, totalSupply() + 1);
		}
	}

	//presale via teh signatures. maxssimum 5 jpegs ;)
	function presaleBuy2(
		bytes32 hash,
		bytes memory sig,
		uint256 qty,
		string memory nonce
		...
		require(presaleLive, "presale not live");
		require(matchAddresSigner2(hash, sig), "no direct mint");
		require(!_usedNonces[nonce], "hashed used");
		require(qty <= 5, "no more than 5");
		require(hashTransaction(msg.sender, qty, nonce) == hash, "hash check failed");
		require(totalSupply() + qty < maxSupply, "out of stock");
	...
	function setBaseURI(string memory newBaseURI) public onlyOwner notLocked {
		_baseTokenURI = newBaseURI;
	}

	function setContractURI(string memory newuri) public onlyOwner notLocked {
		_contractURI = newuri;
	}
