  ...
  IDemoMarketplace
{
  ...
  function bid(
    ...
    require(requiredCost <= IERC20(demoTokenAddress).balanceOf(sender), "Marketplace:bid04");
    ...
    IERC20(demoTokenAddress).transferFrom(sender, address(this), requiredCost);
  ...
  function lazyMintingBuy(
    ...
    IDemoNFT(demoNFTAddress).redeem(_creator, _id, _royaltyPercentage, 0, _price, 0, 0, 0, 1, _signature);
  ...
  function lazyMintingBid(
    ...
    address sender = _msgSender();
    require(requiredCost <= IERC20(demoTokenAddress).balanceOf(sender), "Marketplace:lazyMintingBid04");

    IDemoNFT(demoNFTAddress).redeem(
      _creator,
      _id,
      _royaltyPercentage,
      _priceType,
    ...
    IDemoNFT(demoNFTAddress).setApprovalForAllByMarketplace(_creator, address(this), true);
    IERC721(demoNFTAddress).safeTransferFrom(_creator, address(this), _id);
  ...
  function buyNow(uint256 _id, address _owner) external {
    ...
    require(requiredCost <= IERC20(demoTokenAddress).balanceOf(sender), "Marketplace:buyNow04");
  ...
  function lazyMintingBuyNow(
    ...
    IERC20(demoTokenAddress).transferFrom(sender, address(this), _buyNowPrice + variables[1] + variables[0]);
    ...
    // IERC20(demoTokenAddress).transferFrom(sender, _creator, _buyNowPrice - variables[2]);
  ...
  function _refundBid(uint256 _id, address _owner) private {
    ...
    if (bidBalance[currentBid.bidder] > 0 && currentBid.bidder != _owner) {
      ...
      bidBalance[currentBid.bidder] = bidBalance[currentBid.bidder] - (currentBid.price + buyerMarketFee + royaltyFee);
      IERC20(demoTokenAddress).transfer(currentBid.bidder, currentBid.price + buyerMarketFee + royaltyFee);
    }
  }
