...
contract Soul is ERC721Enumerable, Mintable, Pausable {
    ...
    event AddressesSet(address rays, address wallet, address durabilityReducer);
    ...
    function _beforeTokenTransfer(
        ...
        if  (from != address(0)) {
            // 耐久値が1以上の場合のみtransfer可能(mint時は除く)
            require(remainingDurability[tokenId] > 0, "Soul: remainingDurability must be grater than 0");
        }
    ...
    function setAddresses(address _rays, address _wallet, address _durabilityReducer) external onlyOwner {
        rays = IERC20(_rays);
        wallet = _wallet;
        durabilityReducer = _durabilityReducer;

        emit AddressesSet(_rays, _wallet, _durabilityReducer);
    }
