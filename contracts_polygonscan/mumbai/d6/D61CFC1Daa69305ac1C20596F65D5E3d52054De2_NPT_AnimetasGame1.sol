...
contract NPT_AnimetasGame1 is VRFConsumerBase, Pausable, Ownable {
    enum PackTier {
        GOLD
    }
    ...
    constructor(
        ...
        slowModeTime = 1 hours; // 30 minutes

        packContents[PackTier.GOLD] = 12; // 12 pieces per pack for this drop
        packPrices[PackTier.GOLD] = 0 ether; // free packs

        whitelistedAddress[owner()] = true;

        prizeFactory = new PrizeFactory(
            "NPT x Animetas - Prizes",
            "NPTxAnimetasPrizes"
        );
        prizeFactory.transferOwnership(owner());

        pieceFactory = new PieceFactory(
            "NPT x Animetas - Pieces",
            "NPTxAnimetasPieces"
        );
        ...
        whenNotPaused
    {
        ...
        prize.claimed = true;
        uint256 tokenId = _mintPrize(recipient);
    ...
    function _mintPrize(address recipient) internal returns (uint256) {
        uint256 tokenId = prizeFactory.mint(recipient);
        return tokenId;
    }
    ...
    function setPrizeTokenURI(string memory tokenUri) public onlyWhitelisted {
        prizeFactory._setTokenURI(tokenUri);
    }
    ...
    Ownable
{
    ...
    string public TOKEN_URI = "QmaYVLDAxGVoWsrSQfEGz6PfjEqcYWyQwbPWLcD24tyhBA";
        ...
        returns (string memory)
    {
        ...
        string memory _tokenURI = string(
            abi.encodePacked(IPFS_TOKEN_BASE_URI, TOKEN_URI)
        );
    ...
    function _setTokenURI(string memory _tokenURI)
        public
        virtual
        whenNotPaused
        onlyAdmin
