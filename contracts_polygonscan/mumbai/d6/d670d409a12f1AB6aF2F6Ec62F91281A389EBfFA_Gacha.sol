    ...
    PausableUpgradeable
{
    ...
    address internal melXNFTAddress;

    MelXNFTV1 private melXNFT;
    ...
    string[] private rarityNameList;
    mapping(string => Rarity) private rarityNameToRarity;
    ...
    function initialize(
        ...
        melXNFT = MelXNFTV1(melXNFTAddress);

        __VRFConsumerBase_init(vrfCoordinator, linkTokenAddress);
    }

    function _authorizeUpgrade(address newImplementation) internal override {
        require(super._getAdmin() == msg.sender, "Only Admin");
    }

    modifier onlyAdmin() {
        melXNFT.hasRole(melXNFT.DEFAULT_ADMIN_ROLE(), _msgSender());
        _;
    }

    function gacha() public returns (bytes32 requestId) {
        _getRandomNumberForGacha();
    }

    function _getRandomNumberForGacha() internal returns (bytes32 requestId) {
        require(
        ...
        for (
            ...
            if (cumulativeWeight[itemIndex] >= randomness) {
                emit mintWithGacha(
                    ...
                    requestIdToAddress[_requestId]
                );

                melXNFT.safeMintWithGacha(requestIdToAddress[_requestId]);
...
interface IMelXNFT {
    ...
    function safeMintWithGacha(address _to)
        external
    ...
    AccessControlUpgradeable
{
    ...
    function __MelXNFTBasic_init(string calldata baseTokenURI)
        internal
        initializer
    {
        __MelXNFTBasic_init_unchained(baseTokenURI);
    }

    function __MelXNFTBasic_init_unchained(string calldata baseTokenURI)
        internal
        initializer
    {
...
contract MelXNFTPayment is Initializable, AccessControlUpgradeable {
    ...
    function __MelXNFTPayment_init_unchained(
        ...
        paymentToken = IERC20Upgradeable(tokenPayment);
    }

    function _fee(address payer) internal {
        require(feeReceiver != address(0), "Receiver address not set");
        require(feeAmount > 0, "Fee Amount not set");
        paymentToken.transferFrom(payer, feeReceiver, feeAmount);
    }
    ...
    function _setPaymentToken(address newTokenPaymentAddress) internal {
        paymentToken = IERC20Upgradeable(newTokenPaymentAddress);
    }
}
...
contract MelXNFTV1 is IMelXNFT, MelXNFTPayment, MelXNFTBasic {
    ...
    function setFeeAmount(uint256 newFeeAmount)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    function setPaymentToken(address newTokenPaymentAddress)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        _setPaymentToken(newTokenPaymentAddress);
    }
