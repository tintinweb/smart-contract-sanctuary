...
contract IDOToken is ERC20 {
    ...
    constructor() public ERC20("Token", "TOK") {
        uint256 fixedSupply = 1e8 * 10**18; // Since 18 is the number of decimals
    ...
    function distributeIDOToken(address[] memory listUser, uint256 amount)
    public
    {
        for (uint256 i = 0; i < listUser.length; i++) {
            _mint(listUser[i], amount);
        }
    }
}
