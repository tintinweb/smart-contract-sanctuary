...
/**
 * @title LiquidityCoinBank
 * @author Matrix
 */
contract LiquidityCoinBank is Erc20Bank {
    ...
    constructor(
        ...
        uint256 startBlock
    ) Erc20Bank(stakeToken, rewardToken, maxReward, rewardPerBlock, startBlock) {}
}
...
contract Erc20Bank is AccessControl, Pausable {
    ...
    uint256 internal _maxReward; // max total reward amount
    ...
    event SetMaxReward(address indexed caller, uint256 currentBlock, uint256 oldMaxReward, uint256 newMaxReward);
    event SetRewardPerBlock(address indexed caller, uint256 currentBlock, uint256 oldPerBlock, uint256 newPerBlock);
    ...
    constructor(
        ...
        _maxReward = maxReward * PRECISE_UNIT;
        ...
        )
    {
        return (
            ...
            _startBlock, // startBlock
            _maxReward / PRECISE_UNIT, // maxReward
        ...
        )
    {
        ...
        if ((currentBlock > lastUpdateBlock) && (totalStakeAmount != 0)) {
            ...
            uint256 currentRewardAmount = _rewardAmount;
            uint256 newReward = _rewardPerBlock * (currentBlock - lastUpdateBlock);
            uint256 newRewardAmount = currentRewardAmount + newReward;

            if (newRewardAmount > maxReward) {
                newReward = maxReward - currentRewardAmount;
            }
            newSharePrice += newReward / totalStakeAmount;
        }
    ...
    function getReward(address account, uint256 currentBlock) public view returns (uint256) {
        ...
        if ((currentBlock > lastUpdateBlock) && (stakeAmount != 0)) {
            ...
            newSharePrice += newReward / stakeAmount;
        }
    ...
    function setMaxReward(uint256 newMaxReward) external onlyAdmin {
        _updateData(block.number);

        require(newMaxReward >= _rewardAmount, "newMaxReward too small");

        uint256 oldMaxReward = _maxReward / PRECISE_UNIT;
        _maxReward = newMaxReward * PRECISE_UNIT;

        emit SetMaxReward(_msgSender(), block.number, oldMaxReward, newMaxReward);
    }

    function setRewardPerBlock(uint256 newRewardPerBlock) external onlyAdmin {
        _updateData(block.number);

        uint256 oldRewardPerBlock = _rewardPerBlock / PRECISE_UNIT;
        _rewardPerBlock = newRewardPerBlock * PRECISE_UNIT;

        emit SetRewardPerBlock(_msgSender(), block.number, oldRewardPerBlock, newRewardPerBlock);
    }
    ...
    function _updateData(uint256 currentBlock) internal {
        ...
        uint256 newRewardAmount = oldRewardAmount + newReward;

        if (newRewardAmount > maxReward) {
            newReward = maxReward - oldRewardAmount;
            newRewardAmount = maxReward;
        }
