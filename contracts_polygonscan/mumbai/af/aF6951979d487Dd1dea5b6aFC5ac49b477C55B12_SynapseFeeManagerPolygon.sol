...
abstract contract SynapseFeeManager {
...
import { SynapseFeeManager } from "./SynapseFeeManager.sol";

contract SynapseFeeManagerPolygon is SynapseFeeManager, Ownable {
    /// @notice address of USDC stable coin
    address public stableCoin;

    /// @notice address of staking contract
    address public staking;
    /// @notice address of burn collector
    address public burn;
    ...
    uint256 public totalFeeAlloc = stakingFeeAlloc + burnFeeAlloc + lpFeeAlloc;

    constructor(address _token, address _stableCoin) SynapseFeeManager(_token) {
    ...
    function canSyncFee(address, address recipient) external view override returns (bool shouldSyncFee) {
        if (recipient == address(uniswapPair)) {
            shouldSyncFee = true; // when swap SNP > USDC
        }
    }
    ...
    function _syncFee() internal override {
        ...
        if (totalAmount >= minTokens) {
            ...
            if (burn != address(0)) {
                ...
                IERC20(token).transfer(burn, burnAmount);
            }

            uint256 lpAmount = totalAmount - stakingAmount - burnAmount;

            uint256 swapAmount = lpAmount / 2;
            uint256 liquidityAmount = lpAmount - swapAmount;

            // swap half for USDC
            uint256 preUSDC = IERC20(stableCoin).balanceOf(address(this));
            _swapTokens(swapAmount);
            uint256 postUSDC = IERC20(stableCoin).balanceOf(address(this));

            // add other half with received USDC
            _addTokensToLiquidity(liquidityAmount, postUSDC - preUSDC);
        }
    }
    ...
    function _addTokensToLiquidity(uint256 tokensAmount, uint256 usdAmount) private {
        if (tokensAmount != 0 && usdAmount != 0) {
            ...
            try uniswapRouter.addLiquidity(token, stableCoin, tokensAmount, usdAmount, 0, 0, destination, block.timestamp) {} catch {}
        }
    }
    ...
    function setFeeAlloc(
        uint256 _stakingFeeAlloc,
        uint256 _burnFeeAlloc,
        uint256 _lpFeeAlloc
    ) public onlyOwner {
        require(_stakingFeeAlloc >= 0 && _stakingFeeAlloc <= 100, "_stakingFeeAlloc is outside of range 0-100");
        ...
        totalFeeAlloc = stakingFeeAlloc + burnFeeAlloc + lpFeeAlloc;
    }
