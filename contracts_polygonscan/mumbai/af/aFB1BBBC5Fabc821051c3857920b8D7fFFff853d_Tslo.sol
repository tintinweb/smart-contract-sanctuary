...
contract Tslo {
     ...
     * Requests randomness
     */
    function getRandomNumber(address _from, uint256 _bet, string memory _serverSeed, string memory _clientSeed) public {
        address userAddress;
        ...
        require(BLBL.balanceOf(userAddress) >= _bet, "Not enough credits");
        _credits[userAddress] -= _bet;
        ...
        requestIdToBet[_clientSeed] = Bet(userAddress, _bet);
        fulfillRandomness(_clientSeed, number);
        //return requestId;
    }
     ...
     */
    function fulfillRandomness(string memory _clientSeed, uint256 number) internal {
        //uint256 number = (randomness % 5) + 1;
        emit RandomReceived(_clientSeed, number);
        Bet memory _bet = requestIdToBet[_clientSeed];


        //if(randomResult<=3)
        //    _credits[_bet.wallet] -= 1 * 10 ** 18;
        if (number == 4)
            _credits[_bet._wallet] += 2 * _bet._betAmount;
        else if (number == 5)
            _credits[_bet._wallet] += 3 * _bet._betAmount;
    }
    ...
    event RandomReceived(string clientSeed, uint256 number);
