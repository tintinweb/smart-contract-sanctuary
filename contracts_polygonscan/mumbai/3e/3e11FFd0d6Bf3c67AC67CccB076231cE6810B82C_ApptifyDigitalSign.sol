...
contract ApptifyDigitalSign {
    ...
    function execute(
        ...
        require(
            blockchainExecutor == msg.sender || blockchainExecutor == address(0)
        );
        ...
        for (uint256 i = 0; i < docSigner.length; i++) {
            ...
            require(recovered > lastAdd && recovered == docSigner[i].identity);
            signers[i].identity = recovered;
            signers[i].signingTime = docSigner[i].signingTime;
        }
        require(signers.length == docSigner.length);
