...
contract EntropyManager is VRFConsumerBase, AccessControl {
  ...
  event EntropyRequest(address indexed targetAddress, uint256 indexed tokenId, uint256 indexed index, bytes32 requestID);
  ...
  mapping(address => mapping(uint256 => mapping(uint256 => Request))) public entropyStorage;
  ...
  function requestEntropy(address _address, uint256 _tokenId, uint256 _index) external onlyRole(REQUESTER_ROLE) {
    require(entropyStorage[_address][_tokenId][_index].finished == false, "Requested existent entropy");
    Request memory request = Request(false, _address, _tokenId, _index, 0);
    bytes32 requestId = getRandomNumber();
    entropyRequests[requestId] = request;
    emit EntropyRequest(request.targetAddress, request.tokenId, request.index, requestId);
  }

  function getEntropy(address _address, uint256 _tokenId, uint256 _index) public view returns (uint256) {
    require(entropyStorage[_address][_tokenId][_index].finished, "Queried non-existent entropy");
    return entropyStorage[_address][_tokenId][_index].value;
  }

  // TODO - allow admin to update VRF values
  ...
  */
  function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
    ...
    entropyStorage[request.targetAddress][request.tokenId][request.index] = request;
    entropyRequests[requestId] = request;
...
contract YouthScouts is ERC721EnumerableUpgradeable, AccessControlUpgradeable {
  using Strings for uint256;
  ...
  function initialize(string memory _description, string memory _name, address _entropyManager) initializer public {
    __ERC721_init(_description, _name);
    entropyManager = EntropyManager(_entropyManager);
    contract_uri = "https://api.metassocer.com/metadata/scouts/collection.json";
    beginning_uri = "https://api.metassocer.com/metadata/scouts/";
    ending_uri = ".json";
    _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
  ...
  function mintScout(address _owner, uint256 _tokenId, string memory _tokenGenerator, uint8 _randomSeeds) external onlyRole(MINTER_ROLE) {
  ...
  function supportsInterface(bytes4 _interfaceId) public view virtual override(ERC721EnumerableUpgradeable, AccessControlUpgradeable) returns (bool) {
    return super.supportsInterface(_interfaceId);
  }

  function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
    return string(abi.encodePacked(beginning_uri, _tokenId.toString(), ending_uri, "?generator=", tokenGenerator[_tokenId]));
  }
  ...
  function setBeginningURI(string memory _new_uri) external onlyRole(DEFAULT_ADMIN_ROLE) {
    beginning_uri = _new_uri;
  }

  function setEndingURI(string memory _new_uri) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ending_uri = _new_uri;
  }

  function setContractURI(string memory _new_uri) external onlyRole(DEFAULT_ADMIN_ROLE) {
    contract_uri = _new_uri;
  }
...
contract TicketsRedeemer {
  ...
  function redeemTicket(uint256 _tokenId) external {
    ...
    scoutsContract.mintScout(ticketOwner, _tokenId, tokenGenerator, requiredSeeds);
    emit TicketRedeemed(_tokenId, _tokenId, scoutsContract.ownerOf(_tokenId));
  }
