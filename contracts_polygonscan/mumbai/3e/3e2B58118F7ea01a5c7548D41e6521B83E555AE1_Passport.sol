    ...
    ERC721Data
{
    ...
    mapping(address => mapping(address => bool))
        private visitDelegationApprovals;
    ...
    function setPlanetContract(Planet _planetContract)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        planetContract = _planetContract;
    ...
    function isVisitDelegationApproved(address owner, address operator)
        public
        view
        returns (bool)
    {
    ...
    function overrideUpgradePass(uint256 tokenId, uint256 templateId) external onlyRole(MINTER_ROLE) onlyTokenOwner(tokenId) {
    ...
    function overrideSafeMint(address to, Data calldata data, uint256 templateId) external onlyRole(MINTER_ROLE) {
    ...
    /// @param owner the owner of the Passport
    /// @param tokenId the token ID of the Passport
    /// @param planetId the token ID of the Planet to visit
    /// @param stampId the token ID of the stamp to use
    function delegateVisitPlanet(
        ...
        require(
            isVisitDelegationApproved(owner, msg.sender),
        ...
        // Make sure passport is owned by `owner`
        require(ownerOf(tokenId) == owner, "Not owner of Passport");
...
abstract contract ERC721Data is ERC721, AccessControl, Pausable {
    ...
    /// Modifier to only allow method call if token is not locked
