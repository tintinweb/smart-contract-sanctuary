...
import "./extensions/ERC3664CrossSynthetic.sol";
import "./Synthetic/ISynthetic721.sol";
import "../utils/Base64.sol";
import "./extensions/ERC3664Updatable.sol";
import "./extensions/ERC3664Generic.sol";

interface ICustomMetadata {
    function tokenURI(uint256 tokenId) external view returns (string memory);
}

abstract contract BaseAttributes is ERC3664CrossSynthetic, ERC3664Updatable, ERC721Enumerable, Ownable {
    using Strings for uint256;
    address private _customURI;
    address private immutable _originalContract;
    mapping(address => bool) private _validNFTs;

    constructor(address originalContract_, string memory name_, string memory symbol_) ERC721(name_, symbol_) ERC3664Updatable() Ownable() {
        _originalContract = originalContract_;
        _validNFTs[originalContract_] = true;
    }
    ...
    function setValidNFTs(address nft, bool enable) public onlyOwner {
        _validNFTs[nft] = enable;
    }

    function generate(uint256 tokenId) external {
        require(
            ERC721Enumerable(_originalContract).totalSupply() >= tokenId,
            "FIFADAO: original token has not been minted yet"
        );
        // TODO: require it does not exist already
        super._mint(ERC721(_originalContract).ownerOf(tokenId), tokenId);
    }
    ...
    function attach(
        ...
        bytes memory text,
        bool isPrimary
    ) public virtual override(ERC3664, ERC3664Generic) {
    ...
    function combine(
        ...
        require(
            ...
            "FIFADAO: caller is not token owner"
        );
    ...
    function separate(uint256 tokenId) public {
        require(
            ...
            "FIFADAO: caller is not token owner nor approved"
        );
    ...
    function getAttributes(uint256 tokenId) public view returns (string memory) {
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            data = abi.encodePacked(
                data,
                ",",
                concatAttribute(ISynthetic721(tokens[i].token).coreName(), tokens[i].id.toString())
            );
        }
        return string(data);
    }

    function concatAttribute(string memory key, string memory value) internal pure returns (bytes memory) {
    ...
    function coreTokenURI(uint256 tokenId) public view returns (string memory) {
        ...
        parts[1] = string(
            abi.encodePacked("FIFADAO #", tokenId.toString(), '</text></svg>')
        );
            ...
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "FIFADAO #',
                        tokenId.toString(),
                        '", "description": "FIFADAO is an identity system which can make all metaverse citizens join into different metaverses by using same FIFADAO Identity. The first modular NFT with FIFADAO at its core, with arbitrary attributes addition and removal, freely combine and divide each components. Already adapted to multiple metaverse blockchain games. FUTURE IS COMMING", "image": "data:image/svg+xml;base64,',
    ...
    view
    virtual
    override(ERC721Enumerable, ERC3664, ERC3664Generic)
    returns (bool)
    {
...
interface IERC3664 is IERC165 {
     ...
     */
    event NewAttribute(address indexed operator, uint256 indexed attrId, string name, string symbol, string uri);
     ...
     */
    event TransferSingle(address indexed operator, uint256 from, uint256 to, uint256 indexed attrId, uint256 value);
     ...
     */
    function balanceOfBatch(uint256 tokenId, uint256[] calldata attrIds) external view returns (uint256[] memory);
...
contract PlayerAttributes is BaseAttributes {
    constructor(address originalContract_) BaseAttributes(originalContract_, "Player", "PLAYER") {}
}
