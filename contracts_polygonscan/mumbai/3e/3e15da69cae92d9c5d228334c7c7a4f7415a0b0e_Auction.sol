    ...
    AuctionValidator
{
    ...
    event PlatformAuctioneer(address indexed auctioneer, bool hasApproval);
    event OpenAuctioneer(address indexed auctioneer, bool hasApproval);
    event SellerAuctioneer(
        address indexed owner,
        address indexed auctioneer,
        bool indexed hasApproval
    );
    event RemovedAuctioneer(address indexed auctioneer, bool isRemoved);
    ...
    // Auction fee in percentage basis points
    mapping(address => uint96) public auctioneerFee;

    mapping(address => bool) public oAuctioneer;
    mapping(address => bool) public rAuctioneer;

    mapping(address => bool) public pAuctioneer;

    // Mapping from owner to operator approvals
    mapping(address => mapping(address => bool)) private _oAuctioneerApprovals;

    uint256 auctioneerRegistrationFee;
    uint96 platformFee;

    function __Auction_init(
        uint96 _platformFee,
        address _auctioneer,
        uint96 _auctioneerFee,
        uint256 _auctioneerRegistrationFee
    ) external initializer {
        __Ownable_init();
        __AuctionValidator_init_unchained();
        setPlatformFee(_platformFee);
        setAuctioneerRegistrationFee(_auctioneerRegistrationFee);
        setPlatformAuctioneer(_auctioneer, _auctioneerFee, true);
    }


    function setPlatformFee(uint96 _platformFee) public virtual onlyOwner {
        platformFee = _platformFee;
    }

    function setPlatformAuctioneer(
        address _auctioneer,
        uint96 _auctioneerFee,
        bool _hasApproval
    ) public virtual onlyOwner {
        pAuctioneer[_auctioneer] = _hasApproval;
        auctioneerFee[_auctioneer] = _auctioneerFee;
        emit PlatformAuctioneer(_auctioneer, _hasApproval);
    }

    function openPlatformAuctioneer(
        address _auctioneer,
        uint96 _auctioneerFee,
        bool _hasApproval
    ) external payable virtual {
        require(
            msg.value == auctioneerRegistrationFee,
            "Incorrect registration fee"
        );
        oAuctioneer[_auctioneer] = _hasApproval;
        auctioneerFee[_auctioneer] = _auctioneerFee;
        emit OpenAuctioneer(_auctioneer, _hasApproval);
    }

    function setAuctioneerRegistrationFee(uint256 _auctioneerRegistrationFee)
        public
        virtual
        onlyOwner
    {
        auctioneerRegistrationFee = _auctioneerRegistrationFee;
    }

    function setAuctioneerApproval(address _auctioneer, bool approved)
        public
        virtual
    {
        require(_auctioneer != _msgSender(), "Self auctioneer");

        _oAuctioneerApprovals[_msgSender()][_auctioneer] = approved;
        emit SellerAuctioneer(_msgSender(), _auctioneer, approved);
    }

    function isApprovedAuctioneer(address owner, address auctioneer)
        public
        view
        virtual
        returns (bool)
    {
        return
            !(rAuctioneer[auctioneer]) &&
            (pAuctioneer[auctioneer] ||
                (oAuctioneer[auctioneer] &&
                    _oAuctioneerApprovals[owner][auctioneer]));
    }

    function removeAuctioneer(address auctioneer, bool _isRemoved)
        public
        virtual
        onlyOwner
    {
        rAuctioneer[auctioneer] = _isRemoved;
        emit RemovedAuctioneer(auctioneer, _isRemoved);
    }

    
    function getSpecificStakeInfo(
        address _nft,
        uint256 _tokenId,
        address addr
    ...
    function getTotalStakeInfo(address addr)
        public
        view
        virtual
        returns (uint256)
    ...
    /**
       Owner puts the item on auction
    */
    ...
    */
    function stakeSpecific(address _nft, uint256 _tokenId)
        external
        payable
        virtual
    {
    ...
    /**
      Before making off-chain stakes potential bidders need to stake eth and either they will get it back when the auction ends or they can withdraw it any anytime.
    */
    function stakeTotal() external payable virtual {
        require(msg.sender != address(0));
        totalStake[msg.sender] += msg.value;
    }
    ...
    function executeLazyAuction(
        ...
        (
            ...
            LibPart.Part memory minter
        ) = IGlipERC721Lazy(_bid.token).decodeLazyMintData(_bid.tokenData);
        ...
        // Verify if auctioneer is approved and collected bid
        require(
            isApprovedAuctioneer(_firstOwner, _auction.maker),
            "Auctioneer must be approved by platform or creator"
        );
        require(
        ...
        // Verify token ids in bid, auction and asset
        require(
            _tokenId == _bid.tokenId,
            "Bid token id does not match asset token id"
        );
        require(
            _tokenId == _auction.tokenId,
        ...
        // Bid value should be less than or equal to staked value of the bid maker
        require(
            _bid.value <= totalStake[_bid.maker],
        ...
        // Transfer required tokens to auction taker
        uint256 auctioneerTake = (value * auctioneerFee[_auction.maker]) / 100;
        uint256 minterTake = (value * minter.value) / 100;
        uint256 platformTake = (value * platformFee) / 100;

        totalStake[_auction.taker] += auctioneerTake;
        totalStake[minter.account] += minterTake;
        totalStake[owner()] += platformTake;
        ...
        for (uint256 i = 0; i < payouts.length; i++) {
            totalStake[payouts[i].account] += (value * payouts[i].value) / 100;
    ...
    function withdrawSpecificStake(address _nft, uint256 _tokenId)
        external
        virtual
    {
        require(msg.sender != address(0));
    ...
    function withdrawTotalStake(uint256 _amount) external virtual {
        require(msg.sender != address(0));
        require(
    ...
    function stakeTotal2() external payable virtual {
        require(msg.sender != address(0));
        totalStake[msg.sender] += msg.value;
    }

    function stakeSpecific2(address _nft, uint256 _tokenId)
        external
        payable
        virtual
    {
    ...
    function upsertDefaultMinter(
        address token,
        address minter,
        uint96 fee
    ) external virtual onlyOwner {
        IGlipERC721Lazy(token).upsertDefaultMinter(minter, fee);
    }
}
...
interface IGlipERC721Lazy is IERC721Upgradeable {
    ...
    function decodeLazyMintData(bytes calldata encoded) external virtual view returns(
            uint256, // Token id
            uint256, // Reserve
            address, // Primary creator
            LibPart.Part[] memory, // Payout split %s
            LibPart.Part memory // Minter fee %
    );
    ...
    function upsertDefaultMinter(address minter, uint96 fee) external virtual;
...
library LibLazyBidERC721 {
    ...
    bytes32 constant BID_TYPEHASH = keccak256(
        "Bid(address maker,address taker,address token,address auctioneer,uint256 tokenId,bytes tokenData,uint256 value,uint256 start,uint256 end)"
    );
    struct Bid {
        address maker;
        address taker;
