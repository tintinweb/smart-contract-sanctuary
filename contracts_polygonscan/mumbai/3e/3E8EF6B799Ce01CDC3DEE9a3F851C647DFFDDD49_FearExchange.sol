 ...
 */
contract FearExchange is ERC1155Holder, AccessControl, Pausable, ReentrancyGuard {
    ...
    IFearNft private fearNftToken = IFearNft(0xB9aeE1Ad85bed213B53329F060A13328EB26b7a0);
    address private fearEarnToPlayAddress = address(0);
    ...
    function _exchangeRateIsSet(uint256 id) private view returns (bool) {
        uint256 nftPriceSum = 0;
        for (uint256 j = 0; j < currencyAssets.length(); j++)
            nftPriceSum = nftPriceSum.add(exchangeRates[id][currencyAssets.at(j)]);
    ...
    function _allExchangeRatesAreSet() private view returns (bool) {
        for (uint256 i = 0; i < tradableNfts.length(); i++) {
            if (!_exchangeRateIsSet(tradableNfts.at(i)))
                return false;
        }
    ...
    /**
    * @dev Sets the Fear NFT contract address.
    * @param addr The address of Fear NFT contract.
    * Requirements:
    * - the caller must have the `DEFAULT_ADMIN_ROLE`;
    * - contract must be paused;
    * - address must be non-zero.
    */
    function setFearNftTokenContract(address addr) external onlyAdmin whenPaused {
        require(addr != address(0), "Cannot set FEAR NFT Token contract to zero address");
        fearNftToken = IFearNft(addr);
    }

    /**
    * @dev Sets the Fear PlayToEarn contract address.
    * @param addr The address of Fear PlayToEarn contract.
    * Requirements:
    * - the caller must have the `DEFAULT_ADMIN_ROLE`;
    * - contract must be paused;
    * - address must be non-zero.
    */
    function setFearPlayToEarnAddress(address addr) external onlyAdmin whenPaused {
        require(addr != address(0), "Cannot set FEAR Play To Earn contract to zero address");
        fearEarnToPlayAddress = addr;
    }

    /**
    * @dev Adds tradable NFT to the exchange.
    * @param id NFT id.
    * Requirements:
    * - the caller must have the `DEFAULT_ADMIN_ROLE`;
    * - contract must be paused;
    ...
    */
    function addTradableNft(uint256 id) external onlyAdmin whenPaused {
        require(!tradableNfts.contains(id), "Tradable NFTs collection already contains this id");
        tradableNfts.add(id);
    }

    /**
    * @dev Sets tradable NFT exchange rate.
    ...
    * - assets array must be not empty;
    * - prices array must be not empty;
    * - assets and prices array must be the same length;
    * - exchange rate for NFT with the same id wasn't set before.
    */
    function setTradableNftExchangeRate(uint256 id, address[] calldata assets, uint256[] calldata prices) external onlyAdmin whenPaused {
        require(tradableNfts.contains(id), "Tradable NFTs collection doesn't contain this id");
        require(assets.length != 0, "Assets must contain at lease one asset address");
        require(assets.length == prices.length, "Assets and prices lengths must be equal");
        require(!_exchangeRateIsSet(id), "Exchange rate for this NFT is already set");

        for (uint256 i = 0; i < assets.length; i++) {
            require(assets[i] != address(0), "Cannot set zero asset address");
            require(prices[i] > 0, "Cannot set zero price");
    ...
    */
    ...
    /**
    * @dev Returns commission percentage.
    * This percentage of each buy will be automatically transferred to Fear PlayToEarn contract.
    ...
    * Transfers assets from caller address to this contract according to exchange rate, sends requested amount of NFT back to callers address.
    * @param id NFT id.
    * @param amount Amount of NFTs to buy.
    * Requirements:
    * - NFT with this id must be added as tradable NFT;
    * - Amount must be greater than zero;
    * - Caller address must have enough assets to buy specified amount of NFTs;
    * - FearExchange contract must be approved to transfer enough amount of required assets.
    */
    function buyNft(uint256 id, uint256 amount) external whenNotPaused nonReentrant {
        ...
        for (uint256 i = 0; i < currencyAssets.length(); i++) {
            ...
            if (price != 0) {
                ...
                require(allowance  >= price.mul(amount), "Insufficient FearExchange allowance");

                asset.transferFrom(_msgSender(), address(this), price.mul(amount));
                asset.transfer(fearEarnToPlayAddress, price.mul(amount).div(100).mul(commissionPercentage));
            }
        }
    ...
    * Transfers NFT from caller address to this contract, sends assets back to callers address according to exchange rate minus commission.
    * @param id NFT id.
    * @param amount Amount of NFTs to sell.
    * Requirements:
    * - NFT with this id must be added as tradable NFT;
    * - Amount must be greater than zero;
    * - Caller address must have enough NFTs on balance;
    * - FearExchange contract must be approved to transfer caller's NFTs.
    */
    function sellNft(uint256 id, uint256 amount) external whenNotPaused nonReentrant {
        ...
        require(contractApproved, "FearExchange contract must be approved to operate caller's NFTs");
        ...
        for (uint256 i = 0; i < currencyAssets.length(); i++) {
            ...
            if (price != 0) {
                IERC20 asset = IERC20(currencyAssets.at(i));
                asset.transfer(_msgSender(), price.mul(amount).sub(price.mul(amount).div(100).mul(commissionPercentage)));
            }
        }
    ...
    /**
    * @dev Pauses all exchange operations.
    * Requirements:
    ...
    * - the caller must have the `PAUSER_ROLE`;
    * - contract must be unpaused;
    * - this contract must be a minter of Fear NFT;
    * - Fear PlayToEarn contract must be set;
    * - At least one NFT must be added as tradable NFT;
    * - All exchange rates must be set.
    */
    function unpause() external onlyPauser whenPaused {
        require(_isMinterOfFearNft(), "Before unpausing exchange contract must be a minter of FearNft contract");
        require(fearEarnToPlayAddress != address(0), "Before unpausing FEAR Play To Earn contract address must be set");
        require(tradableNfts.length() > 0, "Before unpausing at least one tradable NFT must be added");
        require(_allExchangeRatesAreSet(), "Before unpausing all exchange rates must be set");
    ...
    */
    function emergencyNftWithdrawal(uint256[] calldata ids, address to) external onlyAdmin {
    ...
    * - 'to' address must be non-zero.
    */
    function emergencyAssetWithdrawal(address asset, address to) external onlyAdmin {
