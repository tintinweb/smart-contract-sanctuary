...
// File: game/libs/IMasterChef.sol
...
// Miner governs the transaction mining of TOSSY rewards and distribution of referral fees.
/*
Miner contract is responsible for the following tasks:
- Collect TOSSY yield from MasterChef using a dummy ERC20 token.
...
contract Miner is Ownable, ReentrancyGuard {
    ...
    // TOSSY token contract.
    IERC20 constant TOSSY_TOKEN = IERC20(0x08B71e73fF566616bef1E0BE33614e38B79549EA);

    // MasterChef contract is the yield farm for TOSSY token.
    IMasterChef constant masterChef = IMasterChef(0x5486C9632A1D0d29dcC462039345F2090B4884e4);

    // SpintossTossy contract address. Distribute some TOSSY emissions to TOSSY game contract.
    address public spintossTossy;

    // Percentage of TOSSY harvested to go to TOSSY game contract.
    uint public spintossTossyPct = 10;
    ...
    // Total pending TOSSY rewards by all users.
    uint public allPendingReward;

    // Map user address to pending TOSSY rewards.
    ...
    // Stake dummy token in MasterChef pool to earn TOSSY.
    ...
    // Harvest TOSSY token from MasterChef pool.
    function harvestMasterChef() internal {

        // Store initial balance.
        uint initialBalance = TOSSY_TOKEN.balanceOf(address(this));

        // Harvest TOSSY from masterChef.
        masterChef.withdraw(pid, 0);

        // Compute amount of TOSSY harvested.
        uint harvestAmount = TOSSY_TOKEN.balanceOf(address(this)) - initialBalance;

        // Allocate some TOSSY to TOSSY game contract.
        safeTossyTransfer(spintossTossy, harvestAmount.mul(spintossTossyPct).div(100));
    }
    ...
    // Set SpintossTossy contract address.
    function setSpintossTossy(address _spintossTossy) external onlyOwner {
        spintossTossy = _spintossTossy;
    }

    // Set percentage of TOSSY harvested to go to TOSSY game contract.
    function setSpintossTossyPct(uint _spintossTossyPct) external onlyOwner {
        spintossTossyPct = _spintossTossyPct;
    }
    ...
    // Allow Spintoss game contract to add pending TOSSY rewards for user.
    ...
    // Withdraw pending TOSSY rewards.
    function withdraw() public nonReentrant {
        uint pending = userReward[msg.sender];
        harvestMasterChef();
        require(pending <= TOSSY_TOKEN.balanceOf(address(this)), "Withdrawal exceeds balance. Wait for Miner to farm more TOSSY.");
        if (pending > 0) {
            allPendingReward = allPendingReward.sub(pending);
            userReward[msg.sender] = 0;
            safeTossyTransfer(msg.sender, pending);
            payReferralFee(msg.sender, pending);
            emit Withdraw(msg.sender, pending);
        }
    }

    // Safe TOSSY transfer function, in case if rounding error causes pool to not have enough TOSSY.
    function safeTossyTransfer(address _to, uint _pending) internal {
        uint tossyBal = TOSSY_TOKEN.balanceOf(address(this));
        if (_pending > tossyBal) {
            TOSSY_TOKEN.transfer(_to, tossyBal);
        } else {
            TOSSY_TOKEN.transfer(_to, _pending);
        }
    }
    ...
    function payReferralFee(address _user, uint _pending) internal {
        if (referralFeeBP > 0) {
            ...
            if (referrer != address(0) && referrer != 0x000000000000000000000000000000000000dEaD) {
                ...
                safeTossyTransfer(referrer, referralFee);
