...
contract PropertyVerification is Ownable {
    ...
    /**
     * NOTE this should exist in the marketplace contract (todo remove once marketplace is built)
     * @dev create a Booking nft after checking that the sender is the property owner
     * @param listingId listing id provided by the backend
     * @param checkInTime check in time for the rental property
     * @param checkOutTime check out time for the rental property
     * @param propertyId the property id in chain
     * @param rentCost the payment amoutn for the rent
     */
    function approveBooking(
        ...
        require(
            ...
            'the check in time has passed already'
        );
        // check out time must be later than the check in time
        require(
            checkInTime < checkOutTime,
            'the check out time must be after check in time'
        );
        require(rentCost > 0, 'rent cost must be a positive number');

        require(msg.value == rentCost, 'Not enough money sent');
        ...
        uint256 escrowId = EscrowContract.createEscrow{value: rentCost}(
            // the escrow should be released once the dispute time is over
            checkInTime + disputeTimeLimit,
    ...
    function unlistProperty(uint256 propertyId) external onlyWhitelisted {
        ...
        // TODO send verification deposit to marketplace here. 
    }
