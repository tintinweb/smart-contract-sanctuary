...
contract MeowItem is AccessControl, ERC1155, ContextMixin, NativeMetaTransaction {
  ...
  constructor(string memory name_, string memory uri_, address target) ERC1155(uri_) {
    name = name_;
    owner = msg.sender;
  ...
  function mint(address account, uint id, uint amount) external onlyRole(MINTER_ROLE) {
    _mint(account, id, amount, "");
  }

  function batchMint(address account, uint[] calldata ids, uint[] calldata amounts) external onlyRole(MINTER_ROLE) {
    _mintBatch(account, ids, amounts, "");
  }

  function bulkMint(address[] calldata accounts, uint[] calldata ids, uint[] calldata amounts) external onlyRole(MINTER_ROLE) {
    for (uint i = 0; i < accounts.length; ++i) {
      _mint(accounts[i], ids[i], amounts[i], "");
    }
  }

  mapping(uint => bool) private _nonce;

  function issue(address account, uint id, uint nonce, bytes32 sigR, bytes32 sigS, uint8 sigV) external {
    require(issuer != address(0), "no issuer");
    require(_nonce[nonce] == false, "duplicated");
    bytes32 h = keccak256(abi.encodePacked(this, account, id, nonce));
  ...
  function issued(uint nonce) external view returns (bool) {
    return _nonce[nonce];
  }

  event Equip(uint, uint);

  function equip(uint meowId, uint id) external {
    require(IERC721(targetAddress).ownerOf(meowId) == _msgSender(), "not owner");
    _burn(_msgSender(), id, 1);
    emit Equip(meowId, id);
  }
