...
contract Market is Ownable {
    ...
    event MarketFeeReceiverModified( address receiver );
    ...
    // 手数料の受け取りアドレス
    address private _market_fee_receiver;
    ...
    //-----------------------------------------
    constructor() Ownable() {
        _market_fee_receiver = msg.sender;

        emit MarketFeeReceiverModified( _market_fee_receiver );

        _default_fee_rate_market = 500;         //  5.0 %
        _default_fee_rate_collection = 1500;    // 15.0 %
    ...
    function marketFeeReceiver() external view returns (address) {
        return( _market_fee_receiver );
    }
    ...
    function setMarketFeeReceiver( address receiver ) external onlyOwner {
        _market_fee_receiver = receiver;
    ...
    function setFixedFeeRateForMarket( address contractAddress, uint256 rate ) external onlyOwner {
        ...
        emit MarketFeeModified( contractAddress, _fixed_fee_rate_market[contractAddress] );
    }
    ...
    function setFixedFeeRateForCollection( address contractAddress, uint256 rate ) external onlyOwner {
        ...
        emit CollectionFeeModified( contractAddress, _fixed_fee_rate_collection[contractAddress] );
    }
    ...
    // [external/onlyOwner] 運営による代理購入（クレカ支払い等を想定）
    function buyWithAdmin( uint256 saleId, address buyer ) external payable onlyOwner {
        require( address(_sale) != address(0), "invalid sale" );
        require( buyer != address(0), "invalid buyer" );

        _sale.buy( buyer, saleId, msg.value );
    ...
    // [external/onlyOwner] 運営による代理購入（クレカ支払い等を想定）
    function dutchAuctionBuyWithAdmin( uint256 auctionId, address buyer ) external payable onlyOwner {
        require( address(_dutch_auction) != address(0), "invalid dutch_auction" );
        require( buyer != address(0), "invalid buyer" );

        _dutch_auction.buyDutchAuction( buyer, auctionId, msg.value );
    ...
    function present( address contractAddress, uint256 tokenId, address to ) external {
        require( address(contractAddress) != address(0), "invalid contract address" );
    ...
    function _pay( uint256[3] memory words ) internal {
        ...
        // マーケット手数料の受け取りアドレス
        address payable market = payable( _market_fee_receiver );
