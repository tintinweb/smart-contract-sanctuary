...
contract Pool is Ownable, ReentrancyGuard, IPool {
    ...
    event Redeemed(address user, uint256 dollarAmount, uint256 shareAmount, uint256 collateralAmount);
    event Minted(address user, uint256 shareAmount, uint256 collateralAmount, uint256 dollarAmount);
    event RedemptionCollected(address user, uint256 shareAmount, uint256 collateralAmount);
    ...
    function mint(
        uint256 _collateral_amount, // 0.000
        uint256 _share_amount, // 0
        uint256 _dollar_out_min // 0
        ...
        emit Minted(msg.sender, _required_share_amount, _collateral_amount, _actual_dollar_amount);
    }
    ...
    function redeem(
        ...
        emit Redeemed(msg.sender, _dollar_amount, _share_output_amount, _collateral_output_amount);
    }
