...
contract CakeVault is Ownable, Pausable {
    ...
    constructor(
        ...
        // Infinite approve
        IERC20(_token).safeApprove(address(_masterchef), MASK);
    }
    ...
    function calculateHarvestCakeRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingsocialswap(0, address(this));
     ...
     */
    function calculateTotalpendingsocialswapRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingsocialswap(0, address(this));
        amount = amount.add(available());
