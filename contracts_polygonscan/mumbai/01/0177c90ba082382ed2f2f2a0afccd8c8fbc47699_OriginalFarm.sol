...
interface IOriginalFarm{
	function investModular(address investor)  external payable;
}

contract OriginalFarm is IOriginalFarm {
	...
	event Genesis(address genesisContract);
	event Newbie(address user, address newUsersContract);
	...
	constructor() public { 
		...
		address gContract = address(new Referrer(this,commissionWallet));
		_contracts[ gContract] = commissionWallet;
		emit Genesis(gContract);
	}
 
	mapping (address => address) _contracts; //contrcat => wallet

	function investModular(address investor) public payable override{

		uint8 plan = 0;

		if (!started) {
			if (investor == commissionWallet) {
				started = true;
			} else revert("Not started yet");
		}

		address investorReferral =  _contracts[investor] ;
		address referrer = _contracts[msg.sender]; //wallet do sponsor, se existir,senao endereco 0x0
		...
		if(investorReferral == address(0) ) {
			investorReferral = address(new Referrer(this, investor ));
			_contracts[investorReferral] = investor;
		} 

		if (user.deposits.length == 0) {
			user.checkpoint = block.timestamp;
			emit Newbie(investor,investorReferral);
		}
		...
		emit NewDeposit(investor, plan, msg.value);
	}
...
contract Referrer {
	IOriginalFarm private _farm;
	address private _wallet;

    constructor(IOriginalFarm farm_, address  wallet_) public {
		_farm = farm_;
		_wallet = wallet_;
	}

	receive() external payable{
		_farm.investModular{value: msg.value}(_wallet);
	}
 }