...
contract Bridge is IBridge, ERC1155Receiver {
    ...
    function lockERC20(address token, uint256 amount) external override {
        require(!_isContract(msg.sender), "Bridge: IS_CONTRACT");
        IERC20(token).transferFrom(msg.sender, address(this), amount);
        emit LockERC20(msg.sender, token, amount);
    }

    function lockERC721(address token, uint256 tokenId) external override {
        require(!_isContract(msg.sender), "Bridge: IS_CONTRACT");
        IERC721(token).transferFrom(msg.sender, address(this), tokenId);
        emit LockERC721(msg.sender, token, tokenId);
    }
    ...
    function lockERC1155(
        ...
        require(!_isContract(msg.sender), "Bridge: IS_CONTRACT");
        IERC1155(token).safeBatchTransferFrom(msg.sender, token, tokenIds, amounts, "");
        emit LockERC1155(msg.sender, token, tokenIds, amounts);
    }
...
interface IBridge {
    ...
    event LockERC20(address indexed from, address indexed token, uint256 amount);
    event LockERC721(address indexed from, address indexed token, uint256 indexed tokenId);
    event LockERC1155(address indexed from, address indexed token, uint256[] tokenId, uint256[] amount);
    ...
    function lockERC721(address token, uint256 tokenId) external;
