...
// File contracts/ApymonWarrior.sol
    ...
    ERC721EnumerableUpgradeable
{
    function initialize(
        ...
        __ApymonWarrior_init(name, symbol, baseTokenURI);
    }

    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
    string public _baseTokenURI;

    function __ApymonWarrior_init(
        string memory name,
        string memory symbol,
        string memory baseTokenURI
        ...
        __ERC721Enumerable_init_unchained();
        __ApymonWarrior_init_unchained(name, symbol, baseTokenURI);
    }

    function __ApymonWarrior_init_unchained(
        string memory name,
        string memory symbol,
        string memory baseTokenURI
    ) internal initializer {
    ...
    function setBaseURI(string memory uri) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "ApymonWarrior: Must be admin to update.");
        _baseTokenURI = uri;
    }

    function mint(address to, uint256 tokenId) public  {
        require(hasRole(MINTER_ROLE, _msgSender()), "ApymonWarrior: must have minter role to mint");
        _mint(to, tokenId);
    }
