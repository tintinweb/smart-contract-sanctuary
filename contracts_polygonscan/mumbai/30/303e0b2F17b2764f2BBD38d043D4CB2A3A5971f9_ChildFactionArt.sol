...
import {ERC1155Supply, ERC1155, Context} from "@openzeppelin/contracts/token/ERC1155/extensions/ERC1155Supply.sol";
    ...
    ContextMixin
{
    ...
    constructor (
        ...
        ERC1155(uri_)
    {
        _setupContractId("ChildFactionArt");
    ...
    */
        ...
        only(DEPOSITOR_ROLE)
    {
        ...
        require(
            user != address(0),
            "ChildFactionArt: INVALID_DEPOSIT_USER"
        );
    ...
    function mint(
        ...
        // Restrict minting of tokens to only the minter role
        require((id > uint(0)), "ChildFactionArt: INVALID_TOKEN_ID");
        require(((amount > uint(0)) &&
          ...
          (id > uint(4)))), "ChildFactionArt: INVALID_AMOUNT");
    ...
    function mintBatch(
        ...
        for (uint i = 0; i < ids.length; i++) {
            require((ids[i] > uint(0)), "ChildFactionArt: INVALID_TOKEN_ID");
            require(((amounts[i] > uint(0)) &&
                 (((ids[i] == uint(1)) && (amounts[i] == restTotalSupply(ids[i], amounts[i]))) ||
                  ...
                  (ids[i] > uint(4)))), "ChildFactionArt: INVALID_AMOUNT");
        }
    ...
    function burn(
        ...
        require(
            ...
            "ChildFactionArt: caller is not owner nor approved"
        );
    ...
    function restTotalSupply(uint256 _id, uint256 _amount) public view returns (uint256) {
      ...
      if (_id == uint(1)) {
        require(( subtotal <= uint256(645_000_000_000_000_000_000) ), "ChildFactionArt: EXCEED MAX_AMOUNT");
      } else if (_id == uint(2)) {
        require(( subtotal <= uint256(1263_000_000_000_000_000_000) ), "ChildFactionArt: EXCEED MAX_AMOUNT");
      } else if (_id == uint(3)) {
        require(( subtotal <= uint256(1267_000_000_000_000_000_000) ), "ChildFactionArt: EXCEED MAX_AMOUNT");
      } else if (_id == uint(4)) {
        require(( subtotal <= uint256(3141_000_000_000_000_000_000) ), "ChildFactionArt: EXCEED MAX_AMOUNT");
      }
    ...
    function _beforeTokenTransfer(
        ...
        require(!paused(), "ChildFactionArt: token transfer while paused");
