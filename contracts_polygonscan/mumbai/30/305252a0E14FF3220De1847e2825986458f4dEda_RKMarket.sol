...
contract RKMarket is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    //    function lotsOfSeller(address seller) external view returns (uint256[] calldata ids) {
    //        for(uint256 i = 0; i < lots.length; i++) {
    //            if (lots[i].seller == seller) {
    //                ids.push(i);
    //            }
    //        }
    //    }
    ...
    function secondarySale(
        ...
        require(artists[nft][tokenId] != address(0), "add artist first");
    ...
    function buy(uint256 lotId) external splitterExist nonReentrant {
        ...
        address _artist = artists[_lot.nft][_lot.tokenId];
        if (_lot.isPrimary) {
            (
                address[] memory addresses,
                uint256[] memory shares
            ) = SplitterContract(splitter).getPrimaryDistribution(_artist);
            ...
            (
                address[] memory addresses,
                uint256[] memory shares
            ) = SplitterContract(splitter).getSecondaryDistribution(_artist);
            require(addresses.length == shares.length, "arrays not equal");
            require(addresses.length > 0, "arrays are empty");
        }
        ...
        if (_lot.isPrimary) {
            SplitterContract(splitter).primaryDistribution(_artist, _lot.price);
        } else {
            SplitterContract(splitter).secondaryDistribution(
                _artist,
                _lot.seller,
                _lot.price
            );
