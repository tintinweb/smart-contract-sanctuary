...
interface IinvestedPool {
    ...
    function getTotalRedumptedAmount() external view returns(uint256);
...
contract riskPool is Ownable {
    ...
    uint256 public constant T30days = 2592000;
    ...
    uint256 public insuredAddressParameter;
    ...
    function returnExcessPremium(uint256 insuredId, address _receiver) public {
        require(msg.sender == insuredAddress[insuredId], "You are not insured.");
        require(earthquakeOccuranceTime[0] > Contents[insuredId].applyingtime + baseInsuredPeriod
