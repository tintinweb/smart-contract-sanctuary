...
contract TribeGold is ERC20, Pausable, Ownable {
    bool public mintFrozen = false;

    constructor(uint256 initialAmount) ERC20("TribeGold", "TRBG") {
        mintTribeGold(_msgSender(), initialAmount);
    }

    function mintTribeGold(address account, uint256 amount) public onlyOwner returns (bool) {
        require(!mintFrozen, "minting is frozen");
        _mint(account, amount);
        return true;
    }

    function freezeMint() public onlyOwner {
        mintFrozen = true;
    }
    ...
    function _beforeTokenTransfer(
        ...
        require(!paused(), "ERC20Pausable: Cannot transfer tokens while paused");
    }
