...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
  ...
  // mapping for token creator
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if(revealEndTime >= block.timestamp) {
...
contract CryptocornNFT721V1 is ERC721 {
    ...
    uint256 public remainingAssert = 8;
    ...
    function createCollectible( string memory tokenURI, uint256 fee, Sign memory sign, bool _bountyNFT) public payable returns (uint256) {
        ...
        count = whitelisted[msg.sender] && block.timestamp <= revealStartTime ? count + 1 : mintLimit();
        ...
        if(_bountyNFT) {
            ...
            holdingTime[msg.sender][newItemId] = revealEndTime;
        }
        tokenCounter = tokenCounter + 1;
        return newItemId;
    }
    
    function mintLimit() internal returns(uint256){
        require(block.timestamp >= revealStartTime,"minting is not Enable");
        if(block.timestamp <= revealStartTime.add(1 days)) {
            require(count < remainingAssert,"ERC721: today limit is reached");
            count = count + 1;
            return count;
        }
        else {     
            revealStartTime = revealStartTime.add(1 days);
            remainingAssert = remainingAssert.add(remainingAssert.sub(count));
            count = 1;
            return count;
        }
    }
    ...
    function enableMint() public onlyOwner returns(bool){
        mintEnable = true;
        revealStartTime = block.timestamp.add(1 hours);
        revealEndTime = revealStartTime.add(4 days);
        return(true);    
    }
