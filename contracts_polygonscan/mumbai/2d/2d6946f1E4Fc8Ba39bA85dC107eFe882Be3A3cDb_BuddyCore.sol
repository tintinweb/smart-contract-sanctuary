...
contract BuddyCore is IBuddyCore, ERC721Full, ERC721Holder {
    ...
    function buyGen0() external payable hasPayment whenNotPaused {
    ...
    function massApoptosis(uint256[] memory buddies, uint256[] memory receivers)
        external
        whenNotPaused
    {
        for (uint256 i = 0; i < buddies.length; i++) {
    ...
    function toggleLock(uint256 _tokenId) external whenNotPaused {
        BuddyLib.Buddy storage base = _data[_tokenId];
    ...
    function isLocked(uint256 _tokenId) public view override returns (uint8) {
        ...
        if (base.lock == 0) {
            return 0;
        } else if (base.lock + 300 > block.timestamp) {
            return 1;
        }
...
interface IBuddyLogic {
    ...
    function calcFounderClaim(BuddyLib.Buddy calldata base)
        external
        view
        returns (uint256);

    function founderClaim(BuddyLib.Buddy calldata base)
        external
        returns (uint256);
    ...
    function simpleReplicatePreview(BuddyLib.Buddy calldata base)
        external
        view
        returns (uint256);
