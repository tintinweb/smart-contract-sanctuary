...
contract LuckyShiba is Ownable, ERC20Pausable { 
    ...
    event BuyEvent(string period, uint256 ticket, uint256 amount);
    ...
    constructor() ERC20("Metaverse", "META") {
        ...
        uint256 initialSupply = 1 * oneBillion * placeOfDecimals; // 1 Billion
    ...
    function buy(string memory period, uint256 ticket) public payable {  
        require(isDisabled() == false, "The activity is not started yet!");

        _ticketRecord[period][ticket].push(msg.sender);
        ...
        emit BuyEvent(period, ticket, uint256(msg.value));
    }
    ...
    function randomLuckyFigures(uint256 randomSeed) public onlyOwner { 
        _randomNumber = randomSeed;
    } 
    ...
    function sendReward(string memory period) public onlyOwner {
        require(_randomNumber > 0, "The random number hasn't set up yet!");

        uint256 n = _randomNumber;
        address[] memory addrs = _ticketRecord[period][n];
        ...
        uint256 toEveryWinner = (balance * _feeRate) / 100 / addrs.length;

        // to platform
        // uint256 toThePlatform = balance * 0.4;

        // distribute to every winner
        for (uint256 i = 0; i < addrs.length; i++) {
            payable(addrs[i]).transfer(toEveryWinner); 
        }

        emit SendRewardEvent(period, n, toEveryWinner, addrs);
    }
