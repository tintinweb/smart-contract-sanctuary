 ...
 *         \/        \/            \/      \/      \/      \/              \/      \/
 *
 * @title Atlantis World Seasons
 *
 * @author Carlo Miguel Dy
 *
 * @notice An NFT airdrop manager for all community members & supporters on various Seasons
 *
 * @dev
 ...
 * - It is capable of adding multiple {Season} struct to allow various airdrop events long term
 * - It is capable of airdropping NFTs of a specified {Season} to a list of recipients and only
 ...
 * - It will emit an event when a merkle root is added to a specific {Season}, and can only be set
 *   for once and can never change a merkle root
 * - It will emit an event when a new {Season} has been created by a AW Core Team member
    ...
    AccessControlEnumerable
{
    ...
    /**
     * @dev The total count of all {Season} stored in `_seasonIdToSeason` mapping.
     */
    ...
    /**
     * @dev Stores all of the created {Season}.
     */
    mapping(uint256 => Season) private _seasonIdToSeason;

    /**
     * @dev Stores all merkle root for a corresponding {Season}.
     */
    mapping(uint256 => bytes32) private _seasonIdToMerkleRoot;

    /**
     * @dev Indicates if a {Season} already has a merkle root set.
     */
    mapping(uint256 => bool) private _seasonIdToMerkleRootStatus;
    ...
    // /**
    //  * @dev Keeps track of all the mint count for each {Season} stored
    //  * in `_seasonIdToSeason` mapping since there are various {Season}
    //  * with different JSON metadata structure. This mapping is most
    //  * likely used by the method `getSeasonTotalMintCount` to expose
    //  * the total mint count for each {Season} NFTs.
    //  *
    //  * @custom:deprecated
    //  * This does not work as per {ERC721-ownerOf} standard.
    //  */
    // mapping(uint256 => Counters.Counter) private _seasonIdToCount;

    /**
     * @dev Store if a specific address have claimed for a {Season}.
     */
    mapping(address => mapping(uint256 => bool))
        private _claimantToSeasonIdToClaimStatus;

    /**
     * @dev The data structure for how seasons are stored to `_seasonIdToSeason` mapping.
     */
    struct Season {
        /**
         * @dev The address that created this {Season}.
         */
        address author;
        /**
         * @dev Will match the to `_seasonIds` the counter upon creation.
         */
        uint256 id;
        /**
         ...
         * to check if a {Season} exists in mapping `_seasonIdToSeason`
         */
        bool exists;
    }

    constructor() ERC721("Atlantis World Seasons", "AWS") {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
    }

    /**
     * @dev When an NFT is claimed by a whitelisted address for a specific {Season}.
     */
    event GiftClaimed(
    ...
    /**
     * @dev When an NFT is airdropped by an AW Core Team member for a specific {Season}.
     */
    event Airdrop(
    ...
    /**
     * @dev When a new {Season} has been created.
     */
    event SeasonCreated(address indexed author, uint256 indexed seasonId);

    /**
     * @dev When a new merkle root for a {Season} is added.
     */
    event SeasonMerkleRootAdded(
    ...
    /**
     * @notice Checks if the provided `seasonId` parameter has an existing {Season} stored in `_seasonIdToSeason` mapping
     * @param seasonId The identifier of a {Season} stored from `_seasonIdToSeason` mapping
     */
    modifier existingSeason(uint256 seasonId) {
        require(
            _seasonIdToSeason[seasonId].exists,
            "There is no existing Season on given ID."
        );
        _;
    }
    ...
    /**
     * @notice Get all the stored {Season} from `_seasonIdToSeason` mapping
     *
     * @dev
     * When traversing items from `_seasonIdToSeason` mapping, must add
     * plus 1 to its index to correctly get the value from the mapping since
     * the `_seasonIds` counter starts at 1 and not 0.
     */
    function getSeasons() external view returns (Season[] memory) {
        uint256 totalSeasonCount = _seasonIds.current();

        Season[] memory seasons = new Season[](totalSeasonCount);

        for (uint256 index = 0; index < totalSeasonCount; index++) {
            seasons[index] = _seasonIdToSeason[index + 1];
        }
    ...
    /**
     * @param seasonId The identifier of a {Season} from `_seasonIdToSeason` mapping
     * @return The {Season} that corresponds to given `seasonId`
     */
    function getSeason(uint256 seasonId) external view returns (Season memory) {
        return _seasonIdToSeason[seasonId];
    }

    /**
     * @return The total count for all the seasons created.
     */
    function getTotalSeasonCount() external view returns (uint256) {
        return _seasonIds.current();
    }
    ...
    // /**
    //  * @param seasonId The identifier of a {Season} from `_seasonIdToSeason` mapping
    //  * @return The total count for all that have minted of a specific {Season}
    //  *
    //  * @custom:deprecated
    //  * This does not work as per {ERC721-ownerOf} standard.
    //  */
    // function getSeasonTotalMintCount(uint256 seasonId)
    //     external
    //     view
    //     returns (uint256)
    // {
    //     return _seasonIdToCount[seasonId].current();
    // }

    /**
     * @param claimant The whitelisted address
     * @param seasonId The identifier of a {Season} from `_seasonIdToSeason` mapping
     * @return When the address have minted for a {Season} then it returns `true`
     */
    function claimed(address claimant, uint256 seasonId)
        public
        view
        returns (bool)
    {
        return _claimantToSeasonIdToClaimStatus[claimant][seasonId];
    }
    ...
    function isWhitelisted(
        ...
        return
            MerkleProof.verify(
                proof,
                _seasonIdToMerkleRoot[seasonId],
     ...
     * @param seasonId The identifier of a {Season} stored from `_seasonIdToSeason` mapping
     */
    function claim(bytes32[] calldata proof, uint256 seasonId)
        external
        existingSeason(seasonId)
        nonReentrant
    {
        require(isWhitelisted(msg.sender, seasonId, proof), "Not whitelisted.");
        require(!claimed(msg.sender, seasonId), "You have already claimed!");

        // The {Season} object by a given `seasonId`
        // from `_seasonIdToSeason` mapping
        Season memory season = _seasonIdToSeason[seasonId];

        // Increment the token mint counter for a {Season}
        _tokenIds.increment();

        // The current mint token count for a {Season}
        uint256 currentTokenId = _tokenIds.current();

        // Set the mint status to true, indicates as claimed for a {Season}
        _claimantToSeasonIdToClaimStatus[msg.sender][seasonId] = true;

        _safeMint(msg.sender, currentTokenId);
        _setTokenURI(currentTokenId, season.tokenURI);

        emit GiftClaimed(msg.sender, seasonId, currentTokenId);
    }

    /**
     * @notice
     * Make an airdrop of a specific {Season} NFT to specificed `recipients`, and
     ...
     * @param seasonId The identifier of a {Season} stored from `_seasonIdToSeason` mapping
     */
    function airdrop(address[] calldata recipients, uint256 seasonId)
        external
        onlyAdmin
        existingSeason(seasonId)
    {
        ...
        for (uint256 index = 0; index < recipients.length; index++) {
            ...
            require(
                !_claimantToSeasonIdToClaimStatus[recipient][seasonId],
            ...
            // Set the mint status to true, indicates as claimed for a {Season}
            _claimantToSeasonIdToClaimStatus[recipient][seasonId] = true;

            _safeMint(recipient, currentTokenId);
            _setTokenURI(currentTokenId, season.tokenURI);

            emit Airdrop(recipient, seasonId, currentTokenId);
        }
    }
    ...
    /**
     * @dev Stores a new {Season} into `_seasonIdToSeason` mapping
     * @param tokenURI The base64 ecnoded JSON metadata for a {Season}
     * @return The current `seasonId` for the recently created {Season}
     */
    function createSeason(string calldata tokenURI)
        external
        onlyAdmin
        returns (uint256)
    {
        // Increment `_seasonIds` after a new {Season} is stored
        // to the `_seasonIdToSeason` mapping
        _seasonIds.increment();

        uint256 currentSeasonId = _seasonIds.current();
        _seasonIdToMerkleRootStatus[currentSeasonId] = false;

        _seasonIdToSeason[currentSeasonId] = Season({
            author: msg.sender,
            id: currentSeasonId,
            tokenURI: tokenURI,
            exists: true
        ...
        emit SeasonCreated(msg.sender, currentSeasonId);

        return currentSeasonId;
    }

    /**
     * @notice Adds a new merkle root for a {Season} whitelist.
     * @param seasonId The identifier of a {Season} stored from `_seasonIdToSeason` mapping
     * @param merkleRoot The generated merkle root from the whitelisted addresses for the {Season}
     */
    function addSeasonMerkleRoot(uint256 seasonId, bytes32 merkleRoot)
        external
        onlyAdmin
        existingSeason(seasonId)
    {
        require(
            !_seasonIdToMerkleRootStatus[seasonId],
            "A merkle root was already set."
        );

        _seasonIdToMerkleRootStatus[seasonId] = true;
        _seasonIdToMerkleRoot[seasonId] = merkleRoot;

        emit SeasonMerkleRootAdded(msg.sender, seasonId);
    }
    ...
    // /**
    //  * @dev Ovverrides the default function declaration from {ERC721}.
    //  * See {ERC721-_baseURI} for reference.
    //  */
