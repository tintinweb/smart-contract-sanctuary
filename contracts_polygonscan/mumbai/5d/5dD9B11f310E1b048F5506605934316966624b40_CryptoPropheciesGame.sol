...
contract CryptoPropheciesGame is ReentrancyGuard, Ownable {
    ...
    function createBattle(
        ...
        require(block.timestamp < _expireTime, "Invalid battle expire time");
        require(
    ...
    function _transferFundsToWinner(uint256 _battleId) internal {
        ...
        if (battle.winner == address(0)) {
            ...
            emit KingdomFeeDeducted(kingdomFeeTCP + kingdomFeebTCP);

            // refund weager amount to both players which deducted kingdom fee
            IERC20(TCP).safeTransfer(battle.player1, battle.TCPAmount[0] - kingdomFeeTCP / 2);
            IERC20(TCP).safeTransfer(
                battle.player2,
                battle.TCPAmount[1] - kingdomFeeTCP + kingdomFeeTCP / 2
            );

            IERC20(bTCP).safeTransfer(battle.player1, battle.bTCPAmount[0] - kingdomFeebTCP / 2);
            IERC20(bTCP).safeTransfer(
                battle.player2,
                battle.bTCPAmount[1] - kingdomFeebTCP + kingdomFeebTCP / 2
            );
