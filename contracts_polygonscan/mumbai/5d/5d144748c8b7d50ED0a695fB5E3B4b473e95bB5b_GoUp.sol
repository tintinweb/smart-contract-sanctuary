...
contract GoUp is ERC20, ERC20Burnable, Pausable, AccessControl {
    ...
    uint256 private _tTotal = 420000000 * 10**_decimals;        // total supply : 5000
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "go up";
    string private _symbol = "UP";
    
    address private buffContract;   // you need to set buff contract address after deploying buff contract
    
    uint256 public _taxFee = 2;                 // 1% to reflection
    uint256 private _previousTaxFee = _taxFee;
    
    uint256 public _liquidityFee = 2;          // 2% to liquidity
    uint256 private _previousLiquidityFee = _liquidityFee;

    uint256 public _extraFee = 1;            // team + donation (0.5 + 0.5 = 1)
    ...
    uint256 public _maxTxAmount = 100000000 * 10**_decimals;          // anti-whale : 0.05 per transaction
    uint256 private numTokensSellToAddToLiquidity = 3000 * 10**_decimals;
    ...
    constructor() ERC20("go up", "UP") {
        ...
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff);   // matic main net
        // IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0xD99D1c33F9fC3444f8101754aBC46c52416550D1);   // bsc test net
        ...
        _isExcludedFromFee[getOwner()] = true;
    ...
    function setBufferContract(address _buffer) external onlyRole(DEFAULT_ADMIN_ROLE){
        buffContract = _buffer;
    }
    ...
    function setExtraFeePercent(uint256 extraFee) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _extraFee = extraFee;
    }
    ...
    function setMaxTxPercent(uint256 maxTxPercent) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        ...
        uint256 tTransferAmount = tAmount - (tFee) - (tLiquidity) - (tExtra);
    ...
    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tExtra, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        ...
        uint256 rExtra = tExtra * (currentRate);
        uint256 rTransferAmount = rAmount - (rFee) - (rLiquidity) - (rExtra);
    ...
    function _takeExtra(uint256 tExtra) private {
        ...
        _rOwned[buffContract] = _rOwned[buffContract] + (rExtra);
        if(_isExcluded[buffContract])
            _tOwned[buffContract] = _tOwned[buffContract] + (tExtra);
    }
    
    function _burn(address sender, uint256 tBurn) internal override {
        ...
        if (_isExcluded[sender])
             _tOwned[sender] = _tOwned[sender] - (tBurn);
    ...
    function burn(address sender, uint256 tBurn) public {
        _burn(sender,tBurn);
    }
