...
contract MusicNFT is ERC1155, ERC165Storage, Ownable, IMusicNFT {
    ...
    event Mint(uint256 indexed tokenId, string dbId);
    ...
    /// @param dbId an id for syncing a traditional database
    function mint(
        address account,
        uint256 amount,
        string memory uri_,
        ...
        require(!dbIds[dbId], "nft:UUID already in use");
        dbIds[dbId] = true;
        _tokenId.increment();
    ...
    /// @param _dbIds an array of ids for syncing a traditional database
    function mintBatch(
        address to,
        uint256[] memory amounts,
        string[] memory _uris,
        string[] memory _dbIds
    ) public {
        require(whitelisted[msg.sender] || msg.sender == owner(), "not authorized to mint");
        require(amounts.length == _uris.length && _uris.length == _dbIds.length,
        ...
        for (uint256 i = 0; i < amounts.length; i++) {
            ...
            // should always pass, but just in case
            require(properties[tokenId].creator == address(0), "NFT has already been minted");
            require(!dbIds[_dbIds[i]], "nft:UUID already in use");
            ids[i] = tokenId;
        }
