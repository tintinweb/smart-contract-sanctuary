...
//TODO: I think the bid struct can be refactored out
//TODO: gas efficiency, there are like a gazillion writes
...
//TODO: see if it's worth dropping auctionId and just use the UUID
...
contract Auction is IAuction, ERC1155Receiver, Ownable {
  ...
  // currently for testing, should this be left in for prod?
  ...
  function createAuction(
    ...
    require(NftContract.balanceOf(msg.sender, tokenId) > 0, "does not own NFT");
    ...
    Status currentStatus = block.timestamp < startTime ? Status.PENDING : Status.ACTIVE;

    //TODO: try this with auctions[auctionId] = Auction{ auctionId: auctionId, etc }
    // compare and see if one is more gas effecient
    auctions[auctionId].id = auctionId;
    auctions[auctionId].owner = msg.sender;
    auctions[auctionId].nftContract = nftContract;
    ...
    auctions[auctionId].currency = currency;
    auctions[auctionId].status = currentStatus;
    auctions[auctionId].dbId = dbId;

    //TODO: put a reentrancy guard on it
  ...
  // currently tops off if has already bid
  function bid(uint256 auctionId, uint256 amount) external payable returns(bool){
    // needs reserve price check
    require(block.timestamp >= auctions[auctionId].startTime, "auction is not yet active");
    require(block.timestamp < auctions[auctionId].endTime, "auction has ended");
    require(auctions[auctionId].status != Status.CANCELLED, "auction is cancelled");
    require(amount > 0, "bid must be > 0");
    require(msg.sender != auctions[auctionId].owner, "cannot bid on own nft");
    if(auctions[auctionId].status == Status.PENDING) auctions[auctionId].status = Status.ACTIVE;

    if(auctions[auctionId].currency != ETH) {
      require(amount > bids[auctionId][highestBid[auctionId]].amount, "bid not high enough");
      IERC20 Token = IERC20(auctions[auctionId].currency);

      require(Token.transferFrom(msg.sender, address(this), amount),
        "nft auction:ERC20 transfer failed"
      );
    } else {
      require(msg.value == amount, "mismatch of value and args");
      require(msg.value > bids[auctionId][highestBid[auctionId]].amount, "insufficient ETH sent");
    }
    ...
    escrow[auctions[auctionId].currency] += amount - lastAmount;

    if(bids[auctionId][msg.sender].bidder == address(0)) {
      bids[auctionId][msg.sender].bidder = msg.sender;
    }

    bids[auctionId][msg.sender].amount += amount;
  ...
  function claimNft(uint256 auctionId, address recipient) external returns(bool) {
    require(msg.sender == highestBid[auctionId] ||
      msg.sender == auctions[auctionId].owner,
      "cannot claim nft"
    );
    require(auctions[auctionId].status == Status.ENDED ||
    ...
    uint256 totalFundsToPay = msg.sender == auctions[auctionId].owner ? 0 : 
      bids[auctionId][highestBid[auctionId]].amount;
    if(msg.sender == highestBid[auctionId]) {
      require(auctions[auctionId].status == Status.ENDED, 
        "cannot claim from auction");
      require(bids[auctionId][highestBid[auctionId]].amount >= 
    ...
    if(totalFundsToPay > 0){
      ...
      (address artistAddress, uint256 royalties) = Nft.royaltyInfo(auctions[auctionId].tokenId, totalFundsToPay);

      // system fee
      (address systemWallet, uint256 fee) = Registry.feeInfo(totalFundsToPay);
      totalFundsToPay -= fee;
      ...
      // seller gains
      claimableFunds[auctions[auctionId].owner][auctions[auctionId].currency] += totalFundsToPay;
    } else {
      escrow[auctions[auctionId].currency] -= bids[auctionId][highestBid[auctionId]].amount;
      //TODO: system fee
      claimableFunds[auctions[auctionId].owner][auctions[auctionId].currency] += bids[auctionId][highestBid[auctionId]].amount;
    }
    ...
    auctions[auctionId].status = Status.ENDED_AND_CLAIMED; 
  ...
  function claimFunds(address tokenContract) external {
    ...
    emit FundsClaimed(msg.sender, tokenContract, payout);
  }

  function updateAuctionStatus(uint256 auctionId) external {
    require(auctions[auctionId].status != Status.CANCELLED,
    ...
    if(
      ...
      require(auctions[auctionId].status == Status.PENDING,
        "auction already active or ended"
      );
      auctions[auctionId].status = Status.ACTIVE;
    }
    if(block.timestamp >= auctions[auctionId].endTime) {
      auctions[auctionId].status = Status.ENDED;
    }

    emit AuctionStatusChanged(auctionId, auctions[auctionId].status);
  }
...
interface IAuction {
  ...
  // mapping (auctionId => Auction) private auctions;

  event NewAuction(uint256 indexed auctionId, Auction newAuction);
  event AuctionStatusChanged(uint256 auctionId, Status status);
  ...
  event FundsClaimed(address claimant, address tokenContract, uint256 payout);

  // modifier: onlyAuctionOwnerOrAdmin
...
interface IRegistry {
  ...
  function getSystemWallet() external view returns(address);
  ...
  function changeContractStatus(address toChange, bool status) external;
}
