...
contract TomiPool is Ownable {
    ...
    event ClaimReward(address indexed user, address indexed pair, address indexed rewardToken, uint256 amountTOMI);
    event AddReward(address indexed pair, uint256 amount);

    mapping(address => mapping(address => UserInfo)) public users;

    mapping(address => uint256) public pairAmountPerShare;
    mapping(address => uint256) public pairReward;
    ...
    function addRewardFromPlatform(address _pair, uint256 _amount) external {
        require(msg.sender == PLATFORM, 'TOMI POOL: FORBIDDEN');
        uint256 balanceOf = IERC20(TOMI).balanceOf(address(this));
        require(balanceOf.sub(totalReward) >= _amount, 'TOMI POOL: ADD_REWARD_EXCEED');

        uint256 rewardAmount = ITomiConfig(CONFIG).getConfigValue(ConfigNames.FEE_LP_REWARD_PERCENT).mul(_amount).div(
            10000
        );
        _addReward(_pair, rewardAmount);

        uint256 remainAmount = _amount.sub(rewardAmount);
        uint256 fundingAmount = ITomiConfig(CONFIG)
            .getConfigValue(ConfigNames.FEE_FUNDME_REWARD_PERCENT)
            .mul(remainAmount)
            .div(10000);
        ...
        uint256 lotteryAmount = ITomiConfig(CONFIG)
            .getConfigValue(ConfigNames.FEE_LOTTERY_REWARD_PERCENT)
            .mul(remainAmount)
            .div(10000);
    ...
    function addReward(address _pair, uint256 _amount) external {
    ...
    function _addReward(address _pair, uint256 _amount) internal {
        pairReward[_pair] = pairReward[_pair].add(_amount);
        uint256 totalProdutivity = ITomiPair(_pair).totalSupply();
    ...
    function _auditUser(address _pair, address _user) internal {
        ...
        uint256 balance = ITomiPair(_pair).balanceOf(_user);
        uint256 accAmountPerShare = pairAmountPerShare[_pair];
        UserInfo storage userInfo = users[_user][_pair];
        uint256 pending = balance.mul(accAmountPerShare).div(1e12).sub(userInfo.rewardDebt);
    ...
    function queryReward(address _pair, address _user) external view returns (uint256) {
        require(ITomiFactory(FACTORY).isPair(_pair), 'TOMI POOL: INVALID PAIR');
...
interface ITomiPair {
    ...
    function mintReward() external returns (uint256 rewardAmount);

    function getTGASReserve() external view returns (uint256);
}
...
interface ITomiFactory {
    ...
    function playerPairs(address player, uint256 index) external view returns (address pair);
...
interface ITomiGovernance {
    ...
    function onBehalfDeposit(address _user, uint256 _amount) external returns (bool);
}
