...
contract TamiGAWDtchi is TrustedForwarderRecipient {
    ...
    function worship(uint16 gawd) public isTrueGawd(gawd) {
        require(getAlive(gawd), "no longer with us");
        require(getSacrifice(gawd) < 80, "im too tired to eat");
        require(getMarvel(gawd) < 80, "im feeling too gross to eat");
    ...
    function marvel(uint16 gawd) public isTrueGawd(gawd) {
        require(getAlive(gawd), "no longer with us");
        require(getMarvel(gawd) > 0, "i dont need a bath");
    ...
    function sacrifice(uint16 gawd) public isTrueGawd(gawd) {
        require(getAlive(gawd), "no longer with us");
        require(getWorship(gawd) < 80, "im too hungry to play");
        require(getCower(gawd) < 80, "im too sleepy to play");
        require(getMarvel(gawd) < 80, "im feeling too gross to play");
    ...
    function cower(uint16 gawd) public isTrueGawd(gawd) {
        require(getAlive(gawd), "no longer with us");
        require(getMarvel(gawd) < 80, "im feeling too gross to sleep");
        require(getCower(gawd) > 0, "im not feeling sleepy");
    ...
    function resurrect(uint16 gawd) public isTrueGawd(gawd) {
        require(getAlive(gawd) == false, "already alive");
        require(
    ...
    function getEarliestBlockForResurrection(uint16 gawd)
        ...
        returns (uint256)
    {
        require(getAlive(gawd) == false, "still alive");
        return
            1 +
