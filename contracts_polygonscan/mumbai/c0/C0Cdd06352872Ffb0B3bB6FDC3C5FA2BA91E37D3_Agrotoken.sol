...
contract Agrotoken is IERC20 {
    ...
    event FeeHasChanged(uint8 _feeType, uint256 _newAmount);
     ...
     * @dev To lock you must inform a _lockedUntilBlock higher than actual block.number. To unlock you need to inform a block number lower than actual block.number.
    ...
    * @param _newAmount new fee value (consider feeDecimals: if feeDecimals equals 4, then a 2% fee should be passed as 20000)
    * @return processing status
    */
    function changeFee(uint8 _feeType, uint256 _newAmount) external boardOrAdmin returns (bool) {
        require(_newAmount<=MAX_AGROTOKEN_FEE, "Invalid or exceed white paper definition");
        ...
        emit FeeHasChanged(_feeType, _newAmount);
        return true;
    }
    ...
    * @param _fee value of the fee (consider feeDecimals: if feeDecimals equals 4, then a 2% fee should be passed as 20000)
    ...
    */
    function calcFees(uint256 _fee, uint256 _amount) public pure returns(uint256 serviceAmount, uint256 netAmount ) {
        serviceAmount = (_amount * _fee) / (10**feeDecimals * 100);
