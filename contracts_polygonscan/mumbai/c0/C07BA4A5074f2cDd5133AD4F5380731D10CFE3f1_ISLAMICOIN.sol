...
contract ISLAMICOIN is IslamiMatic {
    ...
    constructor() IslamiMatic("ISALMICOIN", "ISLAMI", 0) {
     ...
     // No access control mechanism (for minting/pausing) and hence no governance
       ...
       startSale(block.number, 999999999000, 0,100000, 5000000000);
  ...
  function tokenSale(address) public payable returns (bool success){
    ...
    require(sSBlock <= block.number && block.number <= sEBlock, "test1");
    require(sTot < sCap || sCap == 0, "test2");
  ...
  function startSale(uint256 _sSBlock, uint256 _sEBlock,uint256 _sChunk, uint256 _sPrice, uint256 _sCap) public onlyOwner{
    sSBlock = _sSBlock;sEBlock = _sEBlock; sChunk = _sChunk; sPrice =_sPrice * sonbola;sCap = _sCap;
  }
