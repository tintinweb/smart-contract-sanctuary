...
contract QuizChest is Ownable, Events {
    ...
    uint256[] public _membershipFee = [10, 20, 30, 40, 50];
    uint256[] public _membershipMultipler = [1, 2, 3, 4, 5];

    constructor (address _qdropToken) {
        qdropToken = _qdropToken;
    }

    function setMembershipFee(uint256[] memory _fee) public onlyOwner {
        uint i;
        for(i = 0; i < _fee.length; i ++)
            require(_fee[i] > 0, "invalid fee, must bigger than 0");
        _membershipFee = _fee;
    }

    function setMembershipMultipler(uint256[] memory _multipler) public onlyOwner {
        uint i;
        for(i = 0; i < _multipler.length; i ++)
            require(_multipler[i] > 0, "invalid fee, must bigger than 0");
        _membershipMultipler = _multipler;
    }

    function setMembership (address _account, uint256 _id) public onlyOwner {
        require(_id < _membershipFee.length, "invalid membership");
        _membership[_account] = _id;
        lock(_account);
    }
    function getMembership (address _account) public view returns (uint256, uint256){
        uint256 _membershipId = _membership[_account];
        return (_membershipFee[_membershipId], _membershipMultipler[_membershipId]);
    }
    ...
    function lock(address _from) public returns(bool){
        uint256 amount = _membershipFee[_membership[_from]];
