...
contract TestNFT is IERC721, IERC165, IERC721Metadata, IERC721Enumerable {
    ...
    uint256 public constant MIN_MINTING_LIMIT = 1;
    uint256 public constant MAX_MINTING_LIMIT = 20;
    ...
    function mint(uint256 amount) external payable reentrancyGuard mintingEnabled returns (uint256) {
        require(msg.value >= getNFTPrice(amount), Errors.NOT_ENOUGH_ETC);
        
        
        require(amount >= MIN_MINTING_LIMIT, Errors.MINTING_MIN_MINT_AMOUNT);
        require(amount <= MAX_MINTING_LIMIT, Errors.MINTING_MAX_MINT_AMOUNT);
