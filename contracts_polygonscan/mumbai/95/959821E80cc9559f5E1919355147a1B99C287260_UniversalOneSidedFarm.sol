...
contract UniversalOneSidedFarm is ReentrancyGuard {
    ...
    struct IntermediateSwapVars {
        address intermediatePair;
        uint256 intermediateRes0;
        uint256 intermediateRes1;
        address intermediateToken0;
        address intermediateToken1;
    }

    IntermediateSwapVars private intermediateSwapVars;
    ...
    function poolLiquidity(
        ...
        if (_fromToken != token0 && _fromToken != token1) {
            intermediateSwapVars.intermediatePair = _fromToken == address(0)
                ? factory.getPair(wmatic, _toToken)
                : factory.getPair(_fromToken, _toToken);

            (
                intermediateSwapVars.intermediateRes0,
                intermediateSwapVars.intermediateRes1,

            ) = IPolydexPair(intermediateSwapVars.intermediatePair)
                .getReserves();

            (
                intermediateSwapVars.intermediateToken0,
                intermediateSwapVars.intermediateToken1
            ) = _getPairTokens(intermediateSwapVars.intermediatePair);
            ...
            if (_fromToken == address(0)) {
                ...
                if (slippageAdjustedMinLP > 0) {
                    _amountOut = router.getAmountOut(
                        msg.value,
                        reserveA,
                        reserveB
                    );
                    if (
                        !_isWithinDesiredSlippage(
                            _amountOut,
                            _toToken,
                            _pairAddress
                        )
        ...
        uint256 lpBought = _poolLiquidity(_fromToken, _pairAddress, toInvest);
    ...
    function _isWithinDesiredSlippage(
        uint256 _amountIn,
        address _fromToken,
        address _pairAddress
    ) internal view returns (bool) {
        uint256 estimatedLps = getEstimatedLPTokens(
            _amountIn,
            _fromToken,
            _pairAddress
        );
        ...
        )
    {
        ...
        IERC20(_toToken0).safeApprove(address(router), token0Bought);
        IERC20(_toToken1).safeApprove(address(router), token1Bought);
    ...
    function _swapTokens(
        ...
        IPolydexPair pair = IPolydexPair(factory.getPair(_toToken0, _toToken1));
    ...
    {
        return
            sqrt(reserveIn.mul(userIn.mul(3988000).add(reserveIn.mul(3988009))))
    ...
    */
    function _swapTokensInternal(
        ...
        IERC20(_fromToken).safeApprove(address(router), tokens2Trade);
    ...
    function getEstimatedLPTokens(
        ...
        (uint256 res0, uint256 res1, ) = IPolydexPair(_pairAddress)
            .getReserves();
        (address _token0, address _token1) = _getPairTokens(_pairAddress);
