...
contract FomoMarketplace is Ownable, ReentrancyGuard {
    ...
    function listItem(
        ...
        if (_supportERC721(_nftAddress)) {
            ...
            require(_quantity == 1, "Quantity value must be one.");
        ...
        if (listingFeeToken == address(0)) {
            require(msg.value >= listingFee, "Invalid listing fee");
            (bool feeTransferSuccess, ) = feeReceipient.call{value: listingFee}(
                ""
            );
            require(
                feeTransferSuccess,
                "FomoMarketplace: listing fee transfer failed"
            );
        } else {
