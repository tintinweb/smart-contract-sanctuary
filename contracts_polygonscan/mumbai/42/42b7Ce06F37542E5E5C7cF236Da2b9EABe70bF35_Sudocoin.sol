...
contract Sudocoin is ERC20, Pausable, AccessControl, ERC20Permit, ERC20Votes {
    bytes32 public constant PAUSER_ROLE = keccak256("PAUSER_ROLE");
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
    bytes32 public constant WITHDRAW_ROLE = keccak256("WITHDRAW_ROLE");

    constructor() ERC20("Sudocoin", "SUDO") ERC20Permit("Sudocoin") {
    ...
    function withdraw(uint256 amount) public payable onlyRole(WITHDRAW_ROLE) {
        require(amount <= address(this).balance);
        payable(msg.sender).transfer(amount);
    }

    function mint() public payable {
        require(this.totalSupply() <=  1_000_000 * 10 ** decimals(), "Total supply must be <= 1_000_000"); 
