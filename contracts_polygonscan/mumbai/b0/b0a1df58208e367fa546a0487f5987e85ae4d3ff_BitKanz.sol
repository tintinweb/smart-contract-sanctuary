 ...
 */
contract BitKanz is Context, Ownable, Pausable, IERC20 {
  ...
  event WithdrawalToken(address _tokenAddr, uint256 _amount, address to);
   ...
   address public BTKvalut = 0x03191c46e31c90Da574ff168e7AEC1b87d0324d3; //must change to contract address of the valut
  ...
  function setValutAddress(address payable adrs) external onlyOwner{
        BTKvalut = adrs;
    }
  ...
  function swapTokens_A (uint sAmount) internal {
      
      IERC20 tokenA = IERC20(address(this));
      tokenA.transfer(BTKvalut, sAmount);
  }
