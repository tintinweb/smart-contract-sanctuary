...
interface IFractionableERC721 {
    ...
    function renounceTokenManager(address _manager) external;
...
contract PerformanceCard is ICard, GasPriceLimited {
    ...
    function upgrade(address _newCardContract) external onlyOwner {    
        require(
            Address.isContract(_newCardContract), 
            "PerformanceCard: _newCardContract invalid"
        );
    
        uint256 reserveAmount = reserveToken.balanceOf(address(this));

        // transfer to new reserve contract
        reserveToken.transfer(_newCardContract, reserveAmount);

        // cease admin to new contract
        nftRegistry.renounceTokenManager(_newCardContract);

        // destroy and send possible remaining funds
        selfdestruct(
            payable(owner())
        );
    }
