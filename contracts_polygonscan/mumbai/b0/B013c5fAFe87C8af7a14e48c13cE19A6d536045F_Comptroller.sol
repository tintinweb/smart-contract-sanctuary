  ...
  Exponential
{
  ...
  function exitMarket(address cTokenAddress) external returns (uint256) {
    ...
    if (allowed != 0) {
      return
        failOpaque(Error.REJECTION, FailureInfo.EXIT_MARKET_REJECTION, allowed);
    }
  ...
  function _setCollateralFactor(
    ...
    if (msg.sender != admin) {
      return
        fail(Error.UNAUTHORIZED, FailureInfo.SET_COLLATERAL_FACTOR_OWNER_CHECK);
    }
    ...
    if (
      newCollateralFactorMantissa != 0 && oracle.getUnderlyingPrice(cToken) == 0
    ) {
      return
        fail(
          Error.PRICE_ERROR,
  ...
  function _supportMarket(CToken cToken) external returns (uint256) {
    ...
    if (markets[address(cToken)].isListed) {
      return
        fail(Error.MARKET_ALREADY_LISTED, FailureInfo.SUPPORT_MARKET_EXISTS);
    }
    ...
    returns (uint256)
  {
    if (msg.sender != admin) {
      return
        fail(Error.UNAUTHORIZED, FailureInfo.SET_PAUSE_GUARDIAN_OWNER_CHECK);
    }
   ...
   */
  function claimEtha(address holder) public {
    return claimEtha(holder, allMarkets);
  }
   ...
   */
  function claimEtha(address holder, CToken[] memory cTokens) public {
    address[] memory holders = new address[](1);
    holders[0] = holder;
    claimEtha(holders, cTokens, true, true);
  }
  ...
  function _addCompMarketInternal(address cToken) internal {
    ...
    if (
      compSupplyState[cToken].index == 0 && compSupplyState[cToken].block == 0
    ) {
      compSupplyState[cToken] = CompMarketState({
    ...
    if (
      compBorrowState[cToken].index == 0 && compBorrowState[cToken].block == 0
    ) {
      compBorrowState[cToken] = CompMarketState({
        index: compInitialIndex,
        block: safe32(getBlockNumber(), "block number exceeds 32 bits")
  ...
  function setEthaAddress(address etha_) public returns (address) {
    require(etha == address(0), "Etha address already set");
    require(msg.sender == admin, "only admin can add etha address");
    etha = etha_;
  }
...
contract Comp {
  /// @notice EIP-20 token name for this token
  string public constant name = "ETHA";

  /// @notice EIP-20 token symbol for this token
  string public constant symbol = "ETHA";
  ...
  //Matic Child chain manager gateway
  ...
  /**
   * @notice Construct a new Etha token
   * @param gateway_ Matic child chain Manager gateway
   */
  ...
  function deposit(address user, bytes calldata depositData) external {
    ...
    uint96 amount = safe96(rawAmount, "deposit: amount exceeds 96 bits");
  ...
  function withdraw(uint256 rawAmount) external {
    uint96 amount = safe96(rawAmount, ":withdraw: amount exceeds 96 bits");
  ...
  function _mint(address account, uint96 amount) internal {
    ...
    totalSupply = add96(totalSupply, amount, "_mint: mint amount overflows");
    balances[account] = add96(
      balances[account],
      amount,
      "_mint: mint amount overflows"
    );
