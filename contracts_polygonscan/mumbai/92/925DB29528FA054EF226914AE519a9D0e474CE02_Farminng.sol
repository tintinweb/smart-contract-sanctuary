...
// will be transferred to a governance smart contract once Binamars is sufficiently
// distributed and the community can show to govern itself.
//
// Have fun reading it. Hopefully it's bug-free. God bless.
contract Farminng is Ownable {
    ...
    //     //
    //     // We do some fancy math here. Basically, any point in time, the amount of Binamarss
    //     // entitled to a user but is pending to be distributed is:
    //     //
    //     //   pending reward = (user.amount * pool.accBinaTokenPerShare) - user.rewardDebt
    //     //
    //     // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
    //     //   1. The pool's `accBinaTokenPerShare` (and `lastRewardBlock`) gets updated.
    ...
    // Info of each pool.
    struct PoolInfo {
        Binamars.Level level; // Address of LP token contract.
        uint256 allocPoint; // How many allocation points assigned to this pool. Binamarss to distribute per block.
        uint256 lastRewardBlock; // Last block number that Binamarss distribution occurs.
        uint256 accBinaTokenPerShare; // Accumulated Binamarss per share, times 1e12. See below.
    }
    // The Binamars TOKEN!
    ...
    uint256 public feeRewardRate = 5; // 5%
    // Amount block number halving.
    uint256 public halvingBlockAmount;
    // Binamars tokens created per block.
    uint256 public binaTokenPerBlock;
    // Bonus muliplier for early binaToken makers.
    uint256 public constant HAVING_MULTIPLIER = 9; // -10%
    uint256 public constant REWARD_MULTIPLIER = 10**6;
    // Info of each pool.
    PoolInfo[] public poolInfo;
    // mapping(uint256 => mapping(address => UserInfo)) public userInfo;
    mapping(Binamars.Level => mapping(address => uint256))
        public userRewardDebt;
    mapping(Binamars.Level => mapping(address => EnumerableSet.UintSet))
        internal userNFT;
    mapping(Binamars.Level => EnumerableSet.UintSet) private farmingNFT;
    // Total allocation poitns. Must be the sum of all allocation points in all pools.
    uint256 public totalAllocPoint = 0;
    // The block number when Binamars mining starts.
    ...
    constructor(
        ...
        //         EPIC,
        //         LEGENDARY
        add(0, Binamars.Level.__EGG, true);
        add(10, Binamars.Level.COMMON, true);
        add(15, Binamars.Level.NORMAL_RARE, true);
        add(20, Binamars.Level.RARE, true);
        add(30, Binamars.Level.EPIC, true);
        add(150, Binamars.Level.LEGENDARY, true);
    }

    function configFarming(
        uint256 _binaTokenPerBlock,
        uint256 _startBlock,
        ...
        binaTokenPerBlock = _binaTokenPerBlock.div(REWARD_MULTIPLIER);
        halvingBlockAmount = _halvingBlockAmount;
        startBlock = _startBlock;
    }

    function poolSize(Binamars.Level _level) public view returns (uint256) {
        return farmingNFT[_level].length();
    }

    function add(
        uint256 _allocPoint,
        Binamars.Level _level,
        bool _withUpdate
    ) internal {
        if (_withUpdate) {
            massUpdatePools();
        ...
        poolInfo.push(
            PoolInfo({
                ...
                accBinaTokenPerShare: 0
            })
        );
    }

    // Update the given pool's Binamars allocation point. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        bool _withUpdate
    ...
    // View function to see pending Binamarss on frontend.
    function pendingBinaToken(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        uint256 accBinaTokenPerShare = pool.accBinaTokenPerShare;
        console.log(
            "Pool info: per share %s, allocPoint: %s",
            accBinaTokenPerShare,
            pool.allocPoint
        );

        uint256 binaSupply = farmingNFT[pool.level].length();

        console.log("Supply: %s", binaSupply);
        if (block.number > pool.lastRewardBlock && binaSupply != 0) {
            uint256 multiplier = getMultiplier(
                pool.lastRewardBlock,
                block.number
            );
            console.log("multiplier: %s", multiplier);
            console.log("binaTokenPerBlock: %s", binaTokenPerBlock);
            uint256 binaTokenReward = multiplier
            .mul(binaTokenPerBlock)
            .mul(pool.allocPoint)
            .div(totalAllocPoint);
            console.log("binaTokenReward: %s", binaTokenReward);
            accBinaTokenPerShare = accBinaTokenPerShare.add(
                binaTokenReward.mul(1e12).div(binaSupply)
            );
        }
        uint256 pendingBinaToken = userNFT[pool.level][_user]
        .length()
        .mul(accBinaTokenPerShare)
        .div(1e12)
        .sub(userRewardDebt[pool.level][_user]);
        console.log("user %s, pendingBinaToken: %s", _user, pendingBinaToken);
        return pendingBinaToken;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 binaSupply = farmingNFT[Binamars.Level(_pid)].length();
        if (binaSupply == 0) {
        ...
        binaToken.farm(devaddr, binaTokenReward.div(10));
        binaToken.farm(address(this), binaTokenReward);
        pool.accBinaTokenPerShare = pool.accBinaTokenPerShare.add(
            binaTokenReward.mul(1e12).div(binaSupply)
        );
        pool.lastRewardBlock = block.number;
    }

    // Deposit NFT tokens to Farming for Binamars allocation.
    function deposit(uint256 _tokenId) public {
        Binamars.Bina memory bina = binaNFT.getBina(_tokenId);
        Binamars.Level level = bina.level;
        uint256 _pid = uint256(level);
        console.log("Sender: %s deposit %s", msg.sender, _pid);
        PoolInfo storage pool = poolInfo[_pid];
        updatePool(_pid);
        if (userNFT[pool.level][msg.sender].length() > 0) {
            uint256 pending = userNFT[pool.level][msg.sender]
            .length()
            .mul(pool.accBinaTokenPerShare)
            .div(1e12)
            .sub(userRewardDebt[level][msg.sender]);
            safeBinaTokenTransfer(msg.sender, pending);
        }
        binaNFT.transferFrom(address(msg.sender), address(this), _tokenId);
    ...
    function withdraw(uint256 _tokenId) public {
        ...
        .mul(pool.accBinaTokenPerShare)
        .div(1e12);
        binaNFT.transferFrom(address(this), address(msg.sender), _tokenId);
    ...
    function emergencyWithdraw(uint256 _pid) public {
        ...
        for (uint256 index = 0; index < userFarms; index++) {
            ...
            binaNFT.transferFrom(address(this), address(msg.sender), tokenId);
    ...
    // Safe binaToken transfer function, just in case if rounding error causes pool to not have enough Binamarss.
    function safeBinaTokenTransfer(address _to, uint256 _amount) internal {
        uint256 binaTokenBal = binaToken.balanceOf(address(this));
        uint256 amount;
        if (_amount > binaTokenBal) {
            amount = binaTokenBal;
        } else {
            amount = _amount;
        }
        uint256 feeReward = amount.mul(feeRewardRate).div(100);
        if (feeReward > 0) {
            binaToken.transfer(_to, feeReward);
        }
        binaToken.transfer(_to, amount.sub(feeReward));
    }
    ...
    function setFeeReward(uint256 _feeRewardRate) public {
        require(msg.sender == devaddr, "dev: wut?");
        feeRewardRate = _feeRewardRate;
    }
