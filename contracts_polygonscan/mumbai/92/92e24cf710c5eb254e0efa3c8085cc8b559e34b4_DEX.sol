...
contract DEX is Ownable {
  ...
  function init() public {
    XNFT = INFT(0x38733c96739d67d3eb5A9E115b2Ee30CDd670918); 
  ...
  _supportNft[0x38733c96739d67d3eb5A9E115b2Ee30CDd670918] = true;
  ...
  // function setCollectionsConfig(address xCollection_, address factory_) public {
  //   require(!collectionConfig,"collections already configured");
  //   XNFT = INFT(xCollection_);
  //   factory = IFactory(factory_);
  //   // listing in supported collections list
  //   _supportNft[xCollection_] = true;

  //   collectionConfig = true;

  //   emit CollectionsConfigured(xCollection_, factory_);
  // }
...
//     if(_royality[_tokenId] > 0 && _contract == address(XNFT)) { // currently only supporting royality for non-user defined collection
...
//  // added nftContract param in function to support generalization
//   function getSellDetail(address nftContract, uint256 tokenId) public view returns (address, uint256, uint256, address, uint256, uint256, uint256) {
//       if(_saleTokens[tokenId][nftContract].seller != address(0)){
//         uint256 salePrice = _saleTokens[tokenId][nftContract].price;
//         return (_saleTokens[tokenId][nftContract].seller, salePrice > 0 ? salePrice : _dollarPrice[tokenId][nftContract], _saleTokens[tokenId][nftContract].timestamp, address(0), 0, 0,salePrice == 0 ? 1 : 0);
//       }else{
//           return (_auctionTokens[tokenId][nftContract].seller, _auctionTokens[tokenId][nftContract].bidAmount, _auctionTokens[tokenId][nftContract].endTime, _auctionTokens[tokenId][nftContract].bidder, _auctionTokens[tokenId][nftContract].minPrice,  _auctionTokens[tokenId][nftContract].startTime, 0);
//       }
//   }
//  // added nftContract param in function to support generalization
