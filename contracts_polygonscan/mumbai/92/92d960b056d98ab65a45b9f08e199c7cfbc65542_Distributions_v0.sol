...
contract BasicMetaTransaction {
    ...
    // Mapping of {userAddress, batchId} => nonce
    ...
    function executeMetaTransaction(address userAddress,
        bytes memory functionSignature, string memory message, string memory length, uint256 batchId,
        bytes32 sigR, bytes32 sigS, uint8 sigV) public payable returns(bytes memory) {

        require(verify(userAddress, message, length, nonces[userAddress][batchId], getChainID(), batchId, sigR, sigS, sigV), "Signer and signature do not match");
        // Append userAddress and relayer address at the end to extract it from calling context
        (bool success, bytes memory returnData) = address(this).call(abi.encodePacked(functionSignature, userAddress));

        require(success, "Function call not successfull");
        nonces[userAddress][batchId] = nonces[userAddress][batchId].add(1);
        emit MetaTransactionExecuted(userAddress, msg.sender, functionSignature);
        return returnData;
    }

    function getNonce(address user, uint256 batchId) public view returns(uint256 nonce) {
        nonce = nonces[user][batchId];
    }



    function verify(address owner, string memory message, string memory length, uint256 nonce, uint256 chainID,
        uint256 batchId, bytes32 sigR, bytes32 sigS, uint8 sigV) public pure returns (bool) {

        string memory nonceStr = uint2str(nonce);
        string memory chainIDStr = uint2str(chainID);
        string memory batchIdStr = uint2str(batchId);
        bytes32 hash = keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n", length, message, nonceStr, chainIDStr, batchIdStr));
...
contract Distributions_v0 is Initializable, Ownable, BasicMetaTransaction {
