...
// File contracts/OnlyNFTChild.sol
  ...
  ERC721PausableUpgradeable
{
  ...
  function initialize(
    ...
    address childChainManagerProxyAddress
  ) public initializer {
    ERC721Upgradeable.__ERC721_init(nftName, nftSymbol);
    _baseTokenURI = baseTokenURI;
    _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    _setupRole(DEPOSITOR_ROLE, childChainManagerProxyAddress);
   ...
   */
  // ChildChainManagerProxy testnet = 0xb5505a6d998549090530911180f38aC5130101c6
  function deposit(address user, bytes calldata depositData) external {
    require(
      hasRole(DEPOSITOR_ROLE, _msgSender()),
      "OnlyNFTChild: must have DEPOSITOR_ROLE to deposit"
    );
    // deposit single
  ...
  function withdraw(uint256 tokenId) external {
    require(
      ...
      "OnlyNFTChild: INVALID_TOKEN_OWNER"
    );
  ...
  function withdrawBatch(uint256[] calldata tokenIds) external {
    uint256 length = tokenIds.length;
    require(length <= BATCH_LIMIT, "OnlyNFTChild: EXCEEDS_BATCH_LIMIT");
    ...
    for (uint256 i; i < length; i++) {
      ...
      require(
        _msgSender() == ownerOf(tokenId),
        string(abi.encodePacked("OnlyNFTChild: INVALID_TOKEN_OWNER ", tokenId))
      );
      withdrawnTokens[tokenId] = true;
      _burn(tokenId);
    }
  ...
  /**
   * This is the "normal" mint function, it will only be called by
   * the NFT token factory smart contract. It can only create token IDs
   * that do not already exist in the root chain
   */
  function mint(address to, uint256 tokenId) public virtual {
    require(
      hasRole(MINTER_ROLE, _msgSender()),
      "OnlyNFTChild: must have minter role to mint"
    );
    require(
      !withdrawnTokens[tokenId],
      "OnlyNFTChild: TOKEN_EXISTS_ON_ROOT_CHAIN"
    );
    _mint(to, tokenId);
  }
  ...
  function pause() public virtual {
    require(
      ...
      "OnlyNFTChild: must have pauser role to pause"
    );
    _pause();
  }
  ...
  function unpause() public virtual {
    require(
      hasRole(PAUSER_ROLE, _msgSender()),
      "OnlyNFTChild: must have pauser role to unpause"
    );
    _unpause();
  }
  ...
  function setBaseURI(string calldata baseURI) external virtual {
    require(
      ...
      "OnlyNFTChild: must have admin role to set base URI"
    );
