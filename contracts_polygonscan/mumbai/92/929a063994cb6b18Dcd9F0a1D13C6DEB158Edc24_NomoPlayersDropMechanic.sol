    ...
    RandomNumberConsumer
{
    ...
    event LogPurchaseParameters(
        bool _period,
        address _sender,
        uint256 _quantity
    );
    event LogRandomNumberTEST(uint256 _randomNumber);
     ...
     */
    function buyTokens(address from, uint256 quantity) private nonReentrant {
        require(
            (quantity > 0) && (quantity <= maxQuantity),
            "Invalid quantity"
        );
        require(quantity < tokens.length , "Insufficient available quantity");
        require(
            addressToRandomNumber[from] != 0,
        ...
        for (uint256 i = 0; i < quantity; i++) {
            ...
            IERC721(erc721Address).safeTransferFrom(tokensVault, from, tokenId);
        }

        addressToRandomNumber[from] = 0;

        //! Intenally commented out for test purposes in etherscan

        // IERC20(erc20Address).transferFrom(from, daoWalletAddress, fraction);
     ...
     */
    function buyTokensOnPresale(address from) public {
        require(
        ...
        require(whitelisted[from], "Claiming is forbidden");

        buyTokens(from, 1);
        whitelisted[from] = false;
    }

    /**
     * @notice Invokes `buyTokens` with the quantity requested.
     */
    function buyTokensOnSale(address from, uint256 quantity) public {
        require(
            block.timestamp > (presaleStartDate + presaleDuration),
            "Sale period not started!"
        );

        buyTokens(from, quantity);
    }

    /**
     * @notice Requests random number from Chainlink VRF.
     */
    function activatePurchase(uint256 quantity) public {
        require(
            block.timestamp > presaleStartDate,
        ...
        console.log("afasfsfa");
        Purchase memory purchase = Purchase(isPresale, quantity);
        purchases[msg.sender] = purchase;

        lastRequestId = getRandomNumber();
        console.log("afasfsfa");
        emit LogPurchaseParameters(isPresale, msg.sender, quantity);
    }
    ...
    function saveRandomNumber(address from, uint256 n) internal override {
        ...
        if (purchases[from].presale) {
            console.log("1");
            buyTokensOnPresale(from);
        } else {
            console.log("2");
            buyTokensOnSale(from, purchases[from].quantity);
        }

        emit LogRandomNumberTEST(n);
    }
