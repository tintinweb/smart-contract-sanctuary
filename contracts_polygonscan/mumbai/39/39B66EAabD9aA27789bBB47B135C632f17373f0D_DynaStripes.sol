...
contract DynaStripes is DynaTokenBase, ERC2981PerTokenRoyalties {
    ...
    function mintStripes(uint24 _randomSeed, 
                         ...
                         uint8 _tintBlue,
                         uint8 _tintAlpha,
                         uint16 _rotationDegrees, 
                         uint8 _rotationRange, 
        ...
        require(_rotationDegrees <= 360, "Rotation degrees invalid");
        require(_rotationRange <= 180, "Rotation range invalid");
        ...
        dynaParamsMapping[tokenId] = DynaModel.DynaParams(_randomSeed, _zoom, _tintRed, _tintGreen, _tintBlue, _tintAlpha, _rotationDegrees, _rotationRange, _stripeWidthMin, _stripeWidthMax, _speedMin, _speedMax);
    ...
    function tokenURI(uint256 _tokenId) public view override(ERC721) returns (string memory) {
        ...
        return string(abi.encodePacked("data:text/plain,{\"name\":\"DynaStripes #", StringUtils.uintToString(_tokenId), "\", \"description\":\"on-chain, generative DynaStripes artwork\", ", copyright, ", ", traits, ", \"image\":\"data:image/svg+xml,", svg, "\"}")); 
    }
    ...
    function getViewBoxClipRect(uint _zoom) private pure returns (string memory, string memory) {
        _zoom = _zoom * 20;
        string memory widthHeight = StringUtils.uintToString(1000 + _zoom);

        if (_zoom > 1000) {
            string memory offset = StringUtils.uintToString((_zoom - 1000) / 2);
            ...
            string memory offset = StringUtils.uintToString((_zoom == 1000 ? 0 : (1000 - _zoom) / 2));
            string memory viewBox = string(abi.encodePacked(offset, " ", offset, " ",  widthHeight, " ", widthHeight));
            string memory clipRect = string(abi.encodePacked("x='", offset, "' y='", offset, "' width='",  widthHeight, "' height='", widthHeight, "'"));
    ...
    function getRects(DynaModel.DynaParams memory _dynaParams) private pure returns (string memory) {
        ...
        while ((2000 - xPos) > 0) {
            ...
            rects = string(abi.encodePacked(rects, "<rect x='", StringUtils.uintToString(xPos), "' y='0' width='", StringUtils.uintToString(stripeWidth), "' height='2000' fill='", firstColour, "' opacity='0.8'", " transform='rotate(",  getRotation(randomSeed + 1, _dynaParams), " 1000 1000)'><animate begin= '0s' dur='", getSpeed(randomSeed + 2, _dynaParams), "ms' attributeName='fill' values='", colours, ";' fill='freeze' repeatCount='indefinite'/></rect>"));
    ...
    function getRotation(uint _randomSeed, DynaModel.DynaParams memory _dynaParams) private pure returns (string memory) {
        uint randomDegrees = randomIntFromInterval(_randomSeed, 0, _dynaParams.rotationRange);
        uint rotation;

        if (randomDegrees == 0) {
            rotation = _dynaParams.rotationDegrees;
        } else if (randomDegrees < _dynaParams.rotationRange) {
            rotation = 360 + _dynaParams.rotationDegrees - randomDegrees + _dynaParams.rotationRange / 2; 
        } else {
            rotation = _dynaParams.rotationDegrees + randomDegrees - _dynaParams.rotationRange / 2;
        }
        if (rotation > 360) {
...
library DynaTraits {
    ...
    function getTraits(DynaModel.DynaParams memory dynaParams) internal pure returns (string memory) {
        ...
        string memory rotationTraits = string(abi.encodePacked("{\"trait_type\": \"rotation degrees\", \"value\": \"", StringUtils.uintToString(dynaParams.rotationDegrees), "\"}, {\"trait_type\": \"rotation range\", \"value\": \"", StringUtils.smallUintToString(dynaParams.rotationRange), "\"}")); 
    ...
    function getTextTraits(DynaModel.DynaParams memory dynaParams) internal pure returns (string memory) {
        return getTextTraitsExpanded(dynaParams.zoom, dynaParams.tintRed, dynaParams.tintGreen, dynaParams.tintBlue, dynaParams.tintAlpha, dynaParams.rotationDegrees, dynaParams.rotationRange, dynaParams.stripeWidthMin, dynaParams.stripeWidthMax, dynaParams.speedMin, dynaParams.speedMax);
    }

    function getTextTraitsExpanded(
                    uint8 _zoom,
                    uint8 _tintRed,
                    uint8 _tintGreen,
                    uint8 _tintBlue,
                    ...
                    uint8 _stripeWidthMax, 
                    uint8 _speedMin, 
                    uint8 _speedMax) public pure returns (string memory) { 
