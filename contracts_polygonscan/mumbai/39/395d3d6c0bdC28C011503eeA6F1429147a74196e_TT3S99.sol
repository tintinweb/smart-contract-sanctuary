...
contract TT3S99 is ERC721URIStorage, Ownable, IERC2981Royalties {
    ...
    function getGameHighestScoreAndTokens()
        public
        view
        returns (uint256, string memory)
    {
        uint256 highScore = 0;
        string memory lastWinnerFound = "";

        for (uint256 i = 0; i < _tokenCounter; i++) {
            if (_nextScoreIds[_mintedIds[i]] > highScore) {
                lastWinnerFound = uint2str(i);
                highScore = _nextScoreIds[_mintedIds[i]];
            }
            if (_nextScoreIds[_mintedIds[i]] == highScore) {
                lastWinnerFound = string(
                    abi.encodePacked(lastWinnerFound, ", ", uint2str(i))
                );
            }
        }

        return (highScore, lastWinnerFound);
    }
    ...
    function payWinner() public payable onlyOwner {
        ...
        _scoreIds[_mintedIds[_winnerTokenId]] = 0;
        _nextScoreIds[_mintedIds[_winnerTokenId]] = 0;
