...
contract NFTCollection is ERC1155, Ownable, Pausable, ERC1155Supply {
    ...
    constructor() ERC1155("https://img.youtube.com/vi/") { //base URI
    ...
    function uri(uint256 id) public view virtual override returns (string memory) {
        return string( abi.encodePacked( super.uri(id), lookupmap[id], ".json" ));
    }
...
contract NFTMarketplace is ERC1155Holder{
  ...
  constructor(address _nftCollection, address _vnd) {
    nftCollection = NFTCollection(_nftCollection);
    VND = IERC20(address(_vnd));
  }
  ...
  function fillOffer(uint _offerId, uint amount) public payable {
    ...
    // uint256 VNDbalance = VND.balanceOf(msg.sender);
    // uint VNDpriceTotal = _offer.price * amount;
    // require(VNDbalance >= VNDpriceTotal, 'The VND amount should match with the NFT Price');
    // VND.transferFrom(msg.sender, address(this), VNDpriceTotal);
