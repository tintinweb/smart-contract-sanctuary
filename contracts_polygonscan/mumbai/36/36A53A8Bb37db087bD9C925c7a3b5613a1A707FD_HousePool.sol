...
contract HousePool{
    ...
    function PendingRewards(address _user,uint _box) public ValidBox(_user,_box)   view returns(uint)
    {
        ...
        // if(RewardAmount > 0)
        // {
        //     accrewardpershare =accrewardpershare+((RewardAmount*1e12)/TotalDepositedAmount);
        // }
    ...
    function withdraw(uint amount,uint _box) public ValidBox(msg.sender,_box) returns(bool success) 
    {
        ...
        if(amount > 0) {
            if(user.ActualBalance - amount== 0)
            {
                ...
                // {
                //     if(UserBoxNums[msg.sender][i] == _box)
                //     {
    ...
    function setMyPresentBalance(address _user,uint _box)public returns(uint){//Keep it internal
       UserBoxStruct storage user = Users[_user][_box];
       //require(user.Balance > 0,"The Balance is Low");
    ...
    function CalculateAccumulatePerShare(uint _amount)public {
    ...
    //Extra
    function CalculateLossPerShare(uint _amount)public{
