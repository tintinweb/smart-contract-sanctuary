...
contract PULSEROLL is VRFConsumerBase {
    ...
    mapping(bytes32 => uint) playerOddEvenStatus;
    mapping(bytes32 => uint) playerRangeUpperLimit;
    mapping(bytes32 => uint) playerRangeLowerLimit;
    mapping(bytes32 => bool) playerRangeTrue;
    ...
    //--->modifiers
    //modifier betIsValid(uint256 _betSize, uint256 _playerNumber,bool isRangeTrue, uint _OddEvenStatus ,uint rangeUpperLimit, uint rangeLowerLimit, bytes32 requestId) {
        //require (((
           // (((_betSize * (100 - (_playerNumber - 1)) /(_playerNumber - 1) + _betSize)) * houseEdge) / 
           // houseEdgeDivisor) - _betSize <= IHousePool(StakingContract).maxProfit() ));
        //require(calcTempPlayerProfit (_playerNumber, _betSize, playerRangeTrue[requestId], _OddEvenStatus, rangeUpperLimit, rangeLowerLimit,requestId)<= IHousePool(StakingContract).maxProfit());
    ...
    */
    ...
    function playerRollDice(uint256 rollUnder,uint _OddEvenStatus ,uint rangeUpperLimit, uint rangeLowerLimit) public payable gameIsActive CheckZero 
      returns(bytes32)
    {
        
        // OddEvenStatus 0 for disbled , 1 for odd and 2 for even.
        require(rangeUpperLimit<rollUnder,"Range cannot be greater than the rollUnder");
        require(rangeLowerLimit<rollUnder,"Range cannot be greater than the rollUnder");
        ...
        if (rangeUpperLimit == 0 || rangeLowerLimit == 0 ){
            playerRangeTrue[requestId] = false;
        }
        else {
        ...
        playerProfit[requestId] = calcTempPlayerProfit (rollUnder, msg.value, playerRangeTrue[requestId], _OddEvenStatus, rangeUpperLimit, rangeLowerLimit,requestId);
            // (((
            //     ((msg.value * (100 - (rollUnder- 1))) /
            //         (rollUnder-1) +msg.value)
            // ) * houseEdge) / houseEdgeDivisor) -
            // msg.value;
        require(playerProfit[requestId] <= IHousePool(StakingContract).maxProfit());
    ...
    //calculating temporary player profit for display purposes
    function calcTempPlayerProfit (uint256 rollUnder, uint256 Betvalue, bool isRangeTrue, uint _OddEvenStatus, uint rangeUpperLimit, uint rangeLowerLimit,bytes32 requestId) public returns (uint256) {
       uint256 tempPlayerProfit= (((
                ((Betvalue * (100 - (rollUnder- 1))) /
                    (rollUnder-1) + Betvalue)
            ) * houseEdge) / houseEdgeDivisor) -
            Betvalue;
        ...
        if(isRangeTrue == true){
            uint  midNum= (rangeUpperLimit + rangeLowerLimit) /2;
            if(rangeUpperLimit > midNum && rangeLowerLimit <= midNum){
                //tempPlayerProfit = tempPlayerProfit + tempPlayerProfit * 10000 / 100000;
            
                if(midNum >= 90 && midNum < 99){
                        tempPlayerProfit = tempPlayerProfit + tempPlayerProfit * 2000 / 100000;
                    }

                else if(midNum >= 80 && midNum < 90){
                        tempPlayerProfit = tempPlayerProfit + tempPlayerProfit * 4000 / 100000;
                    }

                else if(midNum >= 70 && midNum < 80){
                        tempPlayerProfit = tempPlayerProfit + tempPlayerProfit * 6000 / 100000;
                    }

                else if(midNum >= 60 && midNum < 70){
                        tempPlayerProfit = tempPlayerProfit + tempPlayerProfit * 8000 / 100000;
                    }

                else if(midNum >= 50 && midNum < 60){
                        tempPlayerProfit = tempPlayerProfit + tempPlayerProfit * 10000 / 100000;
                    }

                else if(midNum >= 40 && midNum < 50){
                        tempPlayerProfit = tempPlayerProfit + tempPlayerProfit * 12000 / 100000;
                    }

                else if(midNum >= 30 && midNum < 40){
                        tempPlayerProfit = tempPlayerProfit + tempPlayerProfit * 14000 / 100000;
                    } 

                else if(playerDieResult[requestId]>= 20 && playerDieResult[requestId]< 30){
                        tempPlayerProfit = tempPlayerProfit + tempPlayerProfit * 16000 / 100000;
                    } 

                else if(midNum >= 10 && midNum < 20){
                        tempPlayerProfit = tempPlayerProfit + tempPlayerProfit * 18000 / 100000;
                    } 

                else if(midNum >= 2 && midNum < 10) {
                        tempPlayerProfit = tempPlayerProfit + tempPlayerProfit * 20000 / 100000;
                    } 
            }

        }
        return tempPlayerProfit;
    }
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal payoutsAreActive override {
        ...
        if (playerDieResult[requestId] < playerNumber[requestId]) {
            ...
            if(playerRangeTrue[requestId] == true){
            if(playerRangeLowerLimit[requestId] <= playerDieResult[requestId] && playerRangeUpperLimit[requestId] > playerDieResult[requestId] ) {
            /**
        ...
        uint checkOdd = playerDieResult[requestId] % 2;
        require(checkOdd != 0,"number not Odd");
        ...
        uint checkEven = playerDieResult[requestId] % 2;
        require(checkEven == 0,"No is not Even");
         if(playerRangeTrue[requestId] == false){
            /**
    ...
    function DistributeFunds(address _receiver)public onlyOwner{
       ...
       IHousePool(StakingContract).CalculateAccumulatePerShare(FiftyPercentOfAmount);
