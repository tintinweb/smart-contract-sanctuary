...
contract Mint is Ownable, Pausable {
    ...
    uint256 _landsSoldCounter = 11000;
    ...
    function buyToken(uint256 tokenId) public whenNotPaused {
        ...
        require(tokenId >= 11000, "Token id is outside of tokens on sale");
    ...
    function claimToken(uint256 tokenId) public whenNotPaused {
        require(tokenId >= 1000 && tokenId < 11000, "Token id is outside reserved tokens range");
        require(!_nftsContract.tokenExists(tokenId), "This token already minted");
        require(_nftsContract.isTransferAllowed(_msgSender(), 1), "Mint not allowed: exceeded max token number in target wallet");
    ...
    function getTokenPrice(uint256 landsSold) internal pure returns (uint256) {
        if (landsSold < 12) {
            return 10000000000000000;
            // WETH is with 18 decimals
        } else if (landsSold >= 26000 && landsSold < 52000) {
            return 20000000000000000;
        } else if (landsSold >= 52000 && landsSold < 78000) {
            return 30000000000000000;
        } else if (landsSold >= 78000 && landsSold < 10400) {
