...
contract RustyRats is ContextMixin,ERC721Enumerable,NativeMetaTransaction, Ownable {
    ...
    constructor(string memory baseURI) ERC721("Rusty Rats", "RUSTY")  {
        setBaseURI(baseURI);
    ...
    function mint(address _to, uint256 _optionId) external {
        require( !_paused,                              "Sale paused" );
        require( _optionId == 1,                         "Invalid option id" );
        require( _to != address(0),                      "Invalid address" );
        _safeMint(_to, totalSupply()+1);
    }

    // function tokenURI(uint256 _optionId) external view returns (string memory) {
    //     return _baseTokenURI + _optionId;
    // }

    function adopt(uint256 num) public payable {
        uint256 supply = totalSupply();
        require( !_paused,                              "Sale paused" );
        require( num < 21,                              "You can adopt a maximum of 20 Rats" );
        require( supply + num < 9999 - _reserved,      "Exceeds maximum Rats supply" );
    ...
    function giveAway(address _to, uint256 _amount) external onlyOwner() {
        require( _amount <= _reserved, "Exceeds reserved Rat supply" );
