    ...
    ContextMixin
{
    ...
    */
    ...
    function mint(
        ...
        require(((amount > uint(0)) &&
                 (((id == uint(1)) && (amount == restTotalSupply(id, amount))) ||
                  ...
                  ((id == uint(5)) && (amount == restTotalSupply(id, amount))))), "ChildNFTMetaKey: INVALID_AMOUNT");
        ...
        for (uint i = 0; i < ids.length; i++) {
          ...
          require(((amounts[i] > uint(0)) &&
                 (((ids[i] == uint(1)) && (amounts[i] == restTotalSupply(ids[i], amounts[i]))) ||
                  ...
                  ((ids[i] == uint(5)) && (amounts[i] == restTotalSupply(ids[i], amounts[i])))) ), "ChildNFTMetaKey: INVALID_AMOUNT");
        }
    ...
    function restTotalSupply(uint256 _id, uint256 _amount) public view returns (uint256) {
      ...
      if (_id == uint(1)) {
        require(( subtotal <= uint256(500) ), "ChildNFTMetaKey: EXCEED MAX_AMOUNT");
      } else if (_id == uint(2)) {
        require(( subtotal <= uint256(750) ), "ChildNFTMetaKey: EXCEED MAX_AMOUNT");
      } else if (_id == uint(3)) {
        require(( subtotal <= uint256(1000) ), "ChildNFTMetaKey: EXCEED MAX_AMOUNT");
      } else if (_id == uint(4)) {
        require(( subtotal <= uint256(7500) ), "ChildNFTMetaKey: EXCEED MAX_AMOUNT");
      } else if (_id == uint(5)) {
        require(( subtotal <= uint256(255) ), "ChildNFTMetaKey: EXCEED MAX_AMOUNT");
      }
