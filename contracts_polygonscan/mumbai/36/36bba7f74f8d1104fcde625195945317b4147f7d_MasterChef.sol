...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of RPMes
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. RPMes to distribute per block.
        uint256 lastRewardBlock;  // Last block number that RPMes distribution occurs.
        uint256 accRPMPerShare;   // Accumulated RPMes per share, times 1e18. See below.
        uint16 depositFeeBP;      // Deposit fee in basis points
        uint256 lpSupply;
    }

    // The RPM TOKEN!
    CoinToken public RPM;
    address public devAddress;
    address public feeAddress;
    // address public vaultAddress;

    // RPM tokens created per block.
    uint256 public RPMPerBlock = 0.01 ether;
    
    uint256 constant MAX_RPM_PERBLOCK= 2 ether;
    ...
    // View function to see pending RPMes on frontend.
    ...
    // Safe RPM transfer function, just in case if rounding error causes pool to not have enough FOXs.
    ...
    function updateEmissionRate(uint256 _RPMPerBlock) external onlyOwner {
        require(_RPMPerBlock<=MAX_RPM_PERBLOCK,"Too high");
        massUpdatePools();
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (address(referral) != address(0) && referralCommissionRate > 0) {
            ...
            if (referrer != address(0) && commissionAmount > 0) {
                RPM.mint(referrer, commissionAmount);
