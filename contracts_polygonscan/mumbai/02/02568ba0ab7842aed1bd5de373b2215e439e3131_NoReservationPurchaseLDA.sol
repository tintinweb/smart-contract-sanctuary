...
contract NoReservationPurchaseLDA is RoyalPausableUpgradeable {
    ...
    function updatePaymentWallet(address newWallet) external onlyOwner whenNotPaused {
        _paymentWallet = newWallet;
    }

    // @note Puts a tier up for sale 
    // setBuyPrice in wei units for this ERC-20 contract
    function registerTierForPurchase(uint128 tierID, uint256 priceWei, address erc20Contract, address ldaContract, address ldaHolderForTier) external onlyOwner whenNotPaused {
    ...
    function updateTierPurchaseRegistration(uint128 tierID, uint256 priceWei, address erc20Contract, address ldaContract, address ldaHolderForTier) external onlyOwner whenNotPaused {
        tierPaymentERC20Contract[tierID] = erc20Contract;
        tierPurchasePriceWei[tierID] = priceWei;
        tierLDAContract[tierID] = ldaContract;
        tierLDAHolder[tierID] = ldaHolderForTier;
    ...
    function buy(uint128 tierID, address buyer, string memory correlationID, bytes memory proof) external whenNotPaused {
        ...
        // Move the token 
        IERC20(paymentAddress).safeTransferFrom(buyer, _paymentWallet, purchaseAmount);
    ...
    function transferForOffchainPurchase(uint128 tierID, address buyer, string memory correlationID) external onlyOwner whenNotPaused {
        // Check that this tier has been registered 
        require(tierRegistered(tierID), "Tier unregistered");
        require(nextTokenToSellForTier[tierID] <= maxSupplyForTier[tierID], "Tier exhausted");
    ...
    RoyalPausableUpgradeable
{
    ...
    */ 
    ...
    function mintLDAToOwner(address owner, uint256 ldaID, bytes calldata data) public onlyOwner whenNotPaused {
