...
contract MZKZ_ASCENSION is ERC1155, Ownable, BaseOpenSea, ERC1155Burnable {
    ...
    function airdropPacks(address[] calldata _addresses, uint256[] memory _amounts) external onlyOwner {
		for(uint256 a = 0; a < _addresses.length; a++) {
            uint256 airdropVolume = _amounts[a] * dispatchesPerPack;
            for(uint256 i = 0; i < airdropVolume; i++) {
                _gatherDispatch(_addresses[a]);
            }
        }
    }

    function mintPacks(uint256 amount) public payable {
		require(msg.value == packPrice * amount, "Invalid payment amount");
        ...
        uint256 purchaseVolume = amount * dispatchesPerPack;
        for(uint256 i = 0; i < purchaseVolume; i++) {
            _gatherDispatch(msg.sender);
        }
    }

    function _gatherDispatch(address account) private {
    ...
    function _generateRandomTokenId(address account) private view returns (uint256) {
		uint256 seed =
			uint256(
				keccak256(
					abi.encodePacked(block.timestamp + block.difficulty + block.gaslimit + (mintedDispatches*block.number) +
						((uint256(keccak256(abi.encodePacked(account)))) / (block.timestamp+mintedDispatches))
				)
			));

		// uint256 id_index = seed.mod(dispatch_ids.length);
		// for (uint256 i = 0; i < dispatch_ids.length; i++) {
		// 	id_index = (id_index+i).mod(dispatch_ids.length);
		// 	if (totalSupply(dispatch_ids[id_index]) < MAX_PUZZLES_SUPPLY) {
		// 		return dispatch_ids[id_index];
		// 	}
		// }
		return dispatch_ids[seed.mod(dispatch_ids.length)];
	}
