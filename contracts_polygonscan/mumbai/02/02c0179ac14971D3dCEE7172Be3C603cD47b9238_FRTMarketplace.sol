...
contract FRTMarketplace is Ownable{
    ...
    function buyNFT(uint256 _tokenId) public returns(string memory){
        require(listedNFTs[_tokenId], "Token not listed for sale");
        require(checkAlowance(msg.sender) >= baseNFTPrice, "Transfer amount exceeds allowance");

        if(downloadAllowed[msg.sender][_tokenId]){
    ...
    function listNFT(uint256 _tokenId) public onlyOwner{
        require(!listedNFTs[_tokenId], "NFT listed already");
