...
contract MaticMike is ERC721Enumerable, VRFConsumerBase  {
...
/*
    ...
    Portions of this contract for drawing pixels & initial trait setup
    are recycled directly from Anonymice. The code has been studied to be
    ...
    start of something much bigger than just a game or cartoon.
*/
    ...
    //uint256
    uint256 UPGRADE_CHANCE = 9499; // may be adjustable if too high
    uint256 UPGRADE_COST = 2000000000000000000; // set low for testing
    ...
    constructor() 
        ...
        ERC721("MM Testing Counter", "MIKE")
    {
        _owner = msg.sender;
    ...
    */
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        ...
        require((randomness % 10000) >= 0, "Bad roll, upgrade attempt failed");
                        ...
                        bytes(
                            string(
                                abi.encodePacked(
                                    ...
                                    '", "description": "Matic Mike is a collection of 10000 NFTs on the Polygon Chain. Staking, dynamic upgrades, dance battles and boss fights, Matic Mike is the cool new kid on Polygon.", "image": "data:image/svg+xml;base64,',
    ...
    */
    ...
    function currentHghCost() public view returns (uint256) {
        if (_tokenIds.current() >= 2 && _tokenIds.current() <= 4)
            return 1000000000000000000;
        if (_tokenIds.current() > 4 && _tokenIds.current() <= 6)
            return 2000000000000000000;
        if (_tokenIds.current() > 6 && _tokenIds.current() <= 8)
            return 3000000000000000000;
        if (_tokenIds.current() > 8 && _tokenIds.current() <= 10000)
    ...
    function mintMike() public {
        ...
        if (_tokenIds.current() < 2) return mintInternal();
    ...
    function donationMint() public payable {
        ...
        require(10000000000000000 == msg.value, "Insuffient amount sent.");
