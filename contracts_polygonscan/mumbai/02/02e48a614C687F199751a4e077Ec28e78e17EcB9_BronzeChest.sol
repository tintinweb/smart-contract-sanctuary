...
// File contracts/inventory/CDHAccessControl.sol
...
// File contracts/pool/CardPool.sol
...
// File contracts/inventory/CDHInventory.sol
...
EIP712MetaTransaction("Crazy Defence Heroes Inventory", "1.0") {
    ...
    function card(uint256 tokenId) public view returns (uint256) {
        return (tokenId >> 80) << 80;
    }

    function info(uint256 id) public view returns (string memory name, bytes16 cardType, bytes1 rarity, uint256 rank, uint256 level, string memory _uri, uint256 blockNum) {
    ...
    function setPool(bytes1 poolId, address poolAddress) public onlyAdmin(DEFAULT_ADMIN_ROLE) {
    ...
    function pool(bytes1 poolId) public view returns (address) {
        return availablePools[poolId];
    }

    function pool(uint256 token) public view returns (address) {
        return tokenPools[token];
    }

    function setURI(string memory newuri) public onlyAdmin(DEFAULT_ADMIN_ROLE) {
        _setURI(newuri);
    }
    ...
    function mint(address account, uint256 cardId, uint256 amount, bytes memory data) public onlyMinter {
    ...
    function countByCardId(uint256 _cardId) public view returns (uint256) {
        return counter[_cardId];
    }

    function pause() public onlyAdmin(DEFAULT_ADMIN_ROLE) {
        _pause();
    }

    function unpause() public onlyAdmin(DEFAULT_ADMIN_ROLE) {
        _unpause();
    }
...
// File contracts/chest/ChestOperator.sol
...
contract ChestOperator is Ownable, Pausable {
    ...
    constructor(address _targetInventory)
    Ownable()
    Pausable() {
    ...
    function setInventory(address _inventory) external {
        cdhInventory = CDHInventory(_inventory);
    }
...
// File contracts/chest/Chest.sol
...
abstract contract Chest is ChestOperator, EIP712MetaTransaction {
    ...
    function setRandomNumberSender(address _newSender) public onlyOwner {
    ...
    function setBlackHoleContract(address _blackHoleContract) public onlyOwner {
    ...
    function _generateBaseSeed(address sender, uint256 _quantity, bytes memory _sig) internal returns (bytes32) {
        ...
        seedNonce[sender]++;
        return keccak256(abi.encodePacked(block.timestamp, _sig));
    }
    ...
    function open(uint256 _quantity, bytes memory _sig) public {
...
// File contracts/chest/BronzeChest.sol

pragma solidity >=0.6.6;

contract BronzeChest is Chest {

    constructor(
        string memory _name,
        string memory _version,
        address _bronzeChestToken,
        address _randomNumberSender,
        address _inventory,
        address _equipmentPool,
        address _heroPool,
        ...
        address _towerPool,
        address _blackHoleContract
    ) Chest(_name, _version, _bronzeChestToken, _randomNumberSender, _inventory,
    ...
    function _getProbabilisticRarity(uint256 rarityProb, uint256 counter) internal override pure returns (bytes1) {
        if (counter == 1) {
            ...
            if (rarityProb <= 60) {
                return Constants.RARITY_COMMON;
            } else if (rarityProb > 60 && rarityProb <= 96) {
                return Constants.RARITY_RARE;
            } else if (rarityProb > 96 && rarityProb <= 100) {
                return Constants.RARITY_EPIC;
            }
        } else if (counter == 3) {
            if (rarityProb <= 12) {
                return Constants.RARITY_COMMON;
            } else if (rarityProb > 12 && rarityProb <= 52) {
                return Constants.RARITY_RARE;
            } else if (rarityProb > 52 && rarityProb <= 100) {
                return Constants.RARITY_EPIC;
            }
        }
