...
contract ModifiableMain is Ownable, ERC721Custom {
    ...
    // events
    event Mint(address minter, uint256 price);

    // CMDFT
    event ColorModificaton (uint256 indexed tokenId, uint8 position, uint64 color);
     ...
     */
    constructor (string memory name_, string memory symbol_, uint256 saleStartTS, uint256 freeMintsLimit) 
        ...
        COLOR_FREEZE_TS = AUCTION_END_TS.sub(86400 * 4);
        FREE_MINTS_LIMIT = freeMintsLimit;

        // mint masterpiece at index 0. NFT pieces start with index 1. And mint 50 first for early supporters and giveaways
    ...
    */
    ...
    function getNFTPrice() public view returns (uint256) {
        ...
        if (currentSupply < 200) {
            return MIN_PRICE.mul(2);
        } else if (currentSupply < 400) {
            return MIN_PRICE.mul(4);
        } else { 
    ...
    */
    function mintPiece() public payable {
        ...
        // if free minter - check rate limit
        if (msg.value == 0){
            freeMintersCounter[msg.sender] ++;
            require(freeMintersCounter[msg.sender] <= FREE_MINTS_LIMIT, "You are not allowed to free mint more than limit");
        }
    ...
    */
     ...
     */
    function modifyColor(uint256 tokenId, uint8[] memory positions, uint64[] memory colors) public {
    ...
    */
    function getTraitsOfTokenId(uint256 tokeinId) public view returns (uint256 length)
    {
        ...
        uint256 pieceIndex = tokeinId.sub(1).add(startingIndex) % 1024;
