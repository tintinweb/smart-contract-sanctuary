...
// File: contracts/mocks/IAlohaNFT.sol
...
// File: contracts/mocks/AlohaGovernanceRewards.sol
...
// File: contracts/mocks/AlohaGovernance.sol
...
contract AlohaGovernance is Ownable, ReentrancyGuard, AlohaGovernanceRewards {
    ...
    ******************/
    ...
    EVENTS
    ******************/
    event ProcessedProposal(uint256 indexed proposalId, address indexed proposer, string details, uint256 created);
    event ReviewedProposal(uint256 indexed proposalId, address indexed proposalModerator, ReviewStatus newStatus, uint256 created);
    event VotedProposal(uint256 indexed proposalId, address indexed user, Vote vote, uint256 created);
    event ExecutedProposal(uint256 indexed proposalId, address indexed user);
...
// File: contracts/AlohaStakingV2.sol
...
contract AlohaStakingV2 is Ownable{
    using SafeMath for uint256;
    ...
    // Info of each pool.
    struct PoolInfo {
        uint256 accPerShare;     // Accumulated Waffles per share, times 1e12. See below.
        uint256 lastRewardBlock;    // Last block number that WAFs distribution occurs.
    }

    // The REWARD TOKEN
    IERC20 public rewardToken;
    AlohaGovernance alohaGovernance;
    ...
    // Fees
    uint256 maxFee = 500; // 5%
    uint256[] requiredPowerForCustomFee = [1, 5, 50, 100];
    uint256[] customFeeByRequiredPower  = [400, 300, 200, 100];
    ...
    constructor(
        IERC20 _rewardToken,
        AlohaGovernance _alohaGovernance,
    ...
    function deposit(uint256 amount) public {
        ...
        require(user.amount.add(amount) <= limitAmount, 'AlohaStakingV2: Exceed the top');
    ...
    function withdraw(uint256 _amount) public {
        ...
        require(user.amount >= _amount, "AlohaStakingV2: Too much amount");

        updatePool();

        uint256 remainingRewards = (IERC20(rewardToken).balanceOf(address(this))).sub(totalStaked);
        uint256 pending = user.amount.mul(pool.accPerShare).div(1e12).sub(user.rewardDebt);
        uint256 feePercentage = maxFee;
        uint256 feeAmount = 0;
        ...
        if(_amount > 0) {
            ...
            (, , uint256 power) = AlohaGovernance(alohaGovernance).users(msg.sender);
            for (uint8 i = 0; i < customFeeByRequiredPower.length; i++) {
                if (power >= requiredPowerForCustomFee[i]) {
                    feePercentage = customFeeByRequiredPower[i];
                }
            }

            feeAmount = _amount.mul(feePercentage).div(10000);

            IERC20(rewardToken).transfer(msg.sender, _amount.sub(feeAmount));
        }
        user.rewardDebt = user.amount.mul(pool.accPerShare).div(1e12);
    ...
    function updateWithdrawFees(
        uint256 _maxFee,
        uint256[] memory _requiredPowerForCustomFee,
        uint256[] memory _customFeeByRequiredPower
    )
        public
        onlyOwner()
    {
        require(_maxFee < 5000, "AlohaFarming: Fee must be under 50%");
        require(_customFeeByRequiredPower.length <= 5, "AlohaFarming: Custom fee max length exeeced");
        require(_customFeeByRequiredPower.length == _customFeeByRequiredPower.length, "AlohaFarming: Custom fee and required power must have same length");

        for (uint8 i = 0; i < _customFeeByRequiredPower.length; i++) {
            require(_requiredPowerForCustomFee[i] < 5000, "AlohaFarming: Fee must be under 50%");
        }

        maxFee = _maxFee;
        requiredPowerForCustomFee = _requiredPowerForCustomFee;
        customFeeByRequiredPower  = _customFeeByRequiredPower;
    }
