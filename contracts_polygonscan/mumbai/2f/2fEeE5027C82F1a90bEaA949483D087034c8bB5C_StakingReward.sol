...
contract StakingReward is ReentrancyGuard, Ownable {
    ...
    uint public depositFees=30;
    uint public withdrawFees=50;

    // Duration that rewards are streamed over
    uint256 public rewardsDuration = 500;
    ...
    constructor(address _stakingToken,address[] memory _rewardsToken) Ownable() {
    ...
    function _earned(address _user,address _rewardsToken,uint256 _balance,uint256 supply) internal view returns (uint256) {
        return _balance.mul(
    ...
    function claimableRewards(address account) external view returns (RewardData[] memory rewards) {
        ...
        for (uint256 i = 0; i < rewards.length; i++) {
            ...
            rewards[i].amount = _earned(account, rewards[i].token, balances[account], totalSupply);
        }
        return rewards;
    }
    ...
    function stake(uint256 amount) external nonReentrant updateReward(msg.sender) {
        ...
        uint feesToken=amount.mul(depositFees).div(denominator);
        IERC20(stakingToken).safeTransferFrom(msg.sender,owner(),feesToken);
        amount=amount.sub(feesToken);
    ...
    function withdraw(uint256 amount) public nonReentrant updateReward(msg.sender) {
        ...
        uint feesToken=amount.mul(withdrawFees).div(denominator);
        IERC20(stakingToken).safeTransfer(owner(),feesToken);
        amount=amount.sub(feesToken);
    ...
    // Withdraw full balance and claim pending rewards
    ...
    function changeWithdrawFees(uint _amount) external onlyOwner{
        withdrawFees=_amount;
    }
    
    function chnageDepositFees(uint _amount)external onlyOwner{
        depositFees=_amount;
    }
