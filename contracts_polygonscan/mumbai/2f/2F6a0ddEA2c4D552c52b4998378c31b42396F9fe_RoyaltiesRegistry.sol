...
import "@orderinbox/royalties/contracts/Royalties.sol";
...
contract RoyaltiesRegistry is IRoyaltiesProvider, OwnableUpgradeable {
    ...
    function royaltiesFromContract(address token, uint tokenId) internal view returns (LibPart.Part[] memory) {
        if (IERC165Upgradeable(token).supportsInterface(LibRoyalties._INTERFACE_ID_ROYALTIES)) {
            Royalties royalties = Royalties(token);
            try royalties.getRoyalties(tokenId) returns (LibPart.Part[] memory result) {
                return result;
            } catch {}
        }
