...
contract KOITools is Ownable {
  ...
  function approveKOI(uint256 _KOI_ID) public payable{
    KOIContract.approve(address(this), _KOI_ID);
  }

 function approve_all(uint256[] calldata _ids) external payable {
        for (uint i = 0; i < _ids.length; i++) {
            KOIContract.approve(address(this), _ids[i]);
        }
    }

  function approveKOISpec(uint256[] memory _KOI_IDs) public payable{
  ...
  function approveKOIAll(uint256 _limite) external payable{
  ...
  function CollectLuckyMoneySpec(uint256[] memory _KOI_IDs) public payable{
  ...
  function CollectLuckyMoneyAllLimite(uint256 _limite) external payable{
    uint256[] memory result = new uint256[](_limite);
