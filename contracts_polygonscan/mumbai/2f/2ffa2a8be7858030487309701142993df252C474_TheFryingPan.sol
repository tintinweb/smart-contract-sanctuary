...
contract TheFryingPan is Ownable, IERC721Receiver, ReentrancyGuard {
	// maximum str score for a COP
	uint8 public constant MAX_STR = 10;
	uint256 public STR_RATIO = 1000; //can ajust the str ratio of gains per STR level if needed
	...
	// maps str to all Bacon stakes
	mapping(uint256 => Stake[]) public pack;

	// tracks location of each Bacon in Pack
	mapping(uint256 => uint256) public packIndices;

	// total str scores staked
	uint256 public totalStrStaked = 0;
	// any rewards distributed when no copbacons are staked

	uint256 public unaccountedRewards = 0;

	// amount of $GREASE due for each str point staked
	uint256 public GreasePerSTR = 0;

	// sheep earn 10000 $GREASE per day
	uint256 public constant DAILY_GREASE_RATE = 10000 ether;

	// sheep must have 2 days worth of $GREASE to unstake or else it's too cold
	...
	function _addBaconCopToPack(uint256 tokenId) internal {
		uint256 str = _strengthForBacon(tokenId);
		totalStrStaked += str;
		packIndices[tokenId] = pack[str].length; // Store the location of the Bacon in the Pack
		pack[str].push(Stake({ owner: msg.sender, tokenId: tokenId, value: GreasePerSTR })); // Add the Bacon to the Pack
		emit TokenStaked(msg.sender, tokenId, GreasePerSTR);
	}
	...
	/**
	 * realize $GREASE earnings for a single Bacon and optionally unstake it
	 * Wolves earn $GREASE proportional to their str rank
	 * @param tokenId the ID of the Bacon to claim earnings from
	 * @param unstake whether or not to unstake the Bacon
	 * @return owed - the amount of $GREASE earned
	 */
	function _claimBaconCopFromPack(uint256 tokenId, bool unstake) internal returns (uint256 owed) {
		...
		uint256 str = _strengthForBacon(tokenId);
		Stake memory _stake = pack[str][packIndices[tokenId]];
		require(_stake.owner == msg.sender, "msg.sender is not stake.owner");
		owed = ((str * STR_RATIO) / 1000) * (GreasePerSTR - _stake.value); // Calculate portion of tokens based on str
		if (unstake) {
			totalStrStaked -= str; // Remove str from total staked
			bacon.safeTransferFrom(address(this), msg.sender, tokenId, ""); // Send back Bacon
			Stake memory lastStake = pack[str][pack[str].length - 1];
			pack[str][packIndices[tokenId]] = lastStake; // Shuffle last Bacon to current position
			packIndices[lastStake.tokenId] = packIndices[tokenId];
			pack[str].pop(); // Remove duplicate
			delete packIndices[tokenId]; // Delete old mapping
		} else {
			pack[str][packIndices[tokenId]] = Stake({
				owner: msg.sender,
				tokenId: uint256(tokenId),
				value: uint256(GreasePerSTR)
			}); // reset stake
		}
	...
	function rescue(uint256[] calldata tokenIds) external nonReentrant {
		...
		for (uint256 i = 0; i < tokenIds.length; i++) {
			...
			if (isBacon(tokenId)) {
				...
				str = _strengthForBacon(tokenId);
				_stake = pack[str][packIndices[tokenId]];
				require(_stake.owner == msg.sender, "msg.sender is not stake.owner");
				totalStrStaked -= str; // Remove str from total staked
				bacon.safeTransferFrom(address(this), msg.sender, tokenId, ""); // Send back Bacon
				lastStake = pack[str][pack[str].length - 1];
				pack[str][packIndices[tokenId]] = lastStake; // Shuffle last Bacon to current position
				packIndices[lastStake.tokenId] = packIndices[tokenId];
				pack[str].pop(); // Remove duplicate
				delete packIndices[tokenId]; // Delete old mapping
	...
	function _payBaconTax(uint256 amount) internal {
		...
		GreasePerSTR += (amount + unaccountedRewards) / totalStrStaked;
	...
	modifier _updateEarnings() {
		if (totalGREASEEarned < MAXIMUM_GLOBAL_GREASE) {
			totalGREASEEarned +=
				((block.timestamp - lastClaimTimestamp) * totalSheepStaked * DAILY_GREASE_RATE) /
				1 days;
	...
	function setSTRRatio(uint256 _newRatio) external onlyDev {
		STR_RATIO = _newRatio;
	}
	...
	/**
	 * gets the strengh score for a Bacon (higher is better)
	 * @param tokenId the ID of the Bacon to get the str score for
	 * @return the str score of the Bacon
	 */
	function _strengthForBacon(uint256 tokenId) internal view returns (uint256) {
		(, uint256 strIndex) = bacon.tokenTraits(tokenId);
		return strIndex; // higher is better
	}
	...
	function randomBaconOwner(uint256 seed) external view returns (address) {
		if (totalStrStaked == 0) return address(0x0);
		uint256 bucket = (seed & 0xFFFFFFFF) % totalStrStaked; // choose a value from 0 to total str staked
		uint256 cumulative;
		seed >>= 32;
		// loop through each bucket of Wolves with the same str score
		for (uint256 i = 1; i <= MAX_STR; i++) {
			cumulative += pack[i].length * i;
			// if the value is not inside of that bucket, keep going
			if (bucket >= cumulative) continue;
			// get the address of a random Bacon with that str score
