...
interface IBarn {
	function addToBarn(address account, uint256 tokenId) external;
...
contract Wolf is IWolf, ERC721Enumerable, Ownable, ReentrancyGuard {
	...
	uint256 public PAID_TOKENS = 1000;
	 ...
	 */
	constructor(address _wool) ERC721("Wolf Game", "WGAME") {
		devWallet = msg.sender;
		woolERC20 = WOOL(_wool);
	}
	...
	function mint(uint256 qty) external payable nonReentrant {
		...
		for (uint256 i = 0; i < qty; i++) {
			...
			totalWoolCost += mintCost(totalSupply() + 1);

			if (totalWoolCost > 0) {
				woolERC20.burn(_msgSender(), totalWoolCost);
			}

			//set the token traits
			uint256 isSheepInt = seed % 100; //90% sheep
			uint256 alpha = 1;
			bool isSheep = true;
			if (isSheepInt >= 90) {
				isSheep = false;
				alpha = (isSheepInt % 10) + 1; //rank 1-10
			}

			SheepWolf memory sw = SheepWolf(isSheep, alpha);
	...
	/**
	 * the first are paid in ETH, then in $WOOL
	...
	function selectRecipient(uint256 seed) internal view returns (address) {
		...
		address thief = barn.randomWolfOwner(seed);
	...
	function withdraw() external {
		...
		uint256 devFee = _calcPercentage(totalBalance, 1000);
...
contract WOOL is ERC20, Ownable {
	...
	// withdraw accidentally sent ETH to this contract (not used)
