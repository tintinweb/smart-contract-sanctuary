...
contract FuzzyswapStaker is IFuzzyswapStaker, Multicall {
    ...
    function poolAddresses(IncentiveKey memory key, uint256 tokenId) external view override returns(address keyPool, address computedAddress){
        (IFuzzyswapPool pool,,,) = NFTPositionInfo.getPositionInfo(deployer, nonfungiblePositionManager, tokenId);
        computedAddress = address(pool);
        keyPool = address(key.pool);
    }
...
interface IFuzzyswapStaker is IERC721Receiver, IMulticall {
    ...
    function poolAddresses(IncentiveKey memory key, uint256 tokenid) view external returns(address keyPool, address computedAddress);
