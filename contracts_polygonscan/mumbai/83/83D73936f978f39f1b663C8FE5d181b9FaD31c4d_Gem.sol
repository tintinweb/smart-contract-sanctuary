...
contract Gem is Context, Ownable, ERC721, ERC721Pausable, EIP712 {
    ...
    string private _uri = "https://gateway.pinata.cloud/ipfs/QmY3MPjwB1PLG1M6YXthL2SigzAgJRznXTj7GVH8zcvJcc/token.json";
    string private _contractUri =
        "https://gateway.pinata.cloud/ipfs/QmY3MPjwB1PLG1M6YXthL2SigzAgJRznXTj7GVH8zcvJcc/collection.json";

    constructor() ERC721("Gem", "GEM") EIP712("Gem", "1.0.0") {}
    ...
    function alumniMint(SignedGemVoucher calldata _voucher) external checkIfUsedAndMarkAsUsed(_voucher.signature) {
        ...
        if (hnIdToTokenId[_voucher.hnId] != 0) {
            // This hn id has minted a token before

            // Case 1 burn the existing gem
            if (ownerOf(hnIdToTokenId[_voucher.hnId]) != address(0)) _burn(hnIdToTokenId[_voucher.hnId]);

            // Case 1+2 mint to the new wallet
            _safeMint(_voucher.wallet, hnIdToTokenId[_voucher.hnId]);
        } else {
        ...
        checkIfUsedAndMarkAsUsed(_voucher.signature)
    {
        ...
        // Burn the gem
        _burn(hnIdToTokenId[_voucher.hnId]);
