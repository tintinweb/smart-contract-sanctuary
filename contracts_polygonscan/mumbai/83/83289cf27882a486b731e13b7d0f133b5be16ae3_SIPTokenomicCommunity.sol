...
import "./SIPTokenomic.sol";

contract SIPTokenomicCommunity is SIPTokenomic {}
...
abstract contract SIPTokenomic is AccessControlUpgradeable, PausableUpgradeable {
    ...
    function transfer(address addr, uint256 amount) public virtual whenNotPaused onlyRole(WITHDRAWER) {
    ...
    function tokenTransfer(
        ...
        address addr,
        uint256 amount
    ) public whenNotPaused onlyRole(WITHDRAWER) {
    ...
    Configurable
{
    ...
    function initialize(
        ...
        __ERC20_init("Space Invaders Token", "SIP");
        __Ownable_init();
        __Pausable_init();
        ...
        _mint(ido, 50_000_000 * 10**18);
        _mint(team, 150_000_000 * 10**18);
        _mint(partnership, 30_000_000 * 10**18);
        _mint(community, 50_000_000 * 10**18);
        _mint(reward, 530_000_000 * 10**18);
        _mint(liquidity, 20_000_000 * 10**18);
        _mint(reserves, 70_000_000 * 10**18);
    }
    ...
    function issueBlackFunds(bytes32 offchain) external virtual onlyOwner {
        require(_dirtyFunds > 0, "empty dirty funds");
        _mint(_rewardAddr, _dirtyFunds);
        _dirtyFunds = 0;
    ...
    function destroyBlackFunds(address _blackListedUser) external virtual onlyOwner {
        require(isBlackListed[_blackListedUser], "the address is not in the blacklist");
        uint256 funds = balanceOf(_blackListedUser);
        _burn(_blackListedUser, _dirtyFunds);
        _dirtyFunds += funds;
