...
contract ElementalDinos is ERC721Enumerable, Ownable {
  using Strings for uint256;
...
struct Text { 
  ...
  constructor(
    ...
    address _initContractAddress
  ) ERC721(_name, _symbol) {
    setBaseURI(_initBaseURI);
    setNotRevealedURI(_initNotRevealedUri);
    setContractAddress(_initContractAddress);
  }
...
function UpdateCost(uint256 supply) internal view returns (uint256 _cost){
      if(supply < 500 ) {
          return 1.5 ether;
      }
      if(supply < 850 ) {
...
function claim() public {
    ...
    require(ownedAmount >= requiredAmount, "You don't own enough Pixelated Derpy Dinosaur NFTs");
    uint256 supply = totalSupply();
    require(supply + 1 <= 12500, "Max supply reached");
     Text memory newText = Text("");
    texts[supply + 1] = newText;
