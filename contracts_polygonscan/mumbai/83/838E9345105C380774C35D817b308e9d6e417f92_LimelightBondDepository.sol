...
contract LimelightBondDepository is Ownable {
    ...
    IERC20 public immutable LMLT; // token given as payment for bond
    IERC20 public immutable principle; // token used to create bond
    ITreasury public immutable treasury; // mints LMLT when receives principle
    ...
    struct Bond {
        uint payout; // LMLT remaining to be paid
    ...
    function deposit( 
        ...
        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 LMLT ( underflow protection )
        ...
        uint balanceBefore = LMLT.balanceOf(address(this));
        /**
            principle is transferred in
            approved and
            deposited into the treasury, returning (_amount - profit) LMLT
         */
        ...
        if ( fee != 0 ) { // fee is transferred to dao 
            LMLT.safeTransfer( DAO, fee ); 
        }
        require(balanceBefore.add(profit) == LMLT.balanceOf(address(this)), "Not enough LMLT to cover profit");
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            LMLT.transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                LMLT.approve( address(stakingHelper), _amount );
                stakingHelper.stake( _amount, _recipient );
            } else {
                LMLT.approve( address(staking), _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return LMLT.totalSupply().mul( terms.maxPayout ) / 100000 ;
    }
    ...
    /**
     *  @notice calculate current ratio of debt to LMLT supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns ( uint debtRatio_ ) {   
        uint supply = LMLT.totalSupply();
    ...
    /**
     *  @notice calculate amount of LMLT available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or LMLT) to the DAO
     *  @return bool
     */
    function recoverLostToken(IERC20 _token ) external returns ( bool ) {
        require( _token != LMLT, "NAT" );
