...
contract Comptroller is ComptrollerV6Storage, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError {
    ...
    /// @notice Emitted when LP Vault info is changed
    event NewLPVaultInfo(address vault_, uint releaseStartBlock_, uint releaseInterval_);

    /// @notice Emitted when Atlantis LP Vault rate is changed
    event NewAtlantisLPVaultRate(uint oldAtlantisLPVaultRate, uint newAtlantisLPVaultRate);

    /// @notice Emitted when Atlantis is distributed to LP Vault
    event DistributedLPVaultAtlantis(uint amount);
    ...
    function distributeSupplierAtlantis(address aToken, address supplier) internal {
        if (address(lpVaultAddress) != address(0)) {
            releaseToVault();
        }
 ...
 /**
     * @notice Set the amount of Atlantis distributed per block to LP Vault
     * @param atlantisLPVaultRate_ The amount of Atlantis wei per block to distribute to LP Vault
     */
    function _setAtlantisLPVaultRate(uint atlantisLPVaultRate_) public {
        require(msg.sender == admin, "only admin can Atlantis LP vault rate");

        uint oldAtlantisLPVaultRate = atlantisLPVaultRate;
        atlantisLPVaultRate = atlantisLPVaultRate_;
        emit NewAtlantisLPVaultRate(oldAtlantisLPVaultRate, atlantisLPVaultRate_);
    }

    /**
     * @notice Set the LP Vault infos
     * @param vault_ The address of the LP Vault
     * @param releaseStartBlock_ The start block of release to LP Vault
     * @param minReleaseAmount_ The minimum release amount to LP Vault
     */
    function _setLPVaultInfo(address vault_, uint256 releaseStartBlock_, uint256 minReleaseAmount_) public {
        require(msg.sender == admin, "only admin can set LP vault info");

        lpVaultAddress = vault_;
        releaseStartBlock = releaseStartBlock_;
        minReleaseAmount = minReleaseAmount_;
        emit NewLPVaultInfo(vault_, releaseStartBlock_, minReleaseAmount_);
    }

    /**
     * @notice Transfer Atlantis to LP Vault
     */
    function releaseToVault() public {
        ...
        // releaseAmount = atlantisLPVaultRate * deltaBlocks
        uint256 _releaseAmount = mul_(atlantisLPVaultRate, deltaBlocks);
        ...
        atlantis.transfer(lpVaultAddress, actualAmount);
        emit DistributedLPVaultAtlantis(actualAmount);

        ILPVault(lpVaultAddress).updatePendingRewards();
    }
    ...
    function getAtlantisAddress() public view returns (address) {
        return 0x8bA90676E6Ef92C3f92D59271606E0F02fF4A34C;
    }
}
