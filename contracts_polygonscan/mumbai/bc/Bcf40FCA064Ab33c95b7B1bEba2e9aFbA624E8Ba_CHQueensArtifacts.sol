...
contract CHQueensArtifacts is Delegated, MaticERC1155{
  ...
  function create( address initialOwner, uint supply, string calldata uri_, bytes calldata data_ )
    ...
    require( _delegates[_msgSender()], "Invalid delegate" );
  ...
  function mint( uint id, uint quantity ) external payable{
    ...
    _safeTransferFrom( creators[id], _msgSender(), id, quantity, "" );
  }

  function mintTo( address[] calldata accounts, uint[] calldata ids, uint[] calldata quantities ) external payable {
    require( _delegates[_msgSender()], "Invalid delegate" );
    ...
    for(uint i; i < ids.length; ++i ){
      ...
      _safeTransferFrom( creators[ids[i]], accounts[i], ids[i], quantities[i], "" );
    }
  }
  ...
  function setToken(uint id, string memory name_, string calldata uri_, uint supply,
    ...
    bool isMintActive, uint mintPrice ) public {
    require( _delegates[_msgSender()], "Invalid delegate" );
  ...
  function setActive(uint id, bool isBurnActive, bool isMintActive) public {
    require( _delegates[_msgSender()], "Invalid delegate" );
    require( id < tokens.length, "Specified token (id) does not exist" );
  ...
  function setPrice(uint id, uint burnPrice, uint mintPrice) public {
    require( _delegates[_msgSender()], "Invalid delegate" );
    require( id < tokens.length, "Specified token (id) does not exist" );
  ...
  function setSupply(uint id, uint supply) public {
    require( _delegates[_msgSender()], "Invalid delegate" );
    require( id < tokens.length, "Specified token (id) does not exist" );
    ...
    if( supply > token.supply ){
      //increase
      _mint(_msgSender(), id, supply - token.supply, "" );
    }
    else if( supply < token.supply ){
      _burn(_msgSender(), id, token.supply - supply);
    }
    token.supply = supply;
  }
