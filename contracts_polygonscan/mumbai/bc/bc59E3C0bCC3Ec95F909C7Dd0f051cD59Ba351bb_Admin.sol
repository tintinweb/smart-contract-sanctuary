...
interface IShortStaking_ is IShortStaking, IOwnable {
    function add(uint256 _rootPid, IShortToken _shortToken, bool _withUpdate) external;
...
abstract contract AssetAdmin {
    ...
    function asset_registerAsset(
        ...
        IAsset_(asset).registerAsset(assetToken, assetOracle, auctionDiscount, minCRatio, isInPreIPO, poolId, ipoParams);
    }
    ...
    function asset_updateAsset(
        ...
        IAsset_(asset).updateAsset(assetToken, assetOracle, auctionDiscount, minCRatio, isInPreIPO, poolId, ipoParams);
    }
...
pragma solidity ^0.8.2;
contract Admin is ShortStakingAdmin, ShortLockAdmin, AssetAdmin, MintAdmin, MasterChefAdmin {
    ...
    function transferAllOwnership(address newOwner) external onlyOwner {
        require(newOwner != address(0), "Admin: new owner is the zero address");
        IShortStaking_(staking).transferOwnership(newOwner);
    ...
    function whiteList(
        string memory nTokenName,
        string memory nTokenSymbol,
        string memory sTokenName,
        string memory sTokenSymbol,
        uint allocPoint_,
        AggregatorV3Interface oracle,
        uint16 auctionDiscount,
        uint16 minCRatio,
        ...
        AssetToken nToken = new AssetToken(nTokenName, nTokenSymbol);
        nToken.transferOwnership(mint);

        ShortToken sToken = new ShortToken(sTokenName, sTokenSymbol);
        sToken.transferOwnership(staking);
        ...
        masterChef_add(allocPoint_, IERC20(pair), false);

        // add a pool in LongStaking
        // TODO ...

        // add a short farm pool in MasterChef
        masterChef_add(allocPoint_, IERC20(sToken), false);
        ...
        uint poolId = IShortStaking(staking).poolLength() - 1;
