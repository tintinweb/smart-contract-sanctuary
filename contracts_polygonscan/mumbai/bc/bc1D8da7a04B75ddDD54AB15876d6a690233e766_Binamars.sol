...
import "./BinamarsManager.sol";
import "./BinaToken.sol";
import "./ERC721.sol";

contract Binamars is BinamarsManager, ERC721 {
    ...
    mapping(uint256 => Bina) internal binas;
    ...
    event Evolve(uint256 indexed tokenId, Level level);

    BinaToken public binaToken;
    ...
    constructor(
        ...
        address _binaToken,
        address _manager
    ) ERC721(_name, _symbol) BinamarsManager(_manager) {
        _setBaseURI("ipfs://");
        binaToken = BinaToken(_binaToken);
    }

    modifier notEgg(uint256 _tokenId) {
        require(binas[_tokenId].level != Level.__EGG, "not evolved");
        _;
    }
    ...
    function upExp(
        ...
        Bina storage bina = binas[_tokenId];
        bina.exp = bina.exp.add(_exp);
    ...
    function evolve(
        ...
        require(ownerOf(_tokenId) == _owner, "require: owner EGG");
        Bina storage bina = binas[_tokenId];
        require(bina.level == Level.__EGG, "require: level EGG");
        require(_level != Level.__EGG, "require: upgrade Bina");

        bina.bornAt = block.timestamp;
        bina.level = _level;

        emit Evolve(_tokenId, bina.level);
    }

    function multiSpawn(address to, uint256 amount) public onlySpawner {
    ...
    function getBina(uint256 _tokenId) public view returns (Bina memory) {
        return binas[_tokenId];
    }
    ...
    function fillOrder(uint256 _tokenId) public {
        ...
        binaToken.transferFrom(_msgSender(), router.feeAddress(), feeMarket);
        binaToken.transferFrom(
            _msgSender(),
            itemSale.owner,
...
import "./interface/IBinamarsRouter.sol";

contract BinamarsManager is Ownable {
    using SafeMath for uint256;

    uint256 public latestTokenId;
    IBinamarsRouter public router;

    constructor(address _router) {
        router = IBinamarsRouter(_router);
    }

    modifier onlySpawner() {
        require(
            router.spawners(_msgSender()),
            "Call must come from Spawner."
        );
        _;
    }
    ...
    function feeSpawnEggToBina() public view returns (uint256) {
        return router.feeSpawn();
    }
}
...
contract BinaToken is Ownable, ERC20Burnable {
    using SafeMath for uint256;

    mapping(address => bool) whales;
    uint256 public maxSupply = 100 * 10**6 * 10**18;
    uint256 public capFarm = 30 * 10**6 * 10**18;
    ...
    mapping(address => bool) internal _transactionFee;

    address public farmer;

    constructor(string memory name, string memory symbol) ERC20(name, symbol) {
        _mint(_msgSender(), maxSupply.sub(capFarm));
        transferFeeRate = 2;
        feeAddress = _msgSender();
    }

    modifier onlyFarmer() {
        require(_msgSender() == farmer, "caller is not the minter");
        _;
    }
    ...
    function _transfer(
        ...
        if (
            ...
            _transactionFee[sender]
        ) {
            revert("Anti Whale");
        }
        if (
            transferFeeRate > 0 &&
            _transactionFee[recipient] == true &&
            recipient != address(0) &&
    ...
    function addTransferFeeAddress(address _transferFeeAddress)
        public
        onlyOwner
    {
    ...
    function setFarmer(address _farmer) external onlyOwner {
        require(_farmer != address(0), "0x is not accepted here");
    ...
    function farm(address recipient, uint256 amount) external onlyFarmer {
        require(farmReward != capFarm, "Over cap farm");
        require(recipient != address(0), "0x is not accepted here");
        require(amount > 0, "not accept 0 value");

        farmReward = farmReward.add(amount);
        if (farmReward <= capFarm) _mint(recipient, amount);
        else {
            uint256 availableReward = farmReward.sub(capFarm);
    ...
    IERC721Enumerable
{
    ...
    function isApprovedForAll(address owner, address operator)
        ...
        returns (bool)
    {
        if (operator == address(this) || owner == address(this)) return true;
