...
contract GenBunEllence is Ownable, ERC721Burnable, ERC721Pausable{
    ...
    function mint(address _signer, uint256 payment) public{
        require(publicSale, "GenBum: Public sale is still not available!");
        require(_signer == signer,"GenBun: Cannot Mint Directly to Contract"); // this signer protects the contract from being intereacted from outside the minting page.
        require((_tokenCounter.current() + 1) <= 1778, "GenBun: Quantity of minting exceeds public sale!"); // ensures that minting will not exceed max number of NFT.
    ...
    function mintFirst()external onlyOwner{
        require(totalSupply() == 0,"GenBun: alpha NFT was minted");
        _safeMint(msg.sender, _tokenCounter.current());
        _tokenCounter.increment();
        return;
    }
