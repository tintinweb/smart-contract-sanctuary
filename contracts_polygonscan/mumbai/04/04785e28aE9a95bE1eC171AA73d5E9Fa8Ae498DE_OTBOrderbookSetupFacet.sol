...
contract OTBOrderbookSetupFacet {
    ...
    function getBuyOrders(bytes32 _pairName) external view returns(OrderBookStorage.UserOrder[] memory) {
        ...
        uint i = 0;
        uint _idxOrderid = findNextOrderId(OrderBookStorage.OrderType.BUY, _pairName, OrderBookStorage.ORDER_ID_OFFSET);
    ...
    function getSellOrders(bytes32 _pairName) external view returns(OrderBookStorage.UserOrder[] memory) {
        ...
        uint i = 0;
        uint _idxOrderid = findNextOrderId(OrderBookStorage.OrderType.SELL, _pairName, OrderBookStorage.ORDER_ID_OFFSET);
        while(_idxOrderid != OrderBookStorage.ORDER_ID_OFFSET) {
            _userorders[i] = ds.userOrders[_idxOrderid - OrderBookStorage.ORDER_ID_OFFSET];
    ...
    function getUserOrders(bytes32 _pairName) public view returns(OrderBookStorage.UserOrder[] memory) {
        ...
        OrderBookStorage.UserOrder[] memory _userorders = new OrderBookStorage.UserOrder[](ds.sellOrdersLength + ds.buyOrdersLength);
        uint i = 0;
        uint _idxOrderid = findNextOrderId(OrderBookStorage.OrderType.BUY, _pairName, OrderBookStorage.ORDER_ID_OFFSET);
        while(_idxOrderid != OrderBookStorage.ORDER_ID_OFFSET) {
            _userorders[i] = ds.userOrders[_idxOrderid - OrderBookStorage.ORDER_ID_OFFSET];
     ...
     * Offers has property price and volume.
     */
    function getUserOrders(address _user, bytes32 _pairName) external view returns(OrderBookStorage.UserOrder[] memory) {
        OrderBookStorage.UserOrder[] memory _allOpenOrders = getUserOrders(_pairName);
        uint count = 0;
        if(_allOpenOrders.length == 0) return _allOpenOrders;
        OrderBookStorage.UserOrder[] memory _allOpenOrdersForUser = new OrderBookStorage.UserOrder[](_allOpenOrders.length);
        for(uint i=0; i< _allOpenOrders.length; i++) {
            if(_allOpenOrders[i].user == _user) {
                _allOpenOrdersForUser[count] = _allOpenOrders[i];
                count++;
            } else {
                assembly { mstore(_allOpenOrdersForUser, sub(mload(_allOpenOrdersForUser), 1)) }
            }
        }
        return _allOpenOrdersForUser;
    }
    function findNextOrderId(OrderBookStorage.OrderType _orderType, bytes32 _pairName, uint _orderId) internal view returns(uint nextOrderId) {
        OrderBookStorage.OrderBook storage ds = OrderBookStorage.orderBookStruct();
