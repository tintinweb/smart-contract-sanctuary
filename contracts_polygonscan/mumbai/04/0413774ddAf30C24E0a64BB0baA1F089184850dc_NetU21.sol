...
contract NetU21 is ERC1155, AccessControl, ERC1155Burnable, ERC1155Supply, ERC1155Holder, Ownable {
    ...
    struct NetU21_NFT{
        ...
        mapping(uint => address) idToOwner; // idToOwner[NFT_ID][id] = 0x...addresswallet
        mapping(address => uint) NFTBalanceOf; // NFTBalanceOf[NFT_ID][0x...addresswallet] = #amount;
    }

    mapping(uint => NetU21_NFT) internal nftInfo;
    uint totalNFTRegistered = 0;
    ...
    /*
    function registerNFTCollection(uint256 NFTRegistered, uint256 NFTEquivalentTokens, string memory _baseURI, address AdminNFT, string memory _Type, bool Permanent, bool ForRentToOwner, uint ComissionRentToOwner) public onlyRole(MINTER_ROLE){
        totalNFTRegistered++;
        nftInfo[totalNFTRegistered] = NetU21_NFT(
            totalNFTRegistered,
            0,
            NFTRegistered,
            NFTEquivalentTokens,
            ...
            Permanent,
            ForRentToOwner,
            ComissionRentToOwner
        );

        emit newNFTCollection(totalNFTRegistered, 0, NFTRegistered, NFTEquivalentTokens, _baseURI, AdminNFT, _Type, Permanent, ForRentToOwner, ComissionRentToOwner);
        _mint(AdminNFT, totalNFTRegistered, NFTRegistered, "");
    }
*/
    //ok
    ...
    //OK
    function updateNFTTokenEquivalent(uint amount, uint _tokenId) public onlyRole(ADMIN_NFT){
    ...
    function sellNFT(address buyer, uint _tokenId, uint _tokenSerial, uint amount, bytes memory data) public onlyRole(MINTER_ROLE){

        require(buyer != address(0), "ZERO_ADDRESS");
        require(buyer != msg.sender, "SAME_ADDRESS_DESTINATION");
        require(nftInfo[_tokenId].idToOwner[_tokenSerial] == address(0), "NFT_ALREADY_EXISTS"); //in burned carbon credits

        uint256 totalTokens = (amount * nftInfo[_tokenId].NFTEquivalentTokens) * (10 ** decimals);

        //buyer: 68 from total aoperation% 
        //dividends: 17%
        uint256 valdividends = (totalTokens * percentForUpline) / percentForPublic;

        //validate balance
        uint256 _tot = totalTokens + valdividends;
    ...
    function _addNFToken(address _to, uint _tokenId, uint _tokenSerial, uint _amount) internal virtual {
    ...
    function ownerOf(uint256 _tokenId, uint _tokenSerial) external view returns (address){
        return nftInfo[typeNFT[_tokenId]].idToOwner[_tokenSerial];
    }
