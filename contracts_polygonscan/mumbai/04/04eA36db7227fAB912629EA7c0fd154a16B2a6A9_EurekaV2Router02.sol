...
library EurekaV2Library {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'ab150c3860bc9540eaae722dc55d4ca4fb0e41ee47d0b05bbb80437880d67dc3' // init code hash
...
contract EurekaV2Router02 is IEurekaV2Router02 {
    ...
    function swapTokensForExactTokens(
        ...
        amountOut = takeSwapFee(IEurekaV2Factory(factory).getPair(path[0], path[1]), path[0], amountOut, false);
    ...
    {
        require(path[0] == WETH, 'EurekaV2Router: INVALID_PATH');
        amountOut = takeSwapFee(IEurekaV2Factory(factory).getPair(path[0], path[1]), path[0], amountOut, true);
        amounts = EurekaV2Library.getAmountsIn(factory, amountOut, path);
   ...
   function takeAddLiquidityFee(address _lp,address _token0, address _token1, uint256 _amount0, uint256 _amount1, bool isEth) internal returns(uint256, uint256){
    ...
    function takeSwapFee(address lp, address token, uint256 amount, bool isEth) internal returns(uint256){
        uint256 pcent = IEurekaV2Factory(factory).PERCENT100();
        uint256 adminfee = amount.mul(IEurekaV2Factory(factory).swapAdminFee()).div(pcent);
        uint256 fee = amount.mul(IEurekaV2Factory(factory).swapFee()).div(pcent);
