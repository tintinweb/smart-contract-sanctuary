...
contract HeroDAO is ERC20, EIP712 {
    ...
    constructor(string memory _name, string memory _symbol, address _signer) ERC20(_name, _symbol) EIP712("HeroDAO", "1") {
    ...
    function claim(uint256 amountV, bytes32 r, bytes32 s) external {
        require(block.timestamp < END_AIRDROP, "HeroDAO: AirDrop Finished");

        uint256 amount = uint248(amountV);
        uint8 v = uint8(amountV >> 248);
        uint256 total = _totalSupply + amount;
        require(total <= MAX_SUPPLY, "HeroDAO: Exceed max supply");
        require(minted(msg.sender) == 0, "HeroDAO: Claimed");
        bytes32 digest = keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32",
                ECDSA.toTypedDataHash(_domainSeparatorV4(),
                keccak256(abi.encode(MINT_CALL_HASH_TYPE, msg.sender, amount))
        )));
        require(ecrecover(digest, v, r, s) == cSigner, "HeroDAO: Invalid signer");
        _totalSupply = total;
    ...
    function _checkSenderLock(uint256 amount) internal override view{
        if(msg.sender == ADDR_STAKING){
            ...
            if(passed <= 60){
                ...
                require(balanceOf(ADDR_STAKING) >= least_amount, "HeroDAO: Transfer Locked");
            }
        }
        if(msg.sender == ADDR_DAO || msg.sender == ADDR_LP){
                require(block.timestamp > END_AIRDROP, "HeroDAO: Transfer Locked");
        }
    }
}