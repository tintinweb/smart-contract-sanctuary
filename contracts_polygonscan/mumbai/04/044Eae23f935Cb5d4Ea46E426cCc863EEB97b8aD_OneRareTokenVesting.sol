...
contract OneRareTokenVesting is ITokenVesting, Ownable, VestingConfig {
    ...
    function addOrUpdateInvestor(
        ...
        _addInvestor(roundType_, investor_, allocation_);
        emit InvestorAdded(roundType_, investor_, allocation_);
    }

    function addOrUpdateInvestors(
        Round[] memory roundType_,
        address[] memory investor_,
        uint256[] memory allocation_
    ) external onlyOwner {
        uint256 length = roundType_.length;
        require(investor_.length == length && investor_.length == length, "INVALID_ARGUMENTS");
        for (uint256 i = 0; i < length; i++) {
            _addInvestor(roundType_[i], investor_[i], allocation_[i]);
        }
        emit InvestorsAdded(roundType_, investor_, allocation_);
    }
    ...
    function _addInvestor(
        ...
        require(investorAddress_ != address(0), "INVALID_ADDRESS");

        RoundInfo memory round = roundInfo[roundType_];
        Investor storage investor = investorInfo[roundType_][investorAddress_];

        if (investor.totalAssigned == 0) {
            require(round.supplyLeft >= allocation_, "INSUFFICIENT_SUPPLY");

            _investors[roundType_].push(investorAddress_);
            roundInfo[roundType_].supplyLeft = round.supplyLeft - allocation_;
        } else {
            ...
            roundInfo[roundType_].supplyLeft = round.supplyLeft + investor.totalAssigned - allocation_;
        }

        investor.totalAssigned = allocation_;

        if (round.noOfVestings > 0) {
            uint256 initialReleaseTokens = (allocation_ * round.initialReleasePercent) / _PERCENTAGE_MULTIPLIER;
            investor.vestingTokens = (allocation_ - initialReleaseTokens) / round.noOfVestings;
        }
    }
...
interface ITokenVesting {
    ...
    function addOrUpdateInvestors(
        VestingConfig.Round[] memory roundType_,
        address[] memory investor_,
        uint256[] memory allocation_
    ) external;
