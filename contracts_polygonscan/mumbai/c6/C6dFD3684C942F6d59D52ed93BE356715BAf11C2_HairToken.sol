...
contract HairToken is BEP20('Hair Token', 'HAIR') {
    ...
    */
        ...
        external
    {
        ...
        require(now <= expiry, "HAIR::delegateBySig: signature expired");
...
// MasterChef is the master of Hair. He can make HAIR and he is a fair guy.
//
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 accHairPerShare;   // Accumulated HAIRs per share, times 1e12. See below.
    ...
    // Bonus muliplier for early hair makers.
    ...
    function pendingHair(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accHairPerShare = accHairPerShare.add(hairReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accHairPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 hairReward = multiplier.mul(HairPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        hair.mint(devaddr, hairReward.div(10));
        hair.mint(address(this), hairReward);
        pool.accHairPerShare = pool.accHairPerShare.add(hairReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for HAIR allocation.
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accHairPerShare).div(1e12).sub(user.rewardDebt);
            if (pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accHairPerShare).div(1e12);
    ...
    function updateEmissionRate(uint256 _HairPerBlock) public onlyOwner {
        massUpdatePools();
        HairPerBlock = _HairPerBlock;
