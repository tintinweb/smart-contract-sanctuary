...
// File: contracts/SingleFloorDungeon.sol
...
/**
 * @title SingleFloorDungeon contract
 * @dev Extends ERC721 Non-Fungible Token Standard basic implementation
 */
contract SingleFloorDungeon is ERC721Enumerable, Ownable {
    ...
    constructor() ERC721("SingleFloorDungeon", "SFDUN") {
        MAX_NFTS = 9910;  // reserve some for giveaways, etc, which will be minted via reserveNfts below
        MAX_NFTS_PLUS_1 = 9911; // to save gas during 3rd "require" in main mint below 
        BASE_METADATA_URI = "https://singlefloor-metadata.generativedungeon.com/"; 
        CONTRACT_METADATA_URI = "https://generativedungeon.com/storefront-singlefloor-metadata.json"; 
    }
    ...
    /**
    * airdrops NFTs on the Polygon Network (for people copying the SingleFloorDungeon contract, NEVER do this on the Ethereum network, 
    *                                       as your gas fees will be ~50K for 1000 addresses)
    */
    function airDrop(address[] memory _addresses) public onlyOwner {
        require(totalSupply() + _addresses.length < MAX_NFTS_PLUS_1, "You would exceed the limit");

        uint256 supply = totalSupply(); // store to save gas
        
        uint256 finalAmt = _addresses.length+1;

        for(uint256 i=1; i < finalAmt; i++) { // start at Token #1 so IDs aren't zero-based
