...
contract MetaBadge is ERC721Namable, Ownable{
   ...
   mapping(address => bool) whiteListTier2;
    ...
    /*
    Mint NFT for msg.sender address
    You should send 2 arguments:
    1)uint256 _id - its ID of token that you mint
    2)uint256 tier - number of tier of NFT(1 tier - price 900 MATIC, 2 tier - 450 MATIC)
    3)If you mint for whitelisted address, you can send any number of tier.
    */
    function mint(uint256 _id, uint256 tier) public payable {
        require(MAX_NFT != totalSupply(), "NFT are over! Sorry :(");
        require(balanceOf(msg.sender) + 1 <= 5, "More than 5 NFTs");
        
        if( walletIsInWhiteListTier1(msg.sender) || walletIsInWhiteListTier2(msg.sender)){
