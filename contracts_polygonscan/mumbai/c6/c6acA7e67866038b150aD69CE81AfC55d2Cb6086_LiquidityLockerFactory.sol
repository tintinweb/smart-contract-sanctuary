...
contract LiquidityLockerFactory is Ownable, CloneBase {
    event VestingLaunched(address _vestingContract);
    ...
    function launchVesting(
        ...
        address referrer,
        address _liquidityLockerLibrary,
        ...
        // LiquidityLocker linearVesting = new LiquidityLocker(
        //     address(this),
        //     _exchangeIdentifier
        // );
        address liquidityLocker = createClone(_liquidityLockerLibrary);

        emit VestingLaunched(liquidityLocker);

        ILiquidityLockerMinimalProxy(liquidityLocker).init(
            _erc20Token,
            _start,
            _end,
        ...
        ILiquidityLockerMinimalProxy(liquidityLocker).transferOwnership(msg.sender);
