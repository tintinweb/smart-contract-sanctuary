...
// File contracts/KommunitasVoting.sol
...
// File contracts/KommunitasStaking.sol
...
contract KommunitasStaking is Ownable {
    ...
    mapping(uint256 => uint256) public durationMultiplier;
    ...
    constructor(address _komToken, uint256 _apy, uint256 _minDuration, uint256 _maxDuration) {
        require(_maxDuration > _minDuration, "Max duration should be greater than Min");
        komToken = ERC20Burnable(_komToken);
        komvToken = new KommunitasVoting(); // KOM Governance Token Deployment
        apy = _apy;
        minDuration = _minDuration * 1 days;
        maxDuration = _maxDuration * 1 days;
    }

    function calculateReward(uint256 _amount, uint256 _duration) 
    public 
    view returns (uint256 _lockReward) {   
        uint256 multiplier = durationMultiplier[_duration];
        if (multiplier == 0){
            multiplier = 10;
        }
        uint256 effectiveAPY = multiplier.mul(apy).mul(_duration).mul(1e10).div(yearDuration).div(10); 
        _lockReward = effectiveAPY.mul(_amount).div(1e12);
    }

    /**
     * @dev Locks specified amount of tokens against an address,
     *      for a specified time
     * @param _amount Number of tokens to be locked
     * @param _duration Lock duration in days
     */
    function lockedStake(uint256 _amount, uint256 _duration) external {
        _stake(msg.sender, _amount, _duration);
    }
    
    /**
     * @dev Locks a specified amount of tokens for other user,
     *      for a specified time
     * @param _user Address for which tokens need to be delegate Staked
     * @param _amount Number of tokens to be transferred and locked
     * @param _duration Lock time in days
     */
    function delegateLockedStaking(address _user, uint256 _amount, uint256 _duration) external {
        _stake(_user, _amount, _duration);
    }
    ...
    function _stake(address _user, uint256 _amount, uint256 _duration) internal {
        ...
        uint256 lockReward = calculateReward(_amount, _duration);

        komToken.transferFrom(msg.sender, address(this), _amount);
        locks[_user].push(TokenLock(_amount, matureUntil, lockReward, false));
        communityStaked = communityStaked.add(_amount);
        if(getUserStakedTokens(_user) > 3000*1e8 && komvToken.balanceOf(_user) == 0){
            komvToken.mint(_user, 1);
        }
        emit Locked(_user, _amount, lockReward, matureUntil);
    }
    ...
    function unlock() external returns (uint256) {
        ...
        uint256 locksLength = locks[_user].length;
        for (uint256 i = 0; i < locksLength; i++) {
            if (locks[_user][i].maturity <= block.timestamp && !locks[_user][i].claimed) {
                unlockableTokens = unlockableTokens.add(locks[_user][i].amount).add(locks[_user][i].reward);
                unlockablePrincipalStakedAmount = unlockablePrincipalStakedAmount.add(locks[_user][i].amount);
                locks[_user][i].claimed = true;
            }
        }

        require(unlockableTokens > 0, "Nothing to draw");
        communityStaked = communityStaked.sub(unlockablePrincipalStakedAmount);
        if(getUserStakedTokens(_user) < 3000*1e8 && komvToken.balanceOf(_user) > 0){
            komvToken.burn(_user, 1);
        }
        komToken.transfer(_user, unlockableTokens);
        emit Unlocked(_user, unlockableTokens);
     ...
     */
    function preMatureWithdraw() external returns (uint256) {
        uint256 unlockableTokens;
        ...
        require(unlockableTokens > 0, "Nothing to draw");
        uint256 withdrawableAmount = unlockableTokens;

        if(penaltyFeesPercentage > 0){
            uint256 remainingFeePercentage = 100 - penaltyFeesPercentage;
    ...
    function emergencyWithdraw() external returns (uint256) {
        require(isEmergency, "Not an emergency");
        uint256 totalStakedTokens = getUserStakedTokens(msg.sender);
        require(totalStakedTokens > 0, "Nothing to draw");
        communityStaked = communityStaked.sub(totalStakedTokens);
        if(getUserStakedTokens(msg.sender) < 3000*1e8 && komvToken.balanceOf(msg.sender) > 0){
            komvToken.burn(msg.sender, 1);
        }
        komToken.transfer(msg.sender, totalStakedTokens);
        emit EmergencyUnlocked(msg.sender, totalStakedTokens);
     ...
     * @param _duration Duration to set multiplier for in days.
     */
    function setDurationMultiplier(uint256 _multiplier, uint256 _duration) external onlyOwner {
        uint256 durationInSecs = _duration.mul(86400);
        require(durationInSecs <= maxDuration && durationInSecs >= minDuration , "Duration should be within max-min range");
        durationMultiplier[durationInSecs] = _multiplier;
    }
    
    /**
     * @dev Change minimum and maximum lock durations.
     * @param _minDuration Value of minimum lock duration in days.
     * @param _maxDuration Value of maximum lock duration in days.
     */
    function updateLockDuration(uint256 _minDuration, uint256 _maxDuration) external onlyOwner {
        require(_minDuration < _maxDuration, "Maximum lock duration has to exceed minimum lock duration!");
        minDuration = _minDuration * 1 days;
        maxDuration = _maxDuration * 1 days;
    }

    function updatePenaltyFees(uint256 _feesPercentage) external onlyOwner{
    ...
    function emergencyEnable(bool _isEmergency) external onlyOwner{
