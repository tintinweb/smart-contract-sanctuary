...
contract Visitors is ERC721, ERC721Enumerable, Ownable, Pausable {
  ...
  uint256 public totalVisitors = 0;
  ...
  function _mintTokens(uint256 numberOfTokens) internal {
    for (uint256 i = 0; i < numberOfTokens; i++) {
      _mint(msg.sender, totalVisitors + 1);
      totalVisitors += 1;
    }
  }
  ...
  * we surpass MAX_MINTABLE Visitors the mintTokens is
  ...
  */
  modifier mintCountMeetsSupply(uint256 numberOfTokens) {
    require(
      totalVisitors + numberOfTokens <= MAX_MINTABLE,
  ...
  modifier meetsRecyclingSupply() {
    require(
      recycleSupply > 0,
      "There are no Visitors available via recycling"
    );
    _;
  }
  ...
  modifier meetsRecyclingCost(uint256[] memory tokenIds) {
    require(
      ...
      "Incorrect number of Visitors passed for recycling"
    );
    _;
  }
  ...
  modifier meetsOwnership(uint256[] memory tokenIds) {
    for (uint256 i = 0; i < tokenIds.length; i++) {
      require(this.ownerOf(tokenIds[i]) == msg.sender, "You may only recycle Visitors you own");
    }
    _;
  }
