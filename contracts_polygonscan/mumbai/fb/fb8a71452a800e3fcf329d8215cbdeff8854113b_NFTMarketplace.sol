...
contract NFTMarketplace is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable, UUPSUpgradeable {
    ...
    function buyItem(address tokenAddress, uint tokenId) payable public nonReentrant {
        (Item memory item, bytes32 orderHash) = _getOrder(tokenAddress, tokenId);

        // TODO: rework when it is decided how to handle fees and everything
        ...
        if (token.supportsInterface(ROYALTIES_INTERFACE_ID)) {
            ...
            if (royaltiesPercentage > 0) {
                ...
                payable(item.owner).transfer(msg.value - royaltyValue);
            } else {
                // only pay seller
