...
contract MarginTrading is Ownable{
    ...
    mapping(address => uint256) public loanAmountLocked;
    // Event for deposit
    ...
    function deposit(address tokenAddress, uint256 amount, uint256 leverage) external payable {
        ...
        loanAmountLocked[tokenAddress] = loanAmountLocked[tokenAddress].add(depositInfo.loanAmount);
    ...
    function createPositionandTrade(address tokenAddress, address targetToken, uint256 amount) external {
        ...
        depositInfo.lockedMargin = depositInfo.lockedMargin.add(amount.mul(20).div(100));
        loanAmountLocked[tokenAddress] = loanAmountLocked[tokenAddress].sub(amount.mul(80).div(100));
    ...
    function trade(uint256 tradeAmount, address[] memory path) internal returns(uint256, uint256) {
        ...
        uint deadline = block.timestamp + tradeDeadline;
        uint[] memory price = uniswapV2Router.getAmountsOut(tradeAmount, path);
        if(path[0] == uniswapV2Router.WETH()) {
            // Trade and get asset out
            result = uniswapV2Router.swapExactETHForTokens{value: tradeAmount}(0, path, address(this), deadline);
        }
        else if(path[1] == uniswapV2Router.WETH()){
            // Approve token in
            IERC20(path[0]).approve(address(uniswapV2Router), tradeAmount); 
            // Trade and get asset out    
            result = uniswapV2Router.swapExactTokensForETH(tradeAmount, 0, path, address(this), deadline);
        }
        else {
            // Approve token in
            IERC20(path[0]).approve(address(uniswapV2Router), tradeAmount);
            // Trade and get asset out
            result = uniswapV2Router.swapExactTokensForTokens(tradeAmount, 0, path, address(this), deadline);
        }
    ...
    function closePosition(uint256 _positionID) public payable {
        ...
        (uint256 _result, uint256 tradePrice) = trade(positionInfo.targetAmount, path);

        positionInfo.reverseAmount = _result;
        positionInfo.closedPrice = tradePrice;
        ...
        loanAmountLocked[path[1]] = loanAmountLocked[path[1]].add(_result.mul(80).div(100));
        depositInfo.lockedMargin = depositInfo.lockedMargin.sub(positionInfo.assetAmount.mul(20).div(100));
        emit closeTrade(_positionID);
    }

    function withdraw(address tokenAddress, uint256 amount) external payable {
        Deposit storage depositInfo = deposits[msg.sender][tokenAddress];
        require(depositInfo.assetAmount.sub(depositInfo.usedMargin) > 0, "Withdraw call : Insufficient Deposit");    
        uint256 withdrawAmount;
        accureInterest(tokenAddress);
        uint256 _interest = depositInfo.interestAccumulated;
        uint256 leverage = depositInfo.assetAmount.div(depositInfo.loanAmount);
        ...
        depositInfo.timestamp = block.number;  
        loanAmountLocked[tokenAddress] = loanAmountLocked[tokenAddress].sub(amount*leverage); 
    }
    ...
    function liquidateOpenPosition(uint256 _positionID, address user) external onlyOwner {
        Position storage positionInfo = positionIDs[user][_positionID];
        require(positionInfo.status == 0, "Liquidate Position : Invalid Position");
    ...
    function withdrawInterestAccumulated(address tokenAddress) external onlyOwner {
        if(tokenAddress == address(0)) {
    ...
    function liquidateMargin(address userAddress, address tokenAddress) external onlyOwner {
        Deposit storage depositInfo = deposits[userAddress][tokenAddress];
        require(depositInfo.usedMargin == 0, "Liquidate Margin : Close postion first");
        userbalances[address(this)][tokenAddress] = userbalances[address(this)][tokenAddress].add(depositInfo.assetAmount);
        depositInfo.assetAmount = 0;
        depositInfo.loanAmount = 0;
        depositInfo.leverage = 0;
    }
}