...
// File: DailyRewards/metatx/EIP712Base.sol
...
// File: DailyRewards/metatx/EIP712MetaTransaction.sol
...
contract StarChestRewardsTracker is Ownable, Pausable, EIP712MetaTransaction {
     ...
     */
    mapping(address => string) public _dailyUserUuid;

    // mapping(address => mapping(uint256 => string)) public _dailyUserTimestampUuid;
    ...
    function checkDailyUuidValidity(string memory uniqueUuid) public view returns (bool){
        if (keccak256(abi.encodePacked(_dailyUserUuid[_msgSender()])) == keccak256(abi.encodePacked(uniqueUuid))) {
            return false;
        }
        return true;
    }
     ...
     */
    function dailyLog(string calldata uniqueUuid) public whenNotPaused {
        require(checkDailyTimestampValidity(), "Checkin already done.");
        require(checkDailyUuidValidity(uniqueUuid), "Checkin already done.");

        address msgSender = _msgSender();
        _dailyTrackerTimestampDays[msgSender] = currentDaysTimestamp();
        _dailyUserUuid[msgSender] = uniqueUuid;


        // _dailyUserTimestampUuid[msgSender][block.timestamp] = uniqueUuid;
