...
contract KiBridgeV15 is KiBridgeV1Factory {
  ...
  struct UserInfo {
    ...
    bytes32[] peningLiquidityRequestQueue;
  }
  struct TokenInfo {
    bool transferable;
    bool liquidable;
    ...
    private
  {
    ...
    userInfo[msg.sender].peningLiquidityRequestQueue.push(requestId);
    emit OracleRequest(
      _specId,
      msg.sender,
      requestId,
  ...
  external
  {
    ...
    require(userInfo[msg.sender].peningLiquidityRequestQueue[_index] == _requestId, "RequestId not in the queue");
    delete userInfo[msg.sender].peningLiquidityRequest[_requestId];
    bytes32 requestId = userInfo[msg.sender].peningLiquidityRequestQueue[userInfo[msg.sender].peningLiquidityRequestQueue.length - 1];
    userInfo[msg.sender].peningLiquidityRequestQueue[_index] = requestId;
    userInfo[msg.sender].peningLiquidityRequestQueue.pop();
    ...
    returns (bool success)
  {
    ...
    uint256 index;
    for(uint256 i = 0; i < userInfo[_user].peningLiquidityRequestQueue.length; i++) {
      if(userInfo[_user].peningLiquidityRequestQueue[i] == _requestId) {
        index = i;
      }
    }
    require(userInfo[_user].peningLiquidityRequestQueue[index] == _requestId, "RequestId not in the queue");
    bytes32 requestId = userInfo[_user].peningLiquidityRequestQueue[userInfo[_user].peningLiquidityRequestQueue.length - 1];
    userInfo[_user].peningLiquidityRequestQueue[index] = requestId;
    userInfo[_user].peningLiquidityRequestQueue.pop();
    if(_calAmount == 0) {
    ...
    external 
  {
    ...
    require(userInfo[msg.sender].peningLiquidityRequestQueue.length <= 10, "Exceeded number of requests");
  ...
  function getUserPeningLiquidityRequestId(
    address _user, 
    uint256 _index
  )
    external 
  ...
  {
    return userInfo[_user].peningLiquidityRequestQueue[_index];
  }
  function getUserPeningLiquidityRequestQueueLength(
    address _user
  )
    external 
    view 
    returns (uint256 length) 
  {
    return userInfo[_user].peningLiquidityRequestQueue.length;
  }
