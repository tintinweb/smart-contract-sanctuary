...
contract Governance is Config {
    ...
    event TokenBridgeableUpdate(address indexed token, bool bridgeable);
    ...
    /// @notice Bridgeable tokens list
    mapping(uint16 => bool) public bridgeableTokens;
    ...
    /// @notice Set token bridgeable
    /// @param _tokenAddr Token address
    /// @param _tokenBridgeable Token bridgeable status
    function setTokenBridgeable(address _tokenAddr, bool _tokenBridgeable) external {
        requireGovernor(msg.sender);

        uint16 tokenId = this.validateTokenAddress(_tokenAddr);
        if (bridgeableTokens[tokenId] != _tokenBridgeable) {
            bridgeableTokens[tokenId] = _tokenBridgeable;
            emit TokenBridgeableUpdate(_tokenAddr, _tokenBridgeable);
        }
    }
