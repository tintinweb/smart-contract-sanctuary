...
contract BDay is Ownable {
    ...
    function addGift(uint _id, string memory _pass, address _claimer, string memory _data) public payable onlyOwner  {
        ...
        gifts.push(Gift(_id, msg.value, _pass, new address[](0), _data));
        Gift memory gift = getGift(_id);
        gift.claimers[gift.claimers.length] = _claimer;
        
    }
    
    function getGift(uint _id) private view returns(Gift memory){
    ...
    function claimGift(string memory _pass)  public payable returns(string memory) {
        for(uint i=0; i < gifts.length; i++) {
            ...
            bool found = false;
            for (uint j = 0; j < gift.claimers.length; j++) {
                if(gift.claimers[j] == msg.sender)
                    found = true;
            }
            ...
            if(keccak256(abi.encodePacked((gift.pass))) == keccak256(abi.encodePacked((_pass)))) {
                ...
                if(gift.am > 0) {
                    ...
                    return "Congrats, check your wallet";
