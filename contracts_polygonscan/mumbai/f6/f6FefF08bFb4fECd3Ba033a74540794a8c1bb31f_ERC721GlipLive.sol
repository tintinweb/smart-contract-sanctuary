    ...
    RoyaltiesV2Impl
{
        ...
        returns (IGlipERC721LazyData.DecodedMintData memory)
    {
        ...
        uint96 fee = IMinterUpgradeable(minter).getFee(token, data.creators[0].account, signer);
    ...
    function mintAndTransfer(
        ...
        IMinterUpgradeable(minter).getFee(address(this), data.creators[0].account, signer);
...
library LibERC721LazyMint {
    ...
    struct Mint721Data {
        ...
        bytes signature; // TODO change
    }

    bytes32 public constant MINT_AND_TRANSFER_TYPEHASH =
        keccak256("Mint721(uint256 tokenId,uint256 reserve,string tokenURI,address minter,Part[] creators,Part[] royalties)Part(address account,uint96 value)");
