...
contract BaldrCore is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of BALDRs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accBaldrPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accBaldrPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. BALDRs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that BALDRs distribution occurs.
        uint256 accBaldrPerShare;   // Accumulated BALDRs per share, times 1e12. See below.
    ...
    // BALDR tokens created per block.
    uint256 public baldrPerBlock;
    // Bonus muliplier for early baldr makers.
    ...
    ILocker baldrLocker;
    // Locker adresse
    address public baldrLockerAddress;
    ...
    // The block number when BALDR mining starts.
    uint256 public startBlock;
    // Total locked up rewards
    uint256 public totalLockedUpRewards;

    // Baldr referral contract address.
    IReferral public baldrReferral;
    ...
    event BaldrReferralUpdated(address indexed user, IReferral newAddress);
    event BaldrLockerUpdated(address indexed user, ILocker newAddress);
    ...
    constructor(
        BaldrToken _baldr,
        uint256 _startBlock,
        uint256 _baldrPerBlock,
        address _baldrLockerAddress
    ) public {
        baldr = _baldr;
        startBlock = _startBlock;
        baldrPerBlock = _baldrPerBlock;
        ...
        baldrLockerAddress = _baldrLockerAddress;
        baldrLocker = ILocker(_baldrLockerAddress);
    }
    ...
    // Update the given pool's BALDR allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending BALDRs on frontend.
    function pendingBaldr(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accBaldrPerShare = pool.accBaldrPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 baldrReward = multiplier.mul(baldrPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accBaldrPerShare = accBaldrPerShare.add(baldrReward.mul(1e12).div(lpSupply));
        }
        uint256 pending = user.amount.mul(accBaldrPerShare).div(1e12).sub(user.rewardDebt);
        return pending.add(user.rewardLockedUp);
    }

    // View function to see if user can harvest BALDRs.
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 baldrReward = multiplier.mul(baldrPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        baldr.mint(devAddress, baldrReward.mul(100).div(1000));
        // Automatically burn 2% of minted tokens
        baldr.mint(BURN_ADDRESS, baldrReward.mul(20).div(1000));
        // Automatically mint some BALDR for the lottery pot
        if (address(lotteryAddress) != address(0) && lotteryMintRate > 0) {
            baldr.mint(lotteryAddress, baldrReward.mul(lotteryMintRate).div(10000));
        }        
        baldr.mint(address(this), baldrReward);
        pool.accBaldrPerShare = pool.accBaldrPerShare.add(baldrReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for BALDR allocation.
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        if (_amount > 0 && address(baldrReferral) != address(0) && address(baldrReferral) != BURN_ADDRESS && _referrer != address(0) && _referrer != BURN_ADDRESS && _referrer != msg.sender) {
            baldrReferral.recordReferral(msg.sender, _referrer);
        }
        payOrLockupPendingBaldr(_pid,false);
        if (_amount > 0) {
            pool.lpToken.safeTransferFrom(address(msg.sender), address(this), _amount);
            if (address(pool.lpToken) == address(baldr)) {
                uint256 burnTax = _amount.mul(baldr.burnRateTax()).div(10000);
        ...
        user.rewardDebt = user.amount.mul(pool.accBaldrPerShare).div(1e12);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        updatePool(_pid);
        payOrLockupPendingBaldr(_pid,true);
        if (_amount > 0) {
    ...
    // Pay or lockup pending BALDRs.
    function payOrLockupPendingBaldr(uint256 _pid, bool _isWithdrawal) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        uint256 pending = user.amount.mul(pool.accBaldrPerShare).div(1e12).sub(user.rewardDebt);
        ...
        if (_isWithdrawal) {
            ...
            if (noWithdrawFee(_pid, msg.sender)==false) {
                ...
                safeBaldrTransfer(BURN_ADDRESS, withdrawalfeeamount);     
            }
        ...
        if (canHarvest(_pid, msg.sender)) {
            if (pending > 0 || user.rewardLockedUp > 0) {
                ...
                if (address(baldrLocker) != address(0)){
                    uint256 startReleaseBlock = ILocker(baldrLocker).getStartReleaseBlock();
                    if (lockerRate > 0 && block.number < startReleaseBlock) {
                        ...
                        IBEP20(baldr).safeIncreaseAllowance(address(baldrLockerAddress), _lockerAmount);
                        ILocker(baldrLocker).lock(msg.sender, _lockerAmount); 
                    }
                }
                    

                // send rewards 
                safeBaldrTransfer(msg.sender, totalRewards);
    ...
    // Safe baldr transfer function, just in case if rounding error causes pool to not have enough BALDRs.
    function safeBaldrTransfer(address _to, uint256 _amount) internal {
        uint256 baldrBal = baldr.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > baldrBal) {
            transferSuccess = baldr.transfer(_to, baldrBal);
        } else {
            transferSuccess = baldr.transfer(_to, _amount);
        }
        require(transferSuccess, "safeBaldrTransfer: transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _baldrPerBlock) public onlyOwner {
        massUpdatePools();
        emit EmissionRateUpdated(msg.sender, baldrPerBlock, _baldrPerBlock);
        baldrPerBlock = _baldrPerBlock;
    }

    // Update the baldr referral contract address by the owner
    function setBaldrReferral(IReferral _baldrReferral) public onlyOwner {
        baldrReferral = _baldrReferral;
        emit BaldrReferralUpdated(msg.sender, _baldrReferral);
    }
    ...
    // Update the baldr locker contract address by the owner
    function setBaldrLocker(ILocker _baldrLocker) public onlyOwner {
        baldrLocker = _baldrLocker;
        emit BaldrLockerUpdated(msg.sender, _baldrLocker);
    }   
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (address(baldrReferral) != address(0) && referralCommissionRate > 0) {
            address referrer = baldrReferral.getReferrer(_user);
            uint256 commissionAmount = _pending.mul(referralCommissionRate).div(10000);

            if (referrer != address(0) && referrer != BURN_ADDRESS && commissionAmount > 0) {
                baldr.mint(referrer, commissionAmount);
                baldrReferral.recordReferralCommission(referrer, commissionAmount);
