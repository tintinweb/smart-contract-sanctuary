...
contract Scholarship is Initializable, AccessControlUpgradeable, PausableUpgradeable  {
    ...
    event RacerAssignEvent(bool _event, address user, uint tokenId, uint timestamp);
    ...
    function assignToManager(address _manager, uint _tokenId) public onlyOwner(_tokenId) {
        require (holders[_tokenId].manager == address(0), "Racer is already assigned");
        holders[_tokenId] = Holder(_manager, address(0), _tokenId);
        emit RacerAssignEvent(true, _manager, _tokenId, block.timestamp);
    }
    ...
    function withdrawFromAll(uint _tokenId) public onlyOwner(_tokenId) {
        ...
        emit RacerAssignEvent(false, _manager, _tokenId, block.timestamp);
    }

    function assignToScholar(address _scholar, uint _tokenId) public ownerOrManager(_tokenId) {
        require (holders[_tokenId].manager != address(0), "Racer is not assigned to any manager. Please assign a manager first.");
        holders[_tokenId].scholar = _scholar;
        emit RacerAssignEvent(true, _scholar, _tokenId, block.timestamp);
    }

    function withdrawFromScholar(uint _tokenId) public ownerOrManager(_tokenId) {
        require (holders[_tokenId].scholar != address(0), "Racer is not assigned to any scholar. Please assign a scholar first.");
        address _scholar = holders[_tokenId].scholar;
        holders[_tokenId].scholar = address(0);
        emit RacerAssignEvent(false, _scholar, _tokenId, block.timestamp);
    }
