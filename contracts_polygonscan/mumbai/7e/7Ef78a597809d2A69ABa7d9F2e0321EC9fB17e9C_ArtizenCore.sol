...
// TODO run through each function from auditor POV
// TODO check function permissions make sense with Artizen team
/**
    @title ArtizenCore
 */
contract ArtizenCore is IArtizenCore, Ownable {
    struct Grant {
        ...
        bool cancelled; //TODO go through all functions to see this is used in all correctly
    }
    ...
    */
    ...
    function renewGrant(
        ...
        uint256 _startTime,
        uint256 _endTime
    ) external override notShutdown onlyOwnerOrGrantAdmins(_grantID) {
        ...
        require(
            ...
            "ART_CORE: ADMIN FEES NOT CLAIMED"
        );
    ...
    function cancelGrant(uint256 _grantID)
        ...
        onlyOwnerOrGrantAdmins(_grantID)
    {
        // Revert if already cancelled
        require(!grants[_grantID].cancelled, "ART_CORE: ALREADY CANCELLED");
        // Cannot cancel after grant has ended
        require(
            block.timestamp < grants[_grantID].endTime,
    ...
    function claimRefundFromCancelledGrant(uint256 _grantID) external override {
        // Can't claim refunds unless grant is cancelled
        require(grants[_grantID].cancelled, "ART_CORE: GRANT NOT CANCELLED");
        require(
            grantDonationsMap[_grantID][msg.sender] > 0,
            "ART_CORE: NO REFUNDABLE BALANCE"
        );

        uint256 _amount = grantDonationsMap[_grantID][msg.sender];
        grantDonationsMap[_grantID][msg.sender] = 0;
        grants[_grantID].totalDonations -= _amount;

        _refundDonation(msg.sender, _amount);

        emit RefundClaimed(msg.sender, _grantID, _amount);
    }
    ...
    */
    function _donate(
        ...
        DAI.transferFrom(msg.sender, address(this), _amountDAIDonated);
    ...
    */
    ...
    function _payOutGrantAdminFees(uint256 _grantID) internal {
        require(
            !grants[_grantID].adminFeeClaimed,
    ...
    {
        require(
            !grantProjects[_grantID][_projectID].donationsClaimed,
            "ART_CORE: FUNDS ALREADY CLAIMED"
        );
        require(!grants[_grantID].cancelled, "ART_CORE: GRANT CANCELLED");
        require(
            block.timestamp > grants[_grantID].endTime,
    ...
    // TODO refactor to private functions to save gas
    modifier onlyProjectOwners(uint256 _projectID) {
        require(
            isOwnerOfProject[_projectID][msg.sender],
            "ART_CORE: NOT PROJECT OWNER"
        );
        _;
    }

    modifier onlyGrantAdmins(uint256 _grantID) {
        require(
            isAdminOfGrant[_grantID][msg.sender],
            "ART_CORE: NOT GRANT ADMIN"
        );
        _;
    }
...
interface IArtizenCore {
    ...
    function claimRefundFromCancelledGrant(uint256 _grantID) external;
