...
contract Exchange is ExchangeCore, TransferManager {
    function __Exchange_init(
        ...
        uint newDefaultSellerProtocolFee,
        uint newDefaultBuyerProtocolFee,
        ...
        __TransferManager_init_unchained(newDefaultSellerProtocolFee, newDefaultBuyerProtocolFee, newDefaultFeeReceiver, newRoyaltiesProvider);
...
import "@orderinbox/transfer-proxy/contracts/transfer/TransferExecutor.sol";
...
abstract contract TransferManager is OwnableUpgradeable, ITransferManager {
    ...
    ProtocolFees defaultProtocolFees;
    mapping(address => ProtocolFees) public protocolFees;
    ...
    function __TransferManager_init_unchained(
        ...
        defaultProtocolFees = ProtocolFees(newDefaultSellerProtocolFee, newDefaultBuyerProtocolFee, true);
    ...
    function setDefaultProtocolFees(uint newDefaultSellerProtocolFee, uint newDefaultBuyerProtocolFee) external onlyOwner {
        defaultProtocolFees = ProtocolFees(newDefaultSellerProtocolFee, newDefaultBuyerProtocolFee, true);
    }
    ...
    function setProtocolFeesFor(address operator, uint256 sellerFee, uint256 buyerFee) external onlyOwner {
        require(sellerFee + buyerFee <= 5000, "Protocol fees are too high (>50%)");

        protocolFees[operator] = ProtocolFees(
            sellerFee,
            buyerFee,
            true
        );
    ...
    function removeProtocolFeesFor(address operator) external onlyOwner {
        delete protocolFees[operator];
    }

    function getProtocolFeesFor(address operator) override internal view returns (uint256, uint256) {
        
        ProtocolFees memory operatorProtocolFees = protocolFees[operator];

        if(operatorProtocolFees.init) {
            return (operatorProtocolFees.sellerFee, operatorProtocolFees.buyerFee);
        }

        return (defaultProtocolFees.sellerFee, defaultProtocolFees.buyerFee);
    }

    function getProtocolFees() override internal view returns (uint256, uint256) {
        return getProtocolFeesFor(msg.sender);
    }
    ...
    function doTransfersWithFees(
        ...
        (uint256 protocolFeeNft, uint256 protocolFeeCalculate) = getProtocolFees(); // seller + buyer fees
        totalAmount = calculateTotalAmount(amount, protocolFeeCalculate, dataCalculate.originFees);
        uint rest = transferProtocolFee(totalAmount, amount, protocolFeeNft, protocolFeeCalculate, from, matchCalculate, transferDirection);
    ...
    function transferProtocolFee(
        ...
        uint amount, 
        uint protocolFeeNft,
        uint protocolFeeCalculate,
        address from,
        LibAsset.AssetType memory matchCalculate,
        bytes4 transferDirection
    ) internal returns (uint) {
        (uint rest, uint fee) = subFeeInBp(totalAmount, amount, protocolFeeNft + protocolFeeCalculate);
        if (fee > 0) {
...
abstract contract ITransferManager is ITransferExecutor {
    ...
    function getProtocolFeesFor(address operator) internal view virtual returns (uint256, uint256);
    function getProtocolFees() internal view virtual returns (uint256, uint256);
...
abstract contract ExchangeCore is Initializable, OwnableUpgradeable, AssetMatcher, TransferExecutor, OrderValidator, IExchange, ITransferManager {
    using SafeMathUpgradeable for uint;
    ...
    function getExchangeFeesFor(address operator) override external view returns (uint256, uint256) {        
        return getProtocolFeesFor(operator);
    }

    function getExchangeFees() override external view returns (uint256, uint256) {
        return getProtocolFees();
    }
...
abstract contract TransferExecutor is Initializable, OwnableUpgradeable, ITransferExecutor {
    ...
    function getTransferProxy(bytes4 assetType) internal view returns (address) {
