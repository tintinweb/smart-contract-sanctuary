...
contract SuperNFT is ERC721, ReentrancyGuard, KeeperCompatibleInterface, VRFConsumerBase {
    ...
    Winner[] public winners; // TODO map?

    event NewWinner(uint256 id, address owner, uint256 timestamp);
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override nonReentrant {
        ...
        winners.push(Winner(winnerId, _counter + 1, recipient, block.timestamp));
        _superToken.transfer(recipient, 1 * 10**18);
        emit NewWinner(winnerId, recipient, block.timestamp);
    }
}
