...
contract Owned is Context {
    ...
    modifier onlyOwner {
        require(_msgSender() == _owner, "USDTToken: Only Owner can perform this task");
        _;
    }
    ...
    function acceptOwnership() public {
        require(_msgSender() == _newOwner, "USDTToken: Token Contract Ownership has not been set for the address");
 ...
 *
 */
contract USDTToken is IBEP20, Owned {
    ...
    function approve(address spender, uint256 amount) public override returns (bool) {
        ...
        require(spender != address(0), "USDTToken: approve to the zero address");
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        ...
        require(amount <= _allowances[sender][_msgSender()],"USDTToken: Check for approved token count failed");
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        require(recipient != address(0),"USDTToken: Cannot have recipient as zero address");
        require(sender != address(0),"USDTToken: Cannot have sender as zero address");
        require(_balances[sender] >= amount,"USDTToken: Insufficient Balance" );
        require(_balances[recipient] +amount >= _balances[recipient],"USDTToken: Balance check failed");
    ...
    function burn(address account, uint256 amount) public onlyOwner {
        ...
        require(account != address(0), "USDTToken: burn from the zero address");

        if( _balances[account] == _unlockedTokens[account]){
            _unlockedTokens[account] = _unlockedTokens[account].sub(amount, "USDTToken: burn amount exceeds balance");
        }

        _balances[account] = _balances[account].sub(amount, "USDTToken: burn amount exceeds balance");
        ...
        if(account != _msgSender()){
            ...
            require(amount <= _allowances[account][_msgSender()],"USDTToken: Check for approved token count failed");
    ...
    function _updateUnLockedTokens(address _from, uint256 tokens) private returns (bool success) {
        ...
        // if _unlockedTokens are less than "tokens" of "to", update _unlockedTokens by checking record with "now" time
            ...
            // otherwise revert
                ...
                revert("USDTToken: Insufficient unlocked tokens");
            }
        }
    }
