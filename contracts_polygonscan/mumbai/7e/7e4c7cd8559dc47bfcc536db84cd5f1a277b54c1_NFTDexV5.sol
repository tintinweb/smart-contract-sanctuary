...
//   event SetValue(address indexed from, uint256 main, uint256 _author, uint256 _platform);
  ...
  function sellNFT(address nft_a,uint256 tokenId, address seller, uint256 price, uint256 priceDollar) public{
    ...
    require(_minPrice > 0 || priceDollar > 0, "100");
  ...
  function placeBid(uint256 _tokenId, uint256 _amount, bool awardType) public{
    ...
    require(temp.minPrice > 0 || _dollarPrice[_tokenId] > 0,"104");
    require(_amount > 0 && (temp.minPrice < _amount || _dollarPrice[_tokenId] < _amount), "105");
  ...
  function claimAuction(uint256 _tokenId, bool awardType) public {
    ...
    if(!awardType) {  
      ...
      price = SafeMath.div(SafeMath.mul(_auctionTokensBids[_tokenId][msg.sender].bidAmount,reserve1),SafeMath.mul(reserve0,1000000000000));

      ALIA.transferFrom(msg.sender, _auctionTokens[_tokenId].seller, (price  / 1000) * mainPerecentage); 
      ALIA.transferFrom(msg.sender, _tokenAuthors[_tokenId], (price  / 1000) * authorPercentage);  
