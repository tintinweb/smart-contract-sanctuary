    ...
    MinimalForwarderUpgradeable
{
    ...
    function initialize(
        ...
        MinimalForwarderUpgradeable forwader
    ) public initializer {
        require(
        ...
        __Ownable_init();
        AdminRole._initializeAdminRole(_msgSender());
        __MinimalForwarder_init();
        __ERC2771Context_init(address(forwader));
        _MAX_NUM_TOKENS = _maxNumTokens;
        _RESERVED_NUM_TOKENS = _reservedNumTokens;
        _MAX_NUM_TOKENS_PER_MINT = _maxNumTokensPerMint;
        _TOKEN_PRICE = _tokenPrice;
        _PUBLIC_MINTING_ENABLED = _publicMintingEnabled;
        _BASE_URI = _baseUri;
    ...
    function adminWithdrawAll() external onlyAdmin {
        ...
        _withdraw(address(_msgSender()), address(this).balance);
    }
    ...
    function adminMintTo(address _toAddress, uint8 _count) external onlyAdmin {
        ...
        for (uint8 i = 0; i < _count; i++) {
            _mintAnElement(_toAddress, address(_msgSender()));
    ...
    function mint(uint256 _count) public payable {
        ...
        for (uint256 i = 0; i < _count; i++) {
            _mintAnElement(address(_msgSender()), address(_msgSender()));
        }
    }
