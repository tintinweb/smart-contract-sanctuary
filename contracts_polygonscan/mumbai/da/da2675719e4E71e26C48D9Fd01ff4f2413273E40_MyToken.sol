...
contract MyToken is ERC1155Permit, Ownable {
    constructor(string memory _baseURI) ERC1155Permit(_baseURI) {}
...
abstract contract ERC1155Permit is ERC1155, IERC1155Permit, EIP712 {
    ...
    bytes32 private immutable _PERMIT_TYPEHASH =
        ...
        keccak256(
            "Permit(address owner,address operator,bool approved,uint256 nonce)"
        );
 ...
 * https://eips.ethereum.org/EIPS/eip-2612[EIP-2612].
 *
 * Adds the {setPermitForAll} method, which can be used to change an account's ERC1155 allowance by
...
interface IERC1155Permit {
    /**
     * @dev Sets `approved` for `operator` so that `operator` can spend ``owner``'s tokens,
     * given ``owner``'s approval as signaled via signature
     *
