...
contract Hanoux2 is Context, Ownable {
     ...
     */
    function release(address contractUSDC, uint256 amount) public virtual {
        // проверка на разрешение перевода
        require(
        ...
        for (uint256 index = 0; index < _payees.length; index++) {
            address addr = _payees[index];
            uint256 partAmount = amount * _shares[addr] / 100;

            // перечисление дивидентов от заказчика клиентам (предварительно заказчику нужно делать approve всей суммы)
    ...
    //  */
    // function transfer(address account, uint256 amount) public virtual returns (bool) {
