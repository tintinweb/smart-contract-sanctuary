...
contract ChubbyApe is ERC721Enumerable, Ownable {
    ...
    modifier onlyManufacturer() {
        require(
            _manufacturer == _msgSender(),
            "Caller is not the Manufacturer"
        );
        _;
    }

    constructor(address payable _newincomeWallet, address _newmanufacturer)
        ERC721("CAP", "CAPE")
    {
        _incomeWallet = _newincomeWallet;
        _manufacturer = _newmanufacturer;
    }
    ...
    function mintChubbyApe(uint256 numberOfTokens) public payable {
    ...
    //Only manufacturer
    function mintChubbyApeByManufacturer(uint256 numberOfTokens)
        external
        onlyManufacturer
    {
        require(saleIsActive, "Mint is not active");
    ...
    function setMAX_NFT_PURCHASE(uint256 _MAX_NFT_PURCHASE) external onlyOwner {
        MAX_NFT_PURCHASE = _MAX_NFT_PURCHASE;
    }

    /** _manufacturer settings*/
    function setManufacturer(address _newmanufacturer) external onlyOwner {
        _manufacturer = _newmanufacturer;
    }
}
