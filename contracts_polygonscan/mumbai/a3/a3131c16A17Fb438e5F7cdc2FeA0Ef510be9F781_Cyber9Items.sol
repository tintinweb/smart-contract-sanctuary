...
contract Cyber9Items is ERC1155, Ownable, ReentrancyGuard{
    ...
    uint256 public CdKitPrice = 500;
    uint256 public HPKitPrice = 250;

    ICyber9Token private cyber9token;

    constructor(string memory _baseURI, ICyber9Token _tokenAddr) ERC1155(_baseURI){
        baseURI = _baseURI;
        cyber9token = _tokenAddr; 
    }
    ...
    //Orb functions
    function mintOrbs(uint256 _amount) external nonReentrant{
        uint256 balance = cyber9token.balanceOf(msg.sender);
        require(balance > OrbPrice*_amount, "Insufficient funds");
        require(_amount > 0, "Amount must be higher than 0");

        cyber9token.burnBei(msg.sender, OrbPrice*_amount);
        _mint(msg.sender, ORB, _amount, "");
    }
    ...
    //HP kit functions
    function mintHpKit(uint256 _amount) external nonReentrant{
        uint256 balance = cyber9token.balanceOf(msg.sender);
        require(balance > HPKitPrice*_amount, "Insufficient funds");
        require(_amount > 0, "Amount must be higher than 0");

        cyber9token.burnBei(msg.sender, HPKitPrice*_amount);
        _mint(msg.sender, HP_KIT, _amount, "");
    }
    ...
    //Reset cooldown functions
    function mintCdKit(uint256 _amount) external nonReentrant{
        uint256 balance = cyber9token.balanceOf(msg.sender);
        require(balance > CdKitPrice*_amount, "Insufficient funds");
        require(_amount > 0, "Amount must be higher than 0");

        cyber9token.burnBei(msg.sender, CdKitPrice*_amount);
        _mint(msg.sender, CD_KIT, _amount, "");
    }

    function useCdKit(address _from) external nonReentrant validCaller{
        require(balanceOf(_from, CD_KIT) > 0, "Insufficient HP kit" );
