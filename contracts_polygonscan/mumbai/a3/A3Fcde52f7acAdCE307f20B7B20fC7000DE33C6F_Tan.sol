...
/// @notice Defines set of methods required by Tan contract after minting has been closed
...
contract Tan is
    ...
    /// @param _priceFeedEthUsdAddress    Chainlink ETH/USD price feed address
    /// @param _priceFeedMaticUsdAddress  Chainlink MATIC/USD price feed address
    /// @param _wEthContract              Polygon WETH contract address
    ...
    constructor(
            ...
            address                  _priceFeedEthUsdAddress,
            address                  _priceFeedMaticUsdAddress,
        ...
        PriceFeed(
            _priceFeedEthUsdAddress,
            _priceFeedMaticUsdAddress
        )
    ...
    uint constant public MAX_TANS_PER_TX  = 14;

    uint constant public PRICE_WL         = 2 * 1e16;
    uint constant public PRICE_PUBLIC     = 3 * 1e16;

    string private constant NOT_ENOUGH_MATIC_SENT = "Not enough MATIC sent";
    ...
    mapping (address => bool) public addressOGWLMinted;
    ...
    /// @notice Public mint method
    /// @dev Does not perform any checks
    /// @param numTans number of Tans to mint
    function mint(uint numTans) external payable
    {
        mintLocalMatic(numTans, PRICE_PUBLIC);
    }


    /// @notice Public mint method, paid in WETH
    /// @dev Does not perform any checks
    /// @param numTans number of Tans to mint
    function mintWETH(uint numTans) external
    {
        wEthContract.transferFrom(_msgSender(), address(this), numTans * PRICE_PUBLIC);
        mintLocal(numTans);
    }


    /// @notice Mint helper method
    /// @dev Checks state, number of tans per TX and paused. Does not check price or 1-time OG/WL mint
    /// @param numTans number of Tans to mint
        ...
        whenNotPaused()
    {
        ...
        uint curMintCounter = totalSupply();
        for (uint mintedTan = 1; mintedTan <= numTans; mintedTan++) {
            _mint(msgSender, curMintCounter + mintedTan);
        }
    }


    /// @notice Mint helper method for paid mints, returns surplus MATIC sent
    /// @dev Checks whether MATIC sent with TX is at least the current mint price in MATIC
    function mintLocalMatic(uint numTans, uint price) private
    {
        uint mintPriceInMatic = getPriceOfTansInMatic(numTans, price);
        require(
            mintPriceInMatic <= msg.value,
            NOT_ENOUGH_MATIC_SENT
        );

        mintLocal(numTans);

        returnSurplusMatic(mintPriceInMatic);
    }
    ...
    /// @notice OG mint method. Allowed once per OG minter, OG proof is by merkle proof
    ...
    /// @notice WL mint method. Allowed once per WL minter, WL proof is by merkle proof
    ...
    function mintWL(bytes32[] calldata merkleProof, uint numTans) external payable
        ...
        oneMint()
    {
        mintLocalMatic(numTans, PRICE_WL);
    }


    /// @notice WL mint method, paid with WETH. Allowed once per WL minter, WL proof is by merkle proof
    /// @param merkleProof Merkle proof of minter address for WL tree
    /// @param numTans     Number of Tans to mint
    function mintWLWETH(bytes32[] calldata merkleProof, uint numTans) external
        isValidMerkleProof(merkleRootWL, merkleProof)
        oneMint()
    {
        wEthContract.transferFrom(_msgSender(), address(this), numTans * PRICE_WL);
        mintLocal(numTans);
    }


    /// @notice Send <msg.value> - <mintPriceInMatic> back to the transaction sender
    /// @param mintPriceInMatic Mint price in MATIC
    function returnSurplusMatic(uint mintPriceInMatic) private {
        uint valueToReturn = msg.value - mintPriceInMatic;

        if (valueToReturn > 0) {
            (bool sent,) = _msgSender().call{value: valueToReturn }("");
            require(
                sent,
                "TX_FAILED"
            );
    ...
    /// @param numTans Number of Tans to mint
    modifier limitTans(uint numTans) {
        require(
            numTans >= 1 &&
            numTans <= MAX_TANS_PER_TX &&
    ...
    /// @notice Require maximum one OG/WL mint per address
    modifier oneMint() {
        address msgSender = _msgSender();

        require(
            addressOGWLMinted[msgSender] == false,
            "Only one OG/WL mint allowed"
        );

        addressOGWLMinted[msgSender] = true;
    ...
    /// @notice Change to generation closed state. Requests generation seed for next generation
    /// @dev Timestamp can be reset here, as this method cannot be called again when state == GENERATIONCLOSED; timeout mechanism takes over there
    ...
    /// @notice Change to generation closing state, as announced over the Tangram contract
    ...
    /// @notice Change to generation started state; called over VRF response from getting the new generation seed
    ...
    string private constant NO_FUNDS_TO_PAYOUT = "No funds to payout";
    string private constant TX_ERROR           = "Transaction error";

    /// @notice Pay out MATIC funds to the team wallet
    ...
    /// @notice Pay out WETH funds to the team wallet
    function payoutWETH() external
        onlyOwner()
    {
        ...
        bool sent = wEthContract.transfer(owner(), balance);
    ...
    /// @notice Receive secondary sales fees
    ...
    string private placeholderMetadataURI = "";

    /// @notice Get contract metadata URI
    /// @dev Used by OpenSea to automatically retrieve initial collection details
    ...
    /// @param _contractMetadataURI URI containing contract metadata
    function setContractMetadataURI(string memory _contractMetadataURI) external
        onlyOwner()
    {
    ...
    /// @notice Set placeholder metadata URI
    /// @param _placeholderMetadataURI URI containing placeholder metadata
    function setPlaceholderMetadataURI(string memory _placeholderMetadataURI) external
        onlyOwner()
    {
    ...
    ///  ___ ___ ___| |_ ___ ___ ___| |_   | | | |   __|_   _|  |  |
    /// |  _| . |   |  _|  _| .'|  _|  _|  | | | |   __| | | |     |
    /// |___|___|_|_|_| |_| |__,|___|_|    |_____|_____| |_| |__|__|

    IERC20 private immutable wEthContract;
}
...
///              _               _      _____     _         _____           _ 
///  ___ ___ ___| |_ ___ ___ ___| |_   |  _  |___|_|___ ___|   __|___ ___ _| |
/// |  _| . |   |  _|  _| .'|  _|  _|  |   __|  _| |  _| -_|   __| -_| -_| . |
/// |___|___|_|_|_| |_| |__,|___|_|    |__|  |_| |_|___|___|__|  |___|___|___|

/// @notice Tactical Tangrams PriceFeed contract
/// @author tacticaltangrams.io
/// @notice Implements the ETH to MATIC rate used for minting
contract PriceFeed {

    // TODO: Remove this before deployment. Only used to test with low tan costs due to the MATIC faucet only giving 1 MATIC on Mumbai
    uint private constant MATIC_PRICE_TEST_FACTOR = 1e10;
    ...
    /// @notice Initializes Chainlink price feed functionality
    /// @param priceFeedEthUsdAddress Contract address of ETH/USD price feed
    constructor(address priceFeedEthUsdAddress, address priceFeedMaticUsdAddress)
    {
        // Mainnet: 0xF9680D99D6C9589e2a93a78A04A279e509205945
        // Mumbai : 0x0715A7794a1dc8e42615F059dD6e406A6594651A
        priceFeedEthUsd   = AggregatorV3Interface(priceFeedEthUsdAddress);

        // Mainnet: 0xAB594600376Ec9fD91F8e885dADF0CE036862dE0
        // Mumbai : 0xd0D5e3DB44DE05E9F294BB0a3bEEaF030DE24Ada
        priceFeedMaticUsd = AggregatorV3Interface(priceFeedMaticUsdAddress);
    }


    /// @notice Gives the price for <numTans> Tans in MATIC
    /// @param numTans Number of Tans to get price for
    /// @param tanPrice Price per Tan in 0.01 ETH (3 for public mint, 2 for whitelist mint)
    /// @return priceOfTansInMatic Price of <numTans> in 100 MATIC (divide by 100)
    function getPriceOfTansInMatic(uint numTans, uint tanPrice) public view returns
        (
            uint priceOfTansInMatic
        )
    {
        (
            ,
        ...
        uint priceUsd = uint(priceEthUsd) * tanPrice * numTans;
        priceOfTansInMatic = priceUsd / uint(priceMaticUsd) / MATIC_PRICE_TEST_FACTOR;
    }


    AggregatorV3Interface private immutable priceFeedEthUsd;
    AggregatorV3Interface private immutable priceFeedMaticUsd;
}
...
abstract contract State {
    ...
    StateType public state = StateType.MINT;
...
abstract contract VRFD20 is VRFConsumerBase {
    ...
    /// @param _keyHash        Public key against which randomness is created
    /// @param _link           LINK token address
    constructor(address _vrfCoordinator, bytes32 _keyHash, address _link)
    ...
    /// @notice Chainlink fee in LINK for VRF; 0.0001 LINK for Mumbai and Mainnet
    uint private constant fee = 100000000000000;
