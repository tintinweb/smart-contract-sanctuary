...
contract ArtisFarm is IERC777Recipient {
    ...
    mapping(address => uint256) public artisBalance;
    ...
    */
    function tokensReceived(
        ...
        if (isStaking[from] == true) {
            ...
            artisBalance[from] = artisBalance[from].add(toTransfer);
        }
        artisBalance[from] = artisBalance[from].add(amount);
    ...
    function compoundYield(address staker) private {
        require(
            isStaking[staker] = true && artisBalance[staker] > 0,
            "Nothing to unstake"
        );
        uint256 yield = calculateYieldTotal(staker);
        startTime[staker] = block.timestamp;
        artisBalance[staker] = artisBalance[staker].add(yield);
        emit Compound(staker, yield);
    }

    function compoundBatch(address[] calldata stakers) external onlyAdmin {
    ...
    function unstake(uint256 amount) public {
        require(
            isStaking[msg.sender] = true && artisBalance[msg.sender] >= amount,
            "Nothing to unstake"
        );
        ...
        artisBalance[msg.sender] = artisBalance[msg.sender].sub(amount);
        artisToken.send(msg.sender, amount.add(yield), "");
        if (artisBalance[msg.sender] == 0) {
    ...
    function unstakeAll() external {
        require(artisBalance[msg.sender] > 0, "Nothing to unstake");
        uint256 amount = artisBalance[msg.sender];
        unstake(amount);
    }
    ...
    function calculateYieldTotal(address user) public view returns (uint256) {
        ...
        uint256 rate = 600; //10 min release 100% interest
        uint256 timeRate = time.div(rate);
        uint256 rawYield = (artisBalance[user].mul(timeRate).div(10**18));
