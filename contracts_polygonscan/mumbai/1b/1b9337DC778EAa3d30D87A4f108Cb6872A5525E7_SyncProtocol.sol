    ...
    ERC1967Upgrade
{
    ...
    function setFee(
        ...
        uint256 sequence = useSequence(msg.sender);
        require(nonce == sequence, "Invalid nonce");

        bytes32 inputHash = keccak256(abi.encode(newFee, nonce));
    ...
    function syncData(
        ...
        require(nonce == useNonce(), "Invalid nonce");

        bytes32 inputHash = keccak256(abi.encode("syncData", nonce, payload));
        _checkSignatures(sigV, sigR, sigS, inputHash);
    ...
    function useNonce() internal returns (uint256 nonce) {
        _nonce = _nonce + 1;
        return _nonce;
    }
