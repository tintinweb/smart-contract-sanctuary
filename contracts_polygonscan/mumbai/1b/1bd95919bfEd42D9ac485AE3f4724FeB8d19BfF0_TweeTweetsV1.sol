...
// File: contracts/TweeTweetsV1.sol
...
contract TweeTweetsV1 is Ownable, ERC721("TweeTweets", "TTT") {
    ...
    function tokenURI(uint256 tokenId) public view override(ERC721) returns (string memory) {
        ...
        return bytes(__baseURI).length > 0 ? string(abi.encodePacked(__baseURI, idstr, ".json")) : "";
    }

    function mintBatch(uint256 start, uint256 end, address to) external onlyOwner {
        require(start > 0);
        require(end < 1001);
    ...
    function batchTransfer(address[] calldata tos, uint256[] calldata ids) external {
