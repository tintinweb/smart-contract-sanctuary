        ...
        if (auctionId != 0) {
            ...
            if (auctionId != type(uint256).max) {
                ...
                (
                    ...
                    uint8 state,
                    bool approved
                ) = IArtAuction(artAuction).getArtAuctionInfo();
                ...
                if (state == 0) {
                    status.state = IBidDb.State.Cancel;
                } else if (state == 2) {
                    status.state = IBidDb.State.End;
                } else {
                    revert("no auctionId when state==1");
                }
            }
        }
    }
