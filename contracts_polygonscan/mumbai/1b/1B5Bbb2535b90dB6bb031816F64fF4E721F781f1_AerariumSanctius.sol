...
contract AerariumSanctius is Ownable {
  ...
  // Mapping for token to amount deposited
  mapping(IERC20 => uint256) public tokenAmounts;



  /** @dev - Constructor
   * @param _endBlock - The block after which the rewards can be withdrawn
   * @param _rwdToken - The reward token used to withdraw rewards
  ...
  function withdrawExcess(address _receiver) external onlyOwner {
    ...
    for (uint256 i = 0; i < tokensHeld.length; i++) {
      uint256 balance = tokensHeld[i].balanceOf(address(this));
      tokensHeld[i].safeTransfer(_receiver, balance);
    }
  }
...
contract Aureus is ERC20Capped {
  ...
  constructor(
    ...
    uint256 _cap,
    address _programAdministrator
  ) ERC20(_name, _symbol) ERC20Capped(_cap) {
    ERC20._mint(_programAdministrator, _cap);
  }
