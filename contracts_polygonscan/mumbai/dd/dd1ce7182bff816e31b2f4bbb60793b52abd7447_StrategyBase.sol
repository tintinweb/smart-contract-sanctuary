...
abstract contract StrategyFarmBase is BaseStrategyMasterChef {
    ...
    // Token addresses for TOKEN1
    address public  token1;
    address public  token2;
    address public multiHarvest = 0x0000000000000000000000000000000000000000;

    uint256 public underlyingPoolId;

    uint256 public keepFXS = 2000;
    uint256 public harvestCutoff = 10**12;

    uint256 public constant keepFXSmax = 10000;

    // Uniswap swap paths
    address[] public token1_token2_path;
    address[] public quick_token2_path;

    constructor(
        address _rewards,
        address _want,
        address _strategist,
        uint256 _underlyingPool,
        address _token1,
        address _token2,
        address _currentRouter
    )
        ...
        )
    {
        token1 = _token1;
        token2 = _token2;
        underlyingPoolId = _underlyingPool;
        token1_token2_path = new address[](2);
        token1_token2_path[0] = token1;
        token1_token2_path[1] = token2;
    }
    ...
    function harvest() public override {
        ...
        // Collects TOKEN2 tokens
        _getReward();

        //Swap 1/2 TOKEN2 for TOKEN1
        uint256 _token1Balance = IERC20(token1).balanceOf(address(this));
        if (_token1Balance > harvestCutoff){
            if (_token1Balance > 0) {
                _swapUniswapWithPath(token1_token2_path, _token1Balance.div(2));
            }

            // Add liquidity for Token1/Token2
            uint256 _token2 = IERC20(token2).balanceOf(address(this));
            uint256 _token1 = IERC20(token1).balanceOf(address(this));
            if (_token2 > 0 && _token1 > 0) {
                IERC20(token2).safeApprove(currentRouter, 0);
                IERC20(token2).safeApprove(currentRouter, _token2);
...
contract StrategyBase is StrategyFarmBase {
    // Token addresses
    address public SPADE_MASTER_CHEF;
    address public LP;
    string public pairName;
    constructor(address _masterchef,address _lp, string memory _pairName,uint256 _underlyingPoolId,address _token1,address _token2, address _currentRouter)
        public
        StrategyFarmBase(
            _masterchef,
            _lp,
