...
import "./interfaces/IPlayersArtRoot.sol";
import "./interfaces/IPlayersArtNFTSaleFactory.sol";
import "./PlayersArtNFTSale.sol";
import "./interfaces/IPlayersArtNFT.sol";

contract PlayersArtNFTSaleFactory is Ownable, IPlayersArtNFTSaleFactory {
    IPlayersArtRoot public root;

    PlayersArtNFTSale[] public sales;

    event SaleCreated(address creator, address sale, address nft, address token, uint256 tokenPerNft, string meta);

    modifier onlyOwnerOrAdmin() {
        require(msg.sender == owner() || root.isAdmin(msg.sender), "Not admin or owner");
        _;
    }

    constructor(IPlayersArtRoot _root) {
        require(address(_root) != address(0), "Invalid root");

        root = _root;
    }

    function isAdmin(address addr) external view override returns (bool) {
        return root.isAdmin(addr);
    }
    ...
    function deployPlayersArtNFTSale(
        IPlayersArtNFT nft,
        IERC20 token,
        uint256 tokenPerNft,
        string memory meta
    ) external onlyOwnerOrAdmin returns (address) {
        PlayersArtNFTSale sale = new PlayersArtNFTSale(this, nft, token, tokenPerNft, meta);

        sales.push(sale);

        sale.transferOwnership(msg.sender);

        emit SaleCreated(msg.sender, address(sale), address(nft), address(token), tokenPerNft, meta);
...
interface IPlayersArtNFTSaleFactory {
    function isAdmin(address) external view returns (bool);
}
...
contract PlayersArtNFTSale is Ownable {
    using SafeERC20 for IERC20;
    IPlayersArtNFTSaleFactory public factory;
    ...
    event DropNFTTokenIdInited(uint256 count);
    event NFTSold(address buyer, uint256 tokenId);

    event DropCancelled(bool cancelled);

    event DropEnded(uint256 endedTimestamp);

    event DropMetaChanged(string meta);

    modifier onlyOwnerOrAdmin() {
        require(msg.sender == owner() || factory.isAdmin(msg.sender), "Not admin or owner");
        _;
    }
    ...
    modifier onlyActiveSale() {
        require(
            block.timestamp >= startTime &&
                availableTokenIds.length == totalNFTCount - totalNFTSold &&
                availableTokenIds.length > 0 &&
                !cancelled,
            "Sale is not active now"
        );
    ...
    constructor(
        IPlayersArtNFTSaleFactory _factory,
        IPlayersArtNFT _nft,
        IERC20 _token,
    ...
    function setMeta(string memory _meta) external onlyOwnerOrAdmin {
        meta = _meta;
        emit DropMetaChanged(meta);
    }

    function initTokenIds(uint256 _count) external onlyOwnerOrAdmin {
        ...
        emit DropNFTTokenIdInited(endId - startId + 1);
    }
    ...
    function initSale(
        ...
        emit DropDataSet(startTime, whitelistDuration, minId, maxId, normalPurchaseCount);
    }
