...
contract LootBox is Ownable, Pausable, ReentrancyGuard, VRFConsumerBase {
  ...
  address[] public discountWhitelistArray;
  mapping (address => bool) public clusterDiscountWhitelistArray;
  mapping (address => bool) public discountWhitelistMap;
  uint256 public whitelistDiscountPercent;
   ...
   */
  function getNumberOfWhitelist() public view returns (uint256) 
  {
    return discountWhitelistArray.length;
  }
  ...
  {
    address add = discountWhitelistArray[_index];
    bool isWhiteList = discountWhitelistMap[add];
    return (add, isWhiteList);
  }
  ...
  function setDiscountWhiteList(
    ...
    for (uint i = 0; i < _addresses.length; i++){
      ...
      if (clusterDiscountWhitelistArray[add]) {
        clusterDiscountWhitelistArray[add] = true;
        discountWhitelistArray.push(add);
      } 
      
      discountWhitelistMap[add] = _isWhitelist;
    }
  }

  /**
   * @dev Set whitelist discount percent
   */
  function setWhiteListDiscountPercent(
    uint256 _whitelistDiscountPercent
  ) public onlyOwner {
    
    require(_whitelistDiscountPercent < 100);

    whitelistDiscountPercent = _whitelistDiscountPercent;
  }
  ...
  function _mint(
    ...
    if (_amount >= settings.amountToDiscount) {
      // tier discount
      cost = cost * (100 - settings.discountPercent) / 100;
    } else if (discountWhitelistMap[_toAddress]) {
      // global discount
      cost = cost * (100 - whitelistDiscountPercent) / 100;
    }
