...
contract MCHCMine is AccessOperatable {
    ...
    function claim(Balance memory _balance, uint16 _chainId, uint8 v, bytes32 r, bytes32 s) external whenNotPaused {
        require(getSignatureAddress(_balance, _chainId, v, r, s) == validator, "Mine: invalid claim signature");
    ...
    function getSignatureAddress(Balance memory _balance, uint16 _chainId, uint8 v, bytes32 r, bytes32 s) public pure returns (address) {
        return ecrecover(toEthSignedMessageHash(prepareMessage(_balance, _chainId)), v, r, s);
    }
