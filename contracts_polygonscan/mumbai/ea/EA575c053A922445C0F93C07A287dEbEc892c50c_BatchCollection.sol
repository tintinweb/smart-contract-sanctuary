...
import './BatchCollectionStorage.sol';

contract BatchCollection is
    IBatchCollection,
    ERC721EnumerableUpgradeable,
    OwnableUpgradeable,
    PausableUpgradeable,
    UUPSUpgradeable,
    BatchCollectionStorage
{
    ...
    function initialize(address _contractRegistry) public initializer {
        ...
        __ERC721_init_unchained(
            'CO2KEN Batch Retirement Collection',
            'CO2KEN-BNFT'
        );
        __Ownable_init_unchained();
        __Pausable_init_unchained();
    ...
    function updateBatchWithData(
        ...
        require(
            CarbonProjects(c).projectIds(_projectIdentifier) == true,
        ...
        if (!strcmp(metaDataHash, nftList[tokenId].metaDataHash)) {
            require(
                metaDataHashes[metaDataHash] == false,
                'Error: metaDataHash already exists'
            );
            nftList[tokenId].metaDataHash = metaDataHash;
    ...
    /// @dev Adds or updates additional project-vintage data
    function updateAdditionalData(
        ...
        nftList[tokenId].isCorsiaCompliant = isCorsiaCompliant;
        nftList[tokenId].isCCPcompliant = isCCPcompliant;
        nftList[tokenId].coBenefits = coBenefits;
        nftList[tokenId].correspAdjustment = correspAdjustment;
    }
    ...
    function mintBatchWithData(
        ...
        nftList[newItemId].metaDataHash = metaDataHash;
        metaDataHashes[metaDataHash] = true;
    }
    ...
    /// @notice Function that automatically coverts to ERC20s via corresponding contract
    /// @dev front-end must submit the pvId string in order to save gas
    /// pvId = projectId+vintage
    function fractionalize(uint256 tokenId, string memory pvId)
        external
        virtual
    {
        require(
        ...
        address ERC20Factory = IContractRegistry(contractRegistry)
        ...
        returns (string memory)
    {
        ...
        string memory metaDataHash = nftList[tokenId].metaDataHash;
        string memory base = _baseURI();

        // If there is no base URI, return just the hash
...
contract BatchCollectionStorage {
    using CountersUpgradeable for CountersUpgradeable.Counter;
...
contract ProjectERC20 is Context, ERC20, IERC721Receiver {
        ...
        )
    {
        ...
        string memory region;
        string memory storageMethod;
        (standard, methodology, region, storageMethod) = CarbonProjects(c)
    ...
    UUPSUpgradeable
{
    ...
    function deployNewToken(
        ...
        address c = IContractRegistry(contractRegistry).carbonProjectsAddress();
        require(
            CarbonProjects(c).projectIds(projectId) == true,
            'Project does not yet exist'
        );
    ...
    function deployFromTemplate(uint256 tokenId) public virtual whenNotPaused {
    ...
    function checkExistence(string memory _pvId)
        internal
        view
        virtual
        returns (bool)
