...
contract TrainingGame is Initializable, TrainingGame721Receiver, TrainingGameRewards, TrainingGameTimeKeeper {
    ...
    function _processReward(uint256 _tokenId, address _owner, uint256 _randomness) private {
        ...
        if(_burnResult < _chanceWizardBurned) {
            ...
            _findAndSendSAReward(_tokenId, _owner, _newRandom, _wizardProficiency);
    ...
    function _findAndSendSAReward(uint256 _tokenId, address _owner, uint256 _random, uint8 _wizardProficiency) private {
        ...
        emit RewardMinted(_owner, _tokenId, _rewardId, _amount);
    }
...
abstract contract TrainingGameRewards is Initializable, TrainingGameContracts {
    ...
    function setRewardSettings(uint256[] calldata _rewardIds, uint32[] calldata _rewardOdds, RewardMultiplier[] calldata _rewardMultipliers) external onlyAdminOrOwner {
        ...
        // Clear the old.
        for(uint256 i = rewardIds.length() - 1; i >= 0; i--) {
            uint256 _oldRewardId = rewardIds.at(i);
            rewardIds.remove(_oldRewardId);
            delete rewardIdToOdds[_oldRewardId];
            delete rewardIdToMultiplier[_oldRewardId];
        }
...
abstract contract TrainingGameState is Initializable, AdminableUpgradeable {
    ...
    event RewardMinted(address indexed _owner, uint256 indexed _tokenId, uint256 indexed _rewardId, uint256 _amount);
    ...
    EnumerableSet.UintSet internal rewardIds;
    ...
    EnumerableSet.UintSet internal burntWizards;
...
interface ISacrificialAlter is IERC1155 {
