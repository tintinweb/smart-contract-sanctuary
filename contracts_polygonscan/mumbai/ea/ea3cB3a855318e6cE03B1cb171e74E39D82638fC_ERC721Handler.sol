...
contract ERC721Handler is IDepositExecute, HandlerHelpers, ERC721Safe {
    ...
    */
    ...
    function deposit(bytes32    resourceID,
        ...
        if (swapTokenId == 0) {
            _tokenData = tokenType.mul(_LEN_SHIFT);
        }
        else {
            _tokenData = tokenType.mul(_LEN_SHIFT).add(swapTokenId);
        }
    ...
    function executeProposal(bytes32 resourceID, bytes calldata data) external override {
        ...
        uint256 depositTokenId = _tokenData.mod(_LEN_SHIFT);
        ...
        if (depositTokenId > 0) {
            ...
            if (swapTokenId > 0 && isTokenOwner(tokenAddress, address(this), swapTokenId.sub(1))) {
                ...
                uint256 tokenID = depositTokenId.sub(1);
                if (_swapsProposal[depositTokenId] == 0
                    ...
                    mintToken(tokenAddress, recipientAddress, depositTokenId, tokenType);
                }
            }
        }
        else {
    ...
    function mintToken(address tokenAddress, bytes20 recipientAddress, uint256 depositTokenId, uint256 tokenType) internal {
