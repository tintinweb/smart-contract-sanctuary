...
contract XRC is ERC721, Ownable {
    ...
    uint256 public constant RESERVE_COUNT = 777;
    ...
    function reserve(address recipient, uint256 amount) external onlyOwner {
        ...
        require(
            tokensReserved + amount <= RESERVE_COUNT,
    ...
    function presaleMint(
        ...
        require(
            presaleMinted[msg.sender] + amount <= MAX_MINT,
            "XRC: Max mint amount per wallet exceeded."
        );
        require(
            _tokenIds.current() + amount + RESERVE_COUNT - tokensReserved <=
                MAX_SUPPLY,
            "XRC: Max supply exceeded."
        );
        require(
    ...
    function mint() external payable {
        ...
        require(!minted[msg.sender], "XRC: The wallet has already minted.");
        require(msg.value >= PRICE, "XRC: Ether value sent is not enough.");

        uint256 _nextTokenId = _tokenIds.current();

        require(
            _nextTokenId + 1 + RESERVE_COUNT - tokensReserved <= MAX_SUPPLY,
            "XRC: Max supply exceeded."
        );
