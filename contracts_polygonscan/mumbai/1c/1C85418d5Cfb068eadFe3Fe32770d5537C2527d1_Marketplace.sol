...
interface IMintERC721 is IERC721 {
  ...
  function getMarketplaceFee() external view returns (uint marketplaceFee);
...
contract Marketplace is Ownable, Pausable, MarketplaceStorage {
    ...
    */
    ...
    /**
    * @dev Sets fee colector address
    * @param feeCollector_ - new fee colector address
    */
     ...
     *  can only be canceled by seller or the contract owner or nft token itself
    ...
    function _createOrder(
        ...
        IMintERC721 erc721Token = IMintERC721(nftAddress);
    ...
    /**
     * @dev Cancel an already published order
     *  can only be canceled by seller or the contract owner, or the nft token itself
     * @param nftAddress - Address of the NFT
     * @param assetId - ID of the published NFT
     */
        ...
        returns (Order memory)
    {
        ...
        require(
            order.seller == sender || sender == owner() || sender == nftAddress,
            "Unauthorized user"
        );
    ...
    function _executeOrder(
        ...
        bytes32 orderId = order.id;
        delete orderByAssetId[nftAddress][assetId];
        if (erc721Token.getMarketplaceFee() > 0) {
            // Calculate sale share
            saleShareAmount = price.mul(erc721Token.getMarketplaceFee()).div(1000000);

            // Transfer share amount for marketplace feeCollector
            require(
                acceptedToken.transferFrom(sender, feeCollector, saleShareAmount),
