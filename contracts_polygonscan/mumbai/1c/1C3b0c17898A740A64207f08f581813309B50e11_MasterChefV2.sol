...
contract Token is ERC20Burnable, Ownable {
    ...
    event TaxInfo(uint256 amount, uint256 taxAmount);
    event OraclePrice(uint256 price, bool valid);

    constructor( address _oracle ) public ERC20("Token", "Token") {
    ...
    function getCurrentTaxRate() internal returns (uint256) {
        if (useOracle == false) {
            return taxWithoutOracle;
        } else {
            (uint256 price, bool valid) = oracle.capture();
            emit OraclePrice(price, valid);
            if( ! valid ){
                return taxWithoutOracle;
            }
            if (price >= 90e16) {
    ...
    modifier onlySecondOperator(){
        require(operator == _msgSender(),"NOT_AUTHORIZED");
        _;
    }
...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of SBs
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. SBs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that SBs distribution occurs.
        uint256 accTokenPerShare;   // Accumulated SBs per share, times 1e12. See below.
    ...
    // SB tokens created per block.
    uint256 public tokenPerBlock = 0.1 ether;
    uint256 public TWAP = 0;
    uint256 public constant ONE = 1 ether; // control price
    uint256 public constant MIN = 0.1 ether; // control price
    uint256 public constant BLOCKS = 43200; // daily number of blocks Polygon network
    ...
    // The block number when SB mining starts.
    ...
    // Update the given pool's SB allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending SBs on frontend.
    ...
    // Deposit LP tokens to MasterChef for SB allocation.
    ...
    // Safe token transfer function, just in case if rounding error causes pool to not have enough SBs.
    ...
    uint public emissionUpdateInterval = 30 minutes; // approx 3hrs at 2 blocks/seconds
    function updateEmissionIfNeeded() internal {
    ...
    function updateEmissionRate() internal {
        ...
        massUpdatePools();
        if (TWAP >= ONE) {
            FACTOR = (TWAP.sub(ONE)).mul(200).div(1000);
            tokenPerBlock = BLOCKS.mul(1e18).add( BLOCKS.mul(FACTOR) ).mul(1e18).div( BLOCKS.mul(TWAP) );
        } else {
            tokenPerBlock = MIN;
        }
        // reward user that executed the oracle
        token.mint(msg.sender, tokenPerBlock);
        emit UpdateEmissionRate(msg.sender, tokenPerBlock);
    }
    function testUpdateEmissionRate(uint256 num) public onlyOwner {
        oracle.set(num, 100, true);
        updateEmissionRate();
    }
}