...
abstract contract ERC721Enumerable is ERC721, IERC721Enumerable {
  ...
  contract PlayBirdMansion is ERC721Enumerable, ContextMixin, Ownable {
    ...
    uint256 public birdPrice = 75 ether;
    ...
    mapping(address => uint256) public numReferrals;
    ...
    function mintBirdWithReferral(uint numberOfBirds, address referral) external payable reentrancyGuard {
        ...
        require(numberOfBirds <= maxBirdPurchase, "Can only mint 20 birds at a time");
        require(totalSupply() + numberOfBirds <= MAX_BIRDS, "Purchase would exceed max supply of Birds");
        require(validReferral(referral), "Referral address is not a valid referral address: ");
    ...
    function validReferral(address referral) private view returns (bool) {
        ...
        require(!(alreadyReferred[msg.sender]), "User has already referred once");
   ...
   */
    ...
    function returnArtist() external view returns(address) {
        return artist;
    }

    function returnMarketer() external view returns(address) {
        return marketer;
    }

    function returnDeveloper() external view returns(address) {
        return developer;
    }
