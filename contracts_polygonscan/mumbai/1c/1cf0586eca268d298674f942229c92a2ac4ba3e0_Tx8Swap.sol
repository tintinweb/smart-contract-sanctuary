...
contract Tx8Swap is ReentrancyGuard, Ownable {
  ...
  IERC20 public immutable tx8Token;
  IERC20 public immutable usdtToken;
  uint256 private priceUSDTX8; // 1 USDT ~ priceT TX8
  mapping(address => mapping(address => uint256)) private s_swappableAmount;

  constructor(address tx8Address, address usdtAddress) {
    tx8Token = IERC20(tx8Address);
   ...
   * @param amountUSDT count of liquidity being added
   */
  function addLiquidity(
    ...
    onlyOwner()
  {
    uint256 amountTX8 = amountUSDT.mul(priceUSDTX8);
    _addLiquidity(amountTX8, address(usdtToken), address(tx8Token));
    _addLiquidity(amountUSDT, address(tx8Token), address(usdtToken));

    tx8Token.safeTransferFrom(msg.sender, address(this), amountTX8);
   ...
   * @param amountUSDT count of liquidity being removed
   */
  ...
  {
    uint256 amountTX8 = amountUSDT.mul(priceUSDTX8);  
    _removeLiquidity(amountTX8, address(usdtToken), address(tx8Token));
    _removeLiquidity(amountUSDT, address(tx8Token), address(usdtToken));

    tx8Token.safeTransfer(msg.sender, amountTX8);
    usdtToken.safeTransfer(msg.sender, amountUSDT);
  }
   ...
   */
  function swapUSDT2TX8(
    uint256 amount
  )
    external
    nonReentrant()
  {
    uint256 amountTX8 = amount.mul(priceUSDTX8); 
    _addLiquidity(amount, address(tx8Token), address(usdtToken));
    _removeLiquidity(amountTX8, address(usdtToken), address(tx8Token));

    emit TokensSwapped(amount, address(usdtToken), address(tx8Token), msg.sender);
   ...
   * @param amount count of tokens being swapped
   */
  function swapTX82USDT(
    uint256 amount
  )
    external
    nonReentrant()
  {
    uint256 amountUSDT = amount.div(priceUSDTX8);
    _removeLiquidity(amount, address(usdtToken), address(tx8Token));
    _addLiquidity(amountUSDT, address(tx8Token), address(usdtToken));

    emit TokensSwapped(amount, address(tx8Token), address(usdtToken), msg.sender);

    tx8Token.safeTransferFrom(msg.sender, address(this), amount);
    usdtToken.safeTransfer(msg.sender, amountUSDT);
  }
