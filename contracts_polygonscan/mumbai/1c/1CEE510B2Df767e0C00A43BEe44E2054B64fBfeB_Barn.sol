...
contract Barn is Ownable, IERC721Receiver, ReentrancyGuard {
	...
	// reference to the wolf NFT contract
	Wolf wolf;
	...
	// there will only ever be (roughly) 2.5 billion $WOOL earned through staking
	uint256 public constant MAXIMUM_GLOBAL_WOOL = 2500000000 ether;
	...
	constructor(address _wolf, address _wool) {
		wolf = Wolf(_wolf);
		wool = WOOL(_wool);
	...
	/**
	 * adds Sheep and Wolves to the Barn
	 * requires allowance
	 * @param tokenId the ID of the Sheep or Wolf to stake
	 */
	function addToBarn(uint256 tokenId) external nonReentrant {
		require(wolf.ownerOf(tokenId) == msg.sender, "msg.sender not owner of tokenID");
		require(wolf.exists(tokenId), "tokenID not existing");

		wolf.transferFrom(msg.sender, address(this), tokenId);

		if (isSheep(tokenId)) {
			_addSheepToBarn(tokenId);
		} else {
			_addWolfToPack(tokenId);
		}
	}
	 ...
	 */
	function _addSheepToBarn(uint256 tokenId) internal _updateEarnings {
		barn[tokenId] = Stake({ owner: msg.sender, tokenId: tokenId, value: block.timestamp });
		totalSheepStaked += 1;
	 ...
	 */
	function _addWolfToPack(uint256 tokenId) internal {
		...
		pack[alpha].push(Stake({ owner: msg.sender, tokenId: tokenId, value: woolPerAlpha })); // Add the wolf to the Pack
		emit TokenStaked(msg.sender, tokenId, woolPerAlpha);
	}
	...
	function _claimSheepFromBarn(uint256 tokenId, bool unstake) internal returns (uint256 owed) {
		...
		require(stake.owner == msg.sender, "msg.sender not stake.owner");
		require(
		...
		if (unstake) {
			...
			wolf.safeTransferFrom(address(this), msg.sender, tokenId, ""); // send back Sheep
			delete barn[tokenId];
			totalSheepStaked -= 1;
		} else {
	...
	function _claimWolfFromPack(uint256 tokenId, bool unstake) internal returns (uint256 owed) {
		require(wolf.ownerOf(tokenId) == address(this), "AINT A PART OF THE PACK");
		uint256 alpha = _alphaForWolf(tokenId);
		Stake memory stake = pack[alpha][packIndices[tokenId]];
		require(stake.owner == msg.sender, "msg.sender is not stake.owner");
		owed = (alpha) * (woolPerAlpha - stake.value); // Calculate portion of tokens based on Alpha
		if (unstake) {
			totalAlphaStaked -= alpha; // Remove Alpha from total staked
			wolf.safeTransferFrom(address(this), msg.sender, tokenId, ""); // Send back Wolf
	...
	modifier _updateEarnings() {
		if (totalWoolEarned < MAXIMUM_GLOBAL_WOOL) {
			totalWoolEarned +=
				((block.timestamp - lastClaimTimestamp) * totalSheepStaked * DAILY_WOOL_RATE) /
				1 days;
	...
	function isSheep(uint256 tokenId) public view returns (bool sheep) {
		(sheep, ) = wolf.tokenTraits(tokenId);
	}

	/**
	 * gets the alpha score for a Wolf (lower is better)
	 * @param tokenId the ID of the Wolf to get the alpha score for
	 * @return the alpha score of the Wolf
	 */
	function _alphaForWolf(uint256 tokenId) internal view returns (uint256) {
		(, uint256 alphaIndex) = wolf.tokenTraits(tokenId);
