...
contract NexusMintableToken is ERC721, ERC721URIStorage, Ownable {
    ...
    function acceptOffer(uint256 tokenId) external onlyNftOwner(tokenId){
        ...
        require(bidders[tokenId] != address(0));
        
        super.safeTransferFrom(msg.sender,bidders[tokenId],tokenId,"0x");
        bidded[tokenId] = false;
        bidders[tokenId]=address(0);
        address payable buyer = payable(msg.sender);
        buyer.transfer(prices[tokenId]);
        
        emit OfferAccepted(tokenId,prices[tokenId],msg.sender,bidders[tokenId]);
