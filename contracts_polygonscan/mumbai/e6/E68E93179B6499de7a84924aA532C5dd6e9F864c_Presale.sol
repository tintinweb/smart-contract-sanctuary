...
contract Presale is ReentrancyGuard, Context, Ownable {
    ...
    function buyTokens(address beneficiary) public nonReentrant icoActive payable {
        ...
        // Add person to distrubuted map
        distributed[beneficiary].wallet = beneficiary;
        distributed[beneficiary].amount += weiAmount.mul(reward);

        emit TokensPurchased(_msgSender(), beneficiary, weiAmount, tokens); // Emit event
        _forwardFunds(); // Send funds to wallet
    }
    ...
    // fallback
    function _deliverTokens(address beneficiary, uint256 tokenAmount) public onlyOwner icoActive() {
        _token.transfer(beneficiary, tokenAmount);
    }

    function withdrawBoughtTokens() external onlyOwner {
        _token.transfer(address(this), distributed[address(this)].amount);
    }
