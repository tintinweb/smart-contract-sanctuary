...
contract TamiGAWDtchi is TrustedForwarderRecipient {
    ...
    function resurrect(uint16 gawd) public isTrueGawd(gawd) {
        ...
        uint8 startingValue = 50; // reset @ 50%
        _worship[gawd] = startingValue;
        _sacrifice[gawd] = startingValue;
        _cower[gawd] = startingValue;
        _marvel[gawd] = startingValue;
        _lastWorshipBlock[gawd] = block.number;
        _lastMarvelBlock[gawd] = block.number;
        _lastSacrificeBlock[gawd] = block.number;
        _lastCowerBlock[gawd] = block.number;
    ...
    function getTotalScore(uint16 gawd) public view isTrueGawd(gawd) returns (uint256) {
        return getWorship(gawd) + getMarvel(gawd) + getSacrifice(gawd) + getCower(gawd);
    }
    ...
    function getBlocksPerTick() public view returns (uint16) {
        return _BLOCKS_PER_TICK;
    }
}
