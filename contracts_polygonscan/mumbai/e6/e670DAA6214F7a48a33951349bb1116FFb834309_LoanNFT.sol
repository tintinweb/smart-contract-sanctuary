...
library FixidityLib {
  ...
  function convertFixed(
    ...
    if (_originDigits > _destinationDigits) {
      ...
      //     10**38 < 2**128-1
      require(x <= maxInt256() / int256(uint256(10)**uint256(decimalDifference)));
      require(x >= minInt256() / int256(uint256(10)**uint256(decimalDifference)));
      return x * (int256(uint256(10)**uint256(decimalDifference)));
    }
  ...
  function safeIntToUint(int256 x) internal pure returns (uint256) {
    require(x >= 0, "Cannot cast negative signed integer to unsigned integer.");
    return uint256(x);
  }
  ...
  function safeUintToInt(uint256 x) internal pure returns (int256) {
    require(x <= safeIntToUint(maxInt256()), "Cannot cast overflowing unsigned integer to signed integer.");
    return int256(x);
  }
   ...
   */
  function newFixedFraction(int256 numerator, int256 denominator) internal pure returns (int256) {
   ...
   * will be truncated accordingly.
   */
  function newFixed(int256 x, uint8 _originDigits) internal pure returns (int256) {
   ...
   */
  function fromFixed(int256 x, uint8 _destinationDigits) internal pure returns (int256) {
...
contract LoanNFT is ERC721URIStorage, ILoanNFT {
  ...
  mapping(address => int256) public totalLoanValues;
    ...
    )
  {
    ...
    if (loan.status == LoanStatus.DISBURSED) {
      int256 feeRepayment24 = _loanRepayment24.multiply(loan.balances.fee24).divide(loan.balances.outstanding24);
      loan.balances.fee24 = loan.balances.fee24.subtract(feeRepayment24);
      loan.balances.outstanding24 = loan.balances.outstanding24.subtract(_loanRepayment24);
      loan.amountRepaid24 = loan.amountRepaid24.add(_loanRepayment24);
      loans[_loanID] = loan;

      (, int256 originatorFee24, int256 governanceFee24) = (ICopraGlobal(copraGlobal)).getFees24();

      int256 totalFees24 = originatorFee24.add(governanceFee24);
      _amountToPool = (_loanRepayment24.subtract(feeRepayment24)).toUint(DECIMALS);
      _amountToOriginator = (feeRepayment24.multiply(originatorFee24).divide(totalFees24)).toUint(DECIMALS);
      _amountToGovernance = (feeRepayment24.multiply(governanceFee24).divide(totalFees24)).toUint(DECIMALS);

      _recalculateLoanNetValue(_loanID, loan);

      if (loan.balances.netValue24 <= 0) {
        loans[_loanID].status = LoanStatus.CLOSED;
      }

      emit LoanPaidAmountUpdated(_loanID, _loanRepayment24.toUint(DECIMALS));
    }
  }
  ...
  function _updateLoanValue(uint256 _loanID) internal {
    ...
    } else if (loan.status == LoanStatus.DISBURSED && block.timestamp - loan.balances.lastUpdatedAt >= 1 days) {
  ...
  function _updateLoanBalances(Loan memory _loan) internal view returns (Loan memory) {
    if (block.timestamp == _loan.actualTimeDisbursed) {
      _loan.balances.outstanding24 = _loan.principal24;
    } else {
      (, int256 originatorFee24, int256 governanceFee24) = (ICopraGlobal(copraGlobal)).getFees24();
      _loan.balances.outstanding24 = _loan.balances.outstanding24.add(
        _loan.balances.outstanding24.multiply(_loan.dailyRate24)
      );
      _loan.balances.fee24 = _loan.balances.fee24.add(
        _loan.balances.outstanding24.multiply(_loan.dailyRate24).multiply(originatorFee24.add(governanceFee24))
      );
      if (block.timestamp > _loan.repaymentDate + 1 days) {
        _loan.balances.late24 = _loan.balances.late24.add(_loan.balances.outstanding24.multiply(_loan.lateFee24));
      }
    }
  ...
  function _recalculateLoanNetValue(uint256 _loanID, Loan memory _loan) internal {
    int256 prevNetValue24 = _loan.balances.netValue24;
    _loan.balances.netValue24 = _calculateLoanValue24(_loan);
    totalLoanValues[ownerOf(_loanID)] = totalLoanValues[ownerOf(_loanID)].add(
      _loan.balances.netValue24.subtract(prevNetValue24)
    );
    loans[_loanID] = _loan;

    emit LoanUpdated(
      _loanID,
      _loan.balances.netValue24.toUint(DECIMALS),
      ownerOf(_loanID),
      totalLoanValues[ownerOf(_loanID)].toUint(DECIMALS)
    );
  }
  ...
  function mintNewLoan(bytes calldata _loan) external override returns (uint256) {
    ...
    Loan memory loan = Loan({
      ...
      principal24: _principal.toFixed(DECIMALS),
      amountRepaid24: 0,
      lateFee24: _lateFee.toFixed(DECIMALS).divide(uint256(100).toFixed()),
      borrower: _borrowerAddress,
      disbursementDate: _timeDisbursed,
      actualTimeDisbursed: 0,
      dailyRate24: _dailyRate.toFixed(DECIMALS).divide(uint256(100).toFixed()),
  ...
  function getTotalLoanValue(address _loanOwner) public view override returns (uint256) {
    ...
    return totalLoanValues[_loanOwner].toUint(DECIMALS);
  }
  ...
  {
    LoanBalances memory loanBalances = loans[_loanID].balances;
    loanOutstandingBal = loanBalances.outstanding24.toUint(DECIMALS);
    loanFeeBal = loanBalances.fee24.toUint(DECIMALS);
    loanLatePenaltyBal = loanBalances.late24.toUint(DECIMALS);
    loanPoolBal = loanBalances.pool24.toUint(DECIMALS);
    netLoanValue = loanBalances.netValue24.toUint(DECIMALS);
    lastUpdatedAt = loanBalances.lastUpdatedAt;
  }

  function _calculateLoanValue24(Loan memory _loan) internal view returns (int256) {
    int256 loanBalance24 = _loan.balances.outstanding24.add(_loan.balances.late24);
    if (loanBalance24 <= 0) {
      loanBalance24 = 0;
    }
    int256 loanValue24 = loanBalance24.subtract(_loan.balances.fee24);
    if (loanValue24 < 0) {
      loanValue24 = 0;
    }

    int256 loanLateDays24 = block.timestamp.toFixed().subtract(_loan.repaymentDate.toFixed()).divide(
      uint256(1 days).toFixed()
    );
    if (loanLateDays24 < 0) {
      return loanValue24;
    }

    int256 one24 = uint256(1).toFixed();
    return
      loanValue24.multiply(
        one24.subtract(loanLateDays24.divide((ICopraGlobal(copraGlobal)).getMaxLateDays().toFixed()))
      );
  }
