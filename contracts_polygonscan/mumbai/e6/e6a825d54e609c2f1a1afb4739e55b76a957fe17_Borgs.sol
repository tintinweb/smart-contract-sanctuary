...
contract Borgs is ERC721Enumerable, ERC721Holder, Ownable, ContractStates, CommonObjects, NumberUtils{
    ...
    function getListingByTokenId(uint256 tokenId) public view returns(Listing memory listing){
        // Get listing
        listing = _listings[tokenId];
    }
    
    function getListingByIndex(uint256 index) public view returns(Listing memory listing){
        // Get tokenId
        uint256 tokenId = getListingsTokenIdByIndex(index);
