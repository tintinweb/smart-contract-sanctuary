...
contract ERC20InvictusFund is OwnableUpgradeable, ERC20PresetMinterPauserUpgradeable, ERC2771ContextUpgradeable {
    ...
    address private _trustedForwarder;
    address constant public dealAddress = 0x000000000000000000000000000000000000deA1;
    ...
    uint24 public maxRedemptionsPerTx;
    uint24 public maxInvestmentsPerTx;
    ...
    event RedemptionEvent(uint256 eventId, uint256 TokenAmount, uint256 redemtionCurrencyAmount,string redemptionCurrency, address particpant, uint128 dealingNonce);
    event RedemptionFailed(uint256 eventId,uint256 TokenAmount, uint256 redemtionCurrencyAmount, string redemptionCurrency, address participant, uint256 dealingNonce);
    event InvestmentEvent(uint256 eventId, uint256 investmentCurrencyAmount, uint256 tokenAmount,string investmentCurrency, string transactionHash, address participant, uint128 dealingNonce);
    ...
    function initializeInvictusFund(
        ...
        require(whitelistContractInput != address(0) || feeAddressInput != address(0) , 'all address params required');
    ...
    /**
     * @dev Purges the investments array.
     */
    function purgeInvestments() external onlyOwner {
        delete investments;
    }

    /**
     * @dev Purges the redemptions array.
     */
    function purgeRedemptions() external onlyOwner {
        console.log('purging redemptions');
        delete redemptions;
    }
     ...
     */
    function calculateExitFee(uint256 redemptionAmount) internal view returns(uint256) {
     ...
     * @param redemptionCurrency The currency you would like to recieved your redemption in.
     */
    function requestRedemption(uint256 withdrawAmount, string memory redemptionCurrency) external onlyWhitelisted onlyRedemptionCurrencies(redemptionCurrency) {
     ...
     */
    function redemptionsLength() public view returns(uint) {  
     ...
     */
    function investmentsLength() public view returns(uint) {  
        return investments.length;
    } 

    /**
     * @dev Request investment in a supported currencies to be processed on the next dealing.
     ...
     * @param participant The currency you would like to recieved your redemption in.
     * @param chainId The currency you would like to recieved your redemption in.
     * @param output The currency you would like to recieved your redemption in.
     */
     ...
     */
    function removeLiquidity(uint256 amount, string memory ticker) external onlyPricingOracle onlyRedemptionCurrencies(ticker) {
     ...
     * @param newRedemptionsPerTx The amount of redemptions per dealing transaction.
     */
    function setRedemptionsPerTx(uint24 newRedemptionsPerTx) external onlyOwner {
        require(newRedemptionsPerTx > 0, 'Invalid param');
    ...
    /**
     * @dev Set the maximum number of investements to be processed per dealing transaction
     * @param newMaxInvestmentsPerTx The amount of investments per dealing transaction.
     */
    function setMaxInvestmentsPerTx(uint24 newMaxInvestmentsPerTx) external onlyOwner {
        require(newMaxInvestmentsPerTx > 0, 'Invalid param');
     ...
     */
    function addCurrency(string memory ticker, address contractAddress, uint256 numerator, uint24 decimals) external onlyPricingOracle {
     ...
     * @param participant The ticker of the currency. eg. ETH
     ...
     */
    function increaseTokenAmendment(address participant, uint256 amount, string memory reason, string memory txHash) external onlyOwner {
        require(amount > 0, 'Invalid amount');
     ...
     * @param participant The address that you want to decrease the token amount for.
     ...
     */
    function decreaseTokenAmendment(address participant, uint256 amount, string memory reason, string memory originalTxHash) external onlyOwner {
        require(amount > 0, 'Invalid amount');
        //Should this be burnFrom
        burnFrom(participant, amount);
        emit DecreaseTokenAmendment(participant, amount, reason, originalTxHash);
    }
     ...
     * @param participant The users address who is doing the redemption.
     ...
     * @param redemptionCurrency The currency requested to recive your redemtion in.
     */
    function dealRedemption(uint256 eventId, address participant, uint256 redemptionCurrencyAmount, uint256 tokenAmount, string memory redemptionCurrency) public onlyPricingOracle {
        address stableContract = currencies[redemptionCurrency].contractAddress;
        require(ERC20Upgradeable(stableContract).balanceOf(address(this)) > redemptionCurrencyAmount, 'not enough liquidity');
        ...
        ERC20Upgradeable(stableContract).safeTransfer(participant, (redemptionCurrencyAmount.sub(exitFee)));
        emit RedemptionEvent(eventId, tokenAmount, redemptionCurrencyAmount, redemptionCurrency, participant, dealingNonce);
    }
     ...
     * @param participant The users address who is doing the investment.
    ...
    function dealInvestment(
        ...
        emit InvestmentEvent(eventId, investmentCurrencyAmount, fundTokenAmount, investmentCurrency, transactionHash, participant, dealingNonce);
    }
     ...
     */
    function calculateTokenAmount(string memory investmentCurrency, uint256 investmentCurrencyAmount) internal view returns(uint256) {
    ...
    /**
     * @dev Calculates the amount of tokens a participant will recieved for their redemption.
     * @param redemptionCurrency The requested currency the participant wants to recieve the redemption in.
     * @param tokenAmount The ammount of fund tokens the participant wants to redeem.
     */
    function calculateRedemptionCurrencyAmount(string memory redemptionCurrency, uint256 tokenAmount) internal view returns(uint256) {
    ...
    function _handleRedemptionsDealing() internal returns (bool) {
        uint256 numberOfRedemptions = MathUpgradeable.min(redemptionsLength(), maxRedemptionsPerTx);
        uint256 endingIndex = redemptionsLength().sub(numberOfRedemptions);
        Redemption memory redemption;
        for(uint i = redemptionsLength(); i > endingIndex; i--){
            ...
            if(redemption.timestamp < cutOffTime &&  InvictusWhitelist(whitelistContract).isWhitelisted(redemption.participant)){
                //dealRedmption
                dealRedemption(i-1, redemption.participant, calculateRedemptionCurrencyAmount(redemption.redemptionCurrency, redemption.amount), redemption.amount, redemption.redemptionCurrency);
                redemptions.pop();
            }
        }
    ...
    function _handleInvestmentsDealing() internal returns (bool) {
        uint256 endingIndex = investmentsLength().sub(MathUpgradeable.min(investmentsLength(), maxInvestmentsPerTx));
        Investment memory investment;
        
        for(uint i = investmentsLength(); i > endingIndex; i --){
            ...
            if(investment.timestamp < cutOffTime && InvictusWhitelist(whitelistContract).isWhitelisted(investment.participant)) {
                investments.pop();
                //deal investment
                dealInvestment(i-1, investment.participant, investment.amount, calculateTokenAmount(investment.investmentCurrency, investment.amount), investment.investmentCurrency, investment.transactionHash);     
            }
        }
        uint256 nextStartingIndex = endingIndex <= 1 ? 0 : endingIndex - 1;
