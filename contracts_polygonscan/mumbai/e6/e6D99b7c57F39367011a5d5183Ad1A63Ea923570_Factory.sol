...
contract Factory is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    event QuestionCreated(address indexed questionAddress, uint256 questionArrayLength);
    ...
    //     uint256 startTime;
    //     uint256 lockTime;
    //     uint256 rewardRate; // 90%
    //     uint256 treasuryRate; // 10%
    // }
    ...
    function deployQuestions(bytes32 _id, QuestionEncodedData[] memory questionData) external onlyOperator {
        ...
        if (questionAddressArray[questionData[0].gameID].length == 0) {
            emit GameCreated(questionData[0].gameID);
        }
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            bytes4 funcSig = bytes4(keccak256(bytes("initialize(bytes,uint256,uint256,uint256)")));
            bytes memory payload = abi.encodeWithSelector(
                funcSig,
                questionData[i].data,
                questionData[i].minBetAmount,
                questionData[i].startTime,
                questionData[i].lockTime
            );
            ...
            emit QuestionCreated(questionClone, questionData.length);
        }
    }
