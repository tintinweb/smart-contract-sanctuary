...
contract CryptoPropheciesGame is ReentrancyGuard, Ownable {
    ...
    function _transferFundsToWinner(uint256 _battleId) internal {
        ...
        uint256 ticketAmount = battle.TCPAmount / 2;
        ...
        if (battle.winner == address(0)) {
            IDailyPrize(ctDailyPrize).addTickets(battle.player1, (ticketAmount * multipliers[player1Rarity][0]) / 2);
            IDailyPrize(ctDailyPrize).addTickets(battle.player2, (ticketAmount * multipliers[player2Rarity][0]) / 2);
            emit DailyPrizeTicketAdded(battle.player1, (ticketAmount * multipliers[player1Rarity][0]) / 2, block.timestamp);
            emit DailyPrizeTicketAdded(battle.player2, (ticketAmount * multipliers[player2Rarity][0]) / 2, block.timestamp);
        }
