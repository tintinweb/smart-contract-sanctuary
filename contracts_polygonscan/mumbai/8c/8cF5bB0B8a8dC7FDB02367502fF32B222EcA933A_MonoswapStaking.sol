...
contract MonoswapStaking is Initializable, OwnableUpgradeable, ERC1155HolderUpgradeable {
    ...
    function emergencyWithdraw(uint256 _pid) public validPool(_pid) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        safeLPTransfer(pool.lpToken, pool.lpTokenId, address(this), address(msg.sender), user.amount);
    ...
    function safeLPTransfer(IERC1155 _lpToken, uint256 _lpTokenId, address _from, address _to, uint256 _amount) internal returns (uint256) {
        ...
        if (_to == address(this)) { // receive
           ...
           require(amountOut <= _amount, "transfer: overflow");
           return amountOut;
        }  
    }
