...
contract World is Context, ReentrancyGuard, Ownable {
  ...
  function init() external onlyOwner returns (bool success) {
  ...
  function initMock() external onlyOwner returns (bool success) {
    require(state == State.INIT, "state: already init");
    // init lands

    MockLand l = new MockLand(address(this));
  ...
  function initSettler() external onlyPlayable returns (bool success) {
  ...
  function settler(address user) public view returns (address _settler) {
...
interface ILand {
  ...
  function sendTroops(uint, uint, uint, uint) external;
  function retreat(uint, uint, uint) external; // remove troops from attack or defend
}
...
contract Settler is Context, ReentrancyGuard {
  ...
  function addResources(uint _food, uint _wood, uint _stone, uint _ore) external nonReentrant onlyLand {
    food = food.add(_food);
    wood = wood.add(_wood);
  ...
  function requestWorker(uint v) external onlyLand {
    worker.settler = worker.settler.sub(v, "worker not enough");
  }
  
  function returnWorker(uint v) external onlyLand {
    require(worker.settler.add(v) <= worker.current, "worker exceed limit");
    worker.settler = worker.settler.add(v);
  }

  function requestTroops(uint _infantry, uint _archer, uint _chevalier) external onlyLand {
  ...
  function returnTroops(uint _infantry, uint _archer, uint _chevalier) external onlyLand {
...
contract MockLand is Context, ReentrancyGuard, Ownable {
  ...
  function collect() external nonReentrant returns (bool success) {
    address settler = _world.settler(_msgSender());
    safeAddResources(ISettler(settler), 1000, 2000, 3000, 4000);
    return true;
  }

  function collectResources(uint food, uint wood, uint stone, uint ore) external nonReentrant returns (bool success) {
    address settler = _world.settler(_msgSender());
    safeAddResources(ISettler(settler), food, wood, stone, ore);
    return true;
  }

  function safeAddResources(ISettler s, uint food, uint wood, uint stone, uint ore) internal virtual {
    bytes memory returndata = address(s).functionCall(
      abi.encodeWithSelector(s.addResources.selector, food, wood, stone, ore), 
      "land: add resource failed"
    );
    if (returndata.length > 0) {
      require(abi.decode(returndata, (bool)), "land: add resource failed");
    }
  }

  function sendTroops(uint _infantry, uint _archer, uint _chevalier, uint flag) external nonReentrant {
    address s = _world.settler(_msgSender());
    _safeRequestTroops(ISettler(s), _infantry, _archer, _chevalier);
  }
  function retreat(uint _infantry, uint _archer, uint _chevalier) external nonReentrant {
    address s = _world.settler(_msgSender());
    _safeReturnTroops(ISettler(s), _infantry, _archer, _chevalier);
  }

  function _safeRequestTroops(ISettler s, uint _infantry, uint _archer, uint _chevalier) internal virtual {
    bytes memory returndata = address(s).functionCall(
      abi.encodeWithSelector(s.requestTroops.selector, _infantry, _archer, _chevalier),
      "land: request troops failed"
    );
    if (returndata.length > 0) {
      require(abi.decode(returndata, (bool)), "land: request troops failed");
    }
  }
  function _safeReturnTroops(ISettler s, uint _infantry, uint _archer, uint _chevalier) internal virtual {
    bytes memory returndata = address(s).functionCall(
      abi.encodeWithSelector(s.returnTroops.selector, _infantry, _archer, _chevalier),
      "land: return troops failed"
    );
    if (returndata.length > 0) {
      require(abi.decode(returndata, (bool)), "land: return troops failed");
    }
  }

  function sendWorker(uint v) external nonReentrant {
    address s = _world.settler(_msgSender());
    _safeRequestWorker(ISettler(s), v);
  }

  function retrieveWorker(uint v) external nonReentrant {
    address s = _world.settler(_msgSender());
    _safeReturnWorker(ISettler(s), v);
  }

  function _safeRequestWorker(ISettler s, uint v) internal virtual {
    bytes memory returndata = address(s).functionCall(
      abi.encodeWithSelector(s.requestWorker.selector, v), 
      "land: request worker failed"
    );
    if (returndata.length > 0) {
      require(abi.decode(returndata, (bool)), "land: request worker failed");
    }
  }

  function _safeReturnWorker(ISettler s, uint v) internal virtual {
    bytes memory returndata = address(s).functionCall(
      abi.encodeWithSelector(s.returnWorker.selector, v), 
      "land: request worker failed"
    );
    if (returndata.length > 0) {
      require(abi.decode(returndata, (bool)), "land: request worker failed");
