...
contract DexilonTest {
    ...
    */
        ...
        ) 
    {
        ...
        marketFullBalance = int(availableBalance) +
            btcToUsdcMarketConvert(btcBalance) +
    ...
    **/
    function updateUserBalances(
        ...
        if (btcBalance > 0) {
            newAvailableBalance = availableBalance - amountBTC*rate/BTC_UNIT_FACTOR;
        } else if (btcBalance == 0 || amountBTC < 0) {
            newAvailableBalance = availableBalance - abs(amountBTC*rate/BTC_UNIT_FACTOR);
        } else {
            newAvailableBalance = availableBalance + amountBTC*(2*abs(lockedBalance*BTC_UNIT_FACTOR/btcBalance) - rate)/BTC_UNIT_FACTOR;
        }
        ...
        if ((btcBalance > 0 && amountBTC < 0) || (btcBalance < 0 && amountBTC > 0)) {
            newLockedBalance = lockedBalance - abs(amountBTC*lockedBalance/btcBalance);
        } else {
            newLockedBalance = lockedBalance + abs(amountBTC*rate/BTC_UNIT_FACTOR);
        }
