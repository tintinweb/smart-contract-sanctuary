...
// File: contracts/Farm/EurekaFarm.sol
...
contract EurekaFarm is Ownable {
    using SafeMath for uint256;
    ...
    event AddReward(address lp, address rewardToken0, address rewardToken1, uint256 reward0, uint256 reward1);
    event Claim(address indexed user, uint256 indexed pid, uint256 reward0, uint256 reward1);
    event Paused();
    event UnPaused();
    event AddOperator(address _operator);
    event RemoveOperator(address _operator);
    event AddLpInfo( IERC20 _lpToken, IERC20 _rewardToken0, IERC20 _rewardToken1);
    ...
    constructor(address _factory, address _owner) public {
    ...
    function addReward(address _lp,address token0, address token1, uint256 amount0, uint256 amount1) public {
        ...
        pool.lastRewardBlock = block.number;
        emit AddReward(address(pool.lpToken), address(pool.rewardToken0), address(pool.rewardToken1), reward0, reward1);
    }

    function deposit(uint256 _pid, uint256 _amount) public isPaused isPoolExist(_pid){
    ...
    function claimReward(uint256 _pid) public isPaused isPoolExist(_pid) {
        ...
        if (user.amount > 0) {
            ...
            pendingReward1 =
                user.amount.mul(pool.accPerShare1).div(1e12).sub(
                    user.rewardDebt1
                );
    ...
    function withdraw(uint256 _pid, uint256 _amount) public isPaused isPoolExist(_pid) {
        require(_amount > 0, "zero amount");
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
