...
// File: contracts/libraries/relations.sol
...
contract RelationsGame is Initializable, AccessControl {
    ...
    address[] private _playingCustomers;
    ...
    uint256 public playRelationsBase = 1000000000000000000 wei;
    ...
    function addNewCustomer(address customer) private {}

    // Game

    function playWithFactory(uint256 factory, uint256[] memory ids) public {
        require(ids.length > 0, INVALID_EMPLOYEES);
        require(employees.balanceOf(msg.sender) > 0, INVALID_EMPLOYEES);
        require(canFactoryPlay(msg.sender, factory), INVALID_FACTORIES);
        require(allEmployeesCanPlay(msg.sender, ids), INVALID_EMPLOYEES);

        if (_playRewards[msg.sender] == 0) {
            _playingCustomers.push(msg.sender);
        }
        ...
        uint8 fact = factories.getType(factory);
        ...
        for (uint8 i = 0; i < ids.length; i++) {
            uint8[4] memory parts = employees.getParts(ids[i]);

            for (uint8 j = 0; j < parts.length; j++) {
                if (fact == parts[j]) types++;
            }

            points += employees.getPoints(ids[i]);
            _employeePlayTime[ids[i]] = block.timestamp;
        }

        sinergy = ((types * 100) / (ids.length * 4));
    ...
    function allEmployeesCanPlay(address owner, uint256[] memory employee)
        public
        view
        returns (bool)
    {
        for (uint8 i = 0; i < employee.length; i++) {
            if (!canEmployeePlay(owner, employee[i])) return false;
        }
