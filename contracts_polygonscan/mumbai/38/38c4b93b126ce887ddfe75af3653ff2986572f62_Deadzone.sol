...
///*EVENTS*
    ...
    event Attack(uint16 attackerId, uint16 defender16, bool successfulAttack);
    event Deployment(uint16 sentryId, uint timestamp);
    event Evacuation(uint16 sentryId, uint timestamp);
// *VARS*
    uint randNonce = 1;
    //0=city 1=forrest 2=caves
    uint8 public currentEnvironment = 0;
    uint public totalDeployedSentries;
    uint maxYield = 10000 ether;
    uint dailyBitRate = 500 ether;
    bool _deadzoneActive = true;

    //array of stats needed for iteration upon battle call
    GameplayStats[] public allStats;

// *CONTRACT_REFERNCES*
    ISentryFactory private factory;
    IDeploymentParty private party;
    IBits public bits;
...
///*MAPPINGS*
    ...
    mapping(uint8 => uint[3]) public battleMultipliers;
    mapping(uint8 => uint[3]) public luckMultipliers;
...
///*MODIFIER*
    modifier onlyController(address _address) {
        require(controllers[_address], "You cannot access this functionality!");
        _;
    }
    modifier onlyTokenOwner(uint16 sentryId, address _address) {
        require(factory.getSentryOwner(sentryId) == _address, "You do not own this sentry!");
        _;
    }
...
///*FUNCTIONS*
    // risky biz
    function soloDeploy(uint16 sentryId, uint8 riskCode) external onlyTokenOwner(sentryId, msg.sender){
        require(deadzoneActive(), "The deadzone is currently not active!");
    ...
    function soloEvac(uint16 sentryId) external onlyTokenOwner(sentryId, msg.sender) {
        ...
        emit Evacuation(sentryId, block.timestamp);
    }

    function attack(uint16 sentryId) external onlyTokenOwner(sentryId, msg.sender) validInfectedSentry(sentryId) {
        require(totalDeployedSentries > 0, "There are currently no deployed Sentries!");
        ...
        GameplayStats memory target = allDeployed[rng(allDeployed.length)];
        ...
        // zombie is rewarded, sentry is penalized
            ...
            deployOrEvacHandler(target.sentryId, 0, false);
            ...
            allStats[getStatsIndex(target.sentryId)].successfulDefends++;

        }

        emit Attack(sentryId, target.sentryId, successfulAttack);
    }
...
///*HELPERS*
    ...
    function deployOrEvacHandler(uint16 sentryId,uint8 riskCode ,bool deploy) private {
        ...
        if(deploy) {
            require(!idToStats[sentryId].isDeployed, "This Sentry is already deployed!");
            ...
            require(idToStats[sentryId].isDeployed, "This Sentry is not deployed!");
    ...
    function getDeployedSentries() private view returns(GameplayStats [] memory) {
        uint count = totalDeployedSentries;
        GameplayStats[] memory deployed = new GameplayStats[](count);
        uint arrCounter;
        for(uint i = 0; i < allStats.length; i++) {
            GameplayStats memory stat = allStats[i];
                if(allStats[i].isDeployed) {
                    deployed[arrCounter] = stat;
                    arrCounter++;
                }
        }
        return deployed;
    ...
    function battle(uint16 attackerId, uint16 defenderId) private returns(bool) {
        ...
        uint attackStat = (attacker.attack*battleMultipliers[attacker.species][currentEnvironment]);
        uint defendStat = (defender.defense*battleMultipliers[defender.species][currentEnvironment]);

        return rng(100) < (attackStat*100 / (attackStat+defendStat));
    }
    ...
    function deadzoneActive() public view returns(bool) {
        return true;
        // return _deadzoneActive && block.timestamp.getDayOfWeek() < 6;
    }
...
///*INTERFACE*
    ...
    function deploy(uint16 sentryId) external onlyController(msg.sender) {
        require(!factory.getSentryTraits(sentryId).infected, "Infected Sentries cannot deploy!");
        deployOrEvacHandler(sentryId, 0, true);
    }
    // This interface is to be used by the deployment party contract
    function evac(uint16 sentryId) external onlyController(msg.sender) {
        deployOrEvacHandler(sentryId, 0, false);
    }

    // will be called when a sentry is created;
    function createGameplayStats(uint16 sentryId) public onlyController(msg.sender) {
    ...
    function sentryListDeploy(uint16[] memory sentryId) external onlyController(msg.sender) {
    ...
    function editRiskCode(uint16 sentryId ,uint8 newCode) external onlyController(msg.sender){
...
///*OWNER*
    ...
    function setBattleMultiplier(uint8 ind,uint8[3] memory newSet) public onlyOwner {
        battleMultipliers[ind] = newSet;
    }
    function setLuckMultiplier(uint8 ind,uint8[3] memory newSet) public onlyOwner {
        luckMultipliers[ind] = newSet;
    }
    function setDeadzoneActive(bool _state) public onlyOwner {
        _deadzoneActive = _state;
    }
}