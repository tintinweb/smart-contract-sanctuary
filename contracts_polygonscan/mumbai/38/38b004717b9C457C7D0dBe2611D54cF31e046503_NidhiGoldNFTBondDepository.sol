...
interface IMarketplace {
  function priceFromOracle(address principle) external view returns (uint256);
}
...
contract NidhiGoldNFTBondDepository is Ownable, IERC721Receiver {
    ...
    onlyPolicy
  {
    if (_parameter == PARAMETER.VESTING) {
      ...
      // 1
      require(_input <= 2500, "Payout cannot be above 1 percent");
   ...
   */
  function setMarketplace(address _marketplace) external onlyPolicy {
    require(address(_marketplace) != address(0));
    marketplace = _marketplace;
  }
  ...
  function deposit(
    ...
    ITreasury(treasury).depositNFT(address(principle), _tokenId, profit, false);
  ...
  function assetPrice() public view returns (uint256) {
    return IMarketplace(marketplace).priceFromOracle(principle);
  }
