...
contract HappyRedPacket is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    external payable {
        ...
        }
        {
            ...
            emit CreationSuccess(received_amount, _id, _name, _message, msg.sender, block.timestamp, _token_addr, number, ifrandom, duration, token_type, scanner++);
        }
    }
    ...
    function claim(bytes32 id, uint8 _v, bytes32 _r, bytes32 _s) external returns (uint claimed) {
        ...
        // Claim success event
        emit ClaimSuccess(id, msg.sender, claimed_tokens, address(uint160(unbox(packed.packed2, 64, 160))), scanner++);
    ...
    function check_availability(bytes32 id) external view returns ( address token_address, uint balance, uint total,
        uint claimed, bool expired, uint256 claimed_amount, uint256 _token_type, address _creator) {
        RedPacket storage rp = redpacket_by_id[id];
        Packed memory packed = rp.packed;
        return (
    ...
    function refund(bytes32 id) external {
        ...
        emit RefundSuccess(id, token_address, remaining_tokens, scanner++);
    }
