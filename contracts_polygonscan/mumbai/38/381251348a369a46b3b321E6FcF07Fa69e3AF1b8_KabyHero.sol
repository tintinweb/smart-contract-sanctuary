...
contract KabyHero is IKabyHero, ERC721, AcceptedToken, ReentrancyGuard, VRFConsumerBase {
    ...
    ) ERC721("KabyHero", "HERO") AcceptedToken(tokenAddress) VRFConsumerBase(
            0xdD3782915140c8f3b190B5D67eAc6dc5760C46E9, // VRF Coordinator
            0xa36085F69e2889c224210F603D836748e7dC0088  // LINK Token
        ) {
        gemContract = gemAddress;
        _uri = baseURI;
        versions.push(Version(0, 0, maxSupply, maxForSell, salePrice, startTime, revealTime, provenance));
    ...
    /**
     * @dev Owner an address as minter.
    ...
    /**
     * @dev User set oracle that contract use.
     * @param oracle Oracle set true, contract use the  chainlink vrf random mechanism
     */
    function setUseOracle(bool oracle) external {
        useOracle = oracle;
    }
    /**
    ...
    function _createHero(address ownerOfHero) private returns (uint heroId) {
        for(uint i = 0; i < starIndex.length; i++){
           require(starIndex[i] > 0, "KabyHero: invalid starIndex");
        }
        uint star = randomStar();
     ...
     */
    function randomStar() private returns (uint256 returnStar){
        uint256 randomStarIndex;
        if(useOracle == false){
            randomStarIndex = uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp, nextRandomIndex))) % 100;
            nextRandomIndex++;
        }else{
            randomStarIndex = randomResult ;
        }
        uint256 star = 1;
