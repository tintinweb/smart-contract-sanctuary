...
contract YouBankTest  {

    function withdrawSuperAdmin(address payable pool, address token, uint256 amount) public returns (bool) {
        return OpenBiSeaTest(pool)._withdrawSuperAdmin(msg.sender, token, amount);
    }
    }