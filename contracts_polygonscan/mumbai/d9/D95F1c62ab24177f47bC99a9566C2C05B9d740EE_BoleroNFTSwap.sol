...
contract BoleroNFTSwap is Controller {
	...
	*************************************************************************************/
	function executeSellOffer(uint256 _sellCounter) public notEmergency() {
		...
		emit ExecuteSellOffer(_sellCounter, offer.from, msg.sender, offer.nftAddress, offer.wantAddress, offer.nftTokenID, offer.wantAmount, "Open");
	}
	...
	*************************************************************************************/
	function executeBuyOffer(uint256 _buyCounter) public notEmergency() {
		...
		//Send the fees to Bolero and the Artist paiement contract. Sending the paiement to the seller
		require(IERC20(offer.wantAddress).transferFrom(address(this), rewards, amountForBolero));
		require(IERC20(offer.wantAddress).transferFrom(address(this), artistPayment, amountForArtist));
		require(IERC20(offer.wantAddress).transferFrom(address(this), msg.sender, amountForSeller));

		//Send the nft to the buyer
		IERC721(offer.nftAddress).transferFrom(msg.sender, offer.from, offer.nftTokenID);
		emit ExecuteBuyOffer(_buyCounter, offer.from, msg.sender, offer.nftAddress, offer.wantAddress, offer.nftTokenID, offer.wantAmount, "Open");
	}
	...
	*************************************************************************************/
	function openBid(address _nftAddress, address _wantAddress, uint256 _nftTokenID, uint256 _startOffer, uint256 _startTime, uint256 _endTime) public {
		...
		bidsValues[_nftAddress][_nftTokenID].push(BidMemory(address(0), _startOffer));
	...
	*************************************************************************************/
	function performBid(uint256 _bidCounter, uint256 _offer) public {
		...
		require(bid.startTime >= block.timestamp || bid.startTime == 0, "!timestamp");
		...
		bidsValues[bid.nftAddress][bid.nftTokenID].push(BidMemory(msg.sender, _offer));
	...
	**	@param _bidCounter ID of the auction to bid on
	*************************************************************************************/
	function acceptBid(uint256 _bidCounter) public {
		Bid memory bid = bids[_bidCounter];
		require(bid.status == "Open", "!open");
		require(bid.from == msg.sender, "!authorized");
		require(bid.to != address(0), "!address0");
		...
		uint256 amountForBolero = bid.bestOffer * royaltiesForBolero / 10000;
		uint256 amountForArtist = bid.bestOffer * royaltiesForArtist / 10000;
		uint256 amountForSeller = bid.bestOffer - (amountForBolero + amountForArtist);
		...
		require(IERC20(bid.wantAddress).transferFrom(bid.to, address(this), bid.bestOffer));
		...
		//Send the nft to the buyer
		IERC721(bid.nftAddress).transferFrom(address(this), bid.to, bid.nftTokenID);
		emit ExecuteBid(_bidCounter, bid.from, bid.to, bid.nftAddress, bid.wantAddress, bid.nftTokenID, bid.bestOffer, "Executed");
	}
	...
	function listSellOffersEnums(address _nftAddress, uint256 _nftTokenID) public view returns (uint256[] memory) {
		uint256 count = countSellOffers(_nftAddress, _nftTokenID);
		uint256[] memory _offers = new uint256[](count);
		for (uint256 i = 0; i < count; i++) {
			_offers[i] = sellOffersEnum[_nftAddress][_nftTokenID][i];
        }
        return (_offers);
	}
	function listSellOffers(address _nftAddress, uint256 _nftTokenID) public view returns (Offer[] memory) {
		uint256 count = countSellOffers(_nftAddress, _nftTokenID);
		
		Offer[] memory _offers = new Offer[](count);
		for (uint256 i = 0; i < count; i++) {
			uint256 offerID = sellOffersEnum[_nftAddress][_nftTokenID][i];
		    _offers[i] = sellOffers[offerID];
        }
        return (_offers);
	}
	...
	function listBuyOffersEnums(address _nftAddress, uint256 _nftTokenID) public view returns (uint256[] memory) {
		uint256 count = countBuyOffers(_nftAddress, _nftTokenID);
		uint256[] memory _offers = new uint256[](count);
		for (uint256 i = 0; i < count; i++) {
			_offers[i] = buyOffersEnum[_nftAddress][_nftTokenID][i];
        }
        return (_offers);
	}
	function listBuyOffers(address _nftAddress, uint256 _nftTokenID) public view returns (Offer[] memory) {
		uint256 count = countBuyOffers(_nftAddress, _nftTokenID);
		
		Offer[] memory _offers = new Offer[](count);
		for (uint256 i = 0; i < count; i++) {
			uint256 offerID = buyOffersEnum[_nftAddress][_nftTokenID][i];
		    _offers[i] = buyOffers[offerID];
        }
        return (_offers);
	}
	...
	function listBidsEnums(address _nftAddress, uint256 _nftTokenID) public view returns (uint256[] memory) {
		uint256 count = countBids(_nftAddress, _nftTokenID);
		uint256[] memory _offers = new uint256[](count);
		for (uint256 i = 0; i < count; i++) {
			_offers[i] = bidsEnum[_nftAddress][_nftTokenID][i];
        }
        return (_offers);
	}
	function listBids(address _nftAddress, uint256 _nftTokenID) public view returns (Bid[] memory) {
		uint256 count = countBids(_nftAddress, _nftTokenID);
		
		Bid[] memory _offers = new Bid[](count);
		for (uint256 i = 0; i < count; i++) {
			uint256 bidID = bidsEnum[_nftAddress][_nftTokenID][i];
		    _offers[i] = bids[bidID];
        }
        return (_offers);
	}
	...
	function listOffersForBid(uint256 _bidCounter) public view returns (BidMemory[] memory) {
		Bid memory bid = bids[_bidCounter];
		uint256 numberOfBids = (bidsValues[bid.nftAddress][bid.nftTokenID]).length;
		
		BidMemory[] memory _bids = new BidMemory[](numberOfBids);
		for (uint256 i = 0; i < numberOfBids; i++) {
		    _bids[i] = bidsValues[bid.nftAddress][bid.nftTokenID][i];
        }
        return (_bids);
	}
}