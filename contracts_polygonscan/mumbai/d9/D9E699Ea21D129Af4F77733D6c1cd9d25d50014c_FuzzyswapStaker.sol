...
contract FuzzyswapStaker is IFuzzyswapStaker, Multicall {
    ...
    function createIncentive(IncentiveKey memory key, uint256 reward) external override {
        ...
        address virtualPool = address(new FuzzyswapVirtualPool(address(key.pool), address(this))); 
        key.pool.setIncentive(virtualPool, uint32(key.endTime));
    ...
    function unstakeToken(IncentiveKey memory key, uint256 tokenId) external override {
        ...
        uint160 secondsPerLiquidityInsideX128 = IFuzzyswapVirtualPool(incentive.virtualPoolAddress).getInnerSecondsPerLiquidity(deposit.tickLower, deposit.tickUpper);
    ...
    function _stakeToken(IncentiveKey memory key, uint256 tokenId) private {
        ...
        virtualPool.applyLiquidityDeltaToPosition(tickLower, tickUpper, int128(liquidity));
...
contract FuzzyswapVirtualPool is IFuzzyswapVirtualPool{
    ...
    address immutable private poolAddress;
    address immutable private stakerAddress;
    ...
    function finish(uint _endTimestamp) external override onlyPool{
    ...
    function processSwap(
        ...
        globalSecondsPerLiquidityCumulative = globalSecondsPerLiquidityCumulative +
                    ((uint160(currentTimestamp-previousTimestamp) << 128) / (liquidity > 0 ? liquidity : 1));
    ...
    IERC721Permit
{
    ...
    event IncreaseLiquidity(uint256 indexed tokenId, uint128 liquidity, uint256 amount0, uint256 amount1, address pool);
...
interface IFuzzyswapPoolOwnerActions {
    ...
    function setIncentive(address virtualPoolAddress, uint32 endTimestamp) external;
...
library PoolAddress {
    ...
    bytes32 internal constant POOL_INIT_CODE_HASH = 0x75540cffbf6fa0f7384d7cd847b36e21a896b818798ea322666054ba953ae16a;
...
interface IFuzzyswapVirtualPool{
    ...
    function finish(uint _endTimestamp) external;
