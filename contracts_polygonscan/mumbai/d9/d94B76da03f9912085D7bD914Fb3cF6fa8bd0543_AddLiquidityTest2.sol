...
import "./PermitTokenFactory.sol";

contract AddLiquidityTest2 is PermitTokenFactory{
    ...
    external
  {
    ...
    IPermitToken(getLiquidity[_token]).mint(msg.sender, _amount);
  }
  ...
  {
    require(_amount > 0,"Amount must be greater than zero");
    IPermitToken(getLiquidity[_token]).burnFrom(msg.sender, _amount);
    liquidityInfo[_token].toAllChainableBalance -= _amount;
 ...
 */
abstract contract EIP712public {
 ...
 * _Available since v3.4._
 */
abstract contract PermitTest2 is ERC20, IERC20Permit, EIP712public {
     ...
     */
    constructor(string memory name, string memory symbol) ERC20(name,symbol) EIP712public(name, "1") {}
...
import "./interfaces/IPermitTokenDeployer.sol";
import "./interfaces/IPermitToken.sol";
import "./PermitTest2.sol";

contract PermitToken is IPermitToken, PermitTest2 {
  address public immutable override bridge;
  address public immutable override token;

  constructor() PermitTest2("Permit Token", "PT") {
    (bridge, token) = IPermitTokenDeployer(msg.sender).parameters();
  }
...
contract  PermitTokenDeployer is IPermitTokenDeployer {
  ...
  returns (address liquidity) 
  {
    ...
    liquidity = address(new PermitToken{salt: keccak256(abi.encode(_bridge, _token))}());
    delete parameters;
  }
}
...
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import "./interfaces/IPermitTokenFactory.sol";
import "./PermitTokenDeployer.sol";
import "./PermitToken.sol";

contract PermitTokenFactory is IPermitTokenFactory, PermitTokenDeployer {
  bytes32 public constant INIT_CODE_LIQ_HASH = keccak256(abi.encodePacked(type(PermitToken).creationCode));
...
interface IPermitToken is IERC20mb {
...
interface IPermitTokenDeployer {
