...
contract AnrkeyGameMarketplace is Ownable{
    ...
    uint256 public platformCharges;
    uint256 public sendUserPercentage;
    address public platformAccount;
    
    IERC1155 public goldenchalicetoken;
    IERC20 public anrxtoken;
    
    modifier shouldLessThan10000(uint256 amount) {
        require(amount <= 10000, "Charges are in percentage, you can use 3 precisions so it should be less than 10000!");
        _;
    }
    ...
    modifier checkTokenOwner(uint256 tokenId) {
        require(goldenchalicetoken.balanceOf(msg.sender,tokenId) != 0, 'You didn\'t own the token or you\'re not allowed to operate this tokens!');
        _;  
    }
    
    event MakeTokenBuyable(address indexed owner, uint indexed tokenId, uint time);
    event RemoveTokenFromBuyable(address indexed owner, uint indexed tokenId, uint time);
    event ChangeTokenStatus(address indexed owner, uint indexed tokenId, uint time);
    event SaleToken(address indexed buyer, address indexed seller, uint indexed tokenID, uint tokenAmt);
    
    constructor(address goldenchaliceToken,uint256 platformCharge,address anrxToken,address platformAccountAddress) shouldLessThan10000(platformCharges) {
        require(goldenchaliceToken != address(0), "anrkeyGameMarketplace: goldenchaliceToken cannot be zero address");
        require(anrxToken != address(0), "anrkeyGameMarketplace: anrxToken cannot be zero address");
        require(platformAccountAddress != address(0), "anrkeyGameMarketplace: platformAccountAddress cannot be zero address");
        goldenchalicetoken = IERC1155(goldenchaliceToken);
        platformCharges = platformCharge;
        anrxtoken = IERC20(anrxToken);
        sendUserPercentage = 1000 - platformCharge;
        platformAccount = platformAccountAddress;
    }
    
    
    // Making token avalible for sale.
    function makeTokenBuyable(uint256 tokenId)external checkTokenOwner(tokenId) returns(bool) {
        // goldenchalicetoken.setApprovalForAll(address(this),true);
        ...
        // from node side -->  goldenchaliceToken.setApprovalForAll(this.address,true)
        emit MakeTokenBuyable(msg.sender, tokenId, block.timestamp);
        return true;
    }
    
    // Remove token from sale.
    function removeTokenfromBuyable(uint256 tokenId)external checkTokenOwner(tokenId) returns(bool) {
        delete saleItems[tokenId];
        // goldenchaliceToken.setApprovalForAll(address(this),false)
        emit RemoveTokenFromBuyable(msg.sender, tokenId, block.timestamp);
        return true;
    }
    
    function changeTokenStatus(uint256 tokenId,bool status)internal returns(bool) {
        onSaleItem storage saleToken = saleItems[tokenId];
        saleToken.sold = status;
        return true;
    }

    // Change token for avalible for sale or not.
    function changeTokenStatusFromUser(uint256 tokenId, bool status)external tokenNotSoldAlready(tokenId) checkTokenOwner(tokenId) returns(bool) {
        onSaleItem storage saleToken = saleItems[tokenId];
        saleToken.sold = status;
        emit ChangeTokenStatus(msg.sender, tokenId, block.timestamp);
        return true;
    }

    // For buying token from platform
    function buyToken(uint256 tokenId,address buyer,uint256 anrxAmount)external tokenNotSoldAlready(tokenId) returns(orderDetails memory) {
        // check wether tokens is in buyable list or not!
        require(saleItems[tokenId].timeOnsale!=0,"Token is not buyable!");
        
        // onSaleItem storage item = orderLogs[msg.sender][i];
        onSaleItem storage item = saleItems[tokenId];
        require(!item.sold,"Token is already sold!");
        require(goldenchalicetoken.isApprovedForAll(item.owner,address(this)),'Token is not approved to tranfer!');
        
        // uint256 tokenSendToSeller=SafeMath.mul(anrxAmount,sendUserPercentage);
        
        // Check the allowance or permitted token
        uint256 allowance = anrxtoken.allowance(buyer,address(this));
        require(allowance >= anrxAmount,"Check the token allowance");
        uint256 tokenSendToSeller = anrxAmount * sendUserPercentage / 10000;
        uint256 tokenSendToPlatform = anrxAmount * platformCharges / 10000;
        orderDetails memory newOrder = orderDetails({
            tokenId: tokenId,
            buyer: buyer,
            seller: item.owner,
            totalPrice:anrxAmount,
            platformCharges:tokenSendToPlatform,
            time:block.timestamp
        });
        orderLogs[msg.sender].push(newOrder);
        
        // remove token from saleItems list
        changeTokenStatus(tokenId,true);
        
        anrxtoken.safeTransferFrom(buyer,item.owner,tokenSendToSeller);
        anrxtoken.safeTransferFrom(buyer,platformAccount,tokenSendToPlatform);
        goldenchalicetoken.safeTransferFrom(item.owner,buyer,tokenId,1,'0*0');
        emit SaleToken(buyer, item.owner, tokenId, anrxAmount);
        return newOrder;
    }
    
    // Get the all order logs for perticualr buyer.
    function viewOrderLogs() external view returns(orderDetails[] memory) {
        return orderLogs[msg.sender];
    }
    
    // Change the percentage send to platform account.
    function changePlatformCharge(uint256 newAmount) external onlyOwner shouldLessThan10000(newAmount) returns(bool) {
        platformCharges = newAmount;
        sendUserPercentage = 10000 - newAmount;
        return true;
    }
    
    // Get transfer ratio of distribution of amount at time of buyable executed.
    function getTransferRatio() external view onlyOwner returns(uint256,uint256) {
        return (platformCharges,sendUserPercentage);
    }

    // Change anrxtoken adress to interact with.    
    function updateAnrxtokenAddress(address tokenAddress)external onlyOwner returns(bool) {
        require(tokenAddress != address(0), "anrkeyGameMarketplace: anrxToken cannot be zero address");
        anrxtoken = IERC20(tokenAddress);
        return true;
    }
    
    // Change Goldanchalicetoken adress to interact with.
    function updateGoldanChaliceAddress(address nftAddress)external onlyOwner returns(bool) {
        require(nftAddress != address(0), "anrkeyGameMarketplace: goldenchaliceToken cannot be zero address");
        goldenchalicetoken = IERC1155(nftAddress);
        return true;
    }
    
    // Change platformAccountAddress for transferring charge. 
    function updatePlatfromAddress(address platformAccountAddress)external onlyOwner returns(bool) {
        platformAccount = platformAccountAddress;
        return true;
    }
}