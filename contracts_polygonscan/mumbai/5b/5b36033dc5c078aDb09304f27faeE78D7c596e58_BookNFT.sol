...
contract BookNFT is Ownable, ERC721Enumerable {
    ...
    function initBookData(
        ...
        address _buyToken = bookManager.getBuyToken(bookManager.getCurrentBuyTokenVersion());
        buyToken = _buyToken;
        address _deployerAddress = bookManager.getDeployerAddress();
        deployerAddress = _deployerAddress;
        address _factoryAddress = bookManager.getFactory(bookManager.getCurrentFactoryVersion());
    ...
    function mint(address _to) public {
        ...
        require(
            currentTimestamp >= uint256(endDate), "Current is not time mint"
        );
    ...
    function register() public {
        ...
        require(
            currentTimestamp >= uint256(startDate) && currentTimestamp <= uint256(endDate), "Current is not time register"
        );
        IERC20(buyToken).transferFrom(_msgSender(),address(this), price * 10**18);
    ...
    function claimMoneySaleBook() external onlyAuthor {
        uint256 currentTimestamp = block.timestamp;
        require(
            currentTimestamp > uint256(endDate), "You cannot claim money"
        );
        if (totalRegisters <= totalPublish) {
            transferERC20(buyToken, authorAddress, totalRegisters * price * 10**18);
        } else {
            transferERC20(buyToken, authorAddress, totalPublish * price * 10**18);
        }
    }
