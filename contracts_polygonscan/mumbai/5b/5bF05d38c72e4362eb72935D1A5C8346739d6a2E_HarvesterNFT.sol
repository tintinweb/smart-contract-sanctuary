...
contract HarvesterNFT is ERC721URIStorage, ERC721Holder, ReentrancyGuard, Ownable {
    ...
    // 用户拥有的采集器映射：拥有者 => 数量
    mapping(address => uint) private ownerHarvesterCount; 
    // OCT发币地址
    ...
    //市场合约地址
    address public marketContractAddress = 0x3d9A69F30Ee3F3C78fE8CbE2e22A6EE945df4F54; 
    MetaGlobeNFTMarket internal metaGlobeNFTMarket;
    ...
    constructor( uint256 _seed) ERC721("HarvesterNFT", "Harvester") {
        ...
        metaGlobeNFTMarket = MetaGlobeNFTMarket(payable(marketContractAddress));
        randomResult = _seed;
    }
  ...
  //设置市场合约地址
  function setMarketContract(address  _marketContractAddress) public onlyOwner {
      ...
      returns (uint256)
  {
      ...
      harvesterOwner[_msgSender()] = true;
      ownerHarvesterCount[_msgSender()] += 1;
  ...
  //获取用户拥有的采集器总数
  function getHarvesterOwnCount(address addr) public view returns(uint256) {
    if (harvesterOwner[addr] == false) return 0;
    return ownerHarvesterCount[addr];
  }

  // 获取用户拥有的采集器列表， 可用于游戏
  function getHarvesterListByOwner(address owner) public view returns (uint[] memory) {
      // 为了节省gas消耗 在内存中创建结果数组方法之后完后就会销毁
      uint[] memory result = new uint[](ownerHarvesterCount[owner]);
      uint counter = 0;
...
contract MetaGlobeNFTMarket is IERC721Receiver, Ownable, ReentrancyGuard, Pausable{
  using SafeMath for uint256;
  using SafeERC20 for IERC20;
  ...
  function addApprovedNFT(address _address) public nonReentrant onlyOwner {
    approvedNFTs[_address] = true;
    nftAddresses.push(_address);
  }

  function removeApprovedNFT(address _address) public nonReentrant onlyOwner {
  ...
  function createMarketItem(
    ...
    require(isNFTApproved(nftContract), "nftContract is not approve");
  ...
  function createMarketSale(
    ...
    //触发事件
    emit OnMarketItemSold(itemId, nftContract, tokenId, msg.sender, address(this), price);    

    return true;
  }

  //撤单
  function cancelOrder(address nftContract, uint256 itemId) public nonReentrant returns (bool) {
    require(msg.sender != address(0));
    require(nftContract != address(0));     
    require(paused() == false, "contract is paused");
    require(isNFTApproved(nftContract), "nftContract is not approve");
