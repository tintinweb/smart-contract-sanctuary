...
contract KRC721 {
    ...
    event Evolve(uint256 tokenId, uint16 stage);
...
contract PetCore is PetMinting {
    ...
    function feedOnPet(uint256 _petId, uint256 _amount) external {
        Pet storage pet = pets[_petId];
        if (pet.stages == 1){
            require(IKRC20(dpetToken).transferFrom(msg.sender, address(this), amountToAdulthood));
            pet.stages += 1;
            emit Evolve(_petId, pet.stages);
        } else if (pet.stages == 2){
            require(IKRC20(dpetToken).transferFrom(msg.sender, address(this), amountToMiddleAge));
            pet.stages += 1;
            emit Evolve(_petId, pet.stages);
        } else{
            _amount += 1;
