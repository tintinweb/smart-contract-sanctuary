...
contract TestEggCarton is FxBaseChildTunnel, VRFConsumerBase, Ownable {
    ...
    constructor(address _fxChild) FxBaseChildTunnel(_fxChild) VRFConsumerBase(
    ...
    function viewRarestTraits(uint _tokenID, uint _mom, uint _dad) view external returns(uint32[8] memory metaData){
        metaData = _findRarest(_tokenID, _mom, _dad);
    }

    function _findRarest(uint _tokenID, uint _mom, uint _dad) internal view returns(uint32[8] memory metaData){
        ...
        if(Egg.traitInfo(0, mom[0]).rarity < Egg.traitInfo(1, mom[1]).rarity){
            momRarest = 0;
            mom2ndRarest = 1;
        }
        else{
        ...
        if(Egg.traitInfo(0, dad[0]).rarity < Egg.traitInfo(1, dad[1]).rarity){
            dadRarest = 0;
            dad2ndRarest = 1;
        }
        else{
        ...
        for(uint8 i=2; i < 8; i++){
            ...
            if(Egg.traitInfo(i, mom[i]).rarity <  Egg.traitInfo(momRarest, mom[momRarest]).rarity){
            ...
            if(Egg.traitInfo(i, dad[i]).rarity <  Egg.traitInfo(dadRarest, dad[dadRarest]).rarity){
        ...
        //metaData = new uint[](8);
        if(dad[dadRarest] == mom[momRarest]){
            if(Egg.traitInfo(dad2ndRarest, dad[dad2ndRarest]).rarity < Egg.traitInfo(mom2ndRarest, mom[mom2ndRarest]).rarity){
                metaData[dad2ndRarest] = dad[dad2ndRarest];
                metaData[momRarest] = mom[momRarest];
            }
            else{
                metaData[mom2ndRarest] = mom[mom2ndRarest];
                metaData[dadRarest] = dad[dadRarest];
            }
        }
        else{
            if(Egg.traitInfo(dadRarest, dad[dadRarest]).rarity < Egg.traitInfo(momRarest, mom[momRarest]).rarity){
                metaData[dadRarest] = dad[dadRarest];
                if(dadRarest == momRarest){//traitID is the same, so use the moms second rarest
                    metaData[mom2ndRarest] = mom[mom2ndRarest];
                }
                else{//Use the rarest for the mom
                    metaData[momRarest] = mom[momRarest];
                }
            }
            else{
                ...
                else{//Use the rarest for the dad
                    metaData[dadRarest] = dad[dadRarest];
                }
            }
        }
    ...
    function _findEggBonus(uint baby, uint _mom, uint _dad) internal{
        ...
        uint bonus = 0;
        for(uint8 i=0; i < 8; i++){
            momSum += Egg.traitInfo(i, mom[i]).rarity;
            dadSum += Egg.traitInfo(i, dad[i]).rarity;
        }
    ...
    function findTraits(uint tokenID) external {
        ...
        uint32[8] memory metaData = _findRarest(tokenID, mom, dad);
        ...
        for (uint8 i=0; i<8; i++){
            if(newCrow[i] == 0){//means this trait is not set
                ...
                for(uint32 j=Egg.eventTraitStartingIndex(0, i); j<Egg.eventTraitStartingIndex(1, i); j++){
                    sum += uint256(Egg.traitInfo(i, j).rarity);
                    if(rng <= sum){
                        newCrow[i] = j;
                        break;
                    }
    ...
    function _calculateEggBreedingTraits(uint tokenID, uint randomNumber) internal{
        ...
        for (uint8 i=0; i<8; i++){
            if(newCrow[i] == 0){//means this trait is not set
                ...
                uint rng = expandedRandomness[i] % Egg.eventTraitSum();
                for(uint32 j=Egg.eventTraitStartingIndex(0, i); j<Egg.eventTraitStartingIndex(1, i); j++){
                    sum += maturityMultiplier * maturity * uint256(Egg.traitInfo(i, j).rarity) / (maxMaturity * baseMultiplier);
