...
contract Gacha is VRFConsumerBase {
     ...
     * Requests randomness
     */
    function _gacha() internal returns (bytes32 requestId) {
        require(
    ...
    {
        uint256[] memory cumulativeWeight = new uint256[](
            rarityNameList.length
        );
        ...
        uint256 maxCumulativeWeight = cumulativeWeight[
            cumulativeWeight.length - 1
        ];
...
import {Gacha} from "../gacha/Gacha.sol";

contract MelXNFT is MelXNFTPayment, MelXNFTBasic, Gacha {
    using CountersUpgradeable for CountersUpgradeable.Counter;
    CountersUpgradeable.Counter internal _tokenIdCounter;

    bytes32 public constant URI_SETTER_ROLE = keccak256("URI_SETTER");

    event mintWithGacha(string rarityName, uint256 rate, uint256 basePoint);
    ...
    function addRarity(
        ...
        _addRarity(_name, _rate, _basePoint);
    }
    ...
    {
        requestIdToRandomness[requestId] = randomness;
        Rarity memory rarity = _getGachaResult(requestId);

        safeMint(requestIdToAddress[requestId]);

        emit mintWithGacha(rarity.name, rarity.rate, rarity.basePoint);
    }
}
