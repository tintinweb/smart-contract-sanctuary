...
abstract contract GAME_GameTokens is GAME_MasterMinions {
  ...
  returns(uint tokenId_) {
    ...
    bytes32 inGameAddress = erc721Contract.getOrCreateInGameAccount(game_, sender);
    ...
    uint fixedXp;
    (tokenId_, fixedXp) = _createToken(inGameAddress, game_, set_, card_, bytes32(uint(-1)));
    ...
    internal
  {
    ...
    if(fixedXp_ == 0) {
      ...
      if(randUint16 < 512) { // 1% chance of legendary (12% for the first, 6% for the 8th)
        xp = 524288 + randUint16b.mul(8);
        xpPerHour_ = 16;
      } else if (randUint16 < 2048) { // 3% chance of epic (38% for first, 19% for the 8th)
        xp = 131072 + randUint16b.mul(4);
        xpPerHour_ = 8;
      } else if (randUint16 < 8192) { // 10% chance of rare (50% for first, 75% for 8th)
        xp = 16384 + randUint16b;
        xpPerHour_ = 4;
      } else if (randUint16 < 24576) { // 25% chance of uncommon (0% for first 8)
  ...
  function _workerBuyCollectible(
    ...
    internal
  {
    bytes32 inGameAddress = erc721Contract.getOrCreateInGameAccount(0, purchaser_);
    // Create the token, and transfer it to the user
    (uint tokenId, uint fixedXp) = _createToken(inGameAddress, 0, partnerId_, card_, purchaseId_);
  ...
  returns(uint[] memory _tokenIds) {
    ...
    for (uint i = 0; i < cards_.length; i++) {
      (_tokenIds[i], fixedXp) = _createToken(recipient_, game_, set_, cards_[i], purchaseId_);
    }
  }
