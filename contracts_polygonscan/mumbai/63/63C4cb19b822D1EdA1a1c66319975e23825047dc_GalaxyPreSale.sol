...
contract GalaxyPreSale is Ownable, Pausable, ReentrancyGuard {
     ...
     */
    constructor (address _tokenContract, address _USDCAddress) {
    ...
    function createPreSale(
        ...
        tokenPrice = _tokenPrice; // 1 USDC = 10 ** 6
        ...
        tokenContract.safeTransferFrom(msg.sender, address(this), _preSaleTokens);
    } 
    ...
    /**
     * Updates Price of Token (in USDC)
     */
    ...
    function addPreSaleTokens(uint256 _numOfTokens) external onlyOwner {
        ...
        tokenContract.safeTransferFrom(msg.sender, address(this), _numOfTokens);
    }
    ...
    /**
     * Calculate Price for Number of Tokens (in USDC)
     */
     ...
     */
    function buy(uint256 _numOfTokens, uint256 _USDCAmount) external payable nonReentrant whenNotPaused {
        ...
        require(_USDCAmount >= minBuy && _USDCAmount <= maxBuy, "Amount should be between minBuy and maxBuy");
        require(tokenContract.balanceOf(address(this)) >= _numOfTokens, "Contract doesn't have enough balance");
        
        USDCContract.safeTransferFrom(msg.sender, address(this), calculatePrice(_numOfTokens));
        tokenContract.safeTransfer(msg.sender, _numOfTokens);
     ...
     */
    function withdrawUSDC(address _to, uint256 _amount) external onlyOwner nonReentrant {
        require(_to != address(0), "_to is Zero Address");
        uint256 balance = USDCContract.balanceOf(address(this));
        require(balance >= _amount, "Insufficient balance");
        
        USDCContract.safeTransfer(_to, _amount);
    }
