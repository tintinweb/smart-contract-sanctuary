...
contract Xmas is ERC721URIStorage, Ownable, ERC721Enumerable {
    using Strings for uint256;
    using Counters for Counters.Counter;

    uint256 public maxSupply = 5160;
    ...
    constructor() ERC721("Christmas Animals Party", "XAP") {}

    // PUBLIC FUNCTIONS:

    function mint(address _caller, uint256 _mintAmount) public payable {
        ...
        for (uint256 i = 0; i < _mintAmount; i++) {
            // _safeMint(to, tokenId)
            _safeMint(_caller, (supply + i));
            // _setTokenURI(tokenId, _tokenURI)
            _setTokenURI((supply + i), (supply + i + 1).toString());
        }
    }
