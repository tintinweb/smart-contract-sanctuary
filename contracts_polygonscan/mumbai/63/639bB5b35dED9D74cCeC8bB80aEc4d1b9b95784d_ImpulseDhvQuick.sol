...
interface IImpulseStrategy {
    ...
    function wantPriceInUsd(uint256 _wantAmt)
    external
    view
    returns (uint256);
}
...
contract BaseImpulseStrategy is IImpulseStrategy, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    uint256 internal commissionAccuracy;
    address swapToUnderlyingToken;
    address usdToken;
    ...
    function initialize(
        ...
        wantToken = ERC20(_wantToken);
        stakingRewards = IStakingRewards(_stakingRewards);
        router = _router;
        underlyingTokens = _underlyingTokens;
        rewardToken = ERC20(_rewardToken);
        commissionAccuracy = 10 ** rewardToken.decimals();
    ...
    // Main want token compounding function
    function earn() nonReentrant external override onlyRole(BACKEND_ROLE) virtual {
        _getRewards();
        ...
        if (rewardBalance >= DUST) {
            if (commission > 0 && treasury != address(0)) {
                ...
                if (rewardCommission > 0) {
                    rewardBalance = rewardBalance - rewardCommission;
                    rewardToken.transfer(treasury, rewardCommission);
                }
            }
    ...
    function deposit(
        uint256 _wantAmt
    ) nonReentrant external override onlyRole(STRATEGIST_ROLE) returns (uint256) {
        uint256 shares = 0;
        ...
        wantTotal = wantTotal + _wantAmt;
        totalSupplyShares = totalSupplyShares + shares;
    ...
    function _swapRewards(uint256 _rewardAmount) virtual internal {
    ...
    function _depositLpToken(uint256 _wantAmt) virtual internal {
    ...
    /// @param _wantAmt Amount of want token.
    function _withdrawLpToken(uint256 _wantAmt) virtual internal {
        stakingRewards.withdraw(_wantAmt);
    }

    /// @notice Get rewards from staking contract.
    function _getRewards() virtual internal {
        stakingRewards.getReward();
    }

    /// @notice Swap rewards token to underlying tokens.
    /// @param _rewardAmt Amount of rewards token.
    function _swapRewardsToUnderlings(uint256 _rewardAmt) virtual internal {
        _swapRewards(_rewardAmt / 2);
    }

    /// @notice Swap all underlying tokens to want tokens.
    /// @return Want received amount.
    function _swapAllUnderlyingToWant() virtual internal returns (uint256) {
        uint256 [] memory amounts = new uint256[](2);
        for (uint256 i = 0; i < 2; i++) {
            amounts[i] = ERC20(underlyingTokens[i]).balanceOf(address(this));
            ERC20(underlyingTokens[i]).safeApprove(router, 0);
            ERC20(underlyingTokens[i]).safeApprove(router, amounts[i]);
        }
    ...
    /// @param _wantAmt Shares amount.
    /// @return Price of shares in usd.
    function wantPriceInUsd(uint256 _wantAmt) external view override virtual returns (uint256) {
        require(usdToken != address(0), "Wrong usd address");
        uint256 result = 0;
        uint256 lpTotalSupply = wantToken.totalSupply();
        for (uint256 i = 0; i < underlyingTokens.length; i++) {
            uint256 amount = ERC20(underlyingTokens[i]).balanceOf(address(wantToken));
            address [] memory path = swapRewardRoutes[underlyingTokens[i]][usdToken];
            result = result + IUniswapV2Router01(router).getAmountsOut(
                _wantAmt * amount / lpTotalSupply,
                path
            )[path.length - 1];
        }
        return result;
    ...
    function adminWithdraw(address _token) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        if (balance > 0) {
            ERC20(_token).transfer(_msgSender(), balance);
        }
...
// File: ImpulseDhvQuick.sol

contract ImpulseDhvQuick is BaseImpulseStrategy {
