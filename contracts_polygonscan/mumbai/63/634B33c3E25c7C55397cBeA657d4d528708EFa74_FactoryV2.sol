...
contract FactoryV2 is AccessControl {
    ...
    event SetAddresses(address moneyHandler, address treasury);
    ...
    function addExternalAddresses(address _moneyHandler, address _treasury)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        emit SetAddresses(_moneyHandler, _treasury);
    }
...
contract CollectionV2 is ERC1155, AccessControl {
    ...
    event SetAddresses(address token, address treasury, address moneyHandler);
    ...
    constructor(CollectionData memory collecData) ERC1155(collecData.uri) {
        ...
        childChainManager = 0x195fe6EE6639665CCeB15BCCeB9980FC445DFa0B;
    ...
    function addExternalAddresses(
        ...
        emit SetAddresses(_token, _treasury, _moneyHandler);
    }
    ...
    function setChildChainManger(address childChain)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        revokeRole(DEPOSITOR_ROLE, childChainManager);

        childChainManager = childChain;

        grantRole(DEPOSITOR_ROLE, childChain);
    }
