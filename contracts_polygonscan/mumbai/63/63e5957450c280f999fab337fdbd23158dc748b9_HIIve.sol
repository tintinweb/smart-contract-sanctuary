...
contract HIIve is Context, IERC20, Ownable,Pausable{
    using SafeMath for uint256;

    //Token related variables
    uint256 public _totalSupply = 1000000 * 10 ** 18;
    uint256 public maxTxLimit = 100000* 10 ** 18;
    string _name = 'HiveTest';
    string _symbol = 'HIVET';
    uint8 _decimals = 18;
    ...
    function hiveFee() public view returns (uint256) {
        return _HiveFee;
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 amount) private {
        ...
        (uint256 tAmount, uint256 rAndDFee, uint256 marketingFee) = _getCalculatedFees(amount);
    ...
    function _getCalculatedFees(uint256 amount) internal view returns(  uint256, uint256, uint256) {
        ...
        fee.rAndDFee = amount.mul(_HiveFee).div(100);
        fee.marketingFee = amount.mul(_TreasuryFee).div(100);

        uint256 deductedAmount = amount.sub(fee.rAndDFee.add(fee.marketingFee));

        return (deductedAmount, fee.rAndDFee, fee.marketingFee);
    }
    ...
    constructor(address _Treasury, address _Hive) {
