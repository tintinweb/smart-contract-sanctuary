...
// BrewEco is MasterChef's left hand and kinda a wizard. He can brew Eco from pretty much anything!
// This contract handles "serving up" rewards for xEco holders by trading tokens collected from fees for Eco.

// T1 - T4: OK
contract EcoMaker is Ownable {
    ...
    modifier onlyAuth() {
        require(isAuth[msg.sender] || anyAuth, "BrewEco: FORBIDDEN");
        _;
    }
    ...
    function setBridge(address token, address bridge) external onlyOwner {
        // Checks
        require(
            token != Eco && token != wmatic && token != bridge,
            "BrewEco: Invalid bridge"
        );
    ...
    modifier onlyEOA() {
        ...
        require(msg.sender == tx.origin, "BrewEco: must use EOA");
        _;
    }

    // F1 - F10: OK
    // F3: _convert is separate to save gas by only checking the 'onlyEOA' modifier once in case of convertMultiple
    // F6: There is an exploit to add lots of Eco to the xEco, run convert, then remove the Eco again.
    //     As the size of the EcoxEco has grown, this requires large amounts of funds and isn't super profitable anymore
    ...
    function _convert(address token0, address token1) internal {
        ...
        if(token0 == token1) {
            ...
            require(address(pair) != address(0), "BrewEco: Invalid pair");
    ...
    // F1 - F10: OK
    // C1 - C24: OK
    // All safeTransfer, _swap, _toEco, _convertStep: X1 - X5: OK
    function _convertStep(
        address token0,
        address token1,
        uint256 amount0,
        uint256 amount1
    ) internal returns (uint256 EcoOut) {
        // Interactions
        if (token0 == token1) {
            uint256 amount = amount0.add(amount1);
            if (token0 == Eco) {
                IERC20(Eco).safeTransfer(xEco, amount);
                EcoOut = amount;
            } else if (token0 == wmatic) {
                EcoOut = _toEco(wmatic, amount);
            } else {
                address bridge = bridgeFor(token0);
                amount = _swap(token0, bridge, amount, address(this));
                EcoOut = _convertStep(bridge, bridge, amount, 0);
            }
        } else if (token0 == Eco) {
            // eg. Eco - ETH
            IERC20(Eco).safeTransfer(xEco, amount0);
            EcoOut = _toEco(token1, amount1).add(amount0);
        } else if (token1 == Eco) {
            // eg. USDT - Eco
            IERC20(Eco).safeTransfer(xEco, amount1);
            EcoOut = _toEco(token0, amount0).add(amount1);
        } else if (token0 == wmatic) {
            // eg. ETH - USDC
            EcoOut = _toEco(
                wmatic,
    ...
    function _swap(
        ...
        require(address(pair) != address(0), "BrewEco: Cannot convert");
    ...
    // F1 - F10: OK
    // C1 - C24: OK
    function _toEco(address token, uint256 amountIn)
        internal
    ...
    {   
        ...
        amountOut = _swap(token, Eco, amount, xEco);
    }

    function getAmountOut(uint amountIn, uint reserveIn, uint reserveOut) internal pure returns (uint amountOut) {
        require(amountIn > 0, 'BrewEco: INSUFFICIENT_INPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'BrewEco: INSUFFICIENT_LIQUIDITY');
