...
contract SlavSquad2 is ERC721, Ownable {
    ...
    uint public giveawayMaxItems = 3;
    uint public preMintMaxItems = 3;
    ...
    constructor() ERC721("SlavSquadTEST2", "GOPNIKTEST2") {}
    ...
    function preMint(address to, uint amount) external payable {
        require(!preMintPaused, "preMint: Paused");
        require(preMintCount + amount <= preMintMaxItems, "preMint: Surpasses cap");
        require(amount <= preMints[to], "preMint: Amount greater than allocation");
        preMintCount += amount;
        preMints[to] -= amount;
        _mintWithoutValidation(to, amount);
    }

    function publicMint(address to, uint amount) external payable {
