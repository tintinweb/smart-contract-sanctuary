...
contract Exchange is OrderState, TransferSafe {
    ...
    function sell(Order calldata order, Sig calldata sig, Sig calldata buyerFeeSig, address[] memory juryReceipt) payable external {
    ...
    function buy(Order calldata order, Sig calldata sig, Sig calldata buyerFeeSig, address[] memory juryReceipt) payable external {
       require((block.timestamp<= order.expiryTime), "Signature expired" );
    ...
    function bid(Order calldata order, Sig calldata sig,Sig calldata buyerSig, Sig calldata buyerFeeSig, address buyer, uint256 buyingAmount, 
        address[] memory juryReceipt) payable external {
        require((block.timestamp <= order.expiryTime ), "Signature expired" );
