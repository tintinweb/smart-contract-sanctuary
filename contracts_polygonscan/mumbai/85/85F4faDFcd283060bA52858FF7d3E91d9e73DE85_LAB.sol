...
contract LAB {
    address public biveAddress;
    address public bizAddress;
    address public vraAddress;
    ...
    constructor(){
        biveAddress = 0x2Fbee4B84918a92097503f7336bDE49a2AAaB58F;
        bizAddress = 0x137bE3Af4357a607Bc4DfEDB0dfd3893AFd21E6D;
        vraAddress = 0xc1C2D0754A2d4a0F79F248a0e4E6E9f87e6b0D99;
        BIVE = IERC20(biveAddress);
        VRA = IERC20(vraAddress);
    }
    ...
    function swapVRA(uint amountIn) public returns (uint amountOut){
        ...
        bool minted = IBIZ(bizAddress).labMint(msg.sender,amountIn);
        if (!minted) revert();
        amountOut = amountIn;
    }
    ...
    function swapExactBiveToBiz(
        ...
        require(BIVE.transferFrom(msg.sender, address(this), amountIn), "Swap BIVE to BIZ: Transfer VRA failed");
        address[] memory path;
        path[0] = biveAddress;
        path[1] = bizAddress;
        uint[] memory amounts = IBizverseRouter01(router).swapExactTokensForTokens(amountIn, amountOutMin,path,msg.sender, deadline);
        if (amounts.length == 0) revert();
        bool minted = IBIZ(bizAddress).labMint(msg.sender,amounts[0]);
        if (!minted) revert();
        return true;
    }
