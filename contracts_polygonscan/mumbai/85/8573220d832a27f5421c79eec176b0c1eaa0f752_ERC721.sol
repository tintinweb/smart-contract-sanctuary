 ...
 */
    ...
    function setTokenId(address tokenerc) public onlyOwner{
        dappToken = IERC20(tokenerc);
    }
     ...
     */
    function mintTokenByETH(uint8 _mrId, uint256 _count , uint256 amount) public  {
        require(_count > 0);
        ...
        require(
            amount == mrInfos[_mrId].mrPrice * _count,
    ...
    function getUserMrPrice(uint256 _tokenId) public view returns (uint256) {
        ...
        mrCount[mrIdBurnt] = mrCount[mrIdBurnt] - 1;
        mrBurnCount[mrIdBurnt] = mrBurnCount[mrIdBurnt]+ 1;
        _burn(_tokenId);
    }


    // buy item from MarketPlace
    function buyItemFromMarketPlace(uint256 tokenId, uint256 amount) public {
        require(ownedNftState[tokenId] == 1, "Token not in sell state");
        uint256 price = getUserMrPrice(tokenId);
        require(price > 0, "mrid price shoud be greater than sale price");
        require(amount >= price , "price shoud be greater than sale price");
        ...
        dappToken.transfer(msg.sender, amount - fee);
    ...
    function setMrPrice(uint8 mrId_, uint256 price_) public onlyOwner {
     ...
     function withdraw(uint256 amount) public onlyOwner{
        require(amount <= dappToken.balanceOf(address(this)), "Not enough tokens in the reserve");
  ...
  /**
    ...
    function createBid(string memory tname, address addr, uint256 amount, uint256 Id) public {
        require(tokenType[tname].bid, "oops! Could not create a Bid with this Token");
        require(msg.sender != ownerOf(Id), "oops! you could not create a Bid");
    ...
    function placeBid(string memory bidToken,address addr, uint256 amount, uint256 Id) public {
        ...
        _placeBid(bidToken, addr, owner(), amount, Id);
        tokenTrans(Id, msg.sender, addr);
        tokenData[Id][addr].isCompleted = true;
    }

    function _placeBid(string memory tname,address from, address admin, uint256 amount, uint256 Id) internal{
        uint256 serviceValue = 10 * 1e18;
        ...
        require(msg.sender == ownerOf(Id), "oops! Is Not a Owner");
