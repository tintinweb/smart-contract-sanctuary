...
contract SlumStaking is ERC721Holder, ERC1155Holder, Ownable {
  ...
  mapping(address => Stake) private investors;
	...
	// verified
	function stakeSlumbo(uint256 _slumboId) external {
		Stake storage staked = investors[msg.sender];
		require(staked.slumboTickets.length < MAX_SLUMBO_TICKETS, "Stake: Max Slumboginis is 3");

		ISBOToken.safeTransferFrom(msg.sender, address(this), _slumboId);
		staked.slumboTickets.push(Ticket(block.timestamp, _slumboId));
	}

	// verified
	function stakeSlumdoge(uint256 _slumdogeId) external {
		Stake storage staked = investors[msg.sender];
		require(staked.slumdogeTickets.length < MAX_SLUMBO_TICKETS, "Stake: Max SlumdogeBillionaires is 3");

		ISDBToken.safeTransferFrom(msg.sender, address(this), _slumdogeId);
		staked.slumdogeTickets.push(Ticket(block.timestamp, _slumdogeId));
	}
