...
contract BoobdropNFT is ERC721URIStorage, Ownable, AccessControl, IChildToken {
  ...
  /**
   * Only admin to mint tokens. Should be minted with some generic default metadata
   * then updated with generated metadata from the token hash
   * Should verify if token is withdrawn by checking `withdrawnTokens` mapping
   * @param user user for whom tokens are being minted
   * @param uri metadata uri
   */
  function mint(address user, string memory uri) public onlyRole(DEFAULT_ADMIN_ROLE) {
    _tokenIds.increment();
