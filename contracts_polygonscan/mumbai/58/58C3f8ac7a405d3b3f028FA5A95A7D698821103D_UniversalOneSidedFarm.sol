...
contract UniversalOneSidedFarm is ReentrancyGuard {
    ...
    /**
    @notice This function is used to invest in given Uniswap V2 pair through any compatible ERC20 token
    ...
    function poolLiquidity(
        ...
        if(_fromToken != token0 && _fromToken != token1){
            if(_fromToken == address(0)) {
                toInvest = _swapTokensInternal(wmatic, _toToken, msg.value);
            }
            else{
                toInvest = _swapTokensInternal(_fromToken, _toToken, _fromTokenAmount);
            }
            _fromToken = _toToken;
        }
        ...
        hybridStaking.depositFor(_pid, lpBought, msg.sender);
