...
contract TokenplayToken is IERC20, Ownable {
...
contract PolygonSwapAgentImpl {
    ...
    event Initialized(
        uint256 fee,
        bytes32 indexed bscSwapInitTxHash,
    ...
    constructor(
        ...
        TokenplayToken tokenplayToken = new TokenplayToken();

        registeredBEP20 = bep20Addr;
        registeredERC20 = address(tokenplayToken);

        string memory name = IERC20(registeredERC20).name();
        string memory symbol = IERC20(registeredERC20).symbol();
        uint8 decimals = IERC20(registeredERC20).decimals();

        emit Initialized(
            fee,
            bscSwapInitTxHash,
            bep20Addr,
            address(tokenplayToken),
            symbol,
            name,
            decimals
        );
    ...
    function transferOwnershipOfGlitch(address payable newOwner)
        public
        onlyOwner
    {
        require(
        ...
        TokenplayToken(registeredERC20).transferOwnership(newOwner);
    }
    ...
    function fillBSC2PolygonSwap(
        ...
        for (uint256 i = 0; i < bscTxHashArr.length; i++) {
            ...
            TokenplayToken(registeredERC20).mintTo(amountArr[i], toAddressArr[i]);
     ...
     * @dev swapBSC2ETH
     */
    function swapPolygon2BSC(uint256 amount)
        external
        payable
        notContract
        returns (bool)
    {
        ...
        TokenplayToken(registeredERC20).burn(amount);
        if (msg.value != 0) {
