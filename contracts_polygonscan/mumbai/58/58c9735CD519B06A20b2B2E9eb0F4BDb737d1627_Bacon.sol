...
interface IFRYINGPAN {
	function randomBaconOwner(uint256 seed) external view returns (address);
}

interface IBacon {
	// struct to store each token's traits
	struct CopBacon {
		bool isSheep; //rest is handled in api/ipfs
		uint256 alphaRank; //1-10
	}

	function getPaidTokens() external view returns (uint256);

	function getTokenTraits(uint256 tokenId) external view returns (CopBacon memory);
}

//TODO: BEFORE GOING LIVE, PLEASE CHECK ALL TODOS!!
//TODO: BEFORE GOING LIVE, PLEASE CHECK ALL TODOS!!
//TODO: BEFORE GOING LIVE, PLEASE CHECK ALL TODOS!!
contract Bacon is IBacon, ERC721Enumerable, Ownable, ReentrancyGuard, PaymentSplitter {
	using Address for address;
	using Strings for uint256;
	// mint price
	uint256 public MINT_PRICE = .03 ether;
	...
	mapping(uint256 => CopBacon) public tokenTraits;
	...
	GREASE public greaseERC20;
	address private devWallet;

	//payment splitter
	address[] private addressList = [
		0x58EE23f28ACa78007D68c9E1B450FCE8B5aE6FE5, //m
		0x415365B365d618aceA382Dd9A2eb5cDAd59f5Adc //a
		//0xd32e3fA4a855b2688dbe6D5B867C5b3c7613c6cd
	];
	uint256[] private shareList = [95, 5];

	bool public locked; //metadata lock
	string public _contractBaseURI = "ipfs://xxxxxxxxxxx/metadata/";
	string public _contractURI = "ipfs://xxxxxxxxxxxx";
	...
	constructor(address _grease)
		ERC721("Bacon Game!", "BGAME")
		PaymentSplitter(addressList, shareList)
	{
		devWallet = msg.sender;
		greaseERC20 = GREASE(_grease);
	}

	function delete12122Me() internal pure {} //polyscan trick

	/**
	 * mint a token - 90% BACON, 10% COP BACON
	 * The first 20% are free to claim, the remaining cost $GREASE
	 */
	function mint(uint256 qty) external payable nonReentrant {
		require(totalSupply() + qty <= MAX_TOKENS, "All tokens minted");
		require(qty <= 5, "Invalid mint qty");
		if (totalSupply() < PAID_TOKENS) {
			//if there's presale
			require(totalSupply() + qty <= PAID_TOKENS, "All tokens on-sale already sold");
			require(qty * MINT_PRICE == msg.value, "Invalid payment amount");
		} else {
			require(msg.value == 0);
		}

		uint256 totalgreaseCost = 0;

		uint256 seed;
		for (uint256 i = 0; i < qty; i++) {
			seed = random(1, 2**256 - 1, totalSupply());
			address recipient = selectRecipient(seed);
			totalgreaseCost += mintCost(totalSupply() + 1);
			...
			uint256 isBaconInt = seed % 100; //90% sheep
			uint256 alpha = 1;
			bool isCop = true;
			if (isBaconInt >= 90) {
				isCop = false;
	 ...
	 * the remaining 80% have a 10% chance to be given to a random staked Bacon
	...
	function selectRecipient(uint256 seed) internal view returns (address) {
		...
		address thief = theFryingPan.randomBaconOwner(seed);
	...
	/** READ */
	function getTokenTraits(uint256 tokenId) external view override returns (CopBacon memory) {
	...
	//TODO: delete me when live!!!!!!!!!!!!!!!!!!
	function changePricePerToken(uint256 newPrice) external {
		MINT_PRICE = newPrice;
	}

	//TODO: delete me when live
	function setPaidTokensAmount(uint256 newAmount) external {
...
contract GREASE is ERC20, Ownable {
	// a mapping from an address to whether or not it can mint / burn
	mapping(address => bool) controllers;

	constructor() ERC20("$GREASE", "$GREASE") {}

	function delete3313Me() internal pure {} //polyscan trick
	...
	function reclaimERC20(ERC20 erc20Token) public onlyOwner {
		erc20Token.transfer(msg.sender, erc20Token.balanceOf(address(this)));
	}
}
