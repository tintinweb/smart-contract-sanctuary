    ...
    function fiatMethod(uint256 _type,address _nftAddress , address _to , uint256 _id,string memory uri ,uint _nftType)external{
        ...
        if(_type == 1){
            ...
            require(_nftAddress == address(_EXGOStore),"Invalid Token Address");
             erc1155safeMintwithuriTransferFrom(IEXGOStore(_EXGOStore),owner(),_to,_id,1,uri);  
        }
        else{
            require(_nftAddress != address(0x00),"Invalid Token Address");
            require(_nftType == 1 || _nftType == 2,"Invalid NFT Type");
            if (_nftType == 1){
                 erc721safeTransferFrom(IERC721(_nftAddress),_msgSender(),_to,_id);
            }
            else{
                erc1155safeTransferFrom(IERC1155(_nftAddress), _msgSender(),_to,_id,1);
            }
        }
    }
