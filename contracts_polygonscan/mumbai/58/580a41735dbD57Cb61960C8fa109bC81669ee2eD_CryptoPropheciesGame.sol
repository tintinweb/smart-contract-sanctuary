...
/// createBattle function adding validation of equal players' address
...
contract CryptoPropheciesGame is ReentrancyGuard, Ownable {
    ...
    event BattleFinished(address indexed account, uint256 amount);
    ...
    function createBattle(
        ...
        require(_challenger != _charger, "Players addresses are the same");
    ...
    function endBattle(uint256 _battleId, address _winner) external nonReentrant onlyGC {
        ...
        require(battle.challenger == _winner || battle.charger == _winner || _winner == address(0), "Invalid winner address");
    ...
    function _transferFundsToWinner(uint256 _battleId) internal {
        ...
        if (battle.winner == address(0)) {
            IERC20(TCP).safeTransfer(battle.challenger, (battle.TCPAmount - kingdomFee) / 2);
            IERC20(TCP).safeTransfer(battle.charger, (battle.TCPAmount - kingdomFee) / 2);
        } else {
        ...
        if (battle.winner == address(0)) {
            emit DailyPrizeTicketAdded(battle.challenger, (battle.TCPAmount * multipliers[chargerRarity][0]) / 2);
            emit DailyPrizeTicketAdded(battle.charger, (battle.TCPAmount * multipliers[chargerRarity][0]) / 2);
        }

        emit BattleFinished(battle.winner, battle.TCPAmount - kingdomFee);
    }
}
