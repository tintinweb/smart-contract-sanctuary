...
contract Marketplace is AdminAndTangibleAccess, IMarketplace, IERC721Receiver {
    ...
    function _payStorage(ITangibleNFT nft, uint256 tokenId, uint256 _years, uint256 tokenPrice) internal {
        require(_years > 0, "YZ");

        uint256 amount = factory.adjustStorageAndGetAmount(nft, tokenId, _years, tokenPrice);

        IFactory(factory).USDC().safeTransferFrom(msg.sender, IFactory(factory).feeStorageAddress(), amount);
