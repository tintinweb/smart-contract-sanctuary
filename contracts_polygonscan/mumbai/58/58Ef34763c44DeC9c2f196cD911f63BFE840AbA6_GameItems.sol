...
contract GameItems is ERC1155, Ownable {
    ...
    event BulletAmount(address _account, uint256 _amount);
    ...
    function checkTotalSupply(uint256 _id) public view returns (uint256) {
        require(totalSupply[_id] >= 0, "[checkTotalSupply] totalSupply is less than 0");
    ...
    function buyBullets(
      ...
      require(
        ...
        uint256 bulletAmount = balanceOf(_account, _id);

        emit BulletAmount(msg.sender, bulletAmount);
    }
    ...
    function burn(
        ...
        require(balanceOf(_account, _id) - _amount >= 0, "[burn] Can't burn item to be less than 0");
        require(totalSupply[_id] - _amount >= 0, "[burn] Can't burn totalSupply to be less than 0");
    ...
    function burnBatch(
        ...
        //add require _burnBatch
        _burnBatch(_account, _ids, _amounts);
        for (uint256 i = 0; i < _ids.length; ++i) {
            require(totalSupply[_ids[i]] - _amounts[i] >= 0, "[burnBatch] Can't burn totalSupply to be less than 0");
            totalSupply[_ids[i]] -= _amounts[i];
        }
    }
}
