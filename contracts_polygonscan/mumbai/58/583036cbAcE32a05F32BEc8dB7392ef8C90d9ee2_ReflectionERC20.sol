...
contract ReflectionERC20 is ERC20 , Ownable {
        ...
        if(whitlisted[from_].tax || whitlisted[to_].tax){
            ...
            if(taxAmount > 0) {
                ...
                if(from_ != wavaxPair && to_ != wavaxPair) {
                    _makeSwap( currentAppliedTax, taxAmount);
                } else {
                    _totalReserved += taxAmount;
                    // TODO: if the buy and sell are triggered one after the other the last one will be applied
                    // trying to be accurate will result in more gas and complications
                    _reservedTax = currentAppliedTax; 
                }
    ...
    function _makeSwap (Tax storage currentTax_, uint256 taxAmount) internal {
        uint256 totalToSwap = taxAmount.add(_totalReserved);
        uint256 swapOut = swapHundler.swapToAvax(totalToSwap);

        uint256 reserveShare = swapOut.mul(_totalReserved).div(totalToSwap);
        //uint256 currentShare = swapOut.sub(reserveShare);

        uint256 teamAmount = reserveShare.mul(_reservedTax.team).div(_reservedTax.total) + swapOut.sub(reserveShare).mul(currentTax_.team).div(currentTax_.total);
        uint256 holderAmount = reserveShare.mul(_reservedTax.holder).div(_reservedTax.total) + swapOut.sub(reserveShare).mul(currentTax_.holder).div(currentTax_.total);
        uint256 treasuryAmount = reserveShare.mul(_reservedTax.treasury).div(_reservedTax.total) + swapOut.sub(reserveShare).mul(currentTax_.treasury).div(currentTax_.total);
    ...
    function setBuyerTax (uint256 team_, uint256 holder_, uint256 treasury_, uint256 charity_) external onlyOwner{
        uint256 total = team_.add(holder_).add(treasury_).add(charity_);
        transferTax = Tax(
            team_,holder_,treasury_,charity_, total > 0 ? total : 1 // to avoid division by zero
        );
    }
...
contract SwapHandler is Ownable {
    ...
    function swapToAvax (uint256 amountIn_) isInSwap external returns (uint256 swapOut) {
