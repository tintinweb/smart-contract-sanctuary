...
contract RobeVesting is Ownable, ReentrancyGuard {
    ...
    modifier onlyIfVestingScheduleNotRevoked(bytes32 vestingScheduleId) {
        require(
            ...
            "Vesting schedule id not exist"
        );
        require(
            vestingSchedules[vestingScheduleId].revoked == false,
            "Vesting schedule id revoked"
        );
        _;
    }
     ...
     */
    function createVestingSchedule(address _beneficiary, uint256 _tokenId)
        public
        returns (bytes32)
    {
        require(
        ...
        uint256 totalAmount = totalEmmissionRate;

        if (getVestingSchedulesCountByBeneficiary(_beneficiary) != 0) {
            bytes32 vestingScheduleIdExist = this
                .computeCurrentVestingScheduleIdForHolder(_beneficiary);

            if (
                vestingSchedules[vestingScheduleIdExist].nftId == _tokenId &&
                vestingSchedules[vestingScheduleIdExist].beneficiary !=
                _beneficiary
            ) {
                vestingSchedules[vestingScheduleIdExist].end = getCurrentTime();
                uint256 amount = computeReleasableAmount(
                    vestingScheduleIdExist
                );
        ...
        vestingSchedulesTotalAmount = vestingSchedulesTotalAmount.add(
            totalAmount
        );
    ...
    function _computeClaimableAmount(VestingSchedule memory vestingSchedule)
        internal
        view
        returns (uint256)
    {
        ...
        if (
            ...
            currentTime < vestingSchedule.startClaimTime
        ) {
            return 0;
        } else if (
            currentTime >=
            vestingSchedule.start.add(vestingSchedule.duration).add(
                startCalimTimeInMinutes
            )
        ) {
            return vestingSchedule.amountTotal.sub(vestingSchedule.released);
        } else {
            uint256 timeFromStartClaim = currentTime.sub(
                vestingSchedule.startClaimTime
            );
            uint256 claimableAmount = timeFromStartClaim.mul(
                emmissionRatePerSecond
            );
     ...
     */
    function _computeVestableAmount(VestingSchedule memory vestingSchedule)
        internal
        view
        returns (uint256)
    {
