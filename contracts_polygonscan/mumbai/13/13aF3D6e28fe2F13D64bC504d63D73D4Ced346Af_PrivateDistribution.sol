...
// File contracts/PrivateDistribution.sol
...
contract PrivateDistribution is Ownable {
    ...
    IERC20 private _dfynToken;
    address[] public investors;

    enum AllocationType { SEED, PRIVATE }

    uint256[] private _privateVesting = [
    ...
    function withdrawTokens(AllocationType _allocationType) external onlyInvestor(_allocationType) initialized() {
        ...
        _dfynToken.safeTransfer(_msgSender(), tokensAvailable);
        ...
        returns (uint256 availablePercentage)
    {
        ...
        uint256 vestingDuration = _initialTimestamp + 526 days;
        uint256 noOfRemaingDays = BokkyPooBahsDateTimeLibrary.diffDays(currentTimeStamp,vestingDuration);
        ...
        if (currentTimeStamp > _initialTimestamp) {
            if (allocationType == AllocationType.PRIVATE) {
                ...
                uint256 initialReleasePercentage = uint256(0);
                uint256 remainingDistroPercentage = uint256(100);
