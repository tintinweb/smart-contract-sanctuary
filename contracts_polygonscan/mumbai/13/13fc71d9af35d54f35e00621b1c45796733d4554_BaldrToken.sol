...
// BaldrToken with Governance.
contract BaldrToken is BEP20 {
    ...
    // Addresses that are excluded from tx tax
    ...
    event BaldrSwapRouterUpdated(address indexed operator, address indexed router, address indexed pair);
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "BALDR::antiWhale: Transfer amount exceeds the maxTransferAmount");
            }
        }
        _;
    }
    ...
    /**
     * @notice Constructs the BaldrToken contract.
     */
    constructor() public BEP20("Baldr Token", "BALDR") {
    ...
    /// @dev overrides transfer BEP20 function to meet tokenomics of BALDR
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        ...
        if (recipient == BURN_ADDRESS || _excludedFromTrsfTax[sender] == true || _excludedFromTrsfTax[recipient] == true) {            
            ...
            require(amount == sendAmount + burnAmount, "BALDR::transfer: Burn value invalid");
    ...
    function updateBurnRate(uint16 _burnRateTax) public onlyOperator {
        require(_burnRateTax <= 200, "BALDR::updateBurnRate: Burn rate must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 10000, "BALDR::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "BALDR::transferOperator: new operator is the zero address");
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "BALDR::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "BALDR::delegateBySig: invalid nonce");
        require(now <= expiry, "BALDR::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "BALDR::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying BALDR (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "BALDR::_writeCheckpoint: block number exceeds 32 bits");
    ...
    //a way to get back other BEP20 tokens sended by error into the BALDR token contract
