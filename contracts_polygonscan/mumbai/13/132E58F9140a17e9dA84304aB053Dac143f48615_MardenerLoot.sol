...
contract MardenerLoot is ERC721Enumerable, ReentrancyGuard, Ownable {

    /**
    @dev COMPONENT COLORS */
    string[] colors = [ 
        "#E71414",
        "#FF4C29",
        "#FD5F00",
    ...
    function getEye(uint256 tokenId) public view returns(string memory) {
        ...
        uint256 eyeNum = rand % 9;
        if(eyeNum == 0) {
            string memory eyecolor = getEyeColor(tokenId);
            return MardenerEyes.laserEye(eyecolor);
        } else if(eyeNum == 1) {
            return MardenerEyes.robotEye();
        } else if(eyeNum == 2) {
            return MardenerEyes.tdEye();
        } else if(eyeNum == 3) {
            return MardenerEyes.thugEye();
        } else if(eyeNum == 4) {
            return MardenerEyes.heartEye();
        } else if(eyeNum == 5) {
            return MardenerEyes.smileEye();
        } else if(eyeNum == 6) {
            return MardenerEyes.brokenFace();
        } else if(eyeNum == 6) {
            return MardenerEyes.mustacheFace();
        } else {
            return MardenerEyes.wardenEye();
        }
    }
    ...
    function getBgColor(uint256 tokenId) public view returns(string memory) {
        return pluck(tokenId, "BACKGROUND", bgColors);
    }

    function getWingColor(uint256 tokenId) public view returns(string memory, string memory) { 
    ...
    function getBodyColor(uint256 tokenId) public view returns(string memory) {
        return pluck(tokenId, "BODY", colors);
    }

    function getRotation(uint256 tokenId) public view returns(string memory) {
        return pluck(tokenId, "Rotation", rotation);
    }
    ...
    function pluck(uint256 tokenId, string memory prefix, string[] memory sourceArray) internal pure returns (string memory) {
        ...
        /**
        @dev Background color is represent the rarity of MardenerLoot
        "#F0E5CF" = COMMON 
        "#149e22" = RARE
        "#f1e61d" = EPIC
        "#a80fc3" = LEGENDARY
        */
        if(MardenerUtils.compareString(prefix, "BACKGROUND")) {
    ...
    function tokenURI(uint256 tokenId) override public view returns(string memory) {
        ...
        parts[2] = MardenerUtils.getStyles(
            MardenerUtils.Styles(
        ...
        parts[3] = MardenerHead.getHead();
        parts[4] = getEye(tokenId);
        parts[5] = MardenerBody.getBody();
        parts[6] = MardenerSymbol.getSymbol();
        parts[7] = MardenerWing.getWing();
        parts[8] = '</svg>';
    ...
    function claim(uint256 tokenId) public nonReentrant {
        require(tokenId > 0 && tokenId < 2801, "Token ID invalid");
        _safeMint(_msgSender(), tokenId);
    }

    function ownerClaim(uint tokenId) public nonReentrant onlyOwner {
        require(tokenId > 2800 && tokenId < 3001 , "Token Id invalid");
        _safeMint(_msgSender(), tokenId);
    }
    ...
    constructor() ERC721("Mardener 16 bit", "MADTEST1") Ownable() {
    }
