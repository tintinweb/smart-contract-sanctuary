...
contract RefillERC20 is AccessControl, Pausable, Utils {
    ...
    event Recharge(uint256 indexed _maticAmount, uint256 indexed _ticketNumber, address indexed sender, string nonce);
    ...
    function recharge(uint256 _maticAmount, uint256 _tokenNumber, bytes32 hash, bytes memory signature, string memory nonce) public payable whenNotPaused{
        //验 证hash
        require(hashRechargeTransaction(_maticAmount, _tokenNumber, msg.sender, nonce) == hash, "Invalid hash!");
        //验证签名
        require(matchAddressSigner(hash, signature), "Invalid signature!");
        require(msg.value >= _maticAmount, "The ether of be sent must be more than the maticAmount!");
        payable(address(this)).transfer(_maticAmount);
        if (msg.value > _maticAmount) {
            payable(msg.sender).transfer(msg.value - _maticAmount);
        }
        emit Recharge(_maticAmount, _tokenNumber, msg.sender, nonce);
    }
    ...
    function withdraw(uint256 _amount,address _contractAddress, uint256 _tokenId, address _fromAddress,bytes32 hash, bytes memory signature, string memory nonce) public whenNotPaused{
        ...
        if (_fromAddress != address(0)){
            ...
            assert(_mintNFT(_amount,_contractAddress, _tokenId));
        }
        emit WithdrawNFT(_amount, _contractAddress, _tokenId, _fromAddress, msg.sender, nonce);
    }

    function batchWithdraw(uint256[] memory _amountArray,address[] memory _contractAddressArray, uint256[] memory _tokenIdArray, address[] memory _fromAddressArray, bytes32 hash, bytes memory signature, string memory nonce) public whenNotPaused{
        //验证hash
        ...
        for (uint256 i = 0; i < _amountArray.length; i++) {
            if (_fromAddressArray[i] != address(0)){
                ...
                assert(_mintNFT(_amountArray[i],_contractAddressArray[i],_tokenIdArray[i]));
                ++totalNum;
            }
        }
    ...
    function _mintNFT(uint256 _amount,address _contractAddress, uint256 _tokenId) internal returns(bool){
        if (_checkProtocol(_contractAddress) == 1) {
            Common1155NFT withdrawNFTContract = Common1155NFT(_contractAddress);
            withdrawNFTContract.mint(msg.sender, _tokenId, _amount,abi.encode(msg.sender));
            return true;
        }
        if (_checkProtocol(_contractAddress) == 2) {
...
//    }
    ...
    function hashRechargeTransaction(uint256 _maticAmount, uint256 _ticketNumber, address sender, string memory nonce) private pure returns (bytes32) {
        bytes32 hash = keccak256(
            abi.encodePacked(
                "\x19Ethereum Signed Message:\n32",
                keccak256(abi.encodePacked(_maticAmount, _ticketNumber, sender, nonce, "ichiban_recharge"))
            )
        );
        return hash;
    }
    ...
    function withdrawETH() public payable {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender));
        uint256 ethAmount = address(this).balance - 0.01 ether;
