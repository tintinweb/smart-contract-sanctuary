...
contract BizverseWorldERC20 is ERC20Pausable, ERC20Blacklistable, Ownable {
    ...
    /// @dev Only can set lab
    function setLab(address labAddress_) public virtual onlyOwner returns (bool) {
        require(labAddress != labAddress_);
        labAddress = labAddress_;
        return true;
    }

    /// @dev Lab can mint
    function labMint(address to, uint256 amount)
        public
        virtual
        returns (bool)
    {
        require(_msgSender() == labAddress,"Only Bizverse Lab");
        _mint(to, amount);
        return true;
    }
 ...
 */
contract BIZ is BizverseWorldERC20, AllowanceWhitelistable {
    constructor(uint256 initialSupply) BizverseWorldERC20("BUSINESS", "BIZ") {
