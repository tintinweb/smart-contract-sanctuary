...
/**
 * @title FXN | Randolph Security Token
 * @author Stobox Technologies Inc.
 * @dev FXN ERC20 Token | This contract is opt for digital securities management.
 */
 
contract FXNToken is IERC20, Roles {
    ...
    modifier onlyWithUnfrozenFunds(address _account) {
        require(
            ...
            "FXN: funds are frozen."
        );
        _;
    }

    /**
     * @notice FXNToken simply implements a ERC20 token.
     */
    constructor() public Roles(msg.sender) {
        _name = "FraXion Security Token";
        _symbol = "FXN";
        _decimals = 0;
        _kDecimals = 18;
        _k = 10**_kDecimals;
    ...
    function burn(address _account, uint256 _amount) external onlySuperAdmin {
        require(balanceOf(_account) >= _amount, "FXN: balance too low");
    ...
    function transferFunds(
        ...
        require(balanceOf(_from) >= _amount, "FXN: not enough tokens");
    ...
    function splitOrMerge(uint256 _x, uint256 _y) external onlySuperAdmin {
        require(_x != _y, "FXN: _x must not be equal to _y");
        require(_x > 0, "FXN: _x must be greater than 0");
        require(_y > 0, "FXN: _y must be greater than 0");
    ...
    */
        ...
        returns (bool)
    {
        ...
        _approve(
            ...
            _allowances[_sender][msg.sender].sub(
                ...
                "FXN: transfer amount exceeds allowance"
            )
        );
        ...
        returns (bool)
    {
        _approve(
            ...
            _allowances[msg.sender][_spender].sub(
                ...
                "FXN: decreased allowance below zero"
            )
        );
    ...
    function _transfer(
        ...
        require(_sender != address(0), "FXN: transfer from the zero address");
        require(_recipient != address(0), "FXN: transfer to the zero address");

        _beforeTokenTransfer(_sender, _recipient, _amount);

        _balances[_sender] = _balances[_sender].sub(
            _amount,
            "FXN: transfer amount exceeds balance"
        );
    ...
    function _mint(address _account, uint256 _amount) internal virtual {
        require(_account != address(0), "FXN: mint to the zero address");
    ...
    function _burn(address _account, uint256 _amount) internal virtual {
        require(_account != address(0), "FXN: burn from the zero address");

        _beforeTokenTransfer(_account, address(0), _amount);

        _balances[_account] = _balances[_account].sub(
            _amount,
            "FXN: burn amount exceeds balance"
        );
    ...
    function _approve(
        ...
        require(_owner != address(0), "FXN: approve from the zero address");
        require(_spender != address(0), "FXN: approve to the zero address");
    ...
    function _updateTransferLimit(address _account, uint256 _amount) private {
        ...
        _transferLimits[_account].allowedToTransfer = _transferLimits[_account]
            .allowedToTransfer
            .sub(_amount, "FXN: transfer exceeds your transfer limit");
    }
    ...
    function _updateTransactionCountLimit(address _account) private {
        if (_isEnabledTransactionCount) {
            ...
            _transactionCountLimits[_account]
                ...
                .sub(1, "FXN: transfer exceeds your transaction count limit");
        }
    }
}