...
/**
 * @title TradeablePolygonERC721Token
 * TradeablePolygonERC721Token - ERC721 contract that whitelists a trading address, and has minting functionality (Polygon network).
 */

contract TradeablePolygonERC721Token is TradeableERC721Token, ContextMixin, NativeMetaTransaction {

    constructor(string memory  _name, string memory  _symbol, string memory _contractUri) TradeableERC721Token(_name, _symbol, _contractUri) {
...
contract TradeableERC721Token is ERC721, ERC721URIStorage, Pausable, Ownable, ERC721Burnable, AccessControl {
    ...
    function safeMintWithURI(address to, string memory _tokenURI) public onlyRole(MINTER_ROLE) {
        _safeMint(to, _tokenIdCounter.current());
