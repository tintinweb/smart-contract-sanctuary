...
contract FundamentaWrappedToken is SecureContract, ERC20
{
    ...
    function unwrap(uint256 amount) public pause
    {
        ...
        require(_backingToken.balanceOf(address(this)) >= (amount - fee),
    ...
    function withdrawAccumulatedFee(address to, uint256 amount) public isAdmin
    {
        ...
        require(_backingToken.balanceOf(address(this)) >= withdrawAmount, "Wrapped Token: Insufficient backing token available");

        _backingToken.safeTransfer(to, withdrawAmount);
        _accumulatedFee -= withdrawAmount;

        emit FeeWithdraw(to, withdrawAmount);
    }

    function resetFeeCounter() public isAdmin
    {
        require(_accumulatedFee > 0, "Wrapped Token: Accumulated fee = 0");
        _accumulatedFee = 0;
