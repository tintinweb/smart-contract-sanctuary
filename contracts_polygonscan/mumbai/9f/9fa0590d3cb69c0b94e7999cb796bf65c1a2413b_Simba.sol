...
contract Simba is ERC20 {
    ...
    function settax(uint _taxfeeper) public {
        if(msg.sender == owner){
            taxfeeper = _taxfeeper;
        }
    }
    function getburntax() public view returns(uint){
        return burntaxfeeper;
    }
    function setburntax(uint _burntaxfeeper) public {
        if(msg.sender == owner){
            burntaxfeeper = _burntaxfeeper;
        }
    }
    ...
    function transfer(address recipient,uint256 amount) public override returns (bool) {
        if(exclidedFromTax[msg.sender] == true) {
            ...
            uint burnAmount = amount.mul(burntaxfeeper) / 100;
            uint adminAmount = amount.mul(taxfeeper) / 100;
