...
contract MadTEST is Ownable, ReentrancyGuard, AccessControl {
    ...
    struct UserInfo {
        ...
        uint256 nftID;
        bool hasNFT;
        uint256 powerStaking;          // User power
        uint256 powerAmount;          // User power
    }
    ...
    function withdraw() external nonReentrant {
        ...
        uint256 pending = _calculatedUserAmount().mul(accTokenPerShare).div(PRECISION_FACTOR).sub(user.rewardDebt);
        ...
        if (user.hasNFT){
            uint256 lastExperience;
            lastExperience = _getNFTExperience(user.nftID);
            theEndlessFactoryNFT.setExperience(user.nftID, lastExperience.add(100));
        }
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        uint256 calculatedAmount = _calculatedUserAmount();
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            ...
            return calculatedAmount.mul(accTokenPerShare).div(PRECISION_FACTOR).sub(user.rewardDebt);
        }
    }
    ...
    function _userHasSandManNFT(uint256 _nftID) internal  {
        UserInfo storage user = userInfo[msg.sender];        
        
        if (theEndlessFactoryNFT.ownerOf(_nftID) == msg.sender){
          user.hasNFT = true;
          user.nftID = _nftID;
    ...
    function _updatePower() internal {
        ...
        uint256 powerStaking = user.powerStaking + factor;
        
        user.powerAmount = (user.amount * powerStaking).div(factor);
        
        poolInfo.totalPower = poolInfo.totalPower.add(user.powerStaking);

    }

    function _userHarvest() internal {
        UserInfo storage user = userInfo[msg.sender];
        
        if (user.amount > 0) {
            uint256 newUserAmount = _calculatedUserAmount();
            uint256 pending = newUserAmount.mul(poolInfo.accRewardPerShare).div(1e18).sub(user.rewardDebt);
            if (pending > 0) {
    ...
    function _calculatedUserAmount() public view returns (uint256) {
        UserInfo storage user = userInfo[msg.sender];
    ...
    function _updateHarvestLookup() internal{
        UserInfo storage user = userInfo[msg.sender];

        if (user.hasNFT)
            user.nextHarvestUntil = block.timestamp.add(harvestInterval / 2);
        else
    ...
    function _getNFTPowerStaking(uint256 _nftID) internal view returns (uint256) {
        ...
        ,
        ,
        ) = theEndlessFactoryNFT.characters(0);
