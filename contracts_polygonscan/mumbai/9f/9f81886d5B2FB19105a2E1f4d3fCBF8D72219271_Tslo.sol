...
contract Tslo is ERC20,VRFConsumerBase {
    ...
    mapping(address => uint256) playerCredits;
    ...
    function transferCredits(uint256 amount) public{
        IERC20 token = IERC20(_blblToken);
        token.transferFrom(msg.sender,address(this),amount);
        emit TransferCredits(msg.sender,amount);
    }
    ...
    fallback () external payable {
        // player name will be unknown
        emit FallbackReceived2(msg.sender,msg.value,msg.data);
    }
    ...
    event FallbackReceived2(address user, uint amount, bytes data);
}