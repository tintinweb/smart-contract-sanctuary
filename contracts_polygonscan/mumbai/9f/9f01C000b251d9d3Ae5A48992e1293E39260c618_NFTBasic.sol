...
contract NFTBasic is ERC721Enumerable, Ownable{
    ...
    mapping(uint256=>NftFarm) farms;

    event minting(uint256 id, uint256 allocPoint);

    constructor(IERC20 _rewardToken) ERC721("NFTBasic", "NFTBasic") {
        _baseTokenURI = "https://nft.polyshield.finance/";
    ...
    function mint_NFT(uint256 allocPoint) public payable onlyOwner returns (uint256){
        require(totalSupply() <= 5, "enough minting");

        _tokenIds.increment();
        uint256 newCharacter = _tokenIds.current();

        _mint(msg.sender, newCharacter);
        farms[newCharacter] = NftFarm({
            allocPoint: allocPoint,
            claimedAmount: 0
        });
        totalAllocPoint += allocPoint;
        emit minting(newCharacter, allocPoint);
    ...
    function transferFrom(address from, address to, uint256 tokenId) public override {
        //solhint-disable-next-line max-line-length
        require(_isApprovedOrOwner(_msgSender(), tokenId), "ERC721: transfer caller is not owner nor approved");
        uint256 claimableAmount = getClaimableAmount(tokenId);
    ...
    */
    ...
    function getClaimableAmount(uint256 tokenId) public view returns (uint256) {
        ...
        uint256 allocReward = (totalBalance+totalClaimedAmount)*(farms[tokenId].allocPoint)/(totalAllocPoint);
        return allocReward-farms[tokenId].claimedAmount;
    }
    ...
    function claim(uint256 tokenId) public {
        ...
        uint256 claimableAmount = getClaimableAmount(tokenId);
        rewardToken.transfer(owner, claimableAmount);
        totalClaimedAmount += claimableAmount;
        farms[tokenId].claimedAmount += claimableAmount;
    }
}