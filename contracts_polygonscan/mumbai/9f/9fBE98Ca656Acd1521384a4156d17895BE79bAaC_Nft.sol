...
contract Nft is AllowlistMinting, IERC721Metadata, ERC721Enumerable, ERC721Burnable, ERC721Pausable, Ownable {
    ...
    bool public treasuryGenesisMinted = false;
    uint256 constant public TREASURY_TOKENS = 20;
    bool public treasuryGenerativeMinted = false;
    ...
    constructor(address payable payeeAddress, address payable treasuryAddress, string memory envBaseURI, string memory envContractURI) ERC721("Crypto Cunts NFT", "CCNFT") {
        ...
        _payee = payeeAddress;
        _mintGenesisAndTreasury(treasuryAddress);
    }
    ...
    function _mintGenesisAndTreasury(address _addressToMint) internal {
        mintGenesisTreasury(_addressToMint);
        mintGenerativeTreasury(_addressToMint);
    }
    function mintGenesisTreasury(address _addressToMint) public onlyOwner {
        require(!treasuryGenesisMinted, "Treasury Genesis Minted");
    ...
    function mintGenerativeTreasury(address _addressToMint) public onlyOwner {
        require(!treasuryGenerativeMinted, "Treasury Generative Minted");
        // mint to treasury address - generative artwork
        for (uint256 i = 0; i < TREASURY_TOKENS; i++) {
            uint256 tokenId = _tokenIdTracker.current();
            _tokenIdTracker.increment();
            _dnaMapping[tokenId] = _random();
            _safeMint(_addressToMint,tokenId);
        }
        treasuryGenerativeMinted=true;
    }
