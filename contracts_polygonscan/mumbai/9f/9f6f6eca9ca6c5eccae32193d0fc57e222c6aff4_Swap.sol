...
contract Swap is ERC20BurnableUpgradeable, CrossChainStableCoinPool, Governable, ReentrancyGuardUpgradeable {
    using SafeERC20Upgradeable for IERC20Upgradeable;
    using SafeMathUpgradeable for uint256;
    IBridge internal bridge;
    string[3] internal toTokens;
    event RequestBridge(string[3] _toTokens, uint256[3] amountsOut);
    event ClaimToken(string[3] _toTokens, uint256[3] amountsOut);

    function initialize(address _bridgeContract, address _token0, address _token1, address _token2) public initializer {
        __Governable_initialize();
        __CrossChainStableCoinPool_initialize(_token0, _token1, _token2);
...
function requestBridge(
    ...
    function claimToken(
        ...
        if(keccak256(abi.encodePacked(_name)) == keccak256(abi.encodePacked(ERC20Upgradeable(token0).name()))){
            uint256[3] memory amountsIn = [_amount, 0, 0];
            swapToken(amountsIn, _amountsOut, _toAddr);
        } else if(keccak256(abi.encodePacked(_name)) == keccak256(abi.encodePacked(ERC20Upgradeable(token1).name()))){
            uint256[3] memory amountsIn = [0, _amount, 0];
            swapToken(amountsIn, _amountsOut, _toAddr);           
        } else if(keccak256(abi.encodePacked(_name)) == keccak256(abi.encodePacked(ERC20Upgradeable(token2).name()))){
            uint256[3] memory amountsIn = [0, 0, _amount];
            swapToken(amountsIn, _amountsOut, _toAddr); 
        }
...
// ADDLIQUIDITY FUNCTION
        ...
        returns (uint256)
    {
        ...
        if (_totalSupply == 0) {
            ...
            totalReceivedLP = totalAddIn.mul(_totalSupply).div(totalPoolValue);
        }
...
// WITHDRAW LIQUIDITY FUNCTION
    function withdrawLiquidity(
        ...
        totalMinusLP = totalOut.mul(_totalSupply).div(totalPoolValue);
        ...
        __burn(msg.sender, totalMinusLP);
    }
}
...
interface ICrossChainStableCoinLP {
    event __Approval(address indexed owner, address indexed spender, uint value);
    event __Transfer(address indexed from, address indexed to, uint value);
    ...
    function _allowance(address owner, address spender) external view returns (uint);

    function __approve(address spender, uint value) external returns (bool);
    function __transfer(address to, uint value) external returns (bool);
    function __transferFrom(address from, address to, uint value) external returns (bool);

    function DOMAIN_SEPARATOR() external view returns (bytes32);
    function PERMIT_TYPEHASH() external pure returns (bytes32);
    function nonces(address owner) external view returns (uint);

    function __permit(address owner, address spender, uint value, uint deadline, uint8 v, bytes32 r, bytes32 s) external;
}
...
contract CrossChainStableCoinLP is
    DTOUpgradeableBase,
    ICrossChainStableCoinLP,
    ChainIdHolding
{
    using SafeMath for uint256;

    string public constant override _name = "DTO CCSC LP";
    string public constant override _symbol = "CCSC-LP";
    uint8 public constant override _decimals = 18;
    uint256 public override _totalSupply;
    mapping(address => uint256) public override _balanceOf;
    mapping(address => mapping(address => uint256)) public override _allowance;
    ...
    function __mint(address to, uint256 value) internal {
        _totalSupply = _totalSupply.add(value);
        _balanceOf[to] = _balanceOf[to].add(value);
        emit __Transfer(address(0), to, value);
    }

    function __burn(address from, uint256 value) internal {
        _balanceOf[from] = _balanceOf[from].sub(value);
        _totalSupply = _totalSupply.sub(value);
        emit __Transfer(from, address(0), value);
    }
    ...
    function ___approve(
        ...
        emit __Approval(owner, spender, value);
    }

    function ___transfer(
        address from,
        address to,
        uint256 value
    ) private {
        _balanceOf[from] = _balanceOf[from].sub(value);
        _balanceOf[to] = _balanceOf[to].add(value);
        emit __Transfer(from, to, value);
    }

    function __balanceOf(address from) internal view returns (uint256) {
        return _balanceOf[from];
    }

    function __approve(address spender, uint256 value)
        external
        override
        returns (bool)
    {
        ___approve(msg.sender, spender, value);
        return true;
    }

    function __transfer(address to, uint256 value)
        external
        override
        returns (bool)
    {
        ___transfer(msg.sender, to, value);
        return true;
    }
    ...
    function __permit(
        ...
        require(deadline >= block.timestamp, "CrossChainStableCoinLP: EXPIRED");
        ...
        require(
            ...
            "CrossChainStableCoinLP: INVALID_SIGNATURE"
        );
        ___approve(owner, spender, value);
    }
}
