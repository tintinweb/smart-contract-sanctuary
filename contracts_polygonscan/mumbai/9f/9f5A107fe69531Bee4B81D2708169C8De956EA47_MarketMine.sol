...
// File: contract/titan/libs/token/ERC20/ERC20.sol
...
// File: contract/titan/core/titanToken/TitanToken.sol
...
contract TitanToken is Context, ERC20, Ownable, ITitan, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    modifier onlyAuthorizedMintCaller() {
        require(_msgSender() == owner() || _authorizedMintCaller[_msgSender()],"TitanToken: MINT_CALLER_NOT_AUTHORIZED");
        _;
    }

    constructor () public ERC20("TitanToken Swap Token", "TitanToken") {}
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override(ERC20, IERC20) returns (bool) {
        ...
        _approve(sender, _msgSender(), allowance.sub(amount, "TitanToken: TRANSFER_AMOUNT_EXCEEDED"));
        return true;
    }
    ...
    function stake(address token, uint256 amount) external override nonReentrant returns (bool) {
        require(_unlockFactor[token] > 0, "TitanToken: FACTOR_NOT_SET");
        require(_unlockBlockGap[token] > 0, "TitanToken: BLOCK_GAP_NOT_SET");
    ...
    function unstake(address token, uint256 amount) external override nonReentrant returns (bool) {
        require(amount > 0, "TitanToken: ZERO_UNSTAKE_AMOUNT");
        LpStakeInfo storage info = _stakingRecords[_msgSender()][token];
        require(amount <= info.amountStaked, "TitanToken: UNSTAKE_AMOUNT_EXCEEDED");
        uint256 unlockedAmount = _settleUnlockAmount(_msgSender(), token, info.amountStaked, info.blockNumber);
    ...
    function mintUnlockedToken(address to, uint256 amount) onlyAuthorizedMintCaller external override {
        ...
        require(totalSupply() <= 10**26, "TitanToken: TOTAL_SUPPLY_EXCEEDED");
    }
...
// File: contract/titan/core/mine/MarketMine.sol
...
contract MarketMine is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. TITAN to distribute per block.
        uint256 lastRewardBlock;  // Last block number that TITAN distribution occurs.
        uint256 accEggPerShare;   // Accumulated TITAN per share, times 1e12. See below.
    }
    ...
    // titan tokens created per block.
    uint256 public titanPerBlock;
    ...
    // The block number when titan mining starts.
    ...
    constructor(
        TitanToken _titan,
        address _devaddr,
        uint256 _titanPerBlock,
        uint256 _startBlock
    ) public {
        titan = _titan;
        devaddr = _devaddr;
        titanPerBlock = _titanPerBlock;
    ...
    // Update the given pool's titan allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending TITAN on frontend.
    function pendingEgg(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 eggReward = multiplier.mul(titanPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 eggReward = multiplier.mul(titanPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        titan.mintUnlockedToken(devaddr, eggReward.div(10));
        titan.mintUnlockedToken(address(this), eggReward);
        pool.accEggPerShare = pool.accEggPerShare.add(eggReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MarketMine for titan allocation.
    ...
    // Withdraw LP tokens from MarketMine.
    ...
    // Safe titan transfer function, just in case if rounding error causes pool to not have enough TITAN.
    function safeEggTransfer(address _to, uint256 _amount) internal {
        uint256 eggBal = titan.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > eggBal) {
            transferSuccess = titan.transfer(_to, eggBal);
        } else {
            transferSuccess = titan.transfer(_to, _amount);
        }
    ...
    function updateEmissionRate(uint256 _titanPerBlock) public onlyOwner {
        massUpdatePools();
        titanPerBlock = _titanPerBlock;
        emit UpdateEmissionRate(msg.sender, _titanPerBlock);
    }
}