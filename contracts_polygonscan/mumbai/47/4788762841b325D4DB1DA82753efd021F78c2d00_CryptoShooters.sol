...
contract CryptoShooters is Initializable, AccessControlUpgradeable, ICharacter {
    ...
    function mintWeapon() public onlyNonContract oncePerBlock(msg.sender) {
        ...
        uint256 seed = randoms.getRandomSeed(msg.sender);
        weapons.mint(false, msg.sender, seed);
    }
    ...
    function getTargetsInternal(
        ...
        function performFight(
            ...
            if (playerRoll > monsterRoll) {
                ...
                // if (RandomUtil.combineSeeds(seed,block.timestamp) % chanceOfWinningOverflow == 0) { 
    ...
    // get max power for characters using getPowerAtLevel
    ...
    function startNextEpoch() external {
        ...
        if (excessBalance > 0) {
            ...
            if (burnPercentage > 0) {
                ...
                tagToken.safeTransfer(address(0), burnAmount);
            }
    ...
    function _calculateFightRewardBaseline(uint256 totalRewards) internal view returns (uint256) {
        ...
        uint256 maxNumOfFights = 1000 * 7 * epochDurationInDays; // TODO: adjust -- update 1000 to current elite count
...
contract Weapons is Initializable, ERC721Upgradeable, AccessControlUpgradeable {
    ...
    function mint(bool freeWeapon, address minter, uint256 seed) public restricted returns (uint256) {
        uint256 quality;
        uint256 gunType;
        uint8 element;
        uint256 selectedURI;
