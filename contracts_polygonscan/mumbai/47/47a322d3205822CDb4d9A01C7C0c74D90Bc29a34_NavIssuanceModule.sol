...
contract NavIssuanceModule is ModuleBase, ReentrancyGuard {
    ...
    mapping(IMatrixToken => NavIssuanceSetting) internal _navIssuanceSettings;
     ...
     * @param weth          Address of wrapped eth
     */
    ...
    function getIssuanceSetting(IMatrixToken matrixToken) external view returns (NavIssuanceSetting memory) {
        return _navIssuanceSettings[matrixToken];
    }
    ...
    function addReserveAsset(IMatrixToken matrixToken, address reserveAsset) external onlyManagerAndValidMatrix(matrixToken) {
        require(!_isReserveAssets[matrixToken][reserveAsset], "N0"); // "Reserve asset already exists"
    ...
    function removeReserveAsset(IMatrixToken matrixToken, address reserveAsset) external onlyManagerAndValidMatrix(matrixToken) {
        require(_isReserveAssets[matrixToken][reserveAsset], "N1"); // "Reserve asset does not exist"
    ...
    function editPremium(IMatrixToken matrixToken, uint256 premiumPercentage) external onlyManagerAndValidMatrix(matrixToken) {
        require(premiumPercentage <= _navIssuanceSettings[matrixToken].maxPremiumPercentage, "N2"); // "Premium must be less than maximum allowed"
    ...
    function editManagerFee(
        ...
        require(managerFeePercentage <= _navIssuanceSettings[matrixToken].maxManagerFee, "N3"); // "Manager fee must be less than maximum allowed"
    ...
    function editFeeRecipient(IMatrixToken matrixToken, address managerFeeRecipient) external onlyManagerAndValidMatrix(matrixToken) {
        require(managerFeeRecipient != address(0), "N4"); // "Fee recipient must not be 0 address"
    ...
    {
        require(navIssuanceSetting.reserveAssets.length > 0, "N5a"); // "Reserve assets must be greater than 0"
        require(navIssuanceSetting.maxManagerFee < PreciseUnitMath.preciseUnit(), "N5b"); // "Max manager fee must be less than 100%"
        require(navIssuanceSetting.maxPremiumPercentage < PreciseUnitMath.preciseUnit(), "N5c"); // "Max premium percentage must be less than 100%"
        require(navIssuanceSetting.managerFees[0] <= navIssuanceSetting.maxManagerFee, "N5d"); // "Manager issue fee must be less than max"
        require(navIssuanceSetting.managerFees[1] <= navIssuanceSetting.maxManagerFee, "N5e"); // "Manager redeem fee must be less than max"
        require(navIssuanceSetting.premiumPercentage <= navIssuanceSetting.maxPremiumPercentage, "N5f"); // "Premium must be less than max"
        require(navIssuanceSetting.feeRecipient != address(0), "N5g"); // "Fee Recipient must be non-zero address"

        // Initial mint cannot use NAVIssuance since minMatrixTokenSupply must be > 0
        require(navIssuanceSetting.minMatrixTokenSupply > 0, "N5h"); // "Min SetToken supply must be greater than 0"

        for (uint256 i = 0; i < navIssuanceSetting.reserveAssets.length; i++) {
            require(!_isReserveAssets[matrixToken][navIssuanceSetting.reserveAssets[i]], "N5i"); // "Reserve assets must be unique"
     ...
     * Issuance settings and reserve asset states are deleted.
     */
    ...
    function _validateIssuanceInfo(
        ...
        require(issueInfo.previousMatrixTokenSupply >= _navIssuanceSettings[matrixToken].minMatrixTokenSupply, "N7a"); // "Supply must be greater than minimum to enable issuance"
    ...
    function _validateRedemptionInfo(
        ...
        require(redeemInfo.newMatrixTokenSupply >= _navIssuanceSettings[matrixToken].minMatrixTokenSupply, "N8a"); // "Supply must be greater than minimum to enable redemption"
