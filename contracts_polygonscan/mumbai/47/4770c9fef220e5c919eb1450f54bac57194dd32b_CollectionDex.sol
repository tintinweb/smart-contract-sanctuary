 ...
 */
  function createCollection(string memory name_, string memory symbol_) public {
    ...
    if(msg.sender == blindAddress){
      collectionsWithRoyalties[col] = true;
    }
 ...
 */
 ...
 function mintWithCollection(address collection, address to, string memory tokenURI, uint256 royalty, string memory ownerId, address from ) isValid(collection) public returns(uint256) {
    ...
    if(to == nonCryptoNFTVault){
      _nonCryptoOwners[tokenId][address(collection)] = ownerId;
      emit MintWithTokenURINonCrypto(from, ownerId, tokenURI, collection);
    }
    if(collectionsWithRoyalties[collection]){
       _tokenAuthors[tokenId][address(collection)]._address = admin;
       _tokenAuthors[tokenId][address(collection)].royalty = royalty;
  ...
  function setBlindAddress(address _address) onlyOwner public {
    blindAddress = _address;
  }
