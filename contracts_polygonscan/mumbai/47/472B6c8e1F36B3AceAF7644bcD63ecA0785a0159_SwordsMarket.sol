...
contract SwordsMarket is ISwordsMarket {
     ...
     */
    modifier onlySwordsCaller() {
        require(
            swordsContract == msg.sender,
            "SwordsMarket: Only swords contract"
        );
        _;
    }
    ...
    /**
     * @notice Sets the Swords contract address. This address is the only permitted address that
     * can call the mutable functions. This method can only be called once.
     */
    function configure(address swordsContractAddress) external override {
        require(msg.sender == _owner, "SwordsMarket: Only owner");
        require(
            swordsContract == address(0),
            "SwordsMarket: Already configured"
        );
        require(
            swordsContractAddress != address(0),
            "SwordsMarket: cannot set swords contract as zero address"
        );

        swordsContract = swordsContractAddress;
    }
    ...
    function setAsk(uint256 tokenId, Ask memory ask)
        ...
        onlySwordsCaller
    {
        require(isValidBid(ask.amount), "SwordsMarket: Ask invalid");
     ...
     */
    function removeAsk(uint256 tokenId) external override onlySwordsCaller {
    ...
    function setBid(
        ...
        Bid memory bid,
        address spender
    ) public override onlySwordsCaller {
        require(
            bid.bidder != address(0),
            "SwordsMarket: bidder cannot be 0 address"
        );
        require(bid.amount != 0, "SwordsMarket: cannot bid amount of 0");
        require(
            bid.currency != address(0),
            "SwordsMarket: bid currency cannot be 0 address"
        );
        ...
        onlySwordsCaller
    {
        ...
        require(bid.amount > 0, "SwordsMarket: cannot remove bid amount of 0");
        ...
        onlySwordsCaller
    {
        ...
        require(bid.amount > 0, "SwordsMarket: cannot accept bid of 0");
        require(
            bid.amount == expectedBid.amount &&
                bid.currency == expectedBid.currency,
            "SwordsMarket: Unexpected bid found."
        );
    ...
    function _finalizeNFTTransfer(uint256 tokenId, address bidder) private {
        ...
        uint256 eventId = Swords(swordsContract).getEventIdFromTokenId(tokenId);
        IERC20 token = IERC20(bid.currency);

        // Transfer 90% to current owner of token
        token.safeTransfer(
            IERC721(swordsContract).ownerOf(tokenId),
            splitShare(Decimal.D256(uint256(90).mul(Decimal.BASE)), bid.amount)
        );

        // Transfer 3.33% share to first artist address
        token.safeTransfer(
            Swords(swordsContract).eventArtistsA(eventId),
            splitShare(
                Decimal.D256(uint256(10).div(3).mul(Decimal.BASE)),
                bid.amount
            )
        );
        // Transfer 3.33% share to second artist address
        token.safeTransfer(
            Swords(swordsContract).eventArtistsB(eventId),
            splitShare(
                Decimal.D256(uint256(10).div(3).mul(Decimal.BASE)),
                bid.amount
            )
        ...
        // Transfer 3.33% share to organizer address
        token.safeTransfer(
            Swords(swordsContract).eventOrganizers(eventId),
            splitShare(
                Decimal.D256(uint256(10).div(3).mul(Decimal.BASE)),
                bid.amount
            )
        ...
        Swords(swordsContract).exchangeTransfer(tokenId, bid.bidder);
