...
contract AnrkeyGameStaking is Ownable {
    ...
    event StakeTokens(address indexed player, uint amount, uint indexed gameTime);
    ...
    constructor(address admin, address anrx) {
    ...
    //stake anrx tokens
    function stakeTokens(uint256 amount, uint256 gameEndTime) external returns (bool) {
        IERC20 anrxtoken;
        ...
        emit StakeTokens(msg.sender, amount, gameEndTime);
        return true;
    }
    ...
    function getUserStakeInfo() public view returns (Stake memory) {
