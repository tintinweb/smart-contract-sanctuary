...
contract MockStrategy is Ownable, ReentrancyGuard {
    ...
    // Info of each pool.
    struct PoolInfo {
        uint256 accRewardTokensPerShare; // Accumulated matic/wbtc/weth per share (depends on the rewardToken), times 1e12.
    }

    struct RewardsInfo {
        uint256 pendingRewardTokens;
    }
    ...
    function updateStrategyDepositFee(uint16 _strategyDepositFeeBP) external onlyOwner {
        updatePool();
    ...
    function transferRewardTokenRewards() external onlyOwner {
        updatePool();
        uint256 rewardTokenRewards = IERC20(rewardToken).balanceOf(address(this));
    ...
    function getPendingRewards(address _user) external view returns (uint256 pendingRewardTokens) {
        ...
        uint256 totalRewardTokenRewards = IERC20(rewardToken).balanceOf(address(this));

        if (totalInputTokensStaked != 0) {
            if (rewardsTransferMode.isRewardTokenEnabled) {
                //since there is no drip of wmatic
                accRewardTokensPerShare = totalRewardTokenRewards.mul(1e12).div(
                    totalInputTokensStaked
                );
            }
        }
        pendingRewardTokens = user.amount.mul(accRewardTokensPerShare).div(1e12);
    }
    ...
    function updatePool() public {
        ...
        if (rewardsTransferMode.isRewardTokenEnabled) {
            //since there is no drip of wmatic
            pool.accRewardTokensPerShare = totalRewardTokenRewards.mul(1e12).div(
                totalInputTokensStaked
            );
        }
        ...
        returns (uint256 depositedAmount)
    {
        ...
        require(_token == address(asset), "Invalid token for deposit");
        PoolInfo storage pool = poolInfo;
        UserInfo storage user = userInfo[_user];
        updatePool();
        _transferPendingRewards(_user);
        if (strategyDepositFeeBP > 0) {
            uint256 depositFee = _amount.mul(strategyDepositFeeBP).div(10000);
            user.amount = user.amount.add(_amount.sub(depositFee));
        } else {
            user.amount = user.amount.add(_amount);
        }
        totalInputTokensStaked = totalInputTokensStaked.add(_amount);
        user.rewardTokensDebt = user.amount.mul(pool.accRewardTokensPerShare).div(1e12);
        ...
        returns (uint256 withdrawnAmount)
    {
        ...
        if (_amount > 0) {
            ...
            if (strategyWithdrawalFeeBP > 0) {
                uint256 strategyWithdrawalFee = _amount.mul(strategyWithdrawalFeeBP).div(10000);
                IERC20(_token).approve(address(msg.sender), _amount.sub(strategyWithdrawalFee));
                withdrawnAmount = _amount.sub(strategyWithdrawalFee);
            } else {
    ...
    function _transferPendingRewards(address _user) internal {
        ...
        rewardInfo.pendingRewardTokens = user
            .amount
            .mul(pool.accRewardTokensPerShare)
            .div(1e12)
            .sub(user.rewardTokensDebt);
        if (rewardsTransferMode.isRewardTokenEnabled && rewardInfo.pendingRewardTokens > 0) {
            TransferHelper.safeTransfer(
                address(rewardToken),
                _user,
                rewardInfo.pendingRewardTokens
            );
        }
    }
    ...
    function rescueFunds(address _token) external returns (uint256 rescuedAssetAmount) {
        require(_token == address(asset), "Invalid token for rescue funds");
        require(liquidityManagers[msg.sender], "Invalid Liquidity Manager");
        updatePool();
        if (totalInputTokensStaked > 0) {
