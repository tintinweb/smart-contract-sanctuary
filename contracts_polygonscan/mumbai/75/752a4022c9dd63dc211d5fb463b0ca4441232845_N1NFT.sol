...
contract N1NFT is Ownable, ERC721, ContextMixin, NativeMetaTransaction {
    
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;
    Counters.Counter private _amtId;
    
    
    event NewAMT(uint amtId, string name, uint dna);
    ...
    AMT[] public amts;
    mapping (uint => address) public amtOwner;
    mapping (address => uint) ownerAMTCount;
    ...
    function _createAMT(string memory _name, uint _dna, uint _id) private {
        amts.push(AMT(_name, _dna));
        amtOwner[_id] = msg.sender;
        ownerAMTCount[msg.sender]++;
        emit NewAMT(_id, _name, _dna);
    }

    function _genDNA(string memory _str) private view returns (uint) {
        return random(_str) % dnaModulus;
    }

    function createRandAMT(string memory _name, uint256 _id) private {
        uint256 randDna = _genDNA(_name);
        _createAMT(_name, randDna, _id);
    }
    
    function manufacture(string memory tokenURI) public payable returns (uint256) {
        ...
        createRandAMT(toString(newItemId), newItemId);
