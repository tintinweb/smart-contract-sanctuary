...
// testing functions of multi leg mints
...
contract multisaletest is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  uint256 public maxSupplyPre = 100;
  uint256 public maxMintAmount = 10;
  bool public FirstsalePause = true;
...
for(uint i = 0; i < _mintAmount; i++) {
            uint mintIndex = uint(keccak256(abi.encodePacked(block.timestamp, block.difficulty, msg.sender))) % 1000;
            mintIndex = mintIndex + 5;
  ...
  function mintPreSale(uint256 _mintAmount) public payable {
    uint256 supply = totalSupply();
    require(!PresalePause);
    require(_mintAmount > 0);
    require(_mintAmount <= maxMintAmount);
    require(supply + _mintAmount <= maxSupplyPre);
...
for(uint i = 0; i < _mintAmount; i++) {
  ...
  function setFirstsalePause(bool _state) public onlyOwner {
    FirstsalePause = _state;
  }
