...
contract QuasarIDOBondDepository is PolicyOwnable {
    ...
    event BondRedeemed( address indexed recipient, uint payout, uint stakingRewards );
    ...
    function deposit( 
        ...
        // bond updated
            ...
            // use existing vault
            address vaultAddress = bondInfo[ _depositor ].vault;
            // deposit squas to the vault
            IERC20 ( sQUAS ).safeTransfer( vaultAddress, payout );
            bondInfo[ _depositor ] = Bond({ 
    ...
    function redeem( address payable _recipient, bool _stake ) external returns ( uint ) {        
        ...
        if ( percentVested >= 10000 ) { // if fully vested
            ...
            uint stakingRewards = redeemed - info.payout;
            emit BondRedeemed( _recipient, info.payout, stakingRewards ); // emit bond data
    ...
    function pendingRewardsFor( address _depositor ) external view returns ( uint pendingRewards_ ) {
        bool redeemed = bondInfo[ _depositor ].redeemed;
        if ( !redeemed ) {
            uint payout = bondInfo[ _depositor ].payout;
            pendingRewards_ = payout.sub( IERC20( sQUAS ).balanceOf( bondInfo[ _depositor ].vault ) ); 
        } else {
            pendingRewards_ = 0;
        }
    }
