...
contract Erc721Bank is AccessControl, Pausable, IERC721Receiver {
    ...
    constructor(
        ...
        require(!tokens.hasDuplicate(), "duplicate stake token");
        ...
        )
    {
        return (
            ...
            _startBlock, // startBlock
            _rewardPerBlock / PRECISE_UNIT // rewardPerBlock
        );
    }
        ...
        )
    {
        ...
        unstakeSum = _unstakeSum;
        rewardAmount = _rewardAmount / PRECISE_UNIT;
        claimSum = _claimSum / PRECISE_UNIT;

        sharePrice = _sharePrice / PRECISE_UNIT;
        ...
        )
    {
        ...
        uint256 totalStakeAmount = _stakeAmount;
        uint256 newSharePrice = _sharePrice;
        uint256 lastUpdateBlock = _lastUpdateBlock;

        if ((currentBlock > lastUpdateBlock) && (totalStakeAmount != 0)) {
            newSharePrice += (_rewardPerBlock * (currentBlock - lastUpdateBlock)) / totalStakeAmount;
        }

        stakeAmount = userInfo.stakeAmount;
        sharePrice = newSharePrice / PRECISE_UNIT;
        rewardAmount = (userInfo.stakeAmount * (newSharePrice - userInfo.sharePrice) + userInfo.rewardAmount) / PRECISE_UNIT;
    ...
    function stakeSomeTokens(address[] calldata tokens, uint256[] calldata tokenIds) external whenNotPaused {
    ...
    function stakeOneToken(address token, uint256 tokenId) external whenNotPaused {
        _updateData(block.number);
    ...
    function unstakeSomeTokens(address[] calldata tokens, uint256[] calldata tokenIds) external whenNotPaused {
        uint256 count = tokenIds.length;
        require(count == tokenIds.length, "tokens and tokenIds mismatch");
    ...
    function unstakeOneToken(address token, uint256 tokenId) external whenNotPaused {
        _updateData(block.number);
