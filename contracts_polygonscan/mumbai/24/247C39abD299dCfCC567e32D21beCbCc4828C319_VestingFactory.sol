...
contract VestingFactory {
    ...
    event VestingLaunched(LinearVesting _vestingContract); 

    LinearVesting[] public linearVestings;
    ...
    function launchVesting(
        ...
        uint256 _exchangeIdentifier
    ) external returns(address) {
        require(address(_erc20Token) != address(0), "Incorrect token address");

        LinearVesting linearVesting = new LinearVesting(address(this), _exchangeIdentifier);
        ...
        emit VestingLaunched(linearVesting);
...
contract LinearVesting is ReentrancyGuard, Ownable {
     ...
     * @param _vestingFactory Vesting factory contract address
     */
    constructor(address _vestingFactory, uint256 _exchangeIdentifier) public {
        require(
            _vestingFactory != address(0),
            "Invalid vesting factory address"
        );

        vestingFactory = _vestingFactory;
        exchangeIdentifier = _exchangeIdentifier;
    }
    ...
    function init(
        ...
        require(msg.sender == vestingFactory, "Caller must be vesting factory");
