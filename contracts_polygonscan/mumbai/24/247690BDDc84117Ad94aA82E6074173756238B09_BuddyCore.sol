...
contract BuddyCore is IBuddyCore, ERC721Full, ERC721Holder {
    ...
    constructor(
        ...
        lastFounderClaimBlock = block.number;
    }
    ...
    function getBlackHole() external view override returns (uint256) {
        return blackHole;
    }
    ...
    function replicate(
        ...
        BuddyLib.Buddy storage base = _data[_tokenId];
        base.repCur += 1;
        uint256 dna = _buddyLogic.repBuddy(
            base,
            dominantLock,
            recessiveSwap,
        ...
        whenNotPaused
    {
        ...
        BuddyLib.Buddy storage base = _data[_tokenId];
        base.repCur += 1;
        uint256 dna = _buddyLogic.simpleRepBuddy(
            base,
            abi.encode(_data[getTokenId()])
        );
        BuddyLib.Buddy memory child = BuddyLib.generate(dna);
    ...
    // TODO: Make user select another buddy for GOO to go to
        ...
        whenNotPaused
    {
        ...
        _goo.mint(_tokenId, calcApop(_tokenId));
        _burn(_tokenId);
    }
        ...
        returns (uint256)
    {
        ...
        if (
            lastFounderClaim == 0 ||
            block.number - lastFounderClaimBlock >= 45500
        ) {
        ...
        whenNotPaused
    {
        ...
        if (bn - lastFounderClaimBlock >= 45500) {
            lastFounderClaim = 0;
        }

        base.lastRewardBlock = bn;

        if (lastFounderClaim == 0) {
            lastFounderClaim = claimAmount;
            lastFounderClaimBlock = bn;
        }
    ...
    Ownable
{
    ...
    function safeMint(address to) internal returns (uint256 tokenId) {
        ...
        _setTokenURI(
            current,
            string(
                abi.encodePacked(Strings.toString(current), ".json")
            )
        );
...
interface IBuddyCore {
    ...
    function getBlackHole() external view returns (uint256);
}
...

{
  ...
  "libraries": {
    "contracts/libraries/BuddyLib.sol": {
      "BuddyLib": "0x9833a762af0c651847b38a8b63169df83b9c076f"
    }
  }
}