...
contract PULSEROLL is VRFConsumerBase {
    ...
    mapping(bytes32 => uint)public playerMultiplier;
     ...
     modifier betIsValid(uint256 _betSize, uint256 _playerNumber,uint256 Profit) {
        require (  _playerNumber >= minNumber &&  _playerNumber <= maxNumber && Profit <= IHousePool(StakingContract).maxProfit() );
        _;
    }
    ...
    uint256 public  houseEdgeDivisor = 1000; //supports upto one decimal only
    ...
    uint FounderNum=5000;
    uint HousePoolNum=5000;
    uint CombinedDivisor=10000;
    ...
    */
    ...
    function playerRollDice(uint256 rollUnder,uint _OddEvenStatus ,uint rangeLowerLimit,uint rangeUpperLimit,uint Profit) public payable gameIsActive betIsValid(msg.value,rollUnder,Profit) CheckZero 
    returns(bytes32)
    {
        ...
        contractBalance=contractBalance+playerProfit[requestId];
    ...
    function TransferProfit(bytes32 requestId)internal{
         ...
         */
         ...
         contractBalance=contractBalance-playerTempReward[requestId];
    ...
    {
        // require(requestId.length != 0x0,"The requestId is empty");
    ...
    function DistributeFunds(address _receiver,uint _amount)internal{
       ...
       payable(StakingContract).transfer(HousePoolAmount);
       IHousePool(StakingContract).CalculateAccumulatePerShare(HousePoolAmount);
