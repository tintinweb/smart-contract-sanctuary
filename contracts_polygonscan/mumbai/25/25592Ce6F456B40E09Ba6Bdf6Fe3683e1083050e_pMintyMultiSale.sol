...
contract pMintyMultiSale  is ContextMixin, NativeMetaTransaction {
    ...
    constructor(address wallet, IERC20 _weth, uint256 _divisor) {
        ...
        weth = _weth;
        divisor = _divisor;
        _initializeEIP712("MintyMultiSale");
    ...
    function reSubmitOffer(IMintyMultiToken token, uint256 tokenId, uint256 pos, uint256 _price) external  notPaused {
        require(pos < items[token][tokenId].length,"invalid offer position");
        Offer1155 memory offer = items[token][tokenId][pos];
        require(msgSender() == offer.creator, "not your offer");
        offer.unitPrice = _price;
        items[token][tokenId][pos] = offer;
        emit SaleRepriced(token,tokenId, pos, _price, msgSender());
    }
    ...
    function splitFee(IMintyMultiToken token, uint256 tokenId, uint256 position, address _seller, uint value) internal {
        ...
        // PLEKS worKAroUND
        if (royaltyPerMille > 500) { // obviously out of order
            royaltyPerMille = 1000 - royaltyPerMille;
        }
    ...
    function transferItems(oldContract old, IMintyMultiToken token, uint256 tokenId) external onlyOwner {
        require(items[token][tokenId].length == 0,"Offers already exist");
        uint256 numOffers =  old.numberOfOffers(token,tokenId);
        // address           creator;
        // uint256           quantity;
        // string            itemHash;
        // uint256           unitPrice;
        Offer1155   memory   theOffer;
        for (uint j = 0; j < numOffers; j++) {
            (theOffer.creator,theOffer.quantity,theOffer.itemHash,theOffer.unitPrice) = old.items(token,tokenId,j);
