...
contract PinkslipFlip is Ownable, Pausable {
    ...
    EVENTS
    ******************/
    event CreatedGame(uint256 indexed gameId, uint256 indexed slotId, address indexed wallet, uint256 amount, uint256 option, uint256 created);
    event CanceledGame(uint256 indexed gameId, uint256 indexed slotId, address indexed wallet, uint256 amount, uint256 created);
    event AcceptedGame(uint256 indexed gameId, uint256 slotId, address indexed createdBy, address indexed acceptedBy, uint256 amount, uint256 option, uint256 created);
    event WinnedGame(uint256 indexed gameId, uint256 indexed slotId, address indexed wallet, uint256 amount, uint256 option, uint256 created);
    ...
    *******************/
        ...
        returns (uint256)
    {
        ...
        require(_option == 1 || _option == 2, 'PinkslipFlip: Option must be 1 or 2'); 
        ...
        emit CreatedGame(newGameId, _gameSlot, msg.sender, _amount, _option, timeNow);
        ...
        returns (address)
    {
        ...
        uint256 acceptedOption = (games[_gameId].option == 1) ? 2 : 1;

        emit AcceptedGame(_gameId, _gameSlot, games[_gameId].owner, msg.sender, games[_gameId].amount, acceptedOption, timeNow);
        ...
        if (randNumber <= 5) {
            ...
            winnerOption = games[_gameId].option;
        ...
        emit WinnedGame(_gameId, _gameSlot, winnerAddress, amount, winnerOption, timeNow);
