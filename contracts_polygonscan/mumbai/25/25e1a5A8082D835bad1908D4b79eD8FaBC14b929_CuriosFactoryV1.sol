...
contract CuriosFactoryV1 is AccessControl {

    bytes32 public constant CONTRACT_CREATOR_ROLE = keccak256("CONTRACT_CREATOR_ROLE");
    
    //Array of all ERC721 contracts
    address[] internal ERC721Contracts;
    
    //Array of all ERC1155 contracts
    address[] internal ERC1155Contracts;

    event ERC721ContractCreated(address indexed ERC721Contract);
    event ERC1155ContractCreated(address indexed ERC1155Contract);

    constructor(address _admin_contract_creator) {
        _setupRole(DEFAULT_ADMIN_ROLE, _admin_contract_creator);
        _setupRole(CONTRACT_CREATOR_ROLE, _admin_contract_creator);
    }

    /// @notice Deploys an ERC721 Proxy
    /// @dev Deploys an ERC1967 proxy, connects it to a deployed ERC721 implementation contract, initializes the proxy based on that implementation, and pushes the deployed proxy address to ERC721Contracts array
    /// @param implementation The implementation contract address
    /// @param name The ERC721 name variable
    /// @param symbol The ERC721 symbol variable
    /// @param commissionAccount The main account address where commissions should be sent
    /// @param _defaultAdmin The account address which should hold the DEFAULT_ADMIN role for the contract
    /// @param _minterPauserAddresses An array of account addresses which should hold the MINTER and PAUSER roles for the contract
    /// @param _adminAddresses An array of account addresses which should hold the BURN and UPGRADER roles for the contract
    function createERC721ContractInstance (
        address implementation, 
        string memory name, 
        string memory symbol,
        address commissionAccount,
        address _defaultAdmin,
        address[] memory _minterPauserAddresses,
        address[] memory _adminAddresses) external onlyRole(CONTRACT_CREATOR_ROLE){
        ERC1967Proxy newProxy = new ERC1967Proxy(implementation, abi.encodeWithSignature("initialize(string,string,address,address,address[],address[])", name, symbol, commissionAccount, _defaultAdmin, _minterPauserAddresses, _adminAddresses));
        ERC721Contracts.push(address(newProxy));
        emit ERC721ContractCreated(address(newProxy));
    }

    /// @notice Deploys an ERC1155 Proxy
    /// @dev Deploys an ERC1967 proxy, connects it to a deployed ERC1155 implementation contract, initializes the proxy based on that implementation, and pushes the deployed proxy address to ERC1155Contracts array
    /// @param implementation The implementation contract address
    /// @param _defaultAdmin The account address which should hold the DEFAULT_ADMIN role for the contract
    /// @param _minterPauserAddresses An array of account addresses which should hold the MINTER and PAUSER roles for the contract
    /// @param _adminAddresses An array of account addresses which should hold the BURN and UPGRADER roles for the contract
    function createERC1155ContractInstance (
        address implementation,
        address _defaultAdmin,
        address[] memory _minterPauserAddresses,
        address[] memory _adminAddresses) external onlyRole(CONTRACT_CREATOR_ROLE) {
        ERC1967Proxy newProxy = new ERC1967Proxy(implementation, abi.encodeWithSignature("initialize(address,address[],address[])", _defaultAdmin, _minterPauserAddresses, _adminAddresses));
        ERC1155Contracts.push(address(newProxy));
        emit ERC1155ContractCreated(address(newProxy));
    }
    
    /// @notice Retrieves all deployed ERC721 proxy addresses
    /// @dev Returns the ERC721Contracts address array
    /// @return Returns all addresses contained in the address array
    function getERC721ContractInstances() external view returns(address[] memory) {
        return ERC721Contracts;
    }

    /// @notice Retrieves all deployed ERC1155 proxy addresses
    /// @dev Returns the ERC1155Contracts address array
    /// @return Returns all addresses contained in the address array
    function getERC1155ContractInstances() external view returns(address[] memory) {
        return ERC1155Contracts;
    }
}
...
//Inspired by Known Origin's ERC721 implementation, which can be found at https://github.com/knownorigin/known-origin-contracts/blob/master/contracts/v2/KnownOriginDigitalAssetV2.sol
   ...
   */
    function burn(uint256 _tokenId) public onlyRole(BURN_ROLE) whenPaused{
    ...
    */
    ...
    /**
    * @dev Required override of _authorizeUpgrade from UUPSUpgradeable.sol
     **/ 
    function _authorizeUpgrade(address newImplementation) internal virtual override onlyRole(UPGRADER_ROLE) {}

    /**
    * @dev Utility function for concatination of strings
     **/ 
...
contract ERC1155ImplementationV1 is Initializable, AccessControlUpgradeable, ERC1155PausableUpgradeable, UUPSUpgradeable {

//Inspired by OpenSea's ERC1155 implementation which can be found at https://github.com/ProjectOpenSea/opensea-creatures/blob/master/contracts/ERC1155Tradable.sol
    ...
    */
    modifier creatorOnly(uint256 _id) {
        require(creators[_id] == msg.sender, "ERC1155#creatorOnly: ONLY_CREATOR_ALLOWED");
        _;
    }
    ...
    */
    modifier ownersOnly(uint256 _id) {
        require(balanceOf(msg.sender, _id) > 0, "ERC1155#ownersOnly: ONLY_OWNERS_ALLOWED");
        _;
    }

    function initialize (address _defaultAdmin, address[] memory _minterPauserAddresses, address[] memory _adminAddresses) public initializer {
        _setupRole(DEFAULT_ADMIN_ROLE, _defaultAdmin);
        for (uint256 i = 0; i < _adminAddresses.length; i++) {
            _setupRole(UPGRADER_ROLE, _adminAddresses[i]);
            _setupRole(BURN_ROLE, _adminAddresses[i]);
        ...
        __AccessControl_init_unchained();
        __Pausable_init_unchained();
        __ERC1155_init_unchained("https://ipfs.infura.io/ipfs/");
    }
    ...
    * NOTE: The uri function of ERC1155Upgradeable requires the token id as a parameter for client-side id substitution. This is not used in this implementation but the id is still required argument for the function.
    ...
    **/
    function mint (address to, uint256 tokenId, uint256 amount, bytes memory data) public onlyRole(MINTER_ROLE) {
    ...
    * @param data        Data to pass if receiver is contract
    */
    function mintBatch (address to, uint256[] memory tokenIds, uint256[] memory amounts, bytes memory data) public onlyRole(MINTER_ROLE){
        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 id = tokenIds[i];
            require(creators[id] == msg.sender, "ERC1155#batchMint: ONLY_CREATOR_ALLOWED");
     ...
     */
    function burn(address account, uint256 tokenId, uint256 amount) public whenPaused onlyRole(BURN_ROLE){
     ...
     */
    function burnBatch(address account, uint256[] memory tokenIds, uint256[] memory amounts) public whenPaused onlyRole(BURN_ROLE){
        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 id = tokenIds[i];
            uint256 amount = amounts[i];
            tokenSupply[id] -= amount;
