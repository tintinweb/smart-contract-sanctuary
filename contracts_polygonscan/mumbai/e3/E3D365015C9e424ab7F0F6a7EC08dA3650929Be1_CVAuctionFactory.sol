...
/** 
 *  SourceUnit: /Users/iggyi/Git/culture-vault/cv-contract-hardhat/contracts/auction-v6-factory.sol
*/
...
contract StandardAuctionV6 is Pausable, ReentrancyGuard {
    using SafeMath for uint256;
    event Start();
    ...
    ERC1155Tradable public nft1155;
    ERC721Tradable public nft721; 
    ...
    modifier onlySeller(address nft_address) {
        if (ERC165Checker.supportsInterface(nft_address, 0x80ac58cd)) {
            nft721 = ERC721Tradable(nft_address);
            require(
                nft721.ownerOf(nftId) == msg.sender,
                "Ownable: caller is not the owner"
            );
        } else {
            nft1155 = ERC1155Tradable(nft_address);
            require(
                nft1155.balanceOf(msg.sender, nftId) > 0,
                "Ownable: caller is not the owner"
            );
        }
        _;
    ...
    function start() external onlySeller(nft) whenNotPaused {
    ...
    function withdraw(address bidderAddress) external whenPaused nonReentrant onlySeller(nft){
    ...
    function forceAcceptEnd() external whenNotPaused onlySeller(nft) nonReentrant {
...
////import "./auction-v6.sol";

contract CVAuctionFactory {
    StandardAuctionV6[] public auctions;
    ...
    function create(address _nft,
        ...
        StandardAuctionV6 auction = new StandardAuctionV6(_nft, _nftId, _nftAmount, _startingBid, _endAt, _startAt, _seller);
    ...
    {
        StandardAuctionV6 auction = auctions[_index];
