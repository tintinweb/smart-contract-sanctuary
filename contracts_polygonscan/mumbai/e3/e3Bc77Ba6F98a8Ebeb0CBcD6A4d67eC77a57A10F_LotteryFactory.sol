...
contract LotteryFactory is Ownable {
  ...
  function createLottery(string memory lotteryName, uint startTime, uint entryFee, uint maxEntries, uint maxEntriesPerPlayer) external onlyOwner {
    ...
    LotteryContract newLottery = new LotteryContract(lotteryName, startTime, entryFee, maxEntries, maxEntriesPerPlayer, msg.sender);
...
contract LotteryContract {
  ...
  constructor(string memory _lotteryName, uint _startTime, uint _entryFee, uint _maxEntries, uint _maxEntriesPerPlayer, address _owner) {
  ...
  function setLotteryStartTime(uint _lotteryStartTime) external restricted {
  ...
  function pauseLottery() external restricted {
    isLotteryLive = false;
  }
  ...
  // need to change to Chainlink VRF to go live
  function drawIndex() internal view returns (uint256 index) {
    uint256 i = uint(blockhash(block.number - 1)) % players.length;
    index = i;
  }

  function checkEntry(address addr) public view returns(bool) {
    return isNewPlayer(addr) ? false : true;
  }
