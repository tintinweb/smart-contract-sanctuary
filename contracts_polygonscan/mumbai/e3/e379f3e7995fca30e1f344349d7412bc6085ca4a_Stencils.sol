...
contract Stencils is 
    ...
    mapping (bytes32 => uint) public tokenRandomness;
    mapping (uint => Traits) public tokenTraits;
    ...
    function initialize(
        ...
        __Ownable_init();
        __VRFConsumerBase__init(vrfCoordinator, linkToken);

        keyhash = keyHash;
        fee = 100000000000000; // 0.0001 LINK
        colors = [0, 1, 1, 1, 2, 2, 3, 3, 4, 4];
    }

    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        uint tokenId = tokenRandomness[requestId];

        uint colorIdx = randomness % colors.length;
        uint whateverIdx = (randomness + block.timestamp) % colors.length;
    
        tokenTraits[tokenId] = Traits({backgroundColor: colors[colorIdx], whatever: colors[whateverIdx]});
    }
...
abstract contract VRFConsumerBase is Initializable, VRFRequestIDBase {
  ...
  //   address _link
  // ) {
  function __VRFConsumerBase__init(
    address _vrfCoordinator,
    address _link
