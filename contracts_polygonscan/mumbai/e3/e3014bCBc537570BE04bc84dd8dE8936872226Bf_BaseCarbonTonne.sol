    ...
    BaseCarbonTonneStorage
{
    ...
    function initialize() public initializer {
        ...
        __Ownable_init_unchained();
        __Pausable_init_unchained();
        __ERC20_init_unchained('BaseCarbonTonne Pool', 'BCT');
    }
    ...
    function setContractRegistry(address _address) public virtual onlyOwner {
        contractRegistry = _address;
    }
    ...
    function switchMapping(string memory _mappingName, bool accepted)
        ...
        onlyOwner
    {
        if (strcmp(_mappingName, 'vintages')) {
            accepted
                ? vintageIsAcceptedMapping = true
                : vintageIsAcceptedMapping = false;
        } else if (strcmp(_mappingName, 'regions')) {
            accepted
    ...
    function addAttributes(
        ...
        uint256 vintagesLen = _vintages.length;
        if (vintagesLen > 0) {
            for (uint256 i = 0; i < vintagesLen; i++) {
                if (addToList == true) {
                    vintages[_vintages[i]] = true;
                } else {
                    vintages[_vintages[i]] = false;
                }
            }
        }
    ...
    function checkEligible(address erc20Addr)
        ...
        returns (bool)
    {
        bool isCO2kenContract = IContractRegistry(contractRegistry).checkERC20(
            erc20Addr
        );
        ...
        // If not CO2KEN native contract, check if address is whitelisted
        else {
            require(
        ...
        returns (bool)
    {
        ProjectData memory projectData;
        VintageData memory vintageData;
        (projectData, vintageData) = ProjectERC20(erc20Addr).getAttributes();

        /// @dev checks if any one of the attributes are blacklisted.
        // If mappings are set to "whitelist"-mode, require the opposite
        // VINTAGE
        if (vintageIsAcceptedMapping == false) {
            require(vintages[vintageData.vintage] == false);
        } else {
            require(vintages[vintageData.vintage] == true);
        }

        // REGION
        if (regionsIsAcceptedMapping == false) {
            require(regions[projectData.region] == false);
        } else {
            require(regions[projectData.region] == true);
        }

        // STANDARD
        if (standardsIsAcceptedMapping == false) {
            require(standards[projectData.standard] == false);
        } else {
            require(standards[projectData.standard] == true);
        }

        // METHODOLOGY
        if (methodologiesIsAcceptedMapping == false) {
            require(methodologies[projectData.methodology] == false);
        } else {
            require(methodologies[projectData.methodology] == true);
        }
...
contract BaseCarbonTonneStorage {
    ...
    bool public vintageIsAcceptedMapping;
    mapping(uint16 => bool) public vintages;
