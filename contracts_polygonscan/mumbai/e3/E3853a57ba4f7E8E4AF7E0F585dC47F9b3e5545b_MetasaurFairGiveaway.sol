...
contract MetasaurFairGiveaway is VRFConsumerBase, Ownable {
	...
	event IPFSProofAdded(uint256 currentDrawID, string proof);
	...
	function addContestData(string memory ipfsHash) external onlyOwner {
		...
		emit IPFSProofAdded(totalDraws, ipfsHash);
	}
	...
	//second, you may call this if many winners are required
	function pickManyWinnersOffset(
		uint256 offset,
		uint256 numWinners,
		uint256 drawId,
		uint256 totalEntries
		...
		uint256[] memory expandedValues = new uint256[](numWinners);
		for (uint256 i = offset; i < numWinners; i++) {
			expandedValues[i] =
				(uint256(keccak256(abi.encode(randomResults[drawId], i))) % totalEntries) +
				1;
		}
