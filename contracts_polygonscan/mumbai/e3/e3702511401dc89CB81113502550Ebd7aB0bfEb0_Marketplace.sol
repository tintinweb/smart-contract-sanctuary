...
/** 
 *  SourceUnit: /home/andrei/upwork/pictos_contracts/contracts/Marketplace.sol
*/
...
interface IMarketplace {
    ...
    event Sold(uint256 indexed tokenId, address indexed seller, address indexed buyer, uint256 sellerId, uint256 buyerId, uint256 price, address currency, uint256 timestamp);
...
contract Marketplace is Ownable, IMarketplace {
    ...
    function sellForOffer(uint256 listingId, uint256 offerId) external override{
        ...
        emit Sold(tokenId, msg.sender, offer.offerend, 0,0, offer.amount, offer.currency, block.timestamp);
    ...
    function buyExactMatchNative(uint256 userId,uint256 tokenId) external payable override{
        ...
        emit Sold(tokenId, _list.seller, msg.sender, _list.sellerId, userId, msg.value, address(0), block.timestamp);
        clearStorage(tokenId);
    }
    ...
    function buyExactMatchTokens(uint256 userId, uint256 tokenId, uint256 amount, address currency) external override{
        ...
        emit Sold(tokenId, _list.seller, msg.sender,_list.sellerId, userId, amount, currency, block.timestamp);
        clearStorage(tokenId);
    }
