...
contract AutumnLeaf is ERC721Enumerable, Ownable, ReentrancyGuard{
    ...
    uint256 public price = 0.02021 ether;
    ...
    function getBunches(uint256 tokenId) internal view returns(uint256){
        ...
        if(rand >= 190 && rand < 195){return ps=6;}
        if(rand >= 195 && rand < 198){return ps=7;}
        if(rand >= 198 && rand < 201){return ps=8;}
        return ps;
    }
    ...
    function getSelectedleaves(Leaf memory leaf,uint256 tokenId) internal view returns(string memory){
        ...
        if(leaf.bunches==4){
            selectedLeaf = string(abi.encodePacked(pathstart[1],' stroke="',stemcolors[colorindex],'" ','fill="',colors[colorindex],'" ','/>',pathstemone[1],' stroke ="',
            stemcolors[colorindex],'"','/>','</g>'));
        }
        if(leaf.bunches==5){
    ...
    function AnimationTop(Leaf memory leaf) internal view returns(string memory){
    ...
    function AnimationBottom(Leaf memory leaf) internal view returns(string memory){
        string memory animation = '';
        if(leaf.animation)
            animation = string(abi.encodePacked(animationBegin,'-360 ',animationEnd));
        return animation;
    ...
    function getSVGImage(Leaf memory leaf,uint256 tokenId) internal view returns(string memory){
        ...
        (leaf.bunches > 0 && leaf.bunches < 3)? parts[4] = AnimationBottom(leaf): parts[4] = '';
