...
contract Tx8Swap is ReentrancyGuard, Ownable {
    ...
    nonReentrant()
  {
    uint256 amount2;  
    if (usdt2tx8) {
      amount2 = amount.mul(priceUSDTX8);
      _addLiquidity(amount, address(tx8Token), address(usdtToken));
      _removeLiquidity(amount2, address(usdtToken), address(tx8Token));

      emit TokensSwapped(amount, address(usdtToken), address(tx8Token), msg.sender);

      usdtToken.safeTransferFrom(msg.sender, address(this), amount);
      tx8Token.safeTransfer(msg.sender, amount2);
    } else {
      amount2 = amount.div(priceUSDTX8);
      _addLiquidity(amount2, address(usdtToken), address(tx8Token));
      _removeLiquidity(amount, address(tx8Token), address(usdtToken));
      ...
      usdtToken.safeTransfer(msg.sender, amount2);
    }
