...
/**
 * @title IBZNFT
 * IBZNFT - Smart Contract for IbizaToken NFT
 */
contract IBZNFT is ERC721, ERC721URIStorage, ERC721Burnable, Ownable {
    ...
    mapping(uint256 => uint256) private _nftPricesMapping;
    mapping(address => uint256) private _vault;
    mapping(address => bool) private _minters;
    uint256 percentage = 5; // Which is 100 / 20
    address openseaProxyAddress;
    address umiProxyAddress;
    address umiGateAddress;
    ...
    constructor(
        ...
        umiGateAddress = _umiGateAddress;
    ...
    function _burn(uint256 _tokenId) internal override(ERC721, ERC721URIStorage) {
        require(burningEnabled, "IBZNFT: Burning is disabled");
    ...
    function fixUmiProxyAddress(address newAddress) public onlyOwner {
        umiProxyAddress = newAddress;
    }

    function fixUmiGateAddress(address newAddress) public onlyOwner {
        umiGateAddress = newAddress;
    }

    function fixContractURI(string memory newURI) public onlyOwner {
        contract_ipfs_json = newURI;
    }

    function fixPercentage(uint256 newPercentage) public onlyOwner {
        percentage = newPercentage;
    }

    function fixIBZContract(address ibzAddress) public onlyOwner {
        IBZ = IERC20(ibzAddress);
    }

    function fixPrice(uint256 tokenId, uint256 price) public {
        address owner = ownerOf(tokenId);
        require(msg.sender == owner, "IBZNFT: Only owner can fix price");
        _nftPricesMapping[tokenId] = price;
    }
    ...
    */
    ...
    /*
        This method will get ERC-20 balance
    */

    function ibzBalance(address _address) public view returns (uint256) {
        uint256 balance = IBZ.balanceOf(_address);
        return balance;
    }

    /*
        This method will return IBZ allowance
     */

    function ibzAllowance(address _address) public view returns (uint256) {
        uint256 allowance = IBZ.allowance(_address, address(this));
        return allowance;
    }

    /*
        This method will allow parties to withdraw all ethers
     */

    function ibzWithdraw() public {
        uint256 balance = _vault[msg.sender];
        require(balance > 0, 'IBZNFT: Nothing to withdraw!');
        IBZ.transfer(msg.sender, balance);
        _vault[msg.sender] = 0;
    }

    function ibzVault(address _address) public view returns (uint256){
        return _vault[_address];
    }
    
    /*
        This method will allow owner to withdraw all IBZ
     */

    function ownerWithdraw() public onlyOwner {
        uint256 balance = _vault[address(this)];
        require(balance > 0, 'IBZNFT: Nothing to withdraw!');
        IBZ.transfer(msg.sender, balance);
        _vault[address(this)] = 0;
    }

    /*
        This method will mint the token to provided minter
    */
    function mintNFT(string memory _tokenURI)
        public
        returns (uint256)
    {
        require(!nftExists(_tokenURI), "IBZNFT: Trying to mint existent nft");
        require(isMinter(msg.sender), "IBZNFT: Only minters can mint");
    ...
    */
    function buyNFT(uint256 tokenId)
        public
    {
        uint256 price = _nftPricesMapping[tokenId];
        require(price > 0, "IBZNFT: This NFT is not in selling");
        uint256 allowance = ibzAllowance(msg.sender);
        require(allowance >= price, "IBZNFT: Contract can't move IBZ tokens");
        address owner = ownerOf(tokenId);
        require(owner != msg.sender, "IBZNFT: Can't buy from yourself");
        uint256 fee = price.div(percentage);
        _vault[address(this)] = _vault[address(this)] + fee;
        uint256 earn = price.sub(fee);
        _vault[owner] = _vault[owner] + earn;
        _nftPricesMapping[tokenId] = 0;
        IBZ.transferFrom(msg.sender, address(this), price);
    ...
    */
    function proxyMintNFT(address to, string memory _tokenURI)
        ...
        returns (uint256)
    {
        require(proxyMintingEnabled, "IBZNFT: Proxy minting is disabled");
        require(!nftExists(_tokenURI), "IBZNFT: Trying to mint existent nft");
        require(
            msg.sender == umiProxyAddress,
            "IBZNFT: Only Proxy Address can Proxy Mint"
        );
    ...
    */
        ...
        returns (bool isOperator)
    {
        ...
        if (
            ...
            _operator == address(umiGateAddress) ||
