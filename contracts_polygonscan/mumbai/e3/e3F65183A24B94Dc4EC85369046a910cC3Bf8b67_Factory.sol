...
contract Factory {
    ...
    function addAsset(address asset) public {
        require(!assets[asset], "Asset already added");
        assets[asset] = true;
    }

    function removeAsset(address asset) public {
        require(assets[asset], "Asset not exists");
        assets[asset] = false;
    }
    ...
    function createPool(
        ...
        Pool newPool = new Pool(name, structure, asset, minDeposit);
...
interface PoolInterface {
    ...
    event ProcessFailed(string operation, string error);
...
contract Pool is PoolStorage {
    ...
    function deposit() external payable override returns (bool success) {
        if (minDeposit > msg.value) {
            emit ProcessFailed(
                "deposit", 
                "Deposited amount is lower than minDeposit"
            );
            return false;
        }
        ...
        lockedTokens[_msgSender()] = lockedTokens[_msgSender()].add(lpTokens);

        lpToken.mint(_msgSender(), lpTokens);
        return true;
    }

    function withdraw(uint256 _tokenAmount) external override returns (bool success) {
        uint256 lpTokens = _tokenAmount;

        if (lpTokens <= 0) {
            emit ProcessFailed("withdraw", "Invalid token amount");
            return false;
        }
        if (lpTokens > lockedTokens[_msgSender()]) {
            emit ProcessFailed("withdraw", "Insufficient tokens");
            return false;
        }

        lockedTokens[_msgSender()] = lockedTokens[_msgSender()].sub(lpTokens);
        balances[_msgSender()] = balances[_msgSender()].sub(lpTokens);
        _totalDeposited = _totalDeposited.sub(lpTokens);
        lpToken.burn(lpTokens);

        payable(_msgSender()).transfer(lpTokens);

        emit Withdrawn(_msgSender(), lpTokens);
        return true;
    }
