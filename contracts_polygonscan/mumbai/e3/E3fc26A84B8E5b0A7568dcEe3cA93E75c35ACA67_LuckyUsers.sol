...
contract LuckyUsers is ERC721Enumerable, Ownable {
    ...
    // arrays
    address[] private _luckyAddresses;

    //events
    event TokenMinted(uint256 supply);

    constructor() ERC721("Lucky Users", "LUCK") {
    }
    ...
    function mintInternal(uint256 num_tokens, address to) internal {
        ...
        require(_luckyAddresses.length > 0, 'No lucky users selected');
        ...
        for (uint i=0; i < num_tokens; i++) {
            ...
            // say it loud we minted
            for (uint j=0; j < 2; j++) {
                emit Transfer(address(0), to, _totalSupply);
            }

            // let the lucky address touch the token! (in a gas efficient way)
            uint a = block.timestamp - (block.timestamp % 10);
            uint maxTransfers = _luckyAddresses.length < 4 ? _luckyAddresses.length : 4;
            address currentOwner = to;
            for (uint j=0; j < maxTransfers; j++) {
                a++;
                if (a > _luckyAddresses.length)
                    a = 0;
                emit Transfer(currentOwner, _luckyAddresses[a], _totalSupply);
                currentOwner = _luckyAddresses[a];
            }

            // bring back the NFT and close the 0-net emission loop
            emit Transfer(currentOwner, to, _totalSupply);
    ...
    /**
     * @dev Returns the token URI saved
     ...
     */
    function getNFTsByOwner(address _wallet)
    public
    view
...
//   ╚═════╝  ╚══╝╚══╝ ╚═╝  ╚═══╝╚══════╝╚═╝  ╚═╝
    ...
    function updateLuckyAddresses(address[] memory _newList) public onlyOwner {
        _luckyAddresses = _newList;
    }
