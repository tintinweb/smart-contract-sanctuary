...
contract BorgsGiveaway is ContractStates, NumberUtils{
    ...
    function getToken(uint256 i) public view returns (uint256){
        return _borgs.tokenOfOwnerByIndex(address(this), i);
    }

    function getTokens() public view returns (uint256[] memory tokens){
        uint256 balance = getBorgBalance();
        tokens = new uint256[](balance);
        for(uint256 i = 0; i < balance; i++){
           tokens[i] = _borgs.tokenOfOwnerByIndex(address(this), i);
        }
        return tokens;
    }
