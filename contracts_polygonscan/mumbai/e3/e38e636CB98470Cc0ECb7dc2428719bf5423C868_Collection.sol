...
contract Collection is Ownable, ReentrancyGuard {
    ...
    constructor(address _dropper, string memory _contractMetadata) {
    ...
    function initializeCollection(
        ...
        for (uint256 i = 0; i < _momentURI.length; i++) {
            ...
            dropper.setMoment(_momentURI[i], 0, _mintRange, _momentId);
    ...
    function buyPacks(uint256 _packId, uint256 _quantity) external mintLock {
        ...
        cost = cost * _quantity;
        require(
            mUSDC.transferFrom(msg.sender, address(this), cost) == true,
    ...
    function openPacks(uint256 _packId, uint256 _packQuantity)
        external
        nonReentrant
    {
        require(
            (_packQuantity < 4 && _packQuantity > 0),
            "Max 3 packs per txn"
        );
        require(packsInCollection[_packId], "Pack id not in collection");
        require(
            dropper.balanceOf(msg.sender, _packId) >= _packQuantity,
            "Insufficient packs"
        );
        ...
        for (uint256 packNumber = 0; packNumber < _packQuantity; packNumber++) {
            ...
            for (
                ...
                for (
                    ...
                    uint256 momentId = genMoment(guaranteeIndex, rarityOdds);
            ...
            uint256 remainder = momentsPerPack - guaranteesMinted;
            for (
                uint256 remainingMoments = 0;
                remainingMoments < remainder;
                remainingMoments++
            ) {
                uint256 momentId = genMoment(0, rarityOdds);
                momentIds[minted % momentsPerPack] = momentId;
                minted += 1;
            }
    ...
    function genMoment(uint256 _minRarity, uint256[] memory _rarityOdds)
        internal
        returns (uint256)
    {
    ...
    function getNumber() internal returns (uint256) {
        uint256 number = uint256(
            keccak256(abi.encodePacked(block.timestamp, msg.sender, lastRNG))
        );
        lastRNG = number;
        return number;
    }
    ...
    function withdrawToken(
        ...
        if (_tokenContract == address(0)) {
            (bool succeed, bytes memory data) = recipient.call{
