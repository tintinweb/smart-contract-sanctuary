...
contract MetaRich is Ownable, ERC721, RandomlyAssigned {
    ...
    uint256 constant PRICE_PHASE2 = 1 ether;
    uint256 constant PRICE_PHASE3 = 2 ether;
    ...
    function mintBatch(uint256 _count) public payable {
        if(whitelistSale){
            require(msg.value == PRICE_PHASE2 * _count, "Value isn't correct");
            ...
            require(msg.value == PRICE_PHASE3 * _count, "Value isn't correct");
            _count = (totalSupply_ + wlCount) <= MAX_NFT ? _count : MAX_NFT - totalSupply_;
            
        }
        else{
            revert("Mint not available now");
        }
