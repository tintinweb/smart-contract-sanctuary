...
contract KommunitasStaking is Ownable {
    ...
    uint256 public penaltyFeesPercentage = 2;   //2% inital penalty fee
    ...
    function _calculateReward(uint256 _amount, uint256 _duration) 
    internal 
    view returns (uint256 _lockReward) {   
        ...
        uint256 durationSeconds = _duration.mul(1 days);
        uint256 effectiveAPY = multiplier.mul(apy).mul(durationSeconds).mul(1e10).div(yearDuration).div(10); 
        _lockReward = effectiveAPY.mul(_amount).mul(durationSeconds).div(yearDuration).div(1e12);
    }
    ...
    function setDurationMultiplier(uint256 _multiplier, uint256 _duration) external onlyOwner {
        require(_duration <= maxDuration && _duration >= minDuration , "Duration should be within max-min range");
        durationMultiplier[_duration] = _multiplier;
    }
