...
interface IERC20 {
   ...
   function placeBid(uint256 _tokenId, uint256 _amount, bool awardType, address from, string calldata ownerId)  external;
...
//   event SetValue(address indexed from, uint256 main, uint256 _author, uint256 _platform);
  ...
  event Claim(address indexed bidder, address nftContract, uint256 tokenId, uint256 amount, bool awardType,string ownerId);
  ...
  function claimAuction(uint256 _tokenId, bool awardType, string memory ownerId, address from) public {
    ...
    emit Claim(temp.bidder, temp.nftContract, _tokenId, temp.bidAmount, awardType, ownerId);
