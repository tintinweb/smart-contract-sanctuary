...
contract StakingRewards is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    function getBalance(address _account) public view returns (uint) {
        ...
        uint diffYears = BokkyPooBahsDateTimeLibrary.diffYears(stakeTime, current);
        if (diffYears > 0) {
            balance = getBalancePerYears(balance, diffYears, rewardPerYear);
            current = BokkyPooBahsDateTimeLibrary.subYears(current, diffYears);
        }

        uint diffMonths = BokkyPooBahsDateTimeLibrary.diffMonths(stakeTime, current);
        if (diffMonths > 0) {
            balance = getBalancePerMonths(balance, diffMonths, rewardPerMonth);
            current = BokkyPooBahsDateTimeLibrary.subMonths(current, diffMonths);
        }

        uint diffDays = BokkyPooBahsDateTimeLibrary.diffDays(stakeTime, current);
        if (diffDays > 0) {
            balance = getBalancePerDays(balance, diffDays, rewardPerDay);
            current = BokkyPooBahsDateTimeLibrary.subDays(current, diffDays);
        }

        uint diffHours = BokkyPooBahsDateTimeLibrary.diffHours(stakeTime, current);
        if (diffHours > 0) {
            balance = getBalancePerHours(balance, diffHours, rewardPerHour);
        }
