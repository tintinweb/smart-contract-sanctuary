...
contract Project0Treasury is Ownable {
    ...
    constructor(
       ...
       // isLiquidityToken[_OHMTEST] = true;
     //   liquidityTokens.push(_OHMTEST);
    ...
    function deposit( uint256 _amount, address _token, uint256 _profit) external returns (uint256 send_) {
        ...
        IOxygen(Oxygen).mint(msg.sender, send_);
    ...
    function withdraw(uint256 _amount, address _token) external {
        ...
        uint256 value = valueOf(_token, _amount);
        IOxygen(Oxygen).burnFrom(msg.sender, value);
    ...
    function incurDebt(uint256 _amount, address _token) external {
        ...
        uint256 maximumDebt = IERC20(Water).balanceOf(msg.sender); // Can only borrow against sOHM held
    ...
    function repayDebtWithOHM( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );
        IOxygen(Oxygen).burnFrom( msg.sender, _amount );
        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );
        emit RepayDebt( msg.sender, Oxygen, _amount, _amount );
    }
    ...
    function mintRewards(address _recipient, uint256 _amount) external {
        ...
        IOxygen(Oxygen).mint(_recipient, _amount);
    ...
    function excessReserves() public view returns (uint256) {
        return totalReserves.sub(IERC20(Oxygen).totalSupply().sub(totalDebt));
    }
    ...
    function valueOf(address _token, uint256 _amount) public view returns (uint256 value_) {
        if (isReserveToken[_token]) {
            ...
            value_ = _amount.mul(10**IOxygen(Oxygen).decimals()).div(10**IOxygen(_token).decimals());
