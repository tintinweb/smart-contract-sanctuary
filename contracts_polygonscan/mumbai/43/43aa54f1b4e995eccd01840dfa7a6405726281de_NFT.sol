...
contract NFT is Initializable, OwnableUpgradeable, ERC721Upgradeable, Storage {
    ...
    initializer
  {
    ...
    preMintSupply = projectConfig._preMintSupply;
    reservedTokenSupply = projectConfig._reservedTokenSupply;
    maxPreMintTx = projectConfig._maxPreMintTx;
  ...
  function preMint(uint8 _amount) external payable {
    ...
    require(pre_saleIsActive, 'Sale must be active');
    require(
      _amount > 0 && _amount <= maxPreMintTx,
      'Exceeds Maximum Mints Per Transaction'
    );
    require(
      _tokenIds.current() + _amount <= preMintSupply,
  ...
  // Mint Handler
  function handleMint(address _to, uint8 _amount) internal {
  ...
  function mintFromReserve(address _to, uint256 _amount) external onlyOwner {
    require(
      _amount <= reservedTokenSupply,
      'Amount will exceed reserve amount!'
    );
    require(
...
contract Storage {
  ...
  uint16 public preMintSupply; // pre-minted supply
  uint16 public reservedTokenSupply; // reserved token supply
  bool public pre_saleIsActive; // is this a pre-release?
