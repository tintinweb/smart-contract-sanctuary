...
contract TimeLockedWalletFactory {
    ...
    function newTimeLockedWallet(address _owner, uint256 _unlockDate) payable public returns (address payable wallet) {
        require(_unlockDate > block.timestamp);
        wallet = payable(new TimeLockedWallet(_owner, _unlockDate));
...
contract TimeLockedWallet is Ownable {
    ...
    function info() public view returns (address, uint256, uint256) {
        return (owner(), _unlockDate, address(this).balance);
    }
