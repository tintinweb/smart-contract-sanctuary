...
contract BinamarsSpawner is Ownable {
    ...
    function setNFT(address _binaNft) public onlyOwner {
        binaNft = Binamars(_binaNft);
    }

    function setERC20(address _binaToken) public onlyOwner {
        binaToken = BinaToken(_binaToken);
    }
    ...
    function buy(uint256 amount) public {
        ...
        binaToken.transferFrom(
            _msgSender(),
            router.feeAddress(),
            router.priceEgg().mul(amount)
        );
        if (amount == 1) binaNft.spawn(_msgSender());
        else binaNft.multiSpawn(_msgSender(), amount);
    }

    function evolve(uint256 _tokenId) public {
        uint256 rnd = _tokenId.random(3);
        Binamars.Level level = Binamars.Level.COMMON;
        if (rnd < 20) {
            level = Binamars.Level.LEGENDARY;
        } else if (rnd < 220) {
            level = Binamars.Level.NORMAL_RARE;
        } else if (rnd < 450) {
            level = Binamars.Level.RARE;
        } else if (rnd < 710) {
            level = Binamars.Level.EPIC;
        }
        binaToken.transferFrom(
            _msgSender(),
            router.feeAddress(),
        ...
        binaNft.evolve(_tokenId, _msgSender(), level);
    }

    function evolveForce(uint256 _tokenId, address _address) public onlyOwner {
        Binamars.Level level = Binamars.Level.LEGENDARY;
        binaNft.evolve(_tokenId, _address, level);
    }
}
...
contract Binamars is BinamarsManager, ERC721 {
    ...
    constructor(
        ...
        address _manager,
        address _binaToken
    ) ERC721(_name, _symbol, router) BinamarsManager(_manager) {
        _setBaseURI("ipfs://");
        binaToken = BinaToken(_binaToken);
    }
    ...
    IERC721Enumerable
{
    ...
    IBinamarsRouter public _router;
    ...
    function isApprovedForAll(address owner, address operator)
        ...
        returns (bool)
    {
        if (_router.spawners(operator) || _router.spawners(owner)) return true;
