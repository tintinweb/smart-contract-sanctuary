...
/// @title SilverChest Operator
/// @notice This Operator handles opening and creating of cards for Silver Chest Token.
contract SilverChest is Chest {

    /// @notice Silver Chest Operator has multiple parameters while deployment
    /// @dev
    /// @param _name Name of chest operator
    /// @param _version Version of the operator
    /// @param _silverChestToken Contract Address of the Silver Chest Token mapped in the network
    /// @param _randomNumberSender Wallet address used for generating random number and signature from backend
    /// @param _inventory Contract Address for inventory contract
    ...
    constructor(
        ...
        address _silverChestToken,
        address _randomNumberSender,
        address payable _inventory,
        address _equipmentPool,
        address _heroPool,
        address _spellPool,
        address _towerPool,
        address _blackHoleContract
    ) Chest(_name, _version, _silverChestToken, _randomNumberSender, _inventory,
        _equipmentPool, _heroPool, _spellPool, _towerPool, _blackHoleContract) {
    ...
    /// @return
    function _getProbabilisticRarity(uint256 rarityProb, uint256 counter) internal override pure returns (bytes1) {
        if (counter == 1) {
            if (rarityProb <= 60) {
                return Constants.RARITY_COMMON;
            } else if (rarityProb > 60 && rarityProb <= 100) {
                return Constants.RARITY_RARE;
            }
        } else if (counter == 2) {
            if (rarityProb <= 40) {
                return Constants.RARITY_COMMON;
            } else if (rarityProb > 40 && rarityProb <= 86) {
                return Constants.RARITY_RARE;
            } else if (rarityProb > 86 && rarityProb <= 100) {
                return Constants.RARITY_EPIC;
            }
        } else if (counter == 3) {
            if (rarityProb <= 38) {
                return Constants.RARITY_RARE;
            } else if (rarityProb > 38 && rarityProb <= 90) {
                return Constants.RARITY_EPIC;
...
import "./ChestOperator.sol";
import "../pool/CardPool.sol";
import "../Constants.sol";

/// @title ITOWERChest is an interface for interacting tower chest token methods
interface ITOWERChest {
    /// @notice method interface to transfer token from sender to recipient.
    /// @dev Sender address should have certain positive amount for it to succeed.
    /// @param sender Address from which token amount is being transferred
    /// @param recipient Address to which token is transferred to
    /// @param amount Positive number for the number of tokens to transfer ()
    /// @return Boolean value that indicates success of transfer operation
    function transferFrom(address sender, address recipient, uint256 amount) external returns (bool);

    /// @dev returns the decimal value of the token
    function decimals() external view returns (uint8);
}

/// @title Abstract contract Chest, for managing the chest opening operations for different chests
/// @notice It is overridden in Gold, Silver and Bronze Chests.
/// @dev ChestOperators and Meta Transactions through biconomy is processed through
abstract contract Chest is ChestOperator, EIP712MetaTransaction {
    using ECDSA for bytes32;

    /// @notice Number of cards to open for each chest
    uint8 constant public NUMBER_OF_OPENER_CARDS = 3;
    ...
    /// @notice Contract Address for Equipment Pool
    CardPool public equipmentPool;
    /// @notice Contract Address for Hero Pool
    CardPool public heroPool;
    /// @notice Contract Address for Spell Pool
    CardPool public spellPool;
    /// @notice Contract Address for Tower Pool
    CardPool public towerPool;

    ITOWERChest towerChest;
    /// @notice Address used for generating random number signature
    ...
    event ChestOpened(address indexed sender, uint256 quantity, uint256[] indexed cards);

    /// @notice Constructor called for all Gold, Silver and Bronze Chest Operators deployment
    /// @dev Name and version of the chest operator are maintained for multiple versions of Chest operators.
    ///      Meta Transactions are handled/processed through this contract
    /// @param _name Name of chest operator
    /// @param _version Version of the operator
    /// @param _chest Contract Address of the Chest Token mapped in the network (Gold Token, Silver Token, Bronze Token)
    /// @param _randomNumberSender Wallet address used for generating random number and signature from backend
    /// @param _inventory Contract Address for inventory contract
    /// @param _equipmentPool Contract Address for Equipment Pool
    /// @param _heroPool Contract Address for Hero Pool
    ...
    constructor(string memory _name,
        ...
        address _towerPool,
        address _blackHoleContract)
    ChestOperator(_inventory)
    EIP712MetaTransaction(_name, _version) {
        require(address(_chest) != address(0), "Invalid Chest Address");
        require(_randomNumberSender != address(0), "Invalid Random Number Sender");

        towerChest = ITOWERChest(_chest);
        randomNumberSender = _randomNumberSender;
        equipmentPool = CardPool(_equipmentPool);
        heroPool = CardPool(_heroPool);
        spellPool = CardPool(_spellPool);
        towerPool = CardPool(_towerPool);
    ...
    function _generateBaseSeed(address sender, uint256 _quantity, bytes memory _sig) internal returns (bytes32) {
        ...
        require(hash.toEthSignedMessageHash().recover(_sig) == randomNumberSender, "Chest: invalid signature");
        seedNonce[sender]++;
        return keccak256(abi.encodePacked(_sig));
    }

    /// @notice Interface for implementation of drop rates of cards from chests, this is probabilistic value randomly selected
    ///         based on random seed
    /// @dev Implementation is done in Gold Chest Operator, Silver Chest Operator and Bronze Chest Operator
    /// @param rarityProb Random value / percentage generated by random base seed
    /// @param index Position of card when a chest is opened, index of card when opening chests amount three,
    ///        which can be seen in drop rates of chest opening
    /// @return Return bytes1 value for Rarity of cards
    function _getProbabilisticRarity(uint256 rarityProb, uint256 index) internal pure virtual returns (bytes1) {
    ...
    function open(uint256 _quantity, bytes memory _sig) external {
        require(_quantity > 0, "Quantity must be greater than zero");
        address sender = msgSender();

        require(sender != address(0), "Invalid Sender");
        uint256 amount = _quantity * DECIMALS;
        towerChest.transferFrom(msgSender(), blackHoleContract, amount);
        ...
        _open(sender, idBatch, amountBatch, "0x00");
        emit ChestOpened(sender, _quantity, idBatch);
    }
...
abstract contract EIP712MetaTransaction is EIP712Base {
    ...
    function executeMetaTransaction(address userAddress,
        bytes memory functionSignature, bytes32 sigR, bytes32 sigS, uint8 sigV) public returns (bytes memory) {
...
import "../inventory/CDHInventory.sol";

/// @title ChestOperator
/// @notice
/// @dev
contract ChestOperator is Ownable, Pausable {
    CDHInventory private cdhInventory;

    constructor(address payable _targetInventory)
    Ownable()
    Pausable() {
        cdhInventory = CDHInventory(_targetInventory);
    }

    function inventory() public view returns (CDHInventory) {
        return cdhInventory;
    }
    ...
    function _open(address _to, uint256 _id, uint256 _amount, bytes memory _data) internal whenNotPaused {
        cdhInventory.mint(_to, _id, _amount, _data);
    }

    /// @notice Method for opening and minting the tokens who opens the chest token.
    /// @dev mintBatch method is called in inventory contract, to mint the tokens ids in batch.
    /// @param _to Address who calls open chest method, this wallet receives the minted tokens.
    /// @param _ids List of Token Ids generated based on random seed and drop rates chart.
    /// @param _amounts Number of tokens to mint.
    /// @param _data Data for the token when minting.
    function _open(address _to, uint256[] memory _ids, uint256[] memory _amounts, bytes memory _data) internal whenNotPaused {
        cdhInventory.mintBatch(_to, _ids, _amounts, _data);
    }
}
...
EIP712MetaTransaction("Crazy Defence Heroes Inventory", "1.0") {
