...
abstract contract BasePool is IBasePool, BasePoolAuthorization, BalancerPoolToken, TemporarilyPausable {
     ...
     * Pool.
     */
    function _scalingFactor(IERC20 token) internal view returns (uint256 result) {
        // prettier-ignore
...
contract WeightedPool is BaseMinimalSwapInfoPool, WeightedMath {
    ...
    function _normalizedWeight(IERC20 token) internal view virtual returns (uint256 result) {
        // prettier-ignore
    ...
    function _doJoin(
        ...
        uint256[] memory normalizedWeights,
        bytes memory userData
    ) private view returns (uint256 result1, uint256[] memory result2) {
