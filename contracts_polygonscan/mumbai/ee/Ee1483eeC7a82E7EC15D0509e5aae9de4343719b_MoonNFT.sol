...
import "../../buildship/ReferralNFT.sol";
import "../../buildship/TierNFT.sol";
...
contract MoonNFT is ReferralNFT, TierNFT {
    ...
    function initTiers(uint8) internal override {
        ...
        _priceByTier[TierId.wrap(uint8(Tier.Standard))] = 1 ether;
        _priceByTier[TierId.wrap(uint8(Tier.Elite))] = 5 ether;
        _priceByTier[TierId.wrap(uint8(Tier.VIP))] = 7 ether;
        _priceByTier[TierId.wrap(uint8(Tier.Prestige))] = 1 ether;
        _priceByTier[TierId.wrap(uint8(Tier.President))] = 3 ether;
        _priceByTier[TierId.wrap(uint8(Tier.Genesis))] = 1 ether; // isn't used, because only 1 and it's reserved
