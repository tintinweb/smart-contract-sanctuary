...
contract DanceOff is VRFConsumerBase, Ownable{
    ...
    function enterRoyale(uint256 _tokenId, address _address, uint8 _hghJuice) public returns (uint256){
        ...
        if(_address == mmAddress){
            require(IMaticMike(mmAddress).ownerOf(_tokenId) == msg.sender || IHgh(hghAddress).getStaker(_tokenId) == msg.sender || IECL(eclAddress).getStaker(_tokenId) == msg.sender, "Not the owner of token");
        }
        else{
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        ...
        if(responseIdToBattle[requestId]._contract == mmAddress){
            ...
            if(curHolder != hghAddress && curHolder!= eclAddress){
                ...
                tokenHolder = IECL(eclAddress).getStaker(responseIdToBattle[requestId].tokenId);
            }
            ...
            if(curHolder != hghAddress){
                ...
                tokenHolder = IHgh(responseIdToBattle[requestId]._contract).expansionGetStaker(responseIdToBattle[requestId]._contract, responseIdToBattle[requestId].tokenId);
            }
        }
        else{
