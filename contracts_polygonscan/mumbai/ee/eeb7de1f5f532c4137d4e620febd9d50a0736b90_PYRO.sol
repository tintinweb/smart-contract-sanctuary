...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    ...
    uint256 public fundPercent = 45; // 0.045 = 4.5%
    uint256 public burnPercent = 5; // 0.005 = 0.5%

    address public fund = 0xEAe12592170251D793e0F4d70A76fD0849be92eb; // special fund account, gets 4.5%
    ...
    function editBurnPercent(uint _newpercent) public onlyOwner {burnPercent = _newpercent;}
    ...
    uint numWhitelisted;
    mapping (uint => Whitelist) Whitelisted;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        // dedcuts the full amount from the sender
        uint256 senderBalance = _balances[sender];
        require(senderBalance >= amount, "ERC20: transfer amount exceeds balance");
        _balances[sender] = senderBalance - amount;

        // checks if the sender is whitelisted or not
        // if yes, only trasfer, no burn or tax
        ...
        else { // burn and tax applies

        uint256 burnAmount = ((amount * burnPercent) / 10**21) * 10**18; // tricky when 0.5% 
        uint256 fundAmount = ((amount * fundPercent) / 10**21) * 10**18; // tricky when 0.5% 
        uint256 remainderAmount = amount - burnAmount - fundAmount;
...
contract PYRO is Ownable, ERC20PresetFixedSupply, ERC20Pausable {

    address private constant OWNER = 0xEE2a7b2c72217f6EbF0401DAbb407C7a600d910F;
    // address private constant fund = 0xEAe12592170251D793e0F4d70A76fD0849be92eb;

    constructor()
    ERC20PresetFixedSupply('PYRO on Mumbai', 'PYRO', 200_000_000 ether, OWNER) {
