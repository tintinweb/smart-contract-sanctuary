...
// File: contracts/JoeNiftysPunkEdition.sol
...
contract JoeNiftysPunkEdition is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  address payable public maticSink;

  constructor(
    string memory _initBaseURI,
    address payable _sink
  ) ERC721("JoeNiftysPunkEdition", "jPUNK") {
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    if(supply >= 25) {
        ...
        giftValue = supply + _mintAmount == maxSupply ? address(this).balance : msg.value * 10 / 100;
    }

    if(supply >= 25) {
        (bool zs, ) = payable(maticSink).call{value: msg.value * 85 / 100}("");
        require(zs);
        (bool gs, ) = payable(giftAddress).call{value: giftValue}("");
        require(gs, "Could not send value!");
    }    
