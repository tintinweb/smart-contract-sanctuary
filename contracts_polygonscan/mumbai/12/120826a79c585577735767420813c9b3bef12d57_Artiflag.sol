...
contract Artiflag is ERC721Enumerable, Ownable {
    ...
    uint256 public preSaleDate =    1621830400;
    uint256 public preSaleEndDate = 1631830400;
    uint256 public publicSaleDate = 1633830400;
    ...
    function presaleValidations(uint256 _ownerMintedCount, uint256 _mintAmount, uint256 _supply) internal {
        ...
        require(_ownerMintedCount + _mintAmount <= nftPerAddressLimitPresale, "max NFT per address exceeded");
        require(msg.value >= actualCost * _mintAmount, "insufficient funds");
        require(_mintAmount <= maxMintAmountPresale,"max mint amount exceeded");
        require(_supply + _mintAmount <= preSaleMaxSupply,"max NFT presale limit exceeded");
    }

    function publicsaleValidations(uint256 _mintAmount) internal {
    ...
    function mint(uint256 _mintAmount) public payable notPaused saleStarted minimumMintAmount(_mintAmount) {
        ...
        block.timestamp < publicSaleDate ? presaleValidations(ownerMintedCount, _mintAmount, supply) : publicsaleValidations(_mintAmount);
