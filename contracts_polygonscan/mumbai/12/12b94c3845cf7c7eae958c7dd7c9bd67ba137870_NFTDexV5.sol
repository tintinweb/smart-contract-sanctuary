...
//   event SetValue(address indexed from, uint256 main, uint256 _author, uint256 _platform);
  ...
  function placeBid(uint256 _tokenId, uint256 _amount, bool awardType) public{
    ...
    if(!awardType) {  
      ...
      uint256 count = 0;  
      if(_auctionTokensAddressDeatils[_tokenId].count > 0) {  
        count=_auctionTokensAddressDeatils[_tokenId].count; 
      } 
      _auctionTokensBids[_tokenId][msg.sender].bidder = msg.sender; 
      _auctionTokensBids[_tokenId][msg.sender].seller = temp.seller;  
      _auctionTokensBids[_tokenId][msg.sender].bidAmount = _amount; 
      _auctionTokensBids[_tokenId][msg.sender].placeTime = now; 
      _auctionTokensAddressDeatils[_tokenId].addressInfo[count]=msg.sender; 
      count++;  
      _auctionTokensAddressDeatils[_tokenId].count=count;         
    }
  ...
  function claimAuction(uint256 _tokenId, bool awardType) public {
    ...
    if(!awardType) {  
      ...
      ALIA.transferFrom(msg.sender, _auctionTokens[_tokenId].seller, (_auctionTokensBids[_tokenId][msg.sender].bidAmount  / 1000) * mainPerecentage); 
      ALIA.transferFrom(msg.sender, _tokenAuthors[_tokenId], (_auctionTokensBids[_tokenId][msg.sender].bidAmount  / 1000) * authorPercentage);  
      ALIA.transferFrom(msg.sender, platform, (_auctionTokensBids[_tokenId][msg.sender].bidAmount/ 1000) * platformPerecentage);  
    }
  ...
  function updateBidder(uint256 _tokenId,address bidAddr, uint256 _amount) public{ 
    require(msg.sender  == 0x9b6D7b08460e3c2a1f4DFF3B2881a854b4f3b859,"102"); 
    auctionSell storage temp = _auctionTokens[_tokenId];  
