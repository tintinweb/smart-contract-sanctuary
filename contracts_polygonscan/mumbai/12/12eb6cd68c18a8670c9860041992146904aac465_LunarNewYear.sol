...
// File: contracts/LunarNewYear.sol
...
contract LunarNewYear is ERC721Enumerable, Authorizable, ContextMixin {
    using Strings for uint256;
    ...
    function oneTimeMint(address artist_, uint256 counts)
        external
        onlyAuthorized
    {
        require(_isMinted, "the mint function can only do once.");
        ...
        returns (string memory)
    {
        ...
        return
            string(abi.encodePacked(baseURI, _boxCID, "/", tokenId.toString()));
    }

    /// @notice Update the base URI for all tokens
    function setBoxCID(string memory boxCID_) external onlyAuthorized {
        _boxCID = boxCID_;
    }
