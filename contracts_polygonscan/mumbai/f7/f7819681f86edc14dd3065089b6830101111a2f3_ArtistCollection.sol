    ...
    AccessControl
{
    ...
    /**  
    * @dev required for verisart to provide gasless minting 
    */
    bytes32 public constant SIGNABLE_MINTER_ROLE = keccak256("SIGNABLE_MINTER_ROLE");

    address private _royaltyFallback;
    ...
    constructor(
        ...
        */
        ...
        if (signableMinter != address(0)) {
            _setupRole(SIGNABLE_MINTER_ROLE, signableMinter);
        }
     ...
     * @dev Throws if called by any account other than an approved minter.
     */
    modifier onlySignableMinter() {
        require(hasRole(SIGNABLE_MINTER_ROLE, msg.sender), "Restricted to approved signable minters");
        _;
    }
    ...
    function mintVerisart(
        ...
        address to = Signable.recoverPersonalAddressGasless(_tokenURI, v, r, s);
    ...
    function mintEditionsVerisart(
        ...
        address to = Signable.recoverPersonalAddressBulkGasless(
            _tokenURIs,
            v,
            r,
            s
    ...
    {
        return payable(_royaltyFallback);
    }
...
library Signable {
    ...
    /**
     * @dev Personal: recovers the address from a personal sign from the user when doing gasless minting on polygon
     */
    function recoverPersonalAddressBulkGasless(
        string[] memory _tokenURIs,
        uint8 v,
        bytes32 r,
        bytes32 s
    ...
    function recoverPersonalAddressGasless(
        string memory _tokenURI,
        uint8 v,
        bytes32 r,
        bytes32 s
        ...
        bytes memory prefix = "\x19Ethereum Signed Message:\n32";
        bytes32 h = keccak256(abi.encode(this, _tokenURI));
        bytes32 prefixedHash = keccak256(abi.encodePacked(prefix, h));
        address _address = ecrecover(prefixedHash, v, r, s);
