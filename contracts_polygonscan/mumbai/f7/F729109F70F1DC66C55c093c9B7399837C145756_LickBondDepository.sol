...
// File: contracts/LickBondDepository.sol
...
contract LickBondDepository is Ownable {
    ...
    /* ======== STATE VARIABLES ======== */
    address public immutable LICK; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints LICK when receives principle
    ...
    struct Bond {
        uint payout; // LICK remaining to be paid
    ...
    function deposit(
        ...
        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 LICK ( underflow protection )
        ...
        /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) LICK
         */
        ...
        if ( fee != 0 ) { // fee is transferred to dao
            IERC20( LICK ).safeTransfer( DAO, fee );
        }
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            IERC20( LICK ).transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                IERC20( LICK ).approve( stakingHelper, _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return IERC20( LICK ).totalSupply().mul( terms.maxPayout ).div( 100000 );
    }
    ...
    /**
     *  @notice calculate current ratio of debt to LICK supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns ( uint debtRatio_ ) {
        uint supply = IERC20( LICK ).totalSupply();
    ...
    /**
     *  @notice calculate amount of LICK available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or LICK) to the DAO
     *  @return bool
     */
