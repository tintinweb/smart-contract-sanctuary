...
contract BaseDeployer is Initializable, AccessControl {
    ...
    function getTypeProbability(uint8 build) public view returns (uint16) {
        for (uint256 i = 0; i < buildTypes.length; i++) {
            if (build == buildTypes[i]) return typeProbabilities[i];
        }
        return typeProbabilities[0];
    }

    function getArrayTotal(uint16[] memory probabilities)
        public
        pure
        returns (uint16)
    {
    ...
    function getProbabilitiesTotal() public view returns (uint16) {
        return getArrayTotal(typeProbabilities);
    }
        ...
        returns (EmployeeLibrary.DeployerData memory)
    {
        return
            EmployeeLibrary.DeployerData(
                ...
                buildModels,
                typeProbabilities,
                getArrayTotal(typeProbabilities)
            );
    }

    // Alterators

    function addbuildTypes(uint8 build, uint16 p) public onlyRole(MAIN_OWNER) {
        buildTypes.push(build);
        typeProbabilities.push(p);
    }
        ...
        returns (uint8)
    {
        ...
        return
            typeByRandom(
                newRandom(0, getArrayTotal(probabilities)),
                probabilities
            );
    }

    function randomModel() public returns (uint8) {
        return buildModels[newRandom(0, buildModels.length - 1)];
    }
        ...
        returns (uint8)
    {
        ...
        for (uint256 i = 0; i < probabilities.length; i++) {
            ...
            if (momentAdd <= random && random <= nextAdd) {
                return buildTypes[i];
            }
            momentAdd += probabilities[i];
        }

        return buildTypes[buildTypes.length - 1];
    }
