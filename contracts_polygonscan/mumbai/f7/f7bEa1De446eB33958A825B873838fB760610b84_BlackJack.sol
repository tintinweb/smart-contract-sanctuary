...
import "./utils/BlackJackFunction.sol";
    ...
    // IGame
{
    ...
    mapping(uint256 => uint8[]) public nonBustedPlayers;
    mapping(uint256 => uint8[]) public dealerHand;
    mapping(address => mapping(uint256 => uint8[])) public playerHand;
    mapping(address => mapping(uint256 => uint8[])) public playerSplit;
    mapping(address => mapping(uint256 => bool)) public playerInsurance;
    mapping(address => mapping(uint256 => bool)) public isPlayerInGame;
    ...
    modifier onlyNewGame(uint8 _rId) {
        require(
            games[_rId].state == GameState.NewGame,
            "BJ:1"
        );
        _;
    }
    ...
    modifier onlyOnGoingGame(uint8 _rId) {
        require(
            games[_rId].state == GameState.OnGoingGame,
            "BJ:2"
        );
        _;
    }

    modifier onlyEndedGame(uint8 _rId) {
        require(
    ...
    // modifier onlyPlayerInPosition(
    //     uint8 _rId,
    //     uint8 _position,
    //     address _player
    // ) {
    //     require(
    //         games[_rId].players[_position] == _player,
    //         "BJ:5"
    //     );
    //     _;
    // }
    ...
    modifier checkHash(uint8 _rId, bytes32 _hash) {
        require(
            keccak256(abi.encodePacked(_hash)) == games[_rId].childHash,
            "BJ:6"
        );
        _;
    }
    ...
    function createRoom(
        ...
        uint8 _rId,
        bytes32 _childHash
    ) external onlyRole(WORKER_ROLE) onlyEndedGame(_rId) {
        require(_rId < maxRoom, "273");
        Game storage game = games[_rId];
        // //retrieve the privious room data before setting the new game state.
        // Game memory prevGameRecord = game;
        ...
        prepareDeck(_rId);
        uint8 waitingPlayerCount;
        for (uint8 pIndex; pIndex < game.maxPlayer * 2; pIndex++) {
                if (game.playerState[pIndex] == PlayerState.WaitingPlayer) {
                    //set the new game state with waiting player in the game.
                    //we keep the waitingPlayer other states the same i.e. bets, position, etc.
                    // game.players[pIndex] = game.players[pIndex];
                    // game.bets[pIndex] = prevGameRecord.bets[pIndex];
                    game.playerState[pIndex] = PlayerState.NotBusted;
                    waitingPlayerCount +=1;
                    // isPlayerInGame[game.players[pIndex]][_rId] =true;
                } 
                else /*if (game.playerState[pIndex] != PlayerState.NoPlayer)*/{
                    // address player = game.players[pIndex];
                    game.playerState[pIndex] = PlayerState.NoPlayer;
                    game.players[pIndex] = address(0);
                    game.bets[pIndex] = 0;
                    delete isPlayerInGame[game.players[pIndex]][_rId];
                }
        }
    ...
    function joinGame(
        ...
        uint8 _position,
        uint128 _betAmount
    ) external whenNotPaused onlyNewGame(_rId) {
        require(_rId < maxRoom, "333");
        require(
            !isPlayerInGame[msg.sender][_rId],
            "336"
        );
        ...
        require(
            _betAmount >= game.minBet && _betAmount <= game.maxBet,
            "343"
        );

        require(
            _position < game.maxPlayer,
            "348"
        );

        require(
            game.playerState[_position] == PlayerState.NoPlayer,
            "353"
        );
        ...
        isPlayerInGame[msg.sender][_rId] = true;
        //if player joins the game in time, push the player to play instantly
        if (game.startTime > block.timestamp || game.startTime == 0){
            ...
            if (game.playerCount == 1) {
                ...
                game.startTime = block.timestamp;
            } else {//players more than 1 and less than max players.
                game.startTime += extraWaitingTime;
            }
        ...
        //player didn't join in time so they are forced to join the next game.
        else{
            //count the waiting player to be forced to join in the next game, not this game.
            // game.waitingPlayerCount ++;
            game.playerState[_position] = PlayerState.WaitingPlayer;

            emit JoinGame(
                game.gId+1, //gameId is the next game.
                _rId,//same roomId
                msg.sender,
                _position,
                _betAmount,
                block.timestamp //has to use the next game startTime but can't figure out how so we use the current time they came to sit instead
            );
        }
    }

    function startGame(uint8 _rId) external onlyNewGame(_rId) {
        Game storage game = games[_rId];
        ...
        onlyNotBusted(_rId, _position)
    {
        ...
        require(
            dealerHand[game.gId].length == 1 &&
                helper.getHandPower(dealerHand[game.gId]) == 11,
            "474"
        );
        ...
        checkHash(_rId, _parentHash)
    {
        ...
        require(
            playerSplit[player][_rId].length == 0,
            "568"
        );

        require(
            helper.getCardPower(playerHand[player][_rId][0]) ==
                helper.getCardPower(playerHand[player][_rId][1]) &&
                playerHand[player][_rId].length == 2,
            "575"
        );
    ...
    function _split(
        ...
        playerHit(_rId, _position, _parentHash);
        playerHit(_rId, split_position, _parentHash);
    }
        ...
        checkHash(_rId, _parentHash)
    {
        ...
        bytes32 newHash = keccak256(
            abi.encodePacked(_parentHash, results[_rId])
        );
        ...
        checkHash(_rId, _parentHash)
    {
        drawPlayerCard(
            _rId,
            _position,
            keccak256(abi.encodePacked(_parentHash, results[_rId]))
        );
        ...
        private
    {
        ...
        for (uint8 i; i < _leftPlayers.length; i++) {
            ...
            if (game.playerState[i] == PlayerState.HasBlackjack) {
                vault.subGameBalance(player, game.bets[position], 0);
            }
