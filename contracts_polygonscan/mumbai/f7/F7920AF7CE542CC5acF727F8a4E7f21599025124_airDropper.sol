...
contract airDropper is Ownable, ERC1155Holder {
    ...
    function arrayTesting(address [] memory _amount) public pure returns (address[] memory){
        return _amount;
    }
    ...
    function showCurrentBalance(address _address) public view returns(uint){
        uint currentBalance = currentToken.balanceOf(_address, tokenId);
        return currentBalance;
    }
    
    function drop(address[] memory dropAddresses, uint [] memory dropAmounts) external onlyOwner() {
        // limit drops to 100
        ...
        uint _amount = dropAmounts[i];
        currentToken.safeTransferFrom(address(this), _to, tokenId, _amount, "");
        }
    }
}