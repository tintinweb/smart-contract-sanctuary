...
contract AwesomeNfts is ERC721Enumerable, Ownable {                 //contract Name is NErdyCoderCLone and inherited from the openzaplin libraries
  ...
  uint256 public cost = 0.8 ether;                                      //amouont of each nft in ether if this in polygon so it also works as same but in matic (native ) token 
  ...
  constructor(                                                     //when you deply the contract first time so give this fdetails
    ...
    mint(msg.sender, 10);                                         //rewar6ds with the 20nft to the deployer of the contract free of cpst
  }
