...
contract Motion is ERC721, Ownable, RandomlyAssigned {
    ...
    ) ERC721("The Motion Collection", "TMC")
    RandomlyAssigned(150, 1)
    {
        setSaleStart(_saleStart);
        setBaseURI(_baseURI);
        setNotRevealedURI(_notRevealedURI);
        setWhitelistedUsers(_whitelisted);
    }
    ...
    function mint() public payable {
        ...
        if(msg.sender != owner()){
            if(isSale()){
                require(whitelisted[msg.sender], "User not in whitelist!");
                _mintToken();
            } else {
                revert("Sale has not yet started!");
            }
        } else {
            _mintTokenByOwner();
        }
    ...
    function removeWhitelistedUsers(address[] memory _users) public onlyOwner {
        for(uint256 i = 0; i < _users.length; i++){
            require(_users[i] != address(0), "Null address");
