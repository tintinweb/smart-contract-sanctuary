...
import "./interfaces/ISavageNFT.sol";
...
contract SavageStock is Ownable {
  ...
  function nft() public view returns (ISavageNFT) {
    return _nft;
  }
  ...
  function salesCount() public view returns (uint256) {
    return _salesCount;
  }
  ...
  event AuctionFinished(uint256 saleId, address indexed caller);
  ...
  constructor(
    ...
    for (uint256 i = 0; i < paymentTokens_.length; i++) {
      _supportedPaymentTokens.add(paymentTokens_[i]);
    }
    _supportedPaymentTokens.add(wrappedToken_);
    _supportedPaymentTokens.add(savageToken_);
    _supportedPaymentTokens.add(stripeToken_);
  }

  function addPaymentToken(address token) external returns (bool) {
    require(!isPaymentTokenExist(token), "Token already exists");
  ...
  function removePaymentToken(address token) external returns (bool) {
    require(isPaymentTokenExist(token), "Token is not supported");
  ...
  function bid(address bidder, uint256 saleId) external payable returns (bool) {
    ...
    require(_supportedPaymentTokens.contains(_wrappedToken), "WrappedToken is unsupported");
    IWETH(_wrappedToken).deposit{value: value}();
    return _bid(bidder, saleId, _wrappedToken, value);
  }

  function bid(address bidder, uint256 saleId, address tokenIn, uint256 amountIn) external returns (bool) {
    require(_supportedPaymentTokens.contains(tokenIn), "TokenIn is unsupported");
    return _bid(bidder, saleId, tokenIn, amountIn);
  }

  function _bid(address bidder, uint256 saleId, address tokenIn, uint256 amountIn) private returns (bool success) {
    ...
    if (sale_.buyer != address(0)) {
      ...
      IERC20(sale_.tokenIn).safeTransfer(sale_.buyer, amountOut);
      requiredStableAmount += sale_.step;
    }
    if (tokenIn != _wrappedToken) IERC20(tokenIn).safeTransferFrom(caller, address(this), amountIn);
    if (tokenIn != _intermediateStablecoin && tokenIn != _stripeToken) {
      amountIn = _swap(_getBidPath(tokenIn), amountIn);
    }
    ...
    emit Bidded(saleId, caller, bidder, tokenIn, amountIn);
  }
  ...
  function finishAuction(uint256 saleId) external returns (bool) {
    ...
    bool buyerExist = sale_.buyer != address(0);
    if (buyerExist) {
      uint256 amountOut = sale_.price;
      if (sale_.tokenIn == _stripeToken) {
        require(_collateralPool.swap(this_, amountOut), "CollateralPool swap failure");
      } else {
        amountOut = _swap(_getOutPath(_savageToken), amountOut);
      }
      if (sale_.tokenIn == _savageToken) {
        uint256 discountAmountOut = (amountOut * (100 - _savageDiscountPercent)) / 100;
        if (discountAmountOut > 0) {
          amountOut -= discountAmountOut;
          IERC20(_savageToken).safeTransfer(sale_.buyer, discountAmountOut);
        }
      } 
      IERC20(_savageToken).safeTransfer(sale_.seller, amountOut);
    }
    _nft.transferFrom(this_, buyerExist ? sale_.buyer : sale_.seller, sale_.tokenIds[0]);
    sale_.active = false;
    emit AuctionFinished(saleId, msg.sender);
    return true;
  }
  ...
  function createAndSaleNFT(
    ...
    uint256 startTimestamp,
    uint256 endTimestamp
  ) external onlyOwner returns (uint256[] memory tokenIds, uint256 saleId) {
    uint256 step = 0;
    ...
    if (count > 1) {
      require(!isAuction, "Auction when count gt 1");
    } else {
      if (isAuction) {
    ...
    tokenIds = _nft.mint(address(this), count);
    saleId = _salesCount;
    Sale storage sale_ = _sales[saleId];
    sale_.creator = creator;
    sale_.seller = creator;
    sale_.tokenIds = tokenIds;
    sale_.price = price;
  ...
  function stopSale(uint256 saleId) external returns (bool) {
    ...
    _nft.multiTransferFrom(address(this), sale_.seller, sale_.tokenIds);
    sale_.buyer = zero;
    sale_.price = 0;
    sale_.stopped = true;
    sale_.active = false;
  ...
  function _getBidPath(address tokenFrom) private view returns (address[] memory path) {
    address tokenTo = _intermediateStablecoin;
  ...
  function _getOutPath(address tokenTo) private view returns (address[] memory path) {
    ...
    path = new address[](length);
    path[0] = _intermediateStablecoin;
    path[length - 1] = tokenTo;
    if (length > 2) path[1] = _wrappedToken;
  }
...
interface ISavageNFT is IERC721 {
  ...
  event Minted(uint256 id, address indexed to);
  event CollectionMinted(Token[] tokens, address indexed to, uint256 collectionId);

  function mint(address to, uint256 count) external returns (uint256[] memory tokenIds);
