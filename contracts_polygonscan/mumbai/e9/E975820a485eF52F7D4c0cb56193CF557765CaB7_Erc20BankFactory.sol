...
import {Erc20Bank} from "./Erc20Bank.sol";

/**
 * @title Erc20BankFactory
 * @author Matrix
 */
contract Erc20BankFactory {
    // ==================== Constants ====================
    ...
    function getBank(uint256 index) external view returns (address) {
        return _banks[index];
    }

    function getLastBank() external view returns (address) {
        uint256 count = _banks.length;

        return count > 0 ? _banks[count - 1] : address(0);
    }
    ...
    function create(
        ...
        uint256 startBlock
    ) external returns (Erc20Bank) {
        Erc20Bank bank = new Erc20Bank(stakeToken, rewardToken, rewardPerBlock, startBlock);
        _banks.push(address(bank));

        // ADMIN_ROLE
        bank.grantRole(ADMIN_ROLE, msg.sender);
        bank.revokeRole(ADMIN_ROLE, address(this));

        // DEFAULT_ADMIN_ROLE
        bank.grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
        bank.revokeRole(DEFAULT_ADMIN_ROLE, address(this));
 ...
 * @author Matrix
 */
contract Erc20Bank is AccessControl, Pausable {
    ...
    uint256 internal _unstakeCount;
    uint256 internal _claimCount;
    ...
    event Stake(address indexed caller, address indexed from, uint256 stakeAmount, uint256 currentStakeAmount);
    event Unstake(address indexed caller, address indexed to, uint256 unstakeAmount, uint256 currentStakeAmount);
    ...
    {
        return (_userCount, _stakeCount, _unstakeCount, _claimCount);
    }
    ...
    function stake(address from, uint256 amount) external whenNotPaused {
        if (from == address(0)) {
            from = _msgSender();
        }

        _stakeToken.safeTransferFrom(from, address(this), amount);
        ...
        if (amount > 0) {
            _stakeCount++;
            _stakeAmount += amount;
            _userCount += (userInfo.sharePrice == 0) ? 1 : 0;

            uint256 newStakeAmount = userInfo.stakeAmount + amount;
            user.stakeAmount = newStakeAmount;
            emit Stake(_msgSender(), from, amount, newStakeAmount);
        }
    }

    function unstake(address to, uint256 amount) external whenNotPaused {
        if (to == address(0)) {
            to = _msgSender();
        }
        ...
        UserInfo memory userInfo = user;
        require(amount <= userInfo.stakeAmount, "unstake too many");
        ...
        if (amount > 0) {
            ...
            emit Unstake(_msgSender(), to, amount, newStakeAmount);
        }
    }
