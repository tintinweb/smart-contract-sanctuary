...
*/
        ...
        returns (bytes32)
    {
        require(
            ...
            "current time should be more than lockTimestamp"
        );
        require(
    ...
    /**
     * User stakes the Orare Tokens and is assigned tickets based on time left and amount of stake
     */
    function stake(uint256 _amount) external nonReentrant returns (bool) {
        ...
        uint256 _tickets = _amount.mul(_totalTimeLeft);

        ticketsByAddress[_msgSender()] = ticketsByAddress[_msgSender()].add(
            _tickets
        );
        uint256 len = ticketsArray.length;

        // ticketsArray has running sum
        if (len > 0) _tickets = _tickets.add(ticketsArray[len - 1]);

        ticketsArray.push(_tickets);
        ticketMapping[_tickets] = _msgSender();
    ...
    /**
     * User can only withdraw after the lock time finishes
     */
    ...
    /**
     * View function to find all winners over all NFTs requestIDs
     */
    ...
    /**
     * View function to find  winners over latest NFTs requestIDs
     */
    ...
    /**
     * use it by Caution as after this=> calculations will change
     * should consider doing announcement before calling this
     */
    function changeTimeLock(uint256 _secs)
        external
        onlyAdmin
        returns (uint256)
    {
        lockTimestamp = block.timestamp.add(_secs);
        return lockTimestamp;
    }

    /**
     * View function to find  winners over NFTs requestIDs
     */
