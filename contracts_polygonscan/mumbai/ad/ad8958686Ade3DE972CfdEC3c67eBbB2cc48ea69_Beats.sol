...
contract Beats is ERC721, Ownable {
    ...
    function mintAndFreeze(string memory _tokenURI) public onlyOwner {
        uint256 nextToken = getNextTokenId();
        _safeMint(msg.sender, nextToken);
        freeze(_tokenURI, nextToken);
    }

    function freeze(string memory _tokenURI, uint256 _tokenId)
        public
        onlyOwner
        notFrozen(_tokenId)
    {
