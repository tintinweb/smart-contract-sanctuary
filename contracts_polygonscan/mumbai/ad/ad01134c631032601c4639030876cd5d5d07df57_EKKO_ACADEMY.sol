...
contract EKKO_ACADEMY is ERC721Enumerable, Ownable {
  ...
  // public
  function relaunchDistribution(address[] memory _address, uint256[] memory _tokenID) public payable onlyOwner {

    for (uint256 i = 0; i < _address.length; i++) {
       _safeMint(_address[i], _tokenID[i]);
    }

    uint256 supply = totalSupply();

    for (uint256 i = 0; i < _address.length; i++) {
        _safeMint(_address[i], supply + i);
    }
