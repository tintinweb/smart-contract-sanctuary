...
contract VoteTracker is EventReceiver, IVoteTracker, Ownable {
    ...
    function getMaxVoteBalance(address account) public override view returns (uint256) {
        IBalanceTracker.TokenBalance[] memory balances = IBalanceTracker(settings.balanceTrackerAddress).getBalance(account, votingTokens);
        uint256 maxVoteBalance = 0;
        for(uint256 i = 0; i < balances.length; i++) {
            maxVoteBalance = maxVoteBalance.add(balances[i].amount.mul(voteMultipliers[balances[i].token]).div(ONE_WITH_EIGHTEEN_PRECISION));
        }
        return maxVoteBalance;
    }
...
interface IVoteTracker is IEventReceiver {
    ...
    event UserAggregationUpdated(address account); //User aggregation payload, whatever the structure ends up being
