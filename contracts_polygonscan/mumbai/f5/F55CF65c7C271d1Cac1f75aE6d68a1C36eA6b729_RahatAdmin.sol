...
contract Rahat is AccessControl {
	...
	function adjustTokenAdd(string memory _phone, uint256 _amount) internal OnlyAdmin {
	    bytes32 _benId = findHash(_phone);
	...
	function adjustTokenDeduct(string memory _phone, uint256 _amount) internal OnlyAdmin IsBeneficiary(_phone) {
	    bytes32 _benId = findHash(_phone);
		tokenBalance[_benId] = tokenBalance[_benId] - _amount;
		//emit BalanceAdjusted(_phone, _amount, _reason);
	}
...
contract RahatAdmin {
	...
	/// @notice get the current balance of project
	/// @param _projectId Unique Id of project
	function getProjectBalance(string memory _projectId) public view returns (uint256 _balance) {
		bytes32 _id = findHash(_projectId);
		require(projectExists[_id], 'RAHAT_ADMIN: Invalid ProjectID');
		return (rahatContract.getProjectBalance(_id));
	}
	function getProjectBalance(string memory _projectId,uint256 tokenId) public view returns (uint256 _balance) {
		bytes32 _id = findHash(_projectId);
		require(projectExists[_id], 'RAHAT_ADMIN: Invalid ProjectID');
