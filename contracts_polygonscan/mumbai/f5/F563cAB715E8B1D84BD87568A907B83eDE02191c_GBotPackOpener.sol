...
// GBots rarity
    ...
    uint internal constant GBOT_RARITY_MYTHICAL = 5;
    uint internal constant GBOT_RARITY_ULTIMATE = 6;
    ...
    uint256 internal constant _COMMON_PACK_DROP_RATE_THRESH_ULTIMATE = 0;
    ...
    uint256 internal constant _RARE_PACK_DROP_RATE_THRESH_MYTHICAL = 0.400 * 1000;
    uint256 internal constant _RARE_PACK_DROP_RATE_THRESH_ULTIMATE = 0;
    ...
    uint256 internal constant _EPIC_PACK_DROP_RATE_THRESH_MYTHICAL = 2 * 1000;
    uint256 internal constant _EPIC_PACK_DROP_RATE_THRESH_ULTIMATE = 0;
    ...
    uint256 internal constant _LEGENDARY_PACK_DROP_RATE_THRESH_MYTHICAL = 8 * 1000;
    uint256 internal constant _LEGENDARY_PACK_DROP_RATE_THRESH_ULTIMATE = 0;
}
...
contract GBotPackOpener is ERC1155TokenReceiver, Ownable, Pausable, OpenerProps {
    ...
    function openPack(address from, bytes calldata sig, uint256 collectionId) private {
        require(signerKey != address(0), "GBot PackOpener: signer key not set");
        // Determine collection type
        uint packTier = collectionId; 
    ...
    function determineRarity(uint256 packTier, uint256 seed) private pure returns (uint256 rarity) {
        ...
        if (packTier == PACK_TIER_RARE) {
                if (seedling < _RARE_PACK_DROP_RATE_THRESH_MYTHICAL) {
                    return GBOT_RARITY_MYTHICAL;
                }
        ...
        if (packTier == PACK_TIER_EPIC) {
                if (seedling < _EPIC_PACK_DROP_RATE_THRESH_MYTHICAL) {
                    return GBOT_RARITY_MYTHICAL;
                }
        ...
        if (packTier == PACK_TIER_LEGENDARY) {
                if (seedling < _LEGENDARY_PACK_DROP_RATE_THRESH_MYTHICAL) {
                    return GBOT_RARITY_MYTHICAL;
                }
                return GBOT_RARITY_LEGENDARY;
            }
    ...
    function onERC1155Received(
        ...
        openPack(from, sig, id);
        gBotPacksContract.burnFrom(address(this), id, value);
    ...
    function onERC1155BatchReceived(
        ...
        gBotPacksContract.batchBurnFrom(address(this), ids, values);
