...
// File: contracts/EthermonWorld.sol
...
interface EtheremonTransformSettingInterface {
    ...
    function getLayEggInfo(uint32 _classId)
        external
        view
        returns (uint8 layingLevel, uint8 layingCost);
    ...
    SafeMathEthermon
{
    ...
    // hatching range
    uint256 public hatchStartTime = 2; // hour
    uint256 public hatchMaxTime = 46; // hour
    uint256 public removeHatchingTimeFee = 0.05 ether; // ETH
    uint256 public buyEggFee = 0.09 ether; // ETH
    ...
    // events
    event EventLayEgg(address indexed trainer, uint256 objId, uint256 eggId);
    event EventHatchEgg(address indexed trainer, uint256 eggId, uint256 objId);
    ...
    constructor(
        ...
        tradeContract = address(0); //_tradeContract;
    ...
    function setConfig(
        uint256 _removeHatchingTimeFee,
        uint256 _buyEggFee,
        uint256 _hatchStartTime,
        uint256 _hatchMaxTime
    ) external onlyModerators {
    ...
    */
    ...
    // function removeHatchingTimeWithToken(address _trainer)
    //     external
    //     isActive
    //     onlyModerators
    // {
    ...
    //         egg.hatchTime,
    //         egg.newObjId
    //     ) = transformData.getHatchingEggData(_trainer);
    ...
    //         monsterNFTContract
    //     );
    //     uint256 objId = monsterNFT.mintMonster(
    //         egg.classId,
    //         egg.trainer,
    //         "..name me..."
    //     );
    ...
    // function buyEggWithToken(address _trainer)
    //     external
    //     isActive
    //     onlyModerators
    // {
    ...
    //         _trainer,
    //         block.number - 1,
    //         transformData.totalEgg()
    //     );
    //     uint32 classId = EtheremonTransformSettingInterface(
    //         transformSettingContract
    //     ).getRandomClassId(seed);
    //     if (classId == 0) revert();
    //     uint64 eggId = transformData.addEgg(
    //         0,
    //         classId,
    //         _trainer,
    //         block.timestamp + (hatchStartTime + (seed % hatchMaxTime)) * 3600
    //     );
    //     // deduct exp
    ...
    function getGen0ObjInfo(uint64 _objId)
        public
        view
        returns (
            uint32,
    ...
    // function calculateMaxEggG0(uint64 _objId)
    //     public
    //     view
    //     returns (uint256)
    // {
    ...
    //     if (classId > GEN0_NO || classId == 20 || classId == 21) return 0;

    //     Gen0Config memory config = gen0Config[classId];
    //     // the one from egg can not lay
    //     if (createIndex > config.total) return 0;

    //     // calculate agv price
    //     uint256 avgPrice = config.originalPrice;
    //     uint256 rate = config.originalPrice / config.returnPrice;
    //     if (config.total > rate) {
    //         uint256 k = config.total - rate;
    //         avgPrice =
    //             (config.total *
    //                 config.originalPrice +
    //                 (config.returnPrice * k * (k + 1)) /
    //                 2) /
    //             config.total;
    //     }
    ...
    //     }
    //     return
    //         ceil(((catchPrice - totalEarn) * 15 * 1000) / avgPrice, 10000) /
    //         10000;
    // }

    // function layEgg(uint64 _objId) external isActive {
    //     EthermonTransformData transformData = EthermonTransformData(
    //         transformDataContract
    //     );
    //     // make sure no hatching egg at the same time
    ...
    //     } else {
    //         uint8 layingLevel;
    //         (layingLevel, temp) = EtheremonTransformSettingInterface(
    //             transformSettingContract
    //         ).getLayEggInfo(classId);
    //         if (
    //             layingLevel == 0 ||
    //             currentLevel < layingLevel ||
    //             currentLevel < temp
    //         ) revert();
    ...
    // function hatchEgg() external isActive {
    ...
    //         egg.hatchTime,
    //         egg.newObjId
    //     ) = transformData.getHatchingEggData(msg.sender);
    //     // not hatching any egg
    //     if (egg.eggId == 0 || egg.trainer != msg.sender) revert();
    //     // need more time
    ...
    // function removeHatchingTime() external payable isActive {
    //     EthermonTransformData transformData = EthermonTransformData(
    //         transformDataContract
    //     );
    //     MonsterEgg memory egg;
 ...
 */
    ...
    function transform(
        ...
        //     // legends
        //     if (getBonusEgg(_objId) > 0) revert();
        // } else {
        if (objInfo.classId > GEN0_NO)
    ...
    // function buyEgg() external payable isActive {
    ...
    // read
    // function getBonusEgg(uint64 _objId) public view returns (uint256) {
    //     EthermonTransformData transformData = EthermonTransformData(
    //         transformDataContract
    //     );
    //     uint256 totalBonusEgg = calculateMaxEggG0(_objId);
    //     if (totalBonusEgg > 0) {
