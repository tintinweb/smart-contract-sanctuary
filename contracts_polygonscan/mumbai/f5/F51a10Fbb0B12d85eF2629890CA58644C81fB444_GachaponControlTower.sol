...
contract GachaponControlTower is TreasuryManager, CurrenciesManager, GachaponOperator, VRFManager {
  ...
  event GachaponCreated(address indexed _factory, string _title, address _gachapon, address indexed _currency, uint _timestamp);
  ...
  function createGachapon(string memory _machineTitle, IERC20 _currency) public {
    ...
    emit GachaponCreated(address(gachaponFactory), _machineTitle, address(_gachapon), address(_currency), block.timestamp);
...
contract OperatorsManager is Ownable {
  ...
  event OperatorsAuthorizationChanged(address indexed _address, bool indexed _isAuthorized);
  ...
  function addAddressToAuthorizedOperators(address _address) public onlyOwner {
    ...
    emit OperatorsAuthorizationChanged(_address, authorizedOperators[_address]);
  }
