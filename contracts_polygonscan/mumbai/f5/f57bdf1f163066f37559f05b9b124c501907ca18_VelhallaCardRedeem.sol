...
contract VelhallaCardRedeem is ERC1155Supply, Ownable {
    ...
    string public constant name = "Velhalla Card";
...
//	uint256 public totalSeriesNumber = 1;
	...
	uint256 public maxSilverCardCopy = 500;
...
//	uint256 public latestRevealChapter = 0;
    ...
    uint256 public GOLD_CARD_COPY = 500;
...
/*
	function redeembyChapter(address _from, uint32 _chapterNo) external{
        ...
        maxPageCopy -= pageNumberPerChapter ;
	}
*/

    function claimSilverCardByChapter(uint32 _chapter) public returns (bool) {
        ...
        require(totalSupply(idtemp) < maxSilverCardCopy, "max redeem of Silver Card exceed");
...
//        require(tokenContract.allowance(msg.sender, address(this)) >= unitPrice, "allowance too low");
        comicContract.redeembyChapter(msg.sender, uint32(idtemp));
//        _safeTransferFrom(tokenContract, msg.sender, address(this), unitPrice);
        return true;
	}

    function claimSilverCardBySeries(uint32 _series) public returns (bool) {
        require((!silverPaused)&&(publicSilverRedeemStart <= block.timestamp), "the redeem for Silver Card is paused");
	    uint256[] memory ids = new uint256[](uint256(chaperNumberPerSeries));
        uint256[] memory amounts = new uint256[](uint256(chaperNumberPerSeries));   
        uint256 idstart;
        uint256 idpagetemp;
        uint256 idtemp;	

        idstart = (_series - 1) * chaperNumberPerSeries;	
		for (uint32 i = 0; i < chaperNumberPerSeries; i++) {
		...
		for (uint32 k = 0; k < chaperNumberPerSeries; k++) {
            idtemp =  idstart + k + 1; //chapter 1
            comicContract.redeembyChapter(msg.sender, uint32(idtemp)); 
            ids[k] =  idtemp;		    
            amounts[k] = 1; //1 copy of Card
		}
...
//        _safeTransferFrom(tokenContract, msg.sender, address(this), unitPrice);
    ...
    function claimGoldCard(uint256 _series) public returns (bool) {
        require((!goldPaused)&&(publicGoldRedeemStart <= block.timestamp), "the redeem for Gold Card is paused");
        require(_series <= ((idGoldCardNFTIndex+GOLD_CARD_COPY-ID_GOLD_CARD_START)/GOLD_CARD_COPY), "need to redeem under revealed series");
        require((idGoldCardNFTIndex-ID_GOLD_CARD_START) < GOLD_CARD_COPY,  "max redeem of Gold Card under revealed series exceeded"); 
	    uint256[] memory ids = new uint256[](uint256(chaperNumberPerSeries));
        uint256[] memory amounts = new uint256[](uint256(chaperNumberPerSeries));   
        uint256 idstart;
        uint256 idtemp;	
		...
		for (uint32 i = 0; i < chaperNumberPerSeries; i++) {
            ...
            ids[i] =  idtemp;		    
            amounts[i] = 1; //1 copy of Card
		}

//        require(tokenContract.allowance(msg.sender, address(this)) >= unitPrice, "allowance too low");
		
		idGoldCardNFTIndex +=1;
...
/* set for 721 token uri like */
    ...
    function setMaxSilverCardCopy(uint256 _maxCopy) public onlyOwner {
        maxSilverCardCopy = _maxCopy;
    }
