...
// TODO: remove console.log to reduce gas fee a bit on production

/**
 * @notice Custody smart contracts aim to provide a secure trading environment by holding the assets on the erc20 chain so that the user and broker can freely trade off-chain.
 */
contract SimpleVault is Initializable, AccessControlUpgradeable, IVault {
    ...
    mapping(address => mapping(bytes32 => bool)) private _sigUsage; // Prevent deposit & withdrawal signature can be reused before the signature expires.
    ...
    modifier onlyValidSignature(
        ...
        console.log("[Vault]", "(onlyValidSignature)", "account:", account);
        ...
        console.log(
            ...
            "(onlyValidSignature)",
            "action:",
            action == DEPOSIT_TYPE ? "deposit" : "withdraw"
        );
        {
     ...
     */
    function deposit(bytes calldata payload, bytes memory signature)
        public
        payable
        returns (bool)
    {
    ...
    function _deposit(
        ...
        internal
        onlyValidSignature(account, DEPOSIT_TYPE, payload, signature)
        returns (bool)
    {
        bytes32 sigHash = keccak256(signature);
        (bytes32 rid, , , Asset[] memory assets) = _extractPayload(
            account,
            sigHash,
            payload
        );
    ...
    function _withdraw(
        ...
        internal
        onlyValidSignature(account, WITHDRAW_TYPE, payload, signature)
        returns (bool)
    {
        bytes32 sigHash = keccak256(signature);
        (bytes32 rid, , , Asset[] memory assets) = _extractPayload(
        ...
        )
    {
        (
            ...
            address destination,
            Asset[] memory assets
        ) = abi.decode(payload, (bytes32, uint64, address, Asset[]));

        console.log("[Vault]", "(_extractPayload)", "rid:", uint256(rid));
        console.log(
            "[Vault]",
            "(_extractPayload)",
            "destination:",
        ...
        console.log("[Vault]", "(_extractPayload)", "expire", expire);
        console.log(
            "[Vault]",
            "(_extractPayload)",
            "block.timestamp",
