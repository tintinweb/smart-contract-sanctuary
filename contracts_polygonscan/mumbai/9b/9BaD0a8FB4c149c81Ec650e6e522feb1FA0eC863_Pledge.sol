...
contract Pledge is Context,AccessControlEnumerable,Ownable{

    bytes32 public constant PREDICTION = keccak256("PREDICTION");

    address rewardCardAddress = 0x8d643df4F8452FeFc2636c50cF6a5bf8a12FA111;
    ...
    struct roundInfo {
        ...
        uint256 endBlock;
        uint256 unitReward;
        uint256 unitAccumulativeCalculatingForce;
    }
    ...
    constructor(){
        init();
        _setupRole(PREDICTION, _msgSender());
    }
    ...
    function showAward(uint256 _tokenId) public view virtual returns (uint256, uint256){
        ...
        uint256 lastUnitReward = roundsDetails[last].unitAccumulativeCalculatingForce;
        uint256 calculateTheRounds = currentRounds - 1;
        uint256 unitReward = roundsDetails[calculateTheRounds].unitAccumulativeCalculatingForce;
        return ((unitReward - lastUnitReward) * power, calculateTheRounds);
    }

    //领取奖励
    function claimAward(uint256[] memory _tokenIds) public virtual {
    ...
    //预测合约注入资金 开启下一轮
    function capitalInjection() onlyRole(PREDICTION) public payable virtual{
        roundInfo storage info = roundsDetails[currentRounds];
        if(msg.value > 0){
            info.assetsNumber = info.assetsNumber + msg.value;
        }
        info.endBlock = block.number;
        info.unitReward = bonusPerUnitOfCalculatingPower(info.assetsNumber,info.sumPower);
        info.unitAccumulativeCalculatingForce = info.unitReward + roundsDetails[currentRounds-1].unitAccumulativeCalculatingForce;
        ...
        //如果上一轮总算力为0 所有资产自动加入到下一轮
        if(info.sumPower == 0){
            newInfo.assetsNumber = info.assetsNumber;
        }
