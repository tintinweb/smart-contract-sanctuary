        ...
        returns (string memory)
    {
        ...
        for (uint8 i = 0; i < 9; i++) {
            ...
            for (
                ...
                if (placedPixels[plotting[i][j].x][plotting[i][j].y]) continue;
                ...
                placedPixels[plotting[i][j].x][plotting[i][j].y] = true;
            }
        }
    ...
    **********************************/
        ...
        onlyOwner
    {
        for (uint256 i = 0; i < traits.length; i++) {
            for(uint16 j=0; j< traits[i].pixelCount; j++){
                string memory thisPixel = MaticMikeLibrary.substring(
                    traits[i].pixels,
                    j * 4,
                    j * 4 + 4
                ...
                uint8 x = letterToNumber(MaticMikeLibrary.substring(thisPixel, 0, 1));
                uint8 y = letterToNumber(MaticMikeLibrary.substring(thisPixel, 1, 2));
                plotting[_traitTypeIndex][j] = Rect(
                x,
                y,
                string(
                    abi.encodePacked(
