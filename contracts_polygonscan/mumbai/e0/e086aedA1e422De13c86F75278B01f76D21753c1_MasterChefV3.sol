...
contract MasterChefV3 is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 compoundAmount; // compound share out of accTokenPerShare
    }
    ...
    /**
        compound share tracks user compoundings with pool allocation points
        and helps to calculate compound token rewards
        user's compound share for one pool is;
        shareForPool = poolAllocPoint / totalallocpoint * tokenperblock * userShareInCompounding * deposit * 1e12
        allshares = sum(sharesOfPools)
        compound reward = allShares * totalCompoundRewards / 1e12 - compoundRewardDept
    */
    mapping(address => uint256) public compoundShare;
    mapping(address => uint256) public compoundRewardDept;
    // compound accumulated reward
    uint256 public totalCompoundRewards;
    ...
    function add(uint256 _allocPoint, IBEP20 _lpToken, uint16 _withdrawFeeBP, bool _withUpdate) public onlyOwner nonDuplicated(_lpToken) {
        ...
        if (address(_lpToken) == address(token)) {
            autoStakePID = poolInfo.length.sub(1);
        }
    }
    ...
    function pendingToken(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (user.compounding && _pid != autoStakePID) return 0;
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (_pid != autoStakePID && pool.compoundAmount > 0) {
            uint256 tokenSupply = token.balanceOf(address(this));
            uint256 compoundReward = tokenReward.mul(pool.compoundAmount).div(lpSupply);
            totalCompoundRewards = totalCompoundRewards.add(compoundReward);
        }
    }

    function normalizeCompound(address _user) public {
        UserInfo storage user = userInfo[autoStakePID][_user];
        uint256 compoundReward = totalCompoundRewards.mul(compoundShare[msg.sender]).div(1e12).sub(compoundRewardDept[msg.sender]);
        user.amount = user.amount.add(compoundReward);
        if (compoundReward > 0) {
            compoundRewardDept[msg.sender] = compoundRewardDept[msg.sender].add(compoundReward);
        }
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (_pid == autoStakePID) {
            normalizeCompound(msg.sender);
        }
        uint256 pending = user.amount.mul(pool.accTokenPerShare).div(1e12).sub(user.rewardDebt);

        if (pending > 0 && !user.compounding) {
        ...
        if (_amount > 0) {
            ...
            uint256 lpSupply = pool.lpToken.balanceOf(address(this));
            uint256 finalAmount = lpSupply.sub(balanceBefore);
            user.amount = user.amount.add(finalAmount);

            if (user.compounding && finalAmount > 0) {
                uint256 share = pool.allocPoint
                    .mul(tokenPerBlock)
                    .div(totalAllocPoint)
                    .mul(finalAmount.mul(1e12))
                    .div(lpSupply);

                compoundShare[msg.sender] = compoundShare[msg.sender].add(share);
                pool.compoundAmount = pool.compoundAmount.add(finalAmount);
            }
        }
    ...
    function compound(uint256 _pid) public nonReentrant {
        ...
        .mul(user.amount)
        .mul(1e12)
        .div(pool.lpToken.balanceOf(address(this)));
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        updatePool(_pid);
        // if user is compounding, reward is always 0
        uint256 pending = user.compounding && _pid != autoStakePID ? 0 : user.amount.mul(pool.accTokenPerShare).div(1e12).sub(user.rewardDebt);
        if (pending > 0) {
            safeTokenTransfer(msg.sender, pending);
        }
        if (_amount > 0) {
