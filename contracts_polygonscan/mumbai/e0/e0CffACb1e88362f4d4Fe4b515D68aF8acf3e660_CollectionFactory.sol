...
interface ICollectionFactory {
    ...
    function createCollectionWithAdmin( address creator, string calldata tokenName, string calldata tokenSymbol ) external;
}
...
interface ICollection {
    ...
    function updateMetaHashWithAdmin( uint256 tokenId, address creator, string calldata metaHash ) external;
}
...
contract Collection is Ownable, ERC721, ICollection {
    ...
    //-------------------------------------------------------------------
    // [external] 管理者によるミントは許可しない
    //-------------------------------------------------------------------
    function createTokenWithAdmin( address /*creator*/, string calldata /*metaHash*/ ) external pure override{
        require( false, "not allowed" );
    }

    //-------------------------------------------------------------------
    // [external] 管理者によるメタデータの更新は許可しない
    //-------------------------------------------------------------------
    function updateMetaHashWithAdmin( uint256 /*tokenId*/, address /*creator*/, string calldata /*metaHash*/ ) external pure override {
        require( false, "not allowed" );
    }
...
contract CollectionFactory is Ownable, ICollectionFactory, ICollectionConfig {
    ...
    //-----------------------------------------
    constructor() Ownable() {
        _signer_public_key = msg.sender;
        _sign_required = false;
        _sign_required_for_public = true;
    ...
    //-----------------------------------------------------------
    // [external/onlyOwner] コレクションの作成（管理者が代理作成する）
    //-----------------------------------------------------------
    function createCollectionWithAdmin( address creator, string calldata tokenName, string calldata tokenSymbol ) external override onlyOwner {
        uint256 collectionId = _collections.length + COLLECTION_ID_OFFSET;

        // コレクションの作成
        address contractAddress = address( new Collection( collectionId, ICollectionConfig(this), tokenName, tokenSymbol, creator ) );
        _collections.push( contractAddress );

        // event
        emit CollectionCreated( creator, tokenName, tokenSymbol, collectionId, contractAddress );
    }
