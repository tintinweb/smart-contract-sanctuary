...
contract FRTMarketplace is Ownable{
    ...
    mapping(address => mapping(uint256 => Refund)) public voteRefunds;
    mapping(uint256 => mapping(address => bool)) public testingAllowed;
    mapping(address => mapping(uint256=> uint256)) public rewardBalance;
    ...
    function refreshQueue(uint256 _tokenId) private{
        ...
        if(voteNFTQueue[_tokenId].up.length >= approvalVotes ){
            _approveNFT(_tokenId);

        }else if(voteNFTQueue[_tokenId].down.length > (maxVotes-approvalVotes)){
            _rejectNFT(_tokenId);
        }
    }
    ...
    function _approveNFT(uint256 _tokenId) private {
        ...
        for (uint256 i = 0; i < voteNFTQueue[_tokenId].up.length; ++i) {
            ...
            rewardBalance[voteNFTQueue[_tokenId].up[i]][_tokenId] += baseVoteReward;

        }

        for (uint256 i = 0; i < voteNFTQueue[_tokenId].down.length; ++i) {
            voteRefunds[voteNFTQueue[_tokenId].down[i]][_tokenId] = Refund(block.timestamp + 10 minutes, baseTestingPrice );
        }
    ...
    function _rejectNFT(uint256 _tokenId) private {
        ...
        for (uint256 i = 0; i < voteNFTQueue[_tokenId].down.length; ++i) {
            ...
            rewardBalance[voteNFTQueue[_tokenId].down[i]][_tokenId] += baseVoteReward;
        }
        // TODO: 
        
        for (uint256 i = 0; i < voteNFTQueue[_tokenId].up.length; ++i) {
            voteRefunds[voteNFTQueue[_tokenId].up[i]][_tokenId] = Refund(block.timestamp + 10 minutes, baseTestingPrice );
        }
    }
    ...
    function withdrawRewards(uint256 _tokenId) public {
        ...
        baseToken.transfer(msg.sender, rewardBalance[msg.sender][_tokenId]);
    ...
    function withdrawRefunds(uint256 _tokenId) public {
        ...
        baseToken.transfer(msg.sender, voteRefunds[msg.sender][_tokenId].amount);
