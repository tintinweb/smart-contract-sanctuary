...
contract HopImplL2 is ImplBase {
    ...
    ) external view override onlyRegistry returns (uint256) {
        address _amm = abi.decode(_data, (address));
        uint8 _idx = HopAMM(_amm).getTokenIndex(_token);
        return HopAMM(_amm).calculateSwap(_idx, _idx == 0 ? 0 : 1, _amount);
    }
}
