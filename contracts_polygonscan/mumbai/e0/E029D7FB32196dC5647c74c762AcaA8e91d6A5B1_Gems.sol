...
contract Gems is ERC1155, Ownable, VRFConsumerBase {
  ...
  function requestNewGems(uint256 amount, bool isMint) private {
    require(
      LINK.balanceOf(address(this)) >= fee,
      'NOT_ENOUGH_LINK'
    );
    bytes32 requestId = requestRandomness(keyHash, fee);
    _requestToData[requestId] = RequestData(msg.sender, amount, isMint);
  }

  function fulfillRandomness(bytes32 requestId, uint256 randomBase)
    internal
    override
  {
    uint256 amount = _requestToData[requestId].amount;
    bool isMint = _requestToData[requestId].isMint;

    for (uint256 i = 0; i < amount; i++) {
      uint256 level = 1;

      if (isMint) {
        level = Utils.getWeightedLevel(randomBase, i + 1);
      }

      uint256 luster = Utils.getWeightedLuster(randomBase, i + 3);
      uint256 color = Utils.getRandomColor(randomBase, i + 2);
  ...
  function mintMany(uint256 amount) public payable {
    ...
    require(amount <= limitPerTransaction, 'MINT_LIMIT');
  ...
  function salvage(uint256 tokenId, uint256 amount) public {
    require(craftingIsActive, 'CRAFTING_DISABLED');
    require(tokenId >= 10, 'WRONG_TOKEN_TYPE');

    uint256 multiplier = 1;

    if (_gemDetails[tokenId].color == 8) {
      multiplier = 8;
    }
  ...
  function createFromDust(uint256 amount) public {
    require(craftingIsActive, 'CRAFTING_DISABLED');
    _burn(msg.sender, DUST, createGemDustCost * amount);
    requestNewGems(amount, false);
  }

  function changeColor(uint256 tokenId, uint256 amount) public {
    require(craftingIsActive, 'CRAFTING_DISABLED');
    require(
      _gemDetails[tokenId].color != 8,
      'CANNOT_CHANGE_COLOR'
    );
    ...
    _burn(
      msg.sender,
      DUST,
      changeColorDustCost * _gemDetails[tokenId].level * amount
    );
    _burn(msg.sender, tokenId, amount);
    mintGem(newColor, _gemDetails[tokenId].level, _gemDetails[tokenId].luster, amount);
  }
  ...
  function upgradeLevel(
    ...
    uint256 nextLevel = _gemDetails[tokenId].level + 1;
    require(nextLevel <= LEVEL_MAX, 'MAX_LVL_REACHED');

    _burn(msg.sender, tokenId, amount * upgradeGemCost);
    mintGem(_gemDetails[tokenId].color, nextLevel, _gemDetails[tokenId].luster, amount);
  }

  function combineColors(uint256 level, uint256 luster, uint256 amount) public {
    // TODO: triple check this function's safety
    require(craftingIsActive, 'CRAFTING_DISABLED');

    uint256[] memory tokenIds = new uint256[](NUM_COLORS);
    uint256[] memory amounts = new uint256[](NUM_COLORS);

    for (uint256 i = 0; i < NUM_COLORS; i++) {
      tokenIds[i] = Utils.getTokenId(i+1, level, luster);
      amounts[i] = amount;
    }

    _burnBatch(msg.sender, tokenIds, amounts);
    mintGem(8, level, luster, amount);
  }

  modifier onlyAllowedBurners {
    require(allowedBurners[msg.sender]);
    _;
   }
  ...
  function setBurner(address burner, bool allowed) public onlyOwner {
    allowedBurners[burner] = allowed;
  }

  function setMinter(address burner, bool allowed) public onlyOwner {
    allowedMinters[burner] = allowed;
  }

  function externalBurn(address sender, uint256 tokenId, uint256 amount) public onlyAllowedBurners {
    _burn(sender, tokenId, amount);
  }

  function externalMint(address sender, uint256 tokenId, uint256 amount) public onlyAllowedMinters {
  ...
  function setRules(
    bool _saleIsActive,
    bool _craftingIsActive,
    uint256 _mintCap,
    uint256 _mintPrice,
    uint256 _limitPerTransaction,
    uint256 _createGemDustCost,
    uint256 _salvageToDustRatio,
    uint256 _changeColorDustCost,
    ...
    craftingIsActive = _craftingIsActive;
    createGemDustCost = _createGemDustCost;
    salvageToDustRatio = _salvageToDustRatio;
    changeColorDustCost = _changeColorDustCost;
  ...
  //   LINK.transfer(msg.sender, 10000);
  // }

  function uri(uint256 tokenId) public view override returns (string memory) {
    return GetMetadata.getMetadata(tokenId, _gemDetails[tokenId].color, _gemDetails[tokenId].level, _gemDetails[tokenId].luster);
  }
}
...
library Utils {
  uint256 private constant NUM_COLORS = 7;
  ...
  function getRandomColor(uint256 randomNumber, uint256 salt) internal pure returns (uint256) {
    uint256 colorBase = getRandomSafe(randomNumber, salt);
    return (colorBase % (NUM_COLORS - 1)) + 1;
  }

  function getWeightedLevel(uint256 randomNumber, uint256 salt) internal pure returns (uint256) {
  ...
  function getWeightedLuster(uint256 randomNumber, uint256 salt) internal pure returns (uint256) {
    uint256 luster = getRandomSafe(randomNumber, salt);

    if (luster > 50) { // FIXME: final setting
      luster = 1;
    } else {
      luster = 0;
    }
