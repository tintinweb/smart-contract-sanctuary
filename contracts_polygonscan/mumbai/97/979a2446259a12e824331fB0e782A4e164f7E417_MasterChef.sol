...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    // Info of each pool.
    struct PoolInfo {
        uint256 fixedmultiplier; //Fixed multiplier for each pool.
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, uint16 _depositFeeBP, uint256 _fixedmultiplier) external onlyOwner nonDuplicated(_lpToken) {
        require(_depositFeeBP <= 10000, "add: invalid deposit fee basis points");
        require(_fixedmultiplier >= 1, "add: multiplier should be at least 1");
        uint256 lastRewardBlock = block.number > startBlock ? block.number : startBlock;
        totalAllocPoint = totalAllocPoint.add(_allocPoint);
        poolExistence[_lpToken] = true;
        poolInfo.push(PoolInfo({
            fixedmultiplier: _fixedmultiplier,
            lpToken: _lpToken,
    ...
    function set(uint256 _pid, uint256 _allocPoint, uint16 _depositFeeBP, uint256 _fixedmultiplier) external onlyOwner {
        require(_fixedmultiplier >= 1, "add: multiplier should be at least 1");
        require(_depositFeeBP <= 10000, "set: invalid deposit fee basis points");
        totalAllocPoint = totalAllocPoint.sub(poolInfo[_pid].allocPoint).add(_allocPoint);
        poolInfo[_pid].allocPoint = _allocPoint;
        poolInfo[_pid].depositFeeBP = _depositFeeBP;
        poolInfo[_pid].fixedmultiplier = _fixedmultiplier;
    }
    ...
    function pendingIris(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 fixedmultiplier = pool.fixedmultiplier;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 irisReward = multiplier.mul(irisPerBlock).mul(fixedmultiplier).mul(pool.allocPoint).div(totalAllocPoint);
            accIrisPerShare = accIrisPerShare.add(irisReward.mul(1e18).div(lpSupply));
        }
        return user.amount.mul(accIrisPerShare).div(1e18);
    }
    ...
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accIrisPerShare).div(1e18);
            if (pending > 0) {
