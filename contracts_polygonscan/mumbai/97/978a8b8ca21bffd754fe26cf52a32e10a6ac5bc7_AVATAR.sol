    ...
    AdminRole
{
    ...
    uint256 public PRICE; // 0.06 ETH
    string public PREFIX_TOKEN_URI;
    string public SUFFIX_TOKEN_URI;
    bool public SALE;
    ...
    function initialize(
        ...
        MAX_ITEMS = _maxItems;
        PRICE = _price;
        PREFIX_TOKEN_URI = _prefixTokenUri;
        SUFFIX_TOKEN_URI = _suffixTokenUri;
        SALE = _sale;
    }

    /**
     * @notice Allows a Avatar admin to update  config variables.
     */
    function adminUpdateConfig(
        string memory prefixTokenUri,
        string memory suffixTokenUri,
        uint256 maxItems,
        uint256 _price
    ) external onlyAdmin {
        MAX_ITEMS = maxItems;
        PRICE = _price;
        PREFIX_TOKEN_URI = prefixTokenUri;
        SUFFIX_TOKEN_URI = suffixTokenUri;
    }

    /**
     * @dev Mints Reserve token by admin .
     * @param _count number of tokens to be minted by admin
     */
    function mintReserve(uint8 _count) external onlyAdmin {
    ...
    /**
     * @dev Mints tokens by user.
     * @param _count number of tokens to be minted by user
     */
    ...
    function _mintAnElement() private {
        ...
        _safeMint(address(msg.sender), id);
    ...
    /**
     * @notice Allows a Avatar admin to start or stop the sale.
     */
    function publicSale(bool value) external onlyAdmin {
        SALE = value;
    }

    /**
     * @notice Allows a Avatar admin to withdraw all the funds.
     */
    function withdrawAll() external payable onlyAdmin {
        require(address(this).balance > 0, "No funds left");
        _widthdraw(address(msg.sender), address(this).balance);
    }
