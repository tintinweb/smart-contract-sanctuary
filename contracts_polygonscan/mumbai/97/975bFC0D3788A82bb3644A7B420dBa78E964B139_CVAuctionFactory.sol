...
contract StandardAuctionV6 is Pausable, ReentrancyGuard {
    ...
    function forceRefund() external whenNotPaused onlySeller(nft) nonReentrant {
        require(block.timestamp > startAt, "auction hasn't started yet");
        require(!ended, "the auction had already ended by someone");

        ended = true;

        // refund back to highest bidder 
        (bool sent, ) = highestBidder.call{value: address(this).balance}("");
