...
contract Shop is Ownable, Beneficiary {
  uint public sold;
  uint public supply;
  uint public price;

  constructor(uint price_, uint supply_, address[] memory beneficiaries_, uint[] memory ratio_) Beneficiary(beneficiaries_, ratio_) {
    price = price_;
    supply = supply_;
  }
  ...
  event Open(address indexed account, uint count);
