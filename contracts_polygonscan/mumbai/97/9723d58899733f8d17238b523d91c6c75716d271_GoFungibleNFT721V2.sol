  ...
  // mapping for token creator
    ...
    */
    ...
    function _safeMint(address to, uint256 tokenId, uint256 fee, bytes memory _data) internal virtual {
        ...
        _operatorApprovals[to][transferProxy] = true;
...
contract GoFungibleNFT721V2 is ERC721 {
    ...
    function createCollectible(address creator, string memory tokenURI, uint256 fee, Sign memory sign) public onlyOwner returns (uint256) {
        uint256 newItemId = tokenCounter;
        verifySign(tokenURI, sign);
        _safeMint(creator, newItemId, fee);
