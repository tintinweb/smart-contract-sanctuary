...
contract GrailMarketplace is Ownable, ReentrancyGuard {
    ...
    function buyItem(
        ...
        if (IGrailNFT1155(_nftAddress).creators(_tokenId) != address(0) && royalties[_tokenId] != uint8(0)) {
            uint256 royaltyFee = msg.value.sub(feeAmount).mul(royalties[_tokenId]).div(100);
            (bool royaltyTransferSuccess, ) = payable(IGrailNFT1155(_nftAddress).creators(_tokenId)).call{ value: royaltyFee }("");
    ...
    function registerRoyalty(
        ...
        if (IERC165(_nftAddress).supportsInterface(INTERFACE_ID_ERC1155)) {
            require(IGrailNFT1155(_nftAddress).creators(_tokenId) == _msgSender(), "Not minter of this item.");
        } else {
            revert("Invalid NFT address.");
        }
