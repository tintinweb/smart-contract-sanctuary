...
contract FundamentaWrappedToken is SecureContract, ERC20
{
    ...
    function wrap(uint256 amount) public pause
    {
        ...
        require(_backingToken.balanceOf(msg.sender) >= amount + fee, "Wrapped Token: Insufficient backing token balance");

        _backingToken.safeTransferFrom(msg.sender, address(this), amount + fee);
    ...
    function crossChainWrap(address user, uint256 amount, uint256 teleportFee) public isBridge pause
    {
        if (_fmtaWrapFee > 0)
        {
        ...
        uint256 fee = teleportFee + calculateWrapFee(amount);
        require(_backingToken.balanceOf(user) >= amount + fee, "Wrapped Token: Insufficient backing token balance");
        
        _backingToken.safeTransferFrom(user, address(this), amount + fee);
    ...
    function crossChainUnwrap(address user, uint256 amount, uint256 teleportFee) public isBridge pause
    {
        if (_fmtaUnwrapFee > 0)
        {
        ...
        uint256 fee = teleportFee + calculateUnwrapFee(amount);
...
interface WrappedTokenInterface is TokenInterface {
    function crossChainWrap(address user, uint256 amount, uint256 fee) external;
    function crossChainUnwrap(address user, uint256 amount, uint256 fee) external;
}
