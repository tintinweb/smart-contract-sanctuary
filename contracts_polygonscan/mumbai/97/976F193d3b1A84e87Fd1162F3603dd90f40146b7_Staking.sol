...
contract Staking is AccessControl, ERC20 {
    ...
    uint256 public withdrawBufferTime = 7 days;
    ...
    constructor(address _admin, address _lpToken) ERC20('Staking Shares', 'SHRS') {
    ...
    function depositLpToken(address _to, uint256 _amount) public {
        require(_to != address(0), 'Address add lp tokens this address 0');
    ...
    function claimPlasma(address _to) public {
        Deposit memory _d = lastDeposit[msg.sender];
        _mintForExistingWeight(_d, msg.sender);

        uint256 _currentBalance = balanceOf(msg.sender);
        uint256 possiblePlasmaPosssible = possiblePlasmaPointsReleasedTillNow();
        uint256 plasmaToGenerate = possiblePlasmaPosssible.mul(_currentBalance).div(totalSupply());
        require(possiblePlasmaPosssible >= plasmaClaimedTillNow.add(plasmaToGenerate), "Can't mint more than what curve permits");
    ...
    function increaseLinear(
        uint256 _amount,
        uint256 _timestamp,
        uint256 _startTime,
        uint256 _month
    ) internal pure returns (uint256) {
        return _amount.mul(_timestamp.sub(_startTime).sub(_month.mul(30 days))).div(30 days);
    }

    function possiblePlasmaPointsReleasedTillNow() public view returns (uint256) {
        uint256 _ts = block.timestamp;
        uint256 _month = (_ts.sub(startTime)).div(30 days);
        if (_month == 0) {
            return increaseLinear(200000 * (10**18), _ts, startTime, _month);
        } else if (_month == 1) {
            return increaseLinear(150000 * (10**18), _ts, startTime, _month).add(200000 * (10**18));
        } else if (_month == 2) {
            return increaseLinear(100000 * (10**18), _ts, startTime, _month).add(350000 * (10**18));
        } else if (_month == 3) {
            return increaseLinear(50000 * (10**18), _ts, startTime, _month).add(450000 * (10**18));
        } else if (_month == 4) {
            return increaseLinear(25000 * (10**18), _ts, startTime, _month).add(500000 * (10**18));
        } else if (_month == 5) {
            return increaseLinear(1000000 * (10**18), _ts, startTime, _month).add(525000 * (10**18));
        } else if (_month == 6) {
            return increaseLinear(1000000 * (10**18), _ts, startTime, _month).add(1525000 * (10**18));
        } else if (_month == 7) {
            return increaseLinear(1000000 * (10**18), _ts, startTime, _month).add(2525000 * (10**18));
        } else if (_month == 8) {
            return increaseLinear(500000 * (10**18), _ts, startTime, _month).add(3525000 * (10**18));
        } else if (_month == 9) {
            return increaseLinear(500000 * (10**18), _ts, startTime, _month).add(4025000 * (10**18));
        } else if (_month == 10) {
            return increaseLinear(500000 * (10**18), _ts, startTime, _month).add(4525000 * (10**18));
        } else if (_month == 11) {
            return increaseLinear(250000 * (10**18), _ts, startTime, _month).add(5025000 * (10**18));
        } else if (_month == 12) {
            return increaseLinear(250000 * (10**18), _ts, startTime, _month).add(5275000 * (10**18));
        } else if (_month == 13) {
            return increaseLinear(250000 * (10**18), _ts, startTime, _month).add(5525000 * (10**18));
        } else if (_month == 14) {
            return increaseLinear(135000 * (10**18), _ts, startTime, _month).add(5775000 * (10**18));
        } else if (_month == 15) {
            return increaseLinear(135000 * (10**18), _ts, startTime, _month).add(5910000 * (10**18));
        } else if (_month == 16) {
            return increaseLinear(135000 * (10**18), _ts, startTime, _month).add(6045000 * (10**18));
        } else if (_month == 17) {
            return increaseLinear(50000 * (10**18), _ts, startTime, _month).add(6180000 * (10**18));
        } else if (_month == 18) {
            return increaseLinear(50000 * (10**18), _ts, startTime, _month).add(6230000 * (10**18));
        } else if (_month == 19) {
            return increaseLinear(50000 * (10**18), _ts, startTime, _month).add(6280000 * (10**18));
        } else if (_month == 20) {
            return increaseLinear(10000 * (10**18), _ts, startTime, _month).add(6330000 * (10**18));
        } else {
            return 6340000 * (10**18);
        }
    }
    ...
    modifier onlyIfAmountIsfLeft(uint256 _amount) {
        ...
        require(_amountRemaining != 0, 'No remaining left to withdraw');
        _;
    }
}
