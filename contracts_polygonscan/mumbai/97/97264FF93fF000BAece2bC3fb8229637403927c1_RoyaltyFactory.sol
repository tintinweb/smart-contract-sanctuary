...
contract RoyaltyFactory {
    ...
    event LoyaltyERC721Created(string name, string symbol, uint256);

    constructor() {
        royaltyCounter = 0;
    }

    function createRoyaltyERC721(string memory name, string memory symbol, string calldata uri, address royaltyAddress, uint16 bps)
        external
        returns (address loyalty, uint256 counter)
    {
        ...
        emit LoyaltyERC721Created(name, symbol, allRoyalties.length);

        _loyalty.mintWithRoyalty(msg.sender,uri,royaltyAddress,bps);
