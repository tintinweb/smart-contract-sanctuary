...
// File: contracts/BlindBoxPolygon.sol
...
using SafeMath for uint112;
 ...
 function init1() public {
    ...
    if(boxDetails[name].initialize == 0){
      ...
      for(uint256 i = 0; i < 4; i++){
        for (uint256 j = 0; j < 5; j++) {
          ...
          uint256 randNumber =  (uint256(keccak256(abi.encode(randomValueMain, (block.timestamp + counter++ )))) % 50);
  ...
  function buyBox(address seller, string memory ownerId, string memory name, uint256 selectedTime, bool isPaid) public {
    ...
    for(uint256 i =0; i<Category[name].countNFT; i++)
    {
     ...
     tokenId = IERC20(xanaliaDex).blindBox(seller, Category[name].tokenUris[randNumber], true, msg.sender, ownerId);
 ...
 function timeTester() internal {
  ...
  function getCountByTime(uint256 selectedTime, string memory name) public view returns(uint256) {
    uint256 dayCount = (selectedTime > 0 ? selectedTime : block.timestamp  - boxDetails[name].initialize)/(60 * 30);
    return boxDetails[name].usedCountByday[dayCount].count;
  }
  function isBoxSold(uint256 selectedTime, string memory name) public view returns(bool) {
    uint256 dayCount = (selectedTime > 0 ? selectedTime : block.timestamp  - boxDetails[name].initialize)/(60 * 30);
    return Category[name].count == 0 || boxDetails[name].usedCountByday[dayCount].isSold || boxDetails[name].initialize >= block.timestamp || dayCount >= Category[name].count / boxDetails[name].dailiyLimit ;
  }
