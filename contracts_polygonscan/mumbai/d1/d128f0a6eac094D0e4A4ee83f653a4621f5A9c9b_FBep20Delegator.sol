...
// File: contracts/FBep20Delegator.sol

pragma solidity ^0.5.16;


/**
 * @title Fortress's FBep20Delegator Contract
 * @notice FTokens which wrap an EIP-20 underlying and delegate to an implementation
 * @author Fortress
 */
contract FBep20Delegator is FTokenInterface, FBep20Interface, FDelegatorInterface {
    /**
     ...
     */
    function _setImplementation(address implementation_, bool allowResign, bytes memory becomeImplementationData) public {
        require(msg.sender == admin, "FBep20Delegator::_setImplementation: Caller must be admin");
     ...
     */
    function liquidateBorrow(address borrower, uint repayAmount, FTokenInterface fTokenCollateral) external returns (uint) {
        borrower; repayAmount; fTokenCollateral; // Shh
        delegateAndReturn();
    }
   ...
   /**
    ...
    function () external payable {
        require(msg.value == 0,"FBep20Delegator:fallback: cannot send value to fallback");
