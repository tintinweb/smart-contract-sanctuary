...
contract DockSpeedSprintSession is Ownable{
    ...
    function finishSession(uint256 _sessionId,uint32 _finishSession)public{
        ...
        require(playerIndexInASession[_sessionId][msg.sender] != 0,"Join a session to finsish a session");
    ...
    function FinishASession(uint256 _sessionID)public onlyOwner{
        uint32[] memory raceTimeArray = SessionDictionary[_sessionID].raceTime;
        uint32 temp;
        uint32 playerIndex;
        for(uint32 i = 0;i < raceTimeArray.length;i++){
            if(i == 0){
                temp = raceTimeArray[i];
                playerIndex = i;
            }
            else{
                if(temp > raceTimeArray[i]){
                    temp = raceTimeArray[i];
                    playerIndex = i;
                }
            }
