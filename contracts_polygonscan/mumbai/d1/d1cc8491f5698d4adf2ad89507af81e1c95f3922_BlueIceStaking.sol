...
/*
    BlueIceStaking is a for from mHopeStakingPool contract of Firebird Finance
*/
contract BlueIceStaking is Initializable, Ownable {
    ...
    modifier lock() {
        require(_locked == 0, "BlueIceStaking: LOCKED");
        _locked = 1;
        _;
        _locked = 0;
    }
    ...
    function initialize(
        ...
        require(block.timestamp < _startRewardTime, "late");
    ...
    function getRewardPerSecond() external view returns (uint256) {
        return getRewardPerSecond(block.timestamp, block.timestamp + 1);
    }
    ...
    function allocateMoreRewards(uint256 _addedReward, uint256 _days) external onlyReserveFund {
        uint256 _pendingSeconds = (endRewardTime >  block.timestamp) ? endRewardTime.sub( block.timestamp) : 0;
        ...
        if (_days > 0) {
            if (endRewardTime <  block.timestamp) {
                endRewardTime =  block.timestamp.add(_days.mul(1 days));
            } else {
