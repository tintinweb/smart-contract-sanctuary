...
contract RaidsPoly {
    ...
    function _claim(uint256 id) internal {
        ...
        if (cmp.reward > 0 && _ended(campaigns[id])) {
            ...
            if (cmp.seed != 0) {
                ...
                reward = _getReward(raid, id, level, cmp.seed, "RAID") + (cmp.double ? _getReward(raid, id, level, cmp.seed, "DOUBLE RAID") : 0);
            } 
    ...
    function _ended(Campaign memory cmp) internal view returns(bool) {
        ...
        return cmp.end == 0 || block.timestamp > (giantCrabHealth == 0 ? cmp.end - (cmp.double ? 2 days : 1 days) : cmp.end) && rdn != 0;
    }
