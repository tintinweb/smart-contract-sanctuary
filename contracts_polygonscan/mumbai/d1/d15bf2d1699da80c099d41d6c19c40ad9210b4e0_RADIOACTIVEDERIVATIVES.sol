...
// File contracts/RADIOACTIVEDERIVATIVES.sol
...
These punks survived to make derivative artwork!
...
@title  RADIOACTIVEDERIVATIVES

@dev    Extends ERC1155 Token Standard to allow pausing, burning,
        ...
        NFTs were to survive nuclear fallout, these would be the ones. Tokens
        are minted and airdropped directly into a list of community wallets.
*/
...
contract RADIOACTIVEDERIVATIVES is ERC1155PresetMinterPauser, ERC2981, Ownable {
  using Strings for uint256;

  address payable public PAYMENT_SPLITTER = payable(0xFc9961d08Ef8e04DB145b9fb3d48cf8DbA96116D ); // PaymentSplitter
  // Mapping from token ID to metadata URLs
  mapping(uint256 => string) private metadataURIs;

  // the URL here is meaningless and unused
  constructor() ERC1155PresetMinterPauser("https://radioactivepunks.xyz") {}

  // withdraw any funds to splitter
  function withdraw() public onlyOwner {
    uint256 balance = address(this).balance;
    PAYMENT_SPLITTER.transfer(balance);
  }

  // allow splitter to update
  function setPaymentSplitter(address payable newAddress) external onlyOwner {
    PAYMENT_SPLITTER = newAddress;
  }

  /*
   * Airdrop to a set of addresses using a specific token ID + metadata URL pair
   * this should both mint the tokens to the addr and mark the new metadataURL
   */
  function airdrop (address[] memory receivers, string memory metadataURL, uint256 tokenId) external onlyOwner {
    metadataURIs[tokenId] = metadataURL;
  ...
  /*
   * ERC2981 Royalties Standard for ERC1155
   */
  ...
  function supportsInterface(bytes4 interfaceId) public view override(ERC1155PresetMinterPauser, ERC2981) returns (bool) {
    return super.supportsInterface(interfaceId);
  }

  /**
   * @dev Removing the token ERC1155 substituion and replacing it with a system
   *      where it indexes into a map of metadataURLs for each token, set by
   *      the airdropper upon airdrop time.
   */
  function uri(uint256 token) public view virtual override returns (string memory) {
      string memory baseURI = super.uri(token);
      return bytes(baseURI).length > 0 ? metadataURIs[token] : "";
  }
}