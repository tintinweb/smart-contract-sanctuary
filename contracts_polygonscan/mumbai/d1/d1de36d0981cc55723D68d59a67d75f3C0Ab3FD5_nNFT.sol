...
contract nNFT is IERC721, Ownable {
    ...
    // You can use this hash to verify the image file containing all the peckers
...
string memory _nftName,
    ...
    // // A record of peckers that are offered for sale at a specific minimum value, and perhaps to a specific person
    // mapping(uint256 => Offer) public peckersOfferedForSale;

    // // A record of the highest pecker bid
    // mapping(uint256 => Bid) public peckerBids;

    // mapping(address => uint256) public pendingWithdrawals;

    // modifier onlyTokenOwner(uint256 _tokenId) {
    //     require(_tokenId < 10000, "pecker number is wrong");
    //     require(ownerOf(_tokenId) == msg.sender, "Incorrect owner.");
    //     _;
    // }

    // function peckerNoLongerForSale(uint256 peckerIndex)
    //     public
    //     reentrancyGuard
    //     onlyTokenOwner(peckerIndex)
    // {
    //     _peckerNoLongerForSale(peckerIndex);
    // }

    // function _peckerNoLongerForSale(uint256 peckerIndex) private {
    //     peckersOfferedForSale[peckerIndex] = Offer(
    //         false,
    //         peckerIndex,
    //         msg.sender,
    //         0,
    //         address(0)
    //     );
    //     emit PeckerNoLongerForSale(peckerIndex);
    // }

    // function offerPeckerForSale(uint256 peckerIndex, uint256 minSalePriceInWei)
    //     public
    //     reentrancyGuard
    //     onlyTokenOwner(peckerIndex)
    // {
    ...
    //         address(0)
    //     );
    //     emit PeckerOffered(peckerIndex, minSalePriceInWei, address(0));
    // }

    // function offerPeckerForSaleToAddress(
    //     uint256 peckerIndex,
    //     uint256 minSalePriceInWei,
    //     address toAddress
    // ) public reentrancyGuard onlyTokenOwner(peckerIndex) {
    //     require(marketPaused == false, "Market Paused");
    //     peckersOfferedForSale[peckerIndex] = Offer(
    //         true,
    //         peckerIndex,
    ...
    //         toAddress
    //     );
    //     emit PeckerOffered(peckerIndex, minSalePriceInWei, toAddress);
    // }

    // function buyPecker(uint256 peckerIndex) public payable reentrancyGuard {
    //     require(marketPaused == false, "Market Paused");
    //     require(peckerIndex < 10000, "pecker # is wrong");
    //     Offer memory offer = peckersOfferedForSale[peckerIndex];
    //     require(offer.isForSale, "pecker not for sale");
    //     require(
    //         offer.onlySellTo == address(0) || offer.onlySellTo == msg.sender,
    //         "pecker not of this user"
    //     );
    //     require(msg.value >= offer.minValue, "amount not enough");
    //     require(
    //         ownerOf(peckerIndex) == offer.seller,
    //         "Seller no longer owner of pecker"
    //     );

    //     address seller = offer.seller;

    //     _safeTransfer(seller, msg.sender, peckerIndex, "");
    //     _peckerNoLongerForSale(peckerIndex);
    //     pendingWithdrawals[seller] += msg.value;
    //     emit PeckerBought(peckerIndex, msg.value, seller, msg.sender);

    //     // Check for the case where there is a bid from the new owner and refund it.
    //     // Any other bid can stay in place.
    //     Bid memory bid = peckerBids[peckerIndex];
    //     if (bid.bidder == msg.sender) {
    //         // Kill bid and refund value
    //         pendingWithdrawals[msg.sender] += bid.value;
    //         peckerBids[peckerIndex] = Bid(false, peckerIndex, address(0), 0);
    //     }
    // }

    // function withdraw() public payable reentrancyGuard {
    ...
    // function enterBidForPecker(uint256 peckerIndex)
    //     public
    //     payable
    //     reentrancyGuard
    // {
    //     require(marketPaused == false, "Market Paused");
    //     require(peckerIndex < 10000, "pecker number is wrong");
    //     require(
    //         ownerOf(peckerIndex) != msg.sender,
    //         "you can not bid on your pecker"
    //     );
    //     require(msg.value > 0, "bid can not be zero");
    //     Bid memory existing = peckerBids[peckerIndex];
    //     require(
    ...
    //     }
    //     peckerBids[peckerIndex] = Bid(true, peckerIndex, msg.sender, msg.value);
    //     emit PeckerBidEntered(peckerIndex, msg.value, msg.sender);
    // }

    // function acceptBidForPecker(uint256 peckerIndex, uint256 minPrice)
    //     public
    //     reentrancyGuard
    //     onlyTokenOwner(peckerIndex)
    // {
    ...
    //     Bid memory bid = peckerBids[peckerIndex];
    //     require(bid.value > 0, "there is not a bid");
    //     require(bid.value >= minPrice, "bid is lower than min price");

    //     _peckerNoLongerForSale(peckerIndex);
    //     _safeTransfer(seller, bid.bidder, peckerIndex, "");

    //     uint256 amount = bid.value;
    //     peckerBids[peckerIndex] = Bid(false, peckerIndex, address(0), 0);
    //     pendingWithdrawals[seller] += amount;
    //     emit PeckerBought(peckerIndex, bid.value, seller, bid.bidder);
    // }

    // function withdrawBidForPecker(uint256 peckerIndex) public reentrancyGuard {
    //     require(peckerIndex < 10000, "pecker number is wrong");
    //     require(ownerOf(peckerIndex) != msg.sender, "wrong action");
    //     require(
    //         peckerBids[peckerIndex].bidder == msg.sender,
    //         "Only bidder can withdraw"
    //     );
    //     Bid memory bid = peckerBids[peckerIndex];
    //     emit PeckerBidWithdrawn(peckerIndex, bid.value, msg.sender);
    //     uint256 amount = bid.value;
    //     peckerBids[peckerIndex] = Bid(false, peckerIndex, address(0), 0);
