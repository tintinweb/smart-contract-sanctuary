...
contract Summoning is Ownable, VRFConsumerBase {
    ...
    function withdrawMagic() external onlyOwner {
        magic.safeTransfer(msg.sender, magic.balanceOf(address(this)));
    }

    function summon(uint256 orbID) external {
        require(orb.balanceOf(msg.sender, orbID) > 0, "not owning the orb");

        magic.safeTransferFrom(msg.sender, address(this), summoningAmount);
    ...
    function _summon(bytes32 requestID, uint256 randomness) internal {
        ...
        (uint256 orbType, , , , , , , uint16 generation) = orb.orbs(
            request.orbID
        );

        uint16 rarity = _getRarity(request.orbID, randomness);
    ...
    function upgradeItem(uint256[] memory ids) external {
        ...
        item.mint(msg.sender, generation, rarity + 1, class, magicSource);
    }
    ...
    function _getRarity(uint256 orbID, uint256 randomness)
        internal
        view
        returns (uint16)
    {
...
interface IOrb {
     ...
     */
    function orbs(uint256 orbId) external view returns (uint256, string memory, uint16, uint16, uint16, uint16, uint16, uint16);
