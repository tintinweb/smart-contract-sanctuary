...
contract NFTAuction is Ownable, ReentrancyGuard {
    ...
    function _isSaleOngoing(address _nftContractAddress, uint256 _tokenId)
        ...
        returns (bool)
    {
        uint256 saleEndTimestamp = nftContractSale[_nftContractAddress][_tokenId].saleEnd;
        return (saleEndTimestamp == 0 ||
        ...
        internal
    {
        ...
        nftContractSale[_nftContractAddress][_tokenId].nftSeller = msg
            .sender;    
        nftContractSale[_nftContractAddress][_tokenId].salePeriod = _salePeriod;
        nftContractSale[_nftContractAddress][_tokenId].saleEnd = _salePeriod.add(block.timestamp);
    }
    ...
    {
        require(
            nftContractSale[_nftContractAddress][_tokenId].saleEnd >0, 
            "Sale Does not exist"
        );
        uint256 buyNowPrice= nftContractSale[_nftContractAddress][_tokenId].buyNowPrice;
        // require(buyNowPrice > 0, Sale does not exist);
        require(msg.value >= buyNowPrice, 
    ...
    {   
        ...
        payable(_royaltyOwner).transfer(royaltyAmount);
        address owner= owner();
