...
contract NFTSale is MultiWhitelist {
    ...
    mapping(NFTType => uint256) private _typeCounter;
    mapping(NFTType => uint256) private _typeMax;
    uint256 legendaryCounter = 401;
    uint256 legendaryMax = 1200;
    uint256 mythicCounter = 1;
    uint256 mythicMax = 400;
    ...
    constructor(
        ...
        // RARE 2401-4096
        _typeCounter[NFTType.RARE] = 2401;
        _typeMax[NFTType.RARE] = 4096;
        // EPIC 1201-2400
        _typeCounter[NFTType.EPIC] = 1201;
        _typeMax[NFTType.EPIC] = 2400;
        // LEGENDARY 401-1200
        _typeCounter[NFTType.LEGENDARY] = 401;
        _typeMax[NFTType.LEGENDARY] = 1200;
        // MYTHIC 1-100
        _typeCounter[NFTType.MYTHIC] = 1;
        _typeMax[NFTType.MYTHIC] = 400;
        ...
        onlyWhitelisted
    {
        ...
        require(_typeCounter[nftType] <= _typeMax[nftType], "Sold out");
        address buyer = _msgSender();
        string memory URI = _placeholderURIs[nftType];
        uint256 nftId = _typeCounter[nftType];

        TOKEN.transferFrom(buyer, address(this), price);
        NFT.mint(msg.sender, nftId, URI);

        _typeCounter[nftType] = _typeCounter[nftType].add(1);

        _userPurchaseCounter[msg.sender] = _userPurchaseCounter[msg.sender].add(
            1
        );
...
abstract contract MultiWhitelist is AccessProtected {
    ...
    mapping(address => uint256) internal _userPurchaseCounter;
    ...
    modifier onlyLimited() {
        require(
            _userPurchaseCounter[_msgSender()] < purchaseLimit,
