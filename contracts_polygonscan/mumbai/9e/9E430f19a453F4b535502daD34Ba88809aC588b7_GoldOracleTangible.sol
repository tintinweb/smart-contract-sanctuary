...
contract GoldOracleTangible is AdminAccess, IPriceOracle {
    ...
    struct GoldBar{
        uint256 grams;
        uint256 premiumPrice; //in USD: 30000 -> 300 USD;  50 -> 0.5 USD
    }
    ...
    function latestAnswer(ITangibleNFT nft) external view override returns(uint256){
        ...
        uint256 result = ((convertToOracleDecimals(goldBars[nft.name()].grams, 0) * uint256(price)) / convertToOracleDecimals(unz, uint8(7))) 
                + convertToOracleDecimals(goldBars[nft.name()].premiumPrice, 2);
        return  result;
    }
    ...
    function convertToOracleDecimals(uint256 price, uint8 priceDecimals) internal view returns (uint256) {
    ...
    function addGoldBar(ITangibleNFT nft, uint256 grams, uint256 premiumPrice) external onlyAdmin() {
        ...
        if(goldBars[nft.name()].grams > 0){
            ...
            goldBars[nft.name()].premiumPrice = premiumPrice;
        } else {
            //we add new
        ...
        emit GoldBarAdded(nft.name(), grams, premiumPrice);
