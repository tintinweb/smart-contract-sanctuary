...
contract MetaMataPassport is ERC721Enumerable, ReentrancyGuard, Ownable, AccessControl {
  ...
  uint256 private l1Price = 0.1 * 10**18;
  uint256 private l2Price = 0.2 * 10**18;
  uint256 private l3Price = 0.3 * 10**18;
  ...
  uint256 public giveawaySupply = 2000;
  ...
  modifier mintNotPaused() {
    require(!mintPaused, "MetaMata Passport: Mint event is paused");
    _;
  }
  ...
  event L1PriceUpdated(address account, uint256 price);

  event L2PriceUpdated(address account, uint256 price);

  event L3PriceUpdated(address account, uint256 price);

  event MintPaused(address account);

  event MintUnpaused(address account);

  event GiveawaySupplyUpdated(address account, uint256 supply);
  ...
  constructor() ERC721("MetaMata Passport", "MATAPASS") {
  ...
  function smartMint(address _addr, uint256 _tokenId, uint256 _price) private {
    ...
    require(times < 10, "MetaMata Passport: sender has already already minted");
  ...
  function setBaseURI(string memory _baseURI) public onlyRole(META_ADMIN_ROLE) {
    baseURI = _baseURI;
  }
  ...
  function withdraw() public onlyRole(META_ADMIN_ROLE) {
    uint256 balance = address(this).balance;

    payable(_msgSender()).transfer(balance);
  }

  function setL1Price(uint256 _price) public onlyRole(META_ADMIN_ROLE) {
    l1Price = _price;

    emit L1PriceUpdated(_msgSender(), _price);
  }

  function setL2Price(uint256 _price) external onlyRole(META_ADMIN_ROLE) {
    l2Price = _price;

    emit L2PriceUpdated(_msgSender(), _price);
  }
  ...
  function setGiveawaySupply(uint256 _supply) external onlyRole(META_ADMIN_ROLE) {
    giveawaySupply = _supply;

    emit GiveawaySupplyUpdated(_msgSender(), _supply);
  }

  function giveAway(address _to, uint256 tokenId) external onlyRole(META_ADMIN_ROLE) {
    require(giveawaySupply > 0, "MetaMata Passport: Exceeds max passport giveaway supply" );
    giveawaySupply -= 1;
    _safeMint(_to, tokenId);
  }

  function pauseMint() public onlyRole(META_ADMIN_ROLE) {
    mintPaused = true;
    emit MintPaused(_msgSender());
  }

  function unpauseMint() public onlyRole(META_ADMIN_ROLE) {
    mintPaused = false;
