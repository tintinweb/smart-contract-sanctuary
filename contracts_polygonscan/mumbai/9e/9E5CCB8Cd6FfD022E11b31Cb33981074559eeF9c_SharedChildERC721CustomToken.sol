...
// File: contracts/child/SharedChildERC721CustomToken.sol
 ...
 */
contract SharedChildERC721CustomToken is
    BaseRelayRecipient,
    Context,
    EIP712Base,
    AccessControlMixin,
    ...
    IChildToken
{
    ...
    function setTrustedForwarder(address forwarder) public {
        require(
            ...
            "ERC721CustomToken: must have the maintainer role to update dependencies"
        );
        trustedForwarder = forwarder;
    }

    function setVersionRecipient(string memory recipient) public {
        require(
            hasRole(MAINTAINER_ROLE, _msgSender()),
            "ERC721CustomToken: must have the maintainer role to update dependencies"
        );
        _versionRecipient = recipient;
    }
    ...
    constructor(
        ...
        _setupContractId("ChildERC721CustomToken"); /** AccessControlMixin required here... **/

        _setupRole(DEPOSITOR_ROLE, childChainManager);

        _initializeEIP712(name, "1");

        maxSupply = _maxSupply;
    }

    function getMaxSupply() public view returns (int256) {
        return maxSupply;
    }
    ...
    function mint(address to) public {
        ...
        require(
            int256(tokenId) < maxSupply || maxSupply < 0,
            "ERC721CustomToken: The token is at maximum supply"
        );
        require(
    ...
    function pause() public virtual {
        require(
            ...
            "ERC721CustomToken: must have pauser role to pause"
        );
        _pause();
    }
    ...
    function unpause() public virtual {
        require(
            hasRole(PAUSER_ROLE, _msgSender()),
            "ERC721CustomToken: must have pauser role to unpause"
        );
        _unpause();
    }
    ...
    function grantMinterRole(address minter) public {
        require(
            hasRole(MAINTAINER_ROLE, _msgSender()),
            "ERC721CustomToken: must have the maintainer role to grant the minter role"
        );
