...
contract CHATTERPOLY is ERC721, ERC721Pausable, ERC721Enumerable, ERC721URIStorage, IChildToken, AccessControlMixin, EIP712Base, ContextMixin {
    ...
    constructor(uint256 _royaltyFee) ERC721("CHATTERPOLY", "CHP") {
        _setupContractId("CHATTERPOLY");
        _setupRole(DEPOSITOR_ROLE, address(0xb5505a6d998549090530911180f38aC5130101c6)); 
         _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _initializeEIP712("CHATTERPOLY");
    ...
    **/
    ...
    function withdraw(uint256 tokenId) external {
        require(_msgSender() == ownerOf(tokenId), "CHATTERPOLY: INVALID_TOKEN_OWNER");
    ...
    function withdrawBatch(uint256[] calldata tokenIds) external {
        ...
        uint256 length = tokenIds.length;
        require(length <= BATCH_LIMIT, "CHATTERPOLY: EXCEEDS_BATCH_LIMIT");
        ...
        for (uint256 i; i < length; i++) {
            ...
            require(_msgSender() == ownerOf(tokenId), string(abi.encodePacked("CHATTERPOLY: INVALID_TOKEN_OWNER ", tokenId)));
            withdrawnTokens[tokenId] = true;
            _burn(tokenId);
    ...
    function Mint(uint256 tokenId, string calldata uri) public only(DEFAULT_ADMIN_ROLE) {
        require(!withdrawnTokens[tokenId], "CHATTERPOLY: TOKEN_EXISTS_ON_ROOT_CHAIN");
