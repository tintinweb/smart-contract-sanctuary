...
contract BAFNFT721 is ERC721, VRFConsumerBase {
    using SafeMath for uint256;
    ...
    mapping (uint256 => slot) validSlot;
    ...
    constructor (string memory name, string memory symbol, string memory tokenURIPrefix) ERC721 (name, symbol) VRFConsumerBase( 0x8C7382F9D8f56b33781fE506E897a4F1e2d17255, 0x326C977E6efc84E512bB9C30f76E30c160eD06FB )  {
    ...
    function createCollectible( uint256 fee, Sign memory sign) public returns (uint256) {
        uint256 newItemId = tokenCounter;
        string[] memory attri;
        for(uint256 i = 0; i <= 7 ; i++) {
            getRandomNumber();
            attri[i] = convertToString(random(5));
            
        }
       string memory tokenURI = appendAttributes(attri);
    ...
    function getRandomNumber() public returns (bytes32 requestId) {
        bytes32 val = requestRandomness(keyHash, linkFee);
        return val;
    }
    ...
    function random (uint256 attribute) public view returns(uint256) {
        uint256 r = randomResult.mod(attribute);
        return r;
    }
    ...
    function appendAttributes( string[] memory value) internal pure returns (string memory) {
    bytes memory firstHalf = abi.encodePacked( value[0], value[1], value[2], value[3]);
    bytes memory secondHalf = abi.encodePacked( value[4], value[5], value[6], value[7]);
