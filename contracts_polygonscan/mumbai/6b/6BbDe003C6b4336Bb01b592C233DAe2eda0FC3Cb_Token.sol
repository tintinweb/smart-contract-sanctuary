 ...
 * @title IToken
 * @author softtech
 * @notice The concrete class for the Is marketplace `NFT` token.
*/
contract Token is
    ERC721Enhanced,
    ReentrancyGuard,
    ...
    Governable
{
    ...
    ***************************************/
     ...
     * @param _registry The address of the [`Registry`](./Registry) contract.
    */
    constructor(address _governance, address _registry) ERC721Enhanced("Is NFT", "IST") Governable(_governance){
    ...
    ***************************************/
    ...
    /**
     * @notice Creates new token and returns the token id.
     * @param to The token owner to mint.
     * @param tokenUri The URI link.
     * @return tokenId The id of the token.
    */
    function create(address to, string memory tokenUri) external override whileNotPaused nonReentrant returns (uint256 tokenId) {
        require(to != address(0x0), "zero address owner");
        require(bytes(tokenUri).length > 0, "invalid token uri");
        _tokenIds.increment();
        tokenId = _tokenIds.current();
        _mint(to, tokenId);
        _setTokenURI(tokenId, tokenUri);
        _approve(registry.getMarketplaceManager(), tokenId);
        emit TokenCreated(to, tokenId);
        return tokenId;
    }
    ...
    */
    function burn(uint256 tokenId) external override tokenMustExist(tokenId) {
 ...
 * @dev The base is OpenZeppelin's `ERC721Enumerable` and `ERC721URIStorage` which also includes the `Metadata` extension. This extension includes simpler transfers, gasless  approvals, and better enumeration.
 */
abstract contract ERC721Enhanced is ERC721Enumerable, ERC721URIStorage, IERC721Enhanced, EIP712 {
    ...
    ***************************************/
    ...
    /// @dev Call will revert if the token does not exist.
 ...
 * @title IToken
 * @author softtech
 * @notice Interface class for the Is marketplace `NFT` token.
*/
interface IToken {
