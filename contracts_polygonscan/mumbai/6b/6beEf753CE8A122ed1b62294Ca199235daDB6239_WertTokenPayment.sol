...
contract WertTokenPayment is ReentrancyGuard {
    ...
    function buyToken(
      ...
      address takerAddress
        ...
        bool isValidOrderSignature = IExchange(exchangeAddress).isValidOrderSignature(order, signature);
        // Validate order signature
        require(isValidOrderSignature, "The order signature is not valid");
        LibOrder.OrderInfo memory orderInfo = IExchange(exchangeAddress).getOrderInfo(order);
        ...
        uint256 tokenId = order.makerAssetData.toUint256(36);
        address tokenAddress= order.makerAssetData.toAddress(16);
        // Transfer token from maker to taker
        IERC721(tokenAddress).safeTransferFrom(order.makerAddress, takerAddress, tokenId);
