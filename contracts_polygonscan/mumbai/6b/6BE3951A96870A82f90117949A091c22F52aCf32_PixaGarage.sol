...
contract PixaGarage is ReentrancyGuard {
  ...
  function createMarketSale(
    ...
    require(msg.sender !=  idToMarketItem[itemId].seller, "You can not buy your own token!");
  ...
  function fetchWhatISold(uint page, uint limit) public view returns (MarketItem[] memory) {
    ...
    for (uint256 i = totalItemCount - page; i > 0; i--) {
      if (idToMarketItem[i].creator == msg.sender && idToMarketItem[i].sold && !idToMarketItem[i].assetColor) {
        itemCount += 1;
        if (itemCount == limit) {
          break;
        }
