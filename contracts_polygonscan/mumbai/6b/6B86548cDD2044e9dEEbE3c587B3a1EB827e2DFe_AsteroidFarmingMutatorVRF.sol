...
import './AsteroidFarmingMutator.sol';

contract AsteroidFarmingMutatorVRF is AsteroidFarmingMutator, VRFConsumerBase {
    enum MutationStatus {
        Pending,
        Fullfilled
    }
    ...
    // Mapping to track requestIds => MutationInfo
    mapping(bytes32 => MutationInfo) public userRequests;

    event MutationRequested(address indexed u, uint256 indexed t, bytes32 indexed r);
    event MutationFullfilled(address indexed u, uint256 indexed t, bytes32 indexed r);
    ...
    constructor(
        ...
        uint256 _fee,
        bytes32 _keyHash
    ) public AsteroidFarmingMutator(_nft) VRFConsumerBase(_coordinator, _link) {
        fee = _fee;
        keyHash = _keyHash;
    }

    function mutate(uint256 _type) external override {
        ...
        if (config.chances == 100) {
            ...
            require(LINK.balanceOf(address(this)) >= fee, 'AsteroidFarming: Not enough LINK');

            bytes32 requestId = requestRandomness(keyHash, fee);

            userRequests[requestId] = MutationInfo(_msgSender(), _type, MutationStatus.Pending);
            emit MutationRequested(_msgSender(), _type, requestId);
        }
    }

    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        MutationInfo storage info = userRequests[requestId];
        MutationConfig memory config = mutations[info.mutationType];

        uint256 random = randomness % 100;

        info.status = MutationStatus.Fullfilled;

        if (random <= config.chances) {
            nft.mint(info.user, config.outType, 1);
            emit MutationSuccess(info.user, info.mutationType);
        } else {
            emit MutationFailure(info.user, info.mutationType);
        }
    }
