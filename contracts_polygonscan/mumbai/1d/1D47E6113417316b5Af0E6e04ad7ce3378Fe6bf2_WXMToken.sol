...
contract WXMToken is ERC20, BaseRelayRecipient {
    ...
    modifier onlyOwner() {
        require(_msgSender() == owner, "Only the owner can perfom this action");
        _;
    }

    constructor(uint initialSupply, address _trustedForwarder) ERC20("WXM Token", "WXM") {
        owner = _msgSender();
    ...
    function transfer(address to, uint256 tokens) public override returns (bool){
        ...
        require(tokens <= balances[_msgSender()], "WXM: tokens should be less/equal to the balance of the sender");
        balances[_msgSender()] = balances[_msgSender()] - tokens;
    ...
    function transferFrom(address from, address to, uint tokens) public override returns (bool){
        require(allowed[from][_msgSender()] >= tokens,"WXM: the allowance should be more or equal to the tokens to be transfered");
        balances[from] = balances[from] - tokens;
        allowed[from][_msgSender()] = allowed[from][_msgSender()] - tokens;
        balances[to] = balances[to] + tokens;
