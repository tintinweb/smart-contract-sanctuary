...
contract FuzzyswapStaker is IFuzzyswapStaker, Multicall {
    ...
    IFuzzyswapFactory public immutable override factory;
    ...
    /// @param _factory the Fuzzyswap factory
    /// @param _nonfungiblePositionManager the NFT position manager contract address
    /// @param _maxIncentiveStartLeadTime the max duration of an incentive in seconds
    /// @param _maxIncentiveDuration the max amount of seconds into the future the incentive startTime can be set
    constructor(
        IFuzzyswapFactory _factory,
    ...
    function _stakeToken(IncentiveKey memory key, uint256 tokenId) private {
        require(block.timestamp >= key.startTime, 'FuzzyswapStaker::stakeToken: incentive not started');
        require(block.timestamp < key.endTime, 'FuzzyswapStaker::stakeToken: incentive ended');
...
interface IFuzzyswapStaker is IERC721Receiver, IMulticall {
    ...
    /// @notice The Fuzzyswap Factory
    function factory() external view returns (IFuzzyswapFactory);
...
library NFTPositionInfo {
    /// @param deployer The address of the Fuzzyswap Factory used in computing the pool address
...
interface IFuzzyswapPoolImmutables {
    function externalOracle() external view returns (IExternalOracle);
...
library PoolAddress {
    ...
    bytes32 internal constant POOL_INIT_CODE_HASH = 0x3ffc367594a5eaebb55f1b3dc358e9ab25b9daa285eb14c27573a1410fd036dc;
