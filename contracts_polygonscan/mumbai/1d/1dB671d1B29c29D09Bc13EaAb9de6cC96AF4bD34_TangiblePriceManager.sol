...
contract TangiblePriceManager is ITangiblePriceManager, AdminAndTangibleAccess{
    ...
    function getPriceOracleForCategory(ITangibleNFT category) external view override returns (IPriceOracle){
        return oracleForCategory[category];
    }

    /// @dev The function returns current price from oracle for provided category.
    function setOracleForCategory(ITangibleNFT category, IPriceOracle oracle) external override onlyFactoryOrAdmin(){
