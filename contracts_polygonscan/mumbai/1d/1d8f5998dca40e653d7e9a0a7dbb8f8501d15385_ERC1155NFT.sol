...
contract ERC1155NFT is ERC1155, Ownable, VRFConsumerBase{
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        if(requestToPack[requestId] == Pack.PREMIUM) {
            buyPremiumPack(randomness, requestId);
        }else if(requestToPack[requestId] == Pack.FRESH) {
            buyFreshPack(randomness, requestId);
        }else {
            buyBasePack(randomness, requestId);
        }
    }

    function buyPremiumPack(uint256 _randomNumner, bytes32 _requestId) public {
        require(PremiumPack > 0, "Contract::premiumPack: PremiumPacks are sold out.");
    ...
    function buyFreshPack(uint256 _randomNumner, bytes32 _requestId) internal {
        require(FreshPack > 0, "Contract::premiumPack: PremiumPacks are sold out.");

        FreshPack = FreshPack - 1;
    }

    function buyBasePack(uint256 _randomNumner, bytes32 _requestId) internal {
        require(BasePack > 0, "Contract::premiumPack: PremiumPacks are sold out.");
