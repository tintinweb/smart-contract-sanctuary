...
abstract contract AbstractLender is AbstractTreasury{
    ...
    mapping(address => mapping(address => uint)) internal lastAccruedTimeStamp;
    ...
    function borrowAgainstIndexToken(address _token, uint collateralAmount, uint borrowAmount) public{
        ...
        uint transactionFee = borrowAmount.mulTruncate(_transaction_fee);
        token.transferFrom(msg.sender, address(this), collateralAmount);
        stableCoin.mint(msg.sender, _token, borrowAmount, collateralAmount, transactionFee);
    }
    ...
    function addIndexTokenCollateral(address _token, uint amount) public{
        ...
        StableCoin stableCoin = StableCoin(stableCoinFactory.getCoin(tokens[_token].currency));
        stableCoin.mint(msg.sender, _token, 0, amount, 0);
    }
    ...
    function accrue(address _account, address _token) public {
        StableCoin stableCoin = StableCoin(stableCoinFactory.getCoin(tokens[_token].currency));
        
        uint _lastAccruedTimeStamp = lastAccruedTimeStamp[_account][_token];
        // Number of seconds since accrue was called
        uint256 elapsedTime = block.timestamp - _lastAccruedTimeStamp;
        if (elapsedTime == 0) {
            return;
        }
        _lastAccruedTimeStamp = uint(block.timestamp);

        uint existingBorrowedAmount = stableCoin.getIndexWiseCoinBorrowed(_account, _token);

        // Accrue interest
        uint interest = existingBorrowedAmount.mulTruncate(_interest_per_second).mulTruncate(elapsedTime);
        stableCoin.setIndexWiseCoinBorrowed(_account, _token, existingBorrowedAmount + interest);
        lastAccruedTimeStamp[_account][_token] = _lastAccruedTimeStamp;
    }
}
...
contract StableCoin is ERC20, Ownable{
    ...
    function mint(address _account, address _token, uint borrowedAmount, uint collateralAmount, uint transactionFees) external onlyOwner{
        if (borrowedAmount > 0){
            _mint(_account, borrowedAmount - transactionFees);
        }
    ...
    function setIndexWiseCoinBorrowed(address _account, address _token, uint amount) external onlyOwner{
        _indexWiseCoinBorrowed[_account][_token] = amount;
    }
...
abstract contract Params {
    ...
    uint internal _denominator = 10000;
    uint internal _min_collateral_ratio = uint(13000).divPrecisely(_denominator);          // 130%
    uint internal _liquidation_ratio = uint(11000).divPrecisely(_denominator);             // 110%
    uint internal _transaction_fee = uint(50).divPrecisely(_denominator);                  // 0.5% 
    uint internal _max_borrow_limit = uint(7000).divPrecisely(_denominator);               // 70%
    uint internal _interest_per_second = uint(100).divPrecisely(_denominator
                                            ).divPrecisely(3600 * 24 * 365.25);             // 1%
}