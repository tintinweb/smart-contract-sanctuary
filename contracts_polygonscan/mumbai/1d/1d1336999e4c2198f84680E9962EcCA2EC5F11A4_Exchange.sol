...
contract ExchangeCore is ReentrancyGuarded, EIP712, Ownable {
    ...
    function createSale_(Order memory seller, bytes memory signature) internal {
        Collection collection = Collection(seller.target);
        bytes32 firstHash = hashOrder_(seller);

        /* Check first order validity. */
        require(
            validateOrderParameters_(seller, firstHash),
            "Sller order has invalid parameters"
        );

        /* Check first order authorization. */
        require(
            validateOrderAuthorization_(firstHash, seller.maker, signature),
            "Seller order failed authorization"
        );
        ...
        require(
            collection.ownerOf(seller.tokenId) == seller.maker,
            "Seller must be have this token"
        );

        require(
            collection.lockOf(seller.tokenId) != true,
            "This card already on sale"
        );
    ...
    function cancelSale_(
        ...
        require(collection.lockOf(tokenId) == true, "This card not on sale");

        require(
            collection.ownerOf(tokenId) == seller,
    ...
    AccessControl
{
    ...
    function mint(
        ...
        emit NewCardOwner(_to, _id, _username, _uri);
    }

    function mintNoVault(
        address _to,
        uint256 _id,
        string memory _uri
    ) external virtual override onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        emit NewNFTOwner(_to, _id, customUri[_id]);
    }
