...
contract MuonNFTBridge is Ownable, IERC721Receiver {
  ...
  //TODO: can we handle multiple deposits?
  function deposit(
  ...
  function claim(
    ...
    // txParams[2] = tokenId 
    // txParams[3] = txId 
    // txParams[4] = muonTimestamp

    require(block.timestamp - txParams[4]  < acceptableMuonDelay, 'expired');
    ...
    bytes32 hash = keccak256(
      abi.encodePacked(
        ...
        abi.encodePacked(user, APP_ID, txParams[4])
      )
    );

    require(muon.verify(_reqId, uint256(hash), _sigs), '!verified');

    require(!claimedTxs[txParams[0]][txParams[3]], 'alreay claimed');
  ...
  function addMainToken(address _tokenContract) public {
    ...
    IMuonNFTBridgeERC721 token = IMuonNFTBridgeERC721(_tokenContract);
    //TODO: check to make sure that its an NFT contract
  ...
  function addBridgeToken(
    ...
    bytes32 hash = keccak256(
      abi.encodePacked(_tokenId, _name, _symbol, APP_ID)
    );
    require(muon.verify(_reqId, uint256(hash), _sigs), '!verified');
...
contract MuonNFTBridgeTokenFactory{
    ...
    function create(string calldata _name, string calldata _symbol) external returns(address created){
        created = address(new MuonNFTBridgeToken(bridge, _name, _symbol));
...
contract MuonNFTBridgeToken is ERC721{
    ...
    function _baseURI() internal view virtual override returns (string memory) {
        return string(abi.encodePacked(_baseTokenURI, address(this)));
    }
}
