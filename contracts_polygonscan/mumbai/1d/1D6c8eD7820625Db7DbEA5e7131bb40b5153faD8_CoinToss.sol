...
contract CoinToss is Game {
    ...
    function wager(
        ...
        _coinTossBets[bet.id] = CoinTossBet(bet, face);
        ...
        override
    {
        ...
        uint256 payout = _resolveBet(
            ...
            getPayout(coinTossBet.bet.amount)
        );

        emit Roll(
            coinTossBet.bet.id,
            coinTossBet.bet.user,
            coinTossBet.bet.token,
            coinTossBet.bet.amount,
            coinTossBet.face,
            rolledCoinSide,
            payout
        );
        ...
        returns (CoinTossBet[] memory)
    {
        ...
        if (betsLength > 0) {
            for (uint256 i = betsLength; i >= dataLength; i--) {
                if (_coinTossBets[userBetsIds[i - 1]].bet.resolved == false) {
                    unresolvedBetsCounter++;
                }
            }
        }
        CoinTossBet[] memory unresolvedBets = new CoinTossBet[](
            unresolvedBetsCounter
        );
        if (unresolvedBetsCounter > 0) {
            ...
            for (uint256 i = betsLength; i >= dataLength; i--) {
                if (_coinTossBets[userBetsIds[i - 1]].bet.resolved == false) {
                    unresolvedBets[unresolvedBetsCounter] = _coinTossBets[
                        userBetsIds[i - 1]
                    ];
                    unresolvedBetsCounter++;
                }
