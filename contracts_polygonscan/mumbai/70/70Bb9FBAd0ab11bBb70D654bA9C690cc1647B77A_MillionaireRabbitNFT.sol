...
contract MillionaireRabbitNFT is ERC721URIStorage, VRFConsumerBase, Ownable {
    ...
    string internal baseURI = "ipfs://somewhereovertherainbowwayuphigh/";
    ...
    function rabbitCharCreationFulfillment(bytes32 requestId, uint256 randomNumber) internal {
        ...
        uint256 wonBattles = 0;
        uint256 lostBattles =0;
        ...
        MillionaireRabbit memory rabbitChar = MillionaireRabbit(willBattle,boosted, boostime, 
        net_worth, attack, defense, Jedi_Rabbit_Power, newMintRequest.bunnyName, genesis, wonBattles, lostBattles);
        millionairerabbits.push(rabbitChar);
        _safeMint(newMintRequest.sender, newId);
    }

    //passive battle mode allows you to be an active defender and auto accept battles - BEWARE AUTO LOSE BETS TOO!!
    function _registerReady(uint256 bunnyId) public payable {
        require(msg.value >= baseFee,"Sorry it costs 0.1 matic to register as ready to battle.");
        millionairerabbits[bunnyId].willBattle == true;
    }

    //active battle mode lets you select oponents and battle if they are passive
    function _registerBattle(uint256 bunnyOneId, uint256 bunnyTwoId) public payable returns (bytes32 requestId) {
        require(msg.value >= baseFee,"Sorry it costs 0.1 matic to play");
        require(millionairerabbits[bunnyTwoId].willBattle == true, "That little bunny doesnt want to battle.....");
        ...
        _paymentToken.transferFrom(address(ownerOf(bunnyTwoId)), address(this), battleFee);     
    ...
    function _UP_Attack_W_JediPower(uint256 tokenId, uint256 jediPower) public payable{
        require(msg.value >= baseFee,"Fee is 0.1 matic to boost!");
        require(_isApprovedOrOwner(msg.sender, tokenId), "You don't own this Bunny!");
        require(millionairerabbits[tokenId].Jedi_Rabbit_Power >= jediPower, "Not enough super_jedi_rabbit_power");
        millionairerabbits[tokenId].attack += jediPower;
        millionairerabbits[tokenId].Jedi_Rabbit_Power -= jediPower;
    }

    //boost once daily up to 5 carrots

    function _Up_JediPwr_W_Carrots(uint256 tokenId, uint256 _Carrots) public payable
    {//1 carrot = 5 power points
         if(!boosted){
           require(_isApprovedOrOwner(msg.sender, tokenId), "You don't own this Bunny!");
           require(_Carrots<=5, "Too many carrots! Max 5 a day and one time only so choose wisely!");
           require(millionairerabbits[tokenId].boosted = false, "Already Boosted! try again in roughly 43200 blocks!");
           uint conversion = _Carrots * 5;
           _paymentToken.transferFrom(msg.sender, address(this), _Carrots);
           ...
           millionairerabbits[tokenId].boostime == block.number;
            ...
            _Up_JediPwr_W_Carrots(tokenId,_Carrots);}
            }
            if(!authorized[msg.sender])
            {
    ...
    function _UP_Defense_W_JediPower(uint256 tokenId, uint256 jediPower) public payable {
        require(msg.value >= 0.1*10**18,"no no no costs to play");
        require(_isApprovedOrOwner(msg.sender, tokenId), "You don't own this Bunny!");
        require(millionairerabbits[tokenId].Jedi_Rabbit_Power >= jediPower, "Not enough super_jedi_rabbit_power");
        millionairerabbits[tokenId].defense += jediPower;
        millionairerabbits[tokenId].Jedi_Rabbit_Power -= jediPower;
    }
    ...
    function battleFulfillment(bytes32 requestId, uint256 randomness) internal {
        ...
        if (winningNumber1 >= winningNumber2) {
            millionairerabbits[battleRequest.rabbitHolderOne].Jedi_Rabbit_Power += 2;
            millionairerabbits[battleRequest.rabbitHolderOne].wonBattles ++ ;
            millionairerabbits[battleRequest.rabbitHolderTwo].lostBattles ++;
            if (millionairerabbits[battleRequest.rabbitHolderTwo].Jedi_Rabbit_Power >= 10) {
                millionairerabbits[battleRequest.rabbitHolderTwo].Jedi_Rabbit_Power -= 1;
            }
            if (millionairerabbits[battleRequest.rabbitHolderTwo].Jedi_Rabbit_Power >= 100) {
                millionairerabbits[battleRequest.rabbitHolderTwo].Jedi_Rabbit_Power -= 20;
            }
            emit battleDone(battleRequest.rabbitHolderOne);
         _paymentToken.transferFrom(address(this), msg.sender, battleFee*75/100*2);
        } else {
            millionairerabbits[battleRequest.rabbitHolderTwo].Jedi_Rabbit_Power += 2;
            millionairerabbits[battleRequest.rabbitHolderTwo].wonBattles ++ ;
            millionairerabbits[battleRequest.rabbitHolderOne].lostBattles ++;
            if (millionairerabbits[battleRequest.rabbitHolderOne].Jedi_Rabbit_Power >= 10) {
                millionairerabbits[battleRequest.rabbitHolderOne].Jedi_Rabbit_Power -= 1;
            }
            if (millionairerabbits[battleRequest.rabbitHolderOne].Jedi_Rabbit_Power >= 100) {
                millionairerabbits[battleRequest.rabbitHolderOne].Jedi_Rabbit_Power -= 20;
            }
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomNumber) internal override {
       ...
       } else {battleFulfillment(requestId, randomNumber);}
    }
    ...
    returns (string memory)
  {
    ...
    function __setBattlePrice(uint256 _newPrice) public {
        require(authorized[msg.sender]);
    ...
    function __purgePT() public {
     require(authorized[msg.sender]);
     uint256 BadTokenBalance = IERC20(_paymentToken).balanceOf(address(this));
     IERC20(_paymentToken).transfer(msg.sender, BadTokenBalance);
    } 
}