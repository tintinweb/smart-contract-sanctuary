...
// hevm: flattened sources of /nix/store/b748ii3m6qrlwq7qk8p0lfpl3f36awcf-geb-proxy-actions/dapp/geb-proxy-actions/src/GebProxyAuctionActions.sol
...
////// /nix/store/b748ii3m6qrlwq7qk8p0lfpl3f36awcf-geb-proxy-actions/dapp/geb-proxy-actions/src/GebProxyAuctionActions.sol
/// GebProxyActions.sol
...
abstract contract AccountingEngineLike_1 {
    ...
    function safeEngine() external virtual returns (SAFEEngineLike_2);
}

abstract contract DebtAuctionHouseLike_1 {
...
abstract contract SurplusAuctionHouseLike_1 {
...
contract AuctionCommon {
    ...
    function claimProxyFunds(address tokenAddress) public {
        DSTokenLike_2 token = DSTokenLike_2(tokenAddress);
...
contract GebProxyDebtAuctionActions is Common, AuctionCommon {
    ...
    function startAndDecreaseSoldAmount(address coinJoin, address accountingEngineAddress, uint amountToBuy) public {
        AccountingEngineLike_1 accountingEngine = AccountingEngineLike_1(accountingEngineAddress);
        DebtAuctionHouseLike_1 debtAuctionHouse = DebtAuctionHouseLike_1(accountingEngine.debtAuctionHouse());
        SAFEEngineLike_2 safeEngine = SAFEEngineLike_2(CoinJoinLike_1(coinJoin).safeEngine());
    ...
    function decreaseSoldAmount(address coinJoin, address auctionHouse, uint auctionId, uint amountToBuy) public {
        DebtAuctionHouseLike_1 debtAuctionHouse = DebtAuctionHouseLike_1(auctionHouse);
        SAFEEngineLike_2 safeEngine = SAFEEngineLike_2(CoinJoinLike_1(coinJoin).safeEngine());
    ...
    function settleAuction(address coinJoin, address auctionHouse, uint auctionId) public {
        ...
        claimProxyFunds(address(CoinJoinLike_1(coinJoin).systemCoin()));
    ...
    function exitCoin(address coinJoin, uint wad) public {
        // Allows adapter to access to proxy's COIN balance in the safeEngine
        if (CoinJoinLike_1(coinJoin).safeEngine().canModifySAFE(address(this), address(coinJoin)) == 0) {
            CoinJoinLike_1(coinJoin).safeEngine().approveSAFEModification(coinJoin);
        }
        CoinJoinLike_1(coinJoin).exit(msg.sender, wad);
    }

    /// @notice Exits full Coin balance to proxy owner
    /// @param coinJoin CoinJoin
    function exitAllCoin(address coinJoin) public {
        exitCoin(coinJoin, CoinJoinLike_1(coinJoin).safeEngine().coinBalance(address(this)) / RAY);
    }
}
...
contract GebProxySurplusAuctionActions is Common, AuctionCommon {
    ...
    function startAndIncreaseBidSize(address accountingEngineAddress, uint bidSize) public {
        AccountingEngineLike_1 accountingEngine = AccountingEngineLike_1(accountingEngineAddress);
        SurplusAuctionHouseLike_1 surplusAuctionHouse = SurplusAuctionHouseLike_1(accountingEngine.surplusAuctionHouse());
        DSTokenLike_2 protocolToken = DSTokenLike_2(surplusAuctionHouse.protocolToken());
    ...
    function increaseBidSize(address auctionHouse, uint auctionId, uint bidSize) public {
        SurplusAuctionHouseLike_1 surplusAuctionHouse = SurplusAuctionHouseLike_1(auctionHouse);
        DSTokenLike_2 protocolToken = DSTokenLike_2(surplusAuctionHouse.protocolToken());
    ...
    function settleAuction(address coinJoin, address auctionHouse, uint auctionId) public {
        ...
        CoinJoinLike_1(coinJoin).exit(msg.sender, toWad(amountToBuy));
...
contract GebProxyStakedTokenAuctionActions is Common, AuctionCommon {
    ...
    function startAndIncreaseBidSize(address coinJoin, address stakingPool, uint bidSize) public {
        ...
        SurplusAuctionHouseLike_1 auctionHouse = SurplusAuctionHouseLike_1(pool.auctionHouse());
        SAFEEngineLike_2 safeEngine = SAFEEngineLike_2(CoinJoinLike_1(coinJoin).safeEngine());
    ...
    function increaseBidSize(address coinJoin, address auctionHouse_, uint auctionId, uint bidSize) public {
        SurplusAuctionHouseLike_1 auctionHouse = SurplusAuctionHouseLike_1(auctionHouse_);
        SAFEEngineLike_2 safeEngine = SAFEEngineLike_2(CoinJoinLike_1(coinJoin).safeEngine());
    ...
    function settleAuction(address auctionHouse_, uint auctionId) public {
        SurplusAuctionHouseLike_1 auctionHouse = SurplusAuctionHouseLike_1(auctionHouse_);
        DSTokenLike_2 stakedToken = DSTokenLike_2(auctionHouse.stakedToken());
