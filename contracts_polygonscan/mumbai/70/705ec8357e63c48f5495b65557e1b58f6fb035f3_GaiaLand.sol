  ...
  IERC721Enumerable
{
  ...
  /**
   * @dev See {IERC721-forceTransferFrom}.
   */
  function forceTransferFrom(
    address from,
    address to,
...
contract GaiaLand is ERC721, Ownable {
  ...
  constructor() ERC721('GAIALAND', 'GAIALAND') {
    ...
    // WMATICAddress = 0x2d7882beDcbfDDce29Ba99965dd3cdF7fcB10A1e;
  }
  ...
  function mintGaiaLandToken(
    ...
    for (uint256 i = 0; i < numberOfTokens; i++) {
      ...
      if (totalSupply() < MAX_GAIALANDS) {
        ...
        ownerMap[mintIndex] = msg.sender;
        _safeMint(msg.sender, mintIndex);
        transferTokenPayable(address(this), tPrice);
      }
    }
  }

  function mintGaiaLandCoin(
    uint256 numberOfTokens,
    uint256 numberOfGoodLands,
    uint256 numberOfRegularLands
  ) public {
    ...
    for (uint256 i = 0; i < numberOfTokens; i++) {
      ...
      if (totalSupply() < MAX_GAIALANDS) {
        ...
        ownerMap[mintIndex] = msg.sender;
        _safeMint(msg.sender, mintIndex);
        transferMATICOrWMATIC(payable(address(this)), tPrice);
      }
    }
  }

  function mintGaiaLandTokenNoPay(
    uint256 numberOfTokens
  ) public {
    require(saleIsActive, 'Sale must be active to mint GaiaLand');
    require(
  ...
  function buy(uint256 _id, uint256 _price) external payable {
    ...
    forceTransferFrom(_previousOwner, _newOwner, _id);
    ownerMap[_id] = msg.sender;
    listedMap[_id] = false;
    delete auctions[_id];
  }

  function buyToken(uint256 _id, uint256 _price) external payable {
    _validate(_id);
    require(listedMap[_id] == true, 'Already colsed');
    require(price[_id] >= _price, 'Error, price is not match');
    address _previousOwner = ownerMap[_id];
    ...
    uint256 _commissionValue = price[_id].mul(25).div(1000);
    uint256 _sellerValue = price[_id].sub(_commissionValue);
    transferTokenPayable(_previousOwner, _sellerValue);
    forceTransferFrom(_previousOwner, _newOwner, _id);
    ownerMap[_id] = msg.sender;
    listedMap[_id] = false;
    delete auctions[_id];
  ...
  function transferTokenPayable(address to, uint256 value) private {
