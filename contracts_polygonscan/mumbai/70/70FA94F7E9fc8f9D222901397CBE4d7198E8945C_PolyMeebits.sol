...
contract PolyMeebits is IERC721 {
    ...
    string internal nftName = "Polymeebits";
    string internal nftSymbol = "Polymeebits";
    ...
    uint public nftPrice = 0.0001 ether;
     ...
     */
    function takeMaticUser() external payable reentrancyGuard returns(uint){
        require(users[msg.sender].isExist == true, "user not valid");
        require(users[msg.sender].wallet> 0, "user have not enough matic");
 ...
 function _calculateShares(uint value) internal pure returns ( uint _feeBOneShare, uint _feeBTwoShare) {
        ...
        _feeBOneShare = _fraction(4 , 1000, totalFeeValue);
        _feeBTwoShare = _fraction(6 , 1000, totalFeeValue);
    ...
    function mint(uint256 amount) external payable reentrancyGuard returns (uint) {
        require(publicSale, "Sale not started.");
        require(!marketPaused);
        require(numSales < SALE_LIMIT, "Sale limit reached.");
        require(amount>20, "Amount is not valid.");
        require(amount<1, "Amount is not valid.");
        
        uint256 finalPrice;
        finalPrice=amount*nftPrice;
        require(finalPrice>=msg.value, "Your value is not enough");
        
        

        
        require(msg.value >= finalPrice, "Insufficient funds to purchase.");
        ...
        uint ownerMatic = 85;
        uint everylevelOneMatic = 0;
        uint everylevelTwoMatic= 0;

        if(poolUserLevelOne>0){
            everylevelOneMatic = ((nftPrice * 5)/100) / poolUserLevelOne;
        }else{
            ownerMatic += 5;
        }
        ...
        if(poolUserLevelTwo>0){
            everylevelTwoMatic = ((nftPrice * 10)/100) / poolUserLevelTwo;
        }else{
            ownerMatic += 10;
        }
        ...
        for(uint i = 1; i <= countPool ; i++){
            ...
            if(users[userPoolAddress].level == 2 && poolUserLevelTwo >0){
                users[userPoolAddress].wallet += everylevelTwoMatic;
            }else if(users[userPoolAddress].level == 1 && poolUserLevelOne> 0){
                users[userPoolAddress].wallet += everylevelOneMatic;
            }
        }
        
        uint feeAmount = nftPrice.mul(ownerMatic);
