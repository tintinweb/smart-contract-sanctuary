    ...
    ContextMixin
{
    ...
    constructor() public ERC721("wTHCtest", "wthct") {
        _setBaseURI("https://ipfs.io/ipfs/QmRGSYJZZWkxNuWYaYmxSjCTBVmpDoEBDgfWz3BGuAmdUw/");
        _setupContractId("THCTEST");
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _setupRole(DEPOSITOR_ROLE, 0xA6FA4fB5f76172d178d61B04b0ecd319C5d1C0aa);
        _initializeEIP712("wTHCtest");
        deployMint(10);
    }

    function deployMint(uint256 nbToken) public only(DEFAULT_ADMIN_ROLE) {
        uint256 i = 1;
        while (i <= nbToken) {
         mint(msgSender(), i);
         i++;
    ...
    function withdraw(uint256 tokenId) external {
        require(_msgSender() == ownerOf(tokenId), "TheHappyCloud: INVALID_TOKEN_OWNER");
    ...
    function withdrawBatch(uint256[] calldata tokenIds) external {
        ...
        uint256 length = tokenIds.length;
        require(length <= BATCH_LIMIT, "TheHappyCloud: EXCEEDS_BATCH_LIMIT");
        ...
        for (uint256 i; i < length; i++) {
            ...
            require(_msgSender() == ownerOf(tokenId), string(abi.encodePacked("TheHappyCloud: INVALID_TOKEN_OWNER ", tokenId)));
            withdrawnTokens[tokenId] = true;
            _burn(tokenId);
    ...
    function mint(address user, uint256 tokenId) public only(DEFAULT_ADMIN_ROLE) {
        require(!withdrawnTokens[tokenId], "TheHappyCloud: TOKEN_EXISTS_ON_ROOT_CHAIN");
        _mint(user, tokenId);
    }
