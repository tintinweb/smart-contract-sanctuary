...
contract KOITools is Ownable {
  ...
  IKOI KOIContract = IKOI(0x7E13c36aCC833BbA1CA219d344d71BB3767EFdA3);

  IAURA AURAContract = IAURA(0xA627903e540CfFC35fF4C1C95DaAFC27D6E1F865);
  ...
  function LevelStats() external view returns (uint256[] memory) {
    uint256 _totalSupply = KOIContract.totalSupply();
    uint256 _Level;
    uint256[] memory output = new uint256[](100);
    for (uint256 i = 0; i < _totalSupply; i++) {
  ...
  function LevelUpAllLimite(uint256 _limite) external {
    uint256[] memory result = new uint256[](_limite);
  ...
  function AURAtransferAllIDToOne(uint256 _limite, uint256 _to_id) external {
    ...
    if (result.length > 0) {
      AURAtransferSpecIDToOne(result, _to_id);
    }
  }
  ...
  function ERC721multisend_SpecTokenToAddress(
    ...
    while (i < _KOI_ids.length) {
      if (KOIContract.ownerOf(_KOI_ids[i]) == msg.sender) {
        ERC721(0x7E13c36aCC833BbA1CA219d344d71BB3767EFdA3).transferFrom(
          msg.sender,
          _to,
          _KOI_ids[i]
        );
