...
contract DPSDocks is ERC721Holder, ReentrancyGuard, Ownable {
    ...
    function lockVoyageItems(LockedVoyage memory _lockedVoyage) external nonReentrant {
        ...
        _lockedVoyage.claimedTime = block.timestamp;
    ...
    function claimRewards(uint256 _voyageId, CausalityParams memory causalityParams) external nonReentrant {
        causalityParams.userAddress = getLockedVoyageOwner(_voyageId);
        require(causalityParams.userAddress != address(0), "Voyage does not exists");
        ...
        VoyageResult memory voyageResult = computeVoyageState(lockedVoyage, voyageConfig, causalityParams);
        lockedVoyage.claimedTime = block.timestamp;
        finishedVoyages[causalityParams.userAddress][lockedVoyage.voyageId] = lockedVoyage;
        finishedVoyagesIds[causalityParams.userAddress].push(lockedVoyage.voyageId);
        awardRewards(voyageResult, voyageConfig, lockedVoyage, causalityParams.userAddress);
        cleanLockedVoyage(lockedVoyage.voyageId, causalityParams.userAddress);
    ...
    function computeVoyageState(
        ...
        for (uint256 i; i < voyageConfig.sequence.length; i++) {
            ...
            if (
                ...
                claimingRewardsCache.randomCheckIndex + 1 >= causalityParams.blockNumber.length - 1
            ) {
