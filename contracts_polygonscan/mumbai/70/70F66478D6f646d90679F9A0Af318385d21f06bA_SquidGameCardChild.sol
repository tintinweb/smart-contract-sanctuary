...
contract SquidGameCardChild is SquidGameCard {
    uint256 public constant BATCH_LIMIT = 20;
    
    address deployer;
    address childChainManagerProxy;

    event TransferNFT(address indexed sender, address indexed receiver, uint256 tokenId);
    event WithdrawnBatch(address indexed user, uint256[] tokenIds);
    // event TransferWithMetadata(address indexed from, address indexed to, uint256 indexed tokenId, bytes metaData);

    constructor() public SquidGameCard() {
        childChainManagerProxy = msg.sender; // temporal
        // childChainManagerProxy = 0xb5505a6d998549090530911180f38aC5130101c6 // mumbai
        // childChainManagerProxy = 0xA6FA4fB5f76172d178d61B04b0ecd319C5d1C0aa // polygon
    ...
    function deposittest(address user, uint256 tokenId) external {
    ...
    function deposit(address user, bytes calldata depositData) external {
        ...
        if (depositData.length == 32) {
            ...
            for (uint256 i; i < length; i++) {
                ...
                emit TransferNFT(address(0), user, tokenIds[i]);
            }
        }
    }
    ...
    function withdraw(uint256 tokenId) external {
        ...
        _burn(tokenId);
        _seeds[tokenId] = 0;
        emit TransferNFT(_msgSender(), address(0), tokenId);
    }
