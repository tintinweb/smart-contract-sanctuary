    ...
    UUPSUpgradeable
{
        ...
        checkIfUsedAndMarkAsUsed(_voucher.signature)
    {
        ...
        require(_msgSender() == _voucher.wallet, "Sender must match voucher wallet");
...
contract GemMetadata is Ownable, IGemMetadata {
    ...
    function drawGem(uint256 id) public view returns (string memory) {
        string
            memory svgAccumulator = '<svg version="1.1" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" viewBox="0 0 128 128" xml:space="preserve"><style type="text/css"> .st0{fill:#FFFFFF;} .st1{fill:#FF4000}</style>';
        svgAccumulator = strConcat(svgAccumulator, '<rect x="0" y="0" class="st1" width="100%" height="100%"/>');
        svgAccumulator = strConcat(svgAccumulator, '<text x="10" y="20" class="st0">YC Gem</text>');
