...
contract RandomSVG is ERC721URIStorage, VRFConsumerBase {
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomNumber)
        ...
        override
    {
        //Chainlink VRF has max gas of 200,000 gas(computing units)
        ...
        returns (string memory pathSvg)
    {
        ...
        for (uint256 i = 0; i < numberOfPathCommands; i++) {
            uint256 newRNG = uint256(
                keccak256(abi.encode(_randomness, size + i))
            );
    ...
    function formatTokenURI(string memory imageURI)
        ...
        returns (string memory)
    {
        string memory basedURL = "data:application/json;base64,";
        return
            string(
                abi.encodePacked(
                    basedURL,
                    Base64.encode(
                        bytes(
                            abi.encodePacked(
                                '{"name":"SVG NFT",',
                                '"description":"An NFT baseed on SVG!",',
                                '"attributes":"",',
                                '"image":"',
                                imageURI,
                                '"}'
