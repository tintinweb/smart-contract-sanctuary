...
contract GiddyCamToken is
    ...
    address public uniswapV2Router; // QuickSwap
    ...
    function initialize(address amTokenAddress, address underlyingStablecoinAddress, string memory name, string memory symbol, address _devAddress, address _trustedForwarder) public initializer {
        __Ownable_init();
        __Pausable_init();
        __ERC20_init(name, symbol);
        __ERC2771Context_init(_trustedForwarder);

        Token = amTokenAddress;
        usdc = underlyingStablecoinAddress;
    ...
    function _swapTokens(
        ...
        tokenBought = IUniswapV2Router02(uniswapV2Router).swapExactTokensForTokens(
            amountIn,
            amountOutMin,
            path,
            toAddress,
    ...
    function deposit(uint256 _amount) public {
        IERC20(usdc).transferFrom(_msgSender(), address(this), _amount);
        if (IERC20(usdc).allowance(address(this), uniswapV2Router) < _amount)
            IERC20(usdc).approve(uniswapV2Router, 9999999999999999999999999999999);
        ...
        if (totalShares == 0 || totalTokenLocked == 0) { // If no camToken exists, mint it 1:1 to the amount put in
            if(devFee > 0){
                // calculate devFee
                uint256 depositFee = userTokenDepositAmount.mul(devFee).div(10000);
                _mint(devAddress, depositFee);
                _mint(_msgSender(), userTokenDepositAmount.sub(depositFee));
            }else{
                _mint(_msgSender(), userTokenDepositAmount);
            }
        } else {
            uint256 camTokenAmount = userTokenDepositAmount.mul(totalShares).div(totalTokenLocked);
            if(devFee > 0){
                uint256 depositFee = camTokenAmount.mul(devFee).div(10000);
                _mint(devAddress, depositFee);
                _mint(_msgSender(), camTokenAmount.sub(depositFee));
            }else{
                _mint(_msgSender(), camTokenAmount);
            }
        }

         emit Deposit(_msgSender(), _amount, userTokenDepositAmount);
    }
    ...
    function harvestMaticIntoToken() public onlyOwner() {
        ...
        if(_wmaticBalance > 2) {
            ...
            IERC20(wMatic).approve(uniswapV2Router, 0);
            IERC20(wMatic).approve(uniswapV2Router, _wmaticBalance);
            
            // if successful this should increase the total MiMatic held by contract
            IUniswapV2Router02(uniswapV2Router).swapExactTokensForTokens(_wmaticBalance, uint256(0), path, address(this), block.timestamp.add(1200));
            
            uint256 newBalance = IERC20(usdc).balanceOf(address(this));

            // Just being safe
            IERC20(usdc).approve(LENDING_POOL, 0);
            // Approve Transfer _amount usdc to lending pool
            IERC20(usdc).approve(LENDING_POOL, newBalance);
    ...
    function withdraw(uint256 _share) public {
        if(_share>0){
            ...
            IERC20(Token).transfer(_msgSender(), amTokenAmount);
        }
    }
    ...
    {
        if (_fee == true) {
            uint256 ethValue = _getQuote(_amount, usdc, wMatic, true);
            tokenValue = _getQuote(ethValue, wMatic, Token, true);
        } else {
            uint256 ethValue = _getQuote(_amount, usdc, wMatic, false);
        ...
        returns (uint256 usdValue)
    {
        if (_fee == true) {
            ...
            usdValue = _getQuote(ethValue, wMatic, usdc, true);
        } else {
            uint256 ethValue = _getQuote(_amount, Token, wMatic, false);
            usdValue = _getQuote(ethValue, wMatic, usdc, false);
        }
    }
