...
contract VoteTracker is EventReceiver, IVoteTracker, Ownable {
    ...
    //Normally these would be generated during construction against the current chain id
    ...
    mapping(address => mapping(bytes32 => UserVotePlacement)) public userVoteItems;
    ...
    constructor(
        ...
        settings.systemAggDetails.voteSessionKey = initialVoteSession;
    ...
    function vote(UserVotePayload calldata userVotePayload, uint8 v, bytes32 r, bytes32 s) external override {
        require(_getChainID() == userVotePayload.chainId, "INVALID_CHAIN");

        address account = userVotePayload.account;

        // Validate the signer is the account the votes are on behalf of
        address signatureSigner = _hash(userVotePayload).recover(v, r, s);
        require(signatureSigner == account, "MISMATCH_SIGNER");

        require(settings.systemAggDetails.voteSessionKey == userVotePayload.voteSessionKey, "NOT_CURRENT_VOTE_SESSION");        
        ...
        if (proxySubmitters[msg.sender]) {
            require(lastUserProxyVoteBlock[account].add(settings.voteEveryBlockLimit) < block.number, "TOO_FREQUENT_VOTING");
            lastUserProxyVoteBlock[account] = block.number;
        }
        ...
        //bytes32[] memory allocationKeys = new bytes32[](userVotePayload.allocations.length);
        UserVotePlacement[] memory newPlacements = new UserVotePlacement[](userVotePayload.allocations.length);
        for(uint256 i = 0; i < userVotePayload.allocations.length; i++) {
            ...
            //Saving
            newPlacements[i] = UserVotePlacement({
                reactorKey: userVotePayload.allocations[i].reactorKey,
                pct: userVotePayload.allocations[i].amount.mul(ONE_WITH_EIGHTEEN_PRECISION).div(userVotePayload.totalVotes),
                amount: userVotePayload.allocations[i].amount
            });            
        }
        require(summedVotes == userVotePayload.totalVotes, "VOTE_TOTAL_MISMATCH");

        emit UserVoted(account, userVotePayload);

        // Data is validated and persisted, update the totals
        _updateUserVoteTotals(account, userVotePayload.totalVotes, newPlacements, maxAvailableVotes, false, block.number);
    }
    ...
    function updateUserVoteTotals(address account) public override {
        ...
        UserVotePlacement[] memory placements = new UserVotePlacement[](keys.length);
        for(uint256 i = 0; i < keys.length; i++) {
            placements[i] = userVoteItems[account][keys[i]];
        }
        uint256 maxAvailableVotes = getMaxVoteBalance(account);

        //Call here emits
       _updateUserVoteTotals(account, userVoteDetails[account].totalUsedVotes, placements, maxAvailableVotes, true, userVoteDetails[account].lastVoteBlock);
    } 
    ...
    function getSystemVotes() public override view returns(SystemVotes memory systemVotes) {
        ...
        systemVotes = SystemVotes({
            details: settings.systemAggDetails,
            votes: votes
        });
    }
    ...
    function _onCycleRollover(bytes calldata data) private {
        ...
        settings.systemAggDetails.voteSessionKey = newKey;
    ...
    function _updateUserVoteTotals(address account, uint256 totalUsedVotes, UserVotePlacement[] memory newPlacements, uint256 maxAvailableVotes, bool calcAmounts, uint256 lastVoteBlock) private {
        ...
        if (userAggLength > 0) {            
            for(uint256 i = userAggLength; i > 0; i--) {                                            
                uint256 amt = userVoteItems[account][currentAccountVoteKeys[i-1]].amount;
                systemAggregations[currentAccountVoteKeys[i-1]] = systemAggregations[currentAccountVoteKeys[i-1]].sub(amt);
                settings.systemAggDetails.totalVotes = settings.systemAggDetails.totalVotes.sub(amt);
        ...
        for(uint256 x = 0; x < newPlacements.length; x++) {
            ...
            if (calcAmounts) {
                uint256 votedAmt = maxVotesToUse.mul(newPlacements[x].pct).div(ONE_WITH_EIGHTEEN_PRECISION);                
                newPlacements[x].amount = votedAmt;
            }
            //Update user aggregation
            userVoteItems[account][newPlacements[x].reactorKey] = newPlacements[x];
            ...
            settings.systemAggDetails.totalVotes = settings.systemAggDetails.totalVotes.add(newPlacements[x].amount);
        }
        ...
        userVoteDetails[account] = UserVoteDetails({
            ...
            lastVoteSessionKey: settings.systemAggDetails.voteSessionKey,
    ...
    function _hash(UserVotePayload memory userVotePayload) private view returns (bytes32) {
        bytes32 x =  
            keccak256(
                abi.encodePacked(
                    EIP191_HEADER,
...
interface IVoteTracker is IEventReceiver {
    ...
    struct VoteTrackSettings {
        ...
        SystemVoteDetails systemAggDetails;
    }
    ...
    struct UserVotes {
        ...
        UserVotePlacement[] votes;
    }

    struct UserVoteDetails {
        bytes32 lastVoteSessionKey;
