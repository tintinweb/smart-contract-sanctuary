...
contract KoNFTFactory is NativeMetaTransaction {
    ...
    mapping(address => bool) storedGallery;

    constructor(){
        _initializeEIP712("KoNFTFactory");
    }

    modifier userHaveNotContract(){
        address contractAddress = userGallery[msg.sender];
        bool exist = storedGallery[contractAddress];
        require(!exist,"You have already gallery.");
        _;
    }

    function deployERC721(string memory _token, string memory _trigram) external userHaveNotContract returns (address) {
        address contractAddress = address(new ChildMintableERC721(_token, _trigram, CHILD_CHAIN_MANAGER_PROXY_POLYGON));
        userGallery[msg.sender] = contractAddress;
        storedGallery[contractAddress] = true;
