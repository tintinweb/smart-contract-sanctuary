...
contract NFTPlayToEarnGame is EIP712MetaTransaction {
    ...
    struct Player {
        ...
        address nftTokenAddress; // address of the NFT token that player used for joining
    ...
    struct Game {
        ...
        Player winner; // address of the winner
    ...
    mapping(uint256 => mapping(address => mapping(uint256 => address))) public nftPlayer; // game id to ( nftTokenAddress to (nftTokenId to player) mapping
    ...
    mapping (address => bool) public supportedNFTAddresses; // NFT token is supported or not
     ...
     * @param _creatorNftTokenAddress address of the NFT token holded by the creator
     * @param _creatorNftTokenId the tokenId of the NFT that creator holds
     */
    function createGame(
        uint256 _numPlayers,
        uint256 _buyIn,
        address _creator,
        address _creatorNftTokenAddress,
        uint256 _creatorNftTokenId
    ) public onlyAdmin {
        address nftOwner = IERC721(_creatorNftTokenAddress).ownerOf(_creatorNftTokenId);
        require(supportedNFTAddresses[_creatorNftTokenAddress], "NFT token not supported");
        require(nftOwner == _creator, "Player must hold the NFT token.");
        require(
            playerFunds[_creator] >= _buyIn,
            "Insuffient funds."
        );
        ...
        Player memory player = Player({
            player: _creator,
            nftTokenAddress: _creatorNftTokenAddress,
            nftTokenId: _creatorNftTokenId
        });
        players[numGames].push(player);
        nftPlayer[numGames][_creatorNftTokenAddress][_creatorNftTokenId] = _creator;
        playerFunds[_creator] = playerFunds[_creator].sub(
     ...
     * @param onEthereum indicates if funds needs to be withdrawn on Ethereum. If false, funds are withdrawn on Polygon
     */
     ...
     * @dev Join a game
     * @param _gameId id of the game
     * @param _player address of the player joining game
     * @param _nftTokenAddress address of the NFT token that player holds
     * @param _nftTokenId the tokenId of the NFT that player holds
     */
    function joinGame(uint256 _gameId, address _player, address _nftTokenAddress, uint256 _nftTokenId) public onlyAdmin {
        ...
        require(supportedNFTAddresses[_nftTokenAddress], "NFT token not supported");
        address nftOwner = IERC721(_nftTokenAddress).ownerOf(_nftTokenId);
        require(nftOwner == _player, "Player must hold the NFT token.");
        require(
            nftPlayer[_gameId][_nftTokenAddress][_nftTokenId] == address(0),
        ...
        nftPlayer[_gameId][_nftTokenAddress][_nftTokenId] = _player;
    ...
    function exitGame(uint256 _gameId, address _player) public onlyAdmin {
        ...
        address nftTokenAddress = players[_gameId][index].nftTokenAddress;
        nftPlayer[_gameId][nftTokenAddress][nftTokenId] = address(0);
    ...
    function endGame(uint256 _gameId, address _winner) public onlyAdmin {
        ...
        Player memory winner = Player({
            player: _winner,
            nftTokenAddress: players[_gameId][index].nftTokenAddress,
            nftTokenId: players[_gameId][index].nftTokenId
        });
        ...
        playerFunds[_winner] = playerFunds[_winner].add(winnerAmount);
    ...
    /**
     * @dev Reclaim any player's funds
     * @param _player address of the player to claim funds
     * @param onEthereum indicates if funds needs to be withdrawn on Ethereum. If false, funds are withdrawn on Polygon
     */
    function reclaimFunds(address _player, bool onEthereum) public onlyOwner {
        if (onEthereum) {
            ILiquidityPoolManager(LIQUIDITY_POOL_MANAGER_ADDRESS).depositErc20(
                WETH,
                msgSender(),
            ...
            IERC20(WETH).transfer(msgSender(), playerFunds[_player]);
        }
        playerFunds[_player] = 0;
    }
    
    /**
     * @dev Adds NFT token address to the list of supported NFT tokens
     * @param _nftTokenAddress address of the NFT token
     */
    function addNFT(address _nftTokenAddress) public onlyOwner {
        supportedNFTAddresses[_nftTokenAddress] = true;
    }
    
    /**
     * @dev Removes NFT token address from the list of supported NFT tokens
     * @param _nftTokenAddress address of the NFT token
     */
    function removeNFT(address _nftTokenAddress) public onlyOwner {
        supportedNFTAddresses[_nftTokenAddress] = false;
    }
