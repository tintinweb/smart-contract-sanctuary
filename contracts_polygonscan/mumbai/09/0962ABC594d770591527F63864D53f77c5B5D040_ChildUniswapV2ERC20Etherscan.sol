...
contract ChildUniswapV2ERC20Etherscan {
    ...
    // @notice Address of polygon childChainManagerProxy for bridge
     ...
     */
    constructor(address account, address minter_, address childChainManagerProxy_, uint mintingAllowedAfter_) public {
    ...
    /**
     * @notice change childChainManagerProxy
     * @notice newChildChainManagerProxy New address for newChildChainManagerProxy
     */
    function updateChildChainManager(address newChildChainManagerProxy) external {
	require(newChildChainManagerProxy != address(0), "Bad ChildChainManagerProxy address");
	require(msg.sender == minter, "Wsb::updateChildChainManager: minter address only.");
	childChainManagerProxy = newChildChainManagerProxy;
    }

    /**
     * @notice credit Polygon briged deposit
     * @param dst The account to credit
     * @param depositData The abi encoded deposit info
     */
    function deposit(address dst, bytes calldata depositData) external {
	require(msg.sender == childChainManagerProxy, "Wsb::deposit: Only ChildChainManagerProxy can deposit");
	require(dst != address(0), "Wsb::deposit: cannot transfer to the zero address");
	uint256 rawAmount = abi.decode(depositData, (uint256));
	// `amount` token getting minted here & equal amount got locked in RootChainManager

        // mint the amount
        uint96 amount = safe96(rawAmount, "Wsb::deposit: amount exceeds 96 bits");
        totalSupply = safe96(SafeMath.add(totalSupply, amount), "Wsb::deposit: totalSupply exceeds 96 bits");
    ...
    /**
     * @notice start Polygon briged withdrawal
     * @param amount Amount to withdraw
     */
     function withdraw(uint256 amount) external {
	 uint96 safeAmount = safe96(amount, "Wsb::withdraw: amount exceeds 96 bits");
         balances[msg.sender] = sub96(balances[msg.sender], safeAmount, "ERC20: burn amount exceeds balance");
	 require (totalSupply >= safeAmount, "Wsb::withdraw: can't burn more than totalSupply");
	 totalSupply = SafeMath.sub(totalSupply, safeAmount);

	 emit Transfer(msg.sender, address(0), safeAmount);
	 _moveDelegates(msg.sender, address(0), safeAmount);
     }
