	...
	OwnableUpgradeable
{
	...
	function initialize(IPixelCityDescritor _descriptor) public initializer {
		__ERC721_init("Pixel City", "PIXEL");
		__Ownable_init();
		__Pausable_init();
	...
	/**
	 * @dev Change sale state.
	 * @param _isActive New state.
	 */
	...
	/**
	 * @dev Change revealed state.
	 * @param _isRevealed New state.
	 */
	...
	/**
	 * @dev Set descriptor contract address.
	...
	/**
	 * @dev Withdraw all contract balance to the owner.
	 */
	...
	/**
	 * @dev Withdraw certain amount from the contract to the owner.
	 */
	function withdrawAmount(uint256 _amount) external onlyOwner {
		uint256 balance = address(this).balance;
		require(_amount <= balance, "Invalid ether amount");
		...
		returns (string memory)
	{
		if (isRevealed) {
			...
			return descriptor.genTokenURI(_tokenId, pixel);
		} else {
	...
	/**
	 * @dev A function to return all tokens owned by an address.
	...
	/**
	 * @dev Show token's metadata in Pixel format.
	 */
	...
	function claimPixels(uint8 _amount) external payable {
		...
		require(
			addressToMinted[msg.sender] + _amount <= maxMintAmount,
			"Address mint cap reached"
		);

		// If the amount is just 1, there's no need to loop, saving gas.
	...
	/**
	 * @dev Mint new tokens to address.
	...
	function claimPixelsTo(address _to, uint8 _amount) external payable {
		...
		require(_amount > 0 && _amount <= maxMintAmount, "Invalid mint amount");
		require(
			addressToMinted[_to] + _amount <= maxMintAmount,
			"Address mint cap reached"
		);
	...
	/**
	 * @dev Base minting function to avoid code duplication.
	 * @param _to Recipient address.
	 */
	...
	/**
	 * @dev Pause minting and transfer.
	 */
	...
	/**
	 * @dev Unpause minting and transfer.
	 */
