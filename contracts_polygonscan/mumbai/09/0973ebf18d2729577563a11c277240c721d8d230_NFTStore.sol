    ...
    NFTStoreStorage
{
    ...
    function createNFT(
        ...
        (bool success, bytes memory cid)  = address(nft).staticcall(abi.encodeWithSignature("getCid(address)"));
        if (success) {
            string memory cidstr = string(cid);
            require(nftByCid[cidstr] == address(0), "cid Already exists");
