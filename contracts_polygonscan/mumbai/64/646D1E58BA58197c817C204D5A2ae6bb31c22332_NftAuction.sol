...
contract NftAuction is OwnableUpgradeable, ERC2771ContextUpgradeable, UUPSUpgradeable, PausableUpgradeable {
    ...
    */
    ...
    function initialize(uint256 fee, address feeRecipient, address _trustedForwarder) initializer public {
        __Ownable_init();
    ...
    **/
    function _escrowTokensToSell(bool isErc721, address nftAddress, address seller, uint256 tokenId, uint256 amount) internal {
        if (!isErc721) {
            require(amount > 0);
            require(IERC1155Upgradeable(nftAddress).balanceOf(seller, tokenId) >= amount, "ERC1155 token balance is not sufficient for the seller..");
            IERC1155Upgradeable(nftAddress).safeTransferFrom(seller, address(this), tokenId, amount, "");
        } else {
            require(IERC721Upgradeable(nftAddress).ownerOf(tokenId) == seller, "ERC721 token does not belong to the author.");
    ...
    function _transferNFT(bool isErc721, address nftAddress, address recipient, uint256 tokenId, uint256 amount) internal {
        if (!isErc721) {
            IERC1155Upgradeable(nftAddress).safeTransferFrom(address(this), recipient, tokenId, amount, "");
        } else {
            IERC721Upgradeable(nftAddress).safeTransferFrom(address(this), recipient, tokenId);
        }
    }
    ...
    /**
      ...
      */
    function _transferAssets(address erc20Address, uint256 amount, address recipient, bool settleOrReturnFee, uint256 royaltiPercentage) internal {
        uint256 fee = amount * _auctionFee / 10000;
        uint256 royalti = amount * royaltiPercentage / 100;
        uint256 totalFee = fee + royalti;
        if (erc20Address != address(0)) {
            if (settleOrReturnFee) {
                IERC20Upgradeable(erc20Address).transfer(recipient, amount - totalFee);
                IERC20Upgradeable(erc20Address).transfer(_auctionFeeRecipient, totalFee);
            } else {
                IERC20Upgradeable(erc20Address).transfer(recipient, amount);
            }
        } else {
            if (settleOrReturnFee) {
                AddressUpgradeable.sendValue(payable(recipient), amount - totalFee);
                AddressUpgradeable.sendValue(payable(_auctionFeeRecipient), fee);
            } else {
    ...
    * @param minBid - minimum bid price
    * @param endedAt - expired date of the auction
    */
    function createAuction(string memory id, bool isErc721, address nftAddress, uint256 tokenId,
        address seller, uint256 minBid, uint256 endedAt) public whenNotPaused {
        uint256 minBidWithoutFee = minBid / (10000 + _auctionFee) * 10000;
        require(_auctions[id].startedAt == 0, "Auction already existed for current auction Id");
        require(endedAt > block.timestamp, "Auction must last at least 5 blocks from this block");
        // check if the seller owns the tokens he wants to put on auction
        // transfer the tokens to the auction house
        _escrowTokensToSell(isErc721, nftAddress, seller, tokenId, 1);

        _auctionCount++;
        Auction memory auction = Auction(seller, nftAddress, tokenId, isErc721, endedAt, block.timestamp, address(0), 1, minBidWithoutFee, address(0), minBid);
        _auctions[id] = auction;
        emit AuctionCreated(isErc721, nftAddress, tokenId, id, 1, address(0), endedAt);
    }
    ...
    */
    function bidForExternalBidder(string memory id, uint256 bidValue, address bidder) public whenNotPaused {
        ...
        require(auction.endedAt > block.timestamp, "Auction has already ended. Unable to process bid. Aborting.");
        uint256 bidWithoutFee = bidValue / (10000 + _auctionFee) * 10000;
        require(auction.endingPrice < bidWithoutFee, "Bid free of the auction fee is lower than actual highest bid price. Aborting.");
        require(IERC20Upgradeable(auction.erc20Address).allowance(bidder, address(this)) >= bidValue, "Insufficient approval for ERC20 token for the auction bid. Aborting.");

        Auction memory newAuction = Auction(auction.seller, auction.nftAddress, auction.tokenId, auction.isErc721, auction.endedAt, block.timestamp, auction.erc20Address, auction.amount, auction.endingPrice, auction.bidder, auction.highestBid);
        // reentrancy attack - we delete the auction temporarily
        delete _auctions[id];

        IERC20Upgradeable token = IERC20Upgradeable(newAuction.erc20Address);
        ...
        if (newAuction.bidder != address(0) && newAuction.highestBid != 0) {
            _transferAssets(newAuction.erc20Address, newAuction.highestBid, newAuction.bidder, false, 0);
        }
    ...
    * @param bidValue - bid value + the auction fee
    */
    function bid(string memory id, uint256 bidValue) external payable whenNotPaused {
        Auction memory auction = _auctions[id];
        uint256 bidWithoutFee = bidValue / (10000 + _auctionFee) * 10000;
        require(auction.endedAt > block.timestamp, "Auction has already ended. Unable to process bid. Aborting.");
        ...
        if (auction.erc20Address != address(0)) {
            require(IERC20Upgradeable(auction.erc20Address).allowance(_msgSender(), address(this)) >= bidValue, "Insufficient approval for ERC20 token for the auction bid. Aborting.");
        }
        ...
        if (newAuction.erc20Address != address(0)) {
            IERC20Upgradeable token = IERC20Upgradeable(newAuction.erc20Address);
            if (!token.transferFrom(_msgSender(), address(this), bidValue)) {
        ...
        newAuction.endingPrice = bidWithoutFee;
        newAuction.highestBid = bidValue;
        newAuction.bidder = _msgSender();

        _auctions[id] = newAuction;
        emit AuctionBid(_msgSender(), bidValue, id);
    }

    /**
    * Settle the already ended auction -
    */
    function settleAuction(string memory id, uint itemId, address marketplaceAddress, address creator, uint royaltiPercentage) external virtual payable {
        // fee must be sent to the fee recipient
        // NFT token to the bidder
        // payout to the seller
        Auction memory auction = _auctions[id];
        require(auction.seller == _msgSender() || _msgSender() == owner(), "Auction can't be settled from other than seller or owner. Aborting.");
        // require(auction.endedAt < block.timestamp, "Auction can't be settled before it reaches the end.");
        ...
        if(auction.seller != creator){
            uint royalti = highestBid * royaltiPercentage / 100;
            _transferAssets(erc20Address, highestBid - royalti, auction.seller, true, royaltiPercentage);
            AddressUpgradeable.sendValue(payable(creator), royalti);
        }
        else{
            _transferAssets(erc20Address, highestBid, auction.seller, true, 0);
        }

        NFTMarketplaceAbstact blMarket = NFTMarketplaceAbstact(marketplaceAddress);
        blMarket.updateAuctionedItem(itemId, auction.seller, bidder, highestBid);
    ...
    */
    function cancelAuction(string memory id) external virtual payable {
        Auction memory auction = _auctions[id];
        require(auction.seller != address(0), "Auction is already settled. Aborting.");
        require(auction.seller == _msgSender() || _msgSender() == owner(), "Auction can't be cancelled from other than seller or owner. Aborting.");
        bool isErc721 = auction.isErc721;
        address nftAddress = auction.nftAddress;
        uint256 amount = auction.amount;
        uint256 tokenId = auction.tokenId;
        ...
        if (bidder != address(0) && highestBid != 0) {
            _transferAssets(erc20Address, highestBid, bidder, false, 0);
        }
...
abstract contract NFTMarketplaceAbstact {
    function updateAuctionedItem(
        ...
        address seller,
        address bidder,
        uint price) public payable virtual;
}
