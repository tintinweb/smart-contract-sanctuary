...
contract WildEarthNFT is HasSecondarySaleFees, ContextMixin, ERC721Enumerable, ERC721Pausable, Ownable, ERC721Burnable {
    ...
    bool public trustWalletEnabled = true;
    ...
    constructor(
        ...
        // 10000 Bps
        defaultSecondarySalePercentage = _salesFeeBPS;
    ...
    function mint(uint256 _mintAmount, address receiver) public payable whenNotPaused {
        uint256 supply = totalSupply();
        bool isTrustEnabled = trustWalletEnabled && supply + _mintAmount * 2 <= maxSupply;
        require(_mintAmount > 0, "mint amount > 0");
        require(mintable, "Minting has been frozen");
        if (isTrustEnabled) {
            require(supply + _mintAmount * 2 <= maxSupply, "max NFT limit exceeded");
        } else {
            require(supply + _mintAmount <= maxSupply, "max NFT limit exceeded");
        }
        if (msg.sender != owner()) {
            require(msg.value == mintSaleFee * _mintAmount, "insufficient funds");
        }
        ...
        if (isTrustEnabled) {
            ...
            for (uint256 i = 1; i <= _mintAmount; i++) {
                _safeMint(trustWallet, currentSupply + i);
            }
        }
    }
    ...
    function setTrustWallet(address _trustWallet) public onlyOwner {
        trustWallet = _trustWallet;
    }

    function setTrustWalletEnable(bool _trustWalletEnabled) public onlyOwner {
        trustWalletEnabled = _trustWalletEnabled;
    }
    ...
    function changeDefaultSecondarySalePercentage(uint256 _basisPoints) external onlyOwner {
        require(_basisPoints <= basisPointsDenominator);
        defaultSecondarySalePercentage = _basisPoints;
    }

    function updateMintSaleFee(uint256 newMintSaleFeeAmount) external onlyOwner {
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC721, ERC165Storage, ERC721Enumerable) returns (bool) {
