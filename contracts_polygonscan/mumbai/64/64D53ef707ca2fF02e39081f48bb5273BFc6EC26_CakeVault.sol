...
contract CakeVault is Ownable, Pausable {
    ...
    struct UserInfo {
        ...
        uint256 lastReferralRewardClaimed;
    }
    ...
    mapping(address => address[]) public parentToChild;
    ...
    function calculateReferralFee(address child) private returns (uint256){
        UserInfo storage user = userInfo[child];
        uint256 reward = ((balanceOf().mul(user.shares)).div(totalShares)).sub(user.cakeAtLastUserAction);

        uint refReward = reward.mul(refRewardRate).div(10000);
        if (user.lastReferralRewardClaimed > refReward){
            return 0;
        }
        refReward = refReward.sub(user.lastReferralRewardClaimed);
        user.lastReferralRewardClaimed += refReward;

        return refReward;
    }

    function claimReferralReward(address _referrer) external{
        require(_referrer == msg.sender, "NOT REFERRER");
        address[] memory children = parentToChild[_referrer];
        uint256 totalReward;
        for (uint256 i = 0; i<children.length; i++){
            totalReward.add(calculateReferralFee(children[i]));
        }
        IFundStorage(FundStorage).transferReferral(_referrer, totalReward);
        totalReferralRewardClaimed += totalReward;
    }

    function addReferralLocal(address[] memory referrals ) public {
        for(uint256 i=0; i<referrals.length; i++){
            if(checkIfAChild(msg.sender, referrals[i])){
                parentToChild[msg.sender].push(referrals[i]);
            }
        }
    }

    function checkIfAChild(address _parent, address _child) public returns(bool){
        IReferral ref = IReferral(referralContract);
