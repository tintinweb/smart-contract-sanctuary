...
contract AwesomeNfts is ERC721Enumerable, Ownable {                 //contract Name is NErdyCoderCLone and inherited from the openzaplin libraries
  ...
  address public account = 0xAb8483F64d9C6d1EcF9b849Ae677dD3315835cb2;                                              //if we want to stop the minting it is like play and pause button 
  uint public tokenCost = 10;
  ...
  function setApprove(uint _amount) public payable  returns(bool) {
      IERC20 usdt = IERC20(conractAddress);
      require(usdt.balanceOf(msg.sender) >= tokenCost * _amount);
      return usdt.approve(address(this), tokenCost * _amount);
  }

  function checkBalance(address _customerAddress) public view returns (uint) {
      IERC20 usdt = IERC20(conractAddress);
      return usdt.balanceOf(_customerAddress);
  }


  function checkAvailabile (uint _amount) public payable{
        IERC20 usdt = IERC20(conractAddress);   
        require(usdt.allowance(msg.sender, address(this)) >= 10, "not approved");
        require(usdt.balanceOf(msg.sender) >= tokenCost, "balannce low");
        usdt.transferFrom(msg.sender ,account , tokenCost);
        uint256 supply = totalSupply();
        for (uint256 i = 1; i <= _amount; i++) {                                //mint the nft to the caller account
         _safeMint(msg.sender, supply + i);
        }
  }

  // https://yjx0ches3nxy.usemoralis.com/
 ...
 function whitelistUser(address _user) public onlyOwner {
  ...
  function setTokenCost(uint _cost) public onlyOwner {
    tokenCost = _cost;
  }

  function setContractTokenAddress (address _address) public onlyOwner {
      conractAddress = _address;
  }
