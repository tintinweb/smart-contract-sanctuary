...
contract OneRareStaking is Ownable, TokensRecoverable, ReentrancyGuard, VRFConsumerBase{
    ...
    // timestamp for end of staking period 
    uint256 public lockTimestamp; //= block.timestamp + 30 days
...
*/
    ...
    constructor(address _erc1155Token, address _ORareToken, uint256 _timePeriod) public VRFConsumerBase(
            0x8C7382F9D8f56b33781fE506E897a4F1e2d17255, // VRF Coordinator
            0x326C977E6efc84E512bB9C30f76E30c160eD06FB  // LINK Token
    )
    {
        require(_timePeriod>0,"_timePeriod cannot be zero");
        require(_erc1155Token!=address(0),"_erc1155Token cannot be zero address");
        require(_ORareToken!=address(0),"_ORareToken cannot be zero address");
        
        ERC1155Token = IERC1155(_erc1155Token);
        lockTimestamp = block.timestamp + _timePeriod;
    ...
    function getRandomNoForAirdrop(uint256[] memory ids) external onlyAdmin returns (bytes32) {
        require(block.timestamp>=lockTimestamp,"current time should be more than lockTimestamp");
    ...
    function stake(uint256 _amount) external nonReentrant returns (bool){
        ...
        require(_amount>=1e18,"_amount should be atleast 1 OneRare Token");

        ORareToken.transferFrom(_msgSender(),address(this),_amount);
        balances[_msgSender()]= balances[_msgSender()].add(_amount);
        uint256 _totalTimeLeft = lockTimestamp.sub(block.timestamp);
        uint256 _tickets = _amount.mul(_totalTimeLeft).div(1e18);// divided by 1e18
        for(uint256 i=0;i<_tickets;i++){
            ticketMapping[i+totalTickets] = _msgSender();
        }
   ...
   function withdraw() external nonReentrant returns (bool){
        ...
        ORareToken.transfer(_msgSender(), amount);
        return true;
    }
    ...
    function claimWinning(bytes32 requestId) public returns (bool){
        ...
        if(winnersAnnounced[requestId] && winners[_msgSender()][requestId].isWinner && !winners[_msgSender()][requestId].claimed){

            uint256[] memory tokenIdsWon = winners[_msgSender()][requestId].ids;
            uint256[] memory tokenIdsWonQty = new uint256[](tokenIdsWon.length);
            for(uint i=0;i<tokenIdsWon.length;i++)
                tokenIdsWonQty[i]=1;
            
            ERC1155Token.safeBatchTransferFrom(address(this), _msgSender(), tokenIdsWon, tokenIdsWonQty, "0x");
    ...
    function findAllWinners() external onlyAdmin nonReentrant returns (bool){
        for(uint i=0;i<allRequestIds.length;i++)
            findWinners(allRequestIds[i]);
        return true;
    }

    function findLatestWinners() external onlyAdmin nonReentrant returns (bool){
        findWinners(latestRequestId);
        return true;
    }


    function findWinners(bytes32 requestId) public onlyAdmin returns (bool){

        if(!winnersAnnounced[requestId] &&  block.timestamp>=lockTimestamp){
            ...
            for(uint i=0;i<ids.length;i++){
                ...
                address winnerAddress = ticketMapping[winnerTicket];
    ...
    modifier onlyAdmin() {
        require(AdminByAddress[_msgSender()] == true || _msgSender()==owner());
        _;
    }
