...
contract CashswapLocker is Ownable, ReentrancyGuard {
  using SafeMath for uint256;
  using EnumerableSet for EnumerableSet.AddressSet;

  ICashFactory public cashswapFactory;
  ...
  constructor(ICashFactory _cashswapFactory) {
    ...
    cashswapFactory = _cashswapFactory;
  }
  ...
  function lockLPToken (address _lpToken, uint256 _amount, uint256 _unlock_date, address payable _referral, bool _fee_in_eth, address payable _withdrawer) external payable nonReentrant {
    ...
    ICashswapV2Pair lpair = ICashswapV2Pair(address(_lpToken));
    address factoryPairAddress = cashswapFactory.getPair(lpair.token0(), lpair.token1());
