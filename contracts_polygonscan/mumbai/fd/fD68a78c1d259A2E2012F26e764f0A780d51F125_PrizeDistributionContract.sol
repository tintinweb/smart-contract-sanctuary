...
interface IMerkleDistributor {
    ...
    function isClaimed(uint256 index, uint256 _epoch) external view returns (bool);
    // Claim the given amount of the token to the given address. Reverts if the inputs are invalid.
    function claim(uint256 index, address account, uint256 amount, bytes32[] calldata merkleProof, uint256 _epoch) external;
