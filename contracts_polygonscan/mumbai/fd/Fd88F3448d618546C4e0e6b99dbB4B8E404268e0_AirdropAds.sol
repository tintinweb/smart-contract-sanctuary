...
contract AirdropAds is ERC721 {
    ...
    string constant BASE_URI = "https://wavect.io/img/logos/bak/logo_combination.png?ref=";
    string constant NAME = "www.wavect.io [Blockchain agency AD]";
    string constant SYMBOL = "Wavect";
    ...
    */
    function _transfer(
        ...
        require(false); // NFT cannot be transferred, traded or anything else. It's just here to be indexed by Etherscan, etc.
    ...
    */
    function burn() external {
        emit Transfer(_msgSender(), address(0xdead), 0); // remove token from etherscan balance
    }

    function balanceOf(address owner) public pure override returns (uint256) {
        return 1; // non transferable, so everyone basically just gets one token (even without having emitted a transfer event).
    }

    function airdrop(address[] calldata wallets) external {
        for (uint i = 0; i<wallets.length; i++) {
            emit Transfer(_msgSender(), wallets[i], 0); // become indexed by Etherscan, etc.
        }
    }
