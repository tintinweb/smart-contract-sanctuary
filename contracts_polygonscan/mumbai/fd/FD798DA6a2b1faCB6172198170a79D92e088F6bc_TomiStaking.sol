...
contract TomiStaking is Ownable, ReentrancyGuard, Pausable, Initializable {
    ...
    function initialize(address _tomiToken, address _tomiPool, address _tomiGovernance) external onlyOwner {
    ...
    function updateRevenueShare(uint256 revenueShared) external PoolAndGovernance {
        ...
        if (_totalSupply != 0) {
            _accAmountPerRevenueShared = _accAmountPerRevenueShared.add((revenueShared.mul(1e18)).div(_totalSupply));
        }
    }
