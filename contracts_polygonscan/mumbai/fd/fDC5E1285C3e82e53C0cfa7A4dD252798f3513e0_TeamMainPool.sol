 ...
 * 1% of #DG(510,000.00 #DG) will be locked in smart contract until 1st of May 2022 (and
 * gradually released in three release events, one each 1 days from the May 1st 2022).
 * 1% of #DG(510,000.00 #DG) will be locked until 1st May 2023 (and gradually released in three 
 * release events, one each 1 days from the May 1st 2023).
 * remaining 1% will be locked until 31st December 2023 and released gradually in four years 
 * from then, so in 48 release events (one each 1 days).
 */
contract TeamMainPool is Ownable, ReentrancyGuard {
  ...
  uint256 public lockTime1 = 1640191200;
  /* 1st Lock time 1st of May 2023 GMT Timezone */
  uint256 public lockTime2 = 1640277600;
  /* 1st Lock time 31st of December 2023 GMT Timezone */
  uint256 public lockTime3 = 1640364000;
  /* Left token after sending rewards */
  uint256 public leftAmount;
  /* the address of the token contract */
	IERC20 public dataGen;

  address public deadAddress = 0x63F0aB2B6c23dF32b7dCa489717a12D800CDb96E;
  ...
  function releaseSalary() external nonReentrant {
    ...
    if(block.timestamp < lockTime1) {
      ...
      uint256 epochs = block.timestamp.sub(lockTime1).div(1).add(2);
      ...
      uint256 epochs = block.timestamp.sub(lockTime2).div(600).add(5);
      ...
      uint256 epochs = block.timestamp.sub(lockTime3).div(600).add(1);
