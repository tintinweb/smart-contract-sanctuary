...
import "../libs/token/ERC20/IERC20mb.sol";
import "./interfaces/IKiBridgeV1Liquidity.sol";
import "./KiBridgeV1Factory.sol";

contract KiBridgeV17c is KiBridgeV1Factory {
  ...
  struct UserInfo {
    ...
    mapping(uint256 => LiquidityRequest) pendingToChainLiquidityRequest;
  }
  struct TokenInfo {
    bool transferable;
    bool liquidable;
    ...
    mapping(uint256 => bool) toChainable;
  }
  ...
  mapping(IERC20 => uint256) public bridgeTokenBalance;
  mapping(IERC20 => TokenInfo) public tokenInfo;
  mapping(IERC20 => LiquidityInfo) public liquidityInfo;
    ...
    private
  {
    ...
    uint256 expiration = block.timestamp + 5 minutes;
    userInfo[msg.sender].nonce += 1 ;
    require(userInfo[msg.sender].pendingToChainLiquidityRequest[_toChainId].params == 0, "Must use a unique ID");
    userInfo[msg.sender].pendingToChainLiquidityRequest[_toChainId] = LiquidityRequest({ 
        requestId: requestId,
  ...
  external
  {
    require(userInfo[msg.sender].pendingToChainLiquidityRequest[_toChainId].requestId == _requestId, "RequestId not in pendingQueue");
    LiquidityRequest memory liquidityRequest = userInfo[msg.sender].pendingToChainLiquidityRequest[_toChainId];
    ...
    delete userInfo[msg.sender].pendingToChainLiquidityRequest[_toChainId];
    if(liquidityInfo[liquidityRequest.token].pendingLiquidityToChain[liquidityRequest.toChainId] >= liquidityRequest.amount) {
      liquidityInfo[liquidityRequest.token].pendingLiquidityToChain[liquidityRequest.toChainId] -= liquidityRequest.amount;
    } else {
      liquidityInfo[liquidityRequest.token].fulfilledLiquidityFromChain[liquidityRequest.toChainId] =
      liquidityInfo[liquidityRequest.token].fulfilledLiquidityFromChain[liquidityRequest.toChainId] +
      liquidityRequest.amount - liquidityInfo[liquidityRequest.token].pendingLiquidityToChain[liquidityRequest.toChainId];
      liquidityInfo[liquidityRequest.token].pendingLiquidityToChain[liquidityRequest.toChainId] = 0;
    }
    ...
    returns (bool success, bytes memory result)
  {
    ...
    if(tokenInfo[_receiveToken].liquidable) {
      if(liquidityInfo[_receiveToken].pendingLiquidityToChain[_fromChainId] >= _amount) {
        liquidityInfo[_receiveToken].pendingLiquidityToChain[_fromChainId] -= _amount;
      } else {
        liquidityInfo[_receiveToken].fulfilledLiquidityFromChain[_fromChainId] =
        liquidityInfo[_receiveToken].fulfilledLiquidityFromChain[_fromChainId] +
        _amount - liquidityInfo[_receiveToken].pendingLiquidityToChain[_fromChainId];
        liquidityInfo[_receiveToken].pendingLiquidityToChain[_fromChainId] = 0;
      }
    }
    ...
    returns (bool success)
  {
    ...
    require(userInfo[_user].pendingToChainLiquidityRequest[toChainId].requestId == _requestId, "RequestId not in pendingQueue");
    require(
      userInfo[_user].pendingToChainLiquidityRequest[toChainId].params == keccak256(
        abi.encode(address(this),_fulfillFunctionId, _expiration)),
       "Params do not match request ID"
    );
    require(fromChainId == block.chainid, "Wrong chainId");
    delete userInfo[_user].pendingToChainLiquidityRequest[toChainId];
    if(_calAmount == 0) {
      if(liquidityInfo[transaction.sendToken].pendingLiquidityToChain[toChainId] >= transaction.amount) {
        liquidityInfo[transaction.sendToken].pendingLiquidityToChain[toChainId] -= transaction.amount;
      } else {
        liquidityInfo[transaction.sendToken].fulfilledLiquidityFromChain[toChainId] =
        liquidityInfo[transaction.sendToken].fulfilledLiquidityFromChain[toChainId] +
        transaction.amount - liquidityInfo[transaction.sendToken].pendingLiquidityToChain[toChainId];
        liquidityInfo[transaction.sendToken].pendingLiquidityToChain[toChainId] = 0;
      }
      if(transaction.executeFunctionId == this.liquidityOutTransferToUser.selector){
        userInfo[_user].pendingLiquidityBalance[transaction.sendToken] -= transaction.amount;
        liquidityInfo[transaction.sendToken].toAllChainableBalance += transaction.amount;
        IKiBridgeV1Liquidity(getLiquidity[transaction.sendToken]).mint(_user, transaction.amount);
        return false;
      }
      userInfo[_user].pendingTokenBalance[transaction.sendToken] -= transaction.amount;
      transaction.sendToken.safeTransfer(_user, transaction.amount);
      return false;
    }
    if(transaction.executeFunctionId == this.liquidityOutTransferToUser.selector){
      userInfo[_user].pendingLiquidityBalance[transaction.sendToken] -= _calAmount;
    } else {
      userInfo[_user].pendingTokenBalance[transaction.sendToken] -= _calAmount;
    }
    transaction.amount = _calAmount;
    userInfo[_user].transactions[fromChainId][toChainId].push(transaction);
    return true;
  }
 function fulfillRemoveLiquidity(
    bytes32 _requestId, 
    bytes4 _fulfillFunctionId,
    address _user,
    uint256 _expiration, 
    ...
    returns (bool success)
  {
    (uint256 toChainId,IERC20 token, uint256 amount) = abi.decode(
      _data, (uint256, IERC20, uint256));
    require(userInfo[_user].pendingToChainLiquidityRequest[toChainId].requestId == _requestId, "RequestId not in pendingQueue");
    require(
      userInfo[_user].pendingToChainLiquidityRequest[toChainId].params == keccak256(
        abi.encode(address(this),_fulfillFunctionId, _expiration)),
    ...
    require(toChainId == block.chainid, "Wrong chainId");
    delete userInfo[_user].pendingToChainLiquidityRequest[toChainId];
    userInfo[_user].pendingLiquidityBalance[token] -= amount;
    if(_calAmount == 0) {
        liquidityInfo[token].toAllChainableBalance += amount;
        IKiBridgeV1Liquidity(getLiquidity[token]).mint(_user, amount);
      return false;
    }
    amount = _calAmount;
    ...
    external 
  {
    ...
    require(userInfo[msg.sender].pendingToChainLiquidityRequest[_toChainId].requestId == 0, "number of requests exceeded");
...
function liquidityInReqTransferToChainRemoveLiq(
    bytes32 _specId, 
    uint256 _toChainId, 
    IERC20 _token, 
    uint256 _amount
    ...
    internal 
  {
    require(tokenInfo[_token].liquidable, "Liquidable Not Permitted");
    require(userInfo[msg.sender].pendingToChainLiquidityRequest[_toChainId].requestId == 0, "number of requests exceeded");
    require(_amount > 0,"Amount must be greater than zero");
    ...
    bytes memory data = abi.encode(
      ...
      _toChainId,
      Transaction({
        executeFunctionId: this.liquidityOutTransferToUser.selector,
        sendToken: _token,
        receiveToken: tokenInfo[_token].toChainToken[_toChainId],
        amount: _amount
      }),
...
function liquidityInRequestRemoveLiq(
    bytes32 _specId, 
    IERC20 _token, 
    uint256 _amount
  ) 
  ...
  {
    require(tokenInfo[_token].liquidable, "Liquidable Not Permitted");
    require(userInfo[msg.sender].pendingToChainLiquidityRequest[block.chainid].requestId == 0, "number of requests exceeded");
    require(_amount > 0,"Amount must be greater than zero");
    userInfo[msg.sender].pendingLiquidityBalance[_token] += _amount;
    bytes memory data = abi.encode(
      block.chainid, 
    ...
    oracleLiquidityRequest(
      _specId,  
      address(this), 
      this.fulfillRemoveLiquidity.selector, 
      _token, 
      block.chainid,
      _amount, 
      data
    ...
    public
  {
    ...
    liquidityInfo[_token].toAllChainableBalance -= _amount;
    liquidityInRequestRemoveLiq(_specId,_token,_amount);
  }
  function removeLiquidityToChain(
    bytes32 _specId,
    uint256 _toChainId,
    IERC20 _token,
    uint256 _amount
    ...
    public
  {
    ...
    IKiBridgeV1Liquidity(getLiquidity[_token]).burnFrom(msg.sender, _amount);
    liquidityInfo[_token].toAllChainableBalance -= _amount;
    liquidityInReqTransferToChainRemoveLiq(_specId, _toChainId, _token, _amount);
  }
  function removeLiquidityWithPermit(
    ...
    removeLiquidity(_specId, _token, _amount);
  }
  function removeLiquidityToChainWithPermit(
    bytes32 _specId,
    uint256 _toChainId,
    IERC20 _token,
    uint256 _amount,
    ...
    IERC20Permit(getLiquidity[_token]).permit(msg.sender, address(this), _amount, _deadline, _v, _r, _s);
    removeLiquidityToChain(_specId, _toChainId, _token, _amount);
  }
  function liquidityOutTransferToUser(
    address _user, 
    IERC20 _token, 
    uint256 _amount
  ) 
  ...
  {
    userInfo[_user].tokenBalance[_token] -= _amount;
    IERC20mb(address(_token)).mint(_user, _amount);
  }
    ...
    onlyContractCall()
  {
    ...
    _token.safeTransferFrom(_user, address(this), _amount);
    _amount = _token.balanceOf(address(this)) - balanceBefore;
    IERC20mb(address(_token)).burn(_amount);
    bridgeTokenBalance[_token] += _amount;
  }
  function tokenInBurnFrom(
    address _user,
    IERC20 _token, 
    uint256 _amount
  ) 
  ...
  {
    require(
      tokenInfo[_token].tokenInFunctionId == this.tokenInBurnFrom.selector, 
      "FunctionId Not Permitted"
    );
    IERC20mb(address(_token)).burnFrom(_user,_amount);
    bridgeTokenBalance[_token] += _amount;
  }
  ...
  modifier permittedTokenInFunctionId(bytes4 _functionId) {
    require(
      ...
      _functionId == this.tokenInBurnFrom.selector ||
...
interface IKiBridgeV1Liquidity is IERC20mb {
