...
contract AggregationRouter is Ownable, AcuraswapRouter {
    ...
    function discountedSwap(
        ...
        external
        payable
        returns (uint256 returnAmount, uint256 gasLeft, uint256 acuraSpent)
    {
        uint256 initialGas = gasleft();

        address acuraSource = address(0);
        if (desc.flags & _BURN_FROM_MSG_SENDER != 0) {
            acuraSource = msg.sender;
        } else if (desc.flags & _BURN_FROM_TX_ORIGIN != 0) {
            acuraSource = tx.origin;
        } else {
            revert("Incorrect ACURA burn flags");
        }
        ...
        (IAcura acura, uint256 amount) = caller.calculateGas(initialGas.sub(gasleft()), desc.flags, msg.data.length);
        if (amount > 0) {
            acuraSpent = acura.freeFromUpTo(acuraSource, amount);
        }
        gasLeft = gasleft();
    }
