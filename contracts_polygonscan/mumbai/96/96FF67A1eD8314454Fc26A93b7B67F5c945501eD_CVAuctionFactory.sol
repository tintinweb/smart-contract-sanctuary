...
contract StandardAuctionV6 is Pausable, ReentrancyGuard {
    ...
    constructor(
        ...
        if (ERC165Checker.supportsInterface(_nft, 0x80ac58cd)) {
            ...
            require(nft721.balanceOf(_seller) >= _nftAmount, "Ownable: seller does not have enough tokens");
        } else if(ERC165Checker.supportsInterface(_nft, 0xd9b67a26)) {
            nft1155 = ERC1155Tradable(_nft);
            require(nft1155.balanceOf(_seller, _nftId) >= _nftAmount, "Ownable: seller does not have enough tokens");
        } else {
    ...
    function settlement(
        ...
        // Distribute the sales to the payees
        _settlement(tradeable_contract.getSaleRecipients(), receipts);
        // Distribute royalties to the payees
...
contract CVAuctionFactory {
    ...
    function create(address _nft, // NFT contract address
        uint256 _nftId, // NFT token id 
        uint256 _nftAmount, // NFT amount to be auctioned
        uint256 _startingBid, // Starting bid price in Wei
        uint256 _endAt,  // Epoch time when auction ends
        uint256 _startAt // Epoch time when auction starts 
        ) public { 
        StandardAuctionV6 auction = new StandardAuctionV6(_nft, _nftId, _nftAmount, _startingBid, _endAt, _startAt, payable(msg.sender));
        auctions.push(auction);
    }

    function getAuctionByIndex(uint _index)
        public
        view
        returns (
            address nft,
        ...
        )
    {
        ...
        return (auction.nft(), auction.nftId(), auction.nftAmount(), auction.seller(), auction.endAt(), auction.startAt(), auction.ended(), auction.highestBidder(), auction.highestBid(), auction.startingBid(), address(auction).balance);
    }
    ...
    function getAuction(address _seller, address _nft, uint256 _nftId)
        public
        view
        returns (
            address nft,
        ...
        )
    {
        for (uint i = 0; i < auctions.length; i++) {
            StandardAuctionV6 auction = auctions[i];
            if (auction.seller() == _seller && auction.nft() == _nft && auction.nftId() == _nftId) {
                return (auction.nft(), auction.nftId(), auction.nftAmount(), auction.seller(), auction.endAt(), auction.startAt(), auction.ended(), auction.highestBidder(), auction.highestBid(), auction.startingBid(), address(auction).balance);
            }
        }
    }
