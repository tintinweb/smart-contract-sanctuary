...
contract CanvasManager is AccessControl, Ownable, Pausable, ICanvas, ReentrancyGuard {
    ...
    mapping(uint256 => IArtAuction) public artAuctions;

    PixNft public mPixNft; // pixel nft contract
    PixMoment public mPixMoment; // moment nft contract

    uint256 public mNftId = 0; // current count of arts
    IERC20[] public mTokens; // token array
    uint256[] public _basePrices; // the first price of draw pixel
    ...
    constructor(
        ...
        _curatorProxy = CuratorProxy(curatorProxy);
    }
    ...
    function endArt(uint256 nftId) external {
        ...
        require(block.timestamp >= art.endTime, "NE");
