...
import "./DreamStrategyV2.sol";

pragma solidity ^0.8.3;

contract DreamKingdomFactoryV2 is Ownable {
    event NewDreamStrategy(address indexed dreamStrategyAddress);
    ...
    function deployPool(
        ...
        bytes memory bytecode = type(DreamStrategyV2).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(_stakedToken, _rewardToken, _startBlock));
        address dreamStrategyAddress;

        assembly {
            dreamStrategyAddress := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }

        DreamStrategyV2(dreamStrategyAddress).initialize(
            _stakedToken,
            _rewardToken,
            _rewardPerBlock,
            _startBlock,
        ...
        poolInfo.push(PoolInfo({strategyAddress: dreamStrategyAddress}));
        emit NewDreamStrategy(dreamStrategyAddress);
    }
...
contract DreamStrategyV2 is Ownable, ReentrancyGuard, Gamificable {
    ...
    // View function to see if user can harvest reward token.
    function claimHaverst(address _user) external onlyOwner {
...
abstract contract Gamificable is Ownable, Pausable {
    ...
    //ChainLink Oracules
    AggregatorV3Interface internal oracle = AggregatorV3Interface(0xd0D5e3DB44DE05E9F294BB0a3bEEaF030DE24Ada); //matic usd
    ...
    function canClaimHarvest(address _user) public view returns (bool) {
        ...
        // si el precio no cambia nadie gana o ya reclamo el harvest
    ...
    function userIsPlayingNow(address _user) external view returns (bool) {
        ...
        if (ledger[_user].roundNumber > 0)
            if ((ledger[_user].roundNumber - _currentRound() ) == 0 )
                _status = true;
        return _status;
    }
    ...
    function userHasBid(address _user) external view returns (bool) {
        bool _status = false;
        if (ledger[_user].roundNumber > 0)
            if ((ledger[_user].roundNumber - ledger[_user].lastRoundHarvest ) == 0 )
                _status = true;
        return _status;
    }
    ...
    function _startRound(uint256 roundNumber, int256 price) internal {
        ...
        // emit StartRound(roundNumber, block.num);
    }
