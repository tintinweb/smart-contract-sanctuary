...
contract TT3S99 is ERC721URIStorage, Ownable, IERC2981Royalties {
    ...
    constructor(
        ...
        _royalties = RoyaltyInfo(owner(), 1000000000000000);
    }

    function getInternalId(uint256 tokenId)
        public
        view
        onlyOwner
        returns (uint256)
    ...
    /// @param value static amount in gwei
    function setRoyalties(uint256 value) public onlyOwner {
        _royalties = RoyaltyInfo(owner(), uint24(value));
    }
    ...
    function payWinner() public payable onlyOwner {
        ...
        uint256 winnerTokenId = _highScores[winnerId];

        _scoreIds[_mintedIds[winnerTokenId]] = 0;
        _nextScoreIds[_mintedIds[winnerTokenId]] = 0;

        payable(ownerOf(winnerTokenId)).transfer(address(this).balance);
    }
        ...
        returns (string memory)
    {
        ...
        uint256 _targetID = _mintedIds[1];
