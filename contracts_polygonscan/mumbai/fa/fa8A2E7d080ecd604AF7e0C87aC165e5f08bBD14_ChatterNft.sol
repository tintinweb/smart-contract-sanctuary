...
contract ChatterNft is ERC721Enumerable, Ownable {
    ...
    //constructor args
    constructor(string memory name_, string memory symbol_,  address _royaltyWallet,
        uint256 _royaltyFee, uint256 _price) ERC721(name_, symbol_) {
    ...
    // function priceInfo(uint256) external view returns (uint256) {
    //     return (price);
    // }
    ...
    function mint(string memory tokenURI) payable public {
        require(started, "not started");
        //require(_times > 0 && _times <= maxToMint, "You can only mint 10 at a time");
        //require(totalMints + _times <= totalCount, "Sold out!");
        require(msg.value == price, "Please send correct amount of ETH");
        
        payable(owner()).transfer(msg.value);
        emit Mint(_msgSender(), totalMints + 1, 1);
        uint256 newTokenId = totalMints++;
        //_safeMint(_msgSender(), newTokenId);
        for(uint256 i = 1; 1 >= i; i++){
