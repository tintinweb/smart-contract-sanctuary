...
contract BEP1155 is Context, BEP165, IBEP1155, IBEP1155MetadataURI {
    ...
    bytes4 private constant _INTERFACE_ID_BEP1155 = 0xd9b67a26;

    bytes4 private constant _INTERFACE_ID_BEP1155_METADATA_URI = 0x0e89341c;

    constructor (string memory name_, string memory symbol_) {
        _name = name_;
        _symbol = symbol_;

        _registerInterface(_INTERFACE_ID_BEP1155);
        _registerInterface(_INTERFACE_ID_BEP1155_METADATA_URI);
    }
    ...
    */    
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        require(_exists(tokenId), "BEP1155Metadata: URI query for nonexistent token");
    ...
    function balanceOf(address account, uint256 tokenId) public view override returns (uint256) {
        require(_exists(tokenId), "BEP1155Metadata: balance query for nonexistent token");
        ...
        returns (uint256[] memory)
    {
        ...
        for (uint256 i = 0; i < accounts.length; ++i) {
            require(accounts[i] != address(0), "BEP1155: batch balance query for the zero address");
    ...
    */
    ...
    /**
        ...
        After the above conditions are met, this function MUST check if `_to` is a smart contract (e.g. code size > 0). If so, it MUST call `onBEP1155Received` on `_to` and act appropriately (see "Safe Transfer Rules" section of the standard).
        @param from    Source address
        @param to      Target address
        @param tokenId      ID of the token type
        @param amount   Transfer amount
        @param data    Additional data with no specified format, MUST be sent unaltered in call to `onBEP1155Received` on `_to`
    */    
        ...
        override
    {
        ...
        _balances[tokenId][from] = _balances[tokenId][from].sub(amount, "BEP1155: insufficient balance for transfer");
    ...
    /**
        ...
        After the above conditions for the transfer(s) in the batch are met, this function MUST check if `_to` is a smart contract (e.g. code size > 0). If so, it MUST call the relevant `BEP1155TokenReceiver` hook(s) on `_to` and act appropriately (see "Safe Transfer Rules" section of the standard).
        @param from    Source address
        @param to      Target address
        ...
        @param data    Additional data with no specified format, MUST be sent unaltered in call to the `BEP1155TokenReceiver` hook(s) on `_to`
    */
    ...
    function safeBatchTransferFrom(
        ...
        override
    {
        require(tokenIds.length == amounts.length, "BEP1155: ids and amounts length mismatch");
        require(to != address(0), "BEP1155: transfer to the zero address");
        require(
            from == _msgSender() || isApprovedForAll(from, _msgSender()),
    ...
    */
    ...
    function _mint(uint256 tokenId, uint256 _supply, string memory _uri, uint256 _fee) internal {
        require(!_exists(tokenId), "BEP1155: token already minted");
    ...
    */    
    ...
    function _burn(address account, uint256 tokenId, uint256 amount) internal virtual {
        require(_exists(tokenId), "BEP1155Metadata: burn query for nonexistent token");
        ...
        _balances[tokenId][account] = _balances[tokenId][account].sub(
            amount,
            "BEP_holderTokens1155: burn amount exceeds balance"
        );
    ...
    */
    ...
    {
        if (to.isContract()) {
            try IBEP1155Receiver(to).onBEP1155Received(operator, from, tokenId, amount, data) returns (bytes4 response) {
    ...
    {
        if (to.isContract()) {
            try IBEP1155Receiver(to).onBEP1155BatchReceived(operator, from, tokenIds, amounts, data) returns (bytes4 response) {
...
contract Nebula1155v2 is BEP1155 {
    ...
    constructor (string memory name, string memory symbol, string memory tokenURIPrefix) BEP1155 (name, symbol) {
        owner = msg.sender;
