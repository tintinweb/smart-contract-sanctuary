...
interface IUserPositions {
     ...
     */
    function migrateUser(uint256 strategyId, MigrateStrategy[] calldata users) external;
}
...
interface IStrategyMap {
    ...
    event UpdateStrategy(uint256 indexed id, Integration[] integrations, Token[] tokens);
 ...
 */
    function increaseTokenBalance(uint256 id, IUserPositions.TokenMovement[] calldata tokens) external;
...
contract SushiSwapIntegrationV2 is Controlled, IAMMIntegration {
    ...
    function _deploy(uint32 poolId) internal {
        ...
        if (balanceA == 0 || balanceB == 0) {
            ...
            (, , liquidityAcquired) = ISushiSwapRouter(swapRouterAddress).addLiquidity(
                pool.tokenA,
                pool.tokenB,
                amountA,
                amountB,
    ...
    function getLiquidityToWithdraw(
        ...
        if (tokenAddress == pool.tokenA) {
            ...
            estimatedTokenBAmount = getAmountOut(tokenAddress, pool.tokenB, amount / 2);
        } else if (tokenAddress == pool.tokenB) {
            estimatedTokenAAmount = getAmountOut(tokenAddress, pool.tokenA, amount / 2);
            estimatedTokenBAmount = amount / 2;
        }

        uint256 liquidityPercent;

        if (estimatedTokenAAmount > amountOfTokenAInPool || estimatedTokenBAmount > amountOfTokenBInPool) {
        ...
        liquidityToWithdraw =
            (ISushiSwapMasterChefV2(masterChef).userInfo(stakings[poolId], address(this)).amount * (liquidityPercent)) /
            100;
    }
    ...
    function _withdraw(
        ...
        (uint256 amountOfTokenAInPool, uint256 amountOfTokenBInPool) = getTokensPoolValue(poolId);
    ...
    function getRewardTokenAmounts(uint32 poolId)
        external
        view
        returns (uint256[] memory amounts, address[] memory tokens)
    {
        ...
        if (address(pool.tokenA) != address(wethAddress)) {
            amounts[0] = getRewardTokenAmount(poolId, pool.tokenA);
            tokens[0] = pool.tokenA;
        }

        if (address(pool.tokenB) != address(wethAddress)) {
            amounts[amounts.length] = getRewardTokenAmount(poolId, pool.tokenB);
            tokens[tokens.length] = pool.tokenB;
        }
    }

    function getRewardTokenAmount(uint32 poolId, address token) internal view returns (uint256) {
        return IERC20MetadataUpgradeable(token).balanceOf(address(this)) - getBalance(poolId, token);
    }
    ...
    /// @dev only call this for pools that have an exclusive (non-weth) token :)
    function countExtraRewards(uint32 poolId) external onlyManager returns (uint256 extraRewards) {
        Pool memory pool = getPool(poolId);
        address token = (address(pool.tokenA) == wethAddress) ? pool.tokenB : pool.tokenA;

        extraRewards = _selfBalance(token) - getBalance(poolId, token);
        if (extraRewards > 0) {
            rewardBalances[poolId][token] += extraRewards;
        }
    }
}
...
contract SushiSwapIntegration is Controlled, IAMMIntegration {
    ...
    function getLiquidityToWithdraw(
        ...
        liquidityToWithdraw =
            (ISushiSwapMasterChef(masterChef).userInfo(stakings[poolId], address(this)).amount * (liquidityPercent)) /
            100;
    }
...
contract PolygonQuickSwapIntegration is Controlled, IAMMIntegration {
    ...
    function initialize(
        ...
        swapRouterAddress = swapRouterAddress_;
        slippageNumerator = slippageNumerator_;
        wmaticAddress = IIntegrationMap(moduleMap.getModuleAddress(Modules.IntegrationMap)).getWethTokenAddress();
    }
    ...
    function _configureStaking(
        ...
        require(IStakingRewards(stakingRewards).stakingToken() == pairAddress, "Incorrect staking address");

        stakings[poolId] = IStakingRewards(stakingRewards);

        // let staking contract pull tokens
        if (IERC20MetadataUpgradeable(pairAddress).allowance(address(this), stakingRewards) < type(uint256).max) {
            IERC20MetadataUpgradeable(pairAddress).safeApprove(stakingRewards, 0);
            IERC20MetadataUpgradeable(pairAddress).safeApprove(stakingRewards, type(uint256).max);
        }
    }
    ...
    function harvestYieldByPool(
        uint32 poolId,
        uint256 rewardsRatioX1000, // price ratio for rewards token!
        uint256, // unused sushi ratio
        bool convertToWmatic
    ) external onlyManager {
        address rewardsToken = stakings[poolId].rewardsToken();

        // check initial balances
        uint256 amountBefore = _selfBalance(wmaticAddress);
        uint256 rewardAmountBefore = _selfBalance(rewardsToken);

        // harvest
        stakings[poolId].getReward();
        uint256 yieldAmount = _selfBalance(wmaticAddress) - amountBefore;

        // check and convert
        uint256 rewardAmount = _selfBalance(rewardsToken) - rewardAmountBefore;

        if (convertToWmatic) {
            if (rewardAmount > 0) {
                checkPriceCeiling(rewardsToken, wmaticAddress, rewardsRatioX1000);
                uint256[] memory amounts = swapExactInput(
                    rewardsToken,
                    wmaticAddress,
                    address(this),
                    rewardAmount,
                    getAmountOutMinimum(rewardsToken, wmaticAddress, rewardAmount)
                );
                yieldAmount += amounts[1];
            }
        } else {
            balances[poolId][rewardsToken] += rewardAmount;
        }
        if (yieldAmount > 0) {
            yieldBalances[poolId] += yieldAmount;
        }
    ...
    function getLiquidityToWithdraw(
        ...
        liquidityToWithdraw = (stakings[poolId].balanceOf(address(this)) * (liquidityPercent)) / 100;
    }
    ...
    /// @dev Returns total amount of pending yield in SUSHI rewards
    /// @param poolId The pool to check for pending SUSHI rewards
    ...
    {
        return ([stakings[poolId].rewardsToken()], [stakings[poolId].earned(address(this))]);
    }
...
contract FantomSpookySwapIntegration is Controlled, IAMMIntegration {
    ...
    function getLiquidityToWithdraw(
        ...
        liquidityToWithdraw =
            (ISpookySwapMasterChef(masterChef).userInfo(stakings[poolId], address(this)).amount * (liquidityPercent)) /
            100;
    }
...
contract FantomSpiritSwapIntegration is Controlled, IAMMIntegration {
    ...
    function initialize(
        ...
        require(swapRouterAddress_ != address(0), "bad address");
        require(masterChef_ != address(0), "bad address");
        require(spirit_ != address(0), "bad address");
        __Controlled_init(controllers_, moduleMap_);
        factoryAddress = factoryAddress_;
        swapRouterAddress = swapRouterAddress_;
        masterChef = masterChef_;
    ...
    function harvestYieldByPool(
        uint32 poolId,
        uint256 spiritRatioX1000, // price ratio for spirit!
        bool convertToWeth
    ) external onlyManager {
        // MasterChefV1 does not have an explicit harvest method
        // deposit 0 accomplishes the rewards harvesting
        ...
        if (convertToWeth) {
            ...
            uint256 spiritBalance = IERC20MetadataUpgradeable(spirit).balanceOf(address(this));

            if (spiritBalance > 0) {
                checkPriceCeiling(spirit, wftmAddress, spiritRatioX1000);
                uint256[] memory amounts = swapExactInput(
                    spirit,
                    wftmAddress,
                    address(this),
                    spiritBalance,
                    getAmountOutMinimum(spirit, wftmAddress, spiritBalance)
                );
                yieldBalances[poolId] += amounts[1];
            }
        }
    ...
    function getLiquidityToWithdraw(
        ...
        liquidityToWithdraw =
            (ISpiritSwapMasterChef(masterChef).userInfo(stakings[poolId], address(this)).amount * (liquidityPercent)) /
            100;
    }
    ...
    function getPendingYield(address token, uint256 poolId) external view returns (uint256) {
        ...
        return ISpiritSwapMasterChef(masterChef).pendingSpirit(poolId, address(this));
    }
    ...
    function checkPriceCeiling(
        ...
        address pairAddress = address(pairFor(tokenA, tokenB));
        require(pairAddress != address(0), "SpiritSwapIntegration:BadPair");
...
contract YieldManager is Initializable, ModuleMapConsumer, Controlled, IYieldManager {
    ...
    function _ethToRewards(uint256 strategyId, uint256 ethRewardsAmount) private {
        if (ethRewardsAmount > 0) {
            IIntegrationMap integrationMap = IIntegrationMap(moduleMap.getModuleAddress(Modules.IntegrationMap));
            IWeth9 weth = IWeth9(integrationMap.getWethTokenAddress());
            ...
            weth.withdraw(ethRewardsAmount);
    ...
    ReentrancyGuardUpgradeable
{
    ...
    function updateTokenRewardWeight(address tokenAddress, uint256 updatedWeight) external onlyRole(MANAGER_ROLE) {
        IIntegrationMap(moduleMap.getModuleAddress(Modules.IntegrationMap)).updateTokenRewardWeight(
            tokenAddress,
            updatedWeight
        );

        emit TokenSettingUpdated(tokenAddress, TokenConfigs.TokenSettingName.rewardWeight, updatedWeight);
    }
    ...
    {
        IIntegrationMap(moduleMap.getModuleAddress(Modules.IntegrationMap)).updateTokenReserveRatioNumerator(
            tokenAddress,
            reserveRatioNumerator
        );
    ...
    function updateTokenTargetLiquidityRatioNumerator(address tokenAddress, uint256 targetLiquidityRatioNumerator)
        ...
        onlyRole(MANAGER_ROLE)
    {
        IIntegrationMap(moduleMap.getModuleAddress(Modules.IntegrationMap)).updateTokenTargetLiquidityRatioNumerator(
            tokenAddress,
            targetLiquidityRatioNumerator
        );
        emit TokenSettingUpdated(
    ...
    function updateTokenTransferFeeKValueNumerator(address tokenAddress, uint256 transferFeeKValueNumerator)
        ...
        onlyRole(MANAGER_ROLE)
    {
        IIntegrationMap(moduleMap.getModuleAddress(Modules.IntegrationMap)).updateTokenTransferFeeKValueNumerator(
            tokenAddress,
            transferFeeKValueNumerator
        );
    ...
    function updateTokenTransferFeePlatformRatioNumerator(
        ...
        IIntegrationMap(moduleMap.getModuleAddress(Modules.IntegrationMap))
            .updateTokenTransferFeePlatformRatioNumerator(tokenAddress, transferFeePlatformRatioNumerator);
    ...
    function withdraw(
        ...
        if (ethWithdrawn > 0) {
            IWeth9(IIntegrationMap(moduleMap.getModuleAddress(Modules.IntegrationMap)).getWethTokenAddress()).withdraw(
                ethWithdrawn
            );
...
contract UserPositions is Initializable, ModuleMapConsumer, Controlled, IUserPositions {
    ...
    function deposit(
        ...
        for (uint256 tokenId; tokenId < tokens.length; tokenId++) {
            ...
            if (migration) {
                ...
                IBiosRewards(moduleMap.getModuleAddress(Modules.BiosRewards)).increaseRewards(
                    tokens[tokenId],
                    depositor,
                    actualAmount
                );
    ...
    function _withdraw(
        ...
        for (uint256 tokenId; tokenId < tokens.length; tokenId++) {
            ...
            // Decrease rewards
            IBiosRewards(moduleMap.getModuleAddress(Modules.BiosRewards)).decreaseRewards(
                tokens[tokenId],
                recipient,
                actualTransferAmount
            );
    ...
    function enterStrategy(uint256 strategyID, TokenMovement[] calldata tokens) external override {
    ...
    function exitStrategy(uint256 strategyID, TokenMovement[] calldata tokens) external override {
    ...
    function migrateUser(uint256 strategyId, MigrateStrategy[] calldata users) external override onlyController {
        IStrategyMap strategyMap = IStrategyMap(moduleMap.getModuleAddress(Modules.StrategyMap));
...
contract StrategyMap is Initializable, ModuleMapConsumer, Controlled, IStrategyMap {
    ...
    function updateStrategy(
        ...
        for (uint256 i = 0; i < currentStrategy.tokens.length; i++) {
            deployAmount[currentStrategy.integrations[currentStrategy.tokens[i].integrationPairIdx].integration][
                currentStrategy.integrations[currentStrategy.tokens[i].integrationPairIdx].ammPoolID
            ][currentStrategy.tokens[i].token] -= int256(
                (strategyTokenBalances[id][currentStrategy.tokens[i].token] * currentStrategy.tokens[i].weight) /
                    TOKEN_WEIGHT
            );
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            if (strategyTokenBalances[id][tokens[i].token] > 0) {
                deployAmount[integrations[tokens[i].integrationPairIdx].integration][
                    ...
                    (strategyTokenBalances[id][tokens[i].token] * tokens[i].weight) / TOKEN_WEIGHT
                );
            }
        }

        _verifyAndSetStrategy(id, currentStrategy.name, integrations, tokens, maxCap);
    ...
    function increaseTokenBalance(uint256 id, IUserPositions.TokenMovement[] calldata tokens)
        external
        override
        onlyController
    {
...
contract AvalancheTraderJoeIntegration is Controlled, IAMMIntegration {
    ...
    function initialize(
        ...
        require(swapRouterAddress_ != address(0), "bad address");
        require(masterChef_ != address(0), "bad address");
        require(joe_ != address(0), "bad address");
        __Controlled_init(controllers_, moduleMap_);
        factoryAddress = factoryAddress_;
        swapRouterAddress = swapRouterAddress_;
        masterChef = masterChef_;
        slippageNumerator = slippageNumerator_;
        joe = joe_;
        wavaxAddress = IIntegrationMap(moduleMap.getModuleAddress(Modules.IntegrationMap)).getWethTokenAddress();
    }
    ...
    function _deploy(uint32 poolId) internal {
        ...
        if (balanceA == 0 || balanceB == 0) {
            ...
            (, , liquidityAcquired) = IJoeRouter(swapRouterAddress).addLiquidity(
                pool.tokenA,
                pool.tokenB,
                amountA,
                amountB,
    ...
    function harvestYield() external override onlyController {
        ...
        if (yieldAmount > 0) {
            IERC20MetadataUpgradeable(wavaxAddress).safeTransfer(
                moduleMap.getModuleAddress(Modules.YieldManager),
                yieldAmount
            );
            emit YieldReceived(yieldAmount);
    ...
    function harvestYieldByPool(
        uint32 poolId,
        uint256 joeRatioX1000, // price ratio for SUSHI!
        bool convertToWeth
    ) external onlyManager {
        // MasterChefV1 does not have an explicit harvest method
        // deposit 0 accomplishes the rewards harvesting
        ...
        if (convertToWeth) {
            ...
            uint256 joeBalance = IERC20MetadataUpgradeable(joe).balanceOf(address(this));

            if (joeBalance > 0) {
                checkPriceCeiling(joe, wavaxAddress, joeRatioX1000);
                uint256[] memory amounts = swapExactInput(
                    joe,
                    wavaxAddress,
                    address(this),
                    joeBalance,
                    getAmountOutMinimum(joe, wavaxAddress, joeBalance)
                );
                yieldBalances[poolId] += amounts[1];
            }
        }
    ...
    function getLiquidityToWithdraw(
        ...
        liquidityToWithdraw =
            (IJoeMasterChef(masterChef).userInfo(stakings[poolId], address(this)).amount * (liquidityPercent)) /
            100;
    }
    ...
    function getPendingYield(address token, uint256 poolId) external view returns (uint256) {
        ...
        return IJoeMasterChef(masterChef).pendingTokens(poolId, address(this));
    }
...
contract TestIntegration is ModuleMapConsumer, Controlled, IIntegration {
    ...
    /// @dev using the relevant integration's own function
    /// @param amount The amount of available yield for the specified token
    function getPendingYield(address token) external view override returns (uint256 amount) {
        require(true, "Not relevant to testing");
        return 42069;
    }
}
...
contract StrategyManager is Initializable, ModuleMapConsumer, Controlled, IStrategyManager {
    ...
    function updateStrategy(
        ...
        IStrategyMap(moduleMap.getModuleAddress(Modules.StrategyMap)).updateStrategy(id, integrations, tokens, maxCap);
    }
...
contract Interconnects is Initializable, ModuleMapConsumer, Controlled, IInterconnects, ReentrancyGuardUpgradeable {
        ...
        nonReentrant
    {
        ...
        // if they are entering a strategy remotely, we want to decrease their entry amount by the fees
        if (req.enterStrat) {
            for (uint256 i; i < req.movements.length; i++) {
                req.movements[i].amount -= fees[i];
        ...
        // if they are exiting a strategy remotely, we want to decrease their local balance by the fees
        // this needs to be done after the above RSE call because that will update their local balances accordingly
        if (!req.enterStrat) {
...
contract IntegrationMap is Initializable, ModuleMapConsumer, Controlled, IIntegrationMap {
    ...
    function initialize(
        ...
        _addToken(wethTokenAddress_, true, true, true, true, 1_000, 50_000, 100_000, 40_000, 50_000);
        _addToken(biosTokenAddress_, true, true, true, true, 1_000, 0, 100_000, 40_000, 50_00);
    }
...
interface IDynamicRangeOrdersIntegration is IIntegration {
    ...
    function setDynamicRangeOrdersIntegrationDeployer(address dynamicRangeOrdersIntegrationDeployerAddress) external;
