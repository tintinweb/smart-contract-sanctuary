...
abstract contract ABase is IBase, AStorage {
    ...
    modifier onlyOwner(
        ...
        require(containsOwnerAddress(addresses) && containsVAddress(addresses), "ABase::onlyOwner - must include owner and Vesto signatures");
        _;
    }
    ...
    modifier onlyRecovery(
        ...
        address[] memory addresses = validateSignatures(signatures);
        require(containsRecoveryAddress(userId, addresses) && containsVAddress(addresses), "ABase::onlyRecovery - must include recovery and Vesto signatures");
        _;
    }
    ...
    modifier onlyVesto(
        ...
        address[] memory addresses = validateSignatures(signatures);
        require(containsVAddress(addresses), "ABase::onlyVesto - must include Vesto signature");
        _;
    }
    ...
    modifier onlyAdmin(
        ...
        address[] memory addresses = validateSignatures(signatures);
        require(containsAdminAddresses(addresses, _userId) && containsVAddress(addresses), "ABase::onlyAdmin - must include admin and Vesto signatures");
        _;
    }
    ...
    modifier onlySuperAdmin(
        ...
        address[] memory addresses = validateSignatures(signatures);
        require(containsSuperAdminAddresses(addresses) && containsVAddress(addresses), "ABase::onlySuperAdmin - must include super admin and Vesto signatures");
        _;
    }
    ...
    modifier onlyVErc20(
        ...
        require(vLighthouse().isVErc20(_address), "ABase::onlyVErc20 - only vErc20 addresses supported");
        _;
    }
    ...
    modifier onlyVWallet(
        ...
        require(vLighthouse().isVWallet(_address), "ABase::onlyVWallet - only vWallet addresses supported");
        _;
    }

    modifier onlyEthereum(
    ) {
        require(vLighthouse().chainId() == ETHEREUM_MAINNET || vLighthouse().chainId() == ETHEREUM_GOERLI, "ABase::onlyEthereum - function only supported on Ethereum Goerli or Mainnet");
        _;
    }

    modifier onlyPolygon(
    ) {
        require(vLighthouse().chainId() == POLYGON_MUMBAI || vLighthouse().chainId() == POLYGON_MAINNET, "ABase::onlyPolygon - function only supported on Polygon");
        _;
    }
    ...
    function validateSignatures(
        ...
        require((end >= block.timestamp) && (begin < block.timestamp), "ABase::validateSignatures - invalid timestamps");
        require(!isThisNonceUsed(signatures.nonce), "ABase::validateSignatures - possible replay attack");
        ...
        for (uint i = 0; i < addresses.length; i++) {
            ...
            require(csv == 27 || csv == 28, "ABase::validateSignatures - invalid signature version");
    ...
    function spliceTimestamps(
        ...
        require(_bytes.length == 56, "ABase::spliceTimestamps - invalid nonce length");
