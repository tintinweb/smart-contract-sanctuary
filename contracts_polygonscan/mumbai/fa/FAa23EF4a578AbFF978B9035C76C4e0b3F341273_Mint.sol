...
contract Mint is Ownable, Pausable {
    ...
    mapping(uint256 => uint256) private tokenSoldPrices;
    ...
    function buyToken(uint256 tokenId) public whenNotPaused {
        ...
        saveTokenPrice(tokenId, tokenPrice);
        _landsSoldCounter++;
    }
    ...
    function buyTokens(uint tokensNumber) public whenNotPaused {
        ...
        uint256[] memory tokenIds = _nftsContract.mintTokens(_msgSender(), tokensNumber);
    ...
    function saveTokenPrices(uint256[] memory tokenIds, uint256[] memory prices) internal {
        for (uint i = 0; i < tokenIds.length; i++) {
            saveTokenPrice(tokenIds[i], prices[i]);
        }
    }

    function getTokenSoldPrice(uint256 tokenId) public view returns(uint256) {
