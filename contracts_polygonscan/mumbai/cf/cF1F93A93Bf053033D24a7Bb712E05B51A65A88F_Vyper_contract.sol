...
def updateInterest():
    ...
    reward_per_seconds: uint256 = (current_atoken_balance - self.lastAtokenBalance) / (block.timestamp - self.lastRewardCalcTimeStamp)
    self.accumulatedRewardPerShare = self.accumulatedRewardPerShare + reward_per_seconds/self.totalContractBalance    
...
def fxsell(withdraw: uint256) -> bool:
    ...
    interest_on_amount: uint256 = self.accumulatedRewardPerShare * withdraw
    log Bugger(interest_on_amount, 'interest on withdrawal')
    usd_amount_plus_interest: uint256 = usd_amount + ((interest_on_amount / self._buyrate) / 10**2)
