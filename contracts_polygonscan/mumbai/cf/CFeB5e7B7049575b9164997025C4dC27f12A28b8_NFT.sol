...
contract NFT is Ownable, ERC721Enumerable{
    ...
    // pillem els strings, així puc convertir l'ID del token que es un uint en un string 
    using Strings for uint256;
    
    // supply màxim
    uint _maxSupply = 31;
     
    // crido el construcotor OpenZeppelin i declaro el nom i el síbmol del puto token, i col·loco la base URI
    constructor(string memory _initialURI) ERC721("Test31", "T31") {
        setBaseURI(_initialURI);
    }
    
    // l'owner és l'únic que pot mintart
    function safeMint(uint _ammount) public onlyOwner{
        require ((totalSupply()+_ammount) <= _maxSupply, "No hi ha prou supply, Pallasso");
