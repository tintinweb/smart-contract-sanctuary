...
contract NFT is ERC721URIStorage, Ownable {
    ...
    event NFTMintersWhitelistChanged(address indexed paymentToken, bool allowance);
    ...
    constructor(address marketplaceAddress) ERC721("ASPO NFTs", "ASPO Item") {
        ...
        if (tokenCount == 0) {
            ...
            for (aspoId = 1; aspoId <= totalAspo; aspoId++) {
                if (ownerOf(aspoId) == ownerAddress) {
                    ...
                    resultIndex++;
                    if (resultIndex == tokenCount) break; // break to save gas if possible
                }
            }
