...
contract Controller is ControllerStorage, Rewards, ControllerErrorReporter, Ownable, ReentrancyGuard, NonZeroAddressGuard {
    ...
    event PoolCreated(address indexed pool, address indexed owner, address stableCoin, uint256 minDeposit, uint8 structure);
    ...
    event LenderWhitelisted(address lender, address pool);
    ...
    function requestPoolWhitelist(address pool) external nonZeroAddress(pool) returns (uint256) {
        BorrowerPool storage borrowerPool = pools[pool];

        if(borrowerWhitelists[borrowerPool.owner][msg.sender]) {
            return uint256(Error.NO_ERROR);
        }

        borrowerWhitelists[borrowerPool.owner][msg.sender] = true;
        borrowerPool.members[msg.sender] = true;

        emit LenderWhitelisted(pool, msg.sender);
        return uint256(Error.NO_ERROR);
    }

    function createPool(string memory name, uint256 minDeposit, address stableCoin, Pool.Structure structureType) external nonReentrant {
        address pool = Clones.createClone(_poolLibrary);

        Pool _poolI = Pool(pool);
        require(!_poolI.isInitialized(), "pool already initialized");
        _poolI.initialize(msg.sender, stableCoin, name, minDeposit, structureType);

        BorrowerPool storage _pool = pools[pool];
        _pool.owner = msg.sender;
        _pool.isActive = true;
        _pool.members[msg.sender] = true;

        borrowerPools[msg.sender].push(pool);

        // reward variables
        rewardPools.push(pool);
        rewardsState[pool] = State(true, 0, 0, 0, 0);

        emit PoolCreated(pool, msg.sender, stableCoin, minDeposit, uint8(structureType));
    }
    ...
    function whitelistBorrower(address borrower, uint256 debtCeiling, uint256 rating) external onlyOwner returns (uint256) {
        ...
        if (_borrower.whitelisted) {
            return uint256(Error.BORROWER_ALREADY_WHITELISTED);
        }
    ...
    function blacklistBorrower(address borrower) external onlyOwner returns (uint256) {
        ...
        require(_borrower.created, toString(Error.BORROWER_NOT_CREATED));
        if (!_borrower.whitelisted) {
            return uint256(Error.BORROWER_IS_WHITELISTED);
        }
    ...
    function lendAllowed(address pool, address lender, uint256 amount) external returns (uint256) {
        ...
        if (
            Pool(pool).structureType() == uint8(Pool.Structure.Private) &&
    ...
    function borrowAllowed(address pool, address borrower, uint256 amount) external returns (uint256) {
        ...
        /* Check if borrower is member of the pool */
        if (!_pool.members[borrower]) {
    ...
    function createCreditLineAllowed(address pool, address borrower, uint256 collateralAsset) external virtual nonReentrant returns (uint256) {
        ...
        // Check if borrower is whitelisted
        require(borrowers[borrower].whitelisted, toString(Error.BORROWER_NOT_WHITELISTED));
    ...
    // Internal function
    function isAllowed(address _owner) internal nonReentrant {
...
abstract contract ControllerStorage {
    ...
    mapping(address => BorrowerPool) public pools;
...
abstract contract Rewards is ExponentialDouble {
    ...
    mapping(address => State) public rewardsState;
    ...
    function getBorrowReward(address account, address pool) external view returns (uint256) {
        ...
        return getBorrowerAcrrueAmount(pool, account, poolIndex);
    }
    ...
    function getSupplyReward(address account, address pool) external view returns (uint256) {
        ...
        return getSupplierAcrrueAmount(pool, account, poolIndex);
    }
    ...
    function getNewBorrowIndex(address pool) internal view returns (uint256) {
        ...
        State storage state = rewardsState[pool];

        vars.speed = amptPoolSpeeds[pool];
        (, vars.totalPrincipal) = getPoolInfo(pool);
        vars.blockNumber = getBlockNumber();
        vars.deltaBlocks = sub_(vars.blockNumber, state.borrowBlockNumber);
        if (vars.deltaBlocks > 0 && vars.speed > 0) {
            ...
            vars.index = add_(Double({mantissa: state.borrowIndex }), vars.ratio);
    ...
    function getNewSupplyIndex(address pool) internal view returns (uint256) {
        ...
        vars.blockNumber = getBlockNumber();
        vars.deltaBlocks = sub_(vars.blockNumber, state.supplyBlockNumber);
        if (vars.deltaBlocks > 0 && vars.speed > 0) {
            vars.tokensAccrued = mul_(vars.deltaBlocks, vars.speed / 2); 
            vars.ratio = vars.totalSupply > 0 ? fraction(vars.tokensAccrued, vars.totalSupply) : Double({mantissa: 0});
            vars.index = add_(Double({mantissa: state.supplyIndex }), vars.ratio);
            
            return vars.index.mantissa;
        }

        return state.supplyIndex;
    }
    ...
    function getBorrowerAcrrueAmount(address pool, address holder, uint poolIndex) internal view returns (uint256) {
        BorrowerAccruedLocalVars memory vars;
        Reward storage state = borrowerState[holder][pool];
    ...
    function getSupplierAcrrueAmount(address pool, address holder, uint poolIndex) internal view returns (uint) {
...
contract Pool is Ownable, Lendable, Borrowable {
    ...
    uint8 public structureType;

    event StructureChanged(uint8 newStructureType);

    constructor() {}

    function initialize(address _admin, address _stableCoin, string memory _name, uint256 _minDeposit, Structure _structureType) external {
        _initialize(_admin, _stableCoin, _name, _minDeposit, _structureType);
    }

    function _initialize(address _admin, address _stableCoin, string memory _name, uint256 _minDeposit, Structure _structureType) internal nonReentrant {
        isInitialized = true;

        name = _name;
        minDeposit = _minDeposit;
        structureType = uint8(_structureType);
    ...
    function updateStructure(Structure _structureType) external onlyOwner {
        structureType = uint8(_structureType);
        emit StructureChanged(structureType);
    }
