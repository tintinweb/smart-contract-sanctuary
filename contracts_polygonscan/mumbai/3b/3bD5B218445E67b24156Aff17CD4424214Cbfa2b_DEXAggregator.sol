...
// File contracts/interface/IUnoswapRouter.sol
...
contract DEXAggregator is Ownable {
    ...
    function isNativeToken(IERC20 inputToken) internal pure returns (bool){
        return (address(inputToken) == 0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE) ? true : false;
    }

    function swap(
        IUnoswapRouter[] calldata router,
        address[][] calldata path,
        ...
        if (
            ...
            returnAmounts = router[0].swapExactETHForTokens{value: msg.value}(
                amountOutMin[0],
                path[0],
                address(this),
        ...
        else if(!isNativeToken(inputToken) || router.length > 1)
        {   
            ...
            returnAmounts = router[i].swapExactTokensForTokens(
                amountIn[i],
                amountOutMin[i],
                path[i],
                address(this),
