...
contract StakingPool is IStakingPool, Ownable {
    ...
    mapping(uint => uint) public stakingSubHero;
    ...
    /**
     * @dev Check a hero is current staking or not
     */
    function isCurrentStaking(address account, uint heroId) external override view returns (bool) {
        require(heroId < kabyHeroContract.totalSupply(), "StakingPool: invalid hero");
        if(_stakingHeros[account].contains(heroId))
            return true;
        else return false;
    }

    /**
     * @dev Stake Kaby token for upgrade hero star and receive reward
     * @param mainHeroId id of hero want to upgrade star.
     * @param subHeroId id of second hero to stake.
     * @param amount amount of kabyToken to stake.
     */
    function stakeForUpgradeStar(uint mainHeroId, uint subHeroId, uint amount) external override {
        require(mainHeroId < kabyHeroContract.totalSupply() && subHeroId < kabyHeroContract.totalSupply(), "StakingPool: invalid hero");
        require(kabyHeroContract.isOnSale(mainHeroId) == false && kabyHeroContract.isOnSale(subHeroId) == false, "StakingPool: not allow staking hero is onsale");
        require(kabyHeroContract.getHeroStar(mainHeroId) <= kabyHeroContract.getHeroStar(subHeroId) && mainHeroId != subHeroId, "StakingPool: invalid sub hero");
        ...
        require(!_stakingHeros[account].contains(mainHeroId) && !_stakingHeros[account].contains(subHeroId), "StakingPool: Hero is staking for upgrade star");
        stakingSubHero[mainHeroId] = subHeroId;
        _stakingHeros[account].add(subHeroId);
        _stake(account, mainHeroId, amount, StakingType.upgradeStar);
    }

    /**
     * @dev Stake Kaby token for receive reward
     * @param heroId id of hero want to upgrade star
     * @param amount amount of kabyToken to stake.
     */
    function stakeForReceiveReward(uint heroId, uint amount) external override{
        require(heroId < kabyHeroContract.totalSupply(), "StakingPool: invalid hero");
        require(kabyHeroContract.isOnSale(heroId) == false, "StakingPool: not allow staking hero is onsale");
        address account = msg.sender;
        require(!_stakingHeros[account].contains(heroId), "StakingPool: Hero is staking for upgrade star");
        _stake(account, heroId, amount, StakingType.noLockTime);
    }
    ...
    function upgradeStarForHero(uint heroId) external override {
        require(heroId < kabyHeroContract.totalSupply(), "StakingPool: invalid hero");
        address account = msg.sender;
        require(_stakingHeros[account].contains(heroId), "StakingPool: Hero is not staking for upgrade star");
        StakingData storage stakingHero = stakingData[account][heroId];
        _harvest(heroId, account);
        require(canUpStar[heroId], "StakingPool: Cannot upgrade star for this hero");
        kabyHeroContract.upgradeStar(heroId, 1);
        _stakingHeros[account].remove(stakingSubHero[heroId]);
    ...
    /**
     * @dev Check and update status of upgradeStar for hero and amount of reward
     * @param heroId id of hero.
     * @param account wallet address of user
     */
    ...
    /**
     * @dev Internal function of Stake to calculate and store info of staking when user stake
     */
    ...
    /**
     * @dev Internal function of Unstake to update info of staking when user unstake
     */
...
//SPDX-License-Identifier: MIT
import "./IBaseStakingPool.sol";

pragma solidity ^0.8.0;

interface IStakingPool is IBaseStakingPool {
     ...
     */
    function stakeForUpgradeStar(uint mainHeroId, uint subHeroId, uint amount) external;
    
    /**
     * @notice Stake KABY token for receive reward.
     */
    function stakeForReceiveReward(uint heroId, uint amount) external;
