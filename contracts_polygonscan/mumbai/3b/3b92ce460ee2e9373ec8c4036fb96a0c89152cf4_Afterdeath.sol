...
contract Afterdeath is IAfterdeath, IDescriptorUser, Initializable {
  ...
  /*
    ...
    because they are still waiting to resurrection.
  */
  ...
  function getMorgue() external view override returns(uint[] memory) {
    return _morgue;
  }  

  function getSanctuary() external view override returns(uint[] memory) {
    return _sanctuary;
  }
  ...
  */
  function bury(uint requestedBurials) external override {
    ...
    for (uint i = 0; i < maxBurials; i++) {
      uint tokenId = _morgue[_morgue.length - 1];
      Descriptor memory d = _state.getDescriptor(tokenId);
      // decide what we do with token before we pop it out of morgue
      if (_snook.isLocked(tokenId) == true && d.deathTime > 0 && d.deathTime + _BurialDelayInSeconds * 1 seconds < block.timestamp) {
        // tokens to burn
        burials += 1;
        _snook.lock(tokenId, false, 'bury');
        _snook.burn(tokenId);
        _state.deleteDescriptor(tokenId);
      } else if (_snook.isLocked(tokenId) == true && d.deathTime > 0 && d.deathTime + _BurialDelayInSeconds * 1 seconds >= block.timestamp) { 
        // tokens which are in resurrection waiting state
        _sanctuary.push(tokenId);
      } else {
  ...
  function getBodyNumber() external override view returns (uint) {
