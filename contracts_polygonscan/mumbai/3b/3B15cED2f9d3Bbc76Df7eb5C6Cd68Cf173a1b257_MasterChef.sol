...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    // The timestamp when token Harvest countdown starts.
    ...
    function payOrLockupPendingToken(uint256 _pid, bool _compound) internal returns (uint256 totalRewards) {
        ...
        if (user.nextHarvestUntil == 0) {
            // fair harvest start for every one, in case compound just add half harvestInterval
