...
contract NFT is ERC721("Biconomy NFT", "BNFT"), AccessProtected {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds; // nft token ids
    mapping(uint256 => uint8) private tokenNftTypes; // tokenId => nftType

    /**
     * Mint + Issue NFTF
     *
     * @param recipient - NFT will be issued to recipient
     * @param data - NFT Metadata URI/Data
    ...
    function issueToken(
        ...
        require(nftType != 0, "Nft Type must be greater than 0");
        _tokenIds.increment();
        uint256 newTokenId = _tokenIds.current();
        _safeMint(recipient, newTokenId);
        _setTokenURI(newTokenId, data);
        tokenNftTypes[newTokenId] = nftType;
        return newTokenId;
    }

    /**
     * Fetch Nft Type for a Given Token Id
     *
     * @param _tokenId - Token Id which needs to be burnt
     */
    function getNFTTypeForGivenTokenId(uint256 _tokenId) external view returns (uint8) {
        return tokenNftTypes[_tokenId];
    }
}
