...
contract ProtocolControl is AccessControlEnumerable {
    ...
    /// @dev Returns all info about added modules
    function getAllModules(uint256 maxType) external view returns (Module[] memory allModules) {
        allModules = new Module[](totalModules);

        uint256 index = 0;

        for (uint256 j = 0; j < maxType; j++) {
            uint256 numOfModules = numOfModuleType[j];

            for (uint256 i = 0; i < numOfModules; i += 1) {
                bytes32 moduleId = keccak256(abi.encodePacked(i, j));
                allModules[index] = Module(moduleId, j, modules[moduleId]);
    ...
    /// @dev withdraw funds collected, only callabale by protocol admin
    function withdrawFunds(address to, address currency) external onlyProtocolAdmin {
        ...
        if (currency == address(0)) {
            ...
            _currency.safeTransfer(registryTreasury, registryTreasuryFee);
