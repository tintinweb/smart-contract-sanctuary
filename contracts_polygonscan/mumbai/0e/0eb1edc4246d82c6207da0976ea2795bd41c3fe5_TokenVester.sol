...
contract TokenVester {
    ...
    function claim() public {
        require(block.timestamp >= vestingCliff, 'Claim: not time yet');
        uint256 amount;
        if (block.timestamp >= vestingEnd) {
            amount = IERC20(wirtual).balanceOf(address(this));
        } else {
            amount = vestingAmount * (block.timestamp - lastUpdate) / (vestingEnd - vestingBegin);
        }
        lastUpdate = block.timestamp;
        IERC20(wirtual).transfer(recipient, amount);
    }
}