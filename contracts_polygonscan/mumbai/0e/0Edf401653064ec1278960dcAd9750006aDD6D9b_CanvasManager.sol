...
contract CanvasManager is AccessControl, Ownable, Pausable, ICanvas, ReentrancyGuard {
    ...
    uint8 internal _defaultCurrencyDecimals; // decimals of default auction currency
    ...
    {
        return (_defaultAuctionDuration, _defaultAuctionCurrency, _defaultCurrencyDecimals);
    }
    ...
    function setDefaultAuctionInfo(
        ...
        _defaultCurrencyDecimals = decimals;
    }
    ...
    function claimeArtAuctionFee(uint256 nftId, address contributor) external nonReentrant {
        ...
        (uint256 thisShares, uint256 allShares) = calcShares(nftId, contributor);

        IArtAuction artAuction = _getArtAuction(nftId);
        artAuction.claimeArtAuctionFee(contributor, thisShares, allShares);
    }
    ...
    function addConsumeToken(IERC20 token, uint256 basePrice) external {
        onlyAdmin();
        require(basePrice > 0, "ACT");
    ...
    function setConsumeToken(
        uint256 index,
        IERC20 token,
        uint256 basePrice
    ) external {
        onlyAdmin();
        require(index < mTokens.length && basePrice > 0, "SCT");
        mTokens[index] = token;
    ...
    /// @return thisShares shares of contributor in this PixNft
    /// @return allShares all shares of contributor from first PixNft to this PixNft(include this PixNft)
    function calcShares(uint256 nftId, address contributor) public view returns (uint256 thisShares, uint256 allShares) {
        checkNftId(nftId);
    ...
    function _endArt(uint256 nftId) internal {
        ...
        if (art.moments.length > 0) {
            ...
            artAuction.createArtAuction(_defaultAuctionDuration, 10**_defaultCurrencyDecimals);
        }
...
contract ZoraAuction is IArtAuction, IERC721Receiver, Ownable {
    ...
    uint256 internal immutable _thisNftShares; // moment count of this NFT
    uint256 internal immutable _allNftShares; // total moment count of all NFTs before this NFT
    ...
    constructor(
        ...
        _allNftShares = allNftShares;
        _thisNftShares = thisNftShares;
        _nftId = nftId;
    ...
    function createArtAuction(uint256 duration, uint256 reservePrice) external onlyOwner auctionNotStarted returns (uint256) {
        require(duration > 0, "CAA1");
        require(reservePrice > 0, "CAA2");
     ...
     * @param contributor the address of receiver
     * @param thisSharesOfContributor the contribution in this NFT
     * @param allSharesOfContributor the contribution in all NFT(include this NFT)
     */
    function claimeArtAuctionFee(
        address contributor,
        uint256 thisSharesOfContributor,
        uint256 allSharesOfContributor
    ) external onlyOwner {
        require(thisSharesOfContributor <= allSharesOfContributor, "CF1");
        require(thisSharesOfContributor <= _thisNftShares, "CF2");
        require(allSharesOfContributor <= _allNftShares, "CF3");
        address tokenOwner = IERC721(_nftContract).ownerOf(_nftId);
        ...
        uint256 paymentSum = (thisArtFee * thisSharesOfContributor) / _thisNftShares + (allArtsFee * allSharesOfContributor) / _allNftShares;
        ...
        emit ClaimeArtAuctionFee(address(this), _msgSender(), contributor, thisSharesOfContributor, allSharesOfContributor, paymentSum, pendingPay);
    }
