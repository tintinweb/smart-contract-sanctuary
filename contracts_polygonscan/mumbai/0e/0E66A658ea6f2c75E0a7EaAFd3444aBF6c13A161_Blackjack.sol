...
contract Blackjack is Ownable {
    ...
    // nonce for client seed generation
    uint256 internal nonce;

    // instance of xLLTH token
    IxLLTH internal _xLLTH;

    // mapping to player's client seed
    mapping(address => bytes32) public clientSeeds;

    // mapping to player's bets
    mapping(address => uint256) public bets;

    // emitted when client seed has been generated
    event ClientSeedGenerated(address player, bytes32 clientSeed);

    //-------------------------------------------------------------------------
    // CONSTRUCTOR
    //-------------------------------------------------------------------------

    constructor(address xLLTH_) {
    ...
    function _generateClientSeed(address player) internal returns (bytes32) {
        return keccak256(abi.encodePacked(address(player), uint256(++nonce)));
    }
    ...
    function deal() external {
        bytes32 clientSeed = _generateClientSeed(msg.sender);

        emit ClientSeedGenerated(msg.sender, clientSeed);
    }
