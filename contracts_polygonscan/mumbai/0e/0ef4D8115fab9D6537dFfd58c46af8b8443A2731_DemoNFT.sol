  ...
  IDemoRoyalty
{
  ...
  {
    return
      _interfaceId == type(IDemoNFT).interfaceId ||
      _interfaceId == type(IDemoCreator).interfaceId ||
      _interfaceId == type(IDemoRoyalty).interfaceId ||
  ...
  function burn(uint256 _id) external override {
    require(ownerOf(_id) == _msgSender() || hasRole(BURNER_ROLE, _msgSender()), "DemoNFT:01");
    tokenInfo[_id].owner = address(0);
    _burn(_id);
  }

  function setTokenOwner(address _account, uint256 _id) external override {
    require(_msgSender() == marketplace, "DemoNFT:05");
  ...
  function setRoyaltyPercentage(uint256 _id, uint256 _royaltyPercentage) external override {
    require(creators[_id] == msg.sender || hasRole(ASSIGNER_ROLE, _msgSender()), "DemoNFT:02");
    royalties[_id] = _royaltyPercentage;
  }
    ...
    return
      _hashTypedDataV4(
        keccak256(
          abi.encode(
            keccak256(
              "DemoNFT(address _creator,uint256 _id,uint256 _royaltyFeePercentage,uint256 _priceType,uint256 _price,uint256 _buyNowPrice,uint256 _startTime,uint256 _endTime,uint256 _status)"
            ),
            _creator,
            _id,
  ...
  function setApprovalForAll(address _operator, bool _approved) public override {
    require(_msgSender() != _operator, "DemoNFT:04");
