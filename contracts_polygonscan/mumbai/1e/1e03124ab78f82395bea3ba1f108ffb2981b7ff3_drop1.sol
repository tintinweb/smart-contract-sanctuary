...
contract drop1 {
    ...
    function random() external returns(uint256 index) {
       index = uint256(keccak256(abi.encodePacked(nonce, msg.sender, block.difficulty, block.timestamp)));
       nonce++;
    }

    function ramdom1() external returns(uint256 index) {
       index = uint256(keccak256(abi.encodePacked(nonce, msg.sender, block.difficulty, block.timestamp))) % 10;
       nonce++;
    }
