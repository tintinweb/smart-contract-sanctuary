...
contract MarginTrading {
    ...
    function deposit(address tokenAddress, uint256 amount, uint256 leverage) external payable {
        ...
        uint256 _interest;
        uint256 _amount = depositInfo.assetAmount.add(amount);
        if(depositInfo.assetAmount > 0) {
            _interest = depositInfo.interestAccumulated.add(checkInterestAccumulated(msg.sender, tokenAddress));
        }    
    ...
    function withdraw(address tokenAddress, uint256 amount) external payable {
        ...
        require(userbalances[msg.sender][tokenAddress] >= amount, "Withdraw call : Insufficient Deposit");
        if(tokenAddress == address(0)) {
            uint256 interest = checkInterestAccumulated(msg.sender, address(0));
            if(interest>0) {
            ...
            uint256 interest = checkInterestAccumulated(msg.sender, tokenAddress);
            if(interest>0) {
                if(withdrawAmount>interest) {
                    withdrawAmount = withdrawAmount - interest;  
                }
    ...
    function checkInterestAccumulated(address userAddress, address tokenAddress) public view returns(uint256) {
        uint256 depositAmount = deposits[userAddress][tokenAddress].assetAmount;
        require(depositAmount > 0, "Interest call : Insufficient Deposit");
        uint256 loanAmount = (depositAmount.mul(deposits[userAddress][tokenAddress].leverage)).sub(depositAmount);
        uint256 interval = block.number.sub(deposits[userAddress][tokenAddress].timestamp);
        uint interest = loanAmount.mul(interval.mul(interestPerBlock).div(100)).div(1E18);
        return interest;
    }
}