...
contract Naffiti20Token is ERC20, ERC20Burnable, Pausable, Ownable, ERC20Permit {

    uint256 decimal = 18;
    uint256 weiValue = (10**decimal);
    uint256 initialSupply = 10000000000 * weiValue; //10 Billion
    uint256 seedRound = ((20 * 100000000000000000) * initialSupply) / (100 * weiValue); //200 Million (2% of initial Supply)
    uint256 firstPresale = ((80 * 100000000000000000) * initialSupply) / (100 * weiValue); //800 Million (8% of initial supply)
    uint256 secondPresale = ((0 * weiValue) * initialSupply) / (100 * weiValue); // 0 Million (0% of initial Supply)
    uint256 publicSaleListingPrice = ((20 * 100000000000000000) * initialSupply) / (100 * weiValue); //200 Million (2% of initial Supply)
    uint256 team = ((30 * weiValue) * initialSupply) / (100 * weiValue); //3 Billion (30% of initial supply)
    uint256 advisors = ((20 * weiValue) * initialSupply) / (100 * weiValue); //2 Billion (20% of Initial Supply)
    uint256 ecoSystemFund = ((18 * weiValue) * initialSupply) / (100 * weiValue); //1.8 Billion (18% of Initial Supply)
    ...
    uint256 preApprovedSupply = publicSaleListingPrice + team + ecoSystemFund + liquidity + community + reserve;

    uint256 seedVestingDuration = 365; //days
    uint256 seedWithdrawalDuration = 1 days;

    constructor() ERC20("Naffiti Token", "NAFF") ERC20Permit("Naffiti Token") {
        _mint(address(this), initialSupply);
        _transfer(address(this), msg.sender, preApprovedSupply);
    }
    ...
    function addBeneficiary(address payable _beneficiary, uint _totalAllocation) onlyOwner public {
        ...
        uint totalAllocationInWei = _totalAllocation * weiValue;
        beneficiaryBalance[_beneficiary] = totalAllocationInWei;
        beneficiaryMaxWithdrawPerPeriod[_beneficiary] = totalAllocationInWei / seedVestingDuration;
        beneficiaryAccountStatus[_beneficiary] = AccountStatus.unlocked;
        beneficiaryLastClaimTime[_beneficiary] = block.timestamp - seedWithdrawalDuration;
        isBeneficiary[_beneficiary] = true;
        seedAllocated += totalAllocationInWei;
    }
    ...
    function viewBeneficiary(address payable _beneficiary) public view returns(uint totalAmountLeft, uint totalAmountWithdrawn, AccountStatus accountStatus, bool isABeneficiary, uint amountWithdrawable, uint daysAfterLastWithrawal)
    {
        ...
        daysAfterLastWithrawal = timeDiff / seedWithdrawalDuration;
