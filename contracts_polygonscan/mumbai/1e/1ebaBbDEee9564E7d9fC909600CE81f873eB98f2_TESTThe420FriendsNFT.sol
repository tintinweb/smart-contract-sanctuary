...
contract TESTThe420FriendsNFT is ERC721Enumerable, Ownable { // contract name to yours
  using Strings for uint256;

  string public baseURI;
  string public baseExtension = ".json";
  uint256 public cost = 50 ether; // change price per unit
  uint256 public maxSupply = 1099; // max supply, contract will stop minting after this amount is reach in your open sea colection
  uint256 public maxMintAmount = 100; // max allowed to mint at one time N.O.T.E must be => than line 1241 - mint(msg.sender, 10);
