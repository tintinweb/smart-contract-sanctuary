...
interface ILottery{
    ...
    function buyLottery(uint256[6][]memory values) external;
    function claimThePrize(uint256 roundNumber) external;
    function calculateBonus(address user,uint256 roundNumber) external view returns(uint256 amount);
    function getWinningNumber(uint256 roundNumber)external view returns(uint256[] memory expandedValues);

    event LotteryEvent(uint256 roundNumber);
    event SettlementLottery(uint256 roundNumber,uint256 prizePool,uint256 prizePoolRemainder);
...
contract LabsLottery is ILottery,IRandomCaller,Ownable,ReentrancyGuard,NotContract{
    ...
    ERC20Interface public LabsToken;
    ...
    constructor (address randomer,address token)  {
        ...
        LabsToken = ERC20Interface(token);
    ...
    function startNewRound(uint256 ticketPrice,uint256 endTime) public onlyOwner override {
        ...
        Lotteries[CurrentRound].PrizePool = Lotteries[CurrentRound - 1].PrizePoolRemainder;
    ...
    function settlementLottery() public override onlyOwner{
        _settlementLottery(CurrentRound);
    }
    ...
    function buyLottery(uint256[6][] memory numbers) public notContract nonReentrant override {
        require(Lotteries[CurrentRound].Status == lotterystatus.Open,"The current round is not started or has ended!");
        ...
        UserHistories[msg.sender].push(history);
        ...
        for(uint8 i = 0; i < numbers.length; i++){
            ...
            TicketsOfUser[msg.sender][CurrentRound].push(numbers[i]);
        }
    ...
    function claimThePrize(uint256 roundNumber) public notContract nonReentrant override{
        ...
        require(!Lotteries[roundNumber].Claimed[msg.sender],"You have received this round of bonuses");

        uint256 amount = calculateBonus(msg.sender,roundNumber);
    ...
    function calculateBonus(address user,uint256 roundNumber) public override view returns(uint256 amount){
        uint256[] memory winningNumber = getWinningNumber(roundNumber);
    ...
    function randomCallback(uint256 roundNumber) public override{
        ...
        _settlementLottery(roundNumber);
    }

    // Get the buy tickets history of users
    function getUserBuyHistoriesWithPage(address user,uint256 page,uint256 rows,bool desc) public view returns(History[] memory histories,uint256 totalHistories,uint256 totalPage){
        History[] memory _histories = UserHistories[user];

        totalHistories = _histories.length;

        if (totalHistories == 0) {
            return (_histories,0,0);
        }
        ...
        if (desc == false){
            ...
            histories = new History[](endIndex - startIndex);
            for(uint i = startIndex ;i < endIndex; i++){
                History memory history = History({
                    TimeStamp:_histories[i].TimeStamp,
                    RoundNumber:_histories[i].RoundNumber,
                    TotalTickets:_histories[i].TotalTickets
                });
        ...
        {   
            ...
            histories = new History[](startIndex - endIndex + 1);
            uint256 j = 0;
            for(uint i = startIndex; i >= endIndex; i--){
                History memory history = History({
                    TimeStamp:_histories[i].TimeStamp,
