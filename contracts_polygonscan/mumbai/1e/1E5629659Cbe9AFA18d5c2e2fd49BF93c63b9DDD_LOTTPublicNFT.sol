...
abstract contract TokenMetaData is ERC721{
    ...
    mapping(uint256 => string) private tokenPublicInfo;
    mapping(uint256 => string) private tokenPrivateInfo;
    mapping(uint256 => string) private tokenPublicFileHash;
    ...
    function publicInfo(uint256 tokenId) external returns(string memory _publicInfo) {
    ...
    function publicFileHash(uint256 tokenId) external returns(string memory _publicFileHash) {
        require(_exists(tokenId), "TokenMetaData: nonexistent token");
