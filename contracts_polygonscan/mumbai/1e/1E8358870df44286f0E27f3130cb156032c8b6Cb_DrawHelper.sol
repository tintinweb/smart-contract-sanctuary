...
contract DrawHelper {
    ...
    PixMoment internal immutable _pixMoment;
    IPixelCoin internal immutable _pixelCoin;
    uint256 internal immutable _drawPixelIndex;
    uint256 internal immutable _rewardPerPixel;
    ...
    constructor(
        ...
        address pixelCoin,
        uint256 index,
        uint256 rewardPerPixel
    ) {
        _canvas = CanvasManager(canvas);
        _pixMoment = PixMoment(pixMoment);
        _pixelCoin = IPixelCoin(pixelCoin);
        _drawPixelIndex = index;
        _rewardPerPixel = rewardPerPixel;

        _pixelCoin.approve(canvas, type(uint256).max);
    }
    ...
    function getDrawPixelIndex() external view returns (uint256) {
        return _drawPixelIndex;
    }

    function getRewardPerPixel() external view returns (uint256) {
        return _rewardPerPixel;
    }
    ...
    function drawPixel(
        ...
        uint256 price = _canvas.getDrawPrice(nftId, index, _drawPixelIndex);

        _pixelCoin.mint(address(this), price);
        _canvas.drawPixel(_drawPixelIndex, nftId, index, color, price);
        _pixelCoin.burn(address(_canvas), price);

        uint256 iMomentId = _pixMoment.tokenOfOwnerByIndex(address(this), 0);
        _pixMoment.transferFrom(address(this), msg.sender, iMomentId);

        _pixelCoin.mint(msg.sender, _rewardPerPixel);
    }
    ...
    function drawMultiPixel(
        ...
        for (uint256 i = 0; i < count; i++) {
            uint256 price = _canvas.getDrawPrice(nftId, indexes[i], _drawPixelIndex);
            totalCost += price;
            prices[i] = price;
        }

        _pixelCoin.mint(address(this), totalCost);
        for (uint256 i = 0; i < count; i++) {
            _canvas.drawPixel(_drawPixelIndex, nftId, indexes[i], colors[i], prices[i]);
            uint256 iMomentId = _pixMoment.tokenOfOwnerByIndex(address(this), 0);
            _pixMoment.transferFrom(address(this), msg.sender, iMomentId);
        }
        _pixelCoin.burn(address(_canvas), totalCost);

        _pixelCoin.mint(msg.sender, _rewardPerPixel * count);
    }
}
