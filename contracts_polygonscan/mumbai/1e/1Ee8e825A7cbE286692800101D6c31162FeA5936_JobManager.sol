...
*/
contract JobManager is IIterableFunctions, StorageAccessor, FeeManager {
    struct JobCreationParams {
        address creator;
        uint256 externalId;
        uint256 previousJob;
        string jobDescriptionUri;
    ...
    string constant ETH_INSUFFICIENT_REASON = "JM:ETH != declared";
    string constant NOT_CREATOR_REASON = "JM:not creator";
    string constant NO_SOLUTION = "JM:no solution";
    string constant TYPE = "JOB";
    bytes32 private constant _STORAGE_SLOT = keccak256("hyve.works.storage.job");

    event JobCreated(uint256 indexed externalId, uint256 indexed jobId);
    event JobCancelled(uint256 indexed jobId);
    event JobApplicationCreated(uint256 indexed jobId, uint256 indexed applicationId, address applicant);
    event JobApplicationAccepted(uint256 indexed jobId, uint256 indexed applicationId);
    event JobSolutionSubmitted(uint256 indexed jobId, uint256 indexed applicationId, uint256 solutionId, address applicant);
    event JobSolutionAccepted(uint256 indexed jobId, uint256 solutionId);
    event JobSolutionRejected(uint256 indexed jobId, uint256 solutionId);

    function __JobManager_init(address jobStorage) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()));
        setStorageAddress(jobStorage);
    }

    function getFunctions() override external pure returns (bytes4[] memory){
        bytes4[] memory functions = new bytes4[](6);
        functions[0] = this.__JobManager_init.selector;
        functions[1] = this.createJob.selector;
        functions[2] = this.cancelJob.selector;
        functions[3] = this.submitJobSolution.selector;
        functions[4] = this.acceptJobSolution.selector;
        functions[5] = this.rejectJobSolution.selector;
        return functions;
    }
    ...
    function getJobStorage() private view returns (JobStorage) {
        return JobStorage(getStorageAddress());
    }

    function getConfigStorage() override virtual internal view returns (IConfigStorage) {
        return getJobStorage().configStorage();
    }
    ...
    function createJob(
        JobCreationParams calldata jobParams,
        address applicant
    ) external payable {
        require(jobParams.amount > 0, "JM:0 amount");
        require(
            (jobParams.previousJob == 0 && jobParams.creator == _msgSender()) || jobParams.creator != _msgSender(),
            "JM:wrong creator");
        bool isEth = (jobParams.tokenAddress == address(0)) && msg.value > 0;
        //checks
        uint256 tax = calculateTax(jobParams.isHyveCommission, isEth, jobParams.amount, jobParams.tokenAddress);

        if (msg.value > 0) {
            require(jobParams.tokenAddress == address(0), "JM:Token address included");
            isEth = true;
            if (jobParams.isHyveCommission) {
                checkUserTokenBalance(jobParams.creator, getConfigStorage().getHyveERC(), tax);
                require(msg.value == jobParams.amount, ETH_INSUFFICIENT_REASON);
            } else {
                require(msg.value == jobParams.amount + tax, ETH_INSUFFICIENT_REASON);
            }
        } else {
            require(
                jobParams.tokenAddress != getConfigStorage().getHyveERC() || jobParams.isHyveCommission,
                "JM:tax not HYVE");
            if (jobParams.isHyveCommission) {
                checkUserTokenBalance(jobParams.creator, jobParams.tokenAddress, jobParams.amount);
                checkUserTokenBalance(jobParams.creator, getConfigStorage().getHyveERC(), tax);
            } else {
                checkUserTokenBalance(jobParams.creator, jobParams.tokenAddress, jobParams.amount + tax);
            }
        }

        //effects
        uint256 jobId = getJobStorage().getNewJobId();
        getJobStorage().addNewJob(
            jobId,
            JobStorage.Job(
                jobParams.previousJob,
                jobParams.jobDescriptionUri,
                jobParams.jobDescriptionHash,
                jobParams.tokenAddress,
                jobParams.amount,
                jobParams.creator,
                jobParams.period,
                jobParams.period + block.timestamp,
                jobParams.isHyveCommission,
                false,
                0,
                0)
        );
        ...
        //interactions
        if (jobParams.isHyveCommission) {
            splitTax(jobParams.creator, false, tax, IERC20Upgradeable(getConfigStorage().getHyveERC()));
        } else {
            splitTax(jobParams.creator, isEth, tax, IERC20Upgradeable(jobParams.tokenAddress));
        }

        if (isEth) {
            IEscrow(getConfigStorage().getEscrow()).createDepositETH{value : jobParams.amount}(jobId, TYPE, jobParams.creator, jobParams.amount);
        } else {
            IEscrow(getConfigStorage().getEscrow()).createDeposit(address(this), jobId, TYPE, jobParams.creator, jobParams.tokenAddress, jobParams.amount);
        }

        emit JobCreated(jobParams.externalId, jobId);
        uint256 applicationId = getJobStorage().getNewApplicationId();
        getJobStorage().addNewApplication(applicationId, UtilityStorage.Application(jobId, applicant));
        emit JobApplicationCreated(jobId, applicationId, applicant);
        getJobStorage().acceptApplication(applicationId);
        emit JobApplicationAccepted(jobId, applicationId);
    }

    function cancelJob(uint256 jobId) external {
        JobStorage.Job memory job = getJobStorage().getJob(jobId);
        require(_msgSender() == job.creator, NOT_CREATOR_REASON);
        getJobStorage().cancelJob(jobId);
        IEscrow(getConfigStorage().getEscrow()).redeemDeposit(jobId, TYPE);
        emit JobCancelled(jobId);
    }

    function submitJobSolution(uint256 applicationId, string memory solutionUri, string memory solutionHash) external {
        require(applicationId <= getJobStorage().getLastApplicationId(), "JM:no application");
        UtilityStorage.Application memory application = getJobStorage().getApplication(applicationId);
        require(_msgSender() == application.worker, "JM:not applicant");
        uint256 solutionId = getJobStorage().getNewSolutionId();
        getJobStorage().addNewSolution(solutionId, UtilityStorage.Solution(applicationId, solutionUri, solutionHash, ""));
        emit JobSolutionSubmitted(application.listingId, applicationId, solutionId, _msgSender());
    }

    function acceptJobSolution(uint256 solutionId, bool renew) external payable {
        UtilityStorage.Application memory application = getJobStorage().getApplication(getJobStorage().getSolution(solutionId).applicationId);
        JobStorage.Job memory job = getJobStorage().getJob(application.listingId);
        require(_msgSender() == job.creator, "JM:not job creator");
        getJobStorage().acceptSolution(solutionId);
        ...
        emit JobSolutionAccepted(application.listingId, solutionId);
        //re-create Job
        if (renew) {
            uint256 eth = 0;
            if (job.token == address(0)) {
                require(msg.value > 0, "JM:ETH needed");
                eth = msg.value;
            }
            this.createJob{value : eth}(JobCreationParams(
                    job.creator,
                    0,
    ...
    function rejectJobSolution(uint256 solutionId, string memory reason) external {
        require(solutionId <= getJobStorage().getLastSolutionId(), NO_SOLUTION);
        UtilityStorage.Application memory application = getJobStorage().getApplication(getJobStorage().getSolution(solutionId).applicationId);
        require(_msgSender() == getJobStorage().getJob(application.listingId).creator, NOT_CREATOR_REASON);
        getJobStorage().rejectSolution(solutionId, reason);
        emit JobSolutionRejected(application.listingId, solutionId);
    }
}
...
contract JobStorage is UtilityStorage {
    ...
    mapping(address => uint256[]) private _creatorJobs;
    mapping(uint256 => Job) private _jobs;

    function getNewJobId() external onlyRole(OPERATOR_ROLE) returns (uint256){
        _jobId = _jobId + 1;
        return _jobId;
    }

    function getLastJobId() external view returns (uint256){
        return _jobId;
    }

    function getJobsCreatedBy(address creator) external view returns (uint256[] memory){
        return _creatorJobs[creator];
    }

    function addNewJob(uint256 jobId, Job memory job) external onlyRole(OPERATOR_ROLE) {
        _creatorJobs[job.creator].push(jobId);
        _jobs[jobId] = job;
    }
    ...
    function cancelJob(uint256 jobId) external onlyRole(OPERATOR_ROLE){
        JobStorage.Job memory job = _jobs[jobId];
        require(job.deadline < block.timestamp, "JS: deadline not reached");
        require(_applicationSolutions[job.applicationId].length == 0, "TS: Job has solutions submitted!");
        _jobs[jobId].canceled = true;
    }

    function acceptApplication(uint applicationId) external onlyRole(OPERATOR_ROLE) {
        _jobs[_applications[applicationId].listingId].applicationId = applicationId;
    }

    function acceptSolution(uint256 solutionId) external onlyRole(OPERATOR_ROLE) {
        require(solutionId <= _solutionId, "JS: no solution");
        _jobs[_applications[_solutions[solutionId].applicationId].listingId].solutionId = solutionId;
    }
}
