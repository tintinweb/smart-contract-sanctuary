...
contract NFTSale is Ownable {
    ...
    function setContractStatus(bool _isActive) external onlyAdmin {
        isActive = _isActive;
        emit ContractStatusSet(msg.sender, _isActive);
    }
    ...
    function sellNFT(
        ...
        Sale memory sale = Sale(nftId, price, owner, true);
        _nftSales[nftId] = sale;
        ERC721(nftAddress).transferFrom(msg.sender, address(this), nftId);
        emit SaleAdded(nftId, price, owner, now);
    }
    ...
    function cancelSale(uint256 nftId) external onlyAdmin {
        ...
        ERC721(nftAddress).transferFrom(address(this), owner(), nftId);
        emit SaleCancelled(nftId, _nftSales[nftId].price, _nftSales[nftId].owner, msg.sender, now);
    }
    ...
    function purchaseNFT(uint256 nftId) external payable {
        ...
        require(msg.value == _nftSales[nftId].price, "value!=price");
        ...
        ERC721(nftAddress).transferFrom(address(this), buyer, nftId);

        emit Sold(nftId, seller, buyer, price, now);
    }
    ...
    function releaseNFT(uint256 nftId, address payable buyer) external onlyAdmin {
        ...
        emit Sold(nftId, seller, buyer, 0, now);
    }
