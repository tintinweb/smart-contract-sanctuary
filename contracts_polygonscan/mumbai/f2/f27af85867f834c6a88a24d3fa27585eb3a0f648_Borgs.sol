...
contract Borgs is ERC721Enumerable, ERC721Holder, CommonObjects, NumberUtils{
    ...
    function addBorgAttributeToLayer(string calldata layerName, uint256 chance, string memory borgAttributeName, bytes[] calldata colours) external onlyOwner editable{
        // Get the layer
    ...
    function _combineBorgAttributes(string[] memory borgAttributeNames) internal view returns (string[] memory hexPixals){
        ...
        for(uint256 i=0;i<borgAttributeNames.length;i++){
             ...
             for(uint256 j = 0;j<borgAttribute.colors.length;j+=3){
                string memory colour = string(borgAttribute.colors[j]);
                uint32 colorStartPosition = toUint32(borgAttribute.colors[j + 1]);
                uint32 colorLength = toUint32(borgAttribute.colors[j + 2]);
                
                for(uint256 k = colorStartPosition;k<colorLength;k++){
                    hexPixals[k] = colour;
                }
            }
        }
