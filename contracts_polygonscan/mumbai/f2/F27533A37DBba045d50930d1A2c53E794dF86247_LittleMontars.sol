...
contract LittleMontars is ERC721Enumerable, Ownable, ERC721Burnable, ERC721Pausable {
    ...
    uint256 public constant MAX_ITEMS = 50000;
    ...
    uint256 public PRICE = 1E16; // 0.07 ETH
    address public constant treasuryAddress = 0xfF2f9079987A0a1F8D46BD0BAfef3C64B87fE2F8;
    address public constant devAddress = 0x3b70058237176aCa3D2A8935F4f9F1875305a102;
    ...
    event CreateMontar(uint256 indexed id);

    constructor(string memory baseURI, address wethAddress) ERC721("LITTLE MONTARS", "MONTARS") {
        setBaseURI(baseURI);
        pause(true);
        presale = false;
        sale = false;
        REVEAL_TIMESTAMP = block.timestamp + (86400 * 7);
        _weth = ERC20(wethAddress);
    }
    ...
    function presaleMint(address _to, uint256 _count) public {
        uint256 total = _totalSupply();
        ...
        require(_weth.balanceOf(_to) >= totalPrice, "Not enough WETH"); 
        require(_weth.allowance(_to, address(this)) >= totalPrice, "Not enough WETH allownance");

        for (uint256 i = 0; i < _count; i++) {
            _mintAnElement(_to);
        }

        _weth.transferFrom(_to, address(this), totalPrice);
    }
    ...
    */
    ...
    function withdrawAll() public payable onlyOwner {
        ...
        require(balance > 0);
        uint256 devAmount = balance.mul(30).div(100);
        uint256 treasuryAmount = balance - devAmount;

        _weth.transfer(devAddress, devAmount);
        _weth.transfer(treasuryAddress, treasuryAmount);
    }
