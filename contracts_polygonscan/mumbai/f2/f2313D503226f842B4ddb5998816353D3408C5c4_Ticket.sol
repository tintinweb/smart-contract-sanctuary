...
import "../Raffle/Entities.sol";

contract Ticket is Raffle, ERC721 {
    constructor() ERC721("CryptoRaffle Ticket", "CRT") {}

    using SafeMath for uint256;

    uint256 ticketCount = 0;

    mapping(uint256 => uint256) ticketToRaffle;

    function buyTicket(address _to, uint _raffleId) external  {
        Entities.Raffle memory raffle = raffles[_raffleId];
        require(block.timestamp < raffle.resolutionTimestamp);
        _safeMint(_to, ticketCount);
        ticketCount = ticketCount.add(1);
    }

    function getRaffleByTicketId(uint256 _ticketId) public view returns (Entities.Raffle memory) {
        uint256 raffleId = ticketToRaffle[_ticketId];
        Entities.Raffle memory raffle = raffles[raffleId];
        return raffle;
    }
}
