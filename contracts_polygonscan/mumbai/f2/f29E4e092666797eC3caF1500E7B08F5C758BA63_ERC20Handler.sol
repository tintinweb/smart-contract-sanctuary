...
contract ERC20Handler is IDepositExecute, HandlerHelpers, ERC20Safe {
    ...
    */
    ...
    function deposit(
        ...
        require(amount >= IBridge(_bridgeAddress)._amountMinLimit(), "amount min limit");
        require(amount <= IBridge(_bridgeAddress)._amountMaxLimit(), "amount max limit");

        uint256 fee = IBridge(_bridgeAddress)._fee();
        uint256 feeAmount = amount.mul(fee).div(FEE_BASEPOINT);
        tokenFeeAmounts[tokenAddress] = feeAmount;
    ...
    function executeProposal(bytes32 resourceID, bytes calldata data) external override onlyBridge {
        ...
        require(_contractWhitelist[tokenAddress], "provided tokenAddress is not whitelisted");
        uint256 availableAmount = balanceOfERC20(tokenAddress, address(this));
        if (availableAmount > tokenFeeAmounts[tokenAddress]) {
            availableAmount = availableAmount.sub(tokenFeeAmounts[tokenAddress]);
        }
        else {
            availableAmount = 0;
        }

        if (_burnList[tokenAddress] || availableAmount < amount) {
    ...
    function withdraw(address tokenAddress, address recipient, uint amount) external override onlyBridge {
        uint256 feeAmount = tokenFeeAmounts[tokenAddress];
        if (feeAmount > amount) {
            feeAmount = amount;
        }

        tokenFeeAmounts[tokenAddress] = tokenFeeAmounts[tokenAddress].sub(feeAmount);

        releaseERC20(tokenAddress, recipient, amount);

        emit Withdraw(tokenAddress, amount, feeAmount);
    }
}
...
contract HandlerHelpers is IERCHandler {
    ...
    function setBurnable(address contractAddress) external override onlyBridge{
        _setBurnable(contractAddress, true);
    }
    ...
    function setNotBurnable(address contractAddress) external override onlyBridge{
        _setBurnable(contractAddress, false);
    }
    ...
    function _setBurnable(address contractAddress, bool isBurnable) internal {
        require(_contractWhitelist[contractAddress], "provided contract is not whitelisted");
        _burnList[contractAddress] = isBurnable;
    }
}
...
contract ERC20Safe {
    ...
    /**
        @notice Used to getting balance of addres.
        @param tokenAddress Address of ERC20 to transfer.
        @param owner Address of current token owner.
     */
    function balanceOfERC20(address tokenAddress, address owner) public returns (uint256){
...
interface IERCHandler {
    ...
    /**
        @notice Marks {contractAddress} as not mintable/burnable.
        @param contractAddress Address of contract to be used when making or executing deposits.
     */
    function setNotBurnable(address contractAddress) external;
    /**
        @notice Used to manually release funds from ERC safes.
        @param tokenAddress Address of token contract to release.
        @param recipient Address to release tokens to.
