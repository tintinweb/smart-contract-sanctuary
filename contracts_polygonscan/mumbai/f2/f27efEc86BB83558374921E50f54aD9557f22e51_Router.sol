...
contract Router is Refundable {
        ...
        returns (int64)
    {
        ...
        if (decimals > 6) {
            return
                int64(
                    uint64(uint256(amount)) / uint64(10)**(uint64(6) - decimals)
                );
        } else if (decimals < 6) {
            return
                int64(
    ...
    function sell(Pool pool, int64 inputAmount) public {
        ...
        baseTokenBalances[pool] += inputAmount;
        pool.token().safeTransferFrom(
            msg.sender,
            address(this),
            uint256(uint64(inputAmount))
        );
...
contract YieldFarmablePool is YieldFarmable, Pool {
    ...
    function decimals() public view virtual override(Pool, ERC20) returns (uint8) {
        return 6;
    }
}
