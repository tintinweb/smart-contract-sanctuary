...
contract MartabakCult is ERC721Enumerable, Ownable, IMartabakCult {
    ...
    uint256 private immutable breedPrice = 1500 ether; // 1000 MTBK
        ...
        ERC721("Martabak Cult", "MC")
    {
        ...
        // We are minting 50 tokens for give away, martketing and so on
    ...
    function mintPhase1(uint256 amount, bool boosted) external override {
        require(saleActive, "Sale innactive");
        require(amount <= maxMintAtOnce, "Max 3 per tx");
        require(totalSupply() + amount <= maxSupplyPhase1(), "Exceed max available");
    ...
    function mintPresale(uint256 amount, bool boosted) external override {
        require(presaleActive, "Presale innactive");
        require(whiteList[msg.sender], "Not on the WhiteList");
        require(amount <= maxMintAtOnce, "Max 3 per tx");
        require(totalSupply() + amount <= maxSupplyPhase1(), "Exceed max available");
        require(whiteListClaimed[msg.sender] + amount <= 3, "Purchase exceeds max allowed");
    ...
    function _mintPhase1(uint256 amount, address to, bool boosted) internal {
    ...
    function breed(uint256 amount, uint256 firstId, uint256 secondId, bool boosted) external override {
        require(breedActive, "Breeding innactive");
        require(totalSupply() + amount <= maxSupplyPhase1() + maxBreed, "Max breeds reached");
        require(martabakContract.balanceOf(msg.sender) >= breedPrice * amount, "Not enough Martabak");
        require(msg.sender == ownerOf(firstId) && msg.sender == ownerOf(secondId), "You're not the owner");
    ...
    function isBreedCompatible(uint256 t1, uint256 t2) internal pure returns (bool)
    {
        return
    ...
    function _balanceByIdOf(address user) internal view returns (Droid[] memory)
    {
    ...
    function balanceByIdOf(address user) external view override returns (Droid[] memory)
    {
    ...
    {
        //(uint256[] memory tokensIds, uint256[] memory tokensTypes) = _balanceByIdOf(user);
        Droid[] memory droids = _balanceByIdOf(user);

        uint256 claimable = 0;

        for (uint256 i = 0; i < droids.length; i++) {
            uint256 time = calculateYieldTime(droids[i].id);
            uint256 rate = 86400;
