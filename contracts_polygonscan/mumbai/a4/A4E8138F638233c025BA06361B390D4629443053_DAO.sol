...
contract DAO {
     ...
     */
    function setBool(bytes32 tag, bool data) external onlyGov {
        bytes32ToBool[tag] = data;
        emit SetBool(tag, data);
    }
    function setUint(bytes32 tag, uint data) external onlyGov {
        bytes32ToUint[tag] = data;
        emit SetUint(tag, data);
    }
    function setInt(bytes32 tag, int data) external onlyGov {
        bytes32ToInt[tag] = data;
        emit SetInt(tag, data);
    }
    function setAddress(bytes32 tag, address data) external onlyGov {
        bytes32ToAddress[tag] = data;
        emit SetAddress(tag, data);
    }
    function setString(bytes32 tag, string memory data) external onlyGov {
        bytes32ToString[tag] = data;
        emit SetString(tag, data);
    }
    function setBytes(bytes32 tag, bytes memory data) external onlyGov {
