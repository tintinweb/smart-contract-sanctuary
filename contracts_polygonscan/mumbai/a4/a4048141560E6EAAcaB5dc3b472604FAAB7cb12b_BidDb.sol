...
contract AuctionHouse is IAuctionHouse, ReentrancyGuard {
    ...
    // auction id => IBidDb.Bid[]
    mapping(uint256 => IBidDb.Bid[]) internal _bids;

    uint256[] internal _auctionIds;

    IBidDb internal _bidDb;
     ...
     * Constructor
     */
    constructor(address _zora, address _weth, address bidDb) public {
        require(
        ...
        _auctionIdTracker._value = _bidDb.getMaxOldAuctionId();
    }
    ...
    function getAllNft() public view returns(address[] memory, uint256[] memory) {
        address[] memory tokens = new address[](_auctionIds.length);
        uint256[] memory tokenIds = new uint256[](_auctionIds.length);

        for (uint256 i = 0; i < _auctionIds.length; i++) {
            Auction storage auction = auctions[_auctionIds[i]];
            tokens[i] = auction.tokenContract;
    ...
    function getNftIdsByToken(address token) public view returns(uint256[] memory) {
        uint256 count = 0;
        for (uint256 i = 0; i < _auctionIds.length; i++) {
            Auction storage auction = auctions[_auctionIds[i]];
            if (auction.tokenContract == token) {
                count++;
            }
        }
    ...
    function getNftIdsByOwner(address token, address owner) public view returns(uint256[] memory) {
        uint256 count = 0;
        for (uint256 i = 0; i < _auctionIds.length; i++) {
            Auction storage auction = auctions[_auctionIds[i]];
            if (auction.tokenOwner == owner && auction.tokenContract == token) {
                count++;
            }
        }
    ...
    function getAuctions() public view returns(IAuctionHouse.Auction[] memory) {
        IAuctionHouse.Auction[] memory result = new IAuctionHouse.Auction[](_auctionIds.length);
        for (uint256 i = 0; i < _auctionIds.length; i++) {
            result[i] = auctions[_auctionIds[i]];
        }

        return result;
    }

    function getAuctionsByAccount(address account) public view returns(IAuctionHouse.Auction[] memory) {
        uint256 count = 0;
        for (uint256 i = 0; i < _auctionIds.length; i++) {
            Auction storage auction = auctions[_auctionIds[i]];
            if (auction.tokenOwner == account) {
                count++;
            }
        }

        uint256 index = 0;
        IAuctionHouse.Auction[] memory result = new IAuctionHouse.Auction[](count);
        for (uint256 i = 0; i < _auctionIds.length; i++) {
            Auction storage auction = auctions[_auctionIds[i]];
            if (auction.tokenOwner == account) {
    ...
    function getAuctionByTokenId(address token, uint256 tokenId) public view returns(Auction memory, IBidDb.Bid[] memory) {
        for (uint256 i = 0; i < _auctionIds.length; i++) {
            uint256 auctionId = _auctionIds[i];
            Auction storage auction = auctions[auctionId];
            if (auction.tokenContract == token && auction.tokenId == tokenId) {
                return (auction, _bids[auctionId]);
            }
        }
    }
    ...
    function setAuctionReservePrice(uint256 auctionId, uint256 reservePrice) external override auctionExists(auctionId) {
        ...
        _onSetAuctionReservePrice(auctionId, reservePrice);
    ...
    nonReentrant
    {
        ...
        _onCreateBid(auctionId, msg.sender, amount, auctions[auctionId].duration);
    ...
    function endAuction(uint256 auctionId) external override auctionExists(auctionId) nonReentrant {
        ...
        _onEndAuction(auctionId, curatorFee, tokenOwnerProfit);
    ...
    function _approveAuction(uint256 auctionId, bool approved) internal {
        ...
        _onApproveAuction(auctionId, approved);
    ...
    function _onCreateAuction(uint256 auctionId) internal {
        _auctionIds.push(auctionId);

        Auction storage auction = auctions[auctionId];
        _bidDb.createAuction(
            auctionId, 
            auction.tokenId, 
    ...
    function _onApproveAuction(uint256 auctionId, bool approved) internal {
        _bidDb.setAuctionApproval(auctionId, approved);
    }

    function _onSetAuctionReservePrice(uint256 auctionId, uint256 reservePrice) internal {
        _bidDb.setAuctionReservePrice(auctionId, reservePrice);
    }

    function _onCreateBid(uint256 auctionId, address bidder, uint256 amount, uint256 duration) internal {
        IBidDb.Bid memory bid = IBidDb.Bid({
            amount: amount,
        ...
        _bidDb.createBid(auctionId, bidder, amount, duration);
    }

    function _onEndAuction(uint256 auctionId, uint256 curatorFee, uint256 tokenOwnerProfit) internal {
        delete _bids[auctionId];
       
        for (uint256 i = 0; i < _auctionIds.length; i++) {
            if (_auctionIds[i] == auctionId) {
                _auctionIds[i] = _auctionIds[_auctionIds.length -1];
                _auctionIds.pop();
                break;
            }
        }

        _bidDb.endAuction(auctionId, curatorFee, tokenOwnerProfit);
    }

    function _onCancelAuction(uint256 auctionId) internal {
        delete _bids[auctionId];
       ...
       _bidDb.cancelAuction(auctionId);
    }
}
