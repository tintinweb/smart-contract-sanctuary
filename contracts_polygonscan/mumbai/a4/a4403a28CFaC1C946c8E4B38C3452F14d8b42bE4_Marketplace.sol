...
contract Marketplace is PriceConsumerV3, EIP712MetaTransaction {
    ...
    /**
        ...
        - if w2 or w3user wants to pay in USD, they pay gas fee to Mocha first
        - then Mocha triggers setForSaleWithUSD for them
  ...
  /**
    ...
    * 2. trigger setApprovalForAll() from Mocha EOA to approve this contract
    ...
    */
    function setForSaleWithUSD(uint256 _tokenId, uint256 _priceUSD, bool _auction) public isActive onlyMocha {
        ...
        require(
            mochaToken.isApprovedForAll(mochaToken.mochaTrust(), address(this)),
            "Mocha did not approve Marketplace contract"
        );
  ...
  /**
    ...
    */
    function removeSetForSaleUSD(uint256 _tokenId) public isActive onlyMocha {
        require(_tokenId > 0, "token id is zero or lower");
  ...
  /**
    ...
    */
    function setForSaleWithETH(uint256 _tokenId, uint256 _priceUSD, bool _auction, bytes32 _passcode) public isActive verifyPasscode(_passcode) {
        ...
        require(tokenPriceUSD[_tokenId] == 0, "token has already been set for sale");
        require(_priceUSD > 0, "price is zero or lower");
        address owner = mochaToken.ownerOf(_tokenId);
        require(owner == msgSender(), "caller is not a token owner");
        require(mochaToken.isApprovedForAll(msgSender(), address(this)),
  ...
  /**
    ...
    */
        ...
        onlyMocha
    {
        ...
        address owner = mochaToken.ownerOf(_tokenId);
        (bool isW3user, address w3owner, string memory w2owner) = mochaToken.getTokenOwnership(_tokenId);

        if (isW3user) {
            require(owner == w3owner, "wrong owner");
            require(w3owner != address(0), "w3owner is empty");
            mochaToken.safeTransferFrom(w3owner, mochaToken.mochaTrust(), _tokenId);
        } else {
            require(owner == mochaToken.mochaTrust(), "wrong owner");
            require(bytes(w2owner).length > 0, "w2owner is empty");
        }

        mochaToken.changeTokenOwnership(_tokenId, address(0), _buyerId);
        tokenPriceUSD[_tokenId] = 0;
        tokenOnAuction[_tokenId] = false;
    ...
    */
        ...
        onlyMocha
    {
        ...
        if (isW3user) {
            require(owner == w3owner, "wrong owner");
            require(w3owner != address(0), "w3owner is empty");
            mochaToken.safeTransferFrom(w3owner, _buyer, _tokenId);
        } else {
            require(owner == mochaToken.mochaTrust(), "wrong owner");
            require(bytes(w2owner).length > 0, "w2owner is empty");
            mochaToken.safeTransferFrom(mochaToken.mochaTrust(), _buyer, _tokenId);
        }

        mochaToken.changeTokenOwnership(_tokenId, _buyer, "");
        tokenPriceUSD[_tokenId] = 0;
        tokenOnAuction[_tokenId] = false;
    ...
    */
    function purchaseWithETHw3user(uint256 _tokenId, uint256 wethAmount, bytes32 _passcode) public isActive verifyPasscode(_passcode) {
        ...
        if (isW3user) {
            require(owner == w3owner, "wrong owner");
            require(w3owner != address(0), "w3owner is empty");
            mochaToken.safeTransferFrom(w3owner, msgSender(), _tokenId);
        } else {
            require(owner == mochaToken.mochaTrust(), "wrong owner");
            require(bytes(w2owner).length > 0, "w2owner is empty");
            mochaToken.safeTransferFrom(mochaToken.mochaTrust(), msgSender(), _tokenId);
        }

        mochaToken.changeTokenOwnership(_tokenId, msgSender(), "");
        tokenPriceUSD[_tokenId] = 0;
        tokenOnAuction[_tokenId] = false;
  ...
  /**
    ...
    */
    function placeBidWithETHw3user(uint256 _tokenId, uint256 _bidPriceUSD, uint256 wethAmount, bytes32 _passcode) public isActive verifyPasscode(_passcode) {
        ...
        _send(sellerReceiveAmount, mochaToken.mochaTrust());
    ...
    */
        ...
        onlyMocha
    {
        ...
        if (isW3user) {
            require(owner == w3owner, "wrong owner");
            require(w3owner != address(0), "w3owner is empty");
            mochaToken.safeTransferFrom(w3owner, w3user ? _receiver : mochaToken.mochaTrust(), _tokenId);
        // w2user
        } else {
            require(owner == mochaToken.mochaTrust(), "wrong owner");
            require(bytes(w2owner).length > 0, "w2owner is empty");
            if (w3user) {
                mochaToken.safeTransferFrom(mochaToken.mochaTrust(), _receiver, _tokenId);
            }
        }

        mochaToken.changeTokenOwnership(_tokenId, w3user ? _receiver : address(0), w3user ? "" : _receiverId);
        tokenPriceUSD[_tokenId] = 0;
        tokenOnAuction[_tokenId] = false;
  ...
  /**
    ...
    */
    function _validateAmount(uint256 _tokenId, uint256 wethAmount) private {
        ...
        uint256 buyerFee = tokenPrice.mul(mochaToken.feePercent()).div(1000);
  ...
  /**
    ...
    */
    function _validateBidAmount(uint256 _bidPriceUSD, uint256 wethAmount) private {
        int256 currentPriceOfETHtoUSD = getCurrentPriceOfETHtoUSD();
        uint256 buyerFee = _bidPriceUSD.mul(mochaToken.feePercent()).div(1000);
  ...
  /**
    ...
    */
    function _sendRoyalty(uint256 _tokenId, uint256 wethAmount) private returns (uint256) {
        ...
        _send(royalty, mochaToken.mochaTrust());
        return royalty;
    }

  /**
    * Send money to seller
    * Mocha keeps it if seller is w2user
    *
    ...
    */
    function _sendToSeller(
        ...
        private
    {
        _send(sellerReceiveAmount, _isW3user ? _w3owner : mochaToken.mochaTrust());
    }
  ...
  /**
    ...
    */
    ...
    function setPasscode(bytes32 passcode_) external {
        require(msgSender() == address(mochaToken));
        passcode = passcode_;
    }
}
