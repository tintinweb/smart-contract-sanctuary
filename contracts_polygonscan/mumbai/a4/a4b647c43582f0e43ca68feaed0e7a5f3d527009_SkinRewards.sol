...
interface ISkinRewards {
  ...
  function computeGoals(uint tokenId) external view 
    returns(
      uint traitcountOfPreviousEligiblePeriod,
      uint traitcountToEligibility
    );
  function pause() external;
  function unpause() external;
}
...
contract SkinRewards is ISkinRewards, IDescriptorUser, AccessControlEnumerableUpgradeable, PausableUpgradeable {
  uint public constant TRAITCOUNT_DELTA = 7;
  ...
  function _getTraitcountOfPreviousEligiblePeriod(uint tokenId) internal view
    returns (uint traitCount)
  {
    Period storage period = _periods[_currentPeriodIdx];
    uint previousEligiblePeriodIdx = _snook.getPreviousEligiblePeriodIdx(tokenId);
    uint currentEligiblePeriodIdx = _snook.getCurrentEligiblePeriodIdx(tokenId);
    
    if (period.tokenStarsUpdated[tokenId] == true) { 
      traitCount = _periodTokenTraitcount[previousEligiblePeriodIdx][tokenId];
    }
    else { // previous eligible period for the first game in season is indexed by currentEligiblePeriodIdx 
      traitCount = _periodTokenTraitcount[currentEligiblePeriodIdx][tokenId];
    }
  }

  function computeGoals(uint tokenId) external view override returns(
    uint traitcountOfPreviousEligiblePeriod,
    uint traitcountToEligibility
  )
  {
    traitcountOfPreviousEligiblePeriod = 0;
    traitcountToEligibility = 0;
    Descriptor memory d = _game.describe(tokenId);
    traitcountOfPreviousEligiblePeriod = _getTraitcountOfPreviousEligiblePeriod(tokenId); 
    uint target = traitcountOfPreviousEligiblePeriod + TRAITCOUNT_DELTA + 1;
    uint traitcountNow = d.traitCount; 
    if (target > traitcountNow) {
      traitcountToEligibility = target - traitcountNow;  
    }
  }
