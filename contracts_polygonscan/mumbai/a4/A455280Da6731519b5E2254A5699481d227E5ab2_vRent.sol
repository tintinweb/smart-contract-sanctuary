...
contract vRent is IvRent, ERC721Holder, ERC1155Receiver, ERC1155Holder { 
     ...
     * - `_nftPrices` and `_dailyLeasePrices` should be between 9999.9999 and 0.0001
     */
    function lease(
    ...
    */
    ...
    function distributePayments(
        ...
        uint256 nftPrice =
            _LeasingRenting.Leasing.leaseAmount *
                _unwrapPrice(_LeasingRenting.Leasing.nftPrice, scale);
        uint256 rentPrice =
    ...
    // distributes the `nftPrices` amount 
    ...
    function manageLease(CallData memory _cd) private {
        ...
        _safeTransfer(
            ...
            _sliceArr(_cd.leaseAmounts, _cd.left, _cd.right, 0)
        );
    }
...

            {
                ...
                uint256 nftPrice =
                    item.Leasing.leaseAmount *
                        _unwrapPrice(item.Leasing.nftPrice, scale);
    ...
    function manageClaimCollateral(CallData memory _cd) private {
        for (uint256 i = _cd.left; i < _cd.right; i++) {
            ...
            _verifyIsNotNull(item.Leasing);
            _verifyIsNotNull(item.renting);
            _verifyIsClaimable(item.renting, block.timestamp);
   ...
   // verifys the NFT standard
    ...
    /**
     * convert the `dailyLeasePrices` and `nftPrices` from bytes4 into decimal
    */
    ...
    */
    ...
    function _verifyIsNull(Leasing memory _Leasing) private pure {
        ...
        require(_Leasing.nftPrice == 0, "vRent::nft price not zero");
    }
    ...
    function _verifyIsNotNull(Leasing memory _Leasing) private pure {
        ...
        require(_Leasing.nftPrice != 0, "vRent::nft price is zero");
    }
    ...
    */
    ...
    function _verifyIsClaimable(Renting memory _renting, uint256 _blockTimestamp)
        private
        pure
    {
        require(
    ...
    */
    ...
    function getLeasing(address nft, uint256 tokenId, uint256 LeaseId) external view returns(Leasing memory) {
    return leasingRenting[
                    keccak256(
                        abi.encodePacked(
                            nft,
                            tokenId,
    ...
    function getRenting(address nft, uint256 tokenId, uint256 LeaseId) external view returns(Renting memory) {
    return leasingRenting[ 
                    keccak256(
                        abi.encodePacked(
                            nft,
