...
contract LaunchpadFactory {
    ...
    event CrowdsaleLaunched(address indexed crowdsaleAddress, IERC20 token, uint256 indexed crowdsaleStartTime);
    ...
    function launchCrowdsale (
     ...
     uint256 _rate,
     uint128 _crowdsaleStartTime,
     uint128 _crowdsaleEndTime,
     uint128 _vestingStartTime,
     uint128 _vestingEndTime,
     uint128 _cliffDurationInSecs) 
    public 
    returns (address)
    {
        require(address(_tokenAddress) != address(0),"Cant be Zero address");
        ...
        require(_inputToken.length > 0, "Invalid input tokens");
        ...
        newCrowdsale.init(_tokenAddress, _inputToken, _amountAllocation, _rate,_crowdsaleStartTime,_crowdsaleEndTime, _vestingStartTime,_vestingEndTime, _cliffDurationInSecs);
                        
        crowdsales.push(CrowdsaleInfo({     //creating a variable newCrowdsaleInfo which will hold value in format that of CrowdsaleInfo 
        ...
        emit CrowdsaleLaunched(address(newCrowdsale), _tokenAddress, _crowdsaleStartTime);
        crowdsaleIndex++;
...
contract Crowdsale is ReentrancyGuard  {
    ...
    */
    ...
    constructor(address _owner,address _launchpad) {
    ...
    public {
        ...
        for(uint256 i = 0; i < _inputToken.length; i++){
            validInputToken[address(_inputToken[i])] = true;
        }
        token = _tokenAddress;
        rate = _rate;
    ...
    function purchaseToken(IERC20 _inputToken, uint256 _inputTokenAmount) external {   
        require(whitelistedAddress[msg.sender], "Not Whitelisted");
        require(_getNow() >= crowdsaleStartTime,"Crowdsale isnt started yet");
        require(validInputToken[address(_inputToken)],"Unsupported Input token");
        if(crowdsaleEndTime != 0){
            require(_getNow() < crowdsaleEndTime, "Crowdsale Ended");
        }
        
        uint8 inputTokenDecimals = _inputToken.decimals();
        uint256 tokenPurchased = inputTokenDecimals >= 18 ? _inputTokenAmount.mul(rate).mul(10**(inputTokenDecimals-18)) : _inputTokenAmount.mul(rate).mul(10**(18-inputTokenDecimals));
        ...
        if(maxUserAllocation != 0) require(vestedAmount[msg.sender].add(tokenPurchased) <= maxUserAllocation);
        ...
        emit TokenPurchase(msg.sender,_inputTokenAmount,tokenPurchased,_inputToken,tokenRemainingForSale);
    }
    ...
    */
     ...
     * Can only be called by the current owner.
     */
    function whitelistUser(address user) public virtual onlyOwner {
     ...
     * Can only be called by the current owner.
     */
    function whitelistUsers(address[] calldata users) public virtual onlyOwner {
     ...
     * Can only be called by the current owner.
     */
    function updateMaxUserAllocation(uint256 _maxUserAllocation) public onlyOwner {
