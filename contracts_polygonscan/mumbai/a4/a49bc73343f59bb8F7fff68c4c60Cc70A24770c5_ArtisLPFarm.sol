...
// File contracts/ARTISLPFarm.sol

pragma solidity ^0.8.0;
contract ArtisLPFarm is IERC777Recipient {
    ...
    string public name = "ArtisLife Network LP Farm";
    ...
    constructor() {
        lpToken = IERC20(0x326C977E6efc84E512bB9C30f76E30c160eD06FB);
...
{        require(amount > 0, "Must stake more than zero.");
        if (stakers[msg.sender].isStaking == true) {
            ...
            stakers[msg.sender].artisBalance = stakers[msg.sender]
                .artisBalance
                .add(toTransfer);
            totalARTISRewards = totalARTISRewards.sub(toTransfer);
        }
        stakers[msg.sender].lpBalance = stakers[msg.sender].lpBalance.add(
            amount
        );
        totalLPTokensLocked = totalLPTokensLocked.add(amount);
    ...
    function unstake(uint256 amount) public {
        require(
            stakers[msg.sender].isStaking =
                true &&
                stakers[msg.sender].lpBalance >= amount,
            "Nothing to unstake"
        );
        uint256 yield = calculateYieldTotal(msg.sender);
        totalARTISRewards = totalARTISRewards.sub(yield);
        totalLPTokensLocked = totalLPTokensLocked.sub(amount);
        stakers[msg.sender].startTime = block.timestamp;
        artisToken.send(
            msg.sender,
            yield.add(stakers[msg.sender].artisBalance),
            ""
        );
        stakers[msg.sender].artisBalance = 0;
        artisToken.send(msg.sender, amount, "");
        stakers[msg.sender].lpBalance = stakers[msg.sender].lpBalance.sub(
            amount
        );
        if (stakers[msg.sender].lpBalance == 0) {
    ...
    function unstakeAll() external {
        require(stakers[msg.sender].lpBalance > 0, "Nothing to unstake");
        uint256 amount = stakers[msg.sender].lpBalance;
        unstake(amount);
    }
    ...
    function emergencyWithdrawLP() external onlyAdmin {
        lpToken.transfer(ADMIN_ROLE, lpToken.balanceOf(address(this)));
    }
    ...
    function setLP(address newLP) external onlyAdmin {
        lpToken = IERC20(newLP);
    }
