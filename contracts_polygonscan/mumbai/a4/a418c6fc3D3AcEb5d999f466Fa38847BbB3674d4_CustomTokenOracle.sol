...
contract CustomTokenOracle is Ownable, IOracle {
    ...
    function consult() external view override returns (uint256) {
        uint256 _priceCustomTokenCollateral = oracleCustomTokenCollateral
            .consult(customToken, PRICE_PRECISION);
        return
            (priceCollateralUsd() * _priceCustomTokenCollateral) /
            PRICE_PRECISION;
    }
    ...
    function setOracleCustomTokenCollateral(
        IPairOracle _oracleCustomTokenCollateral
    ) public onlyOwner {
        require(
            _oracleCustomTokenCollateral != IPairOracle(address(0)),
            "!pairOracle"
        );
