...
contract IrisToken is ERC20("Iris", "IRIS"), Ownable {
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "IRIS::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "IRIS::delegateBySig: invalid nonce"
        );
        require(now <= expiry, "IRIS::delegateBySig: signature expired");
        ...
        returns (uint256)
    {
        require(
            ...
            "IRIS::getPriorVotes: not yet determined"
        );
    ...
    function _delegate(address delegator, address delegatee) internal {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying IRIS (not scaled);
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            "IRIS::_writeCheckpoint: block number exceeds 32 bits"
        );
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    function pendingIris(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 irisReward = multiplier
            .mul(irisPerBlock)
            .mul(pool.allocPoint)
            .div(totalAllocPoint);
            accIrisPerShare = accIrisPerShare.add(
                irisReward.mul(1e18).div(lpSupply)
            );
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accIrisPerShare = pool.accIrisPerShare.add(
            irisReward.mul(1e18).div(lpSupply)
        );
        pool.lastRewardBlock = block.number;
    }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accIrisPerShare).div(1e18).sub(
            user.rewardDebt
        );
        if (pending > 0) {
            safeIrisTransfer(msg.sender, pending);
