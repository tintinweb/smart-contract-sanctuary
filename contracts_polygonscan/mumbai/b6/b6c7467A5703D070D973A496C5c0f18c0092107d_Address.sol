...
struct staking{
 ...
 mapping(address => staking) public bon;
 address[] public bonu;
 
 function claimBonus(uint8 bonusPending ,uint8 bonusRecieved)public pure returns (uint256){
     uint256 totalBonus = bonusPending + bonusRecieved;
     return totalBonus;
 }
 function addBonus(uint8 bonusPending, uint8 totalBonus)public pure returns (uint8){
     uint8 newBonus = bonusPending + totalBonus;
     return newBonus;
 
} 
 
  function transferFrom(address sender, address recipient, uint256 amount) public pure returns (bool) {
        claimBonus(sender, recipient, amount);
        return true;
      
  }
 function claimBonus(address sender, address recipient, uint256 amount)  public pure  {
        require(sender != address(0), " transfer from the zero address");
        require(recipient != address(0), " transfer to the zero address");
        require(amount > 0);
 }
 function approve(address owner, address spender, uint256 amount) public pure returns (bool) {
