...
contract VaultChef is Ownable, ReentrancyGuard, Operators {
    ...
    function _withdraw(uint256 _pid, uint256 _wantAmt, address _to) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_to]; // TODO: get it reviewed
        ...
        if (_wantAmt > 0) {
            uint256 sharesRemoved = IStrategy(poolInfo[_pid].strat).withdraw(_to, _wantAmt); // TODO: get it reviewed
            ...
            pool.want.safeTransfer(msg.sender, _wantAmt); // TODO: get it reviewed
        }
        emit Withdraw(_to, _pid, _wantAmt);// TODO: get it reviewed
    }
