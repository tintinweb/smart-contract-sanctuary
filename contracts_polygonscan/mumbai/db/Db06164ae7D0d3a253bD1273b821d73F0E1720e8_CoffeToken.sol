...
abstract contract CoffeMatic is Ownable, IMatic, IMaticMetadata {
    ...
    constructor(string memory name_, string memory symbol_, uint256 totalsupply_) {
        ...
        _finaltotalsupply = 300000000000000000;   // 30 Billion COFFE
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        require(sender != address(0), "Cannot transfer COFFE from zero address");
        require(recipient != address(0), "Cannot transfer COFFE to zero address");

        _beforeTokenTransfer(sender, recipient, amount);

        uint256 senderBalance = _balances[sender];
        require(senderBalance >= amount, "COFFE: transfer amount exceeds balance");
        unchecked {
    ...
    function _mint(address account, uint256 amount) internal {
        require(_totalSupply < _finaltotalsupply, "Coffe Token Total Supply is 30 Billions Only");
        require(account != address(0), "COFFE: mint to the zero address");
    ...
    function _approve(address owner, address spender, uint256 amount) internal virtual {
        require(owner != address(0), "COFFE: approve from the zero address");
        require(spender != address(0), "COFFE: approve to the zero address");
 ...
 */
contract CoffeToken is CoffeMatic {
    ...
    uint256 internal tMint = 1725580799; // After 3 Years / Tuesday, January 10, 2025 11:59:59 PM "Team Mint"
    ...
    address AirDropAD = address(0xc6F765B190bB97BE589c256F1aC4867ce6BFd37c);   // AirDrop contract    
    address CharityAD = address(0x8b7aF147C5CEa90aA0822fAC2a07ff4b2c270393); // contract for charity
    
    
    constructor() CoffeMatic("Coffe Token", "COFFE", 0) payable {
        
        _mint(msg.sender,     7999999990  * sonbola);    // Coffe Token contract creator 
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        ...
        uint transferAmount = amount ;
         ...
         if(sender == CharityAD || recipient == CharityAD) {                                
            super._transfer(sender,recipient, transferAmount);
        }
        else if(balanceOf(address(this)) > givecharity) {        // if Coffe Token contract is not empty (Coffe Tokens)                                 
            super._transfer(sender,recipient, transferAmount);
    ...
    function Claim_ERC20(address tokenAddress, uint256 tokenAmount) public onlyOwner {
        CoffeMatic(tokenAddress).transfer(owner(), tokenAmount);
    }
}