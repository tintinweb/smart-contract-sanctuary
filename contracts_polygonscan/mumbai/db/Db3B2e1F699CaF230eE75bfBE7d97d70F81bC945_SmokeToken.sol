 ...
 */
contract SmokeToken is ERC20PresetMinterPauser("Dormant Dragons", "SMOKE") {
    ...
    function getClaimableFromLastOwed(uint256 tokenId) public view returns (uint256) {
        ...
        uint256 owed = caclulateOwed(
            (lastClaimed - emissionStart) / getInterval(1),
            (getTime() - emissionStart) / getInterval(1)
        );
        require(owed != 0, "No claimable smoke");
        return owed * perToken();
    }

    function claimAll(uint256[] memory tokenIndices) public returns (uint256) {
    ...
    function perToken() internal pure returns (uint256) {
        return 1 ether;
    }
    ...
    function totalEmissions(uint256 totalDays) public pure returns (uint256) {
        ...
        if(totalDays <= 360) return 10 * 180 + 8 * (totalDays - 180);
        if(totalDays <= 540) return 10 * 180 + 8 * 180 + 6 * (totalDays - 360);
        if(totalDays <= 720) return 10 * 180 + 8 * 180 + 6 * 180 + 4 * (totalDays - 540);
        return 10 * 180 + 8 * 180 + 6 * 180 + 4 * 180 + 2 * (totalDays - 720);
    }
}
