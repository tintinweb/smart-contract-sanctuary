...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 accSandManPerShare;   // Accumulated DELIRIUMs per share, times 1e12. See below.
    ...
    function pendingSandMan(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && pool.lpSupply != 0 && totalAllocPoint > 0) {
            ...
            accSandManPerShare = accSandManPerShare + ((deliriumReward * 1e12) / pool.lpSupply);
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accSandManPerShare = pool.accSandManPerShare + ((deliriumReward * 1e12) / pool.lpSupply);
        pool.lastRewardBlock = block.number;
    }
