...
contract ProxyV1 is Base, Proxy {
  ...
  constructor(string memory name_, string memory symbol_, uint maxSupply_, uint reserved_, uint price_, address[] memory beneficiaries_, uint[] memory ratio_, address impl) {
...
contract NFTv1 is Base, Ownable, ERC721 {
  string private _URI;
  ...
  constructor(string memory name_, string memory symbol_, uint maxSupply_, uint reserved_, uint price_, address[] memory beneficiaries_, uint[] memory ratio_) ERC721(name_, symbol_) {
    require(maxSupply == 0, "inited");
    require(beneficiaries_.length > 0, "no beneficiaries");
    require(beneficiaries_.length == ratio_.length, "invalid ratio length");
    uint total = 0;
...
contract PolygonNFTv1 is NFTv1, ContextMixin, NativeMetaTransaction {
  constructor(string memory name_, string memory symbol_, uint maxSupply_, uint reserved_, uint price_, address[] memory beneficiaries_, uint[] memory ratio_) NFTv1(name_, symbol_, maxSupply_, reserved_, price_, beneficiaries_, ratio_) {
...
contract NFT1155 is Ownable, ERC1155 {
  ...
  function bulkMint(address account, uint[] calldata ids, uint[] calldata amounts) external {
...
contract EthNFTv1 is NFTv1 {
  ...
  constructor(string memory name_, string memory symbol_, uint maxSupply_, uint reserved_, uint price_, address[] memory beneficiaries_, uint[] memory ratio_, address proxy) NFTv1(name_, symbol_, maxSupply_, reserved_, price_, beneficiaries_, ratio_) {
  ...
  function isApprovedForAll(address owner_, address operator) override public view returns (bool) {
    // Whitelist OpenSea proxy contract for easy trading.
    if (proxyRegistry.proxies(owner_) == operator) {
      return true;
    }
    return super.isApprovedForAll(owner_, operator);
  }
