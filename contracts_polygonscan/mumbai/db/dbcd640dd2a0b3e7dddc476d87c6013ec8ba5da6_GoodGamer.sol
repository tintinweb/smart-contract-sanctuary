...
contract GoodGamer is ERC721Enumerable, AccessControl {
    ...
    // INITS
    mapping(uint256 => bool) private drop_inited;
    mapping(uint256 => string) private drop_name;
    mapping(uint256 => uint256) private drop_start_date;
    mapping(uint256 => uint256) private drop_end_date;
    mapping(uint256 => uint256) private drop_max;
    mapping(uint256 => uint256) private drop_price;
    mapping(uint256 => uint256) private token_drop;
    mapping(uint256 => uint256) private drop_token;
    ...
    function initDrop(
        ...
        drop_inited[drop] = true;
        drop_name[drop] = name;
        drop_start_date[drop] = start_date;
        drop_end_date[drop] = end_date;
        drop_max[drop] = max;
    ...
    function mint(uint256 drop, uint256 count) public payable {
        ...
        if (getDropTokensCount(drop) == getDropMax(drop))
            require(true, "All drops have been sold out.");
        require(
        ...
        // Limit numner of nft one can mint
