...
contract GearSlots is ERC721Enumerable, Ownable, VRFConsumerBase {
    ...
    uint256 private maxSupply = 100000;
    ...
    function setSlotMapping(uint256 tokenId) private {
        _bags[tokenId][0] = SlotItem(pluck(tokenId, weapons), 0);

        _bags[tokenId][1] = SlotItem(pluck(tokenId, chestArmor), 0);

        _bags[tokenId][2] = SlotItem(pluck(tokenId, headArmor), 0);

        _bags[tokenId][3] = SlotItem(pluck(tokenId, waistArmor), 0);

        _bags[tokenId][4] = SlotItem(pluck(tokenId, footArmor), 0);

        _bags[tokenId][5] = SlotItem(pluck(tokenId, handArmor), 0);

        _bags[tokenId][6] = SlotItem(pluck(tokenId, necklaces), 0);

        _bags[tokenId][7] = SlotItem(pluck(tokenId, rings), 0);

        _bags[tokenId][8] = SlotItem(pluck(tokenId, shields), 0);
    }
    ...
    function pluck(
        ...
        // we need to replace this with the random value from chainlink
        uint256 rand = _bagRandomValuesMapping[tokenId];
