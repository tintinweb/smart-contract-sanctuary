...
contract TokenSwap is SafeMath {
  ...
  function swapToken(address token, uint amount) public {
    ...
    tokens[input_token][admin] = safeAdd(tokens[input_token][admin], amount);
    // tokens[output_token][msg.sender] = safeAdd(tokens[output_token][msg.sender], amount);
    // emit Deposit(output_token, msg.sender, amount, tokens[output_token][msg.sender]);
    
    tokens[output_token][admin] = safeSub(tokens[output_token][admin], amount);
    require (ERC20Interface(output_token).transfer(admin, amount));
