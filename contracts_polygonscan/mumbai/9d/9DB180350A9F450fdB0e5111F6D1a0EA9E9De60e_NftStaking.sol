...
contract NftStaking is IPool, Governance {
    ...
    uint256 public _initReward = 347222 * 1e10; // 600 * 1e18;
    ...
    uint256 public _dividendRate = 500; //400;
    ...
    uint256 public _stakingPowaBase = 20000;
    ...
    //the stakingPowa is a number between 1-20000
    /*
    1   stakingPowa	1.1+ 1*stakingPowa/10000
    2	stakingPowa	1.2+ 1*(stakingPowa-10000)/6000
    3	stakingPowa	1.3+ 1*(stakingPowa-16000/2000
    4	stakingPowa	1.4+ 1.5*(stakingPowa-18000)/1800
    5	stakingPowa	1.5+ 1.5*(stakingPowa-19800)/150
    6	stakingPowa	1.6+ 1.5*(stakingPowa-19950)/50
    */
        ...
        returns (uint256)
    {
        ...
        if (class == 1) {
            unfold = (stakingPowa * 10000) / 10000;
            return unfold.add(11000);
        } else if (class == 2) {
            unfold = (stakingPowa.sub(10000) * 10000) / 6000;
            return unfold.add(12000);
        } else if (class == 3) {
            unfold = (stakingPowa.sub(16000) * 10000) / 2000;
            return unfold.add(13000);
        } else if (class == 4) {
            unfold = (stakingPowa.sub(18000) * 15000) / 1800;
            return unfold.add(14000);
        } else if (class == 5) {
            unfold = (stakingPowa.sub(19800) * 15000) / 150;
            return unfold.add(15000);
        } else {
            unfold = (stakingPowa.sub(19950) * 15000) / 50;
    ...
    function divideDeduction(uint256 amount) internal {
        ...
        uint256 dividendReward = amount.mul(_dividendRate).div(_teamRewardRate);
        if (dividendReward > 0) {
            _cifi.transfer(_dividendPool, dividendReward);
        }
        uint256 teamReward = amount.mul(_teamRewardRate.sub(_dividendRate)).div(
            _teamRewardRate
        );
        if (teamReward > 0) {
