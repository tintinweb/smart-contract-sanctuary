...
contract VestingFactory {
    ...
    function launchVesting(
        ...
        string calldata _vestingName
    ) external returns(address) {
        require(address(_erc20Token) != address(0), "Incorrect token address");

        LinearVesting linearVesting = new LinearVesting(address(this));
...
contract LinearVesting is ReentrancyGuard, Ownable {
     ...
     */
    constructor(address _vestingFactory) public {
        require(
     ...
     * @param _vestingName Vesting name
     */
    function init(
        IERC20 _token,
        uint256 _start,
