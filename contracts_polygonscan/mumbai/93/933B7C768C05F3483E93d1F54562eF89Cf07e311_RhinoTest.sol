...
contract RhinoTest is ERC721Enumerable, Ownable {
    ...
    function ownerMint() external onlyOwner {
      /* Allow max items to accomodate Dev mint */
      maxItemsPerTx = teamMintTotal;

      /* Complete Dev Mint Automatically - to stated amount */
      _mintWithoutValidation(msg.sender, maxItemsPerTx);
    ...
    function _mintWithoutValidation(address to, uint amount) internal {
      ...
      for (uint i = 0; i < amount; i++) {
        ...
        require(revealed);
        emit PermanentURI(string(abi.encodePacked(_baseTokenURI,'/',i)), totalSupply());
      }
    }
