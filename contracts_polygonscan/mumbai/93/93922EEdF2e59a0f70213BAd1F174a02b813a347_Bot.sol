...
contract Bot is OwnableUpgradeable {
    ...
    event CreditCharged(address indexed owner, uint256 amount);
    //Summoner Task
    ...
    function run(
        ...
        require(credits[summonerOwner] >= 1 ether, "Bot: low credit");
        ...
        if (runned) {
            uint256 fee = (startGasLeft - gasleft()) * tx.gasprice * feePercentage / 100;
            credits[summonerOwner] -= fee;
            fees += fee;

            emit CreditCharged(summonerOwner, fee);
        }
