    ...
    ERC721Pausable
{
    ...
    constructor(
        ...
        string memory extensionURI_,
        uint256 hardCap_
    ) ERC721(name, symbol) {
        _baseTokenURI = baseTokenURI_;
        _extensionURI = extensionURI_;
        _cap = hardCap_;
        _grantRole(DEFAULT_ADMIN_ROLE,  _msgSender());
        _grantRole(MINTER_ROLE, _msgSender());
        _grantRole(PAUSER_ROLE, _msgSender());
        _grantRole(BURNER_ROLE, _msgSender());
        _grantRole(NFT_ADMIN_ROLE, _msgSender());
    }

    modifier onlyWithinRange(uint256 id){
        require (id>0 && id<= _cap);
        _;
    }
    ...
    function toggleUriState() public virtual onlyRole(DEFAULT_ADMIN_ROLE){
        _uriOpen = !_uriOpen;
    }

    function setBaseTokenURI(string memory newTokenURI) public virtual onlyRole(NFT_ADMIN_ROLE){
        require (_uriOpen, "NFT: FINAL URI SET CANNOT CHAGE");
        _baseTokenURI = newTokenURI;
    }

    function setExtensionURI(string memory newExtensionURI) public virtual onlyRole(NFT_ADMIN_ROLE) {
        require (_uriOpen, "NFT: FINAL URI SET CANNOT CHAGE");
        _extensionURI = newExtensionURI;
    }

    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        string memory uri = super.tokenURI(tokenId);
        return bytes(uri).length >0 ? string(abi.encodePacked(uri,_extensionURI)) : "" ;
    }
     ...
     */
    function mint(address to, uint256 tokenId) public virtual onlyRole(MINTER_ROLE) onlyWithinRange(tokenId){
        _mint(to, tokenId);
    }
