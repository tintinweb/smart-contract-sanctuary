...
contract SliverAllocator is AccessControlEnumerable {
    ...
    function mintAvailableTokens() public onlyRole(MANAGER_ROLE) {
        if (SLIVER.distributionsAvailable()) {
            SLIVER.distributeAllTokens();
        }
    }

    function allocateAvailableFunds() public onlyRole(MANAGER_ROLE) {
        CreatorsFund creators = CreatorsFund(creatorsAddress);
        LiquidityFund liquidity = LiquidityFund(liquidityAddress);
        ...
        // liquidity
        if (liquidity.hasUnallocatedTokens()) {
            liquidity.allocateTokens();
        }
        // marketing
    ...
    function setLiquidityFund(address fundAddress)
        public
        onlyRole(MANAGER_ROLE)
    {
        liquidityAddress = fundAddress;
    }
