...
// *VARS*
    ...
    // Cycles based on date from start, uncontrollable. 
...
///*BATTLE LEDGER*
// log all battle instances
    struct Battle {uint id; uint16 attacker; uint16 defender; bool successfulAttack; uint timestamp;}
///*MAPPINGS*
    mapping(uint8 => Battle[]) public battleLedger; 
...
///*FUNCTIONS*
    // risky biz
    // Basic checks provided by require, most of the heavy lifting provided by deployOrEvacHandler
    ...
    function attack(uint16 sentryId) external onlyTokenOwner(sentryId, msg.sender) validInfectedSentry(sentryId) {
        ...
        bool successfulAttack = battle(sentryId, target.sentryId, isInParty);
        if(successfulAttack) {
        // zombie is rewarded, sentry is penalized
            ...
            idToStats[target.sentryId].successfulDefends++;
            allStats[getStatsIndex(target.sentryId)].successfulDefends++;
            
        }
        publishToBattleLedger(sentryId, target.sentryId, successfulAttack);
    }
...
///*HELPERS*
    ...
    function battle(uint16 attackerId, uint16 defenderId, bool isInParty) private returns(bool) {
        // bool return refers to if the ATTACKER won
        // Determine set effect. Parties of same type (all human) will recieve a bonus
        Sentry memory attacker = factory.getSentryTraits(attackerId);
        Sentry memory defender = factory.getSentryTraits(defenderId);
        // Set mult provides bonus to sentries who deploy with others of same species. 
        uint setMult = 100; 
        if(isInParty) {
            setMult = party.generateSetEffect(defenderId);
        }
        uint attackStat = (attacker.attack*((battleMultipliers[attacker.species][currentEnvironment]*setMult)/100));
    ...
    function publishToBattleLedger(uint16 attacker, uint16 defender, bool successfulAttack) private {
        battleLedger[currentSeason][battleLedger[currentSeason].length] = Battle(battleLedger[currentSeason].length, attacker, defender, successfulAttack, block.timestamp);
        emit Attack(attacker, defender, successfulAttack);
...
///*INTERFACE*
    ...
    function deploy(uint16[] memory sentryIdArray) external onlyController(msg.sender) {
        for(uint i = 0; i < sentryIdArray.length; i++) {
        require(!factory.getSentryTraits(sentryIdArray[i]).infected, "Infected Sentries cannot deploy!");
            deployOrEvacHandler(sentryIdArray[i], 0, true);
        }
    }
    // This interface is to be used by the deployment party contract
    ...
    function editRiskCode(uint16 sentryId ,uint8 newCode) external onlyController(msg.sender){
        ...
        allStats[getStatsIndex(sentryId)].riskCode = newCode;
        emit StatsUpdated(sentryId);
    }

    function getAllAvailInfected() external view returns(uint) {
        uint counter = 0;
        for(uint i = 0; i < allStats.length; i++) {
            if(factory.getSentryTraits(allStats[i].sentryId).infected) {
                if(block.timestamp > allStats[i].cooldownTimestamp) {
                    counter++;
                }
            }
        }
    ...
    function viewAverageDefense() external view returns(uint) {
