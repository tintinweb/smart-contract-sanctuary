...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
  ...
  // mapping for token creator
    ...
    mapping (uint256 => bool) public bountyNFTs;
    ...
    */
    ...
    function getDividends(address account, uint256 tokenId) public view returns(uint256){
        uint256 stakeTime = block.timestamp.sub(holdingTime[account][tokenId]);
        return stakeTime;
    }
...
contract CryptocornNFT721V2 is ERC721 {
    ...
    constructor (string memory name, string memory symbol, string memory tokenURIPrefix) ERC721 (name, symbol, msg.sender){
        tokenCounter = 1;
        signer = msg.sender;
    ...
    function createCollectible( string memory tokenURI, uint256 fee, Sign memory sign, bool _bountyNFT) public payable returns (uint256) {
        require(balanceOf(msg.sender) < 5,"you reached the minting limit");
        require(tokenCounter <= totalSupply(),"ERC721: Minting is not available");
        require(msg.value >= mintingFee, "Minting: value must be equal to MintingFee ");
        ...
        if(_bountyNFT) {
            bountyNFTs[newItemId] = true;
            holdingTime[msg.sender][newItemId] = block.timestamp;
        }
        tokenCounter += 1;
        return newItemId;
    }
    ...
    function enableMint() public onlyOwner returns(bool){
        mintEnable = true;
        revealStartTime = block.timestamp + 1 hours;
        return(true);    
    }
    ...
    function addWhiteListed(address[] memory account) public onlyOwner returns(bool) {
        for(uint i = 0; i < account.length; i++) {
            require(account[i] != address(0),"whitelisted account is a zero address");
            whitelisted[account[i]] = true;
        }
        return true;
    }
    
    function removeWhiteListed(address account) public onlyOwner returns(bool) {
