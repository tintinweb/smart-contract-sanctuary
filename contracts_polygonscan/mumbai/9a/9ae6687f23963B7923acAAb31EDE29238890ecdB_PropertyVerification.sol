...
contract PropertyVerification is Ownable {
    ...
    mapping(uint256 => propertyData) properties;
    ...
    /**
     * @dev create a new verified property Id
     */
    function verify(address propertyOwnerAddress)
        external
        onlyWhitelisted
        returns (uint256)
    {
        ...
        newProperty.propertyOwner = propertyOwnerAddress;
        newProperty.verified = true;

        properties[newPropertyId] = newProperty;

        emit newPropertyVerified(
    ...
    /**
     * @dev if a property who's verified status has been paused
     *  it can be unpaused with this function
     * @param propertyId property id to be unpaused
     */
    function unpauseVerify(uint256 propertyId) external onlyOwner {
