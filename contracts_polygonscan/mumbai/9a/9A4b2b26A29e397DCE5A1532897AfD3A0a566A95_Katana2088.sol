...
import "./Samurai2088.sol";

contract Katana2088 is ERC20Capped, Ownable {
    mapping(address => uint256) public samuraisByHolder;
    mapping(address => uint256) public weights;
    uint256 public totalWeight;
    address[] public samuraiHolders;

    constructor(uint256 supplyCap)
        ERC20("Katana", "KAT")
    ...
    function samuraiHoldersCount() public view returns (uint256) {
        return samuraiHolders.length;
    }

    function airdrop(address payable samuraiContractAddress, uint256 amount)
        external
        onlyOwner
    {
        return _airdrop(samuraiContractAddress, amount, 2);
    }

    function airdropWithCustomParams(
        address payable samuraiContractAddress,
        uint256 amount,
        uint256 p
    ) external onlyOwner {
        return _airdrop(samuraiContractAddress, amount, p);
    }
    ...
    function _airdrop(
        ...
        refreshSamuraiHolders(samuraiContractAddress);

        totalWeight = 0;
        for (uint256 i = 0; i < samuraiHoldersCount(); i++) {
            address holder = samuraiHolders[i];
            uint256 weight = samuraisByHolder[holder]**p;
        ...
        for (uint256 i = 0; i < samuraiHoldersCount(); i++) {
            address holder = samuraiHolders[i];
            uint256 amount = (weights[holder] * totalAmount) / totalWeight;
            _mint(holder, amount);
        }
    }

    function refreshSamuraiHolders(address payable samuraiContractAddress)
        public
    {
        _resetSamuraiHolders();
        Samurai2088 s = Samurai2088(samuraiContractAddress);
        for (uint256 i = 0; i < s.totalToken(); i++) {
            address owner = s.ownerOf(i + 1);
            if (samuraisByHolder[owner] == 0) {
                samuraiHolders.push(owner);
            }
            samuraisByHolder[owner] += 1;
        }
    }

    function _resetSamuraiHolders() internal {
        uint256 count = samuraiHoldersCount();
        for (uint256 i = 0; i < count; i++) {
            address holder = samuraiHolders[i];
            delete samuraisByHolder[holder];
        }
        for (uint256 i = 0; i < count; i++) {
            samuraiHolders.pop();
        }
