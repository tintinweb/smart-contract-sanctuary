...
contract Traitz is ERC721, ERC721Burnable, ERC721Enumerable, Ownable, VRFConsumerBase, ContextMixin {
  ...
  uint256 private LINK_FEE = 0.0001 * 10 ** 18; // 0.0001 LINK;
  bytes32 private LINK_KEY_HASH;
  address private _minter;
  uint256 private _tokenIds;
  string private _baseTokenURI;
  string private _contractURI;
  string private _metadataURI;
  mapping(bytes32 => uint256) private _traitzRequestIds;
  mapping(uint256 => bytes32) private _traitz;
  ...
  function setMinter(address minter) public onlyOwner {
    require(_minter == address(0), "Minter has already been set");
  ...
  function trait(uint256 tokenId) public view returns (bytes32) {
    ...
    require(_traitz[tokenId] != bytes32(0), "Randomness has not been fulfilled");

    return _traitz[tokenId];
  }

  function _setTrait(uint256 tokenId) internal {
    require(LINK.balanceOf(address(this)) >= LINK_FEE, "Not enough LINK");

    _traitzRequestIds[requestRandomness(LINK_KEY_HASH, LINK_FEE)] = tokenId;
  }

  function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
    _traitz[_traitzRequestIds[requestId]] = bytes32(randomness);
  }
