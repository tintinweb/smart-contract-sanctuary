...
import {IChildToken} from "./common/IChildToken.sol";
import {NativeMetaTransaction} from "./common/NativeMetaTransaction.sol";
import {ContextMixin} from "./common/ContextMixin.sol";

contract ChildBridgableToken is
    ERC721,
    IChildToken,
    AccessControlMixin,
    ...
    ContextMixin
{
    ...
    constructor(
        ...
        _setupContractId("ChildBridgableToken");
    ...
    function withdraw(uint256 tokenId) external {
        require(_msgSender() == ownerOf(tokenId), "ChildBridgableToken: INVALID_TOKEN_OWNER");
        _burn(tokenId);
    }
    ...
    function withdrawBatch(uint256[] calldata tokenIds) external {
        uint256 length = tokenIds.length;
        require(length <= BATCH_LIMIT, "ChildBridgableToken: EXCEEDS_BATCH_LIMIT");
        for (uint256 i; i < length; i++) {
            uint256 tokenId = tokenIds[i];
            require(_msgSender() == ownerOf(tokenId), string(abi.encodePacked("ChildBridgableToken: INVALID_TOKEN_OWNER ", tokenId)));
            _burn(tokenId);
        }
