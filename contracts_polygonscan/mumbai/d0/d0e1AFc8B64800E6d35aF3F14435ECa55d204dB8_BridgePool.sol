...
contract BridgePool is AccessControl, ReentrancyGuard, Multicall, EIP712, IBridgePool {
    ...
    bytes32 public constant WITHDRAW_TYPEHASH = keccak256("Withdraw(bytes,address,uint256,uint256,address)");

    event Deposit(address sender, address token, uint amount, uint8 to, bool bonus, bytes receiver);
    event Withdraw(address token, uint amount, bytes id, address receiver);
    ...
    function deposit(
        ...
        emit Deposit(msg.sender, address(token), amount, to, bonus, receiver);
    }
    ...
    function withdraw(
        ...
        if (signature.length == 0) {
            ...
            bytes32 digest = _hashTypedDataV4(keccak256(abi.encode(WITHDRAW_TYPEHASH, id, token, amount, bonus, receiver)));
        ...
        emit Withdraw(address(token), amount, id, receiver);
    }
