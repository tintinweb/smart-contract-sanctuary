...
contract ERC721Tradable is ERC721Full, Ownable {
	...
	// Optional mapping for token URIs
  	mapping(uint256 => string) private _tkURIs;

	mapping(uint => bool) private _idExistTracker;
     ...
     */
    function mintTo(address _to, uint256 tkId, string memory uri) public onlyOwner {
        // require that the id is unique (does not exist already)
        require(!_idExistTracker[tkId]);
    ...
    function setTkUri1(uint256 tkId, string memory uri) public onlyOwner {
        _tkURIs[tkId] = uri ;
    }
...
// File: contracts/GreatArtNFT.sol
...
/**
 * @title GreatArtNFT
 * GreatArtNFT - a contract for Great Art semi-fungible tokens.
 */
contract GreatArtNFT is ERC721Tradable {
    constructor(address _proxyRegistryAddress)
        public
        ERC721Tradable("GreatArt", "GRT", _proxyRegistryAddress)
    {}
