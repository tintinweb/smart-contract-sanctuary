...
import "./INewentProxy.sol";
import "@openzeppelin/contracts/access/Ownable.sol";

contract NewentProxy is INewentProxy, Ownable {
    ...
    function setMinter(address _owner, bool state) external onlyOwner {
        minters[_owner] = state;
    }


    function setOperator(address _operator, bool _state) external onlyOwner {
        operators[_operator] = _state;
    }

    function authorised(address _owner) external view override returns (bool) {
        require(operators[msg.sender],"Unauthorised Access");
