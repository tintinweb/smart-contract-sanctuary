    ...
    ContextMixin
{
    ...
    constructor() public ERC721("Korean Consonants Card", "KoCoCard") {
        _setupContractId("KoCoCardChild");
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
        // childChainManagerProxy = _msgSender(); // temporal
        ...
        _initializeEIP712("Korean Consonants Card");
        deployer = _msgSender();
    }
    ...
    // test purpose only, do not use in production
    function deposittest(address user, uint256 tokenId) external {
        require(_msgSender() == deployer, "You're not allowed to deposit-test");
        require(_seeds[tokenId] == 0, "Seed exists in the tokenId");
        _seeds[tokenId] = genSeed(tokenId);
        safeMint(user, tokenId);
        emit Transfer(address(0), user, tokenId);
    ...
    // SHOULD BE CUSTOMIZED - end


    //
    // Korean consonancts card contract codes
    //
