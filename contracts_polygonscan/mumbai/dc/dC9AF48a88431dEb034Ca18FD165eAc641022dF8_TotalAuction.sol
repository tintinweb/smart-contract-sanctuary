...
// File: contracts/TotalAuction.sol
...
contract TotalAuction is VRFConsumerBase, Ownable {
    uint256[] public participants;

    uint256 public minimalCost;
    uint8 public maxRange;

    uint256 public startTimestamp;
    uint256 public midTimestamp;
    ...
    function startAuction() external onlyOwner {
        ...
        midTimestamp = startTimestamp;// + 2*86400;
        auctionState = 1;
    }
   ...
   */
    ...
    function addBid(
        ...
        require(auctionState == 1, 'auction not active');
        bids.push(
            BID(
                auctionID,
                projectID,
    ...
    function fulfillRandomness(
        ...
        finalTimestamp =
            midTimestamp +
            (randomness % (endTimestamp - midTimestamp));
       finalizeAuction();
    }
    ...
    */
    function finalizeAuction() public {
        ...
        for (i = 0; i < totalBids; i++) {
            ...
            if (bid.timestamp > finalTimestamp) break;
        ...
        for (i = 0; i < totalBids; i++) {
            BID storage bid = bids[i];
            if (
                bid.timestamp <= finalTimestamp && bid.projectID == winnerProjectID
            ) {
                if (lockingFundsIndex[bid.owner][bid.tokenAddress] > 0) {
                    ...
                    lockingFundsIndex[bid.owner][bid.tokenAddress] =
                        lockingFunds.length +
                        1;
                    lockingFunds.push(TokenFund(bid.owner, bid.tokenAddress, bid.amount, bid.chainID));
                }
            } else {
                if (claimFundsIndex[bid.owner][bid.tokenAddress] > 0) {
                    ...
                    claimFundsIndex[bid.owner][bid.tokenAddress] =
                        claimFunds.length +
                        1;
                    claimFunds.push(TokenFund(bid.owner, bid.tokenAddress, bid.amount, bid.chainID));
                }
            }
        }
