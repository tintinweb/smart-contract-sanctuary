...
contract NFT is ERC721URIStorage, IERC721Enumerable, Governance {
    address contractAddress;
    mapping(address => bool) public _minters;

    constructor(address marketplaceAddress, address minterAddress) ERC721("Metaverse", "METT") {
        contractAddress = marketplaceAddress;
        _minters[minterAddress] = true;
    }

    function createToken(address _to, string memory tokenURI) public returns (uint) {
        require(_minters[msg.sender], "!minter");

        uint256 newItemId = _allTokens.length +1;

        _mint(_to, newItemId);
        _setTokenURI(newItemId, tokenURI);
        if (!isApprovedForAll(_to, contractAddress)){
            _setApprovalForAll(_to, contractAddress, true);
        }
        return newItemId;
    }

    function updateTokenURI(uint256 _tokenId, string memory _uri) public onlyGovernance {
     ...
     * Requirements:
     *
     * - The caller must own `tokenId` or be an approved operator or minter.
     */
    function burn(uint256 tokenId) public virtual {
        //solhint-disable-next-line max-line-length
        require(_isApprovedOrOwner(_msgSender(), tokenId) || _minters[msg.sender], "ERC721Burnable: caller is not owner nor approved nor minter");
        _burn(tokenId);
    }
}
