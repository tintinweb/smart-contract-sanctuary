...
abstract contract BaseStrategyMasterChef is BaseStrategy {
    ...
    function getHarvestable() external override view returns (uint256) {
        // replace with pending spade
        return IMasterChef(rewards).pendingSpade(poolId, address(this));
    }
...
abstract contract StrategyFarmBase is BaseStrategyMasterChef {
    ...
    event PerformanceFeeDistributed(address _to, uint256 _totalAmount, uint256 _tranferAmount);
        ...
        if (_token1Balance > harvestCutoff){
            ...
            if (_want > 0) {
                ...
                if (performanceFee>0) {
                    ...
                    emit PerformanceFeeDistributed(strategist,_want,performanceFee);
                }
            }
        }
        // Stake the LP tokens
