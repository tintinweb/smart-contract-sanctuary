...
contract TMAPStacking is ReentrancyGuard, Ownable {
  ...
  /**
   * @dev used to for the UI to retrieve pending rewards for a certain collector (NFT holder)
   */
  function pending(address _collector) external view returns (uint256) {
    ...
    return IERC721(dsp).balanceOf(_collector) * tmapPerRound * roundsToReward * 1e18;
  }
  ...
  function allPending() external view returns (uint256) {
    ...
    return mintedTokens.length * tmapPerRound * roundsToReward * 1e18;
  }
