...
contract ERC721Namable is ERC721Enumerable {
        ...
        virtual
    {
        ...
        require(_newBirthdate > birthdate[_tokenId], "Invalid Birthdate");
...
contract BabyApesRescue is ERC721Namable, Ownable {
    ...
    // Track stolen id type and total count
    ...
    // Tracks whether NFT is already revealed or no
    ...
    function instantReveal(uint256 id) external {
        require(id > 2500 && id <= _tokenId, "Invalid token id");
        require(ownerOf(id) == msg.sender, "Caller not owner");
    ...
    function setStolenCountAndType(uint256 _id) private {
        if (_id <= 10000) {
            if (isMonster(_id)) {
                monsterStolenCount++;
            } else {
    ...
    // Returns the addresses that own any monster
    function getMonsterOwners() external view returns (address[] memory) {
        address[] memory result = new address[](monsterList.length);

        for (uint256 index; index < monsterList.length; index += 1) {
            result[index] = ownerOf(monsterList[index]);
        }
    ...
    function _getBarBurnAmount(uint256 index) internal view returns (uint256) {
        uint256 currentSupply = _tokenId + index;
    ...
    function migrateMint(
        ...
        require(_to == msg.sender, "Unauthorized Caller");
        require(
    ...
    function handleMint(address _to, uint256 _count) private {
        ...
        _tokenId++;
        if (isSteal) {
            setStolenCountAndType(_tokenId);
        }

        if (isMonster(_tokenId)) {
            monsterList.push(_tokenId);
            monsterBalance[_to]++;
        } else {
            cloneBalance[_to]++;
        }
        _safeMint(_to, _tokenId);
        emit BabyApesRescueMinted(_tokenId, msg.sender, _to, isSteal);
    }
