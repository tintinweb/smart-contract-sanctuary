...
contract NidhiTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SGURU }
    ...
    uint public sGURUQueue; // Delays change to sGURU address
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sGURU ).balanceOf( msg.sender ); // Can only borrow against sGURU held
    ...
    /**
        ...
        @param _amount uint
     */
    function repayDebtWithGURU( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );
    ...
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            ...
            value_ = _amount.mul( 10 ** IERC20( GURU ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SGURU ) { // 9
