...
contract PricePrediction is Ownable, Pausable {
    ...
    struct RoundTokenDetails {
        uint256 winTokenRewards;
        uint256 loseTokenRewards;
        uint256 lossBaseCalAmount;
    }

    struct Round {
        uint256 epoch;
        RoundTokenDetails roundTokenDetails;
        uint256 startBlock;
        uint256 lockBlock;
        uint256 endBlock;
        int256 lockPrice;
    ...
    //tokens distributed per betting round
    uint256 public winTokenRate = 1;
    uint256 public loseTokenRate = 10; 
    ...
    event Claim(address indexed sender, uint256 indexed currentEpoch, uint256 amount, uint256 amountToken);
    event ClaimTreasury(uint256 amount);
    event RatesUpdated(uint256 indexed epoch, uint256 rewardRate, uint256 treasuryRate);
    event TokenRatesUpdated(uint256 indexed epoch, uint256 winTokenRate, uint256 loseTokenRate);
    ...
    function setTokenRate(uint256 _winTokenRate, uint256 _loseTokenRate) external onlyAdmin {
        winTokenRate = _winTokenRate;
        loseTokenRate = _loseTokenRate;

        emit TokenRatesUpdated(currentEpoch, winTokenRate, loseTokenRate);
    }
    ...
    function claim(uint256 epoch) external notContract {
        ...
        emit Claim(msg.sender, epoch, reward, 0);
    }
    ...
    function batchClaim(uint256 epoch, address user) internal {
        ...
        if (rounds[epoch].oracleCalled) {
            ...
            if(claimable(epoch, user)) { //if winning round
                ...
                tokenReward = ledger[epoch][user].amount.mul(winTokenRate).div(round.rewardBaseCalAmount);
            } else {
                tokenReward = ledger[epoch][user].amount.mul(loseTokenRate).div(round.roundTokenDetails.lossBaseCalAmount);
            } 
        }
        ...
        if (tokenReward > 0 && TUNA.balanceOf(address(this)) > tokenReward) {
            TUNA.transfer(address(user), tokenReward);
        }

        emit Claim(user, epoch, reward, tokenReward);
    }
    ...
    function withdrawTokens() external onlyAdmin {
        uint256 currentTokenAmount = TUNA.balanceOf(address(this));
        TUNA.transfer(adminAddress, currentTokenAmount);
    }
    ...
    function _calculateRewards(uint256 epoch) internal {
        ...
        if (round.closePrice > round.lockPrice) {
            ...
            lossBaseCalAmount = round.bearAmount;
        ...
        // Bear wins
            ...
            lossBaseCalAmount = round.bullAmount;
            rewardAmount = round.totalAmount.mul(rewardRate).div(TOTAL_RATE);
            treasuryAmt = round.totalAmount.mul(treasuryRate).div(TOTAL_RATE);
        }
        // House wins
        ...
        round.roundTokenDetails.lossBaseCalAmount = lossBaseCalAmount;
        round.roundTokenDetails.winTokenRewards = winTokenRate;
        round.roundTokenDetails.loseTokenRewards = loseTokenRate;
        // Add to treasury
    ...
    function _distributeRewards(uint256 epoch) internal {
        ...
        for (uint i=0; i<round.usersThisRound.length; i++) {
            ...
            batchClaim(epoch, user); //pass round, need way to get executer reward added as well
        }
    }
