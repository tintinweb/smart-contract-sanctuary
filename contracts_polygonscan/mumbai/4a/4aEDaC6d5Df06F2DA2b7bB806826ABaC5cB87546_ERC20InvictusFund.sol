...
contract ERC20InvictusFund is OwnableUpgradeable, ERC20PresetMinterPauserUpgradeable {
    ...
    function requestRedemption(uint256 withdrawAmount, string memory redemptionCurrency) external onlyWhitelisted {
        ...
        burn(withdrawAmount);
        redemptions.push(Redemption(withdrawAmount, block.timestamp, redemptionCurrency, msg.sender));
        emit RedemptionRequest(withdrawAmount, block.timestamp, redemptionCurrency, msg.sender);
    }
    ...
    function addCurrency(address stableContractAddress, uint256 numerator, uint256 decimals) external onlyPricingOracle {
    ...
    modifier onlyBurner(){
        _roleRequire(BURNER_ROLE, 'ERC20InvictusFund: must have burner role to burn', msg.sender);
        _;
    }

    modifier onlyPricingOracle() {
        _roleRequire(PRICING_ORACLE_ROLE, 'ERC20InvictusFund: must have pricing oracle role', msg.sender);
        _;
    }

    modifier onlyAdjustable() {
        _roleRequire(ADJUSTABLE_ROLE, 'ERC20InvictusFund: must have adjustable role', msg.sender);
        _;
    }

    modifier onlyInvestmentOracle() {
        _roleRequire(INVESTMENT_ORACLE_ROLE, 'ERC20InvictusFund: must have Investment oracle role', msg.sender);
        _;
    }
