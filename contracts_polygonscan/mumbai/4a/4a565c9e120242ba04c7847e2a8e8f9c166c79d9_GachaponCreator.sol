...
contract GachaponCreator is IGachaponCreator {
  address public gachaponDeployer;

  constructor(address _gachaponDeployer) {
    gachaponDeployer = _gachaponDeployer;
  }

  /// @inheritdoc IGachaponCreator
  function createGachapon(string memory _machineTitle, address _currency) override external returns (address) {
    require(address(gachaponDeployer) == address(msg.sender), "Unauthorized");

    Gachapon _gachapon = new Gachapon(
      _machineTitle, 
      _currency, 
      gachaponDeployer, 
      IOracleManager(msg.sender).oracleVRFCoordinator(), 
      IOracleManager(msg.sender).oracleToken()
    );

    _gachapon.transferOwnership(msg.sender);

    return address(_gachapon);
  }
}
...
contract Gachapon is PrizePoolManager, StateManager, IGachapon, VRFConsumerBase {
  ...
  constructor(string memory _gachaponTitle, address _currency, address _deployer, address _VRFCoordinator, address _linkToken) 
  ...
  function play(uint8 _times) external override {
    ...
    require(playOncePrice > 0, "The price of a gachapon can't be 0");
    require(_times > 0 && _times <= 10, "You can only play between 1 and 10 times");
    require(_times <= prizePoolSize - _pendingRoundsAmount, "Not enough NFTs left to play");
    ...
    _rounds[_requestId].player = msg.sender;
  ...
  function fulfillRandomness(bytes32 _requestId, uint256 _randomness) internal override {
    ...
    for (uint8 _i = 0; _i < _rounds[_requestId].times; _i++) {
      uint256 _randomNumber = _expandRandomNumber(_randomness, prizePoolSize - _i);
      _rounds[_requestId].prizes[_i] = prizePool[_randomNumber];
      nfts[prizePool[_randomNumber]].amount--;
      prizePool[_randomNumber] = prizePool[prizePoolSize - 1 - _i];
    }

    prizePoolSize -= _rounds[_requestId].times;
  ...
  /// @dev Will always return an array with 10 elements, but only '_rounds[_requestId].times' of them will have a valid value
  function getRoundPrizes(bytes32 _requestId) external view override returns (Nft[10] memory _prizes) {
    for(uint _i = 0; _i < _rounds[_requestId].times; _i++) {
      _prizes[_i] = nfts[_rounds[_requestId].prizes[_i]];
    }
  ...
  function claimPrizes(bytes32 _requestId) external override {
    ...
    for (uint i = 0; i < _rounds[_requestId].times; i++) {
      ...
      nfts[_rounds[_requestId].prizes[i]].amount--;
    }
...
/// @author KirienzoEth for DokiDoki
contract StateManager is Ownable, IDeployerControlledState {
  ...
  /// @dev the address of the factory that deployed this gachapon
  ...
  function ban() external override onlyDeployer {
    isBanned = true;
    emit BanStatusChanged(isBanned);
  }

  /// @inheritdoc IDeployerControlledState
  function unban() external override onlyDeployer {
    isBanned = false;
    emit BanStatusChanged(isBanned);
  }

  /// @inheritdoc IDeployerControlledState
  function setArtistProfitsSharing(address[] memory _addresses, uint16[] memory _rates) external override onlyDeployer {
  ...
  /// @inheritdoc IDeployerControlledState
  function setArtistRate(uint16 _rate) external override onlyDeployer {
  ...
  /// @inheritdoc IDeployerControlledState
  function setTokenomicsRate(uint16 _rate) external override onlyDeployer {
  ...
  /// @inheritdoc IDeployerControlledState
  function setDaoRate(uint16 _rate) external override onlyDeployer {
