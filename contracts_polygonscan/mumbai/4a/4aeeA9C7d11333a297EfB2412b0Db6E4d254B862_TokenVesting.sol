...
contract TokenVesting is Ownable {
    ...
    function suspendLockTransferToReceiver(uint256 id) external onlyOwner returns (bool) {
        require(id > 0 && id <= totalVestings, "Id out of bounds");
        VestingDetails storage vestingDetail = vestingID[id];
        require(!vestingDetail.expired, "ID expired");
        require(
    ...
    function suspendLockTransferToOwner(uint256 id) external onlyOwner returns (bool) {
        require(id > 0 && id <= totalVestings, "Id out of bounds");
        VestingDetails storage vestingDetail = vestingID[id];
        require(!vestingDetail.expired, "ID expired");
        require(
