...
contract l4vender is Context, IBEP20, IBEP20Metadata,Ownable,Pausable {
    ...
    uint256 public _maxTransationAmount = 1000000 * 10**_decimals;
    ...
    function IsAccountBlackList(address _isBlackList)view public returns(bool){
        return tokenBlacklist[_isBlackList];
    }
    /**
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual whenNotPaused {
        if(sender!=owner() && recipient!= owner()){
            require(amount <= _maxTransationAmount,"Transaction limit Exceeded");
        }
        require(tokenBlacklist[msg.sender] == false ,"You are availabled as Black List");
    ...
    function _mint(address account, uint256 amount) internal virtual {
     ...
     function _blackList(address _address, bool _isBlackListed) public onlyOwner() {
    ...
    function setMaxTransactionAmount(uint256 _maxAmount) external onlyOwner() {
        _maxTransationAmount = _maxAmount;
    }
