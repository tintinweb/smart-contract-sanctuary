...
contract EnvoyStaking is Ownable {
    ...
    */
    function calculateRewardPerPeriod() public view returns(uint rewardPerPeriod){
            if((totalStakingBalance(latestRewardPeriod, 1) > 0) && stakingToken.balanceOf(address(this)) 
                > totalStakingBalance(latestRewardPeriod, 0)
                    + totalNewStake(0)
                    + totalLockedRewards(0)){
                rewardPerPeriod = (stakingToken.balanceOf(address(this))
                    - totalStakingBalance(latestRewardPeriod, 0)
                    - totalNewStake(0)
                    - totalLockedRewards(0)) * decimalPrecision
    ...
    function withdrawFunds() public {
        ...
        uint fee = (cooldown > 0) ? stakeholder.releaseAmount * timeToEnd * earlyWithdrawalFee / (cooldown * decimalPrecision * 100) : 0;
