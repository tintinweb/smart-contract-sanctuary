...
contract NFTicket is AccessControl, Ownable, ERC721Enumerable {
    ...
    string public burnedURIString = "burnedURIString";
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        if (address(nfTicketContest) != address(0) && nfTicketContest.isTokenBurned(tokenId)) {
            return string(abi.encodePacked(burnedURIString, tokenId.toString(), ".json"));
        } else {
    ...
    function transferFrom(
        ...
        if (
            address(nfTicketContest) != address(0) && 
            nfTicketContest.isTokenBurned(tokenId) &&
            _exists(tokenId)
        ) {
            _burn(tokenId);
        } else {
    ...
    */
    ...
    function setBurnURI(string memory _newBurnURI) external onlyOwner {
        burnedURIString = _newBurnURI;
    }
