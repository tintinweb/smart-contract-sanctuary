...
contract GameMaster is Initializable, AccessControlUpgradeable, PausableUpgradeable, ICharacter {
    ...
    function performFight(
        ...
        if ((((targetTrait + 1) % 4) == charTrait)) {
            playerRoll = uint24(playerRoll.add(uint24(ABDKMath64x64.divu(1, 20).mulu(playerRoll))));
        }
        ...
        if ((((charTrait + 1) % 4) == targetTrait)) {
            monsterRoll = uint24(monsterRoll.add(uint24(ABDKMath64x64.divu(1, 20).mulu(monsterRoll))));
        }
...
contract Weapons is Initializable, ERC721Upgradeable, AccessControlUpgradeable {
    ...
    function getBonusPowerForFight(
        ...
        if (wepType == 0 && charClass == 2)
            totalPower = uint24(basePower.sub(uint24(ABDKMath64x64.divu(2, 10).mulu(basePower))));
        if (wepType == 0 && charClass == 3)
            totalPower = uint24(basePower.sub(uint24(ABDKMath64x64.divu(1, 10).mulu(basePower))));
        ...
        if (charTrait == wepTrait) totalPower.add((uint24(ABDKMath64x64.divu(1, 10).mulu(basePower))));
