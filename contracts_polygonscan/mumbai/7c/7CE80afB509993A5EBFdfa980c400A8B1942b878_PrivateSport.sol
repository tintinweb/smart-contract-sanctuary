...
contract PrivateSport is Ownable {
    ...
    modifier canParticipate(
        ...
        require(_vote < 3 && _vote > 0, "PrivateSport: Invalid Vote");
    ...
    function createPool(uint256 _eventId) external checkValidEvent(_eventId) {
        ...
        pools[totalPools].id = totalPools;
        pools[totalPools].eventId = _eventId;
        pools[totalPools].aRate = 0;
        pools[totalPools].bRate = 0;
        pools[totalPools].balance = 0;
        pools[totalPools].aBalance = 0;
        pools[totalPools].bBalance = 0;
        pools[totalPools].result = Result.NONE;
        pools[totalPools].isEnded = false;
        pools[totalPools].owner = msg.sender;
    ...
    function claimAward(uint256 _poolId) external checkValidPool(_poolId) {
        ...
        winner.award = (winner.award * (100 - serviceFee - creatorFee)) / 100;
        require(
