...
contract JAXBlast1 is Album {
    ...
    */
    ...
    Album(994, "Tawanda CrypoJazz Festival",// solhint-disable-line func-visibility
    ...
    /**
        ...
        @dev Series 1 :
                ...
                Distribution duration : 7 days (Jul 09 00:00:01 to Jul 15 59:59:59 GMT)
            Series 2 :
                Epochs : 1017-1017
                Total JAX distributed : 25,000
                Distribution duration : 7 days (Jul 16 00:00:01 to Jul 22 59:59:59 GMT)
            Series 3 :
                Epochs : 1018-1038
                Total JAX distributed : 15,000
                Distribution duration : 7 days (Jul 23 00:00:01 to Jul 29 59:59:59 GMT)
            Series 4 :
                Epochs : 1039-1059
                Total JAX distributed : 10,000
                Distribution duration : 7 days (Jul 30 00:00:01 to Aug 05 59:59:59 GMT)
    ...
    */
    function rewardRate(uint256 epoch) public pure override returns (uint256) {
        ...
        if (epoch >= 994) {
            ...
            } else if (epoch >= 1017 && epoch <= 1017) {
                seriesRewards = 25000;// 25,000
            } else if (epoch >= 1018 && epoch <= 1038) {
                seriesRewards = 15000;// 15,000
            } else if (epoch >= 1039 && epoch <= 1059) {
                seriesRewards = 10000;// 10,000
            }
            if (seriesRewards > 0) {
                seriesRewards = seriesRewards.mul(1e18).div(7 days);
            }
        }
    ...
    /**
        ...
        @dev Series 1 :
                Epochs : 647-730 58962-59065
                Total JAX distributed : 50,000
                Distribution duration : 28 days (Jul 09 00:00:01 to Aug 05 59:59:59 GMT)
        @param epoch : 8-hour window number
        @return JAX Tokens distributed per second during the given epoch
    */
    function rewardRate2(uint256 epoch) public pure override returns (uint256) {
        require(epoch >= 994, "epoch cannot be below 994");
        if (epoch > 1059) {
            return 0;
        } else {
            uint256 seriesRewards = 50000;// 50,000
            return seriesRewards.mul(1e18).div(28 days);
        }
    }
