...
contract PartnerTreasure is BaseTreasure {
    constructor(address _myteContractAddress)
        BaseTreasure(_myteContractAddress)
    {
        nominalAmount = 3e8 ether; // ~300,000,000
        // 100% is locked in the beginning
        lockedAmount = nominalAmount;

        // 50% can be unlocked after every yearly
        periodicUnlockAmount = (nominalAmount * 50) / 100;
    }

    function unlock() external override onlyRole(ADMIN_ROLE) {
        require(unlockTimes < 2, "PEPPER::ALL_MYTE_IS_UNLOCKED");
        require(
    ...
    function _isUnlockTimeValid(uint256 _times) internal view returns (bool) {
        // check timestamp
        return
            _getBlockTimestamp() >= initialTimestamp + _times * 365 * 24 * 3600;
    }
}
