...
contract PublicTRSelector is PublicSelector {
    constructor(
        address _vrfCoordinator,
        address _link,
        bytes32 _keyHash,
...
contract GenericContestSelector is Ownable, VRFConsumerBase {
    ...
    /// @param winnersCount Number of potential winners to select
    /// @return Contest ID
    function createNewContest(
        ...
        // Check and call 'selectContestWinners' for contest
        if(stepsBeforeContestSelection[contest.id].length() == 4 && winnersCount > 0)
            selectContestWinners(contest.id, winnersCount);
     ...
     * Requests randomness
     */
    function getRandomNumber(uint256 contestId) public virtual contestExists(contestId) returns(bytes32) {
        require(LINK.balanceOf(address(this)) >= fee, "Not enough LINK - provide LINK to the contract");
        return requestRandomness(keyHash, fee);
    }
    ...
    function selectContestWinners(uint256 contestId, uint256 count) public onlyOwner contestExists(contestId) {
        ...
        if (contest.previousWinnerSeed == 0) {
            contest.previousWinnerSeed = randomResult;
        }
...
import "../GenericContestSelector.sol";

contract GenericContestSelectorTest is GenericContestSelector {
    using EnumerableSet for EnumerableSet.UintSet;

    constructor(uint256 random) GenericContestSelector(address(0), address(0), keccak256("random"), 1) {
        randomResult = random;
    }

    function getRandomNumber(uint256 contestId) public override contestExists(contestId) returns(bytes32) {
