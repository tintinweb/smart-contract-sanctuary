...
contract DolomiteAmmRouterProxy is ReentrancyGuard {
    ...
    event MarginPositionClose(
        ...
        address inputToken,
        address outputToken,
        Events.BalanceUpdate inputAmountUpdate, // the amount of held amount being sold to repay debt
        Events.BalanceUpdate outputAmountUpdate, // the amount of borrow amount being repaid
    ...
    function _logEvents(
        ...
        if (cache.params.isPositiveMarginDeposit && cache.params.accountNumber > 0) {
            Types.Par memory newOutputPar = cache.soloMargin.getAccountPar(accounts[0], cache.marketPath[cache.marketPath.length - 1]);
            ...
            newPar: newInputPar
                ...
                Events.BalanceUpdate({
            deltaWei: Types.Wei(true, cache.amountsWei[cache.amountsWei.length - 1]),
            newPar: cache.soloMargin.getAccountPar(accounts[0], cache.marketPath[cache.marketPath.length - 1])
            }),
                Events.BalanceUpdate({
