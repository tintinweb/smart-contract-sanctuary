...
contract HuanCasino is LoserGameRankData {
  ...
  uint256 public minimumBet = 1000 ether;

  uint256 public maximumBet = 10000 ether; 
  ...
  uint256 public unlucky_deal = 8000;
 ...
 fallback() external payable {}
  ...
  function bet(uint256 numberSelected, uint betValue) public payable {
    ...
    require(betValue >= minimumBet);

    pendingWithdrawals[msg.sender] -= betValue;
    ...
    if (numberOfBets >= maximumBetsNr) {
      ...
      emit Bet(msg.sender, betValue, scoreMap[msg.sender]);
    }
  ...
  function distributePrizes(uint256 numberWin) private {
    ...
    //收取平台费用，每次1%
    uint256 allDeal = totalBet * platform_deal / DEAL_BASE;
    IERC20 token = IERC20(lowbTokenAddress);
    require(token.transfer(owner, allDeal), "platform deal Lowb transfer failed");
    uint256 tempTotal = totalBet - allDeal;
    ...
    emit Test(salt, numberWin, tempTotal, allDeal);
    ...
    if (tempTotal > 0) {
      require(token.transfer(owner, allDeal), "tempTotal Lowb transfer failed");
    }
    tempTotal = 0;
