...
def updateInterest():
    ...
    if current_atoken_balance == 0:
        return
    reward_per_seconds: uint256 = (current_atoken_balance - self.lastAtokenBalance)
    log Bugger(current_atoken_balance - self.lastAtokenBalance, 'balance difference')
    log Bugger((current_atoken_balance - self.lastAtokenBalance) * 10**12, 'balance difference scaled')
    self.accumulatedRewardPerShare = self.accumulatedRewardPerShare + ((reward_per_seconds * 10**12)/self.totalContractBalance)  
    log Bugger(self.accumulatedRewardPerShare, 'rewardPerShare')
    log Bugger(reward_per_seconds, 'rewardPerSeconds')
...
def fxsell(withdraw: uint256) -> bool:
    ...
    interest_on_amount: uint256 = (self.accumulatedRewardPerShare * withdraw) / 10**12
