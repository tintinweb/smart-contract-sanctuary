...
import "./StakeCurrency.sol";

// TODO(bvv): Add claim function that allows you to claim the currency while still having the NFT staked
contract StakeHolder is Initializable, IERC721ReceiverUpgradeable, PausableUpgradeable, OwnableUpgradeable, UUPSUpgradeable {
    struct Stake {
        uint256 tokenId;
        uint80 value;
        address owner;
    ...
    // tokens earn 10000 CURRENCY per day
    uint256 public constant DAILY_CURRENCY_RATE = 10000 ether;
    // tokens must have 2 days worth of CURRENCY to unstake
    uint256 public constant MINIMUM_TO_EXIT = 2 days;

    /// @custom:oz-upgrades-unsafe-allow constructor
    constructor() initializer {}

    function initialize(StakeableNFT _token, StakeCurrency _currency) initializer public {
        __Pausable_init();
        __Ownable_init();
    ...
    function addManyToHolder(address account, uint16[] calldata tokenIds) external {
    ...
    function _calculateCurrencyOwed(uint256 tokenId) internal view returns (uint256 owed) {
        Stake memory stake = staked[tokenId];

        if(stake.owner == _msgSender()) {
            // Currently no logic for MAXIMUM
            // Currently no logic for stopping the production
            owed = (block.timestamp - stake.value) * DAILY_CURRENCY_RATE / 1 days;
        } else {
            owed = 0;
        }
    }
    ...
    function _claimSingleFromHolder(uint256 tokenId) internal returns (uint256 owed) {
        ...
        require(!(block.timestamp - stake.value < MINIMUM_TO_EXIT), "NOT ENOUGH CURRENCY TO RETURN TO EARTH");

        owed = _calculateCurrencyOwed(tokenId);
    ...
    function balance(uint256[] calldata tokenIds) external view returns(uint256) {
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            // Currently no logic for MAXIMUM
            // Currently no logic for stopping the production
            owed += _calculateCurrencyOwed(tokenIds[i]);
        }
        return owed;
    }
...
// TODO: Rename to StakeableNFT
contract StakeableNFT is Initializable, ERC721Upgradeable, ERC721EnumerableUpgradeable, PausableUpgradeable, OwnableUpgradeable, UUPSUpgradeable {
    using CountersUpgradeable for CountersUpgradeable.Counter;
    using SafeMath for uint256;

    CountersUpgradeable.Counter private _tokenIdCounter;
    StakeHolder public stakeHolder;
    StakeCurrency public currency;
    ...
    function initialize(StakeCurrency _currency) initializer public {
        __ERC721_init("SpaceMonkey", "SMK");
        __Pausable_init();
        __Ownable_init();
        __UUPSUpgradeable_init();
    ...
    function _baseURI() internal pure override returns (string memory) {
        return "http://space-monkey.com/token/";
    }
    ...
    function setStakeHolder(StakeHolder _stakeHolder) public onlyOwner {
    ...
    function mintNft(address _to, uint256 _count) public payable {
        uint256 total = _totalSupply();
        require(total <= maxItems, "Max limit");
        require(total + _count <= maxItems, "Max limit");
        require(total + _count <= maxSale, "Max sale limit");
        require(total <= maxMint, "Max mint");
        require(total + _count <= maxMint, "Max mint");
    ...
    function transferFrom(
        ...
        // Hardcode the StakeHolder's approval so that users don't have to waste gas approving
        if (_msgSender() != address(stakeHolder))
...
contract StakeCurrency is Initializable, ERC20Upgradeable, ERC20BurnableUpgradeable, PausableUpgradeable, OwnableUpgradeable, UUPSUpgradeable {
