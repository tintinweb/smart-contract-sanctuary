...
import { ERC721Mintable, IERC721Receiver, Address } from "./libs/ERC721.sol";
...
contract TradeTrustERC721 is TitleEscrowCloner, ERC721Mintable, IERC721Receiver {
  ...
  function destroyToken(uint256 tokenId) external onlyMinter {
    ...
    _safeTransformFromAsRegistry(ownerOf(tokenId), _burnAddress, tokenId);
  }
  ...
  function restoreTitle(
    ...
    _safeTransformFromAsRegistry(address(this), newTitleEscrow, tokenId);
  ...
  function _beforeTokenTransfer(
    ...
    } else if (to == _burnAddress || to == address(0)) {
      require(ownerOf(tokenId) == address(this), "TokenRegistry: Token has not been surrendered for burning");
    } else {
      require(to.isContract(), "TokenRegistry: Recipient address is not a title escrow");
      try IERC165(to).supportsInterface(type(ITitleEscrow).interfaceId) returns (bool retval) {
        require(retval, "TokenRegistry: Unsupported Title Escrow");
  ...
  function _safeTransformFromAsRegistry(
    address from,
    address to,
    uint256 tokenId
  ) internal {
    _safeTransformFromAsRegistry(from, to, tokenId, "");
  }
...
import { ERC721, ERC165 } from "./libs/ERC721.sol";
...
contract TitleEscrowCloneable is Context, Initializable, ITitleEscrow, HasHolderInitializable, HasNamedBeneficiaryInitializable, ERC165  {
  ...
  address internal _approvedOwner;
  ...
  /**
   * @notice Approves a non Title Escrow as new owner
   * @dev TradeTrust uses the Title Escrows to maintain joint ownerships.
   * However, implementations that wants to allow exiting to EOAs can extend from this function and utilise
   * the `_transferTo` function for transfers.
   */
  function _approveNewOwner(
    address newOwner
  ) internal virtual {
  ...
  function _transferTo(address newOwner) internal virtual {
  ...
  function endorse(
    ...
    address toHolder
  ) public isHoldingToken onlyHolder allowTransferTitleEscrow(toBeneficiary, toHolder) {
    address fromBeneficiary = beneficiary;
    address fromHolder = holder;

    emit Endorsement(_tokenId, toBeneficiary, toHolder, fromBeneficiary, fromHolder);
  ...
  function nominate(
    address toBeneficiary,
    address toHolder
  ) public onlyBeneficiary isHoldingToken {
    require(toBeneficiary != address(0), "TitleEscrow: Transferring to 0x0 is not allowed");
    require(toHolder != address(0), "TitleEscrow: Transferring to 0x0 is not allowed");

    emit Nomination(_tokenId, toBeneficiary, toHolder);

    approvedBeneficiary = toBeneficiary;
    approvedHolder = toHolder;
  }
...
import  { ERC721, IERC165, IERC721Receiver } from "../libs/ERC721.sol";
...
import { IERC721Receiver } from "../libs/ERC721.sol";
...
contract ERC721Mintable is ERC721, MinterRole {
  ...
  function supportsInterface(bytes4 interfaceId) public view virtual override(ERC721, MinterRole) returns (bool) {
    return
      interfaceId == type(IERC721).interfaceId ||
      interfaceId == type(IMinterRole).interfaceId ||
...
import "../interfaces/ITitleEscrow.sol";
import "../libs/Initializable.sol";
import { Context } from "../libs/ERC721.sol";
