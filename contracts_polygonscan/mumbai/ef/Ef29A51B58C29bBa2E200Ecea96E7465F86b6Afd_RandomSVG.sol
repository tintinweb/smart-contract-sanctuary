...
library SVGShapes {
    ...
    function generateSVG(uint256 _randomNumber, uint256 _maxNumberOfShapes, uint256 _size) internal pure returns (string memory finalSVG) {
        uint256 numberOfShapes = (_randomNumber % _maxNumberOfShapes) +1;
    ...
    function generateShape(uint256 _randomNumber, uint256 _size) internal pure returns(string memory pathSvg) {
        //need to work out a shape
        pathSvg = "";
        if (_randomNumber % 2 == 0) {
            //draw a circle
            uint256[] memory circleVars = expand(_randomNumber, 6);
            pathSvg = string(abi.encodePacked(pathSvg, "<circle cx='", string(Strings.toString(circleVars[0] % _size)), "' cy='", string(Strings.toString(circleVars[1] % _size)), "' r='", string(Strings.toString(circleVars[2] % (_size / 2))), "' stroke='#blue' stroke-width='", string(Strings.toString(circleVars[4] % (_size / 5))), "' fill='#red' />"));
            } else {
            //draw a square
            uint256[] memory rectVars = expand((_randomNumber + 1), 7);
            pathSvg = string(abi.encodePacked(pathSvg, "<rect x='", string(Strings.toString(rectVars[0] % _size)), "' y='", string(Strings.toString(rectVars[1] % _size)), "' width='", string(Strings.toString(rectVars[2] % (_size / 2))),"' height='", string(Strings.toString(rectVars[3] % (_size / 2))), "' stroke='#green' stroke-width='", string(Strings.toString(rectVars[5] % (_size / 5))), "' fill='#yellow' />"));
        } 
...
contract RandomSVG is ChainlinkClient, ERC721URIStorage, VRFConsumerBase {
    ...
    event CreatedRandomSVG(uint256 indexed tokenId, string tokenURI);
     ...
     ERC721 ("RandomSVG", "rsNFT") {
        ...
        //may need to remove this for own implementation
    ...
    function requestRandomWord() public {
        tokenIdToRandomWord[tokenCounter] = "pineapple";
    }
    */
    // ------- use for testnet/live deployment
    ...
    function finishMint (uint256 _tokenId) public {
        ...
        string memory svg = SVGShapes.generateSVG(randomNumber, maxNumberOfShapes, size);
