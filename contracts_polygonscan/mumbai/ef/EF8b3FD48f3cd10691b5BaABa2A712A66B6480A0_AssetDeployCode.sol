...
contract Asset is InitialAsset, IAsset {
    ...
    function __Asset_init(
        ...
        zVault = oracleAndZVault[1];

        AssetLib.checkIfTokensHavePair(_tokensInAsset, factory);
        ...
        returns (uint256)
    {
        ...
        (uint256[] memory buyAmounts, uint256[] memory oldDistribution) = AssetLib.buyTokensMint(
            totalWeth,
            _tokensInAsset,
            [weth_, _factory],
            tokensDistribution,
        ...
        // get mint amount
        uint256 mintAmount = AssetLib.getMintAmount(
            _tokensInAsset,
            buyAmounts,
            oldDistribution,
            totalSupply(),
    ...
    function redeem(uint256 amount, address currencyToPay) public override returns (uint256) {
        ...
        address weth_ = _weth;
        AssetLib.checkCurrency(currencyToPay, weth_, isTokenWhitelisted);
        ...
        (uint256 feeTotal, uint256[] memory inputAmounts, uint256 outputAmountTotal) = AssetLib
        .redeemAndTransfer(
            [amount, _totalSupply],
            [sender, currencyToPay, weth_, factory],
            totalTokenAmount,
            _tokensInAsset,
            feePercentages
        );
        ...
        if (feeTotal > 0 && currencyToPay != address(0) && currencyToPay != weth_) {
            feeAmountToZ += AssetLib.safeSwap([currencyToPay, weth_], feeTotal, factory, 0);
        } else if (feeTotal > 0) {
    ...
    function makeIme() external nonReentrant onlyManagerOrAdmin {
...

        {
            ...
            uint256[][3] memory tokensIncomeAmounts = AssetLib.initTokenInfoFromWhitelist(
                _tokenWhitelist,
                tokenEntersIme
            );
        ...
        nonReentrant
    {
        ...
        (uint256[][3] memory _tokensOldInfo, uint256 oldWeight) = AssetLib.initTokenToSellInfo(
            _tokensOld,
            oracle,
            totalTokenAmount
        );
    ...
    function _autoTransferFee(bool isForce) private {
        ...
        if (_feeAmountToZ > 0 && (isForce || _feeAmountToZ >= feeLimitForAuto)) {
        ...
        if (
            ...
            (isForce || _feeAmountInAssetToken >= feeLimitForAutoInAssetToken)
        ) {
            isRedeemingFee = true;
            //require(balanceOf(address(this)) >= _feeAmountInAssetToken,  "Internal error 11");
            uint256 feeAmount = redeem(_feeAmountInAssetToken, address(0));
            totalAmountFee += feeAmount;
            isRedeemingFee = false;
            feeAmountInAssetToken = 0;

            emit AssetsFromFeesConvertedToWeth(_feeAmountInAssetToken, feeAmount);
        }
    ...
    */
    function _rebase(
         ...
         */
        (uint256[][5] memory tokenToBuyInfo, uint256[] memory tokensPrices) = AssetLib
        .initTokenToBuyInfo(tokensToBuy, totalWeightNow, tokensDistribution, oracle);
        ...
        address weth_ = _weth;
        address _factory = factory;
        uint256 availableWeth = AssetLib.sellTokensInAssetNow(
            tokensInAssetNow,
            tokensInAssetNowInfo,
            weth_,
            _factory,
        ...
        uint256[] memory outputAmounts = AssetLib.buyTokensInAssetRebase(
            tokensToBuy,
            tokenToBuyInfo,
...
contract InitialAsset is ERC20Upgradeable, ReentrancyGuard {
    ...
    event UserClaimedIme(address indexed user, uint256 userWeight, uint256 tokenAmount);
    event ImeProceeded(uint256 totalWeight, uint256 totalAssetMintFromIme);
    event EnterIme(address indexed user, address indexed token, uint256 amount);
    ...
    function __InitialAsset_init(
        ...
        require(_imeStartTimestamp >= block.timestamp && _imeStartTimestamp < _imeEndTimestamp, "Time error");
        imeStartTimestamp = _imeStartTimestamp;
        imeEndTimestamp = _imeEndTimestamp;

        AssetLib.checkAndWriteWhitelist(_tokenWhitelist, factory, _whitelistIndexes, isTokenWhitelisted);
    ...
    function _enterIme(
        ...
        emit EnterIme(user, token, amount);
    }
    ...
    function _calculateUserWeight(address user) private view returns (uint256) {
        ...
        return AssetLib.calculateUserWeight(user, _tokensInIme, _tokenPricesInIme, userEnters);
    }
