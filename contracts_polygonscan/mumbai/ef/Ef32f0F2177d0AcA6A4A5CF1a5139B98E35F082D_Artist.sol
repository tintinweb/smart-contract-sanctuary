...
import "./Artist/ERC721Base.sol";

contract Artist is ERC721Base, ReentrancyGuard {
  // ============ Constants ============
  ...
  mapping(uint256 => uint256) private _songs;
  //index mapping of song id to current supply size (copies)
  mapping(uint256 => uint256) private _copies;
  //mapping of song id to fixed uri
  mapping(uint256 => string) private _uri;
  //mapping of song id to royalty percent
  mapping(uint256 => uint16) private _royaltyPercent;
  //mapping of song id to royalty recipient
  ...
  {
    uint256 songId = songOf(_tokenId);
    //if no royalty percent or no royalty recipient
    if (_royaltyPercent[songId] == 0 
      || _royaltyRecipient[songId] == address(0)
    ) {
    ...
    return (
      payable(_royaltyRecipient[songId]), 
      (_salePrice * _royaltyPercent[songId]) / MAX_PERCENT
    );
  }

  /**
   * @dev Returns the royalty percent for a `songId`
   */
  function royaltyPercent(uint256 songId) 
    external 
    view 
    virtual 
    returns(uint16) 
  {
    return _royaltyPercent[songId];
  }

  /**
   * @dev Returns a royalty recipient for a `songId`
   */
  function royaltyRecipient(uint256 songId) 
    external 
    view 
    virtual 
    returns(address) 
  {
    return _royaltyRecipient[songId];
  }

  /**
   * @dev Returns the current supply size of `songId`
   */
  function songCopies(uint256 songId) 
    public view virtual returns(uint256) 
  {
    return _copies[songId];
  }

  /**
   * @dev Returns the song given `tokenId`
   */
  function songOf(uint256 tokenId) 
    public view virtual returns(uint256) 
  {
  ...
  /**
   * @dev Returns the URI of a song 
   */
  function songURI(uint256 songId) 
  ...
  /**
   * @dev ERC165 - Add support for ERC2981 Royalty Standard
   */
    ...
    public view virtual override returns(string memory) 
  {
    ...
    require(_exists(tokenId), "Token does not exist");
    //get song id
    uint256 songId = songOf(tokenId);
    //make sure there is a song
    require(songId > 0, "Token is not a song");
  ...
  /**
   * @dev Allows anyone to get a token that was approved by anyone with 
   * a `MINTER_ROLE`
   */
  function authorize(
    uint256 songId, 
    ...
    require(hasRole(MINTER_ROLE, ECDSA.recover(
      ECDSA.toEthSignedMessageHash(
        keccak256(abi.encodePacked(songId, tokenId, recipient))
      ),
      proof
    )), "Invalid proof");
    //mint first and wait for errors
    _safeMint(recipient, tokenId);
    //then make a copy of the song
    _makeCopy(tokenId, songId);
  }

  /**
   * @dev Allows admin to mint a token for someone
   */
  function mint(uint256 songId, uint256 tokenId, address recipient) 
  ...
  /**
   * @dev Sets a fixed `uri` for a `songId`
   */
  function setSongURI(uint256 songId, string memory uri) 
  ...
  /**
   * @dev Deploys a new payment splitter contract, then sets the royalty
   */
  function createRoyalty(
    ...
    require(percent <= MAX_PERCENT, "Percent exceeds 10000");
    require(
      recipients.length == shares.length, 
      "Recipients and shares mismatch"
    );

    //create a payment splitter
    PaymentSplitter recipient = new PaymentSplitter(recipients, shares);
    //now add the royalty
    addRoyalty(songId, address(recipient), percent);
  }

  /**
   * @dev Sets a `percent` royalty for a `songId` to `recipient`
   */
  function addRoyalty(
    uint256 songId, 
    address recipient, 
    ...
    _royaltyPercent[songId] = percent;
    _royaltyRecipient[songId] = recipient;
  }

  // ============ Internal Methods ============

  /**
   * @dev Makes a copy of a song. Maps `tokenId` to `songId`
   */
  function _makeCopy(uint256 tokenId, uint256 songId) internal virtual {
    require(_exists(tokenId), "Token does not exist");
    require(_songs[tokenId] == 0, "Token is already a song");
    //add token to song
    _songs[tokenId] = songId;
    //add to the supply
    _copies[songId] += 1;
  }
}
