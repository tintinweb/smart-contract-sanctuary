...
contract Tslo is ERC20,VRFConsumerBase {
    ...
    address internal _blblToken = 0xEBc055Ea2BCb075C7ba1905Fbe85a051CB13370E;

    mapping(address => uint256) public _credits; // List of user credits.
    mapping(bytes32 => address) public requestIdToAddress;


    constructor() ERC20("Tslo", "TSLO")
    VRFConsumerBase(
    ...
    function getRandomNumber() public returns (bytes32 requestId) {
        require(LINK.balanceOf(address(this)) >= _linkFee, "Not enough LINK - fill contract with faucet");
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        emit RandomReceived(requestId,randomness);
        address requestAddress = requestIdToAddress[requestId];


        uint256 randomResult = (randomness % 5) + 1;
        if(randomResult<=3)
            _credits[requestAddress] -= 1;
        else if(randomResult==4)
            _credits[requestAddress] += 2;
        else if(randomResult==5)
            _credits[requestAddress] += 3;
    }
    ...
    event RandomReceived( bytes32 requestId, uint256 number );
    event ValueReceived(address user, uint amount);
    event FallbackReceived(address user, uint amount, bytes data);
    event TokenReceived(address _from, uint256 _value);
}