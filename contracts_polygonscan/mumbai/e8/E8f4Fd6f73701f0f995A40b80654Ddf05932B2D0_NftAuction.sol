...
contract NftAuction is OwnableUpgradeable, UUPSUpgradeable, PausableUpgradeable {
    ...
    */
    ...
    function initialize(uint256 fee, address feeRecipient) initializer public {
        __Ownable_init();
    ...
    */
    function bid(string memory id, uint256 bidValue) external payable whenNotPaused {
        ...
        if (auction.erc20Address != address(0)) {
            require(IERC20Upgradeable(auction.erc20Address).allowance(msg.sender, address(this)) >= bidValue, "Insufficient approval for ERC20 token for the auction bid. Aborting.");
        }
    ...
    */
    function settleAuction(string memory id, uint itemId, address marketplaceAddress, address creator, uint royaltiPercentage) external virtual payable {
        ...
        Auction memory auction = _auctions[id];
        require(auction.seller == msg.sender || msg.sender == owner(), "Auction can't be settled from other than seller or owner. Aborting.");
    ...
    */
    function cancelAuction(string memory id) external virtual payable {
        ...
        require(auction.seller == msg.sender || msg.sender == owner(), "Auction can't be cancelled from other than seller or owner. Aborting.");
        bool isErc721 = auction.isErc721;
        address nftAddress = auction.nftAddress;
        uint256 amount = auction.amount;
        uint256 tokenId = auction.tokenId;
