...
// Spintoss is the dealer of random number games at Spintoss.xyz: Coin Flip, Dice Roll, Spin Down, Roulette.
/*
v2 has the same logic as v1, with the following updates:
- Transaction Mining: Contract is linked to TransactionMiner contract to award TOSSY tokens to gamblers who lost bets.
...
- Reward Computation: Calculates txn mining reward based on rewardPct, TOSSY/MATIC price, bet amount, and probability of loss.
...
*/
contract Spintoss is VRFConsumerBase, Ownable, ReentrancyGuard {
    ...
    // Variables for computing price of TOSSY in MATIC. Used for computing TOSSY reward for transaction mining.
    uint constant SAFETY_FACTOR = 1e7; // Prevent truncation of price in integer datatype by multiplying with SAFETY_FACTOR.
    // Tossy / Matic ( Polygon Mainnet )
    // address constant TOSSY_TOKEN = 0x08B71e73fF566616bef1E0BE33614e38B79549EA;
    // address constant MATIC_TOKEN = 0x5B67676a984807a212b1c59eBFc9B3568a474F0a;
    // address public tossyMaticLP =  0xACA7c4e1c5cFFCa6e2bdD96a0B5Aa3f0C0568e75;
    
    // Tossy / Matic ( Polygon Testnet )
    address constant TOSSY_TOKEN = 0x08B71e73fF566616bef1E0BE33614e38B79549EA;
    address constant MATIC_TOKEN = 0x9c3C9283D3e44854697Cd22D3Faa240Cfb032889;
    address public tossyMaticLP =  0xACA7c4e1c5cFFCa6e2bdD96a0B5Aa3f0C0568e75;

    uint public tossyMaticRatio; // Price of TOSSY/MATIC
    uint public tossyMaticBlockTimestampLast;
    ...
    // Fallback & receive payable function used to top up the bank.
    ...
    // Set tossyMaticLP if we change our main trading LP for price reference.
    function setTossyMaticLP(address _tossyMaticLP) external onlyOwner {
        tossyMaticLP = _tossyMaticLP;
    }
    ...
    // Get TOSSY/MATIC price with safeguards against oracle attacks.
    function updateTossyMaticRatio() internal {
        // Run function if 30 minutes elapsed.
        if (block.timestamp - tossyMaticBlockTimestampLast >= 1800) {

            // Get ratio of TOSSY to MATIC multiplied by SAFETY_FACTOR to avoid losing precision.
            uint newTossyMaticRatio = IERC20(TOSSY_TOKEN).balanceOf(tossyMaticLP) * SAFETY_FACTOR / IERC20(MATIC_TOKEN).balanceOf(tossyMaticLP);

            if (tossyMaticRatio == 0) {
                // Update tossyMaticRatio if first time updating.
                tossyMaticRatio = newTossyMaticRatio;
            } else if (newTossyMaticRatio * SAFETY_FACTOR / tossyMaticRatio > SAFETY_FACTOR * 110 / 100) {
                // Cap any increase to +10% per update.
                tossyMaticRatio = tossyMaticRatio * 110 / 100;
            } else if (newTossyMaticRatio * SAFETY_FACTOR / tossyMaticRatio < SAFETY_FACTOR * 80 / 100) {
                // Cap any decrease to -10% per update.
                tossyMaticRatio = tossyMaticRatio * 90 / 100;
            } else {
                tossyMaticRatio = newTossyMaticRatio;
            }
            tossyMaticBlockTimestampLast = block.timestamp;
        }
    }
    ...
    function placeBet(uint betMask, uint modulo, address referrer) external payable nonReentrant {
        ...
        // Update TOSSY/MATIC price if data is outdated.
        updateTossyMaticRatio();
    ...
    function getTossyReward(uint amount, uint modulo, uint rollUnder) private view returns (uint) {
        // TOSSY reward equals token price multiplied by house edge fees, divided by win probability, multiplied by rewardPct.
        uint tossyReward = tossyMaticRatio * amount * (houseEdgeBP + getEffectiveWealthTaxBP(amount)) / 10000 * modulo / (modulo - rollUnder) * rewardPct / 100 / SAFETY_FACTOR;
