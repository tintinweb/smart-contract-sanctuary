...
//   event SetValue(address indexed from, uint256 main, uint256 _author, uint256 _platform);
  ...
  event awardAuctionNFT(address indexed from, address nft_a, uint256 tokenId, string seller, uint256 startPrice, uint256 endTime);
  ...
  function MintAndAuctionNFT(address to, string memory tokenURI,uint256 quantity, bool flag, address _contract, uint256 _minPrice, string memory ownerId, uint256 _endTime, uint256 royality) public {
      uint256 _tokenId;
      for(uint256 i = 0; i<quantity; i++)
      {
      ...
      if(msg.sender == nonCryptoNFTVault){
        ...
        emit MintWithTokenURINonCrypto(msg.sender, ownerId, tokenURI, quantity, flag);
              _nonCryptoOwners[tokenId] = ownerId;
          emit awardAuctionNFT(msg.sender, address(this), tokenId, ownerId, _minPrice, _endTime);
        }
      }
      // tokenMapping[tokenURI].inserted = true;
      // tokenMapping[tokenURI].commercialUse = flag;
