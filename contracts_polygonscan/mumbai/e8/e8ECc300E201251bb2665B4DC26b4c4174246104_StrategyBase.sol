...
abstract contract BaseStrategy is Ownable {
    ...
    // Dex
    address public constant currentRouter = 0x9e54d8dF3EF5b35F93b87Ad444aF998f96995856; //Quickswap router
...
abstract contract StrategyFarmBase is BaseStrategyMasterChef {
    ...
    address public constant gamma = 0x907EEaE40B8b605B7f9146B21a75046975fA4747;
    ...
    // Uniswap swap paths
    address[] public gamma_usdc_path;
    ...
    {
        gamma_usdc_path = new address[](2);
        gamma_usdc_path[0] = gamma;
        gamma_usdc_path[1] = usdc;
    }
    ...
    function harvest() public override {
        ...
        // Collects GAMMA tokens
        _getReward();

        // Swap 1/2 GAMMA for USDC
        uint256 _gammaBalance = IERC20(gamma).balanceOf(address(this));
        if (_gammaBalance > harvestCutoff){
            if (_gammaBalance > 0) {
                _swapUniswapWithPath(gamma_usdc_path, _gammaBalance.div(2));
            }

            // Add liquidity for GAMMA/USDC
            uint256 _usdc = IERC20(usdc).balanceOf(address(this));
            uint256 _gamma = IERC20(gamma).balanceOf(address(this));
            if (_usdc > 0 && _gamma > 0) {
                IERC20(usdc).safeApprove(currentRouter, 0);
                IERC20(usdc).safeApprove(currentRouter, _usdc);
                IERC20(gamma).safeApprove(currentRouter, 0);
                IERC20(gamma).safeApprove(currentRouter, _gamma);
                ...
                IERC20(gamma).safeTransfer(
                    strategist,
                    IERC20(gamma).balanceOf(address(this))
                );
            }
...
contract StrategyBase is StrategyFarmBase {
    // Token addresses
    address public GAMMA_MASTER_CHEF = 0x5221DC5efAD1D1271B46E06964C776a227d29456;
    address public GAMMA_USDC_LP = 0xDD6f8F0307Dd4ec14F02e6A2e18e28FB6EA3839f;
