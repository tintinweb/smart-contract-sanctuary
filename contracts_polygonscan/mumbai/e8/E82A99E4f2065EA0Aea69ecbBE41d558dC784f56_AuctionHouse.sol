...
contract AuctionHouse is IAuctionHouse, ReentrancyGuard, Ownable {
    ...
    mapping(uint256 => IBidDb.Bid[]) public _bids;

    uint256[] internal _auctionIds;

    IBidDb public _bidDb;

    uint256 public _beginAuctionId;
     ...
     * Constructor
     */
    constructor(address _zora, address _weth, uint256 beginAuctionId) public {
        require(
        ...
        _beginAuctionId = beginAuctionId;
        for (uint256 i = 0; i < _beginAuctionId; i++) {
    ...
    function migrateOldData(address otherBidDb) external onlyOwner {
        _bidDb.migrateOldData(otherBidDb, _beginAuctionId);
    }
...
interface IBidDb {
    ...
    function getAuctionByNftId(address token, uint256 tokenId) external view returns (
        IAuctionHouse.Auction memory, 
        AuctionStatus memory, 
        Bid[] memory
    );
    ...
    function migrateOldData(address otherBidDb, uint256 maxAuctionId) external;
}
...
contract BidDb is IBidDb, Ownable {
    ...
    mapping(uint256 => IAuctionHouse.Auction) public _auctions;

    // auction id => AuctionStatus
    mapping(uint256 => AuctionStatus) public _auctionStatus;

    // auction id => Bid[]
    mapping(uint256 => Bid[]) internal _bids;

    // token => tokenId => array of auction id 
    ...
    function getAuctionByNftId(address token, uint256 tokenId) external view override returns (
        IAuctionHouse.Auction memory, 
        AuctionStatus memory, 
        Bid[] memory) 
    {
            ...
            onlyOwner 
    {
        ...
        _auctionIds[auction.tokenContract][auction.tokenId].push(auctionId);
    }
    ...
    function migrateOldData(address otherBidDb, uint256 maxAuctionId) external override onlyOwner {
        for (uint256 i = 0; i < maxAuctionId; i++) {
            delete _auctions[i];
            delete _auctionStatus[i];
            delete _bids[i];

            IAuctionHouse.Auction memory oldAuction = _getOldAuction(otherBidDb, i);
            ...
            IBidDb.Bid[] memory oldBids = _getOldBids(otherBidDb, i);
            for (uint256 j = 0; j < oldBids.length; j++) {
                _bids[i].push(oldBids[j]);
            }
            
            IBidDb.Status memory oldStatus = _getOldStatus(otherBidDb, i);
            ...
            // assume current BidDb has no data at this time
            _auctionIds[oldAuction.tokenContract][oldAuction.tokenId].push(i);
        }
    }

    function _getOldAuction(address otherBidDb, uint256 auctionId) internal view returns (IAuctionHouse.Auction memory) {
        (bool success, bytes memory returnData) = otherBidDb.staticcall(abi.encodeWithSignature("auctions(uint256)", auctionId));
    ...
    function _getOldStatus(address otherBidDb, uint256 auctionId) internal view returns (IBidDb.Status memory) {
        (bool success, bytes memory returnData) = otherBidDb.staticcall(abi.encodeWithSignature("getBidStatus(uint256)", auctionId));
    ...
    function _getOldBids(address otherBidDb, uint256 auctionId) internal view returns (IBidDb.Bid[] memory) {
        (bool success, bytes memory returnData) = otherBidDb.staticcall(abi.encodeWithSignature("getBidHistory(uint256)", auctionId));
