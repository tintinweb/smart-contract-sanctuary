...
contract Memes is ERC1155, Ownable, Royalties {
    ...
    function mint(
        ...
        creatorList[_id] = creator;
        percentRoyaltyList[_id][creator] = percentRoyalty;
    ...
    function updatePercentRoyalty(
        uint256 _id,
        address _creator,
        uint256 newPercentRoyalty
    ) public {
        percentRoyaltyList[_id][_creator] = newPercentRoyalty;
    }
    ...
    function sendRoyalty(
        ...
        address payable _creator = creatorList[id];
        //get percent Royalty of creator
        uint256 percentRoyalty = percentRoyaltyList[id][_creator];
        //calculate royalty of creator
        uint256 royaltyCreator = percentRoyalty * (totalAmount / (100));
        uint256 amountSeller = totalAmount - (royaltyCreator);
        (bool successTramsferToCreator, ) = _creator.call{
            value: royaltyCreator
        }("");
        (bool successTramsferSeller, ) = ownerAddress.call{value: amountSeller}(
            ""
        );
        require(
            successTramsferToCreator,
            "sendRoyalty function: unable transfer erther to Creator, recipient may have reverted"
        );
        require(
            successTramsferSeller,
            "sendRoyalty function: unable transfer erther to Seller, recipient may have reverted"
        );
        this.royaltiesRecieved(_creator, _buyer, royaltyCreator);
    }
    ...
    {
        return percentRoyaltyList[_id][_creator];
    }
}
...
contract ERC1155 is Context, ERC165, IERC1155, IERC1155MetadataURI, TokenURI {
        ...
        override
    {
        ...
        console.log("before _balances[id][from] ",_balances[id][from]);
        _balances[id][from] = _balances[id][from].sub(amount, "ERC1155: insufficient balance for transfer");
        _balances[id][to] = _balances[id][to].add(amount);
        console.log("_balances[id][from] ",_balances[id][from]);
