...
contract Presale is ReentrancyGuard, Context, Ownable {
    ...
    function buyTokens(address beneficiary) public nonReentrant icoActive payable {
        ...
        // Calc reward in tokens
        // uint256 value = msg.value.div(1e18);
        // uint256 reward = _rewardTokenCount.div(1e18);
        // uint256 rewardInMatic = value.mul(reward);
        // uint256 rewardValue = rewardInMatic.mul(1e18);

        uint256 value = msg.value / 1e18;
        uint256 reward = _rewardTokenCount / 1e18;
        uint256 rewardInMatic = value * reward;
        uint256 rewardValue = rewardInMatic * 1e18;
        availableTokensICO = availableTokensICO - rewardValue;
        
        // Add person to distrubuted map and tokens bought
        distributed[beneficiary].wallet = beneficiary;
        distributed[beneficiary].amount += rewardValue;
