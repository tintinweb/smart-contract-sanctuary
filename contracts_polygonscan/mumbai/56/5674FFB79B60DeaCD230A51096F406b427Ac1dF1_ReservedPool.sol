...
contract ReservedPool is Ownable, ReentrancyGuard {
  ...
  function releaseDataGen() external firstRelease nonReentrant {
    ...
    if(srStart > block.timestamp){
      // uint256 epochs = block.timestamp.sub(frStart).div(15 * 24 * 3600);
      uint256 epochs = block.timestamp.sub(frStart).div(300);
      if (epochs >= 12) epochs = 11;
      ...
      dataGen.transfer(companyWallet, releaseAmount);
    }
    else {
      uint256 balance = dataGen.balanceOf(address(this));
      ...
      uint256 epochs = block.timestamp.sub(srStart).div(12 * 3600);
