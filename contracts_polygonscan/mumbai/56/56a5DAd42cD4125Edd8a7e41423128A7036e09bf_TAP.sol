...
contract TAP is ERC20, AccessControl {
  ...
  ) ERC20("Thoughts and Prayers", "TAP") {
  ...
  function whitelisted(address _wallet) public view returns (bool) {
  ...
  function transfer(address to, uint256 amount) public override returns (bool) {
    ...
    if (!whitelist[to] && !whitelist[msg.sender] && !waiveFees) {
      tokensForVault = calculatePortion(amount, vaultFee, vaultFeeDecimals);
      remainder = amount - tokensForVault;

      // calculate the number of tokens the Foundation should take
      tokensForFoundation = calculatePortion(
        amount,
        foundationFee,
    ...
    super.transfer(vaultAddress, tokensForVault);
    super.transfer(foundationAddress, tokensForFoundation);
...
contract TAPw is ERC20, AccessControl, ERC20Permit, ERC20Votes {
  ...
  modifier onlyTAPorOwner() {
    require(msg.sender == tapAddress || hasRole(OWNER_ROLE, msg.sender), "Address is not TAP");
    _;
  }
  ...
  function transfer(address to, uint256 amount) public override returns (bool) {
    ...
    if (!whitelist[to] && !whitelist[msg.sender] && !waiveFees) {
      tokensForWishingWell = calculatePortion(
        amount,
        wishingWellFee,
        wishingWellFeeDecimals
      );
      remainder = amount - tokensForWishingWell;
    ...
    super.transfer(wishingWellAddress, tokensForWishingWell);
    super.transfer(foundationAddress, tokensForFoundation);
    super.transfer(to, remainder);
