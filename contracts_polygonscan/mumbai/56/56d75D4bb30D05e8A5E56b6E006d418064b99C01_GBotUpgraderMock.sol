...
uint256 constant internal ONES = uint(~0);
...
contract GBotUpgraderMock is ERC721Receiver, Ownable, Pausable, TokenTimelock {
...
event GBotUpgraded(address indexed owner, uint256 tokenId, uint256 cost, uint256 newMetadata);
    ...
    function upgradeGBot(
        ...
        uint256 lockTime = determineUpgradeTime(metadata);
    ...
    function determineUpgradeTime(uint256 metadata) public view returns (uint256) {
        uint256 rarity = getMetadataForProperty(metadata, UpgProps.RARITY_BITS);
        require(rarity >= UpgProps.GBOT_RARITY_COMMON && rarity <= UpgProps.GBOT_RARITY_ULTIMATE, "GBotMetadataGenerator: Mop generation failed, unexpected rarity provided");

        if (rarity == UpgProps.GBOT_RARITY_COMMON) {
          return UpgProps.COMMON_UPG_TIME;
        }
        if (rarity == UpgProps.GBOT_RARITY_RARE) {
          return UpgProps.RARE_UPG_TIME;          
        }
        if (rarity == UpgProps.GBOT_RARITY_EPIC) {
          return UpgProps.EPIC_UPG_TIME;
        }
        if (rarity == UpgProps.GBOT_RARITY_LEGENDARY) {
          return UpgProps.LEGENDARY_UPG_TIME;
        }
        if (rarity == UpgProps.GBOT_RARITY_MYTHICAL) {
          return UpgProps.MYTHICAL_UPG_TIME;
        }
        if (rarity == UpgProps.GBOT_RARITY_ULTIMATE) {
          return UpgProps.ULTIMATE_UPG_TIME;
        }
    }
    ...
    function getMetadataForProperty(uint256 metadataId, uint256 position) internal pure returns (uint256) {
        uint bits = 8;
        if (position == UpgProps.VISUALS_BITS) {
            bits = 16;
         }
        require(0 < bits && position < 256 && position + bits <= 256);
        return metadataId >> position & UpgProps.ONES >> 256 - bits;
    }

    function calculateCop(uint256 metadata) private pure returns (uint256) {
        UpgProps.COP memory copData;
        copData.strength= getMetadataForProperty(metadata, UpgProps.STRENGTH_BITS);
        copData.speed= getMetadataForProperty(metadata, UpgProps.SPEED_BITS);
        copData.battery= getMetadataForProperty(metadata,UpgProps. BATTERY_BITS);
        copData.HP= getMetadataForProperty(metadata, UpgProps.HP_BITS);
        copData.attack= getMetadataForProperty(metadata, UpgProps.ATTACK_BITS);
        copData.defense= getMetadataForProperty(metadata, UpgProps.DEFENSE_BITS);
        copData.critical= getMetadataForProperty(metadata, UpgProps.CRITICAL_BITS);
        copData.luck = getMetadataForProperty(metadata, UpgProps.LUCK_BITS);
        copData.special = getMetadataForProperty(metadata, UpgProps.SPECIAL_BITS);
