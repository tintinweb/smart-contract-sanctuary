    ...
    ContextMixin
{
    ...
    uint256 public cost = 40 ether;// 0.02 ether;
    uint256 public maxSupply = 7500;
    uint256 public maxMintAmount = 20;
    bool public paused = false;
    address owner;
    string name_ = "Neon Prism NFTs";
    string symbol_ = "Neon Prisma";
    string baseURI_ = "https://bimensionstorage.blob.core.windows.net/prisma/json/";

    constructor() public ERC721(name_, symbol_) {
        _setupContractId("ChildMintableERC721");
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _setupRole(DEPOSITOR_ROLE, _msgSender()); //childChainManager
...
// public
  function mint(address _to, uint256 _mintAmount) public payable {
    ...
    require(_mintAmount > 0, "Mint Amount must be greater 0");
    require(supply + _mintAmount <= maxSupply, "Mint amount greater than max supply");
  ...
  function contractURI() public view returns (string memory) {
    ...
    return
      bytes(currentBaseURI).length > 0
        ? string(
          abi.encodePacked(currentBaseURI, "collection", baseExtension) //  , baseExtension
        )
        : "";
  }
...
//   }
  ...
  //only owner
  function setCost(uint256 _newCost) public  { //public onlyOwner
  ...
  function setBaseURI(string memory _newBaseURI) public only(DEFAULT_ADMIN_ROLE) {
...
//   }
  ...
  function reserveNFT(uint256 _amount) public {        
        require(owner == _msgSender());
        uint supply = totalSupply();
        uint start_index = 1;
        for (start_index; start_index <= _amount; start_index++) {
            _safeMint(_msgSender(), supply + start_index);
        }
  }
}