...
abstract contract Currency { 
  ...
  function get_busd_toen(address _currencyAddress) external virtual  view returns(uint256 busd_token);
  function get_wbnb_rate(address _currencyAddress) external virtual  view returns(uint256 wbnb_rate);
  function get_wbnb_toen(address _currencyAddress) external virtual  view returns(uint256 wbnb_token);
}
...
contract CURRENCY is TransferOwnable {    
    ...
    function get_busd_token(address _currencyAddress) public view returns(uint256 busd_token){
        CURRENCYS memory c = currencysInfo[_currencyAddress]; 
        busd_token = c.busd_token; 
    }
    function get_wbnb_rate(address _currencyAddress) public view returns(uint256 wbnb_rate){
        CURRENCYS memory c = currencysInfo[_currencyAddress]; 
        wbnb_rate = c.wbnb_rate; 
    }
    function get_wbnb_token(address _currencyAddress) public view returns(uint256 wbnb_token){
        CURRENCYS memory c = currencysInfo[_currencyAddress]; 
        wbnb_token = c.wbnb_token; 
    }
