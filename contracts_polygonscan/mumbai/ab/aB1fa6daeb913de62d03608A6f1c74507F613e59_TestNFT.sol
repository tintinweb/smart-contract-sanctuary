...
contract TestNFT is ERC721Burnable, ERC721Pausable, RandomlyAssigned, WithSaleStart, WithContractMetaData, WithIPFSMetaData {
    using SafeMath for uint256;

    bool public allowListing = false; // during mint
    uint256 public constant PRICE = 0.075 ether; //3 * 10**16;
    uint256 public constant MAX_BY_MINT_PUBLIC = 5;
    uint256 public constant MAX_BY_MINT_PRIVATE = 5; // max one ticket per wallet in private sale
    ...
    function setAllowListing() external onlyOwner {
        allowListing = true; // cannot be deactivated afterwards
    }

    function setApprovalForAll(address operator, bool approved) public override {
        require(allowListing || tokenCount() == totalSupply(), "Minting active"); // tradeable after minting has finished
