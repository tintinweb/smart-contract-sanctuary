...
// File: contracts/Polyshield.sol


pragma solidity 0.7.6;


contract Polyshield is ERC20("Polyshield", "SH13LD"), Ownable {
...
// MasterChef is the master of Polyshield. He can make SH13LD and he is a fair guy.
//
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 accShieldPerShare;   // Accumulated SH13LD per share, times 1e12. See below.
        uint16 depositFeeBP;      // Deposit fee in basis points
    }

    // The SHIELD TOKEN!
    Polyshield public polyshield;
    
    // Addresses to link to price.
    IERC20 public usdc;
    address public usdcShieldLP = address(0x0); // set after listing.
    ...
    uint256 public shieldPerBlock = 1 ether;
    ...
    // The block number when SH13LD mining starts.
    ...
    constructor(
        ...
        maxEmissionRate = _shieldPerBlock;
    ...
    function pendingShield(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 shieldReward = multiplier.mul(shieldPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accShieldPerShare = accShieldPerShare.add(shieldReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accShieldPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (nftaddr!=address(0x0)){
            polyshield.mint(nftaddr, shieldReward.div(20));
            polyshield.mint(devaddr, shieldReward.div(20));
        }else{
            polyshield.mint(devaddr, shieldReward.div(10));
        }
        polyshield.mint(address(this), shieldReward);
        pool.accShieldPerShare = pool.accShieldPerShare.add(shieldReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for SH13LD allocation.
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accShieldPerShare).div(1e12).sub(user.rewardDebt);
            if (pending > 0) {
    ...
    function harvest(uint256 _pid) public nonReentrant {
        ...
        uint256 pending = 0;
        if (user.amount > 0) {
             pending = user.amount.mul(pool.accShieldPerShare).div(1e12).sub(user.rewardDebt);
            if (pending > 0) {
                safeShieldTransfer(msg.sender, pending);
            }
        }
    ...
    // Safe shield transfer function, just in case if rounding error causes pool to not have enough SH13LD.
    function safeShieldTransfer(address _to, uint256 _amount) internal {
        uint256 shieldBal = polyshield.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > shieldBal) {
            transferSuccess = polyshield.transfer(_to, shieldBal);
        } else {
            transferSuccess = polyshield.transfer(_to, _amount);
        }
        require(transferSuccess, "safeShieldTransfer: transfer failed");
    }
   ...
   function updateEmissionIfNeeded() public {
       ...
       if (usdcShieldLP==address(0x0)){
        ...
        if (block.number - lastBlockUpdate > emissionUpdateInterval) {
            ...
            uint shieldBalance = polyshield.balanceOf(usdcShieldLP);
            uint priceCents = bottomPrice * 100;
            if (shieldBalance > 0) {
            massUpdatePools();
                // usdc token decimals = 6, gen token decimals = 18
                priceCents = usdc.balanceOf(usdcShieldLP).mul(1e12).div(shieldBalance);
            }

            uint256 emissionRatePercent = getEmissionRatePercent(priceCents);
            shieldPerBlock = maxEmissionRate.div(100).mul(emissionRatePercent);
            massUpdatePools();
        }
    }

    function getEmissionRatePercent(uint256 shieldPriceCents) public view returns (uint256 epr) {
        
       if (shieldPriceCents>=topPrice*100){return (1);}
       if (shieldPriceCents<=bottomPrice*100){return (100);}

       uint256 shieldPricePercentOfTop = shieldPriceCents.mul(100).div(topPrice*100);
       int256 curveCoefficient = FixidityLib.reciprocal(int256(curveRate));
       uint256 f1 = uint256(100).sub(shieldPricePercentOfTop);
       int256 p1 = FixidityLib.fromFixed(FixidityLib.multiply(curveCoefficient,int256(f1)));
       int256 p2 = FixidityLib.fromFixed(FixidityLib.divide(100,int256(shieldPricePercentOfTop)));
    ...
    function updateEmissionRate(uint256 _shieldPerBlock) public onlyOwner {
        massUpdatePools();
        shieldPerBlock = _shieldPerBlock;
        emit UpdateEmissionRate(msg.sender, _shieldPerBlock);
    }
    ...
    function setNFTAddress(address _address) public onlyOwner {
        require (_address!=address(0) &&  nftaddr==address(0));
        nftaddr = _address;
    }
    ...
    // For checking prices to link emission rate
    function setUsdcShieldLPAddress(address _address) public onlyOwner {
        require (_address!=address(0));
        usdcShieldLP = _address;
    }
