...
contract FanFeeManager is FeeManager, Ownable {
    ...
    constructor(address _token, address _wMatic) FeeManager(_token) {
        require(_wMatic != address(0), "_wMatic address cannot be 0");
        wMatic = _wMatic;
        minTokens = 100 * 10**18;
    }
    ...
    function setUniswap(address _uniswapPair, address _uniswapRouter) external onlyOwner {
        ...
        IERC20(wMatic).approve(address(uniswapRouter), 0);
        IERC20(wMatic).approve(address(uniswapRouter), type(uint256).max);
    }
    ...
    function canSyncFee(address, address recipient) external view override returns (bool shouldSyncFee) {
        if (recipient == address(uniswapPair)) {
            shouldSyncFee = true; // when swap FAN > MATIC
        }
    }
    ...
    function _syncFee() internal override {
        ...
        if (totalAmount >= minTokens && totalFeeAlloc > 0) {
            ...
            if (lpAmount >= 2) {
                ...
                // swap half
                uint256 pre = IERC20(wMatic).balanceOf(address(this));
                _swapTokens(swapAmount);
                uint256 post = IERC20(wMatic).balanceOf(address(this));

                // add other half
                _addTokensToLiquidity(liquidityAmount, post - pre);
            }
        }
    }
    ...
    function _addTokensToLiquidity(uint256 tokenAmount, uint256 wMaticAmount) private {
        if (tokenAmount != 0 && wMaticAmount != 0) {
