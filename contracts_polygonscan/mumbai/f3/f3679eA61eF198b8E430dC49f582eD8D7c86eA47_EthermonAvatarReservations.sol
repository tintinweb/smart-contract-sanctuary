...
contract EthermonAvatarReservations is AccessControl {
    ...
    bool public whitelist1Enabled = false;
    ...
    mapping(address => bool) public hasPurchases;
    mapping(address => uint256) public purchases;
    mapping(address => uint256) public purchaserToIndex;
    mapping(uint256 => address) public indexToPurchaser;
    uint256 public purchasersCount = 0;
    ...
    uint256 minTokenBalanceRequired = 1000000000000000000000; //1000 EMON to mint 
    ...
    function mint(uint256 _num) public payable returns (bool) {
        ...
        // Check user need to hold EMON to mint
        uint256 emonBalance = emonToken.balanceOf(msg.sender);

        // require(
        //     !onlyMintingByTokenHoldersAllowed ||
        //         (onlyMintingByTokenHoldersAllowed &&
        //             (emonBalance >= minTokenBalanceRequired)
        //         ),
        //     "Insufficient EMON balance"
        // );
        ...
        require(
            ...
            || 
            (
                ( whitelist2Enabled && whitelist2[msg.sender] && (emonBalance >= minTokenBalanceRequired) )
            )
        ...
        require(
            ...
            (
                ...
                (
                    whitelist2Enabled
                    && ( whitelist2[msg.sender] && (emonBalance >= minTokenBalanceRequired) )
        ...
        if(!hasPurchases[msg.sender]){
            hasPurchases[msg.sender] = true;
            purchaserToIndex[msg.sender] = purchasersCount;
            indexToPurchaser[purchasersCount] = msg.sender;
            purchasersCount++;
        }

        uint256 previousPurchases = purchases[msg.sender];
        purchases[msg.sender] = previousPurchases + uint256(_num);
    ...
    function addToWhiteList2() external {
        require(!whitelist2[msg.sender], "Cannot add duplicate address to whitelist 2");

        // User need to hold EMON to add themselves to whiteList2
        uint256 emonBalance = emonToken.balanceOf(msg.sender);
        require( emonBalance >= minTokenBalanceRequired, "Insufficient EMON balance to add to whiteList 2" );
