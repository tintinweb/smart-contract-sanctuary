...
contract PancakeSwapLottery is ReentrancyGuard, IPancakeSwapLottery, Ownable {
    ...
    uint256 public openingTime = 2 minutes;
     ...
     * @dev Callable by users
     */
    function buyTickets(uint256 _lotteryId, uint32[] calldata _ticketNumbers) // todo
        external
        override
        notContract
        nonReentrant
    ...
    function closeLottery(uint256 _lotteryId, uint256 externalNumber) external override onlyOperator nonReentrant {
        ...
        finalNumber += uint8(_randomNumber);
        ...
        finalNumber += uint8(_randomNumber) * 10;
        // 3
         _structHash = keccak256(
            abi.encode(
                _blockhash,
        ...
        finalNumber += uint8(_randomNumber) * 100;
        ...
        _randomNumber = uint256(_structHash) * 1000;
        assembly {
            _randomNumber := add(mod(_randomNumber, _maxNumber), 1)
        }
        finalNumber += uint8(_randomNumber);
        ...
        finalNumber += uint8(_randomNumber) * 10000;
        ...
        finalNumber += uint8(_randomNumber) * 100000;
