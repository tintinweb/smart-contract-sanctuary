...
contract Operator {
    ...
    function mintSingle(address nftAddress, address creator, string memory tokenURI) public onlyMinters returns(bool) {
        IBaseNFT721(nftAddress).createCollectible(creator, tokenURI);
        return true;
    }

    function mintMultiple(address nftAddress, address creator, string memory tokenURI, uint256 supply) public onlyMinters returns(bool) {
    ...
    function safeTransferMultiple(address nftAddress, address from, uint256 tokenId, address[] memory receivers, uint256[] memory amounts) public onlyMinters returns(bool) {
        require(receivers.length == amounts.length, " Operator: ids and amounts length mismatch");
