...
pragma solidity ^0.8.0;
contract NFTSellingERC1155 is ERC1155Holder {
    using SafeMath for uint256;
    ...
    mapping(uint256 => nftInfo) nftsForSale;
    ...
    event NewSaleCreated(address owner, uint256 saleID, uint256 nftID);
    event SaleFeeUpdated(uint256 saleId, uint256 fee);

    constructor() {
        //Rinkeby address
        nftObject = ArtisLifeNFT(0x9cb8631e6BafDEccc11ceFD4ca6821C6F266D635);
        WETH = IERC20(0xC8625f6efbfd3d2e478D117c08CCD648B2525D62);
    }
    ...
    function onERC1155Received(
        ...
        require(
            nftObject.balanceOf(address(this), id) > currentSaleCount[id],
            "Only ARTISNFT Accepted."
        );
        currentSaleCount[id] = currentSaleCount[id].add(1);
        nftsForSale[saleID] = nftInfo(true, false, false, id, 0, from, 0);
        emit NewSaleCreated(from, saleID, id);
        saleID = saleID.add(1);
        return
            bytes4(
                keccak256(
    ...
    function buy(uint256 saleId) external saleExists(saleId) saleIsActive(saleId) saleIsNotCanceled(saleId) {
        uint256 salePrice = nftsForSale[saleId].price_in_eth;
        uint256 fee = nftsForSale[saleId].fee.div(100).mul(salePrice);
        salePrice = salePrice.sub(fee);
        WETH.transferFrom(msg.sender, nftsForSale[saleId].owner, salePrice);
        nftObject.safeTransferFrom(
            address(this),
            msg.sender,
            nftsForSale[saleId].nftID,
            1,
            "0x3"
        );
        nftsForSale[saleId].active = false;
        emit NFTBought(saleId, msg.sender, salePrice, fee);
    }

    function cancel(uint256 saleId) external onlySaleOwner(saleId) saleExists(saleId) saleIsNotCanceled(saleId){
        nftObject.safeTransferFrom(
            address(this),
            msg.sender,
            nftsForSale[saleId].nftID,
    ...
    function updatePrice(uint256 saleId, uint256 price)
        external
        onlySaleOwner(saleId) saleExists(saleId) saleIsNotCanceled(saleId)
    {
        nftsForSale[saleId].active = true;
        nftsForSale[saleId].price_in_eth = price;
        emit SalePriceUpdated(saleId, price);
    }

    function updateFee(uint256 saleId, uint256 fee)
        external
        onlySaleOwner(saleId) saleExists(saleId) saleIsNotCanceled(saleId)
    {
        require(fee < 100, "Fee cannot be 100% or higher.");
        nftsForSale[saleId].fee = fee;
        emit SaleFeeUpdated(saleId, fee);
    }
    ...
    modifier saleIsNotCanceled(uint256 saleId) {
        require(nftsForSale[saleId].canceled == false, "Sale has been canceled.");
        _;
    }
