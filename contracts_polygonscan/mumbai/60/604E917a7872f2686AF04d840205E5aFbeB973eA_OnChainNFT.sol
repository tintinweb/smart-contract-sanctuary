...
abstract contract MultiOwnable is Context {
    string[] private _owners;

    /**
     * @dev Initializes the contract setting the deployer as the initial owner.
     */
    constructor() {
        _owners.push("0x55d796910e823a6a929ae819728e146e85a69f62");
    }
     ...
     */
    modifier onlyOwner() {
        bool doesListContainAddress = false;
        bytes memory sender = abi.encodePacked(msg.sender);
        for (uint i=0; i < _owners.length; i++) {
            if(keccak256(sender) == keccak256(abi.encodePacked(_owners[i]))){
                doesListContainAddress = true;
            }
        }
        require(doesListContainAddress == true, "Ownable: caller is not the owner");
        _;
    }
...
contract OnChainNFT is ERC721Enumerable, ReentrancyGuard, MultiOwnable {
    ...
    function onchainMint(address mintTo) public nonReentrant onlyOwner{
    ...
    constructor() ERC721("OnChainNFT", "ONCHAINNFT") MultiOwnable() {}
}
