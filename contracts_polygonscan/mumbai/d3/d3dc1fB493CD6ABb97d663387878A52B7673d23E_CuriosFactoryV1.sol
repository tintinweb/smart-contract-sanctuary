...
contract CuriosFactoryV1 is AccessControl {
    ...
    function createERC721ContractInstance (
        ...
        string memory _baseURI) external onlyRole(CONTRACT_CREATOR_ROLE){
        ERC1967Proxy newProxy = new ERC1967Proxy(implementation, abi.encodeWithSignature("initialize(string,string,address,address,address[],address[],string)", name, symbol, commissionAccount, _defaultAdmin, _minterPauserAddresses, _adminAddresses, _baseURI));
    ...
    function createERC1155ContractInstance (
        ...
        address[] memory _adminAddresses,
        string memory _baseURI) external onlyRole(CONTRACT_CREATOR_ROLE) {
        ERC1967Proxy newProxy = new ERC1967Proxy(implementation, abi.encodeWithSignature("initialize(address,address[],address[],string)", _defaultAdmin, _minterPauserAddresses, _adminAddresses, _baseURI));
...
//Inspired by Known Origin's ERC721 implementation, which can be found at https://github.com/knownorigin/known-origin-contracts/blob/master/contracts/v2/KnownOriginDigitalAssetV2.sol
    ...
    _commissionAccount, address _defaultAdmin, address[] memory _minterPauserAddresses, address[] memory _adminAddresses, string memory __tokenBaseURI) public initializer  {
