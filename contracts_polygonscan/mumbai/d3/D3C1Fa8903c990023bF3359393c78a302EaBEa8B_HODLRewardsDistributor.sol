...
contract HODLRewardsDistributor is Ownable {
        ...
        address immutable public wavax;
         ...
         */
        function depositWavaxRewards(uint256 amount_) external {
            IWAVAX(wavax).transferFrom(msg.sender, address(this), amount_);
            IWAVAX(wavax).withdraw(IWAVAX(wavax).balanceOf(address(this)));
        }

        function setShare(address sharholderAddress_, uint256 amount_) onlyOwner onlyIncluded(sharholderAddress_) external {
        ...
        */
        function excludeFromRewards (address shareHolderToBeExcluded_ ) external onlyOwner {
        ...
        */
        function includeInRewards (address shareHolderToBeIncluded_) external onlyOwner {
         ...
         */
        function _updateUserShares(address sharholderAddress_, uint256 newAmount_ ) internal {
            ShareHolder storage user = shareHolders[sharholderAddress_];
        ...
        */
        function claimPending(address sharholderAddress_) public {
            ...
            IWAVAX(wavax).withdraw(pendingAmount);
            payable(sharholderAddress_).transfer(pendingAmount);
         ...
         */
        function pending(address sharholderAddress_) public view returns (uint256 pendingAmount) {
            ShareHolder storage user = shareHolders[sharholderAddress_];
            pendingAmount = user.shares.mul(accPerShare).div(1e36).sub(user.rewardDebt);
        }

        function totalPending () public view returns (uint256 ) {
            return accPerShare.mul(totalShares).div(1e36);
        }
