...
import './interfaces/IDexBondingCalculator.sol';
...
contract DexBondingCalculator is IDexBondingCalculator {
    ...
    function markdown(address _pair) external view override returns (uint256) {
        ...
        uint256 reserve;
        if (IUniswapV2Pair(_pair).token0() == DEX1) {
            reserve = reserve1;
        } else {
            reserve = reserve0;
        }
        return
            reserve.mul(2 * (10**IERC20(DEX1).decimals())).div(
                getTotalValue(_pair)
            );
    }
}
...
interface IDexBondingCalculator {
