...
contract Borgs is ERC721Enumerable, ERC721Holder, CommonObjects, NumberUtils{
    ...
    function convertBytesToBytes2(bytes memory inBytes) public pure returns (bytes2 outBytes8){
        ...
        assembly {
            outBytes8 := mload(add(inBytes, 2))
        }
    }
    
    function toUInt(bytes memory value) public pure returns(uint8){
        return uint8(value[0]);
    }
    
    function toUInt(bytes2 value) public pure returns(uint16){
        return uint16(value);
    }
    
    function toUInt2(bytes4 value) public pure returns(uint32){
        return uint32(value);
    }
