...
contract Erc721Bank is AccessControl, Pausable, IERC721Receiver {
    ...
    function stakeTokens(address[] calldata tokens, uint256[] calldata tokenIds) external whenNotPaused {
    ...
    function stakeToken(address token, uint256 tokenId) external whenNotPaused {
        _updateData(block.number);
    ...
    function unstakeTokens(address[] calldata tokens, uint256[] calldata tokenIds) external whenNotPaused {
        uint256 count = tokenIds.length;
        require(count == tokenIds.length, "tokens and tokenIds mismatch");
    ...
    function unstakeToken(address token, uint256 tokenId) external whenNotPaused {
        _updateData(block.number);
