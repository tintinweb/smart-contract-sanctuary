...
contract SantaFactoryV2 is Governance, VRFConsumerBase, IERC721Receiver {
    ...
    ERC20 _acceptedToken = ERC20(0x0);
    ERC20 _cifiTokenContract = ERC20(0x0);
    SantaV2 _santa = SantaV2(0x0);
    ...
    constructor(
        ...
        _acceptedToken = ERC20(acceptedToken);
        _cifiTokenContract = ERC20(cifi);
        _maxNftCount = maxNftCount;
        _santa = SantaV2(santa);
    ...
    function mint(uint256 face_value) public returns (uint256) {
        require(_minters[msg.sender], "can't mint");
        require(face_value > 0, "face_value should be grater than 0!");
        require(
            _acceptedToken.allowance(msg.sender, address(this)) >= face_value,
            "Allownce is less than face_value!"
        );

        require(
            _acceptedToken.balanceOf(msg.sender) >= face_value,
        ...
        _acceptedToken.transferFrom(msg.sender, address(this), face_value);
    ...
    function burn(uint256 tokenId) external returns (bool) {
        ...
        _acceptedToken.transfer(_santa.ownerOf(tokenId), santa_data.face_value);
        _santa.burn(tokenId);
        return true;
    }
