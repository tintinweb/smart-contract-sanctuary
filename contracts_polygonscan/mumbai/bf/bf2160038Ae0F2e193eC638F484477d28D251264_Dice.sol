...
contract Dice is Game {
    ...
    constructor(address bankAddress, address chainlinkCoordinatorAddress)
        payable
        Game(bankAddress, chainlinkCoordinatorAddress)
    {}
    ...
    {
        uint256[] memory userBetsIds = _bets[user];
...
abstract contract Game is Ownable, Pausable, Multicall, VRFConsumerBaseV2 {
    ...
    function _newBet(
        ...
        Bet memory newBet = Bet(false, payable(user), token, id, betAmount);
        _bets[user].push(id);
