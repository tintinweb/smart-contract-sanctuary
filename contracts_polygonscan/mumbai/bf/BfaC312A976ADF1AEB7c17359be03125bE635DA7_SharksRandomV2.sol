...
contract SharksRandomV2 is VRFConsumerBase, Ownable {
	...
	//shuffling process
	uint256[] public randomPool; //keeps track of the random numbers used in the drawing
	uint256 public nftMaxSupply = 7777; //prepare 400+ tx for 10k nfts....
	mapping(uint256 => uint256) public shuffledNFTsTracker;
	...
	//brute force it
	function partialShuffle(uint256 randomPoolIndex) external onlyOwner {
		require(shuffledNFTs.length < nftMaxSupply, "shuffling complete");
		for (uint256 i = 0; i < 400; i++) {
			uint256 winner = (uint256(keccak256(abi.encode(randomPool[randomPoolIndex], i))) %
				nftMaxSupply) + 1;
			if (shuffledNFTsTracker[winner] == 0) {
				shuffledNFTs.push(winner);
				shuffledNFTsTracker[winner] = 1;
			}
		}
	}

	//Given two integers a and b which are coprime, then (a x + b) modulo n will visit all integers from 0 to n - 1 exactly once.
	function primeShuffle(
		uint256 drawID,
		uint256 start,
		uint256 limit
	) external onlyOwner {
		require(shuffledNFTs.length < nftMaxSupply, "shuffling complete");
		uint256 a = 97; //   # Random prime
	...
	//helper function
	function shuffledNFTsLength() public view returns (uint256) {
		return shuffledNFTs.length;
	}
	...
	/**
	 * fill a pool of random numbers.
	 */
	function fillRandomPool(uint256 drawID) external onlyOwner {
		for (uint256 i = 0; i < 300; i++) {
			randomPool.push(uint256(keccak256(abi.encode(randomResults[drawID], i))));
		}
	}
