...
contract BalanceVault is Ownable, AccessControl, ReentrancyGuard{
    ...
    */
     ...
     */
    function withdrawNaka(uint256 _nakaAmount) external whenBalanceVaultNotPaused nonReentrant{
    ...
    function safeNakaTransfer(address _userAddress, uint256 _nakaAmount) internal {
        ...
        if (_nakaAmount > nakaBalance) {
            token.transfer(_userAddress, nakaBalance);
        } else {
            token.transfer(_userAddress, _nakaAmount);
        }
    }
