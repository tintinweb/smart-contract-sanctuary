...
contract KiBridgeV18N is KiBridgeV1Factory {
    ...
    returns (bool success)
  {
    ...
    uint256 refund = transaction.amount - _calAmount;
    if(liquidityInfo[transaction.sendToken].pendingLiquidityToChain[toChainId] >= refund) {
      liquidityInfo[transaction.sendToken].pendingLiquidityToChain[toChainId] -= refund;
    } else {
      liquidityInfo[transaction.sendToken].fulfilledLiquidityFromChain[toChainId] =
      liquidityInfo[transaction.sendToken].fulfilledLiquidityFromChain[toChainId] +
      refund - liquidityInfo[transaction.sendToken].pendingLiquidityToChain[toChainId];
    ...
    if(transaction.executeFunctionId == this.liquidityOutTransferToUser.selector){
      ...
      liquidityInfo[transaction.sendToken].toAllChainableBalance += refund;
      IKiBridgeV1Liquidity(getLiquidity[transaction.sendToken]).mint(_user, refund);
    } else {
      userInfo[_user].pendingTokenBalance[transaction.sendToken] -= transaction.amount;
      transaction.sendToken.safeTransfer(_user, refund);
    }
    transaction.amount = _calAmount;
    userInfo[_user].transactions[fromChainId][toChainId].push(transaction);
    if(_calAmount == 0) return false;
    return true;
  }
 function fulfillRemoveLiquidity(
    ...
    returns (bool success)
  {
    ...
    IKiBridgeV1Liquidity(getLiquidity[token]).mint(_user, refund);
...
function liquidityInRequestRemoveLiq(
  ...
  {
    return userInfo[_user].pendingToChainLiquidityRequest[_toChainId];
  }
