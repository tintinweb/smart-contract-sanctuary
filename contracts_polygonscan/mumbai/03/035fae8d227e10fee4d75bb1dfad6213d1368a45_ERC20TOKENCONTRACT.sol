...
contract ERC20TOKENCONTRACT is ERC20("PEE", "FUNNY") {
    
    bool public canBurnOnDeployment = true;
    
    constructor() {
        _mint(msg.sender, 100000000000 * 10 ** decimals()); //Owner
    }
    
    function burnOnDeployment() external onlyOwner {
        require(canBurnOnDeployment == true, "Already burned!");
        
        _burn(msg.sender, (balanceOf(address(msg.sender)) * 9 / 10));
        
        canBurnOnDeployment = false;
    }
    
    function airdropInitial(address[] calldata addressList) external onlyOwner {
        require(balanceOf(address(deployer)) > 0, "No tokens to airdrop!");
        
        uint256 deployerBalance = balanceOf(address(deployer));
        uint256 amount = ((deployerBalance/20)/(addressList.length))-(((deployerBalance/20)/(addressList.length))/200);
