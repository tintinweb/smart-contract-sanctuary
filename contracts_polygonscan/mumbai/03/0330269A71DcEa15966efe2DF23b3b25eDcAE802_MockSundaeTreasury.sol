...
contract MockSundaeTreasury is Ownable {
  using SafeMath for uint256;
  ...
  address public sSUN;
  uint256 public sSUNQueue; // Delays change to sSUN address
  ...
  constructor(
    ...
    // isLiquidityToken[ _SUNDAI ] = true;
    // liquidityTokens.push( _SUNDAI );

    blocksNeededForQueue = _blocksNeededForQueue;
  }

  /**
        @notice allow approved address to deposit an asset for SUN
        @param _amount uint
  ...
  function deposit(
    ...
    (_token, _amount);
    // mint SUN needed and store amount of rewards for distribution
    send_ = value.sub(_profit);
    IERC20Mintable(SUN).mint(msg.sender, send_);
  ...
  /**
        @notice allow approved address to burn SUN for reserves
        @param _amount uint
        @param _token address
     */
  function withdraw(uint256 _amount, address _token) external {
    ...
    uint256 value = valueOfToken(_token, _amount);
    ISUNERC20(SUN).burnFrom(msg.sender, value);
  ...
  function incurDebt(uint256 _amount, address _token) external {
    ...
    uint256 maximumDebt = IERC20(sSUN).balanceOf(msg.sender); // Can only borrow against sSUN held
  ...
  /**
        @notice allow approved address to repay borrowed reserves with SUN
        @param _amount uint
     */
  function repayDebtWithSUN(uint256 _amount) external {
    require(isDebtor[msg.sender], "Not approved");

    ISUNERC20(SUN).burnFrom(msg.sender, _amount);

    debtorBalance[msg.sender] = debtorBalance[msg.sender].sub(_amount);
    totalDebt = totalDebt.sub(_amount);

    emit RepayDebt(msg.sender, SUN, _amount, _amount);
  }
  ...
  function mintRewards(address _recipient, uint256 _amount) external {
    ...
    IERC20Mintable(SUN).mint(_recipient, _amount);
  ...
  function excessReserves() public view returns (uint256) {
    return totalReserves.sub(IERC20(SUN).totalSupply().sub(totalDebt));
  }
  ...
  /**
        @notice returns SUN valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
  ...
  {
    if (isReserveToken[_token]) {
      // convert amount to match SUN decimals
      value_ = _amount.mul(10**IERC20(SUN).decimals()).div(
    ...
    returns (bool)
  {
    ...
    } else if (_managing == MANAGING.SSUN) {
      // 9
      sSUNQueue = block.number.add(blocksNeededForQueue);
    } else return false;
