    ...
    UUPSUpgradeable
{
    ...
    event AvatarCreated(uint256 indexed tokenId, address indexed toAddress, address indexed byAddress);
     ...
     */
    function adminUpdatePublicMinting(bool enabled) external onlyRole(ADMIN_ROLE) {
     ...
     */
    function adminMintTo(address _toAddress, uint8 _count) external onlyRole(MINTER_ROLE) {
        require(
            _count <= _MAX_NUM_TOKENS_PER_MINT,
            "Too many number of tokens to mint in one transaction"
        );
        require(
            _reservedTokenCounter.current() + _count <= _RESERVED_NUM_TOKENS,
    ...
    function mint(uint256 _count) public payable {
        ...
        require(
            _tokenIdTracker.current() -
                ...
                _count <=
                _MAX_NUM_TOKENS.sub(_RESERVED_NUM_TOKENS),
    ...
    function getPrice(uint256 _count) public view returns (uint256) {
        return _TOKEN_PRICE.mul(_count);
    }
