...
contract GameTheory is ERC721, ERC721URIStorage, VRFConsumerBase, AttributesAndRarity{
...
//    Counters.Counter private _tokenIdCounter;
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomNumber) internal override {
        ...
        uint256[5] memory attributesOfDude;
        ...
        for (uint256 i=0; i < randomValues.length; i++){
            ...
            //TODO FETCH API DATA
            uint256[] memory avgPriceArray;

            categoryToRarityArray[i] = getRarityForCategory(avgPriceArray);
            attributesOfDude[i] = getAttributePerCategory(randomValues[i], categoryToRarityArray[i]);
        }
...
contract AttributesAndRarity{
    ...
    //TODO: REMOVE HARDED (3) NUMBER OF CATEGORIES
    function getAttributesArray(uint256[5] memory randomNumbers, uint256[][] memory rarityArrays) internal pure returns (uint256[] memory){
    ...
    function getAttributePerCategory(uint256 randomNumber, uint256[] memory rarityValues) internal pure returns (uint256){
        uint256 i = 0;
    ...
    function getRarityForCategory(uint256[] memory avgPriceArray) internal pure returns (uint256[] memory) {
