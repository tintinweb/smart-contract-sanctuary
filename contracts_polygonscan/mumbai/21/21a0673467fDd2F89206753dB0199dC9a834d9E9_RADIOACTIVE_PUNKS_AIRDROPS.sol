...
These punks survived to be made into community art!
...
@title  RADIOACTIVE_PUNKS_AIRDROPS

@dev    Extends ERC1155 Token Standard to allow pausing, burning,
        and royalties via the ERC2981 NFT Royalty Standard. The metadata and
        images are stored on Arweave, meaning all NFT data is on-chain. If any
        ...
        This contract is designed for the Polygon blockchain to keep gas fees
        low and inexpensive, therefore it has Meta-transactions with OpenSea
        enabled to allow for gasless listing, transfer, and sale.
        Stay sludgy!
*/
...
contract RADIOACTIVE_PUNKS_AIRDROPS is ERC1155PresetMinterPauser, ERC2981, HasSecondarySaleFees, Ownable, ContextMixin {
  // allows calls toString on uint256
  ...
  /*
   * @dev Airdrop to a set of addresses using a specific token ID + metadata
   *      URL pair this should both mint the tokens to the addr and mark
   *      the new metadataURL
   */
  function airdrop (
    address[] memory receivers, string memory metadataURL, uint256 tokenId, address payable artist
  ) external onlyOwner {
    // set the URL and artist address in both maps
    metadataURIs[tokenId] = metadataURL;
    artists[tokenId] = artist;

    // mint 1 token to the each receiver
  ...
  /*
   * @dev ERC2981 Royalties Standard for ERC1155
   */
  function royaltyInfo(
    uint256 _tokenId, uint256 _value, bytes calldata _data
  ...
  returns (
    ...
    return (artists[_tokenId], _value / 10, _data); // 10%
  }

  /*
   * @dev ERC2981 Royalties Standard for ERC1155, as well as standard ERC155.
   */
  function supportsInterface(
    bytes4 interfaceId
  ) public view override(ERC1155PresetMinterPauser, ERC2981, HasSecondarySaleFees) returns (bool) {
    return super.supportsInterface(interfaceId);
  }

  /*
   * Rarible/Foundation V1 Royalties Schema
   */
   ...
   *      Allows for gasless listing on OpenSea etc.
   */
  ...
  /**
   * @dev This is used instead of msg.sender as transactions won't be sent
   *      by the original token owner, but by OpenSea.
   *      Allows for gassless transfers and sales.
   */
