...
contract CrowdsaleIdo is ReentrancyGuard {
    ...
    uint256 leashLockPeriod;

    uint256 maxUserUsdLimit;
    ...
    // function buyTokenWithStableCoin(
    //     IERC20 _stableCoin,
    //     uint256 _stableCoinAmount
    // ) external {
    //     require(_getNow() >= crowdsaleStartTime, "Crowdsale isnt started yet");
    //     require(
    //         _stableCoin == usdt || _stableCoin == usdc || _stableCoin == dai,
    //         "Unsupported StableCoin"
    //     );
    //     if (crowdsaleEndTime != 0) {
    //         require(_getNow() < crowdsaleEndTime, "Crowdsale Ended");
    //     }
    //     uint256 maxUsdSpendAmount = stakingLaunchpad.usersUsdLimitData(
    //         msg.sender
    //     );
    //     uint256 amountUserCanInvest = maxUsdSpendAmount.sub(
    //         totalUsdAmountInvested[msg.sender]
    //     );
    //     if (_stableCoin == usdt || _stableCoin == usdc) {
    //         require(_stableCoinAmount <= amountUserCanInvest.div(1e12));
    //     } else {
    //         require(_stableCoinAmount <= amountUserCanInvest);
    //     }

    //     uint256 tokenPurchased = _stableCoin == dai
    //         ? _stableCoinAmount.mul(rate)
    //         : _stableCoinAmount.mul(rate).mul(1e12);
    ...
    //     }
    //     if (_stableCoin == dai) {
    //         totalUsdAmountInvested[msg.sender] = totalUsdAmountInvested[
    //             msg.sender
    //         ].add(_stableCoinAmount);
    //     } else {
    //         totalUsdAmountInvested[msg.sender] = totalUsdAmountInvested[
    //             msg.sender
    //         ].add(_stableCoinAmount.mul(1e12));
    //     }
    ...
    function buyFromWhiteListCrypto(
        ...
        require(
            lockLeash(
                ((tokenPurchased.mul(1e18)).div(rate).div(2).mul(1e6)).div(leashPrice)
            )
        );
    ...
    function lockLeash(uint256 _stakeAmount) internal returns (bool) {
        ...
        if (block.timestamp >= user.unlockTokenTime) {
