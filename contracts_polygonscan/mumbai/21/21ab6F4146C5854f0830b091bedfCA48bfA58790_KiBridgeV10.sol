...
contract KiBridgeV10 {
    ...
    returns (bool success)
  {
    ...
    = abi.decode(_data, (uint256, uint256, uint256, uint256, Transaction));
    require(fromChainId == block.chainid, "Wrong chainId");
    delete userInfo[_user].peningLiquidityRequest[_requestId];
    if(_calAmount == 0) {
      if(fulfilled >= transaction.amount) {
        liquidityInfo[transaction.sendToken].fulfilledLiquidityFromChain[toChainId] += transaction.amount;
      } else {
        liquidityInfo[transaction.sendToken].fulfilledLiquidityFromChain[toChainId] += fulfilled;
        liquidityInfo[transaction.sendToken].peningLiquidityToChain[toChainId] = 
        liquidityInfo[transaction.sendToken].peningLiquidityToChain[toChainId] -
        transaction.amount + fulfilled;
      }
