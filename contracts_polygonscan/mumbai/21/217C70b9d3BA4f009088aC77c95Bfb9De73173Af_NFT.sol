...
contract NFT is ERC721URIStorage, Ownable {
    ...
    function setMarketContract(address newMarketAddress) external onlyOwner {
        marketContract = newMarketAddress;
    }
    ...
    // mint a new token
    function createToken(string memory tokenURI) public onlyWhitelistedMinter returns (uint) {
        _tokenIds.increment();
        ...
        if (marketContract != address(0)) {
            approve(marketContract, newItemId);
        }
        return newItemId;
    }
    ...
    function tokenIdsOfOwner(address ownerAddress) external view returns (uint256[] memory) {
        ...
        if (tokenCount == 0) {
            ...
            uint256 totalAspo = _tokenIds.current();
            uint256 resultIndex = 0;

            // We count on the fact that all nfts have IDs starting at 1 and increasing
            // sequentially up to the totalAspo count.
            uint256 aspoId;

            for (aspoId = 1; aspoId <= totalAspo; aspoId++) {
                if (ownerOf(aspoId) == ownerAddress) {
                    result[resultIndex] = aspoId;
                    resultIndex++;
                    if (resultIndex == tokenCount) break;
                    // break to save gas if possible
                }
            }
    ...
    function tokenItemOfOwner(address ownerAddress) external view returns (TokenItem[] memory) {
        ...
        if (tokenCount == 0) {
            ...
            for (aspoId = 1; aspoId <= totalAspo; aspoId++) {
                if (ownerOf(aspoId) == ownerAddress) {
                    result[resultIndex].tokenId = aspoId;
                    result[resultIndex].tokenURI = tokenURI(aspoId);
                    result[resultIndex].locked = _lockTokens[aspoId];
                    resultIndex++;
                    if (resultIndex == tokenCount) break;
                    // break to save gas if possible
                }
    ...
    function lockToken(uint256 tokenId, bool locked) public {
        ...
        uint256 totalAspo = _tokenIds.current();
        require(1 <= tokenId && tokenId <= totalAspo, "ASPO NFTs: invalid tokenId");
