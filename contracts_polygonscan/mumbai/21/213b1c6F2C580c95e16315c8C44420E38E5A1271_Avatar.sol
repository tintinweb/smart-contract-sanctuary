    ...
    UUPSUpgradeable
{
     ...
     */
    function adminMintTo(address _toAddress, uint8 _count)
        external
     ...
     * @param _count number of tokens to be minted
     */
    function mint(uint8 _count) public payable mintingValidation(_count) {
        require(
    ...
    function getOwnersAllTokenURIs(address owner)
        public
        view
        returns (string[] memory)
    {
        uint256 balance = balanceOf(owner);
        string[] memory a = new string[](balance);
        for (uint256 i = 0; i < balance; i++) {
            uint256 tokenId = tokenOfOwnerByIndex(owner, i);
            a[i] = tokenURI(tokenId);
        }
        return a;
    }
