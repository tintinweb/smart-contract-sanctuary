...
contract GoldmaTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SGMA }
    ...
    address public sGMA;
    uint public sGMAQueue; // Delays change to sGMA address
    ...
    constructor (
        ...
        isReserveToken[ _MATIC ] = true;
        reserveTokens.push( _MATIC );

        blocksNeededForQueue = _blocksNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for GMA
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint GMA needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( GMA ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn GMA for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IGMAERC20( GMA ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sGMA ).balanceOf( msg.sender ); // Can only borrow against sGMA held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with GMA
        @param _amount uint
     */
    function repayDebtWithGMA( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IGMAERC20( GMA ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, GMA, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( GMA ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( GMA ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns GMA valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match GMA decimals
            value_ = _amount.mul( 10 ** IERC20( GMA ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SGMA ) { // 9
            sGMAQueue = block.number.add( blocksNeededForQueue );
        } else return false;
