...
// ENSURE THAT NO OTHER FILES ARE IMPORTED FROM THE OTHER PROJECT TO MAINTAIN SEPARATION
import "./../tokens/erc-721/ERC721Rarible.sol";
...
contract Auction is OwnableUpgradeable, IERC721ReceiverUpgradeable, SignatureChecker {
    ...
    */
    function createTokenAuction(
        ...
        ERC721Rarible(_nft).safeTransferFrom(owner, address(this), _tokenId);
    ...
    function testMinting(
        ...
        ERC721Rarible(_nft).safeTransferFrom(_transferFrom, _transferTo, _tokenId);
    }
    ...
    */
    ...
    function executeLazyAuction(LibLazyBidERC721.Bid memory _bid, LibLazyAuctionERC721.Auction memory _auction) external {
        ...
        (uint256 _tokenId, address _firstOwner, address _auctioneer) = ERC721Rarible(_bid.token).decodeLazyMintData(_bid.tokenData);
        ...
        //      to prevent anyone else starting the auction and 
        //      to allow asset data contract to have arbitrary auctioneer logic
        bytes32 _auctionHash = LibLazyAuctionERC721.hash(_auction, getEIP712DomainHash());
        bool _isAuctioneer = checkSignature(_auctionHash, _auction.signature, _auctioneer);
        require(_isAuctioneer, "Invalid Auctioneer");
        ...
        require(_bid.value >= _auction.min);
        ...
        ERC721Rarible(_bid.token).mintAndTransferEncodedData(_bid.tokenData);

        // Transfer to bid taker (Remember that taker can be different from bid maker)
        ERC721Rarible(_auction.token).safeTransferFrom(
                _firstOwner,
                _bid.taker,
                _tokenId
        );  
    ...
    */
    function executeSale(address _nft, uint256 _tokenId, address bidder, uint256 amount, bytes memory sig) external {
        ...
        ERC721Rarible(_nft).safeTransferFrom(
                address(this),
                bidder,
                _tokenId
            );
    ...
    function cancelAuction(address _nft, uint256 _tokenId, address[] memory _bidders) external {
        ...
        ERC721Rarible(_nft).safeTransferFrom(address(this), auction.seller, _tokenId);
    }
...
library LibLazyBidERC721 {
    ...
    function hash(Bid memory bid, bytes32 EIP712DomainHash) internal pure returns (bytes32) {
...
library LibLazyAuctionERC721 {
    ...
    function hash(Auction memory auction, bytes32 EIP712DomainHash) internal pure returns (bytes32) {
