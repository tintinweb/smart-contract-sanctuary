...
contract PTokenOption is IPTokenOption, ERC721 {
    ...
    mapping(uint256 => Position) _tokenIdPositions;
    ...
    function getPosition(address owner) public view override returns (Position memory) {
        return _tokenIdPositions[_ownerTokenId[owner]];
    }

    function updatePosition(address owner, Position memory position) public override _pool_ {
        int256 preVolume = _tokenIdPositions[_ownerTokenId[owner]].volume;
        ...
        _tokenIdPositions[_ownerTokenId[owner]] = position;
        emit UpdatePosition(
            owner,
            position.volume,
            position.cost,
    ...
    function burn(address owner) public override _pool_ {
        ...
        if (_tokenIdPositions[tokenId].volume != 0) {
            _numPositionHolders--;
        }
