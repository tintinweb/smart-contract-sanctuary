...
// TypeError: Member "length" not found or not visible after argument-dependent lookup in mapping(uint8 => struct Traits.Layer storage ref).
    ...
    function assembleTraits(uint8 species, uint8 gen, bool infected) external onlyController returns (Traits memory) {
        Traits memory newTraits = Traits(
            layerCount[infected ? 3 : species][0],
    ...
    function _compileAttributes(uint16 tokenId) public view returns (string memory) {
        ...
        string memory traits = string(abi.encodePacked(
            ...
            _sintleAttributeCompilation("Clothing", layers[sentry.species][0][sentryTraits.body].name), ",",
            _sintleAttributeCompilation("Suit",  layers[sentry.species][0][sentryTraits.body].name), ",",
            _sintleAttributeCompilation("Accessory",  layers[sentry.species][0][sentryTraits.body].name), ",",
            _sintleAttributeCompilation("Backpack",  layers[sentry.species][0][sentryTraits.body].name), ",",
            _sintleAttributeCompilation("Eyewear",  layers[sentry.species][0][sentryTraits.body].name), ",",
            _sintleAttributeCompilation("Weapon",  layers[sentry.species][0][sentryTraits.body].name), ","
        ));
        return string(abi.encodePacked(
        '[',
            traits,
    ...
    function buildMetadata(uint16 _tokenId) public view returns(string memory) {
        ...
        return string(abi.encodePacked(
                'data:application/json;base64,', _base64(bytes(abi.encodePacked(
                            ...
                            'Sentry #', _tokenId, 
                            '", "description":"', 
                            'Humanities last chance against the MATIC virus. Sentries are augmented soldiers designed to defend Polygonia by any means necessary.',
                            '", "image": "', 
