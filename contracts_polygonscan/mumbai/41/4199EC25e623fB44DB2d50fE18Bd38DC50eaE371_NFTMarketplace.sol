...
contract NFTMarketplace is IERC721Receiver, Ownable, ILayerZeroReceiver {
    ...
    uint startTime;
    uint endTime;
    uint public testTokenId;  // receiving token ID
    ...
    function buy(uint _tokenId, uint16 _chainId, bytes memory _destination) public payable {
        // Time constraints
        ...
        // Transfer ETH
        mktWallet.transfer(msg.value);

        // Mint NFT
        nft.safeMint(msg.sender, _tokenId);

        // notify secondary chain
        // test call with chain and contract - prod will have mapping
        notify(_tokenId, _chainId, _destination);
    ...
    function lzReceive(uint16 , bytes memory _srcCounterMockAddress, uint64 _nonce, bytes memory _payload) override external {
        ...
        // Receive token ID
        testTokenId = abi.decode(_payload, (uint));
    }

    /// @notice Notifies the other chain that token ID X is taken
    function notify(uint _tokenId, uint16 _chainId, bytes memory _destination) public payable {
        bytes memory tokenIdBytes = abi.encode(_tokenId);
