...
contract DPolyPay is PMGPermission {
    ...
    function banlanceOf(string memory merchant, string memory symbol)
        public
        view
        returns (
            uint256,
        ...
        )
    {
        return (
            ...
            _merchantRecord[merchant][symbol].withdraw
        );
    }
    ...
    function withDraw(
        ...
        if (
            ...
            // do nothing, money now to contract
            payable(to).transfer(_merchantRecord[merchant][symbol].total);
        } else {
