...
contract RacerData {
    ...
    constructor(address racerChildAddress) {
        RACER_CHILD = RacerChild(racerChildAddress);
    }

    function requestNewSeed() public {
        // set new seed and set seedStarts to ID that new seed will apply to
        // testing here...
        uint256 randomNumber = uint256(keccak256(abi.encode(block.timestamp)));
        fulfillRandomness(randomNumber);
    }

    function fulfillRandomness(uint256 randomNumber) public {
        randomSeeds.push(randomNumber);
