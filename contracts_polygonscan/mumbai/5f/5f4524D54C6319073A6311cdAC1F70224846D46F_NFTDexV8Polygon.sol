...
//   event SetValue(address indexed from, uint256 main, uint256 _author, uint256 _platform);
  ...
  function claimAuction(uint256 _tokenId, bool awardType, string memory ownerId, address from) public {
    ...
    //   _nonCryptoWallet[_nonCryptoOwners[_tokenId]] += (price / 1000) * mainPerecentage;
      ...
      revenueShare.transferAlia(price, tokenId, [mainPerecentage, platformPerecentage,authorPercentage], [aliaHolder, temp.seller, _tokenAuthors[tokenId],platform, nonCryptoNFTVault], _nonCryptoOwners[_tokenId]);
