...
contract YieldlyMarketplace is ReentrancyGuard, Ownable {
    ...
    function addCreatorMap(bool _isNew, uint[] memory _newtokenIds, address[] memory _creators, uint256[] memory _prices, address[] memory _owners, uint256[] memory _royalties, bool[] memory _listedMap) external onlyOwner {
    ...
    function buy(uint[] memory _ids) external payable {
        for (uint i = 0; i < _ids.length; i ++) {
            _validate(_ids[i]);
            // require(price[_ids[i]] == _prices[i], "Error, price is not match");
            address _previousOwner = ownerMap[_ids[i]];
            address _newOwner = msg.sender;

            // 2.5% commission cut
            uint256 _commissionValue = price[_ids[i]].mul(25).div(1000);
            uint256 _royaltyValue = price[_ids[i]].mul(royaltyMap[_ids[i]]).div(100);
            uint256 _sellerValue = price[_ids[i]].sub(_commissionValue + _royaltyValue);
            // _owner.transfer(_owner, _sellerValue);
            transferMATICOrWMATIC(payable(_previousOwner), _sellerValue);
            transferMATICOrWMATIC(payable(creatorMap[_ids[i]]), _royaltyValue);
            transferMATICOrWMATIC(payable(adminRecoveryAddress), _commissionValue);
            Yieldly(nftContract).transferFrom(address(this), _newOwner, _ids[i]);
    ...
    function _validate(uint _id) internal view {
        ...
        require(msg.sender != Yieldly(nftContract).ownerOf(_id), "Can not buy what you own");
    ...
    function updateListingStatus(uint _tokenId, bool shouldBeListed) public returns (bool) {
        require(msg.sender == Yieldly(nftContract).ownerOf(_tokenId), "Error, you are not the owner");
        ...
        onlyCreatorOrWinner(tokenId)
    {
        ...
        if(winner == address(0)) {
            ...
            if (Creator == creatorMap[tokenId]) {
                transferMATICOrWMATIC(payable(Creator), amount.sub(_commissionValue));
            } else {
                uint256 _royaltyValue = amount.mul(royaltyMap[tokenId]).div(100);
                transferMATICOrWMATIC(payable(creatorMap[tokenId]), _royaltyValue);
                transferMATICOrWMATIC(payable(Creator), amount.sub(_royaltyValue).sub(_commissionValue));
            }
    ...
    function mint(string memory _tokenURI, uint256 _price, bool _isListOnMarketplace, uint256 _royalty, uint256 startDate, uint256 auctionLength) public {
        ...
        emit Minted(msg.sender, _price, newTokenId, _tokenURI, _isListOnMarketplace);

        if (_isListOnMarketplace) createAuction(newTokenId, startDate, auctionLength, _price);
    }
...
contract Yieldly is ERC721URIStorage, Ownable {

    constructor() ERC721("Yieldly NFT", "YieldlyNFT") {}
