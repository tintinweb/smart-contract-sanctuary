...
contract swapContract is AccessControl, Pausable, ECDSAOffsetRecovery{
    ...
    mapping(uint128 => IUniswapV2Router02) public blockchainRouters;
    mapping(uint128 => address) public blockchainPools;
    mapping(uint128 => address) public blockchainFeeAddresses;
    ...
    /**
      ...
      * @param blockchain Number of blockchain
      * @param tokenIn Address of a token being sold
      * @param tokenInAmount Maximum amount of a token being sold
      * @param firstPath Path used for swapping tokenIn to *RBC
      * @param secondPath Path used for swapping *RBC to tokenOut
      * @param exactRBCtokenOut Exact amount of RBC to get after first swap
      * @param tokenOutMin Minimal amount of tokens to get after second swap
      * @param newAddress Address in the blockchain to which the user wants to transfer
      */
    function transferToOtherBlockchain(uint128 blockchain, IERC20 tokenIn,
    ...
    external payable whenNotPaused
    {
        ...
        require(
            msg.value >= blockchainCryptoFee[blockchain],
            "swapContract: Not enough crypto provided"
        );
        require(tokenIn.transferFrom(msg.sender, address(this), tokenInAmount),
                                                'swapContract: Transfer tokens from sender failed');
        require(tokenIn.approve(address(blockchainRouters[numOfThisBlockchain]), tokenInAmount), 'approve failed.');
        uint256[] memory amounts = blockchainRouters[numOfThisBlockchain].swapTokensForExactTokens(exactRBCtokenOut, tokenInAmount,
                                                                firstPath, blockchainPools[numOfThisBlockchain],
                                                                block.timestamp + deadline);
        tokenIn.transfer(_msgSender(), tokenInAmount - amounts[0]);
    ...
    {
        require(
            amountWithFee > feeAmountOfBlockchain[numOfThisBlockchain],
        ...
        _processTransaction(user, amountWithFee, originalTxHash, concatSignatures);

        uint256 amountWithoutFee = amountWithFee - feeAmountOfBlockchain[numOfThisBlockchain];
        require(IERC20(path[0]).transferFrom(blockchainPools[numOfThisBlockchain], address(this), amountWithoutFee),
                                                        "swapContract: transfer from pool failed");
        require(IERC20(path[0]).transferFrom(blockchainPools[numOfThisBlockchain],
                                                blockchainFeeAddresses[numOfThisBlockchain],
                                                feeAmountOfBlockchain[numOfThisBlockchain]),
                                                        "swapContract: fee transfer failed");
        require(IERC20(path[0]).approve(address(blockchainRouters[numOfThisBlockchain]), amountWithoutFee),
                                                        "swapContract: approve to Swap failed");
        blockchainRouters[numOfThisBlockchain].swapExactTokensForTokens(
            amountWithoutFee,
            amountOutMin,
            path,
            user,
    ...
    /**
      * @dev Changes/Set Router address to certain blockchain number
      * @param _blockchainNum number of blockchain to change the router
      * @param _router the new Router address
      */

    function setRouterToBlockchain(uint128 _blockchainNum, IUniswapV2Router02 _router) external onlyOwnerAndManager{
        blockchainRouters[_blockchainNum] = _router;
    }

    /**
      * @dev Changes/Set Pool address of blockchain
      * @param _blockchainNum number of blockchain to change the pool address
      * @param _poolAddress the new Pool address
      */

    function setPoolAddressOfBlockchain(uint128 _blockchainNum,address _poolAddress) external onlyOwnerAndManager{
        blockchainPools[_blockchainNum] = _poolAddress;
    }
    ...
    /** 
      * @dev Changes address which receives fees from transfers to certain blockchain
      * @param blockchainNum Number of blockchain to set fee address
      * @param newFeeAddress New address for fees
      */
    function setFeeAddressOfBlockchain(uint128 blockchainNum, address newFeeAddress) external onlyOwnerAndManager
    {
        blockchainFeeAddresses[blockchainNum] = newFeeAddress;
    }
