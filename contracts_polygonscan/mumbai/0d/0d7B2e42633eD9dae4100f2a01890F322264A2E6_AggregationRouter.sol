...
import "./AcuraswapRouter.sol";
import "./libraries/AcuraERC20.sol";
import "./interfaces/IAggregationExecutor.sol";
import "./interfaces/IAcura.sol";

contract AggregationRouter is Ownable, AcuraswapRouter {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    using AcuraERC20 for IERC20;
        ...
        returns (uint256 returnAmount, uint256 gasLeft, uint256 chiSpent)
    {
        ...
        (IAcura chi, uint256 amount) = caller.calculateGas(initialGas.sub(gasleft()), desc.flags, msg.data.length);
        if (amount > 0) {
        ...
        returns (uint256 returnAmount, uint256 gasLeft)
    {
        ...
        uint256 initialSrcBalance = (flags & _PARTIAL_FILL != 0) ? srcToken.acuraBalanceOf(msg.sender) : 0;
        uint256 initialDstBalance = dstToken.acuraBalanceOf(dstReceiver);
        ...
        returnAmount = dstToken.acuraBalanceOf(dstReceiver).sub(initialDstBalance);

        if (flags & _PARTIAL_FILL != 0) {
            spentAmount = initialSrcBalance.add(desc.amount).sub(srcToken.acuraBalanceOf(msg.sender));
    ...
    function rescueFunds(IERC20 token, uint256 amount) external onlyOwner {
        token.acuraTransfer(payable(msg.sender), amount);
    }
...
contract AcuraswapRouter is Permitable {
    ...
    function acuraswapWithPermit(
        ...
        return acuraswap(srcToken, amount, minReturn, pools);
    }
...
library AcuraERC20 {
    ...
    function acuraBalanceOf(IERC20 token, address account) internal view returns (uint256) {
        if (isETH(token)) {
    ...
    function acuraTransfer(IERC20 token, address payable to, uint256 amount) internal {
        if (amount > 0) {
            if (isETH(token)) {
                to.transfer(amount);
            } else {
    ...
    function acuraApprove(IERC20 token, address to, uint256 amount) internal {
