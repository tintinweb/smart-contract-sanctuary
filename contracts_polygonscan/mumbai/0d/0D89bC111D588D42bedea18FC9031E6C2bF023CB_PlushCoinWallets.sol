...
contract PlushCoinWallets is Ownable {
    ...
    constructor(address _plushAddress)
    {
    ...
    {
        require(_amount >= minimumBet, 'Not enough balance.');
        require(plush.balanceOf(msg.sender) >= _amount, 'Not enough balance.');
        plush.approve(address(this), _amount);
        plush.transferFrom(msg.sender, address(this), _amount);
        increaseWalletAmount(msg.sender, _amount);
    }

    function increaseWalletAmount(address wallet, uint256 amount) private
    {
        walletInfo[wallet].balance += amount;
    }

    function decreaseWalletAmount(address wallet, uint256 amount) private
    {
        walletInfo[wallet].balance -= amount;
    }
