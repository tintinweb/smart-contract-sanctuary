    ...
    ReentrancyGuard
{
    ...
    uint256 public startCalimDateInMinutes = 1 * oneMinute;
    uint256 public startCalimDateInDays = 365 * oneDay;
    uint256 public vestingDurationInMinutes = 12 * oneMinute;
    uint256 public vestingDurationInDays = 1825 * oneDay;

    uint256 public emmissionRatePerSecond = 1;
    uint256 public totalEmmissionRate = 720;
    ...
    modifier onlyIfVestingScheduleNotExists(
        ...
        require(vestingSchedules[vestingScheduleId].initialized == false);
        require(vestingSchedules[vestingScheduleId].revoked == true);
        require(vestingSchedules[vestingScheduleId].nftId != _tokenId);
        _;
    }
    ...
    function createVestingSchedule(address _beneficiary, uint256 _tokenId)
        public
        returns (
            // onlyIfVestingScheduleNotExists(
            //     this.computeCurrentVestingScheduleIdForHolder(_beneficiary),
            //     _tokenId
            // )
            bytes32
        )
    {
        ...
        uint256 startClaimDate = startCalimDateInMinutes;
        bool revocable = true;
        uint256 released = 0;
        ...
        onlyIfVestingScheduleNotRevoked(vestingScheduleId)
    {
        ...
        require(
            (vestingSchedule.startClaimDate + vestingSchedule.duration) <=
                getCurrentTime(),
        ...
        returns (uint256)
    {
        ...
        if (
            vestingSchedule.revoked == true ||
            currentTime < vestingSchedule.startClaimDate
        ) {
            return 0;
        }
        // else if (
        ...
        // }
            ...
            uint256 timeFromStartClaim = currentTime.sub(
                vestingSchedule.startClaimDate
            );
