...
// File: contracts/ThortlesNFT.sol
...
contract ThortlesNFT is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  uint256 public whiteListedCost = 0.5 ether;
  uint256 public maxSupply = 5000;
  uint256 public maxMintAmount = 5;
  uint256 public maxMintAmountForWhitelist = 3;
  ...
  function updateCost(bool isWhitelistedAdd) internal view returns (uint256 _cost) {
      if (isWhitelistedAdd) {
          return whiteListedCost;
      }
      if (!isWhitelistedAdd) {
          return cost;
      }
  ...
  /*
    ...
    @description - Mints _mintAmount of ThortlesNFTs for sender address.
    @param <uint256> _mintAmount - The number of ThortlesNFTs to mint.
  */
    function mint(uint256 _mintAmount) public payable {
    require(!paused, "The contract is paused");
    uint256 supply = totalSupply();
    require(_mintAmount > 0, "You need to mint at least 1 ThortleNFT");
    require(_mintAmount <= maxMintAmount, "Max mint amount per session exceeded");
    require(supply + _mintAmount <= maxSupply, "Max ThortleNFT limit exceeded");

    if (msg.sender != owner()) {
        if(onlyWhitelisted == true) {
            require(isWhitelisted(msg.sender), "User is not whitelisted");
            uint256 ownerMintedCount = addressMintedBalance[msg.sender];
            require(ownerMintedCount + _mintAmount <= maxMintAmountForWhitelist, "Max ThortlesNFT per address exceeded");
        }

        //make sure dynamic cost can be turned off/on
        if (dynamicCost) {
            require(msg.value >= updateCost(onlyWhitelisted) * _mintAmount, "Insufficient funds. Please add more AVAX.");   
        } else {
            require(msg.value >= cost * _mintAmount, "Insufficient funds. Please add more AVAX.");
        }
    }
  ...
  */
  ...
  /*
    ...
    @description - Gets all whitelisted addresses
  */
    function getAllWhiteListedAddresses(bool showAddresses) public view returns (address[] memory) {
        showAddresses == true;
        if (showAddresses) {
  ...
  /*
    @function removeAllWhiteListedAddresses()
    @description - Removes all addresses from whitelist
  */
    function removeAllWhiteListedAddresses() public onlyOwner () {
  ...
  */
  ...
  function setWhiteListCost(uint256 _newWLCost) public onlyOwner {
      whiteListedCost = _newWLCost;
  }
  /*
    @function setmaxMintAmount()
    @description - Sets max mint amount
  */
  function setMaxMintAmount(uint256 _newmaxMintAmount) public onlyOwner {
   ...
   /*
    ...
    @description - Sets the number of NFTs a whitelisted address can mint
  */
  function setMaxMintAmountForWhitelist(uint256 _limit) public onlyOwner {
    maxMintAmountForWhitelist = _limit;
  }
  ...
  */
  ...
  /*
    @function dynamicCostState()
    @description - Turns dynamic costs on or off (True/False)
  */
  function dynamicCostState(bool _state) public onlyOwner {
      dynamicCost = _state;
  }

  /*
    @function setOnlyWhitelisted()
    @description - Setting whitelist address to True/False for minting
  */
  ...
  /*
    ...
    @description - Add user address to whitelist for priority minting
  */
  function addWhitelistUsers(address[] calldata _users) public onlyOwner {
    ...
    /*
      ...
      @description - Manually activates lottery
    */
      function activateLottery(bool state) public onlyOwner {
        lotteryActive = state;
  ...
  */
    ...
    */
      function withDrawToStake() public payable onlyOwner {
