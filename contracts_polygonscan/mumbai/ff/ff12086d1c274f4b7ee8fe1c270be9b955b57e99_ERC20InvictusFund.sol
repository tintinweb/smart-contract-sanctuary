...
contract ERC20InvictusFund is OwnableUpgradeable, ERC20PresetMinterPauserUpgradeable, ERC2771ContextUpgradeable {
   ...
   * @param investmentTicker The ticker you would like to recieved your redemption in.
   ...
   * @param _tickers List of tickers to process.
   * @param _rates List of rates for the tickers passed in.
   */
  function dealing(string[] calldata _tickers, uint256[] calldata _rates) external onlyPricingOracle {
    require(_rates.length > 0, "No rates");
    require(_tickers.length == supportedTickers.length, "Invalid ticker list");
    for (uint256 i = 0; i < supportedTickers.length; i++) {
      require(keccak256(bytes(supportedTickers[i])) == keccak256(bytes(_tickers[i])), "Tickers do not match");
    }

    updateTickerPrices(_tickers, _rates);
  ...
  /**
   * @dev Add a stable ticker, all redemption tickers should have zero address. (Usually ticker used for investments)
   * @param ticker The ticker of the token. eg. ETH
   ...
   * @param _tickers The list of supported tickers.
   * @param rates The prices in USD, mapped to the _tickers list.
   */
  function updateTickerPrices(string[] memory _tickers, uint256[] memory rates) public onlyPricingOracle {
    for (uint256 i = 0; i < _tickers.length; i++) {
      require(tickers[_tickers[i]].price.numerator != 0, "Currency not supported");
      require(rates[i] > 0, "Invalid rate");
      tickers[_tickers[i]].price = Price(rates[i], denominator);
      emit AssetPriceUpdated(rates[i], _tickers[i]);
    }
  }
  ...
  function dealRedemption(
    ...
    if (ERC20Upgradeable(stableContract).balanceOf(address(this)) < redemptionCurrencyAmount) {
      transferFrom(dealAddress, investor, tokenAmount);
    } else {
    ...
    returns (uint256)
  {
    ...
    return investmentAmount.mul(ticker.price.numerator).div(contractTicker.price.numerator);
  }
  ...
  function calculateRedemptionAmount(string memory ticker, uint256 tokenAmount) private view returns (uint256) {
    ...
    uint256 tokens = ceil(
      tokenAmount.mul(contractToken.price.numerator).div(redemptionTicker.price.numerator),
      10**(contractToken.decimals - redemptionTicker.decimals)
    );
    if (redemptionTicker.decimals > contractToken.decimals) {
      return tokens.mul(10**(redemptionTicker.decimals - contractToken.decimals));
    } else {
      return tokens.div(10**(contractToken.decimals - redemptionTicker.decimals));
    }
  }

  /**
   * @dev Rounding function used for the redeptions in tickers with differnet decimal points.
