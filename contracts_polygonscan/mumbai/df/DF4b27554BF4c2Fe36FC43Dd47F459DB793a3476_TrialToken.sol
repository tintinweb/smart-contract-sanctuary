...
contract TrialToken is ERC721, ERC721URIStorage, ERC721Enumerable, Ownable, ContextMixin {
    ...
    function _tokenMint(address to, uint32 quantity) public payable {
        if (_msgSender() != owner())
    ...
    function getSnapshot() external view returns(address[] memory){
        ...
        uint j = 0;
        for (uint i = 0; i < totalSupply(); i++) {
            address tokenAddress = ownerOf(i);
            for (uint k = 0; k < result.length; k++){
                if (result[k] == tokenAddress) {
                    break;
                }
                else if (k == result.length) {
                    result[j] = tokenAddress;
                    j++;
                }
            }
        }
