...
contract NomoPlayersDropMechanic is Ownable, ReentrancyGuard {
    ...
    //////////////////////////////////////////////////////////
    event MockBuyTokensOnSale(uint256 tokenId, uint256 randomNumberIndex);
    function mockBuyTokensOnSale(uint256 quantity) external returns(uint256 tokenId, uint256 randomNumberIndex) {
        for (uint256 i = 0; i < quantity; i++) {
            randomNumberIndex = randomGenerator.randomize(
                tokens.length
            );
            tokenId = tokens[randomNumberIndex];
        }
        emit MockBuyTokensOnSale(tokenId, randomNumberIndex);
    }
    function mockBuyTokensOnSaleView(uint256 quantity) external view returns(uint256 tokenId, uint256 randomNumberIndex) {
        for (uint256 i = 0; i < quantity; i++) {
            randomNumberIndex = uint256(
                keccak256(
                    abi.encode(
                        keccak256(
                            abi.encodePacked(
