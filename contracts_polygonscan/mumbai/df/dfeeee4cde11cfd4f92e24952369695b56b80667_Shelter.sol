    ...
    Pausable
{
    ...
    uint256 public bonePerAlpha;
    // pup earn 100 $BONE per day
    uint256 public DAILY_BONE_RATE;
    ...
    uint256 public BONE_CLAIM_TAX_PERCENTAGE;
    // there will only ever be (roughly) 10 billion $BONE earned through staking
    uint256 public MAXIMUM_GLOBAL_BONE;
    ...
    function initializeShelter(address _punkedPup, address _bone)
        public
        initializer
    {
        Ownable.initialize();
        ...
        bonePerAlpha = 0;
        DAILY_BONE_RATE = 100 ether;
        MINIMUM_TO_EXIT = 2 days;
        BONE_CLAIM_TAX_PERCENTAGE = 20;
        MAXIMUM_GLOBAL_BONE = 10000000000 ether;
        rescueEnabled = false;
    ...
    // function generateAllTokenId(uint16[] calldata tokenIds)
    //     public
    //     returns (
    //         uint256,
    //         uint256,
    ...
    //         uint8 alphaIndex;
    //         // uint256 supply = punkedPup.totalSupply();
    ...
    //             (seed % 2 == 0 || seed % 3 == 0) &&
    //             regularPupCount <
    //             (maxRegularPupCount * ((supply / 1000) + 1)) / 10
    //         ) {
    //             // add in regular pups
    //             regularPupCount++;
    //             alphaIndex = uint8(randomWithRange(seed, 5)) + 1;
    //             tokenTraits[tokenIds[i]] = PupCat(true, alphaIndex, false);
    //             NFTtype[tokenIds[i]] = "regular";
    //         } else {
    //             // add in other catagories
    //             uint256 seed2 = random(seed);
    //             if (
    //                 seed2 % 2 == 0 &&
    //                 pugCount < (maxPugCount * ((supply / 1000) + 1)) / 10
    //             ) {
    //                 // add in pugs
    //                 pugCount++;
    //                 alphaIndex = 6;
    //                 tokenTraits[tokenIds[i]] = PupCat(true, alphaIndex, false);
    //                 NFTtype[tokenIds[i]] = "pugs";
    //             } else {
    //                 uint256 seed3 = random(seed2);
    //                 if (
    //                     seed3 % 2 == 0 &&
    //                     catCount < (maxCatCount * ((supply / 1000) + 1)) / 10
    //                 ) {
    //                     // add in cats
    //                     catCount++;
    //                     alphaIndex = uint8(randomWithRange(seed, 5)) + 1;
    //                     tokenTraits[tokenIds[i]] = PupCat(
    //                         false,
    //                         alphaIndex,
    //                         false
    //                     );
    //                     NFTtype[tokenIds[i]] = "cats";
    //                 } else if (
    //                     seed3 % 3 == 0 &&
    //                     dogCatcherCount <
    //                     (maxDogCatcherCount * ((supply / 1000) + 1)) / 10
    //                 ) {
    //                     // add in dog catcher
    //                     dogCatcherCount++;
    //                     alphaIndex = uint8(randomWithRange(seed, 5)) + 1;
    ...
    //                         true
    //                     );
    //                     NFTtype[tokenIds[i]] = "dogcatchers";
    //                 } else if (
    //                     seed3 % 5 == 0 &&
    //                     customPupCount <
    //                     (maxCustomPupCount * ((supply / 1000) + 1)) / 10
    //                 ) {
    //                     // add in custom pups
    //                     customPupCount++;
    //                     alphaIndex = 8;
    ...
    function _addCatToPack(address account, uint256 tokenId) internal {
        ...
        if (isDogCatcher(tokenId)) {
            ...
            dogHouseIndices[tokenId] = dogHouse.length - 1; // Store the location of the dogCatcher in the dogHouse
        }
        ...
        returns (uint256 owed)
    {
        ...
        bool isPupVar;
        uint8 alpha;
        bool isDogCatcherVar;
        (isPupVar, alpha, isDogCatcherVar) = punkedPup.tokenTraits(tokenId);
        // uint8 alpha = punkedPup.tokenTraits[tokenId].alphaIndex;
        ...
        returns (uint256 owed)
    {
        ...
        if (unstake) {
            ...
            if (isDogCatcher(tokenId)) {
                dogHouse[dogHouseIndices[tokenId]] = dogHouse[
                    dogHouse.length - 1
                ]; // Shuffle last DogCatcher to current position
                dogHouseIndices[
                    dogHouse[dogHouse.length - 1]
                ] = dogHouseIndices[tokenId];
    ...
    function isPup(uint256 tokenId) public view returns (bool pup) {
        ...
        (isPupVar, alpha, isDogCatcherVar) = punkedPup.tokenTraits(tokenId);
        return isPupVar;
        // return punkedPup.tokenTraits[tokenId].isPup;
    }
        ...
        returns (bool dogCatcher)
    {
        ...
        return isDogCatcherVar;
        // return punkedPup.tokenTraits[tokenId].isDogCatcher;
    }
    ...
    function _alphaForCat(uint256 tokenId) internal view returns (uint8) {
        ...
        uint8 alphaIndex;
        bool isDogCatcherVar;
        (isPupVar, alphaIndex, isDogCatcherVar) = punkedPup.tokenTraits(
            tokenId
        );
        // uint8 alphaIndex = punkedPup.tokenTraits[tokenId].alphaIndex;
