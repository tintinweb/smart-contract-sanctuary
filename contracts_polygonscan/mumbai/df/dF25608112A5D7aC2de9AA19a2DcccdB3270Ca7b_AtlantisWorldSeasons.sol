    ...
    AccessControlEnumerable
{
    ...
    // /**
    //  * @dev Keeps track of all the mint count for each {SeasonLoot} stored
    //  * in `_seasonLootIdToSeasonLoot` mapping since there are various {SeasonLoot}
    //  * with different JSON metadata structure. This mapping is most
    //  * likely used by the method `getSeasonTotalMintCount` to expose
    //  * the total mint count for each {SeasonLoot} NFTs.
    //  *
    //  * @custom:deprecated
    //  * This does not work as per {ERC721-ownerOf} standard.
    //  */
    // mapping(uint256 => Counters.Counter) private _seasonLootIdToCount;
    ...
    // /**
    //  * @param seasonLootId The identifier of a {SeasonLoot} from `_seasonLootIdToSeasonLoot` mapping
    //  * @return The total count for all that have minted of a specific {SeasonLoot}
    //  *
    //  * @custom:deprecated
    //  * This does not work as per {ERC721-ownerOf} standard.
    //  */
    // function getSeasonTotalMintCount(uint256 seasonLootId)
    //     external
    //     view
    //     returns (uint256)
    // {
    //     return _seasonLootIdToCount[seasonLootId].current();
    // }
     ...
     * @param tokenURI The base64 ecnoded JSON metadata for a {SeasonLoot}
     * @return The current `seasonLootId` for the recently created {SeasonLoot}
     */
    function createSeasonLoot(string calldata tokenURI)
        external
        onlyAdmin
        returns (uint256)
    {
