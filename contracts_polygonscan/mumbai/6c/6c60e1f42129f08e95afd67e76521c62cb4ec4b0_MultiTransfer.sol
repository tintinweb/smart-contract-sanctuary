...
contract MultiTransferLocal is Pausable {

event MultiTransferLocalEvent(address indexed _from,uint256 indexed _value,address _to,uint256 _amount);
event MultiTransferTokenEvent(address indexed _from,uint256 indexed _value,address _to,uint256 _amount);
  ...
  payable external whenNotPaused returns(bool)
  {
    ...
    for (uint8 i; i < _addresses.length; i++) {
      ...
      emit MultiTransferLocalEvent(msg.sender, msg.value, _addresses[i], _amounts[i]);
    }
    return true;
  }
  ...
  ) payable external whenNotPaused
  {
    ...
    for (uint8 i; i < _addresses.length; i++) {
      ...
      emit MultiTransferTokenEvent(msg.sender, msg.value, _addresses[i], _amounts[i]);
    }
  }
}

contract MultiTransfer is Pausable, Escapable, MultiTransferLocal
{
  ...
  receive() external payable {
    revert("Can not accept token directly.");
  }
