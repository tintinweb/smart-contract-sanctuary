...
contract NomoVault is OwnableUpgradeable, ERC20Upgradeable {
    ...
    function nftSaleCallback(
        ...
        require(
            tokensIds.length == prices.length,
            "NomoVault: length mismatch"
        );
    ...
    function distributeReward() external onlyOwner {
        ...
        for (uint256 i = 0; i < leagueIds.length; i++) {
            address league = router.leagues(leagueIds[i]);
     ...
     */
    function upgradeStrategy(IStrategy strategy_) public onlyOwner {
        strategy.getReward();
     ...
     */
    function removeLeagues(uint256[] memory removedLeagueIds)
        external
        onlyOwner
    {
    ...
    function want() public view returns (IERC20) {
        return IERC20(strategy.wantToken());
    }
     ...
     */
    function _getTokenLeague(uint256 tokenId)
        private
        view
