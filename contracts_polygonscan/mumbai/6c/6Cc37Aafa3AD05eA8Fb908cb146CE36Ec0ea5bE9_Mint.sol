...
interface ILandsNftContract {
    ...
    function isTokensNumberBreakTargetLimit(address to, uint delta) external view returns (bool);
...
contract Mint is Ownable, Pausable {
    ...
    function buyToken(uint256 tokenId) public whenNotPaused {
        ...
        require(_nftsContract.isTokensNumberBreakTargetLimit(_msgSender(), 1), "Mint not allowed: exceeded max token number in target wallet");
    ...
    function buyTokens(uint tokensNumber) public whenNotPaused {
        ...
        require(_nftsContract.isTokensNumberBreakTargetLimit(_msgSender(), tokensNumber), "Mint not allowed: exceeded max token number in target wallet");
