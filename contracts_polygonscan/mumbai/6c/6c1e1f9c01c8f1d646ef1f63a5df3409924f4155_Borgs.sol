...
contract Borgs is ERC721Enumerable, Ownable, ContractStates, CommonObjects, NumberUtils{
    ...
    Listing[] private _displayListings;
    
    // Listing positions
    mapping (uint256 => uint256) private _displayListingToTokenIndexes;
    ...
    function removeListing(uint256 tokenId) public{
        ...
        _removeDisplayListing(listing.borgId);
        
        // Transfer listing back to user
        this.transferFrom(address(this), listing.seller, tokenId);
    ...
    function purchaseListing(uint256 tokenId) public payable{
        ...
        require(listing.buyer == msg.sender || listing.buyer == address(0), "Buyer has either been specified or does not match callers address");
        
        // Check amount is enough to match listing
        require(listing.price == msg.value, "Price has not been met");
    ...
    function recover(address payable toAddress, uint256 amount) public onlyOwner{
    ...
    function _addDisplayListing(Listing memory listing) internal {
        // Add the listing
        _displayListings.push(listing);
    }
    
    function _removeDisplayListing(uint tokenId) internal returns(bool) {
        // Get index of the listing
        uint index = _displayListingToTokenIndexes[tokenId];
        
        // Ensure index is in bounds of array
        if (index >= _displayListings.length) 
            return false;

        // Shift the array
        for (uint i = index; i<_displayListings.length-1; i++){
            _displayListings[i] = _displayListings[i+1];
        }
        
        // Remove it from list
        _displayListings.pop();
        return true;
    }
