...
contract Trade{
    ...
    */
    ...
    function buyToken(address _token, uint _price, uint _amount) public{
        ...
        uint ethRequired = _price * _amount / 1000000000000000000;
        require(ethRequired > 0, "ethRequired must > 0");
        
        //checks for overflow
        require(ethBalance[msg.sender] >= ethRequired, "Your's balance don't have enough to make this transaction" );
        require(ethBalance[msg.sender] - ethRequired >= 0, "Your's balance don't have enough to make this transaction");
        require(ethBalance[msg.sender] - ethRequired <= ethBalance[msg.sender], "Your's balance don't have enough to make this transaction");
        ...
        if (loadedToken.amountSellPrice == 0 || loadedToken.minSellPrice > _price){
            ...
            while(buyPrice <= _price && remainingAmount > 0 && offerPointer <= loadedToken.sellBook[buyPrice].offerLength){
                ...
                while(offerPointer <= loadedToken.sellBook[buyPrice].offerLength && remainingAmount > 0){
                    ...
                    if(volumeAtPointer <= remainingAmount) {
                        ethAmount = volumeAtPointer * buyPrice / 1000000000000000000;
                        ...
                        ethAmount = remainingAmount * buyPrice / 1000000000000000000;
                        require(ethBalance[msg.sender] - ethAmount <= ethBalance[msg.sender], "ethBalance[msg.sender] - ethAmount must <= ethBalance[msg.sender]");
            ...
            uint ethFee = (qty * _price / 1000000000000000000) * fee / 1000;
    ...
    function sellToken(address _token, uint _price, uint _amount) public{
        ...
        require(tokenBalance[msg.sender][_token] >= _amount, "Your's token don't have enough to make this transaction" );
        ...
        if(loadedToken.amountBuyPrices == 0 || loadedToken.maxBuyPrice < _price ){
            ...
            while (sellPrice >= _price && remainingAmount > 0 && offerPointer <= loadedToken.buyBook[sellPrice].offerLength){
                ...
                while(offerPointer <= loadedToken.buyBook[sellPrice].offerLength && remainingAmount>0){
                    ...
                    if (volumeAtPointer <= remainingAmount){
                        uint ethRequiredNow = volumeAtPointer * sellPrice / 1000000000000000000;
                        ...
                        ethRequired = remainingAmount * sellPrice / 1000000000000000000;
