...
contract MaticFaucet is AccessProtected {

    uint public faucetAmount = 0.1 ether;

    event Deposit(address indexed depsitor, uint amount);

    receive() external payable {
        emit Deposit(msg.sender, msg.value);
    }

    function getMatic() external onlyAdmin {
        require(getBalance() >= faucetAmount, "not enough matic in contract");
    ...
    function setFaucetAmount(uint _newAmount) external onlyOwner {
