...
contract ReservedPool is Ownable, ReentrancyGuard {
  ...
  uint256 public frStart = 17380000;
  ...
  modifier firstRelease() {
    require(block.number >= frStart, "Claim is not allowed.");
    _;
  }
  ...
  function releaseDataGen() public nonReentrant {
    ...
    if(srStart > block.number ) {
      // uint256 epochs = block.number.sub(frStart).div(15 * 24 * 3600);
      uint256 epochs = (block.number.sub(frStart)).div(300).add(1);
      ...
      uint256 epochs = (block.number.sub(srStart)).div(12 * 3600).add(1);
