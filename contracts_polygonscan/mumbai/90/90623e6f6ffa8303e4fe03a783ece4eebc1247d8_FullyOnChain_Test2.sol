...
// FullyOnChain_Kanji
contract FullyOnChain_Test2 is ERC721Enumerable, ReentrancyGuard, Ownable {
  using Strings for uint256;
  ...
  string[] public cha = new string[](1000);
  string[] public meanx = new string[](1000);
  ...
  string private rl1='{"name": "FullyOnChain_Test #';
  string private rl3='"}';
  ...
  function randomOne(uint256 tokenId) internal view returns (Kanji memory) {
    Kanji memory kanji;
    kanji.bg = usew(bg_w,random(string(abi.encodePacked(ra1,tokenId.toString()))) % 65);
    kanji.ccol = usew(ccol_w,random(string(abi.encodePacked(ra3,tokenId.toString()))) % 215);
    kanji.rg1 = uint256(random(string(abi.encodePacked(ra4,tokenId.toString()))) % 13+7).toString();
    kanji.rg2 = uint256(random(string(abi.encodePacked(ra5,tokenId.toString()))) % 13+7).toString();
    kanji.rg3 = uint256(random(string(abi.encodePacked(ra2,tokenId.toString()))) % 13+7).toString();
    return kanji;
  }
  ...
  function genSVG1(Kanji memory kanji) internal view returns (string memory) {
    ...
    string memory output= string(abi.encodePacked(zb[1],meanx[tokenId],zb[2],bg[kanji.bg]));
    ...
    return string(abi.encodePacked(Base64.encode(bytes(genSVG1(kanji))),bytes(cha[tokenId]),Base64.encode(bytes(genSVG2(kanji,tokenId)))));
  }
...
//}
  ...
  function tokenURI(uint256 tokenId) override public view returns (string memory) {
    Kanji memory kanji = randomOne(tokenId);
    string memory title = string(abi.encodePacked(rl1,tokenId.toString()," ",meanx[tokenId],getTraits(kanji)));
    return string(abi.encodePacked(rl4,Base64.encode(bytes(title)),Base64.encode(bytes(genSVG(kanji,tokenId))),rl3));
  }


  function claim(uint256 tokenId,string memory chara, string memory mean) public nonReentrant onlyOwner{
...
//gasless
  ...
  constructor() ERC721("FullyOnChain_Test2", "TST") Ownable() {}
}