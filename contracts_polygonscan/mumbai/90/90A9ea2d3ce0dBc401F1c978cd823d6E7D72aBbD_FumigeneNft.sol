...
contract FumigeneNft is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable, EIP712MetaTransaction {
  using Counters for Counters.Counter;

  Counters.Counter private _tokenIdCounter;

  constructor()  ERC721("Fumigene", "FUM") {}
  ...
  function safeMint(address to, string memory uri, address addressToApprove) public onlyOwner returns(uint, address) {
...
contract EIP712MetaTransaction {
    ...
    bytes32 internal constant META_TRANSACTION_TYPEHASH = keccak256(bytes("MetaTransaction(uint256 nonce,address from,bytes functionSignature)"));
    bytes32 internal DOMAIN_SEPARATOR = keccak256(abi.encode(
            EIP712_DOMAIN_TYPEHASH,
            keccak256(bytes("Fumigene")),
            keccak256(bytes("1")),
                bytes32(getChainID()), // Local
            address(this)
        ));
    ...
    function executeMetaTransaction(address userAddress, bytes memory functionSignature, bytes32 r, bytes32 s, uint8 v)
        ...
        bool valid = verify(userAddress, metaTx, r, s, v);
    ...
    function verify(address user, MetaTransaction memory metaTx, bytes32 sigR, bytes32 sigS, uint8 sigV) internal view returns (bool) {
        address signer = ecrecover(hashMetaTransaction(metaTx), sigV, sigR, sigS);
