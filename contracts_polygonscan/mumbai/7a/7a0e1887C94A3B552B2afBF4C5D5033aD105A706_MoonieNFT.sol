...
// File contracts/common/IChildERC721Mintable.sol



pragma solidity 0.8.4;

interface IChildERC721Mintable is IERC721 {
...
// File contracts/matic/MoonieNFT.sol
    ...
    ContextMixin
{
    ...
    function withdraw(uint256 tokenId) external {
        require(_msgSender() == ownerOf(tokenId), "MoonieNFT: INVALID_TOKEN_OWNER");
    ...
    function withdrawBatch(uint256[] calldata tokenIds) external {
        ...
        uint256 length = tokenIds.length;
        require(length <= BATCH_LIMIT, "MoonieNFT: EXCEEDS_BATCH_LIMIT");
        ...
        for (uint256 i; i < length; i++) {
            ...
            require(_msgSender() == ownerOf(tokenId), string(abi.encodePacked("MoonieNFT: INVALID_TOKEN_OWNER ", tokenId)));
            withdrawnTokens[tokenId] = true;
            _burn(tokenId);
     ...
     */
    function mint(address user, uint256 tokenId) external override only(MINTER_ROLE)  {
        require(!withdrawnTokens[tokenId], "MoonieNFT: TOKEN_EXISTS_ON_ROOT_CHAIN");
        _mint(user, tokenId);
        // TODO URI storage
    }
