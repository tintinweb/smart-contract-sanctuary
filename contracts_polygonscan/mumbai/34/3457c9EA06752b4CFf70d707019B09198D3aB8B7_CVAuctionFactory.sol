...
/** 
 *  SourceUnit: /Users/iggyi/Git/culture-vault/cv-contract-hardhat/contracts/auction-v5-factory.sol
*/
...
contract StandardAuctionV5 is Ownable, Pausable, ReentrancyGuard {
    using SafeMath for uint256;
    event Start();
    ...
    constructor(
        ...
        if (ERC165Checker.supportsInterface(_nft, 0x80ac58cd)) {
            nft721 = IERC721(_nft);
        } else if(ERC165Checker.supportsInterface(_nft, 0xd9b67a26)) {
            nft1155 = IERC1155(_nft);
        } else {
            revert("NFT not supported");
        }
        nft = _nft;
        nftId = _nftId;
        ...
        Tradable tradeable_contract = Tradable(nft);
        Tradable.Recipient[] memory cv_sale =  tradeable_contract.getSaleRecipients();
        for (uint256 i = 0; i < cv_sale.length; i++) {
            cv_sale[i].recipient = SPayees[i];
            cv_sale[i].points = SShares[i];
        }
        Tradable.Recipient[] memory cv_fee = tradeable_contract.getFeeRecipients();
        for (uint256 i = 0; i < cv_fee.length; i++) {
            cv_fee[i].recipient = fPayees[i];
            cv_fee[i].points = fShares[i];
        }

        require(SPayees.length == SShares.length, "PaymentSplitter: sales payees and shares length mismatch");
        require(SPayees.length > 0, "PaymentSplitter: no sales payees");
    }
    ...
    function start() external onlyOwner whenNotPaused {
        ...
        if (ERC165Checker.supportsInterface(nft, 0x80ac58cd)) {
            require(nft721.balanceOf(msg.sender) >= 1, "auction can't be started as you don't have the nft");
        } else {
            require(nft1155.balanceOf(msg.sender, nftId) >= 1, "auction can't be started as you don't have the nft");
        }
    ...
    function bid() external payable whenNotPaused nonReentrant {
        require(msg.sender.balance >= startingBid + tx.gasprice, "Insufficient balance to pay for both NFT and gas"); 
        ...
        require(msg.value >= startingBid, "you must bid at least the starting bid");
        ...
        // refund money back to last bidder execpt the first bidder
        if (highestBid > 0) {
    ...
    function forceAcceptEnd() external whenNotPaused onlyOwner nonReentrant {
        ...
        if (highestBidder != address(0)) {
            if (ERC165Checker.supportsInterface(nft, 0x80ac58cd)) {
                nft721.safeTransferFrom(msg.sender, highestBidder, nftId);
            } else {
                nft1155.safeTransferFrom(msg.sender, highestBidder, nftId, nftAmount, "");
            }
            // TODO shares redefine
            // for (uint256 i = 0; i < SPayees.length; i++) {
            //     release(payable(SPayees[i]));
            // }
            settlement(address(this).balance);
        } else {
            if (ERC165Checker.supportsInterface(nft, 0x80ac58cd)) {
                nft721.safeTransferFrom(msg.sender, seller, nftId);
            } else {
                nft1155.safeTransferFrom(msg.sender, seller, nftId, nftAmount, "");
            }
        }
    ...
    function getIsApproved() external view returns (bool) {
        if (ERC165Checker.supportsInterface(nft, 0x80ac58cd)) {
                return nft721.isApprovedForAll(seller, address(this));
            } else {
                return nft1155.isApprovedForAll(seller, address(this));
            }
    }
    ...
    //     return (totalReceived * shares_[account]) / _totalShares - alreadyReleased;
    // }

    // function release(address payable account) private {
    //     require(shares_[account] > 0, "PaymentSplitter: account has no shares");
    ...
    //     require(shares_[account] == 0, "PaymentSplitter: account already has shares");

    //     payees.push(account);
    //     shares_[account] = _shares;
    //     _totalShares = _totalShares + _shares;
    ...
    function settlement(
        ...
        Tradable tradeable_contract = Tradable(nft);
        uint256 fee = money.mul(tradeable_contract.getSellerFeeBasisPoints()).div(100);
        uint256 receipts = money.sub(fee);
        uint256 firstAmount = tradeable_contract.getFistAmount(seller, nftId);
        if (firstAmount >= nftAmount) {
            _settlement(tradeable_contract.getSaleRecipients(), receipts);
        } else if (firstAmount > 0) {
            uint256 firstReceipts = receipts.div(nftAmount).mul(firstAmount);
            _settlement(tradeable_contract.getSaleRecipients(), firstReceipts);
            payable(address(seller)).transfer(receipts.sub(firstReceipts));
        } else {
            payable(address(seller)).transfer(receipts);
        }
        _settlement(tradeable_contract.getFeeRecipients(), fee);
    }
...
////import "./auction-v5.sol";

contract CVAuctionFactory {
    StandardAuctionV5[] public auctions;

    function create(address _nft,
        uint256 _nftId,
        uint256 _nftAmount,
        uint256 _startingBid, 
        uint256 _endAt, 
        ...
        StandardAuctionV5 auction = new StandardAuctionV5(_nft, _nftId, _nftAmount, _startingBid, _endAt, _startAt, _seller);
    ...
    {
        StandardAuctionV5 auction = auctions[_index];

        return (auction.nftId(), auction.nftAmount(), auction.seller(), auction.endAt(), auction.startAt(), auction.started(), auction.ended(), auction.highestBidder(), auction.highestBid(), auction.startingBid(), address(auction).balance);
    }
}