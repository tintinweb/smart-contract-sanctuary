...
import "./NidhiMarket.sol";

contract NidhiNFT is ERC721, INidhiNFT {
    ...
    string private _marketBaseURI;
    string private _tokenBaseURI;

    NidhiMarket private immutable _marketplace;

    mapping (uint => address) private _tokenCreators;
    mapping (uint => bool) private _publiclyTradeable;
    mapping (uint => StoredMetadata) private _metadata;
    ...
    constructor(
        ...
        string memory marketBaseURI,
        string memory tokenBaseURI
    )
        ERC721("Nidhi Passive Income NFT", "NPI")
    {
        _marketBaseURI = marketBaseURI;
        _tokenBaseURI = tokenBaseURI;
        _owner = msg.sender;
        _marketplace = NidhiMarket(marketplaceAddress);
        setApprovalForAll(marketplaceAddress, true);
    }

    /**
     * @dev Creates a new token with the provided name, description
     * and image URI. Upon successful completion, the `TokenCreated`
        ...
        external returns (uint tokenId)
    {
        ...
        _afterTokenTransfer(address(0), creator, tokenId);
        StoredMetadata memory meta;
        meta.name = name_;
        meta.description = description_;
        meta.image = imageURI_;
    ...
    /**
     * @dev Allows or prevents NFT to be traded on other marketplaces.
     * Public trading will be allowed after the initial sale on Nidhi's
     * marketplace. This ensures that every Nidhi NFT has an intrinsic
     * value.
     */
    function approvePublicTrading(
        uint tokenId,
        bool approve
    ) external override {
        require(msg.sender == address(_marketplace));
        _publiclyTradeable[tokenId] = approve;
    }

    function setMarketBaseURI(string memory baseURI) external onlyOwner {
        _marketBaseURI = baseURI;
    }
    ...
    /**
     * @dev Burns the token, removes all its stored data and also ensures
     * that references to it are removed from the Nidhi marketplace.
     */
    function burn(uint tokenId) external {
        ...
        _burn(tokenId);
        _marketplace.afterBurnToken(
            _tokenCreators[tokenId],
            address(this),
            tokenId
        );
        delete _tokenCreators[tokenId];
        delete _publiclyTradeable[tokenId];
    ...
    {
        require(_exists(tokenId), "Meta query for nonexistent token");
        uint itemId = _marketplace.getMarketItemId(address(this), tokenId);
        StoredMetadata storage storedMeta = _metadata[tokenId];
        meta.name = storedMeta.name;
        meta.description = storedMeta.description;
        meta.image = storedMeta.image;
        if (itemId != 0) {
            meta.externalURL = string(
                abi.encodePacked(_marketBaseURI, Strings.toString(itemId))
            );
            meta.intrinsicValue = _marketplace.intrinsicValue(itemId);
        }
    }

    /**
     * @dev Returns the address of the creator of the provided token id.
     */
    function creatorOf(uint tokenId) external view override returns (address) {
    ...
    {
        require(
            from == address(0) || to == address(0) || _publiclyTradeable[tokenId],
            "public trading not allowed"
        );
    }
    ...
    function _afterTokenTransfer(address from, address to, uint256 tokenId)
        private
    {
        if (from != address(_marketplace) && to != address(_marketplace) && from != to) {
            if (from != address(0)) {
                _marketplace.removeTokenFromOwnerCollection(address(this), tokenId);
            }
            if (to != address(0)) {
                _marketplace.importMarketItem(address(this), tokenId);
            }
            if (from == address(0)) {
                _marketplace.addTokenToCreatorCollection(to, address(this), tokenId);
            }
        }
    }
...
contract NidhiMarket is ReentrancyGuard {
    ...
    uint public minPrice = 1e9; // 1 GURU
        ...
        external nonReentrant
    {
        ...
        _approveForPublicTrading(nftContract, tokenId, true);

        erc721.transferFrom(seller, address(this), tokenId);

        _removeItemFromOwnerCollection(seller, itemId);
        _listedItems.append(itemId);

        address creator = _getCreator(nftContract, tokenId);
        if (creator != address(0)) {
            _addListedItemToCreatorCollection(creator, itemId);
        }
    ...
    function delistMarketItem(uint itemId) external nonReentrant {
        ...
        _addItemToOwnerCollection(seller, itemId);
        _listedItems.remove(itemId);

        address creator = _getCreator(nftContract, tokenId);
        if (creator != address(0)) {
            _removeListedItemFromCreatorCollection(creator, itemId);
        }

        if (item.stakedAmount == 0) {
            _approveForPublicTrading(nftContract, tokenId, false);
        }
    ...
    function importMarketItem(address nftContract, uint tokenId) external {
        ...
        require(
            owner == msg.sender || _isWhitelistedNFT[msg.sender],
        ...
        _addItemToOwnerCollection(owner, itemId);
    ...
    function purchaseMarketItem(
        ...
        require(_isPaymentToken[tokenAddress], "invalid payment token");
        ...
        if (item.price > 0) {
            ...
            address[] storage path = _routerPaths[tokenAddress];
            if (path.length == 0) {
                GURU.safeTransferFrom(buyer, address(this), item.price);
            } else {
                IERC20(tokenAddress).safeTransferFrom(
                    buyer,
                    address(this),
                    amountInMax);
                IERC20(tokenAddress).approve(address(router), amountInMax);
                uint[] memory amounts = router.swapTokensForExactTokens(
                    ...
                    address(this),
                    block.timestamp);
                uint overpaid = amountInMax - amounts[0];
                if (overpaid > 0) {
                    IERC20(tokenAddress).transfer(buyer, overpaid);
                }
            }
            ...
            if (item.payoutAmount > 0) {
                if (fee > 0) {
                    ...
                    GURU.transfer(item.seller, payout);
                    GURU.transfer(feeCollector, feeAmount);
                } else {
                    GURU.transfer(item.seller, item.payoutAmount);
                }
            }

            uint staked = item.price - item.payoutAmount;
        ...
        _listedItems.remove(itemId);
        _addItemToOwnerCollection(buyer, itemId);

        address creator = _getCreator(item.nftContract, item.tokenId);
        if (creator != address(0)) {
            _removeListedItemFromCreatorCollection(creator, itemId);
        }
    ...
    function redeem(uint itemId) external {
        ...
        SafeERC20.safeTransfer(SGURU, owner, rv);
        item.stakedAmount = iv;
        item.gons = SGURU.gonsForBalance(item.stakedAmount);
    ...
    function afterBurnToken(
        ...
        if (itemId != 0) {
            ...
            _removeItemFromCreatorCollection(creator, itemId);
            MarketItem storage item = _idToMarketItem[itemId];
    ...
    function removeTokenFromOwnerCollection(
        ...
        if (itemId != 0) {
            ...
            _removeItemFromOwnerCollection(owner, itemId);
        }
    }
    ...
    /**
      * @dev Returns a page of other listed market items by creator.
      */
    function fetchOtherItemsByCreator(uint itemId, uint pageSize)
        public view returns (MarketItemDetails[] memory items)
    {
        MarketItem storage item = _idToMarketItem[itemId];
        address creator = _getCreator(item.nftContract, item.tokenId);
        NidhiCollection listedItems = _listedItemsByCreator[creator];
        if (creator != address(0) && address(listedItems) != address(0)) {
            uint itemCount = listedItems.size();
            if (itemCount > 1) {
                NidhiCollection.Item memory current = listedItems.tail();
                uint resultSize;
                for (uint i = 0; i < itemCount && resultSize < pageSize; i++) {
                    item = _idToMarketItem[current.itemId];
                    if (item.itemId != itemId) {
                        resultSize++;
                    }
                    current = listedItems.get(current.prev);
                }
                if (resultSize > 0) {
                    items = new MarketItemDetails[](resultSize);
                    current = listedItems.tail();
                    uint j = 0;
                    while (j < resultSize) {
                        item = _idToMarketItem[current.itemId];
                        if (item.itemId != itemId) {
    ...
    function _approveForPublicTrading(
        address nftContract,
        uint tokenId,
        bool approve
    ) private {
        try INidhiNFT(nftContract).approvePublicTrading(tokenId, approve) {
        } catch {
            // Any outcome is okay here.  If the call failed, assume that the NFT
            // contract was not a NidhiNFT contract. In that case we can't and we
            // we don't want to stop it from being publicly traded.
        }
    }

    /**
     * @dev Ensure the owner's collection exists and add the provided item id.
     */
    function _addItemToOwnerCollection(
        address owner,
        uint itemId
    ) private {
        if (address(_itemsByOwner[owner]) == address(0)) {
            _itemsByOwner[owner] = new NidhiCollection();
        } else {
            _purgeOwnerCollection(owner);
        }
        if (_itemsByOwner[owner].get(itemId).itemId != itemId) {
            _itemsByOwner[owner].append(itemId);
        }
    }

    /**
     * @dev Ensure the creator's collection exists and add the provided item id.
     */
    function _addListedItemToCreatorCollection(
        address creator,
        uint itemId
    ) private {
        if (address(_listedItemsByCreator[creator]) == address(0)) {
            _listedItemsByCreator[creator] = new NidhiCollection();
        }
        _listedItemsByCreator[creator].append(itemId);
    }

    /**
     * @dev Removes the provided item id from the creator's collection.
     */
    function _removeItemFromCreatorCollection(
        address creator,
        uint itemId
    ) private {
        if (address(_itemsByCreator[creator]) != address(0)) {
            _itemsByCreator[creator].remove(itemId);
        }
    }

    /**
     * @dev Removes the provided item id from the owner's collection.
     */
    function _removeItemFromOwnerCollection(
        address owner,
        uint itemId
    ) private {
        if (address(_itemsByOwner[owner]) != address(0)) {
            _itemsByOwner[owner].remove(itemId);
            _purgeOwnerCollection(owner);
        }
    }
     ...
     * @dev Removes the provided item id from the creator's collection.
     */
    function _removeListedItemFromCreatorCollection(
        address creator,
        uint itemId
    ) private {
        if (address(_listedItemsByCreator[creator]) != address(0)) {
            _listedItemsByCreator[creator].remove(itemId);
        }
    }

    function _purgeOwnerCollection(address owner) private {
        NidhiCollection collection = _itemsByOwner[owner];
        NidhiCollection.Item memory item = collection.head();
        while (item.itemId != 0) {
            MarketItem storage marketItem = _idToMarketItem[item.itemId];
            address tokenOwner = IERC721(marketItem.nftContract).ownerOf(marketItem.tokenId);
            if (tokenOwner != owner) {
                collection.remove(marketItem.itemId);
            }
            item = collection.get(item.next);
        }
    }
    ...
    function _getCreator(
        ...
        try INidhiNFT(nftContract).creatorOf(tokenId) returns (address creator) {
            return creator;
        } catch {
            return address(0);
        }
    ...
    function _getMarketItemDetails(
        ...
        if (details.listed = item.listed) {
            ...
            details.payoutPercentage = (item.stakedAmount * 10000) / price;
            uint pt = _paymentTokens.length;
            if (pt > 0) {
                PriceForToken[] memory prices = new PriceForToken[](pt);
                for (uint i = 0; i < pt; i++) {
                    address tokenAddress = _paymentTokens[i];
                    uint[] memory amounts = router.getAmountsIn(price, _routerPaths[tokenAddress]);
                    prices[i] = PriceForToken(tokenAddress, amounts[0]);
                }
