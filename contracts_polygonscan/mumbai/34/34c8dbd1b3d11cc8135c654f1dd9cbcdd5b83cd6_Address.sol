...
contract BoringBananasCo is ERC721, Ownable {
    ...
    string public BANANA_PROVENANCE = ""; // OG IPFS URL WILL BE ADDED AFTER ALL ARE SOLD OUT
    ...
    uint256 public constant MAX_BANANAS = 12345;

    bool public saleIsActive = false;
    
    mapping(uint => string) public bananaNames; //LETS PPL CUSTOMIZE NAME OF NFT
    
    // Reserve 345 Bananas for team - Giveaways/Prizes etc
    uint public bananaReserve = 345;
    
    event bananaNameChange(address _by, uint _tokenId, string _name);  //logs in events when name changes take place
    
    //event licenseisLocked(string _licenseText);

    constructor() ERC721("Boring Bananas Co.", "BBC") { }
    
    function withdraw() public onlyOwner { //withdraws all the money in the contract
