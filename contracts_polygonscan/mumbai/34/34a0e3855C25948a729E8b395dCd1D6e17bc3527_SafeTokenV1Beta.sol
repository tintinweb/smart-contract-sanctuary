...
interface IERC721 {
    ...
    function getTrackDuration(uint256 trackId) external view returns(uint256 trackDuration);
    function getTrackRewards(uint256 trackId) external view returns(uint256 rewardsRate);
...
abstract contract Tokenomics {
    ...
    string internal constant NAME = "BeatsTokens";
    string internal constant SYMBOL = "Beats";
    ...
    ReentrancyGuard
{
    ...
    uint256 public lockInterval = 0; // by default around 90 days with 13.25s block time

    // from address to tokenId to value added
    mapping(address => mapping(uint256 => uint256)) internal token2value;
    mapping(address => mapping(uint256 => uint256)) internal summarizedNftTable;
    /** address => nft tokens count => beats amount */
    mapping(address => mapping(uint256 => uint256)) internal detailedNftTable;
    mapping(address => uint256) internal nftAddress2Count;
    ...
    mapping(address => bool) isCurrentlyVesting;
    ...
    //uint256 public vestingDurationInMinutes = 12 * oneMinute;
    ...
    modifier onlyIfBenificiaryHasNoCurrentVestingSchedule(VestingSchedule memory vestingSchedule) {
        require(getCurrentTime() > vestingSchedule.start + vestingSchedule.duration,
        'benificiary is already vesting now');
        _;
    }
    ...
    /** this function mints token in <<BEATS>> _nftToken contract!! ... */
    function mintNftCard (
        ...
        uint256 tokenId = _nftToken.mintCollectable(_owner, _tokenURI, _name, _price, _category, 0, 0, _isAdvesrtisement);
        /** add contract address, tokenId and amount in detailed table and summarized table */
        
        /** detailedNftTable */ 
        // nftAddress2Count[address(_nftToken)] = nftAddress2Count[address(_nftToken)] + 1;
        // uint256 count = nftAddress2Count[address];
        // detailedNftTable[address(_nftToken)][count] = ;

        /** summarizedNftTable */ 
        //summarizedNftTable[address(_nftToken)][tokenId] = summarizedNftTable[address(_nftToken)][tokenId] + _amount;
     ...
     */
    function addNftBeats(
        uint256 _tokenId,
        uint256 _amount
    ) public {
        address _msgSender = _msgSender();
        address _owner = owner();
        address nftTokenOwner = _nftToken.ownerOf(_tokenId);
        require(_msgSender == nftTokenOwner || _msgSender == _owner,
        "you are not authorized to add tokens here");
        ...
        token2value[nftTokenAddress][_tokenId]=token2value[nftTokenAddress][_tokenId] + _amount;
        _transfer(_msgSender, owner(), _amount);
    }

    /**
     * this function checks if beneficiary is already investing in a vesting schedule or not
     */
    function isBeneficiaryCurrentlyVesting(address _beneficiary) public view returns(bool){
        uint256 _VestingCount = holdersVestingCount[_beneficiary];
        if(_VestingCount == 0){
            return false;
        }
        else{
            bytes32 vestingScheduleId = this.computeCurrentVestingScheduleIdForHolder(_beneficiary);
            VestingSchedule storage vestingSchedule = vestingSchedules[
                vestingScheduleId
            ];

            uint256 _currentTime = getCurrentTime();
            if(_currentTime > vestingSchedule.start + vestingSchedule.duration){
                return false;
            }else{
                return true;
            }
    ...
    function _calculateVestableDuration(uint256 _tokenId, uint256 _trackId)internal view returns(uint256){
        address nftTokenAddress = address(_nftToken);
        uint256 trackDuration = _nftToken.getTrackDuration(_trackId);
        uint256 trackRewardsRate = _nftToken.getTrackRewards(_trackId);
        uint256 totalReward = trackDuration.mul(trackRewardsRate);
        uint256 availableBeatsInNftForVesting = token2value[nftTokenAddress][_tokenId];
        uint256 duration;
    ...
    function _calcualteVestableAmount(uint256 _tokenId, uint256 _trackId)internal view returns(uint256){
        address nftTokenAddress = address(_nftToken);
        uint256 trackDuration = _nftToken.getTrackDuration(_trackId);
        uint256 trackRewardsRate = _nftToken.getTrackRewards(_trackId);
        uint256 totalReward = trackDuration.mul(trackRewardsRate);
    ...
    function _createVestingSchedule(
        ...
        address nftTokenAddress = address(_nftToken);
        //token2value[nftTokenAddress][_tokenId];
        ...
        token2value[nftTokenAddress][_tokenId] = token2value[nftTokenAddress][_tokenId] - vestingAmount;
        emit ScheduleAdded(_beneficiary, vestingScheduleId, vestingAmount);
        
        return vestingScheduleId;
    }

    function getNftTokenBalance(uint256 _tokenId) public view returns(uint256){
        address nftTokenAddress = address(_nftToken);

        return token2value[nftTokenAddress][_tokenId];
    }

    /** get all Vesting schedules for a user */
    function getUserVestingSchedulesAndRewards(address _beneficiary) public view returns(VestingSchedule[] memory, uint256[] memory){
        uint256 vestingCount = this.getVestingSchedulesCountByBeneficiary(_beneficiary);
        VestingSchedule[] memory vestingSchedulesMeta = new VestingSchedule[](vestingCount);
        uint256[] memory ReleasableAmounts;
        uint256 counter = 0;

        for(uint i=0; i<vestingCount; i++){
            VestingSchedule memory vestingSchedule = this.getVestingScheduleByAddressAndIndex(_beneficiary, i);
            ReleasableAmounts[counter] = _computeReleasableAmount(vestingSchedule);
            vestingSchedulesMeta[counter] = vestingSchedule;
            
            counter ++;
        }
        return (vestingSchedulesMeta, ReleasableAmounts);
    }
    ...
    function createVestingSchedule(
        ...
        require(
            msg.sender == owner() || _nftToken.ownerOf(_tokenId) == _beneficiary,
            "You should own Beats NFT or you are the contract owner"
        );
        uint256 vestingDuration = _calculateVestableDuration(_tokenId, _trackId);
        uint256 vestingAmount   = _calcualteVestableAmount(_tokenId, _trackId);

        require(vestingAmount > 0 && vestingDuration > 0,
        "beneficiary doesn't have enough Beats");

        uint256 _trackRewardsRate = _nftToken.getTrackRewards(_trackId);
    ...
    function _updateTokenToValue(address beneficiary, uint256 tokenId, uint256 amount) internal {
        uint256 currentValue = token2value[beneficiary][tokenId];
        uint256 newValue = currentValue.sub(amount);

        token2value[beneficiary][tokenId] = newValue;
    }
        ...
        onlyIfVestingScheduleNotRevoked(vestingScheduleId)
    {
        ...
        _updateTokenToValue(vestingSchedule.beneficiary, vestingSchedule.tokenId, amount);
    ...
    function getNftListeningValue(address beneficiary, uint256 tokenId) public view returns(uint256){
        return token2value[beneficiary][tokenId];
    }
...
abstract contract SafeToken is BaseRfiToken, Liquifier, Antiwhale {
    ...
    constructor(Env _env, IERC721 _nftToken) BaseRfiToken(_nftToken) {
...
contract SafeTokenV1Beta is SafeToken {
    constructor(IERC721 _nftToken) SafeToken(Env.Testnet, _nftToken) {
