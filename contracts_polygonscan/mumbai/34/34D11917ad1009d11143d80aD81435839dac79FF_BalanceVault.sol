...
contract BalanceVault is Ownable, AccessControlEnumerable{
    ...
    */
    ...
    /**
    * @dev Modifier to only allow permitted role to call function.  
    */
    modifier onlyVaultAdmin() {
        require(hasRole(VAULT_ADMIN, msg.sender), "[BalanceVault.onlyVaultAdmin] Restricted to vault admins.");
        _;
    }
    ...
    */
    modifier whenBalanceVaultNotPaused() {
        require(!_paused, "[BalanceVault.whenBalanceVaultNotPaused] Not Paused");
        _;
    }
    ...
    */
    modifier whenBalanceVaultPaused() {
        require(_paused, "[BalanceVault.whenBalanceVaultPaused] Paused");
        _;
    }
     ...
     */
    function depositNaka(uint256 _nakaAmount) external whenBalanceVaultNotPaused{
     ...
     */
    function withdrawNaka(uint256 _nakaAmount) external whenBalanceVaultNotPaused{
        IERC20 token = IERC20(tokenAddress);
     ...
     */
    function increaseBalance(address _userAddress, uint256 _nakaAmount) external whenBalanceVaultNotPaused onlyVaultAdmin{
     ...
     */
    function decreaseBalance(address _userAddress, uint256 _nakaAmount) external whenBalanceVaultNotPaused onlyVaultAdmin{
        nakaAmountbyAddress[_userAddress] -= _nakaAmount;
        nakaAmountbyAddress[address(this)] -= _nakaAmount;
    ...
    */
    function pauseBalanceVault() external onlyRole(DEFAULT_ADMIN_ROLE) whenBalanceVaultNotPaused {
        _paused = true;
    ...
    * can only be called by the creator of contract.
    */
    function unpauseBalanceVault() external onlyRole(DEFAULT_ADMIN_ROLE) whenBalanceVaultPaused {
        _paused = false;
