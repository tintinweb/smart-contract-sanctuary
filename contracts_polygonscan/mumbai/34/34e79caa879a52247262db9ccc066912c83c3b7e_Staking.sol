...
contract Staking is Initializable, OwnableUpgradeable {
        ...
        returns (uint256 pending)
    {
        ...
        if (user.unbondedAt + unbondPeriod >= block.timestamp) {
            return 0;
        }
    ...
    function deposit(uint256 amount, address to) public {
        ...
        require(
            user.unbondedAt + unbondPeriod < block.timestamp,
     ...
     * @dev Updates accRewardPerShare and lastRewardTime.
     */
    function unbond(address to) external {
        UserInfo storage user = userInfo[msg.sender];
        require(user.unbondedAt == 0, "Unbond: Already unbonding");
        require(user.amount > 0, "Unbond: Nothing to unbond");
        ...
        user.rewardDebt =
            accumulatedReward -
            ((user.amount * accRewardPerShare) / ACC_REWARD_PRECISION).toInt256();
        user.unbondedAt = block.timestamp;

        totalUnbondingAmount = totalUnbondingAmount + user.amount;
