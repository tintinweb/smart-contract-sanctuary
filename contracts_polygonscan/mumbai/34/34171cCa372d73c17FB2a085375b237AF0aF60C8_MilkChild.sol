...
contract MilkChild is ERC20, IChildToken, HSystemChecker {
    ...
    /// @dev In the event we hit any transfer issues when creating the pool, we can whitelist addresses
    /// to allow transfers. We dont want to give TREASURY_ROLE out to anyone but our trusted contracts
    /// and admin addresses
    ...
    /// @notice Prevent transfers before they are unlocked
    /// @dev This is a measure to help combat unfavourable LP trades early on
    /// @param from Address is being move from
    /// @param to Address is being move to
    /// @param amount Amount of token
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal virtual override(ERC20) {
        // lock transfers for every address except treasury during the buffer period
        // we want to prevent bot spiking in first few minutes of LP opening
        if(transfersLocked){
            require(
                _systemChecker.hasRole(TREASURY_ROLE, _msgSender()) ||
                _systemChecker.hasRole(POOL_ROLE, _msgSender()),
                "MILK: transfers locked");
        }
        super._beforeTokenTransfer(from, to, amount);
    }

    /// @notice Method to unlock transfers, can not be locked again
    function unlockTransfers() external onlyRole(MASTER_ROLE) {
...
contract HSystemChecker is RolesAndKeys {
    ...
    function setSystemCheckerAddress(address systemCheckerContractAddress) external onlyRole(ADMIN_ROLE) {
        _systemCheckerContractAddress = systemCheckerContractAddress;
        _systemChecker = ISystemChecker(systemCheckerContractAddress);
    }
}
