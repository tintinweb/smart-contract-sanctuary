...
contract Knives is ERC721Enumerable, Ownable {  
    ...
    uint256 public constant mintAtOnce = 3;

    // Price of each token
    uint256 public price = 0.04 ether;

    // Maximum limit of packs that can ever exist
    uint256 constant MAX_SUPPLY = 30; // 1. yet to decide
    ...
    constructor (string memory newBaseURI) ERC721 ("Knives", "KNI") {
    ...
    function mintPresale(uint256 _amount) public payable {
        uint256 supply = totalSupply() / mintAtOnce; // 2. changed to accomodate minting 3 at once
        uint256 reservedAmt = presaleReserved[msg.sender] * mintAtOnce; // 3. changed to accomodate minting 3 at once
        ...
        for(uint256 i; i < _amount * mintAtOnce; i++){  // 4. changed to accomodate minting 3 at once
    ...
    function mintToken(uint256 _amount) public payable {
        uint256 supply = totalSupply() / mintAtOnce; // 5. changed to accomodate minting 3 at once
        require( saleActive,                     "Sale isn't active" );
        require( _amount > 0 && _amount < 11,    "Can only mint between 1 and 10 tokens at once" );
        require( supply + _amount <= MAX_SUPPLY, "Can't mint more than max supply" );
        require( msg.value == price * _amount,   "Wrong amount of ETH sent" );
        for(uint256 i; i < _amount * mintAtOnce; i++){ // 6. changed to accomodate minting 3 at once
            _safeMint( msg.sender, supply + i );
        }
    }
    ...
    function mintReserved(uint256 _amount) public onlyOwner {
        ...
        reserved -= _amount;
        uint256 supply = totalSupply() / mintAtOnce; // 7. changed to accomodate minting 3 at once
        for(uint256 i; i < _amount * mintAtOnce; i++){   // 8. changed to accomodate minting 3 at once
            _safeMint( msg.sender, supply + i );
        }
    }
    ...
    function withdraw() public payable onlyOwner {
        require(a1 != address(0), "account does not exist");
