...
contract SimplePriceOracle is PriceOracle, AdminRole {

    address maticUnderlying = address(0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE);
    ...
    function getUnderlyingPrice(CToken cToken) public view returns (uint) {
        if (compareStrings(cToken.symbol(), "cETH")) {
            return 1e18;
        } else if (compareStrings(cToken.symbol(), "cMATIC")) {
            return prices[maticUnderlying];
        }
        else {
    ...
    function setUnderlyingPrice(CToken cToken, uint underlyingPriceMantissa) public onlyAdmin{
        if (compareStrings(cToken.symbol(), "cMATIC")) {
            emit PricePosted(maticUnderlying, prices[maticUnderlying], underlyingPriceMantissa, underlyingPriceMantissa);
            prices[maticUnderlying] = underlyingPriceMantissa;
        }
        else {
