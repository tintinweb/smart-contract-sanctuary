...
contract Marketplace is Ownable, Pausable, MarketplaceStorage {
    ...
    function _createOrder(
        ...
        assetIdsWithPlacedOrders[nftAddress].add(assetId);
        ...
        returns (Order memory)
    {
        ...
        assetIdsWithPlacedOrders[nftAddress].remove(assetId);
    ...
    function _executeOrder(
        ...
        if (erc721Token.getMarketplaceFee() > 0) {
            ...
            saleShareAmount = price.mul(erc721Token.getMarketplaceFee()).div(
                1000000
            );
...
contract MarketplaceStorage {
    ...
    // Mapping from nft address to its (enumerable) set of asset ids with open order
    mapping(address => EnumerableSet.UintSet) internal assetIdsWithPlacedOrders;
    ...
    function ordersLength(address nftAddress)
        public
        view
        virtual
        returns (uint256)
    {
        return assetIdsWithPlacedOrders[nftAddress].length();
    }

    function assetIdByIndex(address nftAddress, uint256 index)
        public
        view
        virtual
        returns (uint256)
    {
        return assetIdsWithPlacedOrders[nftAddress].at(index);
    }

    function orderByIndex(address nftAddress, uint256 index)
        public
        view
        virtual
        returns (
    ...
    {
        Order memory currOrder = orderByAssetId[nftAddress][
            assetIdsWithPlacedOrders[nftAddress].at(index)
        ];
        return (
            currOrder.id,
            currOrder.seller,
            currOrder.nftAddress,
            currOrder.price,
            currOrder.expiresAt
        );
    }
