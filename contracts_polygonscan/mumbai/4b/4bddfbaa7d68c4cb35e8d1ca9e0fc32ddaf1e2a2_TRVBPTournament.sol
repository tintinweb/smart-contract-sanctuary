...
contract TRVBPTournament is Ownable, ReentrancyGuard {
    ...
    event SetWinners(uint256 indexed tournamentId, uint16 indexed firstWinner, uint16 indexed secondWinner);
    event SetSize(uint256 indexed start, uint256 indexed end, uint256 size);
    event SetFee(uint256 indexed start, uint256 indexed end, uint256 fee);
    ...
    function cancelTournament(uint256 _tournamentId) external nonReentrant {
        require(msg.sender == owner() || hasRole(CANCEL_ROLE, msg.sender), "Caller does not have permission.");
    ...
    // verified
    function setWinner(uint256 _tournamentId, uint16 _firstBest, uint16 _secondBest) external nonReentrant {
        ...
        tournaments[_tournamentId].first_winner = _firstBest;
        tournaments[_tournamentId].second_winner = _secondBest;
        ...
        // emit event
        emit SetWinners(_tournamentId, _firstBest, _secondBest);
    }
    ...
    function setTournamentSize(uint256 _start, uint256 _end, uint256 _size) external onlyOwner {
        ...
        emit SetSize(_start, _end, _size);
    }
    ...
    function withdrawFees() external onlyOwner { // withdraw WETH
        IWETH(WETH_ADDRESS).transfer(TREASURY_ADDRESS, platformShare);
    }
