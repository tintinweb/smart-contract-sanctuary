...
contract Auction721 is Ownable {
  ...
  address payable public immutable owner_wallet;
  uint256 public immutable auction_start;
  uint256 public auction_end;
  uint256 public immutable extend_time = 600;
  uint256 public immutable price_reserve;
  ...
  constructor(
    ...
    require(_reserve > 0, "Construct: Reserve must be greater than 0");
    require(_increment > 0, "Construct: Increment must be greater than 0");
  ...
  function bid() external payable isActive {
    require(msg.value >= price_reserve, "Bid must be at least reserve");
  ...
  function registerBidToHistory(address _sender, uint256 _value) internal {
    ...
    emit Bid(_sender, _value, thisBid.time);
  }
  ...
  function processAuction() external {
    ...
    require(
      latestBid.sender != address(0),
      "Auction winner cannot be address 0"
    );

    uint256 _value = address(this).balance;
    uint256 ownerPart = _value;
    payable(owner_wallet).transfer(ownerPart);

    emit AuctionWinningAmountPaid(latestBid.sender, latestBid.value);
  }
  ...
  {
    BidStruct storage thisBid = bid_history[bid_count];
    return (thisBid.sender, thisBid.value, thisBid.time);
  }
