...
contract SynSideV11LP {
  ...
  {
    return userInfo[_user].transactions[_fromChainId][_chainId].length;
  }
  ...
  modifier permittedToChainIds(IERC20 _token, uint256[] calldata _supportedToChainId) {
    for (uint256 i = 0; i < _supportedToChainId.length; i++) {
      require(
        liquidityInfo[_token].toChainToken[_supportedToChainId[i]] != IERC20(address(0)), 
        "ToChainIds Not Permitted"
      );
    }
    _;
  }
  // modifier permittedTxIndex(address _user, uint256 _fromChainId, uint256 _toChainId, uint256 _txIndex) {
  //   require(
  //     userInfo[_user].transactions[_fromChainId][_toChainId].length == _txIndex,
  //      "Transactions index out of order"
  //   );
  ...
  modifier liquiditySupportable(IERC20 _token) {
    require(liquidityInfo[_token].supportable, "ToChainIds Not Permitted");
    _;
  }
