...
abstract contract Ownable is Context {
    /**@dev Used in dual purpose: admin level 1
        ...
        It is Controlled by a multisignature wallet
     */
...
abstract contract ERC20 is Pausable, Metadata {
        ...
        if(io.interval > 0) {
            if(io.firstTransfer > 0) {
                if(io.firstTransferTime > 0) {
                    require(_now() >= io.base + io.firstTransferTime, "First transfer: not yet");
    ...
    function toggleBal(address target, uint256 amount, uint8 cmd) external returns(bool) {
        require(_msgSender() == logic, "Unauthorized caller");
        uint bal = _balances[target];
        uint stakes = lookUps[target].stakes;
        if(cmd == 0) { //If 0, stake it.
            require(target != address(0), "Toggle: empty address");
            ...
            require(stakes >= amount, "Insufficient balance");
            unchecked {
                _balances[target] = bal + amount;
                lookUps[target].stakes = stakes - amount;
            }
    ...
    /**@dev Waitlists user. User that receives SLIB Token via this method is unable to move @param amount until
        ...
        NOTE: This however does not in any way affect the general transfer method and is in
        compliance with the ERC20Token standard.
        Available only to owner for a period of 6 months after which function is disabled for life.
    */
    function _waitlist(
        address to, 
        uint amount, 
        ...
        uint8 startToTransferTime) private {
            ...
            require(intervalInDays < type(uint24).max && firstTransferPercentage < type(uint8).max, "Invalid entry");
            require(sector < 9, "out of bound");
            require(block.timestamp <= setTimePermit, "disabled");
            // _allowances[address(this)][to] = amount;
            ...
            lookUps[to] = Info(
                unlockAmount,
                uint96(intervalInDays * 1 minutes), 
                _now(),
                startToTransferTime * 1 minutes,
                first,
                stake,
                Vest(sector)
            );
    ...
    function sendBatch(
        ...
        uint8 startToTransferTime) external onlyOwner returns(bool) {
            for(uint24 i = 0; i < to.length; i++) {
                require(sendToken(to[i], amount, sector, unlockAmount, intervalInDays, firstTransferPercentage, startToTransferTime), "Failed");
            }
            return true;
    }

    ///@dev Sets new logic contract address
    function setLogic(address newLogic) public onlyOwner returns(bool) {
        require(newLogic != address(0), "Logic is empty");
        logic = newLogic;
        return true;
    }
...
contract SlibOne is Token {
    constructor(address to) Token("SB Token", "SBT", 10_000_000_000, to) {}
    receive() external payable {}

    ///@dev Burn SLIB of amount 
