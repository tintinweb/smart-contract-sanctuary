...
abstract contract WithLimitedSupply {
    ...
    function availableTokenCount() internal view returns (uint256) {
        return _totalSupply - tokenCount();
    }
...
abstract contract RandomlyAssigned is WithLimitedSupply {
    ...
    function nextToken() internal override ensureAvailability returns (uint256) {
        uint256 maxIndex = _totalSupply - tokenCount();
...
contract NFT is ERC721Enumerable, Ownable, RandomlyAssigned {
  using Strings for uint256;
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    //owner can mint up to 200 NFTs per mint
    else {
        ownerMint = ownerMint + 205;
        
        // require(_mintAmount <= ownerMint);
        require(supply + _mintAmount <= maxSupply, "max NFT limit exceeded");
    }
