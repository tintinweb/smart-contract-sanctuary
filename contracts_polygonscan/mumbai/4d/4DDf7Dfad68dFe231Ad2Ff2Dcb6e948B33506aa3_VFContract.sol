...
// File: contracts/child/ChildToken/VFContract.sol
...
library EnumerableSet {
    ...
    function add(AddressSet storage set, address value)
        ...
        returns (bool)
    {
        uint256 _addr = uint256(uint160(address(value)));
        return _add(set._inner, bytes32(_addr));
    }
    ...
    {
        uint256 _addr = uint256(uint160(address(value)));
        return _remove(set._inner, bytes32(_addr));
    }
    ...
    {
        uint256 _addr = uint256(uint160(address(value)));
        return _contains(set._inner, bytes32(_addr));
    }
...
library EnumerableMap {
    ...
    function set(
        ...
        uint256 _addr = uint256(uint160(address(value)));
        return _set(map._inner, bytes32(key), bytes32(_addr));
    }
...
library Counters {
    ...
    //https://docs.soliditylang.org/en/v0.8.6/control-structures.html?#checked-or-unchecked-arithmetic
    ...
    ContextMixin
{
    ...
    // "devmode" turn off validation for relay transactions
    bool private _devMode = false; // dev mode testing

    uint256 private _tokenId; //last token minted

    //bool private _isNewAsset; //new asset? on mint...
    ...
    function updateDevMode(
        ...
        require(
            ...
            "VFContract: Not Authorized to Update DevMode"
        );
        // require(
        //     hasRole(_roles[role].adminRole, _msgSender()),
        //     "VFContract: No AdminRole to Update DevMode"
        // );
        _devMode = _mode;
    }
    ...
    /**
     * @dev asset state at mint time
     */
    struct AssetStateOnMint {
        bool state;
    }

    Asset[] private assets;
    AssetStateOnMint[] private assetStateOnMints;
    Percentage[] private percentages;

    //map asset state on mint to tokenId
    mapping(uint256 => uint256) public assetMintStateToToken;
    ...
    address private _owner; //for itx extra relay transaction owner address, previous contract
    ...
    /**
     * @dev return asset from token id
     */
    ...
    /**
     * @dev return asset from struct id
     */
    ...
    /**
     * @dev returns asset currentValue from token id
     */
    ...
    /**
     * @dev update asset currentValue on appraisal/update
     */
    function updateAssetCurrentValue(
        ...
        require(
            _isOwner(message, nonce, signature),
            "VFContract: Not Authorized to Update Asset Value"
        );
    ...
    function updateAssetName(
        ...
        require(
            _isOwner(message, nonce, signature),
            "VFContract: Not Authorized to Update Asset Name"
        );
        Asset storage _asset = assets[index];
        _asset.name = _name;
        assets[index] = _asset;
     ...
     * @dev private add new asset uri to mapping set on mint
     */
    function _setAssetStateOnMint(uint256 tokenId, bool _state) internal {
        assetStateOnMints.push(AssetStateOnMint(_state));
        uint256 id = assetStateOnMints.length - 1;
        assetMintStateToToken[tokenId] = id;
    }
    ...
    function _isOwner(
        ...
        require(!usedNonces[_owner][nonce], "VFContract: _isOwner Bad Nonce");
     ...
     *
     * ~ plan is to mint here use DEFAULT_ADMIN_ROLE
     */
    function mint(
        address to,
        ...
        require(
            _isOwner(message, nonce, signature),
            "VFContract: Not Authorized to Mint"
        );
        _tokenId = _tokenIdTracker.current();
        require(
            //!withdrawnTokens[tokenId],
            !withdrawnTokens[_tokenId],
            "VFContract: TOKEN_EXISTS_ON_ROOT_CHAIN"
        );
        _setAssetStateOnMint(_tokenId, newAsset);
        _mint(to, _tokenId);

        if (newAsset) {
            _setAssetOnMint(_tokenId, _assetName, _currentValue, _percentage);
        } else {
            require(
                _setExistingAssetOnMint(_tokenId, assetIndex, _percentage),
                "VFContract: Asset % Exceeds Availablity"
            );
            Asset storage _asset = assets[assetIndex];
            _currentValue = _asset.currentValue;
        }
        _setPercentageOnMint(_tokenId, _percentage);
    ...
    function withdraw(uint256 tokenId) external {
        require(
            ...
            "VFContract: INVALID_TOKEN_OWNER"
        );
    ...
    function withdrawBatch(uint256[] calldata tokenIds) external {
        uint256 length = tokenIds.length;
        require(length <= BATCH_LIMIT, "VFContract: EXCEEDS_BATCH_LIMIT");
     ...
     * be decoded back by them correctly during exiting
     *
     * @param tokenId Token to get asset state on mint to reconstruct on exit
     */
    function encodeNewAsset(uint256 tokenId)
        external
        view
        virtual
        returns (bytes memory)
    {
        ...
        // and pack more data in byte array which can be decoded back
        // in L1
        uint256 _id = assetMintStateToToken[tokenId];
        AssetStateOnMint memory asom = assetStateOnMints[_id];
        return abi.encode(asom.state);
    }
     ...
     * be decoded back by them correctly during exiting
     *
     * @param tokenId Token to get asset index with name and reconstruct on exit
     */
    function encodeAssetName(uint256 tokenId)
        external
        view
        virtual
        returns (bytes memory)
     ...
     * be decoded back by them correctly during exiting
     *
     * @param tokenId Token to get asset index with current value and reconstruct on exit
     */
    function encodeCurrentValue(uint256 tokenId)
        external
        view
        virtual
        returns (bytes memory)
    {
        ...
        uint256 _id = assetToToken[tokenId];
        Asset memory p = assets[_id];
        return abi.encode(p.currentValue);
    }
     ...
     * be decoded back by them correctly during exiting
     *
     * @param tokenId Token to get asset index with current used value and reconstruct on exit
     */
    function encodeAssetIndex(uint256 tokenId)
        external
        view
        virtual
        returns (bytes memory)
     ...
     * be decoded back by them correctly during exiting
     *
     * @param tokenId Token to get percentage index and reconstruct on exit
     */
    function encodePercentageIndex(uint256 tokenId)
        external
        view
        virtual
        returns (bytes memory)
    {
        ...
        return abi.encode(_percentageId);
    }
     ...
     * be decoded back by them correctly during exiting
     *
     * @param tokenId Token to get percentage value and reconstruct on exit
     */
    function encodePercentage(uint256 tokenId)
        external
        view
        virtual
        returns (bytes memory)
