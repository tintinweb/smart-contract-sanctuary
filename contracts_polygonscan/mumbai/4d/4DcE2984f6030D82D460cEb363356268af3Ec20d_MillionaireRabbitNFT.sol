...
interface IERC20 {
    /**
     * @dev Returns the amount of tokens in existence.lkkkkk
     */
...
contract MillionaireRabbitNFT is ERC721URIStorage, VRFConsumerBase, Ownable {
    ...
    mapping(bytes32 => NewBunnyRequest) public requestToNewChampionRequest;
    ...
    function requestNewRandomChampion(string memory name) public returns (bytes32) {
        ...
        requestToNewChampionRequest[requestId] = NewBunnyRequest(name, msg.sender);
    ...
    function getLevel(uint256 tokenId) public view returns (uint256) {
        return sqrt(millionairerabbits[tokenId].magicrabbitjedipower);
    }
    ...
    {
        return (millionairerabbits[tokenId].attack, millionairerabbits[tokenId].defense, millionairerabbits[tokenId].magicrabbitjedipower);
    }
    ...
    function characterCreationFulfillment(bytes32 requestId, uint256 randomNumber) internal {
        ...
        uint256 magicrabbitjedipower = 0;
        NewBunnyRequest memory champRequest = requestToNewChampionRequest[requestId];
        MillionaireRabbit memory character = MillionaireRabbit(attack, defense, magicrabbitjedipower, champRequest.championName);
    ...
    function battleFulfillment(bytes32 requestId, uint256 randomness) internal {
        ...
        if (winningNumber >= attack) {
            millionairerabbits[battleRequest.rabbitHolderOne].magicrabbitjedipower += 2;
            if (millionairerabbits[battleRequest.rabbitHolderTwo].magicrabbitjedipower >= 10) {
                millionairerabbits[battleRequest.rabbitHolderTwo].magicrabbitjedipower -= 1;
            }
            emit battleDone(battleRequest.rabbitHolderOne);
        } else {
            millionairerabbits[battleRequest.rabbitHolderTwo].magicrabbitjedipower += 2;
            if (millionairerabbits[battleRequest.rabbitHolderOne].magicrabbitjedipower >= 10) {
                millionairerabbits[battleRequest.rabbitHolderOne].magicrabbitjedipower -= 1;
            }
    ...
    function boostAttackWithJediPower(uint256 tokenId, uint256 amount) public {
        require(_isApprovedOrOwner(msg.sender, tokenId), "You don't own this Bunny!");
        require(millionairerabbits[tokenId].magicrabbitjedipower >= amount, "Not enough magicrabbitjedipower");
        millionairerabbits[tokenId].attack += amount;
        millionairerabbits[tokenId].magicrabbitjedipower -= amount;
    }

    //once you have some magic rabbit jedi power you can trade some to up your defense power

    function boostDefenceWithJediPower(uint256 tokenId, uint256 amount) public {
        require(_isApprovedOrOwner(msg.sender, tokenId), "You don't own this Bunny!");
        require(millionairerabbits[tokenId].magicrabbitjedipower >= amount, "Not enough magicrabbitjedipower");
