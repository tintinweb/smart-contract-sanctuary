...
contract NFTCollection is ERC1155, Ownable, Pausable, ERC1155Supply {
    ...
    constructor() ERC1155("https://ipfs.infura.io/ipfs/") { //base URI
    ...
    function uri(uint256 id) public view virtual override returns (string memory) {
        return string( abi.encodePacked( super.uri(id), lookupmap[id], "?clear" ));
    }
...
contract NFTMarketplace is ERC1155Holder,Ownable{
  ...
  constructor(address _nftCollection, address _eth) {
  ...
  function setAddressWeth(address newAdressWeth) public onlyOwner{
    ...
    WETH = IERC20(address(_addressWETH));
  ...
  function fillOffer(uint _offerId, uint amount) public payable {
    ...
    // uint WETHpriceTotal = _offer.price * amount;
    // require(WETHbalance >= WETHpriceTotal, 'The WETH amount should match with the NFT Price');
    // WETH.transferFrom(msg.sender, address(this), WETHpriceTotal);
