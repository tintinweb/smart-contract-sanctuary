...
contract HarvesterFactory is Ownable, ReentrancyGuard, IFactory {
   ...
   // 合约部署者增加操作员
     ...
     *
     */
    function geneMetaData(address owner, uint256 _id, string memory _imageUrl) external override returns (string memory){
        ...
        _itemIds.increment();
        uint256 currItemId = _itemIds.current();
        itemIdOwners[currItemId] = owner;
        ownerCount[owner] ++;
        harvesterHolders[currItemId] = _id;
        ...
        string memory json = string(abi.encodePacked('{"name":', nftNameHarvester, '","description":"Harvester for meta globe game", "attributes":[{"trait_type":"Energy Efficiency","value":', toString(energy_efficiency),'}, {"trait_type":"Uru Efficiency","value":',toString(uru_efficiency),'}, {"trait_type":"Max Durability","value":', toString(max_durability),'}],'));
        json = string(abi.encodePacked(json, '"image":"',_imageUrl,'","external_url":"https://api.mglobe.net/token/', toString(_id),'"}'));
        string memory output = string(abi.encodePacked('data:application/json;base64,', json));
        emit OnGeneMetaData(_id, owner, energy_efficiency, uru_efficiency, max_durability, output);
     ...
     *
     */
    function upgradeMetaData(address owner, uint256 oldId, uint256 newId, string memory _imageUrl) external override returns (string memory) {
        //msg.sender是erc1155合约
        require(operators[msg.sender], "Sender is not an operator.");
        ...
        //将旧的编号各种属性设置为0
        characters[oldId] = Character(
                0,
                0,
                0,
                characters[oldId].itemId
        );

        // ownerCount[owner] ++; //不需要增加
        harvesterHolders[currItemId] = newId;
        ...
        string memory json = string(abi.encodePacked('{"name":', nftNameHarvester, '","description":"Harvester for meta globe game", "attributes":[{"trait_type":"Energy Efficiency","value":', toString(energy_efficiency),'}, {"trait_type":"Uru Efficiency","value":',toString(uru_efficiency),'}, {"trait_type":"Max Durability","value":', toString(max_durability),'}],'));
        json = string(abi.encodePacked(json, '"image":"',_imageUrl,'","external_url":"https://api.mglobe.net/token/', toString(newId),'"}'));
        string memory output = string(abi.encodePacked('data:application/json;base64,', json));

        emit OnUpgradeHarvester(owner, oldId, newId, energy_efficiency, uru_efficiency, max_durability, output);
    ...
    //操作员设置采集器升级许可及升级的数值
    function setAllowUpgrades(address owner, uint tokenId, uint256 energy_efficiency, uint256 uru_efficiency, uint256 max_durability)       
            public
            nonReentrant
            isOperator
            returns (bool)
    {
        ...
        allowUpgrades[hash].energy_efficiency = energy_efficiency;
        allowUpgrades[hash].uru_efficiency = uru_efficiency;
        allowUpgrades[hash].max_durability = max_durability;
        allowUpgrades[hash].isAllow = true;
        emit OnAllowUpgrade(owner, tokenId, energy_efficiency, uru_efficiency, max_durability);
        return true;
    }
