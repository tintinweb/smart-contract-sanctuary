...
interface IGearSlotsData {
    function getWeapons() external view returns (string[] memory);
    function getShields() external view returns (string[] memory);
    ...
    function getHandArmor() external view returns (string[] memory);
    ...
    function getItemData() external view returns (string[][] memory);
}
...
contract GearSlots is ERC721Enumerable, Ownable, VRFConsumerBase {
    ...
    address private _gearSlotsDataAddress;
    ...
    function setSlotMapping(uint256 tokenId) private {
        ...
        IGearSlotsData _gearSlotsData = IGearSlotsData(_gearSlotsDataAddress);

        _bags[tokenId][0] = SlotItem(pluck(tokenId, "WEAPON", _gearSlotsData.getWeapons()), 0);

        _bags[tokenId][1] = SlotItem(pluck(tokenId, "CHEST", _gearSlotsData.getChestArmor()), 0);

        _bags[tokenId][2] = SlotItem(pluck(tokenId, "HEAD", _gearSlotsData.getHandArmor()), 0);

        _bags[tokenId][3] = SlotItem(pluck(tokenId, "WAIST", _gearSlotsData.getWaistArmor()), 0);

        _bags[tokenId][4] = SlotItem(pluck(tokenId, "FOOT", _gearSlotsData.getFootArmor()), 0);

        _bags[tokenId][5] = SlotItem(pluck(tokenId, "HAND", _gearSlotsData.getHandArmor()), 0);

        _bags[tokenId][6] = SlotItem(pluck(tokenId, "NECK", _gearSlotsData.getNecklaces()), 0);

        _bags[tokenId][7] = SlotItem(pluck(tokenId, "RING", _gearSlotsData.getRings()), 0);

        _bags[tokenId][8] = SlotItem(pluck(tokenId, "SHIELD", _gearSlotsData.getShields()), 0);
    ...
    function mint(address _to, uint256 _mintAmount) public payable {
        ...
        require(_mintAmount > 0, "ERC721: Mint amount not met.");
        require(_mintAmount <= maxMintAmount, "ERC721: Exceeded max mint amount");
        require(msg.value >= cost * _mintAmount, "ERC721: Value not met.");
        require(supply + _mintAmount <= maxSupply, "ERC721: Mint amount exceeds max supply");
        require(addressBalance < _limitPerAddress, "Max tokens for address");
        require(_paused == false, "Minting is paused");

        if (addressBalance + _mintAmount > _limitPerAddress){
            _mintAmount = _limitPerAddress - addressBalance;
        }
        ...
        for (uint256 i = 0; i < _mintAmount; i++) {
            ...
            _bagRandomValuesMapping[tokenId] = uint256(keccak256(abi.encode(_currentRandom, tokenId)));
    ...
    function setGearSlotsDataAddress(address _t) public onlyOwner {
        _gearSlotsDataAddress = _t;
    }
    ...
    function togglePaused() public payable onlyOwner {
        _paused = !_paused;
    }

    function setLimitPerAddress(uint256 limitPerAddress) public payable onlyOwner {
    ...
    function pluck(
        ...
        string[] memory suffixes = _gearSlotsData.getSuffixes();
        string[] memory namePrefixes = _gearSlotsData.getNamePrefixes();
        string[] memory nameSuffixes = _gearSlotsData.getNameSuffixes();
