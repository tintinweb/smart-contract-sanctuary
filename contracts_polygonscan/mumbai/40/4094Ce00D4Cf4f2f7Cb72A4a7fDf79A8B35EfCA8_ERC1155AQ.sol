  ...
  ReentrancyGuard
{
  ...
  function nonce(address _address) external view returns (uint256) {
    return _nonce[_address];
  }
  ...
  function safeTransferFromWithPaymentSignature(
    ...
    bytes32 p = keccak256(abi.encode(_message));

    require(
      paymentFulfilled[p] != true,
      "ERC1155AQ: payment has been fulfilled"
    );

    uint256 nonce = _nonce[_to];

    {
      bytes32 h = keccak256(abi.encode(_message, nonce));
      bytes32 m = keccak256(
    ...
    // require(
    //   _verifySignature(_message, _sigV, _sigR, _sigS, _nonce[_to], _to),
    //   "ERC1155AQ: signature cannnot be verified"
    // );
    safeTransferFrom(_from, _to, _tokenId, _amount, "");
    TransferSingleWithPrice(_from, _to, _tokenId, _amount, _price, _currency);
    paymentFulfilled[p] = true;
    _nonce[_to] = nonce.add(1);
  }
  ...
  function mintWithSignature(
    ...
    bytes32 h = keccak256((abi.encode(_uri, _nonce[_account])));
    bytes32 m = keccak256(
      abi.encodePacked("\x19Ethereum Signed Message:\n32", h)
    );
    address addressFromSig = ecrecover(m, _sigV, _sigR, _sigS);
    ...
    _nonce[_account] = _nonce[_account].add(1);
  }
}
