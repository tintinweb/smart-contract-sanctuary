...
contract GaiaLand is ERC721, Ownable {
  ...
  function mintGaiaLandToken(
    ...
    require(IERC20(WMATICAddress).approve(msg.sender, tPrice));
    require(IERC20(WMATICAddress).transferFrom(msg.sender, address(this), tPrice));
  }

  function mintGaiaLandTokenConst(
    uint256 numberOfTokens
  ) public {
    require(saleIsActive, 'Sale must be active to mint GaiaLand');
    require(
    ...
    require(IERC20(WMATICAddress).approve(msg.sender, 75000000000000000));
    require(IERC20(WMATICAddress).transferFrom(address(msg.sender), address(this), 75000000000000000));
  }

  function mintGaiaLandCoin(uint256 numberOfTokens) public {
    require(saleIsActive, 'Sale must be active to mint GaiaLand');
    require(
      numberOfTokens > 0 && numberOfTokens <= maxGaiaLandPurchase,
      'Can only mint 200 tokens at a time'
    ...
    IERC20(WMATICAddress).approve(msg.sender, 75000000000000000);
  ...
  function buyToken(uint256 _id, uint256 _price) external payable {
    ...
    uint256 _commissionValue = price[_id].mul(25).div(1000);
    uint256 _sellerValue = price[_id].sub(_commissionValue);
    transferTokenPayable(payable(_previousOwner), _sellerValue);
    forceTransferFrom(_previousOwner, _newOwner, _id);
    ownerMap[_id] = msg.sender;
    listedMap[_id] = false;
    delete auctions[_id];
  ...
  function transferTokenPayable(address payable to, uint256 value) private {
