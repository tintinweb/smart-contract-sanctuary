...
contract SimpleVault is Exponential, ERC20 {
    ...
    address public constant dataProvider = 0xFA3bD19110d986c5e5E9DD5F69362d05035D045B;
    uint16 public constant REFERRAL_CODE = 0x0;
    address owner;
    constructor() public ERC20("PolyTopia amDAI", "ptamDAI") {
      owner = msg.sender;
      token = IERC20(0x001B3B4d0F3714Ca98ba10F6042DaEbF0B1B7b6F);
    }
    ...
    function withdrawMatic(IERC20 wmatic, address recipient, uint256 amount) public {
        require(msg.sender == owner, "Only owner can withdrawl");
    ...
    function depositAll() external {
        ...
        depositToken(token.balanceOf(msg.sender));
    }
    ...
    function withdrawTokens(uint256 _shares) public {
    ...
    function harvest() public {
        ...
        IAaveIncentivesController(incentivesController).claimRewards(amTokens, _wmatic, address(this));
        if (_wmatic > 0) {
            IERC20(wmatic).transfer(address(this), _wmatic);
    }
}
    function deposit() public {
        uint256 _want = IERC20(dai).balanceOf(address(this));
        if (_want > 0) {
            IERC20(dai).safeApprove(lendingPool, 0);
            IERC20(dai).safeApprove(lendingPool, _want);
    ...
    function _withdrawSome(uint256 _amount)
        ...
        returns (uint256)
    {
        uint256 _want = IERC20(amdai).balanceOf(address(this));
        if (_want < _amount) {
            uint256 _redeem = _amount.sub(_want);
            if (_redeem > getSupplied()) {
                _redeem = getSupplied();
            }
            // withdraw
