...
contract DanceOff is VRFConsumerBase, Ownable{
    ...
    mapping(uint256 => uint256) royaleOpen;
        ...
        VRFConsumerBase(0x8C7382F9D8f56b33781fE506E897a4F1e2d17255, 0x326C977E6efc84E512bB9C30f76E30c160eD06FB)
    {
        ...
        royaleOpen[_rumbleId.current()] = block.timestamp;
    }
    ...
    function getTimeTrigger() public view returns (uint256){
        return royaleOpen[_rumbleId.current()];
    }
    ...
    function getLeaderboards(uint256 limit) public view returns (Leaderboards memory){
        Leaderboards memory leaders;
        uint256 index = 0;
        if(firstPlacements.length > limit){
            for(uint256 i=(firstPlacements.length-limit); i<firstPlacements.length; i++){
                leaders.firstP[index] = firstPlacements[i];
                leaders.secondP[index] = secondPlacements[i];
                leaders.thirdP[index] = thirdPlacements[i];
                index++;
            }
        }
        else{
        ...
        if(
            ...
            && (
                ...
                || (royaleParticipants[rumbleId] >= minimumSize 
                    && (block.timestamp - royaleOpen[rumbleId]) >= maxTime
                    )
                )
            )
        {
