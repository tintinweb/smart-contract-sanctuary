  ...
  IERC721EnumerableUpgradeable
{
   ...
   */
  function __ERC721_init(string memory name_, string memory symbol_, uint256 supply_) internal initializer {
    __Context_init_unchained();
    __ERC165_init_unchained();
...
abstract contract NFT721Metadata is NFT721Creator {
   ...
   * `QmfPsfGwLhiJrU8t9HpG4wuyjgPo9bk8go4aQqSu9Qg4h7/metadata.json`
   */
  function _updateTokenIPFSPath(uint256 tokenId, string memory _tokenIPFSPath) internal {
    // 46 is the minimum length for an IPFS content hash, it may be longer if paths are used
    require(bytes(_tokenIPFSPath).length >= 46, "NFT721Metadata: Invalid IPFS path");
    require(!creatorToIPFSHashToMinted[msg.sender][_tokenIPFSPath], "NFT721Metadata: NFT was already minted");
    
    creatorToIPFSHashToMinted[msg.sender][getTokenIPFSPath(tokenId)] = false;
    creatorToIPFSHashToMinted[msg.sender][_tokenIPFSPath] = true;
    _setTokenURI(tokenId, _tokenIPFSPath);
  }
 ...
 */
abstract contract NFT721Mint is Initializable, ERC721Upgradeable, NFT721Creator, NFT721Metadata, FoundationAdminRole {
//   using AddressLibrary for address;
   ...
   */
  function addWhitelist(address[] memory brands) public onlyFoundationAdmin {
      for(uint i; i < brands.length; i++) {
          whitelisted[brands[i]] = true;
      }
  }
  
  /**
   * @notice Allows foundation admin to de-whitelist users
   */
  function removeWhitelist(address[] memory brands) public onlyFoundationAdmin {
      for(uint i; i < brands.length; i++) {
          whitelisted[brands[i]] = false;
      }
  }
   ...
   */
  function mint(string memory tokenIPFSPath) public onlyWhitelistedUsers returns (uint256 tokenId) {
    tokenId = nextTokenId++;
   ...
   */
  function mintAndApproveMarket(string memory tokenIPFSPath) public onlyWhitelistedUsers returns (uint256 tokenId) {
   ...
   */
  function updateTokenURI(uint256 tokenId, string memory tokenIPFSPath) public {
    address owner = ownerOf(tokenId);
  ...
  NFT721Mint
{
  ...
  address private blingMaster;
    
   constructor() {
       blingMaster=msg.sender;
   }
...
// File: contracts/blingMaster.sol
...
contract Bling_Master {
    ...
    event CollectionCreated(address creator, string ColCode, string Colname, string ColDescription, string[] ColProperties, address myContract, uint256 quantity);
    
    event CollectionUpdated(address creator, string ColCode, string Colname, string ColDescription, string[] ColProperties, address myContract);
    
    
    function createCollection(string memory _colCode, string memory _colName, string memory _colDescription, uint256 _colQuantity, string[] memory _colProperties) 
        external returns(address collection) {
        
        // Add require condition to check 
        require(getCollection[msg.sender][_colCode] == address(0), 'Bling_Master: COLLECTION_EXISTS'); // single check is sufficient
        ...
        emit CollectionCreated(msg.sender, _colCode, _colName, _colDescription, _colProperties, collection,_colQuantity);
    }
    //Add quantity
    function updateCollection(address _colContract, string memory _colCode, string memory _colName, string memory _colDescription, uint256 _colQuantity, string[] memory _colProperties) 
        external {
            
        collectionInfo storage collection  = collections[msg.sender][_colCode];
        
        require(getCollection[msg.sender][_colCode] == _colContract, 'Bling_Master: COLLECTION_NOT_EXISTS');
        // Add require condition to check 
        require(Bling_Collection(_colContract).totalSupply() == 0, 'Bling_Master: UPDATE_NOT_ALLOWED'); // single check is sufficient
        
        
        collection.name =  _colName;
        collection.quantity= _colQuantity;
        collection.description= _colDescription;
        collection.properties= _colProperties;

        
        emit CollectionUpdated(msg.sender, _colCode, _colName, _colDescription, _colProperties, _colContract);
    }
