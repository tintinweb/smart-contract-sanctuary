...
contract GearSlots is ERC721Enumerable, Ownable, VRFConsumerBase {
    ...
    function getItemData(uint256 number) public view returns (string[][] memory) {
        IGearSlotsData _gearSlotsData = IGearSlotsData(_gearSlotsDataAddress);

        return _gearSlotsData.getItemData();
    }

    function getWeapons(uint256 number) public view returns (string[] memory) {
        IGearSlotsData _gearSlotsData = IGearSlotsData(_gearSlotsDataAddress);

        return _gearSlotsData.getWeapons();
    }    
