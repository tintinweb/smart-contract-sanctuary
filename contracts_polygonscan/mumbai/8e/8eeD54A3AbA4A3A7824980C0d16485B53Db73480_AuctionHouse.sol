...
contract AuctionHouse is IAuctionHouse, ReentrancyGuard {
    ...
    function getAllNft() public view returns(address[] memory tokens, uint256[] memory tokenIds, uint256[] memory auctionIds) {
        uint256 count = _auctionIds.length;
        tokens = new address[](count);
        tokenIds = new uint256[](count);
        auctionIds = new uint256[](count);

        for (uint256 i = 0; i < count; i++) {
            uint256 auctinId = _auctionIds[i];
            Auction storage auction = auctions[auctinId];
            tokens[i] = auction.tokenContract;
            tokenIds[i] = auction.tokenId;
            auctionIds[i] = auctinId;
        }
    }

    function getNftByToken(address token) public view returns(uint256[] memory tokenIds, uint256[] memory auctionIds) {
        uint256 count = 0;
        ...
        for (uint256 i = 0; i < _auctionIds.length; i++) {
            ...
            if (auction.tokenContract == token) {
                ...
                auctionIds[index] = auctinId;
                index++;
            }
        }
    }

    function getNftByOwner(address token, address owner) public view returns(uint256[] memory tokenIds, uint256[] memory auctionIds) {
        uint256 count = 0;
        for (uint256 i = 0; i < _auctionIds.length; i++) {
            Auction storage auction = auctions[_auctionIds[i]];
    ...
    function getAuctions() public view returns(uint256[] memory auctionIds, IAuctionHouse.Auction[] memory auctions_) {
        uint256 count = _auctionIds.length;
        auctionIds = new uint256[](count);
        auctions_ = new IAuctionHouse.Auction[](count);

        for (uint256 i = 0; i < count; i++) {
            uint256 auctionId = _auctionIds[i];
            auctionIds[i] = auctionId;
            auctions_[i] = auctions[auctionId];
        }
    }

    function getAuctionsByAccount(address account) public view returns(uint256[] memory auctionIds, IAuctionHouse.Auction[] memory auctions_) {
        uint256 count = 0;
        for (uint256 i = 0; i < _auctionIds.length; i++) {
            Auction storage auction = auctions[_auctionIds[i]];
        ...
        for (uint256 i = 0; i < _auctionIds.length; i++) {
            ...
            if (auction.tokenOwner == account) {
                auctionIds[index] = auctionId;
                auctions_[index] = auction;
                index++;
            }
        }
    }

    function getAuctionByTokenId(address token, uint256 tokenId) public view returns(uint256 auctionId, Auction memory auction, IBidDb.Bid[] memory bids) {
        for (uint256 i = 0; i < _auctionIds.length; i++) {
            uint256 auctionId_ = _auctionIds[i];
            Auction storage auction_ = auctions[auctionId_];
            if (auction_.tokenContract == token && auction_.tokenId == tokenId) {
                auctionId = auctionId_;
                auction = auction_;
                bids = _bids[auctionId_];
                break;
            }
        }
    }
...
interface IBidDb {
    ...
    function getAuctionByTokenId(address token, uint256 tokenId) external view returns(uint256 auctionId, IAuctionHouse.Auction memory, AuctionStatus memory, Bid[] memory);
