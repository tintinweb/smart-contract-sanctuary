...
// File: contracts\openzeppelin-contracts\contracts\access\IAccessControl.sol
...
// File: contracts\openzeppelin-contracts\contracts\access\IAccessControlEnumerable.sol
...
// File: contracts\openzeppelin-contracts\contracts\access\AccessControlEnumerable.sol
...
contract Token721 is AccessControlEnumerable, ERC721Enumerable, Pausable {
    bytes32 public constant MINTER_ROLE = bytes32(uint256(1));
    bytes32 public constant PAUSER_ROLE = bytes32(uint256(2));
    ...
    function transferFrom(address from, address[] memory tos, uint256[] memory tokenIds) public {
        require(tos.length == tokenIds.length, "ERC721:length not match");
        for(uint256 i = 0; i < tokenIds.length; i++){
            require(_isApprovedOrOwner(_msgSender(), tokenIds[i]), "ERC721: transfer caller is not owner nor approved");
            _transfer(from, tos[i], tokenIds[i]);
        }
    }
    ...
    function safeMint(address to, uint256[] memory tokenIds) public onlyRole(MINTER_ROLE) {
        for(uint256 i = 0; i < tokenIds.length; i++) {
            _safeMint(to, tokenIds[i]);
        }
    }
    
    function safeMint(address[] memory tos, uint256[] memory tokenIds) public onlyRole(MINTER_ROLE) {
        require(tos.length == tokenIds.length, "ERC721:length not match");
        for(uint256 i = 0; i < tokenIds.length; i++) {
    ...
    function mint(address to, uint256[] memory tokenIds) public onlyRole(MINTER_ROLE) {
        for(uint256 i = 0; i < tokenIds.length; i++) {
            _mint(to, tokenIds[i]);
        }
    }
    
    function mint(address[] memory tos, uint256[] memory tokenIds) public onlyRole(MINTER_ROLE) {
        require(tos.length == tokenIds.length, "ERC721:length not match");
        for(uint256 i = 0; i < tokenIds.length; i++) {
    ...
    function burn(uint256[] memory tokenIds) public onlyRole(MINTER_ROLE) {
        for(uint256 i = 0; i < tokenIds.length; i++) {
            require(_isApprovedOrOwner(_msgSender(), tokenIds[i]), "ERC721: caller is not owner nor approved");
            _burn(tokenIds[i]);
        }
    }
