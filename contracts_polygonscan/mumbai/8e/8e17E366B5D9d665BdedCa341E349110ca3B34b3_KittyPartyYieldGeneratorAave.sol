...
contract KittyPartyYieldGeneratorAave is Initializable, IKittyPartyYieldGenerator, OwnableUpgradeable {
        ...
        returns (uint256 tokensRec)
    {
        ...
        uint256 lpTokenBalance = IERC20Upgradeable(kittyPartyYieldInfo[msg.sender].lpTokenAddress).balanceOf(address(this));
        ...
        payload = abi.encodeWithSignature("withdraw(address,uint256,address)",
                                          kittyPartyYieldInfo[msg.sender].sellTokenAddress,
                                          kittyPartyYieldInfo[msg.sender].yieldGeneratedInLastRound,
                                          _treasuryContract);
        (bool success,) = address(AaveContract).call(payload);
        require(success, 'Withdraw failed');
        emit YieldClaimed(kittyPartyYieldInfo[msg.sender].yieldGeneratedInLastRound);
