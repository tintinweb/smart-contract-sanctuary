...
contract Presale is Ownable, ReentrancyGuard {
    ...
    constructor(uint256 _fee, address _occ) {
        currentFee = _fee;
        feeWallet = payable(msg.sender);
        raiseFeeWallet = payable(msg.sender);
        OCCToken = IERC20(_occ);
    }

    function updateOcc(address _occ) external onlyOwner {
        OCCToken = IERC20(_occ);
    }
        ...
        require(
            whitelist1[_saleId][msg.sender] == true ||
                (outsideContributionBalance != 0 &&
                    OCCToken.balanceOf(msg.sender).add(getStakedAmount()) >=
    ...
    function tier2Sale(uint256 _saleId) internal {
        bool _balanceContributionAvailable = OCCToken
