...
contract GameMaster is Initializable, AccessControlUpgradeable, PausableUpgradeable, ICharacter {
    ...
    event FightOutcome(address indexed owner, uint256 indexed character, uint256 weapon, uint32 targetPower, uint8 targetTrait, uint24 playerRoll, uint24 enemyRoll, uint16 xpGain, uint256 tagsGain);
    ...
    function getTargetsInternal(
        ...
        function fight(
            ...
            (
                ...
                // uint64 timestamp
                    ...
                    characters.getFightDataAndDrainStamina( 
                        char,
                        uint8(fightMultiplier.mul(staminaCostFight))
                    )
            );
            ...
            weapons.getFightDataAndDrainDurability(
                    wep,
                    uint8(fightMultiplier.mul(durabilityCostFight))
            );
    ...
    function getTokenGainForFight(uint24 monsterPower) internal view returns (uint256) {
        ...
        uint24 maxMonsterPower = uint24(ABDKMath64x64.mulu(
            ABDKMath64x64.divu(11, 10),
            (characters.getPowerAtLevel(255).add(maxWeaponPower))
        ));
        uint24 minMonsterPower = uint24(ABDKMath64x64.mulu(
            ABDKMath64x64.divu(9, 10),
            (characters.getPowerAtLevel(0).add(minWeaponPower))
        ));
    ...
    function _calculateFightRewardBaseline(uint256 totalRewards) internal view returns (uint256) {
        ...
        uint256 maxNumOfFights = (characters.getOperatorsCountPerRole(CharacterType.Elite)).mul(FIGHTS_PER_ELITE_PER_DAY).mul(epochDurationInDays);
