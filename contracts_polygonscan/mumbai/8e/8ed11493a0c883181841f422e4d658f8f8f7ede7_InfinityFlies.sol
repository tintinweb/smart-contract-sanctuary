...
// File: contracts/InfinityFlies.sol
...
contract InfinityFlies is ERC721Enumerable, Ownable {
    ...
    constructor() ERC721("Infinity Flies", "FLIES") {
        _setVerifier(_msgSender());
    }
    ...
    function transferVerifier(address newVerifier) public virtual onlyOwner {
    ...
    function claim(
        ...
        require(total + _tokensId.length <= TOTAL_SUPPLY, "Max limit");
        ...
        require(signerOwner == _verifier, "Not authorized to claim");

        require(block.timestamp >= _timestamp - 30, "Out of time");

        for (uint8 i = 0; i < _tokensId.length; i++) {
            require(
                rawOwnerOf(_tokensId[i]) == address(0) &&
                    _tokensId[i] > 0 &&
    ...
    function signatureWallet(
        ...
        return
            ECDSA.recover(
                ECDSA.toEthSignedMessageHash(keccak256(abi.encode(wallet, _tokensId, _timestamp))),
                _signature
            );
    }
