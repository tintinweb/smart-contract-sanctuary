...
pragma solidity ^0.5.5;
 ...
 contract Flag_Crowdsale is Crowdsale, Ownable {
	...
	// Hard Cap
	uint256 private constant HARDCAP = 1000 ether;
	// Start ICO
	uint256 private constant START_ICO = 1633824000; // Date and time (GMT): Sunday, 10 October 2021 г., 0:00:00

    constructor(
        IERC20 token
    ) public Crowdsale(4, 0xa5824a8f9e997e50B62B5E4130B4605B6D5404Cb, token) {  
    ...
    function _preValidatePurchase(address beneficiary, uint256 weiAmount) internal view {
        ...
        require((token().balanceOf(beneficiary)).add(weiAmount) <= HARDCAP);		
    }	
