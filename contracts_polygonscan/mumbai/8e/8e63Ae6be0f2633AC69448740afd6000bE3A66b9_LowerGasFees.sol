...
contract LowerGasFees is ERC721, Ownable {
  using Strings for uint256;
  ...
  constructor()
    ERC721("Pixelated Elemental Dinosaurs", "PED") {
    setBaseUri("ipfs://QmQDSinCvXt9BWQBHVwMDupznQCTqiDkHaFxSN6iTP2mBG/");
    setHiddenMetadataUri("ipfs://QmYCysLxfkZHdXvUTAvbhNmeNs6tMzsMn6EF3ta9JfUmCD/unknown.json"); 
...
function UpdateCost(uint256 maxsupply) internal view returns (uint256 _cost){
      if(maxsupply < 500 ) {
          return 1.5 ether;
      }
      if(maxsupply < 850 ) {
  ...
  function mint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) {
    ...
    require(msg.value >= UpdateCost(maxSupply) * _mintAmount, "Not Enough Funds!");
 ...
 function mintForOwner(uint256 _mintAmount) public onlyOwner {
    ...
    require(_mintAmount <= _mintAmount);
    require(maxsupply + _mintAmount <= maxSupply);
...
function claim() public {
    ...
    require(ownedAmount >= requiredAmount, "You don't own enough Scare Bear NFT's");
    uint256 maxsupply = totalSupply();
