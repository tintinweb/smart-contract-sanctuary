...
contract MonoswapStaking is Initializable, OwnableUpgradeable, ERC1155HolderUpgradeable {
        ...
        returns (uint256)
    {
        ...
        return user.amount.mul(accMonoPerShare).div(1e12).mul(90).div(100).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public validPool(_pid) {
        ...
        uint256 userReward = monoReward.div(1e12).mul(90).div(100);
    ...
    function deposit(uint256 _pid, uint256 _amount) public validPool(_pid) {
        ...
        if (user.amount > 0) {
            uint256 accMonoPerShare = pool.accMonoPerShare;
            uint256 pending = user.amount.mul(accMonoPerShare).div(1e12).mul(90).div(100).sub(user.rewardDebt);
            if (user.oldReward > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accMonoPerShare).div(1e12).mul(90).div(100);
