...
contract QuizChest is Ownable, Events {
    ...
    function lock(address _from, uint256 amount) public returns(bool){
        ...
        _locks[_from].lockedAmount = amount;
    ...
    function unlock(address _to) public returns(bool){
        ...
        uint256 amount = uint256(_lock.lockedAmount);
        uint256 tax;
        
        require(_to != address(0x0) && _lock.locked,"Wrong address or not locked");

        if(_lock.lastLockTimeStamp + unlocklimitDays * 1 days > block.timestamp){
