...
contract Famoso is ERC20 {
    ...
    modifier lock() { require(unlocked == 1, "FamosoL LOCKED"); unlocked = 0; _; unlocked = 1; }
    ...
    function skim(address to) external lock {
        address _token0 = token0; // gas savings
        address _token1 = token1; // gas savings
        _safeTransfer(_token0, to, ERC20(_token0).balanceOf(address(this)).sub(reserve0));
        _safeTransfer(_token1, to, ERC20(_token1).balanceOf(address(this)).sub(reserve1));
    }
...
contract FamosoFactory {
    ...
    function getAllFamoso(uint index) public returns (address famoso) {
        famoso = allFamosos[index];
    }

    function createFamoso(address tokenA, address tokenB) external returns (address famoso)
    {
