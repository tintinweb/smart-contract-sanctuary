...
contract TheWallDaily is Context, IERC721Receiver, ReentrancyGuard
{
    ...
    event BlockedSet(uint256 indexed areaId, bool b);
    ...
    function setBlocked(uint256 areaId, bool b) public
    {
        // It is impossible to setContent() call from new tenant for blocked areas.
        // But anyway it is possible to prolong rent for current tenant.
        Area storage area = _pool[areaId];
        require(area.provider != address(0), "TheWallDaily: No area found");
        require(area.provider == _msgSender(), "TheWallDaily: No permissions");
        area.blocked = b;
        emit BlockedSet(areaId, b);
    }
    ...
    function setContent(uint256 areaId, bytes memory content) public payable
    {
        ...
        require(!area.blocked || t == _msgSender(), "TheWallDaily: Area is blocked by provider");
