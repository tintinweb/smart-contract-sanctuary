...
*/
contract Whitelist is Pausable, Ownable, IWhitelist {
    ...
    */
    constructor(
        ...
        require(_maxWhitelistLength > 0 || _whitelistEndDate > 0, 'Either length or duration has to be set');
        require(_whitelistEndDate == 0 || _whitelistEndDate > block.timestamp, 'End date has to be in the future');
        require(_offchainSigner != address(0), 'Offchain signer address cannot be 0');
     ...
     */
    function getWhitelistLength() public view override returns (uint256) {
    ...
    */
    ...
    function isFinished() public view override returns (bool) {
        return
            (whitelistEndDate != 0 && block.timestamp >= whitelistEndDate) ||
    ...
    */
    function isWhitelisted(address _account) public view override returns (bool) {
    ...
    */
    function getReferredCount(address _account) public view override returns (uint256) {
        int256 ret = whitelistList[_account].baseLevel + int256(whitelistList[_account].referred.length);

        if (ret <= 0) return 0;
        else return uint256(ret);
    }
    ...
    */
    function activate(address _referrer) public whenWhitelistActive {
    ...
    */
    function setOffchainSigner(address _offchainSigner) public onlyOwner {
        offchainSigner = _offchainSigner;
    }
    ...
    */
    function activateOffchain(
        ...
        require(
            offchainSigner == recover(keccak256(abi.encodePacked(_msgSender(), uint256(_baseLevel))), _signature),
            'Invalid signature.'
        );
    ...
    */
    function setWhitelist(
        ...
        require(
            _active == false || maxWhitelistLength == 0 || whitelistLength < maxWhitelistLength,
            'Whitelist is full.'
        );
