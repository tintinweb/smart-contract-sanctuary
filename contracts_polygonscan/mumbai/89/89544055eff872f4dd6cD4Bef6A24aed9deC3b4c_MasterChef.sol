...
contract Token is ERC20, Ownable {
    ...
    constructor() ERC20("uuuu Token", "UUUU") {
...
contract MasterChef is Ownable {
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, uint16 _depositFeeBP, bool _withUpdate) public nonDuplicated(_lpToken) {
        require(_depositFeeBP <= 10, "add: invalid deposit fee basis points");
    ...
    function set(uint256 _pid, uint256 _allocPoint, uint16 _depositFeeBP, bool _withUpdate) public onlyOwner {
        require(_depositFeeBP <= 10, "set: invalid deposit fee basis points");
    ...
    function updatePool(uint256 _pid) public {
        ...
        token.mint(devaddr, tokenReward.div(12));
    ...
    function safeTokenTransfer(address _to, uint256 _amount) internal {
        ...
        if (sendAmount > tokenBal) {
            transferSuccess = token.transfer(_to, tokenBal);
            tokenBal = 0;
        } else {
            transferSuccess = token.transfer(_to, sendAmount);
            tokenBal = tokenBal.sub(sendAmount);
        }

        if (burnAmount > tokenBal) {
            token.transfer(BURN_ADDRESS, tokenBal);
        } else {
    ...
    function updateEmissionRate(uint256 tokenPerBlock) public onlyOwner {
        massUpdatePools();
