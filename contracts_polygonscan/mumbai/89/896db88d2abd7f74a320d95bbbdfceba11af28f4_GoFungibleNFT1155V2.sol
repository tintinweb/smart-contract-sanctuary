...
contract ERC1155 is Context, ERC165, IERC1155, IERC1155MetadataURI {
    ...
    */
    ...
    function _mint(address creator, uint256 tokenId, uint256 _supply, string memory _uri, uint256 _fee) internal {
        require(!_exists(tokenId), "ERC1155: token already minted");
        require(_supply != 0, "Supply should be positive");
        require(bytes(_uri).length > 0, "uri should be set");

        creators[tokenId] = creator;
        _tokenOwners.set(tokenId, creator);
        _royaltyFee[tokenId] = _fee;
        _balances[tokenId][creator] = _supply;
...
contract GoFungibleNFT1155V2 is ERC1155 {
    ...
    */
    ...
    function mint(address creator, string memory uri, uint256 supply, uint256 fee, Sign memory sign) public onlyOwner {
        verifySign(uri, sign);
