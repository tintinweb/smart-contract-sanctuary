    ...
    function _functionCallWithValue(
        ...
        if (success) {
            ...
            if (returndata.length > 0) {
                // The easiest way to bubble the revert reason is using waterry via assembly
...
contract Distributor is Ownable {
    ...
    constructor( address _treasury, address _oxygen, uint32 _epochLength, uint32 _nextEpochTime ) {        
    ...
    function distribute() external returns ( bool ) {
        if ( nextEpochTime <= uint32(block.timestamp) ) {
            nextEpochTime = nextEpochTime.add32( epochLength ); // set next epoch oxygen
    ...
    function nextRewardAt( uint _rate ) public view returns ( uint ) {
        return oxygen.totalSupply().mul( _rate ).div( 1000000 );
    }
