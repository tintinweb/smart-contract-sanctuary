...
contract DoubleOne{
    ...
    function calculatePercent(uint _packPrice, uint _percent) internal pure returns(uint) {
        uint amount = _packPrice.mul(1e6).mul(_percent).div(10000);
        return amount;
    }
    ...
    function buyPackage(string memory _packageName) external {
        ...
        uint256 packPrice = packagePrice[_packageName];
        ...
        if (referral.hasReferrer(msg.sender)){
            ...
            for (uint i = 0;i<referralRewardBracket.length;i++){
                if (referral.hasReferrer(r)){
                    ...
                    USDT.transferFrom(msg.sender, j, calculatePercent(packPrice, referralRewardBracket[i]));
                    r = j;
                }
            }
            USDT.transferFrom(msg.sender, operator, calculatePercent(packPrice, 6000));
        }
