...
contract BondingCalculator is IBondingCalculator {
    ...
    function markdown( address _pair ) external view returns ( uint ) {
        ...
        uint reserve;
        if ( IUniswapV2Pair( _pair ).token0() == address(Oxygen) ) {
            reserve = reserve1;
        } else {
            require(IUniswapV2Pair( _pair ).token1() == address(Oxygen), "not a Oxygen lp pair");
            reserve = reserve0;
        }
        return reserve.mul( 2 * ( 10 ** Oxygen.decimals() ) ).div( getTotalValue( _pair ) );
    }
}