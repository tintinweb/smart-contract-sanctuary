...
contract drop1 is SysCtrl{
    ...
    uint256 maxLevel = 1000;
    uint256 manufacturedLevel = 0; 
    uint256 morphingTime = (21*24*60*60);  // 21 days 
    uint256 price1Egg = 50000000000000000;  
    uint256 price3Egg = 100000000000000000;

    uint256 availableFree = 10; 
    uint256 intervalFree = (24*60*60); // 24 hours
    uint256 dificultFree = 10;
    ...
    function get3Egg() external payable reentrancyGuard returns(uint256[] memory axies)  {
        require(!sellPaused);
        ...
        emit AxieEgg3(axies[0],axies[1],axies[2], msg.sender, currentLevel, msg.value);
        return axies;
    }

    function TryFree() external payable reentrancyGuard returns(uint256 _axieFree)  {
        require(!sellPaused);
        require(maxLevel > manufacturedLevel && availableFree > 0,"There are no axies available");
        require(usersFree[msg.sender]+intervalFree <= block.timestamp,"Ineligible address, wait time");
        _axieFree = 0;
        if(_exons(0,dificultFree) == 0){
          manufacturedLevel++;
          availableFree--;
    ...
    function getStats() external view returns(uint256 _currentLevel, uint256 _maxLevel, uint256 _manufacturedLevel, uint256 _morphingTime, uint256 _price1Egg, uint256 _price3Egg){
    ...
    function configFree(uint256 _currentLevel, uint256 _maxLevel, uint256 _manufacturedLevel, uint256 _morphingTime, uint256 _price1Egg, uint256 _price3Egg) external onlyAdmin  {
       if(_currentLevel >0) currentLevel = _currentLevel;
       if(_maxLevel >0) maxLevel = _maxLevel;
       if(_manufacturedLevel >0) manufacturedLevel = _manufacturedLevel;
       if(_morphingTime >0) morphingTime = _morphingTime;
    ...
    function _getAxieEgg() internal returns (uint256) {
        ...
        for(uint256 helix=0; helix<=42; helix++){
            ...
            if(helix >= 24){               // Class (D,R1,R2) - 6 elements
               ...
               if(genes > 5 && genes <= 17){  // Beast 
                   genes = 0;
               } 
               if(genes > 17 && genes <= 27) { // Aquatic
                   genes = 2;
               }
               if(genes > 27 && genes <= 29) { // Bird
                   genes = 3;
               }
               if(genes > 29 && genes <= 32) { // Reptile
                   genes = 4;
               }
               if(genes > 32) { // Plant
                   genes = 5;
