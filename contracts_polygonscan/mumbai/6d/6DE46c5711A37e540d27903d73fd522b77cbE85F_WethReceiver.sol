...
contract WethReceiver is EIP712MetaTransaction {
  ...
  modifier isActive {
    require(!mochaToken.isMaintaining());
    _;
  }

  modifier onlyMocha() {
    require(
      msgSender() == mochaToken.owner() ||
        msgSender() == mochaToken.mochaTrust() ||
        mochaToken.isAuthorizedAddress(msgSender()),
      "caller is not one of mocha accounts"
    );
    _;
  }
  ...
  constructor(
    ...
    mochaToken = MochaToken(_tokenAddress);
  ...
  function setBeneficiary(address _beneficiary) external onlyMocha {
    beneficiary = _beneficiary;
  }

  function sendPayout(uint256 amount, address _receiver) external onlyMocha {
...
// CEO adds Mocha EOA in AccessControl
// CEO adds Marketplace contract in AccessControl
// Mocha EOA does all the essential calls
// CEO is a master account who can halt smart contract and
// remove Mocha EOA and Marketplace EOA just in case something happens

interface IMarketplace {
    function setPasscode(bytes32) external;
}

contract MochaToken is ERC721URIStorageEnumerable, AccessControl {
  ...
  /**
    ...
    * Set Mocha token name and symbol
    * Set initial fee percentage which is 2.5%
    *
    * @param _name Mocha token name
    * @param _symbol Mocha token symbol
    * @param _feePercent fee percentage Mocha account will receive
    */
  constructor(
    string memory _name,
  ...
  /**
    ...
    * Mocha keeps the token on w2user's behalf

    * Precondition
    * 1. user pays gas fee to Mocha in USD
    *
    ...
    onlyMocha
  {
    ...
    // Mocha keeps the token on minter's behalf
...
/**
  ...
  /**
    ...
    */
  function claimToken(uint256[] memory _tokenIDs, address _w3user) public onlyMocha {
    for (uint i = 0; i < _tokenIDs.length; i++) {
      uint256 tokenId = _tokenIDs[i];
      require(!tokenOwners[tokenId].isW3user, "Only web2 users token can be claimed");
      require(ownerOf(tokenId) == mochaTrust, "Mocha wallet does not own this token");
      if (_w3user != address(0)) {
        _transfer(mochaTrust, _w3user, tokenId);
...
abstract contract EIP712MetaTransaction is EIP712Base {
  ...
  string internal constant DOMAIN_NAME = "mocha.app";
...
abstract contract AccessControl is EIP712MetaTransaction {
    ...
    address public mochaTrust;
    address public mochaFee;
    ...
    function changeMochaTrust(address _newMochaTrust) onlyCEO external {
        if (_newMochaTrust != address(0)) {
            mochaTrust = _newMochaTrust;
            _authorizedAddressList[_newMochaTrust] = true;
        }
    }

    function changeMochaFee(address _newMochaFee) onlyCEO external {
        if (_newMochaFee != address(0)) {
            mochaFee = _newMochaFee;
        }
    }
