...
contract Barn is Ownable, IERC721Receiver, ReentrancyGuard {
	// maximum str score for a Wolf
	uint8 public constant MAX_STR = 15;
	uint256 public STR_RATIO = 1; //can ajust the str ratio of gains per STR level if needed
	...
	// amount of $WOOL due for each str point staked
	uint256 public woolPerStr = 0;

	// sheep earn 10000 $WOOL per day
	uint256 public constant DAILY_WOOL_RATE = 20000 ether;
	...
	function _addWolfToPack(uint256 tokenId) internal {
		uint256 str = _strengthForWolf(tokenId);
		totalStrStaked += str;
		packIndices[tokenId] = pack[str].length; // Store the location of the wolf in the Pack
		pack[str].push(Stake({ owner: msg.sender, tokenId: tokenId, value: woolPerStr })); // Add the wolf to the Pack
		emit TokenStaked(msg.sender, tokenId, woolPerStr);
	}
	 ...
	 * Wolves earn $WOOL proportional to their str rank
	...
	function _claimWolfFromPack(uint256 tokenId, bool unstake) internal returns (uint256 owed) {
		...
		uint256 str = _strengthForWolf(tokenId);
		Stake memory stake = pack[str][packIndices[tokenId]];
		require(stake.owner == msg.sender, "msg.sender is not stake.owner");
		owed = (str * STR_RATIO) * (woolPerStr - stake.value); // Calculate portion of tokens based on str
		if (unstake) {
			...
			pack[str][packIndices[tokenId]] = lastStake; // Shuffle last Wolf to current position
	...
	function rescue(uint256[] calldata tokenIds) external nonReentrant {
		...
		for (uint256 i = 0; i < tokenIds.length; i++) {
			...
			if (isSheep(tokenId)) {
				...
				str = _strengthForWolf(tokenId);
				stake = pack[str][packIndices[tokenId]];
				require(stake.owner == msg.sender, "msg.sender is not stake.owner");
				totalStrStaked -= str; // Remove str from total staked
				wolf.safeTransferFrom(address(this), msg.sender, tokenId, ""); // Send back Wolf
	...
	function _payWolfTax(uint256 amount) internal {
		...
		woolPerStr += (amount + unaccountedRewards) / totalStrStaked;
	...
	/**
	 * gets the strengh score for a Wolf (higher is better)
	 * @param tokenId the ID of the Wolf to get the str score for
	 * @return the str score of the Wolf
	 */
	function _strengthForWolf(uint256 tokenId) internal view returns (uint256) {
		(, uint256 strIndex) = wolf.tokenTraits(tokenId);
	...
	function randomWolfOwner(uint256 seed) external view returns (address) {
		...
		for (uint256 i = 1; i <= MAX_STR; i++) {
			...
			// get the address of a random Wolf with that str score
