...
contract RewardManagement {
    ...
    mapping(address => uint256) private nodeOwners;
    mapping(address => NodeEntity[]) private _nodesOfUser;

    uint256 public nodePrice;
    uint256 public rewardPerNode;
    Counters.Counter public totalNodesCreated;
    ...
    function createNode(address account, string memory nodeName) external {
        ...
        nodeOwners[account] = _nodesOfUser[account].length;
        totalNodesCreated.increment();
    }
        ...
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < nodesCount; i++) {
            if (nodes[i].lastClaimTime > block.number) continue;
            uint256 passedBlocks = block.number - nodes[i].lastClaimTime;
            rewardCount += passedBlocks * (rewardPerNode / 41000);
        }
        ...
        external
    {
        ...
        for (uint256 i = 0; i < nodesCount; i++) {
            nodes[i].lastClaimTime = block.number;
        }
    }
...
contract Forward is ERC20, Ownable {
    ...
    function createNodeWithTokens(string memory name) public {
        ...
        uint256 nodePrice = rewardManagement.nodePrice();
        require(
            balanceOf(sender) >= nodePrice,
            "NODE CREATION: Balance too low for creation."
        );
        super.transfer(address(this), nodePrice);
        rewardManagement.createNode(sender, name);
    }
    ...
    function cashoutAll() public {
        ...
        uint256 rewardAmount = rewardManagement._getRewardAmountOf(sender);
        require(
            rewardAmount > 0,
            "MANIA CSHT: You don't have enough reward to cash out"
        );

        super.transferFrom(address(this), sender, rewardAmount);
        rewardManagement._cashoutAllNodesReward(sender);
    }
