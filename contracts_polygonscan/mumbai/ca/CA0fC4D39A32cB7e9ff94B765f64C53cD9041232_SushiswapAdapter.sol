...
contract SushiswapAdapter is IComposableExchange, OwnableUpgradeable {
		...
		)
	{
		...
		IERC20(tokenA).safeApprove(address(swapRouter), amountADesired);
		IERC20(tokenB).safeApprove(address(swapRouter), amountBDesired);
		...
		)
	{
		...
		IERC20(token).safeApprove(address(swapRouter), amountTokenDesired);
	...
	function removeLiquidity(
		...
		ISushiswapFactory factory = ISushiswapFactory(swapRouter.factory());
