...
contract XChainTrade { 
    ...
    receive() external payable {
        ...
        require(thedate >= presaleStarts, "XChain: NFT presale not started yet (starts Jan 15th, 2022)!"); // If someone try to purchase a package before: Friday, January 7, 2022 12:00:00 PM (GMT)
        ...
        if (incwei >= 10 ether && thedate < presaleStops) {
            if (incwei >= 200 ether && minted + 400 <= 5000) { // if >= than 200 ETH before 2 hours and 15 minutes from initial time
                uint packages = incwei / 200 ether;
                uint maxPackages = (5000 - minted) / uint(400);
                if (maxPackages < packages) {packages = maxPackages;}
                toBeMinted = packages * 400;
                spent = packages * 200 ether; // adds 200 ETH to the bill
                incwei -= spent; // removes 200 ETH from the budget
            ...
            if (incwei >= 100 ether && minted + toBeMinted + 150 <= 5000) { // else if >= than 100 ETH before 2 hours and 15 minutes from initial time
                spent = spent + 100 ether; // adds 100 ETH to the bill
            ...
            if (incwei >= 50 ether && minted + toBeMinted + 65 <= 5000) { // else if >= than 50 ETH before 2 hours and 15 minutes from initial time
                spent = spent + 50 ether; // adds 50 ETH to the bill
            ...
            if (minted + toBeMinted + 11 <= 5000) { // else if >= than 10 ETH before 2 hours and 15 minutes from initial time
                spent = spent + 10 ether; // adds 10 ETH to the bill
        ...
        if (thedate >= saleStarts && minted + toBeMinted + 1 <= 5000 && incwei >= price) {
            toBeMinted = incwei / price;
            if (toBeMinted + minted > 5000) {toBeMinted = 5000 - minted;}
            spent = price * toBeMinted;
            incwei = incwei - spent;
