...
contract LedgerConnect4 is Ownable {
    ...
    function getLeaderboard() public view returns (address[], uint256[]) {
        ...
        // @dev player with at least 1 point
        uint256 numberOfActivePlayer = 0;
        ...
        uint256[] memory playersAmount = new uint256[](numberOfActivePlayer);
        address[] memory playersAddresses = new address[](numberOfActivePlayer);
        uint256 counter = 0;

        for (uint256 j = 0; j < players.length; j++) {
            player = players[j];
            if (player.amountWon > 0) {
                playersAmount[counter] = player.amountWon;
                playersAddresses[counter] = player.username;
                counter++;
            }
        }
