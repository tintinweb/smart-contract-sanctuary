...
contract CurrencyManager is Ownable {
  using EnumerableSet for EnumerableSet.AddressSet;
  EnumerableSet.AddressSet internal supportedCurrenciesSet;
  ...
  function addCurrencySupport(address _currency) public onlyOwner {
    ...
    require(EnumerableSet.contains(supportedCurrenciesSet, _currency) == false, "This currency is already supported");
    // Add support for the currency
    EnumerableSet.add(supportedCurrenciesSet, _currency);

    emit CurrencySupportChanged(_currency, EnumerableSet.contains(supportedCurrenciesSet, _currency));
  }

  /// @notice Remove support for an ERC20 token
  function removeCurrencySupport(address _currency) public onlyOwner {
    // Avoid user wasting gas
    require(EnumerableSet.contains(supportedCurrenciesSet, _currency) == true, "This currency is already not supported");
    // Remove support for the currency
    EnumerableSet.remove(supportedCurrenciesSet, _currency);
  ...
  function supportedCurrencies() external view returns(address[] memory) {
    uint _currenciesSetLength = EnumerableSet.length(supportedCurrenciesSet);
    address[] memory _currenciesArray = new address[](_currenciesSetLength);
    for(uint _i = 0; _i < _currenciesSetLength; _i++) {
      _currenciesArray[_i] = EnumerableSet.at(supportedCurrenciesSet, _i);
    }
...
/// @title Contains all the variables and functions regarding the management of address authorized to deploy a gachapon.
/// @author KirienzoEth for DokiDoki
contract DeployerManager is Ownable {
  /// @notice Is address authorized to create gachapons
  mapping(address => bool) public authorizedDeployers;

  event DeployerAuthorizationChanged(address indexed _address, bool indexed _isAuthorized);

  /// @notice Allow the provided address to use this deployer
  function addAddressToAuthorizedDeployers(address _address) public onlyOwner {
    // Avoid user wasting gas
    require(authorizedDeployers[_address] == false, "This address is already authorized");
    // Toggle the whitelisting of the address
    authorizedDeployers[_address] = true;

    emit DeployerAuthorizationChanged(_address, authorizedDeployers[_address]);
  }

  /// @notice Revoke the permission for the provided address to use this deployer
  function removeAddressFromAuthorizedDeployers(address _address) public onlyOwner {
    // Avoid user wasting gas
    require(authorizedDeployers[_address] == true, "This address is already unauthorized");
    // Toggle the whitelisting of the address
    authorizedDeployers[_address] = false;
...
import "@openzeppelin/contracts/utils/structs/EnumerableSet.sol";
import "../interfaces/IGachaponCreator.sol";
...
/// @title A factory to deploy and track gachapon machines.
/// @author KirienzoEth for DokiDoki
contract GachaponFactory is TreasuryManager, CurrencyManager, GachaponOperator, VRFManager {
  using EnumerableSet for EnumerableSet.AddressSet;

  bytes4 private INTERFACE_ID_ERC1155 = 0xd9b67a26;
  bytes4 private INTERFACE_ID_ERC721 = 0x80ac58cd;
  ...
  IGachaponCreator public gachaponCreator;

  event GachaponCreated(address indexed _creator, string _title, address _gachapon, address indexed _currency, uint _timestamp);
  event GachaponCreatorChanged(address _previous, address indexed _current);

  constructor() {
    addAddressToAuthorizedDeployers(msg.sender);
  }
  ...
  function createGachapon(string memory _machineTitle, IERC20 _currency) public {
    ...
    assert(address(gachapons[index]) == address(0));
    // Make sure the gachapon creator contract is set
    require(address(gachaponCreator) != address(0), "No gachapon creator contract is set");
    ...
    require(EnumerableSet.contains(supportedCurrenciesSet, address(_currency)), "Currency is not supported");
    // Make sure only authorized addresses can create a gachapon
    require(authorizedDeployers[msg.sender], "You are not authorized to create a gachapon");

    // Create the new gachapon machine
    address _gachapon = gachaponCreator.createGachapon(_machineTitle, address(_currency));
    // Save the gachapon machine in the mapping with the current index
    gachapons[index] = IGachapon(_gachapon);
    // Transfer the ownership to the deployer's address
    IGachapon(gachapons[index]).transferOwnership(msg.sender);

    emit GachaponCreated(address(gachaponCreator), _machineTitle, address(_gachapon), address(_currency), block.timestamp);
  ...
  /// @dev Contract MUST implement IGachaponCreator interface
  function setGachaponCreator(address _gachaponCreator) public onlyOwner {
    address _previous = address(gachaponCreator);
    // Set new gachapon creator address
    gachaponCreator = IGachaponCreator(_gachaponCreator);

    emit GachaponCreatorChanged(_previous, address(gachaponCreator));
  }
}
...
import "./DeployerManager.sol";

/// @title Contract to allow deployers to operate gachapons
/// @author KirienzoEth for DokiDoki
contract GachaponOperator is DeployerManager {
  /// @notice Ban a gachapon
  /// @dev Banned gachapons can't be played anymore
  /// @param _address Address of the targeted gachapon
  function ban(address _address) external onlyDeployer {
    IDeployerControlledState(_address).ban();
  }

  /// @notice Unban a gachapon
  /// @param _address Address of the targeted gachapon
  function unban(address _address) external onlyDeployer {
    IDeployerControlledState(_address).unban();
  }

  /// @notice Set the DAO's share at `_rate` / 1000
  /// @param _address Address of the targeted gachapon
  function setDaoRate(address _address, uint16 _rate) external onlyDeployer {
    IDeployerControlledState(_address).setDaoRate(_rate);
  }

  /// @notice Set the tokenomics' share at `_rate` / 1000
  /// @param _address Address of the targeted gachapon
  function setTokenomicsRate(address _address, uint16 _rate) external onlyDeployer {
    IDeployerControlledState(_address).setTokenomicsRate(_rate);
  }
  
  /// @notice Set the artist's share at `_rate` / 1000
  /// @param _address Address of the targeted gachapon
  function setArtistRate(address _address, uint16 _rate) external onlyDeployer {
    IDeployerControlledState(_address).setArtistRate(_rate);
  }
  ...
  function setArtistProfitsSharing(address _address, address[] memory _addresses, uint16[] memory _rates) external onlyDeployer {
    IDeployerControlledState(_address).setArtistProfitsSharing(_addresses, _rates);
  }

  modifier onlyDeployer {
    require(authorizedDeployers[msg.sender], "Only authorized adresses can perform this action");
    _;
  }
}
...
contract PrizePoolManager is Ownable, IERC1155Receiver {
  ...
  /// @dev Contains every nft available as a prize, anything above the index 'prizePoolSize - 1' is invalid
  mapping(uint => uint) internal prizePool;
  /// @notice The number prizes remaining
  uint public prizePoolSize;
  mapping(address => mapping(uint => uint)) internal collectionToTokenIdToIndex;

  EnumerableSet.AddressSet internal collectionsSet;
  ...
  function registerNft(address _collection, uint _id, uint _amount) private {
    ...
    require(EnumerableSet.contains(collectionsSet, _collection) || EnumerableSet.length(collectionsSet) == 0, "Gachapon only support one collection");

    EnumerableSet.add(collectionsSet, _collection);
    if (prizePoolSize == 0 || nfts[collectionToTokenIdToIndex[_collection][_id]].id != _id) {
      collectionToTokenIdToIndex[_collection][_id] = nftsAmount;
      nfts[nftsAmount] = Nft(_collection, _id, _amount);
      nftsAmount++;
    } else {
      nfts[collectionToTokenIdToIndex[_collection][_id]].amount += _amount;
    }

    for (uint _i = 0; _i < _amount; _i++) {
      prizePool[_i + prizePoolSize] = collectionToTokenIdToIndex[_collection][_id];
    }
    
    prizePoolSize += _amount;
  }
  ...
  function removeNft(address _collection, uint _id, uint _amount) external onlyOwner {
    ...
    require(EnumerableSet.contains(collectionsSet, _collection), "This collection is not in the gachapon");
    require(prizePoolSize > 0 && nfts[collectionToTokenIdToIndex[_collection][_id]].id == _id, "This token ID is not in the gachapon");
    require(nfts[collectionToTokenIdToIndex[_collection][_id]].amount >= _amount , "There is not enough of this nft in the gachapon");
...
/// @title Interface exposing the gachapon methods that are only callable from the GachaponFactory
/// @author KirienzoEth for DokiDoki
...
/// @title Interface implemented by all gachapons created by the GachaponFactory
/// @author KirienzoEth for DokiDoki
interface IGachapon {
  ...
  enum RoundStatus { Undefined, Pending, Unclaimed, Completed }
  struct Round {
  ...
  /// @notice Get the prizes a player obtained in a round
  function getRoundPrizes(bytes32 _requestId) external view returns (PrizePoolManager.Nft[10] memory);
  ...
  event RoundStarted(bytes32 indexed _requestId, address indexed player, uint8 _times);
  /// @dev Oracle answered and the drawn prizes were stored, numbers in _prizes are indexes of the variable 'nfts'
  event RoundCompleted(bytes32 indexed _requestId, address indexed player, uint8 _times, uint256[10] _prizes);
...
interface IGachaponCreator {
  ...
  function createGachapon(string memory _machineTitle, address _currency) external returns (address);
}
