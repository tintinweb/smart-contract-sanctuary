...
contract MarginTrading {
    ...
    function createPositionandTrade(address tokenAddress, address targetToken, uint256 amount) external {
        Deposit storage depositInfo = deposits[msg.sender][tokenAddress];
        require(depositInfo.loanAmount >= amount, "Create Position call : Insufficient Deposit");
        ...
        depositInfo.loanAmount = depositInfo.loanAmount.sub(amount);
    ...
    function withdraw(address tokenAddress, uint256 amount) external payable {
        ...
        require(userbalances[msg.sender][tokenAddress] >= 0, "Withdraw call : Insufficient Deposit");
        accureInterest(tokenAddress);
        ...
        if(amount >= userbalances[msg.sender][tokenAddress]) {
            depositInfo.interestAccumulated = 0;
            withdrawAmount = userbalances[msg.sender][tokenAddress].sub(_interest);
        }
        else {
    ...
    function checkInterestAccumulated(address userAddress, address tokenAddress) public view returns(uint256) {
        ...
        uint256 interval = (block.number.sub(depositInfo.timestamp)).div(10);
        uint interest = depositInfo.interestAccumulated.add(loanAmount.mul(interval.mul(interestPerBlock).div(100)).div(1E18));
        return interest;
    }
}