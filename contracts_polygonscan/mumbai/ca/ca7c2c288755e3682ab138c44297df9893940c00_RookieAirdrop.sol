...
contract RookieAirdrop is Ownable, ERC721 {
    

    string baseURI = "https://gateway.pinata.cloud/ipfs/QmdQS2XfdK4jT26aF1hDajFbbvbWSmWYpcKi2ZybW4wmoB/";
    
    constructor() ERC721("Planets", "RoRooPlanets") {
        setBaseURI(baseURI);
    }
    ...
    function airdropPlanets(address [] memory rookieOwner, uint amount) public onlyOwner {
        require(rookieOwner.length == amount);
        for(uint256 i = 0; i < rookieOwner.length; i++)
        {
            _safeMint(rookieOwner[i], id);
            id = id + 1;
        }
