...
contract DrawHelper {
    CanvasManager public mCanvas;
    WMATIC public mWmatic;
    PixMoment public mPixMoment;
    uint256 public mDrawPixelIndex;
    ...
    constructor(
        ...
        mCanvas = canvas;
        mWmatic = wmatic;
        mDrawPixelIndex = index;
        mPixMoment = pixMoment;
        wmatic.approve(address(canvas), type(uint256).max);
    }
    ...
    function drawPixel(
        ...
        mWmatic.deposit{value: msg.value}();
        mCanvas.drawPixel(mDrawPixelIndex, nftId, index, color, msg.value);
        uint256 iMomentId = mPixMoment.tokenOfOwnerByIndex(address(this), 0);
        mPixMoment.transferFrom(address(this), msg.sender, iMomentId);
    }

    function getMultiPixelPrice(uint256 nftId, uint256[] memory indexes) external view returns (uint256) {
        uint256 totalPrice = 0;

        for (uint256 i = 0; i < indexes.length; ++i) {
            totalPrice += mCanvas.getDrawPrice(nftId, indexes[i], mDrawPixelIndex);
        }
    ...
    function drawMultiPixel(
        ...
        require(indexes.length == colors.length, "wrong array length");
        mWmatic.deposit{value: msg.value}();

        for (uint256 i = 0; i < indexes.length; i++) {
            uint256 price = mCanvas.getDrawPrice(nftId, indexes[i], mDrawPixelIndex);
            mCanvas.drawPixel(mDrawPixelIndex, nftId, indexes[i], colors[i], price);
            uint256 iMomentId = mPixMoment.tokenOfOwnerByIndex(address(this), 0);
            mPixMoment.transferFrom(address(this), msg.sender, iMomentId);
        }
    }
