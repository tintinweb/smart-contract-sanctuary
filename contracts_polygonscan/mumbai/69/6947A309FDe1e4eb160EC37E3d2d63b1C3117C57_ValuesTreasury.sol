...
contract ValuesTreasury is Ownable, IValuesTreasury {
    ...
    address public sVALUES;
    uint256 public sVALUESQueue; // Delays change to sVALUES address
    ...
    constructor(
        ...
        isLiquidityToken[_VALUESDAI] = true;
        liquidityTokens.push(_VALUESDAI);
        bondCalculator[_VALUESDAI] = _bondCalculator;

        blocksNeededForQueue = _blocksNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for VALUES
        @param _amount uint
    ...
    function deposit(
        ...
        // mint VALUES needed and store amount of rewards for distribution
        send_ = value.sub(_profit);
        IERC20Mintable(VALUES).mint(msg.sender, send_);
    ...
    /**
        @notice allow approved address to burn VALUES for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw(uint256 _amount, address _token) external override {
        ...
        uint256 value = valueOfToken(_token, _amount);
        IVALUESERC20(VALUES).burnFrom(msg.sender, value);
    ...
    function incurDebt(uint256 _amount, address _token) external {
        ...
        uint256 maximumDebt = IERC20(sVALUES).balanceOf(msg.sender); // Can only borrow against sVALUES held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with VALUES
        @param _amount uint
     */
    function repayDebtWithVALUES(uint256 _amount) external {
        require(isDebtor[msg.sender], 'Not approved');

        IVALUESERC20(VALUES).burnFrom(msg.sender, _amount);

        debtorBalance[msg.sender] = debtorBalance[msg.sender].sub(_amount);
        totalDebt = totalDebt.sub(_amount);

        emit RepayDebt(msg.sender, VALUES, _amount, _amount);
    }
        ...
        override
    {
        ...
        IERC20Mintable(VALUES).mint(_recipient, _amount);
    ...
    function excessReserves() public view override returns (uint256) {
        return totalReserves.sub(IERC20(VALUES).totalSupply().sub(totalDebt));
    }
    ...
    /**
        @notice returns VALUES valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
        ...
        returns (bool)
    {
        ...
        } else if (_managing == MANAGING.SVALUES) {
            // 9
            sVALUESQueue = block.number.add(blocksNeededForQueue);
        } else return false;
