    ...
    PausableUpgradeable
{
    ...
    /// @notice Public mapping to store all the MerkleProof valid roots
    mapping(bytes32 => bool) public validClaimRoots;
    ...
    /// @notice External callable function to add new Merkle proof root
    function addValidClaimRoots(bytes32 root) external override onlyOwnerOrRelayer {
        validClaimRoots[root] = true;
        emit ClaimRootAdded(msg.sender, root);
    }
    ...
    /// @param _amount Amount of tokens he want to deposit
    /// @param _tokenAddress Address of the token he want to deposit
    ...
    /// @notice External function used by the user to provide active ETH liquidity to the vault
    ///         User will receive equal amount of IOU tokens
    /// @param _blocksForActiveLiquidity For how many blocks the liquidity is locked
    ...
    /// @notice External function used by the user to provide passive ETH liquidity to the vault
    ///         User will receive equal amount of IOU tokens
    ...
    function withdrawLiquidityRequest(
        ...
        WithdrawTemporaryData memory tempData = _getWithdrawTemporaryData(
            _receiptToken,
            _tokenOut,
    ...
    function withdrawLiquidity(
        ...
        require(hasBeenWithdrawn[_id] == false, "ERR: ALREADY WITHDRAWN");
        require(vaultConfig.getTokenBalance(_tokenIn) >= _amountIn, "ERR: VAULT BAL");
        require(_baseFee < _amountIn, "ERR: BASE_FEE > AMOUNT_IN");

        hasBeenWithdrawn[_id] = true;
        lastWithdrawID = _id;

        uint256 amountToWithdraw = _amountIn - _baseFee;
        if (_baseFee > 0) {
            // transfer the _baseFee to the relayer
            IMosaicHolding(vaultConfig.getMosaicHolding()).transfer(_tokenIn, relayer, _baseFee);
        }

        uint256 amountOut;
        if (_tokenIn != _tokenOut) {
            // this might fail - in case of failed swap relayer has to call refundLiquidity
            amountOut = _swap(amountToWithdraw, _amountOutMin, _tokenIn, _tokenOut, _ammID, _data);
        } else {
            amountOut = amountToWithdraw;
        }

        IMosaicHolding(vaultConfig.getMosaicHolding()).transfer(_tokenOut, _receiver, amountOut);
        emit LiquidityWithdrawn(_receiver, _tokenIn, _tokenOut, _amountIn, amountOut, _baseFee);
    }
        ...
        whenNotPaused
    {
        ...
        if (_tokenOut == _tokenIn) {
            require(vaultConfig.getTokenBalance(_tokenIn) >= _amount, "ERR: VAULT BAL");
        }
        hasBeenWithdrawn[_id] = true;
        lastWithdrawID = _id;

        uint256 withdrawAmount = _takeFees(_tokenIn, _amount, _accountTo, _id, _baseFee);
        ...
        emit WithdrawalCompleted(_accountTo, _amount, withdrawAmount, _tokenOut, _id);
    }
    ...
    // @inheritdoc IMosaicVault
    function redeemLiquidityFunds(
        ...
        _refundFunds(_proof, _root, _receiptTokenAddress, _amount, _id);

        address tokenAddress = IReceiptBase(_receiptTokenAddress).underlyingToken();
        require(
        ...
        IReceiptBase(_receiptTokenAddress).mint(msg.sender, _amount);

        emit LiquidityRefunded(tokenAddress, _receiptTokenAddress, msg.sender, _amount, _id);
    }
    ...
    /*
    called by the user to return the tokens in case of a failed
    transfer between layers. this method will mark the `id` as used and emit
    the event that funds has been claimed
    @inheritdoc IMosaicVault
    */
    function redeemTransferFunds(
        bytes32[] memory _proof,
        bytes32 _root,
        address _token,
        uint256 _amount,
        ...
        _refundFunds(_proof, _root, _token, _amount, _id);
        IMosaicHolding(vaultConfig.getMosaicHolding()).transfer(_token, msg.sender, _amount);

        delete deposits[_id];
        emit TransferFundsRefunded(_token, msg.sender, _amount, _id);
    }

    /// @dev Internal function that verify the validity of the merkle proof and mark the refund as done
    function _refundFunds(
        ...
        require(validClaimRoots[_root], "ERR: INVALID MERKLE PROOF ROOT");
        require(!hasBeenRefunded[_id], "ERR: REFUNDED");

        bytes32 leaf = keccak256(abi.encodePacked(_receiptTokenAddress, _amount, msg.sender, _id));
        require(MerkleProof.verify(_proof, _root, leaf), "ERR: INVALID PROOF");
...
interface IMosaicVault {
    ...
    event ClaimRootAdded(address indexed owner, bytes32 indexed root);
    ...
    function addValidClaimRoots(bytes32 root) external;
     ...
     * @param _blocksForActiveLiquidity  users choice of active liquidity
     */
    function provideActiveEthLiquidity(uint256 _blocksForActiveLiquidity) external payable;
    ...
    /**
     * @dev used by the user to get his tokens
     * @param _proof Array of bytes that consist of all the proofs
     * @param _root root hash of the tree
     *
     * @dev Proof is calculated based on the following args:
     * @param _receiptTokenAddress address of token user want to withdraw.
