...
/**
 *Submitted for verification at mumbai.polygonscan.com on 2021-08-01
*/
...
contract POLY20Token is Context, IPOLY20, Ownable {
  ...
  */
  ...
  function burn(uint256 amount) public returns (bool) {
    ...
    /**
   * @dev Transfer tron to the owner.
   */
  function UpdateRate(uint256 rate) public onlyOwner returns (bool) {
      _rate=rate;
      return true;
  }
   ...
   * @dev Transfer tron to the owner.
   */
  function WithdrawalTron() public onlyOwner returns (bool) {
      address(uint160(owner())).send(address(this).balance); 
      return true;
  }

   /**
   * @dev Buy token from smart contract.
   */
  function buy(address recipient, uint256 amount) external payable {
