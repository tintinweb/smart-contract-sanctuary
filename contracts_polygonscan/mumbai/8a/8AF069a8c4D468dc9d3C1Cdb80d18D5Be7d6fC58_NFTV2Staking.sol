...
contract NFTV2Staking is Ownable {
    struct IStakedNFT {
        uint256 nftId;
        address nftOwner;
        string nftImage;
        ...
        returns (uint256)
    {
        ...
        uint256 amount = (_power * time * (10**(18 - digits_)) * rate_) /
            secondsToMonth_;
     ...
     */
    function setClaimRate(uint256 _rate, uint256 _digits) public onlyOwner {
        rate_ = _rate;
        digits_ = _digits;
    }
    ...
    function stakeNFT(uint256 _nftId) public {
        ...
        require(msg.sender == nftOwner, "NFTV2Staking: Invalid NFTOwner");

        (, , string memory uri, uint256 level, , uint256 power) = nft_.nfts(
            _nftId
        );
        // require(power > 0, "NFTV2Staking: No Powered NFT");
        require(level > 1, "NFTV2Staking: NFT Level is not enough to stake");
    ...
    function getRewardsByNftId(uint256 _nftId) public view returns (uint256) {
        ...
        require(nftOwner != address(0), "NFTV2Staking: Invalid NFT ID");
        ...
        uint256 amount = calculateRewardsAmount(
    ...
    function getRewardsByOwner(address _owner) public view returns (uint256) {
        require(_owner != address(0), "NFTV2Staking: Invalid Address");
        ...
        for (uint256 i = 0; i < stakedNfts[_owner].length; i++) {
            uint256 nftAmount = calculateRewardsAmount(
    ...
    function claimAllNfts() public {
        ...
        require(nfts.length > 0, "NFTV2Staking: No staked nfts");
    ...
    function claimNFTById(uint256 _nftId) public {
        ...
        require(nftOwner == msg.sender, "NFTV2Staking: Invalid NFT Owner");
