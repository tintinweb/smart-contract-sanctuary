...
contract GenericContestSelector is Ownable, VRFConsumerBase {
    ...
    function setTicketsNumber(uint256 contestId, uint256 number) public onlyOwner contestExists(contestId) contestPerformed(contestId) {
        Contest storage contest = contests[contestId - 1];
    ...
    function setParticipantsListLink(uint256 contestId, string memory link) public onlyOwner contestExists(contestId) contestPerformed(contestId) {
        Contest storage contest = contests[contestId - 1];
    ...
    function setParticipantsListSha256(uint256 contestId, string memory sha256Hash) public onlyOwner contestExists(contestId) contestPerformed(contestId) {
        Contest storage contest = contests[contestId - 1];
    ...
    function getContestWinnersInRange(uint256 contestId, uint256 from, uint256 to) external view contestExists(contestId) returns(uint256[] memory)  {
        require(from < to, "Incorrect range");
...
contract GenericContestSelectorTest is GenericContestSelector {
    ...
    function getRandomNumber(uint256 contestId) public override contestExists(contestId) returns(bytes32) {
        contests[contestId - 1].randomResult = randomResult;
        stepsBeforeContestSelection[contestId].add(1);
        return bytes32("Test");
    }
}
