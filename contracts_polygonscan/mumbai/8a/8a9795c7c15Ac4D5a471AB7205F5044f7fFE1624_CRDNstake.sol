...
contract CRDNstake{
    ...
    function unstake(uint256 index) public{
        ...
        require(stakersRecord[msg.sender][index].withdrawtime > block.timestamp , "cannot unstake only withdraw available after period");
        ...
        totalUnStakedToken += stakersRecord[msg.sender][index].amount;
        Stakers[msg.sender].totalUnStakedTokenUser += stakersRecord[msg.sender][index].amount;
    ...
    function withdraw(uint256 index) public{
        require(!stakersRecord[msg.sender][index].withdrawan,"can't unstake already withdrawan");
        require(!stakersRecord[msg.sender][index].unstaked,"already unstaked" );
        require(stakersRecord[msg.sender][index].withdrawtime <= block.timestamp , "cannot withdraw only unstake available before period");
        ...
        totalClaimedRewardToken += stakersRecord[msg.sender][index].reward;
    ...
    function SetminimumStakeToken(uint256 value) external onlyowner {
        minimumStakeToken = value ;
    }
