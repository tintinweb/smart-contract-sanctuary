...
contract Dice is Game {
    ...
    mapping(bytes32 => DiceBet) private _diceBets;
    ...
    {
        bytes32[] memory userBetsIds = _bets[user];
    ...
    VRFConsumerBase
{
    ...
    mapping(address => bytes32[]) internal _bets;
    ...
    event BetPayoutTransferFail(bytes32 id, uint256 amount, string reason);
        ...
        returns (Bet memory)
    {
        ...
        if (isNativeToken ? msg.value <= tokenMinBetTxValue : msg.value < tokenMinBetTxValue) {
            revert UnderMinBetTxValue(msg.value, tokenMinBetTxValue);
        }
        uint256 betAmount = isNativeToken
            ? msg.value - tokenMinBetTxValue
            : tokenAmount;
