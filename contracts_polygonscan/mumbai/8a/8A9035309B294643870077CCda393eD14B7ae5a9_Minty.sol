...
contract Minty is ERC721, ERC721Enumerable, Ownable {
    ...
    function availableStreetCred(bytes32 project) public view returns (uint) {
        require(_availableStreetCred[project] != 0, "Minty: Unknown project");
    ...
    function createProject(address recipient, bytes32 project, uint streetCred, string memory cid) public onlyOwner returns (uint256) {
        require(_availableStreetCred[project] == 0, "Minty: Project already exists");
        require(streetCred > 0, "Minty: Project Street Cred must be > 0");

        _availableStreetCred[project] = int(streetCred);
        return _mint(recipient, project, 0, cid); // mint first one with Street Cred 0
    }

    function mint(address recipient, bytes32 project, uint streetCred, string memory cid) public onlyOwner returns (uint256) {
        require(streetCred > 0, "Minty: streetCred must be > 0");
    ...
    function _mint(address recipient, bytes32 project, uint streetCred, string memory cid) internal returns (uint256) {
        require(_availableStreetCred[project] != 0, "Minty: Unknown project");
        require(int(streetCred) <= _availableStreetCred[project], "Minty: StreetCred exceeds availableStreetCred"); // availableStreetCred cannot be 0
        ...
        _availableStreetCred[project] -= int(streetCred);
        if (_availableStreetCred[project] == 0) _availableStreetCred[project] = -1; // remember it existed and is done
