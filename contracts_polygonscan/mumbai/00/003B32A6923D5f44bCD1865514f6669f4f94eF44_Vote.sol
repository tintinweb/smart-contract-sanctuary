...
contract Vote is AccessControl {
  ...
  function voteAmplification(uint256 amount, uint256 timestamp)
    internal
    view
    returns (uint256)
  {
    uint256 daysSince = (block.timestamp - timestamp) / 60 / 60 / 24;
    return amount + (amount * (votingPeriod - daysSince)) / 100;
  }
  ...
  //     if (recipientVotes[month][voteId[voteIdKeys[i]].to] == 0) {
  //       recipients.push(voteId[voteIdKeys[i]].to);
  //     }
  //     recipientVotes[month][voteId[voteIdKeys[i]].to] += voteAmplification(
  //       voteId[voteIdKeys[i]].amount,
  //       voteId[voteIdKeys[i]].timestamp
  //     );
  //     total += voteAmplification(
  //       voteId[voteIdKeys[i]].amount,
  //       voteId[voteIdKeys[i]].timestamp
  //     );
  //   }
