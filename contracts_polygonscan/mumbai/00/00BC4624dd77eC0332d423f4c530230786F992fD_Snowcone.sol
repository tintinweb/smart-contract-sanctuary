...
contract Snowcone is ERC721URIStorage, Ownable, RoyaltiesV2Impl {
    ...
    function mint(address _to, string memory tokenIndivURL) public onlyOwner returns(string memory) {
      /* Increment Token ID */
      ...
      _setTokenURI(_tokenIdTracker.current(), tokenIndivURL);

      /* Emit PermanentURI  */
      emit PermanentURI(tokenIndivURL, _tokenIdTracker.current());

      return tokenIndivURL;
    }
    ...
    function tokenURI(uint256 tokenId, string memory uri) public view virtual returns (string memory) {
