...
import "./AaveConfig.sol";
import "./AaveStrategy.sol";

contract AaveStrategyFactory is IStrategyFactory, Ownable {
    AaveConfig public aaveConfig;

    constructor(AaveConfig _aaveConfig) public {
        aaveConfig = _aaveConfig;
    }

    function newStrategy() external override returns (IStrategy) {
        return new AaveStrategy(aaveConfig, msg.sender);
    }

    function setAaveConfig(address _aaveConfig) external onlyOwner {
        aaveConfig = AaveConfig(_aaveConfig);
    }
}
...
contract AaveStrategy is IStrategy {
    ...
    constructor(AaveConfig _aaveConfig, address _owner) public {
        lendingPool = _aaveConfig.lendingPool();
        aaveConfig = _aaveConfig;
        owner = _owner;
    }
