...
contract Equipments is Ownable, Operatable {
    using UintSet for Set;
    event EquipmentAdded(uint256 nftId, uint256 equipment);
    event EquipmentRemoved(uint256 nftId, uint256 equipment);
    // nftID => equipments

    mapping(uint256 => Set) private characterToEquipments;

    function addEquipment(uint256 _nftId, uint256 _equipment)
        external
        onlyOperator
    {
        characterToEquipments[_nftId].add(_equipment);
        emit EquipmentAdded(_nftId, _equipment);
    }

    function removeEquipment(uint256 _nftId, uint256 _equipment)
        external
        onlyOperator
    {
        characterToEquipments[_nftId].remove(_equipment);
        emit EquipmentRemoved(_nftId, _equipment);
    }

    function getEquipments(uint256 _nftId)
        external
        view
        onlyOperator
        returns (uint256[] memory)
    {
        return characterToEquipments[_nftId].getArray();
    }
}
