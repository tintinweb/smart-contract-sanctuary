...
contract RideUpgradeable is IRideUpgradeable, OwnableUpgradeable, OpenseaERC721Upgradeable {
    ...
    function initialize(
		...
		__OpenseaERC721_init_unchained(_name, _symbol, "RideUpgradeable", "1", address(0));
        _uri = _rideUri;
        capacity = _capacity;
        _mintPrice = _price;
     ...
     */
    function setOpenseaProxyRegistry(address _openseaProxyRegistryAddress) onlyOwner external {
    ...
    */
    ...
    function doAirdrop(address[] memory accounts) external onlyAdmin() {
    ...
    */
    ...
    /**
     * @dev Returns colors array of the NFT at index.
     */
    function tokenColorByIndex(uint256 index) external view returns (bytes4[] memory color) {
        color = _colors[index];
        if (color.length == 0) {
            color = defaultColor;
        }
     ...
     */
    function changeColor(uint256 tokenId, bytes4[] calldata color) external {
        require(colorChangeable == true, "Ride: disabled to change name");
        require(_msgSender() == ownerOf(tokenId), "Ride: caller is not the token owner");
        require(color.length == defaultColor.length, "color length mismatch");
...
contract OpenseaERC721Upgradeable is ERC721Upgradeable, EIP712MetaTxUpgradeable {
    ...
    function __OpenseaERC721_init_unchained(
        ...
        __EIP712MetaTx_init_unchained(_contractName, _contractVersion);
