    ...
    CarbonOffsetBatchesStorage
{
    ...
    function initialize(address _contractRegistry) public virtual initializer {
        ...
        __ERC721_init_unchained(
            'Toucan Protocol: Carbon Offset Project Vintage Batches',
            'TOUCAN-CVB'
        );
        __Ownable_init_unchained();
        __Pausable_init_unchained();
    ...
    /// @dev Set verifier who can confirm BatchNFTs
    function addVerifier(address _verifier) external virtual onlyOwner {
        verifiers[_verifier] = true;
    }

    function removeVerifier(address _verifier) external virtual onlyOwner {
    ...
    Modifiers
{
        ...
        whenNotPaused
    {
        ...
        /// @dev alternative flow initializing separately via low-level `call`
        /// @TODO remove after review
        // (bool success, bytes memory returnData) = address(proxyTCO2).call(payload);
        // require(success);

        // Register deployed pERC20 in ToucanContractRegistry
        ...
        emit TokenCreated(address(proxyTCO2));
    }
