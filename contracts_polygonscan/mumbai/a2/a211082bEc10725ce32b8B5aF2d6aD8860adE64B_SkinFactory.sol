...
import { EIP712MetaTransaction } from "./EIP712/EIP712MetaTransaction.sol";
    ...
    EIP712MetaTransaction
{
    ...
    IProxyRegistry private _proxyRegistry;
    ...
    modifier onlyRoleOrProxy(bytes32 role) {
        _checkRoleOrProxy(role, _msgSender());
        _;
    }

    /** returns the uniform resource identifier (URI) for the contract. */
    function contractURI() public view virtual returns (string memory) {
        return _contractURI;
    }

    /** sets the uniform resource identifier (URI) for the contract. */
    ...
    /** sets base URI for computing {tokenURI} */
    function setBaseURI(string calldata baseTokenURI) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _baseTokenURI = baseTokenURI;
    }

    /** returns the principal account if it has `role`. */
    function _auth(bytes32 role, address account) internal view returns (address) {
        if (hasRole(role, account)) return account;
        for (uint256 i = 0; i < getRoleMemberCount(role); i++) {
            address member = getRoleMember(role, i);
            if (_proxyRegistry.proxies(member) == account) return member;
        }
        return address(0);
    }

    /** revert with a standard message if `account` or principal is missing `role`. */
    function _checkRoleOrProxy(bytes32 role, address account) internal view {
        require(_auth(role, account) != address(0), _getDenyReason(role, account));
    }

    /**
     * returns the revert reason when access is denied.
     *
     * the format of the revert reason is given by the following regular expression:
     *
     *  /^AccessControl: account (0x[0-9a-f]{40}) is missing role (0x[0-9a-f]{64})$/
     */
    function _getDenyReason(bytes32 role, address account) internal pure returns (string memory) {
        return
            string(
                abi.encodePacked(
                    "AccessControl: account ",
                    uint160(account).toHexString(20),
                    " is missing role ",
    ...
    function isApprovedForAll(address owner, address operator) public view virtual override returns (bool) {
        return super.isApprovedForAll(owner, operator) || _proxyRegistry.proxies(owner) == operator;
    }

    /**
     * _msgSender() is used through other contracts such as the ERC721 one. it is overridable and since
     * we want to support meta transactions we need to make sure that the logic applied by the msgSender() function
...
import { IFactoryERC721 } from "../interfaces/IFactoryERC721.sol";
import { IProxyRegistry } from "../interfaces/IProxyRegistry.sol";

abstract contract FactoryERC721 is ERC721Tradable, IFactoryERC721 {
    IERC721 public token;

    constructor(
        string memory saleName,
        string memory saleSymbol,
        string memory saleContractURI,
        string memory saleOptionsURI,
        IERC721 saleToken,
        IProxyRegistry proxyRegistry
    ) ERC721Tradable(saleName, saleSymbol, saleContractURI, saleOptionsURI, proxyRegistry) {
        token = saleToken;
    }

    function mint(uint256 optionId, address toAddress) public virtual;

    /** @inheritdoc IFactoryERC721*/
    ...
    /** mint original token */
    function _transfer(address, address to, uint256 optionId) internal virtual override {
        mint(optionId, to);
    }

    /** @inheritdoc IERC721*/
    function isApprovedForAll(address owner, address operator) public view virtual override returns (bool) {
        return super.isApprovedForAll(owner, operator) || _auth(DEFAULT_ADMIN_ROLE, operator) != address(0);
    }

    /** @inheritdoc ERC721Burnable*/
    function burn(uint256) public virtual override {
        revert("FactoryERC721: option can't be burned");
    }

    /** @inheritdoc IERC721Metadata*/
    function name() public view virtual override(ERC721, IFactoryERC721) returns (string memory) {
        return super.name();
    }

    /** @inheritdoc IERC721Metadata*/
    function symbol() public view virtual override(ERC721, IFactoryERC721) returns (string memory) {
        return super.symbol();
    }

    /** @inheritdoc IERC721Metadata*/
    function tokenURI(uint256 optionId) public view virtual override(ERC721, IFactoryERC721) returns (string memory) {
        return super.tokenURI(optionId);
    }
}
...
interface IProxyRegistry {
    ...
    function setProxy(address account, address accountProxy) external;
}
...
contract Skin is ERC721Tradable {
    using Counters for Counters.Counter;

    address public core;
    mapping(uint256 => string) public names;

    constructor(address core_, IProxyRegistry proxyRegistry)
        ERC721Tradable(
            "ZED Racehorse Skin",
            "ZEDSKIN",
            "https://api.zed.run/api/v1/skins/contract",
            "https://api.zed.run/api/v1/skins/metadata/",
            proxyRegistry
        )
    {
        core = core_;
    ...
    function isApprovedForAll(address owner, address operator) public view virtual override returns (bool) {
        return operator == core || super.isApprovedForAll(owner, operator);
    }

    /**
     * creates a new token for `to` with name `skinName`. its token ID will be automatically
     ...
     */
    function mint(address to, string memory skinName) public virtual onlyRole(MINTER_ROLE) {
        uint256 tokenId = _tokenIdTracker.current();
        names[tokenId] = skinName;
        _mint(to, tokenId);
...
import { IProxyRegistry } from "../interfaces/IProxyRegistry.sol";
import { ERC721Tradable } from "../base/ERC721Tradable.sol";
import { FactoryERC721, IFactoryERC721 } from "../base/FactoryERC721.sol";
import { Skin } from "./Skin.sol";

contract SkinFactory is FactoryERC721 {
    struct Sale {
        string name;
        uint256 supply;
    }
    ...
    constructor(Skin skin, IProxyRegistry proxyRegistry)
        FactoryERC721(
            "ZED Skin Sale",
            "ZEDSKINsale",
            "https://api.zed.run/api/v1/skins/sales-contract",
            "https://api.zed.run/api/v1/skins/sales/",
            skin,
            proxyRegistry
        )
    {}
    ...
    /**
     * creates a new sale with name `saleName` and `supply` units. its option ID will be automatically
     * assigned (and available on the emitted {IERC721-Transfer} event), and the option
     * URI autogenerated based on the base URI passed at construction.
     *
     * see {ERC721-_mint}.
     *
     ...
     * - the caller must have the `MINTER_ROLE`.
     */
    function createSale(string memory saleName, uint256 supply) external {
        address minter = _auth(MINTER_ROLE, _msgSender());
        require(minter != address(0), _getDenyReason(MINTER_ROLE, _msgSender()));

        uint256 optionId = totalSupply();
        sales[optionId] = Sale(saleName, supply);
        _mint(minter, optionId);
    }

    /** @inheritdoc IFactoryERC721*/
    function mint(uint256 optionId, address toAddress) public override onlyRoleOrProxy(MINTER_ROLE) {
        require(!paused(), "SkinFactory: mint while paused");
        require(canMint(optionId), "SkinFactory: option is not available");

        sales[optionId].supply--;
        Skin(address(token)).mint(toAddress, sales[optionId].name);
    }

    /** @inheritdoc IFactoryERC721*/
    function canMint(uint256 optionId) public view override returns (bool) {
        return sales[optionId].supply > 0;
    }
}