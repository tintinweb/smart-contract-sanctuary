...
contract Tx8Swap is ReentrancyGuard, Ownable {
   ...
   * @param amountTX8 count of liquidity being added
   */
   ...
   * @param amountTX8 count of liquidity being removed
   */
    ...
    nonReentrant()
  {
    ...
    if (usdt2tx8) {
      amountTX8 = amount.mul(priceUSDTX8);
      
      require(usdtToken.allowance(msg.sender, address(this)) >= amount, "Spender is not allowed");
      require(usdtToken.balanceOf(msg.sender) >= amount, "Insufficient USDT");
      ...
      amountUSDT = amount.div(priceUSDTX8);

      require(tx8Token.allowance(msg.sender, address(this)) >= amount, "Spender is not allowed");
      require(tx8Token.balanceOf(msg.sender) >= amount, "Insufficient TX8");

      _addLiquidity(amount, address(usdtToken), address(tx8Token));
      _removeLiquidity(amountUSDT, address(tx8Token), address(usdtToken));
