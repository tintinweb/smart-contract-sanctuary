...
contract Asset is AssetStorage, Risk {
    ...
    constructor() ERC721("AmplifyAsset", "AAT") Risk(msg.sender) {}
...
contract Loan is LoanStorage, PoolModifier {
    ...
    event Payed(uint256 indexed amount);
    event Closed();
    ...
    function _calculateLoanAmount(uint256 tokenId) internal view returns (uint256) {
        ...
        uint256 advancedRate = nftFactory.getAdvancedRiskRate(nftRating);

        return nftValue * advancedRate / 100;
    }
...
abstract contract Risk {
    struct RiskItem {
        uint256 interestRate;
        uint256 advancedRate;
    }
    ...
    function insertRiskItem(string memory rating, uint256 interestRate, uint256 advancedRate) public onlyOwner {
        riskData[rating] = RiskItem(interestRate, advancedRate);
    }
    ...
    function getRiskInterestRate(string memory rating) external view returns (uint256 _interestRate) {
        return riskData[rating].interestRate;
    } 

    function getAdvancedRiskRate(string memory rating) external view returns (uint256 _advancedRate) {
        return riskData[rating].advancedRate;
    }
}
