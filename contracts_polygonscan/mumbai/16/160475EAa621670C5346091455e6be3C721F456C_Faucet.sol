...
contract Faucet{
    ...
    constructor(ERC20 _respct ) public {
            respct = _respct;
        }
        ...
        function extractToken() public payable{
            require(respct.balanceOf(msg.sender)<=5*10**18);
            uint256 respctBal = respct.balanceOf(address(this));
            if (respctBal < 20*10**18) {
                respct.transfer(msg.sender, respctBal);
            } else {
                respct.transfer(msg.sender, 20*10**18);
            }
        }
