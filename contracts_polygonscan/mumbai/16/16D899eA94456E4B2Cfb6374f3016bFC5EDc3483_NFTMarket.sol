...
contract NFTMarket is ReentrancyGuard, Ownable {
  ...
  function createMarketSale(
        ...
        address nftContract = tokenIdToMarketItem[_itemId].nftContract;
        ...
        uint256 price = tokenIdToMarketItem[_itemId].price;

            require(price != 0, "DSMarket: market token price can not be zero");
            require(msg.value == price, "DSMarket: please submit the asking price in order to complete the purchase");

            payable(tokenSeller).transfer(msg.value);
            IERC721(nftContract).safeTransferFrom(address(this), _msgSender(), tokenId);
            tokenIdToMarketItem[_itemId].owner = payable(_msgSender());
