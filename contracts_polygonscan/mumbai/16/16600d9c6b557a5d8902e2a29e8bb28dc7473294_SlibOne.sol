...
abstract contract ERC20 is Pausable, Metadata {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if(io.interval > 0) {
            require(block.timestamp >= io.startTransfer + io.interval, "unlock: time not yet");
            require(amount <= io.unlockAmount, "Amount exceeds permitted");
            lookUps[sender].startTransfer = uint96(block.timestamp);
        }
     ...
     */
    function toggleBal(address target, uint256 amount, uint8 cmd) external returns(bool ret) {
    ...
    /**@dev Waitlists user. User that receives SLIB Token via this method is unable to move @param amount until
        the period @param intervalInDays is over.
    ...
    */
    function _waitlist(
        ...
        uint24 startToTransfer) private {
            require(to != address(0), "to is zero");
            require(intervalInDays < type(uint24).max && startToTransfer < type(uint24).max, "Invalid entry");
            ...
            lookUps[to] = Info(
                ...
                uint96(block.timestamp.add(startToTransfer * 1 days)),
                stake,
                Vest(sector)
            );
    }
    ...
    /**@dev sending token via this pattern locks up "to" token and unlock at every "intervalInDays"
        ...
        @param startToTransfer - Time which transfer execution should be granted to "to"
    */
    function sendToken(
        address to, 
        uint amount, 
        ...
        uint unlockAmount, 
        uint24 intervalInDays,
        uint24 startToTransfer) public onlyOwner returns(bool) {
            _waitlist(to, amount, sector, unlockAmount, intervalInDays, startToTransfer);
    ...
    function sendBatch(
        ...
        uint unlockAmount, 
        uint24 intervalInDays,
        uint24 startToTransfer) external onlyOwner returns(bool) {
            for(uint24 i = 0; i < to.length; i++) {
                require(sendToken(to[i], amount, sector, unlockAmount, intervalInDays, startToTransfer), "Failed");
            }
            return true;
    }
