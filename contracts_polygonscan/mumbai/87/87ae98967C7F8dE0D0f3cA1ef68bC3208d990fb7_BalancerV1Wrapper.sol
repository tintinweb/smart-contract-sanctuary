...
/**
 * @summary: Contract that allows you to lock and transfer an NFT
 * @author: Composable Finance - @gcosmintech
 */
pragma solidity ^0.6.8;
...
ReentrancyGuardUpgradeable
{
    ...
    // @dev source network id => source nft address => remote mosaic nft address
    mapping(uint256 => mapping(address => address)) public remoteNftAddress;

    /// source chain Id => source nft contract address => source nft Id => {remote nft contract, mapped id}
    mapping(uint256 => mapping(address => mapping(uint256 => NftInfo))) public remoteNftInfo;

    /// source chain Id => mapped nft contract address => mapped nft Id => {source nft contract, source id}
    mapping(uint256 => mapping(address => mapping(uint256 => NftInfo))) public sourceNftInfo;
    ...
    event NftContractCreated(
        ...
        address indexed remoteNftContract,
    ...
    validAddress(remoteDestinationOwner)
    whenNotPausedNetwork(remoteNetworkID)
    {
        require(
        ...
        require(
            ERC721Upgradeable(nftContract).ownerOf(nftId) == msg.sender,
            "Not authorized to lock this NFT"
        );
        require(transferStatus[nftContract][nftId] == false, "NFT already locked");

        require(feeAmounts[remoteNetworkID][feeToken] > 0, "fee token not accepted");

        ERC721Upgradeable(nftContract).safeTransferFrom(msg.sender, address(this), nftId);
        lastTransfer[msg.sender] = block.timestamp;
        ...
        //mapped from a previous tx, which means we should just release it now
        if (sourceNftInfo[remoteNetworkID][nftContract][nftId].contractAddress != address(0)) {
            //the relayer will read this event and call releaseSeal on the source layer
            ...
            // the relayer will read this event and call summonNFT on the remote layer
    ...
    function _takeFees(
        ...
        uint256 fee = feeAmounts[remoteNetworkId][feeToken];
    ...
    function releaseSeal(
        ...
        require(hasBeenReleased[id] == false, "Already released");
        require(transferStatus[nftContract][nftId] == true, "The NFT is not locked");
        ...
        ERC721Upgradeable(nftContract).safeTransferFrom(address(this), nftOwner, nftId);
    ...
    onlyOwner
    whenNotPaused
    validAddress(nftOwner)
    validAddress(nftContract)
    {
        require(hasBeenSummoned[id] == false, "Already summoned");
        ...
        if (remoteNftAddress[sourceNetworkId][nftContract] == address(0)) {

            // deploy new nft contract
            mosaicNft = new MosaicNFT(nftContract, sourceNetworkId);
            remoteNftAddress[sourceNetworkId][nftContract] = address(mosaicNft);
            uint256 chainId;
            assembly {
                chainId := chainid()
            }
            ...
            mosaicNft = MosaicNFT(remoteNftAddress[sourceNetworkId][nftContract]);
        }

        remoteNftInfo[sourceNetworkId][nftContract][nftId] = NftInfo(
        {contractAddress : address(mosaicNft), id : mosaicNft.idTracker()}
        );
        sourceNftInfo[sourceNetworkId][address(mosaicNft)][mosaicNft.idTracker()] = NftInfo(
        {contractAddress : nftContract, id : nftId}
        );

        mosaicNft.mintNewNFT(nftOwner, nftUri);
    ...
    modifier notAlreadySummoned(bytes32 id) {
        require(hasBeenSummoned[id] == false, "Already summoned");
        _;
    }
...
contract MosaicNFT is Ownable, ERC721Pausable {
    uint256 public idTracker;
    address public underlyingAddress;
    uint256 public networkId;

    constructor(address _underlyingAddress, uint256 _networkId)
    public
    ERC721(
        string(abi.encodePacked("mNFT-", IERC721Metadata(_underlyingAddress).name())),
        string(abi.encodePacked("mNFT-", IERC721Metadata(_underlyingAddress).symbol()))) {
