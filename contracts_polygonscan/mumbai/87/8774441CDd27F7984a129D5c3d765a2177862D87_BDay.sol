...
contract BDay is Ownable {
    ...
    function addGift(uint _id, string memory _pass, uint256 _am, address _claimer, string memory _data) public payable onlyOwner  {
        
        gifts.push(Gift(_id, _am, _pass, _claimer, _data));

    }
    
    function getGift(uint _id) public onlyOwner view returns(Gift memory){
    ...
    function claimGift(string memory _pass)  public payable returns(string memory) {
        for(uint i=0; i < gifts.length; i++) {
