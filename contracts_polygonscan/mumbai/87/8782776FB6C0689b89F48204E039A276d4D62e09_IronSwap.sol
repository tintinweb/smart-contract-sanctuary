...
library IronSwapLib {
    ...
    function addLiquidity(
        ...
        require(amounts.length == nCoins, "amounts size invalid");
    ...
    function removeLiquidityOneToken(
        ...
        require(index < numTokens, "Token not found");
    ...
    function getAdminBalance(SwapStorage storage self, uint256 index) external view returns (uint256) {
        require(index < self.pooledTokens.length, "> range");
    ...
    function get_y(
        ...
        uint256 nCoins = self.pooledTokens.length;
        require(inIndex < nCoins && outIndex < nCoins, "out of range");
        ...
        revert("Approximation not converge");
    }
    ...
    function _calculateRemoveLiquidityOneToken(
        ...
        uint256 fee = dy - ((xp[index] - newY) / self.tokenMultipliers[index]);
        return (dy, fee);
    }
...
contract IronSwap is OwnerPausable, ReentrancyGuard, Initializable, IIronSwap {
    ...
    function initialize(
        ...
        for (uint256 i = 0; i < numberOfCoins; i++) {
            ...
            require(_decimals[i] <= IronSwapLib.POOL_TOKEN_COMMON_DECIMALS, "decimals invalid");
        ...
        swapStorage.initialA = A_ * IronSwapLib.A_PRECISION;
        swapStorage.futureA = A_ * IronSwapLib.A_PRECISION;
    ...
    function getTokenIndex(address token) external view override returns (uint8 index) {
        ...
        require(address(swapStorage.pooledTokens[index]) == token, "notFound");
    }
