...
contract BuddyCore is IBuddyCore, ERC721Full, ERC721Holder {
    IERC20 public _nanoFactoryToken;
    IGoo public _goo;
    IBuddyLogic public _buddyLogic;

    uint256 public lastFounderClaim;
    uint256 public lastFounderClaimTime;

    uint256 public constant blackHole = 1;
    ...
    constructor(
        address _gooAddr,
        address _nanoFactoryAddr,
        address _buddyLogicAddr
    ) {
        _goo = IGoo(_gooAddr);
        _nanoFactoryToken = IERC20(_nanoFactoryAddr);
        _buddyLogic = IBuddyLogic(_buddyLogicAddr);
        lastFounderClaimTime = block.timestamp;
    }
    ...
    function buyGen0() external whenNotPaused {
        ...
        uint256 dna = _buddyLogic.createGen0(tokenId);
        ...
        _data[tokenId] = base;
        _goo.mint(tokenId, _buddyLogic.getStartingGoo() * 1e18);
    ...
    function replicate(
        ...
        uint256[] memory buddies;
        uint256 gooBalance = _goo.balanceOf(_tokenId);

        if (useAllBuddies) {
            buddies = getTokensByOwner();
        ...
        require(gooBalance >= cost, "You do not have enough GOO");
        ...
        BuddyLib.Buddy storage base = _data[_tokenId];
        base.repCur += 1;
        uint256 dna = _buddyLogic.repBuddy(base, dominantLock, recessiveSwap);
        BuddyLib.Buddy memory child = BuddyLib.generate(dna);
        child.parent = _tokenId;
        child.repMax = _buddyLogic.calcReps(child.gen);
        _buddyLogic.getRandom();
        child.dna = dna;
    ...
    function replicatePreview(
        ...
        BuddyLib.Buddy storage base = _data[_tokenId];
        require(base.special == 0, "Specials cannot Replicate");
        ...
        whenNotPaused
    {
        ...
        _goo.transferFrom(blackHole, _tokenId, blackHole, cost);
        _goo.burn(blackHole, (cost * _buddyLogic.getBurnPercent()) / 100);

        BuddyLib.Buddy storage base = _data[_tokenId];
        base.repCur += 1;
        uint256 dna = _buddyLogic.simpleRepBuddy(base);
        BuddyLib.Buddy memory child = BuddyLib.generate(dna);
        child.parent = _tokenId;
        child.repMax = _buddyLogic.calcReps(child.gen);
        _buddyLogic.getRandom();
        ...
        returns (uint256)
    {
        ...
        return _buddyLogic.simpleReplicatePreview(grossGPD);
    }

    function calcGGPD(uint256 _tokenId, bool fertility)
        public
        view
        returns (uint256)
    {
        BuddyLib.Buddy storage base = _data[_tokenId];
        return _buddyLogic.calcGGPD(base, fertility);
    }
    ...
    {
        BuddyLib.Buddy storage base = _data[_tokenId];
        uint256 ggpd = (_buddyLogic.calcGGPD(base, false)) * 90;
        return ggpd * 1e18;
    }
    ...
    function claimAll(uint256[] memory tokens)
        external
        whenNotPaused
        returns (uint256 total)
    {
    ...
    function apoptosis(uint256 _tokenId, uint256[] memory receivers)
        external
        onlyHolder(_tokenId)
        whenNotPaused
    {
        ...
        require(
            base.totalProduced > 100,
            "Buddy must produce at least 100 GOO"
        );
        ...
        for (uint256 i = 0; i < receivers.length; i++) {
            ...
            if (amount > rMax) {
                _goo.mint(rMax, receiver);
                amount -= rMax;
            } else {
                _goo.mint(amount, receiver);
                break;
            }
        }
    ...
    {
        BuddyLib.Buddy storage base = _data[_tokenId];
        require(base.special == 2, "Not a Founder");
        require(
            block.timestamp >= base.lastRewardTime + 1 weeks,
            "you must wait at least 1 week"
        );
        if (
            lastFounderClaim == 0 ||
            block.timestamp >= lastFounderClaimTime + 1 weeks
        ) {
            uint256 claimedAmount = (_goo.balanceOf(blackHole) *
                (100 - _buddyLogic.getBurnPercent())) / 100;
            return claimedAmount;
        }
        return lastFounderClaim;
    }
        ...
        whenNotPaused
    {
        ...
        if (time >= lastFounderClaimTime + 1 weeks) {
            lastFounderClaim = 0;
        }

        base.lastRewardTime = time;

        if (lastFounderClaim == 0) {
            lastFounderClaim = claimAmount;
            lastFounderClaimTime = time;
        }
...
interface IBuddyLogic {
    ...
    function createGen0(uint256 tokenId) external returns (uint256);
    ...
    function calcApop(uint256 totalProduced) external view returns (uint256);
    ...
    function simpleReplicatePreview(uint256 grossGPD)
        external
        view
        returns (uint256);

    function simpleRepBuddy(BuddyLib.Buddy calldata base)
        external
        returns (uint256);

    function calcReps(uint8 gen) external returns (uint8);
...

{
  ...
  "libraries": {
    "contracts/libraries/BuddyLib.sol": {
      "BuddyLib": "0x903bb7c9619ab7af01fc9efc8fb790715da6eb85"
    }
  }
}