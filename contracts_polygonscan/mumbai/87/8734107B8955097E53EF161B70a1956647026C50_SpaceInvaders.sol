...
contract SpaceInvaders is ERC721URIStorage, Ownable {
    ...
    uint256 internal constant _maxSupply = 64;

    constructor(string memory _svg1, string memory _svg2)
        ERC721("SI-T", "Space Invaders Token")
    {
        _tokenCounter = 0;
        nonce = 0;
    ...
    function mint() public payable {
        ...
        _mintableIds.pop();
        string[2] memory attrs = getAttributes(newTokenId);
        string[2] memory svg = generateSVG(newTokenId, msg.sender, attrs);
        string memory imageURI = svgToImageURI(svg[0]);
        _safeMint(msg.sender, _tokenCounter);
        _setTokenURI(
            _tokenCounter,
            formatTokenURI(imageURI, svg[1], newTokenId)
        );
        _tokenCounter += 1;
        ...
        returns (string[2] memory)
    {
        ...
        uint256 f = getIndexAt(_tokenId, 4);
        uint256 g = getIndexAt(_tokenId, 5);
    ...
    function generateSVG(
        ...
        string memory finalSvg = string(
            abi.encodePacked(
                ...
                C[0],
                "--p1: url(#p_1);--p2: url(#m-r);--p4: url(#c-p);}.SI {opacity: 0;}",
                attrs[0],
                _svgPart1
            )
        );
        ...
        finalSvg = string(
            abi.encodePacked(
                finalSvg,
                textNumber,
                '<animate attributeName="startOffset" values="0%;100%" dur="60s" repeatCount="indefinite" /></textPath></text><text class="fill-c7 responsive"><textPath href="#square" side="left" startOffset="-100%" lengthAdjust="spacing" method="align" spacing="auto"  textLength="2480">',
                textNumber,
                _svgPart2
            )
        );
            ...
            string(
                abi.encodePacked(
                    ...
                    Base64.encode(
                        bytes(
                            abi.encodePacked(
                                '{"name":"SPI-T", "description":"SPace Invaders Token - #',
                                uint2str(_tokenId),
