    ...
    RandomNumberConsumer
{
    ...
    mapping(address => uint256) public randomIndexes;
    ...
    function buyTokens(address from, uint256 quantity) private nonReentrant {
        ...
        for (uint256 i = 0; i < quantity; i++) {
            uint256 tokenId = tokens[randomIndexes[msg.sender]];
            transferredTokens[i] = tokenId;
            tokens[randomIndexes[msg.sender]] = tokens[tokens.length - 1];
            tokens.pop();

            IERC721(erc721Address).safeTransferFrom(tokensVault, from, tokenId);
        }

        randomIndexes[msg.sender] = 0;
    ...
    function activatePurchase(uint256 quantity) public {
        ...
        Purchase memory purchase = Purchase(period, quantity);
        purchases[msg.sender] = purchase;

        emit LogPurchaseParameters(period, msg.sender, quantity);
    }

    /**
     *  @notice This is a callback method which is getting called in RandomConsumerNumber.sol
     */
    function saveRandomNumber(address addr, uint256 n) internal override {
        randomIndexes[addr] = n;

        if (purchases[addr].presale) {
            buyTokensOnPresale(addr);
        } else {
            buyTokensOnSale(addr, purchases[addr].quantity);
        }
