...
user_interest_claimed: public(HashMap[address, uint256])
...
totalContractBalance: public(uint256)
...
def updateInterest():
    if block.timestamp <= self.lastRewardCalcTimeStamp:
        return
    current_atoken_balance: uint256 = ERC20(AMUSDC_contract).balanceOf(self)
...
def fxbuy(withdraw: uint256) -> bool:
    ...
    user_balance: uint256 = ERC20(self).balanceOf(msg.sender)
    if (user_balance > 0):
        self.user_interest[msg.sender] += (((user_balance / self._buyrate) / 10**2) * self.accumulatedRewardPerShare) - self.user_interest_claimed[msg.sender]
    
    self._mint(msg.sender,amount2)
    log Bugger(self.user_interest[msg.sender], 'interests') 
    raw_call(
                    self.lendingpool,
                    concat(
    ...
    self.totalContractBalance += amount
    self.user_interest_claimed[msg.sender] = ((ERC20(self).balanceOf(msg.sender) / self._buyrate) / 10**2) * self.accumulatedRewardPerShare
    log Bugger(self.accumulatedRewardPerShare, 'rewardPerShare')
...
def fxsell(withdraw: uint256) -> bool:
    self.updateInterest()
    user_balance: uint256 = ERC20(self).balanceOf(msg.sender)
    assert user_balance  >= withdraw, "not enough balance"

    self._burn(msg.sender,withdraw)

    usd_amount: uint256 = (withdraw / self._buyrate) / 10**2
    log Bugger(self.user_interest_claimed[msg.sender], 'interests claimed') 
    self.user_interest[msg.sender] += (((user_balance / self._buyrate) / 10**2) * self.accumulatedRewardPerShare) - self.user_interest_claimed[msg.sender]
    log Bugger(self.user_interest[msg.sender], 'interests') 
    interest_on_amount: uint256 = usd_amount/((user_balance / self._buyrate) / 10**2) * self.user_interest[msg.sender]
    log Bugger(interest_on_amount, 'interest on withdrawal')
    usd_amount_plus_interest: uint256 = usd_amount + (interest_on_amount / 10**24)
    ...
    self.totalContractBalance -= usd_amount
    self.user_interest_claimed[msg.sender] = ((ERC20(self).balanceOf(msg.sender) / self._buyrate) / 10**2) * self.accumulatedRewardPerShare
    self.saveAtokenBalance()
    return True
