...
contract CryptoPropheciesGame is ReentrancyGuard, Ownable {
    ...
    mapping(string => bool) private _backIds;
    ...
    function createBattle(
        ...
        string calldata _backIdentifier
    ) external nonReentrant onlyGC {
        require(!_backIds[_backIdentifier], "BackIdentifier is not unique");
    ...
    function _transferFundsToWinner(uint256 _battleId) internal {
        ...
        if (battle.winner == battle.player1) {
            IDailyPrize(ctDailyPrize).addTickets(battle.player1, battle.TCPAmount * multipliers[player1Rarity][0]);
            emit DailyPrizeTicketAdded(battle.player1, battle.TCPAmount * multipliers[player1Rarity][0], block.timestamp);

            if (battle.TCPAmount * multipliers[player2Rarity][1] != 0) {
                IDailyPrize(ctDailyPrize).addTickets(battle.player2, battle.TCPAmount * multipliers[player2Rarity][1]);
                emit DailyPrizeTicketAdded(battle.player2, battle.TCPAmount * multipliers[player2Rarity][1], block.timestamp);
            }
        }

        if (battle.winner == battle.player2) {
            IDailyPrize(ctDailyPrize).addTickets(battle.player2, battle.TCPAmount * multipliers[player2Rarity][0]);
            emit DailyPrizeTicketAdded(battle.player2, battle.TCPAmount * multipliers[player2Rarity][0], block.timestamp);

            if (battle.TCPAmount * multipliers[player1Rarity][1] != 0) {
                IDailyPrize(ctDailyPrize).addTickets(battle.player1, battle.TCPAmount * multipliers[player1Rarity][1]);
                emit DailyPrizeTicketAdded(battle.player1, battle.TCPAmount * multipliers[player1Rarity][1], block.timestamp);
            }
        }

        if (battle.winner == address(0)) {
            IDailyPrize(ctDailyPrize).addTickets(battle.player1, (battle.TCPAmount * multipliers[player1Rarity][0]) / 2);
            IDailyPrize(ctDailyPrize).addTickets(battle.player2, (battle.TCPAmount * multipliers[player2Rarity][0]) / 2);
            emit DailyPrizeTicketAdded(battle.player1, (battle.TCPAmount * multipliers[player1Rarity][0]) / 2, block.timestamp);
            emit DailyPrizeTicketAdded(battle.player2, (battle.TCPAmount * multipliers[player2Rarity][0]) / 2, block.timestamp);
        }
