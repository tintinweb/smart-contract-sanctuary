...
// ConverterV3 is Yugen Farm's left hand and kinda a wizard. He can create up YGN from pretty much anything!
...
contract ConverterV3 is Ownable, ReentrancyGuard {
    ...
    address public yugenFarm;
    uint16 public farmAllocation;
    ...
    // Can be used by the owner to update the yugen farm
    function updateYugenFarm(address _yugenFarm)
        external
        onlyOwner
        ensureNonZeroAddress(_yugenFarm)
    {
        yugenFarm = _yugenFarm;
    }
    ...
    */
        ...
        ensureValidRouter(router)
    {
        ...
        require(
            path[path.length - 1] == address(ygn),
    ...
    */
    function _allocateYGN() internal {
        ...
        _safeTransfer(
            address(ygn),
            yugenFarm,
            totalYGNAccumulated.mul(farmAllocation).div(1000)
        );
        _safeTransfer(
            address(ygn),
            ygnStaker,
