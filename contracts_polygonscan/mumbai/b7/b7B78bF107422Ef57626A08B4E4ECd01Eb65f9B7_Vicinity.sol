...
abstract contract VicinityAccess is Ownable, AccessControlEnumerable {
	...
	function renounceAdmin() public {
		require(
			msg.sender != owner(),
			"The contract owner cannot renoumce admin."
		);
		renounceRole(DEFAULT_ADMIN_ROLE, msg.sender);
	}
	
	// Minter
	function minter_role_id() public view onlyRole(DEFAULT_ADMIN_ROLE) returns (bytes32) {
		return MINTER_ROLE_NAME;
	}
	
	function grantMinter(address account) public onlyRole(DEFAULT_ADMIN_ROLE) {
		grantRole(MINTER_ROLE_NAME, account);
	}
	
	function revokeMinter(address account) public onlyRole(DEFAULT_ADMIN_ROLE) {
		revokeRole(MINTER_ROLE_NAME, account);
	}
	
	function renounceMinter() public {
		renounceRole(MINTER_ROLE_NAME, msg.sender);
	}
	
	// Airdrop
	function airdrop_role_id() public view onlyRole(DEFAULT_ADMIN_ROLE) returns (bytes32) {
		return AIRDROP_ROLE_NAME;
	}
	
	function grantAirdrop(address account) public onlyRole(DEFAULT_ADMIN_ROLE) {
		grantRole(AIRDROP_ROLE_NAME, account);
	}
	
	function revokeAirdrop(address account) public onlyRole(DEFAULT_ADMIN_ROLE) {
		revokeRole(AIRDROP_ROLE_NAME, account);
	}
	
	function renounceAirdrop() public {
		renounceRole(AIRDROP_ROLE_NAME, msg.sender);
	}
}
