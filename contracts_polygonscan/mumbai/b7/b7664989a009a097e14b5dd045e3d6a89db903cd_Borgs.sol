...
contract Borgs is ERC721Enumerable, ERC721Holder, CommonObjects, NumberUtils{
    ...
    mapping (uint8 => Layer) public _layers;
    ...
    function getBorgAttribute(string memory borgAttributeName) public view onlyOwner returns (string[] memory){
        // Get attribute
        BorgAttribute storage borgAttribute = _borgAttributes[borgAttributeName];
        
        // Create image to return
        string[] memory imagePixals = new string[](IMAGE_SIZE);
        
        // Add all the colour positions
        /*for(uint256 i = 0;i<borgAttribute.colors.length;i++){
            ...
            // Add the color to each position in array
    ...
    function createBorgAttribute(string calldata borgAttributeName, string[] calldata colors, uint256[][] calldata positions) external onlyOwner editable{
    ...
    function _combineBorgAttributes(string[] memory borgAttributeNames) public view returns (string[] memory hexPixals){
        // Init the 
