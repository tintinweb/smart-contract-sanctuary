    ...
    ERC721URIStorageUpgradeable
{
    ...
    ITransformedNFT public immutable _transformedNftContract;
    ...
    function transform (
        ...
        // get serum metadat   
        TokenMeta memory meta = tokenMeta(_serumId);
        
        //require serum to be full (2) (not used)
        require(meta.status == 2, "Serum is already used");

        /** call the contract to mint t */
        uint256 TransformedNFTId = ITransformedNFT(counterAddr).mintCollectable (
            _owner,
            _transformedUri,
            _name,
        ...
        //_setTokenStatus(_serumId, meta.status - 1);
        _setTokenURI(_serumId, meta.emptyUri); // change token uri

        return TransformedNFTId;        
    }

        function _setTokenStatus(uint256 _tokenId, uint256  _status) private {
        require(_exists(_tokenId), "ERC721Metadata: Sale set of nonexistent token");

        require(ownerOf(_tokenId) == _msgSender() || _msgSender() == owner());

        // set tokenId category metadata to be "_category"
        _tokenMeta[_tokenId].status = _status;
    }
}
