...
contract Ticket is ERC1155WithRoyalty {
  using Strings for string;
  using SafeMath for uint256;

  

  mapping (uint256 => uint256) ticketSupply;
  ...
  // Ticket sale Time;
  mapping(uint256 => uint256) openSaleTime;
  mapping(uint256 => uint256) closeSaleTime;
  ...
  constructor(string memory _name,string memory _symbol,string memory _uri,uint16 _royaltyBPS) ERC1155WithRoyalty(_uri,msg.sender,_royaltyBPS) 
  {
    name = _name;
    symbol = _symbol;
  }
  ...
  */
  function startAndclose(uint256 _id,uint256 startTime,uint256 endTime) external onlyOwner{
  ...
  event BuyTicket(address buyer, uint256 ticketId, uint256 quantity);
  ...
  */
  function buyTicket(uint256 _id,uint256 _quantity)  public payable {
    require(saleIsOpen[_id] && block.timestamp > openSaleTime[_id] && block.timestamp < closeSaleTime[_id],"This ticket sale is not open, please wait");
