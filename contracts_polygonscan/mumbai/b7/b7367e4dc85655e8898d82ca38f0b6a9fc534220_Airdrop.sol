...
contract Airdrop is AccessControlUpgradeable, PausableUpgradeable {
    ...
    uint256 private totalWhitelist;
    mapping(address => Claimed) private whitelist;
    ...
    function initialize() public initializer {
        ...
        _setupRole(WHITELIST_CONTROLLER, msg.sender);
    ...
    /**
     * @dev function to set  Spaceship NFT SC Address to this contract
     * @param spaceshipAddr the address of Spaceship NFT
     */
    ...
    /**
     * @dev function to set Mystery Capsule Address to this contract
     * @param mysteryCapsuleAddr the address of Mystery Capsule
     */
    function setMysteryCapsule(address mysteryCapsuleAddr) public onlyRole(DEFAULT_ADMIN_ROLE) {
        mysteryCapsule = MysteryCapsule(mysteryCapsuleAddr);
    }

    /**
     * @dev function to set Random Number Consumer Address to this contract
     * @param randomAddr the address of Random Number Consumer
     */
    ...
    /**
     * @dev function to add an address to whitelist
     * @param addr the address of user who need to add to whitelist
     */
    function addWhitelist(address addr) public onlyRole(WHITELIST_CONTROLLER) {
        require(!whitelist[addr].isWhitelist, "AWL");
        whitelist[addr] = Claimed(true, false, 0, 0);
        totalWhitelist += 1;
    }

    /**
     * @dev function to add array of address to whitelist
     * @param addrs array of address of users who need to add to whitelist
     */
    function addWhitelists(address[] memory addrs) public onlyRole(WHITELIST_CONTROLLER) {
    ...
    function removeWhitelist(address addr) public onlyRole(WHITELIST_CONTROLLER) {
        ...
        whitelist[addr] = Claimed(false, false, 0, 0);
        totalWhitelist -= 1;
    }

    /**
     * @dev function to remove array of address to whitelist
     * @param addrs array of address of users who need to remove from whitelist
     */
    function removeWhitelists(address[] memory addrs) public onlyRole(WHITELIST_CONTROLLER) {
        for (uint256 i = 0; i < addrs.length; i++) {
            address addr = addrs[i];

            if (whitelist[addr].isWhitelist && !whitelist[addr].claimed) {
                whitelist[addr] = Claimed(false, false, 0, 0);
                totalWhitelist -= 1;
            }
        }
    ...
    {
        Claimed memory w = whitelist[addr];
        return (w.isWhitelist, w.claimed, w.time, w.shipId);
    }
    ...
    /**
     * @dev function to check an address in whitelist
     * @param addr the address that need to check
     * @return the result in bool
     */
    ...
    /**
     * @dev function to claim airdop
     * user must be not smart contract
     * user can only claim 1 time per block
     * user can only claim when this airdrop is not paused
     * user must in whitelist
     * user must be unclaimed
     */
    function claim() public onlyNonContract oncePerBlock(msg.sender) whenNotPaused returns (uint256) {
