...
import "../polygon/SubjectChild.sol";

contract SubjectChildTunnel is FxBaseChildTunnel, SubjectTunnel {
    constructor(address _fxChild, address payable _daoAddress)
        FxBaseChildTunnel(_fxChild)
        SubjectTunnel(_daoAddress)
    {}

    SubjectChild public subjectContract;
    ...
    function _processMessageFromRoot(
        ...
        subjectContract.mintSubjectWithInfo(tokenId, ownerAddress, gene);
    ...
    function moveThroughWormhole(uint256 tokenId)
        ...
        onlyOwner(tokenId)
    {
        uint256 gene = subjectContract.geneOf(tokenId);
        bool isNotVirgin = subjectContract.isNotVirgin(tokenId);
        uint256 genomeChanges = subjectContract.genomeChanges(tokenId);
        subjectContract.burn(tokenId);
    ...
    function setSubjectContract(address payable contractAddress)
        ...
        onlyDAO
    {
        subjectContract = SubjectChild(contractAddress);
    }
}
...
// SPDX-License-Identifier: UNLICENSED
pragma solidity ^0.8.0;
import "./ISubjectsTunnel.sol";
import "../modifiers/DAOControlled.sol";

abstract contract SubjectTunnel is DAOControlled, ISubjectsTunnel {
...
import "./ISubjectChild.sol";

contract SubjectChild is ISubjectChild, SubjectWithGeneChanger {
    ...
    TunnelEnabled
{
    ...
    constructor(
        ...
        uint256 _randomizeGenomePrice,
        string memory _arweaveAssetsJSON
    ) Subject(name, symbol, baseURI, _daoAddress, _arweaveAssetsJSON) {
...
interface ISubjectChild is IERC721 {
    function mintSubjectWithInfo(
        uint256 tokenId,
        address ownerAddress,
        uint256 gene
