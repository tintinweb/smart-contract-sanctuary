...
contract Fakhama is IERC20
{
    ...
    function newInvestment_Internal(uint memberId, uint amount, bool isFromRegistration) internal
    {
        ...
        distributeROI(memberId, amount, amount*18/100, "Investment");
    ...
    function archiveROI(uint memberId, uint endId) public
    {
        ...
        require(endId<=startId+_roiRecordsLimit, "Too many calculations! Please try to archive your ROI first.");
    ...
    function getMemberROIDividendInfo(uint memberId, uint cnt) public view 
        returns (UserROIDistributionInfo[] memory ROIRecords, uint ArchivedROI, bool IsArchiveNeeded)
    {
        ...
        uint _memBalance = _balances[map_Users[memberId].Address];
        UserROIDistributionInfo[] memory _info = new UserROIDistributionInfo[](cnt);
		
		uint _end = _start+cnt;
        IsArchiveNeeded = _start+_roiRecordsLimit<=ROIDISTRIBUTIONID?false:true;
        ...
        return (_info, map_Users[memberId].ROIDividend, IsArchiveNeeded);
    }
