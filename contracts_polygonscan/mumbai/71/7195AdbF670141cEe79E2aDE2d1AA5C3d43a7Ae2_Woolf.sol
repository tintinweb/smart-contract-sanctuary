...
contract Woolf is IWoolf, ERC721Enumerable, Ownable, Pausable {
  ...
  // event swoolBrun(address owner, uint256 value);
  /** 
   * instantiates contract and rarity tables  sheep wolf
   */
  constructor(address _swool, address _traits, uint256 _maxTokens) ERC721("Sheep Wolf Game", 'SWGAME') { 
    wool = SWOOL(_swool);
  ...
  function mint(uint256 amount, bool stake) external payable whenNotPaused {
     ...
     for (uint i = 0; i < amount; i++) {
      ...
      if (!stake || recipient != _msgSender()) {
        _safeMint(recipient, minted);
          barn.addUnShakingList(_msgSender(), minted);
        } else {
    ...
    //  emit swoolBrun(_msgSender(),totalWoolCost);  
    if (totalWoolCost > 0) wool.burn(_msgSender(), totalWoolCost);
    if (stake) 
    {
      barn.addManyToBarnAndPack(_msgSender(), tokenIds);
    }
  ...
  function generate(uint256 tokenId, uint256 seed) internal returns (SheepWolf memory t) {
    ...
    return generate(tokenId, randomer.random(seed));
  }
   ...
   */
  function setContract(address _barn,address _swool) external onlyOwner {
    barn = IBarn(_barn);
    wool = SWOOL(_swool);
  }
