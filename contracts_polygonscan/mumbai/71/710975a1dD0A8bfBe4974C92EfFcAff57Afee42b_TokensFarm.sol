...
contract TokensFarm is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    //TODO: leaderboard, allow to pull all relevant structs for current stakes
    // Relevant structs are all structs from users that are still participating
    // If we have addresses of user we know all relevant structs
    // So by going
    /**
    for(uint256 i = 0; i < participants.length ; i++){
        for(uint256 j = 0; j < stakeInfo[participants[i]].length; j++){

            // amount on specific stake of this user
            stakeInfo[participants[i]][j].amount
            // rewardDebt on specific stake of this user
            stakeInfo[participants[i]][j].rewardDebt
            // depositTime is when this stake happened
            StakeInfo[participants[i]][j].depositTime
            // address of user that holds this stake
            stakeInfo[participants[i]][j].addressOfUser
            // last withdrawal time that happened on this stake
            // if we want to keep every withdrawal time we need array
            // withdrawTime[];
            stakeInfo[participants[i]][j].withdrawTime

        }
    }
    */
    // we can access to every info about every user that is still participating
    // only flaw is that when user is withdrawing it will keep withdrawTime
    // of last withdrawal on that stake not of all withdrawal that happened on that stake

    /**
     * All users that are not participating anymore are not in participants array
     * but their structs will still exist with this structure
     *
     ****** StakeInfo{
                amount = 0;
                rewardDebt = 0;
                depositTime = 1202040415;
                addressOfUser = "0x210200410...";
                withdrawTime = 1301005055;
            }
     */
    ...
    //TODO: rename to have "Set" in the end of each event (not Change)
    ...
    {
        if(noOfUsers == 0){
            // User id begins with 1 because 0 is default
            noOfUsers++;
            // Add users id in participants array
            id[user] = noOfUsers;
            // add address of user
            ...
            // If user id is not 0 that means he is already participant
            // If he staked second time we don't put twice his address
            if(id[user] != 0){
                return false;
            }

            // If he is new add him
            noOfUsers++;
            id[user] = noOfUsers;
            participants.push(user);
        ...
        returns(bool)
    {
        ...
        //If farm has only one user
        if(noOfUsers == 1){
            // User maybe have more stakes than one
            // check if he is withdrawing whole amount
            totalAmount = 0;
            for(uint256 i = 0; i < stakeInfo[user].length; i++){
                totalAmount += stakeInfo[user][i].amount;
            }
            // If he is withdrawing whole amount
            if(amount == totalAmount){
                // delete id 0(1)
                delete id[user];// = id.length = 0;
                participants.pop();// participants.length = 0;
            ...
            // If he is not just withdraw regularly
            if(amount == totalAmount){
                // Pop deletes last element so last user in array transfer,
                // to place of user that is getting deleted
                participants[id[user] - 1] = participants[participants.length - 1];
                // update new id of last user,because he is in different position
                // on position of user that is not participants anymore by withdrawing whole amount
                id[participants[participants.length - 1]] = id[participants[id[user]]] - 1;
                // there are two elements in array with two same id's
                // delete the one that is not participating
                delete id[user];
                // delete user that is not participating
                participants.pop();
                // keep score of participants
                noOfUsers--;
    ...
    */
        ...
        payable
    {
        ...
        //TODO: measure balance of this contract for this token before
        ...
        //TODO:TFF-01 measure balance of this contract for this token after
        //TODO: compute the diff between before and after
        //TODO: if diff is less or equal to _amount, set stakedAmount= diff (to account for deflationary tokens)
        uint256 afterBalance = tokenStaked.balanceOf(address(this));
        if((afterBalance - beforeBalance) <= _amount){
            stakedAmount = (afterBalance - beforeBalance);//was = _amount
        }
        // but then staked amount in bellow computation is going to get even less;
        ...
        validateStakeByStakeId(msg.sender, stakeId)
    {
        ...
        //TODO: If procedure for deposit (before - after) should it be done here like in TFT-01 was named
        // (Lazar added this)
