...
contract DailyPrize is Ownable, VRFConsumerBase {
    ...
    function claimPrize(uint256 _day) public {
        uint256 prize = dailyPrizes[_day][msg.sender];
        ...
        override
    {
        ...
        DailyTicketInfo storage info = dailyTickets[day];
        mapping(address => uint256) storage dailyPrize = dailyPrizes[day];

        uint256 i;
        uint256 j;
        uint256 totalTickets;
        address[] memory winners = new address[](info.players.length);
        uint256 numWinners;
        uint8 totalAllocPoint;

        for (i = 0; i < info.players.length; i++) {
            totalTickets += info.tickets[info.players[i]];
        }

        for (i = 0; i < 25; i++) {
            uint256 rand = uint256(keccak256(abi.encodePacked(randomness, i)));
            rand = rand % totalTickets;
            j = 0;
            while (rand >= info.tickets[info.players[j]]) {
                rand -= info.tickets[info.players[j]];
                j++;
            }
            address winner = info.players[j];
            info.tickets[info.players[j]]--;
            totalTickets--;
        ...
        for (i = 0; i < numWinners; i++) {
            dailyPrize[winners[i]] =
                (info.prize * dailyPrize[winners[i]]) /
                100;
        }

        emit ResultDrawn(day);
    }
