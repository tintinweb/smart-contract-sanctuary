    ...
    ContextMixin
{
    ...
    function addURIEditor(address _editor) public only(MINTER_ROLE) {
        grantRole(LAND_URI_EDITOR_ROLE, _editor);
    }

    function removeURIEditor(address _editor) public only(MINTER_ROLE) {
        revokeRole(LAND_URI_EDITOR_ROLE, _editor);
    }

    function addMinter(address _minter) public only(MINTER_ROLE) {
        grantRole(MINTER_ROLE, _minter);
    }

    function removeMinter(address _minter) public only(MINTER_ROLE) {
     ...
     */
    function mint(address _user, uint256 _tokenId) public only(MINTER_ROLE) {
        require(
