...
contract DDSDividend is Context, ReentrancyGuard {
    ...
    function depositFunds(address[] memory payees, uint256[] memory shares) public payable {
        ...
        for (uint256 i = 0; i < payees.length; i++) {
            uint256 assignedFunds = shares[i] * (_totalFunds / _totalShares);
            _funds[payees[i]] = assignedFunds;
            emit fundsDeposited(payees[i], assignedFunds);
        } 
    } 

    function claimFunds(address payable member) public {
        require(_funds[member] > 0, "DDSDividend: address has no available funds to claim");

        uint256 payment = _funds[member];
        _funds[member] = 0;
        member.transfer(payment);

        emit fundsClaimed(member, payment);
    }
}