...
contract TimeLockedWallet is Initializable {
    ...
    function initialize(address _owner, address _tokenContract, uint _amount,
        uint _numberOfPeriods, uint _firstUnlockTime, uint _periodDuration) public initializer {
        owner = _owner;
        tokenContract = _tokenContract;
        uint periodAmount = _amount / _numberOfPeriods;
        for (uint i = 0; i < _numberOfPeriods; i++) {
            LockBoxStruct memory box;
            box.amount = periodAmount;
            box.unlockTime = _firstUnlockTime + i * _periodDuration;
            box.paid = false;
