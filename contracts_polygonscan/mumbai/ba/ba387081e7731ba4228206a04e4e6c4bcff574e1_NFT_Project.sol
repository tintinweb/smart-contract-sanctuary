...
contract NFT_Project is ERC721Burnable, Ownable, ReentrancyGuard{
    ...
    constructor() ERC721("UNICUSNFT","UNFT") {
        
    }

   event Minted (address indexed Creator,uint256[] indexed _NftId, string msg);

    function MINT_NFT( string[] memory _TokenURI, uint256 _Length) 
    public  nonReentrant returns (uint256[] memory){
        
        // Treasury.transfer(msg.value);
      return( _mintNft(msg.sender,  _TokenURI, _Length));
        
    }
    


    function _mintNft(address creator,  string[] memory _TokenURI , uint256 _Length) 
    internal 
    ...
    {   uint256[] memory _nftID;
        ...
        _nftID[i]=NftId;
        _setTokenURI(NftId,_TokenURI[i]);
        _tokenIdTracker.increment();
         _setTokenURI(NftId,_TokenURI[i]);
        }
    
        emit Minted (msg.sender,_nftID,"succesfully minted");
        return (_nftID); 
    }
