...
// File: contracts/AwarNFTDexPolygon.sol
...
contract NFTDexV7Polygon is ERC721Full, ERC721MetadataMintable, Ownable {
  IERC20 ALIA;
...
//   event SetValue(address indexed from, uint256 main, uint256 _author, uint256 _platform);
  ...
  event OnAuction(address indexed seller, address nftContract, uint256 indexed tokenId, uint256 startPrice, uint256 priceDollar, uint256 endTime);
  event Bid(address indexed bidder, address nftContract, uint256 tokenId, uint256 amount, bool awardType);
  event Claim(address indexed bidder, address nftContract, uint256 tokenId, uint256 amount, bool awardType);
  ...
  event awardAuctionNFT(address indexed from, address nft_a, uint256 tokenId, string seller, uint256 startPrice, uint256 priceDollar, uint256 endTime);
  ...
  function sellNFT(address nft_a,uint256 tokenId, address seller, uint256 price, uint256 priceDollar) public{
    ...
    // function MintAndSellNFT(address to, string memory tokenURI,uint256 quantity, bool flag, uint256 price, string memory ownerId, uint256 royality, uint256 priceDollar) public { 
      uint256 tokenId;
    ...
    require(ownerOf(_tokenId) == msg.sender, "102");
      ...
      if(_minPrice == 0 ) _dollarPrice[_tokenId] = priceDollar;
  ...
  function MintAndAuctionNFT(address to, string memory tokenURI,uint256 quantity, bool flag, address _contract, uint256 _minPrice, string memory ownerId, uint256 _endTime, uint256 royality, uint256 priceDollar, bool _isArtist) public {
      uint256 _tokenId;
      for(uint256 i = 0; i<quantity; i++)
      {
      ...
      if(msg.sender == admin) adminOwner[to] = true;
      if(msg.sender == nonCryptoNFTVault){
      revenueShare.addNonCryptoAuthor(ownerId, tokenId, _isArtist);
      emit MintWithTokenURINonCrypto(msg.sender, ownerId, tokenURI, quantity, flag);
      _nonCryptoOwners[_tokenId] = ownerId;
      emit awardAuctionNFT(msg.sender, address(this), _tokenId, ownerId, _minPrice, priceDollar, _endTime);
        }
      }
    }
  ...
  function claimAuction(uint256 _tokenId, bool awardType, string memory ownerId, address from) public {
    ...
    if(awardType) {
      ...
      // price = revenueShare.calculatePriceUSDToAlia(_dollarPrice[tokenId]);
     ...
     if(revenueShare.checkTokenAuthorIsArtist(tokenId) ){
          ALIA.transferFrom(msg.sender, aliaHolder, (price  / 1000) * authorPercentage);
          revenueShare.transferAliaArtist(aliaHolder, (price  / 1000) * authorPercentage, nonCryptoNFTVault, tokenId,platform, platformPerecentage);
        }else{
  ...
  function getSellDetail(uint256 tokenId) public view returns (address, uint256, uint256, address, uint256, uint256, uint256) {
      if(_saleTokens[tokenId].seller != address(0)){
        ...
        uint256 salePrice = _auctionTokens[tokenId].bidAmount;
        auctionSell storage temp = _auctionTokens[tokenId];
        return (temp.seller, salePrice > 0 ? salePrice : _dollarPrice[tokenId], temp.endTime, temp.bidder, temp.minPrice,  temp.startTime,temp.minPrice == 0 ? 1 : 0);
      }
  }
  ...
  function buyNFT(address nft_a,uint256 tokenId, string memory ownerId) public{
        ...
        //  price = temp.price == 0 ? revenueShare.calculatePriceUSDToAlia(_dollarPrice[tokenId]) : temp.price;
         if(temp.price > 0){
          price = temp.price;
        } else {
          (uint112 reserve0, uint112 reserve1,) =LPAlia.getReserves();
        ...
        if(isArtistAuthor && _tokenAuthors[tokenId] == nonCryptoNFTVault){
          uint256 _artistPrice;
          if(revenueShare.checkArtistOwner(_nonCryptoOwners[tokenId], tokenId)){
            _artistPrice = temp.price;
          }else {
           _artistPrice = (temp.price  / 1000) * authorPercentage;
           ALIA.transferFrom(msg.sender, temp.seller, (temp.price  / 1000) * mainPerecentage);
          //  ALIA.transferFrom(msg.sender, platform, (_artistPrice * platformPerecentage) / 1000);
          }
          ALIA.transferFrom(msg.sender, aliaHolder, _artistPrice);
          revenueShare.transferAliaArtist(aliaHolder, _artistPrice, nonCryptoNFTVault, tokenId, platform,platformPerecentage);
        }else{
          ALIA.transferFrom(msg.sender, temp.seller, (temp.price  / 1000) * mainPerecentage);
          ALIA.transferFrom(msg.sender, platform, (temp.price  / 1000) * platformPerecentage);
          ALIA.transferFrom(msg.sender, _tokenAuthors[tokenId], (temp.price  / 1000) * authorPercentage);
        }
        ...
        if(msg.sender != nonCryptoNFTVault) delete _nonCryptoOwners[tokenId];
        else _nonCryptoOwners[tokenId] = ownerId;
        delete _saleTokens[tokenId];
  ...
  require(msg.sender == 0x773E9CF672c7c4B50C39856A057f5E6581418e3D,"Not Authorized");
  _;
  }
  ...
  function updateTokenURI(uint256 tokenIdT, string memory uriT) public{
        require(_tokenAuthors[tokenIdT] == admin,"102");
