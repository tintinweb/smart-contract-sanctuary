...
import "./RacerData.sol";

contract RacerRedeemer is AccessControlEnumerable {
    bytes32 public constant MANAGER_ROLE = keccak256("MANAGER_ROLE");
    uint256 public constant KEY = 1;

    uint256 public constant MAX_BATCH_SIZE = 1000; // Need to figure out what actual number to use...

    mapping(address => uint256) public quantityRedeemed;
    address[] public _redeemers;

    RacerChild private RC;
    RacerData private RACER_DATA;

    constructor(address racerChildAddress, address racerDataAddress) {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setupRole(MANAGER_ROLE, msg.sender);
        RC = RacerChild(racerChildAddress);
        RACER_DATA = RacerData(racerDataAddress);
    }

    function redeemKeys(address[] memory redeemers, uint256[] memory quantities)
        public
        onlyRole(MANAGER_ROLE)
    {
        require(redeemers.length <= MAX_BATCH_SIZE, "Max batch size exceeded");
        require(
            redeemers.length == quantities.length,
            "input arrays must be same length"
        );
        for (uint256 i = 0; i < redeemers.length; i++) {
            if (quantityRedeemed[redeemers[i]] == 0) {
                _redeemers.push(redeemers[i]);
            }
            for (uint256 j = 0; j < quantities[i]; j++) {
                uint256 meltTokens = 1000 * 10**18;
                uint256 dna = RACER_DATA.getDNA(RC.nextTokenId());
                RC.newToken(
                    dna,
                    redeemers[i],
                    meltTokens,
    ...
    {
        uint256[] memory quantities = new uint256[](_redeemers.length);
        for (uint256 i = 0; i < _redeemers.length; i++) {
            quantities[i] = quantityRedeemed[_redeemers[i]];
        }
        return (_redeemers, quantities);
    }
}
...
contract RacerData is VRFConsumerBase, AccessControlEnumerable {
    ...
    uint256 private _totalAttributes = 15;
    enum Attribute {
        Attribute1,
        Attribute2,
        Attribute3,
        ...
        returns (string memory dnaString)
    {
        ...
        if (Utilities.stringLength(dnaString) < 30) {
            uint256 remainingDigits = 30 - Utilities.stringLength(dnaString);
    ...
    function getAttributes(uint256 tokenID)
        public
        view
        returns (uint256[15] memory attributes)
    {
        string memory dna = getDNAString(tokenID);
        for (uint256 i = 0; i < 28; i += 2) {
            attributes[(i / 2)] = Utilities.stringToUint(
                Utilities.substring(dna, i, i + 1)
            );
        }
    }
