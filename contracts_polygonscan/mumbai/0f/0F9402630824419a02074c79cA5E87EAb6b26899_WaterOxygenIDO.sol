...
contract WaterOxygenIDO is Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    address public Oxygen;
    address public MAI;
    address public addressToSendMAI;
    address public maiOxygenLP;
    ...
    mapping(address => bool) public boughtOxygen;
    ...
    constructor(
        ...
        address _treasury,
        address _staking,
        address _maiOxygenLP
    ) {
        require(_Oxygen != address(0));
        require(_MAI != address(0));
        require(_treasury != address(0));
        require(_staking != address(0));
        require(_maiOxygenLP != address(0));

        Oxygen = _Oxygen;
        MAI = _MAI;
        treasury = _treasury;
        maiOxygenLP = _maiOxygenLP;
        staking = _staking;
        cancelled = false;
        finalized = false;
    }
    ...
    function purchaseOxygen(uint256 _amountMAI) external returns (bool) {
        require(saleStarted() == true, 'Not started');
        require(
            !whiteListEnabled || whiteListed[msg.sender] == true,
            'Not whitelisted'
        );
        require(boughtOxygen[msg.sender] == false, 'Already participated');

        boughtOxygen[msg.sender] = true;
    ...
    function finalize(address _receipt) external onlyOwner {
        require(totalAmount == 0, 'need all Oxygens to be sold');

        uint256 maiInTreasure = 250000 * 1e18;

        IERC20(MAI).approve(treasury, maiInTreasure);
        uint256 OxygenMinted = ITreasury(treasury).deposit(maiInTreasure, MAI, 0);

        require(OxygenMinted == 250000 * 1e9);

        // dev: create lp with 15 MAI per Oxygen
        IERC20(MAI).transfer(maiOxygenLP, 750000 * 1e18);
        IERC20(Oxygen).transfer(maiOxygenLP, 50000 * 1e9);
        uint256 lpBalance = IUniswapV2Pair(maiOxygenLP).mint(address(this));
        uint256 valueOfToken = ITreasury(treasury).valueOfToken(
            maiOxygenLP,
            lpBalance
        );

        IUniswapV2Pair(maiOxygenLP).approve(treasury, lpBalance);
        uint256 zeroMinted = ITreasury(treasury).deposit(
            lpBalance,
            maiOxygenLP,
            valueOfToken
        );
        require(zeroMinted == 0, 'should not mint any Oxygen');
        IERC20(Oxygen).approve(staking, OxygenMinted);
