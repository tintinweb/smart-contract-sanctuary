...
contract GameMembership is Ownable, Events {
    ...
    address public lockAddress = 0xd9145CCE52D386f254917e481eB44e9943F39138;
    address public qdropRewardAddress = 0xd9145CCE52D386f254917e481eB44e9943F39138;

    uint256 memebershipUnit = 5;
    ...
    mapping (address => uint256) private _levelPerAddress;

    constructor (address _qdropToken) {
        qdropToken = _qdropToken;
 
        membershipFee = [0, 10, 20, 30, 40, 50];
        membershipMultipler = [0,1,2,3,4,5];
        membershipSubject = ["none", "Iron", "Bronze", "Silver", "Gold", "Diamond"];

    }

    // level 1 : Iron, level 2 : Bronze, level 3 : Silver, level 4 : Gold, level 5 : Diamond
    function setMembershipSubject(uint256 _level, string memory _subject) public onlyOwner {
        require(_level <= memebershipUnit, "_level exceeds 5");
        require(_level > 0, "_level bigger than 0");
         membershipSubject[_level] = _subject;
    }

    function getMembershipSubject(uint256 _level) public view returns(string memory) {
        require(_level <= memebershipUnit, "_level exceeds 5");
        require(_level > 0, "_level bigger than 0");
        return membershipSubject[_level];
    }

    function setMembership(uint256 _level, uint256 Fee, uint256 Multipler) public onlyOwner {
        require(_level <= memebershipUnit, "_level exceeds 5");
        require(_level > 0, "_level bigger than 0");
        require(Fee > 0 && Multipler > 0, "fee&multipler bigger than 0");
        membershipFee[_level] = Fee;
        membershipMultipler[_level] = Multipler;
    }

     function getMembership(uint256 _level) public view returns(uint256, uint256) {
        require(_level <= memebershipUnit, "_level exceeds 5");
        require(_level > 0, "_level bigger than 0");
        return (membershipFee[_level], membershipMultipler[_level]);
    }
    ...
    function purchaseMembership(address _from, uint256 _level) public returns(bool){
        // require(_from != address(0x0) && !_levelPerAddress[_from].locked, "Already Locked or 0x00 address");
        
        require((_level <= memebershipUnit) && (_level > 0), "Invalid level");
        require(_level > _levelPerAddress[_from], "select upper level");

        uint256 allowance = IERC20(qdropToken).allowance(address(this), _from);
        uint256 _newFee = membershipFee[_level];
        uint256 _oldFee = membershipFee[_levelPerAddress[_from]];
        IERC20(qdropToken).approve(_from, allowance + (_newFee - _oldFee));
        IERC20(qdropToken).transferFrom(_from, lockAddress, (_newFee - _oldFee));
        _levelPerAddress[_from] = _level;
    ...
    function getMembershipPerAddress(address _from) public view returns(uint256, uint256) {
        return (membershipFee[_levelPerAddress[_from]], membershipMultipler[_levelPerAddress[_from]]);
    }
    ...
    //     return (_levelPerAddress[_from].locked, _levelPerAddress[_from].lockedAmount, _levelPerAddress[_from].lastLockTimeStamp);
    // }
}