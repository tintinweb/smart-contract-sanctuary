...
contract KOMPrivateSale is ERC721 {
    ...
    uint256 public daiMintPrice = 2000000000000000000;
    ...
    function mintTokensByDAI(uint8 amount,uint256 _DaiAmount)
        external
        callerIsUser
        mintStarted
    {
        ...
        if (_DaiAmount >= totalMintDAiPrice) {
            dai.transferFrom(msg.sender, address(this), _DaiAmount-totalMintDAiPrice);
        }
    ...
    function mintTokensFromInviteByDAI(uint8 amount, address _ref,uint256 _daiAmount)
        external
        payable
        callerIsUser
        mintStarted
