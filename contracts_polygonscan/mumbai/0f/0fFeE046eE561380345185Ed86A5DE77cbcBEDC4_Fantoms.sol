...
contract Fantoms is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  function mint(uint256 _mintAmount) public payable {
    // Checks
    require(!paused, "Fantoms: minting has not started yet");
    require(_mintAmount > 0, "Fantoms: you have to mint at least one");
    require(_mintAmount <= maxMintAmount,  "Fantoms: you can only mint 10 at a time");
    require(lastSupply >= _mintAmount,  "Fantoms: the collection is sold out");
    if (msg.sender != owner()) {
      require(msg.value >= cost * _mintAmount, "Fantoms: total cost isn't match");
    }
    // Minting
  ...
  function randomGiveaway(address _winner) external onlyOwner {
    // Checks
    require(_winner != address(0), "Fantoms: zero address");
    require(lastSupply != 0,  "Fantoms: the collection is sold out");
    // Mint a random NFT for a winner
    _randomMint(_winner);
  }
  ...
  function _randomMint(address _target) internal returns (uint256) {
    ...
    uint256 _index = _getRandom() % maxSupply;
    uint256 _realIndex = getValue(_index) + 1;
    // Reduce supply
    maxSupply--;
    // Replace used id by last
    remainingIds[_index] = getValue(maxSupply);
    // Mint
  ...
  */
  ...
  /*
  function balanceOfMints(address _owner) public view returns (uint256) {
      return 0;
  }
  */
  ...
  function claimRewards() public {
    // uint count = balanceOfMints(msg.sender);
    // require(count > 0, "Fantoms: you minted no fantoms");
