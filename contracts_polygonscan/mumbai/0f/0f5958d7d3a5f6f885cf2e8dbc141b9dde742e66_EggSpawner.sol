...
library EggCodec {
    ...
    /**
     * @dev Get the `address` shifted to fit in the allocated space in the token
     * @return shiftedAddress_ Shifted address
     */
    function shiftedAddress(address address_) private pure returns (uint256 shiftedAddress_) {
        shiftedAddress_ = uint256(uint160(address_) >> ADDRESS_SHIFT);
    }

    /**
     * @dev Is `address_` minter of `token_`?
     * @param address_ Address
     * @param token_ Token
     * @return isAddressMinter_ Whether `address` is minter of `token`
     */
    ...
    {
        isAddressMinter_ = shiftedAddress(address_) == wallet(token_);
    }
    ...
    /**
     * @dev Set the wallet bits of `token_` to the shifted address of `address_`
     * @param token_ Token
     * @return tokenWithWallet_ `token_` with wallet bits set
     */
    function setWallet(uint256 token_, address address_)
        internal
        pure
        returns (uint256 tokenWithWallet_)
    {
        tokenWithWallet_ = token_ | (shiftedAddress(address_) << WALLET_SHIFT);
    }
    ...
    function setWallet(uint256 token_)
        ...
        returns (uint256 tokenWithWallet_)
    {
        tokenWithWallet_ = setWallet(token_, msg.sender);
    }
}
...
contract EggSpawner is Ownable, Pausable {
    ...
    ///@dev Whether legacy egg minting has taken place.
    bool public legacyMintCompleted;
    /**
    ...
    function isAddressMinter(uint256 token_, address address_)
        ...
        returns (bool isAddressMinter_)
    {
        isAddressMinter_ = token_.isAddressMinter(address_);
    }
    ...
    /**
     * @dev Create presale eggs for investors from the previous project phase.
     *
     * Can only be called once, so make sure all investors are included.
     */
    function spawnLegacyEggs(address[] calldata addresses) external onlyOwner {
        require (!legacyMintCompleted, "Already minted legacy eggs");

        legacyMintCompleted = true;
        
        for (uint256 i = 0; i < addresses.length; ++i) {
            address recipient = addresses[i];
            require(0 == rareEggs[recipient], "Already have Rare egg");
            uint256 token;
            token = token
                .setPolygon()
                .setNonFungible()
                ...
                .setCardAndMagicRare();
            ++rareEggCount;
            rareEggs[recipient] = token | iNft.nextTokenId();
            iNft.mintNFT(recipient, token);
        }
    }
