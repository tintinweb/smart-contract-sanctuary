...
contract Factory is AdminAndTangibleAccess, IFactory {
    ...
    constructor(address _usdc, address _feeStorageAddress, address _priceManager) {
    ...
    function setPriceManager(address _priceManager) onlyAdmin() external {
    ...
    function setDeployer(address _deployer) external onlyAdmin() {
        require( (_deployer != address(0x0)) && (_deployer != address(deployer)), "Wrong deployer");

        emit DeployerAddressSet(address(deployer), _deployer);
        deployer = _deployer;
    }

    function getApprovedVendorInfo(address vendor) external view returns (Vendor memory) {
    ...
    /// @inheritdoc IFactory
    function isFactoryOperator(address operator) external view override returns (bool){
        return isStorageOperator(operator);
    }

    /// @inheritdoc IFactory
    function isFactoryAdmin(address admin) external view override returns (bool){
    ...
    function mint(MintVoucher calldata voucher) external override onlyVendorOrMarketplace() returns (uint256[] memory) {
        ...
        uint256[] memory tokenIds = voucher.token
        ...
        //set Vendor struct
        if (!_hasVendorMintedInCategory(voucher.vendor, address(voucher.token))){
            //push to array
            approvedVendor[voucher.vendor].categories.push(address(voucher.token));
        }
    ...
    function burn(BurnVoucher calldata voucher) external override onlyStorageOperator() {
    ...
    function approveVendor(address vendor, bool approved) external onlyAdmin {
        if (approvedVendor[vendor].vendorId == 0){
        ...
        approved ? grantRole(VENDOR_ROLE, vendor) : revokeRole(VENDOR_ROLE, vendor);
        emit ApprovedVendor(vendor, approved);
    }

    function approveStorageOperator(address storageOperator, bool approved) external onlyAdmin {

        approved ? grantRole(STORAGE_OPERATOR_ROLE, storageOperator) : revokeRole(STORAGE_OPERATOR_ROLE, storageOperator);
         emit ApprovedStorageOperator(storageOperator, approved);
    }

    function _hasVendorMintedInCategory(address vendor, address _category)internal view returns (bool){
...
interface ITangibleNFTDeployer {
    ...
    event SetFactoryDeployer(address indexed oldFactory, address indexed newFactory);
