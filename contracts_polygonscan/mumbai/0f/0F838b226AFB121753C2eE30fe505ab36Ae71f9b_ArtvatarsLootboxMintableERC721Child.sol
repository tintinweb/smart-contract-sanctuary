...
contract ArtvatarsLootboxMintableERC721Child is
ERC721,
IChildToken,
AccessControlMixin,
...
ContextMixin
{
  ...
  function encodeTokenMetadata(uint256 tokenId) external view virtual returns (bytes memory) {
    ...
    // in L1
		Lootbox memory lootbox = LootBoxMapping[tokenId];
    return abi.encode(lootbox,tokenURI(tokenId));
  ...
  function mint(address user, uint256 tokenId) public only(DEFAULT_ADMIN_ROLE) {
	...
	mapping(uint => Lootbox) public LootBoxMapping;
