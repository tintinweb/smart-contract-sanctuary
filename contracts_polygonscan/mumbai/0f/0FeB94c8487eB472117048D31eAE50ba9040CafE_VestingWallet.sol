...
contract VestingWallet is Context {
    ...
    //
    //Get struct hash
    //VESTING_HASH = keccak256("VestingSchedule(address beneficiary,uint64 releaseTimestamp,uint256 amount)");
    //
    //Get hash of the struct with actual values
    //hash = keccak256(abi.encode(VESTING_HASH,_beneficiary,_amount,_releaseTimestamp));
    //
    //Get digest
    //digest = keccak256(abi.encodePacked("\x19\x01", DOMAIN_SEPARATOR, hash));
    //
    ...
    bytes32 constant VESTING_HASH = keccak256("VestingSchedule(address beneficiary,uint64 releaseTimestamp,uint256 amount)");
    ...
    // Limit 5 years (365 * 5 days = 157680000 seconds)
    uint256 private constant MAX_LIMIT = 157680000;
    ...
    function getMessageHash(
        ...
        return keccak256(abi.encodePacked(VESTING_HASH, _beneficiary, _releaseTimestamp, _amount));
    }
    ...
    function recoverSigner(bytes32 _ethSignedMessageHash, bytes memory _signature) public pure returns (address)
    {
        ...
        address recoveredAddress = ecrecover(_ethSignedMessageHash, v, r, s);
        require(recoveredAddress != address(0), "Zero address");
