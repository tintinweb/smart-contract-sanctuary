...
contract DecubateVesting is IDecubateVesting, Ownable, ReentrancyGuard {
    ...
    {
        uint256 idx = vestingPools[_option].hasWhitelist[_wallet].arrIdx;
        WhitelistInfo storage whitelist = vestingPools[_option].whitelistPool[
            idx
        ];
        ...
        returns (bool)
    {
        ...
        if (
            maxTokenTransfer.active && releaseAmount > maxTokenTransfer.amount
        ) {
        ...
        returns (VestingInfo memory)
    {
        return
            VestingInfo({
                ...
                initialUnlockPercent: vestingPools[_strategy]
    ...
    {
        uint256 idx = vestingPools[_option].hasWhitelist[_wallet].arrIdx;
        WhitelistInfo memory whitelist = vestingPools[_option].whitelistPool[
            idx
        ];
        VestingPool storage vest = vestingPools[_option];

        // initial unlock
        uint256 initial = whitelist
            .dcbAmount
            .mul(vest.initialUnlockPercent)
            .div(1000);
        ...
        if (block.timestamp < vest.start) {
            ...
            block.timestamp >= vest.cliff &&
            block.timestamp < vest.cliff.add(vest.duration)
        ) {
