    ...
    NativeMetaTransaction
{
    ...
    function random(uint256 nonce, uint256 range)
        internal
        view
        returns (uint256)
    {
    ...
    function giftBatch(address[] calldata accounts, uint256[] calldata ids)
        external
        virtual
        onlyOwner
    {
        ...
        returns (uint256[] memory)
    {
        ...
        for (uint256 i = 0; i < amount; i++) {
            ids[i] = mintableIds[random(nonce, mintableIds.length)];
            amounts[i] = 1;
            nonce++;
        }
...
contract Mintable is Ownable {
    uint256[] public mintableIds;

    modifier isMintable() {
        require(mintableIds.length > 0, "Mintable: no mintable ids");
        _;
    }

    function setMintableIds(uint256[] calldata ids) external onlyOwner {
        mintableIds = ids;
    }
}
