...
// File: contracts/extentions/address_checker.sol
...
// File: contracts/libraries/LiteralStrings.sol
...
// File: contracts/extentions/IHasSecondarySaleFees.sol
...
// File: contracts/extentions/HasSecondarySaleFees.sol
...
// File: contracts/Simple_chocomold.sol
    ...
    address_checker
{
    ...
    mapping (uint256 => string) private _eachURIs;

    string public customBaseURI;
    string public customBaseURI2;
    string public customBaseURI3;
    string public baseExtension = ".json";
    uint256 public phaseOneEnd = 10000;
    ...
    bool public pausedByOwner = false;
    bool public revealed = false;
    string public notRevealedUri = "ipfs://dachi_9797";
    ...
    function tokenURI(uint256 _tokenId) public view override returns (string memory) {
        ...
        string memory _tokenURI = _eachURIs[_tokenId];
        if (revealed == false){
            return notRevealedUri;
        }else {

                if (bytes(_tokenURI).length != 0 /*&& 10000 < _tokenId && _tokenId <= maxSupply*/) {
                    return 
                    string(
                            _tokenURI
                    );

                } else if (bytes(customBaseURI).length > 0 && 1 <= _tokenId && _tokenId <= phaseOneEnd) {
                    return
                    string(
                        abi.encodePacked(
                            customBaseURI,
                ...
                } else if (bytes(customBaseURI2).length > 0 && phaseOneEnd < _tokenId && _tokenId <= phaseTwoEnd) {
                    return
                    string(
                        abi.encodePacked(
                            customBaseURI2,
                ...
                } else if (bytes(customBaseURI3).length > 0 && phaseTwoEnd < _tokenId && _tokenId <= 10000) {
                    return
                    string(
                        abi.encodePacked(
                            customBaseURI3,
    ...
    function Ownidlist() public view onlyOwner returns  (uint256[] memory) 
    {   
    ...
    function _getOwnidlist() internal onlyOwner returns  (uint256[] memory) 
    {
    ...
    function _giveawaysetter(address[] memory _addList) public onlyOwner returns(uint256) 
    {
        _getOwnidlist();
        require (_getidlength() >= _addList.length);
    ...
    function giveawayWarrior(address[] memory _addList) public onlyOwner 
    {
        _giveawaysetter(_addList);

        require (_addList.length == gvwyidslist.length, "input length must be same");
        
        for(uint g=0; g < _addList.length; g++){

                if(isContract(_addList[g])){
                    //do nothing
    ...
    modifier mintCompliance(uint256 _mintAmount) {
        require(!pausedByOwner);
        require(_mintAmount > 0 && _mintAmount <= maxMintAmount, "Invalid mint amount!");
        require(supply.current() + _mintAmount <= maxSupply, "Max supply exceeded!");
        _;
    }

    function mintMul(uint256 _mintAmount) public payable mintCompliance(_mintAmount) {
    ...
    function mint(address[] memory _toList, uint256[] memory _tokenIdList) public onlyOwner mintCompliance(_tokenIdList.length) {
...
//only owner    
    function SetEachURI(uint256 _tokenId, string memory _tokenURI) public virtual onlyOwner {
        require(_exists(_tokenId), "ERC721Metadata: URI set of nonexistent token");
        _eachURIs[_tokenId] = _tokenURI;
    }    
    ...
    function setCustomBaseURI2(string memory _customBaseURI2) public onlyOwner {
        customBaseURI2 = _customBaseURI2;
    }

    function setCustomBaseURI3(string memory _customBaseURI3) public onlyOwner {
        customBaseURI3 = _customBaseURI3;
    }

    function setPhaseTerm(uint256 _phaseOneEnd, uint256 _phaseTwoEnd) public onlyOwner {
        require(1 < _phaseOneEnd && _phaseOneEnd <= _phaseTwoEnd && _phaseTwoEnd <= 10000);
        phaseOneEnd = _phaseOneEnd;
        phaseTwoEnd = _phaseTwoEnd;    
    }
