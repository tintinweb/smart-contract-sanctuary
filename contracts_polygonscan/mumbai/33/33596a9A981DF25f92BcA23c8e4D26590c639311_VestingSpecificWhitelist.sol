...
contract VestingSpecificWhitelist is
    IVesting,
    EIP712Upgradeable,
    OwnableUpgradeable
{
        ...
        returns (uint256 minAvailAllocation, uint256 maxAvailAllocation)
    {
        ...
        if (
            totalCurrency <= _totalDeposited || specificAllocation[_addr] == 0
        ) {
            return (0, 0);
        }
        ...
        uint256 maxAllocation = specificAllocation[_addr] > 1
    ...
    function _isValidAmount(uint256 _amount)
        ...
        returns (bool)
    {
        uint256 allocation = specificAllocation[_msgSender()];
        if (allocation == 0) return false;

        uint256 depositAmount = deposited[_msgSender()];

        uint256 maxAllocation = allocation > 1 ? allocation : _maxAllocation;
