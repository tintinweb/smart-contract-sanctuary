...
contract IterativeVestingFarm is OwnableUpgradeable, ReentrancyGuardUpgradeable{
    ...
    mapping(address => uint256) public totalUsersPayouts;
    // Array of users
        ...
        onlyOwner
    {
        ...
        require(
            totalUserRewards[user] > totalUsersPayouts[user],
            "User can not be kicked, he is already paid"
        );

        uint256 removedUserPendingBalance =
            totalUserRewards[user].sub(totalUsersPayouts[user]);
        // update totalRewards
        ...
        nonReentrant
    {
        ...
        require(
            totalUsersPayouts[address(msg.sender)] < totalUserRewards[address(msg.sender)],
            "User has been paid out"
        );

        uint256 amountEarned;
        uint256 rewardPerPortion = (totalUserRewards[address(msg.sender)] / nVestingPortions);
        ...
        if (amountEarned > 0){
            totalUsersPayouts[address(msg.sender)] += amountEarned;
