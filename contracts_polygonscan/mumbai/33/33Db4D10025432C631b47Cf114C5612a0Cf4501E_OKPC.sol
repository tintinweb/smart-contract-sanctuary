...
contract OKPCMetadata is IOKPCMetadata {
        ...
        string(
          abi.encodePacked(
            ...
            abi.encodePacked(
              ...
              parts.hat.name,
              '"}, {"trait_type": "Left Border", "value": "',
              parts.leftBorder.name,
              '"}, {"trait_type": "Left Border", "value": "',
              parts.leftBorder.name,
              '"}]'
            ),
  ...
  function _draw(
    ...
    bytes memory svg = abi.encodePacked(
      abi.encodePacked(
        ...
        parts.leftBorder.data,
        '"/><path transform="translate(31, 8) scale(-1,1)" d="',
        parts.rightBorder.data,
  ...
  function drawArt(bytes memory artData) internal pure returns (string memory) {
  ...
  function drawWord(string memory word) internal view returns (string memory) {
...
interface IOKPCParts {
  ...
  // TODO rename these to something regarding font
  ...
  function getHat(uint256 index) external view returns (Vector memory);

  function getBorder(uint256 index) external view returns (Vector memory);
...
contract OKPCParts is IOKPCParts {
  ...
  Vector[8] public hats;
  Vector[8] public borders;
  bytes4[128] public words;

  uint256 public constant NUM_COLORS = 6;
  uint256 public constant NUM_HATS = 8;
  uint256 public constant NUM_BORDERS = 8;
  ...
  function getChar(bytes1 char) public view override returns (string memory) {
    ...
    require(bytes(font[c]).length != 0, 'char not found');
    return font[c];
  }
    ...
    returns (string memory)
  {
    ...
    uint8 index = uint8(bytes(char)[0]);
    if (index >= 65 && index <= 90) index += 32;
    bytes1 c = bytes1(index);
  ...
  function getHat(uint256 index) public view override returns (Vector memory) {
    if (index > NUM_HATS - 1) revert IndexOutOfBounds(index, NUM_HATS - 1);
    return hats[index];
  }
  ...
  {
    if (index > NUM_BORDERS - 1)
      revert IndexOutOfBounds(index, NUM_BORDERS - 1);
    ...
    returns (Parts memory)
  {
    ...
    if (tokenId <= 128) {
      parts.color = getColor((tokenId - 1) % NUM_COLORS);
      parts.word = getWord(tokenId - 1);
    } else {
      parts.color = getColor(
        uint256(keccak256(abi.encodePacked('COLOR', tokenId))) % NUM_COLORS
      );
      parts.word = getWord(
        uint256(keccak256(abi.encodePacked('WORD', tokenId))) % NUM_WORDS
      );
    }

    parts.hat = getHat(
      uint256(keccak256(abi.encodePacked('HAT', tokenId))) % NUM_HATS
    );
    parts.rightBorder = getBorder(
      uint256(keccak256(abi.encodePacked('LEFT BORDER', tokenId))) % NUM_BORDERS
    );
    parts.leftBorder = getBorder(
      uint256(keccak256(abi.encodePacked('RIGHT BORDER', tokenId))) %
        NUM_BORDERS
    );
