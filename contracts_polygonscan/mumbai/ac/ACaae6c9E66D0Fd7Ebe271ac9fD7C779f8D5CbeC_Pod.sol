    ...
    )
{
    ...
    function ownerInitMaxNFTSupply(uint256 maxNFTSupply) public onlyOwner {
        require(
            maxNFTSupply > MAX_NFT_SUPPLY,
            'ERC721: maxNFTSupply value is incorrect'
        );
        for (
            uint256 i = (MAX_NFT_SUPPLY + MIN_BEZOGI_URI);
            i <= maxNFTSupply;
            i++
        ) {
            _uriSet.add(i);
    ...
    function ownerMintNFT(uint256 total) public onlyOwner {
    ...
    function ownerAirdrop(address[] memory _addressArray) public onlyOwner {
    ...
    function minterMintWithSummon(address[] memory _addressArray) public {
        ...
        uint256 mintIndex = totalSupply();
        require(
            mintIndex >= MAX_NFT_SUPPLY,
            'ERC721: mintIndex value is incorrect'
        );
    ...
    function ownerSetBaseTokenURI(string memory baseTokenURI) public onlyOwner {
        _baseTokenURI = baseTokenURI;
    }

    function ownerSetSaleAvailable(bool saleAvailable) public onlyOwner {
        _saleAvailable = saleAvailable;
    }

    function ownerSetSummonAvailable(bool summonAvailable) public onlyOwner {
        _summonAvailable = summonAvailable;
    }

    function ownerSetTransferAvailable(bool transferAvailable)
        public
        onlyOwner
    {
    ...
    function ownerSetPresaleAvailable(bool presaleAvailable) public onlyOwner {
        _presaleAvailable = presaleAvailable;
    }

    function ownerSetBezogiPrice(uint256 bezogiPrice) public onlyOwner {
    ...
    function ownerWithdrawWETH() public onlyOwner {
        require(
            _posWETHToken.transfer(
                msg.sender,
                _posWETHToken.balanceOf(address(this))
            ),
            'Unable to transfer WETH'
        );
    }
     ...
     */
    function ownerWithdrawLink() public onlyOwner {
    ...
    function _beforeTokenTransfer(
        ...
        require(
            from == address(0) || getTransferAvailable(),
            'Unable to transfer during NFT sale.'
        );
        require(
            !publicTokenBlacklisted(from, tokenId),
            'Unable to transfer, your token is blacklisted'
        );
    ...
    function publicMintNFT(uint256 numberOfNfts) public {
        ...
        require(
            _posWETHToken.allowance(msg.sender, address(this)) >=
                (publicGetNFTPrice() * numberOfNfts),
            'Ether allowance value is incorrect'
        );
        require(
            _posWETHToken.balanceOf(msg.sender) >=
                (publicGetNFTPrice() * numberOfNfts),
        ...
        if (getPresaleAvailable()) {
            require(
                publicPresaleWhitelisted(msg.sender),
                'You are not on the pre-sale whitelist'
            );
            require(
                numberOfNfts <= 3,
                'You may not mint more than 3 NFTs during presale'
            );
            // max 3 NFT
            uint256 nftBalance = balanceOf(msg.sender) + numberOfNfts;
            require(
                nftBalance <= 3,
                'ERC721: You cannot mint more than 3 NFTs during presale'
            );
        } else {
            ...
            require(
                nftBalance <= 10,
                'ERC721: You cannot mint more than 10 NFTs'
            );
        }

        _posWETHToken.transferFrom(
            msg.sender,
            address(this),
            (publicGetNFTPrice() * numberOfNfts)
        );
    ...
    function publicGetWETHAllowance(address _address)
        public
        view
        returns (uint256)
    {
    ...
    function summonTokenWithURI(uint256 tokenId, string memory bezogiURI)
        internal
    {
    ...
    /***********************************PRESALE-WHITELIST********************************** */
    function ownerPresaleWhitelistAdd(address[] memory _addressArray)
        public
        onlyOwner
    {
        for (uint256 i = 0; i < _addressArray.length; i++) {
    ...
    function ownerPresaleWhitelistRemove(address[] memory _addressArray)
        public
        onlyOwner
    {
        for (uint256 i = 0; i < _addressArray.length; i++) {
    ...
    function publicPresaleWhitelisted(address _address)
        public
        view
        returns (bool)
    {
    ...
    function minterTokenBlacklistAdd(address _address, uint256 tokenId) public {
        require(
            ...
            'ERC721: must have minter or admin role to add'
        );
        require(
            _address != address(0),
            'ERC721: Cannot add zero address to the blacklist'
        );
        _tokenBlacklist[_address].add(tokenId);
    }

    function minterTokenBlacklistRemove(address _address, uint256 tokenId)
        public
    {
        require(
            hasRole(MINTER_ROLE, msg.sender) ||
                hasRole(DEFAULT_ADMIN_ROLE, msg.sender),
            'ERC721: must have minter or admin role to remove'
        );
        _tokenBlacklist[_address].remove(tokenId);
    }

    function publicTokenBlacklisted(address _address, uint256 tokenId)
        public
        view
        returns (bool)
    {
