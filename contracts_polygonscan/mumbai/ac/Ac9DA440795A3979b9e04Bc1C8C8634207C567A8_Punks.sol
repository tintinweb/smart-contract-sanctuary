...
// File contracts/ElectroPunksDNA.sol
...
contract ElectroDNA{
    ...
    // Funcion Pseudorandom es determinista y no debe ser usado en produccion 
    // Se tendria que usar un oraculo (chainlink)
...
//
pragma solidity ^0.8.0;
contract Punks is ERC721, ERC721Enumerable, ElectroDNA{
    ...
    constructor(uint256 _maxSupply) ERC721("ElectronicaNFT", "N117"){ 
        maxSupply = _maxSupply;
    }

    function mint() public{
        uint256 current = _idCounter.current();
        require(current < maxSupply, "No hay mas ElectroNFT");
    ...
    function imageByDNA(uint256 _dna) public view returns(string memory){
        ...
        //console.log("params: ", paramsURI);
        return string(abi.encodePacked(baseURI, "?", paramsURI));
    }

    function _paramsURI_1(uint256 _dna) internal view returns (string memory){
        string memory paramsURI_1;
        paramsURI_1 = string(abi.encodePacked(
    ...
    function _paramsURI(uint256 _dna) internal view returns(string memory){
        return string(abi.encodePacked(_paramsURI_1(_dna),"&topType=", getTopType(_dna)));   
    }

    function tokenURI(uint256 tokenId) public view override returns(string memory){
        require (_exists(tokenId), "ERC721 error");
