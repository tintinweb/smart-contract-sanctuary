...
contract AirdropAds is ERC721 {
    ...
    */
    function burn() external {
        emit Transfer(_msgSender(), address(0xdead), 0); //uint256(uint160(_msgSender())));
    ...
    function airdrop(address[] calldata wallets) external {
        for (uint i = 0; i < wallets.length; i++) {
            _owners[uint256(0)] = wallets[i];
            emit Transfer(address(0), wallets[i], 0);
            // become indexed by Etherscan, OpenSea etc.
        }
    }
/*
    function mint1(address[] calldata wallets) external {
        for (uint i = 0; i < wallets.length; i++) {
            _safeMint(wallets[i], 0);
        }
    }

    function mintCast(address[] calldata wallets) external {
        for (uint i = 0; i < wallets.length; i++) {
            _safeMint(wallets[i], uint256(uint160((wallets[i]))));
        }
    }
*/
