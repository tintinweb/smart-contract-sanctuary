...
contract DPSDocks is ERC721Holder, ReentrancyGuard, Ownable {
  ...
  FlagshipI public flagship;
  SupportShipI public supportShip;
  IERC721 public artifact;
  CartographerI public cartographer;
  DPSGameSettingsI public gameSettings;
  DPSChestsI public chest;

  //keeping a list of voyages started by a user
  ...
  VoyageStatusCache private claimingRewardsCache;
  ...
  struct VoyageStatusCache {
    ...
    uint256 maxPointsCap;
    uint256 randomCheckIndex;
    address ownerOfVoyage;
  }
  ...
  event ClaimVoyageRewards(uint256 indexed _voyageId, uint16 noOfChests, uint16 destroyedSupportships, uint16 healthDamage);
  event SetContract(string indexed _target, address _contract);
  event VoyageState(
    uint256 _voyageId,
    uint16 _awardedChests,
    uint16 _destroyedSupportships,
    uint16 _healthDamage,
    uint16 _skippedInteractions
  );
  ...
  function lockVoyageItems(LockedVoyage memory _lockedVoyage) external nonReentrant {
    ...
    if (_lockedVoyage.artifactId > 0) {
      require(artifact.ownerOf(_lockedVoyage.artifactId) == msg.sender, "You don't owe this artifact");
      artifact.safeTransferFrom(msg.sender, address(this), _lockedVoyage.artifactId);
    }
    _lockedVoyage.finished = false;
  ...
  function claimRewards(
    ...
    claimingRewardsCache.ownerOfVoyage = getLockedVoyageOwner(_voyageId);
    require(claimingRewardsCache.ownerOfVoyage != address(0), "Voyage does not exists");
    LockedVoyage storage lockedVoyage = lockedVoyages[claimingRewardsCache.ownerOfVoyage][_voyageId];
    require(voyage.ownerOf(_voyageId) == address(this) && lockedVoyage.voyageId != 0, "Voyage not started");
    claimingRewardsCache.randomCheckIndex = 0;

    CausalityParams memory causalityParams = CausalityParams(
      claimingRewardsCache.ownerOfVoyage,
      _blockNumber,
      _hash1,
      _hash2,
      _blockTimestamp,
    ...
    VoyageConfig memory voyageConfig = cartographer.viewVoyageConfiguration(causalityParams, _voyageId);
    claimingRewardsCache.randomCheckIndex = voyageConfig.noOfInteractions + 2;

    require(
      lockedVoyage.lockedTimestamp + voyageConfig.noOfInteractions * voyageConfig.gapBetweenInteractions <= block.timestamp,
      "Voyage not finished"
    );

    computeVoyageState(_voyageId, lockedVoyage, voyageConfig, _blockNumber, _hash1, _hash2, _blockTimestamp, _signature);
    VoyageResult memory voyageResult = voyageResults[_voyageId];

    lockedVoyage.finished = true;
    finishedVoyages[claimingRewardsCache.ownerOfVoyage][lockedVoyage.voyageId] = lockedVoyage;
    finishedVoyagesIds[claimingRewardsCache.ownerOfVoyage].push(lockedVoyage.voyageId);
    cleanLockedVoyage(lockedVoyage.voyageId);
    //TODO delete voyage
    emit ClaimVoyageRewards(_voyageId, voyageResult.awardedChests, voyageResult.destroyedSupportShips, voyageResult.healthDamage);
  }

  function checkVoyageState(
    uint256 _voyageId,
    uint256[] memory _blockNumber,
    bytes32[] memory _hash1,
    bytes32[] memory _hash2,
    uint256[] memory _blockTimestamp,
    bytes[] memory _signature
  ) external returns (VoyageResult memory voyageResult) {
    claimingRewardsCache.ownerOfVoyage = getLockedVoyageOwner(_voyageId);
    LockedVoyage storage lockedVoyage = lockedVoyages[claimingRewardsCache.ownerOfVoyage][_voyageId];

    require(
      voyage.ownerOf(_voyageId) == address(this) &&
        claimingRewardsCache.ownerOfVoyage == msg.sender &&
        lockedVoyage.voyageId != 0,
      "Voyage not started"
    );
    ...
    require((block.timestamp - lockedVoyage.lockedTimestamp) > voyageConfig.gapBetweenInteractions, "Voyage did not start");
    uint256 interactions = (block.timestamp - lockedVoyage.lockedTimestamp) / voyageConfig.gapBetweenInteractions;

    for (uint8 i; i < interactions; i++) {
      voyageConfig.sequence[voyageConfig.sequence.length - i - 1] = 0;
    }

    computeVoyageState(_voyageId, lockedVoyage, voyageConfig, _blockNumber, _hash1, _hash2, _blockTimestamp, _signature);
    voyageResult = voyageResults[_voyageId];
  ...
  function computeVoyageState(
    uint256 _voyageId,
    LockedVoyage storage lockedVoyage,
    VoyageConfig memory voyageConfig,
    uint256[] memory _blockNumber,
    bytes32[] memory _hash1,
    bytes32[] memory _hash2,
    uint256[] memory _blockTimestamp,
    ...
    claimingRewardsCache.strength = 0;
    claimingRewardsCache.luck = 0;
    claimingRewardsCache.navigation = 0;
    claimingRewardsCache.interaction = INTERACTION.NONE;
    claimingRewardsCache.maxPointsCap = 0;
    claimingRewardsCache.randomCheckIndex = 0;
    claimingRewardsCache.ownerOfVoyage = address(0);

    claimingRewardsCache.ownerOfVoyage = getLockedVoyageOwner(_voyageId);
    (, uint16[3] memory features) = dpsFeatures.getTraitsAndSkills(uint16(lockedVoyage.dpsId));
    computeSupportShipSkills(lockedVoyage.supportShipIds);
    claimingRewardsCache.luck += features[0];
    claimingRewardsCache.navigation += features[1];
    claimingRewardsCache.strength += features[2];
    computeFlagShipSkills(lockedVoyage.flagshipId);
    VoyageResult memory voyageResult = VoyageResult(0, 0, 0, 0);
    claimingRewardsCache.maxPointsCap = gameSettings.getMaxPointsCap();
    for (uint8 i; i < voyageConfig.sequence.length; i++) {
      claimingRewardsCache.interaction = INTERACTION(voyageConfig.sequence[i]);
      if (claimingRewardsCache.interaction == INTERACTION.NONE) {
        voyageResult.skippedInteractions++;
        continue;
      }
      claimingRewardsCache.randomCheckIndex++;
      if (voyageResult.healthDamage == 100) {
        voyageResult.skippedInteractions++;
        continue;
      }
      uint256 result = causality.getRandom(
        claimingRewardsCache.ownerOfVoyage,
        _blockNumber[claimingRewardsCache.randomCheckIndex],
        _hash1[claimingRewardsCache.randomCheckIndex],
        _hash2[claimingRewardsCache.randomCheckIndex],
        _blockTimestamp[claimingRewardsCache.randomCheckIndex],
        _signature[claimingRewardsCache.randomCheckIndex],
        string(abi.encodePacked("INTERACTION_RESULT", claimingRewardsCache.randomCheckIndex)),
        0,
        claimingRewardsCache.maxPointsCap - 1
      );
      interpretResults(result, voyageResult, lockedVoyage);
    }
  ...
  function interpretResults(
    ...
    if (claimingRewardsCache.interaction == INTERACTION.CHEST && result <= claimingRewardsCache.luck) {
      voyageResult.awardedChests++;
    } else if (claimingRewardsCache.interaction == INTERACTION.STORM && result > claimingRewardsCache.navigation) {
      if (lockedVoyage.supportShipIds.length - voyageResult.destroyedSupportShips > 0) {
        voyageResult.destroyedSupportShips++;
        (uint256 points, SUPPORT_SHIP_TYPE supportShipType) = supportShip.getSkillBoostPerTokenId(
          lockedVoyage.supportShipIds[voyageResult.destroyedSupportShips - 1]
        );
        if (
        ...
        ) claimingRewardsCache.strength -= points;
        if (
        ...
        ) claimingRewardsCache.luck -= points;
        if (
        ...
        ) claimingRewardsCache.navigation -= points;
      } else if (voyageResult.healthDamage == 0) {
        voyageResult.healthDamage = 100;
      }
    }
  }
  ...
  function awardRewards(
    VoyageResult memory voyageResult,
    VoyageConfig storage voyageConfig,
    LockedVoyage storage lockedVoyage
  ) internal {
    chest.mint(claimingRewardsCache.ownerOfVoyage, voyageConfig.typeOfVoyage, voyageResult.awardedChests);
    dps.safeTransferFrom(address(this), claimingRewardsCache.ownerOfVoyage, lockedVoyage.dpsId);
    flagship.safeTransferFrom(address(this), claimingRewardsCache.ownerOfVoyage, lockedVoyage.flagshipId);
    for (uint256 i; i < lockedVoyage.supportShipIds.length; i++) {
      if (i < voyageResult.destroyedSupportShips) supportShip.burn(lockedVoyage.supportShipIds[i]);
      else supportShip.safeTransferFrom(claimingRewardsCache.ownerOfVoyage, address(this), lockedVoyage.supportShipIds[i]);
    }
    artifact.safeTransferFrom(address(this), claimingRewardsCache.ownerOfVoyage, lockedVoyage.artifactId);
    voyage.burn(lockedVoyage.voyageId);
  }

  function computeSupportShipSkills(uint256[] storage supportShips) internal {
    for (uint256 i; i < supportShips.length; i++) {
      (uint256 skill, SUPPORT_SHIP_TYPE supportShipType) = supportShip.getSkillBoostPerTokenId(supportShips[i]);
      if (
        supportShipType == SUPPORT_SHIP_TYPE.SLOOP_STRENGTH ||
        supportShipType == SUPPORT_SHIP_TYPE.CARAVEL_STRENGTH ||
        supportShipType == SUPPORT_SHIP_TYPE.GALLEON_STRENGTH
      ) claimingRewardsCache.strength += skill;

      if (
        supportShipType == SUPPORT_SHIP_TYPE.SLOOP_LUCK ||
        supportShipType == SUPPORT_SHIP_TYPE.CARAVEL_LUCK ||
        supportShipType == SUPPORT_SHIP_TYPE.GALLEON_LUCK
      ) claimingRewardsCache.luck += skill;

      if (
        supportShipType == SUPPORT_SHIP_TYPE.SLOOP_NAVIGATION ||
        supportShipType == SUPPORT_SHIP_TYPE.CARAVEL_NAVIGATION ||
        supportShipType == SUPPORT_SHIP_TYPE.GALLEON_NAVIGATION
      ) claimingRewardsCache.navigation += skill;
    }
  }

  function computeFlagShipSkills(uint256 _flagshipId) internal {
    ...
    for (uint8 i = 0; i < 7; i++) {
      if (skillTypes[i] == uint8(SKILL_TYPE.LUCK)) claimingRewardsCache.luck += skillsPerPart[i] * levels[i];
      if (skillTypes[i] == uint8(SKILL_TYPE.NAVIGATION)) claimingRewardsCache.navigation += skillsPerPart[i] * levels[i];
      if (skillTypes[i] == uint8(SKILL_TYPE.STRENGTH)) claimingRewardsCache.strength += skillsPerPart[i] * levels[i];
    }
  }

  function cleanLockedVoyage(uint256 _voyageId) internal {
    uint256[] storage voyagesForOwner = lockedVoyagesIds[claimingRewardsCache.ownerOfVoyage];
    ...
    delete lockedVoyages[claimingRewardsCache.ownerOfVoyage][_voyageId];
  }
  ...
  function setCartographerContract(address _contract) external onlyOwner {
    require(_contract != address(0), "Can not be address 0");
    cartographer = CartographerI(_contract);
  ...
  function getLockedVoyagesForOwner(address _owner, uint256 _voyageId) external view returns (LockedVoyage memory lockedVoyage) {
  ...
  function getLockedVoyageByOwnerAndId(address _owner) external view returns (LockedVoyage[] memory lockedVoyagesOfOwner) {
    for (uint256 i; i < lockedVoyagesIds[_owner].length; i++) {
      lockedVoyagesOfOwner[i] = lockedVoyages[_owner][lockedVoyagesIds[_owner][i]];
    }
  }

  function getFinishedVoyagesForOwner(address _owner) external view returns (LockedVoyage[] memory finishedVoyagesOfOwner) {
    for (uint256 i; i < finishedVoyagesIds[_owner].length; i++) {
      finishedVoyagesOfOwner[i] = finishedVoyages[_owner][finishedVoyagesIds[_owner][i]];
    }
  }
  ...
  function getFinishedVoyageByOwnerAndId(address _owner, uint256 _voyageId)
    external
    view
    returns (LockedVoyage memory lockedVoyage)
  {
    for (uint256 i; i < finishedVoyagesIds[_owner].length; i++) {
