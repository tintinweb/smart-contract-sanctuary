...
import "../Interfaces/IMSPToken.sol";
...
/*
 * On deployment a new Uniswap pool will be created for the pair USDM/ETH and its token will be set here.
 ...
 * Funds for rewards will only be added once, on deployment of MSP token,
 ...
 * in order to avoid getting MSP tokens locked.
 * That also means that the start time for the program will be the event that occurs first:
 * either MSP token contract is deployed, and therefore MSP tokens are minted to Unipool contract,
...
contract Unipool is LPTokenWrapper, Ownable, CheckContract, IUnipool {
    ...
    event MSPTokenAddressChanged(address _mspTokenAddress);
    ...
    function setParams(
        ...
        onlyOwner
    {
        checkContract(_mspTokenAddress);
        checkContract(_uniTokenAddress);

        uniToken = IERC20(_uniTokenAddress);
        mspToken = IMSPToken(_mspTokenAddress);
        duration = _duration;

        _notifyRewardAmount(mspToken.getLpRewardsEntitlement(), _duration);

        emit MSPTokenAddressChanged(_mspTokenAddress);
    ...
    function claimReward() public override {
        ...
        mspToken.transfer(msg.sender, reward);
    ...
    function _notifyRewardAmount(uint256 _reward, uint256 _duration) internal {
        assert(_reward > 0);
        assert(_reward == mspToken.balanceOf(address(this)));
...
interface IMSPToken is IERC20, IERC2612 { 
   
    // --- Events ---
    
    event CommunityIssuanceAddressSet(address _communityIssuanceAddress);
    event MSPStakingAddressSet(address _mspStakingAddress);
    event LockupContractFactoryAddressSet(address _lockupContractFactoryAddress);

    // --- Functions ---
    
    function sendToMSPStaking(address _sender, uint256 _amount) external;
...
interface IUnipool {
    function setParams(address _mspTokenAddress, address _uniTokenAddress, uint256 _duration) external;
