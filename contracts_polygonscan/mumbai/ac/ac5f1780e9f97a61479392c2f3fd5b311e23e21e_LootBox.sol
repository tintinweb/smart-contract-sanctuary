...
contract LootBox is Ownable, Pausable, ReentrancyGuard, VRFConsumerBase {
   ...
   * @param _tierId The tier id
   */
  function getNumberOfWhitelist(uint256 _tierId) public view returns (uint256) 
  {
  ...
  function setDiscountWhiteList(
    ...
    for (uint i = 0; i < _addresses.length; i++){
      ...
      discountWhitelistMap[_tierId][add] = _discountPercent;
    }
  }
   ...
   * @param _amountToDiscount   Minimum amount to get the discount price.
   * @param _discountPercent    Discount percent.
   */
  function setTierSettings(
    ...
    require(_amountToDiscount <= _quantityPerOpen[1], "LootBox#setTierSettings: WRONG_AMOUNT_TO_DISCOUNT");
    require(_discountPercent < 100, "LootBox#setTierSettings: WRONG_DISCOUNT_PERCENT");
    ...
    TierSettings memory settings = TierSettings({
      ...
      amountToDiscount : _amountToDiscount,
  ...
  function estimateCost(
    ...
    if (_amount >= settings.amountToDiscount) {
      // tier discount
      cost = cost * (INVERSE_BASIS_POINT - settings.discountPercent) / INVERSE_BASIS_POINT;
    }

    if (discountWhitelistMap[tierDiscountSettings][_toAddress] > 0) {
      // tier whitelist discount
      cost = cost * (INVERSE_BASIS_POINT - discountWhitelistMap[tierDiscountSettings][_toAddress]) / INVERSE_BASIS_POINT;
    } else  if (discountWhitelistMap[globalDiscountsettings][_toAddress] > 0) {
      // global whitelist discount
      cost = cost * (INVERSE_BASIS_POINT - discountWhitelistMap[globalDiscountsettings][_toAddress]) / INVERSE_BASIS_POINT;
    }
