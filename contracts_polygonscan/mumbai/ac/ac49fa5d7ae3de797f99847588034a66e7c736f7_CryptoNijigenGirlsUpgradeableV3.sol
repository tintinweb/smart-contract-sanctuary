...
contract CryptoNijigenGirlsUpgradeableV3 is Initializable, ERC721Upgradeable, OwnableUpgradeable {
    ...
    function setErc20TokenAddress(string memory newErc20TokenAddress) public virtual onlyOwner{

        bytes memory tmp = bytes(newErc20TokenAddress);
        uint160 iaddr = 0;
        uint160 b1;
        uint160 b2;
        ...
        _erc20TokenAddress = address(iaddr);
    }
    ...
    function buyCNG(uint256 _tokenId) external payable {
        ...
        if(_erc20TokenAddress != address(0)){
            IERC20 tokenContract = IERC20(_erc20TokenAddress);
            require(tokenContract.transferFrom(msg.sender, seller, msg.value / 100 * 98), "CNGE020");
        } else {
            payable(seller).transfer(msg.value / 100 * 98); // send the ETH to the seller
        }
