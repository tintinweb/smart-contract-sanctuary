...
contract Weapons is Initializable, ERC721Upgradeable, AccessControlUpgradeable {
    ...
    function mint(address minter, uint256 seed) public restricted returns (uint256) {
        ...
        uint256 gunTypeRoll = RandomUtil.randomSeededMinMax(1, 10000, seed);
    ...
    function getFightData(
        ...
        return (getBonusPowerForFight(id, playerPower, wep.gunType, wep.element, charTrait, charClass));
    }
    ...
    function getBonusPowerForFight(
        ...
        if (wepType == 0 && charClass == 0)
            totalPower = uint24(basePower.add(uint24(ABDKMath64x64.divu(1, 10).mulu(basePower))));
        if (wepType == 0 && charClass == 1)
            totalPower = uint24(basePower.add(uint24(ABDKMath64x64.divu(2, 10).mulu(basePower))));
        ...
        if (charTrait == wepTrait) totalPower.add((uint24(ABDKMath64x64.divu(3, 20).mulu(basePower))));
