...
/////////////////////////////////////////// ERC1155 //////////////////////////////////////////////
    ...
    */
    function safeTransferFrom(address _from, address _to, uint256 _id, uint256 _value, bytes calldata _data) external {
        ...
        require(_value > 0, "Transfer amount needs to be higher than 0.");
...
contract ERC1155MixedFungible is ERC1155 {
    ...
    function safeTransferFrom(address _from, address _to, uint256 _id, uint256 _value, bytes calldata _data) external {
        ...
        if (isNonFungible(_id)) {
            require(_value == 1, "Transfer amount needs to be 1 for non fungibles");
...
*/
contract ArkaneERC1155MixedFungibleMintable is ERC1155MixedFungible, Owned {
   ...
   * @return string representing the name of the grouping of the tokens (ex. game)
   */
   ...
   * @return string representing the symbol of the grouping of the tokens (ex. game)
   */
  function symbol() external view returns (string memory) {
    ...
    /**
   * @dev Returns an URI for a given type ID
   * @param _type uint256 ID of the type to query
   */
  function uri(uint256 _type) public view returns (string memory) {
    return _uris[_type];
  }

  /**
   * @dev Internal function to set the token URI for a given token
   * @param _type uint256 ID of the type to set its URI
   * @param _uri string URI to assign
   */
  function setUri(uint256 _type, string memory _uri) public onlyOwner {
    _uris[_type] = _uri;
    emit URI(_uri, _type);
  }
