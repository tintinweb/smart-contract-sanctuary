...
contract MasterContractManager is BoringOwnable, BoringFactory {
    ...
    /// @notice Other contracts need to register with this master contract so that users can approve them for the AntiqueBox.
...
/// @notice The AntiqueBox is a vault for tokens. The stored tokens can be flash loaned and used in strategies.
