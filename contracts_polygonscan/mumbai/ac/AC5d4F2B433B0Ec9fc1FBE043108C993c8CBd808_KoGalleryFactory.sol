...
contract KoGalleryFactory is NativeMetaTransaction {
    address public constant CHILD_CHAIN_MANAGER_PROXY_POLYGON = 0xb5505a6d998549090530911180f38aC5130101c6; // Testnet

    event DeployERC721(address indexed _from, address indexed _galleryContractAddress);

    mapping(address=>address) public userGallery;
    mapping(address => bool) public storedGallery;

    constructor(){
        _initializeEIP712("KoNFTFactory");
    }

    modifier userHaveNotContract(address _userAddress){
        address contractAddress = userGallery[_userAddress];
        bool exist = storedGallery[contractAddress];
        require(!exist,"You have already gallery.");
        _;
    }

    function deployERC721MetaTransaction(string memory _token, string memory _trigram, address _userAddress) external userHaveNotContract(_userAddress) returns (address) {
        address galleryContractAddress = internalDeployERC721(_token, _trigram, _userAddress);
        return address(galleryContractAddress);
    }

    function deployERC721(string memory _token, string memory _trigram) public userHaveNotContract(msg.sender) returns (address) {
        address galleryContractAddress = internalDeployERC721(_token, _trigram, msg.sender);
        return address(galleryContractAddress);
    }

    function internalDeployERC721(string memory _token, string memory _trigram, address _userAddress) internal  returns (address){
        ChildMintableERC721 galleryContract = new ChildMintableERC721(_token, _trigram, CHILD_CHAIN_MANAGER_PROXY_POLYGON);
        userGallery[_userAddress] = address(galleryContract);
        storedGallery[address(galleryContract)] = true;
        emit DeployERC721(_userAddress, address(galleryContract));
        return address(galleryContract);
    }
}