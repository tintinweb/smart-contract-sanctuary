...
import "../proxies/INewentProxy.sol";
...
contract pMintysaleAdvanced  is ContextMixin, NativeMetaTransaction {
    ...
    constructor(IERC20 _weth, address wallet, uint256 _ownerPerMille, uint256 _creatorPerMille, uint _divisor, INewentProxy _registry) {
    ...
    function offerNew(uint256 tokenId, string memory ipfsString, uint256 price) external {
        ...
        require(registry.isMinter(msgSender()),"You are not allowed to mint tokens here");
    ...
    function migrate(address oldContract, uint tokenId) external onlyOwner {
        pMintysaleAdvanced old_token = pMintysaleAdvanced(oldContract);
        //creator address, itemHash string, price uint256, available bool, minted bool
        (address creator, string memory itemHash, uint256 price, bool available, bool minted) = old_token.items(tokenId);
        items[tokenId] = Offer(creator, itemHash,price, available,minted);
    }
    ...
    function price(uint256 tokenId) external view returns (uint256) {
        Offer memory offer = items[tokenId];
        require(offer.available,"item not available");
        require(token.ownerOf(tokenId) == items[tokenId].creator,"Person offering is not the Owner");
        require(
            token.isApprovedForAll(offer.creator, address(this)) ||
            (token.getApproved(tokenId) == address(this)),
            "Owner has not approved sale contract to sell this item" );
        return offer.price;
    }
...
interface INewentProxy  {

    // This person authorised the contract to trade for them
    function authorised(address _owner) external view returns (bool) ;
