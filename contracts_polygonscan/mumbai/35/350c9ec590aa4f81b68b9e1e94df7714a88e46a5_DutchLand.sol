...
contract DutchToken is ERC20('DutchToken', 'DUTCH') {
...
// DutchLand is the master of DUTCH. He can make dutch and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once dutch is sufficiently
// distributed and the community can show to govern itself.
// Have fun reading it. Hopefully it's bug-free. God bless.
contract DutchLand is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        uint256 accDutchPerShare;   // Accumulated Yieldes per share, times 1e18. See below.
    ...
    // The Dutch TOKEN!
    DutchToken public immutable Dutch;
    address public devAddress;
    address public feeAddress;
    

    // Dutch tokens created per block.
    uint256 public DutchPerBlock = 8 ether;
    
    uint256 constant MAX_DUTCH_PERBLOCK= 12 ether;
    ...
    // Update the given pool's Dutch allocation point and withdraw fee. Can only be called by the owner.
    ...
    // View function to see pending Dutches on frontend.
    function pendingDutch(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0 && totalAllocPoint>0) {
            ...
            accDutchPerShare = accDutchPerShare.add(DutchReward.mul(1e18).div(lpSupply));
        }
        return user.amount.mul(accDutchPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        Dutch.mint(address(this), DutchReward.mul(88).div(100));
        // Automatically burn 2% of minted tokens
        Dutch.mint(BURN_ADDRESS, DutchReward.mul(20).div(1000));
        pool.accDutchPerShare = pool.accDutchPerShare.add(DutchReward.mul(88).div(100).mul(1e18).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for Dutch allocation.
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accDutchPerShare).div(1e18).sub(user.rewardDebt);
            if (pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accDutchPerShare).div(1e18);
    ...
    // Safe Dutch transfer function, just in case if rounding error causes pool to not have enough Dutch.
    ...
    function updateEmissionRate(uint256 _DutchPerBlock) external onlyOwner {
        require(_DutchPerBlock<=MAX_DUTCH_PERBLOCK,"Too high");
        massUpdatePools();
