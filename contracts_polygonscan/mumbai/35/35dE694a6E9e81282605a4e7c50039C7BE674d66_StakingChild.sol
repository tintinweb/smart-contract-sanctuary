...
contract StakingChild is Ownable, FxBaseChildTunnel {
    ...
    event StakeReceived(address from, uint256 amount);
    event UnstakeReceived(address from, uint256 amount);
    ...
    function _processMessageFromRoot(
        ...
        if (_staking) {
            ...
            emit StakeReceived(_staker, _amount);
        } else {
            _unstake(_staker, _amount);
            emit UnstakeReceived(_staker, _amount);
        }
    }
}
