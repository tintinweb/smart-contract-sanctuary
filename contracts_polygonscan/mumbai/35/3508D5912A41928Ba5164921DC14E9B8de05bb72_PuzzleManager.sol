...
contract PuzzleManager is VRFConsumerBase, Pausable {
    ...
    mapping(uint256 => uint256[]) public puzzleGroupToOngoingPuzzleIds;
    mapping(uint256 => string[]) public puzzleIdToPieceCIDs;
    mapping(address => mapping(string => bool)) public winningsForUser;
    ...
    // Events
    event PuzzleStarted(uint256 puzzleGroupId, uint256 puzzleId);
    event PuzzleEnded(
        uint256 puzzleGroupId,
        uint256 puzzleId,
        address winner,
    ...
    function startNewPuzzle(
        ...
        if (puzzleGroupToOngoingPuzzleIds[puzzleGroupId].length == 0) {
            activePuzzleGroups.push(puzzleGroupId);
        }

        puzzleGroupToOngoingPuzzleIds[puzzleGroupId].push(puzzleId);
        puzzleIdToPieceCIDs[puzzleId] = pieces;

        emit PuzzleStarted(puzzleGroupId, puzzleId);
    }
    ...
    function batchStartNewPuzzles(
        ...
        for (uint256 i = 0; i < length; i++) {
            startNewPuzzle(puzzleGroupId, puzzleIds[i], pieces[i]);
        }
    }
    ...
    function buyPackForTier(
        ...
        require(
            puzzleGroupToOngoingPuzzleIds[puzzleGroupId].length > 0,
        ...
        returns (uint256[] memory)
    {
        ...
        uint256[] memory randomPuzzleIds = puzzleGroupToOngoingPuzzleIds[
            pack.puzzleGroupId
        ];
        require(
            puzzleGroupToOngoingPuzzleIds[pack.puzzleGroupId].length > 0,
        ...
        for (uint256 i = 0; i < numPieces; i++) {
            ...
            // Pick random puzzle ID in puzzle group
            uint256 randomPuzzleIdIndex = random % randomPuzzleIds.length;
            uint256 randomPuzzleId = randomPuzzleIds[randomPuzzleIdIndex];

            // Pick random piece in puzzle ID pieces
            string[] memory randomPuzzlePieces = puzzleIdToPieceCIDs[
                randomPuzzleId
            ];
     ...
     * @param oldPuzzlePrize the IPFS CID of the prize for the old puzzle
    ...
    function replaceOrEndPuzzle(
        ...
        uint256[] memory puzzleIds = puzzleGroupToOngoingPuzzleIds[
            puzzleGroupId
        ];
        require(
            puzzleIds.length > 0,
        ...
        for (uint256 i = 0; i < puzzleIds.length; i++) {
            if (puzzleIds[i] == oldPuzzleId) {
                puzzleGroupToOngoingPuzzleIds[puzzleGroupId][
                    i
                ] = puzzleGroupToOngoingPuzzleIds[puzzleGroupId][
                    puzzleIds.length - 1
                ];
                puzzleGroupToOngoingPuzzleIds[puzzleGroupId].pop();
        ...
        // Set winning
        winningsForUser[oldPuzzleWinner][oldPuzzlePrize] = true;
        ...
        if (newPuzzleId > 0 && newPuzzlePieces.length > 0) {
            startNewPuzzle(puzzleGroupId, newPuzzleId, newPuzzlePieces);
        }
    }
     ...
     * @param prize IPFS CID of the final prize NFT
     */
    function claimPrize(address recipient, string memory prize)
        public
        whenNotPaused
    {
        if (!whitelistedAddress[msg.sender]) {
        ...
        require(
            winningsForUser[recipient][prize] == true,
            "PuzzleManager: the winning does not exist or has already been claimed"
        );

        uint256 tokenId = _mintPrize(recipient, prize);
        winningsForUser[recipient][prize] = false;

        emit PrizeClaimed(recipient, tokenId, prize);
    }
    ...
    ERC721Pausable
{
        ...
        returns (string memory)
    {
        ...
        string memory _tokenURI =
            string(abi.encodePacked(IPFS_TOKEN_BASE_URI, _tokenURIs[tokenId]));
