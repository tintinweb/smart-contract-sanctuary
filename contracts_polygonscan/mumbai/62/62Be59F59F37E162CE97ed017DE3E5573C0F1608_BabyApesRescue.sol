...
contract BabyApesRescue is ERC721Namable, Ownable {
    ...
    address public constant SIGNER =
        address(0x5E5e683b687f509968D90Acd31ce6b8Cfa3d25E4);
    ...
    // Track Ids of migrated Baby Apes OG
    ...
    uint256 public maticMintFee = 150 ether;
    ...
    function getDiscountEligiblity(address user) public view returns (bool) {
    ...
    function isMonster(uint256 _id) public pure returns (bool) {
        if (_id <= 2500 || _id > 25000) {
            return false;
        } else return (_id % 6 == 0 ? true : false);
    }
    ...
    function migrateMint(
        ...
        require(
            keccak256(abi.encodePacked(_to, _id))
                .toEthSignedMessageHash()
                .recover(_sig) == SIGNER,
            "Invalid Signature"
        );
        require(_to == msg.sender, "Caller is not eligible to migrate");

        for (uint256 i = 0; i < _id.length; i++) {
            require(
                _id[i] > 0 && _id[i] <= 2500,
                "Can only migrate Baby Apes OG"
            );
            ...
            ogBalance[msg.sender]++;
            ogList.push(_id[i]);
            _safeMint(msg.sender, _id[i]);

            // Reward banana bar for migrating
            bananaBar.rewardOnMint(msg.sender);
        }
    }
    ...
    function mint(uint256 _count) external {
        ...
        if (msg.sender != owner()) {
            ...
            bool canAvailDiscount = getDiscountEligiblity(msg.sender);
    ...
    function mintMatic(uint256 _count) external payable {
        require(msg.sender == tx.origin, "No contract minting allowed!");
        require(
            maxTokenIdMinted + _count <= 10000,
            "Can only mint 10,000 Baby Apes with matic"
        );
    ...
    function handleMint(address _to, uint256 _count) private {
        bool isSteal = false;
