...
contract FunkyPunks {
    ...
    function setMainContract(address _address) public {
        require(msg.sender == owner, "Funky Punks: You are not the owner");
        owner = _address;
    }
...
contract FNK is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    constructor(string memory baseURI) ERC721("Funky Punks", "FNK")  {
        setBaseURI(baseURI);
    }

    function mint(uint _count) public payable nonReentrant {
        require(totalSupply() + _count <= MAX, "Funky Punks: Not enough left to mint");
        require(totalSupply() < MAX, "Funky Punks: Not enough left to mint");
        require(_count <= 5, "Funky Punks: Exceeds the max you can mint");
        require(msg.value >= price(_count), "Funky Punks: Value below price");
        ...
        if (block.timestamp >= _saleTime) {
            ...
            require(_count <= FunkyPunks(_presaleContract).isInPresale(msg.sender), "Funky Punks: Exceeds the max you can mint in the presale");
            require(FunkyPunks(_presaleContract).isInPresale(msg.sender) > 0, "Funky Punks: You are not in the presale");
            require(FunkyPunks(_presaleContract).isInMintedPresale(msg.sender) == false, "Funky Punks: You already minted from the presale");
            for (uint x = 0; x < _count; x++) {
                _safeMint(msg.sender, totalSupply());
            }
            FunkyPunks(_presaleContract).addToMinted(msg.sender);
        }
    }
    ...
    function getReserves44() public onlyOwner {
        require(totalSupply() + 44 <= MAX, "Funky Punks: Not enough left to mint");
        for (uint x = 0; x < 44; x++) {
            _safeMint(msg.sender, totalSupply());
        }
    }
