...
// File contracts/PolkaPetBooster.sol
...
contract PolkaPetBooster is VRFConsumerBase, Ownable, ERC1155Holder, ReentrancyGuard, Pausable {
	...
	address public polkapetNFT;

	uint256[] public basicCards;
	uint256[] public rareCards;
	uint256 public availablePacks;

	mapping(bytes32 => address) public requestByMembers;
	...
	{
		keyHash = 0x6e75b569a01ef56d18cab6a8e71e6600d6ce853834d4a5748b720d06f878b3a4; // Polygon Testnet
		fee = 0.0001 * 10**18; // 0.0001 LINK (Polygon)

		lootBoxNFT = _lootBoxNFT;
		polkapetNFT = _polkapetNFT;
	}
	...
	function openPack(uint256 id) external whenNotPaused nonReentrant {
		require(IERC1155(lootBoxNFT).balanceOf(msg.sender, id) > 0, "PolkaPetBooster: insufficient funds");
		require(
			IERC1155(lootBoxNFT).isApprovedForAll(msg.sender, address(this)),
			"PolkaPetBooster: should be approved"
		);
		require(availablePacks > 0, "PolkaPetBooster: no available packs");

		bytes32 requestId = getRandomNumber(msg.sender);
		requestByMembers[requestId] = msg.sender;

		IChildToken(lootBoxNFT).burn(msg.sender, id, 1);
		availablePacks = availablePacks.sub(1);

		emit OpenPackRequest(msg.sender, requestId);
	}
	...
	function batchAddCards(
		...
		if (rareCards.length > 0)
			resetCards(rareCards);

		if (basicCards.length > 0)
			resetCards(basicCards);

		batchAddFor(rareCards, rareIds, rareAmounts);
		batchAddFor(basicCards, basicIds, basicAmounts);

		require(
			rareCards.length == basicCards.length.div(2),
			"PolkaPetBooster: not compatible length of arrays rare and basic cards"
		);
		availablePacks = rareCards.length;
	}

	function batchAddFor(
		uint256[] storage cards,
		uint256[] memory ids,
		uint256[] memory amounts
	) internal {
		require(ids.length == amounts.length, "PolkaPetBooster: different length or array");
	...
	function resetCards(uint256[] storage cards) internal {
	...
	function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
		address reciepient = requestByMembers[requestId];
		...
		randoms = expand(randomness, 3);

		uint256 randIndex;
		uint256 nftId;

		for (uint256 i = 0; i < 3; i++) {
			cards = i == 0 ? rareCards : basicCards;

			randIndex = randoms[i].mod(cards.length);
			nftId = cards[randIndex];

			ids[i] = nftId;
			amounts[i] = 1;
			_remove(cards, randIndex);
		}

		IERC1155(polkapetNFT).safeBatchTransferFrom(address(this), reciepient, ids, amounts, "0x");

		emit OpenedPack(reciepient, ids[0], ids[1], ids[2]);
	}

	/**
	 * Requests randomness from a user-provided seed
	 */
	function getRandomNumber(address user) internal virtual returns (bytes32 requestId) {
		require(
			LINK.balanceOf(address(this)) >= fee,
			"Not enough LINK - fill contract with faucet"
		);

		uint256 userProvidedSeed = uint256(keccak256(abi.encode(block.timestamp, user)));
