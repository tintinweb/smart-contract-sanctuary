...
// Last Updated: Purrnelopes Game 2
// Fast games, free packs, slowmode
contract PuzzleManager is VRFConsumerBase, Pausable, Ownable {
    // Enums
    // enum PackTier {
    //     BRONZE,
    //     SILVER,
    ...
    mapping(PackTier => uint256) public packTradeInPrices;
    ...
    constructor(
        ...
        // packContents[PackTier.BRONZE] = 15;
        // packContents[PackTier.SILVER] = 25;
         packContents[PackTier.GOLD] = 25; // Used to be 50 pieces per GOLD pack, now 25 for this drop

        // packPrices[PackTier.BRONZE] = 0 ether; // this will actually be used in DAI so 1 ether = 1 DAI
        // packPrices[PackTier.SILVER] = 0 ether;
        packPrices[PackTier.GOLD] = 0 ether;

        // packTradeInPrices[PackTier.BRONZE] = 45;
        // packTradeInPrices[PackTier.SILVER] = 75;
        packTradeInPrices[PackTier.GOLD] = 150;

        whitelistedAddress[owner()] = true;

        // prizeFactory = new PrizeFactory(
        //     "NFT Puzzle Thing - Prizes",
        //     "NPTPrizes"
        // );
        // prizeFactory.transferOwnership(owner());

        // pieceFactory = new PieceFactory(
        //     "NFT Puzzle Thing - Pieces",
        //     "NPTPieces"
        // );
        // pieceFactory.transferOwnership(owner());

        // These contracts already exist, we dont need to redeploy
        prizeFactory = PrizeFactory(0x9F70aA883f593F0Be14268e39d8C1e43911e041a);
        pieceFactory = PieceFactory(0x2f2e41B277912F11e7C5761A4493e3388724233F);
     ...
     * @param maxWinners uint8 number of winners allowed for a puzzle
    ...
    function buyPackForTier(
        ...
        require(packPurchaseStatusForGroup[puzzleGroupId], "PuzzleManager: Pack purchase is currently disabled for this puzzle group ID");    
        // When user's time is less than block's time, allow the user to purchase a pack
        if(slowMode) {
            require(usersTimers[recipient] < block.timestamp, "PuzzleManager: SlowMode is activated please wait for required time");
    ...
    function fulfillListing(
        ...
        listingsForOwner[seller][sellerTokenId][buyerTokenIdTokenURI] = false;
    ...
    function replaceOrEndPuzzle(
        ...
        for (uint8 i = 0; i < puzzles.length; i++) {
            ...
            if (currPuzzleId == oldPuzzleId) {
                ...
                uint8 winnerIndex = puzzle.winnerIndex;
    ...
    /**
     * @dev Trades in expired pieces for a new pack
     * @param pieceIds array of piece token id's that are to be traded in
     * @param tier of pack to trade in for
     * @param puzzleGroupId for group to buy the pack for
     * @param recipient address
     */
    function tradeInPiecesForPack(
        uint256[] memory pieceIds,
        PackTier tier,
        uint256 puzzleGroupId,
        address recipient
    ) public whenNotPaused returns (bytes32[] memory) {
        if (!whitelistedAddress[msg.sender]) {
            require(
                msg.sender == recipient,
                "PuzzleManager: non owners can only trade in their own pieces"
            );
        }

        uint256 length = pieceIds.length;
        require(
            length % packTradeInPrices[tier] == 0,
            "PuzzleManager: given tokenIds not a multiple of trade in price"
        );

        uint256 numPacksToReceive = length / packTradeInPrices[tier];

        // burn pieces 
        _burnPieces(pieceIds, recipient);

        bytes32[] memory requestIds = new bytes32[](numPacksToReceive);

        for (uint256 i = 0; i < numPacksToReceive; i++) {
            requestIds[i] = _requestPackPurchase(
                puzzleGroupId,
                tier,
                recipient
            );
        ...
        returns (uint256[] memory)
    {
        ...
        require(length > 0, "PuzzleManager: pieces length must be greater than 0");
    ...
    function removeWhitelistedAddress(address whitelist) public onlyWhitelisted {
    ...
    function setPackTierTradeInPrice(uint256 _tradeInPrice, PackTier _tier)
        public
        onlyWhitelisted
    {
        packTradeInPrices[_tier] = _tradeInPrice;
    }
