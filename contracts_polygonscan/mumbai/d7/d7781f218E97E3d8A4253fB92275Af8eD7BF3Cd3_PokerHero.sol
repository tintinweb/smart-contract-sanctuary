...
contract PokerHero is Ownable, ERC721Namable{
    ...
    uint256 constant PRICE500 = 1 ether / 10;
    uint256 constant PRICE9500 = 2 ether / 10;

    uint256 constant PRICEFORCHANGENAME = 50 ether;

    uint256 private startSale = 1639482595;
    uint256 private stopSale = 1640358000;
    ...
    function mint(uint256 _id) public payable {
        require(MAX_NFT != ERC721.totalSupply, "NFT are over! Sorry :(");
        //require(startSale <= block.timestamp, "Mint is not availble now!");
        //require(stopSale >= block.timestamp, "Mint time is over!");
        require(balanceOf(msg.sender) + 1 <= 10, "More than 10 NFTs");

        if(ERC721.totalSupply <= 5){
    ...
    function mintBatch(uint256[] memory _ids) public payable{
        require(MAX_NFT != ERC721.totalSupply, "NFT are over! Sorry :(");
        //require(startSale <= block.timestamp  && stopSale >= block.timestamp, "Mint time is over!");
        require(balanceOf(msg.sender) + _ids.length <= 10, "More than 10 NFTs");

        uint256 _count = (ERC721.totalSupply + _ids.length) <= MAX_NFT ? _ids.length : MAX_NFT - ERC721.totalSupply;
