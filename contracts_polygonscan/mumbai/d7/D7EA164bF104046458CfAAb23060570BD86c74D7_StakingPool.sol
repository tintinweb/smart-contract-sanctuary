...
contract StakingPool is IStakingPool, PermissionGroup {
    ...
    uint[8] public payAmountForUpStar = [100, 300, 1500, 15000, 30000, 60000, 120000, 240000];
    ...
    uint[10] expFactor = [120, 120, 116, 112, 108, 108, 108, 104, 104, 104];
     ...
     */
    function setPayAmountForUpStar(uint[8] memory amount) external onlyOwner {
        payAmountForUpStar = amount;
    }
    
    /**
     * @dev Owner set expFactor value
     */
    function setExpFactos(uint[10] memory factors) external onlyOwner {
        expFactor = factors;
    }
     ...
     * @dev Owner set BASE_APY value
     */
    function setHeroLevel(uint heroId, uint level) external onlyOperator {
        uint currentStar = kabyHeroContract.getHeroStar(heroId);
        require(level <= currentStar * 10, "StakingPool: invalid level");
        heroLevel[heroId] = level;
        emit SetHeroLevel(heroId, level);
    }
    ...
    function claim(uint heroId) external override {
        ...
        stakingHero.reward = 0;
        rewardDistributorContract.distributeReward(account, reward);
        emit Claimed(heroId, account, reward, heroLevel[heroId]);
    }
    
    /**
     * @dev Internal function to calculate and get range of level that hero can upgrade
     */
    function searchLevelIndexByExp(uint totalExp) private returns(uint) {
        uint levelIndex = 1;
        uint maxLevelIndex = 90;
        if(totalExp >= expValue[9] * 1e18)
        {
        ...
        { 
            uint minArray = 0;
            uint maxArray = expValue.length - 1;
            while(minArray < maxArray)
            {
                ...
                else if (totalExp >= expValue[midArray] * 1e18)
                {
                    minArray = midArray;
                }
                else
    ...
    function getMaxLevelToUp(
        uint heroId,
        uint levelIndex,
        uint totalExp) private returns(uint) {
            
        uint maxLevel = levelIndex;
        uint expFactorValue = expFactor[levelIndex / 10];
        uint currentStar = kabyHeroContract.getHeroStar(heroId);
        ...
        uint minExp = expValue[levelIndex / 10] * 1e18;
        uint minExpToLevelUp = earnExpToUps[levelIndex / 10] * 1e18;
    ...
    function convertExpToLevels(uint heroId) private {
        ...
        uint levelIndex = searchLevelIndexByExp(totalExp);

        uint maxLevelToUp = getMaxLevelToUp(heroId, levelIndex, totalExp);
    ...
    function upgradeStarForHero(uint heroId) external override {
        ...
        require(heroLevel[heroId] == currentStar * 10, "StakingPool: Cannot upgrade star for this hero");
        
        uint amount = payAmountForUpStar[currentStar - 1] * 10 ** ERC20(address(acceptedToken)).decimals();
        acceptedToken.safeTransferFrom(account, owner(), amount);
        
        heroLevel[heroId] = 1;
        heroCurrentExp[heroId] = 0;
        kabyHeroContract.upgradeStar(heroId, 1);
        emit HeroStarUpgraded(heroId, currentStar + 1, 1);
    }
     ...
     */
    function earned(uint heroId, address account) public view override returns (uint expEarned, uint tokenEarned) {
        require(heroId < kabyHeroContract.totalSupply(), "StakingPool: invalid hero");
        ...
        expEarned = stakingDuration * BASE_APY * KABY_TO_EXP / ONE_YEAR_IN_SECONDS / 100;
    ...
    function _harvest(uint heroId, address account) private {
        (uint expEarned, uint tokenEarned) = earned(heroId, account);
...
interface IStakingPool {
    ...
    event Claimed(uint indexed heroId, address indexed account, uint reward, uint level);
    event UpdateKabyHero(address kabyHeroAddr);
    event UpdateEndTime(uint endTime);
    event SetHeroLevel(uint heroId, uint level);
    event HeroStarUpgraded(uint indexed heroId, uint newStar, uint amount);
    ...
    /**
     * @notice Gets amount of KABY token and exp earned by a hero so far.
     */
    function earned(uint heroId, address account) external view returns (uint expEarned, uint tokenEarned);
