    ...
    IGame
{
    ...
    event EndGame(uint256 indexed gId, uint8 indexed rId);
    ...
    {
        address player = games[_rId].players[_position];
        uint8[] storage mainHand = playerHand[player][_rId];

        require(playerSplit[player][_rId].length == 0, "BJ34");
    ...
    function _split(
        ...
        uint8[] storage mainHand = playerHand[player][_rId];
        uint8[] storage splitHand = playerSplit[player][_rId];
    ...
    function getHand(
        ...
        cards = hasSplit(_rId, _position)
            ? playerSplit[_player][_rId]
            : playerHand[_player][_rId];
    }
        ...
        private
    {
        ...
        for (uint8 i; i < _leftPlayers.length; i++) {
            ...
            if (playerInsurance[player][_rId] == true) {
                _payout(_rId, player, bet);
            }
        }
    }
