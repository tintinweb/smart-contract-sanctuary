...
// File contracts/roles/AccessOperatableWithMinter.sol
...
// File contracts/erc/ERC721MintableByMinter.sol
...
// File contracts/erc/ERC2981.sol
...
abstract contract ERC2981 is IERC2981, ERC165Storage {
    ...
    mapping(uint256 => address) internal receiverById;
    mapping(uint256 => uint256) internal royaltyPercentage;

    address internal _baseReceiver;
    uint256 internal _baseRoyaltyPercentage;

    constructor(address baseReceiver, uint256 baseRoyaltyPercentage) {
        // Using ERC165Storage set EIP-2981
        _registerInterface(_INTERFACE_ID_ERC2981);

        _baseReceiver = baseReceiver;
        _baseRoyaltyPercentage = baseRoyaltyPercentage;
    }
    ...
    returns (address receiver, uint256 royaltyAmount)
    {
        receiver = _receiver(_tokenId);
        royaltyAmount = _royaltyAmount(_tokenId, _salePrice);
    }

    function _receiver(uint256 _tokenId) private view returns (address) {
        address receiver_ = receiverById[_tokenId];
    ...
    function _royaltyAmount(uint256 _tokenId, uint256 _salePrice) private view returns (uint256) {
        uint256 _royaltyPercentage = royaltyPercentage[_tokenId];
        if (_royaltyPercentage == uint256(0)) {
            return (_salePrice * _baseRoyaltyPercentage) / 100;
        }
        return (_salePrice * _royaltyPercentage) / 100;
    }
}
...
// File contracts/NFTStudioToken.sol

pragma solidity ^0.8.0;


interface INFTStudioToken721 is IERC721 {
    function setDefaultURI(string memory _defaultURI) external;

    function setReceiver(uint256 _tokenId, address _address) external;

    function setRoyaltyPercentage(uint256 _tokenId, uint256 _royaltyPercentage) external;
}

abstract contract NFTStudioToken721 is
ERC721Mintable,
ERC721Permit,
ERC721URIStorage,
INFTStudioToken721,
...
ERC2981
{
    ...
    function setDefaultURI(string memory defaultURI_)
    public
    override
    onlyOperator
    {
    ...
    function _beforeTokenTransfer(
        ...
        require(!_paused, "AccessOperatable: token transfer while paused");
    }
    ...
    // call ERC2981 internal _setReceiver method with onlyOperator
    function setReceiver(uint256 _tokenId, address _address)
    public
    override
    onlyOperator
    {
        return super._setReceiver(_tokenId, _address);
    }

    // call ERC2981 internal _setReceiver method with onlyOperator
    function setRoyaltyPercentage(uint256 _tokenId, uint256 _royaltyPercentage)
    public
    override
    onlyOperator
    {
        return super._setRoyaltyPercentage(_tokenId, _royaltyPercentage);
    }
    ...
    view
    virtual
    override(ERC721, ERC721Mintable, IERC165)
    returns (bool)
    {
        return super.supportsInterface(interfaceId);
    }
...
contract NFTStudioToken is NFTStudioToken721 {
    constructor()
    ERC721("NFTStudio:Token", "STD")
    ERC721Permit("NFTStudio:Token")
    ERC2981(_msgSender(), 5)
    {
