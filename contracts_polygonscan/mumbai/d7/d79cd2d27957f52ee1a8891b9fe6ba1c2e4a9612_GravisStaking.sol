...
contract GravisStaking is Ownable {
    ...
    constructor(
        ...
        require(rewardStart_ >= block.number, "Reward start too early");
    ...
    function rewardOf(address account) public view returns (uint256) {
        ...
        if (block.number > lastRewardDistribution && totalStakes > 0) {
            currentRewardPerShare += ((rewardPerBlock *
                (block.number - lastRewardDistribution) *
                MAGNITUDE) / totalStakes);
        }
    ...
    function _distributeRewards() private {
        if (block.number > lastRewardDistribution) {
            if (totalStakes > 0) {
                _magnifiedRewardPerShare += ((rewardPerBlock *
                    (block.number - lastRewardDistribution) *
                    MAGNITUDE) / totalStakes);
            }
