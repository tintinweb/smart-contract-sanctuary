 ...
 */
contract AuctionHouse is IAuctionHouseV2, ReentrancyGuard {
     ...
     * Constructor
     */
    constructor(address _zora, address _weth, address bidDb, uint256 startAuctionId) public {
        require(
        ...
        // production enviroment: startAuctionId=1
        // local test enviroment: startAuctionId=0
         _auctionIdTracker._value = startAuctionId;

        _bidDb = IBidDbV2(bidDb);
    }

    function getBidDb() external view override returns(address) {
        return address(_bidDb);
    }

    function getCurrentAuctionId() external view override returns(uint256) {
        return _auctionIdTracker.current();
    }

    function getAuctionIds() external view override returns(uint256[] memory) {
        return _auctionIds;
    }

    function getAllNft() external view override returns(address[] memory tokens, uint256[] memory tokenIds, uint256[] memory auctionIds) {
    ...
    function getNftByToken(address token) external view override returns(uint256[] memory tokenIds, uint256[] memory auctionIds) {
        uint256 count = 0;
    ...
    function getNftByOwner(address token, address owner) external view override returns(uint256[] memory tokenIds, uint256[] memory auctionIds) {
        uint256 count = 0;
        for (uint256 i = 0; i < _auctionIds.length; i++) {
            Auction storage auction = auctions[_auctionIds[i]];
    ...
    function getAuction(uint256 auctionId) external view override returns(IAuctionHouseV2.Auction memory) {
        return auctions[auctionId];
    }

    function getAuctions() public view override returns(uint256[] memory auctionIds, IAuctionHouseV2.Auction[] memory auctions_) {
        uint256 count = _auctionIds.length;
        auctionIds = new uint256[](count);
        auctions_ = new IAuctionHouseV2.Auction[](count);
    ...
    function getAuctionsByAccount(address account) external view override returns(uint256[] memory auctionIds, IAuctionHouseV2.Auction[] memory auctions_) {
        uint256 count = 0;
        for (uint256 i = 0; i < _auctionIds.length; i++) {
            Auction storage auction = auctions[_auctionIds[i]];
    ...
    function getAuctionByTokenId(address token, uint256 tokenId) external view override returns(uint256 auctionId, Auction memory auction, IBidDb.Bid[] memory bids) {
        for (uint256 i = 0; i < _auctionIds.length; i++) {
    ...
    function createAndApproveAuction(
        ...
        uint256 auctionId = createAuction(tokenId, tokenContract, duration, reservePrice, curator, curatorFeePercentage, auctionCurrency);
        _approveAuction(auctionId, true);
        return auctionId;
    }
