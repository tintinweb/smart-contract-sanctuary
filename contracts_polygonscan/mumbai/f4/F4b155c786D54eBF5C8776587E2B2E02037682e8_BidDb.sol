...
contract BidDb is IBidDbV2, Ownable {
    // auction id => IAuctionHouse.Auction
    mapping(uint256 => IAuctionHouse.Auction) internal _auctions;

    // auction id => IBidDb.Status
    mapping(uint256 => IBidDb.Status) internal _auctionStatus;
    ...
    function getTokenIds(address token) external view override returns(uint256[] memory tokenIds, uint256[] memory auctionIds) {
        tokenIds = _tokenIds[token];
        
        uint256 count = tokenIds.length;
        auctionIds = new uint256[](count);
        for (uint256 i = 0; i < count; i++) {
            uint256[] storage auctionIds_ =_auctionIds[token][tokenIds[i]];
            if (auctionIds_.length > 0) {
                auctionIds[i] = auctionIds_[auctionIds_.length - 1];
            }
             
        }
    }

    function getTokenIds(address token, uint256 offset, uint256 limit) external view override returns(uint256[] memory tokenIds, uint256[] memory auctionIds) {
        require(limit > 0, "limit == 0");
        uint256[] storage tokenIds_ = _tokenIds[token];
        require(offset < tokenIds_.length, "offset is too big");

        uint256 maxLimit = tokenIds_.length - offset;
        if (limit > maxLimit) {
            limit = maxLimit;
        }

        tokenIds = new uint256[](limit);
        auctionIds = new uint256[](limit);
        for (uint256 i = 0; i < limit; i++) {
            uint256 tokenId = tokenIds_[offset];
            tokenIds[i] = tokenId;
            uint256[] storage auctionIds_ =_auctionIds[token][tokenId];
            if (auctionIds_.length > 0) {
                auctionIds[i] = auctionIds_[auctionIds_.length - 1];
            }
            offset++;
    ...
    function getAuction(uint256 auctionId) external view override returns (
        ...
        IBidDb.Status memory, 
        Bid[] memory) 
    {
        return (_auctions[auctionId], _auctionStatus[auctionId], _bids[auctionId]);
    }

    function getAuctionId(address token, uint256 tokenId) external view override returns(uint256) {
        uint256[] storage auctionIds = _auctionIds[token][tokenId];
        return (auctionIds.length > 0) ? auctionIds[auctionIds.length - 1] : 0;
    }
        ...
        Bid[] memory bids) 
    {
        ...
        auctionId = auctionIds[auctionIds.length - 1];
        auction = _auctions[auctionId];
        status = _auctionStatus[auctionId];
        bids = _bids[auctionId];
    }
            ...
            onlyOwner 
    {
        ...
        IBidDb.Status storage auctionStatus = _auctionStatus[auctionId];
        auctionStatus.beginTime = block.timestamp;
    ...
    function createBid(uint256 auctionId, address bidder, uint256 amount, uint256 duration) external override onlyOwner {
        ...
        IBidDb.Status storage auctionStatus = _auctionStatus[auctionId];
        auctionStatus.winner = bidder;
        auctionStatus.count = uint88(_bids[auctionId].length);
        auctionStatus.state = State.InBid;
    }

    function cancelAuction(uint256 auctionId) external override onlyOwner {
        _auctionStatus[auctionId].state = State.Cancel;
    }
