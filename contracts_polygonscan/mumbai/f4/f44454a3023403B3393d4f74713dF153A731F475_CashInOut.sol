    ...
    ICashInOut
{
    ...
    mapping(uint256 => mapping(string => Game)) public games; //gameid=>roomId => Game
    mapping(uint256 => GameInfo) public gameInfo; //gameid => gameInfo
    mapping(uint256 => mapping(string => uint128)) internal reserves; //gId => rId => reserve
    ...
    event SetFreeGame(uint256 indexed gId, bool isFree);

    event SetMaxPlayer(uint256 indexed gId, uint8 max);

    event SetMaxMinBet(uint256 indexed gId, uint128 minBet, uint128 maxBet);

    event SetGameRewardMultiplier(uint256 indexed gId, uint128 multiplier);
    ...
    function setFreeGame(uint8 _gId, bool _bool) external onlyOwner whenPaused {
        gameInfo[_gId].isFreeGame = _bool;
        emit SetFreeGame(_gId, _bool);
    }

    function setGameRewardMultiplier(uint8 _gId, uint128 _multiplier)
        external
        onlyOwner
        whenPaused
    {
        require(_multiplier > 0, "CashInOut.sol: Multiplier can't be zero.");
        gameInfo[_gId].rewardMultiplier = _multiplier;
        emit SetGameRewardMultiplier(_gId, _multiplier);
    }
    ...
    function setMinMaxBet(
        ...
        require(
            _minBet < _maxBet,
            "CashInOut.sol: minBet can not be more than maxBet."
        );
    ...
    function registerGame(
        ...
        require(
            _maxPlayer >= 1,
            "CashInOut.sol: maxPlayer must be more than 0."
        );

        GameInfo storage info = gameInfo[_gId];

        require(
            info.status == GameStatus.Empty,
            "CashInOut.sol: game has been registered."
        );
        ...
        info.minBet = _minBet;
        info.maxBet = _maxBet;
        info.isFreeGame = _isFreeGame;
        info.status = GameStatus.Registered;
    ...
    function joinGame(
        ...
        uint256 _gId,
        string memory _rId
    ) external override whenNotPaused onlyRole(WORKER_ROLE) {
        Game storage game = games[_gId][_rId];
        GameInfo storage info = gameInfo[_gId];

        require(
            _amount >= info.minBet && _amount <= info.maxBet,
            "CashInOut.sol: Bet amount must be more than minBet and less than maxBet."
        );
        require(
            game.playerCount < info.maxPlayer,
            "CashInOut.sol: maxPlayer reached."
        );
        require(
            !playerInGame[_player][_gId][_rId],
            "CashInOut.sol: player already joined this room."
        );

        vault.addGameBalance(_player, _amount);
        uint128 maxReward = getGameRewardPrize(_gId, _amount);
        ...
        playerInGame[_player][_gId][_rId] = true;

        emit JoinGame(_player, _amount, _gId, _rId);
    }
    ...
    function resolveGame(
        ...
        require(
            info.status == GameStatus.Registered,
            "CashInOut.sol: GameID not registered."
        );
        ...
        if (!info.isFreeGame) {
            ...
            require(
                ...
                "CashInOut.sol: Room ID already resolved."
            );

            for (uint128 i; i < _players.length; i++) {
                uint128 reward = _amounts[i];

                vault.subGameBalance(_players[i], reward, 0);
    ...
    function expireGame(uint8 _gId, string memory _rId)
        external
        override
        whenNotPaused
        onlyRole(WORKER_ROLE)
    {
        ...
        require(
            game.gameState == GameState.OnGoingGame,
            "CashInOut.sol: expireGame, already resolved."
        );

        for (uint8 i; i < game.players.length; i++) {
            uint128 bet = game.bets[i];

            vault.subGameBalance(game.players[i], bet, 0);
            reserves[_gId][_rId] -= bet;
        }
        vault.subReserveOperating(reserves[_gId][_rId]);
        reserves[_gId][_rId] = 0;
    ...
    function getGameRewardPrize(uint256 _gId, uint128 _amount)
        public
        view
        returns (uint128)
    {
        return (_amount * gameInfo[_gId].rewardMultiplier) / 100;
    }
...
interface ICashInOut {
    ...
    function expireGame(uint8 _gId, string memory _rId) external;
