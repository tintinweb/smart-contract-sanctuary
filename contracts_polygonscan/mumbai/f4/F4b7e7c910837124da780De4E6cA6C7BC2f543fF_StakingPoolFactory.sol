...
contract StakingPoolFactory is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    constructor(uint256 _poolFee, bool _ownerFunctionalityEnabled) {
        poolFee = _poolFee;
        ownerFunctionalityEnabled = _ownerFunctionalityEnabled;
    }
    // function initialize() public initializer {}

    event PoolsLaunched(uint256 indexed poolsIndex, address indexed poolsAddress);
    ...
    returns (address)
    {
        require(address(_rewardToken) != address(0) && address(_inputToken) != address(0), "Cant be Zero address");
        require(_startBlock >= block.number, "Start time should be greater than current"); // ideally at least 24 hours more to give investors time
        require(_endBlock > _startBlock, "End Block should be greater than StartBlock");  //_crowdsaleEndTime = 0 means crowdsale would be concluded manually by owner
        require(_amountPlusFee > 0, "Allocate some amount for Pool");
        require(_blockReward > 0, "Block Rewards cant be zero");
        
        TransferHelper.safeTransferFrom(address(_rewardToken), msg.sender, address(this), _amountPlusFee);
        StakingPool newPool = new StakingPool(_feeAddress);
        uint256 feeAmount = _amountPlusFee.mul(poolFee).div(1000);
        TransferHelper.safeTransfer(address(_rewardToken), owner(), feeAmount);
        TransferHelper.safeApprove(address(_rewardToken), address(newPool), _amountPlusFee.sub(feeAmount));
        newPool.init(_rewardToken,
                     _amountPlusFee.sub(feeAmount),
                    _inputToken, 
                    _blockReward, 
                    _startBlock, 
                    _endBlock, 
        ...
        emit PoolsLaunched(poolsIndex,address(newPool));
        poolsIndex++;
    ...
    function addRewardToken(
        ...
        newPool.addRewardToken(_startBlock, _endBlock, _rewardToken, _lastRewardBlock, _blockReward, _amount);
    }

    function setFeeAddress(address _feeAddress, address _pool) external onlyPoolOwner(_pool) {
        StakingPool newPool = StakingPool(_pool);
        newPool.setFeeAddress(_feeAddress);
    }

    function updateEndBlock(uint256 _endBlock, uint256 _rewardInfoIndex, address _pool) external onlyPoolOwner(_pool) {
        StakingPool newPool = StakingPool(_pool);
        newPool.updateEndBlock(_endBlock, _rewardInfoIndex);
    }

    function updateBlockReward(uint256 _blockReward, uint256 _rewardTokenIndex, address _pool) 
        external 
        onlyPoolOwner(_pool)
    {
        StakingPool newPool = StakingPool(_pool);
    ...
    function transferRewardToken(uint256 _rewardTokenIndex, uint256 _amount, address _pool)
        external 
        onlyPoolOwner(_pool)
    {
        StakingPool newPool = StakingPool(_pool);
        newPool.transferRewardToken(_rewardTokenIndex, _amount);
    }
    ...
    ReentrancyGuard
{
     ...
     */
    function getMultiplier(uint256 _fromBlock, uint256 _rewardInfoIndex, uint256 _to)
        public
        view
        returns (uint256)
    {
    ...
    function updateEndBlock(uint256 _endBlock, uint256 _rewardInfoIndex) external onlyOwner {
        RewardInfo storage rewardInfo = rewardPool[_rewardInfoIndex];
