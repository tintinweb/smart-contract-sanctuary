...
contract DailyPrize is Ownable, VRFConsumerBase {
    ...
    event DrawRequested(uint256 day, uint256 prize);
    event PrizeClaimed(uint256 day, address indexed player, uint256 prize);
    ...
    mapping(bytes32 => DrawInfo) public drawRequests;
    ...
    function draw(
        ...
        require(_day * SECONDS_PER_DAY > block.timestamp, "invalid day");
        require(!_drawn[_day], "already drawn");
        require(_players.length == _numTickets.length, "invalid input");

        _drawn[_day] = true;

        bytes32 requestID = getRandomNumber();
        DrawInfo storage info = drawRequests[requestID];
        info.day = _day;
        info.prize = _prize;
        for (uint256 i; i < _players.length; i++) {
            info.players.push(_players[i]);
            info.numTickets.push(_numTickets[i]);
        }

        emit DrawRequested(_day, _prize);
    }
    ...
    function fulfillRandomness(bytes32 requestID, uint256 randomness)
        ...
        override
    {
        DrawInfo memory info = drawRequests[requestID];
        mapping(address => uint256) storage dailyPrize = dailyPrizes[info.day];
        ...
        for (i = 0; i < info.players.length; i++) {
            totalTickets += info.numTickets[i];
        }
        ...
        for (i = 0; i < 25; i++) {
            ...
            j = 0;
            while (rand >= info.numTickets[j]) {
                rand -= info.numTickets[j];
                j++;
            }
