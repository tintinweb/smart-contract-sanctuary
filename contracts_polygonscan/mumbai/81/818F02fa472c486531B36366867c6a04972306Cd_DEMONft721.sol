...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
  ...
  // mapping for token creator
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        string memory _tokenURI = tokenToIndex[tokenId].toString();
...
contract DEMONft721 is ERC721 {
    ...
    function getAssignedIndex() public view returns (uint256[] memory){
        uint256[] memory assginedTokenToIndex = new uint256[] (tokenCounter - 1);
        
        for(uint256 i = 0; i < tokenCounter; i++){
            assginedTokenToIndex[i] = tokenToIndex[i];
        }
        return assginedTokenToIndex;        
    }
    
    function check(uint256 tokenId) internal returns(uint256) {
