...
contract TimeLockedWalletFactory {
    ...
    //should be updatable by owner
    address private TOKEN_CONTRACT = 0x45995BAc936657FFd7845785D4154A193C222e30; // M_GMRX
    ...
    function newTimeLockedWallet(address owner, uint amount, uint unlockTime) public returns (address wallet) {
        ...
        ERC20 token = ERC20(TOKEN_CONTRACT);
        require(token.allowance(msg.sender, address(this)) >= amount);

        wallet = address(new TimeLockedWallet(owner, TOKEN_CONTRACT, amount, unlockTime));
...
contract TimeLockedWallet is Ownable {
    ...
    constructor(address owner, address tokenContract, uint amount, uint unlockTime) {
        transferOwnership(owner);
        _tokenContract = tokenContract;
        LockBoxStruct memory box;
        box.amount = amount;
        box.unlockTime = unlockTime;
        box.paid = false;
        lockBoxStructs.push(box);
    }

    // total balance in boxes
    // total balance in token sc
    ...
    function withdrawTokens() onlyOwner public {
        for (uint i = 0; i < lockBoxStructs.length; i++) {
            ...
            if (box.unlockTime <= block.timestamp && box.paid == false) {
                ...
                emit WithdrewTokens(_tokenContract, msg.sender, box.amount);
                box.paid = true;
            }
        }
    }
