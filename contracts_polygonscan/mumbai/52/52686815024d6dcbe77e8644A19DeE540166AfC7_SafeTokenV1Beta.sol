    ...
    ReentrancyGuard
{
    ...
    uint256 public constant startDate = 1 days;

    uint256 public constant oneDay = 1 days;
    uint256 public constant oneMinute = 1 minutes;
    uint256 public startCalimTimeInMinutes = 1 * oneMinute;
    ...
    uint256 public totalEmmissionRate = ((10**18) * 720);
    ...
    modifier onlyIfVestingScheduleNotExists(
        bytes32 _vestingScheduleId,
        uint256 _tokenId
    ) {
        // require(
        //     abi.encodePacked(holdersVestingCount[_beneficiary]).length <= 0
        // );
        require(vestingSchedules[_vestingScheduleId].initialized == false);
        require(vestingSchedules[_vestingScheduleId].revoked == true);
        require(vestingSchedules[_vestingScheduleId].nftId != _tokenId);
        _;
    }
    ...
    function createVestingSchedule(address _beneficiary, uint256 _tokenId)
        public
        returns (
            // onlyIfVestingScheduleNotExists(_beneficiary,_tokenId)
            bytes32
        )
    {
        // require(
        ...
        // if (vestingSchedules[vestingScheduleIdExist].initialized) {
        //     require(
        //         vestingSchedules[vestingScheduleIdExist].initialized == false,
        //         "Vesting schedule already created"
        //     );
        //     require(
        //         vestingSchedules[vestingScheduleIdExist].revoked == true,
        //         "Vesting schedule should end or revoked to start a new schedule for the same NFT Id"
        //     );
        //     require(
        //         vestingSchedules[vestingScheduleIdExist].nftId != _tokenId,
        //         "Vesting schedule already created for the provided NFT Id"
        //     );
        // }
        // require(checkUser(_beneficiary));
        // require(getVestingSchedulesCountByBeneficiary(_beneficiary) <= 0);
        if (getVestingSchedulesCountByBeneficiary(_beneficiary) != 0) {
            bytes32 vestingScheduleIdExist = this
                .computeCurrentVestingScheduleIdForHolder(_beneficiary);
            // require(
            //     vestingSchedules[vestingScheduleIdExist].initialized == false,
            //     "Vesting Already initialized for the provided NFT id"
            // );
            require(
                vestingSchedules[vestingScheduleIdExist].revoked == true,
                "Vesting Already scheduled for the provided NFT id"
            );
            require(
                vestingSchedules[vestingScheduleIdExist].nftId != _tokenId,
                "Vesting Already initialized for the provided NFT id"
    ...
    function checkUser(address _holder) public view returns (uint256) {
        // if(abi.encodePacked(holdersVestingCount[_holder]).length > 0){
        // if(abi.encodePacked(holdersVestingCount[_holder]).length > 0){
        //     return true;
        // }
        // else{
        ...
        // return abi.encodePacked(holdersVestingCount[_holder]).length  ;
    }
        ...
        onlyIfVestingScheduleNotRevoked(vestingScheduleId)
    {
        ...
        uint256 vestedAmount = _computeReleasableAmount(vestingSchedule);
        uint256 claimableAmount = _computeClaimableAmount(vestingSchedule);
        require(
        ...
        returns (uint256)
    {
        ...
        if (
            ...
            // currentTime.sub(vestingSchedule.startClaimTime) >=
            // vestingSchedule.start.add(vestingSchedule.duration)
        ) {
            return vestingSchedule.amountTotal.sub(vestingSchedule.released);
        } else {
            uint256 timeFromStart = currentTime.sub(vestingSchedule.start);
            uint256 secondsPerSlice = slicePeriodSeconds;
