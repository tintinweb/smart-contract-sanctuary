...
contract AnrkeyGame is Ownable {
    ...
    ArkaneERC1155 public goldenChaliceNftInstance;
    ArkaneERC1155 public sacredRelicNftInstance;
    ArkaneERC1155 public oracleShardsNftInstance;
    ...
    constructor(address tokenAddress, address tokenDepositAddress, uint256 armsBoostAmount, uint256 sacredreward, uint256 poolBaseApy, uint256 armamentReward, uint256 goldenChaliceReward, address goldenChaliceNftAddress, address sacredRelicNftAddress, address oracleShardsNftAddress) {
        ...
        require(goldenChaliceNftAddress != address(0), "AnrkeyGame: goldenChaliceNftAddress cannot be zero address");
        require(sacredRelicNftAddress != address(0), "AnrkeyGame: sacredRelicNftAddress cannot be zero address");
        require(oracleShardsNftAddress != address(0), "AnrkeyGame: oracleShardsNftAddress cannot be zero address");
        ...
        goldenChaliceNftInstance = ArkaneERC1155(goldenChaliceNftAddress);
        sacredRelicNftInstance = ArkaneERC1155(sacredRelicNftAddress);
        oracleShardsNftInstance = ArkaneERC1155(oracleShardsNftAddress);
    }
    ...
    function stakeSacredRelic(address poolAddress, uint256 tokenId, uint256 expireTime, uint256 randomSacredReward) external returns (uint256) {
        ...
        uint256 balance = sacredRelicNftInstance.balanceOf(msg.sender,tokenId);
        ...
        sacredRelicNftInstance.burn(tokenId, msg.sender, 1);
    ...
    function stakeGoldenChalice(address poolAddress, uint256 tokenId, uint256 expireTime) external returns (bool) {
        ...
        require(block.timestamp < expireTime, "Error: Your NFT is expired");
        uint256 balance = goldenChaliceNftInstance.balanceOf(msg.sender,tokenId);
        require(balance > 0,"Anrx: you should purchase NFT before stake");
    ...
    function redeemCollectibles(address userAddress, uint nftId, address poolAddress, uint256 powerLevel) external onlyOwner returns (bool) {
        require(oracleShardsNftInstance.balanceOf(userAddress, nftId) >= 1, "You do not own the NFT");
    ...
    //update Nft Address
    function updateGoldenChalicNft(address newGoldenChaliceNft) external onlyOwner returns (bool) {
        goldenChaliceNftInstance = ArkaneERC1155(newGoldenChaliceNft);
        return true;
    }

    function updateSacredRelicNft(address newSacredRelicNft) external onlyOwner returns (bool) {
        sacredRelicNftInstance = ArkaneERC1155(newSacredRelicNft);
        return true;
    }

    function updateOracleShardsNft(address newOracleShardsNft) external onlyOwner returns (bool) {
        oracleShardsNftInstance = ArkaneERC1155(newOracleShardsNft);
        return true;
    }
