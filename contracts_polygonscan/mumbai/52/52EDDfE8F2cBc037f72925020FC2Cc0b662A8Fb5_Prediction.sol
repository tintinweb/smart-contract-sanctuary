...
contract Prediction is AccessProtected, Feed, KeeperCompatible {
    ...
    // Mapping address to pool Id to bets
    mapping(uint256 => Bet[]) public bets;

    // address => poolId => matchIds
    ...
    function addPool(Pool memory _pool) external onlyAdmin {
        require(
    ...
    function getMatchIdsOfPool(uint256 _poolId) public view returns (uint256[] memory) {
    ...
    function matchExists(uint256 _poolId, uint256 _matchId)
        internal
        view
        returns (bool)
    {
    ...
    function totalPredictionsByOwner(address _owner)
        public
        view
        returns (uint256 counter)
    {
        for (uint256 i = 0; i <= pools.length; i++) {
            counter = counter + userBets[_owner][i].length;
        }
    }
