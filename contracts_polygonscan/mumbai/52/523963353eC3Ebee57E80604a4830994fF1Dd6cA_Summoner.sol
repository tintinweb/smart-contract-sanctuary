...
ReentrancyGuardUpgradeable
{
    ...
    // fee token address => fee amount
    mapping(address => uint256) private feeAmounts;
    ...
    whenNotPausedNetwork(remoteNetworkID)
    {
        ...
        require(feeAmounts[feeToken] > 0, "fee token not accepted");
        ...
        if (sourceNftInfo[remoteNetworkID][nftContract][nftId].contractAddress != address(0)) {
            ...
            // take fees
            _takeFees(nftContract, nftId, _id, feeToken);
        }
    }
    ...
    function _takeFees(
        ...
        if (feeToken != address(0)) {
            require(IERC20Upgradeable(feeToken).balanceOf(msg.sender) >= feeAmounts[feeToken],
            ...
            require(msg.value >= feeAmounts[feeToken], "not enough fees");
        }
        // store the collected fee
        feeCollection[_id] = Fee(feeToken, feeAmounts[feeToken]);
        emit FeeTaken(
            msg.sender,
            nftContract,
            nftId,
