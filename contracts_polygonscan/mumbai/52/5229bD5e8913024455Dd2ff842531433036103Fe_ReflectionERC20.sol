...
contract ReflectionERC20 is ERC20 , Ownable {
    ...
    uint256 _hodlReserved;
    uint256 _treasuryReserved;
    uint256 _charityReserved;
    ...
    function _customTransfer(address from_, address to_, uint256 amount_) internal returns (bool){
        ...
        if(whitlisted[from_].tax || whitlisted[to_].tax){
            ...
            if(reflectionEnabled){
                ...
                _transfer(from_, address(swapHundler) , totalTax);
                ...
                if(totalTax > 0) {
                    ...
                    //Tax reservedTax;
                    if(from_ != wavaxPair && to_ != wavaxPair) {
                        // swap and distribute to wallets
                        swapHundler.swapToAvax(_teamReserved, _hodlReserved, _treasuryReserved, _charityReserved);
...
contract SwapHandler is Ownable {
    ...
    function _swap(uint amount_, address to_) internal {
