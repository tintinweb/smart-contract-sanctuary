...
contract Marketplace is PriceConsumerV3 {
    ...
    MetaMundoToken public metamundoToken;
    ...
    modifier isActive {
        require(!metamundoToken.isMaintaining());
        _;
    }

    modifier onlyMetaMundo() {
        require(
            msg.sender == metamundoToken.owner() ||
            msg.sender == metamundoToken.metamundoTrust(),
            "caller is not one of metamundo accounts"
        );
        _;
    }

    constructor(address _tokenAddress) {
        metamundoToken = MetaMundoToken(_tokenAddress);
    }
    ...
    /**
        ...
        - if w2 or w3user wants to pay in USD, they pay gas fee to MetaMundo first
        - then MetaMundo triggers setForSaleWithUSD for them
  ...
  /**
    ...
    * 2. trigger setApprovalForAll() from MetaMundo EOA to approve this contract
    ...
    */
    function setForSaleWithUSD(uint256 _tokenId, uint256 _priceUSD, bool _auction) public isActive onlyMetaMundo {
        ...
        require(
            metamundoToken.isApprovedForAll(metamundoToken.metamundoTrust(), address(this)),
            "MetaMundo did not approve Marketplace contract"
        );
  ...
  /**
    ...
    */
    function removeSetForSaleUSD(uint256 _tokenId) public isActive onlyMetaMundo {
        require(_tokenId > 0, "token id is zero or lower");
  ...
  /**
    ...
    */
    function setForSaleWithETH(uint256 _tokenId, uint256 _priceUSD, bool _auction) public isActive {
        ...
        require(tokenPriceUSD[_tokenId] == 0, "token has already been set for sale");
        require(_priceUSD > 0, "price is zero or lower");
        address owner = metamundoToken.ownerOf(_tokenId);
        require(owner == msg.sender, "caller is not a token owner");
        require(metamundoToken.isApprovedForAll(msg.sender, address(this)),
  ...
  /**
    ...
    * function triggered by MetaMundo
    *
    * Preconditions
    * For NON-AUCTION
    ...
    */
        ...
        onlyMetaMundo
    {
        ...
        address owner = metamundoToken.ownerOf(_tokenId);
        (bool isW3user, address w3owner, string memory w2owner) = metamundoToken.getTokenOwnership(_tokenId);

        if (isW3user) {
            require(owner == w3owner, "wrong owner");
            require(w3owner != address(0), "w3owner is empty");
            metamundoToken.safeTransferFrom(w3owner, metamundoToken.metamundoTrust(), _tokenId);
        } else {
            require(owner == metamundoToken.metamundoTrust(), "wrong owner");
            require(bytes(w2owner).length > 0, "w2owner is empty");
        }

        metamundoToken.changeTokenOwnership(_tokenId, address(0), _buyerId);
        tokenPriceUSD[_tokenId] = 0;
        tokenOnAuction[_tokenId] = false;
        ...
        onlyMetaMundo
    {
        ...
        if (isW3user) {
            require(owner == w3owner, "wrong owner");
            require(w3owner != address(0), "w3owner is empty");
            metamundoToken.safeTransferFrom(w3owner, _buyer, _tokenId);
        } else {
            require(owner == metamundoToken.metamundoTrust(), "wrong owner");
            require(bytes(w2owner).length > 0, "w2owner is empty");
            metamundoToken.safeTransferFrom(metamundoToken.metamundoTrust(), _buyer, _tokenId);
        }

        metamundoToken.changeTokenOwnership(_tokenId, _buyer, "");
        tokenPriceUSD[_tokenId] = 0;
        tokenOnAuction[_tokenId] = false;
    ...
    function purchaseWithETHw3user(uint256 _tokenId) public payable isActive {
        ...
        if (isW3user) {
            require(owner == w3owner, "wrong owner");
            require(w3owner != address(0), "w3owner is empty");
            metamundoToken.safeTransferFrom(w3owner, msg.sender, _tokenId);
        } else {
            require(owner == metamundoToken.metamundoTrust(), "wrong owner");
            require(bytes(w2owner).length > 0, "w2owner is empty");
            metamundoToken.safeTransferFrom(metamundoToken.metamundoTrust(), msg.sender, _tokenId);
        }

        metamundoToken.changeTokenOwnership(_tokenId, msg.sender, "");
        tokenPriceUSD[_tokenId] = 0;
        tokenOnAuction[_tokenId] = false;
    ...
    function placeBidWithETHw3user(uint256 _tokenId, uint256 _bidPriceUSD) public payable isActive {
        ...
        _send(sellerReceiveAmount, metamundoToken.metamundoTrust());
        ...
        onlyMetaMundo
    {
        ...
        if (isW3user) {
            require(owner == w3owner, "wrong owner");
            require(w3owner != address(0), "w3owner is empty");
            metamundoToken.safeTransferFrom(w3owner, w3user ? _receiver : metamundoToken.metamundoTrust(), _tokenId);
        // w2user
        } else {
            require(owner == metamundoToken.metamundoTrust(), "wrong owner");
            require(bytes(w2owner).length > 0, "w2owner is empty");
            if (w3user) {
                metamundoToken.safeTransferFrom(metamundoToken.metamundoTrust(), _receiver, _tokenId);
            }
        }

        metamundoToken.changeTokenOwnership(_tokenId, w3user ? _receiver : address(0), w3user ? "" : _receiverId);
        tokenPriceUSD[_tokenId] = 0;
        tokenOnAuction[_tokenId] = false;
    ...
    function _validateAmount(uint256 _tokenId) private {
        ...
        uint256 buyerFee = tokenPrice.mul(metamundoToken.feePercent()).div(1000);
    ...
    function _validateBidAmount(uint256 _bidPriceUSD) private {
        int256 currentPriceOfETHtoUSD = getCurrentPriceOfETHtoUSD();
        uint256 buyerFee = _bidPriceUSD.mul(metamundoToken.feePercent()).div(1000);
    ...
    function _payBuyerSellerFee() private returns (uint256) {
        uint256 feePercent = metamundoToken.feePercent();
        uint256 fee = msg.value.mul(feePercent.mul(2)).div(feePercent.add(1000));
        _send(fee, metamundoToken.metamundoTrust());
        return fee;
    }

    function _sendRoyalty(uint256 _tokenId) private returns (uint256) {
        uint256 feePercent = metamundoToken.feePercent();
        (, uint16 percentage) = metamundoToken.getRoyaltyReceiver(_tokenId);
        ...
        _send(royalty, metamundoToken.metamundoTrust());
        return royalty;
    }
    ...
    function _sendToSeller(
        ...
        private
    {
        _send(sellerReceiveAmount, _isW3user ? _w3owner : metamundoToken.metamundoTrust());
    }
...
contract MetaMundoToken is ERC721URIStorage, AccessControl {
    ...
    /**
        ...
        * Set MetaMundo token name and symbol
        * Set initial fee percentage which is 2.5%
        *
        * @param _name MetaMundo token name
        * @param _symbol MetaMundo token symbol
        * @param _feePercent fee percentage MetaMundo account will receive
        */
    constructor(
        string memory _name,
    ...
    /**
        ...
        * MetaMundo keeps the token on w2user's behalf

        * Precondition
        * 1. user pays gas fee to MetaMundo in USD
        *
        ...
        onlyMetaMundo
    {
        ...
        // MetaMundo keeps the token on minter's behalf
    ...
    */
    ...
    /**
        ...
        */
    function claimToken(uint256[] memory _tokenIDs, address _w3user) public onlyMetaMundo {
        for (uint i = 0; i < _tokenIDs.length; i++) {
            uint256 tokenId = _tokenIDs[i];
            require(!tokenOwners[tokenId].isW3user, "Only web2 users token can be claimed");
            require(ownerOf(tokenId) == metamundoTrust, "MetaMundo wallet does not own this token");
            if (_w3user != address(0)) {
                _transfer(metamundoTrust, _w3user, tokenId);
...
contract AccessControl {
    ...
    modifier onlyMetaMundo() {
        require(msg.sender == owner || msg.sender == metamundoTrust || msg.sender == marketplace, "caller is not one of MetaMundo accounts");
        _;
    }
    ...
    function changeMetaMundoTrust(address _newMundoTrust) onlyCEO public {
        if (_newMundoTrust != address(0)) {
            metamundoTrust = _newMundoTrust;
        }
    }
