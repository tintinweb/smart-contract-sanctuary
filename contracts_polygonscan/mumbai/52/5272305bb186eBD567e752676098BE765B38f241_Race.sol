...
contract Race is AccessControlEnumerable {
    ...
    mapping(uint256 => mapping(uint256 => uint256)) private currentAcceleration; // currentAcceleration[raceID][racerID];
    ...
    bytes32 public constant VRG_ROLE = keccak256("VRG_ROLE");
    bytes32 public constant GAME_LOOP_ROLE = keccak256("GAME_LOOP_ROLE");
    bytes32 public constant UPDATE_QUEUE_ROLE = keccak256("UPDATE_QUEUE_ROLE");
    ...
    function addVRG(address vrgAddress) public onlyRole(DEFAULT_ADMIN_ROLE) {
        grantRole(VRG_ROLE, vrgAddress);
    }
    ...
    //TODO:
    // make only Callable by registered VRG
    // and set randomness from Chainlink VRF
    function fulfillRandomnessForRace(uint256 raceID) public {
        uint256 randomness = uint256(
            keccak256(abi.encode(block.timestamp, raceID))
        );
        // save random number to use on update
        races[raceID].randomNumbers.push(
            [(races[raceID].updates + 1), randomness]
        );
        // request update from update queue
        UpdateQueue(races[raceID].updateQueue).requestUpdate(raceID);
    }
            ...
            for (
                ...
                if (!completedRace[raceID][racer]) {
                    ...
                    if (currentPosition[raceID][racer][0] != 0) {
                        ...
                        currentAcceleration[raceID][racer] = acceleration;
                    }
    ...
    function getCurrentStats(uint256 raceID, uint256 racerID)
        ...
        returns (
            uint256 speed,
            uint256 acceleration,
            uint256[3] memory lapLanePosition
        )
    {
        return (
            currentSpeed[raceID][racerID],
            currentAcceleration[raceID][racerID],
