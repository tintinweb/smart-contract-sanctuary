...
contract KOMPrivateSale is ERC721 {
    ...
    uint256 public usdtMintPrice = 2000000000000000000;
    ...
    function mintTokensByDAI(uint8 amount,uint256 _usdtAmount)
        external
        callerIsUser
        mintStarted
    {
        ...
        if (_usdtAmount >= totalMintUsdtPrice) {
            dai.transferFrom(msg.sender, address(this), _usdtAmount-totalMintUsdtPrice);
        }
    ...
    function mintTokensFromInviteByDAI(uint8 amount, address _ref,uint256 _usdtAmount)
        external
        payable
        callerIsUser
        mintStarted
