    ...
    */
    ...
    timeTester();
        ...
        for (uint256 j = 0; j < boxesNonGen[boxId].countNFTs; j++) {
          ...
          tokenId = dex.mintWithCollection(collection, msg.sender, nonGenSeries[sId].uris[rand1].uri, nonGenseriesRoyalty[sId], ownerId );
...
interface IDEX {
   ...
   function mintWithCollection(address collection, address to, string calldata tokenURI, uint256 royalty, string calldata ownerId ) external returns(uint256);
