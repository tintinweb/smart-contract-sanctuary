...
contract BattleV2Contract is VRFConsumerBase, Ownable {
    enum STATUS {
        READY,
        END
    }
    ...
    uint256 public BATTLE_TIME;

    Battle[] public battles;
    uint256 public battlesCount;

    uint256 public insuranceFee;
    uint256 public insurancePeriod;
    mapping(address => uint256) public insurances;
     ...
     */
    event ClaimedPresent(uint256 nftId, address nftOwner, uint256 amount);
    ...
    constructor(
        ...
        insuranceFee = 10 * (10**18);
        insurancePeriod = 7 days;

        BATTLE_TIME = 1 minutes;
    }

    /**
     * Set chainlink service fee
     * @param _fee Chainlink fee number
     ...
     * @param _period Insurance period
     */
    function setInsurancePeriod(uint256 _period) external onlyOwner {
        insurancePeriod = _period;
    }

    /**
     * Set Battle Time
     * @param _time Battle Time
     */
    function setBattleTime(uint256 _time) external onlyOwner {
        BATTLE_TIME = _time;
    }
    ...
    modifier _checkValidBattleId(uint256 _battleId) {
        require(
            ...
            "BattleV2Contract: _battleId is invalid number"
        );
        _;
    }
    ...
    modifier _onlyOwner(uint256 _nftId) {
        ...
        require(
            msg.sender == nftOwner,
            "BattleV2Contract: You are not NFT owner"
        );
        _;
    }
    ...
    modifier _canParticipateBattle(uint256 _battleId, uint256 _nftId) {
        ...
        require(
            ...
            "BattleV2Contract: Not able to Join Battle"
        );
        require(
            battle.status == STATUS.READY,
            "BattleV2Contract: Battle had already been ended"
        );

        (, , , uint256 nftLevel, , ) = nft_.nfts(_nftId);
        require(
            nftLevel == battle.level,
            "BattleV2Contract: NFT level is not same as battle's level"
        );
        ...
        require(!wasParticipated, "BattleV2Contract: Already participated");
        _;
    }
    ...
    modifier _canCompleteBattle(uint256 _battleId) {
        ...
        require(
            ...
            "BattleV2Contract: Not able to complete Battle"
        );
        require(
            battle.status == STATUS.READY,
            "BattleV2Contract: Battle had already been ended"
    ...
    modifier _isWinner(uint256 _battleId) {
        ...
        require(
            ...
            "BattleV2Contract: You are not a winner"
        );
        _;
    }
    ...
    function createNewBattle(
        ...
        require(
            ...
            "BattleV2Contract: StartTime is over"
        );
        require(_award > 0, "Invalid award");
    ...
    {
        uint256 count = participantsCount[_battleId];

        Participant memory participant;
        participant.nftId = _nftId;
        participant.nftOwner = msg.sender;
        participant.isClaimed = false;
        participant.hasInsurance = insurances[msg.sender] > block.timestamp;

        participants[_battleId].push(participant);
        count++;

        participantsCount[_battleId] = count;

        nft_.transferNFT(address(this), _nftId);

        emit NewParticipantJoined(_battleId, _nftId, count);
    }
    ...
    function claimPresent(uint256 _battleId) public _isWinner(_battleId) {
        require(
            !battleHistories[_battleId].isClaimed,
            "BattleV2Contract: Already Claimed"
        );

        nft_.upgradeNFT(
            battleHistories[_battleId].nftId,
            battles[_battleId].uri
        );

        nft_.transferNFT(
            battleHistories[_battleId].nftOwner,
            battleHistories[_battleId].nftId
        );
        gfx_.transfer(
            battleHistories[_battleId].nftOwner,
            battles[_battleId].award * (10**18)
        );

        battleHistories[_battleId].isClaimed = true;
    ...
    function buyInsurance() public {
        require(msg.sender != address(0), "BattleV2Contract: Invalid request");
        require(
            gfx_.balanceOf(msg.sender) > insuranceFee,
            "BattleV2Contract: Not enough GFX balance"
        );
        require(
            gfx_.allowance(msg.sender, address(this)) > insuranceFee,
            "BattleV2Contract: Not enough GFX allowance"
        );
        require(
            gfx_.transferFrom(msg.sender, address(this), insuranceFee),
            "BattleV2Contract: GFX transaction failed"
        );
        require(
            insurances[msg.sender] < block.timestamp,
            "BattleV2Contract: Insurance is still active"
        );

        insurances[msg.sender] = block.timestamp + insurancePeriod;
    }

    function retrieveNFT(uint256 _battleId)
        public
        _checkValidBattleId(_battleId)
    {
        require(
            battles[_battleId].status == STATUS.END,
            "BattleV2Contract: Battle is not ended"
        );
        require(
            battleHistories[_battleId].nftOwner != msg.sender,
            "BattleV2Contract: You are winner"
        );
        ...
        for (uint256 i = 0; i < participants[_battleId].length; i++) {
            if (participants[_battleId][i].nftOwner == msg.sender) {
                require(
                    participants[_battleId][i].hasInsurance,
                    "BattleV2Contract: No insurance"
                );
                require(
                    !participants[_battleId][i].isClaimed,
                    "BattleV2Contract: Already claimed"
                );
                ...
                participants[_battleId][i].isClaimed = true;
                isClaimed = true;
        ...
        require(isClaimed, "BattleV2Contract: Didn't participate battle");
    }

    function canRetrieveNFT(uint256 _battleId, address _owner)
        external
        view
        _checkValidBattleId(_battleId)
        returns (bool)
    {
        for (uint256 i = 0; i < participants[_battleId].length; i++) {
            if (participants[_battleId][i].nftOwner == _owner) {
                return participants[_battleId][i].hasInsurance;
            }
        }
