...
contract EmiPrice2 is Initializable, Priviledgeable {
 ...
 string public codeVersion = "EmiPrice2 v1.0-242-g6153cd3";
    ...
    function _getUniswapPrice(
        ...
        for (uint256 i = 0; i < _coins.length; i++) {
            ...
            // get reserve to accurately check prices
            (uint112 reserve0, uint112 reserve1, ) =
                IUniswapV2Pair(
                    IUniswapV2Factory(IUniswapV2Router02(uniRouter).factory())
                        .getPair(_path[0], _path[1])
                )
                    .getReserves();

            uint256 res =
                address(_path[0]) < address(_path[1]) ? reserve0 : reserve1;
            uint256 _inDecimals =
                _valueToDecimals(res) <= target_decimal
                    ? _valueToDecimals(res) - 1
                    : target_decimal;
            uint256 _in = 10**(_inDecimals);

            uint256[] memory _amts =
                IUniswapV2Router02(uniRouter).getAmountsOut(_in, _path);
            if (_amts.length > 0) {
                _prices[i] = _amts[_amts.length - 1].mul(
                    10**(18 - base_decimal + target_decimal - _inDecimals)
                );
            } else {
                _prices[i] = 0;
            }
        ...
        for (uint256 i = 0; i < _coins.length; i++) {
            ...
            for (uint256 m = 0; m < _base.length; m++) {
                ...
                if (_route.length == 0) {
                    ...
                    uint256 res =
                        IEmiswap(
                            _factory.pools(IERC20(_route[0]), IERC20(_route[1]))
                        )
                            .getBalanceForAddition(IERC20(_route[0]));
                    uint256 _inDecimals =
                        _valueToDecimals(res) <= target_decimal
                            ? _valueToDecimals(res) - 1
                            : target_decimal;
    ...
    function _valueToDecimals(uint256 value)
        internal
        pure
        returns (uint256 fullDecimals)
    {
        if (value >= 1e18) {
            fullDecimals = 18;
        } else if (value >= 1e17) {
