...
// import "https://raw.githubusercontent.com/OpenZeppelin/openzeppelin-contracts/release-v3.4-solc-0.7/contracts/access/Ownable.sol";
...
contract CoinFlip is Ownable, VRFConsumerBase {
  ...
  modifier betConditions {
    require(msg.value >= 0.1 ether, "Bet below minimum allowed");
    require(msg.value <= (getContractBalance()).div(2), "Bet above maximum allowed");
    // require(playersByAddress[msg.sender].betOngoing == false);
    _;
  }
  ...
  function bet(uint256 _betChoice) payable public betConditions {
    require(_betChoice == 0 || _betChoice == 1, "Must bet heads or tails (0 or 1)");
  ...
  function checkResult(uint256 _randomResult, bytes32 _requestId) public returns (bool) {
    ...
    if(playersByAddress[player].betChoice == _randomResult) {
      win = true;
      amountWon = (playersByAddress[player].betAmount).mul(2);
      playersByAddress[player].balance = (playersByAddress[player].balance).add(amountWon);
      contractBalance = (contractBalance).sub(amountWon);
    }
  ...
  function withdrawPlayerBalance() public {
    require(msg.sender != address(0), "Must set sender address");
    require(playersByAddress[msg.sender].balance > 0, "No balance to withdraw");
    // require(playersByAddress[msg.sender].betOngoing == false);
  ...
  function withdrawMatic(uint amtMatic) public onlyOwner {
    manager.transfer(amtMatic);
  }
