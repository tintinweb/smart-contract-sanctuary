...
// flattened :  Monday, 16-Aug-21 21:01:21 UTC
...
contract VErc20 is IVErc20, AErc20 {
    ...
    function mintAndDepositToPolygon(
        ...
        Signatures(nonce, keccak256(abi.encodePacked(address(this), "mintAndDepositToPolygon", nonce)), V, R, S)
    ) override public {
        uint256 underlyingTokens = underlying().balanceOf(address(this));
        require(underlyingTokens > 0, "VErc20:mintAndDepositToPolygon - there are no underlying tokens to mint");

        require(underlying().transfer(address(vFinance()), underlyingTokens), "VErc20:mintAndDepositToPolygon - failed to transfer tokens to vFinance");
        ...
        require(approve(address(this), vLighthouse().addressByKey(ERC20_PREDICATE_PROXY_ADDRESS), balanceOf(address(this))), "VErc20:mintAndDepositToPolygon - failed to approve ERC20_PREDICATE_PROXY_ADDRESS");
