  ...
  RoyaltiesV1Impl
{
  ...
  mapping(bytes32 => bool) private paymentFulfilled;
  ...
  function safeTransferFromWithPaymentSignature(
    address _from,
    address _to,
    uint256 _tokenId,
    uint256 _amount,
    ...
    require(
      _from != _to,
      "ERC1155AQ: this method cannot transfer token to current owner"
    );

    require(
      paymentFulfilled[_message] != true,
      "ERC1155AQ: payment has been fulfilled"
    );

    address addressFromSig = ecrecover(_message, _sigV, _sigR, _sigS);

    require(_to == addressFromSig, "ERC1155AQ: signature cannot be verified");
  ...
  function mint(
    ...
    require(_amount <= SUPPLY, "ERC1155AQ: amount should be less than 100");
    lastId = lastId.add(1);
    super._mint(_account, lastId, _amount, "");
    _setTokenURI(lastId, _uri);
    LibPart.Part[] memory royalties = new LibPart.Part[](1);
    royalties[0].account = payable(_account);
    royalties[0].value = ROYALTY;
    _saveRoyalties(lastId, royalties);
  }
  ...
  function mintWithSignature(
    ...
    require(_amount <= SUPPLY, "ERC1155AQ: amount should be less than 100");
    // bytes32 h = keccak256((abi.encode(_uri)));
    uint256 length = bytes(_uri).length;
    bytes32 m = keccak256(
      abi.encodePacked("\x19Ethereum Signed Message:\n", length, _uri)
    );
    address addressFromSig = ecrecover(m, _sigV, _sigR, _sigS);
    emit Restored(m, addressFromSig);
    // require(
    //   _account == addressFromSig,
    //   "ERC1155AQ: signature cannot be verified"
    // );
    // lastId = lastId.add(1);
    // super._mint(_account, lastId, _amount, "");
    // _setTokenURI(lastId, _uri);
