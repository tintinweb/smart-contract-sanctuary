...
/**
    ...
    */
    function referereRegister(uint256 _tokenId, string memory _reffererID ) public {
        ...
        // reffererDetails[_tokenId].reffereralID = referralIDCount;
        tokenIdByRefferalCode[_referralCode]= _tokenId;
        // addressByID[referralIDCount] = _tokenId;

        emit NewReferral(_tokenId, msg.sender, _referralCode, _reffererID, false);
    }

    function addAffiliate(uint256 _tokenId, string calldata _ownerId, uint256 _percentage)nftDexContract  external {
    ...
    function referralUsed(uint256 _tokenId, bytes4 referralCode) nftDexContract external returns(uint256, address,string memory) {
        require(tokenIdByRefferalCode[referralCode] == _tokenId && !refferer[_tokenId].isExpired);
        refferer[_tokenId].isExpired = true;
        affiliate[_tokenId].percentage = 0;
        emit ReferralCodeUsed(_tokenId, referralCode);
        return(affiliate[_tokenId].percentage, refferer[_tokenId]._referralCodes[referralCode].reffererAddress, refferer[_tokenId]._referralCodes[referralCode].reffererId);
    }
