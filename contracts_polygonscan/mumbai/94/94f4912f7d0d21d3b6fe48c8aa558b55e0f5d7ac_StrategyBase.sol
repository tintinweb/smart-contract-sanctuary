...
abstract contract BaseStrategy is Ownable {
    ...
    address public constant weth = 0x2295D66190b93e9DFC7ff99099440496514e74C1; //weth for Matic
    ...
    // Dex
    address public constant currentRouter = 0x1D4Ae238eD425aC9C80a1d8213B6Cb876c3f6770; //SushiSwap router
...
abstract contract StrategyFarmBase is BaseStrategyMasterChef {
    ...
    address public constant spade = 0xa6807bbf6e7BD221A64148669F509cDB430DC73e;
    address public constant matic = 0x2295D66190b93e9DFC7ff99099440496514e74C1;
    address public multiHarvest = 0x0000000000000000000000000000000000000000;

    // Other constants
    uint16 public constant underlyingPoolId = 7;

    uint256 public keepFXS = 400;
    uint256 public harvestCutoff = 10**12;

    uint256 public constant keepFXSmax = 10000;

    // Uniswap swap paths
    address[] public spade_matic_path;
    ...
    {
        spade_matic_path = new address[](2);
        spade_matic_path[0] = spade;
        spade_matic_path[1] = matic;
    }
    ...
    function set_harvestCutoff(uint256 newCutoff) external onlyOwner() {
        require(newCutoff <= 10**18, "New cutoff must be less than 1 Spade");
    ...
    function harvest() public override {
        ...
        // Collects SPADE tokens
        _getReward();

        //Swap 1/2 SPADE for MATIC
        uint256 _spadeBalance = IERC20(spade).balanceOf(address(this));
        if (_spadeBalance > harvestCutoff){
            if (_spadeBalance > 0) {
                _swapUniswapWithPath(spade_matic_path, _spadeBalance.div(2));
            }

            // Add liquidity for SPADE/MATIC
            uint256 _matic = IERC20(matic).balanceOf(address(this));
            uint256 _spade = IERC20(spade).balanceOf(address(this));
            if (_matic > 0 && _spade > 0) {
                IERC20(matic).safeApprove(currentRouter, 0);
                IERC20(matic).safeApprove(currentRouter, _matic);
                IERC20(spade).safeApprove(currentRouter, 0);
                IERC20(spade).safeApprove(currentRouter, _spade);
                ...
                // Donates DUST
                IERC20(matic).safeTransfer(
                    strategist,
                    IERC20(matic).balanceOf(address(this))
                );
                IERC20(spade).safeTransfer(
                    strategist,
                    IERC20(spade).balanceOf(address(this))
                );
            }
...
contract StrategyBase is StrategyFarmBase {
    // Token addresses
    address public SPADE_MASTER_CHEF = 0xc890Ae6139d891B6A1a285B7175c8076D4787431;
    address public SPADE_MATIC_LP = 0x92a881935D5eF4B2df7f30C8AFA093BAbf42511a;
