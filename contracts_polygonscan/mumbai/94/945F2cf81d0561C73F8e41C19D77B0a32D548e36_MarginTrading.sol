...
contract MarginTrading {
    ...
    function createPositionandTrade(address tokenAddress, address targetToken, uint256 amount) external {
        ...
        address pair = IUniswapV2Factory(uniswapV2Router.factory()).getPair(tokenAddress, targetToken);
        ...
        uint256 _result = trade(tokenAddress, targetToken, allotedAmount);    
    ...
    function trade(address tokenAddress, address targetToken, uint256 allotedAmount) internal returns(uint256) {
        ...
        else if(targetToken == uniswapV2Router.WETH()){
            // Approve token in
            IERC20(tokenAddress).approve(address(uniswapV2Router), allotedAmount); 
            // Trade and get asset out    
