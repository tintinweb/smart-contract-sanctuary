...
contract Card is ERC721Enumerable, Ownable {
    ...
    function mint(address to, uint256 tokenId) external onlyOwner pausable {
        require(tokenId != 0, "Invalid ID");
        _mint(to, tokenId);
    }

    function mintBatch(address to, uint256[] calldata ids) external onlyOwner pausable {
        require(ids.length >= 1, "Array length must be > 1");
