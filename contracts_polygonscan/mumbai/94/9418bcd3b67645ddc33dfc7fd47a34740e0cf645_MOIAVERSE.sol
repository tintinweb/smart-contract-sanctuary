  ...
  NFT_Types
{
  ...
  /* Inits Contract */
  function initialize(
    string calldata colectionName,
    string calldata collectionSymbol,
    address _erc20TokenAddress,
    bool _publicMinting,
    string calldata metadata
  ) public initializer {
    __MOI_ERC155_init(colectionName, collectionSymbol, metadata);
    __Access_Control_init(_publicMinting);
    __Royalties_init();
    __Supply_init();
    setERC20_TOKEN_ADDRESS(_erc20TokenAddress);
  }

  function setERC20_TOKEN_ADDRESS(address _tokenAddress) public {
  ...
  function push(
    ...
    uint256 _tokenId,
    uint256 supply,
    uint256 royalties_percentage,
    string calldata uri,
    bool isSupplyFixed,
    string calldata _type
  ) external {
    require(bytes(uri).length > 0, 'NO URI'); // dev: Hash can not be empty!
    require(!_exists[_tokenId], 'Token Exists');
    check_nftType(_type);
    mint_internal(msg.sender, reciever, _tokenId, supply);
    setTokenUri(_tokenId, uri);
    setSupplyFixedStatus(_tokenId, isSupplyFixed);
    _exists[_tokenId] = true;
    NFT_Type[_tokenId] = _type;
    setRoyalties(_tokenId, reciever,royalties_percentage);
    bytes memory payload = abi.encodeWithSignature(
      'mint(address, uint256)',
      reciever,
      1
    );
    (bool success, bytes memory returnData) = address(ERC20_TOKEN_ADDRESS).call(
      payload
    );
    require(success, 'ERC20_TOKEN_ADDRESS_CALL_FAILED');
  }
  ...
  function reSupply(
    ...
    require(mintedBy[_tokenId] == reciever);
    require(_exists[_tokenId]);
    require(fixedSupplyStatus[_tokenId]);
    mint_internal(msg.sender, reciever, _tokenId, supply);
  }
  ...
  function mint_internal(
    ...
    require(whiteListed[sender], 'NWL');
    require(supply > 0, 'NO_SPPLY');
    _mint(reciever, id, supply, '');
    emit Mint(id, reciever);
  }
}
  ...
  Storage
{
  ...
  function updateTokenUri(uint256 _tokenId, string calldata _uri) external {
    ...
    require(mintedBy[_tokenId] == msg.sender, 'NMINTP');
...
contract Access_Control is Initializable, OwnableUpgradeable, Storage {
  ...
  function __Access_Control_init(bool _publicMinting)
    internal
    initializer
  {
  ...
  function setWhiteList_Admin(address _addr) external onlyOwner {
    setWhiteList_internal(_addr, true);
  }
...
contract Royalties is Initializable, OwnableUpgradeable, Storage {
  struct Royalties_Config {
    address owner;
    uint256 percentage;
  }

  mapping(uint256 => Royalties_Config) internal _royalties;

  /* Inits Contract */
  function __Royalties_init() internal initializer {
    __Ownable_init();
  }
  ...
  function setRoyalties(
    ...
    _royalties[_id] = Royalties_Config(_owner, _percentage);
  }
  ...
  {
    return (
      _royalties[_nftId].owner,
      ((_salePrice * 10**18) * _royalties[_nftId].percentage) / 100
    );
  }
}
...
contract Supply is Initializable, OwnableUpgradeable, Storage {
    mapping(uint256 => bool) internal fixedSupplyStatus;
    /* Inits Contract */
    ...
    function isSupplyFixed(uint256 _tokenId) public view returns (bool) {
        require(_exists[_tokenId], "TKN N/A");
        return fixedSupplyStatus[_tokenId];
    }

    function toggleSupplyFixedStatus(uint256 _tokenId) external {
        require(whiteListed[msg.sender], "NWL");
        require(_exists[_tokenId], "TKN N/A");
        require(
...
contract NFT_Types {
  ...
  function check_nftType(string calldata _type)
    internal
    pure
    returns (NFT_TYPES)
  {
    ...
    // Loop to check
    if (Hash == keccak256('art') || Hash == keccak256('ART'))
      return NFT_TYPES.ART;
    if (Hash == keccak256('entities') || Hash == keccak256('ENTITIES'))
      return NFT_TYPES.ENTITIES;
    if (Hash == keccak256('equipment') || Hash == keccak256('EQUIPMENT'))
      return NFT_TYPES.EQUIPMENT;
    if (Hash == keccak256('media') || Hash == keccak256('MEDIA'))
      return NFT_TYPES.MEDIA;
    if (Hash == keccak256('wearables') || Hash == keccak256('WEARABLES'))
      return NFT_TYPES.WEARABLES;
    if (Hash == keccak256('space') || Hash == keccak256('SPACE'))
