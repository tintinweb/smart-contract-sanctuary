...
contract ERC721Rarible is ERC721Base {
    ...
    function __ERC721Rarible_init(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI, address exchangeProxy) external initializer {
        __ERC721Rarible_init_unchained(_name, _symbol, baseURI, contractURI, exchangeProxy); // transferProxy, lazyTransferProxy
        emit CreateERC721Rarible(_msgSender(), _name, _symbol);
    }

    // address transferProxy, address lazyTransferProxy
    function __ERC721Rarible_init_unchained(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI, address exchangeProxy) internal {
        _setBaseURI(baseURI);
...
abstract contract ERC721Lazy is IERC721LazyMint, ERC721Upgradeable, Mint721Validator, RoyaltiesV2Upgradeable, RoyaltiesV2Impl {
    ...
    function verifyLazyAsset(LibERC721LazyMint.Mint721Data memory data) internal virtual view returns(bool) {
        require(!_burned(data.tokenId), "token already burned");
        require(!_exists(data.tokenId), "Token id already exists");
        address creator = address(data.tokenId >> 96);
        address sender = _msgSender();
        
        // In order for ensuring token id has creator encoded in it
        require(creator == data.creators[0].account, "tokenId incorrect");

        // 1 signature should be there.
        require(data.signatures.length == 1);

        // By convention - minter is the last creator and signs first
        bytes32 hash = LibERC721LazyMint.hash(data);
        address minter = data.creators[data.creators.length - 1].account;
        if (minter != sender) {
                validate(minter, hash, data.signatures[0]);
        }

        return true;
    }

    function encodeLazyMintData(LibERC721LazyMint.Mint721Data calldata data) external virtual view returns(bytes memory) {
        bytes memory encoded = abi.encode(address(this), data);
        return encoded;
    }


    // Can be called by exchange to validate and retrieve necessary data to execute sale of an auction/order matching by just having the encoded data vs
    // knwoing the data strucutre itself
    function decodeLazyMintData(bytes calldata encoded) external virtual view returns(uint256, address, address){
        (address token, LibERC721LazyMint.Mint721Data memory data) = abi.decode(encoded, (address, LibERC721LazyMint.Mint721Data));
        require(token == address(this), "Lazy asset not of this contract");
        require(verifyLazyAsset(data), "Lazy asset data is not valid");
        // By convention auctioneer is the last creator and same as minter
        return (data.tokenId, data.creators[0].account, data.creators[data.creators.length - 1].account);
    }

    // Can be called by exchange to validate and retrieve necessary data to execute sale of an auction/order matching by just having the encoded data vs
    // knwoing the data strucutre itself
    function decodeAndFormatLazyMintData(bytes calldata encoded) external virtual view returns(LibERC721LazyMint.Mint721Data memory){
        (address token, LibERC721LazyMint.Mint721Data memory data) = abi.decode(encoded, (address, LibERC721LazyMint.Mint721Data));
        // By convention auctioneer is the last creator and same as minter
        return data;
    }
    ...
    function mintAndTransfer(LibERC721LazyMint.Mint721Data memory data, address to) public override virtual {
        ...
        require(verifyLazyAsset(data), "Asset is not a valid ERC721Lazy asset");
    ...
    // Can be called by an external contract with the encoded data to execute mintAndTransfer function without knowing the lazy form data structure
    function mintAndTransferEncodedData(bytes memory encoded) external virtual {

        (address token, LibERC721LazyMint.Mint721Data memory data) = abi.decode(encoded, (address, LibERC721LazyMint.Mint721Data));
        require(token == address(this), "Lazy asset is not of this contract");
        mintAndTransfer(data, data.creators[0].account);
    }
