...
contract StakeSwap is Initializable, OwnableUpgradeable{
    ...
    function stake( uint _amount, address _recipient ) external returns ( bool ) {
        ...
        if(info.stakeRecordEnd == 0){
            accountStakeRecord[info.id][0] = Record(_amount, block.number);
        }else{
            accountStakeRecord[info.id][info.stakeRecordEnd + 1] = Record(_amount, block.number);
        }
    ...
    function extract( uint _amount, address _recipient ) external returns ( bool ) {
        ...
        require( IVotedToken( _delegates[_recipient] ).deposit(msg.sender, _amount ),
            "SS::003" );
