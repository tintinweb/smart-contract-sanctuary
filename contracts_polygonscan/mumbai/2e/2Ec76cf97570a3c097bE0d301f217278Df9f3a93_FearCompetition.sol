...
import "hardhat/console.sol"; // #FIXME remove debug output import
...
contract FearCompetition is ERC1155Holder, AccessControl, Pausable, ReentrancyGuard {
    ...
    uint256 public constant DEFAULT_BUYIN = 5;
    ...
    mapping (uint256 => mapping (address => bool)) private competitionParticipants;
    ...
    modifier onlyManager() {
        require(hasRole(MANAGER_ROLE, _msgSender()), "Manager role required");
        _;
    }

    constructor(address fearNftTokenAddress, address dateTimeHelperAddress) {
        ...
        Competition memory zeroCompetition;
        competitions[0] = zeroCompetition;
    ...
    function _getCompetitionId(uint256 timestamp) private view returns (uint256) {
        for (uint i = competitionsIds.length - 1; i > 0; i--) {
            if (_competitionAt(i).start <= timestamp && timestamp <= _competitionAt(i).end) // timestamp inside competition start/end
                return competitionsIds[i];
            if (_competitionAt(i).end < timestamp) // sorted lookup cutoff - no need to iterate further (everything else will have earlier start/end)
                return 0;
        }
        return 0;
    }
    ...
    function _getCurrentCompetitionId() private view returns (uint256) {
        return _getCompetitionId(block.timestamp);
    }
    ...
    function _getClosestAvailableEndDate() private view returns (uint256) {
        ...
        for (uint i = competitionsIds.length - 1; i > 0; i--) {
            ...
            if (_competitionAt(i).end < block.timestamp) // sorted lookup cutoff
                return sundayEnd;
        }
        return sundayEnd;
    }
    ...
    function _getNextDefaultCompetition() private view returns (Competition memory) {
        return Competition(competitionsIds.length, _getClosestAvailableStartDate(), _getClosestAvailableEndDate(), DEFAULT_BUYIN, 0);
    }

    function _createCompetition(uint256 id, uint256 start, uint256 end, uint256 buyin) private {
        competitions[id] = Competition(id, start, end, buyin, 0);
    ...
    function _checkParticipation(uint256 id, address user) private view returns (bool) {
        return competitionParticipants[id][user];
    }
    ...
    */
    function enterCurrentCompetition() external whenNotPaused nonReentrant {
        ...
        require (!competitionParticipants[curCompetition.id][_msgSender()], "You are participating in current competition already");

        uint256 userBalance = fearNftToken.balanceOf(_msgSender(), BUYIN_NFT_ID);
        require(userBalance >= curCompetition.buyin, "Insufficient balance to enter current competition");
        ...
        competitionParticipants[curCompetition.id][_msgSender()] = true;
    ...
    * @return Array of NFT ids corresponding to array of NFT amounts caller has on contract balance.
    */
    ...
    * - 'end' must be greater than last block timestamp;
    * - Competition period must not intersect any other existing competition period.
    */
    function addCompetition(uint256 start, uint256 end, uint256 buyin) external onlyManager {
        require(start < end, "Competition start timestamp cannot be greater or equal to end timestamp");
        require(buyin > 0, "Buyin must be greater than zero");
        //require(end > block.timestamp, "Cannot add past competition"); // #FIXME activate this condition before real deployment?s
        ...
        for (uint i = competitionsIds.length - 1; i > 0; i--) {
            if ((_competitionAt(i).start <= start && start <= _competitionAt(i).end)
                ...
                revert("Existing competition with intersecting start/end timestamps found");
            if (_competitionAt(i).end < start)
                break;
        }

        _createCompetition(competitionsIds.length, start, end, buyin);
    }
    ...
    */
    function getCompetitionIds(uint256 fromTime, uint256 toTime) external view onlyManager returns (uint256[] memory) {
        require(fromTime <= toTime, "Incorrect search period: fromTime cannot be greater than toTime");

        uint256[] memory result;
        if (fromTime == toTime) {
            result = new uint256[](1);
            result[0] = _getCompetitionId(fromTime);
        }
        else {
            uint firstIndex;
            uint lastIndex;
    ...
    * - Caller must have 'MANAGER_ROLE'.
    */
    function getCompetition(uint256 id) external view onlyManager returns (Competition memory) {
    ...
    * - Caller must have 'MANAGER_ROLE'.
    */
    function checkParticipation(uint256 id, address user) external view onlyManager returns (bool) {
    ...
    * Requirements:
    * - Caller must have 'MANAGER_ROLE';
    * - The competition must be finished at the current time;
    * - Winners for the competition must not set before;
    ...
    */
    function setWinners(uint256 id, address[] calldata winners) external onlyManager {
        // #FIXME IMPLEMENT FUNCTION
    }
    ...
    */
    function unpause() external onlyPauser whenPaused {
        require(_isMinterOfFearNft(), "Before unpausing the contract must be a minter of FearNft contract");
