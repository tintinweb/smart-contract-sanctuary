...
contract EmbraceCards is Ownable, ERC721Enumerable, GenerationalRandomMintable, ERC721Burnable, ERC721Pausable {
    ...
    uint256 private promotionalMintIndex = 5001;
//    uint256 private promotionalMaxRemaining = 1000;
    ...
    uint256[] private latestNumbersMinted;

    // use msg.sender instead for now
    function mint(/*address payable _to, */ uint256 _count) public payable saleIsOpen {
        ...
        for (uint256 i = 0; i < _count; i++) {
            ...
            _mintAnElement(msg.sender, randomAvailableIndex);
            //            emit MintEvent(_to, randomAvailableIndex);
            latestNumbersMinted.push(randomAvailableIndex);
        }
    }

    function promotionalMint(/*address payable _to, */ uint256 _count) public payable saleIsOpen {
        uint256 total = _totalSupply();

        require(total + _count <= nftMaxSupply, "MINT: count arg would exceed supply.");
        require(total <= nftMaxSupply, "MINT: Please go to the Opensea to buy Embrace Cards.");
        require(promotionalMintIndex - 1 + _count <= 5100, "MINT: max promotional cards reached");

        for (uint256 i = 0; i < _count; i++) {
//            _safeMint(_to, promotionalMintIndex + i);
            _mintAnElement(msg.sender, promotionalMintIndex + i);
        }
        promotionalMintIndex += _count;
        // console.log('promotionalMintIndex', promotionalMintIndex);
    }

    function getLatestMintingInfo() public view returns (uint256, uint256[] memory) {
        return (genStartIndex, latestNumbersMinted);
    }

    function _mintAnElement(address /*payable*/ _to, uint256 mintId) private {
        _safeMint(_to, mintId);
        _tokenIdTracker.increment();
      //  console.log('minted NFT', mintId);
    }
...
contract GenerationalRandomMintable is Ownable {

    uint256 idSpaceStart = 1;
    uint256 private _currentStartIndex;

    // ---- DRAW RANDOM CARD ----
    uint256 public remaining;
    uint256 public genStartIndex;

    // ---- MULTI OR DELETABLE MAPPING ----
    uint256 private _currentMappingVersion;
    mapping(uint256 => uint256) public cache;

    // ---------- ID SPACE DATA (extra data, maybe for future version) -------------------

//    uint256 idSpace;
//
//    constructor(uint256 privilege, uint256 startIndex, uint256 size, uint256 extra)
//    {
//        uint256 creationTime = block.timestamp;
//        uint256 newIdSpace = uint256(startIndex);
//        //        newIdSpace |= startIndex << 64;
//        newIdSpace |= size << 64;
//        newIdSpace |= privilege << 128;
//        newIdSpace |= creationTime << 192;
//        idSpace = newIdSspace;
//        idSpaceStart = startIndex;
//    }
    ...
    struct Generation {
        ...
        uint256 numItems;
        uint256 startIndex;
        uint256 provenanceProof;
    }
    Generation[] private _generationsArr;

    function getCurrentGeneration() public view returns (Generation memory) {
        return _generationsArr[_generationsArr.length -1];
    }

    // Update price and save in price history
    function makeNewGeneration(uint256 numItems) public onlyOwner {
        uint256 countGenerations = _generationsArr.length;
        Generation memory newGen;
        newGen.numItems = numItems;

        if (countGenerations > 0) {
            Generation memory lastGen = _generationsArr[countGenerations - 1];
            newGen.index = lastGen.index + 1;
            newGen.startIndex = lastGen.startIndex + lastGen.numItems;
        } else {
            newGen.index = 1;
            newGen.startIndex = idSpaceStart;
        }
    ...
    function drawIndex() internal returns(uint256 adjIndex) {
        //RNG
        ...
        uint256 index = foundCard == 0 ? i : foundCard;
        adjIndex = index + genStartIndex;
//        console.log('adjIndex', adjIndex, genStartIndex);
