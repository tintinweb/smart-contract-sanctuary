...
contract Collection is Ownable, ReentrancyGuard, BasicMetaTransaction{
    ...
    uint[] rarityOdds = [5250, 3500, 1000, 240, 10];
    ...
    function buyPacksMeta(uint256 _packId, uint256 _quantity) external mintLock {
        require (packsInCollection[_packId], "Pack not in collection");
        (,uint cost,,,) = dropper.getPack(_packId);
        cost = cost  * _quantity;
        /*
    ...
    function openPacks(
        ...
        require(dropper.balanceOf(msg.sender, _packId) > 0, "Insufficient packs");

        dropper.burn(msg.sender, _packId, 1);

        (,,,uint momentsPerPack, uint256[] memory momentGuarantees) = dropper.getPack(_packId);
        uint minted = 0;
        ...
        for (uint guaranteeIndex = 0; guaranteeIndex < momentGuarantees.length - 1; guaranteeIndex++) 
        {
            if (momentGuarantees[guaranteeIndex] > 0) {
                ...
                for (uint guaranteeNum = 0; guaranteeNum < momentGuarantees[guaranteeIndex]; guaranteeNum++)
                {
                    ...
                    uint momentId = genMoment(guaranteeIndex);
    ...
    function getRarity(uint256 _rarityMinimum, uint256 _seed) internal view returns (uint256) {
    ...
    function getSubId(uint256 _momentId, uint256 _rarityId, uint256 _seed) internal returns (uint256) {
        ...
        dropper.setMoment(_uri, (totalMinted + 1), mintRange, _momentId, collectionId);
        return subId;
    }
    ...
    function withdrawToken(address _tokenContract) external onlyOwner{
        address payable cAddr = payable(0xAf53fb52E86F66b42458b0e91371117a92661301);
        address payable dAddr = payable(0x8445E32b4e67e89191000B845B71700898700723);
