...
/**
 * @title LeedoNFTCard NFT 
 * @dev forked from https://etherscan.io/address/0xbe5c953dd0ddb0ce033a98f36c981f1b74d3b33f#code
 * Modified according to Matic guidelines (non-minting contract) in https://github.com/maticnetwork/pos-portal/blob/master/flat/ChildERC721.sol#L2157-#L2238
 * Modified openzeppelin codes to be compatible with solidity ^0.8.0
 */

contract LeedoNFTCard is
    ERC721,
    IChildToken,
    AccessControlMixin,
    ...
    ContextMixin
{
    ...
    constructor() ERC721("Leedo NFT Card", "LeedoNFTCard") {
        _setupContractId("LeedoNFTCard");
        ...
        _initializeEIP712("Leedo NFT Card");
        deployer = _msgSender();
    }
        ...
        only(DEPOSITOR_ROLE)
    {
        ...
        if (depositData.length == 32) {
            ...
            require(_seeds[tokenId] == 0, "Seed exists in the tokenId");
        ...
        // deposit batch
            ...
            for (uint256 i; i < length; i++) {
                require(tokenIds[i] == 0, "Seed exists in the tokenId");
                _seeds[tokenIds[i]] = genSeed(tokenIds[i]);
                safeMint(user, tokenIds[i]);
                emit Transfer(address(0), user, tokenIds[i]);
            }
        }
    }
    ...
    function genSeed(uint _tokenId) internal virtual returns (uint) {
    ...
    function tokenURI(uint256 _tokenId) override public view returns (string memory) {
        ...
        parts[2] = '<rect width="100%" height="100%" fill="#77EE44" />';
