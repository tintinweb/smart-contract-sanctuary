...
contract DailyPrize is OwnableUpgradeable, VRFConsumerBaseUpgradeable {
    ...
    function winnings(uint256 _day) public view returns (address[] memory, uint256[] memory) {
        ...
        for (uint256 i; i < dailyWinners[_day].length; i++) {
            prizes[i] = dailyPrizes[_day][dailyWinners[_day][i]];
        }
    ...
    function drawWinners(uint256 _day, address[] memory _winners, uint256[] memory _prizes) public onlyOwner {
        require(_randFulfilled[_day], "randomness not fulfilled");
        require(_winners.length == _prizes.length, "invalid input data");

        for (uint256 i = 0; i < _winners.length; i++) {
            dailyWinners[_day].push(_winners[i]);
            dailyPrizes[_day][_winners[i]] = _prizes[i];
        }
