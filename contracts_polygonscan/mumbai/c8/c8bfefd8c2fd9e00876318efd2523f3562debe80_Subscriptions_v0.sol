...
contract Subscriptions_v0 is Initializable, Ownable {
    ...
    function cancel(address recipient) external {
        ...
        require(
            msg.sender == payer || msg.sender == recipient,
            "Subscriptions: subscription can be cancelled by payer or recipient only"
        );
    ...
    function renew(address recipient) external {
        ...
        require(
            expiration(recipient) < block.timestamp.add(renewBeforeVal),
            "Subscriptions: too early to renew"
        );
    ...
    function _subscribe(
        ...
        require(
            recipient != address(0),
            "Subscriptions: recipient should not be 0"
        );
        ...
        ISubredditPoints(_subredditPoints).operatorBurn(
            payer,
            priceVal,
            "",
            ""
