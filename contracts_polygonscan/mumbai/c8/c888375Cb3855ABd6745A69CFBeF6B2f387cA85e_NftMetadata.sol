...
contract NftMetadata is INftMetadata, Ownable
{
  ...
  constructor ()
  {
    setNamespaceData ("",
        ...
        "https://extra.domob.eu/BG-x.svg", "ffffff", "Unknown");
    setNamespaceData ("p",
        "A player account for the Xaya platform.  All in-game assets are"
        " associated to account NFTs.",
        "https://extra.domob.eu/BG-p.svg", "ffffff", "Player Account");
    setNamespaceData ("g",
        "The admin account for a game on the Xaya platform.",
        "https://extra.domob.eu/BG-g.svg", "ffffff", "Game");
  }
  ...
  /**
   * @dev Basic URL encoding (mainly of + and /, for base64 data).
   */
  function urlEncode (string memory str) internal pure returns (string memory)
  {
    bytes memory data = bytes (str);
    StringBuilder.Type memory builder = StringBuilder.create (3 * data.length);
    for (uint i = 0; i < data.length; ++i)
      {
        if (data[i] == 0x2B)
          StringBuilder.append (builder, "%2b");
        else if (data[i] == 0x2F)
          StringBuilder.append (builder, "%2f");
        else
          {
            bytes memory literal = new bytes (1);
            literal[0] = data[i];
            StringBuilder.append (builder, string (literal));
          }
      }
  ...
  {
    return string (abi.encodePacked (
      "https://extra.domob.eu/base64.php?t=", urlEncode (mimeType),
      "&d=", urlEncode (Base64.encode (payload))
    ));
    /*
    return string (abi.encodePacked (
      "data:", mimeType,
