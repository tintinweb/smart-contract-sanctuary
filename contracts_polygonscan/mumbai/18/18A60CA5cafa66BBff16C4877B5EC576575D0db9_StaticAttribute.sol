    ...
    AttributeClass
{
    ...
    mapping(uint256 => StaticSettings) public attrs;
    ...
    function create(
        ...
        require(!_exists(_id), "StaticAttribute: attribute _id already exists");
        StaticSettings memory settings = StaticSettings({
            name: _name,
        ...
        emit StaticAttributeCreated(_name, _id, _description, _value);
    }
    ...
    function link(uint256 _nftId, uint256 _attrId)
        ...
        onlyOperator
    {
        require(_exists(_attrId), "StaticAttribute: attribute _id not exists");
        require(
        ...
        emit StaticAttributeLinked(_nftId, _attrId);
    }
        ...
        onlyOperator
    {
        ...
        emit StaticAttributeRemoved(_nftId, _attrId);
    }
...
interface IStaticAttribute {
    ...
    event StaticAttributeCreated(
        string name,
        uint256 id,
        string description,
        string value
    );
    event StaticAttributeLinked(uint256 nftId, uint256 attrId);
    event StaticAttributeRemoved(uint256 nftId, uint256 attrId);
}
