...
//NOTE I MADE MATURITY TO BE OUT OF 1,000,000
//NOTE I MADE IT SO PARENT DATA IS SAVED FROM MAD HOUSE BREEDING
contract EggCarton is FxBaseChildTunnel, VRFConsumerBase, Ownable, ERC721Holder {
    ...
    uint public maxMaturity = 1000000;//NOTE I MADE THIS OUT OF 1,000,000
    uint public minMaturity = 500000;//min required maturity to hatch an egg
    ...
    mapping(uint => bool) public eggOrigin;
    IEgg Egg;
    ...
    //NOTE is there a way to store this info so that bots don't need to know what eggID to call
    mapping(uint => address) public eggsToDispense;//NOTE should I increment and decrement a counter var showing how many eggs need to be dispensed
    //FIFO for egg breeding
    uint public headEgg;
    uint public tailEgg;
    mapping(uint => uint) public eggFIFO;

    //FIFO for mad house breeding
    uint public headMH;
    uint public tailMH;
    mapping(uint => uint) public MHFIFO;
    ...
    function setParameters(uint _maturityMultiplier, uint _bonusMultiplier, uint _baseMultiplier, uint _gen0Bonus, uint _bonusDifficulty) external onlyOwner{
    ...
    function viewMetaData(uint tokenID) public view returns(string[8] memory data){
    ...
    function viewRarest(uint mom, uint dad) public view returns(string[8] memory data){
    ...
    function sendEggToCaller() external returns(uint){
        uint baby = headEgg;
        ...
        if(headEgg == 0){
            //nothing else to do sp wait the task specified amount of time
            return 0;
        }
        else{
    ...
    function _findTraits(bytes memory message) internal {
        (uint tID, uint mom, uint dad) = abi.decode(message, (uint, uint, uint));
        uint[2] memory parents;
        parents[0] = mom;
        parents[1] = dad;
    ...
    function findTraits() external returns(uint) {
        uint tokenID = headMH;
        require(messages[tokenID].rngSet, 'RNG not set');
        require(!messages[tokenID].done, 'Traits already calculated for tokenID');
        uint[2] memory parents = Egg.getParents(tokenID);
        require(parents[0] > 0 || parents[1] > 0, 'Messages is not set');//should stop people from calling findTraits on a hatched egg
