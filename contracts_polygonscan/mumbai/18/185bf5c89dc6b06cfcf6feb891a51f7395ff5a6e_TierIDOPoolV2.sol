...
// File: contracts/TierIDOPoolV2.sol

pragma solidity 0.6.12;

contract TierIDOPoolV2 is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct TierAllowance {
        uint256 min; // Min purchase amount in token
        uint256 max; // Max purchase amount in token
    }
    ...
    mapping(uint8 => TierAllowance) public tierAllowances;
    // TGE claim
    ...
    constructor(
        ...
        require(
            0 <= _tgeClaimPercentage && _tgeClaimPercentage <= 100,
            "_tgeClaimPercentage must be >= 0 and <= 100"
        );
        token = IERC20(_token);
    ...
    function setTierAllowance(
        ...
        require(
            _maxPurchase <= totalAmount,
            "_maxPurchase must be <= _totalAmount"
        );
        tierAllowances[_tier] = TierAllowance(_minPurchase, _maxPurchase);
    }
    ...
    function buy(uint256 amount) external publicSaleActive nonReentrant {
        ...
        // everyone can purchase without tier in round 2
    ...
    function claimTokens() external canClaim nonReentrant {
        ...
        uint256 tgeClaimAmount = sale.amount.mul(tgeClaimPercentage).div(100);
        token.transfer(sale.investor, tgeClaimAmount);
        emit Claim(msg.sender, tgeClaimAmount);
    }
