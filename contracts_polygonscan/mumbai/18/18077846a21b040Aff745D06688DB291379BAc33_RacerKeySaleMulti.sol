...
contract RacerKeySaleMulti is IERC1155Receiver, AccessControlEnumerable {
    ...
    function purchaseKeys(uint256 quantity) public payable {
        ...
        if (saleInNativeToken) {
            ...
            purchaseKeysWithAltToken(quantity);
        }
    }

    function purchaseKeysWithAltToken(uint256 quantity) internal {
        require(
            SALE_TOKEN.allowance(msg.sender, address(this)) >=
                quantity * salePrice,
            "Contract not approved to spend enough user tokens for requested quantity."
        );

        require(
            SALE_TOKEN.balanceOf(msg.sender) >= quantity * salePrice,
            "Wallet balance too low."
        );

        SALE_TOKEN.transferFrom(
            msg.sender,
            address(this),
            quantity * salePrice
        );
        ...
        _keysPurchased[msg.sender] += quantity;
        keysSold += quantity;

        if (allowedMint[msg.sender] > quantity) {
            allowedMint[msg.sender] -= quantity;
        } else {
            allowedMint[msg.sender] = 0;
        }
    }
