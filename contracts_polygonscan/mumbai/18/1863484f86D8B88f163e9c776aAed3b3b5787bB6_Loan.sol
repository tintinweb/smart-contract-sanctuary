...
contract Loan is AccessProtected, ReentrancyGuard, BaseRelayRecipient, IERC721Receiver, EIP712Base {
    ...
    function createLoanableItem(
        ...
        require(_reservedTo != senderAddress, "Cannot reserve loan to owner");
        ...
        loanItems[newLoanId].owner = senderAddress;
        ...
        for (uint256 i = 0; i < _nftIds.length; i++) {
            ...
            IERC721(_nftAddresses[i]).transferFrom(senderAddress, address(this), _nftIds[i]);
        }
        emit LoanableItemCreated(senderAddress, _nftAddresses, _nftIds, newLoanId, _reservedTo, _claimer);
        return newLoanId;
    }
