...
contract SocialTokenDeployer {
    ...
    function spawn(
        ...
        SocialERC20(tokenAddress).initialize(msg.sender, name, symbol, totalCap, initialMint, tax, taxTo);
        emit TokenSpawned(
            tokenAddress,
            msg.sender,
            msg.sender,
    ...
    function spawnFor(
        ...
        address tokenAddress = Clones.clone(_tokenBlueprint);
        SocialERC20(tokenAddress).initialize(owner, name, symbol, totalCap, initialMint, tax, taxTo);
        emit TokenSpawned(tokenAddress, owner, msg.sender, totalCap, initialMint, totalCap - initialMint);
    }
}
...
contract SocialERC20 is ERC20, AccessControl, Initializable {
    ...
    mapping(address => VestingEntry[]) private _vesting;
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        _transfer(_msgSender(), _taxTo, taxAmount);
    ...
    function allocateTokens(
        ...
        for (uint256 i = 0; i < addresses.length; i++) {
            ...
            availableReserves -= amounts[i];
            _vesting[addresses[i]].push(VestingEntry({
                // solhint-disable-next-line
    ...
    function claimTokens(address to) external {
        ...
        require(_vesting[msg.sender].length > 0, "No vested tokens for caller");
        for(uint i = 0;i < _vesting[msg.sender].length;i++) {
            VestingEntry storage entry = _vesting[msg.sender][i];
            // solhint-disable-next-line
