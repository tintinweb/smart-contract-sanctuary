...
contract HarvesterNFT is ERC721URIStorage, ERC721Holder, ReentrancyGuard, Ownable {
    ...
    // 用户是否为采集器持有者映射：拥有者 => true/false
    // mapping(address => bool) private harvesterOwner; 
    ...
    string internal nftName = "MetaGlobe Harvester NFT";
    ...
    constructor( uint256 _seed) ERC721("HarvesterNFT", "Harvester") {
  ...
  //操作员设置采集器升级许可及升级的数值
  function setAllowUpgrades(uint tokenId, uint256 _energy_efficiency, uint256 _uru_efficiency, uint256 _max_durability)       
        public
        isOperator
        nonReentrant
        returns (bool)
  ...
  // 升级方法
  function upgradeHarvester(uint256 tokenId) 
      public
      nonReentrant
      returns (uint256)
  {
  ...
  function getOwnerListByTokenId(uint256 startTokenId, uint256 endTokenId) external view returns (address[] memory) {
    require(endTokenId <= _TokenIds.current(), "endTokenId must less than total");
    require(endTokenId - startTokenId <= 200 , "length must less than 200");
    require(endTokenId >= startTokenId, "startTokenId  must less than endTokenId");
    address[] memory result = new address[](endTokenId - startTokenId +1);
    uint counter = 0;
    for (uint i = startTokenId; i < endTokenId +1 ; i++) {
