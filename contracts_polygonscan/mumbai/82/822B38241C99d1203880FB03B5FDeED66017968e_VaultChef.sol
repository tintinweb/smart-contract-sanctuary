...
contract VaultChef is Ownable, ReentrancyGuard, Operators {
    ...
    address public compounderAddress;

    event AddPool(address indexed strat);
    event Deposit(address indexed user, uint256 indexed pid, uint256 amount);
    event Withdraw(address indexed user, uint256 indexed pid, uint256 amount);
    event EmergencyWithdraw(address indexed user, uint256 indexed pid, uint256 amount);
    event CompounderChanged(address indexed oldCompounder, address indexed newCompounder);
    ...
    modifier onlyCompounder() {
        require(msg.sender == compounderAddress, 'VaultChef: only compounder');
        _;
    }

    function changeCompounder(address _compounderAddress) external onlyOwner nonReentrant {
        emit CompounderChanged(compounderAddress, _compounderAddress);
        compounderAddress = _compounderAddress;
    }
    ...
    function earnPools(uint256 pidFrom, uint256 pidTo) external onlyOwner{
        uint256 len = poolInfo.length;
        require((pidFrom >= 0) && (pidTo <= len) && (pidFrom < pidTo), 'VaultChef: Invalid pool indices');
