...
using SafeMath for uint112;
 ...
 function init1() public {
  ...
  function buyBox(address seller, string memory ownerId, string memory name) public {
    ...
    for(uint256 i =0; i<Category[name].countNFT; i++)
    {
      ...
      uint256 randNumber = (uint256(keccak256(abi.encode(randomValueMain, (block.timestamp +i)))) % Category[name].count);
      ...
      Category[name].tokenUris[randNumber] = Category[name].tokenUris[Category[name].count];
      delete Category[name].tokenUris[Category[name].count];
      Category[name].count--;
 ...
 function timeTester() internal {
  ...
  function getBoxDetail(string memory name) public view returns(string memory, string memory, uint256,uint256, uint256, uint256){
    return (Category[name].title, Category[name].description, Category[name].price,Category[name].usdPrice,Category[name].countNFT, Category[name].count);
  }
