...
import "./interfaces/IAlcheMarketplace.sol";
  ...
  IAlcheMarketplace
{
  ...
  address public alcheTokenAddress;
  address public alcheNFTAddress;
   ...
   * @param _nft address of alche NFT
   */
  function __AlcheMarketplace_init(address _token, address _nft) public initializer {
    __Ownable_init();
  ...
  function supportsInterface(bytes4 interfaceId) public view virtual override(AccessControlUpgradeable) returns (bool) {
    return interfaceId == type(IAlcheMarketplace).interfaceId || super.supportsInterface(interfaceId);
  }
  ...
  function setAddresses(address _token, address _nft) public onlyOwner {
    ...
    alcheTokenAddress = _token;
    alcheNFTAddress = _nft;
  }
  ...
  function buy(uint256 _id, address _owner) public override nonReentrant {
    ...
    require(
      IERC20(alcheTokenAddress).balanceOf(sender) >= (buyPrice + buyerMarketFee + royaltyFee),
      "Marketplace:buy03"
    );
    ...
    IERC20(alcheTokenAddress).transferFrom(_msgSender(), address(this), buyPrice + buyerMarketFee + royaltyFee);
    ...
    if (IERC721(alcheNFTAddress).ownerOf(_id) == address(this)) {
      _transferNFT(address(this), sender, _id);
    } else {
    ...
    IAlchero(alcheNFTAddress).setTokenOwner(sender, _id);
  ...
  function bid(
    ...
    require(requiredCost <= IERC20(alcheTokenAddress).balanceOf(sender), "Marketplace:bid04");
    ...
    IERC20(alcheTokenAddress).transferFrom(sender, address(this), requiredCost);
  ...
  function lazyMintingBuy(
    ...
    IAlchero(alcheNFTAddress).redeem(_creator, _id, _royaltyPercentage, 0, _price, 0, 0, 0, 1, _signature);
  ...
  function lazyMintingBid(
    ...
    address sender = _msgSender();
    require(requiredCost <= IERC20(alcheTokenAddress).balanceOf(sender), "Marketplace:lazyMintingBid04");

    IAlchero(alcheNFTAddress).redeem(
      _creator,
      _id,
      _royaltyPercentage,
      _priceType,
    ...
    IAlchero(alcheNFTAddress).setApprovalForAllByMarketplace(_creator, address(this), true);
    IERC721(alcheNFTAddress).safeTransferFrom(_creator, address(this), _id);
  ...
  function buyNow(uint256 _id, address _owner) external {
    ...
    require(requiredCost <= IERC20(alcheTokenAddress).balanceOf(sender), "Marketplace:buyNow04");
  ...
  function lazyMintingBuyNow(
    ...
    IERC20(alcheTokenAddress).transferFrom(sender, address(this), _buyNowPrice + variables[1] + variables[0]);
    ...
    // IERC20(alcheTokenAddress).transferFrom(sender, _creator, _buyNowPrice - variables[2]);
  ...
  function _refundBid(uint256 _id, address _owner) private {
    ...
    if (bidBalance[currentBid.bidder] > 0 && currentBid.bidder != _owner) {
      ...
      bidBalance[currentBid.bidder] = bidBalance[currentBid.bidder] - (currentBid.price + buyerMarketFee + royaltyFee);
      IERC20(alcheTokenAddress).transfer(currentBid.bidder, currentBid.price + buyerMarketFee + royaltyFee);
    }
  }
  ...
  function _getRoyaltyFeePercentage(uint256 _id) private view returns (uint256) {
    return IAlcheRoyalty(alcheNFTAddress).getRoyaltyPercentage(_id);
  }
  ...
  function _calculateRoyaltyFee(uint256 _id, uint256 _amount) private view returns (uint256) {
    return (_amount * IAlcheRoyalty(alcheNFTAddress).getRoyaltyPercentage(_id)) / NOMINATOR;
  }
  ...
  function _validateBeforeSale(uint256 _id) private view {
    address owner = IERC721(alcheNFTAddress).ownerOf(_id);
    address sender = _msgSender();
  ...
  function _transferNFT(
    ...
    IAlchero(alcheNFTAddress).setApprovalForAllByMarketplace(_from, address(this), true);
    IERC721(alcheNFTAddress).safeTransferFrom(_from, _to, _id);
  }
  ...
  function _transferRoyaltyFeeFromBuyer(uint256 _id, uint256 _amount) private {
    IERC20(alcheTokenAddress).transferFrom(_msgSender(), IAlcheCreator(alcheNFTAddress).getCreator(_id), _amount);
  }

  function _transferRoyaltyFeeFromMarketplace(uint256 _id, uint256 _amount) private {
    IERC20(alcheTokenAddress).transfer(IAlcheCreator(alcheNFTAddress).getCreator(_id), _amount);
  }
  ...
  function _transferMarketFee(uint256 _amount) private {
    IERC20(alcheTokenAddress).transferFrom(_msgSender(), address(this), _amount);
  }
  ...
  function _payout(
    ...
    IERC20(alcheTokenAddress).transfer(_owner, _price - _sellerMarketFee);
    IERC20(alcheTokenAddress).transfer(IAlcheCreator(alcheNFTAddress).getCreator(_id), _royalty);
  }
  ...
  //   uint256 _buyerMarketFee
  // ) private {
  //   IERC20(alcheTokenAddress).transferFrom(_msgSender(), _owner, _price - _sellerMarketFee);
