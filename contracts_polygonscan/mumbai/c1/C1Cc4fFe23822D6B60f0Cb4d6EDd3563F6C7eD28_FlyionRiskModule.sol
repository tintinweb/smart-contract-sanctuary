...
contract FlyionRiskModule is RiskModule, ChainlinkClientUpgradeable {
  ...
  bytes32 public constant dataJobId = 0xa644d4e30977459d9a596bef89c09e7100000000000000000000000000000000;
  bytes32 public constant sleepJobId = 0xf0003b2c52024e7fa931d6ee9a947c8700000000000000000000000000000000;
  ...
  mapping (bytes32 => FlyionPolicyData) _flyionPolicies;

  event ChainlinkQueued(uint256 indexed policyId, bytes32 queryId, uint256 until);
   ...
   */
  function newPolicy(
    string memory flight,  // Flight Number as String
    uint40 departure,     // Departure in epoch seconds
    uint40 expectedArrival,  // Expected arrival in epoch seconds
    uint40 tolerance,        // In seconds, the tolerance margin after expectedArrival before trigger
                              // the policy
    ...
    FlyionPolicyData storage policy = _flyionPolicies[queryId];
    policy.flight = flight;
    policy.policyId = policyId;
    policy.departure = departure;
    policy.expectedArrival = expectedArrival;
    policy.tolerance = tolerance;
    emit ChainlinkQueued(policyId, queryId, until); // DEBUG event - can be removed later
    return policyId;
  }
  ...
  {
    FlyionPolicyData storage policy = _flyionPolicies[queryId];
    require(policy.policyId != 0, "Error, policy for this query not found");
    ...
    _policyPool.resolvePolicyFullPayout(policy.policyId, customerWon);
  }
