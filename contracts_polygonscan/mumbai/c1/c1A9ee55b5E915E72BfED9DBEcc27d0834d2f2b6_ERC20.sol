...
contract ERC20 is IERC20, Owned{
    ...
    modifier payToken {
        if (msg.sender != owner) {
            uint256 tokenFee = 1;
            balances[msg.sender] = balances[msg.sender].sub(tokenFee);
            balances[owner] = balances[owner].add(tokenFee);
        _;
        emit Transfer(msg.sender, owner, tokenFee);
        }
    }
    ...
    // ------------------------------------------------------------------------
    function transfer(address recipient, uint256 amount) public payToken override returns (bool) {
