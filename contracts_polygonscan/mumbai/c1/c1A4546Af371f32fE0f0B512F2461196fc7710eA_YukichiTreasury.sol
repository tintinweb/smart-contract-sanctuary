...
interface IYukichi is IERC20Mintable {
  function mint( address account_, uint256 ammount_ ) external;
}
...
contract YukichiTreasury is Ownable {
    ...
    address public immutable Yukichi;
    ...
    constructor (
        ...
        // uint32 _secondsNeededForQueue = 0;

        require( _yukichi != address(0) );
        Yukichi = _yukichi;
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        IYukichi( Yukichi ).mint( msg.sender, send_ );
    ...
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IOHMERC20( Yukichi ).burnFrom( msg.sender, value );
    ...
    function repayDebtWithOHM( uint _amount ) external {
        ...
        IOHMERC20( Yukichi ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, Yukichi, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        // IERC20Mintable( Yukichi ).mint( _recipient, _amount );
        IYukichi( Yukichi ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( Yukichi ).totalSupply().sub( totalDebt ) );
    }
    ...
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            ...
            value_ = _amount.mul( 10 ** IERC20( Yukichi ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
