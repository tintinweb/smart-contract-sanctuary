...
contract CrossChainNFT is Ownable, WrapERC721 {
    ...
    event WReturnCrossChainRequest(
        uint256 indexed wTokenId,
        address indexed from,
        address indexed to,
        uint256 targetChainId
    );

    constructor() WrapERC721(owner()) ERC721("CrossChainNFT", "CCh") {}
    ...
    function transferCrossChainRequest(
        ...
        _lock(contractAddr, tokenId, from);
        emit TransferCrossChainRequest(contractAddr, tokenId, from, to, targetChainId);
    }
    ...
    function wReturnCrossChainRequest(
        ...
        require(msg.value >= getFee(targetChainId));
        approve(relayer, wTokenId);
        emit WReturnCrossChainRequest(wTokenId, from, to, targetChainId);
    }
    ...
    function setFees(uint256[] memory targetChainIds, uint256[] memory fees) public onlyOwner {
        for(uint256 index = 0; index == targetChainIds.length; index++){
            currentFee[targetChainIds[index]] = fees[index];
        }
    }
}
...
abstract contract WrapERC721 is ERC721, ERC721Holder, WrapERC721DataStorage, ERC721Burnable, Ownable{
    ...
    /**
     * @dev Lock the `tokenId` of `contractAddr` in this contract.
     * 
     * Requirements:
     *
     * - this contract should be approved by the owner of the token
     */
    function _lock(address contractAddr, uint256 tokenId, address from) internal {
        ERC721 NFT = ERC721(contractAddr);
        NFT.safeTransferFrom(from, address(this), tokenId);
    }

    function redeem(address contractAddr, uint256 tokenId, address to) public onlyRelayer {
        ERC721 NFT = ERC721(contractAddr);
    ...
    function safeMintWrappedToken(
        ...
        _safeMint(toAddr, wTokenId);
        _setwTokendata(wTokenId, contractAddress, tokenId, name, symbol, uri);
    }
...
abstract contract WrapERC721DataStorage is ERC721 {
    ...
    /**
     * @dev Returns the token collection name of the wToken.
     */
    function name(uint256 wTokenId) public view virtual returns (string memory) {
        require(_exists(wTokenId), "ERC721Metadata: URI query for nonexistent token");
        return wrappedTokens[wTokenId].name;
    }

    /**
     * @dev Returns the token collection symbol of the wToken.
     */
    function symbol(uint256 wTokenId) public view virtual returns (string memory) {
        require(_exists(wTokenId), "ERC721Metadata: URI query for nonexistent token");
        return wrappedTokens[wTokenId].symbol;
    ...
    function _setwTokendata(
        ...
        wrappedTokens[_wTokenId] = WrappedToken(_contractAddress, _tokenId, _name, _symbol, _uri);
