...
contract RhinoTest is ERC721Enumerable, Ownable {
    ...
    function teamMint200() external {
      require(msg.sender == recipient, "This is a function only dedicated to the project owner");
      require(!teamMintComplete, "This function can only be run once, and it has already been run");
      adjustMaxMintAmount(teamMintTotal);
      /* Complete Dev Mint Automatically - to stated amount */
      for (uint i = 0; i < teamMintTotal; i++) {
        uint currentTotal = totalSupply();
        _mint(recipient, currentTotal);
        emit Mint(recipient, currentTotal);
      }
      teamMintComplete = true;
      adjustMaxMintAmount(maxItemsPerPublicUser);
    }
...
/* require(finalTimestamp >= block.timestamp, "The mint has already closed");
    ...
    function adjustMaxMintAmount(uint _maxMintAmount) internal onlyOwner {
