...
contract GZRewardsUpgradeable is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function claimBySignature(bool _extra, bytes memory _signature) external nonReentrant() {
        address account = _msgSender();
        require(claimedCount < limit, "Reached to limit");
        require(claimed[account] == false, "Already claimed");
        require(_isValidSignature(account, _extra, _signature), "Not allowed");
        ...
        if (0 < zoneAmount) {
            zoneToken.safeTransfer(account, zoneAmount);
        }

        emit Claimed(account, reward, zoneAmount);
    }
