...
contract Loot is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function getRing(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "RING", necklaces);
    }
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Bag #', toString(tokenId), '", "description": "We are building on top of the Loot project and expanding the Lootaverse.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
