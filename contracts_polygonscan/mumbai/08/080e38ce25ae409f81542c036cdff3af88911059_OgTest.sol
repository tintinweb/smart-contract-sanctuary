...
contract OgTest is SysCtrl, OgProfile, IERC721 {
    ...
    function birthAxie(address _to, uint256 _dna, uint256 _bornAt, uint256 _parents) external onlyMint returns (uint256) {
    ...
    function getAxie(uint256 _axieId) external view validNFToken(_axieId) returns(address,uint256,uint256,uint256,uint256,bytes32,uint256) {
         AxieStruct memory _axie = axies[_axieId];
         Offer memory offer = axieForSale[_axieId];

         return(idToOwner[_axieId],_axie.dna,_axie.dataGame,_axie.bornAt,_axie.parents,_axie.name,offer.minValue);
    }
