...
contract Hanoux is Context {
    ...
    function splitePayment(address contractUSDC, uint256 amount) public virtual {
        // проверка на разрешение перевода нужной суммы
        require(amount != 0, "Hanoux: amount empty");
        ...
        for (uint256 index = 0; index < _payees.length; index++) {
            ...
            uint256 payment = amount * _shares[account] / 100;
            
            //  сохранение выплаты и общей суммы выплаты
            _released[account] = _released[account] + payment;
            

            //отправка USDC участникам HANOUX (предварительно заказчику нужно делать approve всей суммы)
            IERC20(contractUSDC).transferFrom(
                _msgSender(),
                account,
                payment
            );
