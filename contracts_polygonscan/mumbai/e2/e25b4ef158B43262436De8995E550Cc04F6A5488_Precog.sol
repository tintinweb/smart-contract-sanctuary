...
contract Precog {
    ...
    event Withdraw(address indexed user, uint amount, address indexed to, uint fee, uint indexed timestamp);
    event Trade(
    ...
    modifier onlyTradingService() {
        require(msg.sender == tradingService, "PrecogV2: NOT_TRADING_SERVICE_ADDRESS");
        _;
    }
    ...
    function setIsStaking(bool _isStaked) external {
        if(_isStaked){
            require(tradeUserIndex[msg.sender] != 0, "PrecogV2: NOT_IS_TRADE_USER");
        }
    ...
    function withdraw(uint256 amount, address to) external {
        ...
        _removeTradingUser(msg.sender);
         emit Withdraw(msg.sender, amount, to, feeWithdrawByUSDC, block.timestamp);
    }
    ...
    function withdrawLiquidate(address futureContract, address to, address user) external {
        ...
        if(isStaked[user] == true){
            ...
            require(requireLocktime != 0, "PrecogV2: no start staking");
            require(block.timestamp >= requireLocktime, "PrecogV2: can't withdraw in locktime");
        ...
        emit Withdraw(user, amount, to, feeWithdrawByUSDC, block.timestamp);
    }
    ...
    function reinvest(address futureContract, address user) external {
        ...
        if(isStaked[user] == true){
            uint requireLocktime = IStakingPool(stakingPool).getRequireLockTime(user);
            require(block.timestamp >= requireLocktime, "Precog: can't withdraw in locktime");
            uint returnStakingAmount = IStakingPool(stakingPool).withdraw(user);
            isStaked[user] == false;
    ...
    function liquidate(address futureContract, address user) public {
        ...
        airdropAmount +=  actualProfit * airdropPercent / 100; 
        liquidatedAmount[user][futureContract] = revenue - debt - (actualProfit * airdropPercent / 100) ;
        actualProfit -=  actualProfit * airdropPercent / 100;
        ...
        if (isStaked[user] == true){
            if (IStakingPool(stakingPool).getEndTime() > expiryDate){
                ...
                require(IERC20(usdc).balanceOf(address(this)) >= amountStaking, "PrecogV2: insufficient balance");
    ...
    function claimAirdrop() external {
        require(airdropReceiverCount > 0, "Precog: CANNOT_RECEIVE_AIRDROP");
        require(isAirdropReceiver[msg.sender] == true, "Precog: USER_CANNOT_RECEIVE_AIRDROP");
