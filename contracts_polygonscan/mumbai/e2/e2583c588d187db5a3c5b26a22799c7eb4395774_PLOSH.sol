...
abstract contract Ownable is Context {
     ...
     */
    function renounceOwnership(bool _renounceOwnership, bool _areYouSure)
        public
        virtual
        onlyOwner
    {
        if (_renounceOwnership == true && _areYouSure == true) {
...
// Thanks to the 1001.digital for sharing this amazing contract utils
// All of this contracts are MIT licence sharing.
...
abstract contract WithLimitedSupply {
    ...
    modifier ensureAvailability() {
        require(availableTokenCount() > 0, "No more NFTs available");
        _;
    }
    ...
    modifier ensureAvailabilityFor(uint256 amount) {
        require(
            ...
            "Requested number of NFTs not available"
        );
        _;
    }
}
...
// SPSASFC - is a limited collection of 5555 Space Pirates of - ERC721

/*
Note:
This contract resides on the Polygon MATIC Network due to high gas 
prices for the Ethereum mainnet at the time of the contract launch
(not fair to buyers)
*/

// 150 NFTs retained for artist, developer, giveaway and contest prizes

// SPSASFC is a project developed by: Dr.willy


contract PLOSH is ERC721, Ownable, RandomlyAssigned {
    using Strings for uint256;
    ...
    uint256 public nftCost = 50 ether;
    uint256 public maxSupply = 25;
    uint256 public maxMintAmount = 3;
    uint256 public maxNftPerAddress = 5;
    bool private contractRevealed = false;
    ...
    function mint(uint256 _mintAmount) public payable {
        ...
        require(
            ...
            "You can't mint more than maximun supply."
        );
        require(
            availableTokenCount() - _mintAmount >= 0,
            "You can't mint more than available token count"
        );
        require(
            tx.origin == msg.sender,
            "You can't mint through a custom contract."
        );
        ...
        if (msg.sender != owner()) {
            ...
            require(
                msg.value >= nftCost * _mintAmount,
                "You must enter the correct Cost."
            );
            require(
                _mintAmount <= maxMintAmount,
                "You're trying to mint more than allowed."
            );

            //require(balanceOf(msg.sender) < _mintAmount, "You already have the max NFT holding allowance.");
            require(
                balanceOf(msg.sender) + _mintAmount <= maxNftPerAddress,
                "You already have the max NFT holding allowance."
            );
        }
    ...
    function setMaxNftPerAddress(uint256 _newLimit) public onlyOwner {
