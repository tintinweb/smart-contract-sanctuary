...
contract NFTMarketplace is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable, UUPSUpgradeable {
    ...
    address payable internal theosProtocolAddress;
    ...
    event ItemListed(address indexed owner, address indexed tokenAddress, uint tokenId, uint price);
    event ItemUnlisted(address indexed owner, address indexed tokenAddress, uint tokenId);
    event ItemSold(address indexed newOwner, address indexed tokenAddress, uint tokenId, uint price);
    ...
    function buyItem(address tokenAddress, uint tokenId) public payable nonReentrant {
        ...
        emit ItemSold(msg.sender, tokenAddress, tokenId, item.price);
    }
    ...
    function _validateListing(Item memory item) internal view {
        ...
        Item memory existingOrder = orders[_generateUniqueOrderHash(item.tokenAddress, item.tokenId)];
        ...
        if (item.owner != tokenAddress.ownerOf(item.tokenId)) 
            revert InvalidOrder();

        if ( ! tokenAddress.isApprovedForAll(msg.sender, address(this)))
