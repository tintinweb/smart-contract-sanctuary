...
contract Shop is ERC1155, Ownable {
    ...
    function buyItems(address _userAddress, uint256 _itemId, uint256 _itemAmount,uint256 _nakaAmount) external returns(uint256){
        uint256 totalItem;
        
        vaultItem.increaseItem(_userAddress, _itemId, _itemAmount);
        vaultBalance.decreaseBalance(_userAddress, _nakaAmount);

        totalItem = vaultItem.getItemAmountbyId(_userAddress, _itemId);
