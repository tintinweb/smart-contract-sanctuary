...
import "../opensea-compatibility/ContextMixin.sol";
...
contract CardToken is
    ...
    // TODO: remove OpenSea compatibility from here
    ContextMixin
{
    //
    // --- Enums
    ...
    function _onlyValidTokenId(bytes32 tokenId) internal view returns (bool) {
        ...
        require(0 < tokenIdAsNumber && tokenIdAsNumber <= tokenSupplyCap());
    }
        ...
        LSP8CappedSupply(tokenSupplyCap)
    {
        ...
        // require(creators.length > 0);
        // require(creators.length == creatorRevenueShares.length);

        uint256 revenueShareSum;
        for (uint256 i = 0; i < creatorRevenueShares.length; i++) {
            revenueShareSum += creatorRevenueShares[i];
        }
        // require(revenueShareSum == 100);

        // require(scoreMin <= scoreMax);
        _scoreMin = scoreMin;
        _scoreMax = scoreMax;

        // this value should be a power of 10, but a sanity check for zero is enough
        // require(scoreScale != 0);
        _scoreScale = scoreScale;

        // require(scoreMaxTokenId != 0);
        // require(scoreMaxTokenId <= tokenSupplyCap);
    ...
    /**
     * Dirty testing for OS compatibility
     */
