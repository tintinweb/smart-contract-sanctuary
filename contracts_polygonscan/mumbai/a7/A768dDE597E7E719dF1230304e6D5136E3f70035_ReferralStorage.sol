...
contract ReferralStorage is OwnableUpgradeable {
    ...
    event RewardClaimed(address indexed claimer);

    // CONSTRUCTOR

    function initialize(uint256 rewardInvitees_, Reward[] memory rewards_) external initializer {
        __Ownable_init();

        rewardInvitees = rewardInvitees_;
    ...
    function claimReward() external {
        require(invitees[msg.sender].length >= rewardInvitees, "Not enough invitees for reward");
        require(!rewardClaimed[msg.sender], "Already claimed");

        rewardClaimed[msg.sender] = true;
        for (uint256 i = 0; i < rewards.length; i++) {
            if (rewards[i].tokenType == TokenType.ERC20) {
                IMintableToken(rewards[i].token).mint(msg.sender, rewards[i].amount);
            } else {
                ITypedNFT(rewards[i].token).mint(msg.sender, rewards[i].typeId, rewards[i].amount);
            }
        }
    ...
    function setRewardInvitees(uint256 rewardInvitees_) external onlyOwner {
        rewardInvitees = rewardInvitees_;
    }

    function setRewards(Reward[] memory rewards_) external onlyOwner {
        for (uint256 i = 0; i < rewards_.length; i++) {
            if (i < rewards.length) {
