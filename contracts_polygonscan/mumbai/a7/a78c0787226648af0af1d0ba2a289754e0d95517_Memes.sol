...
contract Memes is ERC1155, Ownable, Royalties {
    ...
    function sendRoyaltyPolygon(
        address payable seller,
        uint256 totalAmount,
        uint256 id
    ) public payable returns (bool approved) {
        address payable creator = creatorList[id];
        console.log("creator ",creator);
        //get percent Royalty of creator
        uint256 percentRoyalty = percentRoyaltyList[id][creator];
        //calculate royalty of creator
        uint256 royaltyCreator = percentRoyalty.mul(totalAmount.div(100));
        uint256 amountSeller = totalAmount.sub(royaltyCreator);
        if (creator == ownerAddress) return true;
        (bool successTramsferToCreator, ) = creator.call{value: royaltyCreator}(
            ""
        );
        require(
            successTramsferToCreator,
        ...
        this.royaltiesRecieved(creator, seller, royaltyCreator);
        return true;
    }
}
