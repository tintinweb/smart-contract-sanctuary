...
// @name AAVE Interest Yielding Simple Gift Box
...
contract aiySimpleGiftBox is SimpleGiftBox {
  ...
  mapping(address => uint256) public depositedTokens; //interest yielding token's underlying deposit amounts
  uint256 public aWETH; //aWETH received for depositing ETH into AAVE
  ...
  // @name Receive
  // @notice Used internally to invest received native tokens with AAVE
  function _receive(uint256 _amount) internal {
    wethGateway.depositETH{value: _amount}(pool, address(this), uint16(0)); //TODO: get referral code for recipient
    //Track deposited ETH
    aWETH += _amount;
  }
...
interface IGiftRegistry {
  ...
  function createGiftBox(address _recipient) external;

  function createGiftBoxWithTargetDate(address _recipient, uint256 _targetDate) external;

  function createGiftBoxWithTargetBalance(address _recipient, uint256 _targetBalance) external;

  function createGiftBoxWithConservatorTargetBalance(address _recipient, uint256 _targetBalance, address _conservator) external;

  function createGiftBoxWithTargetTokenBalance(address _recipient, address _targetToken, uint256 _targetTokenBalance) external;

  function createInterestYieldingGiftBoxAAVE(address _recipient) external;
}
...
contract ComplexGiftBox is ERC721Holder, IERC777Recipient, IERC1155Receiver, ERC165 {
