...
/**
  Created with https://coinconket.com
*/
...
contract CoinConKet_Dot_Com is Context, IBEP20, Ownable {
    ...
    constructor (string memory _NAME, string memory _SYMBOL, address owner) Ownable(owner) {
        _name = _NAME;
        _symbol = _SYMBOL;
        _decimals = 18;
        _tTotal = 100000000019 * 10 ** _decimals;
        ...
        _devWalletAddress = _msgSender();



        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0xe9ABb0E506C144Cb9790026D64768C6ba487FF5f);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 newBalance = address(this).balance.sub(initialBalance);
         // take dev fee
        uint256 devFee          = newBalance.mul(_devFee).div(100);
        uint256 bnbForLiquidity = newBalance.sub(devFee);
        if (devFee > 0) {
            payable(_devWalletAddress).transfer(devFee);
            emit DevFeeSent(_devWalletAddress, devFee);
        }
