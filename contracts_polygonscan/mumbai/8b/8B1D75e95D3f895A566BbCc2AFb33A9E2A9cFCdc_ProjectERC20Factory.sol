...
struct VintageData {
    uint16 vintage; // This is the year, e.g. 2021
...
// The CarbonProjectVintages represents offsetting vintages as ERC721 tokens.
// This makes it possible to transfer ownership of vintages via transfers.  The
// vintages also serve as a data source for the BatchNFTs and project-vintage
// ERC20s.
    ...
    UUPSUpgradeable
{
    ...
    uint256 public projectVintageTokenCounter;
    ...
    function initialize() public initializer {
        ...
        __ERC721_init_unchained(
            'CO2KEN Project Vintage Collection',
            'CO2KEN-PVNFT'
        );
        __Ownable_init_unchained();
        __Pausable_init_unchained();
    ...
    function addNewVintage(
        ...
        require(projectTokenId != 0, 'projectTokenId cannot be 0');
        address CPaddress = IContractRegistry(contractRegistry)
            .carbonProjectsAddress();

        /// @dev check if corresponding CarbonProject exists
        bool isValidProjectTokenId = CarbonProjects(CPaddress)
            .getValidProjectTokenIds(projectTokenId);
        ...
        vintageData[newItemId].vintage = vintage;
        ...
        emit ProjectVintageMinted(to, newItemId, projectTokenId, vintage);
        pvToTokenId[projectTokenId][vintage] = newItemId;
        return newItemId;
    }
    ...
    function updateProjectVintage(
        ...
        vintageData[tokenId].vintage = vintage;
    ...
    function getVintageId(uint256 tokenId)
        public
        view
        virtual
        returns (uint16)
    {
        return vintageData[tokenId].vintage;
    }
    ...
    // FIXME: do we need baseURI for vintages?
...
contract CarbonProjectVintagesStorage {
    ...
    /// @dev Maps: projectTokenId => vintage year => projectVintageTokenId
    ///
    /// This is the rough reverse of VintageData.projectTokenId, i.e. it's the
    /// way that a caller with a projectTokenId and a vintage year can obtain
    /// the corresponding projectVintageTokenId.  This is particularly important
    /// during the batch NFT approval phase, since prior to confirmation, there
    /// is no direct association between the batch and the project/vintage; only
    /// a long serial number containing info which allows that association.
    mapping(uint256 => mapping(uint16 => uint256)) public pvToTokenId;
}
    ...
    UUPSUpgradeable
{
    ...
    uint256 public projectTokenCounter;
    ...
    function addNewProject(
        ...
        projectData[newItemId].metaDataHash = metaDataHash;
    ...
    function updateProject(
        ...
        string memory oldHash = projectData[tokenId].metaDataHash;

        projectData[tokenId].metaDataHash = newMetaDataHash;
    ...
    function getValidProjectTokenIds(uint256 projectTokenId)
        public
        view
        returns (bool)
    {
        ...
        )
    {
        ...
        return (
            projectData[tokenId].standard,
            projectData[tokenId].methodology,
            projectData[tokenId].region,
            methodologyToStorageMethod[projectData[tokenId].methodology]
        );
    }
        ...
        returns (string memory)
    {
        ...
        string memory metaDataHash = projectData[tokenId].metaDataHash;
        string memory base = _baseURI();
...
// import './ICarbonProjectVintages.sol';
import './CarbonProjectVintageTypes.sol';
import './ProjectERC20Storage.sol';
import './ProjectERC20Factory.sol';

contract ProjectERC20 is
    ERC20Upgradeable,
    IERC721Receiver,
    ProjectERC20Storage
{
    modifier whenNotPaused() {
        address ProjectERC20FactoryAddress = IContractRegistry(contractRegistry)
            .projectERC20FactoryAddress();
        bool _paused = ProjectERC20Factory(ProjectERC20FactoryAddress).paused();
    ...
    function name() public view virtual override returns (string memory) {
        ...
        uint16 vintage;
        (globalProjectId, vintage) = getGlobalProjectIdentifier();
        string memory _vintage = Strings.toString(vintage);
        return
            string(
                abi.encodePacked('Toucan TCO2-', globalProjectId, '-', _vintage)
            );
    }
    ...
    function symbol() public view virtual override returns (string memory) {
        ...
        string memory _vintage = Strings.toString(vintage);
        return
            string(abi.encodePacked('TCO2-', globalProjectId, '-', _vintage));
    }

    function getGlobalProjectIdentifier()
        public
        view
        returns (string memory, uint16)
    {
        ...
        return (projectData.projectId, vintageData.vintage);
    }
    ...
    {
        address pc = IContractRegistry(contractRegistry)
            .carbonProjectsAddress();
        address vc = IContractRegistry(contractRegistry)
...
import './IBatchCollection.sol';
import './CarbonProjects.sol';
import './ProjectERC20FactoryStorage.sol';
...
contract ProjectERC20Factory is
    ProjectERC20FactoryStorage,
    OwnableUpgradeable,
    PausableUpgradeable,
    UUPSUpgradeable,
    ProjectUtils
{
    ...
    function initialize(address _contractRegistry) public initializer {
        __Context_init_unchained();
        __Ownable_init_unchained();
        __Pausable_init_unchained();
        contractRegistry = _contractRegistry;
    ...
    function deployNewProxy(
        ...
        bytes memory payload = abi.encodeWithSignature(
            signature,
            'TCO2 automatic name',
            'TCO2 automatic symbol',
            projectVintageTokenId,
            _contractRegistry
        );

        /// @dev deploy new ProxyTCO2 contract
        BeaconProxy TCO2 = new BeaconProxy(beacon, payload);

        /// @dev alternative flow initializing separately via low-level `call`
        /// @TODO remove after review
        // (bool success, bytes memory returnData) = address(TCO2).call(payload);
        // require(success);

        // Register deployed pERC20 in ContractRegistry
        IContractRegistry(contractRegistry).addERC20(address(TCO2));

        deployedContracts.push(address(TCO2));
        pvIdtoERC20[projectVintageTokenId] = address(TCO2);

        emit TokenCreated(address(TCO2));
    }

    // Deploy an ERC20 token based on a project vintage
    function deployFromVintage(uint256 projectVintageTokenId)
        public
        virtual
        whenNotPaused
    {
        deployNewProxy(projectVintageTokenId, contractRegistry);
    }

    // Checks if same project vintage has already been deployed
...
/// @dev Storage contract for ProjectERC20Factory (UUPS proxy upgradable)
contract ProjectERC20FactoryStorage {
    address public contractRegistry;
...
contract ProjectUtils {
    function checkProjectTokenExists(address contractRegistry, uint256 tokenId)
        ...
        virtual
    {
        address c = IContractRegistry(contractRegistry).carbonProjectsAddress();
        require(
            ICarbonProjects(c).ownerOf(tokenId) != address(0),
...
contract ProjectVintageUtils {
    function checkProjectVintageTokenExists(
        ...
        address c = IContractRegistry(contractRegistry)
            .carbonProjectVintagesAddress();
        require(
            ICarbonProjectVintages(c).ownerOf(tokenId) != address(0),
            'Project does not yet exist'
        );
    }
}
