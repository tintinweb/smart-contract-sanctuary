...
//		newOrder.retailFee = _retailFee;
	...
	function cancelOrder(uint256 _orderId) external whenNotPaused() {
		...
		for (uint i = order.fromVersion; i <= order.toVersion; i++) {
			if (nftVersion[order.tokenAddress][order.tokenId][i] == msg.sender) {
				nftVersionOnSale[order.tokenAddress][order.tokenId][i] = false;
				delete orderIdByVersion[order.tokenAddress][order.tokenId][i];
			}
		}
