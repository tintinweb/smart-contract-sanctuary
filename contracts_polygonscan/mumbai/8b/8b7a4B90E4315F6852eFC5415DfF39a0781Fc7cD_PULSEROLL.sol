...
contract PULSEROLL is VRFConsumerBase {
    ...
    modifier betIsValid(uint256 _betSize, uint256 _playerNumber) {
        require ((((((_betSize * (100 - (_playerNumber - 1)) /(_playerNumber - 1) + _betSize)) * houseEdge) / houseEdgeDivisor) - _betSize <= IHousePool(StakingContract).maxProfit() && _betSize >= IHousePool(StakingContract).minBet() && _playerNumber >= minNumber &&  _playerNumber <= maxNumber && _betSize<=IHousePool(StakingContract).maxBet() ) );
        _;
    }
