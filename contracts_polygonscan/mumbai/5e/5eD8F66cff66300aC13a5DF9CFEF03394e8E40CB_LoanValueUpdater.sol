...
contract LoanValueUpdater is LoanKeepersJob {
  ...
  function _shouldUpdateLoan(uint256 _loanID) internal view override returns (bool) {
    (, , , , , uint256 lastUpdatedAt) = (ILoanNFT(loanNFTAddress)).getLoanValue(_loanID);
    return block.timestamp >= lastUpdatedAt + 1 days;
  }

  // Capped at 10 loans per execution
  function performUpkeep(bytes calldata performData) external override {
    uint256[] memory loanIDs = abi.decode(performData, (uint256[]));
    (ILoanNFT(loanNFTAddress)).updateMultipleLoanValues(loanIDs);
  }
}