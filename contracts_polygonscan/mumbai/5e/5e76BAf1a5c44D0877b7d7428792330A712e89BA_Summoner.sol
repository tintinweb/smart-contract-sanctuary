    ...
    ReentrancyGuardUpgradeable
{
    ...
    function transferERC721ToLayer(
        ...
        require(
            config.getPausedNetwork(destinationNetworkID) == false,
            "NETWORK PAUSED"
        );
        require(
        ...
        require(
            config.getFeeTokenAmount(destinationNetworkID, feeToken) > 0,
            "FEE TOKEN"
        );

        IERC721Upgradeable(sourceNFTAddress).safeTransferFrom(
            msg.sender,
            address(this),
            sourceNFTId
        );
        ...
        // string memory sourceUri = IERC721MetadataUpgradeable(sourceNFTAddress)
        //     .tokenURI(sourceNFTId);

        TransferTemporaryData memory tempData;
        tempData.id = _generateId(_chainId());
        ...
        if (sourceNFTAddress == mosaicNftAddress) {
            ...
            ) = IMosaicNFT(mosaicNftAddress).getOriginalNftInfo(sourceNFTId);
        } else {
            tempData.originalNftAddress = sourceNFTAddress;
            tempData.originalNetworkId = _chainId();
            tempData.originalNftId = sourceNFTId;
        }

        if (
            destinationNetworkID == tempData.originalNetworkId &&
            mosaicNftAddress == sourceNFTAddress
        ) {
    ...
    function _takeFees(
        ...
        if (feeToken != address(0)) {
            require(
                IERC20Upgradeable(feeToken).balanceOf(msg.sender) >= fee,
                "LOW BAL"
            );
    ...
    function releaseSeal(
        ...
        require(
            IERC721Upgradeable(nftContract).ownerOf(nftId) == address(this),
            "NOT LOCKED"
        );
    ...
    function withdrawFees(
        ...
        if (feeToken != address(0)) {
            require(
                IERC20Upgradeable(feeToken).balanceOf(address(this)) >= amount,
                "LOW BAL"
            );
            SafeERC20Upgradeable.safeTransfer(
                IERC20Upgradeable(feeToken),
    ...
    function _generateId(uint256 chainId) private returns (bytes32) {
        return
            keccak256(
                abi.encodePacked(block.number, chainId, address(this), nonce++)
            );
    }
