...
contract RevenueShare {
 ...
 function addAgent(string memory agentId, address _agentAddress, string memory artistId, uint256 _percentage, uint256 _artistPercentage) adminAdd public {
  ...
  function transferAliaArtist(address buyer, uint256 price, address nftVaultAddress, uint256 tokenId, address platform, uint256 platformPerecentage ) private returns(bool) {
  ...
  function transferAlia(uint256 price, uint256 tokenId, uint256[] calldata percentages,  address[] calldata addresses, string calldata ownerId ) external returns(bool) {
    require(msg.sender == xanaliaDex, "not nft address");
    // string storage artistId = _nonCryptoAuthor[tokenId];
    // address addresses[4] = IERC20(xanaliaDex).nonCryptoNFTVault();
    // address addresses[3] = IERC20(xanaliaDex).addresses[3]();
    ...
    if(isArtistAuthor){
      if(checkArtistOwner(ownerId, tokenId)){
        ...
        ALIA.transferFrom(addresses[0], addresses[1], calculatePercentage(price, percentages[0]));
          //  ALIA.transferFrom(msg.sender, addresses[3], (_artistPrice * platformPerecentage) / 1000);
      }
      transferAliaArtist(addresses[0], price, addresses[4], tokenId, addresses[3], percentages[1] );
    }else{
      ALIA.transferFrom(addresses[0], addresses[1], calculatePercentage(price, percentages[0]));
