...
contract ERC721GlipLive is ERC721Lazy {
    ...
    function __ERC721GlipLive_init(string memory _name, string memory _symbol, bool _global, string memory baseURI, string memory contractURI, address exchangeProxy, address defaultMinter, uint96 fee) public initializer {
        __ERC721GlipLive_init_unchained(_name, _symbol, _global, baseURI, contractURI, exchangeProxy, defaultMinter, fee); // transferProxy, lazyTransferProxy
        emit CreateERC721GlipLive(_msgSender(), _name, _symbol);
    }

    // address transferProxy, address lazyTransferProxy
    function __ERC721GlipLive_init_unchained(string memory _name, string memory _symbol, bool _global, string memory baseURI, string memory contractURI, address exchangeProxy, address defaultMinter, uint96 fee) internal {
        _setBaseURI(baseURI);
        __ERC721Lazy_init_unchained(_global, defaultMinter, fee);
...
import "./../IGlipAuction.sol";
import "./../LibMinterFee.sol";
    ...
    RoyaltiesV2Impl
{
    ...
    mapping(address => mapping(address => MinterFee)) private _minterFee;

    struct DefaultMinter {
        address minter;
        uint96 fee;
    }
    DefaultMinter public defaultMinter;

    // Minter needs to get the form signed by the creator and submits to the contract
    function upsertMinter(LibMinterFee.MinterFee memory data) public {

        // If not global contract then handle
        require(
            (global) || data.creator == owner(),
            "Wrong minter"
        );

        bytes32 hash = LibMinterFee.hash(data);
        ...
        _minterFee[data.creator][data.minter] = MinterFee(
            data.fee,
            data.cancelValue,
            data.start,
            data.end
    ...
    function cancelMinter(address minter) public payable {
        require(
            msg.value == _minterFee[_msgSender()][minter].cancelValue,
            "Cancel charge"
        );
        delete _minterFee[_msgSender()][minter];
    }

    function getMinter(address creator, address minter)
        public
        view
        virtual
        returns (MinterFee memory)
    {
        return _minterFee[creator][minter];
    }

    // Upsert default minter can be done by default owner
    function upsertDefaultMinter(address minter, uint96 fee) public {
        require(_isDefaultApproved(_msgSender()), "Not default appr.");
        defaultMinter = DefaultMinter(minter, fee);
    }

    // function getDefaultMinter() public view virtual returns (DefaultMinter memory) {
    //     return defaultMinter;
    // }

    function getMinterFee(address creator, address minter)
        internal
        view
        virtual
        returns (bool, uint96)
    {
        ...
        if (defaultMinter.minter == minter) return (true, defaultMinter.fee);

        // Allowing self signed
        if (creator == minter) return (true, 0);

        require(
            block.timestamp >= _minterFee[creator][minter].start,
            "Minter start"
        );
        require(
            block.timestamp <= _minterFee[creator][minter].end,
            "Minter ended"
        );
        return (true, _minterFee[creator][minter].fee);
    }

    function __ERC721Lazy_init_unchained(
        bool _global,
        address minter,
        uint96 fee
        ...
        )
    {
        ...
        require(verifyAsset(data), "Invalid data");
        (, uint96 fee) = getMinterFee(data.creators[0].account, data.minter);

        return (
            data.tokenId,
            data.reserve,
            data.creators[0].account,
            data.creators,
            LibPart.Part(
                payable(data.minter),
                fee
    ...
    function verifyAsset(LibERC721LazyMint.Mint721Data memory data)
        internal
        view
        returns (bool)
    {
        ...
        require(
            !(global) || creator == data.creators[0].account,
            "tokenId wrong"
        );

        // If creator specific contract then check if it is being minted to the owner
        require(
            global || (data.creators[0].account == owner()),
            "Mint to owner"
        );
    ...
    function mintAndTransfer(
        ...
        require(
            _isDefaultApproved(_msgSender()) ||
                _msgSender() == data.creators[0].account,
            "Not approved"
        );

        // Verify legality of (Mint data + Message signer + Message sender + Contract owner + Approved) combination
        require(verifyAsset(data), "Invalid asset");

        // Verify if creator has allowed the minter and the fee
        
        getMinterFee(data.creators[0].account, data.minter);
    ...
    // --------------- PUBLIC GETTERS (Creators, Exists, Burned) -------------------- //
...
abstract contract ERC721Base is OwnableUpgradeable, ERC721DefaultApproval, ERC721BurnableUpgradeable, HasContractURI {
     ...
     */
    function transferOwnership(address newOwner) public virtual override onlyOwnerOrDefaultApproved {
...
interface IGlipAuction {

    function auctioneer() external virtual view returns (address);
...
library LibMinterFee {

    bytes32 constant MINTER_FEE_TYPEHASH = keccak256(
        "MinterFee(address minter,address creator,address token,uint96 fee,uint256 cancelValue,uint256 start,uint256 end)"
    );
    ...
    function hash(MinterFee memory minterFee) internal pure returns (bytes32) {
...
library LibERC721LazyMint {
    ...
    struct Mint721Data {
        ...
        address minter;
        uint256 reserve;
        bytes[] signatures; // TODO change
    }

    bytes32 public constant MINT_AND_TRANSFER_TYPEHASH =
        keccak256("Mint721(uint256 tokenId,string tokenURI,Part[] creators,Part[] royalties,address minter,uint256 reserve)Part(address account,uint96 value)");
