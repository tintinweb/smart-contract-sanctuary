...
contract YPoolVault is AccessControlUpgradeable, UUPSUpgradeable, PausableUpgradeable {
    ...
    bytes32 public constant ROLE_CLAIM_WORKER = keccak256("ROLE_CLAIM_WORKER");
    ...
    event WithdrawalFulfilled(address indexed recipient, uint256 indexed withdrawID, uint256 amountDepositToken);
    ...
    function initialize(address owner, address manager, address staff, address liquidityWorker, address claimWorker, address _depositToken, address _xyWrappedToken, uint8 _depositTokenDecimal) initializer public {
        ...
        _setRoleAdmin(ROLE_CLAIM_WORKER, ROLE_OWNER);
        ...
        _setupRole(ROLE_CLAIM_WORKER, claimWorker);
    }
    ...
    function _collectCloseSwapGasFees(address receiver) private {
        ...
        if (address(depositToken) == ETHER_ADDRESS) {
            ...
            depositToken.transfer(receiver, _closeSwapGasFees);
        }
    ...
    function completeWithdraw(uint256 _withdrawID, uint256 amount) external whenNotPaused onlyRole(ROLE_LIQUIDITY_WORKER) {
        ...
        // yield rate = (amount ypool token) / (amount wrapped token)
        require(amount * 10 ** (YIELD_RATE_DECIMALS + XY_TOKEN_DECIMALS - depositTokenDecimal) / request.amountXYWrappedToken <= maxYieldRateBound, "ERR_YIELD_RATE_OUT_OF_MAX_BOUND");
        require(amount * 10 ** (YIELD_RATE_DECIMALS + XY_TOKEN_DECIMALS - depositTokenDecimal) / request.amountXYWrappedToken >= minYieldRateBound, "ERR_YIELD_RATE_OUT_OF_MIN_BOUND");
        emit WithdrawalFulfilled(request.sender, _withdrawID, amount);
        request.isComplete = true;
