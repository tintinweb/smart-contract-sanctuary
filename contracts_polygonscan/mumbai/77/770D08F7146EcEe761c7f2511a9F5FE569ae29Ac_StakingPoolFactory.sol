...
contract StakingPoolFactory is Ownable {
    ...
    function launchStakingPool(
        ...
        StakingPool newPool = new StakingPool(_feeBeneficiary);
        _localVars.fetchedFees = 0;
        _localVars.feeTokenAddress = address(0);

        if (isFeeManagerEnabled) {
            (_localVars.feeAmount, _localVars.feeToken) = feeManager
                .getFactoryFeeInfo(address(this));
            require(
                _amount > _localVars.feeAmount,
                "Amount should be more than the feeAmount"
            );
            if (_localVars.feeToken != address(0)) {
                TransferHelper.safeTransferFrom(
                    _localVars.feeToken,
                    msg.sender,
                    address(this),
                    _localVars.feeAmount
                );
                ...
                _localVars.fetchedFees = feeManager.fetchFees();
            } else {
                require(msg.value == _localVars.feeAmount, "Invalid value sent for fee");
                _localVars.fetchedFees = feeManager.fetchFees{value: msg.value}();
            }

            _localVars.feeTokenAddress = _localVars.feeToken;
...
contract StakingPool is Ownable, ReentrancyGuard {
    ...
    function transferRewardToken(uint256 _rewardTokenIndex, uint256 _amount, address poolOwner)
        external
        onlyOwner
    {
