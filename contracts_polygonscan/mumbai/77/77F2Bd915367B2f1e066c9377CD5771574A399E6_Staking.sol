...
contract Staking is ReentrancyGuard, Context, IStaking {
    ...
    uint256 public treasuryYdrPenalty;
    ...
    function _inputBnb(
        ...
        if (token != ydrToken) {
            calculatedAmountToTreasury = (amount * treasury) / 1e4;
        } else {
            calculatedAmountToTreasury = (amount * treasuryYdrPenalty) / 1e4;
        }
    ...
    function _withdrawTokenToBnb(
        ...
        if (token != goodToken) {
            ...
            (uint256 amountToken, uint256 amountETH, bool isValid2) = AssetLib2.removeLiquidityBNB(
                dexRouter,
                token,
                goodToken,
                amount
            ...
            result += amountETH;
            (uint256 result2, bool isValid3) = _withdrawTokenToBnb(
                goodToken,
                goodToken,
                amountToken
            );
            if (isValid3 == true) {
                ...
                (uint256 amountToken2, uint256 amountETH2, uint256 liquidity) = AssetLib2
                .addLiquidityETH(dexRouter, goodToken, amountToken);
    ...
    function _setPercentages(uint256[4] memory distributionPercentages) private {
        ...
        treasury = 10000 - totalPercentages;

        // calculation for YDR_POOL_PENALTY_PERCENTAGES
        uint256 totalPerc = (10000 - totalPercentages) +
            distributionPercentages[2] +
            distributionPercentages[3];
        ...
        treasuryYdrPenalty = 10000 - ydrPercentages - ydrLpPercentages;
