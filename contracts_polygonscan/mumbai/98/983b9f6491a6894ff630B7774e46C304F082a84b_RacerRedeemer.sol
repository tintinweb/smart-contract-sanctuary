...
contract RacerRedeemer is AccessControlEnumerable {
    ...
    constructor(address RacerChildAddress) {
        ...
        onlyRole(MANAGER_ROLE)
    {
        ...
        for (uint256 i = 0; i < redeemers.length; i++) {
            for (uint256 j = 0; j < quantities[i]; j++) {
                //TODO: connect with data source for DNA / traits
                uint256 meltTokens = 1000 * 10**18;
                RC.newToken(
                    112233445566778899101112100000 + i,
                    redeemers[i],
                    meltTokens,
                    "Descriptor 1",
                    "Descriptor 2",
            ...
            quantityRedeemed[redeemers[i]] += quantities[i];
        }
    }
...
contract RacerChild is Racer, IChildToken {
    ...
    // {
    //     dnaString = Utilities.uintToString(_dna[tokenID]);
