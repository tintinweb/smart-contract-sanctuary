...
// File: contracts/HemetaTreasury.sol
...
contract HemetaTreasury is Ownable, IHemetaTreasury {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SHEME }
    ...
    address public sHEME;
    uint public sHEMEQueue; // Delays change to sHEME address
    ...
    constructor (
        ...
        isLiquidityToken[ _HEMEDAI ] = true;
        liquidityTokens.push( _HEMEDAI );
        bondCalculator[_HEMEDAI] = _bondCalculator;

        blocksNeededForQueue = _blocksNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for HEME
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external override returns ( uint send_ ) {
        ...
        // mint HEME needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( HEME ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn HEME for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external override {
        ...
        uint value = valueOfToken( _token, _amount );
        IHEMEERC20( HEME ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sHEME ).balanceOf( msg.sender ); // Can only borrow against sHEME held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with HEME
        @param _amount uint
     */
    function repayDebtWithHEME( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IHEMEERC20( HEME ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, HEME, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external override {
        ...
        IERC20Mintable( HEME ).mint( _recipient, _amount );
    ...
    function excessReserves() public override view returns ( uint ) {
        return totalReserves.sub( IERC20( HEME ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns HEME valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOfToken( address _token, uint _amount ) public view override returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match HEME decimals
            value_ = _amount.mul( 10 ** IERC20( HEME ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyOwner() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SHEME ) { // 9
            sHEMEQueue = block.number.add( blocksNeededForQueue );
        } else return false;
