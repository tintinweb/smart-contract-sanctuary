...
contract FullyOnChain_Test1 is ERC721Enumerable, ReentrancyGuard, Ownable {
  ...
  function tokenURI(uint256 tokenId) override public view returns (string memory) {
    ...
    return string(abi.encodePacked(rl4,Base64.encode(bytes(string(abi.encodePacked(title,getTraits(kanji),Base64.encode(bytes(genSVG1(kanji))))))),Base64.encode(bytes(cha[tokenId])),Base64.encode(bytes(Base64.encode(bytes(genSVG2(kanji,tokenId))))),Base64.encode(bytes(rl3))));
  }
