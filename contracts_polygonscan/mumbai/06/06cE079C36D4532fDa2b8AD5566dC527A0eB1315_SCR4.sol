...
// TODO: upates the error handling with: https://www.youtube.com/watch?v=59MRDldSItU&ab_channel=SmartContractProgrammer
// TODO: clear mapIndexToTokenId by setting latest index = 0 ? latest index: (mapIndexToTokenId[totalTokensRemaining - 1] = 0) https://ethereum.stackexchange.com/questions/110848/get-random-number-without-replacement-in-solidity
    ...
    ERC2981ContractWideRoyalties
{
    ...
    mapping(uint256 => uint256) internal mapIndexToTokenId; // maps vacant index positions to end position token ids
    ...
    mapping(address => bool) public inWhitelist; // use this to view if address in whitelist, as can't return(view) entire whitelist from fn call
    ...
    constructor(
        ...
        /**
        WARNING: becareful when using libraries ONLY in constructor as its address will become undetectable in deployed contract's bytecode.
        For a HardHat fix (via @nomiclabs/hardhat-etherscan): https://hardhat.org/plugins/nomiclabs-hardhat-etherscan.html#libraries-with-undetectable-addresses | https://hardhat.org/plugins/nomiclabs-hardhat-etherscan.html#providing-libraries-from-a-script-or-task
         */
    ...
    function requestRandomIndex() public payable nonReentrant whenNotPaused {
        ...
        processingMint[msg.sender] = true; // TODO: set this higher so that after click, dont need wait transaction confirm to trigger this?
    ...
    {
        // NFT Specific
        require(bytes(_token).length < 5, "16 tokens max.");
...
/** TODO:
0. gas test for all
0. can put variables in string msg (like for require() fn) like python's fstring OR JS's string templates?
0. make payable kolek monayyy - make auto set amount??
*/

/** GOOD RESOURCES:
    0.  upgrades (ideas): https://ether.cards/ ? | Governance?-to decide what params can be changed via setter methods?
    1.  basics: https://solidity-by-example.org/
    2.  docs: https://docs.soliditylang.org/en/v0.8.8/index.html
    3.  how to reduce gas: 
        a.  https://medium.com/coinmonks/8-ways-of-reducing-the-gas-consumption-of-your-smart-contracts-9a506b339c0a
        b.  https://blog.polymath.network/solidity-tips-and-tricks-to-save-gas-and-reduce-bytecode-size-c44580b218e6
