...
contract MasterChef is Ownable {
    ...
    // Info of claimed rewards by parent
    mapping(address => uint) public parentClaimed;
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        if(pending > 0) {
            ...
            if (Referral.hasReferrer(msg.sender)){
                cake.mint(Referral.getReferrer(msg.sender), pending.mul(refRewardRate).div(100));
                parentClaimed[Referral.getReferrer(msg.sender)] += pending.mul(refRewardRate).div(100);
            }
        }
        if(_amount > 0) {
    ...
    function claimReward() public {
        uint poolLen = poolLength();

        for(uint i = 0; i<poolLen; i++){
            if(pendingCake(i, msg.sender) > 0){
                if(i == 0){
                    leaveStaking(0);
                }
                else{
    ...
    function setRefRewardRate(uint _rate) external {
        require(msg.sender == devaddr, "dev: wut?");
        refRewardRate = _rate;
    }
}
