...
contract LuckyBonnieLotteryBNB is AccessControl, Ownable, ReentrancyGuard  {
    ...
    event RoundClose(uint256 indexed roundId, uint256 finalNumber, string signature);
    ...
    event InjectionFunds(address who, uint256 amount, uint256 total);
    event TreasuryClaim(address who, uint256 amount);
    ...
    function closeRound(
        ...
        _rounds[currentRoundId].urlSignature = _signature;
        ...
        emit RoundClose(currentRoundId, _finalNumber, _signature);
    }
    ...
    function historyUserRounds(uint256 _cursor, uint256 _size, address user) public view returns (RoundUserHistoryInfo[] memory result, uint256 total, uint256 cursor) {
        ...
        RoundUserHistoryInfo[] memory _mt =  new RoundUserHistoryInfo[](_size);
        ...
        for (uint256 i = 0; i < _size; i++) {
           ...
           _mt[i].nresult = _rounds[_mt[i].roundId].finalNumber;
        }
    ...
    function updateRewardRates(uint256[6] memory _newRawardsRate)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(
        ...
        require(
            _newRawardsRate[5] > _newRawardsRate[4] &&
            _newRawardsRate[4] > _newRawardsRate[3] &&
            _newRawardsRate[3] > _newRawardsRate[2] &&
            _newRawardsRate[2] > _newRawardsRate[1] &&
            _newRawardsRate[1] > _newRawardsRate[0],
            "invalid division rates"
        );

        rewardMatchRate = _newRawardsRate;
    }
    ...
    function _calculateClaimableRound(
        address _user,
        uint256 _roundId
    ) internal view returns (uint256 _reward, uint256 _index) {
        Round memory _r = _rounds[_roundId];
        uint256 _iu = _indexRoundsUser[_user][_roundId];

        require(_roundsByUser[_user].length > 0 && _roundsByUser[_user].length > _iu, "Not round");

        // final number
        uint256 _f = _r.finalNumber;
        // tickets user
        RoundUserInfo memory _ui = _roundsByUser[_user][_iu];
    ...
    function checkClaimableRound(
        address _customer,
        uint256 _roundId
    ) external view returns (bool claimable, uint256 totalReward) {
        (uint256 reward,) = _calculateClaimableRound(_customer, _roundId);

        return (reward > 0, reward);
    }
    
    function claimRound(uint256 _roundId) external onlyHuman nonReentrant {
        (uint256 reward, uint256 _iu) = _calculateClaimableRound(msg.sender, _roundId);
    ...
    function injectFunds() external payable onlyRole(DEFAULT_ADMIN_ROLE) {
        _injectFromPreviousRound += msg.value;

        emit InjectionFunds(msg.sender, msg.value, _injectFromPreviousRound);
    }

    function claimTreasury() external nonReentrant onlyRole(DEFAULT_ADMIN_ROLE) {
        uint256 currentTreasuryAmount = _treasury;
        _treasury = 0;
