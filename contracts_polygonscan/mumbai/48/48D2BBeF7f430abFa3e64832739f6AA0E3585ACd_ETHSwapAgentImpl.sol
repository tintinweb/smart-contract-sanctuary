...
contract ETHSwapAgentImpl is Context, Initializable {
    ...
    function fillBSC2ETHSwap(bytes32 bscTxHash, address erc20Addr, address toAddress, uint256 amount) onlyOwner external returns (bool) {
         //call needs to be call from the backend
    ...
    //call this function to swap between the erc20 addr to bsc

    function swapETH2BSC(address erc20Addr, uint256 amount) payable external notContract returns (bool) {
          //call needs to come from the frontend 
        require(registeredERC20[erc20Addr], "not registered token");//needs to be registered
        require(msg.value == swapFee, "swap fee not equal");//need to send the msg.value

        IERC20(erc20Addr).safeTransferFrom(msg.sender, address(this), amount);//amount needs to be sent
        if (msg.value != 0) {
