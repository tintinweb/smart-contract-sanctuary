...
/**
 * @title SGWM | Stoneguard Willows MDQ LTD
 * @author Stobox Technologies Inc.
 * @dev SGWM ERC20 Token | This contract is opt for digital securities management.
 */
 
contract SGWMToken is IERC20, Roles {
    ...
    modifier onlyWithUnfrozenFunds(address _account) {
        require(
            ...
            "SGWM: funds are frozen."
        );
        _;
    }

    /**
     * @notice SGWMToken simply implements a ERC20 token.
     */
    constructor() public Roles(msg.sender) {
        _name = "Stoneguard Willows MDQ Ltd Convertible Bond Security Token";
        _symbol = "SGWM";
        _decimals = 0;
        _kDecimals = 18;
        _k = 10**_kDecimals;
     ...
     * @param _value SGWMg that enables or disables the whitelist.
     */
     ...
     * @param _value SGWMg that enables or disables the transactions limit.
     */
    ...
    function burn(address _account, uint256 _amount) external onlySuperAdmin {
        require(balanceOf(_account) >= _amount, "SGWM: balance too low");
    ...
    function transferFunds(
        ...
        require(balanceOf(_from) >= _amount, "SGWM: not enough tokens");
    ...
    function splitOrMerge(uint256 _x, uint256 _y) external onlySuperAdmin {
        require(_x != _y, "SGWM: _x must not be equal to _y");
        require(_x > 0, "SGWM: _x must be greater than 0");
        require(_y > 0, "SGWM: _y must be greater than 0");
    ...
    */
        ...
        returns (bool)
    {
        ...
        _approve(
            ...
            _allowances[_sender][msg.sender].sub(
                ...
                "SGWM: transfer amount exceeds allowance"
            )
        );
        ...
        returns (bool)
    {
        _approve(
            ...
            _allowances[msg.sender][_spender].sub(
                ...
                "SGWM: decreased allowance below zero"
            )
        );
    ...
    function _transfer(
        ...
        require(_sender != address(0), "SGWM: transfer from the zero address");
        require(_recipient != address(0), "SGWM: transfer to the zero address");

        _beforeTokenTransfer(_sender, _recipient, _amount);

        _balances[_sender] = _balances[_sender].sub(
            _amount,
            "SGWM: transfer amount exceeds balance"
        );
    ...
    function _mint(address _account, uint256 _amount) internal virtual {
        require(_account != address(0), "SGWM: mint to the zero address");
    ...
    function _burn(address _account, uint256 _amount) internal virtual {
        require(_account != address(0), "SGWM: burn from the zero address");

        _beforeTokenTransfer(_account, address(0), _amount);

        _balances[_account] = _balances[_account].sub(
            _amount,
            "SGWM: burn amount exceeds balance"
        );
    ...
    function _approve(
        ...
        require(_owner != address(0), "SGWM: approve from the zero address");
        require(_spender != address(0), "SGWM: approve to the zero address");
    ...
    function _updateTransferLimit(address _account, uint256 _amount) private {
        ...
        _transferLimits[_account].allowedToTransfer = _transferLimits[_account]
            .allowedToTransfer
            .sub(_amount, "SGWM: transfer exceeds your transfer limit");
    }
    ...
    function _updateTransactionCountLimit(address _account) private {
        if (_isEnabledTransactionCount) {
            ...
            _transactionCountLimits[_account]
                ...
                .sub(1, "SGWM: transfer exceeds your transaction count limit");
        }
    }
}