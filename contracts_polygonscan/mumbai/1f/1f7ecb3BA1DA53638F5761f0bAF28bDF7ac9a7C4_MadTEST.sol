...
contract MadTEST is Ownable, ReentrancyGuard, AccessControl {
    ...
    function pendingReward(address _user) external returns (uint256) {
        ...
        // if (user.hasNFT)
        //     calculatedAmount = user.powerAmount;
    ...
    function _calculatedUserAmount() internal returns (uint256) {
        
        uint256 newAmount = userInfo[msg.sender].amount;

        if (userInfo[msg.sender].hasNFT)
            newAmount = userInfo[msg.sender].powerAmount;
