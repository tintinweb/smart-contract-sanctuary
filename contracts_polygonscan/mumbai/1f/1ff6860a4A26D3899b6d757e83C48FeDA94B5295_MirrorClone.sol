...
// This is a test for verification functionality
...
contract MirrorClone is ERC721, ERC721Enumerable, ERC721URIStorage {
    ...
    function createToken(string memory _tokenURI) public returns (uint) {
        ...
        _tokenIds.increment();
        uint256 newItmId = _tokenIds.current();

        // Mint token
        _safeMint(msg.sender, newItmId);

        // Set token URI
        _setTokenURI(newItmId, _tokenURI);

        // Set tokenURIToTokenId
        tokenURIToTokenId[_tokenURI] = newItmId;

        // Emit TokemMinted event
        emit TokenMinted(msg.sender, newItmId, _tokenURI);
