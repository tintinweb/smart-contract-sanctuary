...
contract RandomNumberGenerator is VRFConsumerBase {
    ...
    //Because the number generation requires two transactions, you must map the request id to keep track of the operator address in accordance with the ERC1155 standard.
    mapping(bytes32 => address) requesterAddress;
    
    //The random number returned must be mapped to be able to look up programmatically in the future.
    mapping(bytes32 => uint256) public randomNumberReturned;
    ...
    function getRandomNumber() public returns (bytes32 requestId) {
        ...
        requesterAddress[requestId] = msg.sender;
        return requestId;
    }


    //When the Chainlink VRF coordinator calls back with the random number the return is saved to the mapping.
    function fulfillRandomness(bytes32 requestId, uint256 randomNumber) internal override {
        require(msg.sender == vrfCoordinator, "Only the VRF Coordinator may call this function.");
        randomNumberReturned[requestId] = randomNumber;
    }
    ...
    require(link.transfer(receiverAccount, link.balanceOf(address(this))), "Unable to transfer");
        ...
        //Set the receiving account paid for service.
