...
contract pixeLoot is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    function generateColorString(uint256 color) internal pure returns(bytes memory colorString) {
        // Return string representation of the color hex code in bytes array.
        bytes3 colorBytes = generateColorBytes(color);
        return bytes(uint256(bytes32(abi.encodePacked(colorBytes))).toHexString());
    }
