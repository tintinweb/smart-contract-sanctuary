...
contract NiftyArtist is ERC721EnumerableUpgradeable,  AccessControlUpgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    function initialize(string memory name, string memory symbol, string memory uri, address tokenAddress) public virtual initializer {
    ...
    function mint(address to, uint qty) public virtual nonReentrant {
        ...
        require(WETH.transferFrom(msg.sender, address(this), totalPrice), "MINT:INSUFFICIENT WETH");
    ...
    */
    ...
    function withdrawAll(address recipient) public payable {
        require(hasRole(ADMIN_ROLE, msg.sender), "Caller is not admin");

        uint256 balance = WETH.balanceOf(address(this));
        require(WETH.transfer(recipient, balance), "WETH transfer failed");
    }
    ...
    // Transfer tokens in this contract to specific address
    // function transferTokens(address to, uint256 fromIndex, uint256 toIndex) public {
    //     require(hasRole(ADMIN_ROLE, msg.sender), "Caller is not admin");

    //     // Transfer token ID 7 through 31 to new owner
    //     uint256 index;
    //     for (index = fromIndex; index <= toIndex; index++) {
    //         _transfer(address(this),to, index);
    //     }
    // }
}
