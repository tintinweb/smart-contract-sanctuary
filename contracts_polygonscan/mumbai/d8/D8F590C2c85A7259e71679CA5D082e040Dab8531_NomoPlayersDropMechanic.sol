 ...
 */
contract NomoPlayersDropMechanic is Ownable, ReentrancyGuard, VRFConsumerBase {
    ...
    function buyTokens(uint256 quantity) private nonReentrant {
        ...
        for (uint256 i = 0; i < quantity; i++) {
            ...
            getRandomNumber();
            uint256 tokenId = tokens[randomResult];
            transferredTokens[i] = tokenId;
            tokens[randomResult] = tokens[tokens.length - 1];
            tokens.pop();
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness)
        ...
        override
    {
        randomResult = randomness.mod(getTokensLeft());
    }
