...
contract Ownable {
    ...
    mapping(address => string[]) uriowned;
    mapping(string => uint256[]) uriid;
    mapping(string => address[]) uriaddress;
...
interface ERC721Metadata {
    ...
    function tokentitle(uint256 _tokenId) external view returns (string memory);
}
...
contract NFToken is Ownable, ERC721, SupportsInterface {
    ...
    function safeTransferFrom(
        ...
        _safeTransferFrom(_from, _to, _tokenId, _uri, _data);
    }
    ...
    function safeTransferFrom(
        ...
        _safeTransferFrom(_from, _to, _tokenId, _uri, "");
    }
    ...
    function _transfer(
        ...
        _changeholder(from, _to, _tokenId, _uri);
    }
...
contract NFTokenMetadata is NFToken, ERC721Metadata {
    ...
    mapping(uint256 => string) internal idToTitle;
    ...
    function tokentitle(uint256 _tokenId)
        external
        view
        override
        validNFToken(_tokenId)
    ...
    function _setTokenprice(uint256 _tokenId, uint256 _basicprice)
        internal
        validNFToken(_tokenId)
    {
        idToprice[_tokenId] = _basicprice;
    }

    function _setTokentitle(uint256 _tokenId, string memory _title)
        internal
        validNFToken(_tokenId)
    {
        idToTitle[_tokenId] = _title;
    }
}

// File: contracts/CreatorNFT.sol

contract CreatorNFT is NFTokenMetadata {
    uint256 _tokenId;
    ...
    function mintNFT(
        ...
        // uint256 basicprice=_basicprice * n;
        uint256 j;
        for (j = 1; j <= n; j++) {
            super._mint(_to, _tokenId);
            super._setTokenUri(_tokenId, _uri);
            super._setTokenprice(_tokenId, _basicprice);
            super._setTokentitle(_tokenId, _title);
            tokensowned[_to].push(_tokenId);
            uriowned[_to].push(_uri);
            uriid[_uri].push(_tokenId);
            uriaddress[_uri].push(_to);
            _tokenId += 1;
        }
    }
    ...
    {
        return (tokensowned[holder], uriowned[holder]);
    }

    function uriOwned(address holder) external view returns (string[] memory) {
        return uriowned[holder];
    }

    function uriId(string memory uri)
        external
        view
        returns (uint256[] memory, address[] memory)
    {
        return (uriid[uri], uriaddress[uri]);
    }
