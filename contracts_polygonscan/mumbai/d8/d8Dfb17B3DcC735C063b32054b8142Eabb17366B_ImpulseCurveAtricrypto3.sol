...
//        _verifySetup();
    ...
    function wantPriceInUnderlying(uint256 _wantAmt) public view override virtual returns (uint256[] memory) {
    ...
    function wantPriceInUsd(uint256 _wantAmt) external view override virtual returns (uint256) {
        ...
        uint256 result = 0;
        uint256[] memory underlyingBalances = wantPriceInUnderlying(_wantAmt);
        for (uint256 i = 0; i < underlyingTokens.length; i++) {
            uint256 amount = IERC20(underlyingTokens[i]).balanceOf(address(wantToken));
            address[] memory path = swapRewardRoutes[address(underlyingTokens[i])][usdToken];
            result = result + (i + 1) * underlyingBalances[i];
        }
        return result;
    }
...
// Part: ERC20PresetMinterPauserMock
...
contract ImpulseCurveAtricrypto3 is BaseImpulseMultiStrategy {
    ...
    uint256 constant ATRICRYPTO3_UNDERLYING_COUNT = 5;
    ...
    /// @notice Deposit want token to staking contract.
    /// @param _wantAmt Amount of want token.
    function _depositLpToken(uint256 _wantAmt) internal override {
    }

    /// @notice Withdraw want token from staking contract.
    /// @param _wantAmt Amount of want token.
    function _withdrawLpToken(uint256 _wantAmt) internal override {
    }
    ...
    function sharesMock() internal returns (uint256) {
        uint256 result = 0;
        for(uint256 i=0;i<underlyingTokens.length;i++){
            result = result + underlyingTokens[i].balanceOf(address(this));
        }
        return result;
    }
    ...
    /// @notice Swap all underlying tokens to want tokens.
    /// @return Want received amount.
    function _swapAllUnderlyingToWant() internal override returns (uint256) {
        uint newBalance = sharesMock();
        uint256 result = newBalance - wantTotal;
        wantTotal = newBalance;
        ERC20PresetMinterPauserMock(wantToken).mint(address(this), result);
        return result;
    }

    /// @notice Swap all underlying tokens to want tokens.
    function _swapAllWantTolUnderlying(address _receiver) internal override {
        for(uint256 i=0;i<underlyingTokens.length;i++){
            uint b = underlyingTokens[i].balanceOf(address(this)) / 50;
            underlyingTokens[i].transfer(_receiver, b);
        }
        wantTotal = sharesMock();
    }
    
    function _swapAllWantToOneUnderlying(address _receiver, address _underlying) internal virtual override {
            uint256 bal = IERC20(_underlying).balanceOf(address(this)) / 50;
            IERC20(_underlying).transfer(_receiver, bal);
            wantTotal = sharesMock();
        }

    /// @notice Verify constant data, provided on initialize.
    function _verifySetup() internal override {
    }
    ...
    function wantPriceInUnderlying(uint256 _wantAmt) public view override returns (uint256[] memory) {
        uint256[] memory result = new uint256[](underlyingTokens.length);
        for(uint256 i=0;i<underlyingTokens.length;i++){
            result[i] = _wantAmt * underlyingTokens[i].balanceOf(address(this)) / ERC20PresetMinterPauserMock(wantToken).totalSupply();
        }
        return result;
    }
}