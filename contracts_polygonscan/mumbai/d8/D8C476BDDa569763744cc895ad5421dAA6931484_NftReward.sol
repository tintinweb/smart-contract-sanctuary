...
contract NftReward is AccessControl {
    ...
    CanvasManager internal immutable _canvas;
    PixNft internal immutable _pixNft;
    IPixelCoin internal immutable _pixelCoin;

    uint256 internal _defaultReward; // default reward per NFT
    ...
    event SetDefaultReward(uint256 oldDefaultReward, uint256 newDefaultReward);
    ...
    function setDefaultReward(uint256 newDefaultReward) external {
        uint256 oldDefaultReward = _defaultReward;
        _defaultReward = newDefaultReward;

        emit SetDefaultReward(oldDefaultReward, newDefaultReward);
    }

    function getClaimeInfo(uint256 nftId, address contributor) external view returns (bool isClaimed, uint256 amount) {
        (uint256 share, , uint256 total, , ) = _canvas.getNftPorfitInfo(nftId, contributor);
        uint256 rewardOfNft = (_rewardOfNft[nftId] == 0) ? _defaultReward : _rewardOfNft[nftId];

        isClaimed = _isClaimed[nftId][contributor];
        amount = (rewardOfNft * share) / total;
    }

    function claim(uint256 nftId, address contributor) external {
        uint256 defaultReward = _defaultReward;
        require(defaultReward > 0, "not start");
        ...
        (uint256 share, , uint256 total, , ) = _canvas.getNftPorfitInfo(nftId, contributor);
        require(share > 0, "no contribution");
        ...
        uint256 amount = (rewardOfNft * share) / total;
