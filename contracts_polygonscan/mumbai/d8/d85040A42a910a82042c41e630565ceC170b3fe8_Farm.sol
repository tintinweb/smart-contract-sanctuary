...
contract Farm is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        bool childPoolSupportsReferral; //if child farm supports referral mechanism
    }

    // The fYGN TOKEN!
    IFYGN public fYGN;
    // Block number when bonus FYGN period ends.
    uint256 public bonusEndBlock;
    // FYGN tokens created per block.
    uint256 public fYGNPerBlock;
    // YGN Converter address
    ...
    // Bonus muliplier for early fygn makers.
    ...
    // The block number when fYGN minting starts.
    ...
    event PoolAddition(
        ...
        bool childPoolSupportsReferral
    );
    ...
    event UpdatedChildPoolReferralStatus(uint256 indexed pid, bool childPoolSupportsReferral);
    ...
    function updateBlockRate(uint256 _fYGNPerBlock) external onlyOwner {
        massUpdatePools();
        fYGNPerBlock = _fYGNPerBlock;
        emit BlockRateUpdated(fYGNPerBlock);
    }
    ...
    function getLpTokenValue(uint256 _pid) public view validatePoolByPid(_pid) returns (uint256) {
    ...
    function add(
        ...
        bool _childPoolSupportsReferral,
        bool _withUpdate
        ...
        poolInfo.push(
            PoolInfo({
                ...
                childPoolSupportsReferral: _childPoolSupportsReferral
            })
        );
    ...
    function addChildFarmToPool(
        ...
        if (amount > 0) {
            ...
            if (_childPoolSupportsReferral) {
                _childFarm.deposit(_childPoolId, amount, address(ygnConverter));
            } else {
        ...
        pool.childPoolSupportsReferral = _childPoolSupportsReferral;
        ...
        nonReentrant
    {
        ...
        pool.childPoolSupportsReferral = false;
    ...
    function updateChildFarmInPool(
        ...
        bool _newChildPoolSupportsReferral,
        bool _withUpdate
    ) external onlyOwner validatePoolByPid(_pid) nonReentrant {
        require(
            address(poolInfo[_pid].childFarm) != address(0),
        ...
        if (amount > 0) {
            ...
            if (_newChildPoolSupportsReferral) {
                _newChildFarm.deposit(_newChildPoolId, amount, address(ygnConverter));
            } else {
        ...
        pool.childPoolSupportsReferral = _newChildPoolSupportsReferral;
        ...
        emit UpdatedChildFarm(
            ...
            _newChildPoolId,
            _newChildFarmToken,
            _newChildPoolSupportsReferral
        );
    }

    function updateChildPoolReferralSupport(
        uint256 _pid,
        bool _childPoolSupportsReferral,
        bool _withUpdate
    ) external onlyOwner validatePoolByPid(_pid) nonReentrant {
        ...
        updatePool(_pid);
        poolInfo[_pid].childPoolSupportsReferral = _childPoolSupportsReferral;
        emit UpdatedChildPoolReferralStatus(_pid, _childPoolSupportsReferral);
    }

    //confirm if this is needed
    //shouldn't be needed to be called ever.
        ...
        returns (uint256)
    {
        ...
        uint256 lpSupply = getLpTokenValue(_pid);
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 fygnReward = multiplier.mul(fYGNPerBlock).mul(pool.allocPoint).div(
                totalAllocPoint
            );
            accfYGNPerShare = accfYGNPerShare.add(fygnReward.mul(1e12).div(lpSupply));
        }
        uint256 pending = user.amount.mul(accfYGNPerShare).div(1e12).sub(user.rewardDebt);
    ...
    function updatePool(uint256 _pid) public validatePoolByPid(_pid) {
        ...
        pool.accfYGNPerShare = pool.accfYGNPerShare.add(fygnReward.mul(1e12).div(lpSupply));
    ...
    function _deposit(
        ...
        updatePool(_pid);
        payOrLockupPendingFYGN(_pid, _user, _user);

        if (_amount > 0) {
            pool.lpToken.safeTransferFrom(address(msg.sender), address(this), _amount);
            if (address(pool.childFarm) != address(0)) {
                (uint256 beforeDepositAmount, ) = pool.childFarm.userInfo(
                    pool.childPoolId,
                    address(this)
                );
                IERC20(address(pool.lpToken)).approve(address(pool.childFarm), _amount);
                if (pool.childPoolSupportsReferral) {
                    pool.childFarm.deposit(pool.childPoolId, _amount, address(ygnConverter));
                } else {
                    pool.childFarm.deposit(pool.childPoolId, _amount);
                }
                (uint256 afterDepositAmount, ) = pool.childFarm.userInfo(
                    pool.childPoolId,
                    address(this)
                );
                user.amount = user.amount.add(afterDepositAmount).sub(beforeDepositAmount);
            } else {
        ...
        user.rewardDebt = user.amount.mul(pool.accfYGNPerShare).div(1e12);
    ...
    function _withdraw(
        ...
        updatePool(_pid);
        payOrLockupPendingFYGN(_pid, _user, _withdrawer);
    ...
    function emergencyWithdraw(uint256 _pid) external validatePoolByPid(_pid) nonReentrant {
        ...
        if (address(pool.childFarm) != address(0)) {
            drain(_pid);
            pool.childFarm.emergencyWithdraw(pool.childPoolId);
        }
    ...
    function payOrLockupPendingFYGN(
        uint256 _pid,
        address _user,
        address _withdrawer
    ) internal {
        ...
        uint256 pending = user.amount.mul(pool.accfYGNPerShare).div(1e12).sub(user.rewardDebt);
        if (canHarvest(_pid, _user)) {
            if (pending > 0 || user.rewardLockedUp > 0) {
                ...
                if (isRewardManagerEnabled == true) {
                    safeFYGNTransfer(rewardManager, totalRewards);
                    ...
                    safeFYGNTransfer(_withdrawer, totalRewards);
                }
            }
        } else if (pending > 0) {
    ...
    function setYGNConverter(address _ygnConverter) external onlyOwner {
        require(_ygnConverter != address(0), "setYGNConverter: invalid address");
        ygnConverter = _ygnConverter;
        emit SetYGNConverter(msg.sender, _ygnConverter);
    }

    function withdrawFYGN(uint256 _amount) external onlyOwner {
        fYGN.transfer(msg.sender, _amount);
    }
    ...
    function drain(uint256 _pid) public {
        ...
        if (address(pool.childFarmToken) != address(0) && address(pool.childFarm) != address(0)) {
            ...
            if (childFarmTokenBal > 0) {
                pool.childFarmToken.safeTransfer(ygnConverter, childFarmTokenBal);
            }
        }
    }

    // Safe fygn transfer function, just in case if rounding error causes pool to not have enough fYGNs.
    function safeFYGNTransfer(address _to, uint256 _amount) internal {
        uint256 fygnBal = fYGN.balanceOf(address(this));
        if (_amount > fygnBal) {
            fYGN.transfer(_to, fygnBal);
        } else {
            fYGN.transfer(_to, _amount);
        }
    }
}
