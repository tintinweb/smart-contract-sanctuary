...
import "../CMTAT/contracts/CMTAT.sol";
import "hardhat/console.sol";
import "./DocumentsModule.sol";

contract Token is CMTAT, DocumentsModule {
    // https://github.com/OpenZeppelin/openzeppelin-contracts/blob/release-v4.0/contracts/metatx/MinimalForwarder.sol
    ...
    function initToken(
        ...
        __CMTAT_init(_msgSender(), forwarder, name, symbol, symbol, "");
        __Validation_init(IRuleEngine(ruleEngine));
        __Documents_init(document);
        mint(msg.sender, amount);
    }

    function decimals() public view virtual override(CMTAT) returns (uint8) {
        return 18;
    }
...
import "../CMTAT/openzeppelin-contracts-upgradeable/contracts/proxy/utils/Initializable.sol";
...
abstract contract DocumentsModule is Initializable {
    Document public document;

    function __Documents_init(Document memory _document)
        public
        initializer
    {
        document = _document;
        emit DocumentAttached(document.name, document.hash);
    }

    event DocumentAttached(string indexed name, string hash);
}
