...
contract Shop is Ownable, AccessControlEnumerable, ReentrancyGuard {
    ...
    event BuyItem(address indexed userAddress, uint256 timeStamp, uint256 itemId, uint256 itemAmount, uint256 nakaAmount);
    ...
    modifier whenShopNotPaused() {
        require(!_paused, "[Shop.whenShopNotPaused] Paused");
        _;
    }

    modifier whenShopPaused() {
        require(_paused, "[Shop.whenShopPaused] Not paused");
        _;
    }

    function buyItems(address _userAddress, uint256 _itemId, uint256 _itemAmount,uint256 _nakaAmount) external whenShopNotPaused nonReentrant {
        
        vaultItem.increaseItem(_userAddress, _itemId, _itemAmount);
        vaultBalance.decreaseBalance(_userAddress, _nakaAmount);

        emit BuyItem(msg.sender, block.timestamp, _itemId, _itemAmount,_nakaAmount);
    }

    function pauseShop() external onlyRole(DEFAULT_ADMIN_ROLE) whenShopNotPaused {
        _paused = true;
        emit ShopPaused();
    }

    function unpauseShop() external onlyRole(DEFAULT_ADMIN_ROLE) whenShopPaused {
        _paused = false;
        emit ShopUnpaused();
    }
