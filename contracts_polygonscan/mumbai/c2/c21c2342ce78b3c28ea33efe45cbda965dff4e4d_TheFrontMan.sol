...
interface IStakeGameInstance {
    ...
    function totalSharePerPlayer(uint8 epochIndex) external view returns (uint256);
...
contract StakeGameInstance is IStakeGameInstance {
   ...
   struct EpochStatus {
    ...
    function totalSharePerPlayer(uint8 epochIndex) external view override returns (uint256) {
...
contract TheFrontMan is ITheFrontMan {
    address[] private _gameInstances;

    event GameCreated(address gameInstance, uint64 startTime);
    event PlayerStaked(address player, uint256 gameId);

    constructor() {

    }

    function newGame(address[] memory tokens, uint256[] memory pPrizeAmounts, uint64 pStartTime, uint8 pTotalEpochs, uint32 pEpochDuration, uint8 pGameTax, uint8 pVolatility, uint8 pTokenRand, uint256[] memory pMedian) external override returns (address) {  
        address gameInstanceAddress = address(new StakeGameInstance(tokens, pPrizeAmounts, pStartTime, pTotalEpochs, pEpochDuration, pGameTax, pVolatility, pTokenRand, pMedian));
        _gameInstances.push(gameInstanceAddress);

        for (uint256 i=0; i < tokens.length; i++) {
            address tokenAddress = tokens[i];
            uint256 prizeAmount = pPrizeAmounts[i];
            if (prizeAmount > 0) {
                IERC20(tokenAddress).transfer(gameInstanceAddress, prizeAmount);
            }
        }

        emit GameCreated(gameInstanceAddress, pStartTime); 
        return gameInstanceAddress;
    }

    function gameCount() external view override returns (uint256) {
        return _gameInstances.length;
    }

    function gameById(uint256 gameId) external view override returns (address) {
        return _gameInstances[gameId];
    }

    function stake(uint256 gameId) external override {
        StakeGameInstance gameInstance = StakeGameInstance(_gameInstances[gameId]);

        require(gameInstance.gameStarted() == true, "Game has not started yet");
        require(gameInstance.gameEnded() == false, "Game has already ended");

        uint8 epochIndex = gameInstance.currentEpochIndex();

        require(gameInstance.hasStakedAtEpoch(msg.sender, epochIndex) == false, "Player has already staked this epoch");
        require(gameInstance.isAliveAtEpoch(msg.sender, epochIndex) == true, "Player is dead");

        address[] memory tokens = gameInstance.tokens();

        uint256[] memory amountsToStake = gameInstance.amountsToStakeForEpoch(epochIndex);
        for (uint256 i = 0; i < tokens.length; i++) {
            address tokenAddress = tokens[i];
            uint256 amountToStake = amountsToStake[i];
            if (amountToStake > 0) {
                IERC20(tokenAddress).transferFrom(msg.sender, address(gameInstance), amountToStake);
            }
        }
        
        gameInstance._stake(amountsToStake, msg.sender, epochIndex);
        emit PlayerStaked(msg.sender, gameId);
    }

    function transfer(address tokenAddress, address to, uint256 amount) external override {
