...
contract RahatAdmin is ERC1155Holder {
	event ProjectBudgetUpdated(bytes32 indexed projectId, uint256 projectCapital);
		...
		CheckProject(_projectId)
	{
		...
		CheckProject(_projectId)
	{
	    require(tokenContract.exists(tokenId),"RahatAdmin: Token with given id doesn't exists");
	...
	function mintItem(string memory _name,
		string memory _symbol,uint256 _amount) public OnlyOwner {
		    tokenContract.mintItem(_name,_symbol,_amount);
		}
