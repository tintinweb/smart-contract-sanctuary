...
contract EggSpawner is AccessControlEnumerable {
    using SafeERC20 for IERC20;

    bytes32 public constant PHASE_ADVANCER = keccak256("PHASE_ADVANCER");
    bytes32 public constant RATE_SETTER = keccak256("RATE_SETTER");
    bytes32 public constant TEAM_WALLET_SETTER =
        keccak256("TEAM_WALLET_SETTER");
    ...
    */
    ...
    // 97 to 97 (1 bit: 0 to 1) card
    uint256 constant CARD_SHIFT = VERSION_SHIFT + VERSION_LENGTH;
    uint256 constant CARD_LENGTH = 1;
    ...
    uint256 constant MAGIC_MASK =
        0x000000000000000000000000000000000000003C000000000000000000000000;

    // 102 to 255 (154 bits: 0 to 22835963083295358096932575511191922182123945983) wallet (>> 6)
    uint256 constant WALLET_SHIFT = MAGIC_SHIFT + MAGIC_LENGTH;
    uint256 constant WALLET_LENGTH = 154;
    ...
    IINft immutable private iNft;
    uint256 private nonce;

    Phase public phase = Phase.RARE;
    ...
    {
        card = uint8((token & CARD_MASK) >> CARD_SHIFT);
        magic = uint8((token & MAGIC_MASK) >> MAGIC_SHIFT);
        wallet = uint160((token & WALLET_MASK) >> WALLET_SHIFT);
        ...
        require(0 == chain, "Not a Polygon token");
        require(1 == fungible, "Not an NFT");
        require(1 == type_, "Not an egg");
        require(0 == version, "Unknown version");
        require(0 == card || 1 == card, "Invalid card number");
        require(5 == magic || 10 == magic, "Invalid magic");
    }

    function cardToToken(uint16 card, address wallet)
        public
        pure
    ...
    function advancePhase() external notEnded onlyRole(PHASE_ADVANCER) {
        if (Phase.RARE == phase) {
    ...
    function setRate(uint256 rate_) external onlyRole(RATE_SETTER) {
        rate = rate_;
    }

    function setTeamWallet(address teamWallet_)
        external
        onlyRole(TEAM_WALLET_SETTER)
    {
        teamWallet = teamWallet_;
    }

    function spawnEgg() external notEnded {
        if (0 < rate) {
        ...
        if (Phase.RARE == phase) {
            ...
            uint256 token = cardToToken(0, msg.sender);
            token |= iNft.nextTokenId();
            rareEggs[msg.sender] = token;
            iNft.mintNFT(msg.sender, token);
        } else if (Phase.UNCOMMON == phase) {
            require(0 == uncommonEggs[msg.sender], "Already have Uncommon egg");
            uint256 token = cardToToken(1, msg.sender);
            token |= iNft.nextTokenId();
            uncommonEggs[msg.sender] = token;
            iNft.mintNFT(msg.sender, token);
        } else {
            revert("Invalid phase");
        }
