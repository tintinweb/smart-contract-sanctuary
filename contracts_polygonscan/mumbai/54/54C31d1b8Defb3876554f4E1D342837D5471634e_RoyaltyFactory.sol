...
contract RoyaltyFactory {
    ...
    event LoyaltyERC721Created(string name, string symbol, address nft);

    constructor() {
        royaltyCounter = 0;
    }

    function createRoyaltyERC721(string memory name, string memory symbol, string calldata uri, address royaltyAddress, uint16 bps, string calldata proofOfAuthenticity)
        external
        returns (address loyalty, uint256 counter)
    {
        ...
        _loyalty.mintWithRoyalty(msg.sender, uri, royaltyAddress, bps, proofOfAuthenticity);
         emit LoyaltyERC721Created(name, symbol, loyalty);
