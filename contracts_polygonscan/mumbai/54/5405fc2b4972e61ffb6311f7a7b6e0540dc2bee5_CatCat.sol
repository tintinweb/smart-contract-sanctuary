...
// File: contracts/CatCat.sol


pragma solidity ^0.7.0;


contract CatCat is ERC721, Ownable {
    using Strings for uint256;
    ...
    function airDrop(address[] memory saleAddresses) public onlyOwner {
        require(totalCats + saleAddresses.length <= totalCount, "Purchase would exceed max supply of ACTs");
        for(uint i = 0; i < saleAddresses.length; i++) {
            if (totalSupply() < totalCount) {
                emit MintCat(saleAddresses[i], totalCats+1, 1);
                _mint(saleAddresses[i], 1 + totalCats++);
            }
        }
    }
    ...
    function mintCat(uint256 _times) payable public {
