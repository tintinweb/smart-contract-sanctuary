...
contract TossyToken is ERC20("SpinToss", "TOSSY"), Ownable {
...
// MasterChef is the master of Tossy. He can make Tossy and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once Tossy is sufficiently
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of TOSSYs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accTossyPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accTossyPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;         // How many allocation points assigned to this pool. TOSSYs to distribute per block.
        uint256 lastRewardBlock;    // Last block number that TOSSYs distribution occurs.
        uint256 accTossyPerShare;    // Accumulated TOSSYs per share, times 1e18. See below.
    ...
    // The TOSSY TOKEN!
    TossyToken public tossy;
    address public devAddress;
    address public feeAddress;

    // Initial emission rate: 50 TOSSY per block.
    uint256 public constant INITIAL_EMISSION_RATE = 50 ether;
    // Target emission rate: 5 TOSSY per block.
    ...
    // TOSSY tokens created per block.
    uint256 public tossyPerBlock = INITIAL_EMISSION_RATE;
    ...
    // The block number when TOSSY mining starts.
    uint256 public startBlock;

    // Tossy referral contract address.
    ...
    event UpdateEmissionRate(address indexed user, uint256 tossyPerBlock);
    ...
    // Update the given pool's TOSSY allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending TOSSYs on frontend.
    function pendingTossy(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accTossyPerShare = pool.accTossyPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 tossyReward = multiplier.mul(tossyPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accTossyPerShare = accTossyPerShare.add(tossyReward.mul(1e18).div(lpSupply));
        }
        return user.amount.mul(accTossyPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 tossyReward = multiplier.mul(tossyPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        tossy.mint(devAddress, tossyReward.div(10));
        tossy.mint(address(this), tossyReward);
        pool.accTossyPerShare = pool.accTossyPerShare.add(tossyReward.mul(1e18).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for TOSSY allocation.
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accTossyPerShare).div(1e18).sub(user.rewardDebt);
            if (pending > 0) {
        ...
        if (_amount > 0) {
            ...
            if (pool.depositFeeBP > 0) {
                // Transfer 75% of deposit fee to Fee Address for token buyback and SpinToss games
        ...
        user.rewardDebt = user.amount.mul(pool.accTossyPerShare).div(1e18);
    ...
    // Safe tossy transfer function, just in case if rounding error causes pool to not have enough TOSSY.
    function safeTossyTransfer(address _to, uint256 _amount) internal {
        uint256 tossyBal = tossy.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > tossyBal) {
            transferSuccess = tossy.transfer(_to, tossyBal);
        } else {
            transferSuccess = tossy.transfer(_to, _amount);
        }
        require(transferSuccess, "safeTossyTransfer: Transfer failed");
    }
    ...
    function updateEmissionRate() public {
        ...
        require(tossyPerBlock > FINAL_EMISSION_RATE, "updateEmissionRate: Emission rate has reached FINAL_EMISSION_RATE already");
        ...
        uint256 newEmissionRate = tossyPerBlock;
        ...
        require(newEmissionRate < tossyPerBlock, "updateEmissionRate: New emission rate must be less than current emission rate.");

        massUpdatePools();
        lastReductionPeriodIndex = currentIndex;
        tossyPerBlock = newEmissionRate;
        emit UpdateEmissionRate(msg.sender, tossyPerBlock);
    }
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (address(referral) != address(0) && referralCommissionRate > 0) {
            ...
            if (referrer != address(0) && commissionAmount > 0) {
                tossy.mint(_user, commissionAmount);
                tossy.mint(referrer, commissionAmount);
