...
contract MoodyPoops is ERC721, Ownable {
    ...
    uint16 public MAX_PER_ADDRESS = 20;
    ...
    function whitelistAddress(address[] calldata _addresses) public onlyOwner {
    ...
    function mintPresale() public payable {
        ...
        require(balanceOf(msg.sender)<MAX_PER_ADDRESS);
    ...
    function multiMintPresale(uint256 _amount) public payable {
        require(isPresaleActive);
        require(isWhitelisted(msg.sender));
        require(msg.value == NFT_PRICE.mul(_amount));
        require(_currentTokenId < MAXSUPPLY, "Max Supply Reached");
        require(balanceOf(msg.sender)+_amount<=MAX_PER_ADDRESS,"trying to mint more than the max per address limit");
    ...
    function giveaway (address _to) public onlyOwner{
        require(_currentTokenId < MAXSUPPLY, "Max Supply Reached");
        require(balanceOf(_to)<MAX_PER_ADDRESS, "Max Per Address Reached");
    ...
    function increaseSupply(uint16 amount) external onlyOwner {
