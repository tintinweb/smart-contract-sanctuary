...
contract AnapavoToken is ERC721URIStorageEnumerable, AccessControl, ReentrancyGuard {
  ...
  function _beforeTokenTransfer(
    ...
    require(!isMaintaining, "AnapavoToken: it's currently maintaining");
  }

  function mintForUser(
    address _minter,
    string memory _userId,
    string memory _mappingId,
    ...
    onlyAnapavo
  {
    require(bytes(_userId).length > 0, "AnapavoToken: user id is empty");
    require(bytes(_tokenURI).length > 0, "AnapavoToken: token uri is empty");
    ...
    _mint(_minter == address(0) ? owner : _minter, newTokenId);
  ...
  function changeTokenOwnership(
    ...
    onlyAnapavo
  {
    require(_newOwner != address(0), "AnapavoToken: new owner is empty");
    require(bytes(_userId).length > 0, "AnapavoToken: user id is empty");
    ...
    onlyAnapavo 
  {
    ...
    for (uint i = 0; i < _tokenIDs.length; i++) {
      ...
      require(tokenOwners[tokenId].owner == address(0), "AnapavoToken: only empty owner's token can be claimed");
      require(ownerOf(tokenId) == owner, "AnapavoToken: Anapavo does not own this token");
      _transfer(owner, _newOwner, tokenId);
  ...
  function getTokenOwnership(uint256 _tokenId) public view returns (address, string memory) {
    TokenOwner memory tokenOwner = tokenOwners[_tokenId];
    return (tokenOwner.owner, tokenOwner.userId);
  }
...
abstract contract AccessControl is EIP712MetaTransaction {
    ...
    modifier onlyCEO {
        require(msgSender() == owner, "AccessControl: caller is not CEO");
        _;
    }

    modifier isActive {
        require(!isMaintaining, "AccessControl: it's currently maintaining");
        _;
    }

    modifier onlyAnapavo() {
        require(
            msg.sender == owner || _authorizedAddressList[msg.sender] == true, 
            "AccessControl: caller is not one of anapavo accounts"
        );
        _;
    }
    ...
    modifier verifyPasscode(bytes32 _passcode) {
        require(
            _passcode == keccak256(bytes.concat(passcode, bytes20(address(msgSender())))), 
            "AccessControl: invalid passcode"
        );
        _;
    }
...
abstract contract EIP712MetaTransaction is EIP712Base {
  ...
  string internal constant DOMAIN_NAME = "anapavo.com";
