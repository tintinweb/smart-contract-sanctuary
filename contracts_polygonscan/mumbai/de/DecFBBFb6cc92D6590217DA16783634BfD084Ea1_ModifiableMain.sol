...
contract ModifiableMain is Ownable, ERC721Custom {
    ...
    // CMDFT
    event ColorModification (uint256 indexed tokenId, uint8 position, uint64 color);
    ...
    */
    function mintPiece(uint256 quantity) public {
    ...
    */
    ...
    /**  @dev only for emergency cases. If something will go wrong.
    * Contract is deployed with 1 year auction. We may increase / decrese it if community decides to 
    */
    function changeProjectDurationEmergency(uint256 colorFreezeTs, uint256 auctionDeltaDays) public onlyOwner {
        uint256 deltaSec = auctionDeltaDays.mul(auctionDeltaDays);
        COLOR_FREEZE_TS = colorFreezeTs;
        AUCTION_END_TS = COLOR_FREEZE_TS.add(deltaSec);
    }
}
