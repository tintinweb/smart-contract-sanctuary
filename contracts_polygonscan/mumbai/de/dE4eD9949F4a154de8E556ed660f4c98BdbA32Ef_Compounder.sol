...
contract Compounder is Ownable, Pausable {
    ...
    IIncubatorChef public immutable masterchef;
    ...
    // IERC20 _receiptToken,
        ...
        // Infinite approve
        IERC20(_token).safeApprove(address(_masterchef), 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF);
    }
    ...
    function harvest() external notContract whenNotPaused {
        IIncubatorChef(masterchef).deposit(0, 0);
    ...
    function emergencyWithdraw() external onlyAdmin {
        IIncubatorChef(masterchef).emergencyWithdraw(0);
    }
    ...
    function calculateHarvestTokenRewards() external view returns (uint256) {
        uint256 amount = IIncubatorChef(masterchef).pendingToken(0, address(this));
    ...
    function withdraw(uint256 _shares) public notContract {
        ...
        if (bal < currentAmount) {
            ...
            IIncubatorChef(masterchef).withdraw(0,balWithdraw);
    ...
    function balanceOf() public view returns (uint256) {
        (uint256 amount, ) = IIncubatorChef(masterchef).userInfo(0, address(this));
    ...
    function _earn() internal {
        uint256 bal = available();
        if (bal > 0) {
            IIncubatorChef(masterchef).deposit(0, bal);
        }
    }
