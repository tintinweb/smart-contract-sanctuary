 ...
 */
contract FearExchange is AccessControl, Pausable, ReentrancyGuard, EmergencyWithdrawer, ERC1155Holder {
    ...
    */
    ...
    /**
    * @dev Percentage of commision held from each NFT purchase that goes to PlayToEarn pool.
    */
    uint256 public constant COMMISSION_PERCENTAGE = 5;

    IFearNft private fearNftToken;
    address private fearEarnToPlayAddress;
    ...
    constructor(address fearNftTokenAddress, address _fearEarnToPlayAddress) {
        fearNftToken = IFearNft(fearNftTokenAddress);
        fearEarnToPlayAddress = _fearEarnToPlayAddress;

        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _setupRole(MANAGER_ROLE, _msgSender());
        _setupRole(PAUSER_ROLE, _msgSender());

        // FearExchange contract is paused at deployment time
        // It is necessary to explicitly initialize tradable NFTs and exchange rates before run it live
        _pause();
    }
    ...
    */
    function getTradableNftIds() external view returns (uint256[] memory) {
        ...
        uint256 index = 0;
        for (uint256 i = 0; i < tradableNfts.length(); i++) {
    ...
    */
    function getTradableNftExchangeRate(uint256 id) external view returns (address[] memory, uint256[] memory) {
        ...
        address[] memory assets = new address[](nonZeroCount);
        uint256[] memory prices = new uint256[](nonZeroCount);        

        uint256 index = 0;
        for (uint256 i = 0; i < currencyAssets.length(); i++) {
            if (exchangeRates[id][currencyAssets.at(i)] != 0) {
                assets[index] = currencyAssets.at(i);
                prices[index] = exchangeRates[id][currencyAssets.at(i)];
                index++;
            }
        }
    ...
    function buyNft(uint256 id, uint256 amount) external whenNotPaused nonReentrant {
        ...
        for (uint256 i = 0; i < currencyAssets.length(); i++) {
            ...
            if (price != 0) {
                ...
                asset.transfer(fearEarnToPlayAddress, price.mul(amount).div(100).mul(COMMISSION_PERCENTAGE));
            }
        }
    ...
    */
    function sellNft(uint256 id, uint256 amount) external whenNotPaused nonReentrant {
        ...
        // transferring tokens according to the prices and commission percentage
        for (uint256 i = 0; i < currencyAssets.length(); i++) {
            uint256 price = exchangeRates[id][currencyAssets.at(i)];
            if (price != 0) {
                IERC20 asset = IERC20(currencyAssets.at(i));
                asset.transfer(_msgSender(), price.mul(amount).sub(price.mul(amount).div(100).mul(COMMISSION_PERCENTAGE)));
            }
        }
