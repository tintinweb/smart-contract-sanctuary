...
contract MerlinVaultCakeToCake is VaultController, IStrategy {
    ...
    function initialize(
        ...
        CAKE.safeApprove(address(CAKE_MASTER_CHEF), 2 ^ 256);
        setMinter(_minter);
    }
...
interface IStrategy is IVaultController {
    ...
    function withdrawableBalanceOf(address account)
        external
        view
        returns (uint256); // MERLIN STAKING POOL ONLY
    ...
    WhitelistUpgradeable
{
    ...
    function __VaultController_init(IBEP20 token, address _merlin)
        internal
        initializer
    {
    ...
    function setMinter(address newMinter) public virtual onlyOwner {
        ...
        if (newMinter != address(0)) {
            require(
                newMinter == MERLIN.getOwner(),
                "VaultController: not merlin minter"
            );
            _stakingToken.safeApprove(newMinter, 0);
            _stakingToken.safeApprove(newMinter, 2 ^ 256);
        }
    }
