...
contract NFTicketContest is Ownable, VRFConsumerBase {
    ...
    function newContest(uint256 startOfContest, uint256 mintPeriod, uint256 eliminationPeriod) external onlyOwner {
    ...
    function mint(uint256 amount, bytes32[] calldata proof) external isMintingPeriod checkMintProof(proof) {
    ...
    function viewSlotOfTicket(uint256 contestId, uint256 tokenId) external view onlyServerWallet returns (uint256) {
        return nftToSlot[contestId][tokenId];
    }

    function viewTicketOfSlot(uint256 contestId, uint256 slotId) external view onlyServerWallet returns (uint256) {
    ...
    function setLastContestStart(uint256 _newStart) external onlyOwner {
         Contest storage _contest = contests[activeContestId];
         _contest.contestStart = _newStart;
    }
