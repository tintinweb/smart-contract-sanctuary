...
contract Serenade is Context, Pausable, AccessControlEnumerable {
  // ============ Constants ============
  ...
  /**
   * @dev Grants `DEFAULT_ADMIN_ROLE`, `CURATOR_ROLE`, `PAUSER_ROLE` to 
   * the account that deploys the contract. Sets the contract's URI. 
   */
  constructor() {
  ...
  /**
   * Returns an artist contract address
   */
  function artistAddress(uint256 artistId) 
    public view virtual returns(address) 
  {
    return address(_artists[artistId]);
  }

  /**
   * Returns an artist name
   */
  function artistName(uint256 artistId) 
    public view virtual returns(string memory) 
  {
    return _artists[artistId].name();
  }

  /**
   * Returns an artist symbol
   */
  function artistSymbol(uint256 artistId) 
    public view virtual returns(string memory) 
  {
    return _artists[artistId].symbol();
  }

  /**
   * Returns an artist contract uri
   */
  function artistURI(uint256 artistId) 
    public view virtual returns(string memory) 
  {
    return _artists[artistId].contractURI();
  }
  ...
  function createArtist(
    ...
    string memory uri
  ) public virtual whenNotPaused onlyRole(CURATOR_ROLE) {
    _artists[artistId] = new Artist(name, symbol, uri);
  }

  // ============ Pause Methods ============

  /**
   * @dev Pauses write methods
   */
