...
contract HopHopFoodNFTMarketPlace is ERC721URIStorage , Ownable {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds ;
    uint256 public cost = 3 ether;
    
    bool locked = false;
    uint256[] private tokensList;
    ...
    mapping(uint256 => SoldItem) private itemsSold;

    event NewSell(address buyer, uint256 tokenId);
    event PermanentURI(string _value, uint256 indexed _id);

    constructor()  ERC721 ("HopHopFood NFT Collection", "HHFNFT") {
        
    }

    function unlockPayment() public onlyOwner{
           locked = false;
    }
    ...
    function createToken(uint256 tokenId,string memory tokenURI) public onlyOwner {
    ...
    function buyNFT(uint256 tokenId) public payable {
        ...
        require(_exists(tokenId)==true,"Token doesn't exists");
    ...
    function fetchSoldItems() public view returns (SoldItem[] memory) {
        uint256 totalItemCount = tokensList.length;
        SoldItem[] memory items = new SoldItem[](totalItemCount);
        for (uint256 i = 0; i < totalItemCount; i++) {
                SoldItem storage currentItem = itemsSold[i];
                items[i] = currentItem;
    ...
    function fetchMyNFTs() public view returns (SoldItem[] memory) {
        uint256 totalItemCount = tokensList.length;
        uint256 itemCount = 0;
        uint256 currentIndex = 0;

        for (uint256 i = 0; i < totalItemCount; i++) {
            if (itemsSold[i].owner == msg.sender) {
                itemCount += 1;
            }
        }

        SoldItem[] memory items = new SoldItem[](itemCount);
        for (uint256 i = 0; i < totalItemCount; i++) {
            if (itemsSold[i].owner == msg.sender) {
                uint256 currentId = i ;
                SoldItem storage currentItem = itemsSold[currentId];
