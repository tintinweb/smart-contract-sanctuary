...
contract SimpleGiftBoxFactory is ISimpleGiftBoxFactory {
  ...
  function createSimpleGiftBox(address _recipient) external {
    IGiftBox box = new SimpleGiftBox(address(this), _recipient, msg.sender);
    registry.registerGiftBox(_recipient, address(box));
  }

  // @name Create AAVE Interest Yielding Simple Gift Box
  function createAIYSimpleGiftBox(address _recipient) external {
    IGiftBox box = new aiySimpleGiftBox(address(this), _recipient, msg.sender, wethGateway, lendingPoolAddressProvider);
    registry.registerGiftBox(_recipient, address(box));
  }
}
