...
contract MOONDOGSxSIDEQUEST is ERC721, Ownable {
    
    using SafeMath for uint256;

    string public MOONDOGS_PROVENANCE = ""; // DEV to add our hash, as well as the seed used to generate Moondogs

    uint256 public MOONDOGSPRICE = 15000000000000000; // 0.015 WETH (Polygon)

    uint public constant MAXMOONDOGSPURCHASE = 5;

    uint256 public constant MAX_MOONDOGS = 2500;
    ...
    // Reserve WEENIES for those that qualify
    uint public MOONDOGSRESERVE = 1000;

    constructor() ERC721("MOONDOGS SIDE QUEST", "WEENIES") { }
    ...
    function MINTMOONDOGS(uint numberOfTokens) public payable {
        ...
        require(numberOfTokens > 0 && numberOfTokens <= MAXMOONDOGSPURCHASE, "You can only mint 5 tokens at a time");
