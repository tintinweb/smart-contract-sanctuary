...
contract RebornPiecesTest is ERC721, ReentrancyGuard, Ownable {
    ...
    event Mint(uint256 indexed tokenId, address owner, bytes32 hash);
    ...
    function mint() external payable nonReentrant returns (uint256,bytes32) {
        /**======================1.AIRDROP========================**/
        ...
        /**======================2.WHITELIST======================**/
        if (iswhitelisted[msg.sender] && balanceOf(msg.sender) == 0) {
            return _mintSafe(msg.sender);
        }
        /**======================3.SALE===========================**/
    ...
    function _mintSafe(address _to) internal returns (uint256,bytes32) {
    ...
    function addWhitelistMul(address[] calldata _addrs) public onlyOwner {
        require((whiteList.length + _addrs.length) <= 10, "only 100");
        for (uint256 i = 0; i < _addrs.length; i++) {
            iswhitelisted[_addrs[i]] = true;
            whiteList.push(_addrs[i]);
            emit SetWhitelist(_addrs[i],1);
        }
    }
    ...
    //测试版专属 -上线移除
    function setEndTimeOnlyForTest(uint256 _time) external onlyOwner {
        endTime = _time;
    }  
}