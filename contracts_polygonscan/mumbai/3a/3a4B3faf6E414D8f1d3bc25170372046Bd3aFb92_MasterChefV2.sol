...
contract TamagoToken is BEP20('Tamago Swap', 'TMGO') {
    ...
    modifier antiWhale(
        ...
        if (maxTransferAmount() > 0) {
            if (_excludedFromAntiWhale[sender] == false && _excludedFromAntiWhale[recipient] == false) {
                require(amount <= maxTransferAmount(), "TMGO::antiWhale: Transfer amount exceeds the maxTransferAmount");
            }
        }
        _;
    }
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 10000, "TMGO::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
        require(
            _maxTransferAmountRate >= maxTransferAmountRateMinValue,
            "TMGO::updateMaxTransferAmountRate: Max transfer amount rate must be grater than min rate"
        );
