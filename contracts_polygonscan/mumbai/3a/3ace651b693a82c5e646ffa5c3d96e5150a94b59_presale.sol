...
abstract contract prices is Ownable{
    ...
    //cofres
    uint public _CommonChest=1;
    uint public _RareChest=2;
    uint public _EpicChest=3;

    //barcos
    uint public constant _ship_1=4;
    uint public constant _ship_2=5;
    uint public constant _ship_3=6;
    uint public constant _ship_4=7;
    uint public constant _ship_5=8;

    //islas
    uint public constant _isla=9;

    //objeros 
    uint public constant _common=10;
    uint public constant _rare=11;
    uint public constant _epic=12;
    uint public constant _mythical=13;
    uint public constant _legendary=14;
...
contract presale is prices {
    ...
    event buyNft(address indexed _buyer,uint _id,uint _mount);
...
function epicChest()public payable restrictions returns(bool){
    require(_msgValue()==prices.getItemPrecio(3),"mont incorrecto");
...
function islandChestChance(address _to,uint _a) internal {
        ...
        if(getBalanceNFT(12)>=1){
            _TransferNFT(_to,12,1);
        }else{
           islandEstructura(_to);
        }
        }else if(_a>=71 && _a<=90){
        if(getBalanceNFT(13)>=1){
            _TransferNFT(_to,13,1);
        }else{
           islandEstructura(_to);
        }
        }else if(_a>=91 && _a<=99){
            if(getBalanceNFT(14)>=1){
...
function islandEstructura(address _to) internal{
    if(getBalanceNFT(11)>=1){
        _TransferNFT(_to,11,1);
    }else if(getBalanceNFT(12)>=1){
        _TransferNFT(_to,12,1);
    }else if(getBalanceNFT(13)>=1){
        _TransferNFT(_to,13,1);
    }else if(getBalanceNFT(14)>=1){
        _TransferNFT(_to,14,1);
    }else{
        revert("no NFT available");
    }
