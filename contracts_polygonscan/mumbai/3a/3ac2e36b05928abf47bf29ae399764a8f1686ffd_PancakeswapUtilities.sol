...
library PancakeswapUtilities {
    ...
    function buyToken(address tokenToSpend, address tokenToBuy, address account, uint256 amountOut, IUniswapV2Router02 pancakeRouter) external returns(uint256, uint256) {
      ...
      (uint reservesA, uint reservesB) = getReservesOrdered(address(pair), tokenToSpend, tokenToBuy);
    ...
    function getReservesOrdered(address pair, address tokenFirst, address tokenSecond) public view returns(uint, uint) {
      (address token0,) = sortTokens(tokenFirst, tokenSecond);
      (uint tokenAReserve, uint tokenBReserve,) =  IUniswapV2Pair(pair).getReserves();
...
contract IndexPool is ERC20, Ownable, ReentrancyGuard {
    ...
    using PancakeswapUtilities for address;
    address public  _indexController;
    WBNB public  _WBNB;
    IUniswapV2Router02 public _pancakeRouter;
    IUniswapV2Factory public _pancakeFactory;
    ERC20 public immutable _BUSD;
    address[] public _underlyingTokens;
    uint16[] public _tokenWeights;
    uint16 constant public WEIGHT_FACTOR = 1000;
    uint8[] public _categories;
    ...
    */
    function buyIndexWith(uint amountOut, address paymentToken, uint amountInMax) external {
       ...
       // (, uint spent) = PancakeswapUtilities.buyToken(paymentToken, address(_WBNB), address(this), quote, _pancakeRouter);
        (, uint spent) = paymentToken.buyToken(address(_WBNB),address(this), quote, _pancakeRouter);
    ...
    */
    ...
    function getTokenQuote(address token, uint amount) public view returns (uint256) {
        ...
        (uint256 reserveBNB, uint256 reserveToken) =
             address(pair).getReservesOrdered(
                address(_WBNB),
                token
            );
