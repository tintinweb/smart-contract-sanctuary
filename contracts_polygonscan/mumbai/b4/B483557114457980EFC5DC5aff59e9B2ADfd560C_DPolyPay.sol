...
contract DPolyPay is PMGPermission {
    ...
    constructor() {
        ...
        //
        // Initialize data: addTokenSupport(_platFormSymbol, address(0x0));
        _tokensSupport[_platFormSymbol] = STokenSupport(address(0x0), 0);
        _tokenSupprtIndexes.push(_platFormSymbol);
        addMerchant("000");
    }
    ...
    function adjustFee(string memory merchant, string memory symbol, uint256 feeRate) public OnlyOwner {
        require(_ifMerchantExists(merchant) == false, "Err-adjustFee");
    ...
    function getContractAddress(string memory symbol) public view returns (address) {
        return _tokensSupport[symbol]._address;
    }
    ...
    function dealPay(
        ...
        emit EDealPay(merchant, symbol, moneyCount, orderId);
    }
    ...
    function dealSettle(
        ...
        require(total - amount >= 0, "Err-dealSettle-InsufficientBalance-2");

        if (_stringEQ(symbol, _platFormSymbol)) {
            payable(to).transfer(amount);
        } else {
            require(
                IERC20(_tokensSupport[symbol]._address).transfer(to, amount),
                "Err-dealSettle-Transfer"
            );
        }
        _merchantRecord[merchant][symbol].withdrawn += amount;
        emit EDealSettle(merchant, settleId, symbol, to, amount);
        _merchantRecord[merchant][symbol].total -= amount; 
    }
