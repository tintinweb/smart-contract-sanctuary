...
contract FloorTokenChild is ERC721, Ownable {
    ...
    function mint(address user, uint256 tokenId, bytes calldata metaData) external onlyPredicate {
		_mint(address(0), user, tokenId);
		this.setTokenMetadata(tokenId, metaData);
	}

    function setTokenMetadata(uint256 tokenId, bytes calldata data) external onlyPredicate {
        ...
        uint8 size = abi.decode(data, (uint8));
        sizes[tokenId] = size;
        emit FloorCreated(tokenId, size);
    }
}
