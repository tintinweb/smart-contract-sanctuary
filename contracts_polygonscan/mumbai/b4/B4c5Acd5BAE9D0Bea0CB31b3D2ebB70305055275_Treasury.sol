...
contract Treasury is Ownable, ITreasury {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SBRAVO }
    ...
    address public sBRAVO;
    uint public sBRAVOQueue; // Delays change to sBRAVO address
    ...
    constructor (
        ...
        isLiquidityToken[ _BRAVODAI ] = true;
        liquidityTokens.push( _BRAVODAI );
        bondCalculator[_BRAVODAI] = _bondCalculator;

        blocksNeededForQueue = _blocksNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for BRAVO
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external override returns ( uint send_ ) {
        ...
        // mint BRAVO needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( BRAVO ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn BRAVO for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external override {
        ...
        uint value = valueOfToken( _token, _amount );
        IBRAVOERC20( BRAVO ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sBRAVO ).balanceOf( msg.sender ); // Can only borrow against sBRAVO held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with BRAVO
        @param _amount uint
     */
    function repayDebtWithBRAVO( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IBRAVOERC20( BRAVO ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, BRAVO, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external override {
        ...
        IERC20Mintable( BRAVO ).mint( _recipient, _amount );
    ...
    function excessReserves() public override view returns ( uint ) {
        return totalReserves.sub( IERC20( BRAVO ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns BRAVO valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOfToken( address _token, uint _amount ) public view override returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match BRAVO decimals
            value_ = _amount.mul( 10 ** IERC20( BRAVO ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyOwner() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SBRAVO ) { // 9
            sBRAVOQueue = block.number.add( blocksNeededForQueue );
        } else return false;
