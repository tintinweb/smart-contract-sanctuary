...
import './interfaces/IDexTreasury.sol';
...
contract DexTreasury is Ownable, IDexTreasury {
    ...
    address public sDEX1;
    uint256 public sDEX1Queue; // Delays change to sDEX1 address
    ...
    constructor(
        ...
        isLiquidityToken[_DEX1DAI] = true;
        liquidityTokens.push(_DEX1DAI);
        bondCalculator[_DEX1DAI] = _bondCalculator;
        blocksNeededForQueue = _blocksNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for DEX1
        @param _amount uint
    ...
    function deposit(
        ...
        if (isReserveToken[_token]) {
            require(isReserveDepositor[msg.sender], 'Not Appv');
        } else {
            require(isLiquidityDepositor[msg.sender], 'Not Appv');
        }

        uint256 value = valueOfToken(_token, _amount);
        // mint DEX1 needed and store amount of rewards for distribution
        send_ = value.sub(_profit);
        IERC20Mintable(DEX1).mint(msg.sender, send_);
    ...
    /**
        @notice allow approved address to burn DEX1 for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw(uint256 _amount, address _token) external override {
        require(isReserveToken[_token], 'Not Accd'); // Only reserves can be used for redemptions
        require(isReserveSpender[msg.sender] == true, 'Not Appv');

        uint256 value = valueOfToken(_token, _amount);
        IDEX1ERC20(DEX1).burnFrom(msg.sender, value);
    ...
    function incurDebt(uint256 _amount, address _token) external {
        require(isDebtor[msg.sender], 'Not Appv');
        require(isReserveToken[_token], 'Not Accd');

        uint256 value = valueOfToken(_token, _amount);

        uint256 maximumDebt = IERC20(sDEX1).balanceOf(msg.sender); // Can only borrow against sDEX1 held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with DEX1
        @param _amount uint
     */
    function repayDebtWithDEX1(uint256 _amount) external {
        require(isDebtor[msg.sender], 'Not Appv');

        IDEX1ERC20(DEX1).burnFrom(msg.sender, _amount);

        debtorBalance[msg.sender] = debtorBalance[msg.sender].sub(_amount);
        totalDebt = totalDebt.sub(_amount);

        emit RepayDebt(msg.sender, DEX1, _amount, _amount);
    }
    ...
    function manage(address _token, uint256 _amount) external override {
        if (isLiquidityToken[_token]) {
            require(isLiquidityManager[msg.sender], 'Not Appv');
        } else {
            require(isReserveManager[msg.sender], 'Not Appv');
        }

        uint256 value = valueOfToken(_token, _amount);
        require(value <= excessReserves(), 'Insuff reserv');
    ...
    {
        // Not Approved
        require(isRewardManager[msg.sender], 'Not Appv');
        require(_amount <= excessReserves(), 'Insuff reserv');

        IERC20Mintable(DEX1).mint(_recipient, _amount);
    ...
    function excessReserves() public view override returns (uint256) {
        return totalReserves.sub(IERC20(DEX1).totalSupply().sub(totalDebt));
    }
    ...
    /**
        @notice returns DEX1 valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    ...
    {
        if (isReserveToken[_token]) {
            // convert amount to match DEX1 decimals
            value_ = _amount.mul(10**IERC20(DEX1).decimals()).div(
        ...
        returns (bool)
    {
        ...
        } else if (_managing == MANAGING.SDEX1) {
            // 9
            sDEX1Queue = block.number.add(blocksNeededForQueue);
        } else return false;
    ...
    function requirements(
        ...
        if (!status_[_address]) {
            // Must Queue
            require(queue_[_address] != 0, 'Must Q');
            require(queue_[_address] <= block.number, 'Q not expd');
            return true;
        }
        return false;
    }
