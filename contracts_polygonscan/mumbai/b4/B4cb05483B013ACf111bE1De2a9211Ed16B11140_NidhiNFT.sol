...
contract NidhiMarket is ReentrancyGuard {
    ...
    /**
      ...
      */
    function fetchMarketItems(uint page, uint pageSize)
        external view returns (MarketItemDetails[] memory items)
    {
        ...
        if (skip < itemCount) {
            ...
            if (remaining > 0) {
                ...
                NidhiCollection.Item memory current = _listedItems.tail();
                for (uint i = 0; i < skip; i++) {
                    current = _listedItems.get(current.prev);
                }
    ...
    /**
      ...
      */
    function fetchItemsByCreator(address creator, uint page, uint pageSize)
        external view returns (MarketItemDetails[] memory items)
    {
        ...
        if (creator != address(0) && address(createdItems) != address(0)) {
            ...
            if (skip < itemCount) {
                ...
                if (remaining > 0) {
                    uint take = remaining >= pageSize ? pageSize : remaining;
                    items = new MarketItemDetails[](take);
                    NidhiCollection.Item memory current = createdItems.tail();
                    for (uint i = 0; i < skip; i++) {
                        current = createdItems.get(current.prev);
                    }
                    for (uint i = 0; i < take; i++) {
                        items[i] = _getMarketItemDetails(_idToMarketItem[current.itemId]);
                        current = createdItems.get(current.prev);
    ...
    /**
      * @dev Returns a page of listed market items.
      */
    function fetchItemsByOwner(address owner, uint page, uint pageSize)
        public view returns (MarketItemDetails[] memory items)
    {
        ...
        if (address(collection) != address(0)) {
            ...
            uint skip = page * pageSize;
            uint skipped;
            NidhiCollection.Item memory current = collection.tail();
            for (uint i = 0; i < itemCount && skipped < skip; i++) {
                ...
                current = collection.get(current.prev);
            }
