...
contract QuasarTreasury is QuasarAccessControlled, ITreasury {
    /* ========== DEPENDENCIES ========== */
    ...
    function withdraw(uint256 _amount, address _token) external override {
        ...
        uint256 value = tokenValue(_token, _amount);
        QUAS.burnFrom(msg.sender, value);
    ...
    /**
     * @notice mint new QUAS using excess reserves
    ...
    function mint(address _recipient, uint256 _amount) external override {
        ...
        QUAS.mint(_recipient, _amount);
        emit Minted(msg.sender, _recipient, _amount);
    }

    /**
     * DEBT: The debt functions allow approved addresses to borrow treasury assets
     * or QUAS from the treasury, using sQUAS as collateral. This might allow an
     * sQUAS holder to provide QUAS liquidity without taking on the opportunity cost
    ...
    function incurDebt(uint256 _amount, address _token) external override {
        uint256 value;
        if (_token == address(QUAS)) {
            require(permissions[STATUS.QUASDEBTOR][msg.sender], notApproved);
            value = _amount;
        } else {
        ...
        sQUAS.changeDebt(value, msg.sender, true);
        require(sQUAS.debtBalances(msg.sender) <= debtLimit[msg.sender], "Treasury: exceeds limit");
        totalDebt = totalDebt.add(value);

        if (_token == address(QUAS)) {
            QUAS.mint(msg.sender, value);
            quasDebt = quasDebt.add(value);
        } else {
            totalReserves = totalReserves.sub(value);
            IERC20(_token).safeTransfer(msg.sender, _amount);
        }
    ...
    function repayDebtWithReserve(uint256 _amount, address _token) external override {
        ...
        IERC20(_token).safeTransferFrom(msg.sender, address(this), _amount);
        uint256 value = tokenValue(_token, _amount);
        sQUAS.changeDebt(value, msg.sender, false);
     ...
     * @param _amount uint256
     */
    function repayDebtWithQUAS(uint256 _amount) external {
        require(permissions[STATUS.RESERVEDEBTOR][msg.sender] || permissions[STATUS.QUASDEBTOR][msg.sender], notApproved);
        QUAS.burnFrom(msg.sender, _amount);
        sQUAS.changeDebt(_amount, msg.sender, false);
        totalDebt = totalDebt.sub(_amount);
        quasDebt = quasDebt.sub(_amount);
        emit RepayDebt(msg.sender, address(QUAS), _amount, _amount);
    }
    ...
    function enable(
        ...
        if (_status == STATUS.SQUAS) {
            sQUAS = IsQUAS(_address);
        } else {
    ...
    function execute(uint256 _index) external {
        ...
        if (info.managing == STATUS.SQUAS) {
            // 9
            sQUAS = IsQUAS(info.toPermit);
        } else {
    ...
    function excessReserves() public view override returns (uint256) {
        return totalReserves.sub(QUAS.totalSupply().sub(totalDebt));
    }
    ...
    function tokenValue(address _token, uint256 _amount) public view override returns (uint256 value_) {
        value_ = _amount.mul(10**IERC20Metadata(address(QUAS)).decimals()).div(10**IERC20Metadata(_token).decimals());
    ...
    function baseSupply() external view override returns (uint256) {
        return QUAS.totalSupply() - quasDebt;
    }
}
