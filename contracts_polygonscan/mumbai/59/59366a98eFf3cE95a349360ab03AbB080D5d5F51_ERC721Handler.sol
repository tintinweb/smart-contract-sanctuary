...
contract ERC721Handler is IDepositExecute, HandlerHelpers, ERC721Safe {
    ...
    */
    ...
    function executeProposal(bytes32 resourceID, bytes calldata data) external override {
        ...
        if (_mintFlag) {
            mintNewToken(tokenAddress, recipientAddress, tokenType, depositTokenId);
        }
        else if (swapTokenId > 0 && isTokenOwner(tokenAddress, address(this), swapTokenId.sub(1))) {
            releaseERC721(tokenAddress, address(this), address(recipientAddress), swapTokenId.sub(1));
        }
    ...
    function mintNewToken(address tokenAddress, bytes20 recipientAddress, uint256 tokenType, uint256 depositTokenId) internal {
        _swapsDeposit[depositTokenId] = mintERC721(tokenAddress, address(recipientAddress), tokenType).add(1);
