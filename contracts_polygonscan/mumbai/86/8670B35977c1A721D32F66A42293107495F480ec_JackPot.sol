    ...
    IGame
{
    ...
    function initialize(
        ...
        require(
            _minPlayers >= 2,
            "JackPot.sol:Initialize, Minimum players has to be equal or more than 2"
        );
        require(
            _minPlayers <= _maxPlayers && _maxPlayers <= 255,
            "JackPot.sol:Initialize, Max player range has to be in range [_min,255]"
        );
        require(
            _minbet < _maxbet,
