...
contract RhinoTest is ERC721Enumerable, Ownable {
    ...
    string public unrevealedURI = 'https://c83d-105-213-186-99.ngrok.io/fetchTestMetadata/';
    ...
    /* Constructor */
    constructor() ERC721("Rhino Test", "RHINO") {
    ...
    function whitelistMint(uint amount) external payable whitelistMintingOpen {
        ...
        // Check proper amount sent
        require(msg.value == amount * whitelistPrice, "You need more ETH");
    ...
    function _mintWithoutValidation(address to, uint amount) internal {
      ...
      if(msg.sender != recipient){
        require(amount <= maxItemsPerTx + 1, "Max mint amount is 6 Monkeys per Mint");
      } else {
        require(amount <= teamMintTotal + 1, "Max team mint amount is 200 Monkeys");
      }
    ...
    function whitelistUsers(address[] memory users, uint[] memory amounts) external onlyOwner {
      ...
      for (uint i = 0; i < users.length; i++) {
        require(amounts[i] > maxItemsPerWLUser, "The maximum per whitelist user is 4 Monkeys");
