...
contract Auction is
    ...
    */
    ...
    function executeLazyAuction(
        ...
        require(
            _bid.value <= stake[_bid.maker],
            "Bid is for higher value than staked"
        );
        ...
        require(
            _bid.value >= _tokenData.reserve,
            "Bid is lower than min set by the minter/creator"
        );
        ...
        // (bool success, ) = _auction.taker.call{value: _bid.value}("");

        // Split of money
        // 1. auctioneerFee + x(sum of all creator) + platform fee + minter fee = 10000
        // 2. minter fee
        // 3. platform fee
        // 4. x(sum of all creator)

        // Transfer required tokens to auction taker
        uint256 auctioneerTake = (value * auctioneerFee) / 100;
        uint256 minterTake = (value * _tokenData.minter.value) / 100;
        uint256 platformTake = (value * platformFee) / 100;

        stake[_auction.taker] += auctioneerTake;
        stake[_tokenData.minter.account] += minterTake;
        stake[owner()] += platformTake;
        // (bool successAuctionFee, ) = _auction.taker.call{value: auctioneerTake}("");
        // (bool successMinterFee, ) = minter.account.call{value: minterTake}("");
        // (bool successPlatformFee, ) = payable(owner()).call{value: platformTake}("");

        value = value - auctioneerTake - minterTake - platformTake;
        for (uint256 i = 0; i < _tokenData.payouts.length; i++) {
            stake[_tokenData.payouts[i].account] += (value * _tokenData.payouts[i].value) / 100;

            // (bool successCreatorPay, ) = payouts[i].account.call{value: value * payouts[i].value}("");
            // require(successCreatorPay, "Creator payment failed");
        }

        // require(success && successAuctionFee && successMinterFee && successPlatformFee, "Fee payments failed");
