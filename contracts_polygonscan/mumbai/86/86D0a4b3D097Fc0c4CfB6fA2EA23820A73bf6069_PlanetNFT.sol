...
contract PlanetNFT is ERC721Tradable {
    ...
    function getValueOfToken(uint256 tokenId) public view onlyAdmin() returns (PlanetDetail[] memory planets){
    ...
    function doMerge(
        ...
        require(ERC721.ownerOf(tokenIdRcvr) == _msgSender(), "ERC721: transfer of tokenIdRcvr that is not own");
        require(ERC721.ownerOf(tokenIdSndr) == _msgSender(), "ERC721: transfer of tokenIdSndr that is not own");

        require(_isApprovedOrOwner(_msgSender(), tokenIdSndr), "ERC721: transfer caller is not owner nor approved");
        ...
        emit Transfer(_msgSender(), address(0), deadTokenId);
