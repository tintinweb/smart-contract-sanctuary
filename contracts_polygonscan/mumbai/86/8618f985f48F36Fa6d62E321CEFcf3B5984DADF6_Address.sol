...
contract Sketchymails is ERC721Enumerable, Ownable {
  using Strings for uint256;

  string baseURI;
  uint256 cost=0.0 ether;// dont forget to change it later
  ...
  function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
      require(_exists(_tokenId),"ERC721Metadata: URI query for nonexistent token");
        mail memory currentmail = mails[_tokenId];
        if(bytes(currentmail.message).length > 0) {
            return _baseURI(); 
        }
        else {
  ...
  function setMessage(uint256 _tokenId, string memory _newMessage) public {
    ...
    mail storage currentmail = mails[_tokenId];
    currentmail.message = _newMessage;
  }

   function readMessage(uint256 _tokenId) public view returns (string memory message){
    require(msg.sender==owner());
    mail memory currentmail = mails[_tokenId];
    return currentmail.message;
  }
