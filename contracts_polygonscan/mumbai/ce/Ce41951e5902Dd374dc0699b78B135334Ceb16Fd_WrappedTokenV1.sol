...
contract Revocable is ERC20, RevokerRole {
    ...
    function _revoke(address _from, uint256 _amount) internal returns (bool) {
...
// File: contracts/WrappedTokenV1.sol

pragma solidity >=0.4.22 <0.9.0;

/**
 @title Wrapped Token V1
    ...
    RevocableToAddress
{
    ...
    AggregatorV3Interface internal _reserveFeed;
    ...
    function initialize(
        ...
        // NOTE: Reserve is checked and must exceed 'initialSupply' to avoid revert
        _updateOracleAddress(reserveAddress);
    }
    ...
    /**
    Public function to update the address of the code oracle, retricted to owner
     */
    function updateOracleAddress(address newAddress) public onlyOwner {
        _updateOracleAddress(newAddress);
    }
    ...
    */
    ...
    function _checkReserve(uint256 amount) internal view {
        (, int256 answer, , , ) = _reserveFeed.latestRoundData();

        require(
            (answer > 0) && (uint256(answer) > amount.add(ERC20.totalSupply())),
            "Reserve must exceed the total supply"
        );
    }

    function _checkOracle(AggregatorV3Interface reserveFeed)
        internal
        view
        returns (bool, string memory)
    {
        try reserveFeed.latestRoundData() returns (
            uint80,
            int256 answer,
            uint256,
            uint256,
            ...
            return
                (answer > 0) && (uint256(answer) > ERC20.totalSupply())
                    ? (true, "")
                    : (
                        false,
                        "Can not set to an oracle without sufficient reserve"
                    );
        } catch Error(string memory reason) {
            return (false, reason);
        } catch (bytes memory lowLevelData) {
            return (false, string(lowLevelData));
        }
    }

    function _updateOracleAddress(address newAddress) internal {
        _reserveFeed = AggregatorV3Interface(newAddress);
        (bool success, string memory error) = _checkOracle(_reserveFeed);
