...
contract DockSpeedSprintSession is Ownable{
    ...
    IERC20 public Speed;
    IDockSpeed public DockSpeedNFT;
    ...
    constructor(IERC20 _Speed,IDockSpeed _NFT){
        Speed =_Speed;
        DockSpeedNFT = _NFT;
    }
    ...
    function playerFinished(uint256 _sessionId,address racer,uint32 raceTime)public onlyOwner{
        IsplayerFinishedASession[_sessionId][racer] = true;
        uint256 index = playerIndexInASession[_sessionId][racer];
        SessionDictionary[_sessionId].raceTime[index] = raceTime;

        emit PlayerSession(racer, _sessionId , raceTime);
    }
    ...
    function getNftsFromWallet(address _user)public view returns(string[] memory _CarDatas){
        return DockSpeedNFT.getNftsFromWallet(_user);
    }

    function getLiveSessions()public view returns(uint256[] memory _LiveSessionList){
        uint256 index;
        uint256[] memory LiveList;
        for(uint256 i = 0;i < totalSessions;i++){
            if(SessionDictionary[i].live){
                LiveList[index] = i;
                index++;
            }
        }
