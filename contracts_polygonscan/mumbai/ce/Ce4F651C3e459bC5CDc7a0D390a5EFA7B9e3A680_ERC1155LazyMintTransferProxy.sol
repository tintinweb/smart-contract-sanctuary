...
import "@orderinbox/mint/contracts/erc-1155/LibERC1155Mint.sol";
import "@orderinbox/mint/contracts/erc-1155/IERC1155Mint.sol";
...
contract ERC1155LazyMintTransferProxy is OperatorRole, ITransferProxy {
    function transfer(LibAsset.Asset memory asset, address from, address to) override onlyOperator external {
        (address token, LibERC1155Mint.Mint1155Data memory data) = abi.decode(asset.assetType.data, (address, LibERC1155Mint.Mint1155Data));
        IERC1155Mint(token).transferFromOrMint(data, from, to, asset.value);
    }
}
...
import "./LibERC1155Mint.sol";
import "@orderinbox/royalties/contracts/LibPart.sol";


interface IERC1155Mint is IERC1155Upgradeable {
...
/*    event Creators(
    ...
    function mintAndTransfer(
        LibERC1155Mint.Mint1155Data memory data,
        address to,
        uint256 _amount
    ) external; */
