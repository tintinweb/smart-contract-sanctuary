...
contract Distributions_v0 is Initializable, Ownable {
    ...
    function claim(
        ...
        require(
            dr.availablePoints > 0,
            "Distributions: no points to claim in this round"
        );
        require(dr.totalKarma > 0, "Distributions: this round has no karma");

        uint256 userPoints = dr.availablePoints.mul(karma).div(dr.totalKarma);

        require(
            userPoints > 0,
            "Distributions: user karma is too low to claim points"
        );

        emit ClaimPoints(round, account, userPoints);
        ISubredditPoints(subredditPoints).mint(
            address(this),
            account,
            userPoints,
            "",
    ...
    function advanceToRound(
        uint256 round,
        uint256 _roundPoints,
        uint256 _totalKarma
    ) external {
