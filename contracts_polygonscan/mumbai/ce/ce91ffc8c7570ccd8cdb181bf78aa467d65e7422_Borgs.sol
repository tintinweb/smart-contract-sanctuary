...
contract Borgs is ERC721Enumerable, ERC721Holder, CommonObjects, NumberUtils{
    ...
    function getBorgAttribute(string memory borgAttributeName) public view onlyOwner returns (bytes32[] memory imagePixals){
        BorgAttribute storage borgAttribute = _borgAttributes[borgAttributeName];
        
        imagePixals = new bytes32[](IMAGE_SIZE);
        
        for(uint256 i = 0;i<borgAttribute.colors.length;i++){
            bytes8 color = borgAttribute.colors[i];
    ...
    function getListingByTokenId(uint256 tokenId) public view returns(uint256 borgId, uint256 price, address seller, address buyer, uint256 timestamp, bytes8[] memory image){
        // Get listing
    ...
    function getListingByIndex(uint256 index) public view returns(uint256 tokenId, uint256 price, address seller, address buyer, uint256 timestamp, bytes8[] memory image){
        // Get tokenId
    ...
    function getOwnersListingByIndex(address owner, uint256 index) public view returns(uint256 tokenId, uint256 price, address seller, address buyer, uint256 timestamp, bytes8[] memory image){
        // Get tokenId
    ...
    function _combineBorgAttributes(string[] memory borgAttributeNames) internal view returns (bytes8[] memory hexPixals){
        // Init the 
