    ...
    ERC721Pausable
{
        ...
        whenNotPaused
    {
        require(
            ...
            "UninterestedUnicorns: Quantity must be lesser than MAX_SUPPLY = 800"
        );
    ...
    function mintToMany(
        ...
        require(
            totalSupply().add(_toList.length) <= MAX_SUPPLY,
            "UninterestedUnicorns: Quantity must be lesser than MAX_SUPPLY = 800"
        );
        require(
            _toList.length == _tokenURIList.length,
            "UninterestedUnicorns: Input Error"
        );

        for (uint256 i = 0; i < _toList.length; i++) {
            mintTo(_toList[i], _tokenURIList[i]);
        }
    }
