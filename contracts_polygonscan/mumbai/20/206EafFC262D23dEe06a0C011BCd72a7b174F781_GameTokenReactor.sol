...
contract GameTokenReactor {
	...
	mapping(address => bool) public isGameTokenAdded;
	...
	modifier onlyAddedGameToken(address _gameToken) {
		require(isGameTokenAdded[_gameToken] == true, "GameTokenReactor: Only added game token");
		_;
	}
	...
	constructor(address _DAO) {
		require(_DAO != address(0), "GameTokenReactor: set DAO to ZERO address");
	...
	) public onlyDAO returns (address) {
		require(isGameTokenAdded[gameToken_] == false, "GameTokenReactor: Already Existed !");
	 ...
	 */
	function reactorEncoder(address gameToken_, uint256 amount_) public lock onlyWhitelisted onlyAddedGameToken(gameToken_) {
	 ...
	 */
	function reactorDecoder(address partnerToken_, uint256 amount_) public lock onlyWhitelisted onlyPartnerTokenExist(partnerToken_) {
