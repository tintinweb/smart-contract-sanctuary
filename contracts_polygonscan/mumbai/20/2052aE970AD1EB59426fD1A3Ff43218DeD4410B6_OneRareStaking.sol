...
contract OneRareStaking is Ownable, TokensRecoverable, ReentrancyGuard, VRFConsumerBase{
    ...
    event Claimed(address winnerAddress, bytes32 requestId, uint256[] tokenIdsWon, uint256[] tokenIdsWonQty);
...
*/
    ...
    function stake(uint256 _amount) external nonReentrant returns (bool){
        ...
        if(len>0)
            _tickets = _tickets.add(ticketsArray[len-1]);
   ...
   function withdraw() external nonReentrant returns (bool){
    ...
    function binarySearch(uint[] memory arr, uint l, uint r, uint x) internal returns(uint)
    {
        if (r >= l) {
            uint mid = l + (r - l) / 2;
    ...
    function findWinners(bytes32 requestId) public onlyAdmin returns (bool){
        ...
        if(!winnersAnnounced[requestId] &&  block.timestamp>=lockTimestamp){
            ...
            for(uint i=0;i<ids.length;i++){
                ...
                uint ticketArrIndex = binarySearch(ticketsArray,0,ticketsArray.length-1,winnerTicket);
                address winnerAddress = ticketMapping[ticketsArray[ticketArrIndex]];

                // can be further optimised
                // for(uint j=0;j<ticketsArray.length;j++){
                //     // find mapping for address
                //     if(ticketsArray[i]!=ticketsArray.length || ticketsArray[i]>winnerTicket)
                //         winnerAddress = ticketMapping[ticketsArray[i]];
                // }
