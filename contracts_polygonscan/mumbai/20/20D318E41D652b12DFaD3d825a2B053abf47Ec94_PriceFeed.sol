...
/*
* PriceFeed for mainnet deployment 
* 1. Chainlink's MATIC:EUR price computed as MATIC:USD / EUR:USD
* 2. Band gets MATIC:EUR price directly
*/
...
contract PriceFeed is Ownable, CheckContract, BaseMath, IPriceFeed {
    ...
    AggregatorV3Interface public maticPriceAggregator;  // Chainlink aggregator for MATIC:USD
    AggregatorV3Interface public eurPriceAggregator;  // Chainlink aggregator for EUR:USD
    IStdReference public bandCaller; // Mainnet band caller
    // Use to convert a price answer to an 18-digit precision uint
    uint8 constant public TARGET_DIGITS = 18; 
    uint8 constant public TARGET_EUR_DIGITS = 10; 
    uint8 constant public TARGET_MATIC_DIGITS = TARGET_DIGITS + TARGET_EUR_DIGITS; 

    // Maximum time period allowed since Chainlink's latest round data timestamp, beyond which Chainlink is considered frozen.
    uint constant public MATIC_TIMEOUT = 14400;  // 4 hours: 60 * 60 * 4
    uint constant public EUR_TIMEOUT = 86400;  // 24 hours: 60 * 60 * 24
    
    // Maximum deviation allowed between two consecutive Chainlink oracle prices. 18-digit precision.
    uint constant public MAX_PRICE_DEVIATION_FROM_PREVIOUS_ROUND =  5e17; // 50%
    uint constant public MAX_PRICE_DIFFERENCE_BETWEEN_ORACLES = 5e16; // 5%
    // The last good price seen from an oracles
    ...
    struct ChainlinkProcessedResponse {
        uint256 rate;
        uint8 decimals;
        bool isBroken;
        bool isFrozen;
        bool isPriceDevaitionAboveMax;
    }
    ...
    function setAddresses(
        address _maticPriceAggregatorAddress,
        address _eurPriceAggregatorAddress,
        ...
        onlyOwner
    {
        ...
        checkContract(_maticPriceAggregatorAddress);
        checkContract(_eurPriceAggregatorAddress);
        checkContract(_bandCallerAddress);        
       
        maticPriceAggregator = AggregatorV3Interface(_maticPriceAggregatorAddress);
        eurPriceAggregator = AggregatorV3Interface(_eurPriceAggregatorAddress);
        ...
        ChainlinkProcessedResponse memory chainlinkProcessedResponse = _getChainlinkMaticEurPair();
        require(
            !chainlinkProcessedResponse.isBroken && 
            !chainlinkProcessedResponse.isFrozen && 
            !chainlinkProcessedResponse.isPriceDevaitionAboveMax,
            "Something wrong with Chainlink contracts"
        );

        _storePrice(chainlinkProcessedResponse.rate);
    ...
    /*
    * fetchPrice():
    * Returns the latest price obtained from the Oracle.
    *
    ...
    */
    function fetchPrice() external override returns (uint) {
        ...
        if (status == Status.chainlinkWorking) {
            ...
            if (chainlinkProcessedResponse.isBroken) {
                ...
                */
                ...
                _changeStatus(Status.usingBandChainlinkUntrusted);
                return _storePrice(bandResponse.rate);
            }

            // If Chainlink is frozen, try Band
            if (chainlinkProcessedResponse.isFrozen) {
            ...
            if (chainlinkProcessedResponse.isPriceDevaitionAboveMax) {
                ...
                */
                if (_bothOraclesSimilarPrice(chainlinkProcessedResponse, bandResponse.rate)) {
                    return _storePrice(chainlinkProcessedResponse.rate);
                }
        ...
        if (status == Status.usingBandChainlinkUntrusted) {
            ...
            if (_bothOraclesLiveAndUnbrokenAndSimilarPrice(chainlinkProcessedResponse, bandResponse)) {
    ...
    */
    ...
    function _chainlinkIsFrozen(ChainlinkResponse memory _response, uint _timeout) internal view returns (bool) {
        return block.timestamp.sub(_response.timestamp) > _timeout;
    }

    function _chainlinkPriceChangeAboveMax(ChainlinkResponse memory _currentResponse, ChainlinkResponse memory _prevResponse) internal pure returns (bool) {
        uint currentScaledPrice = _scaleChainlinkPriceByDigits(uint256(_currentResponse.answer), _currentResponse.decimals, TARGET_DIGITS);
        uint prevScaledPrice = _scaleChainlinkPriceByDigits(uint256(_prevResponse.answer), _prevResponse.decimals, TARGET_DIGITS);
   ...
   function _bothOraclesLiveAndUnbrokenAndSimilarPrice
    (
        ChainlinkProcessedResponse memory _chainlinkProcessedResponse,
    ...
    returns (bool)
    {
        ...
        (
            ...
            _chainlinkProcessedResponse.isBroken ||
            _chainlinkProcessedResponse.isFrozen
        )
        {
            return false;
        }

        return _bothOraclesSimilarPrice(_chainlinkProcessedResponse, _bandResponse.rate);
    }
    ...
    //    }
    function _bothOraclesSimilarPrice(ChainlinkProcessedResponse memory _chainlinkProcessedResponse, uint256 _bandPrice) internal pure returns (bool) {
        // Get the relative price difference between the oracles. Use the lower price as the denominator, i.e. the reference for the calculation.
        uint minPrice = LiquityMath._min(_bandPrice, _chainlinkProcessedResponse.rate);
        uint maxPrice = LiquityMath._max(_bandPrice, _chainlinkProcessedResponse.rate);
        uint percentPriceDifference = maxPrice.sub(minPrice).mul(DECIMAL_PRECISION).div(minPrice);
    ...
    function _scaleChainlinkPriceByDigits(uint _price, uint _answerDigits, uint8 _target_digits) internal pure returns (uint) {
        /*
        ...
        */
        uint price;
        if (_answerDigits >= _target_digits) {
            // Scale the returned price value down to Liquity's target precision
            price = _price.div(10 ** (_answerDigits - _target_digits));
        }
        else if (_answerDigits < _target_digits) {
            // Scale the returned price value up to Liquity's target precision
            price = _price.mul(10 ** (_target_digits - _answerDigits));
        }
        return price;
    }

    function _storePrice(uint _currentPrice) internal returns (uint){
        // price must have 18 decimals
        if (lastGoodPrice != _currentPrice) {  //upd only if changes
    ...
    function _getChainlinkMaticEurPair() internal view returns (ChainlinkProcessedResponse memory response)
    {   
        ChainlinkProcessedResponse memory maticResponse = _getChainlinkProcessedResponse(Pair.matic_usd);
        ChainlinkProcessedResponse memory eurResponse = _getChainlinkProcessedResponse(Pair.eur_usd);

        response.isBroken = maticResponse.isBroken || eurResponse.isBroken;
        if(response.isBroken){
            return response;
        }

        response.isFrozen = maticResponse.isFrozen || eurResponse.isFrozen;
        if(response.isFrozen){
            return response;
        }        

        uint maticPriceD28 = _scaleChainlinkPriceByDigits(maticResponse.rate, maticResponse.decimals, TARGET_MATIC_DIGITS);
        uint eurPriceD10 = _scaleChainlinkPriceByDigits(eurResponse.rate, eurResponse.decimals, TARGET_EUR_DIGITS);
        response.decimals = TARGET_DIGITS;
        response.rate = maticPriceD28.div(eurPriceD10);
        return response;
    }

    function _getChainlinkProcessedResponse(Pair _pair) internal view returns (ChainlinkProcessedResponse memory response)
    {   
        uint _timeout;
        AggregatorV3Interface _priceAggregator;

        if(_pair == Pair.matic_usd){
            _priceAggregator = maticPriceAggregator;
            _timeout = MATIC_TIMEOUT;
        } else {
            _priceAggregator = eurPriceAggregator;
            _timeout = EUR_TIMEOUT;
        }

        // Collect current and prev responses
        ...
        response.isBroken = _chainlinkIsBroken(chainlinkResponse, prevChainlinkResponse);
        if(response.isBroken){
            return response;
        }

        response.rate = uint256(chainlinkResponse.answer);
        response.decimals = chainlinkResponse.decimals;
        response.isFrozen = _chainlinkIsFrozen(chainlinkResponse, _timeout);
        response.isPriceDevaitionAboveMax = _chainlinkPriceChangeAboveMax(chainlinkResponse, prevChainlinkResponse);

        return response;
    }

    function _getCurrentBandResponse() internal view returns (IStdReference.ReferenceData memory bandResponse){
        try bandCaller.getReferenceData("MATIC", "EUR") returns
        (
    ...
    function _getPrevChainlinkResponse(AggregatorV3Interface _priceAggregator, uint80 _currentRoundId, uint8 _currentDecimals) 
    ...
    function setBandCallerAddress(address _bandCallerAddress) external {
        ...
        require(_bothOraclesSimilarPrice(chainlinkProcessedResponse, bandResponse.rate),
