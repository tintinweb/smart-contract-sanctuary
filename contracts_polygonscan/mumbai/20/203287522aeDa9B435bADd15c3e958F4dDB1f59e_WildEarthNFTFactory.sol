...
pragma solidity >=0.8.0 <0.9.12;

import "./WildEarthNFT.sol";
import "@openzeppelin/contracts/access/Ownable.sol";
import "hardhat/console.sol";

contract WildEarthNFTFactory is Ownable {
    address[] public nfts;
...
contract WildEarthNFT is Ownable, HasSecondarySaleFees, ERC721, ContextMixin {
    ...
    event mintSaleFeeUpdated(uint256 newAmount);
    ...
    constructor(
        ...
        mintSaleFee = _mintSaleFee;
    }

    modifier canMintNft() {
        require(
            owner() == msg.sender || msg.value == mintSaleFee,
            "Mint: only owner or exact buyer"
        );
        _;
    }
        ...
        returns (uint256)
    {
        ...
        require(mintable, "Minting has been frozen");
    ...
    function toggleMintable(bool _mintable) external onlyOwner {
        mintable = _mintable;
        emit Mintable(_mintable);
    }
    ...
    function updateMintSaleFee(uint256 newMintSaleFeeAmount)
        external
        onlyOwner
    {
        mintSaleFee = newMintSaleFeeAmount;
        emit mintSaleFeeUpdated(mintSaleFee);
    }
