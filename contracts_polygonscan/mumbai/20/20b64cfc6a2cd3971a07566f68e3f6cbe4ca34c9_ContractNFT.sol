...
contract ContractNFT is ERC721, ERC721Enumerable, Ownable {
    ...
    uint32 public maxNFTMintingForBulk = 500; // No more than 500 minting
    ...
    function getNextNftMintedNumber()
        public
        view
        returns (uint256)
    {
        ...
        returns (string memory)
    {
        ...
        NFT memory currentNFT = allNFTs[tokenId];
        return string(abi.encodePacked("ipfs://", currentNFT.metadataHash));
    }
    ...
    function mint(uint8[12] memory values, string memory metadataHash)
        public
        payable
    {
        require(
        ...
        allNFTs.push(NFT(values, metadataHash));
        uint256 id = allNFTs.length - 1;
        _mint(msg.sender, id);
    }

    function bulkMint(uint8[12][] memory NFTIndices, string[] memory hashes)
        public
        payable
    {
        ...
        require(
            hashes.length == NumberNFTsToMint,
            "Arrays are not the same size. Can't mint anything !"
        );
        require(
        ...
        for (uint16 index = 0; index < NumberNFTsToMint; index++) {
            NFT memory toMint = NFT(NFTIndices[index], hashes[index]);
            allNFTs.push(toMint);
            uint256 id = allNFTs.length - 1;
            _mint(msg.sender, id);
        }
