...
contract LedgerConnect4 is Ownable {
    ...
    event NewPlayer(address indexed player1);
    ...
    /// @dev Save new player
    /// @param _add The player address
    function _savePlayer(address _add) private {
        bool isPlayerAlreadyExists = false;

        for (uint256 i = 0; i < players.length; i++) {
            if (players[i].username == _add) {
        ...
        if (isPlayerAlreadyExists == false) {
            ...
            newPlayer.username = _add;
            newPlayer.amountWon = 0;
    ...
    function _updatePlayerAmount(address _add, uint256 _amountWon) private {
        uint256 playerIndex;
        bool isPlayerAlreadyExists = false;
        ...
        require(
            isPlayerAlreadyExists == true,
            "Can't add amount to an unknown player"
        );
        players[playerIndex].amountWon += _amountWon;
        emit UpdatePlayerAmount(
            _add,
            _amountWon,
            players[playerIndex].amountWon
        );
    }
        ...
        isGameActive(_gameId)
    {
        ...
        if (game.isPlayer1Next) {
            game.p1Amount += payAmount;
        } else {
            game.p2Amount += payAmount;
        }
    ...
    function getLeaderboard() public view returns (address[], uint256[]) {
