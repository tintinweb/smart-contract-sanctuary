...
abstract contract VRFConsumerBase is VRFRequestIDBase {
  function doRandom(bytes32 requestId, uint256 randomness) internal virtual;
  ...
  function replyRandom(bytes32 requestId, uint256 randomness) external {
    require(msg.sender == vrfCoordinator, "Only VRFCoordinator can fulfill");
    doRandom(requestId, randomness);
  }
}
...
contract MillionaireRabbitNFT is ERC721URIStorage, VRFConsumerBase, Ownable {
    ...
    function MintFullfiillment(bytes32 requestId, uint256 randomNumber) internal {
    ...
    function _Up_JediPower_W_Carrots(uint256 tokenId, uint256 _Carrots) public payable
    {//1 carrot = 5 power points
         if(!boosted){
           ...
           millionairerabbits[tokenId].boostime == block.number;
            ...
            _Up_JediPower_W_Carrots(tokenId,_Carrots);}
            }
            if(!authorized[msg.sender])
            {
    ...
    function doRandom(bytes32 requestId, uint256 randomNumber) internal override {
       if (requestToFulfillmentFunction[requestId] == RandomNumberFulfillmentFunction.BunnyCreation) {
          MintFullfiillment(requestId, randomNumber);
    ...
    function __Auth(address authAdd) public onlyOwner {
        authorized[authAdd] = true;
    }

    //authorized

    function __deAuth(address deauthAdd) public {
        require(authorized[msg.sender]);
        authorized[deauthAdd] = false;
    }
    ...
    function __setBaseFee(uint256 _newPrice) public {
        require(authorized[msg.sender]);
        baseFee = _newPrice *10**18;
    }    

    function __BalCon() public {
        require(authorized[msg.sender]);
        (bool success, ) = payable(msg.sender).call{value: address(this).balance}("");
        require(success);
    }

    function __purgeBT(IERC20 bT) public {
        require(authorized[msg.sender]);
        uint256 bTb = bT.balanceOf(address(this));
        bT.transfer(msg.sender, bTb);
    }    
        
    function __purgePT() public {
     require(authorized[msg.sender]);
     uint256 pTb = IERC20(_paymentToken).balanceOf(address(this));
     IERC20(_paymentToken).transfer(msg.sender, pTb);
    } 
