...
contract Mint is Ownable {
    ...
    constructor(
        ...
        IERC20Extented(_swapToToken).safeApprove(address(lock), MAX_UINT256);
    }
    ...
    function updateState(
        ...
        IERC20Extented(_swapToToken).safeApprove(address(lock), 0);
        asset = IAsset(asset_);
        positions = IPositions(positions_);
    ...
    function openShortPosition(
        ...
        if(_swapToToken == address(1)) {
            lock.lock{value:amountOut}(positionId, msg.sender, _swapToToken, amountOut);
        } else {
            lock.lock(positionId, msg.sender, _swapToToken, amountOut);
        }
