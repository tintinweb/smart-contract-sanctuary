...
contract Info is InfoBase {
    ...
    */
    function currentSettledOwnershipRoot() public view returns (bytes32) {
        (bool isReady, uint8 actualLevel) = isOwnershipRootSettled();
        return
            isReady
    ...
    /**
     * @dev Returns:
     *      (bool) is the currently submitted ownership root settled
     *      (uint8) the challenge level at which it settled
     */
    function isOwnershipRootSettled()
        public
        view
        returns (bool isSettled, uint8 actualLevel)
    {
        (isSettled, actualLevel) = isOwnershipRootSettledPure(
    ...
    {
        bool isSettled;
        (isSettled, actualLevel) = isOwnershipRootSettled();
        isReady =
            isSettled &&
            (plannedTime(
...
contract ChallengeLibStatus {
     ...
     */
    function isOwnershipRootSettledPure(
        uint256 nTXs,
    ...
    ) public pure returns (bool isSettled, uint8 actualLevel) {
        ...
        isSettled = isSettled && isOwnershipMoreRecent;
    }
