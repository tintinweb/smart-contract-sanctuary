...
contract WeirdAssFungis is ERC721Enumerable, Ownable {
    ...
    mapping(uint256 => FUNGI) public idToType;

    constructor() ERC721("Weird ass Fungis", "WAS") {}
    ...
    function getType(uint256 tokenId) external view returns (FUNGI) {
        return idToType[tokenId];
    }

    // ------------- User Api -------------

    function mint(FUNGI _type)
        external
        payable
        onlyWhenActive
        onlyPaid
        onlyHuman
    {
        _mintFor(msg.sender, _type);
    }

    // ------------- Internal -------------

    function _mintFor(address _to, FUNGI _type) internal {
        require(totalSupply() < MAX_SUPPLY, "No supply left");
        idToType[totalSupply()] = _type;
