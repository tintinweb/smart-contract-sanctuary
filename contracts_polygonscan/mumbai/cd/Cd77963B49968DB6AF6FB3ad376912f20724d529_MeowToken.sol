...
contract MeowToken is ERC20("MeowToken", "MEOW"), Ownable {
  ...
  function manualMint(address _to, uint256 _amount) public onlyOwner {
    ...
    uint256 miningMinted = totalSupply().sub(reserve); // MeowMining + DevFund.
    ...
    uint256 canManualMint = (uint256(200000000e18).sub((miningMinted).sub(devFund))).div(5); // 20% of (MeowMining - DevFund)
    require(manualMinted.add(_amount) <= (canManualMint), "MeowToken::manualMint:: manual mint limit exceeded.");
    manualMintAllowedAfter = manualMintAllowedAfter.add(minimumTimeBetweenManualMint);
