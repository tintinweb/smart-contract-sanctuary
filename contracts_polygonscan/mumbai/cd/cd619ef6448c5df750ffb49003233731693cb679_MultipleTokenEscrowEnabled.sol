...
/** 
 *  SourceUnit: /media/aniket/home/aniket/Work/nonceblox-work/nervora/contracts/escrow/contracts/MultipleTokenEscrowEnabled.sol
*/
...
////import "./BasicRelayRecipient.sol";
    ...
    BaseRelayRecipient
{
    ...
    */
    modifier onlyAdmin() {
        require(
            ...
            "MultipleTokenEscrowEnabled: Not admin."
        );
        _;
    }
    ...
    */
    ...
    /**
    @notice - fetch balances for admin.
    */
    function getBalanceAdmin(address addr, address tokenAddr)
        public
        view
        returns (Wallet memory wallet)
    {
        return balances[addr][tokenAddr];
    }
    ...
    /**
    @dev New Balance - Old Balance  is for tokens with fee-enabled contracts.
    */
    function deposit(uint256 amount, address tokenAddr) external {
        uint256 old_balance = ERC20(tokenAddr).balanceOf(address(this));

        ERC20(tokenAddr).transferFrom(_msgSender(), address(this), amount);

        uint256 new_balance = ERC20(tokenAddr).balanceOf(address(this));

        uint256 updated_balance = new_balance - old_balance;

        balances[_msgSender()][tokenAddr].totalBalance += updated_balance;
        emit Deposit(_msgSender(), updated_balance, tokenAddr);
    }

    /**
    @dev default currency deposit
    */
    function deposit_default() external payable {
    ...
    */
    ...
    function exit(uint256 amount, address tokenAddr) external onlyOwner {
        ...
        balances[owner()][tokenAddr].totalBalance -= amount;
        balances[owner()][tokenAddr].reservedBalance = balances[owner()][
            tokenAddr
        ]
        .totalBalance;
    ...
    function exitDefault(uint256 amount) external onlyOwner {
        ...
        defaultBalances[owner()].totalBalance -= amount;
        defaultBalances[owner()].reservedBalance = defaultBalances[owner()]
        .totalBalance;
    ...
    function withdrawDefault(uint256 amount) external {
        require(
            amount <=
    ...
    function setupRoles(address _owner) internal {
        ...
        _setupRole(SYSTEM_ROLE, _owner);
        _setupRole(OWNER_ROLE, _owner);

        _setRoleAdmin(SYSTEM_ROLE, OWNER_ROLE);
