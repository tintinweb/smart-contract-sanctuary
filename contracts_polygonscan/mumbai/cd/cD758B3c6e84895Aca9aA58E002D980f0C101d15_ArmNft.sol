    ...
    Ownable
{
    ...
    function mintBatch(address[] calldata users, uint256[] calldata tokenIds) public onlyOwner {

        uint256 length = tokenIds.length;
        require(length <= BATCH_LIMIT, "ChildMintableERC721: EXCEEDS_BATCH_LIMIT");
        require(users.length == tokenIds.length,"ERROR: wrong length");
        
        // Iteratively mint ERC721 tokens, for performing
        // batch mint
        for (uint256 i; i < length; i++) {
