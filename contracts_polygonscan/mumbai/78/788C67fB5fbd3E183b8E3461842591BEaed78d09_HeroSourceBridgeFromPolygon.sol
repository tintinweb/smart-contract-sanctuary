...
contract HeroSourceBridgeFromPolygon is FxBaseChildTunnel {
    ...
    event Withdrawing(address indexed to, uint256 indexed tokenId, uint256 blockNumber);
    ...
    function withdraw(uint256 tokenId) external {
        ...
        emit Withdrawing(msg.sender, tokenId, block.number);

        // send message to root regarding token burn
        _sendMessageToRoot(abi.encode(WITHDRAW_SINGLE, abi.encode(msg.sender, tokenId, block.number)));
    }
    ...
    function withdrawBatch(uint256[] memory tokenIds) external {
        ...
        for (uint i=0; i<tokenIds.length; i++) {
            ...
            emit Withdrawing(msg.sender, tokenIds[i], block.number);
        }

        // send message to root regarding token burn
        _sendMessageToRoot(abi.encode(WITHDRAW_BATCH, abi.encode(msg.sender, tokenIds, block.number)));
    }
