...
contract PolydiceGameV2 is VRFConsumerBase, Ownable {
	...
	event CheckingIfInPlayers(bool msgsenderInPlayers);
	...
	function isOnPlayers() public returns(bool) {
		emit CheckingIfInPlayers(passLineBetAmounts[msg.sender] > 0);
		return passLineBetAmounts[msg.sender] > 0;
	}


	function addToPlayers() private {
		passLinePlayers[currentPlayers] = payable(msg.sender);
		currentPlayers++;
	}
