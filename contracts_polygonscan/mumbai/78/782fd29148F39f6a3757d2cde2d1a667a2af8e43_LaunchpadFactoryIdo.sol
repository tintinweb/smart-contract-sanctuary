...
import "./CrowdsaleIdo.sol";
import "./StakingLaunchpad.sol";
import "@openzeppelin/contracts/access/Ownable.sol";

contract LaunchpadFactoryIdo is Ownable {
    ...
    struct StakingLaunchpadInfo {
        uint256 CrowdsaleIndex;
    ...
    StakingLaunchpadInfo[] public stakingInfos;

    uint256 public crowdsaleIndex;
    uint256 public stakingIndex;

    event CrowdsaleLaunched(
        uint256 indexed crowdsaleIndex,
        address indexed crowdsaleAddress
    );

    event StakingLaunched(
        uint256 indexed crowdsaleIndex,
        address indexed stakingAddress
    );

    function _preValidateAddress(IERC20 _addr) internal pure {
    ...
    function launchCrowdsale(
        ...
        uint256 _vestingTimeTillEnd,
        uint256 _cliffDurationInSecs
    ) public onlyOwner returns (address) {
        require(
            _crowdsaleStartTime >= block.timestamp,
            "Crowdsale Start time should be greater than current time"
        ); // ideally at least 24 hours more to give investors time
        require(
            _crowdsaleEndTime > _crowdsaleStartTime || _crowdsaleEndTime == 0,
            "Crowdsale End Time can either be greater than _crowdsaleStartTime or 0"
        ...
        address stakingAddress = deployStaking(
            msg.sender,
            _crowdsaleStartTime,
            _tokenAddress
        );
        CrowdsaleIdo newCrowdsale = new CrowdsaleIdo(
            _owner,
            address(this),
            stakingAddress
        );
        ...
        emit CrowdsaleLaunched(crowdsaleIndex, address(newCrowdsale));
        crowdsaleIndex++;
    }
    ...
    function deployStaking(
        ...
        StakingLaunchpad newStakingLaunchpad = new StakingLaunchpad(
            _admin,
            address(this),
            _crowdsaleStartTime
        );

        StakingLaunchpadInfo
            memory newStakingLaunchpadInfo = StakingLaunchpadInfo({
                CrowdsaleIndex: crowdsaleIndex,
                StakingAddress : address(newStakingLaunchpad),
                tokenAddress: _tokenAddress,
                owner: _admin
            });
        stakingInfos.push(newStakingLaunchpadInfo);
        emit StakingLaunched(crowdsaleIndex, address(newStakingLaunchpad));
        stakingIndex++;

        return address(newStakingLaunchpad);
    }
}
...
contract CrowdsaleIdo is ReentrancyGuard {
    ...
    IStakingLaunchpad public stakingLaunchpad;
    ...
    constructor(
        ...
        shibaSwapRouter = IUniswapV2Router01(
            0x8634D033B2d40D9E1ED8e977f175cf2D9e2BC298
        );
        stakingLaunchpad = IStakingLaunchpad(_stakingAddress);
    }
    ...
    function buyTokenWithStableCoin(
        ...
        require(
            _stableCoinAmount <= amountUserCanInvest.mul(uint256(_stableCoin.decimals()))
        );
        ...
        totalUsdAmountInvested[msg.sender] = totalUsdAmountInvested[msg.sender]
            .add(_stableCoinAmount.div(uint256(_stableCoin.decimals())));
    ...
    function buyFromWhiteListCrypto(
        ...
        require(
            _whiteListCryptoAmount <=
                ((amountUserCanInvest.mul(1e6)).div(cryptoPrice)).mul(
                    uint256(_whiteListCrypto.decimals())
                )
        );
        ...
        if (address(_whiteListCrypto) == shibaSwapRouter.WETH()) {
            require(msg.value == _whiteListCryptoAmount);
        } else {
...
contract StakingLaunchpad is ReentrancyGuard {
  using SafeMath for uint256;
  using SafeERC20 for IERC20;

  struct UserInfo {
    uint256 amountLocked; // How many LP tokens the user has provided.
  ...
  // lock time
  uint256 public lockTime = 21 days;
  // Minimum leash token to be staked
  uint256 public minLeashStakePerUser = 1e18;
  // Info of each user that stakes tokens.
  mapping(uint256 => mapping(address => UserInfo)) public userInfo;
  // max usd limit per user
  mapping(address => uint256) public usersUsdLimit;
  // total user in each pool
  mapping(uint256 => uint256) public Pools;
  // total users in each tier
  uint256 public tier1 = 100;
  uint256 public tier2 = 300;
  uint256 public tier3 = 100;

  // Leash Token
  IERC20 public LeashToken = IERC20(0xaa1949F76B208e704Dd86Fd8a97EDC59B3F52049);
  ...
  function updateData(
    ...
    require(msg.sender == owner);
    require(block.timestamp < idoStartTime, "Ido has started");
    tier1 = _tier1;
    tier2 = _tier2;
    tier3 = _tier3;
  }

  function lockLeashToParticipate(uint256 _stakeAmount)
    external
    nonReentrant
    returns (bool)
  {
    require(block.timestamp < idoStartTime, "Ido has started");
    require(_stakeAmount >= minLeashStakePerUser, "Insufficent Stake Amount");

    if (_stakeAmount >= minLeashStakePerUser && _stakeAmount < 100e18) {
      UserInfo storage user = userInfo[0][msg.sender];
      // tier 1
      require(
        !user.isParticipating,
        "User Cannot Pariticipate in a Pool Twice"
      );
      require(Pools[0] <= tier1, "Max Users Reached For Tier 1");
      LeashToken.safeTransferFrom(msg.sender, address(this), _stakeAmount);
      user.amountLocked = user.amountLocked.add(_stakeAmount);
      usersUsdLimit[msg.sender] = usersUsdLimit[msg.sender].add(1000);
      user.unlockTokenTime = idoStartTime.add(lockTime);
      Pools[0] = Pools[0].add(1);
      return true;
    } else if (_stakeAmount >= 100e18 && _stakeAmount < 500e18) {
      UserInfo storage user = userInfo[0][msg.sender];
      // tier 2
      require(
        !user.isParticipating,
      ...
      require(Pools[1] <= tier2, "Max Users Reached For Tier 2");
      LeashToken.safeTransferFrom(msg.sender, address(this), _stakeAmount);
      user.amountLocked = user.amountLocked.add(_stakeAmount);
      usersUsdLimit[msg.sender] = usersUsdLimit[msg.sender].add(2500);
      user.unlockTokenTime = idoStartTime.add(lockTime);
      Pools[0] = Pools[0].add(1);
      return true;
    } else if (_stakeAmount >= 500e18 && _stakeAmount < 1000e18) {
      UserInfo storage user = userInfo[0][msg.sender];
      // tier 3
      require(
        !user.isParticipating,
      ...
      require(Pools[2] <= tier3, "Max Users Reached For Tier 3");
      LeashToken.safeTransferFrom(msg.sender, address(this), _stakeAmount);
      user.amountLocked = user.amountLocked.add(_stakeAmount);
      usersUsdLimit[msg.sender] = usersUsdLimit[msg.sender].add(4000);
      user.unlockTokenTime = idoStartTime.add(lockTime);
      Pools[0] = Pools[0].add(1);
      return true;
    }
  ...
  function unLockLeash(uint256 _pool) external nonReentrant returns (bool) {
    UserInfo storage user = userInfo[_pool][msg.sender];
    require(block.timestamp > idoStartTime.add(lockTime), "Ido has started");
    require(_pool <= 2, "the input pool doesnt exits");
    require(
      block.timestamp >= user.unlockTokenTime,
      "the staked time has not been completed"
    );
    require(user.amountLocked > 0, "Not enough Leash Staked");

    uint256 _amountUnlocked = user.unlockTokenTime;
    user.unlockTokenTime = 0;
    LeashToken.safeTransfer(msg.sender, _amountUnlocked);
    return true;
  }
}
