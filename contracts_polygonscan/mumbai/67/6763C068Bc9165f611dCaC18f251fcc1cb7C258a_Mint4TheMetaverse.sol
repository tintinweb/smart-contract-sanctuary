...
contract Mint4TheMetaverse is ERC721, ERC721Enumerable, Ownable, ContextMixin {
    ...
    uint32 public maxSupply = 100000;
    bool public mintingEnabled;
    uint8 public buyLimit = 10;
    ...
    function tokenMint( uint8 quantity) external payable {
        ...
        for (uint i = 1; i <= quantity; i++) {
            ...
            Pixel storage pixel = pixels[totalSupply()];
            pixel.owner = msg.sender;
            pixel.tokenId = totalSupply();
        }
    }
    ...
    function reserveToken(uint8 quantity) onlyOwner public {
        ...
        for (uint i = 1; i <= quantity; i++) {
            ...
            Pixel storage pixel = pixels[totalSupply()];
            pixel.owner = msg.sender;
            pixel.color = "col_white";
            pixel.tokenId = totalSupply();
            canvasPixel storage canvaspixel = canvas[totalSupply()];
            canvaspixel.pixelNum = totalSupply();
        }
    }

    function fillUpPixel(uint32 tokenId, string memory message, string memory color) external {
        ...
        require(!pixel.isLocked, "This pixel is locked. If you have not done this, please contact the project owner.");
