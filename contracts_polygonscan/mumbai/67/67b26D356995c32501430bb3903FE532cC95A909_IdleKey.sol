...
contract IdleKey is IIdleKey, ERC721Enumerable
{
    ...
    function incrementCounter(uint256 n) public returns (uint256) {
        uint256 currentId = _tokenIds.current();
        require(currentId + n <= _MAX_SUPPLY, "Over max supply");
        for (uint256 i=0; i<n; i++) {
            _tokenIds.increment();
            currentId = _tokenIds.current();
            emit KeyCounterUpdated(currentId);
        }
        return currentId;
    }

    function safeMintKeys(address to, uint256 count) public override {
    ...
    function safeMintKey(address to) public override returns (uint256) {
        uint256 lastId = _tokenIds.current();
