...
contract MasterChef is Ownable,ReentrancyGuard {
    ...
    event ouput(uint256 out);
    event checkTest(uint256 init,uint256 amount, uint256 balance);
    function _swap(address payable _to, PoolInfo storage pool, uint256 _amount, uint8 _slippage)private returns (uint256){
        ...
        // token0init.sub(_amount).sub(address(this).balance)
        emit checkTest(token0init, _amount ,address(this).balance);
        uint256 token0leftOver = pool.isETHPair?token0init.sub(_amount.div(2)).sub(token0Amount):token0.balanceOf(address(this)).sub(token0init); 
