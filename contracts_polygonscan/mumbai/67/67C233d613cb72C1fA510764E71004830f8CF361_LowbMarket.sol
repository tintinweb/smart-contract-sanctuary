...
contract LowbMarket {
    ...
    function getTotalHolds(address nftAddress, address holder) public view returns (uint) {
      ...
      for (uint i=0; i<_punkHolds[nftAddress][holder].length; i++) {
        if (token.holderOf(_punkHolds[nftAddress][holder][i]) == holder) {
          n++;
        }
      }
      return n;
    ...
    function getPunkHolds(address nftAddress, address holder) public view returns (uint[] memory) {
      ...
      for (uint i=0; i<_punkHolds[nftAddress][holder].length; i++) {
        if (token.holderOf(_punkHolds[nftAddress][holder][i]) == holder) {
          punkHolds[j] = _punkHolds[nftAddress][holder][i];
          j++;
        }
      }
      return punkHolds;
    ...
    function rentItem(address nftAddress, uint itemId, uint amount, uint rentDays) public {
        ...
        _punkHolds[nftAddress][msg.sender] = getPunkHolds(nftAddress, msg.sender);
        _punkHolds[nftAddress][msg.sender].push(itemId);

        _makeDeal(offer.owner, amount*rentDays, nftAddress);
