...
contract CanvasManager is AccessControl, Ownable, Pausable, ICanvas, ReentrancyGuard {
    ...
    //     checkNftId(nftId);
    //     IArtAuction artAuction = _getArtAuction(nftId);
    //     return artAuction.createArtAuction(address(_curatorProxy), duration, reservePrice, curatorFeePercentage);
    // }
    ...
    function addConsumeToken(IERC20 addr) external {
        onlyAdmin();
        mTokens.push(addr);
    }
    ...
    function getDrawPrice(uint256 nftId, uint256 index) external view returns (uint256 price) {
        checkNftId(nftId);
        require(index < PixCount, "GPP");

        Art storage art = mArts[nftId];
        price = _newPrice(art.price[index]);
    }
    ...
    function drawPixel(
        ...
        uint256 price = _newPrice(art.price[index]);
        IERC20 token = mTokens[tokenIndex];
        require(token.balanceOf(_msgSender()) >= price, "BAL");
        require(bidPrice >= price, "BP");

        _consumeToken(token, _msgSender(), price);
    ...
    function _newPrice(uint256 oldPrice) internal pure returns (uint256) {
        return (oldPrice == 0) ? 1 : (oldPrice + oldPrice);
    }
