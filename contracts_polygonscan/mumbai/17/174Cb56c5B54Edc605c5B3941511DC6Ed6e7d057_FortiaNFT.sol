...
import "./interfaces/IFortiaNFT.sol";
import "./interfaces/IFortiaCreator.sol";
import "./interfaces/IFortiaRoyalty.sol";

contract FortiaNFT is
  ERC721Upgradeable,
  OwnableUpgradeable,
  ...
  IFortiaRoyalty
{
  ...
  {
    return
      _interfaceId == type(IFortiaNFT).interfaceId ||
      _interfaceId == type(IFortiaCreator).interfaceId ||
      _interfaceId == type(IFortiaRoyalty).interfaceId ||
  ...
  function mint(
    ...
    require(tokenIdsAvailable[_id] == false, "id exist");

    tokenIdsAvailable[_id] = true;
    tokenIds.push(_id);
    maxId = maxId > _id ? maxId : _id;
    creators[_id] = _account;
    royalties[_id] = _royaltyPercentage;
    tokenInfo[_id] = TokenInfo(_id, _account, block.timestamp);
    _mint(_account, _id);
  }
  ...
  function burn(uint256 _id) external override {
    require(ownerOf(_id) == _msgSender() || hasRole(BURNER_ROLE, _msgSender()), "FortiaNFT:01");
    tokenInfo[_id].owner = address(0);
    _burn(_id);
  }

  function setTokenOwner(address _account, uint256 _id) external override {
    require(_msgSender() == marketplace, "FortiaNFT:05");
    tokenInfo[_id].owner = _account;
  }

  /**
   * @dev Set creator
   * @param _id token ID
   * @param _account token creator
   */
  function setCreator(uint256 _id, address _account) external override onlyRole(ASSIGNER_ROLE) {
    creators[_id] = _account;
  }
   ...
   */
  function setRoyaltyPercentage(uint256 _id, uint256 _royaltyPercentage) external override {
    require(creators[_id] == msg.sender || hasRole(ASSIGNER_ROLE, _msgSender()), "FortiaNFT:02");
    royalties[_id] = _royaltyPercentage;
  }
   ...
   * @param _id token ID
   */
  function getRoyaltyPercentage(uint256 _id) external view override returns (uint256) {
  ...
  function redeem(
    ...
    require(
      _verify(
        _hash(_creator, _id, _royaltyFeePercentage, _priceType, _price, _buyNowPrice, _startTime, _endTime, _status),
        _signature
      ),
      "FortiaNFT:03"
    );
    ...
    royalties[_id] = _royaltyFeePercentage;
    tokenInfo[_id] = TokenInfo(_id, _creator, block.timestamp);
    _mint(_creator, _id);
  }
    ...
    return
      _hashTypedDataV4(
        keccak256(
          abi.encode(
            keccak256(
              "FortiaNFT(address _creator,uint256 _id,uint256 _royaltyFeePercentage,uint256 _priceType,uint256 _price,uint256 _buyNowPrice,uint256 _startTime,uint256 _endTime,uint256 _status)"
            ),
            _creator,
            _id,
  ...
  function setApprovalForAll(address _operator, bool _approved) public override {
    require(_msgSender() != _operator, "FortiaNFT:04");
    _operatorApprovals[_msgSender()][_operator] = _approved;
    emit ApprovalForAll(_msgSender(), _operator, _approved);
  }

  /**
   * @dev See {IERC721-setApprovalForAll}. by Marketplace
   */
