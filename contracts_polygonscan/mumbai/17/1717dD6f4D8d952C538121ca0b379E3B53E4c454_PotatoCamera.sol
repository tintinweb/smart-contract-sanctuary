    ...
    TrustedForwarderRecipient
{
    ...
    address public _filmAddress;

    // Events
    event PhotoPosted(address indexed creatorAddress, uint256 photoIndex);

    // Deployment
    constructor(address forwarderAddress_, address filmAddress_)
        ERC721("Internet Camera", "PHOTO")
        TrustedForwarderRecipient(forwarderAddress_)
    {
        _filmAddress = filmAddress_;
    }

    // Public APIs
    function postPhoto(string memory ipfsHash) public override {
        // Withdraw 1 FILM from user's wallet
        IInternetCameraFilm(_filmAddress).loadFilm(_msgSender());
        IERC20(_filmAddress).transferFrom(
            _msgSender(),
            address(this),
            1 * 10**18
        );
        ...
        emit PhotoPosted(_msgSender(), _tokenIdCounter.current());
...
interface IInternetCamera {
    function postPhoto(string memory ipfsHash) external;
}
