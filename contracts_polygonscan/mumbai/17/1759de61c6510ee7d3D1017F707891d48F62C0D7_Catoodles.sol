...
contract Catoodles is ERC721, Ownable {
    ...
    uint16[] public uniqueProbability = [500, 500];
    ...
    function tokenURI(uint tokenId) public override view returns (string memory) {
        ...
        bytes memory name = abi.encodePacked("Catoodle #", Strings.toString(tokenId));
        if (cat.unique) {
            name = "Upside down Catoodle";
        }
    ...
    function drawSVG(Cat memory cat) public view returns (string memory) {
        string memory header = '<svg viewBox="0 0 37 37" xmlns="http://www.w3.org/2000/svg" shape-rendering="crispEdges" preserveAspectRatio="xMinYMin meet">';
        if (cat.unique) {
    ...
    uint public constant MAX_NFT_SUPPLY = 500;
