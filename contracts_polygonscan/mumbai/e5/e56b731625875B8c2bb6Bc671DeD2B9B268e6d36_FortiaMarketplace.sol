...
import "./interfaces/IFortiaMarketplace.sol";
  ...
  IFortiaMarketplace
{
  ...
  address public fortiaTokenAddress;
  address public fortiaNFTAddress;
   ...
   * @param _nft address of fortia NFT
   */
  function __FortiaMarketplace_init(address _token, address _nft) public initializer {
    __Ownable_init();
  ...
  function supportsInterface(bytes4 interfaceId) public view virtual override(AccessControlUpgradeable) returns (bool) {
    return interfaceId == type(IFortiaMarketplace).interfaceId || super.supportsInterface(interfaceId);
  }
  ...
  function setAddresses(address _token, address _nft) public onlyOwner {
    ...
    fortiaTokenAddress = _token;
    fortiaNFTAddress = _nft;
  }
  ...
  function buy(uint256 _id, address _owner) public override nonReentrant {
    ...
    require(
      IERC20(fortiaTokenAddress).balanceOf(sender) >= (buyPrice + buyerMarketFee + royaltyFee),
      "Marketplace:buy03"
    );
    ...
    IERC20(fortiaTokenAddress).transferFrom(_msgSender(), address(this), buyPrice + buyerMarketFee + royaltyFee);
    ...
    if (IERC721(fortiaNFTAddress).ownerOf(_id) == address(this)) {
      _transferNFT(address(this), sender, _id);
    } else {
    ...
    IFortiaNFT(fortiaNFTAddress).setTokenOwner(sender, _id);
  ...
  function bid(
    ...
    require(requiredCost <= IERC20(fortiaTokenAddress).balanceOf(sender), "Marketplace:bid04");
    ...
    IERC20(fortiaTokenAddress).transferFrom(sender, address(this), requiredCost);
  ...
  function lazyMintingBuy(
    ...
    IFortiaNFT(fortiaNFTAddress).redeem(_creator, _id, _royaltyPercentage, 0, _price, 0, 0, 0, 1, _signature);
  ...
  function lazyMintingBid(
    ...
    address sender = _msgSender();
    require(requiredCost <= IERC20(fortiaTokenAddress).balanceOf(sender), "Marketplace:lazyMintingBid04");

    IFortiaNFT(fortiaNFTAddress).redeem(
      _creator,
      _id,
      _royaltyPercentage,
      _priceType,
    ...
    IFortiaNFT(fortiaNFTAddress).setApprovalForAllByMarketplace(_creator, address(this), true);
    IERC721(fortiaNFTAddress).safeTransferFrom(_creator, address(this), _id);
  ...
  function buyNow(uint256 _id, address _owner) external {
    ...
    require(requiredCost <= IERC20(fortiaTokenAddress).balanceOf(sender), "Marketplace:buyNow04");
  ...
  function lazyMintingBuyNow(
    ...
    IERC20(fortiaTokenAddress).transferFrom(sender, address(this), _buyNowPrice + variables[1] + variables[0]);
    ...
    // IERC20(fortiaTokenAddress).transferFrom(sender, _creator, _buyNowPrice - variables[2]);
  ...
  function _refundBid(uint256 _id, address _owner) private {
    ...
    if (bidBalance[currentBid.bidder] > 0 && currentBid.bidder != _owner) {
      ...
      bidBalance[currentBid.bidder] = bidBalance[currentBid.bidder] - (currentBid.price + buyerMarketFee + royaltyFee);
      IERC20(fortiaTokenAddress).transfer(currentBid.bidder, currentBid.price + buyerMarketFee + royaltyFee);
    }
  }
  ...
  function _getRoyaltyFeePercentage(uint256 _id) private view returns (uint256) {
    return IFortiaRoyalty(fortiaNFTAddress).getRoyaltyPercentage(_id);
  }
  ...
  function _calculateRoyaltyFee(uint256 _id, uint256 _amount) private view returns (uint256) {
    return (_amount * IFortiaRoyalty(fortiaNFTAddress).getRoyaltyPercentage(_id)) / NOMINATOR;
  }
  ...
  function _validateBeforeSale(uint256 _id) private view {
    address owner = IERC721(fortiaNFTAddress).ownerOf(_id);
    address sender = _msgSender();
  ...
  function _transferNFT(
    ...
    IFortiaNFT(fortiaNFTAddress).setApprovalForAllByMarketplace(_from, address(this), true);
    IERC721(fortiaNFTAddress).safeTransferFrom(_from, _to, _id);
  }
  ...
  function _transferRoyaltyFeeFromBuyer(uint256 _id, uint256 _amount) private {
    IERC20(fortiaTokenAddress).transferFrom(_msgSender(), IFortiaCreator(fortiaNFTAddress).getCreator(_id), _amount);
  }

  function _transferRoyaltyFeeFromMarketplace(uint256 _id, uint256 _amount) private {
    IERC20(fortiaTokenAddress).transfer(IFortiaCreator(fortiaNFTAddress).getCreator(_id), _amount);
  }
  ...
  function _transferMarketFee(uint256 _amount) private {
    IERC20(fortiaTokenAddress).transferFrom(_msgSender(), address(this), _amount);
  }
  ...
  function _payout(
    ...
    IERC20(fortiaTokenAddress).transfer(_owner, _price - _sellerMarketFee);
    IERC20(fortiaTokenAddress).transfer(IFortiaCreator(fortiaNFTAddress).getCreator(_id), _royalty);
  }
  ...
  //   uint256 _buyerMarketFee
  // ) private {
  //   IERC20(fortiaTokenAddress).transferFrom(_msgSender(), _owner, _price - _sellerMarketFee);
