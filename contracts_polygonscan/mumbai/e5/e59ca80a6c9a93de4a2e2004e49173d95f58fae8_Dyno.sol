...
// File: Dyno/DynoGenerator.sol

pragma solidity ^0.8.11;
 
 
 library DynoGeneGenerator {
...
interface IDyno is IERC721Enumerable {

    function geneOf(uint256 tokenId) external view returns (uint256 gene);
    function redeem(uint256 amount, uint256 mPindex) external;
...
/*
* @title ERC721 token for Dyno, redeemable through burning MintPass tokens
*
* @author Niftydude
*/
contract Dyno is IDyno, ERC721Enumerable, ERC721Pausable, ERC721Burnable, Ownable {
    using DynoGeneGenerator for DynoGeneGenerator.Gene;
    using Strings for uint256;

    uint256 constant MAX_REDEEM = 40;
    
    uint256 constant MAX_SUPPLY_MINTPASS = 5000;
    
    uint256 constant MAX_SUPPLY = 10200;
    ...
    DynoGeneGenerator.Gene internal geneGenerator;

    event Redeemed(address indexed account, uint256 amount, uint256 tokenId, uint256 _gene);
    ...
    */
    ...
    /**
    * @notice Redeem specified amount of MintPass tokens for Dyno
    * @param amount the amount of MintPasses to redeem* 
    * @param mPindex MintPasse id
    */
    function redeem(uint256 amount, uint256 mPindex) external override {
        ...
        require(mintPassContract.balanceOf(msg.sender, mPindex) >= amount, "Redeem: insufficient amount of MintPasses");

        mintPassContract.burnFromRedeem(msg.sender, mPindex, amount);

        for(uint256 i = 0; i < amount; i++) {
            uint256 tokenId = totalSupply() + 1;
            require(tokenId <= MAX_SUPPLY_MINTPASS, "Max Mint Pass Reached");
            _genes[tokenId] = geneGenerator.random();
            _mint(msg.sender, tokenId);
            emit Redeemed(msg.sender, amount, tokenId, _genes[tokenId]);
        }
    ...
    * @param amount the amount of MintPasses to redeem* 
    */
    function mintRemainingDyno(uint256 amount) external onlyOwner {
        
        for(uint256 i = 0; i < amount; i++) {
            uint256 tokenId = totalSupply() + 1;
            require(tokenId <= MAX_SUPPLY, "Max Reached");
            _genes[tokenId] = geneGenerator.random();
            _mint(msg.sender, tokenId);
        }
    } 
    ...
    /**
    * @notice returns the gene combination for a given Dyno
    * 
    * @param tokenId the Dyno id to return genes for
    */
    function geneOf(uint256 tokenId) external view virtual override returns (uint256 gene) {
