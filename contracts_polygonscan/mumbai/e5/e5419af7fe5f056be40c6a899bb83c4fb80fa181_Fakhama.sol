...
contract Fakhama is IERC20
{
    ...
    function withdrawDividend(uint amount) public
    {
        ...
        require(map_Users[memberId].Dividend-map_Users[memberId].DividendWithdrawn>=amount);
    ...
    function getUserInfo(uint memberId) public view returns(User memory, string memory, string memory, uint, uint)
    {
        User memory userInfo = map_Users[memberId];
        string memory CurrentRankName = map_ranks[userInfo.RankId].Name;
        string memory NextRankName = userInfo.RankId<6?map_ranks[userInfo.RankId+1].Name:"";
        uint requiredBusinessForNextRank = map_ranks[userInfo.RankId+1].Business;
        uint coinRate = getCoinRate(memberId);

        return (userInfo, CurrentRankName, NextRankName, requiredBusinessForNextRank, coinRate);
    }
}