...
contract Summoning is Ownable, VRFConsumerBase {
    ...
    modifier owningItems(INFT nft, uint256[5] memory ids) {
        require(
            nft.ownerOf(ids[0]) == msg.sender &&
                nft.ownerOf(ids[1]) == msg.sender &&
                nft.ownerOf(ids[2]) == msg.sender &&
                nft.ownerOf(ids[3]) == msg.sender &&
                nft.ownerOf(ids[4]) == msg.sender,
    ...
    function _summon(bytes32 requestID, uint256 randomness) internal {
        ...
        if (orb.balanceOf(request.requester, request.orbID) > 0) {
            ...
            if (orbType == 0) {
                ...
                require(numMagicSourcePerGen[generation] > 0, "numMagicSourcePerGen not set");
                require(numItemTypePerGen[generation] > 0, "numItemTypePerGen not set");
                item.mint(
                    request.requester,
                    generation,
                    rarity,
    ...
    function upgradeProphet(uint256[5] memory ids) external isUnique(ids) owningItems(prophet, ids) {
        require(
    ...
    function upgradeItem(uint256[5] memory ids) external isUnique(ids) owningItems(item, ids) {
        require(
    ...
    function updateNumRacePerGen(uint16 generation, uint16 numRace) external onlyOwner {
        numRacePerGen[generation] = numRace;
    }

    function updateNumCharPerGen(uint16 generation, uint16 numChar) external onlyOwner {
        numCharPerGen[generation] = numChar;
    }

    function updateNumMagicSourcePerGen(uint16 generation, uint16 numMagicSource) external onlyOwner {
        numMagicSourcePerGen[generation] = numMagicSource;
    }

    function updateNumItemTypePerGen(uint16 generation, uint16 numItemType) external onlyOwner {
