...
contract QuizChest is Ownable, Events {
    ...
    function checkLock(address _from) public view returns(bool, uint8, uint256, uint256){
        uint256 amount = uint256(_locks[_from].membership).mul(1 ether).mul(5);
        return (_locks[_from].locked, _locks[_from].membership, amount, _locks[_from].lastLockTimeStamp);
    }
}