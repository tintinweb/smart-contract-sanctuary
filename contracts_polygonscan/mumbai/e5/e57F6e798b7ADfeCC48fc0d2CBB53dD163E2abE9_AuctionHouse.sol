...
contract AuctionHouse is IAuctionHouse, ReentrancyGuard {
    ...
    function createAuction(
        ...
        bidDb.createAuction(auctionId, auctions[auctionId]);
    ...
    function setAuctionReservePrice(uint256 auctionId, uint256 reservePrice) external override auctionExists(auctionId) {
        ...
        bidDb.setAuctionReservePrice(auctionId, reservePrice);
    ...
    nonReentrant
    {
        ...
        bidDb.createBid(auctionId, amount, auctions[auctionId].firstBidTime, auctions[auctionId].duration, auctions[auctionId].bidder);
    }
    ...
    function endAuction(uint256 auctionId) external override auctionExists(auctionId) nonReentrant {
        ...
        bidDb.endAuction(auctionId, curatorFee, tokenOwnerProfit, auctions[auctionId].bidder);
    ...
    function _approveAuction(uint256 auctionId, bool approved) internal {
        ...
        bidDb.setAuctionApproval(auctionId, approved);
    }
...
/// TODO: remove require and modifier which are used for test
contract BidDb is Ownable {
    enum State {
        Create, // not approved
        Approval, // approved
        InBid, // has bid
    ...
    // A mapping of all auctions
    mapping(uint256 => IAuctionHouse.Auction) public auctions;

    // A mapping of all bid
    mapping(uint256 => BidStatus) internal bidStatus;

    /**
     * @notice Require that the specified auction exists
     */
    modifier auctionExists(uint256 auctionId) {
        require(_exists(auctionId), "BidDb: auction is not exist");
        _;
    }

    function getBidStatus(uint256 auctionId) external view auctionExists(auctionId) returns (Status memory) {
        return bidStatus[auctionId].status;
    }

    function getBidHistory(uint256 auctionId) external view auctionExists(auctionId) returns (Bid[] memory) {
        return bidStatus[auctionId].bids;
    }

    function getBidRecord(uint256 auctionId, uint256 start, uint256 end) external view auctionExists(auctionId) returns (Bid[] memory) {
        require(start < end, "BidDb.getBidRecord: start < end");
        require(end <= bidStatus[auctionId].bids.length, "BidDb.getBidRecord: end <= length");

        uint256 count = end - start;
        Bid[] memory result = new Bid[](count);
        for (uint256 i = 0; i < count; ++i) {
            result[i] = bidStatus[auctionId].bids[start + i];
        }

        return result;
    }

    function createAuction(uint256 auctionId, IAuctionHouse.Auction memory auction) public onlyOwner {
        require(auctions[auctionId].tokenOwner == address(0), "BidDb.createAuction: auction is already exist");

        auctions[auctionId] = auction;
        bidStatus[auctionId].status.beginTime = block.timestamp;
        bidStatus[auctionId].status.state = State.Create;
    }

    function setAuctionApproval(uint256 auctionId, bool approved) external onlyOwner auctionExists(auctionId) {
        require(auctions[auctionId].firstBidTime == 0, "BidDb.setAuctionApproval: auction is started");
        require(bidStatus[auctionId].status.state != State.InBid, "BidDb.setAuctionApproval: state is InBid");
        require(bidStatus[auctionId].status.state != State.Cancel, "BidDb.setAuctionApproval: state is Cancel");
        require(bidStatus[auctionId].status.state != State.End, "BidDb.setAuctionApproval: state is End");

        auctions[auctionId].approved = approved;
        bidStatus[auctionId].status.state = approved ? State.Approval : State.Create;
    }

    function setAuctionReservePrice(uint256 auctionId, uint256 reservePrice) external onlyOwner auctionExists(auctionId) {
        require(auctions[auctionId].firstBidTime == 0, "Auction is started");
        require(bidStatus[auctionId].status.state != State.InBid, "BidDb.setAuctionReservePrice: state is InBid");
        require(bidStatus[auctionId].status.state != State.Cancel, "BidDb.setAuctionReservePrice: state is Cancel");
        require(bidStatus[auctionId].status.state != State.End, "BidDb.setAuctionReservePrice: state is End");

        auctions[auctionId].reservePrice = reservePrice;
    }
    
    function createBid(uint256 auctionId, uint256 amount, uint256 firstBidTime, uint256 duration, address payable bidder) external onlyOwner auctionExists(auctionId) {
        require(auctions[auctionId].approved, "BidDb.createBid: auction is not approved");
        require(bidStatus[auctionId].status.state != State.Create, "BidDb.createBid: state is Create");
        require(bidStatus[auctionId].status.state != State.Cancel, "BidDb.createBid: state is Cancel");
        require(bidStatus[auctionId].status.state != State.End, "BidDb.createBid: state is End");
        require(bidStatus[auctionId].bids.length < type(uint88).max, "BidDb.createBid: length is too big");

        auctions[auctionId].amount = amount;
        auctions[auctionId].firstBidTime = firstBidTime;
        ...
        bidStatus[auctionId].bids.push(bid);
        bidStatus[auctionId].status.state = State.InBid;
        bidStatus[auctionId].status.count = uint88(bidStatus[auctionId].bids.length);
    }

    function cancelAuction(uint256 auctionId) external onlyOwner auctionExists(auctionId) {
        require(bidStatus[auctionId].status.state != State.Cancel, "BidDb.cancelAuction: state is Cancel");
        require(bidStatus[auctionId].status.state != State.End, "BidDb.cancelAuction: state is End");

        bidStatus[auctionId].status.state = State.Cancel;
    }

    function endAuction(uint256 auctionId, uint256 curatorFee, uint256 tokenOwnerProfit, address winner) external onlyOwner auctionExists(auctionId) {
        require(bidStatus[auctionId].status.state != State.Create, "BidDb.endAuction: state is Create");
        require(bidStatus[auctionId].status.state != State.Approval, "BidDb.endAuction: state is Approval");
        require(bidStatus[auctionId].status.state != State.Cancel, "BidDb.endAuction: state is Cancel");
        require(bidStatus[auctionId].status.state != State.End, "BidDb.endAuction: state is End");

        bidStatus[auctionId].status.curatorFee = curatorFee;
        bidStatus[auctionId].status.tokenOwnerProfit = tokenOwnerProfit;
        bidStatus[auctionId].status.winner = winner;
        bidStatus[auctionId].status.state = State.End;
    }
