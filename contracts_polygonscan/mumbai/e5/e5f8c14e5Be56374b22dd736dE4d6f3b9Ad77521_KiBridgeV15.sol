...
contract KiBridgeV15 is KiBridgeV1Factory {
    ...
    returns (bool success)
  {
    ...
    uint256 index;
    for(uint256 i = 0; i < userInfo[msg.sender].peningLiquidityRequestQueue.length - 1; i++) {
      if(userInfo[msg.sender].peningLiquidityRequestQueue[i] == _requestId) {
        index = i;
      }
    }
    require(userInfo[msg.sender].peningLiquidityRequestQueue[index] == _requestId, "RequestId not in the queue");
    bytes32 requestId = userInfo[msg.sender].peningLiquidityRequestQueue[userInfo[msg.sender].peningLiquidityRequestQueue.length - 1];
    userInfo[msg.sender].peningLiquidityRequestQueue[index] = requestId;
    userInfo[msg.sender].peningLiquidityRequestQueue.pop();
    if(_calAmount == 0) {
