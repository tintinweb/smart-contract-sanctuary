...
contract InvestmentSafeFactory is IInvestmentSafeFactory{
    mapping(address => bool) public hasCreatedSafe;
    address public override singleTokenJoin;
    address public override intermediateToken; //weth
    address public override childTokenPortal;
    IUniswapRouter public router;

    IChildTokenPortal public ChildTokenPortal;
    uint256 public override immutable EXIT_GAS = 1000;
    uint256 public override immutable JOIN_GAS = 1000;
    uint256 public override immutable GAS_CREATE_SAFE = 3208491; //this is the gas used on for safe create 

    bytes32 public override immutable _PERMIT_JOIN_TYPEHASH =
        keccak256(
            "PermitJoin(address sender,address targetToken,uint256 amount,uint256 targetAmount,uint256 nonce,uint256 deadline)"
        );

    bytes32 public override immutable _PERMIT_EXIT_TYPEHASH =
        keccak256(
    ...
    constructor(address _singleTokenJoin, address _intermediateToken, address _childTokenPortal, address _router)
    {
        owner = msg.sender;
        singleTokenJoin = _singleTokenJoin;
        intermediateToken = _intermediateToken;
        childTokenPortal = _childTokenPortal;
        ...
        returns (address payable safe)
    {
        ...
        InvestmentSafe(safe).initialize(safeName, safeOwner, address(this), msg.sender);
    ...
    function getWethUsedForGas(uint256 gas) public override view returns (uint256 wethUsedForSafe) {
        address[] memory path = new address[](2);

        path[0] = intermediateToken;
        path[1] = router.WETH(); //matic
        uint256[] memory amounts = router.getAmountsOut(tx.gasprice * gas, path);
        wethUsedForSafe = amounts[amounts.length - 1];
    }
...
interface IInvestmentSafeFactory {
    ...
    function childTokenPortal() external view returns (address);
...
import "./EIP712.sol";
import "./interfaces/IChildTokenPortal.sol";
...
contract InvestmentSafe is EIP712, IInvestmentSafe {
    ...
    function initialize(
        ...
        uint256 amount = investmentSafeFactory.getWethUsedForGas(investmentSafeFactory.GAS_CREATE_SAFE());
        IERC20(investmentSafeFactory.intermediateToken()).safeTransfer(_createor, amount);//payback cost for creating safe todo
    ...
    function setNonce(address user, uint256 _nonce) public onlyOwner {
        require(_nonce > nonces[user], "can only invalidate new nonces");
        nonces[user] = _nonce;
    }
    ...
    function withdrawFromSafe(address token, uint256 amount) public onlyOwner {
    ...
    function balanceOfInSafe(address token) public view returns (uint256) {
    ...
    /// @param amount The amount to be withdrawn
    function withdrawNativeTokenFromSafe(uint256 amount) public onlyOwner {
    ...
    function balanceOfNativeTokenInSafe() public view returns (uint256) {
    ...
    function delegatedJoin(
        ...
        require(block.timestamp <= deadline, "JoinPermit: expired deadline");

        uint256 gasCost = investmentSafeFactory.getWethUsedForGas(investmentSafeFactory.JOIN_GAS());
        IERC20(investmentSafeFactory.intermediateToken()).safeTransfer(msg.sender, gasCost);
        ...
        ISingleTokenJoin.JoinTokenStruct memory joinParams = ISingleTokenJoin
            .JoinTokenStruct(
                investmentSafeFactory.intermediateToken(),
                targetToken,
                amount.sub(gasCost),
                targetAmount,
                deadline,
        ...
        ISingleTokenJoin(investmentSafeFactory.singleTokenJoin()).joinTokenSingle(joinParams);

        IBridgeToken(targetToken).withdrawTo(targetAmount, owner);
        emit DelegatedJoin(targetToken, owner, amount, targetAmount);
    ...
    function delegatedExit(
        ...
        require(block.timestamp <= deadline, "ExitPermit: expired deadline");
        ...
        ISingleTokenJoin.ExitTokenStruct
            memory _exitTokenStruct = ISingleTokenJoin.ExitTokenStruct(
                sourceToken,
                amount,
                targetAmount,
                deadline,
        ...
        IERC20 intermediateToken = IERC20(investmentSafeFactory.intermediateToken());

        uint256 amountBefore = intermediateToken.balanceOf(address(this));

        ISingleTokenJoin(investmentSafeFactory.singleTokenJoin()).exitWeth(_exitTokenStruct);

        uint256 gasCost = investmentSafeFactory.getWethUsedForGas(investmentSafeFactory.JOIN_GAS());
        intermediateToken.safeTransfer(msg.sender, gasCost);
        
        targetAmount = intermediateToken.balanceOf(address(this)).sub(amountBefore);
        intermediateToken.safeIncreaseAllowance(investmentSafeFactory.childTokenPortal(), targetAmount);
        IChildTokenPortal(investmentSafeFactory.childTokenPortal()).getIOU(targetAmount);
        IChildTokenPortal(investmentSafeFactory.childTokenPortal()).withdrawTo(
            targetAmount,
            owner
        );
        emit DelegatedExit(sourceToken, owner, amount, targetAmount);
    }
...
interface IChildTokenPortal {
    function getIOU(uint256 amount) external;
...
interface ISingleTokenJoin {
    ...
    function exitWeth(ExitTokenStruct calldata _exitTokenStruct) external;
}
