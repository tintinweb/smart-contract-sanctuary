...
interface ILotteryApiConsumer {
    ...
    function transferOwnershipApi(address newOwner) external;
}
...
contract LotteryCenter is Ownable, Initializable, Testable {
    ...
    event RequestNumbers(uint256 indexed lotteryId, bytes32 _requestId);

    event CallbackWinningAwardInfo(
        uint256 indexed currentLotteryId,
        bytes32 requestId,
        string numbers
    );
        ...
        returns (uint256)
    {
        ...
        string memory lastTwoDigit = ticketNumber.substring(4, 6);
    ...
    function callbackWinningAward(
        ...
        _lotteries[currentLotteryId]
            .counPersonSecondWinning = _countSecondWinningNumber[
            currentLotteryId
        ][secondNumber];
        ...
        _lotteries[currentLotteryId].lotteryStatus = Status.Completed;

        emit CallbackWinningAwardInfo(currentLotteryId, requestId, numbers);
    }
    ...
    function tranferDependencyToAdmin() public onlyOwner {
        _lotteryTicket.transferOwnership(address(msg.sender));
        _lotteryApiConsumer.transferOwnershipApi(address(msg.sender));
    }
}