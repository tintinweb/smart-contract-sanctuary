...
contract DPSDocks is ERC721Holder, ReentrancyGuard, Ownable {
    ...
    function claimRewards(uint256 _voyageId, CausalityParams memory _causalityParams) external nonReentrant {
        ...
        awardRewards(voyageResult, voyageConfig, lockedVoyage, _causalityParams.userAddress);
    ...
    function checkVoyageState(uint256 _voyageId, CausalityParams memory causalityParams)
        external
        view
        returns (VoyageResult memory voyageResult)
    {
        causalityParams.userAddress = getLockedVoyageOwner(_voyageId);
        LockedVoyage storage lockedVoyage = lockedVoyages[causalityParams.userAddress][_voyageId];
    ...
    function computeVoyageState(
        ...
        claimingRewardsCache = computeFlagShipSkills(lockedVoyage.flagshipId, claimingRewardsCache);
        // console.log("plus flagship skills");
        // console.log("luck ", claimingRewardsCache.luck);
        // console.log("navigation ", claimingRewardsCache.navigation);
        // console.log("strength ", claimingRewardsCache.strength);
        claimingRewardsCache = computeSupportShipSkills(lockedVoyage.supportShipIds, claimingRewardsCache);
        // console.log(
        //     "skills [luck, navigation, strength]",
        ...
        uint256 maxPointsCap = gameSettings.getMaxPointsCap();
        // console.log("getMaxPointsCap ", maxPointsCap);
        ...
        for (uint256 i; i < voyageConfig.sequence.length; i++) {
            INTERACTION interaction = INTERACTION(voyageConfig.sequence[i]);
            ...
            uint256 result;
            if (causalityParams.signature.length > 0) {
                result = causality.getRandom(
                    causalityParams.userAddress,
                    causalityParams.blockNumber[claimingRewardsCache.randomCheckIndex],
                    causalityParams.hash1[claimingRewardsCache.randomCheckIndex],
                    causalityParams.hash2[claimingRewardsCache.randomCheckIndex],
                    causalityParams.timestamp[claimingRewardsCache.randomCheckIndex],
                    causalityParams.signature[claimingRewardsCache.randomCheckIndex],
            ...
            //     claimingRewardsCache.strength
            // );
            if (interaction == INTERACTION.STORM) console.log("random for interaction STORM = ", result);
            else if (interaction == INTERACTION.CHEST) console.log("random for interaction CHEST = ", result);
            else console.log("random for interaction ENEMY = ", result);
            voyageResult = interpretResults(result, voyageResult, lockedVoyage, claimingRewardsCache, interaction);
        }
    ...
    function interpretResults(
        ...
        if (interaction == INTERACTION.CHEST && result <= claimingRewardsCache.luck) {
            voyageResult.awardedChests++;
        } else if (
            (interaction == INTERACTION.STORM && result > claimingRewardsCache.navigation) ||
            (interaction == INTERACTION.ENEMY && result > claimingRewardsCache.strength)
        ) {
    ...
    function awardRewards(
        VoyageResult memory voyageResult,
        VoyageConfig memory voyageConfig,
        LockedVoyage memory lockedVoyage,
        address _owner
    ) internal {
        chests.mint(_owner, voyageConfig.typeOfVoyage, voyageResult.awardedChests);
        dps.safeTransferFrom(address(this), _owner, lockedVoyage.dpsId);

        if (voyageResult.healthDamage > 0)
            flagship.upgradePart(FLAGSHIP_PART.HEALTH, lockedVoyage.flagshipId, 100 - voyageResult.healthDamage);
        flagship.safeTransferFrom(address(this), _owner, lockedVoyage.flagshipId);
        ...
        for (uint256 i; i < lockedVoyage.supportShipIds.length; i++) {
            ...
            else supportShip.safeTransferFrom(_owner, address(this), lockedVoyage.supportShipIds[i]);
        }
        voyage.burn(lockedVoyage.voyageId);
    }

    function computeSupportShipSkills(uint256[] storage supportShips, VoyageStatusCache memory claimingRewardsCache)
        internal
        view
        returns (VoyageStatusCache memory)
    {
        ...
        return claimingRewardsCache;
    }

    function computeFlagShipSkills(uint256 _flagshipId, VoyageStatusCache memory claimingRewardsCache)
        internal
        view
        returns (VoyageStatusCache memory)
    {
        ...
        claimingRewardsCache.luck += flagShipBaseSkills;
        claimingRewardsCache.navigation += flagShipBaseSkills;
        claimingRewardsCache.strength += flagShipBaseSkills;
        for (uint256 i; i < 7; i++) {
            ...
            if (skillTypes[i] == uint8(SKILL_TYPE.NAVIGATION))
                claimingRewardsCache.navigation += skillsPerPart[i] * levels[i];
