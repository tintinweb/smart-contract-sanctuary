...
contract ModifiableMain is Ownable, ERC721Custom {
    ...
    uint256 public constant MIN_PRICE = 10**16;
    uint256 public SALE_START_TIMESTAMP;
    uint256 public REVEAL_TIMESTAMP;
    uint256 public COLOR_FREEZE_TS;
    uint256 public startingIndex;
    uint256 private _randomNum = 1; // !=0 in case of no sales
    uint256 public isHardChanged;
    bytes1[] public traitBytes;

    address public WETH; // mumbai - 0x36997DA706bd2953e63F2560ea219f8b665e658d; mainnet: 0x7ceb23fd6bc0add59e62ac25578270cff1b9f619
    ...
    mapping (uint256 => mapping (uint8 => uint64) ) public _tokenPixelColor;

    // events
    event Mint(address minter, uint256 id);

    // CMDFT
    event ColorModification (uint256 tokenId, uint8 position, uint64 color);

    // reveal
    event Reveal (uint256 startingIndex);
     ...
     */
    constructor (string memory name_, string memory symbol_, uint256 saleStartTS, address wETH, uint256 promoMints) 
        ERC721Custom(name_, symbol_){
        SALE_START_TIMESTAMP = saleStartTS;
        REVEAL_TIMESTAMP = SALE_START_TIMESTAMP.add(86400 * 3);
        AUCTION_END_TS = SALE_START_TIMESTAMP.add(86400 * 365);
        COLOR_FREEZE_TS = AUCTION_END_TS.sub(86400 * 3);
        WETH = wETH;

        // mint masterpiece at index 0. NFT pieces start with index 1. And mint 50 first for early supporters and giveaways promos
        _mint(address(this), 0);

        // mint for promos and early supporters
        if (promoMints >= 1){
            for (uint i=0; i<promoMints; i++){
    ...
    */
    function storeMetadata(bytes1[] memory traitsHex) external onlyOwner
    {
    ...
    /**
    * @dev Mints a piece from 1 to 20 at once
    */
    function mintPiece(uint256 quantity) external {
        require(block.timestamp >= SALE_START_TIMESTAMP, "Sale has not started");
        require(quantity>= 1 && quantity<= 20, "Amount of minted pieces at once should be in [1,20] interval");

        uint256 amount = getNFTPrice().mul(quantity);
    ...
    function finalizeStartingIndex() external {
        require(totalSupply() == MAX_NFT_SUPPLY || block.timestamp >= REVEAL_TIMESTAMP, "Not good time for setting startingIndex");
        _finalizeStartingIndex();
    }

    /**
     * @dev Finalize starting index earlier in case of emergancy. 
    */
    function hardFinalizeStartingIndex() external onlyOwner {
        _finalizeStartingIndex();
    }

    /**
     * @dev base function to finialize index
    */
    function _finalizeStartingIndex() private {
        require(startingIndex == 0, "Starting index is already set");
        // make cheating on startingIndex as hard as possible
        startingIndex = _randomNum.mod(1023);
    ...
    */
    function withdraw() onlyOwner external {
        ...
        // send 
        bool success = IERC20(WETH).transfer(msg.sender, balance);
     ...
     */
    function getPixelColor(uint256 tokenId, uint8 position) public view returns (uint64) {
     ...
     */
    function getPixelsColor(uint256 tokenId) public view returns (uint64[] memory) {
        uint8 N = 20;
    ...
    /**
     * @dev add color of pixel for piece NFT. Colors starting from 1,....
     *
     ...
     */
    function modifyColor(uint256 tokenId, uint8[] memory positions, uint64[] memory colors) external {
        ...
        require(block.timestamp < COLOR_FREEZE_TS, "Cant modify color after color freeze");
        require(positions.length > 0, "Positions length should be > 0");
        require(positions.length == colors.length, "Positions and colors should be equal length");
        for (uint i=0; i<positions.length; i++){
            for (uint j = i + 1; j < positions.length; j++) {
                require(positions[i] != positions[j], "Duplicate position");
            }
            _modifyColorOfOnePxel(tokenId, positions[i], colors[i]);
        }
    }

    /**
    * @dev modify color of 1 pixel
     */
    function _modifyColorOfOnePxel(uint256 tokenId, uint8 position, uint64 color) private {
        require(color <= 16777215, "Color cant be greater than 0xFFFFFF");
        if (startingIndex == 0){
            ...
            uint256 lengthTrait = getTraitsOfTokenId(tokenId);
        ...
        // emit event
        emit ColorModification(tokenId, position, color);
    }
    ...
    */
    function getTraitsOfTokenId(uint256 tokeinId) public view returns (uint256 length)
    {
        ...
        uint256 pieceIndex = (tokeinId.sub(1).add(startingIndex)).mod(1024);
        length = getTraitsOfInitialSeqAtIndex(pieceIndex);
    }
    ...
    */
    function getTraitsOfInitialSeqAtIndex(uint256 index) public view returns (uint256 length)
    {
        require(index < traitBytes.length, "Metadata does not exist for the specified index");
    ...
    */
    function bid(uint256 amount) external {
        ...
        require(amount > highestBid, "Your bid should be higher than the highest one");
        require(!isContract(msg.sender), "Only external accounts are allowed to bid");
        require(IERC20(WETH).transferFrom(msg.sender, address(this), amount));
    ...
    */
    function auctionEnd() external {
        ...
        require(highestBidder != address(0), "Cant end if there are no bidders");
    ...
    */
    function withdrawRoyalty(uint256 tokenId) external {
        ...
        // send 
        bool success = IERC20(WETH).transfer(msg.sender, roaltyPerNft);
        require(success, "Transfer failed.");
    }
    ...
    /**  @dev only for emergency cases. If something will go wrong and only once.
    * Contract is deployed with 1 year auction. We may increase / decrese it if community decides to 
    */
    function hardChangeDuration(uint256 colorFreezeTs, uint256 auctionDelaySeconds) external onlyOwner {
        require(isHardChanged == 0, "Can only hardChange the duration once");

        // save change
        isHardChanged = 1;

        COLOR_FREEZE_TS = colorFreezeTs;
        AUCTION_END_TS = colorFreezeTs.add(auctionDelaySeconds);
    }
}
...
contract ERC721Custom is Context, ERC165, IERC721, IERC721Metadata {
  ...
  // Token name
    ...
    function _baseURI() internal view virtual returns (string memory) {
        return "https://modifiable.art/api/v2/nft/";
    }
