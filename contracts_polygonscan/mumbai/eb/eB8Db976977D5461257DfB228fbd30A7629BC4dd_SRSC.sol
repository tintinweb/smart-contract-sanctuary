...
contract SRSC is ERC721Enumerable, Ownable {
	...
	string private baseUri = "https://api.streetracersskullscrew.com/token?tokenId=";
	...
	uint256 private maxTokensPublicSaleWallet = 10;
	...
	// constructor() ERC721("Street Racers Skulls Crew", "SRSC") {}
	...
	function _mintToken(address to, uint256 numOfToken) internal {
		...
		uint256 toTokenId = totalSupply()+numOfToken;
		...
		emit TokenMinted(to, fromTokenId, toTokenId);
	}
	...
	function mintPreSale(uint256 numOfToken) external payable onlyWhitelisted {
		require(saleStatus == SaleStatus.PreSale, "MINT: not in PreSale");
		require(currentPrice.mul(numOfToken) == msg.value, "MINT: incorrect ether amount");
		require(walletMintedTokensPresale[msg.sender] + numOfToken <= maxTokensWhitelistWallet, "MINT: exceed presale wallet limit");
	...
	function setIsRevealed(bool _newStatus) external onlyOwner {
    	isRevealed = _newStatus;
    }
