...
contract KawaiiDAO is Ownable, ERC721A, ReentrancyGuard {

  constructor(
    uint256 maxBatchSize_,
    uint256 collectionSize_,
    uint256 amountForAuctionAndDev_
  ) ERC721A("KawaiiDAO", "KawaiiDAO", maxBatchSize_, collectionSize_) {
    require(amountForAuctionAndDev_ <= collectionSize_, "larger collection size needed" );
  }
    
   uint256 pricePer = 20 ether; //20 Matic
   uint256 totalCost;
  ...
  function mintGiveaway(uint quantity) external onlyOwner {
      require(totalSupply() + quantity <= 9999, 'sold out');
  ...
  function withdrawAll() public onlyOwner {
      ...
      _withdraw(AiChan, ((balance * 50) / 100));
