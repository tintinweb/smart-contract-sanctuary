...
import "./external/openzeppelin/draft-EIP712.sol";
import "./external/openzeppelin/ECDSA.sol";
import "./external/openzeppelin/Counters.sol";

import "./interfaces/IERC20.sol";
import "./abstract/Ownable.sol";
import "./libraries/Constants.sol";

contract Token is IERC20, Ownable, EIP712 {
    ...
    uint256 private constant MAX_TRANSFER_FEE_BASIS_POINTS = 1000; // 10%
    uint256 private constant BASIS_POINTS_MULTIPLIER = 10000;

    uint256 private _transferFeeBasisPoints = 50; // 0.5%
    ...
    function _net(uint256 n) internal view returns (uint256) {
        return (n * BASIS_POINTS_MULTIPLIER) / (BASIS_POINTS_MULTIPLIER + _transferFeeBasisPoints);
    }

    function balanceOf(address account) public view override returns (uint256) {
        if (_excludedFromFees[account]) return _balances[account];
        return _net(_balances[account]);
    }
    ...
    function transferFrom(
        ...
        if (currentAllowance < type(uint256).max - 1) {
    ...
    function _transfer(
        ...
        uint256 need = amount;
        if (!(_transferFeeBasisPoints == 0 || _excludedFromFees[sender])) {
            uint256 fee = (amount * _transferFeeBasisPoints) / BASIS_POINTS_MULTIPLIER;
            _balances[feeContract] += fee;
            need += fee;
            emit Transfer(sender, feeContract, fee);
        }

        require(_balances[sender] >= need, "ERC20: transfer amount exceeds balance");
        _balances[sender] -= need;
    ...
    function permit(
        ...
        bytes32 structHash = keccak256(abi.encode(PERMIT_TYPEHASH, _owner, spender, value, _nonces[_owner].current(), deadline));
