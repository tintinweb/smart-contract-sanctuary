 ...
 * @summary: Vault for storing ERC20 tokens that will be transfered by external event-based system to another network. The destination network can be checked on "connectedNetwork"
...
contract L2Vault is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    uint256 public claimLockupTime;
    ...
    mapping(address => address) public remoteTokenAddress; // remoteTokenAddress[addressHere] = addressThere
    mapping(address => mapping(address => uint256)) public unlockedFunds; // unlockedFunds[token][account] = X
    mapping(address => uint256) public totalUnlockedFunds;
    ...
    event TokenAdded(address indexed erc20, address indexed remoteTokenAddress);
    event TokenRemoved(
        address indexed erc20,
        address indexed remoteTokenAddress
    );
    ...
    function initialize(string memory newConnectedNetwork, address _feeAddress)
        public
        initializer
    {
        nonce = 0;
        ...
        transferLockupTime = 1800; // 1 day
        claimLockupTime = 86400; // 1 day
        __ReentrancyGuard_init();
        __Ownable_init();
        feeAddress = _feeAddress;
        connectedNetwork = newConnectedNetwork;
    }
    ...
    function addSupportedToken(address tokenAddress, address tokenAddressRemote)
        external
        onlyOwner
    {
        ...
        remoteTokenAddress[tokenAddress] = tokenAddressRemote;

        emit TokenAdded(tokenAddress, tokenAddressRemote);
    }
    ...
    {
        emit TokenRemoved(tokenAddress, remoteTokenAddress[tokenAddress]);

        delete remoteTokenAddress[tokenAddress];
    }

    function setClaimLockupTime(uint256 lockupTime) external onlyOwner {
    ...
    function setMinTransferDelay(uint256 newMinTransferDelay) external onlyOwner {
        require(newMinTransferDelay < maxTransferDelay, "Min TransferDelay cannot be more than max TransferDelay");
    ...
    function setMaxTransferDelay(uint256 newMaxTransferDelay) external onlyOwner {
        require(newMaxTransferDelay > minTransferDelay, "Max TransferDelay cannot be less than min TransferDelay");
    ...
    function getCurrentTokenLiquidity(address tokenAddress)
        ...
        returns (uint256)
    {
        return IERC20(tokenAddress).balanceOf(address(this)).sub(totalUnlockedFunds[tokenAddress]).sub(inTransferFunds[tokenAddress]);
    }
    ...
    function depositERC20(uint256 amount, address tokenAddress, uint256 transferDelay)
        external
        onlySupportedToken(tokenAddress)
        nonReentrant
        whenNotPaused
    ...
    function unlockInTransferFunds(address _token, uint256 _amount, bytes32 _id)
        external
        whenNotPaused
        onlyOwner
    {
        require(hasBeenUnlocked[_id] == false, "Already completed");
        require(inTransferFunds[_token] >= _amount, "More amount than available");
    ...
    function claimUnlockedFunds(address _token)
        external
        nonReentrant
        whenNotPaused
    {
        require(
            unlockedFunds[_token][msg.sender] > 0,
            "You have nothing to claim"
        );
        require(
            lastTransfer[msg.sender].add(claimLockupTime) < block.timestamp,
            "Claiming not yet possible"
        );

        uint256 unlocked = unlockedFunds[_token][msg.sender];
        unlockedFunds[_token][msg.sender] = 0;
        totalUnlockedFunds[_token] = totalUnlockedFunds[_token].sub(unlocked);
        SafeERC20.safeTransfer(IERC20(_token), msg.sender, unlocked);
        emit FundsClaimed(_token, msg.sender, unlocked);
    }
    ...
    function unlockFunds(
        ...
        unlockedFunds[_token][_user] = unlockedFunds[_token][_user].add(amount);
        totalUnlockedFunds[_token] = totalUnlockedFunds[_token].add(amount);
        emit FundsUnlocked(_token, _user, amount, id);
    }

    function getConnectedNetwork() external view returns (string memory) {
        return connectedNetwork;
    }

    modifier onlySupportedToken(address tokenAddress) {
        require(
            remoteTokenAddress[tokenAddress] != address(0),
            "Unsupported token"
        );
        _;
    }
