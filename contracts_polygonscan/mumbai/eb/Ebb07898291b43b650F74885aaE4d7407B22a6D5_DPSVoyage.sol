...
contract DPSVoyage is ERC721Enumerable, AccessControlEnumerable {
  ...
  function burn(uint256 _tokenId) external {
    ...
    _burn(_tokenId);
    //TODO might want to uncomment this but might keep it commented for history
  ...
  function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
    ...
    for (uint8 i; i < config.noOfInteractions; i++) {
      ...
      if (i < config.noOfInteractions - 1) interactions = string(abi.encodePacked(interactions, '"', interactionType, '",'));
