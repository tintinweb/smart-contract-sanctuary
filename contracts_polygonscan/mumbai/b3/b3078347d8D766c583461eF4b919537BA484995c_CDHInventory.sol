...
abstract contract CardPool is Ownable, Pausable {
    ...
    function getUri(uint256 id) public view returns (string memory) {
        return string(abi.encodePacked(uri, uint2str(id)));
    }
    ...
    */
    ...
    whenNotPaused {
        ...
        for(uint256 i; i<ids.length;i++) {
            ...
            uint256 senderTokenListLength = senderTokenList.length;
            for(uint256 j;j<senderTokenListLength;j++) {
                if(senderTokenList[j] == ids[i]) {
                    senderTokenList[j] = senderTokenList[senderTokenListLength-1];
                    senderTokenListLength--;
                }
            }
        }
    }
