...
contract TowerRewardsTracker is  Ownable, Pausable, EIP712MetaTransaction {
     ...
     *      Account Address => blockTimestamp
     */
    mapping(address => uint256) public _dailyTrackerTimestamp;
     ...
     */
    function checkDailyRewardsValidity() public view returns(bool){
        return checkDailyRewardsValidity(_msgSender());
    }
    
    function checkDailyRewardsValidity(address userAddress) public view returns(bool){
        uint256 lastTimestamp = _dailyTrackerTimestamp[userAddress];

        if(lastTimestamp ==0 || lastTimestamp + 24 hours <= block.timestamp){
            return true;
        }
        return false;
    }
    ...
    function dailyLog(string calldata uniqueUuid) public whenNotPaused {
        require(checkDailyRewardsValidity(), "Daily rewards checkin already done." );
        
        address msgSender = _msgSender();
        _dailyTrackerTimestamp[msgSender] = block.timestamp;
