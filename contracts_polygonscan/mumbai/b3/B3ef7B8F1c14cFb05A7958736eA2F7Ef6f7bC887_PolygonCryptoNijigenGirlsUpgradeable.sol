...
contract PolygonCryptoNijigenGirlsUpgradeable is IChildToken, Initializable, ERC721Upgradeable, OwnableUpgradeable {
    ...
    function batchMintFrom(address[] memory users,uint256[] memory ids) external payable{
    ...
    function removeAtIndex(uint index) public onlyOwner{
        require(index < _tokenIds.length, "CNGE910");
        for (uint i = index; i < _tokenIds.length-1; i++) {
