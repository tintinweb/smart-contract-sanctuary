...
contract Bank is AccessControl, Referral {
    ...
    event SetBalanceRisk(uint8 balanceRisk);
    ...
    function _allocateHouseEdge(
        ...
        uint256 referralAllocation = (amount * tokenHouseEdge.referral) / 10000;
        ...
        uint256 dividendAmount = ((amount * tokenHouseEdge.dividend) / 10000) +
            referralAllocationRestPerSplit;
        uint256 treasuryAmount = ((amount * tokenHouseEdge.treasury) / 10000) +
            referralAllocationRestPerSplit;
        uint256 teamAmount = ((amount * tokenHouseEdge.team) / 10000) +
            referralAllocationRestPerSplit;
    ...
    function setBalanceRisk(uint8 _balanceRisk)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    function payout(
        ...
        if (amount + fees > (getBalance(token) * balanceRisk) / 10000) {
            revert WrongPayout();
        }

        _allocateHouseEdge(user, token, fees);
    ...
    function cashIn(
        ...
        bool isGasToken = _isGasToken(token);
        if (isGasToken) {
            amount = msg.value;
        }

        _allocateHouseEdge(user, token, amount);
...
abstract contract Referral {
    ...
    function withdrawReferralCredits(address[] calldata tokens) external {
