...
abstract contract ERC1271Validator is EIP712Upgradeable {
    ...
    function validate1271(
        ...
        if (signature.length == 65) {
            signerFromSig = _hashTypedDataV4(hash).recover(signature);
        }
...
interface IERC721LazyMint is IERC721Upgradeable {
    event Creators(uint256 tokenId, LibPart.PartData[] creators);

    function mintAndTransfer(
        LibERC721LazyMint.LazyMintData memory data,
        address to
    ) external payable;
}
...
library LibERC721LazyMint {
    ...
    struct LazyMintData {
        uint256 tokenId;
        string tokenURI;
        LibPart.PartData[] creators;
        LibPart.PartData[] royalties;
        bytes[] signatures;
    }
    ...
    function hash(LazyMintData memory data) internal pure returns (bytes32) {
...
import "./IIonERC721.sol";

abstract contract ERC721Ion {
    uint256 public percentage;
    IIonERC721 ion;

    function setIon(IIonERC721 ion_) public {
        // only marketplace or owner can call
        ion = ion_;
    }
...
import "../lazy-mint/Mint721Validator.sol";
import "../lazy-mint/IERC721LazyMint.sol";
import "./ERC721Ion.sol";
...
interface IIonERC721 {
    function deposit(uint256 amt) external;

    function revenue(uint256 month) external view returns (uint256);
}