...
contract ModelMintableERC721Child is
ERC721,
IChildToken,
AccessControlMixin,
...
ContextMixin
{
  ...
  mapping (uint256 => Modelnft) public ModelNFTMapping;
  ...
  constructor(
    ...
    _initializeEIP712("ModelNFT");
    _setBaseURI("https://api-na-launchpad.qwellco.de/api/");
  }
  ...
  function createNewNft(string memory newName, string memory newIpsToken, string memory newDescription, string memory newModelName, address NFTOwner, uint8 newDataType) public returns(uint256){
    Modelnft memory newModel = Modelnft(newName,newIpsToken,newDescription,newModelName,counter,newDataType);
    if(NFTOwner == address(0)){
      NFTOwner = msg.sender;
    }
    _safeMint(NFTOwner,counter);
    ModelNFTMapping[counter] = newModel;
    counter++;
  ...
  function encodeTokenMetadata(uint256 tokenId) external view virtual returns (bytes memory) {
    ...
    // in L1
    Modelnft memory modelnft = ModelNFTMapping[tokenId];
    return abi.encode(modelnft,tokenURI(tokenId));
