...
contract PFarmToken is ERC20('PRE-FARM', 'PFARM'), ReentrancyGuard {
    ...
    uint256 public salePriceE35 = 1 * (10 ** 4) * (10 ** 31); 

    uint256 public constant pFarmMaximumSupply = 100 * (10 ** 3) * (10 ** 18);

    // We use a counter to defend against people sending pfarm back
    uint256 public pFarmRemaining = pFarmMaximumSupply;

    uint256 public constant maxFarmPurchase = 2000 * (10 ** 18);
...
contract FarmRedeem is Ownable, ReentrancyGuard {

    // Burn address
    address public constant BURN_ADDRESS = 0x000000000000000000000000000000000000dEaD;

    PFarmToken public pfarm;
    ...
    event FarmSwap(address sender, uint256 amount);
    event burnUnclaimedFarm(uint256 amount);
    event startBlockChanged(uint256 newStartBlock);

    constructor(uint256 _start, address _pfarmAddress, address _farmAddress) {
        require(_pfarmAddress != _farmAddress, "pfarm cannot be equal to farm");
        start   = _start;
        pfarm = PFarmToken(_pfarmAddress);
        farmAddress  = _farmAddress;
    }

    function swapPFarmForFarm(uint256 swapAmount) external nonReentrant {
        require(block.timestamp >= start, "farm redemption hasn't started yet, good things come to those that wait ;)");
        require(IERC20(farmAddress).balanceOf(address(this)) >= swapAmount, "Not Enough tokens in contract for swap");
        pfarm.transferFrom(msg.sender, BURN_ADDRESS, swapAmount);
        IERC20(farmAddress).transfer(msg.sender, swapAmount);

        emit FarmSwap(msg.sender, swapAmount);
    }

    function sendUnclaimedFarmToDeadAddress() external onlyOwner {
        require(block.timestamp > pfarm.end(), "can only send excess farm to dead address after presale has ended");
        require(!hasBurnedUnsoldPresale, "can only burn unsold presale once!");

        require(pfarm.pFarmRemaining() <= IERC20(farmAddress).balanceOf(address(this)),
            "burning too much farm, founders may need to top up");

        if (pfarm.pFarmRemaining() > 0)
            IERC20(farmAddress).transfer(BURN_ADDRESS, pfarm.pFarmRemaining());
        hasBurnedUnsoldPresale = true;

        emit burnUnclaimedFarm(pfarm.pFarmRemaining());
    }
    ...
    function setStartBlock(uint256 _newStart) external onlyOwner {
        ...
        emit startBlockChanged(_newStart);
    }
}