...
interface IIDOFactory {
    ...
    function isIDO(address _ido) external view returns(bool);
}
...
// File contracts/helper/ERC721Initializable.sol
 ...
 */
abstract contract VRFConsumerBaseInitializable is VRFRequestIDBase, Initializable {
  ...
  */
  function initializeVRFConsumer(address _vrfCoordinator, address _link) internal initializer {
    vrfCoordinator = _vrfCoordinator;
    LINK = LinkTokenInterface(_link);
  }
...
// File contracts/LaunchPad/SIMPLE_IDO_CASTLE.sol
 ...
 */
contract CASTLE_IDO is Initializable {
    IIDOFactory private Factory;
    IERC20 private saleToken;
    ICASTLE_IDO_Agreement private Agreement;
    ...
    uint public tokensSold;//How many IDO tokens have been sold
     ...
     * @param amount the amount of tokens they bought
     */
    ...
    function initializeIDO(address agreement) external initializer {
        ...
        Agreement = ICASTLE_IDO_Agreement(agreement);
        saleToken = IERC20(Agreement.saleToken());
        Factory = IIDOFactory(msg.sender);
        maxPerTx = Agreement.whaleStopper();
    }
    ...
    function adjustMaxPerTx(uint _newMax) external onlySudo{
        maxPerTx = _newMax;
    }

    function adjustKey(address _key) external onlySudo{
        key = IKey(_key);
    }
    ...
    function join(
        ...
        require(_amount <= maxPerTx, "Purchase amount too large");
        require((tokensSold + _amount) <= Agreement.totalAmount(), "_amount exceedes sale amount");
        if(block.timestamp < (Agreement.saleStart() + Agreement.roundDelay(0))){//need to look at tiers
            uint userTier = userEventTier(msg.sender);//users the lower value between the snapshot and current
        ...
        uint saleTokensOwed;
        if(key.balanceOf(msg.sender) > 0){
            uint keyBal = key.balanceOf(msg.sender);
            //remove some or all of the cost
            if(keyBal >= _amount){
                key.burn(msg.sender, _amount);
                saleTokensOwed = 0;
            }
            else{
                key.burn(msg.sender, keyBal);
                saleTokensOwed = (_amount - keyBal) * Agreement.price();
            }
        }
        else{
            saleTokensOwed = _amount * Agreement.price();
        }
        if(saleTokensOwed > 0){
        ...
        tokensSold += _amount; //record how many tokens have been sold
    ...
    */
    function getMaxMint(address _caller) public view returns(uint maxMint) {
        if(block.timestamp < (Agreement.saleStart() + Agreement.roundDelay(0))){//need to look at tiers
            uint userTier = userEventTier(_caller);
            for(uint i=3; i>0; i--){
            ...
            maxMint = maxPerTx;//user mint is only limited by whaleStopper
        }
    }
    ...
    function userEventTier(address _caller) public view returns(uint tier){
        uint currentTier = ITierManager(Factory.tierManager()).viewIDOTier(_caller);
        tier = ITierManager(Factory.tierManager()).checkTierIncludeSnapshot(_caller);
        if(currentTier < tier){//return the smallest one
            tier = currentTier;
        }
    }

    function tierStartTime() public view returns(uint time){
        time = Agreement.saleStart();
    }

    function publicStartTime() public view returns(uint time){
        time = Agreement.saleStart() + Agreement.roundDelay(0);
    }
}