 ...
 */
contract MglobeGameERC1155 is Context, AccessControlEnumerable, ERC1155Burnable, ERC1155Pausable {
    ...
    function uri(
        ...
        bytes memory customUriBytes = bytes(customUris[_id]);
        if (customUriBytes.length > 0) {
            return customUris[_id];
        } else {
    ...
    function setURI(string memory _newURI) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "GameNFTS: must have admin role to setURI");
    ...
    function batchMint(
        ...
        for (uint256 i = 0; i < _ids.length; i++) {
            ...
            tokenSupplies[_id] = tokenSupplies[_id].add(quantity);
        }
    ...
    ) public ownersOnly(_id)
    {
        ...
        tokenSupplies[_id] = tokenSupplies[_id].sub(_quantity);
    }
    ...
    ) public
    {
        for (uint256 i = 0; i < _ids.length; i++) {
            ...
            require(balanceOf(_msgSender(), _id) > 0, "GameNFTs#ownersOnly: ONLY_OWNERS_ALLOWED");
            uint256 quantity = _quantities[i];
            tokenSupplies[_id] = tokenSupplies[_id].sub(quantity);
        }
