...
contract TAP is ERC20, AccessControl {
  ...
  constructor(
    ...
    // set the Foundation fee to be 2%
    setFoundationFee(2, 0);
    // set the Vault fee to be 0.1%
    setVaultFee(1, 1);
...
contract TAPw is ERC20, AccessControl, ERC20Permit, ERC20Votes {
  address public tapAddress;

  address wishingWellAddress;
  uint256 public wishingWellFee;
  uint256 public wishingWellFeeDecimals;
  ...
  modifier onlyTAPorOwner() {
    require(
      msg.sender == tapAddress || hasRole(OWNER_ROLE, msg.sender),
      "Address is not TAP"
    );
    _;
  }
  ...
  modifier onlyValidAddress(address wallet) {
    ...
    require(wallet != msg.sender, "The address cannot be the sender");
    require(
      wallet != wishingWellAddress,
      "The address cannot be the wishing well"
    );
    require(
      wallet != foundationAddress,
      "The address cannot be the foundation"
  ...
  constructor(address _wishingWellAddress, address _foundationAddress)
    ERC20("Wish", "TAPw-v3")
    ERC20Permit("Wish")
  {
    wishingWellAddress = _wishingWellAddress;
    foundationAddress = _foundationAddress;
    ...
    // set the Foundation fee to be 2%
    setWishingWellFee(2, 0);
    // set the Vault fee to be 0.1%
    setFoundationFee(1, 1);
  }
  ...
  function mint(address to, uint256 amount) public onlyTAPorOwner {
    _mint(to, amount);
  }
  ...
  // Sets the fee percentage for the TAP Vault
  function setWishingWellFee(uint256 fee, uint256 feeDecimals)
    public
    onlyAdmin
  {
    ...
    wishingWellFee = fee;
    wishingWellFeeDecimals = feeDecimals;
  }
  ...
  function getFees(uint256 amount) public view returns (uint256, uint256) {
    ...
    uint256 tokensForWishingWell = calculatePortion(
      amount,
      wishingWellFee,
      wishingWellFeeDecimals
    );
    return (tokensForWishingWell, tokensForFoundation);
  }
  ...
  function _transfer(
    ...
    super._transfer(from, wishingWellAddress, tokensForWishingWell);
    super._transfer(from, foundationAddress, tokensForFoundation);
    super._transfer(from, to, remainder);
  }
}
