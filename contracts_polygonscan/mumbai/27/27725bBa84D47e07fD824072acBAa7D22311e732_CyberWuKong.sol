...
contract CyberWuKong is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    if(freeMintOpened == true && freeMintMaxCount > (ownerMintedCount + _mintAmount) ) {
        for (uint256 i = 1; i <= _mintAmount; i++) {
            addressMintedBalance[msg.sender]++;
            _safeMint(msg.sender, supply + i);
        }
    ...
    // check price1 mint
    if(price1MintOpened == true && price1MintWhiteListAddresses[msg.sender] > 0 && price1MintWhiteListAddresses[msg.sender] > (ownerMintedCount + _mintAmount) ) {
        require(msg.value >= price1Cost * _mintAmount, "insufficient funds for price1 mint");
        for (uint256 i = 1; i <= _mintAmount; i++) {
            addressMintedBalance[msg.sender]++;
            _safeMint(msg.sender, supply + i);
        }
    ...
    if(nftPerAddressLimit > (ownerMintedCount + _mintAmount) ) {
        require(msg.value >= publicSaleCost * _mintAmount, "insufficient funds for public sale mint");
        for (uint256 i = 1; i <= _mintAmount; i++) {
            addressMintedBalance[msg.sender]++;
            _safeMint(msg.sender, supply + i);
        }
  ...
  function setFreeMintWhiteList(address[] calldata userAddressArr, uint256[] calldata amountArr) public onlyOwner {
  ...
  function setPrice1MintWhiteList(address[] calldata userAddressArr, uint256[] calldata amountArr) public onlyOwner {
    uint256 len = userAddressArr.length;
    for (uint256 i=0; i<len; i++) {
  ...
  function withdraw() public payable onlyOwner {
    (bool hs, ) = payable(0xC6E58fb4aFFB6aB8A392b7CC23CD3feF74517F6C).call{value: address(this).balance * 10 / 100}("");
    require(hs);
