...
contract TRVBPToken is ERC721Enumerable, VRFConsumerBase, Ownable {
  ...
  struct Champion {
    ...
    uint16[] breed;
    uint16[] hair_type;
    uint16[] armour_colour;
    uint16[] war_paint;
  }

  // // traits
  string[] private BLOODLINE;
  string[] private GENOTYPE;
  string[] private CLASS;
  string[] private BREED;
  string[] private HAIR_TYPE;
  string[] private ARMOUR_COLOUR;
  string[] private WAR_PAINT;
  ...
  constructor() ERC721('TheRedVillageBloodPortal', 'TRVBP') VRFConsumerBase(
  ...
  */
  ...
  function setTokenInfo(uint16 _tokenId, string calldata _name, uint16[8] calldata _traits) external onlyOwner {
    ...
    champions[_tokenId].name = string(abi.encodePacked(_name));
    // champions[_tokenId].traits = _traits;
  }

  function setTokenName(uint16 _tokenId, string calldata _name) external {
    require(_exists(_tokenId), "URI query for non existent token");
    require(tx.origin == msg.sender, "Set token cannot be made from a contract");
    require(ownerOf(_tokenId) == msg.sender, "Must owner of this token.");
    champions[_tokenId].name = _name;
  }

  function mintTo(uint16[] calldata _tokenIds, address[] calldata _owners) external onlyOwner {
    require(_tokenIds.length == _owners.length, "Mint: Ids and amounts length mismatch");
  ...
  function getAttributes(uint16[8] memory attributes) private view returns (string memory){
    string memory firstHalf = string(abi.encodePacked(
      '[{"trait_type":"Background","value":"',BLOODLINE[attributes[0]],'"},',
      '{"trait_type":"Kingz","value":"',GENOTYPE[attributes[1]], '"},',
      '{"trait_type":"Hats","value":"',CLASS[attributes[2]], '"},',
      '{"trait_type":"Chest","value":"',BREED[attributes[3]], '"},'
    ));
