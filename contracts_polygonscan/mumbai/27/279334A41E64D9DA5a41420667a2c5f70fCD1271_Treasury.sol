...
contract Treasury is Ownable {
    ...
    IOxygenERC20 public immutable Oxygen;
    ...
    constructor (
        ...
        Oxygen = IOxygenERC20(_Oxygen);

        isReserveToken[ _TEST ] = true;
        reserveTokens.push( _TEST );
    ...
    /**
        @notice allow approved address to deposit an asset for Oxygen
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint Oxygen needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        limitRequirements(msg.sender, send_);
        Oxygen.mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn Oxygen for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        Oxygen.burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = Water.balanceOf( msg.sender ); // Can only borrow against sOHM held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with Oxygen
        @param _amount uint
     */
    function repayDebtWithTime( uint _amount ) external {
        ...
        Oxygen.burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, address(Oxygen), _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        Oxygen.mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( Oxygen.totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns Oxygen valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match Oxygen decimals
            value_ = _amount.mul( 10 ** Oxygen.decimals() ).div( 10 ** IERC20( _token ).decimals() );
...
interface IOxygenERC20 is IERC20Mintable, IERC20 {
