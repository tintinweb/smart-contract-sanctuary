...
contract FundamentaBridge is SecureContract
{
        ...
        returns(uint256, uint256, uint256, uint256)
    {
        ...
        require(sender == msg.sender, "Bridge: Not sent by client");
    ...
    function deposit(bytes[] memory serverSignatures, bytes memory transactionData) public pause
    {
        ...
        TokenInterface(token.info.token).mintTo(msg.sender, depositAmount);
    ...
    function withdraw(uint256 nonce, uint256 srcNetwork, uint256 destNetwork, uint256 tokenId, uint256 amount) public pause returns (uint256)
    {
        TokenData storage token = _tokens[tokenId];
    ...
    function depositAndUnwrap(bytes[] memory serverSignatures, bytes memory transactionData) public pause
    {
        ...
        token.depositNonces[nonce] = true;
        WrappedTokenInterface(token.info.token).crossChainUnwrap(msg.sender, depositAmount);
        token.accumulatedFee += fee;
