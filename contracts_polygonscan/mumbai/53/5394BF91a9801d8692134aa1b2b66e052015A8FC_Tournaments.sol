...
using SafeMath for uint;
    ...
    function depositPrizeTokens(uint _tournamentId, uint _amount) public {
        ...
        IERC20 testToken= IERC20(0xfe4F5145f6e09952a5ba9e956ED0C25e3Fa4c7F1);
       // testToken.approve(address(this),_amount); //this should be called by user on front directly on tokens contract
    ...
    function setDistributionRules(uint _tournamentId, uint[] memory _distribution) public {
      ...
      function setPlacements(uint _tournamentId, address[] memory _placements) public {
        require(msg.sender==tournaments[_tournamentId].ownerAddress || owners[msg.sender]==1, "Permission denied");
        tournaments[_tournamentId].placements=_placements;
    }


     function distrubuteRewards(uint _tournamentId) public {
        require(msg.sender==tournaments[_tournamentId].ownerAddress || owners[msg.sender]==1, "Permission denied");
        require(tournaments[_tournamentId].placements.length==tournaments[_tournamentId].distributionRules.length,"Distribution rules or placements not set");
        
        for (uint i=0;i<tournaments[_tournamentId].distributionRules.length;i++){
            uint amount=tournaments[_tournamentId].prizeAmount.div(100).mul(tournaments[_tournamentId].distributionRules[i]);
            sendToken(tournaments[_tournamentId].placements[i],amount);
