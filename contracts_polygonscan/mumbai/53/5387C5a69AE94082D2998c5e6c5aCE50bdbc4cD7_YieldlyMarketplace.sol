...
contract YieldlyMarketplace is ReentrancyGuard, Ownable {
    ...
    function mint(string memory _tokenURI, address _sender, uint256 _price, bool _isListOnMarketplace, uint256 _royalty, uint256 startDate, uint256 auctionLength) public {
        _tokenIds.increment();

        uint256 newTokenId = _tokenIds.current();
        price[newTokenId] = _price;
        royaltyMap[newTokenId] = _royalty;
        creatorMap[newTokenId] = _sender;
        ownerMap[newTokenId] = _sender;
        listedMap[newTokenId] = _isListOnMarketplace;

        // require (msg.value >= price[newTokenId], "msg.value should be equal to the buyAmount");

        Yieldly(nftContract).mint(newTokenId, _sender, _tokenURI);

        if (_isListOnMarketplace) this.createAuction(newTokenId, startDate, auctionLength, _price, _sender);
        
        emit Minted(_sender, _price, newTokenId, _tokenURI, _isListOnMarketplace);
    }
