...
contract DanceOff is VRFConsumerBase, Ownable{
    ...
    function getFirstPlace(uint256 limit) public view returns (uint256[] memory){
        uint256[] memory returnArr;
        uint256 index = 0;
        if(firstPlacements.length > limit){
            for(uint256 i = firstPlacements.length - limit; i < firstPlacements.length; i++){
                returnArr[index] = firstPlacements[i];
                index++;
            }
        }
        else{
            for(uint256 i = 0; i < firstPlacements.length; i++){
                returnArr[index] = firstPlacements[i];
                index++;
            }
        }
    ...
    function getSecondPlace(uint256 limit) public view returns (uint256[] memory){
        uint256[] memory returnArr;
        uint256 index = 0;
        if(secondPlacements.length > limit){
            for(uint256 i = secondPlacements.length - limit; i < secondPlacements.length; i++){
                returnArr[index] = secondPlacements[i];
                index++;
            }
        }
        else{
            for(uint256 i = 0; i < secondPlacements.length; i++){
                returnArr[index] = secondPlacements[i];
                index++;
            }
        }
    ...
    function getThirdPlace(uint256 limit) public view returns (uint256[] memory){
        uint256[] memory returnArr;
        uint256 index = 0;
        if(thirdPlacements.length > limit){
            for(uint256 i = thirdPlacements.length - limit; i < thirdPlacements.length; i++){
                returnArr[index] = thirdPlacements[i];
                index++;
            }
        }
        else{
            for(uint256 i = 0; i < thirdPlacements.length; i++){
                returnArr[index] = thirdPlacements[i];
                index++;
            }
        }
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        ...
        uint8 powerup = responseIdToBattle[requestId].juicedUp * 10;
        ...
        if(
            (royaleProcessedLink[rumbleId] == royaleParticipants[rumbleId]) 
            && (
                (royaleParticipants[rumbleId] == rumbleSize) 
                || (royaleParticipants[rumbleId] >= minimumSize 
                    && (block.timestamp - royaleTimeTrigger[rumbleId]) >= maxTime
                    )
                )
            )
        {
    ...
    function beginDance(uint256 rumbleId) internal{
        ...
        firstPlacements.push(fpRoll.tokenId);
        secondPlacements.push(spRoll.tokenId);
        thirdPlacements.push(tpRoll.tokenId);
        ...
        tokenToWinner[fpRoll.tokenId].push(
            Winner(
                fpRoll.tokenId,
                1,
                rumbleId,
        ...
        tokenToWinner[spRoll.tokenId].push(
            Winner(
                spRoll.tokenId,
                2,
                rumbleId,
        ...
        tokenToWinner[tpRoll.tokenId].push(
            Winner(
                tpRoll.tokenId,
                3,
                rumbleId,
