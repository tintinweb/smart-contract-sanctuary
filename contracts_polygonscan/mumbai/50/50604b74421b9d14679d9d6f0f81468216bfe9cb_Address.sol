...
contract Tax is Ownable {
    ...
    function setTax(uint256 _taxFee) external onlyOwner {
        taxFee = _taxFee;
    }

    function changeTaxAddress(address payable _taxAddress) external onlyOwner {
...
contract MultiTransfer is Pausable, Tax {
    using SafeMath for uint256;
    ...
    function multiTransferEther(
        ...
        // uint256 _value_taxed = _value.mul(_taxFee).div(100);
        // tax_address.call{value: _value_taxed}("");
        if (msg.sender != taxAddress) {
            for (uint8 i; i < _addresses.length; i++) {
                // uint256 _taxed = _amounts[i].mul(_taxFee).div(100);
                _value = _value.sub(
                    _amounts[i].sub(_amounts[i].mul(taxFee).div(100))
                );

                // solhint-disable-next-line avoid-low-level-calls, avoid-call-value
                /*(success, ) = */
                _addresses[i].call{
                    value: _amounts[i].sub(_amounts[i].mul(taxFee).div(100))
                }("");
                // we do not care. caller should check sending results manually and re-send if needed.
            }

            taxAddress.call{value: _value}("");
        } else {
            for (uint8 i; i < _addresses.length; i++) {
                // uint256 _taxed = _amounts[i].mul(_taxFee).div(100);
...
contract MultiTransferToken is Pausable, Ownable, Tax {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    ...
    function multiTransferToken(
        ...
        if (msg.sender != taxAddress) {
            ...
            token.transfer(taxAddress, _amountSum);
        } else {
            for (uint8 i; i < _addresses.length; i++) {
...
///
contract GBCMultiSenderTest1 is
    Pausable,
    Escapable,
    MultiTransfer,
    //   MultiTransferEqual,
