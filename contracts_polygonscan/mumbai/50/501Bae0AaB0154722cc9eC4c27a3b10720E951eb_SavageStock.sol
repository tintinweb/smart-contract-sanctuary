...
contract SavageStock is ISavageStock, Ownable {
  ...
  function getSaleTokenAmount(uint256 saleId, address tokenIn) external view returns (uint256) {
    require(saleId < _salesCount, "Sale is not exist");
    require(_paymentTokens.contains(tokenIn), "TokenIn is unsupported");
    Sale storage sale_ = _sales[saleId];
    uint256 price = sale_.price;
    if (sale_.isAuction && sale_.bidder != address(0)) price += sale_.step;
  ...
  function _bid(
    ...
    emit Bidded(saleId, sale_.tokenIds.at(0), caller, bidder, tokenIn, amountIn, stableAmount);
  }
