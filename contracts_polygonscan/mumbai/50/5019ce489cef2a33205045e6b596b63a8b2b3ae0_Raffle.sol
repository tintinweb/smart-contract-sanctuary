...
contract Raffle is Ownable, IRandomiserCallback {
    uint256 constant MAX_WINNERS = 10;
    uint256 constant MAX_RANGE = 10_000; // Upper bound of random number to draw (inclusive)

    bool public drawn = false;
    uint256 public randomness = 0;

    uint256[MAX_WINNERS] public winners;
    Randomiser randomiser;

    constructor(address randomiserAddress) {
        randomiser = Randomiser(randomiserAddress);
    }

    modifier notDrawn() {
        require(!drawn, "Raffle has already been drawn!");
        _;
    }
     ...
     */
    function getRandomNumber() external onlyOwner notDrawn {
        require(randomness == 0, "Randomness has already been set");
        randomiser.getRandomNumber(address(this));
    }

    /**
     * Callback to receive a random number from the Randomiser contract.
     */
    function receiveRandomness(uint256 randomness_) external override {
        require(
            msg.sender == address(randomiser),
            "Must be called from the designated randomiser"
        );
        if (randomness != 0) {
            // Ignore if randomness has been set
            return;
        }
    ...
    /**
     * Use the stored random number to generate more random numbers to
     * draw winners for the raffle. Can only be run once!
     */
    function drawWinners()
        external
        onlyOwner
        notDrawn
        returns (uint256[MAX_WINNERS] memory)
    {
        require(randomness != 0, "Must receive a random value from VRF first");
        drawn = true;

        for (uint256 i = 0; i < MAX_WINNERS; i++) {
            uint256 rand = uint256(keccak256(abi.encode(randomness, i)));
            winners[i] = (rand % (MAX_RANGE - 1)) + 1; // range of [1,MAX_RANGE]
        }
    ...
    /**
     * View the winners, if they have already been drawn.
     */
    function getWinners() external view returns (uint256[MAX_WINNERS] memory) {
        require(drawn, "Winners have not been drawn yet!");
        return winners;
    }
}