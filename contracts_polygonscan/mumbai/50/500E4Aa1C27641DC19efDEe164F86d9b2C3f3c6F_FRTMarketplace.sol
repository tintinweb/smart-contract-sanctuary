...
contract FRTMarketplace is Ownable{
    ...
    event MintNFT(string, uint256, address);
    ...
    /******HANDLE NFTs START******/
    function mintNFT(string memory _tokenHash) public returns(uint256 newItemId){
        newItemId = baseNFT.mintNFT( msg.sender, _tokenHash);
        ...
        emit MintNFT(_tokenHash, newItemId, msg.sender);

        return newItemId;
    }

    function buyNFT(uint256 _tokenId) public returns(uint256 _rTokenId){
