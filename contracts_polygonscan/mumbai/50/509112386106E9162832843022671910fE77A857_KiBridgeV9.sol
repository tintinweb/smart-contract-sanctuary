...
contract KiBridgeV9 {
  ...
  struct UserInfo {
    ...
    mapping(bytes32 => PeningLiquidityRequest) peningLiquidityRequest;
    ...
    private
  {
    ...
    userInfo[msg.sender].peningLiquidityRequest[requestId] = PeningLiquidityRequest({ 
    ...
    returns (bool success, bytes memory result)
  {
    ...
    if(tokenInfo[_receiveToken].liquidable) {
      if(liquidityInfo[_receiveToken].peningLiquidityToChain[_fromChainId] >= _amount) {
        ...
        liquidityInfo[_receiveToken].fulfilledLiquidityFromChain[_fromChainId] += _amount;
      }
    }
    ...
    returns (bool success)
  {
    ...
    = abi.decode(_data, (Transaction, uint256, uint256));
    if(transaction.amount == 0) {
      if(fulfilled >= reqAmount) {
        liquidityInfo[transaction.sendToken].fulfilledLiquidityFromChain[_toChainId] += reqAmount;
      } else {
        liquidityInfo[transaction.sendToken].fulfilledLiquidityFromChain[_toChainId] += fulfilled;
        liquidityInfo[transaction.sendToken].peningLiquidityToChain[_toChainId] = 
        liquidityInfo[transaction.sendToken].peningLiquidityToChain[_toChainId] -
        reqAmount + fulfilled;
      }
      userInfo[_user].peningTokenBalance[transaction.sendToken] -= reqAmount;
      transaction.sendToken.safeTransfer(_user, reqAmount);
      return false;
    }
    userInfo[_user].peningTokenBalance[transaction.sendToken] -= transaction.amount;
    userInfo[_user].transactions[_fromChainId][_toChainId].push(transaction);
  ...
  function calculateFulfilledLiquidityFromChain(
    IERC20 _token,
    uint256 _chainId,
    uint256 _amount
  )
