...
contract DexilonTest_v6 {
    ...
    mapping(address => mapping (string => int)) internal usersAssetsBalances;
    mapping(address => mapping (string => uint256)) internal usersAssetsPrices;
    ...
    constructor() {
        ...
        for (uint8 i=0; i<assetsNames.length; i++) {
    ...
    */
        ...
        ) 
    {
        ...
        lockedBalance = usersLockedBalances[user] / uint(ROUNDING_FACTOR);
        balanceBTC = usersAssetsBalances[user]['BTC'];
        priceBTC = usersAssetsPrices[user]['BTC'] / uint(ROUNDING_FACTOR);
        balanceETH = usersAssetsBalances[user]['ETH'];
        priceETH = usersAssetsPrices[user]['ETH'] / uint(ROUNDING_FACTOR);
        balanceXRP = usersAssetsBalances[user]['XRP'];
        priceXRP = usersAssetsPrices[user]['XRP'] / uint(ROUNDING_FACTOR);
        balanceSOL = usersAssetsBalances[user]['SOL'];
        priceSOL = usersAssetsPrices[user]['SOL'] / uint(ROUNDING_FACTOR);
        
        marketFullBalance = 0;
        // marketFullBalance = int(availableBalance) +
        //     btcToUsdcMarketConvert(balanceBTC / leverage) +
        //     (balanceBTC >= 0 ? int(0) : 2*int(lockedBalance));
    }
    ...
    **/
    function trade(
        ...
        emit Trade(isBuyOrder, maker, taker, asset, amountWei, rate, block.timestamp);
    }
    
    /**
    * @dev update the balances of one of the traders
    * @param user user whose balances are updated
    * @param asset string id of the supported asset ('BTC','ETH','XRP','SOL')
    * @param amountWei the amount of the asset in Wei (smallest unit)
    * @param rate price of 1 asset coin in the smallest unit of USDC
    **/
    function updateUserBalances(
        ...
        int assetBalance = usersAssetsBalances[user][asset];
        int leverage = int(usersLeverage[user]);
        // int leverage = 5;
        int assetPrice = int(usersAssetsPrices[user][asset]);
        
        if (((amountWei > 0 && assetBalance < 0) || 
              (amountWei < 0 && assetBalance > 0)) &&
                abs(amountWei) > abs(assetBalance)) {
          
            updateUserBalances(user, asset, -assetBalance, rate);
        ...
        if (assetBalance == 0  
            ...
            newAvailableBalance = availableBalance
                - abs(amountWei*rate*ROUNDING_FACTOR/(leverage*BTC_UNIT_FACTOR));

        } else if (assetBalance > 0 && amountWei < 0) {

            calculateRate = assetPrice * (assetBalance > 0 ? int(1) : int(-1));
            newAvailableBalance = availableBalance 
                - amountWei*calculateRate/(leverage*BTC_UNIT_FACTOR)
                + amountWei*(calculateRate - rate*ROUNDING_FACTOR)/(BTC_UNIT_FACTOR);
        ...
        if ((assetBalance > 0 && amountWei < 0) || (assetBalance < 0 && amountWei > 0)) {
            newLockedBalance = lockedBalance - abs(amountWei*calculateRate/(leverage*BTC_UNIT_FACTOR));
            newAssetPrice = assetPrice;
        } else {
            newLockedBalance = lockedBalance + abs(amountWei*rate*ROUNDING_FACTOR/(leverage*BTC_UNIT_FACTOR));
            newAssetPrice = (assetBalance*assetPrice + amountWei*rate*ROUNDING_FACTOR) / (assetBalance + amountWei);
        }

        require(newLockedBalance >= 0, 'DexilonTest: newLockedBalance < 0');
        usersLockedBalances[user] = uint256(newLockedBalance);
        
        usersAssetsPrices[user][asset] = uint256(newAssetPrice);
        usersAssetsBalances[user][asset] = assetBalance + amountWei;
    }
