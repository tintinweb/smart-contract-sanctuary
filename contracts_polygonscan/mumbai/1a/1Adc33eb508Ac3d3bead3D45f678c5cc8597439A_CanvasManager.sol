...
contract CanvasManager is AccessControl, Ownable, Pausable, ICanvas, ReentrancyGuard {
    ...
    struct Art {
        ...
        uint32 momentIndex; // index no of last moment in this art: momentIndex = size of moments array
    ...
    //     checkNftId(nftId);
    //     IArtAuction artAuction = _getArtAuction(nftId);
    //     auctionId = artAuction.createArtAuction(duration, reservePrice);
    // }
    ...
    /// @return auctionCurrency The current auction currency
    ...
    function setConsumeToken(
        ...
        onlyAdmin();
        require(index < mTokens.length, "LEN");
    ...
    function startNewArt(uint256 endTime) external returns (uint256 nftId) {
        onlyAdmin();
        require(mNftId < MaxArt, "NA1");
        require(block.timestamp < endTime, "NA2");
    ...
    function endArt(uint256 nftId) external {
        checkNftId(nftId);
        Art storage art = mArts[nftId];
        require(art.state == 1, "EA1");
        require(block.timestamp >= art.endTime, "EA2");
    ...
    function getDrawPrice(
        ...
        checkNftId(nftId);
        require(pixelIndex < PixCount, "GP1");
        require(tokenIndex < _basePrices.length, "DP2");
    ...
    function drawPixel(
        ...
        checkNftId(nftId);
        require(index < PixCount, "DP1");
        require(tokenIndex < mTokens.length, "DP2");

        Art storage art = mArts[nftId];
        require(art.state == 1, "DP3");
        require(block.timestamp <= art.endTime, "DP4");

        IERC20 token = mTokens[tokenIndex];
        uint256 price = _getNewPrice(_basePrices[tokenIndex], art.price[index]);
        require(token.balanceOf(_msgSender()) >= price, "DP5");
        require(bidPrice >= price, "DP6");
    ...
    function calcShares(uint256 nftId, address contributor) public view returns (uint256 thisShares, uint256 allShares) {
        ...
        uint256 maxMomentId = art.moments[art.momentIndex - 1];
    ...
    function _endArt(uint256 nftId) internal {
        ...
        if (art.moments.length > 0) {
            ...
            IArtAuction artAuction = new ZoraAuction(
                art.moments[art.momentIndex - 1], // allNftShares
...
contract ZoraAuction is IArtAuction, IERC721Receiver, Ownable {
    ...
    function claimeArtAuctionFee(
        ...
        uint256 artFee = totalReceived - curatorFee; // tokens for users
        uint256 thisArtFee = (artFee * 900) / 1024; // tokens for this NFT users
        uint256 allArtsFee = artFee - thisArtFee; // thkens for all NFT users
