...
contract RETA is ERC20, AccessControl {
  ...
  function setPlatformDevFee(uint256 fee, uint256 feeDecimals) public onlyAdmin {
  ...
  function transfer(address to, uint256 amount) public override returns (bool) {
    ...
    if (!whitelist[to] && !whitelist[msg.sender] && !waiveFees) {
      tokensForCommunity = calculateFee(amount, communityFee, communityFeeDecimals);
      remainder = amount - tokensForCommunity;
    ...
    tokensForTokenFarm = calculateFee(amount, tokenFarmFee, tokenFarmFeeDecimals);
      remainder = amount - tokensForTokenFarm;

    tokensForTeam = calculateFee(amount, teamFee, teamFeeDecimals);
      remainder = amount - tokensForTeam;
    }

    super.transfer(communityAddress, tokensForTeam);
    ...
    if (_isRecipient[to]) {
      // call wish transfer function: mint(to, amount / 10)
    }
