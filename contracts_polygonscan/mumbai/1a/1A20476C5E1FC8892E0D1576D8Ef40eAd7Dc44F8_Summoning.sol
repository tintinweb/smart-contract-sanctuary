...
contract Summoning is Ownable, VRFConsumerBase {
    ...
    mapping(address => mapping(uint256 => bool)) private _requested;
        ...
        )
    {
        ...
        upgradeAmounts[0] = _upgradeAmounts[0];
        upgradeAmounts[1] = _upgradeAmounts[1];
        upgradeAmounts[2] = _upgradeAmounts[2];
        upgradeAmounts[3] = _upgradeAmounts[3];
    ...
    function summon(uint256 orbID) external {
        ...
        require(!_requested[msg.sender][orbID], "already requested to summon");
        _requested[msg.sender][orbID] = true;
    ...
    function _summon(bytes32 requestID, uint256 randomness) internal {
        ...
        if (orb.balanceOf(request.requester, request.orbID) > 0) {
            (uint256 orbType, uint256 orbRarity, uint16 generation) = orb.orbs(
                request.orbID
            );
        ...
        _requested[request.requester][request.orbID] = false;
        delete summoningRequests[requestID];
    }

    function upgradeProphet(uint256[5] memory ids) external {
        require(
            prophet.ownerOf(ids[0]) == msg.sender &&
                prophet.ownerOf(ids[1]) == msg.sender &&
                prophet.ownerOf(ids[2]) == msg.sender &&
                prophet.ownerOf(ids[3]) == msg.sender &&
                prophet.ownerOf(ids[4]) == msg.sender,
            "not owning the prophets"
        );
        require(
            _hasSameGenerationRarityRace(ids[0], ids[1]) &&
                _hasSameGenerationRarityRace(ids[1], ids[2]) &&
                _hasSameGenerationRarityRace(ids[2], ids[3]) &&
                _hasSameGenerationRarityRace(ids[3], ids[4]),
    ...
    function upgradeItem(uint256[5] memory ids) external {
        require(
            item.ownerOf(ids[0]) == msg.sender &&
                item.ownerOf(ids[1]) == msg.sender &&
                item.ownerOf(ids[2]) == msg.sender &&
                item.ownerOf(ids[3]) == msg.sender &&
                item.ownerOf(ids[4]) == msg.sender,
            "not owning the items"
        );
        require(
            _hasSameGenerationRarityClassMagicSource(ids[0], ids[1]) &&
                _hasSameGenerationRarityClassMagicSource(ids[1], ids[2]) &&
                _hasSameGenerationRarityClassMagicSource(ids[2], ids[3]) &&
                _hasSameGenerationRarityClassMagicSource(ids[3], ids[4]),
            "items should have same generation, rarity and magic source"
        );
        ...
        emit UpgradeRequested(msg.sender, false, request.ids, requestID);
    }
    ...
    function _upgrade(bytes32 requestID, uint256 randomness) internal {
        ...
        if (request.isProphet) {
            ...
            prophet.burn(request.ids[0]);
            prophet.burn(request.ids[1]);
            prophet.burn(request.ids[2]);
            prophet.burn(request.ids[3]);
            prophet.burn(request.ids[4]);
            ...
            item.burn(request.ids[0]);
            item.burn(request.ids[1]);
            item.burn(request.ids[2]);
            item.burn(request.ids[3]);
            item.burn(request.ids[4]);
    ...
    function _getRarity(uint256 orbRarity, uint256 randomness)
        internal
        view
        returns (uint16)
    {
    ...
    function updateUpgradeAmount(uint256[4] memory _amounts)
        ...
        onlyOwner
    {
        upgradeAmounts[0] = _amounts[0];
        upgradeAmounts[1] = _amounts[1];
        upgradeAmounts[2] = _amounts[2];
        upgradeAmounts[3] = _amounts[3];
    }
}
...
interface IOrb {
     ...
     */
    function orbs(uint256 orbId) external view returns (uint256, uint256, uint16);

    /**
     * @dev Returns the rarity probabilities.
     */
    function rarities(uint256 rarity) external view returns (uint16, uint16, uint16, uint16, uint16);
