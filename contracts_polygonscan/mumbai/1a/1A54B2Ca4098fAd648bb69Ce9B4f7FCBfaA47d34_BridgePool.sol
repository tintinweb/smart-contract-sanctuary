...
contract BridgePool is AccessControl, ReentrancyGuard, Multicall, EIP712, IBridgePool {
    ...
    event Deposit(address token, uint amount, uint8 to, bool bonus, bytes receiver);
    event Withdraw(address token, uint amount, Source source, address receiver);

    modifier markWithdrawn(Source calldata source) {
        bytes32 key = keccak256(abi.encodePacked(source.from, source.hash));
    ...
    function isWithdrawn(Source calldata source) external view returns (bool) {
        return withdrawn[keccak256(abi.encodePacked(source.from, source.hash))];
    }
    ...
    function deposit(
        ...
        emit Deposit(address(token), amount, to, bonus, receiver);
    }
    ...
    function withdraw(
    ...
    ) override external nonReentrant() markWithdrawn(source) {
        if (signature.length == 0) {
            require(hasRole(AUTHORITY_ROLE, msg.sender), "forbidden");
        } else {
            bytes32 digest = _hashTypedDataV4(keccak256(abi.encode(token, amount, source, receiver, bonus)));
        ...
        emit Withdraw(address(token), amount, source, receiver);
    }
