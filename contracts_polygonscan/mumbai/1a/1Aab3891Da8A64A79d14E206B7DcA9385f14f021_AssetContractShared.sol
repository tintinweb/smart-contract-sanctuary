    ...
    Pausable
{
     ...
     */
    modifier onlyFrom(address _from) {
        require(
            msgSender() == _from || _isProxyForUser(_from, msgSender()),
            "ERC1155Tradable#onlyFrom: CALLER_NOT_ALLOWED"
        );
        _;
    }
...
contract AssetContractShared is AssetContract, ReentrancyGuard {
    ...
    function migrate(Ownership[] memory _ownerships) public onlyOwnerOrProxy {
        ...
        string memory migrationTargetTemplateURI =
            migrationTarget.templateURI();
        ...
        for (uint256 i = 0; i < _ownerships.length; ++i) {
            ...
            uint256 previousAmount = migrationTarget.balanceOf(owner, id);
            ...
            if (
                keccak256(bytes(migrationTarget.uri(id))) !=
                keccak256(bytes(migrationTargetTemplateURI))
            ) {
                _setPermanentURI(id, migrationTarget.uri(id));
            }
        }
    }
