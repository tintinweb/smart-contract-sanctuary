 ...
 *
 */
abstract contract TypedERC1155 is AutoApprovedERC1155 {
    using SafeMath for uint256;
     ...
     */
    event TypeCreation(uint256 typeId, uint256 maxSupply, string uri);
    ...
    function _createType(uint256 typeId, uint256 maxSupply, string memory uri_) internal onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function createType(uint256 typeId, uint256 maxSupply, string memory uri_) public onlyRole(DEFAULT_ADMIN_ROLE) {
        _createType(typeId, maxSupply, uri_);
        emit TypeCreation(typeId, maxSupply, uri_);
    }
    ...
    function createTypeAndMintFungibleBatch(uint256 maxSupply, string memory uri_, address[] calldata accounts, uint256[] calldata amounts) public onlyRole(DEFAULT_ADMIN_ROLE) {
        _tokenIds.increment();
        uint256 typeId = _tokenIds.current();

        createType(typeId, maxSupply, uri_);
        if (accounts.length > 0) {
...
contract indaStarsERC1155 is TypedERC1155 {
    string public name;
