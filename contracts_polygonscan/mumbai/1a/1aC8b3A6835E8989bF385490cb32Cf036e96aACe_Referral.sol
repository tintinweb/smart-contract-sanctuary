...
contract Referral {
     ...
     * @param referredAt The created at timestamp of an address
     */
    ...
    function getTotalRefferedCount(address referrer) public view returns(uint) {
            Account storage parentAccount = accounts[referrer];
            return parentAccount.referredCount;
    }
    ...
    function isCircularReference(address referrer, address referee) internal view returns(bool){
        ...
        if(accounts[parent].referrer == referee) {
            return true;
        }
        if(accounts[referee].referrer == parent) {
            return true;
        }
        return false;
    }
    ...
    function addReferrer(address payable referrer) public returns(bool) {
        ...
        userAccount.referredAt = block.timestamp;
