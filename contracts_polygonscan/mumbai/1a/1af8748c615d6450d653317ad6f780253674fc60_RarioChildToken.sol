...
import "./rario/ChildToken/RarioChildTokenV1.sol";

contract RarioChildToken is RarioChildTokenV1 {

    function initialize(address trustedForwarder, address predicateProxy, string memory baseURI_) external initializer {

        require(trustedForwarder != address(0), "Invalid trustedForwarder Address");
        require(predicateProxy != address(0), "Invalid predicateProxy Address");
        require(bytes(baseURI_).length>0, "Invalid BaseURI");

        __RarioChildTokenV1_init(trustedForwarder, predicateProxy, baseURI_);
    }
...
import "../RarioNFT/RarioNFTV1.sol";

contract RarioChildTokenV1 is RarioNFTV1, IChildToken {
    ...
    function __RarioChildTokenV1_init(address trustedForwarder, address predicateProxy, string memory baseURI_) internal {
        __RarioNFTV1_init(trustedForwarder, baseURI_);
        _setupRole(DEPOSITOR_ROLE, predicateProxy);
    }
     ...
     */
    function mint(address user, uint256 tokenId) override external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(!withdrawnTokens[tokenId], "ChildMintableERC721: TOKEN_EXISTS_ON_ROOT_CHAIN");
        _mint(user, tokenId);
        emit TokenMinted(address(0), user, tokenId);
    }

    function version() external pure override virtual returns(uint8) {
        return 1;
    }
    uint256[49] private __gap;
}
...
import "../Collectible/IRarioCollectible.sol";
...
import "../RarioUpgradeable/RarioUpgradeableV1.sol";

contract RarioNFTV1 is
    RarioUpgradeableV1,
    ERC721Upgradeable,
    ...
    ERC721BurnableUpgradeable
{
    ...
    function __RarioNFTV1_init(address trustedForwarder, string memory baseURI_) internal {
        _rarioBaseURI = baseURI_;

        __RarioUpgradeableV1_init(trustedForwarder);
        __ERC721_init("Rario", "RARIO");
    ...
    function mint(address user, uint256 tokenId) external virtual onlyRole(MINTER_ROLE) {
        require(user != address(0), "Invalid user address");
        _mint(user, tokenId);
    }

    function _baseURI() internal view override returns (string memory) {
        return _rarioBaseURI;
    }

    function safeMint(address to, uint256 tokenId) external whenNotPaused() onlyRole(MINTER_ROLE) {
    ...
    function _msgSender() internal view  override(ContextUpgradeable, RarioUpgradeableV1) returns (address sender) {
        return RarioUpgradeableV1._msgSender();
    }

    function _msgData() internal view override(ContextUpgradeable, RarioUpgradeableV1) returns (bytes calldata) {
        return RarioUpgradeableV1._msgData();
    }
...
interface IRarioCollectible {
    event CollectibleCreated(uint256 collectibleId, uint256 assetId, string rarioId);

    function createCollectible(
        uint256 assetId,
        string calldata collectibleRarioId,
        uint16 season,
        uint16 serialNumber,
        uint8 scarcity
    ) external returns (uint256 collectibleId);

    function existsCollectible(uint256 collectibleId) external view returns (bool);

    function getCollectible(uint256 collectibleId)
        external
        view
        returns (
            uint256 assetId,
            ...
            uint8 scarcity,
            uint8 assetType,
            string memory rarioId,
            string memory schemaId,
            string memory imprint
        );
}
...
abstract contract RarioUpgradeableV1 is
    Initializable,
    PausableUpgradeable,
    ...
    ERC2771ContextUpgradeable
{
    ...
    function __RarioUpgradeableV1_init(address trustedForwarder) internal {
    ...
    function _authorizeUpgrade(address newImplementation) internal override whenPaused() onlyRole(UPGRADER_ROLE) {}

    function _msgSender() internal view  override(ContextUpgradeable, ERC2771ContextUpgradeable) virtual returns (address sender) {
