...
contract AidToken is ERC20,ERC20Snapshot,ERC20Burnable,ERC1155,ERC1155Supply,ERC1155Burnable{
	...
	constructor(
		...
		address _admin
	)  ERC20(_name, _symbol) ERC1155("https://ipfs.rumsan.com/ipfs") {
...
contract Rahat is AccessControl,ERC1155Holder {
	//***** Events *********//
	event Claimed(address indexed vendor, string indexed phone, uint256 amount);
	event ClaimApproved(address indexed vendor, string indexed phone, uint256 amount);
	event Issued(bytes32 indexed projectId, string indexed phone, uint256 amount);
	event ClaimAcquired(address indexed vendor, string indexed beneficiary);
	...
	mapping(bytes32 => uint256) public itemBalance;
	...
	//***** Constructor *********//
	constructor(AidToken _tokenContract, address _admin) {
	...
	modifier IsBeneficiary(string memory _phone) {
	    bytes32 _benId = findHash(_phone);
		require(tokenBalance[_benId] != 0, 'RAHAT: No any token was issued to this number');
		_;
	}
	modifier OnlyVendor {
	...
	function suspendBeneficiary(string memory _phone) public OnlyServer IsBeneficiary(_phone) {
	    bytes32 _benId = findHash(_phone);

		uint256 _balance = tokenBalance[_benId];
		adjustTokenDeduct(_phone, _balance);
	}

	/// @notice adds the token from beneficiary
	function adjustTokenAdd(string memory _phone, uint256 _amount) public OnlyAdmin {
	    bytes32 _benId = findHash(_phone);
		tokenBalance[_benId] = tokenBalance[_benId] + _amount;
		//emit BalanceAdjusted(_phone, _amount, _reason);
	}

	/// @notice deducts the token from beneficiary
	function adjustTokenDeduct(string memory _phone, uint256 _amount) public OnlyAdmin IsBeneficiary(_phone) {
	    bytes32 _benId = findHash(_phone);
		tokenBalance[_benId] = tokenBalance[_benId] - _amount;
		//emit BalanceAdjusted(_phone, _amount, _reason);
	}
	...
	function createClaim(string memory _phone, uint256 _tokens) public IsBeneficiary(_phone) OnlyVendor {
	    bytes32 _benId = findHash(_phone);
		require(tokenBalance[_benId] >= _tokens, 'RAHAT: Amount requested is greater than beneficiary balance.');
	...
	function createClaim(string memory _phone, uint256 _tokens, uint256 _tokenId) public IsBeneficiary(_phone) OnlyVendor {
	    bytes32 _benId = findHash(_phone);
		require(tokenBalance[_benId] >= _tokens, 'RAHAT: Amount requested is greater than beneficiary balance.');
	...
	function getTokensFromClaim(string memory _phone, string memory _otp) public IsBeneficiary(_phone) OnlyVendor {
	    bytes32 _benId = findHash(_phone);
		claim storage ac = recentClaims[msg.sender][_benId];
	...
	function getTokensFromClaim(string memory _phone, string memory _otp,uint256 _tokenId) public IsBeneficiary(_phone) OnlyVendor {
	    bytes32 _benId = findHash(_phone);
		claim storage ac = recentClaims[msg.sender][_benId];
		require(ac.isReleased, 'RAHAT: Claim has not been released.');
		require(ac.date >= block.timestamp, 'RAHAT: Claim has already expired.');
		...
		ac.amount = 0;
		ac.date = 0;
		tokenContract.safeTransferFrom(address(this),msg.sender,_tokenId, ac.amount,'');
	}
	...
	/// @notice generates the hash of the given string
	/// @param _data String of which hash is to be generated
	function findHash(uint256 _data) private pure returns (bytes32) {
		return keccak256(abi.encodePacked(_data));
	}
