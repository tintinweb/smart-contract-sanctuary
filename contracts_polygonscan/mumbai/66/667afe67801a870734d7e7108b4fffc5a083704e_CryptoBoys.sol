 ...
 * @dev see https://eips.ethereum.org/EIPS/eip-721
 */
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable, MinterRole {
...
contract CryptoBoys is ERC721 {
  ...
  // events
  event Redeem(address, uint tokenId);
  event Sell(address seller, uint tokenId, uint price);
  ...
  function mintCryptoBoy(string memory _tokenURI, uint256 _price) external onlyMinter {
  ...
  function buyToken(uint256 _tokenId) public payable {
    ...
    require(tokenOwner != msg.sender, "Buyer cannot not be owner");
    ...
    require(msg.value >= cryptoboy.price, "Underpriced");
    // token should be for sale
    require(cryptoboy.forSale, "Token not for sale");
  ...
  // redeem function 
  // basically a transfer function but emits an event 
  function redeem(uint _tokenId) public {
      require(msg.sender != address(0));
      address tokenOwner = ownerOf(_tokenId);
      require(tokenOwner == msg.sender,"Sender should be owner");
      
      _transfer(tokenOwner, address(this), _tokenId);
      emit Redeem(tokenOwner, _tokenId);
      
  }
  
  // Sell function 
  // User puts up their token on the website 
  // another user can buy and will directly send matic to the owner
  function sell(uint tokenId, uint _price) public {
      require(msg.sender != address(0));
      address tokenOwner = ownerOf(tokenId);
    //   address previousOwner = tokenOwner;
      require(tokenOwner == msg.sender,"Sender should be owner");
      
      CryptoBoy memory cryptoboy = allCryptoBoys[tokenId];
      
      cryptoboy.price = _price;
      
      cryptoboy.forSale = true;
      
      allCryptoBoys[tokenId] = cryptoboy;
      
      _transfer(tokenOwner, address(this), tokenId);
      emit Sell(tokenOwner, tokenId, _price);
  }
