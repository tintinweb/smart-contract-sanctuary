...
contract UFO is ERC721, ERC721Burnable, ERC721Holder, UFOStorage {
    ...
    function RedeemOrigin() public {
        ...
        UfoDetails memory ufo = _generateUfoWithRandomFeatures(totalNumberOfUFOs);
        ufo.ufoType = UfoType.origin;
        ufoStore[totalNumberOfUFOs] = ufo;
        _safeMint(msg.sender, totalNumberOfUFOs);
    }

    function CreateType2(address _to) public onlyBreeder {
        totalNumberOfUFOs = totalNumberOfUFOs.add(1);
        UfoDetails memory ufo = _generateUfoWithRandomFeatures(totalNumberOfUFOs);
        ufo.ufoType = UfoType.type2;
        ufoStore[totalNumberOfUFOs] = ufo;
        _safeMint(_to, totalNumberOfUFOs);
    }

    function CreateType1(address _to) public onlyBreeder {
        totalNumberOfUFOs = totalNumberOfUFOs.add(1);
        UfoDetails memory ufo = _generateUfoWithRandomFeatures(totalNumberOfUFOs);
        ufo.ufoType = UfoType.type1;
        ufoStore[totalNumberOfUFOs] = ufo;
        _safeMint(_to, totalNumberOfUFOs);
    }

    function _generateUfoWithRandomFeatures(uint256 id) internal view returns (UfoDetails memory _ufo) {
        ...
        for (uint256 index = 1; index <= featureCounts; index++) {
            ...
            _ufo.values[index.sub(1)] = (uint256(sha256(abi.encode(block.difficulty.add(index)))) % possibilities).add(1);
        }
    }
