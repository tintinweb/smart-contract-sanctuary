...
contract Locker is ILocker {
    ...
    event Unlock(address indexed addr, uint256 lockingAmount, uint256 penaltyAmount, uint256 totalRemainingLockingAmount);
    ...
    function lock(uint256 lockingAmount, address lockedFor) public {
        knightToken.transferFrom(msg.sender, address(this), lockingAmount);

        LockerState storage lockerState = lockersState[msg.sender];
        lockerState.currentLockingAmount = lockerState.currentLockingAmount.add(lockingAmount);
        lockerState.lockTime = block.timestamp;

        if (lockedFor != msg.sender) {
            lockerState.delegatedFrom += lockerState.delegatedFrom.add(lockingAmount);
            lockersState[lockedFor].delegatedTo = lockersState[lockedFor].delegatedTo.add(lockingAmount);
            delegations[msg.sender][lockedFor] = delegations[msg.sender][lockedFor].add(lockingAmount);
    ...
    function unlock (uint256 unlockingAmount, address lockedFor) public {
        LockerState storage lockerState = lockersState[msg.sender];

        require(lockerState.currentLockingAmount >= unlockingAmount);

        if (lockedFor != msg.sender) {
            require(lockerState.delegatedFrom >= unlockingAmount);
            require(delegations[msg.sender][lockedFor] >= unlockingAmount);

            lockerState.delegatedFrom = lockerState.delegatedFrom.sub(unlockingAmount);
            lockersState[lockedFor].delegatedTo = lockersState[lockedFor].delegatedTo.sub(unlockingAmount);
            delegations[msg.sender][lockedFor] = delegations[msg.sender][lockedFor].sub(unlockingAmount);
            emit Undelegate(msg.sender, lockedFor, unlockingAmount);
        }

        lockerState.currentLockingAmount = lockerState.currentLockingAmount.sub(unlockingAmount);
        uint256 payingAmount = unlockingAmount;
        uint256 penaltyAmount = 0;

        if (block.timestamp - lockerState.lockTime < 14 days) {
            penaltyAmount = unlockingAmount.mul(2).div(100);

            if (penaltyAmount > 0) {
                payingAmount = payingAmount.sub(penaltyAmount);
    ...
    function getLockerState(address addr) public override view returns(LockerState memory) {
