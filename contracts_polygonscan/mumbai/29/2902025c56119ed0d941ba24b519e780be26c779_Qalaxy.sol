...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    event accountLockUpdated(address indexed account, uint256 timePeriod);
    ...
    function _transfer(
        ...
        require(_lockTime[sender] <= block.timestamp, "Lock: currently sender address is locked");
    ...
    function lock(address[] memory accounts, uint256[] memory setTime) public onlyOwner returns(bool) {
        require(accounts.length == setTime.length,"account and time length is mismatched ");
        for(uint256 i = 0; i < accounts.length; i++ ){
            _lockTime[accounts[i]] = block.timestamp + (setTime[i] * (1 days));
    ...
    function unLockAddress(address account) public onlyOwner returns(bool) {
        require(_lockTime[account] != 0, "Lock: account already unlocked");
        _lockTime[account] = 0;
    ...
    function getAccountStatus(address account) public view returns(bool) {
        bool status = _lockTime[account] >= block.timestamp ? true 
                                              : false ;
        return status;
    }
}
