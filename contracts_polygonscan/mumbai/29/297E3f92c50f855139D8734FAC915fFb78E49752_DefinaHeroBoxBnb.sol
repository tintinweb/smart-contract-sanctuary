...
ReentrancyGuard
{
    ...
    event Open(uint indexed tokenId_, uint indexed nftId_);
    ...
    function initialize(IDefinaCard nftToken_, string memory baseTokenURI,
        uint nftPrice_, address tokenReceiveAddress_, uint[] calldata cardIds_,
    ...
    function refreshSeed(uint seed_) onlyRole(ADMIN_ROLE) external {
        seed = seed_ + block.timestamp + nftToken.balanceOf(address(this));
    }
    ...
    function mintMultiTokenForCaller(uint amount) payable whenNotPaused onlyEOA nonReentrant external {
        require(amount > 0, "BlindBox: missing amount");
        uint cost = nftPrice.mul(amount);
        require(msg.value >= cost, 'BNB not enough');
        ...
        // (bool sent, bytes memory data) = tokenReceiveAddress.call{value: msg.value}("");
        // require(sent, "Failed to send Ether receive address.");
    ...
    function pullFunds(address tokenAddress_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        if (tokenAddress_ == address(0)) {
            payable(tokenReceiveAddress).transfer(address(this).balance);
        } else {
            IERC20 token = IERC20(tokenAddress_);
            token.transfer(tokenReceiveAddress, token.balanceOf(address(this)));
        }
    }
}
