...
/*
 * On deployment a new Uniswap pool will be created for the pair USDH/ETH and its token will be set here.
 ...
 * Funds for rewards will only be added once, on deployment of HALAL token,
 ...
 * in order to avoid getting HALAL tokens locked.
 * That also means that the start time for the program will be the event that occurs first:
 * either HALAL token contract is deployed, and therefore HALAL tokens are minted to Unipool contract,
...
contract Unipool is LPTokenWrapper, Ownable, CheckContract, IUnipool {
        ...
        onlyOwner
    {
        ...
        _notifyRewardAmount(halalToken.getLpRewardsEntitlement(), _duration);

        emit HALALTokenAddressChanged(_halalTokenAddress);
    ...
    function claimReward() public override {
        ...
        halalToken.transfer(msg.sender, reward);
    ...
    function _notifyRewardAmount(uint256 _reward, uint256 _duration) internal {
        assert(_reward > 0);
        assert(_reward == halalToken.balanceOf(address(this)));
...
interface IUnipool {
    function setParams(address _halalTokenAddress, address _uniTokenAddress, uint256 _duration) external;
