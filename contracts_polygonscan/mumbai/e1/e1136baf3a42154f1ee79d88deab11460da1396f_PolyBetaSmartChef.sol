...
contract PolyBetaSmartChef is Ownable {
    ...
    struct UserInfo {
        ...
        uint256 lastDepositTime; // Last time the user deposited in pool.
    }
    ...
    // Maximum amount of BETA that can be deposited
    ...
    // The withdraw lockup interval
    uint256 public withdrawInterval;
    // Max withdraw lockup interval: 40 days.
    uint256 public constant MAXIMUM_WITHDRAW_INTERVAL = 40 days;
    ...
    function withdraw(uint256 _amount) public {
        ...
        require(user.lastDepositTime.add(withdrawInterval) >= block.timestamp, 'withdraw: cannot withdraw before lockup time');
        updatePool();
        uint256 pending = user.amount.mul(pool.accRewardPerShare).div(1e18).sub(user.rewardDebt);
        if(pending > 0) {
            rewardToken.safeTransfer(address(msg.sender), pending);
    ...
    function updateWithdrawLockupInterval(uint256 _withdrawInterval) external onlyOwner {
        require(_withdrawInterval <= MAXIMUM_WITHDRAW_INTERVAL, 'withdraw interval: invalid; cannot be greater than MAXIMUM_WITHDRAW_INTERVAL');
