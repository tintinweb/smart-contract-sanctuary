...
contract EtheremonTransformData {
    ...
    function getHatchingEggId(address _trainer)
        external
        constant
        returns (uint64);

    function getHatchingEggData(address _trainer)
        external
        constant
        returns (
            uint64,
    ...
    function getTranformedId(uint64 _objId) external constant returns (uint64);

    function countEgg(uint64 _objId) external constant returns (uint256);

    function setHatchTime(uint64 _eggId, uint256 _hatchTime) external;
...
contract EtheremonWorld {
    ...
    function getTrainerEarn(address _trainer) public constant returns (uint256);
    ...
    function getClassPropertyValue(
        uint32 _classId,
        EtheremonEnum.PropertyType _type,
        uint256 index
...
interface EtheremonBattle {
    ...
    function getMonsterLevel(uint64 _objId) public constant returns (uint8);
}
...
interface EtheremonTransformSettingInterface {
    function getRandomClassId(uint256 _seed) external constant returns (uint32);
...
contract EtheremonTransform is EtheremonEnum, BasicAccessControl, SafeMath {
    ...
    function getRandom(
        ...
        return uint256(keccak256(block.blockhash(_block), _player, _count));
    }
 ...
 */
    ...
    // read
    // function getBonusEgg(uint64 _objId) public constant returns (uint256) {
    //     EtheremonTransformData transformData = EtheremonTransformData(
    //         transformDataContract
    //     );
