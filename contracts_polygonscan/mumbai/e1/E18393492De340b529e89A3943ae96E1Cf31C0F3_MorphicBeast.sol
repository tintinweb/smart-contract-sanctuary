...
contract MorphicBeast is ERC1155, Ownable, Pausable {
    ...
    uint256 public SOLD_OUT_ELEMENTS;
    ...
    function mint(uint256 amount) public payable saleIsOpen {
        ...
        require(total + amount <= (MAX_ELEMENTS - SOLD_OUT_ELEMENTS), "Mint: over Max limit");
        require(amount <= MAX_BY_MINT, "Exceeds number");
        require(balanceOf(msg.sender, _tokenIdTracker.current()) + amount <= MAX_PER_ADDRESS, "Exceeds balance");        
        require(msg.value >= price(amount), "Value below price");
        _tokenIdTracker.increment();
        _mint(msg.sender, _tokenIdTracker.current(), amount, "");
    }
