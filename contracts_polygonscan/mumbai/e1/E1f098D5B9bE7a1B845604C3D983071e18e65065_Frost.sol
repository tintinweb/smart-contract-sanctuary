...
contract Frost is ERC20, AccessControl, ERC721Holder {
    ...
    constructor(address owner, address _collectionAddress) ERC20("Frost", "FROST") AccessControl(){
        // mint initial coins to contract owners
        ...
        welcomeBonusAmount = 5 * 10 ** 18; // 5 tokens welcome bonus, only paid once per tokenId
    }
    ...
    function pendingRewards(address owner, uint256 tokenId) public view returns (uint256 pendingRewardInERC20Tokens){
        StakeInfo memory info = stakeLog[owner][tokenId];

        if(info.lastHarvestBlock < contractCreationBlock) {
            return 0;
        }
        uint256 blocksPassedSinceLastHarvest = block.number - info.lastHarvestBlock;
        pendingRewardInERC20Tokens = (blocksPassedSinceLastHarvest / numberOfBlocksPerRewardUnit - 1) * coinAmountPerRewardUnit;
    }
    ...
    function stake(uint256 tokenId) external {
        ...
        require(IERC721(collectionAddress).ownerOf(tokenId) == address(this),
            "FROST: Error while transferring token");
    ...
    function harvest(uint256 tokenId) public {
        ...
        require( rewardAmountInERC20Tokens > 0, "FROST: Nothing to harvest");
        info.lastHarvestBlock = block.number;
    ...
    function setCollectionAddress(address newAddress) external onlyRole(CONTRACT_ADMIN_ROLE){
        require (newAddress != address(0), "FROST: update to zero address not possible");
