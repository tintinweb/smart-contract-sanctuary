...
contract NFTTRADER{
    ...
    function addListing(uint256 price, address contractAddr, uint256 tokenId) public {
        ...
        require(token.balanceOf(msg.sender, tokenId) >= 0, "Le vendeur doit posseder le nft");
        require(token.isApprovedForAll(msg.sender, address(this)), "Le contrat doit etre valide");
    ...
    function purchase(address contractAddr, uint256 tokenId, uint256 amount) public payable{
        ...
        ERC1155 token = ERC1155(contractAddr);
        require(msg.value >= item.price * amount, "Fonds insuffisants");
        require(amount <= token.balanceOf(item.seller, tokenId),"amount pas possible");
    ...
    function withdraw(uint256 amount, address payable destAddr) public {
        require(amount<= balances[msg.sender], "Fonds Insuffisants");
