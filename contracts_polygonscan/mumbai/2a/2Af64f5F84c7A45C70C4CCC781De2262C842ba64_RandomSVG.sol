...
library SVGShapes {
    ...
    function generateShape(uint256 _randomNumber) internal pure returns(string memory pathSvg) {
        ...
        if (_randomNumber % 2 == 0) {
            ...
            //uint256[] memory circleVars = expand(_randomNumber, 7);
            //pathSvg = string(abi.encodePacked(pathSvg, "<circle cx='", string(Strings.toString(circleVars[0] % 500)), "' cy='", string(Strings.toString(circleVars[1] % 500)), "' r='", string(Strings.toString(circleVars[2] % (500 / 2))), "' stroke='blue' stroke-width='", string(Strings.toString(circleVars[4] % (500 / 5))), "' fill='red' />"));
            ...
            //pathSvg = string(abi.encodePacked(pathSvg, "<circle cx='", string(Strings.toString(circleVars[0] % 500)), "' cy='", string(Strings.toString(circleVars[1] % 500)), "' r='", string(Strings.toString(circleVars[2] % 250)), "' stroke=rgb(255,67,189", ") stroke-width='", string(Strings.toString(circleVars[6] % 100)), "' fill=transparent />"));
            //pathSvg = string(abi.encodePacked(pathSvg, "<circle cx='50' cy='50' r='40' stroke='rgb(255,0,0)' stroke-width='4' fill='transparent' />"));
            } else {
            //draw a square
            uint256[] memory rectVars = expand((_randomNumber + 1), 7);
            //pathSvg = string(abi.encodePacked(pathSvg, "<rect x='", string(Strings.toString(rectVars[0] % 500)), "' y='", string(Strings.toString(rectVars[1] % 500)), "' width='", string(Strings.toString(rectVars[2] % (500 / 2))),"' height='", string(Strings.toString(rectVars[3] % (500 / 2))), "' stroke='green' stroke-width='", string(Strings.toString(rectVars[5] % (500 / 5))), "' fill='yellow' />"));
            //uint256[] memory rectVars = expand((_randomNumber + 1), 7);
...
contract RandomSVG is ChainlinkClient, ERC721URIStorage, VRFConsumerBase {
    ...
    event CreatedRandomSVG(uint256 indexed tokenId, string tokenURI);
     ...
     constructor(address _VFRCoordinator, address _LinkToken, bytes32 _keyHash, bytes32 _jobId, uint256 _fee, address _oracle)
     ...
     ERC721 ("RandomSVG", "rsNFT") {
        ...
        tokenCounter = 0;
        oracle = _oracle;
        //jobId = '54bd0a3d8fb54da6bd8da32b17912ed3'; //mumbai
        //jobId = '06a1aed457cb4938b0d5f928ff9f328f'; //rineby?
        jobId = _jobId;
