...
contract ERC721GlipLive is ERC721Base {

    event CreateERC721GlipLive(address owner, string name, string symbol);

    // address transferProxy, address lazyTransferProxy
    function __ERC721GlipLive_init(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI, address exchangeProxy) public initializer {
        __ERC721GlipLive_init_unchained(_name, _symbol, baseURI, contractURI, exchangeProxy); // transferProxy, lazyTransferProxy
        emit CreateERC721GlipLive(_msgSender(), _name, _symbol);
    }

    // address transferProxy, address lazyTransferProxy
    function __ERC721GlipLive_init_unchained(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI, address exchangeProxy) internal {
        _setBaseURI(baseURI);
    ...
    RoyaltiesV2Impl
{
    ...
    function verifyLazyAsset(LibERC721LazyMint.Mint721Data memory data)
        internal
        view
        virtual
        returns (bool)
        ...
        )
    {
        ...
        return (
            ...
            data.creators[data.creators.length - 1].account
        );
    }
