...
contract SimpleCollectible is ERC721, RoyaltiesV2Impl, Ownable {
    ...
    function createCollectible(string memory myTokenURI, uint256 NFTnum)
        public
        payable
        returns (uint256)
    {
        uint256 newTokenId = tokenCounter;
        if (NFTnum == newTokenId) {
            _safeMint(msg.sender, newTokenId);
            _setTokenURI(newTokenId, myTokenURI);
            setRoyalties(tokenCounter);
            // Set the Floor price
            // $222 USD is 0.050 ETH as of Oct 27, 2021
            uint256 floorPrice = 0.050 * 10**18;
            ...
            // set the returned tokenid to 100000 to  indicate we could not mint
