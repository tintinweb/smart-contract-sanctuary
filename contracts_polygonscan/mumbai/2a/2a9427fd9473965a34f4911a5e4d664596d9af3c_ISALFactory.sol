...
contract PancakeERC20 is IPancakeERC20 {
    ...
    string public constant symbol = 'ISW-LP';
...
contract ISALFactory is IPancakeFactory {
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        require(tokenA != tokenB, 'ISAL: IDENTICAL_ADDRESSES');
        (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'ISAL: ZERO_ADDRESS');
        require(getPair[token0][token1] == address(0), 'ISAL: PAIR_EXISTS'); // single check is sufficient
    ...
    function setFeeTo(address _feeTo) external {
        require(msg.sender == feeToSetter, 'ISAL: FORBIDDEN');
        feeTo = _feeTo;
    }
