...
contract MarvelousNFT_PoS is ERC20Burnable, ERC20Pausable, AccessControl, AccessControlMixin, ContextMixin, IChildToken, NativeMetaTransaction, AccessRights  {
    ...
    function _partialBurn(address from, uint256 amount) internal returns (uint256) {
        ...
        if (_burnAmount > 0) {
            ...
            //1% for burning tokens or 20% of the tokens to be burned
    ...
    function mintTokens(address account, uint256 amount) external only(MINTER_ROLE) {
        uint256 minted = mintedTokens.add(amount);
        require(minted <= mintableTokens, "MarvelousNFT: amount exceeds mintable tokens.");
