...
interface IWolf {
	...
	struct SheepWolf {
		...
		uint256 strength; //more is better
	}
...
contract Wolf is IWolf, ERC721Enumerable, Ownable, ReentrancyGuard {
	...
	uint256 public SHEEP_GEN_PERCENTAGE = 90; //90% of minted tokens are sheeps
	...
	event WolfUpgraded(uint256 wolfID, bool success);
	...
	function delete333Me() internal pure {} //polyscan trick
	...
	function mint(uint256 qty) external payable nonReentrant {
		...
		for (uint256 i = 0; i < qty; i++) {
			seed = random(100, 2**256 - 1, totalSupply());
			...
			uint256 isSheepInt = seed % 100;
			bool isSheep = true;
			if (isSheepInt >= SHEEP_GEN_PERCENTAGE) {
				isSheep = false;
			}

			tokenTraits[totalSupply() + 1] = SheepWolf(isSheep, 1);
			_safeMint(recipient, totalSupply() + 1);
		}
	}

	//increase the STR of a woof whenever it eats a sheep
	function upgradeWolf(uint256 wolfID, uint256 sheepID) external nonReentrant {
		require(ownerOf(wolfID) == msg.sender, "wolf not owned by caller");
		require(ownerOf(sheepID) == msg.sender, "sheep not owned by caller");

		//1. burn the sheep
		_burn(sheepID);

		//30% to succeed
		uint256 seed = random(100, 2**256 - 1, wolfID + sheepID);
		if ((seed % 100) > 30) {
			_burn(wolfID);
			emit WolfUpgraded(wolfID, false);
			return;
		}

		//2. if not burned, increase wolf's str
		if (tokenTraits[wolfID].strength < 15) {
			tokenTraits[wolfID].strength = tokenTraits[wolfID].strength + 1;
		}
		emit WolfUpgraded(wolfID, true);
	}
	...
	//if needed to re-equlibrate the economy
	function changeSheepPercentage(uint256 newPercent) external onlyOwner {
		require(newPercent > 1 && newPercent < 99, "invalid amounts");
		SHEEP_GEN_PERCENTAGE = newPercent;
	}
