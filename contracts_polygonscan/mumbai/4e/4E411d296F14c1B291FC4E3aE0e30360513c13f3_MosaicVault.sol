    ...
    PausableUpgradeable
{
        ...
        whenNotPausedNetwork(_remoteNetworkID)
    {
        ...
        require(vaultConfig.wethAddress() != address(0), "WETH not set");

        // check if ETH transfer is possible
        require(
            vaultConfig.remoteTokenAddress(
                _remoteNetworkID,
    ...
    function _provideLiquidity(
        ...
        if (_blocksForActiveLiquidity > 0) {
            IReceiptBase(vaultConfig.getUnderlyingIOUAddress(_tokenAddress))
                .mint(msg.sender, _amount);
            _updateAvailableTokenAfter(
                _tokenAddress,
                _blocksForActiveLiquidity
            );
            ...
            IReceiptBase(vaultConfig.getUnderlyingReceiptAddress(_tokenAddress))
                .mint(msg.sender, _amount);
            emit DepositPassiveLiquidity(_tokenAddress, msg.sender, _amount);
        }
    }

    function _withdrawLiquidityRequest(
        ...
        address _tokenIn = IReceiptBase(_receiptToken).underlyingToken();
        address remoteTokenIn = _tokenIn;

        if (IReceiptBase(_receiptToken).isIOU()) {
            // is active liquidity
            require(
                vaultConfig.getUnderlyingIOUAddress(_tokenIn) == _receiptToken,
                "TOKEN NOT WHITELISTED"
            );
            // check if liquidity period is over
            require(
                availableAfterBlock[msg.sender][_tokenIn] <= block.number,
                "CANNOT WITHDRAW IN THIS BLOCK"
            );
        } else {
            // passive liquidity
            require(
                vaultConfig.getUnderlyingReceiptAddress(_tokenIn) ==
                    _receiptToken,
                "TOKEN NOT WHITELISTED"
            );
            ...
            if (_networkId != block.chainid) {
                remoteTokenIn = vaultConfig.remoteTokenAddress(
                    _networkId,
                    _tokenIn
                );
            }
    ...
    function withdrawLiquidityRequest(
        ...
        uint256 _ammID,
        bytes calldata _data
    ) external override nonReentrant validAddress(_tokenOut) {
        _withdrawLiquidityRequest(
            _receiptToken,
            _amountIn,
            _tokenOut,
            _amountOutMin,
    ...
    /// @param _receiptToken Address of the iou token provider have
    /// @param _remoteTokenOut Address of the token which LP want to receive on another network
    /// @param _amountIn Amount of tokens provider want to withdraw
    /// @param _remoteNetworkID Id of the network want to receive the other token
    /// @param _amountOutMin Minimum amount of token user should get
    function withdrawLiquidityOnAnotherNetworkRequest(
        address _receiptToken,
        uint256 _amountIn,
        address _remoteTokenOut,
        uint256 _amountOutMin,
        address _remoteDestinationAddress,
        uint256 _remoteAmmId,
        bytes calldata _data,
        ...
        override
        nonReentrant
        onlyWhitelistedRemoteTokens(
            _remoteNetworkID,
            IReceiptBase(_receiptToken).underlyingToken()
        )
    {
        _withdrawLiquidityRequest(
            _receiptToken,
    ...
    function withdrawLiquidity(
        ...
        require(
            vaultConfig.getCurrentTokenLiquidity(_tokenIn) >= _amountIn,
            "ERR: VAULT BAL"
        );
    ...
    function _burnReceiptTokens(
        ...
        require(_amount > 0, "ERR: AMOUNT");
        IReceiptBase receipt = IReceiptBase(_iouTokenAddress);
        require(receipt.balanceOf(_provider) >= _amount, "Balance to low");
    ...
    function _takeFees(
        ...
        uint256 fee = FeeOperations.getFeeAbsolute(
            _amount,
            vaultConfig.calculateFeePercentage(_token, _amount)
        );
        ...
        validAddress(_to)
    {
        ...
        saveFundsTimer = block.timestamp + vaultConfig.saveFundsLockupTime();
    ...
    */
    ...
    modifier whenNotPausedNetwork(uint256 _networkID) {
        ...
        require(
            vaultConfig.pausedNetwork(_networkID) == false,
            "Network is paused"
        );
        _;
    }
...
interface IMosaicVault {
   ...
   * @dev used to provide passive liquidity.
    ...
    /**
     * @dev used to withdraw liquidity on another L2 network.
     * @param _receiptToken Address of the token provider have
     * @param _remoteTokenOut Address of the token provider want to receive
     * @param _amountIn Amount of tokens provider want to withdraw
     * @param _remoteNetworkID Id of the network want to receive the other token
     * @param _amountOutMin Minimum amount of token user should get
     * @param _remoteDestinationAddress SC address of the ERC20 supported tokens in a diff network
     * @param _remoteAmmId remote token amm id
     */
