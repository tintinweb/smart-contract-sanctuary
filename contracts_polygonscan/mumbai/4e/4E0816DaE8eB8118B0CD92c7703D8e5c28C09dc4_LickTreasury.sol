...
// File: contracts/LickTreasury.sol
...
contract LickTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SLICK }
    ...
    address public sLICK;
    uint public sLICKQueue; // Delays change to sLICK address
    ...
    constructor (
        ...
        isLiquidityToken[ _LICKDAI ] = true;
        liquidityTokens.push( _LICKDAI );
        bondCalculator[_LICKDAI] = _bondCalculator;

        blocksNeededForQueue = _blocksNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for LICK
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint LICK needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( LICK ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn LICK for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOfToken( _token, _amount );
        ILICKERC20( LICK ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sLICK ).balanceOf( msg.sender ); // Can only borrow against sLICK held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with LICK
        @param _amount uint
     */
    function repayDebtWithLICK( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        ILICKERC20( LICK ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, LICK, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( LICK ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( LICK ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns LICK valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOfToken( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match LICK decimals
            value_ = _amount.mul( 10 ** IERC20( LICK ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyOwner() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SLICK ) { // 9
            sLICKQueue = block.number.add( blocksNeededForQueue );
        } else return false;
