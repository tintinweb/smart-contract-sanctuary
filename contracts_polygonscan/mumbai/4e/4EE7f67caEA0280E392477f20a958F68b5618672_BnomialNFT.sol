...
contract BnomialNFT is ERC721, Ownable {
    ...
    function renderCard(
        ...
        svg = string(abi.encodePacked(svg, '<g transform="translate('));
        svg = string(abi.encodePacked(svg, Strings.toString(number * 70)));
        svg = string(
            abi.encodePacked(
                svg,
        ...
        svg = string(abi.encodePacked(svg, Strings.toString(id)));
        svg = string(
            abi.encodePacked(
                svg,
        ...
        svg = string(abi.encodePacked(svg, name));
        svg = string(abi.encodePacked(svg, "</text></g>"));
    }

    function renderFloat(uint256 number) public pure returns (string memory svg) {
        if (number == 1000) {
            svg = string(abi.encodePacked(svg, "1"));
        } else {
            string memory numberString = Strings.toString(number);
            svg = string(abi.encodePacked(svg, "0."));
            for (uint256 i = bytes(numberString).length; i < 6; i++) {
                svg = string(abi.encodePacked(svg, "0"));
            }
            svg = string(abi.encodePacked(svg, numberString));
        }
    }

    function renderAnimationTimes(uint256 badgesCount) public pure returns (string memory svg) {
        uint256 badgeTime = 1000000 / badgesCount;
        uint256 showTime = (9 * badgeTime) / 10;

        for (uint256 i = 0; i < badgesCount; i++) {
            svg = string(abi.encodePacked(svg, renderFloat(i * badgeTime)));
            svg = string(abi.encodePacked(svg, ";"));
            svg = string(abi.encodePacked(svg, renderFloat(i * badgeTime + showTime)));
            svg = string(abi.encodePacked(svg, ";"));
        }
    ...
    function renderAnimationValues(uint256 badgesCount) public pure returns (string memory svg) {
        uint256 offsetX = 70;

        svg = string(abi.encodePacked(svg, "0 0;0 0;"));

        for (uint256 i = 1; i < badgesCount; i++) {
            svg = string(abi.encodePacked(svg, "-"));
            svg = string(abi.encodePacked(svg, Strings.toString(i * offsetX)));
            svg = string(abi.encodePacked(svg, " 0;"));
            svg = string(abi.encodePacked(svg, "-"));
            svg = string(abi.encodePacked(svg, Strings.toString(i * offsetX)));
            svg = string(abi.encodePacked(svg, " 0;"));
        ...
        svg = string(abi.encodePacked(svg, "0 0"));
    }

    function renderAnimation(uint256 badgesCount) public pure returns (string memory svg) {
        // <animatetransform attributename="transform" dur="5s" begin="0s" repeatcount="indefinite" type="translate" keytimes="0;0.3;0.33;0.63;0.66;0.96;1" values="0 0;0 0;-70 0;-70 0;-140 0;-140 0;0 0" calcmode="spline"></animatetransform>
        svg = string(
            abi.encodePacked(
                svg,
                '<animateTransform attributeName="transform" dur="5s" begin="0s" repeatCount="indefinite" type="translate" keyTimes="'
            )
        );

        svg = string(abi.encodePacked(svg, renderAnimationTimes(badgesCount)));
        svg = string(abi.encodePacked(svg, ""));
        svg = string(abi.encodePacked(svg, '" values="'));
        svg = string(abi.encodePacked(svg, renderAnimationValues(badgesCount)));
        svg = string(abi.encodePacked(svg, ""));
        svg = string(abi.encodePacked(svg, '" calcMode="spline"></animateTransform>'));
    }
    ...
    function renderSVG(uint256 tokenId) public view returns (string memory svg) {
        ...
        // SVG header
        svg = string(abi.encodePacked('<svg xmlns="http://www.w3.org/2000/svg" version="1.2" viewBox="0 0 100 100">'));
        ...
        svg = string(abi.encodePacked(svg, toAsciiString(owner)));
        svg = string(abi.encodePacked(svg, "</text></g>"));
        ...
        for (uint256 i = 0; i < badges.length; i++) {
            svg = string(abi.encodePacked(svg, renderCard(badges[i], _badgeNames[badges[i]], i)));
        }

        // Animation
        svg = string(abi.encodePacked(svg, renderAnimation(badges.length)));
        ...
        svg = string(abi.encodePacked(svg, Strings.toString(badges.length)));
        svg = string(abi.encodePacked(svg, "</text></g>"));
    ...
    function renderSVG2(uint256 tokenId) public view returns (string memory svg) {
        require(_exists(tokenId), "ERC721Metadata: URI query for nonexistent token");
        ...
        // Badge header
        svg = string(abi.encodePacked(svg, '<rect x="0" y="0" width="100" height="100" fill="#0000ff"/>'));
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        jsonData = string(abi.encodePacked(jsonData, '{"name":"Bnomial Badges",'));
        jsonData = string(
            abi.encodePacked(
                jsonData,
        ...
        jsonData = string(abi.encodePacked(jsonData, '"attributes": [],'));

        jsonData = string(abi.encodePacked(jsonData, '"image":"data:image/svg+xml;base64,'));

        string memory svg = renderSVG2(tokenId);
        jsonData = string(abi.encodePacked(jsonData, Base64.encode(bytes(svg))));
        jsonData = string(abi.encodePacked(jsonData, '"}'));

        string memory json = Base64.encode(bytes(jsonData));
