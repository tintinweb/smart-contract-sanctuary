...
contract HousePool{
    ...
    function deposit() public payable CheckZero returns(bool success) {
        ...
        //Extra
        user.LossDept =(user.Balance *(AccLossPerShare)/(1e18));//assigning the rewards 
    ...
    function withdraw(uint amount,uint _box) public ValidBox(msg.sender,_box) returns(bool success) 
    {
    ...
    //Owner functions
    function Transfer(uint _amount)public OnlyBetting  {
    ...
    function EmergencyTransfer(uint _amount,address _user)public OnlyOwner{
        require(_amount > 0,"The input amount is very less");
        require(_amount <= address(this).balance,"The amount is too high");
        if(_amount <= TotalValueLocked)
        {
    ...
    function CalculateAccumulatePerShare(uint _amount)public OnlyBetting{
