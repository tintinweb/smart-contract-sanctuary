...
// File: contracts/orion/ISaver.sol
...
// File: contracts/orion/StakeOrion.sol
...
contract StakeOrion {
    ...
    modifier onlyOwner() {
        require(msg.sender == SOManager(manager).owner(), "restricted to creator of the factory contract");
        _;
    }
    ...
    function deposit(uint256 amount) external onlyManager returns (uint256) {
        ...
        if (orion.canDepositLocal(stakingToken, amount)) {
            if (
                ...
                amount <= (orion.getLocalDepositLimit() * decimals)
            ) {
    ...
    function _depositExt(uint256 amount) internal {
        if (amount <= orion.getDepositLimit() * decimals) {
    ...
    function getReward() external onlyManager returns (uint256) {
        ...
        uint256 interest = current - balance;
        uint256 fee = _fee(interest);

        if (
            orion.canWithdrawLocal(stakingToken, interest) &&
            interest <= orion.getLocalWithdrawLimit() * decimals
        ) {
    ...
    function _withdraw(uint256 amount) internal returns (uint256) {
        ...
        if (amount == getStakedBalance()) {
            ...
            fee = _fee(interest);
            // todo if still not working try annulating some decimals
            toWithdraw = current;
        }
        if (
            orion.canWithdrawLocal(stakingToken, toWithdraw) &&
            toWithdraw <= orion.getLocalWithdrawLimit() * decimals
        ) {
    ...
    function _withdrawExt(uint256 amount) internal returns (uint256) {
        if (amount <= (orion.getWithdrawLimit() * decimals) && !pending) {
    ...
    function earned() public view returns (uint256) {
        uint256 balance;
        uint256 current;
        (balance, , current) = orion.balanceOf(stakingToken, address(this));
        uint256 diff = current - balance;
        return diff - _fee(diff);
    }
...
// File: contracts/orion/StakeOrionManager.sol
