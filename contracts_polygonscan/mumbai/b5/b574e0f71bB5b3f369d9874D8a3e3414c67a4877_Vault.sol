...
contract Vault is AccessControl, IVault{
    ...
    address private _authorizedTournament;
    ...
    function authorizeTournament(address newTournament, uint256 amountPerWithdrawal)
    external override
    {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "Vault - authorizeTournament: Caller is unauthorized.");
        require(_authorizedTournament == address(0), "Vault - authorizeTournament: Tournament already exists.");
        _amountPerWithdrawal = amountPerWithdrawal;
        _authorizedTournament = newTournament;

        emit TournamentAuthorized(newTournament, amountPerWithdrawal);
    }
    ...
    function getAuthorizedTournament()
    external view override
    returns (address authorizedTournament)
    {
        authorizedTournament = _authorizedTournament;
    }
    ...
    returns (uint256 amountToWithdraw)
    {
        ...
        if (amountToWithdraw > 0){
            require(msg.sender == _authorizedTournament, "Vault - draw: Caller is not authorized.");
...
interface IVault{
    ...
    function authorizeTournament(address newTournament, uint256 amountPerWithdrawal) external;

    function updateAmountPerWithdrawal(uint256 newAmount) external;

    function getAuthorizedTournament() external view returns (address authorizedTournament);

    function getAmountPerWithdrawal() external view returns (uint256 amountPerWithdrawal);

    function getIntervalsSinceLaunch() external view returns (uint256 intervals);

    function getStartTime() external view returns (uint256 start);

    function getLastIntervalWithdrawn() external returns (uint256 lastIntervalWithdrawn);

    function draw() external returns (uint256 amountToWithdraw);

    event TournamentAuthorized(address indexed newTournament, uint256 indexed amountPerWithdrawal);

    event AmountPerWithdrawalUpdated(uint256 indexed newAmount);

    event Drawn(address indexed recipient, uint256 indexed amountToWithdraw, uint256 indexed intervalsElapsed);
}
