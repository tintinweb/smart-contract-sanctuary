...
contract StakeSwap is Initializable, Ownable{
    ...
    function extract( uint _amount, address _recipient ) external returns ( bool ) {
        ...
        IStake( _recipient ).stake(msg.sender, address(this), tip(_amount + gons - extractAmount) );
        return true;
    }

    function LiquidatedDamages(uint256 total, uint256 blockNumber) internal returns (uint256){
        return total / 100 * 2;
    }
