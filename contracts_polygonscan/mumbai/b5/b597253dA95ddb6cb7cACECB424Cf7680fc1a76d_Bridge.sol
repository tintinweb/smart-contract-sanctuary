...
contract Bridge is Ownable {
	...
	function addToken(
		...
		TokenInfo memory tokenInfo = tokens[chainId][token];
		require(tokenInfo.otherToken == address(0), 'Token does already exist');
		tokens[chainId][token] = TokenInfo({otherToken: otherToken, isOtherTokenOrigin: isOtherTokenOrigin});
	}

	function removeToken(uint256 chainId, address token) public onlyOwner {
		TokenInfo memory tokenInfo = tokens[chainId][token];
		require(tokenInfo.otherToken != address(0), 'Token does not exist');

		delete tokens[chainId][token];
	}
	...
	function teleport(
		...
		TokenInfo memory destTokenInfo = tokens[destChainId][srcToken];
		...
		if (!noFeeAddresses[_msgSender()]) {
			...
			IERC20(srcToken).transferFrom(_msgSender(), feeReceiver, feeAmount);
		}

		if (destTokenInfo.isOtherTokenOrigin) {
			IERC20MintableBurnable(srcToken).burnFrom(_msgSender(), teleportAmount);
		} else {
			IERC20(srcToken).transferFrom(_msgSender(), address(this), teleportAmount);
		}
	...
	function claim(bytes memory message, bytes[] memory signatures) public {
		...
		TokenInfo memory srcTokenInfo = tokens[decodedMessage.srcChainId][decodedMessage.destToken];
		require(srcTokenInfo.otherToken != address(0), 'Unknown destination token');

		if (srcTokenInfo.isOtherTokenOrigin) {
