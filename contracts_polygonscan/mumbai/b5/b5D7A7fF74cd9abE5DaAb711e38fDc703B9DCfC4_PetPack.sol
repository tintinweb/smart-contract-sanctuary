...
contract PetPack is ERC721Enumerable, Ownable {
    ...
    function mint(address _to, uint256 _mintAmount) public payable {
        ...
        require(balanceOf(_to) <= maxMintAmount, "Receiving address reached the maximum mintage. Wait until the next collection!");
        require(msg.sender == _to, "You can only mint for yourself, currently!");
        ...
        for (uint256 i = 1; i <= _mintAmount; i++) {
          _safeMint(_to, supply + i); // Uses second paramters to index the baseURI 
        }
    }
    ...
    function withdraw() public payable onlyOwner {
        ...
        uint _30 = address(this).balance * 30 / 100;
        uint _25 = address(this).balance * 25 / 100;
       
        require(payable(developers["SBCLLC"]).send(_40));
        require(payable(developers["SAM"]).send(_25));
        require(payable(developers["ALLISON"]).send(_30));
        // require(payable(developers["SBCLLC"]).send(address(this).balance));
