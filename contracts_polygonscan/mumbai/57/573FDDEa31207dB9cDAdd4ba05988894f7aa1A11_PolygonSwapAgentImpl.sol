...
import "./TokenPlayToken.sol";
import "./Ownable.sol";

contract PolygonSwapAgentImpl is Ownable {
    ...
    constructor(
        ...
        string memory name = TokenPlayToken(erc20Addr).name();
        string memory symbol = TokenPlayToken(erc20Addr).symbol();
        uint8 decimals = TokenPlayToken(erc20Addr).decimals();
    ...
    /**
     * @dev Returns set minimum swap fee from ERC20 to ERC20
     */
    ...
    function fillBSC2PolygonSwap(
        ...
        for (uint256 i = 0; i < bscTxHashArr.length; i++) {
            ...
            TokenPlayToken(registeredERC20).transfer(toAddressArr[i], amountArr[i]);
        ...
        returns (bool)
    {
        ...
        TokenPlayToken(registeredERC20).transferFrom(msg.sender, address(this), amount);
        if (msg.value != 0) {
