...
//        parts[19] = getMount(tokenId);
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Emoji #', toString(tokenId), '", "description": "How lucky we are to have something that makes saying goodbye so hard, our best frients.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
