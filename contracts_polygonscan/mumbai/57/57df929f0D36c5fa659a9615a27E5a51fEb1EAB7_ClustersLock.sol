...
contract ClustersLock is ReentrancyGuardUpgradeable, StakingPools {
    ...
    function deposit(uint256 _pid, uint256 _amount) public override hasPool(_pid) poolRunning(_pid) {
        ...
        for (uint256 i = 0; i < _underlyings.length; i++) {
            _amounts[i] = IERC20(_underlyings[i]).balanceOf(address(this));
            IERC20(_underlyings[i]).safeApprove(strategyRouter, 0);
            IERC20(_underlyings[i]).safeApprove(strategyRouter, _amounts[i]);
        }
        IStrategyRouter(strategyRouter).deposit(_asset, _amounts);
    }
