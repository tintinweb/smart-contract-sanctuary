...
// File: contracts/MarvelousNFT_Polygon.sol
...
contract MarvelousNFT_PoS is ERC20Burnable, ERC20Pausable, AccessControl, AccessControlMixin, ContextMixin, IChildToken, NativeMetaTransaction, AccessRights  {
    ...
    function _partialBurn(address from, uint256 amount) internal returns (uint256) {
        ...
        if (_burnAmount > 0) {
            // Calculate rewarwds
            uint256 _currentTotalStakes = totalStakes;
            if (_currentTotalStakes > 0) {
                //2% distribute to staking pool or 40% of tokens to be burned
            ...
            _transfer(from, address(this), _burnAmount);
             ...
             seasonBattlesPool = seasonBattlesPool.add((_burnAmount.mul(1)).div(5));

             //1% for liquidity pool
             liquidityPool = liquidityPool.add((_burnAmount.mul(1)).div(5));

             //1% for burning tokens or 20% of the 5% tokens to be burned
             _burn(address(this), ((_burnAmount.mul(1)).div(5)));
    ...
    function getValuePerStakedToken() external view only(CREATOR_ROLE) returns (uint256 valuePerStakedToken) {
    ...
    function mint(address account, uint256 amount) external only(DEPOSITOR_ROLE) {
        _mint(account, amount);
        emit MintTokens(_msgSender(), account, amount);
    }

    function updateTotalStakes(uint256 amount) external only(CREATOR_ROLE){
        totalStakes = amount;
    }
    ...
    function addWhitelisted(address _whitelisted) external only(CREATOR_ROLE) {
        whitelisted[_whitelisted] = true;
    }

    function removeWhitelisted(address _whitelisted) external only(CREATOR_ROLE) {
        whitelisted[_whitelisted] = false;
    }

    function setSeasonPoolWallet(address payable wallet) external virtual only(CREATOR_ROLE) {
        seasonPoolWallet = wallet;
        emit SetSeasonPoolWallet(_msgSender(), wallet);
    }

    function setLiquidityPoolWallet(address payable wallet) external virtual only(CREATOR_ROLE) {
        liquidityPoolWallet = wallet;
        emit SetLiquidityPoolWallet(_msgSender(), wallet);
    }

    function getSeasonPoolWallet() view external only(CREATOR_ROLE) returns (address) {
        return seasonPoolWallet;
    }

    function getLiquidityPoolWallet() view external only(CREATOR_ROLE) returns (address) {
        return liquidityPoolWallet;
    }

    function withdrawSeasonPool(uint256 amount) external only(CREATOR_ROLE) {
    ...
    function withdrawLiquidityPool(uint256 amount) external only(CREATOR_ROLE) {
