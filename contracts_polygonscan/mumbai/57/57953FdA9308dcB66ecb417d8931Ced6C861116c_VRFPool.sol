...
contract VRFPool is VRFConsumerBase {
    ...
    function addPool(uint256 _poolId, uint256 _length) external {
        require (pools[_poolId].length == 0, "Pool existed");
        // new pool
        Pool memory pool;
        pool.length = _length;
        pools[_poolId] = pool;
        pools[_poolId].list = new uint256[](_length);
        pools[_poolId].generated = new bool[](_length);
    }
    ...
    function getRandomNumber(uint256 _poolId, uint256 _index) public returns (bytes32 requestId) {
        require (pools[_poolId].length > 0, "Pool not exist");
        require (pools[_poolId].list[_index] == 0, "Generated");
        require(LINK.balanceOf(address(this)) >= fee, "Not enough LINK - fill contract with faucet");
        requestId = requestRandomness(keyHash, fee);
        requests[requestId] = [_poolId, _index];
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        ...
        require (pools[_poolId].list[_index] == 0, "Generated");
        require (_pool.length > _pool.count, "Something wrong");

        // now finding unused item in pool
        uint256 _max = _pool.length - _pool.count;
        uint256 _rand = randomness % _max;
        // uint256 i;
        // bool found;
        // while(!found) {
        //     if (_pool.generated[i]) _rand++; // ignore 
        //     if (_rand == i) found = true;
        //     i++;
        // }

        //update pool
        pools[_poolId].generated[_rand] = true;
        pools[_poolId].list[_index] = _rand + 1;
        pools[_poolId].count++;

        emit DiceLanded(_poolId, _index, _rand+1);

    }

    function getResult(uint256 _poolId, uint256 _index) external view returns (uint256) {
        return pools[_poolId].list[_index];
    }
