...
// File: contracts/MartinMoraIII.sol
...
contract MyToken is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, Ownable, ERC721Burnable, ERC721BatchMint {
    ...
    function safeMintBatch(address to, uint amount) public onlyOwner {
        uint fromTokenId = _tokenIdCounter.current();
        for (uint i=0; i < amount; i++) {
             _safeMint(to, _tokenIdCounter.current());
             _tokenIdCounter.increment();
        }
        uint lastTokenId = _tokenIdCounter.current() - 1;
        emit BatchTransfer(fromTokenId, lastTokenId, address(0), to);
    }

    function safeMint(address to) public onlyOwner {
        uint fromTokenId = _tokenIdCounter.current();
        _safeMint(to, _tokenIdCounter.current());
        emit BatchTransfer(fromTokenId, fromTokenId, address(0), to);
        _tokenIdCounter.increment();
