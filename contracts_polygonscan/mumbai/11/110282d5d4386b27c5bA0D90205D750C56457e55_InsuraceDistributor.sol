...
contract InsuraceDistributor is AbstractDistributor,
IDistributor,
Initializable,
OwnableUpgradeable,
ReentrancyGuardUpgradeable
{
    ...
    function __InsuraceDistributor_init(
        address _masterCover
    ) public initializer {
        __Ownable_init();
    ...
    function buyCoverInsurace(IDistributor.BuyInsuraceQuote memory quote) external override payable nonReentrant {
        if (quote.currency != ETH) {
            ...
            require(IERC20Upgradeable(quote.currency).allowance(_msgSender(), address(this)) >= quote.premium,
                "Error on BU Contract - Need premium allowance");

            // transfer erc20 funds to this contract
            IERC20Upgradeable(quote.currency).safeTransferFrom(_msgSender(), address(this), quote.premium);
            ...
            if (IERC20Upgradeable(quote.currency).allowance(address(this), address(masterCover)) == uint256(0)) {
                ...
                IERC20Upgradeable(quote.currency).safeApprove(address(masterCover), MAX_INT);
            }
