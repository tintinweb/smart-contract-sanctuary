...
contract TestSingleTokenJoin {
   ...
   struct JoinTokenStruct {
    ...
    function joinTokenSingle(JoinTokenStruct calldata _joinTokenStruct)
        external
    {
        TestToken(_joinTokenStruct.inputToken).burn(msg.sender, _joinTokenStruct.inputAmount);
        TestToken(_joinTokenStruct.outputBasket).mint(msg.sender, _joinTokenStruct.outputAmount);
    }


    function exitWeth(ExitTokenStruct calldata _exitTokenStruct) external {
        TestToken(_exitTokenStruct.inputBasket).burn(msg.sender, _exitTokenStruct.inputAmount);
