...
abstract contract VRFConsumerBase is VRFRequestIDBase {
  ...
  function fulfillRandomness( bytes32 requestId, uint256 randomness )internal virtual returns(uint256 );
...
contract BafNFT721 is ERC721, VRFConsumerBase {
    ...
    mapping (bytes32 => uint256) random;
    ...
    function createCollectible(uint256 fee) public returns (uint256) {
     ...
     function getRandomNumber() public returns (bytes32 requestId) {
        ...
        tokenId[rId] = tokenCounter-1;
        return requestId;
    }
    
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override returns(uint256) {
        uint256 randomResult = randomness % 64;
        random[requestId] = randomResult;
        uint256 newItemId = tokenCounter;
        _setTokenURI(newItemId,randomResult.toString());
        emit randomness_with_tokenID(tokenId[requestId],randomResult);
        return randomResult;
