...
contract TangibleNFT is AdminAccess, ERC721, ERC721Enumerable, ERC721Pausable, ITangibleNFT {
    ...
    modifier onlyFactoryOrFactoryAdmin(){
        require(IFactory(factory).isFactoryAdmin(msg.sender) || isFactory(msg.sender), "NFFA");
        _;
    }
    ...
    constructor(
        ...
        storagePercentagePricePerYear = 250; //2.5 percent
    }
    ...
    function tokenURI(uint256 tokenId) public view override(ERC721, IERC721Metadata) returns (string memory) {
        ...
        return bytes(_uri).length > 0 ? string(abi.encodePacked(_baseUriLink, "/", symbol(), "/", tokenId.toString())) : "";
    }

    function isApprovedForAll(address account, address operator) public view override(ERC721, IERC721) returns (bool) {
        return operator == factory || ERC721.isApprovedForAll(account, operator);
    }

    // /// @inheritdoc ITangibleNFT
    // function produceTNFTtoStock(
    //     uint128 vendorId, 
    //     address toStock, 
    //     string calldata brandName
    // ) external override onlyFactory() returns (uint256) {

    //     uint256 tokenId = _produceTNFTtoStock(vendorId, toStock, brandName);
    //     emit ProducedTNFT(tokenId);
    //     return tokenId;
    // }
    ...
    function _produceTNFTtoStock(
        uint128 vendorId, address 
        toStock, string calldata brandName
        ...
        uint256 tokenToMint = uint256(vendorId) << 128 | ++lastVendorProducedTNFT;
    ...
    /// @inheritdoc ITangibleNFT
    function vendorProducedTNFTs(uint128 vendorId) external view override returns (uint256) {
        //returns how much vendor has minted so far, 0 is never minted
        return uint256(_lastVendorsTokenId[vendorId]);
    }

    /// @inheritdoc ITangibleNFT
    function listTNFTsByVendor(uint128 vendorId) external view override returns (uint256[] memory) {
    ...
    /// @inheritdoc ITangibleNFT
    function destroyTNFTs(
        uint256[] calldata tokenIds, 
        address burningFrom
    ) external override onlyFactoryOrFactoryAdmin() {
    ...
    function _destroyTNFT(uint256 tokenId, address burningFrom) internal {
        ...
        if ( ( ( IFactory(factory).isFactoryOperator(tokenOwner)
            || (!isStorageFeePaid(tokenId) 
                && (storageEndTime[tokenId] + 365 days) < block.timestamp 
                && IFactory(factory).isFactoryOperator(burningFrom) ) )
            && !_isTokenMinter(tokenOwner, tokenId) ) || IFactory(factory).isFactoryAdmin(burningFrom) ){
    ...
    function setStoragePricePerYear(uint256 _storagePricePerYear) external onlyFactoryAdmin() {
    ...
    function setStoragePercentPricePerYear(uint256 _storagePercentagePricePerYear) external onlyFactoryAdmin() {
        // price should be higher than 1$ at least in usdc
    ...
    function adjustStorageAndGetAmount(
        ...
        if (storagePriceFixed){
            ...
            amount = (tokenPrice * storagePercentagePricePerYear * _years) / 10000;
        }
    ...
    /// @inheritdoc ITangibleNFT
    function setBrand(uint256 tokenId, string calldata brand) external override onlyFactoryOrAdmin() {
    ...
    function supportsInterface(bytes4 interfaceId) 
       public view virtual override(AccessControl, ERC721, ERC721Enumerable, IERC165) returns (bool) {
    ...
    function _beforeTokenTransfer(
        ...
        if (IFactory(factory).isFactoryAdmin(from) || (factory == from) || from == address(0) || to == address(0)) { return; }
