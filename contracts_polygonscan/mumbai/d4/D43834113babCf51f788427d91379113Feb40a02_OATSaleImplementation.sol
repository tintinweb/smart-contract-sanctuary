...
// OATSaleImplementation SMART CONTRACT
contract OATSaleImplementation is OwnableUpgradeable, BaseRelayRecipient {
    using Address for address;
    using SafeMath for uint256;
     ...
     *
     */
    function initialize(address _nftAddress, address _trustedForwarder)
        external
        initializer
    {
    ...
    /**
     * Sell Artwork Transfer Logic
     *
     * @param nftId - nftId of the Artwork
     * @param price - price to sell Artwork for
     * @param artist - artist or original owner of artwork
     * @param nftOwner - Owner of nft on the blockchain contract
     */
    function _sellArtwork(
        uint256 nftId,
        uint256 price,
        ...
        Sale memory sale = Sale(nftId, price, artist, true);
        _nftSales[nftId] = sale;
        ERC721(nftAddress).transferFrom(nftOwner, address(this), nftId);
        emit SaleAdded(nftId, price, artist, now);
    }

    /**
     * Put up Artwork for Sale
     *
     * @param nftId - nftId of the Artwork
     * @param price - price to sell Artwork for
     * @param artist - artist or original owner of artwork
    ...
    function sellArtwork(
        ...
        require(
            ERC721(nftAddress).getApproved(nftId) == address(this) ||
                ERC721(nftAddress).isApprovedForAll(nftOwner, address(this)),
            "Grant NFT approval to Sale Contract"
        );
        _sellArtwork(nftId, price, artist, nftOwner);
    }

    /**
     * Put up My Artwork for Sale
     *
     * @param nftId - nftId of the Artwork
     * @param price - price to sell Artwork for
     */
    function sellMyArtwork(uint256 nftId, uint256 price) public {
        address nftOwner = ERC721(nftAddress).ownerOf(nftId);
        require(
            nftOwner == _msgSender(),
            "Seller Not Owner of NFTs"
        );
        _sellArtwork(nftId, price, _msgSender(), nftOwner);
    }

    /**
     * Put up Multiple Artworks for Sale
     *
     * @param nftIds - nftIds of the Artworks
     * @param prices - prices to sell Artworks for
     * @param artist - artist or original owner of artwork
     */
    function sellArtworkBatch(
        ...
        require(
            ...
            "nftIds and prices length mismatch"
        );
    ...
    /**
     * Put up Multiple Artworks of mine for Sale
     *
     * @param nftIds - nftIds of the Artworks
     * @param prices - prices to sell Artworks for
     */
    function sellMyArtworkBatch(
        uint256[] calldata nftIds,
        uint256[] calldata prices
    ) external {
        require(
        ...
        for (uint256 i = 0; i < nftIds.length; i++) {
            uint256 nftId = nftIds[i];
            require(
                ERC721(nftAddress).ownerOf(nftId) == _msgSender(),
                "Seller Not Owner of NFTs"
            );
        }
        for (uint256 i = 0; i < nftIds.length; i++) {
    ...
    /**
     * Purchase Artwork Internal
     *
     * @param nftId - nftId of the artwork
     */
    function _purchaseArtwork(uint256 nftId) private {
     ...
     * @param nftId - nftId of the artwork
     */
    function purchaseArtwork(uint256 nftId) external payable {
        require(isActive, "Contract Status in not Active");
        require(
    ...
    /**
     * Batch Purchase Artwork
     *
     * @param nftIds - nftIds of the artwork
     */
    function purchaseArtworkBatch(uint256[] calldata nftIds) external payable {
        require(isActive, "Contract Status in not Active");
        uint256 totalPrice;
        for (uint256 i = 0; i < nftIds.length; i++) {
            uint256 nftId = nftIds[i];
            require(
                _nftSales[nftId].isActive,
                "One or more nfts requested in the batch purchase is not active"
            );
            totalPrice = totalPrice.add(_nftSales[nftId].price);
        }
     ...
     *
     * @param nftId - nftId of the artwork
     * @param buyer - buyer of the artwork
     */
    function releaseArtwork(uint256 nftId, address payable buyer)
        external
        onlyAdmin
    {
        require(isActive, "Contract Status in not Active");
