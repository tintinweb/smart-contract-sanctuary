...
contract LuckyShiba is Ownable, ERC20Pausable { 
    ...
    event BuyEvent(string period, uint256 ticket, address buyer, bytes amount);
    event SendRewardEvent(string period, uint256 ticket, uint256 singleIncome, address[] addresses);


    constructor() ERC20("LuckyShiba", "LUCKYSHIBA") {
        _disabled = false;
        _feeRate = 60;

        uint256 placeOfDecimals = 10 ** 18;
        uint256 oneBillion = 1000000000;
        uint256 initialSupply = 6 * oneBillion * placeOfDecimals; // 1 Billion
    ...
    function buy(string memory period, uint256 ticket) public payable {  
        ...
        emit BuyEvent(period, ticket, _msgSender(), _msgData());
    }
    ...
    function sendReward(string memory period) public onlyOwner {
        require(_randomNumber > 0, "The random number hasnot set up yet!");
