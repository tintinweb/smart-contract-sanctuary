...
contract SnowHeads is Ownable, ERC721Enumerable, Verifiable {
  ...
  constructor(
    ...
    uint _presaleStartAt, // 1638464562657
    uint _revealTimestamp, // 1638464660232
    IERC20 token // 0xfe4F5145f6e09952a5ba9e956ED0C25e3Fa4c7F1
  ) ERC721("SnowHeads", "SH") {
    _ethToken = token;
  ...
  function getSenderEthBalance() public view returns (uint256) { 
      return _ethToken.balanceOf(msg.sender);
  }

  function getContractEthBalance() public view returns (uint256) { 
      return _ethToken.balanceOf(address(this));
  }

  function buy(uint _numTokens) payable public {
    require(isMintActive, "Snowheads: Sale is not active");
    require(_numTokens > 0 && _numTokens <= MAX_PER_PURCHASE, "Snowheads: Invalid number of Snowheads!");
    ...
    uint amountTobuy = mintPrice * _numTokens;
    require(msg.value >= amountTobuy, "Snowheads: Value not valid!");
    uint256 allowance = _ethToken.allowance(msg.sender, address(this));
    require(allowance >= amountTobuy, "Check the token allowance");
    require(_ethToken.transferFrom(msg.sender, address(this), msg.value), "Snowheads: Transfer failed!");
