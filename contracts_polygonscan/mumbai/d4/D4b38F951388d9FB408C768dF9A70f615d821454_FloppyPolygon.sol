...
// Brings in all the NFT/Floppy business logic
import {FloppyBase} from "./FloppyBase.sol";
// Receives support for getting "true caller" thru the GSN Network
// isTrustedForwarder, _setTrustedForwarder, _msgSender...
import {BaseRelayRecipient} from "@opengsn/contracts/src/BaseRelayRecipient.sol";
import {Context} from "@openzeppelin/contracts/utils/Context.sol";
import {Counters} from "@openzeppelin/contracts/utils/Counters.sol";

contract FloppyPolygon is FloppyBase, BaseRelayRecipient {
    ...
    /// Event for when token is transfered to Mainnet
    ...
    constructor(
        ...
        FloppyBase(
            ...
            _NFT_SYMBOL,
            _WACKY_THRESHHOLD,
            _INSECURE_THRESHHOLD,
            _INFECTED_THRESHHOLD
        )
    {
        // Set the trusted GSN forwarder
    ...
    function mint() public {
        ...
        uint256 _newAmountMinted = id + 1;
        uint256 _walletOwns = balanceOf(_msgSender());
        uint256 _newWalletOwns = _walletOwns + 1;

        // Minting can only be performed by the trusted GSN forwarder
        require(
            isTrustedForwarder(msg.sender),
            "Only trusted forwarders can mint tokens"
        );
        // Checks to see if the token was already moved to the root chain
        require(
            !withdrawnTokens[id],
            "PolygonFloppy: TOKEN_EXISTS_ON_ROOT_CHAIN"
        );
        ...
        // Mint the token to the original sender, not forwarder
        _mint(_msgSender(), id);

        // Perform our housekeeping
    ...
    function withdrawWithMetadata(uint256 tokenId) external {
        require(
            ...
            "PolygonFloppy: INVALID_TOKEN_OWNER"
        );
    ...
    {
        return abi.encodePacked(genes[tokenId]);
    }
...
import {SVGBuilder, colorSet, FileListing} from "./SVGBuilder.sol";
...
abstract contract FloppyBase is Ownable, ERC721 {
    using Strings for uint256;
    using Strings for uint8;
    using HexStrings for uint160;
    using ToColor for bytes3;

    CoolSoftware[] internal coolSoftware;

    // Immutable Storage
    uint16 internal immutable WACKY_THRESHHOLD;
    uint16 internal immutable INSECURE_THRESHHOLD;
    uint16 internal immutable INFECTED_THRESHHOLD;

    // Mutable Storage
    bool internal saleActive = true;
    mapping(uint256 => bytes32) internal genes;
    ...
    function isDiskInsecure(bytes32 gene, uint256 threshhold)
        internal
        pure
        returns (bool)
    {
        uint16 insecureDisk = uint16(
            uint8(gene[17]) | (uint16(uint8(gene[18])) << 8)
        );
        return insecureDisk > threshhold ? true : false;
    }

    function getInitialSoftware(bytes32 gene)
        internal
        pure
        returns (BoringSoftware[2] memory)
    {
        string[1] memory boringSoftware = ["WordPerfect 5.1"];
        BoringSoftware[2] memory initialSoftware;
        uint256 boringSoftwares = (uint8(gene[21]) % 2) + 1; // How many boring softwares are ALWAYS on the disk.
    ...
    function getPattern(bytes32 gene, uint256 threshhold)
        internal
        pure
        returns (uint8 patternId)
    {
        uint16 wackyPattern = uint16(
            uint8(gene[19]) | (uint16(uint8(gene[20])) << 8)
        );
        return wackyPattern > threshhold ? uint8(wackyPattern % 3) + 1 : 0;
    }

    function getVirus(bytes32 gene, uint256 threshhold)
        internal
        pure
        returns (string memory virusName)
    {
        ...
        uint16 infectedDisk = uint16(
            uint8(gene[15]) | (uint16(uint8(gene[16])) << 8)
        );
        return
            infectedDisk > threshhold
                ? viruses[infectedDisk % viruses.length]
                : "";
    }
    ...
    function getTraits(
        ...
        bool isInsecure,
        uint256 wackyPattern,
        string memory infectedWith
    ) internal pure returns (string memory) {
        string memory traits = string(
            abi.encodePacked(
                '{"trait_type": "Disk Color", "value": "',
                tokenColors.color1.toColor(),
                '"},',
                '{"trait_type": "Label Color", "value": "',
        ...
        if (wackyPattern > 0) {
            ...
            traits = string(
                abi.encodePacked(
                    traits,
                    ",",
                    '{"trait_type": "Wacky Pattern", "value": "',
                    wackyPatternName,
                    '"}'
                )
            );
    ...
    function getFiles(
        ...
        bool isInsecure,
        string memory infectedWith
    ) internal view returns (FileListing memory) {
        uint256 fileCount;
        uint256 yPos;
        ...
        for (uint256 i = 0; i < coolSoftware.length; i++) {
            try coolSoftware[i].contractInterface.balanceOf(owner) returns (
    ...
    function tokenURI(uint256 id) public view override returns (string memory) {
        ...
        FileListing memory files = getFiles(
            owner,
            getInitialSoftware(gene),
            insecure,
            virusName
        );
        ...
        string memory image = Base64.encode(
            bytes(SVGBuilder.generateSVGofToken(colors, files, pattern))
        );
