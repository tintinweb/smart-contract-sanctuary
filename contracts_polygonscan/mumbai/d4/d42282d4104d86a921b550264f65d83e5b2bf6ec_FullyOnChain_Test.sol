...
// FullyOnChain_Kanji
contract FullyOnChain_Test is ERC721Enumerable, ReentrancyGuard, Ownable {
  using Strings for uint256;
  ...
  function genSVG1(Kanji memory kanji) internal view returns (string memory) {
    ...
    return string(abi.encodePacked(output,zb[3],zb[4],b,zb[5],rl3));
  }
  ...
  function tokenURI(uint256 tokenId) override public view returns (string memory) {
    ...
    return string(abi.encodePacked(rl4,Base64.encode(bytes(output1)),cha[tokenId],Base64.encode(bytes(genSVG2(kanji,tokenId)))));
  }
...
//gasless
  ...
  constructor() ERC721("FullyOnChain_Test", "TST") Ownable() {}
}