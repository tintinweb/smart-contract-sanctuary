...
contract Gems is ERC1155, Ownable, VRFConsumerBase, ExternalActor, Withdrawable, Lockable {
  ...
  uint256 public mintPriceUsd = 1; // NOTE: Set before release
  ...
  constructor(
    ...
    keyHash = _keyhash;
    fee = _fee;
    setUsdContractAddress(_usdToken);
  }
  ...
  function mintMany(uint256 amount, bool usd) public payable {
    ...
    if (usd) {
      totalPrice = (amount * mintPriceUsd);
      // usdc.transfer(address(this), totalPrice * 10**18);
      usdToken.transferFrom(msg.sender, address(this), totalPrice * 10**18);
    } else {
      totalPrice = (amount * mintPrice);
  ...
  function setUsdContractAddress(address _tokenContract) public onlyOwner {
    usdToken = IERC20(_tokenContract);
    usdTokenAddress = _tokenContract;
  }
}
