...
Ownable{
   ...
   constructor(uint maximumcoin)public ERC20Detailed("Duck Coin", "MCDUCK", 4){
//uint maximumcoin;
            _totalSupply = _balances[msg.sender] = maximumcoin*10**4;
    }
...
contract TimeLock {
    ...
    function deposit(address beneficiary, uint amount, uint releaseTime, uint releaseDuration) public returns(bool success) {
        require(token.transferFrom(msg.sender, address(this), amount));
        LockBoxStruct memory l;
        l.beneficiary = beneficiary;
        l.balance = amount;
        l.releaseTime = releaseTime;
        l.releaseDuration = releaseDuration;
        am = (amount*20)/100;
...
uint n;
    ...
    function withdraw(uint lockBoxNumber) public returns(bool success) {
        ...
        require(l.releaseTime <= now,"Tokens are locked");  
      
        while (n==0){
        uint amount = l.balance;
        l.balance -= (amount*20)/100;
        emit LogLockBoxWithdrawal(msg.sender, (amount*20)/100);
        require(token.transfer(msg.sender, (amount*20)/100));
        n++;
        l.releaseTime += l.releaseDuration;
        return true;
    }    
    
      while (n!=0){
          l.balance -= am;
          emit LogLockBoxWithdrawal(msg.sender, am);
          require(token.transfer(msg.sender, am));
          l.releaseTime += l.releaseDuration;
          return true;
      }
