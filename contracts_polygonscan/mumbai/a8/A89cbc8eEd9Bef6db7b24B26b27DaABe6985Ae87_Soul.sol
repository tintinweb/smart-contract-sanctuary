...
contract Soul is ERC721Enumerable, AccessOperatable {
    bytes32 public constant MINTER = keccak256("MINTER");
    bytes32 public constant REDUCER = keccak256("REDUCER");
    ...
    event AddressesSet(address rays, address wallet);
    ...
    constructor() ERC721("MCH Soul", "MCHS") {
        ...
        _setRoleAdmin(REDUCER, OPERATOR);
        _setupRole(REDUCER, msg.sender);
    }
    ...
    function _baseURI() internal view override returns (string memory) {
        return "https://mycryptoheroes.com/soul";
    }

    // @dev https://github.com/OpenZeppelin/openzeppelin-contracts/blob/d95cdaae8e26b3e479ca2ad70cc7c5a303444392/contracts/token/ERC721/ERC721.sol#L331
    function _transfer(
        address from,
        address to,
        uint256 tokenId
    ) internal override {
        // 耐久値が1以上の場合のみtransfer可能
        require(remainingDurability[tokenId] > 0, "remainingDurability is zero");
    ...
    /// @notice 耐久値を減らす
    function reduceDurability(uint256 tokenId, uint256 durability) external onlyRole(REDUCER) {
        // トークンが存在していること
        require(_exists(tokenId), "token does not exist");
        // 残り耐久値が減らしたい耐久値以上であること
        require(remainingDurability[tokenId] >= durability, "insufficient durability");
    ...
    function restoreDurability(uint256 tokenId) external {
        ...
        require(_exists(tokenId), "token does not exist");
        // 耐久値が1以上減っていること
        require(remainingDurability[tokenId] < maxDurability, "durability is full");

        uint256 raysAmount = requiredRaysAmountToRestore(tokenId);
        // raysの量が足りなかったらtransferFrom内でrevert
    ...
    /// @notice 耐久値の回復に必要なRaysの数
    function requiredRaysAmountToRestore(uint256 tokenId) public view returns(uint256) {
        require(_exists(tokenId), "token does not exist");

        //  初期耐久値: maxDurability
        //  summonにかかるraysの数: requiredRaysAmount
        //  回復する耐久値: durabilityToRestore
        // としたとき
        //  回復するのに必要なraysの数: ceil(requiredRaysAmount / maxDurability * durabilityToRestore)
        // となる
    ...
    function mint(address to, uint256 tokenId) external onlyRole(MINTER) {
        // トークンIDが重複している場合は_mint内でrevert
        _mint(to, tokenId);
        remainingDurability[tokenId] = maxDurability;
    ...
    function setAddresses(address _rays, address _wallet) external onlyRole(OPERATOR) {
        rays = IERC20(_rays);
        wallet = _wallet;

        emit AddressesSet(_rays, _wallet);
    }

    function setParameters(uint256 _maxDurability, uint256 _requiredRaysAmount) external onlyRole(OPERATOR) {
