...
contract Presale is ReentrancyGuard, Context, Ownable {
    ...
    // owner can only withdraw tokens if the ico is not active anymore
    function withdraw() external onlyOwner {
         require(address(this).balance > 0, 'Contract has no money');
        _wallet.transfer(address(this).balance);    
    }

    // check amount how much person purchased
    // REMOVE THIS IN ACTUALLY BUILD
    function getDistrubutedWalletAmount(address beneficiary) public view returns(uint256 amount) {
