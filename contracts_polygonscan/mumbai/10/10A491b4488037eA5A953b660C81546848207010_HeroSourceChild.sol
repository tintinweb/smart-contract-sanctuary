...
contract HeroSourceChild is ISourceChild, ERC721Enumerable, Ownable, AccessControl {
    bytes32 public constant TUNNEL = keccak256("TUNNEL");
    ...
    constructor(address heroComposer_) ERC721("Test", "TEST") {
        ...
        _setRoleAdmin(TUNNEL, ADMIN);
        _setupRole(ADMIN, msg.sender);
    }

    function mint(address to, uint256 tokenId, uint256 seed, bytes memory data) override external {
        require(hasRole(TUNNEL, msg.sender));
        seeds[tokenId] = seed;
        _safeMint(to, tokenId);
    }

    function burn(uint256 tokenId, bytes memory data) override external {
        require(hasRole(TUNNEL, msg.sender));
        _burn(tokenId);
    }
