...
contract CanvasManager is AccessControl, Ownable, Pausable, ICanvas, ReentrancyGuard {
    ...
    function calcAllShares() public view returns (uint256) {
        uint256 sum = 0;
        uint256 count = _endArtIds.length;

        for (uint256 i = 0; i < count; ++i) {
            Art storage art = mArts[_endArtIds[i]];
            sum += art.moments.length;
        }
    ...
    function calcShares(uint256 nftId, address contributor) public view returns (uint256 thisShares, uint256 allShares) {
        ...
        uint256 count = _endArtIds.length;
        for (uint256 i = 0; i < count; ++i) {
            uint256 share = _getContribution(_endArtIds[i], contributor);
            allShares += share;

            if (nftId == _endArtIds[i]) {
                thisShares = share;
                break;
            }
        }
    ...
    function _getContribution(uint256 nftId, address contributor) internal view returns (uint256) {
        uint256 shares = 0;
        Art storage art = mArts[nftId];
