...
contract KommunitasStaking is Ownable {
    ...
    uint256 public penaltyFeesPercentage;
    ...
    view returns (uint256 _lockReward) {   
        uint multiplier = durationMultiplier[_duration];
        if (multiplier == 0){
            multiplier = 1;
        }
        uint256 durationSeconds = _duration * 1 days;
        uint256 effectiveAPY = multiplier * apy * durationSeconds * 1e10 / yearDuration; 
        _lockReward = effectiveAPY * _amount * durationSeconds / yearDuration / 1e12;
    }
    ...
    function _stake(address _user, uint256 _amount, uint256 _duration) internal {
        ...
        uint256 matureUntil = block.timestamp + _duration;
    ...
    function unlock(address _of) external returns (uint256) {
        ...
        for (uint256 i = 0; i < locksLength; i++) {
            if (locks[_of][i].maturity <= block.timestamp && !locks[_of][i].claimed) {
                ...
                unlockablePrincipalStakedAmount = unlockablePrincipalStakedAmount + locks[_of][i].amount;
    ...
    function preMatureWithdraw() external returns (uint256) {
        ...
        if (unlockableTokens > 0) {
            ...
            if(penaltyFeesPercentage > 0){
                uint remainingFeePercentage = 100 - penaltyFeesPercentage;
    ...
    function getTotalWithdrawableTokens(address _of) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < locksLength; i++) {
            if (locks[_of][i].maturity <= block.timestamp && !locks[_of][i].claimed) {
                withdrawableTokens = withdrawableTokens + locks[_of][i].amount + locks[_of][i].reward;
            }
        }
    ...
    function getUserPendingRewards(address _of) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < locksLength; i++) {
            if (locks[_of][i].maturity <= block.timestamp && !locks[_of][i].claimed) {
                pendingRewards = pendingRewards + locks[_of][i].reward;
            }
        }
    ...
    function getUserStakedTokens(address _of) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < locksLength; i++) {
            if (!locks[_of][i].claimed) {
                lockedTokens = lockedTokens + locks[_of][i].amount;
            }
        }
        return lockedTokens;
    }
    ...
    function setPenaltyFees(uint256 _feesPercentage) external onlyOwner{
