    ...
    ERC721URIStorageUpgradeable
{
    ...
    ITransformedNFT private immutable _transformedNftContract;
    ...
    constructor (address  _transformedContract){
         _transformedNftContract = ITransformedNFT(_transformedContract);
    }
     ...
     * @dev sets token meta
     * @param _tokenId uint256 token ID (token number)
     * @param _collection CollectionMeta
     *
     * Requirements:
     * `tokenId` must exist
     * `owner` must the msg.owner
     */
    function _setCollectionMeta(uint256 _tokenId, CollectionMeta memory _collection) private {
        require(_exists(_tokenId));
        require(ownerOf(_tokenId) == _msgSender() || owner() == _msgSender());
        _tokenCollection[_tokenId] = _collection;
    }

    /**
     * @dev returns tokenId collection
     * @param _tokenId uint256 token ID (token number)
     *
     * Requirements:
     * `tokenId` must exist
     */
    function collectionMeta(uint256 _tokenId) public view returns (CollectionMeta memory) {
        require(_exists(_tokenId));
        return  _tokenCollection[_tokenId] ;
    }
     ...
     * @dev sets token meta
     * @param _tokenId uint256 token ID (token number)
     * @param _card CardMeta
     *
     * Requirements:
     * `tokenId` must exist
     * `owner` must the msg.owner
     */
    function _setCardMeta(uint256 _tokenId, CardMeta memory _card) private {
        require(_exists(_tokenId));
        require(ownerOf(_tokenId) == _msgSender() || owner() == _msgSender());
        _tokenCard[_tokenId] = _card;
    }
     ...
     * `tokenId` must exist
     */
    function cardMeta(uint256 _tokenId) public view returns (CardMeta memory) {
        require(_exists(_tokenId));
        return  _tokenCard[_tokenId];
    }
     ...
     * @param _status int for token status 
     ...
     * @param _contractAddress contract address of token or collection
     * @param _collectionName collection name incase of collection specifics
     * @param _cardNum card number  incase of card specific
     * Requirements:
     * caller must be the owner
     */
    function addTokenSpecs (
        uint _tokenId,
        ...
        require(_exists(_tokenId));
        TokenMeta memory meta = tokenMeta(_tokenId);
        
        if (meta.classify == 2){ //per collection
            CollectionMeta memory collection = CollectionMeta (
                _contractAddress,
                _collectionName
            );
            _setCollectionMeta(_tokenId, collection);
        
        } else if (meta.classify == 3){  //per card
            CardMeta memory card = CardMeta (
                _contractAddress,
                _cardNum
            );
            _setCardMeta(_tokenId, card);
        }
     ...
     * @param _classify URI for the generated nft
     * Requirements:
     * caller must be the owner
     */
    function transform (
        ...
        // if not code panics
        require(ownerOf(_serumId) == _msgSender() || owner() == _msgSender(), "Ownable: caller is not the owner");
      
        // require caller to be owner of the orignal nft
        require(IERC721Upgradeable(_originalNftContract).ownerOf(_serumId) == _msgSender(), "caller is not the owner of original nft");

        // get serum metadat   
        TokenMeta memory meta = _tokenMeta[_serumId];
        
        //require serum to be full (2) (not used)
        require(meta.status == 2, "Serum is already used");

        /** call the contract to mint t */
        ...
        _tokenMeta[_serumId].status = 1;  // empty 
        _setTokenURI(_serumId, meta.emptyUri); // change token uri
