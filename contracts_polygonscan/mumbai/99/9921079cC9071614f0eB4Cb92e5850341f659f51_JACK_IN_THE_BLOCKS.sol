...
contract JACK_IN_THE_BLOCKS is ERC721, ERC721Enumerable, Ownable {
  ...
  string internal baseCID; // CID of the base URI

  bool public _paused = true;

  bool public premint_paused = true;

  uint256 public _price = 60000000000000000;

  uint256 public _reserved = 100;

  uint16 public mintingSupply = 2978; // minted supply

  uint16 public mintedFromReserve; // reserved token supply

  uint16 public mintedFromSupply; // reserved token supply

  mapping(address => bool) public whitelisted; //List of whitelisted addresses
  ...
  address PUNK_EVOLVE_ADDRESS = 0x83D36DFc7fb35B20f5d60678172B3F444012BfFF; //0xD28c830DED2C84304Fd576F91AE3a78fDc981d27;
  ...
  function pre_mint() external payable {
    ...
    require(msg.value == getDiscountPrice(msg.sender), 'ETH sent not correct!');
    require(
  ...
  function giveAway(address _to, uint256 _amount) external onlyOwner {
    ...
    for (uint256 i; i < _amount; i++) {
      if (mintedFromReserve < _reserved) {
        _tokenIds.increment();
  ...
  function getDiscountPrice(address _address) public view returns (uint256) {
    uint256 balance = ERC721(address(PUNK_EVOLVE_ADDRESS)).balanceOf(_address);
...
library Utility {
  ...
  function createSVGURI(string memory svgStyle, string memory svgContent)
    internal
    pure
    returns (string memory)
  {
    ...
    return
      string(
        abi.encodePacked(
          ...
          svgStyle,
          '<rect width="100%" height="100%" fill="grey"/> <foreignObject x="0" y="0" width="100%" height="100%"> <div class="wrapper" xmlns="http://www.w3.org/1999/xhtml"> <div class="pass" >',
          svgContent,
