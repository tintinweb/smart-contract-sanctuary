...
contract Asset is InitialAsset, IAsset {
        ...
        returns (uint256)
    {
        ...
        emit MintAsset(_msgSender(), tokenToPay, amount, mintAmount, tokensInAsset, buyAmounts);
    ...
    function makeIme(uint256 assetAmount) external nonReentrant onlyManagerOrAdmin {
        _proceedIme(assetAmount);

        {
            if (_totalWeightIme == 0) {
                return;
            }
            ...
            _rebase(tokenWhitelist, tokensIncomeAmounts, _totalWeightIme, tokensInAsset, true);
        }
        ...
        onlyManagerOrAdmin
    {
        ...
        emit WithdrawTokens(tokensInAsset, tokenAmounts);
    }
    ...
    function depositToIndex(
        ...
        emit DepositToAsset(tokensInAsset, tokenAmountsOfY, tokensOfDividends, amountOfDividends);
    }
    ...
    function _transfer(
        ...
        if (sender != zVault && recipient != zVault) {
    ...
    function _autoTransferFee(bool isForce) private {
        ...
        if (feeAmountToZ > 0 && (isForce || feeAmountToZ >= feeLimitForAuto)) {
            IWETH(_weth).withdraw(feeAmountToZ);
            totalAmountFee += feeAmountToZ;
            feeAmountToZ = 0;
        }

        if (
            feeAmountInAssetToken > 0 &&
            (isForce || feeAmountInAssetToken >= feeLimitForAutoInAssetToken)
        ) {
            isRedeemingFee = true;
            //require(balanceOf(address(this)) >= feeAmountInAssetToken,  "Internal error 11");
            uint256 feeAmount = redeem(feeAmountInAssetToken, address(0));
            totalAmountFee += feeAmount;
            isRedeemingFee = false;
            feeAmountInAssetToken = 0;

            emit AssetsFromFeesConvertedToWeth(feeAmountInAssetToken, feeAmount);
        }
...
contract InitialAsset is ERC20Upgradeable, ReentrancyGuard {
    ...
    function changePauseState() external onlyManagerOrAdmin {
        ...
        emit PauseStateChanged(mintPaused);
    }
    ...
    function _proceedIme(uint256 assetMintFromIme_) internal {
        ...
        (uint256 totalWeight_, uint256[] memory tokensPrices) =
            AssetLib.proceedIme(tokenWhitelist, oracle, tokenEntersIme);
        _totalWeightIme = totalWeight_;
        if (totalWeight_ != 0) {
            _tokenPricesInIme = tokensPrices;
            _tokensInIme = tokenWhitelist;
    ...
    function _calculateBalanceFromIme(uint256 weight) private view returns (uint256) {
        if (isImeHelded == false || _totalWeightIme == 0) {
            return 0;
        } else {
            return (weight * _totalAssetMintFromIme) / _totalWeightIme;
        }
    }
