...
contract mintNFT is ERC721{
    ...
    constructor() ERC721("Quest","QST","13.82.217.131:3001/") public{
     ...
     /*Set new Contract Owner Address*/
    function setOwnerAddress(address _address) public onlyOwner returns(bool){
...
contract SLC is ReentrancyGuard{
    ...
    function ADD_REGION_ADMIN(uint region_no,address _address) public validOwner returns(bool){
    ...
    function CHANGE_REGION_ADMIN(uint256 _region_no ,address _address) public validOwner returns(bool){
     ...
     /*Add new owner*/
    function addOwner(address _owner) validOwner public returns(bool){         
    ...
    function removeOwner(address _owner) validOwner public {
    ...
    function setAllowance(address _spender, uint256 _value) public returns (bool success) {
        allowance[msg.sender][_spender] = _value;                               
        emit Approval(msg.sender,_spender, _value);
        return true;
    }
    ...
    function MINT_SLC(address account,uint256 amount) public validOwner {
        mint(account,amount);
    }
    ...
    function withdrawFundsFromContract() public validOwner {
    ...
    function SET_AUCTION_DEPLOYED_ADDRESS(address _address) public validOwner returns(bool){
    ...
    function EnlistAuction(string memory uid,
                           ...
                           uint sl_reserve,
                           uint noOfTokens,
                           string memory property_id) public validOwner returns(bool){
    ...
    /*EndAuction*/
    function EndAuction(string memory uid,string memory property_id) public validOwner returns(bool,uint,uint){
    ...
    function STORE_AUCTION_TOKENS_TO_BE_GIVEN(string memory auctionID,address[] memory BiddersArray,uint[] memory BidAmount) public validOwner{
    ...
    function GET_AUCTION_WIN_SLC(string memory auctionID,address Treasury_Admin) public nonReentrant returns(bool){
...
interface mintNFTcontract{
    ...
    function setOwnerAddress(address newOwnerAddress) external returns(bool);

    function changeBaseURI(string memory new_base_uri) external;
}
...
contract SLF {
    ...
    /*Change BASE_URI of NFT contract*/
    function CHANGE_NFT_BASEURI(string memory newUri) public onlyOwner returns(bool){
        mintNFTcontract(MINTNFT_DEPLOYED_ADDRESS).changeBaseURI(newUri);
        return true;
    }
    
     /*Set NFT Contract Owner Address*/
    function SET_NFT_OWNER_ADDRESS(address _address) public onlyOwner returns(bool){
        mintNFTcontract(MINTNFT_DEPLOYED_ADDRESS).setOwnerAddress(_address);
        return true;
    }
   
    /*Store mintNFT Contract Deployed Address*/
    function SET_NFT_DEPLOYED_ADDRESS(address _address) public onlyOwner returns(bool){
        MINTNFT_DEPLOYED_ADDRESS = _address;
        return true;
    }

    /*Add a new owner in SLC contract*/
    function ADD_OWNERS_OF_SLC(string memory propID,address _address) public onlyOwner returns(bool){
        SLC_CONTRACT(PROPERTY_ERC20_ADDRESS[propID]).addOwner(_address);
        return true;
    }
    
    /*remove a owner in SLC contract*/
    function REMOVE_OWNERS_OF_SLC(string memory propID,address _address) public onlyOwner returns(bool){
         SLC_CONTRACT(PROPERTY_ERC20_ADDRESS[propID]).removeOwner(_address);
        return true;
    }
    
    /*Set new Contract Owner Address*/
    function SET_SLF_OWNER_ADDRESS(address _address) public onlyOwner returns(bool){
        CONTRACT_OWNER = _address;
        return true;
    }
