...
contract DPSDocks is ERC721Holder, ReentrancyGuard, Ownable {
  ...
  function claimRewards(
    ...
    LockedVoyage storage lockedVoyage = lockedVoyages[msg.sender][_voyageId];
    ...
    finishedVoyages[msg.sender][lockedVoyage.voyageId] = lockedVoyage;
    delete lockedVoyages[msg.sender][lockedVoyage.voyageId];
  ...
  function awardRewards(
    ...
    chest.mint(msg.sender, voyageConfig.typeOfVoyage, voyageResult.awardedChests);
    dps.safeTransferFrom(address(this), msg.sender, lockedVoyage.dpsId);
    flagship.safeTransferFrom(address(this), msg.sender, lockedVoyage.flagshipId);
    for (uint256 i; i < lockedVoyage.supportShipIds.length; i++) {
      ...
      else supportShip.safeTransferFrom(msg.sender, address(this), lockedVoyage.supportShipIds[i]);
    }
    artifact.safeTransferFrom(address(this), msg.sender, lockedVoyage.artifactId);
  ...
  function getLockedVoyagesForOwner(address _owner) external view returns (LockedVoyage[] memory lockedVoyagesOfOwner) {
    for (uint256 i; i < lockedVoyagesIds[_owner].length; i++) {
      lockedVoyagesOfOwner[i] = lockedVoyages[_owner][lockedVoyagesIds[msg.sender][i]];
    }
  }
  ...
  function getFinishedVoyagesForOwner(address _owner) external view returns (LockedVoyage[] memory finishedVoyagesOfOwner) {
    for (uint256 i; i < finishedVoyagesIds[_owner].length; i++) {
      finishedVoyagesOfOwner[i] = finishedVoyages[_owner][finishedVoyagesIds[msg.sender][i]];
    }
  }
}
