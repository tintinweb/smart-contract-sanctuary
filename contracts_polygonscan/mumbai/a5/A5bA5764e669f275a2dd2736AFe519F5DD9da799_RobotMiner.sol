    ...
    IGame
{
    ...
    mapping(address => GameInfo) public gameInfos;

    uint8 public constant totalMeteor = 4; // Fix
    ...
    event SetMultiplierPrize(uint8 multiplier);
    ...
    function initialize(
        ...
        emit SetMultiplierPrize(multiplier);
        ...
        onlyOwner
    {
        require(
            _minBet < _maxBet,
            "RobotMiner.sol: minBet has to be less than maxBet."
        );
        maxBet = _maxBet;
        minBet = _minBet;
        emit SetMaxMinBet(maxBet, minBet);
    ...
    function setMultiplierPrize(uint8 _multiplier)
        external
        whenPaused
        onlyOwner
    {
        require(
            _multiplier > 0,
            "RobotMiner.sol: Multiplier Prize can't be zero."
        );
        multiplier = _multiplier;
        emit SetMultiplierPrize(multiplier);
    }
        ...
        onlyOwner
    {
        require(
            ...
            "RobotMiner.sol: Vault address can't be zero address."
        );
        vault = IVault(_vaultAddress);
        emit SetVaultAddress(address(vault));
    }

    function getPick(address _player) external view returns (uint8[] memory) {
        return gameInfos[_player].pick;
    }
    ...
    function mining(
        ...
        require(
            ...
            "RobotMiner.sol: Bet must be more than minimum and less than maximum."
        );

        GameInfo storage gameInfo = gameInfos[msg.sender];

        require(
            gameInfo.state == GameState.Ended,
            "RobotMiner.sol: Game is ongoing."
        );

        if (_meteor0) {
            gameInfo.pick.push(0);
        }
        if (_meteor1) {
            gameInfo.pick.push(1);
        }
        if (_meteor2) {
            gameInfo.pick.push(2);
        }
        if (_meteor3) {
            gameInfo.pick.push(3);
        }
        require(
            gameInfo.pick.length == 3,
            "RobotMiner.sol: Need to select 3 meteors."
        );

        vault.addReserveOperating(getWinnerPrize(_betAmount) - _betAmount);
        vault.addGameBalance(msg.sender, _betAmount);

        gameInfo.state = GameState.OnGoing;
        gameInfo.betAmount = _betAmount;
    ...
    function getWinnerPrize(uint128 _amount) public view returns (uint128) {
        ...
        override
    {
        ...
        GameInfo storage gameInfo = gameInfos[player];

        uint128 winnerBet = getWinnerPrize(gameInfo.betAmount);
        uint8 badMeteor = uint8(randomness % totalMeteor);
        for (uint8 index; index < gameInfo.pick.length; index++) {
            if (gameInfo.pick[index] == badMeteor) {
                vault.subReserveOperating(winnerBet);
                winnerBet = 0;
                break;
            }
        }
        vault.subGameBalance(player, winnerBet, 0);

        delete gameInfos[player];
    ...
    function withdrawLink() external onlyOwner {
        ...
        require(amount > 0, "RobotMiner.sol: Not have LINK");
