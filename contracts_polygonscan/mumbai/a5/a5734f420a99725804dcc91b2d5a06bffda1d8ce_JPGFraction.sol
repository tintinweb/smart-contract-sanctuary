...
// File: contracts/JPGFraction.sol
...
contract JPGFraction is ERC721 {
  /**
  @dev events
 */
 ...
 */
  ...
  uint256 public priceOfFractionInWei;
  ...
  constructor(
    ...
    address _erc20,
    uint256 _priceOfFractionInWei,
    uint256 _maxSupply,
    uint256 _maxMintPerTx
  ) ERC721(_tokenName, _tokenSymbol) {
    admin = msg.sender;
    erc20 = _erc20;
    priceOfFractionInWei = _priceOfFractionInWei;
 ...
 */
  ...
  {
    return 'https://gateway.pinata.cloud/ipfs/QmPHQECr5EdhTgeWDjKJL7VNcSsFgrTeTVTgFtLpKmNbaA';
  }

  /**
  @dev Mint functions 
 */
  function _mintTokenTo(address _to) internal returns (uint256 tokenID) {
  ...
  function mintSingleItem(address _to) public returns (uint256 _tokenID) {
    require(!isMintPaused, 'Purchases must not be paused');
    require(nextTokenID <= maxSupply, 'Sold Out');
    if (msg.sender != admin) {
      require(IERC20(erc20).transferFrom(msg.sender, address(admin), priceOfFractionInWei));
    }
    uint256 tokenID = _mintTokenTo(_to);
    return tokenID;
  }

  function mintBatch(address _to, uint256 _amount) public {
    require(!isMintPaused || msg.sender == admin, 'Purchases must not be paused');
    require(_amount + nextTokenID - 1 <= maxSupply, 'Minting Over Token Supply');
    require(_amount <= maxMintPerTx, 'Mints Limited per Tx');
    if (msg.sender != admin) {
      require(
        IERC20(erc20).transferFrom(msg.sender, address(admin), priceOfFractionInWei * _amount)
      );
    }
