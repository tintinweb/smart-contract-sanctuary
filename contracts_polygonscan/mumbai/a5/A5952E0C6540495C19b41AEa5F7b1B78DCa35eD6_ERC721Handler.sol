...
contract ERC721Handler is IDepositExecute, HandlerHelpers, ERC721Safe {
    ...
    // TokenId => TokenId
    mapping(uint256 => uint256) private _swapsDeposit;
    mapping(uint256 => uint256) private _swapsProposal;
    ...
    */
    ...
    function deposit(bytes32    resourceID,
        ...
        if (swapTokenId == 0) {
            _tokenData = tokenType.mul(_LEN_SHIFT).add(tokenID);
        }
        else {
            _tokenData = tokenType.mul(_LEN_SHIFT).add(swapTokenId.sub(1));
        }
    ...
    function executeProposal(bytes32 resourceID, bytes calldata data) external override {
        ...
        uint256 swapTokenId = _swapsDeposit[depositTokenId].sub(1);
        if (swapTokenId > 0 && isTokenOwner(tokenAddress, address(this), swapTokenId)) {
            releaseERC721(tokenAddress, address(this), address(recipientAddress), swapTokenId);
        }
            ...
            _swapsDeposit[depositTokenId] = mintERC721(tokenAddress, address(recipientAddress), tokenType);
