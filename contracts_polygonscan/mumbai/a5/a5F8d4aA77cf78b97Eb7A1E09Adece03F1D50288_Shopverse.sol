...
contract Shopverse is ERC721Enumerable, Ownable {
	...
	string public _contractURI = "https://to.wtf/contract_uri/shopverse/contract_uri.json";
	address private devWallet;
	uint256 public tokenPrice = 0.149 ether;
	mapping(address => uint256) public usedAddresses; //max 3 per address for whitelist
	mapping(address => uint256) public usedJRNYTokens;
	bool public locked; //metadata lock
	uint256 public maxSupply = 7777;
	IERC721 public JRNYClub;

	uint256 public whitelistStartTime = block.timestamp; //update to correct
	uint256 public jrnyStartTime = block.timestamp; //update to correct
	uint256 public publicSaleStartTime = block.timestamp; //update to correct

	constructor() ERC721("Shopberse", "SHOP") {
		devWallet = msg.sender;
		JRNYClub = IERC721(0x0b4B2bA334f476C8F41bFe52A428D6891755554d); //default
	}
	...
	//jrny is whitelisted
	function jrnyBuy(uint256 qty, uint256 jrnyTokenID) external payable {
		require(tokenPrice * qty == msg.value, "exact amount needed");
		require(usedAddresses[msg.sender] + qty <= 3, "max 3 per wallet");
	...
	//in unix
	function setWhitelistStartTime(uint256 _timeunix) external onlyOwner {
		whitelistStartTime = _timeunix;
	}

	//in unix
	function setJrnyStartTime(uint256 _timeunix) external onlyOwner {
		jrnyStartTime = _timeunix;
	}

	//in unix
	function setPublicSaleStartTime(uint256 _timeunix) external onlyOwner {
		publicSaleStartTime = _timeunix;
	}
