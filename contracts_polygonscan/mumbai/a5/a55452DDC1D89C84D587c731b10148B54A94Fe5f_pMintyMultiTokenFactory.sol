...
import "../multitoken/pMintyMultiToken.sol";            // ERC1155
import "@openzeppelin/contracts/access/Ownable.sol";
import "../proxies/INewentProxy.sol";


contract pMintyMultiTokenFactory is Ownable {


    pMintyMultiToken[] public projects;
    address            public _saleContract;
    INewentProxy       public _registry;
    ...
    event PrivateSaleCreated(
            ...
            PoolEntry [] _initialEntries,
            PoolEntry [] _resaleEntries,
            string       _poolName,
            INewentProxy      proxy
    ...
    constructor(
        address __saleContract,
        INewentProxy __registry
    ) {
        _saleContract = __saleContract;
        _registry = __registry;
    }

    function createNewPrivateSale(
            address            art_owner,
            locking[]   memory _locs, 
            string      memory _lockError,        // "You do not have $MINTY locked"
            uint256             _royaltyPerMille, // 150
            PoolEntry [] memory _initialEntries,  
            PoolEntry [] memory _resaleEntries,
            string       memory _poolName
    ) external onlyAuth {
        pMintyMultiToken pr = new pMintyMultiToken(
                    art_owner, 
                    _saleContract, 
                    _locs, 
                    _lockError, 
        ...
        pr.setDeployer(msg.sender);
    }

    function setAuth(address operator, bool enabled) external onlyAuth {
        require(operator != owner(), "owner cannot be an operator");
...
contract pMintyMultiToken is ContextMixin, ERC1155, NativeMetaTransaction {
    ...
    function setDeployer(address newDeployer) external {
        require(msg.sender == deployer,"Unauthorised : not the deployer");
