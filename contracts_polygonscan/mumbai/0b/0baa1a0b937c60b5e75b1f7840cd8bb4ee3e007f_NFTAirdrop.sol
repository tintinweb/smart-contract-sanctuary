...
contract NFTAirdrop {
    ...
    mapping(bytes => Airdrop) public airdrops;
    mapping(uint256 => bytes) public airdropIDs;

    //Variable to hold ArtisLife NFT Minter token object
    ArtisLifeNFT public nftMinter;

    //Address to Admin Priviledges
    address public ADMIN_ROLE;

    event AirdropCreated(bytes airdropID);
    event AirdropRedeemed(bytes airdropID, address purchaser);
    event AirdropRecipientAdded(bytes airdropID, address recipient);
    ...
    constructor() {
        ...
        nftMinter = ArtisLifeNFT(0x244Ca52f0D1c948a84DF489550C708055047B417);
    }

    function createNewAirdrop(bytes calldata airdropID, uint256 nftId)
        external
        onlyAdmin
    {
        require(
            airdrops[airdropID].created == false,
            "Airdrop ID exists already."
        );
        airdrops[airdropID].created = true;
        airdrops[airdropID].nftID = nftId;
        airdropIDs[nftId] = airdropID;
        emit AirdropCreated(airdropID);
    }

    function addRecipient(
        bytes calldata airdropID,
        uint256 recipientID,
        address recipientAddress
    ) public onlyAdmin airdropExists(airdropID) {
        require(
            airdrops[airdropID].recipients[recipientID] == address(0),
            "Recipient ID already assigned."
        );
        airdrops[airdropID].recipients[recipientID] = recipientAddress;
    }

    function addBunchRecipients(
        bytes calldata airdropID,
        uint256[] calldata recipientID,
        address[] calldata recipientAddress
    ) external onlyAdmin airdropExists(airdropID) {
        require(recipientID.length == recipientAddress.length, "Arrays must be equal in length.");
        for(uint256 i = 0; i < recipientID.length; i++){
            addRecipient(airdropID, recipientID[i], recipientAddress[i]);
        }
    }

    function mintAirdrop(bytes calldata airdropID, uint256 recipientID)
        external
        airdropExists(airdropID)
        isRecipient(airdropID, recipientID)
        nftNotMinted(airdropID, recipientID)
    {
        airdrops[airdropID].hasMinted[recipientID] = true;
        nftMinter.mintFor(
            msg.sender,
            airdrops[airdropID].nftID,
            1,
            recipientID,
            ""
        );
        emit AirdropRedeemed(airdropID, msg.sender);
    }
    function setNewMinter(address newMinter) onlyAdmin public {
        nftMinter = ArtisLifeNFT(newMinter);
    }
    /*
        onlyAdmin
    ...
    */
    ...
    /*
        ...
        Checks to see if airdrop ID exists.
    */
    modifier airdropExists(bytes calldata airdropID) {
        require(
            airdrops[airdropID].created == true,
            "Airdrop ID does not exist."
        );
        _;
    }
    ...
    /*
        ...
        Checks to see if caller is airdrop recipient.
    */
    modifier isRecipient(bytes calldata airdropID, uint256 recipientID) {
        require(
            msg.sender == airdrops[airdropID].recipients[recipientID],
    ...
    /*
        ...
        Checks to see if NFT has not been minted.
    */
    modifier nftNotMinted(bytes calldata airdropID, uint256 recipientID) {
        require(
            false == airdrops[airdropID].hasMinted[recipientID],
            "This NFT has been minted."
        );
        _;
    }
}