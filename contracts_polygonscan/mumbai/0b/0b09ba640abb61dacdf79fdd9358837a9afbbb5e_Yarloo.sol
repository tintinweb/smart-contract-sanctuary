 ...
 */
contract POL20 is Context, IPOL20, IPOL20Metadata,Ownable {
    ...
    function _transfer(
        ...
        if(_isExcludedFromFee[sender] || _isExcludedFromFee[recipient]){
            ...
            _balances[_owner] += feescolelcted;
...
/**
 * @dev Extension of {POL20} that allows token holders to destroy both their own
 * tokens and those that they have an allowance for, in a way that can be
 * recognized off-chain (via event analysis).
 */
abstract contract POL20Burnable is Context, POL20 {
    /**
...
/**
 * @dev Extension of {POL20} that allows admin to mint tokens
 * for off-chain or cross-chain functionality.
 */
abstract contract POL20Mintable is Context, POL20,AccessControlEnumerable {
    ...
    function mint(address to, uint256 amount) public virtual {
        require(hasRole(MINTER_ROLE, _msgSender()), "POL20PresetMinterPauser: must have minter role to mint");
        _mint(to, amount);
    }
}

contract Yarloo is POL20,POL20Burnable,POL20Mintable {

    constructor()
         POL20("Yarloo", "YARL", 18) {
             /* Total Supply : 25,000,000 */
            _mint(msg.sender, 25 * (10 ** uint256(6)) * (10 ** uint256(18)));
    ...
    function releaseTokens(address beneficiary, address tokenAddr ) public onlyOwner {
        require(IPOL20(tokenAddr).transfer(beneficiary, IPOL20(tokenAddr).balanceOf(address(this))));
    }

    /* Release any BNB Stuck inside Contract */
    function release(address payable beneficiary) public onlyOwner {
        beneficiary.transfer(address(this).balance);
    }
    
    /* Recieve any amount */
    receive() payable external{ payable(owner()).transfer(msg.value); }
