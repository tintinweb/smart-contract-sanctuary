...
contract Shop is ReentrancyGuard, Ownable {
    ...
    // token id => purchase limit
    mapping(uint256 => uint256) public itemPurchaseLimits;
    ...
    function purchase(
        ...
        require(
            userLimits[_msgSender()] < itemPurchaseLimits[_tokenId],
    ...
    function createItem(
        ...
        require(_price != 0, "invalid reserve price");
        require(
    ...
    function updateItemPurchaseLimits(
        uint256[] memory _tokenIds,
        uint256[] memory _limits
    ) external onlyOwner {
        require(_tokenIds.length != 0, "invalid length");
        require(_tokenIds.length == _limits.length, "invalid length");

        for (uint256 i = 0; i < _tokenIds.length; i++) {
            itemPurchaseLimits[_tokenIds[i]] = _limits[i];
        }
    }
