...
contract MaticMikeData{
    ...
    **********************************/
    ...
    function mapSvgString(uint256 _traitTypeIndex, uint256 _traitIndex) public onlyOwner{
        for (
            uint16 j = 0;
            j < traitTypes[_traitTypeIndex][_traitIndex].pixelCount;
            j++
        ) {
                string memory thisPixel = MaticMikeLibrary.substring(
                    traitTypes[_traitTypeIndex][_traitIndex].pixels,
                    j * 4,
                    j * 4 + 4
                );
                ...
                plotting[_traitTypeIndex][_traitIndex].push(Rect(
                x,
                y,
                string(
                    abi.encodePacked(
