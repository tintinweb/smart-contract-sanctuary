...
contract SymbolOracleOffChain is IOracleWithUpdate {
    ...
    constructor (address signatory_, uint256 delayAllowance_) {
    ...
    function updatePrice(uint256 symbol_, uint256 timestamp_, uint256 price_, uint8 v_, bytes32 r_, bytes32 s_) external override {
        uint256 lastTimestamp = timestamp;
        if (timestamp_ > lastTimestamp) {
            if (v_ == 27 || v_ == 28) {
                bytes32 message = keccak256(abi.encodePacked(symbol_, timestamp_, price_));
