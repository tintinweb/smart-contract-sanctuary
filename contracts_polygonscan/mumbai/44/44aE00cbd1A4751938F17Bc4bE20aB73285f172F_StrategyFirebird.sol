...
contract StrategyFirebird is Ownable, ReentrancyGuard, Pausable {
    ...
    uint256 public controllerFee = 65; // 6.5%
    ...
    address public converterContract;
    ...
    constructor(
        ...
        uint256 _pid,
        address _wantAddress, // lp address
        ...
        converterContract = _converterContract;
    ...
    function _farm() internal returns (uint256) {
        ...
        // change here
        IYuGenFarm(masterchefAddress).deposit(pid, wantAmt);
    ...
    function withdraw(address _userAddress, uint256 _wantAmt) external onlyOwner nonReentrant returns (uint256) {
        ...
        if (_wantAmt > wantAmt) {
            IYuGenFarm(masterchefAddress).withdraw(pid, _wantAmt.sub(wantAmt));
        ...
        IERC20(wantAddress).safeTransfer(converterContract , withdrawFee);
    ...
    function earn() external nonReentrant whenNotPaused onlyGov {
        // Harvest farm tokens
        IYuGenFarm(masterchefAddress).deposit(pid, 0);

        // Converts farm tokens into want tokens(yugen tokens)
        uint256 earnedAmt = IERC20(earnedAddress).balanceOf(address(this));

        if (earnedAmt > 0) {
            earnedAmt = distributeFees(earnedAmt); // 6.5 %
    ...
    function distributeFees(uint256 _earnedAmt) internal returns (uint256) {
        if (controllerFee > 0) {
            ...
            IERC20(wantAddress).transfer(converterContract , fee);
            // TODO: send to controller contract
            // _safeSwapWmatic(
            //     earnedAddress,
            //     wmaticAddress,
            //     fee,
    ...
    function vaultSharesTotal() public view returns (uint256) {
        (uint256 amount,) = IYuGenFarm(masterchefAddress).userInfo(pid, address(this));
        return amount;
    }
    ...
    function panic() external onlyGov {
        _pause();
        IYuGenFarm(masterchefAddress).emergencyWithdraw(pid);
    }
