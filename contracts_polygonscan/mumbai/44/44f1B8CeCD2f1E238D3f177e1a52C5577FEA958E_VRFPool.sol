...
contract VRFPool is VRFConsumerBase {
    ...
    mapping(uint256 => mapping(uint256 => bool)) generated;
    ...
    function addPool(uint256 _poolId, uint256 _length) external {
        ...
        // new pool
        pools[_poolId] = Pool(_poolId, _length);
    }
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        ...
        uint256 _rand = randomness % _max + 1;
        uint256 i = 1;
        bool found;
        while(!found) {
            if (generated[_poolId][i]) _rand++; // ignore 
            if (_rand == i) found = true;
            i++;
        }

        //update pool
        generated[_poolId][_rand] = true;
        list[_poolId][_index] = _rand;
        pools[_poolId].count++;

        emit DiceLanded(_poolId, _index, _rand);
