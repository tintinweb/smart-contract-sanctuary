...
contract MultiFeeDistribution is ReentrancyGuard, Ownable {
    ...
    function claimableRewards(address account) external view returns (RewardData[] memory reward) {
        reward = new RewardData[](rewardTokens.length);
        for (uint256 i = 0; i < reward.length; i++) {
            ...
            reward[i].token = rewardTokens[i];
            reward[i].amount = _earned(account, reward[i].token, balance, supply);
        }
        return reward;
    }
