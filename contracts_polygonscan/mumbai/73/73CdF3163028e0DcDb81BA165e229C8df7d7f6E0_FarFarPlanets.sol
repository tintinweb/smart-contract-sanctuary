...
// File: contracts/FarFarPlanets.sol
...
contract FarFarPlanets is ERC721, Ownable, RandomlyAssigned {
  using Strings for uint256;
  ...
  ) ERC721("FarFarPlanets", "FFP")
    RandomlyAssigned(maxSupply, 1) {
    setBaseURI(_initBaseURI);
    setNotRevealedURI(_initNotRevealedURI);
    for (uint256 a = 1; a <= maxMintAmount; a++) {
      mint();
    }
  }
  ...
  function mint() public payable {
    ...
    require(tokenCount() + 1 <= totalSupply(), "Max supply reached!");
    require(availableTokenCount() - 1 >= 0, "You can't mint more that available token count");
    ...
    if(tokenCount == totalSupply()) {
      giftAddress = payable(ownerOf(randomNum(tokenCount, block.timestamp, tokenCount) + 1)); 
      giftValue = address(this).balance * 100 / 100;
      winner = giftAddress;
      price = giftValue;
