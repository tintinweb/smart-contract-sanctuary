...
contract ERC1155NFT is ERC1155, Ownable, VRFConsumerBase{
    ...
    function uri(uint256 id) public view override returns(string memory) {
        require(id < 31 && id > 0, "ERC1155NFT::uri: Invalid id.");
    ...
    function buyPremiumPack() external payable preCheck{
        require(PremiumPack > 0, "Contract::buyPremiumPack: PremiumPacks are sold out.");
        require(LINK.balanceOf(address(this)) >= fee, "Not enough LINK - fill contract with faucet");
        require(msg.value <= PremiumPackFee, "Contract::buyPremiumPack: Insufficient amount.");
    ...
    function buyFreshPack() external payable preCheck {
        require(FreshPack > 0, "Contract::buyFreshPack: FreshPacks are sold out.");
        require(LINK.balanceOf(address(this)) >= fee, "Not enough LINK - fill contract with faucet");
        require(msg.value <= FreshPackFee, "Contract::buyFreshPack: Insufficient amount.");
        bytes32 requestId = requestRandomness(keyHash, fee);
    ...
    function buyBasePack() external payable preCheck {
        require(BasePack > 0, "Contract::buyFreshPack: FreshPacks are sold out.");
        require(LINK.balanceOf(address(this)) >= fee, "Not enough LINK - fill contract with faucet");
        require(msg.value <= BasePackFee, "Contract::buyBasePack: Insufficient amount.");
        bytes32 requestId = requestRandomness(keyHash, fee);
    ...
    function withdraw() external onlyOwner {
        require(address(this).balance >= 0, "Contract::withdraw: Not enough ETH.");
        (bool success, ) = owner().call{value: address(this).balance}("");
        require(success, "Contract::withdraw: Withdraw failed.");
    }
}