...
contract PrivateArcadeNFT is ERC721, AccessProtected {
    using Counters for Counters.Counter;
    Counters.Counter private NftId;

    CommunityArcadeNFT NFT;
    
    constructor(address _address) public ERC721("Arcade Network", "NFT") {
        NFT = CommunityArcadeNFT(_address);
    }

    function clone(address _recipient, uint256 _nftId) private returns(uint256){
        NftId.increment();
        uint256 newNftId = NftId.current();
        
        string memory _URI = NFT.tokenURI(_nftId);
        _mint(_recipient, newNftId);
    ...
    function burn(uint256 _nftId) public onlyAdmin {
        _burn(_nftId);
    }
}
