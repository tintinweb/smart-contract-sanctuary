...
// File: DynoMintPass/AbstractMintPassFactory.sol
...
abstract contract AbstractMintPassFactory is ERC1155Pausable, ERC1155Supply, ERC1155Burnable, Ownable {
	...
	event PauseStatus(bool);
	event URISet(string);
    
    function pause() external onlyOwner {
        _pause();
		emit PauseStatus(true);
    }

    function unpause() external onlyOwner {
        _unpause();
		emit PauseStatus(false);
    }    
...
// File: DynoMintPass/DynoMintPass.sol


contract DynoMintPass is AbstractMintPassFactory  {
    ...
    event AddMintPass(uint256 index);
    event UpdatedMintPass(uint256 index);
    ...
    )  ERC1155("http://dynonftadmin.projectsdemo.website/uploads/products/mintpass/{id}.json") {
        name_ = _name;
        symbol_ = _symbol;
    }
    ...
    function addMintPass(
        ...
        mp.totalpasses = _totalpasses;
		
		emit AddMintPass(mpCounter.current());
        mpCounter.increment();
    ...
    function editMintPass( 
        ...
        mintPasses[_mpIndex].totalpasses = _totalpasses; 
		emit UpdatedMintPass(_mpIndex);		
    }       
    ...
    function claim(
        ...
        require(mintPasses[mpIndex].claimedMPs[msg.sender].add(numPasses) <= mintPasses[mpIndex].totalpasses, "Claim: Not allowed to claim given amount");
    ...
    function getClaimedMps(uint256 poolId, address userAdress) external view returns (uint256) {
