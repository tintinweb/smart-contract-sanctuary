...
contract EverWeapons is ERC721Enumerable, Pausable {
    ...
    function mint (uint256 _amount) public whenNotPaused {
        ...
        require(saleConfig.startTime > 0, "EverWeapons_Mint: Sale is not active now.");
        require(
            block.timestamp >= saleConfig.startTime,
            "EverWeapons_Mint: Please wait a bit until sell time."
        );
        require(
            block.timestamp <= saleConfig.startTime + saleConfig.duration,
            "EverWeapons_Mint: Sale has ended."
        );
        require(_amount <= saleConfig.maxCount, "EverWeapons_Mint: Count exceeds max mint.");
