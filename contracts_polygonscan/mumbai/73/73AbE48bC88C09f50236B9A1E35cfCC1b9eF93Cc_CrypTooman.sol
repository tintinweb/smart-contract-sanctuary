...
contract CrypTooman is ERC20, Ownable {
    ...
    function addStableCoin(address stableCoinAddress, address aTokenAddress) public onlyOwner returns (uint256)
    {
    ...
    function developmentWithdraw(address tokenAddress, address recipient, uint256 amountUSD) external onlyOwner {
    ...
    function setEnabledAutoClaimProtocolRewards(bool enabledAutoClaimProtocolRewards_) public onlyOwner {
        _enabledAutoClaimProtocolRewards = enabledAutoClaimProtocolRewards_;
    }

    function claimProtocolRewards(address[] calldata aTokenAddresses) external returns (uint256) {
        return 0;
    }
    ...
    function setUSDDepositingStatus(uint256 USDDepositStatus_) public onlyOwner {
