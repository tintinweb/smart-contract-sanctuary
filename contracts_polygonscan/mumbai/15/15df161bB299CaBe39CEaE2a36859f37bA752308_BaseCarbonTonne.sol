    ...
    UUPSUpgradeable
{
    ...
    modifier onlyManagers() {
        require(
            dataManagers[msg.sender] == true || owner() == msg.sender,
    ...
    /// @notice add managers that can mint data tokens
    function addManagers(address manager) external virtual onlyOwner {
        dataManagers[manager] = true;
    }

    function removeManagers(address manager) external virtual onlyOwner {
        dataManagers[manager] = false;
    }
...
contract CarbonProjectsStorage {
    ...
    /// @dev keeps track of what accounts can manage data tokens
    mapping(address => bool) public dataManagers;
...
/// @notice Base Carbon Tonne pool for KlimaDAO
//  Logic for accepted tokens is defined in PoolFilter
/// @dev PoolFilter must be deployed separately and set via `setPoolFilter(...)`
    ...
    BaseCarbonTonneStorage
{
        ...
        initializer
    {
        ...
        __Ownable_init_unchained();
        __Pausable_init_unchained();
        __ERC20_init_unchained('Toucan Protocol: BaseCarbonTonne Pool', 'BCT');
    ...
    /// Todo: consider nested mappings to support future attributes w/o upgrade
