...
contract Lore is ERC721, Ownable, IERC2981, ERC2771Context {
    ...
    constructor(address forwarder) ERC721("Lore","LORE") ERC2771Context(forwarder) {
        // make a "root" node to be the base case
        sentence[STORY_ROOT] = '';
        imageStyle[STORY_ROOT] = 0;
        _safeMint(_msgSender(), STORY_ROOT);
        totalSupply.increment();
    }

    function mintSeed(string calldata _sentence, uint16 _imageStyle) public payable {
        require(msg.value == 0.08 ether || _msgSender() == owner(), "requires money");
    ...
    {
        bytes32 node = getMerkleLeaf(_msgSender());
        require(
            MerkleProof.verify(_merkleProof, mirrorMerkleRoot, node),
            "Non-backers must pay to mint"
        );
        require(!hasClaimedAllowlist[_msgSender()], "Already claimed");
        hasClaimedAllowlist[_msgSender()] = true;
        _mint(STORY_ROOT, _sentence, _imageStyle);
    }
    /* Convenience methods */
    ...
    * */
    ...
    function withdraw() external onlyOwner {
        (bool sent, ) = address(_msgSender()).call{
