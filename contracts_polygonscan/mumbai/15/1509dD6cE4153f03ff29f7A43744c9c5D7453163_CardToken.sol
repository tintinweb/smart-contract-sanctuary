    ...
    TemporaryLSP4Compatability
{
    ...
    constructor(
        ...
        // TODO: could pack bytes20(address) + bytes12(revenueShare)
    ...
    ICardMarket
{
    ...
    function buyFromMarket(bytes32 tokenId, uint256 amount, address referrer)
        public
        payable
        override
    {
        ...
        uint256 tokenOwnerAmount = IFeeCollector(_feeCollector).revenueShare(market.acceptedToken, amount, referrer);

        if (market.acceptedToken == address(0)) {
            require(msg.value == amount, "CardMarket: buy amount incorrect");
            // solhint-disable-next-line avoid-low-level-calls
            (bool success, ) = payable(tokenOwner).call{ value: tokenOwnerAmount }("");
...
interface ICardMarket {
    ...
    function buyFromMarket(bytes32 tokenId, uint256 amount, address referrer) external payable;
}
...
interface IFeeCollector {
  ...
  // TODO: do we want to support partial percentages?
  ...
  //
  // --- Revenue Share queries
  //

  function royalties() external view returns(Royalties memory);

  //
  // --- Revenue Share logic
  //

  function revenueShare(address token, uint256 amount, address referrer) external payable returns (uint256);

  function pullTokens(address[] memory tokenList) external;

  function pullTokensForMany(address[] memory addressList, address[] memory tokenList) external;
}