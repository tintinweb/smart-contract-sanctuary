...
///*PUBLIC_FUNCTIONS*
    ...
    function createParty(uint16[] memory idArray, uint8[] memory riskArray, bool deploy) external {
        ...
        uint16[] memory pushArray = new uint16[](idArray.length);
        for(uint i = 0; i < idArray.length; i++) {
            ...
            pushArray[i] = idArray[i];
        }
        DeploymentParty memory party = DeploymentParty(allTimePartyCount, pushArray, uint8(idArray.length),msg.sender, false);
