...
contract Raffle is VRFConsumerBase, Ownable {
    ...
    ) external onlyOperator returns (uint32 raffleId) {
     ...
     */
    function calculateRaffleResultForLargeTotal(uint32 raffleId) external {
     ...
     * check RaffleResult event of raffleId first, do not calculate twice
     */
    function calculateRaffleResultForSmallTotal(uint32 raffleId) external {
        require(raffleIdToRandomness[raffleId] > 0, "randomness not fulfilled");
