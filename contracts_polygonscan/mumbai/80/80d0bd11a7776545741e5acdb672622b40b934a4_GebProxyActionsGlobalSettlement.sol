...
// hevm: flattened sources of /nix/store/b748ii3m6qrlwq7qk8p0lfpl3f36awcf-geb-proxy-actions/dapp/geb-proxy-actions/src/GebProxyActions.sol
...
////// /nix/store/b748ii3m6qrlwq7qk8p0lfpl3f36awcf-geb-proxy-actions/dapp/geb-proxy-actions/src/GebProxyActions.sol
/// GebProxyActions.sol
...
abstract contract CollateralJoinLike_1 {
...
abstract contract CoinJoinLike_1 {
    function safeEngine() virtual public returns (SAFEEngineLike_2);
...
abstract contract TaxCollectorLike_1 {
...
contract Common {
    ...
    function _coinJoin_join(address apt, address safeHandler, uint wad) internal {
        ...
        CoinJoinLike_1(apt).systemCoin().approve(apt, wad);
        // Joins COIN into the safeEngine
        CoinJoinLike_1(apt).join(safeHandler, wad);
    }

    // Public functions
    function coinJoin_join(address apt, address safeHandler, uint wad) public {
        // Gets COIN from the user's wallet
        CoinJoinLike_1(apt).systemCoin().transferFrom(msg.sender, address(this), wad);
...
contract BasicActions is Common {
    ...
    function convertTo18(address collateralJoin, uint256 amt) internal returns (uint256 wad) {
        ...
        uint decimals = CollateralJoinLike_1(collateralJoin).decimals();
        wad = amt;
        if (decimals < 18) {
          wad = multiply(
              amt,
    ...
    function _getGeneratedDeltaDebt(
        ...
        uint rate = TaxCollectorLike_1(taxCollector).taxSingle(collateralType);
        require(rate > 0, "invalid-collateral-type");

        // Gets COIN balance of the handler in the safeEngine
        uint coin = SAFEEngineLike_2(safeEngine).coinBalance(safeHandler);
    ...
    function _getRepaidDeltaDebt(
        ...
        (, uint rate,,,) = SAFEEngineLike_2(safeEngine).collateralTypes(collateralType);
        require(rate > 0, "invalid-collateral-type");

        // Gets actual generatedDebt value of the safe
        (, uint generatedDebt) = SAFEEngineLike_2(safeEngine).safes(collateralType, safe);
    ...
    function _getRepaidAlDebt(
        ...
        uint coin = SAFEEngineLike_2(safeEngine).coinBalance(usr);
    ...
    function _generateDebt(address manager, address taxCollector, address coinJoin, uint safe, uint wad, address to) internal {
        ...
        if (SAFEEngineLike_2(safeEngine).canModifySAFE(address(this), address(coinJoin)) == 0) {
            SAFEEngineLike_2(safeEngine).approveSAFEModification(coinJoin);
        }
        // Exits COIN to this contract
        CoinJoinLike_1(coinJoin).exit(to, wad);
    }
    ...
    function _lockETH(
        ...
        SAFEEngineLike_2(ManagerLike(manager).safeEngine()).modifySAFECollateralization(
    ...
    function _repayDebt(
        ...
        if (own == address(this) || ManagerLike(manager).safeCan(own, safe, address(this)) == 1) {
            ...
            modifySAFECollateralization(manager, safe, 0, _getRepaidDeltaDebt(safeEngine, SAFEEngineLike_2(safeEngine).coinBalance(safeHandler), safeHandler, collateralType));
        } else {
             // Joins COIN amount into the safeEngine
            if (transferFromCaller) coinJoin_join(coinJoin, address(this), wad);
            else _coinJoin_join(coinJoin, address(this), wad);
            // Paybacks debt to the SAFE
            SAFEEngineLike_2(safeEngine).modifySAFECollateralization(
                collateralType,
                safeHandler,
                address(this),
                address(this),
    ...
    function _repayDebtAndFreeETH(
        ...
        modifySAFECollateralization(
            ...
            safe,
            -toInt(collateralWad),
            _getRepaidDeltaDebt(ManagerLike(manager).safeEngine(), SAFEEngineLike_2(ManagerLike(manager).safeEngine()).coinBalance(safeHandler), safeHandler, ManagerLike(manager).collateralTypes(safe))
        );
        ...
        CollateralJoinLike_1(ethJoin).exit(address(this), collateralWad);
        // Converts WETH to ETH
        CollateralJoinLike_1(ethJoin).collateral().withdraw(collateralWad);
    }
    ...
    function ethJoin_join(address apt, address safe, uint value) public payable {
        // Wraps ETH in WETH
        CollateralJoinLike_1(apt).collateral().deposit{value: value}();
        // Approves adapter to take the WETH amount
        CollateralJoinLike_1(apt).collateral().approve(address(apt), value);
        // Joins WETH collateral into the safeEngine
        CollateralJoinLike_1(apt).join(safe, value);
    }
    ...
    function freeETH(
        ...
        // Exits WETH amount to proxy address as a token
        CollateralJoinLike_1(ethJoin).exit(address(this), wad);
        // Converts WETH to ETH
        CollateralJoinLike_1(ethJoin).collateral().withdraw(wad);
...
contract GebProxyActions is BasicActions {
    ...
    function tokenCollateralJoin_join(address apt, address safe, uint amt, bool transferFrom) public {
        ...
        if (transferFrom) {
            ...
            CollateralJoinLike_1(apt).collateral().transferFrom(msg.sender, address(this), amt);
            // Approves adapter to take the token amount
            CollateralJoinLike_1(apt).collateral().approve(apt, amt);
        }
        // Joins token collateral into the safeEngine
        CollateralJoinLike_1(apt).join(safe, amt);
    }
    ...
    function freeTokenCollateral(
        ...
        CollateralJoinLike_1(collateralJoin).exit(msg.sender, amt);
    }
    ...
    function repayAllDebt(
        ...
        address safeHandler = ManagerLike(manager).safes(safe);
        bytes32 collateralType = ManagerLike(manager).collateralTypes(safe);
        (, uint generatedDebt) = SAFEEngineLike_2(safeEngine).safes(collateralType, safeHandler);
    ...
    function lockTokenCollateralAndGenerateDebt(
        ...
        CoinJoinLike_1(coinJoin).exit(msg.sender, deltaWad);
    }
    ...
    function openLockGNTAndGenerateDebt(
        ...
        CollateralLike_2(CollateralJoinLike_1(gntJoin).collateral()).transfer(bag, collateralAmount);
    ...
    function repayDebtAndFreeTokenCollateral(
        ...
        transferCollateral(manager, safe, address(this), collateralWad);
        // Exits token amount to the user's wallet as a token
        CollateralJoinLike_1(collateralJoin).exit(msg.sender, collateralAmount);
    }
...
contract GebProxyActionsGlobalSettlement is Common {
    ...
    function _freeCollateral(
        ...
        bytes32 collateralType = ManagerLike(manager).collateralTypes(safe);
        address safeHandler = ManagerLike(manager).safes(safe);
        SAFEEngineLike_2 safeEngine = SAFEEngineLike_2(ManagerLike(manager).safeEngine());
        uint generatedDebt;
        (lockedCollateral, generatedDebt) = safeEngine.safes(collateralType, safeHandler);

        // If SAFE still has debt, it needs to be paid
        if (generatedDebt > 0) {
            GlobalSettlementLike_2(globalSettlement).processSAFE(collateralType, safeHandler);
        ...
        GlobalSettlementLike_2(globalSettlement).freeCollateral(collateralType);
    }
    ...
    function freeTokenCollateral(
        ...
        uint amt = _freeCollateral(manager, globalSettlement, safe) / 10 ** (18 - CollateralJoinLike_1(collateralJoin).decimals());
        // Exits token amount to the user's wallet as a token
        CollateralJoinLike_1(collateralJoin).exit(msg.sender, amt);
    }
    ...
    function prepareCoinsForRedeeming(
        ...
        SAFEEngineLike_2 safeEngine = CoinJoinLike_1(coinJoin).safeEngine();
        ...
        GlobalSettlementLike_2(globalSettlement).prepareCoinsForRedeeming(wad);
    }
    ...
    function redeemETH(
        ...
        GlobalSettlementLike_2(globalSettlement).redeemCollateral(collateralType, wad);
        uint collateralWad = multiply(wad, GlobalSettlementLike_2(globalSettlement).collateralCashPrice(collateralType)) / RAY;
        // Exits WETH amount to proxy address as a token
        CollateralJoinLike_1(ethJoin).exit(address(this), collateralWad);
        // Converts WETH to ETH
        CollateralJoinLike_1(ethJoin).collateral().withdraw(collateralWad);
    ...
    function redeemTokenCollateral(
        ...
        GlobalSettlementLike_2(globalSettlement).redeemCollateral(collateralType, wad);
        // Exits token amount to the user's wallet as a token
        uint amt = multiply(wad, GlobalSettlementLike_2(globalSettlement).collateralCashPrice(collateralType)) / RAY / 10 ** (18 - CollateralJoinLike_1(collateralJoin).decimals());
        CollateralJoinLike_1(collateralJoin).exit(msg.sender, amt);
    }
}
...
contract GebProxyActionsCoinSavingsAccount is Common {
    ...
    function withdraw(
        ...
        uint bal = CoinJoinLike_1(coinJoin).safeEngine().coinBalance(address(this));
        // Allows adapter to access to proxy's COIN balance in the safeEngine
    ...
    function withdrawAll(
        ...
        CoinJoinLike_1(coinJoin).exit(msg.sender, multiply(accumulatedRates, savings) / RAY);
    }
}