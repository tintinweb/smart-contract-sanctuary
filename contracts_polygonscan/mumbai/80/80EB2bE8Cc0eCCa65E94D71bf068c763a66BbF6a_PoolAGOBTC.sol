...
contract PoolAGOBTC is Ownable, ReentrancyGuard, IPool {
    ...
    function mint(
        ...
        if (_target_collateral_ratio > 0) {
            _total_dollar_value = (_collateral_amount * 10**missing_decimals) * (COLLATERAL_RATIO_PRECISION) / (_target_collateral_ratio);
            if (_target_collateral_ratio < COLLATERAL_RATIO_MAX) {
                _required_share_amount = (_total_dollar_value - _collateral_amount) * PRICE_PRECISION / _share_price;
            }
        } else {
    ...
    function redeem(
        ...
        if (_effective_collateral_ratio > 0) {
             _collateral_output_amount = _dollar_amount_post_fee / (10**missing_decimals) * _effective_collateral_ratio / PRICE_PRECISION;
        }
