...
interface BinamarsNFT is IERC721 {
    struct Bina {
        uint256 level;
        uint256 exp;
        uint256 bornAt;
    ...
    function getBina(uint256) external view returns (Bina memory);
}
...
contract Marketplace {
    ...
    IERC20 public binaToken;
    BinamarsNFT public binaNFT;
    IBinamarsManager public router;
    ...
    constructor(
        ...
        binaToken = IERC20(_binaToken);
        binaNFT = BinamarsNFT(_binaNFT);
        router = IBinamarsManager(_manager);
    }

    modifier notEgg(uint256 _tokenId) {
        require(binaNFT.getBina(_tokenId).level != 0, "not evolved");
        _;
    }
    ...
    function placeOrder(uint256 _tokenId, uint256 _price)
        ...
        notEgg(_tokenId)
    {
        require(binaNFT.ownerOf(_tokenId) == msg.sender, "Only owner can sell");
        require(_price > 0, "Cannot sell without fee");
        binaNFT.transferFrom(msg.sender, address(this), _tokenId);
    ...
    function cancelOrder(uint256 _tokenId) public {
        ...
        binaNFT.transferFrom(address(this), msg.sender, _tokenId);
    ...
    function fillOrder(uint256 _tokenId) public {
        ...
        binaToken.transferFrom(msg.sender, router.feeAddress(), feeMarket);
