...
contract BasicPurchaseLDA is RoyalPausableUpgradeable {
    ...
    // (nonce => used) track which nonces we've used
    mapping(uint128 => bool) public _noncesUsed;
    ...
    function registerTierForPurchase(uint128 tierID, uint256 priceWei, address erc20Contract, address ldaContract, address ldaHolderForTier) external onlyOwner {
        // Check the requirements
        require(tierPaymentERC20Contract[tierID] == address(0), "E20 token registered for tier");
        require(tierPurchasePriceWei[tierID] == 0, "tier buy price already set");
        require(tierLDAContract[tierID] == address(0), "lda contract already registered for tier");
        require(tierLDAHolder[tierID] == address(0), "lda holder already registered for tier");
    ...
    function updateTierPurchaseRegistration(uint128 tierID, uint256 priceWei, address erc20Contract, address ldaContract, address ldaHolderForTier) external onlyOwner {
        tierPaymentERC20Contract[tierID] = erc20Contract;
        tierPurchasePriceWei[tierID] = priceWei;
        tierLDAContract[tierID] = ldaContract;
        tierLDAHolder[tierID] = ldaHolderForTier;
    ...
    // User calls this function to actually execute a buy
    // signedData = EthSignature(ldaID <> buyer <> reservationID, ownerWalletPrivateKey);
    function buy(uint256 ldaID, address buyer, uint128 reservationID, bytes memory signedData) external {
        (uint128 tierID,) = _decomposeLDA_ID(ldaID);
        // Check that this tier has been registered 
        require(tierPurchasePriceWei[tierID] != 0);
        require(tierPaymentERC20Contract[tierID] != address(0));

        // Check the signedData
        requireValidSignature(ldaID, buyer, reservationID, signedData);

        // Store nonce so it can't be used again 
        _noncesUsed[reservationID] = true;
        ...
        uint256 startingBalance = IERC20(paymentAddress).balanceOf(_paymentWallet);

        // Check that we have an allowance of at least the purchasePrice
        require(IERC20(paymentAddress).allowance(buyer, _paymentWallet) <= purchaseAmount, "insufficient allowance remaining for purchase"); 

        // Move the token 
        IERC20(paymentAddress).transferFrom(buyer, _paymentWallet, purchaseAmount);

        // Move the LDA
        ERC1155Upgradeable(tierLDAContract[tierID]).safeTransferFrom(tierLDAHolder[tierID], buyer, ldaID, 1, "");

        // Run final checks
        require(ERC1155Upgradeable(tierLDAContract[tierID]).balanceOf(buyer, ldaID) == 1, "lda not transferred");
        require(IERC20(paymentAddress).balanceOf(_paymentWallet) == (startingBalance + purchaseAmount), "paid less than expected");
    }

    function requireValidSignature(uint256 ldaID, address buyer, uint128 nonce, bytes memory messageSignature) internal view {
        require(!_noncesUsed[nonce], "Nonce has already been used");
        // abi.encode(ldaID, buyer, nonce);
        // abi.encodePacked(ldaID, buyer, nonce);

        // Recreate the msg hash
        bytes32 messageHash = keccak256(abi.encode(ldaID, buyer, nonce)); 
        ...
        require(messageSigner == buyer, "Signature invalid");
    }

    function testHash(uint256 ldaID, address buyer, uint128 nonce, bytes memory messageSignature) public pure returns(address) { 
        bytes32 messageHash = keccak256(abi.encode(ldaID, buyer, nonce)); 
        bytes32 messageDigest = ECDSA.toEthSignedMessageHash(messageHash);
        return ECDSA.recover(messageDigest, messageSignature);
    }
    
    /**
    @dev Decompose a raw ldaID into it's two composite parts
     */
    function _decomposeLDA_ID(uint256 ldaID) internal pure returns (uint128 tierID, uint128 tokenID) {
