...
ERC1155SupplyUpgradeable
{
    ...
    function uri(uint256 tokenId) public view virtual override returns (string memory) {
        // require(totalSupply(tokenId) == 1, "ERC1155Metadata: URI query for nonexistent token");
    ...
    function mint(address _to, uint256 _id) public onlyOwner {
        ...
        require(cornersRemaining > 0, "no corners remain");
        _reserveCorner(_id);
        _mint(_to, _id, 1, "0x00");
    }

    function mintBatch(address _to, uint[] memory _ids) public onlyOwner {
        require(_ids.length <= cornersRemaining, "fewer corners remain");
    ...
    // NEW STUFF BELOW
	function mintCorner(uint256 _howMany) public payable {
        if (msg.sender != owner()) {
		    require(cost.mul(_howMany) == msg.value, "insufficient ETH");
		    require(_howMany <= maxMintAmount, "too many corners at once");
        }
        require(_howMany <= cornersRemaining, "fewer corners remain");
    ...
    // INFO: derived from https://ethereum.stackexchange.com/a/110885/31055
    ...
    function mintedCorners() public view returns (uint256) {
        return maxSupply - cornersRemaining;
    }

    function _cornerAt(uint256 i) private view returns (uint256) {
        // NOTE: unintialized data is non-zero in solidity
    ...
    function _reserveRandomCorner() private returns (uint256) {
        require(cornersRemaining > 0, "all corners minted");
        uint256 i = _lolRandom(cornersRemaining); // Pick a random i from 0<->cornersRemaining
    ...
    // This part is the O(1) magic.
    function _reserveCorner(uint256 i) private {
        // Swap the i'th corner with the last-most one.
        _movedCorners[i] = _cornerAt(cornersRemaining - 1);
        _movedCorners[cornersRemaining - 1] = 0;
