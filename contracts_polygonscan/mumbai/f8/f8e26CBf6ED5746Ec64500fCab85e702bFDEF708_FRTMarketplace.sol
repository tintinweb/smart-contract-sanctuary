...
contract FRTMarketplace is Ownable{
    ...
    function setRewardsData(Reward[] memory _reward) public onlyOwner{
        for(uint i=1; i <= _reward.length; i++ ){
    ...
    function giveReward(address _publisher) internal {
        ...
        uint256 _year = (contractCreatedAt - _now) / 365 days;
        if(rewardsData[_year].issuedReward < rewardsData[_year].maxReward && rewardsData[_year].publishedModules < rewardsData[_year].modules ){
