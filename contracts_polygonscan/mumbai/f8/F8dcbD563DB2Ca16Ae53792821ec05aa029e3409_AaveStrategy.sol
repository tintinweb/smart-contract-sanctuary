...
contract AaveStrategy is Ownable, ReentrancyGuard {
    ...
    constructor(ILendingPoolAddressesProvider _lendingPoolAddressesProvider, uint16 _referralCode) {
    ...
    function deposit(
        ...
        uint256 amount,
        address user
    ) external ensureNonZeroAddress(token) nonReentrant {
        ILendingPool lendingPool = getLendingPool();
        IERC20(token).approve(address(lendingPool), amount);
        lendingPool.deposit(token, amount, user, referralCode);
    }
    ...
    function withdraw(
        ...
        ILendingPool lendingPool = getLendingPool();
        lendingPool.withdraw(token, amount, user);
    }
}
