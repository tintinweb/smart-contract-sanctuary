...
contract NumberMinter is ERC721Enumerable, AccessControl, PriceConsumerV3 {
  ...
  string private _dataTypeTokenURI = "data:application/json;base64,";

  event EventBatchMint(
    address _to,
  ...
  // The tokenURIs will be set via the function setTokenURI() by frontend
  // or later by some backend component based on the emited event "EventBatchMint"
  // "num" is the number of tokens to be minted
  function batchMintWithoutTokenURI(uint256 num, uint256 wethAmount)
    external
    isActive
  {
    ...
    onlyDbilia
  {
    ...
    //                           first tokenId    last tokenId
    emit EventBatchMint(receiver, supply, supply + (numTokens - 1), 0);
  }
