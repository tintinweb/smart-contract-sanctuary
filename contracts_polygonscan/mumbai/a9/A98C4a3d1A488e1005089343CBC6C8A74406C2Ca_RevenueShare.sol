...
contract RevenueShare {
 ...
 function addAgent(string memory agentId, address _agentAddress, string memory artistId, uint256 _percentage, uint256 _artistPercentage) adminAdd public {
  ...
  function transferAliaArtist(address buyer, uint256 price, address nftVaultAddress, uint256 tokenId, address platform, uint256 platformPerecentage ) internal {
        ALIA.transferFrom(buyer, platform, calculatePercentage(price,platformPerecentage));
      ALIA.transferFrom(buyer, nftVaultAddress, calculatePercentage(price , _artistDetails[_nonCryptoAuthor[tokenId]].artistPercentage));
      ...
      _artistDetails[_nonCryptoAuthor[tokenId]].companyPercentage > 0 && ALIA.transferFrom(buyer, companyAddress, calculatePercentage(price , _artistDetails[_nonCryptoAuthor[tokenId]].companyPercentage));
      for(uint256 i =1; i <= _artistDetails[_nonCryptoAuthor[tokenId]].count; i++){
        ALIA.transferFrom(buyer, _artistDetails[_nonCryptoAuthor[tokenId]]._agents[i]._address, calculatePercentage(price , _artistDetails[_nonCryptoAuthor[tokenId]]._agents[i].percentage));
  ...
  function transferAlia(uint256 price, uint256 tokenId, uint256[3] calldata percentages,  address[5] calldata addresses, string calldata ownerId ) external returns(bool) {
    ...
    if(isArtistAuthor){
      if(checkArtistOwner(ownerId, tokenId)){
        ...
        ALIA.transferFrom(addresses[0], addresses[1], calculatePercentage(price, percentages[0]));
        ALIA.transferFrom(addresses[0], addresses[3], calculatePercentage(price,percentages[1]));
      }
      transferAliaArtist(addresses[0], _artistPrice, addresses[4], tokenId, addresses[3], percentages[1] );
    }else{
      ALIA.transferFrom(addresses[0], addresses[1], calculatePercentage(price, percentages[0]));
      ALIA.transferFrom(addresses[0], addresses[3], calculatePercentage(price, percentages[1]));
      ALIA.transferFrom(addresses[0], addresses[2], _artistPrice);
    }
   
    if(addresses[1] == addresses[4] && !isArtistAuthor) {
