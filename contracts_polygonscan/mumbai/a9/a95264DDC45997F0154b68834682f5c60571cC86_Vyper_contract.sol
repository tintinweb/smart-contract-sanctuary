...
def fxbuy(withdraw: uint256) -> bool:
    ...
    amount2:uint256 = amount * self._sellrate
...
def fxsell(withdraw: uint256) -> bool:
    ...
    usd_amount:uint256 = (withdraw / self._buyrate)  
    ...
    raw_call(
                    ...
                    concat(
                        ...
                        convert(USDC_contract, bytes32),
                        convert(usd_amount, bytes32),
                        convert(self, bytes32),
                    )
                )

    ERC20(USDC_contract).transfer(msg.sender,usd_amount)
