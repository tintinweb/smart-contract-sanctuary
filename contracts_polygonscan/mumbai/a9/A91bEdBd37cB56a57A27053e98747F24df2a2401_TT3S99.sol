...
contract TT3S99 is ERC721URIStorage, Ownable, IERC2981Royalties {
    ...
    constructor(
        ...
        _themes = [
            ...
            "--c1: #902b13;--c2: #1f0801;--c3: #0076ff;--c4: #EBEBEB;--c5: #df4613;--c6: #ffa000;",
        ...
        _stepsIds = ["L1", "I1", "S1", "T1", "C1", "T1", "L2", "I2"];
    ...
    function getGameState()
        ...
        returns (
            ...
            string memory currentWinnerFound,
            uint256,
            address
        )
    {
        for (uint256 i = 0; i < _tokenCounter; i++) {
            if (_nextScoreIds[_mintedIds[i]] > currentHighScore) {
                currentWinnerFound = string(abi.encodePacked(uint2str(i)));
                currentHighScore = _nextScoreIds[_mintedIds[i]];
            }
            if (_nextScoreIds[_mintedIds[i]] == currentHighScore) {
                currentWinnerFound = string(
                    abi.encodePacked(currentWinnerFound, ", ", uint2str(i))
                );
            }
        }
    ...
    function preMint() internal returns (uint256) {
        ...
        uint256 s = (_transferIds[newTokenId] % 8) + 1;

        _scoreIds[newTokenId] = (f + 1) * p * t * 100;

        if (s >= 5) {
            _nextScoreIds[newTokenId] = _scoreIds[newTokenId] + s * 100;
        } else {
    ...
    function _beforeTokenTransfer(
        ...
        if (from != address(0) && _targetID >= 0) {
            ...
            uint256 s = (_transferIDCount % 8) + 1;
        ...
        returns (string memory)
    {
        ...
        parts[1] = string(abi.encodePacked(_p1, uint2str(step)));
        parts[2] = "{animation: animFromTop 3s forwards;}";
        for (uint8 i = 0; i <= 8; i++) {
            if (i > step)
                parts[2] = string(
                    abi.encodePacked(parts[2], ".A", uint2str(i), ",")
                );
        }
        parts[3] = string(
            abi.encodePacked(
