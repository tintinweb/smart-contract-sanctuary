    ...
    ContextMixin
{
     ...
     */
    function updateAssetCurrentValue(uint256 index, uint256 _currentValue)
        external
        only(RELAY_ADMIN_ROLE)
    {
     ...
     * TODO:
     */
    function updateAssetName(uint256 index, string calldata _name)
        external
        only(RELAY_ADMIN_ROLE)
    {
        Asset storage _asset = assets[index];
    ...
    // added for relay services i.e. itx infura TEST ONLY !!!!!!!!!!!
    // function _isOwner(
    //     bytes32 message,
    //     uint256 nonce,
     ...
     */
    function withdraw(uint256 tokenId) external only(RELAY_ADMIN_ROLE) {
