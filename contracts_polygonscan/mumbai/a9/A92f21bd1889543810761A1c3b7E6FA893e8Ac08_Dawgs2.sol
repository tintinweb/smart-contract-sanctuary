...
contract Dawgs2 is ERC20Burnable, ERC20Permit, Ownable {
    ...
    uint256 private constant __rocketRatioFull = 1e7; // 10 million : 1
    uint256 private constant __rocketRatioHalf = 2e7; // 20 million : 1
    uint256 private constant __rocketSupply = 200000 * 1e9; // 200,000 with 9 decimals
    ...
    event Deployed(address sender, address __rocketSupply);
    event UpgradeDawgs(address sender, uint256 amountDawgs, uint256 amountRocket);
    ...
    constructor(address _dawgsV1, uint256 _startTime) ERC20(__name, __symbol) ERC20Permit(__name) {
        ...
        // The following condition is always true unless there is an error in the rocket contract.
        require(
            rocket.balanceOf(address(this)) == __rocketSupply,
            "Dawgs2: Unexpected Rocket supply"
        );
        rocket.transfer(_msgSender(), __rocketSupply / 2);
        // dawgsV1 contact
        dawgsV1 = IERC20(_dawgsV1);
        // Start Time
        startTime = _startTime;
        emit Deployed(_msgSender(), address(rocket));
    }
    ...
    function upgradeDawgs(uint256 amount) external {
        ...
        if (passedTime < 259200) {
            // First 72 hours after start
            rocketAmount = amount / __rocketRatioFull;
        } else if (passedTime < 259200 * 2) {
            // Second 72 hours after start
            rocketAmount = amount / __rocketRatioHalf;
        } else {
            rocketAmount = 0;
        }
        ...
        if (rocketAmount > 0) {
            rocket.transfer(sender, rocketAmount);
        }
        emit UpgradeDawgs(sender, amount, rocketAmount);
    }

    function rocketToken() external view returns (address) {
        return address(rocket);
    }
    ...
    function mintRockets(address to, uint256 amount) public onlyOwner {
        rocket.mint(to, amount);
    }

    function burnRockets(uint256 amount) public onlyOwner {
        rocket.burn(amount);
    }

    function transferRocketsOwnership(address newOwner) public onlyOwner {
...
contract Rocket is ERC20Burnable, ERC20Permit, Ownable {
    uint8 private constant __decimals = 9;
