...
contract Opener is Pausable,Ownable {
    Token private _lepaToken;
    address private _feeAddress;
    uint public collCount;
    ...
    function setOFC(uint256 _colNum,iCollection _colAddr,string calldata _name,uint256 _cnt) external onlyOwner {
    ...
    function verify(address _signer,address _to,uint256[] calldata _type,uint256[] calldata _ids, string[] calldata _uris,
        uint256 _lepa,uint256 _nonce,bytes calldata signature ) public view returns (bool) {
            
        bytes32 ethMessageHash = toEthSignedMessageHash(keccak256(abi.encode(_to, _type,_ids,_uris,_lepa,_nonce)));
        return SignatureChecker.isValidSignatureNow(_signer,ethMessageHash, signature);
    }
    
    function openMagicBox(address _to,uint256[] calldata _type,uint256[] calldata _ids,
    string[] calldata _uris,uint256 _lepa,uint256 _nonce,bytes calldata signature ) 
    external whenNotPaused{
        require(_type.length == _ids.length,"invalid arguments");
        require(!usedNonces[_nonce],"already used");
        require(verify(owner(),_to,_type,_ids,_uris,_lepa,_nonce,signature),"invalid request");
        
        usedNonces[_nonce] = true;

        if (_lepa>0)
        require(_lepaToken.transferFrom(_msgSender(), _feeAddress, _lepa), "Token transfer failed!"); 
    ...
    function getTokensOfOwner(uint bcindex,address _user) external view returns (uint256[] memory){
