...
contract NFTAirdrop {
    ...
    event AirdropRedeemed(bytes airdropID, address recipient);
    ...
    function addRecipient(
        ...
        require(
            airdrops[airdropID].recipients[recipientAddress] == 0,
            "Recipient ID already assigned."
        );
        airdrops[airdropID].totalSupply = airdrops[airdropID].totalSupply.add(1);
        airdrops[airdropID].recipients[recipientAddress] = airdrops[airdropID].totalSupply;
        emit AirdropRecipientAdded(airdropID, recipientAddress);
    }
    ...
    function addBunchRecipients(
        ...
        for(uint256 i = 0; i < recipientAddress.length; i++){
            addRecipient(airdropID, recipientAddress[i]);
        }
    }

    function mintAirdrop(bytes calldata airdropID)
        external
        airdropExists(airdropID)
        isRecipient(airdropID)
        nftNotMinted(airdropID)
    {
        airdrops[airdropID].hasMinted[msg.sender] = true;
        uint256 edition = airdrops[airdropID].recipients[msg.sender];
        nftMinter.mintFor(
            msg.sender,
    ...
    */
    modifier isRecipient(bytes calldata airdropID) {
        require(
            0 != airdrops[airdropID].recipients[msg.sender],
    ...
    */
    modifier nftNotMinted(bytes calldata airdropID) {
        require(
            false == airdrops[airdropID].hasMinted[msg.sender],
