...
contract GalaxyStorage {
    ...
    bool public takeAdminFee;
    bool public takeLiquidityFee;
    bool public takeBurnFee;
...
contract GalaxyCoin is Initializable, ERC20Upgradeable, GalaxyStorage, GalaxyEvents {
    ...
    /**
     * Function to update fee percentages
     */
    function updateFeePercent(uint256 _adminFeePercent, uint256 _liquidityFeePercent, uint256 _burnPercent) 
    ...
    /**
     * Function to update fee flags
     */
    function updateFeeFlags(bool _takeAdminFee, bool _takeLiquidityFee, bool _takeBurnFee) external virtual onlyOwner 
    {
        takeAdminFee = _takeAdminFee;
        takeLiquidityFee = _takeLiquidityFee;
        takeBurnFee = _takeBurnFee;
    }
     ...
     */
    function calculateTaxes(uint256 _amount) public virtual view returns(uint256 tokensToTransfer, 
    uint256 tokensToAdmin, uint256 tokensToLiquidity, uint256 tokensToBurn) {
        tokensToTransfer = _amount;

        if (takeAdminFee) {
            tokensToAdmin = percent(_amount, adminFeePercent);
            tokensToTransfer = tokensToTransfer.sub(tokensToAdmin).sub(tokensToAdmin);
        }

        if (takeLiquidityFee) {
            tokensToLiquidity = percent(_amount, liquidityFeePercent);
            tokensToTransfer = tokensToTransfer.sub(tokensToLiquidity);
        }
        
        if (takeBurnFee) {
            tokensToBurn = percent(_amount, burnPercent);
            tokensToTransfer = tokensToTransfer.sub(tokensToBurn);
        }

        return (tokensToTransfer, tokensToAdmin, tokensToLiquidity, tokensToBurn);
    }
    ...
    function _collectTaxes(address _sender, uint256 _amount) internal returns(uint256) {
        (uint256 tokensToTransfer, uint256 tokensToAdmin, uint256 tokensToLiquidity, uint256 tokensToBurn) = 
            calculateTaxes(_amount);
