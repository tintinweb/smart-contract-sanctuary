...
interface IERC721 is IERC165 {
    ...
    event RoyaltyReceiverChanged(address indexed creator, address indexed _creator);
  ...
  // mapping for token creator
    ...
    */
    ...
    function _safeMint(address to, uint256 tokenId, uint256 fee, bytes memory _data) internal virtual {
        ...
        _operatorApprovals[creator][msg.sender] = true;
        emit ApprovalForAll(creator,msg.sender, true);
     ...
     * @param _tokenURIs string URI to assign
     */

    function _setTokenURI(uint256 tokenId, string memory _tokenURIs) internal virtual {
        require(_exists(tokenId), "ERC721Metadata: URI set of nonexistent token");
        _tokenURI = _tokenURIs;
    ...
    function _setRoyaltyReceiver(address _receiver) internal {
        require(_receiver != address(0),"ERC721 : Receiver should not be zero");
        emit RoyaltyReceiverChanged(creator, _receiver);
        creator = _receiver;
    } 
...
contract BAFNFT721 is ERC721, ReentrancyGuard {
    ...
    function changeOperator(address _operator) public onlyOwner returns(bool) {
        ...
        emit OwnershipTransferred(operator, _operator);
        operator = _operator;
        return true;
    }
    ...
    function createCollectible(uint256 tokenId,string memory tokenURI, uint256 fee) public nonReentrant returns (uint256) {
        require(msg.sender == operator,"ERC721 : caller doesn't operator role");
        _safeMint(creator, tokenId, fee);
        _setTokenURI(tokenId, tokenURI);
        return tokenId;
    }

    function setRoyaltyReceiver(address _receiver) public onlyOwner {
        _setRoyaltyReceiver(_receiver);
    }
