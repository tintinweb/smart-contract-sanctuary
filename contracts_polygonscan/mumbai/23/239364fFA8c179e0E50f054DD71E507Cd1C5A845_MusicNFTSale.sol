...
contract MusicNFTSale is ISale, Ownable, ReentrancyGuard {
  ...
  //   string dbId; // for syncing app database
  // }
  ...
  function createSale(
    ...
    require(dbIds[dbId] == false, "sale:UUID already in use");
    _saleId.increment();
  ...
  function buy(
    ...
    IMusicNFT Nft = IMusicNFT(currentSale.nftContract);
  ...
  function claimNfts(uint256 saleId) external {
    ...
    sales[saleId].purchased = sales[saleId].amount;
    IMusicNFT Nft = IMusicNFT(sales[saleId].nftContract);
    Nft.safeTransferFrom(
      address(this),
      sales[saleId].owner, 
