...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    // Token name
    string private _name = "ShootToContentNFT";

    // Token symbol
    string private _symbol = "CNFT";
...
contract ShootToContentNFT is ERC721Enumerable, AccessControl {
    using Strings for uint256;
  //  event Mint(address indexed sender, uint256 startWith, uint256 times);
    ...
    string public _contractURI = "https://shoot.to/cm/";
    ...
    //constructor args
    constructor(  
        uint256 _royaltyFee) ERC721("ShootToContentNFT", "CNFT") {
        _setRoyaltyWallet(royaltyWallet);
        _setRoyaltyFee(_royaltyFee);
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setupRole(MINTER_ROLE, msg.sender);
        _setupRole(MINTER_ROLE, address(0xfd70FE24caB65988F87E25DA57501F4cc035E60A));
        _setupRole(MINTER_ROLE, address(0x29258D38454A0fd148c9C28c95aecFDA4a63BD3e));
        _setupRole(MINTER_ROLE, address(0xd81AE27e925F63bC16D6b8A2b63686D63C47329e));
        _setupRole(MINTER_ROLE, address(0xcF4a0BD37c53698Ae179980FC7d82421139Ac81d));
    }
    mapping (uint256 => string) private _tokenURIs;
    ...
    function mint(uint256 tokenId, string memory tokenURI) public {
     ...
     function mintTo(uint256 tokenId, string memory tokenURI, address mintToAddress) public {
        require(hasRole(MINTER_ROLE, msg.sender), "Caller is not a minter");
        require(started, "not started");
        require(mintToAddress != address(0), "INVALID_WALLET");
    ...
    function ViewtokenURI(uint256 tokenId) public view virtual  returns (string memory) {
        require(_exists(tokenId), "ERC721Metadata: URI set of nonexistent token");
