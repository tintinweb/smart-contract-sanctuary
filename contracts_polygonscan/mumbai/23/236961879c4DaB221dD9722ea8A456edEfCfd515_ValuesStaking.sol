...
contract ValuesStaking is Ownable, IValuesStaking {
    ...
    /**
        @notice stake VALUES to enter warmup with referral fee distribution
        @param _amount uint
        @return bool
     */
    function stakeWithReferral(
        ...
        for (uint256 i = 0; i < _referrals.length; i++) {
            Claim memory info = warmupInfo[_referrals[i]];
            require(!info.lock, 'Deposits for referral are locked');
            totalFees = totalFees + _fees[i];
        }
        require(totalFees < _amount, 'Referral fees exceed amount');
        IERC20(VALUES).safeTransferFrom(msg.sender, address(this), _amount);
        ...
        for (uint256 i = 0; i < _referrals.length; i++) {
            warmupInfo[_referrals[i]] = Claim({
                deposit: info.deposit.add(_fees[i]),
                gons: info.gons.add(IsVALUES(sVALUES).gonsForBalance(_fees[i])),
                expiry: epoch.number.add(warmupPeriod),
                lock: false
            });
        }
        uint256 recipientAmount = _amount - totalFees;
        warmupInfo[_recipient] = Claim({
            deposit: info.deposit.add(recipientAmount),
            gons: info.gons.add(
                IsVALUES(sVALUES).gonsForBalance(recipientAmount)
            ),
            expiry: epoch.number.add(warmupPeriod),
            lock: false
        });
