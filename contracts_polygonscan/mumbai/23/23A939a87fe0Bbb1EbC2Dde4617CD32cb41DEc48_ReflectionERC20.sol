...
contract ReflectionERC20 is ERC20 , Ownable {
    ...
    function initDistributor(address distributor_) external {
        ...
        require(hodlRewardDistributor.owner() == address(this));
    ...
    function transferFrom(address from_, address to_, uint256 amount_) public virtual override returns (bool) {
        // check allowance
        require(allowance(from_ ,_msgSender() ) >= amount_);        
    ...
    function _customTransfer(address from_, address to_, uint256 amount_) internal returns (bool){
        ...
        if(whitlisted[from_].tax || whitlisted[to_].tax || _inSwap ){
        ...
        if(taxAmount > 0) {
            _transfer(from_, address(this) , taxAmount);


            uint256 swapOut = _swapTokenWorWavax(taxAmount);
            ...
            uint256 treasuryAmount = swapOut.mul(currentAppliedTax.team).div(100);
            uint256 charityAmount = swapOut.sub(teamAmount).sub(holderAmount).sub(treasuryAmount);

            IWAVAX(wavax).withdraw(teamAmount.add(treasuryAmount).add(charityAmount));
            if(teamAmount > 0)
            ...
            IWAVAX(wavax).approve(address(hodlRewardDistributor), holderAmount);
            hodlRewardDistributor.depositRewards(holderAmount);
        }
        return true;
    }

    function _swapTokenWorWavax(uint256 amountIn_) internal isInSwap returns(uint256 swapOut) {
        // aprove 
        _approve(address(this),swapRouter, amountIn_);
