...
import "./SquidGameBase.sol";

contract SquidGame is
    ERC721Enumerable,
    Ownable,
    VRFConsumerBase,
    ...
    SquidGameBase
{
    ...
    uint256[] private lostCharacters;
    ...
    //     uint256 newBossHp,
    //     uint256 newPlayerHp,
    //     uint256 damangeToBoss,
    //     uint256 damangeToPlayer
    // );

    constructor(
        CharacterAttributes[] memory charactersInfo,
        BigBoss memory bossInfo,
        ...
        ERC721("SquidGame", "SQU")
    {
        bigBoss = BigBoss({
            name: bossInfo.name,
            imageURI: bossInfo.imageURI,
            hp: bossInfo.hp,
            maxHp: bossInfo.hp,
            attackDamage: bossInfo.attackDamage
        });

        for (uint256 i = 0; i < charactersInfo.length; i++) {
            defaultCharacters.push(
                CharacterAttributes({
                    index: i,
                    name: charactersInfo[i].name,
                    imageURI: charactersInfo[i].imageURI,
                    hp: charactersInfo[i].hp,
                    maxHp: charactersInfo[i].maxHp,
                    attackDamage: charactersInfo[i].attackDamage
                })
            );
        }
        ...
        chainlinkFee = chainlinkConfig.chainlinkFee;
    ...
    function mintCharacterNFT(uint256 _characterIndex) external payable {
        ...
        for (tokenId = 1; tokenId <= totalNFTs; tokenId++) {
            if (nftHolderAttributes[tokenId].index == _characterIndex) {
        ...
        require(
            isMintedAlready == false,
            "The selected characterIndex is minted already"
        );
    ...
    function attackBoss(uint256 tokenId) public {
        ...
        require(boss.hp > 0, "Boss doesn't have enough HP");
    ...
    // This function returns a random number in a callback function - fulfillRandomness
    ...
    function expand(
        ...
        uint256 n,
        bytes32 requestId
    ) internal returns (uint256[] memory expandedValues) {
        expandedValues = new uint256[](n);
        for (uint256 i = 0; i < n; i++) {
            expandedValues[i] = uint256(keccak256(abi.encode(randomValue, i)));
        }

        uint256 tokenIdOfPlayer = requestIdToTokenId[requestId];
        address requestAddress = requestIdToAddress[requestId];

        BigBoss storage boss = playerToBoss[requestAddress];
        CharacterAttributes storage player = nftHolderAttributes[
            tokenIdOfPlayer
        ];
        // generate a random number between 1 ~ attack damage
        uint256 attackDamageToBoss = (expandedValues[0] % player.attackDamage) +
            1;
        uint256 attackDamageToPlayer = (expandedValues[1] % boss.attackDamage) +
            1;

        if (boss.hp < attackDamageToBoss) {
            boss.hp = 0;
        } else {
            boss.hp = boss.hp - attackDamageToBoss;
        }

        // Allow boss to attack player.
        if (player.hp < attackDamageToPlayer) {
            player.hp = 0;
        } else {
            player.hp = player.hp - attackDamageToPlayer;
        }

        if (boss.hp == 0 || player.hp == 0) {
            lostCharacters.push(tokenIdOfPlayer);
        }
 ...
 *Pulled from https://gist.github.com/farzaa/f13f5d9bda13af68cc96b54851345832 
 */
...
contract SquidGameBase {
    /*** EVENTS ***/

    /// @notice The CharacterNFTMinted event is emitted when a new token is minted
    /// through mintCharacterNFT method.
    /// @param sender The address of the minter
    /// @param tokenId The token id that a minter is trying to mint
    /// @param characterIndex The order index of the token (This is not the same as the tokenId)
    event CharacterNFTMinted(
        address sender,
        uint256 tokenId,
        uint256 characterIndex
    ...
    /// @notice The AttackComplete event is emitted when the boss is attacked through
    /// attackBoss method
    /// @param newBossHp The new HP of the boss
    /// @param newPlayerHp The new HP of the player
    /// @param damangeToBoss The amount of damange done by the player to the boss
    /// @param damangeToPlayer The amount of damange done by the boss to the player
    event AttackComplete(
        uint256 newBossHp,
        uint256 newPlayerHp,
        uint256 damangeToBoss,
