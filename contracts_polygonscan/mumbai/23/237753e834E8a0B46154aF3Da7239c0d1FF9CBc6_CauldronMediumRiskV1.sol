...
contract CauldronMediumRiskV1 is BoringOwnable, IMasterContract {
    ...
    struct AccrueInfo {
        uint64 lastAccrued;
        uint128 feesEarned;
        uint256 liqFeeEarned;
    }

    AccrueInfo public accrueInfo;

    // Settings
    uint256 private constant INTEREST_PER_SECOND = 335331078; // 1% p.a.
    ...
    uint256 private constant LIQUIDATION_MULTIPLIER = 108000; // add 8%
    uint256 private constant LIQUIDATION_MULTIPLIER_PRECISION = 1e5;

    uint256 private constant LIQUIDATION_FEE = 20000; // 20%
    uint256 private constant LIQUIDATION_FEE_PRECISION = 1e5;    
    
    uint256 private constant BORROW_OPENING_FEE = 500; // 0.5%
    uint256 private constant BORROW_OPENING_FEE_PRECISION = 1e5;

    uint256 public accumulatedLiquidationFee = 0;
    ...
    function liquidate(
        ...
        uint256 collateralFees = (allCollateralShare.sub(allCollateralShare.mul(LIQUIDATION_MULTIPLIER_PRECISION)/LIQUIDATION_MULTIPLIER)).mul(LIQUIDATION_FEE)/LIQUIDATION_FEE_PRECISION;
        uint256 allBorrowShare = bentoBox.toShare(magicInternetMoney, allBorrowAmount, true);

        // Swap using a swapper freely chosen by the caller
        // Open (flash) liquidation: get proceeds first and provide the borrow after
        accrueInfo.liqFeeEarned = accrueInfo.liqFeeEarned + collateralFees;
        bentoBox.transfer(collateral, address(this), to, allCollateralShare.sub(collateralFees));
        if (swapper != ISwapper(0)) {
            swapper.swap(collateral, magicInternetMoney, msg.sender, allBorrowShare, allCollateralShare.sub(collateralFees));
        }
    ...
    /// @notice Withdraws the liquidation fees accumulated.
    function withdrawLiquidationFees() public {
        accrue();
        address _feeTo = masterContract.feeTo();
        uint256 feesShare = accrueInfo.liqFeeEarned;
        bentoBox.transfer(collateral, address(this), _feeTo, feesShare);
        accrueInfo.liqFeeEarned = 0;

        emit LogWithdrawFees(_feeTo, feesShare);
    }
