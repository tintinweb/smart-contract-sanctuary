...
contract MosaicVaultConfig is IMosaicVaultConfig, VaultConfigBase {
    ...
    /// @notice fee threshold before become linear
    ...
    /// @notice Address where fees should be transfer
    ...
    */
    ...
    /// @param _feeAddress Address where fees should be transfer
    /// @param _mosaicHolding Address of the MosaicHolding contract
    function initialize(address _feeAddress, address _mosaicHolding) public initializer {
        require(_mosaicHolding != address(0), "ERR: HOLDING ADDRESS");
        require(_feeAddress != address(0), "ERR: FEE ADDRESS");
    ...
    /// @notice External function called by the user to calculate the fee
    ...
    function setThresholdFee(uint256 _newThresholdFee) external override onlyOwner {
        require(_newThresholdFee < 100, "ERR: THRESHOLD > FACTOR");
    ...
    function setFeeAddress(address _newFeeAddress) external override onlyOwner {
        require(_newFeeAddress != address(0), "ERR: FEE ADD");
    ...
    ReentrancyGuardUpgradeable
{
    ...
    function getTokenBalance(address _token) public view virtual override returns (uint256) {
        require(address(mosaicHolding) != address(0), "Mosaic Holding address not set");
