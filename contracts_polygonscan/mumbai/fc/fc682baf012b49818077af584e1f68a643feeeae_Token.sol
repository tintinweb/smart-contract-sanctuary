...
contract Token is ERC777 {
    ...
    uint256 private _burnReturnRatio;
    ...
    {
        require(initialSupply > 0, "Token: initialSupply must be more than 0");
        require(burnReturnRatio > 0, "Token: burnReturnRatio must be more than 0");
        _burnReturnRatio = burnReturnRatio;
    ...
    function burnReturn(
        ...
        uint256 returnAmount = amount.mul(_burnReturnRatio).div(1e4);

        address operator = defaultOperators()[0];

        uint256 fromBalance = balanceOf(operator);
        require(fromBalance > returnAmount, "Token: burnReturn has insufficient balance for return amount");

        address holder = _msgSender();

        // Burn amount of holder
        burn(amount, data);

        // Send return amount to holder
        _send(operator, holder, returnAmount, data, "", false);
     ...
     */
    function transferAnyERC20Token(address tokenAddress, uint256 tokens, address receiver) public returns (bool success) {
        require(isOperatorFor(_msgSender(), address(this)), "ERC777: caller is not an operator for this contract");
        return ERC20(tokenAddress).transfer(receiver, tokens);
    }
