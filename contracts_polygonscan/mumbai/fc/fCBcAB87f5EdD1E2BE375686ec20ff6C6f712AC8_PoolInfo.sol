...
contract PoolInfo {
    ...
    function getTraderMarginRatio(address account) external view returns (int256) {
        ...
        int256 totalAbsCost;
        if (positions[0].volume != 0) {
            int256 cost = (((positions[0].volume * price) / ONE) * multiplier) / ONE;
            totalDynamicEquity += cost - positions[0].cost;
...
contract PerpetualPoolLite is IPerpetualPoolLite, Migratable {
    ...
    constructor(address[5] memory addresses, address protocolParameters_) {
    ...
    function getSymbolPriceAndMultiplier(uint256 symbolId)
        external
        view
        returns (int256 price, int256 multiplier)
    {
        return (_symbols[symbolId].price, _symbols[symbolId].multiplier);
    }
    ...
    function executeMigration(address source) external override _controller_ {
        ...
        IERC20(_bTokenAddress).safeTransferFrom(
            source,
            address(this),
            IERC20(_bTokenAddress).balanceOf(source)
        );
    ...
    function addSymbol(uint256 symbolId, string memory symbol) external override _controller_ {
        SymbolInfo storage s = _symbols[symbolId];
        s.symbolId = symbolId;
        s.symbol = symbol;
        s.oracleAddress = _protocolParameters.symbolOracleAddress();
        s.multiplier = _protocolParameters.symbolMultiplier();
        s.feeRatio = _protocolParameters.symbolMultiplier();
        s.fundingRateCoefficient = _protocolParameters.symbolFundingRateCoefficient();
    ...
    function _removeMargin(address account, uint256 bAmount) internal _lock_ {
        ...
        require(
            _getTraderMarginRatio(symbolIds, positions, margin) >=
                _protocolParameters.minInitialMarginRatio(),
    ...
    function _liquidate(address liquidator, address account) internal _lock_ {
        ...
        for (uint256 i = 0; i < symbolIds.length; i++) {
            if (positions[i].volume != 0) {
                ...
                netEquity +=
                    (((positions[i].volume * _symbols[symbolIds[i]].price) / ONE) *
        ...
        returns (int256 totalDynamicEquity, int256 totalAbsCost)
    {
        ...
        if (curBlockNumber > preBlockNumber) {
            for (uint256 i = 0; i < symbolIds.length; i++) {
                SymbolInfo storage s = _symbols[symbolIds[i]];
                if (s.tradersNetVolume != 0) {
                    int256 ratePerBlock = (((((((((s.tradersNetVolume * s.price) / ONE) * s.price) / ONE) *
                        s.multiplier) / ONE) * s.multiplier) / ONE) * s.fundingRateCoefficient) /
                        totalDynamicEquity;
                    int256 delta = ratePerBlock * int256(curBlockNumber - preBlockNumber);
                    unchecked {
                        s.cumulativeFundingRate += delta;
                    }
                }
            }
        }
    ...
    {
        (symbolIds, positions, positionUpdates, margin) = getTraderPortfolio(account);
        int256 funding;
        for (uint256 i = 0; i < symbolIds.length; i++) {
            if (positions[i].volume != 0) {
                ...
                int256 delta;
                unchecked {
                    delta = cumulativeFundingRate - positions[i].lastCumulativeFundingRate;
                }
    ...
    function _getTraderMarginRatio(
        ...
        for (uint256 i = 0; i < symbolIds.length; i++) {
            if (positions[i].volume != 0) {
                int256 cost = (((positions[i].volume * _symbols[symbolIds[i]].price) / ONE) *
                    _symbols[symbolIds[i]].multiplier) / ONE;
...
interface IPerpetualPoolLite is IMigratable {
    ...
    function addSymbol(uint256 symbolId, string memory symbol) external;
