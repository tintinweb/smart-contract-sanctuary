...
contract BananaBar is ERC20, Ownable {
    ...
    constructor() ERC20("Banana Bar", "$BAR") {}

    function setBabyApesRescue(address _addr) external onlyOwner {
        babyApesRescue = IBabyApesRescue(_addr);
    }
    ...
    function claimReward() external {
        require(rewardPaused, "Rewards claiming is paused");
        ...
        uint256 _monsterRewards = monsterRewards[msg.sender];
        ...
        if (totalRewardsWithoutMonster > 0) {
            ...
            for (
                uint256 _index;
                _index < monsterOwnersList.length;
                _index += 1
            ) {
                monsterRewards[monsterOwnersList[_index]] += rewardPerMonster;
            }
        }
    ...
    */
    function updateReward(address _from, address _to) external {
        require(msg.sender == address(babyApesRescue), "Caller not authorized");
    ...
    function tipBabyApeHolder(uint256 _tokenId, uint256 _amount) external {
        require(_tokenId > 0 && _tokenId <= 25000, "Invalid token id");
        address _to = babyApesRescue.ownerOf(_tokenId);
        require(
