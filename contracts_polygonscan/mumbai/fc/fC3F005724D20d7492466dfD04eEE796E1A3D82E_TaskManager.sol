...
contract TaskManager is IIterableFunctions, StorageAccessor, FeeManager, EscrowManager {
    ...
    function getFunctions() override external view returns (bytes4[] memory){
        ...
        functions[0] = this.__AccessLevel_init.selector;
        functions[1] = this.grantRole.selector;
        functions[2] = this.hasRole.selector;
        functions[3] = this.revokeRole.selector;
        functions[4] = this.updateStorage.selector;
        functions[5] = this.createTask.selector;
        functions[6] = this.submitSolution.selector;
        functions[7] = this.acceptSolution.selector;
        return functions;
    }
    ...
    function createTask(
        ...
        if (msg.value > 0) {
            ...
            if (taskParams.isHyveCommission) {
                checkUserTokenBalance(_msgSender(), getStorage().getHyveERC(), tax);
                require(msg.value == taskParams.amount, "TM: Attempted to deposit a different amount of ETH than declared");
            } else {
                require(msg.value == taskParams.amount + tax, "TM: Attempted to deposit a different amount of ETH than declared");
            }
        } else {
            require(
                taskParams.tokenAddress != getStorage().getHyveERC() || taskParams.isHyveCommission,
        ...
        //interactions
        if (taskParams.isHyveCommission) {
            splitTax(_msgSender(), isEth, tax, IERC20Upgradeable(getStorage().getHyveERC()));
        } else {
        ...
        uint256 applicationId = getStorage().getNewApplicationId();
        getStorage().addNewApplication(applicationId, TaskStorage.Application(taskId, applicant));
    ...
    function cancelTask(uint256 taskId) external {
        TaskStorage.Task memory task = getStorage().getTask(taskId);
        require(task.creator != address(0), "TM: non-existent task");
        require(_msgSender() == task.creator, "TM: not task creator");
        require(task.deadline < block.timestamp, "TM: deadline not reached!");
        require(task.applicationId == 0, "TM: application already accepted!");
        require(task.solutionId == 0, "TM: solution already accepted!");
        getStorage().cancelTask(taskId);
    ...
    function submitSolution(uint256 applicationId, string memory solutionUri, string memory solutionHash) external {
        require(applicationId <= getStorage().getLastApplicationId(), "TM: nonexistent application");
        ...
        require(applicationId == task.applicationId, "TM: application is not accepted for task");
    ...
    function acceptSolution(uint256 solutionId) external {
        require(solutionId <= getStorage().getLastSolutionId(), "TM: nonexistent solution");
        ...
        payBeneficiary(application.taskId, application.worker, IERC20Upgradeable(task.token), task.amount);
...
abstract contract ERC20Taker {
    ...
    function takeERC20(ERC20Giver mediator, address from, IERC20Upgradeable token, uint256 amount) virtual public {
        mediator.giveERC20(from, address(this), token, amount);
    }
}
...
//@dev - keep these stateless
abstract contract ERC20Giver is AccessLevel {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    function giveERC20(address from, address to, IERC20Upgradeable token, uint256 amount) external onlyRole(OPERATOR_ROLE) {
...
abstract contract StorageAccessor is IAddressManager, AccessLevel {
    ...
    */
    ...
    function splitTax(address customer, bool isEthTax, uint256 taxValue, IERC20Upgradeable token) internal {
        ...
        if (isEthTax) {
            ...
            ERC20Taker(getAddressStorage().getStaking()).takeERC20(ERC20Giver(this), customer, token, stakingValue);
            ERC20Taker(getAddressStorage().getVault()).takeERC20(ERC20Giver(this), customer, token, vaultValue);
        }
    }
}
...
abstract contract EscrowManager is IAddressManager, ERC20Giver {
    ...
    function depositToEscrow(uint256 taskId, address user, IERC20Upgradeable token, uint256 amount) internal {
        IEscrow(getAddressStorage().getEscrow()).createDeposit(address(this), taskId, user, address(token), amount);
    }

    function redeemDepositFromEscrow(uint256 taskId) internal {
        IEscrow(getAddressStorage().getEscrow()).redeemDeposit(taskId);
    }
