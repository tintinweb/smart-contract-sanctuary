...
contract VillaSchedule is IVillaSchedule, Ownable, VillaRoles, CoreUtility {
    ...
    ///      Below are the concrete numbers in this list, which are also tested in "test/VillaSchedule.ts".
    ///
    ///      ```
    ...
    IERC20 public immutable villa;
    uint256 public immutable startTimestamp;

    uint256 public minted;

    constructor(address villa_, uint256 startTimestamp_) public VillaRoles() {
        require(
    ...
    function initialize() external initializer {
        __Ownable_init();
        villa.safeTransferFrom(msg.sender, address(this), MAX_SUPPLY);
    }
    ...
    /// @notice Get the release rate of Villa token at the given timestamp
    /// @param timestamp Timestamp for release rate
    /// @return Release rate (number of Villa token per second)
    ...
    /// @notice Creates `amount` Villa tokens and assigns them to `account`,
    ...
    function mint(address account, uint256 amount) external override onlyMinter {
        ...
        villa.safeTransfer(account, amount);
