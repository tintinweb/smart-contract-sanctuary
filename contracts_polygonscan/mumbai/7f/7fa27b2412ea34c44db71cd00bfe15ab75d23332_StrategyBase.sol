...
abstract contract StrategyFarmBase is BaseStrategyMasterChef {
    ...
    // Uniswap swap paths
    address[] public alpha_matic_path;
    ...
    {
        alpha_matic_path = new address[](2);
        alpha_matic_path[0] = alpha;
        alpha_matic_path[1] = matic;
    }
    ...
    function harvest() public override {
        ...
        if (_alphaBalance > harvestCutoff){
            if (_alphaBalance > 0) {
                _swapUniswapWithPath(alpha_matic_path, _alphaBalance.div(2));
            }

            // Add liquidity for ALPHA/MATIC
            uint256 _matic = IERC20(matic).balanceOf(address(this));
            uint256 _alpha = IERC20(alpha).balanceOf(address(this));
            if (_matic > 0 && _alpha > 0) {
...
contract StrategyBase is StrategyFarmBase {
    ...
    address public ALPHA_MATIC_LP = 0xEcbC3BABEe201E72EeF09471bbaBcD00D3a0024a;
