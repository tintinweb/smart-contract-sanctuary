...
contract DexilonTest_v5 {
    ...
    */
        ...
        ) 
    {
        ...
        btcBalance = usersAssetsBalances[user]['BTC'];
        ethBalance = usersAssetsBalances[user]['ETH'];
        xrpBalance = usersAssetsBalances[user]['XRP'];
        solBalance = usersAssetsBalances[user]['SOL'];
    ...
    function updateUserBalances(
        ...
        if (assetBalance == 0  
            ...
            newAvailableBalance = availableBalance
                - abs(amountWei*rate/(leverage*BTC_UNIT_FACTOR));

        } else if (assetBalance > 0 && amountWei < 0) {

            calculateRate = leverage*lockedBalance*BTC_UNIT_FACTOR/assetBalance;
            newAvailableBalance = availableBalance 
                - amountWei*calculateRate/(leverage*BTC_UNIT_FACTOR)
                + amountWei*(calculateRate - rate)/(BTC_UNIT_FACTOR);
        ...
        if ((assetBalance > 0 && amountWei < 0) || (assetBalance < 0 && amountWei > 0)) {
            ...
            newLockedBalance = lockedBalance + abs(amountWei*rate/(leverage*BTC_UNIT_FACTOR));
        }
