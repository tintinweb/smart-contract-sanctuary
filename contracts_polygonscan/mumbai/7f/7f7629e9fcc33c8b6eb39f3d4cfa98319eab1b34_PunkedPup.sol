    ...
    Pausable
{
    ...
    // mint price after paid tokens
    uint256 public MINT_PRICE_FOR_BONES;
    ...
    // uint256 public PRE_SALE_TOKENS;
    // number of tokens have been minted so far
    uint16 public minted;
    // max count for regular pups
    uint256 public maxRegularPupCount;
    // max count for pugs
    uint256 public maxPugCount;
    // max count for cats
    uint256 public maxCatCount;
    // max count for dogcatchers
    uint256 public maxDogCatcherCount;
    // max count for custom pups
    uint256 public maxCustomPupCount;
    // count for pugs
    uint256 public regularPupCount;
    // count for pugs
    uint256 public pugCount;
    // count for cats
    uint256 public catCount;
    // count for dogcatchers
    uint256 public dogCatcherCount;
    // count for custom pups
    uint256 public customPupCount;
    ...
    mapping(uint256 => PupCat) public tokenTraits;
    ...
    // event for minting
    event Mint(address _to, uint256 indexed tokenId, uint8 indexed alphaIndex);
    ...
    function initializePunkedPup(
        ...
        WETHTokenAddress = 0x6f9A40e4d528d9978E0d18c510c2DAE58e73C6B9;
        WETHToken = IERC20(WETHTokenAddress);
        MINT_PRICE = 0.055 ether;
        PAID_TOKENS = 10000;
        // PRE_SALE_TOKENS = 1500;
        baseUri = "https://api.punkedpups.com/nftMetadata/";
        contractUri = "https://api.punkedpups.com/nftCollectionMetadata/";
        mintLimit = 10;
        // whitelistMintLimit = 10;
        maxRegularPupCount = 7881;
        maxPugCount = 1000;
        maxCatCount = 750;
        maxDogCatcherCount = 250;
        maxCustomPupCount = 119;
    }
    ...
    function mint(uint256 amount) external payable whenNotPaused nonReentrant {
        ...
        if (_msgSender() != owner()) {
            ...
            if (minted < PAID_TOKENS) {
                ...
                uint256 price = 0;
                if (minted > 5000) {
                    price = amount * MINT_PRICE;
                } else if (!_whitelistAddresses[_msgSender()].isWhitelisted) {
                    if (_count[_msgSender()] == 0) {
                        price = (amount - 1) * MINT_PRICE;
                    } else {
                        price = amount * MINT_PRICE;
                    }
                }
                WETHToken.transferFrom(_msgSender(), owner(), price);
            }
            // else {
            //     require(msg.value == 0);
            // }
        ...
        for (uint256 i = 0; i < amount; i++) {
            ...
            uint8 alphaIndex;
            // uint256 supply = tokenIds[i];
            if (tokenTraits[minted].alphaIndex == 0) {
                if (
                    (seed % 2 == 0 || seed % 3 == 0) &&
                    regularPupCount <
                    (maxRegularPupCount * ((minted / 1000) + 1)) / 10
                ) {
                    // add in regular pups
                    regularPupCount++;
                    alphaIndex = uint8(randomWithRange(seed, 5)) + 1;
                    tokenTraits[minted] = PupCat(true, alphaIndex, false);
                    // NFTtype[minted] = "regular";
                } else {
                    // add in other catagories
                    uint256 seed2 = random(seed);
                    if (
                        seed2 % 2 == 0 &&
                        pugCount < (maxPugCount * ((minted / 1000) + 1)) / 10
                    ) {
                        // add in pugs
                        pugCount++;
                        alphaIndex = 6;
                        ...
                        uint256 seed3 = random(seed2);
                        if (
                            seed3 % 2 == 0 &&
                            catCount <
                            (maxCatCount * ((minted / 1000) + 1)) / 10
                        ) {
                            // add in cats
                            catCount++;
                            alphaIndex = uint8(randomWithRange(seed, 5)) + 1;
                            tokenTraits[minted] = PupCat(
                                false,
                                alphaIndex,
                                false
                            );
                            ...
                            seed3 % 3 == 0 &&
                            dogCatcherCount <
                            (maxDogCatcherCount * ((minted / 1000) + 1)) / 10
                        ) {
                            // add in dog catcher
                            dogCatcherCount++;
                            alphaIndex = uint8(randomWithRange(seed, 5)) + 1;
                            ...
                            // NFTtype[minted] = "dogcatchers";
                        } else if (
                            seed3 % 5 == 0 &&
                            customPupCount <
                            (maxCustomPupCount * ((minted / 1000) + 1)) / 10
                        ) {
                            // add in custom pups
                            customPupCount++;
                            alphaIndex = 8;
     ...
     * @param newPrice new price for mint
     */
     ...
     * @param newPrice new price for mint
     */
    function setMintPriceForBones(uint256 newPrice) public onlyOwner {
        MINT_PRICE_FOR_BONES = newPrice;
    }
     ...
     * @param seed a value ensure different outcomes for different sources in the same block
     * @param mod it provides range for random number
     * @return a pseudorandom value
     */
...
abstract contract ReentrancyGuard is Initializable {
    ...
    function initializeReentrancyGuard() public initializer {
        _status = _NOT_ENTERED;
    }
...
abstract contract Pausable is Context, Initializable {
    ...
    function initializePausable() public initializer {
        _paused = false;
    }
...
contract BONE is ERC20, Ownable {
    ...
    function initializeBone(string memory name_, string memory symbol_)
        public
        initializer
    {
        ERC20.initialize(name_, symbol_); // Do not forget this call!
        Ownable.initialize();
    }
