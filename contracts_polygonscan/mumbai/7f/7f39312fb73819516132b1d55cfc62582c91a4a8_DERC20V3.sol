...
contract DERC20V3 is Initializable, ERC20Upgradeable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function initialize() initializer public {
        __ERC20_init("DERC20V3", "DERC20V3");
        __Ownable_init();
		...
		// ~10 tokens per day
		GROWTH_RATE = 115740740740740;
		START = block.timestamp - (10 days);

    }

     // called on D transfers
	function updateReward(address from, address to) external {
		require(msg.sender == address(DMint));
		if(from != address(0)){
	...
	function getPendingReward(address user) internal view returns(uint256) {
		return DMint.balanceOf(user) * GROWTH_RATE * (block.timestamp - (lastUpdate[user] >= START ? lastUpdate[user] : START));
	}

	function setDMint(address dMintAddy) external {
		//require(hasRole(MINTER_ROLE, _msgSender()), "ERC20PresetMinterPauser: must have minter role to set dMintAddy");
		DMint = IDMint(dMintAddy);
	}

	function fertilize(uint256 growth) external {
