...
contract Exchange is Ownable, ReentrancyGuard {
    ...
    function swap(uint256 _bTCPAmount) external nonReentrant whenNotPaused {
        ...
        require(TCP.balanceOf(address(this)) != 0, "Not enough TCP token in the contract");
        require(bTCP.balanceOf(msg.sender) >= _bTCPAmount, "Not enough bTCP token in your wallet");

        IERC20(bTCP).safeTransferFrom(msg.sender, BURN_ADDRESS, _bTCPAmount);
        IERC20(TCP).safeTransfer(msg.sender, _bTCPAmount);
