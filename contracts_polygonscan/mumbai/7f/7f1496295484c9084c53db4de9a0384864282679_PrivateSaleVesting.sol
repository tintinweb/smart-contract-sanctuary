...
contract PrivateSaleVesting is ReentrancyGuard,Ownable {
    ...
    uint256 public unlockPeriod = 20;
    ...
    function createVestingSchedules(
        ...
        require(_beneficiaries.length > 0, "createVestingSchedules: Empty Data");
        require(
     ...
     */
    function createVestingSchedule(address _beneficiary, uint256 _amount) external onlyOwner {
    ...
    external view
    returns (uint256 _vestedAmount, uint256 _totalClaimed, uint256 _claimableBalance, uint256 _remainingBalance) {
        return (
    ...
    function _createVestingSchedule(address _beneficiary, uint256 _amount) internal {
        require(_beneficiary != address(0), "createVestingSchedule: Beneficiary cannot be empty");
        require(_amount > 0, "createVestingSchedule: Amount cannot be empty");

        // Ensure one per address
        require(vestedAmount[_beneficiary] == 0, "createVestingSchedule: Schedule already in flight");
    ...
    function _claim(address _beneficiary) internal {
        require(vestedAmount[_beneficiary] > 0, "_claim: There is no schedule currently in flight");
        ...
        require(token.transfer(_beneficiary, amount), "_claim: Unable to transfer tokens");
    ...
    function _availableClaimableAmount(address _beneficiary) internal view returns (uint256 _amount) {
        ...
        // Vesting completed 
            ...
            _getNow() >= start &&
            _getNow() < start.add(unlockPeriod)
        ) {
            uint256 maxClaimable = (vestedAmount[_beneficiary].mul(666)).div(10000);
            ...
            _getNow() >= start.add(unlockPeriod) &&
            _getNow() < start.add(unlockPeriod.mul(2))
        ) {
            uint256 maxClaimable = (vestedAmount[_beneficiary].mul(1332)).div(10000);
            uint256 userTotalClaimed = totalClaimed[_beneficiary];
            if(userTotalClaimed < maxClaimable) {
                _amount = maxClaimable.sub(userTotalClaimed);
            }
            ...
            _getNow() >= start.add(unlockPeriod.mul(2)) &&
            _getNow() < start.add(unlockPeriod.mul(3))
        ) {
            uint256 maxClaimable = (vestedAmount[_beneficiary].mul(1998)).div(10000);
            uint256 userTotalClaimed = totalClaimed[_beneficiary];
            if(userTotalClaimed < maxClaimable) {
                _amount = maxClaimable.sub(userTotalClaimed);
            }
            ...
            _getNow() >= start.add(unlockPeriod.mul(3)) &&
            _getNow() < start.add(unlockPeriod.mul(4))
        ) {
            uint256 maxClaimable = (vestedAmount[_beneficiary].mul(2664)).div(10000);
            uint256 userTotalClaimed = totalClaimed[_beneficiary];
            if(userTotalClaimed < maxClaimable) {
                _amount = maxClaimable.sub(userTotalClaimed);
            }
            ...
            _getNow() >= start.add(unlockPeriod.mul(4)) &&
            _getNow() < start.add(unlockPeriod.mul(5))
        ) {
            uint256 maxClaimable = (vestedAmount[_beneficiary].mul(333)).div(1000);
            uint256 userTotalClaimed = totalClaimed[_beneficiary];
            if(userTotalClaimed < maxClaimable) {
                _amount = maxClaimable.sub(userTotalClaimed);
            }
            ...
            _getNow() >= start.add(unlockPeriod.mul(5)) &&
            _getNow() < start.add(unlockPeriod.mul(6))
        ) {
            uint256 maxClaimable = (vestedAmount[_beneficiary].mul(3996)).div(10000);
            uint256 userTotalClaimed = totalClaimed[_beneficiary];
            if(userTotalClaimed < maxClaimable) {
                _amount = maxClaimable.sub(userTotalClaimed);
            }
            ...
            _getNow() >= start.add(unlockPeriod.mul(6)) &&
            _getNow() < start.add(unlockPeriod.mul(7))
        ) {
            uint256 maxClaimable = (vestedAmount[_beneficiary].mul(4662)).div(10000);
            uint256 userTotalClaimed = totalClaimed[_beneficiary];
            if(userTotalClaimed < maxClaimable) {
                _amount = maxClaimable.sub(userTotalClaimed);
            }
            ...
            _getNow() >= start.add(unlockPeriod.mul(7)) &&
            _getNow() < start.add(unlockPeriod.mul(8))
        ) {
            uint256 maxClaimable = (vestedAmount[_beneficiary].mul(5328)).div(10000);
            uint256 userTotalClaimed = totalClaimed[_beneficiary];
            if(userTotalClaimed < maxClaimable) {
                _amount = maxClaimable.sub(userTotalClaimed);
            }
            ...
            _getNow() >= start.add(unlockPeriod.mul(8)) &&
            _getNow() < start.add(unlockPeriod.mul(9))
        ) {
            uint256 maxClaimable = (vestedAmount[_beneficiary].mul(5994)).div(10000);
            uint256 userTotalClaimed = totalClaimed[_beneficiary];
            if(userTotalClaimed < maxClaimable) {
                _amount = maxClaimable.sub(userTotalClaimed);
            }
            ...
            _getNow() >= start.add(unlockPeriod.mul(9)) &&
            _getNow() < start.add(unlockPeriod.mul(10))
        ) {
            uint256 maxClaimable = (vestedAmount[_beneficiary].mul(666)).div(1000);
            uint256 userTotalClaimed = totalClaimed[_beneficiary];
            if(userTotalClaimed < maxClaimable) {
                _amount = maxClaimable.sub(userTotalClaimed);
            }
            ...
            _getNow() >= start.add(unlockPeriod.mul(10)) &&
            _getNow() < start.add(unlockPeriod.mul(11))
        ) {
            uint256 maxClaimable = (vestedAmount[_beneficiary].mul(7326)).div(10000);
            uint256 userTotalClaimed = totalClaimed[_beneficiary];
            if(userTotalClaimed < maxClaimable) {
                _amount = maxClaimable.sub(userTotalClaimed);
            }
            ...
            _getNow() >= start.add(unlockPeriod.mul(11)) &&
            _getNow() < start.add(unlockPeriod.mul(12))
        ) {
            uint256 maxClaimable = (vestedAmount[_beneficiary].mul(7992)).div(10000);
            uint256 userTotalClaimed = totalClaimed[_beneficiary];
            if(userTotalClaimed < maxClaimable) {
                _amount = maxClaimable.sub(userTotalClaimed);
            }
            ...
            _getNow() >= start.add(unlockPeriod.mul(12)) &&
            _getNow() < start.add(unlockPeriod.mul(13))
        ) {
            uint256 maxClaimable = (vestedAmount[_beneficiary].mul(8658)).div(10000);
            uint256 userTotalClaimed = totalClaimed[_beneficiary];
            if(userTotalClaimed < maxClaimable) {
                _amount = maxClaimable.sub(userTotalClaimed);
            }
            ...
            _getNow() >= start.add(unlockPeriod.mul(13)) &&
            _getNow() < start.add(unlockPeriod.mul(14))
        ) {
            uint256 maxClaimable = (vestedAmount[_beneficiary].mul(9324)).div(10000);
            uint256 userTotalClaimed = totalClaimed[_beneficiary];
            if(userTotalClaimed < maxClaimable) {
                _amount = maxClaimable.sub(userTotalClaimed);
            }
            ...
            _getNow() >= start.add(unlockPeriod.mul(14)) &&
            _getNow() < start.add(unlockPeriod.mul(15))
        ) {
            uint256 maxClaimable = (vestedAmount[_beneficiary].mul(999)).div(1000);
            uint256 userTotalClaimed = totalClaimed[_beneficiary];
            if(userTotalClaimed < maxClaimable) {
                _amount = maxClaimable.sub(userTotalClaimed);
            }
