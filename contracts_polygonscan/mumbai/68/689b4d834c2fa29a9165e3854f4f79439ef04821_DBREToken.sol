...
/**
 * @title DBRE | Digital Block Real Estate
 * @author Stobox Technologies Inc.
 * @dev DBRE ERC20 Token | This contract is opt for digital securities management.
 */
 
contract DBREToken is IERC20, Roles {
    ...
    modifier onlyWithUnfrozenFunds(address _account) {
        require(
            ...
            "DBRE: funds are frozen."
        );
        _;
    }

    /**
     * @notice DBREToken simply implements a ERC20 token.
     */
    constructor() public Roles(msg.sender) {
        _name = "Digital Block Real Estate Callable Bond Security Token";
        _symbol = "DBRE";
        _decimals = 0;
        _kDecimals = 18;
        _k = 10**_kDecimals;
        _defaultTransferLimit = 99999999999999999;
        _defaultTransactionCountLimit = 0;

        _mint(msg.sender, 10088235);
     ...
     * @param _value DBREg that enables or disables the whitelist.
     */
     ...
     * @param _value DBREg that enables or disables the transactions limit.
     */
    ...
    function burn(address _account, uint256 _amount) external onlySuperAdmin {
        require(balanceOf(_account) >= _amount, "DBRE: balance too low");
    ...
    function transferFunds(
        ...
        require(balanceOf(_from) >= _amount, "DBRE: not enough tokens");
    ...
    function splitOrMerge(uint256 _x, uint256 _y) external onlySuperAdmin {
        require(_x != _y, "DBRE: _x must not be equal to _y");
        require(_x > 0, "DBRE: _x must be greater than 0");
        require(_y > 0, "DBRE: _y must be greater than 0");
    ...
    */
        ...
        returns (bool)
    {
        ...
        _approve(
            ...
            _allowances[_sender][msg.sender].sub(
                ...
                "DBRE: transfer amount exceeds allowance"
            )
        );
        ...
        returns (bool)
    {
        _approve(
            ...
            _allowances[msg.sender][_spender].sub(
                ...
                "DBRE: decreased allowance below zero"
            )
        );
    ...
    function _transfer(
        ...
        require(_sender != address(0), "DBRE: transfer from the zero address");
        require(_recipient != address(0), "DBRE: transfer to the zero address");

        _beforeTokenTransfer(_sender, _recipient, _amount);

        _balances[_sender] = _balances[_sender].sub(
            _amount,
            "DBRE: transfer amount exceeds balance"
        );
    ...
    function _mint(address _account, uint256 _amount) internal virtual {
        require(_account != address(0), "DBRE: mint to the zero address");
    ...
    function _burn(address _account, uint256 _amount) internal virtual {
        require(_account != address(0), "DBRE: burn from the zero address");

        _beforeTokenTransfer(_account, address(0), _amount);

        _balances[_account] = _balances[_account].sub(
            _amount,
            "DBRE: burn amount exceeds balance"
        );
    ...
    function _approve(
        ...
        require(_owner != address(0), "DBRE: approve from the zero address");
        require(_spender != address(0), "DBRE: approve to the zero address");
    ...
    function _updateTransferLimit(address _account, uint256 _amount) private {
        ...
        _transferLimits[_account].allowedToTransfer = _transferLimits[_account]
            .allowedToTransfer
            .sub(_amount, "DBRE: transfer exceeds your transfer limit");
    }
    ...
    function _updateTransactionCountLimit(address _account) private {
        if (_isEnabledTransactionCount) {
            ...
            _transactionCountLimits[_account]
                ...
                .sub(1, "DBRE: transfer exceeds your transaction count limit");
        }
    }
}