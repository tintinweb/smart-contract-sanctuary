...
contract PharoReservePool is AccessControl, ReentrancyGuard, Ownable {
    ...
    PharoInfo[] public pharoInfo;

    // Reserve Pool Info
    struct PoolInfo {
        address asset; // the event creator can select what token to use for their Pharo
        uint256 pharoId; // The Pharo NFT id this pool is backing with its capital/balance
        uint256 balance;
    }
    PoolInfo[] public poolInfo;

    // pharo id => pool id => pool info
    ...
    (
        ...
        ReentrancyGuard()
    {
        pharoToken = IPHROToken(pharoTokenAddress);
    }
        ...
        public
    {
        ...
        PoolInfo storage pool = pharoIdToPoolInfo[pharoId][poolId];
        pool.balance += amount;
        pool.pharoId = pharoId;
        pool.asset = asset;

        // todo: tracking the provider balance for PHRO rewards and % of the service fees they will earn
        // PharoInfo storage pharo = pharoInfo[poolId];
