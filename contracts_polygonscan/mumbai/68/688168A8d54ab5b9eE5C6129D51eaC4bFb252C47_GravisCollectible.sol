...
// File contracts/GravisCollectionMintable.sol
...
// File contracts/GravisCollectible.sol
 ...
 */
contract GravisCollectible is GravisCollectionMintable, Whitelist, ERC165, ERC721 {
    using SafeMath for uint256;

    uint256 public constant TYPE_CAPACITY = 1000000000;
     ...
     */
    constructor() public ERC721("Gravis Cards Collection", "GRVSNFT") {
    }
    ...
    function tokenURI(uint256 _tokenId) public view override returns (string memory) {
        return string(abi.encodePacked(baseURI(), typeData[getTokenType(_tokenId)].uri));
    }
     ...
     */
    function getTokenType(uint256 _tokenId) public pure returns (uint256) {
        return _tokenId / TYPE_CAPACITY;
    }
    ...
    function createNewTokenType(
        ...
        require(_maxTotal <= TYPE_CAPACITY, "GravisCollectible: increase TYPE_CAPACITY");
    ...
    function mint(
        ...
        return _mintInternal(_to, _type, _amount);
    }

    function mintFor(address[] calldata _to, uint256[] calldata _amount, uint256 _type) external onlyMinter {
        ...
        for (uint256 i = 0; i < _to.length; ++i) {
            _mintInternal(_to[i], _type, _amount[i]);
        }
    }
    ...
    function _mintInternal(
        ...
        TypeData storage currentType = typeData[_type];
        uint256 id = TYPE_CAPACITY * _type + currentType.totalSupply;
        uint256 maxId = id + _amount;
        currentType.totalSupply = currentType.totalSupply.add(_amount);
        require(currentType.totalSupply <= typeData[_type].maxSupply, "GravisCollectible: max supply reached");
        while (id < maxId) {
            _mint(_to, id++);
        }
    ...
    /**
     * @dev Burns `_tokenId`. See {ERC71-_burn}.
     */
    function burn(uint256 _tokenId) public {
        require(_isApprovedOrOwner(_msgSender(), _tokenId), "GravisCollectible: caller is not owner nor approved");
        _burn(_tokenId);
    }

    /**
     * @dev Burn `_amount` tokens ot type `_type` for account `_from`.
     */
    function burnFor(
        address _from,
        uint256 _type,
        ...
        require(_msgSender() == _from || ERC721.isApprovedForAll(_from, _msgSender()), "GravisCollectible: caller is not owner nor approved");
        require(_type < typeData.length, "GravisCollectible: type not exist");

        uint256 index = 0;
        uint256 maxIndex = balanceOf(_from);
        uint256 done = 0;
        while (index < maxIndex) {
            uint256 id = tokenOfOwnerByIndex(_from, index);
            if (id / TYPE_CAPACITY == _type) {
                _burn(id);
                if (++done == _amount) return;
                --maxIndex;
            } else {
                ++index;
            }
        }
        revert("GravisCollectible: not enough tokens to burn");
    }
    ...
    function transferFor(
        ...
        revert("GravisCollectible: not enough tokens to transfer");
    }
}