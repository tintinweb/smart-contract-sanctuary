...
// Bitwise operations
    ...
    uint256 internal constant POSITION_BITS = 246; //8
    uint256 internal constant PROP_VALUE_BITS = 238; //8
...
constructor(
        ...
        metadataGeneratorContract = IGBotMetadataGenerator(metadataRepository_);
...
function upgradeGBot(
    uint256[] memory propertyPositions,
    ...
    uint256 tokenId) 
     public virtual {
        ...
        // Calculate cost
        uint256 cost = calculateCostforUpgrade(propertyPositions, propertyValues, metadata);
        ...
        // Upgrade properties
        uint256 updatedMetadata = upgradeMetadata(metadata, propertyPositions, propertyValues);
        gBotContract.upgradeGBot(updatedMetadata, tokenId);
...
function getTokenPosition(uint[] memory tokenIds, uint256 tokenId) private pure returns (uint256){
 ...
 function getMetadataForProperty(uint256 metadataId, uint256 position) internal pure returns (uint256) {
    ...
    function upgradeMetadata(uint256 metadata, uint256[] memory propertyPositions, uint256[] memory propertyValues) internal pure returns (uint256 newMetadata){
       for (uint256 i = 0; i < propertyPositions.length; i++) {
         uint bits = 8;
         uint256 position = propertyPositions[i];
         uint256 propertyValue = propertyValues[i];    
         if (position == VISUALS_BITS) {
              bits = 16;
          }
     
         require(0 < bits && position < 256 && position + bits <= 256);
         metadata |= (propertyValue << position);
       }
       return metadata;
   }

   function calculateCostforUpgrade(
    uint256[] memory propertyPositions,
    uint256[] memory propertyValues,
    uint256 metadata) public pure returns (uint256) {
    uint256 cost = 0;
    for (uint256 i = 0; i < propertyPositions.length; i++) {
        uint256 previousValue = getMetadataForProperty(metadata, propertyPositions[i]);
        uint256 currentValue = propertyValues[i];
        cost += (currentValue - previousValue) * 5;
    }
    return cost;
    }
}