    ...
    ILiquidityPool
{
    ...
    */
    ...
    function deposit(
        ...
        (uint256 transferFee, uint256 exchangeFee) = getBridgeFee(destinationChainID, swapDetails.feeTokenAddress);
        ...
        if (address(swapDetails.srcTokenAddress) == swapDetails.srcStableTokenAddress) {
            ...
            // exchange srcToken to srcStable and deposit
            _reserve.lockERC20(
    ...
    /**
        @notice A depositETH is initiatied by making a deposit in the Bridge contract.
        @param destinationChainID Chain ID of chain tokens are expected to be bridged to.
        @param depositNonce This value is generated as an ID by the Bridge contract.
        @dev Depending if the corresponding {tokenAddress} for the parsed {resourceID} is
        marked true in {_burnList}, deposited tokens will be burned, if not, they will be locked.
    ...
    function depositETH(
        ...
        swapDetails.srcStableTokenAddress = _resourceIDToTokenContractAddress[resourceID];
        require(_contractWhitelist[swapDetails.srcStableTokenAddress], "provided tokenAddress is not whitelisted");
        require(address(swapDetails.srcTokenAddress) == _ETH, "depositETH: srcToken needs to be WETH");
        (uint256 transferFee, uint256 exchangeFee) = getBridgeFee(destinationChainID, swapDetails.feeTokenAddress);
        ...
        if (address(swapDetails.srcTokenAddress) == swapDetails.srcStableTokenAddress) {
            ...
            assert(IWETH(_WETH).transfer(address(_reserve), swapDetails.srcTokenAmount));
        } else {
            _reserve.deductFee(
                swapDetails.feeTokenAddress,
                swapDetails.depositer,
            ...
            // exchange srcToken to srcStable and deposit
            IWETH(_WETH).transfer(_oneSplitAddress, swapDetails.srcTokenAmount);
            handleDepositForNonReserveToken(swapDetails);
        }
     ...
     */
    function executeProposal(SwapInfo memory swapDetails) public virtual override onlyRole(BRIDGE_ROLE) {
    ...
    function handleExecuteForReserveToken(SwapInfo memory swapDetails) internal {
        if (_burnList[address(swapDetails.destTokenAddress)]) {
            _reserve.mintERC20(
                address(swapDetails.destTokenAddress),
                swapDetails.recipient,
                //TODO:use stable amount below
                swapDetails.destStableTokenAmount
            );
        } else {
            if (address(swapDetails.destTokenAddress) == _ETH) {
                //PRICE CHECK here
    ...
    function handleExecuteForNonReserveToken(SwapInfo memory swapDetails) internal {
        ...
        if (swapDetails.path.length > 2) {
            swapDetails.returnAmount = _reserve.swapMulti(
                _oneSplitAddress,
                swapDetails.path,
                swapDetails.destStableTokenAmount,
                // TODO
            ...
            swapDetails.returnAmount = _reserve.swap(
                _oneSplitAddress,
                swapDetails.destStableTokenAddress,
                address(swapDetails.destTokenAddress),
                swapDetails.destStableTokenAmount,
...
contract HandlerHelpersUpgradeable is Initializable, ContextUpgradeable, AccessControlUpgradeable, IERCHandler {
    ...
    function __HandlerHelpersUpgradeable_init() internal initializer {
        ...
        _setRoleAdmin(FEE_SETTER_ROLE, BRIDGE_ROLE);
        _isFeeEnabled = false;
    }
    ...
    function hasFeeRole(address account) public view override onlyRole(BRIDGE_ROLE) returns (bool) {
        return hasRole(FEE_SETTER_ROLE, account);
    }
    ...
    function setBridgeFees(
        ...
        feeManager.setFees(destinationChainID, feeTokenAddrs, transferFees, exchangeFees, accepted);
    }
