...
contract BDay is Ownable {
    ...
    event Log(address _from, address to);
    ...
    function addGift(uint _id, string memory _pass, uint _am, address _claimer, string memory _data) public onlyOwner  {
        
        gifts.push(Gift(_id, _am, _pass, _claimer, _data));

    }
    
    function getGift(uint _id) public view onlyOwner returns(Gift memory){
    ...
    function claimGift(string memory _pass)  public view returns(string memory) {
        for(uint i=0; i < gifts.length; i++) {
