...
contract teziDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("tezi_Dividend_Tracker", "tezi_Dividend_Tracker") {
        claimWait = 86400;
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "tezi_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "tezi_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main tezi contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "tezi_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "tezi_Dividend_Tracker: Cannot update claimWait to same value");
...
contract tazi is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    teziDividendTracker public dividendTracker;

    uint256 public maxSellTransactionAmount = 1000000000000 * (10**9);
 ...
 * 
    ...
    address public marketingWallet = 0x5eEeE8c264cF7Fc46478c60b5ED5b16Ab4C5e431 ;
 */

    constructor() ERC20("tazi", "tazi") {
        MATICRewardsFee = 0;
        liquidityFee = 1;
        extraFeeOnSell = 0; // extra fee on sell
        marketingFee = 2;
        marketingWallet = payable(0x5eEeE8c264cF7Fc46478c60b5ED5b16Ab4C5e431);

        totalFees = MATICRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new teziDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "tezi: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "tezi: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "tezi: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "tezi: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "tezi: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "tezi: Cannot update gasForProcessing to same value");
