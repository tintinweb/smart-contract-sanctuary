...
contract Tournaments is Ownable,Helper, ERC721URIStorage {
    /// @notice Contract managing Banger NFT tokens
    constructor() ERC721("Banger Smart tournaments", "BST") {
    ...
    struct Tournament{
        ...
        uint numberOfTeams;
        //Team[] teams;
        uint status; //0 announced 1 registration 2 pendingstart 3 started 4 finished
    ...
    function createTournament(uint _pricePerPlayer, uint _playersPerTeam, uint _numberOfTeams) public onlyOwner returns (uint){
            tournamentId++;
            //Team[] memory tempTeams;
            Tournament memory t = Tournament(tournamentId,msg.sender,_pricePerPlayer,_playersPerTeam,_numberOfTeams,0);
            tournaments.push(t);
            return tournamentId;
