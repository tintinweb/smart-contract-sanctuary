...
contract AsavaClaimFactory is Ownable {
    ...
    ClaimIDOPoolInfo[] public claimPoolList;
    ...
    function createClaimerForPool(uint256 _idoID, address _idoPool,address _token, uint[] memory times, uint[] memory percents)
         external onlyOwner returns (uint256,address) {
        
        bool isIDOCintract =AsvaInfo.getIDOAddress(_idoPool);

        require(_idoPool != address(0), "IDO Address cannot be a zero address");
        require(isIDOCintract== true, "Address should be added in Pool info befor creating claim");

        IERC20 tokenIDO = IERC20(_token);
        ITierIDOPool idoPool= ITierIDOPool(_idoPool);

        Claimer _claimer = new Claimer(_idoID,
                                       _idoPool,
                                        _token, 
                                        times, 
                                        percents);
        claimPoolList.push(ClaimIDOPoolInfo(address(_idoPool), _idoPool, _token));
        uint256 claimContractId = AsvaInfo.addClaimAddress(address(_claimer), _idoID);
        uint256 totalAmount = idoPool.getTotalAmount();

        tokenIDO.transferFrom(msg.sender, address(_claimer),totalAmount);


        _claimer.transferOwnership(owner());
        ...
        return (claimContractId,address(_claimer));
    }
}
