...
contract DropKit is IDropKit, Initializable, OwnableUpgradeable {
    ...
    function createDropKitCollection(
        ...
        DropKitCollection dropKit = new DropKitCollection();
        dropKit.initialize(name, symbol, _msgSender());
        dropKit.transferOwnership(_msgSender());
        _collections.add(address(dropKit));

        if (affiliate != address(0) && _partners[affiliate].isValue) {
            _referrals[address(dropKit)] = affiliate;
        }

        emit CollectionCreated(address(dropKit));
    }
    ...
    OwnableUpgradeable
{
    ...
    function startSale(
        ...
        require(presale ? _merkleRoot != "" : true, "Invalid presale");
        _saleActive = true;
    ...
    function auctionPrice() public view returns (uint256) {
        if ((block.timestamp - _startedAt) >= _duration) {
            ...
            return
                ((_duration - (block.timestamp - _startedAt)) * _startPrice) /
                _duration;
        }
    }
