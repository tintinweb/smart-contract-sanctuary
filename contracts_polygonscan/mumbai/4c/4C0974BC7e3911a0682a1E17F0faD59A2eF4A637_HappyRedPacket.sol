...
contract HappyRedPacket is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    function create_red_packet (uint _number, bool _ifrandom, uint _duration, string memory _message,
        string memory _name, uint _token_type, address _token_addr, uint _total_tokens)
    external payable {
        nonce ++;
        ...
        }
        {
            ...
            uint token_type = _token_type;
            emit CreationSuccess(received_amount, _id, _name, _message, msg.sender, block.timestamp, _token_addr, number, ifrandom, duration, token_type);
        }
    }

    // It takes the signed msg.sender message as verification passcode
    function claim(bytes32 id, uint8 _v, bytes32 _r, bytes32 _s) external returns (uint claimed) {
        ...
        require(_verify(id, _v, _r, _s), "Verification failed");
        ...
        if (ifrandom == 1) {
            if (total_number - claimed_number == 1)
                ...
                claimed_tokens = random(seed, nonce) % SafeMathUpgradeable.div(SafeMathUpgradeable.mul(remaining_tokens, 2), total_number - claimed_number);
            if (claimed_tokens == 0)
                claimed_tokens = 1;
        } else {
            if (total_number - claimed_number == 1)
                ...
                claimed_tokens = SafeMathUpgradeable.div(remaining_tokens, (total_number - claimed_number));
        }
        ...
        if (token_type == 0)
            ...
            transfer_token(address(uint160(unbox(packed.packed2, 64, 160))), msg.sender, claimed_tokens);
        // Claim success event
        emit ClaimSuccess(id, msg.sender, claimed_tokens, address(uint160(unbox(packed.packed2, 64, 160))));
        return claimed_tokens;
    }

    // as a workaround for "CompilerError: Stack too deep, try removing local variables"
    function _verify(bytes32 _id, uint8 _v, bytes32 _r, bytes32 _s) private view returns (bool verified) {
        bytes32 prefixedHash = keccak256(abi.encode(_id, msg.sender));
        address calculated_public_key = ecrecover(prefixedHash, _v, _r, _s);

        return (calculated_public_key == signer);
    }
    ...
    function updateSinger(address _singer) external onlyOwner {
        require(_singer != address(0), "Invalid _singer");
        require(_singer != signer, "Duplicate _singer");
        signer = _singer;
    }
    ...
    **/
    ...
    function initialize() external initializer{
        ...
        signer = 0x861007B1DFd72744A579970a5F7F93E3ecbfBaeD;
        seed = keccak256(abi.encodePacked(block.timestamp, msg.sender));
    }
