...
contract GFCBlackSmith is Ownable, Whitelist, VRFConsumerBase {
        ...
        ) 
    {
        ...
        genesisWeapon = GFCGenesisWeapon(address(0x0));
        mysteryItem = GFCMysteryItem(address(0x0));
    ...
    function checkIngedients(uint256 category, uint256 tier, uint256[] calldata amounts) internal view{
        ...
        require(amounts.length == weaponForgeRate[tier], "You are try to forge more/less than the required amount of weapons");
    ...
    function mintNextTier(uint256 category, uint256 tier) internal returns (uint256){
        ...
        if(category == 1){
            ...
            weaponId = 1*10000 + (tier+1)*1000 + weaponType; 
        }else{
            uint256 weaponType = randNum % rangedWeaponCount[tier];
            weaponId = 1*20000 + (tier+1)*1000 + weaponType; 
        }
