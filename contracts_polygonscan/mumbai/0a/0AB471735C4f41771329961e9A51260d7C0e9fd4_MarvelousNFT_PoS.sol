...
contract MarvelousNFT_PoS is ERC20Burnable, ERC20Pausable, AccessControl, AccessControlMixin, ContextMixin, IChildToken, NativeMetaTransaction, AccessRights  {
    ...
    function getValuePerStakedToken() external view returns (uint256 valuePerStakedToken) {
        require(hasRole(CREATOR_ROLE, _msgSender()), "MarvelousNFT: must have creator role to mint");
        return _valuePerStakedToken;
    }

    function updateBurnRate(uint256 _newRate) external {
        require(hasRole(CREATOR_ROLE, _msgSender()), "MarvelousNFT: must have creator role to mint");
    ...
    function mintTokens(address account, uint256 amount) external {
        require(hasRole(CREATOR_ROLE, _msgSender()), "MarvelousNFT: must have creator role to mint");
        require(hasRole(MINTER_ROLE, _msgSender()), "MarvelousNFT: must have minter role to mint");

        _mint(account, amount);
        emit MintTokens(_msgSender(), account, amount);
    }

    function updateTotalStakes(uint256 stake) external {
