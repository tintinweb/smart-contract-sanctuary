...
contract emidelivery is ReentrancyGuardUpgradeable, OwnableUpgradeable, OracleSign, timeHelper {
    ...
    */
    ...
    function getDatesStarts() public view returns (uint256 todayStart, uint256 tomorrowStart) {
        return (YMDToTimestamp(timestampToYMD(localTime())), YMDToTimestamp(timestampToYMD(localTime())) + 24 * 60 * 60);
    }
    ...
    {
        (remainderTotal, remainderPreparedForClaim, requestIds, veryFirstRequestDate) = getRemainderOfRequestsbyWallet(msg.sender);
    }
     ...
     * @return requestIds - request ids for the sender wallet
     */
    function getAvailableToClaimbyWallet(address wallet) public view returns (uint256 available, uint256[] memory requestIds) {
        uint256 count;
        for (uint256 i = 0; i < walletRequests[wallet].length; i++) {
            Request memory _req = requests[walletRequests[wallet][i]];
            if (
