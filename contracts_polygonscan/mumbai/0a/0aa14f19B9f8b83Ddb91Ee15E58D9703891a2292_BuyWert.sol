...
contract BuyWert is ReentrancyGuard {
    ...
    //   bytes memory signature
        ...
        console.logBytes(order.makerAssetData);
        require(msg.value == (order.takerAssetAmount + order.takerFee), "Please submit the asking price in order to complete the purchase");
        ...
        IERC721(tokenAddress).transferFrom(order.makerAddress, takerAddress, tokenId);
        
        (bool makerSent,) = order.makerAddress.call{value: order.takerAssetAmount}("");
        require(makerSent, "Failed to transfer to maker");
        (bool feeRecipientSent,) = order.feeRecipientAddress.call{value: order.takerFee}("");
        require(feeRecipientSent, "Failed to transfer to fee recipient");
