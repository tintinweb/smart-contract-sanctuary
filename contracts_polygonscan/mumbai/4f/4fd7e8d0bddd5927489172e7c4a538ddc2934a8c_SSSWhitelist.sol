...
/// @author Social Squid Society
contract SSSWhitelist is Ownable {
  mapping(address => bool) isWhitelisted;
  mapping(address => bool) hasParticipated;
  address[] whitelistedParticipants;
  address[] whitelistedAdds;
  ...
  function whitelist(address _address) external {
    require(whitelistCount < totalSlots, 'whitelisted count: exceeded amount');
    ...
    whitelistedParticipants.push(msg.sender);
    whitelistCount++;
  }

  /// @dev get all whitelisted SOL addresses only Owner
  function getWhitelistedAdds()
    external
    view
    onlyOwner
  ...
  /// @dev get all whitelisted ETH addresses only Owner
  ...
  function getRemaining() external view returns (uint256) {
    uint256 remainingSlots = totalSlots - whitelistCount;
