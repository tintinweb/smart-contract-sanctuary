...
contract SwapYieldAggregator is
    OwnableUpgradeable,
    PausableUpgradeable,
    ERC2771ContextUpgradeable
{
    ...
    function initialize(
        ...
        AaveContract = 0xd41aE58e803Edf4304334acCE4DC4Ec34a63C644; // aave incentives controller
    ...
    function adminInitialDeposit(uint256 _amount, uint256 totalInitialShares) public onlyOwner {
    ...
    function currentNetAssetValue() public view returns (uint256) {
        return tokenToStableRate(IERC20(Token).balanceOf(address(this)), false);
    }
    ...
    function _harvest() internal whenNotPaused {
        ...
        if (nativeBal > 0) {
            ...
            swapRewards();
            deposit(want().balanceOf(address(this)));
            emit Compound(0);
        }
    }
    ...
    function deposit(uint256 _amount) public whenNotPaused {
        ...
        uint256 userTokenDepositAmount = _swapTokens(
            stableCoin,
            Token,
            _amount,
            stableToTokenRate(_amount, true),
            address(this)
        );

        uint256 userTokenDepositAmountUSD = tokenToStableRate(
            userTokenDepositAmount,
            false
        );
 
        if (userTokenDepositAmount > 0) {
            ILendingPool(lendingPool).deposit(Token, userTokenDepositAmount, address(this), 0);
        }
        ...
        updatePricePerShare();
        emit Deposit(_msgSender(), _amount, userTokenDepositAmountUSD);
    }
 ...
 function withdraw(uint256 _shares) public whenNotPaused {
        ...
        require(d._userShare >= _shares);
        uint256 toWithdraw = stableToTokenRate(
            _shares.mul(pricePerShare),
            false
        );
        ...
        ILendingPool(lendingPool).withdraw(Token, toWithdraw, address(this));

        uint256 userUsdWithdrawAmount = _swapTokens(
            Token,
            stableCoin,
            toWithdraw,
            tokenToStableRate(toWithdraw, true),
            _msgSender()
        );
        updatePricePerShare();
        emit Withdraw(_msgSender(), userUsdWithdrawAmount);
    }
    ...
    function stableToTokenRate(uint256 _amount, bool _fee)
        public
        view
        returns (uint256 tokenValue)
    {
        if (_fee == true) {
            ...
            tokenValue = _getQuote(ethValue, wMatic, Token, true);
        } else {
            uint256 ethValue = _getQuote(_amount, stableCoin, wMatic, false);
            tokenValue = _getQuote(ethValue, wMatic, Token, false);
        }
    }

    function tokenToStableRate(uint256 _amount, bool _fee)
        public
        view
    ...
    function _removeAllowances() internal {
        ...
        IERC20(stableCoin).approve(ROUTER, 0);
    }
}
