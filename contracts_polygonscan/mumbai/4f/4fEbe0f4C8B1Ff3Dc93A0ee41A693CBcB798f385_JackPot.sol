    ...
    IGame
{
    ...
    event SetMaxMinPlayers(uint8 indexed minPlayers, uint8 indexed maxPlayers);
    event SetMaxMinBet(uint128 indexed minBet, uint128 indexed maxBet);
    ...
    function initialize(
        ...
        emit SetMaxMinPlayers(minPlayers, maxPlayers);

        //min and max bet (if applied)
        minBet = _minbet;
        maxBet = _maxbet;
        emit SetMaxMinBet(minBet, maxBet);
        ...
        whenPaused
    {
        ...
        require(
            _min <= _max && _max <= 255,
            "JackPot.sol:  Max player range has to be in range [_min,255]"
        );
        minPlayers = _min;
        maxPlayers = _max;
        emit SetMaxMinPlayers(_min, _max);
    }
