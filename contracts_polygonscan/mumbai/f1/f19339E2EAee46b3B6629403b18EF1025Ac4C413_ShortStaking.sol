...
contract ShortStaking is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        IShortToken shortToken;   // Address of short token contract.
    ...
    function add(uint256 _rootPid, IShortToken _shortToken, bool _withUpdate) public onlyOwner {
    ...
    function updatePool(uint256 _pid) public returns (PoolInfo memory pool){
        ...
        if (block.number > pool.lastRewardBlock) {
            uint256 lpSupply = pool.shortToken.balanceOf(address(this));
            if (lpSupply > 0) {
    ...
    function deposit(uint256 _pid, uint256 _amount, address _realUser) external onlyMint nonReentrant {
        ...
        pool.shortToken.mint(address(this), _amount);
