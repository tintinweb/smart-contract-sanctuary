...
contract MultiTransfer {
  ...
  function distributeTokenSingleValue(IERC20 token, address[] memory recipients, uint256 value) external {
    for (uint256 i = 0; i < recipients.length; i++) {
      token.safeTransferFrom(msg.sender, recipients[i], value);
    }
  }

  function distributeTokenMultipleleValue(IERC20 token, address[] memory recipients, uint256[] memory values) external {
    for (uint256 i = 0; i < recipients.length; i++) {
