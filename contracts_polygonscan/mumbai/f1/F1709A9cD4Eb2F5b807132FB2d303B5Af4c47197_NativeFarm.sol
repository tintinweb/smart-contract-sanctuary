...
contract NativeFarm is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 depositFeeBP; // Actual fee deposit of farm.
    }

    // Token address
    address public NATIVE = 0xa459ACdF477743E7E93dEFdd8e6E23090a678e46; //TOKEN
    ...
    // Approx start 18/9/2021 12:00PM EST
    uint256 public startBlock = 18728000;
    //uint256 public startBlock = 18728000; // https://mumbai.polygonscan.com/block/countdown/18728000
    ...
    uint256 public constant INITIAL_HARVEST_TIME = 9600;
...
constructor(
    ...
    function add(uint256 _allocPoint, IERC20 _want, bool _withUpdate, address _strat, uint256 _depositFeeBP) public onlyOwner {
        require (_depositFeeBP < 500,"depositFeeBP");
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function set(uint256 _pid, uint256 _allocPoint,bool _withUpdate, uint256 _depositFeeBP) public onlyOwner {
        require (_depositFeeBP < 500,"depositFeeBP");
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function pendingNATIVE(uint256 _pid, address _user) external view returns (uint256)
    {
        ...
        uint256 pending = user.shares.mul(accNATIVEPerShare).div(1e12).sub(user.rewardDebt);
    ...
    function updatePool(uint256 _pid) public {
        ...
        NATIVEToken(NATIVE).mint(owner(),NATIVEReward.mul(ownerNATIVEReward).div(10000));
    ...
    function deposit(uint256 _pid, uint256 _wantAmt) public nonReentrant {
        ...
        if (_wantAmt > 0) {
            ...
            if (pool.depositFeeBP > 0) {
                ...
                pool.want.safeTransfer(address(_owner), depositFee);
            }
    ...
    //updateMaxSupply
    function updateMaxSupply(uint256 _NATIVEMaxSupply) public onlyOwner {
        NATIVEMaxSupply = NATIVEMaxSupply.add(_NATIVEMaxSupply);
    }
    ...
    // blocks reamining for  finish harvestlockup
    function remainigBlocksHL() external view returns (uint256) {
        uint256 HLblock = startBlockHarvest.add(harvestTime);
        uint256 HLstatus = 0;
        if (HLblock > block.number){
          HLstatus = HLblock.sub(block.number);
        }
        return HLstatus;
    }
}