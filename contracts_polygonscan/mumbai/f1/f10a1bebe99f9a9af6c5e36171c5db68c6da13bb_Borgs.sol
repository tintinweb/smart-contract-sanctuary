...
contract Borgs is ERC721Enumerable, ERC721Holder, CommonObjects, NumberUtils{
    ...
    function _combineBorgAttributes(string[] memory borgAttributeNames) internal view returns (string[] memory hexPixals){
        ...
        for(uint256 i=0;i<borgAttributeNames.length;i++){
             ...
             for(uint256 j = 0;j<borgAttribute.colors.length;j+=3){
                ...
                uint8 colorStartPosition = uint8(borgAttribute.colors[j + 1][0]);
                uint8 colorLength = uint8(borgAttribute.colors[j + 2][0]);
                
                for(uint8 k = colorStartPosition;k<colorLength;k++){
                    hexPixals[k] = colour;
                }
            }
        }
    ...
    function toBytes(uint8 _num) public pure returns (bytes memory _ret) {
        assembly {
            _ret := mload(0x10)
            mstore(_ret, 0x20)
