...
contract Fact is ERC721Enumerable {
    ...
    mapping (uint256 => string) private _facts;

    function mint(string calldata fact) external returns (uint256) {
        require(keccak256(bytes(fact)) != keccak256(bytes("")), "no fact");
        ...
        _facts[newItemId] = fact;
        return newItemId;
    }

    function burn(uint256 tokenId) public returns (uint256) {
        // _factsは残しているので参照可能
        require(msg.sender == super.ownerOf(tokenId), "not owner");

        super._burn(tokenId);
        return tokenId;
    }

    function updateFact(uint256 tokenId, string calldata fact) external returns (uint256) {
        // 所有者は更新出来るが、生産者から離れたら更新出来なくする方が良いかも
        require(keccak256(bytes(fact)) != keccak256(bytes("")), "no fact");
        require(msg.sender == super.ownerOf(tokenId), "not owner");
    ...
    function getTokensOfOwner(address owner) external view returns (uint256[] memory) {
        ...
        if (balance == 0) {
            ...
            for (idx = 0; idx < balance; idx++) {
                result[idx] = super.tokenOfOwnerByIndex(owner, idx);
            }
        }
    ...
    function getFact(uint256 tokenId) public view returns (string memory) {
        return _facts[tokenId];
    }

    function getLastTokenIndex() public view returns (uint256) {
