...
//        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "_tokenName", "description": "How lucky we are to have something that makes saying goodbye so hard, our best frients.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
//        output = string(abi.encodePacked('data:application/json;base64,', json));
    ...
    function claim(uint256 tokenId) public nonReentrant payable {
        ...
        require(msg.value >= 1 ether, "Must more than 1 Polygon !");
