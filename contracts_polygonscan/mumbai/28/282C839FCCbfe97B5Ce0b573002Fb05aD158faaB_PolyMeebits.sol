...
contract PolyMeebits is IERC721 {
 ...
 function _calculateShares(uint value) internal view returns ( uint _feeBOneShare, uint _feeBTwoShare) {
        uint totalFeeValue = _fraction(100, 100, value); 

       

        _feeBOneShare = _fraction(4 , 10, totalFeeValue); // 40% of fee
        _feeBTwoShare = _fraction(6 , 10, totalFeeValue); // 10% of Fee
    ...
    function mint() external payable reentrancyGuard returns (uint) {
        ...
        uint feeAmount = nftPrice * ownerTrx;
