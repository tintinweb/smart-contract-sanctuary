...
contract Props is ERC1155, Ownable {
    ...
    function burn(address from, uint256 id, uint256 amount) external onlyOwner pausable {
        require(exists[id], "Invalid ID");
        _burn(from, id, amount);
    }

    function burnBatch(address from, uint256[] memory ids, uint256[] memory amounts) external onlyOwner pausable {
