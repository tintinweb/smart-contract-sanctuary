...
contract HeroSourceBridgeFromPolygon is FxBaseChildTunnel {
    ...
    event Deposited(address indexed from, address indexed to, uint256 indexed tokenId, uint256 blockNumber);
    event Withdrawing(address indexed from, address indexed to, uint256 indexed tokenId, uint256 blockNumber);
    ...
    function withdraw(address to, uint256 tokenId) external {
        // withdraw token
        require(IERC721(childToken).ownerOf(tokenId) == msg.sender);
        ISourceChild(childToken).burn(tokenId);
        emit Withdrawing(msg.sender, to, tokenId, block.number);

        // send message to root regarding token burn
        _sendMessageToRoot(abi.encode(WITHDRAW_SINGLE, abi.encode(msg.sender, to, tokenId, block.number)));
    }

    // WITHDRAW_BATCH
    function withdrawBatch(address to, uint256[] memory tokenIds) external {
        // withdraw tokens
        for (uint i=0; i<tokenIds.length; i++) {
            require(IERC721(childToken).ownerOf(tokenIds[i]) == msg.sender);
            ISourceChild(childToken).burn(tokenIds[i]);
            emit Withdrawing(msg.sender, to, tokenIds[i], block.number);
        }

        // send message to root regarding token burn
        _sendMessageToRoot(abi.encode(WITHDRAW_BATCH, abi.encode(msg.sender, to, tokenIds, block.number)));
    }
    ...
    function _syncDepositSingle(bytes memory syncData) internal {
        ...
        emit Deposited(from, to, tokenId, blockNumber);
    }
    ...
    function _syncDepositBatch(bytes memory syncData) internal {
        ...
        for (uint i=0; i<tokenIds.length; i++) {
            ...
            emit Deposited(from, to, tokenIds[i], blockNumber);
        }
    }
}
