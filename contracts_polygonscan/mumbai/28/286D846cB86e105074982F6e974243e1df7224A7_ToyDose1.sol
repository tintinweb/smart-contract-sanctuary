...
import "./support/ERC20WithOperators.sol";

contract ToyDose1 is ERC20WithOperators {

    // solhint-disable-next-line const-name-snakecase
    string private constant local_name = "TOYDOSE_1";
    // solhint-disable-next-line const-name-snakecase
    string private constant local_symbol = "TOYDOSE_1";

    constructor(address[] memory holders, uint256[] memory amounts) ERC20WithOperators(local_name, local_symbol) {
        require(holders.length == amounts.length, "TOYDOSE_1 : wrong arguments");
...
import "./WhitelistedOperators.sol";
...
abstract contract ERC20WithOperators is ERC20, WhitelistedOperators {
    ...
    function allowance(address sender, address spender) public override view returns (uint256) {
        if (isOperator(spender)) {
            ...
            return super.allowance(sender, spender);
        }
    }
    ...
    function _approve(address sender, address spender, uint256 value) internal override {
        if (isOperator(spender)) {
            // bypass the internal allowance manipulation and checks for the
            // whitelisted operator (i.e. spender). as a side-effect, the
            // 'Approval' event will not be emitted since the allowance was not
