...
// File: contracts/interfaces/IShrodinger.sol


pragma solidity ^0.8.0;

interface IShrodinger {
...
// File: contracts/interfaces/IShrodingerMetadata.sol


pragma solidity ^0.8.0;

interface IShrodingerMetadata {
...
contract Shrodinger is ERC721Enumerable, Ownable, IShrodinger, IShrodingerMetadata {
  using Strings for uint256;

  uint256 public constant PURCHASE_LIMIT = 5;
  uint256 public constant PRICE = 0 ether;
  uint256 public constant Shrodinger_GIFT = 300000;
  uint256 public constant Shrodinger_PUBLIC = 600000;
  uint256 public constant Shrodinger_MAX = Shrodinger_GIFT + Shrodinger_PUBLIC;
  ...
  function purchase(uint256 numberOfTokens) external payable override {
    ...
    require(totalSupply() < Shrodinger_MAX, 'All tokens have been minted');
     ...
     */
    require(totalPublicSupply < Shrodinger_PUBLIC, 'Purchase would exceed Shrodinger_PUBLIC');
    require(
    ...
    for (uint256 i = 0; i < numberOfTokens; i++) {
      /**
       * @dev Since they can get here while exceeding the Shrodinger_MAX,
       * we have to make sure to not mint any additional tokens.
       */
      if (totalPublicSupply < Shrodinger_PUBLIC) {
        /**
         * @dev Public token numbering starts after Shrodinger_GIFT.
         * And we don't want our tokens to start at 0 but at 1.
         */
        uint256 tokenId = Shrodinger_GIFT + totalPublicSupply + 1;
  ...
  function purchaseWhitelist(uint256 numberOfTokens) external payable override {
    ...
    require(
      totalPublicSupply + numberOfTokens <= Shrodinger_PUBLIC,
      'Purchase would exceed Shrodinger_PUBLIC'
    );
    require(
  ...
  function gift(address[] calldata to) external override onlyOwner {
    require(totalSupply() < Shrodinger_MAX, 'All tokens have been minted');
    require(
      totalGiftSupply + to.length <= Shrodinger_GIFT,
