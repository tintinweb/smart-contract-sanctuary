...
contract Foundry is ShareWrapper, ReentrancyGuard, Ownable {
    ...
    constructor(
        ...
        FoundrySnapshot memory genesisSnapshot = FoundrySnapshot({
            time: block.number,
            rewardReceived: 0,
            rewardPerShare: 0
        });
    ...
    function getLatestSnapshot()
        internal
        view
        returns (FoundrySnapshot memory)
    {
        return foundryHistory[latestSnapshotIndex()];
    }

    function getLastSnapshotIndexOf(address blacksmith)
        public
        view
        returns (uint256)
    {
    ...
    function getLastSnapshotOf(address blacksmith)
        internal
        view
        returns (FoundrySnapshot memory)
    {
    ...
    function canWithdraw(address blacksmith) external view returns (bool) {
        return
            blacksmiths[blacksmith].epochTimerStart + withdrawLockupEpochs <=
            ITreasury(treasury).epoch();
    }
    ...
    function earned(address blacksmith) public view returns (uint256) {
        ...
        return
            (balanceOf(blacksmith) * (latestRPS - storedRPS)) /
            1e18 +
            (blacksmiths[blacksmith].rewardEarned);
    }
        ...
        updateReward(msg.sender)
    {
        require(
            ...
            "Foundry: The blacksmith does not exist"
        );
        require(amount > 0, "Foundry: Cannot withdraw 0");
        require(
            blacksmiths[msg.sender].epochTimerStart + withdrawLockupEpochs <=
                ITreasury(treasury).epoch(),
            "Foundry: still in withdraw lockup"
        );
        claimReward();
    ...
    function claimReward() public updateReward(msg.sender) {
        ...
        if (reward > 0) {
            blacksmiths[msg.sender].epochTimerStart = ITreasury(treasury)
                .epoch(); // reset timer
        ...
        onlyTreasury
    {
        ...
        require(
            totalSupply() > 0,
            "Foundry: Cannot allocate when totalSupply is 0"
        );
