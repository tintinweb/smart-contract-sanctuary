...
contract FullyOnChain_Test is ERC721Enumerable, ReentrancyGuard, Ownable {
  ...
  function genSVG(Kanji memory kanji,uint256 tokenId) internal view returns (string memory) {
    ...
    output = string(abi.encodePacked(output,zb[0],cha[tokenId],zb[1],meanx[tokenId],zb[2]));
    return string(abi.encodePacked(output,bg[kanji.bg],zb[3],zb[4],b,zb[5]));
  }

  function tokenURI(uint256 tokenId) override public view returns (string memory) {
    Kanji memory kanji = randomOne(tokenId);
    string memory title = string(abi.encodePacked(tokenId.toString()," ",meanx[tokenId]));
    return string(abi.encodePacked(rl4,Base64.encode(bytes(string(abi.encodePacked(rl1,title,getTraits(kanji),Base64.encode(bytes(genSVG(kanji,tokenId))),rl3))))));
  }
