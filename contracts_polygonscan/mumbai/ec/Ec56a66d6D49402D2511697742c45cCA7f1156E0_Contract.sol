...
contract Contract is ERC721Tradable {

    using SafeMath for uint256;
    address constant WALLET1 = 0xffe5CBCDdF2bd1b4Dc3c00455d4cdCcf20F77587; // WenMInt wallet
    address constant WALLET2 = 0xeb38B1e8eCb39759704B96B88618Ba54cA74d64f;
    address constant WALLET3 = 0xeb38B1e8eCb39759704B96B88618Ba54cA74d64f;
    uint256 constant SHARE1 = 10; // standard WenMint share
    uint256 constant SHARE2 = 80;
    uint256 constant SHARE3 = 10;
    uint256 public maxPerWallet = 5; // e.g. 5 per wallet in pre-sale
    uint256 public maxPerTransaction = 10; // e.g. 10 per transaction in public sale
    uint256 public preSalePrice = 80000000000000000; // e.g. 80000000000000000 for .08 ETH
    uint256 public pubSalePrice = 80000000000000000;  // e.g. 80000000000000000 for .08 ETH
    uint256 public maxSupply = 10000; // e.g. 10000
    bool public preSaleIsActive = true;
    bool public saleIsActive = false;
    string _baseTokenURI;

    constructor(address _proxyRegistryAddress) ERC721Tradable("Nathan's Test Collection", "NATHAN", _proxyRegistryAddress) {}
    ...
    function mint(uint _quantity) public payable {
        ...
        require(totalSupply().add(_quantity) <= maxSupply, "Would exceed total supply.");
        if(preSaleIsActive) {
            require(preSalePrice.mul(_quantity) <= msg.value, "ETH sent is incorrect.");
            require(_quantity <= maxPerWallet, "Exceeds per wallet pre-sale limit.");
            require(isWhitelisted(msg.sender), "Your address is not whitelisted.");
            require(whitelist[msg.sender].hasMinted.add(_quantity) <= maxPerWallet, "Exceeds per wallet pre-sale limit.");
            require(whitelist[msg.sender].hasMinted <= maxPerWallet, "Exceeds per wallet pre-sale limit.");
            whitelist[msg.sender].hasMinted = whitelist[msg.sender].hasMinted.add(_quantity);
        } else {
    ...
    function withdraw() external onlyOwner {
        ...
        uint256 wallet1Balance = balance.mul(SHARE1).div(100);
        uint256 wallet2Balance = balance.mul(SHARE2).div(100);
        uint256 wallet3Balance = balance.mul(SHARE3).div(100);
        ...
        payable(msg.sender).transfer(
            balance.sub(wallet1Balance.add(wallet2Balance).add(wallet3Balance))
        );
    }
}
