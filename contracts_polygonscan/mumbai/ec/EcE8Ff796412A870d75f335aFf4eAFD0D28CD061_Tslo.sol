...
contract Tslo {
    ...
    function fulfillRandomness(string memory _clientSeed, uint256[3] memory winLines) internal {
        ...
        if(_bet._lines >= 1)
            _credits[_bet._wallet] += winLines[1] * _bet._betAmount * 10 ** 18;
        if(_bet._lines >= 2)
            _credits[_bet._wallet] += winLines[0] * _bet._betAmount * 10 ** 18;
        if(_bet._lines >= 3)
            _credits[_bet._wallet] += winLines[2] * _bet._betAmount * 10 ** 18;
