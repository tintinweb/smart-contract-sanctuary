...
*/
contract TaskManager is IIterableFunctions, StorageAccessor, AccessLevel {
    ...
    event TaskCreated(string externalId, uint256 indexed taskId);
    ...
    function __TaskManager_init(address taskStorage) public {
    ...
    function getFunctions() override external pure returns (bytes4[] memory){
        ...
        //taskManagement
        functions[1] = this.createTask.selector;
        functions[2] = this.cancelTask.selector;
        functions[3] = this.submitTaskSolution.selector;
        functions[4] = this.acceptTaskSolution.selector;
        functions[5] = this.rejectTaskSolution.selector;
    ...
    function createTask(
        ...
        FeeManager feeManager = FeeManager(address(this));
        uint256 tax = feeManager.calculateTax(taskParams.isHyveCommission, isEth, taskParams.amount, taskParams.tokenAddress);
        if (msg.value > 0) {
            require(taskParams.tokenAddress == address(0), "TM: Token address included");
            if (taskParams.isHyveCommission) {
                feeManager.checkUserTokenBalance(_msgSender(), getConfigStorage().getHyveERC(), tax);
            ...
            if (taskParams.isHyveCommission) {
                feeManager.checkUserTokenBalance(_msgSender(), taskParams.tokenAddress, taskParams.amount);
                feeManager.checkUserTokenBalance(_msgSender(), getConfigStorage().getHyveERC(), tax);
            } else {
                feeManager.checkUserTokenBalance(_msgSender(), taskParams.tokenAddress, taskParams.amount + tax);
            }
        }
        ...
        //interactions
        if (taskParams.isHyveCommission) {
            feeManager.splitTax(_msgSender(), false, tax, IERC20Upgradeable(getConfigStorage().getHyveERC()));
        } else {
            feeManager.splitTax(_msgSender(), isEth, tax, IERC20Upgradeable(taskParams.tokenAddress));
        }

        if (isEth) {
            IEscrow(getConfigStorage().getEscrow())
            .createDepositETH{value : taskParams.amount}(taskId, TYPE, _msgSender(), taskParams.amount);
        } else {
            IEscrow(getConfigStorage().getEscrow())
            .createDeposit(address(this), taskId, TYPE, _msgSender(), taskParams.tokenAddress, taskParams.amount);
        }
...
contract ConfigStorage is IConfigStorage, AccessLevel {
    uint8 constant private T0_TAX = 0;
    uint8 constant private T1_TAX = 25;
    uint8 constant private T2_TAX = 50;
    uint8 constant private T3_TAX = 100;
    ...
    mapping(address => address[]) private tokenPaths;
    ...
    function setSwap(address swap) override external onlyRole(DEFAULT_ADMIN_ROLE) {
        _swap = swap;
    }

    function getSwap() override external view returns (address) {
        return _swap;
    }
    ...
    function getTaxLevels() override external view returns (uint8[] memory){
    ...
    function setTaxLevels(uint8[] memory levels) override external onlyRole(DEFAULT_ADMIN_ROLE) {
        revert("Operation not supported");
    }

    function setTokenExchangePath(address token, address[] calldata path) override external onlyRole(DEFAULT_ADMIN_ROLE) {
        tokenPaths[token] = path;
    }

    function getTokenExchangePath(address token) override external view returns (address[] memory) {
        return tokenPaths[token];
    }

    function setTokenExchangeRate(address token, uint rate) override external onlyRole(DEFAULT_ADMIN_ROLE) {
        tokenRates[token] = rate;
    }

    function getTokenExchangeRate(address token) override external view returns (uint256) {
...
contract FeeManager is IIterableFunctions, StorageAccessor, ERC20Sender {
    using SafeERC20Upgradeable for IERC20Upgradeable;
    using SafeMath for uint256;

    uint256 constant private BIPS_DIVISOR = 10000;
    bytes32 private constant _STORAGE_SLOT = keccak256("hyve.works.storage.fee");

    function __FeeManager_init(address owner, address configStorage) initializer public {
        __AccessLevel_init(owner);
        setStorageAddress(configStorage);
    }
    ...
    function getConfigStorage() override virtual internal view returns (IConfigStorage) {
        return IConfigStorage(getStorageAddress());
    }

    function getFunctions() override external pure returns (bytes4[] memory) {
        bytes4[] memory functions = new bytes4[](8);
        functions[0] = this.__FeeManager_init.selector;
        // fee manager
        functions[1] = this.calculateTax.selector;
        functions[2] = this.splitTax.selector;
        functions[3] = this.checkUserTokenBalance.selector;
        //access level
        functions[4] = this.grantRole.selector;
        functions[5] = this.hasRole.selector;
        functions[6] = this.revokeRole.selector;
        //erc20Sender
        functions[7] = this.sendERC20.selector;
    ...
    */
    function calculateTax(bool isHyveCommission, bool isEther, uint256 amount, address tokenAddress) external view returns (uint256) {
        uint8[] memory levels = getConfigStorage().getTaxLevels();
        if (getConfigStorage().getHyveERC() == tokenAddress) {
            require(isHyveCommission, "FM: cannot pay in hyve and not have hyve commission");
            return levels[0];
        } else {
            if (isHyveCommission) {
                return getConvertedAmount(amount, tokenAddress) * levels[1] / BIPS_DIVISOR;
            } else {
                if (isEther || getConfigStorage().isReducedTaxToken(tokenAddress)) {
                    return amount * levels[2] / BIPS_DIVISOR;
                } else {
                    return amount * levels[3] / BIPS_DIVISOR;
                }
            }
        }
    }

    function splitTax(address customer, bool isEthTax, uint256 taxValue, IERC20Upgradeable token) external {
        // check if call was made via Router
        require(_msgSender() == address(this), "not router");
    ...
    function checkUserTokenBalance(address user, address token, uint256 minimumBalance) external view {
        require(
            IERC20Upgradeable(token).balanceOf(user) >= minimumBalance &&
            IERC20Upgradeable(token).allowance(user, address(this)) >= minimumBalance,
            "FM: Too few funds");
    ...
    function getConvertedAmount(uint256 amount, address token) public view returns (uint256){
        address[] memory path = getConfigStorage().getTokenExchangePath(token);
        uint256 convertedAmount;
        if (path.length > 0) {
            IUniswapV2Router02 router = IUniswapV2Router02(getConfigStorage().getSwap());
            uint256[] memory amounts = router.getAmountsOut(amount, path);
            convertedAmount = amounts[amounts.length - 1];
        } else {
            convertedAmount = amount * getConfigStorage().getTokenExchangeRate(token);
            require(convertedAmount > 0, "FM: Could not find a proper exchange rate");
        }
...
interface IConfigStorage {
    ...
    function setSwap(address staking) external;
    ...
    function getTaxLevels() external view returns (uint8[] memory);

    function setTaxLevels(uint8[] memory levels) external;

    function setTokenExchangePath(address token, address[] calldata path) external;

    function getTokenExchangePath(address token) external view returns (address[] memory);

    function setTokenExchangeRate(address token, uint rate) external;

    function getTokenExchangeRate(address token) external view returns (uint256);
}
...
contract Escrow is IEscrow, AccessLevel, ERC20Taker {
    ...
    string constant NO_DEPOSIT = "Escrow: No deposit has been created for given task id!";
    string constant DEPOSIT_EXISTS = "Escrow: A deposit has already been created";
    ...
    event Split(uint256 amountForOwner, uint256 amountForBeneficiary, uint256 totalAmount);
    ...
    function createDeposit(address mediator, uint256 listingId, string calldata listingType, address from, address tokenAddress, uint256 amount) override external onlyRole(OPERATOR_ROLE) {
        ...
        require(!depositExistsWithId(depositId), DEPOSIT_EXISTS);
    ...
    function setBeneficiary(uint256 listingId, string calldata listingType, address beneficiary) override external onlyRole(OPERATOR_ROLE) {
        ...
        require(depositExistsWithId(depositId), NO_DEPOSIT);
        deposits[depositId].beneficiary = beneficiary;
    }

    function depositExistsWithId(uint256 depositId) private view returns (bool) {
        return deposits[depositId].initialDeposit != 0;
    }
    ...
    function createNewDeposit(
        ...
        deposits[depositId] = EscrowDeposit(from, token, amount, address(0));
    }
    ...
    function executePayment(uint256 listingId, string calldata listingType) override external onlyRole(OPERATOR_ROLE) {
        ...
        transferDeposit(depositId, deposit.beneficiary, deposit.initialDeposit);
        emit DepositWithdrawal(listingId, listingType, depositId, deposit.beneficiary, deposit.initialDeposit);
    }
    ...
    function splitDeposit(uint256 listingId, string calldata listingType, uint256 paymentPercentage) override external onlyRole(ARBITER_ROLE) {
        ...
        uint256 depositedAmount = deposits[depositId].initialDeposit;
        ...
        transferDepositNoCleanup(deposit, deposit.beneficiary, amountForApplicant);
        emit DepositWithdrawal(listingId, listingType, depositId, deposit.beneficiary, amountForApplicant);

        transferDepositNoCleanup(deposit, deposit.from, amountForOwner);
        emit DepositWithdrawal(listingId, listingType, depositId, deposit.from, amountForOwner);
    }
    ...
    function redeemDeposit(uint256 listingId, string calldata listingType) override external onlyRole(OPERATOR_ROLE) {
        ...
        transferDeposit(depositId, deposit.from, deposit.initialDeposit);
        emit DepositWithdrawal(listingId, listingType, depositId, deposit.from, deposit.initialDeposit);
    }

    function transferDeposit(uint256 depositId, address beneficiary, uint256 amount) private {
        require(amount <= deposits[depositId].initialDeposit, "Escrow: Amount too large");
        require(amount > 0, "Escrow: Amount should not be 0");

        if (deposits[depositId].tokenAddress == address(0)) {
            //@dev - please make sure deposit amount is zeroed before transferring ETH to avoid re-entrancy attack
            delete deposits[depositId];
            payable(beneficiary).call{value : amount}("");
        } else {
            address tokenAddress = deposits[depositId].tokenAddress;
            delete deposits[depositId];
    ...
    function transferDepositNoCleanup(EscrowDeposit memory deposit, address beneficiary, uint256 amount) private{
        require(amount > 0, "Escrow: Amount should not be 0");
