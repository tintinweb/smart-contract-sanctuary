...
contract XChainTrade { 
    ...
    function currentPrice() public view returns(uint256) {
        ...
        uint timeNow = 1641579609; // in epoch, put the current time (use https://www.epochconverter.com to find the current time)
    ...
    receive() external payable {
        ...
        uint allSalesStop = timeNow + 14400; // 4h after the initial time, the contract stops working
        bool keepGoing = true;
        ...
        while (incwei >= price && keepGoing) { // if there are still funds and something can be bought, stay in the loop
