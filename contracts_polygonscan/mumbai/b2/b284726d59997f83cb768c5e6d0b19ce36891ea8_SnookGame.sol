...
interface ISnookGame is IDescriptorUser {
  ...
  function getSnookPopulationCounter() view external returns(uint);
  function computePpk() view external returns (uint);
  function getKillsAndComputePpkRewards() view external returns (uint kills, uint rewardsAmount);
...
contract SnookGame is ISnookGame, AccessControlEnumerableUpgradeable, PausableUpgradeable {
    ...
    function getSnookPopulationCounter() view external override returns(uint) {
      return _spc;
    }
    ...
    function _getKillsAndComputePpkRewards() view internal returns(uint, uint) {
      uint rewards = _computePpk() * accountKills[msg.sender];
      return (accountKills[msg.sender], rewards);
    }

    function getKillsAndComputePpkRewards() view external override returns(uint, uint) {
      return _getKillsAndComputePpkRewards();
    }

    function claimPpkRewards() external override {
      (, uint rewardsAmount) = _getKillsAndComputePpkRewards();
