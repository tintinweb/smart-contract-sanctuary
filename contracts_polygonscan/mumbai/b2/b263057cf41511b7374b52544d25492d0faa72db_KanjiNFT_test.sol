...
contract KanjiNFT_test is ERC721Enumerable, ReentrancyGuard, Ownable {
  using Strings for uint256;
...
//}
  ...
  function tokenURI(uint256 tokenId) override public view returns (string memory) {
    Kanji memory kanji = randomOne(tokenId);
    string memory title = string(abi.encodePacked(rl1,tokenId.toString()," ",meanx[tokenId-1],getTraits(kanji)));
  ...
  function claim(uint256 tokenId) public nonReentrant onlyOwner{
    require(tokenId > 0 && tokenId < 201, "invalid claim");
...
//gasless
  ...
  constructor() ERC721("KanjiNFT_test", "KNJ1") Ownable() {}
}