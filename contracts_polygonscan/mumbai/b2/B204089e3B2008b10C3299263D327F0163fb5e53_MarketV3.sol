...
//	mapping(bytes32 => uint256) private orderID;
	mapping(address => mapping(uint256 => mapping(uint256 => uint256))) private orderID;
...
//		uint256 _retailFee,
		...
		_listOrderId =  new uint256[](_toVersion.sub(_fromVersion));
...
//			orderID[keccak256(abi.encodePacked(_tokenAddress, _tokenId, i))] = _orderId;
			orderID[_tokenAddress][_tokenId][i] = _orderId;
			_listOrderId[i] = _orderId;
        }

//		emit OrderCreated(_listOrderId, _fromVersion, _toVersion);
	...
	function acceptBid(uint256 _bidId, uint256 _quantity) external whenNotPaused() returns (bool) {
		Bid memory bid = bids[_bidId];
		bytes32 _id = keccak256(abi.encodePacked(bid.tokenAddress, bid.tokenId, bid.version));
		uint256 _orderId = orderID[bid.tokenAddress][bid.tokenId][bid.version];
		Order memory order = orders[_orderId];
...
//				newOrder.retailFee = retailFee;
				...
				orders[i] = newOrder;
//				bytes32 _id = keccak256(abi.encodePacked(tokenAddress, tokenId, version));
//				orderID[_id] = i;
				orderID[tokenAddress][tokenId][version] = i;
			}
		}
