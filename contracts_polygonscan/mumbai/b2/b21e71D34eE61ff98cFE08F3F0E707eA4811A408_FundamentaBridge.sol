...
contract FundamentaBridge is SecureContract
{
        ...
        returns(address, uint256, uint256, uint256, uint256)
    {
        ...
        bytes32 dataHash = prefixAndHash(transactionData);
        address signer = recover(dataHash, clientSignature);
        ...
        returns(address, uint256, uint256, uint256, uint256)
    {
        ...
        address signer = recover(prefixAndHash(transactionData), clientSignature);
        require (signer == msg.sender, "Not sent by client");
        require (signer == sender, "Not signed by client");
    ...
    function prefixAndHash(bytes memory txData) internal pure returns (bytes32)
    {
        bytes memory prefix = "\x19Ethereum Signed Message:\n108";
        return keccak256(abi.encodePacked(prefix, txData));
    }
