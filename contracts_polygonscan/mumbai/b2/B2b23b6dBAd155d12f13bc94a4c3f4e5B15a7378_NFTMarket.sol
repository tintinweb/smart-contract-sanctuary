...
contract NFTMarket is ReentrancyGuard, Ownable {
    ...
    function createMarketSale(uint256 itemId, address erc20Address, uint256 amount) public payable nonReentrant {
        ...
        uint256 priceTotal = item.price * amount;
        uint256 fee = priceTotal * _marketplaceFee / 10000;
        uint256 remainAmount = item.amount - amount;
        ...
        if (item.erc20Address == address(0)) {
            ...
            if (msg.value - priceTotal - fee > 0) {
                Address.sendValue(payable(msg.sender), msg.value - priceTotal - fee);
            }
        } else {
            IERC20 token = IERC20(item.erc20Address);
            require(token.balanceOf(msg.sender) >= priceTotal + fee, "Insufficient ERC20 balance for paying for the asset.");
