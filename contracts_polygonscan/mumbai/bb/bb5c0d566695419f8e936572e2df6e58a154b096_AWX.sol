...
interface IERC20 {
    ...
    /**
     * @dev Returns the amount of tokens owned by `account`, both locked and unlocked
     */
    function balanceOf(address account) external view returns (uint256);

    /**
     * @dev Returns the amount of tokens owned by `account`, and unlocked
     */
    function internalBalance(address account) external view returns (uint256);    

    /**
     * @dev Returns the amount of tokens owned by `account`, and locked
     */
    function internalLocked(address account) external view returns (uint256);        
    ...
    /**
     * @dev Emitted when `owner` unlocks tokens for a wallet
     */
    event Unlock(address indexed target, uint256 amount);

    /**
     * @dev Emitted when a wallet receives locked tokens.
     */
...
contract ERC20 is IERC20, Pausable {
     ...
     * 1 = blacklisted account (cannot send or receive tokens)
     */
    ...
    function _deposit(address user, bytes calldata depositData) internal whenNotPaused {
    ...
    /**
     * Mint tokens to migrate balances
     */
    function _mintTokens(address recipient, uint256 amount) internal {
        require(_migrationOpen == true, "Migration is stopped");
        _balances[recipient] = _balances[recipient].add(amount);
        _totalSupply = _totalSupply.add(amount);
    ...
    /**
     * Burn tokens to migrate balances
     */
    ...
    /**
     * Stop migration stops minting and burning of tokens
     */
    function _stopMigration() internal {
    ...
    function internalLocked(address account) public view returns (uint256) {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        require(whitelist[sender] != 1, "Sender is blacklisted");
        require(whitelist[recipient] != 1, "Recipient is blacklisted");
    ...
    /**
     * @dev Transfer tokens from balance to locked balance of another wallet.
     */
    function _transferToLocked(address sender, address recipient, uint256 amount) internal {
        _balances[sender] = _balances[sender].sub(amount);
        _locked[recipient] = _locked[recipient].add(amount);
        emit Transfer(sender, recipient, amount);
        emit Lock(recipient, amount);
    }    

    /**
     * @dev Owner can unlock wallet balance
     */
    function _unlockBalance(address target, uint256 amount) internal {
        _balances[target] = _balances[target].add(amount);
        _locked[target] = _locked[target].sub(amount);
        emit Unlock(target, amount);
    }

    /**
     * @dev Owner can lock wallet balance
     */
    function _lockBalance(address target, uint256 amount) internal {
        _balances[target] = _balances[target].sub(amount);
        _locked[target] = _locked[target].add(amount);
...
contract AWX is ERC20, Ownable {
    ...
    function burnTokens(address recipient, uint256 amount) external onlyOwner {
     ...
     */
    function setTokenDetails(string memory _name, string memory _symbol) external onlyOwner {
        name = _name;
        symbol = _symbol;
    }    

    /**
     * @dev Transfer to locked balance
     */
    function transferToLocked(address recipient, uint256 amount) external onlyOwner {
        _transferToLocked(msg.sender,recipient,amount);
    }

    /**
     * @dev Unlock balance
     */
    function unlockBalance(address target, uint256 amount) external onlyOwner {
     ...
     * @dev Lock balance
     */
    function LockBalance(address target, uint256 amount) external onlyOwner {
        _lockBalance(target, amount);
    }        
    ...
    function whitelistAddress(address account, uint8 level) external onlyOwner {
        require(level<=1, "Level: Please use level 0 to 1.");
