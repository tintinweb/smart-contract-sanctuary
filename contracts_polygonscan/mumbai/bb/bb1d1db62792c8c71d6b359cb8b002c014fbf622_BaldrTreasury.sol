...
// File: contracts/helpers/interfaces/IBALDRERC20.sol



pragma solidity ^0.8.4;

interface IBALDRERC20 {
...
// File: contracts/regular/BaldrTreasury.sol
...
contract BaldrTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SBALDR }

    address public BALDR;
    ...
    address public sBALDR;
    uint public sBALDRQueue; // Delays change to sBALDR address
    ...
    /**
        @notice allow approved address to deposit an asset for BALDR
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint BALDR needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( BALDR ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn BALDR for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IBALDRERC20( BALDR ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sBALDR ).balanceOf( msg.sender ); // Can only borrow against sBALDR held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with BALDR
        @param _amount uint
     */
    function repayDebtWithBALDR( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved as a Debtor" );

        IBALDRERC20( BALDR ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, BALDR, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( BALDR ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( BALDR ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns BALDR valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match BALDR decimals
            value_ = _amount.mul( 10 ** IERC20( BALDR ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SBALDR ) { // 9
            sBALDRQueue = block.number.add( blocksNeededForQueue );
        } else return false;
