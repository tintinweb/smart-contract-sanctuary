...
contract KingStake is Ownable{
    ...
    function viewReward(address _account,uint256 _stakingID)external view returns(uint256){
    ...
    function claimReward(address _account,uint256[] memory _stakingID)public {
        ...
        for(uint256 i = 0; i < _stakingID.length; i++){
            ...
            userDetails[msg.sender][stakingID].lastClaim = block.timestamp;
            localVariable[2] = userDetails[_account][stakingID].amount.mul(localVariable[0]).mul(localVariable[1]).div(1e12).div(86400);
            
            rewardBalance = rewardBalance.sub(localVariable[2],"KingStake :: reward Balance exceed");
            userDetails[msg.sender][stakingID].reward += localVariable[2];
            
            claimed[msg.sender] = claimed[msg.sender].add(localVariable[2]);
