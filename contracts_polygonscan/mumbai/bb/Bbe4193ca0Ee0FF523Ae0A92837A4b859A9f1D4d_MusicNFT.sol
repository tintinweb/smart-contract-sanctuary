...
/**
 * @title An NFT contract for music.com NFTs
 * @author Linum Labs
...
//TODO: see if there is a need for single-tokenId approvals

contract MusicNFT is ERC1155, ERC165Storage, Ownable, IMusicNFT {
    ...
    uint256 private rate = 5;
    uint256 private scale = 1e2;

    // inherited from IMusicNFT
    // struct Properties {
    //     address creator;
    //     bool isRedeemable;
    ...
    event RedeemDetailsSet(
        uint256 indexed tokenId,
        bool indexed redeemable,
    ...
    event RoyaltyExemptionModified(
        uint256 indexed tokenId,
        address indexed toModify,
        bool indexed isExempt
    );
    event Mint(uint256 tokenId, string db_id);

    modifier onlyCreator(uint256 tokenId) {
        require(
            _msgSender() == properties[tokenId].creator,
            "NFT: only NFT creator"
        );
        _;
    }

    constructor() ERC1155(_uri) {
        ERC165Storage._registerInterface(_INTERFACE_ID_ERC2981);
        ERC165Storage._registerInterface(_INTERFACE_ID_ERC1155);
    }
    ...
    function getNftProperties(uint256 tokenId)
        external
        view
        override
    ...
    function isExemptFromRoyalties(uint256 tokenId, address checkIfExempted)
        external
        view
        override
        returns (bool)
    ...
    function setWhitelisted(address toSet, bool canMint) external onlyOwner {
        if (canMint == whitelisted[toSet]) {
            ...
            revert(
                string(
                    abi.encodePacked("whitelist status is already ", boolString)
                )
            );
        }
    ...
    function setRoyalty(uint256 _rate, uint256 _scale)
        external
        override
        onlyOwner
    {
    ...
    function setRoyaltyExemption(
        ...
        if (isExempt == royaltyExemptions[tokenId][toModify]) {
            ...
            revert(
                string(
                    abi.encodePacked("royalty status is already ", boolString)
                )
            );
        }
    ...
    function setRedeem(
        ...
        require(
            msg.sender == owner() || msg.sender == properties[tokenId].creator,
    ...
    /// @dev if the tokenId already has a non-zero amount, the amount will be added
    /// @dev the db_id is not stored, and is for the purposes of syncing a traditional database
    /// @param account the address the NFTs will be minted to
    /// @param amount the amount of NFTs to mint
    /// @param uri_ the URI of the NFT
    /// @param db_id an id for syncing a traditional database
    function mint(
        address account,
        uint256 amount,
        string memory uri_,
    ...
    /// @dev if the tokenId already has a non-zero amount, the amount will be added
    /// @dev db_ids are not stored, and are for the purposes of syncing a traditional database
    ...
    /// @param db_ids an array of ids for syncing a traditional database
    function mintBatch(
        address to,
        uint256[] memory amounts,
        string[] memory _uris,
        string[] memory db_ids
    ) public {
        require(
            whitelisted[msg.sender] || msg.sender == owner(),
            "not authorized to mint"
        ...
        require(
            amounts.length == _uris.length && _uris.length == db_ids.length,
            "arrays must have equal length"
        );
        uint256[] memory ids = new uint256[](amounts.length);
        for (uint256 i = 0; i < amounts.length; i++) {
            _tokenId.increment();
            ids[i] = _tokenId.current();
        }
        _mintBatch(to, ids, amounts, "");
        for (uint256 i = 0; i < amounts.length; i++) {
            properties[ids[i]].creator = msg.sender;
            _setTokenUri(ids[i], _uris[i]);
            emit Mint(ids[i], db_ids[i]);
        }
    }
...
interface IMusicNFT is IERC1155, IERC2981 {
    struct Properties {
        address creator;
        bool isRedeemable;
        string redeemDescrip; // description of what you can redeem with the NFT
    ...
    function mint(address account, uint256 amount, string memory _uri, string memory db_id) external;
    function mintBatch(address to, uint256[] memory amounts, string[] memory _uris, string[] memory db_ids) external;
}
