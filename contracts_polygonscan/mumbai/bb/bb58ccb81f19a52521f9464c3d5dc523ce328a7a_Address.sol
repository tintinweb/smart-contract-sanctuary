...
abstract contract  DaiTradingVault  {
  function depositDai(
    uint256 amount
    )
  external virtual;
  function claimDai() external virtual;
  function harvest() external virtual ;
  function withdrawDai( 
    uint256 amount
    ) external virtual;
   
     mapping(address => UserManagement.User) public users;
     mapping(address => uint) public  daiToClaim;
}

contract StrategyGainsDai is Ownable, ReentrancyGuard, Pausable  {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    address public constant daiAddress = address(0x8f3Cf7ad23Cd3CaDbD9735AFf958023239c6A063);
    address public constant wantAddress = address(0x8f3Cf7ad23Cd3CaDbD9735AFf958023239c6A063);
    address public constant wmaticAddress = address(0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270);
    address public constant polycatRouterAddress = 0x94930a328162957FF1dd48900aF67B5439336cBD;
    ...
    address public gainsDaiVaultAddress;
    ...
    address[] public daiToWmaticPath = [daiAddress, wmaticAddress];
    address[] public wmaticToDaiPath = [wmaticAddress,daiAddress];
    address[] public daiToUsdcPath = [daiAddress,usdcAddress];
    address[] public daiToFishPath = [daiAddress,usdcAddress,fishAddress];

    DaiTradingVault dtv;
 constructor(DaiTradingVault _dtv) public { dtv = _dtv; }
    ...
    function _vaultDeposit(uint256 _amount) internal
    {
      DaiTradingVault(gainsDaiVaultAddress).depositDai(_amount);
    }
    function _vaultWithdraw(uint256 _amount) internal {
      DaiTradingVault(gainsDaiVaultAddress).withdrawDai(_amount);
    }
    function earn() external nonReentrant whenNotPaused  {
      //Harvest DAI and MATIC
      DaiTradingVault(gainsDaiVaultAddress).harvest();
      uint256 maticAmt = address(this).balance;
      //If there is extra Matic rewards swap to DAI
      if(maticAmt > 0) {
        _safeSwapMaticToToken(maticAmt, wmaticToDaiPath, address(this));
      }

      uint256 earnedAmt = IERC20(daiAddress).balanceOf(address(this));
      earnedAmt = distributeFees(earnedAmt);
      earnedAmt = distributeRewards(earnedAmt);
      earnedAmt = buyBack(earnedAmt);
      DaiTradingVault(gainsDaiVaultAddress).depositDai(earnedAmt);
    }
    function vaultSharesTotal() public view returns (uint256)
    {
        address id = address(this);
        (uint256 daiTotal,,,,) = DaiTradingVault(gainsDaiVaultAddress).users(id);
      return daiTotal;
    }
    ...
    function wantLockedTotal() public view returns (uint256)
    {
        address id = address(this);
        (uint256 daiTotal,,,,) = DaiTradingVault(gainsDaiVaultAddress).users(id);
      return IERC20(daiAddress).balanceOf(address(this)).add(daiTotal);
    }
    function _resetAllowances() internal
    {
      IERC20(daiAddress).safeApprove(gainsDaiVaultAddress, uint256(0));
      IERC20(daiAddress).safeIncreaseAllowance(
          gainsDaiVaultAddress,
          uint256(-1)
      );
    ...
    function _emergencyVaultWithdraw() internal {
    DaiTradingVault(gainsDaiVaultAddress).claimDai();
    }
    ...
    function _safeSwapMaticToToken(
        uint256 _amountIn,
        address[] memory _path,
        address _to
    ) internal {
