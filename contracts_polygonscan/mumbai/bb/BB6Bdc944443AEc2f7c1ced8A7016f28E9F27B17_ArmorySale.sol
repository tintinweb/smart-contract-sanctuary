    ...
    PausableUpgradeable
{
    ...
    function initialize(
        ...
        bundleList[2] = Bundle({count:10,price:50});
    ...
    {
        require(characters.balanceOf(msg.sender) > 0, "Please buy an operator to mint your first free weapon");
        uint256 baseSeed = randoms.getRandomSeed(msg.sender);

        // first weapon free
        if(weapons.balanceOf(msg.sender) == 0) {
            weapons.mint(msg.sender, baseSeed);
            emit weaponMinted(msg.sender, 1);
        } else {
            ...
            for(uint8 i=0; i<b.count; i++) {
                ...
                weapons.mint(msg.sender, indexSeed);
            }
...
contract Weapons is Initializable, ERC721Upgradeable, AccessControlUpgradeable {
    ...
    function mint(address minter, uint256 seed) public restricted returns (uint256) {
        ...
        uint256 qualityRoll = RandomUtil.randomSeededMinMax(1, 10000, seed);
