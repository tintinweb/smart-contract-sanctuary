...
import "./libs/DeliriumToolBox.sol";
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    DeliriumToolBox public deliriumToolBox;
    uint256 public deliriumReleaseView;
    ...
    constructor(
        ...
        deliriumReleaseGradient = _deliriumToolBox.calcEmissionGradient(
            useBlock, _beginningDeliriumEmission, endDeliriumGradientBlock, endGoalDeliriumEmission);
    }
    ...
    function pendingDelirium(uint256 _pid, address _user) external returns (uint256) {
    ...
    function skimPool(uint256 poolId) internal {
        ...
        if (skim > 1e4) {
            pool.lpToken.safeTransfer(address(delirium), skim);
        }
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        deliriumReleaseView = deliriumRelease;
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (!autoUpdateDeliriumGradient())
            updatePool(_pid);
        payOrLockupPendingDelirium(_pid);
        if (_amount > 0) {
    ...
    // Pay pending DELIRIUMs
    function payOrLockupPendingDelirium(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
...
contract DeliriumToolBox is Ownable {

    uint256 public immutable startBlock;
    uint256 public emission1;
    uint256 public emission2;
    ...
    function getDeliriumRelease(bool isIncreasingGradient, uint256 releaseGradient, uint256 gradientEndBlock, uint256 endEmission, uint256 _from, uint256 _to) external returns (uint256) {
        ...
        if (!isFlatEmission(gradientEndBlock, _to)) {
            ...
            emission1 = baseEmission;
            emission2 = totalWidth * baseEmission;
