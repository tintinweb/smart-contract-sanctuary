...
contract Marketplace is ERC1155Holder, AccessControl, ReentrancyGuard {
    ...
    IERC20 trips;
    IERC20 currency; // currency to be used for the marketplace
    ...
    uint256 public adminMatic; //Total Matic available for withdrawal
    uint256 public feeBasisPoint = 100; //4 decimals, applies to listings. Fees collected are held in contract
    uint256 public adminTrips; //Total trips available for withdrawal
     ...
     *
     * @param price: listing price per day
     * @param propertyId: id of the property being listed (from the propertyVerificationContract)
     */
    function createListing(uint256 price, uint256 propertyId) public {
        // price of the listing must be more than 0 per day
        require(price != 0, 'Price cannot be 0');
        // property must be public to list
        require(
            propertyVerificationContract.isPropertyPublic(propertyId),
            'The property with this id is not public'
        );
        ...
        // mapping(uint256 => ListingRequest) storage test;
    ...
    function requestListing(
        ...
        // require that the listing exists and the sender is not the owner
        require(
        ...
        uint256 stayTime = (checkOutTime - checkInTime) / paymentInterval;

        // total stay * price per interval
        uint256 totalPrice = stayTime * listing.price;
        // always round up if there is a remainder
        if ((stayTime % listing.price) > 0) {
            totalPrice += 1;
        }

        require(
            currency.balanceOf(msg.sender) >= listing.price,
            'User does not have enough of the currency'
        );
        // Renter deposits money to Marketplace contract until approved or cancelled
        currency.safeTransferFrom(msg.sender, address(this), listing.price);
    ...
    function approveBooking(uint256 requestId, uint256 customListingId) public {
        ...
        currency.approve(address(EscrowContract), listingRequest.deposited);
