...
//checked
  function Get_rich(uint256 _KOI_id) external {
    require(_isApprovedOrOwner(msg.sender, _KOI_id));
    ...
    Luckymoney[_KOI_id] += (Luckymoney_per_day +
      (_current_level * _current_level * 1e18) +
      (TransferTimes[_KOI_id] * 1e18));
  }
//checked
...
//checked
  function Count_Secrets(uint256 _KOI_id)
    external
    view
    returns (uint256 _Count_Secrets)
  {
    _Count_Secrets = utfStringLength(Secrets[_KOI_id]);
  }
//checked
