...
contract Fakhama is IERC20
{
    ...
    function updateRank(uint _memberId) internal
    {
        ...
        if(currentRank==1)
        {
            if(map_Users[_memberId].NextRankBusiness>=map_ranks[nextRank].Business*(1 ether)
                                        ||
                getDirectsCountByBusiness(_memberId, 100 ether, 5)>=map_ranks[nextRank].RequiredRankQualifiers)
            {
    ...
    function getDirectsCountByBusiness(uint _spId, uint _reqBusiness, uint _maxReq) public view returns(uint)
    {
        uint count=0;
        ...
        for(uint i=0;i<map_Users[_spId].DirectIds.length;i++)
        {
            ...
            if(map_Users[map_Users[_spId].DirectIds[i]].Business>=_reqBusiness)
            {
                count++;
            }
        }
    ...
    function getUserRanks(uint memberId) public view returns (RankInfo[] memory rankInfo)
    {
        ...
        for(uint i=1;i<=6;i++)
        {
            ...
            RankInfo memory temp_RankInfo = RankInfo({
                ...
                UserDirects:i==2?getDirectsCountByBusiness(memberId, 100 ether, 5):getDirectsCountByRank(memberId, i-1),
    ...
    function getRateHistory(uint _days) public view returns (CoinRateHistory[] memory history)
    {
        ...
        history=new CoinRateHistory[](100);

        uint idx = 0;
        for(uint i=RateHistoryCount-1; i>=1; i-=step)
        {
            if(map_CoinRateHistory[i].Timestamp>=startTimestamp)
            {
