...
contract Bank is AccessControl, ReentrancyGuard {
    ...
    /// @dev Units of rates are in basis point.
    struct HouseEdgeSplit {
        // go to holders
        uint16 dividend;
        uint256 dividendAmount;
        // for referral program
        uint16 referralProgram;
        uint256 referralProgramAmount;
        // go to treasury
        uint16 treasury;
    ...
    struct BalanceOverflow {
        uint256 threshold;
        uint16 toTreasury; // in basis point
    }

    bytes32 public constant GAME_ROLE = keccak256("GAME_ROLE");
    bytes32 public constant REFERRAL_PROGRAM = keccak256("REFERRAL_PROGRAM");
    uint8 private constant _BALANCE_RISK_RATE = 200; // in basis point

    /// The allowed tokens
    mapping(address => bool) public allowedTokens;
    mapping(address => HouseEdgeSplit) private _tokensHouseEdgeSplits;
    mapping(address => BalanceOverflow) private _balancesOverflows;
    mapping(address => uint256) private _balancesReferences;

    address payable private immutable _treasury; // multi-sig
    ...
    event SetBalanceOverflow(uint256 threshold, uint16 toTreasury);
    ...
    event ReferralProgramPayout(address user, address token, uint256 amount);
    ...
    constructor(address payable treasury, address payable teamWallet) {
    ...
    function setHouseEdgeSplit(
        ...
        uint16 splitSum = dividend + team + treasury + referralProgram;
        if (splitSum != 10000) {
            revert WrongHouseEdgeSplit(splitSum);
        }

        HouseEdgeSplit storage tokenHouseEdge = _tokensHouseEdgeSplits[token];
        tokenHouseEdge.dividend = dividend;
        tokenHouseEdge.referralProgram = referralProgram;
        ...
        emit SetTokenHouseEdgeSplit(
            ...
            tokenHouseEdge.referralProgram,
    ...
    function setBalanceOverflow(
        ...
        if (threshold == 0 || toTreasury > 10000) {
            revert WrongBalanceOverflow();
        }

        _balancesOverflows[token] = BalanceOverflow(threshold, toTreasury);
        emit SetBalanceOverflow(
            _balancesOverflows[token].threshold,
    ...
    {
        HouseEdgeSplit storage tokenHouseEdge = _tokensHouseEdgeSplits[token];
        _safeTransfer(_treasury, token, tokenHouseEdge.treasuryAmount);
    ...
    function payout(
        ...
        tokenHouseEdge.dividendAmount +=
            (fees * tokenHouseEdge.dividend) /
            10000;
        tokenHouseEdge.referralProgramAmount +=
            (fees * tokenHouseEdge.referralProgram) /
            10000;
        tokenHouseEdge.treasuryAmount +=
            (fees * tokenHouseEdge.treasury) /
            10000;
        tokenHouseEdge.teamAmount += (fees * tokenHouseEdge.team) / 10000;
        ...
        uint256 balance = getBalance(token);
        BalanceOverflow storage balanceOverflow = _balancesOverflows[token];
        if (
            balance >= (_balancesReferences[token] + balanceOverflow.threshold)
        ) {
            uint256 amountToTreasury = (((balance -
                _balancesReferences[token]) * balanceOverflow.toTreasury) /
                10000);
            _balancesReferences[token] = balance - amountToTreasury;

            _safeTransfer(_treasury, token, amountToTreasury);
    ...
    {
        return
            (((getBalance(token) * _BALANCE_RISK_RATE) / 10000) * 1000) /
            multiplier;
    }
    ...
    function getBalance(address token) public view returns (uint256) {
        ...
        HouseEdgeSplit memory tokenHouseEdgeSplit = _tokensHouseEdgeSplits[
            token
        ];
        return
            balance -
            tokenHouseEdgeSplit.dividendAmount -
            tokenHouseEdgeSplit.referralProgramAmount -
