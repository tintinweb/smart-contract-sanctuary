...
contract DockSpeedSprintSession is Ownable{
    ...
    mapping(uint256 => mapping(address => uint256)) public playerIndexInASession;
    ...
    constructor(IERC20 _Speed,IERC721 _nft){
        Speed =_Speed;
        NFT = _nft;
    ...
    function joinSession(uint256 _sessionId)public{
        require(NFT.balanceOf(msg.sender) >= uint256(1),"Required a car to join session");
    ...
    function getPriceOfASession(uint256 _sessionID)public view returns(uint256 _prize){
       return SessionDictionary[_sessionID].poolPrize;
    }

    function playersInASession(uint256 _sessionID)public view returns(address[] memory _racers){
        return SessionDictionary[_sessionID].racers;
    }
