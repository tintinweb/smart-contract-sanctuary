...
contract SnookGame is ISnookGame, AccessControlEnumerableUpgradeable, PausableUpgradeable {
    ...
    string public constant IPFS_URL_PREFIX = 'ipfs://';
    ...
    IPRNG private _prng;
    string[52] private mintTokenCIDs = [
    ...
    function _mint2() internal {
      ...
      if (traitId >= 1 && traitId <=5) {
        base = 0;
        offset = _prng.read(4); // rnd[0,3]
      } 
      else if (traitId >= 6 && traitId <= 15) { 
        base = 8;
        offset = _prng.read(2); // rnd[0,1]
      } 
      ...
      tokenURI = strConcat(IPFS_URL_PREFIX, mintTokenCIDs[base+offset]);
