...
contract StarSystemMinter is Ownable {
    using PlanetCounter for bytes32;
    
    StarSystems public starSystems;
    StarSystemData public starSystemsData;

    constructor(StarSystems _sys, StarSystemData _starSystemsData) {
        starSystems = _sys;
        starSystemsData = _starSystemsData;
    }

    function mintStarSystem(bytes32 _sysMap) external onlyOwner {
        mintStarSystem(_sysMap, msg.sender);
    }

    function mintStarSystem(bytes32 _sysMap, address _recipient) public onlyOwner {
        require(_sysMap.planetCount() > 0, "Map needs planet(s)");
        uint sysId = starSystems.mint(_recipient);
        starSystemsData.setMap(sysId, _sysMap);
    }
}
