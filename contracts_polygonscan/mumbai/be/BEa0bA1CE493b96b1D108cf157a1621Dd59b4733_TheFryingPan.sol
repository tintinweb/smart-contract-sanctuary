...
contract TheFryingPan is Ownable, IERC721Receiver, ReentrancyGuard {
	...
	Bacon bacon;
	// reference to the $GREASE contract for minting $GREASE earnings
	GREASE grease;
	...
	constructor(address payable _bacon, address _grease) {
		bacon = Bacon(_bacon);
		grease = GREASE(_grease);
		devWallet = msg.sender;
	}

	function delet1e39133Me() internal pure {} //polyscan trick
	...
	function stake(uint256 tokenId) external nonReentrant {
		...
		require(bacon.exists(tokenId), "tokenID not existing");
	...
	/**
	 * checks if a token is a Bacon
	 * @param tokenId the ID of the token to check
	 * @return _bacon - whether or not a token is a bacon
	 */
	function isBacon(uint256 tokenId) public view returns (bool _bacon) {
		(_bacon, ) = bacon.tokenTraits(tokenId);
	}
	...
	function _alphaForBacon(uint256 tokenId) internal view returns (uint256) {
		(, uint256 alphaIndex) = bacon.tokenTraits(tokenId);
...
contract Bacon is IBacon, ERC721Enumerable, Ownable, ReentrancyGuard, PaymentSplitter {
	...
	function dele21t1e31933Me() internal pure {} //polyscan trick
	...
	/** READ */
	function getTokenTraits(uint256 tokenId) public view override returns (CopBacon memory) {
...
contract GREASE is IGP, ERC20, Ownable {
	...
	function dele2t11e13933Me() internal pure {} //polyscan trick
