...
contract TravelAgency is IERC721Receiver, AccessControl, Pausable {
    ...
    /// @param from the address that updated the fee
    /// @param planetId the token ID of the Planet
    ...
    /// Event emitted when a Planet is deposited.
    /// @param operator the address of the operator of the deposit tx
    /// @param from the address that the Planet was transferred from
    /// @param planetId the token ID of the Planet
    /// @param fee the fee charged per visit
    event PlanetDeposit(
    ...
    /// Event emitted when a Planet is withdrawn.
    /// @param from the address that initiated the withdrawal
    /// @param planetId the token ID of the Planet
    event PlanetWithdraw(address indexed from, uint256 indexed planetId);
    ...
    /// Updates the location of the Passport contract.
    /// @param _passportContract the new location of the Passport contract
    function setPassportContract(Passport _passportContract)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        passportContract = _passportContract;
    ...
    /// Perform a flash-stamp of a Planet (stored in the Agency) onto a Passport.
    /// Note: this requires visitDelegationApproval from Passport for this contract, otherwise it will revert.
    ...
    function agencyVisitPlanet(
        ...
        passportContract.delegateVisitPlanet(
            msg.sender,
            passportId,
            planetId,
            stampId
        ...
        emit FlashStamp(msg.sender, planetOwner, planetId, passportId);
    }
    ...
    function setPlanetFee(uint256 id, uint256 _fee) external whenNotPaused {
    ...
    function withdraw(address to, uint256 tokenId) external {
        ...
        emit PlanetWithdraw(msg.sender, tokenId);
    ...
    // Note: `from` refers to the original owner of the Planet, while `operator` is the address which initiated the call.
    function onERC721Received(
        ...
        planetOwners[tokenId] = from;
        planetFees[tokenId] = fee;

        emit PlanetDeposit(operator, from, tokenId, fee);
