...
contract GaiaLand is ERC721, Ownable {
    ...
    string public GAIALANDS_PROVENANCE = ""; // IPFS PROVENANCE TO BE ADDED WHEN SOLD OUT

    string public LICENSE_TEXT = "";

    bool licenseLocked = false;
    
    uint256 public gaiaGoodLandPrice = 7500000000000000; // 0.0075 ETH

    uint256 public gaiaRegularLandPrice = 2500000000000000; // 0.0025 ETH

    uint256 public constant maxGaiaLandPurchase = 200;

    uint256 public MAX_GAIALANDS = 124884;

    bool public saleIsActive = false;

    uint256 public gaiaLandReserve = 1550; // Reserve 55 faces for team & community (Used in giveaways, events etc...)

    mapping (uint => uint256) public price;
    mapping (uint => bool) public listedMap;
    mapping (uint => address) public ownerMap;

    address public WMATICAddress = address(0x062f24cb618e6ba873EC1C85FD08B8D2Ee9bF23e);

    event licenseisLocked(string _licenseText);
    address ownerAddress;

    event SenderOwner(address creator, address sender);
    ...
    function setWMATICAddress(address WMATICAddress_) public onlyOwner {
    ...
    function mintGaiaLand(uint256 numberOfTokens, uint256 numberOfGoodLands, uint numberOfRegularLands) public payable {
        require(saleIsActive, "Sale must be active to mint GaiaLand");
        require(
            numberOfTokens > 0 && numberOfTokens <= maxGaiaLandPurchase,
        ...
        require(
            msg.value >= (gaiaGoodLandPrice.mul(numberOfGoodLands) + gaiaRegularLandPrice.mul(numberOfRegularLands)),
    ...
    public
    {
        emit SenderOwner(ownerMap[_id], msg.sender);
    ...
    function buy(uint _id, uint256 _price) external payable {
        _validate(_id);
        ...
        // 12.5% commission cut
        uint256 _commissionValue = price[_id].mul(125).div(1000);
        uint256 _sellerValue = price[_id].sub(_commissionValue);
        transferMATICOrWMATIC(payable(_previousOwner), _sellerValue);
        transferMATICOrWMATIC(payable(address(this)), _commissionValue);
