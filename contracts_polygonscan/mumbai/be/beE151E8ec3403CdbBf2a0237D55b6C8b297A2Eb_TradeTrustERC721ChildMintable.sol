...
interface IChildToken {
  ...
  /**
   * @dev Standard burn token function to zero address.
   * This impermanently burns a token and allows a same token ID to be minted again.
   * This call is only executable by an approved minter and does not require a token to have been surrendered.
   * For acceptance of surrendered token, use {TradeTrustERC721-destroyToken}.
   *
   * Requirements:
   *
   * - the caller must be a `minter`.
   * - the caller must be approved by owner
   * - the token is not surrendered
   *
