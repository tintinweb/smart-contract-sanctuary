...
// File contracts/bridge/OpenSkyChildTickets.sol
...
contract OpenSkyChildTickets is
    ERC721,
    ERC721URIStorage,
    ERC721Burnable,
    AccessControlMixin,
    ...
    ContextMixin
{
    ...
    constructor(
        ...
        _setupContractId("OpenSkyChildTickets");
    ...
    function withdraw(uint256 tokenId) external {
        require(_msgSender() == ownerOf(tokenId), "OpenSkyChildTickets: INVALID_TOKEN_OWNER");
    ...
    function withdrawBatch(uint256[] calldata tokenIds) external {
        ...
        uint256 length = tokenIds.length;
        require(length <= BATCH_LIMIT, "OpenSkyChildTickets: EXCEEDS_BATCH_LIMIT");
        ...
        for (uint256 i; i < length; i++) {
            ...
            require(_msgSender() == ownerOf(tokenId), string(abi.encodePacked("OpenSkyChildTickets: INVALID_TOKEN_OWNER ", tokenId)));
            withdrawnTokens[tokenId] = true;
            _burn(tokenId);
     ...
     */
    function mint(address user, uint256 tokenId, string memory _tokenURI) public only(MINTER_ROLE) {
        require(!withdrawnTokens[tokenId], "OpenSkyChildTickets: TOKEN_EXISTS_ON_ROOT_CHAIN");
