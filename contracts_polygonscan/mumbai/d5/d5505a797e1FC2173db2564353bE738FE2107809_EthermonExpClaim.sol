...
// File: contracts/EthermonExpClaim.sol
...
contract EthermonExpClaim is BasicAccessControl, SafeMathEthermon  {
    ...
    // public
    function extractExpToken(bytes32 _rt) public pure returns(uint32 rId, uint64 monster_id, uint32 exp) {
        rId = uint32(uint(_rt>>192));
        monster_id = uint64(uint(_rt>>128));
        exp = uint32(uint(_rt>>96));

    }
    

    function claimExp(bytes32 _token, uint8 _v, bytes32 _r, bytes32 _s) isActive external {
        ...
        if (eToken.rId == 0 || requestStatus[eToken.rId] > 0) revert();
        ...
        (obj.monsterId, obj.classId, obj.trainer, obj.exp, obj.createIndex, obj.lastClaimIndex, obj.createTime) = data.getMonsterObj(eToken.monster_id);
        
        data.increaseMonsterExp(eToken.monster_id, eToken.exp);

        requestStatus[eToken.rId] = 1;

    }



     function setConfig(address _verifyAddress, address _dataContract ) onlyModerators public {
