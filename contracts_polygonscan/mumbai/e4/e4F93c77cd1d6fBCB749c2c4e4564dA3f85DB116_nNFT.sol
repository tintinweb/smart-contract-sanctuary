...
contract nNFT is IERC721, Ownable {
    ...
    event updateGiveAwayAddress(address newGiveAway);
    ...
    address payable internal giveAway;
    address payable public charity;

    uint256 internal generatedGiveAway = 0;
    uint256 internal maxGiveAway = 2;
    ...
    uint256 private maxDonnations;
    uint256 private totalDonnations = 0;
    ...
    function setGiveAwayAddress(address payable newGiveAway) external onlyOwner {
        giveAway = newGiveAway;
        emit updateGiveAwayAddress(giveAway);
    }
    ...
    function mint(uint256 numberOfNfts, address payable referralAddress) external payable reentrancyGuard {
        ...
        if (msg.sender == giveAway) {
            require(generatedGiveAway.add(numberOfNfts) < maxGiveAway, 'No more giveaway');
            generatedGiveAway = generatedGiveAway.add(numberOfNfts);
        } else {
            ...
            if (totalDonnations < maxDonnations) {
                charityAmount = _calculateCharityFee(msg.value);
                totalDonnations = totalDonnations.add(charityAmount);
