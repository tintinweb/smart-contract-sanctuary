...
using SafeMath for uint;
    ...
    function createTournament(uint _pricePerPlayer, uint _playersPerTeam, uint _numberOfTeams, uint _daysTillRevert) public onlyOwner returns (uint){
            
            uint[] memory tempIds;
            uint[] memory tempDr;
            uint[] memory tempPlacements;
            IERC20 testToken= IERC20(0xfe4F5145f6e09952a5ba9e956ED0C25e3Fa4c7F1);
            Tournament memory t = Tournament(tournamentId,msg.sender,_pricePerPlayer,_playersPerTeam,_numberOfTeams,tempIds,0,0,tempPlacements,testToken,tempDr,addMinutes(block.timestamp,_daysTillRevert));
            tournaments.push(t);
