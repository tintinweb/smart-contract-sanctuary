...
contract Vaccy is Context, IERC20, Ownable {
    ...
    string private constant _name = "VaccyDoge";
    string private constant _symbol = "VACCY";
    uint8 private _decimals = 9;
    uint256 private constant _totalSupply = 24_000_000_000;
    ...
    // getTValues returns too many values otherwise
    struct TValues {
    ...
    uint256 public numTokensSellToAddToLiquidity = 12_000_000 * 10 ** _decimals;
    ...
    constructor () {
        ...
        _charityWalletAddress = msg.sender;
    ...
    function setCharityWalletAddress(address account) public onlyOwner() {
        require(account != address(0), "Cannot set the charity address to burn");
        require(account != _charityWalletAddress, "This is already the charity wallet address");
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        return (rAmount, rTransferAmount, rFee, tValues.tTransferAmount, tValues.tFee, tValues.tLiquidity, tValues.tMarketing, tValues.tCharity);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (TValues memory) {
        ...
        // Subtract in two lines to avoid hitting stack too deep error
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tLiquidity).sub(tMarketing);
        tTransferAmount = tTransferAmount.sub(tCharity);
        return TValues(tTransferAmount, tFee, tLiquidity, tMarketing, tCharity);
    }

    function _getRValues(uint256 tAmount, TValues memory tValues, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
