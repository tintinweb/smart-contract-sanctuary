    ...
    PausableUpgradeable
{
     ...
     */
    function _onlyAdmin() private returns (bool) {
        return
     ...
     */
    function _getRandomNumberForGacha(string calldata _city)
        internal
        returns (bytes32 requestId)
    {
...
contract MelXNFTV1 is IMelXNFT, MelXNFTPayment, MelXNFTBasic {
    ...
    modifier validateLimitMinting(string calldata city) {
        require(mintingLimit[city] > 0, "City is not set yet");
        require(
            mintingLimit[city] > cityToMinted[city] + 1,
    ...
    function safeMintWithGacha(address _to, string calldata city)
        public
        override
        validateLimitMinting(city)
     ...
     */
    function addNewCity(string calldata newCity, uint64 limitMinting)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(mintingLimit[newCity] == 0, "City already setted");
        require(limitMinting > 0, "Cannot set limit to 0");
        mintingLimit[newCity] = limitMinting;
        emit NewCity(newCity, limitMinting);
    }

    /**
     * @dev Update existing city and set the new minting limitation
     */
    function updateCityLimit(string calldata city, uint64 newLimitMinting)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(mintingLimit[city] > 0, "City is not set yet");
        require(
            newLimitMinting > cityToMinted[city] && newLimitMinting > 0,
            "New Limit too low"
        );
        mintingLimit[city] = newLimitMinting;
        emit UpdateCity(city, newLimitMinting);
    }
}
