...
contract ForeignNFTOmnibridge is BasicNFTOmnibridge, GasLimitManager {
    constructor() BasicNFTOmnibridge(address(0)) {}
...
contract NFTbIDStorage is Upgradeable {
    mapping(address => bool) allowedBridges;
    mapping(uint256 => uint256) bscToRealIDERC721;
    mapping(uint256 => uint256) bscToRealIDERC1155;

    modifier onlyBridge() {
        require(allowedBridges[msg.sender], "can be called only by bridge contract");
        _;
    }

    constructor(address _bridgeContract) {
        allowedBridges[_bridgeContract] = true;
    }
    ...
    function allowBridgeContract(address _bridgeContract)
    external
    onlyIfUpgradeabilityOwner {
        require(_bridgeContract != address(0));
        if (!allowedBridges[_bridgeContract]){
            allowedBridges[_bridgeContract] = true;
        }
    }

    function removeBridgeContract(address _bridgeContract)
    external
    onlyIfUpgradeabilityOwner {
        require(_bridgeContract != address(0));
        if (allowedBridges[_bridgeContract]){
            allowedBridges[_bridgeContract] = false;
        }
    }
}
