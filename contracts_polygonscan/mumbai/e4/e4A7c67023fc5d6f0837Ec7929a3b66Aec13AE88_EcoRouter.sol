...
import './interfaces/IEcoRouter02.sol';
import './libraries/EcoLibrary.sol';
import './libraries/IEcoFactory.sol';
import './libraries/IEcoPair.sol';
...
contract EcoRouter is IEcoRouter02 {
        ...
        returns (uint[] memory amounts)
    {
        ...
        amounts = EcoLibrary.getAmountsOut(factory, msg.value, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'EcoRouter: INSUFFICIENT_OUTPUT_AMOUNT');
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(EcoLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
    }
        ...
        returns (uint[] memory amounts)
    {
        require(path[0] == WETH, 'EcoRouter: INVALID_PATH');
        amounts = EcoLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= msg.value, 'EcoRouter: EXCESSIVE_INPUT_AMOUNT');
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(EcoLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
        // refund dust eth, if any
        ...
        ensure(deadline)
    {
        ...
        assert(IWETH(WETH).transfer(EcoLibrary.pairFor(factory, path[0], path[1]), amountIn));
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
