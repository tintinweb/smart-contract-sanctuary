...
contract XVMCrapidAdoptionBoost is Ownable {
    ...
    bool rapidAdoptionBoost; //can only be called once
    /*
    ...
    */
    ...
    uint256 preProgrammedCounter = 290; // used for Rapid Adoption Boost, to decrease rewards periodically
    ...
    function activateRapidAdoptionBoost() external {
        ...
        require(block.timestamp > 1638896400, "Date and time (GMT): Tuesday, December 7, 2021 5:00:00 PM");
        rapidAdoptionBoost = true;
    }
    function updatePreProgrammedRewards() external {
        require(rapidAdoptionBoost && preProgrammedCounter > 0, "Must be activated"); 
        if(block.timestamp > 1640106000 && preProgrammedCounter != 0) {
            IXVMCgovernor(owner()).setInflation(25694201337 * 1e9); //set to roughly 25tokens/block.
            preProgrammedCounter = 0; 
        } else {
            if(preProgrammedCounter == 290) {
                IXVMCgovernor(owner()).setInflation(500 * 1e18);
                lastReward = 500 * 1e18;
                preProgrammedCounter--;
                preProgrammedCounterTimestamp = block.timestamp;
                IERC20(token).safeTransfer(payable(msg.sender), IXVMCgovernor(owner()).costToVote());
            } else if(preProgrammedCounter < 290 && preProgrammedCounter > 46) {
                require(preProgrammedCounterTimestamp + 10 minutes < block.timestamp, "wait 10minutes");
