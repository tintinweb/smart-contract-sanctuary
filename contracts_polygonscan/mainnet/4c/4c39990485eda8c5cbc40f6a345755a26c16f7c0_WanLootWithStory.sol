...
// File contracts/WanLootWithStory.sol
...
contract WanLootWithStory is ERC721("Wan Loot Story Board", "elBOARD") {
        ...
        returns (string memory)
    {
        ...
        parts[
            0
        ] = '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350"><style>.base { fill: black; font-family: serif; font-size: 12px; }.content{line-height:1.2;animation: moveUp 8s 0s infinite;position:relative;color: #ee28fe;}.box{overflow: hidden;height: 150px;}@keyframes moveUp{0% {top:100%;} 100% {top:-100%; display: none;}}</style><rect width="100%" height="100%" fill="black" /><text x="10" y="20" class="base">';

        parts[1] = IWanLoot(eloot).getWeapon(tokenId);

        parts[2] = '</text><text x="10" y="40" class="base">';

        parts[3] = IWanLoot(eloot).getFaces(tokenId);

        parts[4] = '</text><text x="10" y="60" class="base">';

        parts[5] = IWanLoot(eloot).getBody(tokenId);

        parts[6] = '</text><text x="10" y="80" class="base">';

        parts[7] = IWanLoot(eloot).getLocation(tokenId);

        parts[8] = '</text><text x="10" y="100" class="base">';

        parts[9] = IWanLoot(eloot).getSport(tokenId);

        parts[10] = '</text><text x="10" y="120" class="base">';

        parts[11] = IWanLoot(eloot).getPets(tokenId);

        parts[12] = '</text><text x="10" y="140" class="base">';

        parts[13] = IWanLoot(eloot).getFlowers(tokenId);

        parts[14] = '</text><text x="10" y="160" class="base">';

        parts[15] = IWanLoot(eloot).getTraffic(tokenId);
            ...
            bytes(
                string(
                    abi.encodePacked(
                        ...
                        '", "description": "Wan Loot is randomized adventurer gear generated and stored on chain. Stats, images, and other functionality are intentionally omitted for others to interpret. Feel free to use Loot in any way you want.", "image": "data:image/svg+xml;base64,',
