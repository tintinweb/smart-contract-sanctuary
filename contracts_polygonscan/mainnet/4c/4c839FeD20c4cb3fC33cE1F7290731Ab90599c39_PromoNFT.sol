...
contract PromoNFT is Ownable, ERC721Holder  {
    ...
    uint public price = 2; /// 2 GEN
    uint public breedPrice = 200; // 1   GEN / 100
    uint[] public maxValuesSeed;
    mapping(uint=>uint[]) public eggsRanges;
    
    mapping(address=>Breeding) public breedingToUser;
    mapping(address=>Random.Seed) public hatchingToUser;
    mapping(address=>uint) public eggToUser;
    mapping(address=>Random.Seed) public evolvingToUser;
    mapping(address=>uint) public evolvingToken;
    ...
    constructor() {
        ...
        pool = address(0xC2b3CfD8d78e09624E5e4Ff225f7c4957EcF997e);
        
        arenaNFT = ArenaNFT(0xf3041f58f7c7dEE9B146b4D8b8D4FB0c31c7ac82);
    }
    
    function setupRanges() external onlyOwner {
        eggsRanges[0] = [0,600,800,920,970, 1000];
        eggsRanges[1] = [0,400,670,870,950, 1000];
        eggsRanges[2] = [0,200,550,800,920, 1000];

        maxValuesSeed = [0,350,600,800,920,1000];
    }

    function updateParams(uint _price) public onlyOwner {
        price = _price;
    }

    function hatchEgg(uint nftId) public { // 0,1,2
    
        require(arenaNFT.ownerOf(nftId) == msg.sender, "you are not owner");
        require(arenaNFT.getTypeByTokenId(nftId).id == 100, "wrong NFT");
        
        arenaNFT.safeTransferFrom(address(msg.sender), address(this), nftId);
        
        hatchingToUser[msg.sender].init();
    }


    function openEgg() public {
        
        require(hatchingToUser[msg.sender].isReady(),"Open Egg first");
        
        bytes32 ss = hatchingToUser[msg.sender].get();
        ...
        hatchingToUser[msg.sender].reset();
    }  
    
    function _openEgg(uint seed) public {
        uint abilitiesSeed = seed % 1000;
        uint valuesSeed = seed / 1000;

        uint resultValuesSeed = 0;
        uint amount = 0;
        while (eggsRanges[eggToUser[msg.sender] - 1][amount] <= abilitiesSeed) {
            amount ++;
        ...
        uint tokenId = nft.createToken(msg.sender);
