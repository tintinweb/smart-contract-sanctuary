...
interface IKyberFairLaunch {
    function deposit(uint256 _pid, uint256 _amount, bool shouldHarvest) external;
    function withdraw(uint256 _pid, uint256 _amount) external;
    function emergencyWithdraw(uint256 _pid) external;
    function harvest(uint _pid) external;
    function getUserInfo(uint256 _pid, address _account) external view returns (uint256, uint256[] memory, uint256[] memory);
...
interface IRewardsLocker {
    function vestCompletedSchedules(IERC20 token) external;
}
...
contract StrategyKyber is StratManager, FeeManager {
    ...
    IERC20[] public outputToLp0Route;
    IERC20[] public outputToLp1Route;
    ...
    constructor(
        ...
        address[] memory _outputToLp0PoolsPath,
        address[] memory _outputToLp1PoolsPath
        ...
        for (uint i = 0; i < _outputToLp1Route.length; i++) {
            outputToLp1Route.push(IERC20(_outputToLp1Route[i]));
        }
        
        outputToNativePoolsPath = _outputToNativePoolsPath;
        outputToLp0PoolsPath = _outputToLp0PoolsPath;
        outputToLp1PoolsPath = _outputToLp1PoolsPath;
    ...
    function deposit() public whenNotPaused {
        ...
        if (wantBal > 0) {
            IKyberFairLaunch(fairLaunch).deposit(poolId, wantBal, false);
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            IKyberFairLaunch(fairLaunch).withdraw(poolId, _amount.sub(wantBal));
    ...
    function _harvest(address callFeeRecipient) internal whenNotPaused {
        ...
        IRewardsLocker(rewardsLocker).vestCompletedSchedules(IERC20(output));
        
        uint256 outputBal = IERC20(output).balanceOf(address(this));
        if (outputBal > 0) {
            chargeFees(callFeeRecipient);
            addLiquidity();
            uint256 wantHarvested = balanceOfWant().sub(wantBeforeHarvest);
            deposit();
    ...
    function chargeFees(address callFeeRecipient) internal {
        ...
        IDMMRouter02(unirouter).swapExactTokensForTokens(toNative, 0, outputToNativePoolsPath, outputToNativeRoute, address(this), now);
    ...
    function addLiquidity() internal {
        ...
        if (lpToken0 != output) {
            IDMMRouter02(unirouter).swapExactTokensForTokens(outputHalf, 0, outputToLp0PoolsPath, outputToLp0Route, address(this), now);
        }

        if (lpToken1 != output) {
            IDMMRouter02(unirouter).swapExactTokensForTokens(outputHalf, 0, outputToLp1PoolsPath, outputToLp1Route, address(this), now);
        }
        
        uint256 lp0Bal = IERC20(lpToken0).balanceOf(address(this));
        uint256 lp1Bal = IERC20(lpToken1).balanceOf(address(this));
        uint256[2] memory vReserveRatioBounds = [0, uint256(-1)];
        IDMMRouter02(unirouter).addLiquidity(
            IERC20(lpToken0), 
            IERC20(lpToken1), 
            want,
            lp0Bal,
    ...
    function balanceOfPool() public view returns (uint256) {
        (uint256 _amount,,) = IKyberFairLaunch(fairLaunch).getUserInfo(poolId, address(this));
        return _amount;
    }

    // returns rewards unharvested
    function rewardsAvailable() public view returns (uint256) {
        return IKyberFairLaunch(fairLaunch).pendingRewards(poolId, address(this))[0];
    }
    ...
    function callReward() public view returns (uint256) {
        ...
        uint256 nativeOut;
        if (outputBal > 0) {
            try IDMMRouter02(unirouter).getAmountsOut(outputBal, outputToNativePoolsPath, outputToNativeRoute)
    ...
    function retireStrat() external {
        ...
        IKyberFairLaunch(fairLaunch).emergencyWithdraw(poolId);
    ...
    function _giveAllowances() internal {
        IERC20(want).safeApprove(fairLaunch, uint256(-1));
    ...
    function _removeAllowances() internal {
        IERC20(want).safeApprove(fairLaunch, 0);
    ...
    function outputToLp0() external view returns (IERC20[] memory) {
        return outputToLp0Route;
    }

    function outputToLp1() external view returns (IERC20[] memory) {
    ...
    function outputToLp0Pools() external view returns (address[] memory) {
        return outputToLp0PoolsPath;
    }

    function outputToLp1Pools() external view returns (address[] memory) {
        return outputToLp1PoolsPath;
    }
}