...
contract Marketplace is Ownable, AccessControl, ReentrancyGuard {
 ...
 function createOrder (address _userAddress,uint256 _itemId, uint256 _itemAmount, uint256 _nakaAmount) public {
        ...
        orderByOrderId[_userAddress][_orderId] = Order({
        id: _orderId,
        seller: _userAddress,
        itemId: _itemId,
        ...
        if (publicationFeeInWei >= 0) {
            vaultBalance.decreaseBalance(_userAddress,publicationFeeInWei);
            vaultBalance.increaseBalance(_Owner,publicationFeeInWei);
        }

        vaultItem.decreaseItem(_userAddress, _itemId, _itemAmount);
        emit OrderCreated(_orderId, _userAddress, _itemId, _itemAmount, _nakaAmount);
    }
