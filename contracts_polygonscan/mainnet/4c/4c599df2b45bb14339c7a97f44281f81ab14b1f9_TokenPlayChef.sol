...
contract TokenPlayChef is Ownable, ReentrancyGuard {
    ...
    function checkLpToken(IERC20 _lpToken) internal view {
        require(_lpToken.balanceOf(address(this)) >= 0, "Invalid ERC20");
    }
    ...
    function replacePoolLpToken(uint256 _pid, IERC20 _lpToken) external onlyOwner {
        ...
        require(pool.accRewardPerShare == 0 && pool.totalLpSupply == 0, "Cant replace running pool");
        massUpdatePools();
    ...
    function changeStrategy(uint256 _pid, address _newStrategy) external onlyOwner {
        ...
        if (_newStrategy != address(0)) {
            ...
            require(IStrategy(_newStrategy).targetProfitToken() == reward, "!targetProfitToken");
    ...
    function setOperationRate(uint256 _operationRate) external onlyOwner {
        require(_operationRate <= 3000, "too high"); // <= 30%
        massUpdatePools();
    ...
    function setMarketingRate(uint256 _marketingRate) external onlyOwner {
        require(_marketingRate <= 3000, "too high"); // <= 30%
        massUpdatePools();
    ...
    function setGameTreasuryRate(uint256 _gameTreasuryRate) external onlyOwner {
        require(_gameTreasuryRate <= 3000, "too high"); // <= 30%
        massUpdatePools();
    ...
    function depositWithRef(
        ...
        if (_amount > 0) {
            ...
            if (_amount > 0) {
                ...
                if (_depositFeeBP > 0 && operationFund != address(0)) {
                    uint256 _fee = _amount.mul(_depositFeeBP).div(10000);
                    _lpToken.safeTransfer(operationFund, _fee);
    ...
    function _withdraw(
        ...
        if (_amount > 0) {
            ...
            pool.lpToken.safeTransfer(_account, _sentAmount);
        }
        user.rewardDebt = user.amount.mul(pool.accRewardPerShare).div(1e18);
