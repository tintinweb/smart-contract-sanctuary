...
contract NFTMarketplace is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable, UUPSUpgradeable {
    ...
    // percentage fee taken from each side (buyer, seller)
    // example: 
    ...
    // Used to check what is the buy item price
    ...
    function buyItem(address tokenAddress, uint tokenId) public payable nonReentrant {
        (Item memory item, bytes32 orderHash) = _getOrder(tokenAddress, tokenId);
        ...
        require(msg.value == item.price + fee, "Invalid amount");
