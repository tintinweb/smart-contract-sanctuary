...
contract It is ERC721 {
    mapping (address => uint256) public taggedAt;
    address[] tags;

    uint256 public ROUND_IN_SECONDS = 60 * 60 * 24;
    uint256 public TOKEN_ID = 1;

    error GameOver();

    constructor() public ERC721("You're IT", "IT") {
        _mint(msg.sender, TOKEN_ID);
        tag(msg.sender, block.timestamp);
    }

    function getTags() public view returns (address[] memory) {
        return tags;
    }
    ...
    function safeTransferFrom(
        ...
        if(canTransfer(from, block.timestamp)) {
            super.safeTransferFrom(from, to, tokenId);
            tag(to, block.timestamp);

        } else {
            revert("GAME OVER, YOU'RE IT FOREVER");
        }
    }

    function tag(address to, uint256 timestamp) internal {
        taggedAt[to] = timestamp;
        tags.push(to);
    }

    function canTransfer(address from, uint256 timestamp) public view returns (bool) {
        if (from == address(0)) {
            return true;
        }

        return (timestamp <= taggedAt[from] + ROUND_IN_SECONDS);
    }

    function isGameOver(uint256 timestamp) public view returns (bool) {
        return canTransfer(ownerOf(TOKEN_ID), timestamp) == false;
    }
