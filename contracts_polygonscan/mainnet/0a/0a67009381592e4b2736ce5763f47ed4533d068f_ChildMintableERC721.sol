...
// POLYGON MATIC CHILDMINTABLEERC721
    ...
    ContextMixin
{
    ...
    constructor() public ERC721("S3C", "S3C") {
        _setBaseURI("https://ipfs.io/ipfs/QmUNLLsPACCz1vLxQVkXqqLX5R1X345qqfHbsf67hvA3Nn/");
        _setupContractId("Seesr");
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _setupRole(DEPOSITOR_ROLE, 0xA6FA4fB5f76172d178d61B04b0ecd319C5d1C0aa);
        _initializeEIP712("Seesr");
    }


    function setBaseUri(string memory uri) public only(DEFAULT_ADMIN_ROLE) {
        _setBaseURI(uri);
    }
    ...
    function withdraw(uint256 tokenId) external {
        require(_msgSender() == ownerOf(tokenId), "Seesr: INVALID_TOKEN_OWNER");
    ...
    function withdrawBatch(uint256[] calldata tokenIds) external {
        ...
        uint256 length = tokenIds.length;
        require(length <= BATCH_LIMIT, "Seesr: EXCEEDS_BATCH_LIMIT");
        ...
        for (uint256 i; i < length; i++) {
            ...
            require(_msgSender() == ownerOf(tokenId), string(abi.encodePacked("Seesr: INVALID_TOKEN_OWNER ", tokenId)));
            withdrawnTokens[tokenId] = true;
            _burn(tokenId);
    ...
    function mint(address user, uint256 tokenId) public only(DEFAULT_ADMIN_ROLE) {
        require(!withdrawnTokens[tokenId], "Seesr: TOKEN_EXISTS_ON_ROOT_CHAIN");
        _mint(user, tokenId);
    }
