...
contract PolySafe_Test07 is Token {
    ...
    uint private constant TIME_STEP         = 30 * 60; // 1 days;
    ...
    function getStakeMatic_IP(address _addr) view private returns(uint256 value) {
        ...
        uint256 to = now;
        if(fr < to) {
            value = user.sM.totalStaked.mul(to - fr).mul(Tarif).div(86400).div(PERCENT_DIVIDER);
        } else {
            value = 0;
        }
        return value;
    ...
    function getStakeToken_IP(address _addr) view private returns(uint256 value) {
        ...
        uint256 to = now;
        if(fr < to) {
            value = user.sT.totalStaked.mul(to - fr).mul(Tarif).div(86400).div(PERCENT_DIVIDER);
        } else {
            value = 0;
        }
        return value;
