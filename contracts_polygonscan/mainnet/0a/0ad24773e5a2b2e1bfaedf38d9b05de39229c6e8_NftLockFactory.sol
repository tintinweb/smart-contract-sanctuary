...
interface INftTimeLocker {

    function getRealeaseTime() external view returns (uint256);
...
contract NftLockFactory is Ownable{
    uint256 realeaseTime = 123456789;
    mapping (address => address) public timeLockAccount;
    address public nftAddress = address(0xC0f8262D27509dBc9eA1645c7a5740b7312c0F85);
    IERC721 public ierc721;

    event NftLockCreated(address indexed _lockedBy, address indexed _lockAddress);


    constructor() {
        ierc721 = IERC721(nftAddress);
    }

    function balanceOf(address owner) public view returns (uint256){
        return ierc721.balanceOf(owner);
    }
    ...
    function createNftLockAndLockAll() external {
        require(timeLockAccount[msg.sender] == address(0),'FACTORY : You already have a Locker');
        require(ierc721.isApprovedForAll(msg.sender, address(this)),'FACTORY : Not approved for all');
        NftTimeLocker lock = new NftTimeLocker(msg.sender, address(this), nftAddress);
        timeLockAccount[msg.sender] = address(lock);
        // send all nft to the timelockAccount
        uint256 nftCount = ierc721.balanceOf(msg.sender);

        for (uint256 i = 0; i < nftCount; i++) {
            uint256 tokenId = ierc721.tokenOfOwnerByIndex(msg.sender, 0);
            ierc721.transferFrom(msg.sender, address(lock), tokenId);
        }

        emit NftLockCreated(msg.sender, address(lock));
    }



    function createNftLockAndLockListOfNftIds(uint256[] memory _ids) external {
        require(timeLockAccount[msg.sender] == address(0),'FACTORY : You already have a Locker');
        require(ierc721.isApprovedForAll(msg.sender, address(this)),'FACTORY : Not approved for all');
        NftTimeLocker lock = new NftTimeLocker(msg.sender, address(this), nftAddress);
        timeLockAccount[msg.sender] = address(lock);
       ...
       for (uint256 i = 0; i < _ids.length; i++) {
            ...
            require(ierc721.ownerOf(tokenId) == msg.sender,'FACTORY : You dont have this token');
            ierc721.transferFrom(msg.sender, address(lock), tokenId);
        }
    ...
    function lockInTimeLocker(uint256[] memory _ids) external {
        require(timeLockAccount[msg.sender] != address(0),'FACTORY : You dont have a Locker');
        require(ierc721.isApprovedForAll(msg.sender, address(this)),'FACTORY : Not approved for all');
        //locks[_nftId].lockNft(_owner, _lockTime);
       // console.log("lock Addr3", INftTimeLocker(timeLockAccount[msg.sender]).getRealeaseTime());
        for (uint256 i = 0; i < _ids.length; i++) {
            uint256 tokenId = _ids[i];
            require(ierc721.ownerOf(tokenId) == msg.sender,'FACTORY : You dont have this token');
            ierc721.transferFrom(msg.sender, timeLockAccount[msg.sender], tokenId);
        }

    }


    function emergencyWithdraw(uint256[] memory _ids, address _timelocker) external onlyOwner {
          NftTimeLocker nftTimeLocker = NftTimeLocker(_timelocker);
          nftTimeLocker.emergencyWithdraw(_ids);
    }

    function withdrawMyLocker() external {
        require(realeaseTime < block.timestamp, 'FACTORY : You can not withdraw before the release time');
        require(timeLockAccount[msg.sender] != address(0),'FACTORY : You dont have a Locker');
        NftTimeLocker nftTimeLocker = NftTimeLocker(timeLockAccount[msg.sender]);
        nftTimeLocker.withdrawAll();
    }
    //utils

    function changeReleaseTime(uint256 _releaseTime) external onlyOwner {
        realeaseTime = _releaseTime;
    }

    function getTimeLocker(address _owner) external view returns (address) {
        return timeLockAccount[_owner];
    }
...
contract NftTimeLocker is AccessControl{
    ...
    constructor(address _owner, address _factory, address _nftAddress) {
        lockedBy = _owner;
        factoryAddress = _factory;

        _setupRole(OWNER_ROLE, _owner);
        _setupRole(CONTROLLER_ROLE, _factory);
    ...
    function getRealeaseTime() external view returns (uint256) {
        NftLockFactory nftlockfactory = NftLockFactory(factoryAddress);
        return nftlockfactory.getReleaseTime();
    }

    function withdrawAll() external onlyRole(OWNER_ROLE){ //withdraw all NFTs
    NftLockFactory nftlockfactory = NftLockFactory(factoryAddress);
    require(nftlockfactory.getReleaseTime() < block.timestamp, 'FACTORY : You can not withdraw before the release time');
    uint256 nftCount = ierc721.balanceOf(address(this));
    
        for (uint256 i = 0; i < nftCount; i++) {
            uint256 tokenId = ierc721.tokenOfOwnerByIndex(address(this), 0);
            ierc721.transferFrom(address(this), msg.sender, tokenId);
        }

    }

    function emergencyWithdraw(uint256[] memory _ids) external onlyRole(CONTROLLER_ROLE) { 
           
        for (uint256 i = 0; i < _ids.length; i++) {
            uint256 tokenId = _ids[i];
            ierc721.transferFrom(address(this), lockedBy, tokenId);
        }
