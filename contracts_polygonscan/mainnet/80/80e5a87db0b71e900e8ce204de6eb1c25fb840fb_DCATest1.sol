...
// File: https://github.com/QuickSwap/QuickSwap-periphery/blob/master/contracts/interfaces/IUniswapV2Router01.sol
...
// File: https://github.com/QuickSwap/QuickSwap-periphery/blob/master/contracts/interfaces/IUniswapV2Router02.sol
...
contract DCATest1 is AccessControl {
    ...
    address immutable public quickswapRouter;
    ...
    function createDCA(address _fromToken, address _toToken, uint256 _amount, uint256 _interval, uint256 _numberOfSwaps) external {
        DCAList.push(DCA(msg.sender, _fromToken, _toToken, block.timestamp, _interval, 0, _amount, _numberOfSwaps, true, false));
    ...
    function pauseOrResumeDCA(uint256 _id) external {
        require(msg.sender == DCAList[_id].user, 'not your DCA!');
        DCAList[_id].active = !DCAList[_id].active;
    }
    ...
    function swapOnOneInch(uint256 _id, bytes calldata _data) public onlyRole(MANAGER_ROLE) {
        require(getTime(_id) == 0 && DCAList[_id].numberOfSwaps > 0);
        ...
        if (success) {
            ...
            uint256 txFee = gasOneInch * tx.gasprice;
            uint256 koef = getGasFeeKoef(_id, outAmount, txFee);
            uint256 totalFee = outAmount / koef + outAmount * 5 / 10000;
            require(IERC20(DCAList[_id].toToken).transfer(admin, totalFee));
            require(IERC20(DCAList[_id].toToken).transfer(DCAList[_id].user, outAmount - totalFee));
        }
        else {
            revert();
