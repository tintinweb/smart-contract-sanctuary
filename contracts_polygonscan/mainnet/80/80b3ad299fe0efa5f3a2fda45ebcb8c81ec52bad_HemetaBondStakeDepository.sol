...
// File: contracts/interfaces/IsHEME.sol


pragma solidity >=0.7.5;


interface IsHEME is IERC20 {
...
// File: contracts/HemetaBondStakeDepository.sol
...
contract HemetaBondStakeDepository is Ownable {
    ...
    address public immutable HEME; // intermediate reward token from treasury
    address public immutable sHEME; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints HEME when receives principle
    ...
    struct Bond {
        uint256 payout; //HEME at the moment of bond
        uint256 vesting; // Blocks left to vest
        uint256 lastTimestamp; // Last interaction
        uint256 pricePaid; // In DAI, for front end viewing
        uint256 gonsPayout; // sHEME gons remaining to be paid
    }
    ...
    constructor(
        ...
        require(_HEME != address(0));
        HEME = _HEME;
        require(_sHEME != address(0));
        sHEME = _sHEME;
    ...
    function deposit(
        ...
        uint256 value = IHemetaTreasury(treasury).valueOfToken(
            principle,
            _amount
        );
        uint256 payout = payoutFor(value); // payout to bonder is computed

        require(payout >= 10000000, "Bond too small"); // must be > 0.01 HEME ( underflow protection )
        ...
        if (fee != 0) {
            ...
            IERC20(HEME).safeTransfer(DAO, fee);
        }

        // total debt is increased
        totalDebt = totalDebt.add(value);

        IERC20(HEME).approve(staking, payout);
        IHemetaStaking(staking).stake(payout, address(this));
        IHemetaStaking(staking).claim(address(this));

        uint256 stakeGons = IsHEME(sHEME).gonsForBalance(payout);
        ...
        returns (uint256)
    {
        ...
        uint256 _amount = IsHEME(sHEME).balanceForGons(info.gonsPayout);
        emit BondRedeemed(_recipient, _amount, 0); // emit bond data
        IERC20(sHEME).transfer(_recipient, _amount); // pay user everything due
        return _amount;
    }
    ...
    function maxPayout() public view returns (uint256) {
        return IERC20(HEME).totalSupply().mul(terms.maxPayout).div(100000);
    }
    ...
    function bondPriceInUSD() public view returns (uint256 price_) {
        if (isLiquidityBond) {
            price_ = bondPrice()
                .mul(
                    IHemetaBondingCalculator(bondCalculator).markdown(principle)
                )
                .div(100);
        } else {
    ...
    /**
     *  @notice calculate current ratio of debt to HEME supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns (uint256 debtRatio_) {
        uint256 supply = IERC20(HEME).totalSupply();
    ...
    function standardizedDebtRatio() external view returns (uint256) {
        if (isLiquidityBond) {
            return
                debtRatio()
                    .mul(
                        IHemetaBondingCalculator(bondCalculator).markdown(
                            principle
                        )
                    )
                    .div(1e9);
    ...
    /**
     *  @notice calculate amount of HEME available for claim by depositor
     *  @param _depositor address
        ...
        returns (uint256 pendingPayout_)
    {
        ...
        uint256 payout = IsHEME(sHEME).balanceForGons(
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or HEME) to the DAO
     *  @return bool
     */
    function recoverLostToken(address _token) external returns (bool) {
        require(_token != HEME);
        require(_token != sHEME);
