...
contract ProtofiFactory is IProtofiFactory {
    ...
    function setFeeAmount(uint16 _newFeeAmount) external{
        ...
        // 25 = 0.25% (all fee goes directly to the feeTo address)
        // If we update it to 15 for example, 0.10% are going to LP holder and 0.15% to the feeManager
        require(msg.sender == owner(), "caller is not the owner");
        require (_newFeeAmount >= 1, "amount too low");
        require (_newFeeAmount <= 25, "amount too big");
        ...
        if (feeOn) {
            if (_kLast != 0) {
                ...
                if (rootK > rootKLast) {
                    ...
                    uint denominator = rootK.mul(25-IProtofiFactory(factory).feeAmount()).add(rootKLast.mul(IProtofiFactory(factory).feeAmount()));
