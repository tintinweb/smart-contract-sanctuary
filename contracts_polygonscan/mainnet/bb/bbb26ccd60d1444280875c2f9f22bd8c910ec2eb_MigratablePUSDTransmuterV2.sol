...
// File contracts/interfaces/IVaultAdapterV2WithIndirection.sol

pragma solidity ^0.6.12;

/// Interface for all Vault Adapter implementations.
interface IVaultAdapterV2WithIndirection {
  ...
  /// @dev Attempts to withdraw vault tokens from the wrapped vault.
  ///
  /// The amount withdrawn to the recipient may be less than the amount requested.
  ///
  /// @param _recipient the recipient of the vault tokens.
...
// File contracts/libraries/alchemist/PolyquityVaultV2WithIndirection.sol
...
/// @title PolyquityVaultV2WithIndirection
///
/// @dev A library which provides the Vault data struct and associated functions.
library PolyquityVaultV2WithIndirection {
  using PolyquityVaultV2WithIndirection for Data;
  using PolyquityVaultV2WithIndirection for List;
  using SafeMath for uint256;

  struct Data {
    IVaultAdapterV2WithIndirection adapter;
  ...
  function directWithdraw(Data storage _self, address _recipient, uint256 _amount, bool _isHarvest) internal returns (uint256, uint256) {
    ...
    if (_isHarvest) {
      return (_withdrawnAmount, _actualDecreaseValue);
    }
  ...
  /// @dev Indirectly withdraw deposited funds from the vault.
  ///
  /// @param _recipient the account to withdraw the tokens to.
  /// @param _amount    the amount of tokens to withdraw.
  ...
  function harvest(Data storage _self, address _recipient) internal returns (uint256, uint256) {
    // polyquity harvest is not related to total deposit and total value, so just use 0 amount;
...
// File contracts/MigratablePUSDTransmuterV2.sol
 ...
 */
contract MigratablePUSDTransmuterV2 is Context {
    using SafeMath for uint256;
    using SafeERC20 for IERC20Burnable;
    using Address for address;
    using PolyquityVaultV2WithIndirection for PolyquityVaultV2WithIndirection.Data;
    using PolyquityVaultV2WithIndirection for PolyquityVaultV2WithIndirection.List;
    ...
    /// deposits and withdraws. PolyquityVaultV2WithIndirection before the last element are considered inactive and are expected to be cleared.
    PolyquityVaultV2WithIndirection.List private _vaults;
    ...
    event ActiveVaultUpdated(
        IVaultAdapterV2WithIndirection indexed adapter
    );
    ...
    constructor(address _waToken, address _token, address _governance) public {
        require(_governance != ZERO_ADDRESS, "MigratablePUSDTransmuterV2: 0 gov");
    ...
    modifier onlyWhitelisted() {
        require(whiteList[msg.sender], "MigratablePUSDTransmuterV2: !whitelisted");
        _;
    }
    ...
    ///
    modifier onlyGov() {
        require(msg.sender == governance, "MigratablePUSDTransmuterV2: !governance");
        _;
    }
    ...
    function claim() public {
        ...
        require(realisedTokens[sender] > 0, "MigratablePUSDTransmuterV2: no realisedToken balance for sender");
    ...
    function unstake(uint256 amount) public updateAccount(msg.sender) {
        ...
        address sender = msg.sender;
        require(depositedWaTokens[sender] >= amount, "MigratablePUSDTransmuterV2: unstake amount exceeds deposited amount");
    ...
    function stake(uint256 amount)
        ...
        checkIfNewUser()
    {
        require(!pause, "MigratablePUSDTransmuterV2: emergency pause enabled");
    ...
    function transmute() public runPhasedDistribution() updateAccount(msg.sender) {
        ...
        require(pendingz > 0, "MigratablePUSDTransmuterV2: need to have pending in bucket");
        ...
        checkIfNewUser()
    {
        ...
        require(
            ...
            "MigratablePUSDTransmuterV2: !overflow"
        );
    ...
    function setPendingGovernance(address _pendingGovernance) external onlyGov() {
        require(_pendingGovernance != ZERO_ADDRESS, "MigratablePUSDTransmuterV2: 0 gov");
    ...
    function acceptGovernance() external  {
        require(msg.sender == pendingGovernance, "MigratablePUSDTransmuterV2: !pendingGovernance");
    ...
    function setActiveVault(IVaultAdapterV2WithIndirection _adapter) external onlyGov() {
        require(_adapter != IVaultAdapterV2WithIndirection(ZERO_ADDRESS), "MigratablePUSDTransmuterV2: active vault address cannot be 0x0.");
        require(address(_adapter.token()) == token, "MigratablePUSDTransmuterV2.vault: token mismatch.");

        _vaults.push(PolyquityVaultV2WithIndirection.Data({
            adapter: _adapter,
            totalDeposited: 0
        }));
    ...
    function recallAllFundsFromVault(uint256 _vaultId) external {
        require(pause && (msg.sender == governance || msg.sender == sentinel), "MigratablePUSDTransmuterV2: not paused, or not governance or sentinel");
    ...
    function _recallAllFundsFromVault(uint256 _vaultId) internal {
        PolyquityVaultV2WithIndirection.Data storage _vault = _vaults.get(_vaultId);
    ...
    function _plantOrRecallExcessFunds() internal {
        ...
        if (bal > plantableThreshold.add(marginVal)) {
            ...
            PolyquityVaultV2WithIndirection.Data storage _activeVault = _vaults.last();
    ...
    function setSentinel(address _sentinel) external onlyGov() {
        require(_sentinel != ZERO_ADDRESS, "MigratablePUSDTransmuterV2: sentinel address cannot be 0x0.");
        sentinel = _sentinel;
    ...
    function setPause(bool _pause) external {
        require(msg.sender == governance || msg.sender == sentinel, "MigratablePUSDTransmuterV2: !(gov || sentinel)");
        pause = _pause;
    ...
    function setRewards(address _rewards) external onlyGov() {
        ...
        require(_rewards != ZERO_ADDRESS, "MigratablePUSDTransmuterV2: rewards address cannot be 0x0.");
    ...
    function migrateFunds(address migrateTo) external onlyGov() {
        require(migrateTo != address(0), "MigratablePUSDTransmuterV2: cannot migrate to 0x0");
        require(pause, "MigratablePUSDTransmuterV2: set pause first");
    ...
    function getVaultAdapter(uint256 _vaultId) external view returns (IVaultAdapterV2WithIndirection) {
        PolyquityVaultV2WithIndirection.Data storage _vault = _vaults.get(_vaultId);
