...
function _getProtectionPayload(
    ...
    (
        ...
        protectionPayload.swapModule,
        protectionPayload.data
...
function _flashLoan(
    ...
    _lendingPool.flashLoan(
        ...
        abi.encode(
            ...
            _protectionPayload.onBehalfOf,
            _protectionPayload.swapModule,
            _protectionPayload.data
        ),
        0
    );
}
...
function _isPositionSafe(
    ...
    uint256 _discrepancyPercent,
    uint256 _wantedHealthFactor
) pure {
    uint256 discrepancy = (_wantedHealthFactor * _discrepancyPercent) / 100;
...
function _swap(
    ...
    SafeERC20.safeTransfer(_inputToken, _swapModule, _inputAmount);
    bytes memory payload =
        abi.encodeWithSelector(
            IHandler.handle.selector,
            _inputToken,
            _outputToken,
            _inputAmount,
            _minReturn,
    ...
    (bool success, bytes memory returnsData) = _swapModule.call(payload);
    if (!success)
        GelatoBytes.revertWithError(
            returnsData,
            "ProtectionAction.swap: Failed."
        );
    receivedAmt = abi.decode(returnsData, (uint256));
}
...
contract ProtectionAction is FlashLoanReceiverBase, IAction, Ownable {
    uint256 public discrepancyInPercent;
    ...
    constructor(
        ...
        uint256 _discrepancyInPercent
    ) FlashLoanReceiverBase(_addressProvider) {
        _aaveServices = __aaveServices;
        discrepancyInPercent = _discrepancyInPercent;
    }

    /// @dev Set discrepancy of how far the final HF can be to the one wanted.
    function setDiscrepancy(uint256 _discrepancyInPercent) external onlyOwner {
        discrepancyInPercent = _discrepancyInPercent;
    }
        ...
        returns (bool)
    {
        (
            ...
            abi.decode(
                ...
                (bytes32, address, uint256, uint256, address, address, bytes)
            );
        ...
        uint256 fees = amtOfColToWithdraw / 1000;

        _withdrawCollateral(
            LENDING_POOL,
            address(this),
            colToken,
            amtOfColToWithdraw + fees,
            onBehalfOf
        );

        /// @notice Transfer Fees

        _transferFees(colToken, fees);
        ...
        _swap(
            ...
            IERC20(colToken).balanceOf(address(this)),
