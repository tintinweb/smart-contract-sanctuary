...
contract FlagTest is ERC721, Ownable {
    using Counters for Counters.Counter;

    uint256 public engineerLevel = 0;
    Counters.Counter private _tokenIdCounter;
    string public flagSvg;

    constructor(string memory name, string memory symbol, uint _level) ERC721(name, symbol) {
        engineerLevel = _level;
    }

    function init(string memory _flagSvg) onlyOwner external {
        flagSvg = _flagSvg;
        renounceOwnership();
    }
    ...
    function tokenURI(uint256 tokenId) public view override(ERC721) returns (string memory) {
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Flag #', Strings.toString(tokenId), '", "description": "it is a test nft', Strings.toString(engineerLevel) ,'", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(flagSvg)), '"}'))));
