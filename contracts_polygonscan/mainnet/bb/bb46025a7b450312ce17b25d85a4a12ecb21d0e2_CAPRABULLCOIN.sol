...
contract CAPRABULLCOIN is ERC20, Pausable, ERC20Detailed {
    
	address teamWallet = 0xbc6682015fD57a1CFEb9b8b7A0d0dF63bF139881;
    address presaleWallet = 0xa50c82A7fEF5583b5B090b1648457621bb12510E;
    address partnerWallet = 0xabE37EC14D0aD9492b1F37f8B67109654f83d7fD;
    address airdropWallet = 0xE3219a4047DA7D355cE4C5bF625064deb0974Eb6;
	...
	constructor() public ERC20Detailed("CAPRABULLCOIN", "CBC", 9) {  
	        
       
        totalCoins = 10000000000 * 10 ** uint256(decimals());
        _mint(owner(), totalCoins); // total supply fixed at 10 billion coins
        
        ERC20.transfer(teamWallet, 2000000000 * 10 ** uint256(decimals()));  
        ERC20.transfer(presaleWallet, 3000000000 * 10 ** uint256(decimals()));
        ERC20.transfer(partnerWallet, 2000000000 * 10 ** uint256(decimals()));
        ERC20.transfer(airdropWallet, 1000000000 * 10 ** uint256(decimals()));
