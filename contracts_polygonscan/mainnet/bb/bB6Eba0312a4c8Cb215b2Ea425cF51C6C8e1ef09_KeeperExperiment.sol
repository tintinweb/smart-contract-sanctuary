...
contract KeeperExperiment is AccessControlledAndUpgradeable {
  ...
  function wasLastUpdateOlderThanThreshold(uint32 marketIndex) internal view returns (bool) {
  ...
  function isPriceChangeOverThreshold(uint32 marketIndex) internal view returns (bool) {
  ...
  function isPriceChangeOverThresholdNotView(uint32 marketIndex) internal returns (bool, int256) {
    int256 currentAssetPrice = ILongShort(longShortAddress).assetPrice(marketIndex);
    IOracleManager oracle = IOracleManager(
      ILongShort(longShortAddress).oracleManagers(marketIndex)
    );
    ...
    return (percentChange > percentChangeThresholdUnsigned || -percentChange > percentChangeThresholdUnsigned, percentChange);
  }

  function areTherePendingNextPriceActions(uint32 marketIndex) internal view returns (bool) {
    return
