...
import "../PolyquityLPStrategy.sol";

contract StrategyPolyquity_PUSD_USDC is PolyquityLPStrategy {

    address pusd_usdc_diff;

    function initializeStrategy(
        address _store,
        address _vault
    ) public initializer {
        address farmContract = address(0x43D53FD10fd38207815311bd09eb6aE94F7Ab643);
        address pusd_usdc_lp = address(0xc30D6Bc42911Aa21A63E51c7121B33b3E65cC3c4);
        address pusd = address(0x9aF3b7DC29D3C4B1A5731408B6A9656fA7aC3b72);
        ...
        routes[pusd] = [pyq, usdc, pusd];
        routes[usdc] = [pyq, usdc];
    }
}
...
/// @title Polyquity LP Strategy
/// @author Chainvisions
/// @notice Strategy for Polyquity LPs.

contract PolyquityLPStrategy is BaseUpgradeableStrategy {
    ...
    */
    function doHardWork() external onlyNotPausedInvesting restricted {
        if(IUnipool(rewardPool()).earned(address(this)) > 0) {
            IUnipool(rewardPool()).claimReward();
        }
        _liquidateReward();
    ...
    */
    function investedUnderlyingBalance() external view returns (uint256) {
        ...
        return (IUnipool(rewardPool()).balanceOf(address(this)) + IERC20(underlying()).balanceOf(address(this)));
    }
    ...
    */
    function withdrawAllToVault() public restricted {
        if(rewardPool() != address(0)) {
            if(IUnipool(rewardPool()).earned(address(this)) > 0) {
                IUnipool(rewardPool()).withdrawAndClaim();
            } else {
                IUnipool(rewardPool()).withdraw(IUnipool(rewardPool()).balanceOf(address(this)));
            }
        }
        _liquidateReward();
    ...
    */
    function withdrawToVault(uint256 amount) public restricted {
        ...
        if(amount > IERC20(underlying()).balanceOf(address(this))){
            ...
            IUnipool(rewardPool()).withdraw(Math.min(IUnipool(rewardPool()).balanceOf(address(this)), needToWithdraw));
        }
    ...
    */
    ...
    function _investAllUnderlying() internal onlyNotPausedInvesting {
        ...
        if(underlyingBalance > 0) {
            ...
            IUnipool(rewardPool()).stake(underlyingBalance);
        }
    }
    ...
    */
    function _liquidateReward() internal {
        ...
        if(routes[token0].length > 1) {
            ...
            IUniswapV2Router02(sushiRouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(toToken0, 0, routes[token0], address(this), (block.timestamp + 600));
        ...
        if(routes[token1].length > 1) {
            ...
            IUniswapV2Router02(sushiRouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(toToken1, 0, routes[token1], address(this), (block.timestamp + 600));
        ...
        IUniswapV2Router02(sushiRouter).addLiquidity(token0, token1, token0Amount, token1Amount, 0, 0, address(this), (block.timestamp + 600));
    }
