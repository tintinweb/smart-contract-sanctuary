...
// Safe test transfer low-level function, just in case if rounding error causes the pool to do not have enough TESTs,
// or if a loop error reverts the standard transfer functions

contract RescueTokens is OwnableUpgradeSafe {
...
// Test Vault distributes fees equally amongst staked pools
// Have fun reading it. Hopefully it's bug-free. God bless.
contract TestVaultV2 is OwnableUpgradeSafe, RescueTokens {
    using SafeMath for uint256;
    ...
    // Update the given pool's TESTs allocation point. Can only be called by the owner.
    ...
    // ----
    // Function that adds pending rewards, called by the TEST token.
    // ----
    uint256 private testBalance;
    function addPendingRewards(uint256 _) public {
        uint256 newRewards = test.balanceOf(address(this)).sub(testBalance);

        if(newRewards > 0) {
            testBalance = test.balanceOf(address(this)); // If there is no change the balance didn't change
    ...
    function updatePool(uint256 _pid) internal returns (uint256 testRewardWhole) {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        testRewardWhole = pendingRewards // Multiplies pending rewards by allocation point of this pool and then total allocation
        ...
        uint256 testRewardFee = testRewardWhole.mul(DEV_FEE).div(10000);
        uint256 testRewardToDistribute = testRewardWhole.sub(testRewardFee);

        pending_DEV_rewards = pending_DEV_rewards.add(testRewardFee);

        pool.accTestPerShare = pool.accTestPerShare.add(
            testRewardToDistribute.mul(1e12).div(tokenSupply)
        );
    ...
    // Deposit  tokens to TestVault for TEST allocation.
    ...
    function depositFor(address depositFor, uint256 _pid, uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.accTestPerShare).div(1e12); /// This is deposited for address
    ...
    // Withdraw  tokens from TestVault.
    ...
    function _withdraw(uint256 _pid, uint256 _amount, address from, address to) internal {
        ...
        massUpdatePools();
        updateAndPayOutPending(_pid,  pool, user, from); // Update balances of from this is not withdrawal but claiming TEST farmed
    ...
    function updateAndPayOutPending(uint256 _pid, PoolInfo storage pool, UserInfo storage user, address from) internal {
        ...
        uint256 pending = user
            .amount
            .mul(pool.accTestPerShare)
            .div(1e12)
            .sub(user.rewardDebt);

        if(pending > 0) {
            safeTestTransfer(from, pending);
        }
    ...
    function safeTestTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > testBal) {
            ...
            testBalance = test.balanceOf(address(this));
    ...
    function transferDevFee() public {
        ...
        uint256 testBal = test.balanceOf(address(this));
        if (pending_DEV_rewards > testBal) {
            test.transfer(devaddr, testBal);
            testBalance = test.balanceOf(address(this));
        } else {
            test.transfer(devaddr, pending_DEV_rewards);
            testBalance = test.balanceOf(address(this));
        }
