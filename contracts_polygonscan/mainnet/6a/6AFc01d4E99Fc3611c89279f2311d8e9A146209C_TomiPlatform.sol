...
contract TomiPlatform is Ownable {
    ...
    function _addLiquidity(
        ...
        require(ITomiConfig(CONFIG).checkPair(tokenA, tokenB), 'TOMI PLATFORM : ADD LIQUIDITY PAIR CONFIG CHECK FAIL');
    ...
    function _swap(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            (uint256 reserve0, uint256 resereve1, ) = ITomiPair(pair).getReserves();
    ...
    function migrateLiquidity(
        ...
        for (uint256 i = 0; i < users.length; i++) {
            uint256 liquidity = ITomiPair(pair).balanceOf(users[i]);
            if (liquidity > 0) {
...
library TomiSwapLibrary {
    ...
    function pairFor(
        ...
        bytes32 rawAddress = keccak256(
            abi.encodePacked(bytes1(0xff), factory, salt, ITomiFactory(factory).contractCodeHash())
        );
    ...
    function getReserves(
        ...
        (uint256 reserve0, uint256 reserve1, ) = ITomiPair(pairFor(factory, tokenA, tokenB)).getReserves();
