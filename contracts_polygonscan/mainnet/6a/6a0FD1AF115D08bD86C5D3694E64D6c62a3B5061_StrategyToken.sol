...
abstract contract BaseStrategy is Ownable, ReentrancyGuard, Pausable {
    ...
    function withdraw(address _userAddress, uint256 _wantAmt) external onlyOwner nonReentrant returns (uint256) {
        ...
        if (_wantAmt > wantAmt) {
            uint256 balWithdraw = _wantAmt.sub(wantAmt);
            _vaultWithdraw(balWithdraw);
            uint256  wantAmtAfter = IERC20(wantAddress).balanceOf(address(this));
            uint256 diff = wantAmtAfter.sub(wantAmt);
            if (diff < balWithdraw) {
                _wantAmt = wantAmt.add(diff);
            }
        }
