...
interface IE11{
    function tokenToShares(uint _amount) view external returns(uint);
}
...
// BTC-USDC is specific for BTC-USDC pool in Wex.
contract BtcUsdcBigFoot is Ownable, ReentrancyGuard, Bigfoot {
  /// @notice Libraries
  ...
  constructor() public {
    vaultAddress = 0x25B8b36711Ff3f26F63398036a111710b762D886;
    operator = 0xDF6Dae831353Efe17aB3998aD053d476B1B676E5;
    liqStrat = Strategy(0x558Bb385c966886F6dfb28A3e1D234d99Fa8b572);
    addStrat = Strategy(0xf4bbbe8AbD5271f47422eC13a1EA2032Cd773603);
    okStrats[address(addStrat)] = true;
    okStrats[address(liqStrat)] = true;
    lpToken = IUniswapV2Pair(address(VaultInterface(vaultAddress).token()));
    lpToken.approve(address(vaultAddress), uint(-1)); // 100% trust in the vault
  }

/*  ONLY USED IN TEST
  function renounceCritical() external onlyOwner{
      critical = false;
  }

  function setCriticalStuff(address _vaultAddress, address _operator, address _liqStrat, address _addStrat) external onlyOwner{
    require(critical, "don't test in prod KK");
    ...
    okStrats[address(liqStrat)] = true;
    lpToken = IUniswapV2Pair(address(VaultInterface(vaultAddress).token()));
    lpToken.approve(address(vaultAddress), uint(-1)); // 100% trust in the staking pool
  }
...
*/
  ...
  function work(
    ...
    // 1. Convert this position back to 11xxx tokens.
    _removeShare(id);
    // 2. Perform the worker strategy; sending vault tokens + usdc; expecting Vault tokens + usdc.
    ...
    vaultAddress.safeTransfer(strat, IERC20(vaultAddress).balanceOf(address(this)));
    usdc.safeTransfer(strat, IERC20(usdc).balanceOf(address(this)));
    Strategy(strat).execute(user, debt, ext);
    // 3. Add LP tokens back to the farming pool.
    _addShare(id);
    // 4. Return any remaining usdc back to the operator.
    usdc.safeTransfer(msg.sender, IERC20(usdc).balanceOf(address(this)));
  }
  ...
  function health(uint id) external view returns (uint) {
    ...
    (uint totalUsdc, uint totalBtc) = lpToken.token0() == usdc ? (r0, r1) : (r1, r0);
    // 3. Convert the position's LP tokens to the underlying assets.
    uint userUsdc = lpBalance.mul(totalUsdc).div(lpSupply);
    uint userBtc = lpBalance.mul(totalBtc).div(lpSupply);
    // 4. Convert all farming tokens to BNB and return total BNB.
    return
      getMktSellAmount(userBtc, totalBtc.sub(userBtc), totalUsdc.sub(userUsdc)).add(
        userUsdc
      );
  }
  ...
  function liquidate(uint id) external onlyOperator nonReentrant {
    ...
    _removeShare(id);
    vaultAddress.safeTransfer(address(liqStrat), IERC20(vaultAddress).balanceOf(address(this)));
    liqStrat.execute(address(0), 0, abi.encode(0));
    // 2. Return all available BNB back to the operator.
    uint wad = IERC20(usdc).balanceOf(address(this));
    usdc.safeTransfer(msg.sender, wad);
  ...
  function _addShare(uint id) internal {
    ...
    if (balance > 0) {
      shares[id] = shares[id].add(balance);
      totalShare = totalShare.add(balance);
      emit AddShare(id, balance);
    }
  }

  /// @dev Internal function to remove shares of the ID and convert to outstanding vault tokens.
  function _removeShare(uint id) internal {
    uint share = shares[id];
    if (share > 0) {
      //todo check uint balance = lpToBalance(share);
  ...
  function recover(
    ...
    //TODO disable claim ELE and E11 tokens
