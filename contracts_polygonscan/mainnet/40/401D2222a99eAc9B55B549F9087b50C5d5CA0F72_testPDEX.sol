...
contract testPDEX is ERC20, Ownable {
    ...
    modifier onlyPair() {
        require(
            msg.sender == empireWmPair,
            "Empire::onlyPair: Insufficient Privileges"
        );
        _;
    }

    constructor() public ERC20("testPDEX", "testPDEX") Ownable() {
    }

    function beginLGE(IEmpireFactory _factory) external onlyOwner() {
        factory = _factory;
        end = block.timestamp + 30 minutes;
        PairType pairType =
            address(this) < WMATIC
                ? PairType.SweepableToken1
                : PairType.SweepableToken0;
        empireWmPair = _factory.createPair(WMATIC, address(this), pairType, 0);
    }
    ...
    function complete() external onlyOwner() {
        ...
        _mint(empireWmPair, 5000 * 1 ether); //  50% / 5000 for LGE

        IERC20(WMATIC).safeTransfer(empireWmPair, _totalRaised);

        IEmpirePair(empireWmPair).mint(address(this));
    ...
    function claim() external {
        ...
        IERC20(empireWmPair).safeTransfer(
            msg.sender,
            IERC20(empireWmPair)
    ...
    function sweep(uint256 amount, bytes calldata data) external onlyOwner() {
        IEmpirePair(empireWmPair).sweep(amount, data);
    }
    ...
    function unsweep(uint256 amount) external onlyOwner() {
        IERC20(WMATIC).approve(empireWmPair, amount);
        IEmpirePair(empireWmPair).unsweep(amount);
    }
    ...
    function transferFrom(
        ...
        uint256 burned;
        if (from != empireWmPair && to != empireWmPair) {
    ...
    function transfer(address to, uint256 amount)
        ...
        returns (bool)
    {
        if (msg.sender != empireWmPair && to != empireWmPair) {
