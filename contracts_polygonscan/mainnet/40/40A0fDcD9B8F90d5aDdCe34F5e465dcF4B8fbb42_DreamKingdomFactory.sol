...
contract DreamStrategy is Ownable, ReentrancyGuard {
    ...
    function initialize(
        ...
        PRECISION_FACTOR = uint256(10 ** 18); 
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            ...
            uint256 adjustedTokenPerShare = accTokenPerShare.add(tokenReward.mul(1e18).div(stakedTokenSupply));
            return user.amount.mul(adjustedTokenPerShare).div(1e18).sub(user.rewardDebt);
        } else {
    ...
    function _updatePool() internal {
        ...
        uint256 multiplier = _getMultiplier(lastRewardBlock, block.number);
        uint256 tokenReward = multiplier.mul(rewardPerBlock);
        accTokenPerShare = accTokenPerShare.add(tokenReward.mul(1e18).div(stakedTokenSupply));
        lastRewardBlock = block.number;
    }
