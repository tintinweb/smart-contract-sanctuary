...
contract NftSales is ERC721, Ownable, ReentrancyGuard {
    ...
    uint256 public staticPrice = 15000000000000000000; // 0.15 ETH
    ...
    function setStaticPrice(uint256 _price) external onlyAdmin {
        require(!whiteSaleActive, "error: whitesale has already started");
        require(!publicSaleActive, "error: public sale started");
        staticPrice = _price;
    }
    ...
    function mintWhiteSale(uint amount_) public payable {
        require(isWhiteListed[_msgSender()] && whiteSaleActive,"Whitelist sales not active or address not whitelisted");
        ...
        require(msg.value >= amount_ * staticPrice, "Error: ETH send is less then required");
        require(amount_ <= maxNftsPerMint, "Max NFTs per mint would be exceeded");
        require(balanceOf(_msgSender()) + amount_ <= maxNftPerWhitelistWalletAddress, "Error: user cannot mint more NFTs to a whitelist wallet");
