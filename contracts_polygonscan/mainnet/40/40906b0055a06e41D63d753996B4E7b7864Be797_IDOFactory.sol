...
/**
* @dev if updating an IDO and an agreement, then the new IDO implementation needs a new IIDOAgreement interface to work with, it can't use the old one.
**/
contract IDOFactory is Ownable {
    //address[] public IDOImplementation;
    //address[] public AgreementImplemenation;
    ...
    constructor(address _gfi, address _tierManager) {
        gfi = _gfi;
        tierManager = _tierManager;
    }

    function updateSharedVariables(address _swapFactory, address _swapRouter, address _feeManager, address _governance, address _tierManager, uint _slippage, address _farmFactory, address _compounderFactory) external onlyOwner{
    ...
    function createIDO(address agreement) external {
        ...
        bytes32 salt = keccak256(abi.encodePacked(Agreement.IDOToken(), Agreement.saleToken(), Agreement.totalAmount()));
        ...
        //IERC20(Agreement.IDOToken()).transferFrom(msg.sender, IDOClone, Agreement.totalAmount());
        ...
        onlyOwner
    {
        ...
        require(productList[record].IDOImplementation == address(0), 'Gravity Finance: IDOType already exists');
    ...
    function deployFarm(address depositToken, address rewardToken, uint amount, uint blockReward, uint start, uint end, uint bonusEnd, uint bonus) external onlyIDO{
        //create a farm
        IERC20(gfi).approve(farmFactory, amount);
        IFarmFactory(farmFactory).createFarm(depositToken, rewardToken, amount, blockReward, start, end, bonusEnd, bonus);
    }
