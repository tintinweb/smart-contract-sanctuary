...
contract CPredictionFactory is Ownable {
    using SafeERC20 for IERC20;
    ...
    mapping(uint256 => CPrediction) public predictions;
     ...
     * @dev Start genesis round
     */
    function genesisStartRound(uint256 _index) external onlyAdminOrOperator {
     ...
     * @dev Start the next round n, lock price for round n-1, end round n-2
     */
    function executeRound(uint256 _index, int256 price) external onlyAdminOrOperator {
        predictions[_index].executeRound(price);
    }

    /**
     * @dev Bet bear position
     */
    function betBear(uint256 _index, uint256 epoch, uint256 amount) external payable notContract {
        CPrediction pred = predictions[_index];
        if(address(betTokens[_index]) == address(0))
        {
            pred.betBear{value:msg.value}(epoch, msg.sender, msg.value);
        }
        else{
     ...
     * @dev Bet bull position
     */
    function betBull(uint256 _index, uint256 epoch, uint256 amount) external payable notContract {
        CPrediction pred = predictions[_index];
        if(address(betTokens[_index]) == address(0))
        {
            pred.betBull{value:msg.value}(epoch, msg.sender, msg.value);
        }
        else{
            betTokens[_index].safeTransferFrom(msg.sender, address(pred), amount);
    ...
    function betDouble(uint256 _index, uint256 epoch, uint256 bullAmount, uint256 bearAmount, address bullBetter, address bearBetter) external payable notContract {
        CPrediction pred = predictions[_index];
        if(address(betTokens[_index]) == address(0))
        {
            require(msg.value == bullAmount + bearAmount, 'payable amount != bull+bear');
            if (bullAmount > 0)
            {
                pred.betBull{value:bullAmount}(epoch, bullBetter, bullAmount);
            }
            if (bearAmount > 0)
            {
                pred.betBear{value:bearAmount}(epoch, bearBetter, bearAmount);
            }
        }
        else{
            betTokens[_index].safeTransferFrom(msg.sender, address(pred), bullAmount+bearAmount);
            if (bullAmount > 0)
            {
                pred.betBull(epoch, bullBetter, bullAmount);
            }
            if (bearAmount > 0)
            {
                pred.betBear(epoch, bearBetter, bearAmount);
            }
        }
    }
    ...
    function recoverAmount(uint256 _index, uint256 _amount) external onlyAdmin {
        predictions[_index].recoverAmount(msg.sender, _amount);
    }

    function addAllowedContract(address _allowedContract) external onlyAdmin {
        allowedContracts[_allowedContract] = true;
    }

    function removeAllowedContract(address _allowedContract) external onlyAdmin {
        allowedContracts[_allowedContract] = false;
    }
