...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 accDeliriumPerShare;   // Accumulated DELIRIUMs per share, times 1e18. See below.
        uint16 depositFeeBP;      // Deposit fee in basis points
        uint256 lpSupply;
    }

    uint256 public deliriumMaximumSupply = 500 * (10 ** 3) * (10 ** 18); // 500,000 delirium
    uint256 public MAX_EMISSION_RATE = 10 * (10 ** 18); // 10
    // The DELIRIUM TOKEN!
    ...
    function pendingDelirium(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && pool.lpSupply != 0 && totalAllocPoint > 0) {
            ...
            accDeliriumPerShare = accDeliriumPerShare + ((deliriumReward * 1e18) / pool.lpSupply);
        }

        return ((user.amount * accDeliriumPerShare) /  1e18) - user.rewardDebt;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accDeliriumPerShare = pool.accDeliriumPerShare + ((deliriumReward * 1e18) / pool.lpSupply);
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = ((user.amount * pool.accDeliriumPerShare) / 1e18) - user.rewardDebt;
            if (pending > 0) {
        ...
        user.rewardDebt = (user.amount * pool.accDeliriumPerShare) / 1e18;
    ...
    function setEmissionRate(uint256 _deliriumPerBlock) external onlyOwner {
        require(_deliriumPerBlock > 0);
        require(_deliriumPerBlock < MAX_EMISSION_RATE);
