...
contract FlashBot is Ownable {
    ...
    // WETH on Ethereum or WBNB on BSC or WMATIC on Polygon
    ...
    /// The callback function on different DEXes are not same, so use a fallback to redirect to uniswapV2Call
    ...
    function withdraw() external {
        ...
        for (uint256 i = 0; i < baseTokens.length(); i++) {
            ...
            if (balance > 0) {
                // do not use safe transfer here to prevent reverts by any shitty token
    ...
    function removeBaseToken(address token) external onlyOwner {
        ...
        if (balance > 0) {
            // do not use safe transfer here to prevent revert by any shitty token
            IERC20(token).transfer(owner(), balance);
        }
        ...
        )
    {
        ...
        require(pool0Token0 == pool1Token0 && pool0Token1 == pool1Token1, 'Requires Same Token Pair');
        require(baseTokensContains(pool0Token0) || baseTokensContains(pool0Token1), 'No Base Token in the Given Pair');
