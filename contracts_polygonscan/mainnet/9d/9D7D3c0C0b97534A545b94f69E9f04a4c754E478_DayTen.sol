...
contract DayTen is OwnableUpgradeable, ERC721Upgradeable, ERC721EnumerableUpgradeable, ERC2771ContextUpgradeable {
    ...
    address public constant ArtventCalendar = 0x356FB8695842273BD8855De105C00dd8F12E7696;
    ...
    function initialize(address trustedForwarder) public initializer {
        __Ownable_init();
        __ERC721_init("DayTen", "D10");
        __ERC2771Context_init(trustedForwarder);
        _tokenIds.increment();
     }

    function mintFree() public {
        require(sale, "Drop not open yet");
        ERC721EnumerableUpgradeable nfts = ERC721EnumerableUpgradeable(ArtventCalendar);
    ...
    function setTrustedForwarder(address trustedForwarder) public onlyOwner {
        setTrustedForwarderInternal(trustedForwarder);
    }
    ...
    function addProxy(address[] memory addresses) public onlyOwner {
        for(uint i = 0; i < addresses.length; i++) {
            proxyAddresses[addresses[i]] = true;
        }
    }

    function removeProxy(address[] memory addresses) public onlyOwner {
        for(uint i = 0; i < addresses.length; i++) {
            proxyAddresses[addresses[i]] = false;
        }
    }
    ...
    function isApprovedForAll(
        ...
        if(proxyAddresses[_operator] == true)
        {
            return true;
        }
...
abstract contract ERC2771ContextUpgradeable is Initializable, ContextUpgradeable {
    ...
    function setTrustedForwarderInternal(address trustedForwarder) internal {
        _trustedForwarder = trustedForwarder;
    }
