...
interface ICurvePool {
    ...
    function remove_liquidity(
        uint256 _amount,
        uint256[5] calldata min_mint_amount,
        address _receiver
    ) external;
...
contract BaseImpulseMultiStrategy is IImpulseMultiStrategy, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    uint256 public constant UNDERLYING_COUNT = 5;
    ...
    function wantPriceInUsd(uint256 _wantAmt) external view virtual override returns (uint256) {
        address crvPool = ICurvePool(targetPoolProtocol).pool();
        uint256 wantPrice = IPool(crvPool).get_virtual_price();

        return (_wantAmt * wantPrice) / 10**18;
    }
    ...
    function _swapAllUnderlyingToWant() internal virtual returns (uint256) {
        ...
        ICurvePool(targetPoolProtocol).add_liquidity(amounts, 0);
    ...
    function _swapAllWantTolUnderlying(address _receiver) internal virtual {
        ...
        if (wantAmount > 0) {
            ...
            ICurvePool(targetPoolProtocol).remove_liquidity(wantAmount, amountsAtricrypto, _receiver);
        }
    }

    function _swapAllWantToOneUnderlying(address _receiver, address _underlying) internal {
        uint256 index;
        for (uint256 i = 0; i < underlyingTokens.length; i++) {
        ...
        if (wantAmount > 0) {
            IERC20(wantToken).safeApprove(targetPoolProtocol, wantAmount);
            ICurvePool(targetPoolProtocol).remove_liquidity_one_coin(wantAmount, index, 0, _receiver);
        }
    }
    ...
    function _verifySetup() internal virtual {
        ...
        for (uint256 i = 0; i < underlyingTokens.length; i++) {
            ...
            require(ICurvePool(targetPoolProtocol).underlying_coins(i) == address(underlyingTokens[i]), "Wrong underlying");
        }
    }
...
contract ImpulseCurveAtricrypto3 is BaseImpulseMultiStrategy {
    using SafeERC20 for IERC20;
     ...
     * First element is DAI amount.
     * Second element is USDC amount.
     * Third element us USDT amount.
     * Fourth element is BTC.
     # Fifth element is ETH.
     */
    function wantPriceInUnderlying(uint256 _wantAmt) public view override returns (uint256[] memory) {
        ...
        // Calculate amounts of first 3 coins(DAI, USDC, USDT).
        uint256 aaveLPAmount = (_wantAmt * IPool(crvPool).balances(0)) / totalSupply;
        address aaveLP = IPool(crvPool).coins(0);
        address aaveLPPool = address(0x445FE580eF8d70FF569aB36e80c647af338db351); // Address of pool for aaveLP token
        uint256 aaveLPTotalSupply = IERC20(aaveLP).totalSupply();
        for (uint256 i = 0; i < 3; i++) {
            uint256 balance = IPool(aaveLPPool).balances(i);
            result[i] = (aaveLPAmount * balance) / aaveLPTotalSupply;
        }

        // Calculate amounts of the rest 2 coins(WBTC, WETH).
        for (uint256 i = 1; i < 3; i++) {
            uint256 balance = IPool(crvPool).balances(i);
            result[i + 2] = (_wantAmt * balance) / totalSupply;
        }
        return result;
    }
    ...
    function wantPriceInUsd(uint256 _wantAmt) external view virtual override returns (uint256) {
        ...
        path[1] = address(underlyingTokens[1]);
        // Price of WBTC and WETH
        for (uint256 i = 3; i < 5; i++) {
            path[0] = address(underlyingTokens[i]);
             uint256 priceInUsdc = IUniswapV2Router01(router).getAmountsOut(
                underlyingAmounts[i],
                path
            )[1];
            priceInUsdc *= 10**12; // Since USDC has 6 decimals
            wantPrice += priceInUsdc;
        }
