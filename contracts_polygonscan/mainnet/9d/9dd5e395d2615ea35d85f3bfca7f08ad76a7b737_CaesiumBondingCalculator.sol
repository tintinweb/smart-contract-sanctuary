...
// File: contracts/CaesiumBondingCalculator.sol
...
contract CaesiumBondingCalculator is ICaesiumBondingCalculator {
    ...
    function markdown( address _pair ) external view override returns ( uint ) {
        ...
        uint reserve;
        if ( IUniswapV2Pair( _pair ).token0() == CAES ) {
            reserve = reserve1;
        } else {
            reserve = reserve0;
        }
        return reserve.mul( 2 * ( 10 ** IERC20( CAES ).decimals() ) ).div( getTotalValue( _pair ) );
    }
}