...
contract GakuenLoot is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function pluckWithPrefix(uint256 tokenId, string memory keyPrefix, string[] memory sourceArray) internal view returns (string memory) {
        uint256 rand = random(string(abi.encodePacked(keyPrefix, toString(tokenId))));
        string memory output = sourceArray[rand % sourceArray.length];
        bool prefix = rand % 4 == 0;
        if (prefix) {
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        parts[1] = string(abi.encodePacked(getGrade(tokenId), "-", getClass(tokenId)));

        parts[2] = '</text><text x="10" y="40" class="base">';

        parts[3] = string(abi.encodePacked(getLastName(tokenId), " ", getFirstName(tokenId)));

        parts[4] = '</text><text x="10" y="60" class="base">';

        parts[5] = getGuild(tokenId);

        parts[6] = '</text><text x="10" y="80" class="base">';

        parts[7] = getClub(tokenId);
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Student #', toString(tokenId), '", "description": "Gakuen Loot is randomized high school character generated and stored on chain. Stats, images, and other functionality are intentionally omitted for others to interpret. Feel free to use the character in any way you want.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
    ...
    function claim(uint256 tokenId) public nonReentrant {
        require(tokenId >= 1 && tokenId <= 300, "Token ID invalid");
    ...
    constructor() ERC721("Blockchain Gakuen Loot", "GLOOT") Ownable() {
        claim(1);
    }
}
