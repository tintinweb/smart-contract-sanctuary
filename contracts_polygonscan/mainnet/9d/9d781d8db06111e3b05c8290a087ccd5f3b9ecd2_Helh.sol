...
/*
  ...
  _    _      _ _          _              __ _                            
 | |  | |    | | |        (_)            / _(_)                           
 | |__| | ___| | |__   ___ _ _ __ ___   | |_ _ _ __   __ _ _ __   ___ ___ 
 |  __  |/ _ \ | '_ \ / _ \ | '_ ` _ \  |  _| | '_ \ / _` | '_ \ / __/ _ \
 | |  | |  __/ | | | |  __/ | | | | | | | | | | | | | (_| | | | | (_|  __/
 |_|  |_|\___|_|_| |_|\___|_|_| |_| |_| |_| |_|_| |_|\__,_|_| |_|\___\___|
...
/*
 ...
 |_|  |_|\___|_|_| |_|\___|_|_| |_| |_| |_| |_|_| |_|\__,_|_| |_|\___\___|
    ...
    https://helheim.finance
*/
contract Helh is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 0 ether;
    // Initial distribution for the day 2-5 HELH-WMATIC LP -> HELH pool
    uint256 public constant INITIAL_HELH_POOL_DISTRIBUTION = 4106 ether;
    ...
    /**
     * @notice Constructs the HELH ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Helheim Finance", "HELH") {
    ...
    function _getHelhPrice() internal view returns (uint256 _helhPrice) {
        try IOracle(helhOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Helh: failed to fetch HELH price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _helhPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_helhPrice >= taxTiersTwaps[tierId]) {
    ...
    function setHelhOracle(address _helhOracle) public onlyOperatorOrTaxOffice {
        require(_helhOracle != address(0), "oracle address cannot be 0 address");
        helhOracle = _helhOracle;
    }
    ...
    /**
     * @notice Operator mints HELH to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of HELH to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentHelhPrice = _getHelhPrice();
            currentTaxRate = _updateTaxRate(currentHelhPrice);
            if (currentHelhPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_helhPool != address(0), "!_helhPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_helhPool, INITIAL_HELH_POOL_DISTRIBUTION);
