...
contract MyContract {
    ...
    function GetWethAllowance(address owner, address spender) public view returns (uint256) {
        return TokenAddr.allowance(owner, spender);
    }

    function GetMaticBalance(address account) public view returns (uint256) {
        return account.balance / 1000000000000000000;
    }

    function GetWethBalance(address Addr) public view returns (uint256) {
        return TokenAddr.balanceOf(Addr);
    }

    function TransferfromWeth(address FromAddr, uint256 value) public {
