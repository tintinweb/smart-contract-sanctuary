...
contract MasterChefStrategy is BaseStrategyMasterChef {
        ...
        )
    {
        ...
        if(_betweenReward == address(0)) {
            reward_matic_path = [_harvestedToken, WMATIC];
        }
        else {
            reward_matic_path = [_harvestedToken, _betweenReward, WMATIC];
        }
...
contract DinoSwapStrategy is MasterChefStrategy {
    ...
    constructor(
        ...
        public
        MasterChefStrategy(
            0x1948abC5400Aa1d72223882958Da3bec643fb4E5, //Chef
            _want,
            _tokenA,
            _tokenB,
            _betweenToken,
            0x7ceB23fD6bC0adD59E62ac25578270cFf1b9f619, //[DINO, WETH, WMATIC]
            _poolId,
            _strategist,
            0xAa9654BECca45B5BDFA5ac646c939C62b527D394, //DINO
            0x6AC823102CB347e1f5925C634B80a98A3aee7E03 //DinoswapV2Router
        )
    {
...
abstract contract VaultBase is IVault, Ownable, ReentrancyGuard {
    ...
    function deposit(uint256 _amount) public override nonReentrant {
        require(msg.sender == tx.origin || (contractWhitelist != address(0) && IContractWhitelist(contractWhitelist).allowed(msg.sender)), "no contracts");
    ...
    // Withdraw all tokens without caring about rewards if there's an issue with the minter/early withdrawals are required
    ...
    function setBoostHandler(address _handler) public onlyOwner {
        boostHandler = _handler; //Set boost handler to the zero address to disable it
    }

    function setContractWhitelist(address _whitelist) public onlyOwner {
