...
contract FOMOON_TEST is Context, IBEP20, Ownable {
    ...
    string  private constant _NAME = 'FOMOON_TEST';
    ...
    uint256 private _TEAM_FEE = 200; // 2% TO TEAM WALLET ( 1% for liquidity buyback and 1%)
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tFee, uint256 tBurn, uint256 tCharity) = _getTBasics(tAmount, _TAX_FEE, _BURN_FEE, _TEAM_FEE);
    ...
    function removeAllFee() private {
        if(_TAX_FEE == 0 && _BURN_FEE == 0 && _TEAM_FEE == 0) return;
