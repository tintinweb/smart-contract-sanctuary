...
// File contracts/BZCMetaCrystals.sol
...
contract BZCMetaCrystals is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    // mint one each to addresses
    function mintToAddresses(address[] memory _addresses) external onlyOwner
    {
        uint256 supply = totalSupply();
        require(supply + _addresses.length <= maxSupply, "max supply exceeded!");
