...
interface BankConfig {
  /// @dev Return minimum weth debt size per position.
  ...
  /// @dev Return the work factor for the bigfoot + weth debt, using 1e4 as denom. Revert on non-bigfoot.
  function workFactor(address bigfoot, uint debt) external view returns (uint);

  /// @dev Return the kill factor for the bigfoot + weth debt, using 1e4 as denom. Revert on non-bigfoot.
...
interface Bigfoot {
  /// @dev Work on a (potentially new) position. Optionally send weth back to Bank.
  function work(
    uint id,
    address user,
    uint debt,
  ...
  /// @dev Return the amount of weth wei to get back if we are to liquidate the position.
  function health(uint id) external view returns (uint);

  /// @dev Liquidate the given position to weth. Send all weth back to Bank.
...
contract BankEth is Initializable, ERC20, ReentrancyGuardUpgradeSafe, Governable {
  /// @notice Libraries
  ...
  function initialize(BankConfig _config) external initializer {
    ...
    lastAccrueTime = now;
    nextPositionID = 1;
    name = 'eleven interest bearing WETH';
    symbol = 'eleWETH';
    decimals = 18;
    approve();
  }
  ...
  function additionalApprove(address _token, address _spender) onlyGov external{
    require(_token != weth && _token != vault, "no rugs allowed");
  ...
  // @dev transfer from weth and/or 11unusedweth with no errors
  function safeTransferBankCurrency(address _receiver, uint _amount) internal{
    uint inside = weth.myBalance();
    if(inside>=_amount) weth.safeTransfer(_receiver, _amount);
    else {
      safeWithdrawFromFarm(_amount.sub(inside));
      weth.safeTransfer(_receiver, weth.myBalance());
    }
  }
  

  // @dev Total weth inside the contract wether on the vault or not
  function wethBalance() public view returns (uint) {
    return weth.myBalance().add(insideVaultBankCurrency());
  }
  ...
  function pendingInterest(uint msgValue) public view returns (uint) {
    if (now > lastAccrueTime) {
      ...
      uint balance = wethBalance().sub(msgValue);
  ...
  /// @dev Return the weth debt value given the debt share. Be careful of unaccrued interests.
  ...
  /// @dev Return weth value and debt of the given position. Be careful of unaccrued interests.
  ...
  /// @dev Return the total weth entitled to the token holders. Be careful of unaccrued interests.
  function totalBankCurrency() public view returns (uint) {
    return wethBalance().add(glbDebtVal).sub(reservePool);
  }
  


  /// @dev Add more weth to the bank. Hope to get some good returns.
  function deposit(uint[] calldata amounts) external accrue(0)  nonReentrant {
    uint total = totalBankCurrency();
    uint diff = transferWeth(amounts);
    uint share = total == 0 ? diff : diff.mul(totalSupply()).div(total);
    _mint(msg.sender, share);
    require(totalSupply() >= 1e15, 'deposit: total supply too low');
    depositToVault();
  }

  /// @dev Withdraw weth from the bank by burning the share tokens.
  function withdraw(uint share, uint8) external accrue(0) nonReentrant {
    uint amount = share.mul(totalBankCurrency()).div(totalSupply());
    require(amount <= wethBalance(), "Utilization too high, withdraw an smaller amount");
    _burn(msg.sender, share);
    safeTransferBankCurrency(msg.sender, amount);
    uint supply = totalSupply();
    require(supply == 0 || supply >= 1e15, 'withdraw: total supply too low');
  }
  ...
  // @dev Deposit dollars, convert to weth and stake in vault
  function transferWeth(uint[] memory amounts) internal returns(uint){
    uint bfrweth = weth.myBalance();
    if(amounts[0]>0) weth.safeTransferFrom(msg.sender, address(this), amounts[0]);
    uint aftrweth = weth.myBalance();
    return (aftrweth.sub(bfrweth));
  }
  ...
  /// @param loan The amount of weth to borrow from the pool.
  /// @param maxReturn The max amount of weth to return to the pool.
  ...
  function work(
    ...
    uint back;
    {
      uint sendWeth = transferWeth(amounts).add(loan);
      require(sendWeth <= wethBalance(), 'insufficient WETH in the bank');
      uint beforeWeth = wethBalance().sub(sendWeth);
      safeTransferBankCurrency(bigfoot, sendWeth);
      Bigfoot(bigfoot).work(id, msg.sender, debt, data);
      back = wethBalance().sub(beforeWeth);
    }
    ...
    require(glbDebtShare >= 1e15, 'remaining global debt share too small');
    require(glbDebtVal >= 1e15, 'remaining global debt value too small');
  }
  ...
  function kill(uint id) external onlyEOA accrue(0) nonReentrant {
    ...
    // 2. Perform liquidation and compute the amount of weth received.
    uint beforeWeth = wethBalance();
    Bigfoot(pos.bigfoot).liquidate(id);
    uint back = wethBalance().sub(beforeWeth);
  ...
  /// @dev Withdraw weth reserve for underwater positions to the given address.
  /// @param to The address to transfer weth to.
  /// @param value The number of weth tokens to withdraw. Must not exceed `reservePool`.
  function withdrawReserve(address to, uint value) external onlyGov nonReentrant {
    reservePool = reservePool.sub(value);
    require(value<=wethBalance(), "not enough funds");
    safeTransferBankCurrency(to, value);
  }

  
  /// @dev Reduce weth reserve, effectively giving them to the depositors.
  /// @param value The number of weth reserve to reduce.
