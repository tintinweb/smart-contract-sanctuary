...
interface IFarm {
    ...
    function userInfo(uint256 pid, address sender) external view returns (uint256, uint256);
}
...
contract Strategy_BANANA_MATIC is Ownable {
    // This strategy is for polygon network. BANANA - wMATIC pair. RewardToken = "BANANA, wMATIC"
    ...
    address public token1; // banana
    address public vault;

    // ------------> Important constants <--------------
    bool public hasPid = true;
    uint256 public pid = 0;
    address public constant lpToken = 0x034293F21F1cCE5908BC605CE5850dF2b1059aC0;
    address public constant farm = 0x54aff400858Dcac39797a81894D9920f16972D1D;
    address public router = 0xC0788A3aD43d79aa53B09c2EaCc313A787d1d607;
    ...
    function getRewardToken() public view returns (address){
        return IFarm(farm).BANANA();
    }

    function _withdrawFromFarm(uint256 _amount) internal {
        IFarm(farm).withdrawAndHarvest(pid, _amount, address(this));
    }
    
    function _getRewardsFromFarm() internal {
        IFarm(farm).harvest(pid, address(this));
    }
    ...
    function earn() public returns(uint256 _balance) {
        ...
        if(_balance > 0) {
            ...
            IFarm(farm).deposit(pid, _balance, address(this));
            emit Earn(_balance);
        }
    }
    //  -------------------------> END <---------------------------
    ...
    function _swapLPtoWMATIC(uint256 _percent) internal {
        ...
        if (_totalLP > 0) {
            ...
            _approveToken(getRewardToken(), router);
    ...
    function _getToken1Price() internal view returns (uint256) {
        ...
        path[0] = token0;
        path[1] = token1;
        return 1e36 / (_getAmountsOut(router, 50 * 1e18, path) / 50);
    }
    ...
    function _getToken0Price() internal view returns (uint256 result) {
        ...
        path[0] = token1;
        path[1] = token0;
        result = 1e36 / (_getAmountsOut(router, 50 * 1e18, path) / 50);
    }
