...
contract BaseMinter is Extractble {
  ArtBridge public bridge;
  BridgeReserve public reserve;
  ...
  modifier onlyValidMint(uint256 _id, uint256 _amount) {
    ...
    (uint256 maxMintPerTransaction, , ) = reserve.projectToParameters(_id);
    require(_amount <= maxMintPerTransaction, "exceed mint max");
    _;
  }

  constructor(address _bridge, address _reserve) {
    bridge = ArtBridge(_bridge);
    reserve = BridgeReserve(_reserve);
  }
...
contract BridgeReserve is BridgeContext {
  ...
  function setParameters(
    ...
    require(_maxMintPerTransaction <= maxSupply, "invalid mint max");
  ...
  function addMinter(uint256 _id, address _minter) external onlyOwner {
    ...
    if (projectToParameters[_id].maxMintPerTransaction == 0) {
      projectToParameters[_id].maxMintPerTransaction = 1;
    }
  }
...
contract ArtBridge is ERC721Enumerable, ERC721Burnable, BridgeContext {
  ...
  modifier onlyUnreleased(uint256 _id) {
    ...
    require(
      project.startBlock == 0 || project.startBlock > block.number,
      "project must be unreleased"
    );
    _;
  }
  ...
  function setMaxSupply(uint256 _id, uint256 _maxSupply)
    ...
    onlyOwner
  {
    require(projects[_id].maxSupply > _maxSupply, "cannot increase max supply");
  ...
  function _mint(
    ...
    require(
      projects[_id].supply + _amount <= projects[_id].maxSupply,
