...
contract OTCEngine is Ownable, ReentrancyGuard {
    ...
    event DealUpdateTrueMinMaxBalances(
        uint256 dealId,
        uint256 indexed trueMin,
        uint256 indexed trueMax,
        uint256 indexed buyableSaleBalance
    );
    ...
    function emitTrueMinMax(uint256 dealId) internal {
        uint256 saleBalance = globalDealInfos[dealId].saleBalance;

        uint256 calcAbsMinFromPerc = (saleBalance * globalDealInfos[dealId].saleMinPercentage) / 1e12;
        uint256 calcAbsMaxFromPerc = (saleBalance * globalDealInfos[dealId].saleMaxPercentage) / 1e12;
        ...
        // allows the sub-graph to ignore unbuyable deals, which have some balance.
        uint256 buyableSaleBalance = saleBalance < trueMin ? 0 : saleBalance > trueMax ? trueMax : saleBalance;

        emit DealUpdateTrueMinMaxBalances(dealId, trueMin, trueMax, buyableSaleBalance);
    }
