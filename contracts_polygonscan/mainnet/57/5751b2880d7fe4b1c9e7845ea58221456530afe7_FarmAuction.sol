...
/**
 * @title MoonwalkerSwap Farms Auctions.
...
contract FarmAuction is Ownable, ReentrancyGuard {
    ...
    /**
     * @notice Constructor
     * @param _pdustToken: address of the $pDUST token
     ...
     * @param _amount: amount of the bid in $pDust token
    ...
    function bid(uint256 _amount) external nonReentrant {
        ...
        pdustToken.safeTransferFrom(address(msg.sender), address(this), _amount);
    ...
    function claimAuction(uint256 _auctionId) external nonReentrant {
        ...
        pdustToken.safeTransfer(address(msg.sender), claimableAmount);
    ...
    /**
     * @notice Close an auction round, and store $pDust leaderboard threshold
     * @param _bidLimit: minimal $pDust committed to be a winner
     * @dev Callable by operator
     */
    ...
    /**
     * @notice Claim $pDust for all bids by bidders in leaderboard
     * @param _auctionId: auction id
     * @param _bidders: addresses of bidders
    ...
    function recoverToken(address _token, uint256 _amount) external onlyOwner {
        require(_token != address(pdustToken), "Recover: Cannot be pDust token");
