...
contract ExtinctionPool {
    ...
    struct PoolInfo 
    {
        ...
        uint256 accRewardPerShare;          // Accumulated reward per share, times 1e19. See below.
    }
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > poolInfo.lastRewardBlock && dinoSupply != 0) {
            ...
            accRewardPerShare = accRewardPerShare.add(tokenReward.mul(1e19).div(dinoSupply));
        }
        return user.amount.mul(accRewardPerShare).div(1e19).sub(user.rewardDebt);
    }
    ...
    function updatePool() public {
        ...
        uint256 multiplier = getMultiplier(poolInfo.lastRewardBlock, block.number);
        uint256 tokenReward = multiplier.mul(rewardPerBlock);
        poolInfo.accRewardPerShare = poolInfo.accRewardPerShare.add(tokenReward.mul(1e19).div(dinoSupply));
        poolInfo.lastRewardBlock = block.number;
    }
    ...
    function transact(uint256 _amount) public {
        ...
        updatePool();
        if (user.amount > 0) {
            uint256 tempRewardDebt = user.amount.mul(poolInfo.accRewardPerShare).div(1e19);
        ...
        if (block.number < lockBlock && _amount != 0) {
            ...
            user.rewardDebt = user.amount.mul(poolInfo.accRewardPerShare).div(1e19);
