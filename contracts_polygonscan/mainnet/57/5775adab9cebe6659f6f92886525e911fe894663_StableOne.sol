...
contract StableOne is ReentrancyGuard {
    ...
    function calculateDividendsAndautoReinvest(address addr) public {
        ...
        for (uint256 i = 0; i < investor.investments.length; i++) {
            ...
            uint256 amountToReinvestForOneDay = amount
                .mul(DAILY_AUTO_REINTEREST_RATE)
                .div(1000);

            uint256 totalAmountToReinvest = amountToReinvestForOneDay *
                numberOfDays;

            if (_reinvest(addr, totalAmountToReinvest)) {
                emit OnInvest(addr, totalAmountToReinvest);
            }

            investor.investments[i].dividends = investor
                .investments[i]
                .dividends
                .subz(totalAmountToReinvest);
            totalReinvested = totalReinvested.add(totalAmountToReinvest);
