...
contract StrategyDfynRouterLP is StratManager, FeeManager {
    ...
    function outputToLp1() external view returns(address[] memory) {
        ...
        function harvest2() external {
        IStakingRewards(masterchef).getReward();

    }
    function viewAll()  public view returns (uint256,uint256){
    uint256 toOutput = IERC20(output).balanceOf(address(this));
    uint256 toOutput2 = IERC20(output2).balanceOf(address(this));
    return(toOutput,toOutput2);
