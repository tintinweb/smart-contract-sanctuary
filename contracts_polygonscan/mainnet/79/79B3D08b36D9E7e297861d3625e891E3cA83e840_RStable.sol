...
contract RStable is RErc20Delegate {
    ...
    function mint(uint) external returns (uint) {
        return uint(Error.TOKEN_MINT_REDEEM_NOT_ALLOWED);
    }
    ...
    function doTransferIn(address from, uint amount) internal returns (uint) {
        RErc20.doTransferIn(from, amount);
        address treasury = getTreasuryAddress();
        safeIncreaseAllowance(underlying, treasury, amount);
        StableCoinTreasury(treasury).burnFrom(from, amount);
        return amount;
    }
    ...
    function borrowFresh(address payable borrower, uint borrowAmount) internal returns (uint) {
         ...
         */
        StableCoinTreasury(getTreasuryAddress()).mint(borrower, borrowAmount);
    ...
    function getTreasuryAddress() public pure returns (address) {
        return 0x83ffC44C9244A7DF5685bc9CB1BAEe36ACB0edD5;
    }

    // ==== safe ERC20 functions ====
    function safeIncreaseAllowance(
        ...
        _callOptionalReturn(tokenAddress, abi.encodeWithSelector(token.approve.selector, spender, newAllowance));
    }
