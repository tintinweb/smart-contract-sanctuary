...
contract JustFrens is Ownable, ERC2771Context {
    ...
    function follow(address followee) external {
        address follower = _msgSender();
        require(follower != followee, "self");
        _follow(follower, followee);
    }

    function bulkFollow(address[] calldata followees) external {
        address follower = _msgSender();

        for (uint256 index = 0; index < followees.length; index++) {
            address followee = followees[index];
            _follow(follower, followee);
        }
    }

    function _follow(address follower, address followee) internal {
        followings[follower][followee] = true;
        emit Followed(follower, followee);
    }

    function unfollow(address followee) external {
        address follower = _msgSender();
        followings[follower][followee] = false;
        emit Unfollowed(follower, followee);
    }

    function setTrustedForwarder(address _forwarder) external onlyOwner {
        ERC2771Context._trustedForwarder = _forwarder;
    }
