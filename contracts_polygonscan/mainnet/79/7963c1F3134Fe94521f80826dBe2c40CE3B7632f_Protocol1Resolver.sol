...
contract Protocol1Resolver {
    ...
    IProtocolModule internal constant protocolModule = IProtocolModule(0xE24bFb3056CE3BB7a52C27Ce48580542f70c2C42);
    ...
    function getITokenDetails() public view returns (ITokenInfo[] memory iTokens) {
        address[] memory markets = protocolModule.markets();
        iTokens = new ITokenInfo[](markets.length);
        for(uint i = 0; i < markets.length; i++) {
            iTokens[i].token = markets[i];
            iTokens[i].itoken = protocolModule.tokenToItoken(markets[i]);
            TokenInterface iToken = TokenInterface(iTokens[i].token);
            iTokens[i].name = iToken.name();
            iTokens[i].symbol = iToken.symbol();
            iTokens[i].decimals = iToken.decimals();
            (iTokens[i].exchangePrice,) = liquidity.updateInterest(markets[i]);
        }
    }

    function getUserInfo(address user) public returns (PositionInfo[] memory positions, ITokenInfo[] memory iTokens) {
        iTokens = getITokenDetails();
        positions = new PositionInfo[](iTokens.length);
        for(uint i = 0; i < iTokens.length; i++) {
            TokenInterface iToken = TokenInterface(iTokens[i].itoken);
            positions[i].iTokenAmount = iToken.balanceOf(user);
            positions[i].amount = positions[i].iTokenAmount.mul(iTokens[i].exchangePrice).div(1e18);
            positions[i].token = iTokens[i].token;
            positions[i].rewardInfos.rewardTokens = protocolModule.rewardTokens(iTokens[i].token);
            positions[i].rewardInfos.rewardAmounts = protocolModule.updateUserReward(user, iTokens[i].token);
        }
    }
}
