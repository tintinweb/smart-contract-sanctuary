...
contract StakeBurner is IStakeBurner, Manageable, Migrateable, ExternallyCallable {
    struct Settings {
        uint32 secondsInDay;
     ...
     */
    function burnStake(uint256 sessionId) external {
    ...
    function handlePayoutAndPenalty(
        ...
        uint256 APR = contracts.stakeManager.getAPR();
        uint256 minInterestPerDay = (amount * 1e12 * APR) / 100 / 365; // Calculate the minimum interest per day to maintain the minimum APR
        uint256 totalMinInterest = (minInterestPerDay * stakingDays) / 1e12; // Calculate the total interest to maintain the minimum APR

        // If the interest earned is lower then minimum APR, we give the minimum
        if (interest < totalMinInterest) {
            interest = totalMinInterest;
        }
        ...
        // Penalty is no longer used here as we don't send to auction
        (uint256 payout, ) = getPayoutAndPenaltyInternal(amount, start, stakingDays, interest);
    ...
    */
    ...
    function restore(uint32 _secondsInDay, uint32 _bpdDayRange) external onlyMigrator {
...
interface IStakeBurner {
    function externalBurnStake(uint256 sessionId, address staker) external returns (uint256);
}
