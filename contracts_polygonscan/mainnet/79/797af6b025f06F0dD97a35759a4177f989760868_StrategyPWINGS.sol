...
// This strategy is for polygon network. pWINGS - WMATIC pair
// A name Of RewardToken = "pWINGS"
contract StrategyPWINGS is Ownable {
    ...
    address public constant router = 0x5C6EC38fb0e2609672BDf628B1fD605A523E5923;
    address public constant lpToken = 0x3324af8417844e70b81555A6D1568d78f4D4Bf1f;
    address public constant farm = 0x4e22399070aD5aD7f7BEb7d3A7b543e8EcBf1d85;
    address public constant USDT = 0xc2132D05D31c914a87C6611C10748AEb04B58e8F;
    address public constant yelLiquidityRouter = 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff;
    address public constant YELtoken = 0xD3b71117E6C1558c1553305b44988cd944e97300;
    address public constant lpYELtoken = 0x8bAb87ECF28Bf45507Bd745bc70532e968b5c2De;
    ...
    function earn() public {
        uint256 _balance = _getBalanceOfToken(lpToken);
        if(_balance > 0) {
    ...
    function getAvailableWETH() public view returns (uint256) {
    ...
    function takeFee() internal {
        ...
        path[0] = WETH;
        path[1] = USDT;
        uint256 _balanceWETH = getAvailableWETH();
        if(_balanceWETH > 0) {
            rewardsFeeInWETH = _calculateAmountFee(_balanceWETH);
            if(rewardsFeeInWETH > 0) {
                uint256 amount = _getAmountsOut(rewardsFeeInWETH, path);
                if(amount > 100) {
                    _swapExactTokensForTokens(router, rewardsFeeInWETH, amount, path);
                    pendingFee = 0;
    ...
    function updateTotalCapital() public {
        ...
        // calculate total Capital from LPs
        if(totalLP > 0) {
        ...
        uint256 _balance;
        if(token0 != WETH) {
            _balance = _getBalanceOfToken(token0);
            if (_balance > 0) {
                path[0] = token0;
                totalCapital += _getAmountsOut(_balance, path);
            }
        }

        if(token1 != WETH) {
            _balance = _getBalanceOfToken(token1);
            if (_balance > 0) {
                path[0] = token1;
                totalCapital += _getAmountsOut(_balance, path);
            }
    ...
    function withdrawUSDTFee(address _owner) public onlyVault {
        uint256 _balance = _getBalanceOfToken(USDT);
        if(_balance > 0) {
            IERC20(USDT).transfer(_owner, _balance);
        }
    }
    ...
    function migrate(uint256 _percent) public onlyVault {
        _swapLPtoNativeTokens(_percent * 10 ** 12);
        IERC20(WETH).transfer(vault, getAvailableWETH());
        updateTotalCapital();
    }

    function depositAsMigrate() public onlyVault {
        require(getAvailableWETH() > 0, "Not enough WETH to make migration");
        _approveToken(token0);
        _approveToken(token1);
        IERC20(token0).safeApprove(lpToken, type(uint256).max);
        IERC20(token1).safeApprove(lpToken, type(uint256).max);
        _approveBase();
        _swapNativeTokenToTokens();
        earn();
        updateTotalCapital();
    }
    ...
    function claimYel(address _receiver) public onlyVault {
        ...
        if(yelAmount > 0) {
            IERC20(YELtoken).transfer(_receiver, yelAmount);
    ...
    function _getAmountsOut(uint256 _amount, address[] memory path) internal view returns (uint256){
        ...
        amounts = IRouter(router).getAmountsOut(_amount, path);
        return amounts[1];
    }
    ...
    function _addLiquidity(uint256 _amount0, uint256 _amount1) internal {
        ...
        uint256 desired1;
        uint256[] memory amounts;
        amounts = IRouter(router).getAmountsOut(_amount0, path);
    ...
    function _addLiquidityFromAllTokens() internal {
        ...
        if(token0 == WETH) {
            _potentialBalance0 = getAvailableWETH();
        } else {
            _potentialBalance0 = _getBalanceOfToken(token0);
        }
        if(token1 == WETH) {
            _potentialBalance1 = getAvailableWETH();
        } else {
    ...
    function _approveBase() internal {
        IERC20(lpToken).safeApprove(farm, 0);
        IERC20(lpToken).safeApprove(farm, type(uint256).max);
    ...
    function _approveYELToken() internal {
        ...
        IERC20(WETH).safeApprove(yelLiquidityRouter, 0);
        IERC20(WETH).safeApprove(yelLiquidityRouter, type(uint256).max);
    }
    ...
    function _swapLPtoNativeTokens(uint256 _percent) internal {
        ...
        if (_totalLP > 0) {
            ...
            uint256 currentPercent = getAvailableWETH() * 100 * 10**12 / totalCapital;
    ...
    function _swapToYELs(uint256 _percent) internal returns (uint256 newYelBalance){ 
        _swapLPtoNativeTokens(_percent);
    ...
    function _swapTokenToNativeToken(address _token) internal {
        ...
        if(_token != WETH) {
            path[0] = _token;
            amounts = IRouter(router).getAmountsOut(
                _getBalanceOfToken(path[0]), path
            );
            if(amounts[1] > 100) {
                _swapExactTokensForTokens(router, _getBalanceOfToken(path[0]), amounts[1], path);
            }
        }
    }
    ...
    function _swapNativeTokenToToken(address _router, address _token) internal {
        address[] memory path = new address[](2);
        path[0] = WETH;
        path[1] = _token;
        uint256 _balanceWETH = getAvailableWETH();
    ...
    function _swapNativeTokenToTokens() internal {
        address[] memory path = new address[](2);
        path[0] = WETH;
        path[1] = token0;

        uint256 _balance = getAvailableWETH() / 2;
        if (_balance > 0) {
