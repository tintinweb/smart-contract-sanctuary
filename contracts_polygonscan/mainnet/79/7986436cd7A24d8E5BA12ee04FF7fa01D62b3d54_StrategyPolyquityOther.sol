...
contract StrategyPolyquityOther is BaseStrategyPolyquityRewards {
    ...
    constructor(address rewards, address lp, address _tokenA, address _tokenB, address strategist)
        public
    ...
    function swapRewardToWmaticAndDistributeFee(uint256 feeAmount) internal {
        if(feeAmount > 0) {
            _swapUniswapWithPathForFeeOnTransferTokens(reward_matic_path, feeAmount);
    ...
    function harvest() public override onlyHumanOrWhitelisted nonReentrant {
        ...
        if (remainingHarvested > 0) {
            _swapUniswapWithPathForFeeOnTransferTokens(reward_a_path, remainingHarvested);
        }
