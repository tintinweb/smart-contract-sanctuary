...
contract EthalendVaultsQuickSwapStrategy is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //   4. User's `rewardTokensDebt` and `ethaTokensDebt` gets updated.
    }
    ...
    bool public isEthaTokenRewardsEnabled = true;
    bool public isRewardTokenRewardsEnabled = false;
    ...
    //All addresses are concerned with ethalend.
    /**
     ...
     * @param _rewardToken Reward Token is the reward given out by the vaults (wbtc, wmatic)
     ...
     * @param _isEthaTokenRewardsEnabled Boolean flag if we need to enable/disable the reward transfer mode for etha tokens
     * @dev Only owner can call and update the isEthaTokenRewardsEnabled mode.
     */
    function updateEthaTokenRewardsTransferMode(bool _isEthaTokenRewardsEnabled)
        external
        onlyOwner
    {
        updatePool();
        isEthaTokenRewardsEnabled = _isEthaTokenRewardsEnabled;
    }

    /**
     * @notice Can be used by the owner to enable/disable mode for accumulated reward token rewards being sent to user
     * @param _isRewardTokenRewardsEnabled Boolean flag if we need to enable/disable the reward transfer mode for reward tokens
     * @dev Only owner can call and update the _isRewardTokenRewardsEnabled mode.
     */
    function updateRewardTokenRewardsTransferMode(bool _isRewardTokenRewardsEnabled)
        external
        onlyOwner
    {
        updatePool();
        isRewardTokenRewardsEnabled = _isRewardTokenRewardsEnabled;
    }
     ...
     */
    function transferEthaRewards() external onlyOwner {
        updatePool();
        uint256 ethaTokenRewards = IERC20(etha).balanceOf(address(this));
        TransferHelper.safeTransfer(address(etha), feeAddress, ethaTokenRewards);
    }

    // View function to see pending etha rewards.
    ...
    // View function to get total LP staked in vaults.
    ...
    // View function to see pending reward tokens generated.
    ...
    // function to claim rewards (etha and rewardTokens to ethawallet)
    ...
    // function to withdraw asset from ethaWallet to strategy contract.
    function _withdrawAssetFromEthaWallet() internal {
        address[] memory targets = new address[](1);
        bytes[] memory datas = new bytes[](1);
        ...
        datas[0] = abi.encodeWithSignature(
            ...
            address(asset),
            asset.balanceOf(address(ethaWallet)) //withdraw full available balance of the assets
        );
        ethaWallet.execute(targets, datas);
    }
    ...
    function _withdrawRewardsFromEthaWallet() internal {
        uint256 rewardTokenBalance = rewardToken.balanceOf(address(ethaWallet));
        ...
        if (ethaTokenBalance > 0) {
            targets[0] = ethalendContracts.transferLogic;
            datas[0] = abi.encodeWithSignature(
                "withdraw(address,uint256)",
                address(etha),
                ethaTokenBalance //withdraw full available balance of the etha tokens
            );
            ethaWallet.execute(targets, datas);
        }
    }

    //function for fetching the swap path to the secondary asset before adding liquidity
    function getSwapPathForSecondaryAsset() public view returns (address[] memory path) {
        path = new address[](2);
        path[0] = address(asset);
        path[1] = address(secondaryAsset);
    }
    ...
    function _depositAssetToEthalendVaults(uint256 _amount) internal {
        ...
        datas[3] = abi.encodeWithSignature(
            ...
            address(ethalendContracts.eVault),
            1, //can add this as anything since this is fetched from the memory
    ...
    function _withdrawAssetFromEthalendVaults(uint256 _amount) internal {
        //These steps are for building the lego tx.
        address[] memory targets = new address[](3);
        bytes[] memory datas = new bytes[](3);

        //Step 1 - Remove LP tokens based on the number of tokens deposited.
        uint256 totalLPToWithdraw = _amount.mul(getTotalLPStakedInVaults()).div(
        ...
        //need to know setId and setId2 are for tokenA and tokenB respectively.
        ...
        datas[1] = abi.encodeWithSignature(
            ...
            0, //setId (for setting in memory where amount of asset tokens are stored)
            1, //setId2 (for setting in memory where amount of secondary asset tokens are stored)
            1 //divider which is 1 by default
        );
        ...
        datas[2] = abi.encodeWithSignature(
            ...
            1, //getId is 1 since the amount of tokenB is stored at 1 in memory contract
            0, //setId is 0. This is for the amount of asset received after swapping.
            1 //divider which is 1 by default
        );
        ...
        returns (uint256 pendingRewardTokens, uint256 pendingEthaTokens)
    {
        ...
        if (totalInputTokensStaked != 0) {
            if (isRewardTokenRewardsEnabled) {
                accRewardTokensPerShare = accRewardTokensPerShare.add(
                    totalRewardTokensGenerated.mul(1e12).div(totalInputTokensStaked)
                );
            }
            if (isEthaTokenRewardsEnabled) {
                accEthaTokensPerShare = accEthaTokensPerShare.add(
                    pendingEthaRewards.mul(1e12).div(totalInputTokensStaked)
                );
            }
        }
        pendingRewardTokens = user.amount.mul(accRewardTokensPerShare).div(1e12).sub(
    ...
    function updatePool() public {
        ...
        //normal function to check if reward tokens are actually generated
        uint256 totalRewardTokensGenerated = getTotalRewardTokensGenerated();
        uint256 pendingEthaRewards = getPendingEthaRewards();

        if (getTotalRewardTokensGenerated() > 0 || pendingEthaRewards > 0) {
        ...
        returns (uint256 withdrawnAmount)
    {
        ...
        if (_amount > 0) {
            ...
            uint256 initialStrategyBalance = asset.balanceOf(address(this));
            _withdrawAssetFromEthalendVaults(_amount);
            _withdrawAssetFromEthaWallet();
            uint256 afterWithdrawStrategyBalance = asset.balanceOf(address(this));
            withdrawnAmount = afterWithdrawStrategyBalance.sub(initialStrategyBalance);
            //This check is a safety and very particular to the contract in accounting to the withdraw fees
            require(withdrawnAmount <= _amount, "Invalid withdrawn amount processed");
    ...
    function _transferPendingRewards(address _user) internal {
        ...
        if (isRewardTokenRewardsEnabled && rewardInfo.pendingRewardTokens > 0) {
        ...
        rewardInfo.pendingEthaTokens = user.amount.mul(pool.accEthaTokensPerShare).div(1e12).sub(
            user.ethaTokensDebt
        );
        if (isEthaTokenRewardsEnabled && rewardInfo.pendingEthaTokens > 0) {
            TransferHelper.safeTransfer(address(etha), _user, rewardInfo.pendingEthaTokens);
        }
    }
}
