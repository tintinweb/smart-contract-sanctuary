...
abstract contract StrategyBase is Ownable, ReentrancyGuard, Pausable, IStrategy {
    ...
    function exchangeRate(
        ...
        uint256[] memory amounts = IPolyDexFormula(polydexFormula).getAmountsOut(
            _inputToken,
            _outputToken,
            _tokenAmount,
...
contract StrategySdsLpSdoUsdc is StrategyBase {
    using SafeERC20 for IERC20;

    // _controller = MeebFarmingBank
    // _want = QLP SDO/USDC
    // _farmingToken = SDS
    // _farmingPool = IFarmingPool (earns SDS)
    // _farmingPoolId = 0
    // _targetProfitToken = MEEB
    // _farmingToken2UsdcPath = [pair(SDO/USDC)]
    // _farmingToken2TargetProfitTokenPath = [pair(SDO/USDC), pair(USDC/MEEB)]
    constructor(
        address _controller,
        address _want,
        address _farmingToken,
    ...
    function getName() public pure override returns (string memory) {
        return "MeebMaster.com:StrategySdsLpSdoUsdc";
    }
