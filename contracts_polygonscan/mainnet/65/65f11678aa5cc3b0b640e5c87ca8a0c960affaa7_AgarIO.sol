...
contract AgarIO is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, Ownable, ERC721Burnable {
    ...
    struct Circle {
        uint256 size;
        uint32 readyToEatTime;
        uint32 readyToBeEatenTime;
        uint eatCooldownTime;
        uint eatenCooldownTime;
    }

    //mapping from tokenId to a struct containing the circle's traits
    mapping(uint256 => Circle) public circleTraits;

    constructor() ERC721("AgarIO", "AIO") {}
    ...
    function generateCircle(uint256 tokenId) internal{
        uint256 size = 20;
        uint eatCooldownTime = _random(tokenId)%10 seconds;
        uint eatenCooldownTime = _random(tokenId)%7 seconds;
        Circle memory c = Circle(size, uint32(block.timestamp), uint32(block.timestamp), eatCooldownTime, eatenCooldownTime);
        circleTraits[tokenId] = c;
    }

    function eat(uint256 predatorTokenId, uint256 preyTokenId) public {
        require(ownerOf(predatorTokenId) == msg.sender, "You are not the owner of this circle");
        ...
        require((preyCircle.size*80)/100 > 10, "Can't eat this circle as the size will be less than 10");
        require(predatorCircle.size > preyCircle.size, "Can't eat a circle that is not smaller than you");
        uint256 size_to_change=(preyCircle.size*20)/100; 
    ...
    function getCircleSize(uint256 tokenId) public view returns (uint256) {
        return circleTraits[tokenId].size;
    }
    ...
    function _isReadyToEat(Circle storage circle) internal view returns (bool) {
      return (circle.readyToEatTime <= block.timestamp);
    }

    function _isReadyToBeEaten(Circle storage circle) internal view returns (bool) {
