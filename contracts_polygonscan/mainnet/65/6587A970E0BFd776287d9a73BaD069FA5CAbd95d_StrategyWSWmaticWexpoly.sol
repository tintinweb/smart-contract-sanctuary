...
// File: contracts/libs/IStrategyTime.sol
pragma solidity 0.6.12;
...
// File: contracts/libs/IWault.sol
pragma solidity 0.6.12;
...
// File: contracts/strategy/StrategyWSWmaticWexpoly.sol
pragma solidity 0.6.12;

contract StrategyWSWmaticWexpoly is BaseStrategyLPSingle {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    ...
    constructor(
        ...
        address[] memory _earnedToTimePath, // Note: Use WUSD as path
        ...
        stakingRewardsAddress = _stakingRewardsAddress; // Wault MasterChef

        uniRouterAddress = 0x3a1D87f206D12415f5b0A33E786967680AAb4f6d; // Wault UniRouter
    ...
    function _vaultDeposit(uint256 _amount) internal override {
        IWault(stakingRewardsAddress).deposit(pid, _amount, true);
    }

    function _vaultWithdraw(uint256 _amount) internal override {
        IWault(stakingRewardsAddress).withdraw(pid, _amount, true);
    }

    function _vaultHarvest() internal override {
        IWault(stakingRewardsAddress).claim(pid);
    }

    function totalInUnderlying() public override view returns (uint256) {
        (uint256 amount,) = IWault(stakingRewardsAddress).userInfo(pid, address(this));
        return amount;
    }
    ...
    function _emergencyVaultWithdraw() internal override {
        IWault(stakingRewardsAddress).emergencyWithdraw(pid);
    }
}