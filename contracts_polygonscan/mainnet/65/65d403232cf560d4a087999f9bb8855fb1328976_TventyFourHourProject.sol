...
import "./ITventyFourHourProject.sol";
...
contract TventyFourHourProject is ERC721Enumerable, ITventyFourHourProject, Ownable {
    using SafeMath for uint256;
    using Strings for uint256;

    uint256 public override supplyLimit = 576;
    ...
    function giveAway(address _luckyFollowerAddress) public onlyOwner override {
        require(totalSupply() < supplyLimit, "Not enought tokens left.");
        _safeMint(_luckyFollowerAddress, totalSupply());
    }
    ...
    function setWithdrawalWallet(address payable _withdrawalWallet) public onlyOwner override {
        withdrawalWallet = _withdrawalWallet;
        emit WithdrawalWalletChanged(_withdrawalWallet);
    }

    function buy(uint256 _amount) external override payable {
        require(saleActive, "Sale is not active.");
        require(totalSupply().add(_amount) <= supplyLimit,"Not enought tokens left.");
        require(msg.value >= mintPrice.mul(_amount), "Insufficient funds.");
...
interface ITventyFourHourProject {
    //VIEW
    ...
    // OWNER ONLY
    function giveAway(address _luckyFollowerAddress) external;
    ...
    event AddTokenURI(string tokenURI, uint256 index);
}
