...
contract GHERO is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public marketingAddress = payable(0x71FDD8d51359004f89Ba1F0fA61BA0db856c9eB3);
    ...
    uint256 private _tTotal = 1 * 10**9 * 10**3;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "GHERO";
    ...
    uint256 public _maxTxAmount = 1000 * 10**3 * 10**3;
    uint256 private minimumTokensBeforeSwap = 200 * 10**3 * 10**3; 
    ...
    function setBuyFee(uint256 buyTaxFee, uint256 buyLiquidityFee) external onlyOwner {
        ...
        uint256 divValue = (_buyLiquidityFee.add(_sellLiquidityFee)).div(2);
        require (marketingDivisor <= divValue);
    }
    ...
    function setMarketingDivisor(uint256 newDivisor) external onlyOwner {
        uint256 divValue = (_buyLiquidityFee.add(_sellLiquidityFee)).div(2);
        require (newDivisor <= divValue);
        marketingDivisor = newDivisor;
    }
