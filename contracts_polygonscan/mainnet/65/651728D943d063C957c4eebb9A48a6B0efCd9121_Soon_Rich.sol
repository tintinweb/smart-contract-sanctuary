...
contract SRICHDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("SRICH_Dividend_Tracker", "SRICH_Dividend_Tracker") {
    	claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override pure {
        require(false, "SRICH_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override  pure {
        require(false, "SRICH_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SRICH contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 600 && newClaimWait <= 86400, "SRICH_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "SRICH_Dividend_Tracker: Cannot update claimWait to same value");
...
contract Soon_Rich is ERC20, Ownable {
    
    using SafeMath for uint256;

    address public uniswapV2Pair;
    IUniswapV2Router02 public uniswapV2Router;
    SRICHDividendTracker public dividendTracker;

    address public _marketingWallet = 0xDF9c822a67F89962AA693ecB82CEeB3438e0dbD0;

    bool public restrictWhales = true;

    uint256 public maxTxAmount = 50000000000000000 * (10**9);
    uint256 public walletMax = 1000000000000000 * (10**9);
    ...
    constructor() ERC20("Soon Rich", "SRICH") {

    	dividendTracker = new SRICHDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "SRICH: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "SRICH: Account is already the value of 'excluded'");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SRICH: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SRICH: Cannot update gasForProcessing to same value");
