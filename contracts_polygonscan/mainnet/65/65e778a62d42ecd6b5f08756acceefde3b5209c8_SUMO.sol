...
contract SUMO is ERC721, ERC721Enumerable, Ownable, IERC2981 {

  using Strings for uint256;

    uint256 public constant tokenPrice = 40000000000000; // 0.00004 ETH
    uint public maxTokenPurchase = 30;
    ...
    uint public royaltiesPercentage = 5;
    
    constructor() ERC721("Rikishi", "SUMO") {}
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC721, ERC721Enumerable, IERC2981) returns (bool) {
    ...
    function setTokenURI(uint mintIndex) internal {
        string memory currentBaseURI = _baseURI();
        _tokenUri[mintIndex] = bytes(currentBaseURI).length > 0
            ? string(abi.encodePacked(currentBaseURI, ( mintIndex ).toString(), baseExtension))
            : "";
    }
    ...
    function setRoyaltyReceiver(uint256 tokenId, address creator) public onlyOwner {
        royaltyReceivers[tokenId] = creator;
    }  
    
    function _setTokenRoyaltiesPercentage(
        uint value
    ) internal {
        require(value <= 10000, 'ERC2981Royalties: Too high');
        royaltiesPercentage = value;
    }
