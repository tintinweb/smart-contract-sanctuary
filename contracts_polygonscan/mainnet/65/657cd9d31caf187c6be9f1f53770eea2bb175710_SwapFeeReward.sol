...
contract SwapFeeReward is OwnableUpgradeable, ReentrancyGuard, ISwapFeeReward {
  ...
  function pairFor(address tokenA, address tokenB) public view returns (address pair) {
    address _pairAB = tokensPair[tokenA][tokenB];
    if (_pairAB != address(0)) {
      return _pairAB;
    }

    address _pairBA = tokensPair[tokenB][tokenA];
    if (_pairBA != address(0)) {
      return _pairBA;
    }
  ...
  function withdraw(
    ...
    uint256 _totalMined = totalMined;
    require(maxMiningAmount > _totalMined, "SwapFeeReward: Mined all tokens");
    uint256 balance = _balances[msg.sender];
    require(_totalMined.add(balance) <= currentPhase.mul(maxMiningInPhase), "SwapFeeReward: Mined all tokens in this phase");

    permit(msg.sender, balance, v, r, s);
    //reference reward
    address _rewardReferral = rewardReferral;
    if (_rewardReferral != address(0) && _referrer != address(0)) {
      IReferral(_rewardReferral).set(_referrer, msg.sender);
    }
    if (balance > 0) {
      totalMined = _totalMined.add(balance);
      _balances[msg.sender] = _balances[msg.sender].sub(balance);
      emit Withdraw(msg.sender, balance);

      _referrer = _rewardReferral != address(0) ? IReferral(_rewardReferral).refOf(msg.sender) : address(0);
      ...
      if (_referrer != address(0)) {
        ...
        IReferral(_rewardReferral).onHopeCommission(_referrer, msg.sender, _commission);
      }
  ...
  function getSwapReward(
    ...
    uint256 fee = amount.mul(pairFee).div(uint256(10000).sub(pairFee));
  ...
  function getQuantity(
    ...
    if (outputToken == anchorToken) {
      ...
      quantity = oracle.consult(outputToken, outputAmount, anchorToken, pairFor(outputToken, anchorToken));
    } else {
      ...
      for (uint256 index = 0; index < length; index++) {
        ...
        if (pair1 != address(0) && checkPairExist(intermediate, anchorToken)) {
          ...
          uint256 interQuantity = _oracle.consult(outputToken, outputAmount, intermediate, pair1);
          quantity = _oracle.consult(intermediate, interQuantity, anchorToken, pair2);
          break;
        }
      }
    }
  ...
  function setPairsFee(address pair, uint256 fee) public onlyOwner {
    require(fee < 10000, "SwapMining: max fee");
    pairsFee[pair] = fee;
  }
