...
contract TomiConfig is TokenRegistry, Ownable {
    ...
    uint256 public constant TGAS_DECIMAL = 10**18;
    ...
    constructor() public {
        ...
        _initConfig(
            ConfigNames.PROPOSAL_TGAS_AMOUNT,
            100 * TGAS_DECIMAL,
            10000 * TGAS_DECIMAL,
            100 * TGAS_DECIMAL,
            100 * TGAS_DECIMAL
    ...
    function initialize(
        ...
        address _platform,
        address _dev,
        address[] memory _listTokens
    ) public onlyOwner {
        require(_tgas != address(0), 'TomiConfig: ZERO ADDRESS');
        tgas = _tgas;
        platform = _platform;
        dev = _dev;
        for (uint256 i = 0; i < _listTokens.length; i++) {
    ...
    function changeConfigValue(bytes32 _name, uint256 _value) external onlyGovernor returns (bool) {
        Config storage config = configs[_name];
    ...
    function checkToken(address _token) public view returns (bool) {
        ...
        if (tokenStatus[_token] == OPENED) {
            ...
            if (
                getConfigValue(ConfigNames.TOKEN_PENGDING_SWITCH) == 1 &&
                block.number > publishTime[_token] + getConfigValue(ConfigNames.TOKEN_PENGDING_TIME)
            ) {
                return false;
            } else {
                return true;
    ...
    function getDefaultListTokens() external view returns (address[] memory) {
        ...
        for (uint256 i; i < defaultListTokens.length; i++) {
...
contract TokenRegistry is Governable {
    ...
    mapping(address => uint256) public publishTime;
    uint256 public tokenCount;
    address[] public tokenList;
    uint256 public constant NONE = 0;
    uint256 public constant REGISTERED = 1;
    uint256 public constant PENDING = 2;
    uint256 public constant OPENED = 3;
    uint256 public constant CLOSED = 4;

    event TokenStatusChanged(address indexed _token, uint256 _status, uint256 _block);
    ...
    function updateToken(address _token, uint256 _status) external onlyGovernor returns (bool) {
    ...
    function iterateValidTokens(uint256 _start, uint256 _end) external view returns (address[] memory) {
    ...
    function _updateToken(address _token, uint256 _status) internal returns (bool) {
        ...
        if (tokenStatus[_token] == NONE) {
            tokenCount++;
            require(tokenCount <= uint256(-1), 'TokenRegistry: OVERFLOW');
