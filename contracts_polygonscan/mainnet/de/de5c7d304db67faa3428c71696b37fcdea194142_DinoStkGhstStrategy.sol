...
contract DinoStkGhstStrategy is BaseStrategyMasterChef {
        ...
        )
    {
        ...
        reward_a_path[0] = DINO;
        reward_a_path[1] = WETH;
        ...
        IERC20(GHST).safeApprove(QUICKSWAP_ROUTER, uint256(-1));
    }
    ...
    function harvest() public override onlyHumanOrWhitelisted nonReentrant {
        ...
        //Swap harvestedToken for tokenA
        ...
        uint256 ghstWethBal = IERC20(GHST_WETH).balanceOf(address(this));
        if(ghstWethBal > 0) {
            //Deposit any STKGHST_WETH already in contract
            _distributePerformanceFeesAndDeposit();

            //Convert GHST_WETH to STKGHST_WETH
            IConverter(converter).convert();

            //Make sure the converter successfully converted the GHST_WETH to STKGHST_WETH
            require(ghstWethBal == IERC20(STKGHST_WETH).balanceOf(address(this)), "Did not get STKGHST");
        }
    ...
    function setConverter(address _converter) external onlyOwner {
        ...
        IERC20(GHST_WETH).safeApprove(converter, uint256(-1));
    }
}