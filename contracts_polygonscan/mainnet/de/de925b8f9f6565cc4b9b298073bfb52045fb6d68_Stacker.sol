...
contract Stacker is ERC20 {
    ...
    constructor(uint256 _maxLimit, uint256 _minimumLimit)
        ERC20("Reflect USD", "RUSD")
    {
        owner = _msgSender();
        maxLimit = _maxLimit;
    ...
    function isMinimunLevelReached() internal {
        if (totalSupply() < minimumLimit) {
            mint(minimumLimit - totalSupply());
        }
    }

    function isMaximumLevelReached() internal {
        if (totalSupply() > maxLimit) {
            burn(totalSupply() - maxLimit);
        }
    }
}
