...
contract MOXbucks is Context, IERC20Metadata, Ownable { 
    ...
    uint256 public FIRSTOWNER_PERC = 1; //1% goes to the first owner
    ...
    function transferFromMarket(address sender, address recipient, uint256 amount, address firstOwner) external returns (bool) {
        _specialTransferMarket(sender, recipient, amount, firstOwner);

        //uint256 currentAllowance = _allowances[sender][_msgSender()];
        //require(currentAllowance >= amount, "ERC20: transfer amount exceeds allowance");
        //_approve(sender, _msgSender(), currentAllowance - amount);
       
        if ((msg.sender != _marketContractAddress) && (msg.sender != _additionalContract)) {
            _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "ERC20: transfer amount exceeds allowance"));
        }
    ...
    /**
     * @dev Used only in the case of the NFT market, as it also considers DIST_PERC and FIRSTOWNER_PERC
     */
    function _specialTransferMarket(address sender, address recipient, uint256 amount, address firstOwner) internal virtual {
        require(sender != address(0), "ERC20: transfer from the zero address");
        require(recipient != address(0), "ERC20: transfer to the zero address");
        ...
        require(senderBalance >= amount, "ERC20: transfer amount exceeds balance");
        _balances[sender] = senderBalance - amount;
        _balances[recipient] += amount.mul(100-DIST_PERC-FIRSTOWNER_PERC).div(100);
        _balances[firstOwner] += amount.mul(FIRSTOWNER_PERC).div(100);
    ...
    /**
     * @dev Public (only owner) function. Edits the firstOwner percent.
     *
     */ 
    function changeFirstOnwerPerc(uint256 newPerc) public onlyOwner {
        FIRSTOWNER_PERC = newPerc;
    }
