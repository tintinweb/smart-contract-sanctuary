...
// File contracts/SalsaChefV2.sol
...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    uint256 public constant salsaMaximumSupply = 150 * (10 ** 3) * (10 ** 18); // 150,000 salsa
    ...
    event SetUSDCSalsaLPAddress(address lpAddress);
    event UpdateEmissionParameters(uint topPrice, uint bottomPrice, uint emissionUpdateInterval);
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 salsaDevReward = salsaReward / 10;
        if (salsa.totalSupply() > salsaMaximumSupply)
            salsaReward = 0;
        else if (salsa.totalSupply() + salsaReward + salsaDevReward > salsaMaximumSupply)
        {
        ...
        {
            if(salsaDevReward > 0)
                salsa.mint(feeAddress, salsaDevReward);
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (_amount > 0) {
            ...
            if(user.amount == 0) user.lastWithdraw = block.number;
    ...
    function setEmissionRate(uint256 _salsaPerBlock) external onlyOwner {
    ...
    function setUSDCSalsaLPAddress(address _address) external onlyOwner {
        require (_address!=address(0));  // added for good measure
        usdcSalsaLP = _address;

        emit SetUSDCSalsaLPAddress(_address);
    }    
    ...
    function updateEmissionIfNeeded() public {
        ...
        if (block.number - lastBlockUpdate > emissionUpdateInterval) {
            ...
            if (salsaBalance > 0) {
                 priceCents = getSalsaPriceCents();
            }

            // Update pools before changing the emission rate
            massUpdatePools();
            uint256 emissionRatePercent = getEmissionRatePercent(priceCents);
            salsaPerBlock = MAX_EMISSION_RATE * emissionRatePercent / 100;
        }
    }

    // The Reserves ratio for a LP
    function convertToTargetValueFromPair(IUniswapV2Pair pair, uint256 sourceTokenAmount, address targetAddress) public view returns (uint256) {
        ...
        // Note: this implicitly casts uint112 values to uint256
    ...
    // For the Salsa USDC LP, find the pricing ratio of USDC per Salsa
    function getSalsaUSDCReservesRatio(uint256 sourceTokenAmount) public view returns(uint256) {
        if (usdcSalsaLP == address(0)) return 0;

        return(convertToTargetValueFromPair(IUniswapV2Pair(usdcSalsaLP), sourceTokenAmount, address(usdc)));
    }

    // Based on the reserves ratio, return the price in cents
    function getSalsaPriceCents() public view returns (uint256 spc){
        if (usdcSalsaLP == address(0)) return 0;

        // Calculate the USDC price against a large number to allow for low values
        uint256 salsaUSDCReservesRatio = getSalsaUSDCReservesRatio(1e18);

        // usdc token decimals = 6, token decimals = 18, (18-6)=12 + 2  = 14 to convert to cents
        return(salsaUSDCReservesRatio / 1e4);
    }
    ...
    function getEmissionRatePercent(uint256 salsaPriceCents) public view returns (uint256 epr) {
        ...
        uint256 salsaPricePercentOfTop = salsaPriceCents / topPrice;
    ...
    function updateEmissionParameters(uint _topPrice, uint _bottomPrice, uint _emissionUpdateInterval) external onlyOwner {
        topPrice = _topPrice;
        bottomPrice = _bottomPrice;
        emissionUpdateInterval = _emissionUpdateInterval;

        emit UpdateEmissionParameters(_topPrice, _bottomPrice, _emissionUpdateInterval);
    }
                ...
                if(user.amount > 0) {
                    if(isStakingRewardsEligible(_pid)) {
                        ...
                        if(blockDifference >= STAKING_LENGTH_BLOCKS)
                            ...
                            return(blockDifference * STAKING_LENGTH_MULT / STAKING_LENGTH_BLOCKS);
                    }
                }
                return 0;
    }
