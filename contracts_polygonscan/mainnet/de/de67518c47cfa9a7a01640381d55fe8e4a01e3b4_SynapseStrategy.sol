...
contract SynapseStrategy is BaseStrategy {
    ...
    address private constant SYN = 0xf8F9efC0db77d8881500bb06FF5D6ABc3070E695;
    address private constant WMATIC = 0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270;
    address private constant USDC = 0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174;

    address private constant MASTER_CHEF = 0x7875Af1a6878bdA1C129a4e2356A3fD040418Be5;
    address private constant SYN_DEPOSIT = 0xbe67Dbe294bD99402Ca8961dF4f7138b950513DB;

    address private constant QS_ROUTER = 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff;
    ...
    constructor()
        public
        BaseStrategy(
            0x398a2D1b343d09261dF990c2Fcc97B5d5d62c1B5, //nUSD LP
        ...
        )
    {
        ...
        reward_a_path = [harvestedToken, USDC];
        reward_matic_path = [harvestedToken, USDC, WMATIC];

        IERC20(harvestedToken).safeApprove(currentRouter, uint256(-1));
        IERC20(USDC).safeApprove(SYN_DEPOSIT, uint256(-1));
    ...
    function getHarvestable() external override view returns (uint256) {
        return IMiniChef(rewards).pendingSynapse(poolId, address(this));
    }
    ...
    function harvest() public override onlyHumanOrWhitelisted nonReentrant {
        ...
        uint256 h_bal = IERC20(harvestedToken).balanceOf(address(this));
        if(h_bal > 0) {
            IERC20(harvestedToken).safeTransfer(strategist, h_bal);
        }

        _getReward();

        //Distribute fee and swap tokens
        ...
        if (_harvested_balance > 0) {
            ...
            //Swap all SYN for USDC
            _swapUniswapWithPathPreapproved(reward_a_path, _harvested_balance.sub(feeAmount));
        }
        ...
        if (_usdcBalance > 0) {
            ...
            balances[2] = _usdcBalance;
            ISynapseSwap(SYN_DEPOSIT).addLiquidity(balances, 0, now + 60);
        }
