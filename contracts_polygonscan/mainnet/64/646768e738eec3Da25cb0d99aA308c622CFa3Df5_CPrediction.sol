...
contract CPrediction is Ownable, Pausable, ReentrancyGuard {
    ...
    IReferralC public referralSystem;
    ...
    function initialize(
        ...
        referralSystem = IReferralC(_referralSystemAddress);
    }
    ...
    function genesisStartRound() external onlyOwner whenNotPaused {
        ...
        rounds[currentEpoch].genesis = true;
     ...
     */
    function executeRound(int256 currentPrice) external onlyOwner whenNotPaused {
        require(genesisStartOnce, "req genesis");

        // CurrentEpoch refers to previous round (n-1)
        _safeLockRound(currentEpoch, currentPrice);
        if(rounds[currentEpoch].genesis == true){
            currentEpoch = currentEpoch + 1;
            _startRound(currentEpoch);
        }
        else{
     ...
     */
    function betBear(uint256 epoch, address user, uint256 amount) external payable whenNotPaused nonReentrant onlyOwner {
        ...
        //round treasury amount includes the referral bonuses until the calculation
        ...
        emit PredictionsBet(user, epoch, amount, refereeAmt, 0, uint8(Position.Bear));
    }

    /**
     * @dev Bet bull position
     */
    function betBull(uint256 epoch, address user, uint256 amount) external payable whenNotPaused nonReentrant onlyOwner {
        require(epoch == currentEpoch, "Bet earlylate");
        require(_bettable(currentEpoch), "not bettable");
        require(amount >= minBetAmount);
        require(ledger[currentEpoch][user].amount == 0, "alreadybet");
        ...
        emit PredictionsBet(user, epoch, amount, refereeAmt, 0, uint8(Position.Bull));
    }
    ...
    function claimTreasury(address _recipient) external nonReentrant onlyOwner {
        ...
        treasuryAmount = 0;
        _transferAmount(_recipient, currentTreasuryAmount);
    }

    function _transferAmount(address _to, uint256 _amount) internal {
        if(address(betToken) == address(0)){
            (bool success, ) = _to.call{value: _amount}("");
            require(success, "TransferHelper: TRANSFER_FAILED");
        }
        else{
            betToken.safeTransfer(_to, _amount);
        }
    }

    function recoverAmount(address _to, uint256 _amount) external onlyOwner {
        (bool success, ) = _to.call{value: _amount}("");
        require(success, "TransferHelper: TRANSFER_FAILED");
    }
    ...
    function claimable(uint256 epoch, address user) public view returns (bool) {
        ...
        return
            round.completed && betInfo.amount > 0 && !betInfo.claimed &&
    ...
    function refundable(uint256 epoch, address user) public view returns (bool) {
        BetInfo memory betInfo = ledger[epoch][user];
        Round memory round = rounds[epoch];
        return !round.completed && block.timestamp > (timestamps[epoch].closeTimestamp + intervalSeconds) && betInfo.amount != 0 && !betInfo.claimed;
    }
