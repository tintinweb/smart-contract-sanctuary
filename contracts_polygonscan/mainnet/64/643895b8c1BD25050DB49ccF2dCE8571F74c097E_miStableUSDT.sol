...
contract miStableUSDT is ReentrancyGuard {
    ...
    function setUSDTRatePerMai(uint256 _rate) public {
        require(admin==msg.sender, "ser pls no hack");
        usdtRate = _rate;
    }

    function setMaiRatePerUSDT(uint256 _rate) public {
        require(admin==msg.sender, "ser pls no hack");
        maiRate = _rate;
    }
    ...
    function swapTo(uint256 amount) public nonReentrant {
        ...
        require(usdt.balanceOf(address(this)) >= amountToSend, "swapTo: Not enough Mai in reserves");
