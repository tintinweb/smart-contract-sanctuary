...
contract CitadelCitizens is ERC721, ERC721Enumerable, Ownable, Pausable {
  ...
  uint256 public MAX_MINTABLE = 200;
  ...
  constructor(string memory baseURIVal_, address weth_)
    ERC721("Citadel Citizens", "CitadelCitizens")
  {
  ...
  function airdrop(address[] calldata _recipients) external onlyOwner {
        require(
            totalSupply() + _recipients.length <= (MAX_MINTABLE),
        ...
        for (uint256 i = 0; i < _recipients.length; i++) {
            ...
            _mint(_recipients[i],totalTokens+1);
            totalTokens+=1;
        }
  }
  ...
  function mintTokens(uint256 numberOfTokens) public payable whenNotPaused whenMintingNotComplete mintCountMeetsSupply(numberOfTokens) doesNotExceedMaxPurchaseCount(numberOfTokens) {
  ...
  * we surpass MAX_MINTABLE Poly Riders the mintTokens is
  * no longer available.
  */
  modifier mintCountMeetsSupply(uint256 numberOfTokens) {
    require(
      totalTokens + numberOfTokens <= MAX_MINTABLE,
