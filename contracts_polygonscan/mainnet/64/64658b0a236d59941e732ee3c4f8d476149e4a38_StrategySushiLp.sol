...
abstract contract StrategyBase is IStrategy, ReentrancyGuard, Initializable {
    ...
    function setFirebirdRouter(IFirebirdRouter _firebirdRouter) external onlyTimelock {
        ...
        if (targetCompoundToken != address(0) && targetCompoundToken != farmingToken) IERC20(targetCompoundToken).approve(address(firebirdRouter), type(uint256).max);
        if (targetProfitToken != address(0) && targetProfitToken != targetCompoundToken && targetProfitToken != farmingToken) {
            IERC20(targetProfitToken).approve(address(firebirdRouter), type(uint256).max);
        }
    }
...
contract StrategySushiLp is StrategyBase {
    ...
    function initialize(
        ...
        unirouter = IUniswapV2Router(0x5C6EC38fb0e2609672BDf628B1fD605A523E5923);
    ...
    function _deposit() internal {
        ...
        if (_baseBal > 0) {
            IJetMasterChef(farmPool).deposit(poolId, _baseBal);
            emit Deposit(baseToken, _baseBal);
        }
    }

    function _withdrawSome(uint256 _amount) internal override returns (uint256) {
        (uint256 _stakedAmount, ) = IJetMasterChef(farmPool).userInfo(poolId, address(this));
        if (_amount > _stakedAmount) {
            _amount = _stakedAmount;
        }

        uint256 _before = IERC20(baseToken).balanceOf(address(this));
        IJetMasterChef(farmPool).withdraw(poolId, _amount);
    ...
    function _withdrawAll() internal override {
        (uint256 _stakedAmount, ) = IJetMasterChef(farmPool).userInfo(poolId, address(this));
        IJetMasterChef(farmPool).withdraw(poolId, _stakedAmount);
    }

    function claimReward() public override {
        IJetMasterChef(farmPool).deposit(poolId, 0);
    }
    ...
    function balanceOfPool() public view override returns (uint256) {
        (uint256 amount, ) = IJetMasterChef(farmPool).userInfo(poolId, address(this));
        return amount;
    }
    ...
    function claimable_tokens() external view override returns (address[] memory farmToken, uint256[] memory totalDistributedValue) {
        ...
        totalDistributedValue[0] = IJetMasterChef(farmPool).pendingCake(poolId, address(this));
    }
    ...
    function claimable_token() external view override returns (address farmToken, uint256 totalDistributedValue) {
        ...
        totalDistributedValue = IJetMasterChef(farmPool).pendingCake(poolId, address(this));
    }
    ...
    function retireStrat() external override onlyStrategist {
        IJetMasterChef(farmPool).emergencyWithdraw(poolId);
