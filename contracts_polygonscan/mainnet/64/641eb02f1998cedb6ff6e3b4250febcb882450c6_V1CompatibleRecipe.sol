...
contract UniPieRecipe is IRecipe, Ownable {
    ...
    IERC20 immutable baoToken;
    IPollyToken immutable pollyToken;

    uint256 public burnFee = 25e14;
    uint256 public pollyFee = 85e16;
    uint256 public baoFee = 15e16;
    
    address public baoAddress = 0xc81278a52AD0e1485B7C3cDF79079220Ddd68b7D;
    address public pollyAddress = 0x4C392822D4bE8494B798cEA17B43d48B2308109C;
    ...
    constructor(
        ...
        baoToken = IERC20(baoAddress);
        pollyToken = IPollyToken(pollyAddress);
    }
    ...
    function bake(
        ...
        uint256 feeAmount = ((_maxInput - remainingInputBalance) * burnFee) / (1e18 + 1);
        
        //If we are burning any tokens
        if(remainingInputBalance > 0 && burnFee != 0) {
            WETH.approve(address(sushiRouter), 0);
            WETH.approve(address(sushiRouter), type(uint256).max);
            
            // POLLY BURN (85% of fee)
            if(pollyFee != 0){
                uint256 feeAmountPolly = (feeAmount * pollyFee) / (1e18 + 1);
                address[] memory pollyRoute = getRoute(address(WETH), pollyAddress);
                uint256 estimatedAmountPolly = sushiRouter.getAmountsOut(feeAmountPolly, pollyRoute)[1];
                sushiRouter.swapExactTokensForTokens(feeAmountPolly, estimatedAmountPolly, pollyRoute, address(this), block.timestamp + 1);
                pollyToken.burn(pollyToken.balanceOf(address(this)));
            }
            
            // BAO BURN
            if(baoFee != 0){
                uint256 feeAmountBao = (feeAmount * baoFee) / (1e18 + 1);
                address[] memory baoRoute = getRoute(address(WETH), baoAddress);
                uint256 estimatedAmountBao = sushiRouter.getAmountsOut(feeAmountBao, baoRoute)[1];
                sushiRouter.swapExactTokensForTokens(feeAmountBao, estimatedAmountBao, baoRoute, address(this), block.timestamp + 1);
                baoToken.transfer(feeRecipient, baoToken.balanceOf(address(this)));
            }
        }
    ...
    function setPollyFee(uint256 _newFee) external onlyOwner{
        require(_newFee <= 10**18);
        pollyFee = _newFee;
    }    

    function setBaoToken(address _newFeeToken) external onlyOwner {
        baoAddress = _newFeeToken;
    }

    function setPollyToken(address _newFeeToken) external onlyOwner {
        pollyAddress = _newFeeToken;
    }
