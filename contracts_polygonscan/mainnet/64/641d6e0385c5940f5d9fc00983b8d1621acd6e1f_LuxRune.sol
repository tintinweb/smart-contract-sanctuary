...
/**
    Welcome to LuxRune one of the first play to earn projects on Cronos Chain!
 ...
 * 
 *
 * The LuxRune contract itself has been heavily annotated to allow all frens
 ...
 * or dump from the LuxRune. wallets:
 * 
...
contract LuxRune is Context, ICRC20 {
    ...
    string private constant TOKEN_NAME = "LuxRune";
    
    string private constant TOKEN_TICKER = "LuxRune";
...
constructor (){
        ...
        // Creates the USDC-LuxRune pair on the DEX.
        DEXPair = IDEXFactory(
     ...
     *  (but not buy or receive) LuxRune even if their balance exceeds the max wallet limit.
     ...
     *  LuxRune from a pool if the withdrawal would violate the max Tx or max wallet limits.
     ...
     * It sells half of the LuxRune queued for LP conversion when the threshold is reached, triggered by sells.
     * It then grabs the amount of CRO the swap creates.
     * Finally it combines the CRO with LuxRune to add liquidity to the main WCRO-LuxRune pool to the DEX.
     * Over time, it will cause a build-up of LuxRune in this contract address.
     * This happens because swapping half the LuxRune into CRO reduces the price a bit.
     * In order to mitigate any centralization risk, LuxRune built up in the contract will be recovered and locked
    ...
    /** Returns the total LuxRune supply in uint256 format - 1000000000000000 i.e. 1,000,000 tokens. */
    ...
    /** Returns the token ticker - LuxRune. */
    function symbol() external pure override returns (string memory) {
        return TOKEN_TICKER;   
    }

    /** Returns the token name - LuxRune. */
    function name() external pure override returns (string memory) {
        return TOKEN_NAME;   
    }

    /** Returns the LuxRune balance of the address queried in uint256 format e.g. 5000000000000 = 5,000 tokens. */
     ...
     *  taxes. In this case, the charity mechanism could be achieved by mobilizing LuxRune or
     ...
     * bound is higher at 5%. LuxRune can be deflationary or not - frens decide.
     * 
     ...
     *  that may arise, such as being unable to withdraw LuxRune from a token
     ...
     *  which would result in a significant build-up of LuxRune in this contract address
     *  due to the tax and reflections gained. The LuxRune could then be recovered from
    ...
    /**
     * Function: Allows the owner to set the amount of LuxRune in the token contract
     ...
     *  a number of reasons: at launch when the market cap is low and the number of LuxRune
     ...
     *  low, at 50 for example, LuxRune would build up over time and then as the market cap
     ...
     *  LuxRune in the average Tx decreases relative to the max supply.
     * 
     * Risk statement: Owner could set threshold as low as possible in order to increase the
     *  number of LuxRune that accumulate in the LuxRune contract, then recover them from them
     ...
     *  because they hold most of the LuxRune. The burn address is also excluded for The
     ...
     *  deemed to threaten the future of the project by holding more LuxRune than intended,
     ...
     *  in reflections.
     * 
     * Risk statement: Owner could include the main LuxRune LP address in reflections to
     ...
     *  
     * Justification: This function exists primarily to whitelist LuxRune pools so that 
     ...
     *  issues: a fren may be unable to unstake LuxRune from a pool, harvest from a farm or 
     *  break LP if the Tx would take them over the max wallet limit; a fren may be unable 
     *  to receive LuxRune from giveaways and competitions. These isues are only expected to
     ...
     *  to unstake LuxRune or break LP due to it violating the max Tx limit. It can also be used
    ...
    /**
     * Function: Allows the owner to recover a specific amount of LuxRune from the LuxRune 
     *  contract.
     * 
     * Justification: Someone ending LuxRune to the contract address will almost certainly happen 
     *  at some point. This function allows the owner to recover the LuxRune lost so they can be 
     *  returned to the sender.
     * 
     * Risk statement: It is not expected that the contract will hold LuxRune unless someone
     *  accidentally sends them. Owner could choose not to return the LuxRune but there would
     *  be little incentive to do so.
     * 
     * Mitigation: N/A.
     */
    function recoverLuxRune(uint256 _amount) external onlyOwner() {
        require (_amount <= balanceOf(address(this)), "Not enough LuxRune in contract");
        emit LuxRuneRecovered(payableAddr(), _amount);
     ...
     *  from the LuxRune contract.
     *  
     * Justification: As above but for any token. The owner does not preclude the possibility of 
     *  taking some of this LP, breaking it, distributing the LuxRune back to the community via 
     ...
     * 
     * Risk statement: As above but for any token. Could be called to withdraw LuxRune awaiting
    ...
    /**
     * Function: Allows the owner to withdraw all CRO from the LuxRune contract.
     *  
     * Justification: The reason this is called a recover function is that its intended
     *  use is to recover CRO sent accidentally to the LuxRune contract address. This is
     ...
     *  in LuxRune is swapped for CRO and combined to form LP, the swap itself marginally reduces 
     *  the price of LuxRune. The autoLiquidity function does not account for this the next 
     *  time it is called.
     * 
