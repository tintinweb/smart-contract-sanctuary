...
// File: contracts/interfaces/IPandora.sol
...
// File: contracts/interfaces/IPandoraMetadata.sol


pragma solidity ^0.8.0;

interface IPandoraMetadata {
...
contract Pandora is ERC721Enumerable, Ownable, IPandora, IPandoraMetadata {
  using Strings for uint256;

  uint256 public constant PURCHASE_LIMIT = 5;
  uint256 public constant PRICE = 0 ether;
  uint256 public constant Pandora_GIFT = 300000;
  uint256 public constant Pandora_PUBLIC = 600000;
  uint256 public constant Pandora_MAX = Pandora_GIFT + Pandora_PUBLIC;
  ...
  function purchase(uint256 numberOfTokens) external payable override {
    ...
    require(totalSupply() < Pandora_MAX, 'All tokens have been minted');
     ...
     */
    require(totalPublicSupply < Pandora_PUBLIC, 'Purchase would exceed Pandora_PUBLIC');
    require(
    ...
    for (uint256 i = 0; i < numberOfTokens; i++) {
      /**
       * @dev Since they can get here while exceeding the Pandora_MAX,
       * we have to make sure to not mint any additional tokens.
       */
      if (totalPublicSupply < Pandora_PUBLIC) {
        /**
         * @dev Public token numbering starts after Pandora_GIFT.
         * And we don't want our tokens to start at 0 but at 1.
         */
        uint256 tokenId = Pandora_GIFT + totalPublicSupply + 1;
  ...
  function purchaseWhitelist(uint256 numberOfTokens) external payable override {
    ...
    require(
      totalPublicSupply + numberOfTokens <= Pandora_PUBLIC,
      'Purchase would exceed Pandora_PUBLIC'
    );
    require(
  ...
  function gift(address[] calldata to) external override onlyOwner {
    require(totalSupply() < Pandora_MAX, 'All tokens have been minted');
    require(
      totalGiftSupply + to.length <= Pandora_GIFT,
