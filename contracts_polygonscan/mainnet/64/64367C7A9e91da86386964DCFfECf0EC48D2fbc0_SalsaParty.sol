...
contract SalsaParty is ERC20, Ownable, ERC20Permit {
    ...
    // Min amount for minAmountToLiquify
    uint256 public constant MIN_AMOUNT_LIQUIFY_MIN = 10 ether;
    // The swap router, NOT modifiable.
    IUniRouter02 public immutable salsaSwapRouter;
    // the Salsa swap pair, determined by the router, called after Liquidity
    address public salsaSwapPair = address(0);
    // In swap and liquify
    ...
    event UpdateSalsaSwapPair(address salsaSwapPair);
    ...
    constructor(address feeaddr, address _router) ERC20("SalsaParty", "SALSA") ERC20Permit("SalsaParty") {
        _operator = _msgSender();
        emit OperatorTransferred(address(0), _operator);

        require(feeaddr != address(0x0), 'fee address is zero');
        feeAddress = feeaddr;
    ...
    /// @dev Update the Salsa swap pair automatically from the factory
    function updateSalsaSwapPair() external onlyOperator {
        salsaSwapPair = IUniswapV2Factory(salsaSwapRouter.factory()).getPair(address(this), salsaSwapRouter.WETH());
        emit UpdateSalsaSwapPair(salsaSwapPair);
    }
    ...
    */
    ...
    function setMinAmountToLiquify(uint256 amount) external onlyOperator {
