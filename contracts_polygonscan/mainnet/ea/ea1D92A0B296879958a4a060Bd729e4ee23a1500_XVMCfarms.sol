...
contract XVMCfarms is Ownable {
    ...
    */    
    function initiateFarmProposal(
    	...
    	require(poolid == 45 || poolid == 46 || poolid == 11 || poolid == 34 || poolid == 35, "only allowed for these pools"); 
		
		//cumulative allocation for pool 0 and 1 can be max 5%
		//you can propose max 5% each, but it can not be enforced
    	if(poolid == 45 || poolid == 46) {
    	    require(
    ...
    */
    function updateFarm(uint256 proposalID) external {
		...
		uint256 _allocPoint;
        if(_poolID == 45) {
			(, _allocPoint, , , ) = IMasterChef(masterchef).poolInfo(46);
			require(_allocPoint + _newAllocation <= (IMasterChef(masterchef).totalAllocPoint() * 50 / 1000) );
		} else if(_poolID == 46) {
			(, _allocPoint, , , ) = IMasterChef(masterchef).poolInfo(45);
			require(_allocPoint + _newAllocation <= (IMasterChef(masterchef).totalAllocPoint() * 50 / 1000) );
		}
    ...
    */
    function enforceRewardReduction() public {
        ...
        if (IMasterChef(masterchef).XVMCPerBlock() > maxRewards && !isReductionEnforced) {
            ...
            (, allocPoint, , , depositFeeBP) = IMasterChef(masterchef).poolInfo(45);
            IXVMCgovernor(owner()).setPool(
                45, allocPoint * farmMultiplierDuringBoost / 10000, depositFeeBP, false
            );
            
            (, allocPoint, , , depositFeeBP) = IMasterChef(masterchef).poolInfo(46);
            IXVMCgovernor(owner()).setPool(
                46, allocPoint * farmMultiplierDuringBoost / 10000, depositFeeBP, false
            );
        ...
        //the mutliplier shall not change during event!
            (, allocPoint, , , depositFeeBP) = IMasterChef(masterchef).poolInfo(45);
            IXVMCgovernor(owner()).setPool(
                45, allocPoint * 10000 / farmMultiplierDuringBoost, depositFeeBP, false
            );
            
            (, allocPoint, , , depositFeeBP) = IMasterChef(masterchef).poolInfo(46);
            IXVMCgovernor(owner()).setPool(
                46, allocPoint * 10000 / farmMultiplierDuringBoost, depositFeeBP, false
            );
