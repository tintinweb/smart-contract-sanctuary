...
contract MeowAPI is Ownable {
  ...
  function rewardAPY(address _vault, uint256 _pid) public view returns (uint256) {
    ...
    uint256 dominator = totalStake(_pid).mul(price).div(10**decimals);
    return dominator == 0 ? 0 : numerator.div(dominator);
  }
  ...
  function borrowerRewardAPY(address _vault, uint256 _pid) public view returns (uint256) {
    ...
    uint256 dominator = vaultDebtVal(_vault).mul(price).div(10**decimals);
    return dominator == 0 ? 0 : numerator.div(dominator);
  }
  ...
  function getLPValue(address _worker) public view returns (uint256) {
    ...
    return (IERC20(baseToken).balanceOf(lpToken)).mul(uint256(2).mul(tokenPrice).div(10**decimals));
  }
  ...
  function getRewarder(address _worker) public view returns (address) {
    uint256 pid = getPoolIdSushi(_worker);
    address masterChef = getMasterChef(_worker);
    return IMiniChefV2(masterChef).rewarder(pid);
  }
  ...
  function getPendingReward(address _worker) public view returns (uint256) {
    uint256 pid = getPoolIdSushi(_worker);
    address rewarder = getRewarder(_worker);
    (, uint256[] memory rewardAmounts) = IRewarder(rewarder).pendingTokens(pid, _worker, 0);
  ...
  //       balance[i] = IStakingRewards(stakingRewards).totalSupply();
  //       _totalSupply[i] = IUniswapV2Pair(lp).totalSupply();
  //       lpBaseTokenAmount[i] = IERC20(_baseToken).balanceOf(lp);
  //       (, , uint64 _alloc) = IMiniChefV2(masterChef).poolInfo(pid);
  //       allocPoint[i] = uint256(_alloc);
  ...
  function getTVLSushiswap(address[] memory _workers) public view returns (bytes memory) {
    ...
    uint256[] memory pendingReward = new uint256[](len);
    ...
    for (uint256 i = 0; i < _workers.length; i++) {
      ...
      pendingReward[i] = getPendingReward(worker).add(IERC20(wMatic).balanceOf(worker));
      uint256 _totalSupply = IUniswapV2Pair(lp).totalSupply();
      uint256 _tvl = getLPValue(worker).mul(getWorkerStakeAmountSushi(worker)).div(uint256(_totalSupply));
      tvl[i] = _tvl;
      workersList[i] = worker;
      totalTVL = totalTVL.add(_tvl);
    }
    return abi.encode(bps, pendingSushi, pendingReward, tvl, workersList, totalTVL);
  }

  // Return Quickswap TVL.
  function getTVLQuickswap(address[] memory _workers) public view returns (bytes memory) {
    uint256 len = _workers.length;
    uint256 totalTVL = 0;
    uint256[] memory bps = new uint256[](len);
    uint256[] memory pendingQuick = new uint256[](len);
    uint256[] memory tvl = new uint256[](len);
    address[] memory workersList = new address[](len);
    for (uint256 i = 0; i < _workers.length; i++) {
      address worker = _workers[i];
      address lp = getLpToken(worker);
      address stakingRewards = getStakingRewards(worker);
      address quick = getQuickAddr(worker);
      bps[i] = getReinvestBounty(worker);
      pendingQuick[i] = IStakingRewards(stakingRewards).earned(worker);
      uint256 amount = getWorkerStakeAmountQuick(worker);
      uint256 _totalSupply = IUniswapV2Pair(lp).totalSupply();
    ...
    return abi.encode(bps, pendingQuick, tvl, workersList, totalTVL);
  }
}
