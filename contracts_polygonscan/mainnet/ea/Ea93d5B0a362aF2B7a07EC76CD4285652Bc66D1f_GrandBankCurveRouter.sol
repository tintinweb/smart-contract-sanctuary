...
interface ICurveRouterV2 {
      ...
      // function exchange(int128 i, int128 j, uint256 dx, uint256 min_dy) external returns (uint256 dy);
      // def exchange_underlying(i: int128, j: int128, dx: uint256, min_dy: uint256) -> uint256:
      function exchange_underlying(uint256 i, uint256 j, uint256 dx, uint256 min_dy) external returns (uint256 dy);
}
...
contract GrandBankCurveRouter is IGBTradingRoute, WhitelistedRole, ReentrancyGuard {
        ...
        returns(uint256 i, uint256 j)
    {
        ...
        j = _dest == wbtc ? 3 : j;
        j = _dest == weth ? 4 : j;
        require(i != uint256(-1) && j != uint256(-1), "GBCurveRouter: Tokens not supported!");
    }
}