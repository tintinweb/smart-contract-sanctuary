...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 accGammaPulsarPerShare;   
        uint256 harvestInterval;
        uint16 depositFeeBP;  
    }

    GammaPulsarToken public gammaPulsar;
    address public devAddress;
    address public feeCommissionAddress;
    address public feeBurnAddress;
    uint256 public gammaPulsarPerBlock;
    uint256 public constant commission = 5000;
    uint256 public constant forBurn = 5000;
    uint256 public constant BONUS_MULTIPLIER = 1;
    uint256 public constant MAXIMUM_HARVEST_INTERVAL = 14 days;
    uint256 public constant MAXIMUM_DEPOSIT_FEE = 2000;
    ...
    event SetFeeBurnAddress(address indexed user, address indexed newFeeBurnAddress);
    ...
    constructor(
        ...
        gammaPulsarPerBlock = _gammaPulsarPerBlock;
    ...
    function pendingGammaPulsar(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 accGammaPulsarPerShare = pool.accGammaPulsarPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 gammaPulsarReward = multiplier.mul(gammaPulsarPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accGammaPulsarPerShare = accGammaPulsarPerShare.add(gammaPulsarReward.mul(1e18).div(lpSupply));
        }
        uint256 pending = user.amount.mul(accGammaPulsarPerShare).div(1e18).sub(user.rewardDebt);
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 gammaPulsarReward = multiplier.mul(gammaPulsarPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        gammaPulsar.mint(devAddress, gammaPulsarReward.div(10));
        gammaPulsar.mint(address(this), gammaPulsarReward);
        pool.accGammaPulsarPerShare = pool.accGammaPulsarPerShare.add(gammaPulsarReward.mul(1e18).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        updatePool(_pid);
        payOrLockupPendingGammaPulsar(_pid);
        if (_amount != 0) {
            ...
            if (pool.depositFeeBP != 0) {
                ...
                uint256 depositFeeForBurn = depositFee.sub(depositFeeCommission);
                pool.lpToken.safeTransfer(feeCommissionAddress, depositFeeCommission);
                pool.lpToken.safeTransfer(feeBurnAddress, depositFeeForBurn);
                user.amount = user.amount.add(_amount).sub(depositFee);
            } else {
                user.amount = user.amount.add(_amount);
            }
        }
        user.rewardDebt = user.amount.mul(pool.accGammaPulsarPerShare).div(1e18);
    ...
    function payOrLockupPendingGammaPulsar(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        uint256 pending = user.amount.mul(pool.accGammaPulsarPerShare).div(1e18).sub(user.rewardDebt);
        if (canHarvest(_pid, msg.sender)) {
            if (pending != 0 || user.rewardLockedUp != 0) {
                ...
                // send rewards
                safeGammaPulsarTransfer(msg.sender, totalRewards);
            }
        } else if (pending != 0) {
    ...
    function safeGammaPulsarTransfer(address _to, uint256 _amount) internal {
        uint256 gammaPulsarBal = gammaPulsar.balanceOf(address(this));
        if (_amount > gammaPulsarBal) {
            gammaPulsar.transfer(_to, gammaPulsarBal);
        } else {
            gammaPulsar.transfer(_to, _amount);
        }
    }
    ...
    function setFeeCommissionAddress(address _feeCommissionAddress) external {
        require(msg.sender == feeCommissionAddress, "setFeeAddress: FORBIDDEN");
        feeCommissionAddress = _feeCommissionAddress;
        
        emit SetFeeBurnAddress(msg.sender, _feeCommissionAddress);
    }
    function setFeeBurnAddress(address _feeBurnAddress) external {
        require(msg.sender == feeBurnAddress, "setFeeAddress: FORBIDDEN");
        feeBurnAddress = _feeBurnAddress;
        
        emit SetFeeBurnAddress(msg.sender, _feeBurnAddress);
    }
    //Pancake has to add hidden dummy pools inorder to alter the emission, here we make it simple and transparent to all.
    function updateEmissionRate(uint256 _gammaPulsarPerBlock) external onlyOwner {
        massUpdatePools();
        emit EmissionRateUpdated(msg.sender, gammaPulsarPerBlock, _gammaPulsarPerBlock);
        gammaPulsarPerBlock = _gammaPulsarPerBlock;
    }
}