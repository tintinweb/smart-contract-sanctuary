...
contract StrategyKillIs3Usd is Ownable, ReentrancyGuard, Strategy {
  ...
  function execute(
    ...
    address borrow = msg.sender;
    IBorrow bContract = IBorrow(borrow);
    address bank = bContract.operator();
    address banktoken = bContract.bankcurrency();
    address config = IBank(bank).config();
    address bConfig = IConfig(config).bigfoots(borrow);
    ...
    require(IBConfig(bConfig).isStable(borrow),"not stable");
