...
contract StrategySing is StratManager, FeeManager {
    ...
    function addLiquidity() internal {
        ...
        if (lpToken0 != native) {
            IUniswapRouterETH(unirouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(nativeHalf, 0, nativeToLp0Route, address(this), now);
        }

        if (lpToken1 != native) {
            IUniswapRouterETH(unirouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(nativeHalf, 0, nativeToLp1Route, address(this), now);
        }
