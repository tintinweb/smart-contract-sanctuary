...
contract pMintyMultiToken is ContextMixin, ERC1155, NativeMetaTransaction {
    ...
    constructor(
        ...
        INewentProxy _registry
    ) ERC1155("") {
        require(_registry.isRegistry());
        require(_royaltyPerMille < 500, "royalties should never be more than 50%");
        art_owner = __owner;
        ...
        setApprovalForAll(saleContract, true);
        registry = _registry;
    }

    function updateRoyalties(uint newRoyaltyPerMille) external {
        require ( msgSender() == deployer ||  auth[msgSender()] || (msgSender() == art_owner) , "Unauthorised");
        require(newRoyaltyPerMille < 500, "royalties should never be more than 50%");
        royaltyPerMille = newRoyaltyPerMille;
    }
