...
// File contracts/interfaces/IXswapFarm.sol
...
abstract contract StrategyV3 is Ownable, ReentrancyGuard, Pausable {
    ...
    address[] public WMATICToNATIVEPath;
    ...
    function buyBack(uint256 _earnedAmt) internal virtual returns (uint256) {
        ...
        if (uniRouterAddress != buybackRouterAddress) {
            ...
            if (earnedAddress != wmaticAddress) {
                // First convert earn to wmatic
                IERC20(earnedAddress).safeIncreaseAllowance(
                    uniRouterAddress,
                    buyBackAmt
                );
            ...
            // convert all wmatic to Native and burn them
    ...
    function distributeFees(uint256 _earnedAmt) internal virtual returns (uint256) {
        if (_earnedAmt > 0) {
            ...
            if (controllerFee > 0) {
                ...
                _earnedAmt = _earnedAmt.sub(fee.mul(2));
            }
        }
    ...
    function _wrapMATIC() internal virtual onlyAllowGov {
        // MATIC -> WMATIC
        uint256 maticBal = address(this).balance;
        if (maticBal > 0) {
            IWMATIC(wmaticAddress).deposit{value: maticBal}(); // MATIC -> WMATIC
        }
    }
...
// File contracts/StrategyV3_Quickswap.sol

pragma solidity 0.6.12;

interface IXQuickSwapFarm {
    ...
    // get Pending external farm
    function earned(address account) external view returns (uint256);
}

contract StrategyV3_Quickswap is StrategyV3 {
    ...
    constructor(
        ...
        nativeFarmAddress = 0xa7839D6EcbB54d0ae04cF7f58cdA93FC5Ac96f95;
        farmContractAddress = _farmContractAddress;
        govAddress = msg.sender;
        uniRouterAddress = 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff;
        buybackRouterAddress = 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff;

        NATIVEAddress = 0x40038C83E459937A988b669f1159cC78d8fdad68;
        wmaticAddress = 0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270;
        wantAddress = _tokenAddresses[0];
        earnedAddress = _tokenAddresses[1];
        earned0Address = 0x0000000000000000000000000000000000000000;
        ...
        WMATICToNATIVEPath = [wmaticAddress, NATIVEAddress];
    ...
    function _farm() internal override {
        ...
        IXQuickSwapFarm(farmContractAddress).stake(wantAmt);
    }

    function _unfarm(uint256 _wantAmt) internal override {
        IXQuickSwapFarm(farmContractAddress).withdraw(_wantAmt);
    }

    function _harvest() internal override {
        IXQuickSwapFarm(farmContractAddress).getReward();
    }
    
    function workerRedward() public view returns (uint256){
        uint256 WorkerComp;
        WorkerComp = IXQuickSwapFarm(farmContractAddress).earned(address(this));
        uint256 buyBackAmt = WorkerComp.mul(buyBackRate).div(buyBackRateMax);
        uint256 fee = buyBackAmt.mul(controllerFee).div(controllerFeeMax);
