...
// PTrollToken
contract PTrollToken is ERC20('PRE-TROLL', 'PTROLL'), ReentrancyGuard {

    address public feeAddress;

    uint256 public salePriceE35 = 2 * (10 ** 35);

    uint256 public constant pTrollMaximumSupply = 60000* (10 ** 18);

    // We use a counter to defend against people sending plith back
    uint256 public pTrollRemaining = pTrollMaximumSupply;

    uint256 public constant maxPTrollPurchase = 600 * (10 ** 18);

    uint256 public StartTime;
    uint256 public EndTime;

    mapping(address => uint256) public userPTrollTally;

    event pTrollPurchased(address sender, uint256 maticSpent, uint256 plithReceived);
    event StartTimeChanged(uint256 newStartTime, uint256 newEndTime);
    event salePriceE35Changed(uint256 newSalePriceE5);

    constructor(address _feeAddress,uint256 _StartTime,uint256 _EndTime) {
        feeAddress = _feeAddress;
        StartTime = _StartTime;
        EndTime   = _EndTime;
        _mint(address(this), pTrollMaximumSupply);
    }

    function buyPTroll() external payable nonReentrant {
        ...
        require(pTrollRemaining > 0, "No more plith remaining! Come back next time!");
        ...
        require(userPTrollTally[msg.sender] < maxPTrollPurchase, "user has already purchased too much plith");

        uint256 originalPlithAmount = (msg.value * salePriceE35) / 1e35;

        uint256 pTrollPurchaseAmount = originalPlithAmount;

        if (pTrollPurchaseAmount > maxPTrollPurchase)
            pTrollPurchaseAmount = maxPTrollPurchase;

        if ((userPTrollTally[msg.sender] + pTrollPurchaseAmount) > maxPTrollPurchase)
            pTrollPurchaseAmount = maxPTrollPurchase - userPTrollTally[msg.sender];

        // if we dont have enough left, give them the rest.
        if (pTrollRemaining < pTrollPurchaseAmount)
            pTrollPurchaseAmount = pTrollRemaining;

        require(pTrollPurchaseAmount > 0, "user cannot purchase 0 plith");
        ...
        if (pTrollPurchaseAmount < originalPlithAmount) {
            // max pTrollPurchaseAmount = 6e20, max msg.value approx 3e22 (if 10c matic, worst case).
            ...
            maticSpent = ((pTrollPurchaseAmount * msg.value * 1e24) / originalPlithAmount) / 1e24;
        ...
        emit pTrollPurchased(msg.sender, maticSpent, pTrollPurchaseAmount);
    }
    ...
    function setSalePriceE35(uint256 _newSalePriceE35) external onlyOwner {
        require(block.timestamp < EndTime, "Cannot change time after presale ends !!!");
        require(_newSalePriceE35 >= 125 * (10 ** 33), "new price can't too low");
        require(_newSalePriceE35 <= 3 * (10 ** 35), "new price can't too high");
