...
contract EtheremonWorldNFT is BasicAccessControl, NativeMetaTransaction {
    ...
    uint256 public emonRatio = 13367;

    constructor(string memory name, address _emon)
        public
    ...
    function setEmonRatio(uint256 _emonRatio)
        external
        onlyModerators
    {
        ...
        returns (bool catchable, uint256 price)
    {
        ...
        if (class.total > 0)
            price += (class.price * emonRatio * (class.total - 1)) / priceIncreasingRatio;
    ...
    //Calculate price in EMON
    function catchMonsterNFT(
        uint32 _classId,
        string calldata _name,
        uint256 amount
    ) external isActive {
    ...
    //MATIC: CHANGES FOR ETH ON MATIC
    //Calculate price in EMON
    function catchMonster(
        ...
        if (addressWhitelist[msgSender()] == false) {
            revert();
        }
        ...
        uint8 value;
        uint256 seed = getRandom(_player, block.number - 1, objId);
        // generate base stat for the previous one
        for (uint256 i = 0; i < STAT_COUNT; i += 1) {
            seed /= 100;
            value =
