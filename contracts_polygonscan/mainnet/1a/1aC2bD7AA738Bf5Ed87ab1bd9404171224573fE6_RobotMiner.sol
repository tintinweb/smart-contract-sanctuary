    ...
    IGame
{
    ...
    function mining(
        ...
        bool[] memory pick = new bool[](totalMeteor);
        uint8 count;
        if (_meteor0) {
            pick[0] = true;
            count++;
        ...
        require(count == 3, "RobotMiner.sol: Need to select 3 meteors.");

        uint128 prize = getWinnerPrize(_betAmount);
        vault.addReserveOperating(prize - _betAmount);
        vault.addGameBalance(msg.sender, _betAmount);

        uint8 badMeteor = uint8(generateRandom() % totalMeteor);
        if (pick[badMeteor]) {
            vault.subReserveOperating(prize);
            prize = 0;
        }
        vault.subGameBalance(msg.sender, prize, 0);

        emit Mining(++gId, msg.sender, _betAmount, pick, badMeteor, prize);
    }

    function generateRandom() internal returns (uint256 number) {
        number = uint256(
            keccak256(
                abi.encode(
                    block.timestamp,
    ...
    function isPlayerStillPlaying() external pure override returns (bool) {
        return false; // no need to check before delete game
    }
