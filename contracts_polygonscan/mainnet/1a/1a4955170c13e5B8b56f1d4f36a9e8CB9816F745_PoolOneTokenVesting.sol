...
/** 
 *  SourceUnit: /Users/dan/HAI/Contracts-Monorepo/contracts/tge/PoolOneTokenVesting.sol
*/
 ...
 * @title TokenVesting
 */
contract TokenVestingBase is Ownable, ReentrancyGuard {
     ...
     */
    constructor(address token_, address multisig) {
...
contract BuyerTokenVestingBase is TokenVestingBase {
    constructor(
        address token_,
        address multisig,
        VestingBaseInput[] memory buyers
    ) public TokenVestingBase(token_, multisig) {
        for (uint256 i = 0; i < buyers.length; i++) {
            VestingBaseInput memory buyerData = buyers[i];
            _createVestingSchedule(
                buyerData.user,
                block.timestamp,
                0,
                ...
                2 weeks,
                false,
                // enter in 75% of their amount. the other 25% we'll send directly at TGE via Disberse
                buyerData.amount
            );
        }
    }
...
////import "./BuyerTokenVestingBase.sol";

contract PoolOneTokenVesting is BuyerTokenVestingBase {
    constructor(
        address token_,
        address multisig,
        VestingBaseInput[] memory buyers
    ) public BuyerTokenVestingBase(token_, multisig, buyers) {}
}