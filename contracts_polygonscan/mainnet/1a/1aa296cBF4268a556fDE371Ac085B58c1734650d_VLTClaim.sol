...
contract GotchiVault is IERC173, Initializable,PausableUpgradeable, IERC721ReceiverUpgradeable, IERC1155ReceiverUpgradeable {
    ...
    function resetUserArray(address _tokenAddress, address _user) public onlyOwner{
    ...
    function ReturnERC721toOwner(address _tokenAddress, uint256 _id) public onlyOwner{
       ...
       /* //reset our internal tracking of deposits -- address(0) is the default state for mapping addresses
            s.deposits[_tokenAddress][_id] = address(0);
            //remove this from the array of depositor tokens
            if(s.tokenIdsByOwner[_tokenAddress][msg.sender].length > 1){
                // Get the index of the token that has to be removed from the list
                // of tokens owned by the current owner.
                uint256 tokenIndexToDelete = s.ownerTokenIndexByTokenId[_tokenAddress][msg.sender][_id];
...
//this feature is not live yet.  the concept is for the multi-sig owners
//to define these parameters, but allow a single wallet to execute minting, 
        ...
        for(uint i = 0; i < 7; i++){
            ...
            if(mintingSettings.mintPercentages[i] > 0){
                ...
                if(ticketsToBuy > 0){
                    ...
                    IAavegotchi(diamondAddress).setERC1155Listing(stakingAddress, i, ticketsToBuy, mintingSettings.salesPrices[i]);
                }
