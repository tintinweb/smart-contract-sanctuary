...
contract MetaDiceV1 is VRFConsumerBase, Ownable, ReentrancyGuard {
    ...
    uint256 constant MAX_MODULO = 102;
    ...
    uint256 public maxSlotsPayout;

    mapping(uint256 => mapping(uint256 => mapping(uint256 => uint256))) public slotsPayouts;
    ...
    uint256 public wealthTaxThreshold = 500 ether;
    ...
    function setHouseEdgeBP(uint256 _houseEdgeBP) external onlyOwner {
        require(_houseEdgeBP < 1000, "Maximum 10%");
        houseEdgeBP = _houseEdgeBP;
    }

    function setWealthTaxBP(uint256 _wealthTaxBP) external onlyOwner {
        require(_wealthTaxBP < 1000, "Maximum 10%");
        wealthTaxBP = _wealthTaxBP;
    }

    function setWealthTaxThreshold(uint256 _wealthTaxThreshold) external onlyOwner {
        require(_wealthTaxThreshold > 0, "Zero value");
    ...
    function getHouseEdgeFee(uint256 amount) private view returns (uint256) {
        return (amount * (houseEdgeBP + getEffectiveWealthTaxBP(amount))) / 10000;
    }
    ...
    function getPossibleWinAmount(
        ...
        uint256 houseEdgeFee = getHouseEdgeFee(amount);
        if (modulo == 101) {
            return ((amount - houseEdgeFee) * betMask) / 100;
        } else if (modulo == 102) {
            return (amount - houseEdgeFee) * maxSlotsPayout;
        } else {
            require(0 < rollUnder && rollUnder <= modulo, "Win probability out of range");
            return ((amount - houseEdgeFee) * modulo) / rollUnder;
        }
    }

    function getEffectiveWealthTaxBP(uint256 amount) private view returns (uint256 effectiveWealthTaxBP) {
    ...
    {
        uint256 wheelsLength = wheel1.length;
    ...
    function extraRandomness(uint256 number) private pure returns (uint256) {
    ...
    function getSlotsPayout(
        ...
        if (w1 <= w2) {
            if (w2 <= w3) {
                return slotsPayouts[w1][w2][w3];
            } else {
                if (w1 <= w3) {
                    return slotsPayouts[w1][w3][w2];
                } else {
                    return slotsPayouts[w3][w1][w2];
                }
            }
        } else {
            if (w2 <= w3) {
                if (w1 <= w3) {
                    return slotsPayouts[w2][w1][w3];
                } else {
                    return slotsPayouts[w2][w3][w1];
                }
            } else {
                return slotsPayouts[w3][w2][w1];
            }
        }
    }

    function placeBet(uint256 betMask, uint8 modulo) external payable nonReentrant {
        ...
        uint8 rollUnder;
        if (modulo == 101) {
            require(betMask > 100 && betMask <= MULTIPLIER_LIMIT * 100, "Target not within range");

            mask = betMask;
        } else if (modulo == 102) {
            require(betMask >= 1 && betMask <= 7, "Mask not within range");
        ...
        uint256 possibleWinAmount = getPossibleWinAmount(modulo, amount, betMask, rollUnder);

        require(possibleWinAmount <= amount + maxProfit(), "maxProfit violation");

        uint256 newLockedInBets = lockedInBets + possibleWinAmount;
        require(newLockedInBets <= address(this).balance, "Insufficient funds");
        ...
        emit BetPlaced(betId, msg.sender, amount, modulo, rollUnder, mask);
    }
    ...
    function settleBet(bytes32 requestId, uint256 randomNumber) internal nonReentrant {
        ...
        uint256 possibleWinAmount = getPossibleWinAmount(modulo, amount, mask, rollUnder);

        uint256 outcome;
        uint256 winAmount = 0;
        if (modulo == 101) {
            outcome = (MULTIPLIER_LIMIT * 100) / (MULTIPLIER_LIMIT - (randomNumber % MULTIPLIER_LIMIT));
            ...
            uint256 result1 = randomNumber % wheelsLength;

            randomNumber = extraRandomness(randomNumber);
            uint256 result2 = randomNumber % wheelsLength;

            randomNumber = extraRandomness(randomNumber);
            uint256 result3 = randomNumber % wheelsLength;
            ...
            if (totalPayout > 0) {
                winAmount = ((amount - getHouseEdgeFee(amount)) * totalPayout) / numBets;
            }
        } else {
            outcome = randomNumber % modulo;
        ...
        if (winAmount > 0) {
            if (winAmount >= amount) {
                houseProfit = houseProfit - int256(winAmount - amount);
            } else {
                houseProfit = houseProfit + int256(amount - winAmount);
            }

            gambler.transfer(winAmount);
        } else {
            houseProfit = houseProfit + int256(amount);
        }

        emit BetSettled(betId, gambler, amount, modulo, rollUnder, mask, outcome, winAmount);
    }

    function refundBet(uint256 betId) external nonReentrant {
        Bet storage bet = bets[betId];

        require(bet.status == Status.CREATED, "Bet is settled already");
        ...
        emit BetRefunded(betId, gambler, amount);
    }
}