...
contract ZAIN is ERC20, Ownable {
    // a mapping from an address to whether or not it can mint / burn
    mapping(address => bool) controllers;

    constructor() ERC20("Zainist", "ZAIN") {}

    /**
     * mints $Zainist to a recipient
     * @param to the recipient of the $Zainist
     * @param amount the amount of $Zainist to mint
     */
    ...
    /**
     * burns $Zainist from a holder
     * @param from the holder of the $Zainist
     * @param amount the amount of $Zainist to burn
     */
...
contract Zainist is IZainist, ERC721Enumerable, Ownable, Pausable {
    ...
    IBarn public barn;
    // reference to $Zainist for burning on mint
    ZAIN public zain;
    // reference to Traits
     ...
     * The first 20% are free to claim, the remaining cost $ZAIN
    ...
    function mint(uint256 amount) external payable whenNotPaused {
        ...
        for (uint256 i = 0; i < amount; i++) {
            ...
            totalZainCost += mintCost(minted);
            _safeMint(recipient, minted);
        }
        if (totalZainCost > 0) zain.burn(msgSender, totalZainCost);
    }

    /**
     * the first 20% are paid in MATIC
     * the next 20% are 20000 $Zainist
     * the next 20% are 30000 $Zainist
     * the next 20% are 40000 $Zainist
     * the final 20% are 50000 $Zainist
