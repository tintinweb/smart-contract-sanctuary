...
interface IMasterChefV2{
    ...
    function poolInfo(uint _pid) external view returns (address, uint256, uint256, uint256, uint16);
    function userInfo(uint _pid, address user) external view returns (uint256, uint256);
    ...
    function updateEmissionRate(uint256 _eggPerBlock) external;
}

// File: Vault/libs/IStrategyVertex.sol



pragma solidity 0.6.12;

interface IStrategyVertex {
...
// File: Vault/libs/UniswapV2Library.sol
...
abstract contract BaseStrategy is Ownable, ReentrancyGuard, Pausable {
    ...
    address public constant vertexAddress = 0x72572CCf5208b59f4BcC14e6653d8c31Cd1fC5a0;
    address public constant rewardAddress = 0xd624950ccBC809b81D929F551554E109448df413;
    address public constant withdrawFeeAddress = 0x8847A75c52dC68cAe2594b12E143e960df17a9B1;
    address public constant depositFeeAddress = 0x8847A75c52dC68cAe2594b12E143e960df17a9B1;
    address public constant feeAddress = 0xD7050572bfA95f87a8fC244EEEAB54286Ab8BFdB;
    ...
    uint256 public depositFeeFactor = 10000; // 0% deposit fee
    uint256 public constant depositFeeFactorMax = 10000;
    uint256 public constant depositFeeFactorLL = 9900;
    ...
    address[] public earnedToVertexPath;
    ...
    function deposit(address _userAddress, uint256 _wantAmt) external onlyOwner nonReentrant whenNotPaused returns (uint256) {
        ...
        // Proper deposit amount for tokens with fees, or vaults with additional deposit fees
    ...
    function _farm() internal returns (uint256) {
        ...
        // Use before/after method to account for transfer fees
        uint256 sharesBefore = vaultSharesTotal();
         
         uint256 depositFee = wantAmt
            .mul(depositFeeFactorMax.sub(depositFeeFactor))
            .div(depositFeeFactorMax);
    ...
    function distributeRewards(uint256 _earnedAmt) internal returns (uint256) {
        if (rewardRate > 0) {
            ...
            IStrategyVertex(rewardAddress).depositReward(usdcAfter);
    ...
    function setSettings(
        ...
        require(_depositFeeFactor >= depositFeeFactorLL, "_depositFeeFactor too low");
        require(_depositFeeFactor <= depositFeeFactorMax, "_depositFeeFactor too high");
...
contract VotingPower is Ownable {
    address public constant usdcAddress = 0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174;
    address public constant vertexAddress = 0x72572CCf5208b59f4BcC14e6653d8c31Cd1fC5a0;
    address public constant vertexUsdcLPAddress = 0x668269d6E5D2c2dE31D132Ac218044211643622B;
    address public constant vaultChefAddress = 0x434A59C7F85b25415ffA86D5B46cCa0A34d1A430;
    address public constant masterChefAddress = 0x8bE82Ab9B6179bE6EB88431E3E4E0fd93b9E607C;
    
    address[] public vertLPs = [0x668269d6E5D2c2dE31D132Ac218044211643622B, 0xfCD877cb5C3ecBF51E329DF525213BCeEEF741Dd, 0xd4689694e9928564647aD483c075F271419B2A5f, 0xfef87340394bC37B8D54cDe7c2b37E616d741B31]; 
 
    bool public useUSDCForVotingPower = false; // Voting power is in USDC or Vertex?
    
    uint16 public lpVotingPowerModifier = 2; // VERT in LP pairs can count for extra
    
        
    function setLPVotingPowerModifier(uint16 _mod) public onlyOwner{
        lpVotingPowerModifier = _mod;
    }
    
    
    function setUseUSDC(bool _use) public onlyOwner{
         useUSDCForVotingPower = _use;
    }
    
     
    function setVertLPs(address[] calldata _vertLPs) public onlyOwner{
         vertLPs = _vertLPs;
    }
    
     
    function getVertInLP(uint lpShares, IERC20 lpToken) public view returns (uint256){
        // Amount of VERT in an LP pair proportional to lpShares/(totalSupply of LP)
        uint ratioWantStakedByUser = 10**18 *  lpShares / lpToken.totalSupply();
        uint vertStakedByVoter = (ratioWantStakedByUser * IERC20Detailed(vertexAddress).balanceOf(address(lpToken)))/10**18;
        return vertStakedByVoter;
     }
     
    function getUsdcPerVert() public view returns (uint256){
        // Ratio of USDC to VERT (in USDC decimals)
        uint256 vertexLiquidity = IERC20Detailed(vertexAddress).balanceOf(address(vertexUsdcLPAddress));
        uint256 usdcLiquidity = IERC20Detailed(usdcAddress).balanceOf(vertexUsdcLPAddress);
        usdcLiquidity = usdcLiquidity * 10**12 * 10**6;
        uint256 usdcPerVert = usdcLiquidity / (vertexLiquidity);
    
        return usdcPerVert;
     
    }
     
    function getVertStakedInVaults(address voter) public view returns (uint256) {
        // VERT staked by voter in Vertex vaults
        VaultChef chef = VaultChef(vaultChefAddress);
        uint poolLength = chef.poolLength();
        uint total = 0;
        for (uint256 i=0; i<poolLength; i++) {
            (IERC20 wantToken, address strat) = chef.poolInfo(i);
            
            if(BaseStrategy(strat).earnedToToken0Path(0)!=vertexAddress){
                continue; // non-VERT vault
            }
            uint256 shares = chef.stakedWantTokens(i, voter);
           
            uint vertStakedByVoter = lpVotingPowerModifier*getVertInLP(shares, wantToken);
            total+=vertStakedByVoter;
        }
        
        return total;
    }
        
    function getVertStakedInMasterChef(address voter) public view returns (uint256) {
         // VERT staked by voter in Vertex farms/pools
        IMasterChefV2 chef = IMasterChefV2(masterChefAddress);
        uint poolLength = chef.poolLength();
        uint total = 0;
        for (uint256 i=0; i<poolLength; i++) {
            (   address lpToken,
                ...
                uint16 depositFeeBP) = chef.poolInfo(i);
            
            (uint256 amount, uint256 rewardDebt) = chef.userInfo(i, voter);

            total+=chef.pendingEgg(i, voter);
           
            if(address(lpToken)==vertexAddress){
                // Vert pool
                total += amount;
                continue;
            } // else LP Farm
            
            uint vertStakedByVoter = lpVotingPowerModifier*getVertInLP(amount, IERC20(lpToken));
            total+=vertStakedByVoter;
        }
    ...
    function getTotalVertStaked(address voter) public view returns (uint256){
         // VERT staked by voter in vaults or farms/pools
        return getVertStakedInVaults(voter) + getVertStakedInMasterChef(voter);
    }
    
    
    function getVertStakedAsUsdc(address voter) public view returns (uint256){
        return getUsdcPerVert() * getTotalVertStaked(voter) / 10**6;
    }

    
    function getVertHeld(address voter) public view returns (uint256){
         // VERT held in voter's wallet
        return IERC20Detailed(vertexAddress).balanceOf(voter);
    }
    
    
    function getVertHeldLP(address voter) public view returns (uint256){
        // VERT in LPs held in voter's wallet
        uint256 total = 0;
        for(uint i = 0; i< vertLPs.length; i++){
            uint256 shares = IERC20Detailed(vertLPs[i]).balanceOf(voter);
            if(shares>0){
                total+= lpVotingPowerModifier*getVertInLP(shares, IERC20(vertLPs[i]));
            }
        }
        return total;
    }
    ...
    function getTotalVertHeld(address voter) public view returns (uint256){
        // VERT (loose or in LPs) held in voter's wallet
        return getVertHeld(voter) + getVertHeldLP(voter);
    }
    
    
    function getVertHeldAsUsdc(address voter) public view returns (uint256){
        return getUsdcPerVert() * getTotalVertHeld(voter) / 10**6;
    }
 
    
    function getVotingPowerUSDC(address voter) public view returns (uint256){
        return getVertStakedAsUsdc(voter) + getVertHeldAsUsdc(voter); 
    }
    
    
    function getVotingPowerVERT(address voter) public view returns (uint256){
        return getTotalVertStaked(voter) + getTotalVertHeld(voter);
    }
    
    
    function balanceOf(address voter)public view returns (uint256){
        // For using Snapshot's erc20-balance-of strategy
        if(useUSDCForVotingPower){
            return getVotingPowerUSDC(voter);
        } // else use VERT
        
        return getVotingPowerVERT(voter);
