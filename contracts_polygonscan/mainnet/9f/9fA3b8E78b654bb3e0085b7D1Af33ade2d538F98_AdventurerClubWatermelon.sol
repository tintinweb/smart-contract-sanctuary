...
contract AdventurerClubWatermelon is ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;
    using Strings for uint256;

    uint256 public constant MAX_AVAILABLE = 5;
    uint256 public constant PURCHASE_LIMIT = 2;
    uint256 public PRICE = 7000000000000000; // 0.007 ETH

    string private _contractURI = "";
    string private _tokenBaseURI = "";
    bool private _isActive = false;

    Counters.Counter private _currentNFT;

    constructor() ERC721("Adventurer Club - Watermelon Membership", "ACw") {}
    ...
    function purchase(uint256 numberOfTokens) external payable {
        ...
        require(
            _currentNFT.current() < MAX_AVAILABLE,
            "Purchase would exceed MAX_AVAILABLE"
        );
        require(
        ...
        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = _currentNFT.current();

            if (_currentNFT.current() < MAX_AVAILABLE) {
                _currentNFT.increment();
