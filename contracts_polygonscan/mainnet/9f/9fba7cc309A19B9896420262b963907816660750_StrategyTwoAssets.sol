...
interface IJar is IERC20 {
    ...
    function getLastTimeRestaked(address _address)
        external
        view
        returns (uint256);
...
abstract contract BaseStrategy is Ownable {
    ...
    // Dex
    address public constant currentRouter =
    ...
    function _swapUniswapWithPathForFeeOnTransferTokens(
        ...
        IUniswapRouterV2(currentRouter)
...
abstract contract BaseStrategyMasterChef is BaseStrategy {
    ...
    function getHarvestable() external view override returns (uint256) {
        return
            IStakingRewards(rewards).earned(
                address(this),
...
abstract contract StrategyFarmTwoAssets is BaseStrategyMasterChef {
    ...
    address public constant route = 0x16ECCfDbb4eE1A85A33f3A9B21175Cd7Ae753dB4;
    ...
    uint256 public harvestCutoffWmatic = 10**15;
    uint256 public constant keepFXSmax = 10000;

    // Uniswap swap paths
    address[] public matic_wbtc_path;
    ...
    constructor(
        ...
        address _want,
        address _strategist
    ) public BaseStrategyMasterChef(_rewards, _want, _strategist, dfyn) {
        matic_wbtc_path = new address[](4);
        matic_wbtc_path[0] = matic;
        matic_wbtc_path[1] = route;
        matic_wbtc_path[2] = weth;
        matic_wbtc_path[3] = wbtc;
    ...
    function set_harvestCutoffWmatic(uint256 newCutoff) external onlyOwner {
        harvestCutoffWmatic = newCutoff;
    }
    ...
    function harvest() public override {
        ...
        // If dfyn more than cutoff, compound
        ...
        if (_dfynBalance > harvestCutoff) {
            ...
            // swap every DFYN to MATIC
            _swapUniswapWithPath(dfyn_matic_path, _dfynBalancePost);

            // swap half MATIC to WBTC
            uint256 _maticBalancePost = IERC20(matic).balanceOf(address(this));
            _swapUniswapWithPath(matic_wbtc_path, _maticBalancePost.div(2));

            // Add liquidity
            uint256 _matic = IERC20(dfyn).balanceOf(address(this));
            ...
            if (_matic > 0 && _wbtc > 0) {
                ...
                IERC20(wbtc).safeApprove(currentRouter, 0);
                IERC20(wbtc).safeApprove(currentRouter, _wbtc);
...
contract StrategyTwoAssets is StrategyFarmTwoAssets {
    // Token addresses
    address public Dfyn_Rewards = 0x6F1BcAF54BC824Db40198891A41bA84c845a5031;
    address public WBTC_WMATIC_LP = 0x3318801171F0705781Ab0f97eBCCef73b2486165;

    constructor()
        public
        StrategyFarmTwoAssets(Dfyn_Rewards, WBTC_WMATIC_LP, msg.sender)
    {}
