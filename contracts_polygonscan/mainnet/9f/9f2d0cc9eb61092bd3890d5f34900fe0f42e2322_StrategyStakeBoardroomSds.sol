...
contract StrategyStakeBoardroomSds is StrategyBase {
    using SafeERC20 for IERC20;

    // _controller = TokenPlayChef
    // _want = SDS
    // _farmingToken = SDO
    // _farmingPool = IBoardroom (earns SDO)
    // _farmingPoolId = 0 // Not used
    // _targetProfitToken = TOP
    // _farmingToken2UsdcPath = [pair(SDO/USDC)]
    // _farmingToken2TargetProfitTokenPath = [pair(SDO/USDC), pair(USDC/TOP)]
    constructor(
        address _controller,
        address _want,
        address _farmingToken,
        ...
        address[] memory _farmingToken2UsdcPath,
        address[] memory _farmingToken2TargetProfitTokenPath
    ) StrategyBase(_controller, _want, _farmingToken, _farmingPool, 0, _targetProfitToken, _farmingToken2UsdcPath, _farmingToken2TargetProfitTokenPath) {}

    function getName() public pure override returns (string memory) {
        return "MeebMaster.com:StrategyStakeBoardroomSds";
    }
    ...
    function _farm() internal override {
        ...
        IBoardroom(farmingPool).stake(_wantBal);
    }

    function _withdrawSome(uint256 _amount) internal override {
        IBoardroom(farmingPool).withdraw(_amount);
    }

    function _exit() internal override {
        IBoardroom(farmingPool).exit();
    }

    function inFarmBalance() public view override returns (uint256) {
        return IBoardroom(farmingPool).balanceOf(address(this));
    }

    function _harvest() internal override {
        if (IBoardroom(farmingPool).canClaimReward(address(this))) {
            if (pendingHarvest() > 0) {
                IBoardroom(farmingPool).claimReward();
            }
        }
    }

    function pendingHarvest() public view override returns (uint256) {
        return IBoardroom(farmingPool).earned(address(this));
    }
}