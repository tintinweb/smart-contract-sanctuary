...
contract DinoChefWethWmaticStrategy is BaseStrategyMasterChef {
    ...
    address[] public usdc_a_path;
    address[] public usdc_b_path;
        ...
        )
    {
        ...
        reward_usdc_path = new address[](2);
        reward_usdc_path[0] = DINO;
        reward_usdc_path[1] = USDC;

        usdc_a_path = new address[](2);
        usdc_a_path[0] = USDC;
        usdc_a_path[1] = WETH;

        usdc_b_path = new address[](2);
        usdc_b_path[0] = USDC;
        usdc_b_path[1] = WMATIC;
    ...
    function harvest() public override onlyHumanOrWhitelisted nonReentrant {
        ...
        //Swap remaining DINO for USDC
        if(harvestedToken != tokenA) {
            uint256 remainingHarvested = IERC20(harvestedToken).balanceOf(address(this));
            if (remainingHarvested > 0) {
                _swapUniswapWithPath(reward_usdc_path, remainingHarvested, SUSHI_ROUTER);
            }
        }

        //Swap USDC for WMATIC and WETH
        uint256 _usdc_bal = IERC20(USDC).balanceOf(address(this));
        if (_usdc_bal > 0) {
            _swapUniswapWithPath(usdc_a_path, _usdc_bal.div(2));
            _swapUniswapWithPath(usdc_b_path, _usdc_bal.div(2));
        }
