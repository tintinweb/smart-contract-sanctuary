...
contract AaveStrategy01 is Strategy {
    ...
    function aave_claim_matic_to(address to) public onlyOwner() returns (uint256){
        ...
        /*
        AavePair storage AAVE = AavePairOf["WBTC"];
        [
            AAVE.am, 
            AAVE.variableDebtm
        ];
    ...
    // MAIN
    function _aave_deposit(string memory Coin, uint256 tokenAmt) public onlyOwner(){
    ...
    function _aave_withdraw(string memory Coin, uint256 tokenAmt) public onlyOwner(){
        AavePair storage AAVE = AavePairOf[Coin];
        AaveLP.withdraw(AAVE.basic, tokenAmt, address(this));        
    }

    function _aave_borrow(string memory Coin, uint256 tokenAmt) public onlyOwner(){
        AavePair storage AAVE = AavePairOf[Coin];
        IERC20 SToken = IERC20(AAVE.basic);
        AaveLP.borrow(AAVE.basic, tokenAmt, 2, 0, address(this));
    }

    function _aave_repay(string memory Coin, uint256 tokenAmt) public onlyOwner(){
        AavePair storage AAVE = AavePairOf[Coin];
        IERC20 SToken = IERC20(AAVE.basic);
        require(SToken.balanceOf(address(this)) >= tokenAmt);
        if (SToken.allowance(address(this), AAVE_LENDING_POOL) < tokenAmt) {
    ...
    function aave_loop_deposit_001(string memory Coin) public returns(uint256) {
        AavePair storage AAVE = AavePairOf[Coin];
        IERC20 SToken = IERC20(AAVE.basic);
    ...
    function aave_claim_matic_reinvest(string memory Coin) public onlyOwner(){
        ...
        /*
        uint256 x2 = aave_loop_deposit_002(Coin, amountOut);
...
// !!!!!!!!!
    ...
    function aave_limits(string memory Coin, uint256 Amount, uint256 Variable) public view returns (
            uint256 to_borrow,
            uint256 to_withdraw 
        ) {
        AavePair storage AAVE = AavePairOf[Coin];
    ...
    address public STRATEGY_NAME = address(0);
...
*/
    ...
    */
    ...
    function _deposit(uint256 _amount) public onlyOwner() {
    ...
    function _withdraw(address token, uint256 _amount) public onlyOwner() {
