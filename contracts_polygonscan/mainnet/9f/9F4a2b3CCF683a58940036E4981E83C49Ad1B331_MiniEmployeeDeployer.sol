...
contract MiniEmployeeDeployer is Initializable, AccessControl {
    ...
    mapping(uint256 => uint16) private _mergeRecord;
    ...
    function getEmployeeMergeRecord(uint256 employee)
        public
        view
        returns (uint16)
    {
        return _mergeRecord[employee];
    }
    ...
    function mintWithStakingPoints(bool package) public {
        ...
        require(
            factoryStaking.getSavedMinterPoints(msg.sender) >= pointsPrice,
            INVALID_PAYMENT
        );

        factoryStaking.useMinterPoints(msg.sender, minterPointsPrice);
    ...
    function upgradeEmployee(uint256 miniEmployee) public {
        ...
        employees.mint(
            ...
            parts[2],
            parts[3],
            miniEmployees.getPoints(miniEmployee),
            msg.sender
        );
    ...
    function mergeTwoEmployees(uint256 men, uint256 woman) public {
        ...
        require(_mergeRecord[men] < maxMerges, INVALID_MERGE);
        require(_mergeRecord[woman] < maxMerges, INVALID_MERGE);
