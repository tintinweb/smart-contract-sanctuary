...
contract GovBalance is OwnableUpgradeable {
    ...
    function balanceOf(address account) external view returns (uint256 amount) {
        if (sourceType[account] != SourceType.NULL) return 0;
        
        for (uint i; i < lps.length; i++) {
            IUniPair _lp = lps[i];
            (uint supply, uint govAmt) = lpStats(_lp);

            uint lpAmount = IUniPair(_lp).balanceOf(account); //lp tokens in wallet

            for (uint m; m < lpChefs[_lp].length; m++) {    //lp tokens staked in MC
                MasterChefPool storage mc = lpChefs[_lp][m];
                (uint mcLPAmt, ) = mc.chef.userInfo(mc.pid, account);
                lpAmount += mcLPAmt;
            }
            if (lpAmount > 0) 
                amount = lpAmount * govAmt / supply; //underlying value
        }
        
        amount += GOV_TOKEN.balanceOf(account);  // gov tokens in wallet
    
        for (uint i; i < chefs.length; i++) { //staked directly in MC
            MasterChefPool storage mc = chefs[i];
            (uint bal, ) = mc.chef.userInfo(mc.pid, account);
            amount += bal;
        }
        for (uint i; i < pools.length; i++) {   //staked in pools
            (uint poolBal,) = pools[i].userInfo(account);
            amount += poolBal;
        }
    ...
    function balancesOf(address[] calldata accounts) external view returns (uint256[] memory amounts) {
        ...
        for (uint i; i < lps.length; i++) {
            ...
            for (uint k; k < accounts.length; k++) {
                if (sourceType[accounts[k]] != SourceType.NULL) continue;
    ...
    function addLP(IUniPair _lp) external onlyOwner {
    ...
    function delLP(IUniPair _lp) external onlyOwner {
    ...
    function delPool(address _pool) external onlyOwner {
    ...
    function addChefPool(address _chef, uint96 pid) external onlyOwner {
    ...
    function delChefPool(address _chef, uint96 pid) external onlyOwner {
