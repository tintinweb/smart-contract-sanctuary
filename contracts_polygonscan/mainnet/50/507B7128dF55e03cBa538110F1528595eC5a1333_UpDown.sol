...
contract UpDown is IERC721Locker,OwnableUpgradeable, ReentrancyGuard {
    ...
    uint256 public rewardForNFT = 4; // 4% increase
    ...
    function initialize(address _meeb, address _nft) external initializer {
        meeb = IERC20(_meeb);
        nft = _nft;
    ...
    */
    ...
    function settleBet(
        ...
        if (userStakedNft[bet.player] != 0 ) {
            winAmount = winAmount + winAmount * rewardForNFT / 100;
        }
    ...
    function setRewardForNFT(uint256 _rewardForNFT) external onlyOwner {
        require(_rewardForNFT < 20, '< 20%');
        rewardForNFT = _rewardForNFT;
    }
