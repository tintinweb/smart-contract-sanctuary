...
contract StakeReminter is Initializable, Manageable {
    ...
    function remintStake(
        ...
        StakeData1e18 memory previousStake = contracts.stakeManager.getStake(stakeId);

        if (
            previousStake.stakingDays >= 350 &&
            stakingDays >= 1750 &&
            IAxionNFTUtility(nftFactory).balanceOf(msg.sender, 3) == 0
        ) {
            IAxionNFTUtility(nftFactory).mintExternal(3, 1, msg.sender);
        }
