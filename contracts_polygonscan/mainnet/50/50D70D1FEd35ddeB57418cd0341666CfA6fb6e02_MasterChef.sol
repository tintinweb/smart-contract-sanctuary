...
import "./SafeMath.sol";
import "./BEP20.sol";
import "./JUIXToken.sol";

// MasterChef is the master of Juix. He can make Juix and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once JUIX is sufficiently
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of JUIXs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accJuixPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accJuixPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. JUIXs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that JUIXs distribution occurs.
        uint256 accJuixPerShare;   // Accumulated JUIXs per share, times 1e18. See below.
    ...
    // JUIX tokens created per block.
    uint256 public juixPerBlock;
    // Maximum emission rate : juixPerBlock can't be more than 50 per block
    ...
    // Locker interface
    ILocker public juixLocker;
    // Locker address
    address public juixLockerAddress;
    ...
    // The block number when JUIX mining starts.
    uint256 public startBlock;
    // Total locked up rewards
    uint256 public totalLockedUpRewards;

    // Juix referral contract address.
    IReferral public juixReferral;
    ...
    event JuixReferralUpdated(address indexed user, IReferral newAddress);
    event JuixLockerUpdated(address indexed user, address newAddress);
    ...
    constructor(
        JuixToken _juix,
        uint256 _startBlock,
        uint256 _juixPerBlock,
        address _juixLockerAddress
    ) public {
        juix = _juix;
        startBlock = _startBlock;
        juixPerBlock = _juixPerBlock;
        ...
        juixLockerAddress = _juixLockerAddress;
        juixLocker = ILocker(_juixLockerAddress);
    }
    ...
    // Update the given pool's JUIX allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending JUIXs on frontend.
    function pendingJuix(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accJuixPerShare = pool.accJuixPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0  && totalAllocPoint > 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 juixReward = multiplier.mul(juixPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accJuixPerShare = accJuixPerShare.add(juixReward.mul(1e18).div(lpSupply));
        }
        uint256 pending = user.amount.mul(accJuixPerShare).div(1e18).sub(user.rewardDebt);
        return pending.add(user.rewardLockedUp);
    }

    // View function to see if user can harvest JUIXs.
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 juixReward = multiplier.mul(juixPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        juix.mint(devAddress, juixReward.mul(100).div(1000));
        // Automatically burn 2% of minted tokens
        juix.mint(BURN_ADDRESS, juixReward.mul(20).div(1000));
        // Automatically mint some JUIX for the lottery pot
        if (address(lotteryAddress) != address(0) && lotteryMintRate > 0) {
            juix.mint(lotteryAddress, juixReward.mul(lotteryMintRate).div(10000));
        }        
        juix.mint(address(this), juixReward);
        pool.accJuixPerShare = pool.accJuixPerShare.add(juixReward.mul(1e18).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for JUIX allocation.
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        if (_amount > 0 && address(juixReferral) != address(0) && address(juixReferral) != BURN_ADDRESS && _referrer != address(0) && _referrer != BURN_ADDRESS && _referrer != msg.sender) {
            juixReferral.recordReferral(msg.sender, _referrer);
        }
        payOrLockupPendingJuix(_pid);
        if (_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accJuixPerShare).div(1e18);
    ...
    // Pay or lockup pending JUIXs.
    function payOrLockupPendingJuix(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        uint256 pending = user.amount.mul(pool.accJuixPerShare).div(1e18).sub(user.rewardDebt);
        ...
        if (canHarvest(_pid, msg.sender)) {
            ...
            if (noHarvestFee(_pid, msg.sender)==false && pending > 0) {
                ...
                safeJuixTransfer(BURN_ADDRESS, harvestfeeamount);     
            }
            ...
            if (pending > 0 || user.rewardLockedUp > 0) {
                ...
                if (address(juixLocker) != address(0) && _excludedFromLocker[msg.sender] == false){
                    uint256 startReleaseBlock = ILocker(juixLocker).getStartReleaseBlock();
                    if (lockerRate > 0 && block.number < startReleaseBlock) {
                        ...
                        IBEP20(juix).safeIncreaseAllowance(address(juixLockerAddress), _lockerAmount);
                        ILocker(juixLocker).lock(msg.sender, _lockerAmount); 
                    }
                }

                // send rewards 
                safeJuixTransfer(msg.sender, totalRewards);
    ...
    // Safe juix transfer function, just in case if rounding error causes pool to not have enough JUIXs.
    function safeJuixTransfer(address _to, uint256 _amount) internal {
        uint256 juixBal = juix.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > juixBal) {
            transferSuccess = juix.transfer(_to, juixBal);
        } else {
            transferSuccess = juix.transfer(_to, _amount);
        }
        require(transferSuccess, "safeJuixTransfer: transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _juixPerBlock) public onlyOwner {
        require(_juixPerBlock <= MAX_EMISSION_RATE, "Too high");
        massUpdatePools();
        emit EmissionRateUpdated(msg.sender, juixPerBlock, _juixPerBlock);
        juixPerBlock = _juixPerBlock;
    }

    // Update the juix referral contract address by the owner
    function setJuixReferral(IReferral _juixReferral) public onlyOwner {
        juixReferral = _juixReferral;
        emit JuixReferralUpdated(msg.sender, _juixReferral);
    }
    ...
    // Update the juix locker contract address by the owner
    function setJuixLocker(address _juixLockerAddress) public onlyOwner {
        juixLocker = ILocker(_juixLockerAddress);
        juixLockerAddress = _juixLockerAddress;
        emit JuixLockerUpdated(msg.sender, _juixLockerAddress);
    }   
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (address(juixReferral) != address(0) && referralCommissionRate > 0) {
            address referrer = juixReferral.getReferrer(_user);
            uint256 commissionAmount = _pending.mul(referralCommissionRate).div(10000);

            if (referrer != address(0) && referrer != BURN_ADDRESS && commissionAmount > 0) {
                juix.mint(referrer, commissionAmount);
                juixReferral.recordReferralCommission(referrer, commissionAmount);
