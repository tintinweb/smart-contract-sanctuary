...
contract BeebNFTFaucet {
    address public nft;
    uint256 public index = 100;
    ...
    function faucet(uint256 amt) public returns (bool) {
        address _nft = nft;
        address _admin = BRC721(_nft).admin();
        for (uint256 i = 0; i < amt; i++) {
            while (BRC721(_nft).ownerOf(index) != _admin) {
                index++;
            }
            BRC721(_nft).transferFrom(address(this), msg.sender, index);
        }
        return true;
    }

    function faucet2Nft() public returns (bool) {
        return faucet(2);
    }
}