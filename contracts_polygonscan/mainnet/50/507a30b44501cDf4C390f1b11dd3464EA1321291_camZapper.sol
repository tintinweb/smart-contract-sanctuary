...
contract camZapper is Ownable, Pausable {
    ...
    function _camZapToVault(uint256 amount, uint256 vauldId, CamChain memory chain) internal whenNotPaused returns (uint256) {
        ...
        address(chain.camTokenVault).call( // This creates a low level call to the token
            abi.encodePacked( // This encodes the function to call and the parameters to pass to that function
                chain.camTokenVault.ownerOf.selector , // This is the function identifier of the function we want to call
                abi.encode(vauldId) // This encodes the parameter we want to pass to the function
            )
        );
        
        address user = bytesToAddress(returnData);
        ...
        if(success){
            ...
            if(vauldId == 0){
                ...
                emit AssetZapped(address(chain.asset), amount, vauldId);
            } else {
    ...
    function camZapToVault(uint256 amount, uint256 vauldId, address _asset, address _amAsset, address _camAsset, address _camAssetVault) public whenNotPaused returns (uint256) {
        CamChain memory chain = _buildCamChain(_asset, _amAsset, _camAsset, _camAssetVault);
        require(isWhiteListed(chain), "camToken chain not in on allowable list");
        return _camZapToVault(amount, vauldId, chain);
    }
