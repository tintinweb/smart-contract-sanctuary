...
contract ManagerProxy {
    ...
    function getNftByOwner(address owner) external view returns (uint256[] memory tokenIds, uint256[] memory auctionIds) {
        (uint256[] memory tokenIds1, uint256[] memory auctionIds1) = _zoraMarket2.getNftByOwner(_nftToken, owner);
        uint256[] memory tokenIds2 = ERC721Tradable(_nftToken).ownerTokens(owner);

        uint256 count = tokenIds1.length + tokenIds2.length;
        tokenIds = new uint256[](count);
        auctionIds = new uint256[](count);

        uint256 i = 0;
        for (; i < tokenIds1.length; i++) {
            uint256 tokenId = tokenIds1[i];
            tokenIds[i] = tokenId;
            auctionIds[i] = auctionIds1[i];
        }

        for (uint256 j = 0; j < tokenIds2.length; j++) {
            tokenIds[i] = tokenIds2[j];
            auctionIds[i] = 0;
            i++;
        }
    }
