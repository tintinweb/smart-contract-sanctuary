...
contract PolyRiders is ERC721, ERC721Enumerable, Ownable, Pausable {
  ...
  function mintTokens(uint256 numberOfTokens) public payable whenNotPaused whenMintingNotComplete mintCountMeetsSupply(numberOfTokens) doesNotExceedMaxPurchaseCount(numberOfTokens) {
    ...
    function _airdrops(address _to, uint256 numberOfTokens) internal {
    for (uint256 i = 0; i < numberOfTokens; i++) {
      _mint(_to, totalTokens +i);
      totalTokens += i;
    }
  }
  
  function airdrop(address _to, uint256 numberOfTokens) public onlyOwner {
      _airdrops(_to, numberOfTokens);
      if (totalTokens >= MAX_MINTABLE) {
        mintingComplete = true;
      }
}
