...
contract Strategy_MATIC_USDT is Ownable {
    // This strategy is for Polygon network. MATIC - USDT pair. RewardToken = "dQUICK, wMATIC"
    ...
    address public constant lpToken = 0x604229c960e5CACF2aaEAc8Be68Ac07BA9dF81c3;
    address public constant farm = 0xc0eb5d1316b835F4B584B59f922d9c87cA5053E5;
    ...
    function getRewardToken() public view returns (address){
        return IFarm(farm).rewardsTokenA();
    }
    ...
    function autoCompound() external {
        _approveToken(QUICKToken, router);
        _approveToken(WMATIC, router);
        _approveToken(USDT, router);

        if (getAmountLPFromFarm() > 0) {
            _getRewardsFromFarm(); // two tokens as reward
            _swapDQUICKtoQUICK();

            _swapExactTokenForToken(routerReward, QUICKToken, WMATIC, getBalanceOfToken(QUICKToken));
            _takeFee();
    ...
    function _takeFee() internal {
        ...
        bool result = _swapExactTokenForToken(router, WMATIC, USDT, rewardsFeeInWMATIC+pendingRewardsFee);
        if(result) {
            pendingUSDTFee += getBalanceOfToken(USDT) - _balanceUSDT;
            pendingRewardsFee = 0;
            emit TakeFee(rewardsFeeInWMATIC);
        } else {
            pendingRewardsFee += rewardsFeeInWMATIC;
        }               
    }
    ...
    function _addLiquidityFromAllTokens() internal {
        _swapExactTokenForToken(router, token0, token1, getBalanceOfToken(token0)/2);
    ...
    function _addLiquidityETH(uint256 _amountETH) internal {
        _swapExactETHForTokens(router, token1, _amountETH);
        __addLiquidityETH(token1, _amountETH / 2);
    }

    function _swapExactETHForTokens(address _router, address _token, uint256 _amountETH) internal {
        uint256[] memory amounts;
        address[] memory path = new address[](2);
        path[0] = WMATIC;
        path[1] = _token;
        amounts = IRouter(_router).getAmountsOut(_amountETH/2, path);
        uint256 desiredAmountToken = amounts[1];
        if(desiredAmountToken > 100){
            IRouter(_router).swapExactETHForTokens{value:_amountETH/2}(
    ...
    function _getToken1Price() internal view returns (uint256) {
        ...
        path[0] = token0;
        path[1] = token1;
        return (100 * 1e36) / _getAmountsOut(router, 100 * 1e18, path) / 1e12;
    }
    ...
    function _getToken0Price() internal view returns (uint256 result) {
        ...
        path[0] = token1;
        path[1] = token0;
        result = 1e24 / (_getAmountsOut(router, 100 * 1e6, path) / 100);
    }
