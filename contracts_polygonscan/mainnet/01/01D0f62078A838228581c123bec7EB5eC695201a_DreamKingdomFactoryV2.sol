...
abstract contract Gamificable is Ownable, Pausable {
    ...
    uint256 public bufferBlocks = 20;
    ...
    event BetBull(address indexed sender, uint256 indexed currentEpoch);
    event BetBear(address indexed sender, uint256 indexed currentEpoch);
    

      /**
     * @dev Bet bear position
     */
    function betBear() external  {
        // Update user data
        BetInfo storage betInfo = ledger[currentEpoch][msg.sender];
        betInfo.position = Position.Bear;
        userRounds[msg.sender].push(currentEpoch);
        emit BetBear(msg.sender, currentEpoch);
    }

    /**
     * @dev Bet bull position
     */
    function betBull() external {
        // Update user data
        BetInfo storage betInfo = ledger[currentEpoch][msg.sender];
        betInfo.position = Position.Bull;
        userRounds[msg.sender].push(currentEpoch);
        emit BetBull(msg.sender, currentEpoch);
    }
     ...
     */
    function harvestable(uint256 epoch, address user) public view returns (bool) {
