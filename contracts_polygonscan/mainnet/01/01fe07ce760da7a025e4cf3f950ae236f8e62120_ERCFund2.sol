...
//Contract where the fees are sent to before they are converted and sent to the oldErcFund contract
//Owner of this contract is usually the fee converter
contract ERCFund2 is Ownable {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;

    address public TIMELOCK = 0x52D3Dcf0E59237B032802b40E69f65877091171F;

    address public oldErcFund;
    bool public feeSharingEnabled = false;
    uint256 public fee = 3000;
    uint256 public feeMAX = 10000;

    constructor(address _oldErcFund) public {
        oldErcFund = _oldErcFund;
    }

    function notifyFeeDistribution(address token) public {
        uint256 balance = IERC20(token).balanceOf(address(this));

        IERC20(token).safeApprove(oldErcFund, 0);
        IERC20(token).safeApprove(oldErcFund, balance);
        IERCFund(oldErcFund).depositToFeeDistributor(token, balance);
    }
    ...
    function depositToFeeDistributor(address token, uint256 amount) public {
        ...
        IERC20(token).safeApprove(oldErcFund, 0);
        IERC20(token).safeApprove(oldErcFund, amount);
        IERCFund(oldErcFund).depositToFeeDistributor(token, amount);
    }
    ...
    //Transfers ownership to the timelock if there is an issue with the converter
    function acquireOwnershipTimelock() external {
