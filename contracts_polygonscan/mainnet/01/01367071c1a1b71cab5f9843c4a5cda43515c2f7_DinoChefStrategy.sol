...
contract DinoChefStrategy is BaseStrategyMasterChef {
    ...
    address DINO_WETH_LP = 0x9f03309A588e33A239Bf49ed8D68b2D45C7A1F11;
        ...
        )
    {
        ...
        IERC20(DINO_WETH_LP).safeApprove(MASTER_CHEF, uint256(-1));
        IERC20(DINO).safeApprove(QUICKSWAP_ROUTER, uint256(-1));
        IERC20(WETH).safeApprove(QUICKSWAP_ROUTER, uint256(-1));
    }
    ...
    function harvest() public override onlyHumanOrWhitelisted nonReentrant {
        ...
        //Swap 1/2 of harvestedToken for tokenB
        uint256 _balanceA = IERC20(harvestedToken).balanceOf(address(this));
        if (_balanceA > 0) {
            _swapUniswapWithPathPreapproved(reward_other_path, _balanceA.div(2));
        }
