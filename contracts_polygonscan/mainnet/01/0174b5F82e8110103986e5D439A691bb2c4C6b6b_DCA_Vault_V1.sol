...
contract DCA_Vault_V1 is Initializable, ERC20Upgradeable, ERC2222 {
    ...
    // yearn vault for the principalToken
    IYVault public principalTokenVault;
    // yearn vault for the wantTokenVault (O address if none)
    IYVault public wantTokenVault;
    ...
    function initialize(
        ...
        principalTokenVault = IYVault(_principalTokenVault);
        wantTokenVault = IYVault(_wantTokenVault);
        depositCap = 0;
    ...
    function withdraw(
        ...
        if (principalBalance < _principalAmount) {
            ...
            uint256 _totalAssets = principalTokenVault.totalAssets();
    ...
    function harvest(
        ...
        // convert interest to want token
    ...
    function claimableWantOf(address _owner) external view returns (uint256) {
        ...
        if (address(wantTokenVault) != address(0)) {
            pricePerShare = wantTokenVault.pricePerShare();
        }
    ...
    {
        _totalAssets = principalTokenVault.totalAssets();
        ...
        )
    {
        ...
        if (address(wantTokenVault) != address(0)) {
            ...
            wantPricePerShare = wantTokenVault.pricePerShare();
