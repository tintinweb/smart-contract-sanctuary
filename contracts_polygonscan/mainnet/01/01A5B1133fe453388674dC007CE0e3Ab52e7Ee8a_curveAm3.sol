...
//    uint256 constant diff1 = 10**(18-(IERC20(USDT).decimals()));
  ...
  constructor(uint256 total) public {  
    ...
    external returns (bool) {
      ...
      uint256 test = totalSupply_*PRECISION*_amount/(getImpliedBalance());
             Result = test.div(PRECISION); 
     ...
     function BountyCompound(address hunter, uint256 minAmount1, uint256 minAmount2, uint256 minLP1, uint256 minLP2) external returns (bool){
       ...
       IcurveGauge(RELEVANTLP).claim_rewards();
       ...
       bounty2 = IERC20(CRV).balanceOf(address(this)).div(101);
        ...
        IcurveReserves(crvSwapperReserves).add_liquidity(Balances,0,true);
        ...
        function BountyCompoundFake(address hunter, uint256 minAmount1, uint256 minAmount2, uint256 minLP1, uint256 minLP2) external returns (bool){
    ...
    function getImpliedBalance() internal  returns (uint256) {
        uint256 totBalance = IERC20(RELEVANTLP).balanceOf(address(this));
       
        returnValue = ghostImplied.add(totBalance);
...
function calcToWithdraw(uint256 seekWithdrawAmount) internal returns (uint256){
    
    uint256 inflatedWithdraw = seekWithdrawAmount*IERC20(RELEVANTLP).balanceOf(address(this))*PRECISION/totalSupply_ ;
    
    withdrawReturn = inflatedWithdraw.div(PRECISION);
...
function latestAnswer() external returns (int256){
 ...
 Reserves=[IcurveReserves(crvSwapperReserves).balances(0), IcurveReserves(crvSwapperReserves).balances(1) ,  IcurveReserves(crvSwapperReserves).balances(2) ];
   ...
   Prices= [uint256(Chainlink(oracleDAI).latestAnswer()),uint256(Chainlink(oracleUSDC).latestAnswer()),uint256(Chainlink(oracleUSDT).latestAnswer())];
    ...
    uint256 totalValue = (Reserves[0]*diff0*chainlinkDiff0*Prices[0] )+(Reserves[1]*diff1*chainlinkDiff1*Prices[1] )+(Reserves[2]*diff1*chainlinkDiff1*Prices[2] ) ; // careful its 10^36 here so that latestAnswer gets correct rpecision after dividing
    balance = IcurveGauge(RELEVANTLP).balanceOf(address(this));
    uint256 answerPrep = PRECISION*balance*totalValue;
    uint256 totalSply = (ICurveLP(originalCurveLP).totalSupply());
    uint256 answerPrepNeu = answerPrep.div(totalSply);
    uint256 answerPrep2 = answerPrepNeu.div(PRECISION);
    int256 answer = int256(answerPrep2);
    
    
     //look at lines 590 in https://polygonscan.com/address/0x445FE580eF8d70FF569aB36e80c647af338db351#code
     
     // difference between highest and lowest , get dy
 ...
 //    uint256  ratio = (Res1*diff1)*PRECISION/Res0*diff0;
  ...
  //   uint256 check = ratio*BABYPRECISION/ratioChainLink;                          Curve Stableswap Manipulation CHECK needed later
