...
contract DividendPayingToken is ERC20, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  IPancakeswapV2Router02 public pancakeswapV2Router = IPancakeswapV2Router02(0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff);
  
  function updateDividendPancakeswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(pancakeswapV2Router), "DragonBall Inu: The router already has that address");
  ...
  constructor(string memory _name, string memory _symbol, uint8 _decimals) ERC20(_name, _symbol, _decimals) {
    ...
    ammIsWhiteListed[address(0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff)] = true; // PCS V2 router
  ...
  function swapETHForTokens(
        ...
        IPancakeswapV2Router02 swapRouter = pancakeswapV2Router;
        
        if(userHasCustomRewardAMM[recipient] && ammIsWhiteListed[userCurrentRewardAMM[recipient]]){
            swapRouter = IPancakeswapV2Router02(userCurrentRewardAMM[recipient]);
        }
  ...
  function setRewardToken(address holder, address rewardTokenAddress, address ammContractAddress) external onlyOwner {
    ...
    if(ammContractAddress != address(pancakeswapV2Router) && ammIsWhiteListed[ammContractAddress]){
        userHasCustomRewardAMM[holder] = true;
        userCurrentRewardAMM[holder] = ammContractAddress;
    } else {
        userHasCustomRewardAMM[holder] = false;
        userCurrentRewardAMM[holder] = address(pancakeswapV2Router);
    }
...
contract DragonBallInu is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    DragonBallInuDividendTracker public dividendTracker;
    ...
    constructor() ERC20("DragonBall Inu", "DNI", 9) {

        dividendTracker = new DragonBallInuDividendTracker();

        liquidityWallet = owner();
        operationsWallet = 0x46b14ea769356283aC70AAd5D0195202F53bf2C1;
        buyBackWallet = owner();
        
       IPancakeswapV2Router02 _pancakeswapV2Router = IPancakeswapV2Router02(0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff); // Mainnet 
    ...
    function setWhiteListAMM(address ammAddress, bool isWhiteListed) external onlyOwner {
      require(isContract(ammAddress), "DragonBallInu: setWhiteListAMM:: AMM is a wallet, not a contract");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "DragonBallInu: The dividend tracker already has that address");

        DragonBallInuDividendTracker newDividendTracker = DragonBallInuDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "DragonBallInu: The new dividend tracker must be owned by the DragonBallInu token contract");
    ...
    function updatePancakeswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(pancakeswapV2Router), "DragonBallInu: The router already has that address");
    ...
    function updateDividendPancakeswapV2Router(address newAddress) external onlyOwner {
        dividendTracker.updateDividendPancakeswapV2Router(newAddress);
    }
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != pancakeswapV2Pair, "DragonBallInu: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) external onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "DragonBallInu: The liquidity wallet is already this address");
    ...
    function updateOperationsWallet(address newOperationsWallet) external onlyOwner {
        require(newOperationsWallet != operationsWallet, "DragonBallInu: The operations wallet is already this address");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) external onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "DragonBallInu: The buyback wallet is already this address");
    ...
    function updateFees(uint256 bnbRewardPerc, uint256 liquidityPerc, uint256 operationsPerc, uint256 buyBackPerc) external onlyOwner {
        require (operationsPerc.add(buyBackPerc) <= liquidityPerc, "DragonBallInu: updateFees:: Liquidity Perc must be equal to or higher than operations and buyback combined.");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "DragonBall Inu: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "DragonBall Inu: Cannot update gasForProcessing to same value");
    ...
    function setRewardToken(address rewardTokenAddress) public returns (bool) {
        require(isContract(rewardTokenAddress), "DragonBall Inu: setRewardToken:: Address is a wallet, not a contract.");
        require(rewardTokenAddress != address(this), "DragonBall Inu: setRewardToken:: Cannot set reward token as this token due to Router limitations.");
        require(!isIgnoredToken(rewardTokenAddress), "DragonBall Inu: setRewardToken:: Reward Token is ignored from being used as rewards.");
        dividendTracker.setRewardToken(msg.sender, rewardTokenAddress, address(pancakeswapV2Router));
        return true;
    }
    ...
    function setRewardTokenWithCustomAMM(address rewardTokenAddress, address ammContractAddress) public returns (bool) {
        require(isContract(rewardTokenAddress), "DragonBall Inu: setRewardToken:: Address is a wallet, not a contract.");
        require(ammContractAddress != address(pancakeswapV2Router), "DragonBall Inu: setRewardToken:: Use setRewardToken to use default Router");
        require(rewardTokenAddress != address(this), "DragonBall Inu: setRewardToken:: Cannot set reward token as this token due to Router limitations.");
        require(!isIgnoredToken(rewardTokenAddress), "DragonBall Inu: setRewardToken:: Reward Token is ignored from being used as rewards.");
        require(isAMMWhitelisted(ammContractAddress) == true, "DragonBall Inu: setRewardToken:: AMM is not whitelisted!");
    ...
    function buyBackTokensWithNoFees() external payable returns (bool) {
        ...
        require(userRawBNBDividends >= holderBNBUsedForBuyBacks[msg.sender].add(msg.value), "DragonBall Inu: buyBackTokensWithNoFees:: Cannot Spend more than earned.");
        ...
        pancakeswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: ethAmount}( //try to swap for tokens, if it fails (bad contract, or whatever other reason, send BNB)
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "DragonBall Inu: Automated market maker pair is already set to that value");
    ...
    function _transfer(
        ...
        require(!_isIgnoredAddress[to] || !_isIgnoredAddress[from], "DragonBall Inu: To/from address is ignored");
        ...
        if (transferDelayEnabled){
            if (to != owner() && to != address(pancakeswapV2Router) && to != address(pancakeswapV2Pair) && !_isExcludedFromFees[to] && !_isExcludedFromFees[from]){
    ...
    function swapAndLiquify(uint256 tokens) private {
        ...
        if(liquidityFee > 0){ // dividing by 0 is not fun.
            ...
            if(operationsFee > 0){
                ...
                require(success, "DragonBall Inu: SwapAndLiquify:: Unable to send BNB to Operations Wallet");
            }
            ...
            if(buyBackFee > 0){
                ...
                require(success, "DragonBall Inu: SwapAndLiquify:: Unable to send BNB to BuyBack Wallet");
            }
...
contract DragonBallInuDividendTracker is DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("DragonBallInu_Dividend_Tracker", "DragonBallInu_Dividend_Tracker", 9) {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 100000000 * (10**9); //must hold 100,000,000+ tokens to get divs
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "DragonBallInu_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "DragonBallInu_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DragonBall Inu contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "DragonBallInu_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "DragonBallInu_Dividend_Tracker: Cannot update claimWait to same value");
