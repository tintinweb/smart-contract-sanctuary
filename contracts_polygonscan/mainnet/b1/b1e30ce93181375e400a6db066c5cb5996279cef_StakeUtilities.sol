...
contract StakeUtilities is Manageable, Migrateable, ExternallyCallable {
    event StakeNamed(address account, uint256 stakeId, string oldName, string newName);
    ...
    uint256 public splitCost;

    /** @dev name stake: Allow a user to name their stake
        @param newName {string}
        @param stakeId {uint256}
     */
    function nameStake(string memory newName, uint256 stakeId) external payable {
        require(msg.value == nameCost, 'Invalid cost');
        //** Must be owner */
        require(
            contracts.stakeToken.isOwnerOf(msg.sender, stakeId) ||
                contracts.stakeCustodian.isOwnerOf(msg.sender, stakeId),
            'STAKE MANAGER: Not owner of stake.'
        );

        // Get old name for event purposes
        string memory oldName = contracts.stakeManager.getStake(stakeId).name;

        // Set name name
        contracts.stakeManager.setStakeName(newName, stakeId);

        // Emit event
        emit StakeNamed(msg.sender, stakeId, oldName, newName);
    }

    /** @dev name stake: Allow a user to split their stake
        @param stakeId {string}
        @param percentage {uint256}
     */
    function splitStake(uint256 stakeId, uint256 percentage) external payable {
        require(msg.value == splitCost, 'Invalid cost');
        require(
            contracts.stakeToken.isOwnerOf(msg.sender, stakeId) ||
                contracts.stakeCustodian.isOwnerOf(msg.sender, stakeId),
        ...
        require(
            contracts.stakeCustodian.addStake(
                msg.sender,
                contracts.stakeManager.splitStake(stakeId, percentage)
            ) == true,
            'STAKEMINTER: Failed to split stake'
        );
    }

    /** Setters ------------------------------------------------------------------ */
    function setNameCost(uint256 cost) external onlyManager {
        nameCost = cost;
    }

    function setSplitCost(uint256 cost) external onlyManager {
        splitCost = cost;
    }
