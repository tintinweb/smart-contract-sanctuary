...
contract IntegrationCore is IIntegrationCore, AbstractCore {
    ...
    modifier updateUserAssets(bytes32 _assetKey, bool _isSupplyAssets) {
        _;
        assetsRegistry.updateUserAssets(msg.sender, _assetKey, address(0), _isSupplyAssets);
    }
        ...
        returns (uint256 _maxToSupply)
    {
        ...
        _maxToSupply = _getMaxToWithdraw(
            _userAddr,
            _assetKey,
            ...
            _defiCore.disabledCollateralAssets(_userAddr, _assetKey)
        );
    }
    ...
    {
        _maxToWithdraw = _getMaxToWithdraw(
    ...
    function isBorrowExists(address _userAddr, bytes32 _assetKey)
        ...
        returns (bool)
    {
        bool _isActiveBorrow =
            _assetKey.getAssetLiquidityPool(liquidityPoolRegistry).getUserBorrowedAmount(
                _userAddr,
                true
            ) != 0;
        ...
        returns (uint256 _currentBorrowLimit)
    {
        ...
        for (uint256 i = 0; i < _userSupplyAssets.length; i++) {
            ...
            if (!disabledCollateralAssets[_userAddr][_currentAssetKey]) {
                ...
                _currentBorrowLimit += _currentAssetKey.getIntegrationLimitPart(
    ...
    function borrow(
        ...
        // TODO Change requires
        require(
            _debtAmount == 0,
            "IntegrationCore: Unable to borrow because the account is in arrears."
        );
        ...
        _userVaultTokens[msg.sender][_assetKey].add(_tokenToDeposit);
    ...
    function repayBorrowIntegration(
        ...
        if (
            _userBorrowerRouter.depositOfAssetInToken(
                _vaultTokenAddr,
                _assetLiquidityPool.assetAddr()
            ) == 0
        ) {
            _userVaultTokens[msg.sender][_assetKey].remove(_vaultTokenAddr);
        }
    ...
    function _getMaxToWithdraw(
        ...
        if (!_disabledAsCollateral) {
            ...
            uint256 _residualLimit =
                _currentBorrowLimit - (_userLiquidityInUSD * DECIMAL) / _currentColRatio;
    ...
    function _getCorrectLimitPart(
        ...
        return _assetKey.getIntegrationLimitPart(_currentSupplyAmount, _parameters);
    }
