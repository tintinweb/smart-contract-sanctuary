...
abstract contract BaseStrategyIron is BaseStrategy {

    uint256 public constant keepMax = 10000;
    uint256 public poolId;

    //Tokens
    address public constant ICE = 0x4A81f8796e0c6Ad4877A51C86693B0dE8093F2ef;
    address public constant WMATIC = 0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270;
    address public constant USDC = 0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174;

    //Other
    address public FIREBIRD_ROUTER = 0xF6fa9Ea1f64f1BBfA8d71f7f43fAF6D45520bfac;
    address public QUICKSWAP_ROUTER = 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff;
    address public DFYN_ROUTER = 0xA102072A4C07F06EC3B4900FDC4C7B80b6c57429;

    address public IRON_CHEF = 0x1fD1259Fa8CdC60c6E8C86cfA592CA1b8403DFaD;
    address public ERC_FUND = 0x01fE07ce760DA7a025e4Cf3f950aE236F8e62120;

    // Uniswap swap paths
    address[] public ICE_USDC_path;
    address[] public USDC_WMATIC_path;
    ...
    constructor(
        ...
        public
        BaseStrategy(_want, ERC_FUND, ICE, _router, IRON_CHEF)
    {
        poolId = _poolId;
        ...
        //ice -> usdc DFYN
        //usdc -> wmatic Quickswap
        ICE_USDC_path = new address[](2);
        ICE_USDC_path[0] = ICE;
        ICE_USDC_path[1] = USDC;

        USDC_WMATIC_path = new address[](2);
        USDC_WMATIC_path[0] = USDC;
        USDC_WMATIC_path[1] = WMATIC;

        IERC20(_want).safeApprove(IRON_CHEF, uint256(-1));
        IERC20(ICE).safeApprove(DFYN_ROUTER, uint256(-1));
        IERC20(USDC).safeApprove(QUICKSWAP_ROUTER, uint256(-1));
    }

    // **** Views ****

    function balanceOfPool() public override view returns (uint256) {
        (uint256 amount, ) = IIronChef(rewards).userInfo(poolId, address(this));
        return amount;
    }
    ...
    function getHarvestable() external override view returns (uint256) {
        return IIronChef(rewards).pendingReward(poolId, address(this));
    }
    ...
    function swapRewardToWmaticAndDistributeFee(uint256 feeAmount) internal {
        ...
        if(usdc_bal > 0) {
            IERC20(USDC).safeTransfer(strategist, usdc_bal);
        }
        uint256 matic_bal = IERC20(WMATIC).balanceOf(address(this));
        if(matic_bal > 0) {
            IERC20(WMATIC).safeTransfer(strategist, matic_bal);
        }

        if(feeAmount > 0) {
            _swapUniswapWithPathPreapproved(ICE_USDC_path, feeAmount, DFYN_ROUTER);

            uint256 _usdcBal = IERC20(USDC).balanceOf(address(this));
            _swapUniswapWithPathPreapproved(USDC_WMATIC_path, _usdcBal, QUICKSWAP_ROUTER);
    ...
    function deposit() public override {
        uint256 _want = IERC20(want).balanceOf(address(this));
        if (_want > 0) {
            IIronChef(rewards).deposit(poolId, _want, address(this));
        }
    }
    ...
    function _withdrawSome(uint256 _amount)
        ...
        returns (uint256)
    {
        IIronChef(rewards).withdraw(poolId, _amount, address(this));
        return _amount;
    }
    ...
    function _getReward() internal {
        IIronChef(rewards).harvest(poolId, address(this));
    }
    ...
    function emergencyWithdraw() public onlyOwner {
        IIronChef(rewards).emergencyWithdraw(poolId, address(this));
...
contract IceWethStrategy is BaseStrategyIron {

    address public constant WETH = 0x7ceB23fD6bC0adD59E62ac25578270cFf1b9f619;
    address public constant ICE_WETH_LP = 0xf1EE78544a1118F2efb87f7EaCd9f1E6e80e1ea5;

    address[] public USDC_WETH_path;

    constructor() public BaseStrategyIron(ICE_WETH_LP, FIREBIRD_ROUTER, 1) {
        IERC20(WETH).safeApprove(FIREBIRD_ROUTER, uint256(-1));
        IERC20(ICE).safeApprove(FIREBIRD_ROUTER, uint256(-1));

        USDC_WETH_path = new address[](2);
        USDC_WETH_path[0] = USDC;
        USDC_WETH_path[1] = WETH;
    }
    ...
    function harvest() public override onlyHumanOrWhitelisted nonReentrant {
        ...
        //Swap 1/2 of remaining ICE for USDC then swap it for WETH
        uint256 remainingHarvested = IERC20(harvestedToken).balanceOf(address(this));
        if (remainingHarvested > 0) {
            _swapUniswapWithPathPreapproved(ICE_USDC_path, remainingHarvested.div(2), DFYN_ROUTER);
            uint256 _usdcBal = IERC20(USDC).balanceOf(address(this));
            _swapUniswapWithPathPreapproved(USDC_WETH_path, _usdcBal, QUICKSWAP_ROUTER);
        }

        uint256 iceBalance = IERC20(ICE).balanceOf(address(this));
        uint256 wethBalance = IERC20(WETH).balanceOf(address(this));
        if (iceBalance > 0 && wethBalance > 0) {
            IFireBirdRouter(currentRouter).addLiquidity(
                ICE_WETH_LP, 
                WETH, ICE,
                wethBalance, iceBalance, 
                0, 0,
                address(this),
                now + 60
            );
