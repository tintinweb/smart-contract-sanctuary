...
contract StrategyDINO is Ownable {
    ...
    function autoCompound() public {
        ...
        if (amount > 0) {
            ...
            uint256 rewardBalance = _getBalanceOfToken(rewardToken);
            path[0] = rewardToken;
            path[1] = WETH;
            amount = _getAmountsOut(rewardBalance, path);
            if(amount > 100) {
                _swapExactTokensForTokens(rewardBalance, amount, path);
            }
            ...
            amounts = IRouter(liquidityRouter).getAmountsOut(rewardBalance, path1);
            if(amounts[1] > 100 && amounts[2] > 100) {
                _swapExactTokensForTokens(rewardBalance, amounts[2], path1);
