...
contract PolyGardentoken is BEP20("PolyGarden", "APPLE") {
    // Burn address
    address public constant BURN_ADDRESS = 0x42545B133349bc3a881067bfd1Fd089B395ca50e;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        if (recipient == BURN_ADDRESS) {
            ...
            require(amount == sendAmount + burnAmount, "APPLE::transfer: Burn value invalid");
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    // Initial emission rate: 1 APPLE per block.
    uint256 public constant INITIAL_EMISSION_RATE = 1 ether;
    // Minimum emission rate: 0.4 APPLE per block.
    ...
    constructor(
        ...
        APPLEPerBlock = INITIAL_EMISSION_RATE;
    }
    ...
    function safeAPPLETransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > APPLEBal) {
            transferSuccess = APPLE.transfer(_to, APPLEBal);
        } else {
            transferSuccess = APPLE.transfer(_to, _amount);
        }
        require(transferSuccess, "safeAPPLETransfer: Transfer failed");
    }
    ...
    function updateEmissionRate() public {
        ...
        require(APPLEPerBlock > MINIMUM_EMISSION_RATE, "updateEmissionRate: Emission rate has reached the minimum threshold");
        ...
        uint256 newEmissionRate = APPLEPerBlock;
        ...
        if (newEmissionRate >= APPLEPerBlock) {
            return;
        }

        massUpdatePools();
        lastReductionPeriodIndex = currentIndex;
        uint256 previousEmissionRate = APPLEPerBlock;
        APPLEPerBlock = newEmissionRate;
