...
//-----------------------------------------------------------------------------
contract PolyDachshundToken is BEP20("PolyDachshund", "DACHS", 1000000000000000000000000) {
...
contract MasterChef is Ownable, NonReentrant {
  ...
  struct PoolInfo {
    ...
    uint256 accPolyDachshundPerShare;
    uint16 depositFeeBP;
  }
  //---------------------------------------------------------------------------
  PolyDachshundToken public dachshund;
  ...
  constructor(PolyDachshundToken _dachshund, address _feeAddress, uint256 _startBlock) public {
    dachshund = _dachshund;
 ...
 //---------------------------------------------------------------------------
  function add(uint256 _allocPoint, IBEP20 _lpToken, bool _withUpdate) public ownerOnly isUnique(_lpToken) {
    ...
    poolInfo.push(PoolInfo({lpToken: _lpToken, allocPoint: _allocPoint, lastRewardBlock: lastRewardBlock, accPolyDachshundPerShare: 0, depositFeeBP: depositFee})
    );
  }
  function pendingDachshund(uint256 _pid, address _user) external view returns (uint256)  {
    PoolInfo storage pool = poolInfo[_pid];
    UserInfo storage user = userInfo[_pid][_user];
    uint256 share = pool.accPolyDachshundPerShare;
    uint256 pot = pool.lpToken.balanceOf(address(this));
    if (block.number > pool.lastRewardBlock && pot != 0) {
      uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
      uint256 PolyDachshundReward = multiplier.mul(EMISSION_RATE).mul(pool.allocPoint).div(totalAllocPoint);
      share = share.add(PolyDachshundReward.mul(1e12).div(pot));
    }
  ...
  function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
    ...
    updatePool(_pid);
    if (user.amount > 0) {
      uint256 pending = user.amount.mul(pool.accPolyDachshundPerShare).div(1e12).sub(user.rewardDebt);
      if (pending > 0) {
        safePolyDachshundTransfer(msg.sender, pending);
      }
    }
    if (_amount > 0) {
    ...
    user.rewardDebt = user.amount.mul(pool.accPolyDachshundPerShare).div(1e12);
  ...
  function updatePool(uint256 _pid) public {
    ...
    uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
    uint256 PolyDachshundReward = multiplier.mul(EMISSION_RATE).mul(pool.allocPoint).div(totalAllocPoint);
    dachshund.mint(feeAddress, PolyDachshundReward.div(20));
    dachshund.mint(address(this), PolyDachshundReward);
    pool.accPolyDachshundPerShare = pool.accPolyDachshundPerShare.add(PolyDachshundReward.mul(1e12).div(lpSupply));
    pool.lastRewardBlock = block.number;
  }
  ...
  function safePolyDachshundTransfer(address _to, uint256 _amount) internal {
    uint256 pot = dachshund.balanceOf(address(this));
    bool success = false;
    if (_amount > pot) {
      success = dachshund.transfer(_to, pot);
    } else {
      success = dachshund.transfer(_to, _amount);
    }
    require(success, "error");
  }
