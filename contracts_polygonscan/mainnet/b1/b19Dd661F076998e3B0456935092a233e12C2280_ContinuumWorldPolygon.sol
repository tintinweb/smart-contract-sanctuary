...
contract ContinuumWorldPolygon is
    ERC1155,
    IChildToken,
    AccessControlMixin,
    ...
    ContextMixin
{
    ...
    mapping(uint256 => bool) internal metadata_exists;
    ...
    function setTokenCustomUri(uint256 id, string calldata data) external only(DEFAULT_ADMIN_ROLE) {
        metadata_exists[id] = true;
        metadata[id] = data;
    }


    function uri(uint256 id) external view virtual override returns (string memory) {
        if(metadata_exists[id]) return metadata[id];
        return _global_uri;
    }
