...
contract BSTICK is Context, Ownable, ERC20 {
    ...
    /// @param receiver Address of receiver
    /// @param rating The rating of how good the review is (0-10)
    function airdropReviewer(address receiver, uint8 rating) public returns (bool) {
        // 0-10k $BSTICK for Proof of Pasta (pic of yourself at Olive Garden w/ identity proof)
        require(_msgSender() == airdropBuffetWallet, "Can only be initiated by Airdrop Buffet Wallet");
        require(rating <= 10 && rating >= 0, "Rating must be between 0 and 10");
       
        // Rating * 1,000 $BSTICK reward
        uint bsticksForRating = rating * bsticksPerReviewStar;
        require(bsticksForRating <= balanceOf(airdropBuffetWallet), "Airdrop Buffet wallet doesn't have enough $BSTICK for this claim");

        transfer(receiver, bsticksForRating);
        return true;
    }

    /// @notice Claim Breadsticks for proof of eating at (pic of yourself at Olive Garden w/ identity proof)
    /// @param receiver Address of receiver
    function airdropProofOfPasta(address receiver) public returns (bool) {
        // 10k $BSTICK for Proof of Pasta 
        require(_msgSender() == airdropBuffetWallet, "Can only be initiated by airdrop buffet wallet");
        require(bsticksPerProofOfPasta <= balanceOf(airdropBuffetWallet), "Airdrop Buffet wallet doesn't have enough $BSTICK for this claim");
       
        transfer(receiver, bsticksPerProofOfPasta);
        return true;
    }
