...
contract GameReward is Ownable, Events {
    ...
    function UpdateLockFeePercentage(uint8 feePercentage_) external onlyOwner {
        require(unlockTaxPercentage <= 10, "QuizDrop: unlock fee percentage exceeds 10");
        require(unlockTaxPercentage >= 0, "QuizDrop: unlock fee percentage equals 0");
        unlockTaxPercentage = feePercentage_;
    }
    ...
    function lock(address _from, uint256 amount) public returns(bool){
        ...
        require(amount == 5*10**18 || amount == 10*10**18, "Invalid Amount");
  
        _locks[_from].locked = true;
        _locks[_from].membership = uint8(amount.div(1 ether).div(5));
    ...
    function unlock(address _to) public returns(bool){
        ...
        uint256 amount = uint256(_lock.membership).mul(1 ether).mul(5);
        uint256 tax;
        ...
        uint256 allowance = IERC20(qdropToken).allowance(address(this), _to);
        IERC20(qdropToken).approve(lockAddress, allowance + amount.sub(tax));
        ...
        if(tax != 0){
            ...
            IERC20(qdropToken).approve(lockAddress, Reward_allowance + tax); 
