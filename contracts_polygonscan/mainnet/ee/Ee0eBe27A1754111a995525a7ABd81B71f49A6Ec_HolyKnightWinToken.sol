...
contract HolyKnightWinToken is IHolyKnightWinToken, Initializable, ERC20Upgradeable, AccessControlUpgradeable {
    ...
    bytes32 public constant GAME_ADMIN_ROLE = keccak256("Holy Game Administrator");
    ...
    function initialize() public virtual initializer {
        ...
        _mint(address(this), _totalSupply * 79 / 100);
        _mint(_msgSender(), _totalSupply * 21 / 100);
    }

    modifier onlyContractCaller() {
        require(hasRole(GAME_ADMIN_ROLE, msg.sender), "Not Game Admin");
        _;
    }

    modifier onlyGovernance() {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "Governance Access Denied");
        _;
    }

    modifier fiveBlockDelay(address _account) {
        require(lastBlockCalledByUser[_account] < block.number, "Too many transaction");
        lastBlockCalledByUser[_account] = block.number;
        _;
    }

    function migrateFunction(address _holyCard, uint256[] calldata _tokenIds) public onlyContractCaller {
    ...
    function unlockHolyKnightWinToken(uint256 _tokenId) public fiveBlockDelay(msg.sender) returns (uint256) {
        ...
        if (_amount == 19 * 10**8) {
            ...
            fundMap[_tokenId].amount = fundMap[_tokenId].amount.sub(_unlock);
        ...
        if ((_amount == 15 * 10**8) || (_amount == 3 * 10**8) || (_amount == 105 * 10**7)) {
            fundMap[_tokenId].amount = 0;
            _req = transfer(address(this), _sender, _amount * 1e18);
        }
        if (hasRole(DEFAULT_ADMIN_ROLE, msg.sender)) {
            require(block.timestamp.sub(deployedTime) > 730 days, "Release at 2 years");
            _req = transfer(address(this), msg.sender, this.balanceOf(address(this)));
        }
        if (_amount == 25 * 10**8) {
    ...
    function setRate(uint256 burnRate, uint256 rewardRate) public override onlyContractCaller {
    ...
    function excludeFromFee(address account) public override onlyContractCaller {
        _isExcludedFromFee[account] = true;
    }

    function includeInFee(address account) public override onlyContractCaller {
    ...
    function enableOpenTransfer(bool isEnable) public override onlyContractCaller {
