...
interface PaymentMixerInterface{
    ...
    //Mapper
    function GetWallet(bytes32 _secret) external view returns(address);
}
...
contract PaymentMixerV1{
    ...
    PaymentMixerInterface constant private mapper = PaymentMixerInterface(mapperAddress);
    ...
    uint256 private mixNo = 0;
    uint256 internal checkBlock = block.number;
    ...
    function CoinPayment(uint256 _amount, bytes32 _secret) external payable returns(bool){

        require(enabled, "Mixer disabled");
        require(_amount == msg.value, "Amount security check errored");

        address walletAddress = mapper.GetWallet(_secret);

        require(fisk.VerifyCustomerWallet(walletAddress) == true, "Not a FiskPay wallet");

        mix("Matic", _amount, walletAddress);
    
        return true;
    }

    function TokenPayment(address _tokenAddress, uint256 _tokenAmount, bytes32 _secret) external returns(bool){
        ...
        if(token.allowance(address(this), walletAddress) < (_tokenAmount + token.balanceOf(address(this)))){

            token.approve(walletAddress, 2**256-1);
        ...
        mix(token.symbol(), _tokenAmount, walletAddress);
    ...
    function mix(string memory _symbol, uint256 _amount, address _walletAddress) private returns(bool){
        ...
        mixNo += (swapPosition + slot[swapPosition].start);
