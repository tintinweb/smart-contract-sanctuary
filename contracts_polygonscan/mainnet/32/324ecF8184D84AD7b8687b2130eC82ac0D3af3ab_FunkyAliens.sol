...
/**
 * @title FunkyAliens
 * FunkyAliens - ERC-721 FunkyAliens
 */
contract FunkyAliens is ERC721, Ownable {
    ...
    string public notrevealed_nft = "https://ipfs.io/ipfs/QmPNn9vEQWnQHR1XQejtG7PRHqPwGQ1N2F1i3zjChSJKai";
    string public contract_base_uri = "https://ipfs.io/ipfs/QmaoKoGDFfXh2a83Ei9uFNCWW9gjfP1EsGPa4xwwBjUN4L/";
    uint256 minting_price_0 = 50 ether;
    uint256 minting_price_1 = 70 ether;
    uint256 minting_price_3 = 180 ether;
    uint256 minting_price_5 = 250 ether;
    bool public is_whitelisted = true;
    ...
    */
    function revealNFT(uint256 _tokenId) public returns (bool) {
        require(ownerOf(_tokenId) == msg.sender, "FunkyAliens: You must own the NFT");
    ...
    */
    function isWhitelisted(address _toCheck) public view returns (bool) {
        return _whitelist[_toCheck] == true;
    }
    ...
    */
        ...
        payable
    {
        ...
        if(is_whitelisted == true){
            require(isWhitelisted(msg.sender), 'FunkyAliens: Not whitelisted');
            require(msg.value == minting_price_0, 'FunkyAliens: Presale amount invalid');
            amount = 1;
        }else{
            require(msg.value == minting_price_1 || msg.value == minting_price_3 || msg.value == minting_price_5, 'FunkyAliens: Public sale amount invalid');
            if(msg.value == minting_price_1){
                amount = 1;
            }
            if(msg.value == minting_price_3){
                amount = 3;
            }
            if(msg.value == minting_price_5){
                amount = 5;
            }
        }
        require(amount >= 1, 'FunkyAliens: Amount should be at least 1');
        uint256 reached = amount + _tokenIdCounter.current();
        require(reached <= HARD_CAP, "FunkyAliens: Hard cap reached");
        uint j = 0;
        for (j = 0; j < amount; j++) {
    ...
    function fixPriceZero(uint256 price) public onlyOwner {
        minting_price_0 = price;
    }

    function fixPriceOne(uint256 price) public onlyOwner {
        minting_price_1 = price;
    }

    function fixPriceThree(uint256 price) public onlyOwner {
        minting_price_3 = price;
    }

    function fixPriceFive(uint256 price) public onlyOwner {
        minting_price_5 = price;
    }

    function fixWhitelisting(bool state) public onlyOwner {
    ...
    */
    function proxyMintNFT(address _to)
        public
    {
        require(isMinter(msg.sender), "FunkyAliens: Only minters can mint");
    ...
    function withdrawMatic() public onlyOwner {
        ...
        require(balance > 0, 'FunkyAliens: Nothing to withdraw!');
