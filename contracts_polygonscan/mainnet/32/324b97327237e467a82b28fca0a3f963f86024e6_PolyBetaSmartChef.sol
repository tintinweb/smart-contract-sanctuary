...
contract PolyBetaSmartChef is Ownable {
    using SafeMath for uint256;
    ...
    // Maximum amount of SPADE that can be deposited
    ...
    function changeRewardPerBlock(uint256 _amount) public onlyOwner {
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accRewardPerShare = accRewardPerShare.add(tokenReward.mul(1e18).div(lpSupply));
        }
    ...
    function updatePool() public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 tokenReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        pool.accRewardPerShare = pool.accRewardPerShare.add(tokenReward.mul(1e18).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function updateEndBlock(uint256 _block) public onlyOwner {
        bonusEndBlock = _block;
    }

    // A way to get back other ERC20 tokens sended by error into this contract
