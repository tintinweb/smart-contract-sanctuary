...
abstract contract StrategyFarmTwoAssets is BaseStrategyMasterChef {
    ...
    address public constant ust = 0x692597b009d13C4049a947CAB2239b7d6517875F;
        ...
        )
    {
        ...
        usdt_ust_path = new address[](2);
        usdt_ust_path[0] = usdt;
        usdt_ust_path[1] = ust;
    }
    ...
    function harvest() public override {
        ...
        if (_dinoBalance > harvestCutoff){
            _swapUniswapWithPath(dino_usdt_path, _dinoBalance);
            
            // Collect fee
            uint256 _usdtBalance = IERC20(usdt).balanceOf(address(this));
            uint256 performanceFee = _usdtBalance.mul(keepFXS).div(keepFXSmax);
            if (performanceFee>0) {
            ...
            // Swap half of rest
            uint256 _usdtBalancePost = IERC20(usdt).balanceOf(address(this));
            _swapUniswapWithPathDfyn(usdt_ust_path, _usdtBalancePost.div(2));

            // Add liquidity
            uint256 _usdt = IERC20(usdt).balanceOf(address(this));
            uint256 _ust = IERC20(ust).balanceOf(address(this));
            if (_usdt > 0 && _ust > 0) {
                IERC20(usdt).safeApprove(dfynRouter, 0);
                IERC20(usdt).safeApprove(dfynRouter, _usdt);
                IERC20(ust).safeApprove(dfynRouter, 0);
                IERC20(ust).safeApprove(dfynRouter, _ust);

                IUniswapRouterV2(dfynRouter).addLiquidity(
                    usdt,
                    ust,
                    _usdt,
                    _ust,
...
contract StrategyTwoAssets is StrategyFarmTwoAssets {
    ...
    address public UST_USDT_LP = 0x39BEd7f1C412ab64443196A6fEcb2ac20C707224;
