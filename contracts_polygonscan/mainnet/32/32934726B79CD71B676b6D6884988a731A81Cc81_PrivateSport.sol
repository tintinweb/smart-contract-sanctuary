...
// File: contracts\IEventDatabase.sol
...
// File: contracts\PrivateSport.sol
...
contract PrivateSport is Ownable {
    ...
    modifier canEndPool(uint256 _poolId) {
        ...
        require(
            endTime <= block.timestamp && isEnded && result > 0 && !pools[_poolId].isEnded,
    ...
    function participatePool(
        ...
        emit NewParticipantJoined(_poolId, _vote, _amount, msg.sender);
    ...
    function claimAward(uint256 _poolId) external checkValidPool(_poolId) {
        ...
        winners[_poolId].vote = participants[_poolId][msg.sender].vote;
        winners[_poolId].owner = msg.sender;
        winners[_poolId].deposit = participants[_poolId][msg.sender].deposit;

        // If the event is draw
        if (pools[_poolId].result == Result.AB) {
            winners[_poolId].award = participants[_poolId][msg.sender].deposit;
        } else {
            if (participants[_poolId][msg.sender].vote == Vote.A) {
                winners[_poolId].award =
            ...
            winners[_poolId].award =
                (winners[_poolId].award * (100 - serviceFee - creatorFee)) /
                100;

            // Transfer fee to pool owner
            uint256 amount = (winners[_poolId].award * creatorFee) / 100;
            require(
        ...
        require(
            gfx_.transfer(winners[_poolId].owner, winners[_poolId].award),
            "GFX transfer failed"
        );

        participants[_poolId][msg.sender].isClaimed = true;

        emit AwardClaimed(_poolId, msg.sender, winners[_poolId].deposit, winners[_poolId].award);
    }
