    ...
    Ownable 
{
    ...
    mapping (uint256 => uint256) private rentDueTime;
    ...
    function setTokenURI(uint256 tokenId, string memory uri) public only(DEFAULT_ADMIN_ROLE) {
        _tokenURI[tokenId] = uri;
    }
    
    function setHolder(uint256 tokenId, address newHolder, uint256 rentDay) public only(RENT_ROLE) {
        _holderOf[tokenId] = newHolder;
        rentDueTime[tokenId] = block.timestamp + rentDay * 1 days;
    }
    
    function holderOf(uint256 tokenId) public view returns (address) {
        if (block.timestamp > rentDueTime[tokenId]) {
