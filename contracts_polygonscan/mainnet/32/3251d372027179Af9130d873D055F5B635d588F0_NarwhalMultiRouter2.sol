...
// File: contracts/routes/NarwhalMultiRouter2.sol
...
contract NarwhalMultiRouter2 is INarwhalTradingRoute, Ownable, WhitelistedRole, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    constructor(
        ...
        /* require(_routers.length >= 1 && _correspondentTokens.length == _routers.length - 1, "NarR: Invalid lengths"); */
        narwhal = INarwhal(_narwhal);
        routers = _routers;
    ...
    function updateNarwhal(
        address  _narAddr
    )
        public
        onlyOwner
    {
        narwhal = INarwhal(_narAddr);
    }
        ...
        returns (bool)
    {
        ...
        return narwhal.isFeeOnTransferToken(IERC20(_token));
    }
    ...
    function trade(
        ...
        returns(uint256 _destAmount)
    {
        require(_src != _dest, "NarR: Destination token can not be source token");
        ...
        if (_src == etherERC20) {
            require(msg.value == _srcAmount, "NarR: Source amount mismatch");
    ...
    function getDestinationReturnAmount_(
        IERC20 _src,
        IERC20 _dest,
        uint256 _srcAmount
    )
        external
        view
        returns(uint256 _destAmount, address[] memory path1, address[] memory path2)
    {
        require(_src != _dest, "NarR: Destination token can not be source token");
        /* if (isDuplicatedTokenInRoutes(_src) || isDuplicatedTokenInRoutes(_dest)) {
            return (0,[],[]);
