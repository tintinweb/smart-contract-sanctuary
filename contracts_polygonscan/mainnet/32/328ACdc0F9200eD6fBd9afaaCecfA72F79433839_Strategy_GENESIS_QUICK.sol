...
contract Strategy_GENESIS_QUICK is Ownable {
    ...
    function _takeFee() internal {
        uint256 rewardsFeeInQuick = _calculateAmountFee(getBalanceOfToken(QUICKToken));
        bool _swapped = _swapExactTokenForToken(router, QUICKToken, USDT, rewardsFeeInQuick+pendingFee);
        if (_swapped) {
            pendingFee = 0;
    ...
    function _getSimpleTCI(uint256 _amount) internal view returns (uint256 tCI) {
        ...
        uint256 amountQUICK;
        if (_balance0 > 0) {
            amountQUICK = _getToken0Price() * _balance0 / d;
        }

        if (_balance1 > 0) {
            tCI += _getPriceWMATICToQUICK() * (_balance1 + amountQUICK) / d;
        }
