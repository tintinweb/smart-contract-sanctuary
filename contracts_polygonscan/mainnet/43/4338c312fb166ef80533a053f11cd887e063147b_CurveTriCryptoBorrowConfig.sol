...
interface IStableSwap{
    ...
    function get_dy_underlying(uint i,uint j,uint dx) external view returns(uint256);
}
...
contract CurveTriCryptoBorrowConfig is Ownable, BigfootConfig {
  ...
  address public constant tricrypto = 0x3FCD5De6A9fC8A99995c406c77DDa3eD7E406f81;
  ...
  constructor() public {
      tokenToId[IStableSwap(tricrypto).underlying_coins(0)]=0;
      tokenToId[IStableSwap(tricrypto).underlying_coins(1)]=1;
      tokenToId[IStableSwap(tricrypto).underlying_coins(2)]=2;
      tokenToId[IStableSwap(tricrypto).underlying_coins(3)]=3;
      tokenToId[IStableSwap(tricrypto).underlying_coins(4)]=4;
  }
  ...
  function isStable(address bigfoot) public view returns (bool) {
    ...
    uint curvePrice = IStableSwap(tricrypto).get_dy_underlying(tokenToId[token0], tokenToId[token1], 10**uint(IDecimals(token0).decimals()))*1e18/10**uint(IDecimals(token0).decimals()); //r1.mul(1e18).div(r0);
    ...
    require(curvePrice <= price.mul(maxPriceDiff).div(10000), 'curve price too high');
    require(curvePrice >= price.mul(10000).div(maxPriceDiff), 'curve price too low');
    // 3. Done
    return true;
  }
  /// @dev Return whether the given bigfoot accepts more debt.
