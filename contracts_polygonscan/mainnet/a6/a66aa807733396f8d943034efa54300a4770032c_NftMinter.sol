...
contract NftMinter is ERC721Enumerable{
    ...
    bool presaleActive;
    uint presalePrice = 20 ether;
    uint presaleLimit = 500;
    uint totalLimit = 10000;
    ...
    string private CID = "QmW1i1Y9kcs3k3L7ghP56ZdK6cipZErj8DU6UNjnUY9Dj1/";

  

    address payable paymentSplittingAddress ;


    constructor() ERC721("MrHemp", "MH"){
        killSwitch = false;
        nftsMinted = 0;
        presaleActive = true;
        owner = msg.sender;
        paymentSplittingAddress = payable(0x08160119344e85D77Cd45Cb7F1078eD0f18429A2);
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if(!revealed){
            return string(abi.encodePacked(base,"SomeHashForDefaultImage.JSON"));
        }
    ...
    function mint(uint _amount) public payable {
        ...
        if(msg.sender != owner){
             if(presaleActive){
            require(msg.value>= presalePrice*_amount, "Not enough ether was sent");
        }else{
            require(msg.value >= price*_amount, "Not enough ether was sent");
        }
            (bool success,) = paymentSplittingAddress.call{value: msg.value}("");
            require(success,"Payment splitting failed");
        }
        require(_amount <= 10);

        require(nftsMinted +_amount<=10000, "Can not mint more than 10k NFTS");
        

        if(nftsMinted+_amount >= presaleLimit){
    ...
    function walletOfOwner(address queryWallet) public view returns (uint256[] memory) {
