...
// PolyDefi IDO with lock tier function into native farms
...
contract IDO

   {
    ...
    // Info of each tier
    struct TierInfo {
        uint256 lpOctagonUscdMinLock;     // Min lock amount of LP OCTAGON USDC for that tier
        uint256 lpOctagonWMaticMinLock; // Min lock amount of LP OCTAGON WMATIC for that tier
    ...
    // farm lock 1 address / LP OCTAGON USDC
    address public farmLockTierAddress1; 
    // farm lock 2 address / LP OCTAGON WMATIC
    address public farmLockTierAddress2;     
    ...
    constructor (
        ...
        address _farmLockTierAddress1, // LP OCTAGON USDC
        address _farmLockTierAddress2 // LP OCTAGON WMATIC
        
        
        ) public  {
    
        owner = msg.sender;
        farmLockTierAddress1 = _farmLockTierAddress1;
        farmLockTierAddress2 = _farmLockTierAddress2;
    
    }
 
    // Get locked amount in farm
    function getLock(address _user, uint256 _farmLockTierNumber) public view returns (uint256) {
        if (_farmLockTierNumber == 1)
        {
            (,,uint256 lockedAmount, ) = IFarmLockTier(farmLockTierAddress1).userInfo(address(_user));
            return lockedAmount;
        }
        else if (_farmLockTierNumber == 2)
        {
            (,,uint256 lockedAmount, ) = IFarmLockTier(farmLockTierAddress2).userInfo(address(_user));
            return lockedAmount;
        }
        else
        {
    ...
    function getTier(address _user) public view returns (TiersType) {
        ...
        //get user lock for each farm
        uint256 _userLockedAmountFarm1 = getLock(_user, 1);
        //get user lock for each farm
        uint256 _userLockedAmountFarm2 = getLock(_user, 2); 
        
        if (_userLockedAmountFarm1 > 0 || _userLockedAmountFarm2 > 0)
        {

        if (_userLockedAmountFarm1 >= platinum.lpOctagonUscdMinLock || _userLockedAmountFarm2 >= platinum.lpOctagonWMaticMinLock) 
        {
            return TiersType.platinum;
        }
        else if (_userLockedAmountFarm1 >= gold.lpOctagonUscdMinLock || _userLockedAmountFarm2 >= gold.lpOctagonWMaticMinLock) 
        {
            return TiersType.gold;
        }   
        else if (_userLockedAmountFarm1 >= silver.lpOctagonUscdMinLock || _userLockedAmountFarm2 >= silver.lpOctagonWMaticMinLock) 
        {
            return TiersType.silver;
        } 
        else if (_userLockedAmountFarm1 >= bronze.lpOctagonUscdMinLock || _userLockedAmountFarm2 >= bronze.lpOctagonWMaticMinLock) 
        {
    ...
    function initializeTier(uint _tier, uint256 _lpOctagonUscdMinLock, uint256 _lpOctagonWMaticMinLock, uint256 _minInvestment, uint256 _maxInvestment) public onlyOwner {
    
        if (_tier == 1)
        {
            bronze.lpOctagonUscdMinLock = _lpOctagonUscdMinLock;
            bronze.lpOctagonWMaticMinLock = _lpOctagonWMaticMinLock;
            bronze.minInvestment = _minInvestment;
            bronze.maxInvestment = _maxInvestment;
        }
        else if (_tier == 2)
        {
            silver.lpOctagonUscdMinLock = _lpOctagonUscdMinLock;
            silver.lpOctagonWMaticMinLock = _lpOctagonWMaticMinLock;
            silver.minInvestment = _minInvestment;
            silver.maxInvestment = _maxInvestment;
        }  
        else if (_tier == 3)
        {
            gold.lpOctagonUscdMinLock = _lpOctagonUscdMinLock;
            gold.lpOctagonWMaticMinLock = _lpOctagonWMaticMinLock;
            gold.minInvestment = _minInvestment;
            gold.maxInvestment = _maxInvestment;
        }   
        else if (_tier == 4)
        {
            platinum.lpOctagonUscdMinLock = _lpOctagonUscdMinLock;
            platinum.lpOctagonWMaticMinLock = _lpOctagonWMaticMinLock;
