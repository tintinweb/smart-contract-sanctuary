...
// Base contract for MasterChef/SakeMaster/etc chef contract interfaces

abstract contract BaseStrategyMasterChef is BaseStrategy, IERC20 {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;
    ...
    function balanceOfPool() public override view returns (uint256) {
        ...
        return amount;
        //return IMasterChef(chef).balanceOf(poolId, address(this));
    }

    function getHarvestable() external override view returns (uint256) {
        return IMasterChef(chef).earned(poolId, address(this));
    }
    ...
    function deposit() public override {
        ...
        if (_want > 0) {
            ...
            IERC20(want).safeApprove(chef, _want);
            IMasterChef(chef).deposit(poolId, _want, strategist);
        }
    }
    ...
    function _withdrawSome(uint256 _amount)
        ...
        returns (uint256)
    {
        IMasterChef(chef).withdraw(poolId, _amount);
        return _amount;
    }
    ...
    function _getReward() internal {
        IMasterChef(chef).withdraw(poolId, 0);
        //Usually different for other pools based on IMasterChef
        //IMasterChef(chef).claim(poolId);
    }
...
abstract contract StrategyFarmSingle is BaseStrategyMasterChef {
    ...
    address public constant rewardToken = 0x3a3Df212b7AA91Aa0402B9035b098891d276572B; // FISH token contract address of Polycat Finance
    address public multiHarvest = 0x0000000000000000000000000000000000000000; //Auto-compounder
    ...
    address public constant fish = rewardToken;
    address public constant Y = 0x0000000000000000000000000000000000000000; //IRON Stablecoin (Placeholder)
    address public constant X = 0x0000000000000000000000000000000000000000; //TITAN token (Placeholder)
    address public constant usdc = 0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174;

    address[] public fish_X_path;
    address[] public fish_usdc_path;
    ...
    {
        fish_X_path = new address[](3); //placeholder for swapping multiple tokens
        fish_X_path[0] = fish;
        fish_X_path[1] = X;
        fish_X_path[2] = Y;
        
        fish_usdc_path = new address[](2);
        fish_usdc_path[0] = fish;
        fish_usdc_path[1] = usdc;
    }

    // **** State Mutations ****

    function set_fee(uint256 NewFee) external  {
        require(NewFee <= 100, "Cannot be more than 100");
        require(msg.sender == multiHarvest || msg.sender == owner(),"caller not the owner, multiHarvest");
        keepFXS = NewFee;
    }
    ...
    function set_harvestCutoff(uint256 newCutoff) external onlyOwner() {
        ...
        require(msg.sender == owner() || msg.sender == multiHarvest , "not authorized");
    ...
    function harvest() public override {
        ...
        if (_harvestBalance > harvestCutoff){
            if (rewardToken != want) {
                _swapUniswapWithPath(fish_usdc_path, _harvestBalance);
            }
            ...
            if (_want > 0) {
                ...
                if (performanceFee>0) {
                    _swapUniswapWithPath(fish_usdc_path, performanceFee);
...
contract StrategySingle is StrategyFarmSingle {
    ...
    // Token addresses
    address public Fish_MASTER_CHEF = 0x8CFD1B9B7478E7B0422916B72d1DB6A9D513D734; //Masterchef of FISH pool
    address public Want_Token = 0x3a3Df212b7AA91Aa0402B9035b098891d276572B; // FISH token contract address of Polycat Finance
