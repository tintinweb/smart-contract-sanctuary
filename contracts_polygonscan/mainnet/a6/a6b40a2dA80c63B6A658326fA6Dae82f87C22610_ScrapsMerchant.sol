...
contract ScrapsMerchant is Ownable, VRFConsumerBase {
  ...
  mapping(uint256 => uint256) public sprayCosts;
    ...
    )
  {
    ...
    jys = JunkyardScraps(0x1615236f3A8B1A91A9eBA73525C1447D1777970F);
    sprays = JunkyardSprays(0x064807a788503eaA9b6f57178dD5706b51418f07);
    blueprints = JunkyardBlueprints(0xB5Ca6326A5C7C5543d219CF003563c7c04e84dEa);
  ...
  function backupFulfillRandomness(bytes32 requestId, uint256 randomness) external onlyOwner {
  ...
  function MATICPrice() public view returns (uint256) {
    uint256 v = uint256(priceFeed.latestAnswer()); // Get real value
  ...
  function buySprayCan(uint256 sprayId, uint256[] memory scrapIds, uint256[] memory scrapAmounts) public {
    require(scrapIds.length == scrapAmounts.length, "Input arrays length must match");
    require(sprayCosts[sprayId] > 0, "Incorrect spray");
    uint256 burnRate = jys.burningRate();
    require(spendScraps(burnRate, scrapIds, scrapAmounts) >= sprayCosts[sprayId]);
    sprays.mint(msg.sender, sprayId, 1);
  }

  function buyBlueprint(uint256 blueprintId, uint256[] memory scrapIds, uint256[] memory scrapAmounts) public {
    require(scrapIds.length == scrapAmounts.length, "Input arrays length must match");
    uint256 burnRate = jys.burningRate();
    uint256 blueprintCost = burnRate * burnRate;
    require(spendScraps(burnRate, scrapIds, scrapAmounts) >= blueprintCost);
    blueprints.mint(msg.sender, blueprintId, 1);
  }

  function setChance(uint256 level, uint256 chance) public onlyOwner {
    require(level >= 1 && level <= 3, "Incorrect level");
    require(chance >= 1 && chance <= 100, "Incorrect chance");
    chances[level] = chance;
  }

  function setSprayCosts(uint256 tokenId, uint256 cost) public onlyOwner {
    require(tokenId >= 1, "Invalid spray");
    require(cost > 0, "Invalid Cost");
    sprayCosts[tokenId] = cost;
  }
  ...
  function setWithdrawer(address w) public onlyOwner {
    withdrawer = w;
  }

  function withdraw() public {
    require(msg.sender == owner() || msg.sender == withdrawer, "Not allowed");
