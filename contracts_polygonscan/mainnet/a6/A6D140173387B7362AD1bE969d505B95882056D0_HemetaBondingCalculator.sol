...
// File: contracts/HemetaBondingCalculator.sol
...
contract HemetaBondingCalculator is IHemetaBondingCalculator {
    ...
    function markdown( address _pair ) external view override returns ( uint ) {
        ...
        uint reserve;
        if ( IUniswapV2Pair( _pair ).token0() == HETA ) {
            reserve = reserve1;
        } else {
            reserve = reserve0;
        }
        return reserve.mul( 2 * ( 10 ** IERC20( HETA ).decimals() ) ).div( getTotalValue( _pair ) );
    }
}