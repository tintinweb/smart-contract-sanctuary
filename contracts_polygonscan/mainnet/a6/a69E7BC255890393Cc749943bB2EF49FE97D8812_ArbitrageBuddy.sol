...
/// @dev Compare price denominated in quote token between two pools
    ...
    function calcBorrowAmount(OrderedReserves memory reserves) internal pure returns (uint256 amount) {
        ...
        */
        ...
        uint maxBitsNeeded = bitInfo.bitsNeeded_a >= bitInfo.bitsNeeded_b ? bitInfo.bitsNeeded_a : bitInfo.bitsNeeded_b;
        maxBitsNeeded = maxBitsNeeded >= bitInfo.bitsNeeded_c ? maxBitsNeeded : bitInfo.bitsNeeded_c;
        uint bitReductions = 0;
        if (maxBitsNeeded >= 256)
        {
            bitReductions = (maxBitsNeeded - 255 - 1);
        }
        ...
        if (bitInfo.bitsNeeded_m >= 256)
        {
            bitReductions += (bitInfo.bitsNeeded_m - 255 - 1);
        }
