    ...
    function autoCompound() public {
        ...
        for (uint256 i; i < names.length; i++) {
            ...
            if (_amount > 0) {
                ...
                rewardsFeeInCakes = _calculateAmountFee(rewardBalance);
                amounts = IRouter(strategyObj.liquidityRouter).getAmountsOut(rewardsFeeInCakes, path);
                if(amounts[1] > 100) {
                    rewardsFeeInCakes += pendingFeeInCakes;
                ...
                path[0] = WETH;
                path[1] = USDT;
                rewardsFeeInWETH = getAvalaibleWETH();
                amounts = IRouter(strategyObj.liquidityRouter).getAmountsOut(rewardsFeeInWETH, path);
                if(amounts[1] > 100) {
                    rewardsFeeInWETH += pendingFee;
                    IRouter(strategyObj.liquidityRouter).swapExactETHForTokens{value:rewardsFeeInWETH}(
