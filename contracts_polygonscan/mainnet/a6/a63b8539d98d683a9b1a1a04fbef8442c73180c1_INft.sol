...
// File contracts/nft/ERC1155AfterTransfer.sol

//SPDX-Imported-License-Identifier: Unlicense
pragma solidity ^0.8.0;
...
// File contracts/nft/ERC1155Enumerable.sol
...
// File contracts/nft/openSea/ContextMixin.sol

// SPDX-Imported-License-Identifier: UNLICENSED
...
// File contracts/nft/openSea/Initializable.sol
...
// File contracts/nft/openSea/EIP712Base.sol
...
// File contracts/nft/openSea/NativeMetaTransaction.sol
...
// File contracts/nft/codecs/INftCodec.sol
...
///@title Core ERC-1155 iNFT contract.
///@author CryptoDragons.app.
contract INft is
    ...
    NativeMetaTransaction
{
    ...
    ///@dev Used in the creation of the EIP-712 domain.
    string public constant name = "Cryptodragons.app iNft";
    ...
    ///@dev Null byte value for empty data.
    bytes private constant EMPTY = "";
    ///@dev The maximum quantity allowed to be considered an NFT.
    uint256 private constant SINGLE_NFT = 1;

    /**
     * @dev Creates `amount` tokens of token type `id`, and assigns them to `to`.
     * The lower 64 bits of `id` are set to {nextTokenId} to ensure uniqueness.
     * For semi-fungible or fungible items, ensure that the total supply is created during the initial mint,
     * as there will not be an opportunity to mint further items of that class later.
     *
     * Emits a {TransferSingle} event.
     *
     * Requirements:
    ...
    function mint(
        ...
        require(id.isFungible() || SINGLE_NFT == amount, "NFTs cannot have amounts");

        super.mint(to, id.setId(nextTokenId++), amount, data);
    }
     ...
     * The lower 64 bits of each entry in `ids` is set to increments of {nextTokenId} to ensure uniqueness.
     * For semi-fungible or fungible items, ensure that the total supply is created during the initial mint,
     * as there will not be an opportunity to mint further items of that class later.
     *
     * Emits a {TransferBatch} event.
     ...
     * - If `to` refers to a smart contract, it must implement {IERC1155Receiver-onERC1155Received} and return the
     * acceptance magic value.
     * - `ids` must have their fungibility bit set to fungible if amounts > {SINGLE_NFT} are to be minted.
     */
    function mintBatch(
        address to,
        uint256[] memory ids,
        ...
        for (uint256 i = 0; i < ids.length; ++i) {
            require(ids[i].isFungible() || 1 == amounts[i], "NFTs cannot have amounts");

            ids[i] = ids[i].setId(nextTokenId_++);
        }
    ...
    function mintBatch(
        ...
        mintBatch(to, ids, amounts, EMPTY);
    }
     ...
     * Emits a {TransferBatch} event.
     *
     * This is a convenience overload for minting NFTs that doesn't require an amounts parameter, which is set to
     * an array of {SINGLE_NFT} before calling the main overload.
     *
     * Requirements:
     *
     * - `to` cannot be the zero address.
     ...
     * - `ids` must have their fungibility bit set to fungible if amounts > {SINGLE_NFT} are to be minted.
     */
    function mintNFTBatch(address to, uint256[] memory ids) public {
        mintNFTBatch(to, ids, EMPTY);
    }
     ...
     * to a multisig before renouncing the admin role from the deployer wallet.
     */
    function setFlag() external onlyRole(DEFAULT_ADMIN_ROLE) {
        flag = true;
    }
     ...
     * this function emits no events.
     *
     * This is intended to be used to allow migration to IPFS when ready.
     */
    ...
    /**
     * @dev Allows gasless transfers of NFTs on OpenSea, see
     * https://github.com/ProjectOpenSea/meta-transactions/tree/main/contracts and
     * https://docs.opensea.io/docs/polygon-basic-integration for details
     *
     * Returns true if `_operator` is approved to transfer ``_owner``'s tokens
     * or if the operator is OpenSea's proxy contract.
     *
     */
     ...
     * is concerned).
     *
     * This is used instead of msg.sender as transactions won't be sent by the
     * original token owner, but by OpenSea.
     *
     */
    function _msgSender()
        internal
     ...
     * and burning, as well as batched variants.
     *
     * Noop override, required due to multiple inheritence.
     */
    function _beforeTokenTransfer(
        address operator,
        address from,
    ...
    function _burn(
        ...
        uint256 id,
        uint256 amount
    ) internal virtual override(ERC1155, ERC1155AfterTransfer) {
        super._burn(account, id, amount);
    }
    ...
    function safeBatchTransferFrom(
        ...
        uint256[] memory amounts,
        bytes memory data
    ) public virtual override(ERC1155, ERC1155AfterTransfer) {
        super.safeBatchTransferFrom(from, to, ids, amounts, data);
    }
