...
import "./curvefi/ICurve_ERC20Y.sol";

import "./interfaces/IERC20.sol";


contract Earnboku is OwnableUpgradeable {
    ...
    function balanceOfAll() public view returns(uint256[4] memory balances) {
        ...
        for (uint i = 0; i < stablecoins.length; i++) {
            ...
            uint256 yPrice = ICurve_ERC20Y(yCoin).getPricePerFullShare();
    ...
    function normalizedBalance() public view returns(uint256) {
        ...
        uint256 summ;
        for (uint256 index = 0; index < stablecoins.length; index++) {
            summ = summ.add(normalize(stablecoins[index], balances[index]));
        }
        return summ;
    }
     ...
     */
    function deposit(uint256[4] memory _amounts) public {
        address[4] memory stablecoins = ICurveFi_DepositY(curveFi_Deposit).underlying_coins();

        for (uint256 i = 0; i < stablecoins.length; i++) {
            IERC20(stablecoins[i]).transferFrom(_msgSender(), address(this), _amounts[i]);
            IERC20(stablecoins[i]).approve(curveFi_Deposit, _amounts[i]);
        }
     ...
     * @param _amounts Array of amounts for CurveFI stablecoins in pool (denormalized to token decimals)
     */
    function withdraw(uint[4] memory _amounts) public {
        address[4] memory stablecoins = ICurveFi_DepositY(curveFi_Deposit).underlying_coins();
        ...
        for (i = 0; i < stablecoins.length; i++) {
            ...
            uint amount = (balance <= _amounts[i]) ? balance : _amounts[i]; //Safepoint for rounding
