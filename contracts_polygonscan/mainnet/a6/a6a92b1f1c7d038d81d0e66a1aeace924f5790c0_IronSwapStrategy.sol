...
abstract contract BaseStrategyIron is BaseStrategy {
    ...
    address public ERC_FUND = 0x43c085B5aba4E83a7dd1A478B7458D1d472FF6b4;
    ...
    function swapRewardToWmaticAndDistributeFee(uint256 feeAmount) internal {
        if(feeAmount > 0) {
            _swapUniswapWithPath(ICE_USDC_path, feeAmount, DFYN_ROUTER);

            uint256 _usdcBal = IERC20(USDC).balanceOf(address(this));
            _swapUniswapWithPath(USDC_WMATIC_path, _usdcBal, QUICKSWAP_ROUTER);
...
contract IronSwapStrategy is BaseStrategyIron {

    address public constant IS3USD = 0xb4d09ff3dA7f9e9A2BA029cb0A81A989fd7B8f17;
    address public constant IRON_SWAP = 0x837503e8A8753ae17fB8C8151B8e6f586defCb57;

    constructor() public BaseStrategyIron(IS3USD, DFYN_ROUTER, 0) {
        IERC20(USDC).safeApprove(IRON_SWAP, uint256(-1));
    }

    // **** State Mutations ****

    function harvest() public override onlyHumanOrWhitelisted nonReentrant {
        //Transfer any ICE, USDC, WMATIC that may already be in the contract to the fee dist fund
        IERC20(harvestedToken).safeTransfer(strategist, IERC20(harvestedToken).balanceOf(address(this)));
        IERC20(USDC).safeTransfer(strategist, IERC20(USDC).balanceOf(address(this)));
        ...
        if (_harvested_balance > 0) {
            ...
            //Swap 1/2 of remaining ICE for USDC
            _swapUniswapWithPath(ICE_USDC_path, afterFeeAmount.div(2));
        }

        //Add liquidity to IronSwap
        uint256 usdc_balance = IERC20(USDC).balanceOf(address(this));
        if(usdc_balance > 0) {
            uint256[] memory amounts = new uint256[](3);
            amounts[0] = usdc_balance; //usdc
            amounts[1] = 0; //usdt
            amounts[2] = 0; //dai

            IIronSwap(IRON_SWAP).addLiquidity(amounts, 0, now + 60);
        }
