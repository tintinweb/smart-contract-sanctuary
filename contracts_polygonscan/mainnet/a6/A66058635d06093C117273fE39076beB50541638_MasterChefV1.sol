...
contract MasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        uint256 boostAmount;     // How many LP tokens the user has provided.
        uint256 xratio;     // How many LP tokens the user has provided.
    ...
    function add(uint256 _allocPoint, address _lpToken, bool _withUpdate) public {
    ...
    function pendingSushi(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (pending > 0) return pending.mul(user.boostAmount).div(user.amount);
        return pending;
    }

    function _pendingXSushi(UserInfo storage user, uint256 pending) internal view returns (uint256) {
        uint256 xpending = pending * user.xratio;
        return xpending;
    }
    ...
    function formatType(uint256 _type) public pure returns (uint256) {
        if (_type == 0) return 0;
        else if (_type <= 10) return 10;
        else if (_type <= 30) return 30;
        else if (_type <= 90) return 90;
        else if (_type <= 180) return 180;
        else return 365;
    }
    function getTimeMultiplex(uint256 _type) public pure returns (uint256) {
        if (_type == 0) return 100;
        else if (_type <= 10) return 105;
        else if (_type <= 30) return 115;
        else if (_type <= 90) return 150;
        else if (_type <= 180) return 200;
        else return 250;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount, uint256 _type) public payable {
        ...
        if(_amount > 0) {
            ...
            duration = ((lockUntil - block.timestamp) * user.amount + _type * (1 days) * _amount) / (user.amount + _amount);
            user.untilLock = block.timestamp + duration;
            user.boostAmount = user.boostAmount.add(_amount * getTimeMultiplex(_type) / 100);
    ...
    function emergencyWithdraw(uint256 _pid) public {
        ...
        require(block.timestamp > bonusEndBlock + 30 * 40000, "!emergency withdraw: wut?");
