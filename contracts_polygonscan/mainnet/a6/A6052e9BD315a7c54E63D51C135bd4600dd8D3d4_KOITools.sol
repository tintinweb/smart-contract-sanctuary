...
contract KOITools is Ownable {
  ...
  function GenderStatsByAddress(address _address)
    external
    view
    returns (uint256[] memory)
  {
    ...
    uint256[] memory _KOI_ids = KOIContract.tokensByOwner(_address);
    for (uint256 i = 0; i < _KOI_ids.length; i++) {
      _Gender = KOIContract.Gender(_KOI_ids[i]);
      if (output[_Gender] > 0) {
  ...
  function LevelStatsByAddress(address _address)
    external
    view
    returns (uint256[] memory)
  {
    ...
    for (uint256 i = 0; i < _KOI_ids.length; i++) {
      _Level = KOIContract.level(_KOI_ids[i]);
  ...
  function ClaimAURA(uint256 _KOI_id) public {
    ...
    try KOIContract.Luckymoney(_KOI_id) returns (uint256 _result) {
      _AURAclaimable = _result / 7e18;
    } catch {
      _AURAclaimable = 0;
    }
  ...
  function KOI_LuckyMoneyCollectable(address _owner)
    external
    view
    returns (uint256)
  {
