...
import {IMintableInterface} from "../collection/IMintableInterface.sol";
...
contract BatchAirdrop is AccessControl {
    ...
    function mintBatch(
        ...
        require(wallets.length == tokenIds.length, "Incorrect input data size");
        IMintableInterface collection = IMintableInterface(collectionAddr);
        for (uint256 i = 0; i < tokenIds.length; i++) {
            collection.mint(wallets[i], tokenIds[i]);
        }
    }
}
