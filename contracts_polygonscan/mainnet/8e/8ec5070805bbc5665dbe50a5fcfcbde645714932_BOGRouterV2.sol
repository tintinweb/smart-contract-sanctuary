...
contract BOGRouterV2 {
    ...
    function swapMATICToken(
        ...
        require(path[0] == WMATIC, 'BOGRouter: INVALID_PATH');
        IWMATIC(WMATIC).deposit{value: amountIn}();
        assert(IWMATIC(WMATIC).transfer(executors[0].receiver(path[0], path[1]), amountIn));
        uint256 balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swap(executors, path, to);
        uint256 amountOutActual = IERC20(path[path.length - 1]).balanceOf(to) - balanceBefore;
        if(amountOutActual < amountOutMin){
    ...
    function swapTokenMATIC(
        ...
        require(path[path.length - 1] == WMATIC, 'BOGRouter: INVALID_PATH');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, executors[0].receiver(path[0], path[1]), amountIn
        );
        _swap(executors, path, address(this));
        uint256 amountOutActual = IERC20(WMATIC).balanceOf(address(this));
        IWMATIC(WMATIC).withdraw(amountOutActual);
        TransferHelper.safeTransferMATIC(to, amountOutActual);
        if(amountOutActual < amountOutMin){
            uint256 amountTax = amountOutCalculated - amountOutActual;
            require(
                amountTax < amountTaxMax,
