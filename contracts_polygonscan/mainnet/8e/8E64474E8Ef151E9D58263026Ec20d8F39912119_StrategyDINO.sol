...
contract StrategyDINO is Ownable {
    ...
    function takeFee() internal {
        ...
        if(_balanceWETH > 0) {
            ...
            if(rewardsFeeInWETH > 0) {
                ...
                if(amount > 100) {
                    _swapExactTokensForTokens(rewardsFeeInWETH, amount, path);
                    pendingFee = 0;
                } else {
    ...
    function _swapExactTokensForTokens(uint256 _amount, uint256 _amount2, address[] memory _path) internal {
        IRouter(liquidityRouter).swapExactTokensForTokens(
            _amount,
    ...
    function autoCompound() public {
        ...
        if (amount > 0) {
            ...
            if(amount > 100) {
                _swapExactTokensForTokens(_balance, amount, path);
            }
            ...
            _balance = getAvalaibleWETH();
            amounts = IRouter(liquidityRouter).getAmountsOut(_balance, path1);
            if(amounts[1] > 100 && amounts[2] > 100) {
                _swapExactTokensForTokens(_balance, amounts[2], path1);
    ...
    function _swapToYELs(uint256 _percent) internal returns (uint256 newYelBalance){ 
        ...
        if (_totalLP > 0) {
            ...
            newYelBalance = _oldYelBalance - _getBalanceOfToken(YELtoken);
    ...
    function _swapTokenToNativeToken(address _token) internal {
        ...
        if(_token != WETH) {
            ...
            if(amounts[1] > 100) {
                _swapExactTokensForTokens(_getBalanceOfToken(path[0]), amounts[1], path);
            }
        }
    }
