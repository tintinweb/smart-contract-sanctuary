...
contract DexUpdates is Ownable, DexStorage {
  ...
  function migrateNFT(address newCollection, address collectionFrom, uint256[] memory tokenIds) public {
    ...
    address to = IERC20(collectionFrom).ownerOf(tokenIds[i]);
    if(collectionFrom == address(XNFT)){
    INFT(collectionFrom).transferFromAdmin(to, address(0x0), tokenIds[i]);
    }
    uint256 tokenId;
    if(to == address(this)) {
      tokenId = INFT(newCollection).mint(address(this), uri);
      updateSellDetail(tokenIds[i], newCollection, collectionFrom);
    } else {
 ...
 function updateSellDetail(uint256 tokenId,address newCollection,  address collectionAdd) internal {
    fixedSell storage oldData = _saleTokens[tokenId][address(collectionAdd)];
    if(oldData.seller != address(0x0)){
      _saleTokens[tokenId][address(newCollection)].seller = oldData.seller;
      _saleTokens[tokenId][address(newCollection)].price = oldData.price;
      _saleTokens[tokenId][address(newCollection)].timestamp = oldData.timestamp;
      if(oldData.timestamp <= 1640173536 &&  collectionAdd == 0x2c3479B526394d9a5e18E2E454B9f8b1282930AC ){
           _allowedCurrencies[tokenId][address(newCollection)][0] = _allowedCurrencies[tokenId][address(0x2c3479B526394d9a5e18E2E454B9f8b1282930AC)][0];
           _allowedCurrencies[tokenId][address(newCollection)][1] = _allowedCurrencies[tokenId][address(0x2c3479B526394d9a5e18E2E454B9f8b1282930AC)][1];
           _allowedCurrencies[tokenId][address(newCollection)][2] = _allowedCurrencies[tokenId][address(0x2c3479B526394d9a5e18E2E454B9f8b1282930AC)][2];
           _allowedCurrencies[tokenId][address(newCollection)][3] = _allowedCurrencies[tokenId][address(0x2c3479B526394d9a5e18E2E454B9f8b1282930AC)][3];
      }else {
          _allowedCurrencies[tokenId][address(newCollection)][0] =  _allowedCurrencies[tokenId][address(collectionAdd)][0];
           _allowedCurrencies[tokenId][address(newCollection)][1] = _allowedCurrencies[tokenId][address(collectionAdd)][1];
           _allowedCurrencies[tokenId][address(newCollection)][2] = _allowedCurrencies[tokenId][address(collectionAdd)][2];
           _allowedCurrencies[tokenId][address(newCollection)][3] = _allowedCurrencies[tokenId][address(collectionAdd)][3];
      }
     
      if(oldData.seller == nonCryptoNFTVault){
        _nonCryptoOwners[tokenId][address(newCollection)] =  _nonCryptoOwners[tokenId][address(collectionAdd)];
      }
    } else {
        auctionSell storage oldD = _auctionTokens[tokenId][address(collectionAdd)];
      _auctionTokens[tokenId][address(newCollection)].seller = _auctionTokens[tokenId][address(collectionAdd)].seller;
      _auctionTokens[tokenId][address(newCollection)].nftContract = _auctionTokens[tokenId][address(collectionAdd)].nftContract;
      _auctionTokens[tokenId][address(newCollection)].minPrice = _auctionTokens[tokenId][address(collectionAdd)].minPrice;
      _auctionTokens[tokenId][address(newCollection)].startTime = _auctionTokens[tokenId][address(collectionAdd)].startTime;
      _auctionTokens[tokenId][address(newCollection)].endTime = _auctionTokens[tokenId][address(collectionAdd)].endTime;
      _auctionTokens[tokenId][address(newCollection)].bidder = _auctionTokens[tokenId][address(collectionAdd)].bidder;
      _auctionTokens[tokenId][address(newCollection)].bidAmount = _auctionTokens[tokenId][address(collectionAdd)].bidAmount;
      if(oldD.seller == nonCryptoNFTVault ){
         string memory ownerId = _nonCryptoOwners[tokenId][address(collectionAdd)];
        _nonCryptoOwners[tokenId][address(newCollection)] = ownerId;
        _auctionTokens[tokenId][address(newCollection)].isDollar = true;
      }
    }
  }
