...
contract VaultFrankenstein is ERC20, Ownable {
    ...
    uint256 public constant REQUIRED_NUMBER_OF_BLOCKS = 4;
    ...
    function removeStrategy(uint256 _nameID) public onlyOwnerOrGovernment {
    	// swap all LPs from strategy to Vault
    	require(nameExist[_nameID], "The Name of strategy with this ID does not exist");
    ...
    function _checkDeposit(uint256 _totalCapital, uint256 _depositValue) internal {
    ...
    // function withdraw(uint256 _shares) public whenNotBlocked {
    ...
    function claimYel() public {
    	for (uint256 i; i < names.length; i++) {
    		if(IStrategy(strategies[names[i]]).getPendingYel(msg.sender) > 0){
    			IStrategy(strategies[names[i]]).claimYel(msg.sender);
    		}
        }
    }

    function _isBlockedByTime() internal view returns (bool) {
    	uint256 currentBlock = block.number;
		return false ? currentBlock - requestBlock[msg.sender] >= REQUIRED_NUMBER_OF_BLOCKS : true;
    }
    ...
    // деньги на активную стратегий 
        // активную стратегию не можем удалить
        // 
}