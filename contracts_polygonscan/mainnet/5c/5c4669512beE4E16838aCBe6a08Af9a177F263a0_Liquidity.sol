...
contract Liquidity is Ownable {
    ...
    constructor(
      address _DESTRUCTION
    ) {
      DESTRUCTION = IERC20(_DESTRUCTION);
      _allowance();
    }

  receive() external payable {}

  function withdrawDestructionLiquidity() external onlyOwner {
      USDC.transfer(msg.sender, USDC.balanceOf(address(this)));
      DESTRUCTION.transfer(msg.sender, DESTRUCTION.balanceOf(address(this)));
  }
 ...
 function withdrawAll()  external onlyOwner {
  ...
  function AutoLiquidity() external onlyOwner {
    ...
    uint256 destructionBalance = DESTRUCTION.balanceOf(address(this));
    uint256 halfTokenAmount = destructionBalance / 2;

    if (usdcBalanceAfter > 0 && wmaticBalance > 0 && destructionBalance > 0) {
        // add the liquidity destruction-matic
        ...
        // add the liquidity destruction-usdc
        ROUTER.addLiquidity(
            address(DESTRUCTION),
            address(USDC),
            halfTokenAmount,
  ...
  function _allowance() internal {
    ...
    DESTRUCTION.approve(address(ROUTER), type(uint256).max);
  }
