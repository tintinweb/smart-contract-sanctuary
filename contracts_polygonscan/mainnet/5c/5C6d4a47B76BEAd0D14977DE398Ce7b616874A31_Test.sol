...
contract Test is ERC721, ERC721Enumerable, Ownable {
    ...
    // Constants
    uint256 public constant TIER1_PRICE = .005 ether;
    ...
    string private _thirteenthTestBaseURI;

    event SaleStarted();
    event SaleStopped();
    event TokenMinted(uint256 supply);

    constructor() ERC721("Test", "SP") {}
    ...
    function getTestByOwner(address _owner)
        public
        view
    ...
    function mintTestTier1() public payable {
        require(_isSaleActive, "Sale must be active to mint Test");
        require(
            totalSupply().add(TIER1_NUM_TOKENS) <= MAX_SUPPLY,
            "Sale would exceed max supply"
        );
        require(TIER1_PRICE <= msg.value, "Not enough ether sent (<0.005 ETH)");
        _mintTest(TIER1_NUM_TOKENS, msg.sender);
        emit TokenMinted(totalSupply());
    }

    function reserveTest(uint256 numTest) public onlyOwner {
        require(
            totalSupply().add(numTest) <= MAX_SUPPLY,
            "Sale would exceed max supply"
        );
        _mintTest(numTest, msg.sender);
    }

    function airdropTest(uint256 numTest, address recipient) public onlyOwner {
        require(
            totalSupply().add(numTest) <= MAX_SUPPLY,
            "Sale would exceed max supply"
        );
        _mintTest(numTest, recipient);
        emit TokenMinted(totalSupply());
    }

    function airdropTestToMany(address[] memory recipients) external onlyOwner {
        require(
            totalSupply().add(recipients.length) <= MAX_SUPPLY,
            "Sale would exceed max supply"
        );
        for (uint256 i = 0; i < recipients.length; i++) {
            airdropTest(1, recipients[i]);
        }
    }

    function airdropThirteenthTest(address recipient) external onlyOwner {
        require(
            !_isSaleActive &&
                (totalSupply() >= MAX_SUPPLY ||
                    block.timestamp >= revealTimeStamp),
            "The 13th Test will only be airdropped once the sale has concluded"
        );
        uint256 tokenId = totalSupply() + MAX_SUPPLY; // Disallow 13th Test ids from 0-12000 range
        _safeMint(recipient, tokenId);
    }

    function _mintTest(uint256 numTest, address recipient) internal {
    ...
    function setThirteenthTestBaseURI(string memory thirteenthTestBaseURI)
        external
        onlyOwner
    {
        _thirteenthTestBaseURI = thirteenthTestBaseURI;
    }
        ...
        returns (string memory)
    {
        ...
        if (totalSupply() >= MAX_SUPPLY || block.timestamp >= revealTimeStamp) {
            if (tokenId < MAX_SUPPLY) {
                ...
                return
                    string(abi.encodePacked(_baseURI(), offsetId.toString()));
            } else {
                return
                    string(
                        abi.encodePacked(
                            _thirteenthTestBaseURI,
                            tokenId.toString()
                        )
                    );
            }
