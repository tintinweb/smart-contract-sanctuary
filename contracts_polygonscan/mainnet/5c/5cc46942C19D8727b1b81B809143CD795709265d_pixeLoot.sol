...
contract pixeLoot is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    constructor() ERC721("pixeLoot-PolygonTest2", "pxL-P2") {}
    ...
    {
        string memory imagePrefix = "data:image/gif;base64,";
        //
        bytes memory gif = bytes(
            abi.encodePacked(
                "\x47\x49\x46\x38\x39\x61\x01\x00\x01\x00\xf0\x00\x00\x00\x00\x00\x00\x00\x00\x21\xf9\x04\x00\x00\x00\x00\x00\x2c\x00\x00\x00\x00\x01\x00\x01\x00\x00\x02\x02\x44\x01\x00\x3b"
            )
        );

        bytes3 colorBytes = generateColorBytes(color);

        // Loop 3 times to overwrite the 3 color bytes with values from color.
        for (uint8 i = 0; i <= 2; i++) {
            gif[i + 13] = colorBytes[i];
        }

        return abi.encodePacked(imagePrefix, Base64.encode(string(gif)));
    }

    function generateColorBytes(uint256 color) internal pure returns(bytes3 colorBytes) {
        return bytes3(abi.encodePacked(uint32(color) << 8));
    }

    function generateColorString(uint256 color) internal pure returns(bytes memory) {
        // Return representation of the color hex code in bytes array.
        bytes memory colorString = bytes(uint256(bytes32(abi.encodePacked(generateColorBytes(color)))).toHexString());

        return abi.encodePacked(colorString[2],colorString[3],colorString[4],colorString[5],colorString[6],colorString[7]);
    }
        ...
        returns (string memory)
    {
        ...
        bytes memory imageURI = generateImageURI(color);

        bytes memory colorString = generateColorString(color);
...
// Updated from https://github.com/OpenZeppelin/solidity-jwt/blob/master/contracts/Base64.sol
