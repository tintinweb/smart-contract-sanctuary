...
abstract contract BaseStrategy is Ownable {
    ...
    address public constant weth = 0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270; //weth for Matic
...
abstract contract StrategyFarmBase is BaseStrategyMasterChef {
    ...
    address public constant beta = 0xaC3090B7042FCA2cDBF233022e4a9823a032600c;
    ...
    // Other constants
    uint16 public constant underlyingPoolId = 14;

    uint256 public keepFXS = 650;
    uint256 public harvestCutoff = 10**12;

    uint256 public constant keepFXSmax = 10000;

    // Uniswap swap paths
    address[] public beta_kogecoin_path;
    ...
    {
        beta_kogecoin_path = new address[](2);
        beta_kogecoin_path[0] = beta;
        beta_kogecoin_path[1] = kogecoin;
    }
    ...
    function set_harvestCutoff(uint256 newCutoff) external onlyOwner() {
        require(newCutoff <= 10**18, "New cutoff must be less than 1 Yeld");
    ...
    function harvest() public override {
        ...
        // Collects BETA tokens
        _getReward();

        // Swap 1/2 BETA for KOGECOIN
        uint256 _betaBalance = IERC20(beta).balanceOf(address(this));
        if (_betaBalance > harvestCutoff){
            if (_betaBalance > 0) {
                _swapUniswapWithPath(beta_kogecoin_path, _betaBalance.div(2));
            }

            // Add liquidity for BETA/KOGECOIN
            uint256 _kogecoin = IERC20(kogecoin).balanceOf(address(this));
            uint256 _beta = IERC20(beta).balanceOf(address(this));
            if (_kogecoin > 0 && _beta > 0) {
                IERC20(kogecoin).safeApprove(currentRouter, 0);
                IERC20(kogecoin).safeApprove(currentRouter, _kogecoin);
                IERC20(beta).safeApprove(currentRouter, 0);
                IERC20(beta).safeApprove(currentRouter, _beta);
                ...
                // Donates DUST
                IERC20(kogecoin).safeTransfer(
                    strategist,
                    IERC20(kogecoin).balanceOf(address(this))
                );
                IERC20(beta).safeTransfer(
                    strategist,
                    IERC20(beta).balanceOf(address(this))
                );
            }
...
contract StrategyBase is StrategyFarmBase {
    // Token addresses
    address public BETA_MASTER_CHEF = 0x9581EA83B4BCd5F2c5f1705382FBd80a11E57DcD;
    address public BETA_KOGECOIN_LP = 0x83B6f8B40AA5FE76B2bbA0dF5073112785Bd7f49;
