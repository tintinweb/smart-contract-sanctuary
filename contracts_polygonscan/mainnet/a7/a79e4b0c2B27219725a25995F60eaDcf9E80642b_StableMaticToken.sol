...
// File: contracts/StableMaticToken.sol
...
contract StableMaticToken is ERC20("Mock StableMatic Token", "mSMT"), Ownable {
  ...
  function canUnlockAmount(address _account) public view returns (uint256) {
    // When block number less than startReleaseBlock, no SMT can be unlocked
    if (block.number < startReleaseBlock) {
      return 0;
    }
    // When block number more than endReleaseBlock, all locked SMT can be unlocked
    else if (block.number >= endReleaseBlock) {
      return _locks[_account];
    }
    // When block number is more than startReleaseBlock but less than endReleaseBlock,
    // some SMT can be released
    else
    {
  ...
  function unlock() public {
    require(_locks[msg.sender] > 0, "no locked SMT");
  ...
  // @dev move SMT with its locked funds to another account
  ...
  function _delegate(address delegator, address delegatee) internal {
    ...
    uint256 delegatorBalance = balanceOf(delegator); // balance of underlying SMT (not scaled);
