...
contract LIME is ERC20('PolyLime', 'LIME') {
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of Limees
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. Limees to distribute per block.
        uint256 lastRewardBlock;  // Last block number that Limees distribution occurs.
        uint256 accLimePerShare;   // Accumulated Limees per share, times 1e18. See below.
    ...
    uint256 public LimePerBlock = 0.008 ether;
    
    uint256 constant MAX_LIME_PERBLOCK= 0.02 ether;
    ...
    // View function to see pending Limees on frontend.
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalRewards  <= maxSupply) {
            ...
            Lime.mint(devAddress, devReward);   
    ...
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accLimePerShare).div(1e18).sub(user.rewardDebt);
            if (pending > 0) {
    ...
    // Safe Lime transfer function, just in case if rounding error causes pool to not have enough FOXs.
    function safeLimeTransfer(address _to, uint256 _amount) internal {
        uint256 LimeBal = Lime.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > LimeBal) {
            transferSuccess = Lime.transfer(_to, LimeBal);
        } else {
            transferSuccess = Lime.transfer(_to, _amount);
        }
        require(transferSuccess, "safeLimeTransfer: Transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _LimePerBlock) external onlyOwner {
        require(_LimePerBlock<=MAX_LIME_PERBLOCK,"Too high");
        massUpdatePools();
        LimePerBlock = _LimePerBlock;
        emit UpdateEmissionRate(msg.sender, _LimePerBlock);
    }
