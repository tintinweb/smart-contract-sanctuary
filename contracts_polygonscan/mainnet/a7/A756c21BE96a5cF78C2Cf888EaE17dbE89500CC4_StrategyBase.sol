...
interface ILiquidityHelper {
    function addArcadiumLiquidity(address baseTokenAddress, uint256 baseAmount, uint256 nativeAmount) external;
}
...
abstract contract StrategyFarmBase is BaseStrategyMasterChef {
    ...
    address public constant arcadiumLiquidityRouter = 0x75eF87C8994c6c7Bc07C80fEf255AFb32dCE77C2;
    ...
    address[] public matic_arcadium_path;
        ...
        )
    {
        ...
        matic_arcadium_path = new address[](2);
        matic_arcadium_path[0] = matic;
        matic_arcadium_path[1] = arcadium;
    }
    ...
    function harvest() public override {
        ...
        if (_maticBalance > harvestCutoffMatic){
            ...
            _swapUniswapWithPath(matic_arcadium_path, _maticBalancePost.div(2));
            
            // Add liquidity
            uint256 _matic = IERC20(matic).balanceOf(address(this));
            uint256 _arcadium = IERC20(arcadium).balanceOf(address(this));
            if (_matic > 0 && _arcadium > 0) {
                IERC20(matic).safeApprove(arcadiumLiquidityRouter, 0);
                IERC20(matic).safeApprove(arcadiumLiquidityRouter, _matic);
                IERC20(arcadium).safeApprove(arcadiumLiquidityRouter, 0);
                IERC20(arcadium).safeApprove(arcadiumLiquidityRouter, _arcadium);

                ILiquidityHelper(arcadiumLiquidityRouter).addArcadiumLiquidity(
                    matic,
                    _matic,
                    _arcadium
                );
                ...
                IERC20(arcadium).safeTransfer(
                    strategist,
                    IERC20(arcadium).balanceOf(address(this))
                );
            }
...
contract StrategyBase is StrategyFarmBase {
    ...
    address public Want_Token = 0x0bbAFa51e13829C9e96AA84b158F51fC80b276E2;
