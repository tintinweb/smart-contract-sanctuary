...
contract SpaceShipsus is ERC721URIStorage, Ownable {
    ...
    event Hit(address attacker, address defender, uint damage);
    event EndFightResult(uint damageAttacker, uint damageDefender, bool didAttackerWin);
    ...
    function getNextLevelCost(address _account) internal view returns(uint levelCost, uint levelNext) {
         ShipInfo memory nftInfo = allShips[_account];
         uint currentLevel = nftInfo.level;
         uint nextLevel = currentLevel++;
         return((nextLevel * 20) + 50,nextLevel);
       ...
       _structHash = keccak256(
           ...
           if(_energySpent == 5) {
            ...
            if(_energySpent == 5 && randomNumber < LEGENDARY_WEIGHT) {
              ...
              nftInfo.maxEnergy += 2;
              nftInfo.cargo += 50;
              emit ExplorationReward(_account, 4, 750);
              ...
              nftInfo.maxEnergy += 1;
              nftInfo.cargo += 20;
              emit ExplorationReward(_account, 3, 500);
              ...
              nftInfo.life += 3;
              nftInfo.cargo += 10;
              emit ExplorationReward(_account, 2, 250);
           } else if(randomNumber < COMMON_WEIGHT) {
              goldRewards = 100;
              nftInfo.gold += 100;
              nftInfo.attack += 1;
              nftInfo.life += 1;
              emit ExplorationReward(_account, 1, 100);
           }
    ...
    function fightingSimulator(address _attacker, address _defender) public returns(address winner){
        ShipInfo memory attackerInfo = allShips[_attacker];
        ShipInfo memory defenderInfo = allShips[_defender];

        uint lifeAttacker = attackerInfo.life;
        uint attackAttacker = attackerInfo.attack;
        ...
        for(uint i = 0; i < 10; i++){
            ...
            if(hitOrMissAttacker){
                if(lifeDefender <= attackAttacker){
                    ...
                    emit Hit(_attacker, _defender, attackAttacker);
                    lifeDefender -= attackAttacker;
                    damageDealtAttacker += attackAttacker;
                }
            }

            hitOrMissDefender = randomHit(i);

            if(hitOrMissDefender) {
                if(lifeAttacker <= attackDefender) {
                    return _defender;
                } else {
                    emit Hit(_defender, _attacker, attackDefender);
        ...
        emit EndFightResult(damageDealtAttacker, damageDealtDefender, didAttackerWin);
