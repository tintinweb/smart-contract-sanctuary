...
contract Polygon_PearZap_Strat is Strat {
    constructor(
    ...
    function _farm() internal override {
        ...
        IPearZapFarm(farmContractAddress).deposit(pid, wantAmt, msg.sender);
    }

    function _unfarm(uint256 _wantAmt) internal override {
        IPearZapFarm(farmContractAddress).withdraw(pid, _wantAmt);
    }

    function unlockFromLocker() public onlyAllowGov {
       pearLocker.unlock();
    }

    // Update the pear locker contract address by the owner
    function setPearLocker(ILocker _pearLocker) public onlyAllowGov {
