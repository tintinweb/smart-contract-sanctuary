...
/**
 *      Write messages or write a book.
 * 
 *      Messages are stand alones.
 * 
 *      Books get a unique ID number with each new writer(address).
 *      Each subsequent contribution gets a subsequent page number.
 * 
 *      If you want to write more books get more addresses.
 */ 
...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    mapping(uint256 => mapping(uint256 => string)) book;
    mapping(uint256 => uint256) lastPage;
    ...
    function getWriterAddress(uint256 bookindexnr_) public view returns (address Writer) {
        return _writers[bookindexnr_];
    }
    ...
    function _readMessage(uint256 messageIdNr) public view returns(string memory) {
        return _messages[messageIdNr];
    }
    
    function _lastMessage() public view returns (string memory) {
        return lastMessage;
    }
    ...
    function writeBook(string memory newPage) public {
        if(_bookIndexNrs[msg.sender] == 0) {
            ...
            _writers[bookIndexnr] = msg.sender;
        }
        lastPage[_bookIndexNrs[msg.sender]] ++;
        book[_bookIndexNrs[msg.sender]][lastPage[_bookIndexNrs[msg.sender]]] = newPage;
    }

    function mint(string memory uri_, string memory newPage) public {
        require(_msgSender() == _maker, "caller not the owner");
        _tokenId++;
        _uris[_tokenId] = uri_;
