...
contract MasterChef is Ownable {
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (devMultiplier > 0){
            mm.safeTransfer(devaddr, mmReward.mul(devMultiplier).div(devFundDivRate));
        }	
        if (treasuryMultiplier > 0){
            mm.safeTransfer(treasuryAddr, mmReward.mul(treasuryMultiplier).div(treasuryDivRate));
        }
        // mining share already in this smart contract;
        pool.lastRewardBlock = block.number;
    }
    ...
    // claim all pending MM in all pools 
    function claimAll() public {
        for (uint256  pid = 0;pid < poolInfo.length;pid++){
            if (pendingMM(pid, msg.sender) > 0){
                withdraw(pid, 0);
            }
        }
    }
    ...
    function safeMMTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > mmBal) {
            mm.safeTransfer(_to, mmBal);
        } else {
            mm.safeTransfer(_to, _amount);
        }
    }
