  ...
  constructor(uint256 total) public {  
    ...
    external returns (bool) {
     ...
     function BountyCompound(address hunter, uint256 minAmount1, uint256 minAmount2, uint256 minLP1, uint256 minLP2) external returns (bool){
        ...
        uint256 bounty1 =IERC20(Quick).balanceOf(address(this)).div(1001);
        
       
        uint256 bounty2 = IERC20(WMATIC).balanceOf(address(this)).div(1001);
...
function latestAnswer() external view returns(int256){
    ...
    uint256 diff0 = 10**(18-(IERC20(WMATIC).decimals()));                // how to generally get token0 distingusihed???
    uint256 diff1 = 10**(18-(IERC20(USDT).decimals()));
    (uint256 Res0, uint256 Res1,) = QuickSwap(RELEVANTLP).getReserves();
    uint256 latestPrice0=  uint256(Chainlink(WMATIC).latestAnswer());
    uint256 latestPrice1=  uint256(Chainlink(USDT).latestAnswer());
    uint256 chainlinkDiff0 = 10**(18-Chainlink(WMATIC).decimals());
    uint256 chainlinkDiff1 = 10**(18-Chainlink(USDT).decimals());
    ...
    uint256 answerPrep = totalValue/(QuickSwap(RELEVANTLP).totalSupply());
    int256 answer = int256(answerPrep);
    
    
     
     uint256  ratio = Res0*diff0*PRECISION/(Res1*diff1);
