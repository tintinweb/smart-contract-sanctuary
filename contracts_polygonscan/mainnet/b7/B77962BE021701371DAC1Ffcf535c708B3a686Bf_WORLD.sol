...
contract WORLD is NFTbase, ReentrancyGuard, ILocalContract {
    ...
    uint256 public minimumPrice = 2000 * 10**18; // << stored in GAME (so 2000*10^18)
    uint256 public priceDivisor = 8;

    event MinimumPriceUpdated(uint256 minimumPrice);

    event TokenData(uint256 tokenId, string json, uint256 onSaleDate);
    ...
    function getMinimumPrice(uint256 tokenId) public view returns (uint256) {
        ...
        uint256 minPriceEnd = saleStartTime.add(4000);
        uint256 timeNow = block.timestamp;

        if (timeNow >= minPriceEnd)
            return 0;

        if(timeNow <= saleStartTime)
            return minimumPrice;

        return minimumPrice.mul(minPriceEnd.sub(timeNow)).div(4000);
    }

    function getBuyPrice(uint256 tokenId) public view returns (uint256) {
        uint256 supply = tokenSupply[tokenId];
        uint256 printPrice = getPrintPrice(supply + 1);
        uint256 minPrice = getMinimumPrice(tokenId);
        return printPrice > minPrice ? printPrice : minPrice;
    }

    function getSellPrice(uint256 tokenId) public view returns (uint256) {
        return getPrintPrice(tokenSupply[tokenId]);
    }
    ...
    function updateTokenData(
        ...
        emit TokenData(tokenId, json, onSaleDate);
    }

    function buyNFTwithGAME(uint256 tokenId, uint256 pricePaid) public beforeBuy(tokenId) {
        address sender = _msgSender();
        uint256 price = getBuyPrice(tokenId);

        gameToken.transferByContract(sender, address(this), pricePaid);

        _buyNFT(tokenId, price, pricePaid, sender);
    }

    function buyNFTwithMatic(uint256 tokenId, address[] calldata path) public payable nonReentrant beforeBuy(tokenId) {
    ...
    function _calculateEmission(
        ...
        // Calling this with a 2s gap (min gap), causes a negligible loss
        //   10,000 years in the future, the loss is 0.0001%.
        //   10 years in the future, the loss is under 0.00000000001%.
    ...
    function _updateUserBalance(address user, uint prevTotal) internal {
        ...
        // this calculation is 100% based on the previous balance.
    ...
    function _buyNFT(
        ...
        uint256 fee = pricePaid.sub(price);

        uint256 newSupply = tokenSupply[tokenId] + 1;
