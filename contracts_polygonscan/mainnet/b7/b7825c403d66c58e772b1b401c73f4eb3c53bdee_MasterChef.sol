...
// File: contracts/Quarks.sol


// SPDX-License-Identifier: MIT

pragma solidity 0.7.6;
contract Halo is ERC20("Halo", "HLO"), Ownable {
...
// MasterChef is the master of Halo. He can make HLO and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once hlo is sufficiently
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of hlo
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accHloPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accHloPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. HLO to distribute per block.
        uint256 lastRewardBlock;  // Last block number that hlo distribution occurs.
        uint256 accHloPerShare;   // Accumulated HLO per share, times 1e12. See below.
    ...
    IERC20 public busd;
    address public busdHloLP = address(0x0); 
    
    // Burn address.
    address public burnaddr;
    // SH13LD tokens created per block.
    uint256 public hloPerBlock = 1 ether;
    // Maximum emission rate
    uint256 public maxEmissionRate = 1 ether;
    // Bonus muliplier for early hlo makers.
    ...
    // Min harvest % : 25%
    uint256 public constant MINIMUM_HARVEST_PERCENT = 2500;
    ...
    // The block number when hlo mining starts.
    ...
    constructor(
        ...
        address _burnaddr,
        address _feeAddress,
        uint256 _hloPerBlock,
        uint256 _startBlock
    ) {
        halo = _hlo;
        burnaddr = _burnaddr;
        feeAddress = _feeAddress;
        hloPerBlock = _hloPerBlock;
    ...
    // Update the given pool's hlo allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending hlo on frontend.
    function pendingHlo(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accHloPerShare = pool.accHloPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 hloReward = multiplier.mul(hloPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accHloPerShare = accHloPerShare.add(hloReward.mul(1e12).div(lpSupply));
        }
        uint256 pending =  user.amount.mul(accHloPerShare).div(1e12).sub(user.rewardDebt);
    ...
    // View function to see if user can harvest hlo.	
    ...
    function updatePool(uint256 _pid) public {
        ...
        halo.mint(burnaddr, hloReward.div(10));
        halo.mint (address(this),hloReward);
        pool.accHloPerShare = pool.accHloPerShare.add(hloReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for hlo allocation.
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
       payOrLockupPendingHlo(_pid);
        if (_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accHloPerShare).div(1e12);

        updateEmissionIfNeeded();
        emit Deposit(msg.sender, _pid, _amount);
    }
    
    // Pay or lockup pending hlo.	
    function payOrLockupPendingHlo(uint256 _pid) internal {	
        PoolInfo storage pool = poolInfo[_pid];	
        UserInfo storage user = userInfo[_pid][msg.sender];	
        if (user.nextHarvestUntil == 0) {	
            user.nextHarvestUntil = block.timestamp.add(pool.harvestInterval);	
        }
         uint256 pending = user.amount.mul(pool.accHloPerShare).div(1e12).sub(user.rewardDebt);	
        if (canHarvest(_pid, msg.sender)) {	
            if (pending > 0 || user.rewardLockedUp > 0) {	
                ...
                user.nextHarvestUntil = block.timestamp.add(pool.harvestInterval);	
                // send rewards	
                safeHloTransfer(msg.sender, rewardCut);	
            }	
        } else if (pending > 0) {	
    ...
    // Safe hlo transfer function, just in case if rounding error causes pool to not have enough hlo.
    function safeHloTransfer(address _to, uint256 _amount) internal {
        uint256 hloBal = halo.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > hloBal) {
            transferSuccess = halo.transfer(_to, hloBal);
        } else {
            transferSuccess = halo.transfer(_to, _amount);
        }
        require(transferSuccess, "safeHloTransfer: transfer failed");
    }
   ...
   function updateEmissionIfNeeded() public {
        ...
        if (block.number - lastBlockUpdate > emissionUpdateInterval) {
            ...
            uint hloBalance = halo.balanceOf(busdHloLP);
          
            if (hloBalance > 0) {
           
                priceCents = busd.balanceOf(busdHloLP).mul(1e14).div(hloBalance);
            }

            // Update pools before changing the emission rate
            massUpdatePools();
            uint256 emissionRatePercent = getEmissionRatePercent(priceCents);
            hloPerBlock = maxEmissionRate.div(100).mul(emissionRatePercent);
        }
    }

    // for front end
    function getHloPriceCents() public view returns (uint256 spc){
         uint hloBalance = halo.balanceOf(busdHloLP);
          if (hloBalance > 0) {
            uint256 priceCents = busd.balanceOf(busdHloLP).mul(1e14).div(hloBalance);
            return priceCents;
          }
          return 0;
    }

    function getEmissionRatePercent(uint256 hloPriceCents) public view returns (uint256 epr) {
        
       if (hloPriceCents>=topPrice*100){return (1);}
       if (hloPriceCents<=bottomPrice*100){return (100);}

       uint256 hloPricePercentOfTop = hloPriceCents.mul(100).div(topPrice*100);
       int256 curveCoefficient = FixidityLib.reciprocal(int256(curveRate));
       uint256 f1 = uint256(100).sub(hloPricePercentOfTop);
       int256 p1 = FixidityLib.fromFixed(FixidityLib.multiply(curveCoefficient,int256(f1)));
       int256 p2 = FixidityLib.fromFixed(FixidityLib.divide(100,int256(hloPricePercentOfTop)));
    ...
    function updateEmissionRate(uint256 _hloPerBlock) public onlyOwner {
        massUpdatePools();
        hloPerBlock = _hloPerBlock;
        emit UpdateEmissionRate(msg.sender, _hloPerBlock);
    }
    ...
    // For checking prices to link emission rate
    function setBusdHloLPAddress(address _address) public onlyOwner {
        busdHloLP = _address;
    }
