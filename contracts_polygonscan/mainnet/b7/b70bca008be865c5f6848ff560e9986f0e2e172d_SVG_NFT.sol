  ...
  ERC721URIStorageUpgradeable
{
  ...
  function setSVGHeader(string calldata _svgHeader) external {
  ...
  function createURI(
    ...
    string calldata svg_text
  ) public view returns (string memory) {
    string[3] memory parts;
    parts[0] = svgHeader;

    parts[1] = svg_text;
    parts[2] = '</div></foreignObject></svg>';
  ...
  function mint(
    ...
    require(bytes(svg_text).length > 0, 'NO SVG TEXT');
    require(bytes(svg_text).length < 1096, 'EXCEEDS TEXT CAP');
    uint256 tokenId = _tokenIds.current();
    _safeMint(_msgSender(), totalSupply());
    _setTokenURI(tokenId, createURI(name, description, svg_text));
    _tokenIds.increment();
  ...
  function update(
    ...
    require(bytes(name).length > 0, 'NO Name');
    require(bytes(svg_text).length > 0, 'NO SVG TEXT');
    require(tokenMinter[tokenID] == msg.sender, 'Not Token Creator');
    require(tokenHolder[tokenID] == msg.sender, 'NO UPDATE PRIV');
    _setTokenURI(tokenID, createURI(name, description, svg_text));
    return tokenID;
  }
  ...
  /* Inits Contract */
  function initialize(string calldata fullName, string calldata symbl)
    public
    initializer
  {
    __ERC721_init(fullName, symbl);
    __Ownable_init();
  }
