...
interface PreviousPunksMaticContract{
	function punkIndexToAddress(uint punkIndex) external view returns(address);
}
contract PunksMaticMarket is ERC721Enumerable, Ownable, ReentrancyGuard {
	...
	PreviousPunksMaticContract public constant previousPunksMaticContract = PreviousPunksMaticContract(0x4be796EBA3482347059269aaf0f30fC47cc63856);

    uint256 public claimPriceMatic = 1000000000000000000000; // 100 Matic
    uint256 public claimPriceWETH = 50000000000000000; // 0.05 WETH
    uint256 public referralFee = 5;
    uint256 public referralDiscount = 5;
    // WETH On Matic 
    ERC20 public constant weth = ERC20(0x7ceB23fD6bC0adD59E62ac25578270cFf1b9f619);
    ...
    constructor () ERC721("PunksMatic", "\xCF\x81UNKS") {
        punksRemainingToAssign = 10000;
    }


    function updateWETHPrice(uint newPrice) public onlyOwner{
        claimPriceWETH =  newPrice;
    }
    
     
    function updateMaticPrice(uint newPrice) public onlyOwner{
        claimPriceMatic =  newPrice;
    }
    
    function updateReferralFee(uint newReferralFee) public onlyOwner{
        referralFee =  newReferralFee;
    }
    
    function updateReferralDiscount(uint newReferralDiscount) public onlyOwner{
    ...
    function migrateOwnersFromPreviousContract(uint startIndex, uint endIndex) public onlyOwner{
        for (uint256 punkIndex = startIndex; punkIndex <= endIndex; punkIndex++) {
            address punkOwner = previousPunksMaticContract.punkIndexToAddress(punkIndex); 
                _safeMint(punkOwner, punkIndex);
                punksRemainingToAssign--;
        }
     }


    function mintFromWeth() payable public {
        require(weth.approve(address(this), claimPriceWETH));
        require(weth.transferFrom(msg.sender, address(this),claimPriceWETH));
        require(weth.transfer(address(owner()),claimPriceWETH));
        
        require(punksRemainingToAssign > 0, "No remaining punk");
    ...
    function mint() payable public {
        require(punksRemainingToAssign > 0, "No remaining punk");
        require(msg.value >= claimPriceMatic, "Need pay at least claim amount");
        uint256 randIndex = _random() % punksRemainingToAssign;
        uint256 punkIndex = _fillAssignOrder(--punksRemainingToAssign, randIndex);
        _safeMint(_msgSender(), punkIndex);
    ...
    function mintFromWethWithReferral(uint256 tokenId) payable public {
        uint priceWithDiscount = claimPriceWETH - (claimPriceWETH/referralDiscount);
        address refAdress = ownerOf(tokenId);
		uint referralPercentage = priceWithDiscount/referralFee;
        require(weth.approve(address(this), priceWithDiscount));
        require(weth.transferFrom(msg.sender, address(this),priceWithDiscount));
        require(weth.transfer(address(owner()),priceWithDiscount-referralPercentage));
        require(weth.transfer(address(refAdress),referralPercentage));
    ...
    function mintWithReferral(uint256 tokenId) payable public {
        uint priceWithDiscount = claimPriceMatic - (claimPriceMatic/referralDiscount);
        require(punksRemainingToAssign > 0, "No remaining punk");
        require(msg.value >= priceWithDiscount, "Need pay at least claim amount");
        address refAdress = ownerOf(tokenId);
        uint256 randIndex = _random() % punksRemainingToAssign;
        uint256 punkIndex = _fillAssignOrder(--punksRemainingToAssign, randIndex);
        _safeMint(_msgSender(), punkIndex);
		uint referralPercentage = msg.value/referralFee;
		uint mintFees = msg.value-referralPercentage;
        (bool success,) = owner().call{value: mintFees}("");
        require(success);
        
        (bool successRef,) = refAdress.call{value: referralPercentage}("");
        require(successRef);
        emit Assign(_msgSender(), punkIndex);
    }
