...
contract AssetParameters is IAssetParameters, OwnableUpgradeable, AbstractDependant {
    ...
    EnumerableSetUpgradeable.Bytes32Set private _supportedAssets;
    ...
    function getSupportedAssets() external view override returns (bytes32[] memory _resultArr) {
    ...
    function getLiquidityPools(bytes32[] calldata _keysArr)
        external
        view
        override
        returns (address[] memory _resultArr)
    {
        uint256 _keysCount = _keysArr.length;

        _resultArr = new address[](_keysCount);

        for (uint256 i = 0; i < _keysCount; i++) {
            _resultArr[i] = liquidityPools[_keysArr[i]];
        }
    }
    ...
    function withdrawAllReservedFunds(
        ...
        for (uint256 i = _offset; i < _to; i++) {
            ILiquidityPool(liquidityPools[_supportedAssets.at(i)]).withdrawReservedFunds(
                _recipientAddr,
                0,
                true
            );
...
contract LiquidityPoolFactory is AbstractDependant {
    ...
    address private assetParametersAddr;

    function setDependencies(Registry _registry) external override onlyInjectorOrZero {
        registry = _registry;
        liquidityPoolAdmin = ILiquidityPoolAdmin(_registry.getLiquidityPoolAdminContract());
        assetParametersAddr = registry.getAssetParametersContract();
    }

    modifier onlyAssetParameters() {
        require(
            assetParametersAddr == msg.sender,
