...
abstract contract StrategyFarmTwoAssets is BaseStrategyMasterChef {
    ...
    address public constant kogecoin = 0x13748d548D95D78a3c83fe3F32604B4796CFfa23;
    ...
    // Uniswap swap paths
    address[] public quick_koge_path;
    ...
    {
        quick_koge_path = new address[](3);
        quick_koge_path[0] = quick;
        quick_koge_path[1] = matic;
        quick_koge_path[2] = kogecoin;

        quick_matic_path = new address[](2);
        quick_matic_path[0] = quick;
        quick_matic_path[1] = matic;
    }
    ...
    function harvest() public override {
        ...
        if (_quickBalance > harvestCutoff){
            _swapUniswapWithPath(quick_koge_path, _quickBalance.div(2));
            uint256 _quickBalancePost = IERC20(quick).balanceOf(address(this));
            _swapUniswapWithPath(quick_matic_path, _quickBalancePost);

            // Add liquidity
            uint256 _kogecoin = IERC20(kogecoin).balanceOf(address(this));
            uint256 _matic = IERC20(matic).balanceOf(address(this));
            if (_kogecoin > 0 && _matic > 0) {
                IERC20(kogecoin).safeApprove(currentRouter, 0);
                IERC20(kogecoin).safeApprove(currentRouter, _kogecoin);
...
contract StrategyTwoAssets is StrategyFarmTwoAssets {
    // Token addresses
    address public Quick_Rewards = 0xDEb9d9D209687EC73488A89b68fE84B2364a8d86;
    address public KOGE_MATIC_LP = 0x3885503aEF5E929fCB7035FBDcA87239651C8154;
