...
contract RobeVesting is Ownable, ReentrancyGuard {
    ...
    uint256 public constant oneYear = 365 * oneDay;

    uint256 public constant oneMinute = 1 minutes;
    // uint256 public startClaimTimeInSeconds = 5 * oneMinute;
    // uint256 public vestingDurationInSeconds = 15 * oneMinute;

    // uint256 public emissionRatePerSecond = ((10**18) * 1);
    // uint256 public totalEmissionRate = ((10**18) * 900);
    uint256 public startClaimTimeInSeconds;
    ...
    constructor(
        address nftToken_,
        address token_,
        uint256 totalEmissionRate_,
        uint256 emissionRatePerSecond_,
        uint256 vestingDurationInSeconds_,
        uint256 startClaimTimeInSeconds_
    ) {
        ...
        totalEmissionRate = totalEmissionRate_ * (10**18);
        emissionRatePerSecond = emissionRatePerSecond_;

        vestingDurationInSeconds = vestingDurationInSeconds_;
        startClaimTimeInSeconds = startClaimTimeInSeconds_;
    }
    ...
    function setVestingScheduleEndDate(address _beneficiary) public nonReentrant {
        require(
            getVestingSchedulesCountByBeneficiary(_beneficiary) != 0,
            "Vesting schedule not created for the provided address"
        );
    ...
    function createVestingSchedule(
        ...
        // require(getCurrentTime() >= startDateVesting, "Vesting hasn't started yet");
        require(_nftToken.balanceOf(_beneficiary) > 0, "You should have ROBe NFT");

        _nftVesting[_tokenId].totalVestableAmount = totalEmissionRate;

        if (!_nftVesting[_tokenId].initialized) {
            require(
                this.getWithdrawableAmount() >= _nftVesting[_tokenId].totalVestableAmount,
                "TokenVesting: cannot create vesting schedule because not sufficient tokens"
            );
            vestingSchedulesTotalAmount = vestingSchedulesTotalAmount.add(_nftVesting[_tokenId].totalVestableAmount);
            _nftVesting[_tokenId].initialized = true;
            _nftVesting[_tokenId].availableForVesting = totalEmissionRate;
        }
        ...
        uint256 end = 0;
        uint256 duration = vestingDurationInSeconds;
        uint256 startClaimTime = start.add(startClaimTimeInSeconds);
        bool revocable = true;
        uint256 released = 0;
        bool revoked = false;
    ...
    function _computeClaimableAmount(VestingSchedule memory vestingSchedule) internal view returns (uint256) {
        ...
        if (vestingSchedule.revoked == true || currentTime < vestingSchedule.startClaimTime) {
            return 0;
        } else if (currentTime >= vestingSchedule.start.add(vestingSchedule.duration).add(startClaimTimeInSeconds)) {
            return vestingSchedule.amountTotal.sub(vestingSchedule.released);
        } else {
            uint256 timeFromStartClaim = currentTime.sub(vestingSchedule.startClaimTime);
            uint256 claimableAmount = timeFromStartClaim.mul(emissionRatePerSecond);
    ...
    function _computeVestableAmount(VestingSchedule memory vestingSchedule) internal view returns (uint256) {
        ...
        if (vestingSchedule.revoked == true || currentTime < vestingSchedule.start) {
            ...
            uint256 vestedAmount = timeFromStart.mul(emissionRatePerSecond);
