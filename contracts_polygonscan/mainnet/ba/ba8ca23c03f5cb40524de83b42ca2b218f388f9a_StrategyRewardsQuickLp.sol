...
*/
contract StrategyRewardsQuickLp is StrategyBase {
    ...
    function _withdrawSome(uint256 _amount) internal override returns (uint256) {
        uint256 _stakedAmount = IQuickStakingRewards(farmPool).balanceOf(address(this));
    ...
    function balanceOfPool() public view override returns (uint256) {
        uint256 amount = IQuickStakingRewards(farmPool).balanceOf(address(this));
        return amount;
    }

    function claimable_tokens() external view override returns (address[] memory farmToken, uint256[] memory totalDistributedValue) {
        (farmToken, totalDistributedValue) = IQuickStakingRewards(farmPool).bothTokensEarned(address(this));
    }

    function claimable_token() external view override returns (address farmToken, uint256 totalDistributedValue) {
        (address[] memory farmTokens, uint256[] memory totalDistributedValues) = IQuickStakingRewards(farmPool).bothTokensEarned(address(this));
        farmToken = farmTokens[0];
        totalDistributedValue = totalDistributedValues[0];
    }
