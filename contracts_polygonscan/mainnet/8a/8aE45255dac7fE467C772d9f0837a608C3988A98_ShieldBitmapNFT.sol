// SPDX-FileCopyrightText: 2021 Nawoo (@NowAndNawoo)
//
...
import "./Base64.sol";
import "./Bitmap.sol"; // https://github.com/doublejumptokyo/solidity-bitmap

library MathInt {
	function sqrt(int256 x) internal pure returns (int256 y) {
		// https://ethereum.stackexchange.com/a/2913/82443
		int256 z = (x + 1) / 2;
		y = x;
		while (z < y) {
			y = z;
...
contract ShieldBitmapNFT is ERC721Enumerable, Ownable {
	using Strings for uint256;
	using BitmapLib for Bitmap;

	uint256 public constant VERSION = 1;
	uint256 public constant MAX_COUNT = 256;
	uint256 public constant WIDTH = 48;
	uint256 public constant HEIGHT = 48;

	uint256 public nextTokenId = 0;

	constructor() ERC721("ShieldBitmapNFT", "SHIELD") {}
	...
	function mint() external {
		require(nextTokenId < MAX_COUNT, "All tokens minted");
...
library BitmapLib {
	...
	function init(Bitmap memory bitmap, XY memory size) internal pure {
		uint256 linePadding = (uint256(size.x) * pixelSize) % 4 == 0 ? 0 : 4 - ((uint256(size.x) * pixelSize) % 4);
	...
	function setPixel(
		...
		uint256 index = headerSize + uint256(position.y) * bitmap.lineSize + uint256(position.x) * pixelSize;
	...
	function setBody(Bitmap memory bitmap, bytes memory body) internal pure {
		...
		require(bitmap.data.length == headerSize + bodyLength, "invalid body size");
