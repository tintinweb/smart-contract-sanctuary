...
contract CryptoPair is Initializable, Ownable, ReentrancyGuard {
    using CryptoLib for CryptoLib.SwapStorage;
    CryptoLib.SwapStorage public swapStorage;
    ...
    function initialize(
        ...
        address [N_COINS] memory coins,
        uint256 [N_COINS] memory precisions
    ) external onlyOwner initializer {

        swapStorage.crypto.price_scale = initial_price;
        swapStorage.crypto.price_oracle = initial_price;
        swapStorage.crypto.last_prices = initial_price;
        swapStorage.crypto.lp_token = LPToken(lp_token);
        swapStorage.crypto.coins = coins;
        swapStorage.crypto.precisions = precisions;

        swapStorage.fees.admin_fee_receiver = IFactory(factory).feeTo();
        swapStorage.initDefaultParameters();
        operator = _operator;
    }
    ...
    function get_lp_token() external view returns(address) {
        return address(swapStorage.crypto.lp_token);
    }

    function A() external view returns(uint256) {
        return swapStorage.get_A();
    }

    function gamma() external view returns(uint256) {
        return swapStorage.get_gamma();
    }

    function get_virtual_price() external view returns(uint256) {
        return swapStorage.get_virtual_price();
    }

    function exchange(uint256 i, uint256 j, uint256 dx, uint256 min_dy)
    external nonReentrant {
        require(!is_killed, 'killed');
        swapStorage.exchange(i, j, dx, min_dy);
    }

    function get_dy(uint256 i, uint256 j, uint256 dx) external view returns(uint256) {
        return swapStorage.get_dy(i, j, dx);
    }

    function calc_withdraw_one_coin(uint256 token_amount, uint256 i) external view returns(uint256) {
        return swapStorage.calc_withdraw_one_coin(token_amount, i);
    }

    function remove_liquidity_one_coin(uint256 token_amount, uint256 i, uint256 min_amount)
    external nonReentrant returns(uint256){
        require(!is_killed);
        return swapStorage.remove_liquidity_one_coin(token_amount, i, min_amount);
    }

    function claim_admin_fees() nonReentrant external {
        swapStorage.claim_admin_fees();
    }

    function add_liquidity(uint256[N_COINS] calldata amounts, uint256 min_mint_amount)
    external nonReentrant {
        require(!is_killed);
        swapStorage.add_liquidity(amounts, min_mint_amount);
    }

    function remove_liquidity(uint256 amount, uint256[N_COINS] memory minAmounts)
    external nonReentrant {
        swapStorage.remove_liquidity(amount, minAmounts);
    }

    function calc_token_amount(uint256[N_COINS] memory amounts) external view returns(uint256) {
        return swapStorage.calc_token_amount(amounts);
    }

    /* ========== RESTRICTED FUNCTIONS ========== */

    function ramp_A_gamma(uint256 future_A, uint256 future_gamma, uint256 future_time) onlyOperator external {
        require(block.timestamp > swapStorage.initial_A_gamma_time + (MIN_RAMP_TIME-1));
        require(future_time > block.timestamp + (MIN_RAMP_TIME-1));

        uint256[2] memory A_gamma = swapStorage._A_gamma();
        uint256 initial_A_gamma = swapStorage.shift(A_gamma[0], 128);
        initial_A_gamma = initial_A_gamma | A_gamma[1];

        require(future_A > MIN_A - 1 && future_A < MAX_A + 1);
        require(future_gamma > MIN_GAMMA - 1 && future_gamma < MAX_GAMMA + 1);

        uint256 ratio = 10**18 * future_A / A_gamma[0];
        require(ratio < 10**18 * MAX_A_CHANGE + 1 && ratio > 10**18 / MAX_A_CHANGE - 1);

        swapStorage.initial_A_gamma = initial_A_gamma;
        swapStorage.initial_A_gamma_time = block.timestamp;

        uint256 future_A_gamma = swapStorage.shift(future_A, 128);
        future_A_gamma = future_A_gamma | future_gamma;
        swapStorage.future.future_A_gamma_time = future_time;
        swapStorage.future.future_A_gamma = future_A_gamma;

        emit RampAgamma(A_gamma[0], future_A, A_gamma[1], future_gamma, block.timestamp, future_time);
    }

    function stop_ramp_A_gamma() external onlyOperator{
        uint256[2] memory A_gamma = swapStorage._A_gamma();
        uint256 current_A_gamma = swapStorage.shift(A_gamma[0], 128);
        current_A_gamma = current_A_gamma | A_gamma[1];

        swapStorage.initial_A_gamma = current_A_gamma;
        swapStorage.future.future_A_gamma = current_A_gamma;
        swapStorage.initial_A_gamma_time = block.timestamp;
        swapStorage.future.future_A_gamma_time = block.timestamp;
    ...
    function commit_new_parameters(
        ...
        uint256 new_fee_gamma,
        uint256 new_allowed_extra_profit,
        uint256 new_adjustment_step,
        uint256 new_ma_half_time
    ) external onlyOperator {
        require(admin_actions_deadline == 0, 'active action');

        if (new_out_fee < MAX_FEE + 1) {
            require(new_out_fee > MIN_FEE - 1, 'fee is out of range');
        } else {
            new_out_fee = swapStorage.fees.out_fee;
        }
        if (new_mid_fee > MAX_FEE) {
            new_mid_fee = swapStorage.fees.mid_fee;
        }
        require(new_mid_fee <= new_out_fee, 'mid-fee is too high');

        if (new_admin_fee > MAX_ADMIN_FEE) {
            new_admin_fee = swapStorage.fees.admin_fee;
        }

        if (new_fee_gamma < 10**18) {
            require (new_fee_gamma > 0, 'fee_gamma out of range [1 .. 10**18]');
        } else {
            new_fee_gamma = swapStorage.fees.fee_gamma;
        }
        if (new_allowed_extra_profit > 10**18) {
            new_allowed_extra_profit = swapStorage.allowed_extra_profit;
        }
        if (new_adjustment_step> 10**18) {
            new_adjustment_step = swapStorage.adjustment_step;
        }

        if (new_ma_half_time < 7*86400) {
            require (new_ma_half_time > 0, 'MA time should be longer than 1 second');
        } else {
            new_ma_half_time = swapStorage.ma_half_time;
        }

        uint256 deadline = block.timestamp + ADMIN_ACTIONS_DELAY;
        admin_actions_deadline = deadline;
        swapStorage.future.future_admin_fee = new_admin_fee;
        swapStorage.future.future_mid_fee = new_mid_fee;
        swapStorage.future.future_out_fee = new_out_fee;
        swapStorage.future.future_fee_gamma = new_fee_gamma;
        swapStorage.future.future_allowed_extra_profit = new_allowed_extra_profit;
        swapStorage.future.future_adjustment_step = new_adjustment_step;
        swapStorage.future.future_ma_half_time = new_ma_half_time;

        emit CommitNewParameters(deadline, new_admin_fee, new_mid_fee, new_out_fee,
            new_fee_gamma,
            new_allowed_extra_profit, new_adjustment_step,
            new_ma_half_time);
    }

    function apply_new_parameters() external onlyOperator {
        require(block.timestamp >= admin_actions_deadline && admin_actions_deadline != 0);
        admin_actions_deadline = 0;
        uint256 admin_fee = swapStorage.future.future_admin_fee;

        if (admin_fee != swapStorage.fees.admin_fee) {
            swapStorage.claim_admin_fees();
            swapStorage.fees.admin_fee = admin_fee;
        }
        swapStorage.fees.mid_fee = swapStorage.future.future_mid_fee;
        swapStorage.fees.out_fee = swapStorage.future.future_out_fee;
        swapStorage.fees.fee_gamma = swapStorage.future.future_fee_gamma;
        swapStorage.allowed_extra_profit = swapStorage.future.future_allowed_extra_profit;
        swapStorage.adjustment_step = swapStorage.future.future_adjustment_step;
        swapStorage.ma_half_time = swapStorage.future.future_ma_half_time;

        emit NewParameters(swapStorage.fees.admin_fee, swapStorage.fees.mid_fee, swapStorage.fees.out_fee,
            swapStorage.fees.fee_gamma,
            swapStorage.allowed_extra_profit, swapStorage.adjustment_step,
            swapStorage.ma_half_time);

    }

    function revert_new_parameters() external onlyOperator {
        admin_actions_deadline = 0;
    }

    function kill_me() external onlyOperator {
        require(kill_deadline > block.timestamp);
        is_killed = true;
    }

    function unkill_me() external onlyOperator {
        is_killed = false;
    }

    function change_operator(address new_operator) external onlyOperator {
        operator = new_operator;
    }

    /* =============== EVENTS ==================== */
    event CommitNewParameters(uint256 indexed deadline, uint256 admin_fee, uint256 mid_fee, uint256 out_fee,
        uint256 fee_gamma, uint256 allowed_extra_profit, uint256 adjustment_step, uint256 ma_half_time);
    event NewParameters(uint256 admin_fee, uint256 mid_fee, uint256 out_fee, uint256 fee_gamma,
        uint256 allowed_extra_profit, uint256 adjustment_step, uint256 ma_half_time);
    event RampAgamma(uint256 A, uint256 future_A, uint256 gamma, uint256 future_gamme, uint256 timestamp, uint256 future_time);
