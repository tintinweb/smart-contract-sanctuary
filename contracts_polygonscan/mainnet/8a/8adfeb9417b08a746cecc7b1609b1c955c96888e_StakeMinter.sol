...
contract StakeMinter is IStakeMinter, Migrateable, Manageable, ExternallyCallable {
    struct Contracts {
        IToken token;
    ...
    */
    function stake(uint256 amount, uint256 stakingDays) external {
        require(stakingDays != 0, 'Staking: Staking days < 1'); // 1k
        require(stakingDays <= 5555, 'Staking: Staking days > 5555'); //1k

        //on stake axion gets burned
        contracts.token.burn(msg.sender, amount); // 21k gas

        //call stake internal method
        stakeInternal(amount, stakingDays, msg.sender); // to call internal function 0 gas
    }

    /** @dev Recovery Stake
        @param recovery {Recovery} - AXN amount to be staked
    */
    function recoverStake(Recovery[] memory recovery) external onlyManager {
        for (uint256 i = 0; i < recovery.length; i++) {
            Recovery memory recover = recovery[i];
            require(recover.stakingDays != 0, 'Staking: Staking days < 1'); // 1k
            require(recover.stakingDays <= 5555, 'Staking: Staking days > 5555'); //1k

            contracts.stakeCustodian.addStake(
                recover.staker,
                contracts.stakeManager.recoverStake(
                    recover.staker,
                    recover.start,
                    recover.shares,
                    recover.amount,
    ...
    */
    function stakeInternal(
        ...
        require(
            contracts.stakeCustodian.addStake(
                staker,
                contracts.stakeManager.createStake(staker, amount, stakingDays)
            ) == true,
            'STAKEMINTER: Failed to add stake'
        );
    }

    /** @dev
        Description: Converts a stakecustodian stake into a stakeToken stake
        @param stakeId {uint256}
     */
    function convertToNft(uint256 stakeId) external {
        require(
            contracts.stakeCustodian.removeStake(msg.sender, stakeId),
            'STAKE MINTER: Not owner of stake or already converted.'
        );

        contracts.stakeToken.mint(msg.sender, stakeId); // 120k
    }
