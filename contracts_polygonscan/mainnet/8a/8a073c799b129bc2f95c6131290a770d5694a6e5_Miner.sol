...
// Miner governs the transaction mining of HAIR rewards and distribution of referral fees.
/*
Miner contract is responsible for the following tasks:
- Collect HAIR yield from MasterChef using a dummy ERC20 token.
...
contract Miner is Ownable, ReentrancyGuard {
    ...
    // HAIR token contract.
    IERC20 constant HAIR_TOKEN = IERC20(0x100A947f51fA3F1dcdF97f3aE507A72603cAE63C);

    // MasterChef contract is the yield farm for HAIR token.
    IMasterChef constant masterChef = IMasterChef(0xC6Ae34172bB4fC40c49C3f53badEbcE3Bb8E6430);

    // hairGame contract address. Distribute some HAIR emissions to HAIR game contract.
    address public hairGame;

    // Percentage of HAIR harvested to go to HAIR game contract.
    uint public hairGamePct = 10;
    ...
    // Total pending HAIR rewards by all users.
    uint public allPendingReward;

    // Map user address to pending HAIR rewards.
    ...
    // Stake dummy token in MasterChef pool to earn HAIR.
    ...
    // Harvest HAIR token from MasterChef pool.
    function harvestMasterChef() internal {

        // Store initial balance.
        uint initialBalance = HAIR_TOKEN.balanceOf(address(this));

        // Harvest HAIR from masterChef.
        masterChef.withdraw(pid, 0);

        // Compute amount of HAIR harvested.
        uint harvestAmount = HAIR_TOKEN.balanceOf(address(this)) - initialBalance;

        // Allocate some HAIR to HAIR game contract.
        safeHairTransfer(hairGame, harvestAmount.mul(hairGamePct).div(100));
    }
    ...
    // Set hairGame contract address.
    function setHairGame(address _hairGame) external onlyOwner {
        hairGame = _hairGame;
    }

    // Set percentage of HAIR harvested to go to HAIR game contract.
    function setHairGamePct(uint _hairGamePct) external onlyOwner {
        hairGamePct = _hairGamePct;
    }
    ...
    // Allow hair game contract to add pending HAIR rewards for user.
    ...
    // Withdraw pending HAIR rewards.
    function withdraw() public nonReentrant {
        uint pending = userReward[msg.sender];
        harvestMasterChef();
        require(pending <= HAIR_TOKEN.balanceOf(address(this)), "Withdrawal exceeds balance. Wait for Miner to farm more HAIR.");
        if (pending > 0) {
    ...
    // Safe HAIR transfer function, in case if rounding error causes pool to not have enough HAIR.
    function safeHairTransfer(address _to, uint _pending) internal {
        uint hairBal = HAIR_TOKEN.balanceOf(address(this));
        if (_pending > hairBal) {
            HAIR_TOKEN.transfer(_to, hairBal);
        } else {
            HAIR_TOKEN.transfer(_to, _pending);
        }
    }
