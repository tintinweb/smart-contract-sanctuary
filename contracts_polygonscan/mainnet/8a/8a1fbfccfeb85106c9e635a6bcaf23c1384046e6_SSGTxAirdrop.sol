...
contract SSGTxAirdrop is Ownable, ReentrancyGuard{
    using SafeMath for uint256;
    
    address public immutable SSGT_TOKEN_ADDRESS = 0xd0cfd20E8bBDb7621B705a4FD61de2E80C2cD02F;// SSGT MATIC Contract Address
    address public immutable SSGT_STAKING_ADDRESS = 0x6087a0e583dBBF97e73f5740C234dA0CdD967Cb5;
    uint256 public timePeriod = 270 days;// After 9 months DAO can transfer the leftovers to SSGT Staking Reward.
    ...
    function createAirdropSchedules(
        ...
        require(_beneficiaries.length > 0, "SSGTxAirdrop::createAirdropSchedules: Empty Data");
        require(
            _beneficiaries.length == _amounts.length,
            "SSGTxAirdrop::createAirdropSchedules: Array lengths do not match"
        );
    ...
    function transferAmountToStakingContract(uint _amount) external onlyOwner{
        require(block.timestamp > startTime.add(timePeriod), "SSGTAirdrop::Can not transfer tokens");
        require(IERC20(SSGT_TOKEN_ADDRESS).transfer(SSGT_STAKING_ADDRESS, _amount), "SSGTAirdrop:: Unable to transfer tokens");
    }
    
    // Internal

    function _createAirdropSchedule(address _beneficiary, uint256 _amount) internal returns (bool) {
        require(_beneficiary != address(0), "SSGTAirdrop::createAirdropSchedule: Beneficiary cannot be empty");
        require(_amount > 0, "SSGTAirdrop::createAirdropSchedule: Amount cannot be empty");
        ...
        require(
            IERC20(SSGT_TOKEN_ADDRESS).transferFrom(msg.sender, address(this), _amount),
            "SSGTxAirdrop::createAirdropSchedule: Unable to escrow tokens"
        );
    ...
    function _drawDown(address _beneficiary) internal returns (bool) {
        require(airdropAmount[_beneficiary] > 0, "SSGTAirdrop::_drawDown: There is no schedule currently in flight");

        // Increase total drawn amount
        totalDrawn[_beneficiary] = totalDrawn[_beneficiary].add(airdropAmount[_beneficiary]);

        // Issue tokens to beneficiary
        require(IERC20(SSGT_TOKEN_ADDRESS).transfer(_beneficiary, airdropAmount[_beneficiary]), "SSGTxAirdrop::_drawDown: Unable to transfer tokens");
    ...
    function _transferBalanceToOwner(uint _amount) public onlyOwner{
        require(IERC20(SSGT_TOKEN_ADDRESS).transfer(owner, _amount), "SSGTAirdrop:: Unable to transfer tokens");
    }
