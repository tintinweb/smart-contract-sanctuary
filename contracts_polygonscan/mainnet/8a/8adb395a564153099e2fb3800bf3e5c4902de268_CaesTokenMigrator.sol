...
// File: contracts/migration/CaesTokenMigrator.sol
...
interface IUniswapV2Router {
    function addLiquidity(
        ...
        uint256 lpOldCaesAmountDesired,
        uint256 lpMaiAmountMin,
        uint256 lpOldCaesAmountMin,
        address to,
        uint256 deadline
    )
        external
    ...
    function removeLiquidity(
        ...
        address to,
        uint256 deadline
    ) external returns (uint256 lpMaiAmount, uint256 lpOldCaesAmount);
}
...
contract CaesTokenMigrator is Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    /* ========== STATE VARIABLES ========== */

    IERC20 public immutable oldCAES;
    IERC20 public immutable newCAES;
    IERC20 public immutable mai;
    ICaesiumTreasury public immutable oldTreasury;
    ICaesiumTreasury public immutable newTreasury;
    ...
    constructor(
        ...
        require(_oldCAES != address(0));
        oldCAES = IERC20(_oldCAES);
        require(_oldTreasury != address(0));
        oldTreasury = ICaesiumTreasury(_oldTreasury);
        ...
        require(_newCAES != address(0));
        newCAES = IERC20(_newCAES);
        require(_newTreasury != address(0));
        newTreasury = ICaesiumTreasury(_newTreasury);
    ...
    function migrate() external {
        require(caesMigrated);
        uint256 oldCAESAmount = oldCAES.balanceOf(msg.sender);

        require(oldCAESAmount > 0);
        oldCAES.transferFrom(msg.sender, address(this), oldCAESAmount);

        uint256 maiAmount = oldCAESAmount.mul(1e9);
        uint256 newCAESAmountInLP = oldCAESAmount.div(convertRatio);

        oldCAES.safeApprove(address(oldTreasury), oldCAESAmount);
        ...
        newCAES.transfer(msg.sender, newCAESAmountInLP);
    }
    ...
    function migrateContracts() external onlyOwner {
        ...
        oldSupply = oldCAES.totalSupply(); // log total supply at time of migration

        uint256 newCAESTotalSupply = oldSupply.div(convertRatio);

        // withdraw old LP
        address oldPair = quickFactory.getPair(address(oldCAES), address(mai));
        ...
        (uint256 lpMaiAmount, uint256 lpOldCaesAmount) = quickRouter
            .removeLiquidity(
                address(mai),
                address(oldCAES),
                oldLPAmount,
        ...
        // burn old caess
        oldCAES.safeApprove(address(oldTreasury), lpOldCaesAmount);
        uint256 extraMaiAmount = lpOldCaesAmount * 1e9;
        oldTreasury.withdraw(extraMaiAmount, address(mai));

        // deposit mai from burned caess to the new treasury
        ...
        // mint new CAESs from new treasury
        uint256 newCAESAmountInLP = lpOldCaesAmount.div(convertRatio);
        newTreasury.mintRewards(address(this), newCAESAmountInLP);

        mai.safeApprove(address(quickRouter), lpMaiAmount);
        newCAES.safeApprove(address(quickRouter), newCAESAmountInLP);
        ...
        // Mint new CAES to migrator for migration
        mai.safeApprove(address(newTreasury), excessReserves);
        uint256 newCAESMinted = newCAESTotalSupply.sub(newCAESAmountInLP).sub(1);
        uint256 profit = valueOfMai.sub(newCAESMinted);
    ...
    function withdrawToken(
        ...
        require(tokenAddress != address(oldCAES), 'Cannot withdraw: old-CAES');
