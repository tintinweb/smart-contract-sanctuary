...
contract Sky is Ownable, ISky {
   ...
   function totalMatic() public view returns(uint256){
       uint256 total = address(this).balance;
       for(uint256 i = 0; i < cloudsIndex.length; i++){
           total = total.add(address(cloudsIndex[i]).balance);
       }
       return total;
   }
}
...
contract Cloud is Roller {
    ...
    function onWin(uint256) internal override {
        if(alwaysPicked.length > 6){
            alwaysPicked.pop();
        }
    }

    function onLoss(uint256 outcome) internal override {
        if(alwaysPicked.length < 10){
            alwaysPicked.push(polyroll.bets(rolls[rolls.length-1]).outcome % 37);
        }else{
            alwaysPicked[lastAlwaysPickedReplaced % (alwaysPicked.length - 6) + 6] = outcome % 37;
            lastAlwaysPickedReplaced = lastAlwaysPickedReplaced.add(1);
        }
...
contract Charge is Roller {
    ...
    function betAverage(uint256 random) internal view returns(uint256){
        ...
        uint256 average = 0;
        for(uint256 i = outcomes.length - 5; i < outcomes.length; i++){
            average = average.add(outcomes[i]);
        }
    ...
    function onWin(uint256 outcome) internal override{
    ...
    function onLoss(uint256 outcome) internal override{
        ...
        outcomes.push(outcome);
        if(startBalance > 40 ether){
            startBalance = startBalance.sub(2 ether);
        }
    }
...
interface PolyrollMiner {
     ...
     function userReward(address) external returns(uint256);
}
