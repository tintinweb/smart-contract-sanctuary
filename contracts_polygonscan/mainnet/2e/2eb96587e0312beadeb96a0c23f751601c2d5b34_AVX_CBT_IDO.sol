...
interface STAKE {
    ...
    function tierCount5() external view returns (uint);
...
contract StakingDetails {
    ...
    constructor() {
        stakeGetter = STAKE(0x1a05D2ba7C4253bA5B718173401c73dBC20Bf104); // live stake contract address
    ...
    function fiveCount() public view returns (uint) {
       return stakeGetter.tierCount5();
    }
...
contract AVX_CBT_IDO {
    ...
    uint public tier5Number = stakeDetails.fiveCount();
    
    uint public totalAllocation = 975000 * 10**18;
    uint public tier1Alloc = 97500 * 10**18;
    uint public tier2Alloc = 146250 * 10**18;
    uint public tier3Alloc = 195000 * 10**18;
    uint public tier4Alloc = 243750 * 10**18;
    uint public tier5Alloc = 292500 * 10**18;
    ...
    function buyToken() public payable returns(bool) {
        ...
        if(userTier == 0){
            ...
            if(userAlloc[sender] == 0){
                userAlloc[sender] = tier5Alloc / tier5Number;
                require(tokens <= userAlloc[sender], "User allocation error9");
                userAlloc[sender] -= tokens;
            }
            else{
                require(tokens <= userAlloc[sender], "User Allocation Error10");
                userAlloc[sender] -= tokens;            
    ...
    function setTierMemberCount() public {
        ...
        tier5Number = stakeDetails.fiveCount();
    }
   ...
   // add same claim to multiple addresses
    ...
    function getTier5Allocation() public view returns(uint) {
        return tier5Alloc;
    }
    ...
    function tier5Count() public view returns (uint){
        return tier5Number;
    }
