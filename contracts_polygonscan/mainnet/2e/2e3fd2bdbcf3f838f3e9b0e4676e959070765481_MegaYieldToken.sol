...
// 
// MegaYieldToken with Governance.
contract MegaYieldToken is BEP20 {
    ...
    // Min amount to liquify. (default 500 MegaYields)
    uint256 public minAmountToLiquify = 500 ether;
    // The swap router, modifiable. Will be changed to MegaYieldSwap's router when our own AMM release
    IUniswapV2Router02 public MegaYieldSwapRouter;
    // The trading pair
    address public MegaYieldSwapPair;
    // In swap and liquify
    ...
    event MegaYieldSwapRouterUpdated(address indexed operator, address indexed router, address indexed pair);
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (!_excludedFromAntiWhale[sender] && !_excludedFromAntiWhale[recipient]) {
                require(amount <= maxTransferAmount(), "MegaYield::antiWhale: Transfer amount exceeds the maxTransferAmount");
            }
        }
        _;
    }
    ...
    /**
     * @notice Constructs the MegaYieldToken contract.
     */
    constructor() public BEP20("MegaYields", "GMS") {
    ...
    /// @dev overrides transfer function to meet tokenomics of MegaYield
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        // swap and liquify
        if (
            swapAndLiquifyEnabled == true
            && _inSwapAndLiquify == false
            && address(MegaYieldSwapRouter) != address(0)
            && MegaYieldSwapPair != address(0)
            && sender != MegaYieldSwapPair
            && sender != owner()
        ) {
            swapAndLiquify();
        }
        ...
        if (recipient == address(0)) {
            ...
            require(taxAmount == burnAmount + liquidityAmount, "MegaYield::transfer: Burn value invalid");

            // default 95% of transfer sent to recipient
            uint256 sendAmount = amount.sub(taxAmount);
            require(amount == sendAmount + taxAmount, "MegaYield::transfer: Tax value invalid");
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the MegaYieldSwap pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = MegaYieldSwapRouter.WETH();

        _approve(address(this), address(MegaYieldSwapRouter), tokenAmount);

        // make the swap
        MegaYieldSwapRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        // add the liquidity
        MegaYieldSwapRouter.addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount,
    ...
    // To receive BNB from MegaYieldSwapRouter when swapping
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "MegaYield::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateBurnRate(uint16 _burnRate) public onlyOperator {
        require(_burnRate <= 100, "MegaYield::updateBurnRate: Burn rate must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 10000, "MegaYield::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
     ...
     * Can only be called by the current operator.
     */
    function updateMegaYieldSwapRouter(address _router) public onlyOperator {
        MegaYieldSwapRouter = IUniswapV2Router02(_router);
        MegaYieldSwapPair = IUniswapV2Factory(MegaYieldSwapRouter.factory()).getPair(address(this), MegaYieldSwapRouter.WETH());
        require(MegaYieldSwapPair != address(0), "MegaYield::updateMegaYieldSwapRouter: Invalid pair address.");
        emit MegaYieldSwapRouterUpdated(msg.sender, address(MegaYieldSwapRouter), MegaYieldSwapPair);
    }
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "MegaYield::transferOperator: new operator is the zero address");
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "MegaYield::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "MegaYield::delegateBySig: invalid nonce");
        require(now <= expiry, "MegaYield::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "MegaYield::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying MegaYields (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "MegaYield::_writeCheckpoint: block number exceeds 32 bits");
...
contract Referral is Ownable {
    ...
    MegaYieldToken public MegaYields;
        ...
        returns (uint256)
    {
        ...
        for (uint256 i; i < levelRate.length; i++) {
            ...
            if (
                ...
                MegaYields.safeTransfer(accountAddress, c);
    ...
    /**
     * @dev Transfer MegaYields away from Referral contract if added by accident (not supposed to be called)
     * @param _amount Amount to transfer away
     */
    function transferFunds(uint256 _amount) external onlyOwner {
        MegaYields.safeTransfer(owner(), _amount);
    }
...
contract MasterChef is Referral, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 accMegaYieldsPerShare; // Accumulated GEMs per share, times 1e12. See below.
    ...
    // Fee address (non-MegaYield tokens)
    address public feeAddr;
    // MegaYield fee address.
    address public MegaYieldFeeAddr;
    
    uint256 public totalLockedUpRewards;

    uint256 public MegaYieldsPerBlock;
    // Bonus muliplier for early MegaYields makers.
    ...
    constructor(
        MegaYieldToken _MegaYields,
        address _MegaYieldFeeAddress,
        address _feeAddr,
        uint256 _MegaYieldsPerBlock,
        uint256 _startBlock,
        ...
        MegaYields = _MegaYields;
        MegaYieldFeeAddr = _MegaYieldFeeAddress;
        feeAddr = _feeAddr;
    ...
    // Update the given pool's MegaYieldS allocation point. Can only be called by the owner.
    ...
    function pendingMegaYields(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accMegaYieldsPerShare = accMegaYieldsPerShare.add(MegaYieldsReward.mul(1e12).div(lpSupply));
        }
        
        uint256 pending = user.amount.mul(accMegaYieldsPerShare).div(1e12).sub(user.rewardDebt);
        return pending.add(user.rewardLockedUp);

    }
    
    // View function to see if user can harvest MegaYields.
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 MegaYieldsReward = multiplier.mul(MegaYieldsPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        MegaYields.mint(MegaYieldFeeAddr, MegaYieldsReward.div(10));
        MegaYields.mint(address(this), MegaYieldsReward);
        pool.accMegaYieldsPerShare = pool.accMegaYieldsPerShare.add(MegaYieldsReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number; 
    }
    ...
    function deposit(uint256 _pid, uint256 _amount, address _referrerAddress) public nonReentrant {
        ...
        payOrLockupPendingMegaYields(_pid);
        ...
        user.rewardDebt = user.amount.mul(pool.accMegaYieldsPerShare).div(1e12);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
     ...
     function payOrLockupPendingMegaYields(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        uint256 pending = user.amount.mul(pool.accMegaYieldsPerShare).div(1e12).sub(user.rewardDebt);
        if (canHarvest(_pid, msg.sender)) {
            ...
            safeMegaYieldsTransfer(msg.sender, actualReward);
            ...
            if(isReferralsEnabled() && hasReferrer(msg.sender)) {
                ...
                safeMegaYieldsTransfer(MegaYieldFeeAddr, rest.add(fee));
                
            } else if(fee > 0 || referralCut > 0){
                // Transfer to fee address
                safeMegaYieldsTransfer(MegaYieldFeeAddr, referralCut.add(fee));
            }
        }
        else if (pending > 0) {
    ...
    // Safe MegaYields transfer function, just in case if rounding error causes pool to not have enough MegaYields.
    function safeMegaYieldsTransfer(address _to, uint256 _amount) internal {
        uint256 bal = MegaYields.balanceOf(address(this));
        if (_amount > bal) {
            MegaYields.transfer(_to, bal);
        } else {
            MegaYields.transfer(_to, _amount);
        } 
    }
    ...
    function setMegaYieldFeeAddress(address _MegaYieldFeeAddr) public {
        require(msg.sender == _MegaYieldFeeAddr, "setMegaYieldFeeAddress: FORBIDDEN");
        require(_MegaYieldFeeAddr != address(0), "setMegaYieldFeeAddress: ZERO");
        MegaYieldFeeAddr = _MegaYieldFeeAddr;
    }
    
    function updateEmissionRate(uint256 _MegaYieldsPerBlock, bool _updateAllPools) public onlyOwner {
        require(_MegaYieldsPerBlock < 15000000000000000000, "updateEmissionRate: TOO HIGH");
        MegaYieldsPerBlock = _MegaYieldsPerBlock;
        ...
        emit EmissionRateUpdated(msg.sender, MegaYieldsPerBlock, _MegaYieldsPerBlock);
    }
}