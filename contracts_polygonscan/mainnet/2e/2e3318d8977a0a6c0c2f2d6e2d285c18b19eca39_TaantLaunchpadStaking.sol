...
contract TaantLaunchpadStaking {
     ...
     */
    constructor() {
        tiers.push(Tier(365 days, 30, 10 minutes));
        tiers.push(Tier(365 days, 100, 15 minutes));
        tiers.push(Tier(365 days, 160, 20 minutes));
        tiers.push(Tier(365 days, 250, 25 minutes));
    ...
    function stake(uint amt) public returns(bool) {
        ...
        uint amount = user[sender].stakedAmount + amt + withdrawableTilNow(sender);
    ...
    function userEndTimeFunc(address uAddr) public view returns(uint) {
     ...
     function withdrawableTilNow(address addr) public view returns(uint amount) {
