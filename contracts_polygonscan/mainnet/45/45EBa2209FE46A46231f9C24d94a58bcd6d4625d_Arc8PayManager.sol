...
contract Arc8PayManager is ITokenRecipient, UsingUniversalForwarding, Pausable {
    ...
    event Transferred(address indexed user, uint256 amount, string transferId);
    ...
    function transfer(uint256 amount, string calldata transferId) public whenNotPaused {
        require(amount > 0, "Cannot transfer 0");
        transferToken.safeTransferFrom(_msgSender(), tokenRecipient, amount);

        emit Transferred(_msgSender(), amount, transferId);
    }
