...
abstract contract Whelps {
    mapping (uint256 => uint256) public tokenIdToIndexBreed;
...
contract WhelpsStaking is Ownable {
    ...
    function setContracts(address contractWhelpsNFT, address contract7thBreed, address contractProxy, uint256 proxyIndex) external onlyOwner {
        require(!contractsLocked, "Locked");
        _whelpsNFTContract = contractWhelpsNFT;
        _7thBreedContract = contract7thBreed;
        _whelpsProxyContract = contractProxy;
        _whelpsProxyIndex = proxyIndex;
    }
    ...
    bool public manualStakingEnabled = true;
    
    function adminManualStaking(address _owner, uint256 _timestamp, uint256[] calldata _tokenIds, uint256 _tokenId, bool _eggClaimed, bool _tokensClaimed) public onlyOwner {
        require(manualStakingEnabled, "Disabled");
        
        for (uint i = 0; i < _tokenIds.length; i++) {
            tokenIdHasBeenStaked[_tokenIds[i]] = true;
        }
        ...
        keyToStakingSet[lastStakingSetKey] = StakingSet({
            ...
            tokenIds: _tokenIds,
            tokenId: _tokenId,
            eggClaimed: _eggClaimed,
            tokensClaimed: _tokensClaimed
        });
        
        _mintIndex++;

        // emit event and update staking set key
        emit Stake(_owner, lastStakingSetKey);
        lastStakingSetKey++;
    }
    
    function setMintIndex(uint256 index) public onlyOwner {
        require(manualStakingEnabled, "Disabled");
        _mintIndex = index;
    }
    
    struct MintStakingSet {
        address owner;
        uint256 timestamp;
        uint256 tokenId;
        bool eggClaimed;
    ...
    function adminManualStakingArray(uint256[][] calldata _tokenIds, MintStakingSet[] calldata sets) external onlyOwner {
        require(manualStakingEnabled, "Disabled");
        require(_tokenIds.length == sets.length, "Bad length");
        
        for (uint j = 0; j < sets.length; j++) {
          adminManualStaking(sets[j].owner, sets[j].timestamp, _tokenIds[j], sets[j].tokenId, sets[j].eggClaimed, sets[j].tokensClaimed);
        }
    }
    
    function disableManualStaking() public onlyOwner {
        manualStakingEnabled = false;
    }
    ...
    function stake(uint256[] calldata tokenIds) external {
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            ...
            uint256 index_breed = w.tokenIdToIndexBreed(tokenIds[i]);
