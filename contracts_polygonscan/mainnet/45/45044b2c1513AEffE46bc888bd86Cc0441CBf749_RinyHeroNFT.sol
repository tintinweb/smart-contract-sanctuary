...
contract RinyHeroNFT is BNFT, ITinyHeroNFT, ReentrancyGuard {
    using Address for address;
    ...
    constructor() BNFT(_msgSender(), "https://stg-api.tinyhero.com/v1/nft/", "rinyhero.io NFT", "RinyHero") {
    }
    ...
    function getStats(uint256 tokenId) external override view returns (uint16[] memory pvpStats) {
        if (_tinyStats != address(0)) {
            (pvpStats) = ITinyHeroStats(_tinyStats).stats(tokenId);
        }
    }
