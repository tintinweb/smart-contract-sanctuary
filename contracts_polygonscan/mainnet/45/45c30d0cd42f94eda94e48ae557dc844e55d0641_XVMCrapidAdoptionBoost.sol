...
contract XVMCrapidAdoptionBoost is Ownable {
    ...
    */
    ...
    function increaseRewards() external {
        require(block.timestamp > 1640109600, "Tuesday, December 21, 2021 6:00:00 PM (GMT)");
        require(rapidAdoptionBoost && preProgrammedCounter > 100, "Must be activated and preProgrammedCounter below 100"); 

        if(preProgrammedCounter < 166) {
            require(preProgrammedCounterTimestamp + 58 minutes < block.timestamp, "must wait 1 hour since last call");
        } else {
    ...
    function decreaseRewards() external {
        ...
        if(lastReward < (40 * 1e18) || block.timestamp > 1640434340) { killAdoptionBoost(); }
