...
interface IBeebMasterNFT is IERC721 {
...
contract BeebBank is IERC721Locker, OwnableUpgradeable, ReentrancyGuard, ContractGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of BEEBs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accBeebPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accBeebPerShare` (and `lastRewardTime`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. BEEBs to distribute per block.
        uint256 lastRewardTime; // Last timestamp that BEEBs distribution occurs.
        uint256 accBeebPerShare; // Accumulated BEEBs per share, times 1e18. See below.
        address strategy; // Strategy address, use LP to farm and buy back BEEB
    ...
    address public beeb; // BEEB
    address public nft; // BEEBMASTER
    ...
    // The block number when BEEB mining starts.
    ...
    function initialize(
        ...
        totalRewardPerSecond = _totalRewardPerSecond; // 0.278224 BEEB/seconds
        _updateRewardPerSecond();
    ...
    function replacePoolLpToken(uint256 _pid, IERC20 _lpToken) external onlyOwner {
        ...
        require(pool.accBeebPerShare == 0 && pool.totalLpSupply == 0, "Cant replace running pool");
        massUpdatePools();
    ...
    function changeStrategy(uint256 _pid, address _newStrategy) external onlyOwner {
        ...
        if (_newStrategy != address(0)) {
            ...
            require(IStrategy(_newStrategy).targetProfitToken() == beeb, "!targetProfitToken");
    ...
    // View function to see pending BEEBs on frontend.
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accBeebPerShare = pool.accBeebPerShare;
        uint256 lpSupply = pool.totalLpSupplyBoosted;
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
            uint256 _seconds = block.timestamp - pool.lastRewardTime;
            if (totalAllocPoint > 0) {
                uint256 _beebReward = (_seconds * rewardPerSecond * pool.allocPoint) / totalAllocPoint;
                accBeebPerShare += (_beebReward * 1e18) / lpSupply;
            }
        }
        uint256 _totalReward = (_getAccountBoostedAmount(_user, user.amount) * accBeebPerShare) / 1e18;
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalAllocPoint > 0) {
            uint256 _seconds = block.timestamp - pool.lastRewardTime;
            uint256 _beebReward = (_seconds * rewardPerSecond * pool.allocPoint) / totalAllocPoint;
            pool.accBeebPerShare += (_beebReward * 1e18) / lpSupply;
        }
        pool.lastRewardTime = block.timestamp;
    }
    ...
    function _harvestReward(uint256 _pid, address _account) internal {
        ...
        if (_amount > 0) {
            ...
            PoolInfo storage pool = poolInfo[_pid];
            uint256 _totalReward = (_amount * pool.accBeebPerShare) / 1e18;
            ...
            if (_claimableAmount > 0) {
                require(_claimableAmount * 100 <= IERC20(beeb).totalSupply(), "Suspicious big reward amount!!"); // <= 1% total supply
                emit RewardPaid(_account, _claimableAmount);

                _topupFunds(_claimableAmount);

                _safeBeebMint(address(this), _claimableAmount);
                _safeBeebTransfer(_account, _claimableAmount);
            }
        }
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public notContract onlyOneBlock nonReentrant checkHalving {
        ...
        user.rewardDebt = (_getAccountBoostedAmount(msg.sender, user.amount) * pool.accBeebPerShare) / 1e18;
    ...
    function _withdraw(
        ...
        user.rewardDebt = (_getAccountBoostedAmount(_account, user.amount) * pool.accBeebPerShare) / 1e18;
    ...
    function _safeBeebTransfer(address _to, uint256 _amount) internal {
        uint256 _beebBal = IERC20(beeb).balanceOf(address(this));
        if (_beebBal > 0) {
            if (_amount > _beebBal) {
                IERC20(beeb).safeTransfer(_to, _beebBal);
            } else {
                IERC20(beeb).safeTransfer(_to, _amount);
            }
        }
    }

    function _safeBeebMint(address _to, uint256 _amount) internal {
        address _beeb = beeb;
        if (ICappedMintableBurnableERC20(_beeb).minter(address(this)) && _to != address(0)) {
            uint256 _totalSupply = IERC20(_beeb).totalSupply();
            uint256 _cap = ICappedMintableBurnableERC20(_beeb).cap();
            uint256 _mintAmount = (_totalSupply + _amount <= _cap) ? _amount : (_cap - _totalSupply);
            if (_mintAmount > 0) {
                ICappedMintableBurnableERC20(_beeb).mint(_to, _mintAmount);
            }
        }
    }
    ...
    function _topupFunds(uint256 _claimableAmount) internal {
        ...
        if (
            ICappedMintableBurnableERC20(_beeb).minter(address(this)) &&
            IERC20(_beeb).totalSupply() + _totalMintAmount <= ICappedMintableBurnableERC20(_beeb).cap()
        ) {
            ICappedMintableBurnableERC20(_beeb).mint(devFund, _devAmount);
            ICappedMintableBurnableERC20(_beeb).mint(gameTreasuryFund, _gameTreasuryAmount);
            ICappedMintableBurnableERC20(_beeb).mint(jackpotFund, _jackpotAmount);
            ICappedMintableBurnableERC20(_beeb).mint(insuranceFund, _insuranceAmount);
    ...
    function getBoostedPercent(uint256 _tokenId) public view returns (uint256 _boosted) {
        if (_tokenId > 0) {
            (, , uint16 productivityStat, ) = IBeebMasterNFT(nft).getStats(_tokenId);
    ...
    function unlock(uint256 tokenId) external override nonReentrant {
        ...
        IBeebMasterNFT(nft).unlock(tokenId);
    }
