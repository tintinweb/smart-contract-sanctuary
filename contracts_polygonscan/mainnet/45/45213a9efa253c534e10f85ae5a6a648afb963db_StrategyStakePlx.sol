...
abstract contract StrategyBase is Ownable, ReentrancyGuard, Pausable, IStrategy {
    ...
    address public profitReceiver = address(0xD9C3Ed50087B2B2E5700Be70E45378C6750e6156); // Reserve Fund
    ...
    address public timelock = address(0xbd687E8aFcfb7Aaa8bDb665EFD174D7a3274AFA5); // 6h timelock
    ...
    function exchangeRate(
        ...
        try IPolyDexFormula(polydexFormula).getAmountsOut(_inputToken, _outputToken, _tokenAmount, polydexPaths[_inputToken][_outputToken]) returns (
    ...
    function _swapToken(
        ...
        IPolyDexRouterV2(polydexRouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(
            _inputToken,
            _outputToken,
            _amount,
            1,
            polydexPaths[_inputToken][_outputToken],
            _to,
            block.timestamp + 60
        );
    }
...
contract StrategyStakePlx is StrategyBase {
    using SafeERC20 for IERC20;

    // _controller = MeebFarmingBank
    // _want = PLX
    // _farmingToken = PLX
    // _farmingPool = IFarmingPool (earns PLX)
    // _farmingPoolId = 0 // Stake pool
    // _targetProfitToken = MEEB
    // _farmingToken2UsdcPath = [pair(PLX/WETH), pair(WETH/USDC)]
    // _farmingToken2TargetProfitTokenPath = [pair(PLX/WETH), pair(WETH/MEEB)]
    constructor(
        address _controller,
        address _want,
        address _farmingToken,
        ...
        StrategyBase(
            ...
            _farmingPoolId,
            _targetProfitToken,
            _farmingToken2UsdcPath,
            _farmingToken2TargetProfitTokenPath
        )
    {}
