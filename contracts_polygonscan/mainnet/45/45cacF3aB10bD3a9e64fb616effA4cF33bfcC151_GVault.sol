...
contract GVault is Ownable {
    ...
    uint256 public devFee = 50; // in BP
    uint256 public nftFee = 30; // Per NFT, in BP
    ...
    uint public numberOfUsers = 0;
  	...
  	uint public emergencyBlock = 0;
  	function declareEmergencyState() public onlyOwner {
    	 emergencyBlock = block.number + 120000;
    }
  	function confirmEmergencyState() public onlyOwner {
    	require(block.number > emergencyBlock && emergencyBlock != 0);
      	emergencyState = true;
    }
  	function cancelEmergencyState() public onlyOwner {
    	emergencyBlock = 0;
  	...
  	function withdrawNFT(uint id) public {
        ...
        for (uint i=0; i<5; i++) {
            ...
            if (NFTList[i].owner == msg.sender && NFTList[i].nftId == id) {
                ...
                if (NFTList[i].nftType == 5) {
                    NFT5.transferFrom(address(this), NFTList[i].owner, NFTList[i].nftId);
                }
    ...
    function unstakeFromGfarm(uint index) public onlyOwner {
      	require(emergencyState == true);
        gfarmPool.unstakeNft(index);
    }
    
    function transferNFT3(uint id) public onlyOwner {
      	require(emergencyState == true);
        NFT3.transferFrom(address(this), tx.origin, id);
    }
    
    function transferNFT4(uint id) public onlyOwner {
      	require(emergencyState == true);
        NFT4.transferFrom(address(this), tx.origin, id);
    }
    
    function transferNFT5(uint id) public onlyOwner {
      	require(emergencyState == true);
