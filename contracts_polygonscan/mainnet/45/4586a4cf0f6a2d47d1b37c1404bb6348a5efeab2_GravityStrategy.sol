...
contract GravityStrategy is BaseStrategy {
    ...
    address public GFI = 0x874e178A2f3f3F9d34db862453Cd756E7eAb0381;
        ...
        )
    {
        ...
        reward_other_path[0] = GFI;
        reward_other_path[1] = otherToken;

        //Currently no pair for GFI/WMATIC, so route through GFI -> WETH -> WMATIC
        reward_matic_path = new address[](3);
        reward_matic_path[0] = GFI;
        reward_matic_path[1] = WETH;
    ...
    function balanceOfPool() public override view returns (uint256) {
        (uint256 amount, ) = IGravityRewards(rewards).userInfo(address(this));
        return amount;
    }

    function getHarvestable() external override view returns (uint256) {
        return IGravityRewards(rewards).pendingReward(address(this));
    }
    ...
    function deposit() public override {
        ...
        if (_want > 0) {
            ...
            IGravityRewards(rewards).deposit(_want);
        }
    }

    function _withdrawSome(uint256 _amount) internal override returns (uint256) {
        IGravityRewards(rewards).withdraw(_amount);
        return _amount;
    }
    
    function _getReward() internal {
        IGravityRewards(rewards).withdraw(0);
    }
    ...
    function emergencyWithdraw() public onlyOwner {
        IGravityRewards(rewards).emergencyWithdraw();
