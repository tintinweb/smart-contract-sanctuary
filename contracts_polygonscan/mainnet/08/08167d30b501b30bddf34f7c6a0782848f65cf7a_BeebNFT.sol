...
interface IBeebMasterStats {
    ...
    /**
     * @dev Returns BeebMaster stats from the token ID
     */
...
contract BeebNFT is BNFT, IBeebNFT, ReentrancyGuard {
    using Address for address;
    ...
    constructor() BNFT(_msgSender(), "https://nftstatic.meebmaster.com/static/nft/POLYGON/", "Beeb NFT", "BEEBMASTER") {}

    function setBeebStats(address newBeebStats) external onlyAdmin {
        _meebStats = newBeebStats;
    }
        ...
        )
    {
        if (_meebStats != address(0)) {
            (pvpStats, luckStat, productivityStat, otherStats) = IBeebMasterStats(_meebStats).stats(tokenId);
        }
    }

    function lockedBy(uint256 tokenId) public view override returns (address) {
        return _lockedBy[tokenId];
    }

    function lock(uint256 tokenId, address locker) external override nonReentrant {
        require(_lockedBy[tokenId] == address(0), "BeebNFT: already locked");

        //solhint-disable-next-line max-line-length
        require(_isApprovedOrOwner(_msgSender(), tokenId), "ERC721: transfer caller is not owner nor approved");

        _lockedBy[tokenId] = locker;
        require(_checkOnERC721Locked(ownerOf(tokenId), locker, tokenId, ""), "BeebNFT: lock failed");
    }
    ...
    function unlock(uint256 tokenId) external override {
        ...
        //solhint-disable-next-line max-line-length
        require(_locker == _msgSender() || admin() == _msgSender(), "BeebNFT: caller is not locker nor admin");
        require(_checkOnERC721Unlocked(ownerOf(tokenId), _locker, tokenId, ""), "BeebNFT: unlock failed");
    ...
    function _checkOnERC721Locked(
        ...
        if (locker.isContract()) {
            try IERC721Locker(locker).onERC721Locked(_msgSender(), from, tokenId, _data) returns (bytes4 retval) {
                ...
                if (reason.length == 0) {
                    revert("BeebNFT: lock by non ERC721Locker implementer");
                } else {
                    // solhint-disable-next-line no-inline-assembly
                    assembly {
                        revert(add(32, reason), mload(reason))
    ...
    function _checkOnERC721Unlocked(
        ...
        if (locker.isContract()) {
            try IERC721Locker(locker).onERC721Unlocked(_msgSender(), from, tokenId, _data) returns (bytes4 retval) {
                ...
                if (reason.length == 0) {
                    revert("BeebNFT: unlock from a non ERC721Locker implementer");
                } else {
                    // solhint-disable-next-line no-inline-assembly
                    assembly {
                        revert(add(32, reason), mload(reason))
    ...
    function _beforeTokenTransfer(
        ...
        if (_lockedBy[tokenId] != to) {
            require(_lockedBy[tokenId] == address(0), "BeebNFT: token transfer while locked");
        } else {
            delete _lockedBy[tokenId];
        }
    }
