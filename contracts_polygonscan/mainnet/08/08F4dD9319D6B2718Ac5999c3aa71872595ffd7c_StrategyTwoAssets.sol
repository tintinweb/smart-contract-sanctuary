...
abstract contract BaseStrategy is Ownable {
    ...
    // Dex
    address public constant currentRouter = 0xA102072A4C07F06EC3B4900FDC4C7B80b6c57429; //Dfyn router
...
abstract contract StrategyFarmTwoAssets is BaseStrategyMasterChef {
    ...
    address public constant dfyn = 0xC168E40227E4ebD8C1caE80F7a55a4F0e6D66C97;
    ...
    // Uniswap swap paths
    address[] public dfyn_crv_path;
    address[] public dfyn_matic_path;
    ...
    {
        dfyn_crv_path = new address[](2);
        dfyn_crv_path[0] = dfyn;
        dfyn_crv_path[1] = crv;

        dfyn_matic_path = new address[](2);
        dfyn_matic_path[0] = dfyn;
        dfyn_matic_path[1] = matic;
    }
    ...
    function harvest() public override {
        ...
        // If more than cutoff, compound
        uint256 _dfynBalance = IERC20(dfyn).balanceOf(address(this));
        if (_dfynBalance > harvestCutoff){
            
            // Swap to Matic and take performance fee
            uint256 performanceFee = _dfynBalance.mul(keepFXS).div(keepFXSmax);
            if (performanceFee>0) {
                _swapUniswapWithPath(dfyn_matic_path, performanceFee);
            ...
            // Swap to other
            uint256 _dfynBalancePost = IERC20(dfyn).balanceOf(address(this));
            _swapUniswapWithPath(dfyn_crv_path, _dfynBalancePost.div(2));

            // Add liquidity
            uint256 _dfyn = IERC20(dfyn).balanceOf(address(this));
            uint256 _crv = IERC20(crv).balanceOf(address(this));
            if (_dfyn > 0 && _crv > 0) {
                IERC20(dfyn).safeApprove(currentRouter, 0);
                IERC20(dfyn).safeApprove(currentRouter, _dfyn);
                IERC20(crv).safeApprove(currentRouter, 0);
                IERC20(crv).safeApprove(currentRouter, _crv);
...
contract StrategyTwoAssets is StrategyFarmTwoAssets {
    // Token addresses
    address public Dfyn_Rewards = 0x73667b79c083Fbd7d86BDbFC0A83373590a4770B;
    address public CRV_DFYN_LP = 0x4Ea3e2cfC39FA51Df85ebcfa366d7f0eED448a1C;
