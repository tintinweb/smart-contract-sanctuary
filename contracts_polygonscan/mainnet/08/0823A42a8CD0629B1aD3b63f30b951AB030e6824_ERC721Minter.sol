...
contract ERC721Minter is IERC2981, ERC721, Ownable {
    ...
    constructor() ERC721("Shiny Genesis Collection", "SGC") Ownable() { }
...
contract ShinyMedia is EIP712, Ownable {
    using SafeMath for uint256;
    using SafeMath for uint16;
    ...
    event Claim(uint256 _tokenID, address _tokenOwner, address _recipient);
    event Buy(uint256 _tokenID, address _from, address _recipient);
    ...
    // tokenID => Its circulating supply
    mapping(uint256 => uint16) private _tokenTotalCirculatingSupplies;
    ...
    // userAddress => (tokenID => (category => QTY))
    ...
    function _mintToken(
        ...
        // Main ID of the Token
        uint256 _newTokenID;

        if (_tokenHashToTokenID[tokenHash] == 0) {
            // Increase the Token Counter
            _newTokenID = ++_tokenCounter;
            // Store the hash
            _tokenHashToTokenID[tokenHash] = _tokenCounter;
        } else {
            _newTokenID = _tokenHashToTokenID[tokenHash];
        }

        // Get Total Circulating Supply
        uint16 _totalCirculatingSupply = _tokenTotalCirculatingSupplies[
            _newTokenID
        ];

        // Check if the token's total supply is in circulation
        require(
            _totalCirculatingSupply + _quantityToMint <= _newToken._totalSupply,
            "Media: Total Circulating Supply Exceeds Total Supply!"
        );

        if (_quantityToMint > 1) {
            for (uint16 index = 1; index <= _quantityToMint; index++) {
                // Get new ID for each token quantity
                uint256 _tokenID = _newTokenID.mul(1000).add(
                    ++_totalCirculatingSupply
                );
                ...
                string memory _tokenURI = string(
                    abi.encodePacked(
                        _newToken._uri,
                        "/",
                        toString(_totalCirculatingSupply)
                    )
                );

                // Set artist's Token Selling Type
                _tokenSellingTypes[_newToken._artist][_tokenID][
                    _tokenSellingType
                ] = 1;

                _tokenIDToToken[_tokenID] = _newToken;
        ...
        _tokenTotalCirculatingSupplies[_newTokenID] = _totalCirculatingSupply;

        emit MintToken(
            uint16(_newToken._totalSupply),
            _royaltyPoints,
            _newToken._uri,
            _newToken._title,
            _newToken._artist
    ...
    function _transfer(
        ...
        ERC721Minter(_ERC721Address).transferToken(_from, _recipient, _tokenID);
    }
    ...
    function mintAndTransfer(
        ...
        for (uint16 index = 1; index <= _quantity; index++) {
            mintToken(
                _newToken,
                _royaltyPoints,
                1,
            ...
            _transfer(
                _tokenHashToTokenID[tokenHash].mul(1000).add(
                    _tokenTotalCirculatingSupplies[
    ...
    function buyNow(
        ...
        for (uint16 index = 1; index <= _quantity; index++) {
            mint(_newToken, _royaltyPoints, 1, _tokenSellingType, _signature);
            uint256 _newTokenID = _tokenHashToTokenID[tokenHash].mul(1000).add(
                _tokenTotalCirculatingSupplies[_tokenHashToTokenID[tokenHash]]
            );
            _transfer(
                _newTokenID,
                _newToken._artist,
            ...
            _tokenSellingTypes[msg.sender][_newTokenID][
                "Unlockable"
            ] = _tokenSellingTypes[msg.sender][_newTokenID]["Unlockable"].add(
                1
            );
            emit Buy(_newTokenID, _newToken._artist, msg.sender);
        }

        payable(_marketAddress).transfer(msg.value);
        IShinyMarket(_marketAddress)._divideMoney(
            msg.value,
            _adminAddress,
            _newToken._artist
        );
    ...
    function bid(uint256 _tokenID) external payable whenTokenExist(_tokenID) {
        ...
        // Transfer Bid Amount to Market
        payable(_marketAddress).transfer(msg.value);
        IShinyMarket(_marketAddress).bid(
            _tokenID,
            msg.sender,
            msg.value,
            token._bidDuration
    ...
    function claim(uint256 _tokenID) external whenTokenExist(_tokenID) {
        IShinyMarket(_marketAddress).claimNFT(
            _tokenID,
            msg.sender,
            _adminAddress,
        ...
        _transfer(_tokenID, _tokenIDToToken[_tokenID]._artist, msg.sender);
        ...
        // Add Tokens to Unlockable selling type for the New Owner
