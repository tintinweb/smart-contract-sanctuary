...
contract TemenKu is Context, IBEP20, Ownable, ReentrancyGuard {
    ...
    uint256 private _tTotal = 1 * 10 ** 15 * 10 ** 9; // 1 Quadrillion tokens (15 0's, 9 decimal places)
    ...
    uint256 public _liquidityFee = 10; // 5% will be added pool, 5% will be converted to BNB for claim
    uint256 private _previousLiquidityFee = _liquidityFee;
    
    // white list limits to protect post-launch investors
    
    mapping(address => bool) public _isWhiteListWallet;
    mapping(address => uint256) public _whiteListAddressNextSellDate;
    mapping(address => uint256) public _whiteListTokensRemaining;
    uint256 public whiteListTokensAmount;
    uint256 public whiteListLimitLiftDate;
    bool public whiteListLimitInEffect;
    ...
    string private _name = "TEMENKU";
    string private _symbol = "TMNK";
    uint8 private _decimals = 9;

    IPancakeRouter02 public pancakeRouter;
    address public pancakePair;
    address public pancakeRouterAddress = address(0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff); // Matic Mainnet Router
    ...
    constructor (
        ...
        whiteListTokensAmount = 2 * 10 ** 12 * 10 ** 9; // 2 Trillion Tokens for WL participants.
    ...
    // ONLY USE IF ROUTER MUST CHANGE (V2, V3, etc.)  EMERGENCY USE ONLY
    function setPancakeRouterAndPair(address _pancakeRouterAddress, address _pancakePair) onlyOwner() external returns (bool) {
        // Must manually create pair before changing.
    ...
    function addWhiteListWallets(address[] memory whiteListWallets) public onlyOwner() {
        for(uint256 i = 0; i < whiteListWallets.length; i++){
            if(_isWhiteListWallet[whiteListWallets[i]] == false){ // prevent double sending
                _isWhiteListWallet[whiteListWallets[i]] = true;
                _whiteListTokensRemaining[whiteListWallets[i]] = whiteListTokensAmount;
                _whiteListAddressNextSellDate[whiteListWallets[i]] = block.timestamp;
                _tokenTransfer(msg.sender, whiteListWallets[i], whiteListTokensAmount, false);
            }
        }
    }
    ...
    function _transfer(
        ...
        if(whiteListLimitInEffect){ // Check if Limit is in effect
            if(whiteListLimitLiftDate <= block.timestamp){
                whiteListLimitInEffect = false;  // set the limit to false if the limit date has been exceeded
            } else {
                uint256 senderBalance = balanceOf(from); // get total token balance of sender
                if(_isWhiteListWallet[from] && senderBalance.sub(amount) <= _whiteListTokensRemaining[from]){
                    
                    require(_whiteListAddressNextSellDate[from] <= block.timestamp, "_transfer:: White List Wallet cannot sell whitelist tokens until next sell date.  Please read the contract for your next sale date.");
                    uint256 whiteListMaxSell = whiteListTokensAmount.mul(5).div(100);
                    ...
                    uint256 tokensToSubtract = amount.add(_whiteListTokensRemaining[from]).sub(senderBalance);

                    require(tokensToSubtract <= whiteListMaxSell, "_transfer:: May not sell more than 5% of White List tokens in a single day until the White List Limit is lifted.");
                    _whiteListTokensRemaining[from] = _whiteListTokensRemaining[from].sub(tokensToSubtract);
                    _whiteListAddressNextSellDate[from] = block.timestamp + (1 days * (tokensToSubtract.mul(100).div(whiteListMaxSell)))/100; // Only push out timer as a % of the transfer, so 5% could be sold in 1% chunks over the course of a day, for example.
                }
            }
        }
    ...
    function calculateBNBReward(address ofAddress) public view returns (uint256) {
        ...
        uint256 reward = bnbPool.mul(balanceOf(address(ofAddress))).div(10).div(ttlSupply); // spread reward over 10 claim cycles
        return reward;
    }
    ...
    modifier onlyMarketingAddress() {
        require(marketingAddress == _msgSender(), "onlyMarketingAddress: caller is not the Marketing Address");
        _;
    }
    
    function setMarketingAddress(address _marketingAddress) onlyOwner() external returns (bool){
        marketingAddress = _marketingAddress;
        return true;
    }

    
    function claimBNBForMarketing() onlyMarketingAddress() nonReentrant public {
        require(marketingClaimTime <= block.timestamp, 'claimBNBForMarketing:: Error: next available claim time not reached');
        
        
        // claim 2% of BNB pool per day
        uint256 reward = (address(this).balance).mul(2).div(100);
        
        // Move claim timer forward 23 hours (a little less than 1 day to keep timer from being pushed out further)
        marketingClaimTime = block.timestamp + 23 hours;

        emit ClaimedBNB(msg.sender, reward, marketingClaimTime);

        (bool sent,) = address(msg.sender).call{value : reward}("");
        require(sent, 'claimBNBForMarketing:: Error: Cannot withdraw reward');
    }
    ...
    function activateContract() external onlyOwner {
        ...
        //set variables
        setMaxTxPercent(50); // .5% of total, 1% of circulating supply
