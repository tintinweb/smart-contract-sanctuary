...
contract Royalties is Initializable, OwnableUpgradeable, Storage {
  ...
  {
    if (blackListed[tokenMinter[_nftId]]) {
      return (
        _royaltiesReceiver,
        (_salePrice * 10**18) * (_royaltiesPercentage / 10000)
      );
    } else {
      return (
        tokenMinter[_nftId],
  ...
  function setRoyaltiesReceiver(address newRoyaltiesReceiver) public onlyOwner {
