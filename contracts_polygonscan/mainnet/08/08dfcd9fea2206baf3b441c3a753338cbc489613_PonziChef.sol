...
// PonziChef is the master of the ponzi  He can NOT mint but he is a fair guy and will give you rewards.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once Ponzi is sufficiently
// distributed and the community can show to govern itself.
//



contract PonziChef is Ownable {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        uint256 accEggPerShare;   // Accumulated Ponzis per share, times 1e12. See below.
        uint16 depositFeeBP;      // Deposit fee in basis points
    }

    // The Ponzi TOKEN!
    PolyPonzi public Ponzi;
    // Dev address.
    address public devaddr;
    // Ponzi tokens created per block.
    uint256 public PonziPerBlock;
    ...
    constructor(
        ...
        address _devaddr,
        address _feeAddress,
        uint256 _PonziPerBlock,
        uint256 _startBlock
    ) public {
        Ponzi = _Ponzi;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        PonziPerBlock = _PonziPerBlock;
    ...
    // Update the given pool's Ponzi allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending Ponzi on frontend.
    function pendingPonzi(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 PonziReward = multiplier.mul(PonziPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accEggPerShare = accEggPerShare.add(PonziReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 PonziReward = multiplier.mul(PonziPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        Ponzi.transfer(devaddr, PonziReward.div(10));
        pool.accEggPerShare = pool.accEggPerShare.add(PonziReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    // Safe Ponzi transfer function, just in case if rounding error causes pool to not have enough EGGs.
    function safeEggTransfer(address _to, uint256 _amount) internal {
        uint256 PonziBal = Ponzi.balanceOf(address(this));
        if (_amount > PonziBal) {
            Ponzi.transfer(_to, PonziBal);
        } else {
            Ponzi.transfer(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _PonziPerBlock) public onlyOwner {
        massUpdatePools();
        PonziPerBlock = _PonziPerBlock;
    }
