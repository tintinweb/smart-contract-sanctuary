...
contract StrategyNative is BaseStrategySingle {
  ...
  function deposit(address _userAddress, uint256 _wantAmt) external override onlyOwner nonReentrant whenNotPaused returns (uint256) {
...
abstract contract BaseStrategy is Ownable, ReentrancyGuard, Pausable {
    ...
    uint256 public controllerFee = 66; // 0.25%
    uint256 public rewardRate = 300; // 0.75%
    ...
    function deposit(address _userAddress, uint256 _wantAmt) external onlyOwner virtual nonReentrant whenNotPaused returns (uint256) {
        // Call must happen before transfer
        _beforeDeposit(_userAddress);
        uint256 wantLockedBefore = wantLockedTotal();
