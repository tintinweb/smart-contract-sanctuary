...
// File: contracts/FarmMATICDividendPayingToken.sol
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public immutable DividendsToken = address(0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270);
...
// File: contracts/FarmMATIC.sol

//

pragma solidity ^0.7.6;
contract FarmMATIC is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    FarmMATICDividendTracker public dividendTracker;
    ...
    uint256 public _maxWalletToken = 1000000000 * (10**18); // 1.5% of total supply

    uint256 public immutable BSCRewardsFee;
    ...
    constructor() public ERC20("FarmMATIC","FarmMATIC") {
        uint256 _BSCRewardsFee = 11;
        uint256 _liquidityFee = 7;

        BSCRewardsFee = _BSCRewardsFee;
        liquidityFee = _liquidityFee;
        totalFees = _BSCRewardsFee.add(_liquidityFee);


    	dividendTracker = new FarmMATICDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "FarmMATIC: The dividend tracker already has that address");

        FarmMATICDividendTracker newDividendTracker = FarmMATICDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "FarmMATIC: The new dividend tracker must be owned by the Token token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "FarmMATIC: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "FarmMATIC: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "FarmMATIC: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "FarmMATIC: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "FarmMATIC: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "FarmMATIC: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "FarmMATIC: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "FarmMATIC: This account cannot send tokens until trading is enabled");
        }
...
contract FarmMATICDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
