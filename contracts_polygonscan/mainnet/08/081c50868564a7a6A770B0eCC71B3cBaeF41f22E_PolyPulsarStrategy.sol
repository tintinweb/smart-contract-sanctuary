...
contract PolyPulsarStrategy is BaseStrategy {
    ...
    address private constant GPUL = 0x40ed0565eCFB14eBCdFE972624ff2364933a8cE3;

    address private constant CAT_ROUTER = 0x94930a328162957FF1dd48900aF67B5439336cBD;
    address private constant CHEF = 0xA375495919205251a05f3B259B4D3cc30a4d3ED5;
        ...
        )
    {
        ...
        reward_matic_path[0] = GPUL;
        reward_matic_path[1] = WMATIC;

        IERC20(_want).safeApprove(CHEF, uint256(-1));
    }

    // **** Views ****

    function balanceOfPool() public override view returns (uint256) {
        (uint256 amount,,,) = IPulsarChef(rewards).userInfo(poolId, address(this));
        return amount;
    }
    ...
    function deposit() public override {
        uint256 _want = IERC20(want).balanceOf(address(this));
        if (_want > 0) {
            IPulsarChef(rewards).deposit(poolId, _want);
        }
    }

    function _getReward() internal {
        IPulsarChef(rewards).withdraw(poolId, 0);
    }
    ...
    function _withdrawSome(uint256 _amount)
        ...
        returns (uint256)
    {
        IPulsarChef(rewards).withdraw(poolId, _amount);
        return _amount;
    }
    ...
    function emergencyWithdraw() public onlyOwner {
        IPulsarChef(rewards).emergencyWithdraw(poolId);
