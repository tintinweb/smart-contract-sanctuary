...
abstract contract StrategyFarmTwoAssets is BaseStrategyMasterChef {
    ...
    address public constant sfi = 0x35b937583F04A24963eb685F728a542240f28Dd8;
    ...
    function harvest() public override {
        ...
        if (_dinoBalance > harvestCutoff){
            ...
            _swapUniswapWithPath(eth_sfi_path, _ethBalancePost.div(2));

            // Add liquidity
            uint256 _eth = IERC20(eth).balanceOf(address(this));
            uint256 _sfi = IERC20(sfi).balanceOf(address(this));
            if (_eth > 0 && _sfi > 0) {
                IERC20(eth).safeApprove(currentRouter, 0);
                IERC20(eth).safeApprove(currentRouter, _eth);
                IERC20(sfi).safeApprove(currentRouter, 0);
                IERC20(sfi).safeApprove(currentRouter, _sfi);
                ...
                IERC20(sfi).safeTransfer(
                    strategist,
                    IERC20(sfi).balanceOf(address(this))
                );
            }
        }
        // Stake the LP tokens
...
contract StrategyTwoAssets is StrategyFarmTwoAssets {
    ...
    address public Eth_Sfi_LP = 0x7C07CecD8cdd65C0daD449808cc5f9AD74C22bd1;
