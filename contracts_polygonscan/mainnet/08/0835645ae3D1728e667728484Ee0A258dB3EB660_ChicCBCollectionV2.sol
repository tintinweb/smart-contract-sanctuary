...
// Root file: contracts/polygon/ChicCBCollectionV2.sol
...
contract ChicCBCollectionV2 is BaseCollectible {
    ...
    function mintBatch(
        ...
        uint256[] calldata _blockTimestamps,
        ...
        onlyOperator
    {
        ...
        require(
            ...
            len == _blockTimestamps.length &&
            len == _blockNumbers.length,
            "Wrong length"
        );

        for (uint i; i < len; i++) {
            _safeMint(_tos[i], _tokenIds[i]);
            createdAt[_tokenIds[i]] = _blockTimestamps[i];
            createdAtBlock[_tokenIds[i]] = _blockNumbers[i];
        }
    }
}