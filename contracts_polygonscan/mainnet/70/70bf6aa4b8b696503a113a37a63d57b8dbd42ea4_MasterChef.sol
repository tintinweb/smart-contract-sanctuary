...
contract MasterChef is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 lastRewardBlock;  // Last block number that TREE distribution occurs.
        uint256 accTreePerShare;   // Accumulated TREE per share, times 1e12. See below.
    ...
    // View function to see pending tree on frontend.
    function pendingTree(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 treeReward = multiplier.mul(treePerBlock).mul(pool.allocPoint).div(totalAllocPoint);
    ...
    // Safe tree transfer function, just in case if rounding error causes pool to not have enough tree.
