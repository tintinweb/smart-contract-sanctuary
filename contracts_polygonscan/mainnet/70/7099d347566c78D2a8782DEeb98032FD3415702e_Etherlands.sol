    ...
    NativeMetaTransaction
{
    ...
    mapping(uint64 => coordinate) public plot_loc; // mapping from plotId to plot coordinate;
    ...
    function pack_coordinates(int24 b1, int24 b2)
        internal
        pure
        returns (bytes6 answer)
    {
        ...
        assembly {
            mstore(add(result, 3), b1)
            mstore(add(result, 6), b2)
        }
        assembly {
            answer := mload(add(result, 6))
        }
    }
    ...
    function _claimPlot(
        ...
        plot_loc[totalPlots] = coordinate(_x, _z);
