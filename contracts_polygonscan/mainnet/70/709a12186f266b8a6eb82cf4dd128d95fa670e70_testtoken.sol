...
contract testtoken is BEP20("testtoken1", "test") {
    // Burn address
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        if (recipient == BURN_ADDRESS) {
            ...
            require(amount == sendAmount + burnAmount, "test::transfer: Burn value invalid");
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    // Initial emission rate: 1 test per block.
    uint256 public constant INITIAL_EMISSION_RATE = 1 ether;
    // Minimum emission rate: 0.4 test per block.
    ...
    // test referral contract address.
    ItestReferral public testReferral;
    ...
    constructor(
        ...
        testPerBlock = INITIAL_EMISSION_RATE;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 testReward = multiplier.mul(testPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        test.mint(devAddress, testReward.div(10));
    ...
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        ...
        updatePool(_pid);
        if (_amount > 0 && address(testReferral) != address(0) && _referrer != address(0) && _referrer != msg.sender) {
            testReferral.recordReferral(msg.sender, _referrer);
        }
        if (user.amount > 0) {
        ...
        if (_amount > 0) {
            ...
            if (address(pool.lpToken) == address(test)) {
                // burn 1% of token
    ...
    function updateEmissionRate() public {
        ...
        require(testPerBlock > MINIMUM_EMISSION_RATE, "updateEmissionRate: Emission rate has reached the minimum threshold");
        ...
        uint256 newEmissionRate = testPerBlock;
        ...
        if (newEmissionRate >= testPerBlock) {
            return;
        }

        massUpdatePools();
        lastReductionPeriodIndex = currentIndex;
        uint256 previousEmissionRate = testPerBlock;
        testPerBlock = newEmissionRate;
        emit EmissionRateUpdated(msg.sender, previousEmissionRate, newEmissionRate);
    }

    // Update the test referral contract address by the owner
    function settestReferral(ItestReferral _testReferral) public onlyOwner {
        testReferral = _testReferral;
    }
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (address(testReferral) != address(0) && referralCommissionRate > 0) {
            address referrer = testReferral.getReferrer(_user);
            uint256 commissionAmount = _pending.mul(referralCommissionRate).div(10000);

            if (referrer != address(0) && commissionAmount > 0) {
                test.mint(referrer, commissionAmount);
