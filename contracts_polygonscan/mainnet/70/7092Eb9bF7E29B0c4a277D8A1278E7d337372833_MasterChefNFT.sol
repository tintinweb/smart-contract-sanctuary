...
contract MasterChefNFT is Ownable, ReentrancyGuard, ERC721Holder {
    ...
    // Experiencie extra Boosted
    uint256 public expExtraBoosted;
    ...
    function _updateHarvestLockup(uint256 _pid, address _userAddress) internal {
        ...
        if (user.hasNFT && newHarvestInverval > 0) {
            ...
            uint256 extraBoosted = quarterInterval;
            if (user.experience < experienceFactor) {
                extraBoosted = (((user.experience * 100) / experienceFactor) * extraBoosted) / 100;
            }
            newHarvestInverval = newHarvestInverval - quarterInterval - extraBoosted;
        }
        if (newHarvestInverval > 0){
    ...
    function _payNFTBoost(uint256 _pid, address _userAddress, uint256 _pending) internal {
        ...
        uint256 extraBoosted = expExtraBoosted;
        if (user.experience < experienceFactor) {
            extraBoosted = (((user.experience * 100) / experienceFactor) * extraBoosted) / 100;
        }
    ...
    // Set experience factor. OnlyOwner
    function setExperienceFactor(uint256 _experienceFactor) external onlyOwner {
    ...
    // Set experience factor. OnlyOwner
    function setExtraBoosted(uint256 _expExtraBoosted) external onlyOwner {
        require(_expExtraBoosted >= 0);

        expExtraBoosted = _expExtraBoosted;
    }
