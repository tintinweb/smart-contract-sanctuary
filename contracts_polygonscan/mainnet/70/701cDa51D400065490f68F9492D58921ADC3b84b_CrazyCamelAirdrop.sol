...
// File: contracts/CrazyCamelsHonorary.sol
    ...
    Ownable
{
    ...
    constructor()
        ERC721("Crazy Camel Airdrop", "CAMEL")
    {}
    ...
    function setURI(string calldata _newURI) external onlyOwner {
        URI = _newURI;
        emit ChangeURI(_newURI);
    }

    function viewURI() internal view returns (string memory) {
        return URI;
    }
    ...
    function giftMint(address[] calldata _addresses) external onlyOwner {
        ...
        require(
            giftCount + _addresses.length <= GiftSupply,
            "Crazy Camel: max gift supply exceeded"
        );
        ...
        for (uint256 ind = 0; ind < _addresses.length; ind++) {
            require(
                ...
                "Crazy Camel: recepient is the null address"
            );
            _tokenIds.increment();
    ...
    function changeGiftSupply(uint16 _newGiftSupply) public onlyOwner {
        GiftSupply = _newGiftSupply;
    }

    function burn(uint256 tokenId) external {
        require(isBurnEnabled, "Crazy Camel: burning disabled");
        require(
            _isApprovedOrOwner(msg.sender, tokenId),
            "Crazy Camel: burn caller is not owner nor approved"
        );
        _burn(tokenId);
        giftCount = giftCount - 1;
    }
