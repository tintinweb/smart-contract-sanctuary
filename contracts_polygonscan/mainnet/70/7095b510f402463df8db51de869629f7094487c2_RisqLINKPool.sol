 ...
 * https://forum.zeppelin.solutions/t/how-to-implement-ero20-supply-mechanisms/226[How
...
// File: contracts/Pool/RisqLINKPool.sol
...
/**
 * @author macemclain
 * @title Risq LINK Liquidity Pool
 * @notice Accumulates liquidity in LINK from LPs and distributes P&L in LINK
 */
contract RisqLINKPool is
    IERCLiquidityPool,
    Ownable,
    ERC20("Risq LINK LP Token", "writeLINK")
{
    using SafeMath for uint256;
    ...
    /*
     * @nonce A provider supplies LINK to the pool and receives writeLINK tokens
    ...
    /*
     * @nonce Provider burns writeLINK and receives LINK from the pool
     * @param amount Amount of LINK to receive
    ...
    /*
     * @nonce Returns provider's share in LINK
     * @param account Provider's address
     * @return Provider's share in LINK
     */
    ...
    /*
     * @nonce Returns the amount of LINK available for withdrawals
    ...
    /*
     * @nonce Returns the LINK total balance provided to the pool
...
// File: contracts/Options/RisqLINKOptions.sol
...
/**
 * @author macemclain
 * @title Risq LINK (Wrapped Bitcoin) Bidirectional (Call and Put) Options
 * @notice Risq Protocol Options Contract
 */
contract RisqLINKOptions is Ownable, IRisqOptions {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    ...
    RisqLINKPool public pool;
    IUniswapV2Router01 public quickswapRouter;
    address[] public maticToLinkSwapPath;
    IERC20 public link;

    /**
     * @param _priceProvider The address of ChainLink BTC/USD price feed contract
     * @param _quickswap The address of Quickswap router contract
     * @param token The address of LINK ERC20 token contract
     */
    constructor(
        AggregatorV3Interface _priceProvider,
        IUniswapV2Router01 _quickswap,
        ERC20 token,
        IRisqStakingERC20 _settlementFeeRecipient
    ) public {
        pool = new RisqLINKPool(token);
        link = token;
        ...
        maticToLinkSwapPath = new address[](2);
        maticToLinkSwapPath[0] = quickswapRouter.WETH();
        maticToLinkSwapPath[1] = address(link);
    ...
    {
        (uint256 total, uint256 totalMATIC, uint256 settlementFee, uint256 strikeFee, ) =
        ...
        uint amountIn = swapToLINK(totalMATIC, total);
    ...
    function approve() public {
        link.safeApprove(address(pool), uint(-1));
        link.safeApprove(address(settlementFeeRecipient), uint(-1));
    }
     ...
     * @return totalMATIC Total price in MATIC to be paid
        ...
        )
    {
        ...
        totalMATIC = quickswapRouter.getAmountsIn(total, maticToLinkSwapPath)[0];
    }
    ...
    /**
     * @notice Sends profits in LINK from the LINK pool to an option holder's address
    ...
    /**
     * @notice Swap MATIC to LINK via quickswapswap router
     * @param maxAmountIn The maximum amount of MATIC that can be required before the transaction reverts.
     * @param amountOut The amount of LINK tokens to receive.
     */
    function swapToLINK(
        uint maxAmountIn,
        uint amountOut
        ...
        returns (uint)
    {
            uint[] memory amounts = quickswapRouter.swapETHForExactTokens {
                value: maxAmountIn
            }(
                amountOut,
                maticToLinkSwapPath,
