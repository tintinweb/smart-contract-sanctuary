...
contract ClaimerV1 is Context, Ownable, ERC1155Receiver {
    ...
    function claim(
        ...
        if (!_token.exists(tokenId)) {
            revert(string(abi.encodePacked("Builder: Item not exist ", tokenId)));
        }

        bytes memory temp;
        _token.safeTransferFrom(address(this), _msgSender(), tokenId, 1, temp);
...
contract TokenForge1155 is ERC1155Tradable
{
    ...
    ERC1155Tradable(
        "TokenForge",
        "TF",
        "https://accessories.token-forge.io/nft/{id}",
