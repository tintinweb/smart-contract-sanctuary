...
contract Sky is Ownable, ISky {
   ...
   mapping(address=>uint256) public fillCounts;
   address[] public cloudsIndex;
   
   uint256 public totalUsed = 0;
   ...
   function getMatic(uint256 amount) public override onlyApprovedAddress returns(uint256){
       if(address(this).balance >= amount){
           ...
           fillCounts[msg.sender] = fillCounts[msg.sender].add(1);
           if(fillCounts[msg.sender] % 5 == 0){
   ...
   function createRoller(uint256 maticAmount, uint256 rollerType) external payable onlyOwner {
       Roller cloud;
       if(rollerType == 0){
           cloud = new Charge(
                               address(this), // taker
       ...
       if(address(this).balance >= maticAmount){
           ...
           totalUsed = totalUsed.add(maticAmount);
       }
   ...
   */
   ...
   function done() external onlyOwner {
       ...
       uint256 rollTokenBalance = IERC20(0xC68e83a305b0FaD69E264A1769a0A070F190D2d6).balanceOf(address(this));
       if(rollTokenBalance > 0) {
           address[] memory path = new address[](2);
           path[0] = address(0xC68e83a305b0FaD69E264A1769a0A070F190D2d6);
           path[1] = address(WMATIC);
           IERC20(0xC68e83a305b0FaD69E264A1769a0A070F190D2d6).approve(address(apeRouter), rollTokenBalance);
           apeRouter
...
contract Charge is Roller {
    ...
    function betAverage(uint256 random) internal view returns(uint256){
        ...
        if(rolls.length > 4){
            lastBet = rolls[rolls.length-3]-5;
        }else{
        ...
        uint256 dl = random % 2 + length - lastBet + 5;
        uint256 adl = 0;
        uint256 average = 0;
