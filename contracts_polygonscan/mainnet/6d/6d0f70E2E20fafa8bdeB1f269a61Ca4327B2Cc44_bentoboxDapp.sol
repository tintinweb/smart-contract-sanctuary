...
contract bentoboxDapp {
    ...
    function depositUserFunds (uint256 amount_, address token_address, address address_from, address address_to) public  {
        ...
        uint256 WMATIC_balanceInUSD = BentoTokenBalanceOf(WMATIC_ADDRESS, address(this)).mul(uint256(getLatestPrice(WMATIC_ADDRESS))).div(10**20);
        uint256 SUSHI_balanceInUSD = BentoTokenBalanceOf(SUSHI_ADDRESS, address(this)).mul(uint256(getLatestPrice(SUSHI_ADDRESS))).div(10**20);
        uint256 WETH_balanceInUSD = BentoTokenBalanceOf(WETH_ADDRESS, address(this)).mul(uint256(getLatestPrice(WETH_ADDRESS))).div(10**20);
        uint256 WBTC_balanceInUSD = BentoTokenBalanceOf(WBTC_ADDRESS, address(this)).mul(uint256(getLatestPrice(WBTC_ADDRESS))).div(10**10);

        uint256 Total_in_USD = WMATIC_balanceInUSD.add(SUSHI_balanceInUSD).add(WETH_balanceInUSD).add(WBTC_balanceInUSD);
        
        if (Total_in_USD > 0) {
            uint256 WMATIC_share = WMATIC_balanceInUSD.div(Total_in_USD).mul(amount_); 
            uint256 SUSHI_share = SUSHI_balanceInUSD.div(Total_in_USD).mul(amount_);
            uint256 WETH_share = WETH_balanceInUSD.div(Total_in_USD).mul(amount_);
            uint256 WBTC_share = WBTC_balanceInUSD.div(Total_in_USD).mul(amount_); //only has 8 decimals??
            ...
            swap(amount_.div(4), uint256(0), USDCToWMATICPath, address(this), uint256(-1));
            swap(amount_.div(4), uint256(0), USDCToSUSHIPath, address(this), uint256(-1));
            swap(amount_.div(4), uint256(0), USDCToWETHPath, address(this), uint256(-1));
            swap(amount_.div(4), uint256(0), USDCToWBTCPath, address(this), uint256(-1));
        }
        ...
        if (Total_in_USD > 0) {
            uint256 newSharesForUser = amount_.div(Total_in_USD).mul(totalNumberOfShares);
            totalNumberOfShares = totalNumberOfShares.add(newSharesForUser);
            if (userNumberOfShares[address_from] > 0) {
                userNumberOfShares[address_from] = userNumberOfShares[address_from].add(newSharesForUser);
            } else {
                userNumberOfShares[address_from] = newSharesForUser;
            }
        } else {
            userNumberOfShares[address_from] = 100000000;
            totalNumberOfShares = userNumberOfShares[address_from];
        }
    }
    ...
    function withdrawUserFunds(address user, uint256 WMATIC_amount, uint256 SUSHI_amount,uint256 WETH_amount,uint256 WBTC_amount) public {
        ...
        withdraw(WMATIC_amount, WMATIC_ADDRESS, address(this), address(this));
        withdraw(SUSHI_amount, SUSHI_ADDRESS, address(this), address(this));
        withdraw(WETH_amount, WETH_ADDRESS, address(this), address(this));
        withdraw(WBTC_amount, WBTC_ADDRESS, address(this), address(this));

        uint256 WMATICbalance = IERC20(WMATIC_ADDRESS).balanceOf(address(this));
        uint256 SUSHIbalance = IERC20(SUSHI_ADDRESS).balanceOf(address(this));
        ...
        approve_spending(WMATIC_ADDRESS, SUSHISWAP_ROUTER, WMATICbalance);
        approve_spending(SUSHI_ADDRESS, SUSHISWAP_ROUTER, SUSHIbalance);
        approve_spending(WETH_ADDRESS, SUSHISWAP_ROUTER, WETHbalance);
        approve_spending(WBTC_ADDRESS, SUSHISWAP_ROUTER, WBTCbalance);

        swap(WMATICbalance, uint256(0), WMATICToUSDCPath, address(this), uint256(-1));
        swap(SUSHIbalance, uint256(0), SUSHIToUSDCPath, address(this), uint256(-1));
        swap(WETHbalance, uint256(0), WETHToUSDCPath, address(this), uint256(-1));
        swap(WBTCbalance, uint256(0), WBTCToUSDCPath, address(this), uint256(-1));
