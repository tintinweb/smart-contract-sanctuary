...
// new updates 
    ...
    *  pre allocation round, users can send a certain amount of USDTs to this contract;
       ...
       tick is 0.01 unit, round by 1e16
    */ 
    function preAlloc(uint amount) external isready isFunded isSaleOpen{
        require(amount >0, "amount > 0 !!!");
        uint _amount = amount - amount.mod(1e16); // round to 1e16
    ...
    * 2nd round purchase for user
        tick is 0.01 unit, amount == 1e16 ==> 0.01 unit of token 

    */
    function R2purchase(uint amount) isready external{
        require(amount >0,"amount > 0");
        uint _amount = amount - amount.mod(1e16); // round to 1e16
    ...
    */
    function vote(uint _vote) external{
       ...
       require(userInfo[_msgSender()].checked || userInfo2[_msgSender()].times > 0, "participants only!");
