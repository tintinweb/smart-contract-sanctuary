...
contract HopStrategy is BaseStrategyStakingRewards {
    ...
    address public HOP_WMATIC_LP = 0xbc4FB4ED825C65fF48163AF7E59d49e32edb5269;
    address private HMATIC = 0x712F0cf37Bdb8299D0666727F73a5cAbA7c1c24c; //not used

    address public HOP_SWAP = 0x3d4Cc8A61c7528Fd86C55cfe061a78dCBA48EDd1;
    address public HOP_REWARDS = 0x7dEEbCaD1416110022F444B03aEb1D20eB4Ea53f;
    ...
    constructor() public BaseStrategyStakingRewards(HOP_REWARDS, HOP_WMATIC_LP, FUND, WMATIC, QUICKSWAP_ROUTER) {
        multiHarvest = 0x3355743Db830Ed30FF4089DB8b18DEeb683F8546;

        IERC20(WMATIC).safeApprove(HOP_SWAP, uint256(-1));
    }
    ...
    function harvest() public override onlyHumanOrWhitelisted nonReentrant {
        ...
        //Calculate the amount of MATIC harvested
        uint256 balance_before = IERC20(harvestedToken).balanceOf(address(this));
        _getReward();
        //Deduct fee from the harvested MATIC
        ...
        //Deposit WMATIC in pool to get LP tokens
        uint256 _wmaticBalance = IERC20(WMATIC).balanceOf(address(this));
        if (_wmaticBalance > 0) {
            uint256[] memory amounts = new uint256[](2);
            amounts[0] = _wmaticBalance;
            amounts[1] = 0;

            IStableSwap(HOP_SWAP).addLiquidity(amounts, 0, now.add(60));
        }
    }
