...
// VirgoToken with Governance.
contract VirgoToken is BEP20("Virgo Token", "VIRGO") {
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "VIRGO::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "VIRGO::delegateBySig: invalid nonce"
        );
        require(now <= expiry, "VIRGO::delegateBySig: signature expired");
        ...
        returns (uint256)
    {
        require(
            ...
            "VIRGO::getPriorVotes: not yet determined"
        );
    ...
    function _delegate(address delegator, address delegatee) internal {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying VIRGO (not scaled);
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            "VIRGO::_writeCheckpoint: block number exceeds 32 bits"
        );
...
// MasterChef is the master of VIRGO. He can make Virgo and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once VIRGO is sufficiently
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of VIRGO
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accVirgoPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accVirgoPerShare` (and `lastRewardBlock`) gets updated.
    ...
    // The block number when Virgo mining starts.
    ...
    event UpdateEmissionRate(address indexed user, uint256 VirgoPerBlock);
    event addPool(
        uint256 indexed pid,
        address lpToken,
        uint256 allocPoint,
    ...
    constructor(
        ...
        address _devaddr,
        address _feeAddress,
        uint256 _VirgoPerBlock,
        uint256 _startBlock,
        ForgeToken _forgeTokenLeo,
        uint256 _nftIdLeo,
        ...
        forgeTokenLeo = _forgeTokenLeo;
        nftIdLeo = _nftIdLeo;
        forgeTokenCancer = _forgeTokenCancer;
    ...
    function isLeoVip(address _address) public view returns (bool) {
        return forgeTokenLeo.balanceOf(_address, nftIdLeo) > 0;
    }

    // Check if user is holding the CANCER powerup NFT
    function isCancerVip(address _address) public view returns (bool) {
        return forgeTokenCancer.balanceOf(_address, nftIdCancer) > 0;
    }

    // Check if user is holding the NEPTUNE powerup NFT
    function isNeptuneVip(address _address) public view returns (bool) {
        return forgeTokenNeptune.balanceOf(_address, nftIdNeptune) > 0;
    }

    // Check if user is holding the VIRGO powerup NFT
    function isVirgoVip(address _address) public view returns (bool) {
        return forgeTokenVirgo.balanceOf(_address, nftIdVirgo) > 0;
    }
    ...
    // Update the given pool's VIRGO allocation point and deposit fee. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        uint16 _depositFeeBP,
    ...
    function calculateBonus(address _user) public view returns (uint256) {
        ...
        uint256 vipVirgoRewardMultiplier;
        ...
        _isVirgoVip = isVirgoVip(_user);

        if (_isLeoVip) {
            vipLeoRewardMultiplier = 40;
        } else {
            vipLeoRewardMultiplier = 0;
        }

        if (_isCancerVip) {
            vipCancerRewardMultiplier = 20;
        } else {
            vipCancerRewardMultiplier = 0;
        }

        if (_isNeptuneVip) {
            vipNeptuneRewardMultiplier = 20;
        } else {
            vipNeptuneRewardMultiplier = 0;
        }

        if (_isVirgoVip) {
            vipVirgoRewardMultiplier = 20;
        } else {
            vipVirgoRewardMultiplier = 0;
        }

        uint totalReward = vipLeoRewardMultiplier.add(vipCancerRewardMultiplier).add(vipNeptuneRewardMultiplier).add(vipVirgoRewardMultiplier);
        
        return BONUS_MULTIPLIER.add(totalReward).div(100);
    }
    ...
    // Deposit LP tokens to MasterChef for VIRGO allocation.
    ...
    // Safe virgo transfer function, just in case if rounding error causes pool to not have enough VIRGOs.
    function safeVirgoTransfer(address _to, uint256 _amount) internal {
        uint256 virgoBal = virgo.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > virgoBal) {
            transferSuccess = virgo.transfer(_to, virgoBal);
        } else {
            transferSuccess = virgo.transfer(_to, _amount);
        }
        require(transferSuccess, "safeVirgoTransfer: transfer failed");
    }
