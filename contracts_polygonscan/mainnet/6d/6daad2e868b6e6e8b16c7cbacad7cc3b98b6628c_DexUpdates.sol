...
contract DexUpdates is Ownable, DexStorage {
  ...
  function onERC721Received(
        ...
        return INFT(0x2c3479B526394d9a5e18E2E454B9f8b1282930AC).onERC721Received(operator, from,tokenId, data);
  ...
  function migrateNFT(address newCollection, address collectionFrom, uint256[] memory tokenIds) public {
    ...
    function migrateOnSellNFT(address newCollection, address collectionFrom, uint256 tokenIdOld) public {
         
      string memory uri = IERC20(collectionFrom).tokenURI(tokenIdOld);
    
      uint256 tokenId = INFT(newCollection).mint(address(this), uri);
      updateSellDetail(tokenIdOld, tokenId, newCollection, collectionFrom);
       emit MigrateNFT(tokenIdOld, tokenId, newCollection, collectionFrom);
    }

    function migrateNFTSingle(address newCollection, address collectionFrom, uint256 tokenId) public {
         
      string memory uri = IERC20(collectionFrom).tokenURI(tokenId);
      address to = INFT(collectionFrom).ownerOf(tokenId);
      uint256 tokenIdNew = INFT(newCollection).mint(to, uri);
      if(to == nonCryptoNFTVault){
       _nonCryptoOwners[tokenIdNew][address(newCollection)] =  _nonCryptoOwners[tokenId][address(collectionFrom)];
      }
       emit MigrateNFT(tokenId, tokenIdNew, newCollection, collectionFrom);
    }

    function mintNft(address collection, string memory uri, address to) isValid(collection) public {
     require(admin == msg.sender,"not authenticated");
     INFT(collection).mint(to, uri);
    }

    function mintNftOldXanalia(address collection, string memory uri, address to) isValid(collection) public {
     require(admin == msg.sender,"not authenticated");
     INFT(0x2c3479B526394d9a5e18E2E454B9f8b1282930AC).mintWithTokenURI(to, uri);
    }
