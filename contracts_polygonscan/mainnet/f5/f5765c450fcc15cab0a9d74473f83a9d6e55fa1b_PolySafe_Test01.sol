...
contract PolySafe_Test01 is Token {
    ...
    uint private constant MIN_MATIC_STAKE   = 1 ether;
    ...
    function getTokenPrice() public view returns(uint) {
        return getContractMaticBalance().div(availableSupply().add(1e18));
    } 

    function maticToToken(uint maticAmount) public view returns(uint) {
        return maticAmount.div(getTokenPrice());
    }

    function tokenToMatic(uint tokenAmount) public view returns(uint) {
        return tokenAmount.mul(getTokenPrice());
    } 	
