...
// File: contracts/lib/SpookMath.sol
...
library SpookMath {
    ...
    /*
        Integrate spook curve from V1 to V2
        require V0>=V1>=V2>0
...
library PMMPricing {
        ...
        )
    {
        ...
        return
            SpookMath._SolveQuadraticFunctionForTrade(
                state.Q0,
                state.Q0,
                payBaseAmount,
                state.i,
    ...
    function adjustedTarget(PMMState memory state) internal pure {
        if (state.R == RState.BELOW_ONE) {
            state.Q0 = SpookMath._SolveQuadraticFunctionForTarget(
                state.Q,
            ...
            state.B0 = SpookMath._SolveQuadraticFunctionForTarget(
                state.B,
...
// File: contracts/SpookVendingMachine/impl/SVMStorage.sol


contract SVMStorage is ReentrancyGuard {
    using SafeMath for uint256;

    bool public _IS_OPEN_TWAP_ = false;

    bool internal _SVM_INITIALIZED_;
...
// File: contracts/SpookVendingMachine/impl/SVMVault.sol





contract SVMVault is SVMStorage {
    using SafeMath for uint256;
    ...
    function permit(
        ...
        require(deadline >= block.timestamp, "Spook_SVM_LP: EXPIRED");
        ...
        require(
            ...
            "Spook_SVM_LP: INVALID_SIGNATURE"
        );
        _approve(owner, spender, value);
    }
}

// File: contracts/intf/ISpookCallee.sol


interface ISpookCallee {
    function SVMSellShareCall(
        address sender,
...
// File: contracts/SpookVendingMachine/impl/SVMTrader.sol




contract SVMTrader is SVMVault {
    using SafeMath for uint256;
    ...
    function flashLoan(
        ...
        if (data.length > 0)
            ISpookCallee(assetTo).SVMFlashLoanCall(msg.sender, baseAmount, quoteAmount, data);
        ...
        emit SpookFlashLoan(msg.sender, assetTo, baseAmount, quoteAmount);
    }
...
// File: contracts/SpookVendingMachine/impl/SVMFunding.sol


contract SVMFunding is SVMVault {
    // ============ Events ============
    ...
    function sellShares(
        ...
        if (data.length > 0) {
            ISpookCallee(to).SVMSellShareCall(
                msg.sender,
                shareAmount,
                baseAmount,
                quoteAmount,
...
// File: contracts/SpookVendingMachine/impl/SVM.sol

/**
 * @title Spook VendingMachine
 * @author Murciano207
 *
 * @notice SpookVendingMachine initialization
 */
contract SVM is SVMTrader, SVMFunding {
    function init(
        address maintainer,
        ...
        require(!_SVM_INITIALIZED_, "SVM_INITIALIZED");
        _SVM_INITIALIZED_ = true;
