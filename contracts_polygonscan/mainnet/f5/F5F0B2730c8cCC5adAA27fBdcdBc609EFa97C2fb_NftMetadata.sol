...
contract NftMetadata is INftMetadata, Ownable
{
  ...
  /**
   * @dev The base URL used to serve data.  If this is set to a non-empty
   * string, then instead of generating real data: URLs, the contract will
   * generate a URL that passes the data to a server and the server can pass
   * it back to the client.  This circumvents issues with data consumers that
   * don't support data: properly.
   */
  ...
  /**
   * @dev Encodes a given payload of data into a data: URL string or
   * using the data server if configured.
   */
  function buildDataUrl (string memory mimeType, bytes memory payload)
      internal view returns (string memory)
  {
    /* If a data server is set, use that.  */
    if (bytes (dataServerUrl).length > 0)
      return string (abi.encodePacked (
        dataServerUrl,
        "?t=", urlEncode (mimeType),
        "&d=", urlEncode (Base64.encode (payload))
      ));

    /* Generate a real data: URL.  */
    return string (abi.encodePacked (
      "data:", mimeType,
      ...
      internal view returns (string memory)
  {
    ...
    return string (abi.encodePacked (
      "{",
        ...
        /* The base64- or URL encoded data URI can be included literally
