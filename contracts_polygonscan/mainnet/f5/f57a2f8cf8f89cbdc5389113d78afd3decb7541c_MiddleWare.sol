...
contract ChickMining is OwnableContract, ReentrancyGuard, IERC721Receiver{
    ...
    mapping(uint256 => mapping(uint256 => bool)) public nftBlackListPerPool;
    ...
    constructor(address _mining_token, uint256 _mining_start_block) public {
        ...
        phaseEndBlockNumberArray = new uint256[](6);
        setStartBlock(_mining_start_block);

        phasePerBlockRewardArray = new uint256[](6);
        phasePerBlockRewardArray[0] = 192 * 1e16;
        phasePerBlockRewardArray[1] = 96 * 1e16;
        phasePerBlockRewardArray[2] = 48 * 1e16;
        phasePerBlockRewardArray[3] = 24 * 1e16;
        phasePerBlockRewardArray[4] = 12 * 1e16;
        phasePerBlockRewardArray[5] = 6 * 1e16;
    }

    function setStartBlock(uint256 _mining_start_block) internal{
    ...
    function updateStartBlock(uint256 _mining_start_block) public onlyAdmin{
        require(block.number < _mining_start_block && block.number < phase1StartBlockNumber, "_mining_start_block is error.");
        setStartBlock(_mining_start_block);
    }

    function updateBlockReward(uint256 _phaseIndex, uint256 _reward, bool _withUpdate) public onlyOwner {
    ...
    function depositERC20(uint256 _pid, uint256 _amount) public nonReentrant {
    ...
    function depositNFT(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        for(uint256 i=0; i<_amount; i++){
            ...
            require(!nftBlackListPerPool[_pid][tokenId], string(abi.encodePacked("The tokenId ", tokenId.toString(), " is in the blacklist!")));
    ...
    function updateNftBlackList(uint256 poolId, uint256[] memory tokenIds, bool isProblemToken) public onlyAdmin{
        for(uint256 i=0; i<tokenIds.length; i++){
            nftBlackListPerPool[poolId][tokenIds[i]] = isProblemToken;
        }
    }
}
    ...
    function getWalletAndMining(address userAddress) public view returns(address[] memory, uint256[] memory, uint256[] memory){
        ...
        for(uint256 i=0; i<5; i++){
            ...
            if(i == 4){
                ...
                (uint256 amount,,) = chickMining.getUserInfo(poolIdArray[i], _userAddress);
