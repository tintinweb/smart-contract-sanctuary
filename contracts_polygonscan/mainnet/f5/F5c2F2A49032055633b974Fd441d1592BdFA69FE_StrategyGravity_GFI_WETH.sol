    ...
    */
    function _liquidateReward() internal {
        ...
        if(routes[token0].length > 1) {
            IERC20(rewardToken()).safeApprove(quickswapRouter, 0);
            IERC20(rewardToken()).safeApprove(quickswapRouter, toToken0);
            uint256[] memory amounts = IUniswapV2Router02(quickswapRouter).swapExactTokensForTokens(toToken0, 0, routes[token0], address(this), block.timestamp.add(600));
        ...
        if(routes[token1].length > 1) {
            IERC20(rewardToken()).safeApprove(quickswapRouter, 0);
            IERC20(rewardToken()).safeApprove(quickswapRouter, toToken1);
            uint256[] memory amounts = IUniswapV2Router02(quickswapRouter).swapExactTokensForTokens(toToken1, 0, routes[token1], address(this), block.timestamp.add(600));
        ...
        IERC20(token0).safeApprove(quickswapRouter, token0Amount);

        IERC20(token1).safeApprove(quickswapRouter, 0);
        IERC20(token1).safeApprove(quickswapRouter, token1Amount);

        IUniswapV2Router02(quickswapRouter).addLiquidity(token0, token1, token0Amount, token1Amount, 0, 0, address(this), block.timestamp.add(600));
    }
...
// File contracts/strategies/gravity/mainnet/StrategyGravity_GFI_WETH.sol
...
contract StrategyGravity_GFI_WETH is GravityLPStrategy {
    ...
    function initializeStrategy(
        ...
        address farmContract = address(0xA85587945C97359d64d94d141BF1D95C215D1626);
        address gfi_weth_lp = address(0x1587663E8F475E69eA2DBb38482C8c4EE9F388Fb);
