...
import "./interfaces/ISushiFarm.sol";
...
abstract contract StratX2 is Ownable, ReentrancyGuard, Pausable {
    ...
    function _farm() internal virtual {
        ...
        if (isCAKEStaking) {
            ...
            // ISushiFarm(farmContractAddress).deposit(pid, wantAmt);
            ISushiFarm(farmContractAddress).deposit(
                pid,
                wantAmt,
                address(this)
            );
    ...
    function _unfarm(uint256 _wantAmt) internal virtual {
        if (isCAKEStaking) {
            ...
            ISushiFarm(farmContractAddress).withdrawAndHarvest(
                pid,
                _wantAmt,
                address(this)
            );
    ...
    function earn() public virtual nonReentrant whenNotPaused {
        ...
        // send 0.8% of earned amount to an address
        earnedAmt = distributeFees(earnedAmt);

        // buy back 3 percent and send to burn
        ...
        returns (uint256)
    {
        if (_earnedAmt > 0) {
            // Performance fee
            if (TENGas_Fee > 0) {
                uint256 fee = _earnedAmt.mul(TENGas_Fee).div(TENGas_FeeMax);
...
contract TENFIStrategy_Sushi is StratX2 {
    constructor(
