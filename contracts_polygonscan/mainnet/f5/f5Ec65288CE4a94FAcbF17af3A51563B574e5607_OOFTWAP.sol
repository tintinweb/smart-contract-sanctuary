...
contract OOFTWAP {
    ...
    function getTWAP(IOpenOracleFramework OOFContract, uint256[] memory feedIDs, uint256[] memory timestampstart, uint256[] memory timestampfinish) external view returns (uint256[] memory TWAP) {
            ...
            for (uint c = 0; c < feedLen; c++) {
                //require(OOFContract.getHistoricalFeeds(feedIDs[c], timestampstart[c]) != (0,) && OOFContract.getHistoricalFeeds(feedIDs[c],timestampfinish[c]) != (0,));
                uint256 twapc = timestampfinish[c] / timeslot[c] - timestampstart[c] / timeslot[c] + 1;
                uint256[] memory feedIDc = new uint256[](twapc);
                uint256[] memory totalc = new uint256[](twapc);

                uint256[] memory totals = new uint256[](twapc);

                for (uint s = 0; s < twapc; s++) {
                    totalc[s] = timestampstart[c] + s * timeslot[c];
                    feedIDc[s] = feedIDs[c];
                }

                totals = OOFContract.getHistoricalFeeds(feedIDc, totalc);

                for (uint t = 0; t < totals.length; t++){
                    totalt += totals[t];
                }

                TWAP[c] = totalt / totals.length;
            }
