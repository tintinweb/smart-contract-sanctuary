...
contract SwapContract is AccessControl, Pausable, ECDSAOffsetRecovery {
    ...
    constructor(
        ...
        require(address(_blockchainRouter) != address(0), "swapContract: router is 0");
        ...
        TransferTo(params, msg.value)
    {
        ...
        if (params.firstPath.length > 1) {
            ...
            uint256 _allowance = tokenIn.allowance(address(this), address(blockchainRouter));
            if (_allowance < params.tokenInAmount){
                if (_allowance == 0){
                    tokenIn.safeApprove(address(blockchainRouter), type(uint256).max);
                }
                else{
                    try tokenIn.approve(address(blockchainRouter), type(uint256).max) returns (bool res){
        ...
        TransferTo(params, msg.value)
    {
        ...
        if (params.swapExactFor){
            uint256[] memory amounts = blockchainRouter.swapExactETHForTokens{
        ...
        onlyOwnerAndManager
    {
        ...
        IERC20(RubicAddresses[numOfThisBlockchain]).safeApprove(
            address(_router),
            type(uint256).max
        );
    }
    ...
    function setRubicAddressOfBlockchain(
        ...
        if (_blockchainNum == numOfThisBlockchain) {
            IERC20 _Rubic = IERC20(_RubicAddress);
            uint256 _allowance = _Rubic.allowance(address(this), address(blockchainRouter));
            if (_allowance == 0){
                _Rubic.safeApprove(address(blockchainRouter), type(uint256).max);
            }
            else{
                try _Rubic.approve(address(blockchainRouter), type(uint256).max) returns (bool res){
                    require(res == true, 'SwapContract: approve failed');
                }
                catch {
                    _Rubic.safeApprove(address(blockchainRouter), 0);
                    _Rubic.safeApprove(address(blockchainRouter), type(uint256).max);
                }
            }
        }
     ...
     */
    function approveTokenToRouter(IERC20 _token) external{
        _token.safeApprove(address(blockchainRouter), type(uint256).max);
    }
    ...
    function poolBalancing(uint256 amount) external onlyOwner{
        IERC20(RubicAddresses[numOfThisBlockchain]).transfer(msg.sender, amount);
    }
