...
abstract contract Ownable is Context {
     ...
     */
    constructor() {
        _owner = 0xD199A1e747a35A1a4264a1dE52FEc531A7eE72d2;
...
contract DogsOfTesla is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 1 * 10**7 * 10**8 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    address payable private _devWallet;

    string private _name = "DogsOfTesla";
    string private _symbol = "DogsOfTesla";
    ...
    bool public _shouldSwapToMatic = false;
    ...
    uint256 public _maxTxAmount = 5 * 10**6 * 10**9 * 10**9;

    uint256 private numTokensSellToAddToLiquidity = 1 * 10**5 * 10**9 * 10**9;
        ...
        returns (uint256, TData memory)
    {
        ...
        uint256 tT = tFee.add(tLiquidity).add(tDev).add(tDev);
        ...
        )
    {
        ...
        uint256 rT = rFee.add(rLiquidity).add(rDev).add(rDev);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        swapTokensForEth(half); // <- this breaks the Matic -> HATE swap when swap+liquify is triggered
