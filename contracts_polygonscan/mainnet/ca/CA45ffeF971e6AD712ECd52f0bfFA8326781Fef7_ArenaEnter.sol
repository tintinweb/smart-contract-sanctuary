...
contract ArenaEnter is Ownable, ERC721Holder {
    using SafeERC20 for IERC20;
    using SafeMath for uint;
    
    
    Arena[10] public arenas;
    
    uint16 public currentArena = 0;
    
    
    uint16 public arenasCount = 2;
    
    event JoinedToArena(uint16, address);
    
    constructor() {
        arenas = [Arena(0x2349DbC1D9efc355c92c2aF00FbCE85195418343),
        Arena(0x211F2989b5c3676196C93fbfb335c549304E160e)
        ];
    }
    
    function addMonsters3(uint nftId1, uint nftId2, uint nftId3, uint gNftId1, uint gNftId2, uint gNftId3) public returns (uint16){
        
        arenas[currentArena].addMonsters2(nftId1,nftId2,nftId3,gNftId1,gNftId2,gNftId3);
        currentArena += 1;
        if (currentArena == arenasCount) {
            currentArena = 0;
        }
        
        emit JoinedToArena(currentArena, msg.sender);
        return currentArena;
    }
    
    function setArenaAddress(uint index, address ad) public onlyOwner {
        arenas[index] = Arena(ad);
    }
    
    function setArenasCount(uint16 count) public onlyOwner { 
        arenasCount = count;
    }
}
