...
// The DAIrymalock leverages using openzeppelin timelock for maximum safety. Its also DAIryman and haunted.
// To see openzepplin's audits goto: https://github.com/OpenZeppelin/openzeppelin-contracts/tree/master/audit
contract Timelock {
    using SafeERC20 for IERC20;

    IERC20 public DAIryman;

    TokenTimelock[] public Locks;

    constructor (IERC20 _DAIryman) {
    ...
    function forwardTokens() external {
        ...
        uint boos = DAIryman.balanceOf(address(this));

        require(boos > 0, "forwardTokens: no boos!");

        for (uint256 index = 0; index <= 3; index++) {
            DAIryman.transfer(address(Locks[index]), boos / 4);
        }

        // just incase theres any boos left from rounding
        uint leftover = DAIryman.balanceOf(address(this));

        if (leftover > 0) {
            DAIryman.transfer(address(Locks[3]), leftover);
        }
