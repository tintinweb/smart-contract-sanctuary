...
interface IMasterChef {
    ...
    function ace() external view returns (address);

    function acePerBlock() external view returns (uint256);
    ...
    function pendingAce(uint256 nr, address who) external view returns (uint256);
}
...
contract BoringHelperV1 is Ownable {
    ...
    address public maker; // IAceMaker(0xE11fc0B43ab98Eb91e9836129d1ee7c3Bc95df50);
    IERC20 public ace; // IAceToken(0x6B3595068778DD592e39A122f4f5a5cF09C90fE2);
    ...
    IFactory public aceFactory; // IFactory(0xC0AEe478e3658e2610c5F7A4A2E1777cE9e4f2Ac);
    ...
    function getETHRate(IERC20 token) public view returns (uint256) {
        ...
        if (aceFactory != IFactory(0)) {
            pairAce = IPair(aceFactory.getPair(token, WETH));
        }
        if (address(pairUniV2) == address(0) && address(pairAce) == address(0)) {
            return 0;
        }
        ...
        if (address(pairAce) != address(0)) {
            (uint112 reserve0Ace, uint112 reserve1Ace, ) = pairAce.getReserves();
            reserve0 += reserve0Ace;
            reserve1 += reserve1Ace;
            if (token0 == IERC20(0)) {
                token0 = pairAce.token0();
            }
        }
    ...
    struct UIInfo {
        uint256 ethBalance;
        uint256 aceBalance;
        uint256 aceBarBalance;
        uint256 xaceBalance;
        uint256 xaceSupply;
        uint256 aceBarAllowance;
        Factory[] factories;
        uint256 ethRate;
        uint256 aceRate;
        uint256 btcRate;
    ...
    function getUIInfo(
        ...
        if (ace != IERC20(0)) {
            info.aceRate = getETHRate(ace);
            info.aceBalance = ace.balanceOf(who);
            info.aceBarBalance = ace.balanceOf(address(bar));
            info.aceBarAllowance = ace.allowance(who, address(bar));
        }

        if (bar != IERC20(0)) {
            info.xaceBalance = bar.balanceOf(who);
            info.xaceSupply = bar.totalSupply();
        }
        ...
        if (chef != IMasterChef(0)) {
            ...
            for (uint256 i = 0; i < poolLength; i++) {
                pendingAce += chef.pendingAce(i, who);
            }
            info.pendingAce = pendingAce;
        }
    ...
    function pollPools(address who, uint256[] calldata pids) public view returns (UserPoolInfo[] memory) {
        ...
        for (uint256 i = 0; i < pids.length; i++) {
            ...
            pools[i].pending = chef.pendingAce(pids[i], who);
