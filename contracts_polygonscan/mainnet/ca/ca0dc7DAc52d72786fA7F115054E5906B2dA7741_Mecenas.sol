...
contract Mecenas is ERC20m, ReentrancyGuard {
    ...
    mapping(address => uint) internal indexplayers;
    ...
    constructor(address _marketcream, address _underlying, uint _lockdeposits) ERC20m("Mecenas Matic Token 1", "MMT1") {
    ...
    // this function is used to lock or unlock functions deposit() and depositdonation() - true is unlock
    // false is lock 
    ...
    function deposit(uint _amount) external nonReentrant {
        ...
        underlying.approve(address(marketcream), _amount);
    ...
    function withdrawinterest(uint _amount) external nonReentrant onlyowner {
        ...
        underlying.transfer(owner, totransferinterestnet);
    ...
    function pickawinner() external nonReentrant {
        ...
        while(w < players.length || winner < 1 ) {
            ...
            uint randomnumber = uint(keccak256(abi.encode(blockhash(blockNumber), nonce))) % players.length;
        ...
        if (winner == 1) {
            ...
            underlying.transfer(beneficiary, totransferbeneficiary);
            underlying.transfer(developer, totransferdevelop);
            underlying.transfer(seeker, totransferseeker);
    ...
    function withdrawdonations(uint _amount) external nonReentrant onlyowner {
        ...
        underlying.transfer(owner, _amount);
