...
contract Stake {
    ...
    //creator to protect contract
    address private creatorw;
    ...
    function addSaleOne(uint256 value, address buyer) external onlyOwner() {
        ...
        require(value > 0 && buyer != address(0), "data incorrect");
        nSales++;
    ...
    function addReward(uint256 rw) internal{
        ...
        uint256 rewardPercentage = rw.mul(4125).div(10000).div(nS);
        uint256 rewardPercentageN = rw.mul(250).div(10000).div(nS);
        balance[msg.sender] = balance[msg.sender].add(rewardPercentageN);
        allTimeGains[msg.sender] = allTimeGains[msg.sender].add(rewardPercentageN);
    ...
    function claim() public{
        ...
        require(money > 0, "This contract does not own you money");
    ...
    function ChangeOwner(address ioa) public onlyOwner(){
        owner = ioa;
    }
    ...
    function getEthContract() public view returns(IERC20){
        return weth;
    }
    
    function loCker() public view returns(bool){
        return lock;
    }
    
    function getSale(uint256 index) public view returns(Sale memory){
        return orderBuyers[index];
    }
}