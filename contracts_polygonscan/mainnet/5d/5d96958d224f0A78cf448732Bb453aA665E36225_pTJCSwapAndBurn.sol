...
contract pTJCSwapAndBurn {
  ...
  receive() payable external {
    ...
    uint256 amountIn = fullBalance.mul(7).div(10);
    uint256 amountOut = getAmountOut(amountIn,resB,resA);
    _approveTokenIfNeeded(address(router.WETH()),fullBalance);
    uint256[] memory swapedAmounts = router.swapExactTokensForTokens(amountIn,amountOut.mul(95).div(100), path, address(this), block.timestamp.add(10 minutes));
    token.transfer(address(0x000000000000000000000000000000000000dEaD),swapedAmounts[1].div(2));
    uint256 amountDesired = getAmountOut(fullBalance.sub(swapedAmounts[0]),resA.sub(swapedAmounts[0]),resB.add(swapedAmounts[0]));
    require(token.balanceOf(address(this)) >= amountDesired,"Insufficient tokens in the contract");
    _approveTokenIfNeeded(address(token),amountDesired);
    router.addLiquidity(path[0],
                        path[1],
                        fullBalance.sub(swapedAmounts[0]),
                        amountDesired,
                        1,
                        1,
                        safe,
                        block.timestamp.add(10 minutes));
    IERC20(router.WETH()).transfer(safe,IERC20(router.WETH()).balanceOf(address(this)));
