...
interface IBoleroArtistToken {
    ...
    function transferFromICO(address _from, address _to, uint256 _value) external;
...
contract BoleroArtistICO {
    ...
    *******************************************************************************/
    constructor(address _bolero, address _boleroArtist, address _icoWantToken, uint256[2] memory _shares, uint256 _initialPricePerShare) {
        bolero = _bolero;
    ...
    *******************************************************************************/
    function grantTokens(address[] memory recipients, uint256[] memory values) external onlyBoleroOrManagement() {
        for (uint256 i = 0; i < recipients.length; i++)
            boleroToken.transferFromICO(address(this), recipients[i], values[i]);
    }
    ...
    *******************************************************************************/
    function claimTreasure() public {
        ...
        if (boleroArtist.isWithPaymentSplitter()) {
            PaymentSplitterABI(boleroArtist.artistPayment()).releaseToken(address(want));
        }
    }
    ...
    *******************************************************************************/
    function setPricePerShare(uint256 _pricePerShare) public onlyBoleroOrManagement() {
    ...
    *******************************************************************************/
    function setShares(uint256 _shareForBolero, uint256 _shareForArtist) public onlyBoleroOrManagement() {
