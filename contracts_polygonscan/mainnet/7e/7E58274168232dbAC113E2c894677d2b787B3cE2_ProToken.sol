...
contract ProToken is BEP20("PolyPro Token", "Pro") {
    // Burn address
...
// MasterChef is the master of Pro. He can make Pro and he is a fair guy.
//
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    // Initial emission rate: 0.0025 Pro per block.
    uint256 public constant INITIAL_EMISSION_RATE = 0.0025 ether;
    // Minimum emission rate: 0.00025 Pro per block.
    ...
    // Pro referral contract address.
    IProReferral public ProReferral;
    ...
    constructor(ProToken _Pro, uint256 _startBlock) public {
        Pro = _Pro;
        startBlock = _startBlock;

        devAddress = msg.sender;
        feeAddress = msg.sender;
        ProPerBlock = INITIAL_EMISSION_RATE;
    }
    ...
    // Update the given pool's Pro allocation point and deposit fee. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        uint16 _depositFeeBP,
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 ProReward =
                multiplier.mul(ProPerBlock).mul(pool.allocPoint).div(
                    totalAllocPoint
                );
        ...
        return user.amount.mul(accProPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        Pro.mint(devAddress, ProReward.div(10));
    ...
    function deposit(
        ...
        if (
            _amount > 0 &&
            address(ProReferral) != address(0) &&
            _referrer != address(0) &&
            _referrer != msg.sender
        ) {
            ProReferral.recordReferral(msg.sender, _referrer);
        }
        if (user.amount > 0) {
            uint256 pending =
                user.amount.mul(pool.accProPerShare).div(1e12).sub(
                    user.rewardDebt
                );
            if (pending > 0) {
        ...
        if (_amount > 0) {
            ...
            if (address(pool.lpToken) == address(Pro)) {
    ...
    function safeProTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > ProBal) {
            transferSuccess = Pro.transfer(_to, ProBal);
        } else {
            transferSuccess = Pro.transfer(_to, _amount);
        }
        require(transferSuccess, "safeProTransfer: Transfer failed");
    }
    ...
    function updateEmissionRate() public {
        ...
        require(
            ProPerBlock > MINIMUM_EMISSION_RATE,
        ...
        uint256 newEmissionRate = ProPerBlock;
        for (
        ...
        newEmissionRate = newEmissionRate < MINIMUM_EMISSION_RATE
            ...
            : newEmissionRate;
        if (newEmissionRate >= ProPerBlock) {
            return;
        }

        massUpdatePools();
        lastReductionPeriodIndex = currentIndex;
        uint256 previousEmissionRate = ProPerBlock;
        ProPerBlock = newEmissionRate;
    ...
    // Update the Pro referral contract address by the owner
    function setProReferral(IProReferral _ProReferral) public onlyOwner {
        ProReferral = _ProReferral;
    }
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (
            address(ProReferral) != address(0) && referralCommissionRate > 0
        ) {
            address referrer = ProReferral.getReferrer(_user);
            uint256 commissionAmount =
                _pending.mul(referralCommissionRate).div(10000);

            if (referrer != address(0) && commissionAmount > 0) {
                Pro.mint(referrer, commissionAmount);
