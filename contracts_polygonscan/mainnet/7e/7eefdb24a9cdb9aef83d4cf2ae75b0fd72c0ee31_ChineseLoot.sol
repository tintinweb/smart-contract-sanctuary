...
contract ChineseLoot is ERC721("Chinese Loot", "CLOOT"), ReentrancyGuard, Ownable {
    uint256 public maxSupply = 8000;
    mapping (address=>bool) private claimed;
    uint256 public allowedNum = 888;
    ...
    function setAllowedNum(uint256 num) public  onlyOwner{
        require(num<=maxSupply,"Maximum quantity exceeded");
        allowedNum=num;
    }
 ...
 function setSupplyNum(uint256 num) public  onlyOwner{
        maxSupply=num;
    }
    ...
    function pluck(
        ...
        if (greatness >= 15) {
            ...
            if (greatness == 15) {
                if(rand % 2==0){
                    output = string(
                    abi.encodePacked('"', name[0], '" ', output)
                );
                }else{
                    output = string(
                    abi.encodePacked('"', name[1], '" ', output)
                );
                }
        ...
        if (greatness > 19) {
            output = string(
                abi.encodePacked("", "", suffixes[rand % suffixes.length])
            );
        }
        return output;
    }
        ...
        returns (string memory)
    {
        ...
        parts[
            0
        ] = '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350"><style>.base { fill: white; font-family: FZYaoTi-M06S; font-size: 23px; }</style><rect width="100%" height="100%" fill="black" /><text x="10" y="40" class="base">';
    ...
    function claim(uint256 tokenId) payable public nonReentrant onlyEOA {
        require(tokenId > 0 && tokenId < allowedNum+1, "Token ID invalid");
        require(msg.value>=10 ether,"BNB amount is not");
    ...
    function ownerClaim(uint256 tokenId) public nonReentrant onlyOwner {
        require(tokenId > allowedNum && tokenId < maxSupply, "Token ID invalid");
