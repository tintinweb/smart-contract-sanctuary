...
contract DumplingLevelHandler is Base, Random, ILevelHandler {

    constructor(address _constants) Base(_constants) {}

    function checkLevelUp(uint _id, uint _newExperience) public override onlyAllowedContracts {
        address DumplingsContract = IConstants(constantsContract).DumplingsContract();
        if (_newExperience >= IDumplings(DumplingsContract).getDumplingByIndex(_id).experienceForNextLevel) {
            levelUp(_id);
        }
    }
    ...
    function levelUp(uint _id) internal {
        ...
        dumpling.level = dumpling.level + 1;
        dumpling.currentExperience = dumpling.currentExperience - dumpling.experienceForNextLevel;
        dumpling.experienceForNextLevel = (dumpling.experienceForNextLevel * 11) / 10;
        dumpling.hitPoints = dumpling.hitPoints + randModulus(10, dumpling.level);
        dumpling.basicDmg = dumpling.basicDmg + randModulus(3, dumpling.hitPoints);
        dumpling.stamina = dumpling.stamina + randModulus(3, dumpling.basicDmg);
        dumpling.attackPower = dumpling.attackPower + randModulus(3, dumpling.stamina);
        dumpling.defense = dumpling.defense + randModulus(3, dumpling.attackPower);
        dumpling.initiative = dumpling.initiative + randModulus(3, dumpling.defense);
        dumpling.agility = dumpling.agility + randModulus(3, dumpling.initiative);
        if (dumpling.level % 10 == 0 && dumpling.luck < 5) {
            dumpling.luck = dumpling.luck + 1;
        }
        IDumplings(DumplingsContract).executeLevelUp(dumpling.id,
            [dumpling.level, dumpling.currentExperience,
            dumpling.experienceForNextLevel, dumpling.hitPoints, dumpling.basicDmg, dumpling.stamina,
            dumpling.attackPower, dumpling.defense, dumpling.initiative, dumpling.agility, dumpling.luck]);
    }
}
