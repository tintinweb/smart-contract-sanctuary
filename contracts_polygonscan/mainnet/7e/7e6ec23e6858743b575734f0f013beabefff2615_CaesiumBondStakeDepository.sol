...
// File: contracts/interfaces/IsCAES.sol


pragma solidity >=0.7.5;


interface IsCAES is IERC20 {
    function rebase( uint256 caesProfit_, uint epoch_) external returns (uint256);
...
// File: contracts/CaesiumBondStakeDepository.sol
...
contract CaesiumBondStakeDepository is Ownable {
    ...
    address public immutable CAES; // intermediate reward token from treasury
    address public immutable sCAES; // token given as payment for bond
    ...
    struct Bond {
        uint256 payout; //caes at the moment of bond
        uint256 vesting; // Blocks left to vest
        uint256 lastTimestamp; // Last interaction
        uint256 pricePaid; // In DAI, for front end viewing
        uint256 gonsPayout; // sCAES gons remaining to be paid
    }
    ...
    function deposit(
        ...
        uint256 value = ICaesiumTreasury(treasury).valueOfToken(
            principle,
            _amount
        );
        ...
        IERC20(CAES).approve(staking, payout);
        ICaesiumStaking(staking).stake(payout, address(this));
        ICaesiumStaking(staking).claim(address(this));

        uint256 stakeGons = IsCAES(sCAES).gonsForBalance(payout);
        ...
        returns (uint256)
    {
        ...
        uint256 _amount = IsCAES(sCAES).balanceForGons(info.gonsPayout);
        emit BondRedeemed(_recipient, _amount, 0); // emit bond data
        IERC20(sCAES).transfer(_recipient, _amount); // pay user everything due
        return _amount;
    }
    ...
    function bondPriceInUSD() public view returns (uint256 price_) {
        if (isLiquidityBond) {
            price_ = bondPrice()
                .mul(
                    ICaesiumBondingCalculator(bondCalculator).markdown(principle)
                )
                .div(100);
        } else {
    ...
    function debtRatio() public view returns (uint256 debtRatio_) {
        uint256 supply = IERC20(CAES).totalSupply();
    ...
    function standardizedDebtRatio() external view returns (uint256) {
        if (isLiquidityBond) {
            return
                debtRatio()
                    .mul(
                        ICaesiumBondingCalculator(bondCalculator).markdown(
                            principle
                        )
                    )
                    .div(1e9);
        ...
        returns (uint256 pendingPayout_)
    {
        ...
        uint256 payout = IsCAES(sCAES).balanceForGons(
