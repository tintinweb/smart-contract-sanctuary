...
contract EvohFixedMint is EvohERC721 {
    ...
    constructor(
        ...
        EvohERC721(_name, _symbol, _contractURI)
    {
        for (uint256 i = 0; i < _tokenURIs.length; i++) {
            tokenURIs[i] = _tokenURIs[i];
            addOwnership(msg.sender, i);
            emit Transfer(address(0), msg.sender, i);
        }
        totalSupply = _tokenURIs.length;
    }

    function mint(address _account, string calldata _tokenURI) external {
        require(msg.sender == owner);
        uint256 id = totalSupply;
        tokenURIs[id] = _tokenURI;
        addOwnership(_account, id);
        emit Transfer(address(0), _account, id);
        totalSupply = id + 1;
    }
