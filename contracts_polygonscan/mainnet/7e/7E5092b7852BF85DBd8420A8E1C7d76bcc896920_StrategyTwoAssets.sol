...
abstract contract StrategyFarmTwoAssets is BaseStrategyMasterChef {
    ...
    address public constant crystal = 0x76bF0C28e604CC3fE9967c83b3C3F31c213cfE64;
    address public constant xusd = 0x3A3e7650f8B9f667dA98F236010fBf44Ee4B2975;
    ...
    // Other constants
    uint16 public constant underlyingPoolId = 46;
    uint256 public keepFXS = 75;
    uint256 public harvestCutoff = 10**12;
    uint256 public constant keepFXSmax = 10000;

    // Uniswap swap paths
    address[] public crystal_usdc_path;
    address[] public usdc_xusd_path;
    ...
    {
        crystal_usdc_path = new address[](4);
        crystal_usdc_path[0] = crystal;
        crystal_usdc_path[1] = wmatic;
        crystal_usdc_path[2] = dai;
        crystal_usdc_path[3] = usdc;

        usdc_xusd_path = new address[](2);
        usdc_xusd_path[0] = usdc;
        usdc_xusd_path[1] = xusd;
    ...
    function harvest() public override {
        ...
        //Swap 1/2 crystl for CRYSTL
        uint256 _crystalBalance = IERC20(crystal).balanceOf(address(this));
        if (_crystalBalance > harvestCutoff){
            _swapUniswapWithPath(crystal_usdc_path, _crystalBalance);
            
            //Send performance fee to strategist
            uint256 _usdcFee = IERC20(usdc).balanceOf(address(this));
            if (_usdcFee > 0) {
                uint256 performanceFee = _usdcFee.mul(keepFXS).div(keepFXSmax);
                if (performanceFee>0) {
            ...
            _swapUniswapWithPath(usdc_xusd_path, _usdcBalance.div(2));

            // Add liquidity for crystl/MATIC
            uint256 _xusd = IERC20(xusd).balanceOf(address(this));
            uint256 _usdc = IERC20(usdc).balanceOf(address(this));
            if (_xusd > 0 && _usdc > 0) {
                IERC20(xusd).safeApprove(currentRouter, 0);
                IERC20(xusd).safeApprove(currentRouter, _xusd);
...
contract StrategyTwoAssets is StrategyFarmTwoAssets {
    // Token addresses
    address public CRYSTAL_MASTER_CHEF = 0xeBCC84D2A73f0c9E23066089C6C24F4629Ef1e6d ;
    address public USDC_XUSD_LP = 0x9d5373d95E95ad9E47800f7959D4d8c195c35Df1;
