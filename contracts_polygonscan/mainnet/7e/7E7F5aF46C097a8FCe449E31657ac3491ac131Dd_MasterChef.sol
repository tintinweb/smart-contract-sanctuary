...
contract MasterChef is ReentrancyGuard, Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 accSquidPerPower;   // Accumulated Shs per share, times 1e12. See below.
    ...
    uint256 public squidPerBlock = 1e18;
    // Deposit Fee address
    ...
    function updateEmissionRate(uint256 _squidPerBlock) external onlyOwner {
    ...
    function _updatePool(uint256 _pid) internal {
        ...
        uint256 squidReward = blockAmount * squidPerBlock * pool.allocPoint / totalAllocPoint;
    ...
    function claim(uint256 _pid) external nonReentrant {
        ...
        uint256 pending = user.rewardKept + _pendingSquid(_pid, msg.sender);
    ...
    function pendingSquid(uint256 _pid, address _user) internal view returns (uint) {
        UserInfo storage user = userInfo[_pid][_user];
        
        return user.rewardKept + _pendingSquid(_pid, msg.sender);
    }
    // DO NOT includes kept reward
    function _pendingSquid(uint256 _pid, address _user) internal view returns (uint) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accShPerPower = pool.accSquidPerPower;

        if (block.number > pool.lastRewardBlock && pool.amount != 0 && totalAllocPoint > 0) {
            uint256 blockAmount = block.number - pool.lastRewardBlock;
            uint256 squidReward = blockAmount * squidPerBlock * pool.allocPoint / totalAllocPoint;
            accShPerPower += squidReward * 1e12 / pool.amount;
        }

        return user.amount * accShPerPower / 1e12 - user.rewardDebt;
    }

    // Safe sh transfer function, just in case if rounding error causes pool to not have enough Squids.
