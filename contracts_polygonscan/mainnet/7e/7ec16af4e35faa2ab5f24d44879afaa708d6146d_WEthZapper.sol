...
contract CamZapper is Ownable, Pausable {
    ...
    function camZapToVault(uint256 amount, uint256 vaultId) public whenNotPaused returns (uint256) {
        ...
        try camTokenVault.tokenOfOwnerByIndex(msg.sender, vaultId){
            ...
            if(vaultId == 0){
                vaultId = camTokenVault.createVault();
                camToken.approve(address(camTokenVault), camTokenBal);
                camTokenVault.depositCollateral(vaultId, camTokenBal);
