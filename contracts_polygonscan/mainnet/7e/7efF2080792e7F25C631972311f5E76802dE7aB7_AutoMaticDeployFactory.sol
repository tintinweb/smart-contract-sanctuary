...
contract AutoMaticDeployFactory is Ownable {
    ...
    function setStatus(uint _index) external onlyOwner {
        ...
        mati.call(abi.encodeWithSignature("setMinter(address, bool)", pools[_index].vault, false));
    }
    ...
    function deployPool(address _lp, address _rewardPool, uint _flag, string memory _name) external onlyOwner {
        ...
        if (_flag == 0 || _flag == 1) {
            strategy.call(abi.encodeWithSignature("setVault(address)", vault));
            strategy.call(abi.encodeWithSignature("transferOwnership(address)", msg.sender));
        }

        mati.call(abi.encodeWithSignature("setMinter(address, bool)", vault, true));
