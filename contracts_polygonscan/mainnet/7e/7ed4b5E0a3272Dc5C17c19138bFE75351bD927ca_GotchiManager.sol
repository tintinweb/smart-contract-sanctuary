...
contract GotchiManager is IERC173, Initializable, IERC721ReceiverUpgradeable, IERC1155ReceiverUpgradeable, ERC20Upgradeable {
    ...
    // Locks ghst and mints our bGHST (shares)
    function enter(uint256 _amount) public {        
        uint256 totalTokenLocked = IERC20(im_ghstAddress).balanceOf(address(this));

        uint256 totalShares = totalSupply(); // Gets the amount of bGHST in existence

        // Lock the Token in the contract
        IERC20(im_ghstAddress).transferFrom(msg.sender, address(this), _amount);
        
        if (totalShares == 0 || totalTokenLocked == 0) { // If no bGHST exists, mint it 1:1 to the amount put in
                _mint(msg.sender, _amount);
        } else {
            uint256 bGHSTAmount = _amount.mul(totalShares).div(totalTokenLocked);
            _mint(msg.sender, bGHSTAmount);
        }
    }

    // claim ghst by burning bGHST
    function leave(uint256 _share) public {
        if(_share>0){
            ...
            // Now we withdraw the GHST from the bGHST Pool (this contract) and send to user as GHST.
