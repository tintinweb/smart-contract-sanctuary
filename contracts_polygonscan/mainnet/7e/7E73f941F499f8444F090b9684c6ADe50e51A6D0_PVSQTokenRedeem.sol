...
contract PVSQTokenRedeem is Ownable, ReentrancyGuard {
    ...
    bool public emergencyClose = false;
    ...
    event VSQRecoveredAndContractClosed(uint256 VSQRecovered);
    ...
    function redeemAvailableVSQ() external nonReentrant {
        require(!emergencyClose, "The PVSQ redeem contract service has been deactivated temporarily");
    ...
    // Recover all VSQ in case of error and close contract.
    function recoverVSQToDAOAndClose() external onlyOwner {
        uint256 vsqBalance = ERC20(VSQAddress).balanceOf(address(this));

        if (vsqBalance > 0)
            ERC20(VSQAddress).safeTransfer(DAO, vsqBalance);

        emergencyClose = true;
        
        emit VSQRecoveredAndContractClosed(vsqBalance);
    }
