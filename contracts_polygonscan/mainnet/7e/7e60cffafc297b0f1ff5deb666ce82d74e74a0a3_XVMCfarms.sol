...
contract XVMCfarms is Ownable {
    ...
    uint256 allocationPool0 = 2000; //current allocation for farms
    uint256 allocationPool1 = 2000;
    ...
    */    
    function initiateFarmProposal(
    	...
    	require(poolid == 0 || poolid == 1 || poolid == 11, "only allowed for these pools"); 
    ...
    */
    function updateFarm(uint256 proposalID) external whenReady {
        require(proposalFarmUpdate[proposalID].poolid == 0 || proposalFarmUpdate[proposalID].poolid == 1, "invalid pool ID");
		...
		if(proposalFarmUpdate[proposalID].poolid == 0) {
			require(
				proposalFarmUpdate[proposalID].newAllocation + allocationPool1 
				<= (IMasterChef(masterchef).totalAllocPoint() * 5 / 100),
				"maximum total reward for farms"
				);
			allocationPool0 = proposalFarmUpdate[proposalID].newAllocation;
		} else {
			require(
				proposalFarmUpdate[proposalID].newAllocation + allocationPool0 
				<= (IMasterChef(masterchef).totalAllocPoint() * 5 / 100),
				"maximum total reward for farms"
				);
			allocationPool1 = proposalFarmUpdate[proposalID].newAllocation;
		}
        
        IXVMCgovernor(owner()).setPool(proposalFarmUpdate[proposalID].poolid, proposalFarmUpdate[proposalID].newAllocation, 0, true);
        proposalFarmUpdate[proposalID].valid = false;
        
        emit UpdateFarm(proposalID, proposalFarmUpdate[proposalID].poolid, msg.sender);
    }
    function updateFarm11(uint256 proposalID) external whenReady {
        require(proposalFarmUpdate[proposalID].valid, "invalid proposal");
        require(proposalFarmUpdate[proposalID].poolid == 11, "invalid pool ID");
        require(proposalFarmUpdate[proposalID].firstCallTimestamp + IXVMCgovernor(owner()).delayBeforeEnforce() < block.timestamp, "not valid yet");

        IXVMCgovernor(owner()).setPool(11, proposalFarmUpdate[proposalID].newAllocation,
														proposalFarmUpdate[proposalID].newDepositFee, true);
        proposalFarmUpdate[proposalID].valid = false;  
        
        emit UpdateFarm(proposalID, 11, msg.sender);
    }
