...
contract Tan is
    ...
    uint public PRICE_WL         = 2 * 1e16;
    uint public PRICE_PUBLIC     = 3 * 1e16;
        ...
        whenNotPaused()
    {
        ...
        uint mintStartNumber = mintStartNumberForGeneration(nextGeneration) + mintCounter[nextGeneration];

        for (uint mintedTan = 0; mintedTan < numTans; mintedTan++) {
            _mint(
                _for,
                mintStartNumber + mintedTan
            );
        }

        mintCounter[nextGeneration] += numTans;
    }
    ...
    function setMintPrice(uint wl, uint pub) external
        onlyOwner()
    {
        PRICE_WL = wl;
        PRICE_PUBLIC = pub;
    ...
    string private contractMetadataURI = "https://tacticaltangrams.io/metadata/contract_tan.json";
    string private placeholderMetadataURI = "https://tacticaltangrams.io/metadata/placeholder.json";
...
contract PriceFeed {
        ...
        )
    {
        ...
        priceOfTansInMatic = priceUsd / uint(priceMaticUsd);
    }
