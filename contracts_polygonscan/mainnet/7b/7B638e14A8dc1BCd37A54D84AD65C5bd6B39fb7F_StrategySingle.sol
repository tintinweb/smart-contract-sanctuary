...
abstract contract BaseStrategy is Ownable {
    ...
    function getDepositFee() public virtual view returns (uint16);
...
abstract contract BaseStrategyMasterChef is BaseStrategy {
    ...
    function getHarvestable() public override view returns (uint256) {
        return IMasterChef(rewards).pendingBone(poolId, address(this));
    }

    function getDepositFee() public override view returns (uint16) {
        (, , , , uint16 depositFeeBP) = IMasterChef(rewards).poolInfo(poolId);
        return depositFeeBP;
    }
...
abstract contract StrategyFarmSingle is BaseStrategyMasterChef {
    ...
    address public constant rewardToken = 0x6bb45cEAC714c52342Ef73ec663479da35934bf7; //bone address
    ...
    address public constant bone = rewardToken;
    address public constant usdc = 0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174;
    address public constant WMATIC = 0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270; //WMATIC

    address[] public bone_usdc_path;
    address[] public bone_want_path;

    // Other constants
    uint16 public constant underlyingPoolId = 8;

    uint256 public keepFXS = 100;
    uint256 public harvestCutoff = 0;
    ...
    {
        bone_usdc_path = new address[](2);
        bone_usdc_path[0] = bone;
        bone_usdc_path[1] = usdc;

        bone_want_path = new address[](2);
        bone_want_path[0] = bone;
        bone_want_path[1] = WMATIC;
    }
    ...
    function harvest() public override {
        ...
        if (performanceFee>0) {
            _swapUniswapWithPath(bone_usdc_path, performanceFee);
            IERC20(usdc).safeTransfer(strategist, IERC20(usdc).balanceOf(address(this)));
        }

        //Swap rest of rewardToken to want and let it sit in strategy.
        uint256 _afterPerformanceFee = IERC20(rewardToken).balanceOf(address(this));
        _swapUniswapWithPath(bone_want_path, _afterPerformanceFee);        
...
contract StrategySingle is StrategyFarmSingle {
    // Token addresses
    address public masterChef = 0x9DcB2D5e7b5212fAF98e4a152827fd76bD55f68b; //Pup Farms MasterChef
    address public Want_Token = 0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270; //WMATIC
