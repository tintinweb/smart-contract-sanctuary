...
contract FactoryILO is Ownable {   
    //---------- Libraries ----------//
    ...
    function createILO(
      ...
      uint256[13] memory uint_params
        ...
        BaseILO newILO = new BaseILO(address(this), LOCKER);
...
contract BaseILO is ReentrancyGuard {
    //---------- Libraries ----------//
    using SafeMath for uint256;
    using EnumerableSet for EnumerableSet.AddressSet;
    ...
    constructor(address _factory, address _locker) {
        FACTORY = _factory;
        LOCKER = ILiquidityLocker(_locker);
    }
    ...
    function remaining4Sale(uint round_) public view returns(uint256) {
        if(round_ == 1) {
            return (INFO.round1Amount.div(INFO.round1Price) * 1e18).sub(INFO.TOTAL_COLLECTED);
        }
        if(round_ == 2) {
