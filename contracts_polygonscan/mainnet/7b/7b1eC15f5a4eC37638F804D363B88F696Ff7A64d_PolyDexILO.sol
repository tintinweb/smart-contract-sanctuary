...
contract PolyDexILO is ReentrancyGuard, OwnableUpgradeable {
    ...
    uint256 public totalReceivingAmountInUSDC;
    uint256 public totalReceivingAmountInBUSD;

    uint256 public commissionPercent;
    address public rewardReferral;

    uint256 public sellingRate;
    uint256 public receivingRate;
    // address => amount
    ...
    event BuyByBUSD(address indexed user, address indexed referral, uint256 amount);
    ...
    function initialize(
        ...
        uint256 _sellingRate,
        uint256 _receivingRate,
        ...
        totalReceivingAmountInUSDC = 0;
        totalReceivingAmountInBUSD = 0;
        minBuyAmount = _minBuyAmount;
        maxBuyAmount = _maxBuyAmount;
        startTime = _startTime;
        endTime = _endTime;
        sellingRate = _sellingRate;
        receivingRate = _receivingRate;
    }
    ...
    function setRate(uint256 _sellingRate, uint256 _receivingRate) external onlyOwner {
        sellingRate = _sellingRate;
        receivingRate = _receivingRate;
    }
    ...
    function buyWithRef(uint256 _amount, address _referrer) public onlyActive nonReentrant {
        ...
        require(userInfo[msg.sender].amountInUSDC + _amount <= maxBuyAmount, "total amount must <= max");
        require((totalReceivingAmountInUSDC + _amount) * 1e12 + totalReceivingAmountInBUSD <= receivingCap, "overcap");
        ...
        if (userInfo[msg.sender].amountInUSDC == 0) {
            addressList.push(msg.sender);
        }
        userInfo[msg.sender].amountInUSDC += finalDepositAmount;
        totalReceivingAmountInUSDC += finalDepositAmount;
    ...
    function buyByBUSD(
        ...
        require(totalReceivingAmountInUSDC * 1e12 + totalReceivingAmountInBUSD + _amount <= receivingCap, "overcap");
        lockedReceipt[_tx] = true;
        if (rewardReferral != address(0) && _referrer != address(0)) {
            IReferral(rewardReferral).set(_referrer, _userAddress);
        }
        if (userInfo[_userAddress].amountInUSDC == 0 && userInfo[msg.sender].amountInBUSD == 0) {
            addressList.push(msg.sender);
        }
        userInfo[_userAddress].amountInBUSD += _amount;
        totalReceivingAmountInBUSD += _amount;
        emit BuyByBUSD(_userAddress, _referrer, _amount);
    }
    ...
    function claim() external nonReentrant onlyFinished {
        require(userInfo[msg.sender].amountInUSDC > 0 || userInfo[msg.sender].amountInBUSD > 0, "have you participated?");
    ...
    function getUserSellingTokenAmount(address user) public view returns (uint256) {
        ...
        return userInfo[user].amountInUSDC * 1e12 * sellingRate / receivingRate + userInfo[user].amountInBUSD * sellingRate / receivingRate;
    }

    function getMaxUSDCCanPurchase() public view returns (uint256) {
        return (receivingCap - totalReceivingAmountInUSDC * 1e12 - totalReceivingAmountInBUSD) / 1e12;
    }
    ...
    function userTokenStatus(address _user) public view returns (uint256 usdcAmount, uint256 busdAmount, uint256 sellingTokenHarvest) {
        uint256 userOffering = getUserSellingTokenAmount(_user);
        return (userInfo[_user].amountInUSDC, userInfo[_user].amountInBUSD, userOffering);
    }
