...
contract SwapProxy is Ownable {
    ...
    address public immutable WORMHOLE_BRIDGE;


    constructor(address router, address wormholeBridge) {
    ...
    function approveToken (address erc20Address) external onlyOwner {
        IERC20(erc20Address).approve(AGGREGATION_ROUTER_V3, 10 ** 28);

    }
    function swap( bytes calldata _data) external onlyOwner {
        (address _c, SwapDescription memory desc, bytes memory _d) = abi.decode(_data[4:], (address, SwapDescription, bytes));
        
        (bool success, bytes memory _data) = address(AGGREGATION_ROUTER_V3).call(_data);
        if (success) {
    ...
    function completeTransfer(bytes memory encodedVm) public {
        IWormholeBrigde(WORMHOLE_BRIDGE).completeTransfer(encodedVm);
    }
}
