...
contract Converter is IConverter {
    ...
    address public usdc; // usdc is used as default bridge token for all swaps
    address public swapRouter;

    /* ========== CONSTRUCTOR ========== */

    constructor(address _usdc, address _swapRouter) public {
        usdc = _usdc;
    ...
    function swap(address _tokenIn, address _tokenOut, uint _amountIn) public override returns (uint amountOut_) {
    ...
    function addLiquidity(address _tokenA, address _tokenB, uint _amountA, uint _amountB) public override returns (uint lpAmount_) {
    ...
    function removeLiquidity(address _tokenA, address _tokenB, address _lpPair, uint _liquidity) public override returns (uint amountA_, uint amountB_) {
    ...
    function _swap(address _tokenIn, address _tokenOut, uint _amountIn, address _to) internal returns (uint amountOut_) {
        ...
        bool is_usdc = _tokenIn == usdc || _tokenOut == usdc;
        address[] memory path = new address[](is_usdc ? 2 : 3);
        path[0] = _tokenIn;
        if (is_usdc) {
            path[1] = _tokenOut;
        } else {
...
/**
* interface for uniswap style dex.
**/
