...
contract FishStrat is Ownable, Pausable {
    ...
    address[] public rollToWmatic = [roll, wmatic];
     ...
     */
    constructor() public {
        lpPair = 0x905DCc700fcce9a49b7D907E371230995a45ebCE;
        vault = 0x07202D6501a380A7C9d93BEe7E1FA39A23863FB7;

        IERC20(lpPair).safeApprove(masterchef, 0);
        IERC20(lpPair).safeApprove(masterchef, uint(-1));
        IERC20(roll).safeApprove(unirouter, 0);
        IERC20(roll).safeApprove(unirouter, uint(-1));
    ...
    function addliquidity() internal{
        uint amount = IERC20(roll).balanceOf(address(this));
        IPanwexRouter(unirouter).swapExactTokensForTokens(amount.div(2), 0, rollToWmatic, address(this), now + 600);
        IPanwexRouter(unirouter).addLiquidity(roll, wmatic, IERC20(roll).balanceOf(address(this)), IERC20(wmatic).balanceOf(address(this)), 1, 1, address(this), now + 600);
    }
    ...
    function harvest() external whenNotPaused {
        ...
        IMasterChef(masterchef).deposit(pid,0,kk);
        chargeFees();
        addliquidity();
        deposit();
    }
