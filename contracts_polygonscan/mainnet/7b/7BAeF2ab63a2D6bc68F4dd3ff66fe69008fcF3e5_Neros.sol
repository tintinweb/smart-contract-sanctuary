...
contract Neros is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  function addWhitelistedUsers(address[] calldata _addressesToWhitelist) public onlyOwner {
    for (uint i = 0; i < _addressesToWhitelist.length; i++) {
      whitelistedAddresses[_addressesToWhitelist[i]] = true;
    }
  }

  function removeWhitelistedUsers(address[] calldata _addressesToRemove) public onlyOwner {
    for (uint i = 0; i < _addressesToRemove.length; i++) {
      whitelistedAddresses[_addressesToRemove[i]] = false;
    }
  }

  function setOnlyWhitelisted(bool _whitelisted) public onlyOwner {
