...
contract MeowAPI {
  ...
  // Return Sushiswap yield farm APR.
  function getSushiYieldFarmAPR(address[] memory _workers) public view returns (bytes memory) {
  ...
  // Return Quickswap yield farm APR.
  function getQuickYieldFarmAPR(address[] memory _workers) public view returns (bytes memory) {
    uint256 len = _workers.length;
    uint256[] memory yieldFarmAPY = new uint256[](len);
    address[] memory workersList = new address[](len);
    for (uint256 i = 0; i < _workers.length; i++) {
