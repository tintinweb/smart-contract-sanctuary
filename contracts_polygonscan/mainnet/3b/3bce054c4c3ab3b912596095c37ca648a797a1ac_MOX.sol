...
contract MOX is Ownable, ERC721, ERC721Enumerable {
     ...
     * 
     */ 
    constructor(string memory name, string memory symbol, string memory baseTokenURI, address MOXbucksContract, address MOXsharesContract) ERC721(name, symbol) {
        _baseTokenURI = baseTokenURI;
		_owner = _msgSender();
		indexType = 0;
		tokenA = IERC20(MOXbucksContract);
		tokenB = IERC20(MOXsharesContract);
    ...
    function mintNFT(uint32 _type) public payable {
		...
		require(tokenA.balanceOf(msg.sender) >= getPrice(_type), "Not enough tokens");
        
        
        tokenA.transferFrom(msg.sender, address(this), getPrice(_type));
        
        uint toKeep = (getPrice(_type).mul(DIST_PERC)).div(100);
        tokenA.burn(getPrice(_type).sub(toKeep));
