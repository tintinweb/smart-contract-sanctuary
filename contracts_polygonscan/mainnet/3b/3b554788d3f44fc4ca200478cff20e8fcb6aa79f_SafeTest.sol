...
contract SafeTest is Context, IERC20, Ownable {	
    ...
    address public _burnWallet = _msgSender();	
    address public _previousburnWallet = _burnWallet;	
    ...
    address public pancakeswapRouterAddress = 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff;   // it's pointing to Testnet
    ...
    event ExcludedFromburnFee(address);	
    event IncludedInburnFee(address);
    ...
    constructor() public {	
        ...
        //exclude owner and this contract from fee	
        _isExcludedFromburnFee[owner()] = true;	
   ...
   function excludedFromburnFee(address account) external onlyOwner {	
        _isExcludedFromburnFee[account] = true;	
        emit ExcludedFromburnFee(account);	
    }	
    	
    function includeInburnFee(address account) external onlyOwner{	
         require( _isExcludedFromburnFee[account], "Account is not excluded From burnFee");	
        _isExcludedFromburnFee[account] = false;	
        emit IncludedInburnFee(account);	
    ...
    function setOwnerTaxAccount(address _account) external onlyOwner() {	
        _previousburnWallet = _burnWallet;	
        _burnWallet = _account;	
        	
        emit OwnerTaxAccountUpdated(_previousburnWallet,_burnWallet);	
    }	
    ...
    function _transfer(	
        ...
        if(_isExcludedFromburnFee[from]){	
          _tokenTransfer(from,to,amount,takeFee);  	
                	
        }else {	
            	
         //send 1% to owner 	
        uint256 _amountSentToOwner = amount.mul(_burnFee).div(10**2);	
        uint256 _remainingAmount =  amount.sub(_amountSentToOwner);	
            	
        _tokenTransfer(from,_burnWallet,_amountSentToOwner,takeFee);	
