...
contract BestRateQuery {
    ...
    constructor(IGrandBank _grand) public {
        grandBank = _grand;
    }
    ...
    returns (
        ...
        bytes memory payload = abi.encodeWithSelector(grandBank.getDestinationReturnAmount.selector, route, src, dest, srcAmount, partnerIndex);
        (bool success, bytes memory data) = address(grandBank).staticcall(payload);
        if (success) {
