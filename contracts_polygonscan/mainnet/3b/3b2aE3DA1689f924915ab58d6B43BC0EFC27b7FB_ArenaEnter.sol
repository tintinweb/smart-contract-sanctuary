...
contract ArenaEnter is Ownable, ERC721Holder {
    ...
    constructor() {
        arenas = [Arena(0x30c86F7678c51dc479F27473613332FC7CC2F00a),
        Arena(0x8f7cAe5b965978546F90164f504735Fe19838044)
        ];
    }
    
    function addMonsters3(uint16 index, uint nftId1, uint nftId2, uint nftId3, uint gNftId1, uint gNftId2, uint gNftId3) public returns (uint16){
        
        arenas[index].addMonsters2(nftId1,nftId2,nftId3,gNftId1,gNftId2,gNftId3);
        emit JoinedToArena(index, msg.sender);
    ...
    function addPotions(uint index, uint[] memory nftIds, uint amount) public {
        arenas[index].addPotions(nftIds, amount);
    }
    ...
    function setArenaAddress(uint index, address ad) public onlyOwner {
     ...
     function setArenas(address[] memory a) public onlyOwner {
         for (uint8 i=0;i<a.length;i++) { 
            arenas[i] = Arena(a[i]);
         }
         arenasCount = uint16(a.length);
    }
