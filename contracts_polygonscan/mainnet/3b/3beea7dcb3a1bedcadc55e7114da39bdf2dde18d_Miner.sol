...
contract Miner is Ownable, ReentrancyGuard {
    ...
    uint public referralFeeBP = 2500;
    ...
    function setReferralFeeBP(uint16 _referralFeeBP) external onlyOwner {
        require(referralFeeBP <= 1000, "Referral fee basis point exceeds limit");
        referralFeeBP = _referralFeeBP;
    }

    // Set referrer of user.
    function recordReferrer(address _user, address _referrer) external onlyGame {
        if (_referrer != address(0) && _referrer != _user && userReferrer[_user] == address(0)) {
    ...
    // Pay referral fee to the referrer who referred this user.
