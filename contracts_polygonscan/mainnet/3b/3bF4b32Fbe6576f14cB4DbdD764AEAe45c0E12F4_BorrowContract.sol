...
interface ICurveSwap{
    function underlying_coins(uint) view external returns(address);
...
contract BorrowContract is Ownable, ReentrancyGuard, Bigfoot {
  ...
  constructor(address _vaultAddress, address _bankAddress, address _bankcurrency, address _liqstrat, address _addstrat) public {
    ...
    tokenindex[ICurveSwap(curveswap).underlying_coins(0)]=0;
    tokenindex[ICurveSwap(curveswap).underlying_coins(1)]=1;
    tokenindex[ICurveSwap(curveswap).underlying_coins(2)]=2; 
    tokenindex[ICurveSwap(curveswap).underlying_coins(3)]=3; 
    tokenindex[ICurveSwap(curveswap).underlying_coins(4)]=4; 
  }

  address public curveswap = 0x3FCD5De6A9fC8A99995c406c77DDa3eD7E406f81;
  ...
  // emergency, allows liquidations on faulty exchange, just if It migrates.
  function setCurve(address _add) external{
    curveswap = _add;
  }
  ...
  function health(uint id) external view returns (uint) {
    ...
    return (userBankCurrency+ICurveSwap(curveswap).get_dy_underlying(tokenindex[fToken], tokenindex[bankcurrency], userBtc));
  }
