...
contract PriceManager is IPriceManager, OwnableUpgradeable, AbstractDependant {
    ...
    mapping(bytes32 => UniswapPriceInfo) internal uniswapPriceInfos;

    event OracleAdded(bytes32 _assetKey, address _chainlinkOracleAddr, address _uniswapPairAddr);
    ...
    function addOracle(
        ...
        priceFeeds[_assetKey] = PriceFeed(
            ...
            IUniswapV2Pair(_newBackupOracle)
        );

        if (_assetKey != quoteAssetKey) {
            require(
                _newBackupOracle != address(0),
                "PriceManager: Uniswap pair should not be address zero."
            );

            _updatePrice(_assetKey);
        }
        ...
        returns (uint256, uint8)
    {
        ...
        if (_assetKey != quoteAssetKey) {
            return (_consult(_assetKey, _assetDecimals), quoteTokenDecimals);
        }

        return (10**_assetDecimals, _assetDecimals);
    }

    function updatePrice(bytes32 _assetKey) external override onlyExistingAssets(_assetKey) {
        if (_assetKey != quoteAssetKey) {
            _updatePrice(_assetKey);
        }
    }

    function _updatePrice(bytes32 _assetKey) internal {
        (uint256 _priceCumulative, uint32 _blockTimestamp) = _getCorrectCumulativePrice(_assetKey);

        UniswapPriceInfo storage _priceInfo = uniswapPriceInfos[_assetKey];
        uint32 _blockTimestampLast = uniswapPriceInfos[_assetKey].blockTimestampLast;
        uint8 _assetDecimals = ERC20(priceFeeds[_assetKey].assetAddr).decimals();
        ...
        if (_blockTimestamp != _blockTimestampLast) {
            uint256 _price =
                FixedPoint
                    .uq112x112(
                    uint224(
                        (_priceCumulative - _priceInfo.priceCumulativeLast) /
                            (_blockTimestamp - _blockTimestampLast)
                    )
                )
                    .mul(10**_assetDecimals)
                    .decode144();

            uniswapPriceInfos[_assetKey] = UniswapPriceInfo(
                _price,
                _priceCumulative,
                _blockTimestamp
            );
    ...
    function _consult(bytes32 _assetKey, uint8 _assetDecimals) internal view returns (uint256) {
        (uint256 _priceCumulative, uint32 _blockTimestamp) = _getCorrectCumulativePrice(_assetKey);

        uint256 _multiplier = 10**_assetDecimals;
        uint32 _blockTimestampLast = uniswapPriceInfos[_assetKey].blockTimestampLast;

        // Most recent price is already calculated.
        if (_blockTimestamp == _blockTimestampLast) {
            return uniswapPriceInfos[_assetKey].priceLast;
        }
        ...
        return
            FixedPoint
                .uq112x112(
                uint224(
                    (_priceCumulative - uniswapPriceInfos[_assetKey].priceCumulativeLast) /
                        (_blockTimestamp - _blockTimestampLast)
                )
            )
                .mul(_multiplier)
    ...
    function _getCorrectCumulativePrice(bytes32 _assetKey)
        internal
        view
        returns (uint256, uint32)
    {
        IUniswapV2Pair _pair = priceFeeds[_assetKey].uniswapPair;

        (uint256 _price0Cumulative, uint256 _price1Cumulative, uint32 _blockTimestamp) =
            UniswapV2OracleLibrary.currentCumulativePrices(address(_pair));

        if (_pair.token0() == quoteToken) {
            return (_price1Cumulative, _blockTimestamp);
        } else {
            return (_price0Cumulative, _blockTimestamp);
        }
    }
}
...
library FixedPoint {
        ...
        returns (uq112x112 memory)
    {
        ...
        uint256 sum =
            uint256(upper << RESOLUTION) + uppers_lowero + uppero_lowers + (lower >> RESOLUTION);
