...
import "ConfigurableRoyalties.sol";
...
contract ViciERC721 is
    ERC721,
    BaseViciContract,
    ConfigurableRoyalties
{
    using Strings for string;
    using SafeMath for uint256;

    // Creator can create a new token type and mint an initial supply.
    bytes32 internal constant CREATOR_ROLE_NAME = "creator";

    mapping(uint256 => string) internal _customUri;
    string internal _baseUri;
    string internal _contractURI = "";
    uint256 internal distinctTokenCount = 0;
    ...
    ) ERC721(name_, symbol_) ConfigurableRoyalties(royaltyType) {
        _baseUri = baseUri_;
    }
     ...
     */
    function distinctTokensMinted() public view returns (uint256) {
     ...
     * @param toAddress The account to receive the newly minted coins.
     * @param tokenId The id of the new token.
     * @param custom_uri The URI to the content, if not derived from baseUri and tokenId
     * 
     * Requirements:
     *
     ...
     * - `custom_uri` MAY be an empty string.
     */
    function mint(
        address toAddress,
        uint256 tokenId,
        string memory custom_uri
    )
        public
        virtual
        whenNotPaused
        ...
        onlyOwnerOrRole(CREATOR_ROLE_NAME)
    {
        require(
            !_isBanned(toAddress),
            "ViciERC721#mint: You cannot mint to a banned address"
        );

        if (bytes(custom_uri).length > 0) {
            _customUri[tokenId] = custom_uri;
            emit URI(custom_uri, tokenId);
        }
        distinctTokenCount++;
     ...
     * - Calling user MUST NOT be banned.
     * - `tokenId` must exist
     * - The token referenced by `tokenId` must belong to the calling user.
	 */
        ...
        returns (string memory)
    {
        require(
            _exists(tokenId),
            "ViciERC721#tokenURI: URI query for nonexistent token"
        );

        // We have to convert string to bytes to check for existence
        bytes memory customUriBytes = bytes(_customUri[tokenId]);
        if (customUriBytes.length > 0) {
            return _customUri[tokenId];
        } else {
            return
                string(
                    abi.encodePacked(
    ...
    /**
     * @notice gets a uri pointing to metadata about this token collection.
    ...
    function safeTransferFrom(
        ...
        address toAddress,
        uint256 tokenId
    ) public virtual override whenNotPaused noBannedAccounts {
        require(
            !_isBanned(fromAddress) && !_isBanned(toAddress),
            "ViciERC721#safeTransferFrom: You cannot transfer from or to a banned address."
        );
    ...
    function transferFrom(
        ...
        require(
            !_isBanned(fromAddress) && !_isBanned(toAddress),
            "ViciERC721#transferFrom: You cannot transfer from or to a banned address."
        );
        super.safeTransferFrom(fromAddress, toAddress, tokenId);
    }
     ...
     * - `tokenId` MUST belong to calling user.
     *
     * @inheritdoc ERC721
     */
        ...
        noBannedAccounts
    {
        require(
            !_isBanned(operator),
            "ViciERC721#: You cannot grant approval to a banned address."
        );
    ...
    function safeTransferFrom(
        ...
        super.safeTransferFrom(fromAddress, toAddress, tokenId, data);
    }

    /**
     * @notice set the royalties
     * @param tokenId  the token, ignored if royalties type is `CONTRACT_WIDE`.
     * @param recipient the address where the royalties will be sent.
     * @param value the royalties basis points. 100 is 1% and 10000 is 100%.
     * 
     * Requirements:
     * 
     ...
     * - `tokenId` MAY refer to a non-existent token.
     * - `tokenId` SHALL be ignored if the royalties type is `CONTRACT_WIDE`.
     */
    function setRoyalties(
        ...
        require (
            !_isBanned(recipient),
            "You cannot send royalties to a banned address."
        );
        _setRoyalties(tokenId, recipient, value);
    }
}
 ...
 * @author Josh Davis <[email protected]>
 */
abstract contract ViciAccess is Ownable, AccessControlEnumerable {
    // Role for banned users.
    bytes32 internal constant BANNED_ROLE_NAME = "banned";
    ...
    modifier noBannedAccounts() {
        require(
            !hasRole(BANNED_ROLE_NAME, msg.sender),
        ...
        onlyRole(getRoleAdmin(role))
    {
        if (role == BANNED_ROLE_NAME) {
            require(
                account != owner(),
                "ViciAccess#grantRole: You cannot ban the contract owner."
            );
            require(
                msg.sender == owner(),
                "ViciAccess#grantRole: Only the owner can ban accounts."
            );
        }
    ...
    function renounceOwnership() public view override onlyOwner {
        revert(
            "ViciAccess#renounceOwnership: You cannot renounce ownership. You can only transfer to another address."
        );
    }
        ...
        onlyOwner
    {
        ...
        require(
            !_isBanned(newOwner),
            "ViciAccess#transferOwnership: You cannot transfer ownership to a banned address."
        );
        ...
        onlyRole(getRoleAdmin(role))
    {
        require(
            ...
            "ViciAccess#renounceRole: You cannot revoke admin from the contract owner."
        );
        require(
            role != BANNED_ROLE_NAME || msg.sender == owner(),
            "ViciAccess#renounceRole: Only owner can unban accounts."
        );
        ...
        override
    {
        require(
            role != DEFAULT_ADMIN_ROLE || account != owner(),
            "ViciAccess#renounceRole: The contract owner cannot renounce admin."
        );
        require(
            role != BANNED_ROLE_NAME,
            "ViciAccess#renounceRole: You can't unban yourself. Nice try."
        );
...
import "ERC2981NoRoyalties.sol";
import "ERC2981PerTokenRoyalties.sol";

/**
 * @title Configurable Royalties
 * @author Josh Davis <[email protected]>
 * @dev You can extend this contract and pass in the loyalty type 
 *    instead of implementing several similar contracts that only differ
 *    in how royalties are calculated.
 */
contract ConfigurableRoyalties is
    ERC2981ContractWideRoyalties,
    ERC2981NoRoyalties,
    ERC2981PerTokenRoyalties
{
    RoyaltyType internal _royaltyType;

    constructor(RoyaltyType royaltyType) {
        _royaltyType = royaltyType;
    }
        ...
        )
    {
        if (_royaltyType == RoyaltyType.NONE) {
            ERC2981NoRoyalties._setRoyalties(tokenId, recipient, value);
        } else if (_royaltyType == RoyaltyType.PER_TOKEN) {
            ERC2981PerTokenRoyalties._setRoyalties(tokenId, recipient, value);
        } else if (_royaltyType == RoyaltyType.CONTRACT_WIDE) {
            ERC2981ContractWideRoyalties._setRoyalties(
                tokenId,
                recipient,
                value
            );
    ...
    {
        if (_royaltyType == RoyaltyType.NONE) {
            (receiver, royaltyAmount) = ERC2981NoRoyalties._royaltyInfo(
                tokenId,
                value
            );
        } else if (_royaltyType == RoyaltyType.PER_TOKEN) {
            (receiver, royaltyAmount) = ERC2981PerTokenRoyalties._royaltyInfo(
                tokenId,
                value
            );
        } else if (_royaltyType == RoyaltyType.CONTRACT_WIDE) {
            (receiver, royaltyAmount) = ERC2981ContractWideRoyalties
                ._royaltyInfo(tokenId, value);
        } else {
            revert("Unsupported RoyaltyType.");
        }
    }
...
contract ERC2981PerTokenRoyalties is ERC2981Base {
        ...
        returns (address receiver, uint256 royaltyAmount)
    {
        ...
        // We will return ddress of 0 and amount of 0%.
