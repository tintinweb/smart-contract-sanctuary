...
contract TankChef is Ownable, ReentrancyGuard {
    ...
    function updateRewardPerBlock(UpdateRewardType _type) internal {
        ...
        if (
            ...
            (_type == UpdateRewardType.DepositOp && block.number > startBlock)
        ) {
            rewardPerBlock = totalLeft.div(endBlock.sub(block.number));
        } else if (
            _type == UpdateRewardType.UpdateStartOp ||
            (_type == UpdateRewardType.DepositOp && block.number < startBlock)
        ) {
    ...
    /**
     * Updates start block if the new start block is bigger than actual block number and dividend pool has not started
