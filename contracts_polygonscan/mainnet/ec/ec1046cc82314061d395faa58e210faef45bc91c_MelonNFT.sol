...
contract MelonNFT is ERC721Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function safeTransferFrom(address from, address to, uint256 tokenId, bytes memory _data) public virtual override {
        ...
        MarketplaceContract.changeOwner(tokenId, to);
    }
...
contract MelonMarketplace is AccessControlUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function internalMint(MelonLibrary.NFT_MODEL memory nftModel,
        ...
        if (nftModel.saleStatus == MelonLibrary.SaleStatuses.FIX_PRICE)
            require(
                nftModel.price.length > 0 && nftModel.symbol.length > 0 && (nftModel.symbol[0] == MelonLibrary.MATIC || nftModel.symbol[0] == 2),
    ...
    function buyWithERC20(address sender, uint256 tokenID, uint256 symbol, address tokenAddress, uint256 comissionPoints) private {
        ...
        require(tokens[tokenID].symbol[0] == symbol, "wrong currency selected!");
        require(tokens[tokenID].owner != address(0), "you're trying to sell to zero-address");

        uint256 referralFeeTotal = 0;

        IERC20 erc20Token = IERC20(tokenAddress);
        uint256 comissionValue = tokens[tokenID].price[0].mul(comissionPoints).div(1000);
        erc20Token.transferFrom(sender, feesAddress, comissionValue);
        ...
        if (tokens[tokenID].isFirstSale && bytes(abi.encodePacked(tokens[tokenID].referralAddress)).length > 0) {
            ...
            erc20Token.transferFrom(sender, tokens[tokenID].referralAddress[0], referralFeeTotal);
        }

        tokens[tokenID].isFirstSale = false;

        erc20Token.transferFrom(sender, tokens[tokenID].owner, tokens[tokenID].price[0].sub(referralFeeTotal + comissionValue));
        NFTContract.transferToken(getTokenByID(tokenID).owner, sender, tokenID);
        tokens[tokenID].owner = payable(sender);
        tokens[tokenID].saleStatus = MelonLibrary.SaleStatuses.NOT_ON_SALE;
        emit TokenBought(tokenID, sender, block.timestamp);
    }

    function buyWithMelon(uint256 tokenID) external nonReentrant {
        buyWithERC20(msg.sender, tokenID, MelonLibrary.MELON, address(melonToken), 0);
    }

    function buyWithWETH(uint256 tokenID) external nonReentrant {
        buyWithERC20(msg.sender, tokenID, 2, 0x7ceB23fD6bC0adD59E62ac25578270cFf1b9f619, 25);
    }
    ...
    function setForSale(
        ...
        require(symbol == MelonLibrary.MATIC || symbol == 2, "the symbol for the NFT has not been specified!");
    ...
    function changeOwner(uint256 tokenID, address owner) public {
        require(msg.sender == address(NFTContract), "not elighible to change owner");
        if (owner != englishAuctionAddress) {
            tokens[tokenID].owner = payable(owner);
        }
    }
...
contract NFTEnglishAuction is IERC721Receiver, ERC721HolderUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    address private constant wethAddress = 0x7ceB23fD6bC0adD59E62ac25578270cFf1b9f619;
    ...
    function createAuction(
        ...
        require(symbol == 0 || symbol == 2, "MelonEnglishAuction: this token is not supported!");
    ...
    function bidWithERC20(address sender, uint256 tokenID, uint256 bidPrice, uint256 symbol, address tokenAddress) private {
        Auction storage auction = tokenIDToAuction[tokenID];

        require(isBidValid(tokenID, bidPrice), "issue on validation");
        require(auction.symbol == symbol, "symbol is wrong");
        ...
        erc20token.transferFrom(sender, address(this), bidPrice);

        tokenIDToAuction[tokenID].highestBid = bidPrice;
        tokenIDToAuction[tokenID].highestBidder = sender;
        tokenIDToAuction[tokenID].totalBids = totalBids + 1;

        emit BidPosted(tokenID, bidPrice, tokenIDToAuction[tokenID].symbol, sender, block.timestamp);

        if (lastHighestBidder != address(0)) {
            erc20token.approve(address(this), lastHighestBid);
            erc20token.transferFrom(address(this), lastHighestBidder, lastHighestBid);
        }
    }

    function bidWithMelon(uint256 tokenID, uint256 bidPrice) public nonReentrant {
        bidWithERC20(msg.sender, tokenID, bidPrice, 1, address(melonToken));
    }

    function bidWithWETH(uint256 tokenID, uint256 bidPrice) public nonReentrant {
        bidWithERC20(msg.sender, tokenID, bidPrice, 2, wethAddress);
    }
    ...
    function finalize(uint256 tokenID) public nonReentrant {
        ...
        require(msg.sender == auction.tokenOwner || MarketplaceContract.viewAdminRole(msg.sender), "Invalid caller");
        bool minPriceConditionMet = false;
        if (auction.reservePrice == auction.minBid) {
            minPriceConditionMet = auction.highestBid >= auction.reservePrice;
        } else {
            minPriceConditionMet = auction.highestBid > auction.minBid;
        }
        require((block.timestamp >= auction.startTime && block.timestamp <= auction.endTime && minPriceConditionMet) || auction.highestBid >= auction.reservePrice, "Auction can't be ended");
        ...
        if (auction.symbol == 0) {
            ...
            sendPayments(address(melonToken), tokenID, auction.highestBid, 0);
        } else if (auction.symbol == 2) {
            sendPayments(wethAddress, tokenID, auction.highestBid, 25);
        }
    ...
    function withdraw(uint256 tokenID) public nonReentrant {
        ...
        if (auction.symbol == 0 && lastHighestBidder != address(0x0)) {
            ...
            approveTransferERC20(address(melonToken), lastHighestBidder, lastHighestBid);
        } else if (auction.symbol == 2 && lastHighestBidder != address(0x0)) {
            approveTransferERC20(wethAddress, lastHighestBidder, lastHighestBid);
        }
    ...
    function placeOfferWETH(uint256 tokenID, uint256 value) public nonReentrant {
        validateOffer(tokenID, value, msg.sender, 2);
    ...
    function approveOffer(uint256 tokenID, uint256 offerId) public nonReentrant {
        ...
        if (auction.symbol == 0) {
            ...
            sendPayments(address(melonToken), tokenID, offer.offerValue, 0);
        } else if (auction.symbol == 2) {
            sendPayments(wethAddress, tokenID, offer.offerValue, 25);
        }
        ...
        for (uint256 index = 0; index < auction.offerIdArray.length; index++) {
            ...
            if (currentId != offerId && auction.offers[currentId].isExist) {
                if (offer.offerSymbol == 0) {
                    ...
                    approveTransferERC20(address(melonToken), auction.offers[currentId].offerCreator, auction.offers[currentId].offerValue);
                } else if (offer.offerSymbol == 2) {
                    approveTransferERC20(wethAddress, auction.offers[currentId].offerCreator, auction.offers[currentId].offerValue);
                }
            }
        }
        delete tokenIDToAuction[tokenID];
    ...
    function denyOffer(uint256 tokenID, uint256 offerId) public nonReentrant {
        ...
        if (auction.symbol == 0) {
            ...
            approveTransferERC20(address(melonToken), auction.offers[offer.offerId].offerCreator, auction.offers[offer.offerId].offerValue);
        } else if (auction.symbol == 2) {
            approveTransferERC20(wethAddress, auction.offers[offer.offerId].offerCreator, auction.offers[offer.offerId].offerValue);
        }
    ...
    function approveTransferERC20(address tokenAddress, address to, uint256 value) private {
        IERC20 erc20Token = IERC20(tokenAddress);
        erc20Token.approve(address(this), value);
        erc20Token.transferFrom(address(this), to, value);
    }

    function sendPayments(address tokenAddress, uint256 tokenID, uint256 value, uint256 comissionPoints) private {
        IERC20 erc20Token = IERC20(tokenAddress);
        erc20Token.approve(address(this), value);
        uint256 comissionValue = value.mul(comissionPoints).div(1000);
        erc20Token.transferFrom(address(this), feesAddress, comissionValue);
        MelonLibrary.NFT_MODEL memory token = MarketplaceContract.getTokenByID(tokenID);
        Auction storage auction = tokenIDToAuction[tokenID];
        uint256 referralFeeTotal = 0;
        if (token.referralAddress.length > 0 && token.isFirstSale) {
            referralFeeTotal = value.mul(token.referralFee).div(100);
            erc20Token.transferFrom(address(this), token.referralAddress[0], referralFeeTotal);
        }
        erc20Token.transferFrom(address(this), auction.tokenOwner, value.sub(referralFeeTotal + comissionValue));
    }
