...
abstract contract StrategyFarmBase is BaseStrategyMasterChef {
    ...
    address public constant gfi = 0x874e178A2f3f3F9d34db862453Cd756E7eAb0381;
    ...
    function harvest() public override {
        ...
        //Swap 
        uint256 _gfiBalance = IERC20(gfi).balanceOf(address(this));
        if (_gfiBalance > harvestCutoff){
            //Send performance fee to strategist
            uint256 performanceFee = _gfiBalance.mul(keepFXS).div(keepFXSmax);
            if (performanceFee>0) {
                _swapUniswapWithPath(gfi_eth_path, performanceFee);
            ...
            // Swap
            _gfiBalance = IERC20(gfi).balanceOf(address(this));
            _swapUniswapWithPath(gfi_eth_path, _gfiBalance.div(2));

            // Add liquidity
            uint256 _gfi = IERC20(gfi).balanceOf(address(this));
            uint256 _eth = IERC20(eth).balanceOf(address(this));
            if (_gfi > 0 && _eth > 0) {
                IERC20(gfi).safeApprove(currentRouter, 0);
                IERC20(gfi).safeApprove(currentRouter, _gfi);
                ...
                // Donates DUST
                IERC20(gfi).safeTransfer(
                    strategist,
                    IERC20(gfi).balanceOf(address(this))
                );
                IERC20(eth).safeTransfer(
                    strategist,
                    IERC20(eth).balanceOf(address(this))
...
contract StrategyBase is StrategyFarmBase {
    // Token addresses
    address public GFI_MASTER_CHEF = 0xEf943A1B9A5E697Eb26B1cfc5e9225D2Aa00395a;
    address public GFI_ETH_LP = 0x1587663E8F475E69eA2DBb38482C8c4EE9F388Fb;
