...
interface IProxy
{
	...
	function balanceOfLevel(address addr,uint8 level) external view returns(uint256);
...
contract DDAOAccessProxy is AccessControl
{
	...
	{
		return IProxy(AccessGuru).balanceOf(addr);
	}
	function balanceOfLevel(address addr,uint8 level) public view returns(uint256)
	{
		return IProxy(AccessGuru).balanceOfLevel(addr,level);
	}
