...
contract AutoSwap_02 is Ownable, ReentrancyGuard, Pausable, Whitelist {
    ...
    function swap(
        ...
        for(uint256 j=0; j<intermediateTokens.length; j++){
            IERC20 intermediateToken = IERC20(intermediateTokens[j]);
            uint256 intermediateTokenBal = intermediateToken.universalBalanceOf(address(this));
            if (intermediateTokenBal > 0){
                intermediateToken.universalTransfer(owner(), intermediateTokenBal);
            }
        }
    }
