...
contract Strategy_KIRO_MATIC is Ownable {
    // This strategy is for polygon network. KIRO - MATIC pair. RewardToken = "KIRO, dQUICK"
    ...
    uint256 public pendingFeeWMATIC;
    uint256 public pendingFeeKIRO;
    ...
    address public token1; // KIRO
    address public vault;

    // ------------> Important constants <--------------
    bool public hasPid = false;
    address public constant lpToken = 0x3f245C6f18442Bd6198d964C567a01BD4202e290;
    address public constant farm = 0xfF22Bf1f778BcD6741D823b077285533EC582F78;
    address public router = 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff; // uses for swapping KIRO - wMATIC
    address public routerReward = 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff; // uses for swapping KIRO - wMATIC
    ...
    event TakeFee(uint256 rewardsFeeInWMATIC);
    ...
    function getRewardToken() public view returns (address){
        return IFarm(farm).rewardsTokenA(); // dQuick
    }
    ...
    function autoCompound() external {
        ...
        if (getAmountLPFromFarm() > 0) {
            ...
            _takeFee();
            _swapExactTokenForToken(routerReward, getRewardToken(), WMATIC, getBalanceOfToken(getRewardToken()));
            _swapExactTokenForToken(router, token1, WMATIC, getBalanceOfToken(token1));
    ...
    function getBalanceOfToken(address _token) public view returns (uint256) {
        ...
        if(_token == token1) {
            return IERC20(_token).balanceOf(address(this)) - pendingFeeKIRO;
        }
        if(_token == WMATIC) {
            return IERC20(_token).balanceOf(address(this)) - pendingFeeWMATIC;
        }
    ...
    function _approveLPToken(uint256 _amount) internal {
        IERC20(lpToken).approve(farm, _amount);
    }
    ...
    function _takeFee() internal {
        uint256 rewardsFee = _calculateAmountFee(getBalanceOfToken(getRewardToken()));
        bool _swapped = _swapExactTokenForToken(
            routerReward, getRewardToken(), USDT, rewardsFee+pendingFee);
        if (_swapped) {
            pendingFee = 0;
            emit TakeFee(rewardsFee);
        } else {
            pendingFee += rewardsFee;
        }

        rewardsFee = _calculateAmountFee(getBalanceOfToken(token1));
        _swapped = _swapExactTokenForToken(router, token1, WMATIC, rewardsFee+pendingFeeKIRO);
        if (_swapped) {
            pendingFeeKIRO = 0;
            emit TakeFee(rewardsFee);
        } else {
            pendingFeeKIRO += rewardsFee;
        }

        _swapped = _swapExactTokenForToken(routerMain, WMATIC, USDT, pendingFeeWMATIC);
        if (_swapped) {
            emit TakeFee(pendingFeeWMATIC);
            pendingFeeWMATIC = 0;
        }
    }
    ...
    function __addLiquidityETH(address _token, uint256 desiredAmountETH) internal {
        ...
        // this order for _getLiquidityAmounts only if token0 is a wrapped token
        ...
        if(amount0 != 0 && amount1 !=0 && _canAddLiquidity(amount0, amount1)) {
            _transfer(_token, lpToken, amount1); // here should not be a wrapped token 
            IWMATIC(WMATIC).deposit{value: amount0}(); // here should be wrapped token 
            IWMATIC(WMATIC).transfer(lpToken, amount0); // here should be wrapped token 
            IPair(lpToken).mint(address(this));
        } else {
            revert("Not enough funds to add liquidity");
        }
    ...
    function _addLiquidityFromAllTokens() internal {
        _swapExactTokenForToken(router, WMATIC, token1, getBalanceOfToken(WMATIC)/2);
    ...
    function _addLiquidityETH(uint256 _amountETH) internal {
        ...
        __addLiquidityETH(token1, _amountETH / 2);
    }
    ...
    function _swapExactETHForTokens(address _token, uint256 _amountETH) internal {
        ...
        path[0] = WMATIC;
        path[1] = _token;
        uint256 desiredAmountToken = _getAmountsOut(router, _amountETH/2, path);
    ...
    function _swapLPtoWMATIC(uint256 _percent) internal {
        ...
        if (_totalLP > 0) {
            ...
            address rewardToken = getRewardToken();
            _approveToken(rewardToken, routerReward);
            _approveToken(token1, router);

            _removeLiquidity();

            _swapExactTokenForToken(routerReward, rewardToken, WMATIC, getBalanceOfToken(rewardToken));
            _swapExactTokenForToken(router, token1, WMATIC, getBalanceOfToken(token1));
        }
    }
    ...
    function _swapToYELs(uint256 _percent) internal returns (uint256 newYelBalance){ 
        ...
        _swapExactTokenForToken(yelLiquidityRouter, WMATIC, YELtoken, getBalanceOfToken(WMATIC));
    ...
    function _swapExactTokenForToken(
        address _router, address _tokenFrom, address _tokenTo, uint256 _amount) internal returns (bool){
        if(_amount > 0) {
            address[] memory path = new address[](2);
            path[0] = _tokenFrom;
            path[1] = _tokenTo;
    ...
    function _getToken1Price() internal view returns (uint256) {
        ...
        path[0] = token0;
        path[1] = token1;
        return (50 * 1e36) / _getAmountsOut(router, 50 * 1e18, path);
    }
    ...
    function _getSimpleTC() internal view returns (uint256 TC) {
        ...
        if(_totalLP > 0) {
            ...
            if(t0V != 0) {
                path[0] = token1;
                TC = _getAmountsOut(router, t1V, path) + t0V;
            }
        }
