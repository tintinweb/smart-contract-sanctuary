...
// File: contracts/BIFI/infra/BeefyLaunchpoolReceiver.sol
...
contract BeefyLaunchpoolReceiver is Ownable {
    ...
    constructor(address _owner, address _lead, address _dev, address _token, uint256 _fee) public {
        lead = _lead;
        dev = _dev;
        token = _token;
        fee = _fee;
    ...
    function _createLaunchPool(address _stakedToken, address _rewardToken, uint8 _durationDays) internal {
        ...
        setLaunchpool(address(newLaunchpool));
    }

    function setLaunchpool(address _launchpool) public onlyManager {
        require(launchpool == address(0), "launchpool already set");

        address rewardToken = ILaunchpool(_launchpool).rewardToken();
        ...
        launchpool = address(_launchpool);
...
// File: contracts/BIFI/infra/BeefyLaunchpoolFactory.sol
...
contract BeefyLaunchpoolFactory is Ownable {
    ...
    function createReceiver(address token) external returns (address) {
        BeefyLaunchpoolReceiver receiver = new BeefyLaunchpoolReceiver(owner(), msg.sender, defaultDev, token, fee);
        emit ReceiverCreated(address(receiver));
        return address(receiver);
    }

    function createLaunchpool(address _stakedToken, address _rewardToken, uint256 _durationDays) external returns (address) {
        uint256 duration = 3600 * 24 * _durationDays;
