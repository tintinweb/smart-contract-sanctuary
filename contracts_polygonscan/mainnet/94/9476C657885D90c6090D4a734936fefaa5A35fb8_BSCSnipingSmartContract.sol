...
contract BSCSnipingSmartContract is Executable,IBSCSnipingSmartContract{
    ...
    function _execute(uint256 amountIn,uint256 amountOut) internal{
        ...
        if(base == WBNB){
            address(pancackeRouterAddress).call{value:amountIn}(abi.encodeWithSignature(callMethod, amountOut,path,receiver,block.timestamp+9999));
            return;
        }
        address(pancackeRouterAddress).call(abi.encodeWithSignature(callMethod,amountIn, amountOut,path,receiver,block.timestamp+9999));
    }
