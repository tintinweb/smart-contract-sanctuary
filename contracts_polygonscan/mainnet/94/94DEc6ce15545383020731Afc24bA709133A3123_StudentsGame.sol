...
// File: contracts/libraries/student.sol
...
contract StudentsGame is Initializable, AccessControl {
    event Play(StudentsLibrary.GameResult);

    bytes32 public constant MAIN_OWNER = keccak256("MAIN_OWNER");

    string public constant INVALID_EMPLOYEE = "RG: Invalid employee.";
    string public constant NULL_REWARDS = "RG: Null rewards.";
    string public constant INVALID_SELECTION = "RG: Invalid selection.";
    ...
    mapping(uint256 => StudentsLibrary.Selection) public _selections;

    uint64 public minPlayTime = 43200;
    ...
    function initialize(
        ...
        _selections[0] = StudentsLibrary.Selection(100, 250, 0, 100);
        _selections[1] = StudentsLibrary.Selection(90, 150, 0, 30);

        _selections[2] = StudentsLibrary.Selection(
            90,
            50,
        ...
        _selections[3] = StudentsLibrary.Selection(
            70,
            0,
        ...
        _selections[4] = StudentsLibrary.Selection(
            90,
            20,
            25000000000000000000,
            2
    ...
    function getEmployeePlayTime(uint256 id) public view returns (uint256) {
        return _employeePlayTime[id];
    }
    ...
    function getGameData()
        public
        view
        returns (StudentsLibrary.GameData memory)
    {
        return
            StudentsLibrary.GameData(
                [
                    _selections[0],
                    _selections[1],
                    _selections[2],
    ...
    function setMinPlayTime(uint64 time) public onlyRole(MAIN_OWNER) {
        minPlayTime = time;
    }

    function changeSelection(
        uint256 index,
        StudentsLibrary.Selection calldata selection
    ) public onlyRole(MAIN_OWNER) {
        require(_selections[index].points > 0, INVALID_SELECTION);
    ...
    function requestTokens() public {
        require(_playRewards[msg.sender] > 0, NULL_REWARDS);
        manager.sendTokens(msg.sender, _playRewards[msg.sender]);
        _playRewards[msg.sender] = 0;
    }

    // Game

    function playWithMiniEmployee(uint256 employee, uint8 selection) public {
        require(canEmployeePlay(msg.sender, employee), INVALID_EMPLOYEE);
        require(_selections[selection].probability != 0, INVALID_SELECTION);

        uint16 employeePoints = miniEmployees.getPoints(employee);

        require(
            _selections[selection].points <= employeePoints,
            INVALID_SELECTION
        );
        ...
        if (random < _selections[selection].probability) {
            win = true;

            if (_selections[selection].prize > 0) {
                _playRewards[msg.sender] += _selections[selection].prize;
            }

            if (_selections[selection].xp > 0) {
                miniEmployees.levelUp(employee, _selections[selection].xp);
            }
        }

        _employeePlayTime[employee] = block.timestamp;

        emit Play(
            StudentsLibrary.GameResult(
    ...
    function playWithManyEmployees(uint256[] memory employees, uint8 selection)
        public
    {
        require(allEmployeesCanPlay(msg.sender, employees), INVALID_EMPLOYEE);
        require(_selections[selection].probability != 0, INVALID_SELECTION);

        uint256[] memory manyRandoms = RandomUtil.expandedRandomSeededMinMax(
            0,
            99,
            randoms.getRandomSeed(msg.sender),
            _randomCounter,
        ...
        for (uint256 i = 0; i < manyRandoms.length; i++) {
            ...
            if (manyRandoms[i] < _selections[selection].probability) {
                win = true;
                ...
                if (_selections[selection].xp > 0) {
                    miniEmployees.levelUp(
                        employees[i],
                        _selections[selection].xp
                    );
                }
            }

            _employeePlayTime[employees[i]] = block.timestamp;
    ...
    function canEmployeePlay(address owner, uint256 id)
        public
        view
        returns (bool)
    {
        return
            ...
            miniEmployees.validate(id) &&
            miniEmployees.ownerOf(id) == owner;
    }

    function allEmployeesCanPlay(address owner, uint256[] memory employees)
        public
        view
        returns (bool)
    {
        ...
        for (uint256 i = 0; i < employees.length; i++) {
            if (!canEmployeePlay(owner, employees[i])) canPlay = false;
        }
