...
contract DeflationaryToken is ERC20, Ownable  {
    ...
    // Transfer tax rate in basis points. 0.5%
    uint16 public transferTaxRate = 50;

    // Burn 90% of the tax
    uint16 public burnRate = 9000;
    ...
    constructor () ERC20("Contrarium", "cMATICalpha")
    {
    ...
    function updateFeeAddress(address _feeAddress) public {
        require(msg.sender == feeAddress, "Only feeAddress can update");
...
contract Bank is Ownable {
    ...
    address public amToken;
    uint16 public constant referralCode = 0;
    address[] public assets;

    constructor(address _backupToken, address _amToken){
    ...
    function getPrice() public view returns (uint256) {
        ...
        uint256 backedBalance = getBackupAmount();

        if(totalSupply == 0)
            return 1 ether;

        return backedBalance.mul(1e18).div(totalSupply);
    }
    
    function buy(uint256 _amount) public payable {
        require(buyingEnabled == true, "Minting disabled by owner");
        uint256 price = getPrice();
        IERC20(backupToken).safeTransferFrom(msg.sender, address(this), _amount);
        uint256 amount = _amount.mul(1e18).div(price);
        token.mint(msg.sender, amount);
        _supply(_amount);
    }

    function redeem(uint256 amount, bool _inRawAmToken) public {
        require(amount > 0, "Can't redeem 0 tokens");
        uint256 price = getPrice();
        uint256 returnAmount = 0;
        uint256 redeemPenalty = amount.mul(500).div(10000);
        address sender = msg.sender;

        //Transfer tokens to bank
        token.transferFrom(msg.sender, address(this), amount);
        token.burn(amount);

        amount = amount.sub(redeemPenalty);

        returnAmount = amount.mul(price).div(1e18);

        if(_inRawAmToken){
            IERC20(amToken).safeTransfer(sender, returnAmount);            
        } else {
            _removeSupply(returnAmount);
            IERC20(backupToken).safeTransfer(sender, returnAmount);
        }
    }

    function getBackupAmount() public view returns (uint256) {
        return IERC20(amToken).balanceOf(address(this));
    }

    function _supply(uint256 _amount) internal {
        IERC20(backupToken).approve(aaveDepositAddress, _amount);
        IAaveStake(aaveDepositAddress).deposit(backupToken, _amount, address(this), referralCode);
    }
    
    function _removeSupply(uint256 _amount) internal {
        IAaveStake(aaveDepositAddress).withdraw(backupToken, _amount, address(this));
    }

    function compound() public {
        IAaveStake(aaveClaimAddress).claimRewards(assets, 1, address(this));
        
        uint256 earnedAmt = IERC20(backupToken).balanceOf(address(this));
        if (earnedAmt > 0) {
            _supply(earnedAmt);
        }
    }

    function setBuyingEnabled(bool _buyingEnabled) public onlyOwner {
