...
contract yyy is ERC20, AccessControl, ERC721Holder {
    ...
    constructor(address owner, address _collectionAddress) ERC20("yyy", "yyy") AccessControl(){
        _mint(owner, 240000 * 10 ** 18);
        ...
        coinAmountPerRewardUnit = 10 * 10 ** 18; // 10 yyy coins per rewardUnit
        numberOfBlocksPerRewardUnit = 41142; // 24 hours per reward unit
        welcomeBonusAmount = 180 * 10 ** 18; // 420 Only paid once per tokenId
    }
    ...
    function stake(uint256 tokenId) public {
        ...
        require(IERC721(collectionAddress).ownerOf(tokenId) == address(this),
            "yyy: Error while transferring token");
    ...
    function setCollectionAddress(address newAddress) external onlyRole(CONTRACT_ADMIN_ROLE){
        require (newAddress != address(0), "yyy: update to zero address not possible");
