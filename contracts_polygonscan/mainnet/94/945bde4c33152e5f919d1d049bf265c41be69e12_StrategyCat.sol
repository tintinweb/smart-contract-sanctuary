 ...
 * paying for execution may not be the actual sender (as far as an appliMegaion
 * is concerned).
 *
...
interface IERC20 {
     ...
     *
     * Returns a boolean value indiMegaing whether the operation succeeded.
     *
...
library SafeMath {
    ...
    /**
     * @dev Returns the multipliMegaion of two unsigned integers, with an overflow flag.
     *
     * _Available since v3.4._
     */
    ...
    /**
     * @dev Returns the multipliMegaion of two unsigned integers, reverting on
     * overflow.
     *
     * Counterpart to Solidity's `*` operator.
     *
     * Requirements:
     *
     * - MultipliMegaion cannot overflow.
     */
...
abstract contract StrategyCatBase is BaseStrategyMasterChef {

    // Token addresses for MATIC
    address public constant mega =  0x5F0454838289aEd5fa5B71aaC82da1da314Ac5d3;
    ...
    // Uniswap swap paths
    address[] public matic_mega_path;
    ...
    {
        matic_mega_path = new address[](2);
        matic_mega_path[0] = mega;
        matic_mega_path[1] = matic;
    }
    ...
    function set_harvestCutoff(uint256 newCutoff) external onlyOwner() {
        require(newCutoff <= 10**18, "New cutoff must be less than 1 mega");
    ...
    function harvest() public override {
        ...
        if(getReward) {
            // Collects mega tokens
            _getReward();
            getReward = false;
            return;
        }

        //Swap 1/2 mega for MATIC
        uint256 _megaBalance = IERC20(mega).balanceOf(address(this));
        if (_megaBalance > harvestCutoff){
            if (_megaBalance > 0) {
                _swapUniswapWithPath(matic_mega_path, _megaBalance.div(2));
            }
    
            // Add liquidity for mega/MATIC
            uint256 _matic = IERC20(matic).balanceOf(address(this));
            uint256 _mega = IERC20(mega).balanceOf(address(this));
            if (_matic > 0 && _mega > 0) {
                IERC20(matic).safeApprove(currentRouter, 0);
                IERC20(matic).safeApprove(currentRouter, _matic);
                IERC20(mega).safeApprove(currentRouter, 0);
                IERC20(mega).safeApprove(currentRouter, _mega);
                ...
                IERC20(mega).safeTransfer(
                    strategist,
                    IERC20(mega).balanceOf(address(this))
                );
            }
...
contract StrategyCat is StrategyCatBase {
    // Token addresses
    address public MEGA_MASTER_CHEF = 0xd23565f901e33D44f31482A4E7BA83aa097fBB99;
    address public MEGA_MATIC_LP = 0xc33148312b70842f543e321c9060fCBaCdEEB946;
