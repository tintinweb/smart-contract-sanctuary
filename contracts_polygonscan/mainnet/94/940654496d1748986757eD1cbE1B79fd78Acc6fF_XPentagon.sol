...
contract XPentagon is Ownable, CurveUtils, UniswapUtils {
    ...
    IBEP20 private constant WMATIC_ADDRESS =
    ...
    IWETH internal constant wmatic = IWETH(address(WMATIC_ADDRESS));
    ...
    function isWMATIC(IBEP20 token) internal pure returns (bool) {
        return (address(token) == address(WMATIC_ADDRESS));
    }
    ...
    function isofMATIC(IBEP20 token) internal pure returns (bool) {
        return (address(token) == address(ZERO_ADDRESS) ||
            address(token) == address(WMATIC_ADDRESS));
    }
    ...
    function getReserves(
        ...
        IBEP20 _from = isMATIC(fromToken) ? WMATIC_ADDRESS : fromToken;
        IBEP20 _to = isMATIC(toToken) ? WMATIC_ADDRESS : toToken;
        ...
        )
    {
        ...
        if (fromToken == toToken) {
            ...
            if (isMATIC(toToken)) {
                toToken = WMATIC_ADDRESS;
            }

            if (isMATIC(fromToken)) {
                fromToken = WMATIC_ADDRESS;
            }
    ...
    function swap(
        ...
        if (unwrap) {
            wmatic.withdraw(IBEP20(WMATIC_ADDRESS).balanceOf(address(this)));
    ...
    function _swapOnUniswapV2Internal(
        ...
        //toToken = isMATIC(toToken) ? WMATIC_ADDRESS : toToken;
