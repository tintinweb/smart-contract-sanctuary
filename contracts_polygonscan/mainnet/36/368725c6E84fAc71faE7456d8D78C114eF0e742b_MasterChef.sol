...
contract Fish2Token is ERC20("Fish2", "FISH2"), Ownable {
        ...
        external
    {
        ...
        require(signatory != address(0), "FISH2::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "FISH2::delegateBySig: invalid nonce");
        require(now <= expiry, "FISH2::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "FISH2::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying FISH2s (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "FISH2::_writeCheckpoint: block number exceeds 32 bits");
...
// MasterChef is the master of Fish2. He can make Fish2 and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once Fish2 is sufficiently
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    // The FISH2 TOKEN!
    Fish2Token public fish;
    address public devAddress;
    address public feeAddress;
    address public vaultAddress;

    // FISH2 tokens created per block.
    ...
    // The block number when FISH2 mining starts.
    uint256 public startBlock;

    // The block number when can harvest FISH2
    // it prevents from dumping during first hours or minutes
    uint256 public lockupBlock;

    // Fish2 referral contract address.
    ...
    // Update the given pool's FISH2 allocation point and deposit fee. Can only be called by the owner.
    ...
    function updatePool(uint256 _pid) public {
        PoolInfo storage pool = poolInfo[_pid];
        // lockupBlock - The block number when can harvest FISH2. it prevents from dumping during first hours or minutes
...
//      fish.mint(devAddress, fishReward.div(50));
    ...
    // Deposit LP tokens to MasterChef for FISH2 allocation.
    ...
    // Safe fish2 transfer function, just in case if rounding error causes pool to not have enough FISH2.
