...
contract TestBuy {
  ...
  address private constant buyTokenAddress    = 0x47b5C0825590412764ab22495c0857C6b0914EcE;

  function buyToken(uint amount) public payable {
    uint deadline = block.timestamp + 15;
    quickswapRouter.swapExactETHForTokens{ value: msg.value }(amount, getPath(), address(this), deadline);

    // refund leftover WMATIC to user
    (bool success,) = msg.sender.call{ value: address(this).balance }("");
    require(success, "refund failed");
  }

  function estimateBuy(uint amount) public view returns (uint[] memory) {
    return quickswapRouter.getAmountsIn(amount, getPath());
  }
