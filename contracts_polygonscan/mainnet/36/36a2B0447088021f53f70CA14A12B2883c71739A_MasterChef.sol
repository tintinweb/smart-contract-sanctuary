...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    function pendingRadio(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accsugarPerShare = accsugarPerShare.add(radioReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accsugarPerShare = pool.accsugarPerShare.add(radioReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
