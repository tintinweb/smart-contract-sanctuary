...
contract Crabtoken is BEP20('CRABSWAP', 'CRAB') {
...
contract MasterChef is Ownable {
    ...
    function pendingEgg(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accEggPerShare = accEggPerShare.add(CRABReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accEggPerShare = pool.accEggPerShare.add(CRABReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    // Safe CRAB transfer function, just in case if rounding error causes pool to not have enough EGGs.
