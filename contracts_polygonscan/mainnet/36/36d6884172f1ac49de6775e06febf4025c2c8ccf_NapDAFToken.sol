...
contract ContinuousLODAF is Ownable { 
    ...
    uint256 private _precision = 1000000;
	...
	event PoolValo(uint ethPrice, uint etherBalance, uint assetValo, uint tetherBalance, uint poolAssetPercentShare, uint poolValo);
	event EtherToTetherSwap(uint percentageDeltaToSwap, uint deltaToSwapInTether, uint thereticalDeltaToSwapInEther, uint deltaToSwapInEther);
	event TetherToEtherSwap(uint percentageDeltaToSwap, uint theoreticalDeltaToSwapInTether, uint deltaToSwapInTether);
    ...
    */
    ...
    function dafTokenRedemption(uint _amountIn)  public {
        ...
        uint assetValo_tetherbits = (oldEtherBalance*ethPrice)/(10**20);
        uint poolValo = assetValo_tetherbits + oldTetherBalance;
        uint poolAssetPercentShare = assetValo_tetherbits*_precision / (assetValo_tetherbits + oldTetherBalance);
        emit PoolValo(ethPrice, oldEtherBalance, assetValo_tetherbits, oldTetherBalance, poolAssetPercentShare,poolValo);
	    
        uint256 tetherToReimburse = (_amountIn*poolValo)/nbTotalParts ;
        if (tetherToReimburse <= oldTetherBalance){
            ...
            uint deltaToSwapInTether = tetherToReimburse - oldTetherBalance;
            // we compute the delta to swap in weis 10**18 ethers
            // ethPrice is in 10**8
            // deltaToSwapInTether is in 10**6
            // the factor is 8 -6 + 18 
            uint thereticalDeltaToSwapInEther = (deltaToSwapInTether*10**20)/ethPrice;
            uint deltaToSwapInEther;
            if (thereticalDeltaToSwapInEther>oldEtherBalance){
                deltaToSwapInEther = oldEtherBalance;
            } else {
                deltaToSwapInEther = thereticalDeltaToSwapInEther;
            }
            // we swap ether to tether
            emit EtherToTetherSwap(0, deltaToSwapInTether, thereticalDeltaToSwapInEther, deltaToSwapInEther);
            this.swap(_maticWEther,_maticTether,deltaToSwapInEther,0,address(this));
	        emit ContractBalance(_amountIn,this.getUsdtBalance(),this.getEtherBalance());
		    bool transferSuccess = IERC20(_maticTether).transfer(msg.sender, tetherToReimburse);
		    require(transferSuccess,"tether transfer failed");
        }
		RedemptionAccepted(msg.sender, _amountIn, now);
	...
	function usdtSubscription(uint256 _amountIn)  public {
	    ...
	    if (nbTotalParts > 0 && (oldTetherBalance > 0 || oldEtherBalance > 0)){
            ...
            uint poolAssetPercentShare = assetValo_tetherbits*_precision / (assetValo_tetherbits + oldTetherBalance);
            emit PoolValo(ethPrice, oldEtherBalance, assetValo_tetherbits, oldTetherBalance, poolAssetPercentShare,poolValo);
            toAllocate = (_amountIn*nbTotalParts) / poolValo;
	    }
	...
	function blanksSwapAsset(uint _signal) public onlyOwner {
	    uint256 oldTetherBalance = this.getUsdtBalance();
	    uint256 oldEtherBalance = this.getEtherBalance();
        ...
        // ether balance is in weis : * 10**18 
        // ether price is  in : * 10**8
        // asset valo in tether bits : has to be divided by 10**(18+8-6)
        uint assetValo_tetherbits = (oldEtherBalance*ethPrice)/(10**20);
        
        uint poolAssetPercentShare = assetValo_tetherbits*_precision / (assetValo_tetherbits + oldTetherBalance);
        emit PoolShare(ethPrice, oldEtherBalance, assetValo_tetherbits, oldTetherBalance, poolAssetPercentShare);
	
        // first step before = 0 : we swap _signal amount of the tether 
        if (poolAssetPercentShare > _signal){
            // the pool has too much asset compared to signal : we swap ether for tether
            uint percentageDeltaToSwap = poolAssetPercentShare - _signal;
            uint deltaToSwapInTether = (percentageDeltaToSwap * (assetValo_tetherbits + oldTetherBalance)) / _precision;
            // we compute the delta to swap in weis 10**18 ethers
            // ethPrice is in 10**8
            // deltaToSwapInTether is in 10**6
            // the factor is 8 -6 + 18 
            ...
            // we swap ether to tether
            emit EtherToTetherSwap(percentageDeltaToSwap, deltaToSwapInTether, thereticalDeltaToSwapInEther, deltaToSwapInEther);
            //this.swap(_maticWEther,_maticTether,deltaToSwapInEther,0,address(this));
        } else {
            // we swap tether to ether
            uint percentageDeltaToSwap = _signal - poolAssetPercentShare ;
            uint theoreticalDeltaToSwapInTether = (percentageDeltaToSwap * (assetValo_tetherbits + oldTetherBalance)) / _precision;
            uint deltaToSwapInTether;
            if ( theoreticalDeltaToSwapInTether>oldTetherBalance){
                deltaToSwapInTether = oldTetherBalance;
            } else {
                deltaToSwapInTether = theoreticalDeltaToSwapInTether;
            }
            
            emit TetherToEtherSwap(percentageDeltaToSwap,theoreticalDeltaToSwapInTether,deltaToSwapInTether);
	...
	function swapAsset(uint _signal) public onlyOwner {
        ...
        uint poolAssetPercentShare = assetValo_tetherbits*precision / (assetValo_tetherbits + oldTetherBalance);
        emit PoolShare(ethPrice, oldEtherBalance, assetValo_tetherbits, oldTetherBalance, poolAssetPercentShare);
	
        // first step before = 0 : we swap _signal amount of the tether 
        if (poolAssetPercentShare > _signal){
            // the pool has too much asset compared to signal :  we swap ether to tether
            uint percentageDeltaToSwap = poolAssetPercentShare - _signal;
            uint deltaToSwapInTether = (percentageDeltaToSwap * (assetValo_tetherbits + oldTetherBalance)) / _precision;
            // we compute the delta to swap in weis 10**18 ethers
            // ethPrice is in 10**8
            ...
            // we swap ether to tether
            uint percentageDeltaToSwap = _signal - poolAssetPercentShare;
            uint theoreticalDeltaToSwapInTether = (percentageDeltaToSwap * (assetValo_tetherbits + oldTetherBalance)) / precision;
            uint deltaToSwapInTether;
            if ( theoreticalDeltaToSwapInTether>oldTetherBalance){
                deltaToSwapInTether = oldTetherBalance;
            } else {
    ...
    function getDafTokenOwnedAmount() external view returns (uint256) {
	    uint256 dafBalance = _napDaf.balanceOf(msg.sender);
	    return dafBalance;
	}
