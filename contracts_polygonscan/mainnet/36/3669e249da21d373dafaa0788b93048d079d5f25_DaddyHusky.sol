...
contract DaddyHusky is  Ownable{
    ...
    uint256 public PROJECT_FEE =3; // 3%
    uint256 _previousBurnFee = BURN_FEE;
    uint256 _previousLPFee = LP_FEE;
    uint256 _previousProjectFee = PROJECT_FEE;
    address public projectWallet;
    uint256 maxLPCap = 2000000000  * 10 ** decimals();
    ...
    function changeProjectWallet( address _projectWallet) public onlyOwner{
        projectWallet = _projectWallet;
    }
    ...
    function _transfer(
        ...
        if(_excluded[from] == true || _excluded[to] ==true){
            ...
            _balances[to] += amount.sub(burnFee).sub(lpFee).sub(projectFee);
