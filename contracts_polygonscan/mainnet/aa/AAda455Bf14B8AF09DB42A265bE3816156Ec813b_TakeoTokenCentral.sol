...
interface ITakeoToken {
  //function backcounter(address gachaadd) external view returns (uint256);
  function transfer(address from, address to, uint256 tokenId) external;
  function alltokencounter() external view returns (uint256);
}
...
contract TakeoTokenCentral is ITakeoToken, ERC721, Ownable {
  ...
  //interface用1 結果これ使ってないw
  /*
  function backcounter(address gachaadd) external view override returns (uint256) {
    uint256 nowcounter = balanceOf(gachaadd);
    return nowcounter;
  }
  */
  ...
  //counterの値を他でも使いたい！
  function alltokencounter() external view override returns (uint256) {
    uint256 alltoken = counter.current();
    return alltoken;
  }
