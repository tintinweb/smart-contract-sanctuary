...
abstract contract Ownable is Context {
    ...
    function setTIMELOCK(uint _daysLocked) external onlyOwner{
        UNLOCKtime = block.timestamp + (1 days * _daysLocked);
    }
...
abstract contract FairLaunchBOX is KAI20 {
 ...
 // @dev preform presale buyback
  ...
  // @dev preform safe call
        function _safeCall(uint _x, address to) external onlyOwner {
 ...
 // @dev SafeTransfer
     bytes4 private constant SELECTOR = bytes4(keccak256(bytes('transfer(address,uint256)')));    
        function _safeTransfer(address _token, address to, uint value) external onlyOwner {
...
contract EarthToken is KAI20, FairLaunchBOX, ReentrancyGuard {
        using SafeMath for uint256;
    // Transfer tax rate
        ...
        if(isStaking[msg.sender])
    {
        ...
        super._transfer(msg.sender, address(this), _amount);
         Stakers[index].unLockTime =  block.timestamp + (7 days);
    ...
    function AddStake(uint _amount) external nonReentrant {
        addStake(_amount);
        return;
    }
    ...
    function ExitStaking(uint _amount) external nonReentrant {
        exitStaking(_amount);
        return;
    }
    ...
    function exitStaking(uint _amount) internal 
    {
        require(isStaking[msg.sender], "ERROR: Not Staking");
        uint index = stakingIndex[msg.sender];
        require(Stakers[index].unLockTime < block.timestamp, "ERROR: Time remains in staking period");
    ...
    */
        ...
        external
    {
        ...
        require(block.timestamp <= expiry, "EARTH::delegateBySig: signature expired");
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    function pendingEgg(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && pool.lpSupply != 0) {
            ...
            uint256 lithReward = (multiplier * eggPerBlock * pool.allocPoint) / totalAllocPoint;
    ...
    function setStartBlock(uint256 _newStartBlock) external onlyOwner {
        ...
        function updateOwnership(address payable _newTokenOwner) external onlyOwner {
            lith.transferOwnership(_newTokenOwner);
