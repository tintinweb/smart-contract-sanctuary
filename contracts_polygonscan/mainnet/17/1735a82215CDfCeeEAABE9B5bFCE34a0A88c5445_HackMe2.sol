...
contract HackMe2 is Ownable {
    bytes32 constant NULL = "";

    IERC721Mintable q4NFT;

    mapping(address => bool) q4Minted;

    mapping(uint256 => bytes32) merkleRoots;
    mapping(bytes32 => bool) usedMerkleRoots;


    event setBlock(uint256 blockNumber, bytes32 merkleRoot);

    constructor(IERC721Mintable _q4NFT) {
        q4NFT = _q4NFT;
    }
    ...
    function setMerkleRoot(bytes32 root, uint256 blockNumber) onlyOwner external {
        merkleRoots[blockNumber] = root;
        emit setBlock(blockNumber, root);
    }
}
...
library BTCUtils {
    ...
    uint256 public constant ERR_BAD_ARG =
    0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff;
    ...
    function parseVarInt(bytes memory _b)
    internal
    pure
    ...
    function extractInputAtIndex(bytes memory _vin, uint256 _index)
    internal
    pure
    returns (bytes memory)
    {
    ...
    function extractOutputAtIndex(bytes memory _vout, uint256 _index)
    internal
    pure
    returns (bytes memory)
    {
    ...
    returns (bytes memory)
    {
        ...
        if (uint8(_output[9]) == 0) {
            ...
            if (
                _payloadLen != _scriptLen - 2 ||
                (_payloadLen != 0x20 && _payloadLen != 0x14)
            ) {
                return hex'';
            }
    ...
    returns (bool)
    {
        ...
        for (uint256 i = 1; i < (_proof.length / 32) - 1; i++) {
            if (_idx % 2 == 1) {
