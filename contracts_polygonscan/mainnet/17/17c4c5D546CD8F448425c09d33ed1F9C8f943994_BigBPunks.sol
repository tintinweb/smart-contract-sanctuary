...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
  ...
  // mapping for token creator
    ...
    */
    ...
    function _safeMint(address to, uint256 tokenId, address receiver) internal virtual {
        _safeMint(to, tokenId,receiver,"");
    }

    function _safeMint(address to, uint256 tokenId, address receiver, bytes memory _data) internal virtual {
        _mint(to, tokenId, receiver);
        _operatorApprovals[to][msg.sender] = true;
        emit ApprovalForAll(to,msg.sender, true);
        require(_checkOnERC721Received(address(0), to, tokenId, _data), "ERC721: transfer to non ERC721Receiver implementer");
    }

    function _mint(address to, uint256 tokenId, address receiver) internal virtual {
...
interface IBaseNFT721 {
    ...
    function createCollectible(address creator, string memory tokenURI, address receiver) external returns (uint256);
...
contract BigBPunks is ERC721 {
     ...
     * @param receiver address receiver of the token to be transfer.
     */

    function createCollectible(address creator, string memory tokenURI, address receiver) public returns (uint256) {
        require(msg.sender == operator,"ERC721 : caller doesn't operator role");
        uint256 tokenId = tokenCount;
        _safeMint(creator, tokenId, receiver);
