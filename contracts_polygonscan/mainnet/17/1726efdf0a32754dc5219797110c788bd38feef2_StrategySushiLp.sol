...
interface IElysmMasterChef {
    ...
    function pendingELYSM(uint256 _pid, address _user) external view returns (uint256);
...
contract StrategySushiLp is StrategyBase {
    ...
    function _deposit() internal {
        ...
        if (_baseBal > 0) {
            IElysmMasterChef(farmPool).deposit(poolId, _baseBal);
            emit Deposit(baseToken, _baseBal);
        }
    }

    function _withdrawSome(uint256 _amount) internal override returns (uint256) {
        (uint256 _stakedAmount, ) = IElysmMasterChef(farmPool).userInfo(poolId, address(this));
        if (_amount > _stakedAmount) {
            _amount = _stakedAmount;
        }

        uint256 _before = IERC20(baseToken).balanceOf(address(this));
        IElysmMasterChef(farmPool).withdraw(poolId, _amount);
    ...
    function _withdrawAll() internal override {
        (uint256 _stakedAmount, ) = IElysmMasterChef(farmPool).userInfo(poolId, address(this));
        IElysmMasterChef(farmPool).withdraw(poolId, _stakedAmount);
    }

    function claimReward() public override {
        IElysmMasterChef(farmPool).deposit(poolId, 0);
    }
    ...
    function balanceOfPool() public view override returns (uint256) {
        (uint256 amount, ) = IElysmMasterChef(farmPool).userInfo(poolId, address(this));
        return amount;
    }
    ...
    function claimable_tokens() external view override returns (address[] memory farmToken, uint256[] memory totalDistributedValue) {
        ...
        totalDistributedValue[0] = IElysmMasterChef(farmPool).pendingELYSM(poolId, address(this));
    }
    ...
    function claimable_token() external view override returns (address farmToken, uint256 totalDistributedValue) {
        ...
        totalDistributedValue = IElysmMasterChef(farmPool).pendingELYSM(poolId, address(this));
    }
    ...
    function retireStrat() external override onlyStrategist {
        IElysmMasterChef(farmPool).emergencyWithdraw(poolId);
