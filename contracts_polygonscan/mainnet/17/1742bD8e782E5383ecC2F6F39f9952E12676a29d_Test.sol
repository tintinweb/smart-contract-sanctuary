...
contract PolygonTradingHub is QuickSwapTrader, CafeSwapTrader {
    ...
    kPolygonExchange[] availableExchanges = [kPolygonExchange.quickSwap, kPolygonExchange.cafeSwap];
...
contract QuickSwapTrader {
    ...
    IQuickSwapV2Router02 private tradingInterface = IQuickSwapV2Router02(PolygonContracts.quickSwapV2Router02);
    ...
    {
        uint256[] memory amountsOut = tradingInterface.getAmountsOut(
            _amount,
            _path
        );
    ...
    function _quickSwapSwapTokenByAddress(address _tokenInAddress, address _tokenOutAddress, uint256 _amount) private
    {
        ...
        tradingInterface.swapExactTokensForTokens(
            _amount,
            minAmount,
            path,
            receiver,
    ...
    function _quickSwapApproveSpending(address _tokenAddress) private
    {
        ...
        tokens[_tokenAddress].token.approve(address(tradingInterface), MAX_INT);
...
import "../interfaces/ICafeSwapV2Router02.sol";
import "../Library.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";

contract CafeSwapTrader {

    mapping(address => TradingToken) private tokens;

    ICafeSwapV2Router02 private tradingInterface = ICafeSwapV2Router02(PolygonContracts.cafeSwapV2Router02);
    ...
    {
        _cafeSwapSwapTokenByAddress(PolygonTokens.getTokenAddress(_tokenIn), PolygonTokens.getTokenAddress(_tokenOut), _amount);
    }
    ...
    {
        return _cafeSwapGetAmountOutByAddress(PolygonTokens.getTokenAddress(_tokenIn), PolygonTokens.getTokenAddress(_tokenOut), _amount);
    }

    function _cafeSwapGetAmountOutByAddress(address _tokenInAddress, address _tokenOutAddress, uint256 _amount) private view returns(uint256)
    {
        return _cafeSwapGetAmountOutByPath(_createAddressPath(_tokenInAddress, _tokenOutAddress), _amount);
    }

    function _cafeSwapGetAmountOutByPath(address[] memory _path, uint256 _amount) private view returns(uint256)
    {
        uint256[] memory amountsOut = tradingInterface.getAmountsOut(
            _amount,
            _path
    ...
    function _cafeSwapSwapTokenByAddress(address _tokenInAddress, address _tokenOutAddress, uint256 _amount) private
    {
        if (_amount == 0)
        {
            return;
        ...
        _cafeSwapApproveSpending(_tokenInAddress);
        ...
        uint256 amountOut = _cafeSwapGetAmountOutByPath(path, _amount);
    ...
    function _cafeSwapApproveSpending(address _tokenAddress) private
    {
        if (tokens[_tokenAddress].spendingIsApproved)
        {
            return;
