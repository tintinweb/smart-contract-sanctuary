...
contract PooLevelHandler is Base, Random, ILevelHandler {
    ...
    function checkLevelUp(uint _id, uint _newExperience) public override onlyAllowedContracts {
        ...
        if (_newExperience >= IPoos(PoosContract).getPooByIndex(_id).experienceForNextLevel) {
            levelUp(_id);
        }
    }
    ...
    function levelUp(uint _id) internal {
        ...
        poo.level = poo.level + 1;
        poo.currentExperience = poo.currentExperience - poo.experienceForNextLevel;
        poo.experienceForNextLevel = (poo.experienceForNextLevel * 11) / 10;
        poo.hitPoints = poo.hitPoints + randModulus(10, poo.level);
        poo.basicDmg = poo.basicDmg + randModulus(3, poo.hitPoints);
        poo.stamina = poo.stamina + randModulus(3, poo.basicDmg);
        poo.attackPower = poo.attackPower + randModulus(3, poo.stamina);
        poo.defense = poo.defense + randModulus(3, poo.attackPower);
        poo.initiative = poo.initiative + randModulus(3, poo.defense);
        poo.agility = poo.agility + randModulus(3, poo.initiative);
        if (poo.level % 10 == 0 && poo.luck < 5) {
            poo.luck = poo.luck + 1;
        }
        IPoos(PoosContract).executeLevelUp(poo.id,
            [poo.level, poo.currentExperience,
            poo.experienceForNextLevel, poo.hitPoints, poo.basicDmg, poo.stamina,
            poo.attackPower, poo.defense, poo.initiative, poo.agility, poo.luck]);
    }
}
