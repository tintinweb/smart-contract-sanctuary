...
contract MOX is Ownable, ERC721, ERC721Enumerable {
     ...
     * 
     */ 
    constructor(string memory name, string memory symbol, string memory baseTokenURI, address MOXbucksContract) ERC721(name, symbol) {
        _baseTokenURI = baseTokenURI;
		_owner = _msgSender();
		indexType = 0;
		bucksContract = IERC20(MOXbucksContract);
    ...
    /**
     * @dev Modified safeTransferFrom with 3 parameters
     *
     */
    function superSafeTransferFrom(address from, address to, uint256 tokenId) external returns(bool) {
        _transfer(from, to, tokenId);
        
        if (msg.sender != _marketContractAddress) {
            _approve(to, tokenId);
        }
     ...
     * Holds a percent of MOXbucks for distribution. 
     * Updates the mappings based on MOXshares holders' balances.     
     */ 
    function mintNFT(uint32 _type) public payable {
		...
		require(bucksContract.balanceOf(msg.sender) >= getPrice(_type), "Not enough tokens");
        
        bucksContract.transferFrom(msg.sender, address(this), getPrice(_type));
