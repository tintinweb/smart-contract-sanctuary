...
contract CoinFlipPoly is VRFConsumerBaseUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    // Events
    event BetPlaced(uint indexed betId, address indexed gambler, bool indexed choice, uint amount, address token);
    event BetSettled(uint indexed betId, address indexed gambler, bool indexed choice, uint amount, address token, bool outcome, uint winAmount);
    event BetRefunded(uint indexed betId, address indexed gambler);
    event FlipCoin(uint indexed betId, bytes32 currentRequestId, uint timestamp);
    ...
    // Get wealth tax 
    function getWealthTax(address token, uint amount) private view returns (uint wealthTax) {
        if (token == address(0)) {
    ...
    function getWinAmount(address token, uint amount) private view returns (uint winAmount) {
        uint wealthTax = token == PIRATE ? 0 : getWealthTax(token, amount);
        uint houseEdge = amount * (HOUSE_EDGE_PERCENT + wealthTax) / 100;
        winAmount = (amount - houseEdge) * 2;
    }
     ...
     * CORE GAME LOGIC
     */
    function takeBetToken(address token, uint amount, bool playerChoice) public nonReentrant {
        require(amount >= tokenMinBet[token], "minimum 50 token");
        require(amount <= tokenMaxBet[token], "maximum of 3000 tokens");
        require(ERC20(token).balanceOf(address(msg.sender)) >= amount, "Player: Not enough balance.");
        
        uint totalPayout = getWinAmount(token, amount);
        ...
        emit BetPlaced(betsLength, msg.sender, playerChoice, amount, token);

        betsLength++;
    }

    function takeBet(bool playerChoice) public payable nonReentrant {
        require(msg.value >= 1 ether, "minimum 1 MATIC");
        require(msg.value <= 1500 ether, "maximum of 1500 MATIC");

        uint totalPayout = getWinAmount(address(0), msg.value);
        ...
        // Record bet in event logs
        emit BetPlaced(betsLength, msg.sender, playerChoice, msg.value, address(0));
    ...
    function settleBet(bytes32 requestId, uint randomNumber) internal nonReentrant {
        ...
        bool result = randomNumber % 2 == 0; // 0 is heads
        bool won = b.playerChoice == result;

        // unlocking funds regardless of win or lose
        BANK.unlockFunds(b.token, possibleWinAmount);

        if (won) {
            if (b.token == address(0)) {
                BANK.withdrawTo(address(0), possibleWinAmount, player);
            } else {
                BANK.withdraw(b.token, possibleWinAmount);
                require(ERC20(b.token).transfer(player, possibleWinAmount));
            }
        } else {
            possibleWinAmount = 0;
        }
        ...
        b.isSettled = true;
        b.winAmount = possibleWinAmount;
        b.randomNumber = randomNumber;
        b.outcome = result;

        emit BetSettled(betId, player, b.playerChoice, b.betAmount, b.token, result, possibleWinAmount);
    }
