...
abstract contract UniswapFlashSwapper {
    ...
    function traingularFlashSwapHelper(
        ...
        try IUniswapV2Pair(_payPairAddress).swap(amount0Out, amount1Out, address(this), data) {
            emit Log("IUniswapV2Pair Successful swap");
            success = true;
        } catch {
            emit Log("IUniswapV2Pair Unsuccessful Swap");
            success = false;
        }
    }
...
contract FlashSwapper is UniswapFlashSwapper {
   ...
   // JUST FOR TESTING - ITS OKAY TO REMOVE ALL OF THESE VARS
    ...
    function _flashSwap(
      ...
      emit Log("Start Swap");
      emit Log(string(abi.encodePacked("Pair Address", _tokenBorrow)));
      try this.startSwap(_tokenBorrow, _pairAddress, _tokenPay, _amount, _userData) {
        emit Log("Successful Flash Swap");
      } catch {
        emit Log("Unsuccessful Flash Swap");
      }
