...
contract LMLTAirdrop is Ownable, Pausable {
using SafeMath for uint256;
 ...
 struct Participant {  
   ...
   * to the airdrop list with the default of 1000 LMLT
   */
  function bulkAddAddressesToAirDrop(address[] memory _addressesToAdd)
  ifNotPaused
  public
  {
   ...
   */
  function bulkAddAddressesToAirDropWithAward(address[] memory _addressesToAdd, uint256 _tokenAward)
  ifNotPaused
  public
  {
    require(_addressesToAdd.length > 0);
  ...
  internal
  {
      ...
      contrib.tokenAmount = _tokenAward.mul(1e18);
   ...
   * airdrop list with the default of 1000 LMLT
   */
  ...
  internal
  {
      ...
      require(!isAddressInAirdropList(_addressToAdd));
      Contribution storage contrib = contributions[_addressToAdd];
      contrib.tokenAmount = 1000 * 1e18;
      contrib.wasClaimed = false;
      contrib.isValid = true;
  }

   /**
   * @dev _addAddressToByCustomRewardAirdrop provides the function of adding an address to the
   * airdrop list with the default of 1000 LMLT
   */
  function bulkAddParticipantsWithReward(Participant[] memory participants)
  onlyOwner
  external 
  {
    ...
    for (uint i = 0; i < participants.length; i++) {
        require(participants[i].wallet != address(0));
        require(!isAddressInAirdropList(participants[i].wallet));
        Contribution storage contrib = contributions[participants[i].wallet];
        contrib.tokenAmount = participants[i].tokenAmount * 1e18;
        contrib.wasClaimed = false;
        contrib.isValid = true;
    }
   ...
   */
  function bulkRemoveAddressesFromAirDrop(address[] memory _addressesToRemove)
  ifNotPaused
  public
  {
...
function setAirdropAddressWasClaimed(address _addressToChange, bool _newWasClaimedValue)
  ...
  public
  {
    require(_addressToChange != address(0));
...
contract LimelightAirDrop is LMLTAirdrop {
  using SafeMath for uint256;

  address initTokenContractAddress = 0x1F435c17d7Ff631218827F9BA43a696650a9D512;
  
  constructor()
  LMLTAirdrop(initTokenContractAddress)
  public  
  {}
