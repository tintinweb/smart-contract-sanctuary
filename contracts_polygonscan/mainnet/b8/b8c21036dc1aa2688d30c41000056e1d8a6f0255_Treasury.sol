...
contract Treasury is ITreasury, Ownable, ReentrancyGuard {
    ...
    function globalCollateralValue() public view returns (uint256) {
        ...
        for (uint256 i = 0; i < pools_array.length; i++) {
            if (pools_array[i] != address(0)) {
                total_collateral_value += IPool(pools_array[i]).collateralDollarBalance();
            }
        }
    ...
    function calcEffectiveCollateralRatio() public view returns (uint256) {
        ...
        uint256 total_supply_dollar = ((IERC20(dollar).totalSupply() * dollarPrice()) / PRICE_PRECISION); 
    ...
    function refreshCollateralRatio() public withOracleUpdates {
        ...
        if (current_dollar_price > priceTarget + price_band) {
            ...
            target_collateral_ratio = target_collateral_ratio + ratio_step >= COLLATERAL_RATIO_MAX
    ...
    function _swap(
        ...
        uint256[] memory out_amounts = IUniswapRouter(uniswap_router).swapExactTokensForTokens(
            _input_amount,
            _min_output_amount,
            _path,
            address(this),
    ...
    function recollateralize(uint256 _share_amount, uint256 _min_collateral_amount)
        external
        onlyStrategist
        withOracleUpdates
        notMigrated
    ...
    function setOracleCollateralToken(address _oracle) public onlyOwner nonZeroAddress(_oracle){
