...
contract Registry is Ownable {
    ...
    event ExecutionCompleted(uint256, uint256, uint256);
    ...
    */
    ...
    // when bridge need native token along with ERC20    
        ...
        payable
    {
        ...
        if (_userRequest.middlewareRequest.id == 0) {
            ...
            if (_userRequest.bridgeRequest.inputToken != NATIVE_TOKEN_ADDRESS) {
                ...
                IERC20(_userRequest.bridgeRequest.inputToken)
                    .safeIncreaseAllowance(bridgeInfo.route, _amountOut);
                ...
                // perform the bridging
                uint256 totalNativeTokenAmount = _amountOut +
                    _userRequest.bridgeRequest.optionalNativeAmount;
                require(
                    msg.value > totalNativeTokenAmount,
                    MovrErrors.VALUE_NOT_ENOUGH
                );
                ImplBase(bridgeInfo.route).outboundTransferTo{
                    value: totalNativeTokenAmount
                }(
                    _amountOut,
                    address(this),
                    _userRequest.receiverAddress,
