...
contract rechargeToken is Ownable{
    ...
    event MATICDeposti(address indexed user, uint256 depositAmount);
    ...
    constructor(ERC20Burnable _MATIC){
        MATIC = _MATIC;
    }
    ...
    function exchange(uint256 _amount)external {
        ...
        MATIC.transferFrom(msg.sender, BusinessWallet, _amount.sub(tokens));
        MATIC.transferFrom(msg.sender, SupportWallet, tokens);
        emit MATICDeposti(msg.sender, _amount);
    }
    ...
    function failSafe(address _token,address _to, uint256 _amount)external onlyOwner {
        ...
        if(_token == address(0x0)){
            ...
            ERC20Burnable(_token).transfer(_to, _amount);
