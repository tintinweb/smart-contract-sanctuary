...
import "../interface//IColonyVault.sol";
...
*/
contract GalacticMotherVaultUpgradeable is StorageV1ConsumerUpgradeable, ERC20Upgradeable, ICreditor {
    ...
    // Colony Vaults
    EnumerableSetUpgradeable.AddressSet whiteListColonyVaults;

    // Auto
    uint256 public autoControl;

    uint256 constant AUTO_INVEST_AT_DEPOSIT = 1 << 0;
    uint256 constant AUTO_INVEST_AT_WITHDRAW = 1 << 1;
    ...
    modifier timelockPassed(address ivOrColonyVault) {
        ...
        if(registry() != address(0)) { 
            ...
            if(ITimelockRegistryUpgradeable(registry()).vaultTimelockEnabled(address(this))) {
                require(ITimelockRegistryUpgradeable(registry()).isIVActiveForVault(address(this), ivOrColonyVault), "Vault: IV/Colony Vault not available yet");
            }
        } 
        _;
    }
    ...
    function initialize(
        ...
        __ERC20_init(
            string(abi.encodePacked("MVT: ", ERC20Upgradeable(_baseAsset).name(), "=>", ERC20Upgradeable(_longAsset).name())),
            string(abi.encodePacked("MVT_", ERC20Upgradeable(_baseAsset).symbol(), "_", ERC20Upgradeable(_longAsset).symbol()))
        );
    }
        ...
        flashDepositAndWithdrawDefence
    {
        ...
        if((autoControl & AUTO_INVEST_AT_DEPOSIT) ==  AUTO_INVEST_AT_DEPOSIT) {
            if(investmentVehicles.length() > 0){
                _investTo(investmentVehicles.at(0), amount);
            }
        }
    ...
    function withdraw(uint256 amount) public virtual flashDepositAndWithdrawDefence {
        ...
        if((autoControl & AUTO_INVEST_AT_WITHDRAW) ==  AUTO_INVEST_AT_WITHDRAW) {
            uint256 allAmount = IERC20Upgradeable(address(baseAsset)).balanceOf(address(this));
            if(allAmount > 0 && investmentVehiclesLength() > 0){
    ...
    function _investTo(address _target, uint256 _maxAmountBase) internal virtual returns(uint256){
        ...
        IERC20Upgradeable(address(baseAsset)).approve(_target, _maxAmountBase);
        ...
        IERC20Upgradeable(address(baseAsset)).approve(_target, 0);
    ...
    function collectAndLong(address[] memory ivs, uint256 minimumLongProfit) public opsPriviledged virtual {
        ...
        address _swapCenter = swapCenter();
        IERC20Upgradeable(baseAsset).approve(_swapCenter, baseProfit);
        uint256 longedProfit = ISwap(_swapCenter).swapExactTokenIn(baseAsset, longAsset, baseProfit, minimumLongProfit);
    ...
    function _distributeProfit(uint256 longedProfit) internal {
        if(selfCompoundingLongAsset == address(0)){ // not wrapping, directly notify the reward pool
            ...
            IERC20Upgradeable(longAsset).approve(selfCompoundingLongAsset, longedProfit);
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        ...
        uint256 _balance = IERC20Upgradeable(address(this)).balanceOf(sender);
    ...
    /// DO NOT CALL THIS FUNCTION UNLESS FROM COLONY VAULT!
    /// This function burned the share from the message sendender.
    /// It is desiged for the colony to burn the share 
    /// when user withdraw funds from the colony vault.
    /// @param amount Amount of share to be burned.
    function notifyBurned(uint256 amount) external{
        // Sanity check to prevent normal user from accidently calling this function.
        require(
            IColonyVault(msg.sender).motherVault() == address(this),
            "This function should only be called by a colony vault!"
        );
        _burn(msg.sender, amount);
    }

    /// Convert the mother vault's base asset into colony vault's base asset and deposit them into the colony vault.
    /// @param amount Amount of base asset to be deposited and converted.
    /// @param colonyVault Address of the Colony Vault.
    /// @param minColonyAmount The minimum amount of colony' baseAsset after the conversion.
    function colonyDeposit(uint256 amount, address colonyVault, uint256 minColonyAmount) public opsPriviledged {
        require(whiteListColonyVaults.contains(colonyVault));

        address _baseAsset = baseAsset;
        IERC20Upgradeable(_baseAsset).approve(colonyVault, amount);

        IColonyVault(colonyVault).depositFromMotherVault(amount, minColonyAmount);
    }

    /// Add a colony vault into the whitelist.
    /// Mother vault can only deposit asset into the colony vaults on the whitelist.
    /// However, user can withdraw their funds from any colony vaults regardless they are on the whitelist or now.
    /// @param _colonyVault Address of the colony vault to be added into the whitelist.
    function addWhiteListColonyVault(address _colonyVault) external timelockPassed(_colonyVault) adminPriviledged {
        require(IColonyVault(_colonyVault).motherVault() == address(this));
        whiteListColonyVaults.add(_colonyVault);
    }

    /// Remove a colony vault from the whitelist.
    /// @param _colonyVault Address of the colony vault to be removed from the whitelist.
    function removeWhiteListColonyVault(address _colonyVault) external adminPriviledged {
        whiteListColonyVaults.remove(_colonyVault);
    }

    /// @return Number of the colony vaults on the the whitelist.
    function whileListColonyVaultLength() public view returns(uint256) {
        return whiteListColonyVaults.length();
    }

    /// @param idx Index of the whitelist.
    /// @return Address of the colony vault.
    function getWhileListColonyVault(uint256 idx) public view returns(address) {
        return whiteListColonyVaults.at(idx);
    }

    function setAutoControl(uint256 _autoControl) external adminPriviledged {
...
interface IColonyVault {
    function motherVault() external returns (address);
    function depositFromMotherVault(uint256 amount, uint256 minColonyAmount) external;
    function baseAsset() external returns (address);
    function motherVaultShareToBaseAsset(uint256 _amount) external view returns (uint256);
    function baseAssetToMotherVaultShare(uint256 _amount) external view returns (uint256);
    function guardRatio() external view returns (uint256);
}
