...
// PolyKangaroo with Governance.
contract PolyKangaroo is BEP20 {
    ...
    // Min amount to liquify. (default 250 KANGAROOs)
    uint256 public minAmountToLiquify = 250 ether;
    // The swap router set to Quickswap and immuable
    IUniswapV2Router02 public SwapRouter = IUniswapV2Router02(0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff);
    // The trading pair
    address public dragonflySwapPair;
    // In swap and liquify
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxHolding() > 0 && sender != owner() && recipient != owner()) {
            if ( _excludedFromAntiWhale[recipient] == false ) {
                require(amount <= maxHolding() - balanceOf(recipient) && balanceOf(recipient) <= maxHolding(), "KANGAROO::antiWhale: Transfer amount would result in a balance bigger than the maxHoldingRate");
            }
        }
        _;
    }
    ...
    /**z
     * @notice Constructs the PolyKangaroo contract.
     */
    constructor() public BEP20("PolyKangaroo Token", "KANGAROO") {
    ...
    /// @dev overrides transfer function to meet tokenomics of KANGAROO
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        ...
        if (
            ...
            && dragonflySwapPair != address(0)
            && sender != dragonflySwapPair
            && sender != owner()
        ) {
            swapAndLiquify();
        }
        ...
        if (recipient == BURN_ADDRESS || transferTaxRate == 0) {
            ...
            require(taxAmount == burnAmount + liquidityAmount, "KANGAROO::transfer: Burn value invalid");

            // default 95% of transfer sent to recipient
            uint256 sendAmount = amount.sub(taxAmount);
            require(amount == sendAmount + taxAmount, "KANGAROO::transfer: Tax value invalid");
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the KangarooSwap pair path of token -> weth
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "KANGAROO::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateBurnRate(uint16 _burnRate) public onlyOperator {
        require(_burnRate <= 100, "KANGAROO::updateBurnRate: Burn rate must not exceed the maximum rate.");
    ...
    function updateMaxHoldingRate(uint16 _maxHoldingRate) public onlyOperator {
        require(_maxHoldingRate >= 100, "KANGAROO::updateMaxHoldingRate: Max holding rate must not be below the minimum rate.");
    ...
    function updateSwapPair() public onlyOperator {
        dragonflySwapPair = IUniswapV2Factory(SwapRouter.factory()).getPair(address(this), SwapRouter.WETH());
        require(dragonflySwapPair != address(0), "KANGAROO::updateSwapPair: Invalid pair address.");
        emit SwapRouterUpdated(msg.sender, address(SwapRouter), dragonflySwapPair);
    }
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "KANGAROO::transferOperator: new operator is the zero address");
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "KANGAROO::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "KANGAROO::delegateBySig: invalid nonce");
        require(now <= expiry, "KANGAROO::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "KANGAROO::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying KANGAROOs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "KANGAROO::_writeCheckpoint: block number exceeds 32 bits");
