...
contract Invite_TeamWallet is Context, Ownable {
   
    function sendTokens(address token, uint256 amount) public onlyOwner() {
        IERC20(token).transfer(owner(), amount);
    }
   
    function getTeamWalletBalance(address token) public view returns(uint256){
