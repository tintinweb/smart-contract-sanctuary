    ...
    ERC2771Context
{
    ...
    function contractURI() public pure returns (string memory) {
        return
            "https://us-central1-artis-life-network.cloudfunctions.net/contractURIv2";
    }
...
contract NFTAirdrop {
    ...
    struct Airdrop {
        ...
        mapping(uint256 => address) recipientAddresses;
    }
    ...
    constructor() {
        ...
        nftMinter = ArtisLifeNFT(0xA34889128229D6E829167dB5D8A979F725E66eC4);
    }
    ...
    function addRecipient(bytes calldata airdropID, address recipientAddress)
        public
        onlyAdmin
        airdropExists(airdropID)
    {
        ...
        airdrops[airdropID].totalSupply = airdrops[airdropID].totalSupply.add(
            1
        );
        airdrops[airdropID].recipients[recipientAddress] = airdrops[airdropID]
            .totalSupply;
        airdrops[airdropID].recipientAddresses[
            airdrops[airdropID].totalSupply
        ] = recipientAddress;
    ...
    function masterMint(bytes calldata airdropID)
        external
        onlyAdmin
        airdropExists(airdropID)
    {
        uint256 nftCount = airdrops[airdropID].totalSupply;
        for (uint256 i = 0; i < nftCount; i++) {
            address recipient = airdrops[airdropID].recipientAddresses[i];
            if (recipient != address(0) && airdrops[airdropID].hasMinted[recipient] == false) {
                uint256 edition = airdrops[airdropID].recipients[recipient];
                nftMinter.mintFor(
                    recipient,
                    airdrops[airdropID].nftID,
                    1,
                ...
                emit AirdropRedeemed(airdropID, recipient);
            }
        }
    }

    function setNewMinter(address newMinter) public onlyAdmin {
