...
contract MeebFarmingBank is IERC721Locker, OwnableUpgradeable, ReentrancyGuard, ContractGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of MEEBs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accMeebPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accMeebPerShare` (and `lastRewardTime`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. MEEBs to distribute per block.
        uint256 lastRewardTime; // Last timestamp that MEEBs distribution occurs.
        uint256 accMeebPerShare; // Accumulated MEEBs per share, times 1e18. See below.
        address strategy; // Strategy address, use LP to farm and buy back MEEB
    ...
    // The block number when MEEB mining starts.
    ...
    function initialize(
        ...
        devRate = 2647; // 26.47%
        gameTreasuryRate = 1765; // 17.65%
        jackpotRate = 588; // 5.88%
        insuranceRate = 588; // 5.88%

        totalRewardPerSecond = _totalRewardPerSecond; // 0.278224 MEEB/seconds
        _updateRewardPerSecond();
    ...
    function replacePoolLpToken(uint256 _pid, IERC20 _lpToken) external onlyOwner {
        ...
        require(pool.accMeebPerShare == 0 && pool.totalLpSupply == 0, "Cant replace running pool");
        massUpdatePools();
    ...
    function changeStrategy(uint256 _pid, address _newStrategy) external onlyOwner {
        ...
        if (_newStrategy != address(0)) {
            ...
            require(IStrategy(_newStrategy).targetProfitToken() == meeb, "!targetProfitToken");
    ...
    // View function to see pending MEEBs on frontend.
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accMeebPerShare = pool.accMeebPerShare;
        uint256 lpSupply = pool.totalLpSupplyBoosted;
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
            uint256 _seconds = block.timestamp - pool.lastRewardTime;
            if (totalAllocPoint > 0) {
                uint256 _meebReward = (_seconds * rewardPerSecond * pool.allocPoint) / totalAllocPoint;
                accMeebPerShare += (_meebReward * 1e18) / lpSupply;
            }
        }
        uint256 _totalReward = (_getAccountBoostedAmount(_user, user.amount) * accMeebPerShare) / 1e18;
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalAllocPoint > 0) {
            uint256 _seconds = block.timestamp - pool.lastRewardTime;
            uint256 _meebReward = (_seconds * rewardPerSecond * pool.allocPoint) / totalAllocPoint;
            pool.accMeebPerShare += (_meebReward * 1e18) / lpSupply;
        }
        pool.lastRewardTime = block.timestamp;
    }
    ...
    function _harvestReward(uint256 _pid, address _account) internal {
        ...
        if (_amount > 0) {
            ...
            PoolInfo storage pool = poolInfo[_pid];
            uint256 _totalReward = (_amount * pool.accMeebPerShare) / 1e18;
            ...
            if (_claimableAmount > 0) {
                require(_claimableAmount * 100 <= IERC20(meeb).totalSupply(), "Suspicious big reward amount!!"); // <= 1% total supply
                emit RewardPaid(_account, _claimableAmount);

                _topupFunds(_claimableAmount);

                _safeMeebMint(address(this), _claimableAmount);
                ...
                _safeMeebTransfer(_account, _claimableAmount);
            }
        }
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public notContract onlyOneBlock nonReentrant checkHalving {
        ...
        user.rewardDebt = (_getAccountBoostedAmount(msg.sender, user.amount) * pool.accMeebPerShare) / 1e18;
    ...
    function _withdraw(
        ...
        user.rewardDebt = (_getAccountBoostedAmount(_account, user.amount) * pool.accMeebPerShare) / 1e18;
    ...
    function _safeMeebTransfer(address _to, uint256 _amount) internal {
        uint256 _meebBal = IERC20(meeb).balanceOf(address(this));
        if (_meebBal > 0) {
            if (_amount > _meebBal) {
                IERC20(meeb).safeTransfer(_to, _meebBal);
            } else {
                IERC20(meeb).safeTransfer(_to, _amount);
            }
        }
    }

    function _safeMeebMint(address _to, uint256 _amount) internal {
        address _meeb = meeb;
        if (ICappedMintableBurnableERC20(_meeb).minter(address(this)) && _to != address(0)) {
            uint256 _totalSupply = IERC20(_meeb).totalSupply();
            uint256 _cap = ICappedMintableBurnableERC20(_meeb).cap();
            uint256 _mintAmount = (_totalSupply + _amount <= _cap) ? _amount : (_cap - _totalSupply);
            if (_mintAmount > 0) {
                ICappedMintableBurnableERC20(_meeb).mint(_to, _mintAmount);
            }
        }
    }

    function _safeMeebBurn(uint256 _amount) internal {
        uint256 _meebBal = IERC20(meeb).balanceOf(address(this));
        if (_meebBal > 0) {
            if (_amount > _meebBal) {
                ICappedMintableBurnableERC20(meeb).burn(_meebBal);
            } else {
                ICappedMintableBurnableERC20(meeb).burn(_amount);
            }
        }
    }
    ...
    function _topupFunds(uint256 _claimableAmount) internal {
        ...
        uint256 _totalMintAmount = _devAmount + _gameTreasuryAmount + _jackpotAmount + _insuranceAmount;
        if (
            ICappedMintableBurnableERC20(_meeb).minter(address(this)) &&
            IERC20(_meeb).totalSupply() + _totalMintAmount <= ICappedMintableBurnableERC20(_meeb).cap()
        ) {
            ICappedMintableBurnableERC20(_meeb).mint(devFund, _devAmount);
            ICappedMintableBurnableERC20(_meeb).mint(gameTreasuryFund, _gameTreasuryAmount);
            ICappedMintableBurnableERC20(_meeb).mint(jackpotFund, _jackpotAmount);
            ICappedMintableBurnableERC20(_meeb).mint(insuranceFund, _insuranceAmount);
    ...
    function _massUpdatePoolsWithNftLocked(address _account, uint256 _newLockedTokenId) internal {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            ...
            uint256 _amount = userInfo[pid][_account].amount;
            if (_amount > 0) {
