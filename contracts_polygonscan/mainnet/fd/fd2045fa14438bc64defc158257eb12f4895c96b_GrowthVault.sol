...
contract GrowthVault is ERC20, Ownable {
    ...
    function getStrategyInfo(uint256 _nameID) public view returns (
        ...
        _totalCapital = strategyObj.totalCapital;
        _totalLP = strategyObj.totalLP;
    }
        ...
        for (uint256 i; i < names.length; i++) {
            ...
            if(totalLP > 0) {
                ...
                if(strategyObj.token0 == WETH) {
                    ...
                    strategyObj.totalCapital = _getAmountsOut(strategyObj.liquidityRouter, _token1Value, path);
                    totalCapital += strategyObj.totalCapital;
                } else if (strategyObj.token1 == WETH) {
                    path[0] = strategyObj.token0;
                    strategyObj.totalCapital = _getAmountsOut(strategyObj.liquidityRouter, _token0Value, path);
                    totalCapital += strategyObj.totalCapital;
                } else {
                    path[0] = strategyObj.token0;
                    strategyObj.totalCapital = _getAmountsOut(strategyObj.liquidityRouter, _token0Value, path);
    ...
    function _getAmountsOut(
        address _liquidityRouter, uint256 _amount, address[] memory path) internal view returns (uint256){
        uint256[] memory amounts;
        amounts = IRouter(_liquidityRouter).getAmountsOut(_amount, path);
        return amounts[1];
    }

    function _approveToken(address _token, address _to) internal {
    ...
    function autoCompound() public {
        ...
        for (uint256 i; i < names.length; i++) {
            ...
            amount = _getAmountLPFromStrategy(strategyObj.farm, strategyObj.pid);
            
            if (amount > 0) {
                _withdrawFromStrategy(_nameID, amount);
                _approveToken(rewardToken, strategyObj.liquidityRouter);
                _approveToken(WETH, strategyObj.liquidityRouter);
                _approveToken(USDT, strategyObj.liquidityRouter);

                rewardToken = _getCakeToken(strategyObj.farm);
                rewardBalance = _getBalanceOfToken(rewardToken);
                path[0] = rewardToken;
                path[1] = WETH;
                amount = _getAmountsOut(strategyObj.liquidityRouter, rewardBalance, path);
                if(amount > 100) {
                ...
                path[0] = WETH;
                path[1] = USDT;
                rewardsFeeInWETH = _calculateAmountFee(_getBalanceOfToken(WETH));
                amount = _getAmountsOut(strategyObj.liquidityRouter, rewardsFeeInWETH, path);
                if(amount > 100) {
                    _swapExactTokensForTokens(
                        strategyObj.liquidityRouter,
                        rewardsFeeInWETH,
                ...
                path1[0] = rewardToken;
                path1[1] = strategyObj.token0;
                path1[2] = strategyObj.token1;
                ...
                if(amounts[1] > 100 && amounts[2] > 100) {
                    ...
                    if(strategyObj.token0 == WETH) {
                        ...
                        _potentialBalance0 = _getBalanceOfToken(strategyObj.token0);
                    }
                    if(strategyObj.token1 == WETH) {
                        _potentialBalance1 = getAvalaibleWETH();
                    } else {
                        _potentialBalance1 = _getBalanceOfToken(strategyObj.token1);
                    }

                    path[0] = strategyObj.token0;
                    path[1] = strategyObj.token1;
                    _addLiquidity(strategyObj.liquidityRouter,
                        strategyObj.lpToken,
                        strategyObj.token0,
                        strategyObj.token1,
                        _potentialBalance0,
    ...
    function withdraw(uint256 _shares) public {
        ...
        require(_balance > 0, "Too low shares for withdrawing. Try to withdraw more shares");
    ...
    function _addLiquidity(
        ...
        uint256 desired1;
        uint256[] memory amounts;
        amounts = IRouter(_liquidityRouter).getAmountsOut(_amount0, path);
        if(amounts[amounts.length - 1] <= _amount1) {
            desired1 = amounts[1];
        } else {
            desired1 = _amount1;
        }
        IERC20(_token0).transfer(_lpToken, _amount0);
    ...
    function _swapToYELs(uint256 _percent) internal { 
        uint256 _nameID;
        uint256 totalLP;
        address cakeToken;
        uint256 cakeBalance;
        ...
        for (uint256 i; i < names.length; i++) {
            ...
            totalLP = _getAmountLPFromStrategy(strategyObj.farm, strategyObj.pid);
            if (totalLP > 0) {
                _withdrawFromStrategy(_nameID, (_percent * totalLP) / (100 * 10 ** 12));

                // swap cakes to WETH
                cakeToken = _getCakeToken(strategyObj.farm);
                path[0] = cakeToken;
                path[1] = WETH;
                cakeBalance = getAvalaibleRewards(cakeToken);
                IERC20(cakeToken).safeApprove(strategyObj.liquidityRouter, 0);
                IERC20(cakeToken).safeApprove(strategyObj.liquidityRouter, type(uint256).max);
                ...
                _swapTokensToNativeToken(strategyObj.token0, strategyObj.token1, strategyObj.liquidityRouter);

                // check difference of percentage should be no more than 1%
                currentPercent = getAvalaibleWETH() * 100 * 10**12 / totalCapital;
    ...
    function getAvalaibleRewards(address _tokenReward) public view returns (uint256) {
    ...
    function _swapTokensToNativeToken(
        ...
        if(_token0 != WETH) {
            path[0] = _token0;
            amounts = IRouter(_liquidityRouter).getAmountsOut(
                _getBalanceOfToken(path[0]), path
            );
            if(amounts[1] > 100) {
                _swapExactTokensForTokens(
                    _liquidityRouter,
                    _getBalanceOfToken(path[0]),
                    amounts[1],
                    path);
            }
        }
