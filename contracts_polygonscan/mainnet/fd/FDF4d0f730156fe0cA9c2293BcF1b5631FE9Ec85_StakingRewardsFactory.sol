...
contract StakingRewardsFactory is Ownable {
    ...
    function deploy(
        ...
        rewardTokens[_stakingToken] = _rewardTokens;
        rewardAmounts[_stakingToken] = _rewardAmounts;
        info.rewardsDuration = _rewardsDuration;
        info.vestingPeriod = _vesting;
        info.claimable = _claimable;
    ...
    function notifyRewardAmount(address _stakingToken) public {
        ...
        address[] memory _rewardTokens = rewardTokens[_stakingToken];
        uint256[] memory _rewardAmounts = rewardAmounts[_stakingToken];
        for (uint256 i = 0; i < _rewardTokens.length; i++) {
            uint256 rewardAmount = _rewardAmounts[i];
            if (rewardAmount > 0) {
                rewardAmounts[_stakingToken][i] = 0;
                require(
                    IERC20(_rewardTokens[i]).transfer(info.stakingRewards, rewardAmount),
                    'StakingRewardsFactory::notifyRewardAmount: transfer failed'
                );
                StakingRewards(info.stakingRewards).notifyRewardAmount(_rewardTokens[i], rewardAmount);
            }
        }
    }

    // Rescue leftover funds from pool
    function rescueFunds(address _stakingToken, address _tokenAddress) public onlyOwner {
        StakingRewardsInfo storage info = stakingRewardsInfoByStakingToken[_stakingToken];
        require(info.stakingRewards != address(0), 'StakingRewardsFactory::notifyRewardAmount: not deployed');
        StakingRewards(info.stakingRewards).rescueFunds(_tokenAddress, msg.sender);
    }
...
contract StakingRewards is IStakingRewards, RewardsDistributionRecipient, ReentrancyGuard, NativeMetaTransaction {
        ...
        return
            rewardsPerTokenMap[_rewardToken].add(
                lastTimeRewardApplicable()
                    ...
                    .mul(tokenRewardRate[_rewardToken])
                    .mul(1e18)
                    .div(_totalSupply)
            );
    }
    ...
    function tokensEarned(address _account) public view returns (address[] memory, uint256[] memory) {
    ...
    function tokensRewardRate() public view returns (address[] memory, uint256[] memory) {
        uint256 _length = rewardTokens.length;
    ...
    function getRewardForDuration(address _rewardToken) external view override returns (uint256) {
    ...
    function stakeWithPermit(
        ...
        // permit
        IUniswapV2ERC20(address(stakingToken)).permit(_msgSender(), address(this), _amount, _deadline, _v, _r, _s);
    ...
    function getReward() public override nonReentrant updateReward(_msgSender()) {
        ...
        for (uint256 i = 0; i < totalTokens; i++) {
            ...
            if (claimedSplitsForUser == 0 && !hasClaimed[_msgSender()]) {
                ...
                totalVestedRewardForUser[_msgSender()][rewardTokens[i]] = rewards[_msgSender()][rewardTokens[i]].sub(
                    reward
                );
            }
    ...
    {
        uint256 rewardRate = tokenRewardRate[_rewardToken];
        ...
        rewardLastUpdatedTime[_rewardToken] = block.timestamp;
        tokenRewardRate[_rewardToken] = rewardRate;
        periodFinish = block.timestamp.add(rewardsDuration);
        emit RewardAdded(_reward);
    }

    function rescueFunds(address _tokenAddress, address _receiver) external onlyRewardsDistribution {
...
interface IStakingRewards {
    ...
    function rewardPerToken(address _rewardToke) external view returns (uint256);
