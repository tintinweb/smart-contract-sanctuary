...
contract PolyChickenSwap is BEP20('Poly ChickenSwap', 'PolyChickenSwap') {
...
contract MasterChef is Ownable {
    ...
    PolyChickenSwap public ChickenSwap;
    // Dev address.
    address public devaddr;
    // EGG tokens created per block.
    uint256 public ChickenSwapPerBlock;
    // Bonus muliplier for early ChickenSwap makers.
    ...
    constructor(
        PolyChickenSwap _ChickenSwap,
        address _devaddr,
        address _feeAddress,
        uint256 _ChickenSwapPerBlock,
        uint256 _startBlock
    ) public {
        ChickenSwap = _ChickenSwap;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        ChickenSwapPerBlock = _ChickenSwapPerBlock;
    ...
    function pendingEgg(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 ChickenSwapReward = multiplier.mul(ChickenSwapPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accEggPerShare = accEggPerShare.add(ChickenSwapReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 ChickenSwapReward = multiplier.mul(ChickenSwapPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        ChickenSwap.mint(devaddr, ChickenSwapReward.div(10));
        ChickenSwap.mint(address(this), ChickenSwapReward);
        pool.accEggPerShare = pool.accEggPerShare.add(ChickenSwapReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    // Safe ChickenSwap transfer function, just in case if rounding error causes pool to not have enough EGGs.
    function safeEggTransfer(address _to, uint256 _amount) internal {
        uint256 ChickenSwapBal = ChickenSwap.balanceOf(address(this));
        if (_amount > ChickenSwapBal) {
            ChickenSwap.transfer(_to, ChickenSwapBal);
        } else {
            ChickenSwap.transfer(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _ChickenSwapPerBlock) public onlyOwner {
        massUpdatePools();
        ChickenSwapPerBlock = _ChickenSwapPerBlock;
    }
