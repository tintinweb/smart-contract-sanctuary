...
contract TankChef is Ownable {
    ...
    uint256 public immutable burnMultiplier;

    // Block smart contracts. Warning: Also disables multisig wallets
    bool public blockContracts;

    event Deposit(address indexed user, uint256 amount);
    event Withdraw(address indexed user, uint256 amount);
    event EmergencyWithdraw(address indexed user, uint256 amount);
    event DisableContracts(bool block);
    ...
    function beforeAdjustBlockEnd() external onlyOwner {
        currentAmount = rewardToken.balanceOf(address(this));
    }

    function adjustBlockEnd() external onlyOwner {
        require(currentAmount > 0, "call beforeAdjustBlockEnd() first");
        uint256 totalLeft = rewardToken.balanceOf(address(this)).sub(currentAmount);
    ...
    function deposit(uint256 _amount) external {
        if (blockContracts) {
            require(msg.sender == tx.origin, "contracts blocked");
        }
    ...
    function disableContracts(bool _block) external onlyOwner {
        blockContracts = _block;
        emit DisableContracts(_block);
    }
