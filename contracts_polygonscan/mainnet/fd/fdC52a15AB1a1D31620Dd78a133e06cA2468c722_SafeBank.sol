...
contract SafeBank is ReentrancyGuard {
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "SafeBank: caller is not the operator");
        _;
    }

    modifier onlyTimelock() {
        require(timelock == msg.sender, "SafeBank: caller is not timelock");
        _;
    }

    modifier notInitialized() {
        require(!initialized, "SafeBank: already initialized");
        _;
    }
    ...
    function addRewardPool(address _rewardToken, uint256 _rewardPerBlock) public nonReentrant onlyOperator {
        require(rewardPoolInfo.length <= 16, "SafeBank: Reward pool length > 16");
        massUpdatePools();
    ...
    function setUnstakingFrozenTime(uint256 _unstakingFrozenTime) external nonReentrant onlyOperator {
        require(_unstakingFrozenTime <= 7 days, "SafeBank: !safe - dont lock for too long");
    ...
    function withdraw(uint256 _pid, uint256 _wantAmt) public nonReentrant notContract {
        ...
        require(user.shares > 0, "SafeBank: user.shares is 0");
        require(sharesTotal > 0, "SafeBank: sharesTotal is 0");
        ...
        if (_wantAmt > 0) {
            ...
            if (_wantAmt > 0) {
                require(whitelisted[msg.sender] || block.timestamp >= unfrozenStakeTime(_pid, msg.sender), "SafeBank: frozen");
    ...
    function setTimelock(address _timelock) external {
        require(msg.sender == timelock || (timelock == address(0) && msg.sender == operator), "SafeBank: !authorised");
        timelock = _timelock;
    }
    ...
    function executeTransaction(address target, uint256 value, string memory signature, bytes memory data) external onlyTimelock returns (bytes memory) {
        ...
        require(success, "SafeBank::executeTransaction: Transaction execution reverted.");
