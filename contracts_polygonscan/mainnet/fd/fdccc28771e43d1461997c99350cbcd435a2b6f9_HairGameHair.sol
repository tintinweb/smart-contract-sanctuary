...
contract HairGameHair is VRFConsumerBase, Ownable, ReentrancyGuard {
    ...
    // Each bet is deducted 200 basis points (2%) in favor of the house
    ...
    // Place bet
    function placeBet(uint256 amount, uint readibleBet, uint betMask, uint modulo, address referrer) external nonReentrant {
        ...
        emit BetPlaced(bets.length, msg.sender, amount, uint8(readibleBet), uint8(modulo), uint8(rollUnder), uint40(mask));	
        // Store bet in bet list.	
        bets.push(Bet(	
            {	
                amount: amount,	
                readibleBet: uint8(readibleBet),	
                modulo: uint8(modulo),	
