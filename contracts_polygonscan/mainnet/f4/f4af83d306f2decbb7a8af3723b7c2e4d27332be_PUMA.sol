...
contract PUMA is ERC20 {
    // Transfer tax rate in basis points. (default 15%)
    uint16 public transferTaxRate = 47;
     ...
     */
    constructor() public ERC20("PolyPuma Token", "PUMA") {
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, uint16 _depositFeeBP, bool _withUpdate) public onlyOwner {
        require(_depositFeeBP <= 700, "add: invalid deposit fee basis points");
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (_amount > 0) {
            ...
            if (address(pool.lpToken) == address(token)) {
                uint256 transferTax = _amount.mul(token.transferTaxRate()).div(10000);
