...
contract Strategy_USDC_DERC is Ownable {
    ...
    address public router = 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff; // uses for swapping USDC - DERC
    ...
    address public router_WMATIC_USDC = 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff; // uses for swapping USDC - WMATIC
    ...
    function autoCompound() external {
        ...
        if (getAmountLPFromFarm() > 0) {
            ...
            bool result = _swapExactTokenForToken(routerReward, QUICKToken, WMATIC, _balance+pendingRewardsFee);
            if(result) {
                pendingRewardsFee = 0;
            } else{
    ...
    function setRouter_WMATIC_USDC(address _address) public onlyOwner {
        require(_address != address(0), "The address can not be zero address");
        router_WMATIC_USDC = _address;
    }
    ...
    function _getPriceWMATICToUSDC() internal view returns (uint256) {
        ...
        path[0] = WMATIC;
        path[1] = token0;
        return (50 * 1e36) / _getAmountsOut(router_WMATIC_USDC, 50 * 1e18, path) / 1e12;
    }

    function _getPriceWMATICToDERC() internal view returns (uint256 result) {
        // WMATIC to DERC through USDC
        address[] memory path = new address[](2);
        path[0] = WMATIC;
        path[1] = token0;

        uint256 amountUSDC = _getAmountsOut(router_WMATIC_USDC, 50 * 1e18, path);
        if (amountUSDC > 0) {
            path[0] = token0;
            path[1] = token1;
    ...
    function _getSimpleTCI(uint256 _amount) internal view returns (uint256 tCI) {
        if(_amount > 0) {
            ...
            tCI = _getPriceWMATICToDERC() * t1V / 1e18 + _getPriceWMATICToUSDC() * t0V / 1e6;
        }
        ...
        uint256 _balance1 = getBalanceOfToken(token1);
        if (_balance1 > 0) {
            tCI += _getPriceWMATICToDERC() * _balance1 / 1e18;
        }
    ...
    function _getSimpleTC() internal view returns (uint256 TC) {
        ...
        if(_totalLP > 0) {
            ...
            if(t0V + amount != 0) {
                path[0] = token0;
                path[1] = WMATIC;
                TC = _getAmountsOut(router_WMATIC_USDC, t0V + amount, path);
            }
        }

        // calculates total Capital from tokens that exist on the contract
        
        // ----> calculates DERC as USDC <----
        ...
        if(_balance + amount != 0) {
            path[0] = token0;
            path[1] = WMATIC;
            TC += _getAmountsOut(router_WMATIC_USDC, _balance + amount, path);
        }
