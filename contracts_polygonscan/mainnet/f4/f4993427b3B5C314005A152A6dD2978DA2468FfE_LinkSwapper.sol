...
contract LinkSwapper {
  ...
  constructor(IERC20 _token,
      ...
      token.approve(address(pegswap),token.totalSupply());
  ...
  receive() payable external {
    ...
    _approveTokenIfNeededPegSwap(address(token),swapedAmounts[1]);
    pegswap.swap(swapedAmounts[1],address(token),address(targetToken));
    targetToken.transfer(msg.sender,swapedAmounts[1]);
  }
  ...
  function _approveTokenIfNeededPegSwap(address _token,uint256 amount) internal {
      IERC20 tokenContract = IERC20(_token);
      uint256 approved = tokenContract.allowance(address(this),address(pegswap));
      if(approved < amount){
        tokenContract.approve(address(pegswap),tokenContract.totalSupply());
      }
  }
