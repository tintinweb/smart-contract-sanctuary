...
// File: contracts/interfaces/IHemetaBondingCalculator.sol


pragma solidity 0.7.5;

interface IHemetaBondingCalculator {
...
// File: contracts/interfaces/IHemetaTreasury.sol


pragma solidity 0.7.5;

interface IHemetaTreasury {
...
// File: contracts/HemetaBondDepository.sol
...
contract HemetaBondDepository is Ownable {
    ...
    /* ======== STATE VARIABLES ======== */
    address public immutable HETA; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints HETA when receives principle
    ...
    struct Bond {
        uint payout; // HETA remaining to be paid
    ...
    function deposit(
        ...
        uint value = IHemetaTreasury( treasury ).valueOfToken( principle, _amount );
        uint payout = payoutFor( value ); // payout to bonder is computed

        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 HETA ( underflow protection )
        ...
        /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) HETA
         */
        IERC20( principle ).safeTransferFrom( msg.sender, address(this), _amount );
        IERC20( principle ).approve( address( treasury ), _amount );
        IHemetaTreasury( treasury ).deposit( _amount, principle, profit );

        if ( fee != 0 ) { // fee is transferred to dao
            IERC20( HETA ).safeTransfer( DAO, fee );
        }
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            IERC20( HETA ).transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                IERC20( HETA ).approve( stakingHelper, _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return IERC20( HETA ).totalSupply().mul( terms.maxPayout ).div( 100000 );
    }
    ...
    function bondPriceInUSD() public view returns ( uint price_ ) {
        if( isLiquidityBond ) {
            price_ = bondPrice().mul( IHemetaBondingCalculator( bondCalculator ).markdown( principle ) ).div( 100 );
        } else {
    ...
    /**
     *  @notice calculate current ratio of debt to HETA supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns ( uint debtRatio_ ) {
        uint supply = IERC20( HETA ).totalSupply();
    ...
    function standardizedDebtRatio() external view returns ( uint ) {
        if ( isLiquidityBond ) {
            return debtRatio().mul( IHemetaBondingCalculator( bondCalculator ).markdown( principle ) ).div( 1e9 );
        } else {
            return debtRatio();
        }
    }
    ...
    /**
     *  @notice calculate amount of HETA available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or HETA) to the DAO
     *  @return bool
     */
