// contracts/Polymice.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.0;

import "@openzeppelin/contracts/token/ERC721/extensions/ERC721Enumerable.sol";
import "./IpCheeth.sol";
import "./PolymiceLibrary.sol";

contract Polymice is ERC721Enumerable {
    using PolymiceLibrary for uint8;
 ...
 |_|  |_|_|_||_\__|_|_||_\__, | |_| \_,_|_||_\__|\__|_\___/_||_/__/
   ...
   */
    ...
    /**
     * @dev Returns the current pcheeth cost of minting.
     */
    function currentpCheethCost() public view returns (uint256) {
    ...
    function mintInternal() internal {
        ...
        require(!PolymiceLibrary.isContract(msg.sender));
    ...
    function mintMouse() public {
        ...
        //Burn this much pcheeth
        IpCheeth(pcheethAddress).burnFrom(msg.sender, currentpCheethCost());
...
*/
        ...
        returns (string memory)
    {
        ...
        for (uint8 i = 0; i < 9; i++) {
            uint8 thisTraitIndex = PolymiceLibrary.parseInt(
                PolymiceLibrary.substring(_hash, i, i + 1)
            );
            ...
            for (
                ...
                string memory thisPixel = PolymiceLibrary.substring(
                ...
                uint8 x = letterToNumber(
                    PolymiceLibrary.substring(thisPixel, 0, 1)
                );
                uint8 y = letterToNumber(
                    PolymiceLibrary.substring(thisPixel, 1, 2)
                );
                ...
                svgString = string(
                    abi.encodePacked(
                        svgString,
                        "<rect class='c",
                        PolymiceLibrary.substring(thisPixel, 2, 4),
                        "' x='",
                        x.toString(),
                        "' y='",
                        y.toString(),
        ...
        return
            string(
                abi.encodePacked(
                    ...
                    PolymiceLibrary.encode(
                        bytes(
                            string(
                                abi.encodePacked(
                                    '{"name": "Polymice #',
                                    PolymiceLibrary.toString(_tokenId),
                                    '", "description": "Polymice is a collection of 10,000 unique mice. All the metadata and images are generated and stored 100% on-chain. No IPFS, no API. Just the Polygon blockchain.", "image": "data:image/svg+xml;base64,',
                                    PolymiceLibrary.encode(
        ...
        returns (string memory)
    {
        ...
        if (ownerOf(_tokenId) == 0x000000000000000000000000000000000000dEaD) {
            tokenHash = string(
                abi.encodePacked(
                    "1",
                    PolymiceLibrary.substring(tokenHash, 1, 9)
                )
            );
        }
...
|     | \      / |  |  ||     ||  .  \    |  |   |     ||  |  \     | |  |   |  ||     ||  |  |\    |
 \___/   \_/\_/  |__|__||_____||__|\_|    |__|    \__,_||__|__|\____| |__|  |____|\___/ |__|__| \___|
    ...
    */
    ...
    /**
     * @dev Sets the pcheeth ERC20 address
     * @param _pcheethAddress The pcheeth address
     */

    function setpCheethAddress(address _pcheethAddress) public onlyOwner {
        pcheethAddress = _pcheethAddress;
    }
