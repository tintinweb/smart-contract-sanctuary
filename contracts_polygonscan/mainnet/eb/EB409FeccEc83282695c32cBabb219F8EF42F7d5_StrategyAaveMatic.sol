...
contract StrategyAaveMatic is StratManager, FeeManager {
    ...
    function convertDustToEarned(address[] memory _lp0ToWantRoute, address[] memory _lp1ToWantRoute) public whenNotPaused {        
        ...
        uint256 token0Amt = IERC20(_lp0ToWantRoute[0]).balanceOf(address(this));
        if (_lp0ToWantRoute[0] != want && token0Amt > 0) {
            IERC20(_lp0ToWantRoute[0]).safeIncreaseAllowance(unirouter, token0Amt);
        ...
        uint256 token1Amt = IERC20(_lp1ToWantRoute[0]).balanceOf(address(this));
        if (_lp1ToWantRoute[0] != want && token1Amt > 0) {
            IERC20(_lp1ToWantRoute[0]).safeIncreaseAllowance(unirouter, token1Amt);
            // Swap all dust tokens to want tokens
