...
contract TestUniswapFlashSwap is IUniswapV2Callee {
    ...
    function transferToContract(address _token, uint _amount) external ownerOnly {
    ...
    function TestFlashSwap(address _tokenA, address _tokenB, uint _amountBorrow, address[] calldata _path) external ownerOnly {
        ...
        uint _amountBefore = IERC20(_tokenBorrow).balanceOf(address(this));
        bytes memory data = abi.encode(_tokenBorrow, _amountBorrow, _amountBefore, _path);
    ...
    function uniswapV2Call(address _sender, uint _amount0, uint _amount1, bytes calldata _data) external override {
        ...
        (address tokenBorrow, uint amountBorrow, uint amountBefore, address[] memory path) = abi.decode(_data, (address, uint, uint, address[]));
        ...
        require(amountAfter - (amountBorrow + fee) > amountBefore, "amountReceived < (amountBorrow + fee)");
        ...
        emit Log("amount for profit", amountAfter - (amountBorrow + fee) - amountBefore);
