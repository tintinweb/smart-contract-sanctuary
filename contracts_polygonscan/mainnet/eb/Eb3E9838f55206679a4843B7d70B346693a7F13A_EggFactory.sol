...
interface IEggStats {
    // Type egg: 0 - Free, 1 - Common, 2 - Rare, 3 - Epic, 4 - Legendary
     ...
     */
    function stats(uint256 tokenId) external view returns (uint16[] memory eggStats, uint16 luckStat);
...
interface IEggNFT is IERC721 {
    function getInfos(uint256 tokenId) external view returns (uint16[] memory eggStats, uint16 luckStat);
...
contract EggFactory is IEggFactory, OwnableUpgradeable, ReentrancyGuard {
    ...
    function totalBurned() external override view returns (uint256) {
        return IEggNFT(nft_).totalBurned();
    }
    ...
    function mintToken(address receiver, uint256 tokenId, IEggStats.EggInfo memory info) public override onlyAdmin {
        ...
        IEggStats(stats_).setInfo(tokenId, info);
        ++mintedCount_;
    ...
    function mintMultiTokens(address receiver, uint256[] memory tokenIds, IEggStats.EggInfo[] memory infos) external override onlyAdmin {
        uint256 length = tokenIds.length;
        require(length >= 1, "zero token");
        require(length == infos.length, "Wrong array length");
        for (uint256 i = 0; i < length; i++) {
            mintToken(receiver, tokenIds[i], infos[i]);
        }
    }
