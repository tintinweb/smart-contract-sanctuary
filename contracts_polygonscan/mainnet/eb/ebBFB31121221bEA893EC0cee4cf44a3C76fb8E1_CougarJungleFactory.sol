...
contract CougarJungleInitializable is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;

    // The address of the cougar jungle factory
    address public COUGAR_JUNGLE_FACTORY;
    ...
    constructor() public {
        COUGAR_JUNGLE_FACTORY = msg.sender;
    }
    ...
    function initialize(
        ...
        require(msg.sender == COUGAR_JUNGLE_FACTORY, "Not factory");
...
import "@openzeppelin/contracts/access/Ownable.sol";
import "@openzeppelin/contracts/utils/ReentrancyGuard.sol";
import "./CougarJungle.sol";

contract CougarJungleFactory is Ownable {
    event NewCougarJungleContract(address indexed cougarJungle);
     ...
     * @param _withdrawalInterval: the withdrawal interval for stakedToken (if any, else 0)
     * @param _admin: admin address with ownership
     * @return address of new cougar jungle contract
     */
    function deployPool(
        IBEP20 _stakedToken,
        IBEP20 _rewardToken,
        ...
        bytes memory bytecode = type(CougarJungleInitializable).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(_stakedToken, _rewardToken, _startBlock));
        address cougarJungleAddress;

        assembly {
            cougarJungleAddress := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }

        CougarJungleInitializable(cougarJungleAddress).initialize(
            _stakedToken,
            _rewardToken,
            _rewardPerBlock,
            _startBlock,
        ...
        emit NewCougarJungleContract(cougarJungleAddress);
    }
}
