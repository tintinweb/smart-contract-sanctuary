...
interface ISuChefSummoner {
    function summonSuChef(address depositToken, address mochiMol) external returns (address suChef);
}
...
contract MochiMol is ReentrancyGuard {
    ...
    ***************/
    ...
    bool internal initialized; // internally tracks deployment under EIP 1167 proxy pattern
    ...
    ISuChefSummoner constant suChefSummoner = ISuChefSummoner(0x2f7cA2178De81b41b064de0154C8b59F2596b74F); // SuChef deployer - this needs to be updated! 
    ...
    function init(bytes calldata data) public {
        ...
        suChef = suChefSummoner.summonSuChef(_approvedTokens[0], address(this)); // summon suChef 
    ...
    *****************/
    ...
    function collectTokens(address token) external onlyDelegate nonReentrant {
        uint256 amountToCollect = token == address(0) ? address(this).balance : balanceOfThis(token);
        ...
        uint256 shares;
        // skim and store in the BentoBox
        if (token == address(0)) { // if null, assume ETH and wrap into BentoBox
            (, shares) = bento.deposit{value: amountToCollect}(address(0), address(bento), address(this), amountToCollect, 0);
        } else { 
            safeTransfer(token, address(bento), amountToCollect);
            (, shares) = bento.deposit(token, address(bento), address(this), amountToCollect, 0);
        }
