...
import "../../Legacy/Governance/Comp.sol";
import "../../Legacy/Governance/GovernorAlpha.sol";

interface CErc20ForUniswapConfigLens {
    function underlying() external view returns (address);
}

interface RainMakerForLens {
...
interface MinistryLensInterface {
...
interface ComptrollerLensInterface {
    ...
    function getRegistry() external view returns (MinistryLensInterface);

    // NOTE : For now assuming only one type of RainMaker
    function rainMaker() external view returns (RainMakerForLens);
...
contract CompoundLens {
    ...
    function cTokenMetadata(CToken cToken) public returns (CTokenMetadata memory) {
        ...
        (bool isListed, uint collateralFactorMantissa, uint liquidationFactorMantissa, uint liquidationIncentiveMantissa) = comptroller.markets(address(cToken));
        address underlyingAssetAddress;
        uint underlyingDecimals;

        underlyingAssetAddress = cToken.underlying();

        // OLA_ADDITIONS : Testing for native coin by the 'underlying' address
        if (isCTokenForNative(cToken)) {
            underlyingDecimals = 18;
        } else {
            underlyingDecimals = EIP20Interface(underlyingAssetAddress).decimals();
        }
        ...
        return CTokenMetadata({
            ...
            liquidationFactorMantissa: liquidationFactorMantissa,
            liquidationIncentiveMantissa: liquidationIncentiveMantissa,
            underlyingAssetAddress: underlyingAssetAddress,
            cTokenDecimals: cToken.decimals(),
            underlyingDecimals: underlyingDecimals,
            incentiveSpeed: comptroller.hasRainMaker() ? comptroller.rainMaker().compSpeeds(address(cToken)) : 0
        });
    }
    ...
    function cTokenUnderlyingPrice(CToken cToken) public view returns (CTokenUnderlyingPrice memory) {
        ComptrollerLensInterface comptroller = ComptrollerLensInterface(address(cToken.comptroller()));
        MinistryLensInterface ministry = comptroller.getRegistry();

        PriceOracle priceOracle = ministry.getOracleForAsset(cToken.underlying());
    ...
    struct AccountLimits {
        ...
        uint liquidity;
        uint shortfall;
        uint liqFactorLiquidity;
        uint liqFactorShortfall;
    }

    ///    OLA_ADDITIONS : Added 'view'
    function getAccountLimits(ComptrollerLensInterface comptroller, address account) public view returns (AccountLimits memory) {
        ...
        (uint liquidationErrorCode, uint liqFactorLiquidity, uint liqFactorShortfall) = comptroller.getAccountLiquidityByLiquidationFactor(account);
        require(liquidationErrorCode == 0);

        return AccountLimits({
            markets: comptroller.getAssetsIn(account),
            liquidity: liquidity,
            shortfall: shortfall,
            liqFactorLiquidity: liqFactorLiquidity,
            liqFactorShortfall: liqFactorShortfall
        });
    }

    /// OLA_ADDITIONS : This function
    function getAccountLimitsAll(ComptrollerLensInterface comptroller, address[] memory accounts) public view returns (AccountLimits[] memory) {
        uint accountsCount = accounts.length;
        AccountLimits[] memory res = new AccountLimits[](accountsCount);
        for (uint i = 0; i < accountsCount; i++) {
            res[i] = getAccountLimits(comptroller, accounts[i]);
        }
        return res;
    }

    function getAccountLimitsAfterInterest(ComptrollerLensInterface comptroller, address account) public returns (AccountLimits memory) {
        ...
        for (uint i =0; i < marketsLength; i++) {
            require(markets[i].accrueInterest() == 0, "Accrue interest failed");
        }

        // Use the standard function
        return getAccountLimits(comptroller, account);
    }
    ...
    function getCompBalanceMetadataExt(Comp comp, ComptrollerLensInterface comptroller, address account) external returns (CompBalanceMetadataExt memory) {
        ...
        if (comptroller.hasRainMaker()) {
            comptroller.rainMaker().claimComp(account);
        }
        ...
        if (comptroller.hasRainMaker()) {
            accrued = comptroller.rainMaker().compAccrued(account);
        }
...
//            delegate: comp.delegates(account),
    ...
    function isCTokenForNative(CToken cToken) public view returns (bool) {
        return cToken.underlying() == cToken.nativeCoinUnderlying();
    }

    function accountBalance(address payable account) public view returns (uint256 balance) {
...
contract Comp {
    ...
    // OLA_ADDITION : This, instead of the assembly call
    uint public chainId;
     ...
     */
    constructor(address account, uint256 chainId_) public {
        balances[account] = uint96(totalSupply);
        chainId = chainId_; // OLA_ADDITIONS : 'chainId'
...
contract GovernorAlpha {
    ...
    // OLA_ADDITIONS : 'chainId' as member
    ...
    constructor(address timelock_, address comp_, address guardian_, uint256 chainId_) public {
