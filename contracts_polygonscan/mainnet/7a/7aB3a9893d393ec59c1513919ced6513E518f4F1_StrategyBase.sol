...
abstract contract StrategyFarmBase is BaseStrategyMasterChef {
    ...
    // Uniswap swap paths
    address[] public koge_usdc_path;
    address[] public koge_wmatic_path;
    ...
    {
        koge_usdc_path = new address[](2);
        koge_usdc_path[0] = koge;
        koge_usdc_path[1] = usdc;
        
        koge_wmatic_path = new address[](2);
        koge_wmatic_path[0]= koge;
        koge_wmatic_path[1]= wmatic;
    }
    ...
    function harvest() public override {
        ...
        //Swap 
        uint256 _kogeBalance = IERC20(koge).balanceOf(address(this));
        if (_kogeBalance > harvestCutoff){
            
            //Send performance fee to strategist
            uint256 performanceFee = _kogeBalance.mul(keepFXS).div(keepFXSmax);
            if (performanceFee>0) {
                _swapUniswapWithPath(koge_usdc_path, performanceFee);
            ...
            // Swap rest
            _kogeBalance = IERC20(koge).balanceOf(address(this));
            _swapUniswapWithPath(koge_wmatic_path, _kogeBalance.div(2));

            // Add liquidity
            uint256 _koge = IERC20(koge).balanceOf(address(this));
            uint256 _wmatic = IERC20(wmatic).balanceOf(address(this));
            if (_koge> 0 && _wmatic > 0) {
                ...
                // Donates DUST
                IERC20(koge).safeTransfer(
                    strategist,
                    IERC20(koge).balanceOf(address(this))
                );
...
contract StrategyBase is StrategyFarmBase {
    // Token addresses
    address public KOGE_MASTER_CHEF = 0x6275518a63e891b1bC54FEEBBb5333776E32fAbD;
    address public KOGE_WMATIC_LP = 0x3885503aEF5E929fCB7035FBDcA87239651C8154;
