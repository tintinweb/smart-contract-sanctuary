...
contract LimitOrdersGeneric is IModule {
    ...
    function execute(
        ...
        require(
            ...
            "LimitOrders#execute: HANDLERS_LENGTH_MISMATCH"
        );
        ...
        if (_inToken == WETH || address(_inToken) == ETH) {
            ...
            if (_inToken == WETH) {
                ...
                WETH.deposit{value: address(this).balance - fees}();
            }
    ...
    function _swap(
        ...
        for (uint256 i = 0; i < _swapHandlers.length; i++) {
            ...
            require(_swapSuccess, "LimitOrders#_swap: SWAP_MULTICALL_FAILED");
        }
        ...
        if (_outToken == WETH) {
            ...
            require(success, "LimitOrder#_swap: OWNER_ETH_TRANSFER_FAILED");
        }
        ...
        require(
            bought >= _minReturn,
            "LimitOrder#_swap: INSUFFICIENT_OUTPUT_AMOUNT"
        );
    }

    function _payFees(uint256 _fees) private {
        GELATO.sendValue(_fees / 5);
        payable(block.coinbase).sendValue(address(this).balance);
    }
}
...
import {LimitOrdersGeneric} from "../LimitOrdersGeneric.sol";
import {IWETH} from "../../interfaces/IWETH.sol";

contract LimitOrdersParaswap is LimitOrdersGeneric {
    constructor(IWETH _weth, address payable _gelato) 
