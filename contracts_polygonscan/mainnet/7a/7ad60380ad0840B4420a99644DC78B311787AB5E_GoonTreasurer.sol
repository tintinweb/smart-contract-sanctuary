...
// File contracts/interface/ITreasurerExpress.sol
...
// File contracts/lib/TreasurerUtils.sol
...
// File contracts/GoonTreasurer.sol
  ...
  function claimRewardExpress(
    ...
    for (uint256 i = 0; i < length; i++) {
      ...
      if (week > maxClaimableWeek) {
        ...
        rewardToBurn += (expressClaimBurnBP * weekReward) / 1e4;
        claimableReward += weekReward - rewardToBurn;
      }
      else {
        claimableReward += user.rewardsPerWeek[week];
      }
    ...
    if (rewardToBurn > 0) {
      totalLocked -= rewardToBurn;
      goon.burn(rewardToBurn);
    }
    if (claimableReward > 0) {
      totalLocked -= claimableReward;
      _safeGoonTransfer(msg.sender, claimableReward);
    }
    if (claimableReward > 0 || rewardToBurn > 0) {
      emit ExpressClaim(msg.sender, claimableReward, rewardToBurn);
    }
  }
