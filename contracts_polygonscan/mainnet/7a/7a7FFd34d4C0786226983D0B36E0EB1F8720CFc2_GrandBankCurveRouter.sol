...
// File: contracts/interfaces/IGBTradingRoute.sol
...
// File: contracts/routes/GrandBankCurveRouter.sol
...
contract GrandBankCurveRouter is IGBTradingRoute, WhitelistedRole, ReentrancyGuard {
    ...
    function getAmountUnderlying(
        ...
        returns(uint256 _destAmount)
    {
        return curvePool.get_dy_underlying(_i, _j, _srcAmount);
    }
    ...
    {
        require(_src != _dest, "GBCurveRouter: Destination token can not be source token");
        i = -1;
        j = -1;
        i = _src == dai ? 0 : i;
        i = _src == usdc ? 1 : i;
        ...
        j = _dest == wbtc ? 3 : j;
        j = _dest == weth ? 4 : j;
        require(i != -1 && j != -1, "GBCurveRouter: Tokens not supported!");
    }
}