...
// File: .deps/github/Loesil/VaultChef/contracts/DeFi Projects/PolyCrystal/IPolyCrystalMasterChef.sol

interface IPolyCrystalMasterChef
{
...
// File: .deps/github/Loesil/VaultChef/contracts/DeFi Projects/PolyCrystal/VaultPolyCrystalMaster.sol

contract VaultPolyCrystalMaster is Vault
{
    ...
    address public constant REWARD_TOKEN = 0x76bF0C28e604CC3fE9967c83b3C3F31c213cfE64;
    
    address public constant CHEF = 0xeBCC84D2A73f0c9E23066089C6C24F4629Ef1e6d;
    
    address public constant ROUTER = 0xf7770B6B1b7a9538d98Ed99D555195951Da6C616;
    ...
    {
        //tokens
        (depositToken, , , ,) = IPolyCrystalMasterChef(CHEF).poolInfo(_poolID);
    ...
    {
        uint256 balance = IERC20(rewardToken).balanceOf(address(this));
        uint256 pending = IPolyCrystalMasterChef(chef).pendingCrystal(poolID, address(this));
    ...
    {
        return IPolyCrystalMasterChef(chef).userInfo(poolID, address(this));
    }
	
	function getAllocPoints() internal view override returns (uint256)
    {
        (, uint256 alloc, , ,) = IPolyCrystalMasterChef(chef).poolInfo(poolID);
        return alloc;
    }
    ...
    {
        (, , , , uint16 poolDepositFee) = IPolyCrystalMasterChef(chef).poolInfo(poolID);
        return (uint256(poolDepositFee) * PERCENT_FACTOR) / 10000; //10000 = 100% => 1000000
    }
    ...
    function poolDeposit(uint256 _amount) internal override returns(uint256 _received, uint256 _lostRate)
    {
        ...
        IPolyCrystalMasterChef(chef).deposit(poolID, _amount);
    ...
    function poolWithdraw(uint256 _amount) internal override returns(uint256 _received, uint256 _lostRate)
    {
        ...
        IPolyCrystalMasterChef(chef).withdraw(poolID, _amount);
