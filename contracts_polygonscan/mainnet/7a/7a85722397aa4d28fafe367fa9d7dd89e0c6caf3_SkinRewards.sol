...
contract SnookToken is ERC721Upgradeable, ERC721BurnableUpgradeable, ERC721EnumerableUpgradeable, OwnableUpgradeable {
    ...
    mapping(uint => uint[2]) private _tokenEligiblePeriods;
    ...
    function getCurrentEligiblePeriodIdx(uint tokenId) external view returns (uint) {
      return _tokenEligiblePeriods[tokenId][0];
    }

    function getPreviousEligiblePeriodIdx(uint tokenId) external view returns (uint) {
      return _tokenEligiblePeriods[tokenId][1];
    }

    function setCurrentEligiblePeriodIdx(uint tokenId, uint periodIdx) external onlyGameContracts {
      // require(msg.sender == 0xD6588E9C555D35B0274f47Dc6b9D4D7917602347, 'Not game');
      _tokenEligiblePeriods[tokenId][0] = periodIdx;
    }

    function setPreviousEligiblePeriodIdx(uint tokenId, uint periodIdx) external onlyGameContracts {
      // require(msg.sender == 0xD6588E9C555D35B0274f47Dc6b9D4D7917602347, 'Not game');
      _tokenEligiblePeriods[tokenId][1] = periodIdx;
    }
...
interface ISnookGame is IDescriptorUser {
  ...
  event Birth(address indexed to, uint tokenId, uint price);
...
contract SkinRewards is ISkinRewards, IDescriptorUser, AccessControlEnumerableUpgradeable, PausableUpgradeable {
  ...
  struct EventData {
    bool p3Eligible;
    uint currentEligiblePeriodIdx;
    uint previousEligiblePeriodIdx;
    uint p3TotalStars;
  }
  event FixToken(
    ...
    uint p3Traitcount,
    uint p3Stars,
    uint lastPlayedPeriodTraitcount,
    uint lastPlayedPeriodIdx,
    EventData beforeFix,
    EventData afterFix,
    string note
  );
  ...
  // FIX
  // The function finalizes upgrade to new SkinReward mechanism.
  // We init eligible period of token as one in which it played last time. 
  // If token played in period 1 and 2 then it's eligible period is 2.
  // If token played in period 1 and not in 2 then it's eligible period is 1.
  function fixToken(uint tokenId, uint lastPlayedPeriodTraitcount, uint lastPlayedPeriodIdx) public onlyRole(PAUSER_ROLE) {
    require(lastPlayedPeriodIdx == 1 || lastPlayedPeriodIdx == 2, 'Invalid lastPlayedPeriod');

    Period storage p3 = _periods[3];
    bool p3Played = p3.tokenStarsUpdated[tokenId]; 
    uint p3Stars = p3.tokenStars[tokenId];
    uint p3Traitcount = _periodTokenTraitcount[3][tokenId];
    bool p3Eligible = _periodTokenEligible[3][tokenId];

  
    // tracking by event: state before fix
    string memory note;
    EventData memory beforeFix;
    beforeFix.p3Eligible = p3Eligible;
    beforeFix.currentEligiblePeriodIdx = _snook.getCurrentEligiblePeriodIdx(tokenId);
    beforeFix.previousEligiblePeriodIdx = _snook.getPreviousEligiblePeriodIdx(tokenId);
    beforeFix.p3TotalStars = p3.totalStars;
    
    _periodTokenTraitcount[lastPlayedPeriodIdx][tokenId] = lastPlayedPeriodTraitcount;
    _snook.setCurrentEligiblePeriodIdx(tokenId, lastPlayedPeriodIdx);
    _snook.setPreviousEligiblePeriodIdx(tokenId, lastPlayedPeriodIdx);
    
    if (p3Played == true) { // token played in period 3
      if (p3Eligible == true) { // token is assumed to be eligible
        require(_snook.getCurrentEligiblePeriodIdx(tokenId) == 3, 'currentEligiblePeriod is not 3');
        // let's check if it's really eligible
        if (p3Traitcount>lastPlayedPeriodTraitcount && p3Traitcount-lastPlayedPeriodTraitcount>TRAITCOUNT_DELTA && p3Stars>0) {
          // yes, token is really eligible, nothing to change
          note = 'played p3, indeed eligible, nothing to fix';
        }
        else {
          // token is not eligible by rules of period 3, needs a fix
          _snook.setCurrentEligiblePeriodIdx(tokenId, lastPlayedPeriodIdx); // change from 3 to (1 or 2)
          _periodTokenEligible[3][tokenId] = false; // make token not eligible
          p3.totalStars = p3.totalStars - p3Stars; // fix total eligible stars by stars of removed eligible token
          note = 'played p3, changed from eligible to not eligible';
        }
      }
      else { // token is not eligible
        note = 'played p3, not eligible, nothing to fix';
        // nothing to fix
      }
    } 
    else { // token did not play in period 3
      note = 'not played p3, nothing to fix';
      // nothin to fix
    }

    // tracking event: state after fix
    EventData memory afterFix;
    afterFix.p3Eligible = _periodTokenEligible[3][tokenId];
    afterFix.p3TotalStars = p3.totalStars;
    afterFix.currentEligiblePeriodIdx = _snook.getCurrentEligiblePeriodIdx(tokenId);
    afterFix.previousEligiblePeriodIdx = _snook.getPreviousEligiblePeriodIdx(tokenId);
    ...
    onlyGameContracts() whenNotPaused() external override 
  {
    ...
    uint previousEligiblePeriodIdx = _snook.getPreviousEligiblePeriodIdx(tokenId);
    uint currentEligiblePeriodIdx = _snook.getCurrentEligiblePeriodIdx(tokenId); // beforeUpdate (update in if block)
    uint previousTraitCount;

    uint tokenStarsInCurrentEligiblePeriodBeforeUpdate = _periods[currentEligiblePeriodIdx].tokenStars[tokenId];
    bool isCurrentPeriodEligibleBeforeUpdate = _periodTokenEligible[_currentPeriodIdx][tokenId];

    if (period.tokenStarsUpdated[tokenId] == true) { 
      previousTraitCount = _periodTokenTraitcount[previousEligiblePeriodIdx][tokenId];
      if (traitCount>previousTraitCount && traitCount-previousTraitCount>TRAITCOUNT_DELTA) {
        _periodTokenEligible[_currentPeriodIdx][tokenId] = true;
        _snook.setCurrentEligiblePeriodIdx(tokenId, _currentPeriodIdx);
      } 
      else {
        _periodTokenEligible[_currentPeriodIdx][tokenId] = false;
        _snook.setCurrentEligiblePeriodIdx(tokenId, previousEligiblePeriodIdx);
      }
    } 
    else { // first time entry to the game during current period
      previousTraitCount = _periodTokenTraitcount[currentEligiblePeriodIdx][tokenId];
      if (traitCount>previousTraitCount && traitCount-previousTraitCount>TRAITCOUNT_DELTA) {
        _periodTokenEligible[_currentPeriodIdx][tokenId] = true;
        _snook.setCurrentEligiblePeriodIdx(tokenId, _currentPeriodIdx);
      }
      ...
      _snook.setPreviousEligiblePeriodIdx(tokenId, currentEligiblePeriodIdx);
    }
    ...
    uint tokenStarsInCurrentEligiblePeriodAfterUpdate = _periods[_snook.getCurrentEligiblePeriodIdx(tokenId)].tokenStars[tokenId];
    bool isCurrentPeriodEligibleAfterUpdate = _periodTokenEligible[_currentPeriodIdx][tokenId];

    // Here we test if token goes from eligible to not eligible period 
    // or vice versa and update total stars according to the change.
    if (isCurrentPeriodEligibleBeforeUpdate==false && isCurrentPeriodEligibleAfterUpdate==false) {
      // !eligible->!eligible: no update to total stars
    }
    if (isCurrentPeriodEligibleBeforeUpdate==false && isCurrentPeriodEligibleAfterUpdate==true) {
      // !eligible->eligible
      period.totalStars = period.totalStars + tokenStarsInCurrentEligiblePeriodAfterUpdate;
    }
    if (isCurrentPeriodEligibleBeforeUpdate==true && isCurrentPeriodEligibleAfterUpdate==true) {
      // eligible->eligible
      period.totalStars = 
        period.totalStars - tokenStarsInCurrentEligiblePeriodBeforeUpdate + tokenStarsInCurrentEligiblePeriodAfterUpdate;
    }
    if (isCurrentPeriodEligibleBeforeUpdate==true && isCurrentPeriodEligibleAfterUpdate==false) {
      // eligible->!eligible
      period.totalStars = 
        period.totalStars - tokenStarsInCurrentEligiblePeriodBeforeUpdate;
    }
