...
interface BankConfig {
  /// @dev Return minimum usdc debt size per position.
  ...
  /// @dev Return the work factor for the bigfoot + usdc debt, using 1e4 as denom. Revert on non-bigfoot.
  function workFactor(address bigfoot, uint debt) external view returns (uint);

  /// @dev Return the kill factor for the bigfoot + usdc debt, using 1e4 as denom. Revert on non-bigfoot.
...
interface Bigfoot {
  /// @dev Work on a (potentially new) position. Optionally send usdc back to Bank.
  function work(
    uint id,
    address user,
    uint debt,
  ...
  /// @dev Return the amount of usdc wei to get back if we are to liquidate the position.
  function health(uint id) external view returns (uint);

  /// @dev Liquidate the given position to usdc. Send all usdc back to Bank.
...
contract BankUsd is Initializable, ERC20, ReentrancyGuardUpgradeSafe, Governable {
  /// @notice Libraries
  ...
  address constant public vault = address(0xCf4fB717379674819c11089d1Cd8Df42BF0BE8e5);
  ...
  uint public constant DAIID = 0;
  uint public constant USDCID = 1;
  uint public constant USDTID = 2;

  address[] public tokenIndex = [dai, usdc, usdt];
  ...
  function initialize(BankConfig _config) external initializer {
    ...
    curvestorage = 0x445FE580eF8d70FF569aB36e80c647af338db351;
    config = _config;
    lastAccrueTime = now;
    nextPositionID = 1;
    name = 'eleven interest bearing USDC';
    symbol = 'eleUSDC';
    decimals = 6;
    approve();
  }
  ...
  function approve() internal{
    dai.safeApprove(curvestorage, uint(-1));
    usdt.safeApprove(curvestorage, uint(-1));
    usdc.safeApprove(vault, uint(-1));
    usdc.safeApprove(curvestorage, uint(-1));
  }
  ...
  function additionalApprove(address _token, address _spender) onlyGov external{
    require(_token != usdc && _token != vault, "no rugs allowed");
  ...
  // @dev transfer from usdc and/or 11unusedusdc with no errors
  function safeTransferBankCurrency(address _receiver, uint _amount) internal{
    uint inside = usdc.myBalance();
    if(inside>=_amount) usdc.safeTransfer(_receiver, _amount);
    else {
      safeWithdrawFromFarm(_amount.sub(inside));
      usdc.safeTransfer(_receiver, usdc.myBalance());
    }
  }
  

  // @dev Total usdc inside the contract wether on the vault or not
  function usdcBalance() public view returns (uint) {
    return usdc.myBalance().add(insideVaultBankCurrency());
  }
  ...
  function pendingInterest(uint msgValue) public view returns (uint) {
    if (now > lastAccrueTime) {
      ...
      uint balance = usdcBalance().sub(msgValue);
  ...
  /// @dev Return the usdc debt value given the debt share. Be careful of unaccrued interests.
  ...
  /// @dev Return usdc value and debt of the given position. Be careful of unaccrued interests.
  ...
  /// @dev Return the total usdc entitled to the token holders. Be careful of unaccrued interests.
  function totalBankCurrency() public view returns (uint) {
    return usdcBalance().add(glbDebtVal).sub(reservePool);
  }
  
  // @dev Converts all dollar stables to usdc
  function convertToUsdc() public{
    uint amountDai = dai.myBalance();
    uint amountUsdt = usdt.myBalance();
    if(amountDai > 1e14)//ignore small amounts to save gas. bloat is bad.
        CurvePool(curvestorage).exchange_underlying(int128(DAIID), int128(USDCID), amountDai, amountDai.mul(99).div(100000000000000));
    if(amountUsdt > 100)
        CurvePool(curvestorage).exchange_underlying(int128(USDTID), int128(USDCID), amountUsdt, amountUsdt.mul(99).div(100));
  }


  /// @dev Add more usdc to the bank. Hope to get some good returns.
  function deposit(uint[] calldata amounts) external accrue(0)  nonReentrant {
    uint total = totalBankCurrency();
    uint diff = transferDollars(amounts);
  ...
  /// @dev Withdraw usdc from the bank by burning the share tokens.
  function withdraw(uint share, uint8 option) external accrue(0) nonReentrant {
    uint amount = share.mul(totalBankCurrency()).div(totalSupply());
    require(amount <= usdcBalance(), "Utilization too high, withdraw an smaller amount");
    _burn(msg.sender, share);
    if(option == USDCID) safeTransferBankCurrency(msg.sender, amount);
    else if(option == USDTID || option == DAIID){
      safeWithdrawFromFarm(amount);
      //swap to token
      if(option == USDTID)
        CurvePool(curvestorage).exchange_underlying(int128(USDCID), int128(USDTID), usdc.myBalance(), usdc.myBalance().mul(99).div(100));
      else
        CurvePool(curvestorage).exchange_underlying(int128(USDCID), int128(DAIID), usdc.myBalance(), usdc.myBalance().mul(990000000000));        
      tokenIndex[option].safeTransfer(msg.sender, tokenIndex[option].myBalance());
    }
    uint supply = totalSupply();
    require(supply == 0 || supply >= 1e5, 'withdraw: total supply too low');
  }
  ...
  // @dev Deposit dollars, convert to usdc and stake in vault
  function transferDollars(uint[] memory amounts) internal returns(uint){
    uint bfrusdc = usdc.myBalance();
    if(amounts[USDCID]>0) usdc.safeTransferFrom(msg.sender, address(this), amounts[USDCID]);
    if(amounts[USDTID]>0) usdt.safeTransferFrom(msg.sender, address(this), amounts[USDTID]);
    if(amounts[DAIID]>0) dai.safeTransferFrom(msg.sender, address(this), amounts[DAIID]);
    convertToUsdc();
    uint aftrusdc = usdc.myBalance();
    return (aftrusdc.sub(bfrusdc));
  }
  ...
  /// @param loan The amount of usdc to borrow from the pool.
  /// @param maxReturn The max amount of usdc to return to the pool.
  ...
  function work(
    ...
    uint back;
    {
      uint sendUsdc = transferDollars(amounts).add(loan);
      require(sendUsdc <= usdcBalance(), 'insufficient USD in the bank');
      uint beforeUsdc = usdcBalance().sub(sendUsdc);
      safeTransferBankCurrency(bigfoot, sendUsdc);
      Bigfoot(bigfoot).work(id, msg.sender, debt, data);
      back = usdcBalance().sub(beforeUsdc);
    }
  ...
  function kill(uint id) external onlyEOA accrue(0) nonReentrant {
    ...
    // 2. Perform liquidation and compute the amount of usdc received.
    uint beforeUsdc = usdcBalance();
    Bigfoot(pos.bigfoot).liquidate(id);
    uint back = usdcBalance().sub(beforeUsdc);
  ...
  /// @dev Withdraw usdc reserve for underwater positions to the given address.
  /// @param to The address to transfer usdc to.
  /// @param value The number of usdc tokens to withdraw. Must not exceed `reservePool`.
  function withdrawReserve(address to, uint value) external onlyGov nonReentrant {
    reservePool = reservePool.sub(value);
    require(value<=usdcBalance(), "not enough funds");
    safeTransferBankCurrency(to, value);
  }

  
  /// @dev Reduce usdc reserve, effectively giving them to the depositors.
  /// @param value The number of usdc reserve to reduce.
