...
interface LpTokenStaker {
    ...
    function claimable_reward_write(address, address) external view returns (uint256 amount);
    function claimable_reward(address, address) external view returns (uint256 amount);
...
abstract contract StrategyBase {
    ...
    address public constant iceBuyback = 0xf16e81dce15B08F326220742020379B855B87DF9;
    address public constant spiritBuyback = 0x5Cc61A78F164885776AA610fb0FE1257df78E59B;
    address public constant booBuyback = 0x841FAD6EAe12c286d1Fd18d1d525DFfA75C7EFFE;
    ...
    function buybackAndNotify(address _buybackPrinciple, uint256 _buybackAmount) internal {
        if (buybackEnabled == true && _buybackAmount > 0) {
            if(_buybackPrinciple == iceBuyback){
                _multiUniswap(_buybackPrinciple, mmToken, _buybackAmount, sushiRouter, spookyRouter);
            }else if(_buybackPrinciple == spiritBuyback){
                _multiUniswap(_buybackPrinciple, mmToken, _buybackAmount, spiritRouter, spookyRouter);
            }else{
    ...
    function figureOutPath(address _from, address _to, uint256 _amount) public view returns (bool useSushi, address[] memory swapPath){
        ...
        if (_to == mmToken && buybackEnabled == true) {
            if (_from == usdcBuyback || _from == WFTM || _from == wbnb || _from == WMATIC){
                path = new address[](2);
                path[0] = _from;
                path[1] = _to;
            }else if (_from == weth){
                ...
                path = new address[](3);
                path[0] = _from;
                path[1] = usdcBuyback; // use WFTM on Fantom
                path[2] = _to;
            }
        } else{
    ...
    function _multiUniswap(
        ...
        if (_amount > 0){
            // Swap with uniswap
            IERC20(_from).safeApprove(router1, 0);
            IERC20(_from).safeApprove(router1, _amount);
            ...
            UniswapRouterV2  router1 = UniswapRouterV2(router1);
            ...
            uint256 _wftmBal = IERC20(WFTM).balanceOf((address(this)));
            require(_wftmBal > 0, "!_wftmBal");

            IERC20(WFTM).safeApprove(router2, 0);
            IERC20(WFTM).safeApprove(router2, _wftmBal);

            UniswapRouterV2  router2 = UniswapRouterV2(router2);
...
abstract contract StrategyCurveFarmBase is StrategyBase {
        ...
        public StrategyBase(_want, _governance, _strategist, _controller, _timelock)
    {
        ...
        want_decimals = ERC20(_want).decimals();
        IERC20(lpToken).safeApprove(stakingPool, uint256(-1));
    }
	
    function _balanceInStakingPool() public view returns (uint256) {
        return LpTokenStaker(stakingPool).balanceOf(address(this));
    }

    function getHarvestable() public view returns (uint256) {
        return LpTokenStaker(stakingPool).claimable_reward_write(address(this), CRV);
    }

    function getHarvestableMatic() public view returns (uint256) {
        return LpTokenStaker(stakingPool).claimable_reward_write(address(this), WMATIC);
    }
...
interface ICurveFi_2 {
    ...
    function add_liquidity(uint256[2] calldata amounts, uint256 min_mint_amount)
        external;
    function add_liquidity(uint256[2] calldata amounts, uint256 min_mint_amount, bool _use_underlying) external;
...
interface ICurveFi_3 {
    ...
    function add_liquidity(uint256[3] calldata amounts, uint256 min_mint_amount, bool _use_underlying) external;
...
contract StrategyCurveWbtcV1 is StrategyCurveFarmBase {
    address public constant usdc = 0x1BFD67037B42Cf73acF2047067bd4F2C47D9BfD6; //want token
    ...
    // staking constants
    address public constant curveGauge = 0xffbACcE0CC7C19d46132f1258FC16CF6871D153c;
    address public constant curvePool = 0xC2d95EEF97Ec6C17551d45e77B590dc1F9117C67;
    address public constant curveLpToken = 0xf8a57c1d3b9629b77b6726a042ca48990A84Fb49;
    ...
    constructor(
        ...
        public StrategyCurveFarmBase(usdc, curveLpToken, curveGauge, _governance, _strategist, _controller, _timelock)
    {		        
        IERC20(usdc).safeApprove(curvePool, uint256(-1));
        IERC20(curveLpToken).safeApprove(curvePool, uint256(-1));
    }
    ...
    function balanceOfPool() public override view returns (uint256){
        uint256 _lpAmt = IERC20(curveLpToken).balanceOf(address(this)).add(_balanceInStakingPool());
        return lpTokenToWant(_lpAmt);
    }
    ...
    function deposit() public override {
        ...
        if (_wantAmt > 0 && checkSlip(_wantAmt, _maxSlip)) {
            ...
            ICurveFi_2(curvePool).add_liquidity(amounts, _maxSlip, true);
        }
    ...
    function _withdrawSome(uint256 _amount) internal override returns (uint256) {
        ...
        uint256 requiredLpAmt = estimateRequiredLP(_amount);
        requiredLpAmt = requiredLpAmt.mul(DENOMINATOR.add(slippageProtectionOut)).div(DENOMINATOR);// try to remove bit more
		
        uint256 _lpAmt = IERC20(curveLpToken).balanceOf(address(this));
        uint256 _withdrawFromStaking = _lpAmt < requiredLpAmt ? requiredLpAmt.sub(_lpAmt) : 0;
        ...
        _lpAmt = IERC20(curveLpToken).balanceOf(address(this));
        if (_lpAmt > 0){
            requiredLpAmt = requiredLpAmt > _lpAmt ? _lpAmt : requiredLpAmt;
            uint256 maxSlippage = requiredLpAmt.mul(DENOMINATOR.sub(slippageProtectionOut)).div(DENOMINATOR);

            if (want_decimals < 18) {
                maxSlippage = maxSlippage.div(10**(uint256(uint8(18) - want_decimals)));
            }
            ICurveFi_2(curvePool).remove_liquidity_one_coin(requiredLpAmt, 0, maxSlippage);
        }
    ...
    function harvest() public override onlyBenignCallers {
        ...
        // Collects CRV,WMATIC tokens
        _claimRewards();
		
        // covert from CRV -> MATIC
        uint256 _rewards = IERC20(CRV).balanceOf(address(this));
        if (_rewards > 0) {
            address[] memory path = new address[](3);
            path[0] = CRV;
            path[1] = weth;
            path[2] = WMATIC;
            _swapUniswapWithDetailConfig(CRV, WMATIC, _rewards, 1, path, sushiRouter);
        }
        ...
        if (_wmatic > 0) {
            ...
            _swapUniswap(WMATIC, want, IERC20(WMATIC).balanceOf(address(this)));
    ...
    function lpTokenToWant(uint256 _lpAmt) public view returns (uint256) {
        if (_lpAmt == 0){
            return 0;
        }
        // use underestimate of current assets. 
        uint256 virtualOut = virtualPriceToWant().mul(_lpAmt).div(1e18);
        return virtualOut; 
    }

    function getName() external override pure returns (string memory) {
        return "StrategyCurveWbtcV1";
    }
}