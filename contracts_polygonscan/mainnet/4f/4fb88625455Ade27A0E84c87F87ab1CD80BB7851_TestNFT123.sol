...
contract TestNFT123 is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    address public SASHIMONO_ADDRESS_POLYGON = 0xe96f3E06AbE1E2082c03038036da6134D5Ba9b6e;
    SashimonoInterface sashimonoContract = SashimonoInterface(SASHIMONO_ADDRESS_POLYGON);
    uint public constant MAX_SASHIMONOS = 2;

    constructor(string memory baseURI) ERC721("Testing", "TestNFT123") {
        setBaseURI(baseURI);
    }
    ...
    function mintWithSingleSashimono(uint sashimonoId, uint numberOfBushidos) public payable {
        ...
        require(numberOfBushidos <= MAX_SASHIMONOS, "Can only mint 2 bushidos per sashimono");
        ...
        require(canMintWithSashimono(sashimonoId) && sashimonoContract.ownerOf(sashimonoId) == _msgSender(), "Bad owner!");
    ...
    function mintBushidos(uint numberOfBushidos) public payable {
        ...
        require(numberOfBushidos <= MAX_BUSHIDOS_PER_TXN, "Can only mint 8 bushidos at a time");
        require(totalSupply().add(numberOfBushidos) <= MAX_BUSHIDOS, "Mint would exceed max supply of Bushidos");
        require(BUSHIDO_PRICE.mul(numberOfBushidos) <= msg.value, "Ether value sent is not correct");
    ...
    // TODO figure out exact allocations and exact addresses to send them to
