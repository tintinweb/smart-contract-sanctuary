...
contract Collector is Ownable {
    ...
    // There is an exploit to add lots of RADIO to the treasury, run convert, then remove the RADIO again.
    //     As the size of RadioStaking has grown, this requires large amounts of funds and isn't super profitable anymore
    ...
    // All safeTransfer, _swap, _toRADIO, _convertStep
    function _convertStep(
        ...
        if (token0 == token1) {
            ...
            if (token0 == radio) {
                IERC20(radio).safeTransfer(treasury, amount);
                radioOut = amount;
            } else if (token0 == weth) {
            ...
            // eg. RADIO - ETH
            IERC20(radio).safeTransfer(treasury, amount0);
            radioOut = _toRADIO(token1, amount1).add(amount0);
        } else if (token1 == radio) {
            // eg. USDT - RADIO
            IERC20(radio).safeTransfer(treasury, amount1);
    ...
    {
        amountOut = _swap(token, radio, amountIn, treasury);
    }
}
