...
contract XVMCtransitionProxy {
    ...
    constructor(
        ...
        IERC20(_token).safeApprove(address(0xD00a313cAAe665c4a828C80fF2fAb3C879f7B08B), type(uint256).max);
    }


	event Hopping(address sendingContract, address hopInto, uint256 amount, address recipient, uint256 previousLastDepositedTime);
    event Ok(address wallet);
    ...
    function hopDeposit(uint256 _amount, address _recipientAddress, uint256 previousLastDepositedTime) external {
        address hopInto = 0xD00a313cAAe665c4a828C80fF2fAb3C879f7B08B;
        require(msg.sender == 0x9b6ae196A358Ea81c305D8A32018a4F4C90FC207);
		
		token.safeTransferFrom(msg.sender, address(this), _amount);

        IacPool(hopInto).hopDeposit(_amount, _recipientAddress, previousLastDepositedTime, 0);
		
		emit Hopping(msg.sender, hopInto, _amount, _recipientAddress, previousLastDepositedTime);
    }
}