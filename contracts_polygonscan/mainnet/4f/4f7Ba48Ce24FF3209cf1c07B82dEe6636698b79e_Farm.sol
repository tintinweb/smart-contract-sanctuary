...
contract Farm is Ownable, ReentrancyGuard, Pausable {
    ...
    struct PoolInfo {
        ...
        uint256 totalInputTokensStaked;
        IStrategy strategy;
    }
        ...
        nonReentrant
    {
        ...
        require(activeLpTokens[address(_lpToken)] == false, "Input Token already added");
    ...
    //Shouldn't be used and only be used when no PL tokens are staked
    ...
    function rescueFundsFromStrategy(uint256 _pid)
        external
        onlyOwner
        validatePoolByPid(_pid)
        nonReentrant
    {
        ...
        if (address(pool.strategy) != address(0)) {
            uint256 rescuedAssetAmount = pool.strategy.rescueFunds(address(pool.lpToken));
            pool.lpToken.safeTransferFrom(
                address(pool.strategy),
                address(this),
                rescuedAssetAmount
            );
            pool.strategy = IStrategy(address(0));
        }
    }
        ...
        returns (uint256 lpSupply)
    {
        PoolInfo storage pool = poolInfo[_pid];
        if (address(pool.strategy) != address(0)) {
            lpSupply = pool.strategy.getTotalLPStaked();
        } else {
    ...
    function _withdraw(
        ...
        if (_amount > 0) {
            ...
            if (address(pool.strategy) != address(0)) {
                ...
                pool.lpToken.safeTransferFrom(address(pool.strategy), address(this), _amount);
            } else {
                withdrawnAmount = _getWithdrawableAmount(_pid, _amount);
            }
    ...
    function emergencyWithdraw(uint256 _pid) external validatePoolByPid(_pid) nonReentrant {
        ...
        if (address(pool.strategy) != address(0)) {
            ...
            pool.lpToken.safeTransferFrom(address(pool.strategy), address(this), user.amount);
        } else {
            withdrawnAmount = _getWithdrawableAmount(_pid, user.amount);
        }
    ...
    function _getWithdrawableAmount(uint256 _pid, uint256 _amount)
        internal
        view
        returns (uint256 withdrawableAmount)
    {
        PoolInfo storage pool = poolInfo[_pid];
        uint256 totalLpBalance = pool.lpToken.balanceOf(address(this));
        uint256 totalLpAmount = totalLpBalance >= pool.totalInputTokensStaked
            ? pool.totalInputTokensStaked
            : totalLpBalance;
        withdrawableAmount = _amount.mul(totalLpAmount).div(pool.totalInputTokensStaked);
    }
}
...
interface IStrategy {
    ...
    function getTotalLPStaked() external view returns (uint256 totalLPStaked);
}
