...
// File contracts/GameSessionManager.sol
...
/**
 * @title GameSessionManager, a contract to register game sessions in GAMEE Arc8 app.
 */
contract GameSessionManager is  Ownable, Pausable, UsingUniversalForwarding {
    using SafeMath for uint256; 

    /**
     * Event emitted when a game session has been started.
     ...
     * @param sessionType the type of started session - tournament/battle   
     */
    event GameSessionStarted(address indexed user, string sessionId, uint256 sessionType);
    ...
    function start(string calldata sessionId, uint256 sessionType) public whenNotPaused {

        emit GameSessionStarted(_msgSender(), sessionId, sessionType);
    }
