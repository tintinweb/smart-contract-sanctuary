...
contract AssetsRegistry is IAssetsRegistry, AbstractDependant {
    ...
    {
        bytes32[] memory _allAssets = assetParameters.getSupportedAssets();
        ...
        returns (SupplyAssetInfo[] memory _resultArr)
    {
        ...
        for (uint256 i = 0; i < _assetsKeys.length; i++) {
            ...
            MaxSupplyValues memory _maxSupplyValues =
                MaxSupplyValues(
                    _currentLiquidityPool.getMaxToSupply(_userAddr),
                    _currentLiquidityPool.getMaxToWithdraw(_userAddr)
                );

            bool _isAvailableAsCollateral = _parameters.isAvailableAsCollateral(_assetsKeys[i]);
        ...
        returns (BorrowAssetInfo[] memory _resultArr)
    {
        ...
        for (uint256 i = 0; i < _assetsKeys.length; i++) {
            ...
            MaxBorrowValues memory _maxBorrowValues =
                MaxBorrowValues(
                    _currentLiquidityPool.getMaxToBorrow(_userAddr),
                    _currentLiquidityPool.getMaxToRepay(_userAddr)
                );
    ...
    function getAssetsInfo(
        ...
        for (uint256 i = 0; i < _assetsKeys.length; i++) {
            ...
            ILiquidityPool _currentLiquidityPool = _currentKey.getAssetLiquidityPool(_parameters);

            address _assetAddress = _currentLiquidityPool.assetAddr();
            uint256 _userBalance =
                IERC20(_assetAddress).balanceOf(_userAddr).convertTo18(
            ...
            _resultArr[i] = AssetInfo(
                ...
                _isSupply ? _userBalance : _currentLiquidityPool.getMaxToBorrow(_userAddr),
