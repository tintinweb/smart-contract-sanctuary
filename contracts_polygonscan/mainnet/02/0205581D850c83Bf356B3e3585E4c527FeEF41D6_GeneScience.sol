...
contract PetBase is PetAccessControl {
    ...
    function _transfer(address _from, address _to, uint256 _tokenId) internal {
        ...
        addToPetMaps(_to,_tokenId);
        PetIndexToOwner[_tokenId] = _to;
        if (_from != address(0)) {
            ownershipTokenCount[_from]--;
            removeFromPetMapas(_from,_tokenId);
    ...
    mapping(address => uint256[]) petMaps;

    function addToPetMaps(address add, uint256 _petId) internal {
        petMaps[add].push(_petId);
    }
    
    function removeFromPetMapas(address add, uint256 _petId) internal{
        uint pos = 0;
         for (uint i=0; i<petMaps[add].length; i++) {
            if (petMaps[add][i] == _petId){
                pos = i;
                break;
            }
        }
        if (pos > 0){
            for (uint i = pos; i<petMaps[add].length-1; i++){
             petMaps[add][i] = petMaps[add][i+1];
            }
            delete petMaps[add][petMaps[add].length-1];
         petMaps[add].length--;
        }
    }

    function GetPetMaps(address add) external view  returns(uint256[] memory petmapOfAdd) {
        return petMaps[add];
    }
...
contract PetBreeding is PetOwnership {
    ...
    address public dpetToken = 0xcF6007Ecf891340468057b70B58ABE340874e51c;

    /// @dev The Pregnant event is fired when two pets successfully breed and the pregnancy
    ///  timer begins for the matron.
    event Pregnant(address owner, uint256 matronId, uint256 sireId, uint256 cooldownEndBlock);

    uint256 public autoBirthFee = 3*10**18; // pet token
...
contract PetMinting is PetBreeding, IPetCore {
    ...
    uint256 public gen0Price = 3* 10**18;
  ...
  /// @param _owner the future owner of the created pets.
    function createPromoPetMulti(address _owner, uint256 _numPet) external returns(uint256[] memory newPetsr)  {
        require(_numPet > 0, "Num Of Pet Must Be  > 0");
        require(_numPet <= 30, "Num Of Pet Must Be  <= 30");
        uint256 totalPrice = _numPet*gen0Price;
        //require(_amount >= gen0Price, "INVALID AMOUNT");
        require(IKRC20(dpetToken).transferFrom(msg.sender, address(this), totalPrice));
        
        uint256[] memory newPets = new uint256[](_numPet);
        uint256 i;
        for(i = 0; i < _numPet; i++) {
            gen0CreatedCount++;
            uint256 genes = _randomPetGenes();
            newPets[i] =  _createPet(0, 0, 0, genes, _owner);
        }
        return newPets;
...
contract PetCore is PetMinting {
    ...
    uint256 public amountToAdulthood = 3 * 10**18;
    uint256 public amountToMiddleAge = 6 * 10**18;
    ...
    function getPet(uint256 _id)
     ...
     function setPetLevel(uint256 _petId, uint16 _level) external {
         require(_level>0,"Level must be > 0");
         require(_level<=30,"Level must be <= 30");
         
         Pet storage pet = pets[_petId];
         require(pet.lvl < _level, "New Level Must Be Greated");
             pet.lvl = _level;
     }
