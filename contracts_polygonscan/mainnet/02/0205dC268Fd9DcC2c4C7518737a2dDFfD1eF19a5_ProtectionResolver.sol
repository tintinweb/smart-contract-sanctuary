...
contract ProtectionResolver {
    ...
    function multiRepayAndWithdrawAmt(
        RepayAndWithdrawData[] calldata _listRAndWAmt
    ) external view returns (RepayAndWithdrawResult[] memory) {
        RepayAndWithdrawResult[] memory results =
            new RepayAndWithdrawResult[](_listRAndWAmt.length);

        for (uint256 i = 0; i < _listRAndWAmt.length; i++) {
            try this.getRepayAndWithdrawAmt(_listRAndWAmt[i]) returns (
                RepayAndWithdrawResult memory rAndWResult
            ) {
                results[i] = rAndWResult;
            } catch Error(string memory error) {
                results[i] = RepayAndWithdrawResult({
                    id: _listRAndWAmt[i].id,
                    amtOfColToWithdraw: 0,
                    amtToFlashBorrow: 0,
                    message: error.prefix(
                        "ProtectionResolver.getRepayAndWithdrawAmt failed:"
                    )
                });
            } catch {
                results[i] = RepayAndWithdrawResult({
                    ...
                    amtOfColToWithdraw: 0,
                    amtToFlashBorrow: 0,
                    message: "ProtectionResolver.getRepayAndWithdrawAmt failed:undefined"
                });
            }
        }
    ...
    function getRepayAndWithdrawAmt(RepayAndWithdrawData calldata _rAndWAmtData)
        external
        view
        returns (RepayAndWithdrawResult memory)
    {
        ...
        protectionDataCompute.colInETH = _getColInETH(
            _rAndWAmtData.colToken,
            _rAndWAmtData.user,
            pricesInETH[0]
        );

        protectionDataCompute.debtInETH = _getDebtInEth(
            _rAndWAmtData.debtToken,
            _rAndWAmtData.user,
            pricesInETH[1]
        );
...
function _getColInETH(
    address _colToken,
    address _user,
    uint256 _colTknPriceInEth
    ...
    return
        _convertToEth(
            currentATokenBalance,
            _colTknPriceInEth,
            (10**ERC20(_colToken).decimals())
        );
}

function _getDebtInEth(
    address _debtToken,
    address _user,
    uint256 _debtTknPriceInEth
) view returns (uint256) {
...
function _convertToEth(
    uint256 _tknBalance,
    uint256 _tknPriceInEth,
    uint256 _tknDecimal
) pure returns (uint256) {
    return (_tknBalance * _tknPriceInEth) / _tknDecimal;
}
