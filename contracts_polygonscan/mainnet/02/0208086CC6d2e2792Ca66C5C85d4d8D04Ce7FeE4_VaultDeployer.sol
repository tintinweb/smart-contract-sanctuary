...
import "./strategies/StrategyProxy.sol";
import "./Storage.sol";
import "./Vault.sol";
...
contract VaultDeployer {
    ...
    /// @notice Beluga's storage contract for transferring control over contracts.
    address public store;

    /// @notice Implementation contract to use for vault proxies.
    address public vaultImplementation;

    /// @notice The index for the strategy salt, starts at 0 and continues for each strategy deployment.
    uint256 public strategySaltIndex;
    ...
    constructor(address _store, address _vaultImpl) {
        store = _store;
    ...
    function deployVault(
        ...
        VaultProxy proxy = new VaultProxy(vaultImplementation);
        ...
        address strategyImpl = create2Deploy(_bytecode, strategySaltIndex);
        ...
        if(_deployAsMaximizer) {
            ...
            vaultProxy.addRewardToken(vaultReward, 3600);
        }

        vaultProxy.setStorage(store);
        (bool setSuccess, ) = address(strategyProxy).call(abi.encodeWithSignature("setStorage(address)", store));
    ...
    function create2Deploy(
        ...
        assembly {
            deployedContract := create2(0, add(_bytecode, 0x20), mload(_bytecode), _salt)
...
contract Vault is ERC20Upgradeable, IUpgradeSource, ControllableInit, VaultStorage {
    ...
    function withdraw(uint256 _numberOfShares) external override defense updateRewards(msg.sender) {
