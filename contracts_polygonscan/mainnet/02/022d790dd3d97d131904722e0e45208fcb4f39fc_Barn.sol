...
contract Barn is Ownable, IERC721Receiver, Pausable {
  ...
  Woolf public woolf;
  // reference to the SWOOL contract for minting SWOOL earnings
  SWOOL public wool;
  ...
  // amount of SWOOL due for each alpha point staked
  uint256 public woolPerAlpha = 0; 

  uint256 public totalBurnSWOOL = 0; 

  // sheep earn 10000 SWOOL per day
  uint256 public constant DAILY_WOOL_RATE = 10000 ether;
  // sheep must have 2 days worth of SWOOL to unstake or else it's too cold
  uint256 public constant MINIMUM_TO_EXIT = 2 days;
  // wolves take a 20% tax on all SWOOL claimed
  uint256 public constant WOOL_CLAIM_TAX_PERCENTAGE = 20;
  // there will only ever be (roughly) 2.4 billion SWOOL earned through staking
  uint256 public constant MAXIMUM_GLOBAL_WOOL = 2400000000 ether;

  // amount of SWOOL earned so far
  uint256 public totalWoolEarned;
  // number of Sheep staked in the Barn
  uint256 public totalSheepStaked;
  // the last time SWOOL was claimed
  uint256 public lastClaimTimestamp;

  // emergency rescue to allow unstaking without any checks but without SWOOL
  ...
  /**
   * realize SWOOL earnings and optionally unstake tokens from the Barn / Pack
   * to unstake a Sheep it will require it has 2 days worth of SWOOL unclaimed
  ...
  function claimManyFromBarnAndPack(uint16[] calldata tokenIds, bool unstake) external whenNotPaused _updateEarnings {
    ...
    if (owed == 0) return;
    burnValue=owed.mul(50).div(100);
    wool.mint(_msgSender(), owed.sub(burnValue));
    wool.burn(_msgSender(), burnValue);
    totalBurnSWOOL = totalBurnSWOOL.add(burnValue);
  }

  /**
   * realize SWOOL earnings for a single Sheep and optionally unstake it
   * if not unstaking, pay a 20% tax to the staked Wolves
   * if unstaking, there is a 50% chance all SWOOL is stolen
   * @param tokenId the ID of the Sheep to claim earnings from
   * @param unstake whether or not to unstake the Sheep
   * @return owed - the amount of SWOOL earned
   */
  function _claimSheepFromBarn(uint256 tokenId, bool unstake) internal returns (uint256 owed) {
    ...
    if (totalWoolEarned < MAXIMUM_GLOBAL_WOOL) {
      ...
      owed = 0; // SWOOL production stopped already
    } else {
      owed = (lastClaimTimestamp - stake.value) * DAILY_WOOL_RATE / 1 days; // stop earning additional SWOOL if it's all been earned
    }
    if (unstake) {
      if (random(tokenId) & 1 == 1) { // 50% chance of all SWOOL stolen
        _payWolfTax(owed);
        owed = 0;
      }
 ...
 function _delUnShakedList(address account, uint256 tokenId) internal {
  ...
  /**
   * realize SWOOL earnings for a single Wolf and optionally unstake it
   * Wolves earn SWOOL proportional to their Alpha rank
  ...
  /** 
   * add SWOOL to claimable pot for the Pack
   * @param amount SWOOL to add to the pot
   */
  function _payWolfTax(uint256 amount) internal {
    if (totalAlphaStaked == 0) { // if there's no staked wolves
      unaccountedRewards += amount; // keep track of SWOOL due to wolves
      return;
    }
    // makes sure to include any unaccounted SWOOL 
    woolPerAlpha += (amount + unaccountedRewards) / totalAlphaStaked;
    unaccountedRewards = 0;
  }

  /**
   * tracks SWOOL earnings to ensure it stops once 2.4 billion is eclipsed
   */
