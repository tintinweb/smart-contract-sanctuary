...
contract SkinRewards is ISkinRewards, IDescriptorUser, AccessControlEnumerableUpgradeable, PausableUpgradeable {
  ...
  // fixing period 1
  function fixPeriod1TokenStars() public onlyRole(PAUSER_ROLE) {
    Period storage period = _periods[1];
    uint256[14] memory tokenIds = [uint256(6),9,127,14,15,16,17,20,22,23,24,25,26,5];

    uint totalStars = period.totalStars;
    for (uint i=0; i<tokenIds.length; i++) {
      uint tokenId = tokenIds[i];
      uint tokenStars = period.tokenStars[tokenId];
      if (tokenStars == 11) {
        tokenStars = 2;
        totalStars = totalStars - 11 + 2;
      } else if (tokenStars == 21) {
        tokenStars = 3;
        totalStars = totalStars - 21 + 3;
      } else if (tokenStars == 31) {
        tokenStars = 4;
        totalStars = totalStars - 31 + 4;
      }
      period.tokenStars[tokenId] = tokenStars;
    }
    period.totalStars = totalStars;
  }
  
  function fixPeriod2SkinBudget() public onlyRole(PAUSER_ROLE) { 
    Period storage period = _periods[2];
    period.budget = _skill.balanceOf(address(this));
  }
    ...
    external override  
  {
    ...
    if (_currentPeriodIdx > _ClaimablePeriods + 1) { 
      ...
      uint unclaimablePeriodBalance = unclaimablePeriod.budget - unclaimablePeriod.claimedBudget;
      if (unclaimablePeriodBalance > 0) {
        unclaimablePeriod.claimedBudget = unclaimablePeriod.budget;
        _skill.transfer(_treasury, unclaimablePeriodBalance);
      }
    }
