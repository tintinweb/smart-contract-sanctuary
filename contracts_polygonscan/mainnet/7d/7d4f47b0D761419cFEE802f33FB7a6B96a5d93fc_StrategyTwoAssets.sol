...
abstract contract StrategyFarmTwoAssets is BaseStrategyMasterChef {
    ...
    address[] public usdc_dai_path;
    address[] public usdc_mimatic_path;
    ...
    {
        quick_usdc_path = new address[](2);
        quick_usdc_path[0] = quick;
        quick_usdc_path[1] = usdc;

        usdc_dai_path = new address[](2);
        usdc_dai_path[0] = usdc;
        usdc_dai_path[1] = dai;

        usdc_mimatic_path = new address[](2);
        usdc_mimatic_path[0] = usdc;
        usdc_mimatic_path[1] = mimatic;
    }
    ...
    function harvest() public override {
        ...
        if (_quickBalance > harvestCutoff){
            ...
            // Swap to USDC and take performance fee
            _swapUniswapWithPath(quick_usdc_path, _quickBalance);
            uint256 _usdc = IERC20(usdc).balanceOf(address(this));
            if (_usdc > 0) {
                uint256 performanceFee = _usdc.mul(keepFXS).div(keepFXSmax);
                if (performanceFee>0) {
            ...
            // Add Mai/Dai liquidity
            uint256 _usdcBalancePost = IERC20(usdc).balanceOf(address(this));
            _swapUniswapWithPath(usdc_mimatic_path, _usdcBalancePost.div(2));

            uint256 _usdcBalancePost2 = IERC20(usdc).balanceOf(address(this));
            _swapUniswapWithPath(usdc_dai_path, _usdcBalancePost2);

            // Add liquidity
            uint256 _mimatic = IERC20(mimatic).balanceOf(address(this));
            uint256 _dai = IERC20(dai).balanceOf(address(this));
            if (_mimatic > 0 && _dai > 0) {
                IERC20(mimatic).safeApprove(currentRouter, 0);
                IERC20(mimatic).safeApprove(currentRouter, _mimatic);
                IERC20(dai).safeApprove(currentRouter, 0);
                IERC20(dai).safeApprove(currentRouter, _dai);
...
contract StrategyTwoAssets is StrategyFarmTwoAssets {
    // Token addresses
    address public Quick_Rewards = 0xBfBeCAf31F6Aa873660d5b7c98fd8Cbd542cC0fD;
    address public Mai_DAI_LP = 0x74214F5d8AA71b8dc921D8A963a1Ba3605050781;
