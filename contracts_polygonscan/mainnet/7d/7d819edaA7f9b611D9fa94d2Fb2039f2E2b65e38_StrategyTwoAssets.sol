...
abstract contract StrategyFarmTwoAssets is BaseStrategyMasterChef {
    ...
    address public constant dnxc = 0xcaF5191fc480F43e4DF80106c7695ECA56E48B18;
        ...
        )
    {
        ...
        usdc_dnxc_path = new address[](2);
        usdc_dnxc_path[0] = usdc;
        usdc_dnxc_path[1] = dnxc;
    }
    ...
    function harvest() public override {
        ...
        if (_dinoBalance > harvestCutoff){
            ...
            _swapUniswapWithPathQuick(usdc_dnxc_path, _usdcBalancePost.div(2));

            // Add liquidity
            uint256 _usdc = IERC20(usdc).balanceOf(address(this));
            uint256 _dnxc = IERC20(dnxc).balanceOf(address(this));
            if (_usdc > 0 && _dnxc > 0) {
                IERC20(usdc).safeApprove(quickRouter, 0);
                IERC20(usdc).safeApprove(quickRouter, _usdc);
                IERC20(dnxc).safeApprove(quickRouter, 0);
                IERC20(dnxc).safeApprove(quickRouter, _dnxc);
                ...
                IERC20(dnxc).safeTransfer(
                    strategist,
                    IERC20(dnxc).balanceOf(address(this))
                );
            }
        }
        // Stake the LP tokens
...
contract StrategyTwoAssets is StrategyFarmTwoAssets {
    ...
    address public DNXC_USDC_LP = 0xE169a660d720917B4fB7e95f045B6f60a64EB10A;
