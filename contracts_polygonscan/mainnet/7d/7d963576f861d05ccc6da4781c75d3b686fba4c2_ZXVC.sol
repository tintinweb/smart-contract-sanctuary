...
contract TIGStaking is Ownable, IERC1155Receiver, IERC721Receiver, ERC165 {
    ...
    function setUsageFee(uint256 _tokenId, uint56 _fee) public {
        ...
        emit UsageFeeSet(_msgSender(), _tokenId, _fee);
    }
    ...
    function unStakeNFT(uint256 _tokenId) public {
        ...
        if(nft._contractType == stakedNFTHelper.ERC1155) {
            IERC1155(nft._contract).safeTransferFrom(address(this), _msgSender(), nft._id, 1, "");
        } else if(nft._contractType == stakedNFTHelper.ERC721) {
            IERC721(nft._contract).safeTransferFrom(address(this), _msgSender(), nft._id, "");
        } else { require(false, "Impossible!"); }
        
        emit NftUnStaked(_msgSender(), nft._contract, nft._id, block.number, nft._contractType);
    }
