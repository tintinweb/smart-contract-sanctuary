...
contract StrategyMasterchefNative is BaseStrategySingle {
    ...
    /**
        ...
        _configAddress[6]  _destructionAddress
        _configAddress[7]  _feeAddress,
    */
    constructor(
        ...
        address[] memory _earnedToDestructionPath,
        ...
        destructionAddress = _configAddresses[6];
        feeAddress = _configAddresses[7];

        pid = _pid;
        earnedToWnativePath = _earnedToWnativePath;
        earnedToUsdPath = _earnedToUsdPath;
        earnedToDestructionPath = _earnedToDestructionPath;
        earnedToWantPath = _earnedToWantPath;
        operatorAddress = vaultChefAddress;
    ...
    function setFeeAddress(address _to) external onlyOwner {
...
abstract contract BaseStrategySingle is BaseStrategy {
    ...
    function _earn(address _feeAddress) internal override {
        // Harvest farm tokens
        _vaultHarvest();
        ...
        if (earnedAmt > 0) {
            earnedAmt = distributeFees(earnedAmt, _feeAddress);
...
abstract contract BaseStrategy is Ownable, ReentrancyGuard, Pausable {
    ...
    uint256 public distributeFeesAmount;
    uint256 public distributeRewardsAmount;
    ...
    address public rewardAddress = 0xE9D19c5B6D1E3D2e6563721E2FA0091f878A51D9;
    address public withdrawFeeAddress = 0xE9D19c5B6D1E3D2e6563721E2FA0091f878A51D9;
    ...
    uint256 public controllerFee = 66; // 0.66%
    ...
    uint256 public withdrawFeeFactor = 10000; // 0% default withdraw fee
    ...
    address[] public earnedToDestructionPath;
    ...
    modifier onlyOperator() {
        require(_msgSender() == operatorAddress, "!operator");
        _;
    }
    ...
    function deposit(address _userAddress, uint256 _wantAmt) external onlyOperator virtual nonReentrant whenNotPaused returns (uint256) {
    ...
    function withdraw(address _userAddress, uint256 _wantAmt) external onlyOperator nonReentrant returns (uint256) {
    ...
    function distributeFees(uint256 _earnedAmt, address _to) internal returns (uint256) {
        if (controllerFee > 0) {
            ...
            if (fee > 0) {
                ...
                distributeFeesAmount = distributeFeesAmount.add(fee);
    ...
    function distributeRewards(uint256 _earnedAmt) internal returns (uint256) {
        if (rewardRate > 0) {
            ...
            if (fee > 0) {
                ...
                if (earnedAddress == destructionAddress) {
                    // Earn token is DESTRUCTION - don't sell
                ...
                distributeRewardsAmount = distributeRewardsAmount.add(fee);
                _earnedAmt = _earnedAmt.sub(fee);
            }
        }
        ...
        if (buyBackRate > 0) {
            ...
            if (buyBackAmt > 0) {
                ...
                if (earnedAddress == destructionAddress) {
                    ...
                    _safeSwap(
                        buyBackAmt,
                        earnedToDestructionPath,
                        buyBackAddress
                    );
                }
                
                buyBackAmount = buyBackAmount.add(buyBackAmt);
    ...
    function setOperator(address _operatorAddress) external onlyOwner {
        require(address(_operatorAddress) != address(0));
    ...
    function earn(address _feeAddress) external nonReentrant whenNotPaused onlyOperator { 
        _earn(_feeAddress);
    }
