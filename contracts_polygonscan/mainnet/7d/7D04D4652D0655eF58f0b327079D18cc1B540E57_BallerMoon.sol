...
// "To learn then live is Biblical, the same is true in crypto. You must choose to learn in order to truly earn".- SL 
...
contract BallerMoon is Context, Ownable, IERC20, IERC20Metadata {
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;

    string  private constant _name        = "BallerMoon";
    string  private constant _symbol      = "BLM";
    uint8   private constant _decimals    = 18;
    uint256 private constant _totalSupply = 21000000000000 * 10 ** uint256(_decimals);

    // anti-whale
    bool    public _isAntiWhaleEnabled = true;
    uint256 public _antiWhaleThreshold = _totalSupply / 100;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        */
        if (_isAntiWhaleEnabled && !_isExcludedFromAntiWhale[recipient]) {
            require(balanceOf(recipient) <= _antiWhaleThreshold, "Anti whale: can't hold more than the specified threshold");
        }
