...
contract Ice is Ownable, Initializable, ERC20Burnable {
    ...
    uint256 public  maxTotalSupply = 10_000_000_000 ether; // 10 billions
    ...
    function initialize(uint256 _initial) public initializer {
        super._mint(_msgSender(), _initial); // mint a small initial supply to add liquidity
    }

    /* ========== MUTATIVE FUNCTIONS ========== */

    function mint(address _recipient, uint256 _amount) public onlyMinter {
        minters[_msgSender()] = minters[_msgSender()].sub(_amount, "Minting amount exceeds minter cap");
    ...
    function _beforeTokenTransfer(address _from, address _to, uint256 _amount) internal override {
        ...
        if (_to == address(0)) {
            // When burning tokens
            maxTotalSupply = maxTotalSupply.sub(_amount, "Burn amount exceeds max total supply");
        }
    }
    ...
    function resetMaxTotalSupply(uint256 _cap) external onlyOwner {
        require(_cap >= totalSupply(), "_cap is below current total supply");
        maxTotalSupply = _cap;
        emit MaxTotalSupplyUpdated(_cap);
    }
}