...
def ve_for_at(_user: address, _timestamp: uint256) -> uint256:
    """
    @notice Get the veEXP balance for `_user` at `_timestamp`
    @param _user Address to query balance for
    @param _timestamp Epoch time
    @return uint256 veEXP balance
    """
...
def checkpoint_total_supply():
    """
    @notice Update the veEXP total supply checkpoint
...
def claim(_addr: address = msg.sender) -> uint256:
    ...
    @dev Each call to claim look at a maximum of 50 user veEXP points.
         For accounts with many veEXP related actions, this function
