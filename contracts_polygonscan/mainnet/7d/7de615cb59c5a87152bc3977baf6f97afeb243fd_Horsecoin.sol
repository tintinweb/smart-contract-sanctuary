...
contract Horsecoin is Ownable {
    ...
    function setSwapAndLiquifyEnabled(
        ...
        require(swapAndLiquifyEnabled != _swapAndLiquifyEnabled, "Horsecoin: can't set the same status");
    ...
    function setLuckyBonusRate(uint256 _luckyBonusRate) external onlyOwner {
        require(_luckyBonusRate <= 10000, "Horsecoin: the ratio must be less than 10000");
    ...
    function _transferSell(
        ...
        bool overMinTokenBalance = contractTokenBalance >= numTokensSellToAddToLiquidity; // 是否超出加池子阈值
    ...
    // 交换代币并添加流动性
    function _swapAndLiquify() public virtual lockTheSwap {
        uint256 contractTokenBalance = balanceOf(address(this));
        // 将合约中的代币分为两部分
    ...
    // 将代币兑换为BNB
    function _swapTokensForEth(uint256 tokenAmount) public virtual {
    ...
    // 添加流动性
    function _addLiquidity(uint256 tokenAmount, uint256 ethAmount) public virtual {
