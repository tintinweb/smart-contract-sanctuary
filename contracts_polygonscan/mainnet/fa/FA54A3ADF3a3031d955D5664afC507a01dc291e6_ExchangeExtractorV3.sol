...
contract ExchangeExtractorV3 {
  ...
  function estimateSwap(address[] calldata routers, address[][] calldata paths, uint amountIn) external view returns(uint) {
      uint currentAmountIn = amountIn;
      uint[] memory amounts;
      for (uint i = 0; i < routers.length; i++) {
          amounts = IUniswapV2Router02(routers[i]).getAmountsOut(currentAmountIn, paths[i]);
          currentAmountIn = amounts[amounts.length - 1];
      }
      return currentAmountIn;
  }

  function estimateSwaps(address[][] calldata routers, address[][][] calldata paths, uint amountIn) external view returns(uint[] memory) {
      uint[] memory results = new uint[](routers.length);
      for (uint i = 0; i < routers.length; i++) {
          results[i] = this.estimateSwap(routers[i], paths[i], amountIn);
      }
      return results;
  }
}