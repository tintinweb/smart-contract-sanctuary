...
abstract contract JarBase is ERC20, Ownable {
    ...
    function deposit(uint256 _amount) public {
        ...
        address masterChefAddr = IStrategy(strategy).chef();
    ...
    function withdraw(uint256 _shares) public {
        ...
        if (bal < amount) {
            uint256 _withdraw = amount.sub(bal);
        ...
        //    uint256 WithdrawalFee = amount.mul(withdrawPenalty).div(keepMax);
...
//Pair name can be seen at strategies with function of pairname().
//Generates the name "FishVault" and "mFish"
    constructor(IStrategy _strategy)
        
        JarBase(_strategy,
            string(abi.encodePacked(_strategy.pairName(), "Vault")),
            string(abi.encodePacked("m", _strategy.pairName()))
        ) 
    {
