...
contract Polyholand is Context, ERC20, Ownable {
    ...
    // Address that include in monster LP list
    mapping(address => bool) private _includeInMonsterLpList;
    ...
    string private _name = "Polyholand";
    string private _symbol = "PLD";
    uint8 private _decimals = 18;

    uint256 public _taxFee = 200;
    uint256 private _previousTaxFee = _taxFee;
    uint256 public immutable MAX_TAX_FEE = 1000;

    uint256 public _liquidityFee = 800;
    uint256 private _previousLiquidityFee = _liquidityFee;
    uint256 public immutable MAX_LIQUIDITY_FEE = 1000;

    // Default zero for presale. After presale set to a greater amount
    uint256 public _maxTxAmount = 0;
    uint256 public immutable MIN_TX_AMOUNT_HARD_CAP = 1 * 10**18;

    uint256 private minimumTokensBeforeSwap = 50000 * 10**18;
    uint256 private buyBackUpperLimit = 40 * 10**14; //40 * 0.0001 MATIC

    // Minimum MATIC balance before buyback IF lower than this number no buyback
    uint256 public minimumBalanceRequired = 1 * 10**14; //0.0001 MATIC

    // Minimum MONSTER sell order to trigger buyback
    ...
    constructor() public {
        ...
        _includeInMonsterLpList[uniswapV2Pair] = true;
    ...
    function excludeFromReward(address account) public onlyOwner {
        ...
        uint256 rOwnedAccountValue = _rOwned[account];
        if (rOwnedAccountValue > 0) {
            _tOwned[account] = tokenFromReflection(rOwnedAccountValue);
        }
    ...
    function isIncludedInMonsterLpList(address _address)
        public
        view
        returns (bool)
    {
        return _includeInMonsterLpList[_address];
    }

    function setIncludeInMonsterLpList(address _address, bool _isIncludedOrNot)
        public
        onlyOwner
    {
        require(_address != address(0), "BEP20: invalid LP address");
        require(
            _address != uniswapV2Pair || _isIncludedOrNot,
            "Cannot exclude uniswapV2Pair"
        );
        _includeInMonsterLpList[_address] = _isIncludedOrNot;
    }
    ...
    function _transfer(
        ...
        if (
            ...
            !isIncludedInMonsterLpList(from) &&
            from != owner()
        ) {
            ...
            if (isIncludedInMonsterLpList(to)) {
                uint256 balance = address(this).balance;
                if (
                    buyBackEnabled &&
                    balance >= minimumBalanceRequired &&
    ...
    function setMaxTxAmount(uint256 maxTxAmount) external onlyOwner {
        require(
            ...
            "maxTxAmount have to be greater than 1 monster"
        );
