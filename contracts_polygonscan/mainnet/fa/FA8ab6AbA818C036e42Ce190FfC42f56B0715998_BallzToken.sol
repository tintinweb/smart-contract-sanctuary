...
contract BallzToken is IERC20, Ownable {
    ...
    string private _name = "Euro Ballz Token";
    string private _symbol = "BALLZ";
    uint8 private _decimals = 18;

    uint256 public _taxFee = 60;
    uint256 public _liquidityFee = 20;
    uint256 public _burnFee = 10;
    uint256 public _referralPoints = 10;
    ...
    modifier onlyFarm {
        require(msg.sender == ballzFarm, "only farm");
        _;
    }
    ...
    function setTaxFeePercent(uint256 taxFee) external onlyOwner {
        require(taxFee <= 150, "Too high fee");
        _taxFee = taxFee;
    }

    function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner {
        require(liquidityFee <= 150, "Too high fee");
        _liquidityFee = liquidityFee;
    }

    function setBurnFeePercent(uint256 burnFee) external onlyOwner {
        require(burnFee <= 150, "Too high fee");
        _burnFee = burnFee;
    }

    function setReferralFeePercent(uint256 referralPoints) external onlyOwner {
        require(referralPoints <= 150, "Too high value");
        _referralPoints = referralPoints;
    }

    // set _maxTxAmount in permille, not percent
    function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner {
        require(maxTxPercent <= 1000 && maxTxPercent >= 3, "Wrong amount");
        _maxTxPercent = maxTxPercent;
        require(maxTxAmount() > numTokensSellToAddToLiquidity, "must be greater than liquify threshold");
    }

    function setNumTokensSellToAddToLiquidity(uint256 _newValue) external onlyOwner {
    ...
    function _getTValues(uint256 tAmount, bool takeFee) private view returns (uint256) {
        (uint256 tFee, uint256 tLiquidity, uint256 tBurn, uint256 tReferral) = _getFeeValues(tAmount, takeFee);
        uint256 tTransferAmount = tAmount.sub(tFee.add(tLiquidity).add(tBurn).add(tReferral));
    ...
    function _getFeeValues(uint256 tAmount, bool takeFee) private view returns (uint256, uint256, uint256, uint256) {
        uint256 tFee;
        uint256 tLiquidity;
        uint256 tBurn;
        uint256 tReferral;
    ...
    function _reflectFee(uint256 tFee, uint256 tBurn, uint256 tReferral, uint256 currentRate) private {
    ...
    function _takeFee(address recipient, uint256 tAmount, uint256 currentRate) private {
        uint256 rAmount = tAmount.mul(currentRate);
    ...
    function _transfer(
        ...
        if(from != owner() && to != owner() && from != ballzFarm && to != ballzFarm)
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        if(tokenAmount > 1000) { //safety check for not calling swap with zero amount
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        if(ethAmount > 1000) { //safety check for not adding liquidity with a zero value
    ...
    function mint(address account, uint256 amount) external onlyFarm {
        ...
        require(_rOwned[account] <= _rTotal, "user reflection token amount should be small than total reflection tokens");
        if(isExcludedFromReward(account)) {
            _tOwned[account] = _tOwned[account].add(amount);
            _tTotalExcluded = _tTotalExcluded.add(amount);
            _rTotalExcluded = _rTotalExcluded.add(rAmount);
        }
    ...
    function burn(uint256 tBurn) external returns (bool){
        ...
        if(isExcludedFromReward(account)) {
            ...
            _tTotalExcluded = _tTotalExcluded.sub(tBurn);
            _rTotalExcluded = _rTotalExcluded.sub(rBurn);
        }
    ...
    function setBallzFarm(address _farmAddress) external onlyOwner {
        require(ballzFarm == address(0), "already set");
        ballzFarm = _farmAddress;
        excludeFromFee(ballzFarm);
        excludeFromReward(ballzFarm);
    }
