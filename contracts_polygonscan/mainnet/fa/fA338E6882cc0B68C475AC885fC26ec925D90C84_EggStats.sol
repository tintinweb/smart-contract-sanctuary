...
contract EggStats is IEggStats, OwnableUpgradeable {
    address public nft;
    address public factory;
    // The distribution for prize money
    // 10ether -  15ether - 20ether - 25ether - 30ether
    uint256[5] public priceDistribution;
    ...
    function initialize(address _nft, address _factory) external initializer {
        ...
        nft = _nft;
        factory = _factory;
        priceDistribution = [10 ether, 15 ether, 20 ether, 25 ether, 30 ether];
    }
    ...
    function setPriceDistribution(uint256[5] memory _priceDistribution) external onlyOwner {
    ...
    function stats(uint256 tokenId) external override view returns (uint16[] memory eggStats, uint16 luckStat) {
        EggInfo memory _info = info_[tokenId];
        eggStats = new uint16[](2);
        eggStats[0] = _info.typeEgg;
        eggStats[1] = _info.bornAt;
        luckStat = _info.Luck;
    }
