...
// IceToken with Governance.
contract IceToken is ERC20('IceToken', 'ICE') {
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        ...
        if (maxTransferAmount() > 0 && !antiWhaleDeactivate) {
              require(amount <= maxTransferAmount(), "ICE::antiWhale: Transfer amount exceeds the maxTransferAmount");
        }
        _;
    }
    function setWhitelist(
        ...
        if(inWhitelist){ 
            require(_specialSendCommission <= MAXIMUM_TRANSFER_TAX_RATE, "ICE::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
            require(_specialReceiveCommission <= MAXIMUM_TRANSFER_TAX_RATE, "ICE::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
        }
    ...
    function updateTransferTaxRate(uint256 _transferTaxRate) external onlyDeveloper {
        require(_transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "ICE::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function setDeveloperAddress(address _newDeveloper) external onlyDeveloper{
        require(_newDeveloper != address(0), "Developer can not be address 0");
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount){
        ...
        require(amount == sendAmount + taxAmount, "ICE::transfer: Tax value invalid");
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    IceToken public immutable ice;
    address public devAddress;
    address public feeCommissionAddress;
    address public feeTreasuryAddress;
    uint256 public icePerBlock;
    uint256 public constant commission = 5000;
    ...
    event Add(uint256 pid);
    event Set(uint256 pid);
    event SetStartBlock(uint256 indexed newStartBlock);
    event Deposit(address indexed user, uint256 indexed pid, uint256 amount);
    event Withdraw(address indexed user, uint256 indexed pid, uint256 amount);
    event EmergencyWithdraw(address indexed user, uint256 indexed pid, uint256 amount);
    event RewardLockedUp(address indexed user, uint256 indexed pid, uint256 amountLockedUp);
    event SetDeveloper(address indexed user, address indexed newDev);
    event SetFeeCommissionAddress(address indexed user, address indexed newFeeCommissionAddress);
    event SetFeeTreasuryAddress(address indexed user, address indexed newFeeTreasuryAddress);
    event EmissionRateUpdated(address indexed caller, uint256 previousAmount, uint256 newAmount);
    
    constructor(
        IceToken _ice,
        address _devAddress,
        address _feeCommissionAddress,
        address _feeTreasuryAddress,
        uint256 _icePerBlock,
        uint256 _startBlock
    ) public {
        require(_devAddress != address(0), "Developer can not be address 0");
        require(_feeCommissionAddress != address(0), "Fee Commission Address can not be address 0");
        require(_feeTreasuryAddress != address(0), "Fee Treasury Address can not be address 0");
        require(_icePerBlock <= MAX_EMISSION_RATE, "To much");
        require(_startBlock >= block.number);
        ice = _ice;
        devAddress = _devAddress;
        feeCommissionAddress = _feeCommissionAddress;
    ...
    function set(uint256 _pid, uint256 _allocPoint, uint16 _depositFeeBP, uint256 _harvestInterval, bool _withUpdate) external onlyOwner {
        ...
        if(_allocPoint == 0){ poolExistence[poolInfo[_pid].lpToken] = false; }
    ...
    function pendingIce(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accIcePerShare = accIcePerShare.add(iceReward.mul(1e18).div(lpSupply));
        }
        uint256 pending = user.amount.mul(accIcePerShare).div(1e18).sub(user.rewardDebt);
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (_amount != 0) {
            ...
            if (pool.depositFeeBP != 0) {
                ...
                uint256 depositFeeForTreasury = depositFee.sub(depositFeeCommission);
                pool.lpToken.safeTransfer(feeCommissionAddress, depositFeeCommission);
                pool.lpToken.safeTransfer(feeTreasuryAddress, depositFeeForTreasury);
    ...
    function payOrLockupPendingIce(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        uint256 pending = user.amount.mul(pool.accIcePerShare).div(1e18).sub(user.rewardDebt);
        if (canHarvest(_pid, msg.sender)) {
            if (pending != 0 || user.rewardLockedUp != 0) {
                ...
                // send rewards
                safeIceTransfer(msg.sender, totalRewards);
            }
        } else if (pending != 0) {
    ...
    function setDevAddress(address _devAddress) external {
        ...
        emit SetDeveloper(msg.sender, _devAddress);
    }

    function setFeeCommissionAddress(address _feeCommissionAddress) external {
        require(msg.sender == feeCommissionAddress, "setFeeCommissionAddress: FORBIDDEN");
        require(_feeCommissionAddress != address(0), "Fee Commission Address can not be address 0");
        feeCommissionAddress = _feeCommissionAddress;
        
        emit SetFeeCommissionAddress(msg.sender, _feeCommissionAddress);
    }
    function setFeeTreasuryAddress(address _feeTreasuryAddress) external {
        require(msg.sender == _feeTreasuryAddress, "setFeeTreasuryAddress: FORBIDDEN");
        require(_feeTreasuryAddress != address(0), "Fee Treasury Address can not be address 0");
        feeTreasuryAddress = _feeTreasuryAddress;
        
        emit SetFeeTreasuryAddress(msg.sender, _feeTreasuryAddress);
    }
    //Pancake has to add hidden dummy pools inorder to alter the emission, here we make it simple and transparent to all.
    function updateEmissionRate(uint256 _icePerBlock) external onlyOwner {
        require(_icePerBlock <= MAX_EMISSION_RATE, "To much");
        massUpdatePools();
        emit EmissionRateUpdated(msg.sender, icePerBlock, _icePerBlock);
        icePerBlock = _icePerBlock;
    }
}