...
contract MeebMasterAirdrop is ReentrancyGuard {
    address private nft_;
    uint256 private totalAirdrop_;

    /* ========== EVENTS ========== */

    event AirdropToken(address indexed from, address indexed to, uint256 tokenId);
    ...
    function totalAirdrop() external view returns (uint256) {
        return totalAirdrop_;
    }

    /* ========== MUTATIVE FUNCTIONS ========== */

    function disperse(address[] memory receivers, uint256 quantity) external nonReentrant {
        BNFT _nft = BNFT(nft_);
        uint256 _nftBal = _nft.balanceOf(msg.sender);
        uint256 _length = receivers.length;
        require(_nftBal >= _length * quantity, "Not enough balance");
        uint256 _tokenIndex = _nftBal;
        for (uint256 i = 0; i < _length; ++i) {
            address _receiver = receivers[i];
            for (uint256 j = 0; j < quantity; ++j) {
                _tokenIndex--;
                totalAirdrop_++;
                uint256 _tokenId = _nft.tokenOfOwnerByIndex(msg.sender, _tokenIndex);
                _nft.safeTransferFrom(msg.sender, _receiver, _tokenId);
                emit AirdropToken(msg.sender, _receiver, _tokenId);
            }
        }
    }
}