...
contract DiamondDaoSale {
    ...
    function deposite(uint256 _amount)  public onlyEOA {
         require(sold+_amount <= 30000);
        ...
        require(!investor[msg.sender],"deposited");
         require(
           ...
           require(_amount <= 100, 'More than alloted');
        IERC20(baseToken).transferFrom(msg.sender, address(this), _amount * PRICE *  10 ** (ERC20(baseToken).decimals()));
    ...
    function claim() public onlyEOA {
        ...
        IERC20(farmingToken).transfer(msg.sender,  purchasedAmounts[msg.sender] * 10 ** (ERC20(farmingToken).decimals()));
        emit Claim();
    }
    ...
    function setWhitelistEnabled(bool _isWhitelistEnabled) public {
        require(msg.sender == dev, "!dev");
        isWhitelistEnabled = _isWhitelistEnabled;
    }
   function whiteListBuyers(address[] memory _buyers) public {
           require(msg.sender == dev, "!dev");
        for (uint256 i; i < _buyers.length; i++) {
            whiteListed[_buyers[i]] = true;
        }
    }
  function transferDEV(address newDEV) public  {
       require(msg.sender == dev, "!dev");
