...
abstract contract StrategyFarmBase is BaseStrategyMasterChef {
    ...
    address public constant ball = 0x883aBe4168705d2e5dA925d28538B7a6AA9d8419;
    ...
    // Other constants
    uint16 public constant underlyingPoolId = 18;
    ...
    {
        ball_dai_path = new address[](3);
        ball_dai_path[0] = ball;
        ball_dai_path[1] = usdc;
        ball_dai_path[2] = dai;
    }
    ...
    function harvest() public override {
        ...
        // Collects Ball tokens
        _getReward();

        uint256 _ballBalance = IERC20(ball).balanceOf(address(this));
        if (_ballBalance > harvestCutoff){
            
            // Swap to wanted token
            _swapUniswapWithPath(ball_dai_path, _ballBalance);
            
            // Collect fee
            uint256 _daiBalance = IERC20(dai).balanceOf(address(this));
            uint256 performanceFee = _daiBalance.mul(keepFXS).div(keepFXSmax);
            if (performanceFee>0) {
...
contract StrategyBase is StrategyFarmBase {
    // Token addresses
    address public BALL_MASTER_CHEF = 0xB5F383998d4E58C140c15C441c75bB79170b6b45;
    address public Dai_LP = 0x8f3Cf7ad23Cd3CaDbD9735AFf958023239c6A063;
