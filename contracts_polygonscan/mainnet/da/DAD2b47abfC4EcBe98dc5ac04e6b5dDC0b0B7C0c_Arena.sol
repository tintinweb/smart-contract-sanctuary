...
contract Arena is Ownable, ERC721Holder {
    ...
    uint16 public usersLimit = 5;    
    uint public minLevel = 1;
    uint public maxLevel = 2;
    uint public moneyOnStart = 3;
    uint public neededFightAmount = 2;
    
    event Fight(Log log);
    
    constructor(BeastNFT n, IERC20 g) {
        nft = n;
        gen = g;
    }
    ...
    uint public preparingTime = 3 * 30;//mins
    uint public fightingTime = 5 * 30;//mins
    uint public cycleTime = 7 * 30;//mins
    
    function start(uint blocksAway) public onlyOwner {
        require(startBlock == 0, "started already");   
        startBlock = block.number - blocksAway;
    }
    ...
    function fight(address enemy) public {
        ...
        require(totalHealth(enemyTeam) > 0,"You have 0 health");
        ...
        for (uint8 i=0;i<3;i++) {
            if (cycle.health[myTeam[i]] > 0) {
                uint16 damage = attack(i,msg.sender,enemy, mySeed, 110) * 10 + uint16(i);
            ...
            if (cycle.health[enemyTeam[i]] > 0) {
                uint16 damage = attack(i,enemy,msg.sender, enemySeed, 100) * 10 + uint16(i);
    ...
    function attack(uint8 index, address me, address enemy, uint mySeed, uint damageMultiplier) private returns (uint16) {//([damage] [enemyIndex])
        Cycle storage cycle = cyclies[getCycleIndex()];
        ...
        uint8 damage = 0;
            ...
            if (enemyHealth > 0) {
                if (mySeed % 10 >= cycle.defence[enemyId]) { //try block
    ...
    function addMonsters(uint nftId1, uint nftId2, uint nftId3) public {
        ...
        uint abAmount1 = nft.abilitiesAmount(nftId1);
        uint abAmount2 = nft.abilitiesAmount(nftId2);
        uint abAmount3 = nft.abilitiesAmount(nftId3);
        
        require(abAmount1 >= minLevel, "don't fit");
        require(abAmount2 >= minLevel, "don't fit");
        require(abAmount3 >= minLevel, "don't fit");
        require(abAmount1 <= maxLevel, "don't fit");
        require(abAmount2 <= maxLevel, "don't fit");
        require(abAmount3 <= maxLevel, "don't fit");
        ...
        gen.transferFrom(msg.sender, address(this), moneyOnStart * 1e18);
        cycle.money[msg.sender] = moneyOnStart * 1e18;
    }
    ...
    function leave(uint cycleIndex) public {
        ...
        if (cycle.usersCount == 1) {
            gen.safeTransfer(address(msg.sender), moneyOnStart * 1e18);
        } else 
        if ((cycle.places[msg.sender] == 0) && (cycle.deadAmount + 1 == cycle.usersCount)) { // only one standing
            IGenNFT(0x330b06C695CfBb51D0cf9D5ed3debA2BD7eFFfB4).createToken(msg.sender, 100);
        }
    }
    ...
    function withdrawAll() external onlyOwner {
        gen.safeTransfer(msg.sender, gen.balanceOf(address(this)));
    }
    ...
    function revive(uint nftId, address owner) public onlyTrusted {
        nft.revive(nftId);
