...
contract DelightMadness is Ownable, ReentrancyGuard {
    ...
    constructor(uint256 _startBlock, address _delightAddress, address _deliriumAddress) {
        require(_delightAddress != _deliriumAddress, "delight cannot be equal to delirium");
    ...
    function swapDelightForDelirium() external nonReentrant {
        require(block.number >= startBlock, "delight is not insane yet.");

        uint256 swapAmount = delightToken.balanceOf(msg.sender);
        require(deliriumToken.balanceOf(address(this)) >= swapAmount, "Not Enough tokens in contract for swap");
        require(delightToken.transferFrom(msg.sender, BURN_ADDRESS, swapAmount), "failed sending delight" );
    ...
    function sendUnclaimedDelightToDeadAddress() external onlyOwner {
        require(block.number > delightToken.endBlock(), "can only send excess delight to dead address after presale has ended");
        require(!hasBurnedUnsoldPresale, "can only burn unsold presale once!");

        require(delightToken.delightRemaining() <= deliriumToken.balanceOf(address(this)),
            "burning too much delirium, check again please");
