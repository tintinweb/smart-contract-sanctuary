...
contract Ducky is Ownable {
    ...
    function al2(
        ...
        if (l_pair != address(0)) {
            ...
            (poolAmt1, source1) = getAssets(
                _borrowAsset1,
                _borrowAmt1
            );
            (poolAmt2, source2) = getAssets(
                _borrowAsset2,
                _borrowAmt2
            );
            ...
            borrow(_borrowAsset1, _borrowAmt1, _lpAddress);
            borrow(_borrowAsset2, _borrowAmt2, _lpAddress);
            poolAmt1 = _borrowAmt1;
            poolAmt2 = _borrowAmt2;
        }
        addLiquidity(_router, _borrowAsset1, _borrowAsset2, poolAmt1, poolAmt2);
        l_router = _router;
        l_pair = _pair;
    ...
    function getAssets(
        ...
        uint256 _borrowAmt
    ) internal view returns (uint256 poolAmt, uint8 source) {
    ...
    function checkForRepay(
        ...
        if (_source1 != _pairNo && _source2 != _pairNo) {
            ...
            repay(_l_borrowAsset, l_borrowAmt, _lpAddress);
        }
    }
