...
/**
 * @title LGT on matic
 * @dev Issues ERC-721 token for LGT
 */
contract LOOKGoldenTicket is ERC721WithSameTokenURIForAllTokens("LOOK Golden Ticket", "LGT"), BaseRelayRecipient, Initializable {

    // @notice event emitted upon construction of this contract, used to bootstrap external indexers
    event LGTNFTContractDeployed();

    // @notice event emitted when token URI is updated
    event LGTTokenUriUpdate(
        string _tokenUri
    );
    ...
    function setTrustedForwarder(address _trustedForwarder) external  {
        require(
            ...
            "LGT.setTrustedForwarder: Sender must be admin"
        );
        trustedForwarder = _trustedForwarder;
    }
    ...
    /**
     @notice Mints a LGT AND when minting to a contract checks if the beneficiary is a 721 compatible
     ...
     */
    function mint(address _beneficiary) public returns (uint256) {
        require(
            accessControls.hasSmartContractRole(_msgSender()) || accessControls.hasMinterRole(_msgSender()),
            "LGT.mint: Sender must have the minter or contract role"
        );
    ...
    function batchMint(address[] memory _recipients) external returns (uint256) {
        require(
            _recipients.length <= maxPerBatch,
            "LGT.batchMint: Amount cannot exceed maxPerBatch"
        );
    ...
    function updateMaxPerBatch(uint256 _maxPerBatch) external {
        require(accessControls.hasAdminRole(_msgSender()), "LGT.updateMaxPerBatch: Sender must be admin");
        maxPerBatch = _maxPerBatch;
    }

    /**
     @notice Burns a LGT, releasing any composed 1155 tokens held by the token itself
    ...
    function burn(uint256 _tokenId) public {
        ...
        require(
            ...
            "LGT.burn: Only garment owner or approved"
        );
    ...
    function batchBurn(uint256[] memory _tokenIds) external returns (uint256) {
        require(
            ...
            "LGT.batchBurn: Amount cannot exceed maxPerBatch"
        );
    ...
    function setTokenURI(string calldata _tokenUri) external {
        require(
            ...
            "LGT.setTokenURI: Sender must be an authorised contract or admin"
        );
        tokenURI_ = _tokenUri;
        emit LGTTokenUriUpdate(_tokenUri);
    }
    ...
    function updateAccessControls(DigitalaxAccessControls _accessControls) external {
        require(accessControls.hasAdminRole(_msgSender()), "LGT.updateAccessControls: Sender must be admin");
        accessControls = _accessControls;
    }
