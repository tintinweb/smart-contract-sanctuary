...
contract PEARLS is ERC20, AccessControl, ERC721Holder {
    ...
    constructor(address owner, address _collectionAddress) ERC20("PEARLS", "PEARLS") AccessControl(){
        ...
        coinAmountPerRewardUnit = 10 * 10 ** 18; // 10 PEARLS coins per rewardUnit
        numberOfBlocksPerRewardUnit = 41142; // 24 hours per reward unit
        welcomeBonusAmount = 180 * 10 ** 18; // 180 Only paid once per tokenId
    }
    ...
    function stake(uint256 tokenId) public {
        ...
        require(IERC721(collectionAddress).ownerOf(tokenId) == address(this),
            "PEARLS: Error while transferring token");
    ...
    function setCollectionAddress(address newAddress) external onlyRole(CONTRACT_ADMIN_ROLE){
        require (newAddress != address(0), "PEARLS: update to zero address not possible");
