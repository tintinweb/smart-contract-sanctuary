...
// Based on Uniswap's staking reward contract at https://etherscan.io/address/0x7FBa4B8Dc5E7616e59622806932DBea72537A56b#code
...
contract MultiStakingRewards is IRewardsDistributionRecipient, ReentrancyGuard {
    ...
    constructor(address _stakingToken, address _rewardDistributor, IVault _vault, address _HOUR) public {
    ...
    function _getReward(address _rewardToken) internal {
        ...
        if (reward > 0) {
            ...
            // Stake HOUR reward in vault
            if (address(pool.rewardToken) == HOUR) {
                pool.rewardToken.approve(address(vault), reward);
                // stake and lock rewards
                vault.stake(msg.sender, reward);
            } else {
    ...
    function notifyRewardAmount(address _rewardToken, uint256 _amount) external override onlyRewardDistributor updateReward(_rewardToken, address(0)) {
        RewardPool storage pool = rewardPools[_rewardToken];
    ...
    function setRewardDistributor(address _rewardDistributor) external onlyGov {
        rewardDistributor = _rewardDistributor;
    }
    ...
    function setHOUR(address _HOUR) external onlyGov {
        HOUR = _HOUR;
    }
    ...
    modifier onlyRewardDistributor() {
        require(msg.sender == rewardDistributor, "!rewardDistributor");
        _;
    }
...
import "./MultiStakingRewards.sol";

contract MultiStakingRewards_hourUSD is MultiStakingRewards {
  constructor(
