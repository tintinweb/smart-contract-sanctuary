...
contract OliveToken is ERC20('OliveCash Token', 'pOLIVE'), Ownable {
...
// File: contracts/farms/olive/OliveBar.sol
...
contract OliveBar is Ownable {

    OliveToken public olive;

    constructor(OliveToken _olive) public {
        olive = _olive;
    }

    // Safe OLIVE transfer function, just in case if rounding error causes pool to not have enough OLIVEs.
    function safeOliveTransfer(address _to, uint256 _amount) public onlyOwner {
        uint256 oliveBal = olive.balanceOf(address(this));
        if (_amount > oliveBal) {
            olive.transfer(_to, oliveBal);
        } else {
            olive.transfer(_to, _amount);
        }
    }
}

// File: contracts/farms/olive/MasterChef.sol
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    // Update the given pool's OLIVE allocation point. Can only be called by the owner.
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 oliveReward = multiplier.mul(olivePerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        olive.mint(devaddr, oliveReward.div(10));
        olive.mint(address(oliveBar), oliveReward);
    ...
    function safeOliveTransfer(address _to, uint256 _amount) internal {
        oliveBar.safeOliveTransfer(_to, _amount);
    }
