...
contract PizzaKitchen is FactoryERC721, Ownable {
  ...
  */
  ...
  constructor(address _pizzaManiaAddress) {

    _name = "PizzaMania!";
    _symbol = "PZZA";

    pizzaManiaAddress = _pizzaManiaAddress;
  ...
  */
  function tokenURI(uint256 _tokenId) override external pure returns (string memory) {
  ...
  */
  ...
  function mint(uint256 _tokenId, address _toAddress) override external {
      canMint(_tokenId);
      _mint(_tokenId,_toAddress);
  }

  function _mint(uint256 _tokenId, address _toAddress) internal {
    bool statechange;
