...
// 艺术品交易合约
contract MetaMarket is ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    function putOnSale(IERC721 _nftToken, uint256 _tokenId, address _payToken, uint256 _price, uint256 _royaltyRate, uint256 _minPrice) external {
        ...
        uint256 _curMinPrice = _art.author == address(0) ? _minPrice : _art.minPrice;
        if (_curMinPrice > 0) {
            uint256 _minAmount = getPayValue(_payToken, _curMinPrice);
            require(_price >= _minAmount, 'Price too low');
        }
        // 第一个挂单的人设置为作者，并设置版税
...
//        require(blackList[address(_nftToken)] == false, 'Not supported');
    ...
    function buyArt(IERC721 _nftToken, uint256 _tokenId, address _payToken, uint256 _price) external payable {
        ...
        if (_price >= _mintAmount) {
            xhgt.mint(msg.sender, xhgtMintAmount);
            xhgt.mint(_nftToken.ownerOf(_tokenId), xhgtMintAmount);
        }
