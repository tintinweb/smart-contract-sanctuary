...
// ----------- cream manipulative function ------------
    ...
    // todo crtoken_amount在redeem之後是否一定是0? 是的話可以直接_position.crtoken_amount = 0;  ?
...
// ----------------------------------------------- end of charged buncker -----------------------------------------------

    // /// @dev Returns in Normalized Value.
    // function getQuickPendingRewardValue(HLSConfig memory self, bool _singleFarm) public view returns (uint256){
    //     // Quickswap pending reward amount
    //     // dQuick_pendingAmount and WMatic_pendingAmount are already in decimals==18
    //     uint256 dQuick_pendingAmount = 0 ;
    //     uint256 WMatic_pendingAmount = 0 ;
    //     if ( _singleFarm == true ) {
    //         dQuick_pendingAmount = IQuickSingleStakingReward(self.staking_reward).earned(address(this)) ;
    //     }
    //     else if ( _singleFarm == false) {  
    //         dQuick_pendingAmount = IQuickDualStakingReward(self.staking_reward).earnedA(address(this)) ;
    //         WMatic_pendingAmount = IQuickDualStakingReward(self.staking_reward).earnedB(address(this)) ;
    //     }
    //     // Quickswap pending reward value
    //     (uint256 dQuick_pendingValue , uint256 WMatic_pendingValue) = getValueFromAmount(self.dQuick_oracle, self.WMatic_oracle, dQuick_pendingAmount, WMatic_pendingAmount);
    //      return dQuick_pendingValue.add(WMatic_pendingValue) ;
    // }
