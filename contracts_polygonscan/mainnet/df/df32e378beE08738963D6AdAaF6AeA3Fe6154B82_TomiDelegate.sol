...
contract TomiDelegate is Ownable {
    ...
    function approveContract(
        ...
        uint256 allowAmount = IERC20(token).totalSupply();
        ...
        )
    {
        ...
        (_amountToken, _amountETH, _liquidity) = ITomiLP(pair).addLiquidityETH{value: msg.value}(
            msg.sender,
            amountTokenDesired,
            amountTokenMin,
            amountETHMin,
        ...
        )
    {
        ...
        (_amountA, _amountB, _liquidity) = ITomiLP(pair).addLiquidity(
            msg.sender,
            amountA,
            amountB,
            amountAMin,
    ...
    function removeLiquidityETH(
        ...
        address pair = getPair[token][WETH];
        (_amountToken, _amountETH) = ITomiLP(pair).removeLiquidityETH(
            msg.sender,
            liquidity,
            amountTokenMin,
            amountETHMin,
...
contract TomiLP is BaseShareField {
    ...
    event AddLiquidity(address indexed user, uint256 amountA, uint256 amountB, uint256 value);
    event RemoveLiquidity(address indexed user, uint256 amountA, uint256 amountB, uint256 value);
        ...
        )
    {
        ...
        (_amountToken, _amountETH, _liquidity) = ITomiPlatform(PLATFORM).addLiquidityETH{value: msg.value}(
            token,
            amountTokenDesired,
            amountTokenMin,
            amountETHMin,
        ...
        (uint256 amountA, uint256 amountB) = token == tokenA ? (_amountToken, _amountETH) : (_amountETH, _amountToken);
        ...
        )
    {
        ...
        TransferHelper.safeTransferFrom(tokenA, msg.sender, address(this), amountA);
        TransferHelper.safeTransferFrom(tokenB, msg.sender, address(this), amountB);
        (_amountA, _amountB, _liquidity) = ITomiPlatform(PLATFORM).addLiquidity(
            tokenA,
            tokenB,
            amountA,
            amountB,
    ...
    function removeLiquidityETH(
        ...
        (_amountToken, _amountETH) = ITomiPlatform(PLATFORM).removeLiquidityETH(
            token,
            liquidity,
            amountTokenMin,
            amountETHMin,
    ...
    function removeLiquidity(
        ...
        require(msg.sender == owner, 'Tomi LP Forbidden');
        (_amountA, _amountB) = ITomiPlatform(PLATFORM).removeLiquidity(
            tokenA,
            tokenB,
            liquidity,
            amountAMin,
    ...
    function _currentReward() internal view override returns (uint256) {
        ...
        uint256 countractAmount = mintedShare.add(IERC20(shareToken).balanceOf(address(this))).sub(totalShare);
        if (pair != address(0)) {
            uint256 poolAmount = ITomiPool(POOL).queryReward(pair, address(this));
...
contract BaseShareField {
    ...
    function _mint(address user) internal virtual returns (uint256) {
        _update();
        _audit(user);
