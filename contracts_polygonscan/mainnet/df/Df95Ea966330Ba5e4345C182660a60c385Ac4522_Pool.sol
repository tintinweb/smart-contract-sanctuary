...
contract Pool is Ownable, ReentrancyGuard, IPool {
    ...
    function collateralDollarBalance()
        external
        view
        override
        returns (uint256)
 ...
 // 1 CNBTC  = 1000$
    function mint(
        ...
        uint256 _actual_dollar_value_sub_fee = _total_dollar_value - (_total_dollar_value * _minting_fee / PRICE_PRECISION) ;
        uint256 _dollarPrice = ITreasury(treasury).dollarPrice();
        uint256 _actual_dollar_amount = _actual_dollar_value_sub_fee * PRICE_PRECISION / _dollarPrice;
    ...
    function redeem(
        ...
        uint256 _dollarPrice = ITreasury(treasury).dollarPrice();
        uint256 _dollat_value_post_fee = _dollar_amount_post_fee * _dollarPrice / PRICE_PRECISION;

        if (_effective_collateral_ratio < COLLATERAL_RATIO_MAX) {   
            uint256 _share_output_value = _dollat_value_post_fee * (PRICE_PRECISION - _effective_collateral_ratio) / PRICE_PRECISION;
            _share_output_amount = (_share_output_value * PRICE_PRECISION) / _share_price;    
        }

        if (_effective_collateral_ratio > 0) {
            uint256 _collateral_output_value = _dollat_value_post_fee * _effective_collateral_ratio / PRICE_PRECISION;
        ...
        if (_collateral_output_amount > 0) {
            ...
            unclaimed_pool_collateral += _collateral_output_amount;
        }

        if (_share_output_amount > 0) {
            redeem_share_balances[msg.sender] += _share_output_amount;
            unclaimed_pool_share += _share_output_amount;
        }
    ...
    function collectRedemption() external {
        ...
        emit RedemptionCollected(msg.sender, _share_amount, _collateral_amount);
    }

    /* ========== RESTRICTED FUNCTIONS ========== */

    // move collateral to new pool address
    function migrate(address _new_pool)
        external
        override
        nonReentrant
        onlyOwner
    ...
    function transferCollateralToTreasury(uint256 amount)
        external
        override
        onlyTreasury
    {
