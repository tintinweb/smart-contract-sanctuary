...
contract Strategy_USDC_MATIC is Ownable {
    // This strategy is for polygon network. USDC - MATIC pair. RewardToken = "dQUICK, wMATIC"
    ...
    address public constant lpToken = 0x6e7a5FAFcec6BB1e78bAE2A1F0B612012BF14827;
    address public constant farm = 0x14977e7E263FF79c4c3159F497D9551fbE769625;
    address public router = 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff;
    address public routerMain = 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff;
    ...
    function _addLiquidityFromAllTokens() internal {
        _swapWMATICToToken(getBalanceOfToken(token0)/2, router, token1);
    ...
    function _swapExactETHForTokens(address _token, uint256 _amountETH) internal {
        ...
        path[0] = WMATIC;
        path[1] = _token;
        uint256[] memory amounts = IRouter(router).getAmountsOut(_amountETH/2, path);
    ...
    function _swapLPtoWMATIC(uint256 _percent) internal {
        ...
        if (_totalLP > 0) {
            ...
            _swapTokenToWMATIC(rewardToken);

            _removeLiquidity();

            // swap token1 to WMATIC
            _swapTokenToWMATIC(token1);
        }
    }
    ...
    function _swapTokenToWMATIC(address _token) internal {
        address[] memory path = new address[](2);
        path[0] = _token;
        path[1] = WMATIC;
    ...
    function _swapWMATICToToken(uint256 _amount, address _router, address _token) internal {
        ...
        path[0] = WMATIC;
        path[1] = _token;
        // _amount = WMATIC amount
        uint256[] memory amounts = IRouter(_router).getAmountsOut(_amount, path);
        if(amounts[1] > 100) {
            _swapExactTokensForTokens(_router, _amount, amounts[1], path);  
        }
    }
