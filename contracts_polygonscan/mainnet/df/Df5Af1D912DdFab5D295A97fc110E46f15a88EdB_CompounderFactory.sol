...
contract CompounderFactory is Ownable{
    ...
    bool public useOptimizedReinvest = false;
    ...
    **/
    function depositCompounding(address farmAddress, uint amountToDeposit) external compounderExists(farmAddress){
        ...
        if(ShareToken.totalSupply() != 0){
            ...
            sharesOwed = 1000 * 10**18; //1 share distrbuted NOTE Share uses 18 decimals
        }
    ...
    function harvestCompounding(address farmAddress) external compounderExists(farmAddress) returns(uint timeTillValid) {
        ...
        if(farmAddressToShareInfo[farmAddress].depositToken != farmAddressToShareInfo[farmAddress].rewardToken){
            if(farmAddressToShareInfo[farmAddress].lpFarm){
                ...
                if(farmAddressToShareInfo[farmAddress].lpA != farmAddressToShareInfo[farmAddress].rewardToken){ 
        ...
        if(timeTillValid == 0){//Ensure swap price is valid
            ...
            if(reward > 0) {require(RewardToken.transfer(msg.sender, reward), 'Gravity Finance: TRANSFER FAILED');}
        ...
        if (farmAddressToShareInfo[farmAddress].depositToken != farmAddressToShareInfo[farmAddress].rewardToken){
            ...
            if (farmAddressToShareInfo[farmAddress].lpFarm){//Dealing with an LP farm so swap half the reward for deposit and supply liqduity
                ...
                if(farmAddressToShareInfo[farmAddress].rewardToken != farmAddressToShareInfo[farmAddress].lpA){
                    ...
                    (minAmount,) = IPriceOracle(priceOracle).calculateMinAmount(path[0], slippage, amountToReinvest, IUniswapV2Factory(swapFactory).getPair(path[0], path[1]));
                ...
                if(useOptimizedReinvest){
                    ...
                    (minAmount,) = IPriceOracle(priceOracle).calculateMinAmount(path[0], slippage, optimalAmountA, address(DepositToken));
                    amounts = IUniswapV2Router02(router).swapExactTokensForTokens(
                        optimalAmountA,
                        minAmount,
                        path,
                ...
                if((amounts[0] - token0Var) > 0){
                    ...
                    if((amounts[0] - token0Var) > IERC20(path[0]).balanceOf(address(this))){
                        IERC20(path[0]).transfer(dustPan, IERC20(path[0]).balanceOf(address(this)));
                    }
                    else{
                        IERC20(path[0]).transfer(dustPan, (amounts[0] - token0Var));
                    }
                }
                if((amounts[1] - token1Var) > 0){
                    if((amounts[1] - token1Var) > IERC20(path[1]).balanceOf(address(this))){
                        IERC20(path[1]).transfer(dustPan, IERC20(path[1]).balanceOf(address(this)));
                    }
                    else {
                        IERC20(path[1]).transfer(dustPan, (amounts[1] - token1Var));
                    }
                }
