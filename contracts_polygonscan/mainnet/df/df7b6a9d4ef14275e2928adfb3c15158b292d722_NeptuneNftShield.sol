...
// Neptune Shield protects the NeptuneNFT From burning
contract NeptuneNftShield is BEP20("Neptune Nft Shield", "shieldNEPTUNE") {
    //Required amount of tokens to purchase the NFT shield
    uint256 public shieldPrice;

    //Token used for purchasing the Shield
    IBEP20 public purchaseToken;

    //Admin address who can withdraw the staked tokens
    ...
    constructor(
        ...
        IUniswapV2Router02 _uniswapV2RouterAddress
    ) public {
    ...
    function buyNftShield() external {
        purchaseToken.transferFrom(
            msg.sender,
            address(this),
            shieldPrice.mul(1e16).div(2)
        );
        purchaseToken.transferFrom(msg.sender, address(0), shieldPrice.div(2));
        swapTokensForEth(shieldPrice.mul(1e16).div(2));
        _mint(msg.sender, 1);
    }

    function withdrawPurchaseTokens() external onlyOwner {
