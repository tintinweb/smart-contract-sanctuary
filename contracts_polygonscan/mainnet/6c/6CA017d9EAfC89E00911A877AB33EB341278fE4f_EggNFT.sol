...
contract EggNFT is BNFT, IEggNFT, ReentrancyGuard {
    ...
    function getInfos(uint256 tokenId) external override view returns (uint16[] memory eggStats, uint16 luckStat) {
        if (_tinyEggStats != address(0)) {
            (eggStats, luckStat) = IEggStats(_tinyEggStats).stats(tokenId);
        }
    }
