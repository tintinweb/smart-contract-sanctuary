...
contract ShareTaxWorker is OwnableUpgradeSafe, ReentrancyGuard {
    ...
    function rebalance(uint256 _shareAmount) public checkPublicAllow nonReentrant {
        if (lastTradeTime.add(10 minutes) <= now) {
            uint256 _shareBal = IERC20(share).balanceOf(address(this));
            if (_shareAmount > _shareBal) {
                _shareAmount = _shareBal;
            }
            if (_shareAmount > 0) {
                lastTradeTime = now;
    ...
    function processTheShareTax() public checkPublicAllow {
        collectShareRewards();
        rebalance(maxShareAmountToSell);
        depositToShareRewardBank();
    }

    // compatible with ILiquidityFund
    function addLiquidity(uint256) external nonReentrant {
        processTheShareTax();
    }
