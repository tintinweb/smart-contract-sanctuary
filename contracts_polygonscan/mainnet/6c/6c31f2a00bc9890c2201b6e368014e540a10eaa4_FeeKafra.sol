...
contract FeeKafra is Ownable, IFeeKafra {
    ...
    uint256 public depositFeeSum = 10000;
    ...
    function calculateDepositFee(uint256 _wantAmount, address) external view override returns (uint256) {
    ...
    function setTreasuryFeeDeposit(uint256 _fee) external onlyOwner {
        ...
        depositFeeSum = treasuryFeeDeposit + kswFeeDeposit;
    }
    ...
    function distributeDepositFee(IERC20 _token, address) external override {
        ...
        uint256 treasuryFeeAmount = (feeAmount * treasuryFeeDeposit) / depositFeeSum;
        if (treasuryFeeAmount > 0) {
            _token.safeTransfer(treasuryFeeRecipient, treasuryFeeAmount);
        }

        uint256 kswFeeAmount = (feeAmount * kswFeeDeposit) / depositFeeSum;
        if (kswFeeAmount > 0) {
