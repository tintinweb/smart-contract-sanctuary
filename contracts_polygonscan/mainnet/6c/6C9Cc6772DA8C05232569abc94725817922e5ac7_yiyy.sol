...
contract yiyy {
	...
	string constant public name = "yiyy";
	string constant public symbol = "yiyy";
	...
	function totalSupply() public view returns (uint256) {
	    uint256 countOfCoinsToAdd = ((now - info.coinCreationTime) / 0.25 hours);
	...
	function _transfer(address _from, address _to, uint256 _tokens) internal returns (uint256) {
		...
		if(info.stableCoinSystem){
    		...
    		// If time left
    		if(info.coinWorkingTime + 0.25 hours < now)
    		{
    		    uint256 countOfCoinsToAdd = ((now - info.coinCreationTime) / 0.25 hours);
    		    info.coinWorkingTime = now;
