...
// INCLUDED IS THE SOURCE CODE FOR THE AVATAR ELEMENTS ON MATIC
/**
...
/**
 * @title AVATAR ELEMENTS NFT a.k.a. Child NFTs
 * @dev Issues ERC-1155 tokens which can be held by the parent ERC-721 contract
 */
contract AvatarElementals is ERC1155BurnableV2, BaseRelayRecipient {

    // @notice event emitted on contract creation
    event AvatarElementalsDeployed();
    ...
    function setTrustedForwarder(address _trustedForwarder) external  {
        require(
            ...
            "AvatarElementals.setTrustedForwarder: Sender must be admin"
        );
        trustedForwarder = _trustedForwarder;
    }
    ...
    function createChild(string calldata _uri) external returns (uint256 id) {
        require(
            ...
            "AvatarElementals.createChild: Sender must be smart contract"
        );

        require(bytes(_uri).length > 0, "AvatarElementals.createChild: URI is a blank string");
    ...
    function batchCreateChildren(string[] calldata _uris) external returns (uint256[] memory tokenIds) {
        require(
            accessControls.hasSmartContractRole(_msgSender()),
            "AvatarElementals.batchCreateChildren: Sender must be smart contract"
        );

        require(_uris.length > 0, "AvatarElementals.batchCreateChildren: No data supplied in array");

        uint256 urisLength = _uris.length;
        tokenIds = new uint256[](urisLength);
        for (uint256 i = 0; i < urisLength; i++) {
            string memory uri = _uris[i];
            require(bytes(uri).length > 0, "AvatarElementals.batchCreateChildren: URI is a blank string");
            tokenIdPointer = tokenIdPointer.add(1);
    ...
    function mintChild(uint256 _childTokenId, uint256 _amount, address _beneficiary, bytes calldata _data) external {
        require(
            accessControls.hasSmartContractRole(_msgSender()),
            "AvatarElementals.mintChild: Sender must be smart contract"
        );

        require(bytes(tokenUris[_childTokenId]).length > 0, "AvatarElementals.mintChild: Strand does not exist");
        require(_amount > 0, "AvatarElementals.mintChild: No amount specified");
    ...
    function batchMintChildren(
        ...
        require(
            accessControls.hasSmartContractRole(_msgSender()),
            "AvatarElementals.batchMintChildren: Sender must be smart contract"
        );

        require(_childTokenIds.length == _amounts.length, "AvatarElementals.batchMintChildren: Array lengths are invalid");
        require(_childTokenIds.length > 0, "AvatarElementals.batchMintChildren: No data supplied in arrays");

        // Check the strands exist and no zero amounts
        for (uint256 i = 0; i < _childTokenIds.length; i++) {
            uint256 strandId = _childTokenIds[i];
            require(bytes(tokenUris[strandId]).length > 0, "AvatarElementals.batchMintChildren: Strand does not exist");

            uint256 amount = _amounts[i];
            require(amount > 0, "AvatarElementals.batchMintChildren: Invalid amount");
        }
    ...
    function updateAccessControls(DigitalaxAccessControls _accessControls) external {
        require(
            accessControls.hasAdminRole(_msgSender()),
            "AvatarElementals.updateAccessControls: Sender must be admin"
        );

        require(
            address(_accessControls) != address(0),
            "AvatarElementals.updateAccessControls: New access controls cannot be ZERO address"
        );
    ...
    */
    ...
    onlyChildChain
    {
        ...
        require(user != address(0x0), "AvatarElementals: INVALID_DEPOSIT_USER");
