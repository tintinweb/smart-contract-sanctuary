...
contract PolyrollERC20 is VRFConsumerBase, Ownable, ReentrancyGuard {
    ...
    uint public wealthTaxThreshold = 500000000000000000000;
    uint public wealthTaxBP = 100;

    // Minimum and maximum bet amounts.
    uint public minBetAmount = 4000000000000000000;
    uint public maxBetAmount = 2000000000000000000000;

    // Balance-to-maxProfit ratio. Used to dynamically adjusts maxProfit based on balance.
    uint public balanceMaxProfitRatio = 34;
    ...
    address constant DFYN_MATIC_TOKEN = 0x4c28f48448720e9000907BC2611F73022fdcE1fA; // Dfyn uses a different contract for wrapped MATIC
    ...
    function updateRollGameRatio() internal {
        ...
        if (block.timestamp - rollGameBlockTimestampLast >= 1800) {
            require(IERC20(DFYN_MATIC_TOKEN).balanceOf(gameMaticLP) > 10000 ether, "Less than 10000 MATIC in LP");

            // Get ratio of ROLL to game token multiplied by SAFETY_FACTOR to avoid losing precision.
            uint newRollGameRatio = SAFETY_FACTOR * IERC20(ROLL_TOKEN).balanceOf(ROLL_MATIC_LP) / IERC20(MATIC_TOKEN).balanceOf(ROLL_MATIC_LP) * IERC20(DFYN_MATIC_TOKEN).balanceOf(gameMaticLP) / IERC20(gameToken).balanceOf(gameMaticLP);
