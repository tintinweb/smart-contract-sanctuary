...
contract DollarV2 is ERC20UpgradeSafe, OwnableUpgradeSafe, ReentrancyGuard {
    uint256 public cap;
    mapping(address => bool) public minter;

    address public dollarOracle;

    uint256 public underPegTaxRate; // when dollar is under peg
    uint256 public underPegTaxMax;
    uint256 public boardroomRate;
    address public boardroomFund;
    uint256 public jackpotRate;
    address public jackpotFund;
    uint256 public dollarPriceOne;
    uint256 private _totalBurned;
    uint256 private _totalJackpotAdded;
    uint256 private _totalBoardroomAdded;
    ...
    event JackpotAdded(address indexed account, address taxFund, uint256 amount);
    event BoardroomAdded(address indexed account, address taxFund, uint256 amount);
    event Burn(uint256 amount);
    event DeflationTransfer(address indexed from, address indexed to, uint256 value, uint256 sentAmount);
    ...
    function initialize(address _boardroomFund, address _jackpotFund) public initializer {
        __ERC20_init("TafeDollar.Fi Dollar 2.0", "TDO");
        OwnableUpgradeSafe.__Ownable_init();

        boardroomFund = _boardroomFund;
        jackpotFund = _jackpotFund;

        underPegTaxRate = 2000; // taxFee = (1 - price) / 5
        underPegTaxMax = 1000; // 10%
        boardroomRate = 50; // 1%
        jackpotRate = 50; // 1%
        dollarPriceOne = 1 ether; // $1
        _isExcludedFromFee[_msgSender()] = true;
        _isExcludedToFee[_msgSender()] = true;
        _isExcludedFromFee[_boardroomFund] = true;
        _isExcludedToFee[_boardroomFund] = true;
        _isExcludedFromFee[_jackpotFund] = true;
        _isExcludedToFee[_jackpotFund] = true;

        cap = 1000000000 ether;
        _mint(_msgSender(), 5000000 ether); // to compensate old SDO holders and SDO LPs
    }
    ...
    function setDollarPricePeg(uint256 _dollarPriceOne) external onlyOwner {
        require(_dollarPriceOne >= 0.9 ether && _dollarPriceOne <= 1 ether, "out range"); // [$0.9, $1.0]
        dollarPriceOne = _dollarPriceOne;
    }

    function setUnderPegTax(uint256 _underPegTaxRate, uint256 _underPegTaxMax) external onlyOwner {
        require(_underPegTaxRate <= 5000, "too high"); // <= 50%
        require(_underPegTaxMax <= 2000, "too high"); // <= 20%
        underPegTaxRate = _underPegTaxRate;
        _underPegTaxMax = _underPegTaxMax;
    }

    function setBoardroomRate(uint256 _boardroomRate) external onlyOwner {
        require(_boardroomRate <= 1000, "too high"); // <= 10%
        boardroomRate = _boardroomRate;
    }

    function setBoardroomFund(address _boardroomFund) external onlyOwner {
        boardroomFund = _boardroomFund;
    }
    ...
    function totalBoardroomAdded() external view returns (uint256) {
        return _totalBoardroomAdded;
    }

    function getDollarPrice() public view returns (uint256) {
        if (dollarOracle == address(0)) return dollarPriceOne;
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFee[_sender] && !_isExcludedToFee[_recipient]) {
            uint256 _dollarPrice = getDollarUpdatedPrice();
            if (_dollarPrice < dollarPriceOne) {
                {
                    uint256 _jackpotRate = jackpotRate;
                    if (_jackpotRate > 0) {
                        uint256 _jackpotAmount = _amount.mul(_jackpotRate).div(10000);
                        address _jackpotFund = jackpotFund;
                        _sentAmount = _sentAmount.sub(_jackpotAmount);
                        _totalJackpotAdded = _totalJackpotAdded.add(_jackpotAmount);
                        super._transfer(_sender, _jackpotFund, _jackpotAmount);
                        emit JackpotAdded(_sender, _jackpotFund, _jackpotAmount);
                    }
                }
                {
                    uint256 _boardroomRate = boardroomRate;
                    if (_boardroomRate > 0) {
                        uint256 _boardroomAmount = _amount.mul(_boardroomRate).div(10000);
                        address _boardroomFund = boardroomFund;
                        _sentAmount = _sentAmount.sub(_boardroomAmount);
                        _totalBoardroomAdded = _totalBoardroomAdded.add(_boardroomAmount);
                        super._transfer(_sender, _boardroomFund, _boardroomAmount);
                        emit BoardroomAdded(_sender, _boardroomFund, _boardroomAmount);
                    }
                }
                {
                    uint256 _burnAmount = 0;
                    uint256 _underPegTaxRate = underPegTaxRate;
                    if (_underPegTaxRate > 0) {
                        uint256 _burnRate = dollarPriceOne.sub(_dollarPrice).mul(_underPegTaxRate).div(dollarPriceOne);
                        uint256 _underPegTaxMax = underPegTaxMax;
                        if (_burnRate > _underPegTaxMax) {
                            _burnRate = _underPegTaxMax;
                        }
