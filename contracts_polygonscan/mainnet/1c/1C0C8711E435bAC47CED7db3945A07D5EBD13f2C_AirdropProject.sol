// SPDX-License-Identifier: MIT
    ...
    contract AirdropProject is ERC721, Ownable, ERC721Enumerable {
      ...
      string public hiddenURI = ""; // IPFS URI WILL BE SET AFTER ALL TOKENS SOLD OUT
      ...
      event ProjectMinted(uint256 tokenId, address owner);
      event AirdropSent(uint256 tokenId, address owner);
      event AirdropBulkSent(bool result);

      constructor() ERC721("Team Airdrop Test V2","TAV2") {}
      ...
      function mintProject(uint256 numberOfTokens) external payable {
        ...
        for (uint256 i = 0; i < numberOfTokens; i++) {
          ...
          if (totalSupply() < MAX_TOKENS) {
            ...
            emit ProjectMinted(id, msg.sender);
          }
        }
      }
    
      function presaleProject(uint256 numberOfTokens) external payable {
      ...
      function SendAirdrop (address addr,uint256 id ) external onlyOwner {
        _safeMint(addr, id);
        emit AirdropSent(id, addr);
      }

        function SendBulkAirdrop (address[] calldata _addressList,uint256[] calldata _idList ) external onlyOwner {

           require(_addressList.length == _idList.length, "address and list should be equal length..");

            for (uint256 i = 0; i < _idList.length; i++) {
              _safeMint(_addressList[i], _idList[i]);
            }

          emit AirdropBulkSent(true);
      }
        ...
        returns (string memory)
      {
        ...
        if (isRevealed == false) {
          return
            ...
            ? string(abi.encodePacked(hiddenURI, tokenId.toString(), ".json"))
            : "";
        }
