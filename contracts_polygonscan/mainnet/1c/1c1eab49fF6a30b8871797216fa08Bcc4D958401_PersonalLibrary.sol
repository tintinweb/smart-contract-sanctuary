...
contract PersonalLibrary {
    ...
    enum Strategy { 
        ...
        /*//commented this due to gas overflow. will move strategies to separate contract
        STAKE_WITH_PID, 
        DEPOSIT_WITH_TOKEN, 
    ...
    */
    function provideLiquidity(
        ...
        uint256 _amount, 
        uint256 _minPoolTokens
    ) public strategistOrInvestor inFunction returns (uint256) {
    ...
    */
    function stake(
        ...
        //commented some if-else due to gas overflow. will move strategies to separate contract
    ...
    */
    function unstake(
        ...
        uint256 _pid,
        bytes memory extraBytes
    ) public strategistOrInvestor inFunction {
    ...
    */
    function liquidityToVault(
        ...
        //TODO: think about staking directly (to remove this liquidityToVault function)
    ...
    */
    function unstakeAndWithdrawLiquidity(
        ...
        }else if(_wl.toWhomToIssue == investor){
        ...
        if(_wl.toWhomToIssue != address(this)){
    ...
    @notice stopLossExit also will work with this
    @param _un is a struct of variables that will be used in unstake function
    @param _vl ... vaultToLiquidity function
    @param _wl ... withdrawLiquidity function
    ...
    */
    function claimRewards(
        ...
        if(_poolTemplate == Strategy.DEPOSIT_WITH_PID){
            //the pool doesn't have get reward function, so we call empty deposit to get them
            IStakeOrDepositPoolWithPID(_stakeContractAddress).deposit(_pid, 0);
        }else{
            IStakeOrDepositPool(_stakeContractAddress).getReward();
        }
    ...
    */
    ...
    @param _tokens array of reward token addresses to claim
    */
    function claimRewardNativeTokens(address[] memory _tokens) strategistOrInvestor public {
        ...
        for (uint256 i; i < _tokens.length; i++){
            ...
            ethToDevelopment = convertTokenToETH(_factory.developmentFund(), _tokens[i], ethToDevelopment);
    ...
    @param _tokens array of reward token addresses to claim
    */
    function claimRewardYIELDTokens(address[] memory _tokens) strategistOrInvestor public {
        ...
        /*uint256 ethToDevelopmentFund = */convertTokenToETH(_factory.developmentFund(), yieldToken, yieldToDevelopmentFund);
    ...
    /**
    @notice difference with unstakeAndWithdrawLiquidity: 
    ...
    */
    ...
    @return Amount of LP bought
    */
    function stakeReward(address[] memory _tokens) strategistOrInvestor public returns (uint256) {
        ...
        stake(Strategy(yieldStakeStrategy), yieldStakeContract, yieldStakePair, providedLiquidity, yieldStakePid, bytes(""));
    ...
    */
    function claimRewardsFromYieldStakePool() public strategistOrInvestor {
        ...
        if(Strategy(yieldStakeStrategy) == Strategy.DEPOSIT_WITH_PID){
            //the pool doesn't have get reward function, so we call empty deposit to get them
            IStakeOrDepositPoolWithPID(yieldStakeContract).deposit(yieldStakePid, 0);
        }else{
            IStakeOrDepositPool(yieldStakeContract).getReward();
        }
    ...
    */
    function convertTokenToToken(address _toWhomToIssue, address _tokenToExchange, address _tokenToConvertTo, uint256 _amount) inFunction internal returns (uint256) {       
        IFactory _factory = IFactory(factory);//saves 2.5k gas
        ...
        if(!sameExchange){
            ...
            return convertETHToToken(_toWhomToIssue, _tokenToConvertTo, nativeTokenAmount);
        }
    ...
    @param _amount how much will be converted
    */
    function convertTokenToETH(address _toWhomToIssue, address _tokenToExchange, uint256 _amount) inFunction internal returns (uint256)  {
        ...
        (, address router, address[] memory path) = IFactory(factory).checkIfTokensCanBeExchangedWith1Exchange(_tokenToExchange, networkNativeToken);
    ...
    */
    function convertETHToToken(address _toWhomToIssue, address _tokenToExchange, uint256 _amount) inFunction internal returns (uint256)  {
        ...
        (, address router, address[] memory path) = IFactory(factory).checkIfTokensCanBeExchangedWith1Exchange(networkNativeToken, _tokenToExchange);
    ...
    */
    function convertRewardTokensToYIELD(address _toWhomToIssue, address[] memory _tokens) internal returns (uint256) {
        ...
        for (uint256 i; i < _tokens.length; i++){
            ...
            if(_tokens[i] != yieldToken){
                ...
                if(b > 0){
                    convertTokenToToken(_toWhomToIssue, _tokens[i], yieldToken, b);
                }
            }
        }
    ...
    function _claimRewardByType(uint256 _rewardType, address[] memory _tokens) internal{
        if(_rewardType == 1){
            ...
            claimRewardYIELDTokens(_tokens);
