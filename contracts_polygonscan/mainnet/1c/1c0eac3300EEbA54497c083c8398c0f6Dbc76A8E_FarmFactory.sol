...
contract FarmFactory is ReentrancyGuard, Ownable {
    ...
    function getPoolWeights() external returns(uint[] memory) {
        return farmBuilds[msg.sender].poolWeights;
    }

    function getPoolAddresses() external returns(address[] memory) {
        return farmBuilds[msg.sender].poolAddresses;
    }

    function getPoolDepositFees() external returns(uint16[] memory) {
