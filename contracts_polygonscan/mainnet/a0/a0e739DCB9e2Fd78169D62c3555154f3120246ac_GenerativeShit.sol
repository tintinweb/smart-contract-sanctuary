...
contract GenerativeShit is ERC721Enumerable, ERC721URIStorage, Ownable {
    using Counters for Counters.Counter;
    using SafeMath for uint256;
    string private _baseURIPrefix;

    bytes32 public constant PROVENANCE_HASH = 0xe215ea0d55568c8ac745d835bf2ee6c8980f63e27a680cf48f25369942699bcc;
    string public constant GEN_SEED = "Shit";

    //Base price to start
    uint256 private _basePrice;

    uint256 public _elevate;
    uint256 public _multiply;

    uint32 private constant maxTokensPerTransaction = 10;
    uint256 private constant nftsNumber = 2222;
    ...
    constructor(uint256 basePrice, uint256 elevate, uint256 multiply) ERC721("Generative Shit", "GENSHIT") {
    ...
    function howMany() public view returns(uint256 a){
    ...
    function buy(uint32 tokensNumber) public payable {
        require(mainSaleActive, "Maybe later");
        require(tokensNumber > 0, "U cant mint zero bro");
        require(tokensNumber <= maxTokensPerTransaction, "Save some for everyone else!");
        require(_tokenIdCounter.current().add(tokensNumber) <= nftsNumber, "Sry I dont have enough left ;(");
        uint256 toPay;

        for(uint32 i = 0; i < tokensNumber; i++) {
            uint256 supplyBefore = _tokenIdCounter.current();
            uint256 quotedPrice = price(supplyBefore + 1);
            toPay += (quotedPrice * 1 wei);
            _safeMint(msg.sender, _tokenIdCounter.current());
            emit BuyToken(msg.sender, quotedPrice, _tokenIdCounter.current());
            _tokenIdCounter.increment();
        }
        require(msg.value >= toPay, "Not enough funds to make purchase!");
    }
    ...
    function price(uint256 x) public view returns(uint256) {
        return (((x ** _elevate) * _multiply) + _basePrice);
    }

    /**
     * @dev Get price of next token
     * @param tokensNumber the amount of tokens to buy
     * Assuming current bonding curve function of y = x^2 + basePrice
     * @return price at the specific position in bonding curve
     */
    function quote(uint32 tokensNumber) public view returns(uint256) {
        uint256 toPay;
        for(uint32 i = 1; i <= tokensNumber; i++) {
            uint256 supplyBefore = _tokenIdCounter.current();
            uint256 quotedPrice = price(supplyBefore + i);
            toPay += (quotedPrice * 1 wei);
        }
        return toPay;
    }
