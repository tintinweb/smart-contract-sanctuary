...
contract ArkenDexV2 is OwnableUpgradeable {
    ...
    */
        ...
        returns (uint256 returnAmount, uint256 blockNumber)
    {
        ...
        for (uint256 i = 0; i < routes.length; i++) {
            ...
            if (_ETH_ == address(startToken)) {
                IWETH(_WETH_).withdraw(inputAmount);
            }
            ...
            if (_ETH_ == address(endToken) && outputAmount > 0) {
                IWETH(_WETH_).deposit{value: outputAmount}();
            }
    ...
    */
    ...
    function _tradeIDfyn(
        ...
        if (_WETH_DFYN_ == address(_src)) {
            IWETH(_WETH_).withdraw(inputAmount);
            IWETH(_WETH_DFYN_).deposit{value: inputAmount}();
        }
        if (_ETH_ == address(_src)) {
            // ETH => TOKEN
            if (paths[0] == address(_ETH_)) {
        ...
        if (_WETH_DFYN_ == address(_dest)) {
            IWETH(_WETH_DFYN_).withdraw(amounts[amounts.length - 1]);
            IWETH(_WETH_).deposit{value: amounts[amounts.length - 1]}();
        }
    }
