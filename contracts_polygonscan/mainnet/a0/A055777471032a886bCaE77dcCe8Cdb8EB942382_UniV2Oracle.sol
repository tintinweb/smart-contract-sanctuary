...
///@dev need to have keeper to call this contract to fetch prices
contract UniV2Oracle is Ownable, IOracle, KeeperCompatibleInterface {
	...
	uint256 public period; // dpx use 2 hour
	...
	event UpdatePeriod(uint256 period_);
	event PriceUpdated(address asset_, uint256 newPrice_);
	...
	constructor(
		...
		token0 = _token0;
		token1 = _token1;
		require(token_ == _token0 || token_ == _token1, "UV2O: INVALID_TOKEN");
		token = token_;
		decimals = IERC20(token_).decimals();
		(uint112 reserve0, uint112 reserve1, ) = IUniswapV2Pair(pair_).getReserves();
		require(reserve0 != 0 && reserve1 != 0, "UV2O: NO_RESERVES"); // ensure that there's liquidity in the pair
		priceFeed = AggregatorV3Interface(priceFeed_);

		(uint256 price0Cumulative, uint256 price1Cumulative, uint32 blockTimestamp) = UniswapV2OracleLibrary.currentCumulativePrices(pair_);
		(, int256 feedPrice, , , ) = AggregatorV3Interface(priceFeed_).latestRoundData();
		observations.push(Observation(blockTimestamp, price0Cumulative, price1Cumulative, uint256(feedPrice)));

		require(period_ >= 60, "UV20: PRD INV");
		period = period_;
	...
	function update() external {
		require(_update(), "UV2O: PERIOD_NOT_ELAPSED");
	}
	...
	// show token price in usd (10^8 decimals)
	...
	function consult(uint256 amountIn) public view returns (uint256 amountOut) {
		...
		if (token == token0) {
			amountOut = computeAmountOut(_observation.price0Cumulative, price0Cumulative, _observation.feedPrice, amountIn, timeElapsed);
		} else if (token == token1) {
			amountOut = computeAmountOut(_observation.price1Cumulative, price1Cumulative, _observation.feedPrice, amountIn, timeElapsed);
		}
	}
	 ...
	 * @notice check to see whether we need to udpate the oracle
