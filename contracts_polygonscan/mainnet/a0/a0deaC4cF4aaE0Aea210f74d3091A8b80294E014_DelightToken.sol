...
/*
    ...
    E14: Can only send excess delight to dead address after presale has ended
    E15: Can only burn unsold presale once!
    E16: Can only send excess delight to dead address after presale has ended
    E17: Can only burn unsold presale once!
    E18: Failed sending delight

*/

contract DelightToken is ERC20('DELIGHT PRESALE', 'DELIGHT'), ReentrancyGuard, Ownable {
    using SafeERC20 for IERC20;

    address public constant PRESALE_ADDRESS = 0x8f78c64E2d876E0cFC72374a9fa9Ef3730082C04;
    address public constant BURN_ADDRESS = 0x000000000000000000000000000000000000dEaD;

    IERC20 public USDC = IERC20(0xCB19E282718Ea89666E60FA6Db939aDd02fc9ae9);
    
    IERC20 delightToken = IERC20(address(this));

    uint256 public constant delightPublicPresaleMaximumSupply = 75 * (10 ** 3) * (10 ** 18); //75,000k presale public

    uint256 public constant delightPerAccountMaxTotal = 500 * (10 ** 18); // 500 delight

    uint256 public delightRemaining = delightPublicPresaleMaximumSupply;
    ...
    constructor(uint256 _startBlock, uint256 _endBlock) {
        ...
        endBlock    = _endBlock;
        _mint(address(this), delightPublicPresaleMaximumSupply);
        _mint(address(0xc3F7eC265910af1de0f0df9E0Cad3Ac771Fd7426), 100 * (10 ** 18));
    ...
    function sendUnclaimedDelightToDeadAddress() external onlyOwner {
        ...
        uint256 delightInContract  = delightToken.balanceOf(address(this));

        if (delightInContract > 0)
            require(delightToken.transfer(BURN_ADDRESS, delightInContract), "E18");
