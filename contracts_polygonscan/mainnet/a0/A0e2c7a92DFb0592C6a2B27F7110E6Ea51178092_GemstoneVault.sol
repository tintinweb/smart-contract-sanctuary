...
// File: contracts/Referral/ReferralStorage.sol
...
// File: contracts/base/IBEP20.sol
...
// File: contracts/GemstoneToken.sol
...
contract GemstoneToken is BEP20 {
    ...
    modifier transferTaxCheck(address recipient) {
        uint16 _transferTaxRate = transferTaxRate;
        if(_excludedFromTax[recipient]) {
            transferTaxRate = 0;
        }
        _;
        transferTaxRate = _transferTaxRate;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override transferTaxCheck(recipient) antiWhale(sender, recipient, amount) {
        // swap and liquify
        if (
...
// File: contracts/Referral/Referral.sol
...
// File: contracts/base/SafeBEP20.sol
...
// File: contracts/base/IMasterChef.sol
...
contract GemstoneVault is Referral, Pausable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        uint256 gemstonesAtLastUserAction; // keeps track of gemstones deposited at the last user action
        uint256 lastUserActionTime; // keeps track of the last user action time
        uint256 ownedAmount; // keeps track of the 'owned' amount for user (instead of earned through harvest)
    }
    ...
    /**
     * @notice Constructor
     * @param _gemstones: Gemstones token contract
     * @param _masterchef: MasterChef contract
     * @param _admin: address of the admin
     * @param _treasury: address of the treasury (collects fees)
     *
     * @param _decimals: Decimals for payout for referral                       (default: 10000)
     * @param _secondsUntilInactive: Seconds before referral becomes inactive   (default: 60 * 60 * 24 * 30 = 30 days)
     * @param _onlyRewardActiveReferrers: if only active users are paid         (default: true)
     * @param _levelRate: how to distribute reward over levels                  (default: [6000, 3000, 1000])
     * @param _refereeBonusRateMap: bonus rate map                              (default: [1, 10000])
     * @param _referralStorage: address of referral storage contract
     */
    constructor(
        GemstoneToken _gemstones,
        ...
        // Infinite approve
        IBEP20(_gemstones).safeApprove(address(_masterchef), uint256(-1));
    }
    ...
    /**
     * @notice Deposits funds into the Gemstones Vault
     * @dev Only possible when contract not paused.
     * @param _amount: number of tokens to deposit (in GMS Wei)
     */
    function deposit(uint256 _amount, address _referrerAddress) external whenNotPaused notContract nonReentrant {
        ...
        // retrieve the balance of Vault here for LP
        uint256 beforeTransferBalance = gemstones.balanceOf(address(this));

        // transfer tokens to Vault
        gemstones.safeTransferFrom(msg.sender, address(this), _amount);
        
        // check how much the Vault received
        uint256 afterTransferBalance = gemstones.balanceOf(address(this));
        uint256 actualTransferAmount = afterTransferBalance.sub(beforeTransferBalance);
        
        // Set Shares for user
        uint256 currentShares = 0;
        if (totalShares != 0) {
            currentShares = (actualTransferAmount.mul(totalShares)).div(pool);
        } else {
            currentShares = actualTransferAmount;
        }
        UserInfo storage user = userInfo[msg.sender];

        user.shares = user.shares.add(currentShares);
        user.ownedAmount = user.ownedAmount.add(actualTransferAmount);
        user.lastDepositedTime = block.timestamp;

        // Set last active timer
        referralStorage.setAccountLastActive(msg.sender);

        totalShares = totalShares.add(currentShares);

        user.gemstonesAtLastUserAction = user.shares.mul(balanceOf()).div(totalShares);
        user.lastUserActionTime = block.timestamp;

        _earn();

        emit Deposit(msg.sender, actualTransferAmount, currentShares, block.timestamp);
    }
    ...
    /**
     * @notice Reinvests GMS tokens into MasterChef
     * @dev Only possible when contract not paused.
     */
    function harvest() external notContract whenNotPaused nonReentrant {
        IMasterChef(masterchef).deposit(0,0,ZERO_ADDRESS);

        uint256 bal = available();
        uint256 currentPerformanceFee = bal.mul(performanceFee).div(10000);
        gemstones.safeTransfer(treasury, currentPerformanceFee);

        uint256 currentCallFee = bal.mul(callFee).div(10000);
        gemstones.safeTransfer(msg.sender, currentCallFee);
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the Gemstone Vault
     */
    function inCaseTokensGetStuck(address _token) external onlyAdmin {
        require(_token != address(gemstones), "Token cannot be same as deposit token");
     ...
     * @return Expected reward to collect in GMS
     */
    function calculateHarvestGemstoneRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingGemstones(0, address(this));
     ...
     * @return Returns total pending GMS rewards
     */
    function calculateTotalPendingGemstoneRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingGemstones(0, address(this));
        amount = amount.add(available());
    ...
    /**
     * @notice Withdraws from funds from the Gemstone Vault
    ...
    function withdraw(uint256 _shares) public notContract nonReentrant {
        ...
        uint256 totalSharesAmount = (balanceOf().mul(user.shares)).div(totalShares);
        ...
        uint256 referralDue = totalSharesAmount.sub(user.ownedAmount);
        user.ownedAmount = totalSharesAmount.sub(currentAmount);
        uint256 referralCut = referralDue.mul(referralCutPercentage).div(10000);
        ...
        if (user.shares > 0) {
            ...
            user.gemstonesAtLastUserAction = 0;
        }

        user.lastUserActionTime = block.timestamp;

        // Take referral cut from withdraw -- either burn or distribute
        uint256 actualReward = currentAmount.sub(referralCut);
        ...
        if(isReferralsEnabled() && hasReferrer(msg.sender)) {
           ...
           // Pay the referral cut (note: 60% to first level, 30% to second level, 10% to 3rd)
            ...
            gemstones.safeTransfer(treasury, rest.add(currentWithdrawFee));
                
        } else if(currentWithdrawFee > 0 || referralCut > 0){
            // Transfer to fee address
            gemstones.safeTransfer(treasury, referralCut.add(currentWithdrawFee));
        }

        gemstones.safeTransfer(msg.sender, actualReward);

        emit Withdraw(msg.sender, actualReward, _shares);
    }
    ...
    function available() public view returns (uint256) {
        return gemstones.balanceOf(address(this));
    }
    ...
    function balanceOf() public view returns (uint256) {
        ...
        return gemstones.balanceOf(address(this)).add(amount);
    }
    ...
    function _earn() internal {
        uint256 bal = available();
        if (bal > 0) {
            IMasterChef(masterchef).deposit(0,bal,ZERO_ADDRESS);
        }
    }
