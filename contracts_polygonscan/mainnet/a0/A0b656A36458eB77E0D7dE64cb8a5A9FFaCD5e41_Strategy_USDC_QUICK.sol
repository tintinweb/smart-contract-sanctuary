...
contract Strategy_USDC_QUICK is Ownable {
    // This strategy is for polygon network. USDC - QUICK pair. RewardToken = "dQUICK"
    ...
    address public token1; // QUICK
    address public vault;

    // ------------> Important constants <--------------
    bool public hasPid = false;
    address public constant lpToken = 0x1F1E4c845183EF6d50E9609F16f6f9cAE43BC9Cb;
    address public constant QUICKToken = 0x831753DD7087CaC61aB5644b308642cc1c33Dc13;
    address public constant farm = 0x939290Ed45514E82900BA767bBcfa38eE1067039;
    address public router = 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff; // uses for swapping USDC - QUICK
    ...
    function _addLiquidityFromAllTokens() internal {
        _swapWMATICToToken(getBalanceOfToken(WMATIC)/2, router, token0);
        _swapWMATICToToken(getBalanceOfToken(WMATIC)/2, router, token1);
    ...
    function _addLiquidityETH(uint256 _amountETH) internal {
        _swapExactETHForTokens(token0, _amountETH/2);
        _swapExactETHForTokens(token1, _amountETH/2);
    ...
    function _swapLPtoWMATIC(uint256 _percent) internal {
        ...
        if (_totalLP > 0) {
            ...
            _swapTokenToWMATIC(router, token0);
        }
    }
    ...
    function _swapToken0ToToken1(uint256 _amount) internal {
        ...
        path[1] = token1;
        if(_amount > 0) {
            uint256 amount = _getAmountsOut(router, _amount, path);
            if(amount > 100) {
                _swapExactTokensForTokens(router, _amount, amount, path);  
            }
        }
    }
    ...
    function _getToken0Price() public view returns (uint256) {
        ...
        path[0] = token1;
        path[1] = token0;
        return (50 * 1e36) / _getAmountsOut(router, 50 * 1e18, path) / 1e12;
    }
    ...
    function _getToken1Price() public view returns (uint256) {
        ...
        path[0] = token0;
        path[1] = token1;
        return 1e24 / (_getAmountsOut(router, 50 * 1e6, path) / 50);
    }

    function _getPriceWMATICToUSDC() internal view returns (uint256) {
        address[] memory path = new address[](2);
        path[0] = WMATIC;
        path[1] = token0;
        return (50 * 1e36) / _getAmountsOut(router, 50 * 1e18, path) / 1e12;
    ...
    function _getPriceWMATICToQUICK() internal view returns (uint256 result) {
        // WMATIC to QUICK through USDC
        address[] memory path = new address[](2);
        path[0] = WMATIC;
        path[1] = token0;

        uint256 amountUSDC = _getAmountsOut(router, 50 * 1e18, path);
        if (amountUSDC > 0) {
            path[0] = token0;
            path[1] = token1;
            result = 1e36 / (_getAmountsOut(router, amountUSDC, path) / 50);
        }
    }
    ...
    function _getSimpleTCI(uint256 _amount) internal view returns (uint256 tCI) {
        if(_amount > 0) {
            ...
            tCI = _getPriceWMATICToQUICK() * t1V / 1e18 + _getPriceWMATICToUSDC() * t0V / 1e6;
        }
        ...
        uint256 _balance1 = getBalanceOfToken(token1);
        if (_balance1 > 0) {
            tCI += _getPriceWMATICToQUICK() * _balance1 / 1e18;
        }

        if (_balance0 > 0) {
            tCI += _getPriceWMATICToUSDC() * _balance0 / 1e6;
        }
    ...
    function _getSimpleTC() internal view returns (uint256 TC) {
        ...
        if(_totalLP > 0) {
            ...
            // calculate QUICK as USDC
            if(t1V != 0) {
                path[0] = token1;
                path[1] = token0;
                amount = _getAmountsOut(router, t1V, path);
            }
            // calculate USDC to WMATIC 
            if(t0V + amount != 0) {
                path[0] = token0;
                path[1] = WMATIC;
                TC = _getAmountsOut(router, t0V + amount, path);
            }
        }

        // calculates total Capital from tokens that exist on the contract
        
        // ----> calculates QUICK as USDC <----
        uint256 _balance = getBalanceOfToken(token1);
        amount = 0;
        if(_balance != 0) {
            path[0] = token1;
            path[1] = token0;
            amount = _getAmountsOut(router, _balance, path);
        }

        // ----> calculates USDC to WMATIC  <-----
        _balance = getBalanceOfToken(token0);
        if(_balance + amount != 0) {
            path[0] = token0;
            path[1] = WMATIC;
            TC += _getAmountsOut(router, _balance + amount, path);
        }
