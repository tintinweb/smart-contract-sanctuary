...
// File: contracts/matic/StrategyIron3PUpgraderable.sol
...
contract StrategyIron3PUpgraderable is ReentrancyGuardUpgradeSafe, OwnableUpgradeable , Pausable {
    ...
    function initialize(
        ...
        if (isGrandComp) {
            ...
            IERC20(usdc).approve(doppleSwapAddress, uint256(-1)); // 100% trust in the doppleSwap
        }
    ...
    function earn() public whenNotPaused {
        ...
        address[] memory earnedToFTokenPath = new address[](2);
        (earnedToFTokenPath[0], earnedToFTokenPath[1]) = (earnedAddress, usdc);
        ...
        uint256 fTokenAmt = IERC20(usdc).balanceOf(address(this));
        if (fTokenAmt > 0) {
            uint256[] memory amounts = new uint256[](3);
            (amounts[0], amounts[1], amounts[2]) = (IERC20(usdc).balanceOf(address(this)),0,0);
