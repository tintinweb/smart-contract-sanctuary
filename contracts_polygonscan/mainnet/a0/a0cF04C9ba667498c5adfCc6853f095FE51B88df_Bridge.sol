...
contract Bridge is Ownable {
	...
	function mint(uint256 amount, address to) external onlyMinter() returns (bool) {
		require(amount > 0, 'Amount must be greater than 0');
		require(to != address(0), 'Invalid address');
		IMintable(_token).mint(amount, address(this));
	...
	function redeem(uint256 amount) external returns (bool) {
		...
		_balances[msg.sender] -= amount;
		amountHeld -= amount;
		emit Redeemed(amount, msg.sender);
		return true;
	}

	function updateMinter(address who) public onlyOwner() {
	...
	event Redeemed(uint256 amount, address indexed from);

	modifier onlyMinter() {
		require(msg.sender == _minter, 'Caller cannot excute this function');
		_;
	}
}
