...
contract CrypTooman is ERC20 {
    ...
    address public constant ADDRESS_USDT = 0xc2132D05D31c914a87C6611C10748AEb04B58e8F;// 6 decimals
    address public constant ADDRESS_USDC = 0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174;// 6 decimals
    address public constant ADDRESS_DAI = 0x8f3Cf7ad23Cd3CaDbD9735AFf958023239c6A063;// 18 decimals

    // lending pool address provider
    address public constant ADDRESS_AAVE_LENDING_POOL_ADDR_PROVIDER = 0xd05e3E715d945B59290df0ae8eF85c1BdB684744;
    address public constant ADDRESS_AAVE_INCENTIVES_CONTROLLER = 0x357D51124f59836DeD84c8a1730D72B749d8BC23;
    ...
    function claimProtocolRewards(address[] calldata aTokenAddresses, address recipient) external onlyAdmin(ADMIN_PERMISSION_WITHDRAW) returns (uint256) {
        return IAaveIncentivesController(ADDRESS_AAVE_INCENTIVES_CONTROLLER).claimRewards(aTokenAddresses, uint256(-1), recipient);

        // (bool success, /*bytes memory returnData*/) = ADDRESS_AAVE_INCENTIVES_CONTROLLER.
        //     call(abi.encodeWithSignature("claimRewards(address[],uint256,address)", aTokenAddresses, uint256(-1), owner()));
    }

    function claimWalletProtocolRewards(address userAddress, address aTokenAddress) public returns (uint256) {
        require(_userInternalWallets[userAddress] != address(0) && aTokenAddress != address(0), "CTMN: invalid user wallet or aToken");

        address[] memory aTokenAddresses = new address[](1);
        aTokenAddresses[0] = aTokenAddress;
        return CrypToomanInternalWallet(_userInternalWallets[userAddress]).claimProtocolRewards(ADDRESS_AAVE_INCENTIVES_CONTROLLER, aTokenAddresses);
    }
