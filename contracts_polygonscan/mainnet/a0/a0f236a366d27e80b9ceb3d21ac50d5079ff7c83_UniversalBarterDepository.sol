...
contract UniversalBarterDepository is Ownable {
    ...
    function deposit( 
        ...
        uint value = ITreasury( treasury ).valueOf( principle, _amount );  // 1,000,000,000 (1 USV)    13166036
        uint payout = payoutFor( value ); // payout to barterer is computed // 0.002 USV  002000000  Fraction value/barterPrice   13166036

        require( payout >= 10000000, "Barter too small" ); // must be > 0.01 USV (10e7) ( underflow protection )
        require( payout <= maxPayout(), "Barter too large"); // size protection because there is no slippage    13166036 < 6301355
