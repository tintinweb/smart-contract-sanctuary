...
// File contracts/base/FeeRewardForwarder.sol
...
contract FeeRewardForwarder is Governable {
  ...
  */
    ...
    onlyGovernance
  {
    require(
      from == _route[0],
      "The first token of the route must be the from token"
    );
    require(
      to == _route[_route.length - 1],
      "The last token of the route must be the to token"
    );
  ...
  function poolNotifyFixedTarget(address _token, uint256 _amount) external {
    ...
    if (_token == weth) {
      ...
      if (routes[_token][weth].length > 1) {
        ...
        liquidate(_token, weth, balanceToSwap);

        // now we can send this token forward
        uint256 convertedRewardAmount = IERC20(weth).balanceOf(address(this));

        IERC20(weth).safeTransfer(profitSharingPool, convertedRewardAmount);
        ...
        revert("FeeRewardForwarder: liquidation path does not exist");
      }
    }
  }
