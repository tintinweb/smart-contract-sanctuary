...
contract ProtectionAction is FlashLoanReceiverBase, IProtectionAction, Ownable {
    ...
    function setSlippageInBps(uint256 _slippageInBps) external onlyOwner {
        require(
...
function _checkRepayAndFlashBorrowAmt(
    ...
    RepayAndFlashBorrowResult memory rAndFAmtResult =
    ...
    uint256 oneBpsDiscrepancy =
        _oneBpsDiscrepancy(rAndFAmtResult.amtToFlashBorrow);

    bool isFlashBorrowAmtOk =
        _protectionPayload.amtToFlashBorrow <=
            rAndFAmtResult.amtToFlashBorrow + oneBpsDiscrepancy;
    bool isAmtOfDebtToRepayOk =
