...
abstract contract BaseStrategyMasterChef is BaseStrategy {
    ...
    function getHarvestable() external override view returns (uint256) {
        return IMasterChef(rewards).pendingToken(poolId, address(this));
    }
...
abstract contract StrategyFarmBase is BaseStrategyMasterChef {
    ...
    address public constant sun = 0xbF7413FaACAf0099E1BdEb0Cef87C9ec4aAFD434;
    ...
    {
        sun_usdc_path = new address[](2);
        sun_usdc_path[0] = sun;
        sun_usdc_path[1] = usdc;
    }

    // **** State Mutations ****

    function set_fee(uint256 NewFee) external onlyOwner() {
        require(NewFee <= 100, "New fee must be less than 1000 bps");
        keepFXS = NewFee;
    }
    ...
    function harvest() public override {
        ...
        //Swap 
        uint256 _sunBalance = IERC20(sun).balanceOf(address(this));
        if (_sunBalance > harvestCutoff){
            
            //Send performance fee to strategist
            uint256 performanceFee = _sunBalance.mul(keepFXS).div(keepFXSmax);
            if (performanceFee>0) {
                _swapUniswapWithPath(sun_usdc_path, performanceFee);
            ...
            // Swap rest
            _sunBalance = IERC20(sun).balanceOf(address(this));
            _swapUniswapWithPath(sun_usdc_path, _sunBalance.div(2));

            // Add liquidity
            uint256 _dino = IERC20(sun).balanceOf(address(this));
            uint256 _usdc = IERC20(usdc).balanceOf(address(this));
            if (_dino > 0 && _usdc > 0) {
                IERC20(sun).safeApprove(currentRouter, 0);
                IERC20(sun).safeApprove(currentRouter, _dino);
                ...
                // Donates DUST
                IERC20(sun).safeTransfer(
                    strategist,
                    IERC20(sun).balanceOf(address(this))
                );
                IERC20(usdc).safeTransfer(
                    strategist,
                    IERC20(usdc).balanceOf(address(this))
...
contract StrategyBase is StrategyFarmBase {
    // Token addresses
    address public SUN_MASTER_CHEF = 0x939a890BCdAB6D337af9612EaBbbdaeC5CA3a4FE;
    address public SUN_USDC_LP = 0xA144cEA565B9a04054334891ae0E25FE273aE9b8;
