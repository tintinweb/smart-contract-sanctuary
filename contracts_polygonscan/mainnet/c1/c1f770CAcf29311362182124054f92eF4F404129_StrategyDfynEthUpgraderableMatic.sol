...
// File: contracts/proxy/Pausable.sol
...
// File: contracts/matic/StrategyDfynEthUpgraderableMatic.sol
...
contract StrategyDfynEthUpgraderableMatic is ReentrancyGuardUpgradeSafe, OwnableUpgradeable , Pausable {
    ...
    bool public isGrandComp; // this vault is purely for staking. eg. WBNB-Grand staking vault.
    ...
    address public constant quickRouterAddress = 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff; // swap dino to eth
    address public uniRouterAddress; // uniswap, pancakeswap etc

    address public constant ethAddress = 0x7ceB23fD6bC0adD59E62ac25578270cFf1b9f619;
    address public constant maticAddress = 0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270;
    ...
    address public constant buyBackAddress =
        0xec11AC6dd9aFcBfAE623641692b7132A7FC123Dc;

    address public constant apeRouterAddress = 0xC0788A3aD43d79aa53B09c2EaCc313A787d1d607;

    uint256 public entranceFeeFactor; // < 0.1% entrance fee - goes to pool + prevents front-running
    uint256 public constant entranceFeeFactorMax = 10000;
    uint256 public constant entranceFeeFactorLL = 9950; // 0.5% is the max entrance fee settable. LL = lowerlimit

    address[] public earnedToEthPath;
    address[] public ethToToken0Path;

    address[] public ethToToken1Path;
    address[] public ethToEarnPath;

    address[] public token0ToEthPath;
    address[] public token1ToEthPath;
    ...
    function initialize(
        ...
        if (isGrandComp) {
            ...
            earnedToEthPath = [earnedAddress,ethAddress];
            ethToEarnPath = [ethAddress,earnedAddress];
            ethToToken0Path = [ethAddress, token0Address];
            ethToToken1Path = [ethAddress, token1Address];
            token0ToEthPath = [token0Address, ethAddress];
            token1ToEthPath = [token1Address, ethAddress];
        }

        transferOwnership(GrandFarmAddress);
    }
    ...
    function _farm() internal {
        ...
        emit updateShare(wantLockedTotal,sharesTotal);
    }
        ...
        returns (uint256)
    {
        ...
        IERC20(wantAddress).safeTransfer(GrandFarmAddress, _wantAmt);
    ...
    function earn() public whenNotPaused {
        require(sharesTotal > 0 ,"!zero share");
        ...
        IPancakeRouter02(quickRouterAddress)
        ...
        uint256 ethAmt = IERC20(ethAddress).balanceOf(address(this));
        IERC20(ethAddress).safeIncreaseAllowance(
            uniRouterAddress,
            ethAmt
        );

        if (ethAddress != token0Address) {
      ...
      function buyBack(uint256 _earnedAmt) internal returns (uint256) {
        ...
        (path[0], path[1],path[2]) = (earnedAddress,ethAddress,maticAddress);
        ...
        uint256 maticAmt = IERC20(maticAddress).balanceOf(address(this));
        IERC20(maticAddress).safeIncreaseAllowance(
            apeRouterAddress,
            maticAmt
        );

        (path[0], path[1]) = (maticAddress,GrandAddress);

        IPancakeRouter02(apeRouterAddress)
            .swapExactTokensForTokensSupportingFeeOnTransferTokens(
            maticAmt,
            0,
            path,
    ...
    function convertDustToEarned() public whenNotPaused {
        ...
        uint256 token0Amt = IERC20(token0Address).balanceOf(address(this));
        if (token0Address != ethAddress && token0Amt > 0) {
            IERC20(token0Address).safeIncreaseAllowance(
                uniRouterAddress,
                token0Amt
            );
        ...
        uint256 token1Amt = IERC20(token1Address).balanceOf(address(this));
        if (token1Address != ethAddress && token1Amt > 0) {
            IERC20(token1Address).safeIncreaseAllowance(
                uniRouterAddress,
                token1Amt
            );
    ...
    function setGrandAddress(address _grand) public {
        require(msg.sender == govAddress, "Not authorised");
        GrandAddress = _grand;
    }
