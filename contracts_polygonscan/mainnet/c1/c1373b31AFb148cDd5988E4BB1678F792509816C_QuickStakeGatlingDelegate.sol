...
/** 
 *  SourceUnit: /Users/dingyp/vsCode/sister-in-law/contracts/strategy/quick/QuickStakeGatlingDelegate.sol
*/
...
interface ISilConfig {
    function silMaker() external returns (address);
    // 1000-based
...
contract QuickStakeGatlingDelegate is GatlingStorage, Ownable, IStakeGatling, MasterCaller {
    using SafeERC20 for IERC20;
    using SafeMath for uint256; 

    modifier syncTotalLP {
        syncWithStrategy();
        _;
    }
    ...
    function stake(uint256 _amount) external override syncTotalLP onlyMasterCaller() {
    ...
    function withdraw(uint256 _amount) public override syncTotalLP onlyMasterCaller() {
    ...
    function burn(address _to, uint256 _amount) external syncTotalLP  override onlyMasterCaller() returns (uint256 amount0, uint256 amount1) {
        if(profitStrategy != address(0)) {
            updateRate();
    ...
    function updateRate() private {
        if(now.sub(profitRateUpdateTime).mul(updatesPerDay) >= (1 days)) {
            ...
            if(pendingEarn > 0 && pendingEarn >= updatesMin) {
                IProfitStrategy(profitStrategy).earn();
                _execReprofit();
            }
        }
    ...
    function sellEarn2TokenTwice() private{
        ...
        // transfer to maker
        if(silConfig != address(0)) {
            uint256 makerShare = _minedAmount.mul(ISilConfig(silConfig).gatlingFeeRate()).div(1000);
            if(makerShare > 0) {
                IERC20(earnToken).transfer(ISilConfig(silConfig).silMaker(), makerShare);
                // reset _minedAmount
    ...
    function syncWithStrategy() private {
        totalAmount = IProfitStrategy(profitStrategy).balanceOfLP(profitStrategy);
    }
}