...
contract pShibaX is Context, IERC20 {
    ...
    string private _name = "pShibaX";
    string private _symbol = "pShibaX";
    ...
    uint256 constant public maxMarketingFee = 600;
    uint256 constant public maxBuybackFee = 600;
    ...
    // PCS ROUTER
    address private _routerAddress = 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff;

    address public DEAD = 0x000000000000000000000000000000000000dEaD;
    address public ZERO = 0x0000000000000000000000000000000000000000;
    address payable private _marketingWallet = payable(0x7fb93Ab5AdDb2E09DB7c24331c81f13cE81Fc8F7);
    address payable private _buybackWallet = payable(0x48016eCe62Ed7CA6a07A38055635C4e170773aBb);
...
//===============================================================================================================
    ...
    function setTaxesBuy(uint256 reflectFee, uint256 liquidityFee, uint256 marketingFee, uint256 buyback) external onlyOwner {
        require(reflectFee <= maxReflectFee
                && liquidityFee <= maxLiquidityFee
                && marketingFee <= maxMarketingFee
                && buyback <= maxBuybackFee);
        require(liquidityFee + reflectFee + marketingFee <= 5000);
        _buyLiquidityFee = liquidityFee;
        _buyReflectFee = reflectFee;
        _buyMarketingFee = marketingFee;
        _buyBuybackFee = buyback;
    }

    function setTaxesSell(uint256 reflectFee, uint256 liquidityFee, uint256 marketingFee, uint256 buyback) external onlyOwner {
        require(reflectFee <= maxReflectFee
                && liquidityFee <= maxLiquidityFee
                && marketingFee <= maxMarketingFee
                && buyback <= maxBuybackFee);
        ...
        _sellBuybackFee = buyback;
    }

    function setTaxesTransfer(uint256 reflectFee, uint256 liquidityFee, uint256 marketingFee, uint256 buyback) external onlyOwner {
        require(reflectFee <= maxReflectFee
                && liquidityFee <= maxLiquidityFee
                && marketingFee <= maxMarketingFee
                && buyback <= maxBuybackFee);
