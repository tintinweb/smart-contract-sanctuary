...
contract PetNumber is ERC721Enumerable, VRFConsumerBase, Ownable {
  using Strings for uint256;
  ...
  function mint() public payable {
    ...
    if (msg.sender != owner()) {
      ...
      require(supply + 1  - ownerBalance <= 64512, "Supply limit reached for users");
      
    }

    numbers[supply] = petMaker(random256(randomNumberLink, supply+1));
  ...
  function buildImage(uint256 _tokenId)public view returns(string memory){
      ...
      uint256 divisor = 10**16;
      uint256 newNumber = number;
      string memory textSvg='';
      uint256 numPart;
      for (uint256 i = 415; i > 82; i-=83) {
        numPart = newNumber%divisor;
        newNumber /= divisor;
        textSvg = string(abi.encodePacked(
        '<text x="50%" y="',i.toString(),'" fill="hsl(',fgStr,',100%, 80%)">',
        numPart.toString(),
        '</text>',
        textSvg
        ));
      ...
      return Base64.encode(bytes(abi.encodePacked(
        '<svg xmlns="http://www.w3.org/2000/svg" width="500" height="500" font-size="40" text-anchor="middle">',
