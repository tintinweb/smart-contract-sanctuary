...
contract LeoPolygonMainStaking is Ownable, ReentrancyGuard {
    ...
    uint256 public maxIssued = 26000000000000000000000000; //26M
    ...
    function updatePool(uint256 _pid) public {
        ...
        //only frist month we use emissionSchedul
        if (block.number < startBlock + (blockPerDay * 7 * 4)){
          ...
          //after first mnonth, we just cut inflation by 50% every month
          if (latestMonthlyInflationCut == 0 || latestMonthlyInflationCut < block.number - (blockPerDay * 4 * 7)){
            ...
            uint256 temp = tokensPerBlock;
            tokensPerBlock = tokensPerBlock.div(2);
            //sometimes .div(2) doesn't work for some reason.
            if (temp == tokensPerBlock){
              tokensPerBlock = temp / 2;
            }
          }
        }
