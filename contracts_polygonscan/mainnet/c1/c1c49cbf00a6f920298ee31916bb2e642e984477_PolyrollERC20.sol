...
// File: contracts/PolyrollERC20.sol
...
contract PolyrollERC20 is VRFConsumerBase, Ownable, ReentrancyGuard {
    ...
    address constant ROLL_MATIC_LP = 0x905DCc700fcce9a49b7D907E371230995a45ebCE;
    address gameMaticLP;
    uint public rollGameRatio;
    uint public rollGameBlockTimestampLast;
    uint maxReward = 5000 ether;
    ...
    function setMaxReward(uint _maxReward) external onlyPolyroll {
    ...
    function setStakingPool(uint _houseEdgeSharePercent, address _stakingPool) external onlyPolyroll {
        require(_houseEdgeSharePercent <= 50, 'houseEdgeSharePercent exceeds 50%');
    ...
    function updateRollGameRatio() internal {
        ...
        if (block.timestamp - rollGameBlockTimestampLast >= 1800) {
            ...
            uint newRollGameRatio = IERC20(ROLL_TOKEN).balanceOf(ROLL_MATIC_LP) * SAFETY_FACTOR / IERC20(MATIC_TOKEN).balanceOf(ROLL_MATIC_LP) * IERC20(MATIC_TOKEN).balanceOf(gameMaticLP) / IERC20(gameToken).balanceOf(gameMaticLP);

            if (rollGameRatio == 0) {
                // Update rollGameRatio if first time updating.
                rollGameRatio = newRollGameRatio;
            } else if (newRollGameRatio * SAFETY_FACTOR / rollGameRatio > SAFETY_FACTOR * 110 / 100) {
                // Cap any increase to +10% per update.
                rollGameRatio = rollGameRatio * 110 / 100;
            } else if (newRollGameRatio * SAFETY_FACTOR / rollGameRatio < SAFETY_FACTOR * 80 / 100) {
    ...
    function refundBet(uint betId) external nonReentrant {
        ...
        require(amount > 0, "Bet does not exist");
        require(bet.isSettled == false, "Bet is settled already");
        require(block.number > bet.placeBlockNumber + 10800, "Wait before requesting refund");
