...
contract DeFarmTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SOW }
    ...
    address public SOW;
    uint public SOWQueue; // Delays change to SOW address
    ...
    constructor (
        ...
        isLiquidityToken[ _DeFarmDAI ] = true;
        liquidityTokens.push( _DeFarmDAI );

        blocksNeededForQueue = _blocksNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for DeFarm
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint DeFarm needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( DeFarm ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn DeFarm for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IDeFarmERC20( DeFarm ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( SOW ).balanceOf( msg.sender ); // Can only borrow against SOW held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with DeFarm
        @param _amount uint
     */
    function repayDebtWithDeFarm( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IDeFarmERC20( DeFarm ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, DeFarm, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( DeFarm ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( DeFarm ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns DeFarm valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match DeFarm decimals
            value_ = _amount.mul( 10 ** IERC20( DeFarm ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SOW ) { // 9
            SOWQueue = block.number.add( blocksNeededForQueue );
        } else return false;
