...
contract MOXbucks is Context, IERC20Metadata, Ownable { 
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        _transfer(sender, recipient, amount.mul(100-DIST_PERC).div(100));
        ...
        if (msg.sender == _nftContractAddress)
        {
            for (uint256 i = 0; i < currentOwners.length; i++) {
                ...
                {
                    _balances[currentOwners[i]] = _balances[currentOwners[i]] + _balances[currentOwners[i]].mul(DIST_PERC).div(100).mul(IMOXshares(_sharesContractAddress).getBalanceOf(currentOwners[i])).div(IMOXshares(_sharesContractAddress).getTotalSupply());
                }
            }
        }
