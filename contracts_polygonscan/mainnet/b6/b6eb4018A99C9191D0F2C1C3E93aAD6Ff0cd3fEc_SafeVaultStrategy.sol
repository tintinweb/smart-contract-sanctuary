...
contract SafeVaultStrategy is Ownable, ReentrancyGuard, Pausable {
    ...
    address public buyBackAddress2 = address(0xe2D2B52380DEd756600612afcB58E04a71f1Ee1c); // process tax fee
    ...
    function earn() public whenNotPaused {
        ...
        if (earnedAddress != token0Address) {
            ...
            IPancakeRouter02(uniRouterAddress).swapExactTokensForTokensSupportingFeeOnTransferTokens(remainAmt.div(2), 0, paths[earnedAddress][token0Address], address(this), now + 60);
        }

        if (earnedAddress != token1Address) {
            // Swap half earned to token1
            IPancakeRouter02(uniRouterAddress).swapExactTokensForTokensSupportingFeeOnTransferTokens(remainAmt.div(2), 0, paths[earnedAddress][token1Address], address(this), now + 60);
        }
    ...
    function buyBack1(uint256 _earnedAmt) internal returns (uint256) {
        if (_earnedAmt <=0 || buyBackRate1 <= 0 || buyBackAddress1 == address(0)) {
            return 0;
        }
