...
abstract contract StratX2SUSHI is OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    address[] public earnedToDEXTokenPath;
    ...
    function _farm() internal virtual {
        ...
        if (isCAKEStaking) {
            ...
            if(wantAmt > 0)
                IPancakeswapFarm(farmContractAddress).deposit(pid, wantAmt, address(this));
            IPancakeswapFarm(farmContractAddress).harvest(pid, address(this));
        }
    ...
    function _unfarm(uint256 _wantAmt) internal virtual {
        if (isCAKEStaking) {
            ...
            if(_wantAmt > 0)
                IPancakeswapFarm(farmContractAddress).withdraw(pid, _wantAmt, address(this));
            IPancakeswapFarm(farmContractAddress).harvest(pid, address(this));
        }
    }
    ...
    function _distribution(uint256 minSwapAmt) internal virtual {
        ...
        if (earnedAddress != dexToken) {
            // Swap half earned to token0
            _safeSwap(
                uniRouterAddress,
                earnedAmt,
                slippageFactor,
                earnedToDEXTokenPath,
                address(this),
                block.timestamp.add(600)
            );
        }

        uint256 dexTokenAmt = IERC20(dexToken).balanceOf(address(this));
        if (dexTokenAmt > 0) {
            IERC20(dexToken).safeTransfer(rewardDistribution, dexTokenAmt);  
            IRewardDistribution(rewardDistribution).requestRewardDistribution();
        }
    ...
    function setSwapToken(address _token, address[] memory earnedToTokenPath) public virtual onlyAllowGov{
        dexToken = _token;
        earnedToDEXTokenPath = earnedToTokenPath;
    }
...
contract StratX2_HERO_SUSHI is StratX2SUSHI {
    function initialize(
        ...
        bool _isAutoComp,
        address[] memory _earnedToDEXTokenPath,
        ...
        earnedToDEXTokenPath = _earnedToDEXTokenPath;
