...
contract MadnessKingDomNFT is Ownable, ReentrancyGuard {
    ...
    function deposit(uint256 _amount, uint256 _nftID) external nonReentrant {
        ...
        user.rewardDebt = _calculatedUserAmount(msg.sender).mul(accTokenPerShare).div(PRECISION_FACTOR);
    ...
    function withdraw() external nonReentrant {
        ...
        if (_amount > 0) {
            ...
            if (user.hasNFT) {
                uint256 powerStaking = user.powerAmount - _amount;
    ...
    function updatePool() public {
        ...
        uint256 multiplier = _getMultiplier(lastRewardBlock, block.number);
        uint256 tokenReward = multiplier.mul(rewardPerBlock);
        accTokenPerShare = accTokenPerShare.add(tokenReward.mul(PRECISION_FACTOR).div(stakedTokenSupply.add(poolInfo.totalPower)));
        lastRewardBlock = block.number;
    }
    ...
    function _userHarvest(address _userAddress) internal {
        ...
        if (user.amount > 0) {
            if (user.nextHarvestUntil <= block.timestamp) {
                ...
                if (pending > 0) {
                    user.rewardDebt = newUserAmount.mul(poolInfo.accRewardPerShare).div(1e18);
                    rewardToken.safeTransfer(address(msg.sender), pending);
                }
                _updateHarvestLookup(_userAddress);
                user.rewardDebt = newUserAmount.mul(accTokenPerShare).div(PRECISION_FACTOR);
            }
        }
    }
