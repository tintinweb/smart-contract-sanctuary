...
contract SushiToken is ERC20("Sushi Farm Token", "SUSHI") {
    ...
    */
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying SUSHI (not scaled);
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 accSushiPerShare;   // Accumulated SUSHI per share, times 1e18. See below.
    ...
    uint256 public sushiPerBlock = 0.005 ether;
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 sushiReward = multiplier.mul(sushiPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        sushi.mint(devAddress, sushiReward.div(10));
    ...
    function safeSushiTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > sushiBal) {
            transferSuccess = sushi.transfer(_to, sushiBal);
        } else {
            transferSuccess = sushi.transfer(_to, _amount);
        }
        require(transferSuccess, "safeSushiTransfer: Transfer failed");
    }
