...
contract LendingLogicKashi is ILendingLogic {
    ...
    function unlend(address _wrapped, uint256 _amount,address _tokenHolder) external view override returns(address[] memory targets, bytes[] memory data) {
        ...
        targets[0] = _wrapped;
        data[0] =  abi.encodeWithSelector(IKashiPair.removeAsset.selector, _tokenHolder, _amount);
        
        // Remove from BentoBox
        targets[1] = address(bentoBox);
        data[1] =  abi.encodeWithSelector(IBentoBoxV1.withdraw.selector, underlying, _tokenHolder, _tokenHolder, 0, underlyingAmount);
        
        // return targets and call data
        return(targets, data);
    }

    function exchangeRate(address _kaToken) external override returns(uint256) {
