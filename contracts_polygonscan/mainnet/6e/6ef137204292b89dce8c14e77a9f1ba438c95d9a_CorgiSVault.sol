...
// File: contracts/farms/corgi/IMasterChef.sol
...
// File: contracts/farms/corgi/CorgiSVault.sol


pragma solidity 0.6.12;

contract CorgiSVault is Ownable, Pausable {
    ...
    struct UserInfo {
        ...
        uint256 corgiSAtLastUserAction; // keeps track of CorgiS deposited at the last user action
        uint256 lastUserActionTime; // keeps track of the last user action time
    }

    IBEP20 public immutable token; // CorgiS token
    ...
    /**
     * @notice Constructor
     * @param _token: CorgiS token contract
    ...
    /**
     * @notice Deposits funds into the CorgiS Vault
     * @dev Only possible when contract not paused.
     * @param _amount: number of tokens to deposit (in CORGIS)
     */
    function deposit(uint256 _amount) external whenNotPaused notContract {
        ...
        user.corgiSAtLastUserAction = user.shares.mul(balanceOf()).div(totalShares);
    ...
    /**
     * @notice Reinvests CORGIS tokens into MasterChef
     * @dev Only possible when contract not paused.
     */
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the CorgiS Vault
     */
     ...
     * @return Expected reward to collect in CORGIS
     */
    function calculateHarvestCorgiSRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingCorgiS(poolId, address(this));
     ...
     * @return Returns total pending CORGIS rewards
     */
    function calculateTotalPendingCorgiSRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingCorgiS(poolId, address(this));
        amount = amount.add(available());
    ...
    /**
     * @notice Withdraws from funds from the CorgiS Vault
