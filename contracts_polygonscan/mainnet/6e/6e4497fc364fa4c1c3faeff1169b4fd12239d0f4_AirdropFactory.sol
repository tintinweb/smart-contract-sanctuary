...
contract AirdropFactory is Operatorable{
    ...
    event BlackAddress(uint8 index, address addr);
    event NewAirdrop(uint8 index, address pledgeAddress, uint256 pledgeAmount, address claimAddress, uint256 claimAmount, uint256 pledgecount, uint256 block, uint256 timestamp);
    ...
    // 添加项目黑名单
    function updateBlack(uint8 _index, address addr) public onlyOperator AirdropCheck(_index) {
        AirdropV2 storage airdrop = airdrops[_index];
    ...
    function updateBlacks(uint8 _index, address[] memory addrs) public onlyOperator AirdropCheck(_index) {
        AirdropV2 storage airdrop = airdrops[_index];
