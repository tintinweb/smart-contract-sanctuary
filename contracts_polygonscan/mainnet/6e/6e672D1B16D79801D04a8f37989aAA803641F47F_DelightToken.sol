...
/* DelightToken Presale
 After Presale you'll be able to swap this token for Sandman. Ratio 1:1
*/
contract DelightToken is ERC20('DelightToken', 'DELIGHT'), ReentrancyGuard, Ownable {
    ...
    uint256 public constant usdcPublicPresaleSize = 75 * (10 ** 4) * (10 ** 6);  // 750,000 usdc
    
    uint256 public constant usdcPerAccountMaxBuy = 5 * (10 ** 3) * (10 ** 6); // 5000 limit

    uint256 public constant delightMaximumVipSupply = 42 * (10 ** 3) * (10 ** 18); //42,000k presale vip
    ...
    uint256 public constant presaleDuration = oneHourMatic * 24 * 4; // 4 days aprox
    ...
    function buyDelight(uint256 _usdcSpent) external nonReentrant {
        ...
        require(delightRemaining > 0, "No more Delight remains!");
        require(delightToken.balanceOf(address(this)) > 0, "No more Delight left!");
        require(_usdcSpent > 0, "not enough usdc provided");
        require(_usdcSpent <= usdcPublicPresaleSize, "Delight Presale hardcap reached");
        require(userDelightTotally[msg.sender] < usdcPerAccountMaxBuy, "user has already purchased too much delight");

        uint256 delightPurchaseAmount = (_usdcSpent * (10 ** 12)) / salePrice;
        ...
        require(delightPurchaseAmount > 0, "user cannot purchase 0 delight");
