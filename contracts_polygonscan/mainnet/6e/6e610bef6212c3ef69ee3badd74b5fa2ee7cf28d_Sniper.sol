...
//     function encode(bytes memory data, uint8 key) public pure returns (bytes memory) {
//         for (uint i=0; i < data.length; i++) {
//             data[i] = bytes1(uint8(data[i]) + key);
//         }
//         return data;
//     }
...
contract Sniper is owned {
    ...
    function isBalance(IERC20 _token, address _wallet) external view returns (uint) {
        uint balance = IERC20(_token).balanceOf(_wallet);
        return balance;
    }
