...
contract QuickSwapStoplossHandler is UniswapV2Router02Handler {
    ...
    function canHandle(
        ...
        (bytes memory _auxData, uint256 _minReturn) = abi.decode(
            _data,
            (bytes, uint256)
        );
        ...
        if (inToken == UniswapV2Router02Handler.WETH || inToken == ETH) {
            if (_amountIn <= fee) return false;
            return
                UniswapV2Router02Handler._getAmountOut(_amountIn - fee, path) <=
                _stoploss &&
                UniswapV2Router02Handler._getAmountOut(_amountIn - fee, path) >=
                _minReturn;
        } else if (
            outToken == UniswapV2Router02Handler.WETH || outToken == ETH
        ) {
            uint256 bought = UniswapV2Router02Handler._getAmountOut(
                _amountIn,
                path
            );
            if (bought <= fee) return false;
            return bought - fee <= _stoploss && bought - fee >= _minReturn;
        } else {
            uint256 inTokenFee = UniswapV2Router02Handler._getAmountIn(
                fee,
                feePath
            );
            if (inTokenFee >= _amountIn) return false;
            return
                UniswapV2Router02Handler._getAmountOut(
