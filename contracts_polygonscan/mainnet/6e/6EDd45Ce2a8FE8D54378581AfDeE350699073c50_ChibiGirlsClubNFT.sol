...
// Contract built based on the tutorials by HashLips
...
contract ChibiGirlsClubNFT is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  uint256 public giftPercentage = 50;
  ...
  function mint() public payable {
    ...
    if(supply > 0) {
        ...
        giftValue = msg.value * giftPercentage / 100;
  ...
  function withdraw(uint256 _giftPercent) public payable onlyOwner {
    uint256 supply = totalSupply();
    require(supply == maxSupply || block.timestamp >= headStart, "Can not withdraw yet.");
    (bool giftSuccess, ) = payable(ultimateGiftAddress).call{value: address(this).balance * _giftPercent / 100}("");
    require(giftSuccess);
