...
abstract contract BaseStrategy is Ownable {
    ...
    // Dex
    address public constant currentRouter = 0x5C6EC38fb0e2609672BDf628B1fD605A523E5923; //JetSwap router
...
abstract contract StrategyFarmBase is BaseStrategyMasterChef {
    ...
    address public constant pwings = 0x845E76A8691423fbc4ECb8Dd77556Cb61c09eE25;
    ...
    // Other constants
    uint16 public constant underlyingPoolId = 2;
    ...
    // Uniswap swap paths
    address[] public pwings_matic_path;
    ...
    {
        pwings_matic_path = new address[](2);
        pwings_matic_path[0] = pwings;
        pwings_matic_path[1] = matic;
    }
    ...
    function harvest() public override {
        ...
        // Collects TITAN tokens
        _getReward();

        //Swap 1/2 TITAN for MATIC
        uint256 _pwingsBalance = IERC20(pwings).balanceOf(address(this));
        if (_pwingsBalance > harvestCutoff){
            _swapUniswapWithPath(pwings_matic_path, _pwingsBalance.div(2));

            // Add liquidity for TITAN/MATIC
            uint256 _pwings = IERC20(pwings).balanceOf(address(this));
            uint256 _matic = IERC20(matic).balanceOf(address(this));
            if (_pwings > 0 && _matic > 0) {
                IERC20(pwings).safeApprove(currentRouter, 0);
                IERC20(pwings).safeApprove(currentRouter, _pwings);
                ...
                // Donates DUST
                IERC20(pwings).safeTransfer(
                    strategist,
                    IERC20(pwings).balanceOf(address(this))
                );
...
contract StrategyBase is StrategyFarmBase {
    // Token addresses
    address public JET_MASTER_CHEF = 0x4e22399070aD5aD7f7BEb7d3A7b543e8EcBf1d85;
    address public pwings_matic_LP = 0xA0A6e9A5185d5737CF6F7920CB417EA2F07F03B3;
