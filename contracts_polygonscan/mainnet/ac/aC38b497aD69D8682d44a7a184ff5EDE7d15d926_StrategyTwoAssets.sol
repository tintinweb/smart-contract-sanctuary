...
abstract contract StrategyFarmTwoAssets is BaseStrategyMasterChef {
    ...
    address public constant aavegotchiStaking = 0xA02d547512Bb90002807499F05495Fe9C4C3943f;
    address public constant GHST_WETH = 0xcCB9d2100037f1253e6C1682AdF7dC9944498AFF;
    address public constant STKGHST_WETH = 0x388E2a3d389F27504212030c2D42Abf0a8188cd1;
    address public ticketCollector = 0x0000000000000000000000000000000000000000;
        ...
        )
    {
        ...
        eth_ghst_path = new address[](2);
        eth_ghst_path[0] = eth;
        eth_ghst_path[1] = ghst;
    }

    // **** View Frens Balance ****
    function view_frens() public view returns (uint256) {
        return(IAaveGotchi(aavegotchiStaking).frens(address(this)) );
    }

    // **** State Mutations ****
    
    function buy_tickets(uint256[] calldata _ids, uint256[] calldata _values) external {
        require(msg.sender == owner() || msg.sender == strategist || msg.sender == ticketCollector, "not authorized");
        IAaveGotchi(aavegotchiStaking).claimTickets(_ids, _values);
    }

    function salvageTickets(address token, uint256 id, uint256 amount, bytes calldata data) external  {
        require(msg.sender == owner() || msg.sender == strategist || msg.sender == ticketCollector, "not authorized");
        require(token != want && token != harvestedToken, "cannot salvage");
        IERC1155(token).safeTransferFrom(address(this), msg.sender, id, amount, data);
    }
    ...
    function set_TicketCollector(address newCollector) external onlyOwner() {
        ticketCollector = newCollector;
    }
    ...
    function harvest() public override {
        ...
        if (_dinoBalance > harvestCutoff){
            ...
            _swapUniswapWithPath(eth_ghst_path, _ethBalancePost.div(2));

            // Add liquidity
            uint256 _eth = IERC20(eth).balanceOf(address(this));
            uint256 _ghst = IERC20(ghst).balanceOf(address(this));
            if (_eth > 0 && _ghst > 0) {
                IERC20(eth).safeApprove(currentRouter, 0);
                IERC20(eth).safeApprove(currentRouter, _eth);
                IERC20(ghst).safeApprove(currentRouter, 0);
                IERC20(ghst).safeApprove(currentRouter, _ghst);
        ...
        if(ghstWethBal > 0) {
            // Approve
            IERC20(GHST_WETH).safeApprove(aavegotchiStaking, 0);
            IERC20(GHST_WETH).safeApprove(aavegotchiStaking, ghstWethBal);

            //Convert GHST_WETH to STKGHST_WETH
            IAaveGotchi(aavegotchiStaking).stakeGhstWethPoolTokens(ghstWethBal);
        }
...
contract StrategyTwoAssets is StrategyFarmTwoAssets {
    ...
    address public skGHST_WETH_LP = 0x388E2a3d389F27504212030c2D42Abf0a8188cd1;
