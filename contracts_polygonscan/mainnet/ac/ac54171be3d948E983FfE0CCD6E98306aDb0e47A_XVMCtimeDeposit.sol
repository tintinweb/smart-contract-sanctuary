...
contract XVMCtimeDeposit is ReentrancyGuard {
     ...
     */
    function setTrustedSenders(address _sender1) external adminOnly {
     ...
     */
    function setTrustedPools(address _pool6, address _pool5, address _pool4, address _pool3) external adminOnly {
        trustedPool6 = _pool6;
        trustedPool5 = _pool5;
        trustedPool4 = _pool4;
        trustedPool3 = _pool3;
    ...
    function stakeRollover(address _poolInto, uint256 _stakeID) external nonReentrant {
        ...
        if(_poolInto == address(this)) {
			...
			if(_poolInto == trustedPool6 || _poolInto == trustedPool5 || _poolInto == trustedPool4 ||
					_poolInto == trustedPool3) { //if pool is lower hopStake reverts, we need to withdraw and giftDeposit instead
