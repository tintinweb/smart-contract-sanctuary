...
contract LockUp {
    ...
    {
        require(_token != address(0),"token is failed");
        require(_target != address(0),"target is wrong");
    ...
    function unlock(uint256 id) public {
        LockUpRecord storage row = lockup[id];
    ...
    function massUnlock(uint256[]calldata ids, address _target, address _token) public onlyOwner {
        uint256 sum = 0;
        for(uint256 id = 0; id < ids.length; id++) {
            LockUpRecord storage row = lockup[ids[id]];
    ...
    function unlockAvailable(uint256[]calldata ids, address _target, address _token) public view returns (uint256) {
        uint256 sum = 0;
        for(uint256 id = 0; id < ids.length; id++) {
    ...
    function lockUpBalance(uint256[]calldata ids, address _target, address _token) public view returns (uint256) {
        uint256 sum = 0;
        for(uint256 id = 0; id < ids.length; id++) {
            LockUpRecord memory row = lockup[ids[id]];
