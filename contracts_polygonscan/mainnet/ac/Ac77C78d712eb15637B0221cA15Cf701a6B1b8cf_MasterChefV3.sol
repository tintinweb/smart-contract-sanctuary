...
contract MasterChefV3 is Ownable, ReentrancyGuard {
    ...
    // The DESIRE TOKEN!
    DesireTokenV3 public immutable destruction;
    ...
    function pendingDesire(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && pool.lpSupply != 0 && totalAllocPoint > 0) {
            ...
            accDesirePerShare = accDesirePerShare + ((destructionReward * 1e18) / pool.lpSupply);
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accDesirePerShare = pool.accDesirePerShare + ((destructionReward * 1e18) / pool.lpSupply);
        pool.lastRewardBlock = block.number;
    }
    ...
    // Safe destruction transfer function, just in case if rounding error causes pool to not have enough DESIREs.
