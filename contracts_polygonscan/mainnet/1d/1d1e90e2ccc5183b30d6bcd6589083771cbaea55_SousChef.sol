...
contract SousChef {
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        uint256 stakedSupply = gold.balanceOf(address(this));
    ...
    function updatePool() public {
        ...
        uint256 goldSupply = gold.balanceOf(address(this));
        if (goldSupply == 0) {
        ...
        poolInfo.accRewardPerShare = poolInfo.accRewardPerShare.add(tokenReward.mul(1e12).div(goldSupply));
        poolInfo.lastRewardBlock = block.number;
    }

    // Deposit Gold tokens to SousChef for Reward allocation.
    ...
    // Withdraw Gold tokens from SousChef.
    ...
    function emergencyWithdraw() public {
        UserInfo storage user = userInfo[msg.sender];
        gold.safeTransfer(address(msg.sender), user.amount);
