...
contract HemetaTreasury is Ownable, IHemetaTreasury {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SHETA }
    ...
    address public sHETA;
    uint public sHETAQueue; // Delays change to sHETA address
    ...
    constructor (
        ...
        isLiquidityToken[ _HETADAI ] = true;
        liquidityTokens.push( _HETADAI );
        bondCalculator[_HETADAI] = _bondCalculator;

        blocksNeededForQueue = _blocksNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for HETA
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external override returns ( uint send_ ) {
        ...
        // mint HETA needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( HETA ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn HETA for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external override {
        ...
        uint value = valueOfToken( _token, _amount );
        IHETAERC20( HETA ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sHETA ).balanceOf( msg.sender ); // Can only borrow against sHETA held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with HETA
        @param _amount uint
     */
    function repayDebtWithHETA( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IHETAERC20( HETA ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, HETA, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external override {
        ...
        IERC20Mintable( HETA ).mint( _recipient, _amount );
    ...
    function excessReserves() public override view returns ( uint ) {
        return totalReserves.sub( IERC20( HETA ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns HETA valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOfToken( address _token, uint _amount ) public view override returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match HETA decimals
            value_ = _amount.mul( 10 ** IERC20( HETA ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyOwner() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SHETA ) { // 9
            sHETAQueue = block.number.add( blocksNeededForQueue );
        } else return false;
