...
// File: RebaseDAO/HeroNFT.sol


pragma solidity 0.8.10;

    
interface IRightNFT is IERC721 {
    function id2RightInfoMap(uint256 rightNFTId) external returns(RightInfo memory);
}    
...
contract HeroNFT is Ownable, ERC721 {
    ...
    uint256 public tokenId = 0;
    IRightNFT public rightNFT;
    string public baseURI;
    
    mapping(uint256 => uint256) public tokenId2HearIdMap;
    ...
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
        ...
        return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, tokenId2HearIdMap[_tokenId].toString(), ".png")) : "";
    }
    
    function mint(uint256 _rightNFTId) public returns (uint256) {
        rightNFT.transferFrom(msg.sender, deadAddr, _rightNFTId);
        RightInfo memory rightInfo = rightNFT.id2RightInfoMap(_rightNFTId);
        ...
        tokenId2HearIdMap[tokenId] = heroId;
