...
import "./ERC721Orderinbox.sol";

/**
 * @dev This contract is for creating proxy to access ERC721Orderinbox token.
 *
 * The beacon should be initialized before call ERC721OrderinboxFactory constructor.
 *
 */
contract ERC721OrderinboxFactory is Ownable {
    address public beacon;
    address[] defaultOperators;

    event Create721OrderinboxProxy(address proxy);
    event Create721OrderinboxPrivateProxy(address proxy);

    constructor(address _beacon, address[] memory _defaultOperators) {
        beacon = _beacon;
    ...
    function createToken(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI, uint salt) external {
        ...
        ERC721Orderinbox token = ERC721Orderinbox(address(beaconProxy));
        token.transferOwnership(_msgSender());
        emit Create721OrderinboxProxy(beaconProxy);
    }
    ...
    function createPrivateToken(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI, address[] memory operators, uint salt) external {
        ...
        ERC721Orderinbox token = ERC721Orderinbox(address(beaconProxy));
        token.transferOwnership(_msgSender());
        emit Create721OrderinboxPrivateProxy(beaconProxy);
    }
    ...
    function getData(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI) view internal returns(bytes memory){
        return abi.encodeWithSelector(ERC721Orderinbox(address(0)).__ERC721Orderinbox_init.selector, _name, _symbol, baseURI, contractURI, defaultOperators);
    }
    ...
    function getData(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI, address[] memory operators) view internal returns(bytes memory){
        return abi.encodeWithSelector(ERC721Orderinbox(address(0)).__ERC721OrderinboxPrivate_init.selector, _name, _symbol, baseURI, contractURI, operators, defaultOperators);
    }
}
