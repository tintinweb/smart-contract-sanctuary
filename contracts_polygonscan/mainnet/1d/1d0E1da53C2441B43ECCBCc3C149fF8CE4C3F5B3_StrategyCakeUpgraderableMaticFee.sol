...
// File: contracts/matic/StrategyCakeUpgraderableMaticFee.sol
...
contract StrategyCakeUpgraderableMaticFee is ReentrancyGuardUpgradeSafe, OwnableUpgradeable , Pausable {
    ...
    address public constant maiAddress = 0xa3Fa99A148fA48D14Ed51d610c367C61876997F1;
    ...
    function initialize(
        ...
        if (isGrandComp) {
            ...
            earnedToGrandPath = [earnedAddress, maiAddress, GrandAddress];
            if (maiAddress == earnedAddress) {
                earnedToGrandPath = [maiAddress, GrandAddress];
            }

            earnedToToken0Path = [earnedAddress, maiAddress, token0Address];
            if (maiAddress == token0Address) {
                earnedToToken0Path = [earnedAddress, maiAddress];
            }

            earnedToToken1Path = [earnedAddress, maiAddress, token1Address];
            if (maiAddress == token1Address) {
                earnedToToken1Path = [earnedAddress, maiAddress];
            }

            token0ToEarnedPath = [token0Address, maiAddress, earnedAddress];
            if (maiAddress == token0Address) {
                token0ToEarnedPath = [maiAddress, earnedAddress];
            }

            token1ToEarnedPath = [token1Address, maiAddress, earnedAddress];
            if (maiAddress == token1Address) {
                token1ToEarnedPath = [maiAddress, earnedAddress];
            }
        }
    ...
    function _farm() internal {
        ...
        (uint256 amountOld,)= IPancakeswapFarm(farmContractAddress).userInfo(pid,address(this));
        ...
        (uint256 amountNew,)= IPancakeswapFarm(farmContractAddress).userInfo(pid,address(this));

        wantLockedTotal = wantLockedTotal.add(amountNew.sub(amountOld));
