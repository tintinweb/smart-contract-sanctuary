...
contract KrownMaster is ReentrancyGuard, Pausable, Ownable{
     ...
     */
    function getPending(uint256 _pid, address account) public view returns(uint256){
        UserInfo memory user = userInfo[_pid][account];
        PoolInfo memory pool = poolInfo[_pid];
        if(user.amount == 0){
          return 0;  
        }
        return ((user.amount.mul(pool.rewardsPerShare)).div(1e18)).sub(user.rewardMinus);
    }
    ...
    function createPool(address _lpToken, string memory _symbol, uint16 _fee, address _vault, uint256 _krwPerBlock, bool isVaultContract) external onlyOwner{
        ...
        poolInfo.push(PoolInfo(IERC20(_lpToken), _symbol, _fee, _vault, isVaultContract, 0, _krwPerBlock ,block.number, 0));
        if(isVaultContract){
     ...
     */
    function changePoolKrwReward(uint256 _pid, uint256 _krwPerBlock) external onlyOwner{
        PoolInfo storage pool = poolInfo[_pid];
        pool.krwPerBlock = _krwPerBlock;
    }
     ...
     */
    function changeLastBlock(uint256 _lastBlock) external onlyOwner{
        lastBlock = _lastBlock;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) external whenNotPaused nonReentrant{
        ...
        user.rewardMinus = user.rewardMinus.add((pool.rewardsPerShare.mul(newAmount)).div(1e18));
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant{
        ...
        if(_pid == 1){
            require(krw.balanceOf(address(this)) >= pool.totalSupply, "Not enough rewards in contract.");
        }
        require(_amount > 0, "Amount: amount is 0");
        updatePool(_pid);
        krw.safeTransfer(msg.sender, getPending(_pid, msg.sender));
        user.amount = user.amount.sub(_amount);
        user.rewardMinus = user.amount.mul(pool.rewardsPerShare).div(1e18);
    ...
    function updatePool(uint256 _pid) public{
        ...
        if(!pausedUpdatePools && !pausedPool[_pid]){
            ...
            pool.rewardsPerShare = pool.rewardsPerShare.add((rewardToDeliver.mul(1e18)).div(pool.totalSupply));
        }
        changeLastRewardBlock(_pid);
    }
    ...
    function claim(uint256 _pid) external nonReentrant{
        ...
        UserInfo storage user = userInfo[_pid][msg.sender];
        PoolInfo memory pool = poolInfo[_pid];
        PoolInfo memory poolCheck = poolInfo[1];
        require(krw.balanceOf(address(this)) >= poolCheck.totalSupply, "Not enough rewards in contract.");
        if(krw.transfer(msg.sender, getPending(_pid,msg.sender))){
          user.rewardMinus = user.amount.mul(pool.rewardsPerShare).div(1e18);
          emit Claim(msg.sender, _pid, getPending(_pid,msg.sender)); 
        }
    }
}