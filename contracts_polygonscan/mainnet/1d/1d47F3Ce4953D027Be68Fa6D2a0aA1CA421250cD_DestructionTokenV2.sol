...
abstract contract AntiBotBlacklist is ERC20, Ownable {
    ...
    */
    ...
    modifier antiBot(address _sender,address _recipient,uint256 _amount) { 
        //check blacklist
        require(!blacklistCheck(_sender), "ERR3");
        require(!blacklistCheck(_recipient), "ERR4");
        ...
        if (antiBotWorking){
            ...
            if (_isNotExcludedFromAntiBot(_sender) && _isNotExcludedFromAntiBot(_recipient)){
                require(_amount <= maxUserTransferAmount(), "ERR1");

                // Autodetect is sender is a BOT
                if (balanceOf(_sender).add(_amount) > maxUserHoldAmount())
                {
