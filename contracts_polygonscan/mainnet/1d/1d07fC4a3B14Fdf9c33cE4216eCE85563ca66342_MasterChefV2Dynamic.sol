...
contract MasterChefV2Dynamic is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    uint256 public ONE = 1 ether; // control price
    uint256 public MIN = 0.1 ether; // control price
    uint256 public BLOCKS = 43200; // daily number of blocks Polygon network
    uint256 public FACTOR;
    bool public lastIsValid = false;
    ...
    function updateEmissionRate() internal {
        (TWAP, lastIsValid) = oracle.capture();
        massUpdatePools();
        if (TWAP >= ONE) {
    ...
    function updateBonusMultiplier(uint256 newVal) public onlyOwner {
        require( newVal <= 10, "newVal too high");
        BONUS_MULTIPLIER = newVal;
    }
    function updateMinEmission(uint256 newVal) public onlyOwner {
        require( newVal <= 10, "newVal too high");
        MIN = newVal;
    }
    function updateBlocks(uint256 newVal) public onlyOwner {
        BLOCKS = newVal;
    }
    function updateEmissionInterval(uint256 newVal) public onlyOwner {
        emissionUpdateInterval = newVal;
    }
    function tokenTransferOwnership() external onlyOwner {
        require(_isContract(msg.sender), "must be another contract");
        // transfer token back to timelock, for migration.
