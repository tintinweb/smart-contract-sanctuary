...
contract OpenMintMarketplace is Ownable {
  ...
  function _computePublisherFee(uint256 price) internal pure returns(uint256){
    uint256 publisherFee = price * 5 / 100;
    return publisherFee;
  }
  ...
  Ownable
{
  ...
  constructor (address _PaymentGatewayAddress, address payable _publisherWallet) ERC721("MintFace", "MF"){
    _setPaymentGatewayContract(_PaymentGatewayAddress);
    publisherWallet = _publisherWallet;
  }
  ...
  function createArtwork(string memory uri, uint8 royalty) public payable returns(uint256){
    require(royalty > 0, "Royalty cannot be zero or smaller than zero");
    _PaymentGateway.sendPayment{value: msg.value}(publisherWallet);
    _tokenIds.increment();
