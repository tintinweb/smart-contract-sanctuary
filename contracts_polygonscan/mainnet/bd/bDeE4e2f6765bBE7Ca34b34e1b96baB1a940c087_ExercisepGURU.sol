...
interface ICirculatingGURU {
  function GURUCirculatingSupply() external view returns (uint256);
}
...
interface IOldClaimContract {
  function terms(address _claimer) external view returns (Term memory);
}

contract ExercisepGURU is Ownable {
  using SafeMath for uint256;
  using SafeERC20 for IERC20;

  address public owner;
  address public newOwner;
  bool public hasMigrated = false;

  address public immutable pGURU;
  address public immutable GURU;
  address public immutable DAI;
  address public immutable treasury;
  address public immutable circulatingGURUContract;
  ...
  constructor(
    ...
    address _dai,
    address _treasury,
    address _circulatingGURUContract
  ) {
    owner = msg.sender;
    require(_pGURU != address(0));
    pGURU = _pGURU;
    ...
    treasury = _treasury;
    require(_circulatingGURUContract != address(0));
    circulatingGURUContract = _circulatingGURUContract;
  }
  ...
  function migrate(address[] calldata _addresses) external returns (bool) {
    ...
    for (uint256 i = 0; i < _addresses.length; i++) {
      Term memory termsToMigrate = IOldClaimContract(
        0x1006C50dd40b42b1cE8505aF5ADC4289FaF49C26
      ).terms(_addresses[i]);
      terms[_addresses[i]] = termsToMigrate;
    }
  ...
  // Allows wallet to redeem pGURU for GURU
  function exercise(uint256 _amount) external returns (bool) {
    ...
    IPGURU(pGURU).burnFrom(msg.sender, _amount);

    IERC20(DAI).approve(treasury, _amount);
    uint256 GURUToSend = ITreasury(treasury).deposit(_amount, DAI, 0);

    terms[msg.sender].claimed = info.claimed.add(_amount);

    IERC20(GURU).safeTransfer(msg.sender, GURUToSend);
  ...
  function redeemable(Term memory _info) internal view returns (uint256) {
    return
      (
        ICirculatingGURU(circulatingGURUContract)
          .GURUCirculatingSupply()
          .mul(_info.percent)
          .mul(1000)
      ).sub(_info.claimed);
  }
