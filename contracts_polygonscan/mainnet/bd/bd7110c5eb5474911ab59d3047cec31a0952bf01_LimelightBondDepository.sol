...
contract LimelightBondDepository is IPrincipleDepository, Ownable {
    ...
    struct DepositInfo {
        ...
        uint payoutRemaining; // LMLT remaining to be paid
    ...
    address public principleToken; // LMLT-DAI LP
    address public LMLT;
    ...
    function initialize ( address principleToken_, address LMLT_ ) external onlyOwner() returns ( bool ) {
    ...
    function _depositBondPrinciple( uint amountToDeposit_ ) internal returns ( bool ){
        ...
        ITreasury( treasury ).depositPrinciple( amountToDeposit_ ); // Returns LMLT

        IERC20( LMLT ).safeTransfer( stakingContract, profit_.sub( DAOProfit_ ) );
        IERC20( LMLT ).safeTransfer( DAOWallet, DAOProfit_ );
    ...
    function redeemBond() external override returns ( bool ) {
        ...
        if ( percentVested_ >= 10000 ) {
            ...
            IERC20( LMLT ).safeTransfer( msg.sender, payoutRemaining_ );
        ...
        IERC20( LMLT ).safeTransfer( msg.sender, payout_ );
