...
contract Crowdsale is Context, ReentrancyGuard {
    ...
    function buyTokens(address beneficiary, uint256 amount) internal nonReentrant {
        //uint256 weiAmount = amount * 1000000000000; //Match usdc to 18dec
        _preValidatePurchase(beneficiary, amount * 1000000000000);

        // calculate token amount to be created
        uint256 tokens = _getTokenAmount(amount);

        // update state
        _weiRaised = _weiRaised.add(amount * 1000000000000);

        _processPurchase(beneficiary, tokens);
        emit TokensPurchased(_msgSender(), beneficiary, amount, tokens);

        _updatePurchasingState(beneficiary, amount * 1000000000000);
    ...
    Ownable
{
    ...
    uint256 __totalSaleCap = 625000000000000000000;
    uint256 __individualPurchaseCap = 125000000000000000000;

    uint256 __openingTime  = now;
    uint256 __closingTime  = __openingTime + 36 hours;

    address payable walletAddress = address(0xcFeD31FE9D3b825caEcf870BFAa8e8a76fa1483d);

    uint256 __rate = 125;

    uint256 __usdcTotal = (__totalSaleCap / __rate) * 10;

    uint256 __cliffDuration = 1 minutes;
    uint256 __vestDuration = 15 minutes;
