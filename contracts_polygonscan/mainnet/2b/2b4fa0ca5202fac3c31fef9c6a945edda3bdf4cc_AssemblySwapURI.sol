...
// File contracts/NFTSwap/AssemblyNFTInterface.sol
...
// File contracts/NFTSwap/AssemblyNFT.sol
...
// File contracts/NFTSwap/AssemblySwap.sol



pragma solidity ^0.8.0;

contract AssemblySwap is AssemblyNFT {

  event PairCreated(address indexed A, address indexed B, uint256 tokenA, uint256 tokenB);
  event PairSwaped(address indexed A, address indexed B, uint256 tokenA, uint256 tokenB);
  ...
  mapping(address=>mapping(address=>pair)) private tradingPair;

  function getPair(address _A, address _B) public view returns (uint256, uint256, uint256) {
    (address sortedA, address sortedB) = sortAB(_A, _B);
    pair memory p = tradingPair[sortedA][sortedB];
    return (p.tokenA, p.tokenB, p.deadline);
  }

  function sortAB(address _A, address _B) private pure returns (address, address){
    (address sortedA, address sortedB) = _A < _B ? (_A, _B) : (_B, _A);
    return (sortedA, sortedB);
  }

  function createPairAsA(address _B, uint256 _tokenA, uint256 _tokenB, uint256 waitMinutes) public {
    (address sortedA, address sortedB) = sortAB(_msgSender(), _B);
    pair storage p = tradingPair[sortedA][sortedB];
    require(block.timestamp > p.deadline, "pair not settled");
    require(_msgSender() != _B, "can't swap from self");
    require(ownerOf(_tokenA) == _msgSender() && ownerOf(_tokenB) == _B, "not token owner");
    p.tokenA = _tokenA;
    p.tokenB = _tokenB;
    p.deadline = block.timestamp + (waitMinutes * 1 minutes < 1 days ? waitMinutes * 1 minutes : 1 days) ;
    emit PairCreated(_msgSender(), _B, _tokenA, _tokenB);
  }

  function confirmSwapAsB(address _A) public {
    (address sortedA, address sortedB) = sortAB(_A, _msgSender());
    pair storage p = tradingPair[sortedA][sortedB];
    require(block.timestamp < p.deadline, "swap expired");
    require(ownerOf(p.tokenA) == _A && ownerOf(p.tokenB) == _msgSender(), "not token owner");
    _safeTransfer(_A, _msgSender(), p.tokenA, "");
    _safeTransfer(_msgSender(), _A, p.tokenB, "");
    require(ownerOf(p.tokenA) == _msgSender() && ownerOf(p.tokenB) == _A, "swap failed");
    p.deadline = 0;
    emit PairSwaped(_A, _msgSender(), p.tokenA, p.tokenB);
  }
...
// File contracts/NFTSwap/AssemblySwapURI.sol
...
contract AssemblySwapURI is AssemblySwap, Ownable {
    using Strings for uint256;
    
    constructor(string memory name_, string memory symbol_)
        AssemblySwap(name_, symbol_)
    {}
