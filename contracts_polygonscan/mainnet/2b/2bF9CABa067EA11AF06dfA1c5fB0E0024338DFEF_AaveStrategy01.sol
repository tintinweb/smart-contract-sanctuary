...
import './../StrategySwapper/ISwapper.sol';
...
contract AaveStrategy01 is Strategy, Keeper {

    using StringUtilsLib for *;
    using SafeMath for uint256;

    // Swapper
    // 0x1C50A54BBee9Bc165CBcff7305111A7fFA681107
    ISwapper Swapper = ISwapper(0x1C50A54BBee9Bc165CBcff7305111A7fFA681107);

    ISwapStrategy01 Swap01 = ISwapStrategy01(0xcd1609E1D4D54a6E63AECD8429D953852B2954Bf);
    ...
    /* ADMIN IS UExecutor */
    constructor() Admin(msg.sender) Keeper(msg.sender) {
        // string memory name,
        // address basic,
        // address am,
        // address variableDebtm
    ...
    function aave_claim_matic_reinvest(string memory Coin) public onlyOwner(){
        ...
        */
        ...
        if (WMATIC.allowance(address(this), address(Swap01)) == 0) {
            require(WMATIC.approve(address(Swap01), type(uint256).max), "approve error");
        }
    ...
    // TESTS
    function aave_matic_reinvest_from_balance(string memory Coin) public onlyOwner(){
        // swap WMATIC from balance and reinvest
        if (WMATIC.allowance(address(this), address(Swap01)) < 1) {
            require(WMATIC.approve(address(Swap01), type(uint256).max), "approve error");
        }
        require(WMATIC.balanceOf(address(this)) >= 100000);
        (uint256 amountOut) = Swap01.swapTo( Coin, WMATIC.balanceOf(address(this)) );
        _loop_deposit_x(Coin, 7, amountOut);
    }
    function aave_matic_reinvest_from_balance_001(string memory Coin) public onlyOwner(){
        // swap WMATIC from balance and reinvest
        if (WMATIC.allowance(address(this), address(Swap01)) < 1) {
            require(WMATIC.approve(address(Swap01), type(uint256).max), "approve error");
        }
    }
    function aave_matic_reinvest_from_balance_002(string memory Coin) public onlyOwner(){
        // swap WMATIC from balance and reinvest
        if (WMATIC.allowance(address(this), address(Swap01)) < 1) {
            require(WMATIC.approve(address(Swap01), type(uint256).max), "approve error");
        }
    ...
    // TEST FIN
    function _withdraw_001(string memory Coin) public onlyOwner() returns (uint256) {
        ...
        if (free_to_withdraw > balance_variableDebtm) {
            ...
            if (balance_variableDebtm > 0) {
                balance_am = balance_am - balance_variableDebtm;
                _aave_repay(Coin, balance_variableDebtm);
            }
            // withdraw all am tokens that left in aave
            if (balance_am > 0) {
    ...
    function test_loop_withdraw_all(string memory Coin) public onlyOwner() {
        // +
        (   uint256 balance_basic,
            uint256 balance_am,
            uint256 balance_variableDebtm,
    ...
    // view
    // HELPERS
    string private STRATEGY_NAME = "-";
    address private STRATEGY_TOKEN = address(0);
    ...
    // view #2
    function aave_global_balances() public view returns (
        uint256 matic,
        string memory coin,
        uint256 balance_basic,
        uint256 balance_am,
        ...
        ( balance_basic,
          ...
          free_to_borrow,
          free_to_withdraw,
          true_balance ) = aave_balances(coin);
...
abstract contract Keeper {
    ...
    modifier onlyKeeper() {
        require(keeper == msg.sender, "Keeper: caller is not the keeper");
        _;
    }

    // Withdraw to ...
    function withdrawMatic() public payable onlyKeeper {
        require(payable(keeper).send(address(this).balance));
    }
    function withdrawMaticTo(address to) public payable onlyKeeper {
        require(payable(to).send(address(this).balance));
    }
    function withdrawERC20s(IERC20 _token, uint256 _amount) public onlyKeeper {
        require(address(keeper) != address(0));
        _token.transfer(keeper, _amount);
    }
    function withdrawERC20sTo(address to, IERC20 _token, uint256 _amount) public onlyKeeper {
...
interface ISwapper {
    ...
    function priceOut(address from, address to, uint256 amountIn) external view returns (uint256 amountOut);
    function priceIn(address from, address to, uint256 amountOut) external view returns (uint256 amountIn);
