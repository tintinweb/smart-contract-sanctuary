...
// File contracts/strategies/dinoswap/interfaces/IEternalPool.sol
...
// File contracts/strategies/dinoswap/DinoStakingStrategy.sol

pragma solidity 0.5.16;
/// @title Dinoswap Staking Strategy
/// @author Chainvisions
/// @notice Strategy Dinoswap Jurassic Pools.

contract DinoswapStakingStrategy is IStrategy, BaseUpgradeableStrategy {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    ...
    public initializer {
        ...
        require(IEternalPool(rewardPool()).DINO() == underlying(), "Strategy: Pool underlying does not match with underlying");
    }
    ...
    function rewardPoolStake() internal view returns (uint256 stake) {
        (stake,) = IEternalPool(rewardPool()).userInfo(address(this));
    }
    ...
    */
    function emergencyExit() public onlyGovernance {
        IEternalPool(rewardPool()).emergencyWithdraw();
    ...
    */
    ...
    function investAllUnderlying() internal onlyNotPausedInvesting {
        ...
        if(underlyingBalance > 0) {
            ...
            IEternalPool(rewardPool()).deposit(underlyingBalance);
        }
    }
    ...
    */
    function _liquidateReward(uint256 _rewardBalance) internal {
        if(!sell() || _rewardBalance < sellFloor()) {
            emit ProfitsNotCollected(sell(), _rewardBalance < sellFloor());
            return;
        }

        notifyProfitInRewardToken(_rewardBalance);
    }
    ...
    */
    function withdrawAllToVault() public restricted {
        if(rewardPool() != address(0)) {
            IEternalPool(rewardPool()).withdraw(rewardPoolStake());
        }
    ...
    */
    function withdrawToVault(uint256 amount) public restricted {
        ...
        if(amount > IERC20(underlying()).balanceOf(address(this))){
            ...
            IEternalPool(rewardPool()).withdraw(Math.min(rewardPoolStake(), needToWithdraw));
        }
    ...
    */
    function doHardWork() external onlyNotPausedInvesting restricted {
        // We compare the balance before and after to prevent liquidating any DINO.
        uint256 underlyingBalanceBefore = IERC20(underlying()).balanceOf(address(this));
        IEternalPool(rewardPool()).deposit(0);
        uint256 underlyingBalanceAfter = IERC20(underlying()).balanceOf(address(this));
        _liquidateReward(underlyingBalanceAfter.sub(underlyingBalanceBefore));
        investAllUnderlying();
    }
...
// File contracts/strategies/dinoswap/mainnet/StrategyDino_DINO_STAKING.sol

pragma solidity 0.5.16;
contract StrategyDino_DINO_STAKING is DinoswapStakingStrategy {
    
    address dino_staking_diff;
    ...
    function initializeStrategy(
        ...
        address eternalPool = address(0x52e7b0C6fB33D3d404b07006b006c8A8D6049C55);
        address dino = address(0xAa9654BECca45B5BDFA5ac646c939C62b527D394);
        DinoswapStakingStrategy.initializeStrategy(
            _store,
            dino,
            _vault,
            eternalPool,
