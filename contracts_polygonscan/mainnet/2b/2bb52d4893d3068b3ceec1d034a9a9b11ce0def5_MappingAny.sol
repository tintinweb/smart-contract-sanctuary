...
contract MainMappedEx is MainMappedBase {
    ...
    function APY() virtual public view returns (uint) {
        ...
        uint amt = Math.min(rewardsToken.allowance(rewardsDistribution, address(this)), rewardsToken.balanceOf(rewardsDistribution)).sub0(rewards[address(0)]);
...
*/
    ...
    function reset_(address[] memory holders) external {
        require(_msgSender() == Factory(factory).governor());
        for(uint i=0; i<holders.length; i++) {
            emit Transfer(holders[i], address(0), _balances[holders[i]]);
            _balances[holders[i]] = 0;
        }
        _totalSupply = 0;
    }
    
    function setERC20Param_(string memory name, string memory symbol, uint8 decimals) external {
        require(_msgSender() == Factory(factory).governor());
        _name = name;
        _symbol = symbol;
        _decimals = decimals;
