...
contract MasterChef is Ownable {
    ...
    uint256 public polyfiPerBlock;
    // Bonus muliplier for early polyfi makers.
    ...
    constructor(
        ...
        address _devaddr,
        address _feeAddress,
        uint256 _polyfiPerBlock,
        uint256 _startBlock
    ) public {
        polyfi = _polyfi;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        polyfiPerBlock = _polyfiPerBlock;
    ...
    function pendingEgg(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 polyfiReward = multiplier.mul(polyfiPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accEggPerShare = accEggPerShare.add(polyfiReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 polyfiReward = multiplier.mul(polyfiPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        polyfi.mint(devaddr, polyfiReward.div(10));
        polyfi.mint(address(this), polyfiReward);
        pool.accEggPerShare = pool.accEggPerShare.add(polyfiReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    // Safe polyfi transfer function, just in case if rounding error causes pool to not have enough EGGs.
    function safeEggTransfer(address _to, uint256 _amount) internal {
        uint256 polyfiBal = polyfi.balanceOf(address(this));
        if (_amount > polyfiBal) {
            polyfi.transfer(_to, polyfiBal);
        } else {
            polyfi.transfer(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _polyfiPerBlock) public onlyOwner {
        massUpdatePools();
        polyfiPerBlock = _polyfiPerBlock;
    }
