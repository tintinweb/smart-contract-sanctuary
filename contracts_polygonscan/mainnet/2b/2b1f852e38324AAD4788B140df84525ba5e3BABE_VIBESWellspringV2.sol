...
contract NFTTokenFaucetV3 is AccessControlEnumerable {
  ...
  function claim(IERC721 nft, uint256 tokenId, uint256 amount) public {
    ...
    // compute how much we can claim, only pay attention to amount if its less
    // than available
    uint256 availableToClaim = _claimable(nft, tokenId); // throws here on legacy token
    uint256 toClaim = amount < availableToClaim ? amount : availableToClaim;
    require(toClaim > 0, "nothing to claim");
  ...
  function getToken(IERC721 nft, uint256 tokenId) public view returns (TokenView memory) {
    ...
    TokenView memory tokenView = TokenView({
      ...
      balance: data.balance, // will be zero for legacy tokens
      lastClaimAt: data.lastClaimAt, // will be zero for legacy tokens
      claimable: 0, // set below
      unlocksAt: 0 // set below
    });

    // if its a legacy token, query the original contracts for real-time info
    if (data.isLegacyToken) {
      ...
      tokenView.unlocksAt = legacyLock.tokenUnlocksAt(tokenId);
    // else if its an actual valid token, resolve claimable amount and lock status
    } else if (isValid && isSeeded) {
      tokenView.claimable = _claimable(nft, tokenId);
      tokenView.unlocksAt = lock.tokenUnlocksAt(nft, tokenId);
    } else {
      // invalid tokens, leave struct zero-ed out
    }

    return tokenView;
  }

  // determine how many tokens are claimable for a specific NFT -- reverts if
  // called for a invalid or legacy token
  function _claimable(IERC721 nft, uint256 tokenId) private view returns (uint256) {
    TokenData memory data = _tokenData[nft][tokenId];
  ...
  // add a given token to the main index and index by seeder
