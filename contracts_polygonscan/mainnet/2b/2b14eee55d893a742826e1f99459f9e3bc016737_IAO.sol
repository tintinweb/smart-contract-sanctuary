 ...
 */
contract IAO is ReentrancyGuard, Initializable, Ownable {
    ...
    event LockByBUSD(address indexed user, uint256 amount);
    event Claim(
    ...
    function lock(uint256 _amount) external onlyActiveIAO {
    ...
    /// @dev Deposit ERC20 tokens with support for reflect tokens
    function lockWithRef(uint256 _amount, address addressReferral) external onlyActiveIAO {
        require(_amount > 0, "_amount not > 0");
        uint256 pre = getTotalStakeTokenBalance();
        stakeToken.safeTransferFrom(msg.sender, address(this), _amount);
        uint256 finalDepositAmount = getTotalStakeTokenBalance() - pre;
    ...
    /// @dev Deposit ERC20 tokens with support for reflect tokens
    function lockByBUSD(address _userAddress, uint256 _amount, address referralAddress) external onlyAdmin {
        require(_amount > 0, "_amount not > 0");
        ...
        emit LockByBUSD(_userAddress, _amount);
    }

    function claim() external nonReentrant onlyFinishedIAO {
        require(userInfo[msg.sender].amount > 0, "have you participated?");
        require(!userInfo[msg.sender].claimed, "harvest already claimed");
    ...
    function getOfferingAmount(address user) public view returns (uint256) {
        if (totalWeiCap <= totalRaisingInBUSD + totalRaisingInUSDC * 1e12) {
            ...
            return userInfo[user].amount * rate * 1e12 + userInfo[user].amountInBUSD * rate;
            // (/ 1e6 * rate * 1e18)
        }
    }
    ...
    function getUSDCRefundAmount(address _user) public view returns (uint256) {
        ...
        uint256 payAmount = (totalWeiCap / (totalRaisingInBUSD + totalRaisingInUSDC * 1e12)) * userInfo[_user].amount;
