...
contract Sky is Ownable, ISky {
   ...
   function setBalancePercentage(uint256 index, uint256 percentage) external onlyOwner {
       require(index < cloudsIndex.length, "IC");
       Roller(payable(cloudsIndex[index])).setBalancePercentage(percentage);
   }

   function setStartBalance(uint256 index, uint256 balance) external onlyOwner {
       require(index < cloudsIndex.length, "IC");
       Roller(payable(cloudsIndex[index])).setStartBalance(balance);
   }


   function doneAt(uint256 index) external onlyOwner {
       require(index < cloudsIndex.length, "IC");
       Roller(payable(cloudsIndex[index])).stop();
       Roller(payable(cloudsIndex[index])).withdraw();
   }
   ...
   function maticDiff() public view returns(uint256, uint256, uint256){
       return (totalStart, totalOver, totalMatic());
   }
}
...
contract Roller is Ownable,  VRFConsumerBase {
    ...
    function canPlaceBet() public view returns(bool){
        return wasPendingBet > waitForRounds || pendingBet == 0;
    }
    ...
    function roundWaits() internal view returns(uint256){
        return waitForRounds;
    }
...
contract Charge is Roller {
    ...
    function betAverage(uint256 random) internal view returns(uint256){
        ...
        uint256 average = 0;
        for(uint256 i = outcomes.length - 10; i < outcomes.length; i++){
            average = average.add(outcomes[i]);
        }
        
        if(average.div(10) < 25){
            return 50 + (random % 25);
        }
        if(average.div(10) < 50){
            return 95 + (random%4);
        }

        if(average.div(10) > 80){
            return 10 + (random % 50);
        }
