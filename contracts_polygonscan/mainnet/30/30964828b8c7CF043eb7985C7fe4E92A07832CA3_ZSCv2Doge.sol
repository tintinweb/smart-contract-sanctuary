...
import "./ZmaticVerifier.sol";
import "./ZmaticVerifierFeeD.sol";
...
contract ZSCv2Doge {
   ...
   // ZSC zsc;
    ERC20 coin;
    ZmaticVerifier zmaticVerifier;
    ZmaticVerifierFeeD zmaticVerifierFeeD;
    BurnVerifier burnVerifier;
    uint256 public epochLength = 60;
    uint256 public constant fee = 10; 
    uint256 public constant nofee = 0; 
    address private _owner;

    uint256 constant MAX = 4294967295; // *10**18 for deposit-withdraw
    ...
    address public _coin = 0x8A953CfE442c5E8855cc6c61b1293FA648BAE472;
    ...
    constructor(address _zmatic, address _zmaticfee, address _burn) { // visibiility won't be needed in 7.0
        // epoch length, like block.time, is in _seconds_. 4 is the minimum!!! (To allow a withdrawal to go through.)
        _owner = msg.sender;
        coin = ERC20(_coin);
        zmaticVerifier = ZmaticVerifier(_zmatic);
        zmaticVerifierFeeD = ZmaticVerifierFeeD(_zmaticfee);
    ...
    function setEpochLength(uint256 len) external onlyOwner() {
        epochLength = len;
    }
    ...
    function registered_(Utils.G1Point memory y) public view returns (bool) {
    ...
    function enable(Utils.G1Point memory y) public {
        bytes32 yHash = keccak256(abi.encode(y));
    ...
    function fund(Utils.G1Point memory y, uint256 bTransfer) public {
        ...
        require(coin.transferFrom(msg.sender, address(this), bTransfer * 10 ** 24), "Transfer from sender failed.");
        require(coin.balanceOf(address(this)) <= MAX * 10 ** 24, "Fund pushes contract past maximum value.");
    }

       function ftransfer(Utils.G1Point[] memory C, Utils.G1Point memory D, Utils.G1Point[] memory y, Utils.G1Point memory u, bytes memory proof, Utils.G1Point memory beneficiary) public {
        uint256 size = y.length;
        ...
        require(zmaticVerifierFeeD.verifyTransfer(CLn, CRn, C, D, y, lastGlobalUpdate, u, proof), "Transfer proof verification failed!");
    ...
    function transfer(Utils.G1Point[] memory C, Utils.G1Point memory D, Utils.G1Point[] memory y, Utils.G1Point memory u, bytes memory proof, Utils.G1Point memory beneficiary) public {
        ...
        require(registered(beneficiaryHash), "Miner's account is not yet registered.");
        rollOver(beneficiaryHash);
        pending[beneficiaryHash][0] = pending[beneficiaryHash][0].add(Utils.g().mul(nofee));
        ...
        require(zmaticVerifier.verifyTransfer(CLn, CRn, C, D, y, lastGlobalUpdate, u, proof), "Transfer proof verification failed!");
    ...
    function burn(Utils.G1Point memory y, uint256 bTransfer, Utils.G1Point memory u, bytes memory proof) public {
        ...
        require(coin.transfer(msg.sender, bTransfer * 10 ** 24), "This shouldn't fail... Something went severely wrong.");
    }
}
...
contract ZmaticVerifierFeeD {
    ...
    uint256 public constant fee = 10; // set this to be the "transaction fee". can be any integer under MAX.

    struct ZmaticStatement {
        Utils.G1Point[] CLn;
        Utils.G1Point[] CRn;
        Utils.G1Point[] C;
        Utils.G1Point D;
    ...
    function verifyTransfer(Utils.G1Point[] memory CLn, Utils.G1Point[] memory CRn, Utils.G1Point[] memory C, Utils.G1Point memory D, Utils.G1Point[] memory y, uint256 epoch, Utils.G1Point memory u, bytes memory proof) public view returns (bool) {
        ZmaticStatement memory statement;
        ...
        statement.u = u;
        ZmaticProof memory zmaticProof = unserialize(proof);
        return verify(statement, zmaticProof);
    }

    struct ZmaticAuxiliaries {
        uint256 y;
        uint256[64] ys;
        uint256 z;
    ...
    function verify(ZmaticStatement memory statement, ZmaticProof memory proof) internal view returns (bool) {
        ...
        ZmaticAuxiliaries memory zmaticAuxiliaries;
        zmaticAuxiliaries.y = uint256(keccak256(abi.encode(anonAuxiliaries.w))).mod();
        zmaticAuxiliaries.ys[0] = 1;
        zmaticAuxiliaries.k = 1;
        for (uint256 i = 1; i < 64; i++) {
            zmaticAuxiliaries.ys[i] = zmaticAuxiliaries.ys[i - 1].mul(zmaticAuxiliaries.y);
            zmaticAuxiliaries.k = zmaticAuxiliaries.k.add(zmaticAuxiliaries.ys[i]);
        }
        zmaticAuxiliaries.z = uint256(keccak256(abi.encode(zmaticAuxiliaries.y))).mod();
        zmaticAuxiliaries.zs[0] = zmaticAuxiliaries.z.mul(zmaticAuxiliaries.z);
        zmaticAuxiliaries.zs[1] = zmaticAuxiliaries.zs[0].mul(zmaticAuxiliaries.z);
        zmaticAuxiliaries.zSum = zmaticAuxiliaries.zs[0].add(zmaticAuxiliaries.zs[1]).mul(zmaticAuxiliaries.z);
        zmaticAuxiliaries.k = zmaticAuxiliaries.k.mul(zmaticAuxiliaries.z.sub(zmaticAuxiliaries.zs[0])).sub(zmaticAuxiliaries.zSum.mul(1 << 32).sub(zmaticAuxiliaries.zSum));
        zmaticAuxiliaries.t = proof.tHat.sub(zmaticAuxiliaries.k); // t = tHat - delta(y, z)
        for (uint256 i = 0; i < 32; i++) {
            zmaticAuxiliaries.twoTimesZSquared[i] = zmaticAuxiliaries.zs[0].mul(1 << i);
            zmaticAuxiliaries.twoTimesZSquared[i + 32] = zmaticAuxiliaries.zs[1].mul(1 << i);
        }

        zmaticAuxiliaries.x = uint256(keccak256(abi.encode(zmaticAuxiliaries.z, proof.T_1, proof.T_2))).mod();
        zmaticAuxiliaries.tEval = proof.T_1.mul(zmaticAuxiliaries.x).add(proof.T_2.mul(zmaticAuxiliaries.x.mul(zmaticAuxiliaries.x))); // replace with "commit"?
        ...
        sigmaAuxiliaries.A_b = Utils.g().mul(proof.s_b).add(anonAuxiliaries.DR.mul(zmaticAuxiliaries.zs[0].neg()).add(anonAuxiliaries.CRnR.mul(zmaticAuxiliaries.zs[1])).mul(proof.s_sk).add(anonAuxiliaries.CR[0][0].add(Utils.g().mul(fee.mul(anonAuxiliaries.wPow))).mul(zmaticAuxiliaries.zs[0].neg()).add(anonAuxiliaries.CLnR.mul(zmaticAuxiliaries.zs[1])).mul(proof.c.neg())));
        sigmaAuxiliaries.A_X = anonAuxiliaries.y_XR.mul(proof.s_r).add(anonAuxiliaries.C_XR.mul(proof.c.neg()));
        sigmaAuxiliaries.A_t = Utils.g().mul(zmaticAuxiliaries.t).add(zmaticAuxiliaries.tEval.neg()).mul(proof.c.mul(anonAuxiliaries.wPow)).add(Utils.h().mul(proof.s_tau)).add(Utils.g().mul(proof.s_b.neg()));
        sigmaAuxiliaries.gEpoch = Utils.mapInto("Zmatic", statement.epoch);
        sigmaAuxiliaries.A_u = sigmaAuxiliaries.gEpoch.mul(proof.s_sk).add(statement.u.mul(proof.c.neg()));

        sigmaAuxiliaries.c = uint256(keccak256(abi.encode(zmaticAuxiliaries.x, sigmaAuxiliaries.A_y, sigmaAuxiliaries.A_D, sigmaAuxiliaries.A_b, sigmaAuxiliaries.A_X, sigmaAuxiliaries.A_t, sigmaAuxiliaries.A_u))).mod();
        ...
        for (uint256 i = 0; i < 64; i++) {
            ipAuxiliaries.hPrimes[i] = ip.hs(i).mul(zmaticAuxiliaries.ys[i].inv());
            ipAuxiliaries.hPrimeSum = ipAuxiliaries.hPrimeSum.add(ipAuxiliaries.hPrimes[i].mul(zmaticAuxiliaries.ys[i].mul(zmaticAuxiliaries.z).add(zmaticAuxiliaries.twoTimesZSquared[i])));
        }
        ipAuxiliaries.P = proof.BA.add(proof.BS.mul(zmaticAuxiliaries.x)).add(gSum().mul(zmaticAuxiliaries.z.neg())).add(ipAuxiliaries.hPrimeSum);
    ...
    function unserialize(bytes memory arr) internal pure returns (ZmaticProof memory proof) {
...
contract InnerProductVerifier {
    ...
    function verify(InnerProductStatement memory statement, InnerProductProof memory proof, uint256 salt) internal view returns (bool) {
        ...
        // it is an unusual and clever variant of what we already do in ZmaticVerifier.sol:234-249, but with the special property that it requires only 1 inversion.
...
contract BurnVerifier {
    ...
    struct BurnAuxiliaries {
        ...
        uint256[32] ys;
        uint256 z;
        uint256[1] zs; // silly. just to match zmatic.
        uint256 zSum;
