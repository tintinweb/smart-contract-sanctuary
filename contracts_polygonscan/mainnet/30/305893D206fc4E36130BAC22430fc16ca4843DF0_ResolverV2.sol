...
contract ResolverV2 {
  ...
  function hasFunds(address owner) public view returns (bool) {
    address treasury = careCentre.treasury();
    // return ()
  }
  ...
  {
    AavegotchiInfo[] memory aavegotchiInfos = gotchiFacet.allAavegotchisOfOwner(
      owner
    );

    uint256 length = aavegotchiInfos.length;

    uint256 gotchis = 0;

    // count number of gotchis
    for (uint256 x = 0; x < length; x++) {
      if (aavegotchiInfos[x].status == STATUS_AAVEGOTCHI) {
        gotchis += 1;
      }
    }

    uint256[] memory gotchiIds = new uint256[](gotchis);
    uint256 pos = 0;

    // create gotchiIds array
    for (uint256 x = 0; x < length; x++) {
      if (aavegotchiInfos[x].status == STATUS_AAVEGOTCHI) {
        gotchiIds[pos] = aavegotchiInfos[x].tokenId;
        pos += 1;
      }
    }
