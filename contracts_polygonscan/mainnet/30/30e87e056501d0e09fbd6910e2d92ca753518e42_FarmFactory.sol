...
contract FarmFactory is ReentrancyGuard {
    struct FarmBuild {
        string tokenName;
        string tokenSymbol;
        uint startBlock;
    ...
    event Build(address owner, address token, address masterChef, address pair);
    ...
    function getPoolWeights(address owner) external view returns(uint[] memory) {
        return farmBuilds[owner].poolWeights;
    }

    function getPoolAddresses(address owner) external view returns(address[] memory) {
        return farmBuilds[owner].poolAddresses;
    }

    function getPoolDepositFees(address owner) external view returns(uint16[] memory) {
        return farmBuilds[owner].poolDepositFees;
    }

    function setBuildFee(uint _buildFee) external {
    ...
    function build() external payable nonReentrant {
        ...
        emit Build(msg.sender, address(farmToken), address(masterChef), newPair);
    }
}