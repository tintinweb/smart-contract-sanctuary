...
abstract contract BaseStrategyMasterChef is BaseStrategy {
    ...
    function getHarvestable() external override view returns (uint256) {
        return IMasterChef(rewards).pendingDino(poolId, address(this));
    }
...
abstract contract StrategyFarmTwoAssets is BaseStrategyMasterChef {
    ...
    address public constant dino = 0xAa9654BECca45B5BDFA5ac646c939C62b527D394;
    ...
    uint256 public harvestCutoff = 10**15;
    uint256 public constant keepFXSmax = 10000;

    // Define reward router
    address constant public rewardRouter = 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff;

    // Uniswap swap paths
    address[] public dino_usdc_path;
    address[] public dino_eth_path;
        ...
        )
    {
        ...
        dino_eth_path = new address[](3);
        dino_eth_path[0] = dino;
        dino_eth_path[1] = usdc;
        dino_eth_path[2] = eth;
    }
    ...
    function set_harvestCutoff(uint256 newCutoff) external onlyOwner() {
        require(newCutoff <= 10**18, "New cutoff must be less than 1 Token");
    ...
    function harvest() public override {
        ...
        // Collects dino tokens
        _getReward();
        

        //Swap
        uint256 _dinoBalance = IERC20(dino).balanceOf(address(this));
        if (_dinoBalance > harvestCutoff){
            
            //Send performance fee to strategist
                uint256 performanceFee = _dinoBalance.mul(keepFXS).div(keepFXSmax);
                if (performanceFee>0) {
                    _swapUniswapWithPath(dino_usdc_path, _dinoBalance);
                    IERC20(usdc).safeTransfer(
                        strategist,
                        performanceFee
                    );
                }
            _dinoBalance =IERC20(dino).balanceOf(address(this));
            _swapUniswapWithPath(dino_usdc_path, _dinoBalance.div(2));
            uint256 _dinoBalancePost = IERC20(dino).balanceOf(address(this));
            _swapUniswapWithPath(dino_eth_path, _dinoBalancePost);

            // Add liquidity for TITAN/MATIC
            uint256 _eth = IERC20(eth).balanceOf(address(this));
            uint256 _usdc = IERC20(usdc).balanceOf(address(this));
            if (_eth > 0 && _usdc > 0) {
                IERC20(eth).safeApprove(rewardRouter, 0);
                IERC20(eth).safeApprove(rewardRouter, _eth);
                IERC20(usdc).safeApprove(rewardRouter, 0);
                IERC20(usdc).safeApprove(rewardRouter, _usdc);

                IUniswapRouterV2(rewardRouter).addLiquidity(
                    eth,
                    usdc,
                    _eth,
                    _usdc,
...
contract StrategyTwoAssets is StrategyFarmTwoAssets {
    // Token addresses
    address public DINO_MASTER_CHEF = 0x1948abC5400Aa1d72223882958Da3bec643fb4E5;
    address public Eth_USDC_LP = 0x853Ee4b2A13f8a742d64C8F088bE7bA2131f670d;
