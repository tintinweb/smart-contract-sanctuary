...
contract TWAPOracleUpdater is ERC20Permit, VaultOwned {
  ...
  function addTWAPSource( address newTWAPSourceDexPool_ ) external onlyOwner() {
    require( _dexPoolsTWAPSources.add( newTWAPSourceDexPool_ ), "Zero Gravity Space Shuttle: TWAP Source already stored." );
    emit TWAPSourceAdded( newTWAPSourceDexPool_ );
  }

  function removeTWAPSource( address twapSourceToRemove_ ) external onlyOwner() {
    require( _dexPoolsTWAPSources.remove( twapSourceToRemove_ ), "Zero Gravity Space Shuttle: TWAP source not present." );
...
contract ZEROG is Divine {

  using SafeMath for uint256;

    constructor() Divine("Zero Gravity Space Shuttle", "ZEROG", 9) {
    }
