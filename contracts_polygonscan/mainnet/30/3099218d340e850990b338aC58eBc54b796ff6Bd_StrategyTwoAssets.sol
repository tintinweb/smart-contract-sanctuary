...
abstract contract StrategyFarmTwoAssets is BaseStrategyMasterChef {
        ...
        )
    {
        ...
        quick_avax_path = new address[](3);
        quick_avax_path[0] = quick;
        quick_avax_path[1] = eth;
        quick_avax_path[2] = avax;
    }
    ...
    function harvest() public override {
        ...
        if (_quickBalance > harvestCutoff){
            ...
            _swapUniswapWithPath(quick_avax_path, _quickBalancePost);

            // Add liquidity
            uint256 _eth = IERC20(eth).balanceOf(address(this));
            uint256 _avax = IERC20(avax).balanceOf(address(this));
            if (_eth > 0 && _avax > 0) {
                IERC20(eth).safeApprove(currentRouter, 0);
                IERC20(eth).safeApprove(currentRouter, _eth);
                IERC20(avax).safeApprove(currentRouter, 0);
                IERC20(avax).safeApprove(currentRouter, _avax);
                ...
                IERC20(avax).safeTransfer(
                    strategist,
                    IERC20(avax).balanceOf(address(this))
                );
            }
...
contract StrategyTwoAssets is StrategyFarmTwoAssets {
    // Token addresses
    address public Quick_Rewards = 0x02e33E4713Cf231D4b7A9894DE3F075A16e19201;
    address public AVAX_MATIC_LP = 0xEb477AE74774b697B5D515eF8ca09e24FEe413b5;
