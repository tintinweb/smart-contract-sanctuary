...
contract DestructionTokenV3 is ERC20, Ownable {
    ...
    // antibot system
    IAntiBotBlacklist antiBotBlackList;
    // operator role
    ...
    modifier antiBot(address _sender, address _recipient, uint256 _amount) { 
        //check blacklist
        require(_blacklistChecker(_sender), "ERR1");
        require(_blacklistChecker(_recipient), "ERR2");
    ...
    constructor(address _antiBotBlackList) 
        ERC20('DESTRUCTION', 'DESTRUCTION')
    {
      // Exclude operator addresses, lps, etc from antibot system
        ...
        antiBotBlackList = IAntiBotBlacklist(_antiBotBlackList);
            ...
            if (_isNotHolderExcludedFromAntiBot(_sender)){
                if(_isNotOperatorExcludedFromAntiBot(_sender)){
                    if (balanceOf(_sender) > _maxUserHoldAmount()) {
                        antiBotBlackList.addSuspect(_sender, type(uint256).max);
                        return;
                    }
                }
            }
    ...
    function addBotAddress(address _botAddress, uint256 _expirationTime) external onlyOwner {
        antiBotBlackList.addSuspect(_botAddress, _expirationTime);
    }
    
    ///@dev Remove an address from the blacklist. Only the owner can remove. Owner is the address of the Governance contract.
    function removeBotAddress(address _botAddress) external onlyOperator {
        antiBotBlackList.removeSuspect(_botAddress);
    }

    function _blacklistChecker(address _botAddress) internal view returns (bool){
        return !antiBotBlackList.blacklistCheck(_botAddress);
    }
