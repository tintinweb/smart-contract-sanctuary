...
abstract contract StrategyFarmBase is BaseStrategyMasterChef {
    ...
    address public constant bone = 0x6bb45cEAC714c52342Ef73ec663479da35934bf7;
        ...
        )
    {
        ...
        usdc_bone_path = new address[](2);
        usdc_bone_path[0] = usdc;
        usdc_bone_path[1] = bone;
    }
    ...
    function harvest() public override {
        ...
        if (_ballBalance > harvestCutoff){
            ...
            _swapUniswapWithPath(usdc_bone_path, _usdcBalancePost.div(2));

            // Add liquidity for Ball/USDC
            uint256 _bone = IERC20(bone).balanceOf(address(this));
            uint256 _usdc = IERC20(usdc).balanceOf(address(this));
            if (_bone > 0 && _usdc > 0) {
                IERC20(bone).safeApprove(currentRouter, 0);
                IERC20(bone).safeApprove(currentRouter, _bone);
                ...
                // Donates DUST
                IERC20(bone).safeTransfer(
                    strategist,
                    IERC20(bone).balanceOf(address(this))
                );
                IERC20(usdc).safeTransfer(
                    strategist,
