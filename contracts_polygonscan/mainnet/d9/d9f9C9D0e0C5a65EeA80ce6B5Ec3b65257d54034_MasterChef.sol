...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    // Max harvest interval: 8 hours.
    uint256 public constant MAXIMUM_HARVEST_INTERVAL = 28800;
    ...
    // Referral commission rate 2% in basis points.
    ...
    // nonDuplicated modified is the fix for XXX DO NOT add the same LP token more than once. Rewards will be messed up if you do.
    ...
    function canHarvest(uint256 _pid, address _user) public view returns (bool) {
        // Vault addresses can havest at any time
        if (isVault[_user]) return true;
        UserInfo storage user = userInfo[_pid][_user];
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 TokenReward = multiplier.mul(TokenPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        token.mint(bridgeAddress, TokenReward.div(10));
        token.mint(address(this), TokenReward);
        pool.accTokenPerShare = pool.accTokenPerShare.add(TokenReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Compound LP tokens to MasterChef for token allocation.
    function compound(uint256 _pid) public nonReentrant {
        require(block.timestamp >= startTime, "Farm has not started yet");
        require(address(swapRouter) != address(0), "MasterChef::compound: Invalid swapRouter address.");
        ...
        // Second argument is compound true/false
        uint256 total = payOrLockupPendingToken(_pid, true);
        if (total > 0) {
            ...
            if (address(pool.lpToken) == address(token)) {
                ...
                token.approve(address(swapRouter), uint256(-1));
                ...
                if (address(token0) != address(token)) {
                ...
                if (address(token1) != address(token)) {
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        // Initialize the pool lastRewardBlock on the first deposit
        if (pool.lastRewardBlock == 0) pool.lastRewardBlock = block.number;

        updatePool(_pid);
        // Second argument is compound true/false
        payOrLockupPendingToken(_pid, false);
        if (_amount > 0) {
            uint256 beforeDeposit = pool.lpToken.balanceOf(address(this));
            pool.lpToken.safeTransferFrom(address(msg.sender), address(this), _amount);
            uint256 afterDeposit = pool.lpToken.balanceOf(address(this));
            _amount = afterDeposit.sub(beforeDeposit);
            // Vault addresses deposit without Fee/Tax as it has own Fee/Tax setup to avoid double Fee/Tax
            if (isVault[msg.sender]) {
                user.amount = user.amount.add(_amount);
            } else {
                if (pool.depositFeeBP > 0) {
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        // if has passed the harvest lockup then its eglible for reward and send rewards else send to fee address
        if (canHarvest(_pid, msg.sender)) {
            // Second argument is compound true/false
            payOrLockupPendingToken(_pid, false);
        } else {
            // Pending to fee address in case withdraw before nextHarvestUntil.
            uint256 pending = user.amount.mul(pool.accTokenPerShare).div(1e12).sub(user.rewardDebt).add(user.rewardLockedUp);
            if (pending > 0) safeTokenTransfer(feeAddress, pending);
    ...
    function payOrLockupPendingToken(uint256 _pid, bool _compound) internal returns (uint256 totalRewards) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        totalRewards = 0;
        ...
        // compound can be done at any time
        if (canHarvest(_pid, msg.sender) || _compound) {
            if (pending > 0 || user.rewardLockedUp > 0) {
                ...
                totalLockedUpRewards = totalLockedUpRewards.sub(user.rewardLockedUp);
                user.rewardLockedUp = 0;
                if (canHarvest(_pid, msg.sender)) user.compoundAmount = 0;
                // in case compound just add half harvestInterval
                uint256 newHarvestUntil = block.timestamp + pool.harvestInterval.div(_compound ? 2 : 1);
                if (newHarvestUntil > user.nextHarvestUntil) user.nextHarvestUntil = newHarvestUntil;

                // send rewards only if user harvest
                if (! _compound) {
    ...
    /// Update the bridge contract address by the owner
    function setBridgeAddress(address _bridgeAddress) public onlyOwner {
        require(_bridgeAddress != address(0), "setFeeAddress: ZERO");
...
contract OBERONToken is BEP20 {
    ...
    // Max transfer amount rate in basis points. (default is 1.5% of circulating supply = total supply - burn address - fee/tax address - bridge address)
    uint16 public maxTransferAmountRate = 150;
    // Max delay each transfer between src <-> dst: 9 seconds between OBERON token transfers
    uint16 public constant MAXIMUM_TRANSFER_DELAY = 9;
    // Min delay each transfer between src <-> dst. (default is 9 seconds)
    uint16 public minTransferDelay = 9;
    // Addresses that excluded from antiWhale
    mapping(address => bool) public excludedFromAntiWhale;
    // Vault and Masterchef Addresses required to be exclude from antiWhale and transfer token bewtween each other without transferTax
    mapping(address => bool) public isVault;
    // List of OBERON TokenLP for AntiBot
    ...
    // Transfer tax Fee address.
    address public feeAddress;
    // The timestamp when OBERON transfer starts.
    uint256 public startTime;
    // Cross chain bridge contract address, moving between cross brige chain not apply Tax
    address public bridgeAddress;
    ...
    event FeeAddressUpdated(address indexed operator, address indexed previousFeeAddress, address indexed newFeeAddress);
    event BridgeAddressUpdated(address indexed operator, address indexed previousBridgeAddress, address indexed newBridgeAddress);
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "OBERON::antiWhale: Transfer amount exceeds the maxTransferAmount");
            }
        }
        _;
    }
    ...
    /**
     * @notice Constructs the OBERONToken contract.
     */
    constructor(
        uint256 _startTime
    ) public BEP20("OBERON Token", "OBERON") {
    ...
    /// @dev overrides transfer function to meet tokenomics of OBERON
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        if (transferTaxRate == 0
            ...
            || (isVault[sender] && isVault[recipient])
        ) {
            ...
            if (
                ...
                require(startTime <= block.timestamp, "OBERON::anitBot: Transfer token is not started yet");
                require(_lastBlockNumber[TokenLP(sender)][TokenLP(recipient)] <= block.timestamp - minTransferDelay, "OBERON::anitBot: Transfer frequency exceeds the minTransferDelay");
                require(_lastBlockNumber[TokenLP(recipient)][TokenLP(sender)] <= block.timestamp - minTransferDelay, "OBERON::anitBot: Transfer frequency exceeds the minTransferDelay");
            }

            // default tax is 8% of every transfer sent to feeAddress
            uint256 taxAmount = amount.mul(transferTaxRate).div(10000);

            // default 92% of transfer sent to recipient
            uint256 sendAmount = amount.sub(taxAmount);
            require(amount == sendAmount + taxAmount, "OBERON::transfer: Tax value invalid");
    ...
    function maxTransferAmount() public view returns (uint256) {
        uint256 circulatingSupply = totalSupply().sub(balanceOf(BURN_ADDRESS)).sub(balanceOf(feeAddress)).sub(balanceOf(bridgeAddress));
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "OBERON::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 10000, "OBERON::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
    ...
    function updateMinTransferDelay(uint16 _minTransferDelay) public onlyOperator {
        require(_minTransferDelay <= MAXIMUM_TRANSFER_DELAY, "OBERON::updateMinTransferDelay: Transfer block rate must not exceed the maximum rate.");
     ...
     * Can only be called by the current operator.
     */
    function setVault(address _account, bool _excluded) public onlyOperator {
        excludedFromAntiWhale[_account] = _excluded;
        isVault[_account] = _excluded;
    }
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "OBERON::transferOperator: new operator is the zero address");
        emit OperatorTransferred(_operator, newOperator);
        _operator = newOperator;
    }

    /**
     * @dev Set feeAddress of the token to a new account (`newFeeAddress`).
     * Can only be called by the current operator.
     */
    function setFeeAddress(address newFeeAddress) public onlyOperator {
        emit FeeAddressUpdated(msg.sender, feeAddress, newFeeAddress);
        excludedFromAntiWhale[newFeeAddress] = true;
        feeAddress = newFeeAddress;
    }

    /**
     * @dev Set cross chain BridgeAddress of the token to a new account (`newBridgeAddress`).
     * Can only be called by the current operator.
     */
    function setBridgeAddress(address newBridgeAddress) public onlyOperator {
        emit BridgeAddressUpdated(msg.sender, bridgeAddress, newBridgeAddress);
