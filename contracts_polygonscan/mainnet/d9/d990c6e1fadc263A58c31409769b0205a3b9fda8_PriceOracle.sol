...
contract PriceOracle is Initializable {
    ...
    IPairsOracle public pairsOracle;
    
    function initialize(IPairsOracle _pairsOracleAddress, address _wethAddress, address _daiAddress) public initializer {
        require(address(_pairsOracleAddress) != address(0), "PriceOracle: invalid _pairsOracleAddress");
        ...
        pairsOracle = _pairsOracleAddress;
        wethAddress = _wethAddress;
        daiAddress = _daiAddress;
        pairsOracle.addPair(_wethAddress, _daiAddress);
    }

    function addToken(address token) external returns (bool success) {
        return pairsOracle.addPair(wethAddress, token);
    }

    function update(address token) external {
        pairsOracle.update(wethAddress, daiAddress);
        pairsOracle.update(wethAddress, token);
    }
    
    function priceOf(address token, uint256 amount) external view returns (uint256 daiAmount) {
        uint256 wethAmount = pairsOracle.consult(token, wethAddress, amount);
        if (wethAmount == 0) {
            return 0;
        }
        daiAmount = pairsOracle.consult(wethAddress, daiAddress, wethAmount);
    }
}
