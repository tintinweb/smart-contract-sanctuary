...
contract FactoryDeployer is Initializable, AccessControl {
    ...
    function setMergePrice(uint256 _price) public onlyRole(MAIN_OWNER) {
        mergeEmployeePrice = _price;
    }

    function setFactoryPrice(uint256 _price) public onlyRole(MAIN_OWNER) {
        factoryPrice = _price;
    }
    ...
    function pay(
        ...
        if (whiteList[customer] > 0 && whitelist) whiteList[customer]--;
        else {
            require(creator != address(0), INVALID_CREATOR);
            require(factoryStaking != address(0), INVALID_NFT_STAKING);
            require(playToEarnPool != address(0), INVALID_PLAY_TO_EARN);
            require(token.balanceOf(customer) >= amount, INVALID_PAYMENT);

            token.transferFrom(customer, creator, (amount * CREATOR_FEE) / 100);

            token.transferFrom(
                customer,
                factoryStaking,
                (amount * NFT_STAKE_FEE) / 100
            );

            token.transferFrom(
                customer,
                playToEarnPool,
                (amount * PLAY_TO_EARN_FEE) / 100
            );
        }
    ...
    function mintFactory(uint256[] memory ids) public {
        require(ids.length == BURN_EMPLOYEES, INVALID_EMPLOYEE_SEED);
        require(employees.isOwnerOfAll(msg.sender, ids), INVALID_EMPLOYEE_SEED);
        require(pay(msg.sender, factoryPrice, true), INVALID_PAYMENT);
    ...
    function addEmployeeToFactory(uint256 factory, uint256 employee) public {
        require(employees.validate(employee), INVALID_NFT);
        require(employees.ownerOf(employee) == msg.sender, INVALID_OWNER);
        require(factories.validate(factory), INVALID_NFT);
        require(factories.ownerOf(factory) == msg.sender, INVALID_OWNER);
        require(pay(msg.sender, mergeEmployeePrice, false), INVALID_PAYMENT);

        uint8 employeeType = employees.getType(employee);
        uint8 factoryType = factories.getType(factory);
        uint16 employeePoints = employees.getPoints(employee);

        require(employeeType == factoryType, INVALID_NFT_TYPES);

        employees.burn(employee);
        factories.addToMultiplier(factory, employeePoints);
    }
