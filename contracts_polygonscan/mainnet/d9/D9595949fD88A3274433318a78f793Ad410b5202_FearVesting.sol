...
contract FearVesting is Ownable {
  ...
  constructor(address fearToken_, uint256 periodFinish_) {
    fearToken = IERC20(fearToken_);
    periodStart = block.timestamp;
    periodFinish = periodFinish_;
    _term = periodFinish - periodStart;
    require(_term > 0, "FearVesting: term must be greater than 0!");
  }
  ...
  function setTotalRewards(uint256 totalRewards_) external onlyOwner {
    require(totalRewards_ >= 0, "FearVesting: totalRewards must be greater than or equal to 0");
  ...
  /**
   * @dev calculates total amounts must be rewarded and transfers FEAR to the address
   */
  function getReward() public {
    uint256 _earned = earned(msg.sender);
    require(_earned <= rewards[msg.sender], "FearVesting: earned is more than reward!");
    require(_earned > payouts[msg.sender], "FearVesting: earned is less or equal to already paid!");

    uint256 reward = _earned.sub(payouts[msg.sender]);

    if (reward > 0) {
      payouts[msg.sender] = _earned;
      fearToken.safeTransfer(msg.sender, reward);
