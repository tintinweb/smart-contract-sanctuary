...
contract FomoMarketplace is Ownable, ReentrancyGuard {
    ...
    function buyItem(
        ...
        if (
            ...
            royalties[_nftAddress][_tokenId].length != 0
        ) {
            for (
                uint8 i = 0;
                i < royalties[_nftAddress][_tokenId].length;
                i++
            ) {
                uint256 individual = _tokenAmount
                    .sub(feeAmount)
                    .mul(royalties[_nftAddress][_tokenId][i].quota)
                    .div(100);

                listedItem.payToken.safeTransferFrom(
                    _msgSender(),
                    royalties[_nftAddress][_tokenId][i].account,
                    individual
                );
                feeAmount = feeAmount.add(individual);
            }
    ...
    function registerRoyalty(
        ...
        require(
            _accounts.length == _royalties.length && _accounts.length != 0,
            "invalid array length"
        );

        uint256 sumRoyalty;
        for (uint8 i = 0; i < _accounts.length; i++) {
            sumRoyalty = sumRoyalty.add(uint256(_royalties[i]));
        }
        require(sumRoyalty <= MAX_ROYALTY_FEE, "invalid royalty fee");
        ...
        for (uint8 i = 0; i < _accounts.length; i++) {
            royalties[_nftAddress][_tokenId].push(
                Royalty(_accounts[i], _royalties[i])
            );
        }
    }
