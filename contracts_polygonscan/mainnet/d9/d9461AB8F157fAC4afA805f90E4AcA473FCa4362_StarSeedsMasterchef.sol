...
contract StarSeedsMasterchef is Ownable, ReentrancyGuard {
    ...
    uint256 public starPerBlock = 1150000000000000;
    ...
    uint256 public totalAllocPoint = 10700;
    ...
    event BonusMupltiplier(address indexed caller, uint256 previousAmount, uint256 newAmount);
...
/////////////////// View Functions //////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    function remainingRewards() external view returns (uint256) {
        return IERC20(star).balanceOf(address(this)).sub(totalStarInPools);
    }
...
///////////////// Only Owner Functions //////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    function updateBonus(uint _bonus) external onlyOwner{
        ...
        emit BonusMupltiplier(msg.sender, oldBonus, BONUS_MULTIPLIER);
    }
...
//////////// onlyOperator functions //////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    function startFarming() public onlyOperator {
    ...
    //fund the masterchef with star tokens and emit an event for easy tracking.
    function fundMasterChef(uint _amount) external onlyOperator{
        IERC20 Star = IERC20(star);
        require(Star.allowance(msg.sender,address(this)) >= _amount,"You need to set the allowance for star to the funding amount.");
        Star.safeTransferFrom(msg.sender, address(this), _amount); 

        emit Funded(msg.sender,_amount,Star.balanceOf(address(this)).sub(totalStarInPools));
    }

    //set Star Pool index
    function setStarId(uint256 _pid) external onlyOwner{
...
//////////// User functions //////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        pool.lpToken.safeTransferFrom(address(msg.sender), address(this), _amount); //transfer the lp from the user to the contract

        if (pool.depositFeeBP > 0) {
            uint256 depositFee = _amount.mul(pool.depositFeeBP).div(100000); //calcualte the fee
            pool.depositFees = pool.depositFees.add(depositFee); //add collected fee amoutn to fee tracker            
    ...
    // Harvest Star rewards
    function harvestStar(uint256 _pid) public nonReentrant{
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
    ...
    function compound(uint256 _pid)public {
        ...
        if (canHarvest(_pid, msg.sender)) {
            if (pending > 0 || user.rewardLockedUp > 0) {
                ...
                // move reward to star pool
                userInfo[starId][msg.sender].amount = userInfo[starId][msg.sender].amount.add(totalRewards);
                totalStarInPools = totalStarInPools.add(totalRewards);
            }
        }else if (pending > 0) {
...
/////////////////// Internal functions //////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    function payOrLockupPendingStar(uint256 _pid,address _to) internal {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        if (canHarvest(_pid, _to)) {
            if (pending > 0 || user.rewardLockedUp > 0) {
                ...
                // send rewards
                safeStarTransfer(_to, totalRewards);
            }
        } else if (pending > 0) {
            user.rewardLockedUp = user.rewardLockedUp.add(pending);
            totalLockedUpRewards = totalLockedUpRewards.add(pending);
    ...
    function safeStarTransfer(address _to, uint256 _amount) internal {
        ...
        if (Star.balanceOf(address(this)) > totalStarInPools) {
            ...
            if (_amount >= starBal) {
                Star.safeTransfer(_to, starBal);
            } else if (_amount > 0) {
                Star.safeTransfer(_to, _amount);
            }
        }
    }
}