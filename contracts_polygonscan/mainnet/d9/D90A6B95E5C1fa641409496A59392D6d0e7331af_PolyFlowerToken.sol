...
//-----------------------------------------------------------------------------
contract PolyFlowerToken is BEP20("PolyFlower Token", "PolyFlower", 10000000000000000000000) {
...
contract MasterChef is Ownable, NonReentrant {
  ...
  //---------------------------------------------------------------------------
  uint256 public constant EMISSION_RATE = 0.001 ether;
  ...
  struct PoolInfo {
    ...
    uint256 accPolyFlowerPerShare;
    uint16 depositFeeBP;
  }
  //---------------------------------------------------------------------------
  PolyFlowerToken public flower;
  ...
  constructor(PolyFlowerToken _flower, address _feeAddress, uint256 _startBlock) public {
    flower = _flower;
 ...
 //---------------------------------------------------------------------------
  function add(uint256 _allocPoint, IBEP20 _lpToken, bool _withUpdate) public ownerOnly isUnique(_lpToken) {
    ...
    poolInfo.push(PoolInfo({lpToken: _lpToken, allocPoint: _allocPoint, lastRewardBlock: lastRewardBlock, accPolyFlowerPerShare: 0, depositFeeBP: depositFee})
    );
  }
  function pendingPolyFlower(uint256 _pid, address _user) external view returns (uint256)  {
    PoolInfo storage pool = poolInfo[_pid];
    UserInfo storage user = userInfo[_pid][_user];
    uint256 share = pool.accPolyFlowerPerShare;
    uint256 pot = pool.lpToken.balanceOf(address(this));
    if (block.number > pool.lastRewardBlock && pot != 0) {
      uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
      uint256 PolyFlowerReward = multiplier.mul(EMISSION_RATE).mul(pool.allocPoint).div(totalAllocPoint);
      share = share.add(PolyFlowerReward.mul(1e12).div(pot));
    }
  ...
  function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
    ...
    updatePool(_pid);
    if (user.amount > 0) {
      uint256 pending = user.amount.mul(pool.accPolyFlowerPerShare).div(1e12).sub(user.rewardDebt);
      if (pending > 0) {
        safePolyFlowerTransfer(msg.sender, pending);
      }
    }
    if (_amount > 0) {
    ...
    user.rewardDebt = user.amount.mul(pool.accPolyFlowerPerShare).div(1e12);
  ...
  function updatePool(uint256 _pid) public {
    ...
    uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
    uint256 PolyFlowerReward = multiplier.mul(EMISSION_RATE).mul(pool.allocPoint).div(totalAllocPoint);
    flower.mint(feeAddress, PolyFlowerReward.div(20));
    flower.mint(address(this), PolyFlowerReward);
    pool.accPolyFlowerPerShare = pool.accPolyFlowerPerShare.add(PolyFlowerReward.mul(1e12).div(lpSupply));
    pool.lastRewardBlock = block.number;
  }
  function updateStartBlock(uint256 _startBlock) external ownerOnly {
    require(startBlock > block.number, "error");
    startBlock = _startBlock;
  }
  function addNativePool(IBEP20 _lpToken) external ownerOnly {
  ...
  function safePolyFlowerTransfer(address _to, uint256 _amount) internal {
    uint256 pot = flower.balanceOf(address(this));
    bool success = false;
    if (_amount > pot) {
      success = flower.transfer(_to, pot);
    } else {
      success = flower.transfer(_to, _amount);
    }
    require(success, "error");
  }
