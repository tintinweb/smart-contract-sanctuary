...
/**
 Author: DokiDoki Dev: Kaki Any questions? please contact Kaki
 Machine for matic. Machine version 2
 */
contract GachaMachineOnMatic is Ownable, ERC1155Holder {
    ...
    // machine type. 0: AZUKI machine, 1: ETH machine.
    ...
    uint256 public ftotalAmountForAgent;
    ...
    constructor(uint256 _machineId, //machine id
                ...
                IMomijiToken _momijiToken, // momiji token address
    ...
    function _setupAccounts() private {
        _devAccountSet.add(0x2990C82208342A6675db26043a7e63Db6EC0B945);
    ...
    function _transferAndBurnToken(uint256 amount) private {
        ...
        if (forAgentRate != 0) {
            ...
            ftotalAmountForAgent = ftotalAmountForAgent.add(forAgentAmount);
        }
    ...
    function _distributePrize() private {
        for (uint i = 0; i < gameRounds[msg.sender].times; i ++) {
            ...
            momijiToken.safeTransferFrom(address(this), msg.sender, cardId, 1, 'Your prize from Degacha');
    ...
    function cleanMachine() public onlyOwner {
        ...
        for (uint256 i = 0; i < cardIdCount(); i ++) {
            ...
            if (amountWithId[cardId] > 0) {
                momijiToken.safeTransferFrom(address(this), owner(), cardId, amountWithId[cardId], "Reset Machine");
