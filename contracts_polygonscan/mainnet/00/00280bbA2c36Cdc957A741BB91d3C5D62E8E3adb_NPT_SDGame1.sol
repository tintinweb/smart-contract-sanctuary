...
contract NPT_SDGame1 is VRFConsumerBase, Pausable, Ownable {
    enum PackTier {
        GOLD
    }
    ...
    constructor(
        ...
        slowMode = true; // active slowmode
        slowModeTime = 1800; // 30 minutes

        packContents[PackTier.GOLD] = 5; // 5 pieces per pack for this drop
        packPrices[PackTier.GOLD] = 0 ether;

        whitelistedAddress[owner()] = true;

        prizeFactory = new PrizeFactory("NPT X SD - Prizes", "NPTXSDPrizes");
        prizeFactory.transferOwnership(owner());

        pieceFactory = new PieceFactory("NPT X SD - Pieces", "NPTXSDPieces");
