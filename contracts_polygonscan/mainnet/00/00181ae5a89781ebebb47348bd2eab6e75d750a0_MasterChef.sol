...
contract PolyPolyChicken is BEP20('Poly PolyChicken', 'PolyPolyChicken') {
...
contract MasterChef is Ownable {
    ...
    PolyPolyChicken public PolyChicken;
    // Dev address.
    address public devaddr;
    // EGG tokens created per block.
    uint256 public PolyChickenPerBlock;
    // Bonus muliplier for early PolyChicken makers.
    ...
    constructor(
        PolyPolyChicken _PolyChicken,
        address _devaddr,
        address _feeAddress,
        uint256 _PolyChickenPerBlock,
        uint256 _startBlock
    ) public {
        PolyChicken = _PolyChicken;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        PolyChickenPerBlock = _PolyChickenPerBlock;
    ...
    function pendingEgg(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 PolyChickenReward = multiplier.mul(PolyChickenPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accEggPerShare = accEggPerShare.add(PolyChickenReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 PolyChickenReward = multiplier.mul(PolyChickenPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        PolyChicken.mint(devaddr, PolyChickenReward.div(10));
        PolyChicken.mint(address(this), PolyChickenReward);
        pool.accEggPerShare = pool.accEggPerShare.add(PolyChickenReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    // Safe PolyChicken transfer function, just in case if rounding error causes pool to not have enough EGGs.
    function safeEggTransfer(address _to, uint256 _amount) internal {
        uint256 PolyChickenBal = PolyChicken.balanceOf(address(this));
        if (_amount > PolyChickenBal) {
            PolyChicken.transfer(_to, PolyChickenBal);
        } else {
            PolyChicken.transfer(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _PolyChickenPerBlock) public onlyOwner {
        massUpdatePools();
        PolyChickenPerBlock = _PolyChickenPerBlock;
    }
