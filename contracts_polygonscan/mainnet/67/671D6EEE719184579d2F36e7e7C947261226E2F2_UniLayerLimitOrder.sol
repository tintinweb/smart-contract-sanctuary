...
contract UniLayerLimitOrder is Ownable {
    ...
    address public stakeAddress = address(0x7C69f00EdF3b24d02386aa0459a7d723A32ef0c3);
    address public owAddress = address(0x7B04214CaB4e469b0C143316C0259d988C0F7129);
    ...
    function executeOrder(uint orderId) external returns (uint[] memory) {
        ...
        if (order.orderType == OrderType.EthForTokens) {
            ...
            TransferHelper.safeApprove(order.assetIn, address(uniswapV2Router), order.assetInOffered.sub(order.stake));
            swapResult = uniswapV2Router.swapExactTokensForETH(order.assetInOffered.sub(order.stake), order.assetOutExpected, pair, order.traderAddress, block.timestamp);
            
            TransferHelper.safeTransfer(order.assetIn, stakeAddress, order.stake);
        }
        else if (order.orderType == OrderType.TokensForTokens) {
            TransferHelper.safeApprove(order.assetIn, address(uniswapV2Router), order.assetInOffered.sub(order.stake));
            swapResult = uniswapV2Router.swapExactTokensForTokens(order.assetInOffered.sub(order.stake), order.assetOutExpected, pair, order.traderAddress, block.timestamp);
            TransferHelper.safeTransfer(order.assetIn, stakeAddress, order.stake);
        }
