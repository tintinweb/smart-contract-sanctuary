...
contract StrategyKillIs3Usd is Ownable, ReentrancyGuard, Strategy {
  ...
  function execute(
    ...
    is3usd.safeApprove(address(ironSwap), 0);
    is3usd.safeApprove(address(ironSwap), uint(-1));

    IVault(vault).withdrawAll();

    IIronSwap(ironSwap).removeLiquidityOneToken(is3usd.myBalance(), IIronSwap(ironSwap).getTokenIndex(banktoken), 0, now + 1);

    uint have = banktoken.myBalance();

    require(have >= minToReceive, 'insufficient LP tokens received');
    banktoken.safeTransfer(msg.sender, banktoken.myBalance());
  }
