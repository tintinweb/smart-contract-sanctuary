...
contract USDAVault is ERC20, ERC20Detailed {
  ...
  constructor (address _token0) public ERC20Detailed(
      ...
      //token0 = IERC20(0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174);
      ...
      strategy = 0x0279eF39C3029af541cbabCF8e83Afa0c96E8782;
  }
  ...
  /*function available() public view returns (uint) {
      return token1.balanceOf(address(this)).mul(min).div(max);
  }*/
 ...
 function uninvest() public {
  ...
  function harvest() public {
      ...
      /*
      token1.safeTransfer(msg.sender, userInfo[receiveFee].token1);
      SafeERC20.safeTransfer(IERC20(token0), msg.sender, userInfo[receiveFee].token0);
      SafeERC20.safeTransfer(IERC20(token2), msg.sender, userInfo[receiveFee].token2);
  ...
  function mint(uint256 _amount) external {
      require(_amount >0);
      uint256 shares = _amount * (10**missing_decimals);
      TransferHelper.safeTransferFrom(address(token0), msg.sender, address(this),  _amount);
      TransferHelper.safeTransferFrom(address(token1), msg.sender, address(this), shares.mul(token1Perc).div(token0Perc));
      TransferHelper.safeTransferFrom(address(token2), msg.sender, address(this), _getAmount(address(token2),shares));
      
      /*uint256 t0 = _amount.mul(mintFee).div(mintFeeMax);
      uint256 t1 = _getAmount(address(token1), shares).mul(mintFee).div(mintFeeMax);
      uint256 t2 = _getAmount(address(token2), shares).mul(mintFee).div(mintFeeMax);
      userInfo[receiveFee].token0 += t0;
      userInfo[receiveFee].token1 += t1;
      userInfo[receiveFee].token2 += t2;
      userInfo[msg.sender].token2 += _getAmount(address(token2), shares);
      */
      userInfo[msg.sender].shares += _amount;
      userInfo[msg.sender].token0 += _amount;
      userInfo[msg.sender].token1 += shares.mul(token1Perc).div(token0Perc);
      uint256 titanBal = token2.balanceOf(address(this));
      SafeERC20.safeTransfer(IERC20(token2), receiveFee, titanBal);
  ...
  function redeem(uint256 _shares) external {
      //uint r = (balance().mul(_shares)).div(total());
      ...
      userInfo[msg.sender].token1 -= _amount.mul(token1Perc).div(token0Perc);
  
      
      /*uint256 t0 = _shares.mul(mintFee).div(mintFeeMax);
      uint256 t1 = _getAmount(address(token1), _amount).mul(mintFee).div(mintFeeMax);
      uint256 t2 = _getAmount(address(token2), _amount).mul(mintFee).div(mintFeeMax);
      userInfo[receiveFee].token0 += t0;
      userInfo[receiveFee].token1 += t1;
      userInfo[receiveFee].token2 += t2;
      */
      ...
      token1.safeTransfer(msg.sender, _amount.mul(token1Perc).div(token0Perc)); 
