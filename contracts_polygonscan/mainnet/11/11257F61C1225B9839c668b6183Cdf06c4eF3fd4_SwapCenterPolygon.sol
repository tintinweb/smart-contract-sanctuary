...
contract SwapCenterPolygon is ISwap, ReentrancyGuard, Ownable {
  ...
  // Exchange ID for converting between MATIC and WMATIC
  uint256 constant WMATIC_ID = type(uint256).max;
  // Exchange ID for depositing into VaultUpgradeable.
  uint256 constant VAULT_DEPOSIT_ID = type(uint256).max -1 ;
  // Exchange ID for swapping with Quick SWap.
  uint256 constant QUICKSWAP_ID = 0;
  ...
  uint256 constant QUICKSWAP_ZAP_IN = 9;

  uint256 constant SUSHISWAP_ZAP_IN = 10;

  uint256 constant QUICKSWAP_RM_LIQ = 11;

  uint256 constant SUSHISWAP_RM_LIQ = 12;

  // We use this address for MATIC.
  ...
  function swapExactTokenIn(address tokenIn, address tokenOut, uint256 amountIn, uint256 minAmountOut) external override payable nonReentrant returns (uint256) {
    ...
    for(uint256 i = 0; i < exchangeLen; i++) {
      ...
      if(exchangeId == QUICKSWAP_ID) {
        _swapExactTokenIn_uniswapType(0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff, curSellPath);
      ...
      } else if (exchangeId == QUICKSWAP_ZAP_IN) {
        _swapZapIn_uniswapType(0xF231be40d73a9E73d859955344A4fF74f448dF34, curSellPath);
      } else if (exchangeId == SUSHISWAP_ZAP_IN) {
        _swapZapIn_uniswapType(0x47b0eC1beA7d8EcC7cF70C3bF82c5f5d15a96b6D, curSellPath);
      } else if(exchangeId == QUICKSWAP_RM_LIQ) {
        _swapRmLiq_uniswapType(0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff, curSellPath);
      } else if(exchangeId == SUSHISWAP_RM_LIQ) {
      ...
      } else if (exchangeId == WMATIC_ID) {
        _swapWMATIC(curSellPath);
      } else {
  ...
  function _swapAddCurveLiquidity(address [] memory path) internal {
    // Limitation: Adding MATIC is not supported.
    ...
    // Try underlying coins
    for(uint256 i=0; i < n_coins; i++){
      address coin = ICurve(currentCurvePool).underlying_coins(i);
      if(curInputToken == coin){
        uint256 amount = IERC20(curInputToken).balanceOf(address(this));
        ...
        if(n_coins==2) {
          uint256[2] memory amounts;
          amounts[i] = amount;
          ICurve(currentCurvePool).add_liquidity(amounts, 0, true);
        } else if (n_coins==3) {
          uint256[3] memory amounts;
          amounts[i] = amount;
          ICurve(currentCurvePool).add_liquidity(amounts, 0, true);
  ...
  function _swapWMATIC(address[] memory path) internal{
    require(path.length == 2, "Unsupported path");
    ...
    if (curInputToken == ETH_ADDRESS && curOutputToken == wmatic_address) {
      uint256 amountIn = address(this).balance;
      IWETH(wmatic_address).deposit{value:amountIn}();
    } else if (curInputToken == wmatic_address && curOutputToken == ETH_ADDRESS) {
      uint256 amountIn = IERC20(curInputToken).balanceOf(address(this));
      IERC20(curInputToken).safeApprove(wmatic_address, 0);
      IERC20(curInputToken).safeApprove(wmatic_address, amountIn);
      IWETH(wmatic_address).withdraw(amountIn);
    } else {
  ...
  function _swapZapIn_uniswapType(address zap, address[] memory path) internal {
    require(path.length == 2, "Unsupported path");
    ...
    uint256 amountIn = IERC20(curInputToken).balanceOf(address(this));
    IERC20(curInputToken).safeApprove(zap, 0);
    IERC20(curInputToken).safeApprove(zap, amountIn);

    
    IZapIn(zap).ZapIn(
      curInputToken, 
      curOutputToken,
      IERC20(curInputToken).balanceOf(address(this)),
      0,
...
function _swapRmLiq_uniswapType(address router, address[] memory path) internal {
    require(path.length == 2, "Unsupported path");
    ...
    address token0 = IUniswapV2Pair(curInputToken).token0();
    address token1 = IUniswapV2Pair(curInputToken).token1();
    ...
    // Convert the other token to the output token.
    address[] memory path = new address[](2);
    if (curOutputToken ==  token0){
      path[0] = token1;
      path[1] = token0;
    } else if (curOutputToken == token1) {
      path[0] = token0;
      path[1] = token1;
    } else {
      revert();
  ...
  /// Rescue MATIC from the SwapCenter.
...
interface ICurve {
  ...
  function underlying_coins(uint256 i) view external returns (address) ;
  ...
  function add_liquidity(uint256[4] memory amounts, uint256 min_mint_amount, bool _use_underlying) external;
  function add_liquidity(uint256[5] memory amounts, uint256 min_mint_amount, bool _use_underlying) external;
}
...
interface IZapIn {
  function ZapIn ( address _FromTokenContractAddress, address _pairAddress, uint256 _amount, uint256 _minPoolTokens, address _swapTarget, bytes memory swapData, address affiliate, bool transferResidual, bool shouldSellEntireBalance ) external returns ( uint256 );
}
