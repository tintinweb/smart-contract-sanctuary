...
contract PolyPonzi is Context, IBEP20, Ownable {
    ...
    uint256 private constant _tTotal = 69 * 10**9 * 10**18;
    ...
    uint256 public _maxTxAmount = 69 * 10**7 * 10**18; // Max Transaction: (1%)
    uint256 public _numTokensSellToAddToLiquidity = 61 * 10**9 * 10**18;
    uint256 public _maxWalletToken = 207 * 10**7 * 10**18; // Max Wallet:  (3%)

    IPancakeRouter02 public immutable pcsV2Router;
    address public immutable pcsV2Pair;

    string private _name = "PolyPonzi";
    ...
    constructor(address router, address token,address payable addr1) public {
    ...
    function _getAntiDumpMultiplier() private view returns (uint256) {
        ...
        if (time_since_start < 24 * hour) {
            return (4);
        } else if (time_since_start < 36 * hour) {
            return (3);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 half = contractTokenBalance.div(8);
        uint256 donation = contractTokenBalance.sub(half);
        uint256 halfliq =half.div(2);
        uint256 otherHalf = half.sub(halfliq);
        ...
        swapTokensForMATIC(halfliq);

        swapTokensForUSDC(donation);
