...
contract TokenPrice {
    ...
    function getPrice(uint amountIn, address[] memory path) external view returns (uint[2] memory prices)
    {
        if (address(path[0]) == WETH)
        {
            prices = [dfynRouter.getAmountsOut(amountIn, path)[1], 1];
        }
        else
        {
            prices = [sushiRouter.getAmountsOut(amountIn, path)[1], 2];
        }
    }
}