...
contract MasterChef is Ownable {
    ...
    function pendingEgg(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accEggPerShare = accEggPerShare.add(PANTHERReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 PANTHERReward = multiplier.mul(PANTHERPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        PANTHER.mint(devaddr, PANTHERReward.div(10));
        PANTHER.mint(address(this), PANTHERReward);
        pool.accEggPerShare = pool.accEggPerShare.add(PANTHERReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    // Safe PANTHER transfer function, just in case if rounding error causes pool to not have enough EGGs.
