...
contract Campaign is ReEntrancyGuard {
   ...
   // Token claiming by users
    ...
    function buyTierTokens(uint256 value) external noReentrant {
        ...
        collectedMATIC = collectedMATIC.add(value);
    ...
    function addAndLockLP() internal {
        ...
        if ((lpMATICQty > 0 && lpTokenQty > 0) && !liquidityCreated) {
            ...
            (uint256 retTokenAmt, uint256 retMATICAmt, uint256 retLpTokenAmt) = IUniswapV2Router02(lpRouterAddress).addLiquidity(
                address(token),
                address(payToken),
                lpTokenQty,
                lpMATICQty,
    ...
    function recoverUnspentLp() external onlyFactory {
        ...
        if (maticAmt > 0) {
            ...
            payToken.safeTransfer(campaignOwner, maticAmt);
        }
    ...
    function finishUp() external {
        ...
        payToken.safeTransfer(campaignOwner, remainMATIC);
...
contract Factory is IFactoryGetters, Ownable {
    ...
    {
        payToken = 0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174; // USDC 
    ...
    function registerForIDOByFactory(
        ...
        camp.registerForIDOByFactory(_accounts, _tiers, _tokenLockTime);
    }
