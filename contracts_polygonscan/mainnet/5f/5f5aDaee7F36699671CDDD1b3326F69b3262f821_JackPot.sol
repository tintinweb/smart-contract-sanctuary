    ...
    VRFConsumerBaseUpgradeable
{
    ...
    // player struct to help with resolve winner algo
    struct PlayerInfo {
        address player;
        uint128 bet; // bet for this player //change the betting amount to be only uint128 to comply with the vault
        uint128 accBet; // accumulate bet for this player //change the betting amount to be only uint128 to comply with the vault
    }

    mapping(address => uint8) public playerIndex; //newly added to reduce the gas in increaseBet function
    ...
    uint8 public feeRate; //newly added, apply fee rate to the game.
    uint8 playerNum;

    GameState public state;
    uint128 public minBet;
    uint128 public maxBet;
    uint128 public jackPot; // variable to track current jackpot size; //change to uint128 to comply with the Vault.
    ...
    uint256 public waitingTimeBeforeExpired; // any suggest for better name?
    uint256 internal fee;

    bytes32 internal keyHash;
    bytes32 public constant VAULT_ROLE = keccak256("VAULT_ROLE"); //newly added to use vault instead of directly transfer punt 20sep

    IVault public vault; //newly added to use vault instead of directly transfer punt 20sep
    ...
    event FeeRate(uint8 prevFeeRate, uint8 indexed feeRate);
    event MinMaxBet(uint128 indexed _minBet, uint128 indexed _maxBet);
    event IncreaseBet(
        uint256 indexed gId,
        address indexed player,
        uint8 playerId,
    ...
    function initialize(
        address _vaultAddress, //newly added to use vault instead of directly transfer punt 20sep
        uint8 _minPlayers,
        uint8 _maxPlayers,
        uint128 _minbet,
        uint128 _maxbet, //newly added, not sure if the stacks are too dep after adding this.
    ...
    function setWaitingTimeGame(
        ...
        require(_waitingTime != 0, "JackPot.sol: WaitingTime can't be 0");
        require(
            _waitingTimeBeforeExpired != 0,
            "JackPot.sol: WaitingTimeBeforeExpired can't be 0"
        );
        waitingTime = _waitingTime;
        waitingTimeBeforeExpired = _waitingTimeBeforeExpired;
    }

    function setMaxMinPlayers(uint8 _min, uint8 _max)
        public
        onlyOwner
        whenPaused
    {
        require(_max != 0, "JackPot.sol: Max players can't be 0");
        require(
            _max <= 255,
            "JackPot.sol: Max players can't be more than 255 due to the Maximum of playerIndex"
        );
        minPlayers = _min;
        maxPlayers = _max;
    }

    //don't use the direct token transfer anymore. Use vault's function instead.
    // setup token address
    // function setPlayingToken(address _tokenAddress) external onlyOwner {
        ...
        whenPaused
    {
        ...
        minBet = _minBet;
        maxBet = _maxBet;
        emit MinMaxBet(_minBet, _maxBet);
    }
        ...
        whenNotPaused
    {
        ...
        if (playerIndex[msg.sender] == 0) {
            ...
            //due to the vault only allows uint128 so we comply.
            vault.addGameBalance(msg.sender, _betAmount); //newly added to use vault instead of directly transfer punt 20sep

            jackPot += _betAmount; // @PuntPM I suspect re-entrancy attack here
            ...
            players.push(
                PlayerInfo({
                    ...
                    bet: _betAmount,
                    accBet: jackPot
                }) //@PuntPM suspect re-entrancy attack here with jackPot+= betAmount
            );

            //to collect the player's index in players array to reduce the gas used in for loop.
            ...
            require(
                ...
                "you can not join the game once the room is full"
            );
    ...
    //remove nonreentrant because joinGame already implemented
    function increaseBet(address _player, uint128 _betAmount)
        internal
        onlyOngoingGame
    {
        require(
        ...
        require(
            ...
            "JackPot.sol:increaseBet, Game has already started."
        );

        vault.addGameBalance(_player, _betAmount); //newly added to use vault instead of directly transfer punt 20sep
    ...
    /// @dev I might misunderstand this please help review and confirm
    ...
    {
        //use the vault.subGameBalance instead of using fundWithdrawal
        ...
        override
    {
        ...
        delete gameStartTime;
        delete jackPot;
        //foundWinner = false; //newly added to track the winner founding.
