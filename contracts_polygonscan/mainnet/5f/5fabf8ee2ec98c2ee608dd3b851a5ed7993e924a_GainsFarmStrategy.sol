...
contract GainsFarmStrategy is BaseStrategy {

    uint256 internal constant keepMax = 10000;

    address internal constant GNS = 0xE5417Af564e4bFDA1c483642db72007871397896;
    ...
    // Uniswap swap paths
    address[] internal gns_dai_path;
    address[] internal gns_wmatic_path;
    address[] internal quick_wmatic_path;
    address[] internal quick_gns_path;

    constructor()
        public
        BaseStrategy(
            0x6E53cB6942e518376E9e763554dB1A45DDCd25c4, //GNS/DAI LP
    ...
    {
        gns_dai_path = [GNS, DAI];
        gns_wmatic_path = [GNS, DAI, WETH, WMATIC];
        quick_wmatic_path = [QUICK, WMATIC];
        quick_gns_path = [QUICK, WETH, DAI, GNS];

        IERC20(want).safeApprove(rewards, uint256(-1));

        IERC20(DAI).safeApprove(currentRouter, uint256(-1));
        IERC20(GNS).safeApprove(currentRouter, uint256(-1));
    ...
    function balanceOfPool() public override view returns (uint256) {
        (uint256 provided,,,,,) = IFarmPool(rewards).users(address(this));
        return provided;
    }

    function getHarvestable() external override view returns (uint256) {
        return 0; //IFarmPool(rewards).pendingReward();
    }
    ...
    function harvest() public override onlyHumanOrWhitelisted nonReentrant {
        //Transfer WMATIC/GNS/QUICK/DQUICK that may already be in the contract to the fee dist fund
        uint256 h_bal = IERC20(GNS).balanceOf(address(this));
        if(h_bal > 0) {
            IERC20(GNS).safeTransfer(strategist, h_bal);
        }
        ...
        //Deduct perf fee from harvested GNS
        uint256 gnsHarvested = IERC20(harvestedToken).balanceOf(address(this));
        if (gnsHarvested > 0) {
            uint256 feeAmount = gnsHarvested.mul(IERCFund(strategist).getFee()).div(keepMax);
            swapRewardToWmaticAndDistributeFee(feeAmount, gns_wmatic_path);
        }
        ...
        //Deduct perf fee from harvested QUICK
        uint256 quickHarvested = IERC20(QUICK).balanceOf(address(this));
        if (quickHarvested > 0) {
            uint256 feeAmount = quickHarvested.mul(IERCFund(strategist).getFee()).div(keepMax);
            swapRewardToWmaticAndDistributeFee(feeAmount, quick_wmatic_path);
        }

        //Swap all remaining QUICK for GFARM
        uint256 remainingQuick = IERC20(QUICK).balanceOf(address(this));
        if (remainingQuick > 0) {
            _swapUniswapWithPathPreapproved(quick_gns_path, remainingQuick);
        }
        //Swap 1/2 of GNS for DAI
        uint256 gnsBal = IERC20(GNS).balanceOf(address(this));
        if (gnsBal > 0) {
            _swapUniswapWithPathPreapproved(gns_dai_path, gnsBal.div(2));
        }

        uint256 gnsBalance = IERC20(GNS).balanceOf(address(this));
        uint256 daiBalance = IERC20(DAI).balanceOf(address(this));
        if (gnsBalance > 0 && daiBalance > 0) {
            IUniswapRouterV2(currentRouter).addLiquidity(
                harvestedToken, DAI,
                gnsBalance, daiBalance,
                0, 0,
                address(this),
                now + 60
            );
    ...
    function _withdrawSome(uint256 _amount)
        ...
        returns (uint256)
    {
        IFarmPool(rewards).unstake(_amount);
        return _amount;
    }
    ...
    function deposit() public override {
        uint256 _want = IERC20(want).balanceOf(address(this));
        if (_want > 0) {
            IFarmPool(rewards).stake(_want, 0x8AcBb9cBc2E3A8286e2A0f5610470cf81F541764);
        }
    }
