...
contract ZeldaPresale is ReentrancyGuard {
	...
	function claimTokens(address _claimer) external nonReentrant {
		...
		require(COIN.balanceOf(address(this)) >= tokensUnclaimed[_claimer], "There are not enough KUNAI tokens to transfer.");
	...
	function tokenLeft() public view returns(uint256){
		...
		return (maxHardCap - usdcReceived).div(10 ** decimals);
	}
}