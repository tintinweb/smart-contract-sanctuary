...
contract LeoPolygonMainStaking is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. Cub to distribute per block.
        uint256 lastRewardBlock; // Last block number that Cub distribution occurs.
    ...
    uint256 public ownerTokenReward = 0; // 0%

    uint256 public tokensPerBlock = 8000000000000000; // reward tokens created per block
    ...
    // store data about locked tokens
    ...
    //contract to distribute penalty to stakers
    address public PENALTY_ADDRESS = 0x000000000000000000000000000000000000dEaD;
    uint256 public LOCKUP_PERIOD_BLOCKS = 2629743; //~3 months
    ...
    event DepositedFundingToken(address _token, uint256 _amount);
    event UpdatedFundingPid(uint256 _newPid);
    ...
    function updatePool(uint256 _pid) public {
        if (emissionScheduleArray.length >= emissionScheduleLatest + 1){
          ...
          if (emissionScheduleArray[emissionScheduleLatest + 1].startBlock <= block.number){
            tokensPerBlock = emissionScheduleArray[emissionScheduleLatest + 1].amount;
        ...
        pending[msg.sender].push(pendingRewards(tokensReward, block.number + LOCKUP_PERIOD_BLOCKS));
    ...
    //claim lockd rewards
    function claim(bool _all) external nonReentrant {
      uint256 sumLocked;
      uint256 sumUnlocked;
      for (uint256 i = 0; i < pending[msg.sender].length; i++){
        if (pending[msg.sender][i].unlockBlock <= block.number){
          sumUnlocked.add(pending[msg.sender][i].amount);
          if (!_all){
            delete pending[msg.sender][i];
          }
        } else {
          sumLocked.add(pending[msg.sender][i].amount);
        }
      }

      if (_all){
        uint256 totalAmount = sumUnlocked.add(sumLocked.div(2));
        ...
        IToken(rewardToken).mint(msg.sender, totalAmount);
        IToken(rewardToken).mint(PENALTY_ADDRESS, penalty);
      } else {
        IToken(rewardToken).mint(msg.sender, sumUnlocked);
      }
    }

    //in case if gas limit is lower than gas required to claim entire array of pendingClaims
    function claimOnePending(uint256 index) external {
      uint256 amount;
      uint256 penalty;
      if (pending[msg.sender][index].unlockBlock <= block.number){
        amount = pending[msg.sender][index].amount;
      } else {
        amount = pending[msg.sender][index].amount.div(2);
        penalty = pending[msg.sender][index].amount.div(2);
      }

      delete pending[msg.sender][index];

      IToken(rewardToken).mint(msg.sender, amount);
      IToken(rewardToken).mint(PENALTY_ADDRESS, penalty);
    }

    //get sum of all unlocked tokens
    function unlockedTokens() external view returns (uint256) {
      uint256 sumUnlocked;
      for (uint256 i = 0; i < pending[msg.sender].length; i++){
        if (pending[msg.sender][i].unlockBlock <= block.number){
          sumUnlocked.add(pending[msg.sender][i].amount);
    ...
    function deposit(uint256 _pid, uint256 _wantAmt) public nonReentrant {
        ...
        if (user.shares > 0) {
            ...
            if (_pending > 0) {
                safeTokensTransfer(msg.sender, _pending);
            }
        }
        if (_wantAmt > 0) {
    ...
    function setLockupPeriod(uint256 _lockup) external onlyOwner {
      emit UpdateLockupPeriod(LOCKUP_PERIOD_BLOCKS, _lockup);
      LOCKUP_PERIOD_BLOCKS = _lockup;
    }

    function setPenaltyAddress(address _newPenaltyAddress) external onlyOwner {
      PENALTY_ADDRESS = _newPenaltyAddress;
