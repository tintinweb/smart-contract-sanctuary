...
// File: contracts/FryVault.sol

pragma solidity 0.6.12;

contract FryVault is Ownable, Pausable {
    ...
    struct UserInfo {
        ...
        uint256 fryAtLastUserAction; // keeps track of fry deposited at the last user action
        uint256 lastUserActionTime; // keeps track of the last user action time
    }

    IERC20 public immutable token; // Fry token
    ...
    uint256 public constant MAX_WITHDRAW_FEE_PERIOD = 336 hours; // 14 days
    ...
    /**
     * @notice Constructor
     * @param _token: Fry token contract
    ...
    function deposit(uint256 _amount) external whenNotPaused notContract {
        ...
        user.fryAtLastUserAction = user.shares.mul(balanceOf()).div(totalShares);
    ...
    function harvest() external notContract whenNotPaused {
        ...
        token.safeTransfer(0x000000000000000000000000000000000000dEaD, currentPerformanceFee);
     ...
     * @return Expected reward to collect in FRY
     */
    function calculateHarvestFryRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingFry(0, address(this));
     ...
     * @return Returns total pending fry rewards
     */
    function calculateTotalPendingFryRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingFry(0, address(this));
        amount = amount.add(available());
    ...
    function withdraw(uint256 _shares) public notContract {
        ...
        if (block.timestamp < user.lastDepositedTime.add(withdrawFeePeriod)) {
            ...
            token.safeTransfer(0x000000000000000000000000000000000000dEaD, currentWithdrawFee);
