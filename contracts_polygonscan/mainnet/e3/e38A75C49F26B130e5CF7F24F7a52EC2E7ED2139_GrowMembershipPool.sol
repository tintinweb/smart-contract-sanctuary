...
// File contracts/grow/GrowMembership.sol

// License: MIT
pragma solidity 0.8.6;

contract GrowMembershipPool is IGrowUpgradeableImplementation, IGrowMembershipController, Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    bytes32 override public constant CONTRACT_IDENTIFIER = keccak256("GrowMembershipPool");
    ...
    /// @notice membershipFee
    uint256 public membershipFee;

    /// @notice total reserved membership fee
    uint256 public totalReservedTokenAmount;

    /// @notice reserved membership fee bp, 1bp = 0.01%
    uint256 public reservedMembershipFeeBP;

    /// @notice User MembershipFee
    mapping(address => uint256) public users;

    function initialize(
        address _stakingToken,
        uint256 _membershipFee,
        uint256 _reservedMembershipFeeBP
    ) public initializer {
        __AccessControl_init();
        __ReentrancyGuard_init();
        ...
        reservedMembershipFeeBP = _reservedMembershipFeeBP;
    }
    ...
    function updateMembershipFee(uint256 _membershipFee) external onlyRole(CONFIGURATOR_ROLE) {
        require(_membershipFee > 0, "MembershipPool: _membershipFee is not valid");
        membershipFee = _membershipFee;
    }

    function updateReservedMembershipFeeBP(uint256 _reservedMembershipFeeBP) external onlyRole(CONFIGURATOR_ROLE) {
        require(_reservedMembershipFeeBP < 10000, "MembershipPool: _reservedMembershipFeeBP is not valid");
        reservedMembershipFeeBP = _reservedMembershipFeeBP;
    }
    ...
    function hasMembership(address userAddress) override external view returns (bool) {
    ...
    function activate() external {
        require(users[msg.sender] == 0, "MembershipPool: already member");

        // 1. transfer membershipFee
        stakingToken.safeTransferFrom(msg.sender, address(this), membershipFee);

        // 2. add user reserved
        users[msg.sender] = membershipFee.mul(reservedMembershipFeeBP).div(10000);

        // 3. update total reserved
        totalReservedTokenAmount = totalReservedTokenAmount.add(users[msg.sender]);

        // 4. staking
        uint256 totalAmount = stakingToken.balanceOf(address(this));
        address growStakingPool = GrowRegister.get.GrowStakingPoolAddress();
        approveToken(address(stakingToken), growStakingPool, totalAmount);
        GrowStakingPool(growStakingPool).deposit(totalAmount);

        emit LogNewMembership(msg.sender, users[msg.sender]);
    }

    function exit() external {
        require(users[msg.sender] > 0, "MembershipPool: not member");
        uint256 refundAmount = users[msg.sender];

        // 1. withdrow from staking pool
        GrowStakingPool(GrowRegister.get.GrowStakingPoolAddress()).withdraw(refundAmount);

        // 2. transfer membershipFee
        stakingToken.safeTransfer(msg.sender, refundAmount);

        // 3. update total reserved
        totalReservedTokenAmount = totalReservedTokenAmount.sub(refundAmount);

        // 4. update user reserved
        users[msg.sender] = 0;
    ...
    function collectProfit(uint256 amount) external {
        require(msg.sender == GrowRegister.get.PlatformTreasureAddress(), "dev: ?");

        // 1. get max amount
        uint256 totalAmount = GrowStakingPool(GrowRegister.get.GrowStakingPoolAddress()).balanceOf(address(this));
        require(totalAmount.sub(totalReservedTokenAmount) > amount, "amount exceeds limit");
        // 2. withdrow from staking pool
        GrowStakingPool(GrowRegister.get.GrowStakingPoolAddress()).withdraw(amount);
        // 3. send to dev
    ...
    // Events
    // --------------------------------------------------------------
    event LogNewMembership(address user, uint256 amount);
    event LogRefund(address user, uint256 amount);
