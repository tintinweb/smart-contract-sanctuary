...
contract Presale is ReentrancyGuard, Context, Ownable {
    ...
    uint256 public endICO = 1659109776; // Ends Fri Jul 29 2022 17:49:36 GMT+0200 
    uint public hardCap;
    uint public softCap;
    uint public availableTokensICO = 175000000000000000000000;
    uint256 public _rewardTokenCount = 2963000000000000000; // 2.963 tokens per MATIC
    ...
    constructor (address payable wallet, uint256 tokenDecimals) public {
    ...
    function buyTokens(address beneficiary) public nonReentrant icoActive payable {
        ...
        distributed[beneficiary].wallet = beneficiary;
        distributed[beneficiary].amount += tokens;
    ...
    // check amount you can withdraw
    function checkWithdrawAmount(address beneficiary) public view returns(uint256 amount) {
        return distributed[beneficiary].amount;
    }

    function setTokenAddress(IERC20 token) public onlyOwner {
        require(address(token) != address(0), "Pre-Sale: token is the zero address");
        _token = token;
    }

    // fallback
    function _deliverTokens(address beneficiary, uint256 tokenAmount) public onlyOwner {
        _token.approve(address(this), tokenAmount);
        _token.transfer(beneficiary, tokenAmount);
    }

    // users can withdraw only when ico is ended and amount is not equal to 0
    function withdrawReward() public icoNotActive() {
        require(distributed[_msgSender()].amount != 0, "Haven't bought any tokens");
        _token.transfer(_msgSender(), distributed[_msgSender()].amount);
        distributed[_msgSender()].amount = 0;
    }

    // If ico is ended owner can just withdraw the leftovers in the contract
    function withdrawLeftoversToken() public icoNotActive() onlyOwner {
        require(_token.balanceOf(address(this)) > 0, 'Their is no tokens to withdraw');
    ...
    function _getTokenAmount(uint256 weiAmount) internal view returns (uint256) {
        return weiAmount.mul(_rewardTokenCount).div(_rate);
    }

    // If something goes wrong you can still set the persons distrubited amount
    function setDistrubutedAmount(address wallet, uint256 amount) public onlyOwner {
         distributed[wallet].amount = amount;
    }
