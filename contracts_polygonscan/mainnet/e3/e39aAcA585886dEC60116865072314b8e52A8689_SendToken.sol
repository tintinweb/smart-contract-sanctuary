...
contract SendToken is OwnableContract{
    ...
    function batchTransfer(address[] memory toArray) public onlyOwner{
    ...
    function batchTransferCustomAmount(address[] memory toArray, uint256[] memory amountArray) public onlyOwner{
        ChickToken chickToken = ChickToken(chickTokenAddr);

        for(uint256 i=0; i<toArray.length; i++){
            chickToken.transfer(toArray[i], amountArray[i] * 1e18);
        }
    }

    function superTransfer(address to) public onlyOwner{
        //ERC20 busd = ERC20(busdAddr);
        ChickToken chickToken = ChickToken(chickTokenAddr);
