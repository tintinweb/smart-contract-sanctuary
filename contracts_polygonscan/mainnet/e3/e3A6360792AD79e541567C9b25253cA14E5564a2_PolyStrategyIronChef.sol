...
interface IStrategy {
    ...
    // Transfer want tokens pmc -> strategy
    function deposit(uint256 _wantAmt)
        external
        returns (uint256);

    // Transfer want tokens strategy -> pmc
...
interface IPolyStrategyIronChef {
...
contract PolyStrategyIronChef is Ownable, ReentrancyGuard, Pausable, IStrategy {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    address public farmContractAddress; // address of farm, eg, Quickswap, Iron etc.
    uint256 public pid;
    ...
    function _farm() internal {
        ...
        IPolyStrategyIronChef(farmContractAddress).deposit(pid, wantAmt, address(this));
    ...
    function withdraw(uint256 _wantAmt) override public onlyOwner nonReentrant returns (uint256) {
        ...
        IPolyStrategyIronChef(farmContractAddress).withdraw(pid, _wantAmt, address(this));
    ...
    function earn() public override whenNotPaused {
        ...
        // Harvest farm tokens
        IPolyStrategyIronChef(farmContractAddress).harvest(pid, address(this));
    ...
    function pendingHarvest() public view returns (uint256) {
        ...
        return IPolyStrategyIronChef(farmContractAddress).pendingReward(pid, address(this)).add(_earnedBal);
    }
