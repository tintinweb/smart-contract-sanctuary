...
contract CrucibleRouter is MultiSigCheckable, HasTaxDistributor {
    ...
    mapping(address => uint16) public delegatedGroupIds;
		...
		function upgradeRouter(address crucible, address newRouter) external onlyOwner {
    ...
    function setOpenCap(
        ...
        bytes32 message = keccak256(
            abi.encode(SET_OPEN_CAP, crucible, cap, expiry)
        );
    ...
    function depositAndStake(
        ...
        address token = ICrucibleToken(crucible).baseToken();
        SafeAmount.safeTransferFrom(token, msg.sender, crucible, amount);
        ICrucibleToken(crucible).deposit(stake);
        IStakeFor(stake).stakeFor(crucible, to);
    }
    ...
    /**
     * @dev If multiSig is not provided, we will try the open deposit
     */
    function depositAddLiquidityStake(
        ...
        address pool;
        {
            ...
            pool = IUniswapV2Factory(IUniswapV2Router01(ammRouter).factory())
                .getPair(pairToken, baseToken);
            if (pool == address(0)) {
                pool = IUniswapV2Factory(
                    IUniswapV2Router01(ammRouter).factory()
                ).createPair(pairToken, baseToken);
            }
        }
    ...
    bytes32 constant OVERRIDE_FEE_METHOD =
        keccak256(
            "OverrideFee(address crucible,address target,uint8 overrideType,uint64 newFeeX10000,uint64 expiry)"
        );
...
abstract contract MultiSigCheckable is WithAdmin, EIP712 {
    ...
    uint16 public constant GOVERNANCE_GROUP_ID_MAX = 256;
		...
		/*
		 * @dev: Override this to allow public creatig new quorums.
    ...
    function _initialize(
		...
		// @dev: to allow last resort option in case a quorum
    ...
    bytes32 constant REMOVE_FROM_QUORUM_METHOD =
        keccak256("RemoveFromQuorum(address _address,bytes32 salt)");
    function removeFromQuorum(
        ...
        require(q.id != address(0), "MSC: subscription not found");
        bytes32 message = keccak256(
            abi.encode(REMOVE_FROM_QUORUM_METHOD, _address, salt)
        );
        uint64 expectedGroupId = q.ownerGroupId != 0 ? q.ownerGroupId : q.groupId;
    ...
    function _removeFromQuorum(address _address, address qId) internal {
        ...
        quorumsSubscribers[qId] = quorumsSubscribers[qId].sub(1);
        emit RemovedFromQuorum(qId, _address);
		}

    bytes32 constant ADD_TO_QUORUM_METHOD =
        keccak256(
            "AddToQuorum(address _address,address quorumId,bytes32 salt)"
        );
    function addToQuorum(
        ...
        require(salt != 0, "MSC: salt required");
        bytes32 message = keccak256(
            abi.encode(ADD_TO_QUORUM_METHOD, _address, quorumId, salt)
        );
		...
		/**
		 * @dev Remove this method if public can create groupIds.
    ...
    function tryVerifyDigest(
        ...
        bytes memory multiSignature
			...
			require(multiSignature.length != 0, "MSC: multiSignature required");
			MultiSigLib.Sig[] memory signatures = MultiSigLib.parseSig(multiSignature);
			require(signatures.length > 0, "MSC: no zero len signatures");

			address _signer =
				ECDSA.recover(digest, signatures[0].v, signatures[0].r, signatures[0].s);
			...
			for (uint256 i = 1; i < signatures.length; i++) {
					...
					_signer = ECDSA.recover(digest, signatures[0].v, signatures[i].r, signatures[i].s);
    ...
    */
    ...
    modifier governanceGroupId(uint16 expectedGroupId) {
        require(expectedGroupId < GOVERNANCE_GROUP_ID_MAX, "MSC: must be governance");
        _;
    }
}
