...
abstract contract StrategyFarmTwoAssets is BaseStrategyMasterChef {
    ...
    address public constant xmark = 0xf153EfF70DC0bf3b085134928daeEA248d9B30d0;
    ...
    address[] public usdc_xmark_path;
        ...
        )
    {
        ...
        usdc_xmark_path = new address[](2);
        usdc_xmark_path[0] = usdc;
        usdc_xmark_path[1] = xmark;
    }
    ...
    function harvest() public override {
        ...
        if (_dinoBalance > harvestCutoff){
            ...
            _swapUniswapWithPath(usdc_xmark_path, _usdcBalancePost.div(2));

            // Add liquidity
            uint256 _usdc = IERC20(usdc).balanceOf(address(this));
            uint256 _xmark = IERC20(xmark).balanceOf(address(this));
            if (_usdc > 0 && _xmark > 0) {
                IERC20(usdc).safeApprove(currentRouter, 0);
                IERC20(usdc).safeApprove(currentRouter, _usdc);
                IERC20(xmark).safeApprove(currentRouter, 0);
                IERC20(xmark).safeApprove(currentRouter, _xmark);
                ...
                IERC20(xmark).safeTransfer(
                    strategist,
                    IERC20(xmark).balanceOf(address(this))
                );
            }
        }
        // Stake the LP tokens
...
contract StrategyTwoAssets is StrategyFarmTwoAssets {
    ...
    address public USDC_XMark_LP = 0x97A95deb56d689802F02f50c25EBCda5d0A49591;
