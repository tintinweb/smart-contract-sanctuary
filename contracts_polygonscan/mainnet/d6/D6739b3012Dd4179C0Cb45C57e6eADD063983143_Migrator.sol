...
contract Migrator is AccessControl {
    ...
    function migrate(address userAddress, uint256[] calldata amount, uint256 expireBlock, uint256 migrationStatus, bytes calldata sigs)
        public 
    {
        require(hasRole(ORACLE_ROLE, msg.sender), "Migrator::Caller should be oracle");
        require(startBlock < block.number, "Migrator::migrate: Migrator is not started yet");
        require(userAddress != address(0), "Migrator::migrate: Zero address detected");
        require(usersStatus[userAddress] & migrationStatus == 0, "Migrator::migrate: you have used this option before");
        require(amount.length <= tokensAddress.length, "Migrator::migrate: Amount is not valid");
        require(expireBlock >= block.number, "Migrator::migrate: Signature is expired");
        require(migrationStatus <= migrationStatusBound, "Migrator::migrate: Migration status is invalid");

        bytes32 _hash = keccak256(abi.encodePacked(userAddress, expireBlock, migrationStatus, getChainID(), amount));
		verify(_hash.toEthSignedMessageHash(), sigs);

        bytes32 hashKey = keccak256(abi.encodePacked(userAddress, amount, migrationStatus));
        if(!isSigned[hashKey][msg.sender]){
            isSigned[hashKey][msg.sender] = true;
            signatures[hashKey].oracles.push(msg.sender);
            signatures[hashKey].expireBlocks.push(expireBlock);
        }
        else{
            for (uint256 index = 0; index <  signatures[hashKey].oracles.length; ++index){
                if(signatures[hashKey].oracles[index] == msg.sender){
                    signatures[hashKey].expireBlocks[index] = expireBlock;
                }
            }
        }
        
        emit UpdateSignatures(userAddress, amount, expireBlock, migrationStatus, msg.sender);
        if(signatures[hashKey].oracles.length >= minimumRequiredSignature){
            uint256 expBlock = type(uint256).max;
            for(uint256 index = 0; index < signatures[hashKey].oracles.length; ++index){
                if(expBlock > signatures[hashKey].expireBlocks[index]){
                    expBlock = signatures[hashKey].expireBlocks[index];
                }
            }
            require(expBlock >= block.number, "Migrator::migrate: The number of valid signatures are not sufficent");
            for (uint256 index = 0; index < amount.length; ++index) {
                if (amount[index] != 0) {
                    IERC20(tokensAddress[index]).transfer(userAddress, amount[index]);
                }
            }
            usersStatus[userAddress] = usersStatus[userAddress] | migrationStatus;
            emit Migrate(userAddress, amount, migrationStatus);
        }
    }
		...
		public
	{
		...
		emit SetMinimumRequiredSignature(_minimumRequiredSignature);
	}
    ...
    event SetMinimumRequiredSignature(uint256 minimumRequiredSignature);
    ...
    event UpdateSignatures(address userAddress, uint256[] amount, uint256 expireBlock, uint256 migrationStatus, address oracle);
