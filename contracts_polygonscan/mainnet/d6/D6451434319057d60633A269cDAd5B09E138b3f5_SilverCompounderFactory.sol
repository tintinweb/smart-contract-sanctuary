...
//TODO adds stops so that we can control when people enter the vault, cuz AFK can set an early withdraw fee
contract SilverCompounderFactory is Ownable{
    ...
    **/
    ...
    function updateFirstSwap(address _new) external onlyOwner{
        firstSwap = _new;
    }
    ...
    **/
    function harvestCompounding(uint poolId, uint[6] memory minAmounts) external compounderExists(poolId){
    ...
    **/
    function _reinvest(uint poolId, uint amountToReinvest, uint[6] memory minAmounts) internal returns(uint callerReward){
        IERC20 DepositToken = IERC20(poolIdToShareInfo[poolId].depositToken);
        IERC20 RewardToken = IERC20(poolIdToShareInfo[poolId].rewardToken);//could also do Chef.farmInfo.poolId, rewardToken....
        ...
        if (poolIdToShareInfo[poolId].depositToken != poolIdToShareInfo[poolId].rewardToken){
            ...
            if (poolIdToShareInfo[poolId].lpFarm){//Dealing with an LP farm so swap half the reward for deposit and supply liqduity
                ...
                require(amountToReinvest >= minAmounts[5], 'Min LP tokens not met');
                ...
                if(firstSwap != poolIdToShareInfo[poolId].depositToken){
                    path[0] = firstSwap;
