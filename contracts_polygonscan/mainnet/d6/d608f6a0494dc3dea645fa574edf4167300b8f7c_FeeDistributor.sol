...
contract FeeDistributor is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    /// @notice swap all tokens to this one
    address public target;

    mapping(address => bool) public operators;

    IronSwapRouter public ironSwapRouter;

    IBlueIceStaking public blueIceStaking;
    ...
    modifier onlyOperator() {
        require(operators[_msgSender()] || _msgSender() == owner(), "Only operator can operate");
        _;
    }
    ...
    function initialize(
        address _target,
        address _ironSwapRouter,
        address _blueIceStaking
    ) external initializer {
        __Ownable_init();
        require(_target != address(0) && _ironSwapRouter != address(0) && _blueIceStaking != address(0));
        target = _target;
        ironSwapRouter = IronSwapRouter(_ironSwapRouter);
        blueIceStaking = IBlueIceStaking(_blueIceStaking);
    }

    /* ========== MUTATIVE FUNCTIONS ========== */

    function allocateRewards(uint256 _amount, uint256 _days) external onlyOperator {
        IERC20 _rewardToken = IERC20(target);
        require(_rewardToken.balanceOf(address(this)) >= _amount, "Insufficient balance of reward token");
        _rewardToken.approve(address(blueIceStaking), _amount);
        blueIceStaking.allocateMoreRewards(_amount, _days);
    }

    function collect() external onlyOperator {
        for (uint256 i = 0; i < pools.length; i++) {
            IIronSwap(pools[i]).withdrawAdminFee();
        }
    }

    function swap() external onlyOperator {
        for (uint256 i = 0; i < whiteListedTokens.length; i++) {
            address fromToken = whiteListedTokens[i];
            if (fromToken != target) {
                SwapConfig memory swapConfig = getSwapConfig[fromToken];
    ...
    function balanceOfTarget() external view returns (uint256) {
        return IERC20(target).balanceOf(address(this));
    }

    /* ========== INTERNAL FUNCTIONS ========== */

    function swapPlainPool(SwapConfig memory config, address fromTokenAddress) internal {
    ...
    function swapMetaPool(SwapConfig memory config, address fromTokenAddress) internal {
        IERC20 fromToken = IERC20(fromTokenAddress);
        uint256 inAmount = fromToken.balanceOf(address(this));
        if (inAmount > 0) {
            IIronSwap pool = IIronSwap(config.pool);
            ...
            fromToken.safeIncreaseAllowance(address(ironSwapRouter), inAmount);
            ironSwapRouter.swapToBase(
                pool,
                basePool,
                tokenIndexFrom,
                tokenIndexTo,
    ...
    function setTarget(address _target) external onlyOwner {
        for (uint256 i = 0; i < whiteListedTokens.length; i++) {
            if (_target == whiteListedTokens[i]) {
                target = _target;
            }
        }
    }
    ...
    function setSwapRouter(address _swapRouter) external onlyOwner {
        ...
        ironSwapRouter = IronSwapRouter(_swapRouter);
    }
    ...
    function setSwapConfig(
        ...
        require(_fromToken != address(0) && pool != address(0), "zeroAddress");
    ...
    function setBlueIceStaking(address _addr) external onlyOwner {
        require(_addr != address(0), "address-invalid");
        blueIceStaking = IBlueIceStaking(_addr);
    }
    ...
    function rescueFund(address _token, uint256 _amount) external onlyOwner {
        ...
        emit FundRescued(_token, _amount);
    }
