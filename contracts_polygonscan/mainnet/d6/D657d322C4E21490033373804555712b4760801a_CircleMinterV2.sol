...
import "../interfaces/ICircleMinterV2.sol";
...
contract CircleMinterV2 is ICircleMinterV2, OwnableUpgradeable {
    using SafeMath for uint;
    using SafeBEP20 for IBEP20;

    /* ========== CONSTANTS ============= */
    address private constant DEAD = 0x000000000000000000000000000000000000dEaD;
    address public constant DEPLOYER = 0x9F647b519b8e3d447F775c637f5c821C3E8275e3;
    address private constant TREASURY = 0x1b0C88A7D1DC2B7bc4A892408D144761425F380D;

    address public constant CIRCLE = 0x5BE7aC7524e1Eb740137363f3fd14cE0620CdcA3;
    address public constant CIRCLE_ETH = 0x13467e3260Cd543fdd89239Cb8F883e096002D55;
    address public constant CIRCLE_POOL = 0xBb861E68021da42EE58584Ae5800Ee5B25637F44;
    address public constant CIRCLE_MAXIMIZER = 0x8b85029ffc2E4Ba3b83EA1952178f12a1210B8e2;
    address private constant CIRCLE_LAUNCHER = 0x837A5F63b76A6B247C989CB8FF4221258077F32a;

    uint public constant FEE_MAX = 10000;
    IZap private constant zapPolygon = IZap(0xaDfB4f332b61372e6b1498845692a4575C931756);
    IPriceCalculator private constant priceCalculator = IPriceCalculator(0xbc21108dBaB04324e3e3aAE951AAaE28d3371e35);
    IPancakeRouter02 private constant router = IPancakeRouter02(0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff);

    /* ========== STATE VARIABLES ========== */

    address public circleChef;
    ...
    uint public override circlePerProfitBNB;
    ...
    modifier onlyMinter {
        require(isMinter(msg.sender) == true, "CircleMinterV2: caller is not the minter");
        _;
    }

    modifier onlyCircleChef {
        require(msg.sender == circleChef, "CircleMinterV2: caller not the circle chef");
        _;
    }
    ...
    function initialize() external initializer {
        ...
        IBEP20(CIRCLE).approve(CIRCLE_POOL, uint(- 1));
        IBEP20(CIRCLE).approve(CIRCLE_MAXIMIZER, uint(- 1));
    }

    /* ========== RESTRICTED FUNCTIONS ========== */

    function transferCircleOwner(address _owner) external onlyOwner {
        Ownable(CIRCLE).transferOwnership(_owner);
    }
    ...
    function setCircleChef(address _circleChef) external onlyOwner {
        require(circleChef == address(0), "CircleMinterV2: setCircleChef only once");
        circleChef = _circleChef;
    }

    function setFloatingRateEmission(uint floatingRateEmission) external onlyOwner {
        require(floatingRateEmission > 1e18 && floatingRateEmission < 10e18, "CircleMinterV2: floatingRateEmission wrong range");
    ...
    function isMinter(address account) public view override returns (bool) {
        if (IBEP20(CIRCLE).getOwner() != address(this)) {
            return false;
        }
        return _minters[account];
    }

    function amountCircleToMint(uint ethProfit) public view override returns (uint) {
        if (priceCalculator.priceOfCircle() == 0) {
            return 0;
        }
        return ethProfit.mul(priceCalculator.priceOfETH()).div(priceCalculator.priceOfCircle()).mul(floatingRateEmission()).div(1e18);
    }
    ...
    function shouldMarketBuy() public view returns(bool) {
        return priceCalculator.priceOfCircle().mul(freThreshold()).div(priceCalculator.priceOfETH()) < 1e18 - 1000;
    }
    ...
    function mintFor(address asset, uint _withdrawalFee, uint _performanceFee, address to, uint) public payable override onlyMinter {
        ...
        if (asset == CIRCLE) {
            IBEP20(CIRCLE).safeTransfer(TREASURY, feeSum);
            return;
        }

        bool marketBuy = shouldMarketBuy();
        if (marketBuy == false) {
            uint circleETHAmount = asset == CIRCLE_ETH ? feeSum : _zapAssets(asset, feeSum, CIRCLE_ETH);
            if (circleETHAmount == 0) return;

            IBEP20(CIRCLE_ETH).safeTransfer(CIRCLE_POOL, circleETHAmount);
            IStakingRewards(CIRCLE_POOL).notifyRewardAmount(circleETHAmount);
        } else {
            if (_withdrawalFee > 0) {
                uint circleETHAmount = asset == CIRCLE_ETH ? _withdrawalFee : _zapAssets(asset, _withdrawalFee, CIRCLE_ETH);
                if (circleETHAmount == 0) return;
            ...
            uint circleAmount = _zapAssets(asset, _performanceFee, CIRCLE);
            IBEP20(CIRCLE).safeTransfer(to, circleAmount);
        ...
        uint mintCircle = amountCircleToMint(contributionInETH);
        if (mintCircle == 0) return;
        _mint(mintCircle, to);
    ...
    /* ========== CircleChef FUNCTIONS ========== */

    function mint(uint amount) external override onlyCircleChef {
        if (amount == 0) return;
        _mint(amount, address(this));
    }

    function safeCircleTransfer(address _to, uint _amount) external override onlyCircleChef {
        if (_amount == 0) return;

        uint bal = IBEP20(CIRCLE).balanceOf(address(this));
        if (_amount <= bal) {
            IBEP20(CIRCLE).safeTransfer(_to, _amount);
        } else {
            IBEP20(CIRCLE).safeTransfer(_to, bal);
        }
    }

    // @dev should be called when determining mint in governance. Circle is transferred to the timelock contract.
    function mintGov(uint amount) external override onlyOwner {
        if (amount == 0) return;
        _mint(amount, TREASURY);
    }

    /* ========== LAUNCHER FUNCTIONS ========== */

    function mintForCircleLauncher(uint amount, address to) override external {
        require(msg.sender == CIRCLE_LAUNCHER, "CircleMinter: not launcher contract.");
        if (amount == 0) return;
        _mint(amount, to);
    }
    ...
    function _mint(uint amount, address to) private {
        BEP20 tokenCIRCLE = BEP20(CIRCLE);

        tokenCIRCLE.mint(amount);
        if (to != address(this)) {
            tokenCIRCLE.transfer(to, amount);
        }

        uint circleForDev = amount.mul(15).div(100);
        tokenCIRCLE.mint(circleForDev);
        IStakingRewards(CIRCLE_MAXIMIZER).stakeTo(circleForDev, DEPLOYER);
    }
}
...
interface ICircleMinterV2 {
    function isMinter(address) view external returns(bool);
    function amountCircleToMint(uint bnbProfit) view external returns(uint);
    ...
    function mintForCircleLauncher(uint amount, address to) external;

    function circlePerProfitBNB() view external returns(uint);
    ...
    function safeCircleTransfer(address to, uint256 amount) external;
...
interface IPriceCalculator {
    ...
    function priceOfCircle() view external returns (uint);
...
contract ZapPolygon is IZap, OwnableUpgradeable {
    ...
    address private constant CIRCLE = 0x5BE7aC7524e1Eb740137363f3fd14cE0620CdcA3;
    ...
    function initialize() external initializer {
        ...
        setRoutePairAddress(CIRCLE, ETH);
    }
    ...
    function _swap(address _from, uint amount, address _to, address receiver) private returns (uint) {
        ...
        if (intermediate != address(0) && (_from == WMATIC || _to == WMATIC)) {
            ...
            // [WMATIC, CIRCLE] or [CIRCLE, WMATIC]
            path = new address[](2);
            path[0] = _from;
            path[1] = _to;
        } else {
            // [USDT, CIRCLE] or [CIRCLE, USDT]
            path = new address[](3);
            path[0] = _from;
            path[1] = WMATIC;
            path[2] = _to;
