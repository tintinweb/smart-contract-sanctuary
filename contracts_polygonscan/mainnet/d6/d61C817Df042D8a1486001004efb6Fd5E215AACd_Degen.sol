...
contract Degen {
    ...
    constructor() {
        ...
        startBlock = block.timestamp + (5 hours) + (30 minutes);
    }
    ...
    function calculateCost() public view returns(uint256) {
        ...
        uint256 cost = buyCost - buyCost.mul(blockDifference).div(2000);
        if (cost < 0.001 ether) {
            return 0.001 ether;
        }
        return cost;
    ...
    function buy() public {
        ...
        require(block.timestamp > startBlock, "Game not started yet");
