...
//*********IThunderReferral.sol*********

interface IThunderReferral {
    /**
     * @dev Record referral.
     */
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    // Thunder referral contract address.
    IThunderReferral public thunderReferral;
    ...
    // Update the given pool's Thunder allocation point and deposit fee. Can only be called by the owner.
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 thunderReward = multiplier.mul(thunderPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        thunder.mint(devAddress, thunderReward.div(10));
    ...
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        ...
        updatePool(_pid);
        if (_amount > 0 && address(thunderReferral) != address(0) && _referrer != address(0) && _referrer != msg.sender) {
            thunderReferral.recordReferral(msg.sender, _referrer);
        }
        if (user.amount > 0) {
        ...
        if (_amount > 0) {
            ...
            if (address(pool.lpToken) == address(thunder)) {
    ...
    // Safe Thunder transfer function, just in case if rounding error causes pool to not have enough Thunders.
    ...
    // Update the Thunder referral contract address by the owner
    function setThunderReferral(IThunderReferral _thunderReferral) public onlyOwner {
        thunderReferral = _thunderReferral;
    }
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (address(thunderReferral) != address(0) && referralCommissionRate > 0) {
            address referrer = thunderReferral.getReferrer(_user);
            uint256 commissionAmount = _pending.mul(referralCommissionRate).div(10000);

            if (referrer != address(0) && commissionAmount > 0) {
                thunder.mint(referrer, commissionAmount);
