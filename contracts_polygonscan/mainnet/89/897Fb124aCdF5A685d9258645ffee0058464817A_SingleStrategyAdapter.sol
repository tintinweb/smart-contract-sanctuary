...
contract SingleStrategyPolygon is OwnableUpgradeable {
  ...
  */
  ...
  function pendingDQuick() public view returns (uint256) {
    ...
    return earnedDquick.add(dQuickBalance).add(stakedDquick);
  }
  ...
  function pendingReward() public view returns (uint256) {
    ...
    return balance.add(earnedToken);
  }
  ...
  */
  ...
  function enterRewardPool() internal {
    uint256 earned = IStakingRewards(quickRewardPool).earned(address(this));

    if(earned != 0){
      IStakingRewards(quickRewardPool).getReward();
    }
