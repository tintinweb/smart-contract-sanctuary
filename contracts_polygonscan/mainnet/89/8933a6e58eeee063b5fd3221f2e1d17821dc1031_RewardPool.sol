...
library UniswapV2Library {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address) {
        ...
        return(address(uint(keccak256(abi.encodePacked(
                ...
                //hex'1dcc0ddcf5b792d4f279a806c93acbee67b7bff0bab5b6f7d51d2331ead5b153' // init code hash tests
            )))));
    }
...
contract RewardPool is LPTokenWrapper, IRewardDistributionRecipient {
    ...
    function setEmiPriceData(address _uniFactory, address[] memory _route) public onlyOwner {
        if (uniFactory != _uniFactory) {
    ...
    function getStakedValuesinUSD(address wallet) public view returns (uint256 senderStake, uint256 totalStake) {
        ...
        if (tokenMode == 1) {
            ...
            uint256 ESWreserveWallet = getReserve(route[0]).mul(2).mul(lpFractionWallet).div(1e18);
            uint256 ESWreserveTotal = getReserve(route[0]).mul(2).mul(lpFractionTotal).div(1e18);
            senderStake = ESWreserveWallet.mul(price).div(minPriceAmount);
            totalStake = ESWreserveTotal.mul(price).div(minPriceAmount);

            console.log("balanceOf(wallet) %s totalSupply() %s ", balanceOf(wallet), totalSupply());
            console.log("stakeToken.totalSupply() %s", stakeToken.totalSupply());
            console.log("getReserve(route[0] %s", getReserve(route[0]));
            console.log("ESWreserveTotal %s ESWreserveWallet %s", ESWreserveTotal, ESWreserveWallet);
            console.log("lpFractionWallet %s lpFractionTotal %s", lpFractionWallet, lpFractionTotal);
            console.log("senderStake %s totalStake %s", senderStake, totalStake); */
    ...
    function getAmountOut(uint256 amountIn, address[] memory path) public view returns (uint256) {
        return UniswapV2Library.getAmountsOut(uniFactory, amountIn, path)[path.length - 1];
    }

    function getReserve(address token) internal view returns (uint256 reserveA) {
        address token0 = IUniswapV2Pair(address(stakeToken)).token0();
        (uint reserve0, uint reserve1,) = IUniswapV2Pair(address(stakeToken)).getReserves();
        (reserveA, ) = token == token0 ? (reserve0, reserve1) : (reserve1, reserve0);        
        return reserveA;
    }
