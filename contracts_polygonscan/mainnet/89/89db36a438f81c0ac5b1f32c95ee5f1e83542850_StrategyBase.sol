...
abstract contract StrategyFarmBase is BaseStrategyMasterChef {
    ...
    address public constant kul = 0xdA87169B63632766D922B0f56dC5B3dD22cfCFb3;
    ...
    {
        kul_usdc_path = new address[](2);
        kul_usdc_path[0] = kul;
        kul_usdc_path[1] = usdc;
    }
    ...
    function harvest() public override {
        ...
        //Swap 
        uint256 _kulBalance = IERC20(kul).balanceOf(address(this));
        if (_kulBalance > harvestCutoff){
            
            //Send performance fee to strategist
            /*uint256 performanceFee = _kulBalance.mul(keepFXS).div(keepFXSmax);
            if (performanceFee>0) {
                _swapUniswapWithPath(kul_usdc_path, performanceFee);
            ...
            // Swap rest
            _kulBalance = IERC20(kul).balanceOf(address(this));
            _swapUniswapWithPath(kul_usdc_path, _kulBalance);
            ...
            /*
            // Add liquidity
            uint256 _dino = IERC20(dino).balanceOf(address(this));
            uint256 _eth = IERC20(eth).balanceOf(address(this));
            if (_dino > 0 && _eth > 0) {
                IERC20(dino).safeApprove(currentRouter, 0);
                IERC20(dino).safeApprove(currentRouter, _dino);
                ...
                // Donates DUST
                IERC20(dino).safeTransfer(
                    strategist,
                    IERC20(dino).balanceOf(address(this))
                );
                IERC20(eth).safeTransfer(
                    strategist,
                    IERC20(eth).balanceOf(address(this))
...
contract StrategyBase is StrategyFarmBase {
    // Token addresses
    address public KUL_MASTER_CHEF = 0x43E636E6C0338F0C4Ce14Eb524103ef64b8f8b27;
    address public SAFEWORLD_WMATIC_LP = 0x30aac51f59E45a020B4A5A03c5E1Bd2F1aA17942;
