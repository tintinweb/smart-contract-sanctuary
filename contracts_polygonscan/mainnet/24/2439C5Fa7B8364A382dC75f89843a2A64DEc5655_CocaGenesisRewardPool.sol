...
// Note that this pool has no minter key of COCA (rewards).
// Instead, the governance will call COCA distributeReward method and send reward to this pool at the beginning.
contract CocaGenesisRewardPool {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. COCA to distribute.
        uint256 lastRewardTime; // Last time that COCA distribution occurs.
        uint256 accTombPerShare; // Accumulated COCA per share, times 1e18. See below.
    ...
    // The time when COCA mining starts.
    uint256 public poolStartTime;

    // The time when COCA mining ends.
    ...
    // Update the given pool's COCA allocation point. Can only be called by the owner.
    ...
    // View function to see pending COCA on frontend.
    ...
    // Safe COCA transfer function, just in case if rounding error causes pool to not have enough TOMBs.
