...
contract ProfitFinder is ContractOwnable, Initializable {
    ...
    constructor (address payable _bot, address[] memory _pools) {
        initialize(_bot, _pools);
    }

    function initialize(address payable _bot, address[] memory _pools) public onlyOwner {
        initOwner(_msgSender());
        bot = FlashBot(_bot);
        setPools(_pools);
    }
    ...
    returns (address pool0, address pool1, uint256 profit, address baseToken, uint256 checked) {
        profit = 0;
        checked = 0;
        pool0 = address(0);
        pool1 = address(0);
