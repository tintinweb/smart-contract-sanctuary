...
contract WormStrategyLPUsdcWexPoly is BaseWormStrategyLPSingle {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    ...
    function _wormDeposit(uint256 _farmedAmt) internal override {
        IWault(stakingRewardsAddress).deposit(pid, _farmedAmt, true);
    }

    function _wormWithdraw(uint256 _farmedAmt) internal override {
        IWault(stakingRewardsAddress).withdraw(pid, _farmedAmt, true);
    }
    ...
    function totalInUnderlying() public override view returns (uint256) {
        (uint256 farmedAmount,) = IWault(stakingRewardsAddress).userInfo(pid, address(this));
        return farmedAmount;
    }
