...
contract BestBuds is ERC721, Ownable {
    ...
    uint256 public whitelistPrice = 15 ether; // 10 MATIC
    uint256 public tokenPrice = 15 ether; // 10 MATIC
    uint256 public constant maxTokenPurchase = 10;
    uint256 public constant MAX_TOKENS = 4200;
    uint256 public bluntsPassed = 0;
    ...
    constructor() ERC721("BestBuds", "BUDS") { }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        // If there is a baseURI but no tokenURI, concatenate the tokenID to the baseURI.
        if (hidden) {
            return string(abi.encodePacked(hiddenBase, ((tokenId % 10) + 1).toString()));
        } else {
            return string(abi.encodePacked(base, tokenId.toString()));
        }
    }
    ...
    function airdropper(uint256 tokes, address stonerSquad, uint256 positiveOffset, uint256 negativeOffset) external onlyOwner {
        require(negativeOffset <= bluntsPassed+1, "negative offset must be less than bluntsPassed");
        for (uint i = bluntsPassed + 1; i < bluntsPassed + tokes + 1; i++) {
            safeTransferFrom(ownerOf(i), IERC721(stonerSquad).ownerOf(i - negativeOffset + positiveOffset), i);
        }
        bluntsPassed += tokes;
    }
