...
/// @title QuickSwap Adapter for Game cluster
/// @author Blaize.tech team
/// @notice Contract for interacting with QuickSwap router
contract GameAdapter is DexAdapterCore {
    using SafeERC20 for IERC20;

    address public constant comethRouter = address(0x93bcDc45f7e62f89a8e901DC4A0E2c6C427D9F25);

    /// @notice Performs an initial setup.
    /// @notice Sets an addresses of QuickSwap router, WMATIC and USDC in Polygon.
    constructor()
        DexAdapterCore(
    ...
    function _getRouter(address _token) internal view override returns (address) {
        if (_token == address(0x9C78EE466D6Cb57A4d01Fd887D2b5dFb2D46288f)) {
            //Must Token
            return comethRouter;
        }
        return router;
