...
contract SuChefSummoner is CloneFactory {
    ...
    event SummonSuChef(address indexed suChef, IMochiMolMinimal indexed mochiMol, string details);
    
    constructor(address payable _template) public {
        template = _template;
    }
    
    function summonSuChef(address depositToken, IMochiMolMinimal mochiMol, string calldata details) external returns (SuChef suChef) {
        suChef = SuChef(createClone(template));
        
        suChef.init(depositToken, mochiMol);
        
        emit SummonSuChef(address(suChef), mochiMol, details);
    }
}