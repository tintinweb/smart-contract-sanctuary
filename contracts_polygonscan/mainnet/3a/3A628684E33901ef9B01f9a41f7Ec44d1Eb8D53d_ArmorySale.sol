    ...
    PausableUpgradeable
{
    ...
    function setBundlePrice(uint8 index, uint8 _count, uint8 _price) external onlyOwner {
    ...
    function getWeaponListingIDsPage(WeaponsFilter memory filter)
        public 
        view 
    ...
    function _getWeaponListingIDsPage(WeaponsFilter memory filter, uint256 pageEnd, uint256 tokensSize)
        internal
        view
        returns (uint256[] memory) 
    {
    ...
    function getNumberOfWeaponListings(WeaponsFilter memory filter)
        public
        view
        returns (uint256)
    {
    ...
    function _weaponPassesFilter(uint256 weaponId, WeaponsFilter memory filter) internal view returns (bool){
        (   ,
            ...
            uint8 quality,
            ,
            bool holographic) = weapons.get(weaponId);

        uint8 noFilter = 255;

        return (filter.gunType == noFilter || gunType == filter.gunType) &&
