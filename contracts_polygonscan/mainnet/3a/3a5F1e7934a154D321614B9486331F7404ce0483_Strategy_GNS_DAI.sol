...
interface IFarm {
    ...
    function users(address _account) external view returns (uint, uint, uint, uint, uint, address, uint);
    function unstake(uint256 _amount) external;
}

interface IDragonLair {
    // Leave the lair. Claim back DAI.
    function leave(uint256 _dQuickAmount) external;
    // Enter the lair. Pay some DAI. Earn some dragon DAI.
...
contract Strategy_GNS_DAI is Ownable {
    // This strategy is for polygon network. GNS - DAI pair. RewardToken = "dQUICK, GNS"
    ...
    uint256 public pendingRewardsFeeWETH;
    uint256 public pendingRewardsFeeDAI;
    uint256 public pendingRewardsFeeGNS;
    ...
    address public token1; // GNS
    address public vault;

    // ------------> Important constants <--------------
    bool public hasPid = false;
    address public constant lpToken = 0x6E53cB6942e518376E9e763554dB1A45DDCd25c4;
    address public constant QUICKToken = 0x831753DD7087CaC61aB5644b308642cc1c33Dc13;
    address public constant farm = 0x151757c2E830C467B28Fe6C09c3174b6c76aA0c5;
    address public router = 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff; // uses for swapping GNS - DAI
    ...
    swapping WMATIC to GNS using WETH and DAI
    ...
    */
    ...
    function getAmountLPFromFarm() public view returns (uint256 provided) {
        (provided, , , , , , ) = IFarm(farm).users(address(this));
    }
    ...
    function earn() public returns(uint256 _balance) {
        ...
        if(_balance > 0) {
            ...
            IFarm(farm).stake(_balance, address(0));
            emit Earn(_balance);
        }
    }
    ...
    function autoCompound() external {
        ...
        if (getAmountLPFromFarm() > 0) {
            ...
            _getWrappedFromRewards();
            _takeFee();

            _swapExactTokenForToken(routerReward, QUICKToken, WMATIC, getBalanceOfToken(QUICKToken));
            _swapExactTokenForToken(router, token1, token0, getBalanceOfToken(token1));
            _swapExactTokenForToken(router, token0, WETH, getBalanceOfToken(token0));
            _swapExactTokenForToken(router, WETH, WMATIC, getBalanceOfToken(WETH));
    ...
    function _getWrappedFromRewards() internal {
        uint256 _balance = getBalanceOfToken(QUICKToken);
        bool _swapped = _swapExactTokenForToken(routerReward, QUICKToken, WMATIC, _balance+pendingRewardsFee);
        if (_swapped) {
            pendingRewardsFee = 0;
        } else {
        ...
        WETH <-> DAI
        WETH <-> WMATIC */
        _balance = getBalanceOfToken(token1);
        _swapped = _swapExactTokenForToken(router, token1, token0, _balance+pendingRewardsFeeGNS);
        if (_swapped) {
            pendingRewardsFeeGNS = 0;
        } else {
            pendingRewardsFeeGNS += _calculateAmountFee(_balance);
        }

        _balance = getBalanceOfToken(token0);
        _swapped = _swapExactTokenForToken(router, token0, WETH, _balance+pendingRewardsFeeDAI);
        if (_swapped) {
            pendingRewardsFeeDAI = 0;
        } else {
            pendingRewardsFeeDAI += _calculateAmountFee(_balance);
        }

        _balance = getBalanceOfToken(WETH);
        _swapped = _swapExactTokenForToken(router, WETH, WMATIC, _balance+pendingRewardsFeeWETH);
        if (_swapped) {
            pendingRewardsFeeWETH = 0;
        } else {
            pendingRewardsFeeWETH += _calculateAmountFee(_balance);
        }
    }
    ...
    function getBalanceOfToken(address _token) public view returns (uint256) {
        ...
        if(_token == token0) {
            return IERC20(_token).balanceOf(address(this)) - pendingRewardsFeeDAI;
        }
        if(_token == token1) {
            return IERC20(_token).balanceOf(address(this)) - pendingRewardsFeeGNS;
        }
        if(_token == WETH) {
            return IERC20(_token).balanceOf(address(this)) - pendingRewardsFeeWETH;
        }
    ...
    function _takeFee() internal {
        ...
        bool _swapped = _swapExactTokenForToken(routerMain, WMATIC, USDT, rewardsFeeInWMATIC+pendingFee);
        if (_swapped) {
            pendingFee = 0;
    ...
    function _addLiquidityFromAllTokens() internal {  
        _swapExactTokenForToken(router, WMATIC, WETH, getBalanceOfToken(WMATIC));
        _swapExactTokenForToken(router, WETH, token0, getBalanceOfToken(WETH));
    ...
    function _addLiquidityETH(uint256 _amountETH) internal {
        _swapExactETHForTokens(WETH, _amountETH);
        _swapExactTokenForToken(router, WETH, token0, getBalanceOfToken(WETH));
        _swapExactTokenForToken(router, token0, token1, getBalanceOfToken(token0)/2);
    ...
    function _swapExactETHForTokens(address _token, uint256 _amountETH) internal {
        ...
        path[0] = WMATIC;
        path[1] = _token;
        uint256[] memory amounts = IRouter(router).getAmountsOut(_amountETH, path);
    ...
    function _getPriceWMATICToDAI() internal view returns (uint256 result) {
        /*
            DAI(token0) <-> GNS(token1)
            WETH <-> DAI
            WETH <-> WMATIC 
        */
        ...
        path[0] = WMATIC;
        path[1] = WETH;
        uint256 amountWETH = _getAmountsOut(router, 50 * 1e18, path);
        path[0] = WETH;
        path[1] = token0;
        if(amountWETH > 0)
            result = 1e36 / (_getAmountsOut(router, amountWETH, path) / 50);
    }

    function _getAmountDAIfromGNS(uint256 amountGNS) internal view returns (uint256 result) {
        address[] memory path = new address[](2);
        path[0] = token1;
        path[1] = token0;
        if(amountGNS > 0)
            result = _getAmountsOut(router, amountGNS, path);
    }
    ...
    function _getSimpleTCI(uint256 _amount) internal view returns (uint256 tCI) {
        ...
        if(_amount > 0) {
            ...
            tCI = (_getAmountDAIfromGNS(t1V) + t0V) * _getPriceWMATICToDAI() / d;
        }

        // calculates total Capital from tokens that exist in the contract
        uint256 _balance0 = getBalanceOfToken(token0);
        uint256 _balance1 = _getAmountDAIfromGNS(getBalanceOfToken(token1));
        if (_balance0 > 0) {
            tCI += _balance0 * _getPriceWMATICToDAI() / d;
        }

        if (_balance1 > 0) {
            tCI += _getAmountDAIfromGNS(_balance1) * _getPriceWMATICToDAI() / d;
        }
    ...
    function _getSimpleTC() internal view returns (uint256 TC) {
        ...
        if(_totalLP > 0) {
            ...
            // calculates how many wrapped Tokens for token0 and token1
            if(t1V != 0) {
                amountDAI = _getAmountDAIfromGNS(t1V);
            }
            if(t0V + amountDAI != 0) {
                path[0] = token0;
                path[1] = WETH;
                amountWETH = _getAmountsOut(router, t0V + amountDAI, path);
            }

            if(amountWETH != 0) {
                path[0] = WETH;
                path[1] = WMATIC;
                TC = _getAmountsOut(router, amountWETH, path);
            }
        }

        // calculates total Capital from tokens that exist on the contract
        
        // ----> calculates GNS as DAI <----
        uint256 _balance = getBalanceOfToken(token1);
        amountDAI = 0;
        amountWETH = 0;
        if(_balance != 0) {
            amountDAI = _getAmountDAIfromGNS(_balance);
        }

        // ----> calculates GNS to WMATIC  <-----
        _balance = getBalanceOfToken(token0);
        if(_balance + amountDAI != 0) {
            path[0] = token0;
            path[1] = WETH;
            amountWETH = _getAmountsOut(router, _balance + amountDAI, path);
        }
        _balance = getBalanceOfToken(WETH);
        if(amountWETH + _balance != 0) {
            path[0] = WETH;
            path[1] = WMATIC;
            TC += _getAmountsOut(router, amountWETH + _balance, path);
        }
