...
contract Plasma is ERC20Burnable, Ownable {

    using SafeMath for uint256;

    //uint256 public MAX_WALLET_STAKED = 10;
    uint256 public EMISSIONS_RATE = 1;
    //uint256 public CLAIM_END_TIME = 1641013200;
    uint256 public PERIOD_PER_CYCLE = 3600 * 24;

    address nullAddress = 0x0000000000000000000000000000000000000000;

    address public A51Address;
    ...
    //Mapping of staker to alien
    mapping(address => uint256[]) internal stakerToTokenIds;

    constructor() ERC20("Plasma", "PLASMA") {}

    function setA51Address(address _A51Address) public onlyOwner {
        A51Address = _A51Address;
        return;
    }
    ...
    function getTotalSupply()
        ...
        returns (uint256)
    {
        return IERC721Enumerable(A51Address).totalSupply();
    }
    ...
    function stakeByIds(uint256[] memory tokenIds) public {
        ...
        //         MAX_WALLET_STAKED,
        //     "Must have less than 31  staked!"
        // );

        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(
                IERC721(A51Address).ownerOf(tokenIds[i]) == msg.sender &&
                    tokenIdToStaker[tokenIds[i]] == nullAddress,
                "Token must be stakable by you!"
            );

            IERC721(A51Address).transferFrom(
                msg.sender,
                address(this),
                tokenIds[i]
            );
    ...
    function unstakeAll() public {
        ...
        for (uint256 i = stakerToTokenIds[msg.sender].length; i > 0; i--) {
            ...
            totalRewards =
                totalRewards +
                ((block.timestamp - tokenIdToTimeStamp[tokenId]) / PERIOD_PER_CYCLE *
                    EMISSIONS_RATE);
    ...
    function unstakeByIds(uint256[] memory tokenIds) public {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            totalRewards =
                totalRewards +
                ((block.timestamp - tokenIdToTimeStamp[tokenIds[i]]) / PERIOD_PER_CYCLE *
                    EMISSIONS_RATE);
    ...
    function claimByTokenId(uint256 tokenId) public {
        ...
        _mint(
            msg.sender,
            ((block.timestamp - tokenIdToTimeStamp[tokenId]) / PERIOD_PER_CYCLE * EMISSIONS_RATE)
        );
        ...
        returns (uint256)
    {
        ...
        return secondsStaked / PERIOD_PER_CYCLE * EMISSIONS_RATE;
    }
