...
contract Flag is ERC721, Ownable {
    ...
    address public ctfQuestionAddress;

    constructor(string memory name, string memory symbol, uint _level) ERC721(name, symbol) {
        engineerLevel = _level;
    }

    function init(address _ctf, string memory _flagSvg) onlyOwner external {
        ctfQuestionAddress = _ctf;
        flagSvg = _flagSvg;
        renounceOwnership();
    }
    ...
    function mint(
        ...
        require(ctfQuestionAddress == msg.sender, "ctfQuestionAddress can mint only.");
