...
contract DEIProxy is Ownable {
	...
	function Nativecoin2DEI(
		...
		uint DEUS_RESIDUAL = IERC20(deus_address).balanceOf(address(this));
		uint USDC_RESIDUAL = IERC20(collateral_address).balanceOf(address(this));
		if (DEUS_RESIDUAL > 0) IERC20(deus_address).transfer(msg.sender, DEUS_RESIDUAL);
		if (USDC_RESIDUAL > 0) IERC20(collateral_address).transfer(msg.sender, USDC_RESIDUAL);
