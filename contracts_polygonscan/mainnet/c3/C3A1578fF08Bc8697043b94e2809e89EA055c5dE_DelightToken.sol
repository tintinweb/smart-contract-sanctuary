...
contract DelightToken is ERC20('DelightToken', 'DELIGHT'), ReentrancyGuard, Ownable {
    ...
    uint256 public constant delightMaximumSupply = 30000 * (10 ** 18); //30k

    uint256 public delightRemaining = delightMaximumSupply;
    
    uint256 public maxHardCap = 150000 * (10 ** 6); // 150k usdc

    uint256 public constant maxDelightPurchase = 600 * (10 ** 18); // 600 delight
    ...
    function buyDelight(uint256 _usdcSpent) external nonReentrant {
        ...
        require(_usdcSpent <= maxHardCap, "Delight Presale hardcap reached");
        require(userDelightTotally[msg.sender] < maxDelightPurchase, "user has already purchased too much delight");

        uint256 delightPurchaseAmount = (_usdcSpent * 1000000000000) / salePrice;
