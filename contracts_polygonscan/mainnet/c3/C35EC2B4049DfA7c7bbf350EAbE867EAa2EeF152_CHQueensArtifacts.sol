...
contract CHQueensArtifacts is Delegated, ERC1155{
  ...
  function burn( uint id, uint quantity ) external payable{
    require( id < tokens.length,                           "Specified token (id) does not exist" );
    require( tokens[id].isBurnActive,                      "Sale is not active"        );
    require( msg.value >= tokens[id].burnPrice * quantity, "Ether sent is not correct" );
  ...
  function mint( uint id, uint quantity ) external payable{
    require( id < tokens.length,                           "Specified token (id) does not exist" );
    require( tokens[id].isMintActive,                      "Sale is not active"        );
    require( msg.value >= tokens[id].mintPrice * quantity, "Ether sent is not correct" );
