...
//For modified Master Chef single-staking contracts with no poolId - probably only used for Dino
abstract contract SingleChefStrategy is BaseStrategy {
    ...
    function balanceOfPool() public override view returns (uint256) {
        (uint256 amount, ) = ISingleChef(rewards).userInfo(address(this));
        return amount;
    }

    function getHarvestable() external override view returns (uint256) {
        return ISingleChef(rewards).pendingReward(address(this));
    }
    ...
    function deposit() public override {
        uint256 _want = IERC20(want).balanceOf(address(this));
        if (_want > 0) {
            ISingleChef(rewards).deposit(_want);
        }
    }
    ...
    function _withdrawSome(uint256 _amount)
        ...
        returns (uint256)
    {
        ISingleChef(rewards).withdraw(_amount);
        return _amount;
    }
    ...
    function _getReward() internal {
        ISingleChef(rewards).withdraw(0);
    }
    ...
    function emergencyWithdraw() public onlyOwner {
        ISingleChef(rewards).emergencyWithdraw();
...
contract DinoEternalPoolStrategy is SingleChefStrategy {
    ...
    address ETERNAL_POOL = 0x52e7b0C6fB33D3d404b07006b006c8A8D6049C55;
        ...
        )
    {
        ...
        IERC20(DINO).safeApprove(SUSHI_ROUTER, uint256(-1));
    }
