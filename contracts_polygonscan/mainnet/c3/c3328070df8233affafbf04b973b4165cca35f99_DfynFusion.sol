...
contract DfynFusion is Ownable {
    ...
    function swapTokensToNative(
        ...
        if (inputLPs.length > 0) {
            ...
            _swapTokensForWNative(_removeLiquidity(inputLPs),  ZERO_ADDRESS);
        }
        if (inputTokens.length > 0) {
            _transferTokensToDfyn(inputTokens);
            _swapTokensForWNative(inputTokens, ZERO_ADDRESS);
        }
    ...
    function swapTokensToToken(
        ...
        uint256 minOutputAmount,
        address[] memory nativeToTokenPath
    ) public payable {
        if (msg.value > 0) {
        ...
        if (inputLPs.length > 0) {
            _transferTokensToDfyn(inputLPs);
            _swapTokensForWNative(_removeLiquidity(inputLPs), outputTokenAddress);
        }
        if (inputTokens.length > 0) {
            _transferTokensToDfyn(inputTokens);
            _swapTokensForWNative(inputTokens, outputTokenAddress);
        }
    ...
    function _swapTokensForWNative(InputToken[] memory inputTokens, address outputToken)
        private
        returns (uint256)
    {
        uint256 totalNative = 0;
        address outputTokenAddress = outputToken != ZERO_ADDRESS?outputToken:address(wNative);
        for (uint256 i = 0; i < inputTokens.length; i++) {
            // Swap non wNative token
            if (inputTokens[i].token != outputTokenAddress) {
