...
// File: contracts\StakedLPToken04.sol
...
contract StakedLPToken04 is IUniswapV2Pair, PoolToken {
    using SafeToken for address;
	
	bool public constant isStakedLPToken = true;
	string public constant stakedLPTokenType = "04";
	...
	address public constant rewardsToken = 0xf28164A485B0B2C90639E47b0f377b4a438a16B1;
	...
	function reinvest() external nonReentrant update {
		...
		// 3.0 Unwrap QUICK
		IDragonLair(rewardsToken).leave(rewardsToken.myBalance());
		// 3. Convert all the remaining rewards to token0 or token1.
		address tokenA;
		address tokenB;
		if (token0 == QUICK || token1 == QUICK) {
			(tokenA, tokenB) = token0 == QUICK ? (token0, token1) : (token1, token0);
		}
		else {
			swapExactTokensForTokens(QUICK, WETH, QUICK.myBalance());
...
// File: contracts\StakedLPTokenFactory04.sol
