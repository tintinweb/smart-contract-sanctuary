...
import "../uniswap/periphery/interfaces/IUniswapV2Router01.sol";
...
contract Test1 is Initializable, IERC20, Ownable {
    ...
    // Intializer function for upgrade contracts:
    function initialize(address _router) initializer public {
        Ownable.initialize();

        _rTotal = (MAX - (MAX % _tTotal));

        feeRates = feeRatesStruct({rfi: 3, liquidity: 3, Treasury: 1, Burn: 2});

        swapAndLiquifyEnabled = true;

        _maxTxAmount = 1 * 10**11 * 10**_decimals;
        numTokensSellToAddToLiquidity = 1 * 10**7 * 10**_decimals; //0.01%

        _rOwned[_msgSender()] = _rTotal;
        IUniswapV2Router02 _PancakeSwapV2Router = IUniswapV2Router02(_router); //BSC Mainnet
    ...
    function includeInRFI(address account) public onlyOwner {
    ...
    function setMaxTxAmount(uint256 maxTxAmount) external onlyOwner {
        require(
            maxTxAmount <= _tTotal,
            "maxTxAmount cannot exceed total supply"
        );
        uint256 _previoiusAmount = _maxTxAmount;
        ...
        )
    {
        ...
        rTransferAmount = rAmount.sub(rRfi).sub(rLiquidity).sub(rTreasury).sub(
            rBurn
        );
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        ...
        increaseBalance(address(this), tLiquidity, rLiquidity);
    }
    ...
    function transferPrimitive(
        ...
        decreaseBalance(sender, tAmount, s.rAmount);
        increaseBalance(recipient, s.tTransferAmount, s.rTransferAmount);
    ...
    function reflectBurnandTreasuryFee(uint256 tBurn, uint256 tTreasury)
        private
    {
        uint256 currentRate = _getRate();
        ...
        increaseBalance(BurnWallet, tBurn, rBurn);
        _tTreasuryTotal = _tTreasuryTotal.add(tTreasury);
        increaseBalance(TreasuryWallet, tTreasury, rTreasury);
    }
