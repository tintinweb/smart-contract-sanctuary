...
contract SandStrategy is BaseStrategy {

    uint256 internal constant keepMax = 10000;
    uint256 public fee = 300;

    address internal constant SAND = 0xBbba073C31bF03b8ACf7c28EF0738DeCF3695683;
    address internal constant WMATIC = 0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270;

    // Uniswap swap paths
    address[] internal sand_wmatic_path;

    constructor()
        public
        BaseStrategy(
            0x369582d2010B6eD950B571F4101e3bB9b554876F, //SAND/WMATIC LP
            0x01fE07ce760DA7a025e4Cf3f950aE236F8e62120, //ERC Fund
            SAND,
            0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff, //QS router
            0x4AB071C42C28c4858C4BAc171F06b13586b20F30 //Sand Rewards
        )
    {
        sand_wmatic_path = [SAND, WMATIC];

        IERC20(want).safeApprove(rewards, uint256(-1));

        IERC20(SAND).safeApprove(currentRouter, uint256(-1));
    ...
    function getPerformanceFee() public view returns (uint256) {
        //Fee is capped at 30% in ERCFund
    ...
    function swapRewardToWmaticAndDistributeFee(uint256 feeAmount) internal {
        if(feeAmount > 0) {
            _swapUniswapWithPathPreapproved(sand_wmatic_path, feeAmount);
    ...
    function harvest() public override onlyHumanOrWhitelisted nonReentrant {
        //Transfer WMATIC/SAND/QUICK/DQUICK that may already be in the contract to the fee dist fund
        uint256 h_bal = IERC20(SAND).balanceOf(address(this));
        if(h_bal > 0) {
            IERC20(SAND).safeTransfer(strategist, h_bal);
        }
        ...
        //Deduct perf fee from harvested SAND
        ...
        //Swap 1/2 of SAND for MATIC
        uint256 remainingHarvested = IERC20(SAND).balanceOf(address(this));
        if (remainingHarvested > 0) {
            _swapUniswapWithPathPreapproved(sand_wmatic_path, remainingHarvested.div(2));
        }

        uint256 balance0 = IERC20(harvestedToken).balanceOf(address(this));
        uint256 balance1 = IERC20(WMATIC).balanceOf(address(this));
