...
contract GachaMachineOnMatic is Ownable, ERC1155Holder {
    ...
    function setTokenRateForAzukiMachine(
        uint256 _currencyBurnRate,
        uint256 _forArtistRate,
        uint256 _forAgentRate
    ) public onlyAdministrator {
        require(machineType == 0, "this machine is not azuki machine.");
        forBuybackRate = 0;
        forBurnRate = _currencyBurnRate;
    ...
    function _transferAndBurnToken(uint256 amount) private {
        // 1. burn token
        uint256 burnAmount = amount.mul(forBurnRate).div(100);
        currencyToken.transferFrom(msg.sender, azukiBurnAccount, burnAmount);
    ...
    function changeMachineAgentAccount(address account) public onlyAdministrator {
