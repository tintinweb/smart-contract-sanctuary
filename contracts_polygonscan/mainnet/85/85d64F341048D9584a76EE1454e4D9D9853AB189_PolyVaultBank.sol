...
contract PolyVaultBank is ReentrancyGuard {
    using SafeMath for uint256;
    ...
    uint256 public startBlock = 16636865; // https://polygonscan.com/block/countdown/16636865
    ...
    address public timelock = address(0x6deAbfB4d54B44584662996e9AabD0AEF9677B09); // 6h timelock
    ...
    function initialize(
        ...
        initialized = true;
        operator = msg.sender;
        addRewardPool(_pvi, _pviRewardRate);
        emit Initialized(msg.sender, block.number);
    }

    modifier onlyOperator() {
        require(operator == msg.sender, "PolyVaultBank: caller is not the operator");
        _;
    }

    modifier onlyTimelock() {
        require(timelock == msg.sender, "PolyVaultBank: caller is not timelock");
        _;
    }

    modifier notInitialized() {
        require(!initialized, "PolyVaultBank: already initialized");
        _;
    }
    ...
    function checkPoolDuplicate(IERC20 _want) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].want != _want, "PolyVaultBank: existing pool?");
        }
    }
    ...
    function addRewardPool(address _rewardToken, uint256 _rewardPerBlock) public nonReentrant onlyOperator {
        require(rewardPoolInfo.length <= 16, "PolyVaultBank: Reward pool length > 16");
        massUpdatePools();
    ...
    function updateRewardToken(uint256 _rewardPid, address _rewardToken, uint256 _rewardPerBlock) external nonReentrant onlyOperator {
        ...
        require(_rewardPid >= 1, "core reward pool"); // PVI
    ...
    function setUnstakingFrozenTime(uint256 _unstakingFrozenTime) external nonReentrant onlyOperator {
        require(_unstakingFrozenTime <= 7 days, "PolyVaultBank: !safe - dont lock for too long");
    ...
    function withdraw(uint256 _pid, uint256 _wantAmt) public nonReentrant notContract {
        ...
        require(user.shares > 0, "PolyVaultBank: user.shares is 0");
        require(sharesTotal > 0, "PolyVaultBank: sharesTotal is 0");
        ...
        if (_wantAmt > 0) {
            ...
            if (_wantAmt > 0) {
                require(whitelisted[msg.sender] || block.timestamp >= unfrozenStakeTime(_pid, msg.sender), "PolyVaultBank: frozen");
    ...
    function setTimelock(address _timelock) external {
        require(msg.sender == timelock || (timelock == address(0) && msg.sender == operator), "PolyVaultBank: !authorised");
        timelock = _timelock;
    }
    ...
    function executeTransaction(address target, uint256 value, string memory signature, bytes memory data) external onlyTimelock returns (bytes memory) {
        ...
        require(success, "PolyVaultBank::executeTransaction: Transaction execution reverted.");
