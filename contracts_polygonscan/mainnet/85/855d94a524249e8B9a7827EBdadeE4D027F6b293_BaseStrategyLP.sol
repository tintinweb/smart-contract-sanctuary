...
contract BaseStrategyLP is StratManager, FeeManager {
    ...
    address constant public wftm = 0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270; // wMatic;
    ...
    address public constant masterchef = 0xbB703E95348424FF9e94fbE4FB524f6d280331B8; // quick staking Rewards
    ...
    constructor(
        ...
        require(_input != _output, "Base Strat Error (Constructor): Input token cannot be the same as output token");
        ...
        require(_input != lpToken0 && _input != lpToken1, "Base Strat Error (Constructor): Input token cannot be the same as any of the lpTokens");
    ...
    function deposit(uint256 _amount) public whenNotPaused {
        require(
            ...
            "Base Strat Error (Deposit): Unauthorized access. Only the vault, harvester, or this contract can access this."
        );
        ...
        if (_amount > 0 && wantBal > 0) {
            ...
            IStakingRewards(masterchef).stake(_amount);
        }
    }

    function withdraw(uint256 _amount) external {
        require(msg.sender == vault, "Base Strat Error (Withdraw): Unauthorized Access. Only the vault can access this.");
    ...
    function beforeDeposit() external override {
        if (harvestOnDeposit) {
            ...
            msg.sender == address(this) ||
            msg.sender == harvester,
            "Base Strat Error (Before Deposit): Unauthorized access. Only the vault, harvester, or this contract can access this."
        );
            _harvest();
        }
    }
    ...
    function panic() external onlyManager {
        ...
        uint256 _amount = IStakingRewards(masterchef).balanceOf(address(this));
        IStakingRewards(masterchef).withdraw(_amount);
