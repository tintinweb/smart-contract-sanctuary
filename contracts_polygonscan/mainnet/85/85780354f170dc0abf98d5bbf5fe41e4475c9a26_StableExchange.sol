...
contract StableExchange is Ownable{
    ...
    function exchangeForToken(uint256 amount) public {
        ...
        require(balance >= amount, "INSUFFICIENT_FUND");
         ...
         // get decimals
        int256 decimalsBaseTokens = int8(IERC20(baseToken).decimals());
        int256 decimalsExchangeTokens = int8(IERC20(exchangeToken).decimals());
        
        // conversion
        int256 decimalConversionValue = decimalsBaseTokens - decimalsExchangeTokens;
        uint256 exp = uint256(abs(decimalConversionValue));
        bool isPositive = pos(decimalConversionValue);

        uint256 exchangeAmount = 0;        
        if ( isPositive ) {
            exchangeAmount = amount / 10 ** exp;
        } else {
            exchangeAmount = amount * 10 ** exp;
        }
