...
contract Pseudopops is ERC721, ERC721Enumerable, Ownable {
    ...
    constructor() ERC721("PseudoPops", "SUDOPOPS") {
        _owner = msg.sender;
        maxSupply = 10_000;
        maxPerTx = 6;
    }
    ...
    function pourOneOutForDaHommies(uint256 tokenId) public {
        require(msg.sender == ownerOf(tokenId), "Not the owner");
        bottleCaps.burn(msg.sender, SHAKEITUPPRICE);
        sodas[tokenId].spilled =
            sodas[tokenId].spilled +
            (block.timestamp - sodas[tokenId].lastPoured);
        sodas[tokenId].lastPoured = block.timestamp;
    }
    ...
    function _beforeTokenTransfer(
        ...
        sodas[tokenId].poured = block.timestamp;
        sodas[tokenId].lastPoured = block.timestamp;
