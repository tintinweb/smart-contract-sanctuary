...
interface IMeebMasterFactory {
    ...
    function fusion() external view returns (address);

    function totalBurned() external view returns (uint256);

    function totalFusion() external view returns (uint256);

    function totalAging() external view returns (uint256);
    ...
    function mintTokenFast(address receiver, uint256 tokenId) external;
    ...
    function agingToken(uint256 tokenId) external returns (uint256 _agedId, uint256 _agingFee);
}

interface IMeebMasterGenesisSale {
    function getItemPurchaseDate(uint256 tokenId) external view returns (uint256);

    function listForSale(uint256[] memory _tokenIds, uint256 _price) external;
}

contract MeebMasterGenesisMinter is IERC721Receiver, OwnableUpgradeable, ReentrancyGuard {
    ...
    function setGenesis(address _genesisSale) external onlyOwner {
    ...
    function mintAndListToken(uint256 _tokenId, uint256 _price) public onlyAdmin {
        IMeebMasterFactory _factory = IMeebMasterFactory(factory_);
        (, , uint256 birthDate, , ) = _factory.tokenInfo(_tokenId);
        if (birthDate == 0) {
            _factory.mintTokenFast(address(this), _tokenId);
            IERC721(nft_).approve(genesisSale_, _tokenId);
            uint256[] memory _tokenIds = new uint256[](1);
            _tokenIds[0] = _tokenId;
            IMeebMasterGenesisSale(genesisSale_).listForSale(_tokenIds, _price);
        }
    }

    function mintAndListTokens(uint256[] memory _tokenIds, uint256 _price) external onlyAdmin {
        uint256 length = _tokenIds.length;
        for (uint256 i = 0; i < length; i++) {
            mintAndListToken(_tokenIds[i], _price);
        }
    }
