...
contract VaultGetter {
    ...
    struct VaultData {
        VaultFacts facts; //immutables stored in this contract
        VaultInfo info; //fetched live
        VaultUser user; //user-specific data
    ...
    error AlreadySynced(uint len);
    error InvalidRange(uint start, uint end);
    function sync(address vaultHealerAddress) external {
        ...
        if (vhLen == factLen) revert AlreadySynced(vhLen);
    ...
    function getVault(address vaultHealerAddress, address priceGetterAddress, address user, uint pid) public view returns (VaultData memory v) {
        VaultHealer vaultHealer = VaultHealer(vaultHealerAddress);
        IPriceGetter priceGetter = IPriceGetter(priceGetterAddress);

        v.facts = vaultFacts[vaultHealerAddress][pid];

        IStrategy strat = IStrategy(v.facts.strategyAddress);
        try strat.paused() returns (bool _paused) {
            v.info.paused = _paused;
        } catch {}
        try strat.wantLockedTotal() returns (uint _wantLockedTotal) {
            v.info.wantLockedTotal = _wantLockedTotal;
        } catch{ }
        try strat.burnedAmount() returns (uint _burnedAmount) {
            v.info.burnedAmount = _burnedAmount;
        } catch{ }
        try priceGetter.getLPPrice(v.facts.wantAddress, 18) returns (uint256 price) {
            v.info.lpTokenPrice = price;
        } catch {}
        if (v.facts.masterchefAddress != address(0)) {

            try IMasterchef(v.facts.masterchefAddress).totalAllocPoint() returns (uint points) {
                v.info.totalAllocPoint = points;

                address chef = v.facts.masterchefAddress;
                bytes memory input = abi.encodeWithSignature("poolInfo(uint256)",v.facts.chefPid  );
                bytes memory returndata = Address.functionStaticCall(chef, input);

                if (returndata.length == 0x60) { //MiniChefV2
                    (,,v.info.allocPoint) = abi.decode(returndata,(uint128,uint64,uint64));
                } else if (returndata.length > 0x60) {
                    (,v.info.allocPoint,) = abi.decode(returndata,(address,uint256,bytes));
                }
        }   catch {}
        }
        if (user != address(0)) {
            IERC20 wantToken = IERC20(v.facts.wantAddress);
            try wantToken.allowance(user, vaultHealerAddress) returns (uint allowance) {
                v.user.allowance = allowance;
            } catch {}
            try wantToken.balanceOf(user) returns (uint _balance) {
                v.user.tokenBalance = _balance;
            } catch {}
            
            try vaultHealer.userInfo(pid, user) returns (uint shares) {
                v.user.stakedBalance = shares;
                uint numerator = shares * v.info.wantLockedTotal;
                if (numerator > 0) try IStrategy(v.facts.strategyAddress).sharesTotal() returns (uint sharesTotal) {
                    if (sharesTotal > 0) {
                        v.user.stakedWantBalance = numerator / sharesTotal;
                        v.user.stakedBalanceUSD = v.user.stakedWantBalance * v.info.lpTokenPrice;
                    }
                } catch {}
            } catch{}
        }
    ...
    function getVaults(address vaultHealerAddress, address priceGetterAddress, address user) external view returns (VaultData[] memory vaults) {
        return getVaults(vaultHealerAddress, priceGetterAddress, user, 0, 0);
    }

    function getVaults(address vaultHealerAddress, address priceGetterAddress, address user, uint start, uint end) public view returns (VaultData[] memory vaults) {
        uint len = vaultFacts[vaultHealerAddress].length;
        if (end == 0) end = len;
        if (start >= end || end > len) revert InvalidRange(start, end); 

        vaults = new VaultData[](end - start);
        
        for (uint i = start; i < end; i++) {
            vaults[i] = getVault(vaultHealerAddress, priceGetterAddress, user, i);
        }
