...
contract ZapHandlerV1 is Ownable, IZapHandler, ReentrancyGuard {
    ...
    struct Factory {
        address factory;
        uint256 amountsOutNominator;
        uint256 amountsOutDenominator;
    }
    ...
    event FactorySet(address indexed factory, bool indexed alreadyExists, uint256 amountsOutNominator, uint256 amountsOutDenominator);
    event FactoryRemoved(address indexed factory);
    event RouteAdded(IERC20 indexed from, IERC20 indexed to, bool indexed alreadyExists);
    ...
    function setFactory(
        ...
        require(amountsOutDenominator >= amountsOutNominator, "!denom > nom");
        require(factory != address(0), "!zero reserved for subroutes");
        ...
        emit FactorySet(factory, alreadyExists, amountsOutNominator, amountsOutDenominator);
    }
    ...
    function handleSwap(RouteStep memory step, address recipient) internal {
        if (address(step.from) == address(0)) {
            ...
            (uint256 reserve0, uint256 reserve1, ) = step.pair.getReserves();
            bool notInverted = address(step.from) < address(step.to);
            (uint256 reserveIn, uint256 reserveOut) = notInverted
                ? (reserve0, reserve1)
                : (reserve1, reserve0);
            ...
            (uint256 amount0Out, uint256 amount1Out) = notInverted
                ? (uint256(0), amountOut)
                : (amountOut, uint256(0));
            step.pair.swap(amount0Out, amount1Out, recipient, "");
        }
    }
    ...
    function generateRoute(
        ...
        require(route[0] == token0, "!token0 not route beginning");
        require(route[route.length - 1] == token1, "!token1 not route ending");
        ...
        for (uint256 i = 1; i < route.length; i += 2) {
            ...
            if (factory == address(0)) {
                ...
                require(factorySet.contains(factory), "factory does not exist");
                ...
                routes[from][to].push(RouteStep({
                    ...
                    to: to,
                    amountsOutNominator: factories[factory].amountsOutNominator,
                    amountsOutDenominator: factories[factory].amountsOutDenominator
                }));
            }
