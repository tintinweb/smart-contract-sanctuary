...
contract ShrigmaERC20Token is ERC20Permit, VaultOwned {
    ...
    modifier onlyAuthorized() {
        require(msg.sender == vault() || msg.sender == presale);
        _;
    }

    constructor() ERC20("ShrigmaDAO", "SHRIGMA", 9) {
    }

    function _beforeTokenTransfer(address from_, address to_, uint256 amount_) internal override {
        require(!locked || transferAllowed[from_] || transferAllowed[tx.origin], "Shrigma: unable to transfer");
    }

    function mint(address account_, uint256 amount_) external onlyAuthorized() {
