...
contract PairOracle is Ownable, IPairOracle {
    ...
    uint256 MISSING_DECIMALS_0;
    uint256 MISSING_DECIMALS_1;
    ...
    constructor(address pairAddress) {
        ...
        MISSING_DECIMALS_0 = 18 - uint256(IERC20_metadata(token0).decimals());
        MISSING_DECIMALS_1 = 18 - uint256(IERC20_metadata(token1).decimals());
    ...
    function consult(address token, uint256 amountIn)external view override returns (uint256 amountOut){
        if (token == token0) {
            amountOut = price0Average.mul(amountIn * (10**MISSING_DECIMALS_0) ).decode144();
        } else {
            require(token == token1, "PairOracle: INVALID_TOKEN");
            amountOut = price1Average.mul(amountIn * (10**MISSING_DECIMALS_1) ).decode144();
        }
    }
