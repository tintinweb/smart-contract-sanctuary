...
contract KoiosTitanTokenBulk is ERC20PresetMinterPauser { // this adds the mint function
    ...
    bytes32 public constant BULKTRANSFER_ROLE   = keccak256("BULKTRANSFER_ROLE"); // whitelist for bulk transfers
    ...
    function transferBulk(address[] calldata recipients, uint256[] calldata amounts) public returns (bool) {      
        require(recipients.length == amounts.length,"Same length required");
        require(hasRole(BULKTRANSFER_ROLE, _msgSender()), "Must have bulktransfer role");
        for(uint256 i=0; i<recipients.length; i++)          
           _transfer(msg.sender, recipients[i], amounts[i]); // also requires TRANSFER_ROLE
        return true;
    }    
}