...
contract RewardEstimator is ExponentialNoError {
    ...
    function calculateReward(
        RewardControllerInterface controller,
        address holder,
        ...
        for (uint256 i = 0; i < rTokens.length; i++) {
            ...
            uint256 borrowIndexMantissa = calculateRewardBorrowIndex(controller, address(rToken), borrowIndex);
            rewardAccrued = add_(
                rewardAccrued,
                calculateBorrowerReward(controller, address(rToken), holder, borrowIndexMantissa)
            );

            uint256 supplyIndexMantissa = calculateRewardSupplyIndex(controller, address(rToken));
            rewardAccrued = add_(
                rewardAccrued,
                calculateSupplierReward(controller, address(rToken), holder, supplyIndexMantissa)
            );
        }
    ...
    function calculateRewardSupplyIndex(RewardControllerInterface controller, address rToken)
        internal
        view
        returns (uint256)
    {
        (uint256 supplyStateIndex, uint256 supplyStateBlock) = controller.rewardSupplyState(rToken);
        uint256 supplySpeed = controller.rewardSpeeds(rToken);
    ...
    function calculateRewardBorrowIndex(
        ...
        (uint256 borrowStateIndex, uint256 borrowStateBlock) = controller.rewardBorrowState(rToken);
        uint256 borrowSpeed = controller.rewardSpeeds(rToken);
        uint256 blockNumber = block.number;
    ...
    function calculateBorrowerReward(
        ...
        if (borrowerIndex.mantissa > 0) {
            ...
            uint256 borrowerAmount = div_(RToken(rToken).borrowBalanceStored(borrower), Exp(borrowIndexMantissa));
