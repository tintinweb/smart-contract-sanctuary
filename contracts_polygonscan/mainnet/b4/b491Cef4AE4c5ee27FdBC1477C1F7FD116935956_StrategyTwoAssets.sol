...
abstract contract BaseStrategyMasterChef is BaseStrategy {
    ...
    function deposit() public override {
        ...
        if (_want > 0) {
            ...
            IMasterChef(rewards).stake(_want);
        }
    }
    ...
    function _getReward() internal {
        IMasterChef(rewards).claimReward();
    ...
    function emergencyWithdraw() public onlyOwner {
        IMasterChef(rewards).withdrawAndClaim();
...
abstract contract StrategyFarmTwoAssets is BaseStrategyMasterChef {
    ...
    address public constant pyq = 0x5a3064CbDCCF428ae907796cF6aD5a664CD7F3d8;
    address public constant pusd = 0x9aF3b7DC29D3C4B1A5731408B6A9656fA7aC3b72;
    ...
    // Uniswap swap paths
    address[] public pyq_pusd_path;
    address[] public pyq_usdc_path;
    ...
    {
        pyq_usdc_path = new address[](2);
        pyq_usdc_path[0] = pyq;
        pyq_usdc_path[1] = usdc;

        pyq_pusd_path = new address[](3);
        pyq_pusd_path[0] = pyq;
        pyq_pusd_path[1] = usdc;
        pyq_pusd_path[2] = pusd;
    }
    ...
    function harvest() public override {
        ...
        //Swap 1/2 PYQ for PUSD
        uint256 _pyqBalance = IERC20(pyq).balanceOf(address(this));
        if (_pyqBalance > harvestCutoff){
            _swapUniswapWithPathForFeeOnTransferTokens(pyq_pusd_path, _pyqBalance.div(2));
            uint256 _pyqBalancePost = IERC20(pyq).balanceOf(address(this));
            _swapUniswapWithPathForFeeOnTransferTokens(pyq_usdc_path, _pyqBalancePost);

            // Add liquidity for PUSD/USDC
            uint256 _usdc = IERC20(usdc).balanceOf(address(this));
            uint256 _pusd = IERC20(pusd).balanceOf(address(this));
            if (_usdc > 0 && _pusd > 0) {
                IERC20(usdc).safeApprove(currentRouter, 0);
                IERC20(usdc).safeApprove(currentRouter, _usdc);
                IERC20(pusd).safeApprove(currentRouter, 0);
                IERC20(pusd).safeApprove(currentRouter, _pusd);
                ...
                IERC20(pusd).safeTransfer(
                    strategist,
                    IERC20(pusd).balanceOf(address(this))
                );
            }
...
contract StrategyTwoAssets is StrategyFarmTwoAssets {
    // Token addresses
    address public POLYQUITY_MASTER_CHEF = 0xCB881b62A97Cb733D01C59Eb8326B239E283E6a4;
    address public PUSD_USDC_LP = 0xc30D6Bc42911Aa21A63E51c7121B33b3E65cC3c4;

    constructor()
        public
        StrategyFarmTwoAssets(
            POLYQUITY_MASTER_CHEF,
            PUSD_USDC_LP,
            msg.sender
        )
    {}
