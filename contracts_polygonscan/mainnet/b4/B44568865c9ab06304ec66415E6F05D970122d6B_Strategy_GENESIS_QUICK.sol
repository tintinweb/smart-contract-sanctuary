...
contract Strategy_GENESIS_QUICK is Ownable {
    // This strategy is for polygon network. GENESIS - QUICK pair. RewardToken = "GENESIS, dQUICK"
    ...
    address public token0; // GENESIS
    address public token1; // QUICK
    address public vault;

    // ------------> Important constants <--------------
    bool public hasPid = false;
    address public constant lpToken = 0xF0696be85fa54F7a8C9F20AA98aA4409CD5C9D1B;
    address public constant farm = 0x3620418dD43853c35fF8Df90cAb5508FB5df46Bf;
    address public router = 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff; // uses for swapping GENESIS - QUICK
    ...
    function autoCompound() external {
        ...
        if (getAmountLPFromFarm() > 0) {
            ...
            _swapExactTokenForToken(router, token0, token1, getBalanceOfToken(token0));
    ...
    function _takeFee() internal {
        uint256 rewardsFeeInQuick = _calculateAmountFee(getBalanceOfToken(token1));
        bool _swapped = _swapExactTokenForToken(router, token1, USDT, rewardsFeeInQuick+pendingFee);
        if (_swapped) {
            pendingFee = 0;
            emit TakeFee(rewardsFeeInQuick);
        } else {
            pendingFee += rewardsFeeInQuick;
        }
    }
    ...
    function _addLiquidityFromAllTokens() internal {
        _swapExactTokenForToken(router, WMATIC, token1, getBalanceOfToken(WMATIC));
        _swapExactTokenForToken(router, token1, token0, getBalanceOfToken(token1)/2);
    ...
    function _getSimpleTCI(uint256 _amount) internal view returns (uint256 tCI) {
        ...
        if(_amount > 0) {
            ...
            tCI = (t1V + _getToken0Price() * t0V / d) * _getPriceWMATICToQUICK() / d;
        }
        ...
        if (_balance1 > 0) {
            tCI += (_balance1 + _getToken0Price() * _balance0 / d) * _getPriceWMATICToQUICK() / d;
        }
    ...
    function _getSimpleTC() internal view returns (uint256 TC) {
        ...
        if(_totalLP > 0) {
            ...
            if(t0V != 0) {
                path[0] = token0;
                path[1] = token1;
                amount = _getAmountsOut(router, t0V, path);
            }
            if(t1V + amount != 0) {
                path[0] = token1;
                path[1] = WMATIC;
                TC = _getAmountsOut(router, t1V + amount, path);
            }
        }
