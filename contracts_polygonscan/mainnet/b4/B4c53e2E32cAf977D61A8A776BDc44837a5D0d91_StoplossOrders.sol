...
contract StoplossOrders is IModule, ReentrancyGuard {
    /// @notice receive ETH
    ...
    function _transferAmount(
        ...
        if (address(_token) == ETH) {
            ...
            require(
                success,
                "StoplossOrders#_transferAmount: ETH_TRANSFER_FAILED"
            );
        } else {
            require(
                SafeERC20.transfer(_token, _to, _amount),
                "StoplossOrders#_transferAmount: TOKEN_TRANSFER_FAILED"
            );
        }
    }
}