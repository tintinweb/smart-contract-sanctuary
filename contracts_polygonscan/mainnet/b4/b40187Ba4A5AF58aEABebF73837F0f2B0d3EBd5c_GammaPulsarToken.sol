...
contract GammaPulsarToken is ERC20('GammaPulsarToken', 'GPUL') {
    ...
    function updateTransferPvpComission(uint256 _transferPvpComission) external onlyDeveloper {
        require(_transferPvpComission <= MAXIMUM_TRANSFER_PVP_COMISSION, "GPUL::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount){
        ...
        if (recipient == BURN_ADDRESS || (transferTaxRate == 0 && transferPvpComission == 0)) {
            ...
            pvpTaxAmount = amount.mul(transferPvpComission).div(10000);
        }
