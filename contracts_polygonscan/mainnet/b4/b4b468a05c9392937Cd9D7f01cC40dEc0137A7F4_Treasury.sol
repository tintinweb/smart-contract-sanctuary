...
contract Treasury is ITreasury, Ownable, ReentrancyGuard {
    ...
    address public oracleCollateral; // wbtc oracle collateral
    ...
    uint256 public rebalance_cooldown = 12 hours; //bayback and recolaterize restriction
    ...
    modifier withOracleUpdates() {
        if (oracleDollar != address(0))
            IOracle(oracleDollar).updateIfRequired(); //custom token oracle
        if (oracleShare != address(0)) IOracle(oracleShare).updateIfRequired(); //custom token oracle
        if (oracleGovToken != address(0))
            IOracle(oracleGovToken).updateIfRequired(); //custom token oracle
        if (oracleCollateral != address(0))
            IOracle(oracleGovToken).updateIfRequired(); //custom token oracle
    ...
    modifier hasRebalancePool() {
        ...
        require(
            rebalancing_pool_collateral != address(0),
            "!rebalancingPoolCollateral"
        );
        _;
    }
    ...
    modifier checkRebalanceCooldown() {
        ...
        require(
            _blockTimestamp - last_rebalance_timestamp >= rebalance_cooldown,
            "<rebalance_cooldown"
        );
        _;
    ...
    event BoughtBack(
        ...
        uint256 output_share_amount
    );
    event Recollateralized(
        uint256 share_amount,
        uint256 output_collateral_amount,
        uint256 output_collateral_value
    );
    ...
    event New_OracleCollateralToken(address _oracle);
    ...
    constructor(uint256 _startTime, uint256 _epoch_length) {
        require(
            ...
            "Start time initialized to the past"
        );
    ...
    /*=========== VIEWS ===========*/
    function dollarPrice() public view override returns (uint256) {
    ...
    function redemption_fee_adjusted(address _caller)
        public
        view
        returns (uint256 redemptionFee)
    {
        ...
        if (
            IERC20(governanceToken).balanceOf(_caller) >=
            discount_requirenment()
        ) return redemption_fee / 2;
    ...
    function discount_requirenment() public view returns (uint256) {
        ...
        return
            (gov_token_value_for_discount * PRICE_PRECISION * 10**decimals) /
            govTokenPrice;
    }
    ...
    function globalCollateralValue() public view returns (uint256) {
        ...
        for (uint256 i = 0; i < pools_array.length; i++) {
            if (pools_array[i] != address(0)) {
                total_collateral_value += IPool(pools_array[i])
    ...
    function calcEffectiveCollateralRatio() public view returns (uint256) {
        uint256 total_collateral_value = globalCollateralValue(); // 1000$
        uint256 total_supply_dollar = ((IERC20(dollar).totalSupply() *
            dollarPrice()) / PRICE_PRECISION); //2000 agousd
        uint256 ecr = (total_collateral_value * PRICE_PRECISION) /
            total_supply_dollar;
    ...
    function refreshCollateralRatio() public withOracleUpdates {
        ...
        uint256 priceTarget = IOracle(oracleCollateral).consult();

        if (current_dollar_price > priceTarget + price_band) {
            target_collateral_ratio = target_collateral_ratio <= ratio_step
                ? 0
                : target_collateral_ratio - ratio_step;
        } else if (current_dollar_price < priceTarget - price_band) {
            target_collateral_ratio = target_collateral_ratio + ratio_step >=
    ...
    function calcCollateralBalance()
        public
        view
        returns (uint256 _collateral_value, bool _exceeded)
    {
        uint256 total_collateral_value = globalCollateralValue();
        uint256 target_collateral_value = (IERC20(dollar).totalSupply() *
            target_collateral_ratio) / PRICE_PRECISION;
    ...
    function _swap(
        ...
        uint256[] memory out_amounts = IUniswapRouter(uniswap_router)
    ...
    function removePool(address pool_address) public onlyOwner notMigrated {
        require(
            rebalancing_pool != pool_address,
            "Cant`t delete active rebalance pool"
        );
    ...
    function buyback(uint256 _collateral_value, uint256 _min_share_amount)
        external
        onlyStrategist
        withOracleUpdates
        notMigrated
    ...
    {
        (
            uint256 _excess_collateral_value,
            bool _exceeded
        ) = calcCollateralBalance();
        require(_exceeded && _excess_collateral_value > 0, "!exceeded");
        require(
            _collateral_value > 0 &&
                _collateral_value < _excess_collateral_value,
            "invalidCollateralAmount"
        );

        uint256 _collateral_price = IPool(rebalancing_pool)
            .getCollateralPrice();
        uint256 _collateral_amount_sell = (_collateral_value *
            PRICE_PRECISION) / _collateral_price;

        require(
            IERC20(rebalancing_pool_collateral).balanceOf(rebalancing_pool) >
                _collateral_amount_sell,
            "insufficentPoolBalance"
        );
        IPool(rebalancing_pool).transferCollateralToTreasury(
            _collateral_amount_sell
        ); // Transfer collateral from pool to treasury
        uint256 out_share_amount = _swap(
    ...
    {
        (
            uint256 _deficit_collateral_value,
            bool _exceeded
        ) = calcCollateralBalance();
        require(!_exceeded && _deficit_collateral_value > 0, "exceeded");
        require(
            _min_collateral_amount <= _deficit_collateral_value,
            ">deficit"
        );
        uint256 _share_balance = IERC20(share).balanceOf(address(this));
        require(_share_amount <= _share_balance, ">shareBalance");
        uint256 out_collateral_amount = _swap(
            share,
            _share_amount,
            _min_collateral_amount
        );
        uint256 _collateral_balance = IERC20(rebalancing_pool_collateral)
            .balanceOf(address(this));
        if (_collateral_balance > 0) {
            IERC20(rebalancing_pool_collateral).safeTransfer(
                rebalancing_pool,
                _collateral_balance
            ); // Transfer collateral from Treasury to Pool
        }
        uint256 collateral_price = IPool(rebalancing_pool).getCollateralPrice();
        uint256 out_collateral_value = (out_collateral_amount *
            collateral_price) / PRICE_PRECISION;

        emit Recollateralized(
            _share_amount,
            out_collateral_amount,
            out_collateral_value
        );
    }
        ...
        checkEpoch
    {
        ...
        uint256 _collateral_price = IPool(rebalancing_pool)
            .getCollateralPrice();
        uint256 missing_decimals = IPool(rebalancing_pool)
            .getMissing_decimals();
        uint256 _collateral_amount_allocate = ((_excess_collateral_value *
            PRICE_PRECISION) / _collateral_price) / (10**missing_decimals);

        IPool(rebalancing_pool).transferCollateralToTreasury(
            _collateral_amount_allocate
        ); // Transfer collateral from pool to treasury
        IERC20(rebalancing_pool_collateral).safeApprove(foundry, 0);
        IERC20(rebalancing_pool_collateral).safeApprove(
            foundry,
            _collateral_amount_allocate
        ); // div for 18 - decimals
    ...
    function setOracleDollar(address _oracle)
        public
        onlyOwner
        nonZeroAddress(_oracle)
    {
        oracleDollar = _oracle;
        emit New_OracleDollar(oracleDollar);
    }

    function setOracleShare(address _oracle)
        public
        onlyOwner
        nonZeroAddress(_oracle)
    {
    ...
    function setOracleGovToken(address _oracle)
        public
        onlyOwner
        nonZeroAddress(_oracle)
    {
    ...
    function setOracleCollateralToken(address _oracle)
        public
        onlyOwner
        nonZeroAddress(_oracle)
    {
        oracleCollateral = _oracle;
        emit New_OracleCollateralToken(oracleCollateral);
    }

    function setFoundry(address _foundry)
        public
        onlyOwner
        nonZeroAddress(_foundry)
    {
        foundry = _foundry;
    ...
    function setRebalancePool(address _rebalance_pool) public onlyOwner {
        ...
        require(
            IPool(_rebalance_pool).getCollateralToken() != address(0),
            "!poolCollateralToken"
        );

        rebalancing_pool = _rebalance_pool;
        rebalancing_pool_collateral = IPool(_rebalance_pool)
            .getCollateralToken();
