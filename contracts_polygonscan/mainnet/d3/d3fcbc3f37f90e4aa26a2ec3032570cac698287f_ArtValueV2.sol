    ...
    OwnableUpgradeable
{
    ...
    ***********/
    ...
    function mintWithNumberBulk(
        address to,
        uint256[] calldata numbers
    ) public returns (uint256[] memory) {
        require(hasRole(MINTER_ROLE, _msgSender()), "ArtValueNumber: must have minter role");

        uint nbro_numbers = numbers.length;
        uint256[] memory tokenIds = new uint256[](nbro_numbers);
        for (uint i = 0; i < nbro_numbers; i++) {
    ...
    function mintAsPlaceholderBulk(
        address to,
        uint nbro_placeholders
    ) public returns (uint256[] memory) {
        require(hasRole(MINTER_ROLE, _msgSender()), "ArtValueNumber: must have minter role");

        uint256[] memory tokenIds = new uint256[](nbro_placeholders);
        for (uint i = 0; i < nbro_placeholders; i++) {
            tokenIds[i] = _mintWithAutoId(to);
        }
