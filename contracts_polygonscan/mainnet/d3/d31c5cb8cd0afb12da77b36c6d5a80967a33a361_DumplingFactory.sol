...
contract UniswapV2ERC20 {
    ...
    string public constant name = 'DumplingSwap LP Token';
...
contract DumplingPair is UniswapV2ERC20 {
    using SafeMathUniswap  for uint;
...
contract DumplingFactory is IUniswapV2Factory {
    ...
    function pairCodeHash() external pure returns (bytes32) {
        return keccak256(type(DumplingPair).creationCode);
    }
    ...
    function createPair(address tokenA, address tokenB) external override returns (address pair) {
        ...
        bytes memory bytecode = type(DumplingPair).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(token0, token1));
        assembly {
            pair := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        DumplingPair(pair).initialize(token0, token1);
