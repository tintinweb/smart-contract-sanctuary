...
abstract contract BaseStrategyMasterChef is BaseStrategy {
    ...
    function getHarvestable() external override view returns (uint256) {
        return IMasterChef(rewards).pendingELYSM(poolId, address(this));
    }
...
abstract contract StrategyFarmBase is BaseStrategyMasterChef {
    ...
    address public constant ELYSM = 0x7917FB62b993511320Eee5ad70E98D49356580C9;
    ...
    // Uniswap swap paths
    address[] public ELYSM_usdc_path;
    ...
    {
        ELYSM_usdc_path = new address[](2);
        ELYSM_usdc_path[0] = ELYSM;
        ELYSM_usdc_path[1] = usdc;
    }
    ...
    function harvest() public override {
        ...
        //Swap 
        uint256 _ELYSMBalance = IERC20(ELYSM).balanceOf(address(this));
        if (_ELYSMBalance > harvestCutoff){
            
            //Send performance fee to strategist
            uint256 performanceFee = _ELYSMBalance.mul(keepFXS).div(keepFXSmax);
            if (performanceFee>0) {
                _swapUniswapWithPath(ELYSM_usdc_path, performanceFee);
            ...
            // Swap rest
            _ELYSMBalance = IERC20(ELYSM).balanceOf(address(this));
            _swapUniswapWithPath(ELYSM_usdc_path, _ELYSMBalance.div(2));

            // Add liquidity
            uint256 _ELYSM = IERC20(ELYSM).balanceOf(address(this));
            uint256 _usdc = IERC20(usdc).balanceOf(address(this));
            if (_ELYSM > 0 && _usdc > 0) {
                IERC20(ELYSM).safeApprove(currentRouter, 0);
                IERC20(ELYSM).safeApprove(currentRouter, _ELYSM);
                ...
                // Donates DUST
                IERC20(ELYSM).safeTransfer(
                    strategist,
                    IERC20(ELYSM).balanceOf(address(this))
                );
                IERC20(usdc).safeTransfer(
                    strategist,
                    IERC20(usdc).balanceOf(address(this))
...
contract StrategyBase is StrategyFarmBase {
    // Token addresses
    address public ELYSM_MASTER_CHEF = 0xe681c22Dc729E88559a0607ACa4b136Cc9998A6F ;
    address public ELYSM_USDC_LP = 0xD52bF3AC296F9ed1171e48e5ef248Fb217fBfCfD;
