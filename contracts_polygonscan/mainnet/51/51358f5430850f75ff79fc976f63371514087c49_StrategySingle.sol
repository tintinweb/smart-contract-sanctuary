...
abstract contract BaseStrategy is Ownable {
    ...
    address public constant dfynRouter = 0xA102072A4C07F06EC3B4900FDC4C7B80b6c57429; //Dfyn router
    ...
    function _swapUniswapWithPathDfyn(
        address[] memory path,
        uint256 _amount
    ) internal {
        require(path[1] != address(0));
        ...
        // Swap with uniswap
        IERC20(path[0]).safeApprove(dfynRouter, 0);
        IERC20(path[0]).safeApprove(dfynRouter, _amount);

        IUniswapRouterV2(dfynRouter).swapExactTokensForTokens(
            _amount,
            0,
            path,
            address(this),
...
abstract contract StrategyFarmSingle is BaseStrategyMasterChef {
    ...
    address public constant rewardToken = 0x692597b009d13C4049a947CAB2239b7d6517875F;
    ...
    {
        ust_usdt_path = new address[](2);
        ust_usdt_path[0] = ust;
        ust_usdt_path[1] = usdt;

        usdt_dino_path = new address[](3);
        usdt_dino_path[0] = usdt;
        usdt_dino_path[1] = usdc;
        usdt_dino_path[2] = dino;
    }
    ...
    function harvest() public override {
        ...
        if (_harvestBalance > harvestCutoff){
            _swapUniswapWithPathDfyn(ust_usdt_path, _harvestBalance);

            //Send performance fee to strategist
            uint256 _usdt = IERC20(usdt).balanceOf(address(this));
            if (_usdt > 0) {
                uint256 performanceFee = _usdt.mul(keepFXS).div(keepFXSmax);
                if (performanceFee>0) {
            ...
            //Swap for Dino
            uint256 _usdtPost = IERC20(usdt).balanceOf(address(this));
            if (_usdtPost > 0) {
                _swapUniswapWithPath(usdt_dino_path, _usdtPost);
            }
        }
...
contract StrategySingle is StrategyFarmSingle {
    // Token addresses
    address public DINO_MASTER_CHEF = 0x80F23e90f8D7d6f5e3f602B1E26C7b5Fa4E530d3;
