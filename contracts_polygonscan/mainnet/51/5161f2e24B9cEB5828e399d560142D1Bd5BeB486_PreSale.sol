...
contract Token is ERC20('pAPOLLO', 'pAPOLLO'){
...
contract PreSale is ReentrancyGuard, Ownable {
    ...
    ERC20 tmpToken;
    ERC20 iris; // 0xdaB35042e63E93Cc8556c9bAE482E5415B5Ac4B1;
    ...
    uint256 public pAPOLLOPrice;
    uint256 public IRISPrice;

    uint256 public maxTokenPurchase = 4000 ether;
    uint256 public maxTokenPurchasePantheon = 1000 ether;
    uint256 public maxTokenPurchaseHeroes = 1000 ether;
    ...
    mapping(address => bool) public userIsPantheon;
    mapping(address => bool) public userIsHeroes;
    mapping(address => uint256) public userTokenTally;

    event tokenPurchased(address sender, uint256 tokenReceived, uint256 usdcSpent, uint256 irisSpent);
    ...
    constructor(uint256 _startBlock, uint256 _endBlock, uint8 _ratio,
        uint256 _pAPOLLOPrice, uint256 _IRISPrice,
        address _tmpToken, address _iris, address _usdc) {
        startBlock = _startBlock;
        endBlock   = _endBlock;
        ratio = _ratio;
        pAPOLLOPrice = _pAPOLLOPrice;
        IRISPrice = _IRISPrice;
        tmpToken = ERC20(_tmpToken);
        iris = ERC20(_iris);
        usdc = ERC20(_usdc);
        tmpToken.balanceOf(address(this));
        iris.balanceOf(address(this));
    ...
    function quoteAmountInPAPOLLO(uint256 amount) public view returns(uint256){
        return amount.mul(pAPOLLOPrice).div(100);
    }
    // pass any pApollo and get the % amount in USDC
    function quoteAmountInUSDC(uint256 amount) public view returns(uint256){
        uint256 amountInPAPPOLO = quoteAmountInPAPOLLO(amount);
        return amountInPAPPOLO.mul(ratio).div(100);
    }
    // pass any pApollo and get the % amount in IRIS
    function quoteAmountInIRIS(uint256 amount) public view returns(uint256){
        uint256 amountInPAPPOLO = quoteAmountInPAPOLLO(amount);
        uint256 amountInUsdc = quoteAmountInUSDC(amount);
        uint256 amountInUsdcInIRIS = amountInPAPPOLO.sub(amountInUsdc);
        return amountInUsdcInIRIS.div(IRISPrice).mul(100);
    }
    // pass any pApollo and get quotes in USDC and IRIS
    function quoteAmounts(uint256 requestedAmount, address user) public view
    returns(uint256 pAppoloInUSD, uint256 inUsdc, uint256 amountIRIS, uint256 limit, uint256 tokenPurchaseAmount){
        (tokenPurchaseAmount, limit) = getUserLimit(requestedAmount, user);
        pAppoloInUSD = quoteAmountInPAPOLLO(tokenPurchaseAmount);
        inUsdc = quoteAmountInUSDC(tokenPurchaseAmount);
        amountIRIS = quoteAmountInIRIS(tokenPurchaseAmount);
    }
    function getUserLimit(uint256 tokenPurchaseAmount, address user) public view returns(uint256, uint256){
        ...
        if(userIsPantheon[user])
            limit = limit.add(maxTokenPurchasePantheon);
        if(userIsHeroes[user])
            limit = limit.add(maxTokenPurchaseHeroes);
    ...
    function buy( uint256 amount ) external payable nonReentrant {
        ...
        (uint256 pAppoloInUSD, uint256 inUsdc, uint256 amountIRIS, uint256 limit, uint256 tokenPurchaseAmount) = quoteAmounts(amount, msg.sender);
        ...
        require(amountIRIS > 0, "user cannot pay 0 IRIS");
        ...
        uint256 userIrisBalance = iris.balanceOf(address(msg.sender));

        require(inUsdc <= userUsdcBalance, "USDC balance is too low");
        require(amountIRIS <= userIrisBalance, "IRIS balance is too low");

        usdc.safeTransferFrom(msg.sender, feeAddress, inUsdc);
        iris.safeTransferFrom(msg.sender, feeAddress, amountIRIS);
        tmpToken.safeTransfer(msg.sender, tokenPurchaseAmount);

        emit tokenPurchased(msg.sender, tokenPurchaseAmount, inUsdc, amountIRIS);
    ...
    function setpAPOLLOPrice(uint256 _pAPOLLOPrice) external onlyOwner {
        pAPOLLOPrice = _pAPOLLOPrice;
    }
    function setIRISPrice(uint256 _IRISPrice) external onlyOwner {
        IRISPrice = _IRISPrice;
    }
    ...
    function setMaxTokenPurchasePantheon(uint256 _value) external onlyOwner {
        maxTokenPurchasePantheon = _value;
    }
    function setMaxTokenPurchaseHeroes(uint256 _value) external onlyOwner {
        maxTokenPurchaseHeroes = _value;
    }
    function setFeeAddress(address _feeAddress) external onlyOwner {
        feeAddress = _feeAddress;
    }
    function setToken(address _token) external onlyOwner {
        tmpToken = ERC20(_token);
        tmpToken.balanceOf(address(this));
    }
    function setIris(address _iris) external onlyOwner {
        iris = ERC20(_iris);
        iris.balanceOf(address(this));
    }
    ...
    function setUserIsPantheon(address _user, bool _status) external onlyOwner {
        userIsPantheon[_user] = _status;
    }
    function setUserIsHeroes(address _user, bool _status) external onlyOwner {
        userIsHeroes[_user] = _status;
    }
