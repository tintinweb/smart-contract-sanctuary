...
contract DefarmBondingCalculator is IBondingCalculator {

    using FixedPoint for *;
    using SafeMath for uint;
    using SafeMath for uint112;

    address public immutable DeFarm;

    constructor( address _DeFarm ) {
        require( _DeFarm != address(0) );
        DeFarm = _DeFarm;
    }
    ...
    function markdown( address _pair ) external view returns ( uint ) {
        ...
        uint reserve;
        if ( IUniswapV2Pair( _pair ).token0() == DeFarm ) {
            reserve = reserve1;
        } else {
            reserve = reserve0;
        }
        return reserve.mul( 2 * ( 10 ** IERC20( DeFarm ).decimals() ) ).div( getTotalValue( _pair ) );
    }
}