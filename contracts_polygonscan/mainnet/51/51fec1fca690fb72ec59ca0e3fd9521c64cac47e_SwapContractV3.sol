...
contract SwapContractV3Base is SwapBase, ISwapContractV3 {
    ...
    function _approveToken(IERC20 tokenIn, uint256 amount) private {
     ...
     */
    function _swapTokensToOtherBlockchainV3(swapToParams memory params)
        internal
        whenNotPaused
        TransferTo(params, msg.value)
    {
        if (params.firstPath.length > 20) {
            if (params.swapExactFor){
                require(
                    _getLastBytes20(params.firstPath) ==
                        bytes20(uint160(uint256(RubicAddresses[numOfThisBlockchain]))),
                    "swapContract: the last address in the firstPath must be Rubic"
                );
                IERC20 tokenIn = IERC20(address(_getFirstBytes20(params.firstPath)));
                tokenIn.safeTransferFrom(
                    msg.sender,
                    address(this),
                    params.tokenInAmount
                );
                _approveToken(tokenIn, params.tokenInAmount);
                ...
                uint256 amountOut = IUniswapRouterV3(blockchainRouter).exactInput(
                    paramsV3
                );
                params.exactRBCtokenOut = amountOut;
                require(params.exactRBCtokenOut <= maxTokenAmount, 'swapContract: Too many RBC requested');
            }
            else{
                require(params.exactRBCtokenOut <= maxTokenAmount, 'swapContract: Too many RBC requested');
                require(
                    _getFirstBytes20(params.firstPath) ==
                        bytes20(uint160(uint256(RubicAddresses[numOfThisBlockchain]))),
                    "swapContract: the last address in the firstPath must be Rubic"
                );
                IERC20 tokenIn = IERC20(address(_getLastBytes20(params.firstPath)));
                    tokenIn.safeTransferFrom(
                    msg.sender,
                    address(this),
                    params.tokenInAmount
                ...
                _approveToken(tokenIn, params.tokenInAmount);
                IUniswapRouterV3.ExactOutputParams memory paramsV3 = IUniswapRouterV3.ExactOutputParams(
                    params.firstPath,
                    address(this),
                    block.timestamp,
                    params.exactRBCtokenOut,
                ...
                uint256 amountIn = IUniswapRouterV3(blockchainRouter).exactOutput(
                    paramsV3
                );
                tokenIn.safeTransfer(
                    _msgSender(),
                    params.tokenInAmount - amountIn
                );
                params.tokenInAmount = amountIn;
            }
        } else {
     ...
     * @notice User must have approved tokenInAmount of tokenIn
     */
    function _swapCryptoToOtherBlockchainV3(swapToParams memory params)
        internal
        whenNotPaused
        TransferTo(params, msg.value)
    {
        params.tokenInAmount = msg.value - blockchainCryptoFee[params.blockchain];
        if (params.swapExactFor){
            require(
                _getLastBytes20(params.firstPath) ==
                    bytes20(uint160(uint256(RubicAddresses[numOfThisBlockchain]))),
            ...
            IERC20 tokenIn = IERC20(address(_getFirstBytes20(params.firstPath)));
            require(
                address(tokenIn) == IUniswapRouterV3(blockchainRouter).WETH9(),
                "swapContract: the first token must be native"
            );
            IUniswapRouterV3.ExactInputParams memory paramsV3 = IUniswapRouterV3.ExactInputParams(
                params.firstPath,
                address(this),
            ...
            IUniswapRouterV3(blockchainRouter).refundETH();
            payable(_msgSender()).sendValue(params.tokenInAmount - amountIn);
            params.tokenInAmount = amountIn;
        }
    }
     ...
     */
    function _swapTokensToUserWithFeeV3(swapFromParams memory params)
        internal
        onlyRelayer
        whenNotPaused
        TransferFrom(params)
    {
        IERC20 RBCToken = IERC20(address(_getFirstBytes20(params.path)));

        require(
            _getFirstBytes20(params.path) == bytes20(uint160(uint256(RubicAddresses[numOfThisBlockchain]))),
        ...
        if (params.path.length == 20) {
            ...
            IUniswapRouterV3(blockchainRouter).exactInput(
                paramsV3
            );
        }
    ...
    /**
     * @dev Transfers crypto to end user in current blockchain
     */
    function _swapCryptoToUserWithFeeV3(swapFromParams memory params)
        internal
        onlyRelayer
        whenNotPaused
        TransferFrom(params)
    {
        require(params.path.length >= 43, 'swapContract: the path is too short');
        ...
        require(address(_getLastBytes20(params.path)) == IUniswapRouterV3(blockchainRouter).WETH9(),
        'swapContract: last address must be native');
        IUniswapRouterV3.ExactInputParams memory paramsV3 = IUniswapRouterV3.ExactInputParams(
            params.path,
            blockchainRouter,
            block.timestamp,
        ...
        IUniswapRouterV3(blockchainRouter).unwrapWETH9(
            amountOut,
            params.user
        );
     ...
     */
    function _refundTokensToUserV3(swapFromParams memory params)
        internal
        onlyRelayer
        whenNotPaused
        TransferFrom(params)
    {
        require(
            _getLastBytes20(params.path) == bytes20(uint160(uint256(RubicAddresses[numOfThisBlockchain]))),
            "swapContract: the first address in the path must be Rubic"
        );
        ...
        if (params.path.length == 20) {
            ...
            IUniswapRouterV3(blockchainRouter).exactOutput(
                paramsV3
            );
        }
    ...
    /**
     * @dev Swaps RBC from pool to initially spent by user crypto and transfers him
     * @notice There is used the same structure as in other similar functions but amountOutMin should be
     * equal to the amount of tokens initially spent by user (we are refunding them), AmountWithFee should
     * be equal to the amount of RBC tokens that the pool got after the first swap (RBCAmountIn in the event)
     * hashedParams of this originalTxHash
     */
    function _refundCryptoToUserV3(swapFromParams memory params)
        internal
        onlyRelayer
        whenNotPaused
        TransferFrom(params)
    {
        ...
        require(address(_getFirstBytes20(params.path)) == IUniswapRouterV3(blockchainRouter).WETH9(),
        'swapContract: last address must be native');
        ...
        uint256 amountOut = IUniswapRouterV3(blockchainRouter).exactOutput(
            paramsV3
        );
        IUniswapRouterV3(blockchainRouter).unwrapWETH9(
            amountOut,
...
/// @title Swap contract for multisignature bridge
contract SwapContractV3 is SwapContractV3Base{

    function swapTokensToOtherBlockchainV3(swapToParams memory params)
        external
        payable
    {
        _swapTokensToOtherBlockchainV3(params);
    }

    function swapCryptoToOtherBlockchainV3(swapToParams memory params)
        external
        payable
    {
        _swapCryptoToOtherBlockchainV3(params);
    }

    function swapTokensToUserWithFeeV3(swapFromParams memory params)
        external
    {
        _swapTokensToUserWithFeeV3(params);
    }

    function swapCryptoToUserWithFeeV3(swapFromParams memory params)
        external
    {
        _swapCryptoToUserWithFeeV3(params);
    }

    function refundTokensToUserV3(swapFromParams memory params)
        external
    {
        _refundTokensToUserV3(params);
    }

    function refundCryptoToUserV3(swapFromParams memory params)
        external
    {
        _refundCryptoToUserV3(params);
    }
}