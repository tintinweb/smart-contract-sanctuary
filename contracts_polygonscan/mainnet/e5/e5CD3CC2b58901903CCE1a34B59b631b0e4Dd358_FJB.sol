...
contract FJBDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("FJB_Dividend_Tracker", "FJB_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 250000 * (10**9); //must hold 500000+ tokens
    }

    function _transfer(address, address, uint256) internal override pure {
        require(false, "FJB_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override  pure {
        require(false, "FJB_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main FJB contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 600 && newClaimWait <= 86400, "FJB_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "FJB_Dividend_Tracker: Cannot update claimWait to same value");
...
contract FJB is ERC20, Ownable {
    ...
    FJBDividendTracker public dividendTracker;

    address public _marketingWallet = 0x8df127cE56EbA090d9cB99AC2E5a76D3b38f0D77;
    ...
    constructor() ERC20("Biden Loser Unit", "FJB") {

        dividendTracker = new FJBDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "FJB: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "FJB: Account is already the value of 'excluded'");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "FJB: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "FJB: Cannot update gasForProcessing to same value");
    ...
    function swapBack(uint256 tokensToLiquify) internal lockTheSwap {
        ...
        uint256 maticFeeFactor = totalFees.sub(liquidityFee.div(2));
        
        uint256 maticForLiquidity = maticBalance.mul(liquidityFee).div(maticFeeFactor).div(2);
        uint256 maticForReflection = maticBalance.mul(MaticRewardsFee).div(maticFeeFactor);
        uint256 maticForMarketing = maticBalance.sub(maticForLiquidity).sub(maticForReflection);
            
        addLiquidity(tokensToLP, maticForLiquidity);
    
        payable(_marketingWallet).transfer(maticForMarketing);

        (bool success,) = address(dividendTracker).call{value: maticForReflection}("");
        
        if(success) {
            emit SendDividends(tokensToLiquify.mul(MaticRewardsFee).div(totalFees), maticForReflection);
        }
    }
