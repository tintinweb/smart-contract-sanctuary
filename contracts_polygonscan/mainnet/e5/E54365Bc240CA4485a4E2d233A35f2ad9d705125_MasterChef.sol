...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        //   pending reward = (user.amount * pool.accHeatPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accHeatPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 accHeatPerShare;   // Accumulated FISHes per share, times 1e18. See below.
        uint16 depositFeeBP;      // Deposit fee in basis points
    }

    // The FISH TOKEN!
    HeatToken public heatToken;
    address public devAddress;
    address public feeAddress;
    address public vaultAddress;

    // FISH tokens created per block.
    uint256 public fishPerBlock = 0.0065 ether;
    ...
    // Referral commission rate in basis points. (10%)
    uint16 public referralCommissionRate = 500;
    // Dev commission rate in basis points. (1%)
    uint16 public devCommissionRate = 1000;
    ...
    function pendingFish(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 accHeatPerShare = pool.accHeatPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 heatReward = multiplier.mul(fishPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accHeatPerShare = accHeatPerShare.add(heatReward.mul(1e18).div(lpSupply));
        }
        return user.amount.mul(accHeatPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 currentSupply = heatToken.totalSupply();
        uint256 maxSupply = heatToken.maxSupply();
        ...
        uint256 devReward = heatReward.mul(devCommissionRate).div(10000);
        heatToken.mint(devAddress, devReward);
        heatToken.mint(address(this), heatReward);
        pool.accHeatPerShare = pool.accHeatPerShare.add(heatReward.mul(1e18).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accHeatPerShare).div(1e18).sub(user.rewardDebt);
            if (pending > 0) {
              uint256 devReward = pending.mul(devCommissionRate).div(10000);
              safeFishTransfer(msg.sender, pending.sub(devReward));
        ...
        user.rewardDebt = user.amount.mul(pool.accHeatPerShare).div(1e18);
    ...
    // Safe heatToken transfer function, just in case if rounding error causes pool to not have enough FISH.
    function safeFishTransfer(address _to, uint256 _amount) internal {
        uint256 fishBal = heatToken.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > fishBal) {
            transferSuccess = heatToken.transfer(_to, fishBal);
        } else {
            transferSuccess = heatToken.transfer(_to, _amount);
        }
    ...
    function updateEmissionRate(uint256 _heatTokenPerBlock) external onlyOwner {
        massUpdatePools();
        fishPerBlock = _heatTokenPerBlock;
        emit UpdateEmissionRate(msg.sender, _heatTokenPerBlock);
    }
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (address(referral) != address(0) && referralCommissionRate > 0) {
            ...
            if (referrer != address(0) && commissionAmount > 0) {
                heatToken.mint(referrer, commissionAmount);
