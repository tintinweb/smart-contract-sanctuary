...
contract MATIC_IOU is Ownable {
  ...
  function invest(address referrer, uint8 plan) public payable {
	...
	if(block.timestamp >= preUNIX && block.timestamp < startUNIX && firstOrSecondStake) {
	    ...
	    if(plan == 0) { // extra daily return
	      if (msg.value >= 10000 ether) { // 10,000+ = diamond reward
	        percentBoost = 15; // 1.5% daily boost
	      } else if (msg.value >= 1000 ether) { // 1,000 - 9,999 = platinum reward
	      	percentBoost = 8; // 0.8% daily boost
	      } else if (msg.value >= 500 ether) { // 500 - 999 = gold reward
	        percentBoost = 7; // 0.7% daily boost  
	      } else if (msg.value >= 100 ether) { // 100 - 499 = silver reward
	...
	// INSTANT CASHBACK
	// base cashback + 1% for each deposit >= 100 MATIC
  ...
  // Free MATIC from owner!
  function gift() public payable onlyOwner { 
    emit Gift(msg.sender);
  }
}