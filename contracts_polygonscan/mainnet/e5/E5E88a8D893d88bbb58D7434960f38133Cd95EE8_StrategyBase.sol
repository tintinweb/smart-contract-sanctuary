...
abstract contract StrategyFarmBase is BaseStrategyMasterChef {
    ...
    function harvest() public override {
        ...
        if (_usdcBalance > harvestCutoffUSDC){
            ...
            _swapUniswapWithPath(usdc_myFriends_path, _usdcBalancePost.div(2));
            
            // Add liquidity
            uint256 _usdc = IERC20(usdc).balanceOf(address(this));
            uint256 _myFriends = IERC20(myFriends).balanceOf(address(this));
            if (_usdc > 0 && _myFriends > 0) {
                IERC20(usdc).safeApprove(currentRouter, 0);
                IERC20(usdc).safeApprove(currentRouter, _usdc);
                IERC20(myFriends).safeApprove(currentRouter, 0);
                IERC20(myFriends).safeApprove(currentRouter, _myFriends);
                ...
                IERC20(myFriends).safeTransfer(
                    strategist,
                    IERC20(myFriends).balanceOf(address(this))
                );
            }
...
contract StrategyBase is StrategyFarmBase {
    ...
    address public Want_Token = 0xf66502FAbC81e64AD6E3A8Fd60bDA4a705A72BD0;
