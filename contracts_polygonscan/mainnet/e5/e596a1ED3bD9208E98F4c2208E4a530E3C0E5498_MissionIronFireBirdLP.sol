...
import "./MissionIron.sol";
import "./interfaces/IFireRouter.sol";

import "./interfaces/IIronSwap.sol";

// MissionIronLP
contract MissionIronFireBirdLP is MissionIron {
    
    // Ironswap address
    address public ironSwapAddress;

    // FireRouter address
    address public fireRouter;
    ...
    function earn() external override nonReentrant whenNotPaused onlyGov {
        ...
        if (earnedAmt > 0) {
            ...
            if (token0Amt > 0 && token1Amt > 0) {
                IFireRouter(fireRouter).addLiquidity(
                    wantAddress,
                    token0Address,
                    token1Address,
                    token0Amt,
    ...
    function _resetAllowances() internal override {
        ...
        IERC20(token0Address).safeApprove(fireRouter, uint256(0));
        IERC20(token0Address).safeIncreaseAllowance(
            fireRouter,
            uint256(-1)
        );

        IERC20(token1Address).safeApprove(fireRouter, uint256(0));
        IERC20(token1Address).safeIncreaseAllowance(
            fireRouter,
            uint256(-1)
        );
...
import "./BaseMissionLP.sol";

// MissionIron
abstract contract MissionIron is BaseMissionLP {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    // ironchef address
    address public ironChefAddress;
    uint256 public pid;

    function _vaultDeposit(uint256 _amount) internal override {
        IIronChef(ironChefAddress).deposit(pid, _amount, address(this));
    }
    
    function _vaultWithdraw(uint256 _amount) internal override {
        IIronChef(ironChefAddress).withdraw(pid, _amount, address(this));
    }
    
    function vaultSharesTotal() public override view returns (uint256) {
        (uint256 amount,) = IIronChef(ironChefAddress).userInfo(pid, address(this));
        return amount;
    }
    ...
    function _emergencyVaultWithdraw() internal override {
        IIronChef(ironChefAddress).emergencyWithdraw(pid, address(this));
    }
}
