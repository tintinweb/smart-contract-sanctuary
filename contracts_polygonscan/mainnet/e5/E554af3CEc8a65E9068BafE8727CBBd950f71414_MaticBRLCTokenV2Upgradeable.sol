...
import {MaticBRLCTokenUpgradeable} from "./MaticBRLCTokenUpgradeable.sol";

/**
 * @title MaticBRLCTokenV2Upgradeable
 * @dev An MaticBRLCTokenV2Upgradeable is an ERC20 implementation which represents L1 asset,
 * minting and burning on deposits and withdrawals.
 *
 * V2 changes:
 * - `trusted mint` and `trusted burn` functionality has been added.
 */
contract MaticBRLCTokenV2Upgradeable is MaticBRLCTokenUpgradeable {
    ...
    function initialize(
        ...
        __MaticBRLCTokenUpgradeable_init(
            name_,
            symbol_,
            decimals_,
            childChainManager_
        );
        __MaticBRLCTokenV2Upgradeable_init_unchained();
    }

    function __MaticBRLCTokenV2Upgradeable_init_unchained()
        internal
        initializer
    {}
    ...
    /**
     * @dev Function to update masterMinter
     * @param newMasterMinter The address of new masterMinter
     */
    ...
    /**
     * @dev Function to configure a minter
    ...
    /**
     * @dev Get minter details
     * @param minter The address of the minter
     */
    function getMinter(address minter)
        external
        view
        returns (bool isMinter, uint256 mintAllowance)
    {
        isMinter = _minters[minter];
        mintAllowance = _mintersAllowance[minter];
        return (isMinter, mintAllowance);
    }

    /**
     * @dev Function to mint tokens in a trusted way
     * @param to The address that will receive the minted tokens
     * @param amount The amount of tokens to mint. Must be less than
     * or equal to the mintAllowance of the caller.
     * @return A boolean that indicates if the operation was successful.
     */
    function trustedMint(address to, uint256 amount)
        external
        whenNotPaused
        onlyMinters
    ...
    /**
     * @dev Function to burn tokens in a trusted way
     * @param amount The amount of tokens to be burned. Must be less than
     * or equal to the token balance of the caller.
     */
    function trustedBurn(uint256 amount)
        external
        whenNotPaused
        onlyMinters
        notBlacklisted(_msgSender())
