...
interface IBoostHandler {
  ...
  //Returns ADDY earning boost based on amount of locked ADDY, boost is divided by 10000
...
abstract contract VaultBase is IVault, Ownable, ReentrancyGuard {
    ...
    function getBoost() public view returns (uint256) {
        return IBoostHandler(boostHandler).getBoost(msg.sender, address(this));
    }

    //Returns base amount + amount from boost
    function applyBoost(uint256 _amount) internal view returns (uint256) {
        if (boostHandler != address(0)) {
            return _amount.add(_amount.mul(getBoost()).div(BOOST_BASE));
        }
        return _amount;
    }
    ...
    function _claimReward(address _user) internal override {
        ...
        if (user.shares > 0) {
            ...
            if (pendingReward > 0) {
                ...
                pendingReward = applyBoost(pendingReward);
