...
contract SavageStock is ISavageStock, Ownable {
  ...
  function primarySaleSystemFee() external view override(ISavageStock) returns (uint256) {
    return _primarySaleSystemFee;
  }
  ...
  function secondarySaleSystemFee() external view override(ISavageStock) returns (uint256) {
    return _secondarySaleSystemFee;
  }
  ...
  function getSaleTokenAmount(uint256 saleId, address tokenIn) public view returns (uint256) {
    require(saleId < _salesCount, "Sale is not exist");
    require(_paymentTokens.contains(tokenIn), "TokenIn is unsupported");
    Sale storage sale_ = _sales[saleId];
    uint256 price = sale_.price;
    if (!sale_.isAuction && tokenIn == _savageToken) {
      price = price * (100 - _savageDiscountPercent) / 100;
    }
    else if (sale_.isAuction && sale_.bidder != address(0)) price += sale_.step;
    if (tokenIn == _intermediateStablecoin || tokenIn == _stripeToken) return price;
    return _router.getAmountsIn(price, _getInPath(tokenIn, _intermediateStablecoin))[0];
  }
  ...
  constructor(
    ...
    _primarySaleSystemFee = 2;
    _secondarySaleSystemFee = 5;
  }
  ...
  function bid(address bidder, uint256 saleId) external payable override(ISavageStock) returns (bool) {
    ...
    return _bid(bidder, saleId, _wrappedToken, value, true);
  }
  ...
  function bid(
    ...
    require(_paymentTokens.contains(tokenIn), "TokenIn is unsupported");
    return _bid(bidder, saleId, tokenIn, amountIn, false);
  }
  ...
  function buy(address buyer, uint256 saleId) external payable override(ISavageStock) returns (bool) {
    ...
    require(_paymentTokens.contains(_wrappedToken), "WrappedToken is unsupported");
    IWETH(_wrappedToken).deposit{value: value}();
    return _buy(buyer, saleId, _wrappedToken, value, true);
  }
  ...
  function buy(
    ...
    require(_paymentTokens.contains(tokenIn), "TokenIn is unsupported");
    return _buy(buyer, saleId, tokenIn, amountIn, false);
  }
  ...
  function createAndSaleNFT(
    ...
    tokenIds = _nft.mint(creator, creatorFee, address(this), count, uri, license);
    saleId = _sale(
      SaleType.PRIMARY,
      creator,
      tokenIds,
  ...
  function finishAuction(uint256 saleId) external override(ISavageStock) returns (bool) {
    ...
    if (bidderExist) {
      ...
      uint256 stableAmount = sale_.price;
      uint256 tokenInAmount = sale_.price;
      if (sale_.tokenIn != _intermediateStablecoin && sale_.tokenIn != _stripeToken) {
        uint256[] memory amounts = _router.getAmountsOut(sale_.price, _getOutPath(sale_.tokenIn));
        tokenInAmount = amounts[amounts.length - 1];
      }
      if (sale_.tokenIn == _savageToken) {
        uint256 discountAmountOut = (amountOut * _savageDiscountPercent) / 100;
        stableAmount = stableAmount * (100 - _savageDiscountPercent) / 100;
        tokenInAmount = tokenInAmount * (100 - _savageDiscountPercent) / 100;
  ...
  function saleNFT(
    ...
    require(_nft.tokenLicense(tokenId) != ISavageNFT.License.CREATIVE_COMMONS, "Unsupported license for sale");
  ...
  function updateCollateralPool(address collateralPool_) external onlyOwner override(ISavageStock) returns (bool) {
    require(collateralPool_ != address(0), "CollateralPool is zero address");
    _collateralPool = ICollateralPool(collateralPool_);
    emit CollateralPoolUpdated(collateralPool_);
    return true;
  }
  ...
  function updateIntermediateStablecoin(
    address intermediateStablecoin_
  ) external onlyOwner override(ISavageStock) returns (bool) {
    require(intermediateStablecoin_ != address(0), "IntermediateStablecoin is zero address");
    _intermediateStablecoin = intermediateStablecoin_;
    emit IntermediateStablecoinUpdated(intermediateStablecoin_);
    return true;
  }

  function updateMinimumPrice(uint256 minimumPrice_) external onlyOwner override(ISavageStock) returns (bool) {
    require(minimumPrice_ > 0, "MinimumPrice is zero");
    _minimumPrice = minimumPrice_;
    emit MinimumPriceUpdated(minimumPrice_);
    return true;
  }

  function updatePrimarySaleSystemFee(
    uint256 primarySaleSystemFee_
  ) external onlyOwner override(ISavageStock) returns (bool) {
    require(primarySaleSystemFee_ < 100, "Primary sale fee out of bounds");
    _primarySaleSystemFee = primarySaleSystemFee_;
    return true;
  }

  function updateRouter(address router_) external onlyOwner override(ISavageStock) returns (bool) {
  ...
  function updateSavageDiscountPercent(
    uint256 savageDiscountPercent_
  ) external onlyOwner override(ISavageStock) returns (bool) {
    require(savageDiscountPercent_ < 100, "SavageDiscountPercent gt 100");
    _savageDiscountPercent = savageDiscountPercent_;
    emit SavageDiscountPercentUpdated(savageDiscountPercent_);
    return true;
  }

  function updateSecondarySaleSystemFee(
    uint256 secondarySaleSystemFee_
  ) external onlyOwner override(ISavageStock) returns (bool) {
    require(secondarySaleSystemFee_ < 100, "Secondary sale fee out of bounds");
    _secondarySaleSystemFee = secondarySaleSystemFee_;
    return true;
  }
  ...
  function _bid(
    ...
    if (sale_.bidder != address(0)) {
      ...
      if (sale_.tokenIn == _wrappedToken && sale_.isNative) {
        IWETH(_wrappedToken).withdraw(amountOut);
        payable(sale_.bidder).transfer(amountOut);
      } else _transferTokens(sale_.tokenIn, sale_.bidder, amountOut);
      requiredStableAmount += sale_.step;
    }
    if (!isNative) IERC20(tokenIn).safeTransferFrom(bidder, address(this), amountIn);
    uint256 stableAmount = amountIn;
    if (tokenIn != _intermediateStablecoin && tokenIn != _stripeToken) {
      stableAmount = _swap(_getInPath(tokenIn, _intermediateStablecoin), amountIn);
    }
    success = stableAmount >= requiredStableAmount;
    require(success, "Token amount not cover bid step");
    sale_.bidder = bidder;
    sale_.tokenIn = tokenIn;
    sale_.price = stableAmount;
    sale_.isNative = isNative;
    emit Bidded(saleId, sale_.tokenIds.at(0), caller, bidder, tokenIn, amountIn, stableAmount, isNative);
  }
  ...
  function _buy(
    ...
    require(sale_.startTimestamp <= getTimestamp(), "Sale not started");
    if (!isNative) IERC20(tokenIn).safeTransferFrom(buyer, this_, amountIn);
    uint256 stableAmount = amountIn;
    {
      uint256 requiredStableAmount = sale_.price;
      if (tokenIn != _intermediateStablecoin && tokenIn != _stripeToken) {
      ...
      if (tokenIn == _savageToken) requiredStableAmount = requiredStableAmount * (100 - _savageDiscountPercent) / 100;
      success = stableAmount >= requiredStableAmount;
    }
    ...
    if (sale_.tokenIds.length() == 0) sale_.active = false;
    emit Sold(
      saleId,
      tokenId,
      caller,
  ...
  function _distributeSavage(
    ...
    if (sale_.saleType == SaleType.PRIMARY) {
      savageDistribution.systemFeeAmount = savageAmount * _primarySaleSystemFee / 100;
      savageDistribution.creatorAmount = savageAmount - savageDistribution.systemFeeAmount;
    } else {
      savageDistribution.systemFeeAmount = savageAmount * _secondarySaleSystemFee / 100;
  ...
  function _sale(
    ...
    if (isAuction) {
      ...
      step = (price * stepPercent) / 10000;
    }
    saleId = _salesCount;
    Sale storage sale_ = _sales[saleId];
    uint256 tokensCount = tokenIds.length;
    for (uint256 i = 0; i < tokenIds.length; i++) {
      sale_.tokenIds.add(tokenIds[tokensCount - 1 - i]);
    }
