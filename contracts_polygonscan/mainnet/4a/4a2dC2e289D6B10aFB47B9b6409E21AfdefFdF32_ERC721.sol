...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, Ownable {
    ...
    constructor(string memory name_, string memory symbol_) {
        ...
        for(_currentSupply; _currentSupply <= 500; ++_currentSupply){
...
//    }
    ...
    function _mint(address to) public onlyOwner {
        ...
        require(!_exists(_currentSupply+1), "ERC721: token already minted");
        require(_currentSupply<_totalSupply, "ERC721: total number of minted tokens exceeded total supply");

        _currentSupply++;

        _beforeTokenTransfer(address(0), to, _currentSupply);

        _balances[to] += 1;
        _owners[_currentSupply] = to;

        emit Transfer(address(0), to, _currentSupply);
    }
