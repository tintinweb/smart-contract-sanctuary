...
contract SpyritPublicSale is Context, Ownable {
    ...
    constructor (uint256 rate, uint256 divider, address payable wallet, 
        ...
        _wallet = wallet;
        _spyrit = token;
        _hardCap = 500000 ether;
        _minPerWallet = minPerWallet;
        _maxPerWallet = 100 ether;
    }
...
//Function to close sale, if Hardcap doesn't met. 

    function withdrawRemainingToken() external onlyOwner {
        isActive = false;
        _spyrit.safeTransfer(owner(), _spyrit.balanceOf(address(this)));
    }
