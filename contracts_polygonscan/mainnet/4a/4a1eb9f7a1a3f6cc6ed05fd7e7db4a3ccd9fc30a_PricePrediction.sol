...
contract PricePrediction is Ownable, Pausable {
    ...
    modifier onlyExecuter() {
        require(executers[msg.sender] == true, "not whitelisted as an executer");
        _;
    }
    ...
    function setExecuter(address _executerAddress, bool _isExecuter) external onlyAdmin {
        require(_executerAddress != address(0), "Cannot be zero address");
        executers[_executerAddress] = _isExecuter;
    }
     ...
     */
    function executeRound() external onlyExecuter whenNotPaused { 
        require(
    ...
    function batchClaim(uint256 epoch, address user) internal {
        ...
        // Round valid, claim rewards
        if (rounds[epoch].oracleCalled) {
            if(claimable(epoch, user)) { //if winning round
                Round memory round = rounds[epoch];
