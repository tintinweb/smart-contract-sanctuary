...
contract XVMCfarms is Ownable {
    ...
    */    
    function initiateFarmProposal(
		...
		//you can propose max 5% each, but it can not be enforced
    	if(poolid == 0 || poolid == 1) {
    	    require(
    	        newAllocation <= (IMasterChef(masterchef).totalAllocPoint() * 50 / 1000),
    ...
    */
    function updateFarm(uint256 proposalID) external {
		...
		uint256 _allocPoint;
        if(_poolID == 0) {
			(, _allocPoint, , , ) = IMasterChef(masterchef).poolInfo(1);
			require(_allocPoint + _newAllocation <= (IMasterChef(masterchef).totalAllocPoint() * 50 / 1000) );
		} else if(_poolID == 1) {
			(, _allocPoint, , , ) = IMasterChef(masterchef).poolInfo(0);
			require(_allocPoint + _newAllocation <= (IMasterChef(masterchef).totalAllocPoint() * 50 / 1000) );
		}
		
        IXVMCgovernor(owner()).setPool(_poolID, _newAllocation, proposalFarmUpdate[proposalID].newDepositFee, true);
        proposalFarmUpdate[proposalID].valid = false;
    ...
    */
    ...
    function executeRewardsReduction(uint256 proposalID) external {
		require(!isReductionEnforced, "Already running"); //it would mess up the initial allocation if it were executed while reduction was enforced
    	require(
