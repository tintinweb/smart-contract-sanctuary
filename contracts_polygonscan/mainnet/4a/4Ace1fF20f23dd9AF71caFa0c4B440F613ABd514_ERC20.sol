    ...
    ContextMixin
{
    ...
    constructor(address _childChainManager)
        ERC20("CEK Token", "CAKED")
    {
        _setupDecimals(decimals());
        _initializeEIP712("CEK Token");
    ...
    function permit(
        ...
        require(holder == ecrecover(digest, v, r, s), "CAKED: INVALID-PERMIT");
        require(
            expiry == 0 || block.timestamp <= expiry,
            "CAKED: PERMIT-EXPIRED"
        );
        require(nonce == nonces[holder]++, "CAKED: INVALID-NONCE");
        ...
        returns (uint256)
    {
        require(
            ...
            "CAKED::getPriorVotes: not yet determined"
        );
