...
contract StrategyMiniChef is BaseStrategyLPDouble {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    ...
    function _vaultDeposit(uint256 _amount) internal virtual override {
        IMiniChefStake(masterchefAddress).deposit(pid, _amount, address(this));
    }
    
    function _vaultWithdraw(uint256 _amount) internal override {
        IMiniChefStake(masterchefAddress).withdraw(pid, _amount, address(this));
    }
    
    function _vaultHarvest() internal override {
        IMiniChefStake(masterchefAddress).harvest(pid, address(this));
    }
    
    function vaultSharesTotal() public override view returns (uint256) {
        (uint256 amount,) = IMiniChefStake(masterchefAddress).userInfo(pid, address(this));
        return amount;
    }
    ...
    function _emergencyVaultWithdraw() internal override {
        IMiniChefStake(masterchefAddress).emergencyWithdraw(pid, address(this));
    }
