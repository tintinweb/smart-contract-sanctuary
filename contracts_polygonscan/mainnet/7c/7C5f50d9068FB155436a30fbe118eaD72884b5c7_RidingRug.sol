...
contract RidingRug is Context, Ownable, ERC721Enumerable {
    ...
    uint256 public constant SALE_START_TIMESTAMP = 1636723404;
    ...
    */
    function mintNFT(string[] memory _tokenURI) public payable {
        uint256 totalNftsRequired = _tokenURI.length;
        require(totalNftsRequired <= numberOfNfts, "Can not mint more than max allowed nfts");
        require(totalSupply() < MAX_NFT_SUPPLY, "Sale has already ended");
        require(totalSupply().add(totalNftsRequired) <= MAX_NFT_SUPPLY, "Exceeds MAX_NFT_SUPPLY");
        require(msg.value >= getNFTPrice().mul(totalNftsRequired), "Ether value sent is not correct");

        for (uint i = 0; i < totalNftsRequired; i++) {
