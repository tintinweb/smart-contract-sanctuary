...
contract DestructionTokenV3 is ERC20, Ownable {
    ...
    ///@dev Max transfer amount rate. (default is 0.5% of total supply)
    uint16 public maxUserTransferAmountRate = 20;
    
    ///@dev Max holding rate. (default is 5% of total supply)
    uint16 public maxUserHoldAmountRate = 30;
        ...
        if (antiBotWorking){
            ...
            if (_isNotOperatorExcludedFromAntiBot(_sender)){
                if(_isNotOperatorExcludedFromAntiBot(_recipient))
                    require(_amount <= maxUserTransferAmount(), "ERR3");
            }
        }
        _;
    }
    ...
    function _transfer(address _sender, address _recipient, uint256 _amount) internal virtual override antiBot(_sender, _recipient, _amount) {
        ...
        if (antiBotWorking){
            ...
            if (_isNotHolderExcludedFromAntiBot(_sender)){
                if (balanceOf(_sender) > maxUserHoldAmount()) {
    ...
    function maxUserHoldAmount() public view returns (uint256) {
