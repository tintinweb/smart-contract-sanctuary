...
contract PooTamagotchi is Base, ITamagotchi {
    ...
    function isPooExhausted(uint _id) public override view returns (bool _exhausted) {
        ...
        uint exhaustion = IPoos(PoosContract).getPooByIndex(_id).exhaustion;
        return exhaustion > 99;
    }

    function getRestAndHungerOfPooByIndex(uint _id) public view returns (uint _restPoints, uint _hunger, bool _alive) {
    ...
    function getHunger(uint _id) public override view returns (uint _hungerLevel) {
        address PoosContract = IConstants(constantsContract).PoosContract();
        uint lastTimeFed = IPoos(PoosContract).getPooByIndex(_id).lastTimeFed;
        uint hungerLevel = (block.number - lastTimeFed) / IConstants(constantsContract).blocksBetweenHungerPointForPoo();
        if (hungerLevel > 100) {
            hungerLevel = 100;
        }
        return hungerLevel;
    ...
    function getRestPoints(uint _id) public override view returns (uint) {
        address PoosContract = IConstants(constantsContract).PoosContract();
        uint lastTimeRested = IPoos(PoosContract).getPooByIndex(_id).lastTimeRested;
    ...
    function rest(uint _id) public {
        uint restPoints = getRestPoints(_id);
        address PoosContract = IConstants(constantsContract).PoosContract();
        IPoos(PoosContract).executeRest(_id, restPoints);
        emit Rested(_id);
    }
    ...
    function buyInstantExhaustionReset(uint _id) public {
        ...
        IPoos(IConstants(constantsContract).PoosContract()).executeRest(_id, 100);
    ...
    function feed(uint _id) public {
        ...
        address PoosContract = IConstants(constantsContract).PoosContract();
        IPoos(PoosContract).executeFeed(_id);

        if (isPooAlive(_id) == true) {
            emit Fed(_id, true);
        } else {
            IPoos(PoosContract).executeAliveChange(_id, false);
            emit Fed(_id, false);
        }
    }
    ...
    function resurrect(uint _id) public {
        ...
        address PoosContract = IConstants(constantsContract).PoosContract();
        IPoos(PoosContract).executeResurrect(_id);
        IPoos(PoosContract).executeRest(_id, 100);
