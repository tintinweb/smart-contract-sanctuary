...
abstract contract StrategyFarmBase is BaseStrategyMasterChef {
    ...
    // Token addresses
    address public constant sing = 0xCB898b0eFb084Df14dd8E018dA37B4d0f06aB26D;
    ...
    // Uniswap swap paths
    address[] public sing_token1_path;
    ...
    {
        sing_token1_path = new address[](2);
        sing_token1_path[0] = sing;
        sing_token1_path[1] = token1;
    }
    ...
    function harvest() public override {
        ...
        uint256 _singBalance = IERC20(sing).balanceOf(address(this));
        if (_singBalance > harvestCutoff){

            //Send performance fee to strategist
            uint256 performanceFee = _singBalance.mul(keepFXS).div(keepFXSmax);
            if (performanceFee>0) {
                _swapUniswapWithPathForFeeOnTransferTokens(sing_token1_path, performanceFee);
            ...
            uint256 _singBalancePost = IERC20(sing).balanceOf(address(this));
            _swapUniswapWithPathForFeeOnTransferTokens(sing_token1_path, _singBalancePost.div(2));

            // Add liquidity for Ball/USDC
            uint256 _sing = IERC20(sing).balanceOf(address(this));
            uint256 _token1 = IERC20(token1).balanceOf(address(this));
            if (_sing > 0 && _token1 > 0) {
                IERC20(sing).safeApprove(currentRouter, 0);
                IERC20(sing).safeApprove(currentRouter, _sing);
...
contract StrategyBase is StrategyFarmBase {
    // Token addresses
    address public SING_MASTER_CHEF = 0x9762Fe3ef5502dF432de41E7765b0ccC90E02e92;
    address public SING_USDC_LP = 0x6046eB198AbC5Ea4f17027bC00a2aeE0420E84EE;
