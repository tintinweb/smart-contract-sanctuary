...
contract KommunitasProject{
     ...
     */
    function saveMe() isBoosterProgress isNotPaused public {
        require(msg.sender == factory.savior(), "Who are you?");
        require(sale.sub(sold()) > 0 && boosterProgress() == 3, "Not now savior");

        uint buyerId = setBuyer(msg.sender);

        uint tokenReceivedFinal = sale.sub(sold());

        invoices[msg.sender].push(Invoice(buyerId, boosterProgress(), block.timestamp, 0, tokenReceivedFinal));
        
        purchasePerRound[msg.sender][boosterProgress()] += tokenReceivedFinal;
        booster[boosterProgress()].achieve += tokenReceivedFinal;
        
        emit TokenBought(boosterProgress(), msg.sender, 0, tokenReceivedFinal);
    }
