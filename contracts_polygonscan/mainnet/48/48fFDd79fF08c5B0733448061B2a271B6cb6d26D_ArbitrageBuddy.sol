...
/// @dev Compare price denominated in quote token between two pools
    ...
    function updateBitInfoM(OrderedReserves memory reserves, BitInfo_BorrowTerm memory bitInfo, uint256 d_AccuracyReductionShift)internal pure{
    ...
    function calculateBorrowTerms(OrderedReserves memory reserves) internal pure returns (uint256 d, int256 d_AccuracyReduction) {
        ...
        updateBitInfoM(reserves, bitInfo, d_AccuracyReductionShift);
        if (bitInfo.bitsNeeded_m >= 255)
        {
            while(bitInfo.bitsNeeded_m >= 255)
            {
                d_AccuracyReductionShift++;
                updateBitInfoM(reserves, bitInfo, dShift);
            }
        }
