...
contract erc20Stablecoin is ReentrancyGuard, VaultNFTv3 {
    ...
    function getDebtCeiling() public view returns (uint256){
        return mimatic.balanceOf(address(this));
    }
    ...
    function borrowToken(uint256 vaultID, uint256 amount) external onlyVaultOwner(vaultID) {
        ...
        // mimatic
        mimatic.safeTransfer(msg.sender, amount);

        emit BorrowToken(vaultID, amount);
    }

    function payBackToken(uint256 vaultID, uint256 amount) external {
        require(mimatic.balanceOf(msg.sender) >= amount, "Token balance too low");
        ...
        //mimatic
        mimatic.safeTransferFrom(msg.sender, address(this), amount);
    ...
    function liquidateVault(uint256 vaultID) external {
        ...
        require(mimatic.balanceOf(msg.sender) >= halfDebt, "Token balance too low to pay off outstanding debt");

        //mimatic
        mimatic.safeTransferFrom(msg.sender, address(this), halfDebt);
...
/// TRIPLE CHECK THE VERSION WE END UP USING. MIGHT NEED TO UPGRADE ALL OUR CONTRACTS
