...
// This MasterChef is the master of TIME but he cannot make any more TIME because max supply has been reached.
// He can only reward TIME and he is a fair guy.
//
...
contract TimeleapMasterChef is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        uint256 lpSupply;         // Added to help count tokens deposited into pool
    }
    ...
    // MaxSupply of TIME Tokens.
    uint256 public maxSupply = 86400 ether;
    ...
    // The block number when TIME reward starts.
    uint256 public startBlock;

    // Maximum timePerBlock
    uint256 public MAX_EMISSION_RATE = 0.05 ether;
    ...
    event TimeMintError(bytes reason);
    ...
    function pendingTime(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && pool.lpSupply != 0 && totalAllocPoint > 0) {
            ...
            accTimePerShare = accTimePerShare.add(timeReward.mul(1e18).div(pool.lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accTimePerShare = pool.accTimePerShare.add(timeReward.mul(1e18).div(pool.lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function updateEmissionRate(uint256 _timePerBlock) external onlyOwner {
        require(_timePerBlock <= MAX_EMISSION_RATE, "Too high");
        massUpdatePools();
        timePerBlock = _timePerBlock;
