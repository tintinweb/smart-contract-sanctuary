...
contract XVMCbasics is Ownable {
	...
	RolloverBonus[] public rolloverBonuses;
    ...
    event InitiateRolloverBonus(uint256 proposalID, uint256 depositingTokens, address forPool, uint256 newBonus, address enforcer);
    event VetoRolloverBonus(uint256 proposalID, address enforcer);
    event ExecuteRolloverbonus(uint256 proposalID, address enforcer);
    
    event ChangeGovernor(address newGovernor);
    
    modifier whenReady() {
      require(block.timestamp > 1637145462, "after Nov 17");
      _;
    }
  ...
  /**
    ...
    */
    function initiateProposalRolloverBonus(uint256 depositingTokens, address _forPoolAddress, uint256 _newBonus) external whenReady { 
    	require(depositingTokens <= IXVMCgovernor(owner()).maximumVoteTokens(), "No tyranny");
    	require(depositingTokens >= IXVMCgovernor(owner()).costToVote(), "minimum cost to vote");
    
    	IERC20(token).safeTransferFrom(msg.sender, owner(), depositingTokens);
    	rolloverBonuses.push(
    	    RolloverBonus(true, block.timestamp, depositingTokens, _forPoolAddress, _newBonus)
    	    );  
    	    
        emit InitiateRolloverBonus(rolloverBonuses.length, depositingTokens, _forPoolAddress, _newBonus, msg.sender);
    }
    function vetoProposalRolloverBonus(uint256 proposalID) external whenReady {
    	require(rolloverBonuses[proposalID].valid, "already invalid"); 
    	
    	IERC20(token).safeTransferFrom(msg.sender, owner(), rolloverBonuses[proposalID].valueSacrificedForVote);
    	rolloverBonuses[proposalID].valid = false;  
    	
    	emit VetoRolloverBonus(proposalID, msg.sender);
    }

    function executeProposalRolloverBonus(uint256 proposalID) external whenReady {
    	require(
    	    rolloverBonuses[proposalID].valid &&
    	    rolloverBonuses[proposalID].firstCallTimestamp + IXVMCgovernor(owner()).delayBeforeEnforce() < block.timestamp,
    	    "conditions not met"
    	);
    	...
    	emit ExecuteRolloverbonus(proposalID, msg.sender);
    }
