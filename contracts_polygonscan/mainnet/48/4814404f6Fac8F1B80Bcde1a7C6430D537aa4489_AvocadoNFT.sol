...
contract AvocadoNFT is ERC721EnumerableSimple, Ownable, ContextMixin {
    uint256 public constant MAX_AVOCADO_SUPPLY = 10000;
    bool public isSaleActive = false;
    string private baseURI;

    constructor() ERC721("Avocado", "AVCDO") {}
    ...
    function getAvocado(uint256 numAvocados) public payable {
        uint256 _totalSupply = totalSupply();
        require(_totalSupply < MAX_AVOCADO_SUPPLY, "Avocados have sold out!");
        require(
            _totalSupply + numAvocados <= MAX_AVOCADO_SUPPLY,
            "Exceeds the maximum Avocado supply"
        );
        require(
            numAvocados > 0 && numAvocados <= 20,
            "You can mint a minimum of 1 and a maximum 20 Avocados"
        );
        require(
            msg.value >= calculatePrice() * numAvocados,
            "Ether value sent is incorrect"
        );

        for (uint256 i = 0; i < numAvocados; i++) {
    ...
    function calculatePrice() public view returns (uint256) {
        require(isSaleActive, "Sale hasn't started");
        return calculatePriceForToken(totalSupply());
    }

    // Any native currency coded into a contract deployed on Polygon is calculated as MATIC.
    // Polygon doesn't see Ether as the Ethereum coin, but just as a suffix to indicate size
    // https://ethereum.stackexchange.com/a/109824
    function calculatePriceForToken(uint256 _id) public pure returns (uint256) {
        require(_id < MAX_AVOCADO_SUPPLY, "Avocados have sold out!");

        if (_id >= 9900) {
            // #9900 - #10000: 300 MATIC
            return 300 ether;
        } else if (_id >= 9500) {
            // #9500 - #9899: 250 MATIC
            return 250 ether;
