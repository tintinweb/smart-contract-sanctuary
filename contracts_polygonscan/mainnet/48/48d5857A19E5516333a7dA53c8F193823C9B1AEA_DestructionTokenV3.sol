...
contract DestructionTokenV3 is ERC20, Ownable {
    ...
    uint16 public maxUserTransferAmountRate = 50;
    
    ///@dev Max holding rate. (default is 5% of total supply)
    uint16 public maxUserHoldAmountRate = 500;
    ...
    modifier antiBot(address _sender, address _recipient, uint256 _amount) { 
        ...
        // This code will disable after fair launch and before farming
        if (antiBotWorking){
            // check  if sender|recipient has a tx amount is within the allowed limits
            if (_isNotExcludedFromAntiBot(_sender)){
                if(_isNotExcludedFromAntiBot(_recipient))
    ...
    function _addBotAddressToBlackList(address _botAddress, uint256 _expirationTime) internal {
        require(_isNotExcludedFromAntiBot(_botAddress), "ERR5");
    ...
    function _isNotExcludedFromAntiBot(address _userAddress) internal view returns(bool) {
