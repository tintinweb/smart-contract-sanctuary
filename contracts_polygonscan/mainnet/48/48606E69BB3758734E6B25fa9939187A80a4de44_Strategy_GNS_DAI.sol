...
contract Strategy_GNS_DAI is Ownable {
    ...
    */
    ...
    function _getWrappedFromRewards() internal {
        ...
        bool _swapped = _swapExactTokenForToken(router, QUICKToken, WMATIC, _balance+pendingRewardsFee);
        if (_swapped) {
            pendingRewardsFee = 0;
        } else {
    ...
    function _swapLPtoWMATIC(uint256 _percent) internal {
        ...
        if (_totalLP > 0) {
            ...
            /*  swap rewards to WMATIC
                ...
                WETH <-> WMATIC 
            */
            _swapExactTokenForToken(router, QUICKToken, WMATIC, getBalanceOfToken(QUICKToken));
    ...
    function _getSimpleTCI(uint256 _amount) internal view returns (uint256 tCI) {
        if(_amount > 0) {
            ...
            tCI = (_getAmountDAIfromGNS(t1V) + t0V) * _getPriceWMATICToDAI() / 1e18;
        }
        ...
        if (_balance0 > 0) {
            tCI += _balance0 * _getPriceWMATICToDAI() / 1e18;
        }

        if (_balance1 > 0) {
            tCI += _getAmountDAIfromGNS(_balance1) * _getPriceWMATICToDAI() / 1e18;
        }
