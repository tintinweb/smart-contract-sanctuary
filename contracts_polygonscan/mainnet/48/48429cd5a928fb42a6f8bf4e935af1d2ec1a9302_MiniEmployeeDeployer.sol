...
contract MiniEmployeeDeployer is Initializable, AccessControl {
    ...
    FactoryStaking private factoryStakingV2;
    ...
    function initialize(
        ...
        FactoryStaking _factoryStakingV1,
        FactoryStaking _factoryStakingV2,
        ...
        miniEmployees = _miniEmployees;
        baseDeployer = _baseDeployer;
        factoryStakingV1 = _factoryStakingV1;
        factoryStakingV2 = _factoryStakingV2;
    ...
    function payMint(address customer, bool package) private returns (bool) {
        ...
        token.transferFrom(customer, creator, (totalAmount * creatorFee) / 100);
    ...
    function mintWithStakingPoints(bool package) public {
        ...
        uint256 v2MinterPoints = factoryStakingV2.getSavedMinterPoints(
            msg.sender
        );

        require(
            v1MinterPoints + v2MinterPoints >= pointsPrice,
            INVALID_PAYMENT
        );

        uint256 usableMinterPoints = pointsPrice;

        if (v1MinterPoints > 0) {
            if (v1MinterPoints >= usableMinterPoints) {
                factoryStakingV1.useMinterPoints(
                    msg.sender,
                    usableMinterPoints
                );

                usableMinterPoints = 0;
            } else {
                factoryStakingV1.useMinterPoints(msg.sender, v1MinterPoints);
                usableMinterPoints -= v1MinterPoints;
            }
        }

        if (usableMinterPoints > 0) {
            factoryStakingV2.useMinterPoints(msg.sender, usableMinterPoints);
        }
