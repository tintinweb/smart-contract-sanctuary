...
contract TestComethSwap_ZapIn_V2 is ZapInBaseV3 {
	using SafeERC20 for IERC20;

	IUniswapV2Factory private constant comethSwapFactory =
			IUniswapV2Factory(0x5757371414417b8C6CAad45bAeF941aBc7d3Ab32);

	IUniswapV2Router02 private constant comethSwapRouter =
	...
	/**
	@notice Add liquidity to ComethSwap pools with ETH/ERC20 Tokens
	@param _FromTokenContractAddress The ERC20 token used (address(0x00) if ether)
	@param _pairAddress The ComethSwap pair address
	...
	function ZapIn(
			...
			IERC20(_pairAddress).safeTransfer(msg.sender, LPBought); // here transfer to multi-sig
			return LPBought;
	}
	...
	function _uniDeposit(
			...
			_approveToken(_ToUnipoolToken0, address(comethSwapRouter), token0Bought);
			_approveToken(_ToUnipoolToken1, address(comethSwapRouter), token1Bought);

			(uint256 amountA, uint256 amountB, uint256 LP) =
					comethSwapRouter.addLiquidity(
							_ToUnipoolToken0,
							_ToUnipoolToken1,
							token0Bought,
							token1Bought,
	...
	function _swapIntermediate(
			...
			IUniswapV2Pair pair =
					IUniswapV2Pair(
							comethSwapFactory.getPair(_ToUnipoolToken0, _ToUnipoolToken1)
					);
	...
	*/
	function _token2Token(
			...
			_approveToken(
					_FromTokenContractAddress,
					address(comethSwapRouter),
					tokens2Trade
			);

			address pair =
					comethSwapFactory.getPair(
							_FromTokenContractAddress,
			...
			tokenBought = comethSwapRouter.swapExactTokensForTokens(
					tokens2Trade,
					1,
					path,
					address(this),
