...
contract Sky is Ownable, ISky {
   ...
   function createCloud(uint256 maticAmount) external payable onlyOwner {
       Cloud cloud = new Cloud(
                               ...
                               owner(), // ref
                               polyroll, //roll
                               polyrollMiner, // rollminer
                               VRF_COORDINATOR, // coord
                               LINK_TOKEN // link
                               );
   ...
   function stop() public onlyOwner {
       for(uint256 i = 0; i < cloudsIndex.length; i++){
           Cloud(payable(cloudsIndex[i])).stop();
       }
   }
   ...
   function start() external onlyOwner {
       for(uint256 i = 0; i < cloudsIndex.length; i++){
           ...
           Cloud(payable(cloudsIndex[i])).start();
   ...
   function withdraw() public onlyOwner {
       for(uint256 i = 0; i < cloudsIndex.length; i++){
           Cloud(payable(cloudsIndex[i])).withdraw();
       }
   }
...
contract Cloud is VRFConsumerBase, Ownable {
    using SafeMath for uint256;
    ...
    uint256 lastRollCounted;
    uint256 public wins;
    uint256 public losses;
    uint256 public pendingBet = 0;
    uint256 public wasPendingBet = 0;
    uint256 public waitForRounds = 0;
    address public refer;
    uint256 public balancePercentage = 500;
    
    constructor(address skyAddress,
                address pTaker_,
                address refer_,
                Polyroll polyroll_,
                PolyrollMiner polyrollMiner_,
                address VRF_COORDINATOR,
                address LINK_TOKEN) VRFConsumerBase(VRF_COORDINATOR, LINK_TOKEN){
        pTaker = pTaker_;
    ...
    function takeProfit() internal returns(uint256){
        if(address(this).balance > startBalance){
            ...
            payable(pTaker).transfer(address(this).balance.sub(startBalance));
            return profit;
        }
        return 0;
    }
    ...
    function pick(uint256 randomValue) public view returns(uint256, uint256){
        ...
        uint256 addonPicks = 0;
        if(losses >= wins){
            maxPicksModulo = 23;
            addonPicks = (losses - wins) > 23 ? 23 : (losses - wins);
        }
    ...
    function roll(uint256 randomValue) internal {
        ...
        if(rolls.length == 1 || rolls.length > lastRollCounted){
            if(polyroll.bets(rolls[rolls.length-1]).winAmount > 0){
                ...
                losses = losses + 1;
                if(alwaysPicked.length < 13){
                    
                    alwaysPicked.push(polyroll.bets(rolls[rolls.length-1]).outcome % 37);
                }else{
                    alwaysPicked[lastAlwaysPickedReplaced % alwaysPicked.length] = polyroll.bets(rolls[rolls.length-1]).outcome % 37;
        ...
        (uint256 betMask, uint256 numbers) = pick(randomValue);
        if(pendingBet == 1){
            wasPendingBet = wasPendingBet + 1;
        }
        if(wasPendingBet > waitForRounds || pendingBet == 0){
            uint256 betAmount = Utils.calculatePercentageCents(address(this).balance, balancePercentage.add(37 - numbers%37));
            ...
            polyroll.placeBet{ value: betAmount }(betMask, 37, refer);
            pendingBet = 1;
            wasPendingBet = 0;
        }
