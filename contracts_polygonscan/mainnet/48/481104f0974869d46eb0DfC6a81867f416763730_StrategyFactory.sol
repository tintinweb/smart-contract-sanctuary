...
contract StrategyFactory is AccessControlEnumerableUpgradeable {
    ...
    /// @notice A strategy type represents a type 
    ...
    bytes32 public constant REGISTER_STRATEGY_ROLE = keccak256("REGISTER_STRATEGY_ROLE");
    bytes32 public constant REGISTER_PROJECT_ROLE = keccak256("REGISTER_PROJECT_ROLE");
    bytes32 public constant CREATE_VAULT_ROLE = keccak256("CREATE_VAULT_ROLE");

    event StrategyTypeAdded(bytes32 indexed strategyType, ISubFactory indexed subfactory);
    event ProjectRegistered(bytes32 indexed projectId, bytes32 indexed strategyType, bool indexed isUpdate);
    event VaultRegistered(uint256 indexed vaultId, bytes32 indexed projectId, bytes32 indexed strategyId, bool isUpdate);

    function initialize(IVaultChef _vaultChef) external initializer {
    ...
    function registerStrategyType(string calldata strategyType, ISubFactory subfactory) external onlyRole(REGISTER_STRATEGY_ROLE) {
        registerStrategyTypeRaw(keccak256(abi.encodePacked(strategyType)), subfactory);
    }

    function registerStrategyTypeRaw(bytes32 strategyType, ISubFactory subfactory) public onlyRole(REGISTER_STRATEGY_ROLE) {
    ...
    function registerProject(string calldata projectId, string calldata strategyType, bytes calldata params, bytes calldata metadata) external onlyRole(REGISTER_PROJECT_ROLE) {
        registerProjectRaw(keccak256(abi.encodePacked(projectId)), keccak256(abi.encodePacked(strategyType)), params, metadata);
    }


    function registerProjectRaw(bytes32 projectId, bytes32 strategyType, bytes calldata params, bytes calldata metadata) public onlyRole(REGISTER_PROJECT_ROLE) {
        require(strategyTypes.contains(strategyType), "!strategyType not found");
    ...
    function createVaultRaw(bytes32 projectId, bytes32 strategyId, bytes calldata params, bytes calldata metadata, uint16 performanceFee) external onlyRole(CREATE_VAULT_ROLE) returns (uint256, IStrategy) {
        ...
        vars.strategyAddress = subfactoryByType[projectStrategyType[projectId]]
            .deployStrategy(projectParams[projectId], params);
        vars.vaultId = vaultChef.poolLength();
        vaultChef.addVault(vars.strategyAddress, performanceFee);
        ...
        return (vars.vaultId, vars.strategyAddress);
    }

    function getStrategyUID(bytes32 projectId, bytes32 strategyId) internal pure returns(bytes32) {
        return keccak256(abi.encodePacked(projectId, strategyId));
    }


    // FETCH ALL STRATEGIES OF PROJECT

    // FETCH ALL STRATEGIES OF UNDERLYING

    // FETCH ALL STRATEGIES OF 
}
