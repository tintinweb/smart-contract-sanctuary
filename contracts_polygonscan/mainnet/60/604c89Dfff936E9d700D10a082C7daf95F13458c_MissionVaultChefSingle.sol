...
// File contracts/missions/MissionVaultChefSingle.sol



pragma solidity 0.6.12;
contract MissionVaultChefSingle is BaseMissionLPSingle {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    ...
    function _vaultDeposit(uint256 _amount) internal override {
        IVaultChef(masterchefAddress).deposit(pid, _amount);
    }
    
    function _vaultWithdraw(uint256 _amount) internal override {
        IVaultChef(masterchefAddress).withdraw(pid, _amount);
    }
    
    function _vaultHarvest() internal override {
        IVaultChef(masterchefAddress).withdraw(pid, 0);
    }
    
    function vaultSharesTotal() public override view returns (uint256) {
        (uint256 amount) = IVaultChef(masterchefAddress).userInfo(pid, address(this));
        return amount;
    }
    ...
    function _emergencyVaultWithdraw() internal override {
        IVaultChef(masterchefAddress).emergencyWithdraw(pid);
    }
}