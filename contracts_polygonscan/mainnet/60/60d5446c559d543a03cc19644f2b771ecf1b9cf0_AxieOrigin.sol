...
contract AxieOrigin is SysCtrl, OriginProfile, IERC721 {
    ...
    string internal nftSymbol = "Axie";
     ...
     */
    function tokenURI(uint256 _axieId) external validNFToken(_axieId) view  returns (string memory) {
        string memory _base = uriPrefix;
        string memory _id = uint2str(_axieId);
        return string(abi.encodePacked(_base,_id));
    }
