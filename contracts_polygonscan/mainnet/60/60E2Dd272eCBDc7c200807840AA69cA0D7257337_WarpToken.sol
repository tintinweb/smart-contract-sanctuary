...
contract WarpToken is ERC20Upgradeable, VaultOwned {
    //** ====== VARIABLES ======== */
    ...
    modifier whenNotPaused() {
        require(!paused || whitelist[msg.sender], 'Pausable: paused');
        _;
    }
    ...
    function isTimeLimited(address sender, address recipient) internal {
        if (timeLimited && whitelist[recipient] == false && whitelist[sender] == false) {
            ...
            if (timeOfLastTransfer[toDisable] == 0) {
                ...
                require(
                    block.timestamp - timeOfLastTransfer[toDisable] > timeBetweenTransfers,
