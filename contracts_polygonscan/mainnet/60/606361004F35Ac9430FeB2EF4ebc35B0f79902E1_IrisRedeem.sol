...
// FenixToken
abstract contract FenixToken is ERC20('SURF', 'SURF'), ReentrancyGuard, Ownable {
...
contract IrisRedeem is Ownable, ReentrancyGuard {
    ...
    constructor(uint256 _startBlock, address _fenixAddress, address _irisAddress) {
        require(_fenixAddress != _irisAddress, "SURF cannot be equal to WAVE");
        startBlock   = _startBlock;
    ...
    function swapFenixForIris(uint256 swapAmount) external nonReentrant {
        require(block.number >= startBlock, "WAVE redemption hasn't started yet, good things come to those that wait ;)");
    ...
    function sendUnclaimedIrisToDeadAddress() external onlyOwner {
        require(block.number > fenix.endBlock(), "can only send excess WAVE to dead address after presale has ended");
        ...
        require(fenix.fenixRemaining() <= IERC20(irisAddress).balanceOf(address(this)),
            "burning too much WAVE, founders may need to top up");
