...
contract BldmndPresale is ReentrancyGuard {
	using SafeMath for uint256;
	...
	// Price of presale BLDMND, 1 USDC
	uint256 USDCPerToken = 1;
	// Amount of USDC received in presale
	uint256 usdcReceived = 0;

  	uint256 maxHardCap = 125000;
	...
	function getBLDMNDTokensLeft() external view returns (uint256) {
		return BLDMND.balanceOf(address(this));
	}

	function claimTokens(address _claimer) external nonReentrant {
		require(isClaimActive, "Claim is not allowed yet");
		require(tokensOwned[_claimer] > 0, "User should own some BLDMND tokens");
		require(tokensUnclaimed[_claimer] > 0, "User should have unclaimed BLDMND tokens");
		require(BLDMND.balanceOf(address(this)) >= tokensUnclaimed[_claimer], "There are not enough BLDMND tokens to transfer.");

		uint256 callerTokensUnclaimed = tokensUnclaimed[_claimer];
		tokensUnclaimed[_claimer] = 0;
		BLDMND.safeTransfer(_claimer, callerTokensUnclaimed);
	...
	function withdrawUnsoldBLDMND() external onlyOwner {
		uint256 amount = BLDMND.balanceOf(address(this)) - totalTokensSold;
		BLDMND.safeTransfer(msg.sender, amount);
	}

	function withdrawAllBLDMND() external onlyOwner {
		BLDMND.safeTransfer(msg.sender, BLDMND.balanceOf(address(this)));
	}
}