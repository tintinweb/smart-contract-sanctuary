...
contract GrowthVault is ERC20, Ownable {
    ...
    function autoCompound() public {
        ...
        for (uint256 i; i < names.length; i++) {
            ...
            if (_amount > 0) {
                ...
                _swapExactTokensForETH(strategyObj.liquidityRouter, _cakeToken, path);

                // get FEE
                // path[0] = WETH;
                // path[1] = USDT;
                // uint256 desiredAmountETH = _calculateAmountAfterFee(_getBalanceOfToken(WETH));
                // amounts = IRouter(USDTRouter).getAmountsOut(desiredAmountETH, path);
    ...
    function _swapExactTokensForETH(
        ...
        uint256 _balance = _getBalanceOfToken(_token);
        uint256[] memory amounts = IRouter(_liquidityRouter).getAmountsOut(_balance, _path);
    ...
    function _getYELs(uint256 _percent) internal { 
        ...
        for (uint256 i; i < names.length; i++) {
            ...
            _swapExactTokensForETH(strategyObj.liquidityRouter, cakeToken, path);
            ...
            if(strategyObj.token0 != WETH) {
                ...
                _swapExactTokensForETH(strategyObj.liquidityRouter, strategyObj.token0, path);
            }

            path[0] = strategyObj.token1;
            if(strategyObj.token1 != WETH) {
                amountMin = _getAmountMin(strategyObj.token1, WETH, strategyObj.liquidityRouter, path);
                _swapExactTokensForETH(strategyObj.liquidityRouter, strategyObj.token1, path);
            }
    ...
    function _swapTokensToNativeToken(
        ...
        if(_token0 != WETH) {
            ...
            _swapExactTokensForETH(_liquidityRouter, _token0, path);
        }

        if(_token1 != WETH) {
            path[0] = _token1;
            amountMin = _getAmountMin(_token0, WETH, _liquidityRouter, path);
            _swapExactTokensForETH(_liquidityRouter, _token1, path);
        }
    }
