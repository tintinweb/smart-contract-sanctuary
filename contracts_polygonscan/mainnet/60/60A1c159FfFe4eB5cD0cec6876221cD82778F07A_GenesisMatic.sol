...
contract GenesisMatic is ERC20, IChildToken, AccessControlMixin {
    ...
    modifier onlyWorldOrGovernance {
        require(msg.sender == world || msg.sender == governance, "Must be an approved contract");
        _;
    }
    ...
    function setAddresses(address world_, address governance_) external isGlobalAdmin {
        world = world_;
        governance = governance_;
    }

    function mintToAddress(address user, uint256 amount) external onlyWorldOrGovernance {
        _mint(user, amount);
    }
