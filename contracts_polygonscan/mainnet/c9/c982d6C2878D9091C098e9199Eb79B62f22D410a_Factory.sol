...
contract Factory is Initializable, UUPSUpgradeable, OwnableUpgradeable, NativeMetaTransaction {
    ...
    function claimGameTreasury(bytes32 gameID) external onlyAdmin {
        ...
        bytes4 funcSig = bytes4(keccak256(bytes("claimTreasury()")));
        bytes memory payload = abi.encodeWithSelector(funcSig);
    ...
    function claimBet(bytes32 questionID) external {
        ...
        require(gameQuestion != address(0), "Question Does not exist");
        bytes4 funcSig = bytes4(keccak256(bytes("claim(address)")));
        bytes memory payload = abi.encodeWithSelector(funcSig, userAddress);
        (bool success, bytes memory returnData) = address(gameQuestion).call(payload);
        require(success && (returnData.length == 0 || abi.decode(returnData, (bool))), "Claim Failed");
    }

    function claimGameBets(bytes32 gameID, address userAddress) external {
    ...
    function placeTheBet(bytes32 questionID, bytes memory data) external {
        ...
        address gameQuestion = questionAddressMap[questionID];
        require(gameQuestion != address(0), "Question Does not exist");
        bytes4 funcSig = bytes4(keccak256(bytes("placeBet(bytes,address)")));
        bytes memory payload = abi.encodeWithSelector(funcSig, data, userAddress);
        (bool success, bytes memory returnData) = address(gameQuestion).call(payload);
