...
contract WUSDMaster is Ownable, Withdrawable, ReentrancyGuard, Pausable {
    ...
    uint256 internal decimalDifference = 10 ** 12;
    ...
    mapping(address => uint256) public usdcClaimBlock;
    ...
    constructor(IWUSD _wusd, IERC20 _usdc, IERC20 _wex, IWswapRouter _wswapRouter, address _treasury, uint256 _maxStakeAmount, uint256 _maxRedeemAmount, uint256 _maxStakePerBlock) {
        require(
        ...
        treasury = _treasury;
        swapPath = [address(usdc), address(wex)];
        swapPathReverse = [address(wex), address(usdc)];
    ...
    function setSwapPath(address[] calldata _swapPath) external onlyOwner {
        require(_swapPath.length > 1 && _swapPath[0] == address(usdc) && _swapPath[_swapPath.length - 1] == address(wex), "invalid swap path");
        swapPath = _swapPath;
    ...
    function stake(uint256 amount, uint256 amountOutMin) external nonReentrant whenNotPaused {
        ...
        wusd.mint(address(this), amountWithDecimals);
        uint256 wexAmount = amount * wexPermille / 1000;
        usdc.approve(address(wswapRouter), wexAmount);
        ...
        wusdClaimAmount[msg.sender] = amountWithDecimals;
    ...
    function claimUsdc(uint256 amountOutMin) external nonReentrant whenNotPaused {
        ...
        uint256 usdcTransferAmount = amountWithoutDecimals * (1000 - wexPermille - treasuryPermille) / 1000;
