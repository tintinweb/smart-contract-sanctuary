...
// File: contracts/matic/StrategyDinoUstUpgraderableMatic.sol
...
contract StrategyDinoUstUpgraderableMatic is ReentrancyGuardUpgradeSafe, OwnableUpgradeable , Pausable {
    ...
    address public constant sushiRouterAddress = 0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506; // swap dino to usdc
    ...
    function initialize(
        ...
        if (isGrandComp) {
            ...
            earnedToUsdcPath = [earnedAddress,usdcAddress];
            usdcToEarnPath = [usdcAddress,earnedAddress];
            usdcToToken0Path = [usdcAddress, token0Address];
            usdcToToken1Path = [usdcAddress, token1Address];
            token0ToEthPath = [token0Address, usdcAddress];
            token1ToEthPath = [token1Address, usdcAddress];
        }
    ...
    function earn() public whenNotPaused {
        ...
        IPancakeRouter02(sushiRouterAddress)
    ...
    function convertDustToEarned() public whenNotPaused {
        ...
        uint256 token0Amt = IERC20(token0Address).balanceOf(address(this));
        if (token0Address != usdcAddress && token0Amt > 0) {
            IERC20(token0Address).safeIncreaseAllowance(
                uniRouterAddress,
                token0Amt
            );
        ...
        uint256 token1Amt = IERC20(token1Address).balanceOf(address(this));
        if (token1Address != usdcAddress && token1Amt > 0) {
            IERC20(token1Address).safeIncreaseAllowance(
                uniRouterAddress,
                token1Amt
            );
