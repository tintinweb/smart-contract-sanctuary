...
contract NftizePool is IERC721Receiver, AccessControl {
    ...
    function depositNFT(
        ...
        if (poolSlots <= poolNftsCount) revert PoolSlotsFull({ message: "All pool slots are full" });
        ...
        uint256 nftPrice = basePrice + ((i * basePrice) / 10000) + ((socialCauseFee * basePrice) / 10000);

        poolNfts[nftContractAddress][tokenId] = NFTData(msg.sender, nftPrice, i);
    ...
    function withdrawNFT(address nftContractAddress, uint256 tokenId) public {
        ...
        uint256 nftPrice = poolNft.nftPrice;
