...
/**
 *
 * This code is part of ShadowArt project (https://shadowart.one).
 * Developed by Jagat Token (jagatoken.com).
 *
 * ShadowArt is a collection of 3,333 unique NFTs collectible items.
 * A generative art piece inspired by the shadow puppet culture of Java Island.
 *
 * A unique blend of culture, technology, and historical artifacts that has been
 * crafted by talented digital artists.
 */
...
contract ShadowArt is ERC721Enumerable, HasRoot, HasAdmin, SigVerifier {
    ...
    uint256 private mintPrice = 15 * 10 ** 17;
    ...
    event FundTransfer(address indexed receiver, uint256 amount);
    ...
    function mint(address to, uint32 count, uint64 nonce, bool airdrop, Sig memory sig) external payable returns (bool) {
        uint256 amount = msg.value;

        require(count > 0, "Count must be greater than 0");

        if (airdrop){
          require(count == 1, "Airdrop can only mint 1");
        }else{
          require(amount >= count * mintPrice, "Not enough balance to pay");
        }

        require(nonce >= uint64(block.timestamp) / 30, "invalid nonce");

        bytes32 message = sigPrefixed(
            keccak256(abi.encodePacked(OP_MINT, _msgSender(), to, count, nonce, airdrop))
        );
    ...
    function setPrice(uint256 newPrice) external onlyAdminOrRoot {
        mintPrice = newPrice;
    }
    ...
    function transfer(address to, uint256 amount) external onlyRoot {
        require(amount > 0, "Amount must be greater than 0");
        payable(to).transfer(amount);
        emit FundTransfer(to, amount);
    }
