...
// -------------------------------------- manipulative functions -------------------------------------
    ...
    // todo 加了position.total_debts = HLS.... , 檢查邏輯
    ...
    function withdraw() external {
        ...
        if (value > free_fund_value) {
            ...
            // todo 檢查兩次的value(exitPosition前後)一不一樣
        }
...
// -------------------------------------- getters for testing  -----------------------------------
    ...
    function getFreeFunds() public view returns(uint256[9] memory ){
        uint256[9] memory balances ;
        balances[0] = IERC20(Position.token).balanceOf(address(this)); //USDC
        balances[1] = IERC20(Position.supply_crtoken).balanceOf(address(this));//crUSDC
        balances[2] = IERC20(Position.token_a).balanceOf(address(this));//WETH
        balances[3] = IERC20(Position.token_b).balanceOf(address(this));//USDT
        balances[4] = IERC20(Position.lp_token).balanceOf(address(this));//LP
        balances[5] = IERC20(Position.dQuick_addr).balanceOf(address(this));//dQUick
        balances[6] = IERC20(Position.Quick_addr).balanceOf(address(this));//Quick
        balances[7] = IERC20(Position.WMatic_addr).balanceOf(address(this));//WMatic
        balances[8] = IERC20(HLSConfig.staking_reward).balanceOf(address(this));//stakedLP
        return balances ;
    }
...
/* unused-yet functions
    ...
    function getHLSStakedTokenAmount()public view returns(uint256, uint256){
        (uint256 token_a_amt, uint256 token_b_amt) = HLS_boosted.getStakedTokenDeNormAmount(Position.lp_token, Position.lp_token_amount);
        return (token_a_amt, token_b_amt) ;
    }
    
    // half-checked
    function getHLSTotalDebtsBoosted() public view returns(uint256){
        uint256 totalDebts = HLS_boosted.getTotalDebtsBoosted(HLSConfig, Position, true);
        return totalDebts ;
    }

    // checked
    function getHLSValueFromAmount(uint256 a_norm_amt, uint256 b_norm_amt) public view returns(uint256, uint256){
       (uint256 token_a_value, uint256 token_b_value) = HLS_boosted.getValueFromAmount(HLSConfig.token_a_oracle, HLSConfig.token_b_oracle, a_norm_amt, b_norm_amt);
       return (token_a_value,token_b_value) ;
    }

    // checked
    function getHLSAmountFromValue(uint256 a_value, uint256 b_value) public view returns(uint256, uint256){
        (uint256 normalized_a_amount, uint256 normalized_b_amount) = HLS_boosted.getAmountFromValue(HLSConfig.token_a_oracle, HLSConfig.token_b_oracle, a_value, b_value);
       return (normalized_a_amount, normalized_b_amount) ;
    }
    ...
    // checked
    function getHLSNormalizedAmount(uint256 a_amt, uint256 b_amt)public view returns(uint256, uint256){
        (uint256 norm_a_amt , uint256 norm_b_amt) = HLS_boosted.getNormalizedAmount(Position.token_a, Position.token_b, a_amt , b_amt );

        return (norm_a_amt, norm_b_amt) ;
    }

    // checked
    function getHLSDeNormalizedAmount(uint256 user_amount_a, uint256 user_amount_b) public view returns(uint256, uint256){
        (uint256 user_deNormAmount_a , uint256 user_deNormAmount_b) = HLS_boosted.getDeNormalizedAmount(Position.token_a, Position.token_b, user_amount_a, user_amount_b);
        
        return (user_deNormAmount_a, user_deNormAmount_b) ;
    }
*/
// -------------------------------------------------- ending of bunker ---------------------------------------------------
