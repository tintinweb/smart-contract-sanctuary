...
interface ITinyHeroFactory {
    ...
    function tokenInfo(uint256 tokenId) external view returns (uint256[] memory parents, uint256 mintedFee, uint256 birthDate, uint256 deathDate);

    function fusionInfo(uint256[] memory parents) external view returns (uint256 burnedFee, uint256[] memory otherInfo);

    function mintToken(address receiver, uint256 tokenId, ITinyHeroStats.TinyInfo memory info, ITinyHeroStats.TinyAttribute memory attribute) external;

    function mintMultiTokens(address receiver, uint256[] memory tokenId, ITinyHeroStats.TinyInfo[] memory info,  ITinyHeroStats.TinyAttribute[] memory attribute) external;
...
contract RinyHeroFactory is ITinyHeroFactory, OwnableUpgradeable, ReentrancyGuard {
    address private nft_;
    address private stats_;

    mapping(address => bool) private _admin;

    uint256 private fusionCount_;
    uint256 private mintedCount_;
    bool private allowRemint_;

    uint256[] private mintedTokens_;
    mapping(uint256 => uint256) private tokenMintedIndex_;
    mapping(uint256 => uint256[]) private tokenParents_;
    mapping(uint256 => uint256) private tokenMintedFee_;
    mapping(uint256 => uint256) private tokenBirthDate_;
    mapping(uint256 => uint256) private tokenDeathDate_;

    /* ========== EVENTS ========== */

    event AdminUpdate(address indexed account, bool isAdmin);
    event StatsUpdate(address oldStats, address newStats);
    event AllowRemintUpdate(bool oldAllowRemint, bool newAllowRemint);
    ...
    function initialize(address _nft, address _stats) external initializer {
        require(_nft != address(0), "zero");
        __Ownable_init();
        nft_ = _nft;
        stats_ = _stats;
        mintedTokens_.push(0);
    }
    ...
    function setStats(address _stats) external onlyOwner {
        ...
        emit StatsUpdate(stats_, _stats);
        stats_ = _stats;
    }

    function setAllowRemint(bool _allowed) external onlyOwner {
        emit AllowRemintUpdate(allowRemint_, _allowed);
        allowRemint_ = _allowed;
    }
    ...
    function stats() external override view returns (address) {
        return stats_;
    }

    function totalBurned() external override view returns (uint256) {
        return ITinyHeroNFT(nft_).totalBurned();
    }

    function totalFusion() external override view returns (uint256) {
        return fusionCount_;
    }

    function mintedTokens() external override view returns (uint256) {
        return mintedCount_;
    }

    function tokenMintedIndex(uint256 tokenId) external override view returns (uint256 index) {
        index = tokenMintedIndex_[tokenId];
    }

    function tokenByIndex(uint256 index) external override view returns (uint256 tokenId) {
        tokenId = mintedTokens_[index];
    }

    function tokenInfo(uint256 tokenId) external override view returns (uint256[] memory, uint256, uint256, uint256) {
        return (tokenParents_[tokenId], tokenMintedFee_[tokenId], tokenBirthDate_[tokenId], tokenDeathDate_[tokenId]);
    }

    function fusionInfo(uint256[] memory parents) external override view returns (uint256 mintedFee, uint256[] memory otherInfo) {
        mintedFee = type(uint256).max;
        otherInfo = new uint256[](0);
    }

    /* ========== MUTATIVE FUNCTIONS ========== */

    function mintToken(address receiver, uint256 tokenId, ITinyHeroStats.TinyInfo memory info, ITinyHeroStats.TinyAttribute memory attribute) public override onlyAdmin {
        require(allowRemint_ || tokenBirthDate_[tokenId] == 0, "Not allowed to remint tokens (burned or existing)");
        BRC721Enumerable(nft_).safeMint(receiver, tokenId);
        ITinyHeroStats(stats_).setDetails(tokenId, info, attribute);
        ++mintedCount_;
        mintedTokens_.push(tokenId);
        tokenMintedIndex_[tokenId] = mintedCount_;
        tokenMintedFee_[tokenId] = 0;
        tokenBirthDate_[tokenId] = block.timestamp;
        tokenDeathDate_[tokenId] = 0;
    }

    function mintMultiTokens(address receiver, uint256[] memory tokenIds, ITinyHeroStats.TinyInfo[] memory infos, ITinyHeroStats.TinyAttribute[] memory attributes) external override onlyAdmin {
        uint256 length = tokenIds.length;
        require(length >= 1, "zero token");
        require(length == infos.length && length == attributes.length, "Wrong array length");
        for (uint256 i = 0; i < length; i++) {
            mintToken(receiver, tokenIds[i], infos[i], attributes[i]);
        }
    }
    ...
    function transferMultiTokens(address receiver, uint256[] memory tokenIds) external {
        uint256 length = tokenIds.length;
        require(length >= 1, "zero token");
        ITinyHeroNFT _nft = ITinyHeroNFT(nft_);
        for (uint256 i = 0; i < length; i++) {
            uint256 _tokenId = tokenIds[i];
            if (_nft.ownerOf(_tokenId) == msg.sender) {
                _nft.safeTransferFrom(msg.sender, receiver, _tokenId);
            }
        }
    }

    function burnToken(uint256 tokenId) external override nonReentrant {
        ITinyHeroNFT _nft = ITinyHeroNFT(nft_);
        require(_nft.ownerOf(tokenId) == msg.sender, "TinyHeroFactory: burn token that is not own");
        _nft.burn(tokenId);
        tokenDeathDate_[tokenId] = block.timestamp;
    }

    function fusionToken(uint256[] memory parents) external override onlyAdmin {
