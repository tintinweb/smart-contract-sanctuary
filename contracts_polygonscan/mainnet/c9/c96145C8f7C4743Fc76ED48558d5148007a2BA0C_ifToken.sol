...
contract ifToken is ERC20, ERC20Detailed {
    ...
    function setPool(uint8 _pool, address _LP, uint8 _points) public onlyTreasury() {
        require(!exists[_pool], "setPool: pool's already been created. Use setPoints() to update.");
        require(router.getUniswapV2Pair(_LP) != address(0), "setPool: LP address is invalid.");
    ...
    function optimizePoints() public onlyOperator() {
        ...
        if(worst != best) {
            if(points[worst] < 10){
                ...
                points[worst]= (points[worst]-10);
            }

            if(points[best] < 200){
                points[best] = (points[best] + 10);
            }
        }
        invest();
    }
