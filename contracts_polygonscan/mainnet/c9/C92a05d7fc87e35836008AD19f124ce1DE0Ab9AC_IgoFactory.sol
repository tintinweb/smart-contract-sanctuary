...
contract IgoFactory is OwnableUpgradeable {
    using EnumerableSetUpgradeable for EnumerableSetUpgradeable.AddressSet;

    /// @notice Fees in basis point for each listing type
    uint16[] public fees;

    /// @notice Listing price for each type
    uint256[] public prices;

    address public feeTo;

    EnumerableSetUpgradeable.AddressSet private igos;

    event IgoCreated(address igoAddress);
    event FeesSet(uint16[] fees);
    event PricesSet(uint256[] fees);
    ...
    ) external returns (address igoAddress) {
        bytes memory bytecode = type(IgoSale).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(_token, block.number, msg.sender));
        assembly {
            igoAddress := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        uint16 fee = fees[_listingType];
        uint16[3] memory settings = [_liquidityPct, fee, _listingType];
        IgoSale igo = IgoSale(igoAddress);
        igo.initialize(_token, _router, _dates, _caps, _tokenLimits, settings, _features);
        igo.transferOwnership(msg.sender);
        igos.add(igoAddress);
        emit IgoCreated(igoAddress);
    }

    function setFees(uint16[] memory _fees) external onlyOwner {
        fees = _fees;
        emit FeesSet(_fees);
    }
