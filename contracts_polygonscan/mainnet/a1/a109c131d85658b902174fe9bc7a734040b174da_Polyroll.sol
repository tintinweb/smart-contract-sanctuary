...
// File: https://github.com/Uniswap/uniswap-v2-core/blob/272713c81b4b3fc413154f85c774f6cbaaf59d23/contracts/interfaces/IUniswapV2Pair.sol
...
contract Polyroll is VRFConsumerBase, Ownable, ReentrancyGuard {
    ...
    uint public rewardPct = 40;

    // Variables for computing price of ROLL in MATIC. Used for computing ROLL reward for transaction mining.
    uint constant SAFETY_FACTOR = 1e7; // Prevent truncation of price in integer datatype by multiplying price with SAFETY_FACTOR.
    address public rollMaticPair = 0x905DCc700fcce9a49b7D907E371230995a45ebCE; // Address of ROLL-MATIC LP
    uint public rollMaticRatio; // Price of ROLL/MATIC
    uint32 public rollMaticBlockTimestampLast;
    ...
    event BetSettled(uint indexed betId, address indexed gambler, uint outcome, uint winAmount);
    ...
    // Set rollMaticPair if we change our main trading pair for price reference.
    function setRollMaticPair(address _rollMaticPair) external onlyOwner {
        rollMaticPair = _rollMaticPair;
    }

    // Get MATIC/ROLL price with safeguards against oracle attacks.
    function updateRollMaticPair() internal {
        // Run function every 30 minutes
        if (rollMaticBlockTimestampLast - block.timestamp >= 1800) {

            // Get reserves of ROLL and MATIC in ROLL-MATIC Liquidity Pool.
            (uint reserve0, uint reserve1, uint32 blockTimestampLast) = IUniswapV2Pair(rollMaticPair).getReserves();

            // Get ratio of ROLL to MATIC multiplied by SAFETY_FACTOR to avoid losing precision in integer type.
            uint newRollMaticPair = reserve1 * SAFETY_FACTOR / reserve0;

            if (rollMaticRatio == 0) {
                // Update rollMaticPair if this is the first time updating.
                rollMaticRatio = newRollMaticPair;
            } else if (newRollMaticPair * SAFETY_FACTOR / rollMaticRatio > SAFETY_FACTOR * 110 / 100) {
                // Limit any increase in rollMaticRatio to +10% per update.
                rollMaticRatio = rollMaticRatio * 110 / 100;
            } else if (newRollMaticPair * SAFETY_FACTOR / rollMaticRatio < SAFETY_FACTOR * 90 / 100) {
                // Limit any decrease in rollMaticRatio to -20% per update.
                rollMaticRatio = rollMaticRatio * 80 / 100;
            } else {
                rollMaticRatio = newRollMaticPair;
            }
            rollMaticBlockTimestampLast = blockTimestampLast;
        }
    }
    ...
    function settleBet(bytes32 requestId, uint randomNumber) internal nonReentrant {
        ...
        emit BetSettled(betId, gambler, outcome, winAmount);
