...
contract LithRedeem is Ownable, ReentrancyGuard {
    ...
    constructor(uint256 _startBlock, address _plithAddress, address _lithAddress) {
        ...
        plithAddress = _plithAddress;
        lithAddress  = _lithAddress;
    }

    function swapPLithForLith(uint256 swapAmount) public nonReentrant {
        require(block.number >= startBlock, "lith redemption hasn't started yet, good things come to those that wait ;)");
        require(IBEP20(lithAddress).balanceOf(address(this)) >= swapAmount, "Not Enough tokens in contract for swap");
        IBEP20(plithAddress).transferFrom(address(msg.sender), address(this), swapAmount);
        IBEP20(lithAddress).transfer(address(msg.sender), swapAmount);
    }
