...
contract FeeDistributor is Initializable, Ownable {
    ...
    address[3] public whiteListedTokens = [usdc, usdt, dai];

    address public target;
    mapping(address => bool) public operators;

    IIronSwap public ironSwapPool;
    ...
    function initialize(address _target, address _ironSwapPool, address _blueIceStaking) external onlyOwner initializer {
        require (_target != address(0) && _ironSwapPool != address(0) && _blueIceStaking != address(0));
        target = _target;
        ironSwapPool = IIronSwap(_ironSwapPool);
    ...
    function swapPlainPool(address _fromTokenAddress) internal {
        IERC20 fromToken = IERC20(_fromTokenAddress);
        uint256 inAmount = fromToken.balanceOf(address(this));
        if (inAmount > 0) {
            uint8 fromIndex = ironSwapPool.getTokenIndex(_fromTokenAddress);
            uint8 toIndex = ironSwapPool.getTokenIndex(target);
            fromToken.safeIncreaseAllowance(address(ironSwapPool), inAmount);
            ironSwapPool.swap(fromIndex, toIndex, inAmount, 0, block.timestamp + swapTimeout);
        }
    }
    ...
    function setTarget(address _target) external onlyOwner {
        require(_target == usdc || target == usdt || target == dai, 'target-invalid');
        target = _target;
    }

    function setPool(address _pool) external onlyOwner {
        require(_pool != address(0), 'address-invalid');
        ironSwapPool = IIronSwap(_pool);
    }
