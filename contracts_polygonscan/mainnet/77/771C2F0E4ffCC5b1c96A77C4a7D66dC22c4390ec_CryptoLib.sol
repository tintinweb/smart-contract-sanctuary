...
library CryptoLib {
    ...
    function mint(CryptoStorage memory crypto, address to, uint256 amount) internal {
        crypto.lp_token.mint(to, amount);
    }

    function mint_relative(CryptoStorage memory crypto, address to, uint256 frac) internal returns(uint256) {
    ...
    function total_supply(CryptoStorage memory crypto) internal view returns(uint256) {
        return crypto.lp_token.totalSupply();
    }

    function xp(SwapStorage memory self) internal view returns(uint256[N_COINS] memory) {
    ...
    function _fee(FeeStorage memory fee, uint256[N_COINS] memory xp) internal view returns(uint256) {
        uint256 f = xp[0] + xp[1] ;
