...
contract StrategyToken is BaseStrategyToken {
    ...
    function _resetAllowances() internal override {
        ...
        IERC20(usdAddress).safeApprove(uniRouterAddress, uint256(0));
...
abstract contract BaseStrategyToken is BaseStrategy {
    ...
    function _earn(address _to) internal {
        ...
        if (earnedAmt > 0) {
            ...
            uint256 wmaticAmt = IERC20(wNativeAddress).balanceOf(address(this));

            if (wmaticAmt > 0 && earnedAddress != wantAddress){
                _safeSwap(
                    wmaticAmt,
                    wnativeToWantedPath,
                    address(this)
