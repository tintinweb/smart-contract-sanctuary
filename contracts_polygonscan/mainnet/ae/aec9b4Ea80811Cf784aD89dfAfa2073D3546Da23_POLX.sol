...
contract POLX is Context, IERC20, Ownable {
    ...
    // :TODO: declarar Polylastic Token V2 para manipularlo.
    // :TODO: Averiguar como manipular tokens del V2 siendo que es de otra red.

    string private constant _name = "POLX";
    ...
    feeRatesStruct public feeRates =
        feeRatesStruct({rfi: 3, liquidity: 3, Treasury: 1, Burn: 2});
    ...
    //     // This function is called when users from V2 wants to swap theirs tokens into V3:
    //     // We are going to swap in a one to one relationship between versions.
    //     // User must been aproved for this operation before calling it.

    //     // First at all, verify user balance in Token V2:
    //     amount = version2.balanceOf(msg.sender());
    //     require(amount > 0);

    //     // Burn amount of POLX V2 :TODO: CONSULTAR SOBRE LA FALTA DEL BURN EN V2.

    //     // We need to exclude from fees user involved:
    //     excludeFromRFI(msg.sender());

    //     // Make transfer between owner balance and user balance:
    //     transferFrom(owner(), msg.sender(), amount);

    //     // Return excluded value to user:
    //     includeInRFI(msg.sender());
