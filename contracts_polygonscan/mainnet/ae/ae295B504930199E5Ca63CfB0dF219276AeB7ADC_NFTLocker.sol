...
contract NFTLocker is IERC721Receiver, OwnableUpgradeable, ReentrancyGuard {
    using Address for address;
    ...
    function lock(uint256 tokenId, address locker) external nonReentrant {
        BRC721 _nft = BRC721(nft_);
        require(_nft.ownerOf(tokenId) == msg.sender, "!own");
        if (_locked[locker] != 0) {
            _nft.safeTransferFrom(address(this), msg.sender, _locked[locker]);
        }
        _nft.safeTransferFrom(msg.sender, address(this), tokenId);
        _locked[locker] = tokenId;
    }

    function unlock(uint256 tokenId) external {
        BRC721 _nft = BRC721(nft_);
        require(_nft.ownerOf(tokenId) == address(this), "!own");
        uint256 _tokenIdLocked = _locked[msg.sender];
        require(_tokenIdLocked == tokenId || owner() == msg.sender, "TinyHeroNFT: caller is not locker nor admin");
