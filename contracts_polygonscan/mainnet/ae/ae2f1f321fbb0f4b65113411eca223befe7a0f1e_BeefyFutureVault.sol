...
import "contracts/interfaces/platforms/ImooToken.sol";

/**
 * @title Contract for Beefy Future
 * @notice Handles the future mechanisms for mooTokens
 */
contract BeefyFutureVault is HybridFutureVault {
    ...
    function getIBTRate() public view override returns (uint256) {
        return ImooToken(address(ibt)).balance().mul(IBT_UNIT).div(ImooToken(address(ibt)).totalSupply());
    }
}
