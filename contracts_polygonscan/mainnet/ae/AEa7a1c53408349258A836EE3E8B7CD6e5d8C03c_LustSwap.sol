...
/*
    ERROR REF
    ---------
    E1: Lust cannot be equal to desire
    E2: lust is not insane yet.
    E3: Not Enough tokens in contract for swap
    E4: failed sending lust
    E5: can only send excess lust to dead address after presale has ended
    E6: can only burn unsold presale once!
    E7: burning too much desire, check again please
    E8: cannot change start block if presale has already commenced
    E9: cannot set start block in the past
*/
import "./LustToken.sol";

contract LustSwap is Ownable, ReentrancyGuard {
    ...
    // Tokens
    LustToken public immutable  lustToken;
    
    IERC20 public immutable desireToken;

    bool hasBurnedUnsoldPresale;

    uint256 public startBlock;

    event LustToDesire(address sender, uint256 amount);
    event burnUnclaimedLust(uint256 amount);
    event startBlockChanged(uint256 newStartBlock);

    constructor(uint256 _startBlock, address _lustAddress, address _desireAddress) {
        require(_lustAddress != _desireAddress, "E1");
        
        startBlock = _startBlock;
        lustToken = LustToken(_lustAddress);
        desireToken = IERC20(_desireAddress);
    }

    function swapLustForDesire() external nonReentrant {
        require(block.number >= startBlock, "E2");

        uint256 swapAmount = lustToken.balanceOf(msg.sender);
        
        require(desireToken.balanceOf(address(this)) >= swapAmount, "E3");
        require(lustToken.transferFrom(msg.sender, BURN_ADDRESS, swapAmount), "E4");
        
        desireToken.safeTransfer(msg.sender, swapAmount);

        emit LustToDesire(msg.sender, swapAmount);
    }

    function sendUnclaimedDesireToDeadAddress() external onlyOwner {
        require(block.number > lustToken.endBlock(), "E5");
        require(!hasBurnedUnsoldPresale, "E6");
        require(lustToken.lustRemaining() <= desireToken.balanceOf(address(this)), "E7");

        if (lustToken.lustRemaining() > 0)
            desireToken.safeTransfer(BURN_ADDRESS, lustToken.lustRemaining());
        hasBurnedUnsoldPresale = true;

        emit burnUnclaimedLust(lustToken.lustRemaining());
    }
