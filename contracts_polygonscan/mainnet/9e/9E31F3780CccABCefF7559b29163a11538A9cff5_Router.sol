...
contract Router {
    ...
    bool public called;
    Pool[] public pools;
    mapping(Pool => int64) public baseTokenBalances;
    mapping(Pool => int64) public tokenBalances;
    event Buy(address indexed sender, int64 inputAmount, int64 outputAmount);
    event Sell(address indexed sender, int64 inputAmount, int64 outputAmount);
    event Convert(
    ...
    constructor(IERC20Metadata _baseToken) {
    ...
    function convert(
        ...
        emit Convert(msg.sender, inputAmount, outputAmount);
    ...
    function createYieldFarmablePool(
        ...
        chargeBaseToken(pool, initialBaseTokenAmount);
        chargeToken(pool, initialTokenAmount);
        pool.mint(msg.sender, uint256(uint64(initialTokenAmount)));
        pools.push(pool);
    }

    function addLiquidity(Pool pool, int64 inputAmount) public {
        pool.mint(
            msg.sender,
            proportionOf(
                inputAmount,
    ...
    function removeLiquidity(Pool pool, int64 amount) public {
    ...
    function buy(Pool pool, int64 inputAmount) public {
        int64 outputAmount =
            calculateOutputAmount(
                pool,
                baseTokenBalances[pool],
        ...
        emit Buy(msg.sender, inputAmount, outputAmount);
    }

    function sell(Pool pool, int64 inputAmount) public {
        int64 outputAmount =
            calculateOutputAmount(
                pool,
                tokenBalances[pool],
        ...
        payBaseToken(pool, outputAmount);
        chargeToken(pool, inputAmount);
        emit Sell(msg.sender, inputAmount, outputAmount);
    }

    function payToken(Pool pool, int64 amount) public {
        IERC20Metadata token = pool.token();
        token.safeTransfer(msg.sender, scaleUpTokenAmount(token, amount));
        tokenBalances[pool] -= amount;
    }

    function chargeToken(Pool pool, int64 amount) public {
        IERC20Metadata token = pool.token();
        token.safeTransferFrom(
            msg.sender,
            address(this),
            scaleUpTokenAmount(token, amount)
        );
    ...
    function scaleDownTokenAmount(IERC20Metadata token, uint256 amount)
        public
        view
        returns (int64)
    {
        uint64 decimals = uint64((token.decimals()));
        if (decimals > 6) {
            return int64(uint64(amount / (10**(decimals - uint64(6)))));
        } else if (decimals < 6) {
            return
                int64(
                    uint64(uint256(amount)) * uint64(10)**(uint64(decimals) - 6)
                );
        } else {
    ...
    function transferBaseToken(
        ...
        baseTokenBalances[inputPool] -= amount;
        baseTokenBalances[outputPool] += amount;
    }

    function payBaseToken(Pool pool, int64 amount) public {
        baseTokenBalances[pool] -= amount;
        baseToken.safeTransfer(msg.sender, uint256(uint64(amount)));
    }

    function chargeBaseToken(Pool pool, int64 amount) public {
    ...
    function calculateOutputAmount(
        ...
        return
            ...
            (inputSupply +
                inputAmount -
                ((inputAmount * pool.liquidityFee()) / 1000));
    }
...
contract Pool is ERC20 {
    ...
    mapping(address => int64) harvested;
    int64 public immutable liquidityFee;
    ...
    function safeTransfer(
        ...
        _token.safeTransfer(sender, uint256(uint64(amount)));
    }
    ...
    function _beforeTokenTransfer(
        ...
        require(
            ...
            "token is non transferrable"
        );
        super._beforeTokenTransfer(from, to, tokenId);
    }

    function mint(address to, int64 amount) public virtual onlyCreator {
        _mint(to, uint256(uint64(amount)));
    }

    function burn(address who, int64 amount) public virtual onlyCreator {
        _burn(who, uint256(uint64(amount)));
    }
...
contract YieldFarmablePool is YieldFarmable, Pool {
    ...
    function _beforeTokenTransfer(
        ...
        address to,
        uint256 tokenId
    ) internal virtual override(Pool, ERC20) {
        super._beforeTokenTransfer(from, to, tokenId);
    }
