...
contract LiquidateLendsV1 is Ownable {

    using SafeERC20 for IERC20;

    enum SwapType {SimpleSwap, TriangularSwap}
    ...
    constructor(address _WETH, address[] memory _cEth) {
        WETH = _WETH;
        setEthTokens(_cEth, true);
    }
    ...
    function liquidate(address borrower, address tokenCollateral, address tokenBorrow, uint repayAmount, address flashBorrowPair, address flashPayPair, address againstToken) external {
        ...
        if (repayAmount == 0) {
            repayAmount = _calculateAmount(borrower, tokenBorrow, tokenCollateral);
        }
        if (flashPayPair == address(0)) {
            flashPayPair = flashBorrowPair;
        }

        bytes memory data = abi.encode(tokenBorrow, tokenCollateral, borrower, repayAmount, flashBorrowPair, flashPayPair, againstToken);

        address _tokenBorrow = _isEthToken(tokenBorrow) ? WETH : IErcToken(tokenBorrow).underlying();
        address _tokenPay = _isEthToken(tokenCollateral) ? WETH : IErcToken(tokenCollateral).underlying();

        if (_tokenBorrow == _tokenPay || _tokenBorrow == WETH || _tokenPay == WETH) {
            simpleFlashLoan(_tokenBorrow, repayAmount, _tokenPay, data);
        } else {
            triangularFlashLoan(_tokenBorrow, repayAmount, _tokenPay, data);
        }

        transfer(_tokenPay, payable(owner()), type(uint256).max);
    }

    function uniswapV2Call(address _sender, uint _amount0, uint _amount1, bytes calldata _data) external {
        _flashloanCallback(_sender, _amount0, _amount1, _data);
    }

    function _flashloanCallback(address _sender, uint _amount0, uint _amount1, bytes calldata _data) internal {
        (
        SwapType _swapType,
        ...
        if (_swapType == SwapType.SimpleSwap) {
            simpleFlashLoanExecute(_tokenBorrow, _amount, _tokenPay, msg.sender, _userData);
        } else if (_swapType == SwapType.TriangularSwap) {
            triangularFlashLoanExecute(_tokenBorrow, _amount, _tokenPay, _triangleData, _userData);
        }
    }

    function simpleFlashLoan(address _tokenBorrow, uint256 _amount, address _tokenPay, bytes memory _userData) private {

        (,,,, address pairAddress,,) = abi.decode(_userData, (address, address, address, uint, address, address, address));

        uint amount0Out = _tokenBorrow == IUniswapV2Pair(pairAddress).token0() ? _amount : 0;
        uint amount1Out = _tokenBorrow == IUniswapV2Pair(pairAddress).token1() ? _amount : 0;
    ...
    function triangularFlashLoan(address _tokenBorrow, uint _amount, address _tokenPay, bytes memory _userData) private {
        ...
        uint amount0Out = againstToken == IUniswapV2Pair(payPairAddress).token0() ? amountOfAgainstToken : 0;
        uint amount1Out = againstToken == IUniswapV2Pair(payPairAddress).token1() ? amountOfAgainstToken : 0;
        ...
        bytes memory data = abi.encode(
            ...
            _amount,
            _tokenPay,
            abi.encode(borrowPairAddress, amountOfAgainstToken),
            _userData
        );
    ...
    function simpleFlashLoanExecute(address _tokenBorrow, uint _amount, address _tokenPay, address _pairAddress, bytes memory _userData) private {

        bool _isBorrowingEth = _tokenBorrow == WETH;
        bool _isPayingEth = _tokenPay == WETH;
        ...
        IERC20(_tokenPay).safeTransfer(_pairAddress, amountToRepay);
    }
    ...
    function triangularFlashLoanExecute(
        ...
        IERC20(_tokenPay).safeTransfer(_payPairAddress, amountToRepay);
    ...
    function _calculateAmount(address _borrower, address _tokenBorrow, address _tokenCollateral) internal returns (uint256){
        ...
        uint repayMax = IToken(_tokenBorrow).borrowBalanceCurrent(_borrower) * closeFact / uint(10 ** 18);
        uint seizeMax = IErcToken(_tokenCollateral).balanceOfUnderlying(_borrower) * uint(10 ** 18) / liqIncent;
        uint uPriceBorrow = _getUnderlyingPrice(_tokenBorrow);

        repayMax *= uPriceBorrow;
        seizeMax *= _getUnderlyingPrice(_tokenCollateral);
    ...
    function _getUnderlyingPrice(address _cToken) internal view returns (uint){
        IComptroller comptroller = IComptroller(IToken(_cToken).comptroller());
