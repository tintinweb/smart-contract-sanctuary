...
// Coded by Yanis (currently @ygboucherk on instagram, telegram or whatever - but yanis#3059 on discord)
...
contract SiriCoin is Owned {
	...
	constructor() {
		currentChallenge = keccak256("Waiting for block 69420 :D");
	}
	...
	function _newEpoch(uint256 _nonce) internal {
		currentChallenge = bytes32(keccak256(abi.encodePacked(_nonce, currentChallenge, blockhash(block.number - 1), "Hi Siri")));
		if ((epochCount%128) == 0) {
	...
	function _mint(uint256 nonce, bytes32 challenge_digest, address _miner, uint256 feeToPool, address pool) public returns (bool) {
		...
		require(feeToPool <= 100, "Wut fees cant be more than 100%");
		...
		if (feeToPool > 0) {
			...
			balances[pool] = balances[pool].add(poolFees);
			emit Transfer(address(0), pool, poolFees);
		}
		else {
			minerRewards = totalReward;
		}
		balances[_miner] = balances[_miner].add(minerRewards);
		_totalSupply = _totalSupply.add(totalReward);
		emit Mint(_miner, totalReward, epochCount, currentChallenge);
		emit Transfer(address(0), _miner, minerRewards);
		_newEpoch(nonce);
	...
	function mint(uint256 nonce, bytes32 challenge_digest) public returns (bool success) {
		return _mint(nonce, challenge_digest, msg.sender, 0, address(0));
	}
}