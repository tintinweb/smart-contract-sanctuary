...
contract XVMCtimeDeposit is Ownable, Pausable {
    ...
    //there is no trusted pool, but we need it in case pools are changed
     ...
     * There is no trusted pool for last pool, but we need it in case we change the pool contract
     */
    ...
    function withdraw(uint256 _shares) public notContract {
        ...
        if (block.timestamp < user.lastDepositedTime.add(withdrawFeePeriod)) {
            uint256 withdrawFee = uint256(82).sub(((block.timestamp - user.lastDepositedTime).div(86400)).mul(4269).div(100000));
    ...
    function hopDeposit(uint256 _amount, address _recipientAddress, uint256 previousLastDepositedTime) external whenNotPaused {
        require(
            msg.sender == trustedSender1 || msg.sender == trustedSender2 || msg.sender == trustedSender3 || 
            msg.sender == trustedSender4 || msg.sender == trustedSender5,
