...
contract PolySafe_Test09 is Token {
    
    uint private startTime;
    
    uint CB = 0; // Quitar despues
    ...
    uint private constant TIME_TO_UNSTAKE   = 5 * 60;  // 7 days;
    uint private constant NEXT_AIRDROP      = 5 * 60;  // 7 days;
    uint private constant BON_AIRDROP       = 5;
    uint private constant SELL_LIMIT        = 30000 ether; 
    ...
    function changeStartTime(uint unixTime) external onlyOwner {
	    startTime = unixTime;
    } 
    
    function changeUserBonAirdrop(uint bon, address usr) external onlyOwner {
	    users[usr].bonAirdrop = bon;
    }    
