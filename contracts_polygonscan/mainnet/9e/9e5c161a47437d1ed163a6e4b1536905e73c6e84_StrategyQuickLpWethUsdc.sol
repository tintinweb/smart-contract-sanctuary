...
abstract contract StrategyBase is Ownable, ReentrancyGuard, Pausable, IStrategy {
    ...
    address public profitReceiver = address(0xf0024A25c99d673FBA590e66331DddB3B76Bf735); // Treasury Fund
    ...
    address public timelock = address(0x245034C4FcD4EE3FC0e645B0E158C0AA0B05062A); // 24h timelock
    ...
    constructor(
        ...
        if (_farmingToken2UsdcPath.length == 0) quickPaths[_farmingToken][usdc] = [farmingToken, usdc];
        else quickPaths[_farmingToken][usdc] = _farmingToken2UsdcPath;

        if (_farmingToken2TargetProfitTokenPath.length == 0) quickPaths[_farmingToken][targetProfitToken] = [farmingToken, usdc, _targetProfitToken];
        else quickPaths[_farmingToken][targetProfitToken] = _farmingToken2TargetProfitTokenPath;
    ...
    function _checkAutoEarn() internal {
        if (!notPublic && pendingHarvestDollarValue() >= autoEarnLimit) {
            earn();
        }
    }
    ...
    function withdraw(address, uint256 _wantAmt) public override onlyController nonReentrant {
    ...
    function totalBalance() external view override returns (uint256) {
        return IERC20(want).balanceOf(address(this)).add(inFarmBalance());
    }
    ...
    function earn() public whenNotPaused {
        ...
        _earnedAmt = _earnedAmt.sub(_distributeFee);
    ...
    function _distributeFees(uint256 _earnedAmt) internal returns (uint256 _fee) {
        if (_earnedAmt > 0) {
            // Performance fee
            if (operatorFee > 0) {
                _fee = _earnedAmt.mul(operatorFee).div(operatorFeeMax);
                address _farmingToken = farmingToken;
    ...
    function exchangeRate(
        ...
        uint256[] memory amounts = IUniswapV2Router(quickRouter).getAmountsOut(_tokenAmount, _path);
        return amounts[amounts.length - 1];
    }

    function pendingHarvestDollarValue() public view returns (uint256) {
        uint256 _pending = pendingHarvest();
        return (_pending == 0) ? 0 : exchangeRate(farmingToken, usdc, _pending).mul(USDC_MISSING_DECIMALS_MULTIPLER);
    }
    ...
    function setMainPaths(address[] memory _farmingToken2UsdcPath, address[] memory _farmingToken2TargetProfitTokenPath) external onlyOwner {
        address _farmingToken = farmingToken;
        quickPaths[_farmingToken][usdc] = _farmingToken2UsdcPath;
        quickPaths[_farmingToken][targetProfitToken] = _farmingToken2TargetProfitTokenPath;
    }
    ...
    function _swapToken(
        ...
        address[] memory _path = quickPaths[_inputToken][_outputToken];
        if (_path.length == 0) {
            _path = new address[](2);
            _path[0] = _inputToken;
            _path[1] = _outputToken;
        }
        IERC20(_inputToken).safeIncreaseAllowance(address(quickRouter), _amount);
        IUniswapV2Router(quickRouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(_amount, 1, _path, _to, now.add(60));
    }
...
contract StrategyQuickLpWethUsdc is StrategyBase {
    // _controller = ShareRewardBank
    // _want = QLP WETH/USDC
    // _farmingToken = QUICK
    // _farmingPool = StakingRewards (earns QUICK)
    // _farmingPoolId = 0 // not used
    // _targetProfitToken = SDS
    // _farmingToken2UsdcPath = [QUICK, USDC]
    // _farmingToken2TargetProfitTokenPath = [QUICK, USDC, SDS]
    constructor(
        address _controller,
        address _want,
        address _farmingToken,
    ...
    function getName() public pure override returns (string memory) {
        return "SafeDollar.Fi:StrategyQuickLpWethUsdc";
    }
    ...
    function _farm() internal override {
        ...
        IStakingRewards(farmingPool).stake(_wantBal);
    }

    function _withdrawSome(uint256 _amount) internal override {
        IStakingRewards(farmingPool).withdraw(_amount);
    }

    function _exit() internal override {
        IStakingRewards(farmingPool).exit();
    }

    function inFarmBalance() public view override returns (uint256) {
        return IStakingRewards(farmingPool).balanceOf(address(this));
    }

    function _harvest() internal override {
        return IStakingRewards(farmingPool).getReward();
    }

    function pendingHarvest() public view override returns (uint256) {
        return IStakingRewards(farmingPool).earned(address(this));
    }
}