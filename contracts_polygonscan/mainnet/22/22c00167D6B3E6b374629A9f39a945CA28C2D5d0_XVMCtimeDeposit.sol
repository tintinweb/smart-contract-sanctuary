...
contract XVMCtimeDeposit {
    ...
    //address public oldPool = 0x9b6ae196A358Ea81c305D8A32018a4F4C90FC207; //old pool we can migrate from
    
    address public trustedSender1; //need trusted sender for migration
    ...
    uint256 public callFee = 10; // 0.15%
    ...
    function voteForProposal(uint256 proposalID) external {
        ...
        if(proposalID != 0) { 
            if(userVote[msg.sender].delegatingSharesToProposalID != 0) { //if already voting, subtract from old, add to new
                ...
                _updateVotingAddDiff(msg.sender, proposalID, userTotalShares); //if not voting, just add to new
            }
        } else {
            _updateVotingSubDiff(msg.sender, userVote[msg.sender].delegatingSharesToProposalID, userTotalShares); //if resetting to 0, remove from old
        }
