...
contract LiquidityLocker is Ownable {
	...
	mapping(address => LockInfo) lockedTokens;
	...
	function lockLiquidity(address _lptoken, uint256 _amount) external onlyOwner {
		...
		LockInfo storage lockInfo = lockedTokens[_lptoken];
		lockInfo.lockAmount = lockInfo.lockAmount.add(_amount);
	...
	function unlockLiquidity(address _lptoken) external onlyOwner {
		LockInfo storage lockInfo = lockedTokens[_lptoken];
		require(block.timestamp >= lockInfo.lockDate.add(lockDuration), "can't unlock yet");
		IBEP20 lptoken = IBEP20(_lptoken);
		lptoken.safeTransfer(address(owner()), lockInfo.lockAmount);
		lockInfo.lockAmount = 0;
		emit LiquidityUnlocked(lockInfo.lockAmount);
	}
}