...
// File: contracts/libs/ICRABReferral.sol


pragma solidity 0.6.12;

interface ICRABReferral {
    /**
     * @dev Record referral.
     */
...
contract Crabswaptoken is BEP20("CRABSWAP", "CRAB") {
    // Burn address
    ...
    /// @dev overrides transfer function to meet tokenomics of CRAB
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        if (recipient == BURN_ADDRESS) {
            ...
            require(amount == sendAmount + burnAmount, "CRAB::transfer: Burn value invalid");
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "CRAB::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "CRAB::delegateBySig: invalid nonce");
        require(now <= expiry, "CRAB::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "CRAB::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying CRABs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "CRAB::_writeCheckpoint: block number exceeds 32 bits");
...
// MasterChef is the master of CRAB. He can make CRAB and he is a fair guy.
//
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    // CRAB referral contract address.
    ICRABReferral public CRABReferral;
    ...
    constructor(
        ...
        CRABPerBlock = INITIAL_EMISSION_RATE;
    }
    ...
    // Update the given pool's CRAB allocation point and deposit fee. Can only be called by the owner.
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 CRABReward = multiplier.mul(CRABPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        CRAB.mint(devAddress, CRABReward.div(10));
    ...
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        ...
        updatePool(_pid);
        if (_amount > 0 && address(CRABReferral) != address(0) && _referrer != address(0) && _referrer != msg.sender) {
            CRABReferral.recordReferral(msg.sender, _referrer);
        }
        if (user.amount > 0) {
        ...
        if (_amount > 0) {
            ...
            if (address(pool.lpToken) == address(CRAB)) {
                // burn 1% of token
    ...
    function safeCRABTransfer(address _to, uint256 _amount) internal {
        ...
        require(transferSuccess, "safeCRABTransfer: Transfer failed");
    }
    ...
    function updateEmissionRate() public {
        ...
        require(CRABPerBlock > MINIMUM_EMISSION_RATE, "updateEmissionRate: Emission rate has reached the minimum threshold");
        ...
        uint256 newEmissionRate = CRABPerBlock;
        ...
        if (newEmissionRate >= CRABPerBlock) {
            return;
        }

        massUpdatePools();
        lastReductionPeriodIndex = currentIndex;
        uint256 previousEmissionRate = CRABPerBlock;
        CRABPerBlock = newEmissionRate;
        emit EmissionRateUpdated(msg.sender, previousEmissionRate, newEmissionRate);
    }

    // Update the CRAB referral contract address by the owner
    function setCRABReferral(ICRABReferral _CRABReferral) public onlyOwner {
        CRABReferral = _CRABReferral;
    }
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (address(CRABReferral) != address(0) && referralCommissionRate > 0) {
            address referrer = CRABReferral.getReferrer(_user);
            uint256 commissionAmount = _pending.mul(referralCommissionRate).div(10000);

            if (referrer != address(0) && commissionAmount > 0) {
                CRAB.mint(referrer, commissionAmount);
