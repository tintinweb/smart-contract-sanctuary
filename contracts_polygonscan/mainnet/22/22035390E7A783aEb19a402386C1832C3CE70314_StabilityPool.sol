...
contract StabilityPool is LiquityBase, Ownable, IStabilityPool {
    ...
    uint256 internal collAmount;  // deposited Token tracker
    ...
    */
    ...
    event StabilityPoolLUSDBalanceUpdated(uint _newBalance);
    // Fix
    event StabilityPoolTokenBalanceUpdated(uint _newBalance);
    ...
    event ETHGainWithdrawn(address indexed _depositor, uint _ETH, uint _LUSDLoss);
    // Fix
    event TokenGainWithdrawn(address indexed _depositor, uint _AMOUNT, uint _LUSDLoss);
    event LQTYPaidToDepositor(address indexed _depositor, uint _LQTY);
    event LQTYPaidToFrontEnd(address indexed _frontEnd, uint _LQTY);
    event EtherSent(address _to, uint _amount);
    // Fix
    event TokenSent(address _to, uint _amount);
    ...
    * - Updates snapshots for deposit and tagged front end stake */
    function withdrawETHGainToTrove(address _upperHint, address _lowerHint) external override {
        ...
        collAmount = collAmount.sub(depositorETHGain);
        emit StabilityPoolETHBalanceUpdated(collAmount);
        emit EtherSent(msg.sender, depositorETHGain);

        SafeToken.safeApprove(collTokenAddress, address(borrowerOperations), uint(-1));
    ...
    */
    ...
    function _moveOffsetCollAndDebt(uint _collToAdd, uint _debtToOffset) internal {
        ...
        activePoolCached.sendToken(address(this), _collToAdd);
    }
    ...
    */
    ...
    function _sendETHGainToDepositor(uint _amount) internal {
        if (_amount == 0) {return;}
        uint newCollAmount = collAmount.sub(_amount);
        collAmount = newCollAmount;
        emit StabilityPoolETHBalanceUpdated(newCollAmount);
        ...
        if (collTokenAddress == GAS_TOKEN_ADDR) {
            ...
            SafeToken.safeTransfer(collTokenAddress, msg.sender, _amount);
        }
    }
    ...
    function increaseColl(uint256 _amount) external override {
        ...
        StabilityPoolETHBalanceUpdated(collAmount);
    }
