...
contract GuildWhitelistedNFTStaking is BaseRelayRecipient {
    ...
    */
    ...
    function stake(address _whitelistedNFT, address to_user,uint256 _tokenId) external {
        // require();
        _stake(_msgSender(),to_user, _whitelistedNFT, _tokenId);
    }

    /// @notice Stake multiple NFTs and earn reward tokens.
    function stakeBatch(address to_user,address[] memory _whitelistedNFTs,uint256[] memory _tokenIds) external {
        ...
        for (uint i = 0; i < _tokenIds.length; i++) {
            _stake(_msgSender(),to_user, _whitelistedNFTs[i], _tokenIds[i]);
        }
    }
    ...
    */
    function _stake(address _user, address to_user,address _whitelistedNFT, uint256 _tokenId) internal {
