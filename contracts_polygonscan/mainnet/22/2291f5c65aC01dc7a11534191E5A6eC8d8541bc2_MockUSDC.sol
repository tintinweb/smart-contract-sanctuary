...
// Dependency file: contracts\AccessProtected.sol
...
// Root file: contracts\MockUSDC.sol

pragma solidity ^0.8.4;

// import {ERC20} from "contracts\ERC20.sol";
// import {AccessProtected} from "contracts\AccessProtected.sol";
...
contract MockUSDC is ERC20("Mock USDC", "USDC"), AccessProtected {
    ...
    bool public limitMinting = false;
    mapping(address => bool) public hasMinted; // True if the user has already minted in "limitMinting" mode
    mapping(address => bool) public isWhitelisted; // True if the user can send USDC to / receive USDC from any address
    ...
    function mint(address _to) public {
        if (!isWhitelisted[msg.sender]) {
            require(!isRestricted, "Restricted mode on, only admins can call");

            if (limitMinting) {
                require(!hasMinted[msg.sender], "User has already minted");
                require(
                    balanceOf(msg.sender) == 0,
                    "User already has USDC on his wallet"
                );
    ...
    function setRestrictionMode(bool _restrictionMode) external onlyOwner {
        isRestricted = _restrictionMode;
    }

    function setLimitMinting(bool _limitMinting) external onlyOwner {
        limitMinting = _limitMinting;
    }
        ...
        override
    {
        require(
            ...
            "One of the two addresses in the transfer is not whitelisted"
        );
    }
}