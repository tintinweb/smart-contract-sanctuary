...
contract MasterChef is Ownable {
    ...
    event OnStake(address indexed user, uint256 indexed pid, uint256 amount);
    event OnUnstake(address indexed user, uint256 indexed pid, uint256 amount);
    ...
    function enterStaking(uint256 _amount) public {
        ...
        stakeValue[msg.sender] = user.amount = user.amount.add(_amount);
        user.rewardDebt = user.amount.mul(pool.accRewardPerShare).div(1e12);
        emit OnStake(msg.sender, _pid, _amount);
    }
    ...
    function leaveStaking(uint256 _amount) public {
        ...
        if(_amount > 0) {
            stakeValue[msg.sender] = user.amount = user.amount.sub(_amount);
            pool.lpToken.safeTransfer(address(msg.sender), _amount);
        }
        user.rewardDebt = user.amount.mul(pool.accRewardPerShare).div(1e12);
        emit OnUnstake(msg.sender, _pid, _amount);
    }
