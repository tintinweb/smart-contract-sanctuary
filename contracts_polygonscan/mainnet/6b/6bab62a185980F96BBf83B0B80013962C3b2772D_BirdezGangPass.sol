...
contract BirdezGangPass is ERC721Enumerable, Ownable {
    using Strings for uint256;

    constructor()
        ERC721("Priority Pass Birdez Island", "BIRDEZPASS")
    {}
    ...
    function mintTo(address[] memory _addresses) external onlyOwner {
        require(!isMintLocked,"minting-is-locked");
        for (uint256 i = 0; i < _addresses.length; i++) {
            // Prevents duplicate minting to same address
            if (maxMintsPerAddress[_addresses[i]] == 0) {
                _safeMint(_addresses[i], totalSupply());
                maxMintsPerAddress[_addresses[i]]++;
            }
        }
    }
