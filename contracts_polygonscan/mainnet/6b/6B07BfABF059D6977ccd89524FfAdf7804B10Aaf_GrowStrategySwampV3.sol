...
abstract contract BaseGrowStrategyV3 is  IGrowStrategy, Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function _unsettledPerformanceFee(uint256 underlyingWantTokenAmount) internal view returns(uint256) {
        ...
        if (underlyingWantTokenAmount <= lastUnderlyingWantTokenAmount) {
            return unsettledProfit.mul(performanceFeeRate).div(10000);
        }
    ...
    function harvest(bool harvestFromUnderlying, bool chargePerformanceFee) external onlyRole(KEEPER_ROLE) {
        ...
        // if need chargePerformanceFee, must harvestFromUnderlying
        if (chargePerformanceFee) {
            harvestFromUnderlying = true;
        }
...
contract GrowStrategySwampV3 is IGrowUpgradeableImplementation, BaseGrowStrategyV3 {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    bytes32 override public constant CONTRACT_IDENTIFIER = keccak256("GrowStrategySwampV3");
    ...
    function migrateToNewPool(uint256 _NEW_MASTER_CHEF_LIKE_POOL_ID) external onlyRole(CONFIGURATOR_ROLE) {
        (address _STAKING_TOKEN,,,,address _UNDERLYING_STRATEGY) = IMasterChefLike(MASTER_CHEF_LIKE).poolInfo(_NEW_MASTER_CHEF_LIKE_POOL_ID);
        ...
        IMasterChefLike(MASTER_CHEF_LIKE).deposit(_NEW_MASTER_CHEF_LIKE_POOL_ID, currentBalance);
