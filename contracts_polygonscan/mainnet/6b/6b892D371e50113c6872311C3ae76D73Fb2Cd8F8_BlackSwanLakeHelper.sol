...
import "./BlackSwanLake.sol";
import "./RewardPool.sol";
import "./interfaces/Interfaces.sol";

contract BlackSwanLakeHelper {
	BlackSwanLake public swanLake;
	...
	constructor(
		...
		swanLake = BlackSwanLake(payable(_swanLake));
	...
	function getEarnedRewards(address user, uint256 accAmountPerShareSwan)
		public
		view
		returns (uint256, uint256)
	{
		...
		uint256 _accAmountPerShare = swanLake.interestsPerBlock();
		uint256 _accAmountPerShareSwan = accAmountPerShareSwan;
		(, uint256 totalProductivity) = swanLake.getProductivity(user);
		// uint256 lpSupply = totalProductivity;
		if (totalProductivity != 0) {
			uint256 swanCurrentReward = swan.balanceOf(address(rewardPool));
			uint256 poolTokenCurrentReward = rewardPool.balance();
			uint256 multiplier = block.number - swanLake.lastRewardBlock();

			_accAmountPerShare =
				_accAmountPerShare +
				((poolTokenCurrentReward * multiplier * 1e27) /
					totalProductivity);
			_accAmountPerShareSwan =
				_accAmountPerShareSwan +
				(swanCurrentReward * multiplier * 1e27) /
				totalProductivity;
		}
		return (
			(amount * _accAmountPerShare) / 1e27 + rewardEarn - rewardDebt,
			(amount * _accAmountPerShareSwan) /
				1e27 +
				rewardEarnSwan -
				rewardDebtSwan
		);
