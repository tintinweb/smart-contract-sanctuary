...
contract NoReservationPurchaseLDA is RoyalPausableUpgradeable {
    // Events
    event OnChainPurchase(address indexed buyer, uint256 indexed ldaID, address indexed erc20Contract, uint256 amount, string correlationID);
    event OffChainTransfer(address indexed buyer, uint256 indexed ldaID, uint256 amount, string correlationID);
    event TierSoldOut(uint128 indexed tierID, address indexed ldaContract, uint256 numTokens);
    ...
    function registerTierForPurchase(uint128 tierID, uint256 priceWei, address erc20Contract, address ldaContract, address ldaHolderForTier) external onlyOwner {
        ...
        require(Royal1155LDA(ldaContract).tierMaxSupply(tierID) != 0, "tier DNE in LDA contract");
        ...
        maxSupplyForTier[tierID] = Royal1155LDA(ldaContract).tierMaxSupply(tierID);
    }
    ...
    function tierRegistered(uint128 tierID) public view returns (bool registered) {
        return tierPurchasePriceWei[tierID] != 0 && tierLDAContract[tierID] != address(0) && tierLDAHolder[tierID] != address(0);
    }

    function nextTokenToSell(uint128 tierID) external view returns (uint256 ldaID) {
    ...
    function buy(uint128 tierID, address buyer, string memory correlationID, bytes memory proof) external {
        // Check that this tier has been registered 
        require(tierRegistered(tierID), "Tier unregistered");
        require(tierPaymentERC20Contract[tierID] != address(0), "Money unregistered");
        require(proof.length != 0, "Signed data empty");
        require(nextTokenToSellForTier[tierID] <= maxSupplyForTier[tierID], "Tier exhausted");
        ...
        require(IERC20(paymentAddress).allowance(buyer, address(this)) >= purchaseAmount, "insufficient allowance remaining for purchase");

        // Move the token 
        IERC20(paymentAddress).transferFrom(buyer, _paymentWallet, purchaseAmount);

        // Move the LDA
        ldaContract.safeTransferFrom(tierLDAHolder[tierID], buyer, ldaID, 1, "");

        // Run final checks: LDA sent to buyer AND money transfered to paymentWallet
        require(ldaContract.balanceOf(buyer, ldaID) == 1, "lda not transferred");
        require(IERC20(paymentAddress).balanceOf(_paymentWallet) == (startingBalance + purchaseAmount), "paid less than expected");

        emit OnChainPurchase(buyer, ldaID, paymentAddress, purchaseAmount, correlationID);
        if(nextTokenToSellForTier[tierID] > maxSupplyForTier[tierID]){
            emit TierSoldOut(tierID, ldaContractAddress, maxSupplyForTier[tierID]);
        }
    }

    function transferForOffchainPurchase(uint128 tierID, address buyer, string memory correlationID) external onlyOwner {
        // Check that this tier has been registered 
        require(tierRegistered(tierID), "Tier unregistered");
        require(nextTokenToSellForTier[tierID] <= maxSupplyForTier[tierID], "Tier exhausted");
        require(!_correlationIDsUsed[correlationID], "correlationID has already been used");
    ...
    function requireValidSignature(uint128 tierID, address buyer, string memory correlationID, bytes memory messageSignature) internal view {
        require(!_correlationIDsUsed[correlationID], "correlationID has already been used");

        // Recreate the msg hash
        bytes32 messageHash = keccak256(abi.encode(tierID, buyer, correlationID)); 
     ...
     */
    function _composeLDA_ID(uint128 tierID, uint128 tokenID) internal pure returns (uint256 ldaID) {
