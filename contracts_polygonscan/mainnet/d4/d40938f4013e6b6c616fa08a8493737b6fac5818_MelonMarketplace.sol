...
contract MelonMarketplace is AccessControlUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function internalMint(MelonLibrary.NFT_MODEL memory nftModel,
        uint256[] memory ids,
        bytes calldata signIds,
        bytes calldata signFee) private  {
        for (uint256 index = 0; index < ids.length; index++) require(!NFTContract.exists(ids[index]), "at least one token already exists");
        ...
        if (nftModel.saleStatus == MelonLibrary.SaleStatuses.FIX_PRICE)
            require(
                nftModel.price.length > 0 && nftModel.symbol.length > 0 && (nftModel.symbol[0] == MelonLibrary.MATIC),
    ...
    function mintToken(
        ...
        internalMint(nftModel, ids, signIds, signFee);
    }
    ...
    function mintWithEnglishAuction(
        ...
        internalMint(nftModel, tokenIDs, signIds, signFee);
    ...
    function setForSale(
        ...
        require(symbol == MelonLibrary.MATIC, "the symbol for the NFT has not been specified!");
    ...
    function setForSaleMultiple(
        ...
        for (uint256 index = 0; index < tokensIDs.length; index++) {
            require(NFTContract.exists(tokensIDs[index]), "URI set of nonexistent token");
    ...
    function markTokenAsSold(uint256 tokenID, address payable newOwner) external {
        require(NFTContract.exists(tokenID), "URI set of nonexistent token");
    ...
    function burn(uint256 tokenID) external adminOrOwner(tokenID) {
        ...
        require(tokens[tokenID].saleStatus == MelonLibrary.SaleStatuses.NOT_ON_SALE, "Token is on sale");
    ...
    function setTokenOnAuction(uint256[] calldata tokenIDs, uint256 status) external {
        require(msg.sender == englishAuctionAddress, "Access denied");
        for (uint256 index = 0; index < tokenIDs.length; index++) {
            require(NFTContract.exists(tokenIDs[index]), "the token does not exist!");
            tokens[tokenIDs[index]].saleStatus = MelonLibrary.SaleStatuses(status);
        }
    }

    function setNFTAddress(address newAddress) external platformOnly {
...
contract MelonNFT is ERC721Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    using ECDSA for bytes32;
    using Strings for uint256;
    using SafeMath for uint256;
    ...
    function approve(address to, uint256 tokenId) public virtual override {
        ...
        require(
            _msgSender() == owner || isApprovedForAll(owner, _msgSender()) || msg.sender == marketplaceAddress || MarketplaceContract.viewAdminRole(msg.sender),
...
contract NFTEnglishAuction is IERC721Receiver, ERC721HolderUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    using SafeMath for uint256;

    IERC20 private melonToken;
    address payable platformAddress;
    address private nftAddress;
    address private melonMarketplaceAddress;
    address payable feesAddress;
    ...
    function initialize(
        ...
        NFTContract = MelonNFT(_NFTAddress);
        melonMarketplaceAddress = _NFTMarketplaceAddress;
    ...
    function createAuction(
        ...
        require(symbol == 0, "MelonEnglishAuction: this token is not MATIC!");

        require(!MarketplaceContract.isTokenOnSale(tokenIDs), "Already on sale");

        for (uint256 index = 0; index < tokenIDs.length; index++) {
            require(NFTContract.ownerOf(tokenIDs[index]) == msg.sender || MarketplaceContract.viewAdminRole(msg.sender) || melonMarketplaceAddress == msg.sender, "Invalid caller");
        }
    ...
    function bidWithMelon(uint256 tokenID, uint256 bidPrice) public nonReentrant {
        Auction storage auction = tokenIDToAuction[tokenID];
    ...
    function finalize(uint256 tokenID) public nonReentrant {
        require(tokenIDToAuction[tokenID].tokenOwner != address(0x0), "Auction doesn't exist");
        Auction storage auction = tokenIDToAuction[tokenID];
        ...
        MelonLibrary.NFT_MODEL memory token = MarketplaceContract.getTokenByID(tokenID);
        if (auction.symbol == 0) {
            feesAddress.transfer(comissionValue);
            if (token.referralAddress.length > 0 && token.isFirstSale) {
                referralFeeTotal = auction.highestBid.mul(token.referralFee).div(100);
                token.referralAddress[0].transfer(referralFeeTotal);
            }
            ...
            if (token.referralAddress.length > 0 && token.isFirstSale) {
                referralFeeTotal = auction.highestBid.mul(token.referralFee).div(100);
                melonToken.transferFrom(address(this), token.referralAddress[0], referralFeeTotal);
            }
    ...
    function withdraw(uint256 tokenID) public nonReentrant {
        ...
        require(auction.tokenOwner == msg.sender || MarketplaceContract.viewAdminRole(msg.sender) || melonMarketplaceAddress == msg.sender, "Invalid caller");
        require(!auction.isFinished, "Auction already finished");
    ...
    function placeOfferMATIC(uint256 tokenID) public payable nonReentrant {
        validateOffer(tokenID, msg.value, msg.sender, 0);
        createOffer(tokenID, msg.value, msg.sender);
    }

    function placeOfferMelon(uint256 tokenID, uint256 value) public nonReentrant {
    ...
    function approveOffer(uint256 tokenID, uint256 offerId) public nonReentrant {
        Auction storage auction = tokenIDToAuction[tokenID];
        ...
        if (auction.symbol == 0) {
            feesAddress.transfer(comissionValue);
            if (token.referralAddress.length > 0 && token.isFirstSale) {
                referralFeeTotal = offer.offerValue.mul(token.referralFee).div(100);
                token.referralAddress[0].transfer(referralFeeTotal);
            }
    ...
    function denyOffer(uint256 tokenID, uint256 offerId) public nonReentrant {
        Auction storage auction = tokenIDToAuction[tokenID];
    ...
    function changeMarketplaceAddress(address newAddress) external platformOnly {
        MarketplaceContract = MelonMarketplace(newAddress);
        melonMarketplaceAddress = newAddress;
    }

    function changeNFTAddress(address newAddress) external platformOnly {
        NFTContract = MelonNFT(newAddress);
