...
contract StratX2_PYQ is StratX2_PCS {
        ...
        returns (uint256)
    {
        ...
        wantLockedTotal = IRewardPool(farmContractAddress).balanceOf(
            address(this)
        );
    ...
    function _farm() internal override {
        ...
        IRewardPool(farmContractAddress).stake(wantAmt);
        wantLockedTotal = IRewardPool(farmContractAddress).balanceOf(
            address(this)
        );
    }
    ...
    function _unfarm(uint256 _wantAmt) internal override {
        if (_wantAmt == 0) {
            IRewardPool(farmContractAddress).claimReward();
        } else {
            IRewardPool(farmContractAddress).withdraw(_wantAmt);
        }
    }
