...
// File: ICosmicMandalas.sol



pragma solidity ^0.8.4;

interface ICosmicMandalas {
...
contract CosmicMandalas is ICosmicMandalas, ERC721, Ownable {
    ...
    modifier onlyWithdrawalWallet() {
        require(address(withdrawalWallet) == _msgSender(), 
            "CosmicMandalas: caller is not the withdrawal wallet");
        _;
    }
    ...
    function buy(uint256 _amount) external override payable {
        require(saleActive, "CosmicMandalas::buy: Sale is not active.");
        require(_amount <= mintLimit, "CosmicMandalas::buy: Too many tokens for one transaction.");
        require(msg.value >= mintPrice.mul(_amount), "CosmicMandalas::buy: Insufficient payment.");

        if (!smartContractAllowance) {
            require(tx.origin == msg.sender, "CosmicMandalas::buy: Smart contracts are not allowed to buy.");
        }
    ...
    function setTicketAddress(address _ticketAddress) external onlyOwner override {
        require(_ticketAddress != address(0), "CosmicMandalas::setTicketAddress: Invalid address.");
        ticketAddress = _ticketAddress;
        emit TicketAddressChanged(_ticketAddress);
    }

    function presaleBuy(uint256 _ticketId, uint _amount) external payable override {
        require(presaleActive, "CosmicMandalas::presaleBuy: Presale is not Active.");
        require(ERC721(ticketAddress).ownerOf(_ticketId) == msg.sender, "CosmicMandalas::presaleBuy: invalid ticket.");
        require(!usedTickets[_ticketId], "CosmicMandalas::presaleBuy: Ticket already used.");
        require(_amount <= mintLimit, "CosmicMandalas::presaleBuy: Too many tokens for one transaction.");
        require(msg.value >= mintPrice.mul(_amount), "CosmicMandalas::presaleBuy: Insufficient payment.");
    ...
    function whitelistBuy(uint256 _amount) external payable override {
        require(whitelistSaleActive, "CosmicMandalas::whitelistBuy: Whitelist Buy is not Active.");
        require(_amount <= mintLimit, "CosmicMandalas::presaleBuy: Too many tokens for one transaction.");
        require(msg.value >= mintPrice.mul(_amount), "CosmicMandalas::presaleBuy: Insufficient payment.");
        require(whitelist[msg.sender], "CosmicMandalas::whitelistBuy: You are not whitelisted."); 
    ...
    function clearTicket(uint256 _ticketId) external override onlyOwner{
        require(usedTickets[_ticketId], "CosmicMandalas::clearTicket: Ticket is not used");
