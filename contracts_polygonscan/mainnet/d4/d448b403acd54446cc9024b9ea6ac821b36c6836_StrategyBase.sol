...
abstract contract StrategyFarmBase is BaseStrategyMasterChef {
    ...
    address public constant pup = 0xcFe2cF35D2bDDE84967e67d00aD74237e234CE59;
        ...
        )
    {
        ...
        usdc_pup_path = new address[](2);
        usdc_pup_path[0] = usdc;
        usdc_pup_path[1] = pup;
    }
    ...
    function harvest() public override {
        ...
        if (_ballBalance > harvestCutoff){
            ...
            _swapUniswapWithPath(usdc_pup_path, _usdcBalancePost.div(2));

            // Add liquidity for Ball/USDC
            uint256 _pup = IERC20(pup).balanceOf(address(this));
            uint256 _usdc = IERC20(usdc).balanceOf(address(this));
            if (_pup > 0 && _usdc > 0) {
                IERC20(pup).safeApprove(currentRouter, 0);
                IERC20(pup).safeApprove(currentRouter, _pup);
                ...
                // Donates DUST
                IERC20(pup).safeTransfer(
                    strategist,
                    IERC20(pup).balanceOf(address(this))
                );
                IERC20(usdc).safeTransfer(
                    strategist,
