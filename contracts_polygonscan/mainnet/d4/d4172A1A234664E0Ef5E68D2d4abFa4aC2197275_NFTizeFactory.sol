...
contract NFTizeFactory is Initializable, IERC721Receiver, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function createPool(
        ...
        uint256 theosAmount = 1_000_000 ether;
        uint256 poolTokenAmount = 15000 ether;
        ...
        emit NftizePoolCreated(address(nftizePoolContract), address(nftizePoolContract.poolToken()), symbol);
        ...
        sushiswapRouter.addLiquidity(
            ...
            address(nftizePoolContract), // to address for LP tokens
            block.timestamp
        );
        ...
        address sushiLp = nftizePoolContract.addLpProvider(msg.sender, theosAmount, poolTokenAmount);
...
contract NftizePool is IERC721Receiver, AccessControl {
    ...
    function depositNFT(
        ...
        if (!whitelist[nftContractAddress][tokenId]) revert NftNotWhitelisted({message : "Nft must be whitelisted"});

        if (poolSlots <= poolNfts) revert PoolSlotsFull({message : "All pool slots are full"});
        ...
        poolToken.transfer(msg.sender, basePrice - feeAmount);
    ...
    function withdrawNFT(address nftContractAddress, uint256 tokenId) public {
        ...
        poolToken.burn(nftPrice - feeAmount);
        poolToken.transfer(feeAddress, feeAmount);

//      add royalties calculation  token.getRaribleV2Royalties(0);
    ...
    function calculateFee(uint256 price) public view returns (uint256) {
        return (price * socialCauseFee) / 10000;
