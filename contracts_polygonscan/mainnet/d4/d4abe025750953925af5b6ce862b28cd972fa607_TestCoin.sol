...
library Address {
    ...
    function isContract(address account) internal view returns (bool) {
        ...
        // and 0x40810fc5b3898f2debdc19ad847fdd9d42b9a1c5168d311c7e65dc1b5e6168f2 is returned
        // for accounts without code, i.e. `keccak256('')`
        bytes32 codehash;
        bytes32 accountHash = 0x40810fc5b3898f2debdc19ad847fdd9d42b9a1c5168d311c7e65dc1b5e6168f2;
...
contract TestCoin is Context, IERC20, Ownable {
    ...
    address public _charityAddress = 0x1cc55d7d0cFb2ba20633311a4Aa76E330ae66195;
    ...
    function _transfer(
        ...
        if (takeFee){
            ...
            uint256 toCharity = calculateCharityFee(amount);
            uint256 toLiquidity = calculateLiquidityFee(amount);

            (,,uint256 rFee,,uint256 tFee,) = _getValues(amount);

            uint256 feeTotal = toBurn.add(toCharity).add(tFee).add(toLiquidity);

            amount = amount.sub(feeTotal);
            
            _tokenTransfer(from,to,amount, false);
            _tokenTransfer(from, _burnAddress, toBurn, false);
            _tokenTransfer(from, _charityAddress, toCharity, false);
            _takeLiquidity(toLiquidity);
            _takeBurn(toBurn);
            _reflectFee(rFee, tFee.add(toBurn).add(toCharity));
        }
        else{
