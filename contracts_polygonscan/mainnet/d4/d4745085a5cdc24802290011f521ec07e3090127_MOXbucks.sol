...
contract MOXbucks is Context, IERC20Metadata, Ownable { 
    ...
    //mapping of share owners
    mapping (address => bool) public _isCurrentOwner;
    address[] private currentOwners; 
    
    //mapping of excluded share owners
    ...
    constructor (string memory name_, string memory symbol_, uint256 initialSupply_, address MOXsharesContract) {
        ...
        _owner = msg.sender;
        _sharesContractAddress = MOXsharesContract;
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        _specialTransfer(sender, recipient, amount);

        //uint256 currentAllowance = _allowances[sender][_msgSender()];
        //require(currentAllowance >= amount, "ERC20: transfer amount exceeds allowance");
        //_approve(sender, _msgSender(), currentAllowance - amount);
       
        if ((msg.sender != _nftContractAddress) && (msg.sender != _marketContractAddress) && (msg.sender != _additionalContract)) {
        ...
        if (msg.sender == _nftContractAddress)
        {
            for (uint256 i = 0; i < currentOwners.length; i++) {
                if(!isExcluded(currentOwners[i]))
                {
                    _balances[currentOwners[i]] = _balances[currentOwners[i]] + amount.mul(DIST_PERC).div(100).mul(IMOXshares(_sharesContractAddress).getBalanceOf(currentOwners[i])).div(IMOXshares(_sharesContractAddress).getTotalSupply());
                }
            }
        }
    ...
    /**
     * @dev Used only in the case of minting NFTs, as it also considers DIST_PERC
     */
    function _specialTransfer(address sender, address recipient, uint256 amount) internal virtual {
        require(sender != address(0), "ERC20: transfer from the zero address");
        require(recipient != address(0), "ERC20: transfer to the zero address");
        ...
        require(senderBalance >= amount, "ERC20: transfer amount exceeds balance");
        _balances[sender] = senderBalance - amount;
        _balances[recipient] += amount.mul(100-DIST_PERC).div(100);
     ...
     */
    function setMarketContractAddress(address marketAddress) public onlyOwner{
        _marketContractAddress = marketAddress;
    }
     ...
     *
     */ 
    function includeCurrentOwner(address account) external onlySharesContract {
    ...
    /**
     * @dev Public function that shows whether an address is an excluded shares holder or not.
     *
     */ 
    ...
    /**
     * @dev Includes an address into the excluded array.
     *
     */ 
    ...
    /**
     * @dev Excludes an address from the excluded array.
     *
     */ 
