...
interface IMigratorToTIGERSwap {
    // Perform LP token migration from legacy UniswapV2 to TIGERSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to UniswapV2 LP tokens.
    // TIGERSwap must mint EXACTLY the same amount of TIGERSwap LP tokens or
    // else something bad will happen. Traditional UniswapV2 does not
    // do that so be careful!
    function migrate(IERC20 token) external returns (IERC20);
}

// TIGERMasterFarmer is the master of TIGER. He can make TIGER and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once TIGER is sufficiently
// distributed and the community can show to govern itself.
//
// Have fun reading it. Hopefully it's bug-free. God bless.
contract TIGERMasterFarmer is Ownable {
    using SafeMath for uint256;
    ...
    // The TIGER TOKEN!
    TigerSwap public tiger;
    // Dev address.
    address public devaddr;
    // TIGER tokens created per block.
    uint256 public REWARD_PER_BLOCK; // 1000000000000000000
    // Bonus muliplier for early TIGER makers.
    ...
    uint256 public constant PERCENT_LOCK_BONUS_REWARD = 40; // lock 40% of bounus reward in 1 year
    uint256 public constant PERCENT_FOR_DEV = 20; // 20% reward for dev

    uint256 public burnPercent = 0; // init 0% burn tiger

    uint256 public lockFromBlock;
    uint256 public lockToBlock;

    // The migrator contract. It has a lot of power. Can only be set through governance (owner).
    IMigratorToTIGERSwap public migrator;
    ...
    event SendTIGERReward(address indexed user, uint256 indexed pid, uint256 amount, uint256 lockAmount);
    event Lock(address indexed to, uint256 value);
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, bool _withUpdate) public onlyOwner {
        require(poolId1[address(_lpToken)] == 0, "TIGERMasterFarmer::add: lp is already in pool");
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function setMigrator(IMigratorToTIGERSwap _migrator) public onlyOwner {
        migrator = _migrator;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 tigerForBurn;
        uint256 tigerForDev;
        uint256 tigerForFarmer;
        (tigerForBurn, tigerForDev, tigerForFarmer) = getPoolReward(pool.lastRewardBlock, block.number, pool.allocPoint);

        if (tigerForBurn > 0) {
            tiger.mint(address(this), tigerForBurn);
            tiger.burn(tigerForBurn);
        }

        if (tigerForDev > 0) {
            // Mint unlocked amount for Dev
            tiger.mint(devaddr, tigerForDev.mul(100 - PERCENT_LOCK_BONUS_REWARD).div(100));
            // Mint locked amount for Dev
            //For more simple, I lock reward for dev if mint reward in bonus time
            tiger.mint(address(this), tigerForDev.mul(PERCENT_LOCK_BONUS_REWARD).div(100));
            farmLock(devaddr, tigerForDev.mul(PERCENT_LOCK_BONUS_REWARD).div(100));
        }
        tiger.mint(address(this), tigerForFarmer);
        pool.accTIGERPerShare = pool.accTIGERPerShare.add(tigerForFarmer.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function getPoolReward(uint256 _from, uint256 _to, uint256 _allocPoint) public view returns (uint256 forBurn, uint256 forDev, uint256 forFarmer) {
        ...
        uint256 tigerCanMint = tiger.cap().sub(tiger.totalSupply());

        if (tigerCanMint < amount) {
            forBurn = 0;
            forDev = 0;
            forFarmer = tigerCanMint;
        }
        else {
    ...
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply > 0) {
            uint256 tigerForFarmer;
            (, , tigerForFarmer) = getPoolReward(pool.lastRewardBlock, block.number, pool.allocPoint);
            accTIGERPerShare = accTIGERPerShare.add(tigerForFarmer.mul(1e12).div(lpSupply));
    ...
    function _harvest(uint256 _pid) internal {
        ...
        if (user.amount > 0) {
            ...
            uint256 masterBal = tiger.balanceOf(address(this));
            ...
            if(pending > 0) {
                tiger.transfer(msg.sender, pending.mul(100 - PERCENT_LOCK_BONUS_REWARD).div(100));
                ...
                emit SendTIGERReward(msg.sender, _pid, pending, lockAmount);
            }
    ...
    // Deposit LP tokens to TIGERMasterFarmer for TIGER allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        require(_amount > 0, "TIGERMasterFarmer::deposit: amount must be greater than 0");
    ...
    function farmLock(address _holder, uint256 _amount) internal {
        require(_holder != address(0), "ERC20: lock to the zero address");
        require(_amount <= tiger.balanceOf(address(this)), "ERC20: lock amount over blance");
        _locks[_holder] = _locks[_holder].add(_amount);
        _totalLock = _totalLock.add(_amount);
        if (_lastUnlockBlock[_holder] < lockFromBlock) {
            _lastUnlockBlock[_holder] = lockFromBlock;
    ...
    function unlock() public {
        ...
        uint256 amount = canUnlockAmount(msg.sender);
        // just for sure
        if (amount > tiger.balanceOf(address(this))) {
            amount = tiger.balanceOf(address(this));
        }
        tiger.transfer(msg.sender, amount);
        _locks[msg.sender] = _locks[msg.sender].sub(amount);
        _lastUnlockBlock[msg.sender] = block.number;
        _totalLock = _totalLock.sub(amount);
    }
    ...
    function setTransferBurnRate(uint256 _tranferBurnRate) public onlyOwner {
        tiger.setTransferBurnRate(_tranferBurnRate);
    }
    ...
    function removeTransferBurnExceptAddress(address _transferBurnExceptAddress) public onlyOwner {
        tiger.removeTransferBurnExceptAddress(_transferBurnExceptAddress);
    }
}