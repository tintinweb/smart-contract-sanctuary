...
contract Delay is Modifier {
    ...
    // Mapping of queue nonce to veto status.
    mapping(uint256 => bool) public txVetoStatus;
    ...
    constructor(
        ...
        bytes memory initParams =
            abi.encode(_owner, _avatar, _target, _vetoCheck, _cooldown, _expiration);
        setUp(initParams);
    }

    modifier onlyVetoer() {
        require(vetoCheck != address(0), "Veto is disabled");
        require(IVetoCheck(vetoCheck).canVeto(msg.sender), "You are not a valid vetoer");
        _;
    }
    ...
    function setUp(bytes memory initParams) public override {
        ...
        emit DelaySetup(msg.sender, _owner, _avatar, _target); // TODO
    }
    ...
    /// @dev Sets the contract that checks if an account has veto power during the cooldown period. If 0 then vetos will be rejected always.
    /// @param addr Address of IVetoCheck contract.
    /// @notice This can only be called by the owner
    function setVetoCheck(address addr) public onlyOwner {
        vetoCheck = addr;
    }
    ...
    function vetoTransaction(uint256 _nonce) public onlyVetoer {
        txVetoStatus[_nonce] = true;
    }
