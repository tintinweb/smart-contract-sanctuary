...
contract Ticket is Initializable {
    ...
    */
    function generateNumberIndexKey(uint16[] memory numbers) public pure returns (uint256[sizeOfIndex] memory result) {
        ...
        for (uint256 val = 0; val < sizeOfIndex; val++) {
            for (uint256 index = 0; index < len; index++) {
                ...
                if ((val & value) != 0) {
                    uint256 num = numbers[index];
                    if (num < 10) {
                        num = num + 45;
                    }
                    result[val] = result[val] * 100  + num;
                }
            }
        }
    }
    ...
    /**
     * calculate the count for winning
     */
    function calculatorWinningTicket(uint256 _lotteryId, uint256[sizeOfIndex] memory keys) external view returns (uint256[sizeOfLottery + 1] memory sumMatch) {
        uint256 len = sizeOfLottery;
        for (uint256 val = 0; val < sizeOfIndex; val++) {
            uint256 cnt = 0;
            for (uint256 index = 0; index < len; index++) {
                uint256 value = 1 << (len - index -1);
            ...
            sumMatch[cnt] = sumMatch[cnt] + ticketHashes[_lotteryId][keys[val]];
        }
    }
    ...
    function countMatch(
        ...
        uint256[sizeOfLottery + 1] memory sumMatch = this.calculatorWinningTicket(_lotteryId, keys);

        // jackpot
        results[0] = sumMatch[6];
        // 5 number match
        results[1] = sumMatch[5] - 6 * results[0];
        // 4 number match
        results[2] = sumMatch[4] - 5 * results[1] - 15 * results[0];
        // 3 number match
        results[3] = sumMatch[3] - 4 * results[2]  - 10 * results[1] - 20 * results[0];
        // 2 number match
        results[4] = sumMatch[2] - 3 *  results[3] - 6 * results[2] - 10 * results[1] - 15 * results[0];
    }
}