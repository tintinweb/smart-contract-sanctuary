...
contract NFTtreasuryXVMC is Ownable, IERC721Receiver {
    ...
    function claimNFT(address token, address receivingFrom, uint256 tokenID) external onlyOwner returns (bool) {
        if(IERC721(token).getApproved(tokenID) == address(this)) {
            IERC721(token).safeTransferFrom(receivingFrom, address(this), tokenID);
            return true;
        } else {
            return false;
        }
