...
// File: contracts/polygon/MaticVesting.sol
...
contract MaticVesting is Ownable {
    using SafeMath for uint256;

    VESToken public vestToken;
    MaticVESTStaking public stakingPool;
    ...
    uint256 public primaryFeePercentage = 100; // 1% * 100
    uint256 public secondaryFeePercentage = 0; // no fee if staking VEST
    ...
    constructor(address _token, address _staking) public {
        vestToken = VESToken(_token);
        stakingPool = MaticVESTStaking(_staking);
    }
    ...
    function lockTokens(
        ...
        uint256 lastStakedTimestamp;
        uint256 lastUnstakedTimestamp;
        (balance, lastStakedTimestamp, lastUnstakedTimestamp) = stakingPool
        .accountInfos(_withdrawalAddress);

        uint256 feeAmount;

        if (balance > minStakeAmount) {
            feeAmount = _lockAmount.mul(secondaryFeePercentage).div(10000);
        } else {
            feeAmount = _lockAmount.mul(primaryFeePercentage).div(10000);
        }
        ...
        returns (uint256)
    {
        ...
        (balance, lastStakedTimestamp, lastUnstakedTimestamp) = stakingPool
        .accountInfos(sender);
        uint256 totalStaked = vestToken.balanceOf(address(stakingPool));
        uint256 totalTokenFee = tokenFees[_tokenAddress].add(
            tokenFeesClaimed[_tokenAddress]
        );
    ...
    function setFeePercentage(uint256 _primary, uint256 _secondary)
        external
        onlyOwner
    {
        primaryFeePercentage = _primary;
        secondaryFeePercentage = _secondary;
    }
    ...
    function setVestToken(address _token) external onlyOwner {
        vestToken = VESToken(_token);
    }

    function setVestStaking(address _staking) external onlyOwner {
        stakingPool = MaticVESTStaking(_staking);
    }
}