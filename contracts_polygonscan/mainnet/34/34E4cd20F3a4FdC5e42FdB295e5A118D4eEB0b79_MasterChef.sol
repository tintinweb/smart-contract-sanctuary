...
contract BEP20 is Context, IBEP20, Ownable {
    ...
    address public constantsOwner;

    string private _name;
    string private _symbol;
    uint8 private _decimals;
    
    uint256 public _maxSupply = 20000 * (10 ** 18);
    ...
    /**
     * @dev renounces the constantsOwner
     * 
     * This sends the ownership of the constantsOwner to the zero address
     *
     * Requirements
     *
     * - `msg.sender` must be constantsOwner
     */
    function renounceconstantsOwner() public {
        require (msg.sender == constantsOwner);
        constantsOwner = address(0);
    }
    ...
    function _transfer(
        ...
        require(amount <= maxTransactionSize || sender == constantsOwner, "BEP20: transfer amount over maxTransactionSize");
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    event pPEGGMintError(bytes reason);
    ...
    function updatePool(uint256 _pid) public {
        ...
        try egg.mint(devaddr, eggReward.div(10)) {} catch (
            bytes memory reason
        ) {
            eggReward = 0;
            emit pPEGGMintError(reason);
        }
        
        try egg.mint(address(this), eggReward) {} catch (
            bytes memory reason
        ) {
            eggReward = 0;
            emit pPEGGMintError(reason);
