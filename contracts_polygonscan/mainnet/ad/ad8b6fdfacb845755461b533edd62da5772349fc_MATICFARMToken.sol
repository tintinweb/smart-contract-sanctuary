...
//-----------------------------------------------------------------------------
contract MATICFARMToken is BEP20("MATIC FARM", "MATIC FARM", 50000000000000000000000) {
...
contract MasterChef is Ownable, NonReentrant {
  ...
  struct PoolInfo {
    ...
    uint256 accMATICFARMPerShare;
    uint16 depositFeeBP;
  }
  //---------------------------------------------------------------------------
  MATICFARMToken public maticFarm;
  ...
  constructor(MATICFARMToken _maticFarm, address _feeAddress, uint256 _startBlock) public {
    maticFarm = _maticFarm;
 ...
 //---------------------------------------------------------------------------
  function add(uint256 _allocPoint, IBEP20 _lpToken, bool _withUpdate) public ownerOnly isUnique(_lpToken) {
    ...
    poolInfo.push(PoolInfo({lpToken: _lpToken, allocPoint: _allocPoint, lastRewardBlock: lastRewardBlock, accMATICFARMPerShare: 0, depositFeeBP: depositFee})
    );
  }
  function pendingMATICFARM(uint256 _pid, address _user) external view returns (uint256)  {
    PoolInfo storage pool = poolInfo[_pid];
    UserInfo storage user = userInfo[_pid][_user];
    uint256 share = pool.accMATICFARMPerShare;
    uint256 pot = pool.lpToken.balanceOf(address(this));
    if (block.number > pool.lastRewardBlock && pot != 0) {
      uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
      uint256 MATICFARMReward = multiplier.mul(EMISSION_RATE).mul(pool.allocPoint).div(totalAllocPoint);
      share = share.add(MATICFARMReward.mul(1e12).div(pot));
    }
  ...
  function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
    ...
    updatePool(_pid);
    if (user.amount > 0) {
      uint256 pending = user.amount.mul(pool.accMATICFARMPerShare).div(1e12).sub(user.rewardDebt);
      if (pending > 0) {
        safeMATICFARMTransfer(msg.sender, pending);
      }
    }
    if (_amount > 0) {
    ...
    user.rewardDebt = user.amount.mul(pool.accMATICFARMPerShare).div(1e12);
  ...
  function updatePool(uint256 _pid) public {
    ...
    uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
    uint256 MATICFARMReward = multiplier.mul(EMISSION_RATE).mul(pool.allocPoint).div(totalAllocPoint);
    maticFarm.mint(feeAddress, MATICFARMReward.div(20));
    maticFarm.mint(address(this), MATICFARMReward);
    pool.accMATICFARMPerShare = pool.accMATICFARMPerShare.add(MATICFARMReward.mul(1e12).div(lpSupply));
    pool.lastRewardBlock = block.number;
  }
  ...
  function safeMATICFARMTransfer(address _to, uint256 _amount) internal {
    uint256 pot = maticFarm.balanceOf(address(this));
    bool success = false;
    if (_amount > pot) {
      success = maticFarm.transfer(_to, pot);
    } else {
      success = maticFarm.transfer(_to, _amount);
    }
    require(success, "error");
  }
