...
contract TinyEggNFT is BNFT, IEggNFT, ReentrancyGuard {
    using Address for address;

    uint256 private _totalBurned;

    address private _tinyEggStats;

    constructor() BNFT(_msgSender(), "https://api.tinyhero.com/static/nft/egg/", "egg.io NFT", "EggHero") {
    }

    function setTinyEggStats(address newTinyEggStats) external onlyAdmin {
        _tinyEggStats = newTinyEggStats;
    }
    ...
    function getInfos(uint256 tokenId) external override view returns (IEggStats.EggInfo memory eggStats) {
        if (_tinyEggStats != address(0)) {
            (eggStats) = IEggStats(_tinyEggStats).stats(tokenId);
        }
    }
