...
abstract contract StrategyFarmTwoAssets is BaseStrategyMasterChef {
        ...
        )
    {
        ...
        eth_dfyn_path = new address[](2);
        eth_dfyn_path[0] = eth;
        eth_dfyn_path[1] = dfyn;
    }
    ...
    function harvest() public override {
        ...
        if (_dinoBalance > harvestCutoff){
            ...
            _swapUniswapWithPathDfyn(eth_dfyn_path, _ethBalancePost.div(2));

            // Add liquidity
            uint256 _eth = IERC20(eth).balanceOf(address(this));
            uint256 _dfyn = IERC20(dfyn).balanceOf(address(this));
            if (_eth > 0 && _dfyn > 0) {
                IERC20(eth).safeApprove(dfynRouter, 0);
                IERC20(eth).safeApprove(dfynRouter, _eth);
                IERC20(dfyn).safeApprove(dfynRouter, 0);
                IERC20(dfyn).safeApprove(dfynRouter, _dfyn);
...
contract StrategyTwoAssets is StrategyFarmTwoAssets {
    ...
    address public DFYN_WETH_LP = 0x6fA867BBFDd025780a8CFE988475220AfF51FB8b;
