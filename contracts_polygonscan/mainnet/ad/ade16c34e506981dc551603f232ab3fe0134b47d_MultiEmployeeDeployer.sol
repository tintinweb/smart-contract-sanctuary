...
// File: contracts/multiEmployee.sol
...
// File: contracts/multiEmployeeDeployer.sol
...
contract MultiEmployeeDeployer is Initializable, AccessControl {
    ...
    function changeBaseDeployer(BaseDeployer _deployer)
        public
        onlyRole(MAIN_OWNER)
    {
    ...
    function getEmployeesData()
        public
        view
        returns (EmployeeLibrary.MultiEmployeesData memory)
    {
        return
            EmployeeLibrary.MultiEmployeesData(
                employeePrice,
                deployerPrice,
                CREATOR_FEE,
    ...
    function setMaxDeployments(uint16 deployments) public onlyRole(MAIN_OWNER) {
    ...
    function mintEmployee() public payable {
        ...
        token.transferFrom(
            msg.sender,
            liquidityAgregator,
            (employeePrice * LIQUIDITY_AGREGATOR_FEE) / 100
        );

        uint8 buildType = getRandomType();
    ...
    function transferValues() public onlyRole(MAIN_OWNER) {
        payable(liquidityAgregator).transfer(
            (address(this).balance * LIQUIDITY_AGREGATOR_FEE) / 100
        );

        payable(creator).transfer((address(this).balance * CREATOR_FEE) / 100);
    }

    function randomize() public onlyRole(MAIN_OWNER) {
        uint256 totalEmployees = multiEmployees.totalSupply();
        uint256[] memory manyRandoms = getManyRandomTypes(totalEmployees);

        for (uint256 i = 0; i < totalEmployees; i++) {
            uint8 build = baseDeployer.getBuildTypes()[manyRandoms[i]];
            multiEmployees.alterEmployeeType(
                multiEmployees.tokenByIndex(i),
                build,
                baseDeployer.calcEmployeePoints([build, build, build, build])
            );
        }
    }

    function getRandomType() private returns (uint8) {
        uint256 random = (
            RandomUtil.randomSeededMinMax(
                0,
                baseDeployer.getBuildTypes().length - 1,
                randoms.getRandomSeed(msg.sender),
                _randomCounter
            )
        );
        ...
        return baseDeployer.getBuildTypes()[random];
    }

    function getManyRandomTypes(uint256 counter)
        private
        returns (uint256[] memory)
    {
        uint256[] memory manyRandoms = (
            RandomUtil.expandedRandomSeededMinMax(
                0,
                baseDeployer.getBuildTypes().length - 1,
                randoms.getRandomSeed(msg.sender),
