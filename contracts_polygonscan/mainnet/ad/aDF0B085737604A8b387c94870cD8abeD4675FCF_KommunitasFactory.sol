...
contract KommunitasProject{
     ...
     */
    function buyTokenByETH(address[] calldata _path) payable isBuyNotEnded isBoosterProgress isNotPaused public {
        ...
        if(boosterProgress == 0){
            leftToken = sale;
        } else if(boosterProgress == 1){
            leftToken = sale.sub(booster[0].achieve);
        }
        
        if(boosterProgress < 2){
            require(getUserAllocation_d8(msg.sender).mul(leftToken).div(10 ** 8) > 0, "You never staked KOM before");
        }
        ...
        if(boosterProgress < 2){
            uint allocationAmount = getUserAllocation_d8(msg.sender).mul(leftToken).div(10 ** 8);
     ...
     */
    function buyTokenByToken(uint _amountIn, address[] calldata _path) isBuyNotEnded isBoosterProgress isNotPaused public {
     ...
     */
    function buyToken(uint _amountIn, address _tokenIn) isBuyNotEnded isBoosterProgress isNotPaused public {
        require(_amountIn > 0, "You buy nothing");
        require(sale.sub(sold) > 0, "You are out of token sale");
