...
contract KittyPartyYieldGeneratorZapper is Initializable, IKittyPartyYieldGenerator, OwnableUpgradeable {
    ...
    address payable public ZapContract;
    ...
    function setAllowance() public onlyOwner{
        ...
        require(IERC20Upgradeable(sellToken).approve(ZapContract, MASK), "Not able to set allowance");
    }
    ...
    {
        address sellToken = kittyPartyYieldInfo[msg.sender].sellTokenAddress;
        address buyToken = kittyPartyYieldInfo[msg.sender].lpTokenAddress;
        require(IERC20Upgradeable(sellToken).approve(ZapContract, MASK), "Not enough allowance");
        // Check this contract's initial balance
        uint256 initialBalance = IERC20Upgradeable(buyToken).balanceOf(address(this));

        (bool success,) = ZapContract.call{value: msg.value}(zapCallData);
        require(success, 'Zap In Failed');

        vaultTokensRec = IERC20Upgradeable(buyToken).balanceOf(address(this)) - initialBalance;
    ...
    returns (uint256 tokensRec)
    {
        ...
        address buyToken = kittyPartyYieldInfo[kittyParty].sellTokenAddress;
        ...
        uint256 finalBalance = buyToken == address(0)
        ? address(this).balance
        : IERC20Upgradeable(buyToken).balanceOf(address(this));
        ...
        IERC20Upgradeable(buyToken).transfer(_treasuryContract , finalBalance);
    }
    ...
    * @dev See IKittyPartyYieldGenerator.sol
    */
    function setZapContractAddress(address payable _zapContract) external onlyOwner {
