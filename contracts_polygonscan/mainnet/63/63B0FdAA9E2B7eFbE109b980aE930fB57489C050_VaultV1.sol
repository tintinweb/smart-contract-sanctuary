...
abstract contract VaultRoles is AccessControl
{
    //========================
    // ATTRIBUTES
    //========================
    ...
    {   
        //init access control        
        _setRoleAdmin(ROLE_VAULTCHEF, ROLE_VAULTCHEF);
...
abstract contract VaultAccessManager is VaultRoles
{
    ...
    constructor(address _vaultChef)
    {   
        //setup roles
        _setupRole(ROLE_VAULTCHEF, address(_vaultChef));
    }
    ...
    function requireVaultChef() internal view
    {
        require(
            hasRole(ROLE_VAULTCHEF, msg.sender),
            "User is not VaultChef");
    }
    ...
    function requireSecurityMod() internal view
    {
        require(
            isAdmin()
                || hasRole(ROLE_SECURITY_ADMIN, msg.sender)
                || hasRole(ROLE_SECURITY_MOD, msg.sender),
...
interface IVaultStrategy
{
    ...
    function compound(address _user) external returns (uint256 rewardAmount, uint256 dustAmount);
...
interface IVault
{
    ...
    function vaultChef() external view returns (IVaultChef);
...
interface IVaultChef
{
    ...
    function teamTreasuryFee() external view returns (uint256);
    function communityTreasuryFee() external view returns (uint256);
    ...
    function compoundVault(uint256 _vid) external;
...
contract VaultV1 is IVault, VaultAccessManager, ReentrancyGuard
{
    //========================
    // TODO:
    // - make users enumerable for refundUsers
    // - refundUsers
    // - compound delay
    //========================
    ...
    IVaultChef public immutable override vaultChef; //vault chef
    IVaultStrategy public strategy; //current strategy    
    ...
    VaultAccessManager(address(_vaultChef))
    {
    ...
    function deposit(address _user, uint256 _amount) public override nonReentrant
    {
        ...
        depositToken().safeTransferFrom(_user, address(this), _amount); //TODO: check if target = strategy wouldnt be smarter to reduce taxes
        farm();
        ...
        //block lock
        require(user.lastDepositOrWithdraw != block.timestamp, "Block lock");
        user.lastDepositOrWithdraw = block.timestamp;

        //event
        emit Deposit(_user, _amount, userDeposit, userBalanceBefore, balanceOf(_user));
    }
    ...
    function withdraw(address _user, uint256 _amount) public override nonReentrant
    {
        ...
        UserInfo storage user = userMap[_user];
        uint256 userBalanceBefore = balanceOf(_user);
        uint256 amountShares = totalShares.mul(_amount).div(balance());
        uint256 withdrawShares = amountShares;
        uint256 withdrawAmount = _amount;
        if (amountShares > user.shares)
        {
            withdrawShares = amountShares;
            withdrawAmount = userBalanceBefore;
        }
        ...
        //withdraw TODO: check again      
        uint256 balanceBefore = balanceOfVault();
        if (balanceBefore < withdrawAmount)
        {
            uint256 poolWithdraw = withdrawAmount.sub(balanceBefore);
            strategy.withdraw(poolWithdraw);
        ...
        depositToken().safeTransfer(_user, withdrawAmount);

        //event
        emit Withdraw(_user, _amount, withdrawAmount, userBalanceBefore, balanceOf(_user));
    }
    ...
    function compound(address _user) public override nonReentrant
    {
        ...
        (uint256 rewardAmount, uint256 dustAmount) = strategy.compound(_user);
    ...
    {
        depositToken().safeTransfer(address(strategy), depositToken().balanceOf(address(this)));
        strategy.deposit();
    }
    ...
    function pause(bool _pauseDeposit, bool _pauseWithdraw, bool _pauseCompound) public
    {
        //check
        requireSecurityMod();
    ...
    function unpause(bool _unpauseDeposit, bool _unpauseWithdraw, bool _unpauseCompound) public
    {
        //check
        requireSecurityAdmin();
