    ...
    function createWithSwap(
        ...
        if (tok.token != tokenIn) {
            if (address(tok.token) == address(1)) {
                amount = swapper.swapFromNative{ value: amount }(tokens[tokenId].token, address(this), swapPath);
            } else {
                tokenIn.approve(address(swapper), amount);
                amount = swapper.swap(tokenIn, amount, tokens[tokenId].token, address(this), swapPath);
            }
        }
        ...
        orders[nextOrderId] = Order(
            ...
            amount - feeAmount,
            feeAmount,
            tokenDecimals(tokens[tokenId].token),
            destination,
            address(tokenIn),
            tokenOut
        );
