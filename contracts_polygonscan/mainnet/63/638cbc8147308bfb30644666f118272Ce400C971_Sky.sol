...
contract Sky is Ownable, ISky {
   ...
   function createCloud(uint256 maticAmount) external payable onlyOwner {
       ...
       makeLink();
       if(IERC20(LINK_TOKEN).balanceOf(address(this)) >= uint256(10 ** 18).div(3)){
           IERC20(LINK_TOKEN).transfer(address(cloud), uint256(10 ** 18).div(3).sub(1) );
       }
       cloud.start();
   }
   ...
   function start() external onlyOwner {
       for(uint256 i = 0; i < cloudsIndex.length; i++){
           ...
           if(IERC20(LINK_TOKEN).balanceOf(address(cloudsIndex[i])) == 0){
               makeLink();
               IERC20(LINK_TOKEN).transfer(address(cloudsIndex[i]), uint256(10**18).div(3).sub(1));
           }
