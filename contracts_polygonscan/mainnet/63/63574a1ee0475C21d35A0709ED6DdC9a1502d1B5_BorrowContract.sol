...
contract BorrowContract is Ownable, ReentrancyGuard, Bigfoot {
  ...
  constructor(address _vaultAddress, address _bankAddress, address _bankcurrency, address _liqstrat, address _addstrat, address _interToken) public {
  ...
  function lpToBalance(uint balance) public view returns (uint){
    ...
    return balance.mul(1e18).div(pps);
  }

  function balanceToLp(uint balance) public view returns (uint){
    uint pps = VaultInterface(vaultAddress).getPricePerFullShare();
    return balance.mul(pps).div(1e18).mul(999).div(1000);
  }
  ...
  function health(uint id) external view returns (uint) {
    ...
    (uint totalInterToken, uint totalBtc) = lpToken.token0() == interToken ? (r0, r1) : (r1, r0);
    // 3. Convert the position's LP tokens to the underlying assets.
    uint userInterToken = lpBalance.mul(totalInterToken).div(lpSupply);
    uint userBtc = lpBalance.mul(totalBtc).div(lpSupply);
    // 4. Convert all farming tokens to BNB and return total BNB.
    uint interTokenHealth = getMktSellAmount(userBtc, totalBtc.sub(userBtc), totalInterToken.sub(userInterToken)).add(
        userInterToken
    );
    return (ICurveSwap(curveswap).get_dy_underlying(tokenindex[interToken], tokenindex[bankcurrency], interTokenHealth));
  }
