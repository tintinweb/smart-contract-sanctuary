...
contract Izlude is Ownable, ReentrancyGuard {
    ...
    function _deposit(uint256 _amount, address _user) private returns (uint256) {
    ...
    function depositFromMorroc(uint256 _amount, address _user) external onlyMorroc nonReentrant returns (uint256) {
    ...
    function _withdraw(uint256 _jellopy, address _user) private returns (uint256) {
    ...
    function withdrawFromMorroc(uint256 _jellopy, address _user) external onlyMorroc nonReentrant returns (uint256) {
...
contract Morroc1_5 {
    ...
    constructor(address _izlude, address _alberta) {
        izlude = Izlude(_izlude);
        alberta = IAlberta(_alberta);

        IERC20 _want = Izlude(_izlude).want();
        want = _want;

        _want.safeApprove(_alberta, type(uint256).max);
        _want.safeApprove(_izlude, type(uint256).max);
    }

    function resetApprove() external {
        want.safeApprove(address(alberta), 0);
        want.safeApprove(address(alberta), type(uint256).max);

        want.safeApprove(address(izlude), 0);
        want.safeApprove(address(izlude), type(uint256).max);
    }

    function harvestKSW() external view {}

    function _deposit(uint256 _amount) private {
        require(_amount > 0, "invalid amount");

        izlude.depositFromMorroc(_amount, msg.sender);
    }
    ...
    function depositEther(uint256 _maxPriceImpact) external payable {
        uint256 beforeBal = want.balanceOf(address(this));
        if (msg.value > 0) {
            alberta.add{value: msg.value}(address(0), address(want), address(this), 0, _maxPriceImpact);
        }
    ...
    function depositToken(
        ...
        alberta.add(_token, address(want), address(this), _amount, _maxPriceImpact);
    ...
    function depositTokenAll(address _token, uint256 _maxPriceImpact) external {
        depositToken(_token, IERC20(_token).balanceOf(msg.sender), _maxPriceImpact);
    }

    // withdraw lp using jellopy from izlude to this morroc
    function _withdraw(uint256 _jellopy) private returns (uint256) {
        require(_jellopy > 0, "invalid jellopy");

        uint256 lp = izlude.withdrawFromMorroc(_jellopy, msg.sender);
        return lp;
    }
    ...
    function withdrawAll() external {
        withdraw(izlude.balanceOf(msg.sender));
    }

    // withdraw lp then convert to ether
    function withdrawEther(uint256 _jellopy, uint256 _maxPriceImpact) public {
        uint256 beforeWantBal = want.balanceOf(address(this));
        _withdraw(_jellopy);
        uint256 afterWantBal = want.balanceOf(address(this));

        uint256 beforeBal = address(this).balance;
        alberta.remove(address(0), address(want), address(this), afterWantBal - beforeWantBal, _maxPriceImpact);
        uint256 afterBal = address(this).balance;
        payable(msg.sender).sendValue(afterBal - beforeBal);
    }

    function withdrawAllEther(uint256 _maxPriceImpact) external {
        withdrawEther(izlude.balanceOf(msg.sender), _maxPriceImpact);
    }
    ...
    function withdrawToken(
        ...
        alberta.remove(_token, address(want), address(this), afterWantBal - beforeWantBal, _maxPriceImpact);
        uint256 afterBal = IERC20(_token).balanceOf(address(this));
        IERC20(_token).safeTransfer(msg.sender, afterBal - beforeBal);
    }

    function withdrawAllToken(address _token, uint256 _maxPriceImpact) external {
        withdrawToken(_token, izlude.balanceOf(msg.sender), _maxPriceImpact);
    }
