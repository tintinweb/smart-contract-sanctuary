...
// File: .deps/github/Loesil/VaultChef/contracts/DeFi Projects/ApeSwap/IApeMiniChefV2_polygon.sol

interface IApeMiniChefV2_polygon
{    
...
// File: .deps/github/Loesil/VaultChef/contracts/DeFi Projects/ApeSwap/VaultApeSwapMiniPolygon.sol

contract VaultApeSwapMiniPolygon is Vault
{
    //-------------------------------------------------------------------------
    // CONSTANTS
    //-------------------------------------------------------------------------

    address public constant REWARD_TOKEN = 0x5d47bAbA0d66083C52009271faF3F50DCc01023C;
    
    address public constant CHEF = 0x54aff400858Dcac39797a81894D9920f16972D1D;
    
    address public constant ROUTER = 0x365725FC5c7f095c82861Bf13E8367A81C4F0C1A;
    ...
    {
        //tokens
        depositToken = IApeMiniChefV2_polygon(CHEF).lpToken(_poolID);
        rewardToken = REWARD_TOKEN;		
		additionalReward = WMATIC;
    ...
    {
        uint256 balance = IERC20(rewardToken).balanceOf(address(this));
        uint256 pending = IApeMiniChefV2_polygon(chef).pendingBanana(poolID, address(this));
    ...
    {
        return IApeMiniChefV2_polygon(chef).userInfo(poolID, address(this));
    }
	
	function getAllocPoints() internal view override returns (uint256)
    {
        (, , uint64 alloc) = IApeMiniChefV2_polygon(chef).poolInfo(poolID);
        return uint256(alloc);
    }
    ...
    function poolDeposit(uint256 _amount) internal override returns(uint256 _received, uint256 _lostRate)
    {
        ...
        IApeMiniChefV2_polygon(chef).deposit(poolID, _amount, address(this));
    ...
    function poolWithdraw(uint256 _amount) internal override returns(uint256 _received, uint256 _lostRate)
    {
        ...
        IApeMiniChefV2_polygon(chef).withdrawAndHarvest(poolID, _amount, address(this));
