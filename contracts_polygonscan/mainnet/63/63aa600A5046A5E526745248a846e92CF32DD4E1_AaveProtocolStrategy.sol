...
contract AaveProtocolStrategy is BaseStrategy {
    ...
    function withdrawAll() external override onlyRole(STRATEGY_ROUTER) {
        ...
        wantRewardCollected += (IERC20(want).balanceOf(address(this)) - wantDeposited - wantRewardCollected);

        want.safeTransfer(_msgSender(), wantDeposited);
        wantDeposited = 0;

        emit Withdrawn(wantDeposited);
    }
    ...
    function rewardsInEth(uint256 _share, uint256 _total) external view override returns (uint256) {
        if (_total == 0) return 0;
        uint256 earnedReward = (wantRewardCollected * _share) / _total;

        address[] memory tokens = new address[](1);
        tokens[0] = aToken;

        earnedReward += (IAaveController(aaveController).getRewardsBalance(tokens, address(this)) * _share) / _total;
        return earnedReward;
    }
