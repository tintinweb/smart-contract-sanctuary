...
// File: contracts/UnityRewardPool.sol
...
contract LPTokenWrapper is Ownable {
	...
	function balanceOfTaxed(address account) external view returns (uint256) {
		return _taxAmounts(account, _balances[account]);
	}
	...
	/// @dev When setting the tax, 1e18 = 100%, 1e17 = 10%, etc.
	function changeTax(uint256 amount) external onlyOwner {
		require(amount <= 1e18, "Tax will be over 100%");
		_tax = amount;
	}

	/// @dev Tax time is set in seconds to correspond with block.timestamps UNIX time.
	function changeTaxTime(uint256 time) external onlyOwner {
		_taxTime = time;
	}
	...
	function withdraw(uint256 amount) public virtual {
		...
		uint256 afterTaxAmount = _taxAmounts(msg.sender, amount);
		_totalSupply = _totalSupply.sub(amount);
		_balances[msg.sender] = _balances[msg.sender].sub(amount);
		_rewardSupply = _rewardSupply.add(amount.sub(afterTaxAmount));
		_stakeToken.transfer(msg.sender, afterTaxAmount);
	}

	/// @dev Calculates the current tax multiplier based on time
	function taxModifier(address account) public view virtual returns (uint256) {
		uint256 startTime = userStartTime[account];
		uint256 endTime = startTime.add(_taxTime);
		return _tax.mul(endTime - block.timestamp).div(_taxTime);
	}

	function _taxAmounts(address account, uint256 amount) internal view returns (uint256) {
		if (block.timestamp >= userStartTime[account].add(_taxTime)) {
			return amount;
		}
		uint256 taxMod = taxModifier(account).add(1e18);
		uint256 amountTax = amount.mul(taxMod).div(1e18);
		return amount.sub(amountTax.sub(amount));
	}
...
contract UnityRewardPool is LPTokenWrapper, IRewardDistributionRecipient, ReentrancyGuard {
	using SafeMath for uint256;
	using SafeERC20 for IERC20;

	/// @dev Reward period, change this if you want a longer or shorter period.
	...
	constructor(IERC20 stakeToken, IERC20 rewardToken, address unityToken) LPTokenWrapper(stakeToken) {
	...
	function taxModifier(address account) public view override returns (uint256) {
		return super.taxModifier(account);
	}
	...
	function withdraw(uint256 amount) public override updateReward(msg.sender) nonReentrant {
		...
		uint256 afterTax = _taxAmounts(msg.sender, amount);
		emit Withdrawn(msg.sender, afterTax);
	}
	...
	/// @dev Custom setters, you can call these functions exteranlly to manually set the reward and LP token.
