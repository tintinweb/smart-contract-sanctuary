...
contract StakeBurner is IStakeBurner, Manageable, ExternallyCallable {
    ...
    /** Don't allow if blacklisted, we use this function for hacked wallets */
    modifier isNotBlackListed(address account) {
        require(
            _blacklist[account] == false,
            'StakeBurner: Account is blacklisted from withdrawing'
        );
        _;
    }
     ...
     */
    function burnStake(uint256 sessionId) external isNotBlackListed(msg.sender) {
    ...
    */
    function getPayoutAndPenaltyInternal(
        ...
        if (stakingDays > daysStaked) {
            ...
            uint256 payOutAmount = (amountAndInterest * (714 - daysAfterStaking)) / 700;
            uint256 lateUnstakePenalty = amountAndInterest - payOutAmount;
