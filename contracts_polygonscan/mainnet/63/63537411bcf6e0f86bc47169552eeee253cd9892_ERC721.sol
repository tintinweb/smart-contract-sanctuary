...
contract ShibaWars is ERC721 {
    ...
    constructor() ERC721("Polygon News", "MATIC-N") {
    ...
    modifier isDev() {
        ...
        require(_isDev, "Not allowed to mint");
        _;
    }

    // MINT NEW TOKEN
    function mint(string memory _metadata) private isDev {
        metadata[tokenId] = _metadata;
        _safeMint(address(this), tokenId);
        ++tokenId;
    }

    function mintForSale(string memory _metadata, uint256 _price)
        private
        isDev
    {
        metadata[tokenId] = _metadata;
    ...
    function claim(uint256 _tokenId) public {
        require(
            ...
            "This token was claimed already."
        );
        require(price[_tokenId] == 0, "This token must be bought.");
        _transfer(address(this), msg.sender, _tokenId);
    }

    function buy(uint256 _tokenId) public payable {
        require(price[_tokenId] > 0, "This token must be bought.");
        require(msg.value == price[_tokenId], "Incorrect amount sent.");
    ...
    function addDev(address newDev) public isDev {
        devs.push(newDev);
    }
}