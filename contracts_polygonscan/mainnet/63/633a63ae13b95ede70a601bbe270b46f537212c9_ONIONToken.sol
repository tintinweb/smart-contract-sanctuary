...
contract ONIONToken is BEP20("ONION Token", "ONION", 30000000000000000000000) {
...
// MasterChef is the master of ONION. He can make ONION and he is a fair guy.
//
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of ONION
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. ONIONs to distribute per block.
    ...
    // Bonus muliplier for early ONION makers.
    ...
    // The block number when ONION mining starts.
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 onionReward = multiplier
            .mul(ONIONPerBlock)
            .mul(pool.allocPoint)
            .div(totalAllocPoint);
            accONIONPerShare = accONIONPerShare.add(
                onionReward.mul(1e12).div(lpSupply)
            );
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        .mul(pool.allocPoint)
        .div(totalAllocPoint);
        onion.mint(devaddr, onionReward.div(20));
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accONIONPerShare).div(1e12).sub(
            user.rewardDebt
        );
        if (pending > 0) {
            safeONIONTransfer(msg.sender, pending);
