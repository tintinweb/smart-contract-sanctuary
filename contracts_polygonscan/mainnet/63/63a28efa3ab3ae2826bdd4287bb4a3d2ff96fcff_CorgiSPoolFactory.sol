...
// File: contracts/farms/corgis/common/IBEP20Metadata.sol
...
// File: contracts/farms/corgis/CorgiSPool.sol
...
contract CorgiSPool is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    /* CorgiS Admin functions to setup pool */

    function corgiSStartIn(uint256 _blocks) public onlyOwner {
        corgiSSetStart(block.number.add(_blocks));
    }

    function corgiSSetStart(uint256 _startBlock) public onlyOwner {
    ...
    function corgiSSetDuration(uint256 _durationBlocks) public onlyOwner {
        bonusEndBlock = startBlock.add(_durationBlocks);
        corgiSCalcRewardPerBlock();
    }

    function corgiSCalcRewardPerBlock() public onlyOwner {
        uint256 rewardBal = rewardToken.balanceOf(address(this));
        rewardPerBlock = rewardBal.div(rewardDuration());
    }

    function corgiSSetRewardMaxTxAmount(uint256 _maxTxAmount) public onlyOwner {
...
// File: contracts/farms/corgis/CorgiSPoolFactory.sol


pragma solidity 0.6.12;

contract CorgiSPoolFactory is Ownable {
    event NewCorgiSPool(address indexed pool);
    ...
    function createPool(
        ...
        bytes memory bytecode = type(CorgiSPool).creationCode;
        ...
        CorgiSPool(pool).initialize(token, reward, rewardPerBlock, startBlock, endBlock, _poolLimitPerUser, _admin);

        emit NewCorgiSPool(address(pool));
    }
}