...
contract BaseDeployer is Initializable, AccessControl {
        ...
        returns (uint8)
    {
        ...
        return
            typeByRandom(
                newRandom(0, probabilitiesTotal(probabilities)),
                probabilities
            );
    }
...
// File: contracts/employeeDeployer.sol
...
contract EmployeeDeployer is Initializable, AccessControl {
    ...
    uint8 public constant NFT_STAKE_FEE = 25;
    uint8 public constant PLAY_TO_EARN_FEE = 65;
    uint8 public constant EMPLOYEE_POOL_FEE = 5;
    ...
    address public employeeStaking;
    ...
    function getEmployeePoints(uint8 employee) public view returns (uint16) {
        return
            baseDeployer.getProbabilitiesTotal() /
            baseDeployer.getTypeProbability(employee);
    }

    function isInWhitelist(address customer) public view returns (bool) {
    ...
    function changeRedirectAddresses(
        address _creator,
        address _employeeStaking,
        address _playToEarnPool,
        address _employeePool
    ) public onlyRole(MAIN_OWNER) {
        employeeStaking = _employeeStaking;
    ...
    function payMint(address customer) private returns (bool) {
        if (whiteList[customer] > 0) whiteList[customer]--;
        else {
            require(creator != address(0), INVALID_CREATOR);
            require(employeeStaking != address(0), INVALID_NFT_STAKING);
            require(playToEarnPool != address(0), INVALID_PLAY_TO_EARN);
            require(employeePool != address(0), INVALID_EMPLOYEE_POOL);

            token.transferFrom(
                customer,
                creator,
                (employeePrice * CREATOR_FEE) / 100
            );

            token.transferFrom(
                customer,
                employeeStaking,
                (employeePrice * NFT_STAKE_FEE) / 100
            );

            token.transferFrom(
                customer,
                playToEarnPool,
                (employeePrice * PLAY_TO_EARN_FEE) / 100
            );

            token.transferFrom(
                customer,
                employeePool,
                (employeePrice * EMPLOYEE_POOL_FEE) / 100
            );
        }
    ...
    function mintEmployee(uint8 employeeType) public {
        ...
        employees.mint(
            ...
            parts[1],
            parts[2],
            calcEmployeePoints([head, parts[0], parts[1], parts[2]]),
            msg.sender
        );
    }
    ...
    function addToWhitelist(address customer, uint8 counter)
        public
        onlyRole(MAIN_OWNER)
    {
        whiteList[customer] += counter;
    }
}