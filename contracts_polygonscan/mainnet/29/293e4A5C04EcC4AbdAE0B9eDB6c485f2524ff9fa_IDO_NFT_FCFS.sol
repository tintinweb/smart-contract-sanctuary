...
contract IDO_NFT_FCFS is Initializable {
   ...
   //people will have a max mint per round. So when tier 3 round starts only tier 3 people can mint and they can mint up to 10
    ...
    function adminWithdraw(address ERC20Asset) external onlySudo {
        require(ownerShareWithdrawn, "Gravity Finance: FORBIDDEN");
        require(
            block.timestamp > (Agreement.saleStart() + Agreement.timelock()),
            "Gravity Finance: TimeLock not met"
        );
        require(
            IERC20(ERC20Asset).transfer(
                Agreement.clientSudoUser(),
                IERC20(ERC20Asset).balanceOf(address(this))
            )
        );
    }
}