...
// File contracts/NFTLotteryv2.sol

pragma solidity ^0.8.0;
contract NFTLottery_v2 is VRFConsumerBase, IERC777Recipient {
    using SafeMath for uint256;
    ...
    */
    ...
    function buyTicketsAndRedeem(uint256 amount, bytes calldata gameID) external {
        uint256 totalARTIS = ticketFee.mul(amount);
        require(artisToken.balanceOf(msg.sender) > totalARTIS, "Not enough ARTIS.");
        for(uint256 i = 0; i < amount; i++){
            artisToken.operatorSend(msg.sender, address(this), ticketFee, "", "");
    ...
    function createNewLotteryGame(
        ...
        lotteryGames[lotteryGameID].MAX_WINNING_NUMBER = 0;
        lotteryGames[lotteryGameID].winningNumber = lotteryGames[lotteryGameID].MAX_WINNING_NUMBER + 1;
        ...
        lotteryExists(lotteryGameID)
    {
        ...
        lotteryGames[lotteryGameID].winningNumber = lotteryGames[lotteryGameID].MAX_WINNING_NUMBER.add(1);
        lotteryGames[lotteryGameID].newEntryAllowed = true;
        ...
        override
    {
        ...
        lotteryGames[lotteryGameID].winningNumber = (randomness %
            lotteryGames[lotteryGameID].MAX_WINNING_NUMBER - 1);
    ...
    function redeemTicket(bytes calldata lotteryGameID) public {
        redeemTicket(lotteryGameID, msg.sender);
    }

    function redeemTicket(bytes calldata lotteryGameID, address user)
        private
        lotteryExists(lotteryGameID)
    {
        ...
        uint256 number = lotteryGames[lotteryGameID].MAX_WINNING_NUMBER;
        lotteryGames[lotteryGameID].MAX_WINNING_NUMBER = number.add(1);
        ticketBalances[user] = ticketBalances[user].sub(1);
        lotteryGames[lotteryGameID].winningNumber = lotteryGames[lotteryGameID].MAX_WINNING_NUMBER.add(1);
        lotteryGames[lotteryGameID].ticketEntries[number] = user;
        emit TicketRedeemed(user, lotteryGameID, number);
    }

    function redeemTicketBatch(bytes calldata gameID, uint256 amount) external {
        for(uint256 i = 0; i < amount; i++){
            redeemTicket(gameID);
        }
    }
    ...
    */
    ...
    function awardAndRedeemTickets(address user, uint256 amount, bytes calldata lotteryGameID) external onlyAdmin {
        ticketBalances[user] = ticketBalances[user].add(amount);
        for(uint256 i = 0; i < amount; i++){
            redeemTicket(lotteryGameID, user);
        }
    }

    function awardTicketsBatch(address[] calldata users, uint256[] calldata amount) external onlyAdmin{
        require(users.length == amount.length, "Arrays must be of equal length.");
    ...
    function setNewMinter(address newAddress) external onlyAdmin {
        nftMinter = ArtisLifeNFT(newAddress);
    }
