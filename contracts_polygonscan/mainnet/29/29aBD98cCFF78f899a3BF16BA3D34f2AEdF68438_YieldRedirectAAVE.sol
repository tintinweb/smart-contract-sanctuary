...
// File: contracts/YieldRedirectAAVE.sol
contract YieldRedirectAAVE is BaseStrategy {
    ...
    // For Swapping
    IQuickSwapRouter public ROUTER = IQuickSwapRouter(0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff);
    IQuickSwapRouter public constant QS_ROUTER = IQuickSwapRouter(0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff);

    // sushi swap 0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506
    IQuickSwapRouter public constant SUSHI_ROUTER = IQuickSwapRouter(0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506);
    ...
    function _getRealAPR(uint256 _profit) public view returns (uint256) {
      ...
      uint256 realApr = _profit.mul(blocksPerYear).div(diffTime).div(initialDeposit);
      return realApr;
    }

    function getTargetAndDiff(uint256 _profit) public view  returns (uint256 _diffToSell, uint256 _targetProfit) {
      uint256 diffTime = block.number.sub(lastSnapshotBlock);
      uint256 initialDeposit = vault.strategies(address(this)).totalDebt;

      _targetProfit = targetAPR.mul(diffTime).div(blocksPerYear).mul(initialDeposit);
      _diffToSell = _profit.sub(_targetProfit);
    }

    function _getValueOverTarget(uint256 _profit, uint256 _loss) public view  returns (uint256 _diffToSell, uint256 _targetProfit) {
    ...
    )
    {
        ...
        if (_debtOutstanding > 0) {
            ...
            // new yield redirect
            (uint256 toSell, uint256 earned2) = _getValueOverTarget(earned, lost);
            lastSnapshotBlock = block.number;
