    ...
    ReentrancyGuard
{
    ...
    uint16[] public beneficiaryRewardFactors;
    ...
    event DistributeReserve(IERC20 token, uint256 amount, address to);
    event SetBeneficiaries(
        address[] beneficiaryAddresses,
        uint16[] beneficiaryRewardFactors
    );
    event SwapToReserves(Swap[] swaps, uint256[] _amounts);
    event BeneficiaryRemoved(address beneficiaryAddress);
    event ReserveTokenAdded(IERC20 token);
    event ReserveTokenRemoved(IERC20 token);
     ...
     */
    function _beforeCall(bytes32 _id, bytes32 predecessor) private view {
        require(
            isOperationReady(_id),
    ...
    function convertTokensToReserve(
        Swap[] memory _swaps,
        uint256[] memory _amounts
    ) public {
        require(_swaps.length == _amounts.length, "swap and amount have different length");
        for (uint8 i = 0; i < _swaps.length; i++) {
            Swap memory sw = _swaps[i];
            uint256 amount = _amounts[i];
            _convertTokenToReserve(
                sw.router,
                amount,
                sw.path,
                sw.slippage
        ...
        emit SwapToReserves(_swaps, _amounts);
    }

    function _convertTokenToReserve(
        address _router,
        uint256 _amount,
        address[] memory _path,
        uint256 _slippage
    ) internal {
        address tokenStart = _path[0];
        address tokenEnd = _path[_path.length.sub(1)];
        require(tokenEnd != tokenStart, "startToken must not be the same as tokenEnd");

        bool isValidReserve = false;
        for (uint8 i = 0; i < reserveTokens.length; i++) {
            if (reserveTokens[i] == IERC20(tokenEnd)) {
                isValidReserve = true;
                break;
            }
        }
        require(isValidReserve, "only can convert to reserve token");

        uint256 balance = IERC20(tokenStart).balanceOf(address(this));
        require(_amount <= balance, 'amount exceed balance');
        require(_amount > 0, 'amount must be more than 0');

        IERC20(tokenStart).safeApprove(_router, 0);
        IERC20(tokenStart).safeIncreaseAllowance(_router, _amount);
        _safeSwap(
            _router,
            _amount,
            _slippage,
    ...
    function convertTokensToWBNB(
        Swap[] memory _swaps
    ) external onlyRole(EXECUTOR_ROLE) {
        for(uint8 i = 0; i < _swaps.length; i ++) {
            Swap memory s = _swaps[i];
            _convertTokenToWBNB(s.router, s.path, s.slippage);
        }
    }

    function _convertTokenToWBNB(
        address _router,
        address[] memory _path,
        uint256 _slippage // slippage allowance | 990 = 1% | 950 = 5%
    ) internal {
        ...
        if (tokenStartAmt > 0) {
            IERC20(tokenStartAddress).safeApprove(_router, 0);
    ...
    function distributeReservesTo(
        ...
        for (uint8 i = 0; i < beneficiaryAddresses.length; i++) {
            if (_beneficiary == beneficiaryAddresses[i]) {
                isBeneficiary = true;
                break;
            }
        }
        require(isBeneficiary, "not beneficiary address");

        bool isReserve = false;
        for (uint8 i = 0; i < reserveTokens.length; i++) {
            if (_reserve == reserveTokens[i]) {
                isReserve = true;
                break;
            }
        }
        require(isReserve, "not reserve token");

        uint256 balance = IERC20(_reserve).balanceOf(address(this));
        require(_amount <= balance, "invalid amount");

        IERC20(_reserve).transfer(_beneficiary, _amount);
        emit DistributeReserve(_reserve, _amount, _beneficiary);
    }
        ...
        public
    {
        ...
        uint256 rewardsAmt = IERC20(wbnbAddress).balanceOf(address(this));
        ...
        emit DistributeRewards(rewardsAmt);
    }
    ...
    function removeBeneficiary(
        ...
        require(isValidBeneficiary, "invalid beneficiary");

        address lastBeneficiary = beneficiaryAddresses[beneficiaryCount.sub(1)];
        uint16 lastRewardFactor = beneficiaryRewardFactors[beneficiaryCount.sub(1)];

        beneficiaryAddresses[beneficiaryIndex] = lastBeneficiary;
        beneficiaryRewardFactors[beneficiaryIndex] = lastRewardFactor;

        beneficiaryAddresses.pop();
        beneficiaryRewardFactors.pop();
    ...
    function addReserveToken(
        ...
        for (uint8 i; i < reserveTokens.length; i++) {
            if (reserveTokens[i] == _reserve) {
                isDuplicate = true;
                break;
            }
        }
        require(!isDuplicate, "duplicate reserve token is not allowed");

        reserveTokens.push(_reserve);
        emit ReserveTokenAdded(_reserve);
    }

    function removeReserveToken(
        IERC20 _reserve
    ) public {
        require(
            msg.sender == address(this),
        ...
        bool isValidReserveToken = false;
        uint8 reserveIndex;
        uint256 reserveCount = reserveTokens.length;
        for (uint8 i; i < reserveCount; i++) {
            if (reserveTokens[i] == _reserve) {
                reserveIndex = i;
                isValidReserveToken = true;
                break;
            }
        }
        require(isValidReserveToken, "invalid reserve token");

        IERC20 lastReserve = reserveTokens[reserveCount.sub(1)];

        reserveTokens[reserveIndex] = lastReserve;
        reserveTokens.pop();

        emit ReserveTokenRemoved(_reserve);
    }

    function setBeneficiaries(
        address[] memory _beneficiaryAddresses,
        uint16[] memory _beneficiaryRewardFactors
    ) public virtual {
        require(
            msg.sender == address(this),
            "TimelockController: caller must be timelock"
        ...
        require(
            ...
            "TimelockController: rewardFactorsTotal must be max 10000"
        );

        beneficiaryAddresses = _beneficiaryAddresses;
        beneficiaryRewardFactors = _beneficiaryRewardFactors;
        rewardFactorsTotal = rewardFactorsTotal;
