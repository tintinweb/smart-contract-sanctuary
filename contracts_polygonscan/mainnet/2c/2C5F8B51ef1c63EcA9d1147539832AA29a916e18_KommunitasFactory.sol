...
contract KommunitasProject{
     ...
     * @param _fee Fee charged to admin project (in percent)
    ...
    function getTotalPurchase(address _user) public view returns(uint total){
        total = purchasePerRound[_user][0].add(purchasePerRound[_user][1]).add(purchasePerRound[_user][2]);
    }
    
    // **** MAIN AREA ****
    
    /**
     * @dev Update project booster
     */
    function updateProject() public{
        if(block.timestamp > booster[2].end){
            ...
            booster[2].achieve = sold.sub(booster[0].achieve.add(booster[1].achieve));
            ...
            for(uint i=0; i<3; i++){
                if(block.timestamp >= booster[i].start && block.timestamp <= booster[i].end){
                    ...
                    if(i == 1){
                        if(booster[0].achieve == 0){
                            booster[0].achieve = sold;
                        }
                    } else if(i == 2){
                        if(booster[1].achieve == 0){
                            booster[1].achieve = sold.sub(booster[0].achieve);
                        }
                    }
    ...
    function buyTokenByETH(address[] calldata _path) payable isBuyNotEnded isBoosterProgress public {
        ...
        if(boosterProgress < 2){
            require(getUserAllocation_d8(msg.sender).mul(sale.sub(sold)).div(10 ** 8) > 0, "You never staked KOM before");
        }
        ...
        if(boosterProgress < 2){
            uint allocationAmount = getUserAllocation_d8(msg.sender).mul(sale.sub(sold)).div(10 ** 8);
            require(purchasePerRound[msg.sender][boosterProgress].add(tokenReceived) <= allocationAmount, "You buy more than your KOM staked allocation");
        }else{
            require(!publicBought[msg.sender], "You are allowed to buy once in public round");
        ...
        invoices[msg.sender].push(Invoice(buyerId, boosterProgress, block.timestamp, buyAmount, tokenReceived));
        
        revenue += buyAmount;
        purchasePerRound[msg.sender][boosterProgress] += tokenReceived;
        sold += tokenReceived;
    ...
    function buyToken(uint _amountIn, address _tokenIn) isBuyNotEnded isBoosterProgress public {
        ...
        invoices[msg.sender].push(Invoice(buyerId, boosterProgress, block.timestamp, _amountIn, tokenReceived));
...
contract KommunitasFactory is IKommunitasFactory{
    ...
    /**
     * @dev Update all projects booster
     */
