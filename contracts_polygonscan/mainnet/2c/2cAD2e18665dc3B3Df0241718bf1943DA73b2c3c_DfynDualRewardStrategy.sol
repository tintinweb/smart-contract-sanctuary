...
//For Dfyn/Other token pairs with dual rewards
//otherToken must be the 2nd reward token
contract DfynDualRewardStrategy is BaseStrategyStakingRewards {

    uint256 public constant keepMax = 10000;

    address public otherToken;
    address private constant DFYN = 0xC168E40227E4ebD8C1caE80F7a55a4F0e6D66C97;
    address private constant WMATIC = 0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270;
    address private constant DFYN_WMATIC = 0x4c28f48448720e9000907BC2611F73022fdcE1fA;

    address private constant DFYN_ROUTER = 0xA102072A4C07F06EC3B4900FDC4C7B80b6c57429;
    address private constant FUND = 0x01fE07ce760DA7a025e4Cf3f950aE236F8e62120;

    // Uniswap swap paths
    address[] public reward_other_path; //DFYN -> otherToken
    address[] public other_reward_path; //otherToken -> DFYN

    address[] public reward_matic_path;
    address[] public reward2_matic_path;
        ...
        )
    {
        ...
        reward_other_path = [DFYN, otherToken];
        other_reward_path = [otherToken, DFYN];

        reward_matic_path = [DFYN, DFYN_WMATIC];
        reward2_matic_path = [otherToken, DFYN, DFYN_WMATIC];

        //Given: want isn't _harvestedToken or _otherToken, because want is a LP token
        IERC20(DFYN).safeApprove(DFYN_ROUTER, uint256(-1));
        IERC20(_otherToken).safeApprove(DFYN_ROUTER, uint256(-1));
    }

    //Treat DFYN_WMATIC as WMATIC for the purpose of calculating profit
    ...
    //Swap feeAmount to DFYN_WMATIC before sending it to the fee dist, since profit is calculated in terms of WMATIC
    function swapRewardToWmaticAndDistributeFee(uint256 feeAmount, address[] memory path) internal {
        if(feeAmount > 0) {
            _swapUniswapWithPathPreapproved(path, feeAmount);
            uint256 _maticFee = IERC20(DFYN_WMATIC).balanceOf(address(this));
            _notifyJar(_maticFee);
            IERC20(DFYN_WMATIC).safeTransfer(strategist, _maticFee);
        }
    }

    function harvest() public override onlyHumanOrWhitelisted nonReentrant {
        //Transfer WMATIC/DFYN/otherToken that may already be in the contract to the fee dist fund
        uint256 matic_bal = IERC20(DFYN_WMATIC).balanceOf(address(this));
        if(matic_bal > 0) {
            IERC20(DFYN_WMATIC).safeTransfer(strategist, matic_bal);
        }
        uint256 h_bal = IERC20(DFYN).balanceOf(address(this));
        if(h_bal > 0) {
            IERC20(DFYN).safeTransfer(strategist, h_bal);
        }
        uint256 h_bal2 = IERC20(otherToken).balanceOf(address(this));
        if(h_bal2 > 0) {
            IERC20(otherToken).safeTransfer(strategist, h_bal2);
        }

        _getReward();

        //Deduct fee from DFYN + otherToken harvested
        ...
        if (amountHarvested > 0) {
            ...
            swapRewardToWmaticAndDistributeFee(feeAmount, reward_matic_path);
        }
        uint256 otherHarvested = IERC20(otherToken).balanceOf(address(this));
        if (otherHarvested > 0) {
            uint256 feeAmount = otherHarvested.mul(IERCFund(strategist).getFee()).div(keepMax);
            swapRewardToWmaticAndDistributeFee(feeAmount, reward2_matic_path);
            //Swap remaining otherToken for harvestedToken
            _swapUniswapWithPathPreapproved(other_reward_path, otherHarvested.sub(feeAmount));
        }
    ...
    address private constant COACH = 0xB2CAaB51633B88Ac8f7794Cc9C578448FFBd0c6F;
    address private constant MYCA = 0xa1BbD8D39eD536DEa030A32F3F6C5916C845A800;
    address private constant PEDRO = 0xe33Bfd1FcC9Ba62Aa26FDc9B00463061c0181EE3;
    address private constant REV = 0xF2Da8b05f3A9d20e3331326Ee48D19A21842dfeB;

    address public candidate;
    bool public validated;

    function nominate(address newStakingContract) external onlyOwner nonReentrant {
        validated = false; //set validated to false in order to prevent changing candidate after validation
        candidate = newStakingContract;
    }

    //Require validation of the staking contract from another Adamant staff member
    function validate() external nonReentrant {
        require(msg.sender == COACH || msg.sender == MYCA || msg.sender == PEDRO || msg.sender == REV, "not signer");
        validated = true;
    }

    function migrate() external onlyOwner nonReentrant {
        require(validated, "not validated yet");
        ...
        if(balanceOfPool() > 0) {
            ...
            //Deposit (ensures candidate is a valid staking contract)
            deposit();
        }
        else {
            rewards = candidate;
