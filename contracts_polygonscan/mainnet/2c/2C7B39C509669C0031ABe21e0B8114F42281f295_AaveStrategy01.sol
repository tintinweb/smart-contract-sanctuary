...
contract AaveStrategy01 is Strategy {
    ...
    address[] public assets = [0x5c2ed810328349100A66B82b78a1791B101C9D61, 0xF664F50631A6f0D72ecdaa0e49b0c019Fa72a8dC];
    function aave_claim_matic() public onlyOwner(){
    ...
    function _aave_deposit(string memory Coin, uint256 tokenAmt) public onlyOwner(){
        ...
        if (SToken.allowance(address(this), AAVE_LENDING_POOL) < tokenAmt) {
            require(
                SToken.approve(AAVE_LENDING_POOL, type(uint256).max), "approve error");
        }
        // approve
    ...
    // test 700 => 65% (455) 
    function test_aave_loop1(string memory Coin, uint256 tokenAmt) public onlyOwner() {
        // 65% => max 70% for BTC
        _aave_deposit(Coin, tokenAmt);
        
        uint256 percent = 6500;
        uint256 B1 = tokenAmt.mul(percent).div(10000);
