...
contract VaultGetter {
    ...
    function sync(address vaultHealerAddress) external {
        ...
        for (uint i = vaultFacts[vaultHealerAddress].length; i < len; i++) {
    ...
    function getVault(address vaultHealerAddress, address priceGetterAddress, address user, uint pid) public view returns (VaultData memory v) {
        ...
        try IMasterchef(v.facts.masterchefAddress).totalAllocPoint() returns (uint points) {
            ...
            //Figure out allocPoints only if we have a total

            //This can be either a minichef or masterhealer

            (bool success, bytes memory data) = v.facts.masterchefAddress.staticcall(abi.encodeWithSignature("poolInfo(uint256)",v.facts.chefPid));
            ...
            if (success) {
                ...
                if (data.length == 0x60) {// only 3 returndata items ie minichef

                    (,,allocPoint) = abi.decode(data,(uint128,uint64,uint64));

                } else {

                    (,allocPoint,) = abi.decode(data,(address,uint256,bytes));
    ...
    function getVaults(address vaultHealerAddress, address priceGetterAddress, address user, uint start, uint end) public view returns (VaultData[] memory vaults) {
        ...
        uint len = VaultHealer(vaultHealerAddress).poolLength();

        if (end == 0) end = len;

        require(start < end, "invalid range");

        require(end <= len, "end pid out of bounds");
