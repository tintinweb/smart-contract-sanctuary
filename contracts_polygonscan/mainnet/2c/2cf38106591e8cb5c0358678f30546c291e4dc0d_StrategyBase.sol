...
* via msg.sender and msg.data, they should not be accessed in such a direct
    ...
    abstract contract BaseStrategy is Ownable {
        ...
        // Dex
        address public constant currentRouter = 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff; //QS router
    ...
    interface IMasterChef {
        function deposit(uint256 _pid, uint256 _amount, address _strategist) external;
    ...
    abstract contract BaseStrategyMasterChef is BaseStrategy {
        ...
        function getHarvestable() external override view returns (uint256) {
            return IMasterChef(rewards).pendingIris(poolId, address(this));
        }
        ...
        function _getReward() internal {
            ...
            //IMasterChef(rewards).harvest(poolId);
        }
    ...
    abstract contract StrategyFarmBase is BaseStrategyMasterChef {
        ...
        address public constant Iris = 0xdaB35042e63E93Cc8556c9bAE482E5415B5Ac4B1;
        address public constant wmatic = 0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270;
        address public constant usdc = 0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174;
        address public multiHarvest = 0x0000000000000000000000000000000000000000;
    
        // Other constants
        uint16 public constant underlyingPoolId = 8;
    
        uint256 public keepFXS = 75;
        uint256 public keepFXSmax= 10000;
        uint256 public harvestCutoff = 10**16;
    
    
        
    
        // Uniswap swap paths
        address[] public Iris_usdc_path;
        address[] public Iris_wmatic_path;
        ...
        {
            Iris_usdc_path = new address[](2);
            Iris_usdc_path[0] = Iris;
            Iris_usdc_path[1] = usdc;
    
            Iris_wmatic_path = new address[](2);
            Iris_wmatic_path[0] = Iris;
            Iris_wmatic_path[1] = wmatic;
        ...
        function set_fee(uint256 NewFee) external onlyOwner() {
            require(NewFee <= 100, "New fee must be less than 100 bps");
            keepFXS = NewFee;
        }
        ...
        function set_harvestCutoff(uint256 newCutoff) external onlyOwner() {
            require(newCutoff <= 10**18, "New cutoff must be less than 1 shield");
        ...
        function harvest() public override {
            ...
            require(msg.sender == tx.origin || msg.sender == owner() || msg.sender == strategist || msg.sender == multiHarvest, "not authorized");
    
            // Collects tokens
            _getReward();
    
            //Swap 
            uint256 _IrisBalance = IERC20(Iris).balanceOf(address(this));
            if (_IrisBalance > harvestCutoff){    
    
    
                
                //Send performance fee to strategist
                uint256 performanceFee = _IrisBalance.mul(keepFXS).div(keepFXSmax);
                if (performanceFee>0) {
                    _swapUniswapWithPath(Iris_usdc_path, performanceFee);    
                ...
                // Swap rest
                uint256 _IrisBalancePost = IERC20(Iris).balanceOf(address(this));
                _swapUniswapWithPath(Iris_wmatic_path, _IrisBalancePost.div(2));
    
                // Add liquidity
                uint256 _Iris = IERC20(Iris).balanceOf(address(this));
                uint256 _wmatic = IERC20(wmatic).balanceOf(address(this));
                if (_Iris > 0 && _wmatic > 0) {
                    IERC20(Iris).safeApprove(currentRouter, 0);
                    IERC20(Iris).safeApprove(currentRouter, _Iris);
                    IERC20(wmatic).safeApprove(currentRouter, 0);
                    IERC20(wmatic).safeApprove(currentRouter, _wmatic);
                    ...
                    // Donates DUST
                    IERC20(Iris).safeTransfer(
                        strategist,
                        IERC20(Iris).balanceOf(address(this))
                    );
                    IERC20(wmatic).safeTransfer(
                        strategist,
                        IERC20(wmatic).balanceOf(address(this))
                    );
                }
            }
            // Stake the LP token
    ...
    contract StrategyBase is StrategyFarmBase {
        // Token addresses
        address public Iris_MASTER_CHEF = 0x4aA8DeF481d19564596754CD2108086Cf0bDc71B;
        address public Iris_WMATIC_LP = 0x86ad6271809f350522085F95F5A67d46ff7ed3AB;
