...
contract BeefyLaunchpoolFactory is Ownable {
    ...
    function createReceiver(address token) external returns (address launchpool) {
        bytes memory bytecode = type(BeefyLaunchpoolReceiver).creationCode;
        assembly {
            launchpool := create(0, add(bytecode, 32), mload(bytecode))
        }
        BeefyLaunchpoolReceiver(launchpool).initialize(multisig, dev, token, fee);
        emit LaunchpoolCreated(launchpool);
    }
