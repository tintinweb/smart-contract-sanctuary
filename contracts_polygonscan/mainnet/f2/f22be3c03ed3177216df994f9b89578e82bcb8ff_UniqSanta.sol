...
contract UniqSanta is ERC721ClaimableV2 {
    ...
    function contractURI() public pure returns (string memory) {
        return "https://uniqly.io/api/nft-collections/santa";
    }
    ...
    // ----- OWNERS METHODS ----- //
    function mintGiftToken(address _receiver, uint256 _tokenTypeNum) external onlyOwner{
        _safeMint(_receiver, _tokenNum);
        _tokenType[_tokenNum] = _tokenTypeNum;
        _tokenNum++;
    }

     function batchMintGifts(
        address[] memory _addresses,
        uint256[] memory _tokenTypeNums
    ) external onlyOwner {
        uint len = _tokenTypeNums.length;
        require(len == _addresses.length, "Arrays length");
        uint256 i = 0;
        for (i = 0; i < len; i++) {
            _safeMint(_addresses[i], _tokenNum);
            _tokenType[_tokenNum] = _tokenTypeNums[i];
            _tokenNum++;
        }
    }
