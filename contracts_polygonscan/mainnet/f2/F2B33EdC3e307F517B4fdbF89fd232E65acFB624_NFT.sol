    ...
    ERC721Enumerable
{
    ...
    function batchMint(address[] memory receivers, string[] memory _tokenURIs) public returns (uint256[] memory){
        require(
            hasRole(MINTER_ROLE, _msgSender()),
            "Must have minter role to mint"
        );
        require(receivers.length == _tokenURIs.length, "receivers and ids length mismatch");
        
        uint256[] memory _ids = new uint256[](receivers.length);

        for (uint256 i = 0; i < receivers.length; i++) {
            _tokenIdTracker.increment();
            uint256 newId = _tokenIdTracker.current();
            _safeMint(receivers[i], newId);
