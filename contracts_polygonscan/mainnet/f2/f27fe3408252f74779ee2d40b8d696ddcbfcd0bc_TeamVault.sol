...
contract TeamVault is Initializable, Manageable {
    using SafeERC20Upgradeable for IERC20Upgradeable;
    using EnumerableSetUpgradeable for EnumerableSetUpgradeable.AddressSet;

    EnumerableSetUpgradeable.AddressSet internal wallets;

    function addUser(address _wallet) external onlyManager {
        require(!wallets.contains(_wallet), 'TEAM: Member already added');

        wallets.add(_wallet);
    }

    function removeUser(address _wallet) external onlyManager {
    ...
    function withdrawMatic(address to, uint256 amount) external onlyManager {
