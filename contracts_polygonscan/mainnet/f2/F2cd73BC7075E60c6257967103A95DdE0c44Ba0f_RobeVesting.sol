...
contract RobeVesting is Ownable, ReentrancyGuard {
    ...
    uint256 public constant startDateVesting = 1640771156;
    ...
    function setVestingScheduleEndDate(address _beneficiary) public nonReentrant {
        bytes32 vestingScheduleIdExist = this.computeCurrentVestingScheduleIdForHolder(_beneficiary);
        VestingSchedule storage vestingSchedule = vestingSchedules[vestingScheduleIdExist];
        vestingSchedule.end = getCurrentTime();
        uint256 nftId = vestingSchedule.nftId;

        uint256 amount = computeVestableAmount(vestingScheduleIdExist);
        amount = amount.add(vestingSchedule.released);

        _nftVesting[nftId].availableForVesting = _nftVesting[nftId].availableForVesting.sub(amount);
    }
    ...
    function createVestingSchedule(
        ...
        if (!_nftVesting[_tokenId].initialized) {
            _nftVesting[_tokenId].initialized = true;
        ...
        if (_from != _beneficiary && getVestingSchedulesCountByBeneficiary(_from) != 0) {
            ...
            if (vestingSchedule.nftId == _tokenId && vestingSchedule.beneficiary != _beneficiary) {
                vestingSchedule.end = getCurrentTime();
                uint256 amount = computeVestableAmount(vestingScheduleIdExist);
                amount = amount.add(vestingSchedule.released);
                _nftVesting[_tokenId].availableForVesting = _nftVesting[_tokenId].availableForVesting.sub(amount);
            } else {
        ...
        require(
            this.getWithdrawableAmount() >= totalAmount,
        ...
        onlyIfVestingScheduleNotRevoked(vestingScheduleId)
    {
        ...
        // _nftVesting[vestingSchedule.nftId].availableForVesting = _nftVesting[vestingSchedule.nftId]
        //     .availableForVesting
        //     .sub(amount);
