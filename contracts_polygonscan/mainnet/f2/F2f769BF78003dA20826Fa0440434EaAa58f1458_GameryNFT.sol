...
contract GameryNFT is ERC721Enumerable, Ownable  {
    ...
    modifier onlyMinter {
        require(_isMinter[msg.sender] || msg.sender == owner(), "not Minter");
        _;
    }
    ...
    constructor() ERC721("GameryNFT","GameryNFT") {} 
    ...
    mapping (address=> mapping (uint=>uint)) private amountByTypeByWallet;
    
    
    function createToken(address holder, uint typeId) public onlyMinter returns (uint tokenId) {
        tokenId = ++nextTokenId;
        typeIdByTokenId[tokenId] = typeId;
        _mint(holder, tokenId);
    }
    
    function createRandomNFT(address holder, uint rarity, uint seed) public onlyMinter returns (uint) {
        ...
        amountByTypeByWallet[holder][typeId] = amountByTypeByWallet[holder][typeId] + 1;
        return createToken(holder, typeId);
    }
    
    function changeType(uint tokenId, uint toTypeId) public onlyMinter {
        address holder = ownerOf(tokenId);
        amountByTypeByWallet[holder][typesById[tokenId].id] = amountByTypeByWallet[holder][typesById[tokenId].id] - 1;
        typeIdByTokenId[tokenId] = toTypeId;
        amountByTypeByWallet[holder][toTypeId] = amountByTypeByWallet[holder][toTypeId] + 1;
    }
    ...
    function getAmountByTypes(address user, uint typeId) view external returns (uint) {
        return amountByTypeByWallet[user][typeId];
    }
    
    function _transfer(address from, address to, uint256 tokenId) internal override virtual {
        super._transfer(from,to, tokenId);
        uint typeId = typesById[tokenId].id;
        amountByTypeByWallet[from][typeId] = amountByTypeByWallet[from][typeId] - 1;
        amountByTypeByWallet[to][typeId] = amountByTypeByWallet[to][typeId] + 1;
    }
}
