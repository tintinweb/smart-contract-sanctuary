...
contract ClaimNFT is ERC721URIStorage, Ownable {
    ...
    // Royalties address
    address private royaltyAddress = 0x2c1754E836D357295d918DCA11DF5DeE76012DB1;
    ...
    constructor(bytes32 merkleRoot_) public ERC721("ClaimNFT", "NFT") {
    ...
    function claim(uint256 tokenId, address recipient, bytes32[] calldata merkleProof) external {
        require(!isClaimed(tokenId), 'ClaimNFT: NFT already claimed.');

        // Verify the merkle proof.
        bytes32 node = keccak256(abi.encodePacked(recipient, tokenId));
        require(MerkleProof.verify(merkleProof, merkleRoot, node), 'ClaimNFT: Invalid proof.');
