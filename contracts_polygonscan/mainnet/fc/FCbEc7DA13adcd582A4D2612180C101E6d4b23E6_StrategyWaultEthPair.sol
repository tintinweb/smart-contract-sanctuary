...
abstract contract BaseStrategyWaultChef is BaseStrategy {

    address public WAULT_ROUTER = 0x3a1D87f206D12415f5b0A33E786967680AAb4f6d;
    address public waultChef = 0xC8Bd86E5a132Ac0bf10134e270De06A8Ba317BFe;

    address public constant WMATIC = 0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270;
    address public constant WEXpoly = 0x4c4BF319237D98a30A929A96112EfFa8DA3510EB;
    ...
    constructor(
        ...
        public
        BaseStrategy(_want, _strategist, WEXpoly, WAULT_ROUTER, waultChef)
    {
        poolId = _poolId;
    }
    ...
    function balanceOfPool() public override view returns (uint256) {
        //amount, rewardDebt, pendingRewards
        (uint256 amount,,) = IWaultChef(rewards).userInfo(poolId, address(this));
        return amount;
    }
    
    function getHarvestable() external override view returns (uint256) {
        return IWaultChef(rewards).pendingWex(poolId, address(this));
    }
    ...
    function deposit() public override {
        ...
        if (_want > 0) {
            ...
            IWaultChef(rewards).deposit(poolId, _want, false);
        }
    }
    ...
    function _withdrawSome(uint256 _amount)
        ...
        returns (uint256)
    {
        IWaultChef(rewards).withdraw(poolId, _amount, false);
        return _amount;
    }
    ...
    function _getReward() internal {
        IWaultChef(rewards).claim(poolId);
    }
    ...
    function emergencyWithdraw() public onlyOwner {
        IWaultChef(rewards).emergencyWithdraw(poolId);
...
//For ETH/other pairs
contract StrategyWaultEthPair is BaseStrategyWaultChef {
    ...
    address[] public reward_eth_path;
    address[] public eth_other_path;
        ...
        )
    {
        ...
        //Trades routed thru WEX -> WMATIC -> WETH, then 1/2 thru WETH -> OTHER
        reward_eth_path = new address[](3);
        reward_eth_path[0] = harvestedToken;
        reward_eth_path[1] = WMATIC;
        reward_eth_path[2] = WETH;

        eth_other_path = new address[](2);
        eth_other_path[0] = WETH;
        eth_other_path[1] = otherToken;

        reward_matic_path = new address[](2);
        reward_matic_path[0] = harvestedToken;
    ...
    function harvest() public override onlyHumanOrWhitelisted nonReentrant {
        ...
        //Distribute fee, swap the harvested token for ETH, then swap 1/2 of the ETH to the other token
        if (_harvested_balance > 0) {
            ...
            //Can save 200k gas if I swapped all to WMATIC at once...
            swapRewardToWmaticAndDistributeFee(feeAmount);

            _swapUniswapWithPath(reward_eth_path, afterFeeAmount);

            uint256 _eth_balance = IERC20(WETH).balanceOf(address(this));
            _swapUniswapWithPath(eth_other_path, _eth_balance.div(2));
        }

        uint256 ethBalance = IERC20(WETH).balanceOf(address(this));
        uint256 otherBalance = IERC20(otherToken).balanceOf(address(this));
        if (ethBalance > 0 && otherBalance > 0) {
            IERC20(WETH).safeApprove(currentRouter, 0);
            IERC20(WETH).safeApprove(currentRouter, ethBalance);
