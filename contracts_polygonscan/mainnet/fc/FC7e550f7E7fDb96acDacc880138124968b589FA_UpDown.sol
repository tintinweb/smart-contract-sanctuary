...
contract UpDown is IERC721Locker,OwnableUpgradeable, ReentrancyGuard {
    ...
    uint256[3] public bonusReward;// [500 , 300 , 200]
    ...
    function getBoostedPercent(uint256 _tokenId) public view returns (uint256 _boosted) {
        if (_tokenId > 0) {
            ...
            // boosted range: (2%, 30%)
            if (productivityStat >= 400) _boosted = bonusReward[0]; // 30%
            else if (productivityStat <= 120) _boosted = bonusReward[2]; // 2%
            else _boosted = bonusReward[1]; // eg 392 -> 29.2%
        } else {
            _boosted = 0;
        }
    }
    ...
    */
    ...
    function settleBet(
        ...
        uint256 winAmount = _getBoostedAmount(calculateWinAmount(bet.number, bet.isOver, bet.amount), userStakedNft[bet.player]);
