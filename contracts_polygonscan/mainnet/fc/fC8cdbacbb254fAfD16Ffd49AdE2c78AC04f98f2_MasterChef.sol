...
// File: contracts/CherryToken.sol

pragma solidity 0.6.12;

// CherryToken with Governance.
contract CherryToken is ERC20('CherrySwap Token', 'Cherry') {
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 accCherryPerShare;    // Accumulated CHERRYs per share, times 1e18. See below.
    ...
    uint256 public cherryPerBlock = 10 ether;
    ...
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        ...
        if (_amount > 0) {
            ...
            if (pool.depositFeeBP > 0) {
                ...
                // Transfer 75% of deposit fee to Fee Address for token buyback and Polycherry games
    ...
    // Safe cherry transfer function, just in case if rounding error causes pool to not have enough CHERRY.
    ...
    function updateEmissionRate(uint256 _cherryPerBlock) public {
       
        massUpdatePools();
     
        cherryPerBlock = _cherryPerBlock;
        emit UpdateEmissionRate(msg.sender, cherryPerBlock);
    }
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (address(referral) != address(0) && referralCommissionRate > 0) {
            ...
            if (referrer != address(0) && commissionAmount > 0) {
                cherry.mint(_user, commissionAmount);
    ...
    // Transfer Ownership of the Cherry Token
    function transferTokenOwnership(address newOwner) public onlyOwner {
      
        require(newOwner != address(0) , "setNewOwner: FORBIDDEN");
       
        cherry.transferOwnership(newOwner);
