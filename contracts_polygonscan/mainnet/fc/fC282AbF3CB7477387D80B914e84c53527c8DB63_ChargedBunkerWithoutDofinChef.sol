...
contract ChargedBunkerWithoutDofinChef {
...
// -------------------------------------------------- public variables ---------------------------------------------------
    ...
    uint256 public testFreeFundValue;
    uint256 public testTotalDebts;
    uint256 public cream_total_supplied ;
    uint256 public reward_value ;
    uint256 public token_a_value ;
    uint256 public token_b_value ;
    uint256 public crtoken_a_debt;
    uint256 public crtoken_b_debt;
    uint256 public staked_token_a_amt;
    uint256 public staked_token_b_amt;
...
// ---------------------------------------- getters & check ------------------------------------------
    ...
    function getWithdrawAmount() external returns (uint256) {
        ...
        (uint256 freeFund_value, uint256 totalDebts) = getTotalAssets();
        uint256 totalAssets = freeFund_value.add(totalDebts);
    ...
    function getTotalAssets() public returns (uint256,uint256) {

        // Free fund amount -> norm_amount -> norm_value
        testFreeFundValue = HLS_charged.getTokenValueFromDeNormAmount(Position.token, HLSConfig.token_oracle, IERC20(Position.token).balanceOf(address(this)));

        // Total Debts amount from Cream, Quickswap

        uint256[8] memory ans  = HLS_charged.getTotalDebts(HLSConfig, Position, singleFarm);
        testTotalDebts = ans[0].add(ans[1]).add(ans[2]).add(ans[3]);
        cream_total_supplied  = ans[0];
        reward_value  = ans[1];
        token_a_value  = ans[2];
        token_b_value  = ans[3];
        crtoken_a_debt = ans[4];
        crtoken_b_debt = ans[5];
        staked_token_a_amt = ans[6];
        staked_token_b_amt = ans[7];

        return (testFreeFundValue , testTotalDebts);
    }
...
// -------------------------------------- manipulative functions -------------------------------------
    ...
    function autoCompound(uint256 _amountIn, address[] calldata _path, uint256 _wrapType) external {
        ...
        uint256[8] memory ans  = HLS_charged.getTotalDebts(HLSConfig, Position, singleFarm);
        testTotalDebts = ans[0].add(ans[1]).add(ans[2]).add(ans[3]);
        Position.total_debts = testTotalDebts ;
...
// ---------------------------------------- charged buncker manipulative function ---------------------------------------
    ...
    function enterPosition(HLSConfig memory self, Position memory _position, uint256 _type, bool _singleFarm) external returns (Position memory) { 
        ...
        uint256[8] memory ans = getTotalDebts(self, _position, _singleFarm);
        _position.total_debts = ans[0].add(ans[1]).add(ans[2]).add(ans[3]);
...
// ------------------------------------------- charged buncker getter function ------------------------------------------
    ...
    function getTotalDebts(HLSConfig memory self, Position memory _position, bool _singleFarm) public returns (uint256[8] memory) {

        uint256 cream_total_supplied = _position.supply_amount.mul(10**18).div(10**IERC20(_position.token).decimals()) ;

        cream_total_supplied = cream_total_supplied.mul(uint256(LinkOracle(self.token_oracle).latestAnswer())).div(10**LinkOracle(self.token_oracle).decimals());
        ...
        (token_a_value, token_b_value) = helper(self,staked_token_a_amt,staked_token_b_amt,token_a_value,token_b_value,crtoken_a_debt,crtoken_b_debt);

        // return cream_total_supplied.add(reward_value).add(token_a_value).add(token_b_value);
        // return ( cream_total_supplied, claimReward(self, _position, _singleFarm), token_a_value, token_b_value);
        uint256 rewardValue = claimReward(self, _position, _singleFarm);
        return [cream_total_supplied, rewardValue, token_a_value, token_b_value, crtoken_a_debt, crtoken_b_debt, staked_token_a_amt, staked_token_b_amt];

    }

    function helper(HLSConfig memory self, uint256 staked_token_a_amt, uint256 staked_token_b_amt, uint256 token_a_value, uint256 token_b_value, uint256 crtoken_a_debt, uint256 crtoken_b_debt) public view returns (uint256,uint256) {
