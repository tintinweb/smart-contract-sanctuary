...
contract Omikuji is ERC721, ERC721Enumerable, HasSecondarySaleFees, ContextMixin, Ownable {
    using Strings for uint256;
    ...
    function mint() public {
        require(IERC20(WETH).balanceOf(msg.sender) >= COST, "low wETH");
    ...
    function withdrawMATIC() public onlyOwner {
        Address.sendValue(payable(_RecvOwner), address(this).balance);
    }

    function withdrawETH() public onlyOwner {
        IERC20(WETH).transfer(_RecvOwner, IERC20(WETH).balanceOf(address(this)));
    }
