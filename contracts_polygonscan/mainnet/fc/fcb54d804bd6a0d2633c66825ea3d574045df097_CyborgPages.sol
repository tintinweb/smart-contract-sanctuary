...
// Kind Cyborgs (Cyborg Pages)

/*
Kind Cyborgs is the very first NFT collection of Kind Universe. Its a Epic High Sci-Fi Fantasy Play to Earn Action-RPG game set
in a newly discovered world. By claiming this Cyborg page you can contribute to the story of Kind Cyborgs.
In order to claim a Cyborg Page, you must own 2 Darshika - Kind Cyborg NFTs. 
...
contract CyborgPages is ERC721Enumerable, Ownable {
  using Strings for uint256;

  string baseURI;
  uint256 public requiredAmount = 2;
  uint256 public maxCharLimit = 300;
  bool public paused = false;
  string public notRevealedUri;
  address public kcAddress;
  ...
  constructor(
    ...
    address _initkcAddress
  ) ERC721(_name, _symbol) {
    setBaseURI(_initBaseURI);
    setNotRevealedURI(_initNotRevealedUri);
    setkcAddress(_initkcAddress);
  }
  ...
  function claim() public {
    ...
    IERC721 token = IERC721(kcAddress);
    uint256 ownedAmount = token.balanceOf(msg.sender);
    require(ownedAmount >= requiredAmount, "You don't own enough Kind Cyborgs NFTs");
  ...
  //only owner
  function setkcAddress(address _newAddress) public onlyOwner {
    kcAddress = _newAddress;
  }
