...
// File contracts/strategies/GrowStrategyApeV4.sol
...
contract GrowStrategyApeV4 is IGrowUpgradeableImplementation, BaseGrowStrategyV4 {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    bytes32 override public constant CONTRACT_IDENTIFIER = keccak256("GrowStrategyApeV4");
    ...
    function _trySwapUnderlyingRewardToRewardToken() internal override {
        for (uint256 i = 0; i < UNDERLYING_REWARD_TOKENS.length; ++i) {
            ...
            uint256 UNDERLYING_REWARD_TOKEN_AMOUNT = UNDERLYING_REWARD_TOKENS_AMOUNT[UNDERLYING_REWARD_TOKEN];
            ...
            if (UNDERLYING_REWARD_TOKEN_AMOUNT < underlyingRewardTokensSwapThreshold[UNDERLYING_REWARD_TOKEN]) return;
            ...
            UNDERLYING_REWARD_TOKENS_AMOUNT[UNDERLYING_REWARD_TOKEN] = UNDERLYING_REWARD_TOKEN_AMOUNT.sub(rewardTokenAmountBefore.sub(underlyingRewardAmountAfter));
        }
    }
    ...
    function emergencyExit() external override onlyRole(CONFIGURATOR_ROLE) {
        uint256 underlyingWantTokenAmount = _underlyingWantTokenAmount();
        IMasterApeV2(MASTER_CHEF_LIKE).withdraw(MASTER_CHEF_LIKE_POOL_ID, underlyingWantTokenAmount, address(this));
