...
contract MicroMachineManufacturingPlant is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable  {
   ...
   // address[] public EngineContracts;
    ...
    mapping ( uint256 => uint256 ) public futures;
    ...
    function setFuturesOnPlant( uint256 _plant, uint256 _periodsforfutures ) public onlyEngine {
       futures[_plant] =_periodsforfutures;
    }
    ...
    function setPlantFutures ( uint256 _plant, address _owner , uint256 _periods) public onlyEngine {
        require ( futuresEnabled , "Futures not enabled" );
        require ( ManufacturingPlants[_plant]._owner == _owner , "Not rightful owner" );
        require ( _periods < ManufacturingPlants[_plant]._timeunitslocked - ManufacturingPlants[_plant]._periodsmanufactured, "Not Enough remaining Periods ");
        futures[_plant] = _periods;
    }
    ...
    function manufacture( uint256 _plant ) public payable {
        ...
        if ( futures[_plant] > 0 ) require ( isEngineContract(msg.sender), "Plant is under futures contract");
        if ( futures[_plant] > 0 ) futures[_plant]--;
    ...
    function unstakeMicroMachines ( uint256 _plant ) public  {
        ...
        require ( isExpired (ManufacturingPlants[ _plant]._lastmanufacture + manufacturingPeriod) == true ,    "cooldown period not complete" );
        //require ( isExpired (ManufacturingPlants[ManufacturingPlantCount]._timeofexpiration) == true , "time committed not yet fulfilled" );
        require ( ManufacturingPlants[ _plant]._periodsmanufactured >= ManufacturingPlants[ _plant]._timeunitslocked );
        
        ManufacturingPlants[_plant]._lastmanufacture = block.timestamp;
        ManufacturingPlants[_plant]._status = false;
        
        globalwarbotproduction = globalwarbotproduction - ManufacturingPlants[ManufacturingPlantCount]._periodproductionrate;
        globalwarbotmanufacturingplants--;
         ERC20 _token = ERC20(MicroMachineAddress);
