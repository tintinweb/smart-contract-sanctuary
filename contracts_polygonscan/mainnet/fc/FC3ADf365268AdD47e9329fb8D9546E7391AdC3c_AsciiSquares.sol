...
abstract contract BASE_NFT is ERC721Tradable {
    constructor(string memory name, string memory symbol) ERC721Tradable(name, symbol, 0x58807baD0B376efc12F5AD86aAc70E78ed67deaE) {
    }

    function mintNToken(uint256 n, address to) public onlyOwner {
        for(uint256 i; i < n; i++){
            super.mintTo(to);
        }
    }
}
// File: ASCII_SQUARES.sol
...
contract AsciiSquares is BASE_NFT {
    constructor() BASE_NFT("ASCII SQUARES", "ASCII") {
    }

    function baseTokenURI() public override pure returns (string memory) {
        return "https://gateway.pinata.cloud/ipfs/QmcKNXfde1jZoTVevyBBy5gUeJbHtyR67BBtRgqkttKm6v/";
    }
}