...
contract Gemstone is ERC721, Ownable, ContextMixin {
    ...
    constructor(
        ...
        string[] memory gemsMeta,
        uint256 p
    ) ERC721("Gemstones", "GEM") {
        tokenCounter = 0;
        contractDataURL = contractMeta;
        _metaDataUris = gemsMeta;
        price = p;
    }

    function createGem() public payable returns (uint256) {
        require(
            tokenCounter < _metaDataUris.length,
            "max amount of tokens are already reached"
        );
        require(
            owner() == msg.sender || msg.value >= price,
            "need to pay a minimum price for minting."
        );

        uint256 newItemId = tokenCounter;
        _safeMint(msg.sender, newItemId);
        _setTokenURI(newItemId, _metaDataUris[newItemId]);
    ...
    function fillNFTS(string[] memory uris) public onlyOwner {
        for (uint256 i = 0; i < uris.length; i++) {
            _metaDataUris.push(uris[i]);
        }
    }
    ...
    function getLenMetaData() public view onlyOwner returns (uint256) {
