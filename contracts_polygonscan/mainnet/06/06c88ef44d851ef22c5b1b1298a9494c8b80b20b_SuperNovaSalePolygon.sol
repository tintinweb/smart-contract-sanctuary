...
contract SuperNovaSalePolygon is ReentrancyGuard {
    using SafeMath for uint256;
    ...
    // Price of sale SPN, 4 MATIC
    uint256 MATICPerToken = 4;
    // Amount of MATIC received in sale
    ...
    constructor (address _SPN, address _MATIC, uint256 _startingTimestamp) public {
        SPN = IBEP20(_SPN);
        MATIC = IBEP20(_MATIC);
        isSaleActive = true;
        owner = msg.sender;
    ...
    function buy (uint256 _amount, address beneficiary) public nonReentrant {
        ...
        require (maticReceived +  _amount <= 30000 ether, "sale hardcap reached");
        require(block.timestamp >= startingTimeStamp, "sale has not started");

        MATIC.safeTransferFrom(beneficiary, address(this), _amount);
    ...
    function getSPNTokensLeft() external view returns (uint256) {
        return SPN.balanceOf(address(this));
    }

    function claimTokens (address claimer) external {
        require (isClaimActive, "Claim is not allowed yet");
        require (tokensOwned[msg.sender] > 0, "User should own some SPN tokens");
        require (tokensUnclaimed[msg.sender] > 0, "User should have unclaimed SPN tokens");
        require (SPN.balanceOf(address(this)) >= tokensUnclaimed[msg.sender], "There are not enough SPN tokens to transfer.");

        SPN.safeTransfer(msg.sender, tokensUnclaimed[msg.sender]);
        emit TokenClaim(msg.sender, tokensUnclaimed[msg.sender]);
        tokensUnclaimed[msg.sender] = 0;
    }

    function withdrawFunds() external onlyOwner {
        MATIC.safeTransfer(msg.sender, MATIC.balanceOf(address(this)));
    }

    function withdrawUnsoldSPN() external onlyOwner {
        uint256 amount = SPN.balanceOf(address(this)) - totalTokensSold;
        SPN.safeTransfer(msg.sender, amount);
    }

    function withdrawAllSPN() external onlyOwner {
        SPN.safeTransfer(msg.sender, SPN.balanceOf(address(this)));
    }
}