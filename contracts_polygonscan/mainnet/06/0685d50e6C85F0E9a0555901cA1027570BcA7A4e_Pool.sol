...
contract Pool is Ownable, ReentrancyGuard, Initializable, IPool {
    ...
    uint256 public pool_ceiling = 100000000e6; // Total across all collaterals.
    ...
    function recollatTheoColAvailableE18() public view returns (uint256) {
        uint256 _dollar_total_supply = ERC20(dollar).totalSupply();

        (, , , uint256 _tcr, , , ,) = ITreasury(treasury).info();
        uint256 _ecr = calcEffectiveCollateralRatio();

        uint256 _desired_collat_e24 = _tcr * _dollar_total_supply;
        uint256 _effective_collat_e24 = _ecr * _dollar_total_supply;
