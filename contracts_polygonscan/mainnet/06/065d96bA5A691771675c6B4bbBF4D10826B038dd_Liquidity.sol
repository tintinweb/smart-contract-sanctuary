...
contract Liquidity is Ownable {
    ...
    constructor(
      ...
      BANSKY = IERC20(_BANSKY);
      _allowance();
    }

  receive() external payable {}

  function withdrawBanskyLiquidity() external onlyOwner {
      USDC.transfer(msg.sender, USDC.balanceOf(address(this)));
      BANSKY.transfer(msg.sender, BANSKY.balanceOf(address(this)));
  }
 ...
 function withdrawAll()  external onlyOwner {
  ...
  function AutoLiquidity() external onlyOwner {
    ...
    uint256 banskyBalance = BANSKY.balanceOf(address(this));
    uint256 halfTokenAmount = banskyBalance / 2;

    if (usdcBalanceAfter > 0 && wmaticBalance > 0 && banskyBalance > 0) {
        // add the liquidity bansky-matic
  ...
  function _allowance() internal {
    ...
    BANSKY.approve(address(ROUTER), type(uint256).max);
  }
