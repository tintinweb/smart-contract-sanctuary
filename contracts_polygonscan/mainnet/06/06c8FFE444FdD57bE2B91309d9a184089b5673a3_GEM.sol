...
contract GEM is IERC2981, ERC721 {
    ...
    string constant imageGenerationAlgHash = "6211a0a7ea932846c08ac1cdcae784562b6bcbe26fae71aa0b18442366e74a91";

    constructor(address _owner, uint24 _defaultBips, address _defaultReceiver, string memory _baseURI) ERC721("Gemini Onboarding NFTs", "GEM", "") {
        require(_defaultBips <= 10000, 'ERC2981Royalties: Too high');
        _royalties = RoyaltyInfo(_defaultReceiver, _defaultBips);
        owner = _owner;
        baseURI = _baseURI;
    }

    // uint256 _uniqueness => (zero_padding_8_bytes || date_4_bytes || address_20_bytes)
    function mint(address _to, string memory _uniqueness) public onlyOwner {
        gems.push(_uniqueness);
        uint256 tokenId = gems.length - 1;
        safeMint(tokenId, _to);
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return string(abi.encodePacked(baseURI, gems[tokenId]));
    }
