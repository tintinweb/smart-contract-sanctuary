...
/**
 * @title ZGS ERC20 token
 * @dev
 *      Based on the Ampleforth & Safemoon protocol.
 */
contract ZGS is ERC20Detailed, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;

    event LogRebase(uint256 indexed epoch, uint256 ownerSupply);
   ...
   uint256 private constant INITIAL_FRAGMENTS_SUPPLY = 10**15 * 10**DECIMALS;
    ...
    // TOTAL_ZGS is a multiple of INITIAL_FRAGMENTS_SUPPLY so that _ZGS PerFragment is an integer.
    // Use the highest value that fits in a uint256 for max granularity.
    uint256 private constant TOTAL_ZGS = MAX_UINT256 - (MAX_UINT256 % INITIAL_FRAGMENTS_SUPPLY);

    // MAX_SUPPLY = maximum integer < (sqrt(4*TOTAL_ZGS + 1) - 1) / 2
    uint256 private constant MAX_SUPPLY = ~uint128(0);  // (2^128) - 1

    uint256 private _totalSupply;
    uint256 private _ZGSPerFragment;
    mapping(address => uint256) private _zgsBalances;

    // This is denominated in Fragments, because the ZGS-fragments conversion might change before
    // it's fully paid.
    mapping (address => mapping (address => uint256)) private _allowedFragments;

    constructor (address router, address payable _marketingAddress)
        ERC20Detailed("ZeroGravity Space Shuttle", "ZGS", uint8(DECIMALS))
        payable
        public
    {
        ...
        _zgsBalances[msg.sender] = TOTAL_ZGS;
        _ZGSPerFragment = TOTAL_ZGS.div(_totalSupply);
    ...
    function balanceOf(address who)
        ...
        returns (uint256)
    {
        return _zgsBalances[who].div(_ZGSPerFragment);
    }
   ...
   returns (bool)
   {
    ...
    function _transferStandard(address sender, address recipient, uint256 amount) private {
      (uint256 tTransferAmount, uint256 tFee) = _getTValues(amount);
          uint256 zgsDeduct = amount.mul(_ZGSPerFragment);
          uint256 zgsValue = tTransferAmount.mul(_ZGSPerFragment);
          _zgsBalances[sender] = _zgsBalances[sender].sub(zgsDeduct);
          _zgsBalances[recipient] = _zgsBalances[recipient].add(zgsValue);
          _takeFee(tFee);
          emit Transfer(sender, recipient, amount);
    }

    function _transferExcluded(address sender, address recipient, uint256 amount) private {
          uint256 zgsValue = amount.mul(_ZGSPerFragment);
          _zgsBalances[sender] = _zgsBalances[sender].sub(zgsValue);
          _zgsBalances[recipient] = _zgsBalances[recipient].add(zgsValue);
          emit Transfer(sender, recipient, amount);
    }
    ...
    function _takeFee(uint256 tFee) private {
        uint256 rFee = tFee.mul(_ZGSPerFragment);
        _zgsBalances[address(this)] = _zgsBalances[address(this)].add(rFee);
