...
contract UnityCards is ERC721Enumerable, Ownable, IUnityCards, IUnityCardsMetadata, ReentrancyGuard {
  ...
  */
  ...
  function airdrop(address[] calldata recipents) external onlyOwner{
    require(totalSupply() < MAX_SUPPLY, 'All tokens have been minted'); 
    require(airdropActive, 'Airdrop permanently disabled');      

        for (uint256 i = 0; i < recipents.length; i++) {

      totalPublicSupply += 1;
      _safeMint(recipents[i], totalPublicSupply);
    }
  }

  function airdropMany(address recipent, uint256 amount) external onlyOwner{
    require(totalSupply() < MAX_SUPPLY, 'All tokens have been minted'); 
    require(airdropActive, 'Airdrop permanently disabled');      
      ...
      totalPublicSupply += 1;
      _safeMint(recipent, totalPublicSupply);
    }
  }
  ...
  function _killAirdrop() external onlyOwner{
    airdropActive = false;
  }
