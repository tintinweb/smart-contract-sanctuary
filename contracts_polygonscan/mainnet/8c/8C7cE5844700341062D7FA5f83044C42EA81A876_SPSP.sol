...
// File: original_contracts/staking/SPSP.sol
...
contract SPSP is ERC20("Staked PSP", "sPSP"), Ownable {
    using SafeMath for uint256;

    enum WITHDRAW_STATUS { _, LOCKED, RELEASED }
    ...
    constructor(IERC20 _psp, uint256 _timeLockBlocks) public {
        psp = _psp;
        _timeLockBlocks = timeLockBlocks;
    }
    ...
    function enter(uint256 _pspAmount) external {
        uint256 totalPSP = psp.balanceOf(address(this));
        uint256 totalSPSP = totalSupply();
        if (totalSPSP == 0 || totalPSP == 0) {
            _mint(msg.sender, _pspAmount);
        } else {
            uint256 sPSPAmount = _pspAmount.mul(totalSPSP).div(totalPSP);
    ...
    function leave(uint256 _sPSPAmount, uint256 id) external {
        uint256 totalSPSP = totalSupply();
        uint256 pspAmount = _sPSPAmount.mul(psp.balanceOf(address(this))).div(totalSPSP);
        _burn(msg.sender, _sPSPAmount);

        WithdrawalRequests storage request = userVsWithdrawals[msg.sender][id];
        require(request.status == WITHDRAW_STATUS._, "Invalid id");

        request.user = msg.sender;
        request.amountPSP = pspAmount;
        request.releaseBlockNumber = block.number.add(timeLockBlocks);
    ...
    function PSPBalance(address _account) external view returns (uint256 pspAmount_) {
        ...
        uint256 totalSPSP = totalSupply();
        pspAmount_ = sPSPAmount.mul(psp.balanceOf(address(this))).div(totalSPSP);
    }
    ...
    function sPSPForPSP(uint256 _sPSPAmount) external view returns (uint256 pspAmount_) {
        uint256 totalSPSP = totalSupply();
        pspAmount_ = _sPSPAmount.mul(psp.balanceOf(address(this))).div(totalSPSP);
    }
    ...
    function PSPForSPSP(uint256 _pspAmount) external view returns (uint256 sPSPAmount_) {
        ...
        if (totalSPSP == 0 || totalPSP == 0) {
            ...
            sPSPAmount_ = _pspAmount.mul(totalSPSP).div(totalPSP);
        }
    }
}