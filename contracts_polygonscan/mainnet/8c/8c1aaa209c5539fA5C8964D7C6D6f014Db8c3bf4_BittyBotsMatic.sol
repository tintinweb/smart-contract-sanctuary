...
contract BittyBotsMatic is ERC721Enumerable, VRFConsumerBase, Ownable  {
    ...
    function combineFee(uint _combinedCount) public pure returns (uint) {
        if (_combinedCount <= 1) {
            return 0;
        } else if (_combinedCount == 2) {
            return PRICE;
        } else if (_combinedCount == 3) {
            return 2 * PRICE;
        }
        return 0;
    }
    ...
    function combine(uint[] memory _selectedTraits, uint[] memory _selectedBots) external payable {
        ...
        if (newCombinedCount > 1) {
            require(msg.value >= combineFee(newCombinedCount), "Ether value sent is below the combine fee");
        }
    ...
    function raffleTicketsForAddress(address _address) public view returns (uint result) {
        uint[] memory tokens = tokensOfOwner(_address);
        uint powerClass;
        for (uint i = 0; i < tokens.length; i++) {
            (,,, powerClass,) = traitsForTokenId(tokens[i]);
            result += 1 << (powerClass - 1);
        }
    }

    // Note: This can only used for minting existing bots on Ethereum Mainnet
