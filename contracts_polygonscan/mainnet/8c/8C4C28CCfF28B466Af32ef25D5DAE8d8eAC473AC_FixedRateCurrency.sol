...
// File: libs/legacy-currency-contracts/contracts/FixedRateWrapper.sol
...
contract FixedRateWrapper is ERC20 {
  ...
  {
    amountTokens = (_amount * rate) / (PRECISION);
   ...
   */
  function unwrap(uint256 _amount, address burnerAddress)
    internal
    returns (uint256 amountTokens)
  {
    require(balanceOf(msg.sender) >= _amount, 'Not enought tokens to unwrap');
    amountTokens = (total_deposited * _amount) / totalSupply();
    total_deposited = total_deposited - amountTokens;
...
// File: @jarvis-network/atomic-swap/contracts/v1/interfaces/IAtomicSwap.sol
...
// File: libs/legacy-currency-contracts/contracts/v1/FixedRateCurrency.sol
...
contract FixedRateCurrency is Ownable, ERC20Permit, FixedRateWrapper {
  using SafeERC20 for IERC20;

  bool public paused; // Prevents minting when true

  ISynthereumFinder public immutable synthereumFinder;
  ISynthereumPoolOnChainPriceFeed public immutable synthereumPool; // pegSynth-USDC pool
  IERC20 public immutable collateralInstance; // synthereum pool collateral (USDC)
  IAtomicSwap public immutable atomicSwap;
  ...
  {
    tokensRedeemed = super.unwrap(_fixedSynthAmount, msg.sender);
    synth.safeTransfer(recipient, tokensRedeemed);
  ...
  function redeemUSDC(
    ...
    uint256 pegTokensUnlocked =
      super.unwrap(_redeemParams.numTokens, msg.sender);
  ...
  function mintFromSynth(
    ...
    require(
      _exchangeParams.destPool == synthereumPool,
      'Wrong destination pool'
    );
  ...
  function swapForSynth(
    ...
    // burn _synthAmount to get peg tokens jEur into this wallet
    uint256 pegTokensUnlocked = super.unwrap(_fixedSynthAmount, msg.sender);
  ...
  function swapToERC20(
    ...
    uint256 pegSynthRedeemed = super.unwrap(fixedSynthAmountIn, msg.sender);
  ...
  function checkPoolRegistration(
    ISynthereumFinder _synthereumFinder,
    ISynthereumPoolOnChainPriceFeed _synthereumPool
  ) internal view returns (address collateralAddress, address synthAddress) {
    ...
    string memory synthTokenSymbol = _synthereumPool.syntheticTokenSymbol();
    collateralAddress = address(_synthereumPool.collateralToken());
    uint8 version = _synthereumPool.version();
    require(
    ...
    synthAddress = address(_synthereumPool.syntheticToken());
  }
}