...
contract Vesting is AccessControlUpgradeable, OwnableUpgradeable {
    ...
    function addVester(
        ...
        require(records[_account][_name].amount == 0, 'VESTING: Record already exists');
        require(items[_name].amount != 0, 'VESTING: Item does not exist');

        userVests[_account].push(_name);
        records[_account][_name] = Vested({
            amount: _amount,
            totalWithdrawn: 0,
        ...
        emit VesterCreated(_account, _name, _amount);
    }

    function addMultipleVesters(AddMultipleVesters[] calldata vesters) external onlyOwner {
        for (uint256 i = 0; i < vesters.length; i++) {
            addVester(
                vesters[i].name,
                vesters[i].account,
                vesters[i].percentInitialAmount,
                vesters[i].percentAmountPerWithdraw,
                vesters[i].percentBonus,
                vesters[i].amount
            );
        }
    }
