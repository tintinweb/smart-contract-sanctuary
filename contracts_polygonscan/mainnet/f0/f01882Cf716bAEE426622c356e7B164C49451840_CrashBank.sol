...
contract CrashBank is ReentrancyGuard, Ownable {
    ...
    address public maticETHAddress;
    ...
    constructor(address _crashGameAddress, address _rETHAddress, address _hETHAddress, uint256 _protocolFee, uint32 _lastGameNumber) {
        ...
        maticETHAddress = 0x7ceB23fD6bC0adD59E62ac25578270cFf1b9f619;
        protocolFee = _protocolFee;

        crashGame = ICrashGame(crashGameAddress);
        rETH = IrETH(rETHAddress);
        hETH = IhETH(hETHAddress);
        maticETH = IERC20(maticETHAddress);
    ...
    function depositrETH(uint256 amount) external nonReentrant {
        //uint256 amount = msg.value;
        maticETH.safeTransferFrom(msg.sender, address(this), amount);
        rETH.mint(msg.sender, amount);
    }

    function redeemrETH(uint256 amount) external nonReentrant {
        //check that user balance is sufficient
        require(amount <= rETH.balanceOf(msg.sender), "User balance is not sufficient");
        rETH.burn(msg.sender, amount);
        //address payable sender = payable(address(msg.sender));
        //transfer matic ETH back to the user
        maticETH.safeTransfer(msg.sender, amount);
    ...
    function deposithETH(uint256 amount) external nonReentrant {
        ...
        uint256 sharesToMint = 1e18*amount / pricePerShare;
        maticETH.safeTransferFrom(msg.sender, address(this), amount);
    ...
    function redeemhETH(uint256 amount) external nonReentrant {
        ...
        maticETH.safeTransfer(msg.sender, ethToReturn);
    }
    ...
    /**
     * @dev Withdraw other ERC20 tokens sent to contract by mistake
     */
    function withdrawOtherTokens(address tokenAddr) public onlyOwner {
        uint256 balance = IERC20(tokenAddr).balanceOf(address(this));
        IERC20(tokenAddr).safeTransferFrom(address(this), msg.sender, balance);
    }
