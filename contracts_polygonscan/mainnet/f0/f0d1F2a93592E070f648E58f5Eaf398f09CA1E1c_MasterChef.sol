...
contract KiwiToken is BEP20('Kiwi Token', 'KIWI') {
    ...
    */
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying KIWIs (not scaled);
...
// will be transferred to a governance smart contract once KIWI is sufficiently
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of KIWIs
    ...
    // The KIWI TOKEN!
    KiwiToken public kiwi;
    // Dev address.
    address public devaddr;
    // KIWI tokens created per block.
    uint256 public KiwiPerBlock;
    // Bonus muliplier for early kiwi makers.
    ...
    // The block number when KIWI mining starts.
    ...
    event UpdateEmissionRate(address indexed user, uint256 KiwiPerBlock);
    ...
    // Update the given pool's KIWI allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending KIWIs on frontend.
    function pendingKiwi(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accKiwiPerShare = accKiwiPerShare.add(kiwiReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 kiwiReward = multiplier.mul(KiwiPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        kiwi.mint(devaddr, kiwiReward.div(10));
        kiwi.mint(address(this), kiwiReward);
        pool.accKiwiPerShare = pool.accKiwiPerShare.add(kiwiReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    // Safe kiwi transfer function, just in case if rounding error causes pool to not have enough KIWIs.
    ...
    function updateEmissionRate(uint256 _KiwiPerBlock) public onlyOwner {
        massUpdatePools();
        KiwiPerBlock = _KiwiPerBlock;
        emit UpdateEmissionRate(msg.sender, _KiwiPerBlock);
    }
