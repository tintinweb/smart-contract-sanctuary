...
contract CheezyHalloween is ERC721Enumerable, Ownable, ReentrancyGuard {
	using Strings for uint256;
	...
	constructor() ERC721("CheezyHalloween", "Cheezy") {}
    ...
    function Airdropmint(address to, uint256 tokenId) public  onlyOwner {
        safeTransferFrom(address (this),to,tokenId);
    }
    
    
    function airdrop(address[] calldata addresses, uint256 _startid) external onlyOwner {
    startid = _startid;
    for(uint i=0; i < addresses.length; i++){
        Airdropmint(addresses[i],startid);
        startid++;
        }
 }
