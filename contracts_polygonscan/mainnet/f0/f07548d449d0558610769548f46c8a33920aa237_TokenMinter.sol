...
// https://twitter.com/PolyBakerySwap
...
// File: contracts\farming\TokenMinter.sol
...
contract TokenMinter is ERC20, Ownable  {
    ...
    function getTax( uint256 _total ) private view returns (uint256 _amount, uint256 _amount_tax ){
        if( tax == 0 ){
            return( _total, 0 );
        }
        if( tax > 0 ){
