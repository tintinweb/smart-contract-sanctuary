...
abstract contract StrategyFarmBase is BaseStrategyMasterChef {
    ...
    function harvest() public override {
        ...
        if (_pswampBalance > harvestCutoff){
            ...
            uint256 performanceFee = _pswampBalance.mul(keepFXS).div(keepFXSmax);
            if (performanceFee>0) {
            ...
            _pswampBalance = IERC20(pswamp).balanceOf(address(this));
            if (_pswampBalance > 0) {
                _swapUniswapWithPath(pswamp_matic_path, _pswampBalance.div(2));
            }

            // Add liquidity for TITAN/MATIC
            uint256 _matic = IERC20(matic).balanceOf(address(this));
            uint256 _pswamp = IERC20(pswamp).balanceOf(address(this));
            if (_matic > 0 && _pswamp > 0) {
                IERC20(matic).safeApprove(currentRouter, 0);
                IERC20(matic).safeApprove(currentRouter, _matic);
                IERC20(pswamp).safeApprove(currentRouter, 0);
                IERC20(pswamp).safeApprove(currentRouter, _pswamp);
                ...
                IERC20(pswamp).safeTransfer(
                    strategist,
                    IERC20(pswamp).balanceOf(address(this))
                );
            }
        }
        // Stake the LP tokens
...
contract StrategyBase is StrategyFarmBase {
    ...
    address public pSwamp_MATIC_LP = 0x1A477272f6030EAB135Cb3BA40646f3eb26b382a;
