...
// File contracts/NiftybaseAirdrop.sol
...
contract NiftybaseAirdrop is ERC721Enumerable, PaymentSplitter, Ownable {
    ...
    // Additional Whitelist Members
    mapping(address => bool) private _overflowPreSaleList;
    ...
    function addToOverflowList(address[] calldata addresses) external onlyOwner {
      for (uint256 i = 0; i < addresses.length; i++) {
        require(addresses[i] != address(0), "Can't add a null address");
        _overflowPreSaleList[addresses[i]] = true;
      }
    }

    /* Pre Sale */
    function mintPreSaleTokens(address addr, uint8 numberOfTokens, bytes32[] calldata _merkleProof) external payable {
        require(preSaleActive, "Pre mint is not active");
        require(_preSaleList[addr].add(numberOfTokens) <= MAX_PRE_MINTS, "Exceeded max available to purchase");
        ...
        // check proof
        // @dev require address to be on merkle list or overflow list
        bytes32 leaf = keccak256(abi.encodePacked(addr));
        require(MerkleProof.verify(_merkleProof, preSaleRoot, leaf) || _overflowPreSaleList[addr], "Not on the whitelist");
        
        _preSaleList[addr] = _preSaleList[addr] + numberOfTokens;
