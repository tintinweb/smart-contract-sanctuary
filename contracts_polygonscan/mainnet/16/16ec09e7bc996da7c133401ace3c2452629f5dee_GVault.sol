...
contract GVault is Ownable {
    ...
    constructor() {
        ...
        INFT _NFT3 = INFT(0x2D266A94469d05C9e06D52A4D0d9C23b157767c2);
        INFT _NFT4 = INFT(0x02e2c5825C1a3b69C0417706DbE1327C2Af3e6C2);
        INFT _NFT5 = INFT(0x3378AD81D09DE23725Ee9B9270635c97Ed601921);
        NFT3 = _NFT3;
        NFT4 = _NFT4;
        NFT5 = _NFT5;
    ...
    function depositNFT(uint _type, uint id) public {
        ...
        for(uint i=0; i<5; i++) {
            ...
            if(NFTList[i].nftType > toBeReplaced && toBeReplaced != 0) {
                ...
                // 3. TRANSFER NEW NFT FROM THE OWNER TO THIS CONTRACT - CHECKS TYPE
                if (NFTList[i].nftType == 3) {
  	...
  	function withdrawNFT(uint id) public {
  	    ...
  	    // LOOKS AT ALL 5 SLOTS FOR SPECIFIED NFT
        for (uint i=0; i<5; i++) {
    ...
    function compound() public {
        ...
        if(currentBalance > 10**10) {
            ...
            if (devFee > 0) {
                ...
                DAIToken.transfer(devg, currentDAIBalance.mul(devgFee).mul(2).div(devFee));
        ...
        // Adding liquidity if there is a small balance available
        if (GFarmToken.balanceOf(address(this)) > 10**10) {
         	createLPTokens();
