...
contract TestingOracle {
    ...
    function getPrice(
        uint256 amountIn, 
        uint256 price0AverageNew, 
        uint256 price0AverageOld,
        uint256 timestampNew,
        uint256 timestampOld
    ) external pure returns (uint256) {
        uint256 timeElapsed = timestampNew - timestampOld;
        
        FixedPoint.uq112x112 memory average = FixedPoint.uq112x112(
            uint224((price0AverageNew - price0AverageOld) / timeElapsed)
        );
        
        return average.mul(amountIn).decode144();
    }
}