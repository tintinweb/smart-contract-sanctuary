...
// Root file: contracts/RedPandaPartnerSale.sol
...
// import { ICollectible, IERC721 } from "contracts/interfaces/ICollectible.sol";

contract RedPandaPartnerSale is Ownable {
    using SafeMath for uint256;

    uint256 public tokenPrice = 125e18; // 125 MATIC
    uint256 public maxTokenPurchase = 15;
    uint256 public constant maxTokens = 2300;
    uint256 public constant maxBonusTokens = 200;
    ...
    IERC721 public immutable partnerToken;
    ...
    constructor(ICollectible _nft, IERC721 _partnerToken, address _fund) {
        require(
            address(_nft) != address(0) &&
            address(_partnerToken) != address(0) &&
            _fund != address(0),
    ...
    function buy(uint _amount) external payable {
        require(
            partnerToken.balanceOf(_msgSender()) != 0,
            "Collection: only partner token holders"
        );
        ...
        require(
            minted.add(mintAmount) <= maxTokens,
