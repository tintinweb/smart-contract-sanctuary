...
// File: contracts/KunaiPresale.sol
...
contract KunaiPresale is ReentrancyGuard {
  ...
  uint256 maxHardCap = 150000000000;
	...
	function claimTokens(address _claimer) external nonReentrant {
		...
		require(tokensOwned[_claimer] > 0, "User should own some KUNAI tokens");
		require(tokensUnclaimed[_claimer] > 0, "User should have unclaimed KUNAI tokens");
		require(KUNAI.balanceOf(address(this)) >= tokensUnclaimed[_claimer], "There are not enough KUNAI tokens to transfer.");

		uint256 callerTokensUnclaimed = tokensUnclaimed[_claimer];
		tokensUnclaimed[_claimer] = 0;
		KUNAI.safeTransfer(_claimer, callerTokensUnclaimed);
