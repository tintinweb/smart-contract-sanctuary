...
// File contracts/library/StratManager.sol
...
contract StratManager is Ownable, Pausable {
    ...
    bool public enabledAutoCompound = false;

    uint public minSwapAmountLp0 = 0;
    uint public minSwapAmountLp1 = 0;
    ...
    function enableAutoCompound(bool _flag) external onlyOwner {
        enabledAutoCompound = _flag;
    }

    function setMininumSwapAmounts(uint256 _amount0, uint256 _amount1) external onlyManager {
        minSwapAmountLp0 = _amount0;
        minSwapAmountLp1 = _amount1;
    }
...
// File contracts/interfaces/IVaultV3.sol

interface IVaultV3 is IVault {
...
// File contracts/AutoMaticStrategyQuickLp.sol
...
contract AutoMaticStrategyQuickLp is StratManager {
    ...
    function harvest() external whenNotPaused {
        ...
        if (enabledAutoCompound == true) {
            ...
            if (rewards > 0 && vault != address(0)) {
                IERC20(output).safeTransfer(vault, rewards);
                IVaultV3(vault).notifySentReward(rewards);
            }
        }
    ...
    function addLiquidity() internal {
        ...
        if (lpToken0 != output && outputHalf < minSwapAmountLp0) return;

        if (lpToken1 != output && outputHalf < minSwapAmountLp1) return;
