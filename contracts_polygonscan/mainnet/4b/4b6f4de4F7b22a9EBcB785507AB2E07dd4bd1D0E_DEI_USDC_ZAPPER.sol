...
contract DEI_USDC_ZAPPER is Ownable {
        ...
        )
    {
        ...
        if (
            ...
            /// todo: how to convert it? (in quickswap or curve)
            /// how much dei we should swap with usdc?
            (swapAmount, ) = getSwapAmountDeiUniswap(input);
            ...
            uint256 remain;
            (swapAmount, remain) = getSwapAmountUsdc(input);
    ...
    function getSwapAmountDeiUniswap(ZapperInput memory input)
        public
        view
    ...
    function getSwapAmountDeiCurve(ZapperInput memory input)
        public
        view
        returns (uint256, uint256)
    {
        ...
        for (uint256 i = 0; i < input.iterations; i++) {
            ...
            y = IUniswapV2Router02(uniswapRouter).getAmountsOut(
                x,
                input.toUsdcPath
            )[input.toUsdcPath.length - 1];
    ...
    function getSwapAmountUsdc(ZapperInput memory input)
        public
        view
        returns (uint256, uint256)
    {
    ...
    function getSwapAmountDei(ZapperInput memory input)
        public
        view
        returns (uint256, uint256)
    {
    ...
    function getSwapAmountByPath(ZapperInput memory input)
        public
        view
        returns (uint256, uint256)
    {
