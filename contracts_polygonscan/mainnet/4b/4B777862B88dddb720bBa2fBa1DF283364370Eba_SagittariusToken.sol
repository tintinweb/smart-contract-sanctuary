...
// Test token
contract SagittariusToken is ERC20("Sagittarius Token", "SAGITTARIUS") {
    uint256 public transferTaxRate = 800;
    uint256 public constant MAXIMUM_TRANSFER_TAX_RATE = 1000;

    //Start with transfers disabled so no bots can snipe the launch
    uint256 public maxTransfer = 0; //Upon launch set to preffered antiwhale percentage and then can no longer be set to 0 in order to prevent honeypot
    ...
    address constant leoTokenContract =
        0xbE8DAb8Ce8521ecFDe43a8Ff8d5C6644F4dCECb7;
    ...
    constructor() public {
        ...
        // Create a uniswap pair for this new token so no bots trigger off liquidity creation event
        ...
        canBeWhale[address(0)] = true;
        canBeWhale[address(this)] = true;
    ...
    event SetWhale(address indexed dev, address indexed user, bool status);
    ...
    modifier antiWhale(
        ...
        if (canBeWhale[sender] == false && canBeWhale[recipient] == false) {
            require(
                amount <= maxTransferAmount(),
                "sagittarius::antiWhale: Transfer amount exceeds the maxTransferAmount"
            );
        }
        _;
    }

    function setWhitelist(address user, bool inWhitelist)
        external
        onlyDeveloper
    {
        whiteList[user] = inWhitelist;
        emit SetWhitelist(msg.sender, user, inWhitelist);
    }

    function setWhale(address _addr, bool _status) external onlyDeveloper {
        canBeWhale[_addr] = _status;
        emit SetWhale(msg.sender, _addr, _status);
    }

    function setMaxTransfer(uint256 _amount) external onlyDeveloper {
        require(_amount >= 100, "so little"); //To prevent from honeypot
        maxTransfer = _amount;
    ...
    function updateTransferTaxRate(uint256 _transferTaxRate)
        public
        onlyDeveloper
    {
        require(
            _transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE,
            "sagittarius::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate."
        );
    ...
    function swapTokensForLeo() private transferTaxFree lockTheSwap {
