   ...
   function mint(uint256 amountToMint) public payable {
    ...
    function cos(uint16 _angle) public pure returns (int) {
        if (_angle > ANGLES_IN_CYCLE - QUADRANT_LOW_MASK) {
            _angle = QUADRANT_LOW_MASK + _angle - ANGLES_IN_CYCLE;  //fixed bug in cosine
        } else {
...
function drawSpiro (uint16 smallCircle, uint16 penPosition, uint16 loop) public pure returns (string memory)  {  //pen position is 0 to 1000 for 0 to 1, smallCircle is 0 to 100 compared to 100 for bigCircle
    uint16 width=432; uint16 height=432; uint16 R=100; uint16 r=smallCircle; 
    //prepare svg
    string memory stringSVG='<svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 432 432"><defs/><g><animateTransform attributeType="xml" attributeName="transform" type="rotate" from="360 216 216" to="0 216 216" dur="20s" additive="sum" repeatCount="indefinite" /><rect xmlns="http://www.w3.org/2000/svg" fill="red" x="0" y="0" width="432" height="432" rx="216" ry="216"/>';
    
     int[2] memory p0;
     p0[0] = int(uint256((width/2)+(R-r)+r*penPosition/1000));
     p0[1] = int(uint256(height/2));
    
      for (uint16 ii = 0; ii < loop; ii += 15) {
    
        uint16 angle = ii * 8;   // if loopTime is 1000 that is one revolution a second  -  will increment 2*PI each revolution
		// ii is time in milliseconds for watching mode, draw mode is predetermined in increments of 15
	
        int[2] memory xy;
        xy[0]=int(uint256(R))*( (int(uint256(100-r))*(cos(angle)))/int(3276700) + ((int(uint256(penPosition*r))*cos(((100-r)*(angle)/(r))/100))/327670000 )) + int(uint256(width))/2;
        xy[1]=int(uint256(R))*( (int(uint256(100-r))*(sin(angle))/int(3276700)) - (int(uint256(penPosition*r))*sin(((100-r)*angle/r)/100))/327670000 ) + int(uint256(height))/2;
        
       
		//stringSVG
		stringSVG= string(abi.encodePacked(stringSVG,'<path stroke="rgb(255,255,255)" d=" M ',p0[0],' ',p0[1],' L ',xy[0],' ',xy[1],'" />'));

        p0 = xy;

      }
		
        return string(abi.encodePacked(stringSVG,'</g></svg>'));
    }
