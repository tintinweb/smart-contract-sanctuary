...
import "./PolygonalPunks.sol";
import "@openzeppelin/contracts/token/ERC721/IERC721Receiver.sol";
import "@openzeppelin/contracts/access/Ownable.sol";
import "@openzeppelin/contracts/security/ReentrancyGuard.sol";

contract PolygonalPunksRewards is IERC721Receiver, Ownable, ReentrancyGuard {

  PolygonalPunks private nftContract;

  uint256 private rewardPerMint = 11 ether;
  uint256 public totalReward = 0 ether;
  address public claimableWallet = address(0);
  uint256 public deadline;
  uint256 private timeNeeded = 1 days;


  constructor(PolygonalPunks _t) {
    nftContract = _t;
  }

  function mint() payable public {
    nftContract.mint{value:msg.value}();
    totalReward = totalReward + rewardPerMint;
    claimableWallet = msg.sender;
    deadline = block.timestamp + timeNeeded;
  }

  function claimReward() public nonReentrant {
    require(claimableWallet == msg.sender, "Wrong address!");
    require(block.timestamp >= deadline, "Deadline not yet reached, try again later.");
    require(totalReward > 1 ether, "There is no reward in the fund.");

    claimableWallet = address(0);
  ...
  function setTimeNeeded(uint256 time) public onlyOwner {
    timeNeeded = time;
  }

  function setRewardPerMint(uint256 reward) public onlyOwner {
    rewardPerMint = reward;
  }
...
contract PolygonalPunks is ERC721Enumerable, Ownable, ReentrancyGuard, ContextMixin {
...
//  string public baseURI;
  ...
  uint256 public claimPrice = 44 ether;
  ...
  constructor () ERC721("PolygonalPunks", "PGPNK") {
  ...
  function getPunksBelongingToOwner(address _owner) external view returns (uint256[] memory) {
