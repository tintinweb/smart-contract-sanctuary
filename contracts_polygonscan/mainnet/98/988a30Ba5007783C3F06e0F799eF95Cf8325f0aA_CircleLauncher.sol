...
contract CircleLauncher is ICircleLauncher, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    uint public constant engageCap = 3000e18;
    uint public constant launcherDeadline = 1626710400;
    ...
    function create(AuctionInfo memory request) external onlyOwner {
        ...
        IPolyCircleDistributor(request.beneficiary).notifyCreate(auctionCount, request.token);
        emit Create(auctionCount, request.token);
    }

    function archive(uint id) external nonReentrant onlyOwner {
        AuctionInfo memory auction = auctions[id];
        require(!auction.archived, "!archived");
        require(now >= auction.deadline, "!deadline");

        uint totalTokenAmount = swapTokenAmount(id, IERC20(WETH).balanceOf(address(this)));
        ICircleMinterV2(minter).mintForCircleLauncher(totalTokenAmount.mul(3).div(2), auction.beneficiary);
        ...
        if (totalEngaged > 0) {
            IERC20(WETH).safeTransfer(auction.beneficiary, totalEngaged);
        }

        IPolyCircleDistributor(auction.beneficiary).notifyArchive(id, auction.token, totalEngaged);
        emit Archive(id, auction.token, totalEngaged);
    }

    /* ========== FOR USERS ========== */

    function engage(uint id, uint amount) external {
        AuctionInfo memory auction = auctions[id];
        require(!auction.archived, "!archived");
        require(now < launcherDeadline, "!launcherDeadline");
        require(now < auction.deadline, "!deadline");
        require(auction.engaged < engageCap, "CircleLauncher: engage full!");

        if (amount.add(auction.engaged) > engageCap) {
            amount = engageCap.sub(auction.engaged);
        }
        ...
        user.engaged = user.engaged.add(amount);
        users[id][msg.sender] = user;

        auction.engaged = auction.engaged.add(amount);
        auctions[id] = auction;

        IPolyCircleDistributor(auction.beneficiary).notifyEngage(id, msg.sender, amount);
        emit Engage(id, msg.sender, amount);
    }
}
