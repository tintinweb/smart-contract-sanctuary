...
contract CrazyCactusNFT is ERC721Enumerable {
using Strings for uint256;

    address private admin;

    uint256 private mintCostPresale = 160*10**18;
    uint256 private mintCost = 240*10**18;
    ...
    uint256 private preSaleStartDate = 1642892400;
    uint256 private publicSaleStartDate = 1643497200;

    constructor(string memory _base,address _admin) ERC721("Crazy Cactus", "CrazyCactus")  {
        admin = _admin;
        __baseURI = _base;
        // ipfs://CID/
    }
    ...
    function mint(uint256 _mintAmount) public payable {
        ...
        require(_mintAmount <= 20, "max mint amount per transction exceeded");
        require(supply + _mintAmount <= maxSupply, "max NFT limit exceeded");


        if (block.timestamp < publicSaleStartDate ) {
            require(msg.value >= mintCostPresale * _mintAmount, "insufficient funds");
        } else{
  ...
  function setMintCostPresale(uint256 _newCost) public onlyAdmin {
    mintCostPresale = _newCost;
  }
  ...
  function setPreSaleStartDate(uint256 _preSaleStartDate) public onlyAdmin {
    preSaleStartDate = _preSaleStartDate;
  }

  function setPublicSaleStartDate(uint256 _publicSaleStartDate) public onlyAdmin {
  ...
  function isPublicSaleOn() public view returns(bool){
    ...
    function getpublicSaleStartDate() public view returns(uint256){
