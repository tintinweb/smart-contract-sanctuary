...
contract LiquidityRestrictor is Ownable {
    ...
    function setTrustedAgents(address[] memory who, bool isTrustedAgent_)
        external
        onlyOwner
    {
        for (uint256 i = 0; i < who.length; i++) {
    ...
    function _assureLiquidityRestrictions(
        ...
        if (params.startedAt[to] > 0 || params.bypass || !checkPair(token, to))
            return (true, '');
    ...
    function checkPair(address token, address possiblyPair)
        public
        view
        returns (bool isPair)
    {
    ...
    {
        return (
            parameters[token].isInitializer[who],
            parameters[token].isLocalAgent[who],
            isTrustedAgent[who]
        );
    }

    function seeStart(address token, address pair)
        public
        view
