 ...
 */
contract UnusedUsdcBankStrat is Ownable, Pausable {
    ...
    /**
     * @dev Tokens Used:
     * {wmatic} - Required for liquidity routing when doing swaps.
    ...
    address constant public aavebank = address(0x8dFf5E27EA6b7AC08EbFdf9eB090F32ee9a30fcf);
    ...
    address[] public wmaticToUsdc = [wmatic, usdc];
    ...
    constructor(address _vaultAddress) public {
        ...
        IERC20(usdc).safeApprove(aavebank, 0);
        IERC20(usdc).safeApprove(aavebank, uint(-1));
        IERC20(wmatic).safeApprove(unirouter, 0);
        IERC20(wmatic).safeApprove(unirouter, uint(-1));

        setBuybackStrat(0x4eeaA7E075E001F14A2C0856115836384af690c7);
        
        (atoken,,) = IDataProvider(dataProvider).getReserveTokensAddresses(lpPair);
    }
    ...
    function deposit() public {
        ...
        if (pairBal > 0) {
            ILendingPool(aavebank).deposit(usdc, IERC20(usdc).balanceOf(address(this)), address(this), 0);
        }
    }
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (pairBal < _amount) {   
            ILendingPool(aavebank).withdraw(usdc,_amount.sub(pairBal), address(this));
    ...
    function swap() internal{
        uint amount = IERC20(wmatic).balanceOf(address(this));
        IPanwexRouter(unirouter).swapExactTokensForTokens(amount, 0, wmaticToUsdc, address(this), now + 600);
    }
    ...
    function chargeFees() internal {
        if(buybackstrat!=address(0)){
            uint toSell = IERC20(wmatic).balanceOf(address(this)).mul(getBPSFee()).div(10000);
            IERC20(wmatic).transfer(buybackstrat, toSell);
        }
    }
    ...
    function balanceOfPool() public view returns (uint256) {
        (uint256 supplyBal,,,,,,,,) = IDataProvider(dataProvider).getUserReserveData(usdc, address(this));
        return supplyBal;
    }
    ...
    function panic() public onlyOwner {
        ILendingPool(aavebank).withdraw(usdc, type(uint).max, address(this));
        pause();
    }
    ...
    function pause() public onlyOwner {
        _pause();
        IERC20(lpPair).safeApprove(aavebank, 0);
        IERC20(wmatic).safeApprove(unirouter, 0);
        IERC20(usdc).safeApprove(aavebank, 0);
    }
    ...
    function unpause() external onlyOwner {
        ...
        IERC20(lpPair).safeApprove(aavebank, 0);
        IERC20(lpPair).safeApprove(aavebank, uint(-1));
        IERC20(wmatic).safeApprove(unirouter, 0);
        IERC20(wmatic).safeApprove(unirouter, uint(-1));
    }
}