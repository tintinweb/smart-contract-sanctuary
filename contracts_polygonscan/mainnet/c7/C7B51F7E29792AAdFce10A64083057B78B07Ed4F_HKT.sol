...
contract HKT is ERC20, Ownable {
    ...
    address public U;
    uint public buyPoudage = 5;
    uint[] public sellPoudage = [55, 20, 20, 30, 25];//stage,super,big,middle,small
    uint public burnAmount;
    uint public autoAmount;
    address public burnAddress;
    address public node;
    uint public limit = 1e10;
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {
        ...
        if (!white[recipient]) {
            if (!white[_msgSender()] && buyPoudage > 0) {
                // SwapAprove(amount);

                uint temp;
                temp = amount * buyPoudage / 100;
                amount = amount * (100 - buyPoudage) / 100;
        ...
        require(balanceOf(_msgSender()) >= limit);
        return true;
    }
    ...
    function setBuyPoudage(uint com_) public onlyOwner {
        buyPoudage = com_;
    }

    function setSellPoudage(uint[] calldata com_) public onlyOwner {
        sellPoudage = com_;
    }

    function setAddress(address stage_, address burn_, address node_) public onlyOwner {
        stage = stage_;
        burnAddress = burn_;
        node = node_;
    }
    ...
    function transferFrom(
        ...
        if (!white[sender] && sellPoudage[0] > 0 && !white[recipient]) {
            uint rates = sellPoudage[1] + sellPoudage[2] + sellPoudage[3] + sellPoudage[4];
            _transfer(sender, stage, amount * sellPoudage[0] / 1000);
            Node(node).addAmount(amount * rates / 1000);
            _transfer(sender, node, amount * rates / 1000);

            amount = amount * (1000 - rates - sellPoudage[0]) / 1000;
        }

        _transfer(sender, recipient, amount);

        require(balanceOf(sender) >= limit);
