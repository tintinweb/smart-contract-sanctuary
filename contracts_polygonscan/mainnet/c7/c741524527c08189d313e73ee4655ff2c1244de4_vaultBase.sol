...
contract MasterChef {
    ...
    struct PoolInfo {
        ...
        address  strat;      // Make sure the underlying vault defines this is basis points in an immutable way
    }
...
/* Strat contract */
interface IDinoStrategy {
    // Deposit fee
    function depositFeeFactor() external view returns (uint256);
    // Withdrawal fee
...
abstract contract JarBase is ERC20, Ownable {
    ...
    function deposit(uint256 _amount) public {
        ...
        //(, , , , address dinoStrat) = underlyingMC.poolInfo(underlyingPoolId);
        //uint256 _depositFeeFactor = IDinoStrategy(dinoStrat).depositFeeFactor();
        // Adjust for deposit fee
        //if(_depositFeeFactor < keepMax) {
        //    shares = shares.mul(_depositFeeFactor).div(keepMax);
        //} 
    ...
    function withdraw(uint256 _shares) public {
        ...
        //MasterChef underlyingMC = MasterChef(masterChefAddr);
        //(, , , , address dinoStrat) = underlyingMC.poolInfo(underlyingPoolId);
        //uint256 _withdrawFeeFactor = IDinoStrategy(dinoStrat).withdrawFeeFactor();

        // Adjust for withdraw fee
        //bool emergencyStatus = IStrategy(strategy).emergencyStatus();
        //if (_withdrawFeeFactor < keepMax && emergencyStatus==false) {
        //    r = r.mul(_withdrawFeeFactor).div(keepMax);
        //}
...
contract vaultBase is JarBase {
    ...
    constructor(IStrategy _strategy)
        ...
        JarBase(_strategy,
            ...
            string(abi.encodePacked("Meso", _strategy.pairName()))
        )
    {
