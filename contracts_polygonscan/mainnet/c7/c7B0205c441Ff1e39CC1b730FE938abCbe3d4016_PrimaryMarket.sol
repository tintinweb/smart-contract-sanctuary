...
contract PrimaryMarket is IPrimaryMarket, ReentrancyGuard, ITrancheIndex, Ownable {
    ...
    receive() external payable {
        // make sure only receive ETH from Wrapped ETH
        assert(msg.sender == fund.tokenUnderlying());
    }
    function createWETH(uint256 underlying) public payable {
        IERC20(fund.tokenUnderlying()).safeTransferFrom(msg.sender, address(this), underlying);
        if (msg.value > 0) {
            IWETH(fund.tokenUnderlying()).deposit{value: msg.value}();
            underlying = underlying.add(msg.value);
        }
        _create(underlying);
    }
    function create(uint256 underlying) public {
        IERC20(fund.tokenUnderlying()).safeTransferFrom(msg.sender, address(this), underlying);
        _create(underlying);
    }

    function _create(uint256 underlying) internal nonReentrant onlyActive {
    ...
    function claimWETH(address account)
        external
        virtual
        returns (uint256 createdShares, uint256 redeemedUnderlying)
    {
        return _claim(account, false);
    }
    ...
    function _claim(address account, bool recvWETH)
        internal
        nonReentrant
        returns (uint256 createdShares, uint256 redeemedUnderlying)
    {
        ...
        if (redeemedUnderlying > 0) {
            if (recvWETH) {
                ...
                IWETH(fund.tokenUnderlying()).withdraw(redeemedUnderlying);
                safeTransferETH(account, redeemedUnderlying);
            }
