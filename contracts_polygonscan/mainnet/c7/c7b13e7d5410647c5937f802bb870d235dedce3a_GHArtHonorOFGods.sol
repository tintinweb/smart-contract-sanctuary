...
contract GHArtHonorOFGods is ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;
    using Strings for uint256;

    address payable private _PaymentAddress;

    uint256 public GHOG_MAX = 100000;
    uint256 public PRESALE_PRICE = 22.5 ether;
    uint256 public PUBLIC_PRICE = 25 ether;
    uint256 private REVEAL_DELAY = 0 hours;
    uint256 private PRESALE_HOURS = 24 hours;
    uint256 public PURCHASE_LIMIT = 20;
    uint256 public PRESALE_MINT_LIMIT = 20;
    uint256 public PUBLIC_MINT_LIMIT = GHOG_MAX;
    ...
    uint256 private _activeDateTime = 1643403600; //  (GMT): Friday, January 28, 2022 9:00:00 PM
    uint256 private _revealDateTime = 1643490000; //  (GMT): Saturday, January 29, 2022 9:00:00 PM
    
    string private _tokenBaseURI = "";
    string private _revealURI = "";
    
    Counters.Counter private _publicGHOG;

    constructor() ERC721("GHArt-Honor Of Gods", "GHOG") {}
    ...
    function airdrop(address to, uint256 numberOfTokens) external onlyOwner {
        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = _publicGHOG.current();

            if (_publicGHOG.current() < GHOG_MAX) {
                _publicGHOG.increment();
                _safeMint(to, tokenId);
    ...
    function purchase(uint256 numberOfTokens) external payable {
        ...
        require(_publicGHOG.current() < GHOG_MAX,"Purchase would exceed GHOG_MAX");
