...
contract PryzxPryzPool is Initializable, OwnableUpgradeable {
    ...
    function initialize(IERC20 _reward, IERC20 _stakeToken) internal initializer {
        ...
        accRewardPerShare = 0;
        rewardAPY = 10000; // 10% mul 1000
        lastRewardTime = block.timestamp;
    }


    function updateRewardAPY(uint256 _newRewardAPYTime1000) external onlyOwner {
        updatePool();
        rewardAPY = _newRewardAPYTime1000;
    }
        ...
        returns (uint256)
    {
        ...
        if (block.timestamp > lastRewardTime && totalStaking != 0) {
            uint256 currentRewardPerSecond = rewardAPY.mul(totalStaking).div(100000).div(365 days);
