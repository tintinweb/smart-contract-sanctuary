...
contract MarketGeneration is TokensRecoverable, IMarketGeneration
{
    ...
    function activate(IMarketDistribution _marketDistribution, uint mgeLengthInSeconds) public ownerOnly()
    {
        require (!isActive && block.timestamp >= refundsAllowedUntil, "Already activated");        
    ...
    function activateDeactivateDateLimit() public ownerOnly()
    {
        dateLimitActive = !dateLimitActive;
    }
    function setMGELength(uint timeInSeconds) public ownerOnly()
    {
        require (isActive);
