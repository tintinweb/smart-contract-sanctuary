...
contract DEUS_MATIC_ZAPPER is Ownable {
	...
	function zapInNativecoin(
		...
		IWMatic(maticAddress).deposit{value: msg.value - amountToDeus}();
	...
	function zapInERC20(
		...
		IERC20(toMaticPath[0]).safeTransferFrom(msg.sender, address(this), proxyInput.amountIn);
		...
		emit ZappedIn(toMaticPath[0], pairAddress, proxyInput.amountIn, LPBought, transferResidual);
	}
