...
contract SafeOracleToUSDC is OwnableUpgradeSafe, ISafeOracle {
    ...
    address public collateralOracle; // collateral to USDC
    address public pair;
    uint256 public minSafeAssetPrice;
    ...
    event MinSafeAssetPriceUpdated(uint256 value);
    ...
    function initialize(
        ...
        pair = _pair;
        minSafeAssetPrice = _minSafeAssetPrice;
    ...
    function setPair(address _pair) external onlyOwner {
        address _token0 = IUniswapV2Pair(_pair).token0();
        address _token1 = IUniswapV2Pair(_pair).token1();
        if (safeAsset == _token0) {
            require(collateral == _token1, "wrong pair");
        } else {
            require(collateral == _token0 && safeAsset == _token1, "wrong pair");
        }
        pair = _pair;
    ...
    function setCollateralOracle(address _collateralOracle) external onlyOwner {
        require(ISafeOracle(_collateralOracle).consult() >= 1, "invalid oracle");
        collateralOracle = _collateralOracle;
    }
    ...
    function setMinPrice(uint256 _minSafeAssetPrice) external onlyOwner {
        minSafeAssetPrice = _minSafeAssetPrice;
        emit MinSafeAssetPriceUpdated(_minSafeAssetPrice);
    }
    ...
    function consult() external view override returns (uint256 _price) {
        _price = consultTrue();
        if (_price < minSafeAssetPrice) {
            _price = minSafeAssetPrice;
        }
    }

    function consultTrue() public view override returns (uint256 _price) {
        (uint256 _safeAssetRes, uint256 _collateralRes) = getReserves(safeAsset, collateral, pair);
        if (_collateralRes < minReserve) return 1;
        uint256 _collateralResFull = _collateralRes.mul(10**missing_decimals);
        _price = _collateralResFull.mul(getCollateralPrice()).div(_safeAssetRes);
    }

    function getCollateralPrice() public view returns (uint256 _price) {
        return ISafeOracle(collateralOracle).consult();
    }
