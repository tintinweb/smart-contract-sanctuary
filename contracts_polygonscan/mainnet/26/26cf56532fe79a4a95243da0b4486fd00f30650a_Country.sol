...
contract Country is ERC721Enumerable, ReentrancyGuard, Ownable {
    
    string private _tokenBaseURI = "https://nft.zo.xyz/";
    ...
    modifier onlyAllowlisted() {
        require(_mintPerAddress[msg.sender] < allowMintPerAddress[msg.sender], "You have reached your minting limit.");
        _;
    }
    

    function mintPublic() public onlyAllowlisted {
    ...
    function ownerClaim(uint256 tokenId) public nonReentrant onlyOwner {
        require(tokenId > 5555 && tokenId < MAX_MINT + 1, "Token ID invalid");
        _safeMint(owner(), tokenId);
    }
    
    function isAllowedToMint(address addr) public view returns(uint256) {
        return (allowMintPerAddress[addr] - _mintPerAddress[msg.sender]);
    }
    
    function addAddressToAllowList(address addr) onlyOwner public {
        if (allowMintPerAddress[addr] < MAX_MINT_PER_ADDRESS){
            allowMintPerAddress[addr] += 1;
            emit AllowListAddressAdded(addr);
        }
    }
    
    function addAddressesToAllowList(address[] memory addrs) onlyOwner public {
        for (uint256 i = 0; i < addrs.length; i++) {
            addAddressToAllowList(addrs[i]);
        }
    }
    
    function removeAddressFromAllowList(address addr) onlyOwner public {
        if (allowMintPerAddress[addr] > _mintPerAddress[msg.sender]){
            allowMintPerAddress[addr] -= 1;
            emit AllowListAddressAdded(addr);
        }
    }
    
    function removeAddressesFromAllowList(address[] memory addrs) onlyOwner public {
        for (uint256 i = 0; i < addrs.length; i++) {
    ...
    constructor() ERC721("Country", "Country") Ownable() {}
}
