...
contract TheGarden is Ownable {
    ...
    uint public constant tokenNumber = 3;

    // Info of each user.
    struct UserInfo {
        uint256 amount;     // How many LP tokens the user has provided.
        uint256[tokenNumber] rewardDebtTX;
    }
    ...
    struct PoolInfo {
        ...
        IBEP20[tokenNumber] tokenList;
        // Accumulate reward per token
        uint256[tokenNumber] accRewardXPerShare;
        // Reward tokens distributed per block.
        uint256[tokenNumber] rewardPerBlockTX;
        // Reward tokens name
        string[tokenNumber] rewardTokenSYMBOL;
    }    
    ...
    function pendingRewardAll(address _user) external view returns (uint256[tokenNumber] memory) {
        //PoolInfo storage pool = poolInfo;
        UserInfo storage user = userInfo[_user];

        uint256[tokenNumber] memory pendingXReward;
        
        uint256 length = pendingXReward.length;
