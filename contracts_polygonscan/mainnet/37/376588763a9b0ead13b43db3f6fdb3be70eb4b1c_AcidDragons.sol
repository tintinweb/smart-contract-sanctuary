...
contract AcidDragons is ERC721Enumerable, Ownable {
    ...
    uint256 public constant DRAGON_PUBLIC = 4000;
    uint256 public constant DRAGON_MAX = DRAGON_PUBLIC;
    ...
    Counters.Counter private _publicDRAGON;

    constructor() ERC721("Acid Dragons", "DRAGON") {}
        ...
        onlyOwner
    {
        ...
        require(
            _publicDRAGON.current() < 4000,
            "Purchase would exceed DRAGON_PUBLIC"
        );
        

        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = _publicDRAGON.current();

            if (_publicDRAGON.current() < DRAGON_PUBLIC) {
                _publicDRAGON.increment();
                _safeMint(to, tokenId);
            }
        }
    }
    ...
    function purchase(uint256 numberOfTokens) external payable {
        ...
        require(
            ...
            'Each address may only have 3 DRAGONS'
        );
        require(
            numberOfTokens <= PURCHASE_LIMIT,
            "Can only mint up to 1 token at a time"
        );
        require(
            _publicDRAGON.current() < DRAGON_PUBLIC,
            "Purchase would exceed DRAGON_PUBLIC"
        );
        require(
