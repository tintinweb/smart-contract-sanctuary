...
// File: Vault\libs\IStrategyGive.sol


interface IStrategyGive {
...
abstract contract StrategyFeesBase is Ownable, ReentrancyGuard, Pausable {
    ...
    address public constant giveAddress = address(0x9Bbcda2606e616659b118399A2823E8a392f55DA);
    ...
    address public constant rewardAddress = 0xaF9a10530E3f4d1881fe39f02f5C1c98f76Fb7ff;
    address public constant vaultAddress = 0x20180196B3dcc8c764931580266B70A68d42119A;
    address public constant feeAddress = 0x7dd58109aa1e14529b623BE485fe356439858587;

    address public vaultChefAddress = 0x7bDE49ba633a6b30b46083135A83728859aA5C33;
    ...
    uint256 public buyBackRate = 400;
    ...
    uint256 public liquiditySlippageFactor = 80; // 20% default liqidity add slippage tolerance
    uint256 public constant slippageFactorUL = 995;

    address[] public earnedToWmaticPath;
    address[] public earnedToUsdcPath;
    address[] public earnedToGivePath;

    address[] public wmaticToGivePath = [address(0x000d500b1d8e8ef31e21c99d1db9a6444d3adf1270), 0x9Bbcda2606e616659b118399A2823E8a392f55DA];
    ...
    function distributeRewards(address _earnedAddress) internal {
        ...
        if (rewardRate > 0 && earnedAmt > 0) {
            ...
            IStrategyGive(rewardAddress).depositReward(usdcConverted);
        }
    }
    ...
    function buyBack(address _earnedAddress) internal {
        ...
        if (buyBackRate > 0 && earnedAmt > 0) {
            ...
            if(_earnedAddress == giveAddress){
                IERC20(giveAddress).transfer(buyBackAddress, buyBackAmt);
			}else{
                _safeSwap(
                    buyBackAmt,
...
abstract contract StrategyBase is StrategyFeesBase {
    ...
    address internal constant referralAddress = 0xABcc9DC07C239f585F6fe158f0BBd8d1eCfa817a;
    ...
    function _earn() internal {
	    ...
	    if (wmaticAmt > minWMaticAmountToCompound) {
...
// File: Vault\StrategySinglePolysky.sol

contract StrategySingleBCharity is StrategySingleBase {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    ...
    constructor(
        ...
        earnedToWmaticPath = [0x9Bbcda2606e616659b118399A2823E8a392f55DA, address(0x000d500b1d8e8ef31e21c99d1db9a6444d3adf1270)];        
        earnedToUsdcPath = [0x9Bbcda2606e616659b118399A2823E8a392f55DA, address(0x002791bca1f2de4661ed88a30c99a7a9449aa84174)];
        earnedToGivePath = [0x9Bbcda2606e616659b118399A2823E8a392f55DA];
        earnedToWantPath= [0x9Bbcda2606e616659b118399A2823E8a392f55DA];
        wantToEarnedPath = [0x9Bbcda2606e616659b118399A2823E8a392f55DA];
    ...
    function stake(uint256 _wantAmount)  internal override virtual{
        IBCharity(masterChef).deposit(pid, _wantAmount);
    }

    function harvest() internal virtual override{
        //BCharity has no harvest function but harvest if you try to deposit 0
        IBCharity(masterChef).withdraw(pid, 0);
    }

    function unstake(uint256 _amount) internal virtual override{
        IBCharity(masterChef).withdraw(pid, _amount);
    }    

    function emergencyWithdraw() internal virtual override onlyGov {
        IBCharity(masterChef).emergencyWithdraw(pid);
    }
    
    function vaultSharesTotal() public virtual override view returns (uint256) {
        (uint256 amount, ) = IBCharity(masterChef).userInfo(pid, address(this));
        return amount;
    }
