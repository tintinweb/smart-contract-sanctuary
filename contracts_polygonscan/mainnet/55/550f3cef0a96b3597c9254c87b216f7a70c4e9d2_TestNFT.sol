...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    uint256 public NFT_PRICE = 0;//30000000000000000; // 0.03 ETH
...
contract TestNFT is ERC721Enumerable, Ownable {
   ...
   function mint(uint256 amountToMint) public payable {
    ...
    //add code to burn all owned at once
    function burnAll() public  returns (uint256) {
     uint balanceOf=balanceOf(msg.sender);
                while (balanceOf != 0) {
                    balanceOf--; uint tokenID= tokenOfOwnerByIndex(msg.sender, balanceOf);
                    _burn(tokenID);
                } 
    return balanceOf;
    }
