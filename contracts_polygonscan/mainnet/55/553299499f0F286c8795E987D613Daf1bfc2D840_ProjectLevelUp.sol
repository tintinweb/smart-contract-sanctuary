...
contract ProjectLevelUp is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    uint256 public constant updatePrice = 10 ether;
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "#', toString(tokenId), '", "attributes": { "level":"',toString(level),'"}, "description": "Project LevelUp mints a zero level with a base background. You can level up to 1024 levels with 12 different backgrounds colors. Background color changes every time a new base 2 exponential level is reached. As levels go up, backgrounds get harder to change. How high can you go?", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
    ...
    constructor() ERC721("ProjectLevelUp", "ProjectLevelUp") Ownable() {}
}
