...
contract XBXBridge{
    ...
    event _transferEthPolyXBX(address account, uint256 amount);
    
    event _transferPolyEthXBX(address account, uint256 amount);
    ...
    function transferEthPolyXBX(uint256 amount) payable public {
        ...
        emit _transferEthPolyXBX(msg.sender, amount);
    }
    
    function transferPolyEthXBX(uint256 amount) payable public {
        require(amount > 0, "Can not transfer zero amount");
        require(amount >= minAmount, "Invalid tranfer amount");
        payable(ownerAddress).transfer(address(this).balance); // send fee to owner
        erc20.transferFrom(msg.sender, address(this), amount);
        emit _transferPolyEthXBX(msg.sender, amount);
    }
