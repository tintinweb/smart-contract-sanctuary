    ...
    ERC721EnumerableUpgradeable
{
    ...
    bool public enabled; // Allows minting
    bool public transferEnabled; // Allows for transfering
    ...
    function mint(address staker, uint256 id) external override onlyExternalCaller {
        require(enabled, 'STAKE TOKEN: Contract is disabled');
        _safeMint(staker, id);
    }
    ...
    function transferFrom(
        ...
        address to,
        uint256 tokenId
    ) public virtual override(ERC721Upgradeable, IERC721Upgradeable) onlyMigrator whenNotPaused {
        require(transferEnabled, 'STAKE TOKEN: transfer is disabled.');
        ...
        contracts.ventureCapital.transferSharesAndRebalance(from, to, stake.shares);
    }
    ...
    /** Setters  ------------------------------------------------------------------------ */
    /** @dev setEnabled - when disabled minting tokens will be disallowed
        @param _enabled {bool}
     */
    ...
    /** @dev setEnabled - when disabled transfering will be disabled
        @param _enabled {bool}
     */
    function setTransferEnabled(bool _enabled) external onlyManager {
        ...
        returns (string memory)
    {
        ...
        StakeUri memory stake = contracts.stakeManager.getStakeUri(tokenId);
        return
            Base64.encode(
                abi.encodePacked(
                    '{ "image": "',
    ...
    function getAttributes(StakeUri memory stake) internal pure returns (string memory) {
        return
            string(
                abi.encodePacked(
                    '"attributes": [{ "trait_type": "shares", "value": "',
                    uint256((stake.shares / uint64(1e6))).toString(),
                    '"}, { "trait_type": "amount", "value": "',
                    uint256((stake.amount / uint64(1e6))).toString(),
                    '"}, { "trait_type": "stake days", "value": "',
                    uint256(stake.stakingDays).toString(),
                    '"}, { "trait_type": "payout", "value": "',
                    uint256(stake.payout).toString(),
                    '"}, { "trait_type": "withdrawn", "value": "',
                    stake.status == StakeStatus.Withdrawn ? 'Yes' : 'no',
                    '"}]'
                )
            );
    }
    ...
    function initialize(
        ...
        transferEnabled = false; // Initially disabled
