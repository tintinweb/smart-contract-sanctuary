...
contract StratManager is Ownable, Pausable {
     ...
     * {BIFIMaxiVault} - Address of BIFI Maxi vault.
     * {mooRewardPool} - Address of the reward pool for moo BIFI rewards.
     */
    ...
    address public BIFIMaxiVault;
    address public mooRewardPool;
    ...
    constructor(
        ...
        address _BIFIMaxiVault,
        address _mooRewardPool
    ) public {
        keeper = _keeper;
        ...
        BIFIMaxiVault = _BIFIMaxiVault;
        mooRewardPool = _mooRewardPool;
    }
    ...
    /**
     * @dev Updates BIFI Maxi vault.
     * @param _BIFIMaxiVault new BIFI Maxi vault address.
     */
    function setBIFIMaxiVault(address _BIFIMaxiVault) external onlyOwner {
        BIFIMaxiVault = _BIFIMaxiVault;
    }
    
    /**
     * @dev Updates moo BIFI reward pool.
     * @param _mooRewardPool new moo BIFI reward pool address.
     */
    function setMooRewardPool(address _mooRewardPool) external onlyOwner {
        mooRewardPool = _mooRewardPool;
    }
...
contract StrategyMaxiSeriesRewardPool is StratManager, FeeManager {
    ...
    address constant public bifi = address(0xFbdd194376de19a88118e84E279b977f165d01b8);
    address public want;
    ...
    address[] public outputToBifiRoute = [output, eth, bifi];
    ...
    event DepositedToBIFIMaxi();
    event SentMooRewardsToPool();
    ...
    constructor(
        ...
        address _BIFIMaxiVault,
        address _mooRewardPool
    ) StratManager(_keeper, _strategist, _unirouter, _vault, _beefyFeeRecipient, _BIFIMaxiVault, _mooRewardPool) public {
        want = _want;
    ...
    // update reward amount in moo BIFI reward pool
    modifier updateReward(address account) {
        IRewardPool(mooRewardPool).updateReward(account);
        _;
    }

    // puts the funds to work
    function deposit() public whenNotPaused updateReward(msg.sender){
    ...
    function withdraw(uint256 _amount) external updateReward(msg.sender){
        ...
        IRewardPool(mooRewardPool).getReward();
    }
    ...
    function swapRewards() internal {
        ...
        IUniswapRouterETH(unirouter).swapExactTokensForTokens(toBIFI, 0, outputToBifiRoute, address(this), now);
        emit RewardsSwapped();
    }
    
    // put the BIFI tokens to work
    function depositToBIFI() public {
        uint256 bifiBal = IERC20(bifi).balanceOf(address(this));

        if (bifiBal > 0) {
            IRewardPool(BIFIMaxiVault).deposit(bifiBal);
        }
        emit DepositedToBIFIMaxi();
    }
    
    // send moo BIFI tokens to reward pool for collection
    function sendMooRewardsToPool() internal {
        reward = IERC20(BIFIMaxiVault).balanceOf(address(this));
        updateRewardPerToken();
        IERC20(BIFIMaxiVault).safeTransfer(mooRewardPool, reward);
        emit SentMooRewardsToPool();
    }
    
    // take a snapshot of moo BIFI per share at time of harvest
    function updateRewardPerToken() internal {
        rewardPerToken = rewardPerToken.add(reward.div(IERC20(vault).totalSupply()));
    }
    ...
    function _giveAllowances() internal {
        ...
        IERC20(bifi).safeApprove(BIFIMaxiVault, uint256(-1));
    }

    function _removeAllowances() internal {
        IERC20(want).safeApprove(rewardPool, 0);
        IERC20(output).safeApprove(unirouter, 0);
        IERC20(bifi).safeApprove(BIFIMaxiVault, 0);
    }
}