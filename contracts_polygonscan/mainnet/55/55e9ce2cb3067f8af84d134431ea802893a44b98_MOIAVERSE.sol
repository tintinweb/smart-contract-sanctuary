  ...
  NFT_Types
{
  ...
  function setERC20_TOKEN_ADDRESS(address _tokenAddress) public onlyOwner {
  ...
  function push(
    ...
    require(!_exists[totalSupply()], 'Token Exists');
    check_nftType(_type);
    mint_internal(msg.sender, reciever, totalSupply(), supply);
    setTokenUri(totalSupply(), uri);
    setSupplyFixedStatus(totalSupply(), isSupplyFixed);
    _exists[totalSupply()] = true;
    NFT_Type[totalSupply()] = _type;
    setRoyalties(totalSupply(), reciever, royalties_percentage);
    NIDUS_TOKEN(address(ERC20_TOKEN_ADDRESS)).mint(reciever, 1 * 10 ** 18);
    incrementSupply();
  }
