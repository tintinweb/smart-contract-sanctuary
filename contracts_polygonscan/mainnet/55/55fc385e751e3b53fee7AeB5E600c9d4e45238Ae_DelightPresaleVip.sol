...
contract DelightPresaleVip is Ownable, ReentrancyGuard {
    ...
    //SETUP
    uint256 public constant delightPresaleSize = 50 * (10 ** 3); // 100,000 sandman x 50,000 delight
    uint256 public delightSaleVipPriceE35  = 5 * (10 ** 34); // 1 sandman x 0,5 deligth
    uint256 public delightRemaining = delightPresaleSize;
    ...
    function sendUnclaimedDelightToDeadAddress() external onlyOwner {
        require(block.number > endBlock, "can only send excess delight to dead address after presale has ended");
