...
// File: contracts/BIFI/interfaces/aave/IIncentivesController.sol
...
interface IIncentivesController {
    ...
    function enterMarkets(address[] memory RTokens) external returns (uint[] memory);
}

// File: contracts/BIFI/interfaces/aave/ILendingPool.sol
...
// File: contracts/BIFI/strategies/Aave/StrategyAave.sol
...
contract StrategyIronLend is StratManager, FeeManager {
    ...
    // Routes
    address[] public iceToWantRoute;
    address[] public iceToMaticRoute = [ice, eth, matic];
    ...
    constructor(
        ...
        if (want == eth) {
            iceToWantRoute = [ice, eth];
        } else if (want != ice) {
            iceToWantRoute = [ice, eth, want];
        }
        
        address[] memory markets = new address[](1);
        markets[0] = lendingPool;
        
        // allow asset to be used as collateral
        IIncentivesController(incentivesController).enterMarkets(markets);
    ...
    function _leverage(uint256 _amount) internal {
        ...
        for (uint i = 0; i < borrowDepth; i++) {
            ILendingPool(lendingPool).mint(_amount);
            _amount = _amount.mul(borrowRate).div(100);
            ILendingPool(lendingPool).borrow(_amount);
        }
    ...
    function _deleverage() internal {
        ...
        (uint256 supplyBal, uint256 borrowBal, uint256 exchangeRate) = userReserves();

        while (wantBal < borrowBal) {
            ILendingPool(lendingPool).repayBorrow(wantBal);

            (supplyBal, borrowBal, exchangeRate) = userReserves();
            uint256 targetSupply = borrowBal.mul(100).div(borrowRate);
            uint256 redeemAmount = supplyBal.sub(targetSupply).div(exchangeRate);

            ILendingPool(lendingPool).redeemUnderlying(redeemAmount);
            wantBal = IERC20(want).balanceOf(address(this));
        }
        
        (supplyBal, borrowBal, exchangeRate) = userReserves();
        ILendingPool(lendingPool).repayBorrow(borrowBal);
        ILendingPool(lendingPool).redeemUnderlying(supplyBal.div(exchangeRate));
    ...
    function harvest() external whenNotPaused onlyEOA {
        IIncentivesController(incentivesController).claimReward(address(this));
    ...
    function chargeFees() internal {
        uint256 toMatic = IERC20(ice).balanceOf(address(this)).mul(45).div(1000);
        IUniswapRouterETH(unirouter).swapExactTokensForTokens(toMatic, 0, iceToMaticRoute, address(this), now);
    ...
    function swapRewards() internal {
        ...
        IUniswapRouterETH(unirouter).swapExactTokensForTokens(iceBal, 0, iceToWantRoute, address(this), now);
    }
    ...
    function userReserves() public view returns (uint256, uint256, uint256) {
        (, uint256 rTokenBal, uint256 borrowBal, uint256 exchangeRate) = ILendingPool(lendingPool).getAccountSnapshot(address(this));
        uint256 supplyBal = rTokenBal.mul(exchangeRate);
        return (supplyBal, borrowBal, exchangeRate);
    }
    ...
    function _giveAllowances() internal {
        ...
        IERC20(ice).safeApprove(unirouter, uint256(-1));
    }
