...
contract MEGAMine is Ownable, Pausable {
    ...
    struct UserInfo {
        ...
        uint256 MEGAAtLastUserAction; // keeps track of crystl deposited at the last user action
    ...
    function deposit(uint256 _amount) external whenNotPaused notContract {
        ...
        user.MEGAAtLastUserAction = user.shares.mul(balanceOf()).div(totalShares);
    ...
    function harvest() external notContract whenNotPaused {
        IMasterHealer(masterhealer).withdraw(0, 0);
    ...
    function emergencyWithdraw() external onlyAdmin {
        IMasterHealer(masterhealer).emergencyWithdraw(0);
    }
     ...
     * @return Expected reward to collect in MEGA
     */
    function calculateHarvestMEGARewards() external view returns (uint256) {
        uint256 amount = IMasterHealer(masterhealer).pendingMEGA(0, address(this));
     ...
     */
    function calculateTotalPendingMEGARewards() external view returns (uint256) {
        uint256 amount = IMasterHealer(masterhealer).pendingMEGA(0, address(this));
        amount = amount.add(available());
    ...
    function withdraw(uint256 _shares) public notContract {
        ...
        if (bal < currentAmount) {
            ...
            IMasterHealer(masterhealer).withdraw(0, balWithdraw);
    ...
    function balanceOf() public view returns (uint256) {
        (uint256 amount, ) = IMasterHealer(masterhealer).userInfo(0, address(this));
    ...
    function _earn() internal {
        uint256 bal = available();
        if (bal > 0) {
            IMasterHealer(masterhealer).deposit(0, bal);
        }
    }
