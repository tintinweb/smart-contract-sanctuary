...
abstract contract BaseStrategy is Ownable {
    ...
    // Dex
    address public constant currentRouter = 0x3a1D87f206D12415f5b0A33E786967680AAb4f6d; //Wault router
...
abstract contract BaseStrategyMasterChef is BaseStrategy {
    ...
    function getHarvestable() external override view returns (uint256) {
        return IMasterChef(rewards).pendingWex(poolId, address(this));
    }
    ...
    function deposit() public override {
        ...
        if (_want > 0) {
            ...
            IMasterChef(rewards).deposit(poolId, _want, false);
        }
    }
    ...
    function _withdrawSome(uint256 _amount)
        ...
        returns (uint256)
    {
        IMasterChef(rewards).withdraw(poolId, _amount, false);
        return _amount;
    }
    ...
    function _getReward() internal {
        IMasterChef(rewards).withdraw(poolId, 0, true);
...
abstract contract StrategyFarmSingle is BaseStrategyMasterChef {
    ...
    address public constant rewardToken = 0x4c4BF319237D98a30A929A96112EfFa8DA3510EB;
    ...
    address public constant wexpoly = rewardToken;
    address public constant usdc = 0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174;
    address public constant wusd = 0xb8ab048D6744a276b2772dC81e406a4b769A5c3D;

    address[] public wexpoly_usdc_path;
    address[] public usdc_wusd_path;

    // Other constants
    uint16 public constant underlyingPoolId = 28;
    ...
    {
        wexpoly_usdc_path = new address[](2);
        wexpoly_usdc_path[0] = wexpoly;
        wexpoly_usdc_path[1] = usdc;

        usdc_wusd_path = new address[](2);
        usdc_wusd_path[0] = usdc;
        usdc_wusd_path[1] = wusd;
    }
    ...
    function harvest() public override {
        ...
        if (_harvestBalance > harvestCutoff){
            // Swap WexPoly to USDC
            _swapUniswapWithPath(wexpoly_usdc_path, _harvestBalance);
            ...
            // Swap 1/2 USDC to WUSD
            uint256 _usdcBalancePost = IERC20(usdc).balanceOf(address(this));
            _swapUniswapWithPath(usdc_wusd_path, _usdcBalancePost.div(2));
            
            // Add liquidity
            uint256 _usdc = IERC20(usdc).balanceOf(address(this));
            uint256 _wusd = IERC20(wusd).balanceOf(address(this));
            if (_usdc > 0 && _wusd > 0) {
                IERC20(usdc).safeApprove(currentRouter, 0);
                IERC20(usdc).safeApprove(currentRouter, _usdc);
                IERC20(wusd).safeApprove(currentRouter, 0);
                IERC20(wusd).safeApprove(currentRouter, _wusd);
...
contract StrategySingle is StrategyFarmSingle {
    // Token addresses
    address public WAULT_MASTER_CHEF = 0xC8Bd86E5a132Ac0bf10134e270De06A8Ba317BFe;
    address public Want_Token = 0x7B93196CD4861f345509c2da31058CA4B1c30D63;
