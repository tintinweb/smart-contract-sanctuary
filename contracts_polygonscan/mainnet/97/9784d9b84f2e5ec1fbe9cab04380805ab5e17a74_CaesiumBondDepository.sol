...
// File: contracts/interfaces/ICaesiumBondingCalculator.sol


pragma solidity 0.7.5;

interface ICaesiumBondingCalculator {
...
// File: contracts/interfaces/ICaesiumTreasury.sol


pragma solidity 0.7.5;

interface ICaesiumTreasury {
...
// File: contracts/CaesiumBondDepository.sol
...
contract CaesiumBondDepository is Ownable {
    ...
    /* ======== STATE VARIABLES ======== */
    address public immutable CAES; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints CAES when receives principle
    ...
    struct Bond {
        uint payout; // CAES remaining to be paid
    ...
    function deposit(
        ...
        uint value = ICaesiumTreasury( treasury ).valueOfToken( principle, _amount );
        uint payout = payoutFor( value ); // payout to bonder is computed

        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 CAES ( underflow protection )
        ...
        /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) CAES
         */
        IERC20( principle ).safeTransferFrom( msg.sender, address(this), _amount );
        IERC20( principle ).approve( address( treasury ), _amount );
        ICaesiumTreasury( treasury ).deposit( _amount, principle, profit );

        if ( fee != 0 ) { // fee is transferred to dao
            IERC20( CAES ).safeTransfer( DAO, fee );
        }
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            IERC20( CAES ).transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                IERC20( CAES ).approve( stakingHelper, _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return IERC20( CAES ).totalSupply().mul( terms.maxPayout ).div( 100000 );
    }
    ...
    function bondPriceInUSD() public view returns ( uint price_ ) {
        if( isLiquidityBond ) {
            price_ = bondPrice().mul( ICaesiumBondingCalculator( bondCalculator ).markdown( principle ) ).div( 100 );
        } else {
    ...
    /**
     *  @notice calculate current ratio of debt to CAES supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns ( uint debtRatio_ ) {
        uint supply = IERC20( CAES ).totalSupply();
    ...
    function standardizedDebtRatio() external view returns ( uint ) {
        if ( isLiquidityBond ) {
            return debtRatio().mul( ICaesiumBondingCalculator( bondCalculator ).markdown( principle ) ).div( 1e9 );
        } else {
            return debtRatio();
        }
    }
    ...
    /**
     *  @notice calculate amount of CAES available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or CAES) to the DAO
     *  @return bool
     */
