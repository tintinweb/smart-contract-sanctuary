...
contract MustManager is Modulable {
    IERC20 public must;

    constructor(address newMust) public {
        must = IERC20(newMust);
    }

    function transferFrom(address from, address to, uint256 amount) onlyModule external {
        must.transferFrom(from, to, amount);
    }
}
...
contract GameItemsManager is Modulable {
    ERC1155Burnable public gameItems;

    constructor(address gItems) public {
        gameItems = ERC1155Burnable(gItems);
    }
    ...
    function balanceOf(address account, uint256 id)
        ...
        returns (uint256)
    {
        return gameItems.balanceOf(account, id);
    }
    ...
    function safeTransferFrom(
        ...
        gameItems.safeTransferFrom(from, to, id, amount, data);
    }
    ...
    function burn(
        ...
        gameItems.burn(account, id, value);
    }
}
...
import "../MustManager.sol";
import "./PortalStore.sol";
import "../solarsystem/SolarSystemStore.sol";

import "../MinerStoreV2.sol";

import "../items/GameItemsManager.sol";

contract PortalManagerV2 is Ownable, Modulable {
    using PositionsV2 for *;
    using EnumerableSet for EnumerableSet.UintSet;

    event Traveled(
        uint256 indexed minerId,
        uint256 indexed fromSystemID,
        uint256 indexed toSystemID,
        uint256 fees,
        uint32 newDistance,
        int128 newAngle,
        int32 rotationSpeed,
    ...
    IPortalStore public portalStore;
    ISolarSystemStore public solarSystemStore;
    MustManager public mustManager;
    GameItemsManager public itemsManager;
    ...
    uint8 public portalTravelArea = 30;

    mapping(uint256 => uint256) public itemRequirementByPortalId;
    mapping(uint256 => bool) public logicalOrByPortalId;
    // cannot be public because of EnumerableSet
    mapping(uint256 => EnumerableSet.UintSet) private modelRequirementByPortalId;

    uint256 constant public PORTAL_SYSTEM2_MASK = 0x1000000;

    constructor(
        address pStore,
        address ssStore,
        address mManager,
        address gameItemsManager,
        address gtw
    ) public {
        updatePortalStore(pStore);
        updateSolarSystemStore(ssStore);
        updateMustManager(mManager);
        updateItemsManager(gameItemsManager);
        updateGateway(gtw);
        feeReceiver = msg.sender;
    }

    function updateSolarSystemStore(address newStore) public onlyOwner {
        solarSystemStore = ISolarSystemStore(newStore);
    }

    function updatePortalStore(address newStore) public onlyOwner {
        portalStore = IPortalStore(newStore);
    }

    function updateItemsManager(address newItemsManager) public onlyOwner {
      itemsManager = GameItemsManager(newItemsManager);
    }
    ...
    function updatePortalTravelArea(uint8 area) external onlyOwner {
        portalTravelArea = area;
    }

    function updateMustManager(address newMustManager) public onlyOwner {
        mustManager = MustManager(newMustManager);
    }

    function updateGateway(address newGateway) public onlyOwner {
    ...
    function removePortal(uint256 portalId) external onlyOwner {
        portalStore.removePortal(portalId);
    }

    function _getMaskPortalId(uint256 portalId, uint256 solarSystemId) private view returns (uint256) {
      Portal memory portal = portalStore.getPortal(portalId);
      require(
        solarSystemId == portal.system1Id || solarSystemId == portal.system2Id,
        "PortalManager: target solar system does not contain the given portal"
      );

      uint256 portalWithMask = portalId;
      if (solarSystemId == portal.system2Id) {
        portalWithMask = portalId | PORTAL_SYSTEM2_MASK;
      }
      return portalWithMask;
    }

    function updateLogicalOrForPortal(uint256 portalId, uint256 solarSystemId, bool isOr) external onlyOwner {
      uint256 portalWithMask = _getMaskPortalId(portalId, solarSystemId);
      logicalOrByPortalId[portalWithMask] = isOr;
    }

    function updateItemRequirementForPortal(uint256 portalId, uint256 solarSystemId, uint256 itemId) external onlyOwner {
      uint256 portalWithMask = _getMaskPortalId(portalId, solarSystemId);
      itemRequirementByPortalId[portalWithMask] = itemId;
    }

    function updateModelRequirementForPortal(
      uint256 portalId, uint256 solarSystemId, uint256[] calldata models, bool[] calldata activated) external onlyOwner {
      require(models.length == activated.length, "parameters lengths are not the same");
      uint256 portalWithMask = _getMaskPortalId(portalId, solarSystemId);

      for (uint i = 0; i < models.length; i++) {
        uint256 modelId = models[i];
        if (activated[i]) {
          modelRequirementByPortalId[portalWithMask].add(modelId);
        } else {
          modelRequirementByPortalId[portalWithMask].remove(modelId);
        }
      }
    }

    function getRequiredModelsForPortal(uint256 portalId, uint256 solarSystemId) external view returns (uint256[] memory models) {
      uint256 portalWithMask = _getMaskPortalId(portalId, solarSystemId);

      EnumerableSet.UintSet storage set = modelRequirementByPortalId[portalWithMask];
      if (set.length() > 0) {
        models = new uint256[](set.length());
    ...
    function getPortal(uint256 portalId) public view returns (Portal memory) {
        return portalStore.getPortal(portalId);
    }

    function getPortals() public view returns (Portal[] memory) {
        return portalStore.getPortals();
    }

    function getMinerLastDestination(uint256 minerId)
        public
        view
        returns (uint256)
    {
        return portalStore.getMinerLastDestination(minerId);
    }

    function getMinersLastDestination(uint256[] memory minerIds)
        public
        view
        returns (uint256[] memory)
    {
        uint256[] memory result = new uint256[](minerIds.length);

        for (uint256 i = 0; i < minerIds.length; i++) {
            result[i] = portalStore.getMinerLastDestination(minerIds[i]);
        }
    ...
    function addPortal(
        ...
        uint256 system2Id,
        uint256 fees,
        uint32 system1DistanceToCenter,
        uint32 system2DistanceToCenter,
        int128 system1Angle,
        int128 system2Angle,
        int32 rotationSpeed
    ) external onlyOwner {
        portalStore.addPortal(
            system1Id,
            system2Id,
            fees,
            system1DistanceToCenter,
            system2DistanceToCenter,
            system1Angle,
            system2Angle,
            rotationSpeed
        );
    ...
    function _travel(
      uint256 minerId,
      uint256 fromSystemId,
      uint256 toSystemId
    ) private {
        IMinerStoreV2 fromMinerStore =
            IMinerStoreV2(solarSystemStore.minerStoreFor(fromSystemId));

        IMinerStoreV2 toMinerStore =
            IMinerStoreV2(solarSystemStore.minerStoreFor(toSystemId));

        uint256 positionSource =
            uint256(
                keccak256(
                    abi.encodePacked(block.number * block.timestamp + minerId)
                )
            );

        PositionsV2.Polar memory lastPolar =
            PositionsV2.Polar({
                angle: PositionsV2.toReal(int88(positionSource % 360)),
                distance: uint32(
                    (positionSource % 2000) + 5 * toMinerStore.length()
                )
            });
        ...
        toMinerStore.add(
            minerId,
            PositionsV2.Orbit({
                center: PositionsV2.Cartesian({x: 0, y: 0}),
                last: lastPolar,
                lastUpdate: block.timestamp,
                rotationSpeed: defaultRotationSpeed
            }),
            fromMinerStore.pullingPrice(minerId)
        );

        fromMinerStore.remove(minerId);

        portalStore.updateMinerLastDestination(minerId, toSystemId);
        Traveled(
            minerId,
            fromSystemId,
            toSystemId,
    ...
    function forceTravel(
      uint256[] calldata minerIds,
      uint256[] calldata toSystemIds
    ) external onlyOwner {
      require(minerIds.length == toSystemIds.length, "parameters not of same length");
      for (uint i = 0; i < minerIds.length; i++) {
        uint256 minerId = minerIds[i];
        uint256 toSystemId = toSystemIds[i];
        uint256 fromSystemId = portalStore.getMinerLastDestination(minerId);

        _travel(minerId, fromSystemId, toSystemId);
      }
    }

    function _checkRequirements(uint256 portalId, uint256 fromSystemId, uint256 minerId) private {
      uint256 portalWithMask = _getMaskPortalId(portalId, fromSystemId);
      bool isLogicalAnd = !logicalOrByPortalId[portalWithMask];
      uint256 itemId = itemRequirementByPortalId[portalWithMask];
      bool hasModelReq = modelRequirementByPortalId[portalWithMask].length() > 0;

      if (hasModelReq || itemId > 0) {
        bool isCorrectModel = modelRequirementByPortalId[portalWithMask].contains(minerId / 1000000);
        if (isLogicalAnd || (!isCorrectModel && itemId > 0)) {
          itemsManager.burn(msg.sender, itemId, 1);
          if (isLogicalAnd && !isCorrectModel) {
            revert("PortalManager: model is not allowed to travel");
          }
    ...
    function travel(
        ...
        require(
            fromMinerStore.contains(minerId),
            "PortalManager: unknown miner"
        );
        ...
        require(
            ...
            "PortalManager: require owner or approved"
        );

        Portal memory portal = portalStore.getPortal(portalId);
        if (fromSystemId == portal.system1Id) {
            require(
                PositionsV2.collision(
                    PositionsV2.cartesianCoordinate(
                        fromMinerStore.orbit(minerId),
                        time
                    ),
                    PositionsV2.cartesianCoordinate(portal.system1Orbit, time),
                    portalTravelArea
                ),
                "PortalManager: Miner not in range"
            );
        } else {
            require(
                PositionsV2.collision(
                    ...
                    PositionsV2.cartesianCoordinate(portal.system2Orbit, time),
                    portalTravelArea
                ),
                "PortalManager: Miner not in range"
            );
        ...
        _checkRequirements(portalId, fromSystemId, minerId);
        mustManager.transferFrom(msg.sender, feeReceiver, portal.fees);
        _travel(minerId, fromSystemId, toSystemId);
    }
}
...
struct Portal {
    ...
    uint256 system2Id;
    uint256 fees;
    PositionsV2.Orbit system1Orbit;
    PositionsV2.Orbit system2Orbit;
}

interface IPortalStore {
    function countPortals() external view returns (uint256);

    function getPortalAtIndex(uint256 index)
        external
        view
        returns (Portal memory);

    function getPortalIdAtIndex(uint256 index) external view returns (uint256);

    function getPortal(uint256 portalId) external view returns (Portal memory);

    function getPortals() external view returns (Portal[] memory);

    function getMinerLastDestination(uint256 minerdId)
        external
        view
        returns (uint256);

    function updateMinerLastDestination(uint256 minerdId, uint256 systemSolarId)
        external;
...
contract PortalStore is IPortalStore, Modulable {
    using EnumerableSet for EnumerableSet.UintSet;
    using Counters for Counters.Counter;
    using PositionsV2 for *;

    mapping(uint256 => Portal) portals;
    EnumerableSet.UintSet portalsIds;
    Counters.Counter private _portalIdTracker;

    mapping(uint256 => uint256) private minerLasDestination;

    constructor() public {}

    function countPortals() public view override returns (uint256) {
        return portalsIds.length();
    }
    ...
    function getPortalAtIndex(uint256 index)
        ...
        returns (Portal memory)
    {
        return portals[portalsIds.at(index)];
    }

    function getPortalIdAtIndex(uint256 index)
        public
        view
        override
        returns (uint256)
    {
        return portalsIds.at(index);
    }

    function getPortal(uint256 portalId)
        public
        view
        override
        returns (Portal memory)
    ...
    function getMinerLastDestination(uint256 minerdId)
        ...
        returns (uint256)
    {
        return minerLasDestination[minerdId];
    }
    ...
    function updateMinerLastDestination(uint256 minerdId, uint256 systemSolarId)
        ...
        onlyModule
    {
        minerLasDestination[minerdId] = systemSolarId;
    }

    function getPortals() public view override returns (Portal[] memory) {
        Portal[] memory result = new Portal[](portalsIds.length());
        for (uint256 i = 0; i < portalsIds.length(); i++) {
            result[i] = portals[portalsIds.at(i)];
        }
        return result;
    }
    ...
    function addPortal(
        ...
        portalsIds.add(_portalIdTracker.current());
        _portalIdTracker.increment();
    }
    ...
    function updatePortal(
        ...
        int128 system2Angle,
        int32 rotationSpeed
    ) public override onlyModule {
        PositionsV2.Orbit memory system1Orbit =
            PositionsV2.Orbit({
                center: PositionsV2.Cartesian({x: 0, y: 0}),
                last: PositionsV2.Polar({
                    angle: system1Angle,
                    distance: system1DistanceToCenter
                }),
                lastUpdate: block.timestamp,
                rotationSpeed: rotationSpeed
            });

        PositionsV2.Orbit memory system2Orbit =
            PositionsV2.Orbit({
                center: PositionsV2.Cartesian({x: 0, y: 0}),
                last: PositionsV2.Polar({
                    angle: system2Angle,
                    distance: system2DistanceToCenter
                }),
                lastUpdate: block.timestamp,
                rotationSpeed: rotationSpeed
            });
        ...
        portals[portalId] = Portal({
            ...
            system2Id: system2Id,
            fees: fees,
            system1Orbit: system1Orbit,
            system2Orbit: system2Orbit
        });
    }

    function removePortal(uint256 portalId) external override onlyModule {
        delete portals[portalId];
        portalsIds.remove(portalId);
    }
}
