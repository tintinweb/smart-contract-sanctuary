...
contract NFTFarming is Ownable {
    ...
    constructor(uint256 _pointsPerSecond, IERC20 _lpToken, uint256 _participationFee, uint256 _withdrawalFee, address _feeAddress) {
    ...
    function _unDebitedPoints(UserInfo memory user) internal view returns (uint256) {
        return block.timestamp.sub(user.lastUpdateAt).mul(pointsPerSecond).mul(user.amount);
    }
