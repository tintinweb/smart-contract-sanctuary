...
contract PancakeswapBigfootConfig is Ownable, BigfootConfig {
  ...
  mapping(address => PriceOracle) public oracle;
  ...
  function setOracle(address _address, PriceOracle _oracle) external onlyOwner {
    oracle[_address] = _oracle;
  }
  ...
  function isStable(address bigfoot) public view returns (bool) {
    ...
    (uint price, uint lastUpdate) = oracle[bigfoot].getPrice(token0, token1);
