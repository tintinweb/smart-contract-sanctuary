...
contract FooToken is IERC20, Ownable {
    ...
    string private _name = "Foo Token";
    ...
    modifier onlyFarm {
        require(msg.sender == fooFarm, "only farm");
        _;
    }
    ...
    function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner {
        require(maxTxPercent <= 4, "Wrong amount");
    ...
    function setFooFarm(address _farmAddress) external onlyOwner {
        require(fooFarm == address(0), "already set");
        fooFarm = _farmAddress;
        excludeFromFee(fooFarm);
        excludeFromReward(fooFarm);
    }
}
...
contract MasterChef is Ownable, ReentrancyGuard {
	...
	uint256 public constant TOKEN_CAP = 2_000_000 ether;
	...
	struct UserInfo {
		...
		uint256 nextHarvestUntil; // When to harvest again
		//
	...
	struct PoolInfo {
			...
			uint256 accFooPerShare;   // Accumulated FOOs per share, times 1e18. See below.
	...
	event UpdateEmissionRate(address indexed user, uint256 fooPerBlock);
	...
	function pendingFoo(uint256 _pid, address _user) external view returns (uint256) {
		...
		if (block.number > pool.lastRewardBlock && lpSupply != 0) {
			...
			accFooPerShare = accFooPerShare.add(fooReward.mul(1e18).div(lpSupply));
		}

		uint256 pending = user.amount.mul(accFooPerShare).div(1e18).sub(user.rewardDebt);
	...
	function updatePool(uint256 _pid) public {
		...
		mintCapped(devaddr, fooReward.div(10));
		mintCapped(address(this), fooReward);

		pool.accFooPerShare = pool.accFooPerShare.add(fooReward.mul(1e18).div(lpSupply));
		pool.lastRewardBlock = block.number;
	}
	...
	function deposit(uint256 _pid, uint256 _amount, address _referrer) external nonReentrant {
		...
		if (pool.harvestInterval > 0) {
			require(block.number >= startBlock, "deposit: pooling not started");
		}
		...
		if (user.amount > 0) {
			uint256 pending = user.amount.mul(pool.accFooPerShare).div(1e18).sub(user.rewardDebt);
		...
		user.rewardDebt = user.amount.mul(pool.accFooPerShare).div(1e18);
	...
	function payReferralCommission(address _user, uint256 _pending) internal {
		if (address(referral) != address(0) && referralCommissionRate > 0) {
			...
			if (referrer != address(0) && commissionAmount > 0) {
				mintCapped(referrer, commissionAmount);
	...
	function mintCapped(address to, uint256 amount) private {
		uint256 totalSupply = foo.totalSupply();

		if(totalSupply.add(amount) > TOKEN_CAP) {
				foo.mint(to, TOKEN_CAP.sub(totalSupply));
		} else {
				foo.mint(to, amount);
		}
	}
	...
	function updateEmissionRate(uint256 _fooPerBlock) public onlyOwner {
		require(_fooPerBlock <= MAX_EMISSION_RATE, "emission: too high");
		massUpdatePools();
