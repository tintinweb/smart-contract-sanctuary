...
/**
 * @title RouterPOLYSushiV1
 * This contract implements the router interface for the Polygon network using SushiSwap for token swaps
 */
contract RouterPOLYSushiV1 is IRouter, MyPausableUpgradeable {
  ...
  // contains the address of the SushiSwap in POLYGON test- and mainnet
  address constant SUSHISWAP_ROUTER_V2 = address(0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506);

  // interface for the SushiSwapV2 router that enables token swaps
  ...
  function initialize() public initializer {
    ...
    sushiSwapRouter = ISushiSwapRouter(SUSHISWAP_ROUTER_V2);
  }
  ...
  function tradeERC20(
    ...
    require(inputAmount > 0, 'RouterPOLYSushi: inputAmount for token swap cannot be 0');
    ...
    if (input.allowance(address(this), SUSHISWAP_ROUTER_V2) < inputAmount) {
      input.approve(SUSHISWAP_ROUTER_V2, type(uint256).max);
    }
    ...
    returns (uint256)
  {
    ...
    // call external DEX and swap tokens
    uint256[] memory amounts = sushiSwapRouter.swapExactETHForTokens{value: inputAmount}(
      0,
      _getPathForSwap(sushiSwapRouter.WETH(), address(output)),
      _msgSender(),
      deadline
    );
   ...
   * @param dexAddress token contract address of the dex router (contract must implement ISushiSwapRouter interface)
   */
  function setDexRouterAddress(address dexAddress) external whenNotPaused {
    require(
      hasRole(MANAGE_CONTRACTS_ROLE, _msgSender()),
      'RouterPOLYSushi: must have MANAGE_CONTRACTS_ROLE to execute this function'
    );
    require(dexAddress != address(0), 'RouterPOLYSushi: invalid dex router address provided');
    sushiSwapRouter = ISushiSwapRouter(dexAddress);
  }
  ...
  function addSwapPath(
    ...
    require(
      ...
      'RouterPOLYSushi: must have MANAGE_ROUTER_PATHS_ROLE role to execute this function'
    );
    require(
      input != address(0) && output != address(0) && path[0] != address(0),
      'RouterPOLYSushi: invalid data provided'
    );
   ...
   */
  function _getPathForSwap(address input, address output) private view returns (address[] memory) {
