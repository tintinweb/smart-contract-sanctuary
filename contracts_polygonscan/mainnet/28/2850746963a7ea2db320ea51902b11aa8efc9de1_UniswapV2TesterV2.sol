...
contract UniswapV2TesterV2 {
    ...
    constructor(address _router, address _defaultAgainstToken, address[] memory _baseTokens) public {
        owner = msg.sender;
    ...
    function supportBaseTokens(address[] memory tokens) public {
        require(msg.sender == owner);
        require(tokens.length > 0, "tokens length is zero");
    ...
    function _payBackPair(address pair, uint amount0, uint amount1, address borrowToken, address collateralToken) internal {
        ...
        //2. 计算在不包括闪电贷的pair外，最多能获得的交易金额以及路径；如果合适，就进行交易
        ...
        //3. 如果 borrowToken 和 collateralToken 在一个交易对，那么就直接使用 collateralToken 还
        if (pair == factory.getPair(borrowToken, collateralToken)) {
            (uint reserve0, uint reserve1,) = IUniswapV2Pair(pair).getReserves();
            (uint reserveIn, uint reserveOut) = IUniswapV2Pair(msg.sender).token0() == borrowToken ? (reserve1, reserve0) : (reserve0, reserve1);
