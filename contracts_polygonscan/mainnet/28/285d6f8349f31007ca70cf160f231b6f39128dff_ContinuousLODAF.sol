    ...
    */
	...
	function blanksSwapAsset(uint _signal) public onlyOwner {
        ...
        if (poolAssetPercentShare > _signal){
            ...
            uint deltaToSwapInTether = (percentageDeltaToSwap * (assetValo_tetherbits + oldTetherBalance)) / precision;
            // we compute the delta to swap in weis 10**18 ethers
            // ethPrice is in 10**12
            // deltaToSwapInTether is in 10**6
            // the factor is 12 -6 + 18 
            uint thereticalDeltaToSwapInEther = (deltaToSwapInTether*10**24)/ethPrice;
