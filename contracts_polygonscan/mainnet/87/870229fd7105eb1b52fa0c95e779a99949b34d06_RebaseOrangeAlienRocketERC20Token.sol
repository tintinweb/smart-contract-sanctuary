...
contract TWAPOracleUpdater is ERC20Permit, VaultOwned {
  ...
  function addTWAPSource( address newTWAPSourceDexPool_ ) external onlyOwner() {
    require( _dexPoolsTWAPSources.add( newTWAPSourceDexPool_ ), "RebaseOrangeAlienRocketERC20TOken: TWAP Source already stored." );
    emit TWAPSourceAdded( newTWAPSourceDexPool_ );
  }

  function removeTWAPSource( address twapSourceToRemove_ ) external onlyOwner() {
    require( _dexPoolsTWAPSources.remove( twapSourceToRemove_ ), "RebaseOrangeAlienRocketERC20TOken: TWAP source not present." );
...
contract RebaseOrangeAlienRocketERC20Token is Divine {

  using SafeMath for uint256;

    constructor() Divine("RebaseOrangeAlienRocket", "ROAR", 9) {
    }
