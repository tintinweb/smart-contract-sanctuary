...
contract XVMCgovernor {
    ...
    address public immutable acPool1 = 0xF9206B2919cF5Ec2d248101fcB472Afe97ddA1b9;
    address public immutable acPool2 = 0x40e69f8de528D51eA09bC20F71Aa2d23A824E55b;
    address public immutable acPool3 = 0x06ac79A00D7a66e4FA03Eac3306F8701Ee39D09d;
    address public immutable acPool4 = 0xA5E949FB0D288aa02bf129C973379A8D88330E37;
    address public immutable acPool5 = 0x9c0D7F04764A05642d45f2E7f746647a555a799C;
    address public immutable acPool6 = 0x0fd735A017Ab8c56d55C4306E851866bc23d5197;
        
    //pool ID in the masterchef for respective Pool address and dummy token
    uint256 public immutable acPool1ID = 52;
    uint256 public immutable acPool2ID = 53;
    uint256 public immutable acPool3ID = 54;
    uint256 public immutable acPool4ID = 55;
    uint256 public immutable acPool5ID = 56;
    uint256 public immutable acPool6ID = 57;
    ...
    constructor(
        ...
        rollBonus[0xF9206B2919cF5Ec2d248101fcB472Afe97ddA1b9].bonusPoints = 50;
        rollBonus[0x40e69f8de528D51eA09bC20F71Aa2d23A824E55b].bonusPoints = 69;
        rollBonus[0x06ac79A00D7a66e4FA03Eac3306F8701Ee39D09d].bonusPoints = 82;
        rollBonus[0xA5E949FB0D288aa02bf129C973379A8D88330E37].bonusPoints = 130;
        rollBonus[0x9c0D7F04764A05642d45f2E7f746647a555a799C].bonusPoints = 150;
        rollBonus[0x0fd735A017Ab8c56d55C4306E851866bc23d5197].bonusPoints = 200;
        // Infinite approve
        IERC20(_token).safeApprove(address(0xF9206B2919cF5Ec2d248101fcB472Afe97ddA1b9), type(uint256).max); //vaults
        IERC20(_token).safeApprove(address(0x40e69f8de528D51eA09bC20F71Aa2d23A824E55b), type(uint256).max);
        IERC20(_token).safeApprove(address(0x06ac79A00D7a66e4FA03Eac3306F8701Ee39D09d), type(uint256).max);
        IERC20(_token).safeApprove(address(0xA5E949FB0D288aa02bf129C973379A8D88330E37), type(uint256).max);
        IERC20(_token).safeApprove(address(0x9c0D7F04764A05642d45f2E7f746647a555a799C), type(uint256).max);
        IERC20(_token).safeApprove(address(0x0fd735A017Ab8c56d55C4306E851866bc23d5197), type(uint256).max);
    ...
    */
    ...
    function harvest() external {
        ...
        IacPool(acPool1).harvest(msg.sender);
        IacPool(acPool2).harvest(msg.sender);
        IacPool(acPool3).harvest(msg.sender);
        IacPool(acPool4).harvest(msg.sender);
        IacPool(acPool5).harvest(msg.sender);
        IacPool(acPool6).harvest(msg.sender);
	...
	/*
	* Normalizes rewards after boosting period ends
	* Must be called in the time span of 5 days
	* (If the governing contract is relaunched afterwards, the emissions wouldn't be re-set)
	*/
	function normalizeRewards() external {
		require(!normalRewards, "already normalized");
		require(block.timestamp > 1641234642 && block.timestamp < 1641666642, "After (GMT)Monday, January 3, 2022 6:30:42 PM");
		
		normalRewards = true;
		IMasterChef(masterchef).updateEmissionRate(25694201337 * 1e9); //roughly 25 tokens/block
	}
    ...
    function afterDelayOwnership() external onlyTrustee {
        ...
        require(newGovernorRequestBlock2 + 2222 < block.number, "Pending timelock");
