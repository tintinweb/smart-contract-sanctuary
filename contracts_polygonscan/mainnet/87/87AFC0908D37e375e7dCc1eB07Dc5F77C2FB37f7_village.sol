...
contract village is ERC721Enumerable, ReentrancyGuard, Ownable {
  // Phrases of Weapons
  ...
  string[] private summer;

  string[] private autumn;

  string[] private winter;

  string[] private identity;

  string[] private plant;

  string[] private animal;

  string[] private atmosphere;

  uint8 public set_once=1;

  function init(
    string[] memory _spring,
    string[] memory _summer,
    string[] memory _autumn,
    string[] memory _winter,
    string[] memory _identity,
    string[] memory _plant,
    string[] memory _animal,
    string[] memory _atmosphere
  ) public onlyOwner{
    require(set_once>0,"must bigger one!!!");
    spring=_spring;
    summer=_summer;
    autumn=_autumn;
    winter=_winter;
  ...
  function getSpring(uint256 tokenId) public view returns (string memory) {
    return pluck(tokenId, "spring", spring);
  }
  ...
  // a loot bag has
  function getSummer(uint256 tokenId) public view returns (string memory) {
    return pluck(tokenId, "summer", summer);
  }

  function getAutumn(uint256 tokenId) public view returns (string memory) {
    return pluck(tokenId, "autumn", autumn);
  }

  function getWinter(uint256 tokenId) public view returns (string memory) {
    return pluck(tokenId, "winter", winter);
  }
    ...
    returns (string memory)
  {
    ...
    parts[3] = getSpring(tokenId);

    parts[4] = '</text><text x="10" y="60" class="base">';

    parts[5] = getSummer(tokenId);

    parts[6] = '</text><text x="10" y="80" class="base">';

    parts[7] = getAutumn(tokenId);

    parts[8] = '</text><text x="10" y="100" class="base">';

    parts[9] = getWinter(tokenId);

    parts[10] = '</text><text x="10" y="120" class="base">';

    parts[11] = getPlant(tokenId);

    parts[12] = '</text><text x="10" y="140" class="base">';

    parts[13] = getAnimal(tokenId);

    parts[14] = '</text><text x="10" y="160" class="base">';

    parts[15] = getAtmosphere(tokenId);
      ...
      bytes(
        string(
          abi.encodePacked(
            '{"name": "Bag #',
            toString(tokenId),
            '", "description": "you can get a Chinese village image, and the first word in this image is your identity image,you can get it for free!!!", "image": "data:image/svg+xml;base64,',
  ...
  function ownerClaim(uint256 tokenId) public nonReentrant onlyOwner {
    require(tokenId > 4999 && tokenId < 6057, "Token ID invalid");
  ...
  constructor() ERC721("Villiage", "vil") Ownable() {}
}
