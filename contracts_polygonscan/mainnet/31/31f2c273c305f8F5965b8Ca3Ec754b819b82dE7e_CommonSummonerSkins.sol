...
contract CommonSummonerSkins is ReentrancyGuard, Ownable, ERC721Enumerable {
    ...
    constructor(address _rarity, address _gold, address _raritySkinManagerFix) ERC721("Adventurer Costumes", "COSTUMES"){
...
contract CommonSkinURIs is Ownable {
	...
	function metadata(uint256 _tokenId, uint256 _dna) private view returns (string memory) {
		...
		return string(abi.encodePacked(
			'{',
				'"name": "Adventurer Costume #', _tokenId.toString(), '",', 
	...
	function imageData(uint256 _tokenId, uint256 _dna) private view returns (bytes memory) {
		...
		return abi.encodePacked(
			"<svg id='Adventurer Costume #", _tokenId.toString(), "' xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24'>",
...
contract RaritySkinManager is Ownable {
    ...
    modifier classChecked(address implementation, uint tokenId, uint summonerId) {
        try myIERC721(implementation).isStrictOnSummonerClass() returns(bool isStrict) {
            if(isStrict){
                require(myIERC721(rarity).class(summonerId) == myIERC721(implementation).class(tokenId), "Adventurer and costume must be of the same class");
            }
    ...
    classChecked(implementation, tokenId, summonerId) {
        ...
        require(isApprovedOrOwner(rarity, msg.sender, summonerId), "You must be owner or approved for this adventurer");
...
contract RaritySkinManagerFix is Ownable {
    ...
    event Claim(address indexed claimer, uint skinId, uint claimedAmount, bool isRogue);
    event ClaimAll(address indexed claimer, uint claimedAmount);
    event SumonnerSkinAssigned (RaritySkinManager.Skin skin, uint summoner);
    ...
    mapping(uint => RaritySkinManager.Skin) private _skinOf;
    mapping(bytes32 => uint) private _summonerOf;
    mapping(address => bool) private trustedImplementations;
    ...
    address skins;
    uint public rogueReserve;
    uint public roguesTotalLevels;
    uint public tokenPerLevel;
    uint unaccountedRewards;
    mapping(uint => uint) public roguesLevels;
    mapping(uint => uint) public roguesValues;
    mapping(uint => uint) public adventurersTime;

    uint ROGUE_PERCENT = 20; // Percent of tokens for rogues 
    uint TOKENS_PER_DAY = 10000e18; // Tokens claimable for adventurers per day
    uint ROGUE = 9;
    ...
    function skinOf(uint summonerId) public view returns(RaritySkinManager.Skin memory){
    ...
    function summonerOf(bytes32 _skinKey) public view returns(uint summonerId){
    ...
    function _assignSkinToSummoner(address implementation, uint tokenId, uint summonerId) private {
        ...
        if (roguesLevels[tokenId] == 0 && adventurersTime[tokenId] == 0 && implementation == skins) { // check if skin first time take part in event
            if (isRogueClass(tokenId)) { // check if skin is Rogue
    ...
    function isApprovedOrOwner(address nftAddress, address spender, uint tokenId) private view returns (bool) {
        myIERC721 implementation = myIERC721(nftAddress);
        address owner = implementation.ownerOf(tokenId);
        return (spender == owner || implementation.getApproved(tokenId) == spender || implementation.isApprovedForAll(owner, spender));
    }
    ...
    function availableForClaimAll(address owner) external view returns (uint amount) {
        ...
        if (amount * 100 / (100 - ROGUE_PERCENT) > tokenBalance - rogueReserve) {
            amount = (tokenBalance - rogueReserve) * (100 - ROGUE_PERCENT) / 100;
        }
    ...
    function availableForClaim(uint tokenId) public view returns (uint amount) {
        if (isRogueClass(tokenId)) {
            ...
            if (adventurersTime[tokenId] == 0) {
                ...
                return (TOKENS_PER_DAY * (block.timestamp - adventurersTime[tokenId]) / 1 days ) * (100 - ROGUE_PERCENT) / 100;
            }
        }
    }
    ...
    function claim(uint tokenId) external {
        ...
        require(roguesLevels[tokenId] != 0 || adventurersTime[tokenId] != 0, "You must assign costume to adventurer");
        uint res;
        if (isRogueClass(tokenId)) {
            ...
            require(IERC20(token).balanceOf(address(this)) > rogueReserve, "No tokens left for claiming");
    ...
    function _claimByAdventurer(uint tokenId) private returns (uint) {
        ...
        uint earnedAmount = TOKENS_PER_DAY * (block.timestamp - adventurersTime[tokenId]) / 1 days;
        if (earnedAmount > tokenBalance - rogueReserve) {
            earnedAmount = tokenBalance - rogueReserve;
        }
        adventurersTime[tokenId] = block.timestamp;
        uint rogueAmount = earnedAmount * ROGUE_PERCENT / 100;
    ...
    function _claimByRogue(uint tokenId) private returns (uint) {
        uint amount = roguesLevels[tokenId] * (tokenPerLevel - roguesValues[tokenId]);
        rogueReserve -= amount;
        roguesValues[tokenId] = tokenPerLevel;
    ...
    function isRogueClass(uint tokenId) private view returns (bool) {
