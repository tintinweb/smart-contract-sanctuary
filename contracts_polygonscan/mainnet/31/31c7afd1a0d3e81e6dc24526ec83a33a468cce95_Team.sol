...
contract Team is Ownable{
    ...
    uint8 public lockingPercent = 0;
    uint256 public vestingTime = 0;
    uint8 public vestingPercent = 0;
    ...
    **/
    ...
    constructor(address _token) {
        token = IERC20(_token);
        maxVolume = 1800000000000;
        TGE = 1637884800;

        timePerRound = 2*3600; //2 hour

        lockingTime = TGE + timePerRound*6;
        lockingPercent = 25;
        vestingTime = lockingTime + timePerRound*36;
        vestingPercent = 75;
    ...
    function claim() public returns (bool){
        ...
        if (currentRound == totalRound){
            ...
            uint256 sendValue;
            if (whitelist[_msgSender()].lastRoundClaim == 0){
                sendValue = whitelist[_msgSender()].amount*lockingPercent + whitelist[_msgSender()].amount*(currentRound - 1)*vestingPercent/(totalRound - 1);
                require(token.transfer(_msgSender(), sendValue/100));
                emit Claim(_msgSender(), sendValue/100);
                whitelist[_msgSender()].claimed = sendValue/100;
            } else {
                uint256 nextValueClaimed = whitelist[_msgSender()].amount*lockingPercent + (whitelist[_msgSender()].amount*(currentRound - 1)*vestingPercent)/(totalRound - 1);
                sendValue = nextValueClaimed - whitelist[_msgSender()].claimed*100;
                require(token.transfer(_msgSender(), sendValue/100));
                emit Claim(_msgSender(), sendValue/100);
                whitelist[_msgSender()].claimed =  whitelist[_msgSender()].claimed + sendValue/100;
            }
