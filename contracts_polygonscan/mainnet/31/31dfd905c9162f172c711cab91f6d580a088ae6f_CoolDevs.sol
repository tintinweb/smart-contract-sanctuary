...
// File: contracts/GOATZ_V3.sol

// pragma solidity ^0.8.0;

contract CoolDevs is ERC721("Cool Devs", "COOLDEV"), Ownable {
    ...
    function purchaseTokens(uint256 _howMany) public payable {
    ...
    function _mintToken(address _to) private {
        ...
        _mint(_to, purchasedTokens);
    }

    // RESERVE FUNCTIONS
    function gift(address[] calldata to) external onlyOwner {
        require(
            to.length <= tokensRemainingToBeMinted(),
    ...
    function tokensRemainingToBeMinted() public view returns (uint256) {
        return totalTokenToMint - purchasedTokens;
    }

    function isAllTokenMinted() public view returns (bool) {
        return purchasedTokens == totalTokenToMint;
    }
