...
interface ILiquidityHelper {
    function addDarksideLiquidity(address baseTokenAddress, uint256 baseAmount, uint256 nativeAmount) external;
}
interface IDarksideChef {
    function deposit(uint256 _pid, uint256 _amountOrId, bool isNFTHarvest, address _referrer) external;
}

contract DarksideStrategy is BaseStrategyMasterChef {

    uint256 public constant keepMax = 10000;

    //Tokens
    address private constant WMATIC = 0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270;
    address private constant DARKSIDE = 0x1942b8262a0683B54f4f91D0c08dDD92ed6E8FE6;
    address private constant CZDIAMOND = 0x6664C64C8582e62dA861Fcac33a627b0f92fF0F8;
    address public tokenA;
    address public tokenB;

    //Other contracts
    address private constant LIQUIDITY_HELPER = 0xA87bfce2034c40f441a0467e57d5D9fFCc40C85E;
    address private constant MASTER_CHEF = 0x767580F1B2aF12b7267cE33Ba2678addA7388403;
    address private constant SUSHI_ROUTER = 0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506;
    address private constant ERC_FUND = 0x01fE07ce760DA7a025e4Cf3f950aE236F8e62120;

    // Uniswap swap paths
    address[] public a_b_path;
    address[] public darkside_czdiamond_path;
    address[] public czdiamond_darkside_path;
        ...
        )
    {
        ...
        tokenA = _tokenA; //should be one of the native farm tokens
        tokenB = _tokenB; //should be the non-native token

        darkside_czdiamond_path = new address[](2);
        darkside_czdiamond_path[0] = DARKSIDE;
        darkside_czdiamond_path[1] = CZDIAMOND;

        czdiamond_darkside_path = new address[](2);
        czdiamond_darkside_path[0] = CZDIAMOND;
        czdiamond_darkside_path[1] = DARKSIDE;

        a_b_path = new address[](2);
        a_b_path[0] = tokenA;
        a_b_path[1] = tokenB;

        IERC20(_want).safeApprove(MASTER_CHEF, uint256(-1));
    }
    ...
    function swapRewardToWmaticAndDistributeFee(address reward, uint256 feeAmount) internal {
        if(feeAmount > 0) {
            address[] memory reward_wmatic_path = new address[](2);
            reward_wmatic_path[0] = reward;
            reward_wmatic_path[1] = WMATIC;

            _swapUniswapWithPathForFeeOnTransferTokens(reward_wmatic_path, feeAmount);
    ...
    function harvest() public override onlyHumanOrWhitelisted nonReentrant {
        ...
        uint256 balance_before_a = IERC20(DARKSIDE).balanceOf(address(this));
        uint256 balance_before_f = IERC20(CZDIAMOND).balanceOf(address(this));
        _getReward();
        uint256 amountHarvested_a = IERC20(DARKSIDE).balanceOf(address(this)).sub(balance_before_a);
        if (amountHarvested_a > 0) {
            swapRewardToWmaticAndDistributeFee(DARKSIDE, amountHarvested_a.mul(IERCFund(strategist).getFee()).div(keepMax));
        }
        uint256 amountHarvested_f = IERC20(CZDIAMOND).balanceOf(address(this)).sub(balance_before_f);
        if (amountHarvested_f > 0) {
            swapRewardToWmaticAndDistributeFee(CZDIAMOND, amountHarvested_f.mul(IERCFund(strategist).getFee()).div(keepMax));
        }

        //Convert remainder of the other farm token into the tokenA for this LP pair
        if(tokenA == DARKSIDE) {
            uint256 remaining_f = IERC20(CZDIAMOND).balanceOf(address(this));
            if (remaining_f > 0) {
                _swapUniswapWithPathForFeeOnTransferTokens(czdiamond_darkside_path, remaining_f);
            }
        }
        else if(tokenA == CZDIAMOND) {
            uint256 remaining_a = IERC20(DARKSIDE).balanceOf(address(this));
            if (remaining_a > 0) {
                _swapUniswapWithPathForFeeOnTransferTokens(darkside_czdiamond_path, remaining_a);
            }
        }
        ...
        if (_balanceA > 0) {
            _swapUniswapWithPathForFeeOnTransferTokens(a_b_path, _balanceA.div(2));
        }
        ...
        if (aBalance > 0 && bBalance > 0) {
            if(tokenA == DARKSIDE) {
                IERC20(tokenA).safeApprove(LIQUIDITY_HELPER, 0);
                IERC20(tokenA).safeApprove(LIQUIDITY_HELPER, aBalance);
                IERC20(tokenB).safeApprove(LIQUIDITY_HELPER, 0);
                IERC20(tokenB).safeApprove(LIQUIDITY_HELPER, bBalance);

                ILiquidityHelper(LIQUIDITY_HELPER).addDarksideLiquidity(
                    tokenB,
                    bBalance,
                    aBalance
                );
    ...
    function deposit() public override {
        uint256 _want = IERC20(want).balanceOf(address(this));
        if (_want > 0) {
            IDarksideChef(rewards).deposit(poolId, _want, false, 0x8AcBb9cBc2E3A8286e2A0f5610470cf81F541764);
        }
    }
