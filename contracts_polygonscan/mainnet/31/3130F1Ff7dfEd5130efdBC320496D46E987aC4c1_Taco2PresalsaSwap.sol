...
MM88MMM ,adPPYYba,  ,adPPYba,  ,adPPYba,   
  88    ""     `Y8 a8"     "" a8"     "8a  
  88    ,adPPPPP88 8b         8b       d8  2 PRESALSA SWAPPER
...
import "./TacoToken.sol";

contract Taco2PresalsaSwap is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    // Burn address
    address public constant BURN_ADDRESS = 0x000000000000000000000000000000000000dEaD;

    uint256 public constant BLOCKS_DURATION = 43200; // duration in blocks

    uint256 public constant TOKEN_RATIO = 30;

    TacoParty immutable public tacoToken;

    IERC20 immutable public presalsaToken;

    address  tacoAddress;

    bool  hasBurnedUnsoldPresale = false;
    ...
    constructor(uint256 _startBlock, address _tacoAddress, address _presalsaAddress) {
        require(_presalsaAddress != _tacoAddress, "taco cannot be equal to presalsa");
        startBlock = _startBlock;
        tacoToken = TacoParty(_tacoAddress);
        presalsaToken = IERC20(_presalsaAddress);
        endBlock = _startBlock + BLOCKS_DURATION;
    }

    function swapPreSalsaForSalsa() external nonReentrant {
        require(block.number >= startBlock, "swap not yet active.");

        uint256 swapAmount = tacoToken.balanceOf(msg.sender);
        require(swapAmount > 0,'Converting a zero balance.');

        uint256 convertedAmount = swapAmount / TOKEN_RATIO;

        require(presalsaToken.balanceOf(address(this)) >= convertedAmount, "Not Enough tokens in contract for swap");
        require(tacoToken.transferFrom(msg.sender, BURN_ADDRESS, swapAmount), "failed sending taco" );
        presalsaToken.safeTransfer(msg.sender, convertedAmount);

        emit PreTacoToTaco(msg.sender, swapAmount);
    }

    // For this swap, no real need to call this.
    function sendUnclaimedPresalsaToDeadAddress() external onlyOwner {
        require(block.number > endBlock, "can only send excess to dead address after presale has ended");
        require(!hasBurnedUnsoldPresale, "can only burn unsold presale once!");

        uint256 presalsaBalance = presalsaToken.balanceOf(address(this));
        require(presalsaBalance > 0,
            "presalsa balance is zero");

        if (presalsaBalance > 0)
            presalsaToken.safeTransfer(BURN_ADDRESS, presalsaBalance);
        hasBurnedUnsoldPresale = true;

        emit burnUnclaimedTaco(presalsaBalance);
    }
