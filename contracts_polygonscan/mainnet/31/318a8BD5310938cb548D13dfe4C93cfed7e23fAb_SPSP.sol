...
contract SPSP is ERC20Permit, Ownable {
    ...
    mapping(address => mapping(uint256 => WithdrawalRequests)) public userVsWithdrawals;
    ...
    event Reentered(uint256 indexed id, address indexed user, uint256 amount);
    ...
    function reenter(uint256 id) external {

        WithdrawalRequests storage request = userVsWithdrawals[msg.sender][id];
        require(
    ...
    function leave(uint256 _sPSPAmount, uint256 id) external {
        ...
        WithdrawalRequests storage request = userVsWithdrawals[msg.sender][id];
        require(request.status == WITHDRAW_STATUS.UNUSED, "Invalid id");

        request.amountPSP = pspAmount;
        request.releaseBlockNumber = block.number + timeLockBlocks;
        request.status = WITHDRAW_STATUS.LOCKED;

        pspsLocked = pspsLocked + pspAmount;
    ...
    function sPSPForPSP(uint256 _sPSPAmount) external view returns (uint256 pspAmount_) {
        uint256 totalSPSP = totalSupply();
        uint256 pspBalanceAvailable = psp.balanceOf(address(this))- pspsLocked;
    ...
    function PSPForSPSP(uint256 _pspAmount) external view returns (uint256 sPSPAmount_) {
    ...
    function withdraw(uint256 id) public {
        ...
        pspsLocked = pspsLocked / request.amountPSP;
        psp.transfer(msg.sender, request.amountPSP);

        emit Withdraw(id, msg.sender, request.amountPSP);
    }
}
