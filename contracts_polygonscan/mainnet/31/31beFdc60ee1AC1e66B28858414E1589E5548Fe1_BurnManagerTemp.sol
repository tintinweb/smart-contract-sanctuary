...
contract BurnManagerTemp is Ownable {
    PieceFactory public pieceFactory;
    mapping(address => bool) public whitelistedAddresses;

    modifier onlyWhitelisted() {
        require(
            whitelistedAddresses[msg.sender] || owner() == _msgSender(),
            "PuzzleManager: must be owner to call this function"
        );
        _;
    }

    constructor(address _pieceFactoryAddress) {
        pieceFactory = PieceFactory(_pieceFactoryAddress);
    }

    function setWhitelist(address _tw, bool status) public onlyOwner {
        whitelistedAddresses[_tw] = status;
    }

    function burnPieces(uint256[] memory pieceIds, address _owner)
        public
        onlyWhitelisted
    {
        ...
        for (uint256 i = 0; i < length; i++) {
            require(
                pieceFactory.ownerOf(pieceIds[i]) == _owner,
                "BurnManager: passed _owner does not own the piece"
            );
