...
contract UnusedUsdcBankStrat is Ownable, Pausable {
    ...
    address[] public wmaticToWeth = [wmatic, weth];
    ...
    constructor(address _vaultAddress) public {
        ...
        IERC20(weth).safeApprove(aavebank, 0);
        IERC20(weth).safeApprove(aavebank, uint(-1));
    ...
    function deposit() public {
        ...
        if (pairBal > 0) {
            ILendingPool(aavebank).deposit(weth, IERC20(weth).balanceOf(address(this)), address(this), 0);
        }
    }
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (pairBal < _amount) {   
            ILendingPool(aavebank).withdraw(weth,_amount.sub(pairBal), address(this));
    ...
    function swap() internal{
        ...
        IPanwexRouter(unirouter).swapExactTokensForTokens(amount, 0, wmaticToWeth, address(this), now + 600);
    }
    ...
    function balanceOfPool() public view returns (uint256) {
        (uint256 supplyBal,,,,,,,,) = IDataProvider(dataProvider).getUserReserveData(weth, address(this));
        return supplyBal;
    }
    ...
    function panic() public onlyOwner {
        ILendingPool(aavebank).withdraw(weth, type(uint).max, address(this));
        pause();
    }
