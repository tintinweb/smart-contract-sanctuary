...
contract Marketplace is Ownable, ReentrancyGuard {
    ...
    event CancelledSellOffer(uint256 indexed tokenId, address indexed seller);
    event Sold(
        uint256 indexed tokenId,
        address seller,
    ...
    event ArtworkCreated(uint256 artworkId, address indexed artist);
    event ArtworkMinted(
        uint256 artworkId,
        uint256 indexed tokenId,
        address indexed buyer
    ...
    uint256 private MIN_PRICE = 1 ether;
    uint256 private MAX_PRICE = 10000 ether;

    Counters.Counter private _artworkIdCounter;

    mapping(uint256 => Artwork) private artworks;
    mapping(uint256 => SellOffer) private sellOffers;
    ...
    function getDeveloper() external view onlyOwner returns (address) {
        return developer;
    }

    function setPriceLimits(uint256 minPrice, uint256 maxPrice)
        external
        onlyOwner
    {
        MIN_PRICE = minPrice;
    ...
    function withdraw() external onlyOwner nonReentrant {
        ...
        Address.sendValue(payable(developer), royalty);
    }

    function _getStrLen(string memory _str) private pure returns (uint256) {
    ...
    function createArtwork(
        ...
        require(
            totalSupply > 0 && totalSupply <= 10000,
            "The total supply must be between 1 and 10000"
        );
        require(price >= MIN_PRICE && price <= MAX_PRICE, "invalid price");
        require(
            royalty >= 0 && royalty <= 2500,
            "The royalty must be between 0% and 25%"
        );

        require(_getStrLen(name) > 0 && _getStrLen(name) < 60);
        require(_getStrLen(metadata) > 0);

        require(msg.sender != address(0));

        uint256 id = _artworkIdCounter.current();
        ...
        emit ArtworkCreated(id, msg.sender);
    }

    function getArtworkCounts() public view returns (uint256) {
    ...
    function setSaleStatus(
        ...
        uint256 royalty,
        bool onsale
    ) external hasArtwork(id) {
        require(price >= MIN_PRICE && price <= MAX_PRICE, "Invalid price");
        require(royalty >= 0 && royalty <= 2500);

        Artwork memory artwork = artworks[id];
        require(msg.sender == artwork.artist, "Invalid operation.");

        artwork.price = price;
        artwork.royalty = royalty;
        artwork.onsale = onsale;
        artworks[id] = artwork;
    }
    ...
    function getArtwork(uint256 id)
        ...
        view
        hasArtwork(id)
        returns (Artwork memory artwork)
    {
        return artworks[id];
    }

    function mintToken(uint256 id, string memory url)
        external
        payable
        canMint(id)
        nonReentrant
    {
        ...
        uint256 platformCharge = price.mul(platformFee).div(10000);
        uint256 transferred = price.sub(platformCharge);

        require(msg.value >= artwork.price, "Invalid price given");

        token.mint(id, artwork.artist, msg.sender, artwork.royalty, url);
        Address.sendValue(payable(artwork.artist), transferred);

        uint256 tokenId = token.getLatestTokenId();
        artwork.tokenIds.push(tokenId);

        emit ArtworkMinted(id, tokenId, msg.sender);
    }

    function acceptOffer(uint256 tokenId)
        external
        payable
        hasSellOffer(tokenId)
        isMarketable(tokenId)
        nonReentrant
    {
        SellOffer memory offer = sellOffers[tokenId];

        // If the artwork is sold or transffered on different platform.
        if (offer.seller != token.ownerOf(tokenId)) {
            delete (sellOffers[tokenId]);
            emit CancelledSellOffer(tokenId, offer.seller);
            revert("Invalid sell offer");
        }

        require(msg.value >= offer.price, "Amount sent too low");

        uint256 platformCharge = offer.price.mul(platformFee).div(10000);

        uint256 restAmount = offer.price.sub(platformCharge);

        uint256[] memory royalties = token.getFeeBps(tokenId);
        address payable[] memory recipents = token.getFeeRecipients(tokenId);

        for (uint256 i = 0; i < royalties.length; i++) {
            uint256 royalty = royalties[i];
            address payable recipent = recipents[i];

            uint256 amount = offer.price.mul(royalty).div(10000);

            if (amount > 0) {
                Address.sendValue(recipent, amount);
                restAmount = restAmount.sub(amount);
            }
        }

        if (restAmount > 0) {
            Address.sendValue(payable(offer.seller), restAmount);
        }

        token.safeTransferFrom(offer.seller, msg.sender, tokenId);

        delete (sellOffers[tokenId]);
        emit Sold(tokenId, offer.seller, msg.sender, msg.value);
    }

    function makeSellOffer(uint256 tokenId, uint256 price)
        external
        tokenOwnerOnly(tokenId)
    {
        if (price > 0) {
            require(price >= MIN_PRICE && price <= MAX_PRICE, "Invalid price");
            sellOffers[tokenId] = SellOffer({seller: msg.sender, price: price});
            token.approve(address(this), tokenId);
        } else {
            emit CancelledSellOffer(tokenId, msg.sender);
            delete sellOffers[tokenId];
        }
        emit MadeSellOffer(tokenId, msg.sender, price);
    }

    function getSellOffer(uint256 tokenId) external view returns (uint256) {
        return sellOffers[tokenId].price;
    }
    ...
    modifier hasSellOffer(uint256 tokenId) {
        SellOffer memory sellOffer = sellOffers[tokenId];
        bool isAvailable = sellOffer.price != 0 &&
            sellOffer.seller != address(0);
        require(isAvailable, "Invalid sell offer");
        _;
    }

    modifier canMint(uint256 id) {
        Artwork memory artwork = artworks[id];
        require(artwork.artist != address(0) && artwork.totalSupply > 0);
        require(artwork.onsale);
        require(artwork.tokenIds.length < artwork.totalSupply);
        _;
    }

    modifier hasArtwork(uint256 id) {
        Artwork memory artwork = artworks[id];
        require(artwork.artist != address(0) && artwork.totalSupply > 0);
        _;
    }
