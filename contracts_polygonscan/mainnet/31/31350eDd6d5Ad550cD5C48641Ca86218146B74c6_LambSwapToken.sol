...
contract LambSwapToken is  Context, Ownable  {
    ...
    /* --------- max tx info --------- */
	uint256 public _maxSupply = 300e9 * 1e9; // maximum supply: 300,000,000,000
	uint public _limitTxAmount = 300e9 * 1e9; // transaction limit: 300,000,000,000
	uint public numTokensSellToAddToLiquidity = 23e2 * 1e9; // swap token amount: 2300
    ...
    constructor () public {
        ...
        _symbol = "LST";
        _decimals = 9;
        _totalSupply = 23e9*1e9; /// initial supply 23,000,000,000
	...
	function _transfer(address sender, address recipient, uint256 amount) internal {
		...
		if(!_isExcludeFromFee[sender] && !_isExcludeFromFee[recipient]) {
			if(sender == UniswapV2Pair){
				...
				emit Transfer(sender, burnAddress, amount.mul(buyFees.burn).div(100));
				emit Transfer(sender, marketingAddress, amount.mul(buyFees.marketing).div(100));
				emit Transfer(sender, LPAddress, amount.mul(buyFees.liquidity).div(100));
				emit Transfer(sender, address(this), amount.mul(buyFees.buyback).div(100));
			}
			else if(recipient == UniswapV2Pair){
				// sell fee
				_balances[sender] = _balances[sender].sub(amount.mul(_taxRates.sellFee - sellFees.burn).div(100), "LST::_transfer: transfer amount exceeds balance");
                _burn(sender,amount.mul(sellFees.burn).div(100));
				_balances[marketingAddress] += amount.mul(sellFees.marketing).div(100);
				_balances[LPAddress] += amount.mul(sellFees.liquidity).div(100);
				_balances[address(this)] += amount.mul(sellFees.buyback).div(100);

				emit Transfer(sender, marketingAddress, amount.mul(sellFees.marketing).div(100));
				emit Transfer(sender, LPAddress, amount.mul(sellFees.liquidity).div(100));
				emit Transfer(sender, address(this), amount.mul(sellFees.buyback).div(100));
			}
			else {
				// transfer fee
				_balances[sender] = _balances[sender].sub(amount.mul(_taxRates.transferFee - transferFees.burn).div(100), "LST::_transfer: transfer amount exceeds balance");
                _burn(sender,amount.mul(transferFees.burn).div(100));
				_balances[marketingAddress] += amount.mul(transferFees.marketing).div(100);
				_balances[LPAddress] += amount.mul(transferFees.liquidity).div(100);
				_balances[address(this)] += amount.mul(transferFees.buyback).div(100);

				emit Transfer(sender, marketingAddress, amount.mul(transferFees.marketing).div(100));
				emit Transfer(sender, LPAddress, amount.mul(transferFees.liquidity).div(100));
				emit Transfer(sender, address(this), amount.mul(transferFees.buyback).div(100));
			}
		}
