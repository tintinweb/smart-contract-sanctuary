...
contract SecondaryBridge is BaseBridge {
    ...
    function burn(uint256 amount) external returns (bool) {
        ...
        amountHeld -= amount;
        IERC20(_token).safeTransferFrom(msg.sender, address(0), amount);
        emit Burnt(amount, block.timestamp, msg.sender);
        return true;
    }

    event Minted(uint256 amount, uint256 timestamp, address indexed to);
    event Burnt(uint256 amount, uint256 timestamp, address indexed to);
}
