...
contract SHOOTTOPOLY is ERC721, ERC721Pausable, ERC721Enumerable, ERC721URIStorage, IChildToken, AccessControlMixin, EIP712Base, ContextMixin {
    ...
    uint256 public royaltyFee = 12500;
    ...
    constructor() ERC721("SHOOTTOPOLY", "CNFT") {
        _setupContractId("SHOOTTOPOLY");
        _setupRole(DEPOSITOR_ROLE, address(0xb5505a6d998549090530911180f38aC5130101c6)); 
         _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _initializeEIP712("SHOOTTOPOLY");
    ...
    **/
    ...
    function withdraw(uint256 tokenId) external {
        require(_msgSender() == ownerOf(tokenId), "SHOOTTOPOLY: INVALID_TOKEN_OWNER");
    ...
    function withdrawBatch(uint256[] calldata tokenIds) external {
        ...
        uint256 length = tokenIds.length;
        require(length <= BATCH_LIMIT, "SHOOTTOPOLY: EXCEEDS_BATCH_LIMIT");
        ...
        for (uint256 i; i < length; i++) {
            ...
            require(_msgSender() == ownerOf(tokenId), string(abi.encodePacked("SHOOTTOPOLY: INVALID_TOKEN_OWNER ", tokenId)));
            withdrawnTokens[tokenId] = true;
            _burn(tokenId);
     ...
     * This implementation allows only jelly to mint tokens
     * Should verify if token is withdrawn by checking `withdrawnTokens` mapping
     * @param tokenId token which should be minted
     * @param uri resource identifier of the token in the form of an ipfs-cid
     */
    function Mint(uint256 tokenId, string calldata uri) public only(DEFAULT_ADMIN_ROLE) {
        require(!withdrawnTokens[tokenId], "SHOOTTOPOLY: TOKEN_EXISTS_ON_ROOT_CHAIN");
     ...
     */
     // TO-DO
    function setRoyaltyWallet(address _royaltyWallet) external only(DEFAULT_ADMIN_ROLE){
     ...
     */
    function setRoyaltyFee(uint256 _royaltyFee) external only(DEFAULT_ADMIN_ROLE){
     ...
     */
    function withdrawETH() public only(DEFAULT_ADMIN_ROLE) {
