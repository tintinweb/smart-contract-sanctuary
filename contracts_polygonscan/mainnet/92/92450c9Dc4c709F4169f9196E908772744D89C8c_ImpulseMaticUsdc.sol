...
contract BaseImpulseStrategy is IImpulseStrategy, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    uint256 public commissionAccuracy;
    address internal swapToUnderlyingToken;
    ...
    function initialize(
        ...
        commissionAccuracy = 10**IERC20Metadata(_rewardToken).decimals();

        if (_underlyingTokens[0] != _rewardToken) {
            swapToUnderlyingToken = _underlyingTokens[0];
        } else {
            swapToUnderlyingToken = _underlyingTokens[1];
        }
    }
    ...
    function setCommission(uint256 _commission) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_commission < commissionAccuracy, "Wrong commission");
    ...
    // Transfer want tokens autoFarm -> strategy
    function deposit(uint256 _wantAmt) external override nonReentrant onlyRole(STRATEGIST_ROLE) returns (uint256) {
        uint256 shares = 0;
        wantToken.safeTransferFrom(msg.sender, address(this), _wantAmt);
    ...
    // Transfer want tokens strategy -> autoFarm
    function withdraw(uint256 _wantAmt) external override nonReentrant onlyRole(STRATEGIST_ROLE) returns (uint256) {
        require(_wantAmt <= wantTotal && _wantAmt > 0, "withdraw, wrong value");
        uint256 shares = 0;
        _withdrawLpToken(_wantAmt);
        wantToken.safeTransfer(_msgSender(), _wantAmt);
    ...
    function wantPriceInUsd(uint256 _wantAmt) external view virtual override returns (uint256) {
        require(usdToken != address(0), "Wrong usd address");
        uint256 result = 0;
        uint256 lpTotalSupply = wantToken.totalSupply();
        for (uint256 i = 0; i < underlyingTokens.length; i++) {
            uint256 amount = IERC20(underlyingTokens[i]).balanceOf(address(wantToken));
            address[] memory path = swapRewardRoutes[underlyingTokens[i]][usdToken];
            result = result + IUniswapV2Router01(router).getAmountsOut((_wantAmt * amount) / lpTotalSupply, path)[path.length - 1];
        }
        return result;
    }
    ...
    function earn() external virtual override nonReentrant onlyRole(BACKEND_ROLE) {
        ...
        uint256 rewardCommission = 0;

        if (rewardBalance >= DUST) {
            if (commission > 0 && treasury != address(0)) {
                rewardCommission = (rewardBalance * commission) / commissionAccuracy;
                if (rewardCommission > 0) {
                    rewardBalance -= rewardCommission;
                    rewardToken.safeTransfer(treasury, rewardCommission);
                }
            }
            _swapRewardsToUnderlings(rewardBalance);
        ...
        commissions[0] = rewardCommission;
        emit Earning(wantEarned, commissions, wantTotal, totalSupplyShares);
    }

    /// @notice Swap rewards token to underlying tokens.
    /// @param _rewardAmt Amount of rewards token.
    function _swapRewardsToUnderlings(uint256 _rewardAmt) internal virtual {
        _swapRewards(_rewardAmt / 2);
    }

    /// @notice Swaps reward tokens to want token.
    /// @param _rewardAmount Amount of reward token.
    function _swapRewards(uint256 _rewardAmount) internal virtual {
        rewardToken.safeApprove(router, 0);
        rewardToken.safeApprove(router, _rewardAmount);

        address[] memory path = swapRewardRoutes[address(rewardToken)][swapToUnderlyingToken];
        IUniswapV2Router01(router).swapExactTokensForTokens(_rewardAmount, 1, path, address(this), block.timestamp + 1)[path.length - 1];
    }
    ...
    function _depositLpToken(uint256 _wantAmt) internal virtual {
        wantToken.safeApprove(address(stakingRewards), 0);
        wantToken.safeApprove(address(stakingRewards), _wantAmt);
        stakingRewards.stake(_wantAmt);
    }
    ...
    function _swapAllUnderlyingToWant() internal virtual returns (uint256) {
        ...
        for (uint256 i = 0; i < 2; i++) {
            ...
            IERC20(underlyingTokens[i]).safeApprove(router, 0);
            IERC20(underlyingTokens[i]).safeApprove(router, amounts[i]);
        }
        uint256 wantBalanceBefore = wantToken.balanceOf(address(this));
        ...
        return wantToken.balanceOf(address(this)) - wantBalanceBefore;
    }
}

contract BaseImpulseStrategyRwrdTwoSwp is BaseImpulseStrategy {
    using SafeERC20 for IERC20;
    ...
    function initialize(
        ...
        super.initialize(_wantToken, _router, _stakingRewards, _underlyingTokens, _rewardToken);
    }

    /// @notice Swaps reward tokens to want token.
    /// @param _rewardAmount Amount of reward token.
    function _swapRewards(uint256 _rewardAmount) internal override {
        rewardToken.safeApprove(router, 0);
        rewardToken.safeApprove(router, _rewardAmount);

        for (uint256 i = 0; i < 2; i++) {
            uint256 amount = IERC20(underlyingTokens[i]).balanceOf(address(this));
            address[] memory path = swapRewardRoutes[address(rewardToken)][underlyingTokens[i]];
            IUniswapV2Router01(router).swapExactTokensForTokens(_rewardAmount / 2, 1, path, address(this), block.timestamp + 1)[path.length - 1];
        }
    }

    /// @notice Swap rewards token to want tokens.
    /// @param _rewardAmt Amount of rewards token.
    function _swapRewardsToUnderlings(uint256 _rewardAmt) internal virtual override {
        _swapRewards(_rewardAmt);
    }
}
...
contract ImpulseMaticUsdc is BaseImpulseStrategyRwrdTwoSwp {
    using SafeERC20 for IERC20;
    function initialize(
        address _wantToken,
        address _router,
        ...
        commission = 20 * 10**16; //20%
        usdToken = 0x8f3Cf7ad23Cd3CaDbD9735AFf958023239c6A063; //DAI

        super.initialize(_wantToken, _router, _stakingRewards, _underlyingTokens, _rewardToken);
    }

    function migrateImpulse(address newStaking) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(address(stakingRewards) != newStaking, "Incorrect address");
        ...
        ///change contracts
        stakingRewards = IStakingRewards(newStaking);

        /// re-deposit all
        uint256 totBalance = wantToken.balanceOf(address(this));
        require(wantTotal == totBalance, "Incorrect amount");
        if (totBalance > 0) {
            _depositLpToken(totBalance);
        }
    }

    /// @notice Get rewards from staking contract.
    function _getRewards() internal virtual override {
        address dQuick = 0xf28164A485B0B2C90639E47b0f377b4a438a16B1;
        // Get dQuick
        stakingRewards.getReward();

        // turn dQuick into quick;
        uint256 curBalance = IERC20(dQuick).balanceOf(address(this));
        if (curBalance > 0) {
            IdQuick(dQuick).leave(curBalance);
        }
    }
}