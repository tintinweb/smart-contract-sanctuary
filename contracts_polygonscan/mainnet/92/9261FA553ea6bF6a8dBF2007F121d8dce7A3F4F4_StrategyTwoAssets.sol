...
abstract contract StrategyFarmTwoAssets is BaseStrategyMasterChef {
    ...
    address public constant fortoken = 0x546b4c391520E6652897c65153074088BFC0A909;
    address public multiHarvest = 0x0000000000000000000000000000000000000000;

    // Other constants
    uint16 public constant underlyingPoolId = 17;
        ...
        )
    {
        ...
        usdc_for_path = new address[](2);
        usdc_for_path[0] = usdc;
        usdc_for_path[1] = fortoken;
    }
    ...
    function harvest() public override {
        ...
        if (_dinoBalance > harvestCutoff){
            ...
            _swapUniswapWithPathQuick(usdc_for_path, _usdcBalancePost.div(2));

            // Add liquidity
            uint256 _usdc = IERC20(usdc).balanceOf(address(this));
            uint256 _for = IERC20(fortoken).balanceOf(address(this));
            if (_usdc > 0 && _for > 0) {
                IERC20(usdc).safeApprove(quickRouter, 0);
                IERC20(usdc).safeApprove(quickRouter, _usdc);
                IERC20(fortoken).safeApprove(quickRouter, 0);
                IERC20(fortoken).safeApprove(quickRouter, _for);
                ...
                IERC20(fortoken).safeTransfer(
                    strategist,
                    IERC20(fortoken).balanceOf(address(this))
                );
            }
        }
        // Stake the LP tokens
...
contract StrategyTwoAssets is StrategyFarmTwoAssets {
    ...
    address public FOR_USDC_LP = 0xA1c3eb6fE2bB452AaC4d9247478594bf04750017;
