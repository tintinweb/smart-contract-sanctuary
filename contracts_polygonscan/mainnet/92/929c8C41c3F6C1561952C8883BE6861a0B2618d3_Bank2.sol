...
contract Bank2 is Ownable, ReentrancyGuard {
    ...
    USTPool public usdcPool;
    IBEP20 public genesis = IBEP20(0x018F2Ca5c6A26B4923704f41D769281666B9D85e);
    IBEP20 public UST = IBEP20(0x692597b009d13C4049a947CAB2239b7d6517875F); // UST
    ...
    uint public entryMin = 1 ether; //min genesis to enroll lotterypot
    uint public lotsize;
...
*/
    ...
    constructor() public {
        paused = false;
        usdcPool.wkUnit = [0, 0, 0, 0];
        devaddr = address(0x548D885c0C5302227cE9A11e5F2B2F9ad77D97D6);
        wbnb.approve(dfynRouter, uint(- 1));
        UST.approve(dfynRouter, uint(- 1));
        lotlist.push(burnAddress);
    }
    ...
    function usdcinfo() public view returns (USTPool memory){
        return usdcPool;
    }
    ...
    function pendingReward(uint _pid, address _user) public view returns (uint256) {
        ...
        uint256 _accUSTPerShare = pool.accPerShare;
        ...
        if (block.timestamp > pool.lastRewardTime && pool.amount != 0) {
            ...
            _accUSTPerShare = _accUSTPerShare.add(Reward.mul(1e12).div(pool.amount));
        }
        return user.amount.mul(_accUSTPerShare).div(1e12).sub(userP.rewardDebt).div(1e18);
    }
    ...
    function pendingUST(address _user) public view returns (uint256){
        UserInfo storage user = userInfo[_user];
        uint256 _accUSTPerShare = usdcPool.accUSTPerShare;
        if (block.timestamp > usdcPool.lastRewardTime && totalAmount != 0) {
            uint256 multiplier = block.timestamp.sub(usdcPool.lastRewardTime);
            uint256 USTReward = multiplier.mul(usdcPool.usdcPerTime);
            _accUSTPerShare = _accUSTPerShare.add(USTReward.mul(1e12).div(totalAmount));
        }
        return user.amount.mul(_accUSTPerShare).div(1e12).sub(user.rewardDebt).div(1e18);
    }

    /**Public functions */

    function updateUSTPool() internal {
        ...
        usdcPool.accUSTPerShare = usdcPool.accUSTPerShare.add(usdcReward.mul(1e12).div(totalAmount));
        usdcPool.lastRewardTime = block.timestamp;
    }
    ...
    function deposit(uint256 _amount) public onlyApprovedContractOrEOA ispaused {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(usdcPool.accUSTPerShare).div(1e12).sub(user.rewardDebt);
            pending = pending.div(1e18);
            if (pending > 0) {
                safeUSTTransfer(msg.sender, pending);
            }
        }
        if (_amount > 0) {
            uint before = genesis.balanceOf(address(this));
            genesis.safeTransferFrom(address(msg.sender), address(this), _amount);
            genesis.safeTransfer(burnAddress, genesis.balanceOf(address(this)).sub(before));
        ...
        user.rewardDebt = user.amount.mul(usdcPool.accUSTPerShare).div(1e12);
        checkend();
    }
    ...
    function compound() public onlyApprovedContractOrEOA returns (uint){
        ...
        uint beforeSing = genesis.balanceOf(address(this));
        //wbnb=>genesis
        _safeSwap(dfynRouter, wbnb.balanceOf(address(this)).sub(before), address(wbnb), address(genesis));

        //UST=>genesis
        uint256 pending = user.amount.mul(usdcPool.accUSTPerShare).div(1e12).sub(user.rewardDebt);
        pending = pending.div(1e18);
        _safeSwap(dfynRouter, pending, address(UST), address(genesis));
        uint burningSing = genesis.balanceOf(address(this)).sub(beforeSing);
        ...
        genesis.transfer(burnAddress, burningSing);
    ...
    function addRepo(uint _amount) public {
        require(msg.sender == address(genesis) || msg.sender == owner() || msg.sender == devaddr, "not authorized to repo");
    ...
    function checkend() internal {//already updated pool above.
        ...
        if (endtime <= block.timestamp) {
            ...
            if (newRepo > bonus) {//BUSD decimal 18 in bsc. should change on other chains.
                safeUSTTransfer(msg.sender, bonus);
    ...
    function pickwin() internal {
        ...
        safeUSTTransfer(lotwinner, lotsize);
        lotsize = 0;
        lotstart += _mod;
    }
    ...
    function safeUSTTransfer(address _to, uint256 _amount) internal {
        uint256 balance = UST.balanceOf(address(this));
        if (_amount > balance) {
            lastpayout = balance;
            UST.safeTransfer(_to, balance);
            totalpayout = totalpayout.add(balance);
        } else {
            UST.safeTransfer(_to, _amount);
