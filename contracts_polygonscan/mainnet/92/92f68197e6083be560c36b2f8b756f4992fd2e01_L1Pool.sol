...
contract L1Pool is Initializable, OwnableUpgradeable, UUPSUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event CollateralWithDraw(address to, uint256 amount);
    event MaxL1SupplyChanged(uint256 from, uint256 to);
    ...
    // ratio = L1/USDC
    uint256 public ratio = 2e6;
    uint256 public ratioPrecision = 1e6;
    
    /// @notice total amount
    uint256 public MAX_L1_SUPPLY;
    ...
    modifier notRedeemPaused() {
        require(!redeemPaused, "Minting is paused");
        _;
    }
    ...
    function initialize (
        address _l1,
        address _collateral,
        address _liquidAlpha,
        uint256 _mintingFee,
        uint256 _redemptionFee,
        uint256 _maxL1Supply
    )  external initializer{
        __Ownable_init();
        __ReentrancyGuard_init();
        require(_l1 != address(0), "zeroAddress");
        require(_collateral != address(0), "zeroAddress");
        require(_liquidAlpha != address(0), "zeroAddress");
        l1 = IL1(_l1);
        collateral = ERC20(_collateral);
        liquidAlpha = ERC20(_liquidAlpha);
        missingDecimals = 10**(18 - collateral.decimals());
        mintingFee = _mintingFee;
        redemptionFee = _redemptionFee;
        MAX_L1_SUPPLY = _maxL1Supply;
    }

    /**
     * Swap collateral to L1
     */
    function mint(uint256 collateralAmount, uint8 nTransOptions,  bool bMint, bool bHolder) public nonReentrant notMintPaused {
        require(collateralAmount != 0, "invalidInput");
        uint256 feeAmount = collateralAmount.mul(mintingFee).div(PRECISION);
        uint256 l1Amount = collateralAmount.sub(feeAmount).mul(missingDecimals).mul(ratio).div(ratioPrecision);
        require(l1.totalSupply().add(l1Amount) <= MAX_L1_SUPPLY, "> maxL1Supply");
        if(bHolder == true)
            require((!isHolder(msg.sender)), "User is not $Liquid holder");
        collateral.increaseAllowance(address(this), collateralAmount);
        ...
        if(bMint == true)
            l1.mint(msg.sender, l1Amount);
    }

    
    /**
     * Swap collateral to an exact amount of L1
     */
    function mintExact(uint256 l1Amount) public nonReentrant notMintPaused {
        require(l1Amount != 0, "invalidOutput");
        require(l1.totalSupply().add(l1Amount) <= MAX_L1_SUPPLY, "> maxL1Supply");
        
        uint256 collateralAmount = l1Amount.div(missingDecimals).div(ratio).mul(ratioPrecision);
    ...
    /**
     * Swap L1 to collateral
     */
    function redeem(uint256 l1Amount, uint8 nTransOptions, bool bBurn) public nonReentrant notRedeemPaused {
        require(l1Amount != 0, "invalidInput");
        ...
        if(bBurn == true)
            l1.burn(msg.sender, l1Amount);
        if(nTransOptions == 0)
            collateral.safeTransfer(msg.sender, collateralAmountPostFee);
        if(nTransOptions == 1)
            collateral.transfer(msg.sender, collateralAmountPostFee);
    }

    /**
     * Swap L1 to exact amount of collateral
     */
    function redeemExact(uint256 collateralAmount) public nonReentrant notRedeemPaused {
        ...
        uint256 feeAmount = collateralAmountWithFee.sub(collateralAmount);
        uint256 l1Amount = collateralAmountWithFee.mul(missingDecimals).mul(ratio).div(ratioPrecision);
        feeBalance = feeBalance.add(feeAmount);
    ...
    function setMaxL1Supply(uint256 maxSupply) external onlyOwner {
        uint256 oldValue = MAX_L1_SUPPLY;
        MAX_L1_SUPPLY = maxSupply;
        emit MaxL1SupplyChanged(oldValue, maxSupply);
    }

    function toggleMinting(bool _mintPaused) external onlyOwner{
        mintPaused = _mintPaused;
        emit MintPaused(_mintPaused);
    }

    function toggleRedeem(bool _redeemPaused) external onlyOwner{
    ...
    function isHolder(address _address) public view returns (bool) {
        bool isHolded = false;
        if(liquidAlpha.balanceOf(_address) > 0) 
            isHolded = true;
        return isHolded;
    }

    function emergencyWithDrawCollateral(address to, uint256 amount) private onlyOwner {
        require(to != address(0), "zeroAddress");
        require(amount > 0 && amount <= collateral.balanceOf(address(this)), "invalid amount");
        collateral.safeTransfer(to, amount);
        emit CollateralWithDraw(to, amount);
    }
}