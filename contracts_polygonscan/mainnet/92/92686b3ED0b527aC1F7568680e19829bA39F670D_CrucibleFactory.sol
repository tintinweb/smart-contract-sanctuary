    ...
    WithAdmin
{
    ...
    function createCrucibleDirect(
        ...
        uint64 feeOnTransferX10000,
        uint64 feeOnWithdrawX10000
    ) public onlyAdmin returns (address token) {
        return
            _createCrucible(
                baseToken,
                name,
    ...
    /*
     * Tokens accumulated in the factory can be burned by anybody.
     */
...
interface ICrucibleFactory {
    function getCrucible(
		address baseToken,
        uint64 feeOnTransferX10000,
        uint64 feeOnWithdrawX10000) external view returns (address);
...
contract CrucibleToken is ERC20, TokenReceivable, ICrucibleToken {
    ...
    address /*immutable*/
        public
        override baseToken; // Remocing immutables to allow etherscan verification to work. Hopefully etherscan gives us a solution
    uint64 /*immutable*/
        public feeOnTransferX10000;
    uint64 /*immutable*/
        public feeOnWithdrawX10000;
    ...
    constructor() {
		...
		function upgradeRouter(address _router) external override onlyRouter {
    ...
    function withdraw(address to, uint256 amount)
        external
        override
        returns (uint256 fee, uint256 withdrawn)
    {
    ...
    /*
     * Burn the underlying asset. If not burnable, send to the factory.
     */
    ...
    function burnFrom(address from, uint256 amount) external virtual {
        require(from != address(0), "CT: amount required");
        require(amount != 0, "CT: amount required");
        uint256 decreasedAllowance = allowance[from][msg.sender].sub(
            amount,
    ...
    function doBurn(address from, uint256 amount) internal {
    ...
    function _transfer(
        ...
        fee = feeRatioX10k == 0 ? 0 : calculateFeeX10000(amount, feeRatioX10k);
...
interface ICrucibleToken {
	enum OverrideState { Default, OverrideIn, OverrideOut, OverrideBoth }
	function deposit(address to) external returns(uint256);
	function withdraw(address to, uint256 amount) external returns(uint256, uint256);
	function baseToken() external returns(address);
	function overrideFee(address target, OverrideState overrideType, uint64 newFeeX10000) external;
...
interface ICrucibleTokenDeployer {
    function parameters() external returns
		(address, address, uint64, uint64, string memory, string memory);
}
...
abstract contract TokenReceivable {
  ...
  function sync(address token) internal returns (uint256 amount) {
    ...
    amount = IERC20(token).balanceOf(address(this)).sub(inv);
    inventory[token] = inv.add(amount);
  }
