...
contract Marketplace is Context, Ownable, Initializable, IERC721Receiver {
    ...
    /**
     * @dev List ERC721 token in store.
    ...
    function listing(
        ...
        emit Listing(_msgSender(), _tokenAddress, _tokenId, _price);
    }
    ...
    function purchase(address _tokenAddress, uint256 _tokenId) external {
        ...
        if (transactionFee > 0) {
            ...
            acceptedToken.safeTransferFrom(address(this), _offer.seller, _offer.price.sub(_feeAmount));
        } else {
            acceptedToken.safeTransferFrom(address(this), _offer.seller, _offer.price);
        }
    ...
    event Listing(address indexed _user, address _tokenAddress, uint256 _tokenId, uint256 _price);
