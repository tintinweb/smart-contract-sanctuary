...
contract KingToken is ERC20("PolyKing Token", "KING") {
    ...
    */
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying KING (not scaled);
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of KING
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. KING to distribute per block.
        uint256 lastRewardBlock;  // Last block number that KING distribution occurs.
        uint256 accKingPerShare;   // Accumulated KING per share, times 1e18. See below.
    ...
    uint256 public kingPerBlock = 0.004 ether;
    ...
    event UpdateEmissionRate(address indexed user, uint256 kingPerBlock);
    ...
    // Update the given pool's KING allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending KING on frontend.
    function pendingKing(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accKingPerShare = accKingPerShare.add(kingReward.mul(1e18).div(lpSupply));
        }
        return user.amount.mul(accKingPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 kingReward = multiplier.mul(kingPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        king.mint(devAddress, kingReward.div(10));
        king.mint(address(this), kingReward);
        pool.accKingPerShare = pool.accKingPerShare.add(kingReward.mul(1e18).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accKingPerShare).div(1e18).sub(user.rewardDebt);
            if (pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accKingPerShare).div(1e18);
    ...
    function safeKingTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > kingBal) {
            transferSuccess = king.transfer(_to, kingBal);
        } else {
            transferSuccess = king.transfer(_to, _amount);
        }
        require(transferSuccess, "safeKingTransfer: Transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _kingPerBlock) external onlyOwner {
        massUpdatePools();
        kingPerBlock = _kingPerBlock;
        emit UpdateEmissionRate(msg.sender, _kingPerBlock);
    }
