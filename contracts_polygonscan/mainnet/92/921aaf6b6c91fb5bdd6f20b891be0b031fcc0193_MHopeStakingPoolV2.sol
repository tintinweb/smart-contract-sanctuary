...
contract MHopeStakingPoolV2 is OwnableUpgradeSafe, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        uint256 nonReentrantReward;
        uint256 nonReentrantRewardReleased;
        uint256 lastStakeTime;
    }

    // Info of reward pool funding (usdc)
    address public rewardToken = address(0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174); // USDC
    uint256 public lastRewardTime; // Last bnonReentrant number that rewardPool distribution occurs.
    uint256 public rewardPerSecond; // Reward token amount to distribute per bnonReentrant.
    uint256 public accRewardPerShare; // Accumulated rewardPool per share, times 1e30.
    ...
    function pendingReward(address _account) external view returns (uint256) {
        ...
        if (_endRewardTimeApplicable > _lastRewardTime && lpSupply != 0) {
            uint256 _incRewardPerShare = getRewardPerSecond(_lastRewardTime, _endRewardTimeApplicable).mul(1e30).div(lpSupply);
            _accRewardPerShare = _accRewardPerShare.add(_incRewardPerShare);
        }
        return user.amount.mul(_accRewardPerShare).div(1e30).add(user.reward).sub(user.rewardDebt);
    }
    ...
    // Deposit LP tokens
    function _deposit(address _account, uint256 _amount) internal nonReentrant {
        UserInfo storage user = userInfo[_account];
        getReward(_account);
        user.amount = user.amount.add(_amount);
        user.rewardDebt = user.amount.mul(accRewardPerShare).div(1e30);
    ...
    // Withdraw LP tokens.
    function _withdraw(address _account, uint256 _amount) internal nonReentrant {
        UserInfo storage user = userInfo[_account];
        getReward(_account);
        if (_amount > 0) {
    ...
    function getReward(address _account) public {
        ...
        UserInfo storage user = userInfo[_account];
        uint256 _accRewardPerShare = accRewardPerShare;
        uint256 _pendingReward = user.amount.mul(_accRewardPerShare).div(1e30).sub(user.rewardDebt);
        if (_pendingReward > 0) {
            address _rewardToken = rewardToken;
            user.accumulatedEarned = user.accumulatedEarned.add(_pendingReward);
            user.rewardDebt = user.amount.mul(_accRewardPerShare).div(1e30);
