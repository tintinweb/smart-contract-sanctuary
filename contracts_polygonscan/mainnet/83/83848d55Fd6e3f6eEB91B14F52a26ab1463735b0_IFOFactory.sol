...
contract IFOFactory is Ownable {
    address[] public allIFO;

    event NewIFOContract(address indexed IFO);

    constructor() public {
        //
    }

    function allIFOLength() external view returns (uint) {
        return allIFO.length;
    }

    /*
     * @notice Deploy the IFO
     ...
     * @return address of new IFO contract
     */
    function deployIFO(
        IBEP20 _lpToken,
        ...
        bytes32 salt = keccak256(abi.encodePacked(_lpToken, _offeringToken, _startTime, _endTime));
        address IFOAddress;
...
contract IFOInitializable is IIFO, ReentrancyGuard, Ownable {
    ...
    // The start timestamp when IFO starts
    uint256 public startTime;

    // The end timestamp when IFO ends
    ...
    function harvestPool(uint8 _pid) external override nonReentrant notContract {
        ...
        require(block.timestamp > endTime, "Too early to harvest");
    ...
    function setPool(
        ...
        require(block.timestamp < startTime, "IFO has started");
