...
contract PolyVaultsStrategy_IS3USD is IStrategy, Ownable, ReentrancyGuard, Pausable {
    ...
    address public uniRouterAddress = address(0xA102072A4C07F06EC3B4900FDC4C7B80b6c57429); // Dfyn: Router
    address public ironSwapRouterAddress = address(0x837503e8A8753ae17fB8C8151B8e6f586defCb57); // Quickswap: Router
    ...
    uint256 public controllerFee = 690; //6.9%
    ...
    event Compound(address token0Address, uint256 token0Amt);
    ...
    function inFarmBalance() public override view returns (uint256) {
        (uint256 amount, ) = IIronFarm(farmContractAddress).userInfo(pid, address(this));
        return amount;
    }
    ...
    function _farm() internal {
        ...
        IIronFarm(farmContractAddress).deposit(pid, wantAmt, address(this));
    ...
    function withdraw(address, uint256 _wantAmt) external override onlyController nonReentrant returns (uint256) {
        ...
        if (isAutoComp) {
            IIronFarm(farmContractAddress).withdraw(pid, _wantAmt, address(this));
        }
    ...
    function earn() public override whenNotPaused {
        require(isAutoComp, "PolyVaultsStrategy: !isAutoComp");
        require(!notPublic || isAuthorised(msg.sender), "PolyVaultsStrategy: !authorised");

        // Harvest farm tokens
        IIronFarm(farmContractAddress).harvest(pid, address(this));
        ...
        totalEarned = totalEarned.add(uniExchangeRate(remainAmt, paths[earnedAddress][usdcAddress]));

        IERC20(earnedAddress).safeIncreaseAllowance(uniRouterAddress, remainAmt);

        if (earnedAddress != token0Address) {
            // Swap half earned to token0
            IPancakeRouter02(uniRouterAddress).swapExactTokensForTokensSupportingFeeOnTransferTokens(remainAmt, 0, paths[earnedAddress][token0Address], address(this), block.timestamp + 60);
        }

        // Get want tokens, ie. add liquidity
        uint256 token0Amt = IERC20(token0Address).balanceOf(address(this));
        if (token0Amt > 0) {
            IERC20(token0Address).safeIncreaseAllowance(ironSwapRouterAddress, token0Amt);
            uint256[] memory amounts = new uint256[](3);
            amounts[0] = token0Amt; // usdc amount
            IIronSwapRouter(ironSwapRouterAddress).addLiquidity(amounts, token0Amt.mul(100).div(90), block.timestamp + 60);
            emit Compound(token0Address, token0Amt);
        }
    ...
    function buyBack1(uint256 _earnedAmt) internal returns (uint256) {
        ...
        if (earnedAddress != buyBackToken) {
            ...
            IPancakeRouter02(uniRouterAddress).swapExactTokensForTokensSupportingFeeOnTransferTokens(_buyBackAmt, 0, paths[earnedAddress][buyBackToken], buyBackAddress, block.timestamp + 60);
        } else {
    ...
    function pendingHarvest() public view returns (uint256) {
        ...
        return IIronFarm(farmContractAddress).pendingReward(pid, address(this)).add(_earnedBal);
    }
