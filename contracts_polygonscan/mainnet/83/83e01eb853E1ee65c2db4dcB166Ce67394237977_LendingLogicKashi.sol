...
// File contracts/Interfaces/IKashiPair.sol
...
// File contracts/Interfaces/ILendingLogic.sol
...
// File contracts/LendingLogicKashi.sol
...
contract LendingLogicKashi is ILendingLogic {
    ...
    function getAPRFromWrapped(address _token) public view override returns(uint256) {
        ...
        return uint256(IKashiPair(wrapped).accrueInfo().interestPerSecond) * 3154e7;
    }
    ...
    function lend(address _underlying,uint256 _amount, address _tokenHolder) external view override returns(address[] memory targets, bytes[] memory data) {
        ...
        // Deposit into BentoBox
        targets[2] = address(bentoBox);
        data[2] =  abi.encodeWithSelector(IBentoBoxV1.deposit.selector, _underlying, _tokenHolder, _tokenHolder,_amount,0);
        
        // Deposit into Kashi Lending Strategy
        targets[3] = kaToken;
        data[3] =  abi.encodeWithSelector(IKashiPair.addAsset.selector, _tokenHolder, false, _amount);
    ...
    function unlend(address _wrapped, uint256 _amount,address _tokenHolder) external view override returns(address[] memory targets, bytes[] memory data) {
        ...
        //Get underlying of wrapped tokens
        address underlying = address(IKashiPair(_wrapped).asset());
        
        //Get underlying asset amount from wrapped amount
        uint256 underlyingAmount = (exchangeRateView(_wrapped) * _amount) / (1e18);
        
        targets = new address[](3);
        data = new bytes[](3);
        
        targets[0] = _wrapped;
        data[0] =  abi.encodeWithSelector(IKashiPair.accrue.selector);

        // Withdraw from Kashi Lending Strategy
        targets[1] = _wrapped;
        data[1] =  abi.encodeWithSelector(IKashiPair.removeAsset.selector, _tokenHolder, _amount);
        
        // Remove from BentoBox
        targets[2] = address(bentoBox);
        data[2] =  abi.encodeWithSelector(IBentoBoxV1.withdraw.selector, underlying, _tokenHolder, _tokenHolder, 0, underlyingAmount);
