...
contract CIVNFT is ERC721("CIVNFT Token","CIVNFT"), Ownable {
	using ECDSA for bytes32;
	using SafeMath for uint256;

	mapping(address => uint256) public addressToCIVNFT;
	...
	function mint() external {
		...
		require(addressToCIVNFT[msg.sender] == 0, "Receiver already has a CIVNFT");
		addressToCIVNFT[msg.sender] = supply;
		_mint(msg.sender, supply);
	}

	function mintFor(address _owner) external onlyOwner {
		uint256 supply = totalSupply() + 1;
		require(addressToCIVNFT[_owner] == 0, "Receiver already has a CIVNFT");
		addressToCIVNFT[_owner] = supply;
		_mint(_owner, supply);
	}
