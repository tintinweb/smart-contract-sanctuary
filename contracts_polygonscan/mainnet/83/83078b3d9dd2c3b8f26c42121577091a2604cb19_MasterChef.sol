...
// Panda matic with Governance.
contract Newtoken is BEP20('ScamToken', 'Scam') {
...
// MasterChef is the master of MARIO. He can make MARIO and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once MARIO is sufficiently
...
contract MasterChef is Ownable {
    ...
    // The EGG TOKEN!
    Newtoken public MARIO;
    // Dev address.
    address public devaddr;
    // EGG tokens created per block.
    uint256 public MARIOPerBlock;
    // Bonus muliplier for early MARIO makers.
    ...
    constructor(
        ...
        address _devaddr,
        address _feeAddress,
        uint256 _MARIOPerBlock,
        uint256 _startBlock
    ) public {
        MARIO = _MARIO;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        MARIOPerBlock = _MARIOPerBlock;
    ...
    function pendingEgg(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 MARIOReward = multiplier.mul(MARIOPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accEggPerShare = accEggPerShare.add(MARIOReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 MARIOReward = multiplier.mul(MARIOPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        MARIO.mint(devaddr, MARIOReward.div(10));
        MARIO.mint(address(this), MARIOReward);
        pool.accEggPerShare = pool.accEggPerShare.add(MARIOReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    // Safe MARIO transfer function, just in case if rounding error causes pool to not have enough EGGs.
    function safeEggTransfer(address _to, uint256 _amount) internal {
        uint256 MARIOBal = MARIO.balanceOf(address(this));
        if (_amount > MARIOBal) {
            MARIO.transfer(_to, MARIOBal);
        } else {
            MARIO.transfer(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _MARIOPerBlock) public onlyOwner {
        massUpdatePools();
        MARIOPerBlock = _MARIOPerBlock;
    }
