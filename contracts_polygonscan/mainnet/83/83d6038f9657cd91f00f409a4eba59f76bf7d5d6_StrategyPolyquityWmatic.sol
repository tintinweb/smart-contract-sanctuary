...
abstract contract BaseStrategyPolyquityRewards is BaseStrategy {
    ...
    function _getReward() internal {
        IPolyquityRewards(rewards).claimReward();
    }
...
contract StrategyPolyquityWmatic is BaseStrategyPolyquityRewards {

    //Note: PYQ currently has a 25% transfer fee
    address public PYQ = 0x5a3064CbDCCF428ae907796cF6aD5a664CD7F3d8;
    address public USDC = 0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174;
    address public PUSD = 0x9aF3b7DC29D3C4B1A5731408B6A9656fA7aC3b72;
    ...
    //for PUSD-USDC swap PYQ -> USDC -> PUSD
    constructor()
        public
        BaseStrategyPolyquityRewards(
            0x62dcaC823D1f6d27Fb5e2d0728887D5531A29A7A, //rewards
            0x022268Fc83395ACE2BD0129A8D752E35C24Ff90d, //lp
        ...
        )
    {
        ...
        reward_matic_path = new address[](3);
        reward_matic_path[0] = PYQ;
        reward_matic_path[1] = USDC;
        reward_matic_path[2] = WMATIC;
    }
    ...
    function harvest() public override onlyHumanOrWhitelisted nonReentrant {
        ...
        if (amountHarvested > 0) {
            //Swap all PYQ that was harvested during this harvest() call to WMATIC
            //All PYQ liquidity is on Sushiswap, so I need to use Sushiswap
            _swapUniswapWithPathForFeeOnTransferTokens(reward_matic_path, amountHarvested, SUSHI_ROUTER);
            //Deduct the perf fee from the WMATIC we got
            uint256 _maticFee = IERC20(WMATIC).balanceOf(address(this)).mul(IERCFund(strategist).getFee()).div(keepMax);
            _notifyJar(_maticFee);
            IERC20(WMATIC).safeTransfer(strategist, _maticFee);
        }

        //Swap any PYQ in the contract that wasn't obtained from the previous harvest() call to WMATIC as well
        uint256 remainingHarvested = IERC20(harvestedToken).balanceOf(address(this));
        if (remainingHarvested > 0) {
            _swapUniswapWithPathForFeeOnTransferTokens(reward_matic_path, remainingHarvested, SUSHI_ROUTER);
        }

        //Swap 1/2 of WMATIC for PUSD on Quickswap
