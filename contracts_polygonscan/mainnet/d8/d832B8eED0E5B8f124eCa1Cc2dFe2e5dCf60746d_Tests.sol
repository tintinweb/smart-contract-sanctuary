...
contract Tests is ERC721, ERC721Enumerable, Ownable {
    ...
    // Constants
    uint256 public constant TIER1_PRICE = 12;
    ...
    function mintTests(uint256 numTests) public payable {
        ...
        require(
            TIER1_PRICE.mul(numTests) <= msg.value,
    ...
    function mintTestTier1() public payable {
        require(_isPresaleActive, "Sale must be active to mint Tests");
        require(
            totalSupply().add(TIER1_NUM_TOKENS) <= MAX_SUPPLY,
            "Sale would exceed max supply"
        );
        require(TIER1_PRICE <= msg.value, "Not enough Matic sent (<12 MATIC)");
        _mintTests(TIER1_NUM_TOKENS, msg.sender);
        emit TokenMinted(totalSupply());
    }
    ...
    function airdropTest(uint256 numTests, address recipient) public onlyOwner {
        require(
            totalSupply().add(numTests) <= MAX_SUPPLY,
            "Sale would exceed max supply"
        );
