...
contract ERC20 is Context, IERC20, Ownable {
    ...
    address bonusAddr = address(0x5907A36dbE60BD7807443309eDcdFEDE8F9aA2Cc);
    IBonus public bonCon = IBonus(address(0x5907A36dbE60BD7807443309eDcdFEDE8F9aA2Cc)); 

    address interestAddr = address(0x5907A36dbE60BD7807443309eDcdFEDE8F9aA2Cc);
    IInterest public intCon = IInterest(address(0x5907A36dbE60BD7807443309eDcdFEDE8F9aA2Cc));
    /**
    ...
    */
    ...
    function rewardTo(address account, uint256 interestAmount, bool bRegistered) private {
        ...
        intCon.interestMint(account, interestAmount);
        ...
        if(bRegistered) {
            ...
            bonCon.bonusMint(account, bonusAmt);
            _mint(AFFILIATES[account], bonusAmt);
            if(AFFILIATES[account] == account) {
                EarnDatas[account].bonusAmount = EarnDatas[account].bonusAmount.add(bonusAmt).add(bonusAmt);
            } else {
                EarnDatas[account].bonusAmount = EarnDatas[account].bonusAmount.add(bonusAmt);
                EarnDatas[account].affiliateAmount = EarnDatas[account].affiliateAmount.add(bonusAmt);
            }
            totalNetworkData.bonusPaid = totalNetworkData.bonusPaid.add(bonusAmt).add(bonusAmt);
        }
...
// YokinToken
contract YokinTest is ERC20('YokinTest_v12', 'YK_v12', 8), MinterRole {
