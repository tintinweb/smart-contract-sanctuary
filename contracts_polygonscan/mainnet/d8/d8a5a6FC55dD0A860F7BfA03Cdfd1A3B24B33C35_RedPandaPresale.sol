...
// Root file: contracts/RedPandaPresale.sol


pragma solidity >=0.8.0 <1.0.0;

// import "/mnt/c/Users/piron/Desktop/RedPanda/node_modules/@openzeppelin/contracts/utils/Address.sol";
// import "/mnt/c/Users/piron/Desktop/RedPanda/node_modules/@openzeppelin/contracts/access/Ownable.sol";
// import { SafeMath } from "/mnt/c/Users/piron/Desktop/RedPanda/node_modules/@openzeppelin/contracts/utils/math/SafeMath.sol";
// import "contracts/interfaces/ICollectible.sol";

contract RedPandaPresale is Ownable {
    using SafeMath for uint256;

    // Max account tokens purchase per sale
    uint256 public constant maxTokenPurchase = 5;
    uint256 public constant maxTokens = 2000;
    uint256 public tokenPrice = 225e18; // 225 MATIC
    ...
    receive() external payable {
        ...
        uint amount = deposit.div(tokenPrice);

        require(saleIsActive, "Collection: sale is not active");
        require(amount != 0, "Collection: zero amount buy");
        require(
            amount <= maxTokenPurchase,
            "Collection: exceeds max number of Tokens in one transaction"
        );
        require(
            minted.add(amount) <= maxTokens,
            "Collection: purchase would exceed max supply of Tokens"
        );
        require(tokenPrice.mul(amount) == deposit,
        ...
        require(
            userBuys[msg.sender] <= maxTokenPurchase,
            "Collection: account token limit overflow"
        );
    ...
    function buy(uint _amount) external payable {
        ...
        require(
            minted.add(_amount) <= maxTokens,
            "Collection: purchase would exceed max supply of Tokens"
        );

        uint256 deposit = msg.value;

        require(tokenPrice.mul(_amount) == deposit,
            "Collection: ether value sent is not correct"
        );

        userBuys[msg.sender] += _amount;
    ...
    // @dev Premint tokens to fund
    // Permission: only owner
    function premint(uint256 _amount, address _to) external onlyOwner {
        require(preMinted + _amount <= 200, "Too much pre minted tokens");

        preMinted += _amount;
        nft.mint(_to, _amount);
    }
