...
contract GravityStrategy is BaseStrategy {
    ...
    address private constant GRAVITY_ROUTER = 0x57dE98135e8287F163c59cA4fF45f1341b680248;
    ...
    constructor(
        address _rewards,
        address _want,
        address _tokenA, //first token swapped to
        address _tokenB, //50% swapped to B
        address _betweenToken //token between GFI and A in swap if necessary
    )
        public
        BaseStrategy(
            _want,
        ...
        )
    {
        ...
        if(_betweenToken == address(0)) {
            reward_a_path = [GFI, _tokenA];
            a_b_path = [_tokenA, _tokenB];
        }
        else {
            reward_a_path = [GFI, _betweenToken, _tokenA];
            a_b_path = [_tokenA, _tokenB];
        }

        //Currently no pair for GFI/WMATIC, so route through GFI -> WETH -> WMATIC
        reward_matic_path = [GFI, WETH, WMATIC];

        IERC20(want).safeApprove(rewards, uint256(-1));
        IERC20(GFI).safeApprove(GRAVITY_ROUTER, uint256(-1));
        if(GFI != _tokenA) IERC20(_tokenA).safeApprove(GRAVITY_ROUTER, uint256(-1));
        IERC20(_tokenB).safeApprove(GRAVITY_ROUTER, uint256(-1));
    }
