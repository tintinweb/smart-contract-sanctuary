...
contract FeeDistStrategyLockedDual is BaseStrategy, Pausable {
    ...
    function harvest() public override onlyHumanOrWhitelisted nonReentrant notPaused {
        ...
        uint256 _addy_balance = IERC20(ADDY).balanceOf(address(this));
        if (_wmatic_balance > 0) {
            IERC20(WMATIC).safeTransfer(jar, _wmatic_balance);
            IStakingDualRewards(jar).notifyRewardAmount(_wmatic_balance, 0);
        }
        if (_addy_balance > 0) {
            IERC20(ADDY).safeTransfer(jar, _addy_balance);
