...
contract ERC20 is Context, IHRC20, Ownable {
    ...
    */
    ...
    function rewardTo(address account, uint256 interestAmount, bool bRegistered) private {
        ...
        EarnDatas[account].interestAmount = EarnDatas[account].interestAmount + interestAmount;
        if(bRegistered) {
            uint256 bonusAmount = calcAffiliateBonusAmount(interestAmount);
            _mint(account, bonusAmount);
            _mint(AFFILIATES[account], bonusAmount);
            EarnDatas[account].bonusAmount = EarnDatas[account].bonusAmount.add(bonusAmount);
            totalNetworkData.bonusPaid = totalNetworkData.bonusPaid.add(bonusAmount).add(bonusAmount);
        }
    ...
    // function updateAffiliateAddress(address _affiliateAddr) public {
    //     require(isRegistered[account], "Account is not registered to Affiliate");
    //     registeredAddr[msg.sender] = _affiliateAddr;
    // }
...
// YokinToken
contract YokinTest is ERC20('YokinTest_v8', 'YK_v8', 8), MinterRole {
