...
contract SushiswapWorker is OwnableUpgradeSafe, ReentrancyGuardUpgradeSafe, IWorker {
  ...
  function reinvest() external override onlyEOA onlyReinvestor nonReentrant {
    ...
    wNative.safeApprove(address(router), uint256(-1));
    ...
    uint256 rewardNative = wNative.balanceOf(address(this));
    if (reward == 0 && rewardNative == 0) return;
    ...
    uint256 bountyNative = rewardNative.mul(reinvestBountyBps) / 10000;
    if(bountyNative > 0) wNative.safeTransfer(msg.sender, bountyNative);
    // 4. Convert all the remaining rewards to BaseToken via Native for liquidity.
    address[] memory path;
    address[] memory pathNative;
    if (baseToken == wNative) {
      ...
      pathNative = new address[](2);
      pathNative[0] = address(wNative);
      pathNative[1] = address(baseToken);
      router.swapExactTokensForTokens(rewardNative.sub(bountyNative), 0, pathNative, address(this), now);
    }
    ...
    wNative.safeApprove(address(router), 0);
    address(lpToken).safeApprove(address(masterChef), 0);
    emit Reinvest(msg.sender, reward, bounty);
    emit Reinvest(msg.sender, rewardNative, bountyNative);
  }
