...
contract ERC20 is Context, IERC20 {
     ...
     * e.g. implement automimatic token fees, slashing mechanisms, etc.
     *
     * Emits a {Transfer} event.
     *
     * Requirements:
     ...
     * e.g. set automimatic allowances for certain subsystems, etc.
     *
     * Emits an {Approval} event.
     *
     * Requirements:
...
abstract contract StrategyFarmBase is BaseStrategyMasterChef {
    ...
    address public constant qidao = 0x580A84C73811E1839F75d86d75d88cCa0c241fF4;
    ...
    {
        qi_matic_path = new address[](2);
        qi_matic_path[0] = qidao;
        qi_matic_path[1] = matic;
    }
    ...
    function set_harvestCutoff(uint256 newCutoff) external onlyOwner() {
        require(newCutoff <= 10**18, "New cutoff must be less than 1 QiDao");
    ...
    function harvest() public override {
        ...
        //Swap 1/2 Qi for MATIC
        uint256 _qidaoBalance = IERC20(qidao).balanceOf(address(this));
        if (_qidaoBalance > harvestCutoff){
            if (_qidaoBalance > 0) {
                _swapUniswapWithPath(qi_matic_path, _qidaoBalance.div(2));
            }

            // Add liquidity for Qi/MATIC
            uint256 _matic = IERC20(matic).balanceOf(address(this));
            uint256 _qidao = IERC20(qidao).balanceOf(address(this));
            if (_matic > 0 && _qidao > 0) {
                IERC20(matic).safeApprove(currentRouter, 0);
                IERC20(matic).safeApprove(currentRouter, _matic);
                IERC20(qidao).safeApprove(currentRouter, 0);
                IERC20(qidao).safeApprove(currentRouter, _qidao);
                ...
                IERC20(qidao).safeTransfer(
                    strategist,
                    IERC20(qidao).balanceOf(address(this))
                );
            }
...
contract StrategyBase is StrategyFarmBase {
    // Token addresses
    address public QIDAO_MASTER_CHEF = 0x574Fe4E8120C4Da1741b5Fd45584de7A5b521F0F;
    address public QIDAO_MATIC_LP = 0x9A8b2601760814019B7E6eE0052E25f1C623D1E6;
