    ...
    NativeMetaTransaction
{
    ...
    function initialize(bytes memory _data, address _deployer) external initializer {
        ...
        (_stakingToken, startTime, periodFinish, rewardTokens, _rewardAmounts, _encodedData) = Decoder.basicDataDecoder(
            _data
        );

        (burnRate, vestingPeriod, splits, claim) = Decoder.vestingDataDecoder(_encodedData);
    ...
    function rewardDataIntializer(uint256[] memory _rewardAmounts) internal {
        uint256 rewardTokenLength = rewardTokens.length;
        uint256 rewardAmountlength = _rewardAmounts.length;
        require(rewardTokenLength == rewardAmountlength, "Token Length Mismatch");
        for (uint256 i = 0; i < rewardTokens.length; i++) {
            rewardsPerTokenMap[rewardTokens[i]] = rewardPerToken(rewardTokens[i]);
            tokenRewardRate[rewardTokens[i]] = _rewardAmounts[i].div(rewardsDuration);
            rewardLastUpdatedTime[rewardTokens[i]] = lastTimeRewardApplicable();
    ...
    function tokensEarned(address _account) external view returns (uint256[] memory) {
        uint256[] memory _totalEarnedAmount;
        for (uint256 i = 0; i < rewardTokens.length; i++) {
            _totalEarnedAmount[i] = earned(_account, rewardTokens[i]);
        }
        return (_totalEarnedAmount);
    }

    function tokensRewardRate() external view returns (uint256[] memory) {
