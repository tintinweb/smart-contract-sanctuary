...
// File contracts/strategies/GrowStrategyWaultV4.sol
...
contract GrowStrategyWaultV4 is IGrowUpgradeableImplementation, BaseGrowStrategyV4 {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    bytes32 override public constant CONTRACT_IDENTIFIER = keccak256("GrowStrategyWaultV4");
    ...
    function initialize(
        ...
        (address _STAKING_TOKEN,,,) = IMasterChefLike(_MASTER_CHEF_LIKE).poolInfo(_MASTER_CHEF_LIKE_POOL_ID);
    ...
    function updateUnderlyingRewardToken(address _UNDERLYING_REWARD_TOKEN) external onlyRole(CONFIGURATOR_ROLE) {
        UNDERLYING_REWARD_TOKEN = _UNDERLYING_REWARD_TOKEN;
    }

    function updateUnderlyingRewardTokenAmount(uint256 _UNDERLYING_REWARD_TOKEN_AMOUNT) external onlyRole(CONFIGURATOR_ROLE) {
        UNDERLYING_REWARD_TOKEN_AMOUNT = _UNDERLYING_REWARD_TOKEN_AMOUNT;
    }
    ...
    function _trySwapUnderlyingRewardToRewardToken() internal override {
        ...
        uint256 platformFeeAmount = UNDERLYING_REWARD_TOKEN_AMOUNT.mul(platformFeeRate).div(10000);
        IERC20(UNDERLYING_REWARD_TOKEN).safeTransfer(GrowRegister.get.PlatformTreasureAddress(), platformFeeAmount);

        // charge PerformanceFee
        uint256 performanceFeeAmount = UNDERLYING_REWARD_TOKEN_AMOUNT.mul(performanceFeeRate).div(10000);
        _chargePerformanceFee(UNDERLYING_REWARD_TOKEN, performanceFeeAmount);

        // reinvest rest UNDERLYING_REWARD_TOKEN_AMOUNT
        _reinvest(UNDERLYING_REWARD_TOKEN, UNDERLYING_REWARD_TOKEN_AMOUNT.sub(performanceFeeAmount).sub(platformFeeAmount));
    ...
    function _reinvest(address tokenAddress, uint256 tokenAmount) internal {
        ...
        uint256 stakingTokenAmount = IZAP(GrowRegister.get.ZapAddress()).swap(tokens, tokenAmount, address(this), 0);
    ...
    function _depositUnderlying(uint256 amount) internal override returns (uint256) {
        ...
        IMasterChefLike(MASTER_CHEF_LIKE).deposit(MASTER_CHEF_LIKE_POOL_ID, amount, true);
    ...
    function _withdrawUnderlying(uint256 amount) internal override returns (uint256) {
        ...
        IMasterChefLike(MASTER_CHEF_LIKE).withdraw(MASTER_CHEF_LIKE_POOL_ID, amount, false);
    ...
    function _wantTokenPriceIn1e6USDC(uint256 amount) public view override returns (uint256) {
        return IPriceCalculator(GrowRegister.get.PriceCalculatorAddress()).tokenPriceIn1e6USDC(STAKING_TOKEN, amount);
    }
    ...
    function _harvestFromUnderlying() internal {
        uint256 underlyingRewardAmountBefore = IERC20(UNDERLYING_REWARD_TOKEN).balanceOf(address(this));
        IMasterChefLike(MASTER_CHEF_LIKE).claim(MASTER_CHEF_LIKE_POOL_ID);
        uint256 underlyingRewardAmountAfter = IERC20(UNDERLYING_REWARD_TOKEN).balanceOf(address(this));
        UNDERLYING_REWARD_TOKEN_AMOUNT = UNDERLYING_REWARD_TOKEN_AMOUNT.add(underlyingRewardAmountAfter.sub(underlyingRewardAmountBefore));
    }
    ...
    function emergencyExit() external override onlyRole(CONFIGURATOR_ROLE) {
        IMasterChefLike(MASTER_CHEF_LIKE).withdraw(MASTER_CHEF_LIKE_POOL_ID, type(uint256).max, true);
    ...
    function migrateToNewPool(uint256 _NEW_MASTER_CHEF_LIKE_POOL_ID) external onlyRole(CONFIGURATOR_ROLE) {
        (address _STAKING_TOKEN,,,) = IMasterChefLike(MASTER_CHEF_LIKE).poolInfo(_NEW_MASTER_CHEF_LIKE_POOL_ID);
        require(_STAKING_TOKEN == STAKING_TOKEN);
        uint preMigrationTotal = _underlyingWantTokenAmount();
        IMasterChefLike(MASTER_CHEF_LIKE).withdraw(MASTER_CHEF_LIKE_POOL_ID, type(uint256).max, true);

        MASTER_CHEF_LIKE_POOL_ID = _NEW_MASTER_CHEF_LIKE_POOL_ID;

        uint256 currentBalance = IERC20(STAKING_TOKEN).balanceOf(address(this));
        approveToken(STAKING_TOKEN, MASTER_CHEF_LIKE, currentBalance);
        IMasterChefLike(MASTER_CHEF_LIKE).deposit(_NEW_MASTER_CHEF_LIKE_POOL_ID, currentBalance, true);

        require(_underlyingWantTokenAmount() >= preMigrationTotal.mul(999).div(1000));
    }
}