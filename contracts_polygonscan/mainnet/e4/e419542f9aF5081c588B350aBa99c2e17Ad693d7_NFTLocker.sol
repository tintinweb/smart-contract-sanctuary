...
contract NFTLocker is IERC721Receiver, OwnableUpgradeable, ReentrancyGuard {
    ...
    function lock(uint256 tokenId) external nonReentrant {
        ...
        if (_locked[msg.sender] != 0) {
            _nft.safeTransferFrom(address(this), msg.sender, _locked[msg.sender]);
        }
        _nft.safeTransferFrom(msg.sender, address(this), tokenId);
        _locked[msg.sender] = tokenId;
    }
    ...
    function rescueStuckErc721(uint256 _tokenId, address receiver) external onlyOwner {
        BRC721 _nft = BRC721(nft_);
        _nft.safeTransferFrom(address(this),receiver, _tokenId);
    }
}