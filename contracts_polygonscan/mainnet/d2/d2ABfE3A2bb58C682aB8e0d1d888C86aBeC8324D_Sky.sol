...
contract Sky is Ownable, ISky {
   ...
   function getMatic(uint256 amount) public override onlyApprovedAddress returns(uint256){
       if(address(this).balance >= amount){
           ...
           Cloud(payable(msg.sender)).incWaitRounds();
           return amount;
       }
       return 0;
   }
...
contract Cloud is VRFConsumerBase, Ownable {
    ...
    function pick(uint256 randomValue) public view returns(uint256, uint256){
        ...
        uint256 pickMax = (uint256(keccak256(abi.encode(randomValue, randomIndex * 2))) % maxPicksModulo + addonPicks) % 36;
    ...
    function roll(uint256 randomValue) internal {
        ...
        if(rolls.length == 1 || rolls.length > lastRollCounted){
            if(polyroll.bets(rolls[rolls.length-1]).winAmount > 0){
                ...
                if(alwaysPicked.length < 13){
                    ...
                    alwaysPicked[lastAlwaysPickedReplaced % (alwaysPicked.length - 1) + 1] = polyroll.bets(rolls[rolls.length-1]).outcome % 37;
        ...
        if(wasPendingBet > waitForRounds || pendingBet == 0){
            uint256 betAmount = Utils.calculatePercentageCents(address(this).balance,
                                                               balancePercentage.add(
                                                                                     uint256(numbers % 37).mul(10)
                                                                                     ));
