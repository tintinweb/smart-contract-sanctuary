...
contract AvxReseveContract {
    ...
    function updateClaimAddressNew(address addr) public {
        require(msg.sender == owner, "Permission error");
        Claim storage clm = claim[claimAddress];
        for(uint i=0; i < clm.amounts.length;i++){
            claim[addr].amounts.push(clm.amounts[i]);
            claim[addr].times.push(clm.times[i]);
            claim[addr].withdrawn.push(clm.withdrawn[i]);
        }
        delete claim[claimAddress];
        claimAddress = addr;
    }
