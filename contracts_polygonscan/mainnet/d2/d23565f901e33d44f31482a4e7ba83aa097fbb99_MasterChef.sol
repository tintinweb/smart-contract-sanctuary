...
contract MEGAToken is ERC20("MEGA", "MEGA"), Ownable {
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of MEGAes
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. MEGAes to distribute per block.
        uint256 lastRewardBlock;  // Last block number that MEGAes distribution occurs.
        uint256 accMEGAPerShare;   // Accumulated MEGAes per share, times 1e18. See below.
    ...
    uint256 public MEGAPerBlock = 1 ether;
    ...
    // MEGA referral contract address.
    ...
    // View function to see pending MEGAes on frontend.
    function pendingMEGA(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accMEGAPerShare = accMEGAPerShare.add(MEGAReward.mul(1e18).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 MEGAReward = multiplier.mul(MEGAPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        MEGA.mint(devAddress, MEGAReward.div(10));
        MEGA.mint(address(this), MEGAReward);
        pool.accMEGAPerShare = pool.accMEGAPerShare.add(MEGAReward.mul(1e18).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    // Safe MEGA transfer function, just in case if rounding error causes pool to not have enough FOXs.
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (address(referral) != address(0) && referralCommissionRate > 0) {
            ...
            if (referrer != address(0) && commissionAmount > 0) {
                MEGA.mint(referrer, commissionAmount);
