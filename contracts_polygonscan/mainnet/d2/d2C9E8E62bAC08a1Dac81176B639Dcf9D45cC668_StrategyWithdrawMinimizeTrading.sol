...
contract StrategyWithdrawMinimizeTrading is Ownable, ReentrancyGuard, Strategy {
  ...
  function execute(
    ...
    sell(router, debt, fToken,banktoken);
    // 4. Return banktoken back to the original caller. 5. Return remaining farming tokens to user.
    sendRemaining(banktoken, fToken, minFToken, user);
  }
  
  function sell(IUniswapV2Router02 router, uint debt, address fToken, address banktoken) internal{
  ...
  function sendRemaining(address banktoken, address fToken, uint minFToken, address user) internal{
