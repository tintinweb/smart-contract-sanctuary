...
contract MstableStrategy is BaseStrategy {

    uint256 public constant keepMax = 10000;

    address MTA = 0xF501dd45a1198C2E1b5aEF5314A68B9006D842E0;
    address WMATIC = 0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270;
    address USDC = 0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174;

    address mUSD = 0xE840B73E5287865EEc17d250bFb1536704B43B21;
    address imUSD = 0x5290Ad3d83476CA6A2b178Cd9727eE1EF72432af;

    address QUICKSWAP_ROUTER = 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff;
    address imUSD_VAULT = 0x32aBa856Dc5fFd5A56Bcd182b13380e5C855aa29;
    address SAVE_WRAPPER = 0x299081f52738A4204C3D58264ff44f6F333C6c88;

    address FUND = 0x01fE07ce760DA7a025e4Cf3f950aE236F8e62120;

    // Uniswap swap paths
    address[] public matic_usdc_path;
        ...
        )
    {
        ...
        reward_matic_path[0] = MTA;
        reward_matic_path[1] = WMATIC;

        matic_usdc_path = new address[](2);
        matic_usdc_path[0] = WMATIC;
        matic_usdc_path[1] = USDC;

        IERC20(want).safeApprove(rewards, uint256(-1));

        IERC20(MTA).safeApprove(QUICKSWAP_ROUTER, uint256(-1));
        IERC20(WMATIC).safeApprove(QUICKSWAP_ROUTER, uint256(-1));
        IERC20(USDC).safeApprove(SAVE_WRAPPER, uint256(-1));
    }
    ...
    function harvest() public override onlyHumanOrWhitelisted nonReentrant {
        //Transfer any WMATIC/MTA that may already be in the contract to the fee dist fund
        uint256 reward_bal = IERC20(harvestedToken).balanceOf(address(this));
        if(reward_bal > 0) {
            IERC20(harvestedToken).safeTransfer(strategist, reward_bal);
        }
        ...
        //Harvest reward and convert MTA to WMATIC
        _getReward();
        uint256 amountHarvested = IERC20(harvestedToken).balanceOf(address(this));
        if (amountHarvested > 0) {
            _swapUniswapWithPathPreapproved(reward_matic_path, amountHarvested);
        }
        uint256 maticHarvested = IERC20(WMATIC).balanceOf(address(this));
        if (maticHarvested > 0) {
            //Deduct performance fee from total WMATIC
            uint256 feeAmount = maticHarvested.mul(IERCFund(strategist).getFee()).div(keepMax);
            uint256 remaining = maticHarvested.sub(feeAmount);
            _notifyJar(feeAmount);
            IERC20(WMATIC).safeTransfer(strategist, feeAmount);
            //Swap remaining WMATIC to USDC
            _swapUniswapWithPathPreapproved(matic_usdc_path, remaining);
        }

        //Deposit USDC in pool to get imUSD
        uint256 _usdcBalance = IERC20(USDC).balanceOf(address(this));
        if (_usdcBalance > 0) {
            ISaveWrapper(SAVE_WRAPPER).saveViaMint(
                mUSD,
                imUSD,
                imUSD_VAULT,
                USDC,
    ...
    function deposit() public override {
        uint256 _want = IERC20(want).balanceOf(address(this));
        if (_want > 0) {
            IStakingRewards(rewards).stake(address(this), _want);
        }
    }
    ...
    function _getReward() internal {
        IStakingRewards(rewards).claimReward();
    }
