...
contract swapContract is AccessControl, Pausable, ECDSAOffsetRecovery{
        ...
        )
    {
        ...
        if (params.path.length == 1){
            require(RBCToken.transferFrom(blockchainPool, params.user, params.amountOutMin),
                                                            "swapContract: transfer from pool failed");
            emit TransferFromOtherBlockchain(params.user, params.amountOutMin, params.amountOutMin, params.originalTxHash);
        }
        else if (params.path.length > 1){
            ...
            emit TransferFromOtherBlockchain(params.user, RBCSpent, RBCSpent, params.originalTxHash);
        }
        else{
            revert("swapContract: wrong path");
        }
    ...
    /**
      ...
      */
    function setMaxTokenAmount(uint256 _maxTokenAmount) external onlyOwnerAndManager{
