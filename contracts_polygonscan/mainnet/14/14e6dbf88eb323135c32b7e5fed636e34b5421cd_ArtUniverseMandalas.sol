...
contract ArtUniverseMandalas is ERC721, Ownable, RandomlyAssigned {
  using Strings for uint256;
   string baseURI;
 ...
 string public NotRevealedURI = "ipfs://Qmd1aYJf3rsPHyEowMVNy6ZAiDgLJLeDQeYiShkKrHBjRp/SurpriseSample.json";
/* 
 constructor(
    string memory _name,
...
*/ 
  ...
  constructor(
   ...
   // ,string memory _initNotRevealedUri
    ...
    ERC721("ArtUniverseMandalas", "AUM")
    
    RandomlyAssigned(maxSupply,1) // Max. ----- NFTs available; Start counting from 1 (instead of 0)
    {
         setBaseURI(_initBaseURI) ;
  ...
  function mint (uint256 _mintAmount)
      ...
      payable
  {
      require(!paused, "MINTING  ACTUALY NOT ACTIVE");
      ...
      if (msg.sender != owner()) {  
        ...
        require( balanceOf(msg.sender) + _mintAmount <= maxMintAmount,"YOU CAN'T MINT AND OWN MORE THAN AUTHORISED AMOUNT");
...
function revael(bool _revealed) public onlyOwner {
    revealed = _revealed; 
  }
...
function setNotRevealedURI(string memory _newNotRevealedURI) public onlyOwner {
  ...
  function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
    require(
      ...
      if(revealed == false) {
        return NotRevealedURI ; /* NotRevealedURI ; */
   /*       bytes(NotRevealedURI).length > 0  //modif preview
        ? string(abi.encodePacked(NotRevealedURI, tokenId.toString(), ".json")) //modif preview
        : ""; //modif preview       */
      }
