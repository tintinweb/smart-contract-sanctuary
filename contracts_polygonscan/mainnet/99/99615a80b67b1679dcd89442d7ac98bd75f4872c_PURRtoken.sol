...
contract PURRtoken is Context, IERC20Metadata, Ownable { 
    ...
    function prepareBattle(uint16 _fromId, uint16 _toId) internal returns(bool){
        ...
        if ((ICatFight(_catFightContract).calculateBattlePoints(_fromId) + _LPsFrom + _buffBonusFrom) >= (ICatFight(_catFightContract).calculateBattlePoints(_toId) + _LPsTo + _buffBonusTo)) 
        {
            ICatFight(_catFightContract).increaseWins(_fromId);
            ICatFight(_catFightContract).increaseLosses(_toId);
            _mint(IKawaiiCatsNFT(_nftContractAddress).ownerOf(_fromId), WIN_REWARD);
            _mint(IKawaiiCatsNFT(_nftContractAddress).ownerOf(_toId), LOSE_REWARD);
            ...
            _mint(IKawaiiCatsNFT(_nftContractAddress).ownerOf(_fromId), LOSE_REWARD); 
            _mint(IKawaiiCatsNFT(_nftContractAddress).ownerOf(_toId), WIN_REWARD);
