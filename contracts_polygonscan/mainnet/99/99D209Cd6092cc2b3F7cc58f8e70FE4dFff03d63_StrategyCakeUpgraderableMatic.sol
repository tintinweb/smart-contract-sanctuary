...
// File: contracts/matic/StrategyCakeUpgraderableMatic.sol
...
contract StrategyCakeUpgraderableMatic is ReentrancyGuardUpgradeSafe, OwnableUpgradeable , Pausable {
    ...
    function initialize(
        ...
        if (isGrandComp) {
            ...
            earnedToGrandPath = [earnedAddress, maticAddress, GrandAddress];
            if (maticAddress == earnedAddress) {
                earnedToGrandPath = [maticAddress, GrandAddress];
            }

            earnedToToken0Path = [earnedAddress, maticAddress, token0Address];
            if (maticAddress == token0Address) {
                earnedToToken0Path = [earnedAddress, maticAddress];
            }

            earnedToToken1Path = [earnedAddress, maticAddress, token1Address];
            if (maticAddress == token1Address) {
                earnedToToken1Path = [earnedAddress, maticAddress];
            }

            token0ToEarnedPath = [token0Address, maticAddress, earnedAddress];
            if (maticAddress == token0Address) {
                token0ToEarnedPath = [maticAddress, earnedAddress];
            }

            token1ToEarnedPath = [token1Address, maticAddress, earnedAddress];
            if (maticAddress == token1Address) {
                token1ToEarnedPath = [maticAddress, earnedAddress];
            }
        }
