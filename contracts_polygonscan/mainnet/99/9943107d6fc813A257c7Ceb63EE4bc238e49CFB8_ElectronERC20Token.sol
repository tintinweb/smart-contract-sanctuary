...
contract GovernanceOwned is Ownable {

  address internal _governance;

  function setGovernance( address governance_ ) external onlyOwner() returns ( bool ) {
  ...
  modifier onlyGovernance() {
    require( _governance == msg.sender, "GovernanceOwned: caller is not governance" );
    _;
  }

}

contract ElectronERC20Token is ERC20Permit, VaultOwned, GovernanceOwned {

    using SafeMath for uint256;

    bool private initialized = false;
    mapping (address => bool) liquidityPairs;
    uint8 public liquidityFee = 10; // initial 0.1% liquidity fee
    uint8 public constant MAX_LIQUIDITY_FEE = 30; // maximum 0.3% liquidity fee
    uint256 public constant FEE_DENOMINATOR = 10000;
    mapping (address => bool) public feeExcluded;

    constructor() ERC20("Electron DAO Token", "ELECTRON", 9) {
    }
    ...
    function addLiquidityPair(address pair) external onlyGovernance() {
        liquidityPairs[pair] = true;
    }

    function removeLiquidityPair(address pair) external onlyGovernance() {
        delete liquidityPairs[pair];
    }

    function setLiquidityFee(uint8 fee) external onlyGovernance() {
        require(fee <= MAX_LIQUIDITY_FEE, "liquidity fee is above maximum fee");
        liquidityFee = fee;
    }

    function setFeeExcluded(address account, bool excluded) external onlyGovernance() {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        ...
        if (liquidityPairs[recipient] && !feeExcluded[sender]) {
            uint256 fee = amount.mul(liquidityFee).div(FEE_DENOMINATOR);
            amount = amount.sub(fee);
            _transfer(sender, vault(), fee);
        }
