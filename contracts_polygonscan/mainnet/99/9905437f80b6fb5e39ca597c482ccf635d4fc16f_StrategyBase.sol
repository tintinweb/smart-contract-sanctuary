...
abstract contract StrategyFarmBase is BaseStrategyMasterChef {
    ...
    address public constant roll = 0xC68e83a305b0FaD69E264A1769a0A070F190D2d6;
    ...
    // Uniswap swap paths
    address[] public matic_roll_path;
    address[] public quick_matic_path;
    ...
    {
        matic_roll_path = new address[](2);
        matic_roll_path[0] = roll;
        matic_roll_path[1] = matic;
    }
    ...
    function set_harvestCutoff(uint256 newCutoff) external onlyOwner() {
        require(newCutoff <= 10**18, "New cutoff must be less than 1 CRYSTL");
    ...
    function harvest() public override {
        ...
        // Collects CRYSTL tokens
        _getReward();

        //Swap 1/2 CRYSTL for MATIC
        uint256 _ROLLBalance = IERC20(roll).balanceOf(address(this));
        if (_ROLLBalance > harvestCutoff){
            if (_ROLLBalance > 0) {
                _swapUniswapWithPath(matic_roll_path, _ROLLBalance.div(2));
            }

            // Add liquidity for CRYSTL/MATIC
            uint256 _matic = IERC20(matic).balanceOf(address(this));
            uint256 _roll = IERC20(roll).balanceOf(address(this));
            if (_matic > 0 && _roll > 0) {
                IERC20(matic).safeApprove(currentRouter, 0);
                IERC20(matic).safeApprove(currentRouter, _matic);
                IERC20(roll).safeApprove(currentRouter, 0);
                IERC20(roll).safeApprove(currentRouter, _roll);
                ...
                IERC20(roll).safeTransfer(
                    strategist,
                    IERC20(roll).balanceOf(address(this))
                );
            }
...
contract StrategyBase is StrategyFarmBase {
    // Token addresses
    address public ROLL_MASTER_CHEF = 0x3C58EA8D37f4fc6882F678f822E383Df39260937;
    address public ROLL_MATIC_LP = 0x65C37f48781a555e2AD5542e4306ebAb1Ae93Cd7;
