...
contract MockScoofTreasury is Ownable {
  using SafeMath for uint256;
  ...
  address public sSCF;
  uint256 public sSCFQueue; // Delays change to sSCF address
  ...
  constructor(
    ...
    // isLiquidityToken[ _SCFDAI ] = true;
    // liquidityTokens.push( _SCFDAI );

    blocksNeededForQueue = _blocksNeededForQueue;
  }

  /**
        @notice allow approved address to deposit an asset for SCF
        @param _amount uint
  ...
  function deposit(
    ...
    (_token, _amount);
    // mint SCF needed and store amount of rewards for distribution
    send_ = value.sub(_profit);
    IERC20Mintable(SCF).mint(msg.sender, send_);
  ...
  /**
        @notice allow approved address to burn SCF for reserves
        @param _amount uint
        @param _token address
     */
  function withdraw(uint256 _amount, address _token) external {
    ...
    uint256 value = valueOfToken(_token, _amount);
    ISCFERC20(SCF).burnFrom(msg.sender, value);
  ...
  function incurDebt(uint256 _amount, address _token) external {
    ...
    uint256 maximumDebt = IERC20(sSCF).balanceOf(msg.sender); // Can only borrow against sSCF held
  ...
  /**
        @notice allow approved address to repay borrowed reserves with SCF
        @param _amount uint
     */
  function repayDebtWithSCF(uint256 _amount) external {
    require(isDebtor[msg.sender], "Not approved");

    ISCFERC20(SCF).burnFrom(msg.sender, _amount);

    debtorBalance[msg.sender] = debtorBalance[msg.sender].sub(_amount);
    totalDebt = totalDebt.sub(_amount);

    emit RepayDebt(msg.sender, SCF, _amount, _amount);
  }
  ...
  function mintRewards(address _recipient, uint256 _amount) external {
    ...
    IERC20Mintable(SCF).mint(_recipient, _amount);
  ...
  function excessReserves() public view returns (uint256) {
    return totalReserves.sub(IERC20(SCF).totalSupply().sub(totalDebt));
  }
  ...
  /**
        @notice returns SCF valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
  ...
  {
    if (isReserveToken[_token]) {
      // convert amount to match SCF decimals
      value_ = _amount.mul(10**IERC20(SCF).decimals()).div(
    ...
    returns (bool)
  {
    ...
    } else if (_managing == MANAGING.SSCF) {
      // 9
      sSCFQueue = block.number.add(blocksNeededForQueue);
    } else return false;
