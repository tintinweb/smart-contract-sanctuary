...
contract Controllers {
    ...
    */
    modifier isChief {
        require(chief[msg.sender] || msg.sender == IndexInterface(stakeAllIndex).master(), "not-an-chief");
        _;
    }
    ...
    */
    function toggleChief(address _chiefAddress) external {
        require(msg.sender == IndexInterface(stakeAllIndex).master(), "toggleChief: not-master");
    ...
    */
    function removeConnectors(string[] calldata _connectorNames) external isChief {
        for (uint i = 0; i < _connectorNames.length; i++) {
            require(connectors[_connectorNames[i]] != address(0), "removeConnectors: _connectorName not added to remove");
