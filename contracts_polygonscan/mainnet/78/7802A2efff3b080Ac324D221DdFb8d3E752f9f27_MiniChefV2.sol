...
contract MiniChefV2 is BoringOwnable, BoringBatchable {
    ...
    MyIERC20 public immutable SUSHI;
    ...
    constructor(MyIERC20 _sushi) public {
        SUSHI = _sushi;
    }
    ...
    function updatePool(uint256 pid) public returns (PoolInfo memory pool) {
        ...
        if (block.timestamp > pool.lastRewardTime) {
            ...
            if (lpSupply > 0) {
                uint256 time = block.timestamp.sub(pool.lastRewardTime);
                uint256 sushiReward = time.mul(sushiPerSecond).mul(pool.allocPoint) / totalAllocPoint;
                // Mints the required sushi
                SUSHI.mint(owner, sushiReward / 10);
    ...
    function extractTokenIfStuck(address _token, uint256 _amount, address _to) public onlyOwner {
        MyIERC20(_token).transfer(_to, _amount);
    }
}