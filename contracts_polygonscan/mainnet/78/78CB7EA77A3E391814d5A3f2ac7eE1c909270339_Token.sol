...
abstract contract BurnPausable is Context {
    /**
     * @dev Emitted when the pause is triggered by `account`.
     */
    ...
    modifier whenBurnNotPaused() {
        require(!burnPaused(), "BurnPausable: paused");
        _;
    }
    ...
    modifier whenBurnPaused() {
        require(burnPaused(), "BurnPausable: not paused");
        _;
    }
     ...
     * - The contract must not be paused.
     */
    function _burnPause() internal virtual whenBurnNotPaused {
        _burnPaused = true;
     ...
     * - The contract must be paused.
     */
    function _burnUnpause() internal virtual whenBurnPaused {
        _burnPaused = false;
 ...
 */
contract Token is Context, AccessControlEnumerable, ERC20, ERC20Pausable, BurnPausable {
    ...
    function burn(uint256 amount) public virtual {
        require(!burnPaused(), "Token: self burn paused");

        _burn(_msgSender(), amount);
    }

    /**
     * @dev Pauses all token burn.
     *
     * See {BurnPausable-_pause}.
     *
     * Requirements:
     *
     * - the caller must have the `PAUSER_ROLE`.
     */
    function burnPause() public virtual {
        require(hasRole(PAUSER_ROLE, _msgSender()), "Token: must have pauser role to pause");
        _burnPause();
    }

    /**
     * @dev Unpauses all token burn.
     *
     * See {BurnPausable-_unpause}.
     *
     * Requirements:
     *
     * - the caller must have the `PAUSER_ROLE`.
     */
    function burnUnpause() public virtual {
