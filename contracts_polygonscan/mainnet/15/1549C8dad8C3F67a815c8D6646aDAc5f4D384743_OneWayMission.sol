...
contract OneWayMission is Ownable {
    using SafeMath for uint256;
    ...
    struct UserInfo 
    {
        uint256 amount;                     // How many NEPTUNE tokens the user has provided.
    ...
    IERC20 public immutable NEPTUNE;              
    ...
    uint256 public stakedNeptune;
    PoolInfo public poolInfo;
    bool public emergencyActive;

    mapping (address => UserInfo) public userInfo;     // Info of each user that stakes NEPTUNE tokens.
    ...
    constructor(IERC20 _NEPTUNE, IERC20 _REWARD, uint256 _rewardPerBlock, uint256 _startBlock, uint256 _lockBlock, uint256 _endBlock) public {
        require(address(_NEPTUNE) != address(0), "_NEPTUNE address not set!");
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > poolInfo.lastRewardBlock && stakedNeptune != 0) {
            uint256 multiplier = getMultiplier(poolInfo.lastRewardBlock, block.number);
            uint256 tokenReward = multiplier.mul(rewardPerBlock);
            accRewardPerShare = accRewardPerShare.add(tokenReward.mul(1e12).div(stakedNeptune));
        }
    ...
    function updatePool() public {
        ...
        uint256 multiplier = getMultiplier(poolInfo.lastRewardBlock, block.number);
        uint256 tokenReward = multiplier.mul(rewardPerBlock);
        poolInfo.accRewardPerShare = poolInfo.accRewardPerShare.add(tokenReward.mul(1e12).div(stakedNeptune));
        poolInfo.lastRewardBlock = block.number;
    }

    /**
     * @dev Deposit NEPTUNE tokens to Faucet for rewards allocation and/or withdraw outstanding rewards.
     * @param _amount Amount of NEPTUNE tokens to deposit.
     */
    function deposit(uint256 _amount) public {
        ...
        if (block.number < lockBlock && _amount != 0) {
            NEPTUNE.safeTransferFrom(address(msg.sender), address(this), _amount);
            stakedNeptune = stakedNeptune.add(_amount);
            user.amount = user.amount.add(_amount);
            user.rewardDebt = user.amount.mul(poolInfo.accRewardPerShare).div(1e12);
            emit Deposit(msg.sender, _amount);
        }
        if (block.number >= endBlock) {
            NEPTUNE.safeTransfer(burnAddress, user.amount);
            stakedNeptune = stakedNeptune.sub(user.amount);
            user.amount = 0;
            user.rewardDebt = user.amount.mul(poolInfo.accRewardPerShare).div(1e12);
    ...
    function activateEmergency(address _to) external onlyOwner returns(bool) {
        ...
        if (NEPTUNE == REWARD) {
            return REWARD.transfer(_to, REWARD.balanceOf(address(this)) - stakedNeptune);
        } else {
    ...
    function emergencyWithdraw() external {
        ...
        UserInfo storage user = userInfo[msg.sender];
        NEPTUNE.safeTransfer(address(msg.sender), user.amount);
        stakedNeptune = stakedNeptune.sub(user.amount);
