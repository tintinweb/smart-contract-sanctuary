...
contract TokenController is Context, Initializable, AccessControl {
    ...
    mapping(address => bool) private feesWhitelist;
    ...
    function addConnectionContract(address connection)
        ...
        onlyRole(ROLE_ADMIN)
    {
        require(Address.isContract(connection), INVALID_ADDRESS);
    ...
    function addToBlacklist(address black) public onlyRole(ROLE_ADMIN) {
        blacklist[black] = true;
    }

    function removeFromBlacklist(address black) public onlyRole(ROLE_ADMIN) {
        blacklist[black] = false;
    }

    function addToWhitelist(address white) public onlyRole(ROLE_ADMIN) {
        feesWhitelist[white] = true;
    }

    function removeFromWhitelist(address white) public onlyRole(ROLE_ADMIN) {
        feesWhitelist[white] = false;
    }
}
...
contract FactoryStaking is Initializable, AccessControl {
    ...
    uint32 private _timePerMinterPoints = 86400;
    uint16 private _pointsPerTime = 1000;
        ...
        returns (uint256)
    {
        ...
        return ((time**2) * _factoryGrowReward) / 4000;
    }
    ...
    function growFactory(uint256 factory) public {
        ...
        manager.sendTokens(
            msg.sender,
            getRewardPerFactory(msg.sender, factory)
        );
    ...
    function canFactoryGrow(address owner, uint256 id)
        ...
        returns (bool)
    {
        return _stakedFactories[owner][id] <= block.timestamp - _minGrowTime;
    }
...
// File: contracts/factoryStakingV2.sol
...
contract FactoryStakingV2 is Initializable, AccessControl {
    bytes32 public constant MAIN_OWNER = keccak256("MAIN_OWNER");
    bytes32 public constant CONNECTION = keccak256("CONNECTION");
    ...
    Factories private factories;
    TokenController private manager;
    FactoryStaking private factoryStakingV1;
    ...
    mapping(uint256 => address) private _v1Grow;
    mapping(address => bool) private _registered;
    mapping(address => uint256) private _stakedMinterPoints;
    mapping(address => uint256) private _stakedMinterTimestamp;
    mapping(address => uint256[]) private _stakedCustomerFactories;
        ...
        returns (uint256)
    {
        return
            _stakedMinterPoints[customer] +
            factoryStakingV1.getSavedMinterPoints(customer);
    }
    ...
    function requestV1Rewards(uint256 id) public {
        require(
            factoryStakingV1.theFactoryIsStaked(id) &&
                factoryStakingV1.canFactoryGrow(msg.sender, id),
            INVALID_V1_STAKING
        );

        require(canRequestV1Rewards(id), INVALID_V1_STAKING);

        uint256 growRewards = factoryStakingV1.getGrowReward(msg.sender, id);

        require(growRewards > 0, INVALID_REWARDS);

        _v1Grow[id] = msg.sender;
        manager.sendTokens(msg.sender, growRewards);
    }

    function getV1Rewards(uint256 id) public view returns (uint256) {
        return
            canRequestV1Rewards(id)
                ? factoryStakingV1.getGrowReward(msg.sender, id)
                : 0;
    }

    function canRequestV1Rewards(uint256 id) public view returns (bool) {
        return _v1Grow[id] == address(0);
    }
    ...
    function useMinterPoints(address customer, uint256 amount)
        ...
        onlyRole(CONNECTION)
    {
        uint256 v1MinterPoints = factoryStakingV1.getSavedMinterPoints(
            customer
        );

        if (v1MinterPoints > 0) {
            if (amount - v1MinterPoints >= 0) {
                amount -= v1MinterPoints;
                factoryStakingV1.useMinterPoints(customer, v1MinterPoints);
            } else {
                amount = 0;
                factoryStakingV1.useMinterPoints(customer, amount);
            }
        }
