...
contract Strategy_USDC_DERC is Ownable {
    ...
    address public routerLP = 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff;
    address public routerMain = 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff;
    address public routerReward = 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff;
    ...
    function _addLiquidityFromAllTokens() internal {
        _swapWMATICToToken(getBalanceOfToken(WMATIC)/2, routerLP, token0);
        _swapWMATICToToken(getBalanceOfToken(WMATIC)/2, routerLP, token1);
    ...
    function _swapExactETHForTokens(address _token, uint256 _amountETH) internal {
        ...
        path[0] = WMATIC;
        path[1] = _token;
        uint256[] memory amounts = IRouter(routerLP).getAmountsOut(_amountETH/2, path);
    ...
    function _swapLPtoWMATIC(uint256 _percent) internal {
        ...
        if (_totalLP > 0) {
            ...
            _swapTokenToWMATIC(routerLP, rewardToken);

            _removeLiquidity();

            // swap token1 to WMATIC
            _swapTokenToWMATIC(routerLP, token0);
            _swapTokenToWMATIC(routerLP, token1);
        }
    }
    ...
    function _getToken0Price() internal view returns (uint256 result) {
        ...
        path[0] = token1;
        path[1] = token0;
        result = 1e24 / (_getAmountsOut(routerLP, 100 * 1e6, path) / 100);
    }
    ...
    function _getToken1Price() internal view returns (uint256) {
        ...
        path[0] = token0;
        path[1] = token1;
        return (100 * 1e36) / _getAmountsOut(routerLP, 100 * 1e18, path) / 1e12;
    }
