...
contract CHERRY is ERC20('PolyCherry', 'CHERRY') {
...
contract CherryMasterChef is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of Cherryes
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. Cherryes to distribute per block.
        uint256 lastRewardBlock;  // Last block number that Cherryes distribution occurs.
        uint256 accCherryPerShare;   // Accumulated Cherryes per share, times 1e18. See below.
    ...
    uint256 public CherryPerBlock = 0.1 ether;
    
    uint256 constant MAX_CHERRY_PERBLOCK= 0.1 ether;
    ...
    // Cherry referral contract address.
    ...
    event UpdateEmissionRate(address indexed user, uint256 CherryPerBlock);
    ...
    // Update the given pool's Cherry allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending Cherryes on frontend.
    function pendingCherry(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0 && totalAllocPoint>0) {
            ...
            accCherryPerShare = accCherryPerShare.add(CherryReward.mul(1e18).div(lpSupply));
        }
        return user.amount.mul(accCherryPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 CherryReward = multiplier.mul(CherryPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        Cherry.mint(devAddress, CherryReward.div(10));
        Cherry.mint(address(this), CherryReward);
        pool.accCherryPerShare = pool.accCherryPerShare.add(CherryReward.mul(1e18).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accCherryPerShare).div(1e18).sub(user.rewardDebt);
            if (pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accCherryPerShare).div(1e18);
    ...
    // Safe Cherry transfer function, just in case if rounding error causes pool to not have enough FOXs.
    function safeCherryTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > CherryBal) {
            transferSuccess = Cherry.transfer(_to, CherryBal);
        } else {
            transferSuccess = Cherry.transfer(_to, _amount);
        }
        require(transferSuccess, "safeCherryTransfer: Transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _CherryPerBlock) external onlyOwner {
        require(_CherryPerBlock<=MAX_CHERRY_PERBLOCK,"Too high");
        massUpdatePools();
        CherryPerBlock = _CherryPerBlock;
        emit UpdateEmissionRate(msg.sender, _CherryPerBlock);
    }
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (address(referral) != address(0) && referralCommissionRate > 0) {
            ...
            if (referrer != address(0) && commissionAmount > 0) {
                Cherry.mint(referrer, commissionAmount);
