...
contract Ecosystem is Ownable{
    ...
    **/
    ...
    constructor(address _token) {
        token = IERC20(_token);
        maxVolume = 1700000000000;
        TGE = 1637908200;

        timePerRound = 2*3600 ; // 2 hours

        nextRoundTime = TGE+timePerRound;
        totalRound = 48;
    }
    ...
    function claim() public returns (bool){
        require(isStarted, "Owner must be start first");
        require(block.timestamp >= TGE+timePerRound, "Too early");
        ...
        if (currentRound == totalRound){
            ...
            uint256 nextValueClaimed = whitelist[msg.sender].amount*currentRound/totalRound;
            uint256 sendValue = nextValueClaimed - whitelist[msg.sender].claimed;
