...
// ------------------------------------------------- public variables ---------------------------------------------------
    ...
    struct Position {
        ...
        uint256 supply_amount; // token amount that is supplied from to Cream , deNormalized
...
// ----------- cream manipulative function ------------
    ...
    function _supplyCream(Position memory _position) private returns(Position memory) {
        // out_amount : money that is outside bunker
        // supply_amount : money that is used to borrow token_a
        uint256 out_amount = IERC20(_position.token).balanceOf(address(this)).mul(_position.supply_funds_percentage).div(100);
        uint256 denominator = _position.collateral_factor.mul(_position.borrow_percentage).div(10000).add(1);
        // make borrowed token_a value = left token_b value
        // i.e. make suplly_amount * collateral_factor% * borrow_percentage% = out_amount - supply_amount
        uint256 supply_amount = out_amount.div(denominator);
    ...
    function _redeemCream(Position memory _position) private returns (Position memory) {
        ...
        _position.outside_amount = 0; // todo check if after _redeemCream , outside_amount == 0
    ...
    function _borrowCream(HLSConfig memory self, Position memory _position) private returns(Position memory) {
        ...
        uint256 amt = _position.supply_amount.mul(_position.collateral_factor).div(100);
        // only borrow borrow_percentage% value of maximum borrowing limit value , to avoid liquidation due to the price fluctuation of borrowed token.
        amt = _position.supply_amount.mul(_position.borrow_percentage).div(100); 
        uint256 norm_desired_total_value = getTokenValueFromDeNormAmount(_position.token, self.token_oracle, amt);

        // All of the norm_desired_total_value is used to borrow token_a
        (uint256 token_a_borrow_amount,) = getAmountFromValue(self.token_a_oracle, self.token_b_oracle, norm_desired_total_value, 0);
        ...
        // todo used to check if token_a_borrow_amount ==  CErc20Delegator(_position.borrowed_crtoken_a).borrowBalanceCurrent(address(this))
        _position.borrowed_token_b_amount = CErc20Delegator(_position.borrowed_crtoken_a).borrowBalanceCurrent(address(this));
...
// --------- quickswap manipulative function ----------
    ...
    function _addLiquidity(HLSConfig memory self, Position memory _position, bool _reverseAB) private returns (Position memory) {
        ...
        uint256 max_available_staking_a = IERC20(_position.token_a).balanceOf(address(this)); // _position.token_a = WETH
        uint256 max_available_staking_b = IERC20(_position.token_b).balanceOf(address(this)); // _position.token_b = USDC
        ...
        // pair = USDC/WETH , token0:USDC, token1:WETH => misposition
        ...
        // function quote( amountB,  reserveB,  reserveA)   returns ( amountA)
        // function quote( amountA,  reserveA,  reserveB)   returns ( amountB)
        // quote:  amountB / reserveB = amountA / reserveA;
        ...
        uint256 liquidity1;
        if( _reverseAB ){
            IERC20(_position.token_a).approve(self.router, max_available_staking_b); // WETH
            IERC20(_position.token_b).approve(self.router, max_available_staking_a); // USDC
            
            revert(" revert here 4 ") ; // works here

            (liquidity0, liquidity1, ) = IQuickRouter02(self.router).addLiquidity(_position.token_b, _position.token_a, max_available_staking_b, max_available_staking_a, min_0_amnt, min_1_amnt, address(this), block.timestamp);
            ...
            (liquidity0, liquidity1, ) = IQuickRouter02(self.router).addLiquidity(_position.token_a, _position.token_b, max_available_staking_a, max_available_staking_b, min_0_amnt, min_1_amnt, address(this), block.timestamp);
        }

        // Update posititon amount data
        _position.liquidity_0 = liquidity0;
        _position.liquidity_1 = liquidity1;
    ...
    function _removeLiquidity(HLSConfig memory self, Position memory _position, bool _reverseAB) private returns (Position memory) {
        ...
        // Update posititon amount data
        _position.liquidity_0 = 0;
        _position.liquidity_1 = 0;
        _position.lp_token_amount = IERC20(_position.lp_token).balanceOf(address(this));
        _position.token_a_amount = IERC20(_position.token_a).balanceOf(address(this));
        _position.token_b_amount = IERC20(_position.token_b).balanceOf(address(this));
...
// ------------------------------------------- charged buncker getter function ------------------------------------------
    ...
    function getTotalDebts(HLSConfig memory self, Position memory _position, bool _singleFarm, bool _reverseAB) public returns (uint256) {
        ...
        crtoken_a_debt = crtoken_a_debt.mul(10**18).div(10**IERC20(_position.token_a).decimals());
        ...
        staked_token_a_amt = staked_token_a_amt.mul(10**18).div(10**IERC20(_position.token_a).decimals());
        ...
        if (token_a_value == 1){
            ...
            token_a_value = (crtoken_a_debt.sub(staked_token_a_amt)).mul(token_a_price).div(10**LinkOracle(self.token_a_oracle).decimals());
