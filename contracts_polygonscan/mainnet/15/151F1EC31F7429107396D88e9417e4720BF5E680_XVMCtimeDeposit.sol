...
contract XVMCtimeDeposit is Ownable, Pausable {
    ...
    uint256 public immutable withdrawFeePeriod = 1000 days; // roughly 3 years
    ...
    //there is no trusted sender, but we need it in case pools are changed
     ...
     */
    function setTrustedPools(address _pool6) external adminOrOwner {
        trustedPool6 = _pool6;
     ...
     */
    function setTrustedSenders(address _sender1, address _sender2, address _sender3, address _sender4) external adminOrOwner {
    ...
    function withdraw(uint256 _shares) public notContract {
        ...
        if (block.timestamp < user.lastDepositedTime.add(withdrawFeePeriod)) {
            uint256 withdrawFee = uint256(69).sub(((block.timestamp - user.lastDepositedTime).div(86400)).mul(69).div(100));
    ...
    function hopDeposit(uint256 _amount, address _recipientAddress, uint256 previousLastDepositedTime) external whenNotPaused {
        require(
            msg.sender == trustedSender1 || msg.sender == trustedSender2 ||
            msg.sender == trustedSender3 || msg.sender == trustedSender4,
            "only trusted senders(other pools)"
        );
