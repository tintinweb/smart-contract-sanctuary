...
contract Controller is Initializable, OwnableUpgradeable {
    ...
    bytes public constant nullBytes = abi.encodePacked(uint256(57005));

    struct Staking {
        address stakingContract;
        bytes preDepositData;
        bytes postDepositData;
        bytes preWithdrawData;
        bytes postWithdrawData;
        address[] claimTargets;
        bytes[] claimData;
        address[] pendingRewardsInfoTargets;
        bytes[] pendingRewardsInfoCallData;
    }
    ...
    function initialize(
        ...
        strategistFees = _strategistFees;
        keeperFees = _keeperFees;
    }
    ...
    struct DeployPoolParams {
        ...
        bytes preWithdrawData;
        bytes postWithdrawData;
        bytes encodedClaimData;
        bytes encodedPendingRewardsData;
    }

    function deployPool(DeployPoolParams calldata params) external {
        require(approvedDeployers[msg.sender], "Not Approved to deploy");
        ...
        bytes32 salt =
            keccak256(
                abi.encodePacked(
                    ...
                    params.preDepositData,
                    params.postDepositData,
                    params.preWithdrawData,
                    params.postWithdrawData
                )
            );
        ...
        _initializeStakingData(
            ...
            params.preWithdrawData,
            params.postWithdrawData,
            params.encodedClaimData,
            params.encodedPendingRewardsData
        );
    }

    function _initializeStakingData(
        address _poolAddress,
        ...
        bytes memory encodedClaimData,
        bytes memory encodedPendingRewardsData
    ) internal {
        // decode variables
        (address[] memory _claimTargets, bytes[] memory _claimData) =
            abi.decode(encodedClaimData, (address[], bytes[]));
        (
            address[] memory _pendingRewardsInfoTargets,
            bytes[] memory _pendingRewardsInfoCallData
        ) = abi.decode(encodedPendingRewardsData, (address[], bytes[]));
        require(
            (_claimTargets.length == _claimData.length) &&
                (_pendingRewardsInfoTargets.length ==
                    _pendingRewardsInfoCallData.length),
        ...
        Staking memory staking =
            Staking({
                ...
                preDepositData: _preDepositData,
                postDepositData: _postDepositData,
                preWithdrawData: _preWithdrawData,
                postWithdrawData: _postWithdrawData,
                claimTargets: _claimTargets,
                claimData: _claimData,
                pendingRewardsInfoTargets: _pendingRewardsInfoTargets,
                pendingRewardsInfoCallData: _pendingRewardsInfoCallData
            });
    ...
    function predictPoolAddress(
        address _underlying,
        address _stakingContract,
        bytes calldata _preDepositData,
        bytes calldata _postDepositData,
        bytes calldata _preWithdrawData,
        bytes calldata _postWithdrawData
        ...
        returns (address target, bytes memory depositCallData)
    {
        ...
        bytes memory _preDepositData = poolInfo[msg.sender].preDepositData;
        bytes memory _postDepositData = poolInfo[msg.sender].postDepositData;

        if (_compareBytes(_postDepositData, nullBytes)) {
            depositCallData = abi.encodePacked(_preDepositData, _amountIn);
        } else {
        ...
        returns (address target, bytes memory withdrawCallData)
    {
        ...
        bytes memory _preWithdrawData = poolInfo[msg.sender].preWithdrawData;
        bytes memory _postWithdrawData = poolInfo[msg.sender].postWithdrawData;

        if (_compareBytes(_postWithdrawData, nullBytes)) {
            withdrawCallData = abi.encodePacked(_preWithdrawData, _amountOut);
        } else {
    ...
    {
        targets = poolInfo[msg.sender].claimTargets;

        claimCallData = poolInfo[msg.sender].claimData;
    }
    ...
    {
        targets = poolInfo[msg.sender].pendingRewardsInfoTargets;

        pendingRewardsInfoCallData = poolInfo[msg.sender]
            .pendingRewardsInfoCallData;
    }

    function togglePoolActive(address _poolAddress) external onlyOwner {
        Pool(payable(_poolAddress)).togglePoolActive();
    }
    ...
    function setApprovedDeployers(
        ...
        require(deployers.length == isApproved.length, "Invalid Input length");

        for (uint256 i = 0; i < deployers.length; i++) {
            approvedDeployers[deployers[i]] = isApproved[i];
        }
    }
    ...
    function setApprovedKeepers(
        ...
        require(keepers.length == isApproved.length, "Invalid Input length");

        for (uint256 i = 0; i < keepers.length; i++) {
            approvedKeepers[keepers[i]] = isApproved[i];
        }
    }
    ...
    function _verifyFeesRange(
        ...
        require(
            _new_strategistFees >= 0 && _new_strategistFees <= 10000,
            "strategistFees out of range"
        );
        require(
            _new_keeperFees >= 0 && _new_keeperFees <= 10000,
            "keeperFees out of range"
        );
    }

    function _compareBytes(bytes memory b1, bytes memory b2)
        internal
        pure
        returns (bool)
    {
...
contract Pool is Initializable, ERC20Upgradeable {
    ...
    function deposit(
        ...
        (address target, bytes memory depositCallData) =
...
contract MockControllerV2 is Initializable, OwnableUpgradeable {
    uint256 public strategistFees; // bps
    uint256 public keeperFees; // bps
