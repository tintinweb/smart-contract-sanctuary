...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, Ownable{
    ...
    function _mint(address to, uint256 tokenId, string memory ticketUri) private {
        ...
        _balances[to] += 1;
        _owners[tokenId] = to;
        _tokenURIs[tokenId] = ticketUri;

        emit Transfer(address(0), to, tokenId);
    }

    function mint(address to, uint256 tokenId, string memory ticketUri) onlyOwner public {
        return _mint(to, tokenId, ticketUri);
    }
}


contract MiroTickets is ERC721 {
    constructor() ERC721(
        "Collection of NFT innovation space 'Miro Space'", "MIRO", "https://gateway.pinata.cloud/ipfs/QmPBDvjBBdBW1uGSB6L1QTZULs8VgCv8iPQ6erPokhnGdK"
    ){
