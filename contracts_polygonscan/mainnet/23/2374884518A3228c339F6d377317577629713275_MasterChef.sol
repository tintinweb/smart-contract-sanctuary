...
contract PolyBabyInu is BEP20('Poly Baby Inu', 'PolyBabyInu') {
...
contract MasterChef is Ownable {
    ...
    PolyBabyInu public baby;
    // Dev address.
    address public devaddr;
    // EGG tokens created per block.
    uint256 public babyPerBlock;
    // Bonus muliplier for early baby makers.
    ...
    function pendingEgg(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 babyReward = multiplier.mul(babyPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accEggPerShare = accEggPerShare.add(babyReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 babyReward = multiplier.mul(babyPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        baby.mint(devaddr, babyReward.div(10));
        baby.mint(address(this), babyReward);
        pool.accEggPerShare = pool.accEggPerShare.add(babyReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    // Safe baby transfer function, just in case if rounding error causes pool to not have enough EGGs.
    function safeEggTransfer(address _to, uint256 _amount) internal {
        uint256 babyBal = baby.balanceOf(address(this));
        if (_amount > babyBal) {
            baby.transfer(_to, babyBal);
        } else {
            baby.transfer(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _babyPerBlock) public onlyOwner {
        massUpdatePools();
        babyPerBlock = _babyPerBlock;
    }
