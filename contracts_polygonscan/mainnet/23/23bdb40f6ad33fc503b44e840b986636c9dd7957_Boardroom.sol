...
contract Boardroom is ShareWrapper, ContractGuard, OwnableUpgradeSafe {
    ...
    function canWithdraw(address director) external view returns (bool) {
        return directors[director].epochTimerStart.add(withdrawLockupEpochs) <= ITreasury(treasury).epoch();
    }

    function canClaimReward(address director) public view returns (bool) {
        return directors[director].epochTimerStart.add(rewardLockupEpochs) <= ITreasury(treasury).epoch();
    }
    ...
    function getMaximumDepositAmount(address director) public view returns (uint256) {
        uint256 _maxAmount = share.totalSupply().mul(depositMaximumRate).div(10000);
        if (director == daoFund) return _maxAmount;
    ...
    function withdraw(uint256 _amount) public notContract onlyOneBlock directorExists updateReward(msg.sender) {
        ...
        uint256 _fee = 0;
        if (_epochTimerStart.add(withdrawLockupEpochs) > _epoch) {
    ...
    function claimReward() public {
        require(directors[msg.sender].epochTimerStart.add(rewardLockupEpochs) <= epoch(), "Boardroom: still in reward lockup");
