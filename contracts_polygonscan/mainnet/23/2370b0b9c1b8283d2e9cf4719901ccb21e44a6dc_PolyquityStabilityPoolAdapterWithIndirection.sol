...
// File contracts/interfaces/Polyquity/IPolyquityStabilityPool.sol

pragma solidity ^0.6.12;



interface IPolyquityStabilityPool{
...
// File contracts/adapters/PolyquityStabilityPoolAdapter.sol
pragma solidity ^0.6.12;
...
contract PolyquityStabilityPoolAdapter is IVaultAdapterV2 {
  using FixedPointMath for FixedPointMath.uq192x64;
  using SafeERC20 for IDetailedERC20;
  using SafeMath for uint256;

  /// @dev The vault that the adapter is wrapping.
  IPolyquityStabilityPool public vault;

  /// @dev UniswapV2Router
  IUniswapV2Router02 public uniV2Router;

  /// @dev pyqToken
  IDetailedERC20 public pyqToken;

  /// @dev wMatic
  IDetailedERC20 public usdcToken;

  /// @dev pusd
  IDetailedERC20 public pusdToken;
  ...
  constructor(IPolyquityStabilityPool _vault, address _admin, IUniswapV2Router02 _uniV2Router, IDetailedERC20 _pyqToken, IDetailedERC20 _pusdToken, IDetailedERC20 _usdcToken, address _frontendTag) public {
    vault = _vault;
    admin = _admin;
  ...
  modifier onlyAdmin() {
    require(admin == msg.sender, "PolyquityStabilityPoolAdapter: only admin");
    _;
  }
  ...
  function token() external view override returns (IDetailedERC20) {
    return IDetailedERC20(address(pusdToken));
  }
  ...
  function withdraw(address _recipient, uint256 _amount, bool _isHarvest) external override onlyAdmin {
    ...
    if(_isHarvest){
      address[] memory _pathpyq = new address[](3);
      _pathpyq[0] = address(pyqToken);
      _pathpyq[1] = address(usdcToken);
      _pathpyq[2] = address(pusdToken);

      address[] memory _pathMaticPusd = new address[](3);
      _pathMaticPusd[0] = wmatic;
      _pathMaticPusd[1] = address(usdcToken);
      _pathMaticPusd[2] = address(pusdToken);


      if(pyqToken.balanceOf(address(this)) > 0){
        // use swapExactTokensForTokensSupportingFeeOnTransferTokens for pyq transfer fee issue
        uniV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(pyqToken.balanceOf(address(this)),
                                             0,
                                             _pathpyq,
                                             address(this),
                                             block.timestamp+800);
    ...
    pusdToken.transfer(_recipient,pusdToken.balanceOf(address(this)));
  }

  /// @dev Updates the vaults approval of the token to be the maximum value.
  function updateApproval() public {
    IDetailedERC20(address(pusdToken)).safeApprove(address(vault), uint256(-1));
    pyqToken.safeApprove(address(uniV2Router), uint256(-1));
  }
...
// File contracts/adapters/PolyquityStabilityPoolAdapterWithIndirection.sol
pragma solidity ^0.6.12;


/// @title PolyquityStabilityPoolAdapterWithIndirection
///
/// @dev A vault adapter implementation which wraps a vesper vault.
contract PolyquityStabilityPoolAdapterWithIndirection is PolyquityStabilityPoolAdapter {
  using SafeERC20 for IDetailedERC20;
  using SafeMath for uint256;

  // IPolyquityStabilityPool _vault, address _admin, IUniswapV2Router02 _uniV2Router, IDetailedERC20 _pyqToken, IDetailedERC20 _pusdToken, IDetailedERC20 _usdcToken, address _frontendTag

  constructor(IPolyquityStabilityPool _vault, address _admin, IUniswapV2Router02 _uniV2Router, IDetailedERC20 _pyqToken, IDetailedERC20 _pusdToken, IDetailedERC20 _usdcToken, address _frontendTag)
              PolyquityStabilityPoolAdapter(_vault,_admin,_uniV2Router,_pyqToken,_pusdToken,_usdcToken,_frontendTag) public {
  }
  ...
  function indirectWithdraw(address _recipient, uint256 _amount) external onlyAdmin {
      // harvest pyq token
      vault.withdrawFromSP(0);
      console.log("befor transfer balance %s", pyqToken.balanceOf(address(this)));
      // transfer pyq token
      pyqToken.transfer(_recipient, pyqToken.balanceOf(address(this)));
      console.log("after transfer balance %s", pyqToken.balanceOf(address(this)));
  }
}