...
contract WormMegaRace is ERC721Enumerable, Ownable {
    ...
    uint public  freeStop = 0;
    uint public  basePrice = 20*10**18; // 20 MATIC
    ...
    function mint(address _to, uint _count) public payable {
        require(msg.sender == _owner || saleEnable || msg.sender == owner2, "Sale not enable");
        require(totalSupply() +_count <= MAX_TOKEN, "Exceeds limit");
        require(_count <= 100, "Exceeds 100");
        require(msg.value >= price(_count) || msg.sender == _owner || msg.sender == owner2, "Value below price");
    ...
    function WhiteList(address _to, uint _count) public payable {
        require(msg.sender == _owner || saleEnable || msg.sender == owner2, "Sale not enable");
        require(totalSupply() +_count <= MAX_TOKEN, "Exceeds limit");
        require(_count <= 100, "Exceeds 100");
        require(msg.value >= price(_count) || msg.sender == _owner || msg.sender == owner2, "Value below price");
