...
contract TroveManager is LiquityBase, Ownable, ITroveManager {
    ...
    ITokenStaking public override lqtyStaking;
    ...
    */
    ...
    /*
    * L_TOKEN and L_LUSDDebt track the sums of accumulated liquidation rewards per unit staked. During its lifetime, each stake earns:
    *
    * An token gain of ( stake * [L_TOKEN - L_TOKEN(0)] )
    * A LUSDDebt increase  of ( stake * [L_LUSDDebt - L_LUSDDebt(0)] )
    *
    * Where L_TOKEN(0) and L_LUSDDebt(0) are snapshots of L_TOKEN and L_LUSDDebt for the active Trove taken at the instant the stake was made
    */
    uint public L_TOKEN;
    ...
    // Object containing the TOKEN and LUSD snapshots for a given active trove
    struct RewardSnapshot { uint TOKEN; uint LUSDDebt; }
    ...
    **/
    ...
    struct RedemptionTotals {
        ...
        uint totalTokenDrawn;
        uint tokenFee;
        uint tokenToSendToRedeemer;
        ...
        onlyOwner
    {
        ...
        // need to use appropriate address for price feed
    ...
    // change L_ETH to a public view method for compatible reason
    // it will always return correct value
    function L_ETH() external view returns (uint) {
        return L_TOKEN;
    }
    ...
    */
    function liquidateTroves(uint _n) external override {
        ...
        if (totals.totalCollSurplus > 0) {
            contractsCache.activePool.sendToken(address(collSurplusPool), totals.totalCollSurplus);
        }
    ...
    */
    function batchLiquidateTroves(address[] memory _troveArray) public override {
        ...
        if (totals.totalCollSurplus > 0) {
            activePoolCached.sendToken(address(collSurplusPool), totals.totalCollSurplus);
        }
    ...
    */
    ...
    function _sendGasCompensation(IActivePool _activePool, address _liquidator, uint _LUSD, uint _amount) internal {
        if (_LUSD > 0) {
            lusdToken.returnFromPool(gasPoolAddress, _liquidator, _LUSD);
        }

        if (_amount > 0) {
            _activePool.sendToken(_liquidator, _amount);
        }
    }

    // Move a Trove's pending debt and collateral rewards from distributions, from the Default Pool to the Active Pool
    function _movePendingTroveRewardsToActivePool(IActivePool _activePool, IDefaultPool _defaultPool, uint _LUSD, uint _amount) internal {
        _defaultPool.decreaseLUSDDebt(_LUSD);
        _activePool.increaseLUSDDebt(_LUSD);
        _defaultPool.sendTokenToActivePool(_amount);
    }
        ...
        internal returns (SingleRedemptionValues memory singleRedemption)
    {
        ...
        // Get the tokenLot of equivalent value in USD
        singleRedemption.tokenLot = debtToColl(singleRedemption.LUSDLot, _price);
        ...
        uint newColl = (Troves[_borrower].coll).sub(singleRedemption.tokenLot);
    ...
    */
    function _redeemCloseTrove(ContractsCache memory _contractsCache, address _borrower, uint _LUSD, uint _amount) internal {
        _contractsCache.lusdToken.burn(gasPoolAddress, _LUSD);
        // Update Active Pool LUSD, and send ETH to account
        _contractsCache.activePool.decreaseLUSDDebt(_LUSD);

        // send ETH from Active Pool to CollSurplus Pool
        // todo: check outside contract interface
        _contractsCache.collSurplusPool.accountSurplus(_borrower, _amount);
        _contractsCache.activePool.sendToken(address(_contractsCache.collSurplusPool), _amount);
    }
    ...
    */
        ...
        override
    {
        ...
        while (currentBorrower != address(0) && totals.remainingLUSD > 0 && _maxIterations > 0) {
            ...
            totals.totalTokenDrawn = totals.totalTokenDrawn.add(singleRedemption.tokenLot);

            totals.remainingLUSD = totals.remainingLUSD.sub(singleRedemption.LUSDLot);
            currentBorrower = nextUserToCheck;
        }
        require(totals.totalTokenDrawn > 0, "TroveManager: Unable to redeem any amount");

        // Decay the baseRate due to time passed, and then increase it according to the size of this redemption.
        // Use the saved total LUSD supply value, from before it was reduced by the redemption.
        _updateBaseRateFromRedemption(totals.totalTokenDrawn, totals.price, totals.totalLUSDSupplyAtStart);

        // Calculate fee
        totals.tokenFee = _getRedemptionFee(totals.totalTokenDrawn);

        _requireUserAcceptsFee(totals.tokenFee, totals.totalTokenDrawn, _maxFeePercentage);

        // Send token fee to the LQTY staking contract
        contractsCache.activePool.sendToken(address(contractsCache.lqtyStaking), totals.tokenFee);
        contractsCache.lqtyStaking.increaseRedeemingFee(totals.tokenFee);

        totals.tokenToSendToRedeemer = totals.totalTokenDrawn.sub(totals.tokenFee);

        emit Redemption(_LUSDamount, totals.totalLUSDToRedeem, totals.totalTokenDrawn, totals.tokenFee);
        ...
        contractsCache.activePool.sendToken(msg.sender, totals.tokenToSendToRedeemer);
    }
    ...
    function getNominalICR(address _borrower) public view override returns (uint) {
        (uint currentToken, uint currentLUSDDebt) = _getCurrentTroveAmounts(_borrower);

        uint NICR = _computeNominalCR(currentToken, currentLUSDDebt);
        return NICR;
    }
    ...
    function getCurrentICR(address _borrower, uint _price) public view override returns (uint) {
        ...
        uint ICR = _computeCR(currentToken, currentLUSDDebt, _price);
        return ICR;
    }
    ...
    function _getCurrentTroveAmounts(address _borrower) internal view returns (uint, uint) {
        ...
        uint currentToken = Troves[_borrower].coll.add(pendingTokenReward);
        uint currentLUSDDebt = Troves[_borrower].debt.add(pendingLUSDDebtReward);

        return (currentToken, currentLUSDDebt);
    }
    ...
    function _applyPendingRewards(IActivePool _activePool, IDefaultPool _defaultPool, address _borrower) internal {
        if (hasPendingRewards(_borrower)) {
            ...
            Troves[_borrower].coll = Troves[_borrower].coll.add(pendingTokenReward);
            Troves[_borrower].debt = Troves[_borrower].debt.add(pendingLUSDDebtReward);

            _updateTroveRewardSnapshots(_borrower);

            // Transfer from DefaultPool to ActivePool
            _movePendingTroveRewardsToActivePool(_activePool, _defaultPool, pendingLUSDDebtReward, pendingTokenReward);
    ...
    // Update borrower's snapshots of L_TOKEN and L_LUSDDebt to reflect the current values
    ...
    function _updateTroveRewardSnapshots(address _borrower) internal {
        rewardSnapshots[_borrower].TOKEN = L_TOKEN;
        rewardSnapshots[_borrower].LUSDDebt = L_LUSDDebt;
        emit TroveSnapshotsUpdated(L_TOKEN, L_LUSDDebt);
    }

    // Get the borrower's pending accumulated Token reward, earned by their stake
    function getPendingTokenReward(address _borrower) public view override returns (uint) {
        uint snapshotToken = rewardSnapshots[_borrower].TOKEN;
        uint rewardPerUnitStaked = L_TOKEN.sub(snapshotToken);

        if ( rewardPerUnitStaked == 0 || Troves[_borrower].status != Status.active) { return 0; }

        uint stake = Troves[_borrower].stake;

        uint pendingTokenReward = stake.mul(rewardPerUnitStaked).div(DECIMAL_PRECISION);
    ...
    function getPendingETHReward(address _borrower) public view override returns (uint) {
        return getPendingTokenReward(_borrower);
    }
    ...
    function hasPendingRewards(address _borrower) public view override returns (bool) {
        ...
        */
        ...
        return (rewardSnapshots[_borrower].TOKEN < L_TOKEN);
    }
    ...
    function getEntireDebtAndColl(
        ...
        view
        override
        returns (uint debt, uint coll, uint pendingLUSDDebtReward, uint pendingTokenReward)
    {
        ...
        coll = coll.add(pendingTokenReward);
    }
    ...
    function _redistributeDebtAndColl(IActivePool _activePool, IDefaultPool _defaultPool, uint _debt, uint _coll) internal {
        ...
        * error correction, to keep the cumulative error low in the running totals L_TOKEN and L_LUSDDebt:
        *
    ...
    function _closeTrove(address _borrower, Status closedStatus) internal {
        ...
        rewardSnapshots[_borrower].TOKEN = 0;
    ...
    */
    function _updateBaseRateFromRedemption(uint _tokenDrawn,  uint _price, uint _totalLUSDSupply) internal returns (uint) {
        uint decayedBaseRate = _calcDecayedBaseRate();

        /* Convert the drawn Collateral back to LUSD at face value rate (1 LUSD:1 USD), in order to get
        * the fraction of total supply that was redeemed at face value. */
        uint redeemedLUSDFraction = collToDebt(_tokenDrawn, _price).mul(DECIMAL_PRECISION).div(_totalLUSDSupply);
    ...
    function _getRedemptionFee(uint _tokenDrawn) internal view returns (uint) {
        return _calcRedemptionFee(getRedemptionRate(), _tokenDrawn);
    }

    function getRedemptionFeeWithDecay(uint _tokenDrawn) external view override returns (uint) {
        return _calcRedemptionFee(getRedemptionRateWithDecay(), _tokenDrawn);
    }

    function _calcRedemptionFee(uint _redemptionRate, uint _tokenDrawn) internal pure returns (uint) {
        uint redemptionFee = _redemptionRate.mul(_tokenDrawn).div(DECIMAL_PRECISION);
        require(redemptionFee < _tokenDrawn, "TroveManager: Fee would eat up all returned collateral");
