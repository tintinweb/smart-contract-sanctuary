...
contract LiquidityOracle is OwnableUpgradeable {
	...
	function getData() external view returns (uint256, bool) {
		...
		CumulativeLiquidityData memory cumulativeData = cumulativeLiquidityDatas[todayTimestamp];
		if (cumulativeData.counter == 0) {
			return (0, true);
		}
		
		return (cumulativeData.cumulativeLiquidities / cumulativeData.counter, true);
	}

	function update() external {
		uint256 todayTimestamp = block.timestamp / 1 days;
		CumulativeLiquidityData storage cumulativeData = cumulativeLiquidityDatas[todayTimestamp];
		(uint256 tempLiquidityData, , ) =
