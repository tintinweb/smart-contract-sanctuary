...
contract XVMCrapidAdoptionBoost is Ownable {
    ...
    bool public rapidAdoptionBoost; //can only be called once
    /*
    ...
    */
    ...
    uint256 public lastReward = 25 * 1e18;

    uint256 public preProgrammedCounter = 101; // used for Rapid Adoption Boost, to decrease rewards periodically
    uint256 public preProgrammedCounterTimestamp;

    bool public allowRewardDecrease;
     ...
     * Users get rewardedfor calling
     */
    function activateRapidAdoptionBoost() external {
        require(!rapidAdoptionBoost, "Already activated");
        rapidAdoptionBoost = true;
        IXVMCgovernor(owner()).setInflation(lastReward);
    }
    function increaseRewards() external {
        require(preProgrammedCounter > 100, "Decrease activated"); 

		require(preProgrammedCounterTimestamp + 58 minutes < block.timestamp, "must wait 15 minutes since last call");

        lastReward += goldenRatio * 1e15 * 4;
        IXVMCgovernor(owner()).setInflation(lastReward);
        preProgrammedCounter++; 
        preProgrammedCounterTimestamp = block.timestamp;

        if(lastReward > (200 * 1e18)) {
            preProgrammedCounter = 100;
        }

        IERC20(token).safeTransfer(payable(msg.sender), (2500*1e18));
    }

    function decreaseRewards() external {
        require(preProgrammedCounter <= 100 && preProgrammedCounter > 0, "Point of reward decrease not yet reached");

        if(lastReward < (40 * 1e18) || block.timestamp > 1640654801) { killAdoptionBoost(); }

        if(preProgrammedCounter == 100 || preProgrammedCounter > 25) {
            require(preProgrammedCounterTimestamp + 15 minutes < block.timestamp, "must wait 15 minutes since last call");
            lastReward -= goldenRatio * 1e15;
            IXVMCgovernor(owner()).setInflation(lastReward);
            preProgrammedCounter--; 
            preProgrammedCounterTimestamp = block.timestamp;
            IERC20(token).safeTransfer(payable(msg.sender), (1000*1e18));
        } else if(preProgrammedCounter <= 25 && preProgrammedCounter > 15) {
            require(preProgrammedCounterTimestamp + 1 hours < block.timestamp, "must wait 1 hour since last call");
            lastReward -= goldenRatio * 1e15;
            IXVMCgovernor(owner()).setInflation(lastReward);
            preProgrammedCounter--; 
            preProgrammedCounterTimestamp = block.timestamp;
            IERC20(token).safeTransfer(payable(msg.sender), (1000*1e18));
        } else if(preProgrammedCounter <= 15) {
            require(preProgrammedCounterTimestamp + 4 hours < block.timestamp, "must wait 4 hours since last call");
            lastReward -= goldenRatio * 1e15;
            IXVMCgovernor(owner()).setInflation(lastReward);
            preProgrammedCounterTimestamp = block.timestamp;
            IERC20(token).safeTransfer(payable(msg.sender), (1000*1e18));
    ...
    function killAdoptionBoost() internal {
        IXVMCgovernor(owner()).setInflation(25694201337 * 1e9); //set to roughly 25tokens/block.
        preProgrammedCounter = 0; 
    }

    /*
    Cancled due to delays :s . Perhaps it could be re-activated another year
    /**
     ...
     * 
     * Event starts on 23rd of November at roughly 17:12 UTC
     * Roughly +23.6% of entire supply is printed in a period of 7days
     * Those who do not stake, get diluted
     * meant to provide initial boost and encourage users to lock up their tokens
     *//*
    function startFibonacciPayout() external {
		require(!(IXVMCgovernor(owner()).fibonacciDelayed()), "event has been delayed");
        require(!bigFibonacciActivated && block.timestamp > 1637683883);
        ...
        uint256 newReward = toPrint / (169 * 360000 / IXVMCgovernor(owner()).blocks100PerSecond());
    ...
    */ /*
    function terminateFibonacciPayout() external {
