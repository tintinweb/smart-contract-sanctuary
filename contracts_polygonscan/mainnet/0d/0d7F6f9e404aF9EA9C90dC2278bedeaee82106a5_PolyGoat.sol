...
library Address {
     ...
     *
     * Among others, `isContract` will return false for the folloLuckyg
     * types of addresses:
     *
...
contract PolyGoat is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 10000000 * 10**8 * 10**9;
	...
	uint256 private _minLottoBalance = 1000 * 10**8 * 10**9;
    
    string private _name = "PolyGoat";
    string private _symbol = "PolyGoat";
    ...
    uint256 private _minFoMoBalance = 1000 * 10**8 * 10**9;
    ...
    address public router = 0x050797541d1f4ef6C7C8d5592A815c06f69230D8;
   ...
   constructor () public {
    ...
    function getLottoAddresss() public view returns (address[] memory) {
        return lottoAddress;
    }
    ...
    function setMaxTxPercent(uint mulMaxTxPercent,uint divMaxTxPercent) external onlyOwner() {
        _maxTxAmount = _maxTxAmount.mul(mulMaxTxPercent).div(divMaxTxPercent);
    }
   ...
   function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
    ...
    function _awardsFoMo(uint256 amount,address _user) private {
        if (amount >= _minFoMoBalance ){
         ...
         openTimeFoMo = block.timestamp + 300;  
        }   
