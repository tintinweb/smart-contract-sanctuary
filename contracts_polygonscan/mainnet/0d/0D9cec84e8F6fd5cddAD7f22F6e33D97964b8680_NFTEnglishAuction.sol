...
contract NFTEnglishAuction is IERC721Receiver, ERC721HolderUpgradeable {
    ...
    function createAuction(
        ...
        require(!NFTContract.isTokenOnSale(tokenIDs), "Already on sale");

        for (uint256 index = 0; index < tokenIDs.length; index++) {
            require(NFTContract.ownerOf(tokenIDs[index]) == msg.sender || NFTContract.viewAdminRole(msg.sender) || nftAddress == msg.sender, "Invalid caller");
        }

        for (uint256 index = 0; index < tokenIDs.length; index++) {
            address owner = NFTContract.getTokenByID(tokenIDs[index]).owner;
        ...
        NFTContract.setTokenOnAuction(tokenIDs, 3); // didn't find how to expose enum from MelonNFT properly
    ...
    function finalize(uint256 tokenID) public {
        ...
        require(msg.sender == auction.tokenOwner || msg.sender == auction.highestBidder || NFTContract.viewAdminRole(msg.sender), "Invalid caller");
        ...
        if (auction.symbol == 0) {
            ...
            if (NFTContract.getTokenByID(tokenID).referralAddress.length > 0) {
                referralFeeTotal = auction.highestBid.mul(NFTContract.getTokenByID(tokenID).referralFee).div(100);
                NFTContract.getTokenByID(tokenID).referralAddress[0].transfer(referralFeeTotal);
            }
            ...
            if (NFTContract.getTokenByID(tokenID).referralAddress.length > 0) {
                referralFeeTotal = auction.highestBid.mul(NFTContract.getTokenByID(tokenID).referralFee).div(100);
                melonToken.transferFrom(address(this), NFTContract.getTokenByID(tokenID).referralAddress[0], referralFeeTotal);
            }
        ...
        NFTContract.markTokenAsSold(tokenID, payable(auction.highestBidder));
    ...
    function withdraw(uint256 tokenID) public {
        ...
        require(auction.tokenOwner == msg.sender || NFTContract.viewAdminRole(msg.sender) || nftAddress == msg.sender, "Invalid caller");
        require(!auction.isFinished, "Auction already finished");
        ...
        tokenIDs[0] = tokenID;
        NFTContract.setTokenOnAuction(tokenIDs, 0);
        withdrawPendingOffers(auction);
    ...
    function approveOffer(uint256 tokenID, uint256 offerId) public {
        ...
        if (auction.symbol == 0) {
            feesAddress.transfer(comissionValue);
            if (NFTContract.getTokenByID(tokenID).referralAddress.length > 0) {
                referralFeeTotal = offer.offerValue.mul(NFTContract.getTokenByID(tokenID).referralFee).div(100);
                NFTContract.getTokenByID(tokenID).referralAddress[0].transfer(referralFeeTotal);
            }
        ...
        tokenIDToAuction[tokenID].isFinished = true;
        NFTContract.markTokenAsSold(tokenID, payable(offer.offerCreator));
...
contract MelonNFT is ERC721Upgradeable, AccessControlUpgradeable {
    IERC20 private melonToken;
    ...
    SaleStatuses public saleStatuses;
    ...
    mapping(uint256 => NFT_MODEL) public tokens;
    ...
    event SaleStatusChanged(uint256 tokenID, SaleStatuses saleStatus, uint256 symbol, uint256 value, uint256 timestamp);

    event SaleStatusChangedMultiple(uint256[] tokensIDs, SaleStatuses saleStatus, uint256 symbol, uint256 value, uint256 timestamp);
    ...
    function mintToken(
        ...
        if (nftModel.saleStatus == SaleStatuses.FIX_PRICE)
            require(
    ...
    function isTokenOnSale(uint256[] calldata tokenIds) public view returns (bool) {
        for (uint256 index = 0; index < tokenIds.length; index++) {
            if (tokens[tokenIds[index]].saleStatus == SaleStatuses.FIX_PRICE) {
                return true;
            }
        }
    ...
    function getTokenByID(uint256 tokenID) external view returns (NFT_MODEL memory token) {
...
library MelonLibrary {
    ...
    uint8 public constant MATIC = 0;
    uint8 public constant MELON = 1;
}
