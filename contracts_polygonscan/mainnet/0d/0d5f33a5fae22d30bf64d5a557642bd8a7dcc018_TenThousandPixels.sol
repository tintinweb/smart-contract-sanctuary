...
contract TenThousandPixels is ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;
    using Strings for uint256;

    uint256 public constant PIXELS_PUBLIC = 10000;
    uint256 public constant PIXELS_MAX = PIXELS_PUBLIC;
    ...
    Counters.Counter private _publicPIXELS;

    constructor() ERC721("100000 Pixels", "10PXLS") {}
    ...
    // minting
    function minting(uint256 numberOfTokens)
        external
        payable
        onlyOwner
    {
        ...
        require(
            _publicPIXELS.current() < 10000,
            "Purchase would exceed PIXELS_PUBLIC"
        );
  uint256 updatedNumAvailableTokens = _publicPIXELS.current();
    for (uint256 i = 0; i < numberOfTokens; i++) {
      uint256 tokenId = useRandomAvailableToken(numberOfTokens, i);
      _safeMint(msg.sender, tokenId);
      updatedNumAvailableTokens--;
    }
 //   _numAvailableTokens = updatedNumAvailableTokens;
 //       for (uint256 i = 0; i < numberOfTokens; i++) {
 //           uint256 tokenId = _publicPIXELS.current();
            if (_publicPIXELS.current() < PIXELS_PUBLIC) {
                _publicPIXELS.increment();
                //_safeMint(to, tokenId);
            }
        }
    ...
    returns (uint256 tokenId)
  {
    ...
    function purchase(uint256 numberOfTokens) external payable {
        ...
        require(
            _publicPIXELS.current() < PIXELS_PUBLIC,
            "Purchase would exceed PIXELS_PUBLIC"
        );
        require(
