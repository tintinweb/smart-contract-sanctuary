...
contract BearArmsToken is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public swapTokensAtAmount = 0.7 * 10**5 * (10**9); //75 thousand tokens
    uint256 public maxBuyAmount = 2.22 * 10**8 * 10**9; //2.5% of the supply per transaction
    uint256 public maxSellAmount = 1.776 * 10**8 * 10**9; // 2% of the supply per transaction
    uint256 public maxWalletAmount = 12.432 * 10**8 * 10**9; // 16% of the supply
  

    address public marketingWallet = address(0xd471259Fdaa310C3AbC015C1aeE23B2e553fA961); //set your marketing wallet
    
    address public charityWallet = address(0xFb3649c3e74A24EA043Cb9b83586d8c24D035253); //set your charity wallet

    uint8 public rewardsFee = 20; // 2% rewards to holders
    uint8 public liquidityFee = 20; //2% liquidity fee
    uint8 public marketingFee = 22; // 2.2% marketing fee
    uint8 public charityFee = 50; // 5% charity fee
    ...
    constructor() ERC20("Bear Arms", "USA") {
    ...
    function swapAndDistribute(uint256 tokens) private {
        ...
        uint16 walletfees = marketingFee + charityFee;

        swapAndSendToFee(newBalance.mul(walletfees).div(totalFees - liquidityFee),walletfees);
        swapAndSendDividends(newBalance.mul(rewardsFee - liquidityFee).div(totalFees));
