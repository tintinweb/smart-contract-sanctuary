...
contract Presale is Ownable, Pausable {
  ...
  //Where funds go (should be a multisig)
  ...
  event NewPresale(uint8 round, uint32 supply, uint32 cap, uint128 openingTime, uint128 closingTime, uint256 price, bool isPublicSale);
  ...
  function buyNFT(uint256 amount, bytes memory whitelistSig) external {
    ...
    //Interactions
    weth.transferFrom(purchaser, wallet, value);
  ...
  function setPresale(
    ...
    emit NewPresale(round, supply, cap, openingTime, closingTime, price, isPublicSale);
  }
