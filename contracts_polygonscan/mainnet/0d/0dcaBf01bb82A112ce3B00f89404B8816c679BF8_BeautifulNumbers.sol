    ...
    ERC721URIStorage
{
    ...
    uint64 public maxSupply = 999;
    uint64 public royalty = 500;

    constructor(string memory baseURI_) ERC721("Beautiful Numbers", "BFNRS") {
        baseURI = baseURI_;
    }

    function mintQuantity(uint64 _quantityToMint) public onlyOwner {
        require(_quantityToMint >= 1, "Minimum number to mint is 1");
        require(
            (_quantityToMint + totalSupply()) <= maxSupply,
            "Can't mint more than allowed maximum"
        );
