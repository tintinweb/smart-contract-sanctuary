...
contract RewardsDistribution is IRewardsDistribution, OwnableUpgradeable, AbstractDependant {
    ...
    modifier onlyDefiCore {
        require(defiCoreAddr == msg.sender, "RewardsDistribution: Caller not a DefiCore.");
        _;
    }

    modifier onlyEligibleContracts {
        require(
            defiCoreAddr == msg.sender || assetParameters.existingLiquidityPools(msg.sender),
        ...
        returns (uint256 _supplyAPY, uint256 _borrowAPY)
    {
        ILiquidityPool _governanceLP =
            ILiquidityPool(assetParameters.getGovernanceLiquidityPool());
    ...
    function withdrawUserReward(
        ...
        address _userAddr,
        ILiquidityPool _liquidityPool
    ) external override onlyDefiCore returns (uint256 _userReward) {
    ...
    function _getNewUserReward(
        ...
        if (_liquidityAmount > 0) {
            _newAggregatedReward +=
                ((_newSupplyCumulativeSum - userInfo.lastSupplyCumulativeSum) * _liquidityAmount) /
                DECIMAL;
        }

        if (_borrowAmount > 0) {
            _newAggregatedReward +=
                ((_newBorrowCumulativeSum - userInfo.lastBorrowCumulativeSum) * _borrowAmount) /
                DECIMAL;
        }
    }
