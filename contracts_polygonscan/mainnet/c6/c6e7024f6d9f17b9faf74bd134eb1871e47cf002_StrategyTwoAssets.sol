...
abstract contract StrategyFarmTwoAssets is BaseStrategyMasterChef {
        ...
        )
    {
        ...
        dino_usdt_path = new address[](3);
        dino_usdt_path[0] = dino;
        dino_usdt_path[1] = usdc;
        dino_usdt_path[2] = usdt;
    ...
    function harvest() public override {
        ...
        if (_dinoBalance > harvestCutoff){
            ...
            //Swap 1/2 of USDC for USDT
            uint256 _usdcBalance = IERC20(usdc).balanceOf(address(this));
            _swapUniswapWithPath(usdc_usdt_path, _usdcBalance.div(2));

            // Add liquidity for TITAN/MATIC
            uint256 _usdt = IERC20(usdt).balanceOf(address(this));
            uint256 _usdc = IERC20(usdc).balanceOf(address(this));
            if (_usdt > 0 && _usdc > 0) {
                IERC20(usdt).safeApprove(rewardRouter, 0);
                IERC20(usdt).safeApprove(rewardRouter, _usdt);
...
contract StrategyTwoAssets is StrategyFarmTwoAssets {
    ...
    address public USDC_USDT_LP = 0x2cF7252e74036d1Da831d11089D326296e64a728;
