...
// File: contracts/DoveToken.sol

pragma solidity 0.6.12;

// DoveToken with Governance.
contract DoveToken is ERC20('DoveSwap Token', 'Dove') {
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 accDovePerShare;    // Accumulated DOVEs per share, times 1e18. See below.
    ...
    uint256 public dovePerBlock = 10 ether;
    ...
    event UpdateEmissionRate(address indexed user, uint256 dovePerBlock);
    ...
    function pendingDove(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accDovePerShare = accDovePerShare.add(doveReward.mul(1e18).div(lpSupply));
        }
        return user.amount.mul(accDovePerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 doveReward = multiplier.mul(dovePerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        dove.mint(devAddress, doveReward.div(10));
        dove.mint(address(this), doveReward);
        pool.accDovePerShare = pool.accDovePerShare.add(doveReward.mul(1e18).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accDovePerShare).div(1e18).sub(user.rewardDebt);
            if (pending > 0) {
        ...
        if (_amount > 0) {
            ...
            if (pool.depositFeeBP > 0) {
                ...
                // Transfer 75% of deposit fee to Fee Address for token buyback and Polydove games
        ...
        user.rewardDebt = user.amount.mul(pool.accDovePerShare).div(1e18);
    ...
    // Safe dove transfer function, just in case if rounding error causes pool to not have enough DOVE.
    function safeDoveTransfer(address _to, uint256 _amount) internal {
        uint256 doveBal = dove.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > doveBal) {
            transferSuccess = dove.transfer(_to, doveBal);
        } else {
            transferSuccess = dove.transfer(_to, _amount);
        }
        require(transferSuccess, "safeDoveTransfer: Transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _dovePerBlock) public {
       
        massUpdatePools();
     
        dovePerBlock = _dovePerBlock;
        emit UpdateEmissionRate(msg.sender, dovePerBlock);
    }
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (address(referral) != address(0) && referralCommissionRate > 0) {
            ...
            if (referrer != address(0) && commissionAmount > 0) {
                dove.mint(_user, commissionAmount);
                dove.mint(referrer, commissionAmount);
    ...
    // Transfer Ownership of the Dove Token
    function transferTokenOwnership(address newOwner) public onlyOwner {
      
        require(newOwner != address(0) , "setNewOwner: FORBIDDEN");
       
        dove.transferOwnership(newOwner);
