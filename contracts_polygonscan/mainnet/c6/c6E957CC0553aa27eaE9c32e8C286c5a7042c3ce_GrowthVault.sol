...
contract GrowthVault is ERC20, Ownable {
    ...
    function autoCompound() public {
        ...
        for (uint256 i; i < names.length; i++) {
            ...
            if (_amount > 0) {
                ...
                // _swapExactTokensForETH(strategyObj.liquidityRouter, path);
    ...
    function _swapExactTokensForETH(address _liquidityRouter, address[] memory _path) internal {

        uint256 _balance = _getBalanceOfToken(_path[0]);
    ...
    function _swapTokensToNativeToken(
        ...
        if(_token0 != WETH) {
            path[0] = _token0;
            _swapExactTokensForETH(_liquidityRouter, path);
        }
