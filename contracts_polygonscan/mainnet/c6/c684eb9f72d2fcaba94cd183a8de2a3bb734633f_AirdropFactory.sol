...
// 相对时间V2
    struct AirdropV2 {
        ...
        // 领取黑名单 address => height
        mapping(address => uint256) claim_black;
...
contract AirdropFactory is Operatorable{
    ...
    // 空投活动
    mapping(uint256 => AirdropV2) private airdrops;
    
    event BlackAddress(uint256 index, address addr);
    event NewAirdrop(uint256 index, address pledgeAddress, uint256 pledgeAmount, address claimAddress, uint256 claimAmount, uint256 pledgecount, uint256 block, uint256 timestamp);
    ...
    // 鉴权此活动是否存在
    modifier AirdropCheck(uint256 _index) {
        // 活动角标大于最大数
    ...
    // 查看项目信息
    function AirdropOf(uint256 _index) public view AirdropCheck(_index) returns (
    ...
    // 此地址是否已质押
    function PledgeExist(uint256 _index, address addr) public view AirdropCheck(_index) returns (bool) {
        AirdropV2 storage airdrop = airdrops[_index];
    ...
    // 此地址是否已领取
    function ClaimExist(uint256 _index, address addr) public view AirdropCheck(_index) returns (bool) {
        AirdropV2 storage airdrop = airdrops[_index];
    ...
    // 此地址是否已赎回
    function WithdrawExist(uint256 _index, address addr) public view AirdropCheck(_index) returns (bool) {
        AirdropV2 storage airdrop = airdrops[_index];
        require(airdrop.addresses[addr] != 0, "THIS ADDRESS HAS NOT PLEDGE");
        return airdrop.withdraws[addr] != 0;
    }

    // 查询质押时间
    function PledgeTime(uint256 _index, address addr) public view AirdropCheck(_index) returns (uint256) {
        AirdropV2 storage airdrop = airdrops[_index];
        require(airdrop.addresses[addr] != 0, "THIS ADDRESS HAS NOT PLEDGE");
        return airdrop.addresses[addr];
    }

    // 等待领取时间
    function waitClaimTime(uint256 _index, address addr) public view AirdropCheck(_index) returns (uint256) {
        AirdropV2 storage airdrop = airdrops[_index];
        require(airdrop.addresses[addr] != 0, "THIS ADDRESS HAS NOT PLEDGE");
        return (airdrop.addresses[addr] + airdrop.claim_timestamp) - block.timestamp ;
    }


    // 等待赎回时间
    function waitWithdrawTime(uint256 _index, address addr) public view AirdropCheck(_index) returns (uint256) {
        AirdropV2 storage airdrop = airdrops[_index];
        require(airdrop.addresses[addr] != 0, "THIS ADDRESS HAS NOT PLEDGE");
        return (airdrop.addresses[addr] + airdrop.withdraw_timestamp) - block.timestamp ;
    }
    ...
    function CreateAirdrop(
        ...
        emit NewAirdrop(index, _pledge_token, _pledge_amount, _airdrop_token, _airdrop_amount, _airdrop_count, block.number, block.timestamp);
        
        index++;
    }

    // 添加项目黑名单
    function updateBlack(uint256 _index, address addr) public onlyOperator AirdropCheck(_index) {
        AirdropV2 storage airdrop = airdrops[_index];

        // 已参与人数大于修改后的的人数
        require(airdrop.claim_black[addr] == 0, "THIS ADDRESS HAS BEEN BLACKED");
        airdrop.claim_black[addr] = block.number;
        emit BlackAddress(_index, addr);
    }
    function updateBlacks(uint256 _index, address[] memory addrs) public onlyOperator AirdropCheck(_index) {
        AirdropV2 storage airdrop = airdrops[_index];
        
        for (uint256 i = 0; i < addrs.length; i++) {
            address addr = addrs[i];
            if (airdrop.claim_black[addr] == 0){
                // 已参与人数大于修改后的的人数
                airdrop.claim_black[addr] = block.number;
                emit BlackAddress(_index, addr);
            }
    ...
    // 修改活动人数
    function updateCount(uint256 _index, uint256 _count) public onlyOperator AirdropCheck(_index) {
        AirdropV2 storage airdrop = airdrops[_index];
        // 已参与人数大于修改后的的人数 
    ...
    // 修改活动开始区块高度
    function updateStartTimestamp(uint256 _index, uint256 _timestamp) public onlyOperator AirdropCheck(_index) {
        AirdropV2 storage airdrop = airdrops[_index];
    ...
    // 修改活动结束区块高度
    function updateEndTimestamp(uint256 _index, uint256 _timestamp) public onlyOperator AirdropCheck(_index) {
        AirdropV2 storage airdrop = airdrops[_index];
    ...
    // 修改领取空投区块高度
    function updateClaim(uint256 _index, uint256 _timestamp) public onlyOperator AirdropCheck(_index) {
        airdrops[_index].claim_timestamp = _timestamp;
    }
    
    // 修改赎回本金区块高度
    function updateWithdraw(uint256 _index, uint256 _timestamp) public onlyOperator AirdropCheck(_index) {
        airdrops[_index].withdraw_timestamp = _timestamp;
    }
    
    // 结束此项目
    function close(uint256 _index) public onlyOperator AirdropCheck(_index) {
    ...
    // 质押TOKEN
    function pledgeToken(uint256 project_id) public AirdropCheck(project_id) LOCK {
        // 判断项目是否存在
        // 判断项目是否已停止
    ...
    // 领取空投
    function claimToken(uint256 project_id) public AirdropCheck(project_id) LOCK {
        AirdropV2 storage airdrop = airdrops[project_id];

        IERC20 airdrop_token = IERC20(airdrop.airdrop_token);

        // 判断是否黑名单
        require(airdrop.claim_black[_msgSender()] == 0, "THIS ADDRESS IS BLACKED");
    ...
    // 赎回本金
    function withdrawToken(uint256 project_id) public AirdropCheck(project_id) LOCK {
        AirdropV2 storage airdrop = airdrops[project_id];
        ...
        // 判断时间是否满足
        require(airdrop.addresses[_msgSender()] + airdrop.withdraw_timestamp <= block.timestamp, "WITHDRAW TIMESTAMP IS NOT ENOUGH");
        ...
        require(balance >= airdrop.pledge_amount, "CONTRACT BALANCE IS NOT ENOUGH");
