...
contract P2PCards is Trustable {
	uint8 constant status_initial = 0;
	...
	event OrderCreated(uint256 indexed id, Order order);
	...
	function setToken(uint8 tokenId, ERC20 token) external onlyOwner {
		tokens[tokenId] = Token(token);
	...
	function create(
		...
		tok.token.safeTransferFrom(msg.sender, bridge, amount);
		...
		userOrders[msg.sender].add(nextOrderId);

		emit OrderCreated(nextOrderId, orders[nextOrderId]);
		nextOrderId++;
	}

	function complete(uint256 orderId) external onlyTrusted {
		Order memory order = orders[orderId];
		require(order.status == status_initial, "unexpected status in complete");
	...
	function listUserOrders() external view returns (Order[] memory) {
		EnumerableSet.UintSet storage ids = userOrders[msg.sender];
		Order[] memory list = new Order[](ids.length());
		for (uint256 i = 0; i < ids.length(); i++) {
			list[i] = orders[ids.at(i)];
		}
