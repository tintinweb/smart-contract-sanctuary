...
contract District is ERC721Upgradeable, OwnableUpgradeable, IDistrict, NativeMetaTransaction{
    ...
    mapping(bytes=>uint256) public nameDistrictOf;
    mapping(uint256=>bytes6[]) public districtNameOf;
    mapping(bytes6=>bool) public allowedCharacters;
    ...
    function setDistrictName(uint256 district_id, bytes6[] memory districtName) external {
        ...
        require(districtName.length < 16,"District: Name must be under 16 characters");
        for(uint256 i = 0; i < districtName.length; i++){
             encodedName = bytes.concat(encodedName, districtName[i]);
        }
        require(nameDistrictOf[encodedName] == 0, "name taken");
        nameDistrictOf[encodedName] = district_id;
    ...
    function claimDistrictLands(int128[] calldata _xs, int128[] calldata _zs,
                                uint256 _districtId) external override payable {
        require(claimable,
