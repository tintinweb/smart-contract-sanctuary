...
// Very simple timelock which owns some ownable and returns it when time is over
...
contract Timelock {
    ...
    uint public delay = 30 days;

    //Time when this timelock was deployed, masterchef will be locked for delay from here
    uint public timeLockDeployedTimeStamp;

    MasterChefRef public masterChef; //Masterchef under timelock

    constructor(address admin_, MasterChefRef _masterChef) public {
        admin = admin_;
        masterChef = _masterChef;
        timeLockDeployedTimeStamp = getBlockTimestamp();
    }

    receive() external payable { }

    /**
     * @dev Transfers ownership of the masterchef to a new account (`newOwner`).
     * Can only be called by the current admin.
     */
    function transferOwnership() public {
        require(msg.sender == admin, "Timelock::transferOwnership: Call must come from admin.");
        require(getBlockTimestamp() >= timeLockDeployedTimeStamp.add(delay), "Timelock::transferOwnership: Estimated execution block must satisfy delay.");

        masterChef.transferOwnership(admin);
    }
    ...
    function unlockToken(address token, address to, uint256 amount) public {
        require(msg.sender == admin, "Timelock::transferOwnership: Call must come from admin.");
        require(getBlockTimestamp() >= timeLockDeployedTimeStamp.add(delay), "Timelock::transferOwnership: Estimated execution block must satisfy delay.");

        IBEP20 tokenc = IBEP20(token);
        tokenc.transfer(to, amount);
    }

    function isUnlocked() public view returns (bool) {
        if(getBlockTimestamp() >= timeLockDeployedTimeStamp.add(delay)){
            return true;
        }
        return false;
    }
