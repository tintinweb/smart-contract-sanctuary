...
abstract contract StrategyFarmBase is BaseStrategyMasterChef {
    ...
    function harvest() public override {
        ...
        if (_vertBalance > harvestCutoff){
            ...
            // Swap
            _vertBalance = IERC20(vert).balanceOf(address(this));
            _swapUniswapWithPath(vert_usdc_path, _vertBalance.div(2));

            // Add liquidity
            uint256 _vert = IERC20(vert).balanceOf(address(this));
            uint256 _usdc = IERC20(usdc).balanceOf(address(this));
            if (_vert > 0 && _usdc > 0) {
                IERC20(vert).safeApprove(currentRouter, 0);
                IERC20(vert).safeApprove(currentRouter, _vert);
                ...
                // Donates DUST
                IERC20(vert).safeTransfer(
                    strategist,
                    IERC20(vert).balanceOf(address(this))
                );
...
contract StrategyBase is StrategyFarmBase {
    ...
    address public vert_USDC_LP = 0x668269d6E5D2c2dE31D132Ac218044211643622B;
