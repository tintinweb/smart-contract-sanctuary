...
contract Factory is IFactory {
	...
	IQuickswapFactory public uniswapV2Factory;
	...
	constructor(address _admin, address _reservesAdmin, IBDeployer _bDeployer, ICDeployer _cDeployer, IQuickswapFactory _uniswapV2Factory, ISimpleUniswapOracle _simpleUniswapOracle) public {
		admin = _admin;
	...
	function _getTokens(address uniswapV2Pair) private view returns (address token0, address token1) {
		token0 = IQuickswapPair(uniswapV2Pair).token0();
		token1 = IQuickswapPair(uniswapV2Pair).token1();
		require(uniswapV2Factory.getPair(token0, token1) == uniswapV2Pair, "YampFinance: NOT_UNIV2_PAIR");
	}
	...
	function createCollateral(address uniswapV2Pair) external returns (address collateral) {
		...
		require(getLendingPool[uniswapV2Pair].collateral == address(0), "YampFinance: ALREADY_EXISTS");		
	...
	function createBorrowable0(address uniswapV2Pair) external returns (address borrowable0) {
		_getTokens(uniswapV2Pair);
		require(getLendingPool[uniswapV2Pair].borrowable0 == address(0), "YampFinance: ALREADY_EXISTS");		
	...
	function createBorrowable1(address uniswapV2Pair) external returns (address borrowable1) {
		_getTokens(uniswapV2Pair);
		require(getLendingPool[uniswapV2Pair].borrowable1 == address(0), "YampFinance: ALREADY_EXISTS");		
	...
	function initializeLendingPool(address uniswapV2Pair) external {
		...
		require(!lPool.initialized, "YampFinance: ALREADY_INITIALIZED");
		
		require(lPool.collateral != address(0), "YampFinance: COLLATERALIZABLE_NOT_CREATED");
		require(lPool.borrowable0 != address(0), "YampFinance: BORROWABLE0_NOT_CREATED");
		require(lPool.borrowable1 != address(0), "YampFinance: BORROWABLE1_NOT_CREATED");
		...
		string memory name = string(abi.encodePacked("YampFinance UniV2: ", token0Symbol, "-", token1Symbol, "-", lendingPoolId));
		...
		name = string(abi.encodePacked("YampFinance UniV2: ", token0Symbol, "-", lendingPoolId));
		...
		name = string(abi.encodePacked("YampFinance UniV2: ", token1Symbol, "-", lendingPoolId));
	...
	function _setPendingAdmin(address newPendingAdmin) external {
		require(msg.sender == admin, "YampFinance: UNAUTHORIZED");
	...
	function _acceptAdmin() external {
		require(msg.sender == pendingAdmin, "YampFinance: UNAUTHORIZED");
	...
	function _setReservesPendingAdmin(address newReservesPendingAdmin) external {
		require(msg.sender == reservesAdmin, "YampFinance: UNAUTHORIZED");
	...
	function _acceptReservesAdmin() external {
		require(msg.sender == reservesPendingAdmin, "YampFinance: UNAUTHORIZED");
