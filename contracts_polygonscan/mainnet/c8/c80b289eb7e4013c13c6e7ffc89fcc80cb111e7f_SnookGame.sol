...
contract SnookGame is ISnookGame, AccessControlEnumerableUpgradeable, PausableUpgradeable {
    ...
    ) external override onlyRole(EXTRACTOR_ROLE) whenNotPaused()
    {
        ...
        _skinRewards.updatePeriod(tokenId, d.stars, stars);
    ...
    // fix
    function fixPeriod2TokenStars() public onlyRole(PAUSER_ROLE) {
      uint[20] memory tokenIds = [uint256(5),6,7,9,12,14,15,16,17,20,22,23,24,25,26,28,30,35,38,39];
      for (uint i=0; i<tokenIds.length; i++) {
    ...
    ) external override onlyRole(KILLER_ROLE) whenNotPaused
    {
        ...
        _skinRewards.updatePeriod(tokenId, d.stars, 0); // to update totalStars in the _periods
