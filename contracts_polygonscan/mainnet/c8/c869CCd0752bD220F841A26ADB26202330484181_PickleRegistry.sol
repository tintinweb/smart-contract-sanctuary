...
contract PickleRegistry {
  ...
  function developmentVaults() external view returns (VaultDetails[] memory) {
    uint256 vaults = development.length();
    VaultDetails[] memory details = new VaultDetails[](vaults);

    for (uint i = 0; i < vaults; i++) {
      address addr = development.at(i);
      IJar jar = IJar(addr);
      VaultDetails memory vaultDetails = VaultDetails({
        jar: addr
      });
      details[i] = vaultDetails;
    }
