...
contract Liquidity is Ownable {
    ...
    constructor(
      ...
      SANDMAN = IERC20(_SANDMAN);
      _allowance();
    }

  receive() external payable {}

  function withdrawSandManLiquidity() external onlyOwner {
      USDC.transfer(msg.sender, USDC.balanceOf(address(this)));
      SANDMAN.transfer(msg.sender, SANDMAN.balanceOf(address(this)));
  }
 ...
 function withdrawAll()  external onlyOwner {
  ...
  function AutoLiquidity() external onlyOwner {
    ...
    uint256 sandManBalance = SANDMAN.balanceOf(address(this));
    uint256 halfTokenAmount = sandManBalance / 2;

    if (usdcBalanceAfter > 0 && wmaticBalance > 0 && sandManBalance > 0) {
        // add the liquidity sandMan-matic
        ...
        // add the liquidity sandMan-usdc
        ROUTER.addLiquidity(
            address(SANDMAN),
            address(USDC),
            halfTokenAmount,
  ...
  function _allowance() internal {
    ...
    SANDMAN.approve(address(ROUTER), type(uint256).max);
  }
