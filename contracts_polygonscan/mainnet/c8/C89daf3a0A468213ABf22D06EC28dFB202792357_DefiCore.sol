...
contract DefiCore is IDefiCore, AbstractDependant {
    ...
    function getUserLiquidationInfo(
        ...
        _liquidationInfo = UserLiquidationInfo(
            ...
            _receiveAssetPrice,
            _bonusPrice,
            _borrowLiquidityPool.getUserBorrowedAmount(_userAddr, false),
        ...
        for (uint256 i = 0; i < _userSupplyAssets.length; i++) {
            ...
            if (isCollateralAssetEnabled(_userAddr, _currentAssetKey)) {
                uint256 _currentTokensAmount =
                    _currentAssetKey.getCurrentSupplyAmountInUSD(_userAddr, _poolRegistry);
    ...
    function getNewBorrowLimitInUSD(
        ...
        uint256 _newAmount =
            _assetKey.getLimitPart(_liquidityPool.getAmountInUSD(_tokensAmount), assetParameters);
    ...
    function disableCollateral(bytes32 _assetKey) external returns (uint256) {
        ...
        uint256 _currentSupplyAmount =
            _assetKey.getCurrentSupplyAmountInUSD(msg.sender, liquidityPoolRegistry);

        if (assetParameters.isAvailableAsCollateral(_assetKey) && _currentSupplyAmount > 0) {
            (uint256 _availableLiquidity, ) = getAvailableLiquidity(msg.sender);
            uint256 _currentLimitPart =
                _assetKey.getLimitPart(_currentSupplyAmount, assetParameters);
    ...
    function addLiquidity(bytes32 _assetKey, uint256 _liquidityAmount) external {
        ...
        priceManager.updatePrice(_assetKey);

        ILiquidityPool _assetLiquidityPool =
            _assetKey.getAssetLiquidityPool(liquidityPoolRegistry);

        require(
            getTotalSupplyBalanceInUSD(msg.sender) +
                _assetLiquidityPool.getAmountInUSD(_liquidityAmount) <=
                systemParameters.getMaxDepositPerAccountParam(),
            "DefiCore: The deposit cannot be more than the maximum value."
        );
        ...
        assetsRegistry.updateUserAssets(msg.sender, _assetKey, address(_assetLiquidityPool), true);
    }
    ...
    function borrow(bytes32 _assetKey, uint256 _borrowAmount) external {
        ...
        assetsRegistry.updateUserAssets(
            msg.sender,
            _assetKey,
    ...
    function liquidation(
        ...
        priceManager.updatePrice(_supplyAssetKey);
        priceManager.updatePrice(_borrowAssetKey);
        ...
        _assetsRegistry.updateUserAssets(
            _userAddr,
            _supplyAssetKey,
            address(_supplyAssetsPool),
            true
        );
        _assetsRegistry.updateUserAssets(
            _userAddr,
            _borrowAssetKey,
            address(_borrowAssetsPool),
            false
        );
    }
