...
contract ERC721NFTMarketV1 is ERC721Holder, Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        require(_WMATICAddress != address(0), "Operations: WMATIC address cannot be zero");
    ...
    /**
     * @notice Buy token with MATIC by matching the price of an existing ask order
     * @param _collection: contract address of the NFT
     * @param _tokenId: tokenId of the NFT purchased
     */
    function buyTokenUsingMATIC(address _collection, uint256 _tokenId) external payable nonReentrant {
        // Wrap MATIC
    ...
    /**
     * @notice Buy token with WMATIC by matching the price of an existing ask order
     * @param _collection: contract address of the NFT
     * @param _tokenId: tokenId of the NFT purchased
     * @param _price: price (must be equal to the askPrice set by the seller)
     */
    function buyTokenUsingWMATIC(
        address _collection,
        uint256 _tokenId,
        uint256 _price
    ) external nonReentrant {
        IERC20(WMATIC).safeTransferFrom(address(msg.sender), address(this), _price);
    ...
    function claimPendingRevenue() external nonReentrant {
        ...
        IERC20(WMATIC).safeTransfer(address(msg.sender), revenueToClaim);
     ...
     * @param _withMATIC: whether the token is bought with MATIC (true) or WMATIC (false)
     */
    function _buyToken(
        address _collection,
        uint256 _tokenId,
        ...
        // Transfer WMATIC
        IERC20(WMATIC).safeTransfer(askOrder.seller, netPrice);
        ...
        // Emit event
        emit Trade(_collection, _tokenId, askOrder.seller, msg.sender, _price, netPrice, _withMATIC);
    }
