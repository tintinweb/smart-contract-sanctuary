...
contract ShieldRewardBank is OwnableUpgradeSafe, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //   1. The pool's `accShieldPerShare` (and `lastRewardTime`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 lastRewardTime; // Last timestamp that SHIELDs distribution occurs.
        uint256 accShieldPerShare; // Accumulated SHIELDs per share, times 1e18. See below.
        address strategy; // Strategy address, use LP to farm and buy back SHIELD
    ...
    address public shield = address(0x896a4410dEdbc7f04e6143aCBF0713ab4249D8eB); // SHIELD
    ...
    //   > Dev/Mkt:     150,000,000 (15,15%)
    //   > Treasury:       100,000,000 (10,10%)
    //   > Insurance Fund:       50,000,000  (5,05%)
    ...
    event EarlyWithdrawFee(address indexed user, uint256 indexed pid, uint256 amount, uint256 fee);
    ...
    function initialize(address _shield, address _devFund, address _treasuryFund, address _insuranceFund, address _timelock,
        uint256 _totalRewardPerSecond, uint256 _startTime) external initializer {
        ...
        devRate = 1515; // 15,15%
        treasuryRate = 1010; // 10,10%
        insuranceRate = 505; // 5,05%
    ...
    function addPool(uint256 _allocPoint, IERC20 _lpToken, address _strategy, uint16 _depositFeeBP, uint16 _withdrawFeeBP, uint256 _lastRewardTime, uint256 _lockedTime, uint256 _earlyWithdrawFee) external onlyOwner {
        require(_allocPoint <= 100000, "too high allocation point"); // <= 100x
        require(_depositFeeBP <= 1000, "too high fee"); // <= 10%
        require(_withdrawFeeBP <= 1000, "too high fee"); // <= 10%
    ...
    function setPool(uint256 _pid, uint256 _allocPoint, uint16 _depositFeeBP, uint16 _withdrawFeeBP, uint256 _lockedTime, uint256 _earlyWithdrawFee) external onlyOwner {
        require(_allocPoint <= 100000, "too high allocation point"); // <= 100x
        require(_depositFeeBP <= 1000, "too high fee"); // <= 10%
        require(_withdrawFeeBP <= 1000, "too high fee"); // <= 10%
        require(_lockedTime <= 14 days, "locked time is too long");
    ...
    function changeStrategy(uint256 _pid, address _newStrategy) external onlyOwner {
        ...
        if (_newStrategy != address(0)) {
            ...
            require(IStrategy(_newStrategy).targetProfitToken() == shield, "!targetProfitToken");
    ...
    function setTotalRewardPerSecond(uint256 _totalRewardPerSecond) external onlyOwner {
        require(_totalRewardPerSecond <= 200 ether, "insane high rate");
        massUpdatePools();
        totalRewardPerSecond = _totalRewardPerSecond;
        _updateRewardPerSecond();
    }
    ...
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (now > pool.lastRewardTime && lpSupply != 0) {
            ...
            if (totalAllocPoint > 0) {
                uint256 _shieldReward = _seconds.mul(rewardPerSecond).mul(pool.allocPoint).div(totalAllocPoint);
                accShieldPerShare = accShieldPerShare.add(_shieldReward.mul(1e18).div(lpSupply));
            }
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalAllocPoint > 0) {
            uint256 _seconds = now.sub(pool.lastRewardTime);
            uint256 _shieldReward = _seconds.mul(rewardPerSecond).mul(pool.allocPoint).div(totalAllocPoint);
            pool.accShieldPerShare = pool.accShieldPerShare.add(_shieldReward.mul(1e18).div(lpSupply));
        }
        pool.lastRewardTime = now;
    }
    ...
    function _harvestReward(uint256 _pid, address _account) internal {
        ...
        if (user.amount > 0) {
            PoolInfo storage pool = poolInfo[_pid];
            uint256 _claimableAmount = user.amount.mul(pool.accShieldPerShare).div(1e18).sub(user.rewardDebt);
            if (_claimableAmount > 0) {
                require(_claimableAmount.mul(50) <= IERC20(shield).totalSupply(), "Suspicious big reward amount!!"); // <= 2% total supply
                emit RewardPaid(_account, _claimableAmount);

                _topupFunds(_claimableAmount);

                _safeSHIELDMint(address(this), _claimableAmount);
                _safeSHIELDTransfer(_account, _claimableAmount);
            }
        }
    }

    function _depositToStrategy(address _account, IERC20 _lpToken, address _strategy, uint256 _amount) internal {
    ...
    function _withdrawFromStrategy(address _account, IERC20 _lpToken, address _strategy, uint256 _amount) internal {
    ...
    function _checkStrategyBalance(IERC20 _lpToken, address _strategy, uint256 _totalLpBalance) internal view {
    ...
    function withdraw(uint256 _pid, uint256 _amount) public notContract nonReentrant checkHalving {
        ...
        if (_amount > 0) {
            ...
            uint256 _withdrawFeeBP = pool.withdrawFeeBP;
            if (_earlyWithdrawFee > 0 && insuranceFund != address(0) && now < unfrozenDepositTime(_pid, msg.sender)) {
                ...
                emit EarlyWithdrawFee(msg.sender, _pid, _amount, _earlyWithdrawFee);
            } else if (_withdrawFeeBP > 0 && insuranceFund != address(0)) {
                uint256 _fee = _amount.mul(_withdrawFeeBP).div(10000);
                _lpToken.safeTransfer(insuranceFund, _fee);
                _sentAmount = _sentAmount.sub(_fee);
                _lpToken.safeTransfer(insuranceFund, _earlyWithdrawFee);
                emit WithdrawFee(msg.sender, _pid, _amount, _fee);
            }
    ...
    function _safeSHIELDTransfer(address _to, uint256 _amount) internal {
        uint256 _shieldBal = IERC20(shield).balanceOf(address(this));
        if (_shieldBal > 0) {
            if (_amount > _shieldBal) {
                IERC20(shield).safeTransfer(_to, _shieldBal);
            } else {
                IERC20(shield).safeTransfer(_to, _amount);
            }
        }
    }

    function _safeSHIELDMint(address _to, uint256 _amount) internal {
        address _shield = shield;
        if (ICappedMintableBurnableERC20(_shield).minter(address(this)) && _to != address(0)) {
            uint256 _totalSupply = IERC20(_shield).totalSupply();
            uint256 _cap = ICappedMintableBurnableERC20(_shield).cap();
            uint256 _mintAmount = (_totalSupply.add(_amount) <= _cap) ? _amount : _cap.sub(_totalSupply);
            if (_mintAmount > 0) {
                ICappedMintableBurnableERC20(_shield).mint(_to, _mintAmount);
            }
        }
    }
    ...
    function _topupFunds(uint256 _claimableAmount) internal {
        ...
        if (ICappedMintableBurnableERC20(_shield).minter(address(this)) &&
            IERC20(_shield).totalSupply().add(_totalMintAmount) <= ICappedMintableBurnableERC20(_shield).cap()) {
            ICappedMintableBurnableERC20(_shield).mint(devFund, _devAmount);
            ICappedMintableBurnableERC20(_shield).mint(treasuryFund, _treasuryAmount);
            ICappedMintableBurnableERC20(_shield).mint(insuranceFund, _insuranceAmount);
    ...
    function emergencyWithdraw(uint256 _pid) external notContract nonReentrant {
        ...
        } else if (insuranceFund != address(0)) {
            uint256 _withdrawFeeBP = pool.withdrawFeeBP;
            if (_withdrawFeeBP > 0) {
                _withdrawFeeBP = _amount.mul(_withdrawFeeBP).div(10000);
                _sentAmount = _sentAmount.sub(_withdrawFeeBP);
                _lpToken.safeTransfer(insuranceFund, _withdrawFeeBP);
                emit WithdrawFee(msg.sender, _pid, _amount, _withdrawFeeBP);
            }
        }
        user.amount = 0;
        user.rewardDebt = 0;
