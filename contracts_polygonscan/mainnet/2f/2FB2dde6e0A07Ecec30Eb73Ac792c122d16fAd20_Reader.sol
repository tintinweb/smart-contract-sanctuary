...
import "../core/LosslessV2Pool.sol";
import "../core/LosslessV2Token.sol";
import "../interfaces/ILosslessV2Pool.sol";

/// @dev a reader contract for returning multiple pools info at once
/// to ease the loading stress of front end
contract Reader {
	using SafeMath for uint256;
	uint256 public constant PRECISION = 10000;

	/// @dev ------------------ pool last update timestamp ----------------------------

	function poolArrLastTimestamp(address[] memory poolArr_) public view returns (uint256[] memory results_) {
		results_ = new uint256[](poolArr_.length);
		for (uint256 i = 0; i < poolArr_.length; i++) {
			results_[i] = _poolLastTimestamp(poolArr_[i]);
		}
	}

	function poolLastTimestamp(address poolAddr_) public view returns (uint256 lastTimestamp_) {
		lastTimestamp_ = _poolLastTimestamp(poolAddr_);
	}

	/// @dev ------------------- pool status -------------------------------------------
	function poolArrStatus(address[] memory poolArr_) public view returns (ILosslessV2Pool.PoolStatus[] memory results_) {
		results_ = new ILosslessV2Pool.PoolStatus[](poolArr_.length);
		for (uint256 i = 0; i < poolArr_.length; i++) {
			results_[i] = _poolStatus(poolArr_[i]);
		}
	}

	function poolStatus(address poolAddr_) public view returns (ILosslessV2Pool.PoolStatus status_) {
		status_ = _poolStatus(poolAddr_);
	}

	/// @dev --------------------- pool apy boost------------------------------------------
	/// how many times of aave apy. value = actualValue*PRECISION
	function poolArrLongAPYBoost(address[] memory poolArr_) public view returns (uint256[] memory results_) {
		results_ = new uint256[](poolArr_.length);
		for (uint256 i = 0; i < poolArr_.length; i++) {
			results_[i] = _poolLongAPYBoost(poolArr_[i]);
		}
	}

	function poolArrShortAPYBoost(address[] memory poolArr_) public view returns (uint256[] memory results_) {
		results_ = new uint256[](poolArr_.length);
		for (uint256 i = 0; i < poolArr_.length; i++) {
			results_[i] = _poolShortAPYBoost(poolArr_[i]);
		}
	}

	function poolLongAPYBoost(address poolAddr_) public view returns (uint256 longAPY_) {
		longAPY_ = _poolLongAPYBoost(poolAddr_);
	}

	function poolShortAPYBoost(address poolAddr_) public view returns (uint256 shortAPY_) {
		shortAPY_ = _poolShortAPYBoost(poolAddr_);
	}

	/// @dev -------------------pool initial price -------------------------------------
	function poolArrInitialPrice(address[] memory poolArr_) public view returns (int256[] memory results_) {
		results_ = new int256[](poolArr_.length);
		for (uint256 i = 0; i < poolArr_.length; i++) {
			results_[i] = _poolInitalPrice(poolArr_[i]);
		}
	}

	function poolInitialPrice(address poolAddr_) public view returns (int256 initialPrice_) {
		initialPrice_ = _poolInitalPrice(poolAddr_);
	}

	/// @dev --------------------pool total token values---------------------------------
	/// short + long principal values
	function poolArrTotalShortAndLongValues(address[] memory poolArr_) public view returns (uint256[] memory results_) {
		results_ = new uint256[](poolArr_.length);
		for (uint256 i = 0; i < poolArr_.length; i++) {
			uint256 shortValue = _poolTotalShortTokenValues(poolArr_[i]);
			uint256 longValue = _poolTotalLongTokenValues(poolArr_[i]);
			results_[i] = shortValue.add(longValue);
		}
	}

	/// short + long + sponsor principal values
	function poolArrTotalShortLongSponsorValues(address[] memory poolArr_) public view returns (uint256[] memory results_) {
		results_ = new uint256[](poolArr_.length);
		for (uint256 i = 0; i < poolArr_.length; i++) {
			uint256 shortValue = _poolTotalShortTokenValues(poolArr_[i]);
			uint256 longValue = _poolTotalLongTokenValues(poolArr_[i]);
			uint256 sponsorValue = _poolTotalSponsorTokenValues(poolArr_[i]);
			results_[i] = shortValue.add(longValue).add(sponsorValue);
		}
	}

	function poolArrTotalSponsorValues(address[] memory poolArr_) public view returns (uint256[] memory results_) {
		results_ = new uint256[](poolArr_.length);
		for (uint256 i = 0; i < poolArr_.length; i++) {
			results_[i] = (_poolTotalSponsorTokenValues(poolArr_[i]));
		}
	}

	function poolArrTotalShortValues(address[] memory poolArr_) public view returns (uint256[] memory results_) {
		results_ = new uint256[](poolArr_.length);
		for (uint256 i = 0; i < poolArr_.length; i++) {
			results_[i] = (_poolTotalShortTokenValues(poolArr_[i]));
		}
	}

	function poolArrTotalLongValues(address[] memory poolArr_) public view returns (uint256[] memory results_) {
		results_ = new uint256[](poolArr_.length);
		for (uint256 i = 0; i < poolArr_.length; i++) {
			results_[i] = (_poolTotalLongTokenValues(poolArr_[i]));
		}
	}

	function poolTotalSponsorTokenValues(address poolAddr_) public view returns (uint256 totalSponsorValues_) {
		totalSponsorValues_ = _poolTotalSponsorTokenValues(poolAddr_);
	}

	function poolTotalShortTokenValues(address poolAddr_) public view returns (uint256 totalShortValues_) {
		totalShortValues_ = _poolTotalShortTokenValues(poolAddr_);
	}

	function poolTotalLongTokenValues(address poolAddr_) public view returns (uint256 totalLongValues_) {
		totalLongValues_ = _poolTotalLongTokenValues(poolAddr_);
	}

	function poolTotalValues(address poolAddr_) public view returns (uint256 totalShortValues_, uint256 totalLongValues_) {
		totalShortValues_ = _poolTotalShortTokenValues(poolAddr_);
		totalLongValues_ = _poolTotalLongTokenValues(poolAddr_);
	}

	/// @dev -------------------- user token amount --------------------------------
	function userShortTokenAmountPoolArr(address[] memory poolArr_, address userAddr_) public view returns (uint256[] memory results_) {
		results_ = new uint256[](poolArr_.length);
		for (uint256 i = 0; i < poolArr_.length; i++) {
			results_[i] = _userShortTokenAmount(poolArr_[i], userAddr_);
		}
	}

	function userLongTokenAmountPoolArr(address[] memory poolArr_, address userAddr_) public view returns (uint256[] memory results_) {
		results_ = new uint256[](poolArr_.length);
		for (uint256 i = 0; i < poolArr_.length; i++) {
			results_[i] = _userLongTokenAmount(poolArr_[i], userAddr_);
		}
	}

	function userSponsorTokenAmountPoolArr(address[] memory poolArr_, address userAddr_) public view returns (uint256[] memory results_) {
		results_ = new uint256[](poolArr_.length);
		for (uint256 i = 0; i < poolArr_.length; i++) {
			results_[i] = _userSponsorTokenAmount(poolArr_[i], userAddr_);
		}
	}

	function userTokenAmounts(address poolAddr_, address userAddr_) public view returns(uint256 shortAmount_, uint256 longAmount_, uint256 sponsorAmount_) {
		shortAmount_ = _userShortTokenAmount(poolAddr_, userAddr_);
		longAmount_ = _userLongTokenAmount(poolAddr_, userAddr_);
		sponsorAmount_ = _userSponsorTokenAmount(poolAddr_, userAddr_);
	}

	function userShortTokenAmount(address poolAddr_, address userAddr_) public view returns (uint256 shortAmount_) {
		shortAmount_ = _userShortTokenAmount(poolAddr_, userAddr_);
	}

	function userLongTokenAmount(address poolAddr_, address userAddr_) public view returns (uint256 longAmount_) {
		longAmount_ = _userLongTokenAmount(poolAddr_, userAddr_);
	}

	function userSponsorTokenAmount(address poolAddr_, address userAddr_) private view returns(uint256 sponsorAmount_) {
		sponsorAmount_ = _userSponsorTokenAmount(poolAddr_, userAddr_);
	}

	/// @dev --------------------user token values ---------------------------------

	function userShortTokenValuesPoolArr(address[] memory poolArr_, address userAddr_) public view returns(uint256[] memory results_) {
		results_ = new uint256[](poolArr_.length);
		for (uint256 i = 0; i < poolArr_.length; i++) {
			results_[i] = _userShortTokenValue(poolArr_[i], userAddr_);
		}
	}

	function userLongTokenValuesPoolArr(address[] memory poolArr_, address userAddr_) public view returns(uint256[] memory results_) {
		results_ = new uint256[](poolArr_.length);
		for (uint256 i = 0; i < poolArr_.length; i++) {
			results_[i] = _userLongTokenValue(poolArr_[i], userAddr_);
		}
	}
	// short + long
	function userShortLongValuesPoolArr(address[] memory poolArr_, address userAddr_) public view returns(uint256[] memory results_) {
		results_ = new uint256[](poolArr_.length);
		for (uint256 i = 0; i < poolArr_.length; i++) {
			results_[i] = _userLongTokenValue(poolArr_[i], userAddr_).add(_userShortTokenValue(poolArr_[i], userAddr_));
		}
	}

	// short + long + sponsor
	function userShortLongSponsorValuesPoolArr(address[] memory poolArr_, address userAddr_) public view returns(uint256[] memory results_) {
		results_ = new uint256[](poolArr_.length);
		for (uint256 i = 0; i < poolArr_.length; i++) {
			results_[i] = _userLongTokenValue(poolArr_[i], userAddr_).add(_userShortTokenValue(poolArr_[i], userAddr_)).add(_userSponsorTokenAmount(poolArr_[i], userAddr_));
		}
	}

	function userPoolTokenValues(address poolAddr_, address userAddr_) public view returns (uint256 shortValue_, uint256 longValue_) {
		shortValue_ = _userShortTokenValue(poolAddr_, userAddr_);
		longValue_ = _userLongTokenValue(poolAddr_, userAddr_);
	}

	function userShortTokenValue(address poolAddr_, address userAddr_) public view returns (uint256 shortValue_) {
		shortValue_ = _userShortTokenValue(poolAddr_, userAddr_);
	}

	function userLongTokenValue(address poolAddr_, address userAddr_) public view returns (uint256 longValue_) {
		longValue_ = _userLongTokenValue(poolAddr_, userAddr_);
	}

	/// @dev -------------------------privte functions---------------------------------

	function _userShortTokenAmount(address poolAddr_, address userAddr_) private view returns (uint256 shortAmount_) {
		(, address shortTokenAddr, ) = LosslessV2Pool(poolAddr_).poolTokensInfo();
		shortAmount_ = LosslessV2Token(shortTokenAddr).balanceOf(userAddr_);
	}

	function _userLongTokenAmount(address poolAddr_, address userAddr_) private view returns (uint256 longAmount_) {
		(address longTokenAddr, , ) = LosslessV2Pool(poolAddr_).poolTokensInfo();
		longAmount_ = LosslessV2Token(longTokenAddr).balanceOf(userAddr_);
	}

	function _userSponsorTokenAmount(address poolAddr_, address userAddr_) private view returns(uint256 sponsorAmount_) {
		(, , address sponsorTokenAddr) = LosslessV2Pool(poolAddr_).poolTokensInfo();
		sponsorAmount_ = LosslessV2Token(sponsorTokenAddr).balanceOf(userAddr_);
	}

	/// @dev read user pool token value ask short principal balance
	function _userShortTokenValue(address poolAddr_, address userAddr_) private view returns (uint256 shortValue_) {
		shortValue_ = LosslessV2Pool(poolAddr_).userShortPrincipalBalance(userAddr_);
	}

	/// @dev read user pool token value
	function _userLongTokenValue(address poolAddr_, address userAddr_) private view returns (uint256 longValue_) {
		longValue_ = LosslessV2Pool(poolAddr_).userLongPrincipalBalance(userAddr_);
	}

	/// @dev worth of shortToken totalSupply in the poor
	function _poolTotalShortTokenValues(address poolAddr_) private view returns (uint256 totalShortTokenValue_) {
		(, address shortTokenAddr, ) = LosslessV2Pool(poolAddr_).poolTokensInfo();

		uint256 shortTokenAmount = LosslessV2Token(shortTokenAddr).totalSupply();
		uint256 value = LosslessV2Pool(poolAddr_).valuePerShortToken();

		totalShortTokenValue_ = value.mul(shortTokenAmount);
	}

	/// @dev worth of shortToken totalSupply in the poor
	function _poolTotalLongTokenValues(address poolAddr_) private view returns (uint256 totalLongTokenValue_) {
		(address longTokenAddr, , ) = LosslessV2Pool(poolAddr_).poolTokensInfo();

		uint256 longTokenAmount = LosslessV2Token(longTokenAddr).totalSupply();
		uint256 value = LosslessV2Pool(poolAddr_).valuePerLongToken();

		totalLongTokenValue_ = value.mul(longTokenAmount);
	}

	function _poolTotalSponsorTokenValues(address poolAddr_) private view returns (uint256 totalSponsorTokenValue_) {
		(, , address sponsorTokenAddr) = LosslessV2Pool(poolAddr_).poolTokensInfo();
		uint256 tokenAmount = LosslessV2Token(sponsorTokenAddr).totalSupply();
		totalSponsorTokenValue_ = tokenAmount;
	}

	function _poolInitalPrice(address poolAddr_) private view returns (int256 initialPrice_) {
		(, , , , , , , initialPrice_, , ) = LosslessV2Pool(poolAddr_).status();
	}

	function _poolStatus(address poolAddr_) private view returns (ILosslessV2Pool.PoolStatus poolStatus_) {
		(, , , , , , , , , poolStatus_) = LosslessV2Pool(poolAddr_).status();
	}

	/// @dev how many times of aave apy (long+short+sponsor)/long
	function _poolLongAPYBoost(address poolAddr_) private view returns (uint256 longAPY_) {
		uint256 amount = _poolTotalLongTokenValues(poolAddr_).add(_poolTotalShortTokenValues(poolAddr_)).add(_poolTotalSponsorTokenValues(poolAddr_));
		longAPY_ = amount.mul(PRECISION).div(_poolTotalLongTokenValues(poolAddr_));
	}

	function _poolShortAPYBoost(address poolAddr_) private view returns (uint256 shortAPY_) {
		uint256 amount = _poolTotalLongTokenValues(poolAddr_).add(_poolTotalShortTokenValues(poolAddr_)).add(_poolTotalSponsorTokenValues(poolAddr_));
		shortAPY_ = amount.mul(PRECISION).div(_poolTotalShortTokenValues(poolAddr_));
	}

	/// @dev pool last timestamp
	function _poolLastTimestamp(address poolAddr_) private view returns (uint256 lastTimestamp_) {
		(, , , , , , lastTimestamp_, , , ) = LosslessV2Pool(poolAddr_).status();
	}
}
