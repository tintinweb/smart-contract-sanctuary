...
contract Ludus_Deorum is Ownable, ERC721 {
    
    uint public tokenPrice = 0.06 ether;
    uint constant maxSupply = 10000;
    uint public presalePrice = 0.045 ether;
    uint public totalSupply = 0;
    uint public preSupply = 550;
    ...
    constructor() ERC721("Ludus Deorum", "LDM"){}
   ...
   function buy_presale(uint _count) public payable{
        require(_count > 0, "mint at least one token");
        require(_count <= maxPerTransaction, "max per transaction 10");
        require(totalSupply + _count <= preSupply, "Not enough tokens left");
    ...
    function setBaseUri(string memory _uri) external onlyOwner {
     ...
     function publicprice(uint temp) external onlyOwner {
        tokenPrice = temp;
    }
     function presale_price(uint temp) external onlyOwner {
        presalePrice = temp;
    }
