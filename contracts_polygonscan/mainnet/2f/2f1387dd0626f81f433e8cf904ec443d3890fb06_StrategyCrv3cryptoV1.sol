...
abstract contract StrategyBase {
    ...
    // Dex
    address public univ2Router2 = 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff;//quickswao
...
contract StrategyCrv3cryptoV1 is StrategyBase {
    address public constant crv3cypto = 0xdAD97F7713Ae9437fa9249920eC8507e5FbB23d3;
    address public constant USDC = 0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174;
    address public constant CRV = 0x172370d5Cd63279eFa6d502DAB29171933a610AF;

    // staking constants
    address public constant rewardGauge = 0x3B6B158A76fd8ccc297538F454ce7B4787778c7C;
    address public constant stakingGauge = 0x1d8b86e3D88cDb2d34688e87E72F388Cb541B7C8;
    ...
    function _convertWantToBuyback(uint256 _lpAmount) internal override returns (address, uint256){
        ...
        //swap USDC to WMATIC
        if(_USDCBal > 0){
            _swapUniswap(USDC, WMATIC, _USDCBal);
        }
        return (WMATIC, IERC20(WMATIC).balanceOf(address(this)));
    }

    function harvest() public override onlyBenignCallers {

        // Collects WFTM tokens
        _claimRewards();

        // if include CRV rewards
        uint256 _rewardsCRV = IERC20(CRV).balanceOf(address(this));
        if (_rewardsCRV > 0) {
            address[] memory crv2Matic = new address[](3);
            crv2Matic[0] = CRV;
            crv2Matic[1] = weth;
            crv2Matic[2] = WMATIC;
            _swapUniswapWithDetailConfig(CRV, WMATIC, _rewardsCRV, 1, crv2Matic, sushiRouter);
        }
        ...
        if (_wmatic > 0) {
            ...
            _swapUniswap(WMATIC, USDC, IERC20(WMATIC).balanceOf(address(this)));
            uint256 _USDCBal = IERC20(USDC).balanceOf(address(this));
    ...
    function _claimRewards() internal{
        LpTokenStaker(rewardGauge).claim_rewards();//claim WMATIC/CRV
    }
    ...
    function getHarvestableMATIC() public view returns (uint256) {
        return LpTokenStaker(rewardGauge).claimable_reward_write(address(this), WMATIC);
    }

    function getName() external override pure returns (string memory) {
        return "StrategyCrv3cryptoV1";
    }
}