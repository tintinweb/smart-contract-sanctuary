...
// File contracts/YumMUSDVault.sol
...
contract YumMUSDVault is ReentrancyGuard {
  ...
  {
    require(_governance != ZERO_ADDRESS, "YumMUSDVault: governance address cannot be 0x0.");
    require(_sentinel != ZERO_ADDRESS, "YumMUSDVault: sentinel address cannot be 0x0.");
    require(_feeCollector != ZERO_ADDRESS, "YumMUSDVault: feeCollector address cannot be 0x0.");
  ...
  function setPendingGovernance(address _pendingGovernance) external onlyGov {
    require(_pendingGovernance != ZERO_ADDRESS, "YumMUSDVault: governance address cannot be 0x0.");
  ...
  function setTransmuter(address _transmuter) external onlyGov {
    ...
    require(_transmuter != ZERO_ADDRESS, "YumMUSDVault: transmuter address cannot be 0x0.");
  ...
  function setRewards(address _rewards) external onlyGov {
    ...
    // transfers to the address because of `safeTransfer` checks.
    require(_rewards != ZERO_ADDRESS, "YumMUSDVault: rewards address cannot be 0x0.");
  ...
  function setHarvestFee(uint256 _harvestFee) external onlyGov {
    ...
    require(_harvestFee <= PERCENT_RESOLUTION, "YumMUSDVault: harvest fee above maximum.");
  ...
  function setFeeLevel(uint256[] calldata _feeLevel) external onlyGov {
    require(_feeLevel.length == 9, "YumMUSDVault: feeLevel length mismatch");
    feeLevel = _feeLevel;
  }

  function setDiscountTable(uint256[] calldata _discountTable) external onlyGov {
    require(_discountTable.length == 9, "YumMUSDVault: discountTable length mismatch");
  ...
  function setCollateralizationLimit(uint256 _limit) external onlyGov {
    ...
    require(_limit >= MINIMUM_COLLATERALIZATION_LIMIT, "YumMUSDVault: collateralization limit below minimum.");
    require(_limit <= MAXIMUM_COLLATERALIZATION_LIMIT, "YumMUSDVault: collateralization limit above maximum.");
  ...
  /// @dev Set oracle.
  function setOracleAddress(address _oracle, uint256 _peg) external onlyGov {
    linkGasOracle = _oracle;
    pegMinimum = _peg;
    emit OracleUpdated(_oracle, _peg);
  }
  ...
  function initialize(IVaultAdapterV2 _adapter) external onlyGov {
    ...
    require(!initialized, "YumMUSDVault: already initialized");

    require(transmuter != ZERO_ADDRESS, "YumMUSDVault: cannot initialize transmuter address to 0x0");
    require(rewards != ZERO_ADDRESS, "YumMUSDVault: cannot initialize rewards address to 0x0");
  ...
  function harvest(uint256 _vaultId) external expectInitialized noContractAllowed returns (uint256, uint256) {
    ...
    if (_harvestedAmount > 0) {
      ...
      if (_distributeAmount > 0) {
        ...
        xtoken.lowerHasMintedIfNeeded(_distributeAmount);
      }
    }
  ...
  function repay(uint256 _parentAmount, uint256 _childAmount) external nonReentrant noContractAllowed onLinkCheck expectInitialized {
    ...
    if (_parentAmount > 0) {
      ...
      xtoken.lowerHasMinted(_parentAmount);
    }
  ...
  function liquidate(uint256 _amount) external nonReentrant noContractAllowed onLinkCheck expectInitialized returns (uint256, uint256) {
    ...
    xtoken.lowerHasMintedIfNeeded(_withdrawnAmount);

    _cdp.totalDeposited = _cdp.totalDeposited.sub(_decreasedValue, "");
    if (_withdrawnAmount > _cdp.totalDebt) {
      _cdp.totalDebt = 0;
    } else if (_withdrawnAmount > _amount) {
      _cdp.totalDebt = _cdp.totalDebt.sub(_amount, "");
    } else {
  ...
  function mint(uint256 _amount) external nonReentrant noContractAllowed onLinkCheck expectInitialized {
    ...
    if (_totalCredit < _amount) {
      ...
      _cdp.checkHealth(_ctx, "YumMUSDVault: Loan-to-value ratio breached");
    } else {
  ...
  modifier onLinkCheck() {
    if(pegMinimum > 0 ){
      uint256 oracleAnswer = uint256(IChainlink(linkGasOracle).latestAnswer());
  ...
  modifier expectInitialized() {
    require(initialized, "YumMUSDVault: not initialized.");
    _;
  }
  ...
  ///
  modifier onlyGov() {
    require(msg.sender == governance, "YumMUSDVault: only governance.");
    _;
  }
  ...
  function _updateActiveVault(IVaultAdapterV2 _adapter) internal {
    require(_adapter != IVaultAdapterV2(ZERO_ADDRESS), "YumMUSDVault: active vault address cannot be 0x0.");
    require(_adapter.token() == token, "YumMUSDVault: token mismatch.");
  ...
  function _recallFunds(uint256 _vaultId, uint256 _amount) internal returns (uint256, uint256) {
    require(emergencyExit || msg.sender == governance || _vaultId != _vaults.lastIndex(), "YumMUSDVault: not an emergency, not governance, and user does not have permission to recall funds from active vault");
