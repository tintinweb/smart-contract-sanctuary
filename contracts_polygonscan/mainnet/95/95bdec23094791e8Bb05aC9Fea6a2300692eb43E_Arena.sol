...
contract Arena is Ownable, ERC721Holder {
    ...
    constructor() {
        ...
        genNFT = IGenNFT(0x330b06C695CfBb51D0cf9D5ed3debA2BD7eFFfB4);
    ...
    function fight(address enemy) public {
        ...
        for (uint8 i=0;i<3;i++) {
            if (cycle.health[myTeam[i]] > 0) {
                uint16 damage = attack(i,msg.sender,enemy, mySeed, 120, 0) * 10 + uint16(i);
            ...
            if (getRoundIndex() > 0 && cycle.rounds[enemy][getRoundIndex()] == 0) {
                ...
                uint16 damage = attack(i,enemy,msg.sender, enemySeed, 100, 2 + cyclies[getCycleIndex()].fightNumber[msg.sender]) * 10 + uint16(i);
    ...
    function getFighterInfo(address user, uint cycleIndex) view public returns (FighterInfo memory) {
        ...
        return FighterInfo(user,
            ...
            cyclies[roundIndex].rounds[user][roundIndex],
            (roundIndex > 0 && cycle.rounds[user][roundIndex] == 0),
            cycle.places[user]
            );
    }
