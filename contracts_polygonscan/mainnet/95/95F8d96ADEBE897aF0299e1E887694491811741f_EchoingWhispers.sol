 ...
 */ 
 contract EchoingWhispers is Ownable {
    struct QuestData {
    ...
    function setEnclave(address templar, bytes32 enclave) external onlyOwner {
        dataOf[templar].enclave = enclave;
    }
        
    function setCompleted(address templar, uint256 nCompleted) external onlyOwner {
        dataOf[templar].completed = nCompleted;
    }

    /** Owner only method to udpate any/all user quest data if required by enclave of order to support questers */
    function setSavePoint(address templar, bytes32 stepWhenLocked, bytes32 stepWhenUnlocked, uint256 lockedUntil) external onlyOwner {
