...
contract IDOVesting is ReentrancyGuard,Ownable {
    ...
    function _availableClaimableAmount(address _beneficiary) internal view returns (uint256 _amount) {
        uint256 firstUnlock = start + unlockPeriod;

        if (_getNow() < firstUnlock) {
            _amount = 0;
        }
        ...
        // Vesting Ongoing
        else if(_getNow() >= firstUnlock){
            uint256 maxClaimable = (vestedAmount[_beneficiary] * 50) / 100;
            uint256 userTotalClaimed = totalClaimed[_beneficiary];
            if(userTotalClaimed < maxClaimable) _amount = maxClaimable - userTotalClaimed;
        }
