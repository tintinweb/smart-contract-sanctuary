...
contract SeedVault is Ownable, Pausable, ReEntrancyGuard {
    ...
    uint256 public withdrawFeePeriod = 2 hours; // 2 hours
    
    uint256 public seedPoolPid = 0;     // Seed Pool pid in masterchef
    ...
    function recentRewardBalance(address _user) external view returns (uint256) {
        if (totalShares == 0) {
            return 0;
        }
        UserInfo storage user = userInfo[_user];	
        return user.shares.mul(balanceOf()).div(totalShares).sub(user.treeAtLastUserAction)
        .add(user.shares.mul(IMasterChef(masterchef).pendingToken(seedPoolPid, address(this))).div(totalShares));	
    }	
     ...
     */
    function deposit(uint256 _ignore, uint256 _amount, address _referrer) external whenNotPaused notContract {
    ...
    function withdrawAll() external notContract {
        withdraw(seedPoolPid, userInfo[msg.sender].shares);
    }
     ...
     * @dev Only possible when contract not paused.
     */
    function harvest(address _referrer) public whenNotPaused notContract {
        uint256 PendingTree = IMasterChef(masterchef).pendingToken(seedPoolPid, address(this));
    ...
    function calculateHarvestTreeRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingToken(seedPoolPid, address(this));
     ...
     */
    function withdraw(uint256 _ignore, uint256 _shares) public noReentrant notContract {
        UserInfo storage user = userInfo[msg.sender];
     ...
     */
    function _earn(address _referrer) internal {
        uint256 bal = available();
        if (bal > 0) {
            IMasterChef(masterchef).deposit(seedPoolPid, bal, _referrer);
        }
    }
