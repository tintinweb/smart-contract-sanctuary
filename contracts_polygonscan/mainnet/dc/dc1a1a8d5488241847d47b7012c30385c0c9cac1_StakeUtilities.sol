...
contract StakeUtilities is Manageable, ExternallyCallable {
    event StakeNamed(address account, uint256 stakeId, string oldName, string newName);
    event StakeSplit(uint256 newStakeId, uint256 oldStakeId);
    ...
    /** NFT Parameters */
    uint256 public baseNFTCost;
    uint256 public nftScalingAmount;
    uint256 public maxNFTCost;
    ...
    function splitStake(uint256 stakeId, uint256 percentage) external payable {
        require(msg.value == splitCost, 'SplitStake: Invalid cost');
        require(percentage >= 1, 'SplitStake: 1-99% only');
        require(percentage <= 99, 'SplitStake: 1-99% only');
        require(
            contracts.stakeToken.isOwnerOf(msg.sender, stakeId) ||
                contracts.stakeCustodian.isOwnerOf(msg.sender, stakeId),
            'SplitStake: Not owner of stake.'
        );

        uint256 newStakeId = contracts.stakeManager.splitStake(stakeId, percentage);
        require(
            contracts.stakeCustodian.addStake(msg.sender, newStakeId) == true,
            'SplitStake: Failed to split stake'
        );

        emit StakeSplit(newStakeId, stakeId);
    }
    ...
    /** @dev
        ...
        @param stakeId {uint256}
     */
    function convertToNft(uint256 stakeId) external payable {
        StakeData1e18 memory stake = contracts.stakeManager.getStake(stakeId);

        uint256 scale = (stake.amount / nftScalingAmount);
        if (scale > 0) scale - 1;
        uint256 cost = baseNFTCost + (scale * 1e18);
        if (cost > maxNFTCost) cost = maxNFTCost;

        require(msg.value == cost, 'ConvertToNft: Invalid cost');
        require(
    ...
    function setConvertToNftCost(
        uint256 _baseNFTCost,
        uint256 _nftScalingAmount,
        uint256 _maxNFTCost
    ) external onlyManager {
        baseNFTCost = _baseNFTCost;
        nftScalingAmount = _nftScalingAmount;
        maxNFTCost = _maxNFTCost;
    }
     ...
     */
    function release(address to) external onlyManager {
