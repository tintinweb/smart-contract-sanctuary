...
contract Marketplace is PriceConsumerV3, EIP712MetaTransaction, ReentrancyGuard {
  ...
  /**
    ...
    */
    function removeSetForSaleFiat(uint256 _tokenId) 
        public 
        isActive 
        onlyDbilia 
    {
  ...
  /**
    ...
    */
    function purchaseWithFiatw2user(
        uint256 _tokenId, 
  ...
  /**
    ...
    */
    function purchaseWithFiatw3user(
        uint256 _tokenId, 
        address _buyer
    ) 
        public 
  ...
  /**
    ...
    */
        ...
        verifyPasscode(_passcode) 
    {
        ...
        uint256 royaltyAmount = _sendRoyalty(_tokenId, wethAmount, true);
  ...
  /**
    ...
    */
        ...
        verifyPasscode(_passcode) 
    {
        ...
        uint256 fee = _payBuyerSellerFee(wethAmount);
        uint256 royaltyAmount = _sendRoyalty(_tokenId, wethAmount, false);
        uint256 sellerReceiveAmount = wethAmount.sub(fee.add(royaltyAmount));
  ...
  /**
    ...
    * @param _payDirect true sends directly to creator, false to dbiliaTrust 
    */
    function _sendRoyalty(
        uint256 _tokenId, 
        uint256 _wethAmount,
        bool _payDirect
    ) private returns (uint256) {
        uint256 feePercent = dbiliaToken.feePercent();
        (address receiver, uint16 percentage) = dbiliaToken.getRoyaltyReceiver(_tokenId);
        uint256 firstFee = _wethAmount.mul(feePercent).div(feePercent + 1000);
        uint256 royalty = _wethAmount.sub(firstFee).mul(percentage).div(100);
        _send(royalty, _payDirect ? receiver : dbiliaToken.dbiliaTrust());
        return royalty;
    }
  ...
  /**
    ...
    * instead of manual converting from fiat to WETH
    * @param _bidPriceFiat bid price in fiat
    */
