...
contract StrategyToken is BaseStrategyToken {
    ...
    constructor(
        ...
        address[] memory _wnativeToWantedPath
    ) {
        ...
        wnativeToWantedPath = _wnativeToWantedPath;
...
abstract contract BaseStrategyToken is BaseStrategy {
    ...
    address[] public wnativeToWantedPath;
    ...
    function _earn(address _to) internal {
        ...
        if (earnedAmt > 0) {
            ...
            uint256 usdcAmt = IERC20(usdAddress).balanceOf(address(this));

            if (usdcAmt > 0 && earnedAddress != wantAddress){
                _safeSwap(
                    usdcAmt,
                    wnativeToWantedPath,
                    address(this)
    ...
    function buyBack(uint256 _earnedAmt) internal virtual returns (uint256) {
        if (buyBackRate > 0) {
            ...
            if (earnedAddress == destructionAddress) {
                ...
                _safeSwap(
                    buyBackAmt,
                    earnedToWantedPath, // earn to destruction
                    buyBackAddress
                );
            }
