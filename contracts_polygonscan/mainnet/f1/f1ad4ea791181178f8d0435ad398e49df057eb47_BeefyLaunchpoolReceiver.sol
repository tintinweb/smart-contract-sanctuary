...
contract BeefyLaunchpoolReceiver is Ownable {
    ...
    event LaunchpoolCreated(address receiver);
    ...
    modifier onlyManager() {
        require(msg.sender == dev || msg.sender == owner() || msg.sender == multisig, "!manager");
        _;
    }

    function initialize(address _multisig, address _dev, address _token, uint256 _fee) external onlyManager {
    ...
    function setLaunchpool(address _launchpool) external onlyManager {
    ...
    function createLaunchpool(address _stakedToken, uint8 _durationDays) external onlyManager returns (address _launchpool) {
        uint256 duration = 3600 * 24 * _durationDays;
        bytes memory code = type(BeefyLaunchpool).creationCode;
        bytes memory bytecode = abi.encodePacked(code, abi.encode(_stakedToken, token, duration));
        assembly {
            _launchpool := create(0, add(bytecode, 32), mload(bytecode))
        }
        launchpool = _launchpool;
        emit LaunchpoolCreated(launchpool);
    }

    function send() external onlyMultisig {
        require(launchpool != address(0), "!launchpool");

        uint feeAmount = IERC20(token).balanceOf(address(this)).mul(fee).div(FEE_MAX);
        ...
        IERC20(rewardToken).transfer(launchpool, rewardBal);
    }
