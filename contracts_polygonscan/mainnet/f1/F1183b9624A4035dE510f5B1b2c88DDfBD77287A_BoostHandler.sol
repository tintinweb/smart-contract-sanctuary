...
contract BoostHandler is Ownable, IBoostHandler {
    ...
    uint256 public constant MAX_BOOST = 10 ** 18; //1e18 instead of 10000, will need to disable boost in old contracts and deploy a new proxy to be extra safe

    address public multiFeeDist = 0x920f22E1e5da04504b765F8110ab96A20E6408Bd;
    address public lockedStakingVault = 0xC5bCD23f21B6288417eB6C760F8AC0fBb4bb8a56;

    address public calculator = 0xb5331bB28dD7a297319afaB7c32B37D8602bFE6F;
    address public minter = 0xAAE758A2dB4204E1334236Acd6E6E73035704921;

    address public aprCalcAggregator = 0x10b11d80fD03204Bedb503149D81aEc8368092F3;

    address GHST_WETH = 0xcCB9d2100037f1253e6C1682AdF7dC9944498AFF;
    address STKGHST_WETH = 0x388E2a3d389F27504212030c2D42Abf0a8188cd1;

    // **** Views **** //

    function getBoost(address _user, address _vaultAddress) external override view returns (uint256) {
        if(IJar(_vaultAddress).balance() == 0) {
            return 0;
        }

        uint256 ratio = IJar(_vaultAddress).getRatio();
        uint256 userBal = IVault(_vaultAddress).balanceOf(_user).mul(ratio).div(10 ** 18);
        address token = address(IVault(_vaultAddress).token());
        //Yet another STKGHST_WETH special case
        if(token == STKGHST_WETH) {
            token = GHST_WETH;
        }

        //Show user a warning if the boost would be 0, if boost is 0 when their veADDY is > 0?
        (, uint256 valueOfUserStakeUSD) = IPriceCalculatorUSD(calculator).unsafeValueOfAsset(token, userBal);
    ...
    function getBoostForValueStaked(address _user, address _vaultAddress, uint256 valueStaked) external override view returns (uint256) {
        ...
        valueOfUserStakeUSD = valueOfUserStakeUSD.add(valueStaked);
    ...
    function getDoubleLimit(address _user) public override view returns (uint256 doubleLimit) {
        ...
        return getTotalVeAddy(_user).mul(IMinter(minter).addyPerProfitEth());
    }

    function getTotalVeAddy(address _user) public override view returns (uint256 veAddy) {
        if(lockedStakingVault != address(0)) {
            veAddy = veAddy.add(getVeAddyFromLockedStaking(_user));
        }
    }

    function getVeAddyFromLockedStaking(address _user) public override view returns (uint256 veAddy) {
        uint256 endingTimestamp = ILockedAddyVault(lockedStakingVault).getEndingTimestamp(_user);
        if(endingTimestamp > now) {
            //1 veAddy = 4 year lock, boost degrades over time
            uint256 diff = endingTimestamp.sub(now);
            uint256 userAddyLocked = ILockedAddyVault(lockedStakingVault).balanceOf(_user);
            uint256 ratio = ILockedAddyVault(lockedStakingVault).getRatio();
    ...
    function setLockedStakingVault(address _lockedStakingVault) public onlyOwner {
        lockedStakingVault = _lockedStakingVault;
    }
