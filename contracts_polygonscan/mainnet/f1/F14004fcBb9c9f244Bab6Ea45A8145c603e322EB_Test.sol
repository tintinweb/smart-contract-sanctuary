...
contract Test is Ownable, PolygonArbitrageBot {
    ...
    function performSwapIfValuable(address exchangeA, address exchangeB, address tokenA, address tokenB, uint256 _tokenAAmount) external
    {
        _polygonArbitrageBotPerformSwapIfValuable(exchangeA, exchangeB, tokenA, tokenB, _tokenAAmount);
    }

    function getBestOptionForProgramm(address _topLevelToken, uint256 _amount) view external returns(address exchangeA, address exchangeB, address tokenA, address tokenB, uint256 tokenAAmount)
    {
        PolygonArbitrageOption memory option = _polygonArbitrageBotGetBestOption(_topLevelToken, _amount);

        if (option.isValid == false)
        {
            return (option.aToB.exchange, option.bToA.exchange, option.aToB.tokenIn, option.bToA.tokenOut, 0);
        }

        return (option.aToB.exchange, option.bToA.exchange, option.aToB.tokenIn, option.aToB.tokenOut, option.aToB.tokenInAmount);
    }

    function getBestOption(address _topLevelToken, uint256 _amount) view external returns(string memory)
    {
        string memory result = "Options\n\n";

        PolygonArbitrageOption memory option = _polygonArbitrageBotGetBestOption(_topLevelToken, _amount);

        return string(abi.encodePacked(result, _createOptionString(option)));
    }
...
contract PolygonArbitrageBot is PolygonTradingHub {
    ...
    function _polygonArbitrageBotPerformSwapIfValuable(address exchangeA, address exchangeB, address tokenA, address tokenB, uint256 _tokenAAmount) internal
    {
        uint256 initialBalanceA = PolygonTokenTools.getBalance(tokenA);
        uint256 initialBalanceB = PolygonTokenTools.getBalance(tokenB);

        _polygonTradingHubSwapToken(exchangeA, tokenA, tokenB, _tokenAAmount);

        uint256 newBalanceB = PolygonTokenTools.getBalance(tokenB);

        _polygonTradingHubSwapToken(exchangeB, tokenB, tokenA, newBalanceB - initialBalanceB);

        uint256 newBalanceA = PolygonTokenTools.getBalance(tokenA);
    ...
    function _polygonArbitrageBotGetBestOption(address _topLevelToken, uint256 _amount) internal view returns(PolygonArbitrageOption memory)
    {
        ...
        for (uint256 i = 0; i < PolygonTokens.tokens.length; i++)
        {
            ...
            if (fiToken == _topLevelToken)
            {
                continue;
            }

            PolygonArbitrageOption memory fiOption = _polygonArbitrageBotFindOption(_topLevelToken, fiToken, _amount);
    ...
    //     for (uint256 i = 0; i < allAbitrageIntentsLength; i++)
    //     {
    //         PolygonAbitrageIntent memory fiIntent = allAbitrageIntents[i];
...
contract LibraryContract {
    ...
    constructor()
    {
        ...
        uniswapV2Exchanges.push(0xf38a7A7Ac2D745E2204c13F824c00139DF831FFf);
        uniswapV2ExchangesNames[0xf38a7A7Ac2D745E2204c13F824c00139DF831FFf] = "Elk";

        uniswapV2Exchanges.push(0x3a1D87f206D12415f5b0A33E786967680AAb4f6d);
        uniswapV2ExchangesNames[0x3a1D87f206D12415f5b0A33E786967680AAb4f6d] = "Wault Finance";
