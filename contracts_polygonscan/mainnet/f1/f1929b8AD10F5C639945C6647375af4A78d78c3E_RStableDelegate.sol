 ...
 * @author IronFinance
 */
contract RStable is RErc20, RStableInterface {
    /**
    ...
    // ========= OPERATING FUCNTIONS ========
    function _setMaxTotalSupply(uint256 newMaxTotalSupply) external returns (uint256) {
        if (msg.sender != admin) {
            return fail(Error.UNAUTHORIZED, FailureInfo.SET_MAX_TOTAL_SUPPLY_OWNER_CHECK);
        }
...
/**
 * @title Iron's RStableDelegate Contract
 * @notice RTokens which wrap an IRON underlying and are delegated to
 * @author IronFinance
 */
contract RStableDelegate is RStable, RDelegateInterface {
    /**
...
contract RStableInterface is RErc20Interface {
    ...
    function _setMaxTotalSupply(uint256 newMaxTotalSupply) external returns (uint);
}
