...
contract Vault is Ownable, Pausable, DividendToken {
	...
	function _syncDist() internal {
		...
		if (distribution != address(0)) {
			...
			uint256 totalStaked = IDistribution(distribution).balanceOf(
				msg.sender
			);

			// if total staked is bigger, unstake
			if (totalStaked > totalSupplied) {
				IDistribution(distribution).withdraw(
					msg.sender,
					totalStaked.sub(totalSupplied)
				);
			}

			// if total supplied is bigger, stake
			if (totalSupplied > totalStaked) {
				IDistribution(distribution).stake(
					msg.sender,
					totalSupplied.sub(totalStaked)
				);
			}
		}
	}
