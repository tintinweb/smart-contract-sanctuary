...
// BLOBJeX 2021- 1000 NFTs on POLYGON- WHAT WILL YOU CREATE? :-) 
...
contract BLOBjeX is ERC721Enumerable, Ownable {
  using Strings for uint256;
...
for(uint i = 0; i < _mintAmount; i++) {
            uint mintIndex = uint(keccak256(abi.encodePacked(block.timestamp, block.difficulty, msg.sender))) % 990;
            mintIndex = mintIndex + 10;
            if (totalSupply() < maxMintAmount) {
