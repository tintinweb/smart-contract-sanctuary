...
contract Referral {
    ...
    constructor(address _owner, address _meeb) public {
        meeb = IERC20(_meeb);
        owner = _owner;
    ...
    function reward(address addr, uint256 amount) public inGame {
        ...
        // meeb.safeTransferFrom(msg.sender, address(this), amount);
        if (addr != address(0x0)) {
            address ref = refOf(addr);
            if (ref != address(0x0)) {
                uint amountForRef = amount / 20;
                rewards[ref] += amountForRef;
                rewards[addr] += (amount - amountForRef);
            }
            else {
                rewards[addr] += amount;
            }
