...
contract QuickswapLogic is Helpers {
	using SafeMath for uint256;
	using UniversalERC20 for IERC20;
	 ...
	 * @param amount amount of fromTokens to swap
	 * @param getValue read value of tokenAmt from memory contract
	 * @param setValue set value of tokens swapped in memory contract
	 */
	function swap(
		address[] memory path,
		uint256 amount,
		...
		uint256 realAmt = getValue ? getUint() : amount;
	 ...
	 * @param getValue read value of tokenAmt from memory contract
	 * @param setValue set value of LP tokens received in memory contract
	 */
	function addLiquidity(
		IERC20 tokenA,
		IERC20 tokenB,
		...
		uint256 realAmtA =
			amtA == type(uint256).max
				? tokenA.universalBalanceOf(address(this))
				: amtA;

		// Value of tokenB is read from memory if getValue is true
		uint256 realAmtB =
			amtB == type(uint256).max
	 ...
	 * @param getValue read value from memory contract
	 * @param setValue set value of amount tokenB received in memory contract
	 */
	function removeLiquidity(
		IERC20 tokenA,
		IERC20 tokenB,
		...
		uint256 realAmt = getValue ? getUint() : amtPoolTokens;
