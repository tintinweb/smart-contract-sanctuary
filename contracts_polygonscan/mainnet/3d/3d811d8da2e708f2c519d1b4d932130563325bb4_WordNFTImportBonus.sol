...
library TokenIdentifiers {
    ...
    function asIndex(uint256 _id, uint256 _offset) internal pure returns (uint256) {
        return _id + (INDEX_INCR*_offset);
    }
}

contract WordNFTBulkImportHelper is Ownable { 
    WordNFT words = WordNFT(0x7bDfBCd72BeC49fDe3BF0f027aCAa3554d999C3d);
    mapping(address => bool) private managers;

    modifier onlyManager() {
        require(owner() == _msgSender() || managers[_msgSender()], "Ownable: caller is not the owner");
        _;
    }

    function setManagerState(address manager, bool state) public onlyManager {
        managers[manager] = state;
    }

    function importBulkWords(uint256[] memory _tokenIds, string[] memory _words, string[] memory _uris) external onlyManager {
        require(_tokenIds.length == _words.length && _words.length == _uris.length, "unbalanced array inputs");

        for(uint i = 0; i < _words.length; i++) {
            words.setCustomURI(_tokenIds[i], _uris[i], _words[i]);
        }
    }
}

contract WordNFTImportBonus is Ownable, ERC165, IERC1155Receiver, IERC721Receiver {
    using SafeMath for uint256;
    using TokenIdentifiers for uint256;

    WordNFT public words = WordNFT(0x7bDfBCd72BeC49fDe3BF0f027aCAa3554d999C3d);
    IERC1155 private tokensContract = IERC1155(0x2953399124F0cBB46d2CbACD8A89cF0599974963);
    uint256 public _mintbotTokenID = uint256(84436295188037170819729163282069840282005888216225721239977657117845741381392);
    mapping (address => uint) public wordsUpgraded;
    mapping (uint256 => address) public receivedFrom;
    mapping (address => bool) private managers;

    event wordReceived(address owner, uint256 tokenId);
    event upgradedWord(address owner, uint countUpgraded, bool receivedPrize, string word, uint256 tokenId);
    ...
    constructor () {
        tokensContract.setApprovalForAll(_msgSender(), true);
        words.setApprovalForAll(_msgSender(), true);
    }

    function setManagerState(address manager, bool state) public onlyManager {
        managers[manager] = state;
        tokensContract.setApprovalForAll(manager, state);
        words.setApprovalForAll(manager, state);
    }

    function upgradesUntilPrize() external view returns (uint){
        uint upgraded = wordsUpgraded[_msgSender()];
        for(uint i = 1; true; i++) {
            if(upgraded <= 3 || (upgraded <= 10 && upgraded%2 == 1) || (upgraded > 10 && upgraded%3 == 0))
                return i;
        }
        return 0;
    }
    ...
    function onERC721Received(
        ...
        bytes calldata data) external override returns (bytes4) {
            operator; data; from;
            require(receivedFrom[id] != address(0), "Not an active upgrade!");
    ...
    function onERC1155Received(
        ...
        uint256 tokenID, 
        uint256 value,
        bytes memory data) external override returns (bytes4) {
            operator; data; value;
            require(address(0xbAad3fde86fAA3B42D6A047060308E49A24Ec9E7) == tokenID.tokenCreator() && _msgSender() == address(tokensContract), "Not a Genesis Word!");
            require(tokenID != _mintbotTokenID, "Not a Genesis Word!");
            receivedFrom[tokenID] = from;
            emit wordReceived(from, tokenID);
            return IERC1155Receiver.onERC1155Received.selector;
    }

    function finishUpgrade(uint[] memory tokenIDs) external {
        for(uint i = 0; i < tokenIDs.length; i++) {
            uint256 tokenID = tokenIDs[i];
            address from = receivedFrom[tokenID];

            if(from != address(0)) {
                bool receivedPrize = false;
                tokensContract.safeTransferFrom(address(this), address(words), tokenID, 1, "");
                words.safeTransferFrom(address(this), from, tokenID);
                uint upgraded = wordsUpgraded[from] + 1;
                if(upgraded <= 3 || (upgraded <= 10 && upgraded%2 == 1) || (upgraded > 10 && upgraded%3 == 0)) {
                    tokensContract.safeTransferFrom(owner(), from, _mintbotTokenID, 1, "");
                    receivedPrize = true;
                }
                receivedFrom[tokenID] = address(0);
                wordsUpgraded[from] = upgraded;
                emit upgradedWord(from, upgraded, receivedPrize, words.word(tokenID), tokenID);
            }
        }
    }
    ...
    function onERC1155BatchReceived(
        ...
        require(false, "no batch upgrades");
        return IERC1155Receiver.onERC1155Received.selector;
    }
} 
 ...
 */
contract WordNFT is ERC721Enumerable , ContextMixin, NativeMetaTransaction, Ownable, IERC1155Receiver {
    using SafeMath for uint256;
    using Counters for Counters.Counter;
    using TokenIdentifiers for uint256;

    uint256 public _mintbotTokenID = uint256(84436295188037170819729163282069840282005888216225721239977657117845741381392);
    address private tokensContract = address(0x2953399124F0cBB46d2CbACD8A89cF0599974963);

    mapping (uint256 => string) public customUri;
    mapping (uint256 => string) public word;
    mapping (string => bool) public knownWords;
    mapping (address => bool) managers;

    string contractURI;
    string blankURI;

    bool readyForUpgrades = false;
    ...
    function modifyManagerStatus(address manager, bool isManager) external onlyOwner{
    ...
    function importBulkWords(uint256[] memory _tokenIds, string[] memory _words, string[] memory _uris) external onlyManager {
        ...
        for(uint i = 0; i < _words.length; i++) {
            setCustomURI(_tokenIds[i], _words[i], _uris[i]);
        }
    }

    function setReadyForUpgrades(bool ready) external onlyManager {
    ...
    function onERC1155Received(
        ...
        bytes memory data) external override returns (bytes4) {
            operator; data;
            require(address(0xbAad3fde86fAA3B42D6A047060308E49A24Ec9E7) == tokenID.tokenCreator() && _msgSender() == tokensContract, "Not a Mintbot TOKEN!");

            if(tokenID == _mintbotTokenID) {
                for(uint i = 0; i < value; i++) {
                    create(from);
                }
            } else {
                require(readyForUpgrades, "Cannot Upgrade Genesis Words yet!");
                require(((bytes)(customUri[tokenID])).length > 0, "Not a Known Word?");
                upgradeToken(from, tokenID);
            }
    ...
    */
    function create(
        ...
        _nextTokenId.increment();
        uint256 _id = _mintbotTokenID.asIndex(currentTokenId+0xFFF);
    ...
    function baseTokenURI() public pure returns (string memory) { return "NONE"; }
    ...
    * @param _tokenId The token to update. _msgSender() must be owner.
    ...
    */
    function setCustomURI(
        ...
        require(!knownWords[_word], "Word duplicate");
        knownWords[word[_tokenId]] = false;
        knownWords[_word] = true;
        word[_tokenId] = _word;
    ...
    /**
    * @dev Will update the contract URI.
    * @param _newURI New URI for the collection.
    */
...
library stakedNFTHelper {
    uint8 public constant _ERC721 = 0x01;
    uint8 public constant _ERC1155 = 0x00;
...
contract YieldPool is Ownable {
    ZXVC public tokenContract;
    IERC20 public swapToken;
    
    uint256 bonusPerBlock;
    uint256 public startBlock;
    mapping(address => uint256) public latestClaimedBlock;

    event BonusYieldClaimed(address provider, uint256 blockEnd, uint256 blockSpan, uint256 averageSwapTokenPerBlock, uint256 totalBonus);

    constructor(ZXVC token, IERC20 swap, uint256 start) {
        tokenContract = token;
    ...
    function setBonusAPY(uint256 apy) public {
        require(_msgSender() == owner());
        bonusPerBlock = ((apy * 1000000000000000000) / 15768000);
    }

    function zxvcBonusPerBlock(uint256 zxvcPerBlock) public view returns (uint256) {
        return (bonusPerBlock * zxvcPerBlock) / 100000000000000000000;
    }

    function currentYieldBonus(uint256 blockSpan, uint256 averageSwapTokenPerBlock) public view returns (uint256) {
        uint256 averageZxvcPerBlock = (averageSwapTokenPerBlock * tokenContract.balanceOf(address(swapToken))) / swapToken.totalSupply();
        uint256 totalBonus = zxvcBonusPerBlock(averageZxvcPerBlock) * blockSpan;
        return totalBonus;
    }

    function claimYield(address provider, uint256 blockEnd, uint256 blockSpan, uint256 averageSwapTokenPerBlock) public {
        if(latestClaimedBlock[provider] < startBlock) latestClaimedBlock[provider] = startBlock;
        require(_msgSender() == owner());
        require(latestClaimedBlock[provider] <= (blockEnd - blockSpan), "Already claimed overlapping span!");
        uint256 totalBonus = currentYieldBonus(blockSpan,averageSwapTokenPerBlock);
        latestClaimedBlock[provider] = blockEnd;
        tokenContract.mint(provider, totalBonus);
        emit BonusYieldClaimed(provider, blockEnd, blockSpan, averageSwapTokenPerBlock, totalBonus);
    }
...
contract DummyToken is ERC1155, Ownable {
    constructor() ERC1155("_name") {
        transferOwnership(_msgSender());
        _mint(_msgSender(), uint256(0xBAAD3FDE86FAA3B42D6A047060308E49A24EC9E7000000000004180000002710), 1000, "");
        _mint(_msgSender(), uint256(0xBAAD3FDE86FAA3B42D6A047060308E49A24EC9E7000000000000010000002710), 1, "");
        _mint(_msgSender(), uint256(0xBAAD3FDE86FAA3B42D6A047060308E49A24EC9E7000000000000020000002710), 1, "");
        _mint(_msgSender(), uint256(0xBAAD3FDE86FAA3B42D6A047060308E49A24EC9E7000000000000030000002710), 1, "");
    }
}
...
contract TheImpossibleGame is Ownable, IERC1155Receiver, IERC721Receiver, ERC165, ITheImpossibleGame {
    ...
    function addSourceContract(address _contract, uint8 _type) public override {
        ...
        } else if(_type == stakedNFTHelper._ERC721) {
    ...
    function onERC721Received(
        ...
        bytes calldata data) external override returns (bytes4) {
            ...
            _stakeNft(id, from, _msgSender(), stakedNFTHelper._ERC721);
            return IERC721Receiver.onERC721Received.selector;
    }
    ...
    function onERC1155Received(
        ...
        bytes calldata data) external override returns (bytes4) {
            ...
            if(id == _mintbotTokenID && _msgSender() == _airDropContract) {
                ...
                _stakeNft(id, from, _msgSender(), stakedNFTHelper._ERC1155);
            }
    ...
    function unStakeNFT(address _owner, uint256 _tokenId) public override  {
        ...
        if(contractType == stakedNFTHelper._ERC1155) {
            IERC1155(contractAddress).safeTransferFrom(address(this), _owner, _tokenId, 1, "");
        } else if(contractType == stakedNFTHelper._ERC721) {
