    ...
    NativeMetaTransaction
{
 ...
 function initialize(
    ...
    (_stakingToken,_startTime,_endTime,_rewardTokens,_rewardAmounts) = abi.decode(data,(address,uint,uint,address[],uint[]));
        ...
        uint rewardTokenLength=_rewardTokens.length;
        uint rewardAmountlength=_rewardAmounts.length;
        require(rewardTokenLength==rewardAmountlength,"Reward data mismatch");
        for (uint256 i = 0; i < rewardTokens.length; i++) {
            rewardsPerTokenMap[_rewardTokens[i]] = rewardPerToken(
                rewardTokens[i]
            );
            tokenRewardRate[_rewardTokens[i]] = _rewardAmounts[i].div(rewardsDuration);
            rewardLastUpdatedTime[_rewardTokens[i]] = lastTimeRewardApplicable();
        }
 ...
 function _authorizeUpgrade(address newImplementation) internal override onlyOwner {}
    ...
    function getTokenRewardRate(address token)
        public
        view
        returns (uint256){
        if(block.timestamp<startTime){
