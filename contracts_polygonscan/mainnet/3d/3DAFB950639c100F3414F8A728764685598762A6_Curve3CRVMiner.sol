    ...
    */
    ...
    function getTokenStaked() public view returns (uint256 totalTokenStaked) {
        ...
        totalTokenStaked = CURVE.calc_withdraw_one_coin(
            balance_LP,
            uint128(uint8(getCurveIndex(address(CURVE), enterToken)))
        );
    }
    ...
    function getPendingBonus() public view returns (uint256 totalValue) {
        uint256 pendingCRV = CURVE_GAUGE.claimable_reward_write(
            address(this),
            address(CRV)
        );
        ...
        uint256 pendingWMATIC = CURVE_GAUGE.claimable_reward_write(
            address(this),
            address(WMATIC)
        );
