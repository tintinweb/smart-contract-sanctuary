...
contract GrowToken is ERC20, AccessControl {
    ...
    constructor(address admin) ERC20("Ploygon GROW Token", "polyGROW") {
...
contract GrowRewarder is AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    address public constant UNIv2_LIKE_ROUTER = 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff; // Quick swap router
    ...
    uint256 public blockRewardStartBlock;

    /// @notice grow reward pre block
    uint256 public blockRewardGrowPreBlock;
    ...
    function swap(address tokenA, address tokenB, uint256 amount, address receiver) private returns (uint256) {
        ...
        approveToken(tokenA, UNIv2_LIKE_ROUTER, amount);
        IUniv2RouterLike(UNIv2_LIKE_ROUTER).swapExactTokensForTokens(
            amount,
            uint256(0),
            path,
            receiver,
    ...
    function swapGrowToMinter(address token, uint256 amount) private returns (uint256) {
        return swap(token, address(GROW), amount, address(this));
    }
    ...
    function addVault(
        ...
        vault.blockRewardLastRewardBlock = lastRewardBlock;
    ...
    function blockRewardUpdateGrowPreBlock(uint256 amount) external onlyRole(CONFIGURATOR_ROLE) {
        blockRewardGrowPreBlock = amount;
    }

    function blockRewardUpdateStartBlock(uint256 blockNumber) external onlyRole(CONFIGURATOR_ROLE) {
    ...
    function blockRewardUpdateRewards(address strategyAddress) private {
        ...
        uint256 lastRewardBlock = strategies[strategyAddress].blockRewardLastRewardBlock;
    ...
    function calculatePendingBlockRewards(address strategyAddress, address userAddress) internal view returns (uint256) {
        ...
        if (totalShares > 0 && blockRewardTotalAllocPoint > 0 && blockRewardGrowPreBlock > 0) {
            uint256 blockRewardGrow = block.number.sub(strategy.blockRewardLastRewardBlock).mul(blockRewardGrowPreBlock);
    ...
    function calculatePendingRewards(address strategyAddress, address userAddress) external view returns (uint256) {
        ...
        if (strategy.blockRewardLastRewardBlock < block.number && strategy.blockRewardAllocPoint > 0) {
    ...
    function getVaultReward() external onlyVault {
        ...
        uint256 lastRewardBlock = vaults[vaultAddress].blockRewardLastRewardBlock;

        uint256 currentBlock = block.number;

        if (currentBlock <= lastRewardBlock) return;

        if (blockRewardTotalAllocPoint == 0 || blockRewardGrowPreBlock == 0) {
            vaults[vaultAddress].blockRewardLastRewardBlock = currentBlock;
            return;
        }
...
contract Zap is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function zapAndDepositTo(address fromToken, uint amount, address poolAddress, uint minReceive) external payable receiveToken(fromToken, amount) {
        ...
        if (fromToken != wantToken) {
            ...
            if (tokenRouter[wantToken] == wantToken) {
                ...
                wantTokenAmount = _zapTokenToLP(fromToken, amount, wantToken, minReceive);
            }
        }
    ...
    function zapTokenToLP(address fromToken, uint amount, address lpToken) external payable receiveToken(fromToken, amount) returns (uint) {
        require(tokenRouter[fromToken] == fromToken, "fromToken invalid");

        return _zapTokenToLP(fromToken, amount, lpToken);
    }

    function zapTokenToLP(address fromToken, uint amount, address lpToken, uint minLPReceive) external payable receiveToken(fromToken, amount) returns (uint) {
        require(tokenRouter[fromToken] == fromToken, "fromToken invalid");

        return _zapTokenToLP(fromToken, amount, lpToken, minLPReceive);
    }
    ...
    function _swap(address fromToken, address wantToken, uint amount, address receiver, uint minTokenReceive) private returns (uint) {
        ...
        uint[] memory amounts = IUniv2LikeRouter02(pairInfo.ROUTER).swapExactTokensForTokens(amount, minTokenReceive, path, receiver, block.timestamp);
        return amounts[amounts.length - 1];
    }

    function _zapTokenToLP(address fromToken, uint amount, address lpToken) private returns (uint liquidity) {
        return _zapTokenToLP(fromToken, amount, lpToken, 0);
    }

    function _zapTokenToLP(address fromToken, uint amount, address lpToken, uint minLPReceive) private returns (uint liquidity) {
        require(tokenRouter[fromToken] == fromToken, "fromToken invalid");
    ...
    function _zapOutLPtoToken(address lpToken, uint amount, address toToken, address receiver) private {
        ...
        if (token0 == toToken || token1 == toToken) {
            ...
            // swap otherAmount to toToken
            uint otherAmount = _swap(otherToken, toToken, sellAmount, address(this));

            uint tokenAmount = otherToken == token0
                ? amount0 + otherAmount
                : amount1 + otherAmount;

            IERC20(toToken).safeTransfer(receiver, tokenAmount);
        } else {
