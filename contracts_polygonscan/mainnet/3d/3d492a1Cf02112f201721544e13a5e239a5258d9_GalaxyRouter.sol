...
import './interfaces/IGalaxyRouter02.sol';
import './libraries/GalaxyLibrary.sol';
import './libraries/SafeMath.sol';
import './interfaces/IERC20.sol';
import './interfaces/IWETH.sol';

contract GalaxyRouter is IGalaxyRouter02 {
    ...
    modifier ensure(uint deadline) {
        require(deadline >= block.timestamp, 'GalaxyRouter: EXPIRED');
        _;
    }
    ...
    function _addLiquidity(
        ...
        if (reserveA == 0 && reserveB == 0) {
            ...
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, 'GalaxyRouter: INSUFFICIENT_B_AMOUNT');
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint amountAOptimal = GalaxyLibrary.quote(amountBDesired, reserveB, reserveA);
                assert(amountAOptimal <= amountADesired);
                require(amountAOptimal >= amountAMin, 'GalaxyRouter: INSUFFICIENT_A_AMOUNT');
    ...
    function removeLiquidity(
        ...
        require(amountA >= amountAMin, 'GalaxyRouter: INSUFFICIENT_A_AMOUNT');
        require(amountB >= amountBMin, 'GalaxyRouter: INSUFFICIENT_B_AMOUNT');
    }
    ...
    function swapExactTokensForTokens(
        ...
        require(amounts[amounts.length - 1] >= amountOutMin, 'GalaxyRouter: INSUFFICIENT_OUTPUT_AMOUNT');
    ...
    function swapTokensForExactTokens(
        ...
        require(amounts[0] <= amountInMax, 'GalaxyRouter: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, GalaxyLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        );
        _swap(amounts, path, to);
    ...
    {
        require(path[0] == WETH, 'GalaxyRouter: INVALID_PATH');
    ...
    function swapTokensForExactETH(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)
        ...
        returns (uint[] memory amounts)
    {
        require(path[path.length - 1] == WETH, 'GalaxyRouter: INVALID_PATH');
        amounts = GalaxyLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= amountInMax, 'GalaxyRouter: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, GalaxyLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        ...
        returns (uint[] memory amounts)
    {
        require(path[0] == WETH, 'GalaxyRouter: INVALID_PATH');
        amounts = GalaxyLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= msg.value, 'GalaxyRouter: EXCESSIVE_INPUT_AMOUNT');
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(GalaxyLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
        // refund dust eth, if any
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            amountOutput = GalaxyLibrary.getAmountOut(amountInput, reserveInput, reserveOutput);
            }
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        require(
            ...
            'GalaxyRouter: INSUFFICIENT_OUTPUT_AMOUNT'
        );
    }
        ...
        ensure(deadline)
    {
        ...
        require(amountOut >= amountOutMin, 'GalaxyRouter: INSUFFICIENT_OUTPUT_AMOUNT');
    ...
    {
        return GalaxyLibrary.getAmountOut(amountIn, reserveIn, reserveOut);
    }
    ...
    {
        return GalaxyLibrary.getAmountIn(amountOut, reserveIn, reserveOut);
    }
...
import './IGalaxyRouter01.sol';

interface IGalaxyRouter02 is IGalaxyRouter01 {
    function removeLiquidityETHSupportingFeeOnTransferTokens(
        address token,
        uint liquidity,
        uint amountTokenMin,
...
library GalaxyLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'86776f0afa151085f2aaca4e0f26f323d8b12afd74f3c467c537bc053887713f' // init code hash
            ))));
    }
