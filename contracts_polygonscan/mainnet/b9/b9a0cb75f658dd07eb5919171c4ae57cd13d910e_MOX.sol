...
interface IMOX is IERC721, IERC721Enumerable {   
    function getType(uint256 _id) external view returns (uint32);
	function getBirthday(uint256 _id) external view returns (uint32);
...
contract MOX is Ownable, ERC721, ERC721Enumerable {
    ...
    function mintNFT(uint32 _type) public payable {
		...
		require(getPrice(_type) == msg.value, "MATIC value sent is not correct");
        
		uint _mintIndex = totalSupply();
        _safeMint(msg.sender, _mintIndex);
		cardArray.push(CARD(_type, uint32(block.timestamp), 0, 0, getInitialQuality(_type), 0));  
     ...
     *
     */ 
    function createNewType(uint32 _initialPrice, uint32 _initialQuality) public onlyOwner {
    ...
    /**
     * @dev Sets the ask for a NFT, callable only by the owner
     *
     */ 
