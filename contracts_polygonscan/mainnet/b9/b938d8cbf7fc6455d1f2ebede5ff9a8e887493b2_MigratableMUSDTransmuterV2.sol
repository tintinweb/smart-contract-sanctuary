...
// File contracts/interfaces/IMUSDVaultAdapterV2WithIndirection.sol

pragma solidity ^0.6.12;

/// Interface for all Vault Adapter implementations.
interface IMUSDVaultAdapterV2WithIndirection {
...
// File contracts/libraries/alchemist/MUSDVaultV2WithIndirection.sol
...
/// @title MUSDVaultV2WithIndirection
///
/// @dev A library which provides the Vault data struct and associated functions.
library MUSDVaultV2WithIndirection {
  using MUSDVaultV2WithIndirection for Data;
  using MUSDVaultV2WithIndirection for List;
  using SafeMath for uint256;

  struct Data {
    IMUSDVaultAdapterV2WithIndirection adapter;
...
// File contracts/MigratableMUSDTransmuterV2.sol
 ...
 */
contract MigratableMUSDTransmuterV2 is Context {
    using SafeMath for uint256;
    using SafeERC20 for IERC20Burnable;
    using Address for address;
    using MUSDVaultV2WithIndirection for MUSDVaultV2WithIndirection.Data;
    using MUSDVaultV2WithIndirection for MUSDVaultV2WithIndirection.List;
    ...
    /// deposits and withdraws. MUSDVaultV2WithIndirection before the last element are considered inactive and are expected to be cleared.
    MUSDVaultV2WithIndirection.List private _vaults;
    ...
    event ActiveVaultUpdated(
        IMUSDVaultAdapterV2WithIndirection indexed adapter
    );
    ...
    constructor(address _waToken, address _token, address _governance) public {
        require(_governance != ZERO_ADDRESS, "MigratableMUSDTransmuterV2: 0 gov");
    ...
    modifier onlyWhitelisted() {
        require(whiteList[msg.sender], "MigratableMUSDTransmuterV2: !whitelisted");
        _;
    }
    ...
    ///
    modifier onlyGov() {
        require(msg.sender == governance, "MigratableMUSDTransmuterV2: !governance");
        _;
    }
    ...
    function claim() public {
        ...
        require(realisedTokens[sender] > 0, "MigratableMUSDTransmuterV2: no realisedToken balance for sender");
    ...
    function unstake(uint256 amount) public updateAccount(msg.sender) {
        ...
        address sender = msg.sender;
        require(depositedWaTokens[sender] >= amount, "MigratableMUSDTransmuterV2: unstake amount exceeds deposited amount");
    ...
    function stake(uint256 amount)
        ...
        checkIfNewUser()
    {
        require(!pause, "MigratableMUSDTransmuterV2: emergency pause enabled");
    ...
    function transmute() public runPhasedDistribution() updateAccount(msg.sender) {
        ...
        require(pendingz > 0, "MigratableMUSDTransmuterV2: need to have pending in bucket");
        ...
        checkIfNewUser()
    {
        ...
        require(
            ...
            "MigratableMUSDTransmuterV2: !overflow"
        );
    ...
    function setPendingGovernance(address _pendingGovernance) external onlyGov() {
        require(_pendingGovernance != ZERO_ADDRESS, "MigratableMUSDTransmuterV2: 0 gov");
    ...
    function acceptGovernance() external  {
        require(msg.sender == pendingGovernance, "MigratableMUSDTransmuterV2: !pendingGovernance");
    ...
    function setActiveVault(IMUSDVaultAdapterV2WithIndirection _adapter) external onlyGov() {
        require(_adapter != IMUSDVaultAdapterV2WithIndirection(ZERO_ADDRESS), "MigratableMUSDTransmuterV2: active vault address cannot be 0x0.");
        require(address(_adapter.token()) == token, "MigratableMUSDTransmuterV2.vault: token mismatch.");

        _vaults.push(MUSDVaultV2WithIndirection.Data({
            adapter: _adapter,
            totalDeposited: 0
        }));
    ...
    function recallAllFundsFromVault(uint256 _vaultId) external {
        require(pause && (msg.sender == governance || msg.sender == sentinel), "MigratableMUSDTransmuterV2: not paused, or not governance or sentinel");
    ...
    function _recallAllFundsFromVault(uint256 _vaultId) internal {
        MUSDVaultV2WithIndirection.Data storage _vault = _vaults.get(_vaultId);
    ...
    function _plantOrRecallExcessFunds() internal {
        ...
        if (bal > plantableThreshold.add(marginVal)) {
            ...
            MUSDVaultV2WithIndirection.Data storage _activeVault = _vaults.last();
    ...
    function setSentinel(address _sentinel) external onlyGov() {
        require(_sentinel != ZERO_ADDRESS, "MigratableMUSDTransmuterV2: sentinel address cannot be 0x0.");
        sentinel = _sentinel;
    ...
    function setPause(bool _pause) external {
        require(msg.sender == governance || msg.sender == sentinel, "MigratableMUSDTransmuterV2: !(gov || sentinel)");
        pause = _pause;
    ...
    function setRewards(address _rewards) external onlyGov() {
        ...
        require(_rewards != ZERO_ADDRESS, "MigratableMUSDTransmuterV2: rewards address cannot be 0x0.");
    ...
    function migrateFunds(address migrateTo) external onlyGov() {
        require(migrateTo != address(0), "MigratableMUSDTransmuterV2: cannot migrate to 0x0");
        require(pause, "MigratableMUSDTransmuterV2: set pause first");
    ...
    function getVaultAdapter(uint256 _vaultId) external view returns (IMUSDVaultAdapterV2WithIndirection) {
        MUSDVaultV2WithIndirection.Data storage _vault = _vaults.get(_vaultId);
