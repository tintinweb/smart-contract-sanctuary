...
contract OneRingVault is ERC20Upgradeable, OwnableUpgradeable {
    ...
    function _doHardWork(
        ...
        _invest(_amount, _token, _strategy);
    ...
    function doHardWork(
        ...
        _doHardWork(_amount, _token, _strategy);
    }

    function _doHardWorkAll(address _strategy) internal {
        for (uint256 i = 0; i < underlyings.length; i++) {
            uint256 _amount = IERC20(underlyings[i]).balanceOf(address(this));
            _invest(_amount, underlyings[i], _strategy);
        }
        IStrategy(_strategy).doHardWork();
    }

    function doHardWorkAll(address _strategy) external onlyOwner {
        _doHardWorkAll(_strategy);
    }
    ...
    function _invest(
        ...
        if (_amount > 0) {
            IERC20(_token).safeTransfer(address(_strategy), _amount);
            emit Invest(_amount, _token, _strategy);
        }
    }

    function totalUnderlyingBalance() public view returns (uint256 totalBal) {
        for (uint256 i = 0; i < underlyings.length; i++) {
            if (underlyingEnabled[underlyings[i]]) {
                totalBal =
                    totalBal +
                    IERC20(underlyings[i])
                        .balanceOf(address(this))
                        .mul(uint256(10)**uint256(decimals()))
                        .div(
                            uint256(10) **
                                uint256(ERC20(underlyings[i]).decimals())
                        );
            }
        }
    }

    function balanceInVault() public view returns (uint256 balance) {
        uint256 _fee = totalUnderlyingBalance().mul(depositFee).div(
            depositFeeMax
        );
        balance = totalUnderlyingBalance().sub(_fee);
    }

    function balanceWithInvested() public view returns (uint256 balance) {
        balance = balanceInVault();
        for (uint256 i = 0; i < strategies.length; i++) {
            if (strategies[i] != address(0)) {
                balance = balance.add(
                    IStrategy(strategies[i]).investedBalanceInUSD()
                );
            }
        }
    }
    ...
    function _getShareAmountToMint(uint256 _amount, address _underlying)
        internal
        returns (uint256 _toMint)
    {
        ...
        uint256 _vaultUnit = uint256(10)**uint256(decimals());
        uint256 _underlyingUnit = uint256(10) **
            uint256(ERC20(_underlying).decimals());

        _toMint = _amount.mul(_vaultUnit).div(_underlyingUnit);

        _toMint = _toMint.mul(underlyingUnit).div(_sharePrice);
    }
    ...
    function _deposit(
        ...
        uint256 _newLiquidity = IStrategy(activeStrategy).assetToUnderlying(
            _underlying
        );
        _doHardWorkAll(activeStrategy);
        uint256 _newLiquidityInUSD = IStrategy(activeStrategy)
            .getUSDBalanceFromUnderlyingBalance(_newLiquidity);
    ...
    function _withdrawAll(address _underlying) internal {
        for (uint256 i = 0; i < strategies.length; i++) {
            IStrategy(strategies[i]).withdrawAllToVault(_underlying);
        }
    }

    function withdrawAllFromOneStrategy(address _underlying, address _strategy)
        external
        onlyOwner
    {
        require(strategyEnabled[_strategy], "strategy must be enabled");
        _withdrawAllFromOneStrategy(_underlying, _strategy);
    }

    function _withdrawAllFromOneStrategy(address _underlying, address _strategy)
        internal
    {
        IStrategy(_strategy).withdrawAllToVault(_underlying);
    }
    ...
    function disableStrategy(address _strategy) public onlyOwner {
        require(_strategy != address(0), "strategy must be defined");
