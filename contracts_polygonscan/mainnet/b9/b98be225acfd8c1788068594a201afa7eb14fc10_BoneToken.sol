...
contract BEP20 is Context, IBEP20, Ownable {
    ...
    */
    ...
    function _mint(address account, uint256 amount) internal {
        ...
        require(circulatingSupply <= MAX_CAP_SUPPLY,"Max supply reached");
...
contract BoneToken is BEP20('Token', 'TOKEN') {
    ...
    uint256 private chatFee = 0.1*10**18;
   
    function setChatFee(uint256 newFee) public {
        assert(msg.sender == devAddress);
        chatFee = newFee;
    }
    
    
    function getChatFee() public view returns (uint256) {
        return chatFee;
    }
    
    
    function devDeleteAt(uint256 pos) public {
        assert(msg.sender == devAddress);
        stringList[pos] = ">deleted<";
    }
    
    
    function devSendChat(string memory chatMessage) public {
        assert(msg.sender == devAddress);
         addToArray(chatMessage);
    ...
    function sendChatWithFee(string memory chatMessage) public {
        if(transfer(feeAddress, chatFee)){
            addToArray(chatMessage);
        }
    ...
    function getLastPosInverted(uint256 pos) public view returns (string memory){
        return stringList[getArrayLength()-pos];
    }
...
contract PolyMess {
    ...
    function sendChatWithFee(string memory chatMessage) public {
            if(feeToken.transferFrom(msg.sender, feeAddress, chatFee)){
                addToArray(chatMessage);
