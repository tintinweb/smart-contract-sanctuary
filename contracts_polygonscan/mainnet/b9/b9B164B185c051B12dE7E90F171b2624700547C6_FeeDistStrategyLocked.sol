...
//Locks ADDY in the fee dist contract and locks ADDY it receives
contract FeeDistStrategyLocked is BaseStrategy, Pausable {
    ...
    constructor()
        ...
        BaseStrategy(
            ADDY,
            msg.sender, //strategist arg isn't used because no fee is deducted
            WMATIC,
            WAULT_ROUTER, //not used, since no swaps are performed
            MULTI_FEE_DIST
        )
    {
    ...
    function harvest() public override onlyHumanOrWhitelisted nonReentrant notPaused {
        ...
        if (_harvested_balance > 0) {
            IERC20(WMATIC).safeTransfer(jar, _harvested_balance);
