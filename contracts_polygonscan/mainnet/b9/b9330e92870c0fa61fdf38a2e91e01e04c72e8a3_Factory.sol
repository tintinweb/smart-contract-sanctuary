    ...
    NativeMetaTransaction
{
    ...
    mapping(address => address) farmOwner;
    ...
    function authorizeDeployer(address _deployer, uint256 _limit) external onlyOwner {
    ...
    function deauthorizeDeployer(address _deployer) external onlyOwner {
        deployer[_deployer].whitelisted = false;
        emit DeAuthorizeDeployer(_deployer, block.timestamp);
    }

    //Adds new strategie
    function addStrategies(string memory _id, address _strategies) external onlyOwner {
        strategy[_id] = _strategies;
        emit AddedStrategy(_id, _strategies);
    }

    //Deploys farm
    function deployFarm(string memory _id, bytes memory _data)
        external
        ...
        returns (address)
    {
        ...
        bytes memory payload = abi.encodeWithSelector(0xcce2df03, _data, _msgSender());
        ...
        deployedFarmDetails memory details = deployedFarmDetails(strategyClone, _data, _id);
    ...
    function renewFarms(address _farm, bytes memory _data) external  nonReentrant onlyDeployer(_msgSender()) {
        ...
        (_rewardTokens, _rewardAmount) = dataDecoder(_data);
         ...
         bytes memory payload = abi.encodeWithSelector(0x6d4d6e37, _data);
    ...
    function rewardTransfer(address[] memory _rewardTokens, uint256[] memory _rewardAmount, address _farm) internal {
    ...
    function rescueFunds(
        ...
        bytes memory payload = abi.encodeWithSelector(0x1ff9b6f2, _tokenAddress, _receiver);
        (bool success, bytes memory returnData) = address(_farm).call(payload);
    ...
    function dataDecoder(bytes memory _data) internal pure returns (address[] memory, uint256[] memory) {
        (
            ...
            address[] memory _rewardTokens,
            uint256[] memory _rewardAmount
        ) = abi.decode(_data, (address, uint256, uint256, address[], uint256[]));
    ...
    event RenewFarms(address farm,bytes data,uint time);
}
