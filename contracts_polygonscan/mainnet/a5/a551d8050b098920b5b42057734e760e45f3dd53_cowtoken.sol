...
interface IERC20 {
    /**
     * @dev Returns the amount of cowtokens in existence.
     */
    function totalSupply() external view returns (uint256);

    /**
     * @dev Returns the amount of cowtokens owned by `account`.
     */
    function balanceOf(address account) external view returns (uint256);

    /**
     * @dev Moves `amount` cowtokens from the caller's account to `recipient`.
     *
    ...
    /**
     * @dev Returns the remaining number of cowtokens that `spender` will be
    ...
    /**
     * @dev Sets `amount` as the allowance of `spender` over the caller's cowtokens.
     *
     * Returns a boolean value indicating whether the operation succeeded.
     *
    ...
    /**
     * @dev Moves `amount` cowtokens from `sender` to `recipient` using the
    ...
    /**
     * @dev Emitted when `value` cowtokens are moved from one account (`from`) to
     * another (`to`).
     *
...
contract cowtoken is Context, IERC20, Ownable {
    ...
    string private _name = "Polycow Finance";
    ...
    address public ownerAddres = address(0x14B0F96D394959296e7b2987a1a906635b502431);
    uint256 private _previousBurnFee = _burnFee;


    uint256 public _maxTxAmount = 5 * 10 **18;

    event MincowtokensBeforeSwapUpdated(uint256 mincowtokensBeforeSwap);
    event SwapAndLiquifyEnabledUpdated(bool enabled);
    event SwapAndLiquify(
        uint256 cowtokensSwapped,
        uint256 ethReceived,
        uint256 cowtokensIntoLiqudity
    );
    ...
    function balanceOf(address account) public view override returns (uint256) {
        ...
        return cowtokenFromReflection(_rOwned[account]);
    }
    ...
    function _cowtokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        if(!takeFee)
            removeAllFee();
    ...
    function reflectionFromcowtoken(uint256 tAmount, bool deductTransferFee) public view returns(uint256) {
    ...
    function cowtokenFromReflection(uint256 rAmount) public view returns(uint256) {
    ...
    function excludeFromReward(address account) public onlyOwner() {
        ...
        if(_rOwned[account] > 0) {
            _tOwned[account] = cowtokenFromReflection(_rOwned[account]);
        }
    ...
    function _transfer(
        ...
        _cowtokenTransfer(from,to,amount,takeFee);
    }
