...
contract LowbLottery {
  ...
  uint public awardAmount;
  uint public waitingBlock;
  ...
  function setAwardAmount(uint awardAmount_) public {
    require(msg.sender == owner, "You are not admin");
    awardAmount = awardAmount_;
  }
  
  function setWaitingBlock(uint waitingBlock_) public {
    require(msg.sender == owner, "You are not admin");
    waitingBlock = waitingBlock_;
  }
  ...
  function moveToNextRound() public {
    ...
    require(block.number >= currentRound.block + waitingBlock, "One round per 4 hours!");
    nextRound.pool = currentRound.pool + awardAmount;
