...
contract Exchange is Ownable, ExchangeDomainV1 {
    using SafeMath for uint;
    ...
    function exchange(
        ...
        verifyOpenAndModifyOrderState(orderNumber);
        transfer(asset, paying, to);
        emitBuy(asset, orderNumber, paying);
    }


    function transfer() public returns (AssetType aaa) {
        return  AssetType.ERC721;
          
    }

    function transfer(Asset memory asset, uint value, address to) internal {
        if (asset.assetType == AssetType.ERC721) {
            require(value == 1, "value should be 1 for ERC-721");
            IERC721(asset.token).safeTransferFrom(IERC721(asset.token).ownerOf(asset.tokenId), to, asset.tokenId);
        } else {
            IERC1155(asset.token).safeTransferFrom(IERC721(asset.token).ownerOf(asset.tokenId), to, asset.tokenId, value, "");
        }
    }

    function verifyOpenAndModifyOrderState(uint256 orderNumber) internal {
        bool iscompleted = completed[orderNumber];
        require(!iscompleted, "not enough stock of order for buying");
        completed[orderNumber] = iscompleted;
    }

    function emitBuy(Asset memory asset, uint amount, uint256 orderNumber) internal {
        emit Buy(asset.token,
            asset.tokenId,
            asset.assetType,
            msg.sender,
            amount,
