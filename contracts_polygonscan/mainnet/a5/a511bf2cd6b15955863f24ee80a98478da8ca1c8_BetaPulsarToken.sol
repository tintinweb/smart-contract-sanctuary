...
contract BetaPulsarToken is ERC20('BetaPulsarToken', 'BPUL') {
    ...
    uint256 public constant MAXIMUM_TRANSFER_PVP_COMISSION = 2000;
    uint256 public maxTransfer = 500;
    ...
    event TransferPvpComissionUpdated(uint256 previousPvpComission, uint256 newPvpComission);
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        bool antiWhaleDeactivate = (sender == developer || recipient == developer || sender == master || recipient == master);
    ...
    function setMasterAddress(address _master) public onlyDeveloper{ master = _master; }
    
    function setMaxTransfer(uint256 _amount) public onlyDeveloper{
        maxTransfer = _amount;
    ...
    function updateTransferTaxRate(uint256 _transferTaxRate) public onlyDeveloper {
        require(_transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "BPUL::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
        emit TransferTaxRateUpdated(transferTaxRate, _transferTaxRate);
        transferTaxRate = _transferTaxRate;
        totalTransferTaxRate = transferTaxRate + transferPvpComission;
    }
    
    function updateTransferPvpComission(uint256 _transferPvpComission) public onlyDeveloper {
        require(_transferPvpComission <= MAXIMUM_TRANSFER_PVP_COMISSION, "BPUL::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
        emit TransferPvpComissionUpdated(transferPvpComission, _transferPvpComission);
        transferPvpComission = _transferPvpComission;
        totalTransferTaxRate = transferTaxRate + transferPvpComission;
    }
    
    function setDeveloperAddress(address _newDeveloper) public onlyDeveloper{
        require(_newDeveloper != address(0), "Dev address can't be zero.");
        emit SetNewDeveloper(developer, _newDeveloper);
        developer = _newDeveloper;
    }
    
    function setPvpAddress(address _newPvp) public onlyDeveloper{
        emit SetPvpAddress(developer, pvp, _newPvp);
        pvp = _newPvp;
    }
    
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount){
        if (recipient == BURN_ADDRESS || transferTaxRate == 0 || sender == developer || recipient == developer) {
            super._transfer(sender, recipient, amount);
        } else {
            // default tax is 7% of every transfer
            uint256 taxAmount = amount.mul(transferTaxRate).div(10000);
            // default comission is 1% of every transfer
            uint256 pvpTaxAmount = amount.mul(transferPvpComission).div(10000);

            // default 92% of transfer sent to recipient
            uint256 sendAmount = amount.sub(taxAmount).sub(pvpTaxAmount);
            require(amount == sendAmount + taxAmount + pvpTaxAmount, "BPUL::transfer: Tax value invalid");

            super._transfer(sender, BURN_ADDRESS, taxAmount);
            super._transfer(sender, pvp, pvpTaxAmount);
