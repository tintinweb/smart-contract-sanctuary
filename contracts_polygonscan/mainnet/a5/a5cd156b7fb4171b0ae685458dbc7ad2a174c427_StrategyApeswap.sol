...
abstract contract StrategyFeesBase is Ownable, ReentrancyGuard, Pausable {
    ...
    address public vaultChefAddress = 0x3510EE0a98De1d74fCf8217c4161F9049a8E749D;
...
abstract contract StrategyBase is StrategyFeesBase {
    ...
    uint256 public compoundCycle = uint256(-1); 
...
// File: Vault\StrategyApeswap.sol
...
contract StrategyApeswap is StrategyLPBase {
    ...
    function stake(uint256 _wantAmount) internal virtual override{
        IApeswap(apeChef).deposit(pid, _wantAmount, address(this));
    }

    function harvest() internal virtual override{
        IApeswap(apeChef).harvest(pid, address(this));
        
        //sushi gives both wmatic and earned. Convert the wmatic to token0 & token1. This will be converted to LP by earnedToWant() function
        ...
        if (wmaticAmt > 0) {
            ...
            wmaticAmt = IERC20(wmaticAddress).balanceOf(address(this));
    ...
    function unstake(uint256 _amount) internal virtual override{
        IApeswap(apeChef).withdrawAndHarvest(pid, _amount, address(this));
    }    
    
    function vaultSharesTotal() public virtual override view returns (uint256) {
        (uint256 balance,) = IApeswap(apeChef).userInfo(pid, address(this));
        return balance;
    }
    ...
    function _resetAllowances() internal virtual override{
        IERC20(wantAddress).safeApprove(apeChef, uint256(0));
    ...
    function emergencyWithdraw() internal virtual override onlyGov {
        IApeswap(apeChef).emergencyWithdraw(pid, address(this));
    }
}