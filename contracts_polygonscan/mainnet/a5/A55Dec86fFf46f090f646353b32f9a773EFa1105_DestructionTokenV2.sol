...
contract DestructionTokenV2 is AntiBotBlacklist {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override 
             antiBot(sender, recipient, amount) 
             blacklistChecker(sender, recipient) {}
...
abstract contract AntiBotBlacklist is ERC20, Ownable {
    ...
    */
    ...
    modifier antiBot(address sender,address recipient,uint256 amount) { 
        ...
        if (antiBotWorking){
            // check  if sender|recipient send a tx amount is within the allowed limits
            if (_isNotExcludedFromAntiBot(sender))
                require(amount <= maxUserTransferAmount());
            if (_isNotExcludedFromAntiBot(recipient))
                require(amount <= maxUserTransferAmount());

            // autoban user if is holding more than is allowed.
            if (
                balanceOf(sender).add(amount) > maxUserHoldAmount()
                && _isNotExcludedFromAntiBot(sender) // exclude core operators and pairs from botlist  
                ) 
                {
    ...
    modifier blacklistChecker(address _sender, address _recipient) {
        require(!blacklistCheck(_sender));
        require(!blacklistCheck(_recipient));
    ...
    function transferOperator(address newOperator) external onlyOperator {
        require(newOperator != address(0), "Destruction::transferOperator:Error");
     ...
     */
    function removeBotAddress(address botAddress) external onlyOwner{
     ...
     */
    function removeBotAddressBatch(address[] memory _addresses) external onlyOwner{
        require(_addresses.length>0,"addresses is empty");
        for(uint i=0;i<_addresses.length;i++){
    ...
    function _addBotAddressToBlackList(address _botAddress,uint256 _expirationTime) internal{
        require(_botAddress != owner(),"the botAddress cannot be owner");
        require(blacklist[_botAddress]==0,"the botAddress already exist");
    ...
    function _removeBotAddressToBlackList(address _botAddress) internal{
        require(blacklist[_botAddress]>0,"botAddress is not in blacklist");
