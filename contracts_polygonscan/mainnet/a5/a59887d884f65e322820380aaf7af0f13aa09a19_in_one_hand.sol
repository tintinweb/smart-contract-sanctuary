...
contract in_one_hand is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  mapping(address => uint256) public pplwhohavenft;
 ...
 function setLimitWhitelisted(uint256 _state) public onlyOwner {
  ...
  function updateReward(address from, address to) external {
    ...
    function getPendingReward(address user) public view returns(uint256) {
        return  pplwhohavenft[msg.sender] * BASE_RATE * (block.timestamp - (lastUpdate[user] >= START ? lastUpdate[user] : START)) / 60;
    }
    ...
    function getcounttoken(address userAddress) public view returns (uint) {
     return pplwhohavenft[userAddress];
}
}