    ...
    */
    ...
    function addRoutes(ChainData[] calldata _routes) external onlyOwner {
        for (uint256 i = 0; i < _routes.length; i++) {
            minAndMaxForChains[_routes[i].chainId].chainId = _routes[i].chainId;
            minAndMaxForChains[_routes[i].chainId].isEnabled = _routes[i]
                .isEnabled;
            minAndMaxForChains[_routes[i].chainId].minAmount = _routes[i]
                .minAmount;
            minAndMaxForChains[_routes[i].chainId].maxAmount = _routes[i]
                .maxAmount;
        }
    }
