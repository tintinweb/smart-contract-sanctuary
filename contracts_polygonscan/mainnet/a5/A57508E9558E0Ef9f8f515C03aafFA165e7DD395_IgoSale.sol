...
contract IgoSale is Ownable {
    ...
    function addTokens(uint256 _amount) external onlyOwner {
        ...
        require(token.balanceOf(address(this)) >= hardCap + getTokensForLiquidity(), 'addTokens:INSUFFICIENT_BALANCE');
        tokensAdded = true;
    ...
    function removeTokens() external onlyOwner {
        ...
        uint256 amount = hardCap + getTokensForLiquidity();
    ...
    function withdrawFunds() external onlyOwner {
        ...
        uint256 unsoldTokens = token.balanceOf(address(this)) - tokensSold + tokensClaimed;
        uint256 transferTokens = liquidityAdded ? unsoldTokens : unsoldTokens - getTokensForLiquidity();
        if (transferTokens > 0) {
    ...
    function withdrawLiquidity() external onlyOwner {
        ...
        IERC20 lpPair = IERC20(IUniswapV2Factory(router.factory()).getPair(address(this), router.WETH()));
    ...
    function addLiquidity() external {
        ...
        uint256 tokensForLiquidity = getTokensForLiquidity();
    ...
    function getTokensForLiquidity() private view returns (uint256) {
        return (hardCap * liquidityPct) / 10000;
    }
