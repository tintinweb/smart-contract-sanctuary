...
contract ifFactory {
    ...
    string public name = "Uranium Impermax ";
    string public prefix = "ui";
...
contract ifToken is ERC20, ERC20Detailed {
    ...
    uint256 public maxBP = 10000;

    uint256 public totalDeposit;
    uint256 public totalWithdraw;
    uint256 public totalGain;
    uint256 public totalChargedFee;
    ...
    function accruePerformanceFee() public {
        ...
        if(performanceFee>0) {
            ...
            // total gains = deposit minus withdrawal;

            if(totalTokenLocked.add(totalWithdraw) < totalDeposit){
                return;
            }

            uint256 totalGain = totalTokenLocked.add(totalChargedFee).add(totalWithdraw).sub(totalDeposit);

            if(totalGain>0 && performanceFee>0) {
                // charged today = (  total gains * (PerFee)%  ) - charged fee;
                uint256 chargedToday = totalGain.mul(performanceFee).div(maxBP).sub(totalChargedFee);

                if(chargedToday>0){
                    totalChargedFee = totalChargedFee.add(chargedToday);

                    uint256 maTokenAmount = chargedToday.mul(totalSupply()).div(totalTokenLocked);

                    // mint maTokenAmount;
                    _mint(treasury, maTokenAmount);

                    emit AccruePerformanceFee(chargedToday); 
                }
            }                     
        }
    ...
    function pull() internal {
        ...
        for (uint8 i=1; i<=count; i++) {
            ...
            uint256 totalBal = brrwbleMai.totalBalance().mul(1e18).div(brrwbleMai.exchangeRate());

            if(bal > 1 && totalBal > 1){
                bytes memory nothing;
                if(bal > totalBal){
                    router.redeem(brrwbleMai_addr, totalBal, address(this), deadline, nothing);
                } else {
                    router.redeem(brrwbleMai_addr, bal, address(this), deadline, nothing);
                }                
            }
            //redeem(address poolToken, uint256 tokens, address to, uint256 deadline, bytes permitData) 
        }
    ...
    function enter(uint256 _amount) public {
        ...
        uint256 totalShares = totalSupply(); // Gets the amount of maToken in existence
        console.log("before accruePerformanceFee");
        accruePerformanceFee();
        console.log("after accruePerformanceFee");
    ...
    function leave(uint256 _share) public {
        ...
        if(_share>0){
            ...
            totalWithdraw = totalWithdraw.add(maTokenAmount);
