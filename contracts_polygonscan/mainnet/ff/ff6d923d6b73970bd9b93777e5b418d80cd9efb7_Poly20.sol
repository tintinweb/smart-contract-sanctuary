...
contract Poly20 {
	...
	uint256 public START_TIME_11GMT = 1629465811;
	...
	constructor( ) public {
	    ...
	    startUNIX = START_TIME_11GMT;
		
		devAddress = address(0x07100ae53fB0764113Ab192280077DC74b097002);                       
		insuranceContract = address(0x29d879A1665DcEb0E2330679fE124DBC4110276F);               

        plans.push(Plan(14, 200));
        
        refhand = address(0x07100ae53fB0764113Ab192280077DC74b097002);
        // set default referr
	...
	function invest(address referrer, uint8 reffPlan, uint8 plan) public  payable {
		...
		user.latestWithdrawal = 0;
        
		if (user.referrer == address(0) && reffPlan == 0) {
	...
	function withdraw() public {
		...
		if (referralBonus > 0) {
			user.bonus = user.bonus/2;
			totalAmount = totalAmount.add(referralBonus);
		}
        
        require(totalAmount > 0, "User has no dividends");
        require(user.latestWithdrawal + 1 days < block.timestamp, "User have to wait 24h from latest withdrawal");
        withdrawable = totalAmount/2;
