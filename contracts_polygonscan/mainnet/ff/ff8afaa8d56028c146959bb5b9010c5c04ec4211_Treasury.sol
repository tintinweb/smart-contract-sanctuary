...
/**
 *Submitted for verification at polygonscan.com on 2021-06-19
*/
...
contract Treasury is ContractGuard, ITreasury, OwnableUpgradeSafe {
    ...
    function nextEpochLength() public view override returns (uint256 _length) {
        ...
        _length = (dollarPrice >= dollarPriceCeiling) ? EXPANSION_EPOCH_PERIOD : CONTRACTION_EPOCH_PERIOD;
    }

    // oracle
    function getDollarPrice() public view override returns (uint256 dollarPrice) {
    ...
    function initialize(
        ...
        maxSupplyExpansionPercent = 250; // Upto 2.5% supply for expansion
        ...
        allocateSeigniorageSalary = 0.1 ether; // 0.1 SDO salary for calling allocateSeigniorage()
    ...
    function setExtraFunds(
        ...
        address _lpProviderBoardroom,
        uint256 _lpProviderBoardroomSharedPercent,
    ...
    function transferOperatorAndOwnership(address _newOwner) external onlyOwner {
        IBond(bond).transferOperator(_newOwner);
        IBond(bond).transferOwnership(_newOwner);
    }
}