...
abstract contract BaseStrategy is Ownable {
    ...
    function getHarvestable() public virtual view returns (uint256);
    ...
    function withdraw(uint256 _amount) external {
        ...
        // Since this is a strat, leave some room for rounding error
        uint256 _balancePost = IERC20(want).balanceOf(address(this));
        if (_balancePost < _amount) {
            _amount = _balancePost;
        }
...
abstract contract BaseStrategyMasterChef is BaseStrategy {
    ...
    function balanceOfPool() public override view returns (uint256) {
        return IMasterChef(rewards).userInfo(address(this));
        //return IMasterChef(rewards).balanceOf(poolId, address(this));
    }

    function getHarvestable() public override view returns (uint256) {
...
abstract contract StrategyFarmSingle is BaseStrategyMasterChef {
    ...
    address public constant rewardToken = 0xAa9654BECca45B5BDFA5ac646c939C62b527D394; //DINO token of DinoSwap
    ...
    address public constant dino = rewardToken;
    address public constant usdc = 0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174;

    address[] public dino_usdc_path;

    // Other constants
    uint16 public constant underlyingPoolId = 0;
    ...
    {
        dino_usdc_path = new address[](2);
        dino_usdc_path[0] = dino;
        dino_usdc_path[1] = usdc;
    }

    // **** State Mutations ****

    function set_fee(uint256 NewFee) external onlyOwner() {
        require(NewFee <= 100, "Cannot be more than 100 bps");
        keepFXS = NewFee;
    }
    ...
    function harvest() public override {
        ...
        if (_harvestBalance > harvestCutoff){
            ...
            if (performanceFee>0) {
                _swapUniswapWithPath(dino_usdc_path, performanceFee);
...
contract StrategySingle is StrategyFarmSingle {
    // Token addresses
    address public DINO_MASTER_CHEF = 0x52e7b0C6fB33D3d404b07006b006c8A8D6049C55; //DINO Jurassic pool
    address public Want_Token = 0xAa9654BECca45B5BDFA5ac646c939C62b527D394; //DINO token
