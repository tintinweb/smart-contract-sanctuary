...
contract BeliFarm is Ownable, ReentrancyGuard {
    ...
    function pendingLocked(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
    ...
    function payConvertedProfit(uint256 _pid, address _userAddress) internal {
        ...
        if (user.shares > 0) {
            uint256 totalShare = IStrategy(pool.strat).sharesTotal();
            uint256 accBELIProfit = IStrategy(pool.strat).accBELIProfit();
            uint256 amount = user.shares.div(totalShare).mul(accBELIProfit).div(1e12).sub(user.rewardProfitDebt);
            if (amount > 0) {
                IStrategy(pool.strat).transferConvertedProfit(_userAddress, amount);
            }
    ...
    function payOrLockupPendingBELI(uint256 _pid, bool _isWithdrawal) internal {
        ...
        if (IStrategy(pool.strat).profitToBeliRate() > 0) {
    ...
    function withdrawEarly(uint256 _pid, uint256 _wantAmt) public nonReentrant {
        ...
        if (_wantAmt > 0) {
            uint256 sharesRemoved =
                IStrategy(poolInfo[_pid].strat).withdrawEarly(msg.sender, _wantAmt);
    ...
    function emergencyWithdrawEarly(uint256 _pid) public nonReentrant {
        ...
        uint256 preBalance = pool.want.balanceOf(address(this));
        IStrategy(poolInfo[_pid].strat).withdrawEarly(msg.sender, amount);
        uint256 postBalance = pool.want.balanceOf(address(this));
        amount = postBalance.sub(preBalance);
