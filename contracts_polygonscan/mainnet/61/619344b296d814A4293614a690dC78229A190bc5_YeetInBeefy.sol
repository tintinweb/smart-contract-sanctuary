...
contract YeetInBeefy is YeetIn {
    ...
    function PerformYeetIn(
        ...
        uint256 beforeBalanceOfWant = IERC20(_pairAddress).balanceOf(address(this));
        ...
        uint256 afterBalanceOfWant = IERC20(_pairAddress).balanceOf(address(this));
        uint256 beforeBalanceOfMoo = IERC20(beefyVault).balanceOf(address(this));

        _approveToken(_pairAddress, beefyVault);
        BeefyVaultV6(beefyVault).deposit(afterBalanceOfWant.sub(beforeBalanceOfWant));
        uint256 afterBalanceOfMoo = IERC20(beefyVault).balanceOf(address(this));

        uint256 amountBought = beforeBalanceOfMoo.sub(afterBalanceOfMoo);
