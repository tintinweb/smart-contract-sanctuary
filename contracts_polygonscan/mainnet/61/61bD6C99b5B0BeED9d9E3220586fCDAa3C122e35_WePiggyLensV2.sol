...
contract WePiggyLensV2 {
    ...
    constructor(ComptrollerLensInterface _comptroller, PiggyDistributionInterface _distribution,
        string  memory _pNativeToken, string  memory _nativeToken) public {
    ...
    function allForAccount(address payable account) external view returns (AccountLimits memory, PTokenBalances[] memory, PTokenMetadata[] memory, InterestRateModel[] memory){
        ...
        PTokenBalances[] memory balances = pTokenBalancesAll(accountLimits.markets, account);
        PTokenMetadata[] memory metaData = pTokenMetadataAll(accountLimits.markets);
        InterestRateModel[] memory rateModels = getInterestRateModels(accountLimits.markets);
