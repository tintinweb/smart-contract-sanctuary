...
// File: contracts/Vault/IBEP20.sol
...
// File: contracts/Vault/SafeMath.sol
...
// File: contracts/Vault/Address.sol
...
// File: contracts/Vault/SafeBEP20.sol
...
// File: contracts/Vault/BAIVaultStorage.sol
...
// File: contracts/Vault/BAIVaultErrorReporter.sol

pragma solidity ^0.5.16;

contract BAIVaultErrorReporter {
    enum Error {
        NO_ERROR,
        UNAUTHORIZED
    }
...
// File: contracts/Vault/BAIVaultProxy.sol

pragma solidity ^0.5.16;



contract BAIVaultProxy is BAIVaultAdminStorage, BAIVaultErrorReporter {

    /**
      * @notice Emitted when pendingBAIVaultImplementation is changed
      */
    event NewPendingImplementation(address oldPendingImplementation, address newPendingImplementation);

    /**
      * @notice Emitted when pendingBAIVaultImplementation is accepted, which means BAI Vault implementation is updated
      */
    ...
    function _setPendingImplementation(address newPendingImplementation) public returns (uint) {
        ...
        address oldPendingImplementation = pendingBAIVaultImplementation;

        pendingBAIVaultImplementation = newPendingImplementation;

        emit NewPendingImplementation(oldPendingImplementation, pendingBAIVaultImplementation);

        return uint(Error.NO_ERROR);
    }

    /**
    * @notice Accepts new implementation of BAI Vault. msg.sender must be pendingImplementation
    * @dev Admin function for new implementation to accept it's role as implementation
    * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
    */
    function _acceptImplementation() public returns (uint) {
        // Check caller is pendingImplementation
        if (msg.sender != pendingBAIVaultImplementation) {
        ...
        pendingBAIVaultImplementation = address(0);
...
// File: contracts/Vault/BAIVault.sol
...
contract BAIVault is BAIVaultStorage {
    ...
    /**
     * @notice Deposit BAI to BAIVault for XBID allocation
    ...
    function deposit(uint256 _amount) public nonReentrant {
        ...
        user.rewardDebt = user.amount.mul(accXBIDPerShare).div(1e18);
    ...
    /**
     * @notice Claim XBID from BAIVault
     */
    ...
    function _withdraw(address account, uint256 _amount) internal {
        ...
        updateVault();
        updateAndPayOutPending(account); // Update balances of account this is not withdrawal but claiming XBID farmed
    ...
    /**
     * @notice View function to see pending XBID on frontend
     * @param _user The user to see pending XBID
     */
    function pendingXBID(address _user) public view returns (uint256)
    {
        UserInfo storage user = userInfo[_user];

        return user.amount.mul(accXBIDPerShare).div(1e18).sub(user.rewardDebt);
    }

    /**
     * @notice Update and pay out pending XBID to user
     * @param account The user to pay out
     */
    function updateAndPayOutPending(address account) internal {
        uint256 pending = pendingXBID(account);

        if(pending > 0) {
            safeXBIDTransfer(account, pending);
        }
    }

    /**
     * @notice Safe XBID transfer function, just in case if rounding error causes pool to not have enough XBID
     * @param _to The address that XBID to be transfered
     * @param _amount The amount that XBID to be transfered
     */
    function safeXBIDTransfer(address _to, uint256 _amount) internal {
    ...
    function updateVault() internal {
        ...
        accXBIDPerShare = accXBIDPerShare.add(pendingRewards.mul(1e18).div(vaiBalance));
        pendingRewards = 0;
    }
    ...
    function _become(BAIVaultProxy vaiVaultProxy) public {
    ...
    function setBidaoInfo(address _xvs, address _vai) public onlyAdmin {
        xvs = IBEP20(_xvs);
        vai = IBEP20(_vai);
