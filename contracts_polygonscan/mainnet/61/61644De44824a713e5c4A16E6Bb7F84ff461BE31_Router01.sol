...
contract Router01 is IRouter01, IYampFinanceCallee {
	...
	modifier ensure(uint deadline) {
		require(deadline >= block.timestamp, "YampFinanceRouter: EXPIRED");
		_;
	}

	modifier checkETH(address poolToken) {
		require(WETH == IPoolToken(poolToken).underlying(), "YampFinanceRouter: NOT_WETH");
		_;
	}
	...
	function deleverage(
		...
		require(redeemTokens > 0, "YampFinanceRouter: REDEEM_ZERO");		
	...
	function _removeLiqAndRepay(
		...
		require(amountAMax >= amountAMin, "YampFinanceRouter: INSUFFICIENT_A_AMOUNT");
		require(amountBMax >= amountBMin, "YampFinanceRouter: INSUFFICIENT_B_AMOUNT");
		// repay and refund
	...
	function yampFinanceBorrow(address sender, address borrower, uint borrowAmount, bytes calldata data) external virtual override {
		...
		require(sender == address(this), "YampFinanceRouter: SENDER_NOT_ROUTER");
		require(msg.sender == declaredCaller, "YampFinanceRouter: UNAUTHORIZED_CALLER");
	...
	function yampFinanceRedeem(address sender, uint redeemAmount, bytes calldata data) external virtual override {
		redeemAmount;
		CalleeData memory calleeData = abi.decode(data, (CalleeData));
	...
	function _optimalLiquidity(
		...
		if (amountBOptimal <= amountBDesired) {
			require(amountBOptimal >= amountBMin, "YampFinanceRouter: INSUFFICIENT_B_AMOUNT");
			(amountA, amountB) = (amountADesired, amountBOptimal);
		} else {
			uint amountAOptimal = UniswapV2Library.quote(amountBDesired, reserveB, reserveA);
			assert(amountAOptimal <= amountADesired);
			require(amountAOptimal >= amountAMin, "YampFinanceRouter: INSUFFICIENT_A_AMOUNT");
	...
	function getBorrowable(address uniswapV2Pair, uint8 index) public virtual override view returns (address borrowable) {
		require(index < 2, "YampFinanceRouter: INDEX_TOO_HIGH");
		borrowable = address(uint(keccak256(abi.encodePacked(
			hex"ff",
			bDeployer,
			keccak256(abi.encodePacked(factory, uniswapV2Pair, index)),
			hex"8bb7e04a26a72254344801e402eab46576d1a582d88553dc5d409b7a63628612" // Borrowable bytecode keccak256
		))));
	}
	function getCollateral(address uniswapV2Pair) public virtual override view returns (address collateral) {
		collateral = address(uint(keccak256(abi.encodePacked(
			...
			hex"a783712cde224d030d0b69f67ce358762955155d14c2d17c380c6c406e7db251" // Collateral bytecode keccak256
		))));
	}
