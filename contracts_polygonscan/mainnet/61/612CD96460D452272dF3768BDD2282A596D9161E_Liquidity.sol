...
contract Liquidity is Ownable {
    ...
    constructor(
      ...
      DESTINY = IERC20(_DESTINY);
      _allowance();
    }

  receive() external payable {}

  function withdrawDestinyLiquidity() external onlyOwner {
      USDC.transfer(msg.sender, USDC.balanceOf(address(this)));
      DESTINY.transfer(msg.sender, DESTINY.balanceOf(address(this)));
  }
 ...
 function withdrawAll()  external onlyOwner {
  ...
  function AutoLiquidity() external onlyOwner {
    ...
    uint256 destinyBalance = DESTINY.balanceOf(address(this));
    uint256 halfTokenAmount = destinyBalance / 2;

    if (usdcBalanceAfter > 0 && wmaticBalance > 0 && destinyBalance > 0) {
        // add the liquidity destiny-matic
        ...
        // add the liquidity destiny-usdc
        ROUTER.addLiquidity(
            address(DESTINY),
            address(USDC),
            halfTokenAmount,
  ...
  function _allowance() internal {
    ...
    DESTINY.approve(address(ROUTER), type(uint256).max);
  }
