...
import "../interfaces/ICircleLauncher.sol";
import "../interfaces/IPolyCircleDistributor.sol";
import "../interfaces/IStakingRewards.sol";

interface IStrategy {
    function stakeTo(uint amount, address account) external;
}

contract PolygonCircleDistributor is IPolyCircleDistributor, OwnableUpgradeable {
    ...
    address public circleETHPool;  // our CIRCLE-WETH Pool
    address public stakingRewards;  // CIRCLE Maximizer Pool
    ...
    function initialize(address _launcher) external initializer {
        launcher = _launcher;
        __Ownable_init();
    }
    ...
    function notifyCreate(uint256, address _token) override external onlyLauncher {
        token = _token;
    }

    function notifyEngage(uint256 auctionId, address user, uint256 amount) override external onlyLauncher {
        users.push(user);
        amount = ICircleLauncher(launcher).swapTokenAmount(auctionId, amount);
    ...
    // 150% mint -> 50% LP user staking, 50% LP CirclePool rewards, 50% CIRCLE user staking
    // 15% mint -> deployer staking
    function notifyArchive(uint256, address _token, uint256 amount) override external onlyLauncher {
    ...
    function notifyClaim(uint256 auctionId, address user, uint256 amount) override external onlyLauncher {

    }

    function setCircleETHPool(address _masterChef) external onlyOwner {
        circleETHPool = _masterChef;
    }
    ...
    function setCirclePool(address _circlePool) external onlyOwner {
        circlePool = _circlePool;
    }
    ...
    function distributeTokens(uint index, uint length) external onlyOwner {
        ...
        require(circleETHPool != address(0), 'not set circleETHPool');
        require(stakingRewards != address(0), 'not set stakingRewards');

        IERC20(lpToken).safeApprove(circleETHPool, 0);
        IERC20(lpToken).safeApprove(circleETHPool, totalFlipBalance);
    ...
    function sendToRewards() external onlyOwner {
        ...
        IERC20(flipToken()).safeTransfer(circlePool, lpBalance);
        IStakingRewards(circlePool).notifyRewardAmount(lpBalance);
    }
    ...
    function _distributeFlip(address user, uint share) private {
        ...
        IStrategy(circleETHPool).stakeTo(amount, user);
    }
...
interface ICircleLauncher {
    ...
    struct AuctionInfo {
        ...
        uint tokenSupply;               // amount of the token
        uint tokenRemain;               // remain of the token
        uint capacity;                  // total value of token in ETH or BNB (calculated by CircleLauncher)
...
interface IPolyCircleDistributor {
    /**
