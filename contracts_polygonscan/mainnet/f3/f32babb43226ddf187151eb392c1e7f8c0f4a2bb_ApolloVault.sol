 ...
 */
contract ApolloVault is ERC20, Ownable, ReentrancyGuard {
    ...
    address constant public devAddress = 0x1109c5BB8Abb99Ca3BBeff6E60F5d3794f4e0473;
    ...
    function balance() public view returns (uint) {
        return input().balanceOf(address(this)).add(IStrategy(strategy).balanceOf());
    }
    ...
    function deposit(uint _amount) public nonReentrant {
        ...
        input().safeTransferFrom(msg.sender, devAddress, depositFee);
 ...
 //      require(depositFeeMC <= 2000, "Third party MC has over 20% deposit fees");
    ...
    function withdraw(uint256 _shares) public {
        ...
        uint b = input().balanceOf(address(this));
        if (b < r) {
            uint _withdraw = r.sub(b);
            strategy.withdraw(_withdraw);
            uint _after = input().balanceOf(address(this));
