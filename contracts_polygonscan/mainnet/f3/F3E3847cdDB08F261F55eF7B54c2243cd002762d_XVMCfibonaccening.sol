...
contract XVMCfibonaccening is Ownable {
	...
	*/
	function uniteAgainstTheCommonEnemy(uint256 startTimestamp, uint256 delayInSeconds) external whenReady {
		...
		if(consensusProposal.length == 0) {
			consensusProposal.push(
			    ConsensusVote(0, address(this))
			    );
	...
	function initiateTreasuryTransferProposal(uint256 depositingTokens,  address tokenAddress, address recipient, uint256 amountToSend) external whenReady { 
    	require(
    	    depositingTokens <= IXVMCgovernor(owner()).maximumVoteTokens() && depositingTokens >= IXVMCgovernor(owner()).costToVote(),
    	    "Minimum or maximum cost not met/exceeded"
    	    );
    	
    	IERC20(token).safeTransferFrom(msg.sender, address(this), depositingTokens);
    ...
    function vetoTreasuryTransferProposal(uint256 proposalID) external whenReady {
		...
		IERC20(token).safeTransferFrom(msg.sender, address(this), treasuryProposal[proposalID].valueSacrificedForVote); 
    ...
    */
	function approveTreasuryTransfer(uint256 proposalID) external whenReady {
		...
		require(
		 ...
		 require(
            IacPool(acPool6).totalVotesFor(consensusID) >= IacPool(acPool6).totalShares() * 30 / 100,
				"No consensus in Pool 6"
        );
        require(
            IacPool(acPool5).totalVotesFor(consensusID) >= IacPool(acPool5).totalShares() * 30 / 100,
				"No consensus in Pool 5"
        );
        require(
            IacPool(acPool4).totalVotesFor(consensusID) >= IacPool(acPool4).totalShares() * 30 / 100,
				"No consensus in Pool 4"
        );
		
		
        require(
            IacPool(acPool6).totalVotesFor(consensusID + 1) <= IacPool(acPool6).totalShares() * 5 / 100,
				"No consensus in Pool 6"
        );
        require(
    ...
    function changeGovernor(uint256 proposalID) external whenReady { 
		require(consensusProposal.length > proposalID && proposalID % 2 == 1);
        ...
        require(
            IacPool(acPool6).totalVotesFor(proposalID) >= IacPool(acPool6).totalShares() * 70 / 100,
				"No consensus in Pool 6"
        );
        require(
            IacPool(acPool5).totalVotesFor(proposalID) >= IacPool(acPool5).totalShares() * 70 / 100,
				"No consensus in Pool 5"
        );
        require(
            IacPool(acPool4).totalVotesFor(proposalID) >= IacPool(acPool4).totalShares() * 70 / 100,
				"No consensus in Pool 4"
        );
        require(
            IacPool(acPool3).totalVotesFor(proposalID) >= IacPool(acPool3).totalShares() * 70 / 100,
				"No consensus in Pool 3"
        );
        
        require(
            IacPool(acPool6).totalVotesFor(proposalID + 1) <= IacPool(acPool6).totalShares() * 20 / 100,
				"No consensus in Pool 6"
        );
        require(
            IacPool(acPool5).totalVotesFor(proposalID + 1) <= IacPool(acPool5).totalShares() * 20 / 100,
				"No consensus in Pool 5"
        );
        require(
            IacPool(acPool4).totalVotesFor(proposalID + 1) <= IacPool(acPool4).totalShares() * 20 / 100,
				"No consensus in Pool 4"
        );
        require(
            IacPool(acPool3).totalVotesFor(proposalID + 1) <= IacPool(acPool3).totalShares() * 20 / 100,
				"No consensus in Pool 3"
        );
    ...
    function enforceGovernor(uint256 proposalID) external whenReady {
        ...
        require(IXVMCgovernor(owner()).newGovernorRequestBlock() + 103680 < block.timestamp, "must wait roughly 3days");
