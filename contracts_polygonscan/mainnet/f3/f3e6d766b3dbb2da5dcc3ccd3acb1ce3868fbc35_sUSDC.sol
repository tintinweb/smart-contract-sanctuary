...
contract sUSDC is ERC20, ERC20Detailed {
 ...
 function uninvest() public {
  ...
  function mint(uint256 _amount) external {
      ...
      TransferHelper.safeTransferFrom(address(token1), msg.sender, address(this), _getAmount(address(token1),shares));
      ...
      userInfo[msg.sender].token1 += _getAmount(address(token1),shares);
  ...
  function redeem(uint256 _shares) external {
      ...
      userInfo[msg.sender].token1 -= _amount.mul(_shares).div(total);
  
      token0.safeTransfer(msg.sender, _shares);
      token1.safeTransfer(msg.sender, _amount.mul(_shares).div(total)); 
