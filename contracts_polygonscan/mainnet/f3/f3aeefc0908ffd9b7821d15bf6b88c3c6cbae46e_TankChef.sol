...
contract TankChef is Ownable, ReentrancyGuard {
     ...
     * @param _startBlock can be 0, so the updateRewardPerBlock uses startBlock from constructor
     * to calculate the reward tokens per block
     * @param _amount must be bigger than 0
     */
    function depositRewards(uint256 _amount, uint256 _startBlock) external {
        require(_amount > 0, "Deposit value must be greater than 0.");
        rewardToken.safeTransferFrom(msg.sender, address(this), _amount);
        updateRewardPerBlock(_startBlock);
    }
     ...
     * @param _blockNumber if is zero the function will calculate the rewardPerBlock with the startBlock number
     */
    function updateRewardPerBlock(uint256 _blockNumber) internal {
        uint256 totalLeft = rewardBalance();
        if (_blockNumber > 0) {
            rewardPerBlock = totalLeft.div(endBlock.sub(_blockNumber));
        } else {
    ...
    function updateEndBlock(uint256 _endBlock) external onlyOwner {
        ...
        updateRewardPerBlock(block.number);
    }
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        if (_amount > 0) {
            ...
            _amount = poolInfo.token.balanceOf(address(this)).sub(balanceBefore);
