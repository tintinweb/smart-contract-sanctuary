...
abstract contract Strat is Ownable, ReentrancyGuard, Pausable {
    ...
    bool public isCAKEStaking; // only for staking CAKE using swap's native CAKE staking contract.
    bool public isSameAssetDeposit;
    bool public isAutoComp; // this vault is purely for staking. eg. WMATIC-INKU staking vault.
    ...
    address public inkuFarmAddress;
    address public INKUAddress = 0x1Dd9e9e142f3f84d90aF1a9F2cb617C7e08420a4;
    address public TAKOAddress = 0x6D2a71f4edF10ab1E821B9B373363e1E24E5DF6b;
    ...
    address[] public earnedToINKUPath;
        ...
        returns (uint256)
    {
        ...
        IERC20(wantAddress).safeTransfer(inkuFarmAddress, _wantAmt);
    ...
    function buyBack(uint256 _earnedAmt) internal virtual returns (uint256) {
        ...
        if (earnedAddress == INKUAddress || earnedAddress == TAKOAddress) {
...
contract Polygon_Strat is Strat {
    constructor(
        address _inkVaultAddress,
        ...
        inkuFarmAddress = _inkVaultAddress;
        ...
        uniRouterAddress = _uniRouterAddress;
        earnedToINKUPath = [earnedAddress, wmaticAddress, INKUAddress];
        earnedToToken0Path = [earnedAddress, wmaticAddress, token0Address];
        if (wmaticAddress == token0Address) {
            earnedToToken0Path = [earnedAddress, wmaticAddress];
        }

        earnedToToken1Path = [earnedAddress, wmaticAddress, token1Address];
        if (wmaticAddress == token1Address) {
            earnedToToken1Path = [earnedAddress, wmaticAddress];
        }

        token0ToEarnedPath = [token0Address, wmaticAddress, earnedAddress];
        if (wmaticAddress == token0Address) {
            token0ToEarnedPath = [wmaticAddress, earnedAddress];
        }

        token1ToEarnedPath = [token1Address, wmaticAddress, earnedAddress];
        if (wmaticAddress == token1Address) {
            token1ToEarnedPath = [wmaticAddress, earnedAddress];
        }
        controllerFee = _controllerFee;
        rewardsAddress = _rewardsAddress;
        buyBackRate = _buyBackRate;

        transferOwnership(inkuFarmAddress);
    }
}