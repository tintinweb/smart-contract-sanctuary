...
contract MOX is Ownable, ERC721, ERC721Enumerable {
    ...
    function mintNFT(uint32 _type) public payable {
		...
		cardArray.push(CARD(_type, uint256(block.timestamp), 0, 0, getInitialQuality(_type), 0, _owner));  
    ...
    function mintNFTSentIn(uint256 _initialPrice, uint32 _initialQuality, address firstOwner) public onlyOwner {
        ...
        require(activeIndex[__type] == true, "NFT not available");
		...
		cardArray.push(CARD(__type, uint256(block.timestamp), 0, 0, _initialQuality, 0, firstOwner));  
