...
contract XVMCgovernor {
    ...
    address public immutable consensusContract = 0x2d62eD496aFEbBCaa37B577CCCCacA48F622878c;
    address public immutable fibonacceningContract = 0x2b666D8961bca1cbF0deF00b0930E7b6C899DE63;
    address public immutable raContract = 0x043E6b3755C252E1116CDF7cf4876f5AFbf4a069;
    address public immutable farmContract = 0x7E60cFfafc297B0f1fF5DEB666CE82D74E74A0A3; //for setpool
    address public immutable basicContract = 0x486dff3D1fd53521842Af07ED92aAE92537E4c80; //setCallfee, rolloverbonus
    ...
    address public immutable token1 = 0x0bc03A20208edC7B5941b34585C90D6cF63A2B85;
    address public immutable token2 = 0x7C961C3A06f8a45e4dfE3c010032cDd380cC8DEb;
    address public immutable token3 = 0xb8524135e8525f009266A964202a54396bdd7a5f;
    address public immutable token4 = 0xd1306d0CD898ABde5Bf772844999F3E0fBeC826E;
    address public immutable token5 = 0x075818705E1358B6bab4d7AB317892B3d8987B6A;
    address public immutable token6 = 0x1a1FE3Ca6062e19de61b439856a8ce7CDD3BDE57;
        
    //pool ID in the masterchef for respective Pool address and dummy token
    uint256 public immutable acPool1ID = 28;
    uint256 public immutable acPool2ID = 29;
    uint256 public immutable acPool3ID = 30;
    uint256 public immutable acPool4ID = 31;
    uint256 public immutable acPool5ID = 32;
    uint256 public immutable acPool6ID = 33;
    ...
    constructor(
        ...
        rollBonus[0x9b6ae196A358Ea81c305D8A32018a4F4C90FC207].bonusPoints = 50;
        rollBonus[0x38d2503d751F35c2671cdae6E9011e7Be5CdF174].bonusPoints = 69;
        rollBonus[0x418E16d46c66435E72aC646A7bC2a0c286349C55].bonusPoints = 100;
        rollBonus[0x321521b99Dbb21705259eA3d84a1d83c37C98D0A].bonusPoints = 200;
        rollBonus[0x984981089d06A514AB54Bc3562850aFc75620e26].bonusPoints = 300;
        rollBonus[0xfD08FA4a344D147DCcE4f29D258B9F4ae18e6ee0].bonusPoints = 420;
        // Infinite approve
        IERC20(_token).safeApprove(address(0x9b6ae196A358Ea81c305D8A32018a4F4C90FC207), type(uint256).max); //vaults
        IERC20(_token).safeApprove(address(0x38d2503d751F35c2671cdae6E9011e7Be5CdF174), type(uint256).max);
        IERC20(_token).safeApprove(address(0x418E16d46c66435E72aC646A7bC2a0c286349C55), type(uint256).max);
        IERC20(_token).safeApprove(address(0x321521b99Dbb21705259eA3d84a1d83c37C98D0A), type(uint256).max);
        IERC20(_token).safeApprove(address(0x984981089d06A514AB54Bc3562850aFc75620e26), type(uint256).max);
        IERC20(_token).safeApprove(address(0xfD08FA4a344D147DCcE4f29D258B9F4ae18e6ee0), type(uint256).max);
        IERC20(_token).safeApprove(address(0x2b666D8961bca1cbF0deF00b0930E7b6C899DE63), type(uint256).max); //fib contract
    }


       
    modifier onlyTrustee {
      require(block.timestamp < 1637145462, "Contract is immutable after Nov 17");
      require(msg.sender == 0x9c36BC6b8C107014B6E86536D809b74C6fdB8cE9);
      _;
    }
    
    modifier whenReady() {
      require(block.timestamp > 1637145462, "after 17 nov");
      _;
    }
    ...
    */
    ...
    function stakeRolloverBonus(address _giftTo, address _depositToPool, uint256 _bonusToPay) external {
        ...
        IacPool(_depositToPool).giftDeposit(_bonusToPay, _giftTo);
        
        emit GiveRolloverBonus(_giftTo, _bonusToPay, _depositToPool);
    }
