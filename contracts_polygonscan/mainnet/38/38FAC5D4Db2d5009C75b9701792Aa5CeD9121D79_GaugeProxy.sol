...
contract GaugeProxy {
    ...
    constructor(
        ...
        TOKEN = IERC20(address(new MasterVEXP()));
    ...
    // Adjusts _owner's votes according to latest _owner's vEXP balance
    function poke(address _owner) public {
        ...
        uint256 _weight = vEXP.balanceOf(_owner);
    ...
    function addGauge(address _token) external {
        ...
        gauges[_token] = address(
            new Gauge(_token, address(EXP), address(vEXP))
        );
        _tokens.push(_token);
    }
    ...
    // Deposits mvEXP into MasterChef
    ...
    function collect() public {
        (uint256 _locked, ) = masterchef.userInfo(pid, address(this));
        masterchef.withdraw(pid, _locked);
        deposit();
    }
    ...
    function distribute() external {
        collect();
        uint256 _balance = EXP.balanceOf(address(this));
        if (_balance > 0 && totalWeight > 0) {
            for (uint256 i = 0; i < _tokens.length; i++) {
                ...
                if (_reward > 0) {
                    EXP.safeApprove(_gauge, 0);
                    EXP.safeApprove(_gauge, _reward);
...
contract MasterVEXP is ERC20, Ownable {
  constructor() public ERC20('Master vEXP', 'mvEXP') {
...
contract Gauge is ReentrancyGuard {
  ...
  function derivedBalance(address account) public view returns (uint256) {
    ...
    uint256 _adjusted =
      (_totalSupply.mul(vEXP.balanceOf(account)).div(vEXP.totalSupply()))
        .mul(60)
        .div(100);
  ...
  function getReward() public nonReentrant updateReward(msg.sender) {
    ...
    if (reward > 0) {
      ...
      EXP.safeTransfer(msg.sender, reward);
  ...
  {
    EXP.safeTransferFrom(DISTRIBUTION, address(this), reward);
    ...
    uint256 balance = EXP.balanceOf(address(this));
