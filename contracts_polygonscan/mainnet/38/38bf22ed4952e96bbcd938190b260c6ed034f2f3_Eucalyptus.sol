...
contract Eucalyptus is ERC20, AccessControl, ERC721Holder {
    ...
    constructor(address owner, address _collectionAddress) ERC20("Eucalyptus", "EUC") AccessControl(){
        ...
        coinAmountPerRewardUnit = 100 * 10 ** 18; // 50 ERC20 coins per rewardUnit, may be changed later on
        numberOfBlocksPerRewardUnit = 41442; // 12 hours per reward unit , may be changed later on
        welcomeBonusAmount = 500 * 10 ** 18; // 420 tokens welcome bonus, only paid once per tokenId
    }
    ...
    function stake(uint256 tokenId) public {
        ...
        require(IERC721(collectionAddress).ownerOf(tokenId) == address(this),
            "VIBE: Error while transferring token");
    ...
    function unstake(uint256 tokenId) public {
        ...
        require(IERC721(collectionAddress).ownerOf(tokenId) == _msgSender(),
            "EUC: Error while transferring token");
