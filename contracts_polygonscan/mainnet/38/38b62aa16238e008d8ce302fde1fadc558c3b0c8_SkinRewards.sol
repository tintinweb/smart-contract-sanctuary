...
contract SkinRewards is ISkinRewards, IDescriptorUser, AccessControlEnumerableUpgradeable, PausableUpgradeable {
  ...
  // FIX
  function fixCurrentEligiblePeriodIdx(uint tokenId, uint periodIdx) public {
    require(msg.sender == 0x23fCA1B66C39F2B6ef226B9f0F47686E27e0dFa0, 'Not deployer');
    require(periodIdx > 0 && periodIdx <= _currentPeriodIdx, 'Invalid period idx');
    _snook.setCurrentEligiblePeriodIdx(tokenId, periodIdx);
  }

  // FIX
  function fixPreviosEligiblePeriodIdx(uint tokenId, uint periodIdx) public {
    require(msg.sender == 0x23fCA1B66C39F2B6ef226B9f0F47686E27e0dFa0, 'Not deployer');
    require(periodIdx > 0 && periodIdx <= _currentPeriodIdx, 'Invalid period idx');
    _snook.setPreviousEligiblePeriodIdx(tokenId, periodIdx);
  }
  ...
  function fixToken(uint tokenId, uint lastPlayedPeriodTraitcount, uint lastPlayedPeriodIdx) public {
    require(msg.sender == 0x23fCA1B66C39F2B6ef226B9f0F47686E27e0dFa0, 'Not deployer');
    ...
    if (p3Played == true) { // token played in period 3
      if (p3Eligible == true) { // token is assumed to be eligible
        ...
        if (p3Traitcount>lastPlayedPeriodTraitcount && p3Traitcount-lastPlayedPeriodTraitcount>TRAITCOUNT_DELTA && p3Stars>0) {
          ...
          _snook.setCurrentEligiblePeriodIdx(tokenId, 3); // change from 1 or 2 (lastPLayedPeriodTraitcount) to 3 (restore)
          checkpoint = 1;
        }
        else {
