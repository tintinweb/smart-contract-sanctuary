...
contract LendingLogicKashi is ILendingLogic {
    ...
    function lend(address _underlying,uint256 _amount, address _tokenHolder) external view override returns(address[] memory targets, bytes[] memory data) {
        ...
        uint8 _decimal = IKashiPair(_underlying).decimals();
        
        //uint256 share = (10**(_decimal*2) / exchangeRateView(kaToken)  *  _amount) / (10**_decimal);
        uint256 share = bentoBox.toShare(IERC20(_underlying),_amount,false);
        //uint256 test = exchangeRateView(kaToken);
        //console.log("exchange Rate: ", test);
        ...
        targets[3] = kaToken;
        data[3] =  abi.encodeWithSelector(IKashiPair.addAsset.selector, _tokenHolder, false, share);
    ...
    function unlend(address _wrapped, uint256 _amount,address _tokenHolder) external view override returns(address[] memory targets, bytes[] memory data) {
        ...
        //uint256 underlyingAmount = bentoBox.balanceOf(underlying,_tokenHolder);
