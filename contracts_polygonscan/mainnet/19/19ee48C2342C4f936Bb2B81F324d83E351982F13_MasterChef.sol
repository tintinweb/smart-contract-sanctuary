...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of Sandman
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. Sandman to distribute per block.
        uint256 lastRewardBlock;  // Last block number that Sandman distribution occurs.
        uint256 accSandManPerShare;   // Accumulated Sandman per share, times 1e18. See below.
    ...
    uint256 public sandmanPerBlock = 1 ether;
    ...
    // SandMan referral contract address.
    ...
    event UpdateEmissionRate(address indexed user, uint256 sandmanPerBlock);
    ...
    // View function to see pending Sandman on frontend.
    function pendingSandMan(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 sandmanReward = multiplier.mul(sandmanPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accSandManPerShare = accSandManPerShare.add(sandmanReward.mul(1e18).div(lpSupply));
        }
        return user.amount.mul(accSandManPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 sandmanReward = multiplier.mul(sandmanPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        sandman.mint(devAddress, sandmanReward.div(10));
        sandman.mint(address(this), sandmanReward);
        pool.accSandManPerShare = pool.accSandManPerShare.add(sandmanReward.mul(1e18).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accSandManPerShare).div(1e18).sub(user.rewardDebt);
            if (pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accSandManPerShare).div(1e18);
    ...
    // Safe sandman transfer function, just in case if rounding error causes pool to not have enough SANDMAN.
    ...
    function updateEmissionRate(uint256 _sandmanPerBlock) external onlyOwner {
        massUpdatePools();
        sandmanPerBlock = _sandmanPerBlock;
        emit UpdateEmissionRate(msg.sender, _sandmanPerBlock);
    }
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (address(referral) != address(0) && referralCommissionRate > 0) {
            ...
            if (referrer != address(0) && commissionAmount > 0) {
                sandman.mint(referrer, commissionAmount);
...
contract SandManToken is ERC20("SandMan Token", "SANDMAN"), Ownable {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "SANDMAN::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "SANDMAN::delegateBySig: invalid nonce");
        require(now <= expiry, "SANDMAN::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "SANDMAN::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying SANDMANs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "SANDMAN::_writeCheckpoint: block number exceeds 32 bits");
