...
contract VestingRegistry {
  ...
  event MemberRemoved(address member, address vestingAddress);
  ...
  function addMember (address member, address vestingAddress) public isAdmin {
    Disbursement vcontract = Disbursement(vestingAddress);
  ...
  function removeMember (address member, address vestingAddress) public isAdmin {
    Disbursement vcontract = Disbursement(vestingAddress);
    if (address(vcontract) == address(0x0)) {
    ...
    emit MemberRemoved(member, vestingAddress);
  }
