...
contract TestToken is ERC20("Test Token", "TEST") {
    ...
    constructor(IERC20 _leoToken) public {
    ...
    modifier antiWhale(
        ...
        bool antiWhaleDeactivate = (canBeWhale[sender] ||
            canBeWhale[recipient]);

        if (maxTransferAmount() > 0 && !antiWhaleDeactivate) {
            require(
                amount <= maxTransferAmount(),
                "SCORPIO::antiWhale: Transfer amount exceeds the maxTransferAmount"
            );
        }
        _;
    }
        ...
        onlyDeveloper
    {
        require(
            ...
            "SCORPIO::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate."
        );
    ...
    function _transfer(
        ...
        if (whiteList[sender] || whiteList[recipient] || transferTaxRate == 0) {
            taxAmount = 0;
        } else {
        ...
        //     "SCORPIO::transfer: Tax value invalid"
        // );

        if (taxAmount > 0) {
            swapTokensForLeo(taxAmount);
            // withdrawLeoTokens();
        }
        super._transfer(sender, recipient, sendAmount);
    }

    function swapTokensForLeo(uint256 tokenAmount) private {
    ...
    function withdrawLeoTokens() internal {
        uint256 leoBalance = leoToken.balanceOf(address(this));
        if (leoBalance > 0) {
            leoToken.transfer(developer, leoBalance);
        }
    }
}