...
// inspired from https://etherscan.io/address/0x48dc32eca58106f06b41de514f29780ffa59c279
...
contract OracleMainnet is Governable {
    ...
    //Key tokens are used to find liquidity for any given token on Uni and Sushi.
    ...
    //How much decimals the defined output token has
    uint256 public precisionDecimalsDefinedOutputToken = 6;
    ...
    function getPrice(address token) external view returns (uint256) {
        if (token == definedOutputToken) {
            return (10**precisionDecimalsDefinedOutputToken);
        }
        bool uniSushiLP;
    ...
    function getLargestPool(address token, address[] memory tokenList) public view returns (address, address, bool, bool) {
        ...
        return (uniSushiKeyToken, address(0), isUni, !isUni);
    ...
    function getKeyTokenPrice(address token) internal view returns (uint256) {
        ...
        if (token == definedOutputToken) {
            price = 10**precisionDecimalsDefinedOutputToken;
