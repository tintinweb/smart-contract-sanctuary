    ...
    for (uint i=0; i< PayeeCount; i++) {
      ...
      if (_payTimes[i] + _cyclesInDays[i] * _secondsInDay < block.timestamp) {
        ...
        if (PayeeIds(i) == PayeeIds.SKIN) {
          uint balanceOfSkin = _skill.balanceOf(payee);
          if (balanceOfSkin > 0 && balanceOfSkin <= amount) {
            amount = amount - balanceOfSkin; 
          } else if (balanceOfSkin > amount) { 
            amount = 0;
          } else {} // balance is 0, do no changes to amount
        }
        ...
        if (PayeeIds(i) == PayeeIds.SKIN) {
          ISkinRewards skinRewards = ISkinRewards(_payees[i]);
          skinRewards.startNewPeriod(amount);
        }

        if (PayeeIds(i) == PayeeIds.STAKING) {
          IStakingRewards stakingRewards = IStakingRewards(_payees[i]);
