...
// File: contracts/MasterLocker.sol
...
contract MasterLocker is Ownable {
    address[] public lockers;

    mapping(address => bool) _approved;

    constructor(address unityToken) {
        _approved[0x800C861A7Ba4605C7FE4F650cCab465be516c2A0] = true;
        _approved[unityToken] = true;
    }

    modifier onlyApproved() {
        require(_approved[msg.sender], "Not approved");
        _;
    }

    function addApproved(address addr) external onlyApproved {
        _approved[addr] = true;
    }
    ...
    function totalSupply() external view returns (uint256) {
        ...
        for (uint256 i = 0; i < lockers.length; i++) {
            total = total + ILocker(lockers[i]).totalSupply();
        }
        return total;
    }
    ...
    function balanceOf(address account) external view returns (uint256) {
        uint256 total = 0;
        for (uint256 i = 0; i < lockers.length; i++) {
            total = total + ILocker(lockers[i]).balanceOf(account);
        }
        return total;
    }
    ...
    function getClaimable(address account) external view returns (uint256) {
        uint256 total = 0;
        for (uint256 i = 0; i < lockers.length; i++) {
            total = total + ILocker(lockers[i]).claimable(account);
        }
        return total;
    }

    function addLocker(address locker) external onlyApproved {
        lockers.push(locker);
    }

    function withdrawFromAll() external {
        for (uint256 i = 0; i < lockers.length; i++) {
            ILocker(lockers[i]).withdraw(msg.sender);
        }
    }
}