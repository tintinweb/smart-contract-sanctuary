...
contract NFTPool is Ownable {
    ...
    function stake(uint256 amount) public virtual {
        _balances[msg.sender] = _balances[msg.sender] + rewardAmount(msg.sender) + amount;
    ...
    function unstake(uint256 amount) public virtual {
        require(_balances[msg.sender] >= amount, "Not enough user balance to withdraw");
        _balances[msg.sender] = _balances[msg.sender] + rewardAmount(msg.sender) - amount;
        lastUpdate[msg.sender] = block.timestamp;
        unlockTime[msg.sender] = block.timestamp + DURATION;
    ...
    function rewardAmount(address account) private view returns (uint256) {
        return _balances[account] * aprPercent * (block.timestamp - lastUpdate[account]) / 100 / DURATION;
    }
    
    function earned(address account) external view returns (uint256) {
        return rewardAmount(account);
    }
