...
contract GrowthVault is ERC20, Ownable {
    ...
    uint256 public remainingFee; // in native tokens
    ...
    function autoCompound() public {
        ...
        for (uint256 i; i < names.length; i++) {
            ...
            if (_amount > 0) {
                ...
                rewardBalance = _getBalanceOfToken(_cakeToken);
                ...
                path[0] = WETH;
                path[1] = USDT;
                rewardsFee = _calculateAmountFee(_getBalanceOfToken(WETH) - remainingFee);
                amounts = IRouter(strategyObj.liquidityRouter).getAmountsOut(rewardsFee, path);
                if(amounts[1] > 100) {
                    rewardsFee += remainingFee;

                    IRouter(strategyObj.liquidityRouter).swapExactETHForTokens{value:rewardsFee}(
