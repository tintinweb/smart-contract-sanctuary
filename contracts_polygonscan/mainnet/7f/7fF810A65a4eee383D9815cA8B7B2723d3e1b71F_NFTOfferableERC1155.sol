    ...
    ERC2771Context
{
    ...
    constructor()
        ERC1155PresetMinterPauser(
            "https://us-central1-artis-life-network.cloudfunctions.net/tokenURI?id={id}"
        ) ERC2771Context(0x207Fa8Df3a17D96Ca7EA4f2893fcdCb78a304101)
    {
        _setupRole(OWNER_ROLE, msgSender());
        tokenURI = "https://us-central1-artis-life-network.cloudfunctions.net/tokenURI?id={id}";
        uri = "https://us-central1-artis-life-network.cloudfunctions.net/tokenURI?id={id}";
        name = "ArtisLife Network";
        symbol = "ARTISNFT";
    ...
    function contractURI() public pure returns (string memory) {
        return
            "https://us-central1-artis-life-network.cloudfunctions.net/contractURI";
    }
...
// File contracts/NFTOfferableERC1155.sol

pragma solidity ^0.8.0;
contract NFTOfferableERC1155 {
    using SafeMath for uint256;
    ArtisLifeNFT public nftObject;
    IERC20 public WETH;
    ...
    mapping(uint256 => nftInfo) public nftsForOffer;
    ...
    event NewOfferCreated(address owner, uint256 offerID, uint256 nftID, uint256 price);
    event OfferFeeUpdated(uint256 offerId, uint256 fee);

    constructor() {
        nftObject = ArtisLifeNFT(0xa79dF500990cBEf47c4b0b7B8c70666bd2fe389b);
    ...
    function createOffer(
        ...
        emit NewOfferCreated(from, offerID, id, price);
    ...
    {
        uint256 offerPrice = nftsForOffer[offerId].price_in_eth;
        uint256 fee = nftsForOffer[offerId].fee.div(100).mul(offerPrice);
        offerPrice = offerPrice.sub(fee);
        uint256 nftID = nftsForOffer[offerId].nftID;
        WETH.transferFrom(address(this), msg.sender, offerPrice);
        nftObject.safeTransferFrom(msg.sender, nftsForOffer[offerId].owner, nftID, 1, "0x3");
        nftsForOffer[offerId].active = false;
        nftsForOffer[offerId].completed = true;
        currentOfferCount[nftID] = currentOfferCount[nftID].sub(1);
        emit OfferAccepted(offerId, msg.sender, offerPrice, fee);
    }
    ...
    {
        uint256 nftID = nftsForOffer[offerId].nftID;
        uint256 amount = nftsForOffer[offerId].price_in_eth;
        WETH.transferFrom(address(this), msg.sender, amount);
        nftsForOffer[offerId].canceled = true;
        currentOfferCount[nftID] = currentOfferCount[nftID].sub(1);
        emit OfferCanceled(offerId);
    }

    function updatePrice(uint256 offerId, uint256 price)
        external
        onlyOfferOwner(offerId)
        offerExists(offerId)
        ...
        offerIsNotCanceled(offerId)
    {
        nftsForOffer[offerId].price_in_eth = price;
        emit OfferPriceUpdated(offerId, price);
    }

    function updateFee(uint256 offerId, uint256 fee)
        external
        onlyAdmin
        feesActivated
        offerExists(offerId)
        offerIsNotCanceled(offerId)
    {
        ...
        nftsForOffer[offerId].fee = fee;
        emit OfferFeeUpdated(offerId, fee);
    }
    ...
    function activateFees() external onlyAdmin {
        feesActive = true;
    }
    ...
    modifier onlyOfferOwner(uint256 offerId) {
        require(msg.sender == nftsForOffer[offerId].owner, "Not Owner.");
        _;
    }

    modifier offerIsActive(uint256 offerId) {
        require(nftsForOffer[offerId].active == true, "Offer not active.");
        _;
    }

    modifier offerIsNotCanceled(uint256 offerId) {
        require(
            nftsForOffer[offerId].canceled == false,
    ...
    modifier offerExists(uint256 offerId) {
        require(nftsForOffer[offerId].created == true, "Offer ID not valid.");
        _;
    }

    modifier offerIsNotCompleted(uint256 offerId) {
        require(nftsForOffer[offerId].completed == false, "Offer has concluded.");
        _;
    }

    modifier feesActivated() {
        require(feesActive == true, "Fees not active.");
        _;
    }
}