...
// File contracts/grow/GrowStakingPool.sol
...
contract GrowStakingPool is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    /// @notice last rewards update timestamp
    ...
    function rewardsPerSharPerSecond() public view returns(uint256) {
        if (totalSupply == 0) {
            return 0;
        } else {
    ...
    function checkIfNeedGetReward() external onlyRole(KEEPER_ROLE) {
        // 1. update remaining reward
        updateReward();

        // 2. get reward from grow rewarder
        uint256 growBalanceBefore = STAKING_TOKEN.balanceOf(address(this));
        IGrowRewarder(GrowRegister.get.GrowRewarderAddress()).getVaultReward();

        // 3. update remaining reward
        uint256 rewardAmount = STAKING_TOKEN.balanceOf(address(this)).sub(growBalanceBefore);
        remainingRewardTokenAmount = remainingRewardTokenAmount.add(rewardAmount);

        emit LogRewardAdded(rewardAmount);
    }
    ...
    function deposit(uint256 amount) external nonReentrant {
        if (block.timestamp < GrowRegister.get.TestFlightEndTime()) {
            require(IGrowTestPilot(GrowRegister.get.GrowTestPilotAddress()).isTestPilot(msg.sender), "only TestPilot");
        }
        _deposit(amount, msg.sender);
    }

    function _deposit(uint256 amount, address userAddress) private {
        // 1. update remaining reward
        updateReward();
    ...
    function _withdraw(uint256 amount, address user, address to) private {
    ...
    function withdrawAs(uint256 wantTokenAmount, address tokenAddress, uint minReceive) external nonReentrant {
        _withdraw(wantTokenAmount, msg.sender, msg.sender);

        approveToken(address(STAKING_TOKEN), GrowRegister.get.ZapAddress(), wantTokenAmount);

        IZAP(GrowRegister.get.ZapAddress()).zapOut(address(STAKING_TOKEN), tokenAddress, wantTokenAmount, msg.sender, minReceive);
    }
    ...
    function withdrawAllAs(address tokenAddress, uint minReceive) external virtual nonReentrant {
        uint256 withdrawnAmount = balanceOf(msg.sender);

        _withdraw(withdrawnAmount, msg.sender, msg.sender);

        approveToken(address(STAKING_TOKEN), GrowRegister.get.ZapAddress(), withdrawnAmount);

        IZAP(GrowRegister.get.ZapAddress()).zapOut(address(STAKING_TOKEN), tokenAddress, withdrawnAmount, msg.sender, minReceive);
    }
