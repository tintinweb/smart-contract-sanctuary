...
contract FactoryDeployer is Initializable, AccessControl {
    ...
    uint8 public nftStakeFee = 5;
    uint8 public playToEarnFee = 90;
    ...
    function changeFees(
        uint8 _creatorFee,
        uint8 _playToEarnFee,
        uint8 _nftStakeFee
    ) public onlyRole(MAIN_OWNER) {
        require(
            _creatorFee + _playToEarnFee + _nftStakeFee == 100,
            INVALID_FEES
        );

        creatorFee = _creatorFee;
        playToEarnFee = _playToEarnFee;
        nftStakeFee = _nftStakeFee;
    }
