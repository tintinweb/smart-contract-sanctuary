...
contract Lottery is LotteryOwnable, Initializable {
    ...
    // State variables 
    // Instance of Wings token (collateral currency for lotto)
    IERC20 internal wings_;
    // Storing of the NFT
    ...
    uint256 private ticketIndicator_;
    uint256 private totalClaimable;
    
    address private burnPool_;
    ...
    enum Status { 
        ...
        RNRequested,    // Random number requested to chainlink VRF.
    ...
    struct LottoInfo {
        ...
        uint256 prizePoolInWings;    // The amount of wings for prize money
        uint256 costPerTicket;      // Cost per ticket in $wings
        ...
        uint16[] winnerCounts;      // The winner counts
        uint256 startTicketID;      // start ticket id
        uint256 endTicketID;        // end ticket id
    }
    ...
    function initialize(
        ...
        ticketIndicator_ = 1;
        burnPool_ = 0x000000000000000000000000000000000000dEaD;
    ...
    function getClaimableRewardForTicket(uint256 _lotteryId, uint256 _titkcetId) external view returns(uint256) {
        if(allLotteries_[_lotteryId].lotteryStatus != Status.Completed)
            return 0;
            
        uint8 matchingNumbers = _getNumberOfMatching(
            nft_.getTicketNumbers(_titkcetId),
    ...
    function getPrizeForCurrentLottery (
    )
        public view
        returns (uint256 prize)
    {
        prize = wings_.balanceOf(address(this)).sub(totalClaimable);
    }
        ...
        onlyOwner() 
    {
        ...
        // Sets lottery status to randon number requested
        allLotteries_[_lotteryId].lotteryStatus = Status.RNRequested;
        ...
        onlyRandomGenerator()
    {
        ...
        if(requestId_ == _requestId) {
            ...
            allLotteries_[_lotteryId].endTicketID = nft_.getTotalSupply();
            allLotteries_[_lotteryId].prizePoolInWings = getPrizeForCurrentLottery();
        }
        ...
        onlyOwner() 
    {
        ...
        require(
            allLotteries_[_lotteryId].lotteryStatus == Status.Closed,
            "Wait until number drawn"
        );

        uint256 lastTicketId = ticketIndicator_.add(loopLength);
        lastTicketId = lastTicketId > allLotteries_[_lotteryId].endTicketID.add(1) ? allLotteries_[_lotteryId].endTicketID.add(1) : lastTicketId;
        
        uint16[] memory winnerCounts = new uint16[](sizeOfLottery_);
        uint8 matchingNumbers;
        for(uint256 ticketId = ticketIndicator_; ticketId < lastTicketId; ticketId++) {
            
            matchingNumbers = _getNumberOfMatching(
                nft_.getTicketNumbers(ticketId),
                allLotteries_[_lotteryId].winningNumbers
            );
            
            if (matchingNumbers > 1)
                winnerCounts[matchingNumbers - 1]++;
        }
        
        for(uint8 i = 0; i < sizeOfLottery_; i++)
            allLotteries_[_lotteryId].winnerCounts[i] = allLotteries_[_lotteryId].winnerCounts[i].add(winnerCounts[i]);
        
        ticketIndicator_ = lastTicketId;
        if(lastTicketId == allLotteries_[_lotteryId].endTicketID.add(1)) {
            allLotteries_[_lotteryId].lotteryStatus = Status.Completed;
            uint256 winPercent;
            for(uint16 i = 1; i < allLotteries_[_lotteryId].prizeDistribution.length; i++) {
                if(allLotteries_[_lotteryId].winnerCounts[i] > 0)
                    winPercent = winPercent.add(allLotteries_[_lotteryId].prizeDistribution[i]);
            }
            uint256 claimablePrize = allLotteries_[_lotteryId].prizePoolInWings.mul(winPercent).div(100);
            totalClaimable = totalClaimable.add(claimablePrize);
        }
    }
     ...
     *          be [5, 10, 15, 20, 30] = 100%. This means if you get 2 matching would be 10% and so on.
     *          and 5% is burned.
    ...
    {
        uint256 curretTime = getCurrentTime();
        require(
            _startingTimestamp > curretTime && 
            _closingTimestamp > curretTime && 
            _closingTimestamp > _startingTimestamp,
	    ...
	    if(lotteryIdCounter_ > 0) {
            require(
                ...
                "Please draw winning number of previous lottery."
            );
        }
        ...
        LottoInfo memory newLottery = LottoInfo(
            ...
            winningNumbers,
            winnerCounts,
            nft_.getTotalSupply().add(1),
            0
        );
        allLotteries_[lotteryId] = newLottery;

        if(lotteryId > 1) {
            uint256 burnAmount = allLotteries_[lotteryId-1].prizePoolInWings.mul(allLotteries_[lotteryId-1].prizeDistribution[0]).div(100);
            wings_.safeTransfer(burnPool_, burnAmount);
        }
        ...
        notContract()
    {
        ...
        // Transfers the required wings to this contract
        wings_.transferFrom(
            msg.sender, 
            address(this), 
            costWithDiscount
    ...
    function claimReward(uint256 _lotteryId, uint256 _tokenId) external notContract() {
        ...
        totalClaimable = totalClaimable.sub(prizeAmount);
        // Transfering the user their winnings

        uint256 wingsBalance = wings_.balanceOf(address(this));
        if(wingsBalance < prizeAmount)
            wings_.safeTransfer(address(msg.sender), wingsBalance);
        else
            wings_.safeTransfer(address(msg.sender), prizeAmount);
    }
        ...
        notContract()
    {
        ...
        totalClaimable = totalClaimable.sub(totalPrize);
        
        uint256 wingsBalance = wings_.balanceOf(address(this));
        if(wingsBalance < totalPrize)
            wings_.safeTransfer(address(msg.sender), wingsBalance);
        else
            wings_.safeTransfer(address(msg.sender), totalPrize);
    }
     ...
     * @return  uint256: The prize amount in wings the user is entitled to 
     */
        ...
        returns(uint256) 
    {
        ...
        // Getting the winner numbers of the pool the user has won
        uint256 winnerCountsOfPool = allLotteries_[_lotteryId].winnerCounts[_noOfMatching-1];
        // Timesing the percentage one by the pool
        prize = allLotteries_[_lotteryId].prizePoolInWings.mul(perOfPool).div(100).div(winnerCountsOfPool);
