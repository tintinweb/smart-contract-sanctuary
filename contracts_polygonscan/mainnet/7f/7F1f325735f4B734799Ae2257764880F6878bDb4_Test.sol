...
contract Test is ERC721, ERC721Enumerable, Ownable {
  ...
  function getTestByOwner(address _owner) public view returns (uint256[] memory) {
  ...
  function mintTestTier2() public payable {
    require(_isSaleActive, 'Sale must be active to mint Test');
    ...
    _mintTest(TIER2_NUM_TOKENS, msg.sender);
    emit TokenMinted(totalSupply());
  }

  function mintTestTier3() public payable {
    require(_isSaleActive, 'Sale must be active to mint Test');
    ...
    _mintTest(TIER3_NUM_TOKENS, msg.sender);
    emit TokenMinted(totalSupply());
  }

  function mintTestTier4() public payable {
    require(_isSaleActive, 'Sale must be active to mint Test');
    ...
    _mintTest(TIER4_NUM_TOKENS, msg.sender);
    emit TokenMinted(totalSupply());
  }

  function reserveTest(uint256 numTest) public onlyOwner {
    require(totalSupply().add(numTest) <= MAX_SUPPLY, 'Sale would exceed max supply');
  ...
  function setThirteenthTestBaseURI(string memory thirteenthTestBaseURI) external onlyOwner {
  ...
  function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
    ...
    if (totalSupply() >= MAX_SUPPLY || block.timestamp >= revealTimeStamp) {
      if (tokenId < MAX_SUPPLY) {
        ...
        return string(abi.encodePacked(_thirteenthTestBaseURI, tokenId.toString()));
      }
    } else {
