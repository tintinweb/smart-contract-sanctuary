...
contract StrategyLiquidate is Ownable, ReentrancyGuard, Strategy {
  ...
  constructor() public {
    router = IUniswapV2Router02(0x3a1D87f206D12415f5b0A33E786967680AAb4f6d);
  ...
  function setWhitelistTokens(address[] calldata tokens, bool[] calldata statuses, address[] calldata _vaults)
    external
    onlyOwner
  {
    require(tokens.length == statuses.length && statuses.length == _vaults.length, 'tokens & statuses length mismatched');
    for (uint idx = 0; idx < tokens.length; idx++) {
      whitelistedTokens[tokens[idx]] = statuses[idx];
      vaults[tokens[idx]] = _vaults[idx];
    }
  }
  ...
  function execute(
    ...
    (address fToken, uint minUSDC) = abi.decode(data, (address, uint));
    require(whitelistedTokens[fToken], 'token not whitelisted');
    
    IUniswapV2Pair lpToken = IUniswapV2Pair(factory.getPair(fToken, wmatic));
    // 2. Remove all liquidity back to USDC and farming tokens.
    lpToken.approve(address(router), uint(-1));
    IVault(vaults[fToken]).withdrawAll();
    router.removeLiquidity(fToken, wmatic, lpToken.balanceOf(address(this)), 0, 0, address(this), now);
    ...
    uint balance = wmatic.myBalance();
    require(balance >= minUSDC, 'insufficient USDC received');
    wmatic.safeTransfer(msg.sender, balance);
  }
