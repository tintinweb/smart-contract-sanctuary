...
// File @openzeppelin/contracts-upgradeable/security/Ree[email protected]
...
interface IZAP {
    function swap(address[] memory tokens, uint amount, address receiver, uint) external returns (uint);
    function zapOut(address fromToken, address toToken, uint amount, address receiver) external returns (uint);
}

interface IGrowMembershipPool{
    function hasMembership(address userAddress) external view returns (bool);
}
...
abstract contract BaseGrowStrategyV3 is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function withdrawAs(uint256 wantTokenAmount, address tokenAddress) external virtual nonEmergency nonReentrant {
        _getRewards(msg.sender);
        uint256 withdrawnWantTokenAmount = _withdraw(wantTokenAmount);

        approveToken(STAKING_TOKEN, GrowRegister.get.ZapAddress(), withdrawnWantTokenAmount);

        IZAP(GrowRegister.get.ZapAddress()).zapOut(STAKING_TOKEN, tokenAddress, wantTokenAmount, msg.sender);
    }
    ...
    function _withdraw(uint256 wantTokenAmount) internal returns (uint256) {
        ...
        bool isPro = IGrowMembershipPool(GrowRegister.get.GrowMembershipPoolAddress()).hasMembership(msg.sender);
        if (!isPro) {
