...
contract Owls is ERC721Enumerable, Ownable{
    ...
    //TLDR.coffee
    string public OWLS_PROVENANCE = "";
    uint256 public constant NFT_PRICE = 12000000000000000000; // 12 MATIC ~ 1 bag of coffee beans -- TLDR.coffee
    uint public constant MAX_NFT_PURCHASE = 10;
    uint256 public MAX_SUPPLY = 10999;
    ...
    constructor() ERC721("24owls","24owls"){
    }
    
    function contractURI() public view returns (string memory) {
        return "ipfs://QmYCX1fDbdMqqG5hAdLDXFsntbTDDvED3B8FRhs4qQudwK";
    }
    ...
    */
    function setProvenanceHash(string memory provenanceHash) public onlyOwner {
        OWLS_PROVENANCE = provenanceHash;
    }
    ...
    function mint(uint numberOfTokensMax10) public payable {
        require(saleIsActive, "Sale is not active at the moment");
        require(numberOfTokensMax10 > 0, "Number of tokens can not be less than or equal to 0");
        require(totalSupply().add(numberOfTokensMax10) <= MAX_SUPPLY, "Purchase would exceed max supply");
        require(numberOfTokensMax10 <= MAX_NFT_PURCHASE,"Can only mint up to 10 per purchase");
        require(NFT_PRICE.mul(numberOfTokensMax10) == msg.value, "Sent ether value is incorrect");

        for (uint i = 0; i < numberOfTokensMax10; i++) {
