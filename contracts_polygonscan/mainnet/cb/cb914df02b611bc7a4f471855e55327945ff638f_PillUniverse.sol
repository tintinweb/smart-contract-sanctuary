...
contract PillUniverse is ERC20, AccessControl, ERC721Holder {
    ...
    constructor(address owner, address _collectionAddress) ERC20("Medicine Coin", "MDCN") AccessControl(){
        ...
        coinAmountPerRewardUnit = 10 * 10 ** 18; // 5 ERC20 coins per rewardUnit, may be changed later on
        numberOfBlocksPerRewardUnit = 20571; // 12 hours per reward unit , may be changed later on
        welcomeBonusAmount = 100 * 10 ** 18; // 170 tokens welcome bonus, only paid once per tokenId
    }
    ...
    function stake(uint256 tokenId) public {
        ...
        require(IERC721(collectionAddress).ownerOf(tokenId) == address(this),
            "MDCN: Error while transferring token");
    ...
    function setCollectionAddress(address newAddress) external onlyRole(CONTRACT_ADMIN_ROLE){
        require (newAddress != address(0), "MDCN: update to zero address not possible");
