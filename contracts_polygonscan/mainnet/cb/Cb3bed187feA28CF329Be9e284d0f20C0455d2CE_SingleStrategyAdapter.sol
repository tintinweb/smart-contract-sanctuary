...
contract SingleStrategyPolygon is OwnableUpgradeable {
  ...
  */
  ...
  function withdrawReward(address user) public onlyVault {
    ...
    if(_pendingDQuick > 0 && _pendingDQuick <= curPendingDQuick) {
      uint available = withdrawDquickFees(_pendingDQuick, dQuick);

      IERC20(dQuick).safeTransfer(user, available);
    ...
    address dQuickRewardsToken = getSecondRewardsToken();
    uint256 _balance = IERC20(dQuickRewardsToken).balanceOf(address(this));

    if (_balance < _pending) {
      IStakingRewards(dQuickExternalRewardsPool).getReward();

      _balance = IERC20(dQuickRewardsToken).balanceOf(address(this));
    }

    _pending = MathUpgradeable.min(_balance, _pending);

    if(_pending > 0 && _pending <= curPendingReward){
      uint available = withdrawDquickFees(_pending, dQuickRewardsToken);

      // send reward to user
      IERC20(dQuickRewardsToken).safeTransfer(user, available);
      lastPendingReward = curPendingReward.sub(available);
    }
  }

  function getSecondRewardsToken() public view returns (address token) {
    return IStakingRewards(dQuickExternalRewardsPool).rewardsToken();
  }
  ...
  */
  ...
  function withdrawDquickFees(uint256 added, address token) internal returns (uint256) {
    if (added > 0) {
