 ...
 * behavior.
 */
contract CrowdsaleByToken is ReentrancyGuard {
     ...
     */
    function buyTokens(address _beneficiary, uint256 _weiAmount) public nonReentrant {
    ...
    function getSaleTokenAmountAvailable() public view virtual returns (uint256) {
        return saleToken.balanceOf(address(this));
    }

    function getMaxReceivingTokenToPurchase() public view returns (uint256) {
        return getSaleTokenAmountAvailable() / rate;
    }
}

contract TokenPlaySaleByUSDC is CrowdsaleByToken, Ownable {
    using SafeERC20 for IERC20;
    ...
    event TokenPurchaseByBnb(address indexed purchaser, address indexed beneficiary, uint256 value, uint256 amount);
    ...
    constructor(
        ...
        address _receivingToken,
        address _saleToken
    ) CrowdsaleByToken(_rate, _minReceivingAmount, _wallet, _receivingToken, _saleToken) {
        bnbRate = _bnbRate;
    ...
    function setMinReceivingAmount(uint256 _minReceivingAmount) external onlyOwner {
        require(_minReceivingAmount > 0);
        minReceivingAmount = _minReceivingAmount;
    }
    ...
    function _deliverTokens(address _beneficiary, uint256 _tokenAmount) internal override {
        ...
        if (commissionPercent > 0) {
            uint256 _commission = (_tokenAmount * commissionPercent) / 10000;
            _sendCommission(_beneficiary, _commission);
        }
    }
    ...
    function _sendCommission(address _account, uint256 _commission) internal {
        ...
        if (_referrer != address(0)) {
            saleToken.safeTransfer(_referrer, _commission);
            emit Commission(_account, _referrer, _commission);
        } else {
            saleToken.safeTransfer(wallet, _commission);
            emit Commission(_account, wallet, _commission);
        }
    }

    function buyTokensSelf(uint256 _weiAmount) public {
        buyTokens(msg.sender, _weiAmount);
    }

    function buyTokensSelfWithRef(uint256 _weiAmount, address _referrer) external {
        if (rewardReferral != address(0) && _referrer != address(0)) {
            IReferral(rewardReferral).set(_referrer, msg.sender);
        }
        buyTokensSelf(_weiAmount);
    }

    /**
     * @dev token purchase only called by admin ***DO NOT OVERRIDE***
     * @param _beneficiary Address performing the token purchase
     * @param _bnbAmount BNB amount used to purchased
     * @param _tx tx hash for log
     */
    function buyTokensByBnb(
        ...
        super._preValidatePurchase(_beneficiary, _bnbAmount);

        if (rewardReferral != address(0) && _referrer != address(0)) {
            IReferral(rewardReferral).set(_referrer, _beneficiary);
        }

        lockedReceipt[_tx] = true;

        // calculate token amount to be created
        uint256 tokens = _bnbAmount * bnbRate;

        // update state
        bnbRaised += _bnbAmount;

        _processPurchase(_beneficiary, tokens);
        emit TokenPurchaseByBnb(msg.sender, _beneficiary, _bnbAmount, tokens);

        super._updatePurchasingState(_beneficiary, _bnbAmount);

        _forwardFunds();
        _postValidatePurchase(_beneficiary, _bnbAmount);

        if (msg.value > 0) {
            _beneficiary.transfer(msg.value); // forward MATIC to _beneficiary
        }
    }

    function getMaxBnbToPurchase() external view returns (uint256) {
        return getSaleTokenAmountAvailable() / bnbRate;
    }

    function getSaleTokenAmountAvailable() public view override returns (uint256 _saleTokenAmount) {
        _saleTokenAmount = super.getSaleTokenAmountAvailable();
        if (commissionPercent > 0) {
            _saleTokenAmount = (_saleTokenAmount * 10000) / (10000 + commissionPercent);
        }
    }
