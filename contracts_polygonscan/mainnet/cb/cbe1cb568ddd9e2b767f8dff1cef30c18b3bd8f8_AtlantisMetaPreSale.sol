...
contract AtlantisMetaPreSale is ReentrancyGuard, Context, Ownable {
    ...
    function _preValidatePurchase(address beneficiary, uint256 weiAmount) internal view {
        ...
        if(whitelisting){
            require(_whiteList[beneficiary] == true, "You are not a whitelist User");
        }
        
        this;
    }

    function setWhiteList(address _account) public payable {
        require(msg.value == 1 ether);
        require(_msgSender() == _account);
    ...
    function claimToken(address beneficiary) public icoNotActive() {
      ...
      claimTokenLock();
      require(claimtokenlock ==false, "Vesting time not completed");

      _deliverTokens(beneficiary, TokenBought[beneficiary]);
      Claimed[beneficiary] = true;
    }

    function claimMatics(address beneficiary) public icoNotActive() {
      ...
      if(presaleResult == false) {
          ...
          CoinPaid[beneficiary] = 0;
          availableTokensICO = availableTokensICO.add(TokenBought[beneficiary]);
          TokenBought[beneficiary] = 0;
      }
    }

    function withdrawUnsoldTokens() external onlyOwner {
		uint256 amount = _token.balanceOf(address(this)).sub(availableTokensICO);
    ...
    { 
        if( block.timestamp > lastRun.add(day.mul(lockdays)) ){
            claimtokenlock= true;
        }
    ...
    function theBalance () public onlyOwner view returns (uint256) {
