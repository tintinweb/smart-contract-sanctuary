...
contract StrategyLITHIUMUSDC is StratManager, FeeManager {
    ...
    address constant public output = address(0xfE1a200637464FBC9B60Bc7AeCb9b86c0E1d486E);
    ...
    address constant public masterchef = address(0xfcD73006121333C92D770662745146338E419556);
    uint8 public poolId;

    // Routes
    address[] public outputToWbnbRoute = [output, wmatic];
    ...
    */
    ...
    function chargeFees() internal {
        uint256 toWbnb = IERC20(output).balanceOf(address(this)).mul(44).div(1000);
        IUniswapRouterETH(unirouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(toWbnb, 0, outputToWbnbRoute, address(this), now);
        ...
        uint256 paprFeeAmount = wmaticBal.mul(paprFee).div(MAX_FEE);
        IERC20(wmatic).safeTransfer(paprFeeRecipient, paprFeeAmount);
    ...
    function setoutputToLpRoutes(address[] memory _route, address[] memory _route1) public onlyOwner {
    outputToLp0Route = _route;
    outputToLp1Route = _route1;

    }

    function setoutputToWBNBRoute(address[] memory _route) public onlyOwner {
    outputToWbnbRoute = _route;
    }
