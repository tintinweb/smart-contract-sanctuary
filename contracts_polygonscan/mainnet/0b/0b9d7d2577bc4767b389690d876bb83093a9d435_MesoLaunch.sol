...
// File contracts/interfaces/IFeeTo.sol
 ...
 * @author @MesoFinance
 **/
contract MesoLaunch is Ownable {
    ...
    /**
     * @dev Called by MesoLaunchFactory when a new offering is created
     **/
    function initialize(
        IERC20 _token,
        address _router,
        ...
        softCap = _caps[0]; //Minimum amount of tokens to sell for presale to proceed. Ex. 1000000000000000000 or 1
        ...
        maxTokens = _tokenLimits[1]; //Max number of tokens a wallet can hold. Ex. 1000000000000000000 or 1 token
...
// File contracts/MesoLaunchFactory.sol
...
contract MesoLaunchFactory is OwnableUpgradeable {
    ...
    /// @notice All offerings
    EnumerableSetUpgradeable.AddressSet private launches;

    event LaunchCreated(address launchAddress);
    ...
    function createLaunch(
        ...
        bytes memory bytecode = type(MesoLaunch).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(_token, block.number, msg.sender));
        assembly {
            launchAddress := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        uint16 fee = fees[_listingType];
        uint16[3] memory settings = [_liquidityPct, fee, _listingType];
        MesoLaunch sale = MesoLaunch(launchAddress);
        sale.initialize(_token, _router, _dates, _caps, _tokenLimits, settings, _features);
        sale.transferOwnership(msg.sender);
        launches.add(launchAddress);
        emit LaunchCreated(launchAddress);
    }
