...
// File contracts/KillSwitchToken.sol
...
// File contracts/PronteraCastle.sol

pragma solidity ^0.8.0;

interface IStakeWithdrawer {
    function castle() external returns (address);
}
...
contract PronteraCastle is Ownable {
    ...
    function setProntera(address _prontera) external onlyOwner {
        ...
        require(IStakeWithdrawer(_prontera).castle() == address(this), "invalid prontera");
    ...
    function setEmperium(address _emperium) external onlyOwner {
        ...
        require(IStakeWithdrawer(_emperium).castle() == address(this), "invalid emperium");
...
contract Prontera is Ownable, PronteraGuard {
    ...
    // Info of each pool.
    address[] public poolInfoAddresses;
    ...
    function poolLength() external view returns (uint256) {
        return poolInfoAddresses.length;
    }

    function add(uint256 _allocPoint, address _izlude) external onlyOwner {
        if (poolInfoAddresses.length > 0) {
            PoolInfo storage pool = poolInfo[_izlude];
        ...
        poolInfo[_izlude] = PoolInfo({
            izlude: Izlude(_izlude),
            allocPoint: _allocPoint,
            lastRewardBlock: lastRewardBlock,
            accKSWPerShare: 0
        });
        poolInfoAddresses.push(_izlude);
    }

    function remove(address _izlude) external onlyOwner {
        if (poolInfoAddresses.length == 0) {
            return;
        }

        require(_izlude != address(0), "nope");
        PoolInfo storage pool = poolInfo[_izlude];
        require(address(pool.izlude) != address(0), "not found");

        uint256 length = poolInfoAddresses.length;
        for (uint256 i = 0; i < length; ++i) {
            if (poolInfoAddresses[i] == _izlude) {
                // found
                totalAllocPoint -= pool.allocPoint;
                delete poolInfo[_izlude];

                poolInfoAddresses[i] = poolInfoAddresses[length - 1];
                poolInfoAddresses.pop();
                break;
            }
        }
    }
    ...
    function set(address _izlude, uint256 _allocPoint) external onlyOwner {
        massUpdatePools();
        totalAllocPoint -= poolInfo[_izlude].allocPoint;
        totalAllocPoint += _allocPoint;
        poolInfo[_izlude].allocPoint = _allocPoint;
    }
    ...
    function pendingKSW(address _izlude, address _user) external view returns (uint256) {
        ...
        UserInfo storage user = userInfo[_izlude][_user];
    ...
    function massUpdatePools() public {
        uint256 length = poolInfoAddresses.length;
        for (uint256 i = 0; i < length; ++i) {
            updatePool(poolInfoAddresses[i]);
        }
    }

    // Update reward variables of the given pool to be up-to-date.
    function updatePool(address _izlude) public {
        PoolInfo storage pool = poolInfo[_izlude];
...
// File contracts/interfaces/IMorroc1_5.sol
...
// File contracts/MorrocView1_5.sol

pragma solidity ^0.8.0;

contract MorrocView1_5 {
    address private constant COUPON = 0x084bb94e93891D74579B54Ab63ED24C4ef9cd5Ef;
