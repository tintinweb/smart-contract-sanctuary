...
contract BPD is IBpd, Manageable, ExternallyCallable {
        ...
        returns (uint256[2] memory)
    {
        ...
        //less than 350 days
        if ((end - start) / denom == 0) {
            bpdEnd = bpdInterval[0]; //set end interval same as start interval
        } else {
            for (uint256 i = 0; i < 5; i++) {
                if (end > (settings.contractStartTimestamp + denom * (i + 1))) {
                    bpdEnd = i + 1;
                }
            }
        }

        bpdInterval[1] = MathUpgradeable.min(bpdEnd, 5);
