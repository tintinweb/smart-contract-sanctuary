...
contract Referral is OwnableUpgradeable, ReentrancyGuard {
    ...
    function setAuthority(address author, bool status) public onlyOwner {
        authors[author] = status;
    }

    function set(address from, address to) public onlyAuthority {
        if (from != address(0x0)
        && to != address(0x0)
        && from != to
    ...
    function emergencyToken(IERC20 token, uint256 amount) external onlyOwner {
