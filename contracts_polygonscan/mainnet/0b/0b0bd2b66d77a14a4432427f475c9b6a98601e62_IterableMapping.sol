...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
    ...
    address public adminAddress = 0xE81251Aa4CdeEB2a4e7044f421f2a75A7177cE82;
...
contract SlickBNB is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public toBurnSlickAddress = 0xdb4AD33875704B8dC9ED685462450A1EECe7A1f4;
    uint256 public toBurnSlickFee = 2;
    ...
    uint256 public buyBackUpperLimit = 1 * 10 ** 18;        //10 MATIC

    // Minimum MATIC balance before buyback IF lower than this number no buyback
    uint256 public minimumBalanceRequired = 1 * 10 ** 18;   //10 MATIC
    ...
    constructor() ERC20("SlickBNB", "SlickBNB") {
        ...
        teamWallet = 0xc1Ab63200581C5a32C065C4D9e40E4B15Ba6ee4E;
        a80DividendToken = 0x3BA4c387f786bFEE076A58914F5Bd38d668B42c3;
        b20DividendToken = 0xdb4AD33875704B8dC9ED685462450A1EECe7A1f4;
    ...
    function updateBurnSlickAddress(address _newAddress) external onlyOwner {
        toBurnSlickAddress = _newAddress;
    }
    ...
    function setToBurnSlickFee(uint256 newFee) external onlyOwner {
        require(newFee <= MAX_FEE_RATE, "wrong");
        toBurnSlickFee = newFee;
        _updateTotalFee();
    }
    ...
    function _updateTotalFee() internal {
        totalFees = buyBackAndLiquidityFee.add(marketingFee).add(a80DividendRewardsFee).add(b20DividendRewardsFee).add(toBurnSlickFee);
    }
    ...
    function updateA80DividendTracker(address newAddress) external onlyOwner {
        ...
        require(newA80DividendTracker.owner() == address(this), "must be owned by ThunderAda");
    ...
    function updateB20DividendTracker(address newAddress) external onlyOwner {
        ...
        require(newB20DividendTracker.owner() == address(this), "must be owned by ThunderAda");
    ...
    function _transfer(
        ...
        if (
            ...
            if (!swapping && contractTokenBalance >= swapTokensAtAmount) {
                ...
                if(marketingEnabled && buyBackAndLiquifyEnabled && toBurnSlickFee > 0 && buyBackMode){

                    uint256 total3Fees = marketingFee.add(buyBackAndLiquidityFee).add(toBurnSlickFee);
                    ...
                    if(increaseAmount > 0){
                        ...
                        uint256 bnbToBurnSlick = increaseAmount.mul(toBurnSlickFee).div(total3Fees);
                        useBnbToBurnSlick(bnbToBurnSlick);
                    }
                }else{
                    if (marketingEnabled) {
                    ...
                    if(toBurnSlickFee > 0){
                        uint256 swapTokensToBurnSlick = contractTokenBalance.mul(toBurnSlickFee).div(totalFees);
                        buyBackSlickAndBurn(swapTokensToBurnSlick);
                    }
                }
    ...
    function useBnbToBurnSlick(uint256 amount) private {
        // generate the uniswap pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = uniswapV2Router.WETH();
        path[1] = toBurnSlickAddress;
    ...
    function buyBackSlickAndBurn(uint256 amount) private {
        address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();
        path[2] = toBurnSlickAddress;
...
contract A80DividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("BNB80_Dividend_Tracker", "BNB80_Dividend_Tracker", 0x3BA4c387f786bFEE076A58914F5Bd38d668B42c3) {
        claimWait = 3600;
...
contract B20DividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("BNB20_Dividend_Tracker", "BNB20_Dividend_Tracker", 0xdb4AD33875704B8dC9ED685462450A1EECe7A1f4) {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }
