...
contract PrimaryBridge is BaseBridge {
    ...
    function release(uint256 amount, address to, uint256 chainId) external onlyAuthorised() returns (bool) {
        require(amount > 0, "Amount must be greater than 0");
        require(amount <= amountHeld, "Amount must be less than or equal to the amount held");
        require(_balances[chainId] >= amount, "Amount must be greater than or equal to the amount held");
