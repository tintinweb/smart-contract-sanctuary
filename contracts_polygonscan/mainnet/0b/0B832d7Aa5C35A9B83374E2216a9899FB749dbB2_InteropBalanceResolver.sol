...
contract InteropBalanceResolver is Helpers {
     function checkLiquidity (
        ...
        p = _checkLiquidity(position, liquidityContract, isTarget);
    }
    ...
    function _checkLiquidity (
        ...
        if (isTarget) {
            ...
            for (uint256 i = 0; i < supplyLen; i++) {
                ...
                address token = position.supply[i].targetToken;
                p.supply[i].token = token;
                if (token == nativeToken) {
                    p.supply[i].liquidityAvailable = 
                        add(
                            IERC20(wnativeToken).balanceOf(liquidityContract),
                            liquidityContract.balance
                        );
                } else {
                    p.supply[i].liquidityAvailable = IERC20(token).balanceOf(liquidityContract);
                }
                if (amount > p.supply[i].liquidityAvailable) p.isOk = false;
            }
        }
        ...
        if (!isTarget) {
            ...
            for (uint256 i = 0; i < withdrawLen; i++) {
                ...
                address token = position.withdraw[i].sourceToken;
                ...
                if (amount > p.withdraw[i].liquidityAvailable) p.isOk = false;
            }
        }
    }
