...
contract DUCKToken is BEP20('DUCK Token', 'DUCK') {
    ...
    */
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying DUCK (not scaled);
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of DUCK
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accDUCKPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accDUCKPerShare` (and `lastRewardTimestamp`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. DUCK to distribute per second.
        uint256 lastRewardTimestamp;  // Last timestamp that DUCK distribution occurs.
        uint256 accDUCKPerShare;   // Accumulated DUCK per share, times 1e12. See below.
    ...
    // DUCK tokens created per second.
    uint256 public DUCKPerSec;
    ...
    // The timestamp when DUCK mining starts.
    ...
    event UpdateEmissionRate(address indexed user, uint256 DUCKPerSec);
    ...
    // View function to see pending DUCK on frontend.
    function pendingDUCK(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.timestamp  > pool.lastRewardTimestamp  && pool.lpSupply != 0 && totalAllocPoint > 0) {
            ...
            uint256 DUCKReward = multiplier.mul(DUCKPerSec).mul(pool.allocPoint).div(totalAllocPoint);
            accDUCKPerShare = accDUCKPerShare.add(DUCKReward.mul(1e12).div(pool.lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accDUCKPerShare = pool.accDUCKPerShare.add(DUCKReward.mul(1e12).div(pool.lpSupply));
        pool.lastRewardTimestamp = block.timestamp;
    }
    ...
    // Safe DUCK transfer function, just in case if rounding error causes pool to not have enough DUCK.
    ...
    function updateEmissionRate(uint256 _DUCKPerSec) external onlyOwner {
        massUpdatePools();
        DUCKPerSec = _DUCKPerSec;
        emit UpdateEmissionRate(msg.sender, _DUCKPerSec);
    }
