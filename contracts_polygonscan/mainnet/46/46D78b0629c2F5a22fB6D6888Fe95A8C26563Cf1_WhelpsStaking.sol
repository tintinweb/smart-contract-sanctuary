...
contract WhelpsStaking is Ownable {
    ...
    mapping (address => uint256[]) userToStakingSetKeys;
    
    bool public manualStakingEnabled = true;
    
    function numberOfStakingPoolsForUser(address _owner) public view returns (uint256){
        return userToStakingSetKeys[_owner].length;
    }
    
    function stakingPoolKeysForUser(address _owner) public view returns (uint256[] memory) {
        return userToStakingSetKeys[_owner];
    }
    
    function stakingPoolsForUser(address _owner) public view returns (StakingSet[] memory) {
        uint256 len = numberOfStakingPoolsForUser(_owner);
        
        StakingSet[] memory result = new StakingSet[](len);
        
        for (uint256 i = 0; i < len; i++) {
            result[i] = keyToStakingSet[userToStakingSetKeys[_owner][i]];
        }
    ...
    function adminManualStaking(address _owner, uint256 _timestamp, uint256[] calldata _tokenIds, uint256 _tokenId, bool _eggClaimed, bool _tokensClaimed) public onlyOwner {
        ...
        userToStakingSetKeys[_owner].push(lastStakingSetKey);
        lastStakingSetKey++;
    }
    ...
    function stake(uint256[] calldata tokenIds) external {
        ...
        userToStakingSetKeys[msg.sender].push(lastStakingSetKey);
        lastStakingSetKey++;
    }
