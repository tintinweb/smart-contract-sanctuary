...
abstract contract BaseStrategyMasterChef is BaseStrategy {
    ...
    function getHarvestable() external override view returns (uint256) {
        return IMasterChef(rewards).pendingSushi(poolId, address(this));
    }
...
abstract contract StrategyFarmTwoAssets is BaseStrategyMasterChef {
    ...
    address public constant token2 = 0x8f3Cf7ad23Cd3CaDbD9735AFf958023239c6A063;
    ...
    uint256 public harvestCutoffMatic = 10**15;
    uint256 public constant keepFXSmax = 10000;

    // Uniswap swap paths
    address[] public sushi_matic_path;
    address[] public matic_token1_path;
    address[] public matic_token2_path;
        ...
        )
    {
        ...
        sushi_matic_path[1] = matic;

        matic_token1_path = new address[](3);
        matic_token1_path[0] = matic;
        matic_token1_path[1] = eth;
        matic_token1_path[2] = token1;

        matic_token2_path = new address[](4);
        matic_token2_path[0] = matic;
        matic_token2_path[1] = eth;
        matic_token2_path[2] = token1;
        matic_token2_path[3] = token2;
    }
    ...
    function harvest() public override {
        ...
        //Swap Sushi for matic
        uint256 _sushiBalance = IERC20(sushi).balanceOf(address(this));
        if (_sushiBalance > harvestCutoff){
            _swapUniswapWithPath(sushi_matic_path, _sushiBalance);
        }
        ...
        if (_maticBalance > harvestCutoffMatic){
            ...
            // Swap 1/2 remaining to token1 
            uint256 _maticBalancePost = IERC20(matic).balanceOf(address(this));
            _swapUniswapWithPath(matic_token1_path, _maticBalancePost.div(2));

            // Swap 1/2 remaining to token2
            uint256 _maticBalancePost2 = IERC20(matic).balanceOf(address(this));
            _swapUniswapWithPath(matic_token2_path, _maticBalancePost2);
...
contract StrategyTwoAssets is StrategyFarmTwoAssets {
    // Token addresses
    address public SUSHI_MASTER_CHEF = 0x0769fd68dFb93167989C6f7254cd0D766Fb2841F;
    address public dai_USDC_LP = 0xCD578F016888B57F1b1e3f887f392F0159E26747;
