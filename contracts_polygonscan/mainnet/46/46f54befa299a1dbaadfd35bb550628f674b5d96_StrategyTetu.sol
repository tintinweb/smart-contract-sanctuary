...
// File contracts/BIFI/interfaces/Tetu/ISmartVault.sol
...
// File contracts/BIFI/interfaces/Tetu/IXTETU.sol
...
// File contracts/BIFI/strategies/Tetu/StrategyTetu.sol

 

pragma solidity ^0.6.12;
contract StrategyTetu is StratManager, FeeManager {
    ...
    address constant public output = address(0x255707B70BF90aa112006E1b07B9AeA6De021424);  // Rewarded in xTETU but has to be swapped in TETU
    address constant public xTETU = address(0x225084D30cc297F3b177d9f93f5C3Ab8fb6a1454);
    ...
    function deposit() public whenNotPaused {
        ...
        if (wantBal > 0) {
            ISmartVault(smartVault).depositAndInvest(wantBal);
        }
    }
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            ISmartVault(smartVault).withdraw(_amount.sub(wantBal));
    ...
    function _harvest() internal {
        ISmartVault(smartVault).getAllRewards();
        uint256 xTETUBal = IERC20(xTETU).balanceOf(address(this));
        if (xTETUBal > 0) {
            IXTETU(xTETU).withdraw(xTETUBal);
            chargeFees();
            addLiquidity();
            deposit();
    ...
    function balanceOfPool() public view returns (uint256) {
        return ISmartVault(smartVault).balanceOf(address(this));
    }
    ...
    function retireStrat() external {
        ...
        ISmartVault(smartVault).withdraw(balanceOfPool());
    ...
    function _giveAllowances() internal {
        IERC20(want).safeApprove(smartVault, uint256(-1));
    ...
    function _removeAllowances() internal {
        IERC20(want).safeApprove(smartVault, 0);
    ...
    function rewardsAvailable() public view returns (uint256) {
        uint256 pendingxTETU = ISmartVault(smartVault).earnedWithBoost(xTETU, address(this));
        uint256 pendingTETU = pendingxTETU.mul(IXTETU(xTETU).getPricePerFullShare()).div(1e18);
        return pendingTETU;
    }
