...
contract VoxStakingPool is ReentrancyGuard, Pausable {
    ...
    address public treasury = address(0x0c18bC4307b45F5A9cD9DdC7F450a5D5e7658062);
    ...
    uint256 public withdrawalFee = 75; // 0.75 %
    uint256 public withdrawalFeeMax = 500;
    uint256 internal withdrawalFeeBase = 10000;

    uint256 private minimumLock = 2 weeks;
    uint256 private maximumLock = 52 weeks;

    uint256 public multiplier = 5; // 5 %
        ...
        updateReward(msg.sender)
    {
        ...
        uint256 fee = amount.mul(withdrawalFee).div(withdrawalFeeBase);
    ...
    function lock(address account, uint256 shares)
        external
        notPaused
    ...
    function unlock(address account, uint256 shares)
        external
        updateReward(account)
    {
        require(shares > 0, '!shares');
    ...
    function _deposit(address account, uint256 amount)
        internal
    {
        uint256 lockPeriod = _periods[account];
        ...
        // add already deposited amount to current deposit
        uint256 total = _deposits[account].add(amount);
        uint256 shares = 0;
    ...
    event Deposited(address indexed user, uint256 amount, uint256 lock);
    event Withdrawn(address indexed user, uint256 amount);
    event Locked(address indexed user, uint256 shares);
    event Unlocked(address indexed user, uint256 shares);
