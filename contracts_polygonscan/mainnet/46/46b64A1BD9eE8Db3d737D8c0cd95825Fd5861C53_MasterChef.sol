...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. Test to distribute per block.
        uint256 lastRewardBlock;  // Last block number that Test distribution occurs.
    ...
    uint256 constant MAX_Test_SUPPLY = 5000000 ether;

    address public devAddress;
    address public feeAddress;

    // Test tokens created per block. 1.8 Test / Block
    uint256 public Test_PER_BLOCK = 1.8 ether;
    ...
    function getMultiplier(uint256 _from, uint256 _to) public view returns (uint256) {
        if(testToken.totalSupply() >= MAX_Test_SUPPLY) return 0;
    ...
    function pendingTest(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 TestReward = multiplier.mul(Test_PER_BLOCK).mul(pool.allocPoint).div(totalAllocPoint);
            accTestPerShare = accTestPerShare.add(TestReward.mul(1e18).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 TestReward = multiplier.mul(Test_PER_BLOCK).mul(pool.allocPoint).div(totalAllocPoint);
        testToken.mint(devAddress, TestReward.div(10));
        testToken.mint(address(this), TestReward);
        pool.accTestPerShare = pool.accTestPerShare.add(TestReward.mul(1e18).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function safeTestTransfer(address _to, uint256 _amount) internal {
        uint256 TestBalance = testToken.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > TestBalance) {
            transferSuccess = testToken.transfer(_to, TestBalance);
        } else {
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        ...
        if (referrer != address(0) && referrer != _user && referralCommissionRate > 0) {
            ...
            testToken.mint(referrer, commissionAmount);
