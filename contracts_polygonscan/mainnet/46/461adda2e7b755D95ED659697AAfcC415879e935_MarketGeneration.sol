...
contract MarketGeneration is TokensRecoverable, IMarketGeneration
{
    ...
    modifier isController(){
        require(mgeController[msg.sender], "Only controller or owner can do this");
        _;
    }

    function init(IERC20 _baseToken) public ownerOnly() isController()
    {
        require (!isActive && block.timestamp >= refundsAllowedUntil, "Already activated");
        baseToken = _baseToken;
    }
    function setHardCap(uint _hardCap) public ownerOnly() isController()
    {
        require (!isActive, "Already activated");
        hardCap = _hardCap;
    }
    function setSoftCap(uint _softCap) public ownerOnly() isController()
    {
        softCap = _softCap;
    }
    function setMGEController(address _controller) public ownerOnly() isController()
    {
        mgeController[_controller] = !mgeController[_controller];
    }

    function activate(IMarketDistribution _marketDistribution, uint mgeLengthInSeconds) public ownerOnly() isController()
    {
        require (!isActive && block.timestamp >= refundsAllowedUntil, "Already activated");        
    ...
    function activateDeactivateDateLimit() public ownerOnly() isController()
    {
        dateLimitActive = !dateLimitActive;
    }
    function setMGELength(uint timeInSeconds) public ownerOnly() isController()
    {
        require (isActive);
        endDate = startDate + timeInSeconds;
    }

    function setMarketDistribution(IMarketDistribution _marketDistribution) public ownerOnly() isController() active()
    {
    ...
    function complete() public ownerOnly() isController() active()
    {
    ...
    function allowRefunds() public ownerOnly() isController() active()
    {
        isActive = false;
