...
contract Test is Ownable, PolygonArbitrageBot {
    ...
    function getBalanceWETH() view external returns(uint256)
    {
        return PolygonTokens.getBalance(kPolygonToken.weth);
    }
...
contract PolygonArbitrageBot is PolygonTradingHub {
    ...
    struct PolygonArbitrageOptionABICompatible {
        PolygonTradeForecastABICompatible aToB;
        PolygonTradeForecastABICompatible bToA;
    ...
    kPolygonToken[] private availableTokens = [
        kPolygonToken.wmatic,
        kPolygonToken.usdc,
        kPolygonToken.weth,
        kPolygonToken.pbrew,
        kPolygonToken.elon,
    ...
    constructor()
    {
        _polygonArbitrageBotConvertAddTopLevelTradingToken(kPolygonToken.wmatic);
        _polygonArbitrageBotConvertAddTopLevelTradingToken(kPolygonToken.weth);
        _polygonArbitrageBotConvertAddTopLevelTradingToken(kPolygonToken.usdc);

    }

    function _polygonArbitrageBotConvertAddTopLevelTradingToken(kPolygonToken _token) private
    {
        for (uint256 i = 0; i < availableTokens.length; i++)
        {
            kPolygonToken fiToken = availableTokens[i];
    ...
    function _polygonArbitrageBotConvertOptionToABICompatible(PolygonArbitrageOption memory _option) internal pure returns(PolygonArbitrageOptionABICompatible memory)
    {
        ...
        option.aToB = _polygonTradingHubConvertForecastToABICompatible(_option.aToB);
        option.bToA = _polygonTradingHubConvertForecastToABICompatible(_option.bToA);
    ...
    function _polygonArbitrageBotPerformSwapIfValuable(PolygonArbitrageOption memory _option, uint256 _amount) internal returns(bool)
    {
        ...
        uint256 initialBalanceA = PolygonTokens.getBalance(_option.aToB.tokenIn);
        uint256 initialBalanceB = PolygonTokens.getBalance(_option.bToA.tokenOut);

        _polygonTradingHubSwapToken(_option.aToB.exchange, _option.aToB.tokenIn, _option.aToB.tokenOut, _amount);

        uint256 newBalanceB = PolygonTokens.getBalance(_option.aToB.tokenOut);

        _polygonTradingHubSwapToken(_option.bToA.exchange, _option.bToA.tokenIn, _option.bToA.tokenOut, newBalanceB - initialBalanceB);

        uint256 newBalanceA = PolygonTokens.getBalance(_option.bToA.tokenOut);
    ...
    function _polygonArbitrageBotFindOption(kPolygonToken _tokenA, kPolygonToken _tokenB, uint256 _usdcValue) internal view returns(PolygonArbitrageOption memory)
    {
        ...
        uint256 tokenAAmount = _polygonTradingHubGetDefaultDEXTradingAmountIn(_tokenA, kPolygonToken.usdc, _usdcValue);
        ...
        if (forecastAToB.tokenOutAmount <= forecastBToA.tokenOutAmount)
        {
            option.isValid = false;
        ...
        console.log("PolygonArbitrageBot - percentageGain: %n", option.tokenAmountDifferencePercentage1000);
...
contract PolygonTradingHub is QuickSwapTrader, CafeSwapTrader, JetSwapTrader, SushiSwapTrader {
    ...
    function _polygonTradingHubGetBestTradeForecast(kPolygonToken _tokenIn, kPolygonToken _tokenOut, uint256 _amount) internal view returns(PolygonTradeForecast memory)
    {
    ...
    function _polygonTradingHubGetDefaultDEXTradingAmountIn(kPolygonToken _tokenIn, kPolygonToken _tokenOut, uint256 _amount) internal view returns(uint256)
    {
        return _quickSwapGetAmountIn(_tokenIn, _tokenOut, _amount);
    }
    ...
    function _polygonTradingHubGetForcast(kPolygonExchange _exchange, kPolygonToken _tokenIn, kPolygonToken _tokenOut, uint256 _amount) internal view returns(PolygonTradeForecast memory)
    {
        ...
        uint256 amountPer100Mille = _amount / 100000;
        uint256 tokenPer100Mille;

        if (_exchange == kPolygonExchange.quickSwap)
        {
            tokenPer100Mille = _quickSwapGetAmountOut(_tokenIn, _tokenOut, amountPer100Mille);
            forecast.tokenOutAmount = _quickSwapGetAmountOut(_tokenIn, _tokenOut, _amount);
        }
        else if (_exchange == kPolygonExchange.cafeSwap)
        {
            tokenPer100Mille = _cafeSwapGetAmountOut(_tokenIn, _tokenOut, amountPer100Mille);
            forecast.tokenOutAmount = _cafeSwapGetAmountOut(_tokenIn, _tokenOut, _amount);
        }
        else if (_exchange == kPolygonExchange.jetSwap)
        {
            tokenPer100Mille = _jetSwapGetAmountOut(_tokenIn, _tokenOut, amountPer100Mille);
            forecast.tokenOutAmount = _jetSwapGetAmountOut(_tokenIn, _tokenOut, _amount);
        }
        else if (_exchange == kPolygonExchange.sushiSwap)
        {
            tokenPer100Mille = _sushiSwapGetAmountOut(_tokenIn, _tokenOut, amountPer100Mille);
        ...
        if (tokenPer100Mille == 0 || forecast.tokenOutAmount ==0 || _calculatePercentage1000PriceImpact(tokenPer100Mille, forecast.tokenOutAmount) > MAX_PRICE_IMPACT_PERCENTAGE_1000)
        {
            // ignoring result;
...
contract QuickSwapTrader is UniSwapV2Trader {
    ...
    function _quickSwapGetAmountIn(kPolygonToken _tokenIn, kPolygonToken _tokenOut, uint256 _amount) internal view returns(uint256)
    {
        return _uniSwapV2RouterGetAmountIn(tradingRouter, _tokenIn, _tokenOut, _amount);
    }
...
contract CafeSwapTrader is UniSwapV2Trader {
    ...
    function _cafeSwapGetAmountIn(kPolygonToken _tokenIn, kPolygonToken _tokenOut, uint256 _amount) internal view returns(uint256)
    {
        return _uniSwapV2RouterGetAmountIn(tradingRouter, _tokenIn, _tokenOut, _amount);
    }
...
contract JetSwapTrader is UniSwapV2Trader {
    ...
    function _jetSwapGetAmountIn(kPolygonToken _tokenIn, kPolygonToken _tokenOut, uint256 _amount) internal view returns(uint256)
    {
        return _uniSwapV2RouterGetAmountIn(tradingRouter, _tokenIn, _tokenOut, _amount);
    }
...
contract SushiSwapTrader is UniSwapV2Trader {
    ...
    function _sushiSwapGetAmountIn(kPolygonToken _tokenIn, kPolygonToken _tokenOut, uint256 _amount) internal view returns(uint256)
    {
        return _uniSwapV2RouterGetAmountIn(tradingRouter, _tokenIn, _tokenOut, _amount);
    }
...
contract UniSwapV2Trader {
    ...
    function _uniSwapV2RouterGetAmountIn(IUniSwapV2Router02 _router, kPolygonToken _tokenIn, kPolygonToken _tokenOut, uint256 _amount) internal view returns(uint256)
    {
        return _uniSwapV2RouterGetAmountInByAddress(_router, PolygonTokens.getTokenAddress(_tokenIn), PolygonTokens.getTokenAddress(_tokenOut), _amount);
    }
    ...
    function _uniSwapV2RouterGetAmountInByAddress(IUniSwapV2Router02 _router, address _tokenInAddress, address _tokenOutAddress, uint256 _amount) private view returns(uint256)
    {
        return _uniSwapV2RouterGetAmountInByPath(_router, _createAddressPath(_tokenInAddress, _tokenOutAddress), _amount);
    }
    ...
    function _uniSwapV2RouterGetAmountInByPath(IUniSwapV2Router02 _router, address[] memory _path, uint256 _amount) private view returns(uint256)
    {
        console.log("UniSwapV2Trader - Get amount in. tokenIn: %o tokenOut: %o amount: %d", _path[0], _path[_path.length - 1], _amount);

        try _router.getAmountsIn(_amount, _path) returns(uint256[] memory _amountsIn)
        {
