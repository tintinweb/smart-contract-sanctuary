...
contract RoutingManagement is Ownable {
    ...
    */
    struct Route {
      string name;
      bool enable;
      IGBTradingRoute route;
    }

    event AddedTradingRoute(
        address indexed addedBy,
        string name,
        IGBTradingRoute indexed routingAddress,
    ...
    */
    function addTradingRoute(
        string memory _name,
        IGBTradingRoute _routingAddress
    )
      public
      onlyOwner
    {
...
contract Partnership is RoutingManagement {
    ...
    */
    ...
    constructor() public {
        Partner memory partner = Partner(msg.sender, 10, "GRAND"); // 0.1%
        partners[0] = partner;
        emit UpdatePartner(0, msg.sender, 10, "GRAND");
    }
...
// File: contracts/TheGrandBankSwap.sol
...
contract GBTokenPriviledge is Partnership {
    uint256 public eligibleAmount = 10 ether; // 10 GRAND
    IERC20 public grandToken;

    event UpdateGrandToken(IERC20 indexed token);
    event UpdateEligibleAmount(uint256 amount);

    function updateGrandToken(
        IERC20  token
    )
        public
        onlyOwner
    ...
    function isEligibleForFreeTrade(address user)
        ...
        returns (bool)
    {
        if (address(grandToken) == 0x0000000000000000000000000000000000000000) {
            return false;
        }

        return grandToken.balanceOf(user) >= eligibleAmount;
    }
    ...
    function totalHoldingToken(address user)
        ...
        returns (uint256)
    {
      return grandToken.balanceOf(user);
    }
}

contract TheGrandBankSwap is GBTokenPriviledge, WhitelistFeeOnTransfer, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    ...
    */
    ...
    {
        // Load trading route
        IGBTradingRoute tradingRoute = tradingRoutes[tradingRouteIndex].route;
        // Trade to route
