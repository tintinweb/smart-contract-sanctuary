...
// File contracts/ArtArmyStakeHolders.sol
...
contract ArtArmyStakeHolders is IArtArmyStakeHolders  {
    ...
    event EarnsDistributed(bool distributed);
    event EarnsTransferred(address indexed investor);
    ...
    */
    ...
    function getInvestorTotalEarns(address account) external view virtual returns (uint256) {
        return _investor[account].totalEarns;
    }
    ...
    function removeInvestor(address investorAddress) private returns(bool) {
        ...
        emit InvestorRemoved(investorAddress);
    ...
    function distributeEarns() public override payable returns (bool){
        ...
        // If you dont send any msg.value always add 0.
        ...
        if(contractBalance > 0){
            ...
            for (uint i = 0; i < investorsArray.length; i++) {
                uint256 earns = (msg.value * _investor[address(investorsArray[i])].amountStaked ) / contractBalance;
                _investor[address(investorsArray[i])].earns += earns;
                _investor[address(investorsArray[i])].totalEarns += earns;
            }

            emit EarnsDistributed(true);
    ...
    // Get the earns
    function getEarns() public returns (bool){

        address investorAddress = payable(msg.sender);
        uint256 contractBalance = address(this).balance;
        uint256 earns = _investor[investorAddress].earns;

        require(earns > 0, "Art Army Holders Stake: You need have some earns");

        require(contractBalance >= earns, "Art Army Holders Stake: Insufficient funds in contract");

        (bool sent, ) = investorAddress.call{value: earns}("");
        require(sent, "Art Army Sale: Failed to send currency");

        _investor[investorAddress].earns = 0;

        emit EarnsTransferred(address(this));
