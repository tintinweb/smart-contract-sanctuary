...
contract Tiger is ERC721Enumerable, Ownable, RandomlyAssigned {
  using Strings for uint256;
  
  string public baseExtension = ".json";
  uint256 public cost = 0.001 ether;
  uint256 public maxPAW = 7000;
  uint256 public maxMintAmount = 10;
  bool public paused = false;
  
  string public baseURI = "https://ipfs.io/ipfs/Qmdy7FfEbBpXF3iLdh5XdiEwsR8qPBzKYUbpnoxa6tuhSo/";

  constructor(
  ) ERC721("Tiger", "Paw")
  RandomlyAssigned(7000, 1) {}
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    require(totalSupply() + _mintAmount <= maxPAW);
    require(msg.value >= cost * _mintAmount);

    for (uint256 i = 1; i <= _mintAmount; i++) {
        uint256 mintIndex = nextToken();
     if (totalSupply() < maxPAW) {
