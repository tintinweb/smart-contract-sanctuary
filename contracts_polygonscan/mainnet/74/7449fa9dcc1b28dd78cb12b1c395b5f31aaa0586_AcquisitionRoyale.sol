    ...
    ERC721EnumerableUpgradeable
{
    ...
    function foundReserved(address _recipient)
        ...
        nonReentrant
    {
        require(_reservedCount < MAX_RESERVED, "exceeds reserved supply");
        ...
        nonReentrant
    {
        ...
        require(_freeCount < MAX_FREE, "exceeds free supply");
        if (
        ...
        nonReentrant
    {
        ...
        require(
            _auctionCount + _quantity < MAX_AUCTIONED,
            "exceeds auction supply"
        );
        if (msg.sender != owner()) {
            require(
                block.timestamp <= _foundingParameters.endTime,
    ...
    {
        _gameStartCheck();
        require(ownerOf(_enterpriseId) == msg.sender, "not enterprise owner");
        // before any RP operations, update actual balances to match virtual balances
        ...
        _weth.safeTransferFrom(
            msg.sender,
            owner(),
            (_amount * _amountToTreasury) / 1e18
        );
        ...
        nonReentrant
    {
        if (msg.sender != owner()) {
            require(
                ownerOf(_enterpriseId) == msg.sender,
    ...
    function setGameStartTime(uint256 _startTime) external override onlyOwner {
        require(_startTime > 0, "start time cannot be zero");
        if (_gameStartTime > 0) {
    ...
    function _competeUnchecked(
        ...
        _enterprises[_callerId].competes++;
    ...
    function _hostileActionCheck(
        ...
        require(
            ownerOf(_callerId) == _sender,
            "not owner of calling enterprise"
        );
