...
contract NFTPackageCoverImage is NFTPackage {
    using Strings for uint256;
    using Base64 for bytes;

    function tokenURI(uint256 tokenId) public view virtual override(NFTPackage) returns (string memory) {
        string memory _tokenURI = super.tokenURI(tokenId);
        if (0 < bytes(_tokenURI).length) return _tokenURI;

        string memory pkg = tokenPackage(tokenId);
        return string(
            abi.encodePacked(
                ...
                abi.encodePacked(
                    ...
                    tokenId.toString(),
                    '","description":"',
                    bytes(packages[pkg].desc).length == 0 ? config.metaDescription : packages[pkg].desc,
                    '","image":"',
                    packages[pkg].cover,
                    '","attributes":[',
