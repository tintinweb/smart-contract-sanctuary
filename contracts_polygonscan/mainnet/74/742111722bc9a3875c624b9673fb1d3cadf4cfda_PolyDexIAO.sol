...
contract PolyDexIAO is ReentrancyGuard, OwnableUpgradeable {
    using SafeERC20 for IERC20;

    // Info of each user.
    struct UserInfo {
        uint256 amount; // How many tokens the user has provided.
        uint256 amountInBUSD; // How many tokens the user has provided.
    ...
    uint256 public offeringWeiCap;
    // total amount of raising tokens that have already raised
    uint256 public totalRaisingInUSDC;
    uint256 public totalRaisingInBUSD;
    ...
    event LockByBUSD(address indexed user, address indexed referral, uint256 amount);
    event Claim(address indexed user, uint256 offeringWeiCap, uint256 excessAmount);
    ...
    function initialize(
        ...
        status = 0;
        offeringWeiCap = _offeringWeiCap;
        totalWeiCap = _totalWeiCap;
        totalRaisingInUSDC = 0;
        totalRaisingInBUSD = 0;
    ...
    modifier onlyActiveIAO() {
        require(status == 1, "not ido time");
        _;
    }

    modifier onlyFinishedIAO() {
        require(status == 3, "ido is not finished");
        _;
    }

    function setOfferingWeiCap(uint256 _offerAmount) public onlyOwner {
        offeringWeiCap = _offerAmount;
    }
    ...
    function lock(uint256 _amount) external {
        lockWithRef(_amount, address(0));
    }

    /// @dev Deposit ERC20 tokens with support for reflect tokens
    function lockWithRef(uint256 _amount, address _referrer) public onlyActiveIAO nonReentrant {
        require(_amount >= minStakeAmount, "_amount must >= min");
        require(userInfo[msg.sender].amount + _amount <= maxStakeAmount, "total amount must <= max");
        ...
        if (userInfo[msg.sender].amount == 0 && userInfo[msg.sender].amountInBUSD == 0) {
            addressList.push(msg.sender);
        }
        userInfo[msg.sender].amount += finalDepositAmount;
        totalRaisingInUSDC += finalDepositAmount;
        emit Lock(msg.sender, finalDepositAmount);
    }
    ...
    function lockByBUSD(
        ...
        if (userInfo[_userAddress].amount == 0 && userInfo[msg.sender].amountInBUSD == 0) {
            addressList.push(msg.sender);
        }
        userInfo[_userAddress].amountInBUSD += _amount;
        totalRaisingInBUSD += _amount;
        emit LockByBUSD(_userAddress, _referrer, _amount);
    }
    ...
    function _sendCommission(address _account, uint256 _commission) internal {
        ...
        if (_referrer != address(0)) {
            offeringToken.safeTransfer(_referrer, _commission);
            emit Commission(_account, _referrer, _commission);
        } else if (wallet != address(0)) {
            offeringToken.safeTransfer(wallet, _commission);
            emit Commission(_account, wallet, _commission);
        }
    }

    function claim() external nonReentrant onlyFinishedIAO {
        require(userInfo[msg.sender].amount > 0 || userInfo[msg.sender].amountInBUSD > 0, "have you participated?");
        require(!userInfo[msg.sender].claimed, "already claimed");
        // Refunds are only given on the first claim
        uint256 refundingTokenAmount = getUSDCRefundAmount(msg.sender);
        ...
        if (commissionPercent > 0) {
            uint256 _commission = (offeringTokenAmount * commissionPercent) / 10000;
            _sendCommission(msg.sender, _commission);
        }

        // Subtract user debt after refund on initial harvest
        emit Claim(msg.sender, offeringTokenAmount, refundingTokenAmount);
    }
    ...
    /// @param user Address of the user allocation to look up
    function getUserAllocation(address user) public view returns (uint256) {
        // avoid division by zero
        if (totalRaisingInUSDC * 1e12 + totalRaisingInBUSD == 0) {
            return 0;
        }
        ...
        // 1e4 = 1%
        // 1 = 0.0001%
        return (((userInfo[user].amount * 1e12 + userInfo[user].amountInBUSD) * 1e6) / (totalRaisingInUSDC * 1e12 + totalRaisingInBUSD));
    }
    ...
    /// @param user Address of the user allocation to look up
    // Return for bot BUSD, USDC buy
    function getOfferingAmount(address user) public view returns (uint256) {
        if (totalWeiCap <= totalRaisingInBUSD + totalRaisingInUSDC * 1e12) {
            return (offeringWeiCap * getUserAllocation(user)) / 1e6;
        } else {
            // Return an offering amount equal to a proportion of the raising amount
            return userInfo[user].amount * rate * 1e12 + userInfo[user].amountInBUSD * rate; // (/ 1e6 * rate * 1e18)
        }
    }
    ...
    function getUSDCRefundAmount(address _user) public view returns (uint256) {
        ...
        uint256 payAmount = (userInfo[_user].amount * totalWeiCap) / (totalRaisingInBUSD + totalRaisingInUSDC * 1e12);
    ...
    function userTokenStatus(address _user) public view returns (uint256 stakeTokenHarvest, uint256 offeringTokenHarvest) {
        stakeTokenHarvest = getUSDCRefundAmount(_user);
        uint256 userOffering = getOfferingAmount(_user);
        return (stakeTokenHarvest, userOffering);
    }
    ...
    function finalWithdraw(uint256 _stakeTokenAmount, uint256 _offerAmount) external onlyOwner {
    ...
    function safeTransferStakeInternal(address _to, uint256 _amount) internal {
        require(_amount <= getTotalStakeTokenBalance(), "not enough stake token");
