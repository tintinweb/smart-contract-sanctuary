...
contract EvolutionEgg is IERC721Receiver, OwnableUpgradeable, ReentrancyGuard {
    ...
    */
    ...
    // 97, 90 -> 2% epic , 8% rare:
    // 97, 9 -> 2% epic, 90 % rare
    // 67, 39 -> 30% epic, 60% rare
    uint256[6] public odds_;
    ...
    function initialize(address _nftTiny, address _nftEgg, address _statsTiny, address _statsEgg, address _factoryEgg, uint256[6] memory _odds) external initializer {
    ...
    function setOdds(uint256[6] memory _odds) external onlyOwner {
        odds_ = _odds;
    }
    ...
    function evolutionEgg(uint256 _tokenId, uint256 _userRandomHash) external nonReentrant {
        ...
        ITinyHeroStats.TinyRarity _futureTinyRarity = ITinyHeroStats.TinyRarity.COMMON; // Default Common;
        if (_rarityEgg ==  IEggStats.EggRarity.COMMON) {
            // 90% common hero, 8% rare hero , 2% epic hero
            ...
            // 8% common hero, 90% rare hero , 2% epic hero
            if (_luckyNumber > odds_[2]) {
                _futureTinyRarity = ITinyHeroStats.TinyRarity.EPIC; // Epic
            } else if (_luckyNumber > odds_[3]) {
                _futureTinyRarity = ITinyHeroStats.TinyRarity.RARE; // Rare
            }
        } else {
            // 10% common hero, 60% rare hero , 30% epic hero
            if (_luckyNumber > odds_[4]) {
                _futureTinyRarity = ITinyHeroStats.TinyRarity.EPIC; // Epic
