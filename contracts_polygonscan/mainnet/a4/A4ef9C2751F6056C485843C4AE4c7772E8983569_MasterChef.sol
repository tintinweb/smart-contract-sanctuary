...
contract StarToken is ERC20("StarToken", "STAR"), Ownable {
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 accStarPerShare;   // Accumulated STARs per share, times 1e18. See below.
    ...
    uint256 public starPerBlock = 1 ether;
    ...
    function pendingStar(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accStarPerShare = accStarPerShare.add(starReward.mul(1e18).div(lpSupply));
        }
        return user.amount.mul(accStarPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 starReward = multiplier.mul(starPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        star.mint(devAddress, starReward.div(10));
        star.mint(address(this), starReward);
        pool.accStarPerShare = pool.accStarPerShare.add(starReward.mul(1e18).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accStarPerShare).div(1e18).sub(user.rewardDebt);
            if (pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accStarPerShare).div(1e18);
    ...
    function safeStarTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > starBal) {
            transferSuccess = star.transfer(_to, starBal);
        } else {
            transferSuccess = star.transfer(_to, _amount);
        }
        require(transferSuccess, "safeStarTransfer: Transfer failed");
    }
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (address(referral) != address(0) && referralCommissionRate > 0) {
            ...
            if (referrer != address(0) && commissionAmount > 0) {
                star.mint(referrer, commissionAmount);
