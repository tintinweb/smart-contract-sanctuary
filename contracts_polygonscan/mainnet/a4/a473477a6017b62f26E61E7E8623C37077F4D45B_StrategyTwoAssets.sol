...
abstract contract BaseStrategy is Ownable {
    ...
    address public constant quickRouter = 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff; //QuickSwap router
    ...
    function _swapUniswapWithPathQuick(
        address[] memory path,
        uint256 _amount
    ) internal {
        require(path[1] != address(0));
        ...
        // Swap with uniswap
        IERC20(path[0]).safeApprove(quickRouter, 0);
        IERC20(path[0]).safeApprove(quickRouter, _amount);

        IUniswapRouterV2(quickRouter).swapExactTokensForTokens(
            _amount,
            0,
            path,
            address(this),
...
abstract contract StrategyFarmTwoAssets is BaseStrategyMasterChef {
    ...
    address public constant cgg = 0x2Ab4f9aC80F33071211729e45Cfc346C1f8446d5;
    ...
    // Uniswap swap paths
    address[] public dino_matic_path;
    address[] public matic_cgg_path;
    ...
    {
        dino_matic_path = new address[](3);
        dino_matic_path[0] = dino;
        dino_matic_path[1] = eth;
        dino_matic_path[2] = matic;

        matic_cgg_path = new address[](2);
        matic_cgg_path[0] = matic;
        matic_cgg_path[1] = cgg;
    }
    ...
    function harvest() public override {
        ...
        if (_dinoBalance > harvestCutoff){
            // Swap to Eth
            _swapUniswapWithPathQuick(dino_matic_path, _dinoBalance);
            ...
            _swapUniswapWithPath(matic_cgg_path, _maticBalancePost.div(2));

            // Add liquidity
            uint256 _matic = IERC20(matic).balanceOf(address(this));
            uint256 _cgg = IERC20(cgg).balanceOf(address(this));
            if (_matic > 0 && _cgg > 0) {
                IERC20(matic).safeApprove(currentRouter, 0);
                IERC20(matic).safeApprove(currentRouter, _matic);
                IERC20(cgg).safeApprove(currentRouter, 0);
                IERC20(cgg).safeApprove(currentRouter, _cgg);
                ...
                IERC20(cgg).safeTransfer(
                    strategist,
                    IERC20(cgg).balanceOf(address(this))
                );
            }
        }
        // Stake the LP tokens
...
contract StrategyTwoAssets is StrategyFarmTwoAssets {
    ...
    address public CGG_Matic_LP = 0xd74d23d2f23CD06a7D94f740A74c6E906F0C9005;
