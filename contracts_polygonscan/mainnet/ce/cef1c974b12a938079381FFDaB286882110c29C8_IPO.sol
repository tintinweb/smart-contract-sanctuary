...
Objectif : Lancer la presale sans avoir l'adresse du token out et en forçant la valeur
...
contract IPO 

   {
    ...
    uint public outTokenPerInToken;                   
    ...
    function claimTokens() public {
        ...
        if (inputToken6Decimal) {
            redeemtokens = userInvested[msg.sender] * outTokenPerInToken * 1000000000000;
        }
        else {
            redeemtokens = userInvested[msg.sender] * outTokenPerInToken;
        }
    ...
    function remainingClaim(address _address) public view returns (uint256) {
        ...
        if (inputToken6Decimal) {
            redeemtokens = userInvested[_address] * outTokenPerInToken * 1000000000000;
        }
        else {
    ...
    function initializeIPO(address _inputtoken, address _outputtoken, uint256 _startTime, uint256 _inTokenClaimTime, uint256 _endtime, uint256 _outTokenPerInToken, uint256 _maxinvestment, uint256 _minInvestment, bool _inputToken6Decimal, uint256 _forceTotalSupply) public onlyOwner {
        ...
        if (inputToken6Decimal) {
            IPOTarget = totalsupply/outTokenPerInToken/1000000000000;
        }
        else {
            IPOTarget = totalsupply/outTokenPerInToken;
        }        
