...
contract LootContainer is ERC721Base, ERC721Holder, Randomness, Ownable, ILootContainer {
	...
	function tokenURI(uint256 id) public view override returns (string memory) {
		...
		else if (container.class == Containers.RELIQUARY) background = "#12000c";
	...
	function mint(
		...
		id = totalSupply() + 1;
		_safeMint(to, id);
		containers[id] = Container(container, seed, uint80(block.timestamp)); // solhint-disable-line not-rely-on-time

		uint8 item = uint8(type(ILootItem.Items).min) + 1;
		while (item <= uint8(type(ILootItem.Items).max)) {
			if (isItemInside(container, ILootItem.Items(item), seed)) {
				ILootItem lootItemInterface = items[ILootItem.Items(item)];
				lootItemInterface.mint(address(this), container, seed);
				loot[id][ILootItem.Items(item)] = lootItemInterface.totalSupply();
			}
			item++;
		}

		emit ContainerMinted(id, container, seed, to);
	}
	...
	function isItemInside(
		...
		uint256 chance = (uint256(keccak256(abi.encode(randomize(randomness), container, item, totalSupply()))) % 100e18);
