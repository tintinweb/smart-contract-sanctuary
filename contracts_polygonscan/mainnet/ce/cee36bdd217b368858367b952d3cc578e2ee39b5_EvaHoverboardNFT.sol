...
// copied from ERC721/ERC721.sol
// and merged with: ERC721/extensions/ERC721Enumerable.sol
 ...
 */
contract EvaHoverboardNFT is ProxyTarget, Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable, Pausable {
    ...
    function Initialize() onlyOwner external {
        ...
        _tokenName          = "Eva Hoverboard";
        _tokenSymbol        = "HOVR";
        _baseURI            = "https://evaverse.com/api/creature_hoverboard.php?id=";
        _maxTokens          = 20000;
        _initialized        = true;
    }
    ...
    function approve(address to, uint256 tokenId) public virtual override whenNotPaused {
        address owner = EvaHoverboardNFT.ownerOf(tokenId);
    ...
    function _transfer(address from, address to, uint256 tokenId) internal virtual {
        require(EvaHoverboardNFT.ownerOf(tokenId) == from, "ERC721: transfer of token that is not own");
    ...
    function _approve(address to, uint256 tokenId) internal virtual whenNotPaused {
        ...
        emit Approval(EvaHoverboardNFT.ownerOf(tokenId), to, tokenId);
    }
    ...
    function tokenOfOwnerByIndex(address owner, uint256 index) public view virtual override returns (uint256) {
        require(index < EvaHoverboardNFT.balanceOf(owner), "ERC721Enumerable: owner index out of bounds");
    ...
    function tokenByIndex(uint256 index) public view virtual override returns (uint256) {
        require(index < EvaHoverboardNFT.totalSupply(), "ERC721Enumerable: global index out of bounds");
    ...
    function _addTokenToOwnerEnumeration(address to, uint256 tokenId) private {
        uint256 length = EvaHoverboardNFT.balanceOf(to);
    ...
    function _removeTokenFromOwnerEnumeration(address from, uint256 tokenId) private {
        ...
        uint256 lastTokenIndex = EvaHoverboardNFT.balanceOf(from) - 1;
    ...
    function SendGift(address wallet) external onlyOwner {
        require(_tokenCount + 1 < _maxTokens, "Not enough tokens remaining.");
        _safeMint(wallet);
    }
    
    function SendGifts(address[] memory wallets) external onlyOwner {
        require(_tokenCount + wallets.length < _maxTokens, "Not enough tokens remaining.");
        for(uint ii = 0; ii < wallets.length; ii++) {
            _safeMint(wallets[ii]);
        }
    }
    ...
    // This comment was written for the main Evaverse contract, but as I'm copying this function, I thought it was appropriate to copy the comment too:
