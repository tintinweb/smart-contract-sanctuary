...
contract ZeroStratCurveAave is Ownable, Pausable {
    ...
    address public constant swapToken =
    ...
    constructor(address _vault, address _unirouter) public {
    ...
    function setGasCost(uint256 _gasCost) public onlyAdmin {
        require(_gasCost <= 50, "> 50");
        gasCost = _gasCost;
    }
    ...
    function chargeCost() internal {
        ...
        uint256 wmaticGasCost = IERC20(wmatic)
            .balanceOf(address(this))
            .mul(gasCost)
            .div(1000);

        IERC20(wmatic).safeTransfer(admin, wmaticGasCost);
    }
    ...
    function _giveAllowances() internal {
        IERC20(want).safeApprove(rewards, type(uint256).max);
        IERC20(wmatic).safeApprove(unirouter, type(uint256).max);
        IERC20(crv).safeApprove(unirouter, type(uint256).max);
        IERC20(usdc).safeApprove(swapToken, type(uint256).max);
    }
