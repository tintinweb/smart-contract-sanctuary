...
abstract contract StrategyCatBase is BaseStrategyMasterChef {
    ...
    address public constant fish = 0x3a3Df212b7AA91Aa0402B9035b098891d276572B;
    ...
    // Uniswap swap paths
    address[] public matic_fish_path;
    ...
    {
        matic_fish_path = new address[](2);
        matic_fish_path[0] = fish;
        matic_fish_path[1] = matic;
    }
    ...
    function set_harvestCutoff(uint256 newCutoff) external onlyOwner() {
        require(newCutoff <= 10**18, "New cutoff must be less than 1 Fish");
    ...
    function harvest() public override {
        ...
        if(getReward) {
            // Collects FISH tokens
            _getReward();
            getReward = false;
            return;
        }

        //Swap 1/2 FISH for MATIC
        uint256 _fishBalance = IERC20(fish).balanceOf(address(this));
        if (_fishBalance > harvestCutoff){
            if (_fishBalance > 0) {
                _swapUniswapWithPath(matic_fish_path, _fishBalance.div(2));
            }
    
            // Add liquidity for FISH/MATIC
            ...
            if (_matic > 0 && _fish > 0) {
                ...
                IERC20(fish).safeApprove(currentRouter, 0);
                IERC20(fish).safeApprove(currentRouter, _fish);
                ...
                IERC20(fish).safeTransfer(
                    strategist,
                    IERC20(fish).balanceOf(address(this))
                );
            }
