...
contract Wagmipet {
    ...
    function getHunger() public view returns (uint256) {
        return hunger + ((block.number - lastFeedBlock) / 320);
    }
    
    function getUncleanliness() public view returns (uint256) {
        return uncleanliness + ((block.number - lastCleanBlock) / 320);
    }
    
    function getBoredom() public view returns (uint256) {
        return boredom + ((block.number - lastPlayBlock) / 320);
    }
    
    function getSleepiness() public view returns (uint256) {
        return sleepiness + ((block.number - lastSleepBlock) / 320);
    }
}