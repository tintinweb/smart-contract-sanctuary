...
contract NftizePool is IERC721Receiver, AccessControl {
    ...
    error MaxPoolValueI(string message);
    ...
    function depositNFT(
        ...
        if (i > poolMaxValueI) revert MaxPoolValueI({ message: "Maximum i pool value exceeded" });

        IERC721(nftContractAddress).safeTransferFrom(msg.sender, address(this), tokenId);

        nftPrices[nftContractAddress][tokenId] = basePrice + (i * basePrice);
        originalNftDepositors[nftContractAddress][tokenId] = msg.sender;
        poolNfts = poolNfts + 1;

        if (socialCauseFee > 0) {
            uint256 feeAmount = calculateFee(basePrice + (i * basePrice));
            poolToken.transfer(feeAddress, feeAmount);
            poolToken.transfer(msg.sender, basePrice - feeAmount);
        } else {
            poolToken.transfer(msg.sender, basePrice);
        }
    }
