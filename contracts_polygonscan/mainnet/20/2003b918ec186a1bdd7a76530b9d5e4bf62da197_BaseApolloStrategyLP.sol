...
contract StratManager is Ownable, Pausable {
    ...
    modifier onlyManager() {
        require(msg.sender == owner() || msg.sender == keeper, "Strat Error: Unauthorized access. Only the manager can access this.");
        _;
    }

    // verifies that the caller is not a contract.
    modifier onlyEOA() {
        require(msg.sender == tx.origin, "Strat Error: Unauthorized access. Only EOAs can access this.");
        _;
    }

    // Modifier for harvester only functions.
    modifier onlyHarvester() {
        require(msg.sender == harvester, "Strat Error: Unauthorized access. Only the harvester can access this.");
        _;
    }
    ...
    function setHarvester(address _harvester) external onlyManager {
        require(_harvester != address(0), "Strat Error: Harvester cannot be zero address");
    ...
    function setKeeper(address _keeper) external onlyManager {
        require(_keeper != address(0), "Strat Error: Keeper cannot be zero address");
        keeper = _keeper;
    ...
    function setStrategist(address _strategist) external onlyOwner {
        ...
        require(_strategist != address(0), "Strat Error: Strategist cannot be the zero address");
    ...
    function setVault(address _vault) external onlyOwner {
        require(_vault != address(0), "Strat Error: Vault cannot be the zero address");
        require(vault == address(0), "Strat Error: Vault already initialized");
        vault = _vault;
...
abstract contract FeeManager is StratManager {
    uint public STRATEGIST_FEE = 2500;
    uint constant public MAX_FEE = 2500;
...
contract BaseApolloStrategyLP is StratManager, FeeManager {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;

    // Tokens used
    address public input = 0xadbF1854e5883eB8aa7BAf50705338739e558E5b; //ETH-MATICLP
    address public output = 0x831753DD7087CaC61aB5644b308642cc1c33Dc13; //QUICK
    ...
    address constant public weth = 0x7ceB23fD6bC0adD59E62ac25578270cFf1b9f619; //WETH
    address constant public usdt = 0xc2132D05D31c914a87C6611C10748AEb04B58e8F; //USDT

    address public lpToken0;
    address public lpToken1;

    // Third party contracts
    address constant public masterchef = 0x3c1f53fed2238176419F8f897aEc8791C499e3c8; // Quickswap Staking Contract
    address constant public dragonsLair = 0xf28164A485B0B2C90639E47b0f377b4a438a16B1; // dQUICK
    ...
    address[] public matictoquickRoute; 
    ...
    constructor(
        ...
        //require(_input != _output, "Strat Error (Constructor): Input token cannot be the same as output token");
        ...
        //require(_input != lpToken0 && _input != lpToken1, "Strat Error (Constructor): Input token cannot be the same as any of the lpTokens");
        ...
        outputToLp1Route = new address[](3);
        outputToLp1Route[0]= output;
        outputToLp1Route[1]= wmatic;
        outputToLp1Route[2]= lpToken1;
        
        matictoquickRoute = new address[](2);
        matictoquickRoute[0] = wmatic;
        matictoquickRoute[1] = output;

        lpToken0DustToUsdcRoute = new address[](2);
        lpToken0DustToUsdcRoute[0] = lpToken0;
        lpToken0DustToUsdcRoute[1] = usdc;

        lpToken1DustToUsdcRoute = new address[](3);
        lpToken1DustToUsdcRoute[0] = lpToken1;
        lpToken1DustToUsdcRoute[1] = wmatic;
    ...
    function deposit(uint256 _amount) public whenNotPaused {
        require(
            ...
            "Strat Error (Deposit): Unauthorized access. Only the vault, harvester, or this contract can access this."
        );
        ...
        if (_amount > 0 && wantBal > 0) {
            ...
            IRewardPool(masterchef).stake(_amount);
        }
    }

    function withdraw(uint256 _amount) external {
        require(msg.sender == vault, "Strat Error (Withdraw): Unauthorized Access. Only the vault can access this.");

        uint256 wantBal = IERC20(input).balanceOf(address(this));

        if (wantBal < _amount) {
            //IMasterChef(masterchef).withdraw(poolId, _amount.sub(wantBal));
            IRewardPool(masterchef).withdraw(_amount.sub(wantBal));
    ...
    function beforeDeposit() external override {
        if (harvestOnDeposit) {
            ...
            msg.sender == address(this) ||
            msg.sender == harvester,
            "Strat Error (Before Deposit): Unauthorized access. Only the vault, harvester, or this contract can access this."
        );
            _harvest();
        }
    }
    ...
    function _harvest() internal whenNotPaused {
        ...
        IRewardPool(masterchef).getReward();
    ...
    function chargeFees() internal {
            uint256 toOutput = IERC20(wmatic).balanceOf(address(this));
        
            IUniswapRouterETH(unirouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(toOutput,0, matictoquickRoute, address(this), now);
        if (STRATEGIST_FEE > 0) {
    ...
    function balanceOfPool() public view returns (uint256) {
        ...
        //return _amount;
        return IRewardPool(masterchef).balanceOf(address(this));
    }

    // returns rewards unharvested
    function rewardsAvailable() public view returns (uint256) {
        uint256 lairReward = IRewardPool(masterchef).earned(address(this));
    ...
    function panic() external onlyManager {
        ...
        IRewardPool(masterchef).withdraw(balanceOfPool());
    ...
    function unpause() external onlyManager {
        require(panicState == false, "Strat Error (Unpause): Strategy is in panic mode.");
        _unpause();
        _giveAllowances();
