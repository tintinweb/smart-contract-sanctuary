...
contract TimelockRewardsDistributor is
    AccessControl,
    ReentrancyGuard
{
    ...
    address payable public BELIStakingAddress;
    address public constant WBNB_ADDRESS = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;

    uint256 public BELIStakingRewardsFactor; // X/10,000 of rewards go to BELi staking vault. The rest goes to treasury.
    ...
    event BELIStakingRewardsFactorChange(
        uint256 oldBELIStakingRewardsFactor,
        uint256 newBELIStakingRewardsFactor
    );
    ...
    constructor(
        ...
        address payable _BELIStakingAddress,
        uint256 _BELIStakingRewardsFactor,
        uint256 _minDelay,
        uint256 _minDelayReduced // address[] memory proposers, address[] memory executors
    ) public {
        devWalletAddress = _devWalletAddress;
        treasuryAddress = _treasuryAddress;
        BELIStakingAddress = _BELIStakingAddress;
        BELIStakingRewardsFactor = _BELIStakingRewardsFactor;
        minDelay = _minDelay;
    ...
    function setBELIStakingRewardsFactor(uint256 newBELIStakingRewardsFactor)
        external
        virtual
    {
        require(
        ...
        require(
            newBELIStakingRewardsFactor <= 1000,
            "newBELIStakingRewardsFactor > 1000"
        );
        emit BELIStakingRewardsFactorChange(
            BELIStakingRewardsFactor,
            newBELIStakingRewardsFactor
        );
        BELIStakingRewardsFactor = newBELIStakingRewardsFactor;
    }
    ...
    function convertTokenToWBNB(
        ...
        require(tokenStartAddress != WBNB_ADDRESS, "Cannot sell WBNB");
        require(tokenEndAddress == WBNB_ADDRESS, "Must end with WBNB");
        ...
        returns (uint256)
    {
        ...
        uint256 rewardsAmt = IERC20(WBNB_ADDRESS).balanceOf(address(this));

        if (BELIStakingRewardsFactor > 0) {
            uint256 BELIStakingRewards =
                rewardsAmt.mul(BELIStakingRewardsFactor).div(1000);
            IERC20(WBNB_ADDRESS).safeTransfer(
                BELIStakingAddress,
                BELIStakingRewards
            );
            rewardsAmt = rewardsAmt.sub(BELIStakingRewards);
        }

        IERC20(WBNB_ADDRESS).safeTransfer(treasuryAddress, rewardsAmt);
    }
    ...
    function _wrapBNB() internal virtual {
        ...
        if (bnbBal > 0) {
            IWBNB(WBNB_ADDRESS).deposit{value: bnbBal}(); // BNB -> WBNB
        }
    }
