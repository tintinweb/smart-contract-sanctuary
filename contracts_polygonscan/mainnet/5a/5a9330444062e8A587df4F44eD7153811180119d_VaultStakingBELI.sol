...
contract VaultStakingBELI is Vault {
    ...
    uint256 public minTimeToWithdraw; // 90 days
    uint256 public minTimeToWithdrawUL = 15552000; // 6 months (6 * 30 days)
...
abstract contract Vault is Ownable, ReentrancyGuard, Pausable, ERC20 {
    ...
    uint256 public profitToBeliRate = 0;
    uint256 public constant profitToBeliRateMax = 10000; // 100%
    uint256 public constant profitToBeliUL = 5000; // 50%
    ...
    uint256 public accBELIProfitPerShare = 0;
    uint256 public lastAccBELIProfit = 0;
    uint256 public accBELIProfit = 0;
    uint256 public accWithdrawnBELIProfit = 0;
    ...
    function transferConvertedProfit(address _userAddress, uint256 _amount) public onlyOwner returns (uint256) {
        uint256 balance = IERC20(BELIAddress).balanceOf(address(this));
        if (_amount > balance) {
            _amount = balance;
        }
        IERC20(BELIAddress).transfer(_userAddress, _amount);
        accWithdrawnBELIProfit = accWithdrawnBELIProfit.add(_amount);
        return _amount;
    }

    function convertProfitToBeli(uint256 _amount) internal returns (uint256) {
        uint256 convertedAmount = _amount.mul(profitToBeliRate).div(profitToBeliRateMax);
        IERC20(earnedAddress).safeApprove(uniRouterAddress, 0);
        IERC20(earnedAddress).safeIncreaseAllowance(
            uniRouterAddress,
            convertedAmount
        ...
        uint256 balance = IERC20(BELIAddress).balanceOf(address(this));
        uint256 accumulatedBalance = balance.add(accWithdrawnBELIProfit);
        if (accumulatedBalance > accBELIProfit) {
            accBELIProfit = accumulatedBalance;
        }
        uint256 reward = accBELIProfit.sub(lastAccBELIProfit);
        accBELIProfitPerShare = accBELIProfitPerShare.add(reward.mul(1e12).div(sharesTotal));
        lastAccBELIProfit = accBELIProfit;
        return _amount.sub(convertedAmount);
    }

    function mintShareToReceipt(address _userAddress, uint256 _amount) public onlyOwner {
    ...
    function earn() public virtual nonReentrant whenNotPaused {
        ...
        if (profitToBeliRate > 0) {
            earnedAmt = convertProfitToBeli(earnedAmt);
        }
    ...
    function setSettings(
        ...
        profitToBeliRate = _profitToBeli;
