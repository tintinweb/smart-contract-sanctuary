...
MM88MMM ,adPPYYba,  ,adPPYba,  ,adPPYba,   
  88    ""     `Y8 a8"     "" a8"     "8a  
  88    ,adPPPPP88 8b         8b       d8  VAULT CHEF  
...
contract VaultChef is Ownable, ReentrancyGuard, Operators {
    ...
    function _deposit(uint256 _pid, uint256 _wantAmt, address _to) internal {
        ...
        if (_wantAmt > 0) {
            require(pool.want.balanceOf(msg.sender) >= _wantAmt, 'Insufficient balance');

            uint256 balanceBefore = pool.want.balanceOf(address(this));
            pool.want.safeTransferFrom(msg.sender, address(this), _wantAmt);
            _wantAmt = pool.want.balanceOf(address(this)).sub(balanceBefore);
    ...
    function _withdraw(uint256 _pid, uint256 _wantAmt, address _to) internal {
        ...
        if (_wantAmt > 0) {
            // returns the actual amount transferred, not the shares
...
MM88MMM ,adPPYYba,  ,adPPYba,  ,adPPYba,   
  88    ""     `Y8 a8"     "" a8"     "8a  
  88    ,adPPPPP88 8b         8b       d8  OPERATORS  
  88,   88,    ,88 "8a,   ,aa "8a,   ,a8"  
  "Y888 `"8bbdP"Y8  `"Ybbd8"'  `"YbbdP"'   
