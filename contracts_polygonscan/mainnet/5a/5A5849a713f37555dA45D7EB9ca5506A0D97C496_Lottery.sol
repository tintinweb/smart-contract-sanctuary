...
contract Lottery is ReentrancyGuard, ILottery,Ownable{
    ...
    uint256 public maxPriceTicketInFarm = 50 ether;
    uint256 public minPriceTicketInFarm = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInFarm;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] farmPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInFarm;
        uint32 finalNumber;
    }
     ...
     * @param _farmTokenAddress: address of the farm token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _farmTokenAddress, address _randomGeneratorAddress) {
        ...
        nonReentrant
    {
        ...
        // Calculate number of farm to this contract
        uint256 amountFarmToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInFarm,
            _ticketNumbers.length
        );

        // Transfer farm tokens to this contract
        farmToken.safeTransferFrom(address(msg.sender), address(this), amountFarmToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInFarm += amountFarmToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInFarmToTransfer
        uint256 rewardInFarmToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInFarmToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        farmToken.safeTransfer(msg.sender, rewardInFarmToTransfer);

        emit TicketsClaim(msg.sender, rewardInFarmToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInFarm) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in farm for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].farmPerBracket[j] =
                ...
                // A. No farm to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].farmPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInFarm - amountToShareToWinners);

        // Transfer farm to treasury address
        farmToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        farmToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInFarm += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInFarm >= minPriceTicketInFarm) && (_priceTicketInFarm <= maxPriceTicketInFarm),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInFarm: _priceTicketInFarm,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            farmPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInFarm: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(farmToken), "Cannot be farm token");
    ...
    /**
     * @notice Set farm price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInFarm: minimum price of a ticket in farm
     * @param _maxPriceTicketInFarm: maximum price of a ticket in farm
     */
    function setMinAndMaxTicketPriceInFarm(uint256 _minPriceTicketInFarm, uint256 _maxPriceTicketInFarm)
        external
        onlyOwner
    {
        require(_minPriceTicketInFarm <= _maxPriceTicketInFarm, "minPrice must be < maxPrice");

        minPriceTicketInFarm = _minPriceTicketInFarm;
        maxPriceTicketInFarm = _maxPriceTicketInFarm;
    }
     ...
     * @param _priceTicket price of a ticket (in farm)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].farmPerBracket[_bracket];
        } else {
            return 0;
        }
    }
