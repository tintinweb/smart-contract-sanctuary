...
contract StrategyCurveATricrypto is StratManager, FeeManager {
    ...
    function depositLP(bool _isDepositeFee) private {
        ...
        if (wantBal > 0) {
            ...
            IRewardsGauge(rewards).deposit(IERC20(want).balanceOf(address(this)));
        }
    }
    ...
    function chargePerformanceFees() virtual internal {
        ...
        uint256 callFeeAmount = IERC20(wmatic).balanceOf(address(this)).mul(callFee).div(MAX_FEE);
        IERC20(wmatic).safeTransfer(msg.sender, callFeeAmount);

        uint256 performanceFeeAmount = IERC20(wmatic).balanceOf(address(this)).add(callFeeAmount).mul(performanceFee).div(MAX_FEE);
        swapWmaticsToWant(performanceFeeAmount);
    ...
    function addLiquidity() internal {
        ...
        swapWmaticsToWant(wmaticBal);
    }
    
    function swapWmaticsToWant(uint _amount) internal {
        swapLiquidity(_amount);
    ...
    function swapLiquidity(uint _amount) internal {
        IUniswapRouterETH(unirouter).swapExactTokensForTokens(_amount, 0, wmaticToEthRoute, address(this), now);
    }
    ...
    function _giveAllowances() internal {
        ...
        IERC20(eth).safeApprove(swap, uint256(-1));
    }
