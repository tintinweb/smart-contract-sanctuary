...
contract JaguarToken is ERC20("Jaguar Token", "JAGUAR") {
    ...
    */
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying JAGUAR (not scaled);
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of JAGUAR
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. Jaguar to distribute per block.
        uint256 lastRewardBlock;  // Last block number that Jaguar distribution occurs.
        uint256 accJaguarPerShare;   // Accumulated Jaguar per share, times 1e18. See below.
    ...
    uint256 public jaguarPerBlock = 0.004 ether;
    ...
    event UpdateEmissionRate(address indexed user, uint256 jaguarPerBlock);
    ...
    // View function to see pending Jaguar on frontend.
    function pendingJaguar(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accJaguarPerShare = accJaguarPerShare.add(jaguarReward.mul(1e18).div(lpSupply));
        }
        return user.amount.mul(accJaguarPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accJaguarPerShare = pool.accJaguarPerShare.add(jaguarReward.mul(1e18).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accJaguarPerShare).div(1e18).sub(user.rewardDebt);
            if (pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accJaguarPerShare).div(1e18);
    ...
    // Safe Jaguar transfer function, just in case if rounding error causes pool to not have enough.
    ...
    function updateEmissionRate(uint256 _jaguarPerBlock) external onlyOwner {
        massUpdatePools();
        jaguarPerBlock = _jaguarPerBlock;
        emit UpdateEmissionRate(msg.sender, _jaguarPerBlock);
    }
