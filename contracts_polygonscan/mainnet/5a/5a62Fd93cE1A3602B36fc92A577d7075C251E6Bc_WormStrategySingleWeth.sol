...
abstract contract BaseWormStrategy is Ownable, ReentrancyGuard, Pausable {
    ...
    uint256 public resupplyRate = 2500;
    uint256 public constant feeMaxTotal = 7500;
    uint256 public constant feeMax = 10000;

    uint256 public withdrawFeeFactor = 9900; // 1% Withdrawal fees
    uint256 public constant withdrawFeeFactorMax = 10000;
    uint256 public constant withdrawFeeFactorLL = 9600; // Max 4% withdrawal fee (lower limit)
    ...
    function resupplyMC(uint256 _earnedAmt) internal virtual returns (uint256) {
        if (resupplyRate > 0) {
            uint256 resupplyAmt = _earnedAmt.mul(resupplyRate).div(feeMax);
            ...
            _earnedAmt = _earnedAmt.sub(resupplyAmt);
        }
    ...
    function setSettings(
        ...
        require(_controllerFee.add(_resupplyRate) <= feeMaxTotal, "Performance Fees too high");

        controllerFee = _controllerFee;
        resupplyRate = _resupplyRate;
...
abstract contract BaseWormStrategyLPSingle is BaseWormStrategyLP {
    ...
    function leech() external override nonReentrant whenNotPaused onlyGov {
        ...
        if (earnedAmt > 0) {
            ...
            earnedAmt = resupplyMC(earnedAmt);
