...
// File contracts/strategies/dinoswap/interfaces/IMasterChef.sol
...
// File contracts/strategies/dinoswap/DinoswapLPStrategy.sol

pragma solidity 0.5.16;
/// @title Dinoswap LP Strategy
/// @author Chainvisions
/// @notice Strategy for LPs on Dinoswap.

contract DinoswapLPStrategy is IStrategy, BaseUpgradeableStrategy {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    ...
    public initializer {
        ...
        (address tkn,,,) = IMasterChef(rewardPool()).poolInfo(_pid);
        require(tkn == underlying(), "Strategy: Pool token does not match with underlying");
    ...
    */
    function _liquidateReward() internal {
        ...
        if(routes[token0].length > 1) {
            ...
            IERC20(rewardToken()).safeApprove(sushiRouter, toToken0);
            uint256[] memory amounts = IUniswapV2Router02(sushiRouter).swapExactTokensForTokens(toToken0, 0, routes[token0], address(this), block.timestamp.add(600));
        ...
        if(routes[token1].length > 1) {
            IERC20(rewardToken()).safeApprove(sushiRouter, 0);
            IERC20(rewardToken()).safeApprove(sushiRouter, toToken1);
            uint256[] memory amounts = IUniswapV2Router02(sushiRouter).swapExactTokensForTokens(toToken1, 0, routes[token1], address(this), block.timestamp.add(600));
        ...
        IUniswapV2Router02(sushiRouter).addLiquidity(token0, token1, token0Amount, token1Amount, 0, 0, address(this), block.timestamp.add(600));
    }
    ...
    */
    function withdrawAllToVault() public restricted {
        if(rewardPool() != address(0)) {
            IMasterChef(rewardPool()).withdraw(poolId(), rewardPoolStake());
        }
        _liquidateReward();
    ...
    */
    function withdrawToVault(uint256 amount) public restricted {
        ...
        if(amount > IERC20(underlying()).balanceOf(address(this))){
            ...
            IMasterChef(rewardPool()).withdraw(poolId(), Math.min(rewardPoolStake(), needToWithdraw));
        }
    ...
    */
    ...
    function setSell(bool _sell) external onlyGovernance {
        _setSell(_sell);
    }

    function setSellFloor(uint256 _sellFloor) external onlyGovernance {
        _setSellFloor(_sellFloor);
    }
...
// File contracts/strategies/dinoswap/mainnet/StrategyDino_DINO_USDC.sol

pragma solidity 0.5.16;
contract StrategyDino_DINO_USDC is DinoswapLPStrategy {

    address dino_usdc_diff;
    ...
    function initializeStrategy(
        ...
        address masterChef = address(0x1948abC5400Aa1d72223882958Da3bec643fb4E5);
        address dino = address(0xAa9654BECca45B5BDFA5ac646c939C62b527D394);
        address dino_usdc_lp = address(0x3324af8417844e70b81555A6D1568d78f4D4Bf1f);
        address usdc = address(0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174);
        DinoswapLPStrategy.initializeStrategy(
            _store,
            dino_usdc_lp,
            _vault,
            masterChef,
