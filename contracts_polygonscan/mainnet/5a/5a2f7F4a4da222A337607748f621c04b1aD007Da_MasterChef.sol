...
// Uranus with Governance.
contract Uranus is BEP20('PlanetSwap Token', 'URANUS') {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "URANUS::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "URANUS::delegateBySig: invalid nonce");
        require(now <= expiry, "URANUS::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "URANUS::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying URANUSs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "URANUS::_writeCheckpoint: block number exceeds 32 bits");
...
contract MasterChef is Ownable {
    ...
    constructor(
        ...
        // address _devaddr,
        address _feeAddress,
        uint256 _uranusPerBlock,
        uint256 _startBlock
    ) public {
        uranus = _uranus;
        // devaddr = _devaddr;
        feeAddress = _feeAddress;
        uranusPerBlock = _uranusPerBlock;
    ...
    function pendingEgg(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accEggPerShare = accEggPerShare.add(uranusReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 uranusReward = multiplier.mul(uranusPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        if(withdrawFee) {
           uranus.transfer(feeAddress, uranusReward.div(1000).mul(2));
           withdrawFee = false;
        }
        pool.accEggPerShare = pool.accEggPerShare.add(uranusReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function safeEggTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > uranusBal) {
            uranus.transfer(_to, uranusBal);
        } else {
            uranus.transfer(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _uranusPerBlock) public onlyOwner {
        massUpdatePools();
        uranusPerBlock = _uranusPerBlock;
    }
