    ...
    IGame
{
    ...
    uint128 public waitingTimeBeforeCancel;
    ...
    event SetWaitingTimeBeforeCancel(uint128 time);
    ...
    function initialize(
        ...
        // default waiting time before cancel
        waitingTimeBeforeCancel = 10;
        emit SetWaitingTimeBeforeCancel(waitingTimeBeforeCancel);
    ...
    function setWaitingTime(uint128 _waitingTime) external onlyOwner {
        require(
            _waitingTime >= minWaitingTime &&
                _waitingTime > waitingTimeBeforeCancel &&
    ...
    // setup waiting time before cancel
    function setWaitingTimeBeforeCancel(uint128 _waitingTimeBeforeCancel)
        external
        onlyOwner
    {
        require(
            _waitingTimeBeforeCancel < waitingTime,
            "CoinFlip.sol: Cancel time has to be less than expire time."
        );
        waitingTimeBeforeCancel = _waitingTimeBeforeCancel;
        emit SetWaitingTimeBeforeCancel(waitingTimeBeforeCancel);
    }
        ...
        onlyWaitingRoom(_rId)
    {
        ...
        require(
            room.expiredTime - waitingTime + waitingTimeBeforeCancel <
                block.timestamp,
            "CoinFlip.sol: Cancelled time doesn't reach."
        );
