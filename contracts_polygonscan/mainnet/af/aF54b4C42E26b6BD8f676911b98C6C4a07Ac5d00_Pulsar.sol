...
this is an experimental smart contract that uses low level call. 
Your contract logic is encoded in the front end. See the Pulsar repo.
...
contract Pulsar is FlashLoanReceiverBase {
    ...
    address dev;                 
    uint256 devFees;      // 1 Matic   
    address currentCaller;   

    event Sequenced(address user);

    modifier onlyPool() {
        require(msg.sender == lendingPoolAddr, "Illegal :alien:");
        _;
    }

    modifier onlyOwner() {
        require(msg.sender == dev, "Illegal :alien:");
        _;
    }
    ...
    function executeOperation(
        ...
        // we decode the sequence into a struct and now we have to decode it
        Sequencer.TradeSequence memory sequence = _decodeTradeSequence(params);
        // initiate the trade sequence in loop and execute each calls.
        for (uint256 tid = 0; tid < sequence.target.length; ++tid) {
            // we expect the call to pass or revert
           _executeCall(sequence.target[tid],sequence.callData[tid]);
        }
        // after all the trades are executed we must repay the loans by approving all neccesary funds
        // asset's in the loand sequence must match asset's in the withdraws.
        // Withdraws asset array can be greather than the loans one to withdraw any extra token recieves in your
        for (uint256 lid = 0; lid < assets.length; ++lid) {
            require(assets[lid] == sequence.withdraw[lid], "Asset's must match");
            IERC20(assets[lid]).safeApprove(lendingPoolAddr,amounts[lid] + premiums[lid]);
            payback(sequence.withdraw[lid], amounts[lid] + premiums[lid]);

        }
        // if the withdraw[] was longuer we withdraw the other asset's
        if(sequence.withdraw.length > assets.length){
            for(uint256 wid = assets.length; wid < sequence.withdraw.length; ++wid) {
                 payback(sequence.withdraw[wid], 0);
            }
        }
        
        // we return all the asset balance - the aave loan and premiums
    ...
    function flashLoanCall(
        ...
        require(msg.value == devFees,"1 Matic required");
        ILendingPool _lendingPool = ILendingPool(lendingPoolAddr);
        // your loan sequence
        Sequencer.LoanSequence memory loans = loanData;
        // pay the dev
        payable(dev).transfer(devFees);
        // we send funds back to current caller at the end of the sequence
        currentCaller = msg.sender;
        // makes the call to the lending lool for a loan
    ...
    {
        (address[] memory target, bytes[] memory callData, address[] memory withdraw) = abi.decode(
            params,
    ...
    {
        (address[] memory assets, uint256[] memory amounts, uint[] memory modes) = abi.decode(
            params,
    ...
    function payback(address _asset, uint256 _premiums) internal {
        if(_asset == address(0)){
            if((address(this).balance - _premiums)> 0){
                payable(currentCaller).transfer(address(this).balance - _premiums);
            }  
        }else{
            if( (IERC20(_asset).balanceOf(address(this)) - _premiums ) > 0){
                 IERC20(_asset).transfer(currentCaller,IERC20(_asset).balanceOf(address(this)) - _premiums);
            }
        }  
    }
