...
abstract contract StrategyFarmBase is BaseStrategyMasterChef {
    ...
    // Token addresses for Ball/USDC
    address public constant collar = 0x8DF26a1BD9bD98e2eC506fc9d8009954716A05DC;
    address public constant token1 = 0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174;
    ...
    // Uniswap swap paths
    address[] public collar_token1_path;
    ...
    {
        collar_token1_path = new address[](2);
        collar_token1_path[0] = collar;
        collar_token1_path[1] = token1;
    }
    ...
    function harvest() public override {
        ...
        // Collects Collar tokens
        _getReward();

        uint256 _collarBalance = IERC20(collar).balanceOf(address(this));
        if (_collarBalance > harvestCutoff){

            //Send performance fee to strategist
            uint256 performanceFee = _collarBalance.mul(keepFXS).div(keepFXSmax);
            if (performanceFee>0) {
                _swapUniswapWithPath(collar_token1_path, performanceFee);
            ...
            // Swap half of remaining to other
            uint256 _collarBalancePost = IERC20(collar).balanceOf(address(this));
            _swapUniswapWithPath(collar_token1_path, _collarBalancePost.div(2));

            // Add liquidity for Ball/USDC
            uint256 _collar = IERC20(collar).balanceOf(address(this));
            uint256 _token1 = IERC20(token1).balanceOf(address(this));
            if (_collar > 0 && _token1 > 0) {
                IERC20(collar).safeApprove(currentRouter, 0);
                IERC20(collar).safeApprove(currentRouter, _collar);
                IERC20(token1).safeApprove(currentRouter, 0);
                IERC20(token1).safeApprove(currentRouter, _token1);
                ...
                // Donates DUST
                IERC20(collar).safeTransfer(
                    strategist,
                    IERC20(collar).balanceOf(address(this))
                );
                IERC20(token1).safeTransfer(
                    strategist,
                    IERC20(token1).balanceOf(address(this))
...
contract StrategyBase is StrategyFarmBase {
    // Token addresses
    address public COLLAR_MASTER_CHEF = 0xDAbDF071635Df85E87CB5b531C4Db448a0f0cE21;
    address public COLLAR_USDC_LP = 0x33e05daCdd6254FDCcadAB11cCf69a2e877DBc7d;
