...
interface IKommunitasFactory{
    ...
    function createProject(address, uint, uint, uint, uint, uint, uint[3] memory, uint[2] memory, uint) external returns (address);
...
contract KommunitasProject{
    ...
    function buyTokenByETH(address[] calldata _path) payable isBoosterProgress isNotPaused public {
        ...
        require(getUserAllocToken(msg.sender) > 0 && v2Staked > 0, "Staked KOM not found");
    ...
    function saveMe(uint _tokenAmount) isBoosterProgress isNotPaused public {
        ...
        require(boosterProgress() == 3, "Not now savior");
    ...
    function setV2Staked() public {
        require(block.timestamp >= calculation && v2Staked == 0, "Not now");
    ...
    function setMinMax_d2(uint[2] memory _minMaxPublicBuy) public onlyOwner{
        require(boosterProgress() != 3, "FCFS started");
