...
contract DEIProxy is Ownable {
	...
	uint public collateral_missing_decimals_d18 = 1e12;  // missing decimal of collateral token (USDC)
	...
	function USDC2DEI(
	...
	) external returns (uint deiAmount) {
		...
		(, uint deusNeeded)	= calcMintFractionalDEI(MintFD_Params(proxyInput.deusPriceUSD, proxyInput.colPriceUSD, proxyInput.amountIn, globalCollateralRatio));

		// buy needed deus
		uint deiNeededAmount = IUniswapV2Router02(uniswapRouter).getAmountsIn(deusNeeded, dei2deusPath)[0];
		ISSP(sspAddress).swapUsdcForExactDei(deiNeededAmount);
		IUniswapV2Router02(uniswapRouter).swapTokensForExactTokens(deusNeeded, deiNeededAmount, dei2deusPath, address(this), deadline);

		deiAmount = IDEIPool(minterPool).mintFractionalDEI(proxyInput.usdcForMintAmount, deusNeeded, proxyInput.colPriceUSD, proxyInput.deusPriceUSD, proxyInput.expireBlock, proxyInput.sigs);

		IERC20(deiAddress).transfer(msg.sender, deiAmount);

		require(proxyInput.minAmountOut >= deiAmount, "Minter Proxy: Insufficient output amount");
	...
	function getUSDC2DEIInputs(uint amountIn, uint deusPriceUSD, uint colPriceUSD) public view returns (uint amountOut, uint usdcForMintAmount) {
		uint deusNeededAmount;
		uint globalCollateralRatio = IDEIStablecoin(deiAddress).global_collateral_ratio();
		(amountOut, deusNeededAmount) = calcMintFractionalDEI(MintFD_Params(deusPriceUSD, colPriceUSD, amountIn, globalCollateralRatio));
		...
		usdcForMintAmount =  usdcNeededAmount * globalCollateralRatio / 1e6 - globalCollateralRatio;
		uint mintingFee = IDEIPool(minterPool).minting_fee();
		amountOut -= amountOut * mintingFee / 1e6;
	}
