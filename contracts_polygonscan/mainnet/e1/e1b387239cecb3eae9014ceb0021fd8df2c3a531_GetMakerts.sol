...
contract GetMakerts is Ownable {
    ...
    function getMarketDetail() public view returns (uint256 [1][] memory){
        address[] memory _PTokens = getAllPTokens();
        uint256 [1][] memory data = new uint256[1][](0);

        for(uint256 i = 0 ; i < _PTokens.length; i++){
            data[0][i] = getBorrowApy(_PTokens[i]);
            //data.pTokenAddress[i] = _PTokens[i];
            //data.underlyingAddress[i] = getUnderlyingTokenAddress(_PTokens[i]);
            //data.symbol[i] = getSymbol(getUnderlyingTokenAddress(_PTokens[i]));
            /* data[0][i] = getSupplyApy(_PTokens[i]);
            data[1][i] = getBorrowApy(_PTokens[i]);
            data[2][i] = getAllowance(getUnderlyingTokenAddress(_PTokens[i]), _account, _PTokens[i]);
            data[3][i] = getBalanceOf(getUnderlyingTokenAddress(_PTokens[i]), _PTokens[i]);
            data[4][i] = getMarketTotalSupplyInTokenUnit(_PTokens[i]);
            data[5][i] = getMarketTotalBorrowInTokenUnit(_PTokens[i]);
            data[6][i] = getUnderlyingAmount(_PTokens[i]);
            data[7][i] = getUnderlyingPrice(_PTokens[i]);
            data[8][i] = getCollateralFactor(_PTokens[i]);
            data[9][i] = getPctSpeed(_PTokens[i]);
            data[10][i] = getDecimals(getUnderlyingTokenAddress(_PTokens[i])); */
