...
contract FarmLockTier is Ownable, ReentrancyGuard  {
    ...
    struct PoolInfo {
        ...
        uint256 depositAmount; // The sum of all deposited tokens from users
    }
    ...
    function emergencyWithdraw() external nonReentrant {
        ...
        pool.depositAmount = pool.depositAmount.sub(_availableAmount);
    ...
    function emergencyRewardWithdraw(uint256 _amount) external onlyOwner {
        PoolInfo storage pool = poolInfo;
        require(_amount <= (rewardToken.balanceOf(address(this)).sub(pool.depositAmount)), 'emergencyRewardWithdraw: not enough token');
