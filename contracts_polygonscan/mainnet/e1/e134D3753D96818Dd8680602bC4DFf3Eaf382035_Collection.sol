...
interface IFarmV2 {
    function rewardedStones(address) external view returns (uint256);

    function payment(address, uint256) external returns (bool);
...
contract Collection is ChildERC1155 {
    event Minted(
        address indexed operator,
        address indexed to,
        uint256 indexed id,
    ...
    ) ChildERC1155(uri, childChainManager) {
        amount = _amount;
        available = _total;
        total = _total;
    ...
    function recoverToken(address _token) external only(DEFAULT_ADMIN_ROLE) {
        uint256 amount = IERC20(_token).balanceOf(address(this));
        IERC20(_token).transfer(msg.sender, amount);
    }

    function mint(address to, uint256 _id) external only(DEFAULT_ADMIN_ROLE) {
        ...
        returns (uint256)
    {
        ...
        uint256 sellAmount = sellmul / 10**18;
        return sellAmount;
    }

    function setStarTime(uint256 _starTime) external only(DEFAULT_ADMIN_ROLE) {
        startTime = _starTime;
    }

    function setEndTime(uint256 _endTime) external only(DEFAULT_ADMIN_ROLE) {
        endTime = _endTime;
    }
