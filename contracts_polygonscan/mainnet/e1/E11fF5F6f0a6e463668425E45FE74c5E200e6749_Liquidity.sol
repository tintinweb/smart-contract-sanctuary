...
contract Liquidity is Ownable {
    ...
    constructor(
      ...
      DESIRE = IERC20(_DESIRE);
      _allowance();
    }

  receive() external payable {}

  function withdrawDesireLiquidity() external onlyOwner {
      USDC.transfer(msg.sender, USDC.balanceOf(address(this)));
      DESIRE.transfer(msg.sender, DESIRE.balanceOf(address(this)));
  }
 ...
 function withdrawAll()  external onlyOwner {
  ...
  function AutoLiquidity() external onlyOwner {
    ...
    uint256 desireBalance = DESIRE.balanceOf(address(this));
    uint256 halfTokenAmount = desireBalance / 2;

    if (usdcBalanceAfter > 0 && wmaticBalance > 0 && desireBalance > 0) {
        // add the liquidity desire-matic
  ...
  function _allowance() internal {
    ...
    DESIRE.approve(address(ROUTER), type(uint256).max);
  }
