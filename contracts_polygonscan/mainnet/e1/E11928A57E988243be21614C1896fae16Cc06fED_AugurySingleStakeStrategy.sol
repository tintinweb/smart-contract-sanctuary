...
contract AugurySingleStakeStrategy is BaseStrategyMasterChef {
        ...
        )
    {
        ...
        reward_wmatic_path = new address[](3);
        reward_wmatic_path[0] = OMEN;
        reward_wmatic_path[1] = USDC;
        reward_wmatic_path[2] = WMATIC;
    ...
    //Swap feeAmount of harvestedToken through harvestedToken -> USDC -> WMATIC before sending it to the fee dist, since profit is calculated in terms of WMATIC
    ...
    */
    function collectDividends(address _dividends) public onlyOwner {
        IDividends(_dividends).collectDividends();
    }

    //Convert USDC dividends to OMEN (no performance fee taken)
    function compoundDividends() public onlyOwner {
        uint256 _usdc = IERC20(USDC).balanceOf(address(this));
        if (_usdc > 0) {
            _swapUniswapWithPath(usdc_omen_path, _usdc);
        }
    }
