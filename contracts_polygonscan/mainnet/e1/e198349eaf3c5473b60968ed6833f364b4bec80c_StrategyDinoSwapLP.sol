...
contract StrategyDinoSwapLP is StratManager, FeeManager {
    ...
    function addLiquidity() internal {
        ...
        if (lpToken0 != output) {
            IUniswapRouterETH(sushiRouter).swapExactTokensForTokens(outputHalf, 0, outputToLp0Route, address(this), now);
        }

        if (lpToken1 != output) {
            IUniswapRouterETH(sushiRouter).swapExactTokensForTokens(outputHalf, 0, outputToLp1Route, address(this), now);
        }
