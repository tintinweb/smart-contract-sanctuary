...
contract TestERC721 is ERC721EnumerableUpgradeable, OwnableUpgradeable {
        ...
        return
            string(
                bytes(
                    abi.encodePacked(
                        '"attributes": [',
                        '{ "trait_type": "Function", "value": "Stake" },',
                        ...
                        '"}, { "display_type": "number", "trait_type": "amount", "value": "',
    ...
    function getDates(StakeUri memory stake) internal pure returns (string memory) {
        return
            string(
                bytes(
                    abi.encodePacked(
                        '{ "display_type": "number", "trait_type": "Duration", "value": "',
                        uint256(stake.stakingDays).toString(),
                        '"}, { "display_type": "date", "trait_type": "Start Date", "value": "',
                        uint256(stake.start).toString(),
                        '"}, { "display_type": "date", "trait_type": "End Date", "value": "',
                        (uint256(stake.start) + uint256(stake.stakingDays) * 86400).toString(),
                        '"}'
                    )
                )
            );
