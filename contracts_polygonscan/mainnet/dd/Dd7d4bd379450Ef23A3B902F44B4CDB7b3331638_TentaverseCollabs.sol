...
import "./Blimpie/Matic/NativeMetaTransaction.sol";
...
contract TentaverseCollabs is ERC1155, NativeMetaTransaction, Delegated {
  struct Token{
    uint burnPrice;
    uint mintPrice;
    uint balance;
  ...
  function setActive(uint id, bool isBurnActive, bool isMintActive) public onlyDelegates{
    require( exists( id ), "ERC1155: Specified token (id) does not exist" );
    require( tokens[id].isBurnActive != isBurnActive || tokens[id].isMintActive != isMintActive, "ERC1155: New value matches old" );
  ...
  function setPrice(uint id, uint burnPrice, uint mintPrice) public onlyDelegates{
    require( exists( id ), "ERC1155: Specified token (id) does not exist" );
    require( tokens[id].burnPrice != burnPrice || tokens[id].mintPrice != mintPrice, "ERC1155: New value matches old" );
    tokens[id].burnPrice = burnPrice;
    tokens[id].mintPrice = mintPrice;
  }

  function setProxy(address proxy) external onlyDelegates{
    require( proxyRegistryAddress != proxy, "ERC1155: New value matches old" );
  ...
  //internal
  //from '@maticnetwork/pos-portal/contracts/common/ContextMixin.sol';
