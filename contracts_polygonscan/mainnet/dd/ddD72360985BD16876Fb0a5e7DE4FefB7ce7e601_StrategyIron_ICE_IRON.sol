...
import "../IronLPStrategy.sol";

contract StrategyIron_ICE_IRON is IronLPStrategy {
    
    address ice_iron_diff;

    function initializeStrategy(
        address _store,
        address _vault
    ) public initializer {
        address ironChef = address(0x1fD1259Fa8CdC60c6E8C86cfA592CA1b8403DFaD);
        address ice_iron_lp = address(0x7e2cc09d3d36b3aF6EdfF55B214fd62885234E95);
        address ice = address(0x4A81f8796e0c6Ad4877A51C86693B0dE8093F2ef);
        address iron = address(0xD86b5923F3AD7b585eD81B448170ae026c65ae9a);
        __Strategy_init(
            _store,
            ice_iron_lp,
            _vault,
...
/// @title Iron LP Strategy
/// @author Chainvisions
/// @notice Strategy for Iron.finance LPs (Not IS3USD).

contract IronLPStrategy is BaseUpgradeableStrategy {
    ...
    public initializer {
        ...
        require(IMasterChefV2(rewardPool()).lpToken(_pid) == underlying(), "IronLPStrategy: Pool does not match with underlying");
        _setPoolId(_pid);
    }
    ...
    */
    function withdrawAllToVault() public restricted {
        if(rewardPool() != address(0)) {
            IMasterChefV2(rewardPool()).withdrawAndHarvest(poolId(), _rewardPoolStake(), address(this));
        }
        _liquidateReward();
    ...
    */
    function withdrawToVault(uint256 amount) public restricted {
        ...
        if(amount > IERC20(underlying()).balanceOf(address(this))){
            ...
            IMasterChefV2(rewardPool()).withdraw(poolId(), Math.min(_rewardPoolStake(), needToWithdraw), address(this));
        }
    ...
    */
    ...
    function _investAllUnderlying() internal onlyNotPausedInvesting {
        ...
        if(underlyingBalance > 0) {
            ...
            IMasterChefV2(rewardPool()).deposit(poolId(), underlyingBalance, address(this));
        }
    }
    ...
    */
    function _liquidateReward() internal {
        ...
        // Saves gas.
        address[] memory routeToToken0 = routes[token0];
        address[] memory routeToToken1 = routes[token1];

        if(routeToToken0.length > 1) {
            IERC20(rewardToken()).safeApprove(dfynRouter, 0);
            IERC20(rewardToken()).safeApprove(dfynRouter, toToken0);
            uint256[] memory amounts = IUniswapV2Router02(dfynRouter).swapExactTokensForTokens(toToken0, 0, routeToToken0, address(this), (block.timestamp + 600));
            token0Amount = amounts[(amounts.length - 1)];
        } else {
            token0Amount = toToken0;
        }

        if(routeToToken1.length > 1) {
            IERC20(rewardToken()).safeApprove(dfynRouter, 0);
            IERC20(rewardToken()).safeApprove(dfynRouter, toToken1);
            uint256[] memory amounts = IUniswapV2Router02(dfynRouter).swapExactTokensForTokens(toToken1, 0, routeToToken1, address(this), (block.timestamp + 600));
            token1Amount = amounts[(amounts.length - 1)];
        } else {
            token1Amount = toToken1;
        }

        IERC20(token0).safeApprove(dfynRouter, 0);
        IERC20(token0).safeApprove(dfynRouter, token0Amount);

        IERC20(token1).safeApprove(dfynRouter, 0);
        IERC20(token1).safeApprove(dfynRouter, token1Amount);

        IUniswapV2Router02(dfynRouter).addLiquidity(token0, token1, token0Amount, token1Amount, 0, 0, address(this), (block.timestamp + 600));
    }
    
    function _setPoolId(uint256 _value) internal {
        _setUint256("poolId", _value);
    }

    function _rewardPoolStake() internal view returns (uint256 stake) {
        (stake, ) = IMasterChefV2(rewardPool()).userInfo(poolId(), address(this));
    }
}
...
interface IMasterChefV2 {
    ...
    function lpToken(uint) external view returns (address);
