...
contract VaultFrankenstein is ERC20, Ownable {
    ...
    function updateTotalCapital() public {
    	...
    	totalCapital = 0;
        for (uint256 i; i < names.length; i++) {
            IStrategy(strategies[names[i]]).updateTotalCapital();
    ...
    function _requestYelFromStrategies(address _reciever, uint256 _percent) internal {
        for (uint256 i; i < names.length; i++) {
            IStrategy(strategies[names[i]]).withdrawFromStrategy(_reciever, _percent);
        }
    }
    ...
    function deposit() public payable notBlocked {
        autoCompound();
        require(names.length != 0, "There is no strategy");
        payable(getCurrentStrategy()).transfer(msg.value);
        updateTotalCapital();
