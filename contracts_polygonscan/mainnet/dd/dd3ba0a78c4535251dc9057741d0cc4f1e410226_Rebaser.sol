...
contract Rebaser is WarpBase, KeeperCompatibleInterface {
    using EnumerableSetUpgradeable for EnumerableSetUpgradeable.AddressSet;

    event Rebased(address account, uint256 time, uint256 numberOfRebases);

    EnumerableSetUpgradeable.AddressSet private recipients;
    uint256 rebaseTime;
    uint256 timeBetweenRebases;

    /** @notice initialize */
    function initialize(uint256 _rebaseTime, uint256 _timeBetween) public initializer {
        rebaseTime = _rebaseTime;
        timeBetweenRebases = _timeBetween;
    ...
    function rebase() public {
        for (uint256 i = 0; i < recipients.length(); i++) {
            IStaking(recipients.at(i)).rebase();
        }

        emit Rebased(msg.sender, block.timestamp, recipients.length());
    }

    function addRecipient(address _address) external onlyOwner {
        require(!recipients.contains(_address), 'Recipients already contains');
        recipients.add(_address);
    }

    function removeRecipient(address _address) external onlyOwner {
        require(recipients.contains(_address), 'Recipients does not exist');
        recipients.remove(_address);
    }
    ...
    function setTimeBetweenRebases(uint256 _timeBetweenRebases) external onlyOwner {
        timeBetweenRebases = _timeBetweenRebases;
    }
        ...
        )
    {
        ...
        if (rebaseTime < block.timestamp + timeBetweenRebases + 60) {
            upkeepNeeded = true;
        }
    ...
    function performUpkeep(
        ...
        rebaseTime = rebaseTime + timeBetweenRebases;
        rebase();
    }
}
