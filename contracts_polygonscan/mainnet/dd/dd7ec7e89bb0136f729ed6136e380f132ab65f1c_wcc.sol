...
/**
WorldCryptoChart: $wcc
-Multichain farm for all- deposit LP tokens or designated tokens and earn big using $WCC Benefit to $wcc holders.

Tokenomics:
3% of tax goes to existing holders.
15% of tax goes into multi-chain farming to add to the treasury and buy back wcc tokens.



Website:
https://WorldCryptoChart.com

Twitter: 
https://twitter.com/wcryptochart

Medium: https://medium.com/@wcryptocharts
...
contract wcc is Context, IERC20, Ownable {
    ...
    string private constant _name = "WorldCryptoChart";
    ...
    address payable public _wccAddress = payable(0x1a6DA2Dc8aA0f3a6B17f407bBEF6be0871b72f4C); //this is the multichain farming wallet
    address payable public _mktgAddress = payable(0x2ff92b34b459803AB32718c1670dFfA983689A16); //token support wallet
    ...
    event UpdatedwccAddress(address mgc); //investment wallet
    ...
    function sendETHToFee(uint256 amount) private {
        _wccAddress.transfer(amount.div(2));
        _mktgAddress.transfer(amount.div(2));
    }

    function manualSwap() external {
        require(_msgSender() == _wccAddress);
        uint256 contractBalance = balanceOf(address(this));
        swapTokensForEth(contractBalance);
    }

    function manualSend() external {
        require(_msgSender() == _wccAddress || _msgSender() == _mktgAddress);
        uint256 contractETHBalance = address(this).balance;
    ...
    function setMaxWalletSize(uint256 maxWalletSize) public onlyOwner {
     ...
     //set wallet address
    function _setwccAddress(address wccAddress) external onlyOwner {
        require(_wccAddress != address(0), "_wccAddress cannot be 0");
        _isExcludedFromFee[wccAddress] = false;
        wccAddress = payable(_wccAddress);
        _isExcludedFromFee[wccAddress] = true;
        emit UpdatedwccAddress(_wccAddress);
    }
