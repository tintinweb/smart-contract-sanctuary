...
/******************************************/
/*        EternalPool starts here         */
/******************************************/

pragma solidity 0.6.12;

contract EternalPool is Ownable {
    using SafeMath for uint256;
    ...
    IERC20 public immutable DINO;                   // Dino token.
    ...
    uint256 public stakedDinos;                     // Total Dino tokens staked in the contract.
    ...
    constructor(IERC20 _DINO, IERC20 _REWARD, uint256 _rewardPerBlock, uint256 _startBlock, uint256 _endBlock) public {
    ...
    function withdraw(uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(poolInfo.accRewardPerShare).div(1e12);
        DINO.safeTransfer(address(msg.sender), _amount);
        stakedDinos = stakedDinos.sub(_amount);
     ...
     */
    function setDinoPerBlock(uint256 _rewardPerBLock) external onlyOwner returns (uint256) {
        require(_rewardPerBLock > 0, "Dino per Block must be greater than 0.");
        updatePool();
        rewardPerBlock = _rewardPerBLock;
