...
abstract contract StrategyFeesBase is Ownable, ReentrancyGuard, Pausable {
    ...
    uint256 public wmaticSlippageFactor = 800; // 10% default wmatic slippage tolerance
    ...
    function _safeSwap(
        ...
        if(_path[0] == wmaticAddress){
            _safeSwapFromWmatic(_amountIn, _path, _to);
            return;
        }
        if(_path[_path.length.sub(1)] == wmaticAddress){
            _safeSwapToWmatic(_amountIn, _path, _to);
            return;
        }
    ...
    function _safeSwapFromWmatic(
        ...
        IUniRouter02(uniRouterAddress).swapExactETHForTokens(
            amountOut.mul(wmaticSlippageFactor).div(1000),
            _path,
            _to,
            now.add(600)
...
abstract contract StrategyLPBase is StrategyBase {
    ...
    constructor(
        ...
        if(token0Address == wmaticAddress){
            isWmaticPair=true;
            token = token1Address;
        }

        if(token1Address == wmaticAddress){
            isWmaticPair=true;
            token = token0Address;
        }
    }
    ...
    function earnedToWant() internal virtual override{
        ...
        if(earnedAmt > 0){
            ...
            if (token0Amt > 0 && token1Amt > 0) {
                ...
                if(isWmaticPair){
                    uint256 tokenAmt = token == token0Address? token0Amt: token1Amt;
                    uint256 ethAmt = token == token0Address? token1Amt: token0Amt;

                    IUniRouter02(uniRouterAddress).addLiquidityETH(
                        token,
                        tokenAmt,
                        tokenAmt.mul(liquiditySlippageFactor).div(1000),
                        ethAmt.mul(liquiditySlippageFactor).div(1000),
                        address(this),
                        now.add(600)
                    );
                }
