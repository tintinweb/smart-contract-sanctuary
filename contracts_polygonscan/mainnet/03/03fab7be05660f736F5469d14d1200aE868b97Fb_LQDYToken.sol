...
* 
*
*  --- Functionality added specific to the LQDYToken ---
* 
* 1) Transfer protection: blacklist of addresses that are invalid recipients (i.e. core Liquity contracts) in external 
* transfer() and transferFrom() calls. The purpose is to protect users from losing tokens by mistakenly sending LQDY directly to a Liquity
* core contract, when they should rather call the right function.
*
* 2) sendToLQDYStaking(): callable only by Liquity core contracts, which move LQDY tokens from user -> LQDYStaking contract.
*
...
* -sendToLQDYStaking() reverts when the multisig is the sender, blocking the multisig from staking its LQDY.
* 
* After one year has passed since deployment of the LQDYToken, the restrictions on multisig operations are lifted
* and the multisig has the same rights as any other address.
*/

contract LQDYToken is CheckContract, ILQDYToken {
    using SafeMath for uint256;

    // --- ERC20 Data ---

    string constant internal _NAME = "LQDY";
    string constant internal _SYMBOL = "LQDY";
    ...
    address public immutable lqdyStakingAddress;
        ...
        public 
    {
        ...
        lqdyStakingAddress = _lqdyStakingAddress;
        ...
        // Allocate the remainder to the LQDY Multisig: (100 - 2 - 32 - 1.33) million = 64.66 million
    ...
    function sendToLQDYStaking(address _sender, uint256 _amount) external override {
        _requireCallerIsLQDYStaking();
        if (_isFirstYear()) { _requireSenderIsNotMultisig(_sender); }  // Prevent the multisig from staking LQDY
        _transfer(_sender, lqdyStakingAddress, _amount);
    }
    ...
    (
        ...
        override 
    {            
        require(deadline >= now, 'LQDY: expired deadline');
        bytes32 digest = keccak256(abi.encodePacked('\x19\x01', 
                         domainSeparator(), keccak256(abi.encode(
                         _PERMIT_TYPEHASH, owner, spender, amount, 
                         _nonces[owner]++, deadline))));
        address recoveredAddress = ecrecover(digest, v, r, s);
        require(recoveredAddress == owner, 'LQDY: invalid signature');
    ...
    function _requireValidRecipient(address _recipient) internal view {
        require(
            ...
            "LQDY: Cannot transfer tokens directly to the LQDY token contract or the zero address"
        );
        require(
            _recipient != communityIssuanceAddress &&
            _recipient != lqdyStakingAddress,
            "LQDY: Cannot transfer tokens directly to the community issuance or staking contract"
        );
    }
    ...
    function _requireRecipientIsRegisteredLC(address _recipient) internal view {
        ...
        "LQDYToken: recipient must be a LockupContract registered in the Factory");
    }

    function _requireSenderIsNotMultisig(address _sender) internal view {
        require(_sender != multisigAddress, "LQDYToken: sender must not be the multisig");
    }

    function _requireCallerIsNotMultisig() internal view {
        require(!_callerIsMultisig(), "LQDYToken: caller must not be the multisig");
    }

    function _requireCallerIsLQDYStaking() internal view {
         require(msg.sender == lqdyStakingAddress, "LQDYToken: caller must be the LQDYStaking contract");
    }
