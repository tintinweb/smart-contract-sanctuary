...
contract StakeManager is IStakeManager, StakeBase {
    ...
    */
    ...
    {
        uint256 end =
            (stakeData[stakeId].stakingDays * settings.secondsInDay) + stakeData[stakeId].start;
        require(stakeData[stakeId].status == StakeStatus.Active, 'MANAGER: Stake not active');
        require(end > block.timestamp, 'SplitStake: Can not split a matured or withdrawn stake');
    ...
    function upgradeStakeInternal(StakeUpgrade memory stakeUpgrade) internal {
        ...
        //** TODO: Think about this */
        // come back with 12 decimals removed
    ...
    */
    ...
    /** Unset Stake
        ...
        @param id {uint256}
        @param payout {uint256} comes from stake burner
    ...
    function unsetStake(
        ...
        uint256 amount = uint256(stakeData[id].amount) * 1e12;
        uint256 shares = uint256(stakeData[id].shares) * 1e12;
    ...
    */
    ...
    /** @notice returns total share supply */
    function getTotalShares() external view override returns (uint256) {
        return uint256(statFields.sharesTotalSupply) * 1e12;
    }
...
contract StakeBase is Pausable, ExternallyCallable {
    ...
    struct StatFields {
        ...
        uint72 _gap; // 1-e6 total number of shares from accounts that registered for the VCA
