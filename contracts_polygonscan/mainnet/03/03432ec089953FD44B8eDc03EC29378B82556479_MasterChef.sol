...
// File: contracts/CockToken.sol

pragma solidity 0.6.12;

contract CockToken is ERC20("Cock Token", "COCK"), Ownable {
...
// MasterChef is the master of Cock. He can make Cock and he is a fair guy.
//
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 accCockPerShare; // Accumulated FISHes per share, times 1e18. See below.
        uint16 depositFeeBP; // Deposit fee in basis points
    }

    // The COCK TOKEN!
    CockToken public cockToken;
    address public devAddress;
    address public feeAddress;

    // COCK tokens created per block.
    uint256 public cockPerBlock = 1 ether;
    ...
    uint256 constant MAX_COCK_SUPPLY = 80000 ether;
    ...
    event UpdateEmissionRate(address indexed user, uint256 cockPerBlock);


    constructor(
        CockToken _cockToken,
        uint256 _startBlock,
        address _devAddress,
        address _feeAddress
    ) public {
    ...
    // Update the given pool's COCK allocation point and deposit fee. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        uint16 _depositFeeBP
    ...
    function pendingCock(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 cockReward = multiplier
                .mul(cockPerBlock)
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
            accCockPerShare = accCockPerShare.add(
                cockReward.mul(1e18).div(lpSupply)
            );
        }
        return user.amount.mul(accCockPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if(cockToken.owner() != address(this)) {
        ...
        uint256 currentSupply = cockToken.totalSupply();

        if (currentSupply >= MAX_COCK_SUPPLY) {
            pool.lastRewardBlock = block.number;
            return;
        }
        ...
        uint256 devReward = cockReward.div(10);
        cockToken.mint(devAddress, devReward);
        cockToken.mint(address(this), cockReward);
        pool.accCockPerShare = pool.accCockPerShare.add(
            cockReward.mul(1e18).div(lpSupply)
        );
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for COCK allocation.
    function deposit(
        uint256 _pid,
        uint256 _amount
    ) public nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = user
                .amount
                .mul(pool.accCockPerShare)
                .div(1e18)
                .sub(user.rewardDebt);
            if (pending > 0) {
                uint256 devReward = pending.div(10);
                safeCockTransfer(msg.sender, pending.sub(devReward));
            }
        }
        if (_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accCockPerShare).div(1e18);
    ...
    function _withdraw(
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accCockPerShare).div(1e18).sub(
            user.rewardDebt
        );
        if (pending > 0) {
            uint256 devReward = pending.div(10);
            safeCockTransfer(_user, pending);
        }
        if (_amount > 0) {
    ...
    // Safe cockToken transfer function, just in case if rounding error causes pool to not have enough COCK.
    function safeCockTransfer(address _to, uint256 _amount) internal {
        uint256 cockBal = cockToken.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > cockBal) {
            transferSuccess = cockToken.transfer(_to, cockBal);
        } else {
            transferSuccess = cockToken.transfer(_to, _amount);
        }
        require(transferSuccess, "safeCockTransfer: Transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _cockTokenPerBlock)
        external
        onlyOwner
    {
        massUpdatePools();
        cockPerBlock = _cockTokenPerBlock;
        emit UpdateEmissionRate(msg.sender, _cockTokenPerBlock);
    }
    ...
    function transferCockTokenOwnership(address _newOwner)
        external
        onlyOwner
    {
        cockToken.transferOwnership(_newOwner);
    }
}