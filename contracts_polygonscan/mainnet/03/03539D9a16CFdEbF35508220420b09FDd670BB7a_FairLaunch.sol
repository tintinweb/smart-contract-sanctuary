...
interface IFairLaunch {
  ...
  function pendingStableM(uint256 _pid, address _user) external view returns (uint256);
...
// File: contracts/StableMToken.sol
...
contract StableMToken is ERC20("Mock StableM Token", "mSTABLEM"), Ownable {
...
contract FairLaunch is IFairLaunch, Ownable {
  ...
  */
  ...
  function addRewardsDistribution(address _dev) public onlyOwner {
        whitelist[_dev] = true;
        emit AddWhitelist(_dev);
  }

  function removeRewardsDistribution(address _dev) public onlyOwner {
  ...
  */
  ...
  function updatePool(uint256 _pid) public override {
    ...
    // remainingReward
    uint256 remainingReward = stableMReward.sub(stableMReward.mul(10).div(100)).sub(stableMReward.mul(4).div(100));
