...
contract OneSplit is Initializable, IOneSplit, OneSplitRoot, UUPSUpgradeable, AccessControlUpgradeable {
    ...
    function initialize(IOneSplitView _oneSplitView, address _handlerAddress, address _reserveAddress, address _bridgeAddress) public initializer {
        __AccessControl_init();
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
...
//[rusdt, usdt, route]
    function _swap(
        IERC20Upgradeable fromToken,//usdt
        IERC20Upgradeable destToken,//rusdt
        uint256 amount,
        uint256 minReturn,
        uint256[] memory distribution,
        uint256 flags // See constants in IOneSplit.sol
        ...
        if((reserveInstance._contractToLP(address(destToken)) == address(fromToken)) &&
            (destToken.universalBalanceOf(address(reserveInstance)) > amount)) {
                bridgeInstance.unstake(handlerAddress, address(fromToken), amount);
                return amount;
        }
