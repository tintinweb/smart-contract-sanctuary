...
contract pMintyMultiToken is ContextMixin, ERC1155, NativeMetaTransaction {
    ...
    modifier onlyAuth() {
        require (auth[msgSender()] || (msgSender() == art_owner),"unauthorised");
        _;
    }

    function owner() external view returns (address) {
        if (auth[msgSender()]) return art_owner;
        return deployer;
    }
    ...
    function addPools(
        ...
        require ( msgSender() == deployer ||  auth[msgSender()] || (msgSender() == art_owner) , "Unauthorised");
        _addPools(_initialEntries, _resaleEntries,_poolName);
    }
    ...
    function setContractURI(string memory _uri) external {
        require(msgSender() == deployer,"This should be set as part of the deployment process");
        contractURI = _uri;
    } 
