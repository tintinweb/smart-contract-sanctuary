...
contract PHAST {
	...
	string private _symbol;       												//Símbolo del token: PHAST
	string private _name;         												//Nombre largo del token. Podemos llamarlo PHAST o cualquier otra cosa que se nos ocurra. Consultar a Enric
	...
	constructor(string memory token_name, string memory short_symbol, uint8 token_decimals, uint256 token_initialSupply) {
		...
		_total_Supply = token_initialSupply;//El total supply será en principio 0 hasta que se actualice con el token_initialSupply

		owner        = msg.sender;

	    coinBalance[msg.sender] = _total_Supply;
    ...
    /**
	*
	* Función de transferencia de tokens phast a la dirección _to desde la wallet del que ejecuta la función
	*
	*/   
	...
	*/	
	function mint(address _recipient, uint256  _mintedAmount) 
		onlyOwner public {
		require(_recipient != address(0), "PHAST: cannot mint to zero address"); // No puede ser una dirección vacía (0x0)
	...
	*/		
	function burn(address _recipient, uint256 amount) onlyOwner public {
		require(_recipient != address(0), "PHAST: cannot burn from zero address");                      // No puede ser una dirección vacía (0x0)
		require(coinBalance[_recipient] >= amount, "PHAST: Cannot burn more than the _recipient owns"); // Cantidad eliminada debe ser mayor o igual de lo que hay en cartera
