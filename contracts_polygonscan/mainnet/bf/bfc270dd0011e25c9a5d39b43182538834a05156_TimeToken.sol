...
contract TimeToken is ERC721Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    using CountersUpgradeable for CountersUpgradeable.Counter;
    CountersUpgradeable.Counter private _tokenIds;
    mapping (uint256 => TimeMeta) private _tokenMeta;
    ...
    */
    function initialize() public initializer {
        ERC721Upgradeable.__ERC721_init_unchained("TimeToken", "TIME"); //Name of Token
        OwnableUpgradeable.__Ownable_init(); // Set Owner
        ReentrancyGuardUpgradeable.__ReentrancyGuard_init();
        saleIsActive = false; // SaleStatus
        MAX_TOKENS = 1440;  // 24 hours * 60 = 1440
        tokenPrice = 60000000000000000000; //60 Matic
        MAX_GIFT = 50; // Max Gift per time
        MAX_PURCHASE = 15; // Max Purchase per time
        developerAddress = 0x42e98CdB46444c96B8fDc93Da2fcfd9a77FA9575; // Developer Address
        setBaseURI("https://ipfs.abolfazl.dev/token/"); // Data api
    }
    /**
    ...
    */
    function _setTokenMeta(uint256 _tokenId, TimeMeta memory _meta) private {
    ...
    * Mint Time
    */
    function mintTime(uint _TokensNr) public payable {
        require(saleIsActive, "Sale must be active to mint Tokens");
        require(_TokensNr <= MAX_PURCHASE, "Can only mint 15 tokens at a time");
        require(TokenCounter(_TokensNr)  <= MAX_TOKENS, "May supply of Time's");
        ...
        for(uint i = 1; i <= _TokensNr; i++) {
            ...
            if (ItemID <= MAX_TOKENS) {
                ...
                TimeMeta memory metaData = TimeMeta(ItemID);
    ...
    */
    function mintGift(uint _GiftNr) public onlyOwner {
        ...
        require(TokenCounter(_GiftNr)  <= MAX_TOKENS, "Exceed max supply of Time");
        uint ItemID;
    ...
    */
    ...
    function tokenMeta(uint256 _tokenId) public view returns (TimeMeta memory) {
        require(_exists(_tokenId));
