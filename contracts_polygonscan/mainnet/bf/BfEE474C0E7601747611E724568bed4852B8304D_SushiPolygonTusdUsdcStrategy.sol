...
import "../base/MiniChefStrategy.sol";

contract SushiPolygonTusdUsdcStrategy is MiniChefStrategy {
    constructor() public {}

    function initialize(address _vault) public initializer {
        address underlying = address(
            0x4AB66a7138F50Db9B7aDaF8CE8aFB3F638B22f7f
        ); //TusdUsdc SLP
        uint256 poolId = 42; // TusdUsdc Pool id
        ...
        MiniChefStrategy.initializeMiniChefStrategy(
            _vault,
            underlying,
            address(0x0769fd68dFb93167989C6f7254cd0D766Fb2841F), // miniChef contract
        ...
        rewardRoutes[tusd] = [sushi, weth, usdc, tusd];
        rewardRoutes[usdc] = [sushi, weth, usdc];

        secondRewardRoute = [wmatic, sushi];

        fromVaultAssetRoutes[dai][tusd] = [dai, weth, usdc, tusd];
        fromVaultAssetRoutes[dai][usdc] = [dai, weth, usdc];
        fromVaultAssetRoutes[usdt][tusd] = [usdt, usdc, tusd];
        fromVaultAssetRoutes[usdt][usdc] = [usdt, usdc];
        fromVaultAssetRoutes[usdc][tusd] = [usdc, tusd];

        toVaultAssetRoutes[dai][tusd] = [tusd, usdc, weth, dai];
        toVaultAssetRoutes[dai][usdc] = [usdc, weth, dai];
        toVaultAssetRoutes[usdt][tusd] = [tusd, usdc, usdt];
        toVaultAssetRoutes[usdt][usdc] = [usdc, usdt];
        toVaultAssetRoutes[usdc][tusd] = [tusd, usdc];
    }
}
...
import "../../interface/sushiswap/IMiniChef.sol";
import "../../interface/uniswap/IUniswapV2Pair.sol";
import "hardhat/console.sol";

contract MiniChefStrategy is OwnableUpgradeable {
    ...
    mapping(address => mapping(address => address[]))
        public fromVaultAssetRoutes;
    mapping(address => mapping(address => address[])) public toVaultAssetRoutes;
    address[] public secondRewardRoute;
    uint256 public sellFloor;

    constructor() public {}

    function initializeMiniChefStrategy(
        address _vault,
        address _underlying,
        address _miniChef,
        ...
        IERC20 _lpt;
        _lpt = IMiniChef(miniChef).lpToken(_poolId);
        require(
            address(_lpt) == underlying,
    ...
    function setSellFloor(uint256 _floor) external onlyOwner {
        sellFloor = _floor;
    }

    function rewardPoolBalance() public view returns (uint256 bal) {
        (bal, ) = IMiniChef(miniChef).userInfo(poolId, address(this));
    }

    function investedUnderlyingBalance() public view returns (uint256) {
        if (miniChef == address(0)) {
    ...
    function investedBalanceInUSD() public view returns (uint256) {
        uint256 underlyingBalance = investedUnderlyingBalance();
        return getUSDBalanceFromUnderlyingBalance(underlyingBalance);
    }

    function _enterRewardPool() internal {
        uint256 _entireBalance = IERC20(underlying).balanceOf(address(this));
        IERC20(underlying).safeApprove(miniChef, 0);
        IERC20(underlying).safeApprove(miniChef, _entireBalance);
        IMiniChef(miniChef).deposit(poolId, _entireBalance, address(this));
    }

    function _exitRewardPool() internal {
        uint256 _bal = rewardPoolBalance();
        if (_bal != 0) {
            IMiniChef(miniChef).withdraw(poolId, _bal, address(this));
        }
    }
    ...
    function _emergencyExitRewardPool() internal {
        uint256 _bal = rewardPoolBalance();
        if (_bal != 0) {
            IMiniChef(miniChef).emergencyWithdraw(poolId, address(this));
        }
    }
    ...
    function setLiquidationPath(address _token, address[] memory _route)
        ...
        onlyOwner
    {
        rewardRoutes[_token] = _route;
    }

    function assetToUnderlying(address _asset) public returns (uint256) {
        address _token0 = IUniswapV2Pair(underlying).token0();
        address _token1 = IUniswapV2Pair(underlying).token1();
        address[] memory _route0 = fromVaultAssetRoutes[_asset][_token0];
        address[] memory _route1 = fromVaultAssetRoutes[_asset][_token1];
        return _assetToUnderlying(_asset, _route0, _route1);
    }

    function _assetToUnderlying(
        address _asset,
        address[] memory _route0,
        address[] memory _route1
    ) internal returns (uint256) {
        uint256 _bal = IERC20(_asset).balanceOf(address(this));
        uint256 _liquidity;

        if (_bal > 0) {
            address _token0 = IUniswapV2Pair(underlying).token0();
            address _token1 = IUniswapV2Pair(underlying).token1();
            uint256 _toToken0 = _bal.div(2);
            uint256 _toToken1 = _bal.sub(_toToken0);

            uint256 _token0Amount;
            if (_route0.length > 1) {
                _swapSushiswapWithPath(_route0, _toToken0);
                _token0Amount = IERC20(_token0).balanceOf(address(this));
            } else {
                // otherwise we assme token0 is the reward token itself
                _token0Amount = _toToken0;
            }

            uint256 _token1Amount;
            if (_route1.length > 1) {
                _swapSushiswapWithPath(_route1, _toToken1);
                _token1Amount = IERC20(_token1).balanceOf(address(this));
            } else {
                _token1Amount = _toToken1;
            }

            // provide token1 and token2 to SUSHI
            IERC20(_token0).safeApprove(sushiRouter, 0);
            IERC20(_token0).safeApprove(sushiRouter, _token0Amount);

            IERC20(_token1).safeApprove(sushiRouter, 0);
            IERC20(_token1).safeApprove(sushiRouter, _token1Amount);

            // we provide liquidity to sushi
            (, , _liquidity) = IUniswapV2Router02(sushiRouter).addLiquidity(
                _token0,
                _token1,
                _token0Amount,
                _token1Amount,
    ...
    function _assetFromUnderlying(
        ...
        IERC20(underlying).safeApprove(sushiRouter, 0);
        IERC20(underlying).safeApprove(sushiRouter, _liquidity);
    ...
    function _liquidateReward() internal {
        uint256 _secondRewardBalance = IERC20(secondRewardToken).balanceOf(
            address(this)
        );

        if (secondRewardRoute.length > 1 && _secondRewardBalance > 0) {
            _swapSushiswapWithPath(secondRewardRoute, _secondRewardBalance);
        }
        ...
        address _token0 = IUniswapV2Pair(underlying).token0();
        address _token1 = IUniswapV2Pair(underlying).token1();
        address[] memory _route0 = rewardRoutes[_token0];
        address[] memory _route1 = rewardRoutes[_token1];

        if (_rewardBalance > sellFloor) {
            _assetToUnderlying(rewardToken, _route0, _route1);
        }
    }
    ...
    function _swapSushiswapWithPath(address[] memory _path, uint256 _amount)
        internal
    {
        require(_path[1] != address(0));
        IERC20(_path[0]).safeApprove(sushiRouter, 0);
        IERC20(_path[0]).safeApprove(sushiRouter, _amount);
...
interface IMiniChef {
    ...
    function updatePool(uint256 pid) external returns (IMiniChef.PoolInfo memory);
