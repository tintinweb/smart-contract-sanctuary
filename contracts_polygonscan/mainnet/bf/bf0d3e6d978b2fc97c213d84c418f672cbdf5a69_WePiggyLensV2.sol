...
contract WePiggyLensV2 {
    ...
    function pTokenMetadata(PTokenLensInterface pToken) public view returns (PTokenMetadata memory){
        ...
        if (compareStrings(pToken.symbol(), "pMATIC")) {
            ...
            underlyingSymbol = pErc20.symbol();
        }
    ...
    // 获得用户有哪些市场、流动性
    function getAccountLimits(ComptrollerLensInterface comptroller, address payable account) public view returns (AccountLimits memory) {
    ...
    function getWpcSpeed(PiggyDistributionInterface distribution, PTokenLensInterface pToken) public view returns (PTokenWpcSpeed memory){
    ...
    function getWpcSpeeds(PiggyDistributionInterface distribution, PTokenLensInterface[] memory pTokens) public view returns (PTokenWpcSpeed[] memory){
        uint pTokenCount = pTokens.length;
        PTokenWpcSpeed[] memory res = new PTokenWpcSpeed[](pTokenCount);
        for (uint i = 0; i < pTokenCount; i++) {
            res[i] = getWpcSpeed(distribution, pTokens[i]);
        }
        return res;
    }
    ...
    function all(ComptrollerLensInterface comptroller, address payable account) external view returns (AccountLimits memory, PTokenBalances[] memory, PTokenMetadata[] memory, InterestRateModel[] memory ){
    ...
    function multicall(bytes[] calldata data) external returns (bytes[] memory results) {
        ...
        for (uint256 i = 0; i < data.length; i++) {
            (bool success, bytes memory ret) = address(this).call(data[i]);
            require(success);
            results[i] = ret;
        }
        return results;
