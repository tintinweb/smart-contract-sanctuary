...
contract UnusedUsdcBankStrat is Ownable, Pausable {
    ...
    constructor(address _vaultAddress) public {
        ...
        IERC20(wmatic).safeApprove(aavebank, 0);
        IERC20(wmatic).safeApprove(aavebank, uint(-1));
    ...
    function deposit() public {
        ...
        if (pairBal > 0) {
            ILendingPool(aavebank).deposit(wmatic, IERC20(wmatic).balanceOf(address(this)), address(this), 0);
        }
    }
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (pairBal < _amount) {   
            ILendingPool(aavebank).withdraw(wmatic,_amount.sub(pairBal), address(this));
    ...
    function balanceOfPool() public view returns (uint256) {
        (uint256 supplyBal,,,,,,,,) = IDataProvider(dataProvider).getUserReserveData(wmatic, address(this));
        return supplyBal;
    }
    ...
    function panic() public onlyOwner {
        ILendingPool(aavebank).withdraw(wmatic, type(uint).max, address(this));
        pause();
    }
