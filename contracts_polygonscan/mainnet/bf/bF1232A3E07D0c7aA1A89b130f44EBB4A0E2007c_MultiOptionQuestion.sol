...
contract MultiOptionQuestion is
    Initializable,
    UUPSUpgradeable,
    OwnableUpgradable,
    PausableUpgradable,
    ReentrancyGuardUpgradeable
{
    ...
    struct Question {
        ...
        uint256 lockTime;
        uint256 totalAmount;
        mapping(uint8 => uint256) betPositionAmount;
    ...
    event PlaceBet(address indexed sender, bytes32 indexed questionID, uint256 amount, uint8 option);
    ...
    event TimeUpdated(bytes32 indexed questionID, uint256 startTime, uint256 lockTime);
    ...
    function initialize(
        ...
        address _gameToken;
        bytes32 _questionID;
        (_minBetAmount, _startTime, _lockTime, _rewardRate, _treasuryRate, _totalOptions, _gameToken, _questionID) = abi
            .decode(data, (uint256, uint256, uint256, uint8, uint8, uint8, address, bytes32));
        require(_startTime < _lockTime, "GameQuestion: _startTime should be less than _lockTime");
        require(_rewardRate + _treasuryRate == 100, "TotalRate should be 100");
    ...
    function setStartLockTime(uint256 _startTime, uint256 _lockTime) external onlyAdmin {
        require(_startTime < _lockTime, "GameQuestion: _startTime should be less than _lockTime");
        question.startTime = _startTime;
        question.lockTime = _lockTime;
        emit TimeUpdated(questionID, _startTime, _lockTime);
    }
    ...
    function placeBet(bytes memory data, address userAddress) external onlyOwner whenNotPaused nonReentrant {
        ...
        require(option < totalOptions - 1, "Invalid Option");
        ...
        question.betPositionAmount[option] += amount;
        betInfo.betPositionAmount[option] += amount;
        gameToken.safeTransferFrom(userAddress, address(this), amount);

        emit PlaceBet(userAddress, questionID, amount, option);
    }

    function _setAnswer(uint8 option) internal {
        require(option < totalOptions, "Invalid Option");
    ...
    function _calculateRewards() internal {
        ...
        uint256 treasuryAmt;
        // Side A
        if (question.answer < totalOptions - 1) {
            if (_otherOptionsZero()) {
                //otherOptionsZero
                rewardBaseCalAmount = question.betPositionAmount[question.answer];
                rewardAmount = question.totalAmount;
                treasuryAmt = 0;
            } else if (question.betPositionAmount[question.answer] == 0) {
                //winningOptionZero
    ...
    function _otherOptionsZero() internal view returns (bool) {
        bool otherOptionsZero = true;
        for (uint8 i = 0; i < totalOptions - 1; i++) {
            if (i != question.answer && question.betPositionAmount[i] > 0) {
                otherOptionsZero = false;
                break;
            }
        }
        return otherOptionsZero;
    }
    ...
    function claim(address userAddress) external onlyOwner whenNotPaused nonReentrant {
        ...
        uint256 reward;
        uint256 userTotalBetAmount;
        //users total bet amount
        for (uint8 i = 0; i < totalOptions - 1; i++) {
            userTotalBetAmount += betInfo.betPositionAmount[i];
        }

        require(userTotalBetAmount > 0, "Bet amount should be > 0");

        // questn valid, claim rewards
        if (question.answer < totalOptions - 1) {
            require(betInfo.betPositionAmount[question.answer] > 0, "Not eligible for claim"); // Needs to change
            reward =
                (betInfo.betPositionAmount[question.answer] * question.rewardAmount) /
                (question.rewardBaseCalAmount);
        }
        // questn invalid, refund bet amount
        else {
            require(question.answer == totalOptions - 1, "Not eligible for refund");
            reward = userTotalBetAmount;
        }
    ...
    function seeUserInfo(address userAddress, uint8 option)
        external
        view
        returns (uint256 positionAmount, bool claimed)
    {
        positionAmount = ledger[userAddress].betPositionAmount[option];
        claimed = ledger[userAddress].claimed;
    }
}
