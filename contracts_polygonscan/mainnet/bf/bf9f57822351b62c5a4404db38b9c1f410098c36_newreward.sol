...
contract newreward is ERC721Enumerable, Ownable {
 ...
 function setLimitWhitelisted(uint256 _state) public onlyOwner {
  ...
  function updateReward(address from, address to) external {
    ...
    function getTotalClaimable(address user) external returns(uint256) {
        return rewards[user] + getPendingReward(user);
    }

    function getPendingReward(address user) public returns(uint256) {
        balanceoftokens[user]+= pplwhohavenft[user] * BASE_RATE * (block.timestamp - (lastUpdate[user] >= START ? lastUpdate[user] : START)) / 60;
        return balanceoftokens[user];
    }
