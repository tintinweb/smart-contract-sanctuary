...
contract TokenVesting is Ownable {
  ...
  event TokenVestingRevoked(IERC20 token);
  ...
  )
  {
    ...
    require(duration_ > 0);
    require(start_.add(duration_) > block.timestamp);
   ...
   */
  function revoked(IERC20 token) public view returns(bool) {
  ...
  function _vestedAmount(IERC20 token) private view returns (uint256) {
    ...
    if (block.timestamp < _cliff) {
      ...
      /*
      if (_cliffDuration > 0) {
        uint256 tmp =  block.timestamp.sub(_start);
        tmp = tmp.div(_cliffDuration).mul(_cliffDuration);
        return totalBalance.mul(tmp).div(_duration);
      } else {
          return totalBalance.mul(block.timestamp.sub(_start)).div(_duration);
      }
      */
