...
contract Election {
    ...
    function getStatus (uint _proposalId) public view returns(uint8 status){
        status = 1; // pending
        if(proposals[_proposalId-1].startTime.add(proposals[_proposalId-1].createdTime) <= block.timestamp)
        {
            if(proposals[_proposalId-1].endTime.add(proposals[_proposalId-1].createdTime) >= block.timestamp)
            {
                status = 2; //active; 
            }
            else
    ...
    function vote (uint _proposalId, bool opinion) public {
        ...
        voters[_proposalId][msg.sender].voted = true;
