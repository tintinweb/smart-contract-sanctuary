...
contract BeliFarm is Ownable, ReentrancyGuard {
    ...
    function payConvertedProfit(uint256 _pid, address _userAddress) internal {
        ...
        if (user.shares > 0) {
            ...
            uint256 accBELIProfitPerShare = IVault(pool.vault).accBELIProfitPerShare();
            uint256 amount = user.shares.div(totalShare).mul(accBELIProfitPerShare).div(1e12).sub(user.rewardProfitDebt);
            if (amount > 0) {
    ...
    function pendingConvertedProfit(
        ...
        if (user.shares > 0) {
            uint256 totalShare = IVault(pool.vault).sharesTotal();
            uint256 accBELIProfitPerShare = IVault(pool.vault).accBELIProfitPerShare();
            amount = user.shares.div(totalShare).mul(accBELIProfitPerShare).div(1e12).sub(user.rewardProfitDebt);
        }
        return amount;
    }
    ...
    function payOrLockupPendingBELI(uint256 _pid, bool _isWithdrawal) internal {
        ...
        if (pool.isTransferFeeReward || pool.isLpReward) {
            transferRewardFromFee(_pid, msg.sender);
        }

        if (IVault(pool.vault).accBELIProfitPerShare() > 0) {
...
interface IVault {
    ...
    function accBELIProfitPerShare() external view returns (uint256);
