...
contract Level4 is IERC721MintableBurnable, ERC721Enumerable, Ownable {
    ...
    */
    function mint(address to) external override onlyOwner returns (uint256 newTokenId) {
        newTokenId = getNextTokenId();
        require(newTokenId == 1, "{mint} : This is a 1/1 ERC721 token");
        _mintTo(to);
    }
     ...
     * @notice Allows owner to mint a a token to a given address
     * dev Mints a new token to the given address, increments currentTokenId
     * @param to : array of addresses of the future owners of the token
    */
    function batchMint(address[] calldata to) external override onlyOwner {
        for (uint256 i=0; i < to.length; i++) {
            _mintTo(to[i]);
        }
    }

    function batchBurnFrom(uint256[] calldata tokenIds) external override {
        for (uint256 i=0; i < tokenIds.length; i++) {
            _burnFrom(tokenIds[i]);
        }
    }
     ...
     * Can only be called by the current owner.
     */
    function transferOwnership(address newOwner) public override(IERC721MintableBurnable, Ownable) onlyOwner {
     ...
     * @dev Calculates the next token ID based on value of _tokenIdTracker
    ...
    */
     ...
     */
    function _burnFrom(uint256 tokenId) internal virtual {
        //solhint-disable-next-line max-line-length
    ...
    /**
     * @notice Increments the value of _tokenIdTracker
     * @dev Internal function that increases the value of _tokenIdTracker by 1
    */
