...
// Rolling 8 day reward period idea credit goes to denett
// 8 days allows for 1 day buffer for receiving rewards from the rewards distributor (which distributes every 7 days)
// Assuming a worst case scenario from a late deposit, it will underemit briefly, then overemit slightly to compensate
...
contract FraxCrossChainFarm is Owned, ReentrancyGuard {
    ...
    // Reward period
    // 8 * 86400  (8 days). 
    uint256 public rewardsDuration = 691200; 
    ...
    function syncRewards() internal {
        ...
        {
            uint256 elapsed_time = (block.timestamp).sub(lastUpdateTime);
            ttlRew0Owed += rewardRate0.mul(elapsed_time);
            ttlRew1Owed += rewardRate1.mul(elapsed_time);
        }
