...
contract PolygonNFTTester is ERC721Enumerable, Ownable {
    ...
    constructor(uint256 mintingStart, string memory _uri, address poeToken, uint256 buffer) ERC721("PolyTest", "Test") {
        mintingActive = false;
    ...
    function mint(uint256 _amount) public payable {
        ...
        if (claimedFree[msg.sender] == false && block.timestamp < mintingStartTimestamp && totalSupply() < 5) { //free claim for presale particiapnts
            claimedFree[msg.sender] = true;
            _safeMint(msg.sender, tsupply);
            return;
        }
        else if(block.timestamp < mintingStartTimestamp && numPurchased[msg.sender] == 0 && totalSupply() > 5){
