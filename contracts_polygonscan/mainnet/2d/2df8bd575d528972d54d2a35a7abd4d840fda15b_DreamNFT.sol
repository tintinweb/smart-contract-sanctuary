...
/**
 *Submitted for verification at Polygonscam.com on 2021-10-10
 */

//Contract based on https://docs.openzeppelin.com/contracts/4.x/erc721
// SPDX-License-Identifier: MIT

//  yu·me·make 夢メーク     yu·me·make 夢メーク     yu·me·make 夢メーク     yu·me·make 夢メーク
//          DREAM GIRLS NFT COLLECTION      DREAM GIRLS NFT COLLECTION      DREAM GIRLS NFT COLLECTION
//FOR YOU_&_ME      FOR YOU_&_ME    FOR YOU_&_ME    FOR YOU_&_ME    FOR YOU_&_ME    FOR YOU_&_ME
//
//
// .        *,,,,,,%(((/*,@.,,,,,          .%&/
//   (((//((((((#(/(/%(/*#** (*,,.  &((####((((((#%#((((((&
...
contract DreamNFT is ERC721Enumerable, Ownable {
    ...
    // Events
    event TokenMinted(uint256 tokenId, address owner, uint256 anniversary);

    // Token Detail
    mapping(uint256 => DreamDetail) private _dreamDetails;
    ...
    uint256 public MAX_TOKENS = 3150;

    // Current price.
    uint256 public CURRENT_PRICE = 88000000000000000000; // 88 MATIC
    ...
    function reserveTokens() public onlyOwner {
        uint256 i;
        uint256 tokenId;
        uint256 anniversary = block.timestamp;

        for (i = 1; i <= 30; i++) {
            tokenId = totalSupply().add(1);
            if (tokenId <= MAX_TOKENS) {
                _safeMint(msg.sender, tokenId);
                _dreamDetails[tokenId] = DreamDetail(anniversary);
                emit TokenMinted(tokenId, msg.sender, anniversary);
            }
        }
    }
     ...
     * Mint Dream Girl
     */
    function mintDream(uint256 numberOfTokens) public payable {
        ...
        require(
            ...
            "Purchase would exceed max supply of Dream Girl NFTs"
        );
        require(
     ...
     */
    function getDreamDetail(uint256 tokenId)
        public
        view
        returns (DreamDetail memory detail)
    {
        require(_exists(tokenId), "Token was not minted");

        return _dreamDetails[tokenId];
    }
}