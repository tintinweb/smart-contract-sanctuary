...
// File contracts/Token/TokenTaxLP.sol
...
contract TokenTaxLP is BEP20 {
    ...
    // Tax switch block number
    uint256 public taxSwitchCheckpoint = 0;
    ...
    // Min amount to liquify. (default 2)
    uint256 public minAmountToLiquify = 2e18;
    ...
    // Last transfer block number for antibot
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override{
        ...
        // sneaky way to detect lp addition
        if(tx.origin == operator() && taxSwitchCheckpoint == 0)
            taxSwitchCheckpoint = block.number + 43200; //~24 hours

        // Max 2.5% of total supply per txn
        uint256 maxAmt = totalSupply().mul(250).div(10000);


        // Exclude operator/token/masterchef from antiwhale
        require(
            amount < maxAmt 
            || sender    == _operator 
            || sender    == address(this) 
        ...
        // Exclude token/masterchef from antibot
        require(
            (lastTransferBlock[tx.origin] + 30) < block.number 
            || sender    == owner() 
            || sender    == address(this) 
            || recipient == address(this)
            || recipient == owner(), 
            "Antibot - 1 txn every 30 blocks"
        );
        ...
        if (recipient == BURN_ADDRESS || transferTaxRate == 0) {
            ...
            if(taxSwitchCheckpoint > block.number) {
                taxAmount = amount.mul(MAXIMUM_TRANSFER_TAX_RATE).div(10000);
            }
    ...
    function updateswapRouter() public onlyOperator {
        ...
        swapRouter = IUniswapV2Router02(0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff);
...
// File contracts/Masterchef/MasterChefRef.sol
...
contract MasterChefRef is Ownable {
    using SafeMath for uint256;
    ...
    //Don't allow duplicate lp pools
    ...
    function add(uint256 _allocPoint, IBEP20 _lpToken, uint16 _depositFeeBP, bool _burn, bool _withUpdate) public onlyOwner nonDuplicated(_lpToken) {
        // require(_depositFeeBP <= 10000, "add: invalid deposit fee basis points");
        // Max deposit fee of 3.5%
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 tokensPerBlock = tokenPerBlock();

        // in case the emission rate decreased since the last update pool
        // use the last emission rate to ensure rewards are not lost
        // This is under the assumption that the emission rate is decreasing
        if (pool.tokensPerBlock > tokensPerBlock) {
          tokensPerBlock = pool.tokensPerBlock;
        }
        ...
        pool.tokensPerBlock = tokenPerBlock();
        pool.lastRewardBlock = block.number;
    }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        require(pool.burn == false, "withdraw: Burn pools can't withdraw");
    ...
    function burnFromPool(uint256 _pid) public onlyOwner {
        PoolInfo storage pool = poolInfo[_pid];
        require(pool.burn == true, "Can't burn from non-burn pools");
        require(tokenPerBlock() == 0, "Can't burn before farm end");

        pool.lpToken.transfer(BURN_ADDRESS, pool.lpToken.balanceOf(address(this)));
    }
    ...
    function getStartBlock() view public returns (uint256) {
        return startBlock;
    }
