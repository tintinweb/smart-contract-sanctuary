...
contract TokenToken is BEP20 {
    ...
    /**
     * @notice Constructs the TOKEN Token contract.
     */
    constructor() public BEP20("PolyWhale", "WHALE") {
...
// MasterChef, Neptune, is the master of Token. He can make Token and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract if TOKEN is sufficiently
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    uint256 public tokenPerBlock = 0.01 * 10 ** 18;
    // Bonus multiplier for early TOKEN makers.
    uint256 public constant BONUS_MULTIPLIER = 1;
    // Deposit Fee address
    address public feeAddress;
    // Total TOKEN in contract from deposits and rewards
    uint256 private tokenDeposit = 0;
    ...
    function updatePool(uint256 _pid) public {
		...
		uint256 tokenToDev = tokenReward.div(50);
        tokenReward = tokenReward.sub(tokenToDev);
        token.mint(devAddress, tokenToDev); // 2% dev fee.
        
        token.mint(address(this), tokenReward);
        tokenDeposit = tokenDeposit.add(tokenReward); // Add token minted
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if(pending > 0) {
            safeTokenTransfer(msg.sender, pending);
            tokenDeposit = tokenDeposit.sub(pending); // Subtract token reward
        }
        if(_amount > 0) {
    ...
    function dev(address _devAddress) public {
        ...
        require(_devAddress != address(0), "new dev cannot be the zero address");
    ...
    function updateEmissionRate(uint256 _tokenPerBlock) public onlyOwner {
        massUpdatePools();
        require(_tokenPerBlock<=100000000000000000000, "You cannot make TOKEN Per Block more than 100 TOKEN"); // This prevents abuse
        tokenPerBlock = _tokenPerBlock;
