...
contract SnookGame is ISnookGame, AccessControlEnumerableUpgradeable, PausableUpgradeable {
    ...
    bytes32 public constant GAMEKEEPER_ROLE = keccak256("GAMEKEEPER_ROLE");
    ...
    // user generates a snook for himself
    function mint2(uint count) external override {
    ...
    function _mint2() internal {
      ...
      (uint tokenId, uint price) = _mint(to, 1, tokenURI);
      emit Birth2(to, tokenId, price, traitId);
    }
    ...
    function mint(
      ...
      (uint tokenId, uint price) = _mint(to, traitCount, tokenURI_);
      emit Birth(to, tokenId, price);
    }
    ...
    ) internal returns(uint, uint)
    {
        ...
        require(_skill.transferFrom(to, address(this), price), 'SnookGame: No funds');
        ...
        _skinRewards.updatePeriod(tokenId, 0, 0, traitCount);
    ...
    function allowGame(uint256 tokenId) external override whenNotPaused() {
        ...
        require(d.gameAllowed == false, 'Already allowed');
        d.gameAllowed = true;
        _state.setDescriptor(tokenId, d);
        emit GameAllowed(owner, tokenId);
    }

    function enterGame(uint256 tokenId, uint resurrectionCount) 
      external override onlyRole(GAMEKEEPER_ROLE) whenNotPaused()
    {
        Descriptor memory d = _state.getDescriptor(tokenId);
        require(_snook.isLocked(tokenId) == false, 'In play');
        require(d.gameAllowed == true, 'Not allowed');
        require(d.resurrectionCount == resurrectionCount, 'Invalid resurrection count');

        _snook.lock(tokenId, true, 'enterGame');
        emit Entry(_snook.ownerOf(tokenId), tokenId);
    }

    function enterGame2(uint256 tokenId) 
      external override 
      whenNotPaused()
    {
    ...
    ) external override onlyRole(EXTRACTOR_ROLE) whenNotPaused()
    {
        ...
        _skinRewards.updatePeriod(tokenId, d.stars, stars, traitCount);
    ...
    ) external override onlyRole(KILLER_ROLE) whenNotPaused
    {
        ...
        _skinRewards.updatePeriod(tokenId, d.stars, 0, 0); // to update totalStars in the _periods
