...
contract AMPTChild {
    ...
    function deposit(address user, bytes calldata depositData) external {
        ...
        uint96 amount = safe96(abi.decode(depositData, (uint256)), "AMPT::deposit: amount exceeds 96 bits");

        // `amount` token getting minted here & equal amount got locked in RootChainManager
        totalSupply = add96(totalSupply, amount,"AMPT::deposit: update totalSupply failed");
        balances[user] = add96(balances[user], amount, "AMPT::deposit: update user ballance failed");
    ...
    function withdraw(uint96 amount) external {
        balances[msg.sender] = sub96(balances[msg.sender], amount, "ERC20: burn amount exceeds balance");
        totalSupply = sub96(totalSupply, amount, "AMPT::withdraw: update total suply failed");
