...
contract LiquidityMigratorV2 is Ownable, ReentrancyGuard {
    ...
    IPolydexRouter public immutable quickRouter;
    ...
    function switchLiquidity(
        ...
        address buyWMATICFrom,
        uint256 newPid
    )
        external
        nonReentrant
        ensureNonZeroAddress(address(oldLPAddress))
        ensureNonZeroAddress(buyWMATICFrom)
    {
        ...
        require(
            oldLPAddress.balanceOf(address(this)) >= lpAmount,
            "Insufficient Old LP Balance"
        );
        ...
        //Pair shouldn't contain WMATIC token
        require(
            liquidityVars.tokenA != wmatic && liquidityVars.tokenB != wmatic,
            "Invalid LP address"
        );

        //One of the tokens in the LP should be CNT
        require(
            liquidityVars.tokenA == cnt || liquidityVars.tokenB == cnt,
            "Invalid LP address"
        );

        //One of the tokens in the LP should be the token to buy wmatic from
        require(
            liquidityVars.tokenA == buyWMATICFrom ||
                liquidityVars.tokenB == buyWMATICFrom,
            "Invalid buyWMATICFrom address"
        );

        //buyWMATIC from address shouldn't be CNT.
        require(
            buyWMATICFrom != cnt && buyWMATICFrom != wmatic,
            "buyWMATICFrom address cannot be CNT and wmatic"
        );

        //Remove liquidity
        (liquidityVars.amountAReceived, liquidityVars.amountBReceived) = router
            .removeLiquidity(
        ...
        //Buy wmatic from the other token i.e. transform liquidity
        transFormLiquidity(buyWMATICFrom);
        ...
        //Transfer pending tokens wit any remaining dust
        if (wmaticBalance > 0)
            TransferHelper.safeTransfer(wmatic, msg.sender, wmaticBalance);
        ...
        if (
            liquidityVars.newPairAddress.balanceOf(address(this)) >=
            liquidityVars.lpReceived
        )
            _depositLP(
                address(liquidityVars.newPairAddress),
                newPid,
                liquidityVars.lpReceived,
                msg.sender
            );
    ...
    */
    function _swapToken(address _fromToken, uint256 _tokensToSwap)
        internal
        returns (uint256 tokenBought)
    {
        TransferHelper.safeApprove(address(_fromToken), address(quickRouter), 0);
        TransferHelper.safeApprove(
            address(_fromToken),
            address(quickRouter),
            _tokensToSwap
        ...
        tokenBought = quickRouter.swapExactTokensForTokens(
            _tokensToSwap,
            1,
            path,
            address(this),
    ...
    function transFormLiquidity(address buyWMATICFrom) internal {
        if (buyWMATICFrom == liquidityVars.tokenA) {
            (
                liquidityVars.amountAadded,
                liquidityVars.amountBadded,
                liquidityVars.lpReceived
        ...
        } else if (buyWMATICFrom == liquidityVars.tokenB) {
            (
                liquidityVars.amountBadded,
                liquidityVars.amountAadded,
                liquidityVars.lpReceived
    ...
    {
        require(otherToken == cnt, "Other token should be CNT");
        liquidityVars.wmaticPurchased = _swapToken(buyWMATICFrom, inputAmount);

        TransferHelper.safeApprove(
            address(wmatic),
            address(router),
            liquidityVars.wmaticPurchased
        );
        ...
        (wmaticAdded, cntAdded, lpReceived) = router.addLiquidity(
            wmatic,
            cnt,
            liquidityVars.wmaticPurchased,
            otherTokenAmount,
