...
contract FOCIv2 is ERC721, ERC721Enumerable, ERC721URIStorage, HasSecondarySaleFees, ContextMixin, Ownable {
    using Strings for uint256;

    string private constant _d1 = '{"name": "';
    string private constant _d2 = '","description": "';
    string private constant _d3 = '","image": "';
    string private constant _d4 = '"}';

    constructor()
    ERC721("FullyOnChainImgV2", "FOCIv2")
    ...
    function mint(uint256 tokenId, string memory name, string memory disc, string memory data) public onlyOwner {
        _safeMint(_msgSender(), tokenId);

        string memory imgstr = string(abi.encodePacked(data));
        string memory json = string(abi.encodePacked(_d1, name, _d2, imgstr, _d3, disc, _d4));
        ...
        returns (string memory)
    {
        ...
        string memory img = super.tokenURI(tokenId);
        return string(abi.encodePacked("data:application/json,", img));
    }
