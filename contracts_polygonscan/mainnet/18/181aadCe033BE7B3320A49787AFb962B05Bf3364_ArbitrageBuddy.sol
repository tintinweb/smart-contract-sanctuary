...
/// @dev Compare price denominated in quote token between two pools
        ...
        )
    {
        ...
        if (pool0Reserve0 == 0 || pool0Reserve1 == 0 || pool1Reserve0 == 0 || pool1Reserve1 == 0) {
            emptyPool = true;
        }
        else
        {
    ...
    function getOrderedReserves_external(address pool0,address pool1,bool baseTokenSmaller) external view returns (address lowerPool,address higherPool, OrderedReserves memory orderedReserves, bool emptyPool)
    {
        (lowerPool,higherPool, orderedReserves, emptyPool) = getOrderedReserves(pool0, pool1, baseTokenSmaller);
    }
    ...
    function getProfit(address pool0, address pool1) internal view returns (uint256 profit, address baseToken, uint256 borrowAmount) {
        ...
        (, , OrderedReserves memory orderedReserves, bool emptyPool) = getOrderedReserves(pool0, pool1, baseTokenSmaller);
    ...
    function flashArbitrage(address pool0, address pool1) external {
        ...
        OrderedReserves memory orderedReserves;
        (info.lowerPool, info.higherPool, orderedReserves, emptyPool) = getOrderedReserves(pool0, pool1, info.baseTokenSmaller);
        if(emptyPool == true)
        {
            return;
        }
    ...
    function calculateBorrowTerms(OrderedReserves memory reserves) internal pure returns (uint256 d, int256 d_AccuracyReduction) {
        ...
        {
            dShift = (bitInfo.abcMaxBitsNeeded- 254) / 4 + 1;
            updateBitInfoABC(reserves, bitInfo, dShift);
        }
        ...
        if (bitInfo.bitsNeeded_m >= 172)
        {
            d_AccuracyReductionShift = (bitInfo.bitsNeeded_m - 172) / 2 + 1;
            updateBitInfoM(bitInfo, d_AccuracyReductionShift);
        }

        d_AccuracyReduction = int256(1 << d_AccuracyReductionShift);
        d = 1 << dShift;
    }
    ...
    function calcBorrowAmount(OrderedReserves memory reserves) internal pure returns (uint256 amount) {
        ...
        require(d_AccuracyReduction > 0, append("d_AccuracyReduction <= 0 => d_AccuracyReduction: ", uint2str(uint256(d_AccuracyReduction)))); 
