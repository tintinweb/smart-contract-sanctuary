...
contract FlatAirNFT is ERC721, Ownable {
    ...
    event BatchMinted(address indexed minter, uint batchSize);
    ...
    function batchMint(string[] memory _tokenURIs, address _batchMinter, uint[] memory _prices) public returns (bool) {
        require(_tokenURIs.length > 0, "_tokenURIs size should be bigger than 0");
        require(_tokenURIs.length == _prices.length, "_tokenURIs and _prices should have same size");

        uint _batchSize = _tokenURIs.length;
        for (uint i = 0; i < _batchSize; i++) {
            mint(_tokenURIs[i], _batchMinter, _prices[i]);
        }

        emit BatchMinted(_batchMinter, _batchSize);
        return true;
    }
