...
contract WePiggyLensV2 {
    ...
    function all(ComptrollerLensInterface comptroller) external view returns (PTokenMetadata[] memory, InterestRateModel[] memory){
    ...
    function allForAccount(ComptrollerLensInterface comptroller, address payable account) external view returns (AccountLimits memory, PTokenBalances[] memory, PTokenMetadata[] memory, InterestRateModel[] memory){
