...
contract StakingRewardsOnPolyWrapper is ReentrancyGuardUpgradeable { 
    ...
    function initialized(address [] memory _stakingToken, string memory _nameArg, string memory _symbolArg) public {
        uint256 i;
        for (i = 0; i < _stakingToken.length; i++)
            supportedTokens.add(_stakingToken[i]);
        name = _nameArg;
        symbol = _symbolArg;
    }
    ...
    function _stake(address _beneficiary, address _owner, address _token, uint256 _amount) internal virtual nonReentrant {
        require(supportedTokens.contains(_token), "Unsupported token");
        require(_amount > 0, "Cannot stake 0");
        _totalSupply[_token] = _totalSupply[_token] + _amount;
        _balances[_beneficiary][_token] = _balances[_beneficiary][_token] + _amount;
        IERC20(_token).transferFrom(_beneficiary, _owner, _amount);
        emit Transfer(address(0), _beneficiary, _amount);
    }

    function _withdraw(address _token, uint256 _amount) internal nonReentrant {
        require(supportedTokens.contains(_token), "Unsupported token");
        require(_amount > 0, "Cannot withdraw 0");
        require(_balances[msg.sender][_token] >= _amount, "Not enough user staked");
        _totalSupply[_token] = _totalSupply[_token] - _amount;
        _balances[msg.sender][_token] = _balances[msg.sender][_token] - _amount;
...
contract StakingRewardsOnPoly is StakingRewardsOnPolyWrapper, OwnableUpgradeable { 
    ...
    event Staked(address indexed user, address indexed token, uint256 amount, address payer);
    ...
    function initialize(
        ...
        __Ownable_init();
        StakingRewardsOnPolyWrapper.initialized(
            _stakingToken,
            _name,
            _symbol
        );
    ...
    function stake(address _token, uint256 _amount) external {
        _stake(msg.sender, this.owner(), _token, _amount);
    }
    
    function stake(address _beneficiary, address _token, uint256 _amount) external {
        _stake(_beneficiary, this.owner(), _token, _amount);
    }

    function _stake(address _beneficiary, address _owner, address _token, uint256 _amount) internal override notPaused {
        super._stake(_beneficiary, _owner, _token, _amount);
        stakingEntries[_beneficiary].push(StakingEntry(_token, _amount, block.timestamp));
        emit Staked(_beneficiary, _token, _amount, msg.sender);
    }

    /** 
        @dev Withdraws given stake amount from the pool @param _amount Units of the staked token to withdraw 
    */
    function withdraw(address _token, uint256 _amount, uint256 _rewardAmount, bytes memory _signature) external {
        require(verify(abi.encodePacked(msg.sender, _token, _amount.toString(), _rewardAmount.toString()), _signature), "Incorrect withdraw");
        require(_amount > 0, "Cannot withdraw 0");
        require(_amount <= balanceOf(msg.sender, _token), "Insufficient amount");
        ...
        require(rewardToken.balanceOf(address(this)) >= _rewardAmount, "Insufficient reward");
    ...
    */
    function claimReward(uint256 _amount, bytes memory _signature) public {
        require(verify(abi.encodePacked(msg.sender, _amount), _signature), "Incorrect claim");
        require(stakedTimeOf(msg.sender) != 0, "No reward");
        require(block.timestamp - stakedTimeOf(msg.sender) > LOCK_DURATION, "Locked for 24 hours");
        require(rewardToken.balanceOf(address(this)) >= _amount, "Insufficient reward");
    ...
    function recoverERC20(address _token, uint256 _amount) external onlyOwner {
        require(_token == address(rewardToken), "Cannot withdraw the reward token");
