...
contract TestCGPresaleTest is ReentrancyGuard {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;

    // The number of unclaimed TESTCG tokens the user has
    mapping(address => uint256) public testCGUnclaimed;
    // Last time user claimed TESTCG
    mapping(address => uint256) public lastTestCGClaimed;

    // Addresses that excluded 
    mapping(address => bool) private _whiteList;

    // TESTCG token
    IBEP20 public TESTCG;
    // USDC token
    IBEP20 public USDC;
    // whitelist active
    ...
    // Total TESTCG sold
    uint256 public totalTestCGSold = 0;

    // Price of presale TESTCG: 0.01 USDC
    uint256 private constant USDCPerCGS = 1;

    // Time per percent
    uint256 private constant timePerPercent = 60;

    // Max Buy Per User
    uint256 public maxBuyPerUser = 100*(1e6); // decimal 6

    uint256 public firstHarvestTimestamp;

    address payable owner;

    uint256 public constant TESTCG_HARDCAP = 1000*(1e6);
    ...
    function buy(uint256 _amount, address _buyer) public nonReentrant {
        ...
        require(
            totalTestCGSold + tokens <= TESTCG_HARDCAP,
            "TestCG presale hardcap reached"
        );

        require(
            testCGUnclaimed[buyer] + tokens <= maxBuyPerUser,
            "Your amount exceeds the max buy number"
        );

        USDC.safeTransferFrom(buyer, address(this), _amount);

        testCGUnclaimed[buyer] = testCGUnclaimed[buyer].add(tokens);
        totalTestCGSold = totalTestCGSold.add(tokens);
    ...
    */
    ...
    function claim() external {
        ...
        require(
            testCGUnclaimed[msg.sender] > 0,
            "User should have unclaimed TESTCG tokens"
        );
        require(
            TESTCG.balanceOf(address(this)) >= testCGUnclaimed[msg.sender],
            "There are not enough TESTCG tokens to transfer."
        );

        if (lastTestCGClaimed[msg.sender] == 0) {
            lastTestCGClaimed[msg.sender] = firstHarvestTimestamp;
        }

        uint256 allowedPercentToClaim = block
        .timestamp
        .sub(lastTestCGClaimed[msg.sender])
        .div(timePerPercent);

        lastTestCGClaimed[msg.sender] = block.timestamp;
        ...
        uint256 testCGToClaim = testCGUnclaimed[msg.sender]
        .mul(allowedPercentToClaim)
        .div(100);
        testCGUnclaimed[msg.sender] = testCGUnclaimed[msg.sender].sub(testCGToClaim);

        testCGToClaim = testCGToClaim.mul(1e12);
        TESTCG.safeTransfer(msg.sender, testCGToClaim);
        emit TokenClaim(msg.sender, testCGToClaim);
    }
    ...
    function withdrawUnsoldTESTCG() external onlyOwner {
        uint256 amount = TESTCG.balanceOf(address(this)) - totalTestCGSold;
        TESTCG.safeTransfer(msg.sender, amount);
    }

    function emergencyWithdraw() external onlyOwner {
        TESTCG.safeTransfer(msg.sender, TESTCG.balanceOf(address(this)));
    }
    ...
    */
    ...
    function updateMaxBuyPerUser(uint256 _maxBuyPerUser) external onlyOwner {
        require(_maxBuyPerUser <= TESTCG_HARDCAP, "TESTCG::updateMaxBuyPerUser: maxBuyPerUser must not exceed the hardcap.");
