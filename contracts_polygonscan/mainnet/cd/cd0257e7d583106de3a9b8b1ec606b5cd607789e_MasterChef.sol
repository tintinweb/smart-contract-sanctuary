...
contract PolyPolyRock is BEP20('Poly PolyRock', 'PolyPolyRock') {
...
contract MasterChef is Ownable {
    ...
    PolyPolyRock public PolyRock;
    // Dev address.
    address public devaddr;
    // EGG tokens created per block.
    uint256 public PolyRockPerBlock;
    // Bonus muliplier for early PolyRock makers.
    ...
    constructor(
        PolyPolyRock _PolyRock,
        address _devaddr,
        address _feeAddress,
        uint256 _PolyRockPerBlock,
        uint256 _startBlock
    ) public {
        PolyRock = _PolyRock;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        PolyRockPerBlock = _PolyRockPerBlock;
    ...
    function pendingEgg(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 PolyRockReward = multiplier.mul(PolyRockPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accEggPerShare = accEggPerShare.add(PolyRockReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 PolyRockReward = multiplier.mul(PolyRockPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        PolyRock.mint(devaddr, PolyRockReward.div(10));
        PolyRock.mint(address(this), PolyRockReward);
        pool.accEggPerShare = pool.accEggPerShare.add(PolyRockReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    // Safe PolyRock transfer function, just in case if rounding error causes pool to not have enough EGGs.
    function safeEggTransfer(address _to, uint256 _amount) internal {
        uint256 PolyRockBal = PolyRock.balanceOf(address(this));
        if (_amount > PolyRockBal) {
            PolyRock.transfer(_to, PolyRockBal);
        } else {
            PolyRock.transfer(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _PolyRockPerBlock) public onlyOwner {
        massUpdatePools();
        PolyRockPerBlock = _PolyRockPerBlock;
    }
