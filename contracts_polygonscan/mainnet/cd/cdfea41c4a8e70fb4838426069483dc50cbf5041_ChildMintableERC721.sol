 ...
 */
abstract contract ERC721URIStorage is ERC721PresetMinterPauserAutoId {
    using Strings for uint256;
...
contract ChildMintableERC721 is IChildToken, ERC721URIStorage {
    ...
    constructor(
    ) ERC721PresetMinterPauserAutoId("EncodeKeys", "ENCK") {
    ...
    function mint(
     ...
     function mint(address user, string memory _tokenURI) public {
        require(hasRole(MINTER_ROLE, msg.sender));
        require(!withdrawnTokens[_tokenIdCounter.current()], "ChildMintableERC721: TOKEN_EXISTS_ON_ROOT_CHAIN");
        _tokenIdCounter.increment();
        _mint(user, _tokenIdCounter.current());
