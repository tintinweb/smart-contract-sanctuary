...
contract ShortLock is Ownable {
    ...
    mapping(uint256 => PositionLockInfo) public lockInfoMap;

    constructor(address mintAddr_, uint256 lockPeriod_) {
        mintAddr = mintAddr_;
    ...
    function lock(
        ...
        if (lockInfo.assigned) {
            require(
                lockInfo.receiver == receiver,
                "lock: wrong receiver address."
            );
            require(
                lockInfo.lockedToken == IERC20(token),
                "lock: a strange token address."
            );
            lockInfo.lockedAmount += amount;
            lockInfo.unlockTime = block.timestamp + lockPeriod;
        } else {
            lockInfo = PositionLockInfo(
                positionId,
                receiver,
                IERC20(token),
                amount,
        ...
        if (token == NATIVE_TOKEN) {
            ...
            lockInfo.lockedToken.safeTransferFrom(
                msg.sender,
                address(this),
                amount
            );
    ...
    /// @dev Only Mint contract can execute.
    /// @param positionId Position index in Mint contract.
    function release(uint256 positionId) external onlyMint {
        PositionLockInfo memory lockInfo = lockInfoMap[positionId];
        if (lockInfo.assigned == false) {
            return;
        }
        if (address(lockInfo.lockedToken) == NATIVE_TOKEN) {
            payable(msg.sender).transfer(lockInfo.lockedAmount);
    ...
    function setMintAddr(address _mintAddr) external onlyOwner {
        require(
            _mintAddr != address(0),
            "setMintAddr: cannot set a zero address."
        );
        mintAddr = _mintAddr;
    }

    function setLockPeriod(uint256 lockPeriod_) external onlyOwner {
        lockPeriod = lockPeriod_;
    }
