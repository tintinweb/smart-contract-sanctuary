...
contract MoonlightKingdomGames is Ownable {
    ...
    function fightSimulationCalculated(uint256 gAtk, uint256 gDef, uint256 _gMulti, uint256 mAtk, uint256 mDef) public view returns (uint[4] memory) {
        ...
        uint maxGuardianDmg = (((gAtk*_gMulti) /100) *5)/ mDef;
        uint maxMonsterDmg = (mAtk*5)/ gDef;
        uint guardianHit=0;
        uint monsterHit=0;
        bool finish =false;
        ...
        while (finish == false) 
        {
            guardianHit = randomNum(maxGuardianDmg,block.timestamp,maxGuardianDmg+15);
            guardianHittotal =  guardianHittotal + guardianHit;
            
            monsterHit =  randomNum(maxMonsterDmg,block.timestamp,maxMonsterDmg+20);
            monsterHittotal = monsterHittotal + monsterHit;
            
            if (guardianHittotal >= 100 || monsterHittotal >= 100){
                finish = true;
            }
        }
