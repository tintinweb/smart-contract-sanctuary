...
contract Airdrop is Ownable, Pausable, ReentrancyGuard {
    ...
    /// @dev address of User token holder from which transfer to recipient when claiming tokens
     ...
     * @param _token User token address
     */
    constructor(
        ...
        require(_reserveAddress != address(0), "UserAirdrop: invalid reserve address");
        require(_claimSigner != address(0), "UserAirdrop: invalid claim signer address");
        require(_token != address(0), "UserAirdrop: invalid token address");
    ...
    function claimTokens(
        ...
        require(_recipient != address(0), "UserAirdrop: invalid recipient address");
        require(_amount > 0, "UserAirdrop: invalid amount");
        require(!claimed[_recipient], "UserAirdrop: token already claimed to recipient");

        claimed[_recipient] = true;

        bytes32 message = keccak256(abi.encodePacked(_recipient, _amount));

        require(ECDSA.recover(ECDSA.toEthSignedMessageHash(message), _signature) == claimSigner, "UserAirdrop: invalid signature");
