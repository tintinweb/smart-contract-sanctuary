    ...
    Access_Control
{
    ...
    function reSupply(
        ...
        require(exists(_tokenId), "TKN N/A");
        require(getTokenMinter(_tokenId) == msg.sender, "NMINTP");
    ...
    function mint_internal(
        ...
        require(getWhiteListed(msg.sender), "WLF");
        require(!getBlackListed(msg.sender), "BANNED");
    ...
    Storage
{
    ...
    function updateTokenUri(uint256 _tokenId, string calldata _uri) external {
        ...
        require(!getBlackListed(msg.sender), "BANNED");
        require(
            getTokenMinter(_tokenId) == msg.sender,
            "NMINTP"
        );
...
contract Access_Control is Initializable, OwnableUpgradeable, Storage {
    ...
    function setWhiteList(address _addr) external {
        ...
        require(!getWhiteListed(msg.sender), "WLT");
...
contract Storage is Initializable {
    ...
    function getWhiteListed(address _addr) internal view returns (bool) {
        return whiteListed[_addr];
    }

    function getBlackListed(address _addr) internal view returns (bool) {
        return blackListed[_addr];
    }

    function getTokenMinter(uint256 _tokenId) internal view returns (address) {
