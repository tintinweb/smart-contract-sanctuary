...
// File: contracts/libs/IDimStrategy.sol
pragma solidity 0.6.12;

// For interacting with our own DimStrat
interface IDimStrategy {
    // Want address
    ...
    // Total number of tokens managed by DimStrat that is currently staked in StakingRewardsAddr
    function wantLockedTotal() external view returns (uint256);

    // Sum of all shares of users to wantLockedTotal
    function sharesTotal() external view returns (uint256);

    // Calling tunnel() will auto-compound earned tokens and add them back to staking rewards
    function tunnel() external;

    // Transfer want tokens DimensionChef -> strategy
    function deposit(address _userAddress, uint256 _wantAmt) external returns (uint256);

    // Transfer want tokens strategy -> DimensionChef
    function withdraw(address _userAddress, uint256 _wantAmt) external returns (uint256);
}

// File: contracts/DimChef.sol
pragma solidity 0.6.12;

contract DimChef is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        IERC20 earned; // Address of the earned token.
        address dimStrat; // Strategy address that will autocompound want tokens
    }

    PoolInfo[] public poolInfo; // Info of each pool.
    mapping(uint256 => mapping(address => UserInfo)) public userInfo; // Info of each user for shares of want tokens.
    mapping(address => bool) private dimStrats;

    event AddPool(address indexed dimStrat);
     ...
     */
    function addPool(address _dimStrat) external onlyOwner nonReentrant {
        require(!dimStrats[_dimStrat], "Existing strategy");
        IERC20(IDimStrategy(_dimStrat).wantAddress()).allowance(address(this), address(_dimStrat));

        poolInfo.push(
            PoolInfo({
                want: IERC20(IDimStrategy(_dimStrat).wantAddress()),
                earned: IERC20(IDimStrategy(_dimStrat).earnedAddress()),
                dimStrat: _dimStrat
            })
        );

        dimStrats[_dimStrat] = true;
        resetSingleAllowance(poolInfo.length.sub(1));
        emit AddPool(_dimStrat);
    }
    ...
    function stakedWantTokens(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 sharesTotal = IDimStrategy(pool.dimStrat).sharesTotal();
        uint256 wantLockedTotal = IDimStrategy(poolInfo[_pid].dimStrat).wantLockedTotal();
        if (sharesTotal == 0) {
            return 0;
        }
    ...
    // Want tokens moved from user -> this -> dimStrat (staking)
    ...
    function _deposit(uint256 _pid, uint256 _wantAmt, address _to) internal {
        ...
        if (_wantAmt > 0) {
            ...
            uint256 sharesAdded = IDimStrategy(poolInfo[_pid].dimStrat).deposit(_to, _wantAmt);
            user.shares = user.shares.add(sharesAdded);
        }
        emit Deposit(_to, _pid, _wantAmt);
    }

    // Withdraw want tokens from DimChef.
    ...
    function _withdraw(uint256 _pid, uint256 _wantAmt, address _to) internal {
        ...
        uint256 wantLockedTotal = IDimStrategy(poolInfo[_pid].dimStrat).wantLockedTotal();
        uint256 sharesTotal = IDimStrategy(poolInfo[_pid].dimStrat).sharesTotal();
        ...
        if (_wantAmt > 0) {
            uint256 sharesRemoved = IDimStrategy(poolInfo[_pid].dimStrat).withdraw(msg.sender, _wantAmt);
    ...
    function resetAllowances() external onlyOwner {
        for (uint256 i=0; i<poolInfo.length; i++) {
            ...
            pool.want.safeApprove(pool.dimStrat, uint256(0));
            pool.want.safeIncreaseAllowance(pool.dimStrat, uint256(-1));
        }
