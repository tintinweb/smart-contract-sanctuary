 ...
 */
contract RevestA1 is IRevest, AccessControlEnumerable, RevestAccessControl, RevestReentrancyGuard {
    ...
    function withdrawFNFT(uint fnftId, uint quantity, bytes memory args) external revestNonReentrant(fnftId) {
        ...
        if(config.pipeToContract.supportsInterface(OUTPUT_RECEIVER_INTERFACE_V2_ID)) {
            ...
            IOutputReceiverV2(config.pipeToContract).receiveSecondaryCallback(fnftId, payable(_msgSender()), quantity, config, args);
        }
    }
    ...
    function splitFNFT(
        ...
        if(output != address(0) && output.supportsInterface(OUTPUT_RECEIVER_INTERFACE_V2_ID)) {
            // Callback to OutputReceiverV2
            IOutputReceiverV2(output).handleFNFTRemaps(fnftId, newFNFTIds, fnftHandler.getSupply(fnftId) == 0);
        }
    ...
    function depositAdditionalToFNFT(
        ...
        if(createNewSeries && fnft.pipeToContract != address(0) && fnft.pipeToContract.supportsInterface(OUTPUT_RECEIVER_INTERFACE_V2_ID)) {
            // Callback to OutputReceiverV2
            uint[] memory newIds = new uint[](1);
            newIds[0] = newFNFTId;
            // Cleanup arg is false because this operation will never result in an empty series of FNFTs
            IOutputReceiverV2(fnft.pipeToContract).handleFNFTRemaps(fnftId, newIds, false);
        }
