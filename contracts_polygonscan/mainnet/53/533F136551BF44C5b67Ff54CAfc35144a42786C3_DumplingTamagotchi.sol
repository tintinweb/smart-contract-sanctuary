...
contract DumplingTamagotchi is Base, ITamagotchi {

    event Fed(uint indexed _id, bool indexed _fed);
    event Resurrected(uint indexed _id);
    event Rested(uint indexed _id);
    event InstantExhaustionResetBought(uint indexed _id);

    constructor(address _constantsContract) Base(_constantsContract) {}

    function isPooAlive(uint _id) public override view returns (bool _alive) {
        uint hunger = getHunger(_id);
        if (hunger == 100) {
            return false;
        }
        return true;
    ...
    function isPooExhausted(uint _id) public override view returns (bool _exhausted) {
        return IDumplings(IConstants(constantsContract).DumplingsContract()).getDumplingByIndex(_id).exhaustion > 80;
    }

    function getRestAndHungerOfDumplingByIndex(uint _id) public view returns (uint _restPoints, uint _hunger, bool _alive) {
        uint restPoints = getRestPoints(_id);
        uint hunger = getHunger(_id);
        bool alive = isPooAlive(_id);
        return (restPoints, hunger, alive);
    }

    function getHunger(uint _id) public override view returns (uint _hungerLevel) {
        uint lastTimeFed = IDumplings(IConstants(constantsContract).DumplingsContract()).getDumplingByIndex(_id).lastTimeFed;
        uint hungerLevel = (block.number - lastTimeFed) / IConstants(constantsContract).blocksBetweenHungerPointForDumpling();
        if (hungerLevel > 100) {
            hungerLevel = 100;
        }
        return hungerLevel;
    ...
    function getRestPoints(uint _id) public override view returns (uint) {
        address DumplingsContract = IConstants(constantsContract).DumplingsContract();
        uint lastTimeRested = IDumplings(DumplingsContract).getDumplingByIndex(_id).lastTimeRested;
        uint rested = (block.number - lastTimeRested) / IConstants(constantsContract).blocksBetweenRestPoint();
        if (rested > 100) {
            rested = 100;
        }
        return rested;
    ...
    function rest(uint _id) public {
        uint restPoints = getRestPoints(_id);
        address DumplingsContract = IConstants(constantsContract).DumplingsContract();
        IDumplings(DumplingsContract).executeRest(_id, restPoints);
        emit Rested(_id);
    }

    function buyInstantExhaustionReset(uint _id) public {
        payByPoo(IConstants(constantsContract).pooTokenForInstantExhaustionReset());
        IDumplings(IConstants(constantsContract).DumplingsContract()).executeRest(_id, 100);
        emit InstantExhaustionResetBought(_id);
    }

    function feed(uint _id) public {
        payByPoo(IConstants(constantsContract).pooTokenForFeed());
        address DumplingsContract = IConstants(constantsContract).DumplingsContract();
        IDumplings(DumplingsContract).executeFeed(_id);

        if (isPooAlive(_id) == true) {
            emit Fed(_id, true);
        } else {
            IDumplings(DumplingsContract).executeAliveChange(_id, false);
            emit Fed(_id, false);
        }
    }

    function resurrect(uint _id) public {
        payByPoo(IConstants(constantsContract).pooTokenForResurrect());
        address DumplingsContract = IConstants(constantsContract).DumplingsContract();
        IDumplings(DumplingsContract).executeResurrect(_id);
        IDumplings(DumplingsContract).executeRest(_id, 100);
        emit Resurrected(_id);
    }
}
