...
contract SafeCollateralReserve is OwnableUpgradeSafe, ISafeCollateralReserve {
    using SafeMath for uint256;
    ...
    function initialize(
        ...
        if (_collateral == usdc) {
            ...
            shareToCollateralRouterPath = [_share, usdc, _collateral];
        }
    }
    ...
    function receiveShares(uint256 _amount) external override onlyTreasury {
        ...
        uint8 _reserve_share_state = ISafeTreasury(treasury).reserve_share_state();
        if (_reserve_share_state == 2) {
            _sellSharesToCollateral(_amount.div(2)); // sold 50%
    ...
    function _sellSharesToCollateral(uint256 _amount) internal {
        ...
        IUniswapV2Router(router).swapExactTokensForTokensSupportingFeeOnTransferTokens(_amount, 1, shareToCollateralRouterPath, address(this), now.add(60));
        uint256 _after = IERC20(collateral).balanceOf(address(this));
        emit SwapToken(share, collateral, _amount, _after.sub(_before));
    }
