...
contract TomiStaking is Ownable, ReentrancyGuard, Pausable, Initializable {
    ...
    // FE functions
    function getRewardRevenueShare(address account) public view returns (uint256) {
    ...
    function getTotalRewardPool() external view returns (uint256) {
        ...
        uint256 remainRewardCommunityPool = (periodFinish.sub(block.timestamp))
            .mul(rewardRate.mul(rewardsDuration))
            .div(rewardsDuration);
    ...
    modifier onlyRewardsDistribution() {
        require(msg.sender == rewardsDistribution, 'TomiStaking: caller is not reward distribution');
        _;
    }

    modifier PoolAndGovernance() {
        require(
            (msg.sender == tomiPool) || (msg.sender == tomiGovernance),
            'TomiStaking: caller is not TomiPool or TomiGovernance'
        );
        _;
    }
