...
// 
contract NftStaking is IPool, Governance {
    ...
    address public _dividendPool = address(0x0);
    address public _cifiPool = address(0x0);

    uint256 public constant DURATION =  3 days; // 28 days;
    uint256 public _initReward = 11574 * 1e8; //0.1 per day //173611 * 1e10; // 150 per day;
    ...
    uint256 public _LiquidityRate = 500; //400;
    ...
    uint256 public _maxStakedCifi = 200 * 1e18;
    ...
    constructor(
        address cifi,
        address nftAsset /*, address nftFactory*/
    ) {
        _cifi = ICifi_Token(cifi);
        _nftAsset = INftAsset(nftAsset);
        _teamWallet = msg.sender;
        _dividendPool = msg.sender;
        _cifiPool = msg.sender;
    }
    ...
    3	stakingPowa	1.3+ 1*(stakingPowa-8000/1000
    4	stakingPowa	1.4+ 1.5*(stakingPowa-9000)/900
    5	stakingPowa	1.5+ 1.5*(stakingPowa-9900)/75
    6	stakingPowa	1.6+ 1.5*(stakingPowa-9975)/25
    */
        ...
        returns (uint256)
    {
        ...
        if (class == 1) {
            ...
            unfold = (stakingPowa.sub(8000) * 10000) / 1000;
            return unfold.add(13000);
        } else if (class == 4) {
            unfold = (stakingPowa.sub(9000) * 15000) / 900;
            return unfold.add(14000);
        } else if (class == 5) {
            unfold = (stakingPowa.sub(9900) * 15000) / 75;
            return unfold.add(15000);
        } else {
            unfold = (stakingPowa.sub(9975) * 15000) / 25;
            return unfold.add(18000);
        }
    }

    function getClass(uint256 stakingPowa) public view returns (uint256) {
        if (stakingPowa < _stakingPowaBase.mul(1000).div(2000)) {
            return 1;
        } else if (
            _stakingPowaBase.mul(1000).div(2000) <= stakingPowa &&
            stakingPowa < _stakingPowaBase.mul(1600).div(2000)
        ) {
            return 2;
        } else if (
            _stakingPowaBase.mul(1600).div(2000) <= stakingPowa &&
            stakingPowa < _stakingPowaBase.mul(1800).div(2000)
        ) {
            return 3;
        } else if (
            _stakingPowaBase.mul(1800).div(2000) <= stakingPowa &&
            stakingPowa < _stakingPowaBase.mul(1980).div(2000)
        ) {
            return 4;
        } else if (
            _stakingPowaBase.mul(1980).div(2000) <= stakingPowa &&
            stakingPowa < _stakingPowaBase.mul(1995).div(2000)
        ) {
            return 5;
        } else {
            return 6;
    ...
    function divideDeduction(uint256 amount) internal {
        require(amount > 0, "invalid amount");
        uint256 dividendReward = amount.mul(_LiquidityRate).div(_teamRewardRate);
        if (dividendReward > 0) {
            _cifi.transfer(_dividendPool, dividendReward);
        }
        uint256 teamReward = amount.mul(_teamRewardRate.sub(_LiquidityRate)).div(
            _teamRewardRate
        );
        if (teamReward > 0) {
            _cifi.transfer(_teamWallet, teamReward);
        }
    }
    ...
    function getReward() public updateReward(msg.sender) checkHalve checkStart {
        ...
        if (reward > 0) {
            ...
            uint256 harvestReward = reward.mul(_teamRewardRate).div(_baseRate);
            divideDeduction(harvestReward);
            uint256 leftReward = reward.sub(harvestReward);
            uint256 penaltyReward = 0;

            //withdraw time check

            if (block.timestamp < (_lastStakedTime[msg.sender] + _punishTime)) {
                penaltyReward = leftReward.mul(_penaltyRate).div(_baseRate);
            }
            if (penaltyReward > 0) {
                divideDeduction(penaltyReward);
                leftReward = leftReward.sub(penaltyReward);
            }

            if (leftReward > 0) {
                _cifi.transfer(msg.sender, leftReward);
            }
    ...
    modifier checkHalve() {
        if (block.timestamp >= _periodFinish) {
            ...
            // _cifi.transferFrom(_cifiPool, address(this), _initReward);
    ...
    function setDividenRate(uint256 dividendRate) public onlyGovernance {
        _LiquidityRate = dividendRate;
    }
    ...
    function setDividenPool(address dividendPool) public onlyGovernance {
        _dividendPool = dividendPool;
    }

    function setCifiPool(address cifiPool) public onlyGovernance {
        _cifiPool = cifiPool;
    }
