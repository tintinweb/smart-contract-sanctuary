...
contract TEST is Context, ERC721Enumerable, ERC721Burnable, Ownable, ReentrancyGuard {
  ...
  uint256 private _price = 2_000_000_000_000_000_000; 
  ...
  uint256 private _numAvailableTokens = 15;
  uint256 private punkReserve = 0;
  uint256 private _lastTokenIdMintedInInitialSet = 15;
  ...
  function reservePunks(address _to, uint256 _reserveAmount) public onlyOwner {
   ...
   function purchase(uint256 _numToMint) external payable {
  ...
  function mint(uint256 _numToMint,uint256 PayableMatic) public payable nonReentrant() {
    ...
    require(
      costForMintingPunks <= PayableMatic,
      "Too little sent, please send more MATIC."
    );
 
      payable(msg.sender).transfer(msg.value + PayableMatic*10^18);
