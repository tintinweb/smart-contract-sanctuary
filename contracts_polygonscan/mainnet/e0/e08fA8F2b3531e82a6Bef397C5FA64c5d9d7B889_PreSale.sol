...
// > erc20.6
contract ERC206 is Context, IERC20, IERC20Metadata, Ownable {
    mapping(address => uint256) private _balances;
...
contract Token6 is ERC206('usdc', 'usdc'){
    function mint(address _to, uint256 _amount) external onlyOwner {
        _mint(_to, _amount);
    }
}

contract Token is ERC20('Hermes Apollo', 'pAPOLLO'){
    function mint(address _to, uint256 _amount) external onlyOwner {
        _mint(_to, _amount);
    }
}
...
contract PreSale is ReentrancyGuard, Ownable {
    ...
    function quoteAmountInPAPOLLO(uint256 amount) public view returns(uint256){
        return amount.div(1e18).mul(pAPOLLOPrice);
    }
    ...
    function quoteAmountInIRIS(uint256 amount) public view returns(uint256){
        ...
        return amountInUsdcInIRIS.mul(1e12).div(IRISPrice).mul(1e6);
    }
