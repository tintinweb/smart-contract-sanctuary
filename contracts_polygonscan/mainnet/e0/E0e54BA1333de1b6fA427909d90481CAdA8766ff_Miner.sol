...
contract Miner is Ownable, ReentrancyGuard {
    ...
    // Allow MasterChef contract to accept token from Miner contract.
    ...
    function harvestMasterChef() internal {
        ...
        // Transfer some ROLL to ROLL game contract.
    ...
    // Set referrer address only during user's first bet.
    function recordReferrer(address _user, address _referrer) external onlyGame {
        ...
        if (userReferrer[_user] == address(0) && _referrer != _user) {
            if (_referrer == address(0)) {
                // If referrer is blank, set referrer to burn address as placeholder to acknowledge first bet is recorded.
