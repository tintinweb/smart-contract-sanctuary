...
contract StrategyXUSDCMATIC is StratManager, FeeManager {
    ...
    address constant public output = address(0x3A3e7650f8B9f667dA98F236010fBf44Ee4B2975);
    address public want;
    address public lpToken0;
    address public lpToken1;

    // Third party contracts
    address constant public masterchef = address(0xa00493d324cD342834eB657228c59d63b5EB2E89);
    uint8 public poolId;
    ...
    */
    ...
    constructor(
        ...
        if (lpToken0 == wmatic) {
            outputToLp0Route = [output, wmatic];
        } else if (lpToken0 == usdc) {
        ...
        if (lpToken1 == wmatic) {
            outputToLp1Route = [output, wmatic];
        } else if (lpToken1 == usdc) {
    ...
    function deposit() public whenNotPaused {
        ...
        if (pairBal > 0) {
            IMasterChef(masterchef).stake(pairBal);
        }
    }
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (pairBal < _amount) {
            IMasterChef(masterchef).withdraw(_amount.sub(pairBal));
    ...
    function harvest() external whenNotPaused onlyEOA {
        IMasterChef(masterchef).claimReward();
        chargeFees();
        addLiquidity();
        deposit();
    ...
    function balanceOfPool() public view returns (uint256) {
        (uint256 _amount) = IMasterChef(masterchef).balanceOf(address(this));
        return _amount;
    }
    ...
    function retireStrat() external {
        ...
        IMasterChef(masterchef).withdrawAndClaim();
