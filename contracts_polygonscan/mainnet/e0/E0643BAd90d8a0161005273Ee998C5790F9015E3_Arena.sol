...
contract Arena is Ownable, ERC721Holder {
    ...
    uint16 public usersLimit = 20;    
    uint16 public winnersAmount = 5;    
    uint public minLevel = 1;
    uint public maxLevel = 2;
    uint public neededFightAmount = 2;
    uint public winMoney = 1 * 1e8;
    ...
    // uint public preparingTime = 2 * 60 * 25;//mins
    // uint public fightingTime = 60 * 25;//mins
    // uint public cycleTime = 7 * 24 * 60 * 25;//mins
    
    uint public preparingTime = 20 * 25;//mins
    uint public fightingTime = 20 * 25;//mins
    uint public cycleTime = 50 * 25;//mins
    ...
    function fight(address enemy) public {
        ...
        for (uint8 i=0;i<3;i++) {
            if (cycle.health[myTeam[i]] > 0) {
                uint16 damage = attack(i,msg.sender,enemy, mySeed, 120) * 10 + uint16(i);
    ...
    function addMonsters(uint nftId1, uint nftId2, uint nftId3, uint gNftId1, uint gNftId2, uint gNftId3) public {
    ...
    function leave(uint cycleIndex) public {
        ...
        require(cycle.places[msg.sender] == 0, "you are dead");
        ...
        if (id2 > 0) {
         ...
         if (cycle.deadAmount + winnersAmount >= cycle.usersCount) {
            if (cycle.places[msg.sender] == 0) { // only five last standing or first dead
                usdc.safeTransfer(address(msg.sender), winMoney);
            } else  {
                if (cycle.places[msg.sender] + winnersAmount <= 5) {
                    genNFT.createToken(msg.sender, 5);
                } else if (cycle.places[msg.sender] + winnersAmount <= 10) {
                    genNFT.createToken(msg.sender, 4);
                } else if (cycle.places[msg.sender] + winnersAmount <= 15) {
