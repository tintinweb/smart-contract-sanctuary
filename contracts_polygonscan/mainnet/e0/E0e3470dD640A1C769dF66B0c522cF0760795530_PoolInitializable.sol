...
contract PoolInitializable is Ownable, ReentrancyGuard {
    ...
    // The block number of the last pool update, initialized on the first deposit
    uint256 public lastRewardBlock;

    // The pool limit (0 if none)
    uint256 public poolLimitPerUser;

    // Network block time 3 BSC, 3 Heco, 2 Polygon. 1 Fantom for auto adjust rewardPerBlock 0 DISABLE
     ...
     * @param _blockTime: network block time 3 BSC, 3 Heco, 2 Polygon, 1 Fantom for auto adjust rewardPerBlock 0 DISABLE
    ...
    function deposit(uint256 _amount) external nonReentrant {
        require(block.timestamp >= startTime, "Pool has not started yet");
        require(block.timestamp < endTime, "Pool has finished");
        ...
        // Initialize the lastRewardBlock on the first deposit
        if (lastRewardBlock == 0) lastRewardBlock = block.number;
