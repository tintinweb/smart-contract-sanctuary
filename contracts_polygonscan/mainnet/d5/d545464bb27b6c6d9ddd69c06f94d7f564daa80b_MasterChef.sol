...
// IronSwap with Governance.
contract IronSwap is BEP20('TITANv2 Token', 'ICE') {
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    event UpdateEmissionRate(address indexed user, uint256 ICEPerBlock);
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 ICEReward = multiplier.mul(ICEPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        ICE.mint(devaddr, ICEReward.div(10));
    ...
    function updateEmissionRate(uint256 _ICEPerBlock) public onlyOwner {
        massUpdatePools();
        ICEPerBlock = _ICEPerBlock;
        emit UpdateEmissionRate(msg.sender, _ICEPerBlock);
    }
