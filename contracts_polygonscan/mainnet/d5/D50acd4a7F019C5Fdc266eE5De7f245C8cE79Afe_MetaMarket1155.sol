...
// 艺术品交易合约
contract MetaMarket1155 is ReentrancyGuardUpgradeable, OwnableUpgradeable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    IMetaHolder public holder;
    IXhgt public xhgt;

    // 交易费用比例 2.5%
    uint256 public tradeFeeRate = 250;
    // 挖矿数量
    uint256 public xhgtMintAmount = 10 * 10 ** 18;
    ...
    mapping(address => address[2]) public payTokens; // tokenContract => [priceToken, usdtPair]
    mapping(address => bool) public blackList;

    uint256 mintAmount; // 交易挖矿最低交易额(USDT)
    address usdtAddr;

    address private operator;
    enum Operations { SET_OPERATOR, SET_FEE, SET_BLACK_LIST, SET_PAY_TOKEN, SET_XHGT_MINT_AMOUNT, SET_MINT_AMOUNT }
    ...
    function initialize(IMetaHolder _holder, IXhgt _xhgt, address _operator) external initializer {
    ...
    function getPayValue(address _payToken, uint256 _usdtAmount) view internal returns (uint256) {
        address _priceToken = payTokens[_payToken][0];
        address _usdtPair = payTokens[_payToken][1];
        require(_priceToken != address(0), 'Not supported pay token');
        uint256 _tokenBalance = IERC20(_priceToken).balanceOf(_usdtPair);
        uint256 _usdtBalance = IERC20(usdtAddr).balanceOf(_usdtPair);
        require(_tokenBalance != 0, 'Wrong pair');

        return _tokenBalance.mul(_usdtAmount).div(_usdtBalance);
    }
    ...
    function setMintAmount(uint256 _mintAmount, address _usdtAddr) public onlyOperator notLocked(Operations.SET_MINT_AMOUNT) {
        mintAmount = _mintAmount;
        usdtAddr = _usdtAddr;
        timelock[Operations.SET_MINT_AMOUNT] = 0;
    }

    function setPayToken(address _payToken, address[2] memory _priceConfig) external onlyOperator notLocked(Operations.SET_PAY_TOKEN) {
        payTokens[_payToken] = _priceConfig;
        timelock[Operations.SET_PAY_TOKEN] = 0;
    }

    function setXhgtMintAmount(uint256 _mintAmount) public onlyOperator notLocked(Operations.SET_XHGT_MINT_AMOUNT) {
        xhgtMintAmount = _mintAmount;
        timelock[Operations.SET_XHGT_MINT_AMOUNT] = 0;
    }
    ...
    function putOnSale(IERC1155 _nftToken, uint256 _tokenId, address _payToken, uint256 _price, uint256 _count) external {
        ...
        require(payTokens[_payToken][0] != address(0), 'Not supported pay token');
...
//        require(blackList[address(_nftToken)] == false, 'Not supported');
    ...
    function buyArt(IERC1155 _nftToken, address _seller, uint256 _tokenId, uint256 _count, address _payToken, uint256 _price) external payable {
        ...
        // 挖矿
        uint256 _mintAmount = getPayValue(_payToken, mintAmount);
        if (_price.mul(_count) >= _mintAmount) {
            xhgt.mint(msg.sender, xhgtMintAmount);
            xhgt.mint(_seller, xhgtMintAmount);
        }
