...
contract DEIProxy is Ownable {
	...
	uint public usdcForMintErrorRate = 970000;
	...
	function USDC2DEI(
		...
		(, uint deusNeeded)	= calcMintFractionalDEI(MintFD_Params(proxyInput.deusPriceUSD, proxyInput.colPriceUSD, proxyInput.usdcForMintAmount, globalCollateralRatio));
	...
	function getUSDC2DEIInputs(uint amountIn, uint deusPriceUSD, uint colPriceUSD) public view returns (uint amountOut, uint usdcForMintAmount) {
		...
		usdcForMintAmount =  (usdcNeededAmount * globalCollateralRatio / (1e6 - globalCollateralRatio)) * usdcForMintErrorRate / 1e6;
