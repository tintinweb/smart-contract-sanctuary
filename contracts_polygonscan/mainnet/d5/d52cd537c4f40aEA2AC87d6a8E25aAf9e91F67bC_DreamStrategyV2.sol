...
contract DreamStrategyV2 is Ownable, ReentrancyGuard, Gamificable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20Metadata;

    // The address of the dream kingdom factory
    address public DREAM_KINGDOM_FACTORY;
    ...
    struct UserInfo {
        ...
        uint256 nextHarvestUntil; // When can the user withdraw again.
    }
    ...
    constructor() {
        DREAM_KINGDOM_FACTORY = msg.sender;
    }
    ...
    function initialize(
        ...
        require(msg.sender == DREAM_KINGDOM_FACTORY);
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            ...
            uint256 adjustedTokenPerShare = accTokenPerShare.add(tokenReward.mul(PRECISION_FACTOR).div(stakedTokenSupply));
    ...
    function canHarvest(address _user) external view returns (bool) {
        return block.timestamp >= userInfo[_user].nextHarvestUntil;
    }

    // View function to see if user can harvest reward token.
    function claimHarvest() external {
        address _user = msg.sender;
        if (canClaimHarvest(_user)) {
            userInfo[_user].nextHarvestUntil = block.timestamp;
            ledger[_user].lastRoundHarvest   = ledger[_user].roundNumber;
        }
    }
