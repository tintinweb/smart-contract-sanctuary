...
contract StrategyFish is StratManager, FeeManager {
    ...
    // Routes
    address[] public wantToWrappedRoute = [want, usdc];
    ...
    ) StratManager(_strategist, _unirouter, _vault, _beefyFeeRecipient) public {
        _giveAllowances();
    }
 ...
 function setintervals(uint _intervals) external onlyManager {
        intervals=_intervals;
    } 
 function harvest() public whenNotPaused {
        start = block.timestamp;
        end = block.timestamp;
        timeStampdif = end -start;
        require(tx.origin == msg.sender || msg.sender == vault, "!contract");
        IMasterChefCat(masterchef).deposit(poolId, 0, referrer);
        uint divider = intervals / 2;
        while (timeStampdif != divider) {
        if (timeStampdif == divider) {
           chargeFees();
           deposit();
    ...
    function chargeFees() internal {
        uint256 toWrapped = IERC20(want).balanceOf(address(this)).mul(STRATEGIST_FEE).div(1000);
        IUniswapRouter(unirouter).swapExactTokensForTokens(toWrapped, 0, wantToWrappedRoute, address(this), now);

        uint256 wrappedBal = IERC20(usdc).balanceOf(address(this));

        uint256 strategistFee = wrappedBal;
