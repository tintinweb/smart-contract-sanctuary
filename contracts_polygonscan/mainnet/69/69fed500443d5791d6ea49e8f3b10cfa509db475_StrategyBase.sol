...
abstract contract StrategyFarmBase is BaseStrategyMasterChef {
    ...
    address[] public pswamp_mai_path;
        ...
        )
    {
        ...
        pswamp_mai_path = new address[](4);
        pswamp_mai_path[0] = pswamp;
        pswamp_mai_path[1] = matic;
        pswamp_mai_path[2] = usdc;
        pswamp_mai_path[3] = mai;
    }
    ...
    function harvest() public override {
        ...
        if (_pswampBalance > harvestCutoff){
            ...
            _swapUniswapWithPath(pswamp_mai_path, _pswampBalance.div(2));
            uint256 _pswampBalancePost = IERC20(pswamp).balanceOf(address(this));
            _swapUniswapWithPath(pswamp_usdc_path, _pswampBalancePost);

            // Add liquidity
            uint256 _mai = IERC20(mai).balanceOf(address(this));
            uint256 _usdc = IERC20(usdc).balanceOf(address(this));
            if (_mai > 0 && _usdc > 0) {
                IERC20(mai).safeApprove(currentRouter, 0);
                IERC20(mai).safeApprove(currentRouter, _mai);
...
contract StrategyBase is StrategyFarmBase {
    ...
    address public MiMatic_USDC_LP = 0x160532D2536175d65C03B97b0630A9802c274daD;
