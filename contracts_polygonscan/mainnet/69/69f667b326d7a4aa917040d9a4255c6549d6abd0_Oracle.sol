...
contract Oracle is IOracle {
    ...
    function _get(
        ...
        price = price / uint256(IAggregator(aggregator).latestAnswer());

        // @note decimals have to take into account the decimals of the vault asset
        return price / (10**(18 - uint256(IAggregator(aggregator).decimals())));
    }
    
    function getDataParam(address aggregator) public pure returns (bytes memory) {
        return abi.encode(aggregator);
    }
