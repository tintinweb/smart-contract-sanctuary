...
//All dual incentive pairs are WMATIC pairs
contract StrategyDualRewardsQuick is BaseStrategy {
    ...
    address[] public quick_wmatic_path;
        ...
        )
    {
        ...
        quick_wmatic_path = [QUICK, WMATIC];
        reward_other_path = [WMATIC, _otherToken];

        IERC20(WMATIC).safeApprove(currentRouter, uint256(-1));
        IERC20(QUICK).safeApprove(currentRouter, uint256(-1));
        if(otherToken != QUICK && otherToken != WMATIC) IERC20(otherToken).safeApprove(currentRouter, uint256(-1));

        IERC20(QUICK).safeApprove(DQUICK, uint256(-1));
        IERC20(DQUICK).safeApprove(DQUICK, uint256(-1));
    ...
    function harvest() public override onlyHumanOrWhitelisted nonReentrant {
        //Transfer WMATIC/QUICK/DQUICK that may already be in the contract to the fee dist fund
        uint256 dQuick_bal = IERC20(DQUICK).balanceOf(address(this));
        if(dQuick_bal > 0) {
            IERC20(DQUICK).safeTransfer(strategist, dQuick_bal);
        }
        uint256 matic_bal = IERC20(WMATIC).balanceOf(address(this));
        if(matic_bal > 0) {
            IERC20(WMATIC).safeTransfer(strategist, matic_bal);
        }
        uint256 quick_bal = IERC20(QUICK).balanceOf(address(this));
        if(quick_bal > 0) {
            IERC20(QUICK).safeTransfer(strategist, quick_bal);
        }

        _getReward();

        //Convert DQUICK to QUICK
        uint256 dQuickHarvested = IERC20(DQUICK).balanceOf(address(this));
        if (dQuickHarvested > 0) {
            IDragonLair(DQUICK).leave(dQuickHarvested);
        }
        //Convert QUICK to WMATIC
        uint256 amountHarvested2 = IERC20(QUICK).balanceOf(address(this));
        if (amountHarvested2 > 0) {
            _swapUniswapWithPathPreapproved(quick_wmatic_path, amountHarvested2);
        }
        //Deduct fee from the converted Quick + harvested wrapped
        uint256 amountHarvested = IERC20(WMATIC).balanceOf(address(this));
        ...
        //Swap 1/2 of the remaining WMATIC for otherToken
        uint256 remainingHarvested = IERC20(WMATIC).balanceOf(address(this));
        ...
        if (wmaticBalance > 0 && otherBalance > 0) {
            IUniswapRouterV2(currentRouter).addLiquidity(
                WMATIC, otherToken,
                wmaticBalance, otherBalance,
                0, 0,
                address(this),
                now + 60
            );
