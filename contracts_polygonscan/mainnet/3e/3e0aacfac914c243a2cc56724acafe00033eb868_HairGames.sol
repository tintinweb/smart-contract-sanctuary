...
// File: contracts/HairGames.sol

pragma solidity 0.6.12;

// HairGames is the dealer of random number games at Barbershop.finance
/*
v2 has the same logic as v1, with the following updates:
- Transaction Mining: Contract is linked to TransactionMiner contract to award HAIR tokens to gamblers who lost bets.
...
- Reward Computation: Calculates txn mining reward based on rewardPct, HAIR/MATIC price, bet amount, and probability of loss.
...
*/
contract HairGames is VRFConsumerBase, Ownable, ReentrancyGuard {
    ...
    // A bet amount of 50 ether will have a wealth tax of 2% in addition to house edge.
    // A bet amount of 100 ether will have a wealth tax of 4% in addition to house edge.
    uint public wealthTaxThreshold = 50 ether;
    uint public wealthTaxBP = 200;
    ...
    // Variables for computing price of HAIR in MATIC. Used for computing HAIR reward for transaction mining.
    uint constant SAFETY_FACTOR = 1e7; // Prevent truncation of price in integer datatype by multiplying with SAFETY_FACTOR.
    address constant HAIR_TOKEN = 0x100A947f51fA3F1dcdF97f3aE507A72603cAE63C;
    address constant MATIC_TOKEN = 0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270;
    address public hairMaticLP = 0x491C17B1b9Aa867f3A7A480bAFFC0721d59A7393;
    uint public hairMaticRatio; // Price of HAIR/MATIC
    uint public hairMaticBlockTimestampLast;
    uint maxReward = 1000 ether;
    ...
    event BetSettled(uint indexed betId, address indexed gambler, uint amount, uint8 indexed modulo, uint8 rollUnder, uint40 mask, uint outcome, uint winAmount, uint hairReward);
    ...
    // Set hairMaticLP if we change our main trading LP for price reference.
    function setHairMaticLP(address _hairMaticLP) external onlyOwner {
        hairMaticLP = _hairMaticLP;
    }
    ...
    // Get HAIR/MATIC price with safeguards against oracle attacks.
    function updateHairMaticRatio() internal {
        // Run function if 30 minutes elapsed.
        if (block.timestamp - hairMaticBlockTimestampLast >= 1800) {

            // Get ratio of HAIR to MATIC multiplied by SAFETY_FACTOR to avoid losing precision.
            uint newHairMaticRatio = IERC20(HAIR_TOKEN).balanceOf(hairMaticLP) * SAFETY_FACTOR / IERC20(MATIC_TOKEN).balanceOf(hairMaticLP);

            if (hairMaticRatio == 0) {
                // Update hairMaticRatio if first time updating.
                hairMaticRatio = newHairMaticRatio;
            } else if (newHairMaticRatio * SAFETY_FACTOR / hairMaticRatio > SAFETY_FACTOR * 110 / 100) {
                // Cap any increase to +10% per update.
                hairMaticRatio = hairMaticRatio * 110 / 100;
            } else if (newHairMaticRatio * SAFETY_FACTOR / hairMaticRatio < SAFETY_FACTOR * 80 / 100) {
                // Cap any decrease to -10% per update.
                hairMaticRatio = hairMaticRatio * 90 / 100;
            } else {
                hairMaticRatio = newHairMaticRatio;
            }
            hairMaticBlockTimestampLast = block.timestamp;
        }
    }
    ...
    function placeBet(uint betMask, uint modulo, address referrer) external payable nonReentrant {
        ...
        // Update HAIR/MATIC price if data is outdated.
        updateHairMaticRatio();
    ...
    function getHairReward(uint amount, uint modulo, uint rollUnder) private view returns (uint) {
        // HAIR reward equals token price multiplied by house edge fees, divided by win probability, multiplied by rewardPct.
        uint hairReward = hairMaticRatio * amount * (houseEdgeBP + getEffectiveWealthTaxBP(amount)) / 10000 * modulo / (modulo - rollUnder) * rewardPct / 100 / SAFETY_FACTOR;
        if (hairReward > maxReward) {
            hairReward = maxReward;
        }
        return hairReward;
    }
    ...
    function settleBet(bytes32 requestId, uint randomNumber) internal nonReentrant {
        ...
        uint hairReward = getHairReward(amount, modulo, rollUnder);
        ...
        if (winAmount > 0) {
            ...
            miner.addReward(gambler, hairReward);
        }
        
        // Record bet settlement in event log.
        emit BetSettled(betId, gambler, amount, uint8(modulo), uint8(rollUnder), bet.mask, outcome, winAmount, hairReward);
    }
    ...
    // In case you find yourself in a situation like this, just contact Barbershop support.
