...
/// @title Addy Strategy contract
/// @author Blaize.tech team
/// @notice Strategy for staking ADDY token
contract AddyStrategy is BaseStrategy {
    using SafeERC20 for IERC20;

    /// @notice QuickSwap router address.
    address public router;
    /// @notice Array for storing reward tokens.
    address[] public rewardTokens;
    /// @notice Stores swap routes for reward tokens.
    ...
    function initialize(
        ...
        super.initialize(
            address(0xc3FdbadC7c795EF1D6Ba111e06fF8F16A20Ea539), // Addy token
            _strategyRouter,
            address(0x920f22E1e5da04504b765F8110ab96A20E6408Bd), // Adamant MasterChef
            _adapter
        );
        rewardTokens.push(address(0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270)); // Add Wmatic reward
        router = _router;
    }
    ...
    /// @notice Deposits token to Addy protocol.
    ...
    /// @notice Withdraws some amount of tokens from Addy protocol.
    /// @dev Also calculates and transfers reward.
    ...
    /// @notice Withdraws all tokens, staked in Addy protocol.
    /// @dev Transfers all collected reward.
    function withdrawAll() external override onlyRole(STRATEGY_ROUTER) {
        _harvest();
    ...
    /// @notice Adds a new reward tokens.
    /// @param _rewardToken Address of reward token.
    function addRewardToken(address _rewardToken) external onlyRole(STRATEGIST_ROLE) {
        require(_rewardToken != address(0), "Wrong address");
    ...
    function setRoute(address[] memory _newRoute) external onlyRole(STRATEGIST_ROLE) {
        swapRoutes[_newRoute[0]] = _newRoute;
    }
    ...
    /// @notice Claims reward from ADDY protocol.
    ...
    /// @notice Returns array of reward tokens.
    /// @return Array of reward tokens.
    function getRewardTokens() external view returns (address[] memory) {
        return rewardTokens;
    }

    /// @notice Returns amount of ADDY tokens, staked in ADDY protocol.
    ...
    function protectedTokens() public view override returns (address[] memory) {
        ...
        tokens[tokens.length - 1] = address(want);
        return tokens;
    }
    ...
    function rewardsInEth(uint256 _share, uint256 _total) external view virtual override returns (uint256) {
        ...
        uint256 rewardEarned = (wantRewardCollected * _share) / _total;
        IAddyMasterChef.RewardData[] memory rewards = IAddyMasterChef(masterChef).claimableRewards(address(this));
        for (uint256 i = 0; i < rewards.length; i++) {
            if (rewards[i].amount > DUST_AMOUNT) {
                address[] memory path = swapRoutes[rewards[i].token];
                rewardEarned += (IUniswapV2Router01(router).getAmountsOut(rewards[i].amount, path)[path.length - 1] * _share) / _total;
            }
        }
    ...
    function _harvest() internal override returns (uint256) {
        uint256 addyBalanceBefore = want.balanceOf(address(this));
        IAddyMasterChef(masterChef).getReward();
        uint256 addyBalanceAfter = want.balanceOf(address(this));

        wantRewardCollected += (addyBalanceAfter - addyBalanceBefore) + _swapRewards();
        return wantRewardCollected;
    }

    /// @notice Transfers rewards to ClustersLock contract.
    /// @param _addyRewardAmount Amount of reward, collected in ADDY token.
    function _transferRewards(uint256 _addyRewardAmount) internal override {
        wantRewardCollected -= _addyRewardAmount;
        want.safeTransfer(_msgSender(), _addyRewardAmount);

        emit RewardsTransfered(_addyRewardAmount);
    }

    /// @notice Swaps reward tokens to want token.
    /// @return Collected amount of ADDY token.
    function _swapRewards() internal returns (uint256) {
        ...
        for (uint256 i = 0; i < rewardTokens.length; i++) {
            uint256 rewardAmount = IERC20(rewardTokens[i]).balanceOf(address(this));
            if (rewardAmount > DUST_AMOUNT) {
                address[] memory path = swapRoutes[rewardTokens[i]];
                IERC20(rewardTokens[i]).safeApprove(address(swapRouter), 0);
                IERC20(rewardTokens[i]).safeApprove(address(swapRouter), rewardAmount);
                rewardCollected += swapRouter.swapExactTokensForTokens(rewardAmount, 0, path, address(this), block.timestamp + 100)[
                    path.length - 1
                ];
            }
        }
        if (rewardCollected <= DUST_AMOUNT) {
            return 0;
        }
