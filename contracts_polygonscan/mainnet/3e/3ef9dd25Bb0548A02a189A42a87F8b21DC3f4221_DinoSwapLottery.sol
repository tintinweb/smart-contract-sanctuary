...
// File: contracts/DinoSwapLottery.sol

pragma solidity ^0.8.4;
pragma abicoder v2;

/** @title DinoSwap Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract DinoSwapLottery is ReentrancyGuard, IDinoSwapLottery, Ownable {
    ...
    uint256 public maxPriceTicketInDino = 50 ether;
    uint256 public minPriceTicketInDino = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInDino;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] dinoPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInDino;
        uint32 finalNumber;
    }
     ...
     * @param _dinoTokenAddress: address of the DINO token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _dinoTokenAddress, address _randomGeneratorAddress) {
        dinoToken = IERC20(_dinoTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of DINO to this contract
        uint256 amountDinoToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInDino,
            _ticketNumbers.length
        );

        // Transfer dino tokens to this contract
        dinoToken.safeTransferFrom(address(msg.sender), address(this), amountDinoToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInDino += amountDinoToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInDinoToTransfer
        uint256 rewardInDinoToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInDinoToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        dinoToken.safeTransfer(msg.sender, rewardInDinoToTransfer);

        emit TicketsClaim(msg.sender, rewardInDinoToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInDino) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in DINO for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].dinoPerBracket[j] =
                ...
                // A. No DINO to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].dinoPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInDino - amountToShareToWinners);

        // Transfer DINO to treasury address
        dinoToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        dinoToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInDino += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInDino >= minPriceTicketInDino) && (_priceTicketInDino <= maxPriceTicketInDino),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInDino: _priceTicketInDino,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            dinoPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInDino: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(dinoToken), "Cannot be DINO token");
    ...
    /**
     * @notice Set DINO price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInDino: minimum price of a ticket in DINO
     * @param _maxPriceTicketInDino: maximum price of a ticket in DINO
     */
    function setMinAndMaxTicketPriceInDino(uint256 _minPriceTicketInDino, uint256 _maxPriceTicketInDino)
        external
        onlyOwner
    {
        require(_minPriceTicketInDino <= _maxPriceTicketInDino, "minPrice must be < maxPrice");

        minPriceTicketInDino = _minPriceTicketInDino;
        maxPriceTicketInDino = _maxPriceTicketInDino;
    }
     ...
     * @param _priceTicket price of a ticket (in DINO)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].dinoPerBracket[_bracket];
        } else {
            return 0;
        }
    }
