...
// File: contracts/BPMembership.sol
...
contract BPMembership is IERC2981, ERC165Storage, Ownable, IERC1155MetadataURI, ERC1155, MinterRole {
    using Strings for uint256;
    using SafeMath for uint256;

    bytes4 private constant _INTERFACE_ID_ERC2981 = 0x2a55205a;

	string private constant name = "BoojaPresident MemberShip - Community ver.";
	string private constant symbol = "BPMS";
    ...
    */
    ...
    function setTokenURIPrefix(string memory _tokenURIPrefix) external onlyAdmin {
    ...
    function uri(uint256 _id) override(ERC1155, IERC1155MetadataURI) virtual public view returns (string memory) {
        return bytes(tokenURIPrefix).length > 0 ? string(abi.encodePacked(tokenURIPrefix, _id.toString(), ".json")) : "";
    }

    function mint(address _beneficiary, uint256 _supply) public onlyMinter{
        tokenIdPointer = tokenIdPointer.add(1);
        uint256 tokenId = tokenIdPointer;
        safeMint(_beneficiary, tokenId, _supply);
    }
    ...
    function doNotUseThisFunction() public onlyAdmin {
