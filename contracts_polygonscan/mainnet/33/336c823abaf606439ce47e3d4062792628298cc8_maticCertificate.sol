...
contract maticCertificate is ERC721, Ownable {
    ...
    IUniswapV2Router02 private  _sushiRouter = IUniswapV2Router02(SushiRouter);
    ...
    function mintWithNative(string memory _uri, uint256 _carbonAmount) external payable {
        
        address[] memory path = new address[](2);
        path[0] = _sushiRouter.WETH();
        path[1] = usdcTokenAddress;
      
        _sushiRouter.swapExactETHForTokensSupportingFeeOnTransferTokens{value: msg.value}(
            
            _carbonAmount*11*(10**6)+(1*(10**6)),
            path,
            address(this),
            block.timestamp+5
        );
    ...
    function mintWithUsdc(string memory _uri, uint256 _carbonAmount) external {
        ...
        usdc.transferFrom(msg.sender, address(this), _carbonAmount*11*(10**6)+(1*(10**6)));
        uint256 indexID = totalSupply();
        _safeMint(msg.sender, indexID);
        _setTokenURI(indexID, _uri);
        Certificates.push(offsetInfo(_carbonAmount,block.timestamp,msg.sender));
    ...
    function mintWithUsdt(string memory _uri, uint256 _carbonAmount) external {
        ...
        usdt.transferFrom(msg.sender, address(this), _carbonAmount*11*(10**6)+(1*(10**6)));
        uint256 indexID = totalSupply();
        _safeMint(msg.sender, indexID);
        _setTokenURI(indexID, _uri);
        Certificates.push(offsetInfo(_carbonAmount,block.timestamp,msg.sender));
