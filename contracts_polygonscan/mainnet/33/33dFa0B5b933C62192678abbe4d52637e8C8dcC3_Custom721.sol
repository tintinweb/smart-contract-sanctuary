...
contract Custom721 is NFTokenMetadata, Ownable {
    ...
    event MintedNFT(address indexed from, address indexed to, uint256 indexed id);
    ...
    function mint(address _to) external onlyAdmin returns(uint256) {
        ...
        emit MintedNFT(msg.sender,_to,_tokenIds);
        return _tokenIds;
    }
...
contract Custom721Factory {
    ...
    function addAdmin(address _address) public onlyAdmin {
        require(!admin[_address], "Already Admin");
        admin[_address] = true;
        adminList.push(_address);
    }

    function removeAdminByIndex(uint256 _index) public onlyAdmin {
        require(adminList.length >= _index, "Array out of bound");
        require(!admin[adminList[_index]], "Not a admin");
        admin[adminList[_index]] = false;
    ...
    function getAdminByIndex(uint256 i) public view returns (address) {
        return adminList[i];
    }
