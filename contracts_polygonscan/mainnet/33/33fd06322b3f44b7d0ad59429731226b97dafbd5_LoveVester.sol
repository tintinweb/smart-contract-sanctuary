...
/**
  * Love Boat Exchange (www.loveboat.exchange) $LOVE vesting contract
  * For Rory
/*
...
/**
 * @title  Love Token vesting contract
 * @author Love Boat Exchange, thanks Inverse Finance
 * @notice Contract for vesting agreement on LOVE tokens
 * @dev    Vesting calculation is linear
 */
contract LoveVester is Ownable {
    using SafeERC20 for ILove;
    ...
    /// @dev LoveBoat DEX governance timelock contract
    ...
    /// @dev LoveBoat DEX treasury token
    ILove public immutable love;

    /**
     * @dev Prevents non timelock from calling a method
     */
    modifier onlyTimelock() {
        require(msg.sender == timelock, "LoveVester:ACCESS_DENIED");
        _;
    }
    ...
    constructor(
        ...
        require(love_ != address(0) && timelock_ != address(0) && recipient != address(0), "LoveVester:INVALID_ADDRESS");
        require(vestingAmount_ > 0, "LoveVester:INVALID_AMOUNT");
        love = ILove(love_);
    ...
    function activate() public onlyOwner {
        require(vestingBegin == 0, "LoveVester:ALREADY_ACTIVE");
        if (reverseVesting) {
            love.delegate(owner());
        } else {
            love.delegate(timelock);
        }
    ...
    function delegate(address delegate_) public onlyOwner {
        ...
        require(reverseVesting, "LoveVester:DELEGATION_NOT_ALLOWED");
        love.delegate(delegate_);
    }
    ...
    function claimable() public view returns (uint256 amount) {
        if (!active()) {
            ...
            amount = love.balanceOf(address(this));
        } else {
    ...
    function unvested() public view returns (uint256 amount) {
        uint256 balance = love.balanceOf(address(this));
    ...
    function claim() public {
        require(active() && vestingBegin <= block.timestamp, "LoveVester:NOT_STARTED");
        uint256 amount = claimable();
        lastClaimTimestamp = block.timestamp;
        love.safeTransfer(owner(), amount);
    }
    ...
    function interrupt(address collectionAccount) external onlyTimelock {
        require(interruptible || !active(), "LoveVester:CANNOT_INTERRUPT");
        require(collectionAccount != address(0), "LoveVester:INVALID_ADDRESS");
        love.safeTransfer(collectionAccount, unvested());
    ...
    function setTimelock(address newTimelock) external onlyTimelock {
        require(newTimelock != address(0), "LoveVester:INVALID_ADDRESS");
        timelock = newTimelock;
    }
}

/**
 * @title Love token vesting factory contract
 * @author LoveBoat DEX, thanks Inverse Finance
 * @notice Factory contract to create configurable vesting agreement
 */
contract LoveVesterFactory is Ownable {
    using SafeERC20 for ILove;
    using EnumerableSet for EnumerableSet.AddressSet;

    /// @dev Emitted when vesting is created
    event VestingCreated(address recipient, address loveVester, uint256 amount, uint16 vestingDuration);

    /// @dev LoveBoat DEX governance timelock
    address public timelock;
    /// @dev LoveBoat DEX treasury token
    address public immutable love;
    /// @dev Registry of vesting agreements by recipient
    mapping(address => LoveVester[]) public loveVestersByRecipient;
    /// @dev Registry of all recipients
    EnumerableSet.AddressSet private _allRecipients;

    constructor(address love_, address timelock_) {
        require(love_ != address(0) && timelock_ != address(0), "LoveVesterFactory:INVALID_ADDRESS");
        love = love_;
        timelock = timelock_;
    ...
    function newLoveVester(
        ...
        ILove(love).safeTransferFrom(timelock, address(loveVester), vestingAmount);

        loveVestersByRecipient[recipient].push(loveVester);
        _allRecipients.add(recipient);

        emit VestingCreated(recipient, address(loveVester), vestingAmount, vestingDurationInDays);
    }
    ...
    function newNonInterruptibleVestingAgreement(
        ...
        newLoveVester(recipient, vestingAmount, vestingDurationInDays, vestingStartDelayInDays, reverseVesting, false);
    }
    ...
    function newInterruptibleVestingAgreement(
        ...
        newLoveVester(recipient, vestingAmount, vestingDurationInDays, vestingStartDelayInDays, reverseVesting, true);
    }
    ...
    function newSalaryAgreement(
        ...
        newLoveVester(recipient, vestingAmount, vestingDurationInDays, vestingStartDelayInDays, false, true);
    }
     ...
     */
    function getLoveVestersByRecipient(address recipient) external view returns (LoveVester[] memory) {
        return loveVestersByRecipient[recipient];
    }
    ...
    function setTimelock(address newTimelock) external onlyOwner {
        require(newTimelock != address(0), "LoveVesterFactory:INVALID_ADDRESS");
        timelock = newTimelock;
