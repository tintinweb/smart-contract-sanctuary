...
//[rusdt, usdt, route]
//[wmatic, matic, route]
//[matic, rmatic, route] - not possible
//[route, matic , rmatic]
//[route, matic]
...
//rusdc to usdc || rmatic to matic
        if((reserveInstance._contractToLP(address(destToken)) == address(fromToken)) &&
            (destToken.universalBalanceOf(address(reserveInstance)) > amount)) {
                if(destToken.isETH()) {
                    bridgeInstance.unstakeETH(handlerAddress, amount);
                     if (destToken.isETH()) {
...
//usdc to rusdc || matic to rmatic
        if(reserveInstance._lpToContract(address(destToken)) == address(fromToken)) {
             if(fromToken.isETH()) {
                require(address(this).balance >= amount, "Invalid amount for stake ETH");
                bridgeInstance.stakeETH{value: amount}(handlerAddress);
            } else {
