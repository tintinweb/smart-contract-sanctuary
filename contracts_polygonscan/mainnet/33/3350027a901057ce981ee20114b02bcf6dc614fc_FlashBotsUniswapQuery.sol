...
contract FlashBotsUniswapQuery {
    ...
    function getPairsByIndexRange(address factoryAddr, uint256 _start, uint256 _stop) external view returns (address[3][] memory)  {
        uint256 _allPairsLength = IPancakeFactory(factoryAddr).allPairsLength();
        ...
        for (uint i = 0; i < _qty; i++) {
            IUniswapV2Pair _uniswapPair = IUniswapV2Pair(IPancakeFactory(factoryAddr).allPairs(_start + i));
