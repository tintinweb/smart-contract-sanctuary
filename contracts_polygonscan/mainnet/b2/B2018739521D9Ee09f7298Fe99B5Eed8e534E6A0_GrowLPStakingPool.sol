...
interface IZAP {
    function zapTokenToLP(address fromToken, uint amount, address lpToken) external returns (uint);
    function zapTokenToLP(address fromToken, uint amount, address lpToken, uint minLPReceive) external returns (uint);
}
...
contract GrowLPStakingPool is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function setRewardDurationBlocks(uint256 _rewardsDurationBlocks) external onlyRole(CONFIGURATOR_ROLE) {
        require(_rewardsDurationBlocks > 0, "StakePool: _rewardsDurationBlocks is not valid");
        updateReward();
    ...
    function rewardsPreSharePreBlock() public view returns(uint256) {
        if (totalSupply == 0) {
            return 0;
        } else {
            return remainingRewardTokenAmount
                .mul(_DECIMAL)
                .div(rewardDurationBlocks).div(totalSupply);
        }
    }
    ...
    function checkIfNeedGetRewardAndZapToLP() external onlyRole(KEEPER_ROLE) {
        ...
        if (pendingGrows > rawRewardTokenSwapThreshold) {
            ...
            uint256 reward = IZAP(GrowRegister.get.ZapAddress()).zapTokenToLP(address(rawRewardToken), rawRewardToken.balanceOf(address(this)), address(STAKING_TOKEN));
    ...
    function _stakingTokenPriceInUSD(uint256 amount) public view returns (uint256) {
        return IPriceCalculator(GrowRegister.get.PriceCalculatorAddress()).tokenPriceInUSD(address(STAKING_TOKEN), amount);
    }
    ...
    function _deposit(uint256 amount, address userAddress) internal {
        ...
        _depositRewardAddReward(
            userAddress,
            _stakingTokenPriceInUSD(amount)
        );
