...
contract GrayKingDomNFT is Ownable, ReentrancyGuard, Gamificable {
    ...
    function deposit(uint256 _amount, uint256 _nftID) external nonReentrant {
        ...
        if (prevHasNFT)
            user.experienceDebt = _calculatedUserAmount(msg.sender).mul(poolInfo.accExperiencePerShare).div(PRECISION_FACTOR);
    ...
    function _userHarvestExperience(address _userAddress) internal {
        ...
        uint256 newUserAmount = _calculatedUserAmount(_userAddress);
        uint256 pending = newUserAmount.mul(poolInfo.accExperiencePerShare).div(PRECISION_FACTOR).sub(user.experienceDebt);
        if (pending > 0)
