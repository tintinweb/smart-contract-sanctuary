...
import "./libs/TheEndlessFactoryNFT.sol";
...
contract MadnessKingDomNFT is Ownable, ReentrancyGuard, Gamificable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20Metadata;

    // The address of the dream kingdom factory
    TheEndlessFactoryNFT public theEndlessFactoryNFT;
    ...
    event Deposit(address indexed user, uint256 amount, uint256 nftID);
    ...
    function deposit(uint256 _amount, uint256 _nftID) external nonReentrant {
        ...
        emit Deposit(msg.sender, _amount, _nftID);
    }
    ...
    function withdraw() external nonReentrant {
        ...
        uint256 pending = _calculatedUserAmount(msg.sender).mul(poolInfo.accRewardPerShare).div(PRECISION_FACTOR).sub(user.rewardDebt);

        if (pending > 0)
            _userHarvest(msg.sender);

        if (user.hasNFT) {
            if (poolInfo.supplyWithNFT > 0) {
                ...
                poolInfo.accExperiencePerShare = poolInfo.accExperiencePerShare.add(experienceReward.mul(PRECISION_FACTOR).div(poolInfo.supplyWithNFT));
            }
            poolInfo.lastExperienceBlock = block.number;

            uint256 _pendingExperience = _calculatedUserAmount(msg.sender).mul(poolInfo.accExperiencePerShare).div(PRECISION_FACTOR);

            poolInfo.supplyWithNFT = poolInfo.supplyWithNFT.sub(_calculatedUserAmount(msg.sender));

            theEndlessFactoryNFT.setExperience(user.nftID, user.experience.add(_pendingExperience.div(1e18)));
        }
        ...
        if (_amount > 0) {
            ...
            if (user.hasNFT) {
                uint256 powerStaking = user.powerAmount.sub(_amount);
                poolInfo.totalPower = poolInfo.totalPower.sub(powerStaking);
            }
    ...
    function pendingExperience(address _userAddress) external view returns (uint256) {
        ...
        if (block.number > poolInfo.lastRewardBlock && supplyWithNFT != 0) {
            uint256 multiplier = _getMultiplier(poolInfo.lastRewardBlock, block.number);
            uint256 experienceReward = multiplier.mul(experiencePerBlock);
            uint256 adjustedTokenPerShare = accExperiencePerShare.add(experienceReward.mul(PRECISION_FACTOR).div(supplyWithNFT));
            return calculatedAmount.mul(adjustedTokenPerShare).div(PRECISION_FACTOR);
        } else {
            return calculatedAmount.mul(accExperiencePerShare).div(PRECISION_FACTOR);
        }
    }
    ...
    function _updatePower(address _user, uint256 _amount) internal {
        ...
        uint256 powerStaking = (_amount * user.powerStaking).div(factor);

        user.powerAmount = user.amount + (user.amount * user.powerStaking).div(factor);

        poolInfo.totalPower = poolInfo.totalPower.add(powerStaking);

        poolInfo.supplyWithNFT = poolInfo.supplyWithNFT.add(_amount.add(powerStaking));
    ...
    function _userHarvest(address _userAddress) internal {
        ...
        if (user.amount > 0) {
            if (user.nextHarvestUntil <= block.timestamp) {
                ...
                if (pending > 0) {
                    user.rewardDebt = newUserAmount.mul(poolInfo.accRewardPerShare).div(PRECISION_FACTOR);
    ...
    function _updateHarvestLookup(address _userAddress) internal {
        ...
        if (user.hasNFT)
            newHarvestInverval = newHarvestInverval / 2;
    ...
    function _getNFTExperience(uint256 _nftID) internal view returns (uint256) {
        uint256 experience;

        (,experience,) =  theEndlessFactoryNFT.getCharacterOverView(_nftID);
...
import "./ERC721Presets.sol";

contract TheEndlessFactoryNFT is VRFConsumerBase, ERC721Presets {
    using SafeMath for uint256;
    ...
    mapping(bytes32 => uint256) requestToCharacterGeneration;
    ...
    constructor(address _VRFCoordinator, address _LinkToken, bytes32 _keyhash)
        ...
        ERC721("TheEndLess NFT", "TheEndless")
    {   
    ...
    function requestNewRandomEndlessCharacter(
        string memory name,
        uint256 _generation
        ...
        requestToCharacterGeneration[requestId] = _generation;
        return requestId;
    }
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomNumber)
        ...
        override
    {
        lastRandomResult = randomNumber;

        uint256 newId = characters.length;
        uint256 strength = (randomNumber % 100).add(1);
        uint256 agility = ((randomNumber % 10000) / 100 ).add(1);
        uint256 endurance = ((randomNumber % 1000000) / 10000 ).add(1);
        uint256 intelligence = ((randomNumber % 100000000) / 1000000 ).add(1);
        uint256 magic = ((randomNumber % 10000000000) / 100000000 ).add(1);
        uint256 wisdom = ((randomNumber % 1000000000000) / 10000000000).add(1);
        ...
        characters.push(
            Character(
                ...
                wisdom,
                experience,
                requestToCharacterGeneration[requestId],
    ...
    function setExperience(uint256 tokenId, uint256 _newExperience) onlyRole(OPERATOR_ROLE) external {
        characters[tokenId].experience = _newExperience;
    }
...
abstract contract Gamificable is Pausable, AccessControl, Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20Metadata;
     ...
     */
    function betBear() external avoidBotCall whenNotPaused {
     ...
     */
    function betBull() external avoidBotCall whenNotPaused {
        require(ledger[msg.sender].roundNumber != _currentRound());
