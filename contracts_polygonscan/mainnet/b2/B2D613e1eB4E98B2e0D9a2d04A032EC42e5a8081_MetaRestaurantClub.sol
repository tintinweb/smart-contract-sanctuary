// SPDX-License-Identifier: MIT
    ...
    // Created by Codelov3r

    contract MetaRestaurantClub is ERC721, Ownable, ERC721Enumerable {
      ...
      event RestaurantMinted(uint256 tokenId, address owner);
      event RestaurantBulkSent(uint256 tokenId, address owner);

      constructor() ERC721("Meta Restaurant Club","MRC") {}
      ...
      function mintProject(uint256 numberOfTokens) external payable {
        ...
        for (uint256 i = 0; i < numberOfTokens; i++) {
          ...
          if (totalSupply() < MAX_TOKENS) {
            ...
            emit RestaurantMinted(id, msg.sender);
          }
        }
      }
      ...
      function presaleProject(uint256 numberOfTokens) external payable {
        ...
        function BulkMint (uint256 numberOfTokens) external onlyOwner {
        ...
        returns (string memory)
      {
        ...
        if (isRevealed == false) {
          return
            ...
            ? string(abi.encodePacked(hiddenURI, "hidden.json"))
            : "";
        }
