...
contract CHQueensArtifacts is Delegated, ERC1155{
  uint public BURN_PRICE   = 0.04 ether;
  ...
  function burn( uint id, uint quantity ) external payable{
    require( isBurnActive,                       "Sale is not active"        );
    require( msg.value >= BURN_PRICE * quantity, "Ether sent is not correct" );
  ...
  //delegated
  function setActive(bool isBurnActive_, bool isMintActive_) external onlyDelegates{
    require( isBurnActive != isBurnActive_ || isMintActive != isMintActive_, "New value matches old" );
    isBurnActive = isBurnActive_;
    isMintActive = isMintActive_;
  }

  function setBaseURI(uint id, string calldata uri_) external onlyDelegates{
    _uri[id] = uri_;
  }

  function setPrice(uint burnPrice, uint mintPrice) external onlyDelegates{
    require( BURN_PRICE != burnPrice || MINT_PRICE != mintPrice, "New value matches old" );
