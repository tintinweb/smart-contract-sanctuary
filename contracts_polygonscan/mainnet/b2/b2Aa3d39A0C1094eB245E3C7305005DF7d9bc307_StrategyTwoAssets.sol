...
abstract contract StrategyFarmTwoAssets is BaseStrategyMasterChef {
    ...
    uint256 public harvestCutoffRoute = 10**15;
    uint256 public constant keepFXSmax = 10000;

    // Uniswap swap paths
    address[] public route_dfyn_path;
    address[] public dfyn_matic_path;
    address[] public dfyn_route_path;
    ...
    constructor(
        ...
        route_dfyn_path = new address[](2);
        route_dfyn_path[0] = route;
        route_dfyn_path[1] = dfyn;

        dfyn_matic_path = new address[](2);
        dfyn_matic_path[0] = dfyn;
        dfyn_matic_path[1] = matic;

        dfyn_route_path = new address[](2);
        dfyn_route_path[0] = dfyn;
        dfyn_route_path[1] = route;
    }
    ...
    function set_harvestCutoffRoute(uint256 newCutoff) external onlyOwner {
        harvestCutoffRoute = newCutoff;
    }
    ...
    function harvest() public override {
        ...
        // Swap route to dfyn
        uint256 _routeBalance = IERC20(route).balanceOf(address(this));
        if (_routeBalance > harvestCutoffRoute) {
            _swapUniswapWithPath(route_dfyn_path, _routeBalance);
        }
        ...
        if (_dfynBalance > harvestCutoff) {
            ...
            _swapUniswapWithPath(dfyn_route_path, _dfynBalancePost.div(2));

            // Add liquidity
            uint256 _dfyn = IERC20(dfyn).balanceOf(address(this));
            uint256 _route = IERC20(route).balanceOf(address(this));
            if (_dfyn > 0 && _route > 0) {
                IERC20(dfyn).safeApprove(currentRouter, 0);
                IERC20(dfyn).safeApprove(currentRouter, _dfyn);
                IERC20(route).safeApprove(currentRouter, 0);
                IERC20(route).safeApprove(currentRouter, _route);
...
contract StrategyTwoAssets is StrategyFarmTwoAssets {
    // Token addresses
    address public Dfyn_Rewards = 0x4b7Ce241B6E58Cef602F4EE588B8bD40f4e79066;
    address public ROUTE_DFYN_LP = 0xB0dc320ea9eea823a150763AbB4A7bA8286Cd08B;

    constructor()
        public
        StrategyFarmTwoAssets(Dfyn_Rewards, ROUTE_DFYN_LP, msg.sender)
    {}
