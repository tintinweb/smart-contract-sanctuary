...
contract GrowthVault is ERC20, Ownable {
        ...
        for (uint256 i; i < names.length; i++) {
            ...
            if(totalLP > 0) {
                ...
                if(strategyObj.token0 == WETH) {
                    ...
                    strategyObj.totalInBNB = (_token0Value + amounts[1]);
                    totalBNB += strategyObj.totalInBNB;
                    ...
                    strategyObj.totalInBNB = (_token1Value + amounts[1]);
                    totalBNB += strategyObj.totalInBNB;
                } else {
                    path[0] = strategyObj.token0;
                    strategyObj.totalInBNB = _getAmountOfToken(
                        strategyObj.token0, WETH, strategyObj.liquidityRouter, path);
                    totalBNB += strategyObj.totalInBNB;

                    path[0] = strategyObj.token1;
                    strategyObj.totalInBNB += _getAmountOfToken(
                        strategyObj.token1, WETH, strategyObj.liquidityRouter, path);
                    totalBNB += strategyObj.totalInBNB;
                }
            }
        }
    ...
    function autoCompound() public {
        ...
        for (uint256 i; i < names.length; i++) {
            ...
            if (_amount > 0) {
                ...
                _amount = _getAmountMin(_cakeToken, WETH, strategyObj.liquidityRouter, path);
                _swapExactTokensForETH(_amount, strategyObj.liquidityRouter, _cakeToken, path);
                ...
                _swapExactETHForTokens(_getBalanceOfToken(WETH), 0, strategyObj.liquidityRouter, path);
    ...
    function _withdrawFromStrategy(uint256 _nameID, uint256 _amount) internal {
        ...
        if(strategyObj.pid < NONE_PID_VALUE) {
            ...
            IfarmWOpid(strategyObj.farm).withdraw(_amount);
        }
    }
    ...
    function _addLiquidity(address _liquidityRouter, address _token0, address _token1) internal {
        ...
        if(_token0 != WETH) {
            ...
            IRouter(_liquidityRouter).swapExactETHForTokens{value:desiredAmountETH}(
                desiredAmountToken - (desiredAmountToken*toleranceLevelPercent*100/(10000)), // amountOutMin
                path,
                address(this),
                block.timestamp + 1 minutes // deadline
            );
        ...
        if(_token1 != WETH && _token0 == WETH) {
            ...
            // IRouter(_liquidityRouter).addLiquidity(
            //     _token0, // address
            //     _token1, // address
            //     desired0, // desired0
            //     amounts[1], // desired1
            //     desired0 - (desired0*toleranceLevelPercent*100/(10000)), // amountMin0
            //     amounts[1] - (amounts[1]*toleranceLevelPercent*100/(10000)), // amountMin1
            //     address(this), // to
            //     block.timestamp + 1 minutes // deadline
            // );
        }
    ...
    function _getYELs(uint256 _percent) internal { 
        ...
        for (uint256 i; i < names.length; i++) {
            ...
            amountMin = _getAmountMin(cakeToken, WETH, strategyObj.liquidityRouter, path);
            _swapExactTokensForETH(amountMin, strategyObj.liquidityRouter, cakeToken, path);
            ...
            if(strategyObj.token0 != WETH) {
                amountMin = _getAmountMin(strategyObj.token0, WETH, strategyObj.liquidityRouter, path);
                _swapExactTokensForETH(amountMin, strategyObj.liquidityRouter, strategyObj.token0, path);
            }

            path[0] = strategyObj.token1;
            if(strategyObj.token1 != WETH) {
                amountMin = _getAmountMin(strategyObj.token1, WETH, strategyObj.liquidityRouter, path);
                _swapExactTokensForETH(amountMin, strategyObj.liquidityRouter, strategyObj.token1, path);
            }
    ...
    function _removeLiquidityAfterWithdraw(uint256 _nameId) internal {
        ...
        if(_balance > 0) {
            ...
            path[0] = cakeToken;
            path[1] = WETH;
            uint256 amountMin = _getAmountMin(cakeToken, WETH, strategyObj.liquidityRouter, path);
            _swapExactTokensForETH(amountMin, strategyObj.liquidityRouter, cakeToken, path);
        }
    }
    ...
    function _swapTokensToNativeToken(
        ...
        if(_token0 != WETH) {
            path[0] = _token0;
            amountMin = _getAmountMin(_token0, WETH, _liquidityRouter, path);
            _swapExactTokensForETH(amountMin, _liquidityRouter, _token0, path);
        }

        if(_token1 != WETH) {
            path[0] = _token1;
            amountMin = _getAmountMin(_token0, WETH, _liquidityRouter, path);
            _swapExactTokensForETH(amountMin, _liquidityRouter, _token1, path);
        }
    }
