...
contract Roller is Ownable,  VRFConsumerBase {
    ...
    function onNoop()internal virtual{}
...
contract Charge is Roller {
    ...
    function onNoop() internal virtual override{
        uint256 iterations = 0;
        uint256 totalBets = polyroll.betsLength();
        if(lastRollCheck == 0){
            if(rolls.length > 0){
                lastRollCheck = rolls[rolls.length-5];
            }else{
                lastRollCheck = totalBets-12;
            }
        }
        for(uint256 i = lastRollCheck; i < totalBets && iterations < 20; i++){
            Polyroll.Bet memory bet = polyroll.bets(i);
            if(bet.isSettled){
                if(bet.modulo > 0){
                    outcomes.push(bet.outcome.mul(100).div(bet.modulo));
                    lastRollCheck = i;
                }
            }
            iterations++;
    ...
    function betAverage(uint256 random) internal view returns(uint256){
        ...
        if(outcomes.length < adl){
            return 1 + random % 15;
        }
        uint256 average = 0;
        for(uint256 i = outcomes.length-adl; i < outcomes.length; i++){
