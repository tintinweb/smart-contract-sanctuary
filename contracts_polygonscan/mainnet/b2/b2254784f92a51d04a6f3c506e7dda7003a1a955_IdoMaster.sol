...
contract IdoMaster is Ownable {
    ...
    function claim(IERC20 _token) external {
        ...
        emit Claim(_msgSender(), _token, user.tokenAmount);
        _token.safeTransfer(_msgSender(), user.tokenAmount);
        user.tokenAmount = 0;
        user.contributed = 0;
    }
    ...
    function claimReferralBonus(IERC20 _token) external {
        ...
        emit Claim(_msgSender(), _token, user.referralBonus);
        _token.safeTransfer(_msgSender(), user.referralBonus);
    ...
    function refund(IERC20 _token) external {
        ...
        ido.weiRaised = ido.weiRaised.sub(user.contributed);
        ido.allocated = ido.allocated.sub(user.tokenAmount);

        emit Refund(_msgSender(), _token, user.contributed);
        _msgSender().transfer(user.contributed);
        user.tokenAmount = 0;
        user.contributed = 0;
    }
}