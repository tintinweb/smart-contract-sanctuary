...
abstract contract StrategyFarmBase is BaseStrategyMasterChef {
    ...
    address public constant tamago = 0x034D706c3AF9D11F0Ba90d9967947ABEdA7a5758;
    ...
    // Other constants
    uint16 public constant underlyingPoolId = 22;
    ...
    // Uniswap swap paths
    address[] public tamago_kogecoin_path;
    ...
    {
        tamago_kogecoin_path = new address[](2);
        tamago_kogecoin_path[0] = tamago;
        tamago_kogecoin_path[1] = kogecoin;
    }
    ...
    function harvest() public override {
        ...
        uint256 _tamagoBalance = IERC20(tamago).balanceOf(address(this));
        if (_tamagoBalance > harvestCutoff){
            
            //Send performance fee to strategist
            uint256 performanceFee = _tamagoBalance.mul(keepFXS).div(keepFXSmax);
            if (performanceFee>0) {
                IERC20(tamago).safeTransfer(
                    strategist,
                    performanceFee
                );
            }
            ...
            // Swap for Matic
            uint256 _tamagoBalancePost = IERC20(tamago).balanceOf(address(this));
            _swapUniswapWithPath(tamago_kogecoin_path, _tamagoBalancePost.div(2));

            // Add liquidity for Shield/MATIC
            uint256 _tamago = IERC20(tamago).balanceOf(address(this));
            uint256 _kogecoin = IERC20(kogecoin).balanceOf(address(this));
            if (_tamago > 0 && _kogecoin > 0) {
                IERC20(kogecoin).safeApprove(currentRouter, 0);
                IERC20(kogecoin).safeApprove(currentRouter, _kogecoin);
                IERC20(tamago).safeApprove(currentRouter, 0);
                IERC20(tamago).safeApprove(currentRouter, _tamago);
                ...
                IERC20(tamago).safeTransfer(
                    strategist,
                    IERC20(tamago).balanceOf(address(this))
                );
            }
...
contract StrategyBase is StrategyFarmBase {
    // Token addresses
    address public TMGO_MASTER_CHEF = 0x1A807EF7665e3478d62480DEF0a6a0aCa5425424;
    address public TMGO_KOGE_LP = 0x0aD6421c068989cfFBe95Cf1aeF2D8Df08651B38;
