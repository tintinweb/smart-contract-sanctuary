...
contract InvestmentSafe is EIP712, IInvestmentSafe {
    ...
    function initialize(
        ...
        uint256 amount = investmentSafeFactory.getWethUsedForGas(
            investmentSafeFactory.GAS_CREATE_SAFE()
        );
        if (_owner != _creator) {
            IERC20(investmentSafeFactory.weth()).safeTransfer(
                _creator,
                amount
            );
        }
    ...
    function _checkJoinSignature(DelegateJoinData calldata joinData)
        internal
        virtual
    {
    ...
    function delegatedJoin(
        ...
        require(!isSecondJoinStep[msg.sender], "NOT_STEP1");
        isSecondJoinStep[msg.sender] = true;
        ...
        uint256 gasCost = investmentSafeFactory.getWethUsedForGas(
            investmentSafeFactory.JOIN_GAS()
        );
        uint256 amountAfterGasCost = joinData.amount.sub(gasCost);

        {
            _checkJoinSignature(joinData);

            weth.safeTransfer(msg.sender, gasCost);
        }
    ...
    function delegatedJoin2(DelegateJoinData calldata joinData) external {
        ...
        _checkJoinSignature(joinData);
        uint256 targetAmount = IERC20(
            IPolygonERC20Wrapper(joinData.targetToken).underlying()
    ...
    function _checkExitSignature(DelegateExitData calldata exitData)
        internal
        virtual
    {
        bytes32 hash = getPermitHash(
    ...
    function delegatedExit(
        ...
        require(exitStep[msg.sender] == Steps.One, "NOT_STEP_ONE");
        exitStep[msg.sender] = Steps.Two;

        _checkExitSignature(exitData);
...

        {
            ...
            targetAmount = targetAmount.sub(gasCost);
            weth.safeTransfer(msg.sender, gasCost);
        }

        _maxApprove(weth, investmentSafeFactory.childAmunWeth(), targetAmount);
    }

    function delegatedExit2(DelegateExitData calldata exitData)
        external
    {
        ...
        IChildAmunWeth(investmentSafeFactory.childAmunWeth()).getAmunWeth(
            IERC20(investmentSafeFactory.weth()).balanceOf(address(this))
        );
    }

    function delegatedExit3(DelegateExitData calldata exitData)
        external
    {
        ...
        nonces[msg.sender] = nonces[msg.sender] + 1;
        _checkExitSignature(exitData);
        uint256 targetAmount = IERC20(investmentSafeFactory.childAmunWeth())
            .balanceOf(address(this));
     ...
     */
    function _useNonce(address userAddress)
        internal
        virtual
        returns (uint256 current)
    {
        current = nonces[userAddress];
        nonces[userAddress] = current + 1;
    }
}
