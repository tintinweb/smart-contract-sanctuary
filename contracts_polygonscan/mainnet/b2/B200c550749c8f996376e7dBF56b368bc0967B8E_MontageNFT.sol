...
// File: contracts/montage/IMontageMetadataEncoder.sol


pragma solidity ^0.6.0;

interface IMontageMetadataEncoder {
...
contract MontageNFT is IMontage, ERC721, AccessControl, IERC2981 {
    ...
    IMontageMetadataEncoder public montageMetadataEncoder;
    ...
    event NewMetadataEncoderSet(address metadataEncoder);
    ...
    modifier onlyChildChainManagerProxy() {
        require(msg.sender == childChainManagerProxy, "Address is not ChildChainManagerProxy");
        _;
    }
    ...
    constructor(
        ...
        IMontageMetadataEncoder _montageMetadataEncoderAddr,
        ...
        montageDataStorage = _montageDataStorageAddr;
        montageMetadataEncoder = _montageMetadataEncoderAddr;
     ...
     * @return creator the address of the creator of the montage
     */
    function getCreator(uint256 _tokenId) public view override onlyExistingTokens(_tokenId) returns (address creator) {
     ...
     * @return name the name of the montage
     */
    function getName(uint256 _tokenId) public view override onlyExistingTokens(_tokenId) returns (string memory name) {
     ...
     * @return true if the montage can be unpacked
     */
    function canBeUnpacked(uint256 _tokenId) public view override onlyExistingTokens(_tokenId) returns (bool) {
    ...
    function hasUnlockableContentUri(uint256 _tokenId) external view override returns (bool) {
    ...
    function setMetadataEncoder(IMontageMetadataEncoder newMetadataEncoder) external onlyAdmin {
        montageMetadataEncoder = newMetadataEncoder;
        emit NewMetadataEncoderSet(address(montageMetadataEncoder));
    }
    ...
    function deposit(address user, bytes calldata depositData) external onlyChildChainManagerProxy {
        uint256 _tokenId = abi.decode(depositData, (uint256));
        withdrawnTokens[_tokenId] = false;
        _mint(user, _tokenId);
    }
    ...
    function withdraw(uint256 _tokenId) external {
        require(_msgSender() == ownerOf(_tokenId), "ChildMintableERC721: INVALID_TOKEN_OWNER");
    ...
    function withdrawWithMetadata(uint256 _tokenId) external {
        ...
        emit TransferWithMetadata(ownerOf(_tokenId), address(0), _tokenId, this.encodeTokenMetadata(_tokenId));
     ...
     */
    function encodeTokenMetadata(uint256 _tokenId) external view virtual returns (bytes memory) {
        return
            montageMetadataEncoder.encodeMetadata(
                montageDataStorage.getCreator(_tokenId),
                montageDataStorage.getName(_tokenId),
                montageDataStorage.getDescription(_tokenId),
                montageDataStorage.canBeUnpacked(_tokenId),
                montageDataStorage.getTokenIds(_tokenId),
                montageDataStorage.getUnlockableContentUri(_tokenId),
                montageDataStorage.getUnlockableDescription(_tokenId)
            );
    }
