...
// File: contracts/JustOnChain.sol


pragma solidity ^0.8.0;

// @title: JustOnChain is an onchainmonkey Baby Minited just for fun. JustOnChain is a Meme NFT.
// @Modified by NB to JustOnChain
// On chain PFP collection of 20k unique profile images with the following properties:
...
//   - all 10,000 JustOnChain are unique
//   - there are 4 traits with 171 values (including 3 traits of no hat, no clothes no mustache)
...
// Bring on the JustOnChain!
contract JustOnChain is ERC721Enumerable, ReentrancyGuard, Ownable {
  using Strings for uint256;
  ...
  string[] private Skin1 = ["653","532","444","a71","636","ca9","f89","777","049","901","fc5","683","574","bcc","d04","222","889","7f9","fd1"];
  string[] private Skin2 = ["532","653","653","653","221","b99","000","a83","222","000","110","fff","711","344","799","555","8a8","32f","653"];
  uint8[] private Skin_w =[249, 246, 223, 141, 116, 114, 93, 90, 89, 86, 74, 72, 55, 48, 39, 32, 28, 14, 8];
  ...
  string[] private Nose = ["653","ffc","f89","777","049","901","bcc","d04","fd1","ffc","653","f89","777","049","bcc","901","901","bcc","653","d04","ffc","f89","777","049","fd1","f89","777","bcc","d04","049","ffc","901","fd1"];
  uint8[] private Nose_w = [252, 172, 80, 79, 56, 49, 37, 33, 31, 30, 28, 27, 26, 23, 22, 18, 15, 14, 13, 12, 11, 10, 10, 10, 9, 8, 7, 7, 6, 5, 5, 4, 3];
  ...
  string[] private z = ['<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 500 500"><rect x="0" y="0" width="500" height="500" style="fill:#',
    ...
    '"/><circle cx="100" cy="250" r="20" style="fill:#', '"/><circle cx="100" cy="250" r="10" style="fill:#', '"/><circle cx="400" cy="250" r="20" style="fill:#',
    '"/><circle cx="400" cy="250" r="10" style="fill:#', '"/><circle cx="250" cy="253" r="149" style="fill:#', '"/><circle cx="250" cy="253" r="128" style="fill:#',
    '"/><circle cx="208" cy="229" r="36" style="fill:#fff"/><circle cx="302" cy="230" r="36" style="fill:#fff"/><circle cx="206" cy="220" r="26" style="fill:#',
    '"/><circle cx="300" cy="222" r="27" style="fill:#', '"/><circle cx="206" cy="220" r="17" style="fill:#000"/><circle cx="300" cy="220" r="17" style="fill:#000"/>',
    '<ellipse  cx="255" cy="280" rx="15" ry="10" style="fill:#',
     '"/><rect x="199" y="331" width="100" height="8" style="fill:#fff"/><circle cx="200" cy="224" r="1" style="fill:#000"/><circle cx="300" cy="224" r="1" style="fill:#000"/>',
    '</svg>'];
  string private cross='<rect x="94" y="269" width="10" height="20" style="fill:#e0bf62"/><rect x="85" y="285" width="30" height="30" style="fill:#ffdc08"/>';
  string private clo1='<rect width="300" height="120" x="99" y="400" style="fill:#';
  string private clo2='"/><rect width="60" height="16" x="126" y="430" style="fill:#';
  string private hh1='<rect width="183" height="90" x="160" y="40" style="fill:#';
  string private hh2='"/><rect width="323" height="60" x="92" y="102" style="fill:#';
  string private sl1='<rect x="165" y="190" width="83" height="30" style="fill:#';
  string private sl2='"/><rect x="256" y="190" width="83" height="30"  style="fill:#';
  string private mou='<line x1="228" y1="335" x2="217" y2="335" style="stroke:#000;stroke-width:10"/>';
  string private ey1='<rect x="160" y="190" width="75" height="15" style="fill:#';
  string private ey2='"/><rect x="275" y="190" width="75" height="15" style="fill:#';
  string private ey3='<rect x="160" y="236" width="186" height="33" style="fill:#';
  string private zz='"/>';
  string private ea1='<circle cx="97" cy="266" r="4" style="fill:#';
  string private ea2='fe7';
  string private ea3='999';
  string private ea4='"/><circle cx="342" cy="272" r="3" style="fill:#000"/>';
  string private ea5='<circle cx="403" cy="266" r="4" style="fill:#';
  string private ea6='bdd';
  string private mo1='<line x1="';
  string private mo2='" y1="307" x2="';
  string private mo3='" y2="319" style="stroke:#000;stroke-width:10"/>';
  string private mo4='" y1="307" x2="';
  string private mo5='" y2="319" style="stroke:#000;stroke-width:10"/>';
  string private tr1='", "attributes": [{"trait_type": "Background","value": "';
  string private tr2='"},{"trait_type": "Skin","value": "';
  ...
  string private tr7='"},{"trait_type": "Nose","value": "';
  ...
  string private co1=', ';
  string private rl1='{"name": "Just OnChain #';
  string private rl3='"}';
  string private rl4='data:application/json;base64,';

  struct Ape { // a nod to BAYC, "ape" was shorter to type than JustOnChain
    uint8 bg;
    uint8 Skin;
    uint8 eyes;
    uint8 Nose;
  ...
  function randomOne(uint256 tokenId) internal view returns (Ape memory) {
    ...
    ape.Skin = usew(Skin_w,random(string(abi.encodePacked(clo1,tokenId.toString())))%1817);
    ape.eyes = usew(eyes_w,random(string(abi.encodePacked(ra2,tokenId.toString())))%1429);
    ape.Nose = usew(Nose_w,random(string(abi.encodePacked(ra3,tokenId.toString())))%1112);
  ...
  function getTraits(Ape memory ape) internal view returns (string memory) {
    string memory o=string(abi.encodePacked(tr1,uint256(ape.bg).toString(),tr2,uint256(ape.Skin).toString(),tr3,uint256(ape.earring).toString()));
    return string(abi.encodePacked(o,tr4,uint256(ape.hat).toString(),tr5,uint256(ape.eyes).toString(),tr6,uint256(ape.clothes).toString(),tr7,uint256(ape.Nose).toString(),tr8));
  }

  // return comma separated traits in order: hat, Skin, clothes, eyes, earring, Nose, background
  function getAttributes(uint256 tokenId) public view returns (string memory) {
    Ape memory ape = randomOne(tokenId);
    string memory o=string(abi.encodePacked(uint256(ape.hat).toString(),co1,uint256(ape.Skin).toString(),co1,uint256(ape.clothes).toString(),co1));
    return string(abi.encodePacked(o,uint256(ape.eyes).toString(),co1,uint256(ape.earring).toString(),co1,uint256(ape.Nose).toString(),co1,uint256(ape.bg).toString()));
  }
  ...
  function genNose(uint8 h) internal view returns (string memory) {
    string memory out = '';
    uint i;
    if ((h>24) || ((h>8) && (h<16))) {
      for (i=0;i<7;i++) {
  ...
  function genSVG(Ape memory ape) internal view returns (string memory) {
    string memory a=Skin1[ape.Skin];
    string memory b=Skin2[ape.Skin];
    string memory hatst='';
    string memory clost='';
    if (ape.clothes>0) {
    ...
    output = string(abi.encodePacked(output,eyes[ape.eyes],z[11],eyes[ape.eyes],z[12],genEye(a,b,ape.eyes),z[13],Nose[ape.Nose],z[14]));
    return string(abi.encodePacked(output,genNose(ape.Nose),genEarring(ape.earring),hatst,clost,z[15]));
  }
  ...
  function claim() public nonReentrant {
    require(numClaimed >= 0 && numClaimed < 19500, "invalid claim");
    _safeMint(_msgSender(), numClaimed + 1);
    numClaimed += 1;
  }
    
  function ownerClaim(uint256 tokenId) public nonReentrant onlyOwner {
    require(tokenId > 19500 && tokenId < 20001, "invalid claim");
    _safeMint(owner(), tokenId);
  }
 
  function mintForOwner(uint256 _mintAmount) public onlyOwner {
    uint256 supply = totalSupply();
    require(supply + _mintAmount <= 20000);
    require(_mintAmount > 0);

  for (uint256 i = 1; i <= _mintAmount; i++) {
      _safeMint(msg.sender, supply + i);
}}
  constructor() ERC721("JustOnChain", "JSTOC") Ownable() {}
}