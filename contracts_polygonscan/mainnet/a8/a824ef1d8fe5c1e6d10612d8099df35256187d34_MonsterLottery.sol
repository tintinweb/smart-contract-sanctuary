...
// File: contracts/MonsterLottery.sol

pragma solidity 0.8.4;
pragma abicoder v2;

/** @title Monster Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract MonsterLottery is ReentrancyGuard, IMonsterLottery, Ownable {
    ...
    uint256 public maxPriceTicketInMonster = 50 ether;
    uint256 public minPriceTicketInMonster = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInMonster;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] MonsterPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInMonster;
        uint32 finalNumber;
    }
     ...
     * @param _MonsterTokenAddress: address of the MONSTER token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _MonsterTokenAddress, address _randomGeneratorAddress) {
        monsterToken = IERC20(_MonsterTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of MONSTER to this contract
        uint256 amountMonsterToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInMonster,
            _ticketNumbers.length
        );

        // Transfer monster tokens to this contract
        monsterToken.safeTransferFrom(address(msg.sender), address(this), amountMonsterToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInMonster += amountMonsterToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInMonsterToTransfer
        uint256 rewardInMonsterToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInMonsterToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        monsterToken.safeTransfer(msg.sender, rewardInMonsterToTransfer);

        emit TicketsClaim(msg.sender, rewardInMonsterToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInMonster) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in MONSTER for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].MonsterPerBracket[j] =
                ...
                // A. No MONSTER to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].MonsterPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInMonster - amountToShareToWinners);

        // Transfer MONSTER to treasury address
        monsterToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        monsterToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInMonster += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInMonster >= minPriceTicketInMonster) && (_priceTicketInMonster <= maxPriceTicketInMonster),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInMonster: _priceTicketInMonster,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            MonsterPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInMonster: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(monsterToken), "Cannot be MONSTER token");
    ...
    /**
     * @notice Set MONSTER price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInMonster: minimum price of a ticket in MONSTER
     * @param _maxPriceTicketInMonster: maximum price of a ticket in MONSTER
     */
    function setMinAndMaxTicketPriceInMonster(uint256 _minPriceTicketInMonster, uint256 _maxPriceTicketInMonster)
        external
        onlyOwner
    {
        require(_minPriceTicketInMonster <= _maxPriceTicketInMonster, "minPrice must be < maxPrice");

        minPriceTicketInMonster = _minPriceTicketInMonster;
        maxPriceTicketInMonster = _maxPriceTicketInMonster;
    }
     ...
     * @param _priceTicket price of a ticket (in MONSTER)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].MonsterPerBracket[_bracket];
        } else {
            return 0;
        }
    }
