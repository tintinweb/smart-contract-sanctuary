...
// File: contracts/LimeVault.sol

pragma solidity 0.6.12;

contract LimeVault is Ownable, Pausable {
    ...
    struct UserInfo {
        ...
        uint256 limeAtLastUserAction; // keeps track of Lime deposited at the last user action
        uint256 lastUserActionTime; // keeps track of the last user action time
    }

    IERC20 public immutable token; // Lime token
    ...
    /**
     * @notice Constructor
     * @param _token: Lime token contract
    ...
    /**
     * @notice Deposits funds into the LIME Vault
     * @dev Only possible when contract not paused.
     * @param _amount: number of tokens to deposit (in LIME)
     */
    function deposit(uint256 _amount) external whenNotPaused notContract {
        ...
        user.limeAtLastUserAction = user.shares.mul(balanceOf()).div(totalShares);
    ...
    /**
     * @notice Reinvests LIME tokens into MasterChef
     * @dev Only possible when contract not paused.
     */
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the Lime Vault
     */
     ...
     * @return Expected reward to collect in LIME
     */
    function calculateHarvestLimeRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingLime(0, address(this));
     ...
     */
    function calculateTotalPendingLimeRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingLime(0, address(this));
        amount = amount.add(available());
    ...
    /**
     * @notice Withdraws from funds from the Lime Vault
