...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    uint256 public maxIssued = 24700000000000000000000000; //24.7M issues, 1.3M minted at launch (200k dev fund, 1M airdrop, 100k initial liquidity) = 26M total
    ...
    uint256 public tokensPerBlock = 5000000000000000; // reward tokens created per block, can be overwritten by emissionSchedule
    ...
    uint256 public LOCKUP_PERIOD_BLOCKS = blockPerDay * 90; //~3 months

    //information about emission schedule
    ...
    constructor(address _penaltyAddress, uint256 _startBlock, address _rewardToken) public {
      PENALTY_ADDRESS = _penaltyAddress;
      rewardToken = _rewardToken;

      if (_startBlock == 0) _startBlock = block.number;

      uint blockPerWeek = blockPerDay * 7;

      //First 4 weeks have aggresive issuance, 5/block for first week, 4/block for second week...
      ...
      for (uint256 i = 0; i < _emissionAmounts.length; i++){
        emissionScheduleArray.push(EmissionSchedule(_emissionAmounts[i], _startBlock + _emissionDelays[i]));
      }
    }
    ...
    function addPendingClaims() external {
        ...
        pending[msg.sender].push(pendingRewards(totalPending, block.number + LOCKUP_PERIOD_BLOCKS));
    }
    ...
    //claim just limited number of pendingClaims
    function claimLimited(uint256 limit) external {
      ...
      for (uint i = 0; i < limit; i++){
        if (pending[msg.sender][i].unlockBlock <= block.number){
          amount += pending[msg.sender][i].amount;
        } else {
          amount += pending[msg.sender][i].amount.div(2);
          penalty += pending[msg.sender][i].amount.div(2);
        }
        delete pending[msg.sender][i];
      }
      ...
      Claim(msg.sender, amount, penalty);
    }
    ...
    function deposit(uint256 _pid, uint256 _wantAmt) public nonReentrant {
        ...
        if (user.shares > 0) {
            ...
            if (_pending > 0) {
              pending[msg.sender].push(pendingRewards(_pending, block.number + LOCKUP_PERIOD_BLOCKS));
            }
        }
        if (_wantAmt > 0) {
