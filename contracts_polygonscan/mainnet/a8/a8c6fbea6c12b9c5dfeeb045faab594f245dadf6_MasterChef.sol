...
contract PharmToken is Context, IBEP20, Ownable {
    using SafeMath for uint256;
...
contract MasterChef is Ownable  {
    ...
    uint256 public constant MAXIMUM_HARVEST_PERCENT = 2500;
    ...
    function add(uint256 _allocPoint, IBEP20 _lpToken, uint16 _depositFeeBP, uint256 _harvestInterval,uint256 _maxHarvestPercent,
     bool _withUpdate) public onlyOwner {
        require(_depositFeeBP <= 500, "set: invalid deposit fee basis points");
        require(_harvestInterval <= MAXIMUM_HARVEST_INTERVAL, "set: invalid harvest interval");
        require(_maxHarvestPercent <= MAXIMUM_HARVEST_PERCENT, "set: invalid harvest percent");
        ...
        poolInfo.push(PoolInfo({
            ...
            maxHarvestPercent:_maxHarvestPercent
        }));
        
    }

    // Update the given pool's PHARM allocation point and deposit fee. Can only be called by the owner.
    function set(uint256 _pid, uint256 _allocPoint, uint16 _depositFeeBP, uint256 _harvestInterval,uint256 _maxHarvestPercent,
     bool _withUpdate) public onlyOwner {
        require(_depositFeeBP <= 500, "set: invalid deposit fee basis points");
        require(_harvestInterval <= MAXIMUM_HARVEST_INTERVAL, "set: invalid harvest interval");
        require(_maxHarvestPercent <= MAXIMUM_HARVEST_PERCENT, "set: invalid harvest percent");
        ...
        poolInfo[_pid].maxHarvestPercent = _maxHarvestPercent;
    }
    ...
    function pendingPharm(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 pending = user.amount.mul(accPharmPerShare).div(1e12).sub(user.rewardDebt);
        return pending.add(user.rewardLockedUp);
    }

    // View function to see if user can harvest PHARMs.
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 pharmReward = multiplier.mul(pharmPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        pharm.mint(devAddress, pharmReward.div(10));
    ...
    // Pay or lockup pending PHARMs.
    function payOrLockupPendingPharm(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        if (canHarvest(_pid, msg.sender)) {
            if (pending > 0 || user.rewardLockedUp > 0) {
                ...
                uint256 rewardCut = totalRewards.mul(pool.maxHarvestPercent).div(10000);
                // reset lockup
                user.rewardLockedUp = totalRewards.sub(rewardCut);
                user.nextHarvestUntil = block.timestamp.add(pool.harvestInterval);
                // send rewards
                safePharmTransfer(msg.sender, rewardCut);
            }
        } else if (pending > 0) {
    ...
    function safePharmTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > pharmBal) {
            pharm.transfer(_to, pharmBal);
        } else {
            pharm.transfer(_to, _amount);
        }
    }
    ...
    // Pharm has to add hidden dummy pools in order to alter the emission, here we make it simple and transparent to all.
    function updateEmissionRate(uint256 _pharmPerBlock) public onlyOwner {
        massUpdatePools();
        emit EmissionRateUpdated(msg.sender, pharmPerBlock, _pharmPerBlock);
        pharmPerBlock = _pharmPerBlock;
    }
