...
contract PolyPepes is ERC721Metadata {
  ...
  modifier periodStarted() {
    require(block.timestamp >= startTime, "PolyPepes: Period not started");
    _;
  }
  ...
  constructor(
    ...
    address admin_,
    address admin2_
  ) ERC721Metadata("PolyPepes", "POLYPEPE", baseURI_) {
    _admin = admin_;
    _admin2 = admin2_;
  }

  function setStartTime(uint256 _startTime) external onlyOwner {
    require(_startTime > 0, "PolyPepes: invalid _startTime");
    require(_startTime > block.timestamp, "PolyPepes: old start time");
  ...
  function setGiveawayMax(uint256 _giveawayMax) external onlyOwner {
    require(
      ...
      "PolyPepes: invalid max value"
    );
    require(giveawayMax != _giveawayMax, "PolyPepes: already set");
    giveawayMax = _giveawayMax;
  }

  function randomGiveaway(address to) external onlyOwner {
    require(to != address(0), "PolyPepes: zero address");
    require(
      _giveawayCounter.current() < giveawayMax,
      "PolyPepes: overflow giveaways"
    );
  ...
  function purchase(uint256 numberOfNfts) external payable periodStarted {
    require(pendingCount > 0, "PolyPepes: All minted");
    require(numberOfNfts > 0, "PolyPepes: numberOfNfts cannot be 0");
    require(
      numberOfNfts <= 100,
      "PolyPepes: You may not buy more than 20 NFTs at once"
    );
    require(
      totalSupply().add(numberOfNfts) <= MAX_NFT_SUPPLY,
      "PolyPepes: sale already ended"
    );
    require(
      MINT_PRICE.mul(numberOfNfts) == msg.value,
      "PolyPepes: invalid ether value"
    );
  ...
  function claimReward(uint256 tokenId) public {
    ...
    require(
      ...
      "AvaxApes: can't claim for giveaways"
    );
  ...
  function _randomMint(address _to) internal returns (uint256) {
    require(totalSupply() < MAX_NFT_SUPPLY, "PolyPepes: max supply reached");
