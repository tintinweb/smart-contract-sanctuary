...
contract MasterChef {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. BLUEBRY to distribute per block.
        uint256 lastRewardBlock;  // Last block number that BLUEBRY distribution occurs.
        uint256 accBluebryPerShare;   // Accumulated BLUEBRY per share, times 1e12. See below.
        uint16 depositFeeBP;    // Deposit fee of the underlying pool.
    }
...
abstract contract JarBase is ERC20, Ownable {
    ...
    function deposit(uint256 _amount) public {
        ...
        //(, , , , uint16 depositFeeBP) = underlyingMC.poolInfo(underlyingPoolId);
        //uint256 _depositFeeFactor = IBerryStrategy(depositFeeBP).depositFeeFactor();
        
        uint16 _depositFeeFactor = IStrategy(strategy).getDepositFee();
    ...
    function withdraw(uint256 _shares) public {
        ...
        //uint16 underlyingPoolId = IStrategy(strategy).underlyingPoolId();
        //MasterChef underlyingMC = MasterChef(masterChefAddr);
        //(, , , , address berryStrat) = underlyingMC.poolInfo(underlyingPoolId);
        //uint256 _withdrawFeeFactor = IBerryStrategy(berryStrat).withdrawFeeFactor();
