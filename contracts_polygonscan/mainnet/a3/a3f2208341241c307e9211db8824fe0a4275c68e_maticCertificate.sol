...
contract maticCertificate is ERC721, Ownable {
    
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    
    address private usdcTokenAddress = 0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174;
    IERCProxy usdcProxy = IERCProxy(usdcTokenAddress);
    IERC20 private usdc;
    
    address private usdtTokenAddress =0xc2132D05D31c914a87C6611C10748AEb04B58e8F;
    IERCProxy usdtProxy = IERCProxy(usdtTokenAddress);
    IERC20 private usdt;

    address private constant PancakeRouter=0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506;
    ...
    constructor(string memory _name, string memory _symbol) ERC721(_name, _symbol) {
        ...
        address _a = usdcProxy.implementation();
        address _b = usdtProxy.implementation();
    ...
    function updateImplementations() external onlyOwner {
        address _a = usdcProxy.implementation();
        address _b = usdtProxy.implementation();
    ...
    function mintWithUsdc(string memory _uri, uint256 _carbonAmount) external {
        ...
        usdc.transferFrom(msg.sender, address(this), _carbonAmount*16*(10**6));
        uint256 indexID = totalSupply();
        _safeMint(msg.sender, indexID);
        _setTokenURI(indexID, _uri);
        Certificates.push(offsetInfo(_carbonAmount,block.timestamp,msg.sender));
    ...
    function mintWithUsdt(string memory _uri, uint256 _carbonAmount) external {
        ...
        usdt.transferFrom(msg.sender, address(this),  _carbonAmount*16*(10**6));
        uint256 indexID = totalSupply();
        _safeMint(msg.sender, indexID);
        _setTokenURI(indexID, _uri);
        Certificates.push(offsetInfo(_carbonAmount,block.timestamp,msg.sender));
