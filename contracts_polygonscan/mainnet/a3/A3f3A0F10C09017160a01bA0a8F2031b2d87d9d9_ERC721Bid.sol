...
contract ERC721Bid is Ownable, Pausable, ERC721BidStorage {
    ...
    constructor(address _acceptedToken, address _owner) Ownable() Pausable() {
    ...
    function _placeBid(
        ...
        require(_duration >= MIN_BID_DURATION, "The bid should be last longer than a minute");

        require(_duration <= MAX_BID_DURATION, "The bid can not last longer than 6 months");
        ...
        bytes32 bidId = keccak256(
            abi.encodePacked(block.timestamp, msg.sender, _tokenAddress, _tokenId, _price, _duration, _fingerprint)
        );
        ...
        emit BidCreated(bidId, _tokenAddress, _tokenId, msg.sender, _price, expiresAt, _fingerprint);
    }
    ...
    function onERC721Received(
        ...
        if (ownerCutPerMillion > 0) {
            ...
            require(
                acceptedToken.transferFrom(bidder, owner(), saleShareAmount),
        ...
        require(
            acceptedToken.transferFrom(bidder, _from, price.sub(saleShareAmount)),
            "Transfering acceptedToken to owner failed"
        );

        emit BidAccepted(bidId, msg.sender, _tokenId, bidder, _from, price, saleShareAmount);
    ...
    function cancelBid(address _tokenAddress, uint256 _tokenId) public whenNotPaused {
        // Get active bid
        (uint256 bidIndex, bytes32 bidId, , , ) = getBidByBidder(_tokenAddress, _tokenId, msg.sender);
    ...
    function _requireComposableERC721(
        ...
        if (composableToken.supportsInterface(ERC721Composable_ValidateFingerprint)) {
            require(composableToken.verifyFingerprint(_tokenId, _fingerprint), "Token fingerprint is not valid");
        }
    }

    /**
     * @dev Check if the bidder has balance and the contract has enough allowance
     * to use bidder acceptedToken on his belhalf
    ...
    function _requireBidderBalance(address _bidder, uint256 _amount) internal view {
        require(acceptedToken.balanceOf(_bidder) >= _amount, "Insufficient funds");
        require(
            acceptedToken.allowance(_bidder, address(this)) >= _amount,
            "The contract is not authorized to use acceptedToken on bidder behalf"
        );
    }
}
...
contract ERC721BidStorage {
    ...
    event BidCancelled(bytes32 _id, address indexed _tokenAddress, uint256 indexed _tokenId, address indexed _bidder);
