...
contract LeoPolygonMainStaking is Ownable, ReentrancyGuard {
    ...
    address public rewardToken = 0x9f7C4AA6C7B0c064ec9b7324Aa8EB23bB5ebF06f;
    uint256 public maxIssued = 25000000000000000000000000; //25M
    ...
    uint256 public tokensPerBlock = 5000000000000000; // reward tokens created per block
    ...
    uint256 public blockPerDay = 41000;
    ...
    constructor(address _penaltyAddress, uint256 _startBlock) public {
      PENALTY_ADDRESS = _penaltyAddress;

      if (_startBlock == 0) _startBlock = block.number;
      uint8[5] memory _emissionAmounts = [5, 4, 3, 2, 1];
      uint24[5] memory _emissionDelays = [0, 43200, 2 * 43200, 3 * 43200, 4 * 43200];
    ...
    // use lockedTokens & unlockedTokens to see pending tokens
    ...
    function claim(bool _all) external nonReentrant {
      ...
      Claim(msg.sender, sumUnlocked.add(sumLocked.div(2)), sumLocked.div(2));
    }
    ...
    function unlockedTokens(address _user) external view returns (uint256) {
      ...
      for (uint256 i = 0; i < pending[_user].length; i++){
        if (pending[_user][i].unlockBlock <= block.number){
          sumUnlocked.add(pending[_user][i].amount);
        }
      }
    ...
    function lockedTokens(address _user) external view returns (uint256) {
      ...
      for (uint256 i = 0; i < pending[_user].length; i++){
        if (pending[_user][i].unlockBlock > block.number){
          sumLocked.add(pending[_user][i].amount);
        }
      }
