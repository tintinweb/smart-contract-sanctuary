...
contract FarmFactory is ReentrancyGuard, Ownable {
    ...
    function build() external payable nonReentrant {
        ...
        require(build.poolAddresses[2] == address(0));
        require(build.poolDepositFees[0] == 0);
        require(build.poolDepositFees[1] == 0);
        require(build.poolDepositFees[2] == 0);
        masterChef.add(build.poolWeights[0], IERC20(address(farmToken)), 0);
        masterChef.add(build.poolWeights[1], IERC20(newPair), 0);
        masterChef.add(build.poolWeights[2], IERC20(0x6b140057F140e082B2501399915D9629751b011e), 0);

        for (uint i = 3; i < build.poolWeights.length; i++) {
