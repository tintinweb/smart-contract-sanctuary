...
contract Strategy_USDC_QUICK is Ownable {
    ...
    function _addLiquidityFromAllTokens() internal {
        _swapExactTokenForToken(router, WMATIC, token0, getBalanceOfToken(WMATIC)/2);
        _swapExactTokenForToken(routerReward, WMATIC, token1, getBalanceOfToken(WMATIC));
    ...
    function _addLiquidityETH(uint256 _amountETH) internal {
        _swapExactETHForTokens(router, token0, _amountETH/2);
        _swapExactETHForTokens(routerReward, token1, _amountETH/2);
    ...
    function _swapExactETHForTokens(address _router, address _token, uint256 _amountETH) internal {
        ...
        path[0] = WMATIC;
        path[1] = _token;
        uint256[] memory amounts = IRouter(_router).getAmountsOut(_amountETH, path);
        uint256 desiredAmountToken = amounts[1];
        if(desiredAmountToken > 100){
            IRouter(_router).swapExactETHForTokens{value:_amountETH}(
    ...
    function _swapLPtoWMATIC(uint256 _percent) internal {
        ...
        if (_totalLP > 0) {
            ...
            _approveToken(token0, router);
            _approveToken(token1, routerReward);

            _removeLiquidity();

            _swapExactTokenForToken(routerReward, QUICKToken, WMATIC, getBalanceOfToken(QUICKToken));
            _swapExactTokenForToken(routerReward, token1, token0, getBalanceOfToken(token1));
