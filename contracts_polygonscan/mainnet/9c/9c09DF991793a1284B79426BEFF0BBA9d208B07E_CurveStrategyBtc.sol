...
contract CurveStrategyBtc is BaseStrategy {
    ...
    address public pool = 0xC2d95EEF97Ec6C17551d45e77B590dc1F9117C67; //outer pool contract
    ...
    address[] public wmatic_btc_path;
    address[] public crv_btc_path;
    address[] public crv_wmatic_path;

    constructor() public BaseStrategy(btcCRV, FUND, WMATIC, sushiRouter, btcCRV_deposit) {
        multiHarvest = 0x3355743Db830Ed30FF4089DB8b18DEeb683F8546;

        wmatic_btc_path = new address[](3);
        wmatic_btc_path[0] = WMATIC;
        wmatic_btc_path[1] = WETH;
        wmatic_btc_path[2] = WBTC;

        crv_btc_path = new address[](3);
        crv_btc_path[0] = CRV;
        crv_btc_path[1] = WETH;
        crv_btc_path[2] = WBTC;
        ...
        IERC20(btcCRV).safeApprove(btcCRV_deposit, uint256(-1));
        IERC20(WBTC).safeApprove(pool, uint256(-1));
    ...
    function crvToWant() public onlyHumanOrWhitelisted {
        ...
        if (_crv > 0) {
            _swapUniswapWithPathPreapproved(crv_btc_path, _crv);
        }

        //Deposit WBTC in pool
        uint256 _btcBalance = IERC20(WBTC).balanceOf(address(this));
        if (_btcBalance > 0) {
            //IERC20(WBTC).safeApprove(pool, 0);
            //IERC20(WBTC).safeApprove(pool, _btcBalance);
