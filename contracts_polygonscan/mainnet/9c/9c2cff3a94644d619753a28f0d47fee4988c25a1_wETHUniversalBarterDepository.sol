...
contract wETHUniversalBarterDepository is Ownable {
    ...
    function deposit( 
        ...
        require( payout >= 10000000, "Barter too small" ); // must be > 0.01 USV ( underflow protection )
    ...
    function payoutFor( uint _value ) public view returns ( uint ) {
        return FixedPoint.fraction( _value, barterPrice() ).decode112with18().div( 1e14 );
    }
    ...
    function barterPriceInUSD() public view returns ( uint price_ ) {
        price_ = barterPrice().mul( uint( assetPrice() ) ).mul( 1e6 );
    }
    ...
    /**
     *  @notice debt ratio in same terms as reserve barters
     *  @return uint
     */
