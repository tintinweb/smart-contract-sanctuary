...
contract PolyCard is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        for (uint256 i = 12; i < 45; i++) {
            uint256 cardSplit = random(string(abi.encodePacked(toString(tokenId), toString(i))));
            uint256 cardCatagory = cardSplit % 12;
            if (cardCatagory % 12 < 5) {
                creatureCount++;
            } else if (cardCatagory % 12 < 6) {
                artifactCount++;
            } else if (cardCatagory % 12 < 8) {
                spellCount++;
            } else {
                enchantmentCount++;
            }
        ...
        output = string(
            abi.encodePacked(
                output,
                "</text> <text fill='white' xml:space='preserve' style='white-space: pre;' font-family='Georgia' font-size='8' letter-spacing='0em'><tspan x='32' y='308.291'></tspan></text>"
            )
        );

        output = string(
            abi.encodePacked(
                "<svg xmlns='http://www.w3.org/2000/svg' preserveAspectRatio='xMinYMin meet' viewBox='0 0 350 350'> <rect width='350' height='350' fill='url(#paint0_linear)'/> <rect width='318' height='318' transform='translate(16 16)' fill='#16150f'/> <text fill='white' xml:space='preserve' style='white-space: pre;' font-family='Georgia' font-size='20' font-weight='bold' letter-spacing='0em'><tspan x='32' y='62.1865'></tspan></text> <text fill='#8e7cc3' xml:space='preserve' style='white-space: pre;' font-family='comic sans ms' font-size='22' font-weight='bold' letter-spacing='0.16em'><tspan x='32' y='43.582'>PolyCards</tspan></text> <text fill='white' xml:space='preserve' style='white-space: pre;' font-family='flemish script' font-size='14' letter-spacing='0em'>",
                output,
                "<defs> <linearGradient id='paint0_linear' x1='175' y1='350' x2='175' y2='0' gradientUnits='userSpaceOnUse'> <stop stop-color='#8e7cc3'/> <stop offset='1' stop-color='#6a329f'/> </linearGradient> </defs></svg>"
            )
        );
            ...
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "Poly Card #',
                        stringTokenId,
                        '", "description": "PolyCards is a collectable game. Mint them and have fun.", "image": "data:image/svg+xml;base64,',
    ...
    function mintPublic(address _to) public {
        require(_mintPerAddress[ _to] < MAX_PER_ADDRESS, "You have reached your minting limit.");
    ...
    constructor() ERC721("PolyCards", "PCRD") Ownable() {}
}
