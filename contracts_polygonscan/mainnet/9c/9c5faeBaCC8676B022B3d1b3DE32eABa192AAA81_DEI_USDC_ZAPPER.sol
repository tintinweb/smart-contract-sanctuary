...
contract DEI_USDC_ZAPPER is Ownable {
	...
	function getAmountOutLPERC20ORNativecoin(uint amountIn, uint deusPriceUSD, uint collateralPrice, uint iterations, address[] memory path) public view returns (uint percentage, uint lp) {
		uint totalUsdcAmount = amountIn;
		if(path[0] != usdcAddress) {
			totalUsdcAmount = IUniswapV2Router02(uniswapRouter).getAmountsOut(amountIn, path)[path.length - 1];
		}

		(uint swapAmountUsdc, ) = getSwapAmountUSDC(totalUsdcAmount, deusPriceUSD, collateralPrice, iterations);
	...
	function getSwapAmountUSDC(uint amountIn, uint deusPriceUSD, uint collateralPrice, uint iterations) public view returns(uint, uint) {
		...
		for (uint256 i = 0; i < iterations; i++) {
			...
			(y, , ) = IDEIProxy(deiProxyAddress).getUSDC2DEIInputs(x / 1e12, deusPriceUSD, collateralPrice);
	...
	function getSwapAmountByPath(uint amountIn, uint deusPriceUSD, uint collateralPrice, uint iterations, address[] memory path) public view returns(uint, uint) {
		uint totalUsdcAmount = IUniswapV2Router02(uniswapRouter).getAmountsOut(amountIn, path)[path.length-1];

		return getSwapAmountUSDC(totalUsdcAmount, deusPriceUSD, collateralPrice, iterations);
	}
