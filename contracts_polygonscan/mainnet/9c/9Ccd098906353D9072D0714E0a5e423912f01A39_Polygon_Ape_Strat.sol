...
abstract contract Strat is Ownable, ReentrancyGuard, Pausable {
    ...
    function earn() public virtual nonReentrant whenNotPaused {
        ...
        // Harvest farm tokens
        _unfarm(0);
        // Converts farm extra wmatic tokens into reward tokens
        uint256 extraEarnedAmt = IERC20(wmaticAddress).balanceOf(address(this));
        
        if (extraEarnedAmt > 0){
        IERC20(wmaticAddress).safeApprove(uniRouterAddress, 0);
        IERC20(wmaticAddress).safeIncreaseAllowance(
            uniRouterAddress,
            extraEarnedAmt
        );
             // Swap into earned tokens
            _safeSwap(
                uniRouterAddress,
                extraEarnedAmt,
                slippageFactor,
...
contract Polygon_Ape_Strat is Strat {
    constructor(
    ...
    function _farm() internal override {
        ...
        IApeFarm(farmContractAddress).deposit(pid, wantAmt, address(this));
    }

    function _unfarm(uint256 _wantAmt) internal override {
        if (_wantAmt > 0) {
            IApeFarm(farmContractAddress).withdraw(
                pid,
                _wantAmt,
                address(this)
            );
        } else {
            IApeFarm(farmContractAddress).harvest(pid, address(this));
        }
    }
}