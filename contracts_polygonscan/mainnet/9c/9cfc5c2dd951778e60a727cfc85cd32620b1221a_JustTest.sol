...
contract JustTest is Ownable {
    ...
    event Deposit(address indexed sender, address indexed recipient, uint256 amount, uint256 shares, uint256 lastDepositedTime);
     ...
     * Creates a NEW stake
     * 
     * Can work as a Gift Deposit as well
     *  -> Gifts a time-locked stake to another wallet
     * 
     * The funds can be withdrawn at any time (premature withdrawals are penalized)
     * Optionally users can choose to irreversibly lock themself in
     * This option will also be used in distribution to free-claimers
     */
    function deposit(uint256 _amount, address _toAddress, uint256 _minToServeInSecs) external {
        ...
        emit Deposit(msg.sender, _toAddress, _amount, currentShares, block.timestamp);
    }
