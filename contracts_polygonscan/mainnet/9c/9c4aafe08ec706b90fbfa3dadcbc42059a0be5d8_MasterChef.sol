...
//-----------------------------------------------------------------------------
contract HungerGamesToken is BEP20("HungerGames Token", "HungerGames", 10000000000000000000000) {
...
contract MasterChef is Ownable, NonReentrant {
  ...
  struct PoolInfo {
    ...
    uint256 accHungerGamesPerShare;
    uint16 depositFeeBP;
  }
  //---------------------------------------------------------------------------
  HungerGamesToken public hungerGames;
  ...
  constructor(HungerGamesToken _hungerGames, address _feeAddress, uint256 _startBlock) public {
    hungerGames = _hungerGames;
 ...
 //---------------------------------------------------------------------------
  function add(uint256 _allocPoint, IBEP20 _lpToken, bool _withUpdate) public ownerOnly isUnique(_lpToken) {
    ...
    poolInfo.push(PoolInfo({lpToken: _lpToken, allocPoint: _allocPoint, lastRewardBlock: lastRewardBlock, accHungerGamesPerShare: 0, depositFeeBP: depositFee})
    );
  }
  function pendingHungerGames(uint256 _pid, address _user) external view returns (uint256)  {
    PoolInfo storage pool = poolInfo[_pid];
    UserInfo storage user = userInfo[_pid][_user];
    uint256 share = pool.accHungerGamesPerShare;
    uint256 pot = pool.lpToken.balanceOf(address(this));
    if (block.number > pool.lastRewardBlock && pot != 0) {
      uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
      uint256 HungerGamesReward = multiplier.mul(EMISSION_RATE).mul(pool.allocPoint).div(totalAllocPoint);
      share = share.add(HungerGamesReward.mul(1e12).div(pot));
    }
  ...
  function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
    ...
    updatePool(_pid);
    if (user.amount > 0) {
      uint256 pending = user.amount.mul(pool.accHungerGamesPerShare).div(1e12).sub(user.rewardDebt);
      if (pending > 0) {
        safeHungerGamesTransfer(msg.sender, pending);
      }
    }
    if (_amount > 0) {
    ...
    user.rewardDebt = user.amount.mul(pool.accHungerGamesPerShare).div(1e12);
  ...
  function updatePool(uint256 _pid) public {
    ...
    uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
    uint256 HungerGamesReward = multiplier.mul(EMISSION_RATE).mul(pool.allocPoint).div(totalAllocPoint);
    hungerGames.mint(feeAddress, HungerGamesReward.div(20));
    hungerGames.mint(address(this), HungerGamesReward);
    pool.accHungerGamesPerShare = pool.accHungerGamesPerShare.add(HungerGamesReward.mul(1e12).div(lpSupply));
    pool.lastRewardBlock = block.number;
  }
  ...
  function safeHungerGamesTransfer(address _to, uint256 _amount) internal {
    uint256 pot = hungerGames.balanceOf(address(this));
    bool success = false;
    if (_amount > pot) {
      success = hungerGames.transfer(_to, pot);
    } else {
      success = hungerGames.transfer(_to, _amount);
    }
    require(success, "error");
  }
