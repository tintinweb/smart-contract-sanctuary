    ...
    ReentrancyGuard
{
    ...
    //counter to mantain token ID
    Counters.Counter private _tokenIds;

    event Burn(address indexed user, uint256 tokenId);
    event Mint(address indexed user, uint256 indexed tokenId, string metaData);
    ...
    /**
     * @notice to be called to mint token
    ...
    /**
     * @notice called when user wants to burn token
     * @param tokenId tokenId to burn
     */
    function burn(uint256 tokenId) external {
        require(
            _msgSender() == ownerOf(tokenId),
            "ArGoNFT: INVALID_TOKEN_OWNER"
        );
        _burn(tokenId);
    ...
    function transfer(address recipient, uint256 tokenId) external {
        ...
        emit Transfer(_msgSender(), recipient, tokenId);
    }
