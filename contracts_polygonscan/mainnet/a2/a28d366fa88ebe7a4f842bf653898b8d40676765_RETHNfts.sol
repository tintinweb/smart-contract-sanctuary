...
contract RETHNfts is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, AccessControl, ERC721Burnable {
   ...
   event mintSuccessful(address indexed to, uint256 indexed tokenId);
    ...
    Counters.Counter public _tuupIdCounter;
    Counters.Counter public _kuturIdCounter;
    Counters.Counter public _happahIdCounter;
    Counters.Counter public _ranganIdCounter;
   
    
    constructor() ERC721("Realms of Ethernity Genesis Collection", "ROE-NFT") {
    ...
    function setMinBid(uint256 _val) public validate {
        minBid = _val;
    }
    
    function _baseURI() internal pure override returns (string memory) {
        return "realmsofethernity.com/items/";
    }
    

    function mintTuup(address to, string memory uri) public payable validate {
        uint256 tokenId = _tuupIdCounter.current() + 1000;
        _tuupIdCounter.increment();
        require(msg.value >= minBid, 'Please Enter amount higher than bid');
        if(tokenId < 1101){
        _safeMint(to, tokenId);
    ...
    function mintHappah(address to, string memory uri) public payable validate {
        uint256 tokenId = _happahIdCounter.current() + 2000;
        _happahIdCounter.increment();
        require(msg.value >= minBid, 'Please Enter amount higher than bid');
        if(tokenId < 2101){
        _safeMint(to, tokenId);
        _setTokenURI(tokenId, uri);
    ...
    function mintKutur(address to, string memory uri) public payable validate {
        uint256 tokenId = _kuturIdCounter.current() + 3000;
        _kuturIdCounter.increment();
        require(msg.value >= minBid, 'Please Enter amount higher than bid');
        if(tokenId < 3101){
        _safeMint(to, tokenId);
        _setTokenURI(tokenId, uri);
    ...
    function mintRangan(address to, string memory uri) public payable validate {
        uint256 tokenId = _ranganIdCounter.current() + 4000;
        _ranganIdCounter.increment();
        require(msg.value >= minBid, 'Please Enter amount higher than bid');
        if(tokenId < 4101){
        _safeMint(to, tokenId);
        _setTokenURI(tokenId, uri);
