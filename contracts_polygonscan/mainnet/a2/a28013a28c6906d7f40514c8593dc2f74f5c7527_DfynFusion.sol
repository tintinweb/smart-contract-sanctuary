...
contract DfynFusion is Initializable, OwnableUpgradeable {
    ...
    function swapTokensToToken(
        ...
        // IERC20Upgradeable(outputTokenAddress).approve(
        //             address(exchangeRouter),
        //             MAX
        //         );
        uint256 amountOut = IERC20Upgradeable(outputTokenAddress).balanceOf(address(this));

        require(
            amountOut >= minOutputAmount,
            "Expect amountOut to be greater than minOutputAmount."
        );
        IERC20Upgradeable(outputTokenAddress).safeTransfer(
            msg.sender,
            amountOut
        );
    }
    ...
    function swapTokensToLP(
        ...
        IERC20Upgradeable(outputLP).safeTransfer(msg.sender, amountOut);
    }
        ...
        returns (uint256[] memory)
    {
        ...
        for (uint256 i = 0; i < inputTokens.length; i++) {
            IERC20Upgradeable(inputTokens[i].token).safeTransferFrom(
                msg.sender,
                address(this),
        ...
        returns (uint256[] memory)
    {
        ...
        for (uint256 i = 0; i < inputLPs.length; i++) {
            IERC20Upgradeable(inputLPs[i].token).safeTransferFrom(
                msg.sender,
                address(this),
                inputLPs[i].amount
            );
        ...
        returns (InputToken[] memory)
    {
        ...
        for (uint256 i = 0; i < inputLPs.length; i++) {
            IERC20Upgradeable(inputLPs[i].token).approve(address(exchangeRouter), MAX);
        ...
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < inputTokens.length; i++) {
            ...
            if (inputTokens[i].token != address(wNative)) {
                IERC20Upgradeable(inputTokens[i].token).approve(
                    address(exchangeRouter),
                    MAX
                );
    ...
    function _optimalSwapToLp(
        ...
        IERC20Upgradeable(token0).approve(address(exchangeRouter), MAX);
        IERC20Upgradeable(token1).approve(address(exchangeRouter), MAX);
        (
