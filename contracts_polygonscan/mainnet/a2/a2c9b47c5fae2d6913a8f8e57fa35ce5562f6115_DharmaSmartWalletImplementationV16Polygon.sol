...
/**
 * @title DharmaSmartWalletImplementationV16Polygon
 * @author 0age
 * @notice The V16 implementation for the Dharma smart wallet is a non-custodial,
 * meta-transaction-enabled wallet. This implementation also contains methods to
 ...
 * `CREATE2`, which allows for their address to be known ahead of time. V16
 * deprecates old functionality, fixes an improperly-configured safety check on
 * advanced calls and includes the chain ID when deriving action IDs.
 */
contract DharmaSmartWalletImplementationV16Polygon is
  ...
  // The "Upgrade Beacon" references the current implementation contract.
  address internal immutable _UPGRADE_BEACON;
  ...
  address internal immutable _ACCOUNT_RECOVERY_MANAGER;
  ...
  DharmaEscapeHatchRegistryInterface internal immutable _ESCAPE_HATCH_REGISTRY;
  ...
  // The "recovered key rings" registry overrides initial signing key if set.
  RecoveredKeyRingsInterface internal immutable _RECOVERED_KEY_RINGS;
  ...
  constructor(
    ...
    address escapeHatchRegistry,
    address revertReasonHelper,
    address recoveredKeyRings
  ) {
    require(
      upgradeBeacon != address(0),
      "DharmaSmartWalletImplementationV16#constructor: No upgradeBeacon address supplied."
    );
    ...
    require(
      accountRecoveryManager != address(0),
      "DharmaSmartWalletImplementationV16#constructor: No accountRecoveryManager address supplied."
    );
    _ACCOUNT_RECOVERY_MANAGER = accountRecoveryManager;

    require(
      escapeHatchRegistry != address(0),
      "DharmaSmartWalletImplementationV16#constructor: No escapeHatchRegistry address supplied."
    );
    _ESCAPE_HATCH_REGISTRY = DharmaEscapeHatchRegistryInterface(escapeHatchRegistry);
    ...
    require(
      recoveredKeyRings != address(0),
      "DharmaSmartWalletImplementationV16#constructor: No recoveredKeyRings address supplied."
    );
    _RECOVERED_KEY_RINGS = RecoveredKeyRingsInterface(recoveredKeyRings);
  }
  ...
  function initialize(address userSigningKey) external override {
    ...
    address recoveredKeyRing = _RECOVERED_KEY_RINGS.pop();
    if (recoveredKeyRing != address(0)) {
      _setUserSigningKey(recoveredKeyRing);
    } else {
  ...
  function getImplementation() external view returns (address implementation) {
    (bool ok, bytes memory returnData) = address(_UPGRADE_BEACON).staticcall("");
