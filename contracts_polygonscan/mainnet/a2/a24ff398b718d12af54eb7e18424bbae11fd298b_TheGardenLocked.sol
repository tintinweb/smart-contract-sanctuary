...
contract TheGardenLocked is Ownable {
    ...
    function deposit(uint256 _amount) public {
        ...
        // Deposit asset is only possible before the stakeUntil timestamp
        if(_amount > 0 && block.timestamp < pool.stakeUntil) {
    ...
    // View function to see if stake is still possible
    function canStake() public view returns (bool) {
        PoolInfo storage pool = poolInfo[0];
        return block.timestamp < pool.stakeUntil;
    }
