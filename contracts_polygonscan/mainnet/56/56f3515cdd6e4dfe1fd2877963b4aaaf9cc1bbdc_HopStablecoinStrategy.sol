...
contract HopStablecoinStrategy is BaseStrategyStakingRewards {
    ...
    address public hopSwap;
    address public WAULT_ROUTER = 0x3a1D87f206D12415f5b0A33E786967680AAb4f6d; //wault has both WMATIC/USDC and WMATIC/USDT pairs
    address public FUND = 0x01fE07ce760DA7a025e4Cf3f950aE236F8e62120;

    address[] wmatic_underlying_path;
        ...
        )
    {
        ...
        wmatic_underlying_path = new address[](2);
        wmatic_underlying_path[0] = WMATIC;
        wmatic_underlying_path[1] = _underlying;

        IERC20(_underlying).safeApprove(hopSwap, uint256(-1));
        IERC20(WMATIC).safeApprove(WAULT_ROUTER, uint256(-1));
    }
    ...
    function harvest() public override onlyHumanOrWhitelisted nonReentrant {
        ...
        if(underlying != WMATIC) {
            uint256 remainingHarvested = IERC20(harvestedToken).balanceOf(address(this));
            if (remainingHarvested > 0) {
                _swapUniswapWithPathPreapproved(wmatic_underlying_path, remainingHarvested);
            }
        }

        //Deposit underlying in pool to get LP tokens
        uint256 _underlyingBalance = IERC20(underlying).balanceOf(address(this));
        if (_underlyingBalance > 0) {
            uint256[] memory amounts = new uint256[](2);
            amounts[0] = _underlyingBalance;
            amounts[1] = 0;

            IStableSwap(hopSwap).addLiquidity(amounts, 0, now.add(60));
        }
    }
