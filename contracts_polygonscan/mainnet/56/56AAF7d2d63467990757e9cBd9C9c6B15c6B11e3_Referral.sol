...
contract Referral is Ownable {
    ...
    mapping(address => uint256) public referralsCount; // referrer => referrals count
    mapping(address => uint256) public referralsCount2; // referrer => depth 2 referrals count
    ...
    function recordReferral(address _user, address _referrer) public onlyOperator {
        ...
        address _referrer2 = referrers[_referrer];
        if (_referrer2 != address(0)) {
            referralsCount2[_referrer2] += 1;
        }
    }

    function claimReward() public {
        address user = _msgSender();
        uint256 reward = reward1 * referralsCount[user] + reward2 * referralsCount2[user];
        if (rewardClaimed[user] >= reward) {
            return;
        }

        rewardToken.safeTransferFrom(rewardVault, user, reward - rewardClaimed[user]);
