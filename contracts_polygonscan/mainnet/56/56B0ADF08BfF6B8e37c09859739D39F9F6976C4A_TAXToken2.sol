...
contract TAXToken2 is ERC20Burnable {

  uint16 constant public TRANSFER_TAX_RATE = 10;
  uint16 constant public BURN_RATE = 10;
  address immutable public masterchef;
  address immutable public bridge;
  
  constructor(string memory _name,address _masterchef, address _bridge) ERC20("Tax Token", _name) {
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
    if (recipient == masterchef || sender == masterchef || sender == bridge) {
      ...
      uint256 taxAmount = amount * TRANSFER_TAX_RATE / 100;
      ...
      require(amount == sendAmount + taxAmount &&
          taxAmount == burnAmount + farmAmount, "sum error");
