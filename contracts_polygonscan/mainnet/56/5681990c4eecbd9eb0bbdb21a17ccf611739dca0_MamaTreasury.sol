...
contract MamaTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SMAMA }
    ...
    uint public sMAMAQueue; // Delays change to sMAMA address
    ...
    /**
        @notice allow approved address to deposit an asset for MAMA
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint MAMA needed and store amount of rewards for distribution
    ...
    /**
        @notice allow approved address to burn MAMA for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IMAMAERC20( MAMA ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sMAMA ).balanceOf( msg.sender ); // Can only borrow against sMAMA held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with MAMA
        @param _amount uint
     */
    function repayDebtWithMAMA( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IMAMAERC20( MAMA ).burnFrom( msg.sender, _amount );
    ...
    /**
        @notice returns MAMA valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match MAMA decimals
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SMAMA ) { // 9
            sMAMAQueue = block.number.add( blocksNeededForQueue );
        } else return false;
