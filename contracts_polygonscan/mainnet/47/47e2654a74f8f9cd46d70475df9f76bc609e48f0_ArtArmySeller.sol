...

{
    ...
    */
    ...
    /**
   * @dev Cancel the sale of the Token
   *
   */
    function cancelSale(uint256 tokenId) external returns (bool){
        require( _sale[tokenId].onSale, "The NFT must be on sale");
        require(_sale[tokenId].sellerWallet == payable(_msgSender()), "Art Army Sale: The NFT is not on sale by the wallet sender");

        _nftContract.safeTransferFrom(address(this), _msgSender(), tokenId);

        _sale[tokenId].onSale = false;
        _sale[tokenId].price = 0;
        _sale[tokenId].curatorFeeInBasisPoints = 0;
        _sale[tokenId].curatorWallet = payable(_burnWallet);
        _sale[tokenId].sellerWallet = payable(_burnWallet);
