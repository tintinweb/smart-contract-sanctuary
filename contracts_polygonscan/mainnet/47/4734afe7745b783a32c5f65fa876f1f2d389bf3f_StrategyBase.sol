...
abstract contract BaseStrategyMasterChef is BaseStrategy {
    ...
    function getHarvestable() external override view returns (uint256) {
        return IMasterChef(rewards).pendingLith(poolId, address(this));
    }
...
abstract contract StrategyFarmBase is BaseStrategyMasterChef {
    ...
    address public constant lithium = 0xfE1a200637464FBC9B60Bc7AeCb9b86c0E1d486E;
    ...
    // Uniswap swap paths
    address[] public lithium_matic_path;
    ...
    {
        lithium_matic_path = new address[](2);
        lithium_matic_path[0] = lithium;
        lithium_matic_path[1] = matic;
    }
    ...
    function harvest() public override {
        ...
        //Swap 
        uint256 _lithiumBalance = IERC20(lithium).balanceOf(address(this));
        if (_lithiumBalance > harvestCutoff){
            
            //Send performance fee to strategist
            uint256 performanceFee = _lithiumBalance.mul(keepFXS).div(keepFXSmax);
            if (performanceFee>0) {
                _swapUniswapWithPath(lithium_matic_path, performanceFee);
            ...
            // Swap rest
            _lithiumBalance = IERC20(lithium).balanceOf(address(this));
            _swapUniswapWithPath(lithium_matic_path, _lithiumBalance.div(2));

            // Add liquidity
            uint256 _lithium = IERC20(lithium).balanceOf(address(this));
            uint256 _matic = IERC20(matic).balanceOf(address(this));
            if (_lithium > 0 && _matic > 0) {
                IERC20(lithium).safeApprove(currentRouter, 0);
                IERC20(lithium).safeApprove(currentRouter, _lithium);
                ...
                // Donates DUST
                IERC20(lithium).safeTransfer(
                    strategist,
                    IERC20(lithium).balanceOf(address(this))
                );
                IERC20(matic).safeTransfer(
                    strategist,
                    IERC20(matic).balanceOf(address(this))
...
contract StrategyBase is StrategyFarmBase {
    // Token addresses
    address public PWAC_MASTER_CHEF = 0xfcD73006121333C92D770662745146338E419556;
    address public Lithium_Matic_LP = 0xF47553EB96b8665d9F258E3F4FC9A9e7811C3C2B;
