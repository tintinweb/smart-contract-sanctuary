...
contract MadnessKingDomNFT is Ownable, ReentrancyGuard, Gamificable {
    ...
    function withdraw() external nonReentrant {
        ...
        if (user.hasNFT) {
            uint256 _pendingExperience = _calculatedUserAmount(msg.sender).mul(poolInfo.accExperiencePerShare).div(PRECISION_FACTOR).sub(user.experienceDebt);

            poolInfo.supplyWithNFT = poolInfo.supplyWithNFT.sub(_calculatedUserAmount(msg.sender));

            user.experience = user.experience.add(_pendingExperience);

            theEndlessFactoryNFT.setExperience(user.nftID, _getNFTExperience(user.nftID).add(user.experience.div(1e18)));
        }
    ...
    function _userHarvestExperience(address _userAddress) internal {
        ...
        if (pending > 0) {
            user.experienceDebt = newUserAmount.mul(poolInfo.accExperiencePerShare).div(PRECISION_FACTOR);
