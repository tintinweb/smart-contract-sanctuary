...
contract SmartChef is Ownable {
    ...
    // GALAXY tokens created per block.
    ...
    // The block number when GALAXY mining starts.
    uint256 public startBlock;
    // The block number when GALAXY mining ends.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 galaxyReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
    ...
    function depositReward(uint256 _depositAmt) external returns (bool) {
        if (_depositAmt > 0) {
            IERC20(rewardToken).safeTransferFrom(msg.sender, address(this), _depositAmt);
        }
        return true;
    }

    // Stake GALAXY tokens to SmartChef
    ...
    // Withdraw GALAXY tokens from STAKING.
