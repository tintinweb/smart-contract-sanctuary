...
contract MacaronVault is Ownable, Pausable {
    ...
    struct UserInfo {
        ...
        uint256 macaronAtLastUserAction; // keeps track of macaron deposited at the last user action
    ...
    /**
     * @notice Deposits funds into the Macaron Vault
     * @dev Only possible when contract not paused.
     * @param _amount: number of tokens to deposit (in MACARON)
     */
    function deposit(uint256 _amount) external whenNotPaused notContract {
        ...
        user.macaronAtLastUserAction = user.shares.mul(balanceOf()).div(totalShares);
    ...
    /**
     * @notice Reinvests MACARON tokens into MasterChef
     * @dev Only possible when contract not paused.
     */
     ...
     * @return Expected reward to collect in MACARON
     */
    function calculateHarvestMacaronRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingMacaron(0, address(this));
     ...
     */
    function calculateTotalPendingMacaronRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingMacaron(0, address(this));
        amount = amount.add(available());
    ...
    /**
     * @notice Withdraws from funds from the Macaron Vault
    ...
    function withdraw(uint256 _shares) public notContract {
        ...
        if (user.shares > 0) {
            ...
            user.macaronAtLastUserAction = 0;
        }
