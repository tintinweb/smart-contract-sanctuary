...
contract LinearVestingFarm is OwnableUpgradeable, ReentrancyGuardUpgradeable{
    ...
    {
        uint256 totalRewardsUnlocked = 0;
        uint256 totalRewardsLocked = 0;
        uint256 totalWithdrawn = 0;

        for(uint256 i = 0; i < participants.length; i++){
            address user = participants[i];
            if(totalUserPayouts[user] == totalUserRewards[user]){
                totalRewardsUnlocked += totalUserRewards[user];
            }
            else{
                totalRewardsUnlocked += earned(user);
            }
            totalWithdrawn += totalUserPayouts[user];
        }

        totalRewardsLocked = totalRewards.sub(totalRewardsUnlocked);
