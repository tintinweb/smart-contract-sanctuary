...
contract LPRaffle is Ownable, VRFConsumerBase {
  ...
  constructor(address _vrf, address _link, bytes32 _keyHash, uint256 _fee, address _lpToken) VRFConsumerBase(_vrf, _link) {
    keyHash = _keyHash;
    fee = _fee;
  ...
  function gatherBalances() public view returns (Balances[] memory, uint256) {
  ...
  function fulfillRandomness(bytes32, uint256 randomness) internal override {
    ...
    for (uint16 i = 0; i < balances.length; i++) {
      if (!blacklist[balances[i].account] && winnerTicket < balances[i].balance) {
