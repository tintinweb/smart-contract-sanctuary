...
import "./PronteraPolygon.sol";

contract MorrocViewPolygon {
    PronteraPolygon public immutable prontera;

    constructor(PronteraPolygon _prontera) {
        prontera = _prontera;
    }
    ...
    function pendingKSW(IIzludeV2, address) public pure returns (uint256) {
        return 0;
    }
    ...
    function kafraMaxFee(IIzludeV2 izlude) public view returns (uint256) {
        if (address(izlude.feeKafra()) == address(0)) {
            return 0;
        }
        return IFeeKafra(izlude.feeKafra()).MAX_FEE();
    }

    function kafraMaxAllocation(IIzludeV2 izlude) public view returns (uint16) {
        if (address(izlude.allocKafra()) == address(0)) {
            return 0;
        }
        return IAllocKafra(izlude.allocKafra()).MAX_ALLOCATION();
    }
    ...
    function limitAllocation(IIzludeV2 izlude) public view returns (uint16) {
        ...
        return IAllocKafra(izlude.allocKafra()).limitAllocation();
    }
    ...
    function canAllocate(
        ...
        return
            IAllocKafra(izlude.allocKafra()).canAllocate(
                amount,
    ...
    function withdrawFee(IIzludeV2 izlude) public view returns (uint256) {
        ...
        return IFeeKafra(izlude.feeKafra()).withdrawFee();
    }
    ...
    function allocPoint(address) public pure returns (uint256) {
        return 0;
    }
    ...
    function izludeInfo(IIzludeV2 izlude, address user) external view returns (IzludeInfoItem memory izludeItem) {
        ...
        izludeItem.pendingKSW = pendingKSW(izlude, user);
...
contract PronteraPolygon is Ownable, ReentrancyGuard {
    ...
    // Info of each user.
    struct UserInfo {
        uint256 jellopy; // How many jellopy the user has provided.
        uint256 storedJellopy; // how many jellopy in storage
    }
    ...
    event AddPool(address indexed izlude);
    ...
    constructor(address _juno, address _junoGuide) {
        juno = _juno;
        junoGuide = _junoGuide;
    }

    // Add a new izlude to the pool.
    function add(address izlude) external onlyOwner {
        require(IIzludeV2(izlude).prontera() == address(this), "nope");
        require(poolInfo[izlude].izlude == address(0), "duplicated");

        poolInfo[izlude] = PoolInfo({want: IIzludeV2(izlude).want(), izlude: izlude});
        totalPool += 1;
        emit AddPool(izlude);
    }
