...
contract Protocol2Resolver {
    ...
    function getFee() public view returns (uint256) {
        return protocolModule.fee();
    }

    function getTokenAvailability(address[] memory tokens_, uint256[] memory amounts_) public view returns (bool, uint256) {
        uint256 length_ = tokens_.length;
        require(amounts_.length == length_, "lengths-not-same");
        bool availability = true;
        for (uint256 i = 0; i < length_; i++) {
            (, uint256 borrowExchangePrice) = liquidity.updateInterest(tokens_[i]);
            uint256 borrowLimitRaw = liquidity.protocolBorrowLimit(address(protocolModule), tokens_[i]);
            uint256 borrowLimit = borrowLimitRaw.mul(borrowExchangePrice).div(1e18);
            IERC20 token_ = IERC20(tokens_[i]);
            if(token_.balanceOf(address(liquidity)) < amounts_[i] ||  borrowLimit < amounts_[i]) {
                availability = false;
                break;
            }
        }
        return (availability, getFee());
    }
}
