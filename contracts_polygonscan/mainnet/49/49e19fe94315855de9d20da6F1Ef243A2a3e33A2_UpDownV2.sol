...
interface IRandomNumberGenerator {
    function getRandomNumber(uint256 index,  uint256 seed, uint256 number, uint256 blockHash) external returns (uint256 randomNumber);
}


contract UpDownV2 is IERC721Locker,OwnableUpgradeable, ReentrancyGuard {
    ...
    function initialize(address _meeb, address _nft, address _randomNumberGenerator, uint256[4] memory _bonusReward) external initializer {
        meeb = IERC20(_meeb);
        nft = _nft;
    ...
    function getBoostedIndex(uint256 _tokenId) public view returns (uint256 _index) {
        if (_tokenId > 0) {
            (,uint16 luckStat,,) = IMeebMasterNFT(nft).getStats(_tokenId);
            // luckStat range: (120, 392)
            // boosted range: (2%, 30%)
            if (luckStat >= 400) _index = 1; // 30%
            else if (luckStat <= 120) _index = 3; // 2%
