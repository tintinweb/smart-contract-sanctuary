...
contract Stake {
    ...
    mapping(address => userDetails)public jftDetails;
    ...
    function depositJft(uint _amount) public isLock {
        ...
        userDetails storage _user = jftDetails[msg.sender];
        token.transferFrom(msg.sender,address(this),_amount);
        _user.depAmount += _amount;
        _user.time = block.timestamp;
    }
    
    function deposit(uint _type,uint256 _pid,uint256 amount) public isLock isContractCheck(msg.sender){
         ...
         require(jftDetails[msg.sender].depAmount >= commission,"Insufficieent JFT");
         token.transfer(commissionAddr,commission);
         jftDetails[msg.sender].depAmount -= commission;
