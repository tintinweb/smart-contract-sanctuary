...
contract MasterChefV2 is Ownable {
    ...
    struct UserInfo {
        ...
        uint256 lastest;  // Until Lock timestamp.
    ...
    function withdraw(uint256 _pid, uint256 _amount) public payable {
        ...
        updatePool(_pid);
        uint256 fee = 30;
        if (block.timestamp > user.lastest + 3 days) fee = 0;
        uint256 pending = user.amount.mul(pool.accSushiPerShare).div(1e12).sub(user.rewardDebt);
        if(pending > 0) {
            safeSushiTransfer(msg.sender, pending);
        }
        ...
        if(_amount > 0) {
            ...
            uint256 buybackAmount = _amount.mul(fee).div(1000);
    ...
    function emergencyWithdraw(uint256 _pid) public {
        ...
        emit EmergencyWithdraw(msg.sender, _pid, user.amount.mul(97).div(100));
        user.amount = 0;
        user.rewardDebt = 0;
    }
}