...
contract PirateBankPoly is WhitelistUpgradeable {
    ...
    address private constant PIRATE_BNB = 0xb416a46438d9ae2F82b14016Bf9EE13018001f57;
    address private constant KEEPER = 0x3953Eb16238faB5c3C5C535Ea1c5310AEF13AC31;
    IPantherRouter02 private constant WEX_ROUTER = IPantherRouter02(0x3a1D87f206D12415f5b0A33E786967680AAb4f6d);
    ...
    function initialize(address _pool) external initializer {
        ...
        setTokenRouter(address(0), 0x3a1D87f206D12415f5b0A33E786967680AAb4f6d); // BNB -> Wault router
        setTokenRouter(PIRATE, 0x3a1D87f206D12415f5b0A33E786967680AAb4f6d); // PIRATE -> Wault router
        dividendRate = 50; // 5%
        PIRATE_POOL = _pool;

        ERC20(WBNB).approve(address(WEX_ROUTER), uint(~0));
        ERC20(PIRATE).approve(address(WEX_ROUTER), uint(~0));
    }
    ...
    // @dev only token owners can deposit capital into the bank
    ...
    function processDividends(address token, uint amount) internal returns (uint dividendPayout) {
        ...
        if (token == address(0)) {
            ...
            WEX_ROUTER.addLiquidityETH{value: wbnbAmount}(PIRATE, pirateAmount, 0, 0, address(this), block.timestamp);
            ...
            WEX_ROUTER.addLiquidity(WBNB, PIRATE, wbnbAmount.sub(wbnbAmount.div(2)), pirateAmount, 0, 0, address(this), block.timestamp);
        }
    ...
    function processDividendsManually(uint amount) internal returns (uint dividendPayout) {
        ...
        wbnbAmount = dividendPayout.div(2);
        pirateAmount = _swapFromBNB(address(WEX_ROUTER), WBNB, dividendPayout.sub(wbnbAmount), PIRATE, address(this));
        WEX_ROUTER.addLiquidityETH{value: wbnbAmount}(PIRATE, pirateAmount, 0, 0, address(this), block.timestamp);
    ...
    function _approveTokenIfNeeded(address token) private {
        if (ERC20(token).allowance(address(this), address(WEX_ROUTER)) == 0) {
            ERC20(token).approve(address(WEX_ROUTER), uint(~0));
        }
    }
