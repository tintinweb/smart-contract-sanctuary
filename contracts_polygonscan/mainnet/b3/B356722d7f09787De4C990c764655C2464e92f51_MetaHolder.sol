...
contract MetaHolder is ReentrancyGuardUpgradeable, OwnableUpgradeable
{
    ...
    function burnXHGT() external onlyOperator{
        ...
        require(createFeeInfo.inAmount > createFeeInfo.outAmount, "MetaBase: counter error or no need to burn");

        uint256 amount = createFeeInfo.inAmount.sub(createFeeInfo.outAmount);
        IERC20 erc20token = IERC20(xhgt);

        uint256 balance = erc20token.balanceOf(address(this));
        require(balance >= amount, "MetaBase: Not enough XHGT to burn"); //合约要销毁的XHGT不足
    ...
    function burnHGT() external onlyOperator{
        ...
        uint256 balance = erc20token.balanceOf(address(this));
        require(balance >= exchangeHGTAmount, "MetaBase: Not enough HGT to burn"); //合约要销毁HGT不足
    ...
    function exchangeHGTtoXHGT(uint256 amount) public  nonReentrant {
        require(amount > 0, "MetaBase: amount is invalid");

        IERC20 erc20HGT = IERC20(hgt);
        uint256 balance = erc20HGT.balanceOf(msg.sender);
        require(balance >= amount, "MetaBase: Not enough HGT"); //用户账户HGT不足

        IERC20 erc20XHGT = IERC20(xhgt);
        uint256 xbalance = erc20XHGT.balanceOf(address(this));
        require(xbalance >= amount, "MetaBase: Not enough XHGT"); //合约账户XHGT不足
