...
contract DexUpdates is Ownable, DexStorage {
  ...
  function updateCurrencyType(uint256[] memory tokenIds,uint256[] memory oldTokenIds, address oldC, address newC ) public {
  require(msg.sender == admin, "not authorize");
  require(tokenIds.length == oldTokenIds.length);
  for(uint256 i = 0; i< tokenIds.length; i++){
    _saleTokens[tokenIds[i]][address(newC)].currencyType = _saleTokens[oldTokenIds[i]][address(oldC)].currencyType;
  }
