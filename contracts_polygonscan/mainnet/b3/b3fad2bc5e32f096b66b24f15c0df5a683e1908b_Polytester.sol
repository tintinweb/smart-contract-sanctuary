...
// Polytester Token
contract Polytester is ERC20, Ownable {
    // Burn address
    ...
    // + transfer tax rate
    uint16 public transferGalaxyTax = 600;
    ...
    uint256 public minAmountToVaultCash = 30 * 10**18;
    ...
    // 
    uint256 public maxTransferAmountRate = 5000;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        ...
        // swap and vault cash  
        if (swapAndVaultEnabled && !_inSwapAndVaultCash && address(polygalaxySwapRouter) != address(0) && polygalaxySwapPair != address(0) && sender != polygalaxySwapPair) {
            swapAndVaultCash();
        }
    ...
    function updateVaultRate(uint16 _vaultRate) external onlyOperator {
        require(_vaultRate < 90, "GALAXY::updateVaultMarketingRate: rate invalid value!");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) external onlyOperator {
        require(_maxTransferAmountRate <= 10000, "GALAXY::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
