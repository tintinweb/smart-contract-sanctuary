...
contract Comptroller is ComptrollerV5Storage, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError {
    ...
    /// @notice Emitted when Atlantis Vault rate is changed
    event NewAtlantisVaultRate(address vault_, uint oldAtlantisVaultRate, uint newAtlantisVaultRate);

    /// @notice Emitted when Atlantis is distributed to Vault
    event DistributedVaultAtlantis(address vault_, uint amount);
    ...
    /**
     * @notice Set the amount of Atlantis distributed per block to Vault
     * @param vault_ The address of the Vault
     * @param atlantisVaultRate_ The amount of Atlantis wei per block to distribute to Vault
     */
    function _setAtlantisVaultRate(address vault_, uint atlantisVaultRate_) public {
    ...
    /**
     * @notice Set the Vault infos
     * @param vault_ The address of the Vault
     * @param releaseStartBlock_ The start block of release to Vault
     * @param minReleaseAmount_ The minimum release amount to Vault
     */
    function _setVaultInfo(address vault_, uint256 releaseStartBlock_, uint256 minReleaseAmount_) public {
    ...
    /**
     * @notice Transfer Atlantis to Vault
     */
    function releaseToVault() public {
        for (uint i = 0; i < vaults.length; i++) {
            if (address(vaults[i]) != address(0)) {
                _releaseToVaultInternal(vaults[i]);
            }
        }
    }

    function _releaseToVaultInternal(address vault_) internal {
