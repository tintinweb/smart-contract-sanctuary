...
contract UniHelper {
  ...
  constructor (address _weth) public {
    updateWethAddress(_weth);
  }
  function updateWethAddress (address _weth) public {
    WETH = IWETH(_weth);
  }
  ...
  function getReservesByPairs(IUniswapV2Pair[] calldata _pairs) external view returns (ResevresWithTokens[] memory) {
    ResevresWithTokens[] memory result = new ResevresWithTokens[](_pairs.length);
  ...
  function swap(
    ...
    console.log("CONTRACT WMATIC BALANCE: %s", balanceOf(address(this), router.WETH()));
    console.log("CONTRACT TOKENIN BALANCE: %s", balanceOf(address(this), _tokenIn));
    ...
    if (tokenOut.balanceOf(address(this)) > 0 && address(this) != _to) {
      ...
      require(tokenOut.transferFrom(address(this), _to, tokenOutBalance), "UniHelper.swap: Problem transferring token...");
    }
  ...
  function arbitrageSwap(
    ...
    IERC20(_buyToken).approve(address(_router0), _buyAmountIn);
    ...
    IERC20(_sellToken).approve(address(_router1), IERC20(_sellToken).balanceOf(address(this)));
    swap(
      _router1,
      _sellToken,
      _buyToken,
      IERC20(_sellToken).balanceOf(address(this)),
      0,
      payable(address(this))

    );
    IERC20(_buyToken).transfer(msg.sender, IERC20(_buyToken).balanceOf(address(this)));
  }
...
contract FlashLoaner is FlashLoanReceiverBase {
    ...
    UniHelper uniHelper;
    constructor(ILendingPoolAddressesProvider _addressProvider, address _uniHelper) FlashLoanReceiverBase(_addressProvider) {
  ...
  function executeArbitrage (bytes memory _data) public returns(bool, uint[] memory) {
    (
      ...
      address borrowToken,
      uint borrowAmount
    ) = abi.decode(_data, (IUniswapV2Router02, IUniswapV2Router02, address, address, address, uint));
    if (borrowToken == sellToken) {
      return simpleArbitrage(router0, router1, buyToken, sellToken, borrowAmount);
    } else if (borrowToken == buyToken) {
      return inverseArbitrage(router0, router1, buyToken, sellToken, borrowAmount);
    } else {
       return complexArbitrage(router0, router1, buyToken, sellToken, borrowToken, borrowAmount);
    }
  }
  ...
  function simpleArbitrage  (
    ...
    uint[] memory swappedAmounts = uniHelper.swap(    
      router1,
      address(sellToken),
      address(buyToken),
      borrowAmount,
  ...
  ) public returns(bool success, uint[] memory amounts) {
    uint[] memory swappedBorrowTokenAmounts = uniHelper.swap(    
      router0,
      address(borrowToken),
      address(sellToken),
      borrowAmount,
  ...
  // function UniswapSwapper (
  //   IUniswapV2Router02 router,
  //   IERC20[] memory path,
  //   uint[] memory amounts,
  //   address payable _to
  //  ) public returns(uint[] memory) {
  //    return uniHelper.swap(router, path, amounts, _to);
  // }

  function executeLiquidations (bytes memory _data) internal {
    console.log("Sorry, but this funtionality has not been implemented yet, and you just wasted gas to see this message");
  }
