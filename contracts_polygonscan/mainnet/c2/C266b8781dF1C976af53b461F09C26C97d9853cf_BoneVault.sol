...
// File: contracts/BoneVault.sol

pragma solidity >=0.6.0 <0.8.0;

contract BoneVault is Ownable, Pausable {
    ...
    uint256 public withdrawFee = 0; // 0.1%
    ...
    function deposit(uint256 _amount) external whenNotPaused notContract {
        ...
        user.cakeAtLastUserAction = user.amount.mul(balanceOf()).div(totalShares);
    ...
    function calculateHarvestCakeRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingBone(0, address(this));
    ...
    function withdraw(uint256 _shares) public notContract {
        ...
        require(_shares <= user.amount, "Withdraw amount exceeds balance");
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        uint256 pendingRew = IMasterChef(masterchef).pendingBone(pid, _user);
        return pendingRew.add(user.amount);
    }
