...
contract Hashku is Context, Ownable, ERC721, IERC2981 {
    ...
    // check if a msg sender is eligible to mint
...
// Root file: contracts/CalledIt.sol
...
* Contract by Matt Casanova [Twitter: @DevGuyThings]
* 
* Write text to the blockchain, with an off-chain service to dynamically render the NFTs
*
*/
...
contract CalledIt is Hashku {
    ...
    mapping(uint256 => Called) public tokenData;

    constructor() Hashku(
        "Called It!", 
        "CALL", 
        "https://storage.hashku.com/api/hashku/called-it/", 
        "CALL", 
        0, 
        0, 
        0, 
        1000000000000000, 
        5,
        0x9Da0bf8Dc92CBf37f4656Cd419BF4AFCdc40F0ca
    )  {
        isPublic = true;
    }

    function callIt(string[] calldata _lines, uint256 _target) external payable {
        require(!isClosed, "is_closed");
        require(_lines.length == 4, "requires_four_lines");
        require(msg.value == priceNumber, "incorrect_funds");
        if (_target > 0) {
            require(_target > block.timestamp, "not_in_future");
        }
        for (uint256 i; i < _lines.length; i++) {
            require(bytes(_lines[i]).length < 31, "line_too_long");
        }

        uint256 _tokenId = nextToken();
        tokenData[_tokenId] = Called(_lines[0], _lines[1], _lines[2], _lines[3], _target, block.timestamp);
        mint(_msgSender());
        delete _tokenId;
    }

    function whoops(uint256 _tokenId) external payable {
        require(_exists(_tokenId), "no_token");
        require(_msgSender() == ownerOf(_tokenId), "not_owner");
        require(msg.value == (priceNumber * 2), "incorrect_funds");
        tokenData[_tokenId] = Called("", "", "", "", 0, 0);
        _burn(_tokenId);
    }
}