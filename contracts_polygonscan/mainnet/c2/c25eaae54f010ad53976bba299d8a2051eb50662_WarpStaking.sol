...
contract WarpStaking is IStaking, WarpBase {
    ...
    // Penalty
    uint256 minDuration;
    uint256 earlyPenaltyPct; // 1 = 0.1%
    mapping(address => uint256) stakedAt;

    modifier onPlanet(address _recipient) {
        if (address(planets) != address(0) && planetId != 0) {
            // allow owner to stake ahead
            if (_recipient != owner) {
                require(
                    planets.onPlanet(_recipient, planetId),
                    'Stake: You are not docked on this planet'
                );
            }
        }
        _;
    ...
    function initialize(
        ...
        WARP = _WARP;
        require(_sWARP != address(0));
        sWARP = _sWARP;

        minDuration = 3600 * 3; // 3hours
        earlyPenaltyPct = 20; // 2%;
    ...
    /** @notice Turn contract into a planet pool */
    function planetStaking(address _planets, uint256 _planetId) external onlyOwner {
        planets = IPlanets(_planets);
        planetId = _planetId;
    }

    /**
        @notice stake WARP to enter warmup
        @param _amount uint
        @return bool
     */
    function stake(uint256 _amount, address _recipient)
        ...
        returns (bool)
    {
        /** Check to see if this is a staking portal off of */

        rebase();

        IERC20Upgradeable(WARP).safeTransferFrom(msg.sender, address(this), _amount);
        IERC20Upgradeable(sWARP).safeTransfer(_recipient, _amount);

        stakedAt[_recipient] = block.timestamp;
        return true;
    }

    /**
        @notice redeem sWARP for WARP
        @param _amount uint
        @param _trigger bool
     */
    function unstake(uint256 _amount, bool _trigger) external whenNotPaused {
        ...
        IERC20Upgradeable(sWARP).safeTransferFrom(msg.sender, address(this), _amount);

        if (stakedAt[msg.sender] + minDuration > block.timestamp) {
            uint256 penalized = ((_amount * earlyPenaltyPct) / 1000);
            IERC20Burnable(WARP).burn(penalized);
            IERC20Upgradeable(WARP).safeTransfer(msg.sender, _amount - penalized);
        } else {
            IERC20Upgradeable(WARP).safeTransfer(msg.sender, _amount);
        }
    }

    /**
        @notice returns the sWARP index, which tracks rebase growth
        @return uint
     */
    function index() public view override returns (uint256) {
        return IsWarp(sWARP).index();
    }
    ...
    function rebase() public override {
        if (epoch.endTime <= block.timestamp) {
            IsWarp(sWARP).rebase(epoch.distribute, epoch.number);
            ...
            uint256 staked = IsWarp(sWARP).circulatingSupply();
    ...
    /**
        @notice returns contract WARP holdings, including bonuses provided
        @return uint
     */
    function contractBalance() public view returns (uint256) {
        return IERC20Upgradeable(WARP).balanceOf(address(this));
    }

    /** @notice get staked at */
    function getStakedAt(address account) external view override returns (uint256) {
        return stakedAt[account];
    }

    /** @notice get minDuration */
    function getMinDuration() external view returns (uint256) {
        return minDuration;
    }

    /** @notice get penaltyPct */
    function getPenaltyPct() external view returns (uint256) {
        return earlyPenaltyPct;
    }

    /** @notice updateEpochEndtime */
    function updateEpochTime(uint256 _time) external onlyOwner {
        epoch.endTime = _time;
    }
    ...
    /** @notice set minimum duration to not incur a penalty */
    function setStakeMinDuration(uint256 _minDuration) external onlyOwner {
        minDuration = _minDuration;
    }

    /** @notice set minimum duration to not incur a penalty */
    function setEarlyPenaltyPct(uint256 _earlyPenaltyPct) external onlyOwner {
        earlyPenaltyPct = _earlyPenaltyPct;
    }
}
