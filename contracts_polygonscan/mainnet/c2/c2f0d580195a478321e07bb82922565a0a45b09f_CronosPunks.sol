...
contract CronosPunks is ERC721, Ownable, ERC721Enumerable {
    ...
    uint256 public constant MAX_BY_MINT = 20; // how many user can mint each time
    uint256 public constant PRICE = 500 * 10**18; // 500 CRO
    ...
    function mint(address _to, uint256 _count) public payable {
        ...
        pendingWithdrawals[developer] += msg.value;
    }
    ...
    function buyPunk(uint256 tokenId) public payable {
        ...
        require(tokenId < 10000, "tokenId out of range");
    ...
    function enterBidForPunk(uint256 tokenId) public payable {
        // require(marketPaused == false, "Market Paused");
        require(tokenId < 10000, "out of range");
        ...
        onlyTradablePunk(_msgSender(), tokenId)
    {
        ...
        require(bid.value != 0 && bid.value >= minPrice, "bid value");

        punkBids[tokenId] = Bid(false, tokenId, address(0), 0);
        _safeTransfer(_msgSender(), bid.bidder, tokenId, "");

        pendingWithdrawals[_msgSender()] += ((bid.value * 98) / 100);

        uint256 FeeAmount = ((bid.value * 2) / 100);
    ...
    function withdrawBidForPunk(uint256 tokenId) public {
        ...
        uint256 amountBid = punkBids[tokenId].value;
        punkBids[tokenId] = Bid(false, tokenId, address(0), 0);
