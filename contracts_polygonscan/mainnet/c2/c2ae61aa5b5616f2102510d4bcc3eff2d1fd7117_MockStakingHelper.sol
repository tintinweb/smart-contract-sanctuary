...
// File contracts/staking/TransferHelper.sol
...
// File contracts/staking/FullMath.sol
...
// File contracts/staking/EnumerableSet.sol
...
// File contracts/staking/Context.sol
...
// File contracts/staking/MockStakingHelper.sol
...
contract MockStakingHelper is Ownable, ReentrancyGuard, Pausable {
    using EnumerableSet for EnumerableSet.AddressSet;
    ...
    mapping(address => uint256) public userSpp;
    ...
    constructor(uint256 ppMultiplier) {
        SETTINGS.ppMultiplier = ppMultiplier;
    }

    function setPPMultiplier(uint256 _ppMultiplier) external onlyOwner {
        SETTINGS.ppMultiplier = _ppMultiplier;
    }

    function getUserSPP(address _user) external view returns (uint256) {
        return userSpp[_user];
    }

    function removeAllStakers() external onlyOwner {
        for (uint256 i = 0; i < users.length(); ) {
            delete userSpp[users.at(0)];
            users.remove(users.at(0));
        }
    }

    function setUserSPP(address[] memory _user, uint256[] memory _spp)
        external
        onlyOwner
    {
        for (uint256 i = 0; i < users.length(); ) {
        ...
        for (uint256 i = 0; i < _user.length; i++) {
            userSpp[_user[i]] = _spp[i];
            users.add(_user[i]);
        }
    }

    function addOrReplaceUserSpp(address[] memory _user, uint256[] memory _spp)
        external
        onlyOwner
    {
        for (uint256 i = 0; i < _user.length; i++) {
    ...
    function stakerAtIndex(uint256 _index) external view returns (address) {
