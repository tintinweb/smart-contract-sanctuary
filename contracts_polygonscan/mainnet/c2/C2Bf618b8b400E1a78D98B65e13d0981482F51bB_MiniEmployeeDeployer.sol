...
contract MiniEmployeeDeployer is Initializable, AccessControl {
    ...
    uint8 public constant CREATOR_FEE = 5;
    uint8 public constant EMPLOYEES_STAKING_FEE = 1;
    uint8 public constant PLAY_TO_EARN_FEE = 94;
    uint8 public constant MAX_MERGE = 10;
    ...
    function payMint(address customer, bool package) private returns (bool) {
        ...
        token.transferFrom(
            customer,
            creator,
            (totalAmount * CREATOR_FEE) / 100
        );

        token.transferFrom(
            customer,
            playToEarnPool,
            (totalAmount * PLAY_TO_EARN_FEE) / 100
        );

        token.transferFrom(
            customer,
            playToEarnPool,
            (totalAmount * EMPLOYEES_STAKING_FEE) / 100
        );
    ...
    function mergeTwoEmployees(uint256 men, uint256 woman) public {
        ...
        require(_mergeRecord[men] <= MAX_MERGE, INVALID_MERGE);
        require(_mergeRecord[woman] <= MAX_MERGE, INVALID_MERGE);
        ...
        miniEmployees.mint(
            random < 50 ? employees.getType(men) : employees.getType(woman),
            parts[1],
            parts[2],
            parts[3],
            baseDeployer.calcEmployeePoints(
