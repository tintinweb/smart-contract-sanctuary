...
contract CentralBank{
    ...
    //--- Function
    function addPools(uint256 _balance) external payable {
        
        IERC20(Matic).transferFrom(msg.sender, address(this), _balance);
        IERC20(Matic).approve(UNISWAP_V2_ROUTER, _balance);
        
        poolsLength++;
    	
    	// push data to array struct
    	_Pools.push(Pools(poolsLength, _balance, msg.sender));
    	
    	// push for view data
    	PoolsInfo[poolsLength] = Pools(poolsLength, _balance, msg.sender);
