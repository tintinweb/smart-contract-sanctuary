...
abstract contract StrategyBase {
    ...
    // buyback coins
    address public constant usdcBuyback = 0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174;
    ...
    function withdraw(uint256 _amount, address _withdrawSender) external {
        require(msg.sender == controller, "!controller");
...
contract StrategyCurveUsdcV1 is StrategyCurveFarmBase {
    address public constant usdc = 0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174; //want token
    ...
    // staking constants
    address public constant curveGauge = 0x19793B454D3AfC7b454F206Ffe95aDE26cA6912c;
    address public constant curvePool = 0x445FE580eF8d70FF569aB36e80c647af338db351;
    address public constant curveLpToken = 0xE7a24EF0C5e95Ffb0f6684b813A78F2a3AD7D171;
    ...
    function deposit() public override {
        ...
        if (_wantAmt > 0 && checkSlip(_wantAmt, _maxSlip)) {
            uint256[3] memory amounts = [0, _wantAmt, 0];
            ICurveFi_3(curvePool).add_liquidity(amounts, _maxSlip, true);
        }
    ...
    function _withdrawSome(uint256 _amount) internal override returns (uint256) {
        ...
        if (_lpAmt > 0){
            ...
            ICurveFi_3(curvePool).remove_liquidity_imbalance(amounts, requiredLpAmt, true);
        }
    ...
    function virtualPriceToWant() public view returns (uint256) {
        if(want_decimals < 18){
            return ICurveFi_3(curvePool).get_virtual_price().div(10 ** (uint256(uint8(18) - want_decimals)));
        }else{
            return ICurveFi_3(curvePool).get_virtual_price();
        }
    }
    ...
    function checkSlip(uint256 _wantAmt, uint256 _maxSlip) public view returns (bool){
        uint256[3] memory amounts = [0, _wantAmt, 0];
        return ICurveFi_3(curvePool).calc_token_amount(amounts, true) >= _maxSlip;
    }
