...
abstract contract StrategyFeesBase is Ownable, ReentrancyGuard, Pausable {
    ...
    address public constant rewardAddress = 0x3665e5F51008405afe498493eA46cDA18AA5E5Fa;
    ...
    uint256 public withdrawalFee = 10; // 0.1% withdraw fee
    uint256 public constant maxWithdrawalFee = 100; //1%

    uint256 public slippageFactor = 950; // 5% default slippage tolerance
    uint256 public liquiditySlippageFactor = 800; // 20% default liqidity add slippage tolerance
    uint256 public wmaticSlippageFactor = 900; // 10% default wmatic slippage tolerance
    ...
    function setSettings(
        ...
        require(_wmaticSlippageFactor <= slippageFactorUL, "_wmaticSlippageFactor too high");
        ...
        wmaticSlippageFactor = _wmaticSlippageFactor;
    ...
    function _safeSwapWmatic(
        ...
        IUniRouter02(uniRouterAddress).swapExactTokensForETH(
            _amountIn,
            amountOut.mul(wmaticSlippageFactor).div(1000),
            _path,
            _to,
            now.add(600)
        );
...
abstract contract StrategyBase is StrategyFeesBase {
    ...
    function stake(uint256 _wantAmount)  virtual internal;
    function harvest() virtual internal;
    function unstake(uint256 _amount) virtual internal;
    function earnedToWant() virtual internal;
    function convertDustToEarned() virtual external;
    function emergencyWithdraw() virtual public;
    
    uint256 public compoundCycle = 250;
    ...
    function changeCompoundCycle(uint256 _compoundCycle) external onlyOwner nonReentrant whenNotPaused {
        require(_compoundCycle >0, "StrategyQuickSwap: compound cycle has to be at least 1 block");
...
// File: Vault\StrategySingleBase.sol
...
// File: Vault\StrategySinglePolycat.sol


contract StrategySinglePolycat is StrategySingleBase {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    //polycat finance masterchef address
    address public polycatMasterChefAddress;
    uint256 pid;

    constructor(
        address _vaultChefAddress,
        address _polycatMasterChefAddress,
        address _wantAddress,
        address _earnedAddress,
        address _uniRouterAddress,
        uint256 _pid
        ...
        earnedToWmaticPath = [0x3a3Df212b7AA91Aa0402B9035b098891d276572B, address(0x000d500b1d8e8ef31e21c99d1db9a6444d3adf1270)];        
        earnedToUsdcPath = [0x3a3Df212b7AA91Aa0402B9035b098891d276572B, address(0x002791bca1f2de4661ed88a30c99a7a9449aa84174)];
        earnedToSiriusPath = [0x3a3Df212b7AA91Aa0402B9035b098891d276572B, address(0x000d500b1d8e8ef31e21c99d1db9a6444d3adf1270), 0xB1289f48E8d8Ad1532e83A8961f6E8b5a134661D];
        earnedToWantPath= [0x3a3Df212b7AA91Aa0402B9035b098891d276572B];
        wantToEarnedPath = [0x3a3Df212b7AA91Aa0402B9035b098891d276572B];
        
        polycatMasterChefAddress = _polycatMasterChefAddress;
        pid = _pid;
    ...
    function stake(uint256 _wantAmount)  internal override virtual{
        IPolycat(polycatMasterChefAddress).deposit(pid, _wantAmount, referralAddress);
    }

    function harvest() internal virtual override{
        //Polycat has no harvest function but harvest if you try to deposit 0
        IPolycat(polycatMasterChefAddress).withdraw(pid, 0);
    }

    function unstake(uint256 _amount) internal virtual override{
        IPolycat(polycatMasterChefAddress).withdraw(pid, _amount);
    }    

    function emergencyWithdraw() public virtual override onlyGov {
        IPolycat(polycatMasterChefAddress).emergencyWithdraw(pid);
    }
    
    function vaultSharesTotal() public virtual override view returns (uint256) {
        (uint256 amount, ) = IPolycat(polycatMasterChefAddress).userInfo(pid, address(this));
        return amount;
    }
    ...
    function _resetAllowances() internal virtual override {
        IERC20(wantAddress).safeApprove(polycatMasterChefAddress, uint256(0));
        IERC20(wantAddress).safeIncreaseAllowance(
            polycatMasterChefAddress,
            uint256(-1)
        );
    }
}