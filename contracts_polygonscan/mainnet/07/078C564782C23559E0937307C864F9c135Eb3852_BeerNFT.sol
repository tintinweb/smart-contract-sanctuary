...
import "./BeerNFTBase.sol";

contract BeerNFT is BeerNFTBase {
    ...
    constructor(
        ...
        public
        BeerNFTBase(_mintFeePerToken, _maxBeer)
    {
     ...
     */
    function _mintBeer(uint256 numberToken) internal returns (bool) {
        for (uint256 i = 0; i < numberToken; i++) {
            uint256 tokenIndex = totalSupply();
            if (tokenIndex < MAX_BEER) _safeMint(_msgSender(), tokenIndex);
        }
        return true;
    }

    function mintBeer(uint256 numberToken)
        public
        payable
        online
...
contract BeerNFTBase is Ownable, ERC721 {
    ...
    /**
     * @param _mintFeePerToken The Fee (in ETH) collector/user pays for Beer when one token is minted.
     * @param _MAX_BEER Total supply of contract
     */
    constructor(uint256 _mintFeePerToken, uint256 _MAX_BEER)
        public
        ERC721("Free Beer!", "BEER")
    {
        MAX_BEER = _MAX_BEER;
    ...
    modifier mintable(uint256 numberToken, uint256 totalSupply) {
        ...
        require(totalSupply.add(numberToken) < MAX_BEER, "Sold out");
        _;
    }
