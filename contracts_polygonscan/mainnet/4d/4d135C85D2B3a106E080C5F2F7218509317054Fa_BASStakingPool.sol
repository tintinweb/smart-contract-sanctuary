...
contract BASStakingPool is Ownable, ReentrancyGuard {
    ...
    // Refill rewards into Staking Pool
    function refillRewards(uint256 _amount) external nonReentrant{
        updatePool();
        rewardToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        remainingRewards = remainingRewards.add(_amount);
    }

    // Deposit BAS tokens to Staking Pool for USDT allocation.
    ...
    // Withdraw BAS tokens from Staking Pool.
    ...
    // Safe BAS transfer function, just in case if rounding error causes pool to not have enough USDT.
