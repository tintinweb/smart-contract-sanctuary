...
contract Strategy_GNS_DAI is Ownable {
    ...
    */
    ...
    function _getPriceWMATICToDAI() internal view returns (uint256) {
        /*
            DAI(token0) <-> GNS(token1)
            WETH <-> DAI
            WETH <-> WMATIC 
        */
        ...
        path[0] = WETH;
        path[1] = token0;
        return 1e36 / (_getAmountsOut(router, amountWETH, path) / 50);
    }
    ...
    function _getAmountDAIfromGNS(uint256 amountGNS) internal view returns (uint256 result) {
        ...
        path[0] = token1;
        path[1] = token0;
        return _getAmountsOut(router, amountGNS, path);
    }
