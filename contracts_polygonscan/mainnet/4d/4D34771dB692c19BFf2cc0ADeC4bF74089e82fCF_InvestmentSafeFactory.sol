...
contract InvestmentSafeFactory is Ownable, IInvestmentSafeFactory {
    ...
    uint256 public override immutable EXIT_GAS = 669661;
    uint256 public override immutable JOIN_GAS = 1348643;
    ...
    constructor(address _singleTokenJoin, address _singleTokenExit, address _weth, address _childTokenPortal, address _router) Ownable()
    {
    ...
    function getWethUsedForGas(uint256 gas) public override view returns (uint256 wethUsedForSafe) {
        wethUsedForSafe = 40000000000 * gas * maticToEthConversionRate / 1 ether;
    } 
    
    function getWethUsedForGas2(uint256 gas, uint256 gasprice) public  view returns (uint256 wethUsedForSafe) {
        wethUsedForSafe = gasprice * gas * maticToEthConversionRate / 1 ether;
    }
}
...
contract InvestmentSafe is EIP712, IInvestmentSafe {
    ...
    function initialize(
        ...
        IERC20(investmentSafeFactory.weth()).safeTransfer(_createor, amount);
    }
    ...
    function delegatedJoin(
        ...
        require(block.timestamp <= joinData.deadline, "JoinPermit: expired deadline");
        require(
        ...
        require(balance >= joinData.amount, "JoinPermit: amount exceeds balance");
        ...
        
        {
            ...
            address signer = ECDSA.recover(hash, joinData.v, joinData.r, joinData.s);
            require(signer == owner, "JoinPermit: invalid signature");
        
            weth.safeTransfer(msg.sender, gasCost);
        }

        address underlying = IPolygonERC20Wrapper(joinData.targetToken).underlying();
        ...
        _maxApprove(weth, investmentSafeFactory.singleTokenJoin(), amountAfterGasCost);
        ...
        IPolygonERC20Wrapper(joinData.targetToken).withdrawTo(minReturn, owner);
        emit DelegatedJoin(joinData.targetToken, owner, joinData.amount, minReturn);
    }
    ...
    function delegatedExit(
        ...
        require(block.timestamp <= exitData.deadline, "ExitPermit: expired deadline");
        require(
        ...
        address signer = ECDSA.recover(hash, exitData.v, exitData.r, exitData.s);
        ...
        _maxApprove(IERC20(exitData.sourceToken), investmentSafeFactory.singleTokenExit(), exitData.amount);
        ...
        _maxApprove(weth, investmentSafeFactory.childTokenPortal(), targetAmount);

        IChildTokenPortal(investmentSafeFactory.childTokenPortal()).getIOU(
            targetAmount
        );
        ...
        emit DelegatedExit(exitData.sourceToken, owner, exitData.amount, targetAmount);
    }
