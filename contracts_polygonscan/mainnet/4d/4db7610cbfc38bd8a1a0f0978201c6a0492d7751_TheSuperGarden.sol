...
contract TheSuperGarden is Ownable {
    ...
    function pendingRewardAll(address _user) external view returns (uint256[] memory) {
        ...
        for (uint256 i = 0; i < tokensLength; ++i) {
            ...
            if (block.number > poolInfo.lastRewardBlock && lpSupply != 0) {
                multiplier = getMultiplier(poolInfo.lastRewardBlock, block.number);
    ...
    function getAllUserInfo(address _user) external view returns (uint256,uint256[] memory) {
        UserInfo storage user = userInfo[_user];
        uint256[] memory rewardDebtTX = new uint256[](tokensLength);

        for (uint256 i = 0; i < tokensLength; ++i) {
            rewardDebtTX[i] = user.rewardDebtTX[i];
        } 
        
        return (user.amount,rewardDebtTX);
    }      
