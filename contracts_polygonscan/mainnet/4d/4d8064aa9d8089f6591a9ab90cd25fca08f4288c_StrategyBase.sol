...
abstract contract StrategyFarmBase is BaseStrategyMasterChef {
    ...
    // Uniswap swap paths
    address[] public gamma_matic_path;
    ...
    {
        gamma_matic_path = new address[](2);
        gamma_matic_path[0] = gamma;
        gamma_matic_path[1] = matic;
    }
    ...
    function harvest() public override {
        ...
        if (_gammaBalance > harvestCutoff){
            if (_gammaBalance > 0) {
                _swapUniswapWithPath(gamma_matic_path, _gammaBalance.div(2));
            }

            // Add liquidity for GAMMA/MATIC
            uint256 _matic = IERC20(matic).balanceOf(address(this));
            uint256 _gamma = IERC20(gamma).balanceOf(address(this));
            if (_matic > 0 && _gamma > 0) {
...
contract StrategyBase is StrategyFarmBase {
    ...
    address public GAMMA_MATIC_LP = 0x3402bD2db9D5A7B1cb82f95Bbc309dA03726c83e;
