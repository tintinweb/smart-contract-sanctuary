...
contract MeowAPI is Ownable {
  ...
  function meowPerSecondInPool(uint256 _pid) public view returns (uint256) {
    ...
    if (total == 0) return 0;
    return _allocPoint(_pid).mul(1e18).mul(1e18).div(totalAllocPoint().mul(1e18)).mul(meowPerSecond()).div(1e18);
  }
  ...
  function borrowerRewardAPY(address _vault, uint256 _pid) public view returns (uint256) {
    uint256 totalDebt = vaultDebtVal(_vault);
    if (totalDebt == 0) return 0;
    return rewardPerYear(_pid).mul(meowPrice()).mul(uint256(100)).mul(1e18).div(totalDebt.mul(baseTokenPrice(_vault)));
  }
  ...
  function getMaticPerToken(address _lp) public view returns (uint256) {
    ...
    return
      keccak256(bytes(symbol)) == keccak256(bytes("WMATIC"))
        ? uint256(reserve0).mul(uint256(IERC20(IUniswapV2Pair(_lp).token1()).decimals())).div(uint256(reserve1))
        : uint256(reserve1).mul(uint256(IERC20(IUniswapV2Pair(_lp).token0()).decimals())).div(uint256(reserve0));
  }
  ...
  function baseTokenPrice(address _vault) public view returns (uint256) {
    ...
    if (baseToken == wMatic) return (usdcPerMaticQuick+usdcPerMaticSushi)/2;
      ...
      return ((usdcPerMaticQuick.mul(10**decimals).div(tokenPerMaticQuick))+(usdcPerMaticSushi.mul(10**decimals).div(tokenPerMaticSushi)))/2;
  }
  ...
  function getQuickPrice(address _stakingRewardsFactory) public view returns (uint256) {
    address quick = IStakingRewardsFactory(_stakingRewardsFactory).rewardsToken();
  ...
  function getWorkerStakeAmountQuick(address _worker) public view returns (uint256) {
    address stakingRewards = getStakingRewards(_worker);
    return IStakingRewards(stakingRewards).balanceOf(_worker);
  }
  ...
  function getTVLSushiswap(address[] memory _workers) public view returns (bytes memory) {
    ...
    for (uint256 i = 0; i < _workers.length; i++) {
      ...
      uint256 amount = getWorkerStakeAmountSushi(worker);
...
interface IStakingRewardsFactory {
    ...
    function stakingRewardsInfoByStakingToken(address _stakingToken) external view returns (address, uint256, uint256);
