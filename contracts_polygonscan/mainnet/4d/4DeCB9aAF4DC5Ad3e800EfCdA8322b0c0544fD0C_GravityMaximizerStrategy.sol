...
/// @author Chainvisions
/// @notice Strategy for Gravity Finance Maximization.

contract GravityMaximizerStrategy is IStrategy, BaseUpgradeableStrategy {
    ...
    bytes32 internal constant _TARGET_VAULT_SLOT = 0x7790707bd7d3015aaead6b8bf6440f252706f2c8b431f91125825915230afd65;

    constructor() public BaseUpgradeableStrategy() {
        assert(_TARGET_VAULT_SLOT == bytes32(uint256(keccak256("eip1967.strategyStorage.targetVault")) - 1));
    }
    ...
    */
    ...
    function setRoute(address _tokenTo, address[] memory _route) public onlyGovernance {
        routes[_tokenTo] = _route;
    }
    ...
    */
    function _liquidateReward() internal {
        ...
        if(routes[token0].length > 1) {
            ...
            uint256[] memory amounts = IUniswapV2Router02(gravityRouter).swapExactTokensForTokens(toToken0, 0, routes[token0], address(this), block.timestamp.add(600));
        ...
        if(routes[token1].length > 1) {
            ...
            uint256[] memory amounts = IUniswapV2Router02(gravityRouter).swapExactTokensForTokens(toToken1, 0, routes[token1], address(this), block.timestamp.add(600));
        ...
        IUniswapV2Router02(gravityRouter).addLiquidity(token0, token1, token0Amount, token1Amount, 0, 0, address(this), block.timestamp.add(600));
    }
    ...
    function _notifyMaximizerReward() internal {
        ...
        IVault(targetVault()).notifyRewardAmount(targetVault(), targetVaultBalance);
    }
    ...
    */
    function withdrawAllToVault() public restricted {
        if(rewardPool() != address(0)) {
            IGFIFarm(rewardPool()).withdraw(rewardPoolStake());
        }
        _liquidateReward();
    ...
    */
    function withdrawToVault(uint256 amount) public restricted {
        ...
        if(amount > IERC20(underlying()).balanceOf(address(this))){
            ...
            IGFIFarm(rewardPool()).withdraw(Math.min(rewardPoolStake(), needToWithdraw));
        }
    ...
    */
    ...
    function _setTargetVault(address _address) internal {
        setAddress(_TARGET_VAULT_SLOT, _address);
    }
    
    function targetVault() public view returns (address) {
        return getAddress(_TARGET_VAULT_SLOT);
    }
