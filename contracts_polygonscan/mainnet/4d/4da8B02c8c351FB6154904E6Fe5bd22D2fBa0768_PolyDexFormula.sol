...
contract PolyDexFormula is IPolyDexFormula {
    ...
    function getReserveAndWeights(address pair, address tokenA) public override view returns (
        ...
        address token0 = IPolyDexPair(pair).token0();
        (uint reserve0, uint reserve1,) = IPolyDexPair(pair).getReserves();
        uint32 tokenWeight0;
        uint32 tokenWeight1;
        (tokenWeight0, tokenWeight1, swapFee) = getWeightsAndSwapFee(pair);

        if (tokenA == token0) {
            (tokenB, reserveA, reserveB, tokenWeightA, tokenWeightB) = (IPolyDexPair(pair).token1(), reserve0, reserve1, tokenWeight0, tokenWeight1);
        } else {
    ...
    function getAmountOut(
        ...
        // validate input
        require(amountIn > 0, 'PolyDexFormula: INSUFFICIENT_INPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, "PolyDexFormula: INSUFFICIENT_LIQUIDITY");
    ...
    function getAmountIn(
        ...
        // validate input
        require(amountOut > 0, 'PolyDexFormula: INSUFFICIENT_OUTPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, "PolyDexFormula: INSUFFICIENT_LIQUIDITY");
        // special case for equal weights
        if (tokenWeightIn == tokenWeightOut) {
    ...
    function getAmountsOut(address tokenIn, address tokenOut, uint amountIn, address[] calldata path) external override view returns (uint[] memory amounts) {
        require(path.length > 0, 'PolyDexFormula: INVALID_PATH');
        ...
        require(currentTokenIn == tokenOut, 'PolyDexFormula: INVALID_TOKEN_OUT_PATH');
    }
    ...
    function getAmountsIn(address tokenIn, address tokenOut, uint amountOut, address[] calldata path) external override view returns (uint[] memory amounts) {
        ...
        require(currentTokenIn == tokenIn, 'PolyDexFormula: INVALID_TOKEN_IN_PATH');
    }
    ...
    function getWeightsAndSwapFee(address pair) public view returns (uint32 tokenWeight0, uint32 tokenWeight1, uint32 swapFee) {
        try IPolyDexPair(pair).getTokenWeights() returns (uint32 _tokenWeight0, uint32 _tokenWeight1) {
            return (_tokenWeight0, _tokenWeight1, IPolyDexPair(pair).getSwapFee());
    ...
    function getFactoryWeightsAndSwapFee(address factory, address pair) public view returns (uint32 tokenWeight0, uint32 tokenWeight1, uint32 swapFee) {
        return IPolyDexFactory(factory).getWeightsAndSwapFee(pair);
    }
    ...
    function sortTokens(address tokenA, address tokenB) public pure override returns (address token0, address token1) {
        require(tokenA != tokenB, 'PolyDexFormula: IDENTICAL_ADDRESSES');
        (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'PolyDexFormula: ZERO_ADDRESS');
    }
    ...
    function getOtherToken(address pair, address tokenA) external view override returns (address tokenB) {
        address token0 = IPolyDexPair(pair).token0();
        address token1 = IPolyDexPair(pair).token1();
    ...
    function quote(uint amountA, uint reserveA, uint reserveB) external pure override returns (uint amountB) {
        require(amountA > 0, 'PolyDexFormula: INSUFFICIENT_AMOUNT');
        require(reserveA > 0 && reserveB > 0, 'PolyDexFormula: INSUFFICIENT_LIQUIDITY');
    ...
    function mintLiquidityFee(
        ...
        if (collectedFee0 > 0) {
            (uint r0, uint p0) = power(uint(collectedFee0).add(reserve0), reserve0, 50, 100);
            amount = amount.add(totalLiquidity.mul(r0) >> p0).sub(totalLiquidity);
        }
        if (collectedFee1 > 0) {
            (uint r1, uint p1) = power(uint(collectedFee1).add(reserve1), reserve1, 50, 100);
