...
// File: contracts/interfaces/IUniclyXUnicVault.sol
...
// File: contracts/UnicSwap/interfaces/IUnicFarm.sol
...
// File: contracts/UnicSwap/interfaces/IUnicSwapV2Router01.sol
...
// File: contracts/UnicSwap/interfaces/IUnicSwapV2Router02.sol
...
contract Zap is IZap, OwnableUpgradeable {
    ...
    IUnicFarm private constant FARM = IUnicFarm(0x0769fd68dFb93167989C6f7254cd0D766Fb2841F);
    IUnicSwapV2Router02 private constant ROUTER = IUnicSwapV2Router02(0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506);
    ...
    function zapInTokenAndDeposit(address _from, uint amount, uint _pid) external override {
        IERC20 lpToken = FARM.lpToken(_pid);
        ...
        FARM.deposit(_pid, depositAmount, msg.sender);
    }
    ...
    function zapInTokenFor(address _from, uint amount, address _to) private {
        ...
        if (isLP(_to)) {
            ...
            if (_from == token0 || _from == token1) {
                ...
                ROUTER.addLiquidity(_from, other, amount.sub(sellAmount), otherAmount, 0, 0, address(this), block.timestamp);
            } else {
    ...
    function _swapETHToLP(address lp, uint amount) private {
        if (!isLP(lp)) {
            ...
            if (token0 == WETH || token1 == WETH) {
                ...
                ROUTER.addLiquidityETH{value : amount.sub(swapValue)}(token, tokenAmount, 0, 0, address(this), block.timestamp);
            } else {
                uint swapValue = amount.div(2);
                ...
                ROUTER.addLiquidity(token0, token1, token0Amount, token1Amount, 0, 0, address(this), block.timestamp);
            }
        }
    }
    ...
    function _swapETHForToken(address token, uint value) private returns (uint) {
        ...
        uint[] memory amounts = ROUTER.swapExactETHForTokens{value : value}(0, path, address(this), block.timestamp);
    ...
    function _swapTokenForETH(address token, uint amount) private returns (uint) {
        ...
        uint[] memory amounts;
        amounts = ROUTER.swapExactTokensForETH(amount, 0, path, address(this), block.timestamp);
        return amounts[amounts.length - 1];
    }
    ...
    function _approveTokenIfNeeded(address token) private {
        if (!haveApprovedToken[token]) {
            ...
            IERC20(token).safeApprove(address(FARM), uint(- 1));
