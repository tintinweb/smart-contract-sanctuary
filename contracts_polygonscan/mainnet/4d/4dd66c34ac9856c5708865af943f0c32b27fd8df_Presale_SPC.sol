...
contract Presale_SPC is Owned {
    ...
    uint256 public mTokenDecimal = 18;
    address public pTokenAddress;
    uint256 public pTokenDecimal;
    address private recipient;

    uint256 public tokenSold = 0;
    uint256 public totalMIAAmount = 0;

    uint256 public minMiaLimit = 5 * (10 ** mTokenDecimal);
    uint256 public maxMiaLimit = 1000 * (10 ** mTokenDecimal);
    ...
    function set_pTokenDecimal(uint256 _pTokenDecimal) external onlyOwner {
        pTokenDecimal = _pTokenDecimal;
    }
    
    function set_mTokenDecimal(uint256 _mTokenDecimal) external onlyOwner {
        mTokenDecimal = _mTokenDecimal;
    }
    ...
    function buy(uint256 paidAmount) public {
        require(isPresaleOpen, "Presale is not open yet");
        require(whiteListed[msg.sender] == true, "You were't allowed" );
        require(paidAmount > minMiaLimit, "You need to sell at least some min amount");
        require(_paidTotal[msg.sender] + paidAmount <= maxMiaLimit, "It was over maxMiaLimit");
        ...
        uint tokenAmount = paidAmount.div(10**mTokenDecimal).div(tokenPrice).mul(10**pTokenDecimal);
