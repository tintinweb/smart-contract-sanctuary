...
interface IZAP {
    function zapTokenToLP(address fromToken, uint amount, address lpToken, address receiver) external returns (uint);
    function zapTokenToLP(address fromToken, uint amount, address lpToken, address receiver, uint minLPReceive) external returns (uint);
}
...
contract GrowLPStakingPool is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function balanceOf(address userAddress) public view returns(uint256) {
        if (totalSupply == 0) {
            ...
            return userShares[userAddress].mul(pricePreShare()).div(_DECIMAL);
        }
    }

    function pendingRewardsPreShare() public view returns(uint256) {
    ...
    function rewardsPreSharePreSecond() public view returns(uint256) {
        if (totalSupply == 0) {
            return 0;
        } else {
    ...
    function pricePreShare() public view returns(uint256) {
        if (totalSupply == 0) {
            ...
            return totalBalance().mul(_DECIMAL).div(totalSupply).add(pendingRewardsPreShare());
        }
    }
    ...
    function _deposit(uint256 amount, address userAddress) internal {
        ...
        // 2. shares
        uint256 sharesToAdd = amount.mul(_DECIMAL).div(pricePreShare());
    ...
    function _withdraw(uint256 amount, address user, address to) private {
        ...
        // 2. shares
        uint256 sharesToRemove = amount.mul(_DECIMAL).div(pricePreShare());
