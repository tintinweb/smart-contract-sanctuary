...
contract StrategyV2_ApeSwap is Ownable, ReentrancyGuard, Pausable {
    ...
    constructor(
        ...
        if (isAutoComp) {
            ...
            farmContractAddress = 0x54aff400858Dcac39797a81894D9920f16972D1D;
            pid = _pid;
            feeExt = _feeExt;
    ...
    function earn() public nonReentrant whenNotPaused {
        ...
        IXswapFarm(farmContractAddress).harvest(pid, address(this));
    ...
    function convertDustToEarned() public whenNotPaused {
        ...
        //Converts WMatic dust (if any) to earned tokens
        uint256 tokenwmAmt = IERC20(wmaticAddress).balanceOf(address(this));
        if (wmaticAddress != earnedAddress && tokenwmAmt > 0) {
            IERC20(token0Address).safeIncreaseAllowance(
                uniRouterAddress,
                token0Amt
            );
    ...
    function pause() public onlyAllowGov {
        _pause();
        IXswapFarm(farmContractAddress).emergencyWithdraw(pid, address(this));
    }
    ...
    function setNewFee(uint256 _newFee) public onlyAllowGov {
        // Thanks RugDoc for the recommendation.
        require (_newFee >= 0 && _newFee < 500,"depositFee invalid");
        //Thanks RugDoc for the recommendation.
        feeExt = _newFee;
    }
    ...
    function workerCompound() public view returns (uint256){
        ...
        uint256 Comp  = IXswapFarm(farmContractAddress).pendingBanana(pid, address(this));
