...
import "./libs/IWoodReferral.sol";
import "@openzeppelin/contracts/access/Ownable.sol";
import "@openzeppelin/contracts/utils/ReentrancyGuard.sol";
import "./WoodToken.sol";

// Forest is the master of Wood. He can make Wood and he is a fair guy.
//
...
contract Forest is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of WOODs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accWoodPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accWoodPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. WOODs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that WOODs distribution occurs.
        uint256 accWoodPerShare;   // Accumulated WOODs per share, times 1e12. See below.
    ...
    // WOOD tokens created per block.
    uint256 public woodPerBlock;
    // Bonus muliplier for early wood makers.
    uint256 public constant BONUS_MULTIPLIER = 1;
    // Max deposit fee: 4%.
    uint16 public constant MAXIMUM_DEPOSIT_FEE = 400;
    // Lock up block length: 15 days
    uint256 public constant LOCKUP_BLOCKS_LENGTH = 648000;
    // Unlock block length: 2 days
    uint256 public constant UNLOCK_BLOCKS_LENGTH = 86400;
    // Info of each pool.
    ...
    // The block number when WOOD mining starts.
    uint256 public startBlock;
    // Total locked up rewards
    uint256 public totalLockedUpRewards;

    // Wood referral contract address.
    IWoodReferral public woodReferral;
    ...
    constructor(
        WoodToken _wood,
        uint256 _startBlock,
        uint256 _woodPerBlock
    ) public {
        wood = _wood;
        startBlock = _startBlock;
        woodPerBlock = _woodPerBlock;
    ...
    // Update the given pool's WOOD allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending WOODs on frontend.
    function pendingWood(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accWoodPerShare = pool.accWoodPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 woodReward = multiplier.mul(woodPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accWoodPerShare = accWoodPerShare.add(woodReward.mul(1e12).div(lpSupply));
        }
        uint256 pending = user.amount.mul(accWoodPerShare).div(1e12).sub(user.rewardDebt);
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 woodReward = multiplier.mul(woodPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        wood.mint(devAddress, woodReward.div(10));
        wood.mint(address(this), woodReward);
        pool.accWoodPerShare = pool.accWoodPerShare.add(woodReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to Forest for WOOD allocation.
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        if (_amount > 0 && address(woodReferral) != address(0) && _referrer != address(0) && _referrer != msg.sender) {
            woodReferral.recordReferral(msg.sender, _referrer);
        }
        payOrLockupPendingWood(_pid);
        if (_amount > 0) {
            pool.lpToken.safeTransferFrom(address(msg.sender), address(this), _amount);
            if (address(pool.lpToken) == address(wood)) {
                uint256 transferTax = _amount.mul(wood.transferTaxRate()).div(10000);
        ...
        user.rewardDebt = user.amount.mul(pool.accWoodPerShare).div(1e12);
        emit Deposit(msg.sender, _pid, _amount);
    }

    // Withdraw LP tokens from Forest.
    ...
    // Pay or lockup pending WOODs.
    function payOrLockupPendingWood(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];

        uint256 pending = user.amount.mul(pool.accWoodPerShare).div(1e12).sub(user.rewardDebt);
        ...
        if (canHarvest()) {
            if (pending > 0 || user.rewardLockedUp > 0) {
                ...
                // Send reward
                safeWoodTransfer(msg.sender, totalRewards);
    ...
    // View function to see if user can harvest WOODs.
    function canHarvest() public view returns (bool) {
        if (block.number < startBlock) {
            return false;
        }
        uint256 modulusBlocks = block.number.sub(startBlock).mod(
            LOCKUP_BLOCKS_LENGTH.add(UNLOCK_BLOCKS_LENGTH)
        );
        // Harvest available only for UNLOCK_BLOCKS_LENGTH after LOCKUP_BLOCKS_LENGTH from startBlock, and then repeat same process.
        if (modulusBlocks < LOCKUP_BLOCKS_LENGTH) {
            return false;
        }
        return true;
    }

    // Safe wood transfer function, just in case if rounding error causes pool to not have enough WOODs.
    function safeWoodTransfer(address _to, uint256 _amount) internal {
        uint256 woodBal = wood.balanceOf(address(this));
        if (_amount > woodBal) {
            wood.transfer(_to, woodBal);
        } else {
            wood.transfer(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _woodPerBlock) public onlyOwner {
        massUpdatePools();
        emit EmissionRateUpdated(msg.sender, woodPerBlock, _woodPerBlock);
        woodPerBlock = _woodPerBlock;
    }

    // Update the wood referral contract address by the owner
    function setWoodReferral(IWoodReferral _woodReferral) public onlyOwner {
        woodReferral = _woodReferral;
    }
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (address(woodReferral) != address(0) && referralCommissionRate > 0) {
            address referrer = woodReferral.getReferrer(_user);
            uint256 commissionAmount = _pending.mul(referralCommissionRate).div(10000);

            if (referrer != address(0) && commissionAmount > 0) {
                wood.mint(referrer, commissionAmount);
                woodReferral.recordReferralCommission(referrer, commissionAmount);
