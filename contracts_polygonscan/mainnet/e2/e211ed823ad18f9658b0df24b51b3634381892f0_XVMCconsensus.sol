...
contract XVMCconsensus is Ownable {
    ...
    modifier whenReady() {
      require(block.timestamp > 1640106000, "after 21 Dec");
      _;
    }
	...
	*/
     ...
     */
	function initiateTreasuryTransferProposal(uint256 depositingTokens,  address tokenAddress, address recipient, uint256 amountToSend) external { 
    	require(
    ...
    */
	function approveTreasuryTransfer(uint256 proposalID) external {
		...
		require(
			treasuryProposal[proposalID].firstCallTimestamp + 3 * IXVMCgovernor(owner()).delayBeforeEnforce() < block.timestamp,
		...
		require(
		 ...
		 require(
            IacPool(acPool6).totalVotesFor(consensusID) >= IacPool(acPool6).totalShares() * 10 / 100,
				"No consensus in Pool 6"
        );
        require(
		...
		if(IacPool(acPool6).totalVotesFor(consensusID + 1) >= IacPool(acPool6).totalShares() * 3 / 100 ||
		    IacPool(acPool5).totalVotesFor(consensusID + 1) >= IacPool(acPool5).totalShares() * 3 / 100 ||
		        IacPool(acPool4).totalVotesFor(consensusID + 1) >= IacPool(acPool4).totalShares() * 3 / 100) 
        {
            treasuryProposal[proposalID].valid = false;
        } else {
    ...
    function changeGovernor(uint256 proposalID) external { 
        ...
        require(
            IacPool(acPool6).totalVotesFor(proposalID) >= IacPool(acPool6).totalShares() * 35 / 100,
				"No consensus in Pool 6"
        );
        require(
            IacPool(acPool5).totalVotesFor(proposalID) >= IacPool(acPool5).totalShares() * 15 / 100,
				"No consensus in Pool 5"
        );
        require(
            IacPool(acPool4).totalVotesFor(proposalID) >= IacPool(acPool4).totalShares() * 15 / 100,
				"No consensus in Pool 4"
        );
