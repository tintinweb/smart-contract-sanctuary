...
/**
 * @title UglyAssChickens contract
 */
contract UglyAssChickens is ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    modifier whenSaleActive {
        require(saleIsActive, "UglyAssChickens: Sale is not active");
        _;
    }

    modifier whenMetadataNotFrozen {
        require(!METADATA_FROZEN, "UglyAssChickens: Metadata already frozen.");
        _;
    }

    modifier whenProvenanceNotFrozen {
        require(!PROVENANCE_FROZEN, "UglyAssChickens: Provenance already frozen.");
        _;
    }

    constructor() ERC721("Ugly Ass Chickens", "CHICK") {
        saleIsActive = false;
        MAX_GLITCHES = 4444;
    ...
    function tokenURI(uint256 tokenId) public view virtual override(ERC721URIStorage, ERC721) returns (string memory) {
        ...
        return "ipfs://Qme6fLCXri2XtYpMGeuRfUNzgTeUVxgmDFpkyAM9yYJGfG";
    }
    ...
    function mintGlitches(uint256 amount) external payable whenSaleActive {
        ...
        require(totalSupply() + amount <= MAX_GLITCHES, "UglyAssChickens: Purchase would exceed cap");
    ...
    function mintForCommunity(address _to, uint256 _numberOfTokens) external onlyOwner {
        ...
        require(totalSupply() + _numberOfTokens <= MAX_GLITCHES, "UglyAssChickens: Minting would exceed cap");
