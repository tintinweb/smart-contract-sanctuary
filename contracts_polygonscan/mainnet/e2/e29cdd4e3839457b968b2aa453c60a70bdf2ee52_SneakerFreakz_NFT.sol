...
abstract contract ERC2981ContractWideRoyalties is ERC165, IERC2981Royalties {
    ...
    function _setRoyalties(address recipient, uint256 value) internal {
        require(value <= 500, 'ERC2981Royalties: Too high');
...
contract SneakerFreakz_NFT is ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;
    using Strings for uint256;

    uint256 public constant FREAKZ_PUBLIC = 10000;
    uint256 public constant FREAKZ_MAX = FREAKZ_PUBLIC;
    uint256 public constant PURCHASE_LIMIT = 10;
    uint256 public constant PRICE = 150_000_000_000_000_000_000; // 150 MATIC 
    ...
    uint256[FREAKZ_MAX] internal indices;
   
    Counters.Counter private _publicFREAKZ;

    constructor() ERC721("Sneakerfreakz NFT metaverse collection", "SNKRFRKZ") {}
    ...
    function randomIndex() internal returns (uint256) {
        uint256 totalSize = FREAKZ_PUBLIC;
        ...
        onlyOwner
    {
        require(
            ...
            "Chill, homie! 10 token max per mint"
        );
        require(
            _publicFREAKZ.current() < 10000,
        ...
        for (uint256 i = 0; i < numberOfTokens; i++) {
            ...
            if (_publicFREAKZ.current() < FREAKZ_PUBLIC) {
                _publicFREAKZ.increment();
                _safeMint(to, tokenId);
            }
        }
    }
    ...
    function purchase(uint256 numberOfTokens) external payable {
        ...
        require(
            _publicFREAKZ.current() < FREAKZ_PUBLIC,
            "Not gon' be able to do it!"
        );
        require(
            PRICE * numberOfTokens <= msg.value,
            "MATIC amount ain't sufficient, guap up."
        );
