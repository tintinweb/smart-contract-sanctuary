...
//Contract to let the other 1/2 of the ADDY that will be locked for former LP stakers vest for 91 days
contract VestingStorage is ReentrancyGuard, Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    address public weth = 0x7ceB23fD6bC0adD59E62ac25578270cFf1b9f619;

    address public feeDist = 0x920f22E1e5da04504b765F8110ab96A20E6408Bd;
    ...
    constructor(address _lpVaultStrategy) public {
        rewardsToken = IERC20(0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270);
        stakingToken = IERC20(0xc3FdbadC7c795EF1D6Ba111e06fF8F16A20Ea539);
        lpVault = _lpVaultStrategy; //make sure this is the LP vault's strategy contract
    }


    //Claim WMATIC from fee dist and send to LP vault's strategy contract
    function harvest() external nonReentrant {
        IMultiFeeDistribution(feeDist).getReward();
        uint256 wmaticBal = rewardsToken.balanceOf(address(this));
        rewardsToken.transfer(lpVault, wmaticBal);
    }

    /* ========== RESTRICTED FUNCTIONS ========== */

    //Mint the ADDY that will be initially distributed to this contract
    function mintAddy() external onlyOwner nonReentrant {
        require(!minted, "already minted");
        minted = true;

        //Set ADDY minting rate to 300 ADDY/WETH
        //mint 1500 * 300 = 450,000 ADDY, enough to cover roughly 10k LP
        IMinter(minter).mintFor(address(this), weth, 1500 * 1e18);
    }

    //Withdraw ADDY once it's finished vesting and send to LP vault's strategy contract
    function withdrawAddyToVault(uint256 _amount) external onlyOwner {
        IMultiFeeDistribution(feeDist).withdraw(_amount);
        stakingToken.safeTransfer(lpVault, _amount);
    }

    //Withdraw ADDY to owner wallet once it's finished vesting
    //This function exists because:
    //Multiple LP vaults could be deployed, so some ADDY would need to be manually deposited
    //LP vault could change
    //Too much ADDY was minted and not enough people exchanged, so the remainder will be burnt
    function withdrawAddy(uint256 _amount) external onlyOwner {
        IMultiFeeDistribution(feeDist).withdraw(_amount);
