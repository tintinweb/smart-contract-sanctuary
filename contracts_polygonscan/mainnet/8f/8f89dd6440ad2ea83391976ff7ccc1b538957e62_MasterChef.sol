...
//	Layer-2 from Geo Finance. Join the Official Telegram Group https://t.me/geoswapfinance	and Twitter https://twitter.com/GeoFinanceToken
...
contract EarthToken is ERC20("EarthFinance Token", "EARTH") {
...
// will be transferred to a governance smart contract once Earth is sufficiently
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of EARTH
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. EARTH to distribute per block.
        uint256 lastRewardBlock;  // Last block number that EARTH distribution occurs.
        uint256 accEarthPerShare;   // Accumulated EARTH per share, times 1e18. See below.
    ...
    uint256 public earthPerBlock = 0.02 ether;
    ...
    event UpdateEmissionRate(address indexed user, uint256 earthPerBlock);
    ...
    // Update the given pool's EARTH allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending EARTH on frontend.
    function pendingEarth(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accEarthPerShare = accEarthPerShare.add(earthReward.mul(1e18).div(lpSupply));
        }
        return user.amount.mul(accEarthPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 earthReward = multiplier.mul(earthPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        earth.mint(devAddress, earthReward.div(10));
        earth.mint(address(this), earthReward);
        pool.accEarthPerShare = pool.accEarthPerShare.add(earthReward.mul(1e18).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accEarthPerShare).div(1e18).sub(user.rewardDebt);
            if (pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accEarthPerShare).div(1e18);
    ...
    // Safe earth transfer function, just in case if rounding error causes pool to not have enough.
    ...
    function updateEmissionRate(uint256 _earthPerBlock) external onlyOwner {
        massUpdatePools();
        earthPerBlock = _earthPerBlock;
        emit UpdateEmissionRate(msg.sender, _earthPerBlock);
    }
