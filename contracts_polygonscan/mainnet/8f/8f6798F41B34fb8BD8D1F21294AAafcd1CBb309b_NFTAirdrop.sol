...
contract NFTAirdrop is Ownable, ERC721Holder {
    ...
    uint[] public maxValuesSeed;
    
    constructor() {
        users = [0x2D74573Ffaf04343CAfFAA516e94db8462c45229,
            0xdE1570EBDaF2cd56d71FE9e32CE146a288392889,
            0x37E61746AeBA77F38C634e4A8299D5c3fb12e60c];
        length = users.length;
        nft = BeastNFT(0xBD5be1F746cb21B90bc5B13f2C8CedAcB38a9b15);
        maxValuesSeed = [0,350,600,800,920,1000];
    }
    
    function run() public onlyOwner {
        bytes32 seed = blockhash(block.number - 2);
        for (uint8 i = 0; i< length; i++) {
            _openEgg(uint(seed), users[i]);
            seed = rehash(seed);
            _openEgg(uint(seed), users[i]);
            seed = rehash(seed);
            _openEgg(uint(seed), users[i]);
    ...
    function _openEgg(uint valuesSeed, address user) private {
        uint resultValuesSeed = 0;
        uint amount = 0;
        for (uint8 i = 0; i< 6; i++) {
            amount ++;

            resultValuesSeed *= 1000;

            uint currValueSeed = valuesSeed % 1000;
            valuesSeed /= 1000;

            uint valueIndex = 1;
            while (maxValuesSeed[valueIndex] <= currValueSeed) {
                valueIndex++;
            }

            resultValuesSeed += valueIndex;
        }

        uint tokenId = nft.createToken(user);
        nft.updateAbilities(tokenId, amount, resultValuesSeed);
