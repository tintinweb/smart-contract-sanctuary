...
contract RaritySkinManagerFix is Ownable {
    ...
    function myAdventurersYieldPerDay() external view returns (uint tokenAmount, uint numOfAdventurers) {
    ...
    function myRoguesYieldPerDay() external view returns (uint tokenAmount, uint numOfRogues) {
    ...
    function availableForClaimAll() external view returns (uint amount) {
        uint numOfSkins = myIERC721(skins).balanceOf(msg.sender);
        for(uint i = 0; i < numOfSkins; i++) {
            uint id = myIERC721(skins).tokenOfOwnerByIndex(msg.sender, i);
