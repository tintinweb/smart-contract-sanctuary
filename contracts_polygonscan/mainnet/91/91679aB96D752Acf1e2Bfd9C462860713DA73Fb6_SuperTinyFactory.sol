...
contract SuperTinyFactory is ISuperTinyFactory, OwnableUpgradeable, ReentrancyGuard {
    ...
    function totalBurned() external override view returns (uint256) {
        return ISuperTinyNFT(nft_).totalBurned();
    }
    ...
    function mintToken(address receiver, uint256 tokenId, ISuperTinyStats.SuperTinyInfo memory info) public override onlyAdmin {
        ...
        ISuperTinyStats(stats_).setInfo(tokenId, info);
        ++mintedCount_;
    ...
    function mintMultiTokens(address receiver, uint256[] memory tokenIds, ISuperTinyStats.SuperTinyInfo[] memory infos) external override onlyAdmin {
    ...
    function transferMultiTokens(address receiver, uint256[] memory tokenIds) external override{
        uint256 length = tokenIds.length;
        require(length >= 1, "zero token");
        ISuperTinyNFT _nft = ISuperTinyNFT(nft_);
        for (uint256 i = 0; i < length; i++) {
    ...
    function burnToken(uint256 tokenId) external override nonReentrant {
        ISuperTinyNFT _nft = ISuperTinyNFT(nft_);
        require(_nft.ownerOf(tokenId) == msg.sender, "SuperTinyFactory: burn token that is not own");
        _nft.burn(tokenId);
