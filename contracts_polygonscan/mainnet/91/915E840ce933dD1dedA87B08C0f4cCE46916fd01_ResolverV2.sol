...
contract ResolverV2 is Ownable {
  ...
  constructor(address _gotchiFacet, address _careCentre) {
    ...
    treasury = ITreasury(ICareCentre(_careCentre).treasury());

    maxGotchis = 500;
  }

  // to do : check if owner gotchi length is 0
  function checker()
    external
    view
    returns (bool canExec, bytes memory execPayload)
  {
    address[] memory caringOwners = careCentre.getCaringOwners();
    uint256 length = caringOwners.length;

    for (uint256 i = 0; i < length; i++) {
      address owner = caringOwners[i];

      if (!isOperatorApproved(owner)) continue;
      if (!hasFunds(owner)) continue;

      // to do: check for balance
      uint256[] memory gotchiIds = getGotchiIdsOfOwner(owner);

      if (gotchiIds.length == 0) continue;

      uint256 lastInteracted = gotchiFacet
        .getAavegotchi(gotchiIds[0])
        .lastInteracted;

      uint256 nextInteract = lastInteracted + 12 hours;

      if (block.timestamp >= nextInteract) {
        execPayload = getPayload(owner, gotchiIds);
        canExec = true;
  ...
  function getPayload(address owner, uint256[] memory gotchiIds)
    public
    view
    returns (bytes memory execPayload)
  {
    if (!vip.contains(owner) || fren) {
  ...
  function isOperatorApproved(address owner) public view returns (bool) {
    address operator = careCentre.operator();
    return (gotchiFacet.isApprovedForAll(owner, operator));
  }

  function hasFunds(address owner) public view returns (bool) {
    return (treasury.userBalance(owner) >= 0.3 ether);
  }

  function setMaxGotchis(uint256 max) external onlyOwner {
    maxGotchis = max;
  }

  function getGotchiIdsOfOwner(address owner)
    public
    view
    returns (uint256[] memory)
  {
    uint32[] memory gotchiIdsNotFiltered = gotchiFacet.tokenIdsOfOwner(owner);

    uint256 length = gotchiIdsNotFiltered.length;

    uint256 filterCount = 0;

    uint256[] memory filteredGotchis = new uint256[](maxGotchis);

    // count number of gotchis
    for (uint256 x = 0; x < length; x++) {
      uint256 claimTime = gotchiFacet.aavegotchiClaimTime(
        gotchiIdsNotFiltered[x]
      );
      if (claimTime != 0) {
        filteredGotchis[filterCount] = gotchiIdsNotFiltered[x];
        filterCount += 1;
      }
    }

    uint256[] memory gotchiIds = new uint256[](filterCount);

    // create gotchiIds array
    for (uint256 x = 0; x < filterCount; x++) {
      gotchiIds[x] = filteredGotchis[x];
    }

    return gotchiIds;
  }

  function getVips() external view returns (address[] memory _vip) {
    uint256 length = vip.length();
    _vip = new address[](length);
    for (uint256 i = 0; i < length; i++) _vip[i] = vip.at(i);
  }

  function setVip(address[] calldata owners) external onlyOwner {
    for (uint256 x; x < owners.length; x++) {
      vip.add(owners[x]);
    }
  }

  function removeVip(address[] calldata owners) external onlyOwner {
    for (uint256 x; x < owners.length; x++) {
      vip.remove(owners[x]);
    }
  }

  function setFren(bool _fren) external onlyOwner {
    fren = _fren;
  }
}
...
interface ICareCentre {
  ...
  function frenExec(address caringOwner, uint256[] calldata gotchiIds) external;

  function gotchiToFollow(address caringOwner)
    external
    view
    returns (uint256 gotchi);

  function caringOwnerPetCount(address caringOwner)
    external
    view
    returns (uint256 petCount);
