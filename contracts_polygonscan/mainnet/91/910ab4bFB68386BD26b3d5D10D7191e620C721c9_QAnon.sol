...
contract QAnon is ERC20Detailed {
    ...
    uint256 private constant INITIAL_FRAGMENTS_SUPPLY = 500000000 * 10**DECIMALS;
    ...
    uint256 public rebasePeriod = 45 days;
    uint256 public rebaseLimit = 100000000 * 10**DECIMALS;
    ...
    constructor(address adminAddress, address routerAddress) ERC20Detailed("QAnon", "Q", uint8(DECIMALS)) {
        // init Router
        ...
        _gonBalances[adminAddress] = TOTAL_GONS;
        emit Transfer(address(0x0), adminAddress, _totalSupply);
    }

    function canRebase(uint256 epoch) private view returns (uint256) {
        if (lastRebase + rebasePeriod <= epoch && _totalSupply > rebaseLimit) {
            uint256 periods = (epoch.sub(lastRebase)).div(rebasePeriod);
            uint256 burnTotal = 0;
            uint256 newSupply = _totalSupply;
            while (periods > 0) {
                uint256 burn1 = newSupply.mul(burnRate).div(100);
                burnTotal = burnTotal + burn1;
                newSupply = newSupply - burn1;
                periods--;
            }
            return newSupply > rebaseLimit ? burnTotal : _totalSupply.sub(rebaseLimit);
        } else {
            return 0;
        }
    }
    ...
    function _transferFrom(
        ...
        uint256 amountToRebase = canRebase(block.timestamp);
        if (amountToRebase > 0) {
            // Rebase 1% of total supply
            rebase(block.timestamp, -int256(amountToRebase));
        }
