    ...
    function _harvestReward(uint256 _pid, address _account) internal {
        ...
        if (_amount > 0) {
            ...
            if (_claimableAmount > 0) {
                require(_claimableAmount * 200 <= IERC20(meeb).totalSupply(), "Suspicious big reward amount!!"); // <= 0.5% total supply
    ...
    function _massUpdatePoolsWithNftLocked(address _account, uint256 _newLockedTokenId) internal {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            ...
            if (_amount > 0) {
                ...
                updatePool(pid);
                user.rewardDebt = (_getBoostedAmount(_amount, _newLockedTokenId) * pool.accMeebPerShare) / 1e18;
            }
        }
    }
