...
abstract contract StrategyGajBase is BaseStrategyMasterChef {

    // Token addresses for MATIC
    address public constant gaj = 0xF4B0903774532AEe5ee567C02aaB681a81539e92;
    address public constant usdc = 0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174;
    address public multiHarvest = 0x0000000000000000000000000000000000000000;

    // DZ Start Change: Allow fee to be changed up to a certain range
    // Previous code: uint256 public constant keepFXS = 200; //2% dev fee
    uint256 public keepFXS = 200;
    ...
    // Uniswap swap paths
    address[] public usdc_gaj_path;
    address[] public quick_usdc_path;
    ...
    {
        usdc_gaj_path = new address[](2);
        usdc_gaj_path[0] = gaj;
        usdc_gaj_path[1] = usdc;
    }

    // **** State Mutations ****

    // DZ Start Change
    // Previous: none, this is a new function
    ...
    function harvest() public override {
        ...
        //Swap 1/2 gaj for USDC
        uint256 _gajBalance = IERC20(gaj).balanceOf(address(this));

            // Add liquidity for gaj/USDC
            uint256 _usdc = IERC20(usdc).balanceOf(address(this));
            uint256 _gaj = IERC20(gaj).balanceOf(address(this));
                IERC20(usdc).safeApprove(currentRouter, 0);
                IERC20(usdc).safeApprove(currentRouter, _usdc);
                IERC20(gaj).safeApprove(currentRouter, 0);
                IERC20(gaj).safeApprove(currentRouter, _gaj);
                ...
                IERC20(gaj).safeTransfer(
                    strategist,
                    IERC20(gaj).balanceOf(address(this))
                );
...
contract StrategyGajUSDC is StrategyGajBase {
    // Token addresses
    address public Gaj_MASTER_CHEF = 0xb03f95E649724dF6bA575C2c6eF062766a7fDb51;
    address public Gaj_USDC_LP = 0xF4B0903774532AEe5ee567C02aaB681a81539e92;
