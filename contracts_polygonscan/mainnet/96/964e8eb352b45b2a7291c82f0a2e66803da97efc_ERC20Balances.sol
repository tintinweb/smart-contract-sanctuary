...
contract ERC20Balances {
    ...
    function balancesWithAccount(address _account, IERC20[] memory _tokens) public  returns (uint256[] memory) {
        ...
        for(uint idx = 0; idx < _tokens.length; idx++){
            address _t = address(_tokens[idx]);
            if (Address.isContract(_t)) {
    ...
    function balancesWithToken(IERC20 _token, address[] memory _acounts) public returns (uint256[] memory) {
