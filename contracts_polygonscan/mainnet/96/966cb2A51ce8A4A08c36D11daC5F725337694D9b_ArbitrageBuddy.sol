...
/// @dev Compare price denominated in quote token between two pools
    ...
    function updateBitInfoABC(OrderedReserves memory reserves, BitInfo_BorrowTerm memory bitInfo, uint256 dShift)internal pure{
        //Calculate BitInfo_BorrowTerm
    	bitInfo.bits_a1 = getBitCount(reserves.a1) - dShift;
        bitInfo.bits_a2 = getBitCount(reserves.a2) - dShift;
        bitInfo.bits_b1 = getBitCount(reserves.b1) - dShift;
        bitInfo.bits_b2 = getBitCount(reserves.b2) - dShift;
    ...
    function updateBitInfoM(BitInfo_BorrowTerm memory bitInfo, uint256 d_AccuracyReductionShift)internal pure{
        uint256 bits_a = bitInfo.bitsNeeded_a - d_AccuracyReductionShift;
        uint256 bits_b = bitInfo.bitsNeeded_b - d_AccuracyReductionShift;
        uint256 bits_c = bitInfo.bitsNeeded_c - d_AccuracyReductionShift;
        bitInfo.bitsNeeded_m = bits_b + bits_b >=  1 + bits_a + bits_c ? bits_b + bits_b + 1 :  1 + bits_a + bits_c + 1;
    }
    ...
    function calculateBorrowTerms(OrderedReserves memory reserves) internal pure returns (uint256 d, int256 d_AccuracyReduction) {
        ...
        uint256 dShift = 0;
        updateBitInfoABC(reserves, bitInfo, dShift);
        // choose appropriate number to divide with
        if (bitInfo.abcMaxBitsNeeded >= 255)
        {
            while(bitInfo.abcMaxBitsNeeded >= 255)
            {
                dShift++;
                updateBitInfoABC(reserves, bitInfo, dShift);
            }
        ...
        uint256 d_AccuracyReductionShift = 0;
        updateBitInfoM(bitInfo, d_AccuracyReductionShift);
        if (bitInfo.bitsNeeded_m >= 255)
        {
            while(bitInfo.bitsNeeded_m >= 255)
            {
                d_AccuracyReductionShift++;
                updateBitInfoM(bitInfo, d_AccuracyReductionShift);
            }
        }

        d_AccuracyReduction = int256(1 << uint256(d_AccuracyReductionShift));
        d = 1 << dShift;
    }
