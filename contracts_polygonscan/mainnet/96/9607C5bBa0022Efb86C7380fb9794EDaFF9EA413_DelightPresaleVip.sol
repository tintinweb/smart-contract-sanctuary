...
contract DelightPresaleVip is Ownable, ReentrancyGuard {

    // FEE address
    address public constant BURN_ADDRESS = 0x000000000000000000000000000000000000dEaD;
    address public constant FEE_ADDRESS = 0x6D9518bd44fce1ee2EF8d7E3090fBA60304A4ceA;
    address public immutable delightAddress;
    address public immutable sandManAddress;
    
    uint256 public constant swapDuration = 86399;
    uint256 public startBlock;
    uint256 public endBlock = startBlock + swapDuration;

    //SETUP
    uint256 public constant delightPresaleSize = (10 ** 5) * (10 ** 18);
    uint256 public delightSaleVipPrice  = 2 * (10 ** 35);
    uint256 public delightMaximumAvailable = (delightPresaleSize * delightSaleVipPrice) / 1e35;
    uint256 public delightRemaining = delightMaximumAvailable;
    ...
    event SaleVipPriceChanged(uint256 newArcadiumSaleINVPriceE35);

    constructor(uint256 _startBlock, address _delightAddress, address _sandManAddress) {
        require(_delightAddress != address(0), "E8");

        startBlock = _startBlock;
        endBlock = _startBlock + swapDuration;
    ...
    function swapSandManForDelight(uint256 sandManToSwap) external nonReentrant {
        ...
        require(sandManToSwap > 1e6, "E14");

        uint256 originalDelightAmount = (sandManToSwap * delightSaleVipPrice);

        uint256 delightPurchaseAmount = originalDelightAmount;
        ...
        require(IERC20(sandManAddress).transferFrom(msg.sender, address(this), sandManToSwap), "E18");
    ...
    function sendDepreciatedSandManToFeeAddress() external onlyOwner {
        require(block.number > endBlock, "E20");

        uint256 sandmanInContract = IERC20(sandManAddress).balanceOf(address(this));
        
        if (sandmanInContract > 0)
            IERC20(sandManAddress).transfer(FEE_ADDRESS, sandmanInContract);
    
        emit RetrieveDepreciatedSandManAddresss(FEE_ADDRESS, sandmanInContract);
    }
    ...
    function sendUnclaimedDelightToDeadAddress() external onlyOwner {
        ...
        if (delightInContract > 0)
            IERC20(delightAddress).transfer(BURN_ADDRESS, delightInContract);
    ...
    function setSaleVipPrice(uint256 _newDelightSaleVipPrice) external onlyOwner {
        require(block.number < startBlock, "E23");
        require(_newDelightSaleVipPrice > 0, "E24");
        
        delightSaleVipPrice = _newDelightSaleVipPrice;
        delightMaximumAvailable = (delightPresaleSize * delightSaleVipPrice) / 1e35;
        delightRemaining = delightMaximumAvailable;

        emit SaleVipPriceChanged(delightSaleVipPrice);
    }
