...
contract Tests is ERC721, ERC721Enumerable, Ownable {
    ...
    string public TestS_PROVENANCE = "";
    ...
    address public constant ownerAddress =
        0x10df6D8450c18b1Fb5Cb5abbEab74EfFbC7d17a9;
    ...
    function getTestsByOwner(address _owner)
        public
        view
    ...
    function mintTests(uint256 numTests) public payable {
        require(_isSaleActive, "Sale must be active to mint Tests");
        require(
            totalSupply().add(numTests) <= MAX_SUPPLY,
            "Sale would exceed max supply"
        );
        require(
            PRICE.mul(numTests) <= msg.value,
            "Not enough Matic sent (<12 MATIC per Test)"
        );
        _mintTests(numTests, msg.sender);
        emit TokenMinted(totalSupply());
    }

    function reserveTests(uint256 numTests) public onlyOwner {
        require(
            totalSupply().add(numTests) <= MAX_SUPPLY,
            "Sale would exceed max supply"
        );
    ...
    function airdropTest(uint256 numTests, address recipient) public {
        require(
            totalSupply().add(numTests) <= MAX_SUPPLY,
            "Sale would exceed max supply"
        );
        require(msg.sender == developerAddress || msg.sender == ownerAddress);
        _mintTests(numTests, recipient);
        emit TokenMinted(totalSupply());
    }

    function airdropTestToMany(address[] memory recipients) external {
        require(
    ...
    function _mintTests(uint256 numTests, address recipient) internal {
    ...
    function emergencySetPrice(uint256 price) public onlyOwner {
        PRICE = price;
    }
    ...
    function setProvenanceHash(string memory provenanceHash)
        ...
        onlyOwner
    {
        TestS_PROVENANCE = provenanceHash;
    }
