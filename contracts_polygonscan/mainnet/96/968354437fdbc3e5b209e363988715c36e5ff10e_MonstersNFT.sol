...
contract MonstersNFT is ERC721Enumerable, Ownable {
    ...
    function mint(uint256 _amount) public payable {
        ...
        require(msg.value >= mintingPrice.mul(_amount) ||
            (IERC20(poe).balanceOf(msg.sender) > 0 &&
            claimedFree[msg.sender] == false), "Not enough MATIC sent");
        ...
        if (claimedFree[msg.sender] == false && block.timestamp < mintingStartTimestamp && totalSupply() < 500) { //free claim for presale particiapnts
            claimedFree[msg.sender] = true;
            _safeMint(msg.sender, tsupply);
            return;
        }
        else if(block.timestamp < mintingStartTimestamp && numPurchased[msg.sender] == 0 && totalSupply() > 500){
            ...
            require(0 == 1, "Minting Parameters not met! User is not eligible to mint additional NFTs");
        }
