...
contract UserModule is Variables, Events {
    ...
    */
    function flashborrow(address[] memory tokens_, uint[] memory amts_, bytes calldata data_) nonReentrant public {
        require(tokens_.length == amts_.length, "not-equal-length");
        ...
        for (uint i = 0; i < length_; i++) {
            for (uint j = i + 1; j < length_; j++) {
                require(tokens_[i] != tokens_[j], "same-token-repeat");
            }
        ...
        for (uint i = 0; i < length_; i++) {
            tokenContracts[i].safeApprove(address(liquidity), amts_[i]);
            liquidity.payback(tokens_[i], amts_[i]);
            finalBals_[i] = tokenContracts[i].balanceOf(address(this));
            require(finalBals_[i] >= initialBals_[i].add(premiums_[i]), "fee-not-paid-fully");
            require(liquidity.protocolRawBorrow(address(this), tokens_[i]) == 0, "protocol-not-fully-paid");
        }
