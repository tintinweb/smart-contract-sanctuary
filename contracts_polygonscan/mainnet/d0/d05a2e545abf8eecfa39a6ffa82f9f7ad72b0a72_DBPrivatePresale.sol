...
contract DBPrivatePresale is ReentrancyGuard, Ownable {
    ...
    uint256 PrivatePresaleStartTime = 1626177600;
    ...
    function buy (address beneficiary) public payable nonReentrant {
        ...
        if(hasKeyToBuyPrivatePresale[_buyer] == false){
            require(_Matic >= 5 ether,"Wrong Matic Value,SHOULD send more than 5 Matic");
            tryAmount[_buyer] += 1;
          ...
          require(block.timestamp >= PrivatePresaleStartTime || isSaleActive, "NotStart");
   ...
   function randUint256(uint256 _length) public view returns(uint256) {
    ...
    function AddHasKeyAddress(address _address) public onlyOwner{
            PrivatePresaleeBuyer.push(_address);
    }

    function setTryAmountByAddress(address _address,uint256 _amount) public onlyOwner{
            tryAmount[_address] = _amount;
    }

    function SetAddressHasKey(address _address,bool _key) public onlyOwner{
           hasKeyToBuyPrivatePresale[_address] = _key;
    }

    function setSaleStartTime(uint256 _time) public onlyOwner{
        PrivatePresaleStartTime = _time;
    }
