...
contract Horsecoin is Ownable {
    ...
    IUniswapV2Router02 public uniswapV2Router =
        IUniswapV2Router02(0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff); // 路由合约地址  quickswap
    ...
    // 添加被排除的账户
    function excludeFee(address account) external virtual onlyOwner {
        require(!isExcluded[account], "Account is already excluded");
        isExcluded[account] = true;
    }

    // 从排除账户移除
    function includeFee(address account) external virtual onlyOwner {
        require(isExcluded[account], "Account is already excluded");
