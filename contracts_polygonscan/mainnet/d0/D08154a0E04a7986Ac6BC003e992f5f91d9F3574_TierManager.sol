...
contract TierManager is Ownable {
    ...
    function checkTier(address caller) external view returns(uint){
        ...
        uint tmpBal;
        for(uint i=0; i<supportedShareTokens.length; i++){
            tmpBal = IShare(supportedShareTokens[i]).getSharesGFICurrentWorth(caller);
            if(tmpBal > bigBal){
                bigBal = tmpBal;
            }
        }
    ...
    function viewAllGFIBalances(address caller) external view returns(uint[5] memory balances){
        balances[0] = GFI.balanceOf(caller);
        for(uint i=0; i<supportedShareTokens.length; i++){
            balances[i+1] = IShare(supportedShareTokens[i]).getSharesGFICurrentWorth(caller);
        }
    }
