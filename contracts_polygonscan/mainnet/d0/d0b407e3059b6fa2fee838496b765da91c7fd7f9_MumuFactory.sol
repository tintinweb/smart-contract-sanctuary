...
contract PancakeERC20 is IPancakeERC20 {
    ...
    string public constant name = 'MUMU LPs';
    string public constant symbol = 'Mumu-LP';
...
contract MumuFactory is IPancakeFactory {
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        require(tokenA != tokenB, 'Mumu: IDENTICAL_ADDRESSES');
        (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'Mumu: ZERO_ADDRESS');
        require(getPair[token0][token1] == address(0), 'Mumu: PAIR_EXISTS'); // single check is sufficient
    ...
    function setFeeTo(address _feeTo) external {
        require(msg.sender == feeToSetter, 'Mumu: FORBIDDEN');
        feeTo = _feeTo;
    }
