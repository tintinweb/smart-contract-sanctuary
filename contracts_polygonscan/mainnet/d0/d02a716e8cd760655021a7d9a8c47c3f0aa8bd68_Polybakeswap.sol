...
// File: contracts/libs/IBAKEReferral.sol


pragma solidity 0.6.12;

interface IBAKEReferral {
    /**
     * @dev Record referral.
     */
...
contract Polybakeswap is BEP20("Poly Bake Swap", "BAKE") {
    // Burn address
    ...
    /// @dev overrides transfer function to meet tokenomics of BAKE
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        if (recipient == BURN_ADDRESS) {
            ...
            require(amount == sendAmount + burnAmount, "BAKE::transfer: Burn value invalid");
...
// MasterChef is the master of BAKE. He can make BAKE and he is a fair guy.
//
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    // Initial emission rate: 1 BAKE per block.
    uint256 public constant INITIAL_EMISSION_RATE = 1 ether;
    // Minimum emission rate: 0.4 BAKE per block.
    ...
    // BAKE referral contract address.
    IBAKEReferral public BAKEReferral;
    ...
    constructor(
        ...
        BAKEPerBlock = INITIAL_EMISSION_RATE;
    }
    ...
    // Update the given pool's BAKE allocation point and deposit fee. Can only be called by the owner.
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 BAKEReward = multiplier.mul(BAKEPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        BAKE.mint(devAddress, BAKEReward.div(10));
        BAKE.mint(address(this), BAKEReward);
    ...
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        ...
        updatePool(_pid);
        if (_amount > 0 && address(BAKEReferral) != address(0) && _referrer != address(0) && _referrer != msg.sender) {
            BAKEReferral.recordReferral(msg.sender, _referrer);
        }
        if (user.amount > 0) {
        ...
        if (_amount > 0) {
            ...
            if (address(pool.lpToken) == address(BAKE)) {
                // burn 1% of token
    ...
    function safeBAKETransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > BAKEBal) {
            transferSuccess = BAKE.transfer(_to, BAKEBal);
        } else {
            transferSuccess = BAKE.transfer(_to, _amount);
        }
        require(transferSuccess, "safeBAKETransfer: Transfer failed");
    }
    ...
    function updateEmissionRate() public {
        ...
        require(BAKEPerBlock > MINIMUM_EMISSION_RATE, "updateEmissionRate: Emission rate has reached the minimum threshold");
        ...
        uint256 newEmissionRate = BAKEPerBlock;
        ...
        if (newEmissionRate >= BAKEPerBlock) {
            return;
        }

        massUpdatePools();
        lastReductionPeriodIndex = currentIndex;
        uint256 previousEmissionRate = BAKEPerBlock;
        BAKEPerBlock = newEmissionRate;
        emit EmissionRateUpdated(msg.sender, previousEmissionRate, newEmissionRate);
    }

    // Update the BAKE referral contract address by the owner
    function setBAKEReferral(IBAKEReferral _BAKEReferral) public onlyOwner {
        BAKEReferral = _BAKEReferral;
    }
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (address(BAKEReferral) != address(0) && referralCommissionRate > 0) {
            address referrer = BAKEReferral.getReferrer(_user);
            uint256 commissionAmount = _pending.mul(referralCommissionRate).div(10000);

            if (referrer != address(0) && commissionAmount > 0) {
                BAKE.mint(referrer, commissionAmount);
