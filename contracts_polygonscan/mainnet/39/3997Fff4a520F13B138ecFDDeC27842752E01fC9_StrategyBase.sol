...
abstract contract StrategyFarmBase is BaseStrategyMasterChef {
    ...
    function harvest() public override {
        ...
        if (_ballBalance > harvestCutoff){
            ...
            uint256 performanceFee = _ballBalance.mul(keepFXS).div(keepFXSmax);
            if (performanceFee>0) {
                _swapUniswapWithPath(ball_matic_path, performanceFee);
            ...
            // Swap half of remaining to USDC
            uint256 _ballBalancePost = IERC20(ball).balanceOf(address(this));
            _swapUniswapWithPath(ball_matic_path, _ballBalancePost.div(2));

            // Add liquidity for Ball/USDC
            uint256 _ball = IERC20(ball).balanceOf(address(this));
            uint256 _matic = IERC20(matic).balanceOf(address(this));
            if (_ball > 0 && _matic > 0) {
                IERC20(ball).safeApprove(currentRouter, 0);
                IERC20(ball).safeApprove(currentRouter, _ball);
                ...
                // Donates DUST
                IERC20(ball).safeTransfer(
                    strategist,
                    IERC20(ball).balanceOf(address(this))
                );
                IERC20(matic).safeTransfer(
                    strategist,
                    IERC20(matic).balanceOf(address(this))
...
contract StrategyBase is StrategyFarmBase {
    ...
    address public BALL_MATIC_LP = 0xe11eb426206960e9533828031cDFED0B4B61749C;
