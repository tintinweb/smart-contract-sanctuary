...
contract TransparentUpgradeableProxy is AdminUpgradeableProxy, IBeacon {
    ...
    modifier ensureSameContractIdentifier(address newImplementation) {
        require(StorageSlot.getBytes32Slot(_CONTRACT_IDENTIFIER_SLOT).value == IGrowUpgradeableImplementation(newImplementation).CONTRACT_IDENTIFIER(), "TransparentUpgradeableProxy: not same implementation");
        _;
    }
...
contract ProxyFactory is Ownable {
    ...
    function createBeaconProxy(address beacon, bytes memory data) external payable onlyOwner {
        address proxy = address(new BeaconProxy{value: msg.value}(beacon, data));
        emit BeaconProxyCreate(proxy, beacon);
    }
