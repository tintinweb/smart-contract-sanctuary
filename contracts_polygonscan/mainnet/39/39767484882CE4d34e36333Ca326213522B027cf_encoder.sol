...
contract encoder {
   function getEncodedDiamondAddCaller(address _newCaller) external view returns(bytes memory){
       return (abi.encodeWithSelector(CallFacet.addCaller.selector, _newCaller));
   }
}