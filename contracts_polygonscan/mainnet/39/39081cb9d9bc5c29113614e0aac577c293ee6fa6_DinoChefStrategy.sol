...
contract DinoChefStrategy is BaseStrategyMasterChef {
    ...
    function swapRewardToWmaticAndDistributeFee(uint256 feeAmount) internal {
        if(feeAmount > 0) {
            _swapUniswapWithPath(reward_matic_path, feeAmount, SUSHI_ROUTER);
    ...
    function harvest() public override onlyHumanOrWhitelisted nonReentrant {
        ...
        if(harvestedToken != tokenA) {
            ...
            if (remainingHarvested > 0) {
                _swapUniswapWithPath(reward_a_path, remainingHarvested, QUICKSWAP_ROUTER);
            }
        }
