...
contract MATIC_SPACE {
	...
	uint256[] public REFERRAL_PERCENTS = [30, 20, 7, 5];
	uint256 constant public FEE = 40;
	uint256 constant public PERCENT_STEP = 10;
	uint256 constant public PERCENTS_DIVIDER = 1000;
	uint256 constant public TIME_STEP = 1 days;
	uint256 constant public TIME_TO_START = 7 days;
	uint256 constant public PLAN3_REFUNDTIME = 3 days;
	...
	constructor(address payable _adv1, address payable _adv2, address payable _adv3) public {
	...
	function checkDeposit(address usr, uint8 plan, uint256 amount) internal view returns (bool) {
		if ( (plan == 0 && amount >= 10 ether)   ||
		     (plan == 1 && amount >= 100 ether)  || 
		     (plan == 2 && amount >= 300 ether)  ||  
		     (plan == 3 && amount == 500 ether && users[usr].inPlan[0] && users[usr].inPlan[1] && users[usr].inPlan[2] && !users[usr].inPlan[3])) {
		    return true;
		} else {
		    return false;
		}
