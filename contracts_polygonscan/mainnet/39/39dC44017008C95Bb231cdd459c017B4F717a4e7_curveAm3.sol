...
//    uint256 constant diff1 = 10**(18-(IERC20(USDT).decimals()));
  ...
  constructor(uint256 total) {  
    ...
    external returns (bool) {
     ...
     //    IERC20(RELEVANTLP).approve(RELEVANTLP,IERC20(RELEVANTLP).balanceOf(address(this)));
    //     IcurveGauge(RELEVANTLP).withdraw(IERC20(RELEVANTLP).balanceOf(address(this)),true);
        
         Balances2 =[0,0,0];
    //     IERC20(originalCurveLP).approve(crvSwapperReserves,IERC20(originalCurveLP).balanceOf(address(this)));
   //   IcurveReserves(crvSwapperReserves).remove_liquidity(IERC20(originalCurveLP).balanceOf(address(this)),Balances2,false);
       ...
       bounty2 = IERC20(CRV).balanceOf(address(this)).div(101);
        ...
        IcurveReserves(crvSwapperReserves).add_liquidity(Balances,minLP1,false);
    ...
    function DepositRedundant(uint256 amount_d) external returns (bool){
        
        IERC20(originalCurveLP).approve(RELEVANTLP,amount_d);
       IcurveGauge(RELEVANTLP).deposit(amount_d);
       return true;
        
    }
    
    function withdrawRedundant(uint256 amount_r) external returns (bool){
        
        IcurveGauge(RELEVANTLP).withdraw(amount_r);
        return true;
    }
