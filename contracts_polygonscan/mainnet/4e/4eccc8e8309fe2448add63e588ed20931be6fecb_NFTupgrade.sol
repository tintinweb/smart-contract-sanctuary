...
contract NFTupgrade is Initializable, ERC721EnumerableUpgradeable, OwnableUpgradeable {
  using StringsUpgradeable for uint256;

  string baseURI;
  string public baseExtension;
  uint256 public cost;
  uint256 public secondcost;


 function initialize() initializer public {
      __ERC721_init("NFT Memes", "NFT-MEMES");
      __Ownable_init();
      baseExtension = ".json";
      baseURI = "https://nft-memes.io/nfts/";
      cost = 15 ether;
      secondcost = 0.0 ether;
 }
  ...
  // public
  function secondmint(uint256 _mintAmount) public payable {
    uint256 supply = totalSupply();
    require(_mintAmount > 0);

    if (msg.sender != owner()) {
      require(msg.value >= secondcost * _mintAmount);
    }
