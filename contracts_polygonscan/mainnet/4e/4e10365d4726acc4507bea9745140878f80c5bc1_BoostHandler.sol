...
contract BoostHandler is Ownable, IBoostHandler {
    ...
    address public calculator = 0xEEcE062608EEAAb52ae2Ed1938758856bCb6AC0D;
    ...
    function getVeAddyFromLockedStaking(address _user) public override view returns (uint256 veAddy) {
        ...
        //Ratio is currently multiplied by 2 since 1/2 of tokens are still vesting
        ...
        if(endingTimestampPlus2 > now) {
            ...
            uint256 ratio = ILockedAddyVault(lockedStakingPlus_lpVault).getRatio().mul(2);
            veAddy = veAddy.add(userAddyLocked.mul(diff).div(MAX_DURATION).mul(ratio).div(10 ** 18));
        }
    }
