...
// MasterChef is the master of Quokka. He can make Quokka and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once QUOKKA is sufficiently
...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of QUOKKAs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accQuokkaPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accQuokkaPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. QUOKKAs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that QUOKKAs distribution occurs.
        uint256 accQuokkaPerShare;   // Accumulated QUOKKAs per share, times 1e12. See below.
    ...
    // QUOKKA tokens created per block.
    uint256 public quokkaPerBlock;
    // Bonus muliplier for early quokka makers.
    ...
    // Locker interface
    ILocker quokkaLocker;
    // Locker adresse
    address public quokkaLockerAddress;
    ...
    // The block number when QUOKKA mining starts.
    uint256 public startBlock;
    // Total locked up rewards
    uint256 public totalLockedUpRewards;

    // Quokka referral contract address.
    IReferral public quokkaReferral;
    ...
    event QuokkaReferralUpdated(address indexed user, IReferral newAddress);
    event QuokkaLockerUpdated(address indexed user, ILocker newAddress);
    ...
    constructor(
        QuokkaToken _quokka,
        uint256 _startBlock,
        uint256 _quokkaPerBlock,
        address _quokkaLockerAddress
    ) public {
        quokka = _quokka;
        startBlock = _startBlock;
        quokkaPerBlock = _quokkaPerBlock;
        ...
        quokkaLockerAddress = _quokkaLockerAddress;
        quokkaLocker = ILocker(_quokkaLockerAddress);
    }
    ...
    // Update the given pool's QUOKKA allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending QUOKKAs on frontend.
    function pendingQuokka(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accQuokkaPerShare = pool.accQuokkaPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 quokkaReward = multiplier.mul(quokkaPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accQuokkaPerShare = accQuokkaPerShare.add(quokkaReward.mul(1e12).div(lpSupply));
        }
        uint256 pending = user.amount.mul(accQuokkaPerShare).div(1e12).sub(user.rewardDebt);
        return pending.add(user.rewardLockedUp);
    }

    // View function to see if user can harvest QUOKKAs.
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 quokkaReward = multiplier.mul(quokkaPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        quokka.mint(devAddress, quokkaReward.mul(100).div(1000));
        // Automatically burn 2% of minted tokens
        quokka.mint(BURN_ADDRESS, quokkaReward.mul(20).div(1000));
        // Automatically mint some QUOKKA for the lottery pot
        if (address(lotteryAddress) != address(0) && lotteryMintRate > 0) {
            quokka.mint(lotteryAddress, quokkaReward.mul(lotteryMintRate).div(10000));
        }        
        quokka.mint(address(this), quokkaReward);
        pool.accQuokkaPerShare = pool.accQuokkaPerShare.add(quokkaReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for QUOKKA allocation.
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        if (_amount > 0 && address(quokkaReferral) != address(0) && address(quokkaReferral) != BURN_ADDRESS && _referrer != address(0) && _referrer != BURN_ADDRESS && _referrer != msg.sender) {
            quokkaReferral.recordReferral(msg.sender, _referrer);
        }
        payOrLockupPendingQuokka(_pid,false);
        if (_amount > 0) {
            pool.lpToken.safeTransferFrom(address(msg.sender), address(this), _amount);
            if (address(pool.lpToken) == address(quokka)) {
                uint256 burnTax = _amount.mul(quokka.burnRateTax()).div(10000);
        ...
        user.rewardDebt = user.amount.mul(pool.accQuokkaPerShare).div(1e12);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        updatePool(_pid);
        payOrLockupPendingQuokka(_pid,true);
        if (_amount > 0) {
    ...
    // Pay or lockup pending QUOKKAs.
    function payOrLockupPendingQuokka(uint256 _pid, bool _isWithdrawal) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        uint256 pending = user.amount.mul(pool.accQuokkaPerShare).div(1e12).sub(user.rewardDebt);
        ...
        if (_isWithdrawal) {
            ...
            if (noWithdrawFee(_pid, msg.sender)==false) {
                ...
                safeQuokkaTransfer(BURN_ADDRESS, withdrawalfeeamount);     
            }
        ...
        if (canHarvest(_pid, msg.sender)) {
            if (pending > 0 || user.rewardLockedUp > 0) {
                ...
                if (address(quokkaLocker) != address(0)){
                    uint256 startReleaseBlock = ILocker(quokkaLocker).getStartReleaseBlock();
                    if (lockerRate > 0 && block.number < startReleaseBlock) {
                        ...
                        IBEP20(quokka).safeIncreaseAllowance(address(quokkaLockerAddress), _lockerAmount);
                        ILocker(quokkaLocker).lock(msg.sender, _lockerAmount); 
                    }
                }
                    

                // send rewards 
                safeQuokkaTransfer(msg.sender, totalRewards);
    ...
    // Safe quokka transfer function, just in case if rounding error causes pool to not have enough QUOKKAs.
    function safeQuokkaTransfer(address _to, uint256 _amount) internal {
        uint256 quokkaBal = quokka.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > quokkaBal) {
            transferSuccess = quokka.transfer(_to, quokkaBal);
        } else {
            transferSuccess = quokka.transfer(_to, _amount);
        }
        require(transferSuccess, "safeQuokkaTransfer: transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _quokkaPerBlock) public onlyOwner {
        massUpdatePools();
        emit EmissionRateUpdated(msg.sender, quokkaPerBlock, _quokkaPerBlock);
        quokkaPerBlock = _quokkaPerBlock;
    }

    // Update the quokka referral contract address by the owner
    function setQuokkaReferral(IReferral _quokkaReferral) public onlyOwner {
        quokkaReferral = _quokkaReferral;
        emit QuokkaReferralUpdated(msg.sender, _quokkaReferral);
    }
    ...
    // Update the quokka locker contract address by the owner
    function setQuokkaLocker(ILocker _quokkaLocker) public onlyOwner {
        quokkaLocker = _quokkaLocker;
        emit QuokkaLockerUpdated(msg.sender, _quokkaLocker);
    }   
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (address(quokkaReferral) != address(0) && referralCommissionRate > 0) {
            address referrer = quokkaReferral.getReferrer(_user);
            uint256 commissionAmount = _pending.mul(referralCommissionRate).div(10000);

            if (referrer != address(0) && referrer != BURN_ADDRESS && commissionAmount > 0) {
                quokka.mint(referrer, commissionAmount);
                quokkaReferral.recordReferralCommission(referrer, commissionAmount);
...
// QuokkaToken with Governance.
contract QuokkaToken is BEP20 {
    ...
    event QuokkaSwapRouterUpdated(address indexed operator, address indexed router, address indexed pair);
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "QUOKKA::antiWhale: Transfer amount exceeds the maxTransferAmount");
            }
        }
        _;
    }
    ...
    /**
     * @notice Constructs the QuokkaToken contract.
     */
    constructor() public BEP20("Quokka Token", "QUOKKA") {
    ...
    /// @dev overrides transfer BEP20 function to meet tokenomics of QUOKKA
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        ...
        if (recipient == BURN_ADDRESS || _excludedFromTrsfTax[sender] == true || _excludedFromTrsfTax[recipient] == true) {            
            ...
            require(amount == sendAmount + burnAmount, "QUOKKA::transfer: Burn value invalid");
    ...
    function updateBurnRate(uint16 _burnRateTax) public onlyOperator {
        require(_burnRateTax <= 200, "QUOKKA::updateBurnRate: Burn rate must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 10000, "QUOKKA::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "QUOKKA::transferOperator: new operator is the zero address");
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "QUOKKA::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "QUOKKA::delegateBySig: invalid nonce");
        require(now <= expiry, "QUOKKA::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "QUOKKA::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying QUOKKAs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "QUOKKA::_writeCheckpoint: block number exceeds 32 bits");
    ...
    //a way to get back other BEP20 tokens sended by error into the QUOKKA token contract
