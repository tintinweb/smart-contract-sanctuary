...
contract ChargedBunkersFactory {
    ...
    // transfer "_amount" of MATIC to selected bunkers by _ids[] collectively.
    function sendFeesToBunker(uint256[] memory _ids, uint256 _amount) external returns (bool) {
        require(msg.sender == _owner, "Only Owner can call this function");
        for (uint i=0 ; i< _ids.length ; i++) {
            ChargedBunker bunker = ChargedBunker(IdToBunker[_ids[i]]);
            payable(address(bunker)).transfer(_amount);
        }
        return true ;
    }

    // transfer all the balance of selected bunkers back to factory by _ids[] collectively.
    function feesBackFromBunker (uint256[] memory _ids) external returns (bool) {
        require(msg.sender == _owner, "Only Owner can call this function");
        for (uint i=0 ; i< _ids.length ; i++) {
            ChargedBunker bunker = ChargedBunker(IdToBunker[_ids[i]]);
            bunker.feesBack();
...
// --------- quickswap manipulative function ----------
    ...
    function _addLiquidity(HLSConfig memory self, Position memory _position) private returns (Position memory) {
        ...
        uint256 max_available_staking_a = (IERC20(_position.token_a).balanceOf(address(this))).mul(_position.funds_percentage).div(100);
        uint256 max_available_staking_b = (IERC20(_position.token_b).balanceOf(address(this))).mul(_position.funds_percentage).div(100);
