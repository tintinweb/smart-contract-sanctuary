...
contract RollingRain is VRFConsumerBase, Ownable {
  ...
  uint256 public balancePercentage = 500;
  uint256 public handicap = 0;
  ...
  uint256 public lastRollCounted = 0;
  uint256[] public alwaysPicked;
  event Round(uint256 waiting, uint256 wins, uint256 losses, uint256 currentBalance);
  event Profit(uint256 amount, uint256 wins, uint256 losses, uint256 currentBalance);
  event Bet(uint256 wag, uint256 betMask, uint256 modulo, uint256 numbers, uint256 wins, uint256 losses, uint256 currentBalance);
  
  constructor() VRFConsumerBase(VRF_COORDINATOR, LINK_TOKEN) {
      alwaysPicked.push(27);
      alwaysPicked.push(13);
      alwaysPicked.push(6);
      alwaysPicked.push(26);
  }
  ...
  function addAlwaysPicked(uint256 n) external onlyOwner {
      alwaysPicked.push(n);
  }

  function setHandicap(uint256 n) external onlyOwner {
      handicap = n;
  }

  function removeAlwaysPicked(uint256 n) external onlyOwner returns(bool){
      bool found = false;
      if(alwaysPicked.length == 0){
          return false;
      }
      if(alwaysPicked.length == 1){
          if(alwaysPicked[0] == n){
              alwaysPicked.pop();
              return true;
          }
      ...
      for(uint256 i = 0; i < alwaysPicked.length-1; i++){
          if(alwaysPicked[i] == n){
              found = true;
          }
          if(found){
              alwaysPicked[i] = alwaysPicked[i+1];
          }
      }
      if(found){
          alwaysPicked.pop();
  ...
  function takeProfit() internal returns(uint256){
    if(address(this).balance > startBalance.add(minProfit)){
      ...
      emit Profit(profit, wins, losses, address(this).balance);
  ...
  function fulfillRandomness(bytes32 requestId, uint256 randomValue) internal override {
      ...
      rI = rI + 1;
      (uint256 picked, uint256 numbers) = pick(randomValue);
      roll(picked,
           numbers);
  }
  ...
  function pick(uint256 randomValue) public view returns(uint256, uint256){
      ...
      if(losses + handicap >= wins){
          maxPicksModulo = 23;
          addonPicks = (losses + handicap - wins) > 13 ? 13 : (losses + handicap - wins);
      }
      
      uint256 pickMax = uint256(keccak256(abi.encode(randomValue, rI * 2))) % maxPicksModulo + addonPicks;
      
      bool[] memory picks = new bool[](37);
      uint256 i = 0;
      while(i < pickMax){
          picks[36-uint256(keccak256(abi.encode(randomValue, rI + i + 1))) % 37] = true;
          i = i + 1;
      }
      for(i = 0; i < alwaysPicked.length; i++){
          picks[36-alwaysPicked[i]] = true;
      }
      for(i = 0; i < 37; i++){
          number = number << 1;
          if(picks[i]){
  ...
  function roll(uint256 betMask, uint256 numbers) internal {
    ...
    if(wasPendingBet > waitForRounds || pendingBet == 0){
        if(rolls.length > lastRollCounted){
        ...
        if(address(this).balance >= betAmount){
            ...
            if(alwaysBetMask == 0){
                ...
                emit Bet(betAmount, betMask, useModulo, numbers, wins, losses, address(this).balance);
            }else{
                polyroll.placeBet{ value: betAmount }(alwaysBetMask, useModulo, owner());
                emit Bet(betAmount, alwaysBetMask, useModulo, numbers, wins, losses, address(this).balance);
            }
        ...
        if(tookProfit == 0){
            emit Round(wasPendingBet, wins, losses, address(this).balance);
        }
    }
