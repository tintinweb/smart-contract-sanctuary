...
abstract contract BaseStrategy is Ownable {
    ...
    // Dex
    address public constant currentRouter = 0xC0788A3aD43d79aa53B09c2EaCc313A787d1d607; //ApeSwap router
...
abstract contract BaseStrategyMasterChef is BaseStrategy {
    ...
    function getHarvestable() external override view returns (uint256) {
        return IMasterChef(rewards).pendingBanana(poolId, address(this));
    }
    ...
    function deposit() public override {
        ...
        if (_want > 0) {
            ...
            IMasterChef(rewards).deposit(poolId, _want, address(this));
        }
    }
    ...
    function _withdrawSome(uint256 _amount)
        ...
        returns (uint256)
    {
        IMasterChef(rewards).withdraw(poolId, _amount, address(this));
        return _amount;
    }
...
abstract contract StrategyFarmTwoAssets is BaseStrategyMasterChef {
    ...
    address public constant banana = 0x5d47bAbA0d66083C52009271faF3F50DCc01023C;
    ...
    uint256 public harvestCutoffMatic = 10**12;
    uint256 public constant keepFXSmax = 10000;

    // Uniswap swap paths
    address[] public banana_matic_path;
    address[] public matic_eth_path;
    ...
    {
        banana_matic_path = new address[](2);
        banana_matic_path[0] = banana;
        banana_matic_path[1] = matic;

        matic_eth_path = new address[](2);
        matic_eth_path[0] = matic;
        matic_eth_path[1] = eth;
    }
    ...
    function set_harvestCutoffMatic(uint256 newCutoff) external onlyOwner() {
        harvestCutoffMatic = newCutoff;
    }
    ...
    function harvest() public override {
        ...
        //Swap Banana for matic
        uint256 _bananaBalance = IERC20(banana).balanceOf(address(this));
        if (_bananaBalance > harvestCutoff){
            _swapUniswapWithPath(banana_matic_path, _bananaBalance);
        }

        // Collect fee + add LP
        uint256 _maticBalance = IERC20(matic).balanceOf(address(this));
        if (_maticBalance > harvestCutoffMatic){
            
            // Collect fee
            uint256 performanceFee = _maticBalance.mul(keepFXS).div(keepFXSmax);
            if (performanceFee>0) {
            ...
            // Swap Matic for LP
            uint256 _maticBalancePost = IERC20(matic).balanceOf(address(this));
            _swapUniswapWithPath(matic_eth_path, _maticBalancePost.div(2));

            // Add liquidity for Banana/Matic
...
contract StrategyTwoAssets is StrategyFarmTwoAssets {
    // Token addresses
    address public APE_MASTER_CHEF = 0x54aff400858Dcac39797a81894D9920f16972D1D;
    address public Matic_Eth_LP = 0x6Cf8654e85AB489cA7e70189046D507ebA233613;
