...
// File interfaces/IMonsterReferral.sol
...
pragma solidity 0.6.12;
// MasterChef is the master of Monster. He can make Monster and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once MONSTER is sufficiently
// distributed and the community can show to govern itself.
//
// Have fun reading it. Hopefully it's bug-free. God bless.
contract MasterChefTest is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of MONSTER
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;         // How many allocation points assigned to this pool. MONSTER to distribute per block.
        uint256 lastRewardBlock;    // Last block number that MONSTER distribution occurs.
        uint256 accMonsterPerShare; // Accumulated MONSTER per share, times 1e12. See below.
    ...
    uint256 public constant MAX_MONSTER_PER_BLOCK = 2000 * 10 ** 18;
    // Bonus multiplier for early monster makers.
    ...
    // Total MONSTER in MONSTER Pools (can be multiple pools)
    uint256 public totalMonsterInPools = 0;
    ...
    //actual Monster lef in MasterChef can be used in rewards, must excluding all in monster pools
    //this function is for safety check only not used anywhere
    function remainRewards() external view returns (uint256) {
        return monster.balanceOf(address(this)).sub(totalMonsterInPools);
    }
    ...
    // Update the given pool's MONSTER allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending MONSTER on frontend.
    function pendingMonster(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 pending = user.amount.mul(accMonsterPerShare).div(1e12).sub(user.rewardDebt);
        return pending.add(user.rewardLockedUp);
    }

    // View function to see if user can harvest MONSTER.
    ...
    function massUpdatePools() public {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            ...
            pool.accMonsterPerShare = pool.accMonsterPerShare.add(monsterReward.mul(1e12).div(pool.totalLp));
            pool.lastRewardBlock = block.number;

            totalReward.add(monsterReward.div(10));
        }

        safeMonsterTransfer(devAddress, totalReward);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        safeMonsterTransfer(devAddress, monsterReward.div(10));
    }

    // Deposit LP tokens to MasterChef for MONSTER allocation.
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        ...
        payOrLockupPendingMonster(_pid);
        if (_amount > 0) {
            ...
            if (pool.depositFeeBP > 0) {
                ...
                if (address(pool.lpToken) == address(monster)) {
                    totalMonsterInPools = totalMonsterInPools.add(_amount).sub(depositFee);
                }
            } else {
                ...
                if (address(pool.lpToken) == address(monster)) {
                    totalMonsterInPools = totalMonsterInPools.add(_amount);
                }
            }
        }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (_amount > 0) {
            ...
            if (address(pool.lpToken) == address(monster)) {
                totalMonsterInPools = totalMonsterInPools.sub(_amount);
            }
    ...
    function emergencyWithdraw(uint256 _pid) public nonReentrant {
        ...
        if (address(pool.lpToken) == address(monster)) {
            totalMonsterInPools = totalMonsterInPools.sub(amount);
        }
    ...
    // Pay or lockup pending MONSTER.
    function payOrLockupPendingMonster(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        if (canHarvest(_pid, msg.sender)) {
            if (pending > 0 || user.rewardLockedUp > 0) {
                ...
                // send rewards
                safeMonsterTransfer(msg.sender, totalRewards);
    ...
    // Safe monster transfer function, just in case if rounding error causes pool do not have enough MONSTER.
    function safeMonsterTransfer(address _to, uint256 _amount) internal {
        if (monster.balanceOf(address(this)) > totalMonsterInPools) {
            //monsterBal = total monster in MasterChef - total monster in monster pools, this will make sure that MasterChef never transfer rewards from deposited monster pools
            uint256 monsterBal = monster.balanceOf(address(this)).sub(totalMonsterInPools);
            if (_amount >= monsterBal) {
                monster.transfer(_to, monsterBal);
            } else if (_amount > 0) {
                monster.transfer(_to, _amount);
            }
        }
    }
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (address(monsterReferral) != address(0) && referralCommissionRate > 0) {
            ...
            if (referrer != address(0) && commissionAmount > 0) {
                safeMonsterTransfer(referrer, commissionAmount);
                monsterReferral.recordReferralCommission(referrer, commissionAmount);
    ...
    function updateEmissionRate(uint256 _monsterPerBlock) public onlyOperator {
        require(_monsterPerBlock <= MAX_MONSTER_PER_BLOCK, "MONSTER per block too high");
        massUpdatePools();

        emit EmissionRateUpdated(msg.sender, monsterPerBlock, _monsterPerBlock);
        monsterPerBlock = _monsterPerBlock;
    }
