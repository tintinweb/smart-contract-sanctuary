...
contract Golden is Context, IERC20, Ownable {
  ...
  uint256 public duringTime = 10; //20min
  ...
  constructor(
    ...
    _symbol = "GoldYork";
    _decimals = 18;
    _maxRestrictionAmount = 3 * 10 ** 18;
    _totalSupply = 1000 * 10 ** 18;
    _restrictionGas = 10;
    _startTime = startTime;
    _restrictionLiftTime = startTime.add(duringTime * 60);
  ...
  */
  ...
  function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
    _transfer(sender, recipient, amount);
    _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "GYORK: transfer amount exceeds allowance"));
    return true;
  }
  ...
  function decreaseAllowance(address spender, uint256 subtractedValue) public returns (bool) {
    _approve(_msgSender(), spender, _allowances[_msgSender()][spender].sub(subtractedValue, "GYORK: decreased allowance below zero"));
    return true;
  }
   ...
   * - `sender` must have a balance of at least `amount`.
   */
  function _transfer(address sender, address recipient, uint256 amount) internal launchRestrict(sender, recipient, amount){
    require(sender != address(0), "GYORK: transfer from the zero address");
    require(recipient != address(0), "GYORK: transfer to the zero address");

    _balances[sender] = _balances[sender].sub(amount, "GYORK: transfer amount exceeds balance");
    _balances[recipient] = _balances[recipient].add(amount);
    emit Transfer(sender, recipient, amount);
  }
  ...
  function _mint(address account, uint256 amount) internal {
    require(account != address(0), "GYORK: mint to the zero address");
  ...
  function _burn(address account, uint256 amount) internal {
    require(account != address(0), "GYORK: burn from the zero address");

    _balances[account] = _balances[account].sub(amount, "GYORK: burn amount exceeds balance");
    _totalSupply = _totalSupply.sub(amount);
    emit Transfer(account, address(0), amount);
  }
  ...
  function _approve(address owner, address spender, uint256 amount) internal {
    require(owner != address(0), "GYORK: approve from the zero address");
    require(spender != address(0), "GYORK: approve to the zero address");
  ...
  function _burnFrom(address account, uint256 amount) internal {
    ...
    _approve(account, _msgSender(), _allowances[account][_msgSender()].sub(amount, "GYORK: burn amount exceeds allowance"));
  }
  
  function updateStartTime(uint256 time) public onlyOwner {
    _startTime = time;
    _restrictionLiftTime = time.add(duringTime * 60);
  }
  ...
  function tBlacklistAccount(address account) external onlyOwner() {
    _isBlacklisted[account] = true;
  }
  
  function fBlacklistAccount(address account) external onlyOwner() {
  ...
  modifier launchRestrict(address sender, address recipient, uint256 amount) {
    if(_startTime > now) {
        require(_owner == sender, "GYORK: caller is not the owner");
    } else if (_startTime <= now && _restrictionLiftTime > now) {
        // require(tx.gasprice <= _restrictionGas,"GYORK: gas price above limit");
        require(amount <= _maxRestrictionAmount, "GYORK: amount greater than max limit");
    } else {
        require(!_isBlacklisted[sender], "GYORK: this tx is restrict");
    }
    _;
  }
}
