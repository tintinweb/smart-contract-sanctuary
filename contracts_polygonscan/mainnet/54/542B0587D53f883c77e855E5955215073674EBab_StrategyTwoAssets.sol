...
abstract contract StrategyFarmTwoAssets is BaseStrategyMasterChef {
    ...
    address public constant sx = 0x840195888Db4D6A99ED9F73FcD3B225Bb3cB1A79;
    ...
    function harvest() public override {
        ...
        if (_dinoBalance > harvestCutoff){
            ...
            _swapUniswapWithPath(eth_sx_path, _ethBalancePost.div(2));

            // Add liquidity
            uint256 _eth = IERC20(eth).balanceOf(address(this));
            uint256 _sx = IERC20(sx).balanceOf(address(this));
            if (_eth > 0 && _sx > 0) {
                IERC20(eth).safeApprove(currentRouter, 0);
                IERC20(eth).safeApprove(currentRouter, _eth);
                IERC20(sx).safeApprove(currentRouter, 0);
                IERC20(sx).safeApprove(currentRouter, _sx);
                ...
                IERC20(sx).safeTransfer(
                    strategist,
                    IERC20(sx).balanceOf(address(this))
                );
            }
        }
        // Stake the LP tokens
...
contract StrategyTwoAssets is StrategyFarmTwoAssets {
    ...
    address public Eth_SX_LP = 0x1bF9805B40a5f69c7d0f9E5d1Ab718642203c652;
