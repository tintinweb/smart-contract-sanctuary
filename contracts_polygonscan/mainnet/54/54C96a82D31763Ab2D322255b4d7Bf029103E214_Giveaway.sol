...
contract Giveaway is Context, Democratic, ReentrancyGuard {
    ...
    /// @return the address of the token won 
    address public tokenAdd;

    /// @return the address of the community IDO
    address public cidoAdd;

    /// @param totalWon amount won without decimals 
    /// @param TokenClaimed boolean that indicates if the claim has been realized
    struct TokenGivenAway {
        uint256 totalWon;
        bool TokenClaimed;
    }

    /// @dev event is emited each time a claim is created
    /// @param winner address of a winner
    /// @param amount amount in decimals of the winning token
    event Claim (address winner, uint256 amount);

    /// @custom:mapping used to map the user with the amount of token won
    mapping(address => TokenGivenAway) public giveaways;

    /// @dev this function is used to set the addresses of the contracts used
    /// @param tokenAddress address of the token for the giveaway
    /// @param cidoAddress address of the community IDO
    ...
    /// @dev this function is used to set all the winners of our giveaways
    /// @param winners list of winning addresses
    /// @param amountsToGive list of associated winning amount
    /// @notice make sure that the 2 list are perfectly sorted.
    /// @notice This function is restricted to democratic process
    ...
    /// @dev this function is called by users to retreive there winning tokens
    /// @notice we use nonReentrant() modifier from open-zeppelin to protect this function
    /// @notice this function can only be called is liquidity pool is deployed from the community IDO
...
/// @author Nova Labs Scientists
/// @notice  Provides the interface verify some information from the Community IDO
