...
contract StrategyAddTwoSidesOptimalV2 is Ownable, ReentrancyGuard, Strategy {
  ...
  function setVault(address _fToken, address _vault) external onlyOwner{
  ...
  function execute(
    ...
    uint banktokenbalance = banktoken.myBalance();
    uint swapAmt;
    bool isReversed;
    {
      (uint r0, uint r1, ) = lpToken.getReserves();
      (uint banktokenreserve, uint fReserve) = lpToken.token0() == banktoken ? (r0, r1) : (r1, r0);
      (swapAmt, isReversed) = optimalDeposit(banktokenbalance, fToken.myBalance(), banktokenreserve, fReserve);
    }
    ...
    lpToken.approve(address(vaults[fToken]), uint(-1));
    IVault(vaults[fToken]).depositAll();
    vaults[fToken].safeTransfer(msg.sender, vaults[fToken].myBalance());
  }
