...
contract PollyVotesTestFinal {
  ...
  function totalSupply() public view returns (uint256) {
    ...
    (uint256 lp_totalpollyA, uint256 lp_totalpollyB,) = pair.getReserves();
    (uint256 unlockedTotal) = polly.unlockedSupply();
    (uint256 lockedTotal) = polly.totalLock();

    return lp_totalpollyB*(3)+(unlockedTotal/(4))+(lockedTotal/(3));
  }
  ...
  function balanceOf(address owner) public view returns (uint256) {
    ...
    uint256 pair1Underlying = (reserveA_2*(userShare1))/(1000000);
