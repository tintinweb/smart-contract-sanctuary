...
// File: contracts/Pool/RisqMATICPool.sol

pragma solidity 0.6.12;

/**
 * Risq
 * Copyright (C) 2021 Risq Protocol
 *
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the GNU General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
...
/**
 * @author macemclain
 * @title Risq MATIC Liquidity Pool
 * @notice Accumulates liquidity in MATIC from LPs and distributes P&L in MATIC
 */
contract RisqMATICPool is
    IMATICLiquidityPool,
    Ownable,
    ERC20("Risq MATIC LP Token", "writeMATIC")
{
    using SafeMath for uint256;
    ...
    /*
     * @nonce A provider supplies MATIC to the pool and receives writeMATIC tokens
    ...
    /*
     * @nonce Provider burns writeMATIC and receives MATIC from the pool
     * @param amount Amount of MATIC to receive
    ...
    /*
     * @nonce calls by RisqCallOptions to lock the funds
    ...
    /*
     * @nonce Returns provider's share in MATIC
     * @param account Provider's address
     * @return Provider's share in MATIC
     */
    ...
    /*
     * @nonce Returns the amount of MATIC available for withdrawals
    ...
    /*
     * @nonce Returns the total balance of MATIC provided to the pool
...
// File: contracts/Options/RisqMATICOptions.sol
...
/**
 * @author macemclain
 * @title Risq MATIC (Avalanche) Bidirectional (Call and Put) Options
 * @notice Risq MATIC Options Contract
 */
contract RisqMATICOptions is Ownable, IRisqOptions {
    using SafeMath for uint256;

    IRisqStakingMATIC public settlementFeeRecipient;
    ...
    RisqMATICPool public pool;

    /**
     * @param pp The address of ChainLink MATIC/USD price feed contract
     */
    constructor(AggregatorV3Interface pp, IRisqStakingMATIC staking) public {
        pool = new RisqMATICPool();
        priceProvider = pp;
     ...
     */
    function setSettlementFeeRecipient(IRisqStakingMATIC recipient) external onlyOwner {
        require(block.timestamp < contractCreationTimestamp + 14 days);
     ...
     * @param period Option period in seconds (1 days <= period <= 4 weeks)
     * @param strike Strike price of the option
     * @param currentPrice Current price of MATIC
    ...
    /**
     * @notice Sends profits in MATIC from the MATIC pool to an option holder's address
