...
// File contracts/StrategyV3_Autofarm_Curve.sol
...
contract StrategyV3_Autofarm_Curve is StrategyV3 {
    uint256 mode; // 0 = 3amCRV, 1 = btcCRV, 2 = usd_weth_btc_CRV
    address public constant harvesterAddress = 0x2DE373C78AA74A0B3970880Fd2Ad3116ac1036c8;
    address public constant creator_3amCRV = 0x445FE580eF8d70FF569aB36e80c647af338db351;
    address public constant creator_btcCRV = 0xC2d95EEF97Ec6C17551d45e77B590dc1F9117C67;
    address public constant creator_USD_BTC_ETH_CRV = 0x3FCD5De6A9fC8A99995c406c77DDa3eD7E406f81;
    ...
    constructor(
        ...
        strategyAddress = _tokenAddresses[4];
        token0Address = _tokenAddresses[5];
        earnedAddress2 = _tokenAddresses[6];
        ...
        returns (uint256)
    {
        ...
        // If depositFee is set, than _wantAmt - depositFee
        if (depositFeeFactor < depositFeeFactorMax) {
            _wantAmt = _wantAmt.mul(depositFeeFactor).div(depositFeeFactorMax);
        }

        // update wantLockedTotal (should be higher because of external auto-compounding)
        wantLockedTotal = IAutoFarm(farmContractAddress).stakedWantTokens(pid, address(this));
    ...
    function _harvest() internal override virtual {
        // Harvest farm tokens
        IHarvester(harvesterAddress).harvest(strategyAddress);
    ...
    function earn() public override virtual nonReentrant whenNotPaused {
        ...
        // Converts farm tokens into token0Address
        uint256 earnedAmt = IERC20(earnedAddress).balanceOf(address(this));
        ...
        if (token0Amt > 0) {
            if (mode == 0) {
                // 3amCRV
                uint256[3] memory uamounts = [0, token0Amt, 0];
                IERC20(token0Address).safeIncreaseAllowance(creator_3amCRV, token0Amt);
                I3PoolLP(creator_3amCRV).add_liquidity(uamounts, 0, true);

            } else if (mode == 1) {
                // btcCRV
                uint256[2] memory uamounts = [token0Amt, 0];
                IERC20(token0Address).safeIncreaseAllowance(creator_btcCRV, token0Amt);
                I2PoolLP(creator_btcCRV).add_liquidity(uamounts, 0, true);

            } else if (mode == 2) {
                // usd_eth_btc_CRV
                uint256[5] memory uamounts = [0, token0Amt, 0, 0, 0];
                IERC20(token0Address).safeIncreaseAllowance(creator_USD_BTC_ETH_CRV, token0Amt);
                I5PoolLP(creator_USD_BTC_ETH_CRV).add_liquidity(uamounts, 0);
            }
        }
