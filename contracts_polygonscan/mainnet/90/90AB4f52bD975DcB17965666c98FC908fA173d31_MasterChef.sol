...
import "./KavianToken.sol";


// MasterChef is the master of Kavian. He can make Kavian and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once Kavian is sufficiently
...
contract MasterChef is Ownable, ReentrancyGuard {
	...
	struct UserInfo {
		...
		//
		// We do some fancy math here. Basically, any point in time, the amount of Kavians
		// entitled to a user but is pending to be distributed is:
		//
		//   pending reward = (user.amount * pool.accKavianPerShare) - user.rewardDebt
		//
		// Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
		//   1. The pool's `accKavianPerShare` (and `lastRewardBlock`) gets updated.
	...
	struct PoolInfo {
		...
		uint256 allocPoint; // How many allocation points assigned to this pool. KAVIAN to distribute per block.
		uint256 lastRewardBlock; // Last block number that KAVIANs distribution occurs.
		uint256 accKavianPerShare; // Accumulated KAVIAN per share, times 1e12. See below.
		uint16 depositFeeBP; // Deposit fee in basis points
		uint256 harvestInterval; // Harvest interval in seconds
	}

	// The KAVIAN TOKEN!
	KavianToken public kavian;
	// Dev address.
	...
	// KAVIAN tokens created per block.
	uint256 public kavianPerBlock;
	// Bonus muliplier for early kavian makers.
	...
	// The block number when KAVIAN mining starts.
	uint256 public startBlock;
	// Total locked up rewards
	uint256 public totalLockedUpRewards;

	// kavian referral contract address.
	IKavianReferral public kavianReferral;
	...
	constructor(
		KavianToken _kavian,
		uint256 _startBlock,
		uint256 _kavianPerBlock
	) public {
		kavian = _kavian;
		startBlock = _startBlock;
		kavianPerBlock = _kavianPerBlock;
	...
	// Update the given pool's KAVIAN allocation point and deposit fee. Can only be called by the owner.
	function set(
		uint256 _pid,
		uint256 _allocPoint,
		uint16 _depositFeeBP,
	...
	// View function to see pending KAVIANs on frontend.
	function pendingKavian(uint256 _pid, address _user) external view returns (uint256) {
		PoolInfo storage pool = poolInfo[_pid];
		UserInfo storage user = userInfo[_pid][_user];
		uint256 accKavianPerShare = pool.accKavianPerShare;
		uint256 lpSupply = pool.lpToken.balanceOf(address(this));
		if (block.number > pool.lastRewardBlock && lpSupply != 0) {
			uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
			uint256 kavianReward = multiplier.mul(kavianPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
			accKavianPerShare = accKavianPerShare.add(kavianReward.mul(1e12).div(lpSupply));
		}
		uint256 pending = user.amount.mul(accKavianPerShare).div(1e12).sub(user.rewardDebt);
		return pending.add(user.rewardLockedUp);
	}

	// View function to see if user can harvest KAVIANs.
	...
	function updatePool(uint256 _pid) public {
		...
		uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
		uint256 kavianReward = multiplier.mul(kavianPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
		kavian.mint(devAddress, kavianReward.div(10));
		kavian.mint(address(this), kavianReward);
		pool.accKavianPerShare = pool.accKavianPerShare.add(kavianReward.mul(1e12).div(lpSupply));
		pool.lastRewardBlock = block.number;
	}

	// Deposit LP tokens to MasterChef for KAVIAN allocation.
	function deposit(
		uint256 _pid,
		uint256 _amount,
		address _referrer
		...
		updatePool(_pid);
		if (_amount > 0 && address(kavianReferral) != address(0) && _referrer != address(0) && _referrer != msg.sender) {
			kavianReferral.recordReferral(msg.sender, _referrer);
		}
		payOrLockupPendingKavian(_pid);
		if (_amount > 0) {
		...
		user.rewardDebt = user.amount.mul(pool.accKavianPerShare).div(1e12);
	...
	// Pay or lockup pending KAVIANs.
	function payOrLockupPendingKavian(uint256 _pid) internal {
		PoolInfo storage pool = poolInfo[_pid];
		UserInfo storage user = userInfo[_pid][msg.sender];
		...
		uint256 pending = user.amount.mul(pool.accKavianPerShare).div(1e12).sub(user.rewardDebt);
		if (canHarvest(_pid, msg.sender)) {
			if (pending > 0 || user.rewardLockedUp > 0) {
				...
				// send rewards
				safeKavianTransfer(msg.sender, totalRewards);
	...
	// Safe kavian transfer function, just in case if rounding error causes pool to not have enough KAVIANs.
	function safeKavianTransfer(address _to, uint256 _amount) internal {
		uint256 kavianBal = kavian.balanceOf(address(this));
		if (_amount > kavianBal) {
			kavian.transfer(_to, kavianBal);
		} else {
			kavian.transfer(_to, _amount);
		}
	}
	...
	function updateEmissionRate(uint256 _kavianPerBlock) public onlyOwner {
		massUpdatePools();
		emit EmissionRateUpdated(msg.sender, kavianPerBlock, _kavianPerBlock);
		kavianPerBlock = _kavianPerBlock;
	}

	// Update the kavian referral contract address by the owner
	function setKavianReferral(IKavianReferral _kavianReferral) public onlyOwner {
		kavianReferral = _kavianReferral;
	}
	...
	function payReferralCommission(address _user, uint256 _pending) internal {
		if (address(kavianReferral) != address(0) && referralCommissionRate > 0) {
			address referrer = kavianReferral.getReferrer(_user);
			uint256 commissionAmount = _pending.mul(referralCommissionRate).div(10000);

			if (referrer != address(0) && commissionAmount > 0) {
				kavian.mint(referrer, commissionAmount);
				kavianReferral.recordReferralCommission(referrer, commissionAmount);
