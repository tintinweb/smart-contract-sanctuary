...
library DXswapLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
            ...
            hex'ae81bbc68f315fbbf7617eb881349af83b1e95241f616966e1e0583ecd0793fe' // init code hash 1hive's deployment
...
contract LinkSwapper {
  ...
  DXswapRouter public router;

  using SafeMath for uint256;  

  constructor(IERC20 _token,
              IDXswapFactory _factory,
              DXswapRouter _router) public {
      token = _token;
      factory = _factory;
      router = _router;
  ...
  receive() payable external {
    ...
    _approveTokenIfNeeded(address(router.WETH()),amountIn);
  ...
  function checkInputs(uint256 amountIn) public view returns(uint256,address[] memory,address,uint256){
    ...
    path[0] = router.WETH();
    path[1] = address(token);
    return(amountOut.mul(95).div(100), path, msg.sender, block.timestamp.add(10 minutes));
  }

  function getReserves() public view returns(uint256,uint256,uint32){
    IDXswapPair pair = IDXswapPair(factory.getPair(router.WETH(),address(token)));
  ...
  function getAmountOut(uint256 value,uint256 resA,uint256 resB) public view returns(uint256){
    return(router.getAmountOut(value,resA,resB,30));
  }

  function _approveTokenIfNeeded(address _token,uint256 amount) internal {
      IERC20 tokenContract = IERC20(_token);
      uint256 approved = tokenContract.allowance(address(this),address(router));
      if(approved < amount){
        tokenContract.approve(address(router),tokenContract.totalSupply());
      }
  }
