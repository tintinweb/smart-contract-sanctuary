...
contract SushiPolygonFraxUsdcStrategy is MiniChefStrategy {
    constructor() public {}

    function initialize(address _vault, address _parentStrategy) public initializer {
        address underlying = address(
            0x9e20a8d3501BF96EDA8e69b96DD84840058a1cB0
        ); //FraxUsdc SLP
        uint256 poolId = 14; // FraxUsdc Pool id
        ...
        address dai = address(0x8f3Cf7ad23Cd3CaDbD9735AFf958023239c6A063); // main asset - dai
        ...
        address frax = address(0x104592a158490a9228070E0A8e5343B499e125D0);
        ...
        rewardRoutes[frax] = [sushi, weth, usdc, frax];
        rewardRoutes[usdc] = [sushi, weth, usdc];

        secondRewardRoute = [wmatic, sushi];

        fromVaultAssetRoutes[dai][frax] = [dai, weth, usdc, frax];
        fromVaultAssetRoutes[dai][usdc] = [dai, weth, usdc];
        fromVaultAssetRoutes[usdt][frax] = [usdt, usdc, frax];
        fromVaultAssetRoutes[usdt][usdc] = [usdt, usdc];
        fromVaultAssetRoutes[usdc][frax] = [usdc, frax];

        toVaultAssetRoutes[dai][frax] = [frax, usdc, weth, dai];
        toVaultAssetRoutes[dai][usdc] = [usdc, weth, dai];
        toVaultAssetRoutes[usdt][frax] = [frax, usdc, usdt];
        toVaultAssetRoutes[usdt][usdc] = [usdc, usdt];
        toVaultAssetRoutes[usdc][frax] = [frax, usdc];
...
import "../../interface/sushiswap/IComplexRewarderTime.sol";
...
contract MiniChefStrategy is OwnableUpgradeable {
    ...
    uint256 public secondSellFloor;
    ...
    function setSecondSellFloor(uint256 _floor) external onlyOwner {
        secondSellFloor = _floor;
    }

    function setParentStrategy(address _strategy) external onlyOwner {
        parentStrategy = _strategy;
    }
    ...
    function _liquidateReward() internal {
        ...
        if (
            _secondRewardBalance > secondSellFloor &&
            secondRewardRoute.length > 1 &&
            _secondRewardBalance > 0
        ) {
...
interface IMiniChef {
    ...
    function updatePool(uint256 pid)
        external
        returns (IMiniChef.PoolInfo memory);
