...
// MissionIron3Pool
contract MissionIron3PoolLP is MissionIron {
    
    // IronRouter address
    address public ironRouterAddress;

    address public poolAddress;
    address public basePoolAddress;

    constructor(
        address _vaultChefAddress,
        address _uniRouterAddress,
        address _ironChefAddress,
        address _ironRouterAddress,
        ...
        ironRouterAddress = _ironRouterAddress;
    ...
    function earn() external override nonReentrant whenNotPaused onlyGov {
        ...
        if (earnedAmt > 0) {
            ...
            if (tokenAmt > 0) {
                uint256[] memory metaAmounts = new uint256[](2);
                uint256[] memory baseAmounts = new uint256[](3);
                baseAmounts[0] = tokenAmt; // USDC
                IIronRouter(ironRouterAddress).addLiquidity(
                    poolAddress,
                    basePoolAddress,
                    metaAmounts,
                    baseAmounts,
    ...
    function _resetAllowances() internal override {
        ...
        IERC20(usdcAddress).safeApprove(ironRouterAddress, uint256(0));
