...
contract GrowthVault is ERC20, Ownable {
    ...
    function autoCompound() public {
        ...
        for (uint256 i; i < names.length; i++) {
            ...
            if (_amount > 0) {
                ...
                amounts = IRouter(strategyObj.liquidityRouter).getAmountsOut(rewardBalance, path);
                if(amounts[1] > 100) {
                    _swapExactTokensForTokens(
                        strategyObj.liquidityRouter,
                        rewardBalance,
    ...
    function deposit() public payable {
        ...
        _addLiquidityETH(liquidityRouter, lpToken, token0, token1);
        earn();
    ...
    function _addLiquidity(
        ...
        IERC20(_token0).transfer(_lpToken, desired0);
        IERC20(_token1).transfer(_lpToken, desired1);
        IPair(_lpToken).mint(address(this));
    }

    function _addLiquidityETH(address _liquidityRouter, address _lPtoken, address _token0, address _token1) internal {
        address[] memory path = new address[](2);
        uint256[] memory amounts;
        ...
        if(_token1 != WETH && _token0 == WETH) {
            ...
            _addLiquidity(_liquidityRouter, _lPtoken, _token0, _token1, path);
        }
    }
