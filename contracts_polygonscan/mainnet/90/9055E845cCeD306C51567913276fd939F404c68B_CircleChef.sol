...
contract CircleChef is ICircleChef, OwnableUpgradeable {
    ...
    ICircleMinterV2 public minter;

    uint public startBlock;
    uint public override circlePerBlock;
    uint public override totalAllocPoint;

    /* ========== MODIFIERS ========== */

    modifier onlyVaults {
        require(vaults[msg.sender].token != address(0), "CircleChef: caller is not on the vault");
        _;
    }
    ...
    modifier updateRewards(address vault) {
        ...
        if (block.number > vaultInfo.lastRewardBlock) {
            ...
            if (tokenSupply > 0) {
                ...
                uint rewards = multiplier.mul(circlePerBlock).mul(vaultInfo.allocPoint).div(totalAllocPoint);
                vaultInfo.accCirclePerShare = vaultInfo.accCirclePerShare.add(rewards.mul(1e12).div(tokenSupply));
            }
    ...
    event CircleRewardPaid(address indexed user, address indexed vault, uint amount);

    /* ========== INITIALIZER ========== */

    function initialize(uint _startBlock, uint _circlePerBlock) external initializer {
        __Ownable_init();

        startBlock = _startBlock;
        circlePerBlock = _circlePerBlock;
    }
    ...
    function pendingCircle(address vault, address user) public view override returns (uint) {
        UserInfo storage userInfo = vaultUsers[vault][user];
        VaultInfo storage vaultInfo = vaults[vault];

        uint accCirclePerShare = vaultInfo.accCirclePerShare;
        uint tokenSupply = tokenSupplyOf(vault);
        if (block.number > vaultInfo.lastRewardBlock && tokenSupply > 0) {
            uint multiplier = timeMultiplier(vaultInfo.lastRewardBlock, block.number);
            uint circleRewards = multiplier.mul(circlePerBlock).mul(vaultInfo.allocPoint).div(totalAllocPoint);
            accCirclePerShare = accCirclePerShare.add(circleRewards.mul(1e12).div(tokenSupply));
        }
        return userInfo.pending.add(userInfo.balance.mul(accCirclePerShare).div(1e12).sub(userInfo.rewardPaid));
    }

    /* ========== RESTRICTED FUNCTIONS ========== */

    function addVault(address vault, address token, uint allocPoint) public onlyOwner {
        require(vaults[vault].token == address(0), "CircleChef: vault is already set");
        bulkUpdateRewards();
    ...
    function updateVault(address vault, uint allocPoint) public onlyOwner {
        require(vaults[vault].token != address(0), "CircleChef: vault must be set");
        bulkUpdateRewards();
    ...
    function setMinter(address _minter) external onlyOwner {
        require(address(minter) == address(0), "CircleChef: setMinter only once");
        minter = ICircleMinterV2(_minter);
    }

    function setCirclePerBlock(uint _circlePerBlock) external onlyOwner {
        bulkUpdateRewards();
        circlePerBlock = _circlePerBlock;
    }
    ...
    function notifyDeposited(address user, uint amount) external override onlyVaults updateRewards(msg.sender) {
        ...
        uint pending = userInfo.balance.mul(vaultInfo.accCirclePerShare).div(1e12).sub(userInfo.rewardPaid);
        userInfo.pending = userInfo.pending.add(pending);
        userInfo.balance = userInfo.balance.add(amount);
        userInfo.rewardPaid = userInfo.balance.mul(vaultInfo.accCirclePerShare).div(1e12);
    ...
    function safeCircleTransfer(address user) external override onlyVaults updateRewards(msg.sender) returns (uint) {
        UserInfo storage userInfo = vaultUsers[msg.sender][user];
        VaultInfo storage vaultInfo = vaults[msg.sender];
        ...
        minter.mint(amount);
        minter.safeCircleTransfer(user, amount);
        emit CircleRewardPaid(user, msg.sender, amount);
        return amount;
    }
    ...
    function recoverToken(address _token, uint amount) virtual external onlyOwner {
        require(_token != CIRCLE, "CircleChef: cannot recover CIRCLE token");
