...
// File: contracts/interfaces/IRonFarm.sol
...
interface IRonFarm {
    ...
    // View function to see pending DOPs on frontend.
    function pendingReward(uint256 _pid, address _user)
        external
        view
        returns (uint256);

    // Deposit LP tokens to MasterChef for DOP allocation.
...
// File: contracts/matic/StrategyIronUpgraderable.sol
...
contract StrategyIronUpgraderable is ReentrancyGuardUpgradeSafe, OwnableUpgradeable , Pausable {
    ...
    function initialize(
        ...
        earnedToToken0Path = [earnedAddress,usdcAddress, token0Address];
        if (usdcAddress == token0Address) {
            earnedToToken0Path = [earnedAddress, usdcAddress];
        }

        earnedToToken1Path = [earnedAddress,usdcAddress, token1Address];
        if (usdcAddress == token1Address) {
            earnedToToken1Path = [earnedAddress, usdcAddress];
        }

        token0ToEarnedPath = [token0Address,usdcAddress, earnedAddress];
        if (usdcAddress == token0Address) {
            token0ToEarnedPath = [usdcAddress, earnedAddress];
        }

        token1ToEarnedPath = [token1Address,usdcAddress, earnedAddress];
        if (usdcAddress == token1Address) {
            token1ToEarnedPath = [usdcAddress, earnedAddress];
        }
    ...
    function _farm() internal {
        ...
        // Deposit and harvest rewards
        IRonFarm(farmContractAddress).deposit(pid, wantAmt,address(this));
        ...
        returns (uint256)
    {
        ...
        if (isGrandComp) {
            IRonFarm(farmContractAddress).withdraw(pid, _wantAmt,address(this));
        }
    ...
    function earn() public whenNotPaused {
        ...
        // Harvest farm tokens
        IRonFarm(farmContractAddress).harvest(pid,address(this));
