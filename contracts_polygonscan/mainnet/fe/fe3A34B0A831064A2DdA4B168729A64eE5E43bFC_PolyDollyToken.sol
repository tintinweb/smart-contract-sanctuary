...
//-----------------------------------------------------------------------------
contract PolyDollyToken is BEP20("PolyDolly Token", "PolyDolly", 10000000000000000000000) {
...
contract MasterChef is Ownable, NonReentrant {
  ...
  struct PoolInfo {
    ...
    uint256 accPolyDollyPerShare;
    uint16 depositFeeBP;
  }
  //---------------------------------------------------------------------------
  PolyDollyToken public dolly;
  ...
  constructor(PolyDollyToken _dolly, address _feeAddress, uint256 _startBlock) public {
    dolly = _dolly;
 ...
 //---------------------------------------------------------------------------
  function add(uint256 _allocPoint, IBEP20 _lpToken, bool _withUpdate) public ownerOnly isUnique(_lpToken) {
    ...
    poolInfo.push(PoolInfo({lpToken: _lpToken, allocPoint: _allocPoint, lastRewardBlock: lastRewardBlock, accPolyDollyPerShare: 0, depositFeeBP: depositFee})
    );
  }
  function pendingPolyDolly(uint256 _pid, address _user) external view returns (uint256)  {
    PoolInfo storage pool = poolInfo[_pid];
    UserInfo storage user = userInfo[_pid][_user];
    uint256 share = pool.accPolyDollyPerShare;
    uint256 pot = pool.lpToken.balanceOf(address(this));
    if (block.number > pool.lastRewardBlock && pot != 0) {
      uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
      uint256 PolyDollyReward = multiplier.mul(EMISSION_RATE).mul(pool.allocPoint).div(totalAllocPoint);
      share = share.add(PolyDollyReward.mul(1e12).div(pot));
    }
  ...
  function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
    ...
    updatePool(_pid);
    if (user.amount > 0) {
      uint256 pending = user.amount.mul(pool.accPolyDollyPerShare).div(1e12).sub(user.rewardDebt);
      if (pending > 0) {
        safePolyDollyTransfer(msg.sender, pending);
      }
    }
    if (_amount > 0) {
    ...
    user.rewardDebt = user.amount.mul(pool.accPolyDollyPerShare).div(1e12);
  ...
  function updatePool(uint256 _pid) public {
    ...
    uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
    uint256 PolyDollyReward = multiplier.mul(EMISSION_RATE).mul(pool.allocPoint).div(totalAllocPoint);
    dolly.mint(feeAddress, PolyDollyReward.div(20));
    dolly.mint(address(this), PolyDollyReward);
    pool.accPolyDollyPerShare = pool.accPolyDollyPerShare.add(PolyDollyReward.mul(1e12).div(lpSupply));
    pool.lastRewardBlock = block.number;
  }
  ...
  function safePolyDollyTransfer(address _to, uint256 _amount) internal {
    uint256 pot = dolly.balanceOf(address(this));
    bool success = false;
    if (_amount > pot) {
      success = dolly.transfer(_to, pot);
    } else {
      success = dolly.transfer(_to, _amount);
    }
    require(success, "error");
  }
