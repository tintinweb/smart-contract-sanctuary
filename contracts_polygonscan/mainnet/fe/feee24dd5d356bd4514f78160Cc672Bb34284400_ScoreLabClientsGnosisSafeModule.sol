...
contract ScoreLabClientsGnosisSafeModule {
    ...
    */
    ...
    private 
    {
        ...
        uint256 preSwapBalance = 0;

        if (fromToken == 0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE) {
            etherAmount = amount;

            preSwapBalance = safe.balance;
        } else {
            preSwapBalance = IERC20(fromToken).balanceOf(safe);
        }
        ...
        uint256 afterSwapBalance = 0;

        if (fromToken == 0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE) {
            afterSwapBalance = safe.balance;
        } else {
            afterSwapBalance = IERC20(fromToken).balanceOf(safe);
        }

        allowance.amount -= preSwapBalance - afterSwapBalance;
    }
