...
/// @dev Compare price denominated in quote token between two pools
    ...
    function append10(string memory  a, string memory b,string memory  c, string memory d,string memory  e, string memory f,string memory g, string memory h, string memory i, string memory j) internal pure returns (string memory) {
    ...
    function calcBorrowAmount(OrderedReserves memory reserves) internal pure returns (uint256 amount) {
        ...
        */
        ...
        if (maxBitsNeeded >= 255)
        {
            bitReductions = (maxBitsNeeded - 254);
        }
        ...
        {
            bitReductions += (bitInfo.bitsNeeded_m - 254);
        }
        ...
        */
        ...
        require(a > 0, append10("a < 0 => d: ", uint2str(d), " a1: ", uint2str(uint256(a1)), " a2: ", uint2str(uint256(a2)), " b1: ", uint2str(uint256(b1)), " b2: ", uint2str(uint256(b2)))); 
        int256 b = 2 * b1 * b2 * (a1 + a2);
        require(b > 0, append("b < 0 => d: ", uint2str(d))); 
        int256 c = b1 * b2 * (a1 * b2 - a2 * b1);
        require(c > 0, append("c < 0 => d: ", uint2str(d))); 
