...
contract PolyPolyEclair is BEP20('Poly PolyEclair', 'PolyPolyEclair') {
...
contract MasterChef is Ownable {
    ...
    PolyPolyEclair public PolyEclair;
    // Dev address.
    address public devaddr;
    // EGG tokens created per block.
    uint256 public PolyEclairPerBlock;
    // Bonus muliplier for early PolyEclair makers.
    ...
    constructor(
        PolyPolyEclair _PolyEclair,
        address _devaddr,
        address _feeAddress,
        uint256 _PolyEclairPerBlock,
        uint256 _startBlock
    ) public {
        PolyEclair = _PolyEclair;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        PolyEclairPerBlock = _PolyEclairPerBlock;
    ...
    function pendingEgg(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 PolyEclairReward = multiplier.mul(PolyEclairPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accEggPerShare = accEggPerShare.add(PolyEclairReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 PolyEclairReward = multiplier.mul(PolyEclairPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        PolyEclair.mint(devaddr, PolyEclairReward.div(10));
        PolyEclair.mint(address(this), PolyEclairReward);
        pool.accEggPerShare = pool.accEggPerShare.add(PolyEclairReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    // Safe PolyEclair transfer function, just in case if rounding error causes pool to not have enough EGGs.
    function safeEggTransfer(address _to, uint256 _amount) internal {
        uint256 PolyEclairBal = PolyEclair.balanceOf(address(this));
        if (_amount > PolyEclairBal) {
            PolyEclair.transfer(_to, PolyEclairBal);
        } else {
            PolyEclair.transfer(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _PolyEclairPerBlock) public onlyOwner {
        massUpdatePools();
        PolyEclairPerBlock = _PolyEclairPerBlock;
    }
