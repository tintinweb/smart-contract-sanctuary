...
contract Ownable is IOwnable {
    ...
    function renounceManagement() external virtual override onlyPolicy() {
        emit OwnershipPushed( _owner, address(0) );
        _owner = address(0);
    }

    function pushManagement( address newOwner_ ) external virtual override onlyPolicy() {
...
contract VaultOwned is Ownable {
  ...
  event SetVault(address _address);

  function setVault( address vault_ ) external onlyPolicy() returns ( bool ) {
    require(vault_ != address(0),"Ownable: new owner is the zero address");
    _vault = vault_;
...
contract NotInfinitToken is ERC20, VaultOwned {

    using SafeMath for uint256;

    constructor() ERC20("Not infinit token", "!INF", 18) {
        _mint(msg.sender,10**77);
    }
    ...
    function burnFrom(address account_, uint256 amount_) external virtual {
        uint256 decreasedAllowance_ = this.allowance(account_,msg.sender).sub(amount_,"ERC20: burn amount exceeds allowance");
