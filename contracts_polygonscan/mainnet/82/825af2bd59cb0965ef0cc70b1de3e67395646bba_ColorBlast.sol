...
// Inspired, based on, and built with parts from: OpenPalette on ethereum -> @open_pal
...
contract ColorBlast is ERC721, ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;

    Counters.Counter private _tokenIdCounter;

    constructor() ERC721("Color Blast", "#HEX") {}
        ...
        return
            string(
                abi.encodePacked(
                    ...
                    " dur=4s repeatCount=indefinite values=-.15;0.7></stop><stop offset=0.4 stop-color=#222><animate attributeName=offset dur=4s repeatCount=indefinite values=0.1;0.9 linear></stop><stop offset=.5 stop-color=#fff><animate attribute",
                    "name=offset dur=4s repeatCount=indefinite values=0.25;1.1></stop></linearGradient><linearGradient id=g2 x1=-100% x2=200% y1=0 y2=0 gradienttransform=rotate(-90)><stop offset=0 stop-color=#fff></stop><stop offset=0.3 stop-color=#",
                    "fff><animate attributeName=offset dur=3.5s repeatCount=indefinite values=-1;0.4></stop><stop offset=0.4 stop-color=#000><animate attributeName=offset dur=3.5s repeatCount=indefinite values=-0.35;0.6 linear></stop><stop offset=.5",
                    " stop-color=#fff><animate attributeName=offset dur=3.5s repeatCount=indefinite values=0;0.75></stop></linearGradient><rect height=100% width=100% />",
                    getAllPogs(
                        color1,
                        color2,
                        color3,
    ...
    function generateTrait(uint256 _color, uint256 _index)
        internal
        pure
    ...
    {
        uint256 color1 = _getColor(tokenId, 1);
        uint256 color2 = _getColor(tokenId, 2);
        uint256 color3 = _getColor(tokenId, 3);
        uint256 color4 = _getColor(tokenId, 4);
        uint256 color5 = _getColor(tokenId, 5);
        uint256 color6 = _getColor(tokenId, 6);
        uint256 color7 = _getColor(tokenId, 7);
        uint256 color8 = _getColor(tokenId, 8);
        uint256 color9 = _getColor(tokenId, 9);

        string memory attributes = string(
            abi.encodePacked(
                '"attributes" :[',
                generateTrait(color1, 1),
                ",",
                generateTrait(color2, 2),
                ",",
                generateTrait(color3, 3),
                ",",
                generateTrait(color4, 4),
                ",",
                generateTrait(color5, 5),
                ",",
                generateTrait(color6, 6),
                ",",
                generateTrait(color7, 7),
                ",",
                generateTrait(color8, 8),
                ",",
                generateTrait(color9, 9),
                "]}"
            )
        );
            ...
            bytes(
                Base684.encode(
                    abi.encodePacked(
                        '{"name": "Color Blast #',
                        toString(tokenId),
                        '", "description": "on-chain colors palettes for use in decentralized applications and games.",',
    ...
    function getColor(uint256 tokenId, uint256 _colorIndex)
        public
        pure
        returns (string memory)
    {
        // Returns an unlimited number of colors for each token!
        return getColorHexCode(generateColor(toString(_colorIndex), tokenId));
    }

    function _getColor(uint256 tokenId, uint256 _colorIndex)
        internal
        pure
        returns (uint256)
    {
        // Returns an unlimited number of colors for each token!
        return generateColor(toString(_colorIndex), tokenId);
    }
