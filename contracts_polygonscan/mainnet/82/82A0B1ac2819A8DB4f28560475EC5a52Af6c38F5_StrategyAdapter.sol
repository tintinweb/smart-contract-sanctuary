...
import "./AlphaStrategyPolygon.sol";

contract StrategyAdapter is AlphaStrategyPolygon {
  ...
  function initialize(
    ...
    AlphaStrategyPolygon.initializeAlphaStrategy(
      _underlying,
      _vault,
...
contract AlphaStrategyPolygon is OwnableUpgradeable {
  ...
  function unsalvagableTokens(address token) public view returns (bool) {
    return (token == quick || token == underlying);
  }
  ...
  */
  ...
  function pendingDQuick() public view returns (uint256) {
    return IERC20(dQuick).balanceOf(address(this));
  }
  ...
  function withdrawReward(address user) public onlyVault {
    ...
    uint256 _pendingDQuick = IERC20(vault).balanceOf(user)
    .mul(accDQuickPerShare)
    .div(1e36)
    .sub(userDQuickDebt[user]);
    uint256 _dQuickBalance = IERC20(dQuick).balanceOf(address(this));
    if (_dQuickBalance < _pendingDQuick) {
      _pendingDQuick = _dQuickBalance;
    }
  ...
  *   Withdraws all the asset to the vault
  */
  function withdrawToVault(uint256 amount) public onlyOwner {
  ...
  */
  ...
  function claimQuickRewardPool() internal {
      uint256 bal = quickRewardPoolBalance();
      if (bal != 0) {
  ...
  function stakeDQuick() external {
    claimQuickRewardPool();

    uint256 quickRewardBalance = IERC20(quick).balanceOf(address(this));
    if (!sell || quickRewardBalance < sellFloor) {
      // Profits can be disabled for possible simplified and rapid exit
      // emit ProfitsNotCollected(sell, quickRewardBalance < sellFloor);
      return;
    }

    if (quickRewardBalance == 0) {
      return;
    }

    IERC20(quick).safeApprove(dQuick, 0);
    IERC20(quick).safeApprove(dQuick, quickRewardBalance);

    uint256 balanceBefore = IERC20(dQuick).balanceOf(address(this));
    SushiBar(dQuick).enter(quickRewardBalance);
    uint256 balanceAfter = IERC20(dQuick).balanceOf(address(this));
    uint256 added = balanceAfter.sub(balanceBefore);

    if (added > 0) {
      uint256 fee = added.mul(keepFee).div(keepFeeMax);
      IERC20(dQuick).safeTransfer(treasury, fee);
    }
  }

  function setOnxTreasuryFundAddress(address _address) public onlyOwner {
    treasury = _address;
  }
}
