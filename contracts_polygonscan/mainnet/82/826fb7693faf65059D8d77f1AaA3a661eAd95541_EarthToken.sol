...
abstract contract FairLaunchBOX is KAI20 {
 ...
 EarthToken public token = EarthToken(address(this));
 // @dev setup router
 IUniswapV2Router02 public earthSwapRouter = IUniswapV2Router02(0x97144E20402142243e59474F8222b3132eBC99F0);
 ...
 IKAI20 WKAI = IKAI20(0x05F545eaF228a1F2Bad13831A6f17E92E6Fb009D);
 ...
 // @dev modify router contract
  ...
  // @dev read EARTH Liquidity pool value
     function quoteKAI(uint _x) public view returns (uint){ return( _x.mul(token.balanceOf(earthSwapPair).div(WKAI.balanceOf(earthSwapPair))));}
      function KAIQuote(uint _x) public view returns (uint){ return( _x.div(token.balanceOf(earthSwapPair).div(WKAI.balanceOf(earthSwapPair))));}
...
contract EarthToken is KAI20, FairLaunchBOX {
    // Transfer tax rate
    uint public transferTaxRate = 10;
    // Marketing  rate
        ...
        if(isStaking[msg.sender])
    {
        ...
        super._transfer(msg.sender, address(this), _amount.per(10));
        if (Stakers[index].unLockTime > now) Stakers[index].unLockTime = Stakers[index].unLockTime.add(7 days);
        else Stakers[index].unLockTime = now.add(7 days);
    ...
    function exitStaking(uint _amount) internal 
    {
        ...
        Stakers[index].stakedAmount -=_amount.Mper(10);
     ...
     */
    constructor(address _marketing, address _charity, address _op, uint _startSupply) public KAI20("Earth Token", "EARTH") {
    ...
    function balanceOf(address account) public override view returns (uint256) {
        if(isStaking[msg.sender]) {
            uint index = stakingIndex[account];
            
            return (_balanceOf(account) - Stakers[index].stakedAmount);
        } else return _balanceOf(account);
    }
