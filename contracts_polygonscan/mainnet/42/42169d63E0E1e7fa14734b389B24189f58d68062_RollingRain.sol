...
contract RollingRain is VRFConsumerBase, Ownable {
  ...
  uint256 waitForRounds = 1;
  uint256 public maxPicks = 13;
  ...
  event Bet(uint256 wag, uint256 betMask, uint256 modulo, uint256 numbers, uint256 currentBalance);
  ...
  function setMaxPicks(uint256 number) external onlyOwner {
      maxPicks = number;
  }
  ...
  function rollKeysLength() external view returns(uint256) {
  ...
  function fulfillRandomness(bytes32 requestId, uint256 randomValue) internal override {
    ...
    if(losses > wins){
        maxPicksModulo = (maxPicks * 2) % 37;
    }
    uint256 pickMax = uint256(keccak256(abi.encode(randomValue, rI * 2))) % maxPicksModulo + 1;
    bool[] memory picks = new bool[](pickMax+1);
    uint256 i = 0;
    while(i < pickMax){
        picks[uint256(keccak256(abi.encode(randomValue, rI + i + 1))) % 37] = true;
        i = i + 1;
    }
    for(i = 0; i < 37; i++){
        number = number << 1;
  ...
  function manualSpin(uint256 randomValue) public onlyOwner{
      rI = rI + 1;
      uint256 number = 1;
      uint256 max = 2 ** 37;
      uint256 numbers = 0;
  ...
  function spinDryRun(uint256 randomValue) external view returns(uint256, uint256){
  ...
  function roll(uint256 betMask, uint256 numbers) internal {
    ...
    if(wasPendingBet > waitForRounds || pendingBet == 0){
        if(rolls.length > 0){
            if(polyroll.bets(rolls[rolls.length-1]).outcome > 0){
                wins = wins + 1;
            } else {
                losses = losses + 1;
            }
        ...
        if(address(this).balance >= betAmount){
            ...
            if(alwaysBetMask == 0){
                polyroll.placeBet{ value: betAmount }(betMask, useModulo, owner());
                emit Bet(betAmount, betMask, useModulo, numbers, address(this).balance);
            }else{
                polyroll.placeBet{ value: betAmount }(alwaysBetMask, useModulo, owner());
                emit Bet(betAmount, alwaysBetMask, useModulo, numbers, address(this).balance);
            }
