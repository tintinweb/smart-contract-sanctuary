...
contract VaultStakingBELILp is Vault {
        ...
        returns (uint256)
    {
        ...
        uint256 wantAmt = postBalance.sub(preBalance);
        ...
        returns (uint256)
    {
        ...
        uint256 withdrawAmount = _wantAmt;

        uint256 wantAmt = IERC20(wantAddress).balanceOf(address(this));
        if (withdrawAmount > wantAmt) {
            withdrawAmount = wantAmt;
        }

        if (wantLockedTotal < withdrawAmount) {
            withdrawAmount = wantLockedTotal;
        }

        wantLockedTotal = wantLockedTotal.sub(withdrawAmount);

        IERC20(wantAddress).safeTransfer(beliFarmAddress, withdrawAmount);
...
abstract contract Vault is Ownable, ReentrancyGuard, Pausable, ERC20 {
    ...
    uint256 public earnToBeliRate = 0;
    uint256 public constant earnToBeliRateMax = 10000; // 100%
    uint256 public constant earnToBeliUL = 5000; // 50%
