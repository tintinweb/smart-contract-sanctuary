...
contract mooRewardPool is LPTokenWrapper, Ownable {
    ...
    constructor(address _shareToken, address _rewardToken, address _strategy)
        public
        LPTokenWrapper(_shareToken)
    {
        rewardToken = IERC20(_rewardToken);
        strategy = _strategy;
    }

    // compare snapshots and allocate earnings
    ...
    // retrieve most recent snapshot of reward per share
    function rewardPerToken() public view returns (uint256) {
        return IStrategy(strategy).rewardPerToken();
    }

    // calculate earnings based on snapshots and shares owned
