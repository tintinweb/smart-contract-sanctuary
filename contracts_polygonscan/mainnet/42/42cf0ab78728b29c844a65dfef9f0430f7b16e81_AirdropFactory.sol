...
contract AirdropFactory is Operatorable{
    ...
    // 查询质押时间
    function PledgeTime(uint8 _index, address addr) public view AirdropCheck(_index) returns (uint256) {
        AirdropV2 storage airdrop = airdrops[_index];
        require(airdrop.addresses[addr] != 0, "THIS ADDRESS HAS NOT PLEDGE");
        return airdrop.withdraws[addr];
    }

    // 等待领取时间
    function waitClaimTime(uint8 _index, address addr) public view AirdropCheck(_index) returns (uint256) {
        AirdropV2 storage airdrop = airdrops[_index];
        require(airdrop.addresses[addr] != 0, "THIS ADDRESS HAS NOT PLEDGE");
        return (airdrop.claims[addr] + airdrop.claim_timestamp) - block.timestamp ;
    }


    // 等待赎回时间
    function waitWithdrawTime(uint8 _index, address addr) public view AirdropCheck(_index) returns (uint256) {
        AirdropV2 storage airdrop = airdrops[_index];
        require(airdrop.addresses[addr] != 0, "THIS ADDRESS HAS NOT PLEDGE");
        return (airdrop.withdraws[addr] + airdrop.withdraw_timestamp) - block.timestamp ;
    }
    ...
    // 修改活动结束区块高度
    function updateEndTimestamp(uint8 _index, uint256 _timestamp) public onlyOperator AirdropCheck(_index) {
        AirdropV2 storage airdrop = airdrops[_index];
