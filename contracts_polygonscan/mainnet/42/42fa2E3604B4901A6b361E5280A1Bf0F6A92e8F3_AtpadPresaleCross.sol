...
contract AtpadPresaleCross is ReentrancyGuard, Ownable, Pausable {
    ...
    /// @dev constants
    string public constant name = 'AtpadPresaleCross V1.1.10';

    /// @dev variables
    uint public rate = 28571;
    ...
    uint initialVestPercentage= 30;

    uint startVest = 1643468400;
    uint endVest = 1648652400;
    ...
    uint8 public wDecimals = 18;
    uint8 public iDecimals = 6;
    ...
    function depositTokens(uint _amount) external payable onlyOwner { 
        ...
        require(_amount >= (10 * 10**wDecimals) , "min amount is 10 tokens");
    ...
    function _swap(address _from, uint _amount, uint _perc) private { 
        ...
        if (iDecimals >= wDecimals) {
            _decimals += (iDecimals - wDecimals);
            claims[_from].reserved = swaps[_from].mul(rate).div(10**_decimals);
        } else {
            claims[_from].reserved = swaps[_from].mul(rate).mul(10**(wDecimals - iDecimals)).div(10**_decimals);
        }
    ...
    function swapClaim() external payable nonReentrant claimEnabled whenFinished returns (bool) {
    ...
    function _swapClaim(address _from) private {   //
        ...
        uint _released = (claims[_from].reserved * _free).div(100);

        require( claims[_from].claimed < _released, '!nothing to claim');
        
        uint _amount =  _released - claims[_from].claimed;
        ...
        if ( claims[_from].claimed > claims[_from].reserved ) {
            ...
            wantToken.safeTransfer(_from, _amount);
        }
    ...
    function getReleasedPercentageFromTable() view public returns (uint) {
        ...
        if (_time > startVest) _perc = 30;
        if (_time > startVest.add(_month.mul(1))) _perc = 65;
        if (_time > startVest.add(_month.mul(2))) _perc = 100;

        return _perc;
    }

    /** better routine, not unit tested though as per 20.01.22 .....
    function getReleasedPercentageFromCalculation() view public returns (uint) {
        ...
        for(uint i = 0 ; i < 12 ; i++) {
            if (_time > startVest.add(i.mul(_month)))_perc=(10).add(i.mul(7));  
        }
    ...
    function getAllocPercentage(address _wallet) view public returns (uint) {
        ...
        uint _allocPerc = (allocPercentage[_wallet].mul(fcfsPercentage)).div(100);
    ...
    function setInitialVestPercentage(uint8 _initialVestPercentage) external onlyOwner {
    ...
    function setWantToken(address _token, uint8 _decimals) external onlyOwner {
