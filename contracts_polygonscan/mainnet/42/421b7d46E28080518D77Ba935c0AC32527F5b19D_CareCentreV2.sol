...
contract CareCentreV2 is Ownable {
  ...
  function exec(address caringOwner, uint256[] calldata gotchiIds) external {
    require(
  ...
  function frenExec(address caringOwner, uint256[] calldata gotchiIds)
    external
  {
    require(
      executors.contains(msg.sender),
      "CareCentreV2: frenExec: Sender is not executor"
    );
  ...
  // function shouldEndSession(address caringOwner) internal {
  //   if (treasury.userBalance(caringOwner) < rateOfTier[0]) {
  //     caringOwners.remove(caringOwner);
  //     delete caringOwnerPetCount[caringOwner];
  //   }
  // }
