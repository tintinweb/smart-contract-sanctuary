...
// MasterChef is the master of Quarks. He can make QRK and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once qrk is sufficiently
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of qrk
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accQrkPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accQrkPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. QRK to distribute per block.
        uint256 lastRewardBlock;  // Last block number that qrk distribution occurs.
        uint256 accQrkPerShare;   // Accumulated QRK per share, times 1e12. See below.
    ...
    // The QUARKS TOKEN!
    Quarks public quarks;
    
    // Addresses to link to price.
    IERC20 public usdc;
    address public usdcQrkLP = address(0x0); 
    
    // Burn address.
    address public burnaddr;
    // SH13LD tokens created per block.
    uint256 public qrkPerBlock = 1 ether;
    // Maximum emission rate
    uint256 public maxEmissionRate = 1 ether;
    // Bonus muliplier for early qrk makers.
    ...
    // The block number when qrk mining starts.
    ...
    uint public emissionUpdateInterval = 1600; 
    ...
    constructor(
        ...
        address _burnaddr,
        address _feeAddress,
        uint256 _qrkPerBlock,
        uint256 _startBlock
    ) {
        quarks = _qrk;
        burnaddr = _burnaddr;
        feeAddress = _feeAddress;
        qrkPerBlock = _qrkPerBlock;
    ...
    // Update the given pool's qrk allocation point and deposit fee. Can only be called by the owner.
    function set(uint256 _pid, uint256 _allocPoint, uint16 _depositFeeBP,uint256 _harvestInterval,	
        uint256 _maxHarvestPercent,	uint16 _tokenTransferTax, bool _withUpdate) public onlyOwner {
    ...
    // View function to see pending qrk on frontend.
    function pendingQrk(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accQrkPerShare = pool.accQrkPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 qrkReward = multiplier.mul(qrkPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accQrkPerShare = accQrkPerShare.add(qrkReward.mul(1e12).div(lpSupply));
        }
        uint256 pending =  user.amount.mul(accQrkPerShare).div(1e12).sub(user.rewardDebt);
    ...
    // View function to see if user can harvest qrk.	
    ...
    function updatePool(uint256 _pid) public {
        ...
        quarks.mint(burnaddr, qrkReward.div(10));
        quarks.mint (address(this),qrkReward);
        pool.accQrkPerShare = pool.accQrkPerShare.add(qrkReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for qrk allocation.
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
       payOrLockupPendingQrk(_pid);
        if (_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accQrkPerShare).div(1e12);

        updateEmissionIfNeeded();
        emit Deposit(msg.sender, _pid, _amount);
    }
    
    // Pay or lockup pending qrk.	
    function payOrLockupPendingQrk(uint256 _pid) internal {	
        PoolInfo storage pool = poolInfo[_pid];	
        UserInfo storage user = userInfo[_pid][msg.sender];	
        if (user.nextHarvestUntil == 0) {	
            user.nextHarvestUntil = block.timestamp.add(pool.harvestInterval);	
        }
         uint256 pending = user.amount.mul(pool.accQrkPerShare).div(1e12).sub(user.rewardDebt);	
        if (canHarvest(_pid, msg.sender)) {	
            if (pending > 0 || user.rewardLockedUp > 0) {	
                ...
                user.nextHarvestUntil = block.timestamp.add(pool.harvestInterval);	
                // send rewards	
                safeQrkTransfer(msg.sender, rewardCut);	
            }	
        } else if (pending > 0) {	
    ...
    // Safe qrk transfer function, just in case if rounding error causes pool to not have enough qrk.
    function safeQrkTransfer(address _to, uint256 _amount) internal {
        uint256 qrkBal = quarks.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > qrkBal) {
            transferSuccess = quarks.transfer(_to, qrkBal);
        } else {
            transferSuccess = quarks.transfer(_to, _amount);
        }
        require(transferSuccess, "safeQrkTransfer: transfer failed");
    }
   ...
   function updateEmissionIfNeeded() public {
        ...
        if (block.number - lastBlockUpdate > emissionUpdateInterval) {
            ...
            uint qrkBalance = quarks.balanceOf(usdcQrkLP);
          
            if (qrkBalance > 0) {
                // usdc token decimals = 6, token decimals = 18 , = 14 to convert to cents
                priceCents = usdc.balanceOf(usdcQrkLP).mul(1e14).div(qrkBalance);
            }

            // Update pools before changing the emission rate
            massUpdatePools();
            uint256 emissionRatePercent = getEmissionRatePercent(priceCents);
            qrkPerBlock = maxEmissionRate.div(100).mul(emissionRatePercent);
        }
    }

    // for front end
    function getQrkPriceCents() public view returns (uint256 spc){
         uint qrkBalance = quarks.balanceOf(usdcQrkLP);
          if (qrkBalance > 0) {
            uint256 priceCents = usdc.balanceOf(usdcQrkLP).mul(1e14).div(qrkBalance);
            return priceCents;
          }
          return 0;
    }

    function getEmissionRatePercent(uint256 qrkPriceCents) public view returns (uint256 epr) {
        
       if (qrkPriceCents>=topPrice*100){return (1);}
       if (qrkPriceCents<=bottomPrice*100){return (100);}

       uint256 qrkPricePercentOfTop = qrkPriceCents.mul(100).div(topPrice*100);
       int256 curveCoefficient = FixidityLib.reciprocal(int256(curveRate));
       uint256 f1 = uint256(100).sub(qrkPricePercentOfTop);
       int256 p1 = FixidityLib.fromFixed(FixidityLib.multiply(curveCoefficient,int256(f1)));
       int256 p2 = FixidityLib.fromFixed(FixidityLib.divide(100,int256(qrkPricePercentOfTop)));
    ...
    function updateEmissionRate(uint256 _qrkPerBlock) public onlyOwner {
        massUpdatePools();
        qrkPerBlock = _qrkPerBlock;
        emit UpdateEmissionRate(msg.sender, _qrkPerBlock);
    }
    ...
    // For checking prices to link emission rate
    function setUsdcQrkLPAddress(address _address) public onlyOwner {
        usdcQrkLP = _address;
    }
