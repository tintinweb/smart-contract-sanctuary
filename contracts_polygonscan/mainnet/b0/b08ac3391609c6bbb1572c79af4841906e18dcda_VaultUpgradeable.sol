...
contract VaultUpgradeable is StorageV1ConsumerUpgradeable, ERC20Upgradeable, ICreditor {
    ...
    function _accountWithdrawFeeAtDeposit(address _who, uint256 _amount) internal {
        if(!noWithdrawalFee[_who] && withdrawFeeRatio > 0) {
            ...
            withdrawalFeeOccured[_who] = withdrawalFeePending(_who).add(withdrawFeeNewlyOccured);
            lastFeeTime[_who] = block.timestamp;
        }
    }
    ...
    function _withdrawSendwithFee(address _who, uint256 _sendAmount) internal {
        ...
        uint256 _withdrawalFeePending = withdrawalFeePending(_who);
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        ...
        uint256 _withdrawalFeePendingFromSender = withdrawalFeePending(sender);
        
        // If sender have pending withdrawal fees, we transfer the withdrawal fee 
        if(_withdrawalFeePendingFromSender > 0)
        {
            uint256 withdrawFeeTransferred = _withdrawalFeePendingFromSender.mul(amount).div(_balance);
            withdrawalFeeOccured[sender] = _withdrawalFeePendingFromSender.sub(withdrawFeeTransferred);
            lastFeeTime[sender] = block.timestamp;

            withdrawalFeeOccured[recipient] =  withdrawalFeePending(recipient).add(withdrawFeeTransferred); 
            lastFeeTime[recipient] = block.timestamp;   
        }
    ...
    function withdrawalFeePending(address _who) public view returns (uint256 fee) {
