...
contract NftizePool is IERC721Receiver, AccessControl {
    ...
    function depositNFT(
        ...
        if (socialCauseFee > 0 && royaltiesPercentage == 0) {
            ...
            poolToken.mint(address(this), basePrice);
            poolToken.transfer(msg.sender, basePrice - (feeAmount + royaltyAmount));
            poolToken.transfer(feeAddress, feeAmount);
        } else if (socialCauseFee == 0 && royaltiesPercentage == 0) {
            poolToken.mint(msg.sender, basePrice);
        } else {
            royaltyAmount = calculateFee(basePrice, royaltiesPercentage);
            poolToken.mint(address(this), basePrice);
            poolToken.transfer(msg.sender, basePrice - royaltyAmount);
        }

        poolNfts[nftContractAddress][tokenId] = NFTData(msg.sender, nftPrice, i, royaltyAmount);
    ...
    function withdrawNFT(address nftContractAddress, uint256 tokenId) public {
        ...
        if (socialCauseFee > 0) {
            uint256 feeAmount = calculateFee(nftPrice, socialCauseFee);
    ...
    function claimTheosLiquidity(address lpManagerAddress) public {
        INftizePoolLpManager lpManager = INftizePoolLpManager(lpManagerAddress);
