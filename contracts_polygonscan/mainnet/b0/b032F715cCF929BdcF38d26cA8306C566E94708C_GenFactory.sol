...
//A: where GenTickets is connected to - create Gen ticket is connected to here
contract GenFactory is IGenFactory {
    ...
    //A: when you click lock after filling in datas to create ticket
    function createGenTicket(
        ...
        //A: creating a new ticket //this GenFactory is mainly creating new contract on chain - GenTickets contract (ERC1155) 
...
contract GenTickets is ERC1155, ERC1155Receiver {
    ...
    function redeemTicket(address _to, uint256 _id, uint256 _amount) public { //address, 8, 1

        //cliffTranches = 0 -> length of ciff
        //trancheLength = 720 // 240 // 0
        // totalTranches = 1
        ...
        // Tranche past cliff
        uint tranche = _id.div(numTicketTypes); //0/4=0
        ...
        if (ticketType.trancheLength != 0){
            eligibleTraches = (getBlockTimestamp() - ((ticketType.trancheLength).mul(ticketType.cliffTranches).add(TGE))).div(ticketType.trancheLength);
        }
        if (eligibleTraches >= ticketType.totalTranches) {
            eligibleTraches = ticketType.totalTranches;
        } else if (eligibleTraches <= 0 && ticketType.cliffTranches == 0 ) {
            eligibleTraches = 1;
        } else {
            eligibleTraches += 1;
        }
        //Handle div by zero here //amasa
        uint tranchesWiseTokens = ticketType.ticketSize.div((ticketType.totalTranches) - (ticketType.cliffTranches)); //10/(5-0) = 2
        uint averageTokensAvailableToClaim = tranchesWiseTokens.mul(_amount); //2*4 = 8 so 2*1 = 2

        uint redeemedTokens = 0;
        if (tranche >= 1){
            redeemedTokens  = averageTokensAvailableToClaim.mul(tranche); 
        }
        ...
        bytes memory data;
        _mint(_to, _id.add(numTicketTypes * (eligibleTraches - tranche)), _amount, data); //check later
    }
