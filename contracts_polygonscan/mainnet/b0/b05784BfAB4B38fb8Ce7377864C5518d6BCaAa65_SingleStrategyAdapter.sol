...
contract SingleStrategyPolygon is OwnableUpgradeable {
  ...
  */
  ...
  function pendingReward() public view returns (uint256) {
    address rewardToken = getRewardsToken(dQuickExternalRewardsPool);
  ...
  function withdrawReward(address user) public onlyVault {
    ...
    address dQuickRewardsToken = getRewardsToken(dQuickExternalRewardsPool);
    ...
    if(_pending > 0 && _pending <= curPendingReward){
      // send reward to user
      IERC20(dQuickRewardsToken).safeTransfer(user, _pending);
