...
//
/**
 * @title Digitalax whitelisted Staking v3
 * @dev Stake whitelisted nfts and stake membership guild tokens upon them
 * @author DLTA Team
 */
contract GuildWhitelistedNFTStakingV3 is BaseRelayRecipient {
    ...
    mapping(address => bool) public whitelistedTokensNeedsTokenWhitelist;
    mapping(address => mapping(uint256 => bool)) public whitelistedTokensIdWhitelist;
    mapping(address => bool) public whitelistedNFTIs1155;
    ...
    */
    ...
    event AddWhitelistedTokens(
        ...
        bool[] requireTokenIdWhitelist,
        bool[] is1155
    );

    event AddWhitelistedTokenIds(
        address whitelistedToken,
        uint256[] tokenIds
    );

    event RemoveWhitelistedTokenIds(
        address whitelistedToken,
        uint256[] tokenIds
    );
    ...
    function addWhitelistedTokens(address[] memory _whitelistedTokens, bool[] memory _whitelistTokenIdsRequired, bool[] memory _is1155) public {
        require(
        ...
        require(_whitelistedTokens.length == _whitelistTokenIdsRequired.length, "Arrays must be equal size");
        require(_whitelistedTokens.length == _is1155.length, "Arrays must be equal size");
        for (uint i = 0; i < _whitelistedTokens.length; i++) {
            if(!checkInWhitelistedTokens(_whitelistedTokens[i])) {
                ...
                whitelistedTokensNeedsTokenWhitelist[_whitelistedTokens[i]] = _whitelistTokenIdsRequired[i];
                whitelistedNFTIs1155[_whitelistedTokens[i]] = _is1155[i];
            }
            if(whitelistedTokensNeedsTokenWhitelist[_whitelistedTokens[i]] != _whitelistTokenIdsRequired[i]){
                whitelistedTokensNeedsTokenWhitelist[_whitelistedTokens[i]] = _whitelistTokenIdsRequired[i];
            }
            if(whitelistedNFTIs1155[_whitelistedTokens[i]] != _is1155[i]){
                whitelistedNFTIs1155[_whitelistedTokens[i]] = _is1155[i];
            }
        }
    ...
    function whitelistTokenIds(address _whitelistedToken, uint256[] memory _tokenIdsToWhitelist) public {
        ...
        for (uint i = 0; i < _tokenIdsToWhitelist.length; i++) {
            whitelistedTokensIdWhitelist[_whitelistedToken][_tokenIdsToWhitelist[i]] = true;
        }
        emit AddWhitelistedTokenIds(_whitelistedToken, _tokenIdsToWhitelist);
    }

    function removeWhitelistTokenIds(address _whitelistedToken, uint256[] memory _tokenIdsToRemoveWhitelist) public {
        require(
            accessControls.hasAdminRole(_msgSender()),
            "GuildWhitelistedNFTStaking.addWhitelistedTokens: Sender must be admin"
        );
        for (uint i = 0; i < _tokenIdsToRemoveWhitelist.length; i++) {
            whitelistedTokensIdWhitelist[_whitelistedToken][_tokenIdsToRemoveWhitelist[i]] = false;
        }
        emit RemoveWhitelistedTokenIds(_whitelistedToken, _tokenIdsToRemoveWhitelist);
    }
    ...
    */
    function _stake(address _user, address _whitelistedNFT, uint256 _tokenId) internal {
        ...
        if(whitelistedTokensNeedsTokenWhitelist[_whitelistedNFT]) {
            require(whitelistedTokensIdWhitelist[_whitelistedNFT][_tokenId], "The token id is not whitelisted");
        }
    ...
    */
    ...
    function _unstake(address _user, address _whitelistedNFT, uint256 _tokenId, bool _isApprovedParty) internal {
        ...
        if(_isApprovedParty){
            sendNFT(_whitelistedNFT, address(this), approvedParty[_whitelistedNFT][_tokenId], _tokenId);
        } else {
            sendNFT(_whitelistedNFT, address(this), _user, _tokenId);
        }
    ...
    function adminEmergencySafeUnstake(address _whitelistedNFT, uint256 _tokenId) public {
        ...
        sendNFT(_whitelistedNFT, address(this), tokenOwner[_whitelistedNFT][_tokenId], _tokenId);
    }
    ...
    function adminEmergencyUnstake(address _whitelistedNFT, uint256 _tokenId) public {
        ...
        require(!whitelistedNFTIs1155[_whitelistedNFT], "This method is only for ERC721");
    ...
    function updateReward(address _user) public {
        ...
        for (uint i=0; i< _tokens.length; i++) {
            ...
            uint256 thisTokenRewards = IGuildNFTTokenRewards(address(rewardsContract)).WhitelistedTokenRevenueRewards(_tokens[i], lastUpdateTime,
                                                _getNow());
    ...
    function unclaimedRewards(address _user) external view returns(uint256) {
        if(stakers[_user].numberNFTStaked == 0){
            return 0;
        }
    ...
    function sendNFT(address _whitelistedNFT, address _from, address _to, uint256 _tokenId) internal returns (bool){
        if(whitelistedNFTIs1155[_whitelistedNFT]){
            IERC1155(_whitelistedNFT).safeTransferFrom(_from, _to, _tokenId, 1, "0x0"); // Quantity for this is always 1, it is first come first served
        } else {
            IERC721(_whitelistedNFT).safeTransferFrom(_from, _to, _tokenId);
        }
        return true;
    }
