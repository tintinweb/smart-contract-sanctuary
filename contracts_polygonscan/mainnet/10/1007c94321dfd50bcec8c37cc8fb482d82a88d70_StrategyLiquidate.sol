...
contract StrategyLiquidate is Ownable, ReentrancyGuard, Strategy {
  ...
  function execute(
    ...
    address bank = origin.operator();
    address config = IBank(bank).config();
    address bConfig = IConfig(config).bigfoots(msg.sender);
    ...
    require(IBConfig(bConfig).isStable(msg.sender),"not stable");
