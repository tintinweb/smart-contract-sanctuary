...
contract DEI_DEUS_ZAPPER is Ownable {
	...
	function zapInNativecoin(
		...
		emit ZappedIn(address(0), pair_address, msg.value, LPBought, transferResidual);
	}
	...
	function zapInERC20(
		...
		require(LPBought >= minLPAmount, "ZAPPER::zapIn: INSUFFICIENT_OUTPUT_AMOUNT");
	...
	function zapInDEUS(
		...
		(uint256 token0Bought, uint256 token1Bought) = _buyTokens(deus_address, amountIn);
	...
	function getAmountOutLPERC20ORNativecoin(uint amount, address[] memory path) public view returns (uint percentage, uint lp) {
		uint[] memory amounts_out = IUniswapV2Router02(uniswap_router).getAmountsOut(amount, path);
		uint dei_amount = amounts_out[amounts_out.length - 1];
		return getAmountOutLPDEI(dei_amount);
	}
