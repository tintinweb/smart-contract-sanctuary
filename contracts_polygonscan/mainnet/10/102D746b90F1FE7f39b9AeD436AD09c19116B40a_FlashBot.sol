...
contract FlashBot {
    ...
    event BaseTokenChanged(address indexed token);
    event DistributionAddressChanged(address indexed token);
    ...
    function withdraw() internal {
        ...
        if(rewardToken == WETH) {
            ...
            if (balance > 0) {
                uint256 devShare = balance / 50; // 2%
                balance = balance - devShare;
