...
contract PlanetPunks is ERC721Enumerable, ReentrancyGuard, Ownable {


    string private _tokenBaseURI = "https://ipfs.io/ipfs/Qmcv64bmxSWtKdR8Cv77bLGs1tiB5XSgCv9XKFaXs9qvUZ/";
    ...
    function setBaseURI(string memory baseURI) public onlyOwner {
        ...
        function setexURI(string memory exurl) public onlyOwner {
        require(!frozen, "Contract is frozen.");
        externalUrl = exurl;
    }
    ...
    uint256 public MAX_MINT = 10000;

    uint256 public publicIssued = 0;
    uint256 public publicMax = 9000;

    function mintPublic(address _to ,uint count) public {
        require(_mintPerAddress[_to]+count < MAX_PER_ADDRESS+1, "You have reached your minting limit.Max per Address");
        require(publicIssued+count < MAX_MINT, "There are no more NFTs for minting.");
        ...
        require(publicIssued+count < publicMax, "There are no more NFTs for public minting at this time.");
        require(count < 21, "Must be under 21");
        
    for (uint256 i = 0; i < count ; i = i + 1) {
        _mintPerAddress[_to] += 1;

        uint256 tokenId = publicIssued ;
        ...
        function ownerClaim(address _to ,uint count) public nonReentrant onlyOwner {
        require(publicIssued < MAX_MINT, "There are no more NFTs for minting.");
        require(publicIssued+count < MAX_MINT, "There are no more NFTs for minting.");
    ...
    function setaddressmax(uint256 _adrsmax) public onlyOwner {
        MAX_PER_ADDRESS = _adrsmax;
    }
    ...
    constructor() ERC721("PlanetPunks", "PPC") Ownable() {}
}
