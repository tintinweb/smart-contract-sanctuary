...
contract PolyPolySoul is BEP20('Poly PolySoul', 'PolyPolySoul') {
...
contract MasterChef is Ownable {
    ...
    PolyPolySoul public PolySoul;
    // Dev address.
    address public devaddr;
    // EGG tokens created per block.
    uint256 public PolySoulPerBlock;
    // Bonus muliplier for early PolySoul makers.
    ...
    constructor(
        PolyPolySoul _PolySoul,
        address _devaddr,
        address _feeAddress,
        uint256 _PolySoulPerBlock,
        uint256 _startBlock
    ) public {
        PolySoul = _PolySoul;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        PolySoulPerBlock = _PolySoulPerBlock;
    ...
    function pendingEgg(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 PolySoulReward = multiplier.mul(PolySoulPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accEggPerShare = accEggPerShare.add(PolySoulReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 PolySoulReward = multiplier.mul(PolySoulPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        PolySoul.mint(devaddr, PolySoulReward.div(10));
        PolySoul.mint(address(this), PolySoulReward);
        pool.accEggPerShare = pool.accEggPerShare.add(PolySoulReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    // Safe PolySoul transfer function, just in case if rounding error causes pool to not have enough EGGs.
    function safeEggTransfer(address _to, uint256 _amount) internal {
        uint256 PolySoulBal = PolySoul.balanceOf(address(this));
        if (_amount > PolySoulBal) {
            PolySoul.transfer(_to, PolySoulBal);
        } else {
            PolySoul.transfer(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _PolySoulPerBlock) public onlyOwner {
        massUpdatePools();
        PolySoulPerBlock = _PolySoulPerBlock;
    }
