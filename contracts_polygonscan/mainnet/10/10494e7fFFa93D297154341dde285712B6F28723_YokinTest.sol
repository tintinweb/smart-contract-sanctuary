...
contract ERC20 is Context, IHRC20, Ownable {
    ...
    */
    ...
    function locked(address account) internal {
        ...
        LockedDatas[account].regStats = isRegistered[account];

        totalNetworkData.holdsInProgress = totalNetworkData.holdsInProgress + 1;
        totalNetworkData.holdedToken = totalNetworkData.holdedToken + LockedDatas[account].balance;
        // emit locked(account);
    }

    function unLocked(address account) private {
    ...
    function calcInterestRewardAmount(uint256 balance, uint256 lockedDays) public view returns(uint256 amount) {
        ...
        uint256 interestAmount = balance.mul(APY).mul(lockedDays).div(365).div(100);
    ...
    function rewardTo(address account, uint256 interestAmount, bool bRegistered) private {
        ...
        if(bRegistered) {
            ...
            totalNetworkData.bonusPaid = totalNetworkData.bonusPaid + bonusAmount;
        }
        // emit Transfer(account, rewardAmount);
    }

    function RegisterAffiliate(address account) public {
    ...
    function StopHolding() public {
        ...
        if(isValid[msg.sender] == true) {
            ...
            bool bRegistered = LockedDatas[msg.sender].regStats;
            
            rewardTo(msg.sender, interestAmount, bRegistered);
            
            totalNetworkData.interestPaid = totalNetworkData.interestPaid + interestAmount;
        }
        ...
        totalNetworkData.holdsInProgress = totalNetworkData.holdsInProgress - 1;
        totalNetworkData.holsCompleted = totalNetworkData.holsCompleted + 1;
        
        isValid[msg.sender] = false;
    }

    function ResetHolding() public {
        require(isLocked[msg.sender], "Account should be holding");
...
// YokinToken
contract YokinTest is ERC20('YokinTest_v1', 'YK_v1', 8), MinterRole {
