...
contract TokenPresale is Ownable {
    ...
    bool public initializedToken = false;
    bool public initializedDates = false;
    bool public initializedFeaturesAndFees = false;
    ...
    function initializeToken(
        ...
        require(!initializedToken, 'initialize:ALREADY_INITIALIZED');
        initializedToken = true;

        //TOKEN ADDRESS and ROUTER to be used i.e. Quickswap, SushiSwap, Dfyn, ApeSwap, etc.
        token = _token; //The token address that will be up for presale. Ex. 0xD47fC7F8dAc3bb78709fcdcA47841491853643b1
        router = IUniswapV2Router02(_router); //The AMM where the liquidity will be deposited. Ex. 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff
        
        //TOKEN CAP
        softCap = _softCap; //Minimum amount of tokens to sell for presale to proceed. Ex. 1000000000000000000 or 1 token
        hardCap = _hardCap; //Total number of tokens up for sale. Ex. 1000000000000000000 or 1 token
        hardCapEth = _hardCapEth; //How many ETH are you willing to raise? Ex. 1000000000000000000 or 1 token
        
        //USER TOKEN CAP
        minTokens = _minTokens; //Minimum number of tokens a wallet should buy. Ex. 1000000000000000000 or 1 token
        maxTokens = _maxTokens; //Minimum number of tokens a wallet can hold. Ex. 1000000000000000000 or 1 token
    ...
    function initializeDates(
        ...
        require(!initializedDates, 'initialize:ALREADY_INITIALIZED');
        initializedDates = true;

        //DATES
        startTime = _startTime; //In timestamp format, start date of the presale. Ex. 1628902800
        endTime = _endTime; //In timestamp format, end date of the presale. Ex. 1628902800
        claimTime = _claimTime; //In timestamp format, date when the bought tokens can be claimed. Ex. 1628902800
        whitelistStartTime = _whitelistStartTime; //In timestamp format, date the whitelisted addresses can buy the tokens ahead of time. Ex. 1628902800
        liquidityLockTime = _liquidityLockTime; //In timestamp format, when the owner can withdraw the liquidity deposited by presale contract. Ex. 1628902800

    }


    function initializeFeaturesAndFees (
        ...
        require(!initializedFeaturesAndFees, 'initialize:ALREADY_INITIALIZED');
        initializedFeaturesAndFees = true;

        //FEATURES
        useBlocks = _useBlocks; //In bool, if the dates will be in block number format or not. Ex. true
        burnUnsold = _burnUnsold; //In bool, if the unsold tokens in presale will be burnt. Ex. true
        autoAddLiquidity = _autoAddLiquidity; //In bool, if the MATICs will be auto added for liquidity. Ex. true

        //LIQUIDITY AND FEES
        liquidityPct = _liquidityPct; //How many percent of the MATIC and tokens will be put into liquidity? Ex. 10000 or 100%
        feeBP = _feeBP; //Ex. 10000 or 100%
        listingType = _listingType; //Ex. 10000 or 100%

        if (autoAddLiquidity == true) {
    ...
    function whitelistAddress(address _addr, uint256 _amount) public onlyOwner {
    ...
    function set(string memory _key, string memory _value) public onlyOwner {
        settings[_key] = _value;
    }


    function multiSet(string[2][] memory values) public onlyOwner {
    ...
    function getTokensForLiquidity(uint256 tokens) public view returns (uint256) {
        return (tokens * liquidityPct) / 10000;
    }


    function getTokenAmount(uint256 _ethValue) public view returns (uint256) {
        if (hardCapEth > 0) {
    ...
    function timeOrBlock() public view returns (uint256) {
