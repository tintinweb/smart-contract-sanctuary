...
// BallToken with Governance.
contract BallToken is BEP20('Ball Token', 'BALL') {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "BALL::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "BALL::delegateBySig: invalid nonce");
        require(now <= expiry, "BALL::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "BALL::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying BALL (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "BALL::_writeCheckpoint: block number exceeds 32 bits");
...
// MasterChef is the master of BALL. He can make Ball and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once BALL is sufficiently
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of BALL
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accBallPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accBallPerShare` (and `lastRewardBlock`) gets updated.
    ...
    // The BALL TOKEN!
    BallToken public ball;
    // Dev address.
    address public devaddr;
    // BALL tokens created per block.
    uint256 public BallPerBlock;
    // Bonus muliplier for early ball makers.
    ...
    // Safe ball transfer function, just in case if rounding error causes pool to not have enough BALLs.
    function safeBallTransfer(address _to, uint256 _amount) internal {
        uint256 ballBal = ball.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > ballBal) {
            transferSuccess = ball.transfer(_to, ballBal);
        } else {
            transferSuccess = ball.transfer(_to, _amount);
        }
