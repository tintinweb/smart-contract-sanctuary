...
contract PooToken is ERC20Burnable, ERC20Capped, IPooToken {
    ...
    function cap() public view override (ERC20Capped, IPooToken) returns (uint256) {
    ...
    uint public cappedSupply = 100000000 * 10 ** ERC20.decimals();
    uint public singleApproveSum = cappedSupply;
    ...
    constructor(address _constantsContract) ERC20("PooToken", "POO") ERC20Capped(cappedSupply) {
    ...
    function claimOwnerRewards(address to, uint amount) public onlyOwner {
        require(ownerRewards >= amount);
        _mint(address(to), amount);
        ownerRewards -= amount;
    }

    function claimRevRewards(address to, uint amount) public onlyRevShareOwner {
        require(revRewards >= amount);
        _mint(address(to), amount);
        revRewards -= amount;
    }
    ...
    function mintAdditionalRewards(address _receiver, uint _amount) public override onlyAllowedContracts {
        ...
        if ((totalSupply() + totalRewardAmount) <= (cap())) {
