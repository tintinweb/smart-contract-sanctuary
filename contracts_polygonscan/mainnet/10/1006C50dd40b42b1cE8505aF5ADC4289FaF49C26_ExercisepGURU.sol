...
contract ExercisepGURU is Ownable {
    ...
    constructor( address _pGURU, address _guru, address _dai, address _treasury, address _circulatingGURUContract ) {
        owner = msg.sender;
    ...
    function setTerms(address _vester, uint _amountCanClaim, uint _rate ) external onlyManager() returns ( bool ) {
    ...
    function exercise( uint _amount ) external returns ( bool ) {
        ...
        uint GURUToSend = ITreasury( treasury ).deposit( _amount, DAI, 0 );
    ...
    function redeemable( Term memory _info ) internal view returns ( uint ) {
        return ( ICirculatingGURU( circulatingGURUContract ).GURUCirculatingSupply().mul( _info.percent ).mul( 1000 ) ).sub( _info.claimed );
    }
