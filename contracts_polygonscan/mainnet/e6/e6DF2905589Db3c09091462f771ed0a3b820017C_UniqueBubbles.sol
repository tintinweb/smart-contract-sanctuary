...
contract UniqueBubbles is ERC1155Base,Ownable {
    string public name;
    ...
    function mint(Fee[] memory fees, string memory uri) public payable {
        require(msg.value == 1*10**18,"The cost to mint is 1 matic");
        ...
        require(_totalSupply.add(1) <= 10000,"Maximum of 10000 bubbles can be minted");
    ...
    function _sendToFounders(uint256 total) private {
      ...
      for(uint256 i = 0;i<founders.length;i++){
        (bool sent, bytes memory data) = payable(founders[i]).call{value: part}("");
    ...
    function setFounders(address[] calldata _founders) external onlyOwner{
      founders = _founders;
    }
