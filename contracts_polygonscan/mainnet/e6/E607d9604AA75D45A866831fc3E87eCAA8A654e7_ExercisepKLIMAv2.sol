...
interface IOldExercisePKLIMA {
    ...
    function terms(address _vester) external view returns ( Term memory );

}

interface IwsKLIMA {
    
    function sKLIMATowKLIMA( uint amount ) external view returns ( uint );
    function wKLIMATosKLIMA( uint amount ) external view returns ( uint );
}
...
contract ExercisepKLIMAv2 {
    ...
    address public immutable oldPklimaExercise;
    ...
    constructor( address _pKLIMA, address _KLIMA, address _BCT, address _treasury, address _circulatingKLIMAContract, address _oldPklimaExercise, address _wsKLIMA, address _sKLIMA ) {
        owner = msg.sender;
        ...
        oldPklimaExercise = _oldPklimaExercise;
        wsKLIMA = _wsKLIMA;
        sKLIMA = _sKLIMA;
    }
    // Migrate terms from old contract to new contract

    function migrateTerms( address _vester) external returns ( bool ) {
        require( msg.sender == owner, "Sender is not owner" );
        terms[_vester].max = IOldExercisePKLIMA ( oldPklimaExercise ).terms(_vester).max;
        terms[_vester].percent = IOldExercisePKLIMA ( oldPklimaExercise ).terms(_vester).percent;
        terms[ _vester ].claimed = IwsKLIMA(wsKLIMA).sKLIMATowKLIMA( IOldExercisePKLIMA ( oldPklimaExercise ).terms(_vester).claimed.mul( IsKLIMA( sKLIMA ).index()).div(10 ** 18));
    ...
    function exercise( uint _amount ) external returns ( bool ) {
        ...
        require( info.max.sub( IwsKLIMA(wsKLIMA).wKLIMATosKLIMA(info.claimed).mul(1e9) ) >= _amount, 'Claimed over max' );
        ...
        terms[ msg.sender ].claimed = info.claimed.add( IwsKLIMA(wsKLIMA).sKLIMATowKLIMA( _amount.div(1e9)));
    ...
    function redeemable( Term memory _info ) internal view returns ( uint ) {
        return ( ICirculatingKLIMA( circulatingKLIMAContract ).KLIMACirculatingSupply().mul( _info.percent ).div( 1e6 ) ).sub( IwsKLIMA(wsKLIMA).wKLIMATosKLIMA(_info.claimed) ).mul(1e9);
    }
