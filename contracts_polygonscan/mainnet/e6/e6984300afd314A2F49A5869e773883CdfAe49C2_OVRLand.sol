    ...
    ContextMixin
{
    ...
    /**
     * @dev Emitted when minted from bridge.
     */
    event Deposited(uint256 indexed tokenId);

    constructor(address childChainManager)
        public
        ERC721("OVR Land", "OVRLand")
    {
        _setupContractId("ChildMintableERC721");
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _setupRole(DEPOSITOR_ROLE, childChainManager);
        _setupRole(MINTER_ROLE, _msgSender());
        _setupRole(LAND_URI_EDITOR_ROLE, _msgSender());
        _initializeEIP712("OVR Land");
    }

    function addURIEditor(address _editor) public only(DEFAULT_ADMIN_ROLE) {
        grantRole(LAND_URI_EDITOR_ROLE, _editor);
    }

    function removeURIEditor(address _editor) public only(DEFAULT_ADMIN_ROLE) {
        revokeRole(LAND_URI_EDITOR_ROLE, _editor);
    }

    function addMinter(address _minter) public only(DEFAULT_ADMIN_ROLE) {
        grantRole(MINTER_ROLE, _minter);
    }

    function removeMinter(address _minter) public only(DEFAULT_ADMIN_ROLE) {
        revokeRole(MINTER_ROLE, _minter);
    }

    function addAdminRole(address _admin) public only(DEFAULT_ADMIN_ROLE) {
        grantRole(DEFAULT_ADMIN_ROLE, _admin);
        grantRole(MINTER_ROLE, _admin);
        grantRole(LAND_URI_EDITOR_ROLE, _admin);
    }

    function removeAdminRole(address _admin) public only(DEFAULT_ADMIN_ROLE) {
    ...
    function mintWithUri(
        ...
        uint256 _tokenId,
        string memory _uri
    ) public only(MINTER_ROLE) returns (bool) {
        require(
            !withdrawnTokens[_tokenId],
            "ChildMintableERC721: TOKEN_EXISTS_ON_ROOT_CHAIN"
        );
    ...
    IERC721Enumerable
{
        ...
        virtual
    {
        ...
        emit UriChanged(tokenId, _tokenURI);
    }
