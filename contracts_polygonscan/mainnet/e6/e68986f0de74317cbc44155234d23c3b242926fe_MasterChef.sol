...
contract PolyBatmanSwap is BEP20('Poly BatmanSwap', 'PolyBatmanSwap') {
...
contract MasterChef is Ownable {
    ...
    PolyBatmanSwap public BatmanSwap;
    // Dev address.
    address public devaddr;
    // EGG tokens created per block.
    uint256 public BatmanSwapPerBlock;
    // Bonus muliplier for early BatmanSwap makers.
    ...
    constructor(
        PolyBatmanSwap _BatmanSwap,
        address _devaddr,
        address _feeAddress,
        uint256 _BatmanSwapPerBlock,
        uint256 _startBlock
    ) public {
        BatmanSwap = _BatmanSwap;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        BatmanSwapPerBlock = _BatmanSwapPerBlock;
    ...
    function pendingEgg(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 BatmanSwapReward = multiplier.mul(BatmanSwapPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accEggPerShare = accEggPerShare.add(BatmanSwapReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 BatmanSwapReward = multiplier.mul(BatmanSwapPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        BatmanSwap.mint(devaddr, BatmanSwapReward.div(10));
        BatmanSwap.mint(address(this), BatmanSwapReward);
        pool.accEggPerShare = pool.accEggPerShare.add(BatmanSwapReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    // Safe BatmanSwap transfer function, just in case if rounding error causes pool to not have enough EGGs.
    function safeEggTransfer(address _to, uint256 _amount) internal {
        uint256 BatmanSwapBal = BatmanSwap.balanceOf(address(this));
        if (_amount > BatmanSwapBal) {
            BatmanSwap.transfer(_to, BatmanSwapBal);
        } else {
            BatmanSwap.transfer(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _BatmanSwapPerBlock) public onlyOwner {
        massUpdatePools();
        BatmanSwapPerBlock = _BatmanSwapPerBlock;
    }
