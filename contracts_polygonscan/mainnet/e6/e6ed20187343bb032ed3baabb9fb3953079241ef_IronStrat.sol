 ...
 */
contract IronStrat is Ownable, Pausable {
    ...
    function deposit() public whenNotPaused {
        ...
        if (pairBal > 0) {
            IMasterChef(masterchef).deposit(poolId, pairBal, address(this));
        }
    }
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (pairBal < _amount) {   
            IMasterChef(masterchef).withdraw(poolId, _amount.sub(pairBal), address(this));
     ...
     */
    function approveVault(address _token, address _vault, uint _amount) external onlyOwner {
        require(_token != lpPair, "No rugs sir");
