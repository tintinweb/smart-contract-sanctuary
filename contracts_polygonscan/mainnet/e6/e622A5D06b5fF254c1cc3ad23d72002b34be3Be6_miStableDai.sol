...
contract miStableDai is ReentrancyGuard {
    ...
    function setDAIRatePerMimatic(uint256 _rate) public {
        require(admin==msg.sender, "ser pls no hack");
        daiRate = _rate;
    }

    function setMimaticRatePerDAI(uint256 _rate) public {
        require(admin==msg.sender, "ser pls no hack");
        maiRate = _rate;
    }
    ...
    function swapTo(uint256 amount) public nonReentrant {
	    ...
	    require(dai.balanceOf(address(this)) >= amountToSend, "swapTo: Not enough Dai in reserves");
