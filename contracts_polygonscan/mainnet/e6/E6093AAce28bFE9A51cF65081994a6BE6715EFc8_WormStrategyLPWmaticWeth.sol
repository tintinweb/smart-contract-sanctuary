...
abstract contract BaseWormStrategy is Ownable, ReentrancyGuard, Pausable {
    ...
    uint256 public buyBackRate = 750;
    uint256 public constant feeMaxTotal = 2000; // Max 20%
    ...
    address[] public wantToToken0Path;
    address[] public wantToToken1Path;
    address[] public token0ToWantPath;
    address[] public token1ToWantPath;
    ...
    function deposit(address _userAddress, uint256 _wantAmt) external onlyOwner nonReentrant whenNotPaused returns (uint256) {
        ...
        if (token0Amt > 0 && token1Amt > 0) {
            IUniRouter02(farmedRouterAddress).addLiquidity(
                token0Address,
                token1Address,
                token0Amt,
                token1Amt,
    ...
    function withdraw(address _userAddress, uint256 _farmedAmt) external onlyOwner nonReentrant returns (uint256, uint256) {
        ...
        // Check if there's any additional earned tokens harvested due to _wormWithdraw()
        uint256 earnedAmtBefore = IERC20(earnedAddress).balanceOf(address(this));

        // Convert Farmed to Want
        if (_farmedAmt > 0) {

            IUniRouter02(farmedRouterAddress).removeLiquidity(
                token0Address,
                token1Address,
                _farmedAmt,
                0,
        ...
        uint256 token0AmtBefore = IERC20(token0Address).balanceOf(address(this));
        uint256 token1AmtBefore = IERC20(token1Address).balanceOf(address(this));

        uint256 token0AmtAfter = 0;
        uint256 token1AmtAfter = 0;

        if (token0AmtBefore > 0 && token0Address == earnedAddress) {
          token0AmtAfter = token0AmtBefore.sub(earnedAmtBefore);
        } else {
          token0AmtAfter = token0AmtBefore;
        }

        if (token1AmtBefore > 0 && token1Address == earnedAddress) {
          token1AmtAfter = token1AmtBefore.sub(earnedAmtBefore);
        } else {
          token1AmtAfter = token1AmtBefore;
        }

        // Swap token0 and token1 back to want token
        if (token0AmtAfter > 0 && token0Address != wantAddress) {
          _safeSwapFarmed(
            token0AmtAfter,
            token0ToWantPath,
            address(this)
        ...
        if (token1AmtAfter > 0 && token1Address != wantAddress) {
          _safeSwapFarmed(
            token1AmtAfter,
            token1ToWantPath,
            address(this)
...
abstract contract BaseWormStrategyLP is BaseWormStrategy {
    ...
    function convertDustToTime() external nonReentrant whenNotPaused {
        ...
        // Converts token0 dust (if any) to want tokens
        uint256 token0Amt = IERC20(token0Address).balanceOf(address(this));
        if (token0Amt > 0 && token0Address != wantAddress) {
            // Swap all dust tokens to Want
            _safeSwapFarmed(
                token0Amt,
                token0ToWantPath,
                address(this)
        ...
        // Converts token1 dust (if any) to want tokens
        uint256 token1Amt = IERC20(token1Address).balanceOf(address(this));
        if (token1Amt > 0 && token1Address != wantAddress) {
...
// File: contracts/strategy/WormStrategyLP.sol
pragma solidity 0.6.12;

contract WormStrategyLPWmaticWeth is BaseWormStrategyLPSingle {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    ...
    constructor(
        ...
        address[] memory _wantToToken0Path,
        address[] memory _wantToToken1Path,
        address[] memory _token0ToWantPath,
        address[] memory _token1ToWantPath
        ...
        token0Address = IUniPair(farmedAddress).token0();
        token1Address = IUniPair(farmedAddress).token1();
        ...
        wantToToken0Path = _wantToToken0Path;
        wantToToken1Path = _wantToToken1Path;
        token0ToWantPath = _token0ToWantPath;
        token1ToWantPath = _token1ToWantPath;
    ...
    function _resetAllowances() internal override {
        ...
        // For withdraw(), convertDust
        IERC20(token0Address).safeApprove(farmedRouterAddress, uint256(0));
        ...
        // For withdraw(), convertDust
        IERC20(token1Address).safeApprove(farmedRouterAddress, uint256(0));
    ...
    function setFarmedRouterAddress(address _farmedRouterAddress) external onlyGov {
        farmedRouterAddress = _farmedRouterAddress;

        emit SetFarmedRouterAddress(
            _farmedRouterAddress
        );
    }
