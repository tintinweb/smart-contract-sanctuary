...
interface ICCPolyRenderer {
  ...
  function getTokenURI(TokenData calldata data) external view returns (string memory);
}

contract CryptoCardPOLY is ERC721Enumerable, ReentrancyGuard, Ownable {
  using Address for address payable;
  using Strings for uint256;
  using SafeMath for uint256;

  uint256 public mintFee = 115000000000000000000;
  bool public mintEnable = true;
  address private _renderAddress = 0x07c5e6779893eCAd045962a406d2F87A46Cf4385;
  ...
  function tokenURI(uint256 tokenId) override public view returns (string memory) {
    ...
    ICCPolyRenderer.TokenData memory data;
    data.seed = seed;
    data.tokenId = tokenId;
    data.from = _from[tokenId];
    data.header = _header[tokenId];
    data.footer = _footer[tokenId];
    data.balance = _CCBalance[tokenId];
    data.tokenOwnerAddress = ownerOf(tokenId);

    return ICCPolyRenderer(_renderAddress).getTokenURI(data);
  }

  // Public Owner

  function setRenderer(address rend) public onlyOwner {
    _renderAddress = rend;
  }
