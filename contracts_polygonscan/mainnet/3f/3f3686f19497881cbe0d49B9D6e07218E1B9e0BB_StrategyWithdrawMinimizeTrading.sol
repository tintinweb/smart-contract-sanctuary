...
contract StrategyWithdrawMinimizeTrading is Ownable, ReentrancyGuard, Strategy {
  ...
  constructor(IUniswapV2Router02 _router, address _banktoken) public {
  ...
  function setVault(address _ftoken, address _vault) onlyOwner external{
      vaults[_ftoken] = _vault;
  }
  ...
  function execute(
    ...
    IUniswapV2Pair lpToken = IUniswapV2Pair(factory.getPair(fToken, banktoken));
    // 2. Remove all liquidity back to banktoken and farming tokens.
    ...
    // 3. Convert farming tokens to banktoken.
    ...
    if (debt > balance) {
      // Convert some farming tokens to banktoken.
      uint remainingDebt = debt.sub(balance);
      router.swapTokensForExactTokens(remainingDebt, fToken.myBalance(), path, address(this), now);
    }
    // 4. Return banktoken back to the original caller.
    uint remainingBalance = banktoken.myBalance();
    banktoken.safeTransfer(msg.sender, remainingBalance);
