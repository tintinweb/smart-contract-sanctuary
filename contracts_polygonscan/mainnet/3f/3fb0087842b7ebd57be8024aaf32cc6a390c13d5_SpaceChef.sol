...
contract SpaceChef {
    ...
    // The  TOKEN!
    IERC20 public pspacebar;
    // rewards created per block.
    uint256 public rewardPerBlock;

    // Info.
    PoolInfo public poolInfo;
    // Info of each user that stakes pSpacebar tokens.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        uint256 stakedSupply = pspacebar.balanceOf(address(this));
    ...
    function updatePool() public {
        ...
        uint256 spacebarSupply = pspacebar.balanceOf(address(this));
        if (spacebarSupply == 0) {
    ...
    function deposit(uint256 _amount) public {
        ...
        updatePool();
        pspacebar.safeTransferFrom(address(msg.sender), address(this), _amount);
    ...
    // Withdraw pSpacebar tokens from SpaceChef.
    function withdraw(uint256 _amount) public {
        ...
        updatePool();
        pspacebar.safeTransfer(address(msg.sender), _amount);
    ...
    function emergencyWithdraw() public {
        UserInfo storage user = userInfo[msg.sender];
        pspacebar.safeTransfer(address(msg.sender), user.amount);
