...
// Dependency file: contracts/access/Whitelist.sol
...
// Root file: contracts/CowMarket.sol
...
// import "contracts/access/Whitelist.sol";

contract CowMarket is Whitelist {
    ...
    uint256 public tokenPrice = 55000000000000000000; // 0.03 ETH
    ...
    ICollectible public cows;

    event TokenPriceChanged(uint256 price);
    event MaxPurchaseChanged(uint256 value);
    event FundSet(address bankAccount);
    event RolledOver(SaleStatus status);

    constructor(ICollectible _cows, address _fund) {
        require(
            address(_cows) != address(0) &&
            _fund != address(0),
    ...
    receive() external payable {
        ...
        uint256 amount = deposit.div(tokenPrice);

        require(
            SaleStatus.DISABLED != saleStatus,
            "Collection: sale is not active"
        );
        
        if (SaleStatus.WHITELIST == saleStatus) {
            require(isMember(_msgSender()), "Whitelist: not in the list");
        }
        ...
        require(
            cows.totalSupply().add(amount) <= maxTokens,
        ...
        cows.mint(msg.sender, amount);
    }

    function buy(uint _amount) external payable {
        require(SaleStatus.DISABLED != saleStatus, "Collection: sale is not active");
        ...
        require(
            cows.totalSupply().add(_amount) <= maxTokens,
            "Collection: purchase would exceed max supply of Tokens"
        );
