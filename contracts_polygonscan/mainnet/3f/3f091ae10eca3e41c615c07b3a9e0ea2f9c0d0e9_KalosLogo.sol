...
contract KalosLogo is ERC721Enumerable, ERC2981Royalties, Ownable {
	...
	// Operators of the token
	mapping(address => bool) public operators;

	event LogoMinted(address indexed account, uint256 amount);
	event TransferBatch(address indexed from, address[] addrs, uint256[] ids);
	event Burned(uint256[] tokenIds);

	constructor(uint256 _maxSupply, address[] memory _operators) ERC721("Kalos Logo", "KL") {
		// Add operators
	...
	/**
	 * @notice Mint Kalos logos
	 *
	 * @param _amount the amount of Kalos logos to mint
	 */
	function mint(uint256 _amount) public onlyOwner {
		require(_tokenIds.current() + _amount <= maxSupply, "Amount to mint exceeds max supply");
		...
		emit LogoMinted(msg.sender, _amount);
	}

	/**
	 * @param _ids The _tokenIds to burn for morphing
	 */
	function burn(uint256[] memory _ids) public onlyOperator {
	 ...
	 *
	 * @param _operators The addresses allowed to become operators
	 */
	function addOperator(address[] memory _operators) external onlyOwner {
		for (uint256 i = 0; i < _operators.length; i++) {
			operators[_operators[i]] = true;
		}
	}
	...
	modifier onlyOperator() {
		require(operators[_msgSender()], "Unauthorized, not an operator");
		_;
	}
