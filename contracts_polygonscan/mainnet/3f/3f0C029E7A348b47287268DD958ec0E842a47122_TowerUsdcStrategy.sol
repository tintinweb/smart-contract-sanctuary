...
contract TowerUsdcStrategy is BaseStrategyMasterChef {
    ...
    address private constant TOWER = 0x8201532917e55bA29674Ef4E88FFe0b775f1BaE8;
    address private constant IVORY_USDC_LP = 0x10995233Ef7b3abd1a2706a86FFeA456ebae8796;
    address private constant TOWER_USDC_LP = 0xD70f14f13ef3590e537bBd225754248965A3593c;

    address private constant CHEF = 0x4696B1A198407BFb8bB8dd59030Bf30FaC258f1D; //different chef than ivory contracts
        ...
        )
    {
        ...
        IERC20(TOWER).safeApprove(FIREBIRD_ROUTER, uint256(-1));
    ...
    function harvest() public override onlyHumanOrWhitelisted nonReentrant {
        ...
        if (remainingHarvested > 0) {
            swapIvoryToUsdc(remainingHarvested);
        }

        //Swap 1/2 of USDC for Tower
        uint256 _usdc = IERC20(USDC).balanceOf(address(this));
        if (_usdc > 0) {
            swapUsdcToTower(_usdc.div(2));
        }

        //Add liquidity
        uint256 aBalance = IERC20(TOWER).balanceOf(address(this));
    ...
    function swapUsdcToTower(uint256 token_in) internal {
        address[] memory path = new address[](1);
        path[0] = TOWER_USDC_LP;

        uint8[] memory dexIds = new uint8[](1);
        dexIds[0] = 0;

        IFireBirdRouter(FIREBIRD_ROUTER).swapExactTokensForTokens(USDC, TOWER, token_in, 1, path, dexIds, address(this), now + 60);
    }
}