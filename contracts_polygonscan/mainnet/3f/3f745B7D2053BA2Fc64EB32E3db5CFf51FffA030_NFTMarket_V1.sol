...
contract NFTMarket_V1 is Initializable, Payments, ReentrancyGuardUpgradeable, OwnableUpgradeable,AccessControlUpgradeable{
    ...
    bytes32 public constant TOKEN_LIST_ROLE = "TOKEN_LIST_ROLE";

    bytes32 public constant TOKEN_COLLECTOR_ROLE = "TOKEN_COLLECTOR_ROLE";

    
    function initializeNFTMarket_V1(address _iMarketSettings, address _royalty) initializer  public {
        require(
        ...
        require(
            _royalty != address(0),
            "constructor::Cannot have null address for _royalty"
        );
        ...
        _grantRole(TOKEN_LIST_ROLE, msg.sender);
    }
    ...
    modifier onlyTokenCreator(uint256 itemId) {
        require(
    ...
    //mapping to store NFT's with IDs which are confirmed by admin account to be list on exchange
    ...
    function setIERC721CreatorRoyalty(address _address) public onlyOwner {
        ...
        iRoyalty = IRoyalty(_address);
    }


    // 
    function confirmTokenByAdminOrModerator(uint256 _tokenId, address _originContract) external{
       require(iMarketplaceSettings.checkModeratorRole(msg.sender)!= false || msg.sender == owner(),"");

         IERC721Upgradeable erc721 = IERC721Upgradeable(_originContract);
         address tokenOwner = erc721.ownerOf(_tokenId);
         require(tokenOwner != address(0), "confirmTokenByAdmin: token not exist");
         TokenConfirmedByAdmin[_tokenId]  = true;    
    }
    //check if token is confirmed by moderator or admin to be list on exchange
    function isTokenConfirmedByAdmin(uint256 _tokenId) public view returns(bool){
        return TokenConfirmedByAdmin[_tokenId]; 
    }
    
    function setIroyaltyAddressNFT(address _royalty)public onlyOwner{
        iRoyalty = IRoyalty(_royalty);
    }
    ...
    function createMarketItem(
        ...
        require(
        hasRole(TOKEN_LIST_ROLE, msg.sender),
        "TOKEN_LIST_ROLE::Must have TOKEN_LIST_ROLE to list NFT on exchange"
        );
    ...
    {   
        ...
        address tokenCreator = idToMarketItem[itemId].creator;
        ...
        Payments.payout(
            price,
            !iMarketplaceSettings.hasERC721TokenSold(nftAddress, tokenId),
        ...
        // Set token as sold
        iMarketplaceSettings.markERC721Token(nftAddress, tokenId, true);
        emit ISPRimarySale(!iMarketplaceSettings.hasERC721TokenSold(nftAddress, tokenId));
        emit ProductSold(
    ...
    {
        require(hasRole(TOKEN_LIST_ROLE, msg.sender),
            "TOKEN_LIST_ROLE:Must have TOKEN_LIST_ROLE to list NFT on exchange"
        );
        uint256 tokenId = idToMarketItem[itemId].tokenId;
    ...
    /////////////////////////////////////////////////////////////////////////
    /**
     * @dev Gets the sale price of the item
     * @param tokenId uint256 ID of the token
     * @return uint256 sale price of the item of the token
     */

    function tokenPrice(uint256 tokenId, address _nftContract )
        external
        view
        returns (uint256)
    {
        ...
        uint price;
        address seller;
        (itemId,,seller,price,,) =  fetchTokeByID(tokenId);
        //checking if the market place holds token for sale
        IERC721Upgradeable erc721 = IERC721Upgradeable(_nftContract);
        require(
            erc721.ownerOf(tokenId) == address(this),
            "Token is not listed on market place for sale or it is sold"
        );
        require(idToMarketItem[itemId].seller != address(0), "token is not listed for sale");
        require(idToMarketItem[itemId].price == price,"tokenPrice: internal error, price not equal");
     ...
     */
    function tokenPriceFeeIncluded(uint256 tokenId, address _nftContract)
        public
        view
        returns (uint256)
    {
        ...
        require(idToMarketItem[itemId].seller != address(0), "seller: token is not listed for sale");
        require(idToMarketItem[itemId].price == price,"tokenPrice: internal error, price not equal");

        if (idToMarketItem[itemId].seller != address(0) && idToMarketItem[itemId].price == price) {
            return
                idToMarketItem[itemId].price.add(
    ...
    function fetchTokeByID(uint _tokenID) public view returns (
        ...
        function fetchItemId(uint256 _itemId)
        public
        view
        returns (MarketItem[] memory ItemId)
    {
        uint256 totalItemCount = _itemIds.current();
        uint256 itemCount = 0;
        uint256 currentIndex = 0;
        ...
        for (uint256 i = 0; i < totalItemCount; i++) {
            if (idToMarketItem[i + 1].itemId == _itemId) {
                itemCount += 1;
            }
        }
