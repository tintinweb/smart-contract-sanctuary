...
contract Bot is FlashLoanReceiverBase {
    ...
    function executeOperation(
        ...
        (P memory _params) = abi.decode(params, (P));
        uint256 deadline = block.timestamp + 60 * 10;

        for (uint256 i = 0; i < _params.dexes.length; i++) {
            _swap(
                _params.types[i],
                _params.dexes[i],
                _params.paths[i],
                _params.pairs[i],
                _params.inputAmounts[i],
                _params.amountsOutMin[i],
                deadline
            );
        }
    ...
    function _swap(
        ...
        if (rtype == 1) {
            ...
            uint256 returnAmount = IDODOProxy(router).dodoSwapV2TokenToToken(
                tokenIn,
                tokenOut,
                amountIn,
                amountOutMin,
    ...
    function multiSwap(
        address asset_to_arb,
        uint256 amount_to_arb,
        bytes memory params
    ) public _ownerOnly {
        address receiverAddress = address(this);

        address[] memory assets = new address[](1);
        assets[0] = asset_to_arb;

        uint256[] memory amounts = new uint256[](1);
        amounts[0] = amount_to_arb;
