...
contract Masterchef is Ownable, ReentrancyGuard {
    ...
    // Max lockup block length 15 days.
    uint256 public constant MAXIMUM_LOCKUP_BLOCKS_LENGTH = 648000;
    // Max unlock block length 15 days.
    uint256 public constant MAXIMUM_UNLOCK_BLOCKS_LENGTH = 648000;
    // Lock up block length: 6 days
    uint256 public lockupBlockLength = 259200;
    // Unlock block length: 1 days
    uint256 public unlockBlockLength = 43200;
    // Burn address
    address public constant BURN_ADDRESS = 0x000000000000000000000000000000000000dEaD;
    // Info of each pool.
    ...
    // Max Cougar Supply
    uint256 public CougarMaxSupply = 4000000e18;
    ...
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        ...
        if (_amount > 0) {
            ...
            if (pool.depositFeeBP > 0) {
                ...
                pool.lpToken.safeTransfer(syrupAddress, depositFee.div(2));
    ...
    function payOrLockupPendingCougar(uint256 _pid) internal {
        ...
        if (canHarvest()) {
            if (pending > 0 || user.rewardLockedUp > 0) {
                ...
                safeCougarTransfer(BURN_ADDRESS, harvestFeeAmount);
            }
        } else if (pending > 0) {
    ...
    function canHarvest() public view returns (bool) {
        ...
        uint256 modulusBlocks = block.number.sub(startBlock).mod(
            lockupBlockLength.add(unlockBlockLength)
        );
        // Harvest available only for unlockBlockLength after lockupBlockLength from startBlock, and then repeat same process.
        if (modulusBlocks < lockupBlockLength) {
            return false;
        }
        return true;
    }
    ...
    // Update Syrup Address
    function setSyrupAddress(address _syrupAddress) public {
        require(msg.sender == syrupAddress, "setSyrupAddress: FORBIDDEN");
        require(_syrupAddress != address(0), "setSyrupAddress: ZERO");
        syrupAddress = _syrupAddress;
    }
    ...
    // Update the harvest fee by the owner
    function updateLockupBlockLength(uint16 _value) public onlyOwner {
        require(_value <= MAXIMUM_LOCKUP_BLOCKS_LENGTH,"Masterchef::updateLockupBlockLength: lockup block length must not exceed the MAXIMUM_LOCKUP_BLOCKS_LENGTH.");
        lockupBlockLength = _value;
    }

    // Update the harvest fee by the owner
    function updateUnlockBlockLength(uint16 _value) public onlyOwner {
        require(_value <= MAXIMUM_UNLOCK_BLOCKS_LENGTH,"Masterchef::updateUnlockBlockLength: lockup block length must not exceed the MAXIMUM_UNLOCK_BLOCKS_LENGTH.");
        unlockBlockLength = _value;
    }
