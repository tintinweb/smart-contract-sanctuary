...
contract Swapper is ISwapper, Ownable {
    ...
    /**
        @dev Relies on the Uniswap SafeMath.sol file which is by default set to 0.6.4 version of the compiler
        in the uniswap npm package. Change that to ^0.8.0 to make it work with the rest of this contract.
     */
    function sufficientLiquidity(IUniswapV2Router02 router, address tokenA, address tokenB, uint256 amountOut) internal view returns(bool) {
        (address token0, address token1) = sortTokens(tokenA, tokenB);
        IUniswapV2Factory factory = IUniswapV2Factory(router.factory());
        IUniswapV2Pair pair = IUniswapV2Pair(factory.getPair(token0, token1));
        (uint256 reserve0, uint256 reserve1,) = pair.getReserves();
        uint256 relevantReserve = tokenA == token0 ? reserve0 : reserve1;
        return relevantReserve >= amountOut;
    }
    ...
    /**
        @dev Function from UniswapV2Library.sol hardcoded here to avoid the broken hacks 
        in the original UniswapV2Library contract (specifically the pairFor function).
