...
contract Marketplace is Context, Ownable, Initializable, IERC721Receiver {
    ...
    /**
     * @dev Sell ERC721 token in store.
    ...
    function sell(
        ...
        emit Sell(_msgSender(), _tokenAddress, _tokenId, _price);
    }

    /**
     * @dev Cancel listing ERC721 token in store.
     * @param _tokenAddress - address of the ERC721 token
     * @param _tokenId - uint256 of the token id
     */
    ...
    function placeBid(
        ...
        if (_lastBider != address(0)) {
            acceptedToken.safeTransferFrom(address(this), _lastBider, _lastBidPrice);
        }
    ...
    event Sell(address indexed _user, address _tokenAddress, uint256 _tokenId, uint256 _price);
