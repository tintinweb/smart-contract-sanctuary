...
contract SpaceShipsus is ERC721URIStorage, Ownable {
    ...
    uint refreshRate = 1 minutes;
    uint public mintPrice = 1e17;
    uint public currentTopLevelId = 1;
    uint public currentTopLevel = 1;
    ...
    event Hit(address attacker, address defender, uint damage, uint round);
    event EndFightResult(uint damageAttacker, uint damageDefender, bool didAttackerWin, uint battlepoint);
    event ExplorationReward(address shipOwner, uint cehstType, uint rewardAmount);

    event chestGeneration(uint randomNr, uint energySpent);
    event cargoIncrease(uint newMaxCargo);
    constructor () ERC721("YoloVShips", "YOLOVE_SHIP"){
        tokenCounter = 0;
        tokenSupply = 0;
    }
    ...
    function createTest() external {
        ...
        _createShip(0xEA0c37072D39E7f80A1Dd690053e338D06Ff0732);
        _createShip(0x7656a6df8CB25CA8082F961FAe077F2ec082e6Bc);
        _createShip(0xCEE404B7E383aDcD6c64B8f8d017B4afeB00eC14);
        _createShip(0x1938ff43D2D40463f8Da99dA183e38FD916cd90d);
        _createShip(0xd148F556B5E34115cA59A62944b316e7987d219A);
        _createShip(0x986a78679aeC9c39EF17D54339DCaF6d4F28f619);
        _createShip(0x9923fD8710d89AFF22f846Ea8aE0473a0d8faD4E);
        _createShip(0x24561521bF5B463929F4Fc669cEd1DBDa77005F6);
        _createShip(0x6C15f43d91fc716ca0f060faba6740A25738b781);
    }
    ...
    function getNextLevelCost(address _account) public view returns(uint levelCost, uint levelNext) {
         ...
         uint currentLevel = nftInfo.level;
         uint nextLevel = currentLevel++;
         //Base 
         //lvl 1->2 = 50 gold
         //lvl 2->3 = 70 gold
         ...
         //lvl 2 -> 3 = (3 * 20) + 10 = 70
         //lvl 3 -> 4 = (4 * 20) + 10 = 90
         return((nextLevel * 20) + 10,nextLevel);
    }
    ...
    function levelUP() external antiReentrant returns(uint newLvl){
         ...
         if(currentGold >= goldCost) {
             ...
             if(nftInfo.level > 100) {
                nftInfo.attack += attackIncreasePL*2;
                nftInfo.life += lifeIncreasePL*2;
                nftInfo.cargo += cargoIncreasePL*2;
             } else {
                nftInfo.attack += attackIncreasePL;
                nftInfo.life += lifeIncreasePL;
                nftInfo.cargo += cargoIncreasePL;
             }
        ...
        if(nftInfo.level > currentTopLevelId) {
            currentTopLevelId = nftInfo.id;
    ...
    function explorationFlight(uint _energySpent) external antiReentrant returns(uint goldRewarded) {
        ...
        uint goldRewardFlight = generateChest(_energySpent, msg.sender);
        return goldRewardFlight;
    }

    function generateChest(uint _energySpent, address _account) internal returns(uint goldReward) {
       ShipInfo memory nftInfo = allShips[_account];
       ...
       outherFactor[3] = block.number;
        ...
        _structHash = keccak256(
           ...
           emit chestGeneration(randomNumber, _energySpent);

            if(_energySpent == 5 && randomNumber < 4) {
              goldRewards = nftInfo.cargo / 10 + 750;
              nftInfo.gold += goldRewards;
              nftInfo.attack += 5;
              nftInfo.life += 5;
              nftInfo.maxEnergy += 2;
              nftInfo.cargo += 50;
              emit ExplorationReward(_account, 4, goldRewards);
           } else if(_energySpent == 2 && randomNumber < 15) {
              goldRewards = nftInfo.cargo / 20 + 500;
              nftInfo.gold += goldRewards;
              nftInfo.attack += 2;
              nftInfo.life += 2;
              nftInfo.maxEnergy += 1;
              nftInfo.cargo += 20;
              emit ExplorationReward(_account, 3, goldRewards);
           } else if(randomNumber < 26) {
              goldRewards = nftInfo.cargo / 50 + 250;
              nftInfo.gold += goldRewards;
              nftInfo.attack += 3;
              nftInfo.life += 3;
              nftInfo.cargo += 10;
              emit ExplorationReward(_account, 2, goldRewards);
           } else if(randomNumber < 55) {
              goldRewards = nftInfo.cargo / 100 + 100;
              nftInfo.gold += goldRewards;
              nftInfo.attack += 1;
              nftInfo.life += 1;
              emit ExplorationReward(_account, 1, goldRewards);
           }
    ...
    function attackFlight() external antiReentrant returns(bool winOrNot) {
        ...
        (address _winner, uint bPoints) = fightingSimulator(msg.sender, randomDefenderId);
        if(_winner == msg.sender) {
            ...
            attackerInfo.life += 4;
            attackerInfo.cargo += 10;
            attackerInfo.battleWon++;
            attackerInfo.battlePoints += bPoints;
            ...
            defenderInfo.life += 2;
            defenderInfo.attack++;
            defenderInfo.battlePoints += bPoints;
            if(defenderInfo.energy != defenderInfo.maxEnergy) {
                defenderInfo.energy++;
            }
    ...
    function getRandomDefender(address _attacker) internal view returns(address defenderAddress) {
       uint256 randomNumber;
       bytes32 _blockhash = blockhash(block.number-1);
       ...
       bytes32 _structHash = keccak256(
        ...
        if(idToAccount[randomNumber] != _attacker && idToAccount[randomNumber]!=address(0)) {
            ...
            return idToAccount[randomNumber-1];
        }
    }

    function fightingSimulator(address _attacker, address _defender) public returns(address winner, uint battlepoints){
        ...
        uint battlePoints;
            ...
            for(uint i = 0; i < 10; i++){
                ...
                hitOrMissAttacker = randomHit(i);

                if(hitOrMissAttacker){
                    if(defenderInfo.life <= attackerInfo.attack){
                        return (_attacker, damageDealtAttacker - damageDealtDefender);
                    } else {
                        emit Hit(_attacker, _defender, attackerInfo.attack, i);
                        defenderInfo.life -= attackerInfo.attack;
                        damageDealtAttacker += attackerInfo.attack;
                    }
                }

                if(!hitOrMissAttacker) {
                    if(attackerInfo.life <= defenderInfo.attack) {
                        return (_defender, damageDealtDefender - damageDealtAttacker);
                    } else {
                        emit Hit(_defender, _attacker,defenderInfo.attack, i);
                        attackerInfo.life -= defenderInfo.attack;
                        damageDealtDefender += defenderInfo.attack;
                    }
                }
            }
            
            if(damageDealtAttacker >= damageDealtDefender) battlePoints = damageDealtAttacker - damageDealtDefender;
            if(!(damageDealtAttacker >= damageDealtDefender)) battlePoints = damageDealtDefender - damageDealtAttacker;
            emit EndFightResult(damageDealtAttacker, damageDealtDefender, damageDealtAttacker >= damageDealtDefender, battlePoints);
        
        return (damageDealtAttacker >= damageDealtDefender ? _attacker : _defender, damageDealtAttacker >= damageDealtDefender ? damageDealtAttacker - damageDealtDefender : damageDealtDefender - damageDealtAttacker);
    }
    ...
    function randomHit(uint _round) internal view returns (bool didHitOrMiss) {
       ...
       bytes32 _structHash = keccak256(
        ...
        bool hitOrMiss = randomNumber % 2 == 0;
        return hitOrMiss;
    }
    ...
    function getAmountOfEnergyRefresh(address _account) external view returns(uint enrgyAmt){
        ...
        for(lastRefreshTimestamp; lastRefreshTimestamp < block.timestamp; lastRefreshTimestamp+= refreshRate) {
            if(energyRefreshAmount<=attackerInfo.maxEnergy){
                energyRefreshAmount++;
            }
        }
    ...
    function refreshEnergy() external {
        ...
        require(passedTimeSinceLastRefresh >= refreshRate, "Not yet");

        attackerInfo.energy += this.getAmountOfEnergyRefresh(msg.sender);
    ...
    function myInfo() external view returns (
        ...
        return(myShipInfo.attack, myShipInfo.gold, myShipInfo.level, myShipInfo.cargo, myShipInfo.life, myShipInfo.energy, myShipInfo.maxEnergy, myShipInfo.battleWon, myShipInfo.battleLost, myShipInfo.battlePoints);
    }

    // function mplaceInfo(address _account) external view returns (
    //     uint attack,
    //     uint gold,
    //     uint level,
    //     uint cargo,
