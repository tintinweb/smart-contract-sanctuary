...
contract PirateAirdrop {

  function pirateAirdropERC20(IERC20 _token, address[] calldata _to, uint256[] calldata _value) public {
  ...
  function pirateAirdropERC721(IERC721 _token, address[] calldata _to, uint256[] calldata _id) public {
  ...
  function pirateAirdropERC1155(IERC1155 _token, address[] calldata _to, uint256[] calldata _id, uint256[] calldata _amount) public {
    require(_to.length == _id.length, "Receivers and IDs are different length");
    for (uint256 i = 0; i < _to.length; i++) {
