...
/******************************************/
/*** KAjcHfmu6p1IztqCkaQoXDeElnPWi2Rb
...
contract ExtinctionPool  is Ownable{
    ...
    struct UserInfo
    {
        uint256 amount;                     // How many eMYT tokens the user has provided.
    ...
    struct PoolInfo
    {
        ...
        uint256 accRewardPerShare;          // Accumulated reward per share, times i1exx. See below.
    }

    IERC20 public immutable eMYT;
    ...
    uint256 public i1exx;
    mapping (address => UserInfo) public userInfo;     // Info of each user that stakes eMYT tokens.

    event Withdraw(address indexed user, uint256 amount);
    event Deposit(address indexed user, uint256 amount);
    event Burn(uint256 amount);

    constructor(IERC20 _eMYT, IERC20 _REWARD, uint256 _rewardPerBlock, uint256 _startBlock, uint256 _endBlock, uint256 _lockBlock) public {
        require(address(_eMYT) != address(0), "_eMYT address not set!");
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        uint256 mytSupply = eMYT.balanceOf(address(this));
        if (block.number > poolInfo.lastRewardBlock && mytSupply != 0) {
            uint256 multiplier = getMultiplier(poolInfo.lastRewardBlock, block.number);
            uint256 tokenReward = multiplier.mul(rewardPerBlock);
            accRewardPerShare = accRewardPerShare.add(tokenReward.mul(i1exx).div(mytSupply));
        }
        return user.amount.mul(accRewardPerShare).div(i1exx).sub(user.rewardDebt);
    }
    ...
    /**
     * @dev  function to set 1e12.
     * @param _i1exx new 1exx
     * @return bool(true)
     */
    function setI1exx(uint256 _i1exx) public  onlyOwner returns (bool) {
        i1exx = _i1exx;
        return true;
    }

    /**
     * @dev  function to set rewards to totalPoolAddr.
     * @param _sender Address of a specific user.
     * @param blocksReward Address of a input a lot of blocks reward .
     * @return bool(true)
     */
    function addBlocksReward(address _sender,uint256 blocksReward) public  onlyOwner returns (bool) {
        ...
        allReward=allReward.add(blocksReward);
    return true;
    }
    ...
    /**
     * @dev View function to get allReward and reward of not claim.
     * @return allReward and  remained.
     */
    function getRewardInfo() external view returns (uint256 ,uint256 ) {


        uint256 _remained=REWARD.balanceOf(totalPoolAddr);
        return (allReward,_remained);
    }
    ...
    function updatePool() public {
        ...
        poolInfo.accRewardPerShare = poolInfo.accRewardPerShare.add(tokenReward.mul(i1exx).div(mytSupply));
        poolInfo.lastRewardBlock = block.number;
    }
    /**
     * @dev Deposit eMYT tokens to the Extinction Pool for rewards allocation and/or withdraw outstanding rewards.
     * @param _amount Amount of eMYT tokens to deposit.
     */
    function transact(uint256 _amount) public {
        UserInfo storage user = userInfo[msg.sender];
        updatePool();
        if (user.amount > 0) {
            uint256 tempRewardDebt = user.amount.mul(poolInfo.accRewardPerShare).div(i1exx);
        ...
        if (block.number < lockBlock && _amount != 0) {
            eMYT.safeTransferFrom(address(msg.sender), address(this), _amount);
            user.amount = user.amount.add(_amount);
            user.rewardDebt = user.amount.mul(poolInfo.accRewardPerShare).div(i1exx);
            emit Deposit(msg.sender, _amount);
        }
        if (block.number >= endBlock) {
            eMYT.safeTransfer(burnAddress, user.amount);
            user.amount = 0;
            user.rewardDebt = user.amount.mul(poolInfo.accRewardPerShare).div(i1exx);
