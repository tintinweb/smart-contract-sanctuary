...
contract StrategyRewardPoolPolygonLP is StratManager, FeeManager {
    ...
    address constant public _for = address(0x546b4c391520E6652897c65153074088BFC0A909);
    ...
    constructor(
        ...
        } else if (lpToken0 == _for) {
            outputToLp0Route = [output, _for];//QUICK-FOR
        ...
        } else if (lpToken1 == _for) {
            outputToLp1Route = [output, _for];//QUICK-FOR
    ...
    function chargeFees() internal {
        uint256 toMatic = IERC20(output).balanceOf(address(this)).mul(5).div(1000);
        IUniswapRouterETH(unirouter).swapExactTokensForTokens(toMatic, 0, outputToMaticRoute, address(this), now);

        uint256 strategistFee = IERC20(matic).balanceOf(address(this));
