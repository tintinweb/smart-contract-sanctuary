...
// File: contracts/GetMakertsData.sol
...
contract GetMakerts is Ownable {
    ...
    struct PoolInfo {
        ...
        BorrowBalance supplyAndBorrowBalance;
    ...
    address public COMPTROLLER=0x4613aB39074D4Bb949C6de7c265922B3876d92Ec;
    address public ORACLE=0xe61d1704687cd8467C3513A8f3BE1a0fE5646F78;
    ...
    function getUnderlyingTokenAddress(address _pToken) public view returns (address){
        if (_pToken == 0xAC105EFc8250B4b2f3a849f9586D0c2067a87535) {
            return address(0);
        }else{
    ...
    function getSupplyAndBorrowBalance(address _pToken, address _account) public view returns (uint256, uint256, uint256,uint256){
        return PToken(_pToken).getAccountSnapshot(_account);
    }
    ...
    function getSymbol(address _tokenAddress) public view returns (string memory) {
        return _tokenAddress == address(0)? 'MATIC' : UERC20(_tokenAddress).symbol();
    }
    ...
    function getSupplyApys() public view returns (uint256[34] memory){
        address[] memory _PTokens = getAllPTokens();
        uint256[34] memory data;
        for(uint256 i = 0 ; i < 34; i++){
            data[i] = getSupplyApy(_PTokens[i]);
        }
        return data;
    }

    function getBorrowApys() public view returns (uint256[34] memory){
        address[] memory _PTokens = getAllPTokens();
        uint256[34] memory data;
        for(uint256 i = 0 ; i < 34; i++){
            data[i] = getBorrowApy(_PTokens[i]);
        }
        return data;
    }


    function getMarketDetail() public view returns (uint256 [34][2] memory){
        uint256 [34][2] memory data;
        data[0] = getSupplyApys();
        data[1] = getBorrowApys();
        return data;
