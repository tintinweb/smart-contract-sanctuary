...
// File contracts/StrategyV2_Polycrystal.sol
...
contract StrategyV2_Polycrystal is Ownable, ReentrancyGuard, Pausable {
    ...
    uint256 public feeExt;
    address public wantAddress;
    address public token0Address;
    address public token1Address;
    address public earnedAddress;
    address public uniRouterAddress = 0xC0788A3aD43d79aa53B09c2EaCc313A787d1d607; // uniswap, pancakeswap etc
    address public buybackRouterAddress = 0xC0788A3aD43d79aa53B09c2EaCc313A787d1d607; // uniswap, pancakeswap etc
    uint256 public routerDeadlineDuration = 300;  // Set on global level, could be passed to functions via arguments

    address public wmaticAddress = 0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270; // should be WMATIC or USDC
    ...
    constructor(
        ...
        require (feeExt >= 0 && feeExt < 500,"depositFee EXT");
        ...
        if (isAutoComp) {
            ...
            farmContractAddress = 0xeBCC84D2A73f0c9E23066089C6C24F4629Ef1e6d;
            pid = _pid;
            feeExt = _feeExt;
            //earnedAddress = _earnedAddress
            earnedAddress = 0x76bF0C28e604CC3fE9967c83b3C3F31c213cfE64;

            earnedToNATIVEPath = [earnedAddress, wmaticAddress, NATIVEAddress];
            if (wmaticAddress == earnedAddress) {
                earnedToNATIVEPath = [wmaticAddress, NATIVEAddress];
            }
            ...
            earnedToWantPath = [earnedAddress, wmaticAddress, wantAddress];
        ...
        returns (uint256)
    {
        ...
        if(feeExt > 0){
          uint256 depextFee = _wantAmt.mul(feeExt).div(10000);
          depextFee = depextFee.add(10);
          _wantAmt = _wantAmt.sub(depextFee);
        }
    ...
    function _farm() internal {
        ...
        if(feeExt > 0){
          uint256 depextFee = wantAmt.mul(feeExt).div(10000);
          depextFee = depextFee.add(10);
          wantAmt = wantAmt.sub(depextFee);
        }
    ...
    function setNewFee(uint256 _newFee) public onlyAllowGov {
        feeExt = _newFee;
    }
    ...
    function workerCompound() public view returns (uint256){
        uint256 CrystalComp;
        CrystalComp = IXswapFarm(farmContractAddress).pendingCrystal(pid, address(this));
        uint256 buyBackAmt = CrystalComp.mul(buyBackRate).div(buyBackRateMax);
