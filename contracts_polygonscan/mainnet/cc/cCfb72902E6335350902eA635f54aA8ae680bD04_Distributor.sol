...
contract Distributor is WarpBase {
    ...
    mapping(address => Adjust) public adjustments;

    EnumerableSetUpgradeable.AddressSet private recipients;
    mapping(address => Info) public recInfo;
    ...
    /** ===== Initialize ===== */
    function initialize(address _treasury, address _WARP) public initializer {
    ...
    function distribute() external returns (bool) {
        require(recipients.contains(msg.sender), 'Not a distributor');

        address recipient = msg.sender;
        if (recInfo[recipient].nextEpochTime <= block.timestamp) {
            recInfo[recipient].nextEpochTime.add(recInfo[recipient].epochLength);
            ITreasury(treasury).mintRewards(recipient, nextRewardAt(recInfo[recipient].rewardRate));
            adjust(recipient);
        }
     ...
     */
    function adjust(address _index) internal {
        Adjust memory adjustment = adjustments[_index];
        if (adjustment.rate != 0) {
            if (adjustment.add) {
                // if rate should increase
                recInfo[_index].rewardRate = recInfo[_index].rewardRate.add(adjustment.rate);
                if (recInfo[_index].rewardRate >= adjustment.target) {
                    // if target met
                    adjustments[_index].rate = 0;
                }
            } else {
                // if rate should decrease
                recInfo[_index].rewardRate = recInfo[_index].rewardRate.sub(adjustment.rate);
                if (recInfo[_index].rewardRate <= adjustment.target) {
                    // if target met
                    adjustments[_index].rate = 0;
                }
    ...
    function nextRewardAt(uint256 _rate) public view returns (uint256) {
        return IERC20Upgradeable(WARP).totalSupply().mul(_rate).div(1000000);
    }
    ...
    function nextRewardFor(address _recipient) public view returns (uint256) {
        return nextRewardAt(recInfo[_recipient].rewardRate);
    }
    ...
    /**
        ...
        @param _recipient address
        @param _info Info memory
     */
    function addRecipient(address _recipient, Info memory _info) external onlyOwner {
        require(!recipients.contains(_recipient), 'Recipient already included, use update');
        require(_recipient != address(0));
        require(_info.rewardRate != 0, 'Reward Rate must be > 0');

        recipients.add(_recipient);
        recInfo[_recipient] = _info;
    }
    ...
    function removeRecipient(address _recipient) external onlyOwner {
        require(recipients.contains(_recipient), 'Recipient is not included, use add');

        recipients.remove(_recipient);
        delete recInfo[_recipient];
    }

    /**
        @notice update recipient for distributions
        @param _recipient address
        @param _info Info memory
     */
    function updateRecipient(address _recipient, Info memory _info) external onlyOwner {
        require(recipients.contains(_recipient), 'Recipient is not included, use add');
     ...
     */
    function updateRecipientReward(address _recipient, uint256 _rewardRate) external onlyOwner {
        require(recipients.contains(_recipient), 'Recipient is not included, use add');

        recInfo[_recipient].rewardRate = _rewardRate;
    }

    /**
        @notice set adjustment info for a collector's reward rate
        @param _index address
        @param _add bool
        @param _rate uint
        @param _target uint
     */
