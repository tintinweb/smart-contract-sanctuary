...
abstract contract StrategyFarmTwoAssets is BaseStrategyMasterChef {
        ...
        )
    {
        ...
        usdc_mai_path = new address[](2);
        usdc_mai_path[0] = usdc;
        usdc_mai_path[1] = mai;
    }
    ...
    function harvest() public override {
        ...
        if (_dinoBalance > harvestCutoff){
            ...
            _swapUniswapWithPathQuick(usdc_mai_path, _usdcBalancePost.div(2));

            // Add liquidity
            uint256 _mai = IERC20(mai).balanceOf(address(this));
            uint256 _usdc = IERC20(usdc).balanceOf(address(this));
            if (_mai > 0 && _usdc > 0) {
                IERC20(mai).safeApprove(quickRouter, 0);
                IERC20(mai).safeApprove(quickRouter, _mai);
                IERC20(usdc).safeApprove(quickRouter, 0);
                IERC20(usdc).safeApprove(quickRouter, _usdc);

                IUniswapRouterV2(quickRouter).addLiquidity(
                    mai,
                    usdc,
                    _mai,
                    _usdc,
...
contract StrategyTwoAssets is StrategyFarmTwoAssets {
    ...
    address public MAI_USDC_LP = 0x160532D2536175d65C03B97b0630A9802c274daD;
