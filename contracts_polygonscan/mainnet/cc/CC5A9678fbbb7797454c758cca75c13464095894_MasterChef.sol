...
interface IMigratorChef {
    // Perform LP token migration from legacy MacaronSwap to MacaronSwapV2.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to MacaronSwap LP tokens.
    // MacaronSwapV2 must mint EXACTLY the same amount of MacaronSwapV2 LP tokens or
    // else something bad will happen. Traditional MacaronSwap does not
...
// will be transferred to a governance smart contract once MCRN is sufficiently
...
contract MasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of MCRNs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accMacaronPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accMacaronPerShare` (and `lastRewardBlock`) gets updated.
    ...
    MacaronToken public macaron;
    // The CHOCO TOKEN!
    ChocoFall public choco;
    // Dev address.
    address public devaddr;
    // MCRN tokens created per block.
    uint256 public macaronPerBlock;
    // Bonus muliplier for early macaron makers.
    ...
    function add(uint256 _allocPoint, IBEP20 _lpToken, bool _withUpdate, bool _isCLP, ICakeStrategy _cakeStrategy, IBEP20 _syrupToken) external onlyOwner {
        ...
        if(_isCLP && address(_cakeStrategy) != address(0)) {
            IBEP20(_lpToken).approve(address(_cakeStrategy), type(uint256).max);
        }
    ...
    function migrate(uint256 _pid) external validatePoolByPid(_pid) {
        ...
        if(pool.isCLP) {
            bal = pool.syrupToken.balanceOf(address(this));
        }
    ...
    function pendingMacaron(uint256 _pid, address _user) external validatePoolByPid(_pid) view returns (uint256) {
        ...
        if(pool.isCLP) {
            lpSupply = pool.syrupToken.balanceOf(address(this));
        }

        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 macaronReward = multiplier.mul(macaronPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
    ...
    function updatePool(uint256 _pid) public validatePoolByPid(_pid) {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 macaronReward = multiplier.mul(macaronPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        macaron.mint(devaddr, macaronReward.div(10));
        macaron.mint(address(choco), macaronReward);
    ...
    function deposit(uint256 _pid, uint256 _amount) external validatePoolByPid(_pid) {        
        ...
        if (user.amount > 0) {
            ...
            if(pending > 0) {
                safeMacaronTransfer(msg.sender, pending);
            }
        }
        if (_amount > 0) {
            ...
            if(pool.isCLP) {
                ...
                pool.lpToken.safeTransfer(address(pool.cakeStrategy), _amount);
    ...
    function withdraw(uint256 _pid, uint256 _amount) external validatePoolByPid(_pid) {
        ...
        if(_amount > 0) {
            ...
            if(pool.isCLP) {
                require(address(pool.cakeStrategy) != address(0), "cake strategy: no Strategy");
                pool.syrupToken.safeTransfer(address(pool.cakeStrategy), _amount);
    ...
    // Safe macaron transfer function, just in case if rounding error causes pool to not have enough MCRNs.
    function safeMacaronTransfer(address _to, uint256 _amount) internal {
        choco.safeMacaronTransfer(_to, _amount);
    }
