...
contract TheBushV1_5 is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 accRewardTAPerShare; // Accumulated TokenA rewards per share, times 1e30. See below. V1_5
        uint256 accRewardTBPerShare; // Accumulated TokenB rewards per share, times 1e30. See below. V1_5
    ...
    function pendingRewardTA(address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accRewardTAPerShare = accRewardTAPerShare.add(tokenAReward.mul(1e30).div(lpSupply)); // V1_5
        }
    ...
    function pendingRewardTB(address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accRewardTBPerShare = accRewardTBPerShare.add(tokenBReward.mul(1e30).div(lpSupply)); // V1_5
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 tokenAReward = multiplier.mul(rewardPerBlockTA).mul(pool.allocPointTokenA).div(totalAllocPoint); // V1_5
        pool.accRewardTAPerShare = pool.accRewardTAPerShare.add(tokenAReward.mul(1e30).div(lpSupply)); // V1_5        
        uint256 tokenBReward = multiplier.mul(rewardPerBlockTB).mul(pool.allocPointTokenB).div(totalAllocPoint); // V1_5
        pool.accRewardTBPerShare = pool.accRewardTBPerShare.add(tokenBReward.mul(1e30).div(lpSupply)); // V1_5          
