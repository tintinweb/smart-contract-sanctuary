...
contract PolyRabbitToken is ERC20('PolyRabbit Finance', 'RABBIT') {
...
// MasterChef is the master of PolyRabbit. He can make PolyRabbit and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once Rabbit is sufficiently
// distributed and the community can show to govern itself.
// Have fun reading it. Hopefully it's bug-free. God bless.
contract PolyRabbitMasterChef is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of Rabbites
    ...
    struct PoolInfo {
        ...
        uint256 accRabbitPerShare;   // Accumulated Rabbits per share, times 1e18. See below.
        uint16 depositFeeBP;      // Deposit fee in basis points
    }

    // The RABBIT TOKEN!
    PolyRabbitToken public Rabbit;
    address public devAddress;

    // RABBIT tokens created per block.
    uint256 public RabbitPerBlock = 0.07 ether;
    ...
    // RABBIT referral contract address.
    ...
    function pendingRabbit(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accRabbitPerShare = accRabbitPerShare.add(RabbitReward.mul(1e18).div(lpSupply));
        }
        return user.amount.mul(accRabbitPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 RabbitReward = multiplier.mul(RabbitPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        Rabbit.mint(devAddress, RabbitReward.div(10));
        Rabbit.mint(address(this), RabbitReward);
        pool.accRabbitPerShare = pool.accRabbitPerShare.add(RabbitReward.mul(1e18).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accRabbitPerShare).div(1e18).sub(user.rewardDebt);
            if (pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accRabbitPerShare).div(1e18);
    ...
    // Safe Rabbit transfer function, just in case if rounding error causes pool to not have enough FOXs.
    ...
    function updateEmissionRate(uint256 _RabbitPerBlock) external onlyOwner {
        massUpdatePools();
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (address(referral) != address(0) && referralCommissionRate > 0) {
            ...
            if (referrer != address(0) && commissionAmount > 0) {
                Rabbit.mint(referrer, commissionAmount);
