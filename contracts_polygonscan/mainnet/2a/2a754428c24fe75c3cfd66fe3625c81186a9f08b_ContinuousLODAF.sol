    ...
    */
	...
	function swapAsset(uint _signal) public onlyOwner {
        ...
        if (poolAssetPercentShare > _signal){
            ...
            uint percentageDeltaEtherToSwap = poolAssetPercentShare - _signal;
            uint deltaEtherToSwapInTether = (percentageDeltaEtherToSwap * (assetValo + oldTetherBalance)) / 100;
            uint deltaEtherToSwapInEther = deltaEtherToSwapInTether/ethPrice;
            // we swap ether to tether
            emit EtherSwap(percentageDeltaEtherToSwap, deltaEtherToSwapInTether, deltaEtherToSwapInEther);
            //this.swap(_maticWEther,_maticTether,deltaEtherToSwapInTether,0,address(this));
        } else {
            // we swap ether to tether
            uint percentageDeltaTetherToSwap = _signal - poolAssetPercentShare ;
            uint deltaTetherToSwapInTether = (percentageDeltaTetherToSwap * (assetValo + oldTetherBalance)) / 100;
            emit TetherSwap(percentageDeltaTetherToSwap,deltaTetherToSwapInTether);
            //this.swap(_maticWEther,_maticTether,deltaTetherToSwapInTether,0,address(this));
        }
