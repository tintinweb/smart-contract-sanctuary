...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 accKingPerShare;   // Accumulated KINGs per share, times 1e18. See below.
    ...
    // The KING TOKEN!
    KingMantaraySwapToken public king;
    // Dev address.
    ...
    event UpdateEmissionRate(address indexed user, uint256 indexed kingPerBlock);
    event RewardLockedUp(address indexed user, uint256 indexed pid, uint256 amountLockedUp);
    event StartBlockChanged(uint256 oldStartBlock, uint256 newStartBlock);

    constructor(
        KingMantaraySwapToken _king,
        address _devAddress,
        address _feeAddress,
    ...
    function getMultiplier(uint256 _from, uint256 _to) public view returns (uint256) {
        if (king.totalSupply() + totalLockedUpRewards >= maxSupply) return 0;
    ...
    function pendingKing(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 pending = user.amount.mul(accKingPerShare).div(1e18).sub(user.rewardDebt);
        return pending.add(user.rewardLockedUp);
    }


    // View function to see if user can harvest Kings's.
    ...
    // Pay or lockup pending king.
    function payOrLockupPendingKing(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        if (canHarvest(_pid, msg.sender)) {
            if (pending > 0 || user.rewardLockedUp > 0) {
                ...
                user.nextHarvestUntil = getPoolHarvestIntervalAmount(_pid);
                // send rewards
                safeKingTransfer(msg.sender, totalRewards);
    ...
    function updateEmissionRate(uint256 _kingPerBlock) public onlyOwner {
        massUpdatePools();
        kingPerBlock = _kingPerBlock;
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (address(referral) != address(0) && referralCommissionRate > 0) {
            ...
            if (referrer != address(0) && commissionAmount > 0) {
                king.mint(referrer, commissionAmount); 
