...
contract Testone is Ownable, Pausable, ReentrancyGuard {
	...
	function startRun(uint256 gameId, uint256[] calldata paymentAmountsUnitPrice) external onlyOperator whenNotPaused nonReentrant {
		...
		require(run.joinMatchTimestamp < block.timestamp + startRunTimeout);
	...
	function endRun(uint256 gameId, uint256[] calldata endGamePrices) external onlyOperator whenNotPaused nonReentrant {
		...
		if (winner == Winner.PLAYER1) {
			...
			RewardInfo memory player2Info = ledger[gameId][run.player2.playerAddress];
			_calculateRewards(gameId, run.winner, player2Info);
		}
		if (winner == Winner.PLAYER2) {
			run.winner = run.player2.playerAddress;
			RewardInfo memory player1Info = ledger[gameId][run.player1.playerAddress];
			_calculateRewards(gameId, run.winner, player1Info);
		}
	...
	function _calculateRewards(
		...
		uint256 fee = (loser.bettedAmount * treasuryFee) / 10000;
