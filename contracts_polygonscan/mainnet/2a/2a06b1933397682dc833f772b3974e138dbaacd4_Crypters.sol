...
contract Crypters is Ownable, AccessControl, ERC721Enumerable, ERC721Burnable {

    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");

    string public _baseTokenURI;

    constructor() ERC721("Crypters", "CRY") {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        grantRole(MINTER_ROLE, msg.sender);

        setBaseURI("https://hwioz5vceb.execute-api.ap-northeast-1.amazonaws.com/api/metadata/");    
    }
    ...
    function mintBatch(address[] memory tos, uint256[] memory tokenIds) public {
        require(tos.length == tokenIds.length, "Crypters: tos and tokenIds must be same length");
