...
// File: contracts/SUPERPresale.sol
...
contract SUPERPresale is ReentrancyGuard {
	using SafeMath for uint256;
	...
	function buy(uint256 _amount) public nonReentrant {
        ...
        require(_amount + (tokensOwned[msg.sender] * USDCPerToken) <= maximumDepositEthAmount, "high amount than max");
	...
	function getSUPERTokensLeft() external view returns (uint256) {
		return SUPER.balanceOf(address(this));
	}

	function claimTokens() external nonReentrant {
		require(isClaimActive, "Claim is not allowed yet");
		require(tokensOwned[msg.sender] > 0, "User should own some SUPER tokens");
		require(tokensUnclaimed[msg.sender] > 0, "User should have unclaimed SUPER tokens");
		require(SUPER.balanceOf(address(this)) >= tokensUnclaimed[msg.sender], "There are not enough SUPER tokens to transfer.");

		uint256 callerTokensUnclaimed = tokensUnclaimed[msg.sender];
		tokensUnclaimed[msg.sender] = 0;
		SUPER.safeTransfer(msg.sender, callerTokensUnclaimed);
	...
	function withdrawUnsoldSUPER() external onlyOwner {
		uint256 amount = SUPER.balanceOf(address(this)) - totalTokensSold;
		SUPER.safeTransfer(msg.sender, amount);
	}

	function withdrawAllSUPER() external onlyOwner {
		SUPER.safeTransfer(msg.sender, SUPER.balanceOf(address(this)));
	}
}