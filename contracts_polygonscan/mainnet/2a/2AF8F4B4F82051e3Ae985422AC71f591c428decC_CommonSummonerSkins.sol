...
contract CommonSummonerSkins is
    ReentrancyGuard,
    Ownable,
    ERC721Enumerable,
    Pausable
{
    ...
    bool public constant isStrictOnSummonerClass = true;

    mapping(uint256 => uint256) skinDNA;
    ...
    ) ERC721("Adventurer Launch Party Costumes", "COSTUMES") {
    ...
    function mint(uint256 quantity)
        ...
        nonReentrant
        whenNotPaused
        supplyAndPriceAreSufficient(quantity)
    {
        // mint random classes
        for (uint256 i = 0; i < quantity; i++) {
            uint256 randomClass = randomUint(_tokenIds.current()) % 11;
            _mintOneSkin(((randomClass + i) % 11) + 1);
        }
    }

    function _mintOneSkin(uint256 _class) private returns (uint256 skinId) {
        uint256 random = randomUint(_tokenIds.current());
    ...
    function randomUint(uint256 id) private view returns (uint256) {
        return (
            uint256(
    ...
    function _isApprovedOrOwnerInRarity(uint256 summonerId)
        private
        view
        returns (bool)
    {
...
contract CommonSkinURIs is Ownable {
    ...
    function tokenURI(uint256 _tokenId, uint256 _dna)
        external
        view
        returns (string memory)
    {
        return
            string(
                abi.encodePacked(
                    ...
                    Base64.encode(bytes(metadata(_tokenId, _dna)))
                )
            );
    }

    function initializeArt(uint256 startIndex, string[][] calldata data)
        public
        onlyOwner
    {
    ...
    function metadata(uint256 _tokenId, uint256 _dna)
        private
        view
        returns (string memory)
    {
        ...
        return
            string(
                abi.encodePacked(
                    "{",
                    '"name": "Adventurer Costume #',
                    _tokenId.toString(),
                    '",',
                    '"description": "Adventurer Costumes are randomly generated and have 100% on-chain art and metadata - Use them as your Summoner appearences !",',
                    '"image": "data:image/svg+xml;base64,',
                    Base64.encode(imageData(_tokenId, _dna)),
                    '",',
                    '"attributes": [',
                    attributes,
                    "]",
    ...
    function imageData(uint256 _tokenId, uint256 _dna)
        private
        view
        returns (bytes memory)
    {
        uint8[8] memory dna = splitNumber(_dna);
        uint256 class = skins.class(_tokenId) - 1;

        string memory skinOfRanger = class == 7 || class == 1
            ? string(
                abi.encodePacked(
                    '<path d="M15.99 11v-1h1V9h-1V5h-1V4h-1V3h-4v1h-1v1h-1v5h1v1h1v2h-1-1-1v1 1 1h1v2h2v1h-1v5h1 5 1v-1h-1v-5h1v-1h1v-2h-1v-1-1h-1v-2h1z" fill="#',
                    art[7][dna[7]],
                    '"/>'
                )
            )
        ...
        return
            abi.encodePacked(
                "<svg id='Adventurer Costume #",
                _tokenId.toString(),
                "' xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24'>",
                '<path fill="#',
                art[7][dna[7]],
                '" d="M16 11v-1h1V9h-1V5h-1V4h-1V3h-4v1H9v1H8v5h1v1h1v4H8v3h2v1H9v5h1 5 1v-1h-1v-5h1v-1h1v-2h-2v-4h1z"/>', // skin
                skinOfRanger,
    ...
    function accessoriesSvg(uint8[8] memory dna)
        private
        view
        returns (string memory)
    {
...
contract RaritySkinManager is Ownable {
    ...
    modifier classChecked(
        ...
        try myIERC721(implementation).isStrictOnSummonerClass() returns (
            ...
            if (isStrict) {
                require(
                    myIERC721(rarity).class(summonerId) ==
    ...
    function assignSkinToSummoner(
        ...
        uint256 tokenId,
        uint256 summonerId
    ) external classChecked(implementation, tokenId, summonerId) {
        require(
            isApprovedOrOwner(implementation, msg.sender, tokenId),
            "You must be owner or approved for this token"
        );
        require(
            isApprovedOrOwner(rarity, msg.sender, summonerId),
    ...
    function trustedAssignSkinToSummoner(uint256 tokenId, uint256 summonerId)
        external
    ...
    function _assignSkinToSummoner(
        ...
        skinOf[summonerOf[skinKey(Skin(implementation, tokenId))]] = Skin(
            address(0),
            0
        );
...
contract RaritySkinManagerFix is Ownable {
    ...
    function skinOf(uint256 summonerId)
        public
        view
        ...
        returns (uint256 tokenAmount, uint256 numOfAdventurers)
    {
        ...
        for (uint256 i = 0; i < numOfSkins; i++) {
            ...
            if (!isRogueClass(tokenId) && adventurersTime[tokenId] != 0) {
                numOfAdventurers += 1;
            }
        }
    ...
    function myRoguesYieldPerDay(address owner)
        external
        view
        returns (uint256 tokenAmount, uint256 numOfRogues)
    {
        ...
        for (uint256 i = 0; i < numOfSkins; i++) {
            uint256 tokenId = myIERC721(skins).tokenOfOwnerByIndex(owner, i);
            if (isRogueClass(tokenId) && roguesLevels[tokenId] != 0) {
        ...
        if (sumOfLevels != 0) {
            tokenAmount =
                (((TOKENS_PER_DAY * totalAdventurers * ROGUE_PERCENT) / 100) *
                    sumOfLevels) /
                roguesTotalLevels;
        }
        ...
        returns (uint256 amount)
    {
        ...
        if (
            (amount * 100) / (100 - ROGUE_PERCENT) > tokenBalance - rogueReserve
        ) {
            amount =
                ((tokenBalance - rogueReserve) * (100 - ROGUE_PERCENT)) /
                100;
        }
        ...
        returns (uint256 amount)
    {
        if (isRogueClass(tokenId)) {
            return
                roguesLevels[tokenId] * (tokenPerLevel - roguesValues[tokenId]);
        } else {
    ...
    function claim(uint256 tokenId) external {
        ...
        require(
            roguesLevels[tokenId] != 0 || adventurersTime[tokenId] != 0,
        ...
        if (isRogueClass(tokenId)) {
            ...
            require(
                IERC20(token).balanceOf(address(this)) > rogueReserve,
                "No tokens left for claiming"
            );
    ...
    function _claimByAdventurer(uint256 tokenId) private returns (uint256) {
        uint256 tokenBalance = IERC20(token).balanceOf(address(this));
        if (adventurersTime[tokenId] == 0 || tokenBalance <= rogueReserve) {
            return 0;
        }
        ...
        if (roguesTotalLevels == 0) {
            ...
            tokenPerLevel +=
                (rogueAmount + unaccountedRewards) /
                roguesTotalLevels;
    ...
    function _claimByRogue(uint256 tokenId) private returns (uint256) {
        uint256 amount = roguesLevels[tokenId] *
            (tokenPerLevel - roguesValues[tokenId]);
        rogueReserve -= amount;
        roguesValues[tokenId] = tokenPerLevel;
    ...
    function isRogueClass(uint256 tokenId) private view returns (bool) {
        return myIERC721(skins).class(tokenId) == ROGUE;
    }
}
