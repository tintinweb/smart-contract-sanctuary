...
abstract contract StrategyFarmTwoAssets is BaseStrategyMasterChef {
    ...
    address public constant arpa = 0xEE800B277A96B0f490a1A732e1D6395FAD960A26;
        ...
        )
    {
        ...
        eth_arpa_path = new address[](2);
        eth_arpa_path[0] = eth;
        eth_arpa_path[1] = arpa;
    }
    ...
    function harvest() public override {
        ...
        if (_dinoBalance > harvestCutoff){
            // Swap to Eth
            _swapUniswapWithPathQuick(dino_eth_path, _dinoBalance);
            ...
            _swapUniswapWithPath(eth_arpa_path, _ethBalancePost.div(2));

            // Add liquidity
            uint256 _eth = IERC20(eth).balanceOf(address(this));
            uint256 _arpa = IERC20(arpa).balanceOf(address(this));
            if (_eth > 0 && _arpa > 0) {
                IERC20(eth).safeApprove(currentRouter, 0);
                IERC20(eth).safeApprove(currentRouter, _eth);
                IERC20(arpa).safeApprove(currentRouter, 0);
                IERC20(arpa).safeApprove(currentRouter, _arpa);
                ...
                IERC20(arpa).safeTransfer(
                    strategist,
                    IERC20(arpa).balanceOf(address(this))
                );
            }
        }
        // Stake the LP tokens
...
contract StrategyTwoAssets is StrategyFarmTwoAssets {
    ...
    address public Eth_Arpa_LP = 0x590F5E967d73eA06DAE9aED2788108DCF52dA269;
