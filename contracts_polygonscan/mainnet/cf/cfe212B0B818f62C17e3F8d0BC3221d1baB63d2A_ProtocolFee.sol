...
contract ProtocolFee is Ownable, IProtocolFee {
        ...
        returns (uint256)
    {
        ...
        uint256 totalSequenceIn;
        uint256 hopAmount = totalAmountIn;
        uint256 singleSequenceAmountIn = 0;
        uint256 singlePoolFeeAmount = 0;
        ...
        for (uint256 i = 0; i < swapSequences.length; i++) {
            ...
            for (uint256 j = 0; j < swapSequences[i].length; j++) {
                totalSequenceIn = totalSequenceIn.add(swapSequences[i][j].swapAmount);
            }

            for (uint256 k = 0; k < swapSequences[i].length; k++) {
                singleSequenceAmountIn = hopAmount.mul(swapSequences[i][k].swapAmount).div(totalSequenceIn);

                singlePoolFeeAmount = getPoolFeeAmount(swapSequences[i][k].pool, singleSequenceAmountIn);
                totalSwapFeeAmount = totalSwapFeeAmount.add(singlePoolFeeAmount);
            }
            hopAmount = hopAmount.sub(totalSwapFeeAmount);
        }
