...
contract StrategySandMan is Ownable, ReentrancyGuard, Pausable {
    ...
    struct UserInfo {
         ...
         *   pending reward = (amount * pool.accUsdcPerShare) - user.rewardDebt
         *
    ...
    address public constant wantAddress = 0x92172a066bc246756866fbB0b1DBEEfdbDf8E87f;
    ...
    uint256 public accUsdcPerShare = 0;
    ...
    function deposit(address _userAddress, uint256 _wantAmt) external onlyOwner nonReentrant whenNotPaused returns (uint256) {
        ...
        uint256 pending = user.shares.mul(accUsdcPerShare).div(1e18).sub(user.rewardDebt);
        if (pending > 0) {
        ...
        user.rewardDebt = user.shares.mul(accUsdcPerShare).div(1e18);
    ...
    function depositReward(uint256 _depositAmt) external returns (bool) {
        ...
        accUsdcPerShare = accUsdcPerShare.add(_depositAmt.mul(1e18).div(sharesTotal));
