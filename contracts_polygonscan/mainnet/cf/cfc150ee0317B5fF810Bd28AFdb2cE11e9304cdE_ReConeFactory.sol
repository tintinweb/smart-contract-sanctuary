...
import {ReCone} from './ReCone.sol';
import {Ownable} from '@openzeppelin/contracts/access/Ownable.sol';
import {Clones} from '@openzeppelin/contracts/proxy/Clones.sol';
import {Address} from '@openzeppelin/contracts/utils/Address.sol';
import 'hardhat/console.sol';

contract ReConeFactory is Ownable {
    ...
    function mint(
        ...
        address predictedAddress = implementationAddress
            .predictDeterministicAddress(salt, address(this));
        if (!predictedAddress.isContract()) {
            implementationAddress.cloneDeterministic(salt);
            ReCone(predictedAddress).initialize(
                name,
                symbol,
                baseTokenURI,
                '1'
            );
            if (upperLimit[msg.sender] == 0) {
                upperLimit[msg.sender] = 30;
            }
            emit Deployed(msg.sender, implementationAddress, predictedAddress);
        }
        require(
            ReCone(predictedAddress).totalSupply() + mintAmount <=
                upperLimit[msg.sender],
            'Cannot mint more than the upper limit'
        );
        for (uint256 i = 0; i < mintAmount; i++) {
            ReCone(predictedAddress).mint(msg.sender);
        }
    }

    function predictDeployResult(address implementation, address owner)
        public
        view
        returns (address)
    {
    ...
    function updateUpperLimit(address user, uint256 newUpperLimit)
        external
        onlyOwner
    {
        upperLimit[user] = newUpperLimit;
    }

    function getUpperLimit(address user) public view returns (uint256) {
        return upperLimit[user];
    }
}
