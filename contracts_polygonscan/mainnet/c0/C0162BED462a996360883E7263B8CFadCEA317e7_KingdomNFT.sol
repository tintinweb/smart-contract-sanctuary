...
contract KingdomNFT is Ownable, ReentrancyGuard, Pausable, AccessControl {
    ...
    function deposit(uint256 _amount, uint256 _nftID) external nonReentrant {
        ...
        if (iNFTFactory.balanceOf(msg.sender) > 0) {
    ...
    function withdraw() external nonReentrant {
        ...
        if (_amount > 0) {
            ...
            if (user.hasNFT) {
                ...
                iNFTFactory.setExperience(user.nftID, _newExperience);
    ...
    function _initializeNFT(uint256 _nftID, address _userAddress) internal  {
        ...
        if (!nftIDs[_nftID]) {
            if (iNFTFactory.ownerOf(_nftID) == _userAddress) {
                user.hasNFT = true;
                user.nftID = _nftID;
    ...
    function _getNFTPowerStaking(uint256 _nftID) internal view returns (uint256) {
        ...
        if (strength == 0 && agility == 0 ) {
            (
                ...
                wisdom,
                magic
            ) = iNFTFactory.getCharacterStats(_nftID);
        }
    ...
    function _getNFTExperience(uint256 _nftID) internal returns (uint256) {
        (,uint256 experience,) = iNFTFactory.getCharacterOverView(_nftID);
