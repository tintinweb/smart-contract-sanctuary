...
contract LoanSharksRouter{
    ...
    function swap(address receiver, uint amount_usdc, uint max_matic_drawdown) public onlyLive onlyApproved(msg.sender, amount_usdc) returns(uint[] memory){
    ...
    function check_for_usdc_approval() public view onlyLive returns(uint){
        return IERC20(USDC).allowance(msg.sender, QSR);
    }
