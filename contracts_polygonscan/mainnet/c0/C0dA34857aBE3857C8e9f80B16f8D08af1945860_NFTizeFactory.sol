...
contract NFTizeFactory is Initializable, AccessControlUpgradeable, IERC721Receiver, OwnableUpgradeable, UUPSUpgradeable {
    ...
    event NftizePoolCreated(address indexed poolContract, address indexed token, string poolTokenSymbol);
    event LiquidityAdded(address indexed poolContract, uint256 theosTokenAmount, uint256 poolTokenAmount, address sushiLP);

    error InsufficientTheosLiquidity(uint256 available, uint256 required);
    error InvalidPoolCreator();

    function initialize(address theosAddress, address poolCreator) public virtual initializer {
        __Ownable_init();
        __UUPSUpgradeable_init();
        theosToken = IERC20(theosAddress);
        //        _setupRole(POOL_CREATOR_ROLE, poolCreator);
    ...
    function createPool(
        ...
        // This check is disabled in the testing phase
        ...
        TokenData memory tokenData = TokenData(tokenName, symbol, poolTokenAmount);
        nftizePoolContract.initialize(tokenData, basePrice, poolSlots, whitelister, nftContractAddress, tokenId, platformFeeAddress);
        poolsTokenContracts.push(address(nftizePoolContract));

        emit NftizePoolCreated(address(nftizePoolContract), nftizePoolContract.poolToken(), symbol);

        IERC20(nftizePoolContract.poolToken()).approve(SUSHISWAP_ROUTER_ADDRESS, poolTokenAmount);
        theosToken.approve(SUSHISWAP_ROUTER_ADDRESS, theosAmount);

        sushiswapRouter.addLiquidity(
            address(theosToken), // tokenA
            nftizePoolContract.poolToken(), // tokenB
            theosAmount, // desiredATokenAmount
            poolTokenAmount, // desiredBTokenAmount
            minAmount, // amountAMin
            minAmount, // amountBMin
            address(this), // to address for LP tokens
            block.timestamp
        );

        IERC721(nftContractAddress).approve(address(nftizePoolContract), tokenId);
        nftizePoolContract.depositNFT(nftContractAddress, tokenId, i);

        address sushiLP = sushiswapFactory.getPair(address(theosToken), nftizePoolContract.poolToken());
        uint256 unlockTime = block.timestamp + 30 days;
        LPData memory lpData = LPData(msg.sender, sushiLP, unlockTime, theosAmount);
        lpProviders.push(lpData);

        emit LiquidityAdded(address(nftizePoolContract), theosAmount, poolTokenAmount, sushiLP);
    }

    //    function claimLP(address poolToken){
...
contract NftizePool is Initializable, IERC721Receiver, AccessControl {
    ...
    function initialize(
        ...
        poolTokenContract.initialize(tokenData.name, tokenData.symbol, tokenData.initialSupply);
