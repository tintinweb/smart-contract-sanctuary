...
abstract contract Context {
    function _msgSenderr() internal view virtual returns (address) {
        return msg.sender;
    }

    function _msgDataa() internal view virtual returns (bytes calldata) {
        return msg.data;
    }
}
...
abstract contract Ownable is Context {
     ...
     */
    constructor() {
        _transferOwnership(_msgSenderr());
    }
     ...
     */
    modifier onlyOwner() {
        require(owner() == _msgSenderr(), "Ownable: caller is not the owner");
        _;
    }
...
//
pragma solidity ^0.8.0;
contract IncooomApes is Initializable, ContextUpgradeable, Ownable, ERC721EnumerableUpgradeable, ERC721BurnableUpgradeable, ERC721PausableUpgradeable {
    ...
    function batchMint(address [] memory _list) public onlyOwner{
        // We cannot just use balanceOf to create the new tokenId because tokens
        // can be burned (destroyed), so we need a separate counter.
