...
/**
 * @NOTE website: https://artwallstreet.io/
 */
...
// File: AWST_ERC1155.sol

pragma solidity ^0.8.0;

contract AWST_ERC1155 is ERC1155, IERC1155MetadataURI, Ownable {
    /*
    ...
    function _setBatchURI(uint256[] memory ids, string memory baseURI)
        internal
        virtual
    {
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            string memory _uri = string(
                abi.encodePacked(baseURI, "/", Strings.toString(i + 1), ".json")
            );
            _setURI(ids[i], _uri);
        }
    }
    ...
    function mintBatch(
        ...
        require(
            ...
            "Values length must be equal to Token count"
        );
