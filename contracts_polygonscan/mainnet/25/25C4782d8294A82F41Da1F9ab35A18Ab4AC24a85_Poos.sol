...
contract Poos is ERC721, Base, Random, IPoos {
    ...
    */
    function transferFrom(
        ...
        _changePooOwner(from, to, tokenId);
        _resetPooPrice(tokenId);
    }
    ...
    * @dev override see {IERC721-safeTransferFrom}.
    ...
    function _resetPooPrice(uint256 tokenId) internal {
        indexToPoo[tokenId].price = 0;
    }

    function _changePooOwner(address from, address to, uint256 tokenId) internal {
        uint256[] memory changedTokenArray = new uint256[](addressToPoos[from].length - 1);
        uint arrayCounter = 0;
        for (uint i = 0; i < addressToPoos[from].length; i++) {
            if (addressToPoos[from][i] != tokenId) {
                changedTokenArray[arrayCounter] = addressToPoos[from][i];
                arrayCounter++;
            }
        }
        addressToPoos[from] = changedTokenArray;
        addressToPoos[to].push(tokenId);
        indexToPoo[tokenId].owner = payable(to);
    }
    ...
    function approveForMarketplace(address to, uint256 tokenId, uint256 price) public virtual onlyAllowedContracts {
        ...
        indexToPoo[tokenId].price = price;
        _approve(to, tokenId);
    }
    ...
    function transferMarketplaceFrom(address from, address to, uint256 tokenId, uint256 price) public virtual onlyAllowedContracts {
        indexToPoo[tokenId].lastSellPrice = price;
    ...
    event NewPooMinted(uint indexed _id);
    event PooRenamed(uint indexed _id, string _newName);
    event ExecutedPowerUpChange(uint indexed _id);
    event ExecutedMintSetting(uint indexed _id);

    using Address for address;

    mapping(address => uint[]) public addressToPoos;
    mapping(uint => Structs.Poo) indexToPoo;
    mapping(string => bool) nameTaken;

    uint public pooCount;

    constructor(address _constantsContract) Base(_constantsContract) ERC721(" Poos", "Poos") {}

    function createNewPoo() public payable {
        require(msg.value >= IConstants(constantsContract).pooMintCosts(), "The amount sent is smaller the the mint price.");
        pooCount = pooCount + 1;
        address statisticsContract = IConstants(constantsContract).statisticsContract();
        Structs.Poo memory poo;
        poo.id = pooCount;
        poo.level = 1;
        poo.currentExperience = 0;
        poo.experienceForNextLevel = 100;
        poo.name = IHelpers(IConstants(constantsContract).helpersContract()).concat("Poo #", poo.id);
        poo.hitPoints = 10 + randModulus(10, pooCount);
        poo.basicDmg = 3 + randModulus(3, poo.hitPoints);
        poo.stamina = 1 + randModulus(3, poo.basicDmg);
        poo.attackPower = 1 + randModulus(3, poo.stamina);
        poo.defense = 1 + randModulus(1, poo.attackPower);
        poo.initiative = 1 + randModulus(3, poo.defense);
        poo.agility = 1 + randModulus(3, poo.initiative);
        poo.luck = 1;
        poo.owner = payable(msg.sender);
        poo.fightsWon = 0;
        poo.fightsLost = 0;
        poo.exhaustion = 0;
        poo.lastTimeRested = block.number;
        poo.hunger = 0;
        poo.lastTimeFed = block.number;
        poo.alive = true;
        poo.price = 0;
        poo.powerUps = [uint(0), uint(0), uint(0), uint(0), uint(0)];
        poo.hasPowerUps = false;
        poo.restPoints = 0;
        poo.resCounter = 0;
        poo.lastSellPrice = IConstants(constantsContract).pooMintCosts();
        poo.nextPossibleMint = block.number + IConstants(constantsContract).blocksBetweenDumplingMintForPoo();
        poo.mintedDumplings;
        addressToPoos[msg.sender].push(pooCount);
        indexToPoo[pooCount] = poo;
        nameTaken[poo.name] = true;
        _mint(msg.sender, pooCount);
        transferValueToOwner(msg.value);
        IStatistics(statisticsContract).increaseTotalPoosStat();
        emit NewPooMinted(pooCount);
    }

    function renamePoo(uint _id, string calldata _newName) public {
        require(msg.sender == ownerOf(_id), "Sender is not owner.");
        require(!nameTaken[_newName], "Poo name already taken.");
        payByPoo(IConstants(constantsContract).pooTokenForRenamePoo());
        indexToPoo[_id].name = IHelpers(IConstants(constantsContract).helpersContract()).limitNameLength(_newName);
        emit PooRenamed(_id, _newName);
    }

    function checkAddressHasPoo(address _address) public override view returns (bool _hasPoo) {
        return addressToPoos[_address].length > 0;
    }

    function getPooByIndex(uint _id) public override view returns (Structs.Poo memory _poo) {
        return indexToPoo[_id];
    }

    function getFullPooByIndex(uint _id) public override view returns (Structs.Poo memory _poo) {
        _poo = indexToPoo[_id];
        _poo.restPoints = ITamagotchi(IConstants(constantsContract).PooTamagotchiContract()).getRestPoints(_id);
        _poo.hunger = ITamagotchi(IConstants(constantsContract).PooTamagotchiContract()).getHunger(_id);
        if (_poo.hunger == 100) {
            _poo.alive = false;
        }
        return _poo;
    ...
    function getAllPooIDsByAddress(address _address) public override view returns (uint[] memory _id) {
        return addressToPoos[_address];
    }

    function checkPooHasPowerUps(uint _id) public override view returns (bool _hasPowerUps) {
        return indexToPoo[_id].hasPowerUps;
    }

    function canMintDumpling(uint _id) public override view returns (bool canMint) {
        return (block.number > indexToPoo[_id].nextPossibleMint && indexToPoo[_id].mintedDumplings < IConstants(constantsContract).maxMintableDumplingsForPoo());
    }
    ...
    *
    */
    function getPoosBatch(uint _lowerBound, uint _upperbound) public view returns (Structs.Poo[] memory _poo) {
        address statisticsContract = IConstants(constantsContract).statisticsContract();
        uint totalPoos = IStatistics(statisticsContract).totalPoosStat();
        require(_lowerBound > 0 && _lowerBound <= _upperbound && _upperbound <= totalPoos, "Invalid set of bounds provided.");
        Structs.Poo[] memory poosArray = new Structs.Poo[](_upperbound - _lowerBound + 1);
        uint arrayCounter = 0;
        while (_lowerBound <= _upperbound) {
            Structs.Poo memory poo = getFullPooByIndex(_lowerBound);
            poosArray[arrayCounter] = poo;
            _lowerBound++;
            arrayCounter++;
        }
    ...
    function _baseURI() internal pure override returns (string memory) {
        return "https://Poos.fi/PooMetadata/Poos/";
    }

    function executeLevelUp(uint _id, uint[11] memory _newStats) public override onlyAllowedContracts {
        indexToPoo[_id].level = _newStats[0];
        indexToPoo[_id].currentExperience = _newStats[1];
        indexToPoo[_id].experienceForNextLevel = _newStats[2];
        indexToPoo[_id].hitPoints = _newStats[3];
        indexToPoo[_id].basicDmg = _newStats[4];
        indexToPoo[_id].stamina = _newStats[5];
        indexToPoo[_id].attackPower = _newStats[6];
        indexToPoo[_id].defense = _newStats[7];
        indexToPoo[_id].initiative = _newStats[8];
        indexToPoo[_id].agility = _newStats[9];
        indexToPoo[_id].luck = _newStats[10];

        emit ExecutedLevelUp(_id, _newStats[0]);
    }

    function executeFeed(uint _id) public override onlyAllowedContracts {
        indexToPoo[_id].hunger = 0;
        indexToPoo[_id].lastTimeFed = block.number;
        emit ExecutedFeed(_id, indexToPoo[_id].lastTimeFed);
    }

    function executeRest(uint _id, uint _restPoints) public override onlyAllowedContracts {
        uint256 exhaustion = indexToPoo[_id].exhaustion;
        if (_restPoints > exhaustion) {
            indexToPoo[_id].exhaustion = 0;
        } else {
            indexToPoo[_id].exhaustion = exhaustion - _restPoints;
        }
        indexToPoo[_id].lastTimeRested = block.number;
        emit ExecutedRest(_id, indexToPoo[_id].lastTimeRested);
    }

    function executeResurrect(uint _id) public override onlyAllowedContracts {
        indexToPoo[_id].hunger = 0;
        indexToPoo[_id].lastTimeFed = block.number;
        indexToPoo[_id].alive = true;
        indexToPoo[_id].resCounter = indexToPoo[_id].resCounter + 1;
        emit ExecutedResurrect(_id);
    }

    function executeAliveChange(uint _id, bool _aliveStatus) public override onlyAllowedContracts {
        indexToPoo[_id].alive = _aliveStatus;
    ...
    function executePowerUpChange(uint _id, uint[5] memory _powerUps, bool _powerUpStatus) public override onlyAllowedContracts {
        indexToPoo[_id].powerUps = _powerUps;
        indexToPoo[_id].hasPowerUps = _powerUpStatus;
    ...
    function executeExhaustionChange(uint _id, uint _exhaustionToAdd) public override onlyAllowedContracts {
        ...
        if (exhaustion + _exhaustionToAdd > 100) {
            indexToPoo[_id].exhaustion = 100;
        } else {
            indexToPoo[_id].exhaustion = exhaustion + _exhaustionToAdd;
        }
        emit ExecutedExhaustionChange(_id, indexToPoo[_id].exhaustion);
    }

    function executeWinningStatChange(uint _id, uint _xp) public override onlyAllowedContracts {
        indexToPoo[_id].fightsWon = indexToPoo[_id].fightsWon + 1;
        indexToPoo[_id].currentExperience = _xp;
        emit ExecutedWinningStat(_id);
    }

    function executeLosingStatChange(uint _id, uint _xp) public override onlyAllowedContracts {
        indexToPoo[_id].fightsLost = indexToPoo[_id].fightsLost + 1;
        indexToPoo[_id].currentExperience = _xp;
        emit ExecutedLosingStat(_id);
    }

    function executeMintSetting(uint _id) public override onlyAllowedContracts {
        indexToPoo[_id].nextPossibleMint = block.number + IConstants(constantsContract).blocksBetweenDumplingMintForPoo() + (IConstants(constantsContract).baseBlockBetweenDumplingMint() * indexToPoo[_id].mintedDumplings);
        indexToPoo[_id].mintedDumplings = indexToPoo[_id].mintedDumplings + 1;
        emit ExecutedMintSetting(_id);
    }
