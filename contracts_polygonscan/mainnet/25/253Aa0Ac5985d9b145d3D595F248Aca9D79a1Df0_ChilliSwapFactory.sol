...
contract ChilliSwapFactory is IUniswapV2Factory {
    ...
    uint256 public override protocolFeeDenominator = 3000; // uses ~10% of each swap fee
    uint256 public override totalFee = 996; // uses ~10% of each swap fee
    ...
    function setProtocolFee(uint256 _protocolFeeDenominator) external {
        require(msg.sender == feeToSetter, 'ChilliSwap: FORBIDDEN');
    ...
    function changeTotalFee(uint256 _totalFee) external {
        require(msg.sender == feeToSetter, 'ChilliSwap: FORBIDDEN');
...
contract ChilliSwapPair is /* IUniswapV2Pair, */ ChilliSwapERC20 {
    ...
    modifier lock() {
        require(unlocked == 1, 'ChilliSwap: LOCKED');
        unlocked = 0;
        _;
        unlocked = 1;
    }
    ...
    function _safeTransfer(address token, address to, uint value) private {
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), 'ChilliSwap: TRANSFER_FAILED');
    }
    ...
    function initialize(address _token0, address _token1) external {
        require(msg.sender == factory, 'ChilliSwap: FORBIDDEN'); // sufficient check
        token0 = _token0;
        token1 = _token1;
    }
    ...
    function _update(uint balance0, uint balance1, uint112 _reserve0, uint112 _reserve1) private {
        require(balance0 <= uint112(-1) && balance1 <= uint112(-1), 'ChilliSwap: OVERFLOW');
    ...
    function mint(address to) external lock returns (uint liquidity) {
        ...
        require(liquidity > 0, 'ChilliSwap: INSUFFICIENT_LIQUIDITY_MINTED');
        _mint(to, liquidity);
    ...
    function burn(address to) external lock returns (uint amount0, uint amount1) {
        ...
        require(amount0 > 0 && amount1 > 0, 'ChilliSwap: INSUFFICIENT_LIQUIDITY_BURNED');
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        require(amount0Out > 0 || amount1Out > 0, 'ChilliSwap: INSUFFICIENT_OUTPUT_AMOUNT');
        (uint112 _reserve0, uint112 _reserve1,) = getReserves(); // gas savings
        require(amount0Out < _reserve0 && amount1Out < _reserve1, 'ChilliSwap: INSUFFICIENT_LIQUIDITY');
        ...
        require(to != _token0 && to != _token1, 'ChilliSwap: INVALID_TO');
        ...
        require(amount0In > 0 || amount1In > 0, 'ChilliSwap: INSUFFICIENT_INPUT_AMOUNT');
        { // scope for reserve{0,1}Adjusted, avoids stack too deep errors
        uint balance0Adjusted = balance0.mul(1000).sub(amount0In.mul(3));
        uint balance1Adjusted = balance1.mul(1000).sub(amount1In.mul(3));
        require(balance0Adjusted.mul(balance1Adjusted) >= uint(_reserve0).mul(_reserve1).mul(1000**2), 'ChilliSwap: K');
        }
...
abstract contract ChilliSwapERC20 is IUniswapV2ERC20 {
    using SafeMath_0 for uint;

    string public override constant name = 'Chilli LP';
    string public override constant symbol = 'Chilli-LP';
    ...
    function permit(address owner, address spender, uint value, uint deadline, uint8 v, bytes32 r, bytes32 s) external override {
        require(deadline >= block.timestamp, 'ChilliSwap: EXPIRED');
        ...
        require(recoveredAddress != address(0) && recoveredAddress == owner, 'ChilliSwap: INVALID_SIGNATURE');
