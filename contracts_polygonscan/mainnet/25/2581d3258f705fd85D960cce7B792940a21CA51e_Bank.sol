...
// File contracts/5/interfaces/IBankConfig.sol
...
// File contracts/5/interfaces/InterestModel.sol
...
// File contracts/5/interfaces/IGoblin.sol
...
interface IGoblin {
    // Work on a (potentially new) position. Optionally send surplus token back to Bank.
    function work(
        uint256 id,
        address user,
        address borrowToken,
    ...
    // Return the amount of ETH wei to get back if we are to liquidate the position.
...
contract Bank is GTokenFactory, Ownable, ReentrancyGuard {
    ...
    function opPosition(
        ...
        if (isBorrowHt) {
            ...
            require(
                sendHT <= address(this).balance && debt <= banks[production.borrowToken].totalVal,
                "insufficient HT in the bank"
            );
            beforeToken = address(this).balance.sub(sendHT);
        } else {
            beforeToken = SafeToken.myBalance(production.borrowToken);
            require(
                borrow <= beforeToken && debt <= banks[production.borrowToken].totalVal,
        ...
        if (backToken > debt) {
            ...
            require(health.mul(production.openFactor) >= debt.mul(BPS), "bad work factor");
    ...
    function liquidate(uint256 posId) external payable onlyEOA nonReentrant {
        ...
        IGoblin(production.goblin).liquidate(posId, pos.owner, production.borrowToken);
