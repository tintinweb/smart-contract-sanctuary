...
contract SpaceShipsus is ERC721URIStorage, Ownable {
    ...
    function _createShip(address _account) internal {
        ...
        shipIdToEnergyRefreshCD[newItemId] = block.number;
        tokenCounter++;
        tokenSupply++;
    ...
    function refreshEnergy() external {
        ...
        require(passedTimeSinceLastRefresh >= 1 minutes, "Not yet");

        uint energyRefreshAmount;
        for(uint i = 0; i < passedTimeSinceLastRefresh; i+= 1 minutes) {
            if(energyRefreshAmount<=attackerInfo.maxEnergy){
                energyRefreshAmount++;
            }
        }

        attackerInfo.energy += energyRefreshAmount;
        shipIdToEnergyRefreshCD[attackerInfo.id] = block.timestamp;
    }

    function getAmountOfEnergyRefresh() external view returns(uint enrgyAmt){
