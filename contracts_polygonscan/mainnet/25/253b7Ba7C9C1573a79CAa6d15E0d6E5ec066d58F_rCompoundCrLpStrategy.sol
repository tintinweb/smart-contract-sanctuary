...
contract rCompoundCrLpStrategy is rStrategy {
    ...
    function compound() external payable whenNotPaused nonReentrant onlyRole(MANAGER_ROLE) {
        ...
        if (address(_rewardToken) == address(_pairedToken)) { // RAIDER LP Case
            //// Swap 1/2 of the reward token for the LP base token
            // Get the current price from the exchange
            (reserveA, reserveB, ) = _rewardLpToken.getReserves();
            ...
            // Get the current price from the exchange
            (reserveA, reserveB, ) = _rewardLpToken.getReserves(); // Token A is the base token in all of the Raider Pairs and Token B is the _pairedToken
            minAmountBaseToken = _router.getAmountOut(rewardsToCompound, reserveB, reserveA); // accounts for slippage and fees
            ...
            amounts = _router.swapExactTokensForETH(
                ...
                minAmountBaseToken, // minAmount of matic out
                _swapPath, // _router path
                address(this), // address to send matic to (this contract)
                block.timestamp + 20 // deadline for executing the swap
            );
            desiredAmountBaseToken = amounts[1].mul(50 * (10 ** 16)).div(BIG_UNIT);
            delete amounts;
            delete _swapPath;
            ...
            // Compare exchange quote with oracle to make sure it's within our tolerance
            oracleAmount = _oracle.consult(
                address(_stakingLpBaseToken), // address tokenIn
                desiredAmountBaseToken, // uint amountIn
            ...
            // Execute the swap
            _router.swapExactETHForTokens{ value: desiredAmountBaseToken }(
        ...
        { // Enclose in brackets to get around compiler issues                
            ...
            (,, uint newStakingTokens) = _router.addLiquidityETH{ value: desiredAmountBaseToken }(
                ...
                minAmountBaseToken, // minimum amount of matic to pair - same as minimum received from swap
