...
abstract contract BaseStrategy is Ownable {
    ...
    address public constant sushiRouter = 0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506; //Quickswap router
    ...
    function _swapUniswapWithPathSushi(
        address[] memory path,
        uint256 _amount
    ) internal {
        require(path[1] != address(0));
...
abstract contract StrategyFarmTwoAssets is BaseStrategyMasterChef {
    ...
    address public constant dg = 0x2a93172c8DCCbfBC60a39d56183B7279a2F647b4;
    ...
    function harvest() public override {
        ...
        if (_dinoBalance > harvestCutoff){
            // Swap to USDC
            _swapUniswapWithPathSushi(dino_usdc_path, _dinoBalance);
            ...
            _swapUniswapWithPath(usdc_dg_path, _usdcBalancePost.div(2));

            // Add liquidity
            uint256 _usdc = IERC20(usdc).balanceOf(address(this));
            uint256 _dg = IERC20(dg).balanceOf(address(this));
            if (_usdc > 0 && _dg > 0) {
                IERC20(usdc).safeApprove(rewardRouter, 0);
                IERC20(usdc).safeApprove(rewardRouter, _usdc);
                IERC20(dg).safeApprove(rewardRouter, 0);
                IERC20(dg).safeApprove(rewardRouter, _dg);
                ...
                IERC20(dg).safeTransfer(
                    strategist,
                    IERC20(dg).balanceOf(address(this))
                );
            }
        }
        // Stake the LP tokens
...
contract StrategyTwoAssets is StrategyFarmTwoAssets {
    ...
    address public DG_USDC_LP = 0x27CE41B9eeB94cC122eF3B5e409b2900d3e0A629;
