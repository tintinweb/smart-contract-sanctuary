...
// SageToken.
contract SageToken is BEP20('PolySage Finance', 'SAGE') {
...
// MasterChef is the master of SAGE. He can make Sage and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once SAGE is sufficiently
// distributed and the community can show to govern itself.
//
// Have fun reading it. Hopefully it's bug-free. God bless.
contract PolySageMasterChef is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of SAGE
    ...
    struct PoolInfo {
        ...
        uint256 accSagePerShare;   // Accumulated SAGEs per share, times 1e18. See below.
        uint16 depositFeeBP;      // Deposit fee in basis points
        uint256 lpSupply;
    }

    // The SAGE TOKEN!
    SageToken public immutable sage;
    // Dev address.
    ...
    event UpdateEmissionRate(address indexed user, uint256 SagePerBlock);
    ...
    event SageMintError(bytes reason);

    constructor(
        SageToken _sage,
        address _devaddr,
        address _feeAddress,
        uint256 _SagePerBlock,
        uint256 _startBlock
    ) public {
        sage = _sage;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        SagePerBlock = _SagePerBlock;
    ...
    function pendingSage(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && pool.lpSupply != 0 && totalAllocPoint > 0) {
            ...
            accSagePerShare = accSagePerShare.add(sageReward.mul(1e18).div(pool.lpSupply));
        }
        return user.amount.mul(accSagePerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        try sage.mint(devaddr, sageReward.div(10)) {
        } catch (bytes memory reason) {
            sageReward = 0;
            emit SageMintError(reason);
        }
        
        try sage.mint(address(this), sageReward) {
        } catch (bytes memory reason) {
            sageReward = 0;
            emit SageMintError(reason);
        }
        
        pool.accSagePerShare = pool.accSagePerShare.add(sageReward.mul(1e18).div(pool.lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accSagePerShare).div(1e18).sub(user.rewardDebt);
            if (pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accSagePerShare).div(1e18);
    ...
    function updateEmissionRate(uint256 _SagePerBlock) external onlyOwner {
        require(_SagePerBlock <= MAX_EMISSION_RATE, "Too high");
        massUpdatePools();
        SagePerBlock = _SagePerBlock;
        emit UpdateEmissionRate(msg.sender, _SagePerBlock);
    }
