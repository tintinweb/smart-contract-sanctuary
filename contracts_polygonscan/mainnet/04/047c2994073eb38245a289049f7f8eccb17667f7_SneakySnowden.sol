...
contract SneakySnowden is ERC721Enumerable, ERC721Burnable, Ownable {
    ...
    uint256 public presalePrice = 20 ether;
    uint256 public mainsalePrice = 30 ether;
    uint256[] public giveawayMints = [325,941,1833,2168,2465,4501,5113,5905];
    
    bool public snowdenPresaleOpen = true;
    bool public snowdenMainsaleOpen = false;
    
    Counters.Counter private _tokenIdTracker;

    mapping (address => bool) whiteListed;
    
    constructor(string memory _initBaseURI) ERC721("Sneaky Snowden", "SSD")
    {
        ...
        for(uint i=0;i<giveawayMints.length;i++)
        {
            _safeMint(msg.sender, giveawayMints[i]);
        }
    ...
    {
         require(snowdenPresaleOpen == true);
         _;
    }

    modifier isMainsaleOpen
    {
         require(snowdenMainsaleOpen == true);
         _;
    }

    function changeSalePhase() public onlyOwner
    {
        snowdenPresaleOpen = false;
        snowdenMainsaleOpen = true;
    }

    function closeMainsale() public onlyOwner
    {
        snowdenMainsaleOpen = false;
    }
    ...
    function addWhiteList(address[] memory _whiteListedAddress) public onlyOwner
    {
    ...
    function preSale(uint8 mintTotal)public payable isPresaleOpen
    {
        ...
        require(msg.value >= presalePrice * mintTotal, "Minting a Snowden Costs 20 MATIC Each!");
        require(totalToken() < maxSupply, "SOLD OUT!");
       
        for(uint i=0;i<mintTotal;i++)
        {
            _tokenIdTracker.increment();
            if(_exists(totalToken()))
            {
                _tokenIdTracker.increment();
            }
            _safeMint(msg.sender, totalToken());
    ...
    {
        require(mintTotal >= 1 , "Mint Amount Incorrect");
        require(msg.value >= mainsalePrice * mintTotal, "Minting a Snowden Costs 30 MATIC Each!");
        require(totalToken() < maxSupply, "SOLD OUT!");
