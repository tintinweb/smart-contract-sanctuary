...
import { CareSession } from "./CareSession.sol";
import { IOperator } from "./interfaces/IOperator.sol";
import { EnumerableSet } from "@openzeppelin/contracts/utils/structs/EnumerableSet.sol";
import { SafeMath } from "@openzeppelin/contracts/utils/math/SafeMath.sol";
import "hardhat/console.sol";

contract CareCentre is CareSession {
  ...
  mapping(bytes32 => address) public ownerOfReceipt;
  ...
  function startCareForOwner(uint256[] calldata _ids) external {
    ...
    require(
      gotchiFacet.isApprovedForAll(msg.sender, address(operator)),
      "Carer: startCareForMultiple: Operator not approved"
    );

    address _owner = gotchiFacet.getAavegotchi(_ids[0]).owner;
    require(
      msg.sender == _owner,
      "Carer: startCareForMultiple: Starter not owner"
    );

    uint256 _rate = calculateRate(_ids.length);

    CareInfo memory _newCareInfo = CareInfo(_owner, 0, _rate, _ids);

    updateOwnerInfo(_newCareInfo);

    caringOwners.add(_owner);

    LogTaskSubmitted(_owner);
  }
  ...
  function exec(CareInfo calldata _careInfo) external {
    ...
    operator.pet(_careInfo.gotchis);
    console.log("7");
  ...
  function getReceipt(address _owner, CareInfo memory _careInfo)
    public
    pure
    returns (bytes32 _receipt)
  {
    _receipt = keccak256(abi.encode(_owner, _careInfo));
  }
  ...
  {
    _careInfo = careInfoByOwner[_owner];
  }

  function getOwnerOfReceipt(bytes32 _receipt)
    external
    view
    returns (address _owner)
  {
    _owner = ownerOfReceipt[_receipt];
  }
  ...
  function updateOwnerInfo(CareInfo memory _careInfo) internal {
    address _owner = _careInfo.owner;
    bytes32 _receipt = getReceipt(_owner, _careInfo);

    ownerOfReceipt[_receipt] = _owner;

    careInfoByOwner[_owner] = _careInfo;
  }

  function clearOwnerInfo(address _owner) internal {
    CareInfo memory _careInfo = careInfoByOwner[_owner];
    bytes32 _receipt = getReceipt(_owner, _careInfo);

    caringOwners.remove(_owner);
    delete ownerOfReceipt[_receipt];
...
contract CareSession is Ownable {
  using SafeMath for uint256;
  ...
  function payWages(address _owner, uint256 _rate) internal {
    require(
...
interface IOperator {
  function pet(uint256[] calldata _gotchis) external;
}
