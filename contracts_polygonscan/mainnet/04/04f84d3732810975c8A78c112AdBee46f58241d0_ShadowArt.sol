...
contract ShadowArt is ERC721Enumerable, HasRoot, HasAdmin, SigVerifier {
    ...
    uint32 private constant OP_MINT = 1;
    ...
    function mint(address to, uint32 count, uint64 nonce, Sig memory sig) external returns (bool) {

        // we use block's timestamp to ensure nonce is always fresh and not reused
        require(nonce >= uint64(block.timestamp) / 30, "invalid nonce");

        // check for valid digital signature
        bytes32 message = sigPrefixed(
            keccak256(abi.encodePacked(OP_MINT, _msgSender(), to, count, nonce))
        );
