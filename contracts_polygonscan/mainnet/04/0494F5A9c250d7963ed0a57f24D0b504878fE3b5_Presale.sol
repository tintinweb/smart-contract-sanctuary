...
contract Presale is ReentrancyGuard, Context, Ownable {
    ...
    function _preValidatePurchase(address beneficiary, uint256 weiAmount) internal view {
        ...
        for (uint i = 0; i < _whitelist.length; i++) {
            if(_whitelist[i] == beneficiary) { // checking if the address is already defined
                isWhiteListed = true;
            }
        }
        require(isWhiteListed, 'You have no access for the pre-sale');
        this; // silence state mutability warning without generating bytecode - see https://github.com/ethereum/solidity/issues/2691
    }
    ...
    function addWalletToPresaleList(address wallet) external onlyOwner {
        bool isAlreadyWhiteListed = false;
        for (uint i = 0; i < _whitelist.length; i++) {
            if(_whitelist[i] == wallet) { // checking if the address is already defined
                isAlreadyWhiteListed = true;
            }
        }
        require(isAlreadyWhiteListed, "Is already white listed");
