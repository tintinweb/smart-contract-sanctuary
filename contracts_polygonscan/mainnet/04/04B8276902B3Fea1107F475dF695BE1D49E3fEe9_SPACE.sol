...
contract SPACE is ERC20, AccessControl, ERC721Holder {
    ...
    constructor(address owner, address _collectionAddress) ERC20("SPACE", "SPACE") AccessControl(){
        _mint(owner, 70000 * 10 ** 18);
        ...
        coinAmountPerRewardUnit = 5 * 10 ** 18; // 5 SPACE coins per rewardUnit
        numberOfBlocksPerRewardUnit = 18782; // 12 hours per reward unit
        welcomeBonusAmount = 0 * 10 ** 18; // 0 Only paid once per tokenId
    }
    ...
    function stake(uint256 tokenId) public {
        ...
        require(IERC721(collectionAddress).ownerOf(tokenId) == address(this),
            "SPACE: Error while transferring token");
    ...
    function setCollectionAddress(address newAddress) external onlyRole(CONTRACT_ADMIN_ROLE){
        require (newAddress != address(0), "SPACE: update to zero address not possible");
