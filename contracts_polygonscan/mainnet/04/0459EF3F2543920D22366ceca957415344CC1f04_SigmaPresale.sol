...
contract SigmaPresale is ReentrancyGuard {
	...
	function claimTokens() external nonReentrant {
		...
		require(tokensOwned[msg.sender] > 0, "User should own some SIGMA tokens");
		require(tokensUnclaimed[msg.sender] > 0, "User should have unclaimed SIGMA tokens");
		require(SIGMA.balanceOf(address(this)) >= tokensUnclaimed[msg.sender], "There are not enough SIGMA tokens to transfer.");

		uint256 callerTokensUnclaimed = tokensUnclaimed[msg.sender];
		tokensUnclaimed[msg.sender] = 0;
		SIGMA.safeTransfer(msg.sender, callerTokensUnclaimed);
