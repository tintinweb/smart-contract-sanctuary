...
contract DreamStrategyV2 is Ownable, ReentrancyGuard, Gamificable {
    ...
    function canHarvest(address _user, uint256 _round) external view returns (bool) {
        bool _status = false;
        if (block.timestamp >= userInfo[_user].nextHarvestUntil){
            _status = true;
        } else if (claimHarvest(_user, _round)) {
            _status = true;
        }
        return _status;
    }
    ...
    // View function to see if user can harvest reward token.
    function claimHaverst(address _user, uint256 _round) external {
        if (claimHarvest(_user, _round)) {
            userInfo[_user].nextHarvestUntil = block.timestamp;
            userRounds[_user].lastRoundToHarvest = _round;
        }
    }
...
abstract contract Gamificable is Ownable, Pausable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20Metadata;
    ...
    mapping(address => userRoundData) public userRounds;
    ...
    address public operatorAddress = 0x6D9518bd44fce1ee2EF8d7E3090fBA60304A4ceA;
    ...
    //events
    event StartRound(uint256 indexed roundNumber, uint256 blockNumber);
    event LockRound(uint256 indexed roundNumber, uint256 blockNumber, int256 price);
    event EndRound(uint256 indexed roundNumber, uint256 blockNumber, int256 price);
    ...
    function betBear() external avoidBotCall {
        ledger[_currentRound()][msg.sender].position = Position.Bear;
        userRounds[msg.sender].roundNumber = _currentRound();
        userRounds[msg.sender].position = Position.Bear;
    ...
    function betBull() external avoidBotCall {
        ledger[_currentRound()][msg.sender].position = Position.Bull;
        userRounds[msg.sender].roundNumber = _currentRound();
        userRounds[msg.sender].position = Position.Bear;
    ...
    function genesisStartRound() external onlyOwner whenNotPaused {
        ...
        _safeLockRound(_currentRound(), currentPrice);
    ...
    //     require(
    //         block.number <= rounds[_currentRound()].lockBlock.add(bufferBlocks),
    ...
    function executeRound() external onlyOwner whenNotPaused {
        ...
        _safeEndRound(_currentRound(), currentPrice);

        // abre la siguiente ronda
        Counters.increment(roundCounter);
        _safeLockRound(_currentRound(), currentPrice);
        _safeStartRound(_currentRound());
    }

    /**
     * @dev Get the claimable stats of specific roundNumber and user account
     */
    function claimHarvest(address user, uint256 roundNumber) public view returns (bool) {
     ...
     */
    function _safeStartRound(uint256 roundNumber) internal {
        // require(genesisStartOnce, "Can only run after genesisStartRound is triggered");
        // require(rounds[roundNumber - 2].endBlock != 0, "Can only start round after round n-2 has ended");
        // require(block.number >= rounds[roundNumber - 2].endBlock, "Can only start new round after round n-2 endBlock");
        _startRound(roundNumber);
    }

    function _startRound(uint256 roundNumber) internal {
        Round storage round = rounds[roundNumber];
        round.roundNumber = roundNumber;

        emit StartRound(roundNumber, block.number);
    }

    /**
     * @dev Lock round
     */
    function _safeLockRound(uint256 roundNumber, int256 price) internal {
        // require(rounds[roundNumber].startBlock != 0, "Can only lock round after round has started");
        // require(block.number >= rounds[roundNumber].lockBlock, "Can only lock round after lockBlock");
        // require(block.number <= rounds[roundNumber].lockBlock.add(bufferBlocks), "Can only lock round within bufferBlocks");
        _lockRound(roundNumber, price);
    }

    function _lockRound(uint256 roundNumber, int256 price) internal {
        Round storage round = rounds[roundNumber];
        round.lockPrice = price;

        emit LockRound(roundNumber, block.number, round.lockPrice);
    }

    /**
     * @dev End round
     */
    function _safeEndRound(uint256 roundNumber, int256 price) internal {
        // require(rounds[roundNumber].lockBlock != 0, "Can only end round after round has locked");
        // require(block.number >= rounds[roundNumber].endBlock, "Can only end round after endBlock");
        // require(block.number <= rounds[roundNumber].endBlock.add(bufferBlocks), "Can only end round within bufferBlocks");
        _endRound(roundNumber, price);
    }
    ...
    function _endRound(uint256 roundNumber, int256 price) internal {
        ...
        emit EndRound(roundNumber, block.number, round.closePrice);
    }
    ...
    function _currentRound() internal returns (uint256) {
