    ...
    ERC2981Royalties 
{
    ...
    constructor(
        address _payments
    ) ERC721 ("Lyrics Liberated", "LYRE") {
        payments = payable(_payments); 
        // Setting a 10% Royalties on all secondary sales (ERC2981)
        // percentage (using 2 decimals - 10000 = 100, 0 = 0)
        _setRoyalties(_payments, 1000);
    }


    function mintNFT(
        string memory tokenURI, 
        string memory poemName,
        string memory poemText, 
        string memory author, 
        string memory artist
    ) external payable reentrancyGuard {

        require(_tokenIds.current() < MAX_NFT_SUPPLY, "Poems Already Minted");
        _tokenIds.increment();

        _mint(tokenURI, poemName, poemText, author, artist);
    ...
    function _mint(
        ...
        string memory _poemArtist
    ) internal {
        // save poem on chain
        uint256 ItemId = _tokenIds.current();
        poems[ItemId].text = _poemText;
        poems[ItemId].name = _poemName;
        poems[ItemId].author = _poemAuthor;
        poems[ItemId].artist = _poemArtist;
        // mint token && assign ownership of token to msg.sender
        _safeMint(msg.sender, ItemId);
        _setTokenURI(ItemId, _TokenURI);
        // emit TokenMinted event
        emit TokenMinted(ItemId, _poemName, _poemText, _poemAuthor, _poemArtist);
    }
    ...
    function getArtist(uint256 _tokenID) public view returns (string memory) {
        require(_tokenID <= _tokenIds.current());
        return (poems[_tokenID].artist);
    }

    // Withdraw funds to payment contract
