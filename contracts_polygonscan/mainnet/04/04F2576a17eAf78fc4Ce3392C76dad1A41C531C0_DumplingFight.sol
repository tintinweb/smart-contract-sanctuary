...
contract DumplingFight is Fight {
    ...
    *
    * @dev
    * execute individual Dumpling vs Dumpling fight
    *
    */
    function fightIndividualDumplingVsDumpling(uint _challengerId, uint _enemyId) public
    isValidDumplingChallenger(_challengerId) isValidDumplingEnemy(_enemyId)
    isNotSameId(_challengerId, _enemyId) isOwnerOfDumpling(_challengerId)
    isDumplingAlive(_challengerId) isDumplingExhausted(_challengerId) {
        payByPoo(IConstants(constantsContract).pooTokenForFight());
        Structs.FightEntity memory challenger = createDumplingFightStruct(_challengerId);
        Structs.FightEntity memory enemy = createDumplingFightStruct(_enemyId);

        bool won = executeFight(challenger, enemy);

        settleDumplingFightResult(challenger.id, won);
        ...
        emit BattleResult(Battle.DumplingVsDumpling, won, won ? challenger.id : enemy.id, !won ? challenger.id : enemy.id);
    }

    /**
    *
    * @dev
    * execute individual Dumpling vs Poo fight
    *
    */
    function fightIndividualDumplingVsPoo(uint _challengerId, uint _enemyId) public
    isValidDumplingChallenger(_challengerId) isValidPooEnemy(_enemyId) isOwnerOfDumpling(_challengerId)
    isDumplingAlive(_challengerId) isDumplingExhausted(_challengerId) {
        payByPoo(IConstants(constantsContract).pooTokenForFight());
        Structs.FightEntity memory challenger = createDumplingFightStruct(_challengerId);
        ...
        emit BattleResult(Battle.DumplingVsPoo, won, won ? challenger.id : enemy.id, !won ? challenger.id : enemy.id);
    }

    /**
    *
    * @dev
    * execute random Dumpling vs Dumpling fight
    *
    */
    function fightRandomDumplingVsDumpling(uint _challengerId) public
    isValidDumplingChallenger(_challengerId) isOwnerOfDumpling(_challengerId)
    isDumplingAlive(_challengerId) isDumplingExhausted(_challengerId) {
        ...
        emit BattleResult(Battle.DumplingVsDumpling, won, won ? challenger.id : randomIndex, !won ? challenger.id : randomIndex);
    }

    /**
    *
    * @dev
    * execute random Dumpling vs Poo fight
    *
    */
    function fightRandomDumplingVsPoo(uint _challengerId) public
    isValidDumplingChallenger(_challengerId) isOwnerOfDumpling(_challengerId)
    isDumplingAlive(_challengerId) isDumplingExhausted(_challengerId) {
        Structs.FightEntity memory challenger = createDumplingFightStruct(_challengerId);
        ...
        emit BattleResult(Battle.DumplingVsPoo, won, won ? challenger.id : randomIndex, !won ? challenger.id : randomIndex);
    }
}
