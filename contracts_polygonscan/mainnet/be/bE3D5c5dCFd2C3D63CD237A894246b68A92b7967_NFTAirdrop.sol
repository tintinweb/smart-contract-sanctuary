...
contract NFTAirdrop is Ownable, ERC721Holder {
    using SafeERC20 for IERC20;
    using SafeMath for uint;
    using Random for Random.Seed;
    
    address[] public users;
    uint public length = 1;
    BeastNFT public nft;
    
    constructor() {
        users = [0xBf7f015Db6B89cF4B15960E53b055770a4a6debe];
        nft = BeastNFT(0xBD5be1F746cb21B90bc5B13f2C8CedAcB38a9b15);
    }
    
    function run() public onlyOwner {
        bytes32 seed = blockhash(block.number - 2);
        for (uint8 i = 0; i< length; i++) {
            uint body = uint(seed) % 5;
            uint tokenId = nft.createToken(users[i]);
            nft.updateAbilities(tokenId, 1, body);
            nft.breed(tokenId);
