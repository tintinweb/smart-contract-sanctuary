...
contract MiniTest is ERC721Enumerable, Ownable {
  using Strings for uint256;

  string public _baseTokenURI = "";
  string public _hardURI = "";

  mapping(uint256 => string) private _tokenName;
  mapping(string => bool) private _nameReserved;
  bool public _abbilityToChangeName = false;

  constructor(string memory url) ERC721("TEST", "TEST") {
    _baseTokenURI = url;
  }
  ...
  function giftMany(address[] calldata _to, uint256[] calldata _supply)
    public
    onlyOwner
  {
    for (uint256 i = 0; i < _to.length; i++) {
      address who = _to[i];
      uint256 howMany = _supply[i];
      if (howMany > 1) {
        for (uint256 ii = 0; ii < howMany; ii++) {
          uint256 mintIndex = totalSupply();
          _safeMint(who, mintIndex);
        }
      } else {
        uint256 mintIndex = totalSupply();
        _safeMint(who, mintIndex);
