...
// File contracts/grow/GrowLPStakingPool.sol

// License: MIT
pragma solidity 0.8.6;

contract GrowLPStakingPool is IGrowUpgradeableImplementation, Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    bytes32 override public constant CONTRACT_IDENTIFIER = keccak256("GrowLPStakingPool");
    ...
    /// @notice Address of raw reward token
    IERC20 public rawRewardToken;

    /// @notice Raw Reward Token SWAP Threshold
    uint256 public rawRewardTokenSwapThreshold;
    ...
    function initialize(
        ...
        rewardDurationSeconds = 20000;
        rawRewardTokenSwapThreshold = 1e18;

        STAKING_TOKEN = IERC20(_STAKING_TOKEN);
        rawRewardToken = IERC20(_rawRewardToken);
    }
    ...
    function setRewardDurationSeconds(uint256 _rewardsDurationSeconds) external onlyRole(CONFIGURATOR_ROLE) {
        require(_rewardsDurationSeconds > 0, "StakePool: _rewardsDurationSeconds is not valid");
        updateReward();
        rewardDurationSeconds = _rewardsDurationSeconds;
    }

    function setRawRewardTokenSwapThreshold(uint256 _rawRewardTokenSwapThreshold) external onlyRole(CONFIGURATOR_ROLE) {
        require(_rawRewardTokenSwapThreshold > 0, "StakePool: rawRewardTokenSwapThreshold is not valid");
        rawRewardTokenSwapThreshold = _rawRewardTokenSwapThreshold;
    }
    ...
    function balanceOf(address userAddress) public view returns(uint256) {
        if (totalSupply == 0) {
            ...
            return userShares[userAddress].mul(pricePerShare()).div(_DECIMAL);
        }
    }
    ...
    function pendingRewardsPerShare() public view returns(uint256) {
        ...
        return totalPendingRewards.mul(_DECIMAL).div(totalSupply);
    }
    ...
    function rewardsPerSharePerSecond() public view returns(uint256) {
        if (totalSupply == 0) {
            ...
            return remainingRewardTokenAmount
                .mul(_DECIMAL)
                .div(rewardDurationSeconds).div(totalSupply);
        }
    }
    ...
    function pricePerShare() public view returns(uint256) {
        if (totalSupply == 0) {
            ...
            return totalBalance().mul(_DECIMAL).div(totalSupply).add(pendingRewardsPerShare());
        }
    }
    ...
    function totalBalance() public view returns(uint256) {
        return STAKING_TOKEN.balanceOf(address(this)).sub(remainingRewardTokenAmount);
    }

    function sharesOf(address userAddress) public view returns(uint256) {
        return userShares[userAddress];
    }
    ...
    function addReward(uint256 reward) public {
        ...
        // 2. transfer reward
        STAKING_TOKEN.safeTransferFrom(msg.sender, address(this), reward);
    ...
    function checkIfNeedGetRewardAndZapToLP() external onlyRole(KEEPER_ROLE) {

        // get reward from grow rewarder
        IGrowRewarder(GrowRegister.get.GrowRewarderAddress()).getVaultReward();
        uint256 pendingGrows = rawRewardToken.balanceOf(address(this));

        if (pendingGrows > rawRewardTokenSwapThreshold) {
            // 1. update remaining reward
            updateReward();

            // 2. zap reward to lp token

            uint256 reward = IZAP(GrowRegister.get.ZapAddress()).zapTokenToLP(address(rawRewardToken), rawRewardToken.balanceOf(address(this)), address(STAKING_TOKEN), address(this));
    ...
    function _stakingTokenPriceIn1e6USDC(uint256 amount) public view returns (uint256) {
        return IPriceCalculator(GrowRegister.get.PriceCalculatorAddress()).tokenPriceIn1e6USDC(address(STAKING_TOKEN), amount);
    }

    function depositTo(uint256 amount, address userAddress) external onlyRole(CONTROLLER_ROLE) {
        _deposit(amount, userAddress);
    }

    function deposit(uint256 amount) external onlyHumanOrWhitelisted nonReentrant {
    ...
    function _deposit(uint256 amount, address userAddress) internal {
        ...
        // 2. shares
        uint256 sharesToAdd = amount.mul(_DECIMAL).div(pricePerShare());

        // 3. transfer
        STAKING_TOKEN.safeTransferFrom(msg.sender, address(this), amount);

        // 4. write user shares
        userShares[userAddress] = userShares[userAddress].add(sharesToAdd);
        totalSupply = totalSupply.add(sharesToAdd);

        _depositRewardAddReward(
            userAddress,
            _stakingTokenPriceIn1e6USDC(amount)
        );

        emit LogDeposit(userAddress, amount);
    }

    function _withdraw(uint256 amount, address user, address to) private returns (uint256) {
        require(amount > 0, "amount can not be zero");

        // max amount is balanceOf account
        amount = Math.min(amount, balanceOf(user));

        // 1. update remaining reward
        updateReward();

        // 2. shares
        uint256 sharesToRemove = amount.mul(_DECIMAL).div(pricePerShare());

        // 3. write user shares
        userShares[user] = userShares[user].sub(sharesToRemove);
        totalSupply = totalSupply.sub(sharesToRemove);

        // 4. transfer
        if (to != address(this)) STAKING_TOKEN.safeTransfer(to, amount);

        emit LogWithdraw(user, amount);
    ...
    function _withdrawAs(uint256 wantTokenAmount, address tokenAddress, uint minReceive) private {
        require(wantTokenAmount <= balanceOf(msg.sender));
        uint256 amount = _withdraw(wantTokenAmount, msg.sender, address(this));
        approveToken(address(STAKING_TOKEN), GrowRegister.get.ZapAddress(), amount);
        IZAP(GrowRegister.get.ZapAddress()).zapOut(address(STAKING_TOKEN), tokenAddress, amount, msg.sender, minReceive);
    }

    function withdrawAs(uint256 wantTokenAmount, address tokenAddress, uint minReceive) external nonReentrant {
    ...
    function withdrawAllAs(address tokenAddress, uint minReceive) external nonReentrant {
    ...
    function _depositRewardAddReward(address userAddress, uint256 amountInNativeToken) internal {
        IGrowRewarder(GrowRegister.get.GrowRewarderAddress()).depositRewardAddReward(userAddress, amountInNativeToken);
    }

     /* ========== SALVAGE PURPOSE ONLY ========== */

    function recoverToken(address tokenAddress, uint tokenAmount) external onlyRole(CONFIGURATOR_ROLE) {
        require(tokenAddress != address(rawRewardToken), "StakePool: cannot recover token");

        IERC20(tokenAddress).safeTransfer(GrowRegister.get.PlatformTreasureAddress(), tokenAmount);
