...
contract ExchangeExtractorV3 {
  ...
  function estimateSwaps(address[][] calldata routers, address[][][] calldata paths, uint[] calldata amountsIn) external view returns(uint[] memory) {
      uint[] memory results = new uint[](routers.length);
      for (uint i = 0; i < routers.length; i++) {
          results[i] = this.estimateSwap(routers[i], paths[i], amountsIn[i]);
      }
      return results;
  }
}