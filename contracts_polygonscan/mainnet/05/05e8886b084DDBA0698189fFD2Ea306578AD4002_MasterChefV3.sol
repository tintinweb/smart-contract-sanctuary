...
contract MasterChefV3 is Ownable, ReentrancyGuard {
    ...
    uint256 public constant destructionMaximumSupply = 500000 * (10 ** 3) * (10 ** 18); // 5000 destruction
...
contract DestructionTokenV3 is ERC20, Ownable {
    ...
    modifier antiBot(address _sender, address _recipient, uint256 _amount) { 
        ...
        if (antiBotWorking){
            ...
            if (!_excludedOperatorsFromAntiBot[_sender]){
                if(!_excludedOperatorsFromAntiBot[_recipient])
    ...
    function blacklistCheck(address botAddress) external view returns(bool){
        return _blacklistCheck(botAddress);
    }
     ///@dev check if the address is in the blacklist or expired
    function _blacklistCheck(address botAddress) internal view returns(bool){
