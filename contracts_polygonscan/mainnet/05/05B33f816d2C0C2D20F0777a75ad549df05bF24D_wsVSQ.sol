...
contract wsVSQ is ERC20 {
    ...
    /**
        @notice converts wsVSQ amount to sVSQ
        @param _amount uint
        @return uint
     */
    function wsVSQTosVSQ( uint _amount ) public view returns ( uint ) {
        return _amount.mul( IsVSQ( sVSQ ).index() ).div( 10 ** decimals() );
    }

    /**
        @notice converts sVSQ amount to wsVSQ
        @param _amount uint
        @return uint
     */
    function sVSQTowsVSQ( uint _amount ) public view returns ( uint ) {
        return _amount.mul( 10 ** decimals() ).div( IsVSQ( sVSQ ).index() );
    }
