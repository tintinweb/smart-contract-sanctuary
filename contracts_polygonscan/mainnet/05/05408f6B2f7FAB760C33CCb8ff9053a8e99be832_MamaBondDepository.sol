...
contract MamaBondDepository is Ownable {
    ...
    address public immutable Mama; // token given as payment for bond
    ...
    function deposit( 
        ...
        if ( fee != 0 ) { // fee is transferred to dao 
            IERC20( Mama ).safeTransfer( DAO, fee ); 
        }
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            IERC20( Mama ).transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                IERC20( Mama ).approve( stakingHelper, _amount );
                IStakingHelper( stakingHelper ).stake( _amount, _recipient );
            } else {
                IERC20( Mama ).approve( staking, _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return IERC20( Mama ).totalSupply().mul( terms.maxPayout ).div( 100000 );
    }
    ...
    function bondPrice() public view returns ( uint price_ ) {        
        price_ = terms.controlVariable.mul( debtRatio() ).add( 100000000 ).div( 1e7 );
    ...
    function _bondPrice() internal returns ( uint price_ ) {
        price_ = terms.controlVariable.mul( debtRatio() ).div( 1e7 );
        if ( price_ < terms.minimumPrice ) {
            price_ = terms.minimumPrice;        
    ...
    function debtRatio() public view returns ( uint debtRatio_ ) {   
        uint supply = IERC20( Mama ).totalSupply();
