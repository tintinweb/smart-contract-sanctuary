...
// File: https://github.com/polycatfi/polycat-contracts/blob/master/Vault2/libs/IUniRouter01.sol
...
// File: https://github.com/polycatfi/polycat-contracts/blob/master/Vault2/libs/IUniRouter02.sol
...
// File: https://github.com/polycatfi/polycat-contracts/blob/master/Vault2/libs/IUniPair.sol
...
// File: https://github.com/polycatfi/polycat-contracts/blob/master/Vault2/libs/IStrategyFish.sol
...
// File: https://github.com/polycatfi/polycat-contracts/blob/master/Vault2/BaseStrategy.sol
...
interface  DaiTradingVault  {
  ...
  function withdrawDai( 
   ...
   function daiToClaim(address addr) external view returns(uint256);
   function users(address addr) external view returns (uint256,uint256,uint256,uint256,uint256);
}

contract StrategyGainsDai is BaseStrategy  {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    
    address public constant wmaticAddress = 0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270;
    
   address public immutable gainsDaiVaultAddress;
    address[] public daiToWmaticPath =[wantAddress, wmaticAddress];
    address[] public wmaticToDaiPath = [wmaticAddress, wantAddress];
    address[] public daiToUsdcPath = [wantAddress,usdcAddress];
    address[] public daiToFishPath =[wantAddress,usdcAddress,fishAddress];
  constructor(
        address _vaultChefAddress,
        address _uniRouterAddress,
        address _gainsDaiVaultAddress
    ) public {
        govAddress = msg.sender;
        vaultChefAddress = _vaultChefAddress;
        uniRouterAddress = _uniRouterAddress;
        gainsDaiVaultAddress = _gainsDaiVaultAddress;
...
function withdrawalLimit() public view returns(uint256) {
    return   DaiTradingVault(gainsDaiVaultAddress).daiToClaim(address(this));
}
    ...
    require (_amount < withdrawalLimit(), "amount is higher than withdrawalLimit");
      DaiTradingVault(gainsDaiVaultAddress).withdrawDai(_amount);
    }
    function earn() external nonReentrant whenNotPaused override {
    ...
    {
        (uint256 daiTotal,,,,) = DaiTradingVault(gainsDaiVaultAddress).users(address(this));
      return daiTotal;
    }
    ...
    {
      (uint256 daiTotal,,,,) = DaiTradingVault(gainsDaiVaultAddress).users(address(this));
      return IERC20(wantAddress).balanceOf(address(this)).add(daiTotal);
    }
    function _resetAllowances() internal override
    {
      IERC20(wantAddress).safeApprove(gainsDaiVaultAddress, uint256(0));
    ...
    function _emergencyVaultWithdraw() internal override {
        //This will only claim the maximal amount to claim 
