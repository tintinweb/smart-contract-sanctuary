...
contract WHEAT is ERC20, Ownable {

  // a hard cap for limiting token emition.
  uint256 public constant hardcap = 1000000000 ether;
  // a mapping from an address to whether or not it can mint / burn
  mapping(address => bool) controllers;
  
  constructor() ERC20("WHEAT", "WHT") { 
      // Deployer attached as default controller.
      controllers[msg.sender] = true;
  }

  /**
   * mints $WHEAT to a recipient
   * @param to the recipient of the $WHEAT
   * @param amount the amount of $WHEAT to mint
   */
  function mint(address to, uint256 amount) external {
    require(controllers[msg.sender], "Only controllers can mint");
    require(totalSupply() + amount <= hardcap, "You can NOT mint more than hardcap!");
    _mint(to, amount);
  }

  /**
   * burns $WHEAT from a holder
   * @param from the holder of the $WHEAT
   * @param amount the amount of $WHEAT to burn
   */
...
// File: contracts/CharacterMinter.sol
...
contract CharacterMinter  is ICharacter,  ERC721Enumerable, Ownable, VRFConsumerBase, Pausable {

  event Response(bool success, bytes data);
  // Reveal enabled?
  bool public reveal;
  // Dummy tokenURI until reveal is enabled
  string public dummyURI;
  // mint price
  uint256 public constant MINT_PRICE = 10 ether;
  // max number of tokens that can be minted - 50.000 
  uint256 public immutable MAX_TOKENS;
  // number of tokens that can be claimed for initial minting by Matic Token - 20% of MAX_TOKENS
  uint256 public PAID_TOKENS;
  // number of tokens have been minted so far
  uint16 public minted;
  // KeyHash for Link Oracle
  bytes32 keyHash;

  // mapping from tokenId to a struct containing the token's traits
  mapping(uint256 => Character) public tokenCharacteristic;

  // mapping from hashed(tokenCharacteristic) to the tokenId it's associated with
  // used to ensure there are no duplicates
  mapping(uint256 => uint256) public existingCombinations;

  // mapping from VRF RequestID to tokenId
  mapping(bytes32 => uint16) public seedRequests;

  // mapping from tokenId to VRF generated seeds.
  mapping(uint16 => uint256) public tokenSeeds;

  // reference to $WHEAT for burning on mint
  WHEAT public wheat;

  //reference to Generator contract
  Generator public generator;

  // reference to characters
  ICharacter public characters;

    /** 
   * Instantiates contract
   */
  constructor(address _wheat, address payable _generator, address _vrfcoordinator, address _linkToken, uint256 _maxTokens,  bytes32 _keyhash) ERC721("Citizens", 'CTZN') 
    VRFConsumerBase
    (
  ...
  // Fullfill randomness function will generate actual NFT properties
  function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
      // Generating random properties of pre-mined NFT. This function can only be called by VRF coordinator contract. See VRFConsumerbase.sol.
      //Getting erc721id from mapping
      uint16 _minted = seedRequests[requestId];

      // Random seed request sent to VRF Oracle, see fulfillRandomness in order to handle callback
        tokenSeeds[_minted] = randomness;
  }

  /* Since VRF Callback only have 200,000 gas and generate function has multiple recursive function, some generating operations may use excessive gas and
  *  VRF call back function will fail and revert. In order to prevent this, fullfillRandomness function shall be as possible as basic and other process shall be done by
  *  owner at later stage. Minting flow will be like this;
  *  User mints an NFT -> Smart Contract saves randomID#1 -> VRF callback fuction calls fulfillRandomness#2 ->  fulfillRandomness saves random seed given by VRF -> Reveal#3
  *  function does remaining calculations. So, gas of remaining operations will be provided by owner of NFT.
  *  #1 see seedRequests variable.
  *  #2 see fulfillRandomness function.
  *  #3 see reveal function below
  *
  *WARNING! This flow ables to user to pre-calculate NFT. In blockchains which require high gas fees like Etherium, 
  *some users may try to abuse this an try to only revealing valuable NFTs. But, in cheap blockchains like Polygon(which is intented to publish this smart contract)
  *this will only create a negligible advantage on behalf of user(minter). Similar method used against Wolf Game, but here generated seed has an outsource(look ChainlinkVRF)
  *and imposible to generate without knowledge of exact algorithm of Chainlink VRF which has off-chain calculations in order to providing randomness(still not exact Random :) )
  */

  /** 
   * mint a token - 90% Peasants, 9% Thieves, %1 Militia
   * The first 20% are 10 Matic per, the remaining cost $WHEAT
   * As planned %50 of the accumulated matic tokens during initial minting will be pooled with reserved $WHEAT tokens. Planned pooling is 100.000 Matic => 1B WHEAT
   */
  function mint(uint256 amount) external payable whenNotPaused{
    require(tx.origin == _msgSender(), "Only EOA");
    require(LINK.balanceOf(address(this)) >= 0.1 ether, "Not enough LINK - fill contract with faucet");
    ...
    for (uint i = 0; i < amount; i++) {
        ...
        // Random seed request sent to VRF Oracle, see fulfillRandomness in order to handle callback
        seedRequests[requestRandomness(keyHash, 0.0001 ether)] = minted;
        totalWoolCost += mintCost(minted);
    }
    
    if (totalWoolCost > 0) wheat.burn(_msgSender(), totalWoolCost);
  }

  /** 
   * the first 20% are paid in MATIC
   * the next 20% are 8000 $WHEAT
   * the next 20% are 16000 $WHEAT
   * the next 20% are 32000 $WHEAT
   * the next 20% are 440000 $WHEAT
   * @param tokenId the ID to check the cost of to mint
   * return the cost of the given token ID
   */
  function mintCost(uint256 tokenId) public view returns (uint256 value) {
    if (tokenId <= PAID_TOKENS) return 0;
    if (tokenId <= MAX_TOKENS * 2 / 5) return 8000 ether;
    if (tokenId <= MAX_TOKENS * 3 / 5) return 16000 ether;
    if (tokenId <= MAX_TOKENS * 4 / 5) return 32000 ether;
    return 44000 ether;
  }

    /**
   * Reveal components for a specific token, checking to make sure it's unique
   * @param tokenId the id of the token to generate components for
   * @return t - a struct of traits for the given token ID
   */
  function revealToken(uint16 tokenId) public whenNotPaused returns (Character memory t) {
    require(reveal, "Character reveal is not enabled. Please be patient!");
    t = generator.selectTraits(tokenSeeds[tokenId]);
    if (existingCombinations[structToHash(t)] == 0) {
      tokenCharacteristic[tokenId] = t;
      existingCombinations[structToHash(t)] = tokenId;
      return t;
    }

    // In order to prevent revealing same treats, seed will be shifted and retry of revealing. This may cause EXTENSIVE RECURSION AND HIGH CONSUME!
    tokenSeeds[tokenId] >>= 16;
    revealToken(tokenId);
  }
  
  /* Toggle revealability
  ...
  */
    ...
    function changeDummyURI(string memory _dummy) external onlyOwner {
      dummyURI = _dummy;
    }
   ...
   */
  function structToHash(Character memory s) internal pure returns (uint256) {
  ...
  function getTokenSeed(uint16 _tokenId) external view returns(uint256){
     return tokenSeeds[_tokenId];
  }

  function getTokenComponents(uint256 tokenId) public view override returns (Character memory) {
    return tokenCharacteristic[tokenId];
  }

  function getPaidTokens() public view override returns (uint256) {
    return PAID_TOKENS;
  }
  ...
  function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if(reveal)
        {
          return generator.tokenURI(tokenId, getTokenComponents(tokenId),  getPaidTokens());
        }
        else
        {
          return dummyURI;
