...
contract ProtectionResolverV2 is ProtectionResolver {
    ...
    function getBestColAndDebtData(address _user)
        external
        view
        returns (BestColAndDebtDataResult memory result)
    {
        ...
        for (uint256 i = 0; i < reserveList.length; i++) {
            if (!userConfig.isUsingAsCollateralOrBorrowing(i)) continue;
            {
                ...
                vars.priceInETH = IPriceOracle(
                    protectionActionV2.ADDRESSES_PROVIDER().getPriceOracle()
                ).getAssetPrice(reserveList[i]);

                vars.collateralBalanceInETH =
                    (vars.priceInETH *
                        IERC20(reserveData.aTokenAddress).balanceOf(_user)) /
                    10**vars.decimals;
                vars.stableDebtTokenBalanceInETH = userConfig.isBorrowing(i)
                    ? (vars.priceInETH *
                        IERC20(reserveData.stableDebtTokenAddress).balanceOf(
                            _user
                        )) / 10**vars.decimals
                    : 0;
                vars.variableDebtTokenBalanceInETH =
                    (vars.priceInETH *
                        IERC20(reserveData.variableDebtTokenAddress).balanceOf(
                            _user
                        )) /
                    10**vars.decimals;
        ...
        result.flashloanPremium = protectionActionV2
            .LENDING_POOL()
            .FLASHLOAN_PREMIUM_TOTAL();
        result.protectionFee = protectionAction.protectionFeeBps();
    }
