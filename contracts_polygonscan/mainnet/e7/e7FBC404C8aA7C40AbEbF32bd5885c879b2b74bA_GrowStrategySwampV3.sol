...
interface IZAP {
    function swap(address _from, address _to, uint amount, address receiver) external returns (uint);
}

abstract contract BaseGrowStrategyV3 is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function __base_initialize() internal initializer {
        __AccessControl_init();
        __ReentrancyGuard_init();
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);

        rewardDurationBlocks = 64000;
        performanceFeeRate = 1500;
        platformFeeRate = 50;
        performanceFeeThreshold = 1e18;
    ...
    function _chargePerformanceFeeAndPlatformFee(address performanceTokenAddress, uint256 performanceTokenAmount, bool baseOnFee) internal {
        ...
        approveToken(performanceTokenAddress, address(this), swapPerformanceTokenAmount);
        uint256 growTokenAmount = IZAP(GrowRegister.get.ZapAddress()).swap(performanceTokenAddress, growToken, swapPerformanceTokenAmount, address(this));
    ...
    function _unsettledPerformanceFee(uint256 underlyingWantTokenAmount) internal view returns(uint256) {
        return (unsettledProfit.add(underlyingWantTokenAmount.sub(lastPricePerShare.mul(totalShares).div(_DECIMAL)))).mul(performanceFeeRate).div(10000);
    }
    ...
    function depositTo(uint256 wantTokenAmount, address userAddress) external onlyRole(CONTROLLER_ROLE) {
        _getRewards(userAddress);
    ...
    function _deposit(uint256 wantTokenAmount, address userAddress) internal {
        ...
        if (rewarderEnabled) {
            ...
            _depositRewardAddReward(
                userAddress,
                _wantTokenPriceInUSD(wantTokenAdded)
            );
        }
    ...
    function _wantTokenPriceInUSD(uint256 amount) public view virtual returns (uint256);
...
// File contracts/strategies/GrowStrategySwampV3.sol
pragma solidity 0.8.6;
...
contract GrowStrategySwampV3 is BaseGrowStrategyV3 {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    ...
    function _trySwapUnderlyingRewardToRewardToken() internal override {
        ...
        _chargePerformanceFeeAndPlatformFee(UNDERLYING_REWARD_TOKEN, rewardTokenAmount, false);
    }
    ...
    function _tryChargePerformanceFee() internal override {
        ...
        _chargePerformanceFeeAndPlatformFee(STAKING_TOKEN, withdrawnWantTokenAmount, true);
    ...
    function _wantTokenPriceInUSD(uint256 amount) public view override returns (uint256) {
        return IPriceCalculator(GrowRegister.get.PriceCalculatorAddress()).tokenPriceInUSD(STAKING_TOKEN, amount);
    }
