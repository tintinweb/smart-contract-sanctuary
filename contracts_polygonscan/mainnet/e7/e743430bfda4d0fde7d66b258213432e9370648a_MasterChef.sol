...
library SafeBEP20 {
    ...
    function safeTransfer(IKAI20 token, address to, uint256 value) internal {
        _callOptionalReturn(token, abi.encodeWithSelector(token.transfer.selector, to, value));
    }

    function safeTransferFrom(IKAI20 token, address from, address to, uint256 value) internal {
        _callOptionalReturn(token, abi.encodeWithSelector(token.transferFrom.selector, from, to, value));
    }

    /**
     * @dev Deprecated. This function has issues similar to the ones found in
     * {IKAI20-approve}, and its usage is discouraged.
     *
     * Whenever possible, use {safeIncreaseAllowance} and
     * {safeDecreaseAllowance} instead.
     */
    function safeApprove(IKAI20 token, address spender, uint256 value) internal {
    ...
    function safeIncreaseAllowance(IKAI20 token, address spender, uint256 value) internal {
        uint256 newAllowance = token.allowance(address(this), spender).add(value);
        _callOptionalReturn(token, abi.encodeWithSelector(token.approve.selector, spender, newAllowance));
    }

    function safeDecreaseAllowance(IKAI20 token, address spender, uint256 value) internal {
     ...
     */
    function _callOptionalReturn(IKAI20 token, bytes memory data) private {
...
contract MasterChef is Ownable {
    ...
    // Info of each pool.
    struct PoolInfo {
        IKAI20 lpToken;           // Address of LP token contract.
    ...
    function add(uint256 _allocPoint, IKAI20 _lpToken, uint16 _depositFeeBP, bool _withUpdate) public onlyOwner {
