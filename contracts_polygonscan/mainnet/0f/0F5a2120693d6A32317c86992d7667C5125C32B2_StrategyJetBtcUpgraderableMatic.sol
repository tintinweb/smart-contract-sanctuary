...
// File: contracts/matic/StrategyJetBtcUpgraderableMatic.sol
...
contract StrategyJetBtcUpgraderableMatic is ReentrancyGuardUpgradeSafe, OwnableUpgradeable , Pausable {
    ...
    function initialize(
        ...
        if (isGrandComp) {
            ...
            earnedToToken0Path = [earnedAddress, maticAddress,ethAddress,token0Address];
            if (ethAddress == token0Address) {
                earnedToToken0Path = [earnedAddress, maticAddress,ethAddress];
            }

            earnedToToken1Path = [earnedAddress, maticAddress,ethAddress,token1Address];
            if (ethAddress == token1Address) {
                earnedToToken1Path = [earnedAddress, maticAddress,ethAddress];
            }

            token0ToEarnedPath = [token0Address, ethAddress,maticAddress, earnedAddress];
            if (ethAddress == token0Address) {
                token0ToEarnedPath = [ethAddress,maticAddress, earnedAddress];
            }

            token1ToEarnedPath = [token1Address,ethAddress, maticAddress,earnedAddress];
            if (ethAddress == token1Address) {
                token1ToEarnedPath = [ethAddress,maticAddress, earnedAddress];
            }
        }
