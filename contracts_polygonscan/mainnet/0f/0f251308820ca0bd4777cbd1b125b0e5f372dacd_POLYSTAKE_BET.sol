    ...
    function payBetWin() external onlyOwner {    
        ...
        if (currentBet[cb].betWin == 1) {
            ...
            for(uint i = 0; i < currentBet[cb].bettors1.length; i++) {
               users[currentBet[cb].bettors1[i]].bet.profits =  users[currentBet[cb].bettors1[i]].bet.profits.add(Pot_Pay.add(users[currentBet[cb].bettors1[i]].bet.betAmount[cb][0]).div(Pot_Win)); 
            } 
        } else 
