...
contract GrowRegisterStorage {
    ...
    address public constant GrowRewarderAddress = 0x9B5754beF8a45cB46FF2Cf335386548146199703;
    address public constant GrowStakingPoolAddress = 0x1735783c26731789933A8Cd308253a5F4FFA0802;
    ...
    address public constant GrowMembershipPoolAddress = 0x2B99451787B0Fc68143A1a8Ca77Bb0B319dc8449;
    address public constant GrowTestPilotAddress = 0x1C07b30D100C8EC6aBf70f0776af1e2a1F35D150;

    uint public constant TestFlightEndTime = 1628842862;
}
...
contract GrowRewarder is AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    modifier onlyNotVault() {
        require(!strategies[msg.sender].isVault, "GR: is vault");
        _;
    }
    ...
    function depositRewardAddReward(address userAddress, uint256 amountIn1e6USDC) external onlyActiveStrategy {
    ...
    function profitRewardByContribution(uint256 profitGrowAmount) external onlyActiveStrategy {
    ...
    function settleRewards(address strategyAddress, address userAddress) private {
        ...
        if (currentUserShares > 0 && !strategies[strategyAddress].isVault) {
            // Block reward
    ...
    function getRewards(address userAddress) external onlyActiveStrategy {
        settleAndSendRewards(msg.sender, userAddress);
    }

    function getVaultReward() external onlyActiveStrategy onlyVault {
    ...
    function notifyUserSharesUpdate(address userAddress, uint256 sharesUpdateTo, bool isWithdraw) external onlyActiveStrategy onlyNotVault {
        address strategyAddress = msg.sender;
...
contract Zap is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function zapOut(address fromToken, address toToken, uint amount, address receiver, uint minReceive) external {
        ...
        if (fromToken != toToken) {
            ...
            if (tokenRouter[fromToken] == fromToken) {
                ...
                _swap(fromToken, toToken, amount, receiver, minReceive);
            } else {
                // fromToken is lp token
                _zapOutLpToToken(fromToken, amount, toToken, receiver, minReceive);
            }
        } else {
    ...
    function getZapOutAmountOut(address wantToken, uint amount, address poolAddress) external view returns (uint wantTokenAmount) {
        ...
        address fromToken = IGrowPool(poolAddress).STAKING_TOKEN();
        wantTokenAmount = amount;
        if (fromToken != wantToken) {
            require(tokenRouter[fromToken] != address(0) , "fromToken invalid");
            if (tokenRouter[fromToken] == fromToken) {
