...
contract PolygonArbitrageBot is PolygonTradingHub {
    ...
    constructor()
    {
        ...
        abitrageIntents[0].tokenB = kPolygonToken.elon;

        abitrageIntents[1].tokenA = kPolygonToken.wmatic;
        abitrageIntents[1].tokenB = kPolygonToken.elon;
...
function _calculatePercentage1000PriceImpact(uint _amountPerMille, uint256 _amount) pure returns(uint256)
{
    return 100000 - (_amount * 100 * 1000 / (_amountPerMille * 1000));
}
...
contract PolygonTradingHub is QuickSwapTrader, CafeSwapTrader, JetSwapTrader, SushiSwapTrader {
    ...
    uint256 constant MAX_PRICE_IMPACT_PERCENTAGE_1000 = 5000;
    ...
    function _polygonTradingHubGetForcast(kPolygonExchange _exchange, kPolygonToken _tokenIn, kPolygonToken _tokenOut, uint256 _amount) internal view returns(PolygonTradeForecast memory)
    {
        ...
        if (_calculatePercentage1000PriceImpact(tokenPerMille, forecast.tokenOutAmount) > MAX_PRICE_IMPACT_PERCENTAGE_1000)
        {
            // ignoring result;
