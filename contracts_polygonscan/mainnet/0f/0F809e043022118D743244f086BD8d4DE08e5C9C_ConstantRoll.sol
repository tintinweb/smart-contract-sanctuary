...
contract ConstantRoll is VRFConsumerBase, Ownable {
  ...
  uint256 public maxBetMask = 98;
  uint256 public minBetMask = 2;
  uint256 public useModulo = 100;
  ...
  uint256 public alwaysBetMask = 98;
  ...
  uint256 waitForRounds = 2;
  uint256 private rI = 0;
  uint256 public minProfit = 100;
  ...
  event Bet(uint256 wag, uint256 betMask, uint256 currentBalance);
  ...
  function setAlwaysBetMask(uint256 number_) public onlyOwner {
  ...
  function setMinProfit(uint256 number_) external onlyOwner {
    minProfit = number_;
  }
  ...
  function takeProfit() internal returns(uint256){
    if(address(this).balance > startBalance.add(minProfit)){
      ...
      payable(owner()).transfer(address(this).balance.sub(startBalance));
      lastBalance = address(this).balance;
      return profit;
    }
    return 0;
  ...
  function fulfillRandomness(bytes32 requestId, uint256 randomValue) internal override {
    rI = rI + 1;
    roll(uint256(keccak256(abi.encode(randomValue, rI))) % maxBetMask + minBetMask, requestId);   
  }
    
  function roll(uint256 betMask, bytes32 requestId) internal {
    ...
    if(wasPendingBet > waitForRounds || pendingBet == 0){
      ...
      if(address(this).balance >= betAmount){
        ...
        if(alwaysBetMask == 0){
          ...
          emit Bet(betAmount, betMask, address(this).balance);
        }else{
          polyroll.placeBet{ value: betAmount }(alwaysBetMask, useModulo, owner());
          emit Bet(betAmount, alwaysBetMask, address(this).balance);
        }
