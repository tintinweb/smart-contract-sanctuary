...
contract Strategy_DINO_WETH is Ownable {
    // This strategy is for polygon network. DINO - WETH pair. RewardToken = "DINO"
    ...
    address public token1; // DINO
    address public vault;

    // ------------> Important constants <--------------
    bool public hasPid = true;
    uint public pid = 11;
    address public constant lpToken = 0x9f03309A588e33A239Bf49ed8D68b2D45C7A1F11;
    address public constant farm = 0x1948abC5400Aa1d72223882958Da3bec643fb4E5;
    address public router = 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff; // uses for swapping DINO - WETH 
    address public routerMain = 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff; // uses for swapping WETH - USDT
    address public routerInternal = 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff;
    address public routerWMATIC = 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff; // uses for swapping WETH - WMATIC
    ...
    function getBalanceOfToken(address _token) public view returns (uint256) {
        if(_token == token1) {
            return IERC20(_token).balanceOf(address(this)) - pendingFeeDINO;
        }
        if(_token == token0) {
    ...
    function _takeFee() internal {
        uint256 rewardsFeeInDINO = _calculateAmountFee(getBalanceOfToken(token1));
        bool _swapped = _swapExactTokenForToken(router, token1, token0, rewardsFeeInDINO+pendingFeeDINO);
        if (_swapped) {
            pendingFeeDINO = 0;
            emit TakeFee(rewardsFeeInDINO);
        } else {
            pendingFeeDINO += rewardsFeeInDINO;
        }

        _swapped = _swapExactTokenForToken(router, token0, USDT, pendingFeeWETH);
        if (_swapped) {
            emit TakeFee(pendingFeeWETH);
            pendingFeeWETH = 0;
        }
    }
    ...
    function _addLiquidityFromAllTokens() internal {
        _swapExactTokenForToken(router, WMATIC, token0, getBalanceOfToken(WMATIC));
    ...
    function _swapLPtoWMATIC(uint256 _percent) internal {
        ...
        if (_totalLP > 0) {
            ...
            _swapExactTokenForToken(router, token0, WMATIC, getBalanceOfToken(token0));
        }
    }
    ...
    function _getToken1Price() public view returns (uint256) {
        ...
        path[0] = token0;
        path[1] = token1;
        return (10 * 1e36) / _getAmountsOut(router, 10 * 1e18, path);
    }

    function _getPriceWMATICToWETH() internal view returns (uint256 result) {
        address[] memory path = new address[](2);
        path[0] = WMATIC;
        path[1] = token0;
        result = (50 * 1e36) / _getAmountsOut(router, 50 * 1e18, path);
    }
    ...
    function _getSimpleTCI(uint256 _amount) internal view returns (uint256 tCI) {
        ...
        if(_amount > 0) {
            ...
            tCI = (t0V + _getToken1Price() * t1V / d) * _getPriceWMATICToWETH() / d;
        }
        ...
        uint256 amountWETH;
        if (_balance1 > 0) {
            amountWETH = _getToken1Price() * _balance1 / d;
        }

        if (_balance0 > 0) {
            tCI += _getPriceWMATICToWETH() * (_balance0 + amountWETH) / d;
        }
