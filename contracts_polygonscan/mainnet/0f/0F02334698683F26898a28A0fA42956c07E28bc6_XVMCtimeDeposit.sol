...
interface IGovernance {
    ...
    function stakeRolloverBonus(address _giftTo, address _depositToPool, uint256 _bonusToPay, uint256 _stakeID) external;
}
 ...
 */
contract XVMCtimeDeposit is Ownable {
     ...
     * If they increase the stakes, it could run out of gas during staking and migration
     */
    function deposit(uint256 _amount) external {
        ...
        userInfo[msg.sender].push(
                UserInfo(currentShares, block.timestamp, currentShares.mul(balanceOf()).div(totalShares),
                    block.timestamp, 0)
            );
    ...
    function giftDeposit(uint256 _amount, address _toAddress, uint256 _minToServeInSecs) external {
        ...
        require(_minToServeInSecs >= 0, "days to serve must be within limits");
        ...
        userInfo[_toAddress].push(
                UserInfo(currentShares, block.timestamp, currentShares.mul(balanceOf()).div(totalShares),
                    block.timestamp, _minToServeInSecs)
            );
    ...
    */
    ...
    function hopStakeToAnotherPool(uint256 _shares, uint256 _stakeID, address _recipient, address _poolAddress) public {
        ...
        if(_poolAddress == migrationPool) {
            ...
            user = userInfo[_recipient][_stakeID];
            ...
            user = userInfo[msg.sender][_stakeID];
        }
        require(_shares > 0, "Nothing to withdraw");
        require(_shares <= user.shares, "Withdraw amount exceeds balance");
        if(!partialWithdrawals) { require(_shares == user.shares, "must transfer full stake"); } 
        ...
        if(userVote[msg.sender].delegatingSharesToProposalID != 0) {
            _updateVotingSubDiff(_recipient, userVote[msg.sender].delegatingSharesToProposalID, _shares);
        }
    ...
    function hopDeposit(uint256 _amount, address _recipientAddress, uint256 previousLastDepositedTime, uint256 _mandatoryTime) external {
        ...
        userInfo[_recipientAddress].push(
                UserInfo(currentShares, previousLastDepositedTime, currentShares.mul(balanceOf()).div(totalShares),
    ...
    */
    ...
    function voteForProposal(uint256 proposalID) external {
        ...
        if(proposalID != 0) { //resets to nothing(not sure if needed)
            if(userVote[msg.sender].delegatingSharesToProposalID != 0) {
