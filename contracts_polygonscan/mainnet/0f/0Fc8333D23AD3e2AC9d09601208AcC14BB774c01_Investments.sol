...
contract Investments is Ownable {
	...
	function getAaveBalanceV2(address token, address account)
		...
		returns (uint256)
	{
		(, , , , , , , , bool isActive, ) = aaveDataProviderV2
			.getReserveConfigurationData(token == MATIC ? WMATIC : token);
	...
	function getCreamBalance(address token, address user)
		...
		returns (uint256)
	{
		address cToken = memoryContract.getCrToken(token);

		if (cToken == address(0)) return 0;

		(, uint256 balance, , uint256 rate) = ICToken(cToken)
