...
// File: contracts/libs/IPearzap.sol
pragma solidity 0.6.12;
...
// File: contracts/strategy/StrategyPZWmaticPear.sol
pragma solidity 0.6.12;

contract StrategyPZWmaticPear is BaseStrategyLPSingle {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    ...
    constructor(
        ...
        uniRouterAddress = 0x3a1D87f206D12415f5b0A33E786967680AAb4f6d; // WaultSwap UniRouter
    ...
    function _vaultDeposit(uint256 _amount) internal override {
        IPearzap(stakingRewardsAddress).deposit(pid, _amount, feeAddress);
    }

    function _vaultWithdraw(uint256 _amount) internal override {
        IPearzap(stakingRewardsAddress).withdraw(pid, _amount);
    }

    function _vaultHarvest() internal override {
        IPearzap(stakingRewardsAddress).withdraw(pid, 0);
    }

    function totalInUnderlying() public override view returns (uint256) {
        (uint256 amount,) = IPearzap(stakingRewardsAddress).userInfo(pid, address(this));
        return amount;
    }
    ...
    function _emergencyVaultWithdraw() internal override {
        IPearzap(stakingRewardsAddress).emergencyWithdraw(pid);
    }
}