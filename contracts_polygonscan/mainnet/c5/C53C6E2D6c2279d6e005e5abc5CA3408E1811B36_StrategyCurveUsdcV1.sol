...
abstract contract StrategyBase {
    ...
    address public constant maticUsdcBuyback = 0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174;
    ...
    function figureOutPath(address _from, address _to, uint256 _amount) public view returns (bool useSushi, address[] memory swapPath){
        ...
        if (_to == mmToken && buybackEnabled == true) {
            if (_from == usdcBuyback || _from == WFTM || _from == wbnb || _from == WMATIC){
                ...
                sushipath[0] = _from;
                sushipath[1] = _to;
            }else if(_from == maticUsdcBuyback){
...
abstract contract StrategyCurveFarmBase is StrategyBase {
    ...
    function getHarvestable() public view returns (uint256) {
        return LpTokenStaker(stakingPool).claimable_reward(address(this), CRV);
    }
...
contract StrategyCurveUsdcV1 is StrategyCurveFarmBase {
    ...
    // staking constants
    address public constant stakingGauge = 0x19793B454D3AfC7b454F206Ffe95aDE26cA6912c;
    address public constant pool2USDC = 0x445FE580eF8d70FF569aB36e80c647af338db351;
    address public constant lp2USDC = 0xE7a24EF0C5e95Ffb0f6684b813A78F2a3AD7D171;
    ...
    constructor(
        ...
        public StrategyCurveFarmBase(usdc, lp2USDC, stakingGauge, _governance, _strategist, _controller, _timelock)
    {		        
        IERC20(usdc).safeApprove(pool2USDC, uint256(-1));
        IERC20(lp2USDC).safeApprove(pool2USDC, uint256(-1));
    }
    ...
    function balanceOfPool() public override view returns (uint256){
        uint256 _lp2USDC = IERC20(lp2USDC).balanceOf(address(this)).add(_balanceInStakingPool());
        return lp2USDCToWant(_lp2USDC);
    }
    ...
    function deposit() public override {
        ...
        if (_wantAmt > 0 && checkSlip(_wantAmt, _maxSlip)) {
            ...
            ICurveFi_3(pool2USDC).add_liquidity(amounts, _maxSlip, true);
        }
    ...
    function _withdrawSome(uint256 _amount) internal override returns (uint256) {
        ...
        uint256 required2USDC = estimateRequiredLP(_amount);
        required2USDC = required2USDC.mul(DENOMINATOR.add(slippageProtectionOut)).div(DENOMINATOR);// try to remove bit more
		
        uint256 _lp2USDC = IERC20(lp2USDC).balanceOf(address(this));
        uint256 _withdrawFromStaking = _lp2USDC < required2USDC? required2USDC.sub(_lp2USDC) : 0;
        ...
        _lp2USDC = IERC20(lp2USDC).balanceOf(address(this));
        if (_lp2USDC > 0){
            required2USDC = required2USDC > _lp2USDC?  _lp2USDC : required2USDC;

            uint256 maxSlippage = required2USDC.mul(DENOMINATOR.sub(slippageProtectionOut)).div(DENOMINATOR);

            if (want_decimals < 18) {
                maxSlippage = maxSlippage.div(10**(uint256(uint8(18) - want_decimals)));
                required2USDC = required2USDC.div(10**(uint256(uint8(18) - want_decimals)));
            }
            uint256[3] memory amounts = [0, required2USDC, 0];
            ICurveFi_3(pool2USDC).remove_liquidity_imbalance(amounts, maxSlippage, true);
        }
    ...
    function harvest() public override onlyBenignCallers {
        ...
        if (_rewards > 0) {
            _swapUniswap(CRV, WMATIC, _rewards);
        }
    ...
    function virtualPriceToWant() public view returns (uint256) {
        if(want_decimals < 18){
            return ICurveFi_3(pool2USDC).get_virtual_price().div(10 ** (uint256(uint8(18) - want_decimals)));
        }else{
            return ICurveFi_3(pool2USDC).get_virtual_price();
        }
    }
    ...
    function checkSlip(uint256 _wantAmt, uint256 _maxSlip) public view returns (bool){
        uint256[3] memory amounts = [0, _wantAmt, 0];
        return ICurveFi_3(pool2USDC).calc_token_amount(amounts, true) >= _maxSlip;
    }
	
    function lp2USDCToWant(uint256 _lp2USDC) public view returns (uint256) {
        if (_lp2USDC == 0){
            return 0;
        }
        // use underestimate of current assets. 
        uint256 virtualOut = virtualPriceToWant().mul(_lp2USDC).div(1e18);
        return virtualOut; 
    }
