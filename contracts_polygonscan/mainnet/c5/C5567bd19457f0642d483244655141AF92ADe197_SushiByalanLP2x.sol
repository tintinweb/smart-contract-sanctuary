...
contract SushiByalanLP2x is ByalanIsland, Sailor, IByalan {
    ...
    address public constant MASTERCHEF = 0x0769fd68dFb93167989C6f7254cd0D766Fb2841F;
    uint256 public immutable pid;

    // Routes
    address[] public sushiToWmaticRoute;
    address[] public sushiToLp0Route;
    address[] public sushiToLp1Route;
    address[] public bonusRewardToLp0Route;
    address[] public bonusRewardToLp1Route;

    bool public addLpAfterEmergency = true;

    event Harvest(address indexed harvester);

    constructor(
        address _bonusReward,
        address _hydra,
        address _izlude,
        ...
        address _harvester,
        uint256 _pid,
        address[] memory _sushiToLp0Route,
        address[] memory _sushiToLp1Route,
        address[] memory _bonusRewardToLp0Route,
        address[] memory _bonusRewardToLp1Route
    )
        ByalanIsland(
            _hydra,
        ...
        )
    {
        ...
        pid = _pid;
        want = IMasterChef(MASTERCHEF).lpToken(_pid);
        lpToken0 = IUniswapV2Pair(want).token0();
        lpToken1 = IUniswapV2Pair(want).token1();

        sushiToWmaticRoute = [SUSHI, WMATIC];

        if (lpToken0 != SUSHI) {
            require(_sushiToLp0Route[0] == SUSHI, "invalid lp 0 route");
            require(_sushiToLp0Route[_sushiToLp0Route.length - 1] == lpToken0, "invalid lp 0 route");
            require(
                IUniswapV2Router02(unirouter).getAmountsOut(1 ether, _sushiToLp0Route)[_sushiToLp0Route.length - 1] > 0,
                "invalid lp 0 route"
            );
            sushiToLp0Route = _sushiToLp0Route;
        }

        if (lpToken1 != SUSHI) {
            require(_sushiToLp1Route[0] == SUSHI, "invalid lp 1 route");
            require(_sushiToLp1Route[_sushiToLp1Route.length - 1] == lpToken1, "invalid lp 1 route");
            require(
                IUniswapV2Router02(unirouter).getAmountsOut(1 ether, _sushiToLp1Route)[_sushiToLp1Route.length - 1] > 0,
                "invalid lp 1 route"
            );
            sushiToLp1Route = _sushiToLp1Route;
        }

        if (lpToken0 != _bonusReward) {
            require(_bonusRewardToLp0Route[0] == _bonusReward, "invalid lp 0 route");
            require(_bonusRewardToLp0Route[_bonusRewardToLp0Route.length - 1] == lpToken0, "invalid lp 0 route");
            require(
                IUniswapV2Router02(unirouter).getAmountsOut(1 ether, _bonusRewardToLp0Route)[
                    _bonusRewardToLp0Route.length - 1
                ] > 0,
                "invalid lp 0 route"
            );
            bonusRewardToLp0Route = _bonusRewardToLp0Route;
        }

        if (lpToken1 != _bonusReward) {
            require(_bonusRewardToLp1Route[0] == _bonusReward, "invalid lp 1 route");
            require(_bonusRewardToLp1Route[_bonusRewardToLp1Route.length - 1] == lpToken1, "invalid lp 1 route");
            require(
                IUniswapV2Router02(unirouter).getAmountsOut(1 ether, _bonusRewardToLp1Route)[
                    _bonusRewardToLp1Route.length - 1
                ] > 0,
                "invalid lp 1 route"
            );
            bonusRewardToLp1Route = _bonusRewardToLp1Route;
        }
    ...
    function deposit() public override whenNotPaused {
        ...
        if (wantBal > 0) {
            IMasterChef(MASTERCHEF).deposit(pid, wantBal, address(this));
        }
    }
    ...
    function withdraw(uint256 _amount) external override onlyIzlude {
        ...
        if (wantBal < _amount) {
            IMasterChef(MASTERCHEF).withdraw(pid, _amount - wantBal, address(this));
    ...
    function chargeFees() internal {
        uint256 toMatic = (IERC20(SUSHI).balanceOf(address(this)) * totalFee) / MAX_FEE;
        ...
        toMatic = (IERC20(WMATIC).balanceOf(address(this)) * totalFee) / MAX_FEE;
        if (toMatic > 0) {
            IWETH(WMATIC).withdraw(toMatic);
        }
    ...
    function addLiquidity() internal {
        ...
        uint256 sushiHalf = IERC20(SUSHI).balanceOf(address(this)) / 2;
        if (sushiHalf > 0) {
            if (lpToken0 != SUSHI) {
                router.swapExactTokensForTokens(sushiHalf, 0, sushiToLp0Route, address(this), block.timestamp);
            }
            if (lpToken1 != SUSHI) {
                router.swapExactTokensForTokens(sushiHalf, 0, sushiToLp1Route, address(this), block.timestamp);
            }
        }

        uint256 bonusRewardHalf = IERC20(bonusReward).balanceOf(address(this)) / 2;
        if (bonusRewardHalf > 0) {
            if (lpToken0 != bonusReward) {
                router.swapExactTokensForTokens(
                    bonusRewardHalf,
                    0,
                    bonusRewardToLp0Route,
                    address(this),
                    block.timestamp
                );
            }
            if (lpToken1 != bonusReward) {
                router.swapExactTokensForTokens(
                    bonusRewardHalf,
                    0,
                    bonusRewardToLp1Route,
                    address(this),
                    block.timestamp
                );
            }
        ...
        if (token0Bal > 0 && token1Bal > 0) {
            router.addLiquidity(lpToken0, lpToken1, token0Bal, token1Bal, 0, 0, address(this), block.timestamp);
        }
    }
        ...
        returns (IERC20[] memory rewardTokens, uint256[] memory rewardAmounts)
    {
        ...
        uint256 mainAmt = mc.pendingSushi(pid, address(this)) + IERC20(SUSHI).balanceOf(address(this));

        uint256 userAmount = mc.userInfo(pid, address(this)).amount;
        (IERC20[] memory tokens, uint256[] memory amts) = mc.rewarder(pid).pendingTokens(
            pid,
            address(this),
            userAmount
        );
        ...
        rewardTokens[0] = IERC20(SUSHI);
        rewardAmounts[0] = mainAmt;
        for (uint256 i = 0; i < tokens.length; i++) {
            rewardTokens[i + 1] = tokens[i];
            rewardAmounts[i + 1] = amts[i] + IERC20(tokens[i]).balanceOf(address(this));
        }
    }
    ...
    function retireStrategy() external override onlyIzlude {
        ...
        // mc also emit bonus reward when emergency withdraw
        if (addLpAfterEmergency) {
            addLiquidity();
        }
    ...
    function setAddLpAfterEmergency(bool b) external onlyOwner {
        addLpAfterEmergency = b;
    }
    ...
    function _giveAllowances() internal {
        ...
        IERC20(SUSHI).safeApprove(unirouter, type(uint256).max);
        IERC20(bonusReward).safeApprove(unirouter, type(uint256).max);

        // lp token 0 and 1 maybe sushi so approve 0 is needed here
    ...
    function _removeAllowances() internal {
        ...
        IERC20(SUSHI).safeApprove(unirouter, 0);
        IERC20(bonusReward).safeApprove(unirouter, 0);
    ...
    receive() external payable {
        require(msg.sender == address(WMATIC) || msg.sender == unirouter, "reject");
    }
}
...
interface IMasterChef {
    ...
    function userInfo(uint256 _id, address user) external view returns (UserInfo memory);
