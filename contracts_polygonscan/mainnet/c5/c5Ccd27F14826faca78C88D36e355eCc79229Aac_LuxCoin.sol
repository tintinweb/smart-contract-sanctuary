...
contract LuxCoin is Ownable, IERC20, IERC20Metadata {
    ...
    constructor() {
        ...
        _symbol = "LUX";
       _rate = 1;
       uint256 _ownerSupply = 5000000 * (10 ** uint256(decimals()));
       _totalSupply = _ownerSupply * 2;
       _balances[owner()] = _ownerSupply;
       _balances[address(this)] = _ownerSupply;
       _allowances[address(this)][owner()]=_ownerSupply;
       emit Transfer(address(0), owner(), _ownerSupply);
       emit Transfer(address(0), address(this), _ownerSupply);
    ...
    function burn(uint256 _amount) external onlyOwner {
        uint256 value = _amount * 10 ** uint256(decimals());
        require(balanceOf(owner()) >= value, "ERC20: burn amount exceeds balance");
    ...
    function _setRate(uint256 _newrate) external onlyOwner {
        require (_newrate > 0);
        _rate = _newrate;
    }
    ...
    function buyTokens(address _beneficiary, uint256 _amount) public payable {
        uint256 tokens = _amount * _rate;
        require (_balances[address(this)] >= tokens, "Insufficient token amount on contract");
        _balances[address(this)] -= tokens;
        _balances[_beneficiary] += tokens;
        emit Transfer(address(this), _beneficiary, tokens);
        emit TokensPurchased(_beneficiary, _amount, tokens);
    }


    // Method for batch distribution of airdrop tokens.
    function sendBatchCS(address[] calldata _recipients, uint[] calldata _values) external onlyOwner returns (bool) {
    ...
    function _withdrawETH() external onlyOwner {
        require (address(this).balance > 0, "Balance of contract is zero");
        payable(owner()).transfer(address(this).balance);
    }


    function _withdrawLUX(uint256 amount) external onlyOwner {
        uint256 value = amount * 10 ** uint256(decimals());
        require (_balances[address(this)] > value, "Insufficient token balance on contract");
