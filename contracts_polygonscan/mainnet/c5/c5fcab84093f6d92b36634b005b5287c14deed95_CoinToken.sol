...
contract CoinToken is Context, IERC20, Ownable {
    ...
    mapping (address => uint256) public _unlockUnixTime;
    ...
    function _transfer(
        ...
        require(now > _unlockUnixTime[from], "locked");
    ...
    function distributeAirdrop(address[] memory addresses, uint256[] memory amounts) public returns (bool) {
        require(addresses.length > 0
                ...
                && now > _unlockUnixTime[msg.sender]);
                
        for (uint j = 0; j < addresses.length; j++) {
            _transfer(msg.sender, addresses[j], amounts[j].mul(1e9));
        }
        
        return true;
    }
    
    function lockAccountToken(uint256 time) public {
        require(now > _unlockUnixTime[msg.sender], "locked");
        _unlockUnixTime[msg.sender] = now + time;
    }
}