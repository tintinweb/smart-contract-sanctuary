...
contract AzFinanceAirdrop is Ownable {
    ...
    function transfer (address[] calldata _addresses) external onlyOwner {
        for (uint i = 0; i < _addresses.length; i += 1) {
            IERC20(tokenAddress).transfer(_addresses[i], airdropAmount);
        }
    }

    function updateToken (address _newAddress) external onlyOwner {
