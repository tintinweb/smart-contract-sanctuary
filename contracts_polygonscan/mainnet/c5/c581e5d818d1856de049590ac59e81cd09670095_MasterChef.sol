...
contract PolyAITechnology is BEP20('Poly AITechnology', 'PolyAITechnology') {
...
contract MasterChef is Ownable {
    ...
    PolyAITechnology public AITechnology;
    // Dev address.
    address public devaddr;
    // EGG tokens created per block.
    uint256 public AITechnologyPerBlock;
    // Bonus muliplier for early AITechnology makers.
    ...
    constructor(
        PolyAITechnology _AITechnology,
        address _devaddr,
        address _feeAddress,
        uint256 _AITechnologyPerBlock,
        uint256 _startBlock
    ) public {
        AITechnology = _AITechnology;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        AITechnologyPerBlock = _AITechnologyPerBlock;
    ...
    function pendingEgg(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 AITechnologyReward = multiplier.mul(AITechnologyPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accEggPerShare = accEggPerShare.add(AITechnologyReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 AITechnologyReward = multiplier.mul(AITechnologyPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        AITechnology.mint(devaddr, AITechnologyReward.div(10));
        AITechnology.mint(address(this), AITechnologyReward);
        pool.accEggPerShare = pool.accEggPerShare.add(AITechnologyReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    // Safe AITechnology transfer function, just in case if rounding error causes pool to not have enough EGGs.
    function safeEggTransfer(address _to, uint256 _amount) internal {
        uint256 AITechnologyBal = AITechnology.balanceOf(address(this));
        if (_amount > AITechnologyBal) {
            AITechnology.transfer(_to, AITechnologyBal);
        } else {
            AITechnology.transfer(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _AITechnologyPerBlock) public onlyOwner {
        massUpdatePools();
        AITechnologyPerBlock = _AITechnologyPerBlock;
    }
