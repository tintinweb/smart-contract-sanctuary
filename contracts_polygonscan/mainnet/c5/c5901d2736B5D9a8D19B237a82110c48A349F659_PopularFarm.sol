    ...
    NativeMetaTransaction
{
    ...
    mapping(address => mapping(address => uint256))
        public totalVestedRewardForUser;
    ...
    function initialize(bytes memory _data,address _deployer) external initializer {
        ...
        (
            ...
            _rewardAmounts,
            _encodedData
        ) = basicDataDecoder(_data);

        (burnRate, vestingPeriod, splits, claim) = vestingDataDecoder(
            _encodedData
        );
        require(
            periodFinish != block.timestamp && startTime != periodFinish,
        ...
        require(
            rewardTokenLength == rewardAmountlength,
        ...
        for (uint256 i = 0; i < rewardTokens.length; i++) {
            ...
            tokenRewardRate[rewardTokens[i]] = _rewardAmounts[i].div(
                rewardsDuration
            );
 ...
 /* ========== UPGRADABLE FUNCTION ========== */
    ...
    function getUserVestingInfo(address account)
        external
        view
        returns (UserVestingInfo memory)
    {
    ...
    function getReward() public nonReentrant updateReward(_msgSender()) {
        ...
        if (!info.hasOptForVesting) {
            ...
            for (uint256 i = 0; i < rewardTokens.length; i++) {
                ...
                totalBurnableTokens[rewardTokens[i]] = totalBurnableTokens[
                    rewardTokens[i]
                ].add(burnable);
            ...
            for (uint256 i = 0; i < rewardTokens.length; i++) {
                ...
                if (claimedSplitsForUser == 0 && !hasClaimed[_msgSender()]) {
                    totalEarnedReward[_msgSender()][rewardTokens[i]] = rewards[
                        _msgSender()
                    ][rewardTokens[i]];
                    reward = reward.add(
                        (
                            rewards[_msgSender()][rewardTokens[i]]
                                .mul(claim)
                                .div(100)
                        )
                    );
                    totalVestedRewardForUser[_msgSender()][
                        rewardTokens[i]
                    ] = rewards[_msgSender()][rewardTokens[i]].sub(reward);
                }
                if (claimedSplitsForUser < splits) {
                    currentSplit = (currentDate.sub(periodFinish)).div(
                        splitWindow
                    );
                    currentSplit = currentSplit > splits
                        ? splits
                        : currentSplit;
                    reward = reward.add(
                        (
                            totalVestedRewardForUser[_msgSender()][
                                rewardTokens[i]
                            ].mul((currentSplit.sub(claimedSplitsForUser)))
                        ).div(splits)
                    );
                    ...
                    if (reward > 0) {
                        rewards[_msgSender()][rewardTokens[i]] = rewards[
                            _msgSender()
                        ][rewardTokens[i]].sub(reward);
                        IERC20Upgradeable(rewardTokens[i]).safeTransfer(
                            _msgSender(),
                            reward
                        );
    ...
    function rescueBurnableFunds(address receiver, address rewardToken)
        external onlyDeployer(_msgSender())
    {
        uint256 burnAmount = totalBurnableTokens[rewardToken];
        totalBurnableTokens[rewardToken] = 0;
        require(IERC20Upgradeable(rewardToken).transfer(
            receiver,
    ...
    function rescueFunds(address tokenAddress, address receiver) external onlyOwner {
       ...
       require(IERC20Upgradeable(tokenAddress).transfer(
            receiver,
  ...
  /* ========== PURE FUNCTIONS ========== */
       function basicDataDecoder(bytes memory data)
        internal
        pure
        returns (
            address _stakingToken,
    ...
    {
        (_burnRate, _vesting, _splits, _claim) = abi.decode(
            _data,
...
modifier onlyDeployer(address _account) {
        require(
            _account == deployer,
            "OnlyWhitelisted: caller is not whitelistedr"
        );
        _;
    }
