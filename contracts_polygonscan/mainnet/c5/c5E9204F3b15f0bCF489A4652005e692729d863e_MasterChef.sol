...
// MasterChef is the master of Duck. He can make Duck and he is a fair guy.
//
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 accDuckPerShare;   // Accumulated DUCKs per share, times 1e18. See below.
    ...
    // MAX TOKEN SUPPLY
    uint256 public constant MAX_SUPPLY = 99000 ether;

    // MAX POOL FEE
    uint256 public constant MAX_POOL_FEE = 500;
    ...
    event UpdateEmissionRate(address indexed user, uint256 indexed duckPerBlock);
    ...
    // Update the given pool's DUCK allocation point and deposit fee. Can only be called by the owner.
    ...
    function getMultiplier(uint256 _from, uint256 _to) public view returns (uint256) {
        if (duck.totalSupply() + totalLockedUpRewards >= MAX_SUPPLY) return 0;
    ...
    function pendingDuck(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accDuckPerShare = accDuckPerShare.add(duckReward.mul(1e18).div(lpSupply));
        }
    ...
    // View function to see if user can harvest Ducks's.
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 duckReward = multiplier.mul(duckPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        duck.mint(devaddr, duckReward.div(10));
        duck.mint(address(this), duckReward);
        pool.accDuckPerShare = pool.accDuckPerShare.add(duckReward.mul(1e18).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for DUCK allocation.
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        ...
        user.rewardDebt = user.amount.mul(pool.accDuckPerShare).div(1e18);
    ...
    // Pay or lockup pending duck.
    function payOrLockupPendingDuck(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        uint256 pending = user.amount.mul(pool.accDuckPerShare).div(1e18).sub(user.rewardDebt);
        if (canHarvest(_pid, msg.sender)) {
            if (pending > 0 || user.rewardLockedUp > 0) {
                ...
                user.nextHarvestUntil = getPoolHarvestIntervalAmount(_pid);
                // send rewards
                safeDuckTransfer(msg.sender, totalRewards);
    ...
    function safeDuckTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > duckBal) {
            transferSuccess = duck.transfer(_to, duckBal);
        } else {
            transferSuccess = duck.transfer(_to, _amount);
        }
        require(transferSuccess, "safeDuckTransfer: transfer failed");
    }
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (address(referral) != address(0) && referralCommissionRate > 0) {
            ...
            if (referrer != address(0) && commissionAmount > 0) {
                duck.mint(referrer, commissionAmount);
    ...
    function updateEmissionRate(uint256 _duckPerBlock) public onlyOwner {
        massUpdatePools();
        duckPerBlock = _duckPerBlock;
        emit UpdateEmissionRate(msg.sender, _duckPerBlock);
    }
}
...
// DuckToken with Governance.
contract DuckToken is BEP20('Cute Duck', 'CDUCK') {
    ...
    external
    {
        ...
        require(signatory != address(0), "Mustard::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "Mustard::delegateBySig: invalid nonce");
        require(now <= expiry, "Mustard::delegateBySig: signature expired");
    ...
    returns (uint256)
    {
        require(blockNumber < block.number, "Mustard::getPriorVotes: not yet determined");
    ...
    internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying Mustards (not scaled);
    ...
    internal
    {
        uint32 blockNumber = safe32(block.number, "Mustard::_writeCheckpoint: block number exceeds 32 bits");
