...
contract BeliFarm is Ownable, ReentrancyGuard {
    ...
    function payConvertedEarn(
        ...
        UserInfo storage user = userInfo[_pid][_user];
        if (user.shares > 0) {
            uint256 amount = pendingConvertedEarn(_pid, _user);
            if (amount > 0) {
    ...
    function payOrLockupPendingBELI(uint256 _pid, bool _isWithdrawal) internal {
        ...
        if (IVault(pool.vault).accEarnBELIPerShare() > 0) {
            payConvertedEarn(_pid, msg.sender);
        }
    ...
    function transferRewardFromFeeReceiver(uint _pid, address _user) private {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 balance;
        uint256 pendingReward;
        if (pool.isFeeReceiverReward) {
            pendingReward = pendingFeeReceiverBeliReward(_pid, _user);
            ...
            pendingReward = pendingFeeReceiverBeliLpReward(_pid, _user);
