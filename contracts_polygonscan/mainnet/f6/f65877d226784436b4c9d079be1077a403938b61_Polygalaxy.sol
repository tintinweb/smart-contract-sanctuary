...
contract Polygalaxy is ERC20, Ownable {
    ...
    // + transfer tax rate
    uint16 public transferGalaxyTax = 800;
    ...
    uint256 public minAmountToVaultCash = 1000 * 10**18;
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (!_excludedFromAntiWhale[sender]) {
                require(amount <= maxTransferAmount(), "POLYGALAXY::antiWhale: Transfer amount exceeds the maxTransferAmount");
            }
        }
        _;
    }
    ...
    constructor() public {
        ...
        _excludedFromAntiWhale[polygalaxySwapPair] = true; 
        
        launchedAt = block.timestamp + 0*43200;
        mint(msg.sender, 150 * 10**18);
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        ...
        if (recipient == BURN || transferGalaxyTax == 0) {
            super._transfer(sender, recipient, amount);
        } else if (_excludedFromAntiWhale[sender]) {
            super._transfer(sender, recipient, amount);
        } else {
