...
// import 'github.com/OpenZeppelin/openzeppelin-contracts/blob/release-v4.2/contracts/token/ERC20/ERC20.sol';
// import 'github.com/OpenZeppelin/openzeppelin-contracts/blob/release-v4.2/contracts/access/Ownable.sol';
// import 'github.com/OpenZeppelin/openzeppelin-contracts/blob/release-v4.2/contracts/utils/math/SafeMath.sol';
...
contract PeachToken is ERC20('PeachToken', 'PEACH'), Ownable {
    ...
    */
        ...
        external
    {
        ...
        require(block.timestamp <= expiry, "PEACH::delegateBySig: signature expired");
...
// Grower is the master of Peaches. He can make PEACH and he is a fair guy.
//
...
contract Grower is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount ofPEACHs 
    ...
    struct PoolInfo {
        ...
        uint256 accPeachPerShare;  // Accumulated PEACHs per share, times 1e18. See below.
    ...
    // Maximum peach per block.
    uint256 public constant MAX_PEACH_PER_BLOCK = 50;
    // Deposit Fee address
    ...
    function pendingPeach(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accPeachPerShare = accPeachPerShare.add(peachReward.mul(1e18).div(lpSupply));
        }
        return user.amount.mul(accPeachPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accPeachPerShare = pool.accPeachPerShare.add(peachReward.mul(1e18).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to Grower for PEACH allocation.
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accPeachPerShare).div(1e18).sub(user.rewardDebt);
            if (pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accPeachPerShare).div(1e18);
        emit Deposit(msg.sender, _pid, finalDepositAmount);
    }

    // Withdraw LP tokens from Grower.
    ...
    function updateEmissionRate(uint256 _peachPerBlock) external onlyOwner {
        require(_peachPerBlock <= MAX_PEACH_PER_BLOCK, "updateEmissionRate: too high");
        massUpdatePools();
        peachPerBlock = _peachPerBlock;
