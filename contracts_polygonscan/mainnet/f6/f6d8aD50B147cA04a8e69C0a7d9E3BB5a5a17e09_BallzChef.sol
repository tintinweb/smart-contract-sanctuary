...
import "./BallzToken.sol";
import "./libs/IReferral.sol";

// MasterChef is the master of BallzToken. He can make Ballz and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once Ballz is sufficiently
// distributed and the community can show to govern itself.
//
// Have fun reading it. Hopefully it's bug-free. God bless.
contract BallzChef is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    uint256 public constant TOKEN_CAP = 5_000_000 ether;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of BALLZes
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accBallzPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accBallzPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. BALLZes to distribute per block.
        uint256 lastRewardBlock;  // Last block number that BALLZes distribution occurs.
        uint256 accBallzPerShare;   // Accumulated BALLZes per share, times 1e18. See below.
        uint16 depositFeeBP;      // Deposit fee in basis points
    }

    // The BALLZ TOKEN!
    BallzToken public ballzToken;
    address public devAddress;
    address public feeAddress;
    address public vaultAddress;

    // BALLZ tokens created per block.
    uint256 public ballzPerBlock = 1 ether;
    uint256 constant public MAX_EMISSION_RATE = 100 ether;
    ...
    // The block number when BALLZ mining starts.
    uint256 public startBlock;

    // Ballz referral contract address.
    ...
    event UpdateEmissionRate(address indexed user, uint256 ballzPerBlock);
    ...
    // Update the given pool's BALLZ allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending BALLZes on frontend.
    function pendingBallz(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accBallzPerShare = pool.accBallzPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 ballzReward = multiplier.mul(ballzPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accBallzPerShare = accBallzPerShare.add(ballzReward.mul(1e18).div(lpSupply));
        }
        return user.amount.mul(accBallzPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 ballzReward = multiplier.mul(ballzPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        mintCapped(devAddress, ballzReward.div(10));
        mintCapped(address(this), ballzReward);
        pool.accBallzPerShare = pool.accBallzPerShare.add(ballzReward.mul(1e18).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for BALLZ allocation.
    function deposit(uint256 _pid, uint256 _amount, address _referrer) external nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accBallzPerShare).div(1e18).sub(user.rewardDebt);
            if (pending > 0) {
                safeBallzTransfer(msg.sender, pending);
        ...
        user.rewardDebt = user.amount.mul(pool.accBallzPerShare).div(1e18);
    ...
    // Safe ballz transfer function, just in case if rounding error causes pool to not have enough BALLZ.
    function safeBallzTransfer(address _to, uint256 _amount) internal {
        uint256 ballzBal = ballzToken.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > ballzBal) {
            transferSuccess = ballzToken.transfer(_to, ballzBal);
        } else {
            transferSuccess = ballzToken.transfer(_to, _amount);
        }
        require(transferSuccess, "safeBallzTransfer: Transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _ballzPerBlock) external onlyOwner {
        require(_ballzPerBlock <= MAX_EMISSION_RATE, "too high");
        massUpdatePools();
        ballzPerBlock = _ballzPerBlock;
        emit UpdateEmissionRate(msg.sender, _ballzPerBlock);
    }
    ...
    function mintCapped(address to, uint256 amount) private {
        uint256 totalSupply = ballzToken.totalSupply();
        if(totalSupply.add(amount) > TOKEN_CAP) {
            ballzToken.mint(to, TOKEN_CAP.sub(totalSupply));
        } else {
            ballzToken.mint(to, amount);
        }
    }
