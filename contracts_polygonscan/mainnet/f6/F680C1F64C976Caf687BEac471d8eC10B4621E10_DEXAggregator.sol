...
contract DEXAggregator is Ownable {
    ...
    function swap(
        ...
        if (isNativeToken(inputToken) && msg.value > 0) {
            feeAmount = msg.value.mul(aggregatorFee).div(1000);
            feeBeneficiary.transfer(feeAmount);
            IWRAP(wrappedToken).deposit{value: msg.value.sub(feeAmount)}();
        } else if (!isNativeToken(inputToken)) {
            feeAmount = inputAmount.mul(aggregatorFee).div(1000);
        ...
        require(amountIn[0] == inputAmount.sub(feeAmount), "Input token Amount in should be equal to input amount deducted by fee");
