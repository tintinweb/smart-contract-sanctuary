...
// File: contracts/marketplace2.sol
...
contract Marketplace2_0 is AccessControl, ERC721Holder, Pausable  {
    IERC721 public RACE;
    // IERC20 public MATIC;
    ...
    constructor(address _RACE){
        ...
        _setupRole(DEFAULT_ADMIN_ROLE, 0x5a1a41d613676780f6E9684F1C9Fb89e116013f2);
    ...
    function getRacerPrice(uint _ID) public view returns(uint256){
        return trades[_ID].price;
    }

    function buyRacer(uint _tokenId) public payable {
        require(trades[_tokenId].status == LISTED, "TR: Racer not listed");
        require((msg.sender).balance >= trades[_tokenId].price, "TR: Insufficient funds");
        ...
        payable(_safe).transfer(_fivePercent);//MATIC.transfer(_safe, _fivePercent);
