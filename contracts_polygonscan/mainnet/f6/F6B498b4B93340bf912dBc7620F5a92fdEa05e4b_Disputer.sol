...
interface IDisputer {
  function dispute(uint256 _liquidationId, address _sponsor) external;

  function collateralCurrency() external returns (IERC20);
}
...
contract Disputer is Ownable {
    ...
    onlyOwner()
  {
    for (uint256 i=0; i < _derivatives.length; i++) {
      registeredDerivatives[_derivatives[i]] = true;
    }
  }
  ...
  function raiseDispute(
    ...
    require(
      registeredDerivatives[_derivative] == true,
