...
contract HEXToken is ERC20('HumanXchange', 'HEX') {
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of HEXes
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accHEXPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accHEXPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. HEXes to distribute per block.
        uint256 lastRewardBlock;  // Last block number that HEXes distribution occurs.
        uint256 accHEXPerShare;   // Accumulated HEXes per share, times 1e18. See below.
    ...
    // HEX tokens created per block.
    uint256 public HEXPerBlock = 0.015 ether;
    
    uint256 constant MAX_HEX_PERBLOCK= 0.1 ether;
    ...
    // The block number when HEX mining starts.
    uint256 public startBlock;

    // HEX referral contract address.
    ...
    event UpdateEmissionRate(address indexed user, uint256 HEXPerBlock);
    ...
    // Update the given pool's HEX allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending HEXes on frontend.
    function pendingHEX(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accHEXPerShare = pool.accHEXPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0 && totalAllocPoint>0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 HEXReward = multiplier.mul(HEXPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accHEXPerShare = accHEXPerShare.add(HEXReward.mul(1e18).div(lpSupply));
        }
        return user.amount.mul(accHEXPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 HEXReward = multiplier.mul(HEXPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        HEX.mint(devAddress, HEXReward.div(10));
        HEX.mint(address(this), HEXReward);
        pool.accHEXPerShare = pool.accHEXPerShare.add(HEXReward.mul(1e18).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for HEX allocation.
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accHEXPerShare).div(1e18).sub(user.rewardDebt);
            if (pending > 0) {
                safeHEXTransfer(msg.sender, pending);
        ...
        user.rewardDebt = user.amount.mul(pool.accHEXPerShare).div(1e18);
    ...
    // Safe HEX transfer function, just in case if rounding error causes pool to not have enough FOXs.
    function safeHEXTransfer(address _to, uint256 _amount) internal {
        uint256 HEXBal = HEX.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > HEXBal) {
            transferSuccess = HEX.transfer(_to, HEXBal);
        } else {
            transferSuccess = HEX.transfer(_to, _amount);
        }
        require(transferSuccess, "safeHEXTransfer: Transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _HEXPerBlock) external onlyOwner {
        require(_HEXPerBlock<=MAX_HEX_PERBLOCK,"Too high");
        massUpdatePools();
        HEXPerBlock = _HEXPerBlock;
        emit UpdateEmissionRate(msg.sender, _HEXPerBlock);
    }
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (address(referral) != address(0) && referralCommissionRate > 0) {
            ...
            if (referrer != address(0) && commissionAmount > 0) {
                HEX.mint(referrer, commissionAmount);
