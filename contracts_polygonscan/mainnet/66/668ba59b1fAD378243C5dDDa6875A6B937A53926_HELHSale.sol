...
contract HELHSale is Ownable {

    using SafeERC20 for ERC20;
    using Address for address;

    uint constant MATICdecimals = 10 ** 18;
    uint constant HELHdecimals = 10 ** 18;
    uint public constant MAX_SOLD = 60000 * HELHdecimals / 10**9;
    uint public constant PRICE = 1500 * MATICdecimals * 10**9 / HELHdecimals;
    uint public constant MAX_SALE_PER_ACCOUNT = 500 * HELHdecimals / 10**9;
    ...
    constructor( address _dev, uint _presaleTimestamp, address matic_address) {
        dev = _dev;
        presaleTimestamp = _presaleTimestamp;
        MATIC = ERC20(matic_address);    
        }
    ...
    function buyHELH(uint256 amount) public onlyEOA {
        ...
        MATIC.safeTransferFrom( msg.sender, address(this), amount * PRICE);
        invested[msg.sender] += amount;
        sold += amount;
        owed += amount;
    }

    function claimHELH() public onlyEOA {
      ERC20(HELH).transfer(msg.sender, (invested[msg.sender] * 500) * 10**9);
      owed -= invested[msg.sender];
      invested[msg.sender] = 0;

    }
    function setClaimingActive(address helh) public {
    ...
    function getNewClue() public view returns(string memory) {
        return "5.stomach";
    }
}