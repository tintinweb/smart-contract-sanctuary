...
contract StakeMinter is IStakeMinter, Manageable, ExternallyCallable {
    ...
    */
    function stakeInternal(
        ...
        require(amount >= minimumStakeAmount, 'StakeInternal: Not enough axion.');
        require(
    ...
    /** @dev Manager Transfer
        description: Manager functinality for hacked wallets to transfer their stake
        @param stakeId {uint256} - AXN amount to be staked
        @param from {address} - number of days to be staked
        @param to {address} - account address to create the stake for
    */
    function managerTransferFrom(
        ...
        StakeData1e18 memory data = contracts.stakeManager.getStake(stakeId);
        require(
            data.status != StakeStatus.Withdrawn,
            'ManagerTransfer: Stake is withdrawn and can not be transferred'
        );
        require(
            contracts.stakeCustodian.isOwnerOf(from, stakeId),
            'ManagerTransfer: Address does not own stake or stake is NFT'
        );
        require(
            contracts.stakeCustodian.removeStake(from, stakeId),
            'ManagerTransfer: Error removing stakeId'
        );
        require(
            contracts.stakeCustodian.addStake(to, stakeId),
            'ManagerTransfer: Error adding stakeId'
        );

        contracts.ventureCapital.transferSharesAndRebalance(from, to, data.shares);

        emit StakeTransferred(stakeId, from, to);
    }

    /** @notice set minimum stake amount */
    function setMinimumStakeAmount(uint256 minimum) external onlyManager {
        minimumStakeAmount = minimum;
    }
    ...
    /** Events */
    event StakeTransferred(uint256 stakeId, address from, address to);
}
