...
contract CryptoSlimes is ERC721Enumerable, Ownable {
  ...
  function mint(address _to, uint256 _mintAmount) public payable {
    ...
    if(msg.sender != owner()){
        require(_mintAmount <= maxMintAmount,"You can only mint one at a time");
    }
    if (msg.sender != owner()) {
        require(msg.value >= cost, "You paid to little");
    }
  ...
  function nextStage(uint256 currentStage) internal{
      if(totalSupply() == currentMaxSupply){
          if(currentStage == 1){
              cost = 5 ether;
              currentMaxSupply = 2500;
              saleStage = 2;
