...
contract BoostHandler is Ownable, IBoostHandler {
    ...
    address public lockedStakingPlus_lpVault = 0xd82a68EcA83D38906c7a3f9996d93D95f8Aa24B5;
    ...
    //Returns veADDY from the "locked staking plus" contracts (LockedAddyVault)
    function getVeAddyFromLockedStaking(address _user) public override view returns (uint256 veAddy) {
        ...
        uint256 endingTimestampPlus2 = ILockedAddyVault(lockedStakingPlus_lpVault).getEndingTimestamp(_user);
        if(endingTimestampPlus2 > now) {
            //1 veAddy = 4 year lock, boost degrades over time
            uint256 diff = endingTimestampPlus2.sub(now);
            uint256 userAddyLocked = ILockedAddyVault(lockedStakingPlus_lpVault).balanceOf(_user);
            uint256 ratio = ILockedAddyVault(lockedStakingPlus_lpVault).getRatio();
            veAddy = veAddy.add(userAddyLocked.mul(diff).div(MAX_DURATION).mul(ratio).div(10 ** 18));
        }
    }
