...
contract GovernanceToken is ERC20Burnable, IGovernanceToken, Ownable {
    ...
    uint256 public constant MAX_WITHDRAWAL_FEE = 50000; // 50%
    ...
    function emergencyWithdraw() external lock {
        ...
        uint256 _amount = _locked.amount;
        if (_now < _locked.end) {
            uint256 _fee = _amount * earlyWithdrawFeeRate / PRECISION;
            IERC20(lockedToken).safeTransfer(feeCollector, _fee);
    ...
    function setEarlyWithdrawFeeRate(uint256 _earlyWithdrawFeeRate) external onlyOwner {
        require(_earlyWithdrawFeeRate <= MAX_WITHDRAWAL_FEE, "withdrawal fee is too high"); // <= 50%
        earlyWithdrawFeeRate = _earlyWithdrawFeeRate;
        emit EarlyWithdrawFeeSet(_earlyWithdrawFeeRate);
    }

    function setFeeCollector(address _addr) external onlyOwner {
        feeCollector = _addr;
        emit FeeCollectorSet(_addr);
    }
    ...
    event EarlyWithdrawFeeSet(uint256 indexed fee);
