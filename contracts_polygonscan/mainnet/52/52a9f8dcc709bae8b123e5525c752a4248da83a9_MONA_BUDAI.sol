...
contract MONA_BUDAI is ERC20, Ownable {
    ...
    MONA_BUDAIDividendTracker public dividendTracker;
    ...
    constructor() public ERC20("MONA_BUDAI", "MONA_BUDAI") {
    	...
    	dividendTracker = new MONA_BUDAIDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "MONA_BUDAI: The dividend tracker already has that address");

        MONA_BUDAIDividendTracker newDividendTracker = MONA_BUDAIDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "MONA_BUDAI: The new dividend tracker must be owned by the MONA_BUDAI token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "MONA_BUDAI: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "MONA_BUDAI: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "MONA_BUDAI: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "MONA_BUDAI: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "MONA_BUDAI: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "MONA_BUDAI: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "MONA_BUDAI: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "MONA_BUDAI: This account cannot send tokens until trading is enabled");
        }
        ...
        if(isFixedSaleBuy) {
            require(block.timestamp >= fixedSaleStartTimestamp, "MONA_BUDAI: The fixed-sale has not started yet.");
            ...
            if(!openToEveryone) {
                require(fixedSaleEarlyParticipants[to], "MONA_BUDAI: The fixed-sale is only available to certain participants at the start");
            }
...
contract MONA_BUDAIDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    ...
    constructor() public DividendPayingToken("MONA_BUDAI_Dividend_Tracker", "MONA_BUDAI_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "MONA_BUDAI_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "MONA_BUDAI_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MONA_BUDAI contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "MONA_BUDAI_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "MONA_BUDAI_Dividend_Tracker: Cannot update claimWait to same value");
