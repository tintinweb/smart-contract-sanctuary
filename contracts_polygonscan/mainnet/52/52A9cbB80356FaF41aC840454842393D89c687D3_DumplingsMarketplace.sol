...
contract DumplingsMarketplace is Base, IMarketplace {

    event AddedDumplingToMarketplace(uint indexed _id, uint indexed _price);
    event BoughtDumplingFromMarketplace(uint indexed _id, uint indexed _price);
    event RemovedDumplingFromMarketplace(uint indexed _id);

    constructor(address _constantsContract) Base(_constantsContract) {}

    function putDumplingForSale(uint _dumplingId, uint _price) public {
        address DumplingsContract = IConstants(constantsContract).DumplingsContract();
        IDumplings(DumplingsContract).approveForMarketplace(address(this), _dumplingId, _price);
        emit AddedDumplingToMarketplace(_dumplingId, _price);
    }

    function buyDumpling(uint _dumplingId) public payable {
        address DumplingsContract = IConstants(constantsContract).DumplingsContract();
        uint256 price = IDumplings(DumplingsContract).getDumplingByIndex(_dumplingId).price;
        require(msg.value >= price, "The amount sent is smaller the the sell price.");
        address seller = IDumplings(DumplingsContract).ownerOf(_dumplingId);
        uint256 fee = calcFee(price);
        payable(seller).transfer(msg.value - fee);
        transferValueToOwner(fee);
        IDumplings(DumplingsContract).transferMarketplaceFrom(seller, msg.sender, _dumplingId, price);
        IStatistics(IConstants(constantsContract).statisticsContract()).increaseTotalDumplingsSales();
        emit BoughtDumplingFromMarketplace(_dumplingId, price);
    }

    function cancelDumplingSale(uint _dumplingId) public {
        address DumplingsContract = IConstants(constantsContract).DumplingsContract();
        IDumplings(DumplingsContract).approve(address(0), _dumplingId);
        emit RemovedDumplingFromMarketplace(_dumplingId);
    }
