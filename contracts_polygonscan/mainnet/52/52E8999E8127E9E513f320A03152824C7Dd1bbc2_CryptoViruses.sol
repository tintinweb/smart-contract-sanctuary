...
contract CryptoViruses is ERC721, ERC721URIStorage, Ownable {

  using Strings for uint256;

  mapping(address => uint256) private _mintedList;
  mapping(address => bool) private _reservedAddresses;
  ...
  constructor(
    uint256 maxPublic,
    uint256 maxReserved,
    uint256 startingReservedID,
    uint256 maxPerAddress,
    address[] memory reservedAddresses
  ) ERC721("CryptoViruses", "VIRUS")
  {
    ...
    for (uint256 i = 0; i < reservedAddresses.length; i++) {
        require(reservedAddresses[i] != address(0), "Cannot add the null address");
        _reservedAddresses[reservedAddresses[i]] = true;
    }
  }
  ...
  function mintReserved(uint256[] calldata tokenIds) external {
    require(_reservedAddresses[msg.sender], "You are not on the reserve list.");
