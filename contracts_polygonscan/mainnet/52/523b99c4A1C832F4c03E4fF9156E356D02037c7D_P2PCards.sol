...
contract P2PCards is Trustable {
    ...
    event OrderClosed(uint indexed id);
    ...
    function create(uint8 tokenId, uint amount, string memory fiat, string memory trade) external {
    ...
    function complete(uint orderId) external onlyTrusted {
    ...
    function closeOperator(uint orderId) external onlyTrusted {
        Order memory order = orders[orderId];
        require(order.status == status_completed, "unexpected status in close");
    ...
    function closeOwner(uint orderId) external {
        Order memory order = orders[orderId];
        require(order.status == status_completed, "unexpected status in close");
        require(order.sender == msg.sender, "Only owner can close the order");
        order.status = status_closed;
        orderIds.remove(orderId);
        orders[orderId] = order;
        emit OrderClosed(orderId);
    ...
    function withdraw(uint8 tokenId, address to, uint amount) external onlyTrusted {
        Token storage tok = tokens[tokenId];
