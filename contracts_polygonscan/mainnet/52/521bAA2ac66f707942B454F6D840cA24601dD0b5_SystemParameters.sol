...
contract SystemParameters is ISystemParameters, OwnableUpgradeable {
    ...
    bytes32 public constant MAX_DEPOSIT_PER_ACCOUNT_KEY = keccak256("MAX_DEPOSIT_PER_ACCOUNT");
    ...
    function getMaxDepositPerAccountParam() external view override returns (uint256) {
        return _getParam(MAX_DEPOSIT_PER_ACCOUNT_KEY).getUintFromParam();
    }
    ...
    /// @dev New values must be with 10^8 precision. For example, 100$ = 100 * 10^8
    function setupMaxDepositPerAccount(uint256 _newValue) external onlyOwner {
        uint256 _oneUSD = 10**PRICE_DECIMALS;

        require(
            _newValue >= 10 * _oneUSD && _newValue <= 100000 * _oneUSD,
            "SystemParameters: The new value of the max deposit per account is invalid."
        );

        _parameters[MAX_DEPOSIT_PER_ACCOUNT_KEY] = PureParameters.makeUintParam(_newValue);

        emit UintParamUpdated(MAX_DEPOSIT_PER_ACCOUNT_KEY, _newValue);
    }
