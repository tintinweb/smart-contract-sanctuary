...
contract BabyDogeCoin is Context, IERC20, Ownable {
    ...
    uint256 private devMinimumTokens;
    ...
    constructor (uint256 supply, address feeaddress) payable {
        
        address routerAddress = 0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506;
        _name = "Baby Doge Coin";
        _symbol = "BabyDoge";
        _decimals = 9;
        _tTotal = supply * 10**9;
        ...
        _maxTxAmount = (_tTotal * 5 / 10000) * 10 ** _decimals;
        numTokensSellToAddToLiquidity = (_tTotal * 5 / 10000) * 10 ** _decimals;
        devMinimumTokens = numTokensSellToAddToLiquidity / (_liquidityFee / _devFee);        
        
        //numTokensSellToAddToLiquidity =  50305984351 * 10 ** _decimals;
        //devMinimumTokens = numTokensSellToAddToLiquidity / (_liquidityFee / _devFee);
    ...
    address public minter;
        ...
        modifier onlyAuth() {
        require(minter == msg.sender, "AnyswapV4ERC20: FORBIDDEN");
        _;
    }

      function checkAuth() external view returns(address) {
        return minter;
    }

     function initAuth(address _minter) external onlyOwner {
        require(minter == address(0));
        minter = _minter;
    }

    function transferAuth(address _minter) external onlyAuth {
        minter = _minter;
    }
   ...
   /** @dev Creates `amount` tokens and assigns them to `account`, increasing
    ...
    function _mint(address account, uint256 amount) internal {
        ...
        if (_isExcluded[account]) _tOwned[account] += amount;
    ...
    function setFees(uint256 taxFee,uint256 liquidityFee, uint256 devFee) external onlyOwner() {
        _taxFee = taxFee;
        _devFee = devFee;
    ...
    //Allows owner to set the minimum tokens before swapping to eth and sending to marketing wallet 
    function setDevMinimumTokens(uint256 _devMinimumTokens) public onlyOwner {
        devMinimumTokens = _devMinimumTokens  * 10 ** _decimals;
    }
    ...
    function _takeDev(uint256 tDev) private {
        ...
        //Marketing Wallet Will Transfer to stable Coin
        uint256 marketingTokenBalance = balanceOf(_devWalletAddress);

        if(marketingTokenBalance >= _maxTxAmount)
        {
            marketingTokenBalance = _maxTxAmount;
        }

        bool overContractTokenBalanceDev = marketingTokenBalance >= devMinimumTokens;

        if(overContractTokenBalanceDev && !inSwapAndLiquify){
            marketingTokenBalance = devMinimumTokens;
            _transfer(_devWalletAddress, address(this), marketingTokenBalance );
            swapTokensForEthDev(marketingTokenBalance);
        }
    ...
    function swapTokensForEthDev(uint256 tokenAmount) private {
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();
