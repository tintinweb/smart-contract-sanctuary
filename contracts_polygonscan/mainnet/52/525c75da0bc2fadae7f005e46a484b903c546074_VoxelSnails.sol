...
contract VoxelSnails is IERC721, Ownable, Functional {
    ...
    function airDrop(address[] memory voxeldrops) external onlyOwner {
    	uint256 qty = voxeldrops.length;
        require(qty > 0, "req0");
        require(qty + numberMinted <= totalTokens, "req1");
		require(qty + numberMinted > numberMinted, "req2");
		
		uint256 mintSeedValue = numberMinted;
		
        for (uint256 i = 0; i < qty; i++){
		    _mint(voxeldrops[i], mintSeedValue + i);
           	numberMinted ++;
        }
