...
contract DexUpdates is Ownable, DexStorage {
  ...
  event AuctionTimeUpdate(uint256[] tokenId, uint256 endTime, address nft_a, bool isAwardType);
 ...
 function updateAuctionTime(uint256 endTime, address nft_a, uint256[] memory tokenIds, bool isAwardType) public {
    require(admin == msg.sender, "not authorize");
    for(uint256 i = 0; i < tokenIds.length; i++){
      _auctionTokens[tokenIds[i]][nft_a].endTime = endTime;
    
    }
    emit AuctionTimeUpdate(tokenIds, endTime, nft_a, isAwardType);
 }
