...
contract AutoStakingPoolv2 is ReentrancyGuard, Ownable {
    ...
    uint256 public rewardsDuration = 86400 * 7; // 7 days

    // Duration of lock/earned penalty period
    uint256 public lockDuration = rewardsDuration * 13; // 91 days (13 weeks)

    IAuto public stakingToken;
    ...
    constructor (
        ...
        stakingToken = IAuto(_stakingToken);
        ...
        rewardDistributors[_stakingToken][msg.sender] = true;
        rewardTokens.push(address(wmatic));
        rewardData[address(wmatic)].lastUpdateTime = block.timestamp;
        rewardDistributors[address(wmatic)][msg.sender] = true;
    }
    ...
    function claimable(address account) external view returns (uint256 lockedReward, uint256 totalReward) {
        lockedReward = _earned(account, rewardTokens[0], balances[account].locked, lockedSupply);
        totalReward = _earned(account, rewardTokens[1], balances[account].total, totalSupply);
    }
    ...
    // Vested balances may be withdrawn immediately for a 50% penalty
    function vestedBalances(
        address user
    ...
    function vest(address user, uint256 amount) external nonReentrant updateReward(user) {
        totalSupply = totalSupply.add(amount);
        Balances storage bal = balances[user];
        bal.total = bal.total.add(amount);
    ...
    function withdraw(uint256 amount) public nonReentrant updateReward(msg.sender) {
        ...
        if (amount <= bal.unlocked) {
            ...
            require(bal.earned >= remaining, "No balance");
            bal.unlocked = 0;
            ...
            for (uint i = 0; ; i++) {
                ...
                if (penaltyAmount == 0 && userEarnings[msg.sender][i].unlockTime > block.timestamp) {
                    ...
                    require(bal.earned >= remaining, "No balance after penalty");
                    bal.earned = bal.earned.sub(remaining);
                    if (bal.earned == 0) {
