...
contract WormHole is Ownable {
    ...
    struct UserInfo {
        uint256 amount; // How many wormHoledToken tokens the user has provided.
    ...
    IERC20 public immutable wormHoledToken; // Token being worm holed
    ...
    uint256 public stakedWormHoleToken;
    PoolInfo public poolInfo;
    bool public emergencyActive;

    mapping(address => UserInfo) public userInfo; // Info of each user that stakes wormHoledToken tokens.
    ...
    constructor(
        ...
        require(
            address(_wormHoledToken) != address(0),
            "_wormHoledToken address not set!"
        );
        ...
        wormHoledToken = _wormHoledToken;
        REWARD = _REWARD;
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (
            block.number > poolInfo.lastRewardBlock && stakedWormHoleToken != 0
        ) {
            ...
            accRewardPerShare = accRewardPerShare.add(
                tokenReward.mul(1e18).div(stakedWormHoleToken)
            );
        }
        return
    ...
    /**
     * @dev Deposit wormHoledToken tokens to Faucet for rewards allocation and/or withdraw outstanding rewards.
     * @param _amount Amount of wormHoledToken tokens to deposit.
     */
    function deposit(uint256 _amount) public {
        ...
        if (block.number < endBlock && _amount != 0) {
            wormHoledToken.safeTransferFrom(
                address(msg.sender),
                address(this),
                _amount
            );
            stakedWormHoleToken = stakedWormHoleToken.add(_amount);
        ...
        if (block.number >= endBlock) {
            wormHoledToken.safeTransfer(msg.sender, user.amount);
            stakedWormHoleToken = stakedWormHoleToken.sub(user.amount);
            user.amount = 0;
            user.rewardDebt = user.amount.mul(poolInfo.accRewardPerShare).div(
                1e18
            );
    ...
    function activateEmergency(address _to) external onlyOwner returns (bool) {
        ...
        if (wormHoledToken == REWARD) {
            return
                REWARD.transfer(
                    _to,
                    REWARD.balanceOf(address(this)) - stakedWormHoleToken
                );
        } else {
    ...
    function emergencyWithdraw() external {
        ...
        UserInfo storage user = userInfo[msg.sender];
        wormHoledToken.safeTransfer(address(msg.sender), user.amount);
        stakedWormHoleToken = stakedWormHoleToken.sub(user.amount);
