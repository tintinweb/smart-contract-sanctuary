...
import './Protocols/Aave.sol';

contract AaveStrategy02 is Funding, Aave {

    using SafeMath for uint256;

    //TODO CHANGE
    //TODO SWAP STRETEGY ADDRESS
    address private SWAP_STRETEGY_ADDRESS = 0x43A9c07A7cD48FD8D4f4A6b6B8B9f161A8E87985;
    ISwapStrategy01 public SwapStrategy = ISwapStrategy01(SWAP_STRETEGY_ADDRESS);
...
// ++++++++++++++++++++++++++++++++++++++++
    function _aave_deposit(string memory Coin, uint256 tokenAmt) internal {
    ...
    function _aave_withdraw(string memory Coin, uint256 tokenAmt) internal {
        AavePair storage AAVE = AavePairOf[Coin];
        AaveLP.withdraw(AAVE.basic, tokenAmt, address(this));        
    }

    function _aave_borrow(string memory Coin, uint256 tokenAmt) internal {
        AavePair storage AAVE = AavePairOf[Coin];
        IERC20 SToken = IERC20(AAVE.basic);
        AaveLP.borrow(AAVE.basic, tokenAmt, 2, 0, address(this));
    }

    function _aave_repay(string memory Coin, uint256 tokenAmt) internal {
        AavePair storage AAVE = AavePairOf[Coin];
        IERC20 SToken = IERC20(AAVE.basic);
        require(SToken.balanceOf(address(this)) >= tokenAmt);
        if (SToken.allowance(address(this), AAVE_LENDING_POOL) < tokenAmt) {
...
// ++++++++++++++++++++++++++++++++++++++++
    ...
    function aave_add_coin(
        ...
        aaveMaticAssets.push(am);
        aaveMaticAssets.push(variableDebtm);
    }

    // ETH
    // POLIGON TODO in GOLBAL VAL >
    ...
    //TODO moove to constructor
    string public AAVE_BASE_COIN;

    /* ADMIN IS UExecutor */
    constructor(string memory _AAVE_BASE_COIN) Admin(address(0xC6Df4F19D3a5A6c0365D835b00f52c86e0aB392e)) {
        AAVE_BASE_COIN = _AAVE_BASE_COIN;        
        deployer = msg.sender;
    }

    bool private is_init = false;
    function init() public onlyDeployer() returns (uint256) {
    ...
    address[] public aaveMaticAssets;
    ...
    function aave_matic_claim_to(address to) public onlyOwner() returns (uint256){
        // claim MATIC
        uint256 amount = StakedToken.getRewardsBalance(aaveMaticAssets, to);
        StakedToken.claimRewards(aaveMaticAssets, amount, to);
        return amount;
    }
    ...
    function aave_stat_balances() public view returns (
        ...
        matic = StakedToken.getRewardsBalance(aaveMaticAssets, address(this));
        ( balance_basic,
          balance_am,
