...
contract OriginGeniePolygon {
    ...
    function generateData(
        ...
        string memory json = Base674.encode(
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "Origin #',
                        ...
                        _long,
                        '"},',
                        '{"trait_type": "Magnetic", "value": "',
                        _dir,
                        '"},',
                        '{"trait_type": "XYZ", "value": "',
                        _x,
                        ", ",
                        _y,
                        ", ",
    ...
    function _getRects(string[5] memory _colors)
        internal
        pure
        ...
        return
            string(
                abi.encodePacked(
                    ...
                    rect("30%", "20%", "232", "130", _colors[1], "d"),
                    rect("95%", "20%", "10", "245", _colors[1], "d2")
                )
            );
    }

    function checkIfDefault(string memory _aColor)
        internal
        pure
        returns (bool isDefault)
    {
        ...
        bytes memory _buffer = bytes(_aColor);
        bytes memory _bufferTest = bytes("#000");

        for (uint256 i = 0; i < 4; i++) {
            if (_buffer[i] != _bufferTest[i]) {
                isDefault = false;
            }
        }
        ...
        return
            string(
                abi.encodePacked(
                    ...
                    '<style> .a{font-family:"FreeMono", monospace;font-weight:800;} .b{fill:',
                    _colors[3],
                    ";font-size:25px;} .c{font-size:14px;fill:",
                    _colors[0],
                    ";}",
                    " .t{font-size:42px;font-weight:200;fill:",
                    _colors[0],
                    ";} .m{fill:#ffffff;} .d{stroke-width:1;} .d2{ stroke-width:1; stroke:",
                    (checkIfDefault(_colors[0]) ? _colors[0] : _colors[2]),
                    "} .e{fill:",
                    (checkIfDefault(_colors[0]) ? _colors[0] : _colors[2]),
                    "} .f{font-size:22px;fill:",
                    _colors[0],
                    ";} </style>",
