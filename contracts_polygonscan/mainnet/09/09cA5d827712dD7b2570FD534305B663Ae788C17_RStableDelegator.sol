...
/**
 * @title Iron's RErc20Delegator Contract
 * @notice CTokens which wrap an EIP-20 underlying and delegate to an implementation
 * @author IronFinance
 */
contract RStableDelegator is RTokenInterface, RStableInterface, rDelegatorInterface {
    /**
   ...
   /**
    ...
    function hasPool(address _pool) external view returns (bool){
        delegateToViewImplementation(abi.encodeWithSignature("hasPool(address)", _pool));
    }
    ...
    /**
     * @notice Set max total supply of stable coin
     * @param newMaxTotalSupply the new max total supply to use
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function _setMaxTotalSupply(uint newMaxTotalSupply) public returns (uint) {
        bytes memory data = delegateToImplementation(abi.encodeWithSignature("_setMaxTotalSupply(uint256)", newMaxTotalSupply));
        return abi.decode(data, (uint));
    }
