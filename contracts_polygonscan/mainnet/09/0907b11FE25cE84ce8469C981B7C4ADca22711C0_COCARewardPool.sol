...
contract COCARewardPool {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. COCAs to distribute in the pool.
        uint256 lastRewardTime; // Last time that COCAs distribution occurred.
        uint256 accTombPerShare; // Accumulated COCAS per share, times 1e18. See below.
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "CocaRewardPool: caller is not the operator");
        _;
    }
    ...
    function checkPoolDuplicate(IERC20 _token) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].token != _token, "CocabRewardPool: existing pool?");
        }
    }
    ...
    // View function to see pending COCAs on frontend.
    ...
    // Safe coca transfer function, just in case if rounding error causes pool to not have enough COCAs.
