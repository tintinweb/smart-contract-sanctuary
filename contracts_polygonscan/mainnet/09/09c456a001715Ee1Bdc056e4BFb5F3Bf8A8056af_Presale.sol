...
contract Presale is ReentrancyGuard, Context, Ownable {
    ...
    uint public minPurchase = 265000000000000000000;
    uint public maxPurchase = 380000000000000000000;
    uint public hardCap;
    uint public softCap;
    uint public availableTokensICO = 25000000000000000000000000;
    uint256 public _rewardTokenCount = 385 ether; // 2 tokens per MATIC
    ...
    function isWhitelisted(address wallet) public view returns(bool isIndeed) {
        return whitelist[wallet].exists;
    }

    function AddToWhitelist(address wallet) public onlyOwner returns(bool success)  {
        require(!isWhitelisted(wallet));
        whitelist[wallet].wallet = wallet;
        whitelist[wallet].exists = true;
        return true;
    }
    ...
    function buyTokens(address beneficiary) public nonReentrant icoActive payable {
        ...
        // _processPurchase(beneficiary, purchasedReward); // Don't send the token to the user, BMG sends the tokens them self
    ...
    function _preValidatePurchase(address beneficiary, uint256 weiAmount) internal view {
        ...
        require(isWhitelisted(beneficiary), 'You have no access for the pre-sale');
        this; // silence state mutability warning without generating bytecode - see https://github.com/ethereum/solidity/issues/2691
    }
