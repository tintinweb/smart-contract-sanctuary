...
// SwanToken with Governance.
contract SwanToken is BEP20('Swan Token', 'SWAN') {
...
// MasterChef is the master of Swan. He can make Swan and he is a fair guy.
//
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    // Bonus muliplier for early swan makers.
    ...
    event UpdateEmissionRate(address indexed user, uint256 SwanPerBlock);
    ...
    // Update the given pool's SWAN allocation point and deposit fee. Can only be called by the owner.
    ...
    function pendingSwan(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 swanReward = multiplier.mul(SwanPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
    ...
    // Deposit LP tokens to MasterChef for SWAN allocation.
    ...
    function safeSwanTransfer(address _to, uint256 _amount) internal {
        ...
        require(transferSuccess, "safeSwanTransfer: transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _SwanPerBlock) public onlyOwner {
        massUpdatePools();
        SwanPerBlock = _SwanPerBlock;
        emit UpdateEmissionRate(msg.sender, _SwanPerBlock);
    }
