...
/**
 * @title Compound's REther Contract
...
/**
 * @title Compound's RToken Contract
...
contract RToken is RTokenInterface, Exponential, TokenErrorReporter {
    ...
    function liquidateBorrowFresh(address liquidator, address borrower, uint repayAmount, RTokenInterface RTokenCollateral) internal returns (uint, uint) {
        ...
        uint allowed = comptroller.liquidateBorrowAllowed(address(this), address(RTokenCollateral), liquidator, borrower, repayAmount);
        if (allowed != 0) {
        ...
        (uint amountSeizeError, uint seizeTokens) = comptroller.liquidateCalculateSeizeTokens(address(this), address(RTokenCollateral), actualRepayAmount);
