...
contract CASTLE_IDO is Initializable {
    ...
    */
    ...
    function keyBalance(address user) public view returns(uint){
        return key.balanceOf(user);
    }

    function totalCost(address user, uint amountToBuy) public view returns(uint original, uint afterDiscount){
        if(key.balanceOf(user) > 0){
            uint keyBal = key.balanceOf(user);
            //remove some or all of the cost
            if(keyBal >= amountToBuy){
                afterDiscount = 0;
            }
            else{
                afterDiscount = (amountToBuy - keyBal) * Agreement.price();
            }
        }
        else{
            afterDiscount = amountToBuy * Agreement.price();
        }
        original = amountToBuy * Agreement.price();
    }
}