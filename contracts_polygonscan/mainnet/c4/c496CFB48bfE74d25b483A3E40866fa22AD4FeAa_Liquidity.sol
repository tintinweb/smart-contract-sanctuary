...
contract Liquidity is Ownable {
    ...
    constructor(
      ...
      DELIRIUM = IERC20(_DELIRIUM);
      _allowance();
    }

  receive() external payable {}

  function withdrawDeliriumLiquidity() external onlyOwner {
      USDC.transfer(msg.sender, USDC.balanceOf(address(this)));
      DELIRIUM.transfer(msg.sender, DELIRIUM.balanceOf(address(this)));
  }
 ...
 function withdrawAll()  external onlyOwner {
  ...
  function AutoLiquidity() external onlyOwner {
    ...
    uint256 deliriumBalance = DELIRIUM.balanceOf(address(this));
    uint256 halfTokenAmount = deliriumBalance / 2;

    if (usdcBalanceAfter > 0 && wmaticBalance > 0 && deliriumBalance > 0) {
        // add the liquidity delirium-matic
        ...
        // add the liquidity delirium-usdc
        ROUTER.addLiquidity(
            address(DELIRIUM),
            address(USDC),
            halfTokenAmount,
  ...
  function _allowance() internal {
    ...
    DELIRIUM.approve(address(ROUTER), type(uint256).max);
  }
