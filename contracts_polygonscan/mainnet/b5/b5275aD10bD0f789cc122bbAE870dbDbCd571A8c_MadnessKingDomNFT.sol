...
import "./TheEndlessFactoryNFT.sol";
...
contract MadnessKingDomNFT is Ownable, ReentrancyGuard {
    ...
    // dream tokens created per block.
    ...
    function withdraw() external nonReentrant {
        ...
        if (user.hasNFT) {
            theEndlessFactoryNFT.setExperience(user.nftID, user.experience);
        }
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        uint256 calculatedAmount = _calculatedUserAmount(_user);
        ...
        if (block.number > poolInfo.lastRewardBlock && stakedTokenSupply != 0) {
            ...
            uint256 adjustedTokenPerShare = accRewardPerShare.add(tokenReward.mul(PRECISION_FACTOR).div(stakedTokenSupply));
    ...
    function _initializeNFT(uint256 _nftID, address _userAddress) internal  {
        ...
        if (theEndlessFactoryNFT.ownerOf(_nftID) == _userAddress){
          ...
          user.experience = _getNFTExperience(user.nftID).add(100);
        }
    }
    ...
    function _updatePower(address _user, uint256 _amount) internal {
        ...
        user.experience = user.experience.add(powerStaking / PRECISION_FACTOR);
    ...
    function setExperience(uint256 _nftID, uint256 _level) external {
        theEndlessFactoryNFT.setExperience(_nftID, _level);
    }
    ...
    function _getNFTPowerStaking(uint256 _nftID) internal view returns (uint256) {
        ...
        ,
        ,
        ) = theEndlessFactoryNFT.characters(_nftID);
        powerStaking = strength + agility + endurance + intelligence + magic + wisdom;
