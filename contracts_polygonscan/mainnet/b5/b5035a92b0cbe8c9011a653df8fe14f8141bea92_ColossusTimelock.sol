...
// The Colossuslock leverages using openzeppelin timelock for maximum safety. Its also Colossus and haunted.
// To see openzepplin's audits goto: https://github.com/OpenZeppelin/openzeppelin-contracts/tree/master/audit
contract ColossusTimelock {
    using SafeERC20 for IERC20;

    IERC20 public Colossus;
    ...
    function forwardTokens() external {
        ...
        uint boos = Colossus.balanceOf(address(this));

        require(boos > 0, "forwardTokens: no boos!");

        for (uint256 index = 0; index <= 3; index++) {
            Colossus.transfer(address(Locks[index]), boos / 4);
        }

        // just incase theres any boos left from rounding
        uint leftover = Colossus.balanceOf(address(this));

        if (leftover > 0) {
            Colossus.transfer(address(Locks[3]), leftover);
        }
    }
