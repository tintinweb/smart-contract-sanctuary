...
contract BEP20 is Context, IBEP20, Ownable {
    ...
    */
    ...
    /**
     * @dev update `botAddress` to detect router
     *
     */
...
contract BeeSwapToken is BEP20('Bee Swap Token', 'BEE') {
...
contract MasterBee is Ownable {
    ...
    constructor(
        ...
        address _devaddr,
        address _feeAddress,
        uint256 _beePerBlock,
        uint256 _startBlock
    ) public {
        bee = _bee;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        beePerBlock = _beePerBlock;
    ...
    function pendingEgg(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accEggPerShare = accEggPerShare.add(beeReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accEggPerShare = pool.accEggPerShare.add(beeReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    // Safe bee transfer function, just in case if rounding error causes pool to not have enough EGGs.
    function safeEggTransfer(address _to, uint256 _amount) internal {
        uint256 beeBal = bee.balanceOf(address(this));
        if (_amount > beeBal) {
            bee.transfer(_to, beeBal);
        } else {
            bee.transfer(_to, _amount);
        }
    }
