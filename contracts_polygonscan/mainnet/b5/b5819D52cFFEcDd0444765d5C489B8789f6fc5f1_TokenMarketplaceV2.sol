...
ReentrancyGuard
{
  ...
  uint256 public newBuyerDiscount = 0;
  ...
  // Mapping of EditionType => Tiered Mint Purchase enabled
  mapping(uint256 => bool) public mintMode;

  // Mapping of EditionType => quantity sold
  mapping(uint256 => uint256) public mintSold;

  // Mapping of EditionType => [prices]
  mapping(uint256 => uint256[]) public mintPrices;
  // Mapping of EditionType => [quantities]
  mapping(uint256 => uint256[]) public mintQuantity;
  ...
  modifier onlyDuringMintWindow(uint256 _tokenId) {
    ...
    bool purchaseDatesActive = (startDate <= block.timestamp && endDate >= block.timestamp);

    require(!artistIsOwner || purchaseDatesActive, "Token owned by artist outside of minting window");
    _;
  }
  ...
  nonReentrant
  {
    ...
    (, uint256 endDate) = nrdaAddress.purchaseDatesEdition(editionNumber);
    bool purchaseDatesEnded = endDate < block.timestamp;
    bool mintPurchase = (currentOwner == artistAccount && purchaseDatesEnded);
  ...
  whenNotPaused {
    ...
    (
      ...
      /*string memory _tokenURI*/,
      address tokenOwner
    ) = nrdaAddress.tokenData(_tokenId);

    require(tokenOwner == _msgSender(), "Not token owner");

    (address artistAccount, /*uint256 artistCommissionRate*/) = nrdaAddress.artistCommission(editionNumber);
    require(tokenOwner == artistAccount, "Not artist owner");
    require(_to != address(0), "Invalid address");


    if (mintMode[editionType]) {
      // Mintmode actually means tiered pricing mode
      // Bump the quantity sold of this type, even if giving token away
      mintSold[editionType] = mintSold[editionType].add(1);
    } else {
      // If not in mint mode, assume are in mode with auctioning from artist
      minters[editionNumber] = _to;
    }

    nrdaAddress.safeTransferFrom(tokenOwner, _to, _tokenId);

    emit TokenGiven(_tokenId, _to, tokenOwner);
  }
  ...
  whenNotPaused {
    ...
    _buyToken(_tokenId, _msgValue, listing.price);
  ...
  function mintBuyToken(uint256 _tokenId, uint256 _msgValue)
  public
  nonReentrant
  whenNotPaused {
    (
    ...
    require(mintMode[editionType], "Mint tier purchasing disabled");

    (address artistAccount, /*uint256 artistCommissionRate*/) = nrdaAddress.artistCommission(editionNumber);
    require(owner == artistAccount, "Not artist owner");
    require(mintPrices[editionType].length == mintQuantity[editionType].length, "price/quantity tier lengths must match");

    uint256 quantitySold  = mintSold[editionType];
    //require(quantitySold < mintSupply[editionType], "No more editions available");


    // Find the mint price for this token based on quantity of this editionType sold
    uint256 runSum;
    uint256 mintPrice;
    bool foundPrice;
    for (uint i = 0; i < mintQuantity[editionType].length; i++) {
      runSum = runSum.add(mintQuantity[editionType][i]);
      if (quantitySold < runSum && !foundPrice) {
        mintPrice = mintPrices[editionType][i];
        foundPrice = true;
      }
    }

    require(quantitySold < runSum, "No more editions available");

    mintSold[editionType] = quantitySold.add(1);

    if (nrdaAddress.balanceOf(_msgSender()) == 0) {
      if (mintPrice > newBuyerDiscount) {
        mintPrice = mintPrice.sub(newBuyerDiscount);
      } else {
        mintPrice = 0;
      }
    }
    ...
    _buyToken(_tokenId, _msgValue, mintPrice);
  }

  function setMintTiers(uint256 _editionType, uint256[] memory _quantity, uint256[] memory _price)
  public
  onlyIfWhitelisted(_msgSender()) {
    require(_quantity.length == _price.length, "price/quantity tier lengths must match");

    mintPrices[_editionType] = _price;
    mintQuantity[_editionType] = _quantity;
    mintSold[_editionType] = 0;
  }


  function getMintPrices(uint256 _type) external view returns (uint256[] memory _mintPrices) {
    return mintPrices[_type];
  }

  function getMintQuantity(uint256 _type) external view returns (uint256[] memory _mintQuantity) {
    return mintQuantity[_type];
  }

  function setMintSold(uint256 _editionType, uint256 _mintSold)
  public
  onlyIfWhitelisted(_msgSender())
  {
    mintSold[_editionType] = _mintSold;
  }

  function setMintMode(uint256 _editionType, bool _mintMode)
  public
  onlyIfWhitelisted(_msgSender())
  {
    mintMode[_editionType] = _mintMode;
  }

  function _buyToken(uint256 _tokenId, uint256 _msgValue, uint256 _requiredPrice) internal {

    // _msgValue acts more like a sanity check more than anything 
    // just that the user approved at least the correct amount beforehand
    require(_msgValue >= _requiredPrice, "Price not satisfied");
    acceptedToken.safeTransferFrom(_msgSender(), address(this), _requiredPrice);
    ...
    // split funds
    _handleFunds(editionNumber, _requiredPrice, currentOwner, false);

    // transfer token to buyer
    nrdaAddress.safeTransferFrom(currentOwner, _msgSender(), _tokenId);

    // Fire confirmation event
    emit TokenPurchased(_tokenId, _msgSender(), currentOwner, _requiredPrice);
  }
  ...
  function setNewBuyerDiscount(uint256 _newBuyerDiscount) public onlyIfWhitelisted(_msgSender()) {
    newBuyerDiscount = _newBuyerDiscount;
  }
  ...
  function setMinter(uint256 _tokenId, address _minter)
  public
  onlyWhenTokenExists(_tokenId)
  onlyIfWhitelisted(_msgSender()) 
  {
    uint256 editionNumber = nrdaAddress.editionOfTokenId(_tokenId);
    minters[editionNumber] = _minter;
  }
