...
contract PizzaToken is BEP20('Pizza Token', 'PIZZA') {
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    event UpdateEmissionRate(address indexed user, uint256 PizzaPerBlock);
    ...
    // Update the given pool's PIZZA allocation point and deposit fee. Can only be called by the owner.
    ...
    function safePizzaTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > pizzaBal) {
            transferSuccess = pizza.transfer(_to, pizzaBal);
        } else {
            transferSuccess = pizza.transfer(_to, _amount);
        }
        require(transferSuccess, "safePizzaTransfer: transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _PizzaPerBlock) public onlyOwner {
        massUpdatePools();
        PizzaPerBlock = _PizzaPerBlock;
        emit UpdateEmissionRate(msg.sender, _PizzaPerBlock);
    }
