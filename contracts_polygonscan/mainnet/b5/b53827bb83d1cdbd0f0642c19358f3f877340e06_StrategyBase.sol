...
abstract contract StrategyFarmBase is BaseStrategyMasterChef {
    ...
    function harvest() public override {
        ...
        if (_iceBalance > harvestCutoff){
            uint256 performanceFee = _iceBalance.mul(keepFXS).div(keepFXSmax);
            if (performanceFee>0) {
                _swapUniswapWithPath(ice_usdc_path, performanceFee);
            ...
            // Swap
            _iceBalance = IERC20(ice).balanceOf(address(this));
            _swapUniswapWithPath(ice_usdc_path, _iceBalance.div(2));

            // Add liquidity
            uint256 _ice = IERC20(ice).balanceOf(address(this));
            uint256 _usdc = IERC20(usdc).balanceOf(address(this));
            if (_ice > 0 && _usdc > 0) {
                IERC20(ice).safeApprove(currentRouter, 0);
                IERC20(ice).safeApprove(currentRouter, _ice);
...
contract StrategyBase is StrategyFarmBase {
    ...
    address public iceUSDC_LP = 0x34832D9AC4127a232C1919d840f7aaE0fcb7315B;
