...
import "./SpritexNFT.sol";
import "./SpritexToken.sol";
import "@openzeppelin/contracts/utils/Address.sol";
import "@openzeppelin/contracts/math/SafeMath.sol";
import "@openzeppelin/contracts/access/Ownable.sol";

contract SpritexAuction is Ownable {
    ...
    event ClaimSPRITEX(uint256 auctionIndex, address claimer);
    event BurnTEM(uint256 auctionIndex);

    SpritexToken public spritexToken;
    SpritexNFT public spritexNFT;
    address public constant BURNADDRESS = 0x000000000000000000000000000000000000000d;

    constructor(SpritexNFT _spritexNFT, SpritexToken _spritexToken) public {
    spritexNFT = _spritexNFT; 
    spritexToken = _spritexToken;
    }
    ...
    function createAuction(uint256 _assetId,
        ...
        require(spritexNFT.ownerOf(_assetId) == msg.sender);
        require(spritexNFT.getApproved(_assetId) == address(this));
    ...
    function bid(uint256 auctionIndex, uint256 amount) public returns (bool) {
        ...
        if (amount > auction.currentBidAmount) {
            ...
            require(spritexToken.transferFrom(msg.sender, address(this), amount));
            if (auction.currentBidOwner != address(0)) {
                // return funds to the previuos bidder
                if (spritexNFT.spritexOwnersNow() == 0) {
                spritexToken.transfer(
    ...
    function burntem(uint256 auctionIndex) public onlyOwner {
        ...
        require(spritexToken.transfer(BURNADDRESS, bBurnAmount));

        emit BurnTEM(auctionIndex);
    }

    function claimspritex(uint256 auctionIndex) public {
        require(isFinished(auctionIndex));
        Auction storage auction = auctions[auctionIndex];
        ...
        spritexNFT.transferFrom(owner(), winner, auction.assetId);
        
        auction.claimedNFT = true;

        emit ClaimSPRITEX(auctionIndex, winner);
    }
...
contract SpritexNFT is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable, Ownable {
    using SafeMath for uint256;
    using Address for address;
    ...
    mapping (address => bool) private _isSpritexOwner;

    address[] private _spritexOwners;

    string private _name = 'Spritex';
    string private _symbol = 'SPRIX';
    uint256 private constant MAXSUP = 99;
    ...
    function _mintWithTokenURI(address _to, string memory _tokenURI) internal {
        ...
        require(_tokenId <= MAXSUP, "Max supply 99 SPRITE");
        _mint(_to, _tokenId);
        _setTokenURI(_tokenId, _tokenURI);
    }

    function spritexOwnersNow() public view returns (uint256) {
        return _spritexOwners.length;
    }

    function isSpritexOwner(address account) public view returns (bool) {
        return _isSpritexOwner[account];
    }


    function getspritexOwners (uint256 index) public view returns (address) {
        return _spritexOwners[index];
    }
    ...
    function _transfer(address from, address to, uint256 tokenId) internal virtual {
        ...
        if (from == owner() && to != owner() && balanceOf(to) == 0) {
          ...
          _isSpritexOwner[to] = true;
          _spritexOwners.push(to);
          
        } else if (to == owner() && from != owner() && balanceOf(from) == 1) {
         
          for (uint256 i = 0; i < _spritexOwners.length; i++) {
            if (_spritexOwners[i] == from) {
                _spritexOwners[i] = _spritexOwners[_spritexOwners.length - 1];
                
                _isSpritexOwner[from] = false;
                _spritexOwners.pop();
                break;
            }
          }
...
contract SpritexToken is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    ...
    uint256 private constant _tokenTotal = 990000 * 10**9;
    uint256 private _tFeeTotal;
    SpritexNFT public spritexNFT;

    string private _name = 'Spritex.io';
    string private _symbol = 'SPRIX';
    uint8 private _decimals = 9;

    address public constant burnaddr = 0x000000000000000000000000000000000000000d;

    constructor (SpritexNFT _spritexNFT) public {
         spritexNFT = _spritexNFT; 
    ...
    function _distFeeinRef(uint256 tFee, address sender) private {
        uint256 share = tFee.div(spritexNFT.spritexOwnersNow());
        for (uint256 i = 0; i < spritexNFT.spritexOwnersNow(); i++) {
        _rewardsReceived[spritexNFT.getspritexOwners(i)] = _rewardsReceived[spritexNFT.getspritexOwners(i)].add(share);
        _tokenOwned[spritexNFT.getspritexOwners(i)] = _tokenOwned[spritexNFT.getspritexOwners(i)].add(share);
        emit Transfer(sender, spritexNFT.getspritexOwners(i), share);
        }    
