...
contract Liquidate is IERC3156FlashBorrower, Ownable {
    ...
    function _setEthTokens(address[] memory tokens, bool status) public onlyOwner {
    ...
    function flashBorrowForLiquidate(IERC3156FlashLender lender, address token, uint256 amount, bytes calldata data) public {
    ...
    function onFlashLoan(address initiator, address token, uint256 amount, uint256 fee, bytes calldata data) external override returns (bytes32) {
        ...
        //授权给贷款方合约
        _approveMaxInternal(token, address(lender), _repayment);

        //执行清算逻辑
        _flashloanLiquidateInternal(_repayment, data);

        return keccak256("ERC3156FlashBorrower.onFlashLoan");
    }

    function _flashloanLiquidateInternal(uint256 amountOutMin, bytes calldata params) internal {

        address self = address(this);

        (address _tokenBorrow, address _tokenCollateral, address borrower, uint repayAmount, address _uniswapRouter, address[] memory path) = abi.decode(params, (address, address, address, uint, address, address[]));

        _enterMarket(_tokenBorrow, _tokenCollateral);

        // 执行清算
        if (_isEthToken(_tokenBorrow)) {
            IEthToken(_tokenBorrow).liquidateBorrow{value : repayAmount}(borrower, _tokenCollateral);
        } else {
            _approveMaxInternal(IErcToken(_tokenBorrow).underlying(), _tokenBorrow, repayAmount);
            IErcToken(_tokenBorrow).liquidateBorrow(borrower, repayAmount, _tokenCollateral);
        }
        uint collateralBalance = IToken(_tokenCollateral).balanceOf(self);
        IToken(_tokenCollateral).redeem(collateralBalance);

        // 执行交易
        if (_isEthToken(_tokenCollateral)) {
            IUniswapV2Router02(_uniswapRouter).swapExactETHForTokens{value : self.balance}(amountOutMin, path, self, block.timestamp);
        } else if (_isEthToken(_tokenBorrow)) {
            address _underlying = IErcToken(_tokenBorrow).underlying();
            uint amountIn = IERC20(_underlying).balanceOf(self);
            _approveMaxInternal(_underlying, _uniswapRouter, amountIn);
            IUniswapV2Router02(_uniswapRouter).swapExactTokensForETH(amountIn, amountOutMin, path, self, block.timestamp);
        } else {
            address _underlying = IErcToken(_tokenCollateral).underlying();
            uint amountIn = IERC20(_underlying).balanceOf(self);
            _approveMaxInternal(_underlying, _uniswapRouter, amountIn);
            IUniswapV2Router02(_uniswapRouter).swapExactTokensForTokens(amountIn, amountOutMin, path, self, block.timestamp);
        }
    }
