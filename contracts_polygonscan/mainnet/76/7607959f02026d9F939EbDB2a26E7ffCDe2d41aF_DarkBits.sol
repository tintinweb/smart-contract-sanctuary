...
contract DarkBits is ERC1155Supply, ReentrancyGuard, Ownable {
    ...
    uint256 public constant mintPrice = 2 ether;  // where msg.value === 2 MATIC
    ...
    address  aMATIC = 0x8dF3aad3a84da6b69A4DA8aeC3eA40d9091B2Ac4; // aMATIC address
    ...
    function requestMint() public payable nonReentrant
    {
        ...
        require(mintPrice <= msg.value, "Must send 2 MATIC.");
        mintRandomEntity();
    }
    ...
    function transferToTreasury() public onlyOwner {
        ...
        require(IERC20(aMATIC).balanceOf(address(this)) > 0, "Contract has empty aWeth balance.");
    
        IERC20(aMATIC).approve(treasuryAddress, IERC20(aMATIC).balanceOf(address(this)));
        IERC20(aMATIC).transfer(treasuryAddress, IERC20(aMATIC).balanceOf(address(this))); 
    }
    ...
    function getAWethBalance() public view returns (uint256) {
        return IERC20(aMATIC).balanceOf(address(this));
    }

    function getClass(uint256[] memory darkBitsId) private pure returns (uint256) {
         uint256 onesCount = 0;
         for (uint256 i = 0; i < darkBitsId.length; i++) {
            if(darkBitsId[i] == 1) {
    ...
    function generateDarkBitsId(uint256 randomValue) internal pure returns (uint256[] memory darkBitsId) {
        darkBitsId = expand(randomValue, 10);
        for (uint256 i = 0; i < 10; i++) {
            darkBitsId[i] = (uint256(keccak256(abi.encode(randomValue, i))) % 2);
        }
        return darkBitsId;
    }
    ...
    function doRewardClaim() internal 
    {
        ...
        IERC20(aMATIC).approve(msg.sender, reward);
        IERC20(aMATIC).transfer(msg.sender, reward);
    ...
    function mintRandomEntity() internal  {
        uint256[] memory darkBits = generateDarkBitsId(rand());
        uint256 class = getClass(darkBits);
