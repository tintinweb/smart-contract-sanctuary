...
contract Barn is Ownable, IERC721Receiver, Pausable {
  ...
  SWOOL public wool;
  address public liquidityAddr = 0xD052dAFFe1cF7B4C0C13d56F897ec0EA2f8Be5C2;
  ...
  function _claimWolfFromPack(uint256 tokenId, bool unstake) internal returns (uint256 owed) {
    ...
    if (unstake) {
      ...
      _payWolfTax(owed * WOOL_CLAIM_TAX_PERCENTAGE / 100); // percentage tax to staked wolves
      owed = owed * (100 - WOOL_CLAIM_TAX_PERCENTAGE) / 100; // remainder goes to Sheep owner
      uint256 burn =owed*50/100 ;
      owed = owed.sub(burn);
      uint256 liquidity =burn*50/100 ;
      wool.mint(liquidityAddr, liquidity);
      barn[tokenId] = Stake({
        owner: _msgSender(),
        tokenId: uint16(tokenId),
        value: uint80(block.timestamp)
...
//liquidityAddr
  function setLiquidityAddr(address addr_) external onlyOwner{
       liquidityAddr=addr_;
  }
  /**
