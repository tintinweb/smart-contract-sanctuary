...
abstract contract StrategyFarmBase is BaseStrategyMasterChef {
    ...
    // Uniswap swap paths
    address[] public shield_usdc_path;
    ...
    {
        shield_usdc_path = new address[](2);
        shield_usdc_path[0] = shield;
        shield_usdc_path[1] = usdc;
    ...
    function harvest() public override {
        ...
        if (_shieldBalance > harvestCutoff){
            ...
            if (performanceFee>0) {
                _swapUniswapWithPath(shield_usdc_path, performanceFee);
           ...
           uint256 _shieldBalancePost = IERC20(shield).balanceOf(address(this));
            _swapUniswapWithPath(shield_usdc_path, _shieldBalancePost.div(2));

            // Add liquidity
            uint256 _shield = IERC20(shield).balanceOf(address(this));
            uint256 _usdc = IERC20(usdc).balanceOf(address(this));
            if (_shield > 0 && _usdc > 0) {
...
contract StrategyBase is StrategyFarmBase {
    ...
    address public SHIELD_USDC_LP = 0x0CA257A6cae5C89826DAae3BE0A64170e34493C5;
