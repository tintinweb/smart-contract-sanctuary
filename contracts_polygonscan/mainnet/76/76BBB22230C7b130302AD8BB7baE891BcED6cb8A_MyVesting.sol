...
contract MyVesting is Ownable {
  ...
  constructor(address tokenAddress_, uint256 periodFinish_) {
    tokenAddress = tokenAddress_;
    myToken = IERC20(tokenAddress_);
    periodStart = block.timestamp;
    periodFinish = periodFinish_;
    _term = periodFinish - periodStart;
    require(_term > 0, "MyVesting: term must be greater than 0!");
  }
  ...
  function setTotalRewards(uint256 totalRewards_) external onlyOwner {
    require(totalRewards_ >= 0, "MyVesting: totalRewards must be greater than or equal to 0");
  ...
  function getReward() public {
    ...
    require(_earned <= rewards[msg.sender], "Vesting: earned is more than reward!");
    require(_earned > payouts[msg.sender], "Vesting: earned is less or equal to already paid!");
   ...
   * @dev calculates total amounts must be rewarded and transfers to the address
   */
  function getRewardV2() public {
    uint256 _earned = earned(msg.sender);
    require(_earned <= rewards[msg.sender], "MyVesting: earned is more than reward!");
    require(_earned > payouts[msg.sender], "MyVesting: earned is less or equal to already paid!");

    uint256 reward = _earned.sub(payouts[msg.sender]);

    if (reward > 0) {
      payouts[msg.sender] = _earned;
      myToken.safeTransfer(msg.sender, reward);
      emit RewardPaid(msg.sender, reward);
    }
  }
  ...
  function getRewardV5(uint256 amount) public {
    IERC20 token = IERC20(tokenAddress);
