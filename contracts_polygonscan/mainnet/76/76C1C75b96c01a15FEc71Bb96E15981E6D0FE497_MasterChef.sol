...
contract Pizza is ERC20 {
    ...
    /**
     * @notice Constructs the Pizza contract.
     */
    constructor() public ERC20('PolyPizza Token', 'PIZZA') {
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "Pizza::antiWhale: Transfer amount exceeds the maxTransferAmount");
            }
        }
        _;
    }
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 10000, "Pizza::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
    ...
    /// @dev overrides transfer function to meet tokenomics of Pizza
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "PolyPizzaToken::transferOperator: new operator is the zero address");
...
// MasterChef is the master of PolyPizza Token (PIZZA). He can make Pizza and he is a fair guy.
//
...
// contract once $PIZZA is sufficiently distributed and the community can show to govern itself.
//
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    // Initial emission rate: 0.005 Pizza per block.
    ...
    event UpdateEmissionRate(address indexed user, uint256 indexed __PizzaPerBlock);
    ...
    constructor(
        ...
        PizzaPerBlock = INITIAL_EMISSION_RATE;
    ...
    // Update the given pool's Pizza allocation point and deposit fee.
    ...
    function getMultiplier(uint256 _from, uint256 _to)
        ...
        returns (uint256)
    {
        if (IERC20(pizza).totalSupply() + totalLockedUpRewards  >= MAX_SUPPLY_CAP) {
            return 0;
        }
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 PizzaReward =
                multiplier.mul(PizzaPerBlock).mul(pool.allocPoint).div(
                    totalAllocPoint
                );
            accPizzaPerShare = accPizzaPerShare.add(
                PizzaReward.mul(1e18).div(lpSupply)
            );
        }
        uint256 pending =  user.amount.mul(accPizzaPerShare).div(1e18).sub(user.rewardDebt);
        return pending.add(user.rewardLockedUp);
    }

    // View function to see if user can harvest Pizza's.
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        user.rewardDebt = user.amount.mul(pool.accPizzaPerShare).div(1e18);
        emit Deposit(msg.sender, _pid, _amount);
    }

    // Deposit LP tokens to MasterChef for pizza allocation with referral.
    ...
    // Pay or lockup pending pizza.
    function payOrLockupPendingPizza(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        uint256 pending = user.amount.mul(pool.accPizzaPerShare).div(1e18).sub(user.rewardDebt);
        ...
        if (canHarvest(_pid, msg.sender)) {
            if (pending > 0 || user.rewardLockedUp > 0) {
                ...
                // send rewards
                safePizzaTransfer(msg.sender, totalRewards);
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        ...
        if (referrer != address(0) && referrer != _user && refBonusBP > 0) {
            ...
            pizza.mint(referrer, refBonusEarned);
