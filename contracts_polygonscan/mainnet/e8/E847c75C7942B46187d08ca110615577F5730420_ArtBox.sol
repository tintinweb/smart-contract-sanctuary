...
contract ArtBox is ERC721Enumerable, Ownable, Pausable {
  ...
  /** @dev Updates baseURIStored
   * can only be called by the owner (admin)
   */
  function updateBaseURI(string memory _baseURIStored)
    external
    onlyOwner
    whenNotPaused
  {
   ...
   * token will be the concatenation of the `baseURIStored` and the `tokenId`.
   */
