...
contract Lottery is ReentrancyGuard, Ownable, Pausable {
    ...
    struct LottoInfo {
        ...
        uint256[3] prizeMultipliers; // Multiplier for 2, 3, and 4 matches
    ...
    event prizeMultipliersChanged(uint256[3] newMultipliers);
    ...
    uint256[3] public prizeMultipliers;
    uint256 public blockDelay;
    uint256 public currentLottoID;

    constructor(
        uint256 _costPerTicket,
        uint256[3] memory _prizeMultipliers,
        uint256 _blockDelay,
        address _GBTSAddr,
        address _ulpAddr,
        address _rngAddr
    ...
    function play(bytes32 _requestId, uint256 _randomness) external onlyRNG {
        ...
        lotteryList[currentLottoID].winningNumber = _randomness % 10000;
    ...
    function claimPrizes(uint256 _lottoID) public nonReentrant {
        ...
        uint256[3] memory _prizeMultipliers = lotteryList[_lottoID].prizeMultipliers;
    ...
    function getPrizeMultipliers() public view returns (uint256[3] memory) {
        return prizeMultipliers;
    }
    
    /**
     * @dev Returns the winning number of a lottery
     * @param _lottoID the lottery ID to check
     */
    function getWinningNumber(uint256 _lottoID) public view returns (uint256) {
        return lotteryList[_lottoID].winningNumber;
    }
    
    /**
     * @dev Returns a boolean representing whether the given user has claimed prizes for _lottoID
     * @param _lottoID the lottery for which the reward claim status is being checked
     * @param _userAddress the user address to check
     */
     
    function checkRewardClaimed(uint256 _lottoID, address _userAddress) public view returns (bool) {
        return rewardsClaimed[_lottoID][_userAddress];
    }

    // -------------------- Owner Functions --------------------
    function changePrizeMultipliers(uint256[3] memory _prizeMultipliers)
        external
        onlyOwner
    {
        require(
     ...
     * @param _token The ERC-20 to transfer to the contract owner
     */
    ...
    function getRewardMultiplier(
        ...
        uint256[3] memory _prizeMultipliers
    ) internal pure returns (uint256 _rewardMultiplier) {
        require(
            _numberMatching < 5,
