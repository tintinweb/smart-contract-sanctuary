...
contract LiquidityPool is ILiquidityPool, ERC20Upgradeable, AbstractDependant {
    ...
    modifier onlyAssetParameters() {
        require(
            address(assetParameters) == msg.sender,
            "LiquidityPool: Caller not an IAssetParameters."
        );
        _;
    }
    ...
    function getMaxToSupply(address _userAddr) external view override returns (uint256) {
        uint256 _currentUserBalance =
            IERC20(assetAddr).balanceOf(_userAddr).convertTo18(getUnderlyingDecimals());
        uint256 _maxDepositPerAccount = systemParameters.getMaxDepositPerAccountParam();
        uint256 _userTotalSupplyBalance = defiCore.getTotalSupplyBalanceInUSD(_userAddr);

        uint256 _availableAmountInUSD =
            _maxDepositPerAccount > _userTotalSupplyBalance
                ? _maxDepositPerAccount - _userTotalSupplyBalance
                : 0;

        return Math.min(_currentUserBalance, getAmountFromUSD(_availableAmountInUSD));
    }
    ...
    function exchangeRate() public view override returns (uint256) {
        ...
        uint256 _currentBorrowInterest =
            ((_absoluteBorrowedAmount - _aggregatedBorrowedAmount) *
    ...
    function withdrawReservedFunds(
        ...
        bool _isAllFunds
    ) external override onlyAssetParameters {
...
interface IAssetParameters {
    ...
    function getLiquidityPools(bytes32[] memory _keysArr)
        external
        view
