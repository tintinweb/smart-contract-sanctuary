...
contract NFTPackage is NFTUpgradeable, INFTPackage {
    ...
    function tokenURI(uint256 tokenId) public view virtual override(NFTUpgradeable, IERC721MetadataUpgradeable) returns (string memory) {
        ...
        bool imageURIEmpty = bytes(config.imageURI).length == 0;
        return string(
            abi.encodePacked(
                ...
                abi.encodePacked(
                    ...
                    tokenIdString,
                    '","description":"',
                    bytes(packages[_tokenPackage[tokenId]].desc).length == 0 ? config.metaDescription : packages[_tokenPackage[tokenId]].desc,
                    imageURIEmpty ? '","image":"data:image/svg+xml;base64,' : '","image":"',
                    imageURIEmpty ? tokenIdString.textShadow(packages[_tokenPackage[tokenId]].cover).encode() : config.imageURI,
                    '","attributes":[',
    ...
    function _mint(address to, uint256 tokenId, string memory pkg, string memory uri) internal virtual {
        require(hasRole(MINTER_ROLE, msg.sender));
        require(packageTokenCount(pkg) < packages[pkg].totalSupply);
    ...
    function mint(address to, uint256 tokenId, string memory pkg) public virtual override {
        _mint(to, tokenId, pkg, "");
    }

    function mintURI(address to, uint256 tokenId, string memory pkg, string memory uri) public virtual override {
        _mint(to, tokenId, pkg, uri);
    }

    function mintTo(address to, string memory pkg) public virtual override {
        _mint(to, tokenCount()+1, pkg, "");
    }

    function mintToURI(address to, string memory pkg, string memory uri) public virtual override {
        _mint(to, tokenCount()+1, pkg, uri);
    }
}
...
interface INFTPackage is INFTUpgradeable {
    ...
    function mint(address to, uint256 tokenId, string memory pkg) external;
    function mintURI(address to, uint256 tokenId, string memory pkg, string memory uri) external;
    function mintTo(address to, string memory pkg) external;
    function mintToURI(address to, string memory pkg, string memory uri) external;
}
