...
contract MOX is Ownable, ERC721, ERC721Enumerable {
	...
	mapping (uint32 => uint32) public currentQuantity;
    ...
    /**
     * @dev Returns firstOwner address  by id
     *
     */
	function getFirstOwner(uint256 _id) external view returns (address) {
		require(_id < totalSupply(), "NFT not available yet");
		return cardArray[_id].firstOwner;
    }
    ...
    function mintNFT(uint32 _type) public payable {
        ...
        require(currentQuantity[_type]>0, "Not available");
		...
		cardArray.push(CARD(_type, uint256(block.timestamp), getInitialQuality(_type), 0, msg.sender));  
        currentQuantity[_type] = currentQuantity[_type] - 1;
    ...
    /**
     * @dev Public NFT creation function for cards sent in. Creates the NFT type, the NFT, and sends it to the owner.
     *     
     */ 
    function mintNFTSentIn(uint256 _initialPrice, uint32 _initialQuality, address firstOwner) public onlyOwner {
        uint32 __type = indexType + 1;
       
		activeIndex[__type] = true;
		initialPrice[__type] = _initialPrice;
		initialQuality[__type] = _initialQuality;
		currentQuantity[__type] = 0;
		
		uint _mintIndex = totalSupply(); 
        _safeMint(firstOwner, _mintIndex);
		cardArray.push(CARD(__type, uint256(block.timestamp), _initialQuality, 0, firstOwner));  
     ...
     *
     */ 
    function createNewType(uint256 _initialPrice, uint32 _initialQuality, uint32 _quantity) public onlyOwner {
		...
		currentQuantity[indexType] = _quantity;
    }
