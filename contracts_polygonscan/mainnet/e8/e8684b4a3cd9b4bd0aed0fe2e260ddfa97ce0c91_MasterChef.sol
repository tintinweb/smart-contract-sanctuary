...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of GHOSTes
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. GHOSTes to distribute per block.
        uint256 lastRewardBlock;  // Last block number that GHOSTes distribution occurs.
        uint256 accGhostPerShare;   // Accumulated GHOSTes per share, times 1e18. See below.
    ...
    // Ghost referral contract address.
    ...
    // Initial emission rate: 0.6 GHOST per block.
    uint256 public constant INITIAL_EMISSION_RATE = 0.00666 ether;
    // Max GHOST Supply 40 666 GHOST
    uint256 public constant MAX_SUPPLY_CAP = 40666 ether;
    ...
    event UpdateEmissionRate(address indexed user, uint256 indexed ghostPerBlock);
    ...
    constructor(
        ...
        ghostPerBlock = INITIAL_EMISSION_RATE;
    ...
    function getMultiplier(uint256 _from, uint256 _to) public view returns (uint256) {
        if (IERC20(ghost).totalSupply() + totalLockedUpRewards  >= MAX_SUPPLY_CAP) {
            return 0;
        }
    ...
    function pendingGhost(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accGhostPerShare = accGhostPerShare.add(ghostReward.mul(1e18).div(lpSupply));
        }
        uint256 pending =  user.amount.mul(accGhostPerShare).div(1e18).sub(user.rewardDebt);
        return pending.add(user.rewardLockedUp);
    }
    
    // View function to see if user can harvest GHOST's.
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 ghostReward = multiplier.mul(ghostPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        ghost.mint(devAddress, ghostReward.div(10));
        ghost.mint(address(this), ghostReward);
        pool.accGhostPerShare = pool.accGhostPerShare.add(ghostReward.mul(1e18).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        ...
        if (_amount > 0) {
            ...
            if (address(pool.lpToken) == address(ghost)) {
    ...
    // Pay or lockup pending GHOST.
    function payOrLockupPendingGHOST(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        uint256 pending = user.amount.mul(pool.accGhostPerShare).div(1e18).sub(user.rewardDebt);
        if (canHarvest(_pid, msg.sender)) {
            if (pending > 0 || user.rewardLockedUp > 0) {
                ...
                // send rewards
                safeGhostTransfer(msg.sender, totalRewards);
    ...
    function safeGhostTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > ghostBal) {
            transferSuccess = ghost.transfer(_to, ghostBal);
        } else {
            transferSuccess = ghost.transfer(_to, _amount);
        }
        require(transferSuccess, "safeGhostTransfer: Transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _ghostPerBlock) external onlyOwner {
        massUpdatePools();
        ghostPerBlock = _ghostPerBlock;
        emit UpdateEmissionRate(msg.sender, _ghostPerBlock);
    }
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (address(referral) != address(0) && referralCommissionRate > 0) {
            ...
            if (referrer != address(0) && commissionAmount > 0) {
                ghost.mint(referrer, commissionAmount);
