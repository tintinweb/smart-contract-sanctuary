...
contract FarmFactory is ReentrancyGuard, Ownable {
    struct FarmBuild {
        string tokenName;
        string tokenSymbol;
        uint startBlock;
        ...
        uint tokenMaxCapSupply;
        uint16 referralCommissionRate;
        uint[] poolWeights;
        address[] poolAddresses;
        uint16[] poolDepositFees;
    }

    IUniswapV2Factory public uniswap;
    IERC20 public usdc;
    uint public buildFee;
    mapping(address => FarmBuild) public farmBuilds;

    event Build(address owner, address token, address masterChef, address referral, address pair);

    constructor(address _usdc, address _uniswap, uint _buildFee) {
        usdc = IERC20(_usdc);
        uniswap = IUniswapV2Factory(_uniswap);
        buildFee = _buildFee;
    }

    function getPoolWeights() external view returns(uint[] memory) {
        return farmBuilds[msg.sender].poolWeights;
    }

    function getPoolAddresses() external view returns(address[] memory) {
        return farmBuilds[msg.sender].poolAddresses;
    }

    function getPoolDepositFees() external view returns(uint16[] memory) {
        return farmBuilds[msg.sender].poolDepositFees;
    }

    function setBuildFee(uint _buildFee) external onlyOwner {
        buildFee = _buildFee;
    }
    ...
    function configureToken(
        ...
        uint _mintTokenAmount,
        uint _tokenMaxCapSupply
    )
        external
    {
        FarmBuild storage build = farmBuilds[msg.sender];

        build.tokenName = _tokenName;
        build.tokenSymbol = _tokenSymbol;
        build.mintTokenAmount = _mintTokenAmount;
        build.tokenMaxCapSupply = _tokenMaxCapSupply;
    }
        ...
        external
    {
        ...
        build.startBlock = _startBlock;
        build.devAddress = _devAddress;
        build.feeAddress = _feeAddress;
        build.tokenPerBlock = _tokenPerBlock;
        build.referralCommissionRate = _referralCommissionRate;
    }

    function configurePools(
        uint[] calldata _poolWeights,
        address[] calldata _poolAddresses,
        uint16[] calldata _poolDepositFees
    )
        external
    {
        FarmBuild storage build = farmBuilds[msg.sender];

        build.poolWeights = _poolWeights;
        build.poolAddresses = _poolAddresses;
        build.poolDepositFees = _poolDepositFees;
    }

    function build() external payable nonReentrant {
        require(msg.value == buildFee);
        FarmBuild storage build = farmBuilds[msg.sender];
        FarmToken farmToken = new FarmToken(
            build.tokenName,
            build.tokenSymbol,
            build.tokenMaxCapSupply
        );
        ...
        farmToken.mint(build.devAddress, build.mintTokenAmount);
        farmToken.transferOwnership(address(masterChef));

        address newPair = uniswap.createPair(address(farmToken), address(usdc));

        require(build.poolAddresses[0] == address(0));
        require(build.poolAddresses[1] == address(0));
        require(build.poolDepositFees[0] == 0);
        require(build.poolDepositFees[1] == 0);
        masterChef.add(build.poolWeights[0], IERC20(address(farmToken)), 0);
        masterChef.add(build.poolWeights[1], IERC20(newPair), 0);

        for (uint i = 2; i < build.poolWeights.length; i++) {
            masterChef.add(build.poolWeights[i], IERC20(build.poolAddresses[i]), build.poolDepositFees[i]);
        }

        Referral referral = new Referral();
        referral.transferOwnership(msg.sender);
        masterChef.setReferralAddress(IReferral(referral));
        masterChef.updateEmissionRate(build.tokenPerBlock);
        masterChef.setReferralCommissionRate(build.referralCommissionRate);
        masterChef.transferOwnership(msg.sender);

        payable(owner()).transfer(msg.value);

        emit Build(msg.sender, address(farmToken), address(masterChef), address(referral), newPair);
    }
}