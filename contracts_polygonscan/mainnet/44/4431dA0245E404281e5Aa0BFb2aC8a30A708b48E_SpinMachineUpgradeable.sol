...
import { WhitelistableUpgradeable } from "./utils/WhitelistableUpgradeable.sol";
 ...
 */
contract SpinMachineUpgradeable is RescuableUpgradeable, PausableExUpgradeable, WhitelistableUpgradeable, FaucetCallerUpgradeable, RandomableUpgradeable, ISpinMachine {
    ...
    function buyExtraSpin(address spinOwner, uint256 count) public whenNotPaused {
        ...
        IERC20Upgradeable(token).safeTransferFrom(_msgSender(), address(this), extraSpinPrice.mul(count));
     ...
     * Emits an {Spin} event.
     */
    function spin() override external whenNotPaused onlyWhitelisted(_msgSender()) returns (bool success, uint256 winnings) {
    ...
    /**
     * @notice Checks if account is allowed to execute a spin.
     * @param account The address to check.
     * @return True if allowed.
     */
    function canSpin(address account) override external view returns (bool) {
        return (!isWhitelistEnabled() || isWhitelisted(account))
            && (_hasFreeSpin(account) || _hasExtraSpin(account));
    }

    /**
     * @notice Checks if account is allowed to execute free spin.
     * @param account The address to check.
     * @return True if allowed.
     */
...
abstract contract WhitelistableUpgradeable is OwnableUpgradeable {
    ...
    /**
     * @notice Returns whitelister address.
     */
    function getWhitelister() public view virtual returns (address) {
        return _whitelister;
    }
     ...
     * @param newWhitelister The address of new whitelister.
     */
    function setWhitelister(address newWhitelister) external onlyOwner {
     ...
     */
    function isWhitelistEnabled() public virtual view returns (bool) {
        // Shhh compiler wrning..
        _whitelister;
        return true;
    }
}
 ...
 * @notice Allows accounts to renew their Ether balance.
 */
abstract contract FaucetCallerUpgradeable is OwnableUpgradeable {
    ...
    /**
     * @notice Renews recipient's Ether balance.
 ...
 * @notice Extends OpenZeppelin's Pausable contract functionality.
 */
