...
abstract contract ERC721Enumerable is ERC721, IERC721Enumerable {
    ...
    function _removeTokenFromOwnerEnumeration(address from, uint256 tokenId) private {
        ...
        if (tokenIndex != lastTokenIndex) {
            ...
            _ownedTokensIndex[lastTokenId
            ] = tokenIndex; // Update the moved token's index
        }
...
// File contracts/vividot-nft.sol
//// License-Identifier: UNLICENSED
pragma solidity ^0.8.4;


contract VividotNFT is ERC721Enumerable {
    using Strings for uint256;
    ...
    // 0.1%  : 1000
    // 0.01% : 100
    uint256 private constant probabilitySum = 1000000;
    uint256[] private probabilityList = [
        0,
        0,
        100,
        100,
    ...
    constructor() ERC721("vividot nft",
    "VVNFT") {
        address vividot = 0x5485798748221719181499103911646299086586;
        owner = msg.sender;

        for (uint256 i = 1; i < probabilityList.length; i++) {
            probabilityList[i
            ] += probabilityList[i - 1
            ];
        }
        require(
            probabilityList[probabilityList.length - 1
        ] == probabilitySum,
        "probability error"
        );

        setBaseURI(
            "https://gateway.pinata.cloud/ipfs/QmQY5MHqU5SvoXbEgusR6nXebX4NRJXgt1LQPTJgXdkXju"
        );

        reserveNFT(0, vividot);
        for (uint256 i = 0; i < 25; i++) {
            reserveNFT(1, vividot);
        }
        transferOwnership(vividot);
    }

    function setProbability(uint256[] memory _probabilityList)
        public
        onlyOwner()
    {
    ...
    function gacha() public onlyEOA() {
        ...
        uint256 nftType;
        for (uint256 i = 0; i < probabilityList.length; i++) {
    ...
    function reserveNFT(uint256 _nftType, address _to) public onlyOwner() {
        idType[nextTokenId
        ] = _nftType;
        typeCount[_nftType
        ] += 1;
