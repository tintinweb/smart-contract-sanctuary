...
contract PolyHotdogs is BEP20('Poly Hotdogs', 'PolyHotdogs') {
...
contract MasterChef is Ownable {
    ...
    PolyHotdogs public Hotdogs;
    // Dev address.
    address public devaddr;
    // EGG tokens created per block.
    uint256 public HotdogsPerBlock;
    // Bonus muliplier for early Hotdogs makers.
    ...
    constructor(
        PolyHotdogs _Hotdogs,
        address _devaddr,
        address _feeAddress,
        uint256 _HotdogsPerBlock,
        uint256 _startBlock
    ) public {
        Hotdogs = _Hotdogs;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        HotdogsPerBlock = _HotdogsPerBlock;
    ...
    function pendingEgg(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 HotdogsReward = multiplier.mul(HotdogsPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accEggPerShare = accEggPerShare.add(HotdogsReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 HotdogsReward = multiplier.mul(HotdogsPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        Hotdogs.mint(devaddr, HotdogsReward.div(10));
        Hotdogs.mint(address(this), HotdogsReward);
        pool.accEggPerShare = pool.accEggPerShare.add(HotdogsReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    // Safe Hotdogs transfer function, just in case if rounding error causes pool to not have enough EGGs.
    function safeEggTransfer(address _to, uint256 _amount) internal {
        uint256 HotdogsBal = Hotdogs.balanceOf(address(this));
        if (_amount > HotdogsBal) {
            Hotdogs.transfer(_to, HotdogsBal);
        } else {
            Hotdogs.transfer(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _HotdogsPerBlock) public onlyOwner {
        massUpdatePools();
        HotdogsPerBlock = _HotdogsPerBlock;
    }
