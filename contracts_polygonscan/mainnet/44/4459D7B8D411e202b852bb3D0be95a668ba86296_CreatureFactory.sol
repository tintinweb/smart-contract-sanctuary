...
contract CreatureFactory is FactoryERC721, Ownable {
  ...
  string public baseURI = "http://37.187.114.197/TestMetas/";

  /*
   * Enforce the existence of only 100 OpenSea creatures.
   */
  uint256 CREATURE_SUPPLY = 16 * 10**6;
    ...
    returns (string memory)
  {
    return
      string(
        abi.encodePacked(
          baseURI,
          StringsUpperSixHexLength.toHexString(_optionId),
          ".json"
        )
      );
  }
...
/**
 * @dev Modified file from Openzeppelin to fit out format needs: toHexString
 *      returns an hex string with all zeros for 8 char length.
 * @dev String operations.
 */
library StringsUpperSixHexLength {
...
contract Creature is ERC721Tradable {
  ...
  {
    _uri = "http://37.187.114.197/TestMetas/";
  }
    ...
    returns (string memory)
  {
    return
      string(
        abi.encodePacked(
          baseTokenURI(),
          StringsUpperSixHexLength.toHexString(_tokenId),
          ".json"
        )
      );
  }
