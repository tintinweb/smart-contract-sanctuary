...
contract HemetaBondDepository is Ownable {
    ...
    /* ======== STATE VARIABLES ======== */
    address public immutable HEME; // token given as payment for bond
    ...
    struct Bond {
        uint payout; // HEME remaining to be paid
    ...
    function deposit(
        ...
        /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) HEME
         */
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            IERC20( HEME ).transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                IERC20( HEME ).approve( stakingHelper, _amount );
                IStakingHelper( stakingHelper ).stake( _amount, _recipient );
            } else {
                IERC20( HEME ).approve( staking, _amount );
    ...
    function debtRatio() public view returns ( uint debtRatio_ ) {
        uint supply = IERC20( HEME ).totalSupply();
