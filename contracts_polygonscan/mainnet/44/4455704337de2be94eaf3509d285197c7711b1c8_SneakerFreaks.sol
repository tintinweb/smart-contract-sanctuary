...
contract SneakerFreaks is ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;
    using Strings for uint256;

    uint256 public constant FREAKS_PUBLIC = 7000;
    uint256 public constant FREAKS_MAX = FREAKS_PUBLIC;
    uint256 public constant PURCHASE_LIMIT = 20;
    uint256 public constant PRICE = 1_000_000_000_000_000_000; // 10 MATIC 

    string private _contractURI = "";
    string private _tokenBaseURI = "";
    bool private _isActive = false;

    Counters.Counter private _publicFREAKS;
        ...
        onlyOwner
    {
        ...
        require(
            _publicFREAKS.current() < 10000,
            "Purchase would exceed PIXELS_PUBLIC"
        );

        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = _publicFREAKS.current();

            if (_publicFREAKS.current() < FREAKS_PUBLIC) {
                _publicFREAKS.increment();
                _safeMint(to, tokenId);
            }
        }
    }
    ...
    function purchase(uint256 numberOfTokens) external payable {
        ...
        require(
            _publicFREAKS.current() < FREAKS_PUBLIC,
            "Purchase would exceed PIXELS_PUBLIC"
        );
        require(
