...
contract FlashloanArbitrage is IERC3156FlashBorrower, Ownable {
    ...
    function supportDexs(string[] calldata names, ISwap[] calldata handlers) public onlyOwner {
    ...
    function supportDex(string calldata name, ISwap handler) public onlyOwner {
    ...
    function updateDexHandler(string calldata name, ISwap handler, bool status) public onlyOwner {
        require(address(handler) != address(0), "AggregateSwap: handler is the zero address");
    ...
    function onFlashLoan(address initiator, address token, uint256 amount, uint256 fee, bytes calldata data) external override returns (bytes32) {
        ...
        //解构数据
        (address tokenBorrow,uint borrowAmount,address tokenSwapTo,uint256 swapMinAmount,string memory smallHandlerName,string memory biggerHandlerName) = abi.decode(data, (address, uint256, address, uint256, string, string));

        // STEP2:  从最小的交易中获得相关的交易代币
        uint amountOut = swapExtractOut(smallHandlerName, tokenBorrow, tokenSwapTo, address(this), borrowAmount, swapMinAmount, block.number);


        //STEP3:  交易回原来借的币
        swapExtractOut(biggerHandlerName, tokenSwapTo, tokenBorrow, address(this), amountOut, _repayment, block.number);
    ...
    // 设置lender
    function setLender(string calldata name, IERC3156FlashLender lender) public onlyOwner {
        require(address(lender) != address(0), " lender is the zero address");
        flashloanLenders[name] = lender;
        lenders.push(lender);
    }

    function setLenders(string[] calldata names, IERC3156FlashLender[] memory lenders) public onlyOwner {
