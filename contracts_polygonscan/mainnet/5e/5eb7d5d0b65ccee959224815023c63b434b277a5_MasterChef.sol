...
// MasterChef is the master of SPDF. He can make SPDF and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once SPDF is sufficiently
...
contract MasterChef is Ownable {
    ...
    // The EGG TOKEN!
    Newtoken public SPDF;
    // Dev address.
    address public devaddr;
    // EGG tokens created per block.
    uint256 public SPDFPerBlock;
    // Bonus muliplier for early SPDF makers.
    ...
    constructor(
        ...
        address _devaddr,
        address _feeAddress,
        uint256 _SPDFPerBlock,
        uint256 _startBlock
    ) public {
        SPDF = _SPDF;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        SPDFPerBlock = _SPDFPerBlock;
    ...
    function pendingEgg(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 SPDFReward = multiplier.mul(SPDFPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accEggPerShare = accEggPerShare.add(SPDFReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 SPDFReward = multiplier.mul(SPDFPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        SPDF.mint(devaddr, SPDFReward.div(10));
        SPDF.mint(address(this), SPDFReward);
        pool.accEggPerShare = pool.accEggPerShare.add(SPDFReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    // Safe SPDF transfer function, just in case if rounding error causes pool to not have enough EGGs.
    function safeEggTransfer(address _to, uint256 _amount) internal {
        uint256 SPDFBal = SPDF.balanceOf(address(this));
        if (_amount > SPDFBal) {
            SPDF.transfer(_to, SPDFBal);
        } else {
            SPDF.transfer(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _SPDFPerBlock) public onlyOwner {
        massUpdatePools();
        SPDFPerBlock = _SPDFPerBlock;
    }
