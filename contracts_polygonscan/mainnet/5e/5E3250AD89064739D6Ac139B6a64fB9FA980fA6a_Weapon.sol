...
// File: ..\contracts\tools\weapon.sol
...
contract Weapon is ERC721 {
    ...
    function getAttack(uint256 tokenId) view public returns(uint64) {
        return attributes[tokenId].attack;
    }
    ...
    function mintWeapon() public {
        uint64 _attack = uint64(1 + getRnd(77, 15));
        mint(_attack);
    }

    function mint(uint64 _attack) public {
        globalTokenId++;
        _safeMint(msg.sender, globalTokenId);
        attributes[globalTokenId] = Attributes(_attack);
    }
}