...
interface IKommunitasPrivFactory{
    ...
    function createProject(address, address, uint, uint, uint, uint, uint, uint[2] memory, uint[2] memory, uint, uint, uint) external returns (address);
...
interface KommunitasStakingV2{
    function getUserStakedTokens(address _of) external view returns (uint256);
    function getTotalStakedMinPrivateSale() external view returns(uint256 total);
...
contract KommunitasPrivProject{
     ...
     */
    function setTotalStakedMinPrivateSale() public {
        ...
        totalStakedMinPrivateSale = KommunitasStakingV2(factory.stakingV2()).getTotalStakedMinPrivateSale();
    }
...
contract KommunitasPrivFactory is IKommunitasPrivFactory{
    ...
    function createProject(
        ...
        project = address(new KommunitasPrivProject{salt: keccak256(abi.encodePacked(_adminProject))}());

        getProject[_adminProject] = project;
        allProjects.push(project);
        
        KommunitasPrivProject(getProject[_adminProject]).initialize(
            _payment,
            _adminProject,
