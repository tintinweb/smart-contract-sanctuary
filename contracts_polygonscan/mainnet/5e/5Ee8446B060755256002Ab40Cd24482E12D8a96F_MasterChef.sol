...
contract MasterChef is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 accSashimiPerShare;   // Accumulated Sashimi per share, times 1e12. See below.
    ...
    // Update the given pool's SASHIMI allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending SASHIMI on frontend.
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 sashimiReward = multiplier.mul(sashimiPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        sashimi.mint(devaddr, sashimiReward.div(20));
    ...
    // Safe SASHIMI transfer function, just in case if rounding error causes pool to not have enough SASHIMI.
    ...
    function updateEmissionRate(uint256 _sashimiPerBlock) public onlyOwner {
        massUpdatePools();
        sashimiPerBlock = _sashimiPerBlock;
    }
}