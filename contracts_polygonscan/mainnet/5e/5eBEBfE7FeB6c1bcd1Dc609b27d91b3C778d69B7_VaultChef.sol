...
contract VaultChef is Ownable, ReentrancyGuard, Operators {
    ...
    function _deposit(uint256 _pid, uint256 _wantAmt, address _to) internal {
        ...
        if (_wantAmt > 0) {
            ...
            uint256 balanceAfter = pool.want.balanceOf(address(this)).sub(balanceBefore);

            require(balanceAfter > 0, 'deposit zero post tx');
            // uint256 sharesAdded = IStrategy(poolInfo[_pid].strat).deposit(_to, _wantAmt);
            uint256 sharesAdded = IStrategy(poolInfo[_pid].strat).deposit(_to, balanceAfter);
