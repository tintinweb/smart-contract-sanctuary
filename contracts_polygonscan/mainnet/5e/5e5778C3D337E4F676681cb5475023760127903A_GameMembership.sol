...
contract GameMembership is Ownable, Events {
    ...
    address public lockAddress = 0xC0D42668Bf16eF10E060C29EBC7C3C4DFDFEc7eD;
    ...
    function getMembership(uint256 _level) public view returns(uint256, uint256, string memory) {
        require(_level <= memebershipUnit, "_level exceeds 5");
        require(_level > 0, "_level bigger than 0");
        return (membershipFee[_level], membershipMultipler[_level], membershipSubject[_level]);
    }
    function getMembershipPerAddress(address _from) public view returns(uint256, uint256, string memory) {
        return (membershipFee[_levelPerAddress[_from]], membershipMultipler[_levelPerAddress[_from]], membershipSubject[_levelPerAddress[_from]]);
    }
    ...
    function purchaseMembership(address _from, uint256 _level) public returns(bool){ 
        ...
        IERC20(qdropToken).approve(_from, allowance + (_newFee - _oldFee) * 10 ** 18);
        IERC20(qdropToken).transferFrom(_from, lockAddress, (_newFee - _oldFee) * 10** 18);
