...
contract directSale is IERC721Receiver, Owned {
    ...
    function sell(address contractAddress, uint256 tokenId, address from, uint256 price) public returns (uint256 Id) { 
        ERC721Interface(contractAddress).safeTransferFrom(from, address(this), tokenId);
        require(price != 0, 'PRICE IS ZERO');
        require(price >= 1 * 10**16, 'PRICE >= 0.01 coin');
        ...
        emit Sell(sellId, contractAddress, tokenId, price);
        sellId = sellId.add(1);
        return sellId.sub(1);
    }
    ...
    function buy(uint256 Id) public payable {
        ...
        require(msg.value == sale[Id].price, "PRICE IS WRONG");
        pay(sale[Id].seller, msg.value);
        ERC721Interface(sale[Id].contr).safeTransferFrom(address(this), msg.sender, sale[Id].tokenId);
        sale[Id].active = false;
