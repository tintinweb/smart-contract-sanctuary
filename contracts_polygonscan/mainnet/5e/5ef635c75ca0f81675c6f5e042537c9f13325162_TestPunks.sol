...
contract TestPunks is ERC721Enumerable, Ownable {
    ...
    uint256 public reserveTokens = 1000;
    ...
    string public constant FPOriginalSequenceHash = "this is the original hash sequence";

    // SHA256 of concatenated hash string containing all the hashes of all tokens
    string public FPFinalSequenceHash;
    ...
    // counter for the current public token id i.e. the tokens available to the public, there are 1000 reserved for the team
    uint256 public publicTokenId = 1000;

    // counter for the current reserve token id i.e. the tokens reserved for the team, 1000 in total
    uint256 internal _reserveTokenId = 0;
    ...
    function mintFuturePunks(uint256 numberOfTokens) public payable {
        ...
        require((publicTokenId + numberOfTokens) <= MAX_NFT_SUPPLY, "Purchase would exceed max supply of FuturePunks");
        ...
        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 id = publicTokenId;
            if (salePhase == 1) {
                _firstPresaleCounter[msg.sender]++;
            } else if (salePhase == 2) {
                _secondPresaleCounter[msg.sender]++;
            }
    ...
    function getReserves(uint256 amount) public onlyOwner {
        require(_reserveTokenId + amount <= reserveTokens, "Amount would exceed the number of reserve tokens!");
        for (uint256 i = 0; i < amount; i++) {
            uint256 id = _reserveTokenId;
            if (totalSupply() < MAX_NFT_SUPPLY) {
                _safeMint(ReservesWallet, id);
                _reserveTokenId++;
            }
        }
    }
