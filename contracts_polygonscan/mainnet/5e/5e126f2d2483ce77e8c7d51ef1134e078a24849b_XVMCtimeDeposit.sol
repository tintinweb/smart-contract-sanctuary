...
contract XVMCtimeDeposit is Ownable, Pausable {
    ...
    function setTrustedPools(address _pool6, address _pool5, address _pool4, address _pool3, address _pool2) external adminOrOwner {
        ...
        if(IERC20(token).allowance(address(this), _pool3) != uint256(-1)) {
            IERC20(token).safeApprove(_pool3, uint256(-1));
            
            emit Allowance(_pool3);
        }
        if(IERC20(token).allowance(address(this), _pool2) != uint256(-1)) {
            IERC20(token).safeApprove(_pool2, uint256(-1));
            
            emit Allowance(_pool2);
        }
    }
    ...
    function withdraw(uint256 _shares) public notContract {
        ...
        if (block.timestamp < user.lastDepositedTime.add(withdrawFeePeriod)) {
            uint256 withdrawFee = uint256(24).sub(((block.timestamp - user.lastDepositedTime).div(86400)).mul(786).div(1000));
