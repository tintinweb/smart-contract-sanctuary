...
abstract contract BaseStrategy is Ownable {
    ...
    // Dex
    address public constant currentRouter = 0x57dE98135e8287F163c59cA4fF45f1341b680248; //GFI router
...
abstract contract BaseStrategyMasterChef is BaseStrategy {
    ...
    function deposit() public override {
        ...
        if (_want > 0) {
            ...
            IMasterChef(rewards).deposit(_want);
        }
    }
    ...
    function _withdrawSome(uint256 _amount)
        ...
        returns (uint256)
    {
        IMasterChef(rewards).withdraw(_amount);
        return _amount;
    }
    ...
    function _getReward() internal {
        IMasterChef(rewards).withdraw(0);
    ...
    function emergencyWithdraw() public onlyOwner {
        IMasterChef(rewards).emergencyWithdraw();
...
abstract contract StrategyFarmSingle is BaseStrategyMasterChef {
    ...
    address public constant rewardToken = 0x874e178A2f3f3F9d34db862453Cd756E7eAb0381;
    ...
    address public constant gfi = rewardToken;
    address public constant eth = 0x7ceB23fD6bC0adD59E62ac25578270cFf1b9f619;
    address public constant btc = 0x1BFD67037B42Cf73acF2047067bd4F2C47D9BfD6;

    address[] public gfi_eth_path;
    address[] public gfi_btc_path;

    // Other constants
    uint256 public keepFXS = 100;
    uint256 public harvestCutoff = 10**19;
    ...
    {
        gfi_eth_path = new address[](2);
        gfi_eth_path[0] = gfi;
        gfi_eth_path[1] = eth;

        gfi_btc_path = new address[](3);
        gfi_btc_path[0] = gfi;
        gfi_btc_path[1] = eth;
        gfi_btc_path[2] = btc;
    }
    ...
    function harvest() public override {
        ...
        uint256 _harvestBalance = IERC20(rewardToken).balanceOf(address(this));
        if (_harvestBalance > harvestCutoff){
            //Send performance fee to strategist
            uint256 performanceFee = _harvestBalance.mul(keepFXS).div(keepFXSmax);
            if (performanceFee>0) {
            ...
            // Swap 1/2 GFI to eth
            uint256 _harvestBalancePost = IERC20(rewardToken).balanceOf(address(this));
            _swapUniswapWithPath(gfi_eth_path, _harvestBalancePost.div(2));

            // Swap 1/2 GFI to btc
            uint256 _harvestBalancePost2 = IERC20(rewardToken).balanceOf(address(this));
            _swapUniswapWithPathForFeeOnTransferTokens(gfi_btc_path, _harvestBalancePost2);
            
            // Add liquidity
            uint256 _btc = IERC20(btc).balanceOf(address(this));
            uint256 _eth = IERC20(eth).balanceOf(address(this));
            if (_btc > 0 && _eth > 0) {
                IERC20(btc).safeApprove(currentRouter, 0);
                IERC20(btc).safeApprove(currentRouter, _btc);
                IERC20(eth).safeApprove(currentRouter, 0);
                IERC20(eth).safeApprove(currentRouter, _eth);
                ...
                // Donates DUST
                IERC20(btc).safeTransfer(
                    strategist,
                    IERC20(btc).balanceOf(address(this))
                );
...
contract StrategySingle is StrategyFarmSingle {
    // Token addresses
    address public GFI_MASTER_CHEF = 0xa2002c6514387B8386037ea782e626ADc8487fA1;
    address public Want_Token = 0x84D49BeFf6AeCF52C461Eae687d959d64eDf2427;
