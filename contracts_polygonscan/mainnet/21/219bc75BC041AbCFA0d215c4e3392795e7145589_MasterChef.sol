...
contract CrowToken is BEP20 {
     ...
     */
    constructor() public BEP20("PolyCrow Token", "CROW") {
...
contract MasterChef is Ownable, ReentrancyGuard {
  ...
  bool public autoUpdateBlockHarvest = true;
	...
	// Harvest Fee
	uint256 public harvestFee = 6000;
	// Harvest Fee Reduction Rate per period in basis points: 10%.
	uint256 public harvestFeeReductionRate = 1000;
    // Reduce harvest fee every 2939 blocks ~ 2 hours.
    uint256 public constant HARVESTFEE_REDUCTION_PERIOD_BLOCKS = 245;
	// Last reduction period index
    uint256 public lastReductionPeriodIndex = 0;
    // Burn address
  	address public constant BURN_ADDRESS = 0x000000000000000000000000000000000000dEaD;
    // Initial harvest time: 8 hours.
    uint256 public constant INITIAL_HARVEST_TIME = 980;
    // Initial locked time: 2 days.
    uint256 public constant INITIAL_LOCKED_TIME = 743;
    ...
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        ...
        if (_amount > 0) {
            ...
            if (address(pool.lpToken) == address(crow)) {
                if(crow.isExcludedFromFees(msg.sender) == false){

                  uint256 transferTax = _amount.mul(crow.transferTaxRate()).div(10000);
    ...
    function compound(uint256 _amount, address _referrer) public nonReentrant {
      uint256 _pid = 0;
      PoolInfo storage pool = poolInfo[_pid];
      UserInfo storage user = userInfo[_pid][msg.sender];
      updatePool(_pid);
      ...
      if (user.amount > 0) {
          ...
          uint256 harvestRewards = pending;
    ...
    if (block.number >= startBlockHarvest && block.number <= lastBlockHarvest) {
      ...
      if(initialHarvestFee > 0){
        ...
        if (currentIndex > lastReductionPeriodIndex) {
            updateBlock = startBlockHarvest.add(HARVESTFEE_REDUCTION_PERIOD_BLOCKS.mul(currentIndex+1));
