...
import "./JunkyardSprays.sol";
import "./JunkyardBlueprints.sol";

contract JunkyardK9000Parts is ERC1155, Whitelist {
  using Strings for string;
  JunkyardScraps internal jys;
  JunkyardSprays internal sprays;
  JunkyardBlueprints internal blueprints;
  string public contractURI = "https://api.junkyarddogs.io/contract/";

  event BuildDog(uint256[] parts);

  constructor(address jysAddress, address sprayAddress, address blueprintAddress) ERC1155("ipfs://QmXvbRiVdVBa1kQVK72rAF6Et2gWmEVuKzus4v97caPBHw/") {
    jys = JunkyardScraps(jysAddress);
    sprays = JunkyardSprays(sprayAddress);
    blueprints = JunkyardBlueprints(blueprintAddress);
  }

  function buildPart(uint256 blueprint, uint256 sprayCan, uint256[] memory scrapIds, uint256[] memory scrapAmounts) public {
    require(blueprint > 0 && blueprint < 6, "Incorrect blueprint");
    require(sprayCan >= 0 && sprayCan <= 8, "Incorrect Spray Can");
    uint256 tokenId = sprayCan * 10 + blueprint;
    require(blueprints.balanceOf(msg.sender, blueprint) > 0, "You must have the chosen blueprint");
    blueprints.burn(msg.sender, blueprint, 1);
    if (blueprint == 5) {
      require(blueprints.balanceOf(msg.sender, 6) > 0, "You must have the chosen blueprint");
      blueprints.burn(msg.sender, 6, 1);
    }
    if (sprayCan > 0) {
      require(sprays.balanceOf(msg.sender, sprayCan) > 0, "You must have the chosen spray can");
      sprays.burn(msg.sender, sprayCan, 1);
    }
    uint256 burnRate = jys.burningRate();
    uint256 scrapCost = burnRate * burnRate;
    require(spendScraps(burnRate, scrapIds, scrapAmounts) >= scrapCost, "You must pay enough scraps");
    _mint(msg.sender, tokenId, 1, "");
  }

  function buildDog(uint256[] memory parts) public {
    require(parts.length == 5, "You must send 1 of each part");
    bool[] memory types = new bool[](6);
    uint256[] memory amounts = new uint256[](5);
    for(uint256 i = 0; i < parts.length; i++) {
      uint256 part = parts[i];
      require(balanceOf(msg.sender, part) > 0 && !types[part % 10], "You must send 1 of each part");
      types[part % 10] = true;
      amounts[i] = 1;
    }
    _burnBatch(msg.sender, parts, amounts);
    emit BuildDog(parts);
  }

  function paintPart(uint256 part, uint256 sprayCan) public {
    require(balanceOf(msg.sender, part) > 0, "You must have that part");
    require(sprays.balanceOf(msg.sender, sprayCan) > 0, "You must have that spray");
    sprays.burn(msg.sender, sprayCan, 1);
    _burn(msg.sender, part, 1);
    uint256 tokenId = sprayCan * 10 + part % 10;
    _mint(msg.sender, tokenId, 1, "");
  }
  ...
  function alphaBurn(address holder, uint256 tokenId, uint256 amount) public isWhitelisted {
    _burn(holder, tokenId, amount);
  }

  function alphaCraft(address holder, uint256 tokenId, uint256 amount) public isWhitelisted {
...
contract JunkyardSprays is Ownable, ERC1155, Whitelist, JunkyardDroppable1155 {
    using Strings for string;
    string internal cURI = "https://api.junkyarddogs.io/contract/";

    constructor() ERC1155("ipfs://Qmf3uHk2p8L9miRyF86tfQdh2rJgK9MMughZXhXzXtrR1M/") {}
    ...
    function mint(address holder, uint256 sprayId, uint256 amount) public override isWhitelisted {
      _mint(holder, sprayId, amount, "");
    }
...
contract JunkyardBlueprints is Ownable, ERC1155, Whitelist, JunkyardDroppable1155 {
    using Strings for string;
    string internal cURI = "https://api.junkyarddogs.io/contract/";

    constructor() ERC1155("ipfs://QmV92paJxQUQTN4TusR1wRJNxz3m887gq3j7y8Rae7zexu/") {}
    ...
    function burn(address holder, uint256 tokenId, uint256 amount) public isWhitelisted {
      require(
          balanceOf(holder, tokenId) >= amount,
          "Requires enough blueprints to burn"
      );
      _burn(holder, tokenId, amount);
    }
