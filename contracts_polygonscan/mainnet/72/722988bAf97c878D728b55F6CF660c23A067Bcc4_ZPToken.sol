...
contract ZPToken is ERC20, Ownable {
    address[] public _minters;

    function initialize(address _lp_holder, address _presell_address)
        public
        initializer
    {
        Ownable.__Ownable_init();
        ERC20.__ERC20_init("ZeroPlus", "ZP");
        _mint(msg.sender, 1000000 * 1e18);
        _mint(_lp_holder, 19000000 * 1e18);
        _mint(_presell_address, 5000000 * 1e18);
    }
    ...
    function mint(address _to, uint256 _amount) external onlyMinter {
        if (totalSupply() > 100000000 * 1e18) return;
        _mint(_to, _amount);
    }
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "FarmToken::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "FarmToken::delegateBySig: invalid nonce"
        );
        require(
            block.timestamp <= expiry,
            "FarmToken::delegateBySig: signature expired"
        );
        ...
        returns (uint256)
    {
        require(
            ...
            "FarmToken::getPriorVotes: not yet determined"
        );
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            "FarmToken::_writeCheckpoint: block number exceeds 32 bits"
        );
