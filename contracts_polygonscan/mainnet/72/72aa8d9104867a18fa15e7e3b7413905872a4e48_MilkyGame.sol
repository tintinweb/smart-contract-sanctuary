...
contract MilkyGame is Ownable, VRFConsumerBase{
    ...
    uint256 public constant MAX_BURN_BP = 2000;
    ...
    */
    ...
    function initializeGame(uint256 poolNumber_, uint256 pointsPerPool_, uint256 depositFee_, uint256 burnPercent_, uint256 duration_ ,IBEP20 stakingRewardToken_, uint256 initialFunds_ )public onlyOwner{
        require(!gameState.gameInit,'error');
        ...
        require(burnPercent_<= MAX_BURN_BP, 'error');

        gameState.gameInit = true;
        //Initialize gameInfo
        initializeGameInfo(poolNumber_,pointsPerPool_,depositFee_,burnPercent_,duration_,stakingRewardToken_);
    ...
    function rollOverAndAmount() internal {
        ...
        if(finalNumber<gameInfo.poolNumber){
            ...
            uint256 balance = gameInfo.stakingRewardToken.balanceOf(address(this));
            uint256 amountToBurn =  balance.mul(gameInfo.burnPercent).div(10000);
            gameInfo.stakingRewardToken.safeTransfer(DEAD_WALLET, amountToBurn);
        }else{
            uint256 balance = gameInfo.stakingRewardToken.balanceOf(address(this));
            uint256 amountToBurn =  balance.mul(gameInfo.burnPercent).div(10000);
            gameInfo.stakingRewardToken.safeTransfer(DEAD_WALLET, amountToBurn);
    ...
    function initializeGameInfo(uint256 poolNumber_, uint256 pointsPerPool_, uint256 depositFee_,uint256 burnPercent_, uint256 duration_, IBEP20 stakingRewardToken_) internal{
        gameInfo.poolNumber = poolNumber_;
        gameInfo.pointsPerPool = pointsPerPool_;
        gameInfo.depositFee = depositFee_;
        gameInfo.burnPercent = burnPercent_;
