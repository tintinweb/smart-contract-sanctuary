...
contract MockUSDC is ERC20("Mock USDC", "USDC"), AccessProtected {
  ...
  uint256 mintLimit = 100000 ether;

  bool public mintRestriction;
  ...
  function mint() external {
    if (mintRestriction) {
      require(isAdmin(msg.sender), "only admins allowed to mint in mintRestriction mode");
    } else {
      require(mintMap[msg.sender] + mintAmount <= mintLimit, "over mint limit");
    }
    mintMap[msg.sender] += mintAmount;
  ...
  function _beforeTokenTransfer(
    ...
    if (transferRestriction && !isAdmin(from)) {
      require(transferWhitelist[from] || transferWhitelist[to]);
    }
  }
  ...
  function setTransferWhitelist(address _party) external onlyOwner {
    transferWhitelist[_party] = true;
  }
  ...
  function setMintRestriction(bool _restriction) external onlyOwner {
    mintRestriction = _restriction;
  }
