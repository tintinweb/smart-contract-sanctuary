...
NFTs against Humanity is an experiment, but a fun-filled one. Here, our community 
...
contract NFTsAgainstHumanity is ERC721, ERC721URIStorage, Ownable, ContextMixin {
    using Counters for Counters.Counter;
    Counters.Counter private _cardIds;
    uint256 _numBlackCards = 0;
    uint256 _maxBlackCards = 200;
   ...
   */
    ...
    constructor() ERC721("NFTs Against Humanity", "NAH") {}
    ...
    function mintWhiteOrAnswerCard(string memory textOnCard)
        external
        returns (uint256)
    {
        
        uint256 cardId = mintCard();
        setCardInfo(cardId, textOnCard, Color.WHITE);
        return cardId;
    }
    
    
    function mintBlackOrQuestionCard(string memory textOnCard)
        external
        returns (uint256)
    {


        require (_numBlackCards < _maxBlackCards, "Minting 200 black cards only. First come. First mint.");
        _numBlackCards += 1;

        uint256 cardId = mintCard();
        setCardInfo(cardId, textOnCard, Color.BLACK);
        return cardId;
    }
