...
contract SquidGame is Ownable {
    ...
    function startGame(address sender, uint amount) external {
        require(
        ...
        games[sender].steps = new uint[](0);

        emit GameStarted(sender, amount);
    }

    function step(uint8 card) external {
        ...
        game.stepsLeft -= 1;
        game.steps.push(card);

        if (isWin) {
            game.blockNumber = block.number + 1;

            game.winAmount = game.amount * winPercents[game.stepsLeft];
