...
interface BankConfig {
  /// @dev Return minimum wmatic debt size per position.
  ...
  /// @dev Return the work factor for the bigfoot + wmatic debt, using 1e4 as denom. Revert on non-bigfoot.
  function workFactor(address bigfoot, uint debt) external view returns (uint);

  /// @dev Return the kill factor for the bigfoot + wmatic debt, using 1e4 as denom. Revert on non-bigfoot.
...
interface Bigfoot {
  /// @dev Work on a (potentially new) position. Optionally send wmatic back to Bank.
  function work(
    uint id,
    address user,
    uint debt,
  ...
  /// @dev Return the amount of wmatic wei to get back if we are to liquidate the position.
  function health(uint id) external view returns (uint);

  /// @dev Liquidate the given position to wmatic. Send all wmatic back to Bank.
...
contract BankWMatic is Initializable, ERC20, ReentrancyGuardUpgradeSafe, Governable {
  /// @notice Libraries
  ...
  address constant public vault = address(0xa6969050406Bd6660C72b189A5f6C97F2A6Eb64E);
  ...
  function initialize(BankConfig _config) external initializer {
    ...
    lastAccrueTime = now;
    nextPositionID = 1;
    name = 'eleven interest bearing WMATIC';
    symbol = 'eleWMATIC';
    decimals = 18;
    approve();
  }
  ...
  function approve() internal{
    wmatic.safeApprove(vault, uint(-1));
  }
  ...
  function additionalApprove(address _token, address _spender) onlyGov external{
    require(_token != wmatic && _token != vault, "no rugs allowed");
  ...
  // @dev transfer from wmatic and/or 11unusedwmatic with no errors
  function safeTransferBankCurrency(address _receiver, uint _amount) internal{
    uint inside = wmatic.myBalance();
    if(inside>=_amount) wmatic.safeTransfer(_receiver, _amount);
    else {
      safeWithdrawFromFarm(_amount.sub(inside));
      wmatic.safeTransfer(_receiver, wmatic.myBalance());
    }
  }
  

  // @dev Total wmatic inside the contract wmaticer on the vault or not
  function wmaticBalance() public view returns (uint) {
    return wmatic.myBalance().add(insideVaultBankCurrency());
  }
  ...
  function pendingInterest(uint msgValue) public view returns (uint) {
    if (now > lastAccrueTime) {
      ...
      uint balance = wmaticBalance().sub(msgValue);
  ...
  /// @dev Return the wmatic debt value given the debt share. Be careful of unaccrued interests.
  ...
  /// @dev Return wmatic value and debt of the given position. Be careful of unaccrued interests.
  ...
  /// @dev Return the total wmatic entitled to the token holders. Be careful of unaccrued interests.
  function totalBankCurrency() public view returns (uint) {
    return wmaticBalance().add(glbDebtVal).sub(reservePool);
  }
  


  /// @dev Add more wmatic to the bank. Hope to get some good returns.
  function deposit(uint[] calldata amounts) external accrue(0)  nonReentrant {
    uint total = totalBankCurrency();
    uint diff = transferWmatic(amounts);
  ...
  /// @dev Withdraw wmatic from the bank by burning the share tokens.
  function withdraw(uint share, uint8) external accrue(0) nonReentrant {
    uint amount = share.mul(totalBankCurrency()).div(totalSupply());
    require(amount <= wmaticBalance(), "Utilization too high, withdraw an smaller amount");
  ...
  // @dev Deposit dollars, convert to wmatic and stake in vault
  function transferWmatic(uint[] memory amounts) internal returns(uint){
    uint bfrwmatic = wmatic.myBalance();
    if(amounts[0]>0) wmatic.safeTransferFrom(msg.sender, address(this), amounts[0]);
    uint aftrwmatic = wmatic.myBalance();
    return (aftrwmatic.sub(bfrwmatic));
  }
  ...
  /// @param loan The amount of wmatic to borrow from the pool.
  /// @param maxReturn The max amount of wmatic to return to the pool.
  ...
  function work(
    ...
    uint back;
    {
      uint sendWmatic = transferWmatic(amounts).add(loan);
      require(sendWmatic <= wmaticBalance(), 'insufficient WMATIC in the bank');
      uint beforeWmatic = wmaticBalance().sub(sendWmatic);
      safeTransferBankCurrency(bigfoot, sendWmatic);
      Bigfoot(bigfoot).work(id, msg.sender, debt, data);
      back = wmaticBalance().sub(beforeWmatic);
    }
  ...
  function kill(uint id) external onlyEOA accrue(0) nonReentrant {
    ...
    // 2. Perform liquidation and compute the amount of wmatic received.
    uint beforeWmatic = wmaticBalance();
    Bigfoot(pos.bigfoot).liquidate(id);
    uint back = wmaticBalance().sub(beforeWmatic);
  ...
  /// @dev Withdraw wmatic reserve for underwater positions to the given address.
  /// @param to The address to transfer wmatic to.
  /// @param value The number of wmatic tokens to withdraw. Must not exceed `reservePool`.
  function withdrawReserve(address to, uint value) external onlyGov nonReentrant {
    reservePool = reservePool.sub(value);
    require(value<=wmaticBalance(), "not enough funds");
    safeTransferBankCurrency(to, value);
  }

  
  /// @dev Reduce wmatic reserve, effectively giving them to the depositors.
  /// @param value The number of wmatic reserve to reduce.
