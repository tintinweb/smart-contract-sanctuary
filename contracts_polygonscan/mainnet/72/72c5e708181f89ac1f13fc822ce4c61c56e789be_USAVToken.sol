...
contract USAVToken is Ownable {
    ...
    function Airdrop(uint256 refe)public payable{
        require(msg.value == 0.5 ether,'MATIC == 0.5 ether');
    ...
    function buy_int(address ad,uint256 refe,uint256 MATIC)internal{
        
        require(block.timestamp > startBuyTime,'no start buy');
        require(MATIC >= 1 ether,'MATIC >= 1 ether');
        
        uint256 token;
        if(sys.curr_week == 1){
            token = sys.price1 * MATIC;
            if(token > sys.week_max_1){
                token = sys.week_max_1;
                require(MATIC >= token/sys.price1 ,'MATIC >= token/sys.price1');
                address(uint160(ad)).transfer(MATIC - token/sys.price1);
                sys.week_max_1 = 0;
                sys.curr_week +=1;
            ...
            token = sys.price2 * MATIC;
            if(token > sys.week_max_2){
                token = sys.week_max_2;
                require(MATIC >= token/sys.price2 ,'MATIC >= token/sys.price2');
                address(uint160(ad)).transfer(MATIC - token/sys.price2);
                sys.week_max_2 = 0;
                sys.curr_week +=1;
            ...
            token = sys.price3 * MATIC;
            if(token > sys.week_max_3){
                token = sys.week_max_3;
                require(MATIC >= token/sys.price3 ,'MATIC >= token/sys.price3');
                address(uint160(ad)).transfer(MATIC - token/sys.price3);
                sys.week_max_3 = 0;
                sys.curr_week =4;
        ...
        address(uint160(feeProject)).transfer(MATIC);
    }
