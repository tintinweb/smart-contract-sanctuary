...
contract WhelpsNFT is ERC721Enumerable, Ownable {
    ...
    mapping (uint256 => uint256) public tokenIdToIndexBackground;
    mapping (uint256 => uint256) public tokenIdToIndexPattern;
    mapping (uint256 => uint256) public tokenIdToIndexMood;
    mapping (uint256 => uint256) public tokenIdToIndexBreath;
    mapping (uint256 => uint256) public tokenIdToIndexHorns;

    string public baseURI = "https://whelpsio.herokuapp.com/api-poly/";
    ...
    // 2 - use ipfs token id if available, fallback to token id
    // 3 - force use ipfs hash for all
    // 4 - use ipfs hash + computed name if available, fallback to computed name
    ...
    function setIpfsHashByTokenId(uint256 tokenId, string memory hash) external onlyOwnerOrApprovedContract {
        if (ipfsLocked == true) {
          require(bytes(tokenIdToIpfsHash[tokenId]).length == 0);
        }
    ...
    function tokenURIComputedName(uint256 tokenId) public view returns (string memory) {   
        return string(abi.encodePacked(
          namesBreed[tokenIdToIndexBreed[tokenId]],
          "-", namesBackground[tokenIdToIndexBackground[tokenId]],
          "-", namesPattern[tokenIdToIndexPattern[tokenId]],
          "-", namesMood[tokenIdToIndexMood[tokenId]],
          "-", namesBreath[tokenIdToIndexBreath[tokenId]],
          "-", namesHorns[tokenIdToIndexHorns[tokenId]]
        ));
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        string memory ipfsHashByTokenId = tokenIdToIpfsHash[tokenId];
        
        if (metadataSwitch == 1) {
            // force computed name
            return string(abi.encodePacked(base, computedName, "/", tokenId.toString()));
        } else if (metadataSwitch == 2) {
            // ipfs hash / ipfs token id if available, fallback to base token id
            if (bytes(ipfsBase).length == 0) {
              return string(abi.encodePacked(base, tokenId.toString()));
            } else {
              if (bytes(ipfsHashByTokenId).length == 0) {
                return string(abi.encodePacked(ipfsBase, tokenId.toString()));
              } else {
                return string(abi.encodePacked(ipfsBase, ipfsHashByTokenId));
              }
            }
        } else if (metadataSwitch == 3) {
            // force ipfs hash
            return string(abi.encodePacked(ipfsBase, ipfsHash));
        } else if (metadataSwitch == 4) {
            // ipfs hash computed name if available, fallback to base computed name
            if (bytes(ipfsBase).length == 0) {
              return string(abi.encodePacked(base, computedName, "/", tokenId.toString()));
            } else {
              if (bytes(ipfsHash).length == 0) {
                return string(abi.encodePacked(ipfsBase, computedName));
              } else {
                return string(abi.encodePacked(ipfsBase, ipfsHash));
              }
            }
    ...
    struct MintTraitsInfo {
        uint256 indexBreed;
        uint256 indexBackground;
        uint256 indexPattern;
        uint256 indexMood;
        uint256 indexBreath;
        uint256 indexHorns;
    }
            
    function mint(address[] calldata owners, uint256[] calldata tokenIds, MintTraitsInfo[] calldata traits) public onlyOwner {
      require(manualMintAllowed, "Not allowed");
         
      for (uint256 i = 0; i < tokenIds.length; i++) {
        require(!_exists(tokenIds[i]), "Token already minted");
        _mint(owners[i], tokenIds[i]);
        tokenIdToIndexBreed[tokenIds[i]] = traits[i].indexBreed;
        tokenIdToIndexBackground[tokenIds[i]] = traits[i].indexBackground;
        tokenIdToIndexPattern[tokenIds[i]] = traits[i].indexPattern;
        tokenIdToIndexMood[tokenIds[i]] = traits[i].indexMood;
        tokenIdToIndexBreath[tokenIds[i]] = traits[i].indexBreath;
        tokenIdToIndexHorns[tokenIds[i]] = traits[i].indexHorns;
      }
    }
