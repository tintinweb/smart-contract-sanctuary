...
contract StrategyCurveWbtcV1 is StrategyCurveFarmBase {
    address public constant wbtc = 0x1BFD67037B42Cf73acF2047067bd4F2C47D9BfD6; //want token
    ...
    constructor(
        ...
        public StrategyCurveFarmBase(wbtc, curveLpToken, curveGauge, _governance, _strategist, _controller, _timelock)
    {		        
        IERC20(wbtc).safeApprove(curvePool, uint256(-1));
        IERC20(curveLpToken).safeApprove(curvePool, uint256(-1));
        univ2Router2 = 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff;
    ...
    function harvest() public override onlyBenignCallers {
        ...
        if (_wmatic > 0) {
            ...
            //cover wmaitc to want
            _wmatic = IERC20(WMATIC).balanceOf(address(this));
            address[] memory path = new address[](3);
            path[0] = WMATIC;
            path[1] = weth;
            path[2] = want;
            _swapUniswapWithDetailConfig(WMATIC, want, _wmatic, 1, path, sushiRouter);
