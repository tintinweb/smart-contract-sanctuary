...
abstract contract StrategyBase is Ownable, ReentrancyGuard, Pausable, IStrategy {
    ...
    function _checkAutoEarn() internal {
        if (!paused() && !notPublic && pendingHarvestDollarValue() >= autoEarnLimit) {
            earn();
        }
    }
    ...
    function pendingHarvestDollarValue() public view returns (uint256) {
        ...
        return (_pending == 0) ? 0 : exchangeRate(farmingToken, usdc, _pending) * USDC_MISSING_DECIMALS_MULTIPLER;
    }
...
contract StrategySdsLpSdsUsdc is StrategyBase {
    using SafeERC20 for IERC20;
    ...
    // _farmingToken2UsdcPath = [pair(SDS/USDC)]
    // _farmingToken2TargetProfitTokenPath = [pair(SDS/USDC), pair(USDC/MEEB)]
    constructor(
        address _controller,
        address _want,
        address _farmingToken,
    ...
    function getName() public pure override returns (string memory) {
        return "MeebMaster.com:StrategySdsLpSdsUsdc";
    }
