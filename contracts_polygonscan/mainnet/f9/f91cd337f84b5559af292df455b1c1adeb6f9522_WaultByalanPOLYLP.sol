...
// File contracts/interfaces/wault/IWexMaster.sol
...
interface IWexMaster {
    ...
    function claim(uint256 _pid) external; // special harvest metho
...
// File contracts/byalan/ByalanIsland.sol
...
// File contracts/byalan/Sailor.sol
...
// File contracts/byalan/GasThrottler.sol

pragma solidity ^0.8.0;

abstract contract GasThrottler is Ownable {
    address public gasPrice = 0x5d6F545B1953C553D97C1a985BE8E84aE8e02937;
...
// File contracts/byalan/waultswappoly/WaultByalanPOLYLP.sol

pragma solidity ^0.8.0;

contract WaultByalanPOLYLP is ByalanIsland, Sailor, GasThrottler, IByalan, ReentrancyGuard {
    using SafeERC20 for IERC20;
    using Address for address payable;

    // Tokens used
    address public constant WMATIC = 0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270;
    address public constant WEX = 0x4c4BF319237D98a30A929A96112EfFa8DA3510EB;
    address public override want;
    address public lpToken0;
    address public lpToken1;

    // Third party contracts
    address public constant MASTERCHEF = 0xC8Bd86E5a132Ac0bf10134e270De06A8Ba317BFe;
    uint256 public immutable pid;

    // Routes
    address[] public wexToWmaticRoute;
    ...
    constructor(
        ...
        address _treasuryFeeRecipient,
        uint256 _pid,
        address[] memory _wexToLp0Route,
        address[] memory _wexToLp1Route
    )
        ByalanIsland(
            _hydra,
        ...
        )
    {
        ...
        want = IWexMaster(MASTERCHEF).poolInfo(_pid).lpToken;
        lpToken0 = IWaultSwapPair(want).token0();
        lpToken1 = IWaultSwapPair(want).token1();

        wexToWmaticRoute = [WEX, WMATIC];

        if (lpToken0 != WEX) {
            require(_wexToLp0Route[0] == WEX, "invalid lp 0 route");
            require(_wexToLp0Route[_wexToLp0Route.length - 1] == lpToken0, "invalid lp 0 route");
            require(
                IWaultSwapRouter02(unirouter).getAmountsOut(1e18, _wexToLp0Route)[_wexToLp0Route.length - 1] > 0,
                "invalid lp 0 route"
            );
            wexToLp0Route = _wexToLp0Route;
        }

        if (lpToken1 != WEX) {
            require(_wexToLp1Route[0] == WEX, "invalid lp 1 route");
            require(_wexToLp1Route[_wexToLp1Route.length - 1] == lpToken1, "invalid lp 1 route");
            require(
                IWaultSwapRouter02(unirouter).getAmountsOut(1e18, _wexToLp1Route)[_wexToLp1Route.length - 1] > 0,
                "invalid lp 1 route"
            );
            wexToLp1Route = _wexToLp1Route;
        }
    ...
    function deposit() public override whenNotPaused {
        ...
        if (wantBal > 0) {
            IWexMaster(MASTERCHEF).deposit(pid, wantBal, false);
        }
    }
    ...
    function withdraw(uint256 _amount) external override onlyIzlude {
        ...
        if (wantBal < _amount) {
            IWexMaster(MASTERCHEF).withdraw(pid, _amount - wantBal, false);
    ...
    function harvest() external override whenNotPaused onlyEOA gasThrottle {
        // Harvest of Wault it's not deposit 0
        // IWexMaster(MASTERCHEF).deposit(pid, 0, true);
        IWexMaster(MASTERCHEF).claim(pid);
        chargeFees();
        addLiquidity();
        deposit();
    ...
    function chargeFees() internal nonReentrant {
        uint256 toMatic = (IERC20(WEX).balanceOf(address(this)) * totalFee) / MAX_FEE;
        IWaultSwapRouter02(unirouter).swapExactTokensForETH(
            toMatic,
            0,
            wexToWmaticRoute,
            address(this),
        ...
        uint256 callFeeAmount = (maticBal * callFee) / feeSum;
        payable(msg.sender).sendValue(callFeeAmount);

        uint256 treasuryFeeAmount = (maticBal * treasuryFee) / feeSum;
        payable(treasuryFeeRecipient).sendValue(treasuryFeeAmount);

        uint256 kswFeeAmount = (maticBal * kswFee) / feeSum;
        payable(kswFeeRecipient).sendValue(kswFeeAmount);
    }

    // Adds liquidity to AMM and gets more LP tokens.
    function addLiquidity() internal {
        uint256 wexHalf = IERC20(WEX).balanceOf(address(this)) / 2;

        if (lpToken0 != WEX) {
            IWaultSwapRouter02(unirouter).swapExactTokensForTokens(
                wexHalf,
                0,
                wexToLp0Route,
                address(this),
        ...
        IWaultSwapRouter02(unirouter).addLiquidity(
            lpToken0,
            lpToken1,
    ...
    function balanceOfPool() public view override returns (uint256) {
        return IWexMaster(MASTERCHEF).userInfo(pid, address(this)).amount;
    }
    ...
    function pendingReward() external view override returns (uint256) {
        return IWexMaster(MASTERCHEF).pendingWex(pid, address(this)) + IERC20(WEX).balanceOf(address(this));
    }
    ...
    function retireStrategy() external override onlyIzlude {
        IWexMaster(MASTERCHEF).emergencyWithdraw(pid);
    ...
    function _giveAllowances() internal {
        ...
        IERC20(WEX).safeApprove(unirouter, type(uint256).max);

        // lp token 0 and 1 maybe wex so approve 0 is needed here
    ...
    function _removeAllowances() internal {
        ...
        IERC20(WEX).safeApprove(unirouter, 0);
        IERC20(lpToken0).safeApprove(unirouter, 0);
        IERC20(lpToken1).safeApprove(unirouter, 0);
    }
