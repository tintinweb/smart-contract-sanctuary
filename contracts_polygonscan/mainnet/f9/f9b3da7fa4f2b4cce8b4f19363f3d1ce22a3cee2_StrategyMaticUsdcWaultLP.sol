...
contract StratManager is Ownable, Pausable {
     ...
     * @param _krillFeeRecipient address where to send Krill's fees.
     */
    constructor(
        address _keeper,
        address _strategist,
...
abstract contract FeeManager is StratManager {
    uint constant public STRATEGIST_FEE = 350;
    uint constant public MAX_FEE = 1000;
    uint constant public MAX_CALL_FEE = 1000;

    uint constant public WITHDRAWAL_FEE_CAP = 1000;
    uint constant public WITHDRAWAL_MAX = 10000;

    uint public withdrawalFee = 10;

    uint public callFee = 400;
...
contract StrategyMaticUsdcWaultLP is StratManager, FeeManager {
    ...
    ) StratManager(_keeper, _strategist, _unirouter, _vault, _krillFeeRecipient) public {
        want = _want;
        poolId = _poolId;
        chef = _chef;
    ...
    function chargeFees() internal {
        ...
        uint256 krillFeeAmount = nativeBal.mul(krillFee).div(MAX_FEE);
        IERC20(native).safeTransfer(krillFeeRecipient, krillFeeAmount);
    ...
    function convertDustToEarned(address[] memory _lp0ToWantRoute, address[] memory _lp1ToWantRoute) public whenNotPaused {        
        ...
        if (lpToken0 != want && token0Amt > 0) {
            ...
            IUniswapRouter(unirouter).swapExactTokensForTokens(token0Amt, 0, _lp0ToWantRoute, address(this), now);
        ...
        if (lpToken1 != want && token1Amt > 0) {
            ...
            // Swap all dust tokens to want tokens
            IUniswapRouter(unirouter).swapExactTokensForTokens(token1Amt, 0, _lp1ToWantRoute, address(this), now);
        }
    }
}