...
interface IOracle {
    ...
    function getUniSwapPrice(address token) external view returns (Decimal.D256 memory,uint256,uint256);

    function getReserves() external view returns (uint256 dpcpReserves, uint256 tokenReserves);

    function setPair(address _pair,address dpcp) external;

    function getFinalPrice(int256 dpcpAmount) external view returns(Decimal.D256 memory);
...
contract Incentive is Iincentive{
    ...
    constructor(address core,address oracle,address dpcp) public {
        _core = core;
        ...
        (_averagePrice,,) = _oracle.getUniSwapPrice(dpcp);
    ...
    function execute(address sender,address recipient,uint256 amount) external override isCore returns(uint256) {
        ...
        if((_finalPrice.lessThan(_averagePrice))){
            if(isUserSell){ //卖惩罚
                ...
                _dpcp.burnFrom(sender,penalty); // 燃烧
                _totalBurn += penalty;
                ...
                _dpcp.mint(recipient,incentive);
    ...
    function _getCurrentPrice() internal view returns(Decimal.D256 memory ,uint256,uint256){
        (Decimal.D256 memory  price,uint256 dpcpReserve,uint256 otherReserve) = _oracle.getUniSwapPrice(address(_dpcp));
        return (price,dpcpReserve,otherReserve);
    }
    ...
    function _updateVolPrice(uint256 amount) internal {
        ...
        for(uint256 i = 0 ;i< groupNum;i++){
            ...
            if(vol >= _perVol){ // 改组累计成交量已满设定值
                if(i < 9){
                    continue;
                }
            }
            uint256 newVol = vol.add(amount);
            if(newVol <= _perVol){ // 加上新的成交量后依然未超过设定值
                pMap[i].vol = newVol;
            ...
            if(oldPirce.greaterThanOrEqualTo(_finalPrice)){ // 用户卖
            ...
            if(oldPirce.lessThan(_finalPrice)){ // 用户买
    ...
    )
    {
        ...
        if (initialDeviation.equals(Decimal.zero())) {
            uint256 amountToAverage = _getAmountToAvg(reserveDpcp, reserveOther);
    ...
    )
    {
        ...
        (Decimal.D256 memory price, uint256 reserveDpcp,uint256 reserveOther) = _getCurrentPrice();
    ...
    function _getAmountToAvg(
        ...
        Decimal.D256 memory  dpcpPirce = Decimal.one().div(_averagePrice);
        uint256 radicand = dpcpPirce.mul(reserveDpcp).mul(reserveOther).asUint256();
        uint256 root = SafeMathCopy.sqrt(radicand);
        if (root > reserveDpcp) {
            return (root - reserveDpcp).mul(1000).div(997);
        }
        return (reserveDpcp - root).mul(1000).div(997);
    }
    ...
    function setOracle(address uniOracle) external onlyOwner {
        ...
        (_averagePrice,,) = _oracle.getUniSwapPrice(address(_dpcp));
        _finalPrice = _averagePrice;
    }
    ...
    returns (uint256 incentive)
    {
        ...
        uint256 incentivizedAmount = amount;
        // 如果购买结束高于挂钩，则仅激励均价以下部分
        if (finalDeviation.equals(Decimal.zero())) {
            incentivizedAmount = _getAmountToAvg(reserveDpcp, reserveOther);
        }
    ...
    function _calculateBuyIncentiveMultiplier(
        ...
        _calculateIntegratedSellPenaltyMultiplier(finalDeviation, initialDeviation); // 翻转方向

        Decimal.D256 memory buyMultiplier =
        initialDeviation.mul(underAvgWeight).div(_volume);

        if (correspondingPenalty.lessThan(buyMultiplier)) { // 最大的奖励 = 惩罚的金额
