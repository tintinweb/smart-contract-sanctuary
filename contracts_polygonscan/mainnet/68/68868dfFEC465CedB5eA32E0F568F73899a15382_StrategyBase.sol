...
abstract contract BaseStrategy is Ownable {
    ...
    // Dex
    address public constant currentRouter = 0x9055682E58C74fc8DdBFC55Ad2428aB1F96098Fc; //CafeSwap router
...
abstract contract StrategyFarmBase is BaseStrategyMasterChef {
    ...
    // Token addresses 
    address public constant pbrew = 0xb5106A3277718eCaD2F20aB6b86Ce0Fee7A21F09;
    address public constant matic = 0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270;
    address public constant token1 = 0x7ceB23fD6bC0adD59E62ac25578270cFf1b9f619;
    address public constant token2 = 0x53E0bca35eC356BD5ddDFebbD1Fc0fD03FaBad39;
    address public multiHarvest = 0x0000000000000000000000000000000000000000;

    // Other constants
    uint16 public constant underlyingPoolId = 11;
    ...
    // Uniswap swap paths
    address[] public pbrew_token1_path;
    address[] public pbrew_token2_path;
    ...
    {
        pbrew_token1_path = new address[](3);
        pbrew_token1_path[0] = pbrew;
        pbrew_token1_path[1] = matic;
        pbrew_token1_path[2] = token1;

        pbrew_token2_path = new address[](4);
        pbrew_token2_path[0] = pbrew;
        pbrew_token1_path[1] = matic;
        pbrew_token2_path[2] = token1;
        pbrew_token2_path[3] = token2;
    }
    ...
    function harvest() public override {
        ...
        uint256 _pbrewBalance = IERC20(pbrew).balanceOf(address(this));
        if (_pbrewBalance > harvestCutoff){

            //Send performance fee to strategist
            uint256 performanceFee = _pbrewBalance.mul(keepFXS).div(keepFXSmax);
            if (performanceFee>0) {
                _swapUniswapWithPath(pbrew_token1_path, performanceFee);
            ...
            uint256 _pbrewBalancePost = IERC20(pbrew).balanceOf(address(this));
            _swapUniswapWithPath(pbrew_token1_path, _pbrewBalancePost.div(2));

            // Swap remaining to other
            uint256 _pbrewBalancePost2 = IERC20(pbrew).balanceOf(address(this));
            _swapUniswapWithPath(pbrew_token2_path, _pbrewBalancePost2);
...
contract StrategyBase is StrategyFarmBase {
    // Token addresses
    address public CAFESWAP_MASTER_CHEF = 0xca2DeAc853225f5a4dfC809Ae0B7c6e39104fCe5;
    address public ETH_LINK_LP = 0x436803355d26943DD0BC9826D39F9079199a890a;

    constructor()
        public
        StrategyFarmBase(
            CAFESWAP_MASTER_CHEF,
            ETH_LINK_LP,
            msg.sender
        )
    {}
