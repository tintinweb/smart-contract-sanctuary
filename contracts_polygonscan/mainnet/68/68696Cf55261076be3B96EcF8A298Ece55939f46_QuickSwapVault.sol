...
abstract contract PolyCub_QuickSwapVault is Ownable, ReentrancyGuard, Pausable {
    // Maximises yields in QuickSwap, fork of Autofarm

    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    bool public isSameAssetDeposit;
    bool public isCubComp; // this vault is purely for staking. eg. WBNB-CUB staking vault.
    ...
    address public lionsDenAddress;
    address public CUBAddress;
    address public govAddress;
    address public compoundingAddress;
    ...
    function _farm() internal virtual {
        require(isCubComp, "!isAutoComp");
        uint256 wantAmt = IERC20(wantAddress).balanceOf(address(this));
        wantLockedTotal = wantLockedTotal.add(wantAmt);
        IERC20(wantAddress).safeIncreaseAllowance(farmContractAddress, wantAmt);

        IQuickStaker(farmContractAddress).stake(wantAmt);
    }

    function _unfarm(uint256 _wantAmt) internal virtual {
        IQuickStaker(farmContractAddress).exit(_wantAmt);
    }
    ...
    function earn() public virtual nonReentrant whenNotPaused {
        require(isCubComp, "!isCubComp");
        if (onlyGov) {
            require(msg.sender == compoundingAddress, "!compoundingAddress");
        }
    ...
    function setCompoundingAddress(address _newCompAddress) external onlyAllowGov {
        compoundingAddress = _newCompAddress;
    }
...
contract QuickSwapVault is PolyCub_QuickSwapVault {
    constructor(
        ...
        uint256 _entranceFeeFactor,
        uint256 _withdrawFeeFactor,
        address _compoundingAddress
    ) public {
        compoundingAddress = _compoundingAddress;
        wbnbAddress = _addresses[0];
        govAddress = _addresses[1];
        lionsDenAddress = _addresses[2];
