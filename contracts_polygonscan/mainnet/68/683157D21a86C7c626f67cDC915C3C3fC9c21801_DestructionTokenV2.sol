...
import "./libs/AntiBotBlacklist.sol";

// DestructionToken
contract DestructionTokenV2 is AntiBotBlacklist {
    using SafeMath for uint256;
    
    constructor() 
        ERC20('DESTRUCTION', 'DESTRUCTION')
    {
        excludedOperatorsFromAntiBot[msg.sender] = true;
        excludedOperatorsFromAntiBot[address(0)] = true;
        excludedOperatorsFromAntiBot[address(this)] = true;
        excludedOperatorsFromAntiBot[BURN_ADDRESS] = true;
...
abstract contract AntiBotBlacklist is ERC20, Ownable {
    ...
    */
    bool public antiBotWorking = true;
    
    /**
    * @dev Exclude operators from antiBots
    */
    mapping(address => bool) public excludedOperatorsFromAntiBot;
    ...
    */
    ...
    modifier antiBot(address _sender,address _recipient,uint256 _amount) { 
        ...
        if (antiBotWorking){
            ...
            if (_isNotExcludedFromAntiBot(_sender) && _isNotExcludedFromAntiBot(_recipient)){
    ...
    modifier onlyOperator() {
        require(_operator == _msgSender(), "operator:error");
        _;
    }
    ...
    function _addBotAddressToBlackList(address _botAddress, uint256 _expirationTime) internal {
        require(_isNotExcludedFromAntiBot(_botAddress), "ERR4");
        require(blacklist[_botAddress] == 0, "ERR5");

        blacklist[_botAddress] = _expirationTime;
    ...
    function _removeBotAddressToBlackList(address _botAddress) internal{
        require(blacklist[_botAddress] > 0, "ERR6");

        delete blacklist[_botAddress];
    ...
    function blacklistCheck(address botAddress) public view returns(bool){
        if(blacklist[botAddress] > 0)
            return blacklist[botAddress] > block.timestamp;
        else 
            return false;
    }
    /**
    ...
    function blacklistCheckExpirationTime(address botAddress) public view returns(uint256){
        return blacklist[botAddress];
    }


    // EXTERNALS
    function updateOperatorStatusFromAntiBot(address _operatorAddress, bool _status) external onlyOperator {
        require(_operatorAddress != address(0));

        excludedOperatorsFromAntiBot[_operatorAddress] = _status;
    }


    //operator
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "ERR4");
    ...
    function updateMaxUserHoldAmountRate(uint16 _maxUserHoldAmountRate) external onlyOwner {
        require(_maxUserHoldAmountRate >= 50);
    ...
    function updateMaxUserTransferAmountRate(uint16 _maxUserTransferAmountRate) external onlyOwner {
        require(_maxUserTransferAmountRate >= 500);
    ...
    function _isNotExcludedFromAntiBot(address _userAddress) internal view returns(bool) {
        return(!excludedOperatorsFromAntiBot[_userAddress]);
    }
    ...
    function addBotAddressBatch(address[] memory _addresses, uint256 _expirationTime) external onlyOwner {
        require(_addresses.length > 0, "ERR5");
