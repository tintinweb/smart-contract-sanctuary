...
// File: contracts/BIFI/zap/IDMMRouter02.sol
...
// File: contracts/BIFI/zap/BeefyDMMZap.sol
...
contract BeefyDMMZap {
    using LowGasSafeMath for uint256;
    using SafeERC20 for IERC20;
    using SafeERC20 for IBeefyVaultV6;

    IDMMRouter02 public immutable router;
    address public immutable WETH;
    uint256 public constant minimumAmount = 1000;

    constructor(address _router, address _WETH) {
        router = IDMMRouter02(_router);
        WETH = _WETH;
    }
    ...
    function beefOutAndSwap(address beefyVault, uint256 withdrawAmount, address desiredToken, uint256 desiredTokenOutMin) external {
        ...
        path[0] = IERC20(swapToken);
        path[1] = IERC20(desiredToken);
        address[] memory pairs = new address[](1);
        pairs[0] = address(pair);

        _approveTokenIfNeeded(address(path[0]), address(router));
        router.swapExactTokensForTokens(IERC20(swapToken).balanceOf(address(this)), desiredTokenOutMin, pairs, path, address(this), block.timestamp);
    ...
    function _swapAndStake(address beefyVault, uint256 tokenAmountOutMin, address tokenIn) private {
        ...
        path[1] = isInputA ? IERC20(pair.token1()) : IERC20(pair.token0());
        address[] memory pairs = new address[](1);
        pairs[0] = address(pair);
        ...
        if (isInputA) {
            swapAmountIn = _getSwapAmount(address(pair), path[0], path[1], fullInvestment, reserveA, reserveB);
        } else {
            swapAmountIn = _getSwapAmount(address(pair), path[0], path[1], fullInvestment, reserveB, reserveA);
        }

        _approveTokenIfNeeded(address(path[0]), address(router));
        uint256[] memory swapedAmounts = router
            .swapExactTokensForTokens(swapAmountIn, tokenAmountOutMin, pairs, path, address(this), block.timestamp);

        _approveTokenIfNeeded(address(path[1]), address(router));
        (,, uint256 amountLiquidity) = router
            .addLiquidity(path[0], path[1], address(pair), fullInvestment.sub(swapedAmounts[0]), swapedAmounts[1], 1, 1, [0, uint256(-1)], address(this), block.timestamp);
    ...
    function _returnAssets(IERC20[] memory tokens) private {
        uint256 balance;
    ...
    function _getSwapAmount(address pair, IERC20 inputToken, IERC20 outputToken, uint256 investmentA, uint256 reserveA, uint256 reserveB) private view returns (uint256 swapAmount) {
        ...
        uint256 nominator = router.getAmountsOut(halfInvestment, pairs, path)[0];
    ...
    function estimateSwap(address beefyVault, address tokenIn, uint256 fullInvestmentIn) public view returns(uint256 swapAmountIn, uint256 swapAmountOut, address swapTokenOut) {
        ...
        IERC20[] memory path = new IERC20[](2);
        path[0] = IERC20(tokenIn);
        path[1] = IERC20(swapTokenOut);
        address[] memory pairs = new address[](1);
        pairs[0] = address(pair);

        swapAmountIn = _getSwapAmount(address(pair), IERC20(tokenIn), IERC20(swapTokenOut), fullInvestmentIn, reserveA, reserveB);
        swapAmountOut = router.getAmountsOut(swapAmountIn, pairs, path)[0];
    }
