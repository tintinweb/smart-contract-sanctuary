...
New Fairlaunch Token at 20:00 UTC 

Join to our community and get your Cardano Token and Matic Token 
fee reward 
Cardano : 5%
Matic   : 5%
TG      : https://t.me/ADAxMATIC
Website : adaxmatic.finance --> available soon
...
contract ADAxMATIC is ERC20, Ownable {
    ...
    constructor() ERC20("ADAxMATIC", "ADAxMATIC") {
        ...
        teamWallet = 0x583d2A624acFC4a01ad93cdDE2a759ad344992A6;
        AdaDividendToken = 0x7ceB23fD6bC0adD59E62ac25578270cFf1b9f619;
    ...
    function setBuyBackAndLiquifyEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            ...
            totalFees = buyBackAndLiquidityFee.add(marketingFee).add(maticDividendRewardsFee).add(AdaDividendRewardsFee);
            buyBackAndLiquifyEnabled = _enabled;
        }
    ...
    function setAdaDividendEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            ...
            totalFees = AdaDividendRewardsFee.add(marketingFee).add(maticDividendRewardsFee).add(buyBackAndLiquidityFee);
            AdaDividendEnabled = _enabled;
        }
    ...
    function setMaticDividendEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            ...
            totalFees = maticDividendRewardsFee.add(marketingFee).add(AdaDividendRewardsFee).add(buyBackAndLiquidityFee);
            maticDividendEnabled = _enabled;
        }
    ...
    function setMarketingEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            ...
            totalFees = marketingFee.add(maticDividendRewardsFee).add(AdaDividendRewardsFee).add(buyBackAndLiquidityFee);
            marketingEnabled = _enabled;
        }
    ...
    function updateAdaDividendTracker(address newAddress) external onlyOwner {
        ...
        require(newAdaDividendTracker.owner() == address(this), "must be owned by ADAxMATIC");
    ...
    function updateMaticDividendTracker(address newAddress) external onlyOwner {
        ...
        require(newMaticDividendTracker.owner() == address(this), "must be owned by ADAxMATIC");
    ...
    function updateMaticDividendRewardFee(uint8 newFee) external onlyOwner {
        ...
        totalFees = maticDividendRewardsFee.add(AdaDividendRewardsFee).add(marketingFee).add(buyBackAndLiquidityFee);
    }

    function updateMarketingFee(uint8 newFee) external onlyOwner {
        require(newFee <= MAX_FEE_RATE, "wrong");
        marketingFee = newFee;
        totalFees = marketingFee.add(AdaDividendRewardsFee).add(maticDividendRewardsFee).add(buyBackAndLiquidityFee);
    }

    function updateBuyBackAndLiquidityFee(uint8 newFee) external onlyOwner {
        require(newFee <= MAX_FEE_RATE, "wrong");
        buyBackAndLiquidityFee = newFee;
        totalFees = buyBackAndLiquidityFee.add(AdaDividendRewardsFee).add(maticDividendRewardsFee).add(marketingFee);
    }
...
contract MaticDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("Matic_Dividend_Tracker", "Matic_Dividend_Tracker", 0xCC42724C6683B7E57334c4E856f4c9965ED682bD) {
        claimWait = 3600;
