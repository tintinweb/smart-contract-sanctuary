...
contract InteropBetaImplementation is Variables, Initializable, Helpers, Events {
    ...
    function submitSystemAction(
        ...
        bytes32 key = keccak256(abi.encode(sourceChainId, vnonceSystem));
    ...
    function sourceSystemAction(
        string memory systemActionId,
        Position memory position,
        uint256 _vnonceSystem
    ) external onlyOwner {
        uint256 sourceChainId = getChainID();

        bytes32 key = keccak256(abi.encode(sourceChainId, _vnonceSystem));
        require(executeMappingSytem[key] == false, "already-executed");
        executeMappingSytem[key] = true;
...
contract Variables {
    ...
    mapping(bytes32 => bool) public executeMappingSytem;

    uint256 public vnonceSystem;
}
