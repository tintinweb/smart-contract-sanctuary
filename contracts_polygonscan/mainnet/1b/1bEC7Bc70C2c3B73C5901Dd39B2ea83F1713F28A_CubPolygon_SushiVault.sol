...
abstract contract SushiVault is Ownable, ReentrancyGuard, Pausable {
    // Maximises yields in Sushiswap, fork of Autofarm pancakeswap vault
    ...
    function _farm() internal virtual {
        ...
        ISushi(farmContractAddress).deposit(pid, wantAmt, address(this));
    }

    function _unfarm(uint256 _wantAmt) internal virtual {
        if (_wantAmt == 0){
          ISushi(farmContractAddress).harvest(pid, address(this));
        } else {
          ISushi(farmContractAddress).withdraw(pid, _wantAmt, address(this));
        }
    }
    ...
    function earn() public virtual nonReentrant whenNotPaused {
        require(isCubComp, "!isCubComp");
        if (onlyGov) {
            require(msg.sender == compoundingAddress, "!gov");
        }
    ...
    function _wrapMATIC() internal virtual {
        ...
        if (bal > 0) {
            IWMATIC(wbnbAddress).deposit{value: bal}(); // MATIC -> WMATIC
        }
    }
...
contract CubPolygon_SushiVault is SushiVault {
    constructor(
