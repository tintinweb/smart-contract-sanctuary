...
contract ImpulseStakingMulti is AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    /// @param _pid Pool's id
    /// @param _wantAmt Amount to withdraw
    function withdrawInUnderlying(uint256 _pid, uint256 _wantAmt) public virtual nonReentrant poolRunning(_pid) hasPool(_pid) {
        updatePool(_pid);
        PoolInfo memory pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_msgSender()];
        ...
        uint256 sharesAmount = strategy.withdrawInUnderlying(withdrawTotalAmount);
        user.shares -= sharesAmount;
        user.depositInWant -= _wantAmt;
        address[] memory listUnderlying = strategy.listUnderlying();
        for (uint256 u = 0; u < listUnderlying.length; u++) {
    ...
    function withdrawInOneUnderlying(
        ...
        uint256 _wantAmt,
        address _underlying
    ) public virtual nonReentrant poolRunning(_pid) hasPool(_pid) {
        updatePool(_pid);
        PoolInfo memory pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_msgSender()];
        ...
        uint256 userTotalAmount = ((user.shares * strategy.wantLockedTotal()) / strategy.sharesTotal());
        uint256 withdrawTotalAmount = (_wantAmt * userTotalAmount) / user.depositInWant;
        uint256 sharesAmount = strategy.withdrawInOneUnderlying(withdrawTotalAmount, _underlying);

        user.shares -= sharesAmount;
        user.depositInWant -= _wantAmt;

        IERC20(_underlying).safeTransfer(_msgSender(), IERC20(_underlying).balanceOf(address(this)));
