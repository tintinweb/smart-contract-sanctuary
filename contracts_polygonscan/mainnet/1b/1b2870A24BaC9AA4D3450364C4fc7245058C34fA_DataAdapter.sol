...
contract DataAdapter {
	...
	/**
    @dev These are the markets, MATIC, USDT, USDC and DAI.
  **/
	...
	**/
    ...
    @param _user the user that we want to get the data.
	**/
	function getDataForAssetAave(address _asset, address _user)
		public
		view
		returns (AssetDataAave memory data)
	{
		uint8 decimals = IAToken(memoryContract.getAToken(_asset)).decimals();
		string memory symbol = IAToken(memoryContract.getAToken(_asset))
			.symbol();
		...
		) = protocolData.getUserReserveData(_asset, _user);
	...
	**/
		...
		returns (UserDataCream memory data)
	{
		uint256 totalDebt;
		uint256 ltv;
		(, uint256 totalCollateral, ) = compTroller.getAccountLiquidity(_user);

		for (uint256 tokenIndex; tokenIndex < markets.length; tokenIndex++) {
			ICToken crToken = ICToken(
				memoryContract.getCrToken(markets[tokenIndex])
			);
			totalDebt += crToken.borrowBalanceStored(_user);
		}

		if (totalDebt != 0) {
			ltv = totalDebt.div(totalCollateral);
		}

		data = UserDataCream(totalCollateral, totalDebt, 0, ltv, 0);
	}
    ...
    @param _user the user that we want to get the data.
	**/
	function getDataForAssetCream(address _asset, address _user)
		public
		view
		returns (AssetDataCream memory data)
	{
		...
		) = crAsset.getAccountSnapshot(_user);
		uint256 totalUnderlyingSupplied = balance.mul(_exchangeRate).div(
			1 ether
		);
		uint256 storedBorrowBalance = borrowBalance;
		uint8 decimals = crAsset.decimals();
		string memory symbol = crAsset.symbol();
		data = AssetDataCream(
	...
	/**
		...
		@param _user asset to get the data from both protocols.
	**/
	function getDataAssetOfProtocols(address _asset, address _user)
		public
		view
		returns (DataAssetOfProtocols memory data)
	{
		data = DataAssetOfProtocols(
			getDataForAssetAave(_asset, _user),
			getDataForAssetCream(_asset, _user)
		);
	}
