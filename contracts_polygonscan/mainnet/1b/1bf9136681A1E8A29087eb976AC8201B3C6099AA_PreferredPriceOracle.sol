 ...
 * @notice Returns prices from ChainlinkPriceOracleV2 or prices from a secondary oracle (in order of preference).
...
contract PreferredPriceOracle is PriceOracle, BasePriceOracle {
     ...
     */
    BasePriceOracle public secondaryOracle;
    
    /**
     * @dev Constructor to set the primary `ChainlinkPriceOracleV2` and the secondary `PriceOracle`.
     */
    constructor(ChainlinkPriceOracleV2 _chainlinkOracleV2, BasePriceOracle _secondaryOracle) public {
    ...
    function _price(address token) internal view returns (uint) {
        ...
        if (token == 0x7ceB23fD6bC0adD59E62ac25578270cFf1b9f619) return 1e18;

        // Try to get ChainlinkPriceOracleV2 price
        if (address(chainlinkOracleV2.priceFeeds(token)) != address(0)) return chainlinkOracleV2.price(token);

        // Otherwise, get price from secondary oracle
        return secondaryOracle.price(token);
    }
    ...
    function getUnderlyingPrice(CToken cToken) external override view returns (uint) {
        require(!cToken.isCEther(), "CEther is unsupported");
        address underlying = CErc20(address(cToken)).underlying();
        uint256 decimals = uint256(ERC20Upgradeable(underlying).decimals());
        // Comptroller needs prices to be scaled by 1e(36 - decimals)
        // Since `_price` returns prices scaled by 18 decimals, we must scale them by 1e(36 - 18 - decimals)
        return _price(underlying).mul(1e18).div(10 ** decimals);
    }
}
...
contract ChainlinkPriceOracleV2 is PriceOracle, BasePriceOracle {
    ...
    enum FeedBaseCurrency {
        ETH,
        USD,
        BTC /// @notice we are not using BTC
    }

    /**
     * @notice Chainlink ETH/USD price feed contracts.
     */
    AggregatorV3Interface public constant ETH_USD_PRICE_FEED = AggregatorV3Interface(0xF9680D99D6C9589e2a93a78A04A279e509205945);
    ...
    function _price(address underlying) internal view returns (uint) {
        // Return 1e18 for WETH
        if (underlying == 0x7ceB23fD6bC0adD59E62ac25578270cFf1b9f619) return 1e18;
