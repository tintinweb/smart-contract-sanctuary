...
interface IMasterchef {
    ...
    function userInfo(uint256 _pid, address _address) external view returns (uint256, address);
...
abstract contract BaseWormStrategy is Ownable, ReentrancyGuard, Pausable {
    ...
    function withdraw(address _userAddress, uint256 _farmedAmt) external onlyOwner nonReentrant returns (uint256, uint256) {
        ...
        uint256 wantWithdrawn = IERC20(wantAddress).balanceOf(address(this));
        IERC20(wantAddress).safeTransfer(wormChefAddress, wantWithdrawn);
...
contract WormStrategyLPUsdcBall is BaseWormStrategyLPSingle {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
