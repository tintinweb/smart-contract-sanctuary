...
contract UnusedUsdcBankStrat is Ownable, Pausable {
    ...
    address[] public wmaticToWbtc = [wmatic, wbtc];
    ...
    constructor(address _vaultAddress) public {
        ...
        IERC20(wbtc).safeApprove(aavebank, 0);
        IERC20(wbtc).safeApprove(aavebank, uint(-1));
    ...
    function deposit() public {
        ...
        if (pairBal > 0) {
            ILendingPool(aavebank).deposit(wbtc, IERC20(wbtc).balanceOf(address(this)), address(this), 0);
        }
    }
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (pairBal < _amount) {   
            ILendingPool(aavebank).withdraw(wbtc,_amount.sub(pairBal), address(this));
    ...
    function swap() internal{
        ...
        IPanwexRouter(unirouter).swapExactTokensForTokens(amount, 0, wmaticToWbtc, address(this), now + 600);
    }
    ...
    function balanceOfPool() public view returns (uint256) {
        (uint256 supplyBal,,,,,,,,) = IDataProvider(dataProvider).getUserReserveData(wbtc, address(this));
        return supplyBal;
    }
    ...
    function panic() public onlyOwner {
        ILendingPool(aavebank).withdraw(wbtc, type(uint).max, address(this));
        pause();
    }
