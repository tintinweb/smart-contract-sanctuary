...
contract ExchangeExtractorV3 {
    ...
    function estimateSwap(
        ...
        for (uint256 i = 0; i < routers.length; i++) {
            amounts = IUniswapV2Router02(routers[i]).getAmountsOut(
                currentAmountIn,
                paths[i]
            );
        ...
        for (uint256 i = 0; i < routers.length; i++) {
            (bool success, bytes memory res) = address(this).staticcall(
                abi.encodeWithSignature(
                    "estimateSwap(address[],address[][],uint256)",
                    routers[i],
                    paths[i],
                    amountsIn[i]
                )
            ); //estimateSwap(routers[i], paths[i], amountsIn[i]);
            if (success) {
                uint256 amountOut;
                assembly {
                    amountOut := mload(add(res, add(0x20, 0)))
                }
                results[i] = amountOut;
            } else {
                results[i] = 0;
