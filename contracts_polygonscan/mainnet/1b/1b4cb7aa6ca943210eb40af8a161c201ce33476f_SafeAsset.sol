...
contract SafeAsset is ERC20UpgradeSafe, OwnableUpgradeSafe, ReentrancyGuard, ISafeAsset {
    ...
    event SafeAssetBurned(address indexed from, address indexed to, uint256 amount);
    event SafeAssetMinted(address indexed from, address indexed to, uint256 amount);

    /* ========== Modifiers =============== */

    modifier onlyPool() {
        require(ISafeTreasury(treasury).hasPool(msg.sender), "!pool");
        _;
    }
