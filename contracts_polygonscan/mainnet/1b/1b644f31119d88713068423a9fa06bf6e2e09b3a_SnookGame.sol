...
contract SnookToken is ERC721Upgradeable, ERC721BurnableUpgradeable, ERC721EnumerableUpgradeable, OwnableUpgradeable {
    ...
    // multimint which uses in _multimint - a function added to OZ contract which was specially modified
      ...
      external onlyGameContracts() returns (uint[] memory) 
    {
      ...
      _multimint(to, tokenIds);
      return tokenIds;
    }

    function multimint22(address to, string[] calldata tokenURIs) 
      external onlyGameContracts() returns (uint[] memory) 
    {
      uint[] memory tokenIds = new uint[](tokenURIs.length);
      for (uint i=0; i<tokenURIs.length; i++) {
...
interface ISnookGame is IDescriptorUser {
  ...
  function mint21(uint count) external;
  function mint22(uint count) external;
...
contract SnookGame is ISnookGame, AccessControlEnumerableUpgradeable, PausableUpgradeable {
    ...
    uint private constant TRAITCOUNT_MINT2 = 1;
    ...
    function mint2(uint count) external override whenNotPaused() {
      ...
      uint g1 = gasleft(); // 823315
      uint price = _uniswap.getSnookPriceInSkills();
      require(
        _skill.transferFrom(
          msg.sender, // from 
          address(this),  // to 
          count * price // total amount
        ), 
        'SnookGame: No funds'
      );
      ...
      uint g2 = gasleft(); // 285553
      emit GasUsage(g1, g2);  
    }

    function mint21(uint count) external override whenNotPaused() {
      require(count > 0, 'SnookGame: should be greater than 0');
      uint g1 = gasleft(); // 823315
      uint price = _uniswap.getSnookPriceInSkills();
      uint amountPaid = count * price;
      require(
        _skill.transferFrom(
          msg.sender, // from 
          address(this),  // to 
      ...
      for (uint i=0; i<count; i++) { 
        ...
        _state.setDescriptor(tokenIds[i], descriptor); // this can be probably change'd to a single call
      }
    ...
    function mint22(uint count) external override whenNotPaused() {
      require(count > 0, 'SnookGame: should be greater than 0');
      uint g1 = gasleft(); // 823315
      uint price = _uniswap.getSnookPriceInSkills();
      uint amountPaid = count * price;
      ...
      uint[] memory tokenIds = _snook.multimint22(msg.sender, tokenURIs);
    ...
    function _mint2(uint price) internal 
    {
        // uint price = _uniswap.getSnookPriceInSkills();
        ...
        _afterdeath.updateOnMint(TRAITCOUNT_MINT2);
        // emit Birth2(msg.sender, tokenId, price, traitId);
    }
