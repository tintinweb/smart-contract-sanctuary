...
contract PolyPandaFarm is BEP20('Poly PandaFarm', 'PolyPandaFarm') {
...
contract MasterChef is Ownable {
    ...
    PolyPandaFarm public PandaFarm;
    // Dev address.
    address public devaddr;
    // EGG tokens created per block.
    uint256 public PandaFarmPerBlock;
    // Bonus muliplier for early PandaFarm makers.
    ...
    constructor(
        PolyPandaFarm _PandaFarm,
        address _devaddr,
        address _feeAddress,
        uint256 _PandaFarmPerBlock,
        uint256 _startBlock
    ) public {
        PandaFarm = _PandaFarm;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        PandaFarmPerBlock = _PandaFarmPerBlock;
    ...
    function pendingEgg(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 PandaFarmReward = multiplier.mul(PandaFarmPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accEggPerShare = accEggPerShare.add(PandaFarmReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 PandaFarmReward = multiplier.mul(PandaFarmPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        PandaFarm.mint(devaddr, PandaFarmReward.div(10));
        PandaFarm.mint(address(this), PandaFarmReward);
        pool.accEggPerShare = pool.accEggPerShare.add(PandaFarmReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    // Safe PandaFarm transfer function, just in case if rounding error causes pool to not have enough EGGs.
    function safeEggTransfer(address _to, uint256 _amount) internal {
        uint256 PandaFarmBal = PandaFarm.balanceOf(address(this));
        if (_amount > PandaFarmBal) {
            PandaFarm.transfer(_to, PandaFarmBal);
        } else {
            PandaFarm.transfer(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _PandaFarmPerBlock) public onlyOwner {
        massUpdatePools();
        PandaFarmPerBlock = _PandaFarmPerBlock;
    }
