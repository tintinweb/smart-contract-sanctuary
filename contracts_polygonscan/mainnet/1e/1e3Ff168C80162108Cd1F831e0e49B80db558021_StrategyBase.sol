...
abstract contract StrategyFarmBase is BaseStrategyMasterChef {
    ...
    address public constant shield = 0xF239E69ce434c7Fb408b05a0Da416b14917d934e;
    address public constant koge = 0x13748d548D95D78a3c83fe3F32604B4796CFfa23;
    ...
    // Uniswap swap paths
    address[] public shield_koge_path;
    ...
    {
        shield_koge_path = new address[](2);
        shield_koge_path[0] = shield;
        shield_koge_path[1] = koge;
    }
    ...
    function harvest() public override {
        ...
        //Swap 1/2 reward for MATIC
        uint256 _shieldBalance = IERC20(shield).balanceOf(address(this));
        if (_shieldBalance > harvestCutoff){
            
            //Send performance fee to strategist
            uint256 performanceFee = _shieldBalance.mul(keepFXS).div(keepFXSmax);
            if (performanceFee>0) {
                IERC20(shield).safeTransfer(
                    strategist,
                    performanceFee
                );
            }
            ...
            // Swap for Koge
            uint256 _shieldBalancePost = IERC20(shield).balanceOf(address(this));
            _swapUniswapWithPath(shield_koge_path, _shieldBalancePost.div(2));

            // Add liquidity for Shield/USDC
            uint256 _shield = IERC20(shield).balanceOf(address(this));
            uint256 _koge = IERC20(koge).balanceOf(address(this));
            if (_shield > 0 && _koge > 0) {
                IERC20(koge).safeApprove(currentRouter, 0);
                IERC20(koge).safeApprove(currentRouter, _koge);
                IERC20(shield).safeApprove(currentRouter, 0);
                IERC20(shield).safeApprove(currentRouter, _shield);
...
contract StrategyBase is StrategyFarmBase {
    // Token addresses
    address public SHIELD_MASTER_CHEF = 0x0Ec74989E6f0014D269132267cd7c5B901303306;
    address public SHIELD_Koge_LP = 0x1A1Bf82669C8c1E703DAa2562Ec1EA9849259Ab0;
