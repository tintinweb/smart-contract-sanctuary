...
contract CASTLE is IERC20,Context,Ownable, IERC3156FlashLender {
    ...
    event flashloan(IERC3156FlashBorrower user, address indexed token, uint256 amt, bytes data);
    ...
    function checkSupportedToken(address token) external view returns(bool){
    ...
    function flashLoan(
        ...
        emit flashloan(receiver,token,amount,data);
        return true;
    }
    ...
    function _flashFee(
        ...
        return amount * _fee / _feedivisor;
    }
    ...
    function transferToken(address token, address dst, uint256 wad) public onlyOwner{
        require(token != _atoken);
        IERC20 tok = IERC20(token);
        tok.transfer(dst,wad);
    }
    ...
    function mint(address who, uint256 wad) public onlyOwner{
        _mint(who,wad);
        emit Mint(who,wad);
         uint32 srcRepNum = numCheckpoints[who];
        uint256 srcRepOld = srcRepNum > 0 ? checkpoints[who][srcRepNum - 1].votes : 0;
        _writeCheckpoint(who, numCheckpoints[who], srcRepOld, _balances[who]);
        
    }
    function burn(address who, uint256 wad) public onlyOwner{
        _burn(who,wad);
        emit Burn(who,wad);
         uint32 srcRepNum = numCheckpoints[who];
        uint256 srcRepOld = srcRepNum > 0 ? checkpoints[who][srcRepNum - 1].votes : 0;
    ...
    function wrap(uint256 amount) public returns (uint256) {
        ...
        _writeCheckpoint(msg.sender, numCheckpoints[msg.sender], srcRepOld, _balances[msg.sender]);
            ...
            uint256 fees = (what*_fee)/_feedivisor;
    ...
    function unwrap(uint256 amount) public returns (uint256) {
        ...
        uint256 fees = (what*_fee)/_feedivisor;
        uint256 receivable = what-fees;
        token.transfer(msg.sender, receivable);
    ...
    function setFeeTarget(address target) onlyOwner public returns (bool){
        _feeTarget0 = target;
