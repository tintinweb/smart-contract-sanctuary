...
contract ifFactory {
    ...
    string public name="Uranium ";
    string public prefix="u";
    ...
    function createCustomIfToken(address _underlying, string memory _name, string memory _symbol) onlyManager() public returns(address) {
        require(!exists[_underlying], "createIfToken: already exists.");

        ifToken token = new ifToken(_underlying, _name, _symbol, msg.sender);
    ...
    function createIfToken(address _underlying) onlyManager() public returns(address) {
        require(!exists[_underlying], "createIfToken: already exists.");

        EZERC20 Token = EZERC20(_underlying);

        ifToken token = new ifToken(_underlying, append(name, Token.name() ), append(prefix, Token.symbol()), msg.sender);
        exists[_underlying] = true;
        ifTokens[_underlying] = address(token);
...
contract ifToken is ERC20, ERC20Detailed {
    ...
    uint8 public count;
    // Is this MakerDao?
    uint8 public drag; // min reduction for a poor performing pool
    uint8 public push; // min increase for a well-performing pool
    ...
    function addPool(address _LP, uint8 _points) public onlyAdmin() {
        require(!exists[_LP], "addPool: pool's already exists. Use updatePool() to update it or remove it.");
        require(router.getUniswapV2Pair(_LP) != address(0), "addPool: LP address is invalid.");
        require(count<uint8(254), "addPool: too many pools!");

        uint8 _pool = count+1;
        pools[_pool] = _LP;
        points[_pool] = _points;

        (, address colA, address colB) = router.getLendingPool(_LP);

        ImperMaxBrrwble _A = ImperMaxBrrwble(colA);
        ImperMaxBrrwble _B = ImperMaxBrrwble(colB);

        if(_A.underlying() == address(Token)){
            settings[_pool] = false;
        } else if(_B.underlying() == address(Token)) {
            settings[_pool] = true;
        } else{
            revert();
        }
        ...
        emit AddPool(_pool, _LP, settings[_pool], _points);
    }

    function updatePool(uint8 _pool, uint8 _points) public onlyAdmin() {
        points[_pool] = _points;
        emit UpdatePool(_pool, _points);
    }

    function updateDrag(uint8 _drag) public onlyAdmin() {
        require(_drag>1, "updateDrag: invalid value.");
        drag=_drag;
    }

    function updatePush(uint8 _push) public onlyAdmin() {
        require(_push>1, "updatePush: invalid value.");
        push=_push;
    }

    function optimizePoints() public onlyOperator() {
        uint8 worst = findWorstPool();
        uint8 best = findBestPool();

        if(worst != best) {
            if(points[worst] < drag){
                points[worst]=0;
            } else {
                points[worst]= (points[worst]-drag);
            }

            if(points[best] < (254-best)){
                points[best] = (points[best] + best);
            }
        }
        pull();
        invest();
    ...
    modifier onlyTreasury() {
        require(msg.sender ==  treasury, "onlyTreasury: not allowed");
        _;
    }
    ...
    function updatePerformanceFee(uint256 _perfFee) public onlyAdmin() {
        require(_perfFee<=2000, "updatePerformanceFee: max is 2000 basis points.");
        performanceFee=_perfFee;
        emit UpdatePerformanceFee(_perfFee);
    }

    function updateManagementFee(uint256 _mgmtFee) public onlyAdmin() {
        require(_mgmtFee<=200, "updateManagementFee: max is 200 (2%) basis points per year.");
        mgmtFee=_mgmtFee;
    ...
    function findWorstPool() view public returns(uint8) {
    ...
    function getValueLocked() public view returns (uint256) {
        uint256 totalInvestable= IERC20(Token).balanceOf(address(this));
        return  totalInvestable.add(checkInvestedLast());
    }
    ...
    function pull() internal {
        ...
        // pull from all
        for (uint8 i=1; i<=count; i++) {
            address brrwbleMai_addr = getBrrwable(i);
            ImperMaxBrrwble brrwbleMai = ImperMaxBrrwble(brrwbleMai_addr);

            uint256 bal = brrwbleMai.balanceOf(address(this));
            uint256 totalBal = brrwbleMai.totalBalance();//.div(brrwbleMai.exchangeRate());

            if(bal>2){
                bytes memory nothing;
                if(bal >= totalBal){
                    router.redeem(brrwbleMai_addr, totalBal.sub(1), address(this), deadline, nothing);
                } else {
                    router.redeem(brrwbleMai_addr, bal.sub(1), address(this), deadline, nothing);
                }                
            }
            //redeem(address poolToken, uint256 tokens, address to, uint256 deadline, bytes permitData) 
        }
