...
abstract contract StrategyBase is Ownable, ReentrancyGuard, Pausable, IStrategy {
    ...
    function exchangeRate(
        ...
        uint256[] memory amounts = IPolyDexFormula(polydexFormula).getAmountsOut(_inputToken, _outputToken, _tokenAmount, polydexPaths[_inputToken][_outputToken]);
...
contract StrategyStakePlx is StrategyBase {
    ...
    // _farmingToken2UsdcPath = [pair(PLX/USDC)]
    // _farmingToken2TargetProfitTokenPath = [pair(PLX/USDC), pair(USDC/TOP)]
    constructor(
        address _controller,
        address _want,
        address _farmingToken,
