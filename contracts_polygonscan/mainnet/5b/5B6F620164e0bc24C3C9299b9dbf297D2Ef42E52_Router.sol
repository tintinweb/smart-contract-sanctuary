...
// Not necessary in solidity 0.8.X
//import "./SafeMath.sol";
...
contract Router is Ownable, IRouter{
    ...
    constructor(address _equivalenceContract){
    ...
    function isInBlackList(address _address)  override view public returns(bool){
        //console.log("CONTRACT - isInBlackList %s",_address);
    ...
    /*This function is called by Coin Toss House and Predictions House while placing the bets. It transfers the BOLD token from User to this router 
      contract, subtracts treasury fees and burns the remaining amount of BOLD. For ex: if a user places a bet of 10 BOLD on Tails using the coin toss
      contract, it will preserve 0.3 BOLD in form of fees, while burn the remaining 9.97 BOLD*/
    ...
    function transferAndBurn(address _token, address _from, uint _amount,uint treasuryfee) override external onlyAllowedContracts notBlackListed(_from) returns(uint result){
        IERC20(_token).transferFrom(_from, address(this), _amount);
        uint256 treasuryAmt = (_amount * treasuryfee) / 10000;
        uint256 finalAmt = _amount - treasuryAmt;
        BoldToken(_token).burn(address(this), finalAmt);
    }
