...
contract STinyNFT is BNFT, ISuperTinyNFT, ReentrancyGuard {
    using Address for address;

    uint256 private _totalBurned;

    address private _superTinyStats;

    constructor() BNFT(_msgSender(), "https://stg-api.tinyhero.com/v1/egg", "STinyNFT", "STinyNFT") {
    }

    function setSuperTinyStats(address newSuperTinyStats) external onlyAdmin {
        _superTinyStats = newSuperTinyStats;
    }
    ...
    function getInfos(uint256 tokenId) external override view returns (ISuperTinyStats.SuperTinyInfo memory superTinyStats) {
        if (_superTinyStats != address(0)) {
            (superTinyStats) = ISuperTinyStats(_superTinyStats).stats(tokenId);
        }
    }
