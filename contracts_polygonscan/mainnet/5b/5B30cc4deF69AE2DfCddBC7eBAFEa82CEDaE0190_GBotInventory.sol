    ...
    ERC2981ContractWideRoyalties
{
    ...
    function batchMintGBot( 
        ...
        for (uint256 i; i != length; ++i) {
            _setMetadata(nftIds[i], nftIds[i]);
            emit GBotMinted(to, nftIds[i], nftIds[i]);
        }
    }

    /**
     * Upgrades a GBot metadata
     */
    function upgradeGBot(uint256 nftId, uint256 newMetadata) public {
        _requireUpgrader(_msgSender());
        uint256 oldMetadata = getMetadata(nftId);
        require (oldMetadata != 0, "GBots Upgrade: Non-existant bot");
        _setMetadata(nftId, newMetadata);
        emit GBotUpgraded(nftId, oldMetadata, newMetadata);
    }
