...
// File: contracts/PricePrediction.sol
...
contract PricePrediction is Ownable, Pausable {
     ...
     */
    function genesisStartRound() external onlyOperator whenNotPaused { //onlyOperator removed
    ...
    function betBear() external payable whenNotPaused notContract {
        ...
        round.usersThisRound.push(msg.sender);
    ...
    /**
     * @dev Claim reward as part of batch drop/"airdrop"
     */
    function batchClaim(uint256 epoch, address user) internal {
        require(rounds[epoch].startBlock != 0, "Round has not started");
        require(block.number > rounds[epoch].endBlock, "Round has not ended");
        require(!ledger[epoch][user].claimed, "Rewards claimed");
        ...
        if (rounds[epoch].oracleCalled) {
            ...
            if(claimable(epoch, user)) {
                Round memory round = rounds[epoch];
                reward = ledger[epoch][user].amount.mul(round.rewardAmount).div(round.rewardBaseCalAmount);
            }
        }
        // Round invalid, refund bet amount
        else {
    ...
    function _distributeRewards(uint256 epoch) internal {
        Round storage round = rounds[epoch];
        for (uint i=0; i<round.usersThisRound.length; i++) {
            address user = round.usersThisRound[i];
            batchClaim(epoch, user);
        }
    }
