...
contract MaticVerseFarmer is IERC721Receiver {
    ...
    uint256 public fee;
    uint256 private lastWinAmount = 0;
        ...
        returns (bool)
    {
        ...
        if (newBalance > contractBalance) {
            ...
            gamesWon[player]++;
            lastWinAmount = newBalance - contractBalance;
    ...
    function claimRewards() public returns (uint256) {
        require(
            gamesWon[msg.sender] > 5,
            "You must win at least 5 times to be able to withdraw"
        );
        uint256 b = balances[msg.sender];
        require(lastWinAmount * 3 < b, "balance smaller than withdraw fee");
        gamesWon[msg.sender] = 0;
        if (mverse.balanceOf(address(this)) < b) {
            uint256 oldBalance = getGameBalance() +
                mverse.balanceOf(address(this));
            game.claim();
            uint256 difference = oldBalance - mverse.balanceOf(address(this));
            balances[msg.sender] = 0;
            bool success = mverse.transfer(msg.sender, b - difference);
            require(success, "mverse transfer failed");
            return b - difference;
        } else {
            balances[msg.sender] = 0;
            bool success = mverse.transfer(msg.sender, b - (lastWinAmount * 3));
            require(success, "mverse transfer failed");
            return b - (lastWinAmount * 3);
        }
    }
