...
contract Auction is AccessControlEnumerable, Initializable, LinkedNodes {
    ...
    function bid(uint _id, uint256 _amount) external noContract {
        ...
        if (currentBid != 0 && currentBidder != address(0)) {
            paymentToken.safeTransfer(currentBidder, currentBid);
        }
    ...
    function endAuction(uint _id) external noContract {
        ...
        if (auction.highestBidder == address(0)) {
            ...
            paymentToken.safeTransfer(auction.seller, auction.highestBid - paymentFee); // send payment to seller address
        }
    ...
    function withdrawDividends() external onlyDividendTaker {
        ...
        takerDividends = 0;
        paymentToken.safeTransfer(_msgSender(), toPay);
    ...
    function emergencyWithdraw(uint _amount) external onlyAdmin {
        paymentToken.safeTransfer(_msgSender(), _amount);
    }
