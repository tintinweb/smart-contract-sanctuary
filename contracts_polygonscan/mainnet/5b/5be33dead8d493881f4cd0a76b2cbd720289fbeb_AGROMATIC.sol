...
contract AGROMATIC is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public marketingWallet = address(0x26Bc2be9dB9ecD2f2c19A8758C807209E5b326E4);
    address public devWallet = address(0x7925bAECb51930Aa2E65D2fF20df198473DCc13a);
    address public charityWallet = address(0x7949d57f170585cd7BBb678F3a3f465BB82C7AF5);
    ...
    function _transfer(
        ...
        if (
            ...
            uint256 feeTokens = contractTokenBalance.mul(totalFees - liquidityFee).div(
                totalFees
            );
    ...
    function swapAndDistribute(uint256 tokens) private {
        ...
        swapAndSendToFee(newBalance.mul(walletfees).div(totalFees),walletfees);
        swapAndSendDividends(newBalance.mul(rewardsFee).div(totalFees));
        
    }

    function swapAndSendToFee(uint256 amount, uint16 fees) private {
        
        uint256 marketingShare = amount.mul(marketingFee).div(fees);
        uint256 devShare = amount.mul(devFee).div(fees);
        uint256 charityShare = amount.mul(charityFee).div(fees);
