...
interface IKommunitasFactory{
    ...
    function createProject(address, uint, uint, uint, uint, uint, uint[3] memory, uint[2] memory, uint, uint) external returns (address);
...
contract KommunitasProject{
     ...
     * @param _tokenProjectDecimals Token decimals of project (18 default)
     ...
     * @dev Get V2Staked
     */
    function getV2Staked() internal view returns(uint total){
        total = 0;
        uint fetch;
        for(uint i=0; i<3; i++){
            uint lock = KommunitasStakingV2(factory.stakingV2()).lockPeriod(i);
            (,,fetch) = KommunitasStakingV2(factory.stakingV2()).staked(lock);
            total = total.add(fetch);
        }
    }
    ...
    function getUserStakedInfo(StakingChoice _choice, address _target) internal view returns(uint userStaked, uint totalStaked){
        if(_choice == StakingChoice.V1){
            ...
            totalStaked = v2Staked;

            uint v2StakedNow = getV2Staked();
            if(v2Staked > v2StakedNow){
                totalStaked = v2StakedNow;
            }
        }else{
    ...
    function buyTokenByETH(address[] calldata _path) payable isBoosterProgress isNotPaused public {
        ...
        require(v2Staked > 0, "v2Staked haven't been set");
    ...
    /**
     * @dev Set beginning V2 total staked
     */
    function setV2Staked() public {
        require(block.timestamp >= calculation, "Calculation is not started yet");
        require(v2Staked == 0, "v2Staked had been set");
        
        v2Staked = getV2Staked();
    } 
...
contract KommunitasFactory is IKommunitasFactory{
    ...
    function createProject(
        ...
        project = address(new KommunitasProject());

        allProjects.push(project);
        
        KommunitasProject(allProjects[allProjects.length-1]).initialize(
            _payment,
