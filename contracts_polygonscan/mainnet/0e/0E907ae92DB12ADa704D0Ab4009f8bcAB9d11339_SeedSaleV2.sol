...
// Root file: contracts/TestSeedSaleV2.sol
...
/// @title Seed Sale
contract SeedSaleV2 is ReentrancyGuard, Ownable {
    ...
    uint256 public immutable PRECISION; //decimals of base token, for example 10**6, like USDC
    uint256 public immutable BUY_PRICE; //buy price in format 1 base token = amount of sell token, 1 USDC = 50 VINCU
    uint256 public  SOFTCAP; //soft cap
    uint256 public  HARDCAP; //hard cap
    uint256 public  MIN_BASE_TOKEN_PER_WALLET; //min base token per wallet
    uint256 public  MAX_BASE_TOKEN_PER_WALLET; //max base token per wallet
    ...
    uint256 public totalCollected; //total base collected
    ...
    event RefundBaseToken(address recipient, uint256 tokensToRefund);
    event ForceFailed();

    constructor(
        IERC20 _baseToken, //usdc
        uint256 _buyPrice,
        uint256 _softCap,
        uint256 _hardCap,
        uint256 _minBaseTokePerWallet,
        uint256 _maxBaseTokePerWallet,
        uint256 _startTime,
        uint256 _sellLengh,
        uint256 _precision //base token
    ) {
        //for vinci
        PRECISION = 10**_precision; //6 decimals for USDC
        BUY_PRICE = _buyPrice * 1 ether; // 1 VINCI = 0.02 USDC or 1 USDC = 50 VINCI
        SOFTCAP = _softCap * PRECISION; //250k USDC
        HARDCAP = _hardCap * PRECISION; //500k USDC
        MIN_BASE_TOKEN_PER_WALLET = _minBaseTokePerWallet * PRECISION; // 500 USDC or 25,000 VINCI
        MAX_BASE_TOKEN_PER_WALLET = _maxBaseTokePerWallet * PRECISION; // 1000 USDC or 50,000 VINCI
    ...
    function sell(uint256 _amount) external nonReentrant {
        ...
        require(_amount >= MIN_BASE_TOKEN_PER_WALLET, "SeedSale: insufficient purchase amount");
        require(_amount <= MAX_BASE_TOKEN_PER_WALLET, "SeedSale: reached purchase amount");
        require(participants[_msgSender()] < MAX_BASE_TOKEN_PER_WALLET, "SeedSale: the maximum amount of purchases has been reached");
        ...
        if (_amount > MAX_BASE_TOKEN_PER_WALLET - participants[_msgSender()]) {
            _amount = MAX_BASE_TOKEN_PER_WALLET - participants[_msgSender()];
        }
        ...
        //get base tokens from sender
        _deliverTokensFrom(baseToken, _msgSender(), address(this), _amount);
    ...
    function refund() external nonReentrant {
        ...
        uint256 tokensToRefund = participants[_msgSender()];

        participants[_msgSender()] = 0;

        _deliverTokens(baseToken, _msgSender(), tokensToRefund);

        emit RefundBaseToken(_msgSender(), tokensToRefund);
    }

    /// @notice total tokens for sale, send this amount to contract
    function totalTokensNeeded() external view returns (uint256) {
        return (HARDCAP * BUY_PRICE) / PRECISION;
    }


    /// @notice withdraw unsold base tokens to address
    /// @param _recipient recipient address
    /// @dev only if sale success or emergency failed

    function withdrawBaseToken(address _recipient) public onlyOwner {
        require(_recipient != address(0x0), "SeedSale: address is zero");
        require(status() == STATUS.SUCCESS || forceFailed, "SeedSale: sale is not ended with status success");
        _deliverTokens(baseToken, _recipient, baseToken.balanceOf(address(this)));
    }
    ...
    function _deliverTokensFrom(
        ...
        _token.safeTransferFrom(_sender, _recipient, _value);
    }
    ...
    function test_setMIN_BASE_TOKEN_PER_WALLET(uint256 _minTokens) external {
        MIN_BASE_TOKEN_PER_WALLET = _minTokens;
    }

    function test_setMAX_BASE_TOKEN_PER_WALLET(uint256 _maxokens) external {
        MAX_BASE_TOKEN_PER_WALLET = _maxokens;
    }
