...
contract DevFundLock is Ownable {
    ...
    // the Samoz token
    IERC20 public Samoz;
    // dev address to receive Samoz
    address public devaddr;
    // last withdraw block, use Samozswap online block as default
    ...
    constructor(IERC20 _Samoz, address _devaddr) public {
        require(address(_Samoz) != address(0) && _devaddr != address(0), "invalid address");
        Samoz = _Samoz;
        devaddr = _devaddr;
    }
    ...
    function withdraw() public {
        ...
        require(block.number >= unlockBlock, "Samoz locked");
        uint _amount = Samoz.balanceOf(address(this));
        require(_amount > 0, "zero Samoz amount");
        uint amountReal = _amount;
        if (_amount > WITHDRAW_HALF_THRESHOLD) amountReal = _amount.div(2);
        lastWithdrawBlock = block.number;
        Samoz.transfer(devaddr, amountReal);
    }
    ...
    function setSamoz(IERC20 _Samoz) public onlyOwner {
        Samoz = _Samoz;
    }
