    ...
    ReentrancyGuard
{
    ...
    event Minted(uint256 artworkId, uint256 tokenId, address minter);

    constructor()
        ERC721("polyblocks", "PB")
    ...
    address private developer = address(0);
    ...
    function mint(
        ...
        _safeMint(artist, id);
        if (artist != collector) {
            safeTransferFrom(artist, collector, id);
        }

        address payable[] memory royaltyAddresses = new address payable[](1);
        royaltyAddresses[0] = payable(artist);
        uint256[] memory royaltiesWithTwoDecimals = new uint256[](1);
        royaltiesWithTwoDecimals[0] = royalty;
        _setRoyaltiesOf(id, royaltyAddresses, royaltiesWithTwoDecimals);

        _tokenIdCounter.increment();

        emit Minted(artworkId, id, collector);
    }
    ...
    function setTokenURIs(uint256[] memory ids, string[] memory uris)
        external
        onlyOwner
    {
    ...
    function withdraw() external onlyOwner nonReentrant {
        Address.sendValue(payable(developer), address(this).balance);
    }
    ...
    modifier senderIsMarketplace() {
        require(
            msg.sender == marketplace,
            "Needs to be called from our marketplace"
        );
        _;
    }
}
