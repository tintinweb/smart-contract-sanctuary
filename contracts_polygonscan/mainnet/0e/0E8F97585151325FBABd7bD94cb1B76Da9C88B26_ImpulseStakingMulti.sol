...
contract ImpulseStakingMulti is AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function withdrawInWant(uint256 _pid, uint256 _wantAmt) public virtual nonReentrant poolRunning(_pid) hasPool(_pid) {
        updatePool(_pid);
        PoolInfo memory pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_msgSender()];
        ...
        uint256 sharesAmount = strategy.withdrawInWant(withdrawTotalAmount);
        user.shares -= sharesAmount;
        user.depositInWant -= _wantAmt;
