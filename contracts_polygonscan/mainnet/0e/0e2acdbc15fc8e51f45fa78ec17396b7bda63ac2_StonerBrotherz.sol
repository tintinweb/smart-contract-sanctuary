...
contract StonerBrotherz is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  function currentMintCost() public view returns (uint256) {
    ...
    if (_totalSupply <= 500)
        return 0.25 ether;
    if (_totalSupply > 500 && _totalSupply <= 1500)
        return 0.5 ether;
    if (_totalSupply > 1500 && _totalSupply <= 3000)
        return 1 ether;
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    if (msg.sender != owner()) {
        ...
        require(msg.value >= currentMintCost() * _mintAmount, "Insufficient funds");
    }
