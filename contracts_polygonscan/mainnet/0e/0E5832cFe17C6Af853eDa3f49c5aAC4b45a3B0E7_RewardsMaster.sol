...
contract RewardsMaster is Ownable {
    ...
    event RewardDivisorChange(uint256 divisor);

    function setDivisor(uint256 _new) external onlyOwner {
        require(_new > 0, "Divisor must be greater than zero");
        divisor = _new;
        emit RewardDivisorChange(divisor);
    }
        ...
        returns (uint256)
    {
        ...
        _rewardBal = SafeMath.sub(_rewardBal, spentAmount[_addr]);
        _rewardBal = SafeMath.div(_rewardBal, divisor);
        _rewardBal = SafeMath.add(_rewardBal, bonusAmount[_addr]);
        return _rewardBal;
    }

    function addStakingChild(address _addr) external onlyOwner {
