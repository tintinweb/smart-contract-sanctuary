...
contract Cryptogotchi_Genesis_Eggs_2 is ERC721Enumerable, Ownable {
    ...
    constructor() ERC721("Cryptogotchi Genesis Egg", "EGG2"){}
    ...
    function mintNFT(uint256 amount) public {
        ...
        require(TokenInterface(WethAddress).allowance(msg.sender, address(this)) >= price * amount, 'not approved'); // .03 wETH
        ...
        for (uint256 i = 0; i < amount; i++) {
            ...
            // 300 first in flight
            if (tokenId > 250 && tokenId <= 550) {
                _pet = 2;
                _dayPasses = 600;
            }
            // 500 death star
            if (tokenId > 550 && tokenId <= 1050) {
                _pet = 3;
                _dayPasses = 500;
            }
