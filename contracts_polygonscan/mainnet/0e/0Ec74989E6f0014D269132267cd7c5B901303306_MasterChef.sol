...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (nftaddr!=address(0x0)){
            ...
            polyshield.mint(burnaddr, shieldReward.div(20));
        }else{
            polyshield.mint(burnaddr, shieldReward.div(10));
        }
   ...
   function updateEmissionIfNeeded() public {
        ...
        if (block.number - lastBlockUpdate > emissionUpdateInterval) {
            ...
            if (shieldBalance > 0) {
                ...
                priceCents = usdc.balanceOf(usdcShieldLP).mul(1e14).div(shieldBalance);
            }
    ...
    // for front end
    function getShieldPriceCents() public view returns (uint256 spc){
         uint shieldBalance = polyshield.balanceOf(usdcShieldLP);
          if (shieldBalance > 0) {
            uint256 priceCents = usdc.balanceOf(usdcShieldLP).mul(1e14).div(shieldBalance);
            return priceCents;
          }
          return 0;
    }
