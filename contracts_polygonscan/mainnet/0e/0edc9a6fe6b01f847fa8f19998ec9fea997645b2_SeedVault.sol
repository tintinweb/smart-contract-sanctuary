...
contract SeedVault is Ownable, Pausable, ReEntrancyGuard {
    ...
    uint256 public withdrawFeePeriod = 6 hours; // 3 days
    ...
    function recentRewardBalance(address _user) external view returns (uint256) {
        ...
        return user.shares.mul(balanceOf()).div(totalShares) 
            - user.treeAtLastUserAction 	
            + user.shares.mul(IMasterChef(masterchef).pendingToken(seedPoolPid, address(this))).div(totalShares);	
    }	
