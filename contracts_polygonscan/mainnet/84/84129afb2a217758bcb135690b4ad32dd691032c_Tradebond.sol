...
contract Tradebond {
    ...
    ///// Functions called by buyers/sellers  //////
    ...
    function deposit(address payable _creator, uint _contractID) public payable {
        ...
        if (myAgreements[_creator][_contractID].buyer_funded && myAgreements[_creator][_contractID].seller_funded) {
            ...
            //myAgreements[_creator][_contractID].start_date = block.timestamp; //contract clock will begin once created
        }
    ...
    function proposeSettlement(address payable _creator, uint _contractID, uint settlement) public{
        ...
        require(myDisputes[_creator][_contractID].sellerdecision != AcceptReject.Accepted); //proposal cannot be made if seller accepter proposal
        require(myAgreements[_creator][_contractID].buyeracceptreject == AcceptReject.Rejected); //proposal cannot be made if seller accepter work 
    ...
    function respondSettlement(address payable _creator, uint _contractID, bool decision) public{
        ...
        require(myAgreements[_creator][_contractID].buyeracceptreject == AcceptReject.Rejected); //make sure the contract is funded by both sides
        require(myDisputes[_creator][_contractID].sellerdecision != AcceptReject.Accepted); //proposal cannot be made if seller accepter proposal
        require(myAgreements[_creator][_contractID].state == State.Disputed); //proposal cannot be made if seller accepter work 
