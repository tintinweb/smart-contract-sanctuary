...
contract PublicLaunchpad is ReentrancyGuard, Ownable, Pausable {
    ...
    uint256 public constant rate = 1e16; //10**18
    ...
    modifier checkBuyCondition(uint256 amount, address _user) {
        ...
        require(block.timestamp >= startTime, "Trade not started");
        require(amount > 0, "Please send more amount");
        require(isEnded == false, "IDO ended");
        require(
            offeringToken.balanceOf(owner()) > 0,
            "Insufficient buy amount"
        );
        _;
    }

    function buyWithUSD(uint256 _amountUSD)
        public
        whenNotPaused
        nonReentrant
        checkBuyCondition(_amountUSD, msg.sender)
    {
        uint256 tokensToBePurchased = _amountUSD.mul(10**18).div(rate);
        tokensToBePurchased = _verifyAmount(tokensToBePurchased);
        require(
            tokensToBePurchased > 0,
            "You've reached your limit of purchases"
        );
        uint256 totalUSD = tokensToBePurchased.mul(rate).div(10**18);
        USDC.safeTransferFrom(address(msg.sender), owner(), totalUSD);
        totalUSDRaised = totalUSDRaised.add(totalUSD);
        if (userTotalToken[msg.sender] == 0) {
            totalSaleParticipants = totalSaleParticipants.add(1);
        }
        userTotalToken[msg.sender] = userTotalToken[msg.sender].add(
            tokensToBePurchased
        );
        ...
        returns (uint256)
    {
        ...
        uint256 buyCap = personCap.mul(rate);
        if (buyCap > 0 && canBeBought.add(userBought[msg.sender]) > buyCap) {
            canBeBought = buyCap.sub(userBought[msg.sender]);
        }
        if (canBeBought > offeringToken.balanceOf(owner())) {
            canBeBought = offeringToken.balanceOf(owner());
        }
        return canBeBought;
    }
    ...
    function _checkout(address beneficiary, uint256 amount) internal {
        uint256 unlockedTokensEveryRound = amount.div(vestingRounds);
        for (uint256 round = 1; round <= vestingRounds; round++) {
            ...
            emit TokenVestingAdded(
                vestingId,
                beneficiary,
                unlockedTokensEveryRound
            );
        }
        totalTokenRaised = totalTokenRaised.add(amount);
        offeringToken.safeTransferFrom(owner(), address(this), amount);
        emit purchased(beneficiary, amount);
    }

    //<================= vestings functions ==============================================>
    function myVestings(address _address)
        external
        view
    ...
    function myNextRelease() external view returns (Vesting memory vesting) {
        ...
        for (uint256 i = 0; i < _vestings.length; i++) {
            if (
                !_vestings[i].released &&
                (nextRelease == 0 || _vestings[i].releaseTime < nextRelease) &&
                _vestings[i].releaseTime >= block.timestamp
            ) {
