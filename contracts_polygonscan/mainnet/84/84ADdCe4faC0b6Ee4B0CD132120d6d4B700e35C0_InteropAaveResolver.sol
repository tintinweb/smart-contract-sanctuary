...
contract InteropAaveResolver is Helpers {
    function checkAavePosition(
        ...
        p.isOk = isPositionSafe(userAddress, safeRatioPercentage);
        if (!p.isOk) return p;
        p = _checkRatio(position, safeRatioPercentage, isTarget);
        if (!p.isOk) return p;
    }
    ...
    public view returns(PositionData memory p) {
        p.isOk = isPositionSafe(liquidityAddress, safeLiquidityRatioPercentage);
        if (!p.isOk) return p;
...
abstract contract Helpers is DSMath, Variables {
    ...
    function checkSupplyToken(AaveData memory data) public view returns (
        uint256 totalSupply,
    ...
    function checkBorrowToken(AaveData memory data) public view returns (
        uint256 totalBorrow,
        bool isNotOk
    ) {
    ...
    */
    function _checkRatio(
        ...
        (positionData.totalSupply, totalMaxBorrow, totalMaxLiquidation, isSupplyNotOk) = 
            checkSupplyToken(data);
        (positionData.totalBorrow, isBorrowNotOk) =
            checkBorrowToken(data);
    ...
    */
    ...
    function isPositionSafe(address user, uint256 safeRatioPercentage) public view returns (bool isOk) {
        AaveInterface aave = AaveInterface(aaveLendingPoolAddressesProvider.getLendingPool());
        (,,,,,uint healthFactor) = aave.getUserAccountData(user);
...
contract Variables {
    ...
    */
     ...
     * @dev Aave Provider
     */
    AaveLendingPoolProviderInterface public aaveLendingPoolAddressesProvider = AaveLendingPoolProviderInterface(0xB53C1a33016B2DC2fF3653530bfF1848a515c8c5);

    /**
     * @dev Aave Data Provider
     */
    AaveDataProviderInterface public aaveProtocolDataProvider = AaveDataProviderInterface(0x057835Ad21a177dbdd3090bB1CAE03EaCF78Fc6d);
    ...
    address public wnativeToken = 0x2971AdFa57b20E5a416aE5a708A8655A9c74f723;
}
