...
contract MaticSpaceToken is BEP20("MaticSpace", "SPACE") {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        if (recipient == BURN_ADDRESS) {
            ...
            require(amount == sendAmount + burnAmount, "Space::transfer: Burn value invalid");
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    ISpaceReferral public spaceReferral;
    ...
    constructor(MaticSpaceToken _space, uint256 _startBlock) public {
        space = _space;
        startBlock = _startBlock;

        devAddress = msg.sender;
        feeAddress = msg.sender;
        SpacePerBlock = INITIAL_EMISSION_RATE;
    }
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 SpaceReward =
                multiplier.mul(SpacePerBlock).mul(pool.allocPoint).div(
                    totalAllocPoint
                );
    ...
    function updatePool(uint256 _pid) public {
        ...
        space.mint(devAddress, SpaceReward.div(10));
    ...
    function deposit(
        ...
        if (
            _amount > 0 &&
            address(spaceReferral) != address(0) &&
            _referrer != address(0) &&
            _referrer != msg.sender
        ) {
            spaceReferral.recordReferral(msg.sender, _referrer);
        }
        if (user.amount > 0) {
            uint256 pending =
                user.amount.mul(pool.accSpacePerShare).div(1e12).sub(
                    user.rewardDebt
                );
            if (pending > 0) {
        ...
        if (_amount > 0) {
            ...
            if (address(pool.lpToken) == address(space)) {
    ...
    function updateEmissionRate() public {
        ...
        require(
            SpacePerBlock > MINIMUM_EMISSION_RATE,
        ...
        uint256 newEmissionRate = SpacePerBlock;
        for (
        ...
        newEmissionRate = newEmissionRate < MINIMUM_EMISSION_RATE
            ...
            : newEmissionRate;
        if (newEmissionRate >= SpacePerBlock) {
            return;
        }

        massUpdatePools();
        lastReductionPeriodIndex = currentIndex;
        uint256 previousEmissionRate = SpacePerBlock;
        SpacePerBlock = newEmissionRate;
    ...
    // Update the Space referral contract address by the owner
    function setSpaceReferral(ISpaceReferral _spaceReferral) public onlyOwner {
        spaceReferral = _spaceReferral;
    }
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (
            address(spaceReferral) != address(0) && referralCommissionRate > 0
        ) {
            address referrer = spaceReferral.getReferrer(_user);
            uint256 commissionAmount =
                _pending.mul(referralCommissionRate).div(10000);

            if (referrer != address(0) && commissionAmount > 0) {
                space.mint(referrer, commissionAmount);
