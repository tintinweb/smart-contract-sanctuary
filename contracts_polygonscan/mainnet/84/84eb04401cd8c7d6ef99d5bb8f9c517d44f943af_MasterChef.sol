...
contract BoneToken is ERC20('Bone Token', 'BONE') {
...
// MasterChef is the master of BONE. He can make BONE and he is a fair guy.
//
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of Bone
    ...
    struct PoolInfo {
        ...
        uint256 accBonePerShare;   // Accumulated Bone per share, times 1e18. See below.
        uint16 depositFeeBP;      // Deposit fee in basis points
    }

    // The Bone Token!
    BoneToken public boneToken;
    uint256 constant MAX_BONE_SUPPLY = 5000000 ether;

    address public devAddress;
    address public feeAddress;

    // Bone tokens created per block. 1.8 BONE / Block
    uint256 public BONE_PER_BLOCK = 1.8 ether;
    ...
    function getMultiplier(uint256 _from, uint256 _to) public view returns (uint256) {
        if(boneToken.totalSupply() >= MAX_BONE_SUPPLY) return 0;

        return _to.sub(_from);
    }

    // View function to see pending Bone on frontend.
    function pendingBone(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 boneReward = multiplier.mul(BONE_PER_BLOCK).mul(pool.allocPoint).div(totalAllocPoint);
            accBonePerShare = accBonePerShare.add(boneReward.mul(1e18).div(lpSupply));
        }
        return user.amount.mul(accBonePerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 boneReward = multiplier.mul(BONE_PER_BLOCK).mul(pool.allocPoint).div(totalAllocPoint);
        boneToken.mint(devAddress, boneReward.div(10));
        boneToken.mint(address(this), boneReward);
        pool.accBonePerShare = pool.accBonePerShare.add(boneReward.mul(1e18).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accBonePerShare).div(1e18).sub(user.rewardDebt);
            if (pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accBonePerShare).div(1e18);
    ...
    function safeBoneTransfer(address _to, uint256 _amount) internal {
        uint256 boneBalance = boneToken.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > boneBalance) {
            transferSuccess = boneToken.transfer(_to, boneBalance);
        } else {
            transferSuccess = boneToken.transfer(_to, _amount);
        }
        require(transferSuccess, "safeBoneTransfer: Transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _bonePerBlock) external onlyOwner {
        massUpdatePools();
        BONE_PER_BLOCK = _bonePerBlock;
        emit UpdateEmissionRate(msg.sender, _bonePerBlock);
    }
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        ...
        if (referrer != address(0) && referrer != _user && referralCommissionRate > 0) {
            ...
            boneToken.mint(referrer, commissionAmount);
