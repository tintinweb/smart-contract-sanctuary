...
contract PoolDowserV1 is Ownable {
    ...
    function getNameStaked(IPoolV1 pool) public view returns (string memory name) {
    ...
    function getNamePending(IPoolV1 pool) public view returns (string memory name) {
        IERC20Metadata stakeToken = IERC20Metadata(address(pool.STAKE_TOKEN()));
        IERC20Metadata earnedToken = IERC20Metadata(address(pool.REWARD_TOKEN()));
        bytes memory stakeSymbol = longSymbol(stakeToken);
        bytes memory earnedSymbol = longSymbol(earnedToken);
    ...
    function getSymbolStaked(IPoolV1 pool) public view returns (string memory name) {
        IERC20Metadata stakeToken = IERC20Metadata(address(pool.STAKE_TOKEN()));
        IERC20Metadata earnedToken = IERC20Metadata(address(pool.REWARD_TOKEN()));
    ...
    function getSymbolPending(IPoolV1 pool) public view returns (string memory name) {
        IERC20Metadata stakeToken = IERC20Metadata(address(pool.STAKE_TOKEN()));
        IERC20Metadata earnedToken = IERC20Metadata(address(pool.REWARD_TOKEN()));
        bytes memory stakeSymbol = shortSymbol(stakeToken);
        bytes memory earnedSymbol = shortSymbol(earnedToken);
    ...
    function addPool(IPoolV1 pool) external onlyOwner {
