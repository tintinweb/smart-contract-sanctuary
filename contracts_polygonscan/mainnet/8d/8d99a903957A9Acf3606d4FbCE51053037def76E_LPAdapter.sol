...
contract LPAdapter {
    ...
    function fetchLPValue(address _lpToken) public view returns(LPData memory) {
        ...
        string memory token0Name = FullIERC20(token1).name();
        string memory token1Name = FullIERC20(token1).name();
        ...
        LPData memory data = LPData(reserve0Value, token0Name, token0Decimals, reserve1Value, token1Name, token1Decimals);
