...
contract NFT is ERC721 {
    ...
    mapping(uint256 => uint64) public redeemed;
    ...
    0x4f27769c38169E402FFbB38B19F2606Be60FDF97,
    0x97E207378ac49fa2aA053d932d4D78236040890b,
    address(0),
    address(0),
    address(0),
    address(0),
    ...
    // Pre-check minting for a batch
    // This version doesn't write anything to the blockchain, just checks that
    // the settings are valid and it hasn't already been minted.
    // Because of this, it is set as view
    ...
    function mint(
        ...
        require((batch > 0), 'Invalid batch ID.');
        require((time > 0), 'Invalid time.');
        require((limit == 1), 'Batch size is limited to 1.');
        require((bytes(name).length > 0), 'Name not provided.');
        require((bytes(created).length > 0), 'Creation time not provided.');

        // Redundant but included to avert accidents
        require(isManager(), 'You do not have permission to create token batches.');
        tokenId = _generateId(batch, limit, 1);

        require(!_exists(tokenId), 'Batch already exists.');
    }

    // Mint an NFT with content attached
    ...
    function mint(
        ...
        uint256 tokenId;
        require((limit == 1 && sequence == 1), 'Batch size is limited to 1.');

        tokenId = generateId(batch, limit, sequence);

        require(!_exists(tokenId), 'Token already exists.');
        require(isManager(), 'You do not have permission to mint tokens.');
    ...
    // This version is simplified from the full contract
    function canMint(uint256 tokenId) public view returns (bool) {
        // To mint a token
        // 1. The token must not exist
        // 2. The minting account must be a manager
    ...
    // Redemption can optionally include updated content (e.g. adding a hash
    // for an updated image) and a 64-bit redemption number (for a given event
    // this could represent the order in which NFTs were redeemed).

    // If the content is not supplied, existing content is not updated.
    // If the redemption number is not supplied, it defaults to 1 as a true/false flag.
    function can_redeem(uint256 tokenId) public view {
        ...
        require(redeemed[tokenId] == 0, 'Token has already been redeemed.');
    }

    // Redeem without content or number
    function redeem(uint256 tokenId) public {
        return redeem(tokenId, '', 1);
    }

    // Redeem with content update
    function redeem(uint256 tokenId, string memory content) public {
        return redeem(tokenId, content, 1);
    }

    // Redeem with content update and number
    function redeem(uint256 tokenId, string memory content, uint64 number) public {
        token_info memory info;

        can_redeem(tokenId);

        if (bytes(content).length >= 0) {
            info = contents[tokenId];
            info.content = content;
            contents[tokenId] = info;
        }

        redeemed[tokenId] = number;
    }

    // This returns the redemption number if redeemed, or null otherwise
    function isRedeemed(uint256 tokenId) public view returns (uint64){
    ...
    // Make sure we haven't overflowed the 96-bit assigned range
    ...
    // For this version of the contract, this is always 1
    function batchLimitFromId(uint32 batch) public pure returns (uint32) {
        require((batch != 0), 'Invalid batch ID.');
        return 1;
    }

    // Get the 32-bit sequence number from a 96-bit tokenId
    // This will always be 1 but we keep the original code
    ...
    function batchTokenOf(uint256 tokenId) public pure returns (uint256) {
        return generateId(batchId(tokenId), 1, 0);
    }

    // Lookup and return the batch tokenId given just the batchId
    // This version always uses a limit of 1
    function batchTokenFromId(uint32 _batchId) public pure returns (uint256) {
        return generateId(_batchId, 1, 0);
    }
