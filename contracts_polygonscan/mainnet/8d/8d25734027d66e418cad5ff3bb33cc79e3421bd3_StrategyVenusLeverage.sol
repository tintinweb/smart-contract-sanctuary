...
abstract contract StrategyBase is IStrategy, ReentrancyGuard, Initializable {
    ...
    function initialize(
        ...
        unirouter = IUniswapV2Router(0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506);
        firebirdRouter = IFirebirdRouter(0x6466CDC2615f111dc6Dbd0Ef6fDD209F5Ff8d184);
        timeToReleaseCompound = 30 minutes; // 0 to disable
    ...
    function setPerformanceFee(uint256 _performanceFee) external onlyGovernance {
        require(_performanceFee < 10000, "too high");
    ...
    function executeTransaction(
        ...
        require(success, string(abi.encodePacked(getName(), ":execution reverted")));
...
interface IVenusComptroller {
    ...
    //iron function
    function claimReward(address holder, address[] calldata vTokens) external;
...
contract StrategyVenusLeverage is StrategyBase {
    ...
    function initialize(
        ...
        venusComptroller = address(0xF20fcd005AFDd3AD48C85d0222210fe168DDd10c);
    ...
    function balanceOfPool() public view override returns (uint256) {
        return
            IVToken(vToken).balanceOf(address(this)).mul(IVToken(vToken).exchangeRateStored()).div(1e18).mul(uint256(1e18).sub(_venusRedemptionFee())).div(1e18).sub(
                IVToken(vToken).borrowBalanceStored(address(this))
            );
    }
    ...
    function borrowLimit() public view returns (uint256) {
        ...
        return IVToken(vToken).borrowBalanceStored(address(this)).mul(1e18).div(balanceOfUnderlying.mul(_collateralFactor()).div(1e18));
    }
