...
contract GetMakerts is Ownable {
    ...
    address public ORACLE=0xb33e0d522208B3f6421B9562cA38739087Ac0Bd5;
    ...
    function getUnderlyingTokenAddress(address _pToken) public view returns (address){
        if (_pToken == 0x2840AF6f287d329237c8addc5500C349E041C5BB) {
            return address(0);
        }else{
    ...
    function getAllMarketDetail(address _pToken, address _account) public view returns (PoolInfo memory){
        PoolInfo memory _data;
        (uint256 status,uint256 cTokenBalance,uint256 borrowBalance,uint256 exchangeRateMantissa)=getSupplyAndBorrowBalance(_pToken, _account);
        _data = PoolInfo({
                pTokenAddress: _pToken,
                underlyingAddress: getUnderlyingTokenAddress(_pToken),
                symbol: getSymbol(_pToken),
                ...
                supplyAndBorrowBalance: BorrowBalance({status:status,cTokenBalance:cTokenBalance,borrowBalance:borrowBalance,exchangeRateMantissa:exchangeRateMantissa}),
