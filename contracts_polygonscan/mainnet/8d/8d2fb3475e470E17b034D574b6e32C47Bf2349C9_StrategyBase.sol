...
abstract contract StrategyFarmBase is BaseStrategyMasterChef {
    ...
    address public constant ironRouter = 0x4C96C61AF64e78F848fB2Ec965C4da08430dF348;
        ...
        )
    {
        ...
        metaPool = IIronSwapLP(_want).swap();
        metaPoolSize = IIronSwap(metaPool).getNumberOfTokens();
        basePoolLP = IIronSwap(metaPool).getToken(0);
    ...
    function set_basePoolSize(uint16 newPoolSize) external onlyOwner() {
        basePoolSize = newPoolSize;
    }

    function set_metaPoolSize(uint16 newPoolSize) external onlyOwner() {
        metaPoolSize = newPoolSize;
    }
    ...
    function harvest() public override {
        ...
        if (_iceBalance > harvestCutoff){
            ...
            if (usdcBal>0){
                IERC20(usdc).safeApprove(ironRouter, 0);
                IERC20(usdc).safeApprove(ironRouter, usdcBal);
...
contract StrategyBase is StrategyFarmBase {
    ...
    address public IronI3USD_LP = 0x985D40feDAA3208DAbaCDFDCA00CbeAAc9543949;
