...
contract HM is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function getHungMan(uint256 tokenId) public view returns (string memory) {
        ...
        uint256 rand = random(string(abi.encodePacked("HungMan", toString(tokenId), toString(uint256(uint160(address(msg.sender)))), block.timestamp)));
        uint256 _index = rand % 1001;
