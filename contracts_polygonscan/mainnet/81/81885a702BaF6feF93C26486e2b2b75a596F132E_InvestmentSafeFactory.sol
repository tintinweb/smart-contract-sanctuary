...
contract InvestmentSafeFactory is Ownable, IInvestmentSafeFactory {
    ...
    address public override childAmunWrappedWeth;
    IUniswapRouter public router;

    IChildAmunWrappedWeth public ChildAmunWrappedWeth;
    uint256 public override immutable EXIT_GAS = 900460;
    uint256 public override immutable JOIN_GAS = 1327778;
    uint256 public override immutable GAS_CREATE_SAFE = 2829803; //this is the gas used on for safe create 
    ...
    constructor(address _singleTokenJoin, address _singleTokenExit, address _weth, address _childAmunWrappedWeth, address _router) Ownable()
    {
        singleTokenJoin = _singleTokenJoin;
        singleTokenExit = _singleTokenExit;
        weth = _weth;
        childAmunWrappedWeth = _childAmunWrappedWeth;
...
import "./IChildAmunWrappedWeth.sol";
...
interface IInvestmentSafeFactory {
    ...
    function childAmunWrappedWeth() external view returns (address);
...
contract InvestmentSafe is EIP712, IInvestmentSafe {
    ...
    mapping(address => bool) public isSecondExitStep;
    ...
    function delegatedExit(
        ...
        require(!isSecondExitStep[msg.sender], "ExitPermit: Wrong Step");
        isSecondExitStep[msg.sender] = true;
        ...
        _maxApprove(
            weth,
            investmentSafeFactory.childAmunWrappedWeth(),
            targetAmount
        );

        IChildAmunWrappedWeth(investmentSafeFactory.childAmunWrappedWeth()).getIOU(
            targetAmount
        );
    }

    /// @dev Delegated exit and bridging back of target token
    function delegatedExit2(
        DelegateExitData calldata exitData
    ) public {
        require(
            block.timestamp <= exitData.deadline,
        ...
        isSecondExitStep[msg.sender] = false;

        _checkExitSignature(exitData);
        nonces[msg.sender] = nonces[msg.sender].add(1);

        uint256 targetAmount = IERC20(investmentSafeFactory.childAmunWrappedWeth())
            .balanceOf(address(this));

        IChildAmunWrappedWeth(investmentSafeFactory.childAmunWrappedWeth()).withdrawTo(
            targetAmount,
            owner
        );
        emit DelegatedExit(
