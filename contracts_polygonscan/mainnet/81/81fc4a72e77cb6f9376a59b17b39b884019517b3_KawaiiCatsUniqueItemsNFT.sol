 ...
 *  Contract: Unique Items NFT 
 *  
 *  Description: Allows the creation and management of unique outfits for cat NFTs.
 * 
 */
...
contract KawaiiCatsUniqueItemsNFT is Ownable, ERC721, ERC721Enumerable {
    ...
    uint256 public CUSTOM_PRICE = 150000000000000000000; //150 MATIC
    uint256 public CUSTOM_ANIM_PRICE = 300000000000000000000; //300 MATIC
    uint256 public CUSTOM_PURR_PRICE = 7000000000000000000000; //7000 PURR
    ...
    function mintItem(string memory _name, string memory _description, string memory _setName, bool _animated) public onlyOwner {
        ...
        uint8 [7] memory stats = [0,0,0,0,0,0,0];
        _safeMint(msg.sender, mintIndex);
        uniqueItemArray.push(uniqueItem(mintIndex,_name,_description,_setName, _animated, "", "", stats));
    ...
    function setLTG(uint16 _itemId, string memory _lt, string memory _lg) public {
        address owner = ownerOf(_itemId);
        require(_msgSender() == owner, "ERC721: caller is not the owner");
        require(messyStringLength(_lt) < 20, "Not a valid latitude");
        require(messyStringLength(_lg) < 20, "Not a valid longitude");
    ...
    /**
     * @dev Modify costume stats (for future stats bonuses based on costumes)
     */
    function modifyCostumeStats(uint16 _itemId, uint8[7] memory _stats) public onlyOwner{
         uniqueItemArray[_itemId].stats = _stats;
    } 
