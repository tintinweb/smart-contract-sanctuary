...
contract HawkFinance is BEP20('Hawk Token', 'HAWK') {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the Hawk Owner (MasterChef).
    ...
    */
        ...
        returns (uint256)
    {
        ...
        while (upper > lower) {
            uint32 center = upper - (upper - lower) / 2; // ceil, to avoiding overflow
...
|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||- JOIN POLYHAWK AND BECOME ONE WITH THE FLOCK -||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
...
import "./HawkFinance.sol";

// MasterChef is the leader of the HAWK flock. He can make HAWK and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once HAWK is sufficiently
// distributed and the community can show to govern itself.
//
// Have fun reading this code. this is not bug-free because they are not bug, they are unspected features. Hawk bless you.
contract MasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of little HAWKs
    ...
    function pendingHAWK(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 mintQuota = maxSupply.sub(hawk.totalSupply());
            hawkReward = Math.min(mintQuota, hawkReward);
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
       if (lpSupply == 0 || pool.allocPoint == 0 || hawkPerBlock == 0) {
	...
	uint256 mintQuota = maxSupply.sub(hawk.totalSupply());
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 hawkReward = multiplier.mul(hawkPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
uint256 devReward = hawkReward.div(10);
            
            if(hawkReward.add(devReward) >= mintQuota){
                hawkPerBlock = 0;
                hawkReward = Math.min(mintQuota, hawkReward);
                devReward = mintQuota.sub(hawkReward); //0 if hawkReward >= mintQuota
            }
        hawk.mint(devaddr, devReward);
    ...
    function updateEmissionRate(uint256 _hawkPerBlock) public onlyOwner {
        massUpdatePools();
        hawkPerBlock = _hawkPerBlock;
    }
}
