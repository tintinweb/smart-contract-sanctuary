...
contract CoreInternals is Helpers {
    ...
    */
    function collectFees(uint96 NFTID_, bool isLiquidate_)
        internal
    ...
    */
    function _liquidate(
        ...
        (uint amount0Fee_, uint amount1Fee_) = collectFees(NFTID_, false);
...
contract UserModule is CoreInternals {
    ...
    */
    function collectFees(uint96 NFTID_)
        ...
        returns (uint256 amount0_, uint256 amount1_)
    {
        (amount0_, amount1_) = collectFees(NFTID_, false);
...
contract UniswapHelpers is Variables, Events {
    ...
    function getPoolAddress(
        ...
        address token1_,
        uint24 fee_
    ) public view returns (address poolAddr_) {
...
contract OraclesHelpers is UniswapHelpers {
    ...
    {
        if(token_ == WETH_ADDRESS) {
            token_ = Denominations.ETH;
        }

        if(token_ == WBTC_ADDRESS) {
            token_ = Denominations.BTC;
        }
        (, int256 price_, , , ) = registry.latestRoundData(
            token_,
            Denominations.USD
        );
        uint8 decimals_ = registry.decimals(token_, Denominations.USD);
        return (uint256(price_) * 10**(18 - decimals_), decimals_);
    }
     ...
     * @return decimals_ array of token decimals
     */
    function getPricesInUsd(address[] memory tokens_)
        internal
        view
        returns (uint256[] memory amounts_, uint8[] memory decimals_)
    {
        amounts_ = new uint256[](tokens_.length);
        decimals_ = new uint8[](tokens_.length);
        for (uint256 i = 0; i < tokens_.length; i++) {
            (amounts_[i], decimals_[i]) = getPriceInUsd(tokens_[i]);
        }
    }

    /**
     * returns chainlink oracle price of one token w.r.t other token
     * @param base_ Token for which the price needs to calculate
     * @param quote_ Token in which price is needed
     */
    function getOraclePrice(address base_, address quote_)
        public
        view
        returns (uint256)
    {
        // TODO: verify the price in proper decimals
        (, int256 price_, , , ) = registry.latestRoundData(base_, quote_);
        uint8 decimals_ = registry.decimals(base_, quote_);
        return uint256(price_) * 10**(18 - decimals_);
    }
...
contract CoreReadHelpers is OraclesHelpers {
    ...
    /**
     * returns liquidity of the NFT in amount0 & amount1 (just liquidity with fee).
     * @param poolAddr_ address of Uniswap pool.
     * @param tickLower_ Lower tick of the NFT.
     * @param tickUpper_ Upper tick of the NFT.
     * @param liquidity_ Liquidity of the NFT.
    ...
    function getNetNFTLiquidity(
        ...
        int24 tickUpper_,
        uint128 liquidity_
    ) internal view returns (uint256 amount0Total_, uint256 amount1Total_) {
        IUniswapV3Pool pool_ = IUniswapV3Pool(poolAddr_);
        ...
        )
    {
        ...
        if (amount0_ > borrowBals_[0]) {
            amount0Normal_ = amount0_ - borrowBals_[0];
        } else {
            borrow0Normal_ = borrowBals_[0] - amount0_;
        }
        if (amount1_ > borrowBals_[1]) {
            amount1Normal_ = amount1_ - borrowBals_[1];
        } else {
            borrow1Normal_ = borrowBals_[1] - amount1_;
        }
        (
            uint256[] memory tokenPrices_,
            uint8[] memory tokenDecimals_
        ) = getPricesInUsd(_poolMarkets[poolAddr_]);
...
interface IUniswapV3Staker is IERC721Receiver, IMulticall {
    ...
    /// @param rewardIUniswapV3Staker The amount of reward tokens to be distributed
    event IncentiveCreated(
        ...
        uint256 endTime,
        address refundee,
        uint256 rewardIUniswapV3Staker
    );
