...
contract IvoryUsdcStrategy is BaseStrategyMasterChef {
    ...
    address private constant IVORY = 0x88a3aCAc5C48F93121d4d7771A068A1FCDE078BC;
    address private constant IVORY_USDC_LP = 0x10995233Ef7b3abd1a2706a86FFeA456ebae8796;

    address private constant CHEF = 0x43d78BA76dc2C5dF8C1cF3423C5b8Db7f476f33a;
    address private constant QUICKSWAP_ROUTER = 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff;
    address private constant FIREBIRD_ROUTER = 0xb31D1B1eA48cE4Bf10ed697d44B747287E785Ad4;
    address private constant FUND = 0x01fE07ce760DA7a025e4Cf3f950aE236F8e62120;

    // Uniswap swap paths
    address[] private usdc_matic_path;
    ...
    {
        usdc_matic_path = [USDC, WMATIC];

        IERC20(want).safeApprove(rewards, uint256(-1));

        IERC20(IVORY).safeApprove(FIREBIRD_ROUTER, uint256(-1));
        IERC20(USDC).safeApprove(FIREBIRD_ROUTER, uint256(-1));
    ...
    function harvest() public override onlyHumanOrWhitelisted nonReentrant {
        //Transfer USDC that may already be in the contract to the fee dist fund
        ...
        //Swap 1/2 of Ivory for USDC
        uint256 remainingHarvested = IERC20(harvestedToken).balanceOf(address(this));
        if (remainingHarvested > 0) {
            swapIvoryToUsdc(remainingHarvested.div(2));
        }

        //Add liquidity
        uint256 aBalance = IERC20(IVORY).balanceOf(address(this));
        uint256 bBalance = IERC20(USDC).balanceOf(address(this));
        if (aBalance > 0 && bBalance > 0) {
            IFireBirdRouter(FIREBIRD_ROUTER).addLiquidity(
                IVORY_USDC_LP,
                IVORY, USDC,
                aBalance, bBalance,
                0, 0,
    ...
    function swapIvoryToUsdc(uint256 ivory_in) internal {
        address[] memory path = new address[](1);
        path[0] = IVORY_USDC_LP;

        uint8[] memory dexIds = new uint8[](1);
        dexIds[0] = 0;

        IFireBirdRouter(FIREBIRD_ROUTER).swapExactTokensForTokens(IVORY, USDC, ivory_in, 1, path, dexIds, address(this), now + 60);
    }
}