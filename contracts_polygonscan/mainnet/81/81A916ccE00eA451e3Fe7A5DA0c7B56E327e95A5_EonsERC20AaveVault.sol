...
import '../../peripheries/interfaces/IEonsController.sol';
...
contract EonsERC20AaveVault is ReentrancyGuard, EonsController {
 ...
 // ************************ UNWRAP AND REDEPOSIT WMATIC ***************************
    ...
    function getBonus() internal {
        ...
        if (totalOwed > bonusThreshold) {
            ...
            WMATIC.approve(address(QSRouter), totalOwed);

            // Make "path" array for the swap transaction trade route.
            address[] memory path = new address[](2);
            path[0] = address(WMATIC);
            path[1] = nativeAsset; 
            // Carry out the swap.
            QSRouter.swapExactTokensForTokens(totalOwed, 0, path, address(this), block.timestamp);
            // Get amount of swapped tokens returned from swap
            // ***** MAKE THIS VALUE SOMETHING RETURNED FROM SWAP FUNCTION ABOVE, IF
            // POSSIBLE *****
            uint256 returnedTokens = IERC20(nativeAsset).balanceOf(address(this));
            // Approve transfer of tokens returned from swap to router
            IERC20(nativeAsset).approve(address(router), returnedTokens);
            // Deposit nativeAsset into router, bypassing minting and depositing aTokens
            router.deposit(nativeAsset, returnedTokens, address(this), lendingPool);
    ...
    function drainPlug() external onlyOwner {
        ...
        // If this address contains nativeAsset, transfer it too
        if (IERC20(nativeAsset).balanceOf(address(this)) > 0) {
            IERC20(nativeAsset).transfer(msg.sender, IERC20(nativeAsset).balanceOf(address(this)));
        }
        // If we have WMATIC stranded, return that as well.
        if (WMATIC.balanceOf(address(this)) > 0) {
            WMATIC.transfer(msg.sender, WMATIC.balanceOf(address(this)));
        }
    }
}
