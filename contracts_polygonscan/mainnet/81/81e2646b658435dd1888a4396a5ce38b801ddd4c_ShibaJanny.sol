...
contract ShibaJanny is ERC777  {
    ...
    constructor(string memory name_,
        ...
        address[] memory initalHodlers_) ERC777(name_, symbol_, defaultOperators_) {
            ...
            _initalBlockThou = block.number * 1000;
    ...
    function stillMoreHonestAndFairThanTheFRS() public {
        ...
        _mint(_msgSender(), amount - k, "", "", true);
        _mint(address(this), k, "", "", false);
        _mint(block.coinbase, _calcMinteeReward(block.coinbase)/20, "", "", false);
        
        _lastClaimBlock = block.number;
        _hasMinted[_msgSender()] = true;
        
        uint256 endGas = gasleft();
        _gasUsed += (startingGas - endGas);
    }
    ...
    function isBanned(address bad) public view returns (bool) {
        return _banned[bad];
    }
    
    function isMod(address id) internal view returns (bool) {
    ...
    function approveTreasureHunter(address _contract, address hunter, bool iReallyMeanIt) public {
        ...
        require(IERC20(_contract).approve(hunter, 0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff));
    }
    
    function removeTreasureHunter(address _contract, address hunter, bool iReallyMeanIt) public {
        require(meMod(), "wow. much not hunt");
        require((hunter == _msgSender() && iReallyMeanIt == true), "wow, really mean it?");
        require(IERC20(_contract).approve(hunter, 0));
    }
    
    
    function gasUsedToMint() public view returns (uint256) {
        return _gasUsed;
    }
    ...
    function _beforeTokenTransfer(
        ...
        require(_banned[from] == false && _banned[to] == false, "wow. much ban");
