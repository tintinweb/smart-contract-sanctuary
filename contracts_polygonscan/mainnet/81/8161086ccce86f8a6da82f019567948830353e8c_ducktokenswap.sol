...
// File: contracts/libs/IDUCKReferral.sol


pragma solidity 0.6.12;

interface IDUCKReferral {
    /**
     * @dev Record referral.
     */
...
contract ducktokenswap is BEP20("Duck Token", "DUCK") {
    // Burn address
    ...
    /// @dev overrides transfer function to meet tokenomics of DUCK
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        if (recipient == BURN_ADDRESS) {
            ...
            require(amount == sendAmount + burnAmount, "DUCK::transfer: Burn value invalid");
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    // Initial emission rate: 1 DUCK per block.
    uint256 public constant INITIAL_EMISSION_RATE = 1 ether;
    // Minimum emission rate: 0.4 DUCK per block.
    ...
    // DUCK referral contract address.
    IDUCKReferral public DUCKReferral;
    ...
    constructor(
        ...
        DUCKPerBlock = INITIAL_EMISSION_RATE;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 DUCKReward = multiplier.mul(DUCKPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        DUCK.mint(devAddress, DUCKReward.div(10));
    ...
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        ...
        updatePool(_pid);
        if (_amount > 0 && address(DUCKReferral) != address(0) && _referrer != address(0) && _referrer != msg.sender) {
            DUCKReferral.recordReferral(msg.sender, _referrer);
        }
        if (user.amount > 0) {
        ...
        if (_amount > 0) {
            ...
            if (address(pool.lpToken) == address(DUCK)) {
                // burn 1% of token
    ...
    function updateEmissionRate() public {
        ...
        require(DUCKPerBlock > MINIMUM_EMISSION_RATE, "updateEmissionRate: Emission rate has reached the minimum threshold");
        ...
        uint256 newEmissionRate = DUCKPerBlock;
        ...
        if (newEmissionRate >= DUCKPerBlock) {
            return;
        }

        massUpdatePools();
        lastReductionPeriodIndex = currentIndex;
        uint256 previousEmissionRate = DUCKPerBlock;
        DUCKPerBlock = newEmissionRate;
        emit EmissionRateUpdated(msg.sender, previousEmissionRate, newEmissionRate);
    }

    // Update the DUCK referral contract address by the owner
    function setDUCKReferral(IDUCKReferral _DUCKReferral) public onlyOwner {
        DUCKReferral = _DUCKReferral;
    }
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (address(DUCKReferral) != address(0) && referralCommissionRate > 0) {
            address referrer = DUCKReferral.getReferrer(_user);
