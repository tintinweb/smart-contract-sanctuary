...
function _getProtectionPayload(bytes memory _data, bytes memory _offChainData)
    pure
    returns (ProtectionPayload memory)
{
...
contract ProtectionAction is FlashLoanReceiverBase, IAction, Ownable {
    ...
    event SuccessfulProtection(
        uint256 colProtectedInEth,
        uint256 remainingDebtInEth,
        address onBehalfOf
    );
    ...
    function exec(bytes memory _data, bytes memory _offChainData)
        external
        override
        onlyAaveServices
    {
        ProtectionPayload memory protectionPayload =
            _getProtectionPayload(_data, _offChainData);
