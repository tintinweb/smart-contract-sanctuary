...
import './libs/IBorrower.sol';

contract SafeWorld is Context, IERC20, Ownable {
    ...
    uint256 public override totalSupply = 3300000 ether;
    uint256 private _rTotal = (MAX - (MAX % totalSupply));
    uint256 public totalFees;

    string private _name = 'SafeWorld';
    string private _symbol = 'SAFEWORLD';
    uint8 private _decimals = 18;

    uint256 public taxFee = 560;
    uint256 private _previousTaxFee = taxFee;
    uint256 public constant MAX_TAX_FEE = 1000;

    uint256 public burnFee = 160;
    ...
    uint256 public maxTxAmount = 33000 ether;
    uint256 public constant MIN_TX_AMOUNT_HARD_CAP = 1 ether;
    ...
    bool public flashMintEnabled = true;

    event UpdateSwapAndLiquifyEnabled(bool enabled);
    event UpdateMinAmountToLiquify(address indexed operator, uint256 previousAmount, uint256 newAmount);
    event SwapAndLiquify(uint256 tokensSwapped, uint256 ethReceived, uint256 tokensIntoLiqudity);
    event SwapTokensForETH(uint256 amountIn, address[] path);
    event UpdateTaxFee(uint256 newRate);
    event UpdateBurnFee(uint256 newRate);
    event UpdateLiquidityFee(uint256 newRate);
    event UpdateMaxTxAmount(uint256 newAmount);
    event UpdateSwapRouter(address indexed operator, address indexed router, address indexed pair);
    event UpdateFlashMintEnabled(bool enabled);
    ...
    function setSwapRouter(address _router) public onlyOwner {
        ...
        require(swapPair != address(0), 'setSwapRouter: Invalid pair address.');
        emit UpdateSwapRouter(msg.sender, address(swapRouter), swapPair);
    }
    ...
    function swapAndLiquify() private lockTheSwap {
        ...
        if (contractTokenBalance >= minAmountToLiquify) {
            ...
            if (balanceAfterLiquidity > 0) {
                payable(owner()).transfer(balanceAfterLiquidity);
            }
    ...
    function _tokenTransfer(
        ...
        uint256 txAmount = amount.sub(burnAmount);

        if (burnAmount > 0) {
            if (_isExcluded[sender]) {
                _transferBothExcluded(sender, BURN_ADDRESS, burnAmount);
            } else {
                _transferToExcluded(sender, BURN_ADDRESS, txAmount);
            }
        }

        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(sender, recipient, txAmount);
        } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
            _transferToExcluded(sender, recipient, txAmount);
        } else if (_isExcluded[sender] && _isExcluded[recipient]) {
            _transferBothExcluded(sender, recipient, txAmount);
        } else {
    ...
    function setTaxFeePercent(uint256 _taxFee) external onlyOwner() {
        require(_taxFee <= MAX_TAX_FEE, 'taxFee is too high');
        taxFee = _taxFee;
    ...
    function setBurnFeePercent(uint256 _burnFee) external onlyOwner() {
        require(_burnFee <= MAX_BURN_FEE, 'burnFee is too high');
        burnFee = _burnFee;
    ...
    function setLiquidityFeePercent(uint256 _liquidityFee) external onlyOwner() {
        require(_liquidityFee <= MAX_LIQUIDITY_FEE, 'liquidityFee is too high');
    ...
    function setMaxTxAmount(uint256 _maxTxAmount) external onlyOwner() {
        require(_maxTxAmount >= MIN_TX_AMOUNT_HARD_CAP, 'maxTxAmount have to be greater than 1 SafeWorld');
        maxTxAmount = _maxTxAmount;

        emit UpdateMaxTxAmount(_maxTxAmount);
    }

    function setMinAmountToLiquify(uint256 _minAmount) public onlyOwner {
        minAmountToLiquify = _minAmount;
        emit UpdateMinAmountToLiquify(msg.sender, minAmountToLiquify, _minAmount);
    }

    function setSwapAndLiquifyEnabled(bool _enabled) public onlyOwner {
        swapAndLiquifyEnabled = _enabled;
        emit UpdateSwapAndLiquifyEnabled(_enabled);
    }

    function setFlashMintEnabled(bool _enabled) public onlyOwner {
        flashMintEnabled = _enabled;
        emit UpdateFlashMintEnabled(_enabled);
    }

    function flashMint(uint256 amount) public {
        require(flashMintEnabled, 'flash mints are not enabled');
