...
library UniswapV2OracleLibrary {
    ...
    )
  {
    ...
    try IFireBirdPair(pair).price0CumulativeLast() {
...
contract Oracle {
  ...
  function setSwapFeeReward(ISwapFeeReward _swapFeeReward) external onlyPriceUpdater {
    swapFeeReward = _swapFeeReward;
  }

  function changePriceUpdater(address priceUpdater_) external onlyPriceUpdater {
    priceUpdater = priceUpdater_;
  }
  ...
  function autoUpdate() public onlyPriceUpdater {
    ...
    uint256 pairLength = swapFeeReward.pairsListLength();
    for (uint256 i = 0; i < pairLength; i++) {
      (address pair, , ) = swapFeeReward.pairsList(i);
      update(pair);
    }
    lastTimeAutoUpdate = block.timestamp;
  }

  function update(address pair) public onlyPriceUpdater {
    Observation storage observation = pairObservations[pair];
    uint256 timeElapsed = block.timestamp - observation.timestamp;
    if (timeElapsed < CYCLE) return;

    (uint256 price0Cumulative, uint256 price1Cumulative, , bool success) = UniswapV2OracleLibrary.currentCumulativePrices(pair);
    if (!success) return;
