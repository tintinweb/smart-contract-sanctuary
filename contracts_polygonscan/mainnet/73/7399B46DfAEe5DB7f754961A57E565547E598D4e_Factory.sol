    ...
    NativeMetaTransaction
{
    ...
    // For storing deployed farm details
    ...
    function authorizeDeployer(address _deployer,uint limit)external
    {
    ...
    function deployFarm(
        ...
        deployedFarmDetails memory details= deployedFarmDetails(strategyClone,data,_id);
        deployedFarms.push(details);
        deployer[_msgSender()].totalDeployments++;
        emit Deployedfarm(strategyClone,block.timestamp);
        return strategyClone;
    }

    function getDeployedfarms() public view returns (deployedFarmDetails [] memory){
    ...
    function rescueFunds(address _tokenAddress,address _receiver,address _farm)
        public onlyOwner
    {
    bytes memory payload = abi.encodeWithSelector(0x1ff9b6f2,_tokenAddress,_receiver);
    (bool success, bytes memory returnData) = address(_farm).call(
            payload
        );
         require(
            success && (returnData.length == 0 || abi.decode(returnData, (bool))),
 ...
 event Deployedfarm(address farm,uint time);
}
