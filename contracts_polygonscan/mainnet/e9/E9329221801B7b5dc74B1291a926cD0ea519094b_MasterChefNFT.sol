...
contract MasterChefNFT is Ownable, ReentrancyGuard, ERC721Holder{
    ...
    uint256 public constant sandManMaximumSupply = 5 * (10 ** 3) * (10 ** 18); // 5000 sandMan
    ...
    function _initializeNFT(uint256 _pid, uint256 _nftID, address _userAddress) internal  {
        UserInfo storage user = userInfo[_pid][_userAddress];

          if (!nftIDs[_nftID]) {
            if (iNFT.ownerOf(_nftID) == _userAddress) {
                //transfer nft to mc
                ...
                nftIDs[_nftID] = true;
                uint256 _experience = _getNFTExperience(user.nftID);
                if (_experience > 0 && _experience < 1e5) {
                    _experience = _experience * 1e18;
                }
    ...
    function _updateHarvestLookup(uint256 _pid, address _userAddress) internal {
        ...
        uint256 newHarvestInverval = pool.harvestInterval;
        if (user.hasNFT) {
            newHarvestInverval = newHarvestInverval / 2;
            if (user.experience > 0) {
                uint256 _calculatedExperience = (user.experience / 1e18) + 120;
                if (_calculatedExperience < newHarvestInverval) {
                    newHarvestInverval = newHarvestInverval - _calculatedExperience;
                }
            }
        }
    ...
    function payNftBoost(uint256 _pid, address _userAddress, uint256 _pending) internal {
        ...
        uint256 powerStaking = user.powerStaking;
        uint256 experience = user.experience / 1e18;
        if (experience > 0) {
            if (experience < 200) {
                powerStaking = powerStaking + 50;
            } else if (experience < 600) {
                powerStaking = powerStaking + 100;
            } else if (experience < 900) {
                powerStaking = powerStaking + 150;
            } else if (experience < 1300) {
                powerStaking = powerStaking + 200;
            } else if (experience < 1800) {
                powerStaking = powerStaking + 250;
            } else if (experience < 2600) {
                powerStaking = powerStaking + 300;
            } else if (experience < 3300) {
                powerStaking = powerStaking + 350;
            } else {
                powerStaking = powerStaking + 400;
            }
        }
        if (powerStaking > 0) {
            uint256 rewardBoosted = (_pending  * powerStaking) / factor;
            if(rewardBoosted > 0) {
    ...
    function _checkOwnership(uint256 _nftID) internal view returns (bool) {
        return iNFT.ownerOf(_nftID) == msg.sender;
    }
    ...
    function addNFT(uint256 _pid, uint256 _nftID) external nonReentrant {
        ...
        require(!user.hasNFT, "user already added nft");
        require(_checkOwnership(_nftID), "User is not owner of nft sent");

        updatePool(_pid);
        payOrLockupPendingSandMan(_pid);
        _initializeNFT(_nftID, _pid, msg.sender);
        _updateHarvestLookup(_pid, msg.sender);
    ...
    function withdrawNFT(uint256 _pid) external nonReentrant {
        ...
        require(user.hasNFT, "user no has nft");
    ...
    // Deposit LP tokens to MasterChef for SANDMAN allocation.
    function stake(uint256 _pid, uint256 _amount, address _userAddress) external nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_userAddress];
        updatePool(_pid);
