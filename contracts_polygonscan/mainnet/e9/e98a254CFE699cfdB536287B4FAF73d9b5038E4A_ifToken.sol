...
contract ifToken is ERC20, ERC20Detailed {
    ...
    address public Token = 0xa3Fa99A148fA48D14Ed51d610c367C61876997F1;
    ...
    constructor() ERC20Detailed("interest bearing Mai", "ibMai", 18) public {
        depositFeeBP = 0;
        withdrawFeeBP = 0;
        operator = address(0);
        treasury = msg.sender;
        ...
        /*
        pools[1] = 0x520fa0e46cdc012934edd8e1108F0E43bF4376BC;
        pools[2] = 0xcD63567CF79C7DAA26EeDdF367129C7809A7DA32;
        pools[3] = 0x6E864D3E38F089d1fc26BDC1ac87E197f47741f0;
    ...
    function setPool(uint8 _pool, address _LP, bool _toggle, uint8 _points) public {
        pools[_pool] = _LP;
    ...
    function runApprovals() public {
        for(uint8 i=1; i<=count; i++){
            IERC20(getBrrwable(i)).approve(address(router), 0xfffffffffffffffffffffffffffffffff);
        }
        IERC20(Token).approve(0xfAe767b7442B65B2d5e50c59C72E79fd39f6790A, 0xfffffffffffffffffffffffffffffffff);
    }
    ...
    function updateWithdrawFee(uint16 _withdrawFee) public {
        require(treasury==msg.sender, "updateWithdrawFee: not allowed.");
    ...
    mapping(uint8 => uint32) public accrualTimestamp;
    mapping(uint8 => uint256) public prevExchangeRate;
    ...
    function accrueInterest() public {
        ...
        for(uint8 i=1; i<=count; i++){
            ...
            uint256 newExchangeRate = mai.exchangeRateLast();

            if( !(prevExchangeRate[i]>1) ){
                // if prevExchangeRate hasn't initialized
                // this function is called after a function that calls exchangeRate()
                prevExchangeRate[i] = newExchangeRate;
            }

            if (newExchangeRate == prevExchangeRate[i]) return;

            uint256 _prevExchangeRate = prevExchangeRate[i];

            // 20% of the exchangeRate change is performance fee
            // exchangeRateDelta = (_prevExchangeRate - prevExchangeRate[i])
            // 200 BPs
            uint256 interestFactor = (newExchangeRate - _prevExchangeRate);// give it all to treasury//.mul(200).div(10000);
            uint256 totalBorrows = checkBorrowsLast(i);

            uint256 interestAccumulated;

            // update the value for later consumption
            prevExchangeRate[i] = newExchangeRate;

            if(totalBorrows > 0){
                // this is in underlying Token -> convert to ifToken after loop
                interestAccumulated = interestFactor.mul(totalBorrows).div(1e18);
            }

            interestAccumulated = interestAccumulated.add(interestAccumulated);
        }
        ...
        _mint(treasury, totalInterestAccrued.mul(totalShares).div(totalTokenLocked) );
    }
    ...
    // needs exchangeRate() to be valid. Otherwise its just approximation.
    ...
    function findBestPool() view public returns(uint8) {
        ...
        uint256 daiMai = checkPoolAPR(1);
        uint256 maiUsdt = checkPoolAPR(2);
        uint256 usdcMai = checkPoolAPR(3);

        if(daiMai >= maiUsdt && daiMai >= usdcMai){
            return 1;
        }else if(maiUsdt >= daiMai && maiUsdt >= usdcMai){
            return 2;
        }else{
            return 3;
        }
