...
contract Zap is IZAP, Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable, IGrowUpgradeableImplementation {
    ...
    function zapAndDepositTo(address fromToken, uint amount, address poolAddress, uint minReceive) external payable onlyHumanOrWhitelisted receiveToken(fromToken, amount) {
        ...
        if (
            fromToken != wantToken && (
                IGrowUpgradeableImplementation(poolAddress).CONTRACT_IDENTIFIER() == keccak256("GrowStrategySwampCurveV3") || IGrowUpgradeableImplementation(poolAddress).CONTRACT_IDENTIFIER() == keccak256("GrowStrategyCurveV4")
            )
        ) {
    ...
    function getZapAmountOut(address fromToken, uint amount, address poolAddress) external view returns (uint wantTokenAmount) {
        ...
        if (IGrowUpgradeableImplementation(poolAddress).CONTRACT_IDENTIFIER() == keccak256("GrowStrategySwampCurveV3") || IGrowUpgradeableImplementation(poolAddress).CONTRACT_IDENTIFIER() == keccak256("GrowStrategyCurveV4")) {
