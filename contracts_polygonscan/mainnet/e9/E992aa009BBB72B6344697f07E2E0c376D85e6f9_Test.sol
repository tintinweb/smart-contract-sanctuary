...
contract QuickSwapTrader {
    ...
    function _quickSwapSwapToken(kPolygonToken _inToken, kPolygonToken _outToken, uint256 _amount) internal
    {
        _quickSwapSwapTokenByAddress(PolygonTokens.getTokenAddress(_inToken), PolygonTokens.getTokenAddress(_outToken), _amount);
    }

    function _quickSwapGetAmountOut(kPolygonToken _inToken, kPolygonToken _outToken, uint256 _amount) internal view returns(uint256)
    {
        return _quickSwapGetAmountOutByAddress(PolygonTokens.getTokenAddress(_inToken), PolygonTokens.getTokenAddress(_outToken), _amount);
    }

    function _quickSwapGetAmountOutByAddress(address _inTokenAddress, address _outTokenAddress, uint256 _amount) internal view returns(uint256)
    {
        return _quickSwapGetAmountOutByPath(_createAddressPath(_inTokenAddress, _outTokenAddress), _amount);
    }

    function _quickSwapGetAmountOutByPath(address[] memory _path, uint256 _amount) internal view returns(uint256)
    {
    ...
    function _quickSwapSwapTokenByAddress(address _inTokenAddress, address _outTokenAddress, uint256 _amount) internal
    {
        if (_amount == 0)
        {
            return;
        ...
        _quickSwapApproveSpending(_inTokenAddress);
        ...
        address[] memory path = _createAddressPath(_inTokenAddress, _outTokenAddress);
