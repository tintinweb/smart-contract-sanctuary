...
contract VentureCapital is IVentureCapital, Pausable, ExternallyCallable {
    ...
    /** Don't allow if blacklisted */
    modifier isNotBlackListed(address account) {
        require(
            _blacklist[account] == false,
            'VentureCapital: Account is blacklisted from withdrawing'
        );
        _;
    }
    ...
    function withdrawDivTokens(address tokenAddress)
        external
        whenNotPaused
    ...
    function withdrawDivTokensTo(address payable to, address tokenAddress)
        external
        whenNotPaused
        isNotBlackListed(msg.sender)
    {
    ...
    function withdrawDivTokensToAccelerator(
        ...
        withdrawDivTokenInternal(from, acceleratorAddress, tokenAddress);
    }
    ...
    function withdrawAllDivTokensTo(address payable to)
        external
        whenNotPaused
        isNotBlackListed(msg.sender)
    {
    ...
    function withdrawDivTokenInternal(
        ...
        if (tokenAddress != address(0xFFfFfFffFFfffFFfFFfFFFFFffFFFffffFfFFFfF)) {
            IERC20Upgradeable(tokenAddress).safeTransfer(to, tokenInterestEarned);
        } else {
        ...
        onlyExternalCaller
    {
        if (tokenAddress != axion) {
            tokenPricePerShare[tokenAddress] =
                ...
                (amountBought * (1e36)) /
                (contracts.stakeManager.getTotalShares() + 1e12);
        }
    }
