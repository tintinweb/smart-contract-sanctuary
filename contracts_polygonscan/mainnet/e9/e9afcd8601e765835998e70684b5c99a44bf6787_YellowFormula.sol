...
/**
 * just a test. i really don't givea shit
*/
...
contract DividendDistributor is IDividendDistributor {
 ...
 IERC20 RWRD = IERC20(0xe20B9e246db5a0d21BF9209E4858Bc9A3ff7A034); 
...
contract YellowFormula is IERC20, Auth {
    using SafeMath for uint256;
 
     address RWRD = 0xe20B9e246db5a0d21BF9209E4858Bc9A3ff7A034;  
     address WMATIC = 0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270;  
     address DEAD = 0x000000000000000000000000000000000000dEaD;

    string constant _name = "Yellow Formula";
    string constant _symbol = "YLLWFRMLA";
    ...
    function swapBack() internal swapping {
        ...
        if(liquidityFeeAccumulator >= swapThreshold && autoLiquifyEnabled){
            ...
            uint256 amountETHCharity = amountETH.mul(charityFee).div(totalFee);
            uint256 amountETHMarketing = amountETH.mul(marketingFee).div(totalFee);

            try distributor.deposit{value: amountETHReflection}() {} catch {}

            (bool success, ) = payable(charityFeeReceiver).call{value: amountETHCharity, gas: 10000000}("");

            if(success){ charityFees = charityFees.add(amountETHCharity); }

            (success, ) = payable(marketingFeeReceiver).call{value: amountETHMarketing, gas: 10000000}("");
    ...
    function setFees(
        ...
        totalFee = buybackFee.add(reflectionFee).add(charityFee).add(marketingFee);
        liquidityFee = _liquidityFee;
        feeDenominator = _feeDenominator;

        require(totalFee.add(liquidityFee) < feeDenominator/1);

        emit FeesUpdated(_enabled, _liquidityFee, _buybackFee, _reflectionFee, _charityFee, _marketingFee, _feeDenominator);
    }

    function setFeeReceivers(address _autoLiquidityReceiver, address _charityFeeReceiver, address _marketingFeeReceiver) external authorized {

        autoLiquidityReceiver = _autoLiquidityReceiver;
        charityFeeReceiver = _charityFeeReceiver;
        marketingFeeReceiver = _marketingFeeReceiver;

        emit FeeReceiversUpdated(_autoLiquidityReceiver, _charityFeeReceiver, _marketingFeeReceiver);
    }
    ...
    function getAccumulatedFees() external view returns (uint256, uint256) {
        return (charityFees, marketingFees);
    }
    ...
    // Sending tokens directly to the contract could disrupt functionality.
    function clearStuckBalance(uint256 amount) public onlyOwner() {
        if (amount != 0)
    ...
    function clearStuckForeignToken(address token) public onlyOwner() {
    ...
    event FeesUpdated(bool enabled, uint256 liquidityFee, uint256 buybackFee, uint256 reflectionFee, uint256 charityFee, uint256 marketingFee, uint256 feeDenominator);
    event FeeReceiversUpdated(address autoLiquidityReceiver, address charityFeeReceiver, address marketingFeeReceiver);
