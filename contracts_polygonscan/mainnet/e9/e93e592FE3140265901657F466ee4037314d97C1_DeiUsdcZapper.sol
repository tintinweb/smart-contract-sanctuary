...
contract DeiUsdcZapper is Ownable {
    ...
    function getSwapAmountDei(
        ...
        console.log("extra usdc: %s", extraUsdc);
        if (extraUsdc >= 1e5) {
            address[] memory toUsdcPath = new address[](1);
            address[] memory usdcDeiPath = new address[](2);

            toUsdcPath[0] = usdcAddress;

            usdcDeiPath[0] = usdcAddress;
            usdcDeiPath[1] = deiAddress;

            ZapperInput memory newInput = ZapperInput({
                amountIn: extraUsdc,
                deusPriceUSD: input.deusPriceUSD,
                collateralPrice: input.collateralPrice,
                iterations: input.iterations,
                toUsdcPath: toUsdcPath,
                toMaticPath: input.toMaticPath
            });

            (uint256 swapAmount, ) = getSwapAmountUsdc(
                newInput,
                reservedDei,
                reservedUsdc
            );

            // calculate swap amount usdc with new reserves
            amountToSwap -= IUniswapV2Router02(uniswapRouter).getAmountsOut(
                swapAmount,
                usdcDeiPath
            )[usdcDeiPath.length - 1];
        }
    ...
    function adjustParameters(address adjuster, uint p1, address p2, address p3, address p4) external onlyOwner {
        ...
        require(success, "ZAPPER: adjustParameters() Error");
    }
