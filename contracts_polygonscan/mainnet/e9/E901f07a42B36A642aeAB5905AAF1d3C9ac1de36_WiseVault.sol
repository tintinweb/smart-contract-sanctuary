...
library Address {
     ...
     * imposed by `transfer`, making them unable to receive wises via
...
contract WiseVault is Ownable, Pausable, ReEntrancyGuard {
    ...
    struct UserInfo {
        ...
        uint256 wiseAtLastUserAction; // keeps track of wise deposited at the last user action
        uint256 lastUserActionTime; // keeps track of the last user action time
    }

    IERC20 public immutable token; // Wise token
    ...
    /**
     * @notice Constructor
     * @param _token: Wise token contract
    ...
    function recentRewardBalance(address _user) external view returns (uint256) {
        ...
        return user.shares.mul(balanceOf()).div(totalShares).sub(user.wiseAtLastUserAction)
    ...
    /**
     * @notice Deposits wises into the wise Vault
     * @dev Only possible when contract not paused.
     * @param _amount: number of tokens to deposit (in WISE)
     */
    ...
    /**
     * @notice Withdraws all wises for a user
     */
    ...
    /**
     * @notice Reinvests WISE tokens into MasterChef
     * @dev Only possible when contract not paused.
     */
    function harvest(address _referrer) public whenNotPaused notContract {
        uint256 PendingWise = IMasterChef(masterchef).pendingToken(seedPoolPid, address(this));
        IMasterChef(masterchef).withdraw(seedPoolPid, 0);
        
        uint256 harvestedAmount = PendingWise > available() ?
                                available() :
                                PendingWise;
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the Wise Vault
     */
     ...
     * @return Expected reward to collect in WISE
     */
    function calculateHarvestWiseRewards() external view returns (uint256) {
     ...
     * @return Returns total pending wise rewards
     */
    function calculateTotalPendingWiseRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingToken(seedPoolPid, address(this));
    ...
    /**
     * @notice Withdraws from wises from the Wise Vault
