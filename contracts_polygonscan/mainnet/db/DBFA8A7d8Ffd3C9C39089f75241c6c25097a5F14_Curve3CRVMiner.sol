...
interface IAutoTokens {
    function harvest(address strat) external;

    function pendingAUTO(address strat, address _user)
        external
        view
        returns (uint256);
}
...
/**
 * @title https://polygon.curve.fi/ pool + AUTO
...
contract Curve3CRVMiner is Ownable, XChangerUser, CurveUtils {
    ...
    IAutoFarm internal constant AUTOFARM =
        IAutoFarm(0x89d065572136814230A55DdEeDDEC9DF34EB0B76);

    IAutoTokens internal constant PAUTO_SENDER =
        IAutoTokens(0x703F98CB0DA4b8bf64e1C7549e49d140C0acbF94);

    IAutoTokens internal constant WMATIC_SENDER =
        IAutoTokens(0x36477AF584988cb79e2991bfa5CfF2CE275435BE);
    ...
    IBEP20 internal constant PAUTO =
        IBEP20(0x7f426F6Dc648e50464a0392E60E1BB465a67E9cf);
    ...
    function init(
        ...
        CurveUtils.init();
        _init(_enterToken, _LPToken, _pid, _xChanger);
    }
    ...
    */
    ...
    function getLPStaked() public view returns (uint256 totalLPStaked) {
        totalLPStaked = AUTOFARM.stakedWantTokens(pid, address(this)); // NOT USERINFO
    }
    ...
    function getStrategyAddress() public view returns (address strat) {
        (
            ...
            // How many allocation points assigned to this pool. AUTO to distribute per block.
            // Last block number that AUTO distribution occurs.
            // Accumulated AUTO per share, times 1e12. See below.
            strat // Strategy address that will auto compound want tokens
        ) = AUTOFARM.poolInfo(pid);
    }

    /**
     * @dev Get the pending PAUTO -> WMATIC value
     */

    function getPendingBonus() public view returns (uint256 totalValue) {
        address strat = getStrategyAddress();
        uint256 pendingTokens = PAUTO_SENDER.pendingAUTO(strat, address(this));
        uint256 performanceToken = (pendingTokens * claimFee) / 10000;

        totalValue = quote(PAUTO, WMATIC, performanceToken);
        uint256 pendingWMATIC = WMATIC_SENDER.pendingAUTO(strat, address(this));
        totalValue += pendingWMATIC;
    }

    /**
     * @dev Get the pending PAUTO -> [enterToken] value
     */
    ...
    function getPendingEnterToken() public view returns (uint256 totalValue) {
        ...
        totalValue = quote(PAUTO, enterTokenIBEP20, pendingTokens);

        pendingTokens = WMATIC_SENDER.pendingAUTO(strat, address(this));
        if (pendingTokens > 0) {
            claimToken = (pendingTokens * claimFee) / 10000;
            performanceToken = (pendingTokens * performanceFee) / 10000;
            pendingTokens -= (performanceToken + claimToken);
    ...
    function harvest() public {
        address strat = getStrategyAddress();
        PAUTO_SENDER.harvest(strat);
        if (WMATIC_SENDER.pendingAUTO(strat, address(this)) > 0) {
            WMATIC_SENDER.harvest(strat);
        }
    }

    function compoundHarvest() public returns (uint256 returnTokens) {
        uint256 token_balance = PAUTO.balanceOf(address(this));

        if (token_balance > 0) {
            if (claimFee > 0) {
                uint256 claimToken = (token_balance * claimFee) / 10000;
                token_balance -= claimToken;
                swap(PAUTO, WMATIC, claimToken, false);
            ...
            if (performanceFee > 0) {
                ...
                performanceToken = swap(PAUTO, WMATIC, performanceToken, false);
                WMATIC.transfer(feeManager, performanceToken);
            }

            // Change the rest to EnterToken and compound
            returnTokens = swap(PAUTO, enterTokenIBEP20, token_balance, false);
        }
    }
}