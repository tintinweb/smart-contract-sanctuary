...
contract pixeLoot is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    constructor() ERC721("pixeLoot-PolygonTest8", "pxL-P8") {}
    ...
    function generateColorString(uint256 color) internal pure returns(string memory) {
        ...
        for (uint i = 0; i < 3; i++) {
            byteBuffer = bytes(abi.encodePacked(uint256(bytes32(abi.encodePacked(colorBytes[i]))).toHexString()));
