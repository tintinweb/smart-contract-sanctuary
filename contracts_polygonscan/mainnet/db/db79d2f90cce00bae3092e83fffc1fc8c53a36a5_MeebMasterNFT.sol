...
contract MeebMasterNFT is BNFT, IMeebMasterNFT, ReentrancyGuard {
    using Address for address;
    ...
    function _checkOnERC721Unlocked(
        ...
        if (locker.isContract()) {
            try IERC721Locker(locker).onERC721Unlocked(_msgSender(), from, tokenId, _data) returns (bytes4 retval) {
                ...
                if (reason.length == 0) {
                    revert("MeebMasterNFT: unlock from a non ERC721Locker implementer");
                } else {
                    // solhint-disable-next-line no-inline-assembly
                    assembly {
                        revert(add(32, reason), mload(reason))
    ...
    function _beforeTokenTransfer(
        ...
        if (_lockedBy[tokenId] != to) {
            require(_lockedBy[tokenId] == address(0), "MeebMasterNFT: token transfer while locked");
        } else {
            delete _lockedBy[tokenId];
        }
    }
