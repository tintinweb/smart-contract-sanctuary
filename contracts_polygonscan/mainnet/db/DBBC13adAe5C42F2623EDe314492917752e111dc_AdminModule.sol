...
contract Variables {
    ...
    // struct Rates {
    //     uint64 borrowRate; // per sec in 18 decimal
    //     uint64 lastUpdateTime; // in sec
    //     uint128 exchangePrice; // exchange price. Initial exchange price.
    // }

    // token => Rates
    // mapping (address => Rates) internal _rates;

    // starting borrow exchange price.
    // uint128 constant internal _startingExchangePrice = 1e18;

    // borrow exchange price. Token => exchange price
    // mapping (address => uint) internal _exchangePrice;
