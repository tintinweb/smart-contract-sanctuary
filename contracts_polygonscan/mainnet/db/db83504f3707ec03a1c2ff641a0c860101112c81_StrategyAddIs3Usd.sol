...
contract StrategyAddIs3Usd is Ownable, ReentrancyGuard, Strategy {
  ...
  address public constant vault = 0xBba656f215846292B4c429a3Cca692C98530602b;
  ...
  function execute(
    ...
    fToken.safeApprove(address(ironSwap), 0);
    fToken.safeApprove(address(ironSwap), uint(-1));
    banktoken.safeApprove(address(ironSwap), 0);
    banktoken.safeApprove(address(ironSwap), uint(-1));

    uint[] memory _amounts = new uint[](3);

    _amounts[IIronSwap(ironSwap).getTokenIndex(fToken)] = fToken.myBalance();
    _amounts[IIronSwap(ironSwap).getTokenIndex(banktoken)] = banktoken.myBalance();

    IIronSwap(ironSwap).addLiquidity(_amounts,0,now+1);

    uint have = is3usd.myBalance();

    require(have >= minLPAmount, 'insufficient LP tokens received');
    is3usd.safeApprove(vault, uint(-1));
