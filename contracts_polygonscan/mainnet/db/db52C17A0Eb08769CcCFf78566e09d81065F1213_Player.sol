...
// File: ..\contracts\tools\player.sol
...
contract Player is ERC721 {
    ...
    function getStrength(uint256 tokenId) view public returns(uint64) {
        return attributes[tokenId].strength;
    }
    ...
    function mintPlayer() public {
        uint64 _strength = uint64(1 + getRnd(66, 8));
        mint(_strength);
    }

    function mint(uint64 _strength) public {
        globalTokenId++;
        _safeMint(msg.sender, globalTokenId);
        attributes[globalTokenId] = Attributes(_strength);
    }
}