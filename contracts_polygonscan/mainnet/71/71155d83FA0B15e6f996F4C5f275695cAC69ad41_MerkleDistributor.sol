...
contract MerkleDistributor is IMerkleDistributor {
    ...
    address public tokenMDG;
    address public tokenBUSD;
    bytes32 public immutable override merkleRoot;
    
      // constant
    uint256 public constant totalLP = 27017000000000000000000;
    uint256 public constant totalMDG = 21303000000000000000000;
    uint256 public constant totalBUSD = 35755000000000000000000;
    ...
    function claim(uint256 index, address account, uint256 amount, bytes32[] calldata merkleProof) external override notPaused {
        ...
        _setClaimed(index);
        uint256 _amtMDG = (amount.mul(totalMDG)).div(totalLP);
        uint256 _amtBUSD = (amount.mul(totalBUSD)).div(totalLP);
        require(IERC20(tokenMDG).transfer(account, _amtMDG), "MerkleDistributor: Transfer failed.");
        require(IERC20(tokenBUSD).transfer(account, _amtBUSD), "MerkleDistributor: Transfer failed.");

        emit Claimed(index, account, amount);
    }

    function governanceRecoverUnsupported(address _token, uint256 _amount) external onlyOperator {
        IERC20(_token).transfer(address(0x2B9c213580279f02cA558F0d361c718e6A14ca9F), _amount); // In case of emergency, we pull back token to the origin (old strategy)
    }
}