...
contract StakeReader is Initializable {
    ...
    function getStakes(uint256 from, uint256 to) external view returns (StakeUri[] memory) {
        StakeUri[] memory stakes = new StakeUri[](to - from + 1);

        uint256 count = 0;
        for (uint256 i = from; i <= to; i++) {
            stakes[count] = stakeManager.getStakeUri(i);
            count++;
        }
