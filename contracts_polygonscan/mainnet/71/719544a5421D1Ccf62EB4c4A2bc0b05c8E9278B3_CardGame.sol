...
contract CardGame is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    string public constant SET_NAME = "CardGame-Polygon";
    ...
    function getAbility(uint256 tokenId, uint256 position) public view returns (string memory) {
        uint256 cardType = getTypeId(tokenId);
        uint256 rand = random(string(abi.encodePacked(SET_NAME, "ShowAbility", toString(tokenId))));
        uint256 randAbility = random(string(abi.encodePacked(SET_NAME, "Ability", toString(tokenId)))) + position;
    ...
    function getStat(uint256 tokenId, uint256 position) public pure returns (uint256) {
        uint256 rand = random(string(abi.encodePacked(SET_NAME, "Number", position, "-", toString(tokenId))));
        return  rand % 10; 
    }
    ...
    function pluck(
        ...
        uint256 rand = random(string(abi.encodePacked(SET_NAME, keyPrefix, toString(tokenId))));
