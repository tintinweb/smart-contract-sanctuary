...
contract ERC1155 is IERC165 {
  ...
  {
    // Update balances
    balances[_from][_id] -= _amount; // Subtract amount
    balances[_to][_id] += _amount;     // Add amount
 ...
 */
contract UniftyCollection is ERC1155Tradable {
    
    string private _contractURI = "https://unifty.io/meta/uniftyverse.json";
    
	constructor(address _proxyRegistryAddress) ERC1155Tradable("Uniftyverse", "UVERSE", _proxyRegistryAddress) {
...
contract Erc1155MultiBatch is Ownable, WhitelistAdminRole {
    
    event TransferError(address _sender, address _to, uint256[] _ids, uint256[] _amounts);
    //event TransferSuccess(address _sender, address _to, uint256[] _ids, uint256[] _amounts);
    
    function transferMultiBatch( address _erc1155Address, address[] calldata _to,  uint256[] calldata _ids, uint256[] calldata _amounts) external{
        
        require(_to.length != 0, "Erc1155MultiBatch#transferMultiBatch: please add at least one recipient.");
        require(_ids.length != 0, "Erc1155MultiBatch#transferMultiBatch: please add at least one NFT.");
        require(_ids.length == _amounts.length, "Erc1155MultiBatch#transferMultiBatch: ids and amounts differ in length.");
        
        for(uint256 i = 0; i < _to.length; i++){
            try ERC1155Tradable(_erc1155Address).safeBatchTransferFrom(msg.sender, address(_to[i]), _ids, _amounts, "")
            {
                //emit TransferSuccess(msg.sender, address(_to[i]), _ids, _amounts);
            }
            catch{
                
                emit TransferError(msg.sender, address(_to[i]), _ids, _amounts);
            }
        }
    }
}