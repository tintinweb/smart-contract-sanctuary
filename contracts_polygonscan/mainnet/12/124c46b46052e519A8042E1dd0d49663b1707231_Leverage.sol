...
contract Leverage is AccessControl, Initializable {
  ...
  modifier onlyOwner() {
    require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "sender must be the owner");
    _;
  }

  function initialize(IAddressProvider _a, ILendingPool _lendingPool, address _owner) external initializer {
  ...
  function executeOperation(
    ...
    (uint256 parToSell, bytes memory dexTxData) = abi.decode(params, (uint256, bytes));

    token.approve(address(a.core()), 2 ** 256 - 1);
    console.log("token balance before deposit", token.balanceOf(address(this)));
    a.core().depositAndBorrow(assets[0], token.balanceOf(address(this)), parToSell);
    console.log("token balance after deposit", token.balanceOf(address(this)));
    console.log("PAR balance after deposit", IERC20(a.stablex()).balanceOf(address(this)));

    IERC20(a.stablex()).approve(oneInch, parToSell);
    oneInch.call(dexTxData);

    console.log("token balance after trade", token.balanceOf(address(this)));
    console.log("PAR balance after trade", IERC20(a.stablex()).balanceOf(address(this)));

    uint256 flashloanRepayAmount = amounts[0] + premiums[0];
    console.log("flashloanRepayAmount", flashloanRepayAmount);

    // we need to pay back AAVE
    token.approve(address(lendingPool), flashloanRepayAmount);

    a.core().deposit(assets[0], token.balanceOf(address(this)) - flashloanRepayAmount);
  ...
  function leverage(
    address asset,
    uint256 depositAmount, // the initial starting amount, ie. 1 ETH
    uint256 borrowAmount, // the amount to be borrowed, ie. 2 ETH, bringing the total to 3 ETH
    uint256 parToSell, // how much PAR we're gonna borrow so we can repay the leverage
    ...
    bytes memory params = abi.encode(parToSell, dexTxData);
