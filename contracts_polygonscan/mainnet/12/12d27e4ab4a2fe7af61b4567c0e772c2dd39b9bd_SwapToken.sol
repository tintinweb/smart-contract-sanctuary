...
contract SwapToken is Ownable{
    ...
    uint public multiplyerEndsIn = 0 days;
    ...
    constructor(IERC20 _oldToken, IERC20 _newToken){
        ...
        multiplyerEndsIn = 180 days;
    }
    
    function setSwapEnable() public onlyOwner{
        isSwapEnable = true;
    }
    
    function setSwapDisable() public onlyOwner{
        isSwapEnable = false;
    }
    ...
    function setMultiplyerEndsIn(uint _days) public onlyOwner{
        require(multiplyerEndsIn < _days.mul(1 days), "Days must be more than current value");
        multiplyerEndsIn = _days.mul(1 days);
    }
    
    function claimToken(uint _amount) public{
        require(isSwapEnable, "Swap is not enable");
        ...
        if((startTime.add(multiplyerEndsIn)) > block.timestamp){
            oldToken.safeTransferFrom(msg.sender, deadAddress, _amount);
            newToken.mint(msg.sender, _amount.mul(2));
        }
        else{
            oldToken.safeTransferFrom(msg.sender, deadAddress, _amount);
    ...
    function newTokenContractBalance() public view returns(uint){
