...
contract ArbitrageBuddy is Ownable {
    ...
    constructor() {
        ...
        baseTokens.add(0xdAb529f40E671A1D4bF91361c21bf9f0C9712ab7); //BUSD
...
/// @dev Compare price denominated in quote token between two pools
    ...
    function getProfit(address pool0, address pool1) internal view returns (uint256 profit, address baseToken) {
        ...
        (, , OrderedReserves memory orderedReserves) = getOrderedReserves(pool0, pool1, baseTokenSmaller);
         uint256 borrowAmount = calcBorrowAmount(orderedReserves, isOne6Decimal(token0, token1));
    ...
    function getProfit_external(address pool0, address pool1) external view returns (uint256 profit, address baseToken) {
        (profit, baseToken) = getProfit(pool0, pool1);
    }
    ...
    function flashArbitrage(address pool0, address pool1) external {
        ...
        {
            uint256 borrowAmount = calcBorrowAmount(orderedReserves, isOne6Decimal(info.baseToken, info.quoteToken));
    ...
    function calcBorrowAmount(OrderedReserves memory reserves, bool noDivide) internal pure returns (uint256 amount) {
