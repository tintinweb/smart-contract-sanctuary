...
contract Strategy_MATIC_USDT is Ownable {
    ...
    function _addLiquidityFromAllTokens() internal {
        _swapWMATICToToken(getBalanceOfToken(token0)/2, routerMain, token1);
    ...
    function _swapLPtoWMATIC(uint256 _percent) internal {
        ...
        if (_totalLP > 0) {
            ...
            // swap token0 and token1 to WMATIC
    ...
    function _swapTokenToWMATIC(address _token) internal {
        ...
        if(_balance > 0) {
            address _router = _token == token1 ? routerMain : router;
