...
contract GrowStrategyAutoCurveV3 is IGrowUpgradeableImplementation, IGrowStrategyCurve, BaseGrowStrategyV3 {
    ...
    address public _deprecated_UNDERLYING_REWARD_TOKEN;
    ...
    uint256 public _deprecated_UNDERLYING_REWARD_TOKEN_AMOUNT;
    ...
    mapping(address => uint256) public UNDERLYING_REWARD_TOKENS_AMOUNT;
    ...
    function updateUnderlyingRewardTokens(address[] calldata _UNDERLYING_REWARD_TOKENS) external onlyRole(CONFIGURATOR_ROLE) {
        UNDERLYING_REWARD_TOKENS = _UNDERLYING_REWARD_TOKENS;
    }
    ...
    function getOriginTokenAmountOut(address token, uint256 amount) public view override returns (uint256) {
        uint256 tokenIndex = getTokenIndexFromOriginalTokens(token);
        if (ORIGINAL_TOKENS.length == 3) {
            return ICurveCryptoSwap3(CURVE_TOKEN_MINTER).calc_withdraw_one_coin(amount, int128(uint128(tokenIndex)));
        }
        if (ORIGINAL_TOKENS.length == 5) {
            return ICurveCryptoSwap5(CURVE_TOKEN_MINTER).calc_withdraw_one_coin(amount, tokenIndex);
        }
        return 0;
    }
    ...
    function _trySwapUnderlyingRewardToRewardToken() internal override {
        for (uint256 i = 0; i < UNDERLYING_REWARD_TOKENS.length; ++i) {
            ...
            uint256 UNDERLYING_REWARD_TOKEN_AMOUNT = UNDERLYING_REWARD_TOKENS_AMOUNT[UNDERLYING_REWARD_TOKENS[i]];
            ...
            if (UNDERLYING_REWARD_TOKEN_AMOUNT < DUST) return;
            ...
            // set UNDERLYING_REWARD_TOKENS_AMOUNT
            UNDERLYING_REWARD_TOKENS_AMOUNT[UNDERLYING_REWARD_TOKENS[i]] = UNDERLYING_REWARD_TOKEN_AMOUNT.sub(rewardTokenAmountBefore.sub(underlyingRewardAmountAfter));
        }
    }
    ...
    function _wantTokenPriceIn1e6USDC(uint256 amount) public view override returns (uint256) {
        return getOriginTokenAmountOut(USDC_TOKEN, amount);
    }
    ...
    function _getOriginToken(uint256 underlyingWantTokenAmount, address tokenAddress, uint256 tokenIndex, uint256 minReceive) internal returns (uint256) {
        ...
        if (ORIGINAL_TOKENS.length == 3) {
            ICurveCryptoSwap3(CURVE_TOKEN_MINTER).remove_liquidity_one_coin(underlyingWantTokenAmount, int128(uint128(tokenIndex)), minReceive, true);
        }
        if (ORIGINAL_TOKENS.length == 5) {
            approveToken(STAKING_TOKEN, CURVE_TOKEN_MINTER, underlyingWantTokenAmount);
    ...
    modifier updateUnderlyingRewardTokensAmount(uint256 depositAmount) {
        ...
        for (uint256 i = 0; i < UNDERLYING_REWARD_TOKENS.length; ++i) {
            ...
            UNDERLYING_REWARD_TOKENS_AMOUNT[UNDERLYING_REWARD_TOKENS[i]] += underlyingRewardAmountAfter.sub(underlyingRewardTokensAmountBefore[i]);
        }
    }
