...
import "../xDollarMaximizer.sol";

contract StrategyMaximizer_xUSD_USDC is xDollarMaximizer {

    address xusd_usdc_diff;

    function initializeStrategy(
        address _store,
        address _vault
    ) public initializer {
        address polycrystalHealer = address(0xeBCC84D2A73f0c9E23066089C6C24F4629Ef1e6d);
        address xusd_usdc_lp = address(0x9d5373d95E95ad9E47800f7959D4d8c195c35Df1);
        address xdo = address(0x3Dc7B06dD0B1f08ef9AcBbD2564f8605b4868EEA);
        address usdc = address(0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174);
        address crystal = address(0x76bF0C28e604CC3fE9967c83b3C3F31c213cfE64);
        address bxdo_usdc_lp = address(0x61e4cb40be5C07fEdAA3e46B083DdD89aa51b0A6);
        ...
        routes[xdo] = [crystal, usdc, xdo];
        routes[usdc] = [crystal, usdc];
...
/// @title xDollar Maximizer Strategy
/// @author Chainvisions + modification by jlontele
/// @notice Maximizer strategy for xDollar LPs on Polycrystal Farm. 

contract xDollarMaximizer is BaseUpgradeableStrategy {
    using SafeERC20 for IERC20;

    address public constant ApeRouter = address(0xC0788A3aD43d79aa53B09c2EaCc313A787d1d607);
    ...
    public initializer {
        ...
        (lpaddress, , , ,) = IMasterHealer(rewardPool()).poolInfo(_pid);
        require(lpaddress == underlying(), "xDollarMaximizer: Pool does not match with underlying");
        _setPoolId(_pid);
    }
    ...
    */
    function doHardWork() external onlyNotPausedInvesting restricted {
        IMasterHealer(rewardPool()).deposit(poolId(), 0);
        _liquidateReward();
    ...
    */
    function withdrawAllToVault() public restricted {
        if(rewardPool() != address(0)) {
            IMasterHealer(rewardPool()).withdraw(poolId(), _rewardPoolStake());
        }
        _liquidateReward();
    ...
    */
    function withdrawToVault(uint256 amount) public restricted {
        ...
        if(amount > IERC20(underlying()).balanceOf(address(this))){
            ...
            IMasterHealer(rewardPool()).withdraw(poolId(), Math.min(_rewardPoolStake(), needToWithdraw));
        }
    ...
    */
    function emergencyExit() public onlyGovernance {
        IMasterHealer(rewardPool()).emergencyWithdraw(poolId());
    ...
    */
    ...
    function _investAllUnderlying() internal onlyNotPausedInvesting {
        ...
        if(underlyingBalance > 0) {
            ...
            IMasterHealer(rewardPool()).deposit(poolId(), underlyingBalance);
        }
    }
    ...
    */
    function _liquidateReward() internal {
        ...
        if(routes[token0].length > 1) {
            IERC20(rewardToken()).safeApprove(ApeRouter, 0);
            IERC20(rewardToken()).safeApprove(ApeRouter, toToken0);
            uint256[] memory amounts = IUniswapV2Router02(ApeRouter).swapExactTokensForTokens(toToken0, 0, routes[token0], address(this), (block.timestamp + 600));
        ...
        if(routes[token1].length > 1) {
            IERC20(rewardToken()).safeApprove(ApeRouter, 0);
            IERC20(rewardToken()).safeApprove(ApeRouter, toToken1);
            uint256[] memory amounts = IUniswapV2Router02(ApeRouter).swapExactTokensForTokens(toToken1, 0, routes[token1], address(this), (block.timestamp + 600));
            token1Amount = amounts[(amounts.length - 1)];
        } else {
            token1Amount = toToken1;
        }

        IERC20(token0).safeApprove(ApeRouter, 0);
        IERC20(token0).safeApprove(ApeRouter, token0Amount);

        IERC20(token1).safeApprove(ApeRouter, 0);
        IERC20(token1).safeApprove(ApeRouter, token1Amount);

        IUniswapV2Router02(ApeRouter).addLiquidity(token0, token1, token0Amount, token1Amount, 0, 0, address(this), (block.timestamp + 600));
    }
    ...
    function _rewardPoolStake() internal view returns (uint256 stake) {
        (stake, ) = IMasterHealer(rewardPool()).userInfo(poolId(), address(this));
    }
}
