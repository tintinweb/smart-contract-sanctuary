...
// File contracts/interface/SushiBar.sol
...
// File contracts/strategies/base/StakingRewardsBaseStrategy.sol
...
contract StakingRewardsBaseStrategy is OwnableUpgradeable {
    using SafeMath for uint256;
    ...
    function _enterRewardPool() internal {
        ...
        IStakingRewards(rewardPool).stake(_balanceNotEntered);
    }
    ...
    function _liquidateReward() internal {
        uint256 _dRewardBalance = IERC20(dRewardToken).balanceOf(address(this));

        if (_dRewardBalance > sellFloor) {
            SushiBar(dRewardToken).leave(_dRewardBalance);
    ...
    function rewardPoolUnderlyingBalance() public view returns (uint256 _bal) {
        _bal = IStakingRewards(rewardPool).balanceOf(address(this));
    }
    ...
    function setSell(bool _sell) external onlyOwner {
        sell = _sell;
    }
...
// File contracts/strategies/quickswap/QuickPolygonUsdtUsdcStrategy.sol

pragma solidity 0.6.12;

contract QuickPolygonUsdtUsdcStrategy is StakingRewardsBaseStrategy {
    string public name;
    ...
    function initialize(address _vault, address _parentStrategy)
        ...
        initializer
    {
        name = "Polygon QuickSwap USDT-USDC Strategy";
        address underlying = address(
            0x2cF7252e74036d1Da831d11089D326296e64a728
        ); //UsdtUsdc Quick LP 
        address quickUsdtUsdcRewardPool = address(
            0xAFB76771C98351Aa7fCA13B130c9972181612b54
        );

        address quick = address(0x831753DD7087CaC61aB5644b308642cc1c33Dc13);
        address dQuick = address(0xf28164A485B0B2C90639E47b0f377b4a438a16B1);

        StakingRewardsBaseStrategy.initializeStakingRewardsBaseStrategy(
            _vault,
            _parentStrategy,
            underlying,
            quickUsdtUsdcRewardPool,
        ...
        rewardRoutes[usdt] = [quick, wmatic, usdt];
        rewardRoutes[usdc] = [quick, wmatic, usdc];

        fromVaultAssetRoutes[dai][usdt] = [dai, usdt];
        fromVaultAssetRoutes[dai][usdc] = [dai, usdc];
        fromVaultAssetRoutes[usdt][usdc] = [usdt, usdc];
        fromVaultAssetRoutes[usdc][usdt] = [usdc, usdt];

        toVaultAssetRoutes[dai][usdt] = [usdt, dai];
        toVaultAssetRoutes[dai][usdc] = [usdc, dai];
        toVaultAssetRoutes[usdt][usdc] = [usdc, usdt];
        toVaultAssetRoutes[usdc][usdt] = [usdt, usdc];
    }
}