...
contract FruitToken  is ERC20 {
    ...
    address public _marketing = address(0x02C28c3401BE55F4EBD1c4d75aab78F44803265a);
    ...
    require(amount <= totalSupply().mul(5).div(100) || _excludedFromAntiWhale[tx.origin] || _excludedFromAntiWhale[sender]);
    _;
    }
     ...
     */
    constructor() ERC20("FruitSwap.exchange Token", "FST") {
        _operator = msg.sender;
        emit OperatorTransferred(address(0), _operator);
        _excludedFromFees[ _operator] = true;
        _excludedFromAntiWhale[ _operator] = true;
        _mint(msg.sender, 300000 *10 ** 18);
         tokenSwapRouter = IRouter(0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff); //quick
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual checkWhale(sender, amount) override{
        ...
        if (recipient == BURN_ADDRESS || transferTaxRate == 0 || _excludedFromFees[tx.origin]) {
            ...
            uint256 devAmount = taxAmount.mul(1000).div(10000);
            uint256 liquidityAmount = taxAmount.sub(devAmount*2);
            uint256 sendAmount = amount.sub(taxAmount);
            super._transfer(sender, _operator, devAmount);
            super._transfer(sender, _marketing, devAmount);
