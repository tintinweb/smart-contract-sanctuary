...
contract ArkenDex {
    ...
    constructor(ConstructorParams memory params) {
        ...
        wrapperEtherERC20 = params._wrappedEther;
        wrapperEtherERC20Dfyn = params._wrappedEtherDfyn;
        dodoApproveAddress = params._dodoApproveAddress;
    ...
    function _tradeIDfyn(
        ...
        if (_src == ETHER_ERC20) {
            // ETH => TOKEN
            if (paths[0] == address(ETHER_ERC20)) {
                paths[0] = address(wrapperEtherERC20Dfyn);
            }
            amounts = uniRouter.swapExactETHForTokens{value: inputAmount}(
                minOutputAmount,
                paths,
            ...
            // TOKEN => ETH
            if (paths[paths.length - 1] == address(ETHER_ERC20)) {
                paths[paths.length - 1] = address(wrapperEtherERC20Dfyn);
            }
            _src.safeApprove(dexAddr, inputAmount);
            amounts = uniRouter.swapExactTokensForETH(
                inputAmount,
