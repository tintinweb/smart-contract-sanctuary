...
contract GrowthVault is ERC20, Ownable {
    ...
    function _addLiquidity(address _liquidityRouter, address _token0, address _token1) internal {
        ...
        if(_token0 != WETH) {
            ...
            IRouter(_liquidityRouter).swapExactETHForTokens{value:desiredAmountToken}(
        ...
        uint256 desiredAmountFTM = _balance / 2;
        if(_token1 != WETH && _token0 == WETH) {
            ...
            // amounts[1] == desired1
            require(
                balanceToken1 <= amounts[1],
                "Balance on token for the contract is less then calculated desired"
            );
            ...
            IRouter(_liquidityRouter).addLiquidity(
                ...
                desired0,
                amounts[1],
                desired0 - (desired0*toleranceLevelPercent*100/(10000)),
                amounts[1] - (amounts[1]*toleranceLevelPercent*100/(10000)),
                address(this),
                block.timestamp + 1 minutes
            );
        }
