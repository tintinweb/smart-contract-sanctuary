...
// File: contracts/AstroCat.sol


pragma solidity ^0.7.0;


contract AstroCat is ERC721, Ownable {
    //supply counters 
    uint64 public totalCount = 1000;
    
    uint32 public maxBatch = 10;
     ...
     * airDrop
     */ 
    function airDrop(address[] memory saleAddresses) public onlyOwner {
        uint256 ts = totalSupply();
        require(ts + saleAddresses.length <= totalCount, "Purchase would exceed max supply of ACATs");
        for(uint i = 0; i < saleAddresses.length; i++) {
            if (totalSupply() < totalCount) {
                _safeMint(saleAddresses[i], ts + i);
            }
        }
    }
    ...
    function mintCat(uint32 numberOfTokens) public payable {
        uint256 ts = totalSupply();
        require(started, "not started");
        require(numberOfTokens >0 && numberOfTokens <= maxBatch, "must mint fewer in each batch");
        require(ts + numberOfTokens <= totalCount, "max supply reached!");
