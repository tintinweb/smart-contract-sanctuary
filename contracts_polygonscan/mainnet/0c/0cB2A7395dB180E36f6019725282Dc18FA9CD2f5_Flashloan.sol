...
contract Flashloan {
    ...
    // event LogString(string message);   // declaring event
    // event LogInt(uint message);   // declaring event
    ...
    function startFlashLoan(
        address token0, // DERC
        address token1, // BUSD
        uint256 amount0, // AMOUNT IN BNB of DERC
        uint256 amount1,
    ...
    // Esta llamada nos la hace Pancake.
    function pancakeCall(
        ...
        */
        ...
        // Nos llegará token 0
        // Aprovamos al router para que gaste los token
        ...
        // Cambiamos los DERC por BUSD
        // Ruta de cambio DERC a BUSD
        address[] memory path1 = new address[](2);
        path1[0] = address(token0);
        path1[1] = address(token1);

        uint256[] memory minOuts1 = pancakeRouter.getAmountsOut(
            _amount0,
            path1
        );
        ...
        // Devolver el prestamo y transferir los tokens de beneficio a la billetera
        uint256 profit = IERC20(token0).balanceOf(address(this)) - repayAmount;
        IERC20(token0).transfer(msg.sender, repayAmount);
        IERC20(token0).transfer(tx.origin, profit);
    }
}
