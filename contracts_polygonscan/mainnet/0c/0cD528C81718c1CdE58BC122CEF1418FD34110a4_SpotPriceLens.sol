...
contract SpotPriceLens {
    ...
    function consultMetaStableSwap(
        address router,
        address metaPool,
        address basePool,
        address fromToken,
        ...
        uint8 metaIndex = IIronSwap(metaPool).getTokenIndex(fromToken);
        uint8 baseIndex = IIronSwap(basePool).getTokenIndex(toToken);
        uint256 tokenInDenom = 10**ERC20(fromToken).decimals();
        uint256 tokenOutDenom = 10**ERC20(toToken).decimals();
        uint256 dy = IIronSwapRouter(router).calculateSwapToBase(metaPool, basePool, metaIndex, baseIndex, tokenInDenom);
        return getChainlinkPrice(chainlink, (dy * PRECISION) / tokenOutDenom);
    }
