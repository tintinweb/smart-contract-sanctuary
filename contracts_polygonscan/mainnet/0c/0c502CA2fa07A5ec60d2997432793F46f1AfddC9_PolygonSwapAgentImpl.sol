...
library Address {
     ...
     * Requirements:
     *
     * - the calling contract must have an Polygon balance of at least `value`.
 ...
 * successful.
 * To use this library you can add a `using SafERC20 for IERC20;` statement to your contract,
...
// File: contracts/PolygonSwapAgentImpl.sol

pragma solidity 0.6.4;


contract PolygonSwapAgentImpl is Context {
    ...
    event SwapFilled(address indexed erc20Addr, bytes32 indexed opcTxHash, address indexed toAddress, uint256 amount);
    ...
    function registerSwapPairToOPC(address erc20Addr) external returns (bool) {
    ...
    function fillOPC2PolygonSwap(bytes32 opcTxHash, address erc20Addr, address toAddress, uint256 amount) onlyOwner external returns (bool) {
        ...
        emit SwapFilled(erc20Addr, opcTxHash, toAddress, amount);
        return true;
    }

    function swapPolygon2OPC(address erc20Addr, uint256 amount) payable external notContract returns (bool) {
        require(registeredERC20[erc20Addr], "not registered token");
