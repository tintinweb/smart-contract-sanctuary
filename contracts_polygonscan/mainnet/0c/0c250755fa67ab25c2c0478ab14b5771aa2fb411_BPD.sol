...
contract BPD is IBpd, Manageable, ExternallyCallable {
    ...
    function addBpdShares(
        ...
        uint256[2] memory bpdInterval = getBpdIntervalInternal(start, end);
        uint48[5] memory _bpdShares = bpdShares; // one read (SLOAD)

        uint48 shares0Dp = ((shares + 5e17) / 1e18).toUint48(); // adding 5e17 is equivalent to rounding instead of truncating
    ...
    function addBpdMaxShares(
        ...
        uint256[2] memory oldBpdInterval = getBpdIntervalInternal(oldStart, oldEnd);
        uint256[2] memory newBpdInterval = getBpdIntervalInternal(newStart, newEnd);
    ...
    function getBpdPoolsInternal() internal pure returns (uint8[5] memory) {
        return [50, 75, 100, 125, 150];
    }

    function getBPDPools() external pure returns (uint8[5] memory) {
        return getBpdPoolsInternal();
    }
    ...
    function getBpdAmount(
        ...
        uint256 bpdAmount;
        uint8[5] memory bpdPools = getBpdPoolsInternal();
        uint256[2] memory bpdInterval = getBpdIntervalInternal(start, end);
    ...
    function getBpdIntervalInternal(uint256 start, uint256 end)
        internal
        view
    ...
    function getBPDInterval(uint256 start, uint256 end) external view returns (uint256[2] memory) {
        return getBpdIntervalInternal(start, end);
    }

    function setBpdShares(uint48[5] calldata shares) external onlyManager {
        bpdShares = shares;
    }
    ...
    function initialize(address _manager, address _stakeManager) external initializer {
