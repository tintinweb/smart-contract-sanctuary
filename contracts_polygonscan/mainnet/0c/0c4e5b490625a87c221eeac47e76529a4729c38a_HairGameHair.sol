...
contract HairGameHair is VRFConsumerBase, Ownable, ReentrancyGuard {
    ...
    // Events
    event BetPlaced(uint indexed betId, address indexed gambler, uint amount, uint8 readibleBet, uint8 indexed modulo, uint8 rollUnder, uint40 mask);
    event BetSettled(uint indexed betId, address indexed gambler, uint amount, uint8 readibleBet, uint8 indexed modulo, uint40 mask, uint outcome, uint winAmount, uint rollReward);
    ...
    function settleBet(bytes32 requestId, uint randomNumber) internal nonReentrant {
        ...
        emit BetSettled(betId, gambler, amount, uint8(readibleBet), uint8(modulo), bet.mask, outcome, winAmount, rollReward);
    }
