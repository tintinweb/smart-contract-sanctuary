...
contract DEUS_MATIC_ZAPPER is Ownable {
    ...
    /* ========== PUBLIC FUNCTIONS ========== */
    function USDC2DEUS(ProxyInput memory proxyInput)
        internal
    ...
    function ERC202DEUS(ProxyInput memory proxyInput, address[] memory path)
        internal
        returns (uint256 deusAmount)
    {
        if (IERC20(path[0]).allowance(address(this), deiProxy) == 0) {
            IERC20(path[0]).approve(deiProxy, type(uint256).max);
        }
        uint256 deiAmount;
        if (path[0] == deiAddress) {
            deiAmount = proxyInput.amountIn;
        } else {
            deiAmount = IDEIProxy(deiProxy).ERC202DEI(proxyInput, path);
        }

        deusAmount = IUniswapV2Router02(uniswapRouter).swapExactTokensForTokens(
            deiAmount,
            1,
            dei2deusPath,
            address(this),
        ...
        require(
            proxyInput.minAmountOut <= deusAmount,
            "Multi Swap: Insufficient output amount"
        );
    }

    // Should be payable? (I don't think so)
    function Nativecoin2DEUS(
        ProxyInput memory proxyInput,
        address[] memory path
    ) internal returns (uint256 deusAmount) {
        uint256 deiAmount = IDEIProxy(deiProxy).Nativecoin2DEI{
            value: proxyInput.amountIn
        }(proxyInput, path);
    ...
    function buyDeusAndMatic(
        ProxyInput memory proxyInput,
        uint256 amountToDeus,
        uint256 amountToMatic,
        address[] memory toMaticPath,
        address[] memory toUSDCPath
        ...
        if (toUSDCPath[0] == usdcAddress) {
            ...
            if (deusAddress == pair.token0()) {
                ...
                token1Bought = IUniswapV2Router02(uniswapRouter)
                    .swapExactTokensForTokens(
                        ...
                        address(this),
                        deadline
                    )[toMaticPath.length - 1];
            } else {
                token1Bought = USDC2DEUS(proxyInput);
                token0Bought = IUniswapV2Router02(uniswapRouter)
                    .swapExactTokensForTokens(
                        amountToMatic,
                        1,
                        toMaticPath,
            ...
            } else if (toMaticPath[0] == deusAddress) {
                if (deusAddress == pair.token0()) {
                    token0Bought = amountToDeus;
                    token1Bought = IUniswapV2Router02(uniswapRouter)
                        .swapExactTokensForTokens(
                            amountToMatic,
                            1,
    ...
    function zapInNativecoin(
        ...
        uint256 amountToMatic = proxyInput.amountIn - amountToDeus;

        (uint256 token0Bought, uint256 token1Bought) = buyDeusAndMatic(
            proxyInput,
            amountToDeus,
            amountToMatic,
            toMaticPath,
    ...
    function zapInERC20(
        ...
        IERC20(toMaticPath[0]).safeTransferFrom(
            msg.sender,
            address(this),
            proxyInput.amountIn
        );
        ...
        if (toMaticPath[0] != usdcAddress) {
            ...
            if (
                IERC20(toMaticPath[0]).allowance(
                    address(this),
                    uniswapRouter
                ) == 0
            )
                IERC20(toMaticPath[0]).safeApprove(
                    uniswapRouter,
                    type(uint256).max
                );
        }
    ...
    {
        (amountOut, usdcForMintAmount, deusNeededAmount) = IDEIProxy(deiProxy)
            .getUSDC2DEIInputs(amountIn, deusPriceUSD, colPriceUSD);
        amountOut = IUniswapV2Router02(uniswapRouter).getAmountsOut(
            amountOut,
            dei2deusPath
        )[1];
    }

    function getERC202DEUSInputs(
        uint256 amountIn,
        uint256 deusPriceUSD,
        uint256 colPriceUSD,
        address[] memory path
        ...
        )
    {
        if (path[0] == deiAddress) {
            ...
            (amountOut, usdcForMintAmount, deusNeededAmount) = IDEIProxy(
                deiProxy
            ).getERC202DEIInputs(amountIn, deusPriceUSD, colPriceUSD, path);
        }
        amountOut = IUniswapV2Router02(uniswapRouter).getAmountsOut(
            amountOut,
            dei2deusPath
    ...
    {
        IUniwapV2Pair pair = IUniwapV2Pair(pairAddress);
        uint256 reservedDeus;
        uint256 reservedMatic;
        if (deusAddress == pair.token0()) {
        ...
        if (
            input.toUSDCPath.length > 0 && input.toUSDCPath[0] == maticAddress
        ) {
            (swapAmount, ) = getSwapAmountMatic(
                input,
                reservedDeus,
                reservedMatic
            );
        } else {
            (swapAmount, ) = getSwapAmountERC20(
                input,
                reservedDeus,
                reservedMatic
            );
        ...
        uint256 deusAmount;
        if (
            input.toMaticPath.length > 0 && input.toMaticPath[0] == usdcAddress
        ) {
            (
                deusAmount,
                usdcForMintAmount,
            ...
            input.toMaticPath.length > 0 && input.toMaticPath[0] == deusAddress
        ) {
            deusAmount = swapAmount;
        } else {
            (
                deusAmount,
                usdcForMintAmount,
        ...
        percentage = (deusAmount * 1e6) / (reservedDeus + deusAmount);
        lp = (deusAmount * pair.totalSupply()) / reservedDeus;
    }

    function getSwapAmountERC20(
        ZapperInput memory input,
        uint256 reservedDeus,
        uint256 reservedMatic
    ) public view returns (uint256, uint256) {
        uint256 decimals = MyIERC20(input.toMaticPath[0]).decimals();
        uint256 remain = input.amountIn;
        remain = remain * 10**(18 - decimals);
        if (input.toMaticPath[0] == usdcAddress) {
            input.maxRemainAmount = 1e16;
        } else {
            input.maxRemainAmount =
                (IUniswapV2Router02(uniswapRouter).getAmountsIn(
                    1e6,
                    input.toUSDCPath
                )[0] * 10**(18 - decimals)) /
                1e2;
        ...
        for (uint256 i = 0; i < input.iterations; i++) {
            ...
            if (input.toMaticPath[0] == usdcAddress) {
                (y, , ) = getUSDC2DEUSInputs(
                    x / 1e12,
                    input.deusPriceUSD,
                    input.collateralPrice
                );
            } else if (input.toMaticPath[0] == deusAddress) {
                y = x;
            } else {
                (y, , ) = getERC202DEUSInputs(
                    x / 10**(18 - decimals),
                    input.deusPriceUSD,
                    input.collateralPrice,
                    input.toUSDCPath
            ...
            uint256 neededCoinForBuyMatic = IUniswapV2Router02(uniswapRouter)
                .getAmountsIn(
                    (y * reservedMatic) / reservedDeus,
                    input.toMaticPath
                )[0];
          neededCoinForBuyMatic = neededCoinForBuyMatic * 10**(18 - decimals);
          ...
          while (neededCoinForBuyMatic + x > remain) {
                ...
                neededCoinForBuyMatic = IUniswapV2Router02(uniswapRouter)
                    .getAmountsIn(
                        (y * reservedMatic) / reservedDeus,
                        input.toMaticPath
                    )[0];

                neededCoinForBuyMatic =
                    neededCoinForBuyMatic *
                    10**(18 - decimals);
            }
        ...
        return (
            amountToSwapToDeus / 10**(18 - decimals),
            input.amountIn / 10**(18 - decimals)
        );
    }

    function getSwapAmountMatic(
        ZapperInput memory input,
        uint256 reservedDeus,
        uint256 reservedMatic
    ) public view returns (uint256, uint256) {
    ...
    event Temp(uint256 key, uint256 value);
}
