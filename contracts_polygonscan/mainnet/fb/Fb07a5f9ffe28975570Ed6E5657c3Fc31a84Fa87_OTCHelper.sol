...
import "../libs/DarksideToolBox.sol";
...
contract OTCHelper is Ownable, ReentrancyGuard {
    ...
    // The DARKSIDE TOKEN!
    address public immutable darkside;
    // Darksides trusty toolbelt.
    ...
    // user gets a usernameId once they move 10kusdc.
    uint256 public constant usernameUSDCThreshold = 10 ** 10;
    ...
    event FeeSwappedToMatic(address indexed tokenFeeAddress, uint256 tokenQuantity, uint256 maticRecieved);
    ...
    constructor(
        ...
        IUniswapV2Router02 _darksideSwapRouter
    ) public {
    ...
    function swapToWETHAndDistribute(address token) external onlyOwner {
        ...
        if (maticBalance > 0)
            IWETH(wmaticAddress).deposit{value:maticBalance}();

        if (token != wmaticAddress) {
            uint256 amountToSwap = IERC20(token).balanceOf(address(this));

            if (amountToSwap > 0) {
                uint256 wmaticBeforeBalance = wmaticToken.balanceOf(address(this));
                ...
                IERC20(token).approve(address(darksideSwapRouter), amountToSwap);

                // put in a try catch in case a matic pair doesn't exist for the token yet.
                try
                    // make the swap
                ...
                {
                    /* suceeded */
                    emit FeeSwappedToMatic(token, amountToSwap, wmaticToken.balanceOf(address(this)) - wmaticBeforeBalance);
                } catch { /* failed, but we avoided reverting */ }
            }
        }

        uint256 wmaticProfit = wmaticToken.balanceOf(address(this));

        if (wmaticProfit >= 10 ** 14) {
            uint256 CZDiamondFee = wmaticProfit / 2;
            wmaticToken.transfer(address(CZDiamond), CZDiamondFee);
            wmaticToken.transfer(address(darkside), wmaticProfit - CZDiamondFee);
        }
    }
    ...
    function updateUsernameIdMappings(uint256 dealId, address buyerAccount, uint256 buyingAmount) external onlyOwner {
        ...
        uint256 usdcTradeValue = darksideToolBox.getTokenUSDCValue(buyingAmount, wantedTokenAddress, 0, true, usdcCurrencyAddress);

        userVolumeMap[sellerAccount] = userVolumeMap[sellerAccount] + usdcTradeValue;
        userVolumeMap[buyerAccount] = userVolumeMap[buyerAccount] + usdcTradeValue;
        ...
        if (usernameIdMap[sellerAccount] == 0 && userVolumeMap[sellerAccount] > usernameUSDCThreshold) {
        ...
        if (usernameIdMap[buyerAccount] == 0 && userVolumeMap[buyerAccount] > usernameUSDCThreshold) {
            usernameIdMapLength = usernameIdMapLength + 1;
    ...
    function multiBuy(uint256[] calldata dealIds, uint256[] calldata amounts, bool isByWantedTokenAmounts, bool persistOnFail) external payable nonReentrant buyerAddressProvider(msg.sender) {
        uint256 maticExecutionPrice = otcEngine.maticDealCreationFee()/10;
        // We only require enough matic for 1 deal excution, as a buyer may want to try more deals than they have gas for.
        require(msg.value >= maticExecutionPrice, "not enough matic for even 1 deal execution!");
        require(dealIds.length > 0, "no deals requests");
        require(dealIds.length == amounts.length, "unequal array lengths");

        uint256 initialMaticBeforeUser = address(this).balance - msg.value;
        ...
        for (uint256 i = 0;i<dealIds.length;i++) {
            ...
            if (isByWantedTokenAmounts) {
                ...
                try
                    otcEngine.executeDealExactBuyingAmount{value: maticExecutionPrice}(dealId, buyingAmount)
                {
                ...
                try
                    otcEngine.executeDealExactSaleAmount{value: maticExecutionPrice}(dealId, saleAmount)
                {
                    numberOfSucceededBuys = numberOfSucceededBuys + 1;
            ...
            if (address(this).balance < initialMaticBeforeUser + maticExecutionPrice)
                break;
        }
        ...
        // Refunding any unused gas. No need to check the return status of the matic refund at the cost of the whole multi-buy.
        if (address(this).balance > initialMaticBeforeUser)
            payable(msg.sender).call{value: address(this).balance - initialMaticBeforeUser}("");
    }

    // Fetch tokens we can't take fees from automatically, no matic pair, is an LP token etc...
...
contract OTCEngine is Ownable, ReentrancyGuard {
    ...
    // default % fee is 0.12% = 12/10,000
    uint256 public depostFeeBP = 12;
    uint256 public maticDealCreationFee = 0.01 ether;
    ...
    event DealUpdateTrueMinMax(
        uint256 indexed dealId,
        uint256 indexed trueMin,
        uint256 indexed trueMax
    );
    ...
    event SetMaticDealCreationFee(
        uint256 newMaticDealCreationFee
    );
    ...
    function emitTrueMinMax(uint256 dealId) internal {
        uint256 calcAbsMinFromPerc = (globalDealInfos[dealId].saleBalance * globalDealInfos[dealId].saleMinPercentage) / 1e12;
        uint256 calcAbsMaxFromPerc = (globalDealInfos[dealId].saleBalance * globalDealInfos[dealId].saleMaxPercentage) / 1e12;

        uint256 trueMin = globalDealInfos[dealId].saleMinAbsolute < calcAbsMinFromPerc ? calcAbsMinFromPerc : globalDealInfos[dealId].saleMinAbsolute;
        uint256 trueMax = globalDealInfos[dealId].saleMaxAbsolute > calcAbsMaxFromPerc ? calcAbsMaxFromPerc : globalDealInfos[dealId].saleMaxAbsolute;

        emit DealUpdateTrueMinMax(dealId, trueMin, trueMax);
    }
    ...
    function addDeal(address _saleTokenAddress, address _wantedTokenAddress, uint256 _saleBalance, uint256 _wantedBalance,
        ...
        require(depositedAmount > 0 &&
                    darksideToolBox.getTokenUSDCValue(depositedAmount, _saleTokenAddress, 0, true, usdcCurrencyAddress) >= 1 * 1e6 /* 1usdc minimum */,
                        "deal too small");

        otcHelper.updateTokenBalances(_saleTokenAddress, true, depositedAmount);

        {
            require(msg.value >= maticDealCreationFee, "not enough matic provided");
    ...
    function executeDealExactSaleAmount(uint256 dealId, uint256 saleAmount) public payable nonReentrant {
        require(msg.value >= (maticDealCreationFee / 10), "not enough matic provided");
    ...
    // To receive MATIC from user for deal creation, top ups, and executions.
    receive() external payable {}
    ...
    // Update the matic deal creation fee
    function setMaticDealCreationFee(uint256 _maticDealCreationFee) external nonReentrant onlyOwner {
        require(_maticDealCreationFee <= 0.1 ether, "matic fee is max 0.1");
        maticDealCreationFee = _maticDealCreationFee;

        emit SetMaticDealCreationFee(maticDealCreationFee);
    }
}
