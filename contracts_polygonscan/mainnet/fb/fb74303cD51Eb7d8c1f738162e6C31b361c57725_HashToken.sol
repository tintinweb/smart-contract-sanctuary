...
interface iJaegerOwls {
    function balanceOwls(address owner) external view returns(uint256);
}

contract HashToken is ERC20, Ownable {

    iJaegerOwls public JaegerOwls;
    ...
    constructor(address JaegerOwlsAddress) ERC20("HashTokens", "#Ha$h") {
        JaegerOwls = iJaegerOwls(JaegerOwlsAddress);
        START = block.timestamp;
    }

    function updateReward(address from, address to) external {
        require(msg.sender == address(JaegerOwls));
        if(from != address(0)){
    ...
    function burn(address user, uint256 amount) external {
        require(allowedAddresses[msg.sender] || msg.sender == address(JaegerOwls), "Address does not have permission to burn");
        _burn(user, amount);
    }
    ...
    function getPendingReward(address user) internal view returns(uint256) {
        return JaegerOwls.balanceOwls(user) * BASE_RATE * (block.timestamp - (lastUpdate[user] >= START ? lastUpdate[user] : START)) / 86400;
