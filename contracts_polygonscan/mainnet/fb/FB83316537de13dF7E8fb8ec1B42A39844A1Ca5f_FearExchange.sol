 ...
 */
contract FearExchange is AccessControl, Pausable, ReentrancyGuard, EmergencyWithdrawer, FeeTaker, ERC1155Holder {
    using SafeMath for uint256;
    using EnumerableSet for EnumerableSet.UintSet;
    using EnumerableSet for EnumerableSet.AddressSet;

    /**
     * @dev Emitted when NFT exchange rate is set.
     */
    event ExchangeRateSet(uint256 id, address[] assets, uint256[] prices);
    /**
    ...
    /**
     * @dev Emitted when NFT is removed from exchange.
     */
    event NftRemoved(uint256 id);
    ...
    */
    ...
    function _clearPrices(uint256 id) private {
        for (uint i = 0; i < currencyAssets.length(); i++)
            exchangeRates[id][currencyAssets.at(i)] = 0;
    }
    ...
    * Emits {NftBought} event on success.
    */
    ...
    * Emits {NftSold} event on success.
    */
    ...
    /**
    * @dev Adds tradable NFT with exchange rate. If tradable NFT is already added, sets new exchange rate.
    * Exchange rate can be composite containing several assets addresses with according prices for each.
    * @param id NFT id.
    ...
    * - Prices array must not contain zero.
    * Emits {ExchangeRateSet} event on success.
    */
    function addTradableNft(uint256 id, address[] calldata assets, uint256[] calldata prices) external onlyRole(MANAGER_ROLE) {
        ...
        _clearPrices(id); // clearing old prices before adding new (to be sure no old assets prices left)
        ...
        emit ExchangeRateSet(id, assets, prices);
    }

    /**
    * @dev Removes tradable NFT from exchange.
    * @param id NFT id.
    * Requirements:
    * - Caller must have 'MANAGER_ROLE';
    * - NFT with this id must be added to tradable NFTs.
    * Emits {NftRemoved} event on success.
    */
    function removeTradableNft(uint256 id) external onlyRole(MANAGER_ROLE) {
        require(tradableNfts.contains(id), "Tradable NFTs collection doesn't contain this id");
        tradableNfts.remove(id);
        _clearPrices(id);
