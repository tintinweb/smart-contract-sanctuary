...
contract AutoStakingPoolv2 is ReentrancyGuard, Ownable {
    ...
    bool public enabledBurn = true;
    address public penaltyRecipient;
    uint256 public burnAlloc = 10000;
    ...
    constructor (
        ...
        rewardDistributors[address(WMATIC)][_stakingToken] = true; // Will receive trading tax
    }
    ...
    function setBurnAllocation(uint _alloc) external onlyOwner {
        require (_alloc <= 10000, "!valid");
        burnAlloc = _alloc;
    }
    ...
    function setBurnMode(bool _flag) external onlyOwner {
        enabledBurn = _flag;
    }

    function enableStake(bool _flag) external onlyOwner {
        enabledStake = _flag;
    }
    ...
    function stake(uint256 amount, bool lock) external nonReentrant updateReward(msg.sender) {
        require(enabledStake, "!available");
    ...
    function withdraw(uint256 amount) public nonReentrant updateReward(msg.sender) {
        ...
        if (penaltyAmount > 0) {
            enabledBurn ? _burn(penaltyAmount) : _notifyReward(address(stakingToken), penaltyAmount);
        }
    ...
    function _burn(uint256 _amount) internal {
        if (_amount == 0 || stakingToken.balanceOf(address(this)) < _amount) return;

        uint burnAmount = _amount.mul(burnAlloc).div(10000);
        if (burnAmount < _amount) {
            stakingToken.safeTransfer(penaltyRecipient, _amount.sub(burnAmount));
        }
        stakingToken.safeTransfer(burnAddress, burnAmount);
    }
