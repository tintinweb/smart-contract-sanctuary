...
contract PolyStake_2 {
	...
	function invest(address referrer, uint8 plan, bool isSuper) public payable {
		...
		(uint256 percent, uint256 profit, uint256 finish) = getResult(plan, investAmount, isSuper);
		user.deposits.push(Deposit(plan, percent, investAmount, profit, block.timestamp, finish, isSuper));
	...
	function getPercent(uint8 plan, bool isSuper) public view returns (uint256) {
		if (block.timestamp > startTime && isSuper == true) {
	...
	function getResult(uint8 plan, uint256 deposit, bool isSuper) public view returns (uint256 percent, uint256 profit, uint256 finish) {
		percent = getPercent(plan, isSuper);
	...
	function getUserDepositInfo(address userAddress, uint256 index) public view returns(uint8 plan, uint256 percent, uint256 amount, uint256 profit, uint256 start, uint256 finish, bool isSuper) {
	    User storage user = users[userAddress];
		...
		isSuper = user.deposits[index].isSuper;
	}
