...
contract PolyNFT is BNFT, IPolyNFT, ReentrancyGuard {
    using Address for address;

    uint256 private _totalBurned;

    address private _polyStats;

    constructor() BNFT(_msgSender(), "https://api.polydex.fi/api/NFTs", "Poly NFT", "PolyDex") {
    }

    function setPolyStats(address newPolyStats) external onlyAdmin {
        _polyStats = newPolyStats;
    }
    ...
    function getInfos(uint256 tokenId) external override view returns (IPolyStats.PolyInfo memory polyStats) {
        if (_polyStats != address(0)) {
            (polyStats) = IPolyStats(_polyStats).stats(tokenId);
        }
    }
