...
contract DumplingRouter02 is IUniswapV2Router01 {
    using SafeMathUniswap for uint;
	...
	IDumplingRouter(router).swapExactTokensForTokens(amountIn,amountOutMin,path,to,deadline);
    }
    function swapTokensForExactTokens(
        uint amountOut,
        uint amountInMax,
	...
	IDumplingRouter(router).swapTokensForExactTokens(amountOut,amountInMax,path,to,deadline);
    }
    function swapExactETHForTokens(uint amountOutMin, address[] calldata path, address to, uint deadline)
        external
        virtual
	...
	IDumplingRouter(router).swapExactTokensForTokens(amounts[0],amounts[amounts.length-1],path,to,deadline);
	...
	IDumplingRouter(router).swapTokensForExactTokens(amounts[amounts.length-1],amounts[0],path,to,deadline);
