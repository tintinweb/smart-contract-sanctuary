...
contract LPStakingRewards is ILPStakingRewards, BaseRewards {
    ...
    event LPSet(address indexed lpToken, uint256 allocPoint, uint256 totalAllocPoint);
    event StakeBalanceUpdate(address indexed user, address indexed lpToken, uint256 blockNumber, uint256 amount);
    event TotalStakesChanged(address indexed lpToken, uint256 totalStakes);
        ...
        isSupportedLP(lpToken)
    {
        ...
        emit StakeBalanceUpdate(_msgSender(), lpToken, block.number, userInfo[lpToken][user].amount);
        emit TotalStakesChanged(lpToken, poolInfo[lpToken].totalStakes);
    ...
    function updateRewardInternal(address lpToken) internal {
        ...
        pool.lastUpdateBlock = lastBlockApplicable(lpToken);
    }
    ...
    function totalClaimableRewardOf(address user) external view override returns (uint256 totalClaimable) {
        totalClaimable = 0;
        for (uint256 i = 0; i < poolList.length; i++) {
            PoolInfo storage pool = poolInfo[poolList[i]];
            if (block.number >= pool.lockEndBlock) totalClaimable += _earned(user, poolList[i]);
        }
    }
        ...
        returns (uint256)
    {
        ...
        uint256 diff;
        if (lastBlockApplicable(lpToken) > poolInfo[lpToken].lastUpdateBlock) {
            diff = lastBlockApplicable(lpToken) - poolInfo[lpToken].lastUpdateBlock;
        }
        return
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.rewardsEndBlock) return 0;
    ...
    function claimRewardInternal(address user, address lpToken) internal isSupportedLP(lpToken) {
        PoolInfo storage pool = poolInfo[lpToken];
        if (block.number > pool.lockEndBlock) {
            updateReward(user, lpToken);
        ...
        notSupportedLP(lpToken)
    {
        ...
        emit LPSet(lpToken, allocPoint, totalAllocPoint);
    }
    ...
    function isLocked(address lpToken) external view override returns (bool) {
        return block.number <= poolInfo[lpToken].lockEndBlock;
    }

    function setTimeLock(address lpToken, uint256 lockDuration)
        external
        override
        onlyRole(DEFAULT_ADMIN_ROLE)
        whenNotPaused
    {
        poolInfo[lpToken].lockEndBlock = block.number + lockDuration;
    }

    function setTimeLockEndBlock(address lpToken, uint256 lockEndBlock)
        external
        override
        onlyRole(DEFAULT_ADMIN_ROLE)
        whenNotPaused
    {
        poolInfo[lpToken].lockEndBlock = lockEndBlock;
    }

    function setTimeLockForAllLPTokens(uint256 lockDuration)
        external
        override
        onlyRole(DEFAULT_ADMIN_ROLE)
        whenNotPaused
    {
        for (uint256 i = 0; i < poolList.length; i++) {
            poolInfo[poolList[i]].lockEndBlock = block.number + lockDuration;
        }
    }

    function setTimeLockEndBlockForAllLPTokens(uint256 lockEndBlock)
        external
        override
        onlyRole(DEFAULT_ADMIN_ROLE)
        whenNotPaused
    {
        for (uint256 i = 0; i < poolList.length; i++) {
            poolInfo[poolList[i]].lockEndBlock = lockEndBlock;
        }
    }

    function earlyUnlock(address lpToken) external override onlyRole(DEFAULT_ADMIN_ROLE) whenNotPaused {
        poolInfo[lpToken].lockEndBlock = block.number;
    }

    function getLockEndBlock(address lpToken) external view override returns (uint256) {
        return poolInfo[lpToken].lockEndBlock;
    }
