...
contract BridgePool is IBridgePool {
    ...
    */
    ...
    function deposit(IERC20 token, uint amount, uint8 to, bool bonus, bytes calldata receiver) override external payable nonReentrant() {
        ...
        require(address(token) != address(0) && amount > 0 && receiver.length > 0, "invalid input");
        ...
        emit Deposited(msg.sender, address(token), to, amount, bonus, receiver);
    }
    ...
    function withdraw(Withdraw[] memory ws) override external nonReentrant() {
        ...
        for (uint i = 0; i < ws.length; i++) {
            ...
            if (address(w.token) == address(1)) {
                ...
                (bool success, ) = w.receiver.call{value: w.amount}("");
                ...
                w.token.safeTransfer(w.receiver, w.amount);
            }

            if (w.bonus > 0) {
                // may fail on contracts
                w.receiver.call{value: w.bonus}("");
            }

            emit Withdrawn(w.id, address(w.token), w.receiver, w.amount);
        }
    }
...
interface IBridgePool {
    ...
    event Deposited(address indexed sender, address indexed token, uint8 indexed to, uint amount, bool bonus, bytes receiver);
    event Withdrawn(bytes32 indexed id, address indexed token, address indexed receiver, uint amount);

    function deposit(IERC20 token, uint amount, uint8 to, bool bonus, bytes calldata receiver ) external payable;
