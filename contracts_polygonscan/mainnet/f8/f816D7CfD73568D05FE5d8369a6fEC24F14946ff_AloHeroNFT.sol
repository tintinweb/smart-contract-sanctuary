...
interface ITinyHeroNFT is IERC721 {
    function getStats(uint256 tokenId) external view returns (uint16[] memory pvpStats, uint16 luckStat);
...
interface ITinyHeroStats {
     ...
     */
    function stats(uint256 tokenId) external view returns (uint16[] memory pvpStats, uint16 luckStat);
...
contract AloHeroNFT is BNFT, ITinyHeroNFT, ReentrancyGuard {
    using Address for address;
    ...
    address private _tinyStats;

    constructor() BNFT(_msgSender(), "https://stg-api.tinyhero.com/v1/nft/", "alohero.io NFT", "AloHero") {
    }

    function setTinyStats(address newTinyStats) external onlyAdmin {
        _tinyStats = newTinyStats;
    }
    ...
    function getStats(uint256 tokenId) external override view returns (uint16[] memory pvpStats, uint16 luckStat) {
        if (_tinyStats != address(0)) {
            (pvpStats, luckStat) = ITinyHeroStats(_tinyStats).stats(tokenId);
        }
    }

    function exists(uint256 tokenId) external override view virtual returns (bool) {
