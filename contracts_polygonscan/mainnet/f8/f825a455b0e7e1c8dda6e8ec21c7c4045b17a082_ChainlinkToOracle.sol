...
contract ChainlinkToOracle {
    ...
    function getPrice(address, address) external view returns (uint price, uint lastUpdate){
      ...
      uint fPrice = usdc > fToken ? uint(answer)*1e18/(10**oracleDecimals)*(10**decimalsusdc)/(10**decimalsFToken) : 1e36/(uint(answer)*1e18/(10**oracleDecimals)*(10**decimalsusdc)/(10**decimalsFToken));
      uint rTime = time;
