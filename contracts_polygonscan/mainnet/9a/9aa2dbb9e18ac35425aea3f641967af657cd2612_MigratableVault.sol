...
abstract contract BaseStrategyStakingRewards is BaseStrategy {
    ...
    function _notifyJar(uint256 _amount) internal {
        IJar(jar).notifyReward(harvestedToken, _amount);
    }
    ...
    function migrate(address newStakingContract) external onlyOwner {
        //triggers error
        //require(IStakingRewards(newStakingContract).stakingToken() == newStakingContract, "newStakingContract.stakingToken() != newStakingContract");
...
abstract contract BaseStrategyOtherPair is BaseStrategyStakingRewards {
    ...
    //uint256 public constant keep = 200; //2% dev fee
    ...
    /*function getFee() public view returns (uint256) {
        if(IERCFund(strategist).feeShareEnabled()) {
            return IERCFund(strategist).getFee();
        }
        return keep;
    }*/
    ...
    function harvest() public override {
        ...
        if (aBalance > 0 && bBalance > 0) {
            ...
            // Donates DUST
            //IERC20(tokenA).safeTransfer(strategist, IERC20(tokenA).balanceOf(address(this)));
            //IERC20(tokenB).safeTransfer(strategist, IERC20(tokenB).balanceOf(address(this)));
        }
...
abstract contract VaultBase is ERC20, Ownable {
    using SafeERC20 for IERC20;
    using Address for address;
    using SafeMath for uint256;
    ...
    // Info of each user
    struct UserInfo {
        uint256 staked; // How many tokens the user has staked
    ...
    constructor(IStrategy _strategy, address _minter, string memory name, string memory symbol)
        public
        ERC20(name, symbol)
    {
    ...
    function getPendingReward(address _user) public view returns (uint256) {
        return userInfo[_user].staked.mul(accRewardPerShare).div(1e12).sub(userInfo[_user].rewardDebt);
    }
    ...
    function deposit(uint256 _amount) public {
        ...
        user.staked = user.staked.add(shares);
        user.rewardDebt = user.staked.mul(accRewardPerShare).div(1e12);
    ...
    function withdraw(uint256 _shares) public {
        ...
        //This will cause integer underflow if the user got vault tokens from another address, use emergencyWithdraw() if that is the case
        UserInfo storage user = userInfo[msg.sender];
        totalShares = totalShares.sub(_shares);
        user.staked = user.staked.sub(_shares);
        user.rewardDebt = user.staked.mul(accRewardPerShare).div(1e12);
    ...
    function _claimReward(address _user) internal {
        ...
        if (user.staked > 0) {
            uint256 pendingReward = user.staked.mul(accRewardPerShare).div(1e12).sub(user.rewardDebt);
            if (pendingReward > 0) {
...
//Used for Quickswap's staking reward contracts, which migrate each week
contract MigratableVault is VaultBase {

    uint256 public migrationDelayTime = 0;
    ...
    //Can only be called 8 hrs after the migration was executed, which allows users to verify that the vault migrated to the correct contract
    //The timelock cannot be initiated before the migration, which addresses issue 3.3 in the audit
    //In addition, certain highly trusted members of the community have the ability to pause the migration
    function restakeAll() public onlyOwner {
        uint256 lastTimeMigrated = IStrategy(strategy).getLastTimeMigrated();
        require(lastTimeMigrated + 8 hours < now, "wait 8 hrs");
        require(migrationDelayTime + 24 hours < now, "admin delayed");
    ...
    address internal qsDeployer = 0x476307DaC3FD170166e007FCaA14F0A129721463;
    address internal fraxDeployer = 0xa448833bEcE66fD8803ac0c390936C79b5FD6eDf;

    function delayMigration() public {
        require(msg.sender == qsDeployer || msg.sender == fraxDeployer || admins[msg.sender], "Not authorized");
        migrationDelayTime = now;
    }
...
contract StrategyDeployer is Ownable {
    ...
    address FUND = 0xC9E908D24f13c18A0ab803A4ACF469B37c1761e8;
    address MINTER = 0x7A82522742C768358453c11B9211E68160793921;
    ...
    function deploy(address rewards, address lp, address tokenA, address tokenB, string memory _pair_name) public onlyOwner {
        ...
        address jar = address(new MigratableVault(IStrategy(strategy), MINTER));
        StrategyOtherPair(strategy).setJar(jar);
        IMinter(MINTER).setMinter(jar, true);
