...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    function pendingJaguar(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && pool.lpSupply != 0 && totalAllocPoint > 0) {
            ...
            accJaguarPerShare = accJaguarPerShare.add(jaguarReward.mul(1e12).div(pool.lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 jaguarReward = multiplier.mul(JaguarPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        jaguar.mint(devaddr, jaguarReward.div(10));
        jaguar.mint(address(this), jaguarReward);
        pool.accJaguarPerShare = pool.accJaguarPerShare.add(jaguarReward.mul(1e12).div(pool.lpSupply));
        pool.lastRewardBlock = block.number;
    }
