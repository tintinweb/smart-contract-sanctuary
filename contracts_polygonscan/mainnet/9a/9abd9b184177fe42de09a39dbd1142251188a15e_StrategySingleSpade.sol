...
// File: Vault\libs\ISpade.sol


// For interacting with Spade masterchef
interface ISpade {
    // Transfer want tokens vault -> Spade masterchef
...
abstract contract StrategyFeesBase is Ownable, ReentrancyGuard, Pausable {
    ...
    function buyBack(address _earnedAddress) internal {
        ...
        if (buyBackRate > 0 && earnedAmt > 0) {
			...
			//Buy GIVE using quick router. Because our main liquidity is with quickswap and uniRouter 
...
abstract contract StrategyBase is StrategyFeesBase {
    ...
    address internal constant referralAddress = 0x0000000000000000000000000000000000000000;
    ...
    uint256 public compoundCycle = 300; // was uint256(-1), now 5 minutes; 
...
// File: Vault\StrategySingleSpade.sol

contract StrategySingleSpade is StrategySingleBase {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    ...
    constructor(
        ...
        earnedToWmaticPath = [0xf5EA626334037a2cf0155D49eA6462fDdC6Eff19, address(0x000d500b1d8e8ef31e21c99d1db9a6444d3adf1270)];        
        earnedToUsdcPath = [0xf5EA626334037a2cf0155D49eA6462fDdC6Eff19, address(0x002791bca1f2de4661ed88a30c99a7a9449aa84174)];
        earnedToGivePath = [0xf5EA626334037a2cf0155D49eA6462fDdC6Eff19, address(0x000d500b1d8e8ef31e21c99d1db9a6444d3adf1270), 0x9Bbcda2606e616659b118399A2823E8a392f55DA];
        earnedToWantPath= [0xf5EA626334037a2cf0155D49eA6462fDdC6Eff19, address(0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174)];
        wantToEarnedPath = [0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174, address(0xf5EA626334037a2cf0155D49eA6462fDdC6Eff19)];
    ...
    function stake(uint256 _wantAmount)  internal override virtual{
        ISpade(masterChef).deposit(pid, _wantAmount, referralAddress);
    }

    function harvest() internal virtual override{
        //Spade has no harvest function but harvest if you try to deposit 0
        ISpade(masterChef).withdraw(pid, 0);
    }

    function unstake(uint256 _amount) internal virtual override{
        ISpade(masterChef).withdraw(pid, _amount);
    }    

    function emergencyWithdraw() internal virtual override onlyGov {
        ISpade(masterChef).emergencyWithdraw(pid);
    }
    
    function vaultSharesTotal() public virtual override view returns (uint256) {
        (uint256 amount, ) = ISpade(masterChef).userInfo(pid, address(this));
        return amount;
    }
