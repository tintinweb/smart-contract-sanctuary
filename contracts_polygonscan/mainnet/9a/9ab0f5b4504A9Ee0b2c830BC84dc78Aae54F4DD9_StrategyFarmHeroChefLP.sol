...
// File contracts/BIFI/interfaces/common/IMultiFeeDistribution.sol
...
// File contracts/BIFI/interfaces/farmhero/IHeroFarmV3.sol
...
// File contracts/BIFI/strategies/FarmHero/StrategyFarmHeroChefLP.sol

pragma solidity ^0.6.0;
contract StrategyFarmHeroChefLP is StratManager, FeeManager {
    ...
    string public referName;
    address public feeDistribution = address(0x047e7D6E8f4b6dEBa0537A7c7e852C4272981075);
    address public wmaticUsdcLpPair = address(0x6e7a5FAFcec6BB1e78bAE2A1F0B612012BF14827);
    ...
    function deposit() public whenNotPaused {
        ...
        if (wantBal > 0) {
            IHeroFarmV3(chef).deposit(poolId, wantBal, referName);
        }
    }
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            IHeroFarmV3(chef).withdraw(poolId, _amount.sub(wantBal));
    ...
    /**
     * @dev Converts additional rewards tokens to {honor}
     * It swaps {wmatic} for {honor}
     */
    function convertAdditionalRewards() internal {
        uint256 nativeBal = IERC20(native).balanceOf(address(this));
        if (nativeBal > 0) {
            (uint256 wmaticReserve, uint256 usdcReserve,) = IUniswapV2Pair(wmaticUsdcLpPair).getReserves();
            uint256 estimateOut = IUniswapRouterETH(unirouter).getAmountOut(nativeBal, wmaticReserve, usdcReserve);
            if (estimateOut > 0) {
                IUniswapRouterETH(unirouter).swapExactTokensForTokens(nativeBal, 0, nativeToOutputRoute, address(this), now);
            }
        }
    }
    ...
    function balanceOfPool() public view returns (uint256) {
        (uint256 _amount, , ,) = IHeroFarmV3(chef).userInfo(poolId, address(this));
        return _amount;
    }
    ...
    function retireStrat() external {
        ...
        IHeroFarmV3(chef).emergencyWithdraw(poolId);
    ...
    function setReferName(string memory _referName) external onlyStrategist {
        referName = _referName;
    }
}