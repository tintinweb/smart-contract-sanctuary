...
contract BaoElder is ERC721Enumerable, MerkleWhitelist, Ownable {
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        function mintBaoG(uint256 _amount) external payable {
                ...
                require(supply + _amount <= MAX_SUPPLY - reserved, "Can't mint over limit");

                for (uint256 i = 0; i < _amount; i++)
                        _mintNFT(supply + i);
        }

                function mintBaoGWithSignature(uint256 _index, address _account, uint256 _amount, bytes32[] memory _proof) public payable {
        ...
        function _mintNFT(uint256 _id) internal {
                for(uint256 i = 0; i < 4; i++) {
                        ...
                        emit BaoGMinted(_id + i, msg.sender);
                }
        }
