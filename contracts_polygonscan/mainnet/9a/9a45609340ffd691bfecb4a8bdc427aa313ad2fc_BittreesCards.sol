...
contract BittreesCards is ERC1155(""), ReentrancyGuard, Pausable, Ownable {
    ...
    event CreatedCard(uint256 tokenId);
    event LockedCard(uint256 tokenId, bool locked);
    ...
    function getLocked(uint256 tokenId) external view returns (bool) {
        require(bytes(_hashes[tokenId]).length > 0, "Invalid token id");
    ...
    function mint(uint256 tokenId, uint256 count) external payable nonReentrant whenNotPaused {
        ...
        require(_minters[_msgSender()] || !_lockedTokens[tokenId], "Is locked");
        require(msg.value == 0, "Is free");
    ...
    function createCard(string memory ipfsHash) external nonReentrant minterOrOwner {
        uint256 tokenId = ++_totalTokens;
        require(bytes(ipfsHash).length > 0, "Need ipfs hash");

        _hashes[tokenId] = ipfsHash;

        emit CreatedCard(tokenId);
    }
    /** Set the locked state for a token. */
    function setLocked(uint256 tokenId, bool locked) external nonReentrant minterOrOwner {
        require(bytes(_hashes[tokenId]).length > 0, "Invalid token id");

        _lockedTokens[tokenId] = locked;

        emit LockedCard(tokenId, locked);
    }
