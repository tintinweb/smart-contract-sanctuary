...
contract CareerCenter is Ownable, Pausable {
    ...
    struct UserInfo {
        ...
        uint256 fryRewardDebt;
        uint256 wmaticRewardDebt;
        uint256 guppyRewardDebt;
    ...
    function deposit(uint256 _amount) external whenNotPaused notContract {
        ...
        if (totalShares != 0) {
            currentShares = (depositAmount.mul(totalShares)).div(pool);
        } else {
        ...
        user.fryRewardDebt = user.fryRewardDebt.add(currentShares.mul(accFryPerShare).div(ACC_FRY_PRECISION));
        user.guppyRewardDebt = user.guppyRewardDebt.add(currentShares.mul(accGuppyPerShare).div(ACC_GUPPY_PRECISION));
        user.wmaticRewardDebt = user.wmaticRewardDebt.add(currentShares.mul(accWmaticPerShare).div(ACC_WMATIC_PRECISION));
        updateBalances();
    ...
    function harvest(address to) external notContract whenNotPaused {
        ...
        //Give Fry
        UserInfo storage user = userInfo[msg.sender];
        uint256 accumulatedFry = user.shares.mul(accFryPerShare) / ACC_FRY_PRECISION;
        uint256 _pendingFry = accumulatedFry.sub(user.fryRewardDebt);
        user.fryRewardDebt = accumulatedFry;
        //Give Matic
        uint256 accumulatedWmatic = user.shares.mul(accWmaticPerShare) / ACC_FRY_PRECISION;
        uint256 _pendingWmatic = accumulatedWmatic.sub(user.wmaticRewardDebt);
        user.wmaticRewardDebt = accumulatedWmatic;
        //Give Guppy
        uint256 accumulatedGuppy = user.shares.mul(accGuppyPerShare) / ACC_FRY_PRECISION;
        uint256 _pendingGuppy = accumulatedGuppy.sub(user.guppyRewardDebt);
        ...
        returns (uint256)
    {
        ...
        uint256 incomingFryPerShare = incomingFry.mul(ACC_FRY_PRECISION).div(totalShares);
        return user.shares.mul(accFryPerShare.add(incomingFryPerShare)).div(ACC_FRY_PRECISION).sub(user.fryRewardDebt);
    }
        ...
        returns (uint256)
    {
        ...
        uint256 incomingGuppyPerShare = incomingGuppy.mul(ACC_GUPPY_PRECISION).div(totalShares);
        return user.shares.mul(accGuppyPerShare.add(incomingGuppyPerShare)).div(ACC_GUPPY_PRECISION).sub(user.guppyRewardDebt);
    }
   ...
   function pendingWmatic(address _user)
        ...
        returns (uint256)
    {
        ...
        uint256 incomingWmaticPerShare = incomingWmatic.mul(ACC_WMATIC_PRECISION).div(totalShares);
        return user.shares.mul(accWmaticPerShare.add(incomingWmaticPerShare)).div(ACC_WMATIC_PRECISION).sub(user.wmaticRewardDebt);
    }
