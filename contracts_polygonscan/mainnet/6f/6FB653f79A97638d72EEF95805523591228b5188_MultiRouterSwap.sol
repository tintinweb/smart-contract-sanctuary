...
contract MultiRouterSwap is Ownable, ReentrancyGuard {
    ...
    IBEP20 public IRON = IBEP20(0xD86b5923F3AD7b585eD81B448170ae026c65ae9a);
    IBEP20 public USDC = IBEP20(0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174);
    address public IronSwapRouter = 0x4C96C61AF64e78F848fB2Ec965C4da08430dF348;
    ...
    function setup(address iron_router, address dfyn_router,
        address _apollo, address _iron, address _usdc) public onlyOwner{
        IronSwapRouter = iron_router;
        DFYNRouter = dfyn_router;

        APOLLO = IBEP20(_apollo);
        IRON = IBEP20(_iron);
        USDC = IBEP20(_usdc);
    ...
    function approvator( IBEP20 token ) internal {
        token.approve(DFYNRouter, uint(0));
        token.approve(DFYNRouter, uint(- 1));
        token.approve(IronSwapRouter, uint(0));
        token.approve(IronSwapRouter, uint(- 1));
    }

    function buy(uint256 USDC_Amount) nonReentrant external {
        USDC.safeTransferFrom(msg.sender, address(this), USDC_Amount);
        uint256 IRON_Amount = _safeSwap(IronSwapRouter, USDC_Amount, address(USDC), address(IRON));
        uint256 APOLLO_Amount = _safeSwap(DFYNRouter, IRON_Amount, address(IRON), address(APOLLO));
        emit Swap(msg.sender, USDC_Amount, IRON_Amount, APOLLO_Amount);
        APOLLO.safeTransfer(msg.sender, APOLLO_Amount);
    }

    function _safeSwap(address _router, uint256 bal, address token0, address token1) internal returns(uint256) {
        uint256 _before = IBEP20(token1).balanceOf(address(this));
        address[] memory _path = new address[](2);
        _path[0] = token0;
        _path[1] = token1;
        IUniswapV2Router02(_router).swapExactTokensForTokensSupportingFeeOnTransferTokens(bal, 0, _path, address(this), now.add(600));
        uint256 _after = IBEP20(token1).balanceOf(address(this));
