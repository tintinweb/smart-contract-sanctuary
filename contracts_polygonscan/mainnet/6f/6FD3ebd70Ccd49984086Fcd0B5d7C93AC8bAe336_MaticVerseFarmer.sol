...
contract MaticVerseFarmer is IERC721Receiver {
    ...
    event TransferBack(address indexed player, uint256 tokenId);
    ...
    constructor(
        ...
        game = IGame(_game);
        babyV = IMarketPlace(_babyV);
        mverse = IMverse(_mverse);
        babyVAddress = _babyV;
        treasury = _treasury;
        admin = _admin;
        fee = _fee;
    ...
    function transferBack(uint256 tokenId) public {
        ...
        babyV.approve(babyVAddress, tokenId);
        babyV.Gift(tokenId, msg.sender);
        for (uint256 i = 0; i < playerHeroes[msg.sender].length; i++) {
            if (i == tokenId) {
                remove(i, msg.sender);
                break;
            }
    ...
    function onERC721Received(
        ...
        playerHeroes[from].push(tokenId);
    ...
    function remove(uint256 index, address player) public {
        // Move the last element into the place to delete
        playerHeroes[player][index] = playerHeroes[player][
            playerHeroes[player].length - 1
        ];
        // Remove the last element
        playerHeroes[player].pop();
    }

    function getPlayerHeroes(address player)
        public
        view
