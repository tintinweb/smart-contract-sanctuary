...
contract GrowthVault is ERC20, Ownable {
    ...
    function autoCompound() public {
        ...
        for (uint256 i; i < names.length; i++) {
            ...
            if (_amount > 0) {
                ...
                uint256 cakeBalance = _getBalanceOfToken(_cakeToken);
                ...
                amounts = IRouter(strategyObj.liquidityRouter).getAmountsOut(cakeBalance, path);
                ...
                // _swapExactTokensForETH(cakeBalance, strategyObj.liquidityRouter, path);

                // reservedForFee += desiredAmountETH;
                // if(reservedForFee > 10**12) {
                //     // get FEE
                //     path[0] = WETH;
                //     path[1] = USDT;
                //     amounts = IRouter(strategyObj.liquidityRouter).getAmountsOut(desiredAmountETH, path);
                //     IRouter(USDTRouter).swapExactETHForTokens{value:desiredAmountETH}(
                //         amounts[1] - (amounts[1]*toleranceLevelPercent*100/(10000)), // amountOutMin
                //         path,
                //         address(this),
                //         block.timestamp + 1 minutes
                //     );
                ...
                // strategyObj.totalLP = _getBalanceOfToken(strategyObj.lpToken);
    ...
    // function _harvestFromStrategy(uint256 _nameID) internal {
    //     StrategyInfo memory strategyObj = strategyInfo[_nameID];
    //     uint256 pendingAmount;
    //     if(strategyObj.pid < NONE_PID_VALUE) {
    //         pendingAmount = Ifarm(
    //             strategyObj.farm).pendingCake(strategyObj.pid, msg.sender);
    //     } else {
    //         // TODO: different methods for this kind of farms
    //         pendingAmount = Ifarm(strategyObj.farm).pendingCake(msg.sender);
    //     }
    //     Ifarm(strategyObj.farm).leaveStaking(pendingAmount);
    // }
    ...
    function _calculateAmountAfterFee(uint256 amount) internal pure returns(uint256) {
        ...
        */
        uint256 feeAmount = (amount * withdrawFee * 100) / 10000;
    ...
    function _getYELs(uint256 _percent) internal { 
        ...
        for (uint256 i; i < names.length; i++) {
            ...
            path[0] = WETH;
            path[1] = YELtoken;
            _swapExactETHForTokens(_getBalanceOfToken(WETH), 0, yelLiquidityRouter, path);
