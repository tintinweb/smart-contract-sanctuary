...
contract DogShow is Ownable, ReentrancyGuard {
	...
	mapping(uint256 => PupMeta) contestPats;
	mapping(uint256 => Ranking) contestLeaderboard;
	...
	function contestStart(uint256 numberOfDays) external onlyOwner {
		...
		console.log(numberOfDays * 1 minutes);
	...
	function _resetContestData() internal {
		...
		for (uint256 i = 0; i < LEADERBOARD_LIMIT; i++) {
			delete contestLeaderboard[i];
		}

		// clear contest pup pats
		for (uint256 i = 0; i < pupsInContest.length; i++) {
			delete contestPats[pupsInContest[i]];
		}

		// Clear Pups in contest
		pupsInContest = new uint256[](0);
	...
	function pat(uint256 _pupId) external nonReentrant {
		...
		// Sorting Contest Leaderboard separately to maintain daily listing
		if (isContestOn) {
			contestPats[_pupId].pats += 1;
			contestPats[_pupId].owner = pupPats[_pupId].owner;
			pupsInContest.push(_pupId);
			_contestLeaderboardSort(_pupId);
		}
	...
	function _setOwnerAddress(uint256 _pupId) internal {
		...
		try pupContract.ownerOf(_pupId) returns (address addr) {
			ownerAdd = addr;
		} catch Error(
	...
	function _setWinnerPup() internal {
		 ...
		 */
		// console.log('setting winner pup conditions');
		// console.log(!pupPats[leaderboard[0].pupId].wonOnce);
		// console.log(players[pupPats[contestLeaderboard[0].pupId].owner].inContest);
		// console.log(pupPats[leaderboard[0].pupId].owner != address(0));

		if (
			!pupPats[contestLeaderboard[0].pupId].wonOnce &&
			players[pupPats[contestLeaderboard[0].pupId].owner].inContest &&
			pupPats[contestLeaderboard[0].pupId].owner != address(0)
		) {
			// Pushing winner in winner array
			winners.push(contestLeaderboard[0]);

			// Setting pup in main pup array as won once
			pupPats[contestLeaderboard[0].pupId].wonOnce = true;

			// Transfering reward to winner
			collarToken.safeTransfer(
				pupPats[contestLeaderboard[0].pupId].owner,
	...
	function _contestLeaderboardSort(uint256 _pupId) internal {
		// console.log(pupPats[_pupId].owner);
		// console.log(players[pupPats[_pupId].owner].inContest);
		if (
			contestLeaderboard[LEADERBOARD_LIMIT - 1].pup.pats >=
			contestPats[_pupId].pats ||
			pupPats[_pupId].wonOnce ||
			!players[pupPats[_pupId].owner].inContest
		) {
			//didn't make it to leaderboard
			return;
		}
		...
		for (uint256 i = 0; i < LEADERBOARD_LIMIT; i++) {
			if (contestLeaderboard[i].pup.pats < contestPats[_pupId].pats) {
				// resort leaderboard
				if (contestLeaderboard[i].pupId != _pupId) {
					bool duplicate = false;
					Ranking memory currentRanking = contestLeaderboard[i];
					for (uint256 j = i + 1; j < LEADERBOARD_LIMIT + 1; j++) {
						if (contestLeaderboard[j].pupId == _pupId) {
							duplicate = true;
							delete contestLeaderboard[j];
						}
						if (duplicate) {
							contestLeaderboard[j] = currentRanking;
							currentRanking = contestLeaderboard[j + 1];
						} else {
							Ranking memory nextRanking = contestLeaderboard[j];
							contestLeaderboard[j] = currentRanking;
							currentRanking = nextRanking;
						}
					}
				...
				// Add new high score
				contestLeaderboard[i] = Ranking({pup: contestPats[_pupId], pupId: _pupId});
				emit ContestLeaderboardUpdated(_pupId);
				return;
			}
			if (contestLeaderboard[i].pupId == _pupId) {
				// user already is in the leaderboard with higher or equal score
	...
	/**
	 * Converting the Contest leaderboard mapping to an array before sending
	 */
	function getContestLeaderboard() public view returns (Ranking[] memory) {
		Ranking[] memory rankings = new Ranking[](LEADERBOARD_LIMIT);
		for (uint256 i = 0; i < LEADERBOARD_LIMIT; i++) {
			rankings[i] = contestLeaderboard[i];
		}
		return rankings;
	}
	 ...
	 * Is user in contest
	 */
	function isAContestParticipant(address _address) public view returns (bool) {
		return players[_address].inContest;
	}
	...
	function _processDeposit(uint256 _amount) internal {
		...
		require(ownsAPupPunk(), 'You need to own a PupPunk to enter contest');
		require(
			players[msg.sender].inContest == false,
			'Player has already paid contest fees'
		);
		...
		players[msg.sender].inContest = true;
