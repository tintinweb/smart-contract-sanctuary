...
contract Meta721 is ERC721Enumerable, Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    // mapping for token URIs
    string public baseURI = 'https://cryptobase.mypinata.cloud/ipfs/';
    ...
    IMetaHolder immutable holder;

    uint256 public xhgtFee = 100 * 10**18;
    ...
    constructor(IERC20 _xhgt, IMetaHolder _holder, address _operator, uint256 _maxSupply, uint256 _maxDaySupply) ERC721("MetaBaseArt", "MBFree") {
        xhgt = _xhgt;
        holder = _holder;
    ...
    function tokenURI(uint256 _tokenId) public view override returns (string memory) {
        ...
        string memory _tokenType = tokenTypes[_tokenId];
        string memory tokenIdStr = toString(_tokenId);
        string[4] memory _jsons;
        _jsons[0] = string(abi.encodePacked('{"name":"CBFreeArt #',tokenIdStr, '"'));
        _jsons[1] = string(abi.encodePacked(',"image":"' ,_tokenURI, '"'));
        _jsons[2] = ',"description":"Free Creation Artwork for MetaBase.art"';
        _jsons[3] = string(abi.encodePacked(',"attributes":[{"trait_type":"ArtType","value":"',_tokenType ,'"}]}'));
        string memory base64 = Base64.encode(bytes(string(abi.encodePacked(_jsons[0], _jsons[1], _jsons[2], _jsons[3]))));

        return string(abi.encodePacked('data:application/json;base64,', base64));
    }

    function mintWithEther(string memory _tokenURI, string memory _fileHash, string memory _tokenType) external payable {
        ...
        _mintNFT(_tokenURI, _tokenHash, _tokenType);

        emit MintWithEther(msg.sender, _tokenURI, _tokenHash);
    }

    function mintWithXhgt(string memory _tokenURI, string memory _fileHash, string memory _tokenType) external {
        uint256 _tokenHash = uint256(keccak256(abi.encodePacked(_fileHash)));
        // 收取手续费
    ...
    function _mintNFT(string memory _tokenURI, uint256 _tokenHash, string memory _tokenType) private {
