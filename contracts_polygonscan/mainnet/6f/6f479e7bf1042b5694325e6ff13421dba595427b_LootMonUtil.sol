...
// File: contracts/LootMonCard.sol
...
// File: contracts/LootMonUtil.sol
...
library LootMonUtil {
    function getColors(string memory _element) public pure returns(string memory, string memory) {
        if(compareString(_element, "Sun")) {
            return("#FF7A00", "#FA1E0E");
        } else if (compareString(_element, "Earth")) {
            return("#39a9cb", "#005f99");
        } else if (compareString(_element, "Mercury")) {
            return("#91c788", "#1e6f5c");
        } else if (compareString(_element, "Mars")) {
            return("#c68b59", "#5c3d2e");
        } else if (compareString(_element, "Jupiter")) {
...
// File: contracts/LootMonster.sol
...
contract MonsterLoot is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function getHorn(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "HORN", horns);
    }


    function getItemFromLootBag(uint256 tokenId) public view returns(string memory) {
        require(lootAddress != address(0), "loot address is at address(0)");
        uint256 itemInLootBag = 8;
        uint rand = random(string(abi.encodePacked(tokenId)));
        uint itemToGet = rand % itemInLootBag;
    ...
    function pluck(uint256 tokenId, string memory keyPrefix, string[] memory sourceArray) internal view returns (string memory) {
        ...
        if (!LootMonUtil.compareString(keyPrefix, "TYPE") && 
        !LootMonUtil.compareString(keyPrefix, "ELEMENT") && 
        !LootMonUtil.compareString(output, "No Horns") && 
        !LootMonUtil.compareString(output, "No Wings")) 
        {
            if (greatness > 14) {
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        string memory _element = getElement(tokenId);
        string memory _type = getType(tokenId);
        string memory _horns = getHorn(tokenId);
        string memory _wings = getWing(tokenId);
        string memory _weapon = getWeapon(tokenId);
        string memory _itemFromLootBag = getItemFromLootBag(tokenId);

        (string memory gColor1, string memory gColor2) = LootMonUtil.getColors(_element);


        parts[0] = LootMonCard.svgStart();

        parts[1] = LootMonCard.linearGradient(gColor1, gColor2);

        parts[2] = LootMonCard.style();

        parts[3] = LootMonCard.card(_element, _type, _horns, _wings, _weapon);

        parts[4] = LootMonCard.cardDetail("Drop item from LOOT Bags", _itemFromLootBag);

        parts[5] = LootMonCard.svgEnd();

        string memory output = string(abi.encodePacked(parts[0], parts[1], parts[2], parts[3], parts[4], parts[5]));
        
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Monster No #', toString(tokenId), '", "description": "Monster Loot is the cloning project of LOOT, for random monster generated and stored on chain. plus drop 1 item from loot bag!. Stats, images, and other functionality are intentionally omitted for others to interpret. Feel free to use Monster Loot in any way you want.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
    ...
    constructor(address loot) ERC721("Monster Loot", "monLOOT") Ownable() {
        require(loot != address(0), "loot address is at address(0)");
        lootAddress = loot;
    }
}