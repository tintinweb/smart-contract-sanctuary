...
contract StrategySushiswapLP is StratManager, FeeManager {
    ...
    function chargeFees() internal {
        ...
        uint256 krillFeeAmount = maticBal.mul(krillFee).div(MAX_FEE);
        IERC20(matic).safeTransfer(krillFeeRecipient, krillFeeAmount);
    ...
    function convertDustToEarned(address[] memory _lp0ToWantRoute, address[] memory _lp1ToWantRoute) public whenNotPaused {        
        ...
        if (lpToken0 != want && token0Amt > 0) {
            ...
            IUniswapV2Router02(unirouter).swapExactTokensForTokens(token0Amt, 0, _lp0ToWantRoute, address(this), now);
        ...
        if (lpToken1 != want && token1Amt > 0) {
            ...
            // Swap all dust tokens to want tokens
            IUniswapV2Router02(unirouter).swapExactTokensForTokens(token1Amt, 0, _lp1ToWantRoute, address(this), now);
        }
    }
}