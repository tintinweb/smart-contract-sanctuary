...
import "./ETHEcoleTicket.sol";
import "./IERC20Permit.sol";

contract Donation {
  ETHEcoleTicket public immutable ticket;
  IERC20Permit public immutable token;

  constructor(address _ticket) {
    ticket = ETHEcoleTicket(_ticket);
    token = IERC20Permit(ETHEcoleTicket(_ticket).paymentToken());
  }

  function purchase(uint256 amount, string calldata name) external {
    token.transferFrom(msg.sender, address(this), amount);
    token.approve(address(ticket), amount);
    ticket.purchase(msg.sender, name);
    token.transfer(address(ticket), ticket.balanceOf(address(this)));
  }
}
