...
interface IUniStaker is IRageQuit  {
    ...
    function claimReward(address vault, address vaultFactory, address rewardPool, address token, 
                            address rewardToken, address recipient, bool claimBonusReward) external;

    function claimVestedReward(address rewardToken, address rewardPool) external;

    function claimVestedReward(address token, address rewardToken, address rewardPool) external;
}
...
contract UniStaker is IUniStaker, Powered {
    ...
    address public powerSwitchFactory;
    ...
    // whitelisted operators to write to storage
    EnumerableSet.AddressSet private _whitelistSet;

    // reward pool to token to lmRewardData map
    mapping(address => mapping(address => LMRewardData)) private _lmRewards;
    ...
    mapping(address => EnumerableSet.AddressSet) private _vestingLMTokenRewards;
    ...
    /// @param rewardPoolFac address The factory to use for deploying RewardPools
    /// @param powerSwitchFac address The factory to use for deploying PowerSwitches
    constructor(
        address adminAddress,
        address rewardPoolFac,
        address powerSwitchFac
    ) {
        // deploy power switch
        address powerSwitch = IFactory(powerSwitchFac).create(abi.encode(adminAddress));
        Powered._setPowerSwitch(powerSwitch);

        admin = adminAddress;
        rewardPoolFactory = rewardPoolFac;
        powerSwitchFactory = powerSwitchFac;

        // emit event
        emit UniStakerCreated();
    }

    /* helper functions */
    function isValidVault(address vault, address factory) public view returns (bool validity) {
    ...
    function isValidAddress(address target) public view returns (bool validity) {
        ...
        return target != address(this) && target != address(0);
    }

    function whitelisted(address operator) public view returns (bool) {
        return _whitelistSet.contains(operator);
    }

    function _whitelisted(address operator) private view {
        require(_whitelistSet.contains(operator), "forbidden");
    }
    ...
    function whitelist(address operator) public {
        _admin();
        _whitelistSet.add(operator);
    }

    function removeFromWhitelist(address operator) public {
        _admin();
        _whitelistSet.remove(operator);
    }
    ...
    function transferAdminship(address newAdmin) public {
        _admin();
        require(newAdmin != address(0), "new admin can't be the zero address");
    ...
    function registerVaultFactory(address factory) external {
        _admin();
        // add factory to set
        require(_vaultFactorySet.add(factory), "vault factory already registered");
    ...
    function removeVaultFactory(address factory) external {
        ...
        require(_vaultFactorySet.remove(factory), "vault factory not registered");
    ...
    function setLMRewardMultiplier(string calldata tier, uint256 numerator, uint256 denominator) external {
        _admin();
    ...
    function updateOnStake(address user, address vault, address vaultFactory, address rewardPool, address token, uint256 amount) external {
        _whitelisted(msg.sender);
        require(isValidVault(vault, vaultFactory), "vault is not valid");
        // add vault to set
        _vaultSet.add(vault);
        // fetch vault storage reference
        VaultData storage vaultData = _vaults[vault];
        // verify stakes boundary not reached
        require(vaultData.tokens.length() < MAX_TOKENS_STAKED_PER_VAULT, "MAX_TOKENS_STAKED_PER_VAULT reached");
        ...
        if (_lmRewards[rewardPool][token].startedAt != 0) {
            address rewardCalcInstance = _lmRewards[rewardPool][token].rewardCalcInstance;
            (,uint rewardEarned,) = IERC2917(rewardCalcInstance).increaseProductivity(user, amount);
            earnedLMRewards[user][token] = earnedLMRewards[user][token].add(rewardEarned);
        }
    }

    function updateOnUnstake(address user, address vault, address rewardPool, address token, uint256 amount) external {
        _whitelisted(msg.sender);
        require(_vaultSet.contains(vault), "no vault");
        // fetch vault storage reference
        VaultData storage vaultData = _vaults[vault];
        
        // check for sufficient vault stake amount
        require(vaultData.tokens.contains(token), "no token in vault");
        // check for sufficient vault stake amount
        require(vaultData.tokenStake[token] >= amount, "insufficient vault token stake");
        ...
        if (_lmRewards[rewardPool][token].startedAt != 0) {
            address rewardCalcInstance = _lmRewards[rewardPool][token].rewardCalcInstance;
            IERC2917(rewardCalcInstance).decreaseProductivity(user, amount);
        }
    ...
    function startLMRewards(address token, address rewardPool, uint256 amount, uint256 duration) external {
        startLMRewards(token, rewardPool, amount, duration, activeRewardCalcTemplate);
    }

    function startLMRewards(address token, address rewardPool, uint256 amount, uint256 duration, string memory rewardCalcTemplateName) public onlyOnline {
        require(msg.sender == RewardPool(rewardPool).getPowerController(), "forbidden");
        require(_lmRewards[rewardPool][token].startedAt == 0, "A reward program already live for this token");
        ...
        LMRewardData storage lmrd = _lmRewards[rewardPool][token];
        lmrd.amount = amount;
    ...
    function setImplementorForRewardsCalculator(address token, address rewardPool, address newImplementor) public notShutdown {
        require(msg.sender == RewardPool(rewardPool).getPowerController(), "forbidden");
        require(_lmRewards[rewardPool][token].startedAt != 0, "No reward program currently live for this token");
        address rewardCalcInstance = _lmRewards[rewardPool][token].rewardCalcInstance;
        IERC2917(rewardCalcInstance).setImplementor(newImplementor);
    }

    function setLMRewardsPerBlock(address token, address rewardPool, uint value) public onlyOnline {
        require(msg.sender == RewardPool(rewardPool).getPowerController(), "forbidden");
        require(_lmRewards[rewardPool][token].startedAt != 0, "No reward program currently live for this token");
        address rewardCalcInstance = _lmRewards[rewardPool][token].rewardCalcInstance;
    ...
    function addBonusTokenToLMRewards(address token, address rewardPool, address bonusToken, uint256 bonusTokenAmount) public onlyOnline {
        require(msg.sender == RewardPool(rewardPool).getPowerController(), "forbidden");
        require(_lmRewards[rewardPool][token].startedAt != 0, "No reward program currently live for this LM token");
        _lmRewards[rewardPool][token].bonusTokens.add(bonusToken);
        _lmRewards[rewardPool][token].bonusTokenAmounts[bonusToken] = _lmRewards[rewardPool][token].bonusTokenAmounts[bonusToken].add(bonusTokenAmount);
    }

    function endLMRewards(address token, address rewardPool, bool removeBonusTokenData) public onlyOnline {
        require(msg.sender == RewardPool(rewardPool).getPowerController(), "forbidden");
        _lmRewards[rewardPool][token].amount = 0;
        _lmRewards[rewardPool][token].duration = 0;
        _lmRewards[rewardPool][token].startedAt = 0;
        _lmRewards[rewardPool][token].rewardCalcInstance = address(0);
        if (removeBonusTokenData) {
            for (uint index = 0; index < _lmRewards[rewardPool][token].bonusTokens.length(); index++) {
                address bonusToken = _lmRewards[rewardPool][token].bonusTokens.at(index);
                _lmRewards[rewardPool][token].bonusTokens.remove(bonusToken);
                delete _lmRewards[rewardPool][token].bonusTokenAmounts[bonusToken];
            }
        }
    }
    ...
    function getLMRewardData(address token, address rewardPool) external view returns (uint amount, uint duration, uint startedAt, address rewardCalcInstance) {
        return (_lmRewards[rewardPool][token].amount, _lmRewards[rewardPool][token].duration, _lmRewards[rewardPool][token].startedAt, _lmRewards[rewardPool][token].rewardCalcInstance);
    }

    function getLMRewardBonusTokensLength(address token, address rewardPool) external view returns (uint length) {
        return _lmRewards[rewardPool][token].bonusTokens.length();
    }

    function getLMRewardBonusTokenAt(address token, address rewardPool, uint index) external view returns (address bonusToken, uint bonusTokenAmount) {
        return (_lmRewards[rewardPool][token].bonusTokens.at(index), _lmRewards[rewardPool][token].bonusTokenAmounts[_lmRewards[rewardPool][token].bonusTokens.at(index)]);
    }

    function getNumVestingLMTokenRewards(address user) external view returns (uint num) {
       return _vestingLMTokenRewards[user].length();
    }

    function getVestingLMTokenAt(address user, uint index) external view returns (address token) {
        return _vestingLMTokenRewards[user].at(index);
    }
    ...
    /// @notice Exit UniStakerNonFungible without claiming reward
    ...
    /// state machine:
    ///   - when vault exists on this UniStakerNonFungible
    ...
    function rageQuit() external override {
        require(_vaultSet.contains(msg.sender), "UniStakerNonFungible: no vault");
        //fetch vault storage reference
        ...
        require(vaultTokens.length() > 0, "UniStakerNonFungible: no stake");
    ...
    function claimReward(address vault, address vaultFactory, address rewardPool, address token, 
                            address rewardToken, 
                            address recipient, bool claimBonusReward) external override onlyOnline {
        // check if there is a reward program currently running
        uint rewardEarned = earnedLMRewards[msg.sender][token];
        if (_lmRewards[rewardPool][token].startedAt != 0) {
            address rewardCalcInstance = _lmRewards[rewardPool][token].rewardCalcInstance;
            (,uint newReward,) = IERC2917(rewardCalcInstance).decreaseProductivity(msg.sender, 0);
        ...
        if (rewardEarned > 0) {
            ...
            if (claimBonusReward && _lmRewards[rewardPool][token].startedAt != 0) {
                for (uint256 index = 0; index < _lmRewards[rewardPool][token].bonusTokens.length(); index++) {
                    ...
                    uint256 bonusAmount = rewardEarned.mul(_lmRewards[rewardPool][token].bonusTokenAmounts[bonusToken]).div(_lmRewards[rewardPool][token].amount);
                    // transfer bonus token
                    RewardPool(rewardPool).sendERC20(bonusToken, recipient, bonusAmount);
                    // emit event
            ...
            _vestingLMTokenRewards[msg.sender].add(token);
            // set earned reward to 0
            earnedLMRewards[msg.sender][token] = 0;
            // transfer reward tokens from reward pool to recipient
            RewardPool(rewardPool).sendERC20(rewardToken, recipient, multipliedReward.sub(vestingPortion));
            // emit event
            emit RewardClaimed(vault, recipient, rewardToken, rewardEarned);
        }
    }

    function claimVestedReward(address rewardToken, address rewardPool) external override onlyOnline {
        uint numTokens = _vestingLMTokenRewards[msg.sender].length();
        for (uint index = 0; index < numTokens; index++) {
            address token = _vestingLMTokenRewards[msg.sender].at(index);
            claimVestedReward(token, rewardToken, rewardPool, vestingLMRewards[msg.sender][token].length);
        }
    }

    function claimVestedReward(address token, address rewardToken, address rewardPool) external override onlyOnline {
        claimVestedReward(token, rewardToken, rewardPool, vestingLMRewards[msg.sender][token].length);
    }

    function claimVestedReward(address token, address rewardToken, address rewardPool, uint numVests) public onlyOnline {
        ...
        for (uint index = 0; index < numVests; index++) {
            ...
            if (vested >= vest.amount) {
                ...
                if (vests.length == 0) {
                    _vestingLMTokenRewards[msg.sender].remove(token);
                    break;
                }
            } else {
        ...
        if (vestedReward > 0) {
            // transfer reward tokens from reward pool to recipient
            RewardPool(rewardPool).sendERC20(rewardToken, msg.sender, vestedReward);
            // emit event
...
contract MethodNFTFactory is Ownable, IFactory, IInstanceRegistry, ERC721 {
    ...
    uint256 public tokenSerialNumber = 5000;
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal virtual override {
        ...
        address vault = getVaultOfNFT(tokenId);
    ...
    function getActiveTemplate() external view returns (address) {
