...
contract UniswapAnchoredView is UniswapConfig {
    ...
    constructor(OpenOraclePriceData priceData_,
        ...
        nativeHash = keccak256(abi.encodePacked(nativeSymbol));
    ...
    function postPrices(bytes[] calldata messages, bytes[] calldata signatures, string[] calldata symbols) external {
        ...
        // OLA_ADDITIONS : Using 'native price' instead of 'ethPrice
        uint nativePrice = fetchNativePrice();

        // Try to update the view storage
        for (uint i = 0; i < symbols.length; i++) {
            postPriceInternal(symbols[i], nativePrice);
        }
    }
    ...
    // OLA_ADDITIONS : Using 'nativePrice' instead of 'ethPrice'
    function postPriceInternal(string memory symbol, uint nativePrice) internal {
        TokenConfig memory config = getTokenConfigBySymbol(symbol);
        ...
        uint anchorPrice;
        if (symbolHash == nativeHash) {
            anchorPrice = nativePrice;
        } else {
            anchorPrice = fetchAnchorPrice(symbol, config, nativePrice);
        }
...
//    }
    ...
    /**
     * @dev Fetches the current native/usd price from uniswap, with 6 decimals of precision.
     *  Conversion factor is 1e18 for native/usdc market, since we decode uniswap price statically with 18 decimals.
     */
    function fetchNativePrice() internal returns (uint) {
        return fetchAnchorPrice(nativeSymbol, getTokenConfigBySymbolHash(nativeHash), ethBaseUnit);
    }
    ...
    function fetchAnchorPrice(string memory symbol, TokenConfig memory config, uint conversionFactor) internal virtual returns (uint) {
        ...
        if (keccak256(abi.encodePacked(symbol)) == nativeHash) {
