...
contract CHQueensArtifacts is Delegated, MaticERC1155{
  struct Token{
    uint burnPrice;
    uint mintPrice;
    uint balance;
  ...
  //address proxyRegistryAddress = 0xff7Ca10aF37178BdD056628eF42fD7F799fAc77c; //mumbai
  address proxyRegistryAddress = 0x207Fa8Df3a17D96Ca7EA4f2893fcdCb78a304101; //polygon
  /** END: OS required **/
  ...
  function create( address initialOwner, uint supply, string calldata uri_, bytes calldata data_ )
    ...
    setToken( id, "", uri_, supply, false, 0, false, 0 );
    return id;
  }
  ...
  function burn( uint id, uint quantity ) external payable{
    ...
    require( token.isBurnActive,                      "Burn is not active"        );
  ...
  function mintTo( address[] calldata accounts, uint[] calldata ids, uint[] calldata quantities ) external payable onlyDelegates {
    ...
    for(uint i; i < ids.length; ++i ){
      ...
      tokens[ids[i]].balance += quantities[i];
    }
  }


  //delegated
  function setToken(uint id, string memory name_, string calldata uri_, uint supply,
  ...
  function setSupply(uint id, uint supply) public onlyDelegates{
    ...
    Token storage token = tokens[id];
    require( token.supply != supply && token.balance <= supply,  "New value matches old" );
    token.supply = supply;
  }
