...
interface IKyberRewardLocker {
    ...
    event VestingEntryCreated(address indexed token, address indexed beneficiary, uint256 startBlock, uint256 endBlock, uint256 quantity, uint256 index);

    event VestingEntryQueued(uint256 indexed index, address indexed token, address indexed beneficiary, uint256 quantity);

    event Vested(address indexed token, address indexed beneficiary, uint256 vestedQuantity, uint256 index);
     ...
     */
    function vestCompletedSchedulesForMultipleTokens(address[] calldata tokens) external returns (uint256[] memory vestedAmounts);
...
contract StrategyPolycatLp is StrategyBase {
    ...
    uint256 public lastClaimRewardTimestamp;
    uint256 public limitTimeToClaimReward = 12 hours;
    ...
    function _withdrawSome(uint256 _amount) internal override returns (uint256) {
        (uint256 _stakedAmount, , ) = IPolycatMasterChef(farmPool).userInfo(poolId, address(this));
    ...
    function balanceOfPool() public view override returns (uint256) {
        (uint256 amount, , ) = IPolycatMasterChef(farmPool).userInfo(poolId, address(this));
        return amount;
    }
    ...
    function setLimitTimeToClaimReward(uint256 _limitTimeToClaimReward) external onlyStrategist {
