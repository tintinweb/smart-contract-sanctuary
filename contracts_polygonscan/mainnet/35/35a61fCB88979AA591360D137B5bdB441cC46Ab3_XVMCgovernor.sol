...
contract XVMCgovernor {
    ...
    address public immutable farmContract = 0xbd8fC29Cb8DF80Ca542Ddb19E1C20b5e1e972376; //for setpool
    ...
    modifier onlyTrustee {
      require(block.timestamp < 1638535053, "Contract is immutable after Dec 3");
    ...
    */
     ...
     */
    function stakeRolloverBonus(address _giftTo, address _depositToPool, uint256 _bonusToPay) external whenReady {
        require(
     ...
     */
    function setInflation(uint256 rewardPerBlock) external whenReady {
        require(
    ...
    function enforceGovernor() external whenReady {
        ...
        IacPool(acPool1).setAdmin(eligibleNewGovernor, eligibleNewGovernor); //set admin of acPools to new contract
        IacPool(acPool2).setAdmin(eligibleNewGovernor, eligibleNewGovernor); //admin and treasury are currently the same contract
        IacPool(acPool3).setAdmin(eligibleNewGovernor, eligibleNewGovernor); //in the future treasury might be separate
        IacPool(acPool4).setAdmin(eligibleNewGovernor, eligibleNewGovernor);
        IacPool(acPool5).setAdmin(eligibleNewGovernor, eligibleNewGovernor);
        IacPool(acPool6).setAdmin(eligibleNewGovernor, eligibleNewGovernor);
	...
	function treasuryRequest(address _tokenAddr, address _recipient, uint256 _amountToSend) external whenReady {
		require(msg.sender == consensusContract);
	...
	function setDurationForCalculation(uint256 _newDuration) external whenReady {
	    require(msg.sender == farmContract);
	    durationForCalculation = _newDuration;
	}
	
	function delayFibonacci(bool _arg) external whenReady {
	    require(msg.sender == consensusContract || msg.sender == fibonacceningContract);
	    fibonacciDelayed = _arg;
	}

	function setPool(uint256 _pid, uint256 _allocPoint, uint16 _depositFeeBP, bool _withUpdate) external whenReady {
	    require(msg.sender == farmContract);
	    IMasterChef(masterchef).set(_pid, _allocPoint, _depositFeeBP, _withUpdate);
	}
	
	function setThresholdFibonaccening(uint256 newThreshold) external whenReady {
	    require(msg.sender == fibonacceningContract);
	    thresholdFibonaccening = newThreshold;
	}
	
	function setCallFee(address _acPool, uint256 _newCallFee, uint256 _newCallFeeWithBonus) external whenReady {
	    require(msg.sender == basicContract);
	    IacPool(_acPool).setCallFee(_newCallFee, _newCallFeeWithBonus);
	}
	
	function updateCostToVote(uint256 newCostToVote) external whenReady {
	    require(msg.sender == basicContract);
	    costToVote = newCostToVote;
	}
	
	function updateRolloverBonus(address _forPool, uint256 _bonus) external whenReady {
	    require(msg.sender == basicContract);
