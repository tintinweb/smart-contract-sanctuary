...
contract TestCGJungleInitializable is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;

    // The address of the testCG jungle factory
    address public TESTCG_JUNGLE_FACTORY;
    ...
    // The block number when TESTCG mining ends.
    ...
    constructor() public {
        TESTCG_JUNGLE_FACTORY = msg.sender;
    }
    ...
    function initialize(
        ...
        require(msg.sender == TESTCG_JUNGLE_FACTORY, "Not factory");
...
import "@openzeppelin/contracts/access/Ownable.sol";
import "@openzeppelin/contracts/utils/ReentrancyGuard.sol";
import "./TestCGJungle.sol";

contract TestCGJungleFactory is Ownable {
    event NewTestCGJungleContract(address indexed testCGJungle);
     ...
     * @param _withdrawalInterval: the withdrawal interval for stakedToken (if any, else 0)
     * @param _admin: admin address with ownership
     * @return address of new testCG jungle contract
     */
    function deployPool(
        IBEP20 _stakedToken,
        IBEP20 _rewardToken,
        ...
        bytes memory bytecode = type(TestCGJungleInitializable).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(_stakedToken, _rewardToken, _startBlock));
        address testCGJungleAddress;

        assembly {
            testCGJungleAddress := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }

        TestCGJungleInitializable(testCGJungleAddress).initialize(
            _stakedToken,
            _rewardToken,
            _rewardPerBlock,
            _startBlock,
        ...
        emit NewTestCGJungleContract(testCGJungleAddress);
    }
}
