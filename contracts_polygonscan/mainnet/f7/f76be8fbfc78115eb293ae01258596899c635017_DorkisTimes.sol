...
// File: https://github.com/OpenZeppelin/openzeppelin-contracts/blob/v4.2.0/contracts/utils/structs/EnumerableSet.sol
...
abstract contract Mintable is Ownable {
     ...
     */
    function getAdmins() external view returns (address[] memory minters) {
...
// File: contracts/DorkisTimes.sol
...
/**
 * @title Dorkis Times Collection
 * @dev Extends ERC1155 
 * @author @FrankPoncelet
 */

contract DorkisTimes is ERC1155Supply, ERC1155Burnable, ERC1155Pausable, Mintable {
    ...
    */
    ...
    /**
    *  @dev set the price for a specific token.
    */
    function setTokenPrice(uint256 token, uint256 amount) public onlyMinter {
        tokenPrice[token]=amount;
    }
    
    /**
    *  @dev get the price for a specific token.
    */
    function getTokenPrice(uint256 token) public view returns(uint256){
    ...
    *  @dev mint a token
    */
    function mint(uint256 token) public payable{
        require(token!=0,"DorkisTimes: You can't mint token 0.");
        require(saleIsActive, "DorkisTimes: Sale must be active to mint Tokens!");
        require(totalSupply(token) + 1 <= MAX_TOKENS, "DorkisTimes: Purchase would exceed max supply of tokens!");
        require(!paused(), "DorkisTimes: Minting is paused");
        require(tokenPrice[token] <= msg.value, "DorkisTimes: Ether value sent is not correct");
        require(balanceOf(msg.sender,token)==0,"DorkisTimes: You can only mint ONE Dorkis time per wallet!");
