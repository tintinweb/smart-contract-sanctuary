...
contract GnomeShroomClub is ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;
    using Strings for uint256;

    address payable private _PaymentAddress = payable(0x0b1dE0459EfEcd5214a28D6c98C067Ca55242dD0);
    address payable private _DevAddress;

    uint256 public GSC_MAX = 10500;
    uint256 public PRICE = 55_000_000_000_000_000_000; // 55 MATIC
    ...
    uint256 private _activeDateTime = 1637341200; // Date and time (GMT): Friday, November 19, 2021 5:00:00 PM
    ...
    Counters.Counter private _publicGSC;

    constructor() ERC721("Gnome Shroom Club", "GSC") {
    ...
    function airdrop(address to, uint256 numberOfTokens) external onlyOwner {
        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = _publicGSC.current();

            if (_publicGSC.current() < GSC_MAX) {
                _publicGSC.increment();
                _safeMint(to, tokenId);
    ...
    function purchase(uint256 numberOfTokens) external payable {
        ...
        require(_publicGSC.current() < GSC_MAX,"Purchase would exceed GSC_MAX");
