...
MM88MMM ,adPPYYba,  ,adPPYba,  ,adPPYba,   
  88    ""     `Y8 a8"     "" a8"     "8a  
  88    ,adPPPPP88 8b         8b       d8  DIVIDENDS STRATEGY  
...
contract StrategyDividends is Ownable, ReentrancyGuard, Pausable {
    ...
    address public wantAddress; // the token we want to deposit
    address public rewardTokenAddress;  // the token we will reward
    ...
    // Not transfer tax safe!  Check before allowing
    ...
    function withdraw(address _userAddress, uint256 _wantAmt) external onlyOwner nonReentrant returns (uint256) {
        ...
        uint256 heldAmt = IERC20(wantAddress).balanceOf(address(this));
        if (_wantAmt > heldAmt) {
            _wantAmt = heldAmt;
        }
    ...
    function depositReward(uint256 _depositAmt) external returns (bool) {
        uint256 balanceBefore = IERC20(rewardTokenAddress).balanceOf(address(this));
        IERC20(rewardTokenAddress).safeTransferFrom(msg.sender, address(this), _depositAmt);
        _depositAmt = IERC20(rewardTokenAddress).balanceOf(address(this)) - balanceBefore;
        if (sharesTotal == 0) {
            return false;
        }
