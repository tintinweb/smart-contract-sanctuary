...
//XXXXX TEST XXXXX
//-----------------------------------------------------------------------------
contract AnonTestToken is BEP20("AnonTest Token", "AnonTest", 10000000000000000000000) {
...
contract MasterChef is Ownable, NonReentrant {
  ...
  //---------------------------------------------------------------------------
  uint256 public constant EMISSION_RATE = 0.01 ether;
  ...
  struct PoolInfo {
    ...
    uint256 accAnonTestPerShare;
    uint16 depositFeeBP;
  }
  //---------------------------------------------------------------------------
  AnonTestToken public AnonTest;
  ...
  constructor(AnonTestToken _AnonTest, address _feeAddress, uint256 _startBlock) public {
    AnonTest = _AnonTest;
 ...
 //---------------------------------------------------------------------------
  function add(uint256 _allocPoint, IBEP20 _lpToken, bool _withUpdate) public ownerOnly isUnique(_lpToken) {
    ...
    poolInfo.push(PoolInfo({lpToken: _lpToken, allocPoint: _allocPoint, lastRewardBlock: lastRewardBlock, accAnonTestPerShare: 0, depositFeeBP: depositFee})
    );
  }
  function pendingAnonTest(uint256 _pid, address _user) external view returns (uint256)  {
    PoolInfo storage pool = poolInfo[_pid];
    UserInfo storage user = userInfo[_pid][_user];
    uint256 share = pool.accAnonTestPerShare;
    uint256 pot = pool.lpToken.balanceOf(address(this));
    if (block.number > pool.lastRewardBlock && pot != 0) {
      uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
      uint256 AnonTestReward = multiplier.mul(EMISSION_RATE).mul(pool.allocPoint).div(totalAllocPoint);
      share = share.add(AnonTestReward.mul(1e12).div(pot));
    }
  ...
  function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
    ...
    updatePool(_pid);
    if (user.amount > 0) {
      uint256 pending = user.amount.mul(pool.accAnonTestPerShare).div(1e12).sub(user.rewardDebt);
      if (pending > 0) {
        safeAnonTestTransfer(msg.sender, pending);
      }
    }
    if (_amount > 0) {
    ...
    user.rewardDebt = user.amount.mul(pool.accAnonTestPerShare).div(1e12);
  ...
  function updatePool(uint256 _pid) public {
    ...
    uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
    uint256 AnonTestReward = multiplier.mul(EMISSION_RATE).mul(pool.allocPoint).div(totalAllocPoint);
    AnonTest.mint(feeAddress, AnonTestReward.div(20));
    AnonTest.mint(address(this), AnonTestReward);
    pool.accAnonTestPerShare = pool.accAnonTestPerShare.add(AnonTestReward.mul(1e12).div(lpSupply));
    pool.lastRewardBlock = block.number;
  }
  ...
  function safeAnonTestTransfer(address _to, uint256 _amount) internal {
    uint256 pot = AnonTest.balanceOf(address(this));
    bool success = false;
    if (_amount > pot) {
      success = AnonTest.transfer(_to, pot);
    } else {
      success = AnonTest.transfer(_to, _amount);
    }
    require(success, "error");
  }
