...
// File: contracts/EpicStrategy_Polygon_Pickle_DAI2a.sol
...
contract EpicStrategy_Polygon_Pickle_DAI2a{
    ...
    address public constant PICKLE_LP_TOKEN = 0x0519848e57Ba0469AA5275283ec0712c91e20D8E;

    //The token we deposit into the Pool
    address public constant want = DAI;
    ...
    function doApprovals() public {
        IERC20(DAI).safeApprove(PICKLE_POOL,type(uint256).max);
    ...
    function getName() external pure returns (string memory) {
        return "EpicStrategy_Polygon_Pickle_DAI2a";
    }
    ...
    function balanceOfPool() public view returns (uint256) {
        ...
        if (amount > 0) {
            //Balance of Curve LP
            uint256 amountDAI = amount.mul(IPickleVault(PICKLE_POOL).getRatio()).div(1e18);
    ...
    function deposit() public {
        ...
        IPickleVault(PICKLE_POOL).deposit(IERC20(DAI).balanceOf(address(this)));
    ...
    function _withdrawSome(uint256 _amount) internal returns (uint256) {
        ...
        uint256 _amountInPickles = _amount.mul(1e18).div(pps);
    ...
    function harvest() public {
        ...
        if (earned > 0) {
            ...
            IERC20(want).safeTransfer(IEpicVault(vault).rewards(), _fee);
        }
