...
contract CCollateralCapErc20 is CToken, CCollateralCapErc20Interface {
     ...
     */
    function _setCollateralCap(uint256 newCollateralCap) external override {
     ...
     */
    function registerCollateral(address account) external override returns (uint256) {
     ...
     */
    function unregisterCollateral(address account) external override {
        // Make sure accountCollateralTokens of `account` is initialized.
        initializeAccountCollateralTokens(account);
    ...
    function initializeAccountCollateralTokens(address account) internal {
        ...
        if (!isCollateralTokenInit[account]) {
            if (ComptrollerInterfaceExtension(address(comptroller)).checkMembership(account, address(this))) {
    ...
    function mintFresh(
         ...
         */
        if (ComptrollerInterfaceExtension(address(comptroller)).checkMembership(minter, address(this))) {
