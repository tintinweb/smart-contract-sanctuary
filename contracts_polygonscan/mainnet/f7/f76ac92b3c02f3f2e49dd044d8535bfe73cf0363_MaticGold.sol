...
/// @title Matic Gold for Loot holders!
/// @author Satoshi Nakamoto
/// @notice This contract mints Matic Gold for Loot holders and provides
/// administrative functions to the Loot DAO. It allows:
/// * Loot holders to claim Matic Gold
/// * A DAO to set seasons for new opportunities to claim Matic Gold
/// * A DAO to mint Matic Gold for use within the Loot ecosystem
/// @custom:unaudited This contract has not been audited. Use at your own risk.
contract MaticGold is Context, Ownable, ERC20 {
    uint256 public constant _price = 5000000000000000000; // 5 MATIC
    ...
    // Give out 10,000 Matic Gold for every Loot Bag that a user holds
    uint256 public MaticGoldPerTokenId = 10000 * (10**decimals());
    ...
    */
    ...
    constructor() Ownable() ERC20("Matic Gold", "MGLD") {
        ...
        transferOwnership(0x4410f2187b07dF1BE62A2acE99Bd4E44dc531982);
        lootContract = IERC721Enumerable(lootContractAddress);
    }

    /// @notice Claim Matic Gold for a given Loot ID
    /// @param tokenId The tokenId of the Loot NFT
    function claimById(uint256 tokenId) external  payable{
		...
		require(msg.value >= _price, "Please enter 10 MATIC"); 
        // Further Checks, Effects, and Interactions are contained within the
        // _claim() function
        _claim(tokenId, _msgSender());
    }

    /// @notice Claim Matic Gold for all tokens owned by the sender
    ...
    /// Gold in batches.
    function claimAllForOwner() external payable {
    ...
    /// @notice Claim Matic Gold for all tokens owned by the sender within a
    /// given range
    ...
    function _claim(uint256 tokenId, address tokenOwner) internal {
        ...
        // Check that Matic Gold have not already been claimed this season
        // for a given tokenId
        require(
        ...
        // Mark that Matic Gold has been claimed for this season for the
        // given tokenId
        seasonClaimedByTokenId[season][tokenId] = true;

        // Interactions

        // Send Matic Gold to the owner of the token ID
        _mint(tokenOwner, MaticGoldPerTokenId);
    }
    ...
    /// @notice Allows the DAO to set a season for new Matic Gold claims
    /// @param season_ The season to use for claiming Loot
    function daoSetSeason(uint256 season_) public onlyOwner {
        season = season_;
    }

    /// @notice Allows the DAO to set the amount of Matic Gold that is
    /// claimed per token ID
    /// @param MaticGoldDisplayValue The amount of Loot a user can claim.
    ...
    /// 100 * 10^18.
    function daoSetMaticGoldPerTokenId(uint256 MaticGoldDisplayValue)
        public
        onlyOwner
    {
        MaticGoldPerTokenId = MaticGoldDisplayValue * (10**decimals());
    }

    /// @notice Allows the DAO to set the season and Matic Gold per token ID
    /// in one transaction. This ensures that there is not a gap where a user
    /// can claim more Matic Gold than others
    /// @param season_ The season to use for claiming loot
    /// @param MaticGoldDisplayValue The amount of Loot a user can claim.
    /// This should be input as the display value, not in raw decimals. If you
    /// want to mint 100 Loot, you should enter "100" rather than the value of
    ...
    /// MaticGold variables directly. It is better practice for security,
    ...
    function daoSetSeasonAndMaticGoldPerTokenID(
        uint256 season_,
        uint256 MaticGoldDisplayValue
    ) external onlyOwner {
        daoSetSeason(season_);
        daoSetMaticGoldPerTokenId(MaticGoldDisplayValue);
    }
