...
contract PriceCalculator is Initializable, OwnableUpgradeable {
    ...
    function oracleValueOf(address oracleAddress, address tokenAddress, uint amount) internal view returns (uint256 valueInBNB) {
        uint256 price = IPriceFeed(oracleAddress).getData(tokenAddress);
        valueInBNB = price.mul(amount).div(_DECIMAL);
    }
...
// File contracts/utils/OnChainPriceFetcher.sol
...
contract OnChainPriceFetcher is IPriceFeed {
    function getData(address token) external view override returns(uint256) {
        return IZap(0xFC8bBF9cD6014eF91ee4c2Cdce6e4B1c16Ad954f).tokenPriceIn1e6USDC(token);
    }
}