...
contract Roller is Ownable,  VRFConsumerBase {
    ...
    function placeBet(uint256 randomValue) internal {
        ...
        if(address(this).balance < betAmount.mul(5)){
            sky.getMatic(betAmount.mul(5));
        }
...
contract Charge is Roller {
    ...
    function betAverage(uint256 random) internal view returns(uint256){
        ...
        if(rolls.length > 3){
            lastBet = rolls[rolls.length-3];
        }else{
            for(uint256 j = length-1; j > length-50; j--){
                if(polyroll.bets(j).isSettled){
                    lastBet = j - 3;
                    break;
                }
            }
        ...
        for(uint256 i = length - dl; i < length; i++){
            if(polyroll.bets(i).modulo > 0 && polyroll.bets(i).isSettled ){
        ...
        if(average.div(adl) < 25){
            return 1 + (uint256(keccak256(
                                       abi.encode(random, 2)
                                       )) % 25);
        }

        if(average.div(adl) < 50){
            return 35 + (uint256(keccak256(
                                       abi.encode(random, 2)
                                       )) % 35);
        }
        
        if(average.div(adl) < 75){
            return 20 + (uint256(keccak256(
                                       abi.encode(random, 2)
                                       )) % 35);
        }
