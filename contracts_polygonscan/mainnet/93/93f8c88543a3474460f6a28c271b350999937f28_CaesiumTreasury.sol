...
// File: contracts/CaesiumTreasury.sol
...
contract CaesiumTreasury is Ownable, ICaesiumTreasury {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SCAES }
    ...
    address public sCAES;
    uint public sCAESQueue; // Delays change to sCAES address
    ...
    constructor (
        ...
        isLiquidityToken[ _CAESDAI ] = true;
        liquidityTokens.push( _CAESDAI );
        bondCalculator[_CAESDAI] = _bondCalculator;

        blocksNeededForQueue = _blocksNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for CAES
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external override returns ( uint send_ ) {
        ...
        // mint CAES needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( CAES ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn CAES for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external override {
        ...
        uint value = valueOfToken( _token, _amount );
        ICAESERC20( CAES ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sCAES ).balanceOf( msg.sender ); // Can only borrow against sCAES held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with CAES
        @param _amount uint
     */
    function repayDebtWithCAES( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        ICAESERC20( CAES ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, CAES, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external override {
        ...
        IERC20Mintable( CAES ).mint( _recipient, _amount );
    ...
    function excessReserves() public override view returns ( uint ) {
        return totalReserves.sub( IERC20( CAES ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns CAES valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOfToken( address _token, uint _amount ) public view override returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match CAES decimals
            value_ = _amount.mul( 10 ** IERC20( CAES ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyOwner() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SCAES ) { // 9
            sCAESQueue = block.number.add( blocksNeededForQueue );
        } else return false;
