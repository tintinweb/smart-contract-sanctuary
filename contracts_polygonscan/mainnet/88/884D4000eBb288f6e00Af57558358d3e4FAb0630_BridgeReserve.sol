...
contract BridgeReserve is BridgeContext {
  ...
  modifier onlyReservable(uint256 _id) {
    (bool initialized, , bool released, ) = bridge.projectState(_id);
  ...
  function reserve(
    ...
    bytes32 node = keccak256(abi.encode(_id, msg.sender, _reserved));
    require(
  ...
  /// @param _id target bridge project id
  /// @param _maxMintPerTransaction maximum allowed number of mints per transaction
  ...
  function setParameters(
    ...
    BridgeBeams.Project memory project = bridge.projects(_id);
    require(_reservedMints <= project.maxSupply, "invalid reserve amount");
    require(_maxMintPerTransaction <= project.maxSupply, "invalid mint max");
  ...
  /// @notice updates the project maxMintPerTransaction
  /// @param _id target bridge project id
  /// @param _maxMintPerTransaction maximum number of mints per transaction
  function setMaxMintPerTransaction(uint256 _id, uint256 _maxMintPerTransaction)
    external
    onlyReservable(_id)
    onlyOwner
  {
    require(_maxMintPerTransaction > 0, "require positive mint");
    projectToParameters[_id].maxMintPerTransaction = _maxMintPerTransaction;
  }
  ...
  function encodePacked(
    ...
    return abi.encode(_id, msg.sender, _reserved);
  }
  ...
  function keccak256Test(
    ...
    return keccak256(abi.encode(_reserved, msg.sender, _id));
  }
}
...
contract BridgeContext is Extractble {
