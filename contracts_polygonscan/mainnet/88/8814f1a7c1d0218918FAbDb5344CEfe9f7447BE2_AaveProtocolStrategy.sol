...
import "../../interfaces/external/aave/IAaveController.sol";
import "../../interfaces/external/aave/ILendingPool.sol";

contract AaveProtocolStrategy is BaseStrategy {
    using SafeERC20 for IERC20;

    /// @notice Address of aToken.
    address public aToken;
    /// @notice Address of Aave controller.
    address public aaveController;
    /// @notice Amount of deposited want tokens.
    uint256 public wantDeposited;

    /// @notice Performs initial setup.
    /// @param _strategyRouter Address of StrategyRouter contract.
    /// @param _want Address of underlying token.
    /// @param _aToken Address of aToken, corresponed to underlying.
    /// @param _controller Address of Aave controller.
    function initialize(
        ...
        super.initialize(
            _want,
            _strategyRouter,
            address(0x8dFf5E27EA6b7AC08EbFdf9eB090F32ee9a30fcf), // Aave LendingPool
            address(0)
        );

        aToken = _aToken;
        aaveController = _controller;
    }
    ...
    /// @notice Deposits tokens to Aave protocol.
    ...
    function deposit(uint256 _amount) external override onlyRole(STRATEGY_ROUTER) {
        ...
        ILendingPool(masterChef).deposit(address(want), _amount, address(this), 0);
        wantDeposited += _amount;
        emit Deposited(_amount);
    }

    /// @notice Withdraws some amount of tokens from Aave protocol.
    /// @param _amount Amount of tokens to withdraw.
    function withdraw(uint256 _amount) external override onlyRole(STRATEGY_ROUTER) {
        require(_amount != 0 && _amount <= wantDeposited, "Wrong amount");

        ILendingPool(masterChef).withdraw(address(want), _amount, address(this));
        wantRewardCollected += (IERC20(want).balanceOf(address(this)) - _amount - wantRewardCollected);
    ...
    /// @notice Withdraws all tokens, staked in Aave protocol.
    ...
    function withdrawAll() external override onlyRole(STRATEGY_ROUTER) returns (uint256, uint256) {
        _harvest();
        ILendingPool(masterChef).withdraw(address(want), type(uint256).max, address(this));

        uint256 balance = wantDeposited;
    ...
    function emergencyWithdraw() external override onlyRole(STRATEGIST_ROLE) {
        ILendingPool(masterChef).withdraw(address(want), type(uint256).max, _msgSender());
        wantDeposited = 0;
        if (wantRewardCollected != 0) {
            want.safeTransfer(_msgSender(), wantRewardCollected);
            wantRewardCollected = 0;
        }
    }

    /// @notice Claims rewards from Aave protocol.
    ...
    /// @notice Returns amount of Wmatic tokens, staked in Aave protocol.
    ...
    function rewardsInEth() external view override returns (uint256) {
        uint256 earnedReward = wantRewardCollected;

        address[] memory tokens = new address[](1);
        tokens[0] = aToken;

        earnedReward += IAaveController(aaveController).getRewardsBalance(tokens, address(this));
        return earnedReward;
    }
    ...
    /// @notice Harvests reward.
    function _harvest() internal override returns (uint256) {
        // Harvest from controller
        uint256 wantBalBefore = want.balanceOf(address(this));
        address[] memory tokens = new address[](1);
        tokens[0] = aToken;
        IAaveController(aaveController).claimRewards(tokens, type(uint256).max, address(this));
        uint256 wantBalAfter = want.balanceOf(address(this));

        uint256 harvested = wantBalAfter - wantBalBefore;
        wantRewardCollected += harvested;
        // Harvest from aToken
        wantBalBefore = want.balanceOf(address(this));
        ILendingPool(masterChef).withdraw(address(want), type(uint256).max, address(this));
        wantBalAfter = want.balanceOf(address(this));

        uint256 harvested2 = want.balanceOf(address(this)) - wantRewardCollected - wantDeposited;
        wantRewardCollected += harvested2;

        want.safeApprove(masterChef, 0);
        want.safeApprove(masterChef, wantDeposited);
        ILendingPool(masterChef).deposit(address(want), wantDeposited, address(this), 0);

        return harvested + harvested2;
    }

    /// @notice Transfers rewards to ClustersLock contract.
    /// @param _rewardAmount Amount of collected reward.
    function _transferRewards(uint256 _rewardAmount) internal override {
        wantRewardCollected -= _rewardAmount;
        want.safeTransfer(_msgSender(), _rewardAmount);

        emit RewardsTransfered(_rewardAmount);
    }
