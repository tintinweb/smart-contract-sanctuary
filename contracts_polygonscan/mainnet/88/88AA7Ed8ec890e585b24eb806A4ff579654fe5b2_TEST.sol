 ...
 */
contract TEST is ReentrancyGuard, OwnableUpgradeable {
    ...
    function setIAOStatus(uint _status) external onlyOwner {
        status = _status;
    }
    ...
    function lockWithRef(uint256 _amount, address _referrer) public onlyActiveIAO nonReentrant {
        ...
        require(_amount <= maxStakeAmount, "_amount must <= max");
    ...
    /// @dev Deposit ERC20 tokens with support for reflect tokens
    function lockByBUSD(address _userAddress, uint256 _amount, address _referrer, bytes32 _tx) external onlyAdmin {
