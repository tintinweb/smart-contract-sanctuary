...
// d8888b.  .d8b.  d8888b. d8888b. d88888b d8888b. .d8888. db   db  .d88b.  d8888b. 
// 88  `8D d8' `8b 88  `8D 88  `8D 88'     88  `8D 88'  YP 88   88 .8P  Y8. 88  `8D 
// 88oooY' 88ooo88 88oobY' 88oooY' 88ooooo 88oobY' `8bo.   88ooo88 88    88 88oodD' 
// 88~~~b. 88~~~88 88`8b   88~~~b. 88~~~~~ 88`8b     `Y8b. 88~~~88 88    88 88~~~   
// 88   8D 88   88 88 `88. 88   8D 88.     88 `88. db   8D 88   88 `8b  d8' 88      
// Y8888P' YP   YP 88   YD Y8888P' Y88888P 88   YD `8888Y' YP   YP  `Y88P'  88      
                                                                                 
                                                                                 
//  d888b   .d8b.  .88b  d88. d88888b .d8888.                                       
// 88' Y8b d8' `8b 88'YbdP`88 88'     88'  YP                                       
// 88      88ooo88 88  88  88 88ooooo `8bo.                                         
// 88  ooo 88~~~88 88  88  88 88~~~~~   `Y8b.                                       
// 88. ~8~ 88   88 88  88  88 88.     db   8D                                       
//  Y888P  YP   YP YP  YP  YP Y88888P `8888Y'        
...
// File: contracts/HairGameHair.sol
...
- Transaction Mining: Contract is linked to TransactionMiner contract to award HAIR tokens to gamblers who lost their bets.
...
*/
contract HairGameHair is VRFConsumerBase, Ownable, ReentrancyGuard {
    ...
    address public constant GAME_TOKEN = 0x100A947f51fA3F1dcdF97f3aE507A72603cAE63C;
    ...
    // Each bet is deducted 300 basis points (3%) in favor of the house
    uint public houseEdgeBP = 300;
    ...
    uint public minBetAmount = 100 ether;
    ...
    // Events
    event BetPlaced(uint indexed betId, address indexed gambler, uint amount, uint readibleBet, uint8 indexed modulo, uint8 rollUnder, uint40 mask);
    event BetSettled(uint indexed betId, address indexed gambler, uint amount, uint readibleBet, uint8 indexed modulo, uint40 mask, uint outcome, uint winAmount, uint rollReward);
    ...
    // Set maximum HAIR reward that a user can receive per bet.
    function setMaxReward(uint _maxReward) external onlyOwner {
        maxReward = _maxReward;
    }

    // Place bet
    function placeBet(uint256 amount, uint256 readibleBet, uint betMask, uint modulo, address referrer) external nonReentrant {
        ...
        emit BetPlaced(bets.length, msg.sender, amount, readibleBet, uint8(modulo), uint8(rollUnder), uint40(mask));	
        // Store bet in bet list.	
        bets.push(Bet(	
            {	
                amount: amount,	
                readibleBet: readibleBet,	
                modulo: uint8(modulo),	
    ...
    // Expected HAIR tokens to be rewarded if lose bet.
    function getRollReward(uint amount, uint modulo, uint rollUnder) private view returns (uint) {
        // HAIR reward equals house edge fees, divided by win probability, multiplied by rewardPct.
    ...
    function settleBet(bytes32 requestId, uint randomNumber) internal nonReentrant {
        ...
        uint readibleBet = bet.readibleBet;
        ...
        // Send prize to winner, add HAIR reward to loser, and update house profit.
        if (winAmount > 0) {
        ...
        emit BetSettled(betId, gambler, amount, readibleBet, uint8(modulo), bet.mask, outcome, winAmount, rollReward);
    }
