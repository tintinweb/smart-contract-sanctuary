...
contract VaultKlimaStaking is Vault {
    ...
    constructor(
        ...
        stakingAddress = sh.staking();
        stakingToken = IKlimaStaking(stakingAddress).sKLIMA();

        transferOwnership(beliFarmAddress);
    }

    event Mockswap (address _uniRouter, uint _amountIn, uint _slippageFactor, address[] _path);
    function _safeSwap(address _uniRouterAddress, uint _amountIn, uint _slippageFactor, address[] memory _path, address _to, uint _deadline) internal virtual override {
        emit Mockswap(_uniRouterAddress, _amountIn, _slippageFactor, _path);
    }

    function convertDustToEarned() public virtual override whenNotPaused {
    }
    ...
    function rebaseInterest() public view virtual returns (uint256) {
        return _unfarmEarnAmount();
    }

    function _unfarm(uint256 _wantAmt) virtual internal override {
        IStaking staking = IStaking(stakingAddress);
        IERC20 sOHM = IERC20(stakingToken);
        sOHM.safeIncreaseAllowance(stakingAddress, _wantAmt);
        staking.unstake(_wantAmt, false);
    }

    function setWantLockedTotal(uint _t) public virtual {
        wantLockedTotal = _t;
    }

    function _unfarmEarnAmount() internal view virtual override returns (uint256) {
        IERC20 sOHM = IERC20(stakingToken);
