...
contract ImpulseStaking is AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    struct UserInfo {
        ...
        uint256 deposit; // How many ASSET tokensens the user has provided.
    ...
    function deposit(uint256 _pid, uint256 _amount) public virtual nonReentrant hasPool(_pid) poolRunning(_pid) {
        ...
        IImpulseStrategy strategy = IImpulseStrategy(pool.strategy);
        IERC20(pool.assetToken).safeApprove(pool.strategy, 0);
        IERC20(pool.assetToken).safeApprove(pool.strategy, _amount);
        uint256 sharesAmount = strategy.deposit(_amount);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public virtual nonReentrant poolRunning(_pid) hasPool(_pid) {
        ...
        require(user.shares > 0 && user.deposit >= _amount, "withdraw: wrong amount");
        uint256 poolAmountBefore = user.shares;
        IImpulseStrategy strategy = IImpulseStrategy(pool.strategy);
        uint256 userTotalAmount = ((user.shares * strategy.wantLockedTotal()) / strategy.sharesTotal());
        uint256 withdrawTotalAmount = (_amount * userTotalAmount) / user.deposit;
        uint256 sharesAmount = strategy.withdraw(withdrawTotalAmount);
        user.shares -= sharesAmount;
        user.deposit -= _amount;
        IERC20(pool.assetToken).safeTransfer(_msgSender(), withdrawTotalAmount);

        for (uint256 i = 0; i < pool.rewardsTokens.length; i++) {
            _updateUserInfo(pool, user, i, poolAmountBefore);
        }
        poolInfo[_pid].poolSupply -= sharesAmount;
        emit Withdraw(_msgSender(), _pid, _amount, withdrawTotalAmount);
    }
    ...
    function userPoolAmountInUsd(uint256 _pid, address _user) public view returns (uint256) {
        ...
        IImpulseStrategy strategy = IImpulseStrategy(pool.strategy);
        UserInfo memory user = userInfo[_pid][_user];
        return strategy.wantPriceInUsd(user.deposit);
    }
    ...
    function yieldBalance(uint256 _pid, address _user) external view virtual returns (uint256) {
        ...
        UserInfo memory user = userInfo[_pid][_user];
        return ((user.shares * strategy.wantLockedTotal()) / strategy.sharesTotal()) - user.deposit;
    }
