...
contract IDO_NFT_FCFS is Initializable {
   ...
   //people will have a max mint per round. So when tier 3 round starts only tier 3 people can mint and they can mint up to 10
    //once tier 2 can mint, they can mint 10 max, but tier 3 holders can now mint another 10
    //once tier 1 can mint 10, tier 2 and tier 3 can mint another 10
    //once public can mint their is no mint cap 
    function join(
        uint256 _amount
    ) external {
        ...
        fundsRaised += saleTokensOwed;
        //emit some events for the front end
    }
    ...
    function getMaxMint(address _caller) public view returns(uint maxMint) {
        if(block.timestamp < (Agreement.saleStart() + Agreement.roundDelay(0))){//need to look at tiers
            ...
            maxMint = 1000000000;//user mint is only limited by block gas limit
        }
    }
    ...
    function withdraw() external onlySudo {
        ...
        if(gravityCommission > Agreement.commissionCap()){
            gravityCommission = Agreement.commissionCap();
        }
