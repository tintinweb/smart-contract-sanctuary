...
contract IceIronStrategy is BaseStrategyIron {

    address public constant ICE_IRON_LP = 0x7e2cc09d3d36b3aF6EdfF55B214fd62885234E95;
    address public constant IRON = 0xD86b5923F3AD7b585eD81B448170ae026c65ae9a;

    address[] public ICE_IRON_path;

    constructor() public BaseStrategyIron(ICE_IRON_LP, DFYN_ROUTER, 4) {
        ICE_IRON_path = [ICE, IRON];

        IERC20(IRON).safeApprove(DFYN_ROUTER, uint256(-1));
    }
    ...
    function harvest() public override onlyHumanOrWhitelisted nonReentrant {
        ...
        //Swap 1/2 of remaining ICE for IRON
        uint256 remainingHarvested = IERC20(harvestedToken).balanceOf(address(this));
        if (remainingHarvested > 0) {
            _swapUniswapWithPathPreapproved(ICE_IRON_path, remainingHarvested.div(2), DFYN_ROUTER);
        }

        uint256 harvestedTokenBalance = IERC20(harvestedToken).balanceOf(address(this));
        uint256 otherBalance = IERC20(IRON).balanceOf(address(this));
