...
// *******************************************************************************************
//  Stand  a chance to win 100 Matic just by owning your own jazz pharaoh!

//         after every 100 mints we'll pick a random number.
//      And the owner of that token will be awarded with 100 Matic.
//             Join our discord for daily updates.
// 
// project: https://holanext.com/pharaohs
// discord: https://discord.gg/zeSSwzwqzR
// *******************************************************************************************

contract JAZZPharaohs is ERC721, ERC721Enumerable, VRFConsumerBase {
    ...
    uint256 public toAdd;

    uint256 internal randomNo;
    uint256 internal factor = 111;
    ...
    {
        unitPrice = 5 ether;
        // VRF public keyhash for polygon Mainnet
    ...
    function mint(uint256 amount, address reciever) public payable notPaused{
        ...
        require((mintCounter + amount) <= maxSupply);
        if (msg.sender != admin) {
    ...
    //getting random number from chainlink oracle and assingning a unique winner on every 100 mints.

    function getRandomNumber() external onlyOwner returns (bytes32 requestId) {
        uint256 fee = 0.0001 * 1 ether;
        require(
    ...
    function changeBase(string memory base_) external onlyOwner notPaused{
        base = base_;
    }

    function changePrice(uint256 newPriceInMatic) external onlyOwner notPaused{
        unitPrice = newPriceInMatic * 1 ether;
    }
    ...
    function withdraw() external onlyOwner notPaused{
        ...
        require(address(this).balance > prizeMoney * 1 ether);

        uint256 withdrawableAmount = address(this).balance - (prizeMoney * 1 ether);

        (bool os, ) = payable(admin).call{value: withdrawableAmount}("");
        require(os);
    }
    ...
    function withdrawPrize(uint256 id) external notPaused{
        ...
        didWin[id] = false;
        prizeMoney -= 100;
        (bool os, ) = payable(msg.sender).call{value: 100 ether}("");
        require(os);
    }
}
