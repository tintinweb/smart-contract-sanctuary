...
// WiseToken.
contract WiseToken is BEP20('PolyWise Finance', 'WISE') {
...
// MasterChef is the master of WISE. He can make Wise and he is a fair guy.
//
...
contract PolyWiseMasterChef is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of WISE
    ...
    struct PoolInfo {
        ...
        uint256 accWisePerShare;   // Accumulated WISEs per share, times 1e18. See below.
        uint16 depositFeeBP;      // Deposit fee in basis points
        uint256 lpSupply;
    }

    // The WISE TOKEN!
    WiseToken public immutable wise;
    // Dev address.
    ...
    // Maximum WisePerBlock
    uint256 public MAX_EMISSION_RATE = 0.5 ether;
    ...
    event UpdateEmissionRate(address indexed user, uint256 WisePerBlock);
    ...
    event WiseMintError(bytes reason);
    ...
    // Update the given pool's WISE allocation point and deposit fee. Can only be called by the owner.
    ...
    function pendingWise(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && pool.lpSupply != 0 && totalAllocPoint > 0) {
            ...
            accWisePerShare = accWisePerShare.add(wiseReward.mul(1e18).div(pool.lpSupply));
        }
        return user.amount.mul(accWisePerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accWisePerShare = pool.accWisePerShare.add(wiseReward.mul(1e18).div(pool.lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accWisePerShare).div(1e18).sub(user.rewardDebt);
            if (pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accWisePerShare).div(1e18);
    ...
    function safeWiseTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > wiseBal) {
            transferSuccess = wise.transfer(_to, wiseBal);
        } else {
            transferSuccess = wise.transfer(_to, _amount);
        }
        require(transferSuccess, "safeWiseTransfer: transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _WisePerBlock) external onlyOwner {
        require(_WisePerBlock <= MAX_EMISSION_RATE, "Too high");
        massUpdatePools();
        WisePerBlock = _WisePerBlock;
        emit UpdateEmissionRate(msg.sender, _WisePerBlock);
    }
