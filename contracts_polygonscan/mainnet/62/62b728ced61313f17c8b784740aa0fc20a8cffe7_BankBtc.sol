...
interface BankConfig {
  /// @dev Return minimum wbtc debt size per position.
  ...
  /// @dev Return the work factor for the bigfoot + wbtc debt, using 1e4 as denom. Revert on non-bigfoot.
  function workFactor(address bigfoot, uint debt) external view returns (uint);

  /// @dev Return the kill factor for the bigfoot + wbtc debt, using 1e4 as denom. Revert on non-bigfoot.
...
interface Bigfoot {
  /// @dev Work on a (potentially new) position. Optionally send wbtc back to Bank.
  function work(
    uint id,
    address user,
    uint debt,
  ...
  /// @dev Return the amount of wbtc wei to get back if we are to liquidate the position.
  function health(uint id) external view returns (uint);

  /// @dev Liquidate the given position to wbtc. Send all wbtc back to Bank.
...
contract BankBtc is Initializable, ERC20, ReentrancyGuardUpgradeSafe, Governable {
  /// @notice Libraries
  ...
  address constant public vault = address(0x5Ed521d568AC1ac7522eEA64decc7D1c0174b28E);
  ...
  function initialize(BankConfig _config) external initializer {
    ...
    lastAccrueTime = now;
    nextPositionID = 1;
    name = 'eleven interest bearing WBTC';
    symbol = 'eleWBTC';
    decimals = 8;
    approve();
  }
  ...
  function additionalApprove(address _token, address _spender) onlyGov external{
    require(_token != wbtc && _token != vault, "no rugs allowed");
  ...
  // @dev transfer from wbtc and/or 11unusedwbtc with no errors
  function safeTransferBankCurrency(address _receiver, uint _amount) internal{
    uint inside = wbtc.myBalance();
    if(inside>=_amount) wbtc.safeTransfer(_receiver, _amount);
    else {
      safeWithdrawFromFarm(_amount.sub(inside));
      wbtc.safeTransfer(_receiver, wbtc.myBalance());
    }
  }
  

  // @dev Total wbtc inside the contract wether on the vault or not
  function wbtcBalance() public view returns (uint) {
    return wbtc.myBalance().add(insideVaultBankCurrency());
  }
  ...
  function pendingInterest(uint msgValue) public view returns (uint) {
    if (now > lastAccrueTime) {
      ...
      uint balance = wbtcBalance().sub(msgValue);
  ...
  /// @dev Return the wbtc debt value given the debt share. Be careful of unaccrued interests.
  ...
  /// @dev Return wbtc value and debt of the given position. Be careful of unaccrued interests.
  ...
  /// @dev Return the total wbtc entitled to the token holders. Be careful of unaccrued interests.
  function totalBankCurrency() public view returns (uint) {
    return wbtcBalance().add(glbDebtVal).sub(reservePool);
  }
  


  /// @dev Add more wbtc to the bank. Hope to get some good returns.
  function deposit(uint[] calldata amounts) external accrue(0)  nonReentrant {
    uint total = totalBankCurrency();
    uint diff = transferWbtc(amounts);
    uint share = total == 0 ? diff : diff.mul(totalSupply()).div(total);
    _mint(msg.sender, share);
    require(totalSupply() >= 1e3, 'deposit: total supply too low');
    depositToVault();
  }

  /// @dev Withdraw wbtc from the bank by burning the share tokens.
  function withdraw(uint share, uint8) external accrue(0) nonReentrant {
    uint amount = share.mul(totalBankCurrency()).div(totalSupply());
    require(amount <= wbtcBalance(), "Utilization too high, withdraw an smaller amount");
    _burn(msg.sender, share);
    safeTransferBankCurrency(msg.sender, amount);
    uint supply = totalSupply();
    require(supply == 0 || supply >= 1e3, 'withdraw: total supply too low');
  }
  ...
  // @dev Deposit dollars, convert to wbtc and stake in vault
  function transferWbtc(uint[] memory amounts) internal returns(uint){
    uint bfrwbtc = wbtc.myBalance();
    if(amounts[0]>0) wbtc.safeTransferFrom(msg.sender, address(this), amounts[0]);
    uint aftrwbtc = wbtc.myBalance();
    return (aftrwbtc.sub(bfrwbtc));
  }
  ...
  /// @param loan The amount of wbtc to borrow from the pool.
  /// @param maxReturn The max amount of wbtc to return to the pool.
  ...
  function work(
    ...
    uint back;
    {
      uint sendWbtc = transferWbtc(amounts).add(loan);
      require(sendWbtc <= wbtcBalance(), 'insufficient WBTC in the bank');
      uint beforeWbtc = wbtcBalance().sub(sendWbtc);
      safeTransferBankCurrency(bigfoot, sendWbtc);
      Bigfoot(bigfoot).work(id, msg.sender, debt, data);
      back = wbtcBalance().sub(beforeWbtc);
    }
    ...
    require(glbDebtShare >= 1e3, 'remaining global debt share too small');
    require(glbDebtVal >= 1e3, 'remaining global debt value too small');
  }
  ...
  function kill(uint id) external onlyEOA accrue(0) nonReentrant {
    ...
    // 2. Perform liquidation and compute the amount of wbtc received.
    uint beforeWbtc = wbtcBalance();
    Bigfoot(pos.bigfoot).liquidate(id);
    uint back = wbtcBalance().sub(beforeWbtc);
  ...
  /// @dev Withdraw wbtc reserve for underwater positions to the given address.
  /// @param to The address to transfer wbtc to.
  /// @param value The number of wbtc tokens to withdraw. Must not exceed `reservePool`.
  function withdrawReserve(address to, uint value) external onlyGov nonReentrant {
    reservePool = reservePool.sub(value);
    require(value<=wbtcBalance(), "not enough funds");
    safeTransferBankCurrency(to, value);
  }

  
  /// @dev Reduce wbtc reserve, effectively giving them to the depositors.
  /// @param value The number of wbtc reserve to reduce.
