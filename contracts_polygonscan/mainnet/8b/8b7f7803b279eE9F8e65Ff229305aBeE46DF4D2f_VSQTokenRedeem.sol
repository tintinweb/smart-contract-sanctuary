...
contract VSQTokenRedeem is Ownable, ReentrancyGuard {
    ...
    event VSQRecovery(address recipient, uint256 recoveryAmount);
    ...
    // Recover VSQ in case of error, only owner can use.
    function recoverVSQ(address recipient, uint256 recoveryAmount) external onlyOwner {
        if (recoveryAmount > 0)
            ERC20(VSQAddress).safeTransfer(recipient, recoveryAmount);
        
        emit VSQRecovery(recipient, recoveryAmount);
    }
}
