...
contract eaEons is ERC20, Ownable {
  ...
  // ADD _vault address, _controller address
  constructor(address _aToken) ERC20('Eons Interest Bearing Token', 'eEONS') {
  ...
  // ONLY FOR TESTING
  // Update balance to simulate total aTokens
  function newTester(address _tester)
    external
    onlyOwner
  {
    tester = _tester;
    ...
    returns(uint256) 
  {
    ...
    if (eTotalSupply() != 0) {
      // ni = a/(e*i) - WAD + i
      return((aToken.balanceOf(tester).wdiv(eTotalSupply().wmul(i))) + i - WAD);
    } else {
