...
import "../interopBeta/main.sol";
import "./interfaces.sol";

contract InteropBetaFlashloanImplementation is InteropBetaImplementation {
    ...
    // Constant Variables
    uint256 public constant instaFeeBPS = 0; // 0% fee
    address public immutable aggregatorAddr;
    address public immutable wChainTokenAddr;
    // Constant Variables

    constructor(
        address instaIndex_,
        address aggregatorAddr_,
        address wChainTokenAddr_
    ) InteropBetaImplementation(instaIndex_) {
        status = 1;
        aggregatorAddr = aggregatorAddr_;
        wChainTokenAddr = wChainTokenAddr_;
    }

    function initializeStatus() public {
        require(status == 0, "status-already-initialized");
        status = 1;
    }
    ...
    function initiateInstaLoan(
        ...
        require(msg.sender == aggregatorAddr, "caller-not-FLA");
        require(length_ == amounts_.length, "not-equal-length");

        uint256[] memory preLoanBalances_ = new uint256[](length_);
        IERC20 previousToken_ = IERC20(address(0));
        uint256 chainTokenAmountToConvert_;

        for (uint256 i = 0; i < length_; ++i) {
            IERC20 token_ = IERC20(tokens_[i]);
            uint256 amount_ = amounts_[i];

            require(token_ > previousToken_, token_ == IERC20(address(0)) ? "zero-address-passed" : "unsorted-tokens-passed");
            previousToken_ = token_;

            preLoanBalances_[i] = token_.balanceOf(address(this));

            if (tokens_[i] == wChainTokenAddr) {
                if(preLoanBalances_[i] >= amount_) {
                    token_.safeTransfer(receiverAddress_, amount_);
                } else if (preLoanBalances_[i] + address(this).balance >= amount_) {
                    chainTokenAmountToConvert_ = amount_ - preLoanBalances_[i];
                    WChainTokenInterface wChainToken_ = WChainTokenInterface(wChainTokenAddr);
                    wChainToken_.deposit{value: chainTokenAmountToConvert_}();
                    token_.safeTransfer(receiverAddress_, amount_);
                } else {
                    revert("insufficient-balance-for-flashloan");
                }
            } else {
                require(preLoanBalances_[i] >= amount_, "insufficient-balance-for-flashloan");
                token_.safeTransfer(receiverAddress_, amount_);
            }
        }

        // fee is zero.
        uint256[] memory feeAmounts_ = new uint256[](length_);
        InstaFlashReceiverInterface(receiverAddress_).onInstaLoan(tokens_, amounts_, feeAmounts_, msg.sender, data_);

        for (uint256 i = 0; i < length_; ++i) {
            if (tokens_[i] == wChainTokenAddr) {
                WChainTokenInterface wChainToken_ = WChainTokenInterface(wChainTokenAddr);
                uint256 preLoanBalance_ = preLoanBalances_[i] + chainTokenAmountToConvert_;
                uint256 postLoanBalance_ = wChainToken_.balanceOf(address(this));
                require(postLoanBalance_ >= preLoanBalance_ + feeAmounts_[i], "less-amount-returned");
                wChainToken_.withdraw(chainTokenAmountToConvert_);
            } else {
                IERC20 token_ = IERC20(tokens_[i]);
                uint256 preLoanBalance_ = preLoanBalances_[i];
                uint256 postLoanBalance_ = token_.balanceOf(address(this));
                require(postLoanBalance_ >= preLoanBalance_ + feeAmounts_[i], "less-amount-returned");
            }
        }
...
interface WChainTokenInterface is IERC20 {
