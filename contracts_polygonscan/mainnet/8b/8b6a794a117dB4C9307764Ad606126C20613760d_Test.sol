...
contract Test is Ownable, PolygonArbitrageBot {
    ...
    function hitMe(uint256 _amount) external view
    {
        _polygonArbitrageBotPerformSwapIfValuable(kPolygonToken.wmatic, kPolygonToken.pbrew, _amount);
    ...
    function getWMATICpBREW() view external returns(PolygonArbitrageOptionABICompatible memory)
    {
        PolygonArbitrageOption memory option = _polygonArbitrageBotFindOption(kPolygonToken.wmatic, kPolygonToken.pbrew, 10000);
...
contract PolygonArbitrageBot is PolygonTradingHub {
    ...
    uint256 constant minPercentage1000ForTrade = 650;
    ...
    function _polygonArbitrageBotPerformSwapIfValuable(kPolygonToken _tokenA, kPolygonToken _tokenB, uint256 _amount) internal view returns(bool)
    {
        PolygonArbitrageOption memory option = _polygonArbitrageBotFindOption(_tokenA, _tokenB, _amount);
        ...
        if (option.tokenAmountDifferencePercentage1000 < minPercentage1000ForTrade)
        {
            return false;
        }

        // uint256 initialBalanceA = PolygonTokens.getBalance(_tokenA);
        // uint256 initialBalanceB = PolygonTokens.getBalance(_tokenB);

        // _polygonTradingHubSwapToken(option.max.exchange, _tokenA, _tokenB, _amount);

        // uint256 newBalanceB = PolygonTokens.getBalance(_tokenB);

        // _polygonTradingHubSwapToken(option.min.exchange, _tokenB, _tokenA, newBalanceB - initialBalanceB);

        // uint256 newBalanceA = PolygonTokens.getBalance(_tokenA);
...
contract PolygonTradingHub is QuickSwapTrader, CafeSwapTrader, JetSwapTrader {
    ...
    function _polygonTradingHubSwapToken(kPolygonExchange _exchange, kPolygonToken _tokenIn, kPolygonToken _tokenOut, uint256 _amount) internal 
    {
        if (_exchange == kPolygonExchange.quickSwap)
        {
            _quickSwapSwapToken(_tokenIn, _tokenOut, _amount);
        ...
        {
            _cafeSwapSwapToken(_tokenIn, _tokenOut, _amount);
        ...
        {
            _jetSwapSwapToken(_tokenIn, _tokenOut, _amount);
