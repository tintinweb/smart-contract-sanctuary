...
contract HuanCasino is LoserGameRankData {
  ...
  mapping (uint => uint) public totalWinnerMap_20;
  mapping (uint => uint) public totalWinnerMap_50;
...
//get tones token
  ...
  function getTotalTonesToken(uint level, uint tonesIndex) public view returns(uint) {
    if (level == 1) {
      ...
      return totalWinnerMap_20[tonesIndex];
    } else if (level == 5) {
      return totalWinnerMap_50[tonesIndex];
    } else {
      return 0;
    }
  }
  ...
  function distributePrizes_20(uint256 numberWin) private {
    ...
    for (uint256 i = 0; i < players_20.length; i++) {
      ...
      if (playerInfo_20[playerAddress].numberSelected == numberWinner_20) {
        ...
        totalWinnerMap_20[numberWinner_20] += 1;
        countWin++;
      } else {
        losers[countLose] = playerAddress;
        countLose++;
  ...
  function distributePrizes_50(uint256 numberWin) private {
    ...
    for (uint256 i = 0; i < players_50.length; i++) {
      ...
      if (playerInfo_50[playerAddress].numberSelected == numberWinner_50) {
        ...
        totalWinnerMap_50[numberWinner_50] += 1;
        countWin++;
      } else {
        losers[countLose] = playerAddress;
        countLose++;
