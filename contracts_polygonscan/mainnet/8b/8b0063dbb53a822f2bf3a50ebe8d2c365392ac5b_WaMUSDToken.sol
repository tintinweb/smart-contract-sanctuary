...
// File contracts/WaMUSDToken.sol
...
/// @title waMUSD
///
/// @dev This is the contract for the YumVault utillity token usd.
///
...
contract WaMUSDToken is AccessControl, ERC20("Wasabi mUSD", "waMUSD") {
  ...
  modifier onlyWhitelisted() {
    require(whiteList[msg.sender], "waMUSD: YumVault is not whitelisted");
    _;
  }
  ...
  function mint(address _recipient, uint256 _amount) external onlyWhitelisted {
    require(!blacklist[msg.sender], "waMUSD: YumVault is blacklisted.");
    uint256 _total = _amount.add(hasMinted[msg.sender]);
    require(_total <= ceiling[msg.sender],"waMUSD: YumVault's ceiling was breached.");
    require(!paused[msg.sender], "waMUSD: user is currently paused.");
  ...
  /// This function reverts if the caller does not have the sentinel role.
  function pauseYumVault(address _toPause, bool _state) external onlySentinel {
  ...
  /**
    ...
    /**
     * @dev lowers hasminted from the caller's allocation if needed
     */
    function lowerHasMintedIfNeeded(uint256 amount) public onlyWhitelisted {
