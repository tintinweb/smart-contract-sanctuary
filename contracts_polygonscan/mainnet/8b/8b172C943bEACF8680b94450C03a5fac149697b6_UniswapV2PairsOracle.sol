...
// SPDX-License-Identifier: MIT
// Note: Using 0.6.6 version to keep compatibility with UniswapV2 
pragma solidity =0.6.6;
...
import "./interfaces/IPairsOracle.sol";

// fixed window oracle that recomputes the average price for the entire period once every period
// note that the price average is only guaranteed to be over at least 1 period, but may be over a longer period
contract UniswapV2PairsOracle is Initializable, IPairsOracle {
    using FixedPoint for *;
    ...
    function addPair(address tokenA, address tokenB) public override returns (bool) {
        ...
        require(prices[address(_pair)].blockTimestampLast == 0, "UniswapV2PairsOracle: Pair already added");
        ...
        returns (uint256 amountOut)
    {
        ...
        if (tokenIn == IUniswapV2Pair(pair).token0()) {
            ...
            require(tokenIn == IUniswapV2Pair(pair).token1(), "UniswapV2PairsOracle: INVALID_TOKEN");
            amountOut = prices[pair].price1Average.mul(amountIn).decode144();
        }
    }

    function pairFor(address tokenA, address tokenB) private view returns (address) {
