...
contract TinyEggStats is IEggStats, OwnableUpgradeable {
    address public nft;
    ...
    mapping(uint256 => EggInfo) private info_;
    ...
    function initialize(address _nft, address _factory) external initializer {
        require(_nft == IEggFactory(_factory).nft(), "wrong nft & factory");
        __Ownable_init();
        nft = _nft;
        factory = _factory;
    ...
    function info() external override view returns (uint256 totalSupply, uint256 totalBurned) {
        ...
        totalBurned = IEggNFT(nft).totalBurned();
    }

    function stats(uint256 tokenId) external override view returns (EggInfo memory eggStats) {
        EggInfo memory eggStats = info_[tokenId];
    }

    function details(uint256 tokenId) external override view returns (EggInfo memory info) {
        info = info_[tokenId];
    }

    function expectedPrice(uint256 tokenId) external override view returns (uint256 _price) {
        EggInfo memory _info = info_[tokenId];
        _price = priceDistribute_[2]; // EPIC;
        if (_info.Rarity == IEggStats.EggRarity.COMMON) {
            _price = priceDistribute_[0];
        } else if (_info.Rarity == IEggStats.EggRarity.RARE) {
            _price = priceDistribute_[1];
        }
    }

    /* ========== MUTATIVE FUNCTIONS ========== */

    function setInfo(uint256 tokenId, EggInfo memory newInfo) external override onlyAdmin {
