...
abstract contract StrategyRebalanceUniV3Solo {
    ...
    function whitelistHarvesters(address[] calldata _harvesters) external {
        require(
            msg.sender == owner || harvesters[msg.sender],
            "not authorized"
        );
    ...
    function setManipulationResistance(int24 _maxDeviation, uint24 _twapTime)
        external
    {
        require(msg.sender == owner, "!owner");
    ...
    function sweep() public checkDeviation onlyBenevolent {
        require(msg.sender == owner, "!owner");
    ...
    function withdrawAll() external checkDeviation {
        require(msg.sender == owner, "!owner");
        ...
        returns (uint256 _tokenId)
    {
        if (tokenId != 0) {
            ...
            if (_liquidity > 0) {
                (uint256 _liqAmt0, uint256 _liqAmt1) = nftManager
                .decreaseLiquidity(
                    IUniswapV3PositionsNFT.DecreaseLiquidityParams({
                        tokenId: tokenId,
                        liquidity: uint128(_liquidity),
