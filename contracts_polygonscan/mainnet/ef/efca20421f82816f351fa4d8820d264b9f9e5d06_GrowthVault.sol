...
contract GrowthVault is ERC20, Ownable {
    ...
    uint256 public constant toleranceLevelPercent = 5; // 0.05% 
    ...
    function autoCompound() public {
        ...
        for (uint256 i; i < names.length; i++) {
            ...
            if (_amount > 0) {
                ...
                _swapExactETHForTokens(_getBalanceOfToken(WETH), strategyObj.liquidityRouter, path);
    ...
    function deposit() public payable {
        autoCompound();
        uint256 totalBNB = getTotalCapital();
        _addLiquidity(liquidityRouter, token0, token1);

        // uint256 shares = msg.value; // Check with percent
        // if (totalBNB > 0) {
        //     uint256 percent = (msg.value * 100 * 100) / (totalBNB * 100);
        //     shares = percent * totalBNB;
        // }
    ...
    function withdraw(uint256 _shares) canWithdraw public {
        uint256 totalBNB = getTotalCapital();
        uint256 percent = (_shares * 100 * 100)/(totalBNB * 100);
    ...
    function _getAmountLPFromStrategy(address _farm, uint256 _pid) internal view returns (uint256 amount) {
    ...
    function _addLiquidity(address _liquidityRouter, address _token0, address _token1) internal {
        ...
        if(_token0 != WETH){
            path[1] = _token0;
            _swapExactETHForTokens(_balance/2, _liquidityRouter, path);
        }
        ...
        uint256 desired1 = _getBalanceOfToken(_token1);

        require(desired0 != 0 && desired1 != 0, "Desired value is zero");

        // _approveTokens(_token0, _token1, _liquidityRouter);
        // // get new LP from new token0 and token1
        // IRouter(_liquidityRouter).addLiquidity(
        //     _token0,
        //     _token1,
        //     desired0,
        //     desired1,
        //     desired0 - (desired0*toleranceLevelPercent*1000/(100*100*100)),
        //     desired1 - (desired1*toleranceLevelPercent*1000/(100*100*100)),
        //     address(this),
    ...
    function _addStrategy(
        ...
        (address _token0, address _token1) = _getTokensFromLP(strategyObj.lpToken);
    ...
    function _approveTokens(address _token0, address _token1, address _liquidityRouter) internal {
        IERC20(_token0).approve(_liquidityRouter, type(uint256).max);
        IERC20(_token1).approve(_liquidityRouter, type(uint256).max);
    }
    ...
    function _swapExactETHForTokens(
        uint256 _amount, address _liquidityRouter, address[] memory _path) public payable {

        IRouter(_liquidityRouter).swapExactETHForTokens{value:_amount}(
            _amount, _path, address(this), block.timestamp + 1 minutes
        );
    }
    ...
    function _getYELs(uint256 _percent) internal { 
        ...
        for (uint256 i; i < names.length; i++) {
            ...
            path[0] = WETH;
            path[1] = YELtoken;
            _swapExactETHForTokens(_getBalanceOfToken(WETH), yelLiquidityRouter, path);
        }
    ...
    function _getAmountMin(
        ...
        amountMin = _amount - (_amount * toleranceLevelPercent*1000)/(100*100*100);
    }
    ...
    function _getAmountOfToken(
        ...
        address _tokenTo,
        address _liquidityRouter,
        address[] memory _path) internal view returns (uint256) {
