...
contract Barn is Ownable, IERC721Receiver, Pausable {
   ...
   * @param _swool reference to the $SWOOL token
   */
  constructor(address _woolf, address _swool) { 
    woolf = Woolf(_woolf);
    wool = SWOOL(_swool);
  }
  ...
  function addManyToBarnAndPack(address account, uint16[] calldata tokenIds) external {
    ...
    // uint256[] memory shakedList = listShaked[account];
  ...
  function claimManyFromBarnAndPack(uint16[] calldata tokenIds, bool unstake) external whenNotPaused _updateEarnings {
    ...
    wool.burn(_msgSender(), owed.mul(50).div(100));
  }
  ...
  function _claimSheepFromBarn(uint256 tokenId, bool unstake) internal returns (uint256 owed) {
    ...
    if (unstake) {
      ...
      _addUnShakedList(_msgSender(),tokenId);
 ...
 function _delUnShakedList(address account, uint256 tokenId) internal {
    uint8  delIndex=0;
    bool exist=false;
    for( uint8 i=0;  i<unListShaked[account].length;i++)
      {
  ...
  // function addShakingList(address account, uint16 tokenId) external {
  //     _addShakingList(account,tokenId);
  //  }  
  //  function delShakingList(address account, uint16 tokenId) external {
  //     delShakingList(account,tokenId);
  //  }
    function addUnShakingList(address account, uint16 tokenId) external {
     
        _addUnShakedList(account,tokenId);
    
   }  
  // function delelteUnShakingList(address account, uint16 tokenId) external {
  //     _delUnShakedList(account,tokenId);
  //  }
  function _delShakingList(address account, uint256  tokenId) internal{
    uint8  delIndex=0;
    bool exist=false;
    for( uint8 i=0;  i<listShaked[account].length;i++)
      {
 ...
 /**
   ...
   * @param _woolf the address of the Woolf
   * @param _swool the address of the SWOOL
   */
 function setContract(address _woolf,address _swool) external onlyOwner {
    woolf = Woolf(_woolf);
    wool = SWOOL(_swool);
  }
