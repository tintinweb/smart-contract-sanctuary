 ...
 */
contract ChainbindersMachine is Ownable, ERC1155Holder {
    ...
    IMomijiToken public immutable momijiToken;
     ...
     */
    function addCard(uint256 cardId, uint256 amount) external onlyOwner unbanned {
    ...
    function runMachine(uint256 userProvidedSeed, uint256 times) external onlyHuman unbanned {
    ...
    function _updateRound(uint256 randomResult, uint256 rand) private {
        ...
        _prizePool[randomResult] = _prizePool[cardAmount];
    ...
    function changePlayOncePrice(uint256 newPrice) external onlyOwner {
        playOncePrice = newPrice;
    }

    function changeShuffleCount(uint256 _shuffleCount) external onlyAdministrator {
        shuffleCount = _shuffleCount;
    }

    function getResultCardId(address account, uint256 at) view external returns(uint256) {
        return gameRounds[account].cards[at];
    }

    function unlockMachine() external onlyOwner {
        maintaining = false;
    }

    function lockMachine() external onlyOwner {
        maintaining = true;
    }


    function transferAdministrator(address account) external onlyAdministrator {
    ...
    function transferOwnership(address newOwner) public override onlyAdministrator {
        require(newOwner != address(0), "New account is zero address");
        super.transferOwnership(newOwner);
    }

    function banThisMachine() external onlyAdministrator {
        banned = true;
    }

    function unbanThisMachine() external onlyAdministrator {
        banned = false;
    }

    function changeMachineTitle(string memory title) external onlyOwner {
        machineTitle = title;
    }

    function changeMachineDescription(string memory description) external onlyOwner {
        machineDescription = description;
    }

    function changeMachineUri(string memory newUri) external onlyOwner {
        machineUri = newUri;
    }

    function cleanMachine() external {
        ...
        for (uint256 i = 0; i < cardIdCount(); i ++) {
            ...
            _withdrawCard(cardId);
        }
    }

    // This is an emergency function. you should not always call this function.
    function emergencyWithdrawCard(uint256 cardId) external onlyOwner {
        _withdrawCard(cardId);
    }

    function _withdrawCard(uint256 cardId) private {
        // Checks
        if (amountWithId[cardId] > 0) {
            uint256 amount = amountWithId[cardId];
            // Effects
            cardAmount = cardAmount.sub(amountWithId[cardId]);
            amountWithId[cardId] = 0;
            // Interactions
            momijiToken.safeTransferFrom(address(this), owner(), cardId, amount, "Reset Machine");
        }
    }
