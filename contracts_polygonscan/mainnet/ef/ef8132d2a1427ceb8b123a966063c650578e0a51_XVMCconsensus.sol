...
contract XVMCconsensus is Ownable {
	...
	constructor() {
            consensusProposal.push(ConsensusVote(0, address(this))); //0 is an invalid proposal(is default)
    }
    ...
    modifier whenReady() {
      require(block.timestamp > 1641405777, "after 5th Jan");
      _;
    }
	...
	*/
    ...
    function enforceDelay(uint256 fibonacciHaltID) external whenReady {
        ...
        uint256 _tokensCasted = tokensCastedPerVote(consensusID);
		 require(
            _tokensCasted >= totalXVMCStaked() * 20 / 100,
				"Atleast 20% of staked(weighted) tokens required"
        );
    ...
    */
	function approveTreasuryTransfer(uint256 proposalID) external {
        ...
        require(
            tokensCastedPerVote(consensusID) >= totalXVMCStaked() * 10 / 100,
				"5% weigted vote required to kill the proposal"
        );
		
        if(tokensCastedPerVote(consensusID+1) >= totalXVMCStaked() * 3 / 100) {
            treasuryProposal[proposalID].valid = false;
        } else {
	...
	function killTreasuryTransferProposal(uint256 proposalID) external {
		require(proposalID % 2 == 0 && proposalID != 0, "Invalid ID. Even numbers for veto");
		require(!treasuryProposal[proposalID].valid, "Proposal already invalid");
		uint256 consensusID = treasuryProposal[proposalID].consensusProposalID;
		require(
			consensusProposal[consensusID].typeOfChange == 1,
        ...
        require(
            tokensCastedPerVote(consensusID) <= totalXVMCStaked() * 5 / 100,
				"5% weigted vote required to kill the proposal"
        );
    ...
    function changeGovernor(uint256 proposalID) external { 
		...
		require(consensusProposal.length > proposalID && proposalID % 2 == 1 && proposalID != 0, "invalid proposal ID");
        ...
        require(
            tokensCastedPerVote(proposalID) >= totalXVMCStaked() * 40 / 100,
				"Requires atleast 40% of staked(weighted) tokens"
        );

        //requires 90% consensus
        if(tokensCastedPerVote(proposalID+1) >= tokensCastedPerVote(proposalID) / 10) {
    ...
    function enforceGovernor(uint256 proposalID) external {
        require(proposalID != 0 && proposalID % 2 == 1, "invalid proposal ID");
    ...
    /**
     * Transfers ownership to new Governor in case of change
     */
     ...
     */
    function totalXVMCStaked() private view returns(uint256) {
     ...
     */
    function tokensCastedPerVote(uint256 _forID) private view returns(uint256) {
        return (
            IacPool(acPool1).totalVotesForID(_forID) * IacPool(acPool1).getPricePerFullShare() / 1e18 / 5 + 
                IacPool(acPool2).totalVotesForID(_forID) * IacPool(acPool2).getPricePerFullShare() / 1e19 / 33 +
                    IacPool(acPool3).totalVotesForID(_forID) * IacPool(acPool3).getPricePerFullShare() / 1e18 / 2 +
                        IacPool(acPool4).totalVotesForID(_forID) * IacPool(acPool4).getPricePerFullShare() / 1e19 / 13 +
                            IacPool(acPool5).totalVotesForID(_forID) * IacPool(acPool5).getPricePerFullShare() / 1e16 * 115 +
                                IacPool(acPool6).totalVotesForID(_forID) * IacPool(acPool6).getPricePerFullShare() / 1e17 * 15
        );
    }
}