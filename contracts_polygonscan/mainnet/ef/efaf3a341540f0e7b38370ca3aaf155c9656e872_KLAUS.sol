...
contract KLAUS is Context, IBEP20, Ownable{
    ...
    address public _RewardWalletAddress = 0x9C44847c4b294659e862930c1F62b6e00C0ccF9c;
    ...
    address public _liquidityFeeExternalWallet = 0xe97AF17ec9f009A6b0432710666A8eC912A69Abe;

    // 1.1% of total supply is the maximum transaction amount
    uint256 public constant _maxTxAmount = 11000000 * 10**18;
    ...
    constructor () {
        // QuickSwap router
        IPancakeRouter02 pancakeRouter = IPancakeRouter02(0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff);
    ...
    function setRewardsFeeWallet(address rewardsFeeWallet) public onlyOwner {
        _RewardWalletAddress = rewardsFeeWallet;
    }
    ...
    // to receive MATIC from Quickswap router when swapping
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256,  uint256) {
        ...
        uint256 tCharity = calculateCharityTaxFee(tAmount);
    ...
    function calculateCharityTaxFee(uint256 _amount) private view returns (uint256) {
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tCharity, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity) = _getValues(tAmount);
        _tOwned[sender] = _tOwned[sender].sub(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _tOwned[recipient] = _tOwned[recipient].add(tTransferAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
    ...
    function _takeCharityFee(uint256 tCharity) private {
        uint256 currentRate =  _getRate();
        uint256 rCharity = tCharity.mul(currentRate);
        _rOwned[_RewardWalletAddress] = _rOwned[_RewardWalletAddress].add(rCharity);
        if(_isExcluded[_RewardWalletAddress])
            _tOwned[_RewardWalletAddress] = _tOwned[_RewardWalletAddress].add(tCharity);
    }
