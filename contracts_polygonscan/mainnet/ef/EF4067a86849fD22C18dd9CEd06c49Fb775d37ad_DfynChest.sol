...
contract DfynChest is Initializable, ERC20Upgradeable, UUPSUpgradeable, AccessControlUpgradeable {
    ...
    // Enter the chest. Pay some Dfyns. Earn some shares.
    ...
    // returns the total amount of dfyn an address has in the contract including fees earned
    function dfynBalance(address _account) external view returns (uint256 dfynAmount_) {
        uint256 dfynAmount = balanceOf(_account);
        uint256 totalShares = totalSupply();
        dfynAmount_ = dfynAmount.mul(dfyn.balanceOf(address(this))).div(totalShares);
    }
