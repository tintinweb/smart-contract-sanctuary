...
contract DfynFusion is Ownable {
    ...
    function swapTokensToNative(
        ...
        if (inputLPs.length > 0) {
            _transferTokensToDfyn(inputLPs);
            _swapTokensForWNative(_removeLiquidity(inputLPs));
        }
        if (inputTokens.length > 0) {
            _transferTokensToDfyn(inputTokens);
    ...
    function swapTokensToToken(
        ...
        uint256 amountOut = IERC20(outputTokenAddress).balanceOf(address(this));

        require(
            amountOut >= minOutputAmount,
            "Expect amountOut to be greater than minOutputAmount."
        );
        IERC20(outputTokenAddress).safeTransfer(
            msg.sender,
            amountOut
        );
    }
    ...
    // Token version
    function _transferTokensToDfyn(InputToken[] memory inputTokens)
        private
    ...
    // LP version
    function _transferTokensToDfyn(InputLP[] memory inputLPs)
        private
        returns (uint256[] memory)
    {
