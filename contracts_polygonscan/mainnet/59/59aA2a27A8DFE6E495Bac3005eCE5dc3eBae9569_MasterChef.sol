...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    MintableToken public farmToken = MintableToken(0x82C7A74f459968a1836d330311f93d51EC30535B);
    address public tokenTransferOwner = 0x4A2319ba25A82FADB6D7bdc1Fe90373C02aDEC9f;
    address public devAddress = 0x4A2319ba25A82FADB6D7bdc1Fe90373C02aDEC9f;

    // Contract for locking reward
    IRewardLocker public rewardLocker = IRewardLocker(0xe870070b79cC767526c5B3Cf9411f29ab21B7e9E);

    uint256 public tokenPerBlock = 8 ether;
    ...
    uint256 public immutable startBlock = 20373061;
    ...
    event SetTokenTransferOwner(address indexed user, address indexed starTransferOwner);
    event TransferTokenOwnership(address indexed user, address indexed newOwner);

    constructor() public {
        farmToken.safeApprove(address(rewardLocker), uint256(0));
        farmToken.safeIncreaseAllowance(
    ...
    function pendingStar(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && pool.totalDeposited != 0 && totalAllocPoint != 0) {
            ...
            accStarPerShare = accStarPerShare.add(tokenReward.mul(1e18).div(pool.totalDeposited));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 tokenReward = multiplier.mul(tokenPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        farmToken.mint(address(this), tokenReward);
        pool.accStarPerShare = pool.accStarPerShare.add(tokenReward.mul(1e18).div(pool.totalDeposited));
        pool.lastRewardBlock = block.number;
    }
    ...
    */
    function _lockReward(address _account, uint256 _amount) internal {
        uint256 starBal = farmToken.balanceOf(address(this));
        rewardLocker.lock(farmToken, _account, _amount > starBal ? starBal : _amount);
    }
    ...
    function updateEmissionRate(uint256 _tokenPerBlock) external onlyOwner {
        require(_tokenPerBlock <= MAX_EMISSION_RATE, "!overmax");
        massUpdatePools();
    ...
    function setTokenTransferOwner(address _tokenTransferOwner) external {
        require(msg.sender == tokenTransferOwner);
        tokenTransferOwner = _tokenTransferOwner;
        emit SetTokenTransferOwner(msg.sender, _tokenTransferOwner);
    }
    ...
    function transferTokenOwnership(address _newOwner) external {
        require(msg.sender == tokenTransferOwner);
        farmToken.transferOwnership(_newOwner);
        emit TransferTokenOwnership(msg.sender, _newOwner);
    }
}