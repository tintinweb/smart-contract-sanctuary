...
import "./libs/IAntiBotBlacklist.sol";
...
contract DestructionTokenV3 is ERC20, Ownable {
    ...
    IAntiBotBlacklist blacklist = IAntiBotBlacklist(0x341fc99019a6C58787D9D05a064e54E8866291b0);
    // operator role
    ...
    modifier antiBot(address _sender, address _recipient, uint256 _amount) { 
        //check blacklist
        require(!blacklist.blacklistCheck(_sender), "ERR1");
        require(!blacklist.blacklistCheck(_recipient), "ERR2");
            ...
            if (_isNotHolderExcludedFromAntiBot(_sender)){
                if(_isNotOperatorExcludedFromAntiBot(_sender)){
                    if (balanceOf(_sender) > _maxUserHoldAmount()) {
                        blacklist.addSuspect(_sender, type(uint256).max);
                        return;
                    }
                }
            }
