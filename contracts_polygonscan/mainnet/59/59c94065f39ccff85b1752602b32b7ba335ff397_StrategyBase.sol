...
abstract contract BaseStrategyMasterChef is BaseStrategy {
    ...
    function _getReward() internal {
        IMasterChef(rewards).claim_rewards(address(this));
    ...
    function emergencyWithdraw() public onlyOwner {
        uint256 fullBalance = IMasterChef(rewards).balanceOf(address(this));
        IMasterChef(rewards).withdraw(fullBalance);
...
abstract contract StrategyFarmBase is BaseStrategyMasterChef {
    ...
    address public constant swapToken = 0x3FCD5De6A9fC8A99995c406c77DDa3eD7E406f81;
    ...
    {
        crv_matic_path = new address[](3);
        crv_matic_path[0] = crv;
        crv_matic_path[1] = eth;
        crv_matic_path[2] = matic;
    ...
    function harvest() public override {
        ...
        if (_crvBalance > harvestCutoff){
            _swapUniswapWithPath(crv_matic_path, _crvBalance);
        }
        
        // Swap matic for eth
        uint256 _maticBalance = IERC20(matic).balanceOf(address(this));
        if (_maticBalance > maticHarvestCutoff){
            _swapUniswapWithPath(matic_eth_path, _maticBalance);
        }
        
        // Take performance fee
        uint256 _ethBalance = IERC20(eth).balanceOf(address(this));
        if (_ethBalance > 0) {
            uint256 performanceFee = _ethBalance.mul(keepFXS).div(keepFXSmax);
            if (performanceFee>0) {
        ...
        if (ethBal>0){
            IERC20(eth).safeApprove(swapToken, 0);
            IERC20(eth).safeApprove(swapToken, ethBal);
            uint256[5] memory amounts = [0, 0, 0, 0, ethBal];
            ICurveCryptoSwap(swapToken).add_liquidity(amounts, 0);
        }
        // Stake the LP tokens
...
contract StrategyBase is StrategyFarmBase {
    // Token addresses
    address public CURVE_MASTER_CHEF = 0xb0a366b987d77b5eD5803cBd95C80bB6DEaB48C0;
    address public atricrypto_LP = 0x8096ac61db23291252574D49f036f0f9ed8ab390;
