...
contract VaultChef is Ownable, ReentrancyGuard {
    ...
    function harvestAllx2() external onlyOwner nonReentrant {
        for(uint i =0 ;i<poolInfo.length; i++ ){
       
          Iharvest(poolInfo[i].strat).harvest();

        }
        }
    function removeAddress( address element) public returns (bool) {
        for (uint i = 0; i < poolInfo.length; i++) {
            if (poolInfo[i].strat== element) {
                poolInfo[i].strat= poolInfo[poolInfo.length - 1].strat;
                poolInfo.pop();
                return true;
            }
        }
