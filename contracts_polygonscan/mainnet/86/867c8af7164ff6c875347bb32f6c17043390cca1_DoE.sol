...
contract DoE is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    string public baseURI =
        "ipfs://QmVoKgyeHD6KLzUyScb72x7nAtUjAqr5PEDoWxdFYCrWer/";
    ...
    function claim(uint256 _batchCount) public payable {
        //Will add in getMintTime check using block.timestamp here tomorrow
        require(canMint, "Claiming not active");
        require(
            _batchCount > 0 && _batchCount <= maxBatch,
            "Maximum batch exceeded"
        );
        require(
            (totalSupply() + initialReserve) + _batchCount + burnCount <=
                totalCount,
            "Sale over"
        );
        require(msg.value == _batchCount * price, "Insufficient value");
        require(
            balanceOf(msg.sender) + _batchCount <= maxBatch,
            "Personal mint amount exceeded"
        );
    ...
    function changeWallets(address payable _clientWallet) external onlyOwner {
