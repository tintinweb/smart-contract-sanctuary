...
contract KittyFlip is Ownable, VRFConsumerBase {
    ...
    function revealGame(bytes32 requestID, uint256 randomness) internal {
        ...
        balances[game.token][daoAddress] = balances[game.token][daoAddress].add(totalFee);

        uint256 finalReward = totalReward.sub(totalFee);
    
        balances[game.token][game.winner] = balances[game.token][game.winner].add(finalReward);
