...
abstract contract StrategyFarmTwoAssets is BaseStrategyMasterChef {
    ...
    address public constant bel = 0x28C388FB1F4fa9F9eB445f0579666849EE5eeb42;
    ...
    function harvest() public override {
        ...
        require(msg.sender == tx.origin || msg.sender == owner() || msg.sender == strategist || msg.sender == jar || msg.sender == multiHarvest, "not authorized");
        ...
        if (_dinoBalance > harvestCutoff){
            // Swap to Eth
            _swapUniswapWithPath(dino_eth_path, _dinoBalance);
            ...
            // Swap half of rest
            uint256 _ethBalancePost = IERC20(eth).balanceOf(address(this));
            _swapUniswapWithPath(eth_bel_path, _ethBalancePost.div(2));

            // Add liquidity
            uint256 _eth = IERC20(eth).balanceOf(address(this));
            uint256 _bel = IERC20(bel).balanceOf(address(this));
            if (_eth > 0 && _bel > 0) {
                IERC20(eth).safeApprove(rewardRouter, 0);
                IERC20(eth).safeApprove(rewardRouter, _eth);
                IERC20(bel).safeApprove(rewardRouter, 0);
                IERC20(bel).safeApprove(rewardRouter, _bel);
                ...
                IERC20(bel).safeTransfer(
                    strategist,
                    IERC20(bel).balanceOf(address(this))
                );
            }
        }
        // Stake the LP tokens
...
contract StrategyTwoAssets is StrategyFarmTwoAssets {
    ...
    address public Eth_Bel_LP = 0x49ceCfa5c62b3A97F58CAd6B4aCc7c74810E1DDa;
