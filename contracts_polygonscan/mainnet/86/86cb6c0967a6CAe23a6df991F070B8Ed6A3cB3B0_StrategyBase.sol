...
abstract contract BaseStrategyMasterChef is BaseStrategy {
    ...
    function getHarvestable() external override view returns (uint256) {
        return IMasterChef(rewards).pendingCrystal(poolId, address(this));
    }
...
abstract contract StrategyFarmBase is BaseStrategyMasterChef {
    ...
    address public constant xdo = 0x3Dc7B06dD0B1f08ef9AcBbD2564f8605b4868EEA;
    ...
    // Other constants
    uint16 public constant underlyingPoolId = 45;
    ...
    // Uniswap swap paths
    address[] public CRYSTL_usdc_path;
    address[] public usdc_xdo_path;
    ...
    {
        CRYSTL_usdc_path = new address[](4);
        CRYSTL_usdc_path[0] = CRYSTL;
        CRYSTL_usdc_path[1] = wmatic;
        CRYSTL_usdc_path[2] = dai;
        CRYSTL_usdc_path[3] = usdc;

        usdc_xdo_path = new address[](2);
        usdc_xdo_path[0] = usdc;
        usdc_xdo_path[1] = xdo;
    ...
    function harvest() public override {
        ...
        //Swap 
        uint256 _CRYSTLBalance = IERC20(CRYSTL).balanceOf(address(this));
        if (_CRYSTLBalance > harvestCutoff){
            
            _swapUniswapWithPath(CRYSTL_usdc_path, _CRYSTLBalance);
            ...
            if (performanceFee>0) {
                IERC20(usdc).safeTransfer(strategist, performanceFee);
            }
            
            // Swap rest
            uint256 _usdcBalancePost = IERC20(usdc).balanceOf(address(this));
            _swapUniswapWithPath(usdc_xdo_path, _usdcBalancePost.div(2));
            
            // Add liquidity
            uint256 _xdo = IERC20(xdo).balanceOf(address(this));
            uint256 _usdc = IERC20(usdc).balanceOf(address(this));
            if (_xdo > 0 && _usdc > 0) {
                IERC20(xdo).safeApprove(currentRouter, 0);
                IERC20(xdo).safeApprove(currentRouter, _xdo);
                ...
                // Donates DUST
                IERC20(xdo).safeTransfer(
                    strategist,
                    IERC20(xdo).balanceOf(address(this))
                );
...
contract StrategyBase is StrategyFarmBase {
    // Token addresses
    address public CRYSTL_MASTER_CHEF = 0xeBCC84D2A73f0c9E23066089C6C24F4629Ef1e6d;
    address public USDC_XDO_LP = 0x398A0b2823bE1BD42C36d96a750bD73511ae2217;
