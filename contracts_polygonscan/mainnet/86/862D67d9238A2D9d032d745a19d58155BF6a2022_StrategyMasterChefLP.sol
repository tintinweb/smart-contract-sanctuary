...
abstract contract FeeManager is StratManager {
    uint constant public STRATEGIST_FEE = 100;
    uint constant public MAX_FEE = 1000;
    uint constant public MAX_CALL_FEE = 200;
    uint constant public MAX_PROFIT_FEE = 300;
    ...
    function setProfitFee(uint256 _fee) external onlyManager{
...
contract StrategyMasterChefLP is StratManager, FeeManager {
    ...
    function chargeFees() internal {
        uint256 toWrapped = IERC20(output).balanceOf(address(this)).mul(profitFee).div(MAX_FEE);
      ...
      ///  uint256 beefyFeeAmount = wrappedBal.mul(beefyFee).div(MAX_FEE);
        IERC20(wrapped).safeTransfer(beefyFeeRecipient, wrappedBal);
