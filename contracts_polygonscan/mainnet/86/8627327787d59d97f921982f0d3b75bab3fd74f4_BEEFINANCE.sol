...
contract BEEFINANCE is Ownable {
    ...
    uint256 public constant ADMIN_RATE = 40;
    ...
    function _init() private {
        ...
        investmentPlans_.push(Objects.Plan(50, 360*60*60*24)); //5% per day for 360 days 
        investmentPlans_.push(Objects.Plan(70, 360*60*60*24)); //7% per day for 360 days



    }
    
    function getSodaPrice() public view returns(uint256) { // +1% per day 
        uint256 timeMultiplier = (block.timestamp.sub(START_DATE)).div(TIME_STEP); 
        
        uint256 sodaPrice = BNB_PER_SODA.add(BNB_PER_SODA.mul(timeMultiplier).mul(10).div(1000));
    ...
    function _stake(address payable _addr, uint256 _planId, uint256 _amount) private returns(bool){
        require(BEE.balanceOf(_addr)>=_amount, "insufficient amount of soda token");
        ...
        BEE.transferFrom(_addr, address(this), _amount);
    ...
    function swap(uint256 _sodaAmount) public{
        require(BEE.balanceOf(msg.sender)>=_sodaAmount, "insufficient amount of soda token");

        uint256 price = getSodaPrice();
        uint256 payout = _sodaAmount.mul(price).div(1e18);

        BEE.transferFrom(msg.sender, address(this), _sodaAmount);
