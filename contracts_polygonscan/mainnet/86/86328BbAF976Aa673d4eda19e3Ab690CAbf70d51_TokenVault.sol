...
contract TokenVault is Ownable, Pausable {
    ...
    struct UserInfo {
        ...
        uint256 sageAtLastUserAction; // keeps track of token deposited at the last user action
    ...
    function deposit(uint256 _amount) external whenNotPaused notContract {
        ...
        user.sageAtLastUserAction = user.shares.mul(balanceOf()).div(totalShares);
     ...
     */
    function calculateHarvestSageRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingSage(0, address(this));
     ...
     */
    function calculateTotalPendingSageRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingSage(0, address(this));
        amount = amount.add(available());
