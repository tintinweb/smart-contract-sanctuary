...
import { ERC20 } from "openzeppelinv4/token/ERC20/ERC20.sol";
import { AccessProtected } from "./utils/AccessProtected.sol";

contract MockUSDC is ERC20("Mock USDC", "USDC"), AccessProtected {
  ...
  function mint(address _to) external {
    if (restricted == Restricted.ON) {
      require(isAdmin(msg.sender), "restricted mode on, only admins can call");
    }
  ...
  function _beforeTokenTransfer(
    ...
    require(
      from == clearingHouse ||
        ...
        to == clearingHouse ||
        to == insuranceFund,
      "only interactions with matrix protocol are allowed"
    );
  }

  function setRestrictionMode(Restricted _restrictionMode) external onlyOwner {
    restricted = _restrictionMode;
  }

  function setContracts(address _clearingHouse, address _insuranceFund)
    external
    onlyOwner
  {
...
import "openzeppelinv4/access/Ownable.sol";
