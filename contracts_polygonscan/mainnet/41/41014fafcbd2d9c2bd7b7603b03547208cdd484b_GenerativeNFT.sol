...
import "./utils/OpenSeaContractURI.sol";
import "./utils/OpenSeaMatic.sol";

contract GenerativeNFT is ERC721, Ownable, OpenSeaContractURI, ContextMixin, NativeMetaTransaction {
  string private _URI;
  uint public _tokenIds;
  uint public _ownerTokenIds;
  ...
  function bulkMint(address account, uint count) external payable {
    require(count <= 20, "count <= 20");
    require(msg.value >= currentPrice() * count, "incorrect price"); // pay the price of first doll x count
    require(_tokenIds + count <= 10000, "sold out");
  ...
  function _ownerMint(address account) internal {
    require(_ownerTokenIds < 1111, "over quota");
    _ownerTokenIds++;
    _safeMint(account, 10000 + _ownerTokenIds);
  }

  function ownerTransfer(address payable account, uint amount) external onlyOwner {
...
abstract contract OpenSeaContractURI {
  ...
  function _setContractURI(string calldata uri) internal {
    _contractURI = uri;
  }
}
