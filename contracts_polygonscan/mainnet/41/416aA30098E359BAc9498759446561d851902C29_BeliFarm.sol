...
contract BeliFarm is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        uint256 rewardFeeDebt;
        uint256 rewardProfitDebt;
    ...
    struct PoolInfo {
        ...
        bool isTransferFeeReward;
        uint256 accBELIPerShareFromFees; // can be either transfer fee reward/LP reward
    ...
    uint256 public ownerBELIReward = 138; // 12%

    uint256 public BELIMaxSupply = 125000000e18; // 125M
    uint256 public BELIPerBlock = 8000000000000000; // BELI tokens created per block, 0.008
    uint256 public startBlock = 3888888; // TODO: change this once deployed
    ...
    modifier validFee(uint16 _harvestFee) {
        require(_harvestFee <= 1000, "harvest fee exceed 10%");
        _;
    }
    ...
    function setFarmSettings(
        ...
        if (referralCommissionRate != _referralCommissionRate) {
            ...
            require(_referralCommissionRate <= 1000, "commission rate cannot exceed 10%");
    ...
    /**
     * TODO: Add a modifier for unique LP
     */
    function add(
        IERC20 _want,
        bool _withUpdate,
        ...
        uint16 _harvestFee,
        bool _isTransferFeeReward,
        bool _isLpReward
    ) public onlyOwner validFee(_harvestFee) {
        if (_isTransferFeeReward == true) {
            require(_isLpReward == false, "transfer fee reward and lp reward is exclusive to each other");
        }
        if (_isLpReward == true) {
            require(_isTransferFeeReward == false, "transfer fee reward and lp reward is exclusive to each other");
        }
        if (_withUpdate) {
            massUpdatePools();
        }
        ...
        poolInfo.push(
            PoolInfo({
                ...
                isTransferFeeReward: _isTransferFeeReward
            })
        );
    }
    ...
    function set(
        ...
        if (pool.isLpReward) {
            totalLpRewardAllocPoint = totalLpRewardAllocPoint.add(_allocPoint);
        }
        if (pool.isTransferFeeReward) {
            totalFeeRewardAllocPoint = totalFeeRewardAllocPoint.add(_allocPoint);
        }
    ...
    function pendingTransferFeeBeliReward(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        if (pool.isTransferFeeReward) {
            ...
            if (pool.lastAccBELIFromFees < _accBeliFromFees && sharesTotal > 0) {
                ...
                pendingReward = user.shares.mul(accBELIPerShareFromFees).div(1e12).sub(user.rewardFeeDebt);
            }
        }
        return pendingReward;
    }

    function pendingTransferFeeBeliLpReward(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 sharesTotal = IVault(pool.vault).sharesTotal();
        uint256 pendingReward = 0;
        if (pool.isLpReward) {
            ...
            if (pool.lastAccBELIFromFees < _accBeliFromFees && sharesTotal != 0) {
                ...
                uint256 accBELIPerShareFromFees = pool.accBELIPerShareFromFees.add(feeReward.mul(1e12)).div(sharesTotal);
                uint256 pendingFeeReward = user.shares.mul(accBELIPerShareFromFees).div(1e12).sub(user.rewardFeeDebt);
    ...
    // View function to see pending BELI on frontend.
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (pool.isLpReward) {
            ...
            if (pool.lastAccBELIFromFees < accBeliLPFromFees && IVault(pool.vault).sharesTotal() != 0) {
                uint256 beliFeeReward =
                    accBeliLPFromFees.sub(pool.lastAccBELIFromFees).mul(pool.allocPoint).div(totalFeeRewardAllocPoint);
                pool.accBELIPerShareFromFees = pool.accBELIPerShareFromFees.add(
                    beliFeeReward.mul(1e12).div(IVault(pool.vault).sharesTotal())
                );
                pool.lastAccBELIFromFees = accBeliFromFees;
    ...
    function payConvertedProfit(uint256 _pid, address _userAddress) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_userAddress];
        if (user.shares > 0) {
            uint256 totalShare = IVault(pool.vault).sharesTotal();
            uint256 accBELIProfit = IVault(pool.vault).accBELIProfit();
            uint256 amount = user.shares.div(totalShare).mul(accBELIProfit).div(1e12).sub(user.rewardProfitDebt);
            if (amount > 0) {
                IVault(pool.vault).transferConvertedProfit(_userAddress, amount);
            }
            user.rewardProfitDebt = user.rewardProfitDebt.add(amount);
        }
    }
    ...
    function payOrLockupPendingBELI(uint256 _pid, bool _isWithdrawal) internal {
        ...
        if (pool.isTransferFeeReward) {
            transferRewardFromFee(_pid, msg.sender);
        }

        if (IVault(pool.vault).profitToBeliRate() > 0) {
            payConvertedProfit(_pid, msg.sender);
        }
    ...
    // Want tokens moved from user -> Farm (BELI allocation) -> Strat (compounding)
    function deposit(uint256 _pid, uint256 _wantAmt, address _referrer) public nonReentrant {
        ...
        user.rewardFeeDebt = user.shares.mul(pool.accBELIPerShareFromFees).div(1e12);
    ...
    function withdrawEarly(uint256 _pid, uint256 _wantAmt) public nonReentrant {
        ...
        if (_wantAmt > 0) {
            uint256 sharesRemoved =
                IVault(poolInfo[_pid].vault).withdrawEarly(msg.sender, _wantAmt);
    ...
    function emergencyWithdrawEarly(uint256 _pid) public nonReentrant {
        ...
        uint256 preBalance = pool.want.balanceOf(address(this));
        IVault(poolInfo[_pid].vault).withdrawEarly(msg.sender, amount);
        uint256 postBalance = pool.want.balanceOf(address(this));
        amount = postBalance.sub(preBalance);
    ...
    function transferRewardFromFee(uint _pid, address _user) private {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 balance;
        uint256 pendingReward = user.shares.mul(pool.accBELIPerShareFromFees).div(1e12).sub(user.rewardFeeDebt);
        if (pool.isTransferFeeReward) {
    ...
    function mintShareToReceipt(uint256 _pid, uint256 _amount) public nonReentrant {
        UserInfo storage user = userInfo[_pid][msg.sender];
        PoolInfo storage pool = poolInfo[_pid];
    ...
    function burnReceiptForShare(uint256 _pid, uint256 _amount) public nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
...
interface IVault {
    ...
    function accBELIProfit() external view returns (uint256);

    function profitToBeliRate() external view returns (uint256);
    ...
    function transferConvertedProfit(address _userAddress, uint256 _amount) external;

    function mintShareToReceipt(address _userAddress, uint256 _amount) external;
