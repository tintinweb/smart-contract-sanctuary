...
contract Deal is DealStorage, Ownable {
    ...
    event Ruling(uint indexed dealId, address winner, bool neither);
    ...
    /**
     * @dev This can be called by owner to resolve dispute in favor of client or freelancer or neither. On resolution, the winner gets their collateral back 
     ...
     **/
    function resolveDispute(uint dealId, address winner, bool neither)
        external
        onlyOwner
    {
        ...
        require(winner == client || winner == freelancer || neither == true, "Winner is neither client nor freelancer");
        
        deals[dealId].inDispute = false;

        // Dispute was ruled to be too minor to penalize someone
        if (neither == true){
            winner = address(0);
            loser = address(0);
            ...
            loser = client == winner ? freelancer : client;
     ...
     * @param winner The winner of the dispute
     **/
    function voteRuling(uint dealId, address winner, bool neither) external {
        require(deals[dealId].inDispute == true, "Gig is not in dispute");
        ...
        emit Ruling(dealId, winner, neither);
    }
