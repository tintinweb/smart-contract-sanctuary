...
contract IDO is Context, Ownable{
    ...
    */ 
    function preAlloc(uint _amount) external isready isFunded isSaleOpen{
        ...
        uint cost = _amount * price / 10000; // cost for this amount of tokens, please notify users
        require(IERC20(U).transferFrom(_msgSender(), address(this), cost), "usdt transfer failed");
        ...
        userInfo[_msgSender()].amount += _amount;
        userInfo[_msgSender()].uAmount += cost; 
    ...
    */
    function claim() external isready isSaleFinalized{
        ...
        if(amount_desired <= quota){
            ...
            IERC20(U).safeTransfer(_msgSender(), price*(quota-amount_desired)/10000);
            userInfo[_msgSender()].claimed += quota;
            userInfo[_msgSender()].uAmount -= price*(quota-amount_desired)/10000;
            tokensClaimed += quota;
            userInfo[_msgSender()].checked = true;
    ...
    */
    function R2purchase(uint _amount) isready external{
        ...
        IERC20(U).safeTransferFrom(_msgSender(), address(this), _amount*price2/10000);
        ...
        userInfo2[_msgSender()].u_paid += _amount*price2/10000;
