...
contract NFT is ERC721Enumerable, Ownable, INFT, INFTMetadata {
    using Strings for uint256;

    uint256 public constant GIFT = 77;
    ...
    constructor(string memory name, string memory symbol, uint maxPublicSupply) ERC721(name, symbol) {
        PUBLIC = maxPublicSupply;
        MAX = GIFT + PUBLIC;
    }
    ...
    */
    ...
    function purchase(uint256 numberOfTokens) external override payable {
        ...
        require(totalSupply() < MAX, 'All tokens have been minted');
        ...
        */
        require(totalPublicSupply < PUBLIC, 'Purchase would exceed PUBLIC');
        require(PRICE * numberOfTokens <= msg.value, 'ETH amount is not sufficient');

        for (uint256 i = 0; i < numberOfTokens; i++) {
            /**
            * @dev Since they can get here while exceeding the MAX,
            * we have to make sure to not mint any additional tokens.
            */
            if (totalPublicSupply < PUBLIC) {
                /**
                * @dev Public token numbering starts after GIFT.
    ...
    function purchaseAllowList(uint256 numberOfTokens) external override payable {
        ...
        require(totalPublicSupply + numberOfTokens <= PUBLIC, 'Purchase would exceed PUBLIC');
...
contract NFTFactory is ReentrancyGuard, Ownable {
    ...
    event Build(address owner, address token);

    constructor(uint _buildFee) {
        buildFee = _buildFee;
    }
    ...
    function build(
        ...
        uint tokenMaxSupply_,
        string calldata tokenBaseURI_,
        address ownerAddress_
    ) external payable nonReentrant {
        require(msg.value == buildFee);
        NFT nft = new NFT(tokenName_, tokenSymbol_, tokenMaxSupply_);
        nft.setBaseURI(tokenBaseURI_);
        nft.transferOwnership(ownerAddress_);

        payable(owner()).transfer(msg.value);

        emit Build(ownerAddress_, address(nft));
    }
}