...
contract EvolutionEgg is IERC721Receiver, OwnableUpgradeable, ReentrancyGuard {
    ...
    uint256 lastIndexCommon_;
    uint256 lastIndexRare_;
    uint256 lastIndexEpic_;
    ...
    function setIndexTiny(uint256 _lastIndexCommon, uint256 _lastIndexRare, uint256 _lastIndexEpic) external onlyOwner {
        lastIndexCommon_ = _lastIndexCommon;
        lastIndexRare_ = _lastIndexRare;
        lastIndexEpic_ = _lastIndexEpic;
    }
    ...
    function _findTokenIdByType(ITinyHeroStats.TinyType _tinyType,  ITinyHeroStats.TinyRarity _tinyRarity) internal returns (uint256 tokenTinyId) {
        if (_tinyRarity == ITinyHeroStats.TinyRarity.COMMON) {
            // Common
            uint256 length = saleCommonInfo.length;
            for (uint256 i = lastIndexCommon_; i < length; i++) {
            ...
            for (uint256 i = lastIndexRare_; i < length; i++) {
            ...
            for (uint256 i = lastIndexEpic_; i < length; i++) {
                SaleInfo storage sale = saleRareInfo[i];
                (ITinyHeroStats.TinyInfo memory info,) = ITinyHeroStats(statsTiny_).details(sale.tokenId);
                if (sale.status == 3 && info.Type == _tinyType) {
                    tokenTinyId = sale.tokenId;
    ...
    function _open(uint256 _tokenEggId, ITinyHeroStats.TinyType _tinyType, ITinyHeroStats.TinyRarity _tinyRarity, address receiver) internal returns (uint256 tokenTinyId) {
        currentSeed = _genNextSeed(0);
        IEggFactory _factoryEgg = IEggFactory(factoryEgg_);
        tokenTinyId = _findTokenIdByType(_tinyType, _tinyRarity);
        if (_tinyRarity == ITinyHeroStats.TinyRarity.COMMON) {
            // Common
            SaleInfo storage sale = saleCommonInfo[lastIndexCommon_];
            sale.status = 2;
            sale.opener = receiver;
            BRC721(nftTiny_).safeTransferFrom(address(this), receiver, tokenTinyId);
        } else if (_tinyRarity == ITinyHeroStats.TinyRarity.RARE) {
            // Rare
            SaleInfo storage sale = saleRareInfo[lastIndexRare_];
            sale.status = 2;
            sale.opener = receiver;
            BRC721(nftTiny_).safeTransferFrom(address(this), receiver, tokenTinyId);
        } else {
            // EPIC
            SaleInfo storage sale = saleEpicInfo[lastIndexEpic_];
            sale.status = 2;
            sale.opener = receiver;
            BRC721(nftTiny_).safeTransferFrom(address(this), receiver, tokenTinyId);
        }
    ...
    function getLuckyNumber(uint256 _randomNumber) private view returns (uint256) {
        uint256 maxRanger = 100;
        return uint256(keccak256(abi.encodePacked(_randomNumber, maxRanger))) % 100;
    }

    function evolutionEgg(uint256 _tokenId, uint256 _userRandomHash) external nonReentrant returns (uint256 tokenTinyId) {
        ...
        tokenTinyId = _open(_tokenId, _futureTinyType, _futureTinyRarity, msg.sender);
    }

    function evolutionEggs(uint256[] memory tokenIds, uint256 _userRandomHash) external returns (uint256[] memory) {
        uint256 length = tokenIds.length;
        uint256[] memory tokenTinyIDs = new uint256[](length);
        for (uint256 i = 0; i < length; i++) {
            uint256 tokenTinyId = this.evolutionEgg(tokenIds[i], _userRandomHash);
            tokenTinyIDs[i] = tokenTinyId;
        }
        return tokenTinyIDs;
    }
}