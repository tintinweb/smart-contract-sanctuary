...
contract Staking is Initializable, OwnableUpgradeable {
    ...
    event Unbond(address indexed user);
    event EmergencyUnbond(address indexed user);
    ...
    function deposit(uint256 amount, address to) public {
        ...
        user.amount = oldAmount + amount;

        // user unbonded but didn't withdraw, then stake all
        if (user.unbondedAt > 0) {
            user.unbondedAt = 0;
            totalUnbondingAmount = totalUnbondingAmount - oldAmount;
            user.rewardDebt = ((user.amount * accRewardPerShare) / ACC_REWARD_PRECISION).toInt256();
        } else {
            user.rewardDebt = user.rewardDebt + ((amount * accRewardPerShare) / ACC_REWARD_PRECISION).toInt256();
        }
     ...
     */
    function emergencyUnbond() public {
        UserInfo storage user = userInfo[msg.sender];
        require(user.unbondedAt == 0, "Unbond: Already unbonding");
        require(user.amount > 0, "Unbond: Nothing to unbond");
        ...
        emit EmergencyUnbond(msg.sender);
    }
