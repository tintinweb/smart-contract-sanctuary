...
contract forbitspaceX is IforbitspaceX, Payment {
	...
	function aggregate(
		...
		require(amountInAcutual <= amountInTotal, "E_I_A"); // EXCESSIVE_INPUT_AMOUNT
		require(amountOutAcutual >= amountOutTotal, "I_O_A"); // INSUFFICIENT_OUTPUT_AMOUNT

		pay(address(this), recipient, tokenIn, amountInTotal.sub(amountInAcutual, "N_E_T")); // not enough tokens
	...
	function _swap(SwapParam[] calldata params) private {
		for (uint i = 0; i < params.length; i++) {
			...
			) = (p.exchangeTarget, p.addressToApprove, p.tokenIn, p.tokenOut, p.amountIn, p.amountOut, p.swapData);

			approve(addressToApprove, tokenIn, amountIn);
			...
			require(amountInActual <= amountIn, "E_I_A"); // EXCESSIVE_INPUT_AMOUNT
			require(amountOutActual >= amountOut, "I_O_A"); // INSUFFICIENT_OUTPUT_AMOUNT
		}
	}
}