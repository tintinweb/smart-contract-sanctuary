...
import "../apis/sushi/IUniswapV2Router02.sol";
...
contract TestWorker {
  ...
  constructor(
    ...
    sushi = (_masterChef.SUSHI());
    lpToken = IUniswapV2Pair(_masterChef.lpToken(_pid));
    ...
    if(rewardToken == baseToken) same = true;
  }

  function pendingReward() public view returns(address , uint256 ) {
    IERC20[] memory _rewardTokens = new IERC20[](1);
    uint256[] memory _rewardAmounts = new uint256[](1);
    (_rewardTokens, _rewardAmounts) = IRewarder(rewarder).pendingTokens(pid, address(this), 0);
    return (address(_rewardTokens[0]), _rewardAmounts[0]);
  }
