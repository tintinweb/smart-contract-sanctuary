...
contract Meta721Artist is ERC721Enumerable, Ownable {
//    using SafeMath for uint256;
    ...
    enum Operations { SET_OPERATOR, SET_FEE, SET_BASE_URI, SET_ARTIST }
    ...
    constructor(IERC20 _xhgt, IMetaHolder _holder, address _operator, uint256 _maxSupply, uint256 _maxDaySupply) ERC721("MetaBaseArt", "MBArtist") {
        xhgt = _xhgt;
        holder = _holder;
    ...
    function setArtist(address _artist, bool _flag) external onlyOperator notLocked(Operations.SET_ARTIST) {
        artists[_artist] = _flag;
        timelock[Operations.SET_ARTIST] = 0;
    }
    ...
    function tokenURI(uint256 _tokenId) public view override returns (string memory) {
        ...
        _jsons[0] = string(abi.encodePacked('{"name":"CBArtistArt #', tokenIdStr, '"'));
        _jsons[1] = string(abi.encodePacked(',"image":"' , _tokenURI, '"'));
        _jsons[2] = ',"description":"Artist Creation Artwork for MetaBase.art"';
    ...
    function mintWithEther(string memory _tokenURI, string memory _fileHash, string memory _tokenType) external payable {
        require(artists[msg.sender] == true, 'Only artist can mint');
