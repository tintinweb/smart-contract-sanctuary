...
// VSQPresale2
contract VSQPresale2 is Ownable, ReentrancyGuard {
    using SafeERC20 for ERC20;

    address public constant fraxAddress = 0x45c32fA6DF82ead1e2EF74d17b76547EDdFaFF89;

    uint256 public constant salePrice = 26;

    uint256 public constant VSQMaximumSupply = 12500 * 1e9;
    ...
    constructor(uint256 _startBlock, address _treasuryAddress, address _VSQAddress) {
        ...
        endBlock   = _startBlock + oneDayMatic;

        VSQAddress = _VSQAddress;
        treasuryAddress = _treasuryAddress;
    }

    function buyVSQ() external nonReentrant {
        ...
        uint256 VSQPurchaseAmount = 25 * 1e9;
        // Times by the sale price and scale to 18 decimals
        uint256 fraxRequired = VSQPurchaseAmount * salePrice * 1e9;

        require(ERC20(fraxAddress).balanceOf(msg.sender) >= fraxRequired, "not enough Frax in users wallet to partake in presale!");
        ...
        ERC20(fraxAddress).safeTransferFrom(msg.sender, treasuryAddress, fraxRequired);

        whitelist[msg.sender] = false;
        if (remainingBuyers > 0)
            remainingBuyers--;

        emit VSQPurchased(msg.sender, fraxRequired, VSQPurchaseAmount);
    }
    ...
    function setStartBlock(uint256 _newStartBlock) external onlyOwner {
        ...
        endBlock   = _newStartBlock + oneDayMatic;
