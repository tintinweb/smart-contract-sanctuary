...
// File: contracts/mixer/ERC20Mixer.sol
...
contract ERC20Mixer is Mixer {
  ...
  function _processDeposit() internal {
    ...
    if (enableReward) {
      IVolMiningRewards(volMiningRewards).provideVolume(token, denomination, msg.sender);
    }
  }
  ...
  function _processWithdraw(address payable _recipient, address payable _relayer, uint256 _fee, uint256 _refund) internal {
    ...
    if (protocolFeeValue > 0) {
      _safeErc20Transfer(tokenDistributor, protocolFeeValue);
    }
  }
...
// File: contracts/deployment/MixerUSDC100.sol

pragma solidity 0.5.17;

// used for testing only
contract MixerUSDC100 is ERC20Mixer {
  constructor(
    IVerifier _verifier,
    address _tokenDistributor,
    address _volMiningRewards,
