...
contract MasterChef is Ownable {
    ...
    constructor(
        ...
        address _devaddr,
        address _feeAddress,
        uint256 _SPMFPerBlock,
        uint256 _startBlock
    ) public {
        SPMF = _SPMF;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        SPMFPerBlock = _SPMFPerBlock;
    ...
    function pendingEgg(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accEggPerShare = accEggPerShare.add(SPMFReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 SPMFReward = multiplier.mul(SPMFPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        SPMF.mint(devaddr, SPMFReward.div(10));
        SPMF.mint(address(this), SPMFReward);
        pool.accEggPerShare = pool.accEggPerShare.add(SPMFReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    // Safe SPMF transfer function, just in case if rounding error causes pool to not have enough EGGs.
    function safeEggTransfer(address _to, uint256 _amount) internal {
        uint256 SPMFBal = SPMF.balanceOf(address(this));
        if (_amount > SPMFBal) {
            SPMF.transfer(_to, SPMFBal);
        } else {
            SPMF.transfer(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _SPMFPerBlock) public onlyOwner {
        massUpdatePools();
        SPMFPerBlock = _SPMFPerBlock;
    }
