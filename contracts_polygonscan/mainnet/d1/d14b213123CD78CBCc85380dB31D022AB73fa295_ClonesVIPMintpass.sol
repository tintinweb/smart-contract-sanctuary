...
contract ClonesVIPMintpass is ERC721Enumerable, Ownable, ReentrancyGuard {
  uint256 public constant MINTPASSES_SUPPLY = 100;
  ...
  constructor() ERC721("Cloneverse VIP Mint Pass", "CVMP") {}

  function BuyMintPass(uint256 units) public payable {
    uint256 mintIndex = totalSupply();

    require(isSaleActive, "The sale is not active");
    require(units <= MINT_LIMIT, "The mint limit is 10");
    require(mintIndex + units <= MINTPASSES_SUPPLY, "The amount requested exceed the collection size");
