...
interface IEscrowToken {
    ...
    function createEscrow(address to,uint currencyValue,bool canRefund,uint terminatedTime,string calldata currencyCode,uint8 hedgeRate) external;

    function completeEscrow(uint escrowId)external;

    function terminateEscrow(uint escrowId)external;

    function refundEscrow(uint escrowId)external;

    function createEscrowCreaterNFT(uint escrowId)external;

    function createEscrowRecipientNFT(uint escrowId)external;

    function getEscrowDetail(uint escrowId)external view returns(EscrowDetail memory);
}
