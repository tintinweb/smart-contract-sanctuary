  ...
  function create(
    ...
    */
    function totalSupply(
		 ...
		 */
		function getAllTokensOwned(address senderAddr) public view returns(uint256[] memory){
			uint currIndx=0;
			uint[] memory result = new uint[](_currentTokenID);
			for(uint count=1;count<=_currentTokenID;count++){
				uint balance=balanceOf(senderAddr,count);
				if(balance>0){
					result[currIndx]=count;
					currIndx++;
				}
			}
			uint[] memory trimmedResult = new uint[](currIndx);
