...
contract SakeSwapFactory {
    ...
    modifier onlyFeeToSetter() {
        require(feeToSetter == msg.sender, "caller is not feeToSetter");
        _;
    }
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        ...
        ISakeSwapPair(pair).initialize(token0, token1, router, mintSToken);
    ...
    function setRouter(address _router) external onlyFeeToSetter {
        router = _router;
        uint256 length = allPairs.length;
        for (uint256 i; i < length; i++) {
            ISakeSwapPair(allPairs[i]).setRouter(router);
        }
    }

    function setMintSToken(bool _mintSToken) external onlyFeeToSetter {
        mintSToken = _mintSToken;
    }

    function setMintSTokenForAllPair(bool _mintSToken) external onlyFeeToSetter {
        uint256 length = allPairs.length;
        for (uint256 i; i < length; i++) {
            ISakeSwapPair(allPairs[i]).setMintSToken(_mintSToken);
        }
    }

    function setMintSTokenForPair(bool _mintSToken, address _pair) external onlyFeeToSetter {
        require(_pair != address(0), "SakeSwap: ZERO_ADDRESSES");
        uint256 length = allPairs.length;
        for (uint256 i; i < length; i++) {
            if (allPairs[i] == _pair) {
                ISakeSwapPair(allPairs[i]).setMintSToken(_mintSToken);
...
contract SakeSwapPair is SakeSwapERC20 {
    ...
    modifier onlyRouter() {
        if (router != address(0)) {
            require(router == msg.sender, "caller is not router");
        }
        _;
    }

    modifier onlyFactory() {
        require(factory == msg.sender, "caller is not factory");
        _;
    }
    ...
    function dealSlippageWithIn(
        ...
        address to,
        bool ifmint
    ) external lock onlyRouter returns (uint256 amountOut) {
        ...
        // avoids stack too deep errors
        {
            ...
            (uint256 _reserveIn, uint256 _reserveOut) =
                path[0] == _token0 ? (_reserve0, _reserve1) : (_reserve1, _reserve0);
    ...
    function dealSlippageWithOut(
        ...
        address to,
        bool ifmint
    ) external lock onlyRouter returns (uint256 extra) {
        require(path.length == 2, "SakeSwap: INVALID_PATH");
        address _token0 = token0;
    ...
    function setRouter(address _router) external onlyFactory {
        require(_router != address(0), "SakeSwap: router can not be 0");
        router = _router;
    }

    function setMintSToken(bool _mintSToken) external onlyFactory {
        if (_mintSToken != mintSToken) {
            mintSToken = _mintSToken;
            if (mintSToken == true && address(stoken) == address(0)) {
                stoken = new SakeSwapSlippageToken(0);
            }
        }
    }
