...
contract CompetitionPool is Ownable {
    ...
    event BracketsAmountToken(uint256[] _bracketsAmountToken);
    ...
    function setBracketsAmountToken(
        ...
        emit BracketsAmountToken(_bracketsAmountToken);
        for (uint256 i = 0; i < _bracketsAmountToken.length; i++) {
            setRewardRate(
                i,
        ...
        )
    {
        ...
        _player2 = IChainLinkOracleSportData(oracle)
            .getInfoCompetition(requestId)
            .player2;
        return (_sportType, _matchName, _player1, _player2);
        // return ("a", "b", "c", "d");
    }
...
interface IRegularCompetitionContract {
    ...
    event Close(uint256 timestamp, Player playerWon, uint256 winerReward);
    event RewardRate(
        uint256 _rewardRateOfcreator,
        uint256 _rewardRateOfowner,
        uint256 _rewardRateOfWinner
