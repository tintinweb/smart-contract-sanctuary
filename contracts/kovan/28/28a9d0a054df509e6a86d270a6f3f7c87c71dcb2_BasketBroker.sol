...
// File: contracts/protocol/BasketBroker.sol
...
// Adapted from Set Protocol's Basic Issuance Module
contract BasketBroker is ReentrancyGuard {
    ...
    nonReentrant
  {
    ...
    for (uint256 i = 0; i < components.length; i++) {
        console.log(components[i], componentQuantities[i]);
        // Transfer the component to the BasketToken
        //abi.encodeWithSignature("transferFrom(address, address, address, uint256", components[i], _to, address(_basketToken), componentQuantities[i]);
        //transferFrom(IERC20(components[i]), _to, address(_basketToken), componentQuantities[i]);
        //);
        transferFrom(
            IERC20(components[i]),
            msg.sender,
