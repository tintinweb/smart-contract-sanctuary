...
contract Gauge is ReentrancyGuard {
    ...
    function rewardPerToken() public view returns (uint256) {
        ...
        return
            rewardPerTokenStored.add(
                lastTimeRewardApplicable().sub(lastUpdateTime).mul(rewardRate).mul(1e6).div(derivedSupply)
            );
    }
    ...
    function earned(address account) public view returns (uint256) {
        return derivedBalances[account].mul(rewardPerToken().sub(userRewardPerTokenPaid[account])).div(1e6).add(rewards[account]);
    }
...
contract MasterDill {
    ...
    constructor() public {
        ...
        emit Transfer(address(0x0), msg.sender, 1e6);
    }
