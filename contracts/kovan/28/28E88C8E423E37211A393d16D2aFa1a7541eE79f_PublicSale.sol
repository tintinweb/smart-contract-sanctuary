...
///@title Bumper Public Sale contract
    ...
    ReentrancyGuardUpgradeable
{
    ...
    /// @notice amount of bump tokens available for sale up to now
    ...
    /// @notice Purchase BUMP tokens with permit
    ...
    function depositAmountWithPermit(
        ...
        bytes32 r,
        bytes32 s
    ) external whenSaleIsActive(msg.sender, _merkleProof) {
    ...
    ///@notice Purchase BUMP tokens (need approve first)
    ///@param _usdcAmount Amount of USDC for purchase
    ///@param _stakePeriodIndex Index of stake period for StakeRewardFixed contract
    ...
    function _depositAmount(
        ...
        require(bumpPurchaseAllocation > bumpTokensPurchased, "No tokens left" );
        ...
        if (_isLocked == false) {
            ...
            // calculate rewards for given stake option
            uint rewards = StakeRewardFixed(stakingContract).calcUserStakeRewards(bumpTokensPurchased, _stakePeriodIndex);
            // if sale option is locked
            bumpPurchaseAllocation -= bumpTokensPurchased + rewards;            
            BUMPToken(bumpTokenAddress).distributeToAddress(
                stakingContract,
                bumpTokensPurchased + rewards
            );
...
contract StakeRewardFixed is Initializable, BumperAccessControl {
  ...
  /// @notice return stake option info by index
  function getStakeOption(uint index) public view returns (StakeOption memory) {
    return stakeOptions[index];
  }

  /// @notice calculate user stake rewards 
  function calcUserStakeRewards(uint256 amount, uint16 bonusInPercentage) public pure returns (uint256 rewards) {
