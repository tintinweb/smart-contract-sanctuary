...
contract Wrapper {
    ...
    function stake(uint256 _nftId, uint256 _value, bytes memory _bytes) public virtual {
    ...
    function withdraw(uint256 _nftId, uint256 _value, bytes memory _bytes) public virtual {
...
contract TreaPoolToken is Wrapper, Ownable {
    ...
    function stake(uint256 _nftId, uint256 _value, bytes memory _bytes)
        public
        override
        ...
        checkStart
    {
        ...
        super.stake(_nftId, _value, _bytes);
        emit Staked(msg.sender, _value);
    }

    function withdraw(uint256 _nftId, uint256 _value, bytes memory _bytes)
        public
        override
        updateReward(msg.sender)
        checkStart
    {
        ...
        super.withdraw(_nftId, _value, _bytes);
        emit Withdrawn(msg.sender, _value);
    }

    function exit(uint256 _nftId, bytes memory _bytes) external {
        withdraw(_nftId, balanceOf(msg.sender), _bytes);
        getReward();
    }
