...
contract NFTMarket is ReentrancyGuard, ChainlinkClient, Ownable, IMarket {
    ...
    uint256 public FEE_ORACLE;
    address public NUPAY_WALLET_ADDRESS;
    ...
    function fetchSpecificMarketItem(uint256 itemId) public view returns (MarketItem memory item) {
    ...
    constructor(string memory urlRequest_) Ownable() {
        ...
        ORACLE = 0x6f4af7bA9c507304aB460B8517848a0d97A67bf3;
        JOBID = "d6b04fd14ab0446baf8dd3119d5dda27";
        FEE_ORACLE = 1e18;
    }
    ...
    function doRequest(string memory _url, uint256 itemId) public {
        ...
        req.add("get", _url);
        req.add("path", "result");
        bytes32 requestId = sendChainlinkRequestTo(ORACLE, req, FEE_ORACLE);
    ...
    function fulfill(bytes32 _requestId, bytes32 _commission) public recordChainlinkFulfillment(_requestId) {
        string[] memory splitArr = bytes32ToString(_commission).split('|');
    ...
    function createMarketItem(
        ...
        require(nft == address(nftContract), "Invalid NFT contract");
        _itemIds.increment();
    ...
    function setOracle(address _oracle, bytes32 _jobId) public onlyOwner {
        ORACLE = _oracle;
        JOBID = _jobId;
    }

    function setFeeOracle(uint256 _fee) public onlyOwner {
        FEE_ORACLE = _fee;
    }
    ...
    function setUrlRequest(string memory url) public onlyOwner{
        urlRequest = url;
    }
    ...
    function createMarketSale(
        ...
        require(itemId > 0 && itemId <= _itemIds.current(), "Market item not found");
        require(msg.value == idToMarketItem[itemId].price, "Please submit the asking price in order to complete the purchase");
        require(!idToMarketItem[itemId].sold, "Item already paid");
        MarketItem storage item = idToMarketItem[itemId];
        uint256 tokenId = item.tokenId;
        require(idTokenToMarket[tokenId] == 0, "Token already payed");
    ...
    function forceDistributeAssets(
        ...
        idToMarketItem[itemId].commission = comission;
        idToMarketItem[itemId].isPremium = isPremium;
        delete idTokenToMarket[idToMarketItem[itemId].tokenId];
        distributeAssets(itemId, shareMinter);
        return true;
    }

    function resolveDeal(uint256 tokenId, address sender, address recipient) external override returns(bool) {
    ...
    function rejectDeal(uint256 itemId, uint256 fee) external onlyOwner returns(bool) {
        ...
        require(!idToMarketItem[itemId].isResolved, "Payment has been resolved");
        require(!idToMarketItem[itemId].isRejected, "Payment has been rejected");
        require(!idToMarketItem[itemId].isDistributeAssets, "Payment has been distributed");
        require(idToMarketItem[itemId].sold, "Market item not sold");
    ...
    function distributeAssets(uint256 itemId, uint256 feeMinter) private returns(bool) {
        require(_itemIds.current() >= itemId, "Market item not found");
        require(!idToMarketItem[itemId].isDistributeAssets, "Assets have already allocated");
        require(!idToMarketItem[itemId].isRejected, "Payment has been rejected");
        require(feeMinter < HUNDRED_PERCENT, "Share exceeded");
        require(idToMarketItem[itemId].commission < HUNDRED_PERCENT, "Comission exceeded");
        ...
        idToMarketItem[itemId].isDistributeAssets = true;
        delete idTokenToMarket[idToMarketItem[itemId].tokenId];
