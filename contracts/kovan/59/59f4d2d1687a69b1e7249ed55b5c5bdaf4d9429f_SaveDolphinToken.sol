...
contract SaveDolphinToken is Context, Ownable, IERC20 {
    using SafeMath for uint256;
    using Address for address;

    mapping (address => TokenTimelock[]) private lockStatus;
    mapping (address => bool) private lockStatusExists;
    mapping (address => bool) private lockWalletAddress;
    ...
    string private _name = "TSaveDolphin";
    string private _symbol = "TSDOL";
    ...
    constructor() public {
        _tTotal = 1000000000000000 * (10 ** 8); // (50% BURN)
        _maxTxAmount = _tTotal.mul(5).div(10**4);
        numTokensSellToAddToLiquidity = _maxTxAmount;
        _tTotal = _tTotal.div(2); // (50% BURN)
    ...
    function calculateLiquidityPlusCharityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_liquidityFee + _charityFee).div(
            10**2
        );
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 totalLiqFee = _liquidityFee + _charityFee;
        ...
        uint256 finalBalance = newBalance.sub(charityBalance);
        uint256 finalHalf = otherHalf.sub(charityPortion);
    ...
    function transferLockToken(address beneficiary, uint256 amount, uint256 delay) public onlyOwner {
        TokenTimelock lockContract = new TokenTimelock(this, beneficiary, block.timestamp + delay);
        address lockAddress = address(lockContract);
        transfer(lockAddress, amount);

        lockStatus[beneficiary].push(lockContract);
        lockStatusExists[beneficiary] = true;
        lockWalletAddress[lockAddress] = true;
        emit Lock(beneficiary, amount);
    }

    function releaseToken(address[] memory receivers) public onlyOwner {
        for (uint256 i = 0; i < receivers.length; i++) {
            address iReceiver = receivers[i];
            if (lockStatusExists[iReceiver]) {
                TokenTimelock[] memory iLockArray = lockStatus[iReceiver];
                for (uint256 j = 0; j < iLockArray.length; j++) {
                    TokenTimelock iLockStatus = iLockArray[j];
                    if (!iLockStatus.sended() && block.timestamp > iLockStatus.releaseTime()) {
                        iLockStatus.release();
                    }
                }
            }
