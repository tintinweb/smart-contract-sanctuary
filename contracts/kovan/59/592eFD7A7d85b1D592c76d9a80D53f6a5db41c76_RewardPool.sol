...
contract RewardPool is LPTokenWrapper, IRewardDistributionRecipient {
    ...
    function stake(uint256 amount) public override updateReward(msg.sender) {
        ...
        super.stake(amount);
        (uint256 senderStake, uint256 totalStake) = getStakedValuesinUSD(msg.sender);
        //console.log("senderStake %s + totalStake %s  <= totalStakeLimit %s", senderStake, totalStake, totalStakeLimit);
        require(senderStake.add(totalStake) <= totalStakeLimit, "Limit exceeded");
    ...
    function getStakedValuesinUSD(address wallet) public view returns (uint256 senderStake, uint256 totalStake) {
        ...
        // simple ERC-20
        if (tokenMode == 0) {            
            senderStake = balanceOf(wallet).mul(price).div(1e18);
        ...
        if (tokenMode == 1) {            
            ...
            uint256 ESWreserveWallet = IEmiswap(address(stakeToken)).getBalanceForAddition( rewardToken ).mul(2).mul(lpFractionWallet).div(1e18);
            uint256 ESWreserveTotal = IEmiswap(address(stakeToken)).getBalanceForAddition( rewardToken ).mul(2).mul(lpFractionTotal).div(1e18);
            senderStake = ESWreserveWallet.mul(price).div(1e18);
            totalStake = ESWreserveTotal.mul(price).div(1e18); */

            senderStake = IEmiswap(address(stakeToken)).getBalanceForAddition( rewardToken ).mul(2).mul(balanceOf(wallet).mul(1e18).div(stakeToken.totalSupply())).mul(price).div(1e36);
            totalStake = IEmiswap(address(stakeToken)).getBalanceForAddition( rewardToken ).mul(2).mul(totalSupply().mul(1e18).div(stakeToken.totalSupply())).mul(price).div(1e36);
        }
    }
