...
import "../dependencies/FeeToken.sol";
 ...
 * - Can be burned by anyone to forcibly reduce total supply
    ...
    - 1% prism
 */
contract WROOT is ERC20MintSnapshot, FeeToken {
    using SafeERC20 for ERC20;

    event Mint(address indexed _to, uint256 indexed _amount);

    constructor() public ERC20MintSnapshot("wROOT Token", "wROOT") {
    ...
    function _transferFee(address from, address feeTo, uint256 amount) internal override {
        _transfer(from, feeTo, amount);
    }
...
abstract contract FeeToken is Controller {
    ...
    uint256 private MARKETING_TX_FEE = 0.03 ether; //3%
    uint256 private PRIZEPOOL_TX_FEE = 0.03 ether; //3%
    uint256 private DEV_FEE = 0.02 ether; //2%
    uint256 private PRISM_FEE = 0.01 ether; //1%

    uint256 private TOTAL_FEE = 0.09 ether; //9%
    ...
    function _consumeFee(
        ...
        if (!feeless[recipient]) {
            uint256 fee = output.wmul(TOTAL_FEE);
            output -= fee;

            //marketing
            _transferFee(sender, marketingAddress, fee.wmul(MARKETING_TX_FEE));
            //prize pool 
            _transferFee(sender, prizepoolAddress, fee.wmul(PRIZEPOOL_TX_FEE));
            //dev
            _transferFee(sender, devAddress, fee.wmul(DEV_FEE));
            //prism
            _transferFee(sender, prismAddress, fee.wmul(PRISM_FEE));
        }
    }
    ...
    function _transferFee(address from, address feeTo, uint256 amount) internal virtual;

    // Utility Pure Functions

    function getAmountPlusFee(uint256 amount) external view returns (uint256) {
        return amount.wdiv(1 ether - TOTAL_FEE) * 100;
    }

    function getTransactionFee(uint256 amount) external view returns (uint256) {
        return amount.wmul(TOTAL_FEE);
    }

    function updateBridgeFee(uint256 amount) external onlyOperator() {
        require(amount <= 0.05 ether, "Fees must be lower than 5%");
        MARKETING_TX_FEE = amount;
    }
}
