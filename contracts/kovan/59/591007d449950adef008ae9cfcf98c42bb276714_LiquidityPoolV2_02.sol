...
// 
contract LiquidityPoolV2_02 is ReentrancyGuardUpgradeable, AccessControlUpgradeable, PausableUpgradeable, ERC20Upgradeable {
	...
	uint256[N_TOKENS] public TOKENS_MUL; // change it to private constant later
	IERC20Upgradeable[N_TOKENS] public tokens; // rename to TOKENS and change it to private constant later
	...
	function initialize(
		address[N_TOKENS] memory tokens_,
		uint256 depositFee_,
		uint256 borrowFee_,
		uint256 adminFee_
	)
		...
		initializer 
	{
		...
		for (uint256 i = 0; i < N_TOKENS; i++) {
			...
			tokens[i] = IERC20Upgradeable(tokens_[i]);
			TOKENS_MUL[i] = 10 ** (NORM_BASE - uint256(ERC20Upgradeable(tokens_[i]).decimals()));
		}
