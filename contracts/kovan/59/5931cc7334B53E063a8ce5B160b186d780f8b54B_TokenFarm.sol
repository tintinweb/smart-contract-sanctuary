...
contract TokenFarm is Ownable {
    ...
    function setPriceFeedContract(address _token, address _proceFeed) public onlyOwner {
        tokenPriceFeedMapping[_token] = _proceFeed;
    }

    function issueTokens() public onlyOwner {
        //issre tokens to all stakers
        for(uint256 stakersIndex = 0; stakersIndex < stakers.length; stakersIndex++){
            ...
            uint256 userTokenValue = getUserTotalValue(recipient);
            dappToken.transfer(recipient, userTokenValue);
        }        
    }
