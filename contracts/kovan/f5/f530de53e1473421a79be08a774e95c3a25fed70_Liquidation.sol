...
contract IControllerInterface {
    ...
    //是否允许当做保证金
    function isBailTokenAllowed(address _token) public view returns (bool);
...
contract IStorageInterface {
    ...
    function getController() public view returns (IControllerInterface);
}
...
// File: marginSwap/Liquidation.sol
...
contract Liquidation is ILiquidationInterface, Tools, ErrorReporter, CarefulMath {
    //直接清算
    ...
    constructor(IMSPInterface _msp) public {
        msp = _msp;
        capital = msp.capital();
        mspstorage = msp.mspstorage();
        admin = mspstorage.admin();
    ...
    function liquidateBorrowDrectlyFresh(
        ...
        (uint256 err, , , uint256 shortfall) = controller.getAccountLiquidity(_borrower, IMSPInterface(msp), vars.id);
        ...
        MSPStruct.MSPConfig memory mspconfig = mspstorage.getAccountMspConfig(_borrower, vars.id);

        //可以被清算
        if (mspconfig.isAutoSupply) {
            uint256 error = capital.disabledAndDoWithdraw(_borrower, vars.id, address(this));
            require(error == 0, "disabledAndDoWithdraw error!");
        }
        console.log("准备遍历保证金，进行兑换成债务资产!");

        //2. 将全部资产卖掉，得到债务资产，偿还债务，获得债务8%的奖励
        address[] memory bailAssests = mspstorage.getBailAddress(_borrower, vars.id);

        //1. 遍历所有的保证金(包括兑换回来的UNI，空的BUSD，以及后续追加资产)
        for (uint256 i = 0; i < bailAssests.length; i++) {
            vars.currAsset = bailAssests[i];
            MSPStruct.supplyConfig memory scs = mspstorage.getSupplyConfig(_borrower, vars.id, vars.currAsset);
            ...
            capital.doTransferOut(wallet, vars.currAsset, scs.supplyAmount, address(this));
            ...
            if (assetUnderlying == vars.currAsset) {
                EIP20Interface(vars.currAsset).approve(address(capital), scs.supplyAmount);
                capital.doTransferIn(address(this), vars.currAsset, scs.supplyAmount, address(this));
                vars.totalAsset = vars.totalAsset.add(scs.supplyAmount);
            } else {
                //TODO 临时
                // txAggregator.setPToken(controller.getPToken(vars.currAsset), controller.getPToken(assetUnderlying));
            ...
            scs.supplyAmount = 0;
            mspstorage.setSupplyConfig(_borrower, vars.id, vars.currAsset, scs);
            mspstorage.deleteBailAddress(_borrower, vars.id, vars.currAsset);
        }
        ...
        //要考虑穿仓吗？
        //comp情况：清算人先偿还债务，输入参数：借款人，偿还金额（债务），抵押物pToken，偿还部分债务之后，得到对应pToken的1.08倍
        //由于已经偿还了，所以和穿仓与否没有关系，直接去拿pToken就行了，如果现在抵押物太低了，1.08倍数量已经超过用户拥有的pToken数量，那么交易回滚。
        //因此穿仓和comp没有关系

        /**
            直接清算时： 清算人将所有资产卖掉，此时会出现：
            //1. 先将债务0.08拿走
            //2. 从剩余的中扣除债务部分
            //3. 如果还有剩余，返回给用户

            1. 资产充足，可以抵债，直接将债务还清，将债务1.08作为奖励分给AB，剩余资产还给用户
            2. 资产不足 1. 先将债务1.08作为奖励拿到，剩余资产直接还债，剩余的债务保留
        */
        ...
        capital.doTransferOut(liquidator, assetUnderlying, vars.benifits, address(this));

        //2. 从剩余的中扣除债务部分
        vars.remains = vars.totalAsset.sub(vars.benifits);

        if (vars.remains < vars.needToPayback) {
            (uint256 err1, uint256 actualAmt) = capital.doCreditLoanRepayInternal(_borrower, vars.remains, vars.id, address(this));
            ...
            //3. 如果还有剩余，返回给用户
            (uint256 err1, uint256 actualAmt) = capital.doCreditLoanRepayInternal(_borrower, vars.needToPayback, vars.id, address(this));
            ...
            if (vars.backToAccountAmt > 0) {
                ...
                capital.doTransferOut(borrower, assetUnderlying, vars.backToAccountAmt, address(this));
            }
        }
    }
    ...
    function liquidateBorrowFresh(
        ...
        capital.doTransferIn(msg.sender, assetUnderlying, vars.repayAmount, address(this));
        // console.log("vars.repayAmount:", vars.repayAmount);

        (uint256 err, uint256 actualAmt) = capital.doCreditLoanRepayInternal(_borrower, vars.repayAmount, vars.positionId, address(this));
        ...
        MSPStruct.MSPConfig memory mspconfig = mspstorage.getAccountMspConfig(_borrower, _id);
    ...
    function seizeInternal(
        ...
        //1. 空调用 //TODO 未实现
        // uint256 allowed = controller.seizeAllowed(address(this), _seizerToken, _liquidator, _borrower, _seizeTokens);
        // if (allowed != 0) {
        //     return failOpaque(Error1.COMPTROLLER_REJECTION, FailureInfo1.LIQUIDATE_SEIZE_COMPTROLLER_REJECTION, allowed);
        // }
        ...
        // MSPStruct.BailConfig storage borrowerCfg = MSPStruct.bailConfigs[_borrower][_id];
        // supplyConfigSingle storage borrowerScs = borrowerCfg.bailCfgContainer[_collateralUnderlying];
        MSPStruct.supplyConfig memory borrowerSC = mspstorage.getSupplyConfig(_borrower, _id, _collateralUnderlying);
        ...
        if (_isAutoSupply) {
            ...
            capital.doTransferOut(_liquidator, pTokenCollateral, _seizeTokens, address(this));
        } else {
            ...
            //5. 清算人直接获得Token到钱包
            capital.doTransferOut(_liquidator, _collateralUnderlying, _seizeTokens, address(this));
        }

        //勿忘更新!
        mspstorage.setSupplyConfig(_borrower, _id, _collateralUnderlying, borrowerSC);
        if (borrowerSC.pTokenAmount == 0 && borrowerSC.supplyAmount == 0) {
            mspstorage.deleteBailAddress(_borrower, _id, _collateralUnderlying);
        }
