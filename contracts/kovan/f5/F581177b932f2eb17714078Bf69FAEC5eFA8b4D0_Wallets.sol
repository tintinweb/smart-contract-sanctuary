...
contract Wallets is AccessControl, IWallets {
  ...
  event AssetChanged(address asset, bool isAllowed, uint256 max);
  ...
  function depositWallet(address asset, uint256 amount) public override {
    ...
    require(asset != address(0), "depositWallet: Empty user address!");
    require(_isAllowedAsset(asset), "depositWallet: Asset is not allowed!");
    require(amount <= _getUserAvailableLimit(asset, msg.sender), "depositWallet: User limit is reached");
  ...
  function withdrawFromWallet(address asset, uint256 amount) public override {
    ...
    require(asset != address(0), "withdrawFromWallet: Empty user address!");
  ...
  function depositEthWallet() public payable override {
    ...
    require(msg.value <= _getUserAvailableLimit(ETH_ADDRESS, msg.sender), "depositEthWallet: User limit is reached");
  ...
  function depositFromUser(address user, address asset, uint256 amount) public override {
    require(user != address(0), "depositFromUser: Empty user address!");
    require(asset != address(0), "depositFromUser: Empty asset address!");
    require(hasRole(DAO_CONTRACT, msg.sender), "depositFromUser: Only DAO contracts can call this method!");
    require(_isAllowedAsset(asset), "depositFromUser: Asset is not allowed!");
    require(amount <= _getUserAvailableLimit(asset, user), "depositFromUser: User limit is reached");
  ...
  function withdrawFromUser(address user, address asset, uint256 amount) public override {
    require(user != address(0), "withdrawFromUser: Empty user address!");
    require(asset != address(0), "withdrawFromUser: Empty asset address!");
  ...
  /// @dev Deposit ETH, can be called by DAO contracts only.
  /// It is done for combining 2 method in 1 transaction (eg deposit + join SHO and etc).
  function depositFromUser(address user) public payable override {
    require(user != address(0), "depositFromUser: Empty user address!");
    require(hasRole(DAO_CONTRACT, msg.sender), "depositFromUser: Only DAO contracts can call this method!");
    require(_isAllowedAsset(ETH_ADDRESS), "depositFromUser: Asset is not allowed!");
    require(msg.value <= _getUserAvailableLimit(ETH_ADDRESS, user), "depositFromUser: User limit is reached");

    _depositStateChange(user, ETH_ADDRESS, msg.value);

    emit NewDeposit(user, ETH_ADDRESS, msg.value);
  }

  /// @dev When winners are known the SHO contract should be able to receive ETH of the users.
  /// ETH will be transferred to SHO contract, and then to the project wallet address.
  function withdrawFromUser(address user, uint256 amount) public override {
    require(amount > 0, "withdrawFromUser: Amount should be bigger 0!");
    require(user != address(0), "withdrawFromUser: Empty user address!");
    require(hasRole(DAO_CONTRACT, msg.sender), "withdrawFromUser: Only DAO contracts can claim users deposits!");
    require(_balances[user][ETH_ADDRESS].available >= amount, "withdrawFromUser: Not enough deposited assets!");

    _withdrawStateChange(user, ETH_ADDRESS, amount);
    _transferEth(msg.sender, amount);

    emit DaoWithdraw(user, ETH_ADDRESS, amount);
  }

  /// @dev If asset deposit state is enabled, user can deposit that assets to Wallet contract.
  /// Each asset has his min and max size for deposits.
  function changeAssetDepositState(address asset, bool isAllowed, uint256 max) public override {
    require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "changeAssetDepositState: Only admin allowed call this method!");
    ...
    emit AssetChanged(asset, isAllowed, max);
  }
  ...
  /// @dev Get user balance, which can be locked for SHO or withdraw by user.
  function getUserAvailableLimit(address user, address asset) public view override returns (uint256) {
    return _getUserAvailableLimit(user, asset);
  }
  ...
  function _transferEth(address payable receiver, uint256 amount) internal {
    ...
    require(success, "_transferEth: Transfer failed!");
  }

  function _isAllowedAsset(address asset) internal view returns (bool) {
    return _assets[asset].isAllowed;
  }

  function _getUserAvailableLimit(address asset, address user) internal view returns (uint256) {
    if (_assets[asset].max <= _balances[user][asset].available) {
      return 0;
    }

    return _assets[asset].max.sub(_balances[user][asset].available);
  }
