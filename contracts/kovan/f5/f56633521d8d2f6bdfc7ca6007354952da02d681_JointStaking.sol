...
abstract contract LPTokenWrapper is GovernableProxy, StorageBuffer {
    ...
    constructor(address _ice, address _lpToken) public {
...
contract JointStaking is LPTokenWrapper {
    ...
    function sushiPerToken() public view returns (uint256) {
        return _sushiPerToken(masterChef.pendingSushi(pid, address(this))).mul(2);
    }
    ...
    function earned(address account) public view returns (uint256) {
        return _sushiEarned(account, sushiPerToken()).mul(rewardRate).div(DIVISIONER);
    }
    ...
    function getReward() public updateReward(msg.sender) {
        ...
        if (reward > 0) {
            ...
            reward = reward.mul(rewardRate).div(DIVISIONER);
