...
contract TokenFarm is Ownable {
  ...
  // mapping(address => uint256) internal uniqueTokensStaked;
  // mapping(address => address) internal tokenPriceFeedMapping;
  ...
  function unstakeTokens(address _token) public { // IS THIS REENTRENCY ATTACK VULNERABLE ?
  ...
  function getUserSingleTokenValue(address _user, address _token) internal view returns(uint256) {
    if (uniqueTokensStaked[_user] <= 0) {
      return 0;
    }
  ...
  function getTokenValue(address _token) internal view returns(uint256, uint256) {
  ...
  function removeStacker(address _user) internal {
    int256 index = -1;
    ...
    for (uint256 i = uint256(index); i < stakers.length - 1; i++){
