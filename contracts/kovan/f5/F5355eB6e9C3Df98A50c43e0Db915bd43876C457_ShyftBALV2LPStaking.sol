  ...
  function getTwoTokensReward(
    ...
    if (pendingUSD > 0 && priceA > 0 && priceB > 0) {
      ...
      if (totalValue > 0) {
        ...
        amountA = shareA.mul(pendingUSD).div(priceA).div(1e18).div(1e16); // div(1e18) for pendingUSD, div(1e16) for shareA(:priceA)
        amountB = shareB.mul(pendingUSD).div(priceB).div(1e18).div(1e16); // div(1e18) for pendingUSD, div(1e16) for shareB(:priceB)
      }
    }
  }
