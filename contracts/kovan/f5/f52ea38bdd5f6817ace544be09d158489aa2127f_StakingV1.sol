...
contract StakingV1 is ReentrancyGuard, Ownable {
    ...
    function allocateRewards(uint _s, uint _p, address feeToken) public returns (uint _rewards) {
        ...
        uint share = st.periodTokens[_p].mul(1e6).div(p.totalStaked); //six decimals of precision
        _rewards = c.feesCollected.mul(share).div(1e6); //rewards are then the total fees earnd by the pool times the share
