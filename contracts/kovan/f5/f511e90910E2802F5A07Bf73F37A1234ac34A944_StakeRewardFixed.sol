...
contract StakeRewardFixed is Initializable, BumperAccessControl {
    ...
    event Unstaked(address sender, uint256 amount, uint256 rewards );
    ...
    /// @param amount - amount to withdraw, uint.max means all tokens
    function unstake(uint16 stakeIndex, uint amount) external {
        ...
        if (s.autorenewAt > 0) {
            ...
            uint endOfLastPeriod = s.start + periods * 1 days;
            require( block.timestamp > endOfLastPeriod && block.timestamp < endOfLastPeriod + withdrawWindow, "!window" );
            // calculate rewards
            rewards = periods * s.rewards;
        }
        else {
    ...
    function claimRewards(uint16 stakeIndex) external {
        ...
        require( rewards > s.claimed, "!rewards" );

        rewards -= s.claimed;
        s.claimed = rewards + s.claimed;
