...
import "onetoken/contracts/_openzeppelin/math/SafeMath.sol";

/**
 @notice UNI/USD ChainLink Oracle 
 mainnet - 0x553303d460EE0afB37EdFf9bE42922D8FF63220e  
 kovan - 0xDA5904BdBfB4EF12a3955aEcA103F51dc87c7C39
 */

contract UNIOracle is OracleCommon {
    ...
    function read(address token, uint256 amountTokens) external view override returns(uint256 amountUsd, uint256 volatility) {
        amountTokens = (amountTokens.mul(getThePrice())).div(10 ** 18);
    ...
    function amountRequired(address token, uint256 amountUsd) external view override returns(uint256 amountTokens, uint256 volatility) {
        amountUsd = normalizedToTokens(token, amountUsd);
    ...
    function getThePrice() public view returns (uint256 price) {
        ...
        price = price.mul(10 ** 10);  //price is natively in 8 decimals make it 18
    }
}
