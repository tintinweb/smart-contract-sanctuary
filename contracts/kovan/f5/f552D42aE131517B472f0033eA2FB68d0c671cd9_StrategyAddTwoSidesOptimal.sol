...
contract StrategyAddTwoSidesOptimal is ReentrancyGuard, IStrategy {
  ...
  constructor(IUniswapV2Router02 _router, IVault _vault) public {
  ...
  function execute(address user, bytes calldata data) external override nonReentrant
  {
    ...
    vault.requestFunds(user, farmingToken, farmingTokenAmount);
    ...
    require(lpToken.transfer(user, lpToken.balanceOf(address(this))), "StrategyAddTwoSidesOptimal::execute:: failed to transfer LP token to msg.sender");
