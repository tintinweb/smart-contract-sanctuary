...
contract CataLystBridge is Manager {
    ...
    function userWithDrawn(uint256 _roundId) isValidRound(_roundId) whenNotPaused external {
        ...
        require((block.timestamp <= round.startTime + round.depositDuration &&  round.totalWithDrawnFund == 0) ||
                (block.timestamp >= (round.startTime + round.depositDuration + round.stakeDuration) && round.totalWithDrawnFund > 0), "Can-not-withdrawn-now"); // deposit is closed and allow withdrawn
        uint256 amountToWithdrawn = fundOfUser * round.totalWithDrawnFund / round.totalDepositFund;
