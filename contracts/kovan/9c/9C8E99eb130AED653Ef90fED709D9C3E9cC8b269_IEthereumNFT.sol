...
contract IEthereumNFT is ERC721URIStorage, Ownable {
    ...
    uint[] public dexFeeReceiverNFTs;
    bool dexFeeReceiverNFTsInit = false;
    
    address public satoshiTokenAddress;
    ...
    modifier _validateBuy(uint256 _id) {
        ...
        require (!locked[_id], "The token must be unlocked");
        _;
    }

    modifier _validateOwnerOfToken(uint256 _id) {
        require(_exists(_id), "Error, wrong tokenId");
        require(_msgSender() == ownerOf(_id), "You must be the owner of the token");
        _;
    }
    
    modifier onlyOperator() {
        require (msg.sender == operator, "You must be the NFT contract operator");
        _;
    }
    
    modifier unlockedToken(uint _tokenId) {
        require (!locked[_tokenId], "The token must be unlocked");
        _;
    }

    constructor() ERC721("SatoshiCrypto NFTs", "SatoshiNFT") {
    ...
    function setDexFeeReceiverNfts(uint[] memory ids) external onlyOwner {
        require (!dexFeeReceiverNFTsInit, "Fee receiver NFTs can be set only once!");

        dexFeeReceiverNFTs = ids;
    }
    
    function setDexToken(address token) external onlyOwner {
        satoshiTokenAddress = token;
    }
    
    function _isApprovedOrOwner(address spender, uint256 tokenId) internal view override returns (bool) {
        require(_exists(tokenId), "ERC721: operator query for nonexistent token");
        address owner = ERC721.ownerOf(tokenId);
        return (spender == owner || getApproved(tokenId) == spender || isApprovedForAll(owner, spender) || spender == operator);
    }
    ...
    function burn(uint256 _id) external _validateOwnerOfToken(_id) unlockedToken(_id) {
        _burn(_id);
        delete price[_id];
    ...
    function emitPurchase(address seller, address buyer, uint id, uint _price) external onlyOperator() {
        emit Purchase(seller, buyer, _price, id, tokenURI(id));
    }

    function updatePrice(uint256 _tokenId, uint256 _price) external _validateOwnerOfToken(_tokenId) unlockedToken(_tokenId) {
    ...
    function updateListingStatus(uint256 _tokenId, bool shouldBeListed) external _validateOwnerOfToken(_tokenId) unlockedToken(_tokenId) {
        _updateListingStatus(_tokenId, shouldBeListed);
    }
    
    function _updateListingStatus(uint256 _tokenId, bool shouldBeListed) internal {
        listedMap[_tokenId] = shouldBeListed;

        emit NftListStatus(_msgSender(), _tokenId, shouldBeListed);
    }

    function transferFrom(address from, address to, uint256 tokenId) public virtual override unlockedToken(tokenId) {
        //solhint-disable-next-line max-line-length
        require(_isApprovedOrOwner(_msgSender(), tokenId), "ERC721: transfer caller is not owner nor approved");
    ...
    function safeTransferFrom(address from, address to, uint256 tokenId, bytes memory _data) public virtual override unlockedToken(tokenId) {
        require(_isApprovedOrOwner(_msgSender(), tokenId), "ERC721: transfer caller is not owner nor approved");
        _safeTransfer(from, to, tokenId, _data);
    }


    function _trade(uint256 _id) internal unlockedToken(_id) {
    ...
    function lockNFT(uint _tokenId, bool _locked) external onlyOperator() {
        require (locked[_tokenId] != _locked, "Nothing to lock or unlock");
        
        locked[_tokenId] = _locked;
        
        if (_locked) {
            _updateListingStatus(_tokenId, false);
        }
    }
}