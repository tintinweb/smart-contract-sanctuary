 ...
 * logic.
 */
contract RankedAuction is VersionedInit, AdminPausableUpgradeSafe, WETHBase, ReentrancyGuard {
     ...
     */
    event BidsUpdated(address[] outbids, uint256 minimumPrice);
     ...
     */
    function bid(address onBehalfOf, uint256 amount) external nonReentrant whenNotPaused {
     ...
     */
    function bidWithPermit(DataTypes.SimpleBidWithPermitParams memory params) external nonReentrant whenNotPaused {
    ...
    function withdrawBid() external nonReentrant whenNotPaused {
        ...
        if (_currency == address(WETH)) {
            ...
            if (!success) {
                ...
                require(WETH.transferFrom(address(this), msg.sender, returnAmount));
            }
        } else {
     ...
     */
    function receiveFunds(address [] memory toReceive) external nonReentrant onlyAdmin {
