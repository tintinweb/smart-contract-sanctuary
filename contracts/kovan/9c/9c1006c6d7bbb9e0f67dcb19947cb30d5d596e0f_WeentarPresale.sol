...
//Contract based on https://docs.openzeppelin.com/contracts/3.x/erc20
...
contract WeentarPresale is Context, Ownable, ReentrancyGuard {
    ...
    function purchaseToken(uint256 amount) public payable nonReentrant {
        ...
        require(tokenPrice() != 0, "Sale: Token Price is not set");
        require(amount * 1 ether <= _phaseSupplyLeft, "Sale: Amount exceeds remaining supply of the current phase.");
    ...
    function setCurrentPhase(uint256 supply, uint256 timestampStart, uint256 timestampEnd) public onlyOwner {
        require(supply <= _token.balanceOf(address(this)), "Sale: Supply value exceeds the token balance");
        require(timestampStart >= block.timestamp, "Sale: opening time is before current time");
        require(timestampEnd > timestampStart, "Sale: opening time is not before closing time");
