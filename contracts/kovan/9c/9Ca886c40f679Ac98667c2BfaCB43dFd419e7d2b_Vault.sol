    ...
    ReentrancyGuard
{
    ...
    bool startStopNft; // True - Use NFT functionality, False - Stop NFT functionality
     ...
     * @param _increaseMTS Number for increase maxTotalSupply (summed with maxTotalSupply)
     * @param _startStopNft True - Use NFT functionality, False - Stop NFT functionality
    ...
    constructor(
        ...
        increaseMTS = _increaseMTS;

        NFT_TOKEN_ADDRESS = _nft;

        isGetGift[_msgSender()] = false;

        startStopNft = _startStopNft;
    ...
    {
        // Check if User have NFT token  
        // Works if NFT functionality is Enabled
        if (startStopNft == true) {
            require(
        ...
        // Works if NFT functionality is Enabled
        if ((InvitationalErc721(NFT_TOKEN_ADDRESS).ownerOf(_tokenNftId) == _msgSender())&&(startStopNft == true))
            require(totalSupply() <= maxTotalSupply + increaseMTS, "maxTotalSupply");
        else
    ...
    function withdraw(
        ...
        // Works if NFT functionality is Enabled
        if (balanceOf(_msgSender()) == 0 && !(block.timestamp >= startTime[_msgSender()] + daysAfter) && (startStopNft == true)) {
     ...
     */
    function rerange(int24 _tickRange, uint8 _poolId) external onlyGovernance {
    ...
    function setDaysAfter(uint256 _setPeriodInDays) external onlyGovernance {
    ...
    // Show parameter which increase maxTotalSupply
    function showIncreaseMTS() public view returns (uint256) {
        return increaseMTS;
    } 

    // Change parameter which increase maxTotalSupply
    function changeIncreseMTS(uint256 _increaseMTS) external onlyGovernance {
        increaseMTS = _increaseMTS;
    }
    
    // Function for Enabling/Disabling Functionality connected with NFT
    function setStartStopNFT(bool _startStopNft) external onlyGovernance {
        startStopNft = _startStopNft;
    } 

    // Show if Functionality connected with NFT is Enabled(True) or Disabled(False)
    function showStartStopNft() public view returns (bool) {
        return startStopNft;
    }
