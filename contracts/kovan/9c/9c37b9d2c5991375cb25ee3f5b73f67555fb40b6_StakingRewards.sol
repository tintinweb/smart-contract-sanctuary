...
contract StakingRewards is Initializable, OwnableUpgradeable, IStakingRewards, RewardsDistributionRecipient, ReentrancyGuardUpgradeable, PausableUpgradeable , ERC1155HolderUpgradeable {
    ...
    // gives the array of staked token ids
    function getStakedTokenIds(address account) public view returns (uint256[] memory){
        uint256 totalNFTs = stakingToken._currentTokenID();
        uint256[] memory stakedTokenIds=new uint256[](totalNFTs);
        uint256 totalStaked = 0;
        for(uint256 i=0;i<totalNFTs;i++){
            if(_tokenBalances[account][i]>0){
                stakedTokenIds[totalStaked] = i;
                totalStaked++;
            }
        }
				uint[] memory trimmedResult = new uint[](totalStaked);
			  for (uint j = 0; j < trimmedResult.length; j++) {
        	trimmedResult[j] = stakedTokenIds[j];
    		}
        return trimmedResult; 
    }


    function balanceOfNFT(address account, uint256 tokenId) public override view returns (uint256) {
