...
contract MasterChef is Ownable {
    ...
    // Max mint
    uint256 public MAX_MINT = 8400000 * 10 ** 18;
    // Total mint
    ...
    event EmergencyHarvest(address indexed user, uint256 indexed pid, uint256 amount, uint256 treasury);
    ...
    // Withdraw LP tokens from MasterChef.
    function emergencyHarvest(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        ...
        uint256 claimAmount = totalHarvest;

        if (totalHarvest > 0) {
            unclaimedBuni[_pid][msg.sender] = 0;

            uint256 claimAmount = totalHarvest.div(2);
            
            buni.mint(msg.sender, claimAmount);
            buni.mint(treasury, claimAmount);
        }
        require(claimAmount <= totalHarvest, "harvest: not good");
        emit EmergencyHarvest(msg.sender, _pid, claimAmount, claimAmount);
    }
...
interface IERC721 is IERC165 {
    ...
    /**
      * @dev get vested data info in one token
      *
      * Requirements:
      *
...
contract BuniToken is BEP20('Buni Token', 'BUNI') {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "BUNI::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "BUNI::delegateBySig: invalid nonce");
        require(now <= expiry, "BUNI::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "BUNI::getPriorVotes: not yet determined");
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "BUNI::_writeCheckpoint: block number exceeds 32 bits");
