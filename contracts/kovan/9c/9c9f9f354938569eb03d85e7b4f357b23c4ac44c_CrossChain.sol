...
contract CrossChain is Pausable, Ownable{
    ...
    uint256 public confirmRequireNum = 2;
    ...
    function crossChainTransfer(address token, uint256 amount, address to, Chain chain) external payable whenNotPaused returns (bool){
        ...
        //need to limit total amount per day
        IERC20CrossChain(token).transferFrom(msg.sender, address(this), amount);
        IERC20CrossChain(token).burn(amount);
        emit CrossChainTransfer(msg.sender, amount, token, to, chain, msg.value);
        return true;
    }

    function receiveToken(address token, uint256 amount, address receiveAddress, string memory info) external payable onlyRelayer whenNotPaused returns (bool){
        ...
        if(relayInfo[relayInfoHash].length < confirmRequireNum){
            ...
            IERC20CrossChain(token).mint(receiveAddress, amount);
            emit ReceivingToken(receiveAddress, token, amount, info);
            emit ReceiveTokenDone(receiveAddress, token, amount, info);
        }
    }

    function transferToken(address token, uint256 amount, address to) external onlyAdmin returns (bool){
        IERC20CrossChain(token).transfer(to, amount);
    }

    function transfer(uint256 amount, address payable to) external onlyOwner returns (bool){
        to.transfer(amount);
    }
    ...
    function addTotalAmount(address token, uint256 totalAmount, uint256 amount) internal returns (uint256){
        ...
        require(totalAmount <= maxAmountPerDay[token], "Total amount is over max amount");
        return totalAmount;
    }

    function hasRelay(bytes32 relayInfoHash) internal returns (bool){
