...
contract VolumeEscrow is VolumeOwnable , ReentrancyGuard  {
    ...
    address wbnb = 0xd0A1E359811322d97991E03f863a0C30C2cF029C; // WETH on Kovan
    address volume;
    address bakeryRouter = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D; // UNI Router on Kovan
    address lpPool;
    ...
    uint256 totalVolLeftAfterRugPull;
    uint256 totalBNBAfterRugPull;
    uint256 VolBNBPrice;

    /**
     * @dev Constructor.
     */
    constructor(address multisig_) VolumeOwnable(multisig_) {
     ...
     */
    function initialize ( uint256[] memory allocations_ , address volumeAddress) external onlyOwner returns (uint) {
        require(volumeAddress != address(0), "volumeAddress can't be address zero");
        require(initialized != 1, "already initialized ");
    ...
    function createLP () external onlyOwner nonReentrant {
        ...
        address lpPair = IBakerySwapFactory(IBakerySwapRouter(bakeryRouter).factory()).getPair(volume, wbnb);
        require(lpPair == address(0) , 'already created');
        IBEP20(wbnb).safeApprove(bakeryRouter, wbnbBalance);
        IBEP20(volume).safeApprove(bakeryRouter, _allocations[1]);

        // We assume we are the first to create the liquidity 
        IBakerySwapRouter(bakeryRouter).addLiquidity(
            volume,
             wbnb,
            _allocations[1],
            wbnbBalance,
            _allocations[1] - (_allocations[1]/100), // 1% slipage 
            wbnbBalance - (wbnbBalance/100), // 1% slipage 
            address(this),
            block.timestamp + 120000
            );
        
        lpPair = IBakerySwapFactory(IBakerySwapRouter(bakeryRouter).factory()).getPair(volume , wbnb);
        require(lpPair != address(0) , 'lpPair not created');
        lpPool = lpPair;
    }

    /*
    function redeemAfterRugPull (uint256 amount) {
    ...
    */
    function rugPullSimulation (uint slipage) external onlyOwner {
        ...
        require(slipage > 0 && slipage < 100, "There is still fuel can't rug pull");
        ...
        uint256 minVolOut = volPoolBalance * (escrowlpBlance / totalLpSupply);
        uint256 minWBNBOut = wbnbPoolBalance * (escrowlpBlance / totalLpSupply);
        ...
        (uint256 volumeOut , uint256 wbnbOut) = IBakerySwapRouter(bakeryRouter).removeLiquidity(
                                                            ...
                                                            wbnb,
                                                            escrowlpBlance,
                                                            minVolOut - ((minVolOut * slipage )/1000) ,
                                                            minWBNBOut - ((minWBNBOut * slipage)/1000),
                                                            address(this),
                                                            0
                                                        );

        // this should never happen unless something malicious is going on so lets useup all gas left and revert
        assert(volumeOut >= minVolOut && wbnbOut >=  minWBNBOut);

        // the only redeamable vol left is the one in all user's wallets and the team allocation 
        totalVolLeftAfterRugPull = IBEP20(volume).totalSupply() - volumeOut - _allocations[4] - _allocations[2];
        totalBNBAfterRugPull = wbnbOut;
        VolBNBPrice = wbnbOut / totalVolLeftAfterRugPull;
        rugpulled = 1;
    }
    ...
    function setLPAddress (address poolAddress) external onlyOwner {
         require(poolAddress != address(0), "poolAddress: can't be zero address");
        lpPool = poolAddress;
    }
    ...
    function getLPAddress () external view returns (address) {
        require(lpPool != address(0), "Lp address was not set yet" );
        return lpPool;
    }
    ...
    function getVolumeAddress () external view returns (address) {
        require(volume != address(0), "volume address was not set yet" );
        return volume;
    }
    ...
    function getInitialized () external view returns (uint ){
        return initialized;
    }

    function _subAllocation(uint id , uint256 amount) internal {
        _allocations[id] = _allocations[id] - amount;
    }
}
...
interface IVolumeBEP20 is IBEP20{
