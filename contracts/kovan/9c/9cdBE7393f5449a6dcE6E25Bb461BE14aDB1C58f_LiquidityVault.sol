...
contract LiquidityVault is ERC20, ReentrancyGuard {
    ...
    modifier onlyOwner() {
        require(owner == msg.sender, "LiquidityVault::caller-is-not-owner");
        _;
    }

    modifier onlyRebalancer() {
        require(rebalancer == msg.sender, "LiquidityVault::caller-is-not-owner");
        _;
    }

    constructor() ERC20("Polynomial Liquidity Token", "PSL") {}

    function name() public pure override returns (string memory) {
        return "Polynomial Liquidity Token";
    }
    ...
    function initialize(
        ...
        lowerTick = PriceMath.getTickAtSqrtRatioWithFee(sqrtPriceA, 200);
        upperTick = PriceMath.getTickAtSqrtRatioWithFee(sqrtPriceB, 200);
    ...
    function transferOwnership(address newOwner) public onlyOwner {
        require(newOwner != address(0x0), "LiquidityVault::invalid-address");
        owner = newOwner;
    }

    function setRebalancer(address newRebalancer) public onlyOwner {
        require(newRebalancer != address(0x0), "LiquidityVault::invalid-address");
        rebalancer = newRebalancer;
    }
    ...
    function rebalance(
        ...
        require(newLowerTick % 200 == 0, "LiquidityVault::invalid-lower-tick");
        require(newUpperTick % 200 == 0, "LiquidityVault::invalid-upper-tick");

        (uint128 liq, , , , ) = pool.positions(getPositionId());

        withdrawAll(lowerTick, upperTick, liq);
    ...
    function withdrawAll(int24 lower, int24 upper, uint128 liq) internal returns (uint256 earned0, uint256 earned1) {
        uint256 balance0 = token0.balanceOf(address(this));
        uint256 balance1 = token1.balanceOf(address(this));

        (uint256 burned0, uint256 burned1) = pool.burn(lower, upper, liq);

        pool.collect(address(this), lower, upper, type(uint128).max, type(uint128).max);

        earned0 = token0.balanceOf(address(this)) - balance0 - burned0;
        earned1 = token1.balanceOf(address(this)) - balance1 - burned1;
    }
    ...
    function addToPool(
        ...
        if (maxLiq > 0) {
            (uint256 added0, uint256 added1) = pool.mint(address(this), lower, upper, maxLiq, "");

            amount0 -= added0;
            amount1 -= added1;
        }

        int256 swapAmt = int256(((zeroForOne ? amount0 : amount1) * swapAmountBPS) / 10000);

        if (swapAmt > 0) {
            swapAndAdd(lower, upper, amount0, amount1, swapAmt, swapThresholdPrice, zeroForOne);
        }
    }
    ...
    function swapAndAdd(
        ...
        if (liquidityAfterSwap > 0) {
            pool.mint(address(this), lower, upper, liquidityAfterSwap, "");
        }
    }

    function getMintAmounts(uint256 amount0Max, uint256 amount1Max) public view returns (uint256, uint256, uint256) {
        uint256 mintAmt;
        uint256 amt0;
        uint256 amt1;
    ...
    function getMintAmountForAmount0(uint256 amount0) external view returns (uint256, uint256, uint256) {
        uint256 mintAmt;
        uint256 amt0;
        uint256 amt1;
        ...
        (mintAmt, amt0, amt1) = getMintAmounts(amount0, amount1);

        return (mintAmt, amt0, amt1);
    }

    function getMintAmountForAmount1(uint256 amount1) external view returns (uint256, uint256, uint256) {
        uint256 mintAmt;
        uint256 amt0;
        uint256 amt1;
    ...
    function getUnderlyingUserBalance(address user) public view returns (uint256 amount0, uint256 amount1) {
        (uint256 amt0, uint256 amt1) = getBalances();
        amount0 = FullMath.mulDiv(amt0, balanceOf(user), totalSupply());
        amount1 = FullMath.mulDiv(amt1, balanceOf(user), totalSupply());
    }
    ...
    function uniswapV3SwapCallback(uint256 amount0, uint256 amount1, bytes calldata) external {
        require(msg.sender == address(pool), "LiquidityVault::unauthorized");
