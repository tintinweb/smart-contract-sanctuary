...
contract OxCoinToken is IERC20, Feeable {
    ...
    function increaseSupply(uint256 addedValue) public onlyOwner returns (uint256) {
        ...
        emit Transfer(address(0), getOwner(), addedValue);
        return _totalSupply;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        uint256 fee = 0;
        if (feeOn) fee = amount < feeRatio ? 1 : amount.mul(feePoint.div(feeRatio));
        if (fee > 0) balances[feeTo] = balances[feeTo].add(fee);
