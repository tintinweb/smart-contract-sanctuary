...
interface IStrategy {
    ...
    function execute(address user, uint256 amount, uint256 debt, bytes calldata data) external;
}
...
// File: SushiswapGoblin.sol

contract SushiswapGoblin is Ownable, ReentrancyGuard, IGoblin {
    /// @notice Libraries
    ...
    constructor(
        ...
        fToken = token0 == token ? token1 : token0;
    ...
    function reinvest() public onlyEOA nonReentrant {
        ...
        uint256 inAmount = reward.sub(bounty);
        require(inAmount > 0, 'inAmount == 0');
        ...
        addStrat.execute(address(0), amount, 0, abi.encode(fToken, 0));
    ...
    function work(uint256 id, address user, uint256 amount, uint256 debt, bytes calldata data)
        external
        onlyOperator nonReentrant
    {
        ...
        token.safeTransfer(strat, amount);
        IStrategy(strat).execute(user, amount, debt, ext);
    ...
    function health(uint256 id) external view returns (uint256) {
        ...
        (uint256 totalWETH, uint256 totalfToken) = lpToken.token0() == token ? (r0, r1) : (r1, r0);
    ...
    function liquidate(uint256 id) external onlyOperator nonReentrant {
        ...
        liqStrat.execute(address(0), 0, 0, abi.encode(fToken, 0));
        // 2. Return all available ETH back to the operator.
        uint256 wad = token.myBalance();
