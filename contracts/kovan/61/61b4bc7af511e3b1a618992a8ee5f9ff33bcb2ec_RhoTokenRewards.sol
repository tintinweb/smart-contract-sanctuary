    ...
    ReentrancyGuardUpgradeable
{
    ...
    function rewardOf(address user, address rhoTokenAddr) external view override returns (uint256) {
        return _earned(user, rhoTokenAddr);
    }
    ...
    function rewardsPerToken(address rhoTokenAddr) public view override returns (uint256) {
        RhoTokenInfo storage _rhoToken = rhoTokenInfo[rhoTokenAddr];
        ...
        return rewardPerTokenInternal(
            ...
            lastBlockApplicable(rhoTokenAddr) - _rhoToken.lastUpdateBlock,
            rewardRate,
    ...
    function lastBlockApplicable(address rhoTokenAddr) internal view returns (uint256) {
        return _lastBlockApplicable(rhoTokenInfo[rhoTokenAddr].rewardEndBlock);
    }

    function _earned(address user, address rhoTokenAddr) internal view returns (uint256) {
        UserInfo storage _user = userInfo[rhoTokenAddr][user];
        return super._earned(
            IERC20Upgradeable(rhoTokenAddr).balanceOf(user),
            rewardsPerToken(rhoTokenAddr) - _user.rewardPerTokenPaid,
            rhoTokenInfo[rhoTokenAddr].rhoTokenOne,
            _user.reward
        );
    }

    function updateReward(address user, address rhoTokenAddr) public override {
        RhoTokenInfo storage rhoToken = rhoTokenInfo[rhoTokenAddr];
        rhoToken.rewardPerToken = rewardsPerToken(rhoTokenAddr);
        rhoToken.lastUpdateBlock = lastBlockApplicable(rhoTokenAddr);

        if (user != address(0)) {
            userInfo[rhoTokenAddr][user].reward = _earned(user, rhoTokenAddr);
            userInfo[rhoTokenAddr][user].rewardPerTokenPaid = rhoToken.rewardPerToken;
        }
    }
    ...
    function startRewards(address rhoTokenAddr, uint256 rewardDuration) external override onlyRole(DEFAULT_ADMIN_ROLE) {
        require(rewardDuration > 0, "Reward duration cannot be zero");
        RhoTokenInfo storage rhoToken = rhoTokenInfo[rhoTokenAddr];
        require(
            block.number > rhoToken.rewardEndBlock,
            "Previous rewards period must complete before starting a new one"
        );
        updateReward(address(0), rhoTokenAddr);
        rhoToken.lastUpdateBlock = block.number;
        rhoToken.rewardEndBlock = block.number + rewardDuration;
        emit RewardsEndUpdated(rhoTokenAddr, block.number, rhoToken.rewardEndBlock);
    }


    function endRewards(address rhoTokenAddr) external override onlyRole(DEFAULT_ADMIN_ROLE) {
        RhoTokenInfo storage rhoToken = rhoTokenInfo[rhoTokenAddr];
        uint256 currentBlock = block.number;
        if (rhoToken.rewardEndBlock > currentBlock) {
            rhoToken.rewardEndBlock = currentBlock;
            emit RewardsEndUpdated(rhoTokenAddr, currentBlock, currentBlock);
        }
    }

    function claimReward(address user, address rhoTokenAddr) external override {
        address sender = _msgSender();
        require (sender == address(this) || sender == stakingRewardsAddress, "Only RhoTokenRewards or StakingRewards");
        updateReward(user, rhoTokenAddr);
        UserInfo storage _user = userInfo[rhoTokenAddr][user];
        if (_user.reward > 0) {
    ...
    function claimReward(address rhoTokenAddr) external override {
        this.claimReward(_msgSender(), rhoTokenAddr);
    }
    ...
    function hasRhoToken(address rhoTokenAddr) internal view returns (bool) {
        for (uint256 i = 0; i < rhoTokenList.length; i++) {
            if (rhoTokenList[i] == rhoTokenAddr)
                return true;
        }
        return false;
    }
    ...
    function addRhoToken(address rhoTokenAddr, uint256 allocPoint) public override onlyRole(DEFAULT_ADMIN_ROLE) {
        require(!hasRhoToken(rhoTokenAddr), "RhoToken already registered");
        updateRewardForAll();
        totalAllocPoint += allocPoint;
        rhoTokenList.push(rhoTokenAddr);
        uint256 currentBlock = block.number;

        rhoTokenInfo[rhoTokenAddr] = RhoTokenInfo({
            rhoToken: IERC20Upgradeable(rhoTokenAddr),
            allocPoint: allocPoint,
            lastUpdateBlock: currentBlock,
            rewardPerToken: 0,
            rewardEndBlock: currentBlock,
            rhoTokenOne: 10**IERC20MetadataUpgradeable(rhoTokenAddr).decimals()
        });
        emit RhoTokenAdded(rhoTokenAddr);
    }

    // Update the given rhoToken's allocation point. Can only be called by the owner.
    function setRhoToken(address rhoTokenAddr, uint256 allocPoint) public override onlyRole(DEFAULT_ADMIN_ROLE) {
        updateRewardForAll();
        totalAllocPoint = totalAllocPoint - rhoTokenInfo[rhoTokenAddr].allocPoint + allocPoint;
        rhoTokenInfo[rhoTokenAddr].allocPoint = allocPoint;
    }
