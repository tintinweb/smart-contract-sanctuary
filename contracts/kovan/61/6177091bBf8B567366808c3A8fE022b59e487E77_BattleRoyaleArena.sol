...
contract BattleRoyale is ERC721Tradable {
  ...
  // set to true when only wanting 1 sale per user
  bool public singleSale;
  ...
  /*
   * Toggle single sale on or off
   */
  function singleSaleOn(bool _singleSale) external payable onlyOwner {
    singleSale = _singleSale;
  }
  /*
...
contract BattleRoyaleArena is Ownable, VRFConsumerBase {
   ...
   * Toggle auto-start on or off
   */
  function singleSaleOn(address payable _nftAddress, bool _singleSale) external payable onlyOwner {
    require(_nftAddress != address(0));
    BattleRoyale battle = BattleRoyale(_nftAddress);

    battle.singleSaleOn(_singleSale);
  }
  /*
   * Set Fee Rate - aggreed rate the contract takes from the artist for initial sale
   */
  ...
  function testElimination() external onlyOwner returns(bool) {
    for (uint i = 0; i < battleQueue.size(); i++) {
      address payable nftAddress = battleQueue.atIndex(i);
      BattleRoyale battle = BattleRoyale(nftAddress);
      uint256 timestamp = battle.getTimestamp();
      ...
      && block.timestamp >= timestamp + (intervalTime * 1 minutes)) {
        ...
        battleQueue.removeByIndex(i);
        battleQueue.push(nftAddress);
        testUpkeep(addressToBytes(nftAddress));
        return true;
      }
    }
    return false;
  ...
  function testUpkeep(bytes memory performData) internal {
    address payable nftAddress = bytesToAddress(performData);
