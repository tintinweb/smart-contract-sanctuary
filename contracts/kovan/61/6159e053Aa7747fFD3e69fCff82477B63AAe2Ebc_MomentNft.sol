    ...
    NFT721Creator
{
    ...
    mapping(address => mapping(string => bool))
        private creatorToURIHashToMinted;

    // Base URI
    string private _BaseURI;
    event BaseURIUpdated(string baseURI);
    event TokenURIPathUpdated(
     ...
     */
    function getHasCreatorMintedURIHash(
        address creator,
        string memory tokenURIPath
     ...
     */
    function _setTokenURIPath(uint256 tokenId, string memory _tokenURIPath)
        internal
    {
        // 46 is the minimum length for an URI content hash, it may be longer if paths are used
        require(
            bytes(_tokenURIPath).length >= 46,
            "NFT721Metadata: Invalid URI path"
        );
        require(
            !creatorToURIHashToMinted[msg.sender][_tokenURIPath],
    ...
    function _burn(uint256 tokenId)
        internal
        virtual
        override(ERC721URIStorageUpgradeable, ERC721Upgradeable, NFT721Creator)
    {
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(ERC721Upgradeable, ERC721EnumerableUpgradeable, NFT721Creator)
        returns (bool)
    {
        return
            interfaceId == type(IERC721Upgradeable).interfaceId ||
