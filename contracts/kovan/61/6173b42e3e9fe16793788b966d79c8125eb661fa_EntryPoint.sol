...
contract LibVanilla is ILibrary {
    function something() external override {}
}

contract LibWithSelfDestruct is ILibrary {
    function something() external override {}

    function canSelfDestruct() external {
        assembly {
            selfdestruct(caller())
        }
    }
...
contract LibWithDelegate is ILibrary {
    function something() external override {}

    function canDelegate() external {
        bytes memory data = msg.data;
        address sender = msg.sender;
        uint ret;
        assembly {
            ret := delegatecall(gas(), sender, add(data, 32), mload(data), 0, 0)
        }
    }
}

contract WalletWithLib is IWallet {
    ILibrary something;
    constructor(ILibrary _something) {
...
contract AATestRefund {
    constructor() payable {
        TestRefund wallet = new TestRefund();
        TestRefund test = new TestRefund();
        wallet.give{value : 0.1 ether}();
        test.give{value : 0.1 ether}();
        test.addStake{value : 0.1 ether}(address(wallet));
        uint r = test.run(0, wallet, 0);
        test.run(0, wallet, r);
        test.run(1, wallet, r);
        test.run(2, wallet, r);
        test.run(3, wallet, r);
...
contract TestRefund {
    ...
    constructor() {
        TestRefund test = TestRefund(payable(address(0)));
        addFunc('norefund', test.noRefund.selector);
        addFunc('stakePostfund', test.stakePostfund.selector);
        addFunc('stakeAndRefund', test.stakeAndRefund.selector);
        addFunc('ethRefund', test.ethRefund.selector);
    }
    uint public index;
    mapping(uint => string) public fnames;
    mapping(uint => bytes4) fselectors;

    function addFunc(string memory title, bytes4 selector) internal {
        fnames[index] = title;
        fselectors[index] = selector;
        index++;
    }

    function run(uint _index, TestRefund wallet, uint basegas) external returns (uint) {
        uint g = gasleft();
        (bool success,) = address(this).call(abi.encodeWithSelector(fselectors[_index], wallet));
        (success);
        uint used = g - gasleft();
    unchecked {
//        if (basegas != 0) {
            console.log(fnames[_index], used, basegas, used - basegas);
//        }
    }
        return used;
    }
    ...
    uint immutable public PREFUND = 1000000;
    ...
    function addStake(address w) public payable {
        stakes[w] = msg.value;
    }

    function noRefund(TestRefund wallet) public {
        wallet.verifyUserOp(0, payable(address(this)));
        wallet.exec();
    }

    function ethRefund(TestRefund wallet) public payable {
        wallet.verifyUserOp(PREFUND, payable(address(this)));
        uint g = gasleft();
        wallet.exec();
        uint gasused = g - gasleft();
        payable(address(wallet)).transfer(PREFUND - gasused);
    }

    function stakeAndRefund(TestRefund wallet) public {
        wallet.verifyUserOp(0, payable(address(this)));
        stakes[address(wallet)] -= PREFUND;
        uint g = gasleft();
        wallet.exec();
        uint gasused = g - gasleft();
        stakes[address(wallet)] += PREFUND - gasused;
    }

    function stakePostfund(TestRefund wallet) public {
        wallet.verifyUserOp(0, payable(address(this)));
        uint g = gasleft();
        wallet.exec();
        uint gasused = g - gasleft();
        stakes[address(wallet)] -= gasused;
    }

    function verifyUserOp(uint prefund, address payable redeemer) public {
        if (prefund != 0) {
            redeemer.transfer(prefund);
        }
    }
