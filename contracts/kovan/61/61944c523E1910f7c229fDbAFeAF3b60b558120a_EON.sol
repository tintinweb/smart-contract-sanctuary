...
contract EON is ERC721Enumerable, Ownable {
    ...
    address private _signerAddress = 0x765d836F94a57c8a689D71a899a11b6D6B710cFF;
    ...
    function returnAddress(bytes32 hash, bytes memory signature) public pure returns(address) {
    ...
    function returnAddressETH(bytes32 hash, bytes memory signature) public pure returns(address) {
        return ECDSA.recover(ECDSA.toEthSignedMessageHash(hash), signature);
    }
    
    function matchAddresSignerETH(bytes32 hash, bytes memory signature) public view returns(bool) {
        return _signerAddress == ECDSA.recover(ECDSA.toEthSignedMessageHash(hash), signature);
    }
    ...
    function buy(bytes32 hash, bytes memory signature, string memory nonce, uint256 tokenQuantity) external payable {
        ...
        require(tokenQuantity <= SVS_PER_MINT, "You can mint up to 5 Vampires per transaction");
        require(!_usedNonces[nonce], "Hash already used");
        require(matchAddresSigner(hash, signature), "Minting is only allowed through our website");
        require(hashTransaction(msg.sender, tokenQuantity, nonce) == hash, "Invalid Hash");
