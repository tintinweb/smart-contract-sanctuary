...
// File contracts/SportsMining.sol
...
contract SportsMining is Ownable {
    using SafeMath for uint256;
    ...
    event SportsMiningStatusChange(bool newStatus);

    // Enable or disable this contract's functionality. It does not include the staked funds, which always
    // belong to the user.
    function toggleSportsMining() external onlyOwner {
        bool newStatus = !active;

        active = newStatus;

        emit SportsMiningStatusChange(newStatus);
    }
    ...
    function stake(uint256 _amount) external {
        require(active, "Sports mining is not active");
