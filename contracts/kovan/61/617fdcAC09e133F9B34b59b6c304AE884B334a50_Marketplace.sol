...
contract Marketplace {
    ...
    constructor(address tokenContractAddress) {
        ...
        token = DragonFactory(_tokenContractAddress);
    }
    ...
    external tokenOwnerForbidden(tokenId) payable {
        ...
        (bool success,) =activeSellOffers[tokenId].seller.call{value: saleValue}('');
        require(success);
    ...
    payable {
        ...
        if (refundBuyOfferAmount > 0) {
             (bool success,) = payable(previousBuyOfferOwner).call{value: refundBuyOfferAmount}('');
             require(success);
        }
...
contract DragonFactory is Ownable, ERC721Enumerable, ERC721URIStorage {

    
    uint public constant PRICE = 0.0001 ether;
    uint public constant MINT_LIMIT = 9;
    uint256 public constant MAX_SUPPLY = 10000;
    address private _royaltiesReceiver;
    uint256 public constant royaltiesPercentage = 5;
 
    mapping(uint256=> string) private statsIPFSByToken;
    mapping(uint256=> string) private imgIPFSByToken;
    mapping(string => uint8) private hashesOne;
    mapping(string => uint8) private hashesTwo;
    mapping(uint256 => uint256) private salePriceByToken;
    // Events
    event Mint(uint256 tokenId);

constructor(address initialRoyaltiesReceiver) ERC721("Dragon Rascal", "DNFT") {
        _royaltiesReceiver = initialRoyaltiesReceiver;
    }

    function _baseURI() internal pure override returns (string memory) {
        return "https://dragonrascal.com/stats/";
    }
    ...
    function reserveNFT(
        string memory _hashOne,string memory _hashTwo
    ) onlyOwner public returns (uint256 tokenId) {
       return _mintSingleNFT(_hashOne,_hashTwo);
    }

    function mintNFT( string memory _hashOne,string memory _hashTwo) public payable returns (uint256 tokenId) {
        require(
            msg.value >= PRICE,
    ...
    function _mintSingleNFT(string memory _hashOne,string memory _hashTwo)
    public
    returns (uint256 tokenId)
    {
        require(totalSupply() <= MAX_SUPPLY, "All tokens minted");
        require(bytes(_hashOne).length > 0);
        require(bytes(_hashTwo).length > 0); 
        require(hashesOne[_hashOne] != 1); 
        require(hashesOne[_hashTwo] != 1); 
        hashesOne[_hashOne] = 1;
        hashesOne[_hashTwo] = 1;
        uint256 newItemId = totalSupply() + 1;
        statsIPFSByToken[newItemId]=_hashOne;
        imgIPFSByToken[newItemId]=_hashTwo;
        _safeMint(msg.sender,newItemId);
        emit Mint(newItemId);
        return newItemId;
    }

    function GetMaxStatsIPFSByToken(uint256 _tokenId) onlyOwner public view returns (string memory _ipfs){
        return statsIPFSByToken[_tokenId];
    }
}
