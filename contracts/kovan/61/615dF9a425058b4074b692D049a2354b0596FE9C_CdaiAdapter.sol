...
contract CdaiAdapter is AdapterBase {
    ...
    function getTotalWrappedTokenAmountCore()
        ...
        returns (uint256)
    {
        return _cdai.balanceOf(address(this));
    }
        ...
        returns (uint256)
    {
        ...
        uint256 exchangeRateCurrent = _cdai.exchangeRateCurrent();
        uint256 scale = 18 + underlyingDecimals - _cdai.decimals();

        return exchangeRateCurrent.div(10**scale);
    }
        ...
        returns (uint256)
    {
        ...
        uint256 receivedWrappedTokenQuantity =
            amount.mul(10**18).div(_cdai.exchangeRateCurrent());
    ...
    {
        require(maxAmount != 0, "CdaiAdapter: can't redeem 0");

        uint256 beforeBalance = _cdai.balanceOf(address(this));
        uint256 receivedUnderlyingTokenQuantity =
            maxAmount.mul(_cdai.exchangeRateCurrent()).div(10**18);
        require(
            _cdai.redeem(maxAmount) == 0,
