...
contract ElasticPool is Ownable, ReentrancyGuard, ERC20 {
	...
	// Interface of Balancer Vault
	IVault public bVault;
	// Struct to hold Balancer Pool Info
	struct Pool {
		bytes32 id;
		IERC20 bptToken;
		address[] assets;
	...
	// Balancer Pool ID one used for Exchange BAL to In Token
    bytes32 public bPoolIdOne;
    // Balancer Pool ID two used for Exchange BAL to In Token
    bytes32 public bPoolIdTwo;
    // Address of intermediate token for BAL to In Token Exchange
		...
		ERC20('ElascticPoolToken', 'EPT')
	{
		...
		updatebPool();
		inToken.safeApprove(_bVault, type(uint256).max);
		balToken = IERC20(_balToken);
		balToken.safeApprove(_bVault, type(uint256).max);
	...
	* @param _bPoolIdOne - first Balancer Pool ID
    * @param _bPoolIdOne - second Balancer Pool ID
    * @param _bIntermediateToken - intermediate token address (same as inToken in case if exchange is done via singe pool)
	*/
	...
	****************************************/
	...
	function updatebPool()
		public
	{
		(address _bPool, ) = bVault.getPool(bPool.id);
		if(address(bPool.bptToken) == address(0))
			bPool.bptToken = IERC20(_bPool);
		// Trows error if inToken is not registered with pool
		bVault.getPoolTokenInfo(bPool.id, address(inToken));
		(address[] memory _tokens, ,) = bVault.getPoolTokens(bPool.id);
		bPool.assets = _tokens;
		for (uint256 i = 0; i < _tokens.length; i++)
			if(_tokens[i] == address(inToken))
				bPool.inTokenIndex = i;
	}
		...
		external
	{
		...
		_singleSwap.assetIn = address(balToken);
		_singleSwap.assetOut = address(bIntermediateToken);
		_singleSwap.amount = _balAmount;
		uint256 _amountOut = bVault.swap(_singleSwap, _funds, 1, block.timestamp+1);
		if(address(inToken) != address(bIntermediateToken)){
			_singleSwap.poolId = bPoolIdTwo;
			_singleSwap.assetIn = address(bIntermediateToken);
			_singleSwap.assetOut = address(inToken);
			_singleSwap.amount = _amountOut;
			_amountOut = bVault.swap(_singleSwap, _funds, 0, block.timestamp+1);
		}
		inToken.safeTransfer(workerAddress, _amountOut);
	}
		...
		returns (uint256 bpAmount_)
	{
		...
		uint256 _bptBalance = bPool.bptToken.balanceOf(address(this));
		IVault.JoinPoolRequest memory _request;
		_request.assets = bPool.assets;
		_request.maxAmountsIn = new uint256[](bPool.assets.length);
		_request.maxAmountsIn[bPool.inTokenIndex] = inAmount_;
		_request.userData = abi.encode(IVault.JoinKind.EXACT_TOKENS_IN_FOR_BPT_OUT, _request.maxAmountsIn, 0);
		bVault.joinPool(bPool.id, address(this), address(this), _request);
		bpAmount_ = bPool.bptToken.balanceOf(address(this)) - _bptBalance;
		inBalance += inAmount_;
		...
		returns (uint256 amount_, uint256 compensatedAmount_)
	{
		...
		User storage _user = userData[_address];
		require(_user.bpBalance >= bpAmount_, "not enough balance");
		require(block.timestamp -_user.lastTime >= holdTime, 'cannot withdraw, tokens on hold');
		uint256 _inAmount = _user.inBalance * bpAmount_ / _user.bpBalance;
		inBalance -= _inAmount;
		_user.inBalance -= _inAmount;
		_user.bpBalance -= bpAmount_;
		uint256 _inBalance = inToken.balanceOf(address(this));
		IVault.ExitPoolRequest memory _request;
		_request.assets = bPool.assets;
		_request.minAmountsOut = new uint256[](bPool.assets.length);
		// _request.minAmountsOut[bPool.inTokenIndex] = bpAmount_;
		_request.userData = abi.encode(IVault.ExitKind.EXACT_BPT_IN_FOR_ONE_TOKEN_OUT, bpAmount_, bPool.inTokenIndex);
		bVault.exitPool(bPool.id, address(this), payable(address(this)), _request);
		amount_ = inToken.balanceOf(address(this)) - _inBalance;
		...
		inToken.safeTransfer(_address, amount_);
		if(compensatedAmount_ != 0)
			_mint(_address, compensatedAmount_);
		if(_fee != 0){
	...
	****************************************/
	...
	function getBalanceBp()
		...
		returns (uint256)
	{
		return bPool.bptToken.balanceOf(address(this));
	}
		...
		returns (bool)
	{
		...
		return block.timestamp - userData[_address].lastTime < holdTime;
	}
		...
		)
	{
		...
		if(_inAmount > _amount){
			_change = _inAmount- _amount;
			_maxChange = _inAmount * lossCoefficient / COEFFICIENT_PRECISION;
			...
			_change = _amount - _inAmount;
			_maxChange = _change * profitCoefficient * COEFFICIENT_PRECISION;
			returnAmount = _inAmount + _maxChange;
			fee = (_change - _maxChange) * feeCoefficient / COEFFICIENT_PRECISION;
			feeWorker = _change - _maxChange - fee;
		}
