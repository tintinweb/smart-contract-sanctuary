...
contract HedgeyCallsV2 is ReentrancyGuard {
    ...
    ****/
    ...
    function changeNewOption(uint _c, uint _assetAmt, uint _strike, uint _price, uint _expiry) payable public nonReentrant {
        ...
        if (msg.sender == call.short) {
            ...
            call.price = _price;
            call.expiry = _expiry;
            call.totalPurch = _assetAmt.mul(_strike).div(10 ** assetDecimals);
            call.tradeable = true;
    ...
    */
    ...
    event NewOptionSold(uint _c);
    event NewOptionBought(uint _c);
    event OpenOptionSold(uint _c, uint _d, address _long, uint _price);
    event OpenShortRePurchased(uint _c, uint _d, address _short, uint _price);
    event OpenOptionPurchased(uint _c);
    event OptionChanged(uint _c, uint _assetAmt, uint _strike, uint _price, uint _expiry);
    event PriceSet(uint _c);
...
contract HedgeyCallsFactoryV2 {
    ...
    mapping(address => mapping(address => address)) public callContracts;
    ...
    function getPair(address asset, address pymtCurrency) public view returns (address pair) {
        pair = callContracts[asset][pymtCurrency];
    }

    function createContract(address asset, address pymtCurrency) public {
        require(asset != pymtCurrency, "same currencies");
        require(callContracts[asset][pymtCurrency] == address(0), "contract exists");
        HedgeyCallsV2 callContract = new HedgeyCallsV2(asset, pymtCurrency, collector, fee, collectorSet);
        callContracts[asset][pymtCurrency] = address(callContract);
        totalContracts.push(address(callContract));
        if (collectorSet) {
            IHedgeyStaking(collector).addWhitelist(address(callContract));
        }
        emit newCallPairCreated(asset, pymtCurrency, address(callContract));
    }

    event newCallPairCreated(address _asset, address _pymtCurrency, address _callContract);
}