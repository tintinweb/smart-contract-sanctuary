...
interface ITurboHatchery is HasTurboStruct {
    ...
    function createTurbo(uint256 _index, uint256 _creatorFee, string[] memory _outcomeSymbols, bytes32[] memory _outcomeNames, uint256 _numTicks, IArbiter _arbiter, bytes memory _arbiterConfiguration) external returns (uint256);
    function getShareTokens(uint256 _id) external view returns (ITurboShareToken[] memory);
    function mintCompleteSets(uint256 _id, uint256 _amount, address _receiver) external returns (bool);
    function burnCompleteSets(uint256 _id, uint256 _amount, address _receiver) external returns (bool);
