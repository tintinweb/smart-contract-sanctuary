...
contract LPStakingRewards is ILPStakingRewards, BaseRewards {
    ...
    function setRewardsRate(uint256 newRewardsRate) external override onlyRole(DEFAULT_ADMIN_ROLE) {
        updateRewardForAll();
        rewardsRate = newRewardsRate;
        emit LPRewardsRateChanged(block.number, rewardsRate);
    }

    function getRewardsEndBlock(address lpToken) external view override isSupportedLP(lpToken) returns (uint256) {
        return poolInfo[lpToken].rewardsEndBlock;
    }

    function stake(address lpToken, uint256 amount) external override nonReentrant isSupportedLP(lpToken) {
    ...
    function updateRewardInternal(address lpToken) internal isSupportedLP(lpToken) {
        PoolInfo storage pool = poolInfo[lpToken];
        pool.rewardPerToken = rewardsPerToken(lpToken);
        pool.lastUpdateBlock = block.number;
    }

    function updateReward(address user, address lpToken) internal isSupportedLP(lpToken) {
     ...
     */
    function lastBlockApplicable(address lpToken) internal view isSupportedLP(lpToken) returns (uint256) {
    ...
    function _earned(address user, address lpToken) internal view isSupportedLP(lpToken) returns (uint256) {
    ...
    function _withdrawUser(
        ...
        address lpToken,
        uint256 amount
    ) internal nonReentrant isSupportedLP(lpToken) {
        // check and update
    ...
    function claimRewardInternal(address user, address lpToken) internal nonReentrant isSupportedLP(lpToken) {
        updateReward(user, lpToken);
        if (userInfo[lpToken][user].reward > 0) {
            userInfo[lpToken][user].reward = flurryStakingRewards.claimLPTokenReward(
                user,
    ...
    function endRewards(address lpToken) external override onlyRole(DEFAULT_ADMIN_ROLE) isSupportedLP(lpToken) {
        PoolInfo storage pool = poolInfo[lpToken];
...
interface ILPStakingRewards {
    ...
    /**
     * @notice A method to get reward end block for a given LP pool
     * @param lpToken Address of LP Token contract
     */
    function getRewardsEndBlock(address lpToken) external view returns (uint256);
...
abstract contract BaseRewards is AccessControlEnumerableUpgradeable, ReentrancyGuardUpgradeable {
    using SafeERC20Upgradeable for IERC20Upgradeable;
