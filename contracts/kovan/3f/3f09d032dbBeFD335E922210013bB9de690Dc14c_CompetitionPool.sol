...
contract CompetitionPool is Ownable {
    ...
    function setRewardRates(
        ...
        for (uint256 i = 0; i < _bracketsAmountToken.length - 1; i++) {
            require(_bracketsAmountToken[i] < _bracketsAmountToken[i + 1], "CompetitionPool: _bracketsAmountToken must be sorted ascending");
            require(
                _rewardRateOfcreator[i] + _rewardRateOfOwner[i] <=
                    10**(decimalOfRate + 2),"CompetitionPool: rewardRateOfcreator + rewardRateOfOwner must be <=100%"
            );
        }
        ...
        for (uint256 i = 0; i < bracketsAmountToken.length; i++) {
            ...
            setRewardRate(
                ...
                _rewardRateOfcreator[i],
                _rewardRateOfOwner[i],
                10**(decimalOfRate+2) - _rewardRateOfcreator[i] - _rewardRateOfOwner[i]
            );
        }
        emit RewardRate(
