...
contract Vesting is Ownable, ReentrancyGuard {
    ...
    constructor(address newOwner, address plugTokenAddress, uint startTime, uint totalBalance) {
    ...
    function claimInternal (address to) internal {
        ...
        if (currentEpoch > NUMBER_OF_EPOCHS + 1) {
            ...
            _plug.transfer(to, _plug.balanceOf(address (this)));
            return;
        }

        if (currentEpoch > lastClaimedEpoch) {
            _balance = (currentEpoch - 1  - lastClaimedEpoch) * totalDistributedBalance / NUMBER_OF_EPOCHS;
        }
        lastClaimedEpoch = currentEpoch - 1;
        if (_balance > 0) {
            _plug.transfer(to, _balance);
        }
    }

    function balance () public view returns (uint){
        return _plug.balanceOf(address (this));
    }
