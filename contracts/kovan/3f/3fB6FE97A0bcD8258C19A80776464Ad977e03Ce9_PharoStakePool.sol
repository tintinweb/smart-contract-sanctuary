...
contract PharoStakePool is Context, AccessControl, Ownable, ReentrancyGuard, ERC721 {
    ...
    uint256 public PHRO_PER_BLOCK = 90 ether;
    ...
    constructor (
        ...
        public
        ReentrancyGuard()
        ERC721("Pharo Staking Pool", "Pharo Staking Pool")
    {
        WETH = _WETH;
        ...
        // grant to the deployer address for now...
        // will have to figure out who/what will be calling any withdraws fromt the contract
    ...
    // mint nft
    function mintStakingNft (address miner, string memory ipfsHash)
        public
        returns (uint256)
    {
        _tokenIds.increment();
    ...
    */
    ...
    // Fix this!!!!
    function addStakePool(uint256 _allocationPoint, IERC20 _stakedToken, bool _withUpdate) 
        public
        //onlyOwner
        nonDuplicated(_stakedToken)
    {
        if(_withUpdate) {
            massUpdatePools();
        }
        ...
        poolExistence[_stakedToken] = true;
        poolInfo.push(
            PoolInfo({
        ...
        emit LogPoolAddition(poolInfo.length.sub(1), _allocationPoint, stakedToken);
    }
        ...
        public
    {
        ...
        // Create/get staker and pool 
        PoolInfo storage pool = poolInfo[_poolId];
        StakerInfo storage staker = stakerInfo[_poolId][msg.sender];
        staker.amount = staker.amount.add(_amount);
        staker.rewardDebt = staker.amount.mul(pool.accumulatedPharoPerShare).div(1e18);

        // update the pool balances
        updatePool(_poolId);
        // if(staker.amount > 0) {
        //     uint256 pending = staker.amount.mul(pool.accumulatedPharoPerShare).div(1e18).sub(
        //         staker.rewardDebt
        //     );
        //     if(pending > 0) {
        ...
        public 
    {
        ...
        uint256 poolTokenSupply = pool.stakedToken.balanceOf(address(this));
        ...
        phroToken.mint(burnAddress, pharoReward.div(10));
        phroToken.mint(address(this), pharoReward);

        pool.accumulatedPharoPerShare = pool.accumulatedPharoPerShare.add(pharoReward.mul(1e18).div(poolTokenSupply));
        pool.lastRewardBlock = block.number;
    }
        ...
        returns(uint256) 
    {
        ...
        if(stakedSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 pharoReward = multiplier.mul(PHRO_PER_BLOCK).mul(pool.allocationPoint).div(totalAllocationPoint);
            accumulatedPharoPerShare = accumulatedPharoPerShare.add(pharoReward.mul(1e18).div(stakedSupply));
        }

        return staker.amount.mul(accumulatedPharoPerShare).div(1e18).sub(staker.rewardDebt);
    }
        ...
        nonReentrant
    {
        ...
        uint256 pending = staker.amount.mul(pool.accumulatedPharoPerShare).div(1e18).sub(staker.rewardDebt);
        if(pending > 0) {
            safePharoTransfer(msg.sender, pending);
        }
        ...
        staker.rewardDebt = staker.amount.mul(pool.accumulatedPharoPerShare).div(1e18);
        pool.stakedToken.safeTransferFrom(address(this), address(msg.sender), _amount);
...
contract PharoToken is ERC20PresetMinterPauser, TokenRecover {
    ...
    uint256 private _cap = 1000000000 * 10 ** 18; // One Billion Tokens...

    // indicates if minting is finished
    bool private _mintingFinished = false;

    // indicates if minting is finished
    bool private _burningFinished = false;
     ...
     */
    modifier canMint() {
        require(!_mintingFinished, "PharoToken: minting is finished");
        _;
    }
     ...
     */
    modifier canBurn() {
        require(!_burningFinished, "PharoToken: burning is finished");
        _;
    }
    ...
    modifier canTransfer(address from) {
        require(
            _transferEnabled || hasRole(DEFAULT_ADMIN_ROLE, from),
            "PharoToken: transfer is not enabled or from does not have the DEFAULT_ADMIN_ROLE role"
        );
        _;
    }
    ...
    /**
     * @dev Emitted when there is a new request to mint tokens
     */
    event NewMintRequest(uint requestId);
    
    /**
     * @dev Emitted when a mint request is approved by the owner 
     */
    event MintRequestApproved(uint requestId);
    
    /**
     * @dev Emitted when a mint request is rejected by the owner 
     */
    event MintRequestRejected(uint requestId);

    /**
     * @dev Emitted when there is a new request to burn tokens
     */
    event NewBurnRequest(uint256 requestId);

    /**
     * @dev Emitted when a burn request is approved by the owner 
     */
    event BurnRequestApproved(uint256 requestId);

    /**
     * @dev Emitted when a burn request is rejected by the owner 
     */
    event BurnRequestRejected(uint256 requestId);
    
    /**
     * @dev Emits Status of minting request
     */
    enum Status { Pending, Approved, Rejected, Executed }

    /**
     * @dev Defines the structure of a new minting request
     * 
     */
    struct MintRequest {
        address minter;
     ...
     */
    modifier approvedMinter(uint reqId) {
        require(
            MintRequests[reqId].status == Status.Approved,
            "PharoToken:: Request to mint not approved by owner"
        );
        _;
    }

    /**
     * @dev Tokens can be burned only when the corresponding request has been approved by the owner
     */
    modifier approvedBurner(uint reqId) {
        require(
            BurnRequests[reqId].status == Status.Approved,
            "PharoToken:: Request to burn not approved by owner"
        );
        _;
    }
    ...
    modifier checkMinter(uint reqId, address minter) {
        require(
            ...
            "PharoToken:: Mint request not owned by minter address"
        );
        _;
    }
     ...
     * @dev Tokens can be minted only when the corresponding request has been approved by the owner
     */
    modifier checkBurner(uint reqId, address burner) {
        require(
            BurnRequests[reqId].burner == burner,
            "PharoToken:: Burn request not owned by burner address"
        );
        _;
    }
    ...
    function setCap (uint256 cap) 
        ...
        onlyOwner 
    {
        require(_cap > 0, "PHRO:: cap must be > 0");
        require(_cap <= 1000000000 * 10 ** 18, "PHROToken:: cap must be less than or equal to 1 Billion");
        _cap = cap;
    }
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal virtual override {
        ...
        if (from == address(0)) { // When minting tokens
            require(totalSupply().add(amount) <= _cap, "PharoToken:: cap exceeded");
        }
    }
    ...
    /**
     * @dev Function for owner to approve a mint request
     */
    function approveMintRequest(uint reqId) public onlyOwner {
        MintRequests[reqId].status = Status.Approved;
        
        emit MintRequestApproved(reqId);
    }

    /**
     * @dev Function for owner to approve a burn request
     */
    function approveBurnRequest(uint reqId) public onlyOwner {
        BurnRequests[reqId].status = Status.Approved;
        
        emit BurnRequestApproved(reqId);
    }
    
    /**
     * @dev Function for owner to reject a mint request
     */
    function rejectMintRequest(uint reqId) public onlyOwner {
        MintRequests[reqId].status = Status.Rejected;
        
        emit MintRequestRejected(reqId);
    }

    /**
     * @dev Function for owner to reject a burn request
     */
    function rejectBurnRequest(uint reqId) public onlyOwner {
        BurnRequests[reqId].status = Status.Rejected;
        
        emit BurnRequestRejected(reqId);
    }
    
    /**
     * @dev Function to create a new mint request for the owner to approve/reject
     */     
    function createMintRequest(address to, uint256 amount) public onlyOwner {
        MintRequests.push(MintRequest(_msgSender(), to, amount, Status.Pending));
         
        emit NewMintRequest(MintRequests.length - 1);
    }

    /**
     * @dev Function to create a new burn request for the owner to approve/reject
     */     
    function createBurnRequest(address to, uint256 amount) public onlyOwner {
        BurnRequests.push(BurnRequest(_msgSender(), to, amount, Status.Pending));
         
        emit NewBurnRequest(MintRequests.length - 1);
    }
     
    /**
     * @dev Executes an approved/rejected mint request
     */
    function executeMintRequest(uint reqId) public onlyOwner checkMinter(reqId, _msgSender()) approvedMinter(reqId) canMint {
        _mint(MintRequests[reqId].to, MintRequests[reqId].amount);
        MintRequests[reqId].status = Status.Executed;
    }

    /**
     * @dev Executes an approved/rejected burn request
     */
    function executeBurnRequest(uint reqId) public onlyOwner checkBurner(reqId, _msgSender()) approvedBurner(reqId) canBurn {
        _burn(BurnRequests[reqId].burnAddress, BurnRequests[reqId].amount);
        BurnRequests[reqId].status = Status.Executed;
    }
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "PharoToken::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "PharoToken::delegateBySig: invalid nonce");
        require(now <= expiry, "PharoToken::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "PharoToken::getPriorVotes: not yet determined");
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "PharoToken::_writeCheckpoint: block number exceeds 32 bits");
