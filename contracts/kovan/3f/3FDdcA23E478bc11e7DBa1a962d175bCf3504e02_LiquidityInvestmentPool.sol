...
*/
    ...
    */
    ...
    function _removeLiquidity(uint256 totalDeposit,address sender) private returns (uint) {
    ...
    */
    function _withdrawLPartner(address payable sender) external onlyPoolRegistry returns (bool, uint256, address) {
        ...
        uint256 totalAmountReturnFinal = _removeLiquidity(totalAmountReturn,sender);

        if (totalAmountReturnFinal > 0) {
            if (token == address(0)) {
