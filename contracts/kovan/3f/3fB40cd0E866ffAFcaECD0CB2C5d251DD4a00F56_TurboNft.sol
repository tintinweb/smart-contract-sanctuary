...
contract TurboNft is Ownable, ERC721Enumerable, VRFConsumerBase {
    ...
    function mintMultiple(address _to, uint256 _count) external onlyMinter {
    ...
    function setBaseURI(string memory _baseURI, bool _finalBaseUri) external onlyOwner {
        require(!finalBaseURI, "BASE_URI_ALREADY_FINAL");
        _setBaseURI(_baseURI);
        finalBaseURI = _finalBaseUri;
  ...
  */
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        string memory _tokenURI = metadataOf(tokenId);
        string memory _base = _baseURI();

        // If there is no base URI, return the token URI.
        if (bytes(_base).length == 0 || seed == 0) {
            return defaultURI;
        }
    ...
    function metadataOf(uint256 _tokenId) public view returns (string memory) {
        ...
        for (uint256 i = 0; i < maxTokens - 1; i++) {
            uint256 j = i + (uint256(keccak256(abi.encode(_seed, i))) % (maxTokens - i));
