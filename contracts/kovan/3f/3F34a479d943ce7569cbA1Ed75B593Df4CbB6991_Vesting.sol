...
contract Vesting is PausableUpgradeable, BumperAccessControl {
    ...
    function claimToStaking(
        ...
        whenNotPaused
    {
        validateProfits(account, vestingInfo,merkleProof);
        restoreClaimedV1(account, vestingInfo);
        ...
        IERC20(tokenBUMP).safeApprove(staking, amount);
        StakeRewards(staking).stakeFor(amount, stakingOption, account);
    ...
    function restoreClaimedV1(address account, VestingInfo memory vestingInfo) internal { 
    ...
    function getClaimableAmountFor(address account, VestingInfo memory vestingInfo)
        public
        view
        returns (uint256)
    {
        ...
        return vestingInfo.totalAmount + vestingInfo.previousAmount -
            (totalClaimed + totalLockedOf(vestingInfo));
    }
        ...
        returns (uint256)
    {
        (
            ...
            uint256 onStartVestingAmountLock
        ) = getDurationLockAndOnStartVesting(vestingInfo);
        return
            vestingInfo.vestingPerSec *
            durationLock +
            onStartVestingAmountLock;
    }

    function getDurationLockAndOnStartVesting(VestingInfo memory vestingInfo)
        internal
        view
        returns (uint256, uint256)
    {
        ...
        if (block.timestamp >= vestingInfo.end) {
            ...
            return (vestingInfo.end - block.timestamp, 0);
        } else {
            // Everything will be blocked
            return (
                vestingInfo.end -
                    (vestingInfo.start + vestingInfo.cliff),
                vestingInfo.onStartAmount
            );
        }
    }
    ...
    /// @dev Only owner of the vesting escrow can invoke this function.
    function setMerkleRoot(bytes32 root) external onlyGovernanceOrOwner {
        merkleRoot = root;
    }

    /// @notice Pause vesting contract
    /// @dev Only owner of the vesting escrow can invoke this function.
    function pause() external onlyGovernanceOrOwner whenNotPaused {
        _pause();
    }

    /// @notice Unpause vesting contract
    /// @dev Only owner of the vesting escrow can invoke this function.
    function unpause() external onlyGovernanceOrOwner whenPaused {
        _unpause();
    }
}
