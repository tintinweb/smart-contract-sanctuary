...
contract Vote {
	...
	*/
	...
	function createCategorie(string memory _name) public returns (int) {
		// check if categorie name already exist
		if (getIndexCategorieByName(_name) != (-1))
			return (-1);
		structCategories storage newCategorie = categories.push();
		newCategorie.owner = msg.sender;
		newCategorie.isexist = true;
		newCategorie.deleted = false;
		newCategorie.name = _name;
		return int(categories.length - 1);
	}
	...
	//modifier noExistQuestion(uint _categorie, uint _questionnaire, uint _question)
    function addVote(reacVote memory _reacVote) public returns (bool) {
		...
		return addVote(memoryQuestion, _reacVote.sender, int(_reacVote.choice));
	}

	//modifier noExistQuestion(uint _categorie, uint _questionnaire, uint _question) {
    function isVoted(reacVote memory _reacVote) public view returns (bool) {
