...
interface IOracle {
    ...
    function token0PriceMA7() external view returns (uint);
}

// File: contracts/Spark.sol

/* 1.1.1
    1. add calMaxReward function [done]
    2. add timestamp to all events [done]
    3. extension APR calculation correction [done, to be tested]
    4. add max APR to calMaxReward [done, to be tested]
*/
...
contract Spark is Ownable {
    ...
    function get_userDepo(address user, uint index) public view returns (uint, uint, uint){
      return (_userDeposits[user][index].stakedLP, _userDeposits[user][index].timestampSec, _userDeposits[user][index].amountClaimed);
    }
     ...
     */
    function stake(uint amount) external returns (uint64 id) {
        // sanity check
        ...
        uint endTimeStamp = startTimeStamp + 29 days + extDays * 86400;
    ...
    function claim(uint index) public returns (uint) {
        ...
        uint claimableAmt = computeUnitRewardFor(msg.sender, index).sub(amountClaimed);
        if (claimableAmt <= 0) {
        ...
        // 1. update user stats
        _userDeposits[msg.sender][index].amountClaimed = _userDeposits[msg.sender][index].amountClaimed.add(claimableAmt);
        // 2. update global stats
    ...
    function claimBatch(uint[] memory indices) external returns (uint) {
        ...
        for (uint i = 0; i < indices.length; i++) {
            ...
            claimableAmt = computeUnitRewardFor(msg.sender, indices[i]).sub(amountClaimed);
            ...
            // update deposit data
            _userDeposits[msg.sender][indices[i]].amountClaimed = _userDeposits[msg.sender][indices[i]].amountClaimed.add(claimableAmt);
            // update aggregate record
            totalClaimedAmt = totalClaimedAmt + claimableAmt;

            emit TokensClaimed(msg.sender, indices[i], claimableAmt, now);
        }

        // update global data once for all
        totalRewardGiven = totalRewardGiven.add(totalClaimedAmt);
    ...
    function computeUnitRewardFor(address staker, uint index) public view returns (uint) {
        uint stakedTimeStamp = _userDeposits[staker][index].timestampSec;
        uint endTimeNoExt = startTimeStamp + 29 days;
        uint endTimeExt = endTimeNoExt + extDays * 86400;
        require(_userDeposits[staker][index].stakedLP > 0, "computeUnitRewardFor: the deposit does not exist.");
        require(stakedTimeStamp > startTimeStamp, "computeUnitRewardFor: the deposit made before the program started");
        require(stakedTimeStamp <= endTimeExt, "computeUnitRewardFor: the deposit made after the program has ended");

        uint tNotInExt = 0;
        uint tInExt = 0;
        if (stakedTimeStamp < endTimeNoExt) {
          tNotInExt = Math.min(now, endTimeNoExt).sub(stakedTimeStamp).div(1 days);
        }
        if (now > endTimeNoExt) {
          tInExt = Math.min(now, endTimeExt).sub(Math.max(stakedTimeStamp, endTimeNoExt)).div(1 days);
        }
        uint interestRate = computeInterestWithPrecision(tNotInExt, tInExt);
        ...
        return principal.mul(interestRate).div(APR_PREC * 100);
    }

    /**
     * @notice compute the current interest rate of a deposit
     ...
     */
    function computeInterestWithPrecision(uint tNotInExt, uint tInExt) public view returns (uint) {
        uint interestRateExt = extAPR.mul(tInExt).mul(APR_PREC).div(365);

        if (tNotInExt == 0) {
            return interestRateExt;
        } else if (tNotInExt <= 7) {
            return 200.mul(tNotInExt).mul(APR_PREC).div(365).add(interestRateExt);
        } else if (tNotInExt <= 14) {
            return (300 * tNotInExt - 700).mul(APR_PREC).div(365).add(interestRateExt);
            //return (300 * (tNotInExt - 7)+ 200 * 7).mul(APR_PREC).div(365);
        } else if (tNotInExt <= 21) {
            return (400 * tNotInExt - 2100).mul(APR_PREC).div(365).add(interestRateExt);
            //return (400 * (tNotInExt - 14)+ (200 + 300) * 7).mul(APR_PREC).div(365);
        } else {
            return (500 * tNotInExt - 4200).mul(APR_PREC).div(365).add(interestRateExt);
            //return (500 * (tNotInExt - 21)+ (200 + 300 + 400) * 7).mul(APR_PREC).div(365);
        }
    }
     ...
     * @return maxReward estimate of the maxiaml amount of SPA token reward this deposit might collect
               APRMAX the maxiaml possible value of APR (with precision)
     */
    function calMaxReward(uint amountInLP, uint stakedTimeStamp) external view returns (uint maxReward, uint APRMAX) {
        ...
        uint tNotInExtMAX = 0;
        if (stakedTimeStamp < endTimeNoExt) {
          tNotInExtMAX = endTimeNoExt.sub(stakedTimeStamp).div(1 days);
        }
        uint tInExtMAX = endTimeExt.sub(Math.max(stakedTimeStamp, endTimeNoExt)).div(1 days);
        uint interestRateMAX = computeInterestWithPrecision(tNotInExtMAX, tInExtMAX);

        // calculate the value of "amountInLP" LP tokens
        (uint112 reserve0, uint112 reserve1, ) = _pair.getReserves();
        uint principal = computeLPValue(amountInLP, _pair.totalSupply(), ma7Token0Price, uint(reserve0).mul(uint(reserve1)));

        // calcualte and return the maxiaml reward
        maxReward = principal.mul(interestRateMAX).div(APR_PREC * 100);
        APRMAX = interestRateMAX.mul(365).div(tNotInExtMAX + tInExtMAX);
    }
