    ...
    StakingRewards
{
    ...
    function initialize(
        ...
        rewardEscrow = RewardEscrow(stakingParams.rewardEscrow); // Address of vesting contract
     ...
     * @param tokenIndex index of the reward token in rewardTokens
     */
    function initializeReward(uint256 rewardAmount, uint256 tokenIndex)
        ...
        onlyTerminal
    {
        super.initializeReward(rewardAmount, tokenIndex);
    }
...
contract StakingRewards is IStakingRewards, ReentrancyGuard {
     ...
     */
    function getRewardTokensCount() public view override returns (uint256) {
    ...
    function claimReward(address sender) public override updateReward(sender) {
        for (uint256 i = 0; i < rewardTokens.length; ++i) {
            claimRewardForSingleToken(sender, rewardTokens[i]);
        }
    }
     ...
     */
    function claimRewardForSingleToken(address sender, address token) private {
        uint256 rewardAmount = earned(sender, token);
        if (
            rewardAmount > 0 &&
            rewardInfo[token].remainingRewardAmount >= rewardAmount
        ) {
            rewardInfo[token].rewards[sender] = 0;
            ...
            if (rewardsAreEscrowed) {
                ...
                rewardEscrow.appendVestingEntry(token, sender, rewardAmount);
                // Else transfer tokens directly to sender
            } else {
                IERC20(token).safeTransfer(sender, rewardAmount);
            }
        ...
        updateReward(address(0))
    {
        ...
        require(
            rewardInfo[token].rewardRate <= balance.div(rewardsDuration),
            "Provided reward too high"
        );

        rewardTokenInfo.totalRewardAmount = balance;
        rewardTokenInfo.remainingRewardAmount = balance;
    ...
    modifier updateReward(address account) {
        for (uint256 i = 0; i < rewardTokens.length; ++i) {
            ...
            if (account != address(0)) {
                ...
                rewardTokenInfo.userRewardPerTokenPaid[account] = rewardInfo[
                    token
