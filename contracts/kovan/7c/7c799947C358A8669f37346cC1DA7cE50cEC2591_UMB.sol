...
contract UMB is MintableToken, Airdrop, ISwapReceiver {
    ...
    mapping(address => bool) public rewardsTokens;
    ...
    external
    onlyOwner {
        require(_tokens.length > 0, "empty _tokens");
        require(_tokens.length == _statuses.length, "_tokens does not match _statuses");
