...
contract AdamEthSettlement is Ownable {

    using SafeERC20 for IERC20;
    event Settled(address indexed buyer, uint256 settlePrice, address indexed seller, uint256 sellerPrice, address adamWallet, uint256 indexed pid, uint256 deadline);    
    ...
    function settle(address payable _seller, uint256 _sellerPrice, uint256 deadline, uint256 pid, uint8 v, bytes32 r, bytes32 s) payable external {
        ...
        // check the msg.value
        require(msg.value >= _sellerPrice, "the price is invalid");

        // flag the settled payment 
        exclusives[pid] = true; 

        if(_sellerPrice != 0){
            _seller.transfer(_sellerPrice);
        }
        uint256 feeBalance = msg.value - _sellerPrice;
        if(feeBalance != 0){
            adamWallet.transfer(feeBalance);
        }

        emit Settled(msg.sender, msg.value, _seller, _sellerPrice, adamWallet, pid, deadline);
    }

    // withdraw ERC20 locked up in the contract
    function withdrawERC20(address _assetAddress, address _recipient, uint256 _value) onlyOwner public {
    ...
    function changeOperator(address _newOperator) onlyOwner external {
        address old = operator; 
        operator = _newOperator;
        emit OperatorChanged(old, _newOperator, msg.sender);
    }

    function changeAdamWallet(address payable _newWallet) onlyOwner external {
