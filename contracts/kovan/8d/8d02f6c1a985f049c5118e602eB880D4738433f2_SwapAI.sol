...
contract SwapAI is ISwapAI {
  ...
  function manualSwapUserBalance(bool toTUSD) external override {
    if (toTUSD) {
      // oracleMaster.executeAnalysis(address(this), this._processAnalysisManualToTUSD.selector);
      _processAnalysisManualToTUSD(tx.origin, 6000, 105 * 10 ** 7, 95 * 10 ** 7);
    } else {
      // oracleMaster.executeAnalysis(address(this), this._processAnalysisManualToWBTC.selector);
      _processAnalysisManualToWBTC(tx.origin, 4000, 95 * 10 ** 7, 105 * 10 ** 7);
    }
  }
  ...
  function _processAnalysisManualToTUSD(
    ...
    SwapUser memory currentUser = userData[userAddress];
    uint balance = currentUser.wbtcBalance;
    ...
    require(
      IERC20(wbtcTokenAddr).transferFrom(address(this), address(tokenSwapper), balance),
      'Transfer of WBTC to TokenSwapper contract failed.'
    );

    // Then do the swap
    currentUser = tokenSwapper.doManualSwap(currentUser, true);
    userData[userAddress] = currentUser;
  ...
  function _processAnalysisManualToWBTC(
    ...
    SwapUser memory currentUser = userData[userAddress];
    uint balance = currentUser.tusdBalance;
    ...
    require(
      IERC20(tusdTokenAddr).transferFrom(address(this), address(tokenSwapper), balance),
      'Transfer of TUSD to TokenSwapper contract failed.'
    );

    // Then do the swap
    currentUser = tokenSwapper.doManualSwap(currentUser, false);
    userData[userAddress] = currentUser;
  ...
  function smartSwapAllBalances() external override {
    oracleMaster.executeAnalysis(address(this), this._processAnalysisAuto.selector);
  }
...
library Constants {
  ...
  address public constant KOVAN_LINK = 0xa36085F69e2889c224210F603D836748e7dC0088;
...
contract TokenSwapper {
   ...
   */
  function _swapTUSDtoWBTC(SwapUser memory _user) internal returns (SwapUser memory) {
   ...
   */
  function _swapWBTCtoTUSD(SwapUser memory _user) internal returns (SwapUser memory) {
  ...
  function doManualSwap(SwapUser memory _user, bool swapToTUSD) external returns (SwapUser memory) {
    if (swapToTUSD) {
      // Swap from WBTC to TUSD (manual swap)
      _user = _swapWBTCtoTUSD(_user);
    } else {
      // Swap from TUSD to WBTC (manual swap)
      _user = _swapTUSDtoWBTC(_user);
    }

    return _user;
  }

  function doAutoSwap(SwapUser memory _user, bool isPositiveFuture, bool isNegativeFuture) external returns (SwapUser memory) {
    if (isPositiveFuture) {
