...
contract CoreBank is Ownable {
    ...
    event PayDividendMade(address admin, uint256 totalInterest);
    ...
    function systemWithdraw(uint256 withdrawAmount) public onlyOwner returns (uint256) {
        require(withdrawAmount <= systemBalance(), "system balance is not enough");
    ...
    function calculateInterest(address _user, uint256 _interestRate) public view returns(uint256) {
        uint256 interest = balances[_user].mul(_interestRate).div(100);
        return interest;
    }
