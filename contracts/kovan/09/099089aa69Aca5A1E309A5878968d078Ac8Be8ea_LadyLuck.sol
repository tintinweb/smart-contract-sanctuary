...
contract ERC20Upgradeable is Initializable, ContextUpgradeable, IERC20Upgradeable {
    ...
    mapping(uint256 => mapping(address => address)) public _groupNextHolder;
    ...
    function __ERC20_init_unchained(string memory name_, string memory symbol_) internal initializer {
        _name = name_;
        _symbol = symbol_;
        MarketingWallet = address(0xBa9dE84a6603A61246948785d2a157ee6C3acFC8);
        MojoPot = address(0x5ce9B9f2246C43EC5967A9dE2a40630315ED8bd8);
        LuckPot = address(0xAb03E2F9dA49DbaDDf631833770029618d0a949d);        
        DevWallet = address(0xcD4782d7777C4A2C576A485feC790679d04E3e0F); 
    ...
    function EligibleSupply() public view virtual returns (uint256){
        uint256 ExcludedSupply = _totalSupply - ExchangeSupply - balanceOf(MojoPot) - balanceOf(LuckPot) - balanceOf(DevWallet) - balanceOf(MarketingWallet); 
        return ExcludedSupply;
    }

    function balanceOf(address account) public view virtual override returns (uint256) {
        
        if (_isHolder[account] && _weekLotto[account] < WeekCounter && account != DevWallet && account != MojoPot && account != LuckPot && account != MarketingWallet && !account.isContract()){
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if (sender == MojoPot || sender == LuckPot || sender == DevWallet || sender == MarketingWallet || recipient == DevWallet || recipient == MojoPot || recipient == LuckPot || recipient == MarketingWallet) {
            feeAmount = 0;
            gasAmount = 0;
        }        
        ...
        _balances[MojoPot] += feeAmount;
        _balances[MarketingWallet] += gasAmount;
        
        emit Transfer(sender, recipient, amount);
        emit Transfer(sender, MojoPot, feeAmount);        
    ...
    function _burn(address account, uint256 amount) internal virtual {
        ...
        if(account != MojoPot || account != LuckPot){
            _totalSupply -= amount;
        }
    ...
    function AddGroupHolder(address holder) internal {
        if(_isHolder[holder] == false && holder != address(0) && holder != MojoPot && holder != LuckPot && holder != DevWallet && holder != MarketingWallet && !holder.isContract()){
    ...
    function RemoveGroupHolder(address holder) internal {
        if(_isHolder[holder] == true && holder != address(0) && holder != MojoPot && holder != LuckPot && holder != DevWallet && holder != MarketingWallet && !holder.isContract()){
...
contract LadyLuck is Initializable, ERC20Upgradeable, OwnableUpgradeable, PausableUpgradeable, VRFConsumerBase  {
    ...
    uint256[] public GroupBalanceList;
    
    uint256 internal WinningGroupMojo; 
    address public WinningWalletMojo;
    uint256 public WinningNumberMojo;

    uint256 internal WinningGroupLuck;     
    address public WinningWalletLuck;
    uint256 public WinningNumberLuck;

    uint256 public RollingGroupTotal;    
    ...
    function RunLuckLottery() public  {
        uint256[] memory GroupIDList = ShowGroupIDList();  
        require(JackpotStarted == GroupIDList.length, "Please Prepare Lottery Or End Mojo Lottery");        
    ...
    function RunMojoLottery() public  {
        ...
        WinningNumberMojo = SecondRandomResult % RollingGroupTotal + 1;
        
        uint256 i = 0;
        WinningGroupMojo = 0;
        uint256 RollingTotal = 0;        
        while (WinningGroupMojo == 0){
            uint256 CurrentGroupBalance = GroupBalanceList[i];
            if (RollingTotal < WinningNumberMojo && WinningNumberMojo <= RollingTotal + CurrentGroupBalance){
                WinningGroupMojo = GroupIDList[i];
            } else {
                RollingTotal += CurrentGroupBalance;
            }
            i++;
        ...
        address[] memory GroupWinnerList = GetGroupHolders(WinningGroupMojo);
        
        for (uint j=0; j<GroupWinnerList.length; j++) {
            uint256 CurrentUserBalance = balanceOf(GroupWinnerList[j]);
            if (RollingTotal < WinningNumberMojo && WinningNumberMojo <= RollingTotal + CurrentUserBalance){
                  WinningWalletMojo = GroupWinnerList[j];
            }
            RollingTotal += CurrentUserBalance; 
        }
        ...
        uint256 MojoPotBalance = balanceOf(MojoPot);
        _burn(MojoPot, MojoPotBalance);
        _mint(WinningWalletMojo, MojoPotBalance);
    ...
    function GetTrueGroupBalance(uint256 Groups) public view returns(uint256){
        uint256 Temp; 
    ...
    function CurrentMojoPotSize() public view returns(uint256){
        return balanceOf(MojoPot);
    }    
