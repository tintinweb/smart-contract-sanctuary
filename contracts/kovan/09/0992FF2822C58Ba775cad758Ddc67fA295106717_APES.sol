...
contract APES is Ownable, ERC721 {
    ...
    constructor(address _apesWallet) ERC721("ApeOnly","APEO")  {
        apesWallet = payable(_apesWallet);
        SALE_END_DAY = block.timestamp+SALE_DAYS;
    }
    
    function _getCurrentEthAmount() internal pure returns(uint256 ethAmount){
        return 0.3 ether;
    }
    

    function getCurrentEthAmount() external pure returns(uint256){
