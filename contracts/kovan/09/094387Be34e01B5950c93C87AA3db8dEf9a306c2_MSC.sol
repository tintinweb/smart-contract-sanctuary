...
contract MSC is Ownable, Authorizable, usingFlyionBasics { //remove Ownable? Check bytesize
    ...
    modifier contractHasEnoughTokens(address _escrowAddress) { //can process payment
        ...
        require(IERC20(tokenAddress).balanceOf(_escrowAddress) > requiredTokens, "Not enough collateral");
        _;
    }
    ...
    function _killContract(bool _forceKill) public onlyOwner {
        if(_forceKill == false){
            require(IERC20(tokenAddress).balanceOf(escrowAddress) == 0, "Please withdraw Tokens");
    ...
    function _checkTokenBalances() public view onlyOwner returns(uint256 _tokenBalance) {
        _tokenBalance = IERC20(tokenAddress).balanceOf(escrowAddress);
    }
