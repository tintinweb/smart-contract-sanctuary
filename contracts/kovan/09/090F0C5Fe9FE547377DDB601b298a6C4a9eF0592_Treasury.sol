...
contract Comptroller is IComptroller, IRegistry, IDeadline, Ownable, ReentrancyGuard {
    ...
    function setPoolToken(address token) external onlyOwner override {
        PoolRewardToken = token;
    }
    ...
    function getTokenPrice(address token) public view returns (uint quote){
        quote = IfxToken(token).quote();
    }

    function getMinimumCollateralETH(uint tokenAmount, uint ratio, uint unitPrice) public pure returns (uint minimum){
...
interface IfxToken is IERC20 {
    ...
    function quote() external view returns (uint unitPrice);
}
