...
contract OpOnFlashLoan is OpCast {
    ...
    function handleOpenLong(bytes memory arguments) internal {
        bool notOverflow;
        uint256 realAmountLeverageToken;
        uint256 amountSell;
        uint256 pay;
        bool success;
        bytes memory data;
        ...
        (notOverflow, realAmountLeverageToken) = SafeMath.trySub(
            amountLeverageToken,
            flashLoanFee
        );
        require(
            notOverflow == true,
            "CHFRY: amountLeverageToken < flashLoanFee"
        );

        (notOverflow, amountSell) = SafeMath.tryAdd(
            realAmountLeverageToken,
            amountFlashLoan
        );
        ...
        require(success == true, "CHFRY: call AAVEV2 protocol fail");
        ...
        (notOverflow, pay) = SafeMath.tryAdd(amountSell, flashLoanFee);
        require(notOverflow == true, "CHFRY: overflow");
        EventCenterLeveragePositionInterface(
            OperationCenterInterface(opCenterAddress).eventCenterAddress()
    ...
    function handleCloseLong(bytes memory arguments) internal {
        ...
        (notOverflow, temp) = SafeMath.trySub(
        ...
        (notOverflow, temp) = SafeMath.trySub(temp, flashLoanFee);
    ...
    function handleOpenShort(bytes memory arguments) internal {
        ...
        (notOverflow, amountDeposit) = SafeMath.tryAdd(
            realAmountLeverageToken,
            amountFlashLoan
        );
        ...
        require(
            success == true,
            "CHFRY: call AAVEV2 protocol borrowToken fail"
        );
        ...
        (notOverflow, change) = SafeMath.trySub(
            abi.decode(data, (uint256)),
            amountFlashLoan
        );
        require(notOverflow == true, "CHFRY: swap resault not cover flashLoan");
        ...
        if (change > uint256(0)) {
            ...
            require(success == true, "CHFRY: pull back coin fail");
            (notOverflow, change) = SafeMath.tryAdd(
                abi.decode(data, (uint256)),
                flashLoanFee
            );
            require(notOverflow == true, "CHFRY: overflow");
    ...
    function handleCloseShort(bytes memory arguments) internal {
        ...
        require(
            success == true,
            "CHFRY: call UniswapV2 protocol buyToken fail"
        );
        ...
        require(
            notOverflow == true,
            "CHFRY: sell more than amountFlashLoan"
        );
        ...
        require(
            notOverflow == true,
            "CHFRY: withdrawAmount not cover flashLoan"
        );
        ...
        require(notOverflow == true, "CHFRY: change not cover flashLoanFee");
