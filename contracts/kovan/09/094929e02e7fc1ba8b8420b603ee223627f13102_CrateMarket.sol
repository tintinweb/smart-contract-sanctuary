...
contract CrateNFT is Context, ERC721 {
    ...
    Counters.Counter private _saleCrateId;
    ...
    uint256[] private cratesForSale;
    uint256 private _cratePrice;
	uint256 private _hatchPrice;
    uint256 private _totalSupply;

    mapping(uint256 => CrateDetails) private _crateDetails;
    ...
    modifier isValidCrateCount {
        require(
            _crateIds.current() < _totalSupply,
            "Maximum crate limit exceeded!"
        );
        _;
    }
    ...
    modifier isValidSellAmount(uint256 crateId) {
        uint256 purchasePrice = _crateDetails[crateId].purchasePrice;
        uint256 sellingPrice = _crateDetails[crateId].sellingPrice;
        ...
        returns (uint256)
    {
        ...
        _crateDetails[newCrateId] = CrateDetails({
            purchasePrice: _cratePrice,
            sellingPrice: 0,
            forSale: false
        });
    ...
    {
        require(
            (crateCounts() + numOfCrates) <= 1000,
            "Number of crates exceeds maximum crate count"
        );
     ...
     */
    function transferCrate(address buyer) public {
        _saleCrateId.increment();
        uint256 saleCrateId = _saleCrateId.current();

        require(
            msg.sender == ownerOf(saleCrateId),
            "Only initial purchase allowed"
        );

        transferFrom(ownerOf(saleCrateId), buyer, saleCrateId);

        if (!isCustomerExist(buyer)) {
            customers.push(buyer);
        }
        purchasedCrates[buyer].push(saleCrateId);
    }
     ...
     * Update crate details
     */
    function hatchCrate(address seller, uint256 saleCrateId)
        public
    {
        removeCrateFromCustomer(seller, saleCrateId);
        removeCrateFromSale(saleCrateId);

        _crateDetails[saleCrateId] = CrateDetails({
            purchasePrice: _cratePrice,
            sellingPrice: 0,
            forSale: false
        });
	...
	/*
     * Secondary purchase for the crates
     ...
     * Update crate details
     */
    function secondaryTransferCrate(address buyer, uint256 saleCrateId)
        public
        isValidSellAmount(saleCrateId)
    {
        address seller = ownerOf(saleCrateId);
        uint256 sellingPrice = _crateDetails[saleCrateId].sellingPrice;

        transferFrom(seller, buyer, saleCrateId);
    ...
    /*
     * Add crate for sale with its details
    ...
    function setSaleDetails(
        ...
        _crateDetails[crateId].sellingPrice = sellingPrice;
        _crateDetails[crateId].forSale = true;

        if (!isSaleCrateAvailable(crateId)) {
            cratesForSale.push(crateId);
        }

        approve(operator, crateId);
    }
    ...
    // Get next sale crateId
    function getNextSaleCrateId() public view returns (uint256) {
        return _saleCrateId.current();
    }

    // Get selling price for the crate
    function getSellingPrice(uint256 crateId) public view returns (uint256) {
        return _crateDetails[crateId].sellingPrice;
    }

    // Get all crates available for sale
    function getCratesForSale() public view returns (uint256[] memory) {
        return cratesForSale;
    }

    // Get crate details
    function getCrateDetails(uint256 crateId)
        public
        view
        returns (
            uint256 purchasePrice,
    ...
    {
        return (
            _crateDetails[crateId].purchasePrice,
            _crateDetails[crateId].sellingPrice,
            _crateDetails[crateId].forSale
        );
    }
    ...
    // Utility function used to check if crate is already for sale
    function isSaleCrateAvailable(uint256 crateId)
        internal
        view
        returns (bool)
    {
        for (uint256 i = 0; i < cratesForSale.length; i++) {
            if (cratesForSale[i] == crateId) {
                return true;
            }
        }
        return false;
        ...
        internal
    {
        ...
        for (uint256 i = 0; i < numOfCrates; i++) {
            if (purchasedCrates[customer][i] == crateId) {
                for (uint256 j = i + 1; j < numOfCrates; j++) {
                    purchasedCrates[customer][j - 1] = purchasedCrates[
                        customer
                    ][j];
                }
    ...
    function removeCrateFromSale(uint256 crateId) internal {
        uint256 numOfCrates = cratesForSale.length;

        for (uint256 i = 0; i < numOfCrates; i++) {
            if (cratesForSale[i] == crateId) {
                for (uint256 j = i + 1; j < numOfCrates; j++) {
                    cratesForSale[j - 1] = cratesForSale[j];
                }
                cratesForSale.pop();
            }
        }
...
contract AkitaNFT is Context, ERC721 {
    ...
    modifier isValidAkitaCount {
        require(
            _akitaIds.current() < _totalSupply,
            "Maximum akita limit exceeded!"
        );
        _;
    }
    ...
    {
        require(
            (akitaCounts() + numOfAkitas) <= 1000,
            "Number of akitas exceeds maximum akita count"
        );
        ...
        internal
    {
        ...
        for (uint256 i = 0; i < numOfAkitas; i++) {
            if (purchasedAkitas[customer][i] == akitaId) {
                for (uint256 j = i + 1; j < numOfAkitas; j++) {
                    purchasedAkitas[customer][j - 1] = purchasedAkitas[
                        customer
                    ][j];
                }
...
contract KishuNFT is Context, ERC721 {
    ...
    modifier isValidKishuCount {
        require(
            _kishuIds.current() < _totalSupply,
            "Maximum kishu limit exceeded!"
        );
        _;
    }
    ...
    {
        require(
            (kishuCounts() + numOfKishus) <= 1000,
            "Number of kishus exceeds maximum kishu count"
        );
        ...
        internal
    {
        ...
        for (uint256 i = 0; i < numOfKishus; i++) {
            if (purchasedKishus[customer][i] == kishuId) {
                for (uint256 j = i + 1; j < numOfKishus; j++) {
                    purchasedKishus[customer][j - 1] = purchasedKishus[
                        customer
                    ][j];
                }
...
contract CrateMarket {
    ...
    constructor(Token token, CrateNFT crate, AkitaNFT akita) {
        _token = token;
        _crate = crate;
		_akita = akita;
    ...
    function purchaseCrate() public {
        address buyer = msg.sender;

        _token.transferFrom(buyer, _organiser, _crate.getCratePrice());

        _crate.transferCrate(buyer);
    }
	event Purchase(address indexed buyer, address seller, uint256 crateId);
	
	// Stimulate crate and get the dog
    function stimulateCrate(uint256 crateId, uint256 dog_type) public {
        address buyer = _crate.ownerOf(crateId);
        
        _token.transferFrom(buyer, _crate.getOrganiser(), _crate.getHatchPrice());
		// remove the crateId from the customer and sale list
        _crate.hatchCrate(buyer, crateId);
		
		if (dog_type == 1) {
			_akita.transferAkita(buyer);
		}

    }
	
	// Purchase crate from the secondary market hosted by organiser
    function secondaryPurchase(uint256 crateId) public {
        address seller = _crate.ownerOf(crateId);
        address buyer = msg.sender;
        uint256 sellingPrice = _crate.getSellingPrice(crateId);
        ...
        _crate.secondaryTransferCrate(buyer, crateId);

        emit Purchase(buyer, seller, crateId);
    }
