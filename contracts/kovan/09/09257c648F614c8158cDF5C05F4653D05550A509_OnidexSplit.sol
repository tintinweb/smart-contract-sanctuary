...
contract OnidexSplit is OnidexSplitBaseWrap, OnidexSplitWeth, Ownable {
    
    IOnidexSplitView public onidexSplitView;
    using UniversalERC20 for IERC20;
    address public skimAddr = address(0x02aB62496EE0b0dF9ff58f181E237923d8Ad1cBb);

    constructor(IOnidexSplitView _onidexSplitView) {
    ...
    function swap(
        ...
        require(distribution.length <= reserves.length, "OnidexSplit: Distribution array should not exceed reserves array size");
        ...
        require(returnAmount >= minReturn, "OnidexSplit: Return amount was not enough");
    ...
    function _swapOnUniswapV2Internal(
        ...
        IUniswapV2Exchange exchange = uniswapV2Factory.getPair(fromTokenReal, toTokenReal);
        bool needSync;
        bool needSkim;
        (returnAmount, needSync, needSkim) = UniswapV2ExchangeLib.getReturn(exchange, fromTokenReal, toTokenReal, amount);
        if (needSync) {
            exchange.sync();
        }
        else if (needSkim) {
            exchange.skim(skimAddr);
        }
    ...
    // uniswapv3 swap functions
    function _swapOnUniswapV3Internal(
        IERC20 fromToken,
        IERC20 destToken,
        uint256 amount,
        uint256 /*flags*/
        ...
        IERC20 fromTokenReal = fromToken.isETH() ? weth : fromToken;
        IERC20 toTokenReal = destToken.isETH() ? weth : destToken;
        IUniswapV3Exchange exchange = uniswapV3Factory.getPool(address(fromTokenReal), address(toTokenReal), UNISWAPV3_FEE);
        bool needSync;
        bool needSkim;
        (returnAmount, needSync, needSkim) = UniswapV3ExchangeLib.getReturn(exchange, fromTokenReal, toTokenReal, amount);
        if (needSync) {
            exchange.sync();
        }
        else if (needSkim) {
    ...
    // sushi swap functions
    function _swapOnSushiInternal(
        IERC20 fromToken,
        IERC20 destToken,
        uint256 amount,
        uint256 /*flags*/
        ...
        IERC20 fromTokenReal = fromToken.isETH() ? weth : fromToken;
        IERC20 toTokenReal = destToken.isETH() ? weth : destToken;
        IUniswapV2Exchange exchange = sushiswapFactory.getPair(fromTokenReal, toTokenReal);
        bool needSync;
        bool needSkim;
        (returnAmount, needSync, needSkim) = UniswapV2ExchangeLib.getReturn(exchange, fromTokenReal, toTokenReal, amount);
        if (needSync) {
    ...
    function _swapOnKyber(
        ...
        if (!fromToken.isETH()) {
            ...
            returnAmount = kyberNetworkProxy.tradeWithHintAndFee(
                ...
                type(uint256).max,
                0,
                payable(0x02aB62496EE0b0dF9ff58f181E237923d8Ad1cBb),
                (flags >> 255) * 10,
                fromHint
            );
        }
    ...
    function setSkimAddress(address _newSkim) external onlyOwner {
        require(_newSkim != address(0), "OnidexSplit: Zero address");
        skimAddr = _newSkim;
    }
