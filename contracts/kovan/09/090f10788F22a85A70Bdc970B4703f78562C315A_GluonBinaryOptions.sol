...
contract GluonBinaryOptions {
    ...
    function resolveOption(bytes32 identifier) public returns (bool success){
        ...
        if( ! option.resolved){
            ...
            uint256(latestPrice) >= option.price*1e8 ? setOptionResult(option.identifier, Result.Yes) : setOptionResult(option.identifier, Result.No);
            option.resolved = true;
    ...
    function receivePayment(bytes32 identifier) public returns (bool success) {
        Option storage option = Options[identifier];
        require(option.expiryBlock < block.number, 'Not expired');
        require(r > 0, 'Incorrect ratio');
        Bet storage bet = Bets[msg.sender][identifier];
        require(bet.amount > 0, 'Incorrect bet amount');
        require(option.totalPot > 0, 'Total pot size 0');
        require(!bet.paidOut, 'Already paid out');

        if (option.result != Result.Undecided) {
            require(bet.predictedResult == option.result, 'Result not matching'); 
        }
