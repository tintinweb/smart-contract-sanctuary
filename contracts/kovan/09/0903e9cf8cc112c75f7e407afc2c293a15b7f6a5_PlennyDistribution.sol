...
interface IPlennyERC20 is IERC20Upgradeable {
 ...
 */
contract PlennyERC20 is ERC20Upgradeable, ERC20CappedUpgradeable, PlennyBasePausable, IPlennyERC20 {
...
contract PlennyDistribution is Ownable {
    using SafeMath for uint;
    using SafeERC20Upgradeable for PlennyERC20;
    ...
    function distribute(string memory name, string memory symbol,
        bytes32 salt, uint256 _plennyTotalSupply) public onlyOwner {

        bytes32 newsalt = keccak256(
            abi.encodePacked(salt, INTERNAL_HASH, msg.sender)
        );

        plennyToken = Create2.deploy(0, newsalt, type(PlennyERC20).creationCode);
        PlennyERC20(plennyToken).initialize(name, symbol, msg.sender);
        PlennyERC20(plennyToken).mint(msg.sender, _plennyTotalSupply);
    ...
    function plennyTotalSupply() external view returns (uint256) {
        return PlennyERC20(plennyToken).totalSupply();
    }
}