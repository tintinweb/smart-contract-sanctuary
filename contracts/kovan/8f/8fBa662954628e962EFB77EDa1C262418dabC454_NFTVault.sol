...
contract NFTVault {
    ...
    function lockNFT(uint256 id) public {
        require(deposits[msg.sender][id] == false, "already deposited");
        deposits[msg.sender][id] = true;
        nft.safeTransferFrom(msg.sender, address(this), id, 1, "");
    }

    function unlockNFT(uint256 id) public {
        require(deposits[msg.sender][id] == true, "not deposited");
        require(usedInStrat[msg.sender][id] == 0, "strat being used");
        deposits[msg.sender][id] = true;
        nft.safeTransferFrom(address(this), msg.sender, id, 1, "");
    }

    function registerStrat(address strat) public {
        strats[strat] = true;
    }

    function useNFT(address user, uint256 id) public {
        require(strats[msg.sender] == true, "Not registered strat");
        usedInStrat[user][id] = usedInStrat[user][id].add(1);
    }

    function finishUseNFT(address user, uint256 id) public {
        require(strats[msg.sender] == true, "Not registered strat");
        usedInStrat[user][id] = usedInStrat[user][id].sub(1);
    }
}