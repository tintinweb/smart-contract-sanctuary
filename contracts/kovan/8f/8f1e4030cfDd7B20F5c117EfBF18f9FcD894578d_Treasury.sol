...
contract Treasury is ITreasury, AccessControl, ReentrancyGuard, IValidator {
    ...
    mapping(address => uint256) public interestRate;
    ...
    uint256 public mintFeePerMille;
    uint256 public burnFeePerMille;
    uint256 public withdrawFeePerMille;
    ...
    function increaseDebtPosition(
        ...
        vaults[account][fxToken].debt = vaults[account][fxToken].debt.add(
            amount
        );
    }
    ...
    {
        require(
            Comptroller(comptroller).isCollateralValid(WETH),
            "Collateral is invalid"
        );
    ...
    function decreaseDebtPosition(
        ...
        vaults[account][fxToken].debt = vaults[account][fxToken].debt.sub(
            amount
        );
    }
    ...
    function withdrawCollateralETH(
        ...
        require(
            Comptroller(comptroller).getFreeCollateral(msg.sender, fxToken) >=
                amount,
        ...
        returns (uint256)
    {
        ...
        if (isLiquidatingVault(user, fxToken)) return debt;
        uint256 interest = calculateInterest(user, fxToken);
    ...
    modifier onlyAddressOrOperatorExcludeAdmin(address addressAllowed) {
        ...
        require(
            ...
            "Caller is not an operator or an allowed sender, or is an admin"
        );
        _;
    }

    modifier notLiquidatingVault(address account, address fxToken) {
        require(
            !isLiquidatingVault(account, fxToken),
     ...
     * @dev currently assumes all collateral types are worth 1 ETH.
     * @dev TODO convert other collateral types to the appropriate ETH price quote.
        ...
        returns (uint256 balance)
    {
        address[] memory collateralTokens =
            Comptroller(comptroller).getAllCollateralTypes();
        Vault storage vault = vaults[account][fxToken];
        balance = 0;
        for (uint256 i = 0; i < collateralTokens.length; i++) {
            // TODO fetch collateral ETH quote when implementing support for multi-collateral vaults.
            uint256 collateralAsEth =
                vault.collateralBalance[collateralTokens[i]];
    ...
    /**
     * @notice returns whether the vault is being liquidated
     * @dev TODO: implement
     * @param account the account to check
     * @param fxToken the vault to check
     * @return whet
    ...
    function _withdrawCollateralFrom(
        ...
        require(
            Comptroller(comptroller).getFreeCollateral(from, fxToken) >= amount,
            "Not enough free collateral to withdraw"
        );
        ...
        require(
            ...
            "Not enough of collateral type to withdraw"
        );
    ...
    function _updateVaultInterest(address user, address fxToken)
        private
        notLiquidatingVault(user, fxToken)
    {
    ...
    {
        uint256 timeElapsed =
            block.timestamp.sub(interestLastUpdateDate[user][fxToken]);
        address[] memory collateralTokens =
            Comptroller(comptroller).getAllCollateralTypes();
        Vault storage vault = vaults[user][fxToken];
        ...
        for (uint256 i = 0; i < collateralTokens.length; i++) {
            ...
            weightedInterestRate = weightedInterestRate.add(
                interestRate[collateralTokens[i]].mul(collateralAsEth).div(
                    totalCollateralAsEth
                )
            );
        }
        interest = vault
    ...
    ReentrancyGuard
{
    ...
    address public immutable WETH;
    uint256 constant minMintDeviation = 5;
    ...
    function mintWithEth(
        ...
        address token,
        uint256 deadline
    ) external payable override dueBy(deadline) nonReentrant {
        ...
        IfxToken(token).mint(ITreasury(Treasury).FeeRecipient(), feeTokens);
        assert(
        ...
        uint256 debtPosition = ITreasury(Treasury).getDebt(msg.sender, token);
        ITreasury(Treasury).increaseDebtPosition(
            msg.sender,
            tokenAmount,
            token
        );
        assert(
            debtPosition.add(tokenAmount) ==
                ITreasury(Treasury).getDebt(msg.sender, token)
        );
        ...
        balanceBefore = ITreasury(Treasury).getCollateralBalance(
            msg.sender,
            WETH,
            token
        );
        ITreasury(Treasury).depositCollateral(
            msg.sender,
            msg.value,
            WETH,
            token
        ...
        assert(
            ITreasury(Treasury).getCollateralBalance(msg.sender, WETH, token) ==
    ...
    function mint(
        ...
        require(
            isCollateralValid[collateralToken],
            "Collateral token is not valid"
        );

        // Update interest.
        ITreasury(Treasury).updateVaultInterest(msg.sender, token);
    }
    ...
    function mintWithoutCollateral(
        ...
        uint256 quote = getTokenPrice(token);
        require(
            getFreeCollateral(msg.sender, token) >=
                getMinimumCollateralETH(
                    tokenAmount,
                    tokenDetails[token].mintCR,
                    quote
    ...
    function burn(
        ...
        require(isFxTokenValid[token], "fxToken is not valid");
        require(
            IfxToken(token).balanceOf(msg.sender) >= amount,
            "Token balance is lower than burn amount"
        );
        require(
            ITreasury(Treasury).getDebt(msg.sender, token) >= amount,
            "Treasury debt is lower than burn amount"
        );
        ...
        // Withdraw fee.
        ITreasury(Treasury).withdrawCollateralFrom(
            msg.sender,
            WETH,
            ITreasury(Treasury).FeeRecipient(),
            fee,
            token
        );
        ...
        returns (uint256 feePrice)
    {
        ...
        uint256 feePerMille = ITreasury(Treasury).burnFeePerMille();
        feePrice = tokenAmount.mul(feePerMille).div(1000).mul(unitPrice).div(
            1 ether
        );
        uint256 available =
            getFreeCollateral(msg.sender, token).add(
                unitPrice
                    .mul(tokenAmount)
                    .div(1 ether)
                    .mul(tokenDetails[token].mintCR.sub(minMintDeviation))
                    .div(100)
            );
    }
    ...
    function removeFxToken(address token) external override onlyOwner {
        ...
        require(
            tokenIndex < validFxTokens.length,
            "Cannot remove an invalid token"
        );
    ...
    function setValidTokens(
        ...
        for (uint256 i = 0; i < _validFxTokens.length; i++) {
            setFxToken(
                _validFxTokens[i],
                _mintCR,
                _liquidateCR,
    ...
    function getRewardAmount(uint256 rate, uint256 tokenAmount)
        internal
        pure
    ...
    function getMinimumCollateralETH(
        uint256 tokenAmount,
        uint256 ratio,
        uint256 unitPrice
    ) public pure returns (uint256 minimum) {
        minimum = unitPrice
            .mul(tokenAmount)
            .div(1 ether)
            .mul(ratio.sub(minMintDeviation))
            .div(100);
    }
    ...
    function getAllMintCR()
        external
        view
        override
        returns (uint256[] memory ratios)
    {
        uint256[] memory _ratios = new uint256[](validFxTokens.length);
     ...
     */
    function getFreeCollateral(address account, address fxToken)
        public
        view
        override
        returns (uint256 free)
    {
        ...
        for (uint256 i = 0; i < validFxTokens.length; i++) {
            locked = locked.add(
                getMinimumCollateralETH(
                    ITreasury(Treasury).getDebt(account, validFxTokens[i]),
        ...
        for (uint256 i = 0; i < collateralTokens.length; i++) {
            deposit = deposit.add(
                ITreasury(Treasury).getCollateralBalance(
                    account,
                    collateralTokens[i],
                    fxToken
                )
    ...
    function setOracle(address _fxToken, address _oracle)
        ...
        onlyOwner
    {
        require(_fxToken != address(0), "Asset cannot be address 0");
        require(_oracle != address(0), "Oracle cannot be address 0");
...
interface IComptroller {
    ...
    // Events
    event MintToken(uint256 tokenRate, uint256 amountSpent);
    ...
    function getAllMintCR() external view returns (uint256[] memory ratios);
