...
contract TokenaFactory is ITokenaFactory, Ownable, ReentrancyGuard {
	...
	address public immutable _pancakeFactory;
	address public immutable _quickswapFactory;
	...
	constructor(
		...
		require(uniswapFactory != address(0), "Not valid uniswap address");
		require(pancakeFactory != address(0), "Not valid pancake address");
		require(quickswapFactory != address(0), "Not valid quickswap address");
		feeTaker = _feeTaker;
		_uniswapFactory = uniswapFactory;
		_pancakeFactory = pancakeFactory;
		_quickswapFactory = quickswapFactory;
	 ...
	 * @return temp: tete
	 */
	function getUserStakings(address user) external view returns (address[] memory) {
	 ...
	 * @return
	 */
	function getUserLms(address user) external view returns (address[] memory) {
		address[] memory temp = new address[](userStakings[user].length);
		for (uint256 i = 0; i < userStakings[user].length; i++) {
			temp[i] = stakings[userStakings[user][i]];
		}
