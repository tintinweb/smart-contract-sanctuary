...
import "../../protocol/intf/IAutoTrader.sol";
import "../../protocol/intf/ICallee.sol";
import "../../protocol/lib/Account.sol";
import "../../protocol/lib/Decimal.sol";
import "../../protocol/utils/Math.sol";
import "../../protocol/lib/Monetary.sol";
// import "../../protocol/lib/Require.sol";
import "../../protocol/lib/Time.sol";
 ...
 */
contract ExpiryV2 is Ownable, OnlyDxln, ICallee, IAutoTrader {
    ...
    // bytes32 constant FILEEXPV2 = "ExpiryV2";
    ...
    event LogExpiryRampTimeSet(uint256 expiryRampTime);
    ...
    constructor(address dxlnMargin, uint256 expiryRampTime)
        OnlyDxln(dxlnMargin)
    {
    ...
    function ownerSetExpiryRampTime(uint256 newExpiryRampTime)
        external
        onlyOwner
    {
    ...
    function approveSender(address sender, uint32 minTimeDelta) external {
    ...
    function callFunction(
        ...
        bytes memory data
    ) public override onlyDxln(msg.sender) {
    ...
    function getTradeCost(
        ...
        bytes memory data
    ) public override onlyDxln(msg.sender) returns (Types.AssetAmount memory) {
        ...
        (uint256 owedMarketId, uint32 maxExpiry) = abi.decode(
            data,
            (uint256, uint32)
        );
        ...
        // validate expiry
        require(expiry != 0, "Expiry not set");
        require(expiry <= Time.currentTime(), "Borrow not yet expired");
        require(expiry <= maxExpiry, "Expiry past maxExpiry");

        return
            getTradeCostInternal(
                inputMarketId,
                outputMarketId,
                makerAccount,
                oldInputPar,
    ...
    function getExpiry(Account.Info memory account, uint256 marketId)
        public
        view
        returns (uint32)
    {
    ...
    function getSpreadAdjustedPrices(
        ...
        uint32 expiry
    ) public view returns (Monetary.Price memory, Monetary.Price memory) {
        Decimal.D256 memory spread = DXLN_MARGIN.getLiquidationSpreadForPair(
            heldMarketId,
            owedMarketId
        );
        ...
        Monetary.Price memory heldPrice = DXLN_MARGIN.getMarketPrice(
            heldMarketId
        );
        Monetary.Price memory owedPrice = DXLN_MARGIN.getMarketPrice(
            owedMarketId
        );
        owedPrice.value = owedPrice.value.add(
            Decimal.mul(owedPrice.value, spread)
        );
    ...
    function callFunctionSetExpiry(address sender, bytes memory data) private {
        (CallFunctionType callType, SetExpiryArg[] memory expiries) = abi
            .decode(data, (CallFunctionType, SetExpiryArg[]));
        ...
        for (uint256 i = 0; i < expiries.length; i++) {
            ...
            if (exp.account.owner != sender) {
                ...
                uint32 minApprovedTimeDelta = g_approvedSender[
                    exp.account.owner
                ][sender];
                if (
                    minApprovedTimeDelta == 0 ||
                    exp.timeDelta < minApprovedTimeDelta
                ) {
                    continue;
                }
            }
            ...
            if (
                exp.timeDelta != 0 &&
                DXLN_MARGIN
                    .getAccountPar(exp.account, exp.marketId)
                    .isNegative()
            ) {
                // only change non-zero values if forceUpdate is true
                if (
                    exp.forceUpdate || getExpiry(exp.account, exp.marketId) == 0
                ) {
                    uint32 newExpiryTime = Time
                        .currentTime()
                        .add(exp.timeDelta)
                        .to32();
    ...
    function callFunctionSetApproval(address sender, bytes memory data)
        private
    {
        (CallFunctionType callType, SetApprovalArg memory approvalArg) = abi
            .decode(data, (CallFunctionType, SetApprovalArg));
    ...
    function getTradeCostInternal(
        ...
        uint256 owedMarketId,
        uint32 expiry
    ) private returns (Types.AssetAmount memory) {
        Types.AssetAmount memory output;
        Types.Wei memory maxOutputWei = DXLN_MARGIN.getAccountWei(
            makerAccount,
            outputMarketId
        );

        if (inputWei.isPositive()) {
            require(inputMarketId == owedMarketId, "inputMarket mismatch");
            require(!newInputPar.isPositive(), "Borrows cannot be overpaid");
            assert(oldInputPar.isNegative());
            require(maxOutputWei.isPositive(), "Collateral must be positive");
            ...
            require(outputMarketId == owedMarketId, "outputMarket mismatch");
            require(!newInputPar.isNegative(), "Collateral cannot be overused");
            assert(oldInputPar.isPositive());
            require(maxOutputWei.isNegative(), "Borrows must be negative");
        ...
        require(output.value <= maxOutputWei.value, "outputMarket too small");
    ...
    function setExpiry(
        ...
        emit ExpirySet(account.owner, account.number, marketId, time);
    }
    ...
    function heldWeiToOwedWei(
        ...
        uint256 owedMarketId,
        uint32 expiry
    ) private view returns (Types.AssetAmount memory) {
        (
            Monetary.Price memory heldPrice,
            Monetary.Price memory owedPrice
        ) = getSpreadAdjustedPrices(heldMarketId, owedMarketId, expiry);
