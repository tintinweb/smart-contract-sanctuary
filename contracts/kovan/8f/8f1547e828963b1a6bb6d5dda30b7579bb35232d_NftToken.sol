...
// Dependency file: contracts/token/Tracker.sol
...
abstract contract Tracker {
    uint256 internal _id = 1e6; // start at 1000000

    function getID() public view returns (uint256) {
        return _id;
    }

    function updateID() public returns (uint256) {
        _id = _id + 1;
        return _id - 1;
    }
}
...
// Dependency file: contracts/token/MinterRole.sol
...
abstract contract MinterRole {
    mapping (address => bool) internal _roles;

    event MinterSet(address indexed minter, bool indexed permit);

    function getMinter(address minter) public view returns (bool) {
        return _roles[minter];
    }

    function setMinter(address minter, bool permit) public onlyMinter(msg.sender) returns (bool) {
        _roles[minter] = permit;

        emit MinterSet(minter, permit);

        return true;
    }

    modifier onlyMinter(address minter) {
        require(getMinter(minter), "Role: only minter");
        _;
    }
}
...
// Root file: contracts/token/NftToken.sol


pragma solidity 0.6.12;


// import "contracts/token/Tracker.sol";
// import "contracts/token/MinterRole.sol";
// import "contracts/token/INftToken.sol";
// import "@openzeppelin/contracts/token/ERC721/ERC721.sol";

contract NftToken is ERC721, Tracker, MinterRole {
    constructor() ERC721("Nft Token", "NFT") public {
        _roles[msg.sender] = true;
    }

    function mint(address owner) public onlyMinter(msg.sender) returns (bool) {
        uint256 id = updateID();
