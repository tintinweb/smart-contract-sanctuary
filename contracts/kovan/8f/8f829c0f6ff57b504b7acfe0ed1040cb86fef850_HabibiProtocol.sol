...
contract HabibiProtocol is Context, IBEP20, Ownable {
    ...
    uint256 internal _tokenTotal = 121000000001 *10**9;
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        uint256 _BurnFee = amount.mul(150).div(10**(_feeDecimal + 2));
        uint256 _lastBurnFee = _tokenTotal.sub(121000000000 *10**9);
        ...
        if(!isExcludedFromFee[sender] && !isExcludedFromFee[recipient] && !inSwapAndLiquify) {
            ...
            if (_tokenTotal <= 1000000000000000 *10**9 && _tokenTotal > 250000000000000 *10**9) {
                ...
                uint256 liquidityFee = amount.mul(200).div(10**(_feeDecimal + 2));
                ...
                uint256 charityFee = amount.mul(100).div(10**(_feeDecimal + 2));
                ...
                //@dev Take development fee
                uint256 developmentFee = amount.mul(100).div(10**(_feeDecimal + 2));
                ...
                emit Transfer(sender,developmentAddress,developmentFee);
                
                //@dev Take burn fee
                uint256 burnFee = amount.mul(300).div(10**(_feeDecimal + 2));
            ...
            if (_tokenTotal > 121000000000 *10**9 && _tokenTotal.sub(_BurnFee) < 121000000000 *10**9) {
                ...
                //@dev Take charity fee
                uint256 charityFee = amount.mul(50).div(10**(_feeDecimal + 2));
                transferAmount = transferAmount.sub(charityFee);
                _reflectionBalance[charityAddress] = _reflectionBalance[charityAddress].add(charityFee.mul(rate));
                _charityFeeTotal = _charityFeeTotal.add(charityFee);
                emit Transfer(sender,charityAddress,charityFee);
                ...
                //@dev Take development fee
                uint256 developmentFee = amount.mul(50).div(10**(_feeDecimal + 2));
                transferAmount = transferAmount.sub(developmentFee);
                _reflectionBalance[developmentAddress] = _reflectionBalance[developmentAddress].add(developmentFee.mul(rate));
                _developmentFeeTotal = _developmentFeeTotal.add(developmentFee);
                emit Transfer(sender,developmentAddress,developmentFee);
                ...
                //@dev Take burn fee
                transferAmount = transferAmount.sub(_lastBurnFee);
                _tokenTotal = _tokenTotal.sub(_lastBurnFee);
                _burnFeeTotal = _burnFeeTotal.add(_lastBurnFee);
                emit Transfer(sender,address(0),_lastBurnFee);
            ...
            if (_tokenTotal <= 250000000000000 *10**9 && _tokenTotal > 121000000000 *10**9) {
                ...
                //@dev Take burn fee
                uint256 burnFee = amount.mul(150).div(10**(_feeDecimal + 2));
                transferAmount = transferAmount.sub(burnFee);
                _tokenTotal = _tokenTotal.sub(burnFee);
                _burnFeeTotal = _burnFeeTotal.add(burnFee);
                emit Transfer(sender,address(0),burnFee);
            ...
            if (amount > 0) {
                ...
                //@dev Take liquidity fee
                uint256 liquidityFee = amount.mul(100).div(10**(_feeDecimal + 2));
                transferAmount = transferAmount.sub(liquidityFee);
                _reflectionBalance[address(this)] = _reflectionBalance[address(this)].add(liquidityFee.mul(rate));
                _liquidityFeeTotal = _liquidityFeeTotal.add(liquidityFee);
                emit Transfer(sender,address(this),liquidityFee);
