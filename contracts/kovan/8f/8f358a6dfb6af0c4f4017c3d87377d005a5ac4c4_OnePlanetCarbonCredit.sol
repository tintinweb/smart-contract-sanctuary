...
contract OnePlanetCarbonCredit is ERC20Interface, Owned, SafeMath {
    ...
    constructor() public {
        ...
        bonus1 = 500000000000000000000; // bonus 1 5% = 500 tokens
        bonus2 = 1000000000000000000000; // bonus 2 10% = 1000 tokens
        bonus3 = 2500000000000000000000; // bonus 3 15% = 2500 tokens
    ...
    modifier estGas {
        ...
        _;
        gasUsed = safeSub(gasAtStart, gasleft());
    }
    
     modifier OnePlanetOffset {
        uint256 gasAtStartOffset = gasleft();
        _;
        uint256 gasUsedOffset = safeSub(gasAtStartOffset, gasleft());
        
        CO2Emitted = safeMul(gasUsedOffset, gasCO2Factor); //gas used multiplied by C02 coversion factor
        
        OnePlanetInterface OnePlanetInstance = OnePlanetInterface(0xD67D78f4114D94587D226b2af5C21550fb4A6AA7);
        require(OnePlanetInstance.balanceOf(msg.sender) >= CO2Emitted, "Not enough 1Planet tokens to retire"); //Require that the NFT minter has enough 1planet tokens
        OnePlanetInstance.retireOnePL(CO2Emitted, "This is a test!");//Retire 1Planet tokens based on CO2 emitted
     }
    ...
    // ------------------------------------------------------------------------
    receive() external estGas OnePlanetOffset payable {
    ...
    //so the last values of gasUsed gets saved before it updates with this function completion
        gasUsed2 = gasUsed1;
        gasUsed1 = gasUsed;
    ...
    function genAndSendTokens(address to, uint tokens) public onlyOwner estGas OnePlanetOffset returns (bool success) {
        require(now >= startDate && now <= endDate);
    ...
    //--------------------------------------------------------------------------------------
    function setMaxVolume(uint maxVolume) public onlyOwner {
    ...
    //--------------------------------------------------------------------------------------
    function setEthSigDigits(uint digits) public onlyOwner {
        SigDigits = digits;
    }
    ...
    //-------------------------------------------------------------------------------------------
    function retireOnePL(uint tokens, string memory message) public estGas returns (bool success) {
     ...
     */
    function _burn(address account, uint256 amount) public onlyOwner returns (bool success) {
        ...
        //so the last 2 values of gasUsed gets saved before it updates with this function completion
        gasUsed2 = gasUsed1;
        gasUsed1 = gasUsed;
        return true;
    }
