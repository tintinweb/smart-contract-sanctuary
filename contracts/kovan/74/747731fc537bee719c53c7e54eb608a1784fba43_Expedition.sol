...
contract Expedition is Ownable {
    ...
    function updateLockPeriod(uint256 _pid, uint256 _lockPeriod, uint256 _penaltyPercentage) public onlyOwner{
        PoolInfo storage pool = poolInfo[_pid];
    ...
    function stopFarming(uint256 _pid) public onlyOwner{
        PoolInfo storage pool = poolInfo[_pid];
        pool.isStopped = true;
    }
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            for (uint256 index = 0; index < user.stakeInfo.length; index++) {
                ...
                if ((currentTimeStamp.sub(user.stakeInfo[index].timeStamp)) >= pool.lockPeriod ){
                    pending = pending.add(user.stakeInfo[index].amount.mul(accTokensPerShare).div(1e12).sub(user.stakeInfo[index].rewardDebt));
                    rewardwithoutLockPeriod = rewardwithoutLockPeriod.add(pending);
                }else{
                    uint256 reward = user.stakeInfo[index].amount.mul(accTokensPerShare).div(1e12).sub(user.stakeInfo[index].rewardDebt);
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        require(pool.isStopped == false, "CG Staking: Staking Ended, Please withdraw your tokens");
        updatePool(_pid);
        if (user.amount > 0) {
            ...
            for (uint256 index = 0; index < user.stakeInfo.length; index++) {
                ...
                if ((currentTimeStamp.sub((user.stakeInfo[index].timeStamp))) >= pool.lockPeriod ){
                    totalEligible = totalEligible.add(user.stakeInfo[index].amount);
                    totalRewardDebt = totalRewardDebt.add(user.stakeInfo[index].rewardDebt);
                    pending = pending.add(totalEligible.mul(accTokensPerShare).div(1e12).sub(totalRewardDebt));
                    user.stakeInfo[index].rewardDebt = user.stakeInfo[index].amount.mul(accTokensPerShare).div(1e12);
                }
            }
        ...
        for (uint256 index = 0; index < user.stakeInfo.length; index++) {
            ...
            if ( timeToBreak ){
             ...
             if ( user.stakeInfo[index].amount == 0 ){
                ...
                uint256 rewardPerDebtShare  = user.stakeInfo[index].rewardPerDebtShare;
                ...
                if ((currentTimeStamp.sub((user.stakeInfo[index].timeStamp))) < pool.lockPeriod ){
                    ...
                    uint256 currentAmountReward = deductionAmount.mul(pool.accTokensPerShare).div(1e12).sub(deductionAmount.mul(rewardPerDebtShare));
                    uint256 rewardPenalty = currentAmountReward.mul(pool.penaltyPercentage).div(10**2);
                    ...
                    user.stakeInfo[index].rewardDebt = user.stakeInfo[index].rewardDebt.sub(deductionAmount.mul(rewardPerDebtShare));
                }else{
                    rewardForTransfers = rewardForTransfers.add(deductionAmount.mul(pool.accTokensPerShare).div(1e12).sub((deductionAmount.mul(rewardPerDebtShare))));
                }
    ...
    function getStakeInfo(uint256 index) public view returns (uint256, uint256, uint256, uint256){
        UserInfo storage user = userInfo[msg.sender];
        return (user.stakeInfo[index].amount, user.stakeInfo[index].rewardDebt, user.stakeInfo[index].rewardPerDebtShare, user.stakeInfo[index].timeStamp );
    }

    // Let user see total deposits
    function getUserStakesLength() public view returns (uint256){
