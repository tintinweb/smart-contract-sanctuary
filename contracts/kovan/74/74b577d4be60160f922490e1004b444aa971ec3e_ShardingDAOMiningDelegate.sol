...
contract ShardingDAOMining is IInvitation, ActivityBase {
        ...
        if (currentRankCount > 0) {
            ...
            for (uint256 i = 0; i < currentRankCount; i++) {
                ...
                if (i < currentRankCount - 1){
                    ...
                    undistributeDividend = undistributeDividend.sub(averageDividend);
                }
                else{
                    addDividend = undistributeDividend;
                }
    ...
    function calculatePendingSHARD(uint256 _pid, address _user) private view returns (uint256 _pending, uint256 _potential){
        ...
        uint256 poolReward =
            getModifiedRewardToken(pool.lastDividendHeight.add(1), block.number);
    ...
    function modifyWeightByInvitation(
        ...
        uint256 userTotalWeight = getUserModifiedWeight(p.pid, p.user);
        UserInfo storage user = userInfo[p.pid][p.user];
        uint256 poolSHDPerBlock = poolAccs[p.pid];
        ...
        if(p.existedAmount == 0){
            ...
            if(pool.usersTotalWeight == 0 && pool.accumulativeDividend > 0){
                userRevenue = pool.accumulativeDividend;
                pool.accumulativeDividend = 0;
            }
        }else{
            userRevenue = userTotalWeight;
            userRevenue = userRevenue.mul(poolSHDPerBlock).div(1e12).sub(senderRevenueInfo.userRewardDebt);
            senderRevenueInfo.revenue = senderRevenueInfo.revenue.add(userRevenue);
        }
        userTotalWeight = userTotalWeight.add(p.addWeight);    
        if (!senderInfo.isWithdrawn || (p.existedAmount > 0 && user.isCalculateInvitation)) {
            userTotalWeight = userTotalWeight.add(p.addInvitorWeight);
            ...
            if (invitorInfo.amount > 0) {
                ...
                uint256 invitorRevenue = invitorModifiedWeight;
                invitorRevenue = invitorRevenue.mul(poolSHDPerBlock).div(1e12).sub(invitorRevenueInfo.userRewardDebt);
                ...
                invitorRewardDebt = invitorRewardDebt.mul(poolSHDPerBlock).div(1e12);
                invitorRevenueInfo.userRewardDebt = invitorRewardDebt;
            } 
        }
        senderRevenueInfo.userRewardDebt = userTotalWeight.mul(poolSHDPerBlock).div(1e12);
        pool.usersTotalWeight = poolTotalWeight;
    }

    function verifyDescription(string calldata description)
        internal
        pure
