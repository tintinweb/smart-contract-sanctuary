...
interface IBuoy {
    ...
    function setNFT(uint,uint,uint,uint) external;
...
//=======================================staking========================================//
    ...
    */
    ...
    function getRewards(uint _id) public returns(uint) {
        ...
        (,,,,_rewarded,,,) = buoyERC721.viewNFT(_id);
        ...
        while(block.timestamp >= _nextClaim && _payouts != 0) {
            ...
            if(stakingRewards > _contributed)
            _payouts--;
        } 
        buoyERC721.setNFT(_id, _rewarded+stakingRewards, _nextClaim, _payouts);
        if(_payouts == 0) {
