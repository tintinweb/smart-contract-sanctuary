...
contract LGE is Ownable, LivePrice {
    ...
    function swapETHtoDAI(uint256 amountToSend, uint256 swapSlippage) public returns (uint256 amountOut) {
        address[] memory path = new address[](2);
      path[0] = UniSwap.WETH();
      path[1] = DAI; // DAI
      uint256 priceFeed = uint(getLatestPrice());
      uint256 amountOutMin = 0;//amountToSend.mul(priceFeed).div(oracleValueDivisor).mul(100-swapSlippage).div(100); // 30% slippage for test
      UniSwap.swapExactETHForTokens{value: amountToSend}(amountOutMin, path, address(this), block.timestamp.add(180));
      TokenInterface _DAI = TokenInterface(DAI);
      amountOut = _DAI.balanceOf(address(this));
    }
