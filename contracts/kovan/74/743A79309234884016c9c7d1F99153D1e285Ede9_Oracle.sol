...
contract Oracle is IOracle, Ownable
{
  ...
  function _calcTokenETHRate (address token) private view returns (uint256)
  {
    ...
    {
      return ( _uintify(IFeed(_USDFeed[token]).latestAnswer()) * 1e18 ) / _uintify(IFeed(_USDFeed[address(0xd67AB1D41c1bf29B0F8a2284B26f395B40d9F6B6)]).latestAnswer());
    }
    else
    {
      return 0;
  ...
  {
    if (from == to && to == address(0xC22780731758d1E309B9774009027D2Eb325F66f))
    {
      return 1e18;
    }

    uint256 srcRate = from == address(0xd67AB1D41c1bf29B0F8a2284B26f395B40d9F6B6) ? 1e18 : _calcTokenETHRate(from);
    uint256 destRate = to == address(0xd67AB1D41c1bf29B0F8a2284B26f395B40d9F6B6) ? 1e18 : _calcTokenETHRate(to);
