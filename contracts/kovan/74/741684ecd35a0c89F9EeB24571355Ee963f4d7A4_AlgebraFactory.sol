...
contract AlgebraFactory is IAlgebraFactory {
    ...
    bytes32 internal constant POOL_INIT_CODE_HASH = 0xd126771fabb4070c1d7d8b4425d7f660d8a07f23901ba7882aaeb075dce2bdf3;
    ...
    function getAverages(
        ...
        if (lte(time, oldest.blockTimestamp, time - TIME_AGO)) {
            (
                ...
                uint256 volumePerLiquidityCumulativeBefore
            ) = getSingleTimepoint(self, time, TIME_AGO, tick, index, liquidity);
            (
                ,
                uint160 secondsPerLiquidityCumulativeAfter,
                uint112 volatilityAfter,
                uint256 volumePerLiquidityCumulativeAfter
            ) = getSingleTimepoint(self, time, 0, tick, index, liquidity);
            return (
