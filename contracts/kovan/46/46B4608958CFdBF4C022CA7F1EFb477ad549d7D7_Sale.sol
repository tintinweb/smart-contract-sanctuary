...
contract Sale is ReentrancyGuard, ERC721TokenReceiver {
    ...
    function mintBlindGift(address _to, uint256 _total) public virtual hasAccess returns(bool){
    ...
    function buyWithEth(uint256 _punks) public virtual payable nonReentrant returns(bool) {
        require(resolveToken("ETH",_punks,18) <= msg.value,"Underflow Error");
    ...
    function buyWithBnB(uint256 _amount, uint256 _punks) public virtual nonReentrant returns(bool){
        require(resolveToken("BNB", _punks , 18) <= _amount, "Underflow Error");
        require(_punks <= limit,"Max Mint Limit Per Transaction");
    ...
    function resolveToken(string memory _token, uint256 totalPunks ,uint256 _decimal) public virtual view returns(uint256){
