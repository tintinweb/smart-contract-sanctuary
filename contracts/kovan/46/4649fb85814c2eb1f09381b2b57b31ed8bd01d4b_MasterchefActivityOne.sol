...
contract MasterchefActivityOne is Ownable, ActivityBase{
    ...
    function getPoolInfo(uint256 _pid) public view virtual returns(uint256 _allocPoint, uint256 _accumulativeDividend, uint256 _usersTotalWeight, uint256 _lpTokenAmount, uint256 _oracleWeight, address _swapAddress, uint256 _accs){
        PoolInfo storage pool = poolInfo[_pid];
    ...
    returns(uint256[] memory _allocPoint, uint256[] memory _accumulativeDividend, uint256[] memory _usersTotalWeight, uint256[] memory _lpTokenAmount, 
    uint256[] memory _oracleWeight, address[] memory _swapAddress, uint256[] memory _accs){
        ...
        for(uint i = _fromIndex; i <= _toIndex; i ++){
            ...
            _swapAddress[startIndex] = address(pool.lpTokenSwap);
