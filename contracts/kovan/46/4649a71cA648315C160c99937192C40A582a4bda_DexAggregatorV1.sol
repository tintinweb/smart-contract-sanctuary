...
interface DexAggregatorInterface {
    ...
    function getAvgPrice(address desToken, address quoteToken, uint32 timeFrame, bytes memory data) external view returns (uint256 price, uint8 decimals);
}
...
contract DexAggregatorV1 is DexAggregatorInterface, UniV2Dex, UniV3Dex {
    ...
    function calBuyAmount(address buyToken, address sellToken, uint sellAmount, bytes memory data) external view override returns (uint buyAmount) {
        if (data.toDex() == DexData.DEX_UNIV2) {
            ...
            buyAmount = uniV3CalBuyAmount(buyToken, sellToken, sellAmount, data.toFee());
        }
        else {
            require(false, 'Unsupported dex');
        }
    ...
    function getAvgPrice(address desToken, address quoteToken, uint32 timeFrame, bytes memory data) external view override returns (uint256 price, uint8 decimals){
        decimals = priceDecimals;
        if (data.toDex() == DexData.DEX_UNIV3) {
            price = uniV3GetAvgPrice(desToken, quoteToken, timeFrame, decimals, data.toFee());
        }
        else {
            require(false, 'Unsupported dex');
        }
...
contract UniV2Dex {
    ...
    function uniV2Sell(address buyToken, address sellToken, uint sellAmount, uint minBuyAmount) internal returns (uint buyAmount){
        ...
        if (isToken0) {
            ...
            IERC20(sellToken).transferFrom(payer, pair, sellAmount);
...
contract UniV3Dex is IUniswapV3SwapCallback {
    ...
    function uniV3Sell(address buyToken, address sellToken, uint sellAmount, uint minBuyAmount, uint24 fee) internal returns (uint amountOut){
        ...
        getPool(data.tokenIn, data.tokenOut, fee).swap(
            data.payer,
            zeroForOne,
            sellAmount.toInt256(),
    ...
    function uniV3CalBuyAmount(address buyToken, address sellToken, uint sellAmount, uint24 fee) internal view returns (uint) {
    ...
    function uniV3GetPrice(address desToken, address quoteToken, uint8 decimals, uint24 fee) internal view returns (uint256){
        IUniswapV3Pool pool = getPool(desToken, quoteToken, fee);
        (uint160 sqrtPriceX96,,,,,,) = pool.slot0();
        return getPriceBySqrtPriceX96(desToken, quoteToken, sqrtPriceX96, decimals);
    }

    function uniV3GetAvgPrice(address desToken, address quoteToken, uint32 timeFrame, uint8 decimals, uint24 fee) internal view returns (uint256){
...
library DexData {
    ...
    uint constant DEX_UNIV2 = 0;
    uint constant DEX_UNIV3 = 1;
