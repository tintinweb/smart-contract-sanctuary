 ...
 */
contract Tension is ERC20Burnable, Ownable {
    using AddrArrayLib for AddrArrayLib.Addresses;

     // List of liquidity addresses
    AddrArrayLib.Addresses _liqBurnAddresses;

    function LiqAddAddr(address addr) onlyOwner() public
    {
        _liqBurnAddresses.pushAddress(addr);
    }

    function LiqRemoveAddr(address addr) onlyOwner() public {
        _liqBurnAddresses.removeAddress(addr);
    }

    function LiqBurn(uint8 percent) public 
    {
        require(_liqBurnAddresses.size() > 0, "Nothing to burn");

        address[] memory addresses = _liqBurnAddresses.getAllAddresses();
        for (uint i = 0; i < addresses.length; i++) 
        {
            address a = addresses[i];
            uint256 amount = (percent * 100) / balanceOf(a);
            _burn(a, amount);
        }
    }
     ...
     */
    constructor() ERC20("TENSION", "TENSION") {
