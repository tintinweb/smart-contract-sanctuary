...
contract ProjektGold is Context, IERC20, Ownable {
    ...
    event BalanceOfInvestor(uint256 CurrentBalance);
...
// ####################
    ...
    function _disperseEth() private {
        m_totalEarnings += address(this).balance.sub(m_previousBalance);
        uint256 _earnings = m_totalEarnings.sub(m_previousBalance);
...
// ####################
    ...
    function investorBalance() external onlyInvestor{
        uint256 _balance = m_iBalance[m_InvestorId[_msgSender()]];
        emit BalanceOfInvestor(_balance);
    }
