...
contract GLX is IBEP20, AdminRole, Pausable {
...
contract Staking {
        ...
        returns (bool)
    {
        ...
        if (block.timestamp > stakingEnds) {
            if (firstWithdraw) {
                reflectedFees = GLX(tokenAddress)
                    .balanceOf(address(this))
                    .sub(stakedBalance)
                    .sub(totalReward);
                firstWithdraw = false;
                GLX(tokenAddress).excludeFromReward(address(this));
            }
    ...
    function _payTo(
        ...
        GLXInterface = GLX(tokenAddress);
        return GLXInterface.transferFrom(allower, receiver, amount);
    }
    ...
    {
        GLXInterface = GLX(tokenAddress);
        return GLXInterface.transfer(to, amount);
    }
    ...
    modifier _hasAllowance(address allower, uint256 amount) {
        ...
        GLXInterface = GLX(tokenAddress);
        uint256 ourAllowance = GLXInterface.allowance(allower, address(this));
        require(
            amount <= ourAllowance,
