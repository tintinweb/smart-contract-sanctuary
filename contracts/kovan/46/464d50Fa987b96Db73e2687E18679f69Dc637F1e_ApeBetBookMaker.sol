...
contract ApeBetBookMaker is Ownable {
    ...
    modifier hasEnoughBalance(
        ...
        for (uint256 i = 0; i < _bet_event_ids.length; i++) {
            maxPayout = maxPayout
                .add(getRewardEst(_bet_event_ids[i], winners[i], amounts[i]))
                .sub(amounts[i]);
        }
        require(maxPayout < maxPoolReward, "Not enough eth to reward your bet");
        _;
    }
    ...
    function changeBetPool(address payable _betPoolAddress) external onlyOwner {
        ...
        maxPoolReward = address(betPool).balance;
    }
    ...
    function getTotalRewards(bytes16[] memory _bet_event_ids) external {
        ...
        for (uint256 i = 0; i < _bet_event_ids.length; i++) {
            ...
            if (ev.event_status == 2 && ev.winner == bet.bet_winner) {
                bet.bet_reward = bet.bet_amount.mul(odd).div(1e4);
                totalRewards = totalRewards.add(bet.bet_reward);
    ...
    function updateMaxPoolReward(uint256 amount) external {
        maxPoolReward = maxPoolReward.add(amount);
    }
        ...
        hasEnoughBalance(_bet_event_ids, _bet_winners, _bet_amounts)
    {
        for (uint256 i = 0; i < _bet_event_ids.length; i++) {
            ...
            maxPoolReward = maxPoolReward.add(_bet_amounts[i]).sub(
                getRewardEst(
                    _bet_event_ids[i],
                    _bet_winners[i],
                    _bet_amounts[i]
