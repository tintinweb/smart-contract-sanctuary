...
interface IPrecog_ {
    ...
    function invest(
        ...
        uint256 platformIndex,
        uint256 borrowAmount
    ) external view  returns (uint tradeAmount, uint calculatedTradeAmount);
}
...
contract Precog is IPrecog {
    ...
    ) internal view returns (uint256 profit, bool canBorrow, uint256 expiryDate, uint debtAmount) {
                    ...
                    if (loan.platformIndex > 0) {
                        ...
                        if (userBorrowInfo[user][tradeInfo.futureContract].length == 0) {
                            if (tradeAmount > fee.feeLending) {
                                ...
                                return (profit, false, expiryDate, tradeAmount);
                            }
                        }
                        revenue = _getRevenue(tradeAmount, expiryDate, pairs, tradeInfo);
                        debtAmount = _getDebtAmount(loan, block.timestamp, expiryDate);
    ...
    function invest(
        ...
        uint256 platformIndex,
        uint256 borrowAmount
    ) external returns (uint debtAmount, uint calculatedDebtAmount) {
        ...
        (uint profit, bool canBorrow, uint expiryDate, uint calDebtAmount) = _calculateProfit(user, amount, fee, tradeInfo, loan);
        calculatedDebtAmount = calDebtAmount;
