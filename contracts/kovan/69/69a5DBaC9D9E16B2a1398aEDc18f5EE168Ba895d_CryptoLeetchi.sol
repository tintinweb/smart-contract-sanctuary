...
contract Cagnotte {
    ...
    function getDetails() public view returns(address, address payable, bytes32, uint, uint256, uint) {
...
contract CryptoLeetchi {
    ...
    event cagnotteStarted(address _contractAddress, address _cagnotteOwner, bytes32 _multihash, uint256 _targetDate, uint256 _amountToRaise);

    mapping(bytes32 => Cagnotte) public ipfsCagnotte;

    function startCagnotte(address payable _receiver, bytes32 _multihash, uint _targetDate, uint _targetAmount) external returns (Cagnotte) {
        uint targetTime = block.timestamp + _targetDate;
        // bytes32 hash = bytes32(_multihash);
