...
contract SurveyTokenFaucet{
    ...
    /**
     *   Allows to request up to one token per account and per token
    */
    function requestToken(address tokenContract) external {
        require(tokenAllowance[tokenContract][msg.sender] < 1, "You are only allowed to request one token per survey.");
        IERC20 token = IERC20(tokenContract);
        //require(token.balanceOf(address(this)) > 0, "The faucet for this token is empty.");
        token.transfer(msg.sender, 1);
        tokenAllowance[tokenContract][msg.sender] = 1;    // Update the allowance
    }
