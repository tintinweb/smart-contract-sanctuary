...
import "../interfaces/IMultipleTxnsExecutor.sol";

contract MultipleTxnsExecutor is IMultipleTxnsExecutor, AccessControl {
  constructor(address relayersManager_) {
    _setupRole("RelayersManager", relayersManager_);
    _setRoleAdmin("Relayer", "RelayersManager");
  }

  function executeMultiple(
    address refundAsset_,
    address refundOnBehalfOf_,
    TransactionImage[] calldata txns_
  ) external override {
    require(hasRole("Relayer", msg.sender), "MultipleTxnsExecutor.executeMultiple, not relayer");
    for (uint256 i = 0; i < txns_.length; i++) {
      txns_[i].target.relay(
        refundAsset_,
        refundOnBehalfOf_,
        txns_[i].transaction,
        txns_[i].v,
    ...
    if (refundAsset_ == address(0)) {
      payable(refundOnBehalfOf_).transfer(address(this).balance);
    } else {
      IERC20(refundAsset_).transfer(
        refundOnBehalfOf_,
        IERC20(refundAsset_).balanceOf(address(this))
      );
    }
  }
...
interface IMultipleTxnsExecutor {
