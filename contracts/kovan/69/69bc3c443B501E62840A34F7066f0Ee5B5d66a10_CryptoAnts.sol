...
interface ICryptoAnts is IERC721 {
  ...
  event LayingEggsRandomlyInProgress(bytes32 indexed requestId, address indexed layer);
  event LayedEggsRandomly(bytes32 indexed requestId, uint256 indexed result);
...
contract CryptoAnts is ERC721, ICryptoAnts, VRFConsumerBase, Ownable {
  ...
  uint256 public lastEggMintBlock = 0;

  // chainlink Variables 
  bytes32 internal _keyHash;
  uint256 internal _fee;
  uint256 public randomResult;
  mapping(address => uint256) private _randomEggsResults;
  mapping(bytes32 => address) private _rollers;


  constructor(address _eggs) ERC721('Crypto Ants', 'ANTS') VRFConsumerBase(
  ...
  function layEggs(uint256 _amount) external{
    // checks and reverts if a mint happened in the last 10 minutes
    if (lastEggMintBlock - block.number < 46) revert MintTimeLimit();

    eggs.mint(address(this), _amount);
    lastEggMintBlock = block.number;
  }

  function layEggsRandomly() external returns (bytes32 requestId) { 
        // checking LINK balance
        require(LINK.balanceOf(address(this)) >= _fee, 'Not enough LINK to pay fee');

        // checking if msg.sender has already tried to lay eggs
        require(_randomEggsResults[msg.sender] == 0, 'Already laying');

        // requesting randomness
        requestId = requestRandomness(_keyHash, _fee);

        // storing requestId and msg.sender address
        _rollers[requestId] = msg.sender;

        // emitting event to signal rolling of die
        _randomEggsResults[msg.sender] = 0;
        emit LayingEggsRandomlyInProgress(requestId, msg.sender);
  }

  //solhint-disable-next-line
  function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
    uint256 _amount = randomness.mod(20).add(1);
    eggs.mint(address(this), _amount);
    _randomEggsResults[_rollers[requestId]] = _amount;
    emit LayedEggsRandomly(requestId, _amount);
