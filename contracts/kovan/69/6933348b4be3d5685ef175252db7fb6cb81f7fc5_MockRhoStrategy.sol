...
contract MockRhoStrategy is IRhoStrategy, PausableUpgradeable, AccessControlEnumerableUpgradeable {
    ...
    uint256 private _newEffectiveSupplyRate;
    uint256 private _balanceOfUnderlying;
    uint8 constant private _lastDeployGasFee = 0;
    uint8 constant private _lastWithdrawGasFee = 1;
    ...
    function balanceOfUnderlying() external override view returns (uint256) {
        return underlying.balanceOf(address(this));
    }
    function updateBalanceOfUnderlying() external override view returns (uint256) {
        return underlying.balanceOf(address(this));
    }
    function swapToken(uint256 amountIn, uint256 amountOutMin, address[] memory path, uint256 deadline) external {
    }
