...
library PackageLibrary {
	...
	*/
	...
	{
		if(!approve_){
			package_.budgetAllocated -= refund_;
	...
	*/
		...
		onlyExistingPackage(package_)
	{
		...
		if(budgetLeft_ != 0 && package_.totalCollaborators == 0)
...
library CollaboratorLibrary {
	...
	event AddedCollaborator(bytes32 indexed projectId, bytes32 indexed packageId, address indexed collaborator, uint256 mgp);
...
contract Packages is Collaborators {
	...
	*/
	function _approveCollaborator(
		...
		virtual
	{
		packageData[projectId_][packageId_]._approveCollaborator(approve_, refund_);
	}
	...
	*/
	function _finishPackage(
		...
		virtual
	{
		packageData[projectId_][packageId_]._finishPackage(projectId_, packageId_, budgetLeft_);
	}
...
contract Projects is Packages, Observers {
	...
	*/
		...
		override
	{
		...
		super._finishPackage(projectId_, packageId_, budgetLeft_);
	}

	/**
	* @dev Sends approved MGP to collaborator
	* @param projectId_ Id of the project
	* @param packageId_ Id of the package   
	*/
	function _getMgp(
	...
	/**
	* @dev Sends approved Bonus to collaborator
	* @param projectId_ Id of the project
	* @param packageId_ Id of the package   
	*/
	function _getBonus(
...
contract ReBakedDAO is Ownable, ReentrancyGuard, Projects {
	...
	*/
	function approveCollaborator(
		...
		onlyOwner
	{
		uint256 _mgp = collaboratorData[projectId_][packageId_][collaborator_].mgp; 
		_approveCollaborator(projectId_, packageId_, approve_, _mgp);
		super.approveCollaborator(projectId_, packageId_, collaborator_, approve_);
	}
	...
	*/
		...
		onlyInitiator(projectId_)
	{
		_finishPackage(
			projectId_, 
			packageId_,
			packageData[projectId_][packageId_].budget - packageData[projectId_][packageId_].budgetAllocated
		);
	}
