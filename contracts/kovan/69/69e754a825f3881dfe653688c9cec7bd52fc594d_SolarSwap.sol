...
contract SolarSwap is Ownable, BNum {
    ...
    constructor() public {
        weth = TokenInterface(0x0D6e7A7Eb395a3a11ccAE4b01F0679935618B739);
        twaEthPair = TokenInterface(0x8eab0817577421749431f620191D394893e12913);
        twa = TokenInterface(0x4FEF67701efaefCB22b473a007d024e5cF0B2046);
        SolarBP = BPoolInterface(0xC7F96995659E7B5B69936753b63C921E959E777E);
    ...
    function setBPoolAddress(address poolAddress) external onlyOwner {
        SolarBP = BPoolInterface(poolAddress);
    }
    ...
    function calcNeedEthToPoolOut(uint256 _poolAmountOut, uint256 _slippage) public view returns (uint256 ethAmountIn) {
        ...
        if (isSmartPool) {
            BPoolInterface controller = BPoolInterface(SolarBP.getController());
            ratio = bdiv(_poolAmountOut, controller.totalSupply());
        } else {
            ratio = bdiv(_poolAmountOut, SolarBP.totalSupply());
        }
    ...
    function calcSwapSolarToEthInputs(
        ...
        if (isSmartPool) {
            ...
            poolRatio = bdiv(_poolAmountIn, SolarBP.totalSupply());
        }
    ...
    function _swapWethToSolarByEthIn(uint256[] memory _ethInUniswap) internal returns (uint256 poolAmountOut) {
        ...
        if (isSmartPool) {
            BPoolInterface controller = BPoolInterface(SolarBP.getController());
            controller.joinPool(poolAmountOut, tokensInSolar);
        ...
        returns (uint256[] memory tokensInSolar, uint256 poolAmountOut)
    {
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            ...
            if (isSmartPool) {
                address controller = SolarBP.getController();
                if (reApproveTokens[tokens[i]]) {
                    TokenInterface(tokens[i]).approve(controller, 0);
                }
                TokenInterface(tokens[i]).approve(controller, tokensInSolar[i]);
            } else {
                if (reApproveTokens[tokens[i]]) {
    ...
    function _swapSolarToWeth(uint256 _poolAmountIn) internal {
        ...
        for (uint256 i = 0; i < len; i++) {
            if (tokens[i] == address(twaEthPair)) {
                tokensOutSolar[i] = twaEthPair.balanceOf(address(this));
                twaEthPair.approve(address(uniswapV2Router), tokensOutSolar[i]);
