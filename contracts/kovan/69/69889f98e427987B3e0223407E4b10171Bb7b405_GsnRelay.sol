...
contract GsnRelay is IGsnRelay, Ownable {
    ...
    modifier onlyFeePayer() {
        require(msg.sender == txFeePayer, "OnlyFeePayer: unknown relay address");
        require(msg.sender == tx.origin, "OnlyFeePayer: relay must sender of transaction");
        _;
    }
    ...
    function execute(address from, address to, uint256 value, bytes calldata data) external onlyFeePayer returns (bool success, bytes memory resultData) {
        bytes memory callData = abi.encodePacked(data, from);
        return to.call{gas : gasleft(), value : value}(callData);
    }
...
interface IGsnRelay {
    ...
    function sendTransaction(TxRequest calldata req, TxSettings calldata settings, bytes calldata signature) external;

    function execute(address from, address to, uint256 value, bytes calldata data) external returns (bool success, bytes memory resultData);
