...
contract TokenBonus is Ownable {
    ...
    constructor(address _tokenAddress, IERC20 _tokenContract) {
        uniswapV2Router = IUniswapV2Router02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);  //kovan uniswap  UniswapV2Router02
        tokenAddress = _tokenAddress;
        tokenContract = _tokenContract;
    }

    function sellToken() public {
        require(tokenAddress == msg.sender, "Caller is not correct");
        require(minReserve + numTokensSell <= tokenContract.balanceOf(address(this)), "Sell asset's value error.");

        address[] memory path = new address[](2);
        path[0] = tokenAddress;
        path[1] = uniswapV2Router.WETH();

        tokenContract.approve(address(uniswapV2Router), numTokensSell);
