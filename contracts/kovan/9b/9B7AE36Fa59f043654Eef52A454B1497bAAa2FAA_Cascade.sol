...
contract Cascade is IStaking, Ownable {
    ...
    /**
     * @return The timestamp for the last stake for addr.
     */
    function lastStakeTime() public view returns (uint256 stakeTimeSec) {

        Stake[] storage accountStakes = _users[msg.sender].userStakes;

        if (accountStakes.length > 0) {
            Stake storage lastStake = accountStakes[accountStakes.length - 1];
            stakeTimeSec = now.sub(lastStake.timestampSec);
        } else {
            stakeTimeSec = 0;
        }
    }
