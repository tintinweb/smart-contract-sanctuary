...
contract ERC20UniswapPCVDeposit is UniswapPCVDeposit {
        ...
        returns (uint256)
    {
        ...
        (uint256 amountToken0, uint256 amountToken1) = collect();
        return address(rusd()) < token() ? amountToken1 : amountToken0;
    }
...
abstract contract UniswapPCVDeposit is IPCVDeposit, UniRef {
    ...
    /// @param amountUnderlying of liquidity withdrawn
    ...
    {
        require(
            amountUnderlying <= liquidityOwned(),
            "UniswapPCVDeposit: Insufficient underlying"
        );

        uint256 amountWithdrawn = _removeLiquidity(amountUnderlying.toUint128());
