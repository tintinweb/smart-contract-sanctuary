...
contract Staking is ReentrancyGuard, Context, IStaking {
    ...
    function initialize(address token) external override onlyFactory {
     ...
     * @param sender: address of staker
     * @return isNewStake true if it is new stake
     */
    function deposit(uint256 amount, address sender)
        external
        override
        onlyFactory
        returns (bool isNewStake)
    {
        require(amount > 0, "Input error");
        if (users[sender].amountStaked == 0) {
            isNewStake = true;
        }

        users[sender].lastRewardId = _upgradeReward(sender);
        users[sender].amountStaked += amount;
        users[sender].lastTimestamp = block.timestamp;
        totalStaked += amount;
    }
    ...
    /**
     * @notice Manual add reward
     * @dev only factory can call
     * @param amountReward: number of tokens
     */
    function addReward(uint256 amountReward) external override onlyFactory {
        require(totalStaked != 0, "There are no user in stake");
        rewards.push(RewardInfo(amountReward, totalStaked, block.timestamp));
    }

    /**
     * @notice Claim dividends by user address
     * @dev only factory can call
     * @param user: address of user
     */
    function claimDividends(address user) external override onlyFactory returns (uint256) {
        users[user].lastRewardId = _upgradeReward(user);
        uint256 dividends = users[user].amountOfDividends;
        users[user].amountOfDividends = 0;
        return dividends;
    }

    /**
     * @notice Get available dividends by user address
     * @dev only factory can call
     * @param user: address of user
     */
    function availDividends(address user) external view override onlyFactory returns (uint256) {
        UserInfo memory info = users[user];
        uint256 i = info.lastRewardId;
        uint256 total;
        for (; i < rewards.length; i++) {
            if (
                rewards[i].timestamp < info.lastTimestamp ||
                rewards[i].timestamp - info.lastTimestamp > IStakingFactory(FACTORY).getFreezeTime()
            ) {
                total += (info.amountStaked * rewards[i].amountReward) / rewards[i].totalStaked;
            }
        }

        return total + info.amountOfDividends;
    }
     ...
     * @param user: address of user
     */
    function getUserBalance(address user) external view override onlyFactory returns (uint256) {
        return users[user].amountStaked;
    }

    /**
     * @notice Upgrade reward
     * @dev count that reward was after then freeze time otherwise user cant receive
     * @param user: address of user
     */
    function _upgradeReward(address user) private returns (uint256) {
        UserInfo memory info = users[user];
        uint256 i = info.lastRewardId;
        for (; i < rewards.length; i++) {
            if (
                ...
                info.amountOfDividends +=
                    (info.amountStaked * rewards[i].amountReward) /
                    rewards[i].totalStaked;
            }
        }
        users[user] = info;
