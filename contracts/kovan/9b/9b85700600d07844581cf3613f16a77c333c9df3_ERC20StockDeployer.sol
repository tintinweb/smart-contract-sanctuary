...
// File: contracts/Deployers/ERC20StockDeployer.sol

pragma solidity 0.8.6;



contract ERC20StockDeployer is AccessControl { 
    
    bytes32 public constant MODERATOR_ROLE = keccak256("MODERATOR_ROLE");

    CompanyRegistryInterface private _companyRegistryInstance;
    
    event ERC20StockDeployed(address indexed tokenAddress);
    ...
    // MODERATOR_ROLE
    //
    function deployNewERC20Stock(address defaultAdminAddress, uint companyId, string memory name, string memory symbol) public returns (address) {
        require (hasRole(MODERATOR_ROLE, msg.sender), "Sender does not have MODERATOR_ROLE!");
        
        address newContractAddress = address(new ERC20StockContract(defaultAdminAddress, name, symbol, address(_companyRegistryInstance)));
        _companyRegistryInstance.addNewCompanyStock(companyId, newContractAddress);
        
        emit ERC20StockDeployed(newContractAddress);
    ...
    function sanityCheck() public view returns (bool companyRegistrySet, bool registryPermissionSet) {
        ...
        bool registryPermissionSetB = (_companyRegistryInstance.hasRole(keccak256("TOKEN_MANAGER"), address(this)));
