    ...
    ReentrancyGuardUpgradeable
{
    ...
    // Address from which to fetch external interest rates.
    // This currently is the MakerDAO Multi-Collateral-Dai (MCD) Jug contract,
    // but could be an IAggregatorInterface in the future if loading an average
    // interest rate from different protocols other than just DAI.
    address public interestRatesDataSource;
    // Time at which the interest rate was last automatically updated using the
    // configured data source.
    uint256 public interestRateLastUpdated;
    // Struct containing external protocol data needed to fetch info from such.
    // Will not be needed if maintaining an aggregator interface contract
    // in the future.
    mapping(address => ExternalAssetData) externalAssetData;
    // Maximum interest ratio per 1,000 that can be set from an external source.
    ...
    /**
     * @notice attempts to trigger an interest rate update according to the
               currently configured data source.
       @dev it is important that this function does not revert as it is called
    ...
    /**
     * @notice updates the interest rate via the data source
     */
    function updateRates() public override {
    ...
    /**
     * @notice fetches the interest rate for a token directly from the data source.
     ...
     * @dev the current implementation simply reads the interest rate for
    ...
    function fetchRate(address token) public view override returns (uint256) {
        ...
        uint256 stabilityPerSecond =
            jug.base().add(jug.ilks(data.makerDaoCollateralIlk).duty).sub(
                10**27
            ); // subtract one unit from the rate
        ...
        uint256 rate = (stabilityPerSecond * N).mul(1000).div(10**27);
    ...
    /**
     * @notice writes the current R value to storage. Only needed before
    ...
    /**
     * @notice returns the stored R value plus the delta cumulative rate
