...
contract Exchange is ReentrancyGuard, Ownable {
    ...
    */
    enum ReferralSide { Protocol, Relayer, Seller }
     ...
     */
    function setRelayer(address relayer, uint256 relayerFee, uint256 protocolFee, uint256 referralFee, ReferralSide referralSide, bool referralEnabled, bool referralRegistrationEnabled, IAtomicMatchHandler handler) 
        public onlyOwner {
        Relayer memory relayerInfo = Relayer(relayer,relayerFee,protocolFee,referralFee,referralSide,referralEnabled,referralRegistrationEnabled,handler);
        ...
        returns (uint)
    {
        if(order.saleKind == SaleKind.DutchAuction)
            return calculateDutchAuctionCurrentPrice(order);
     ...
     */
    function atomicMatch(Order memory buy, Sig memory buySig, Order memory sell, Sig memory sellSig, address referrer)
        external payable
        nonReentrant
    {
        /** Record Referral */
        ...
        returns (uint)
    {
        ...
        uint totalFee = chargeFee(sell, price, buy.maker);
     ...
     */
    function chargeFee(Order memory sellOrder, uint256 amount, address buyer)
        internal
        returns (uint256)
    {
        uint256 totalFee;
        ...
        if(relayerInfo.protocolFee != 0){
            ...
            /** Token as payment method */
                ...
                IERC20(sellOrder.paymentToken).safeTransferFrom(buyer, protocolFeeRecipient, feeAmount);
        }
        ...
        if(relayerInfo.relayerFee != 0 && tokenDiscount != 10000){
            ...
            /** Token as payment method */
                ...
                IERC20(sellOrder.paymentToken).safeTransferFrom(buyer, sellOrder.relayer, feeAmount);
        }
            ...
            if(recipients.length > 0){
                for (uint256 i = 0; i < recipients.length; i++) {
                    ...
                    if(sellOrder.paymentToken == address(0))
                        ...
                        IERC20(sellOrder.paymentToken).safeTransferFrom(buyer,recipients[i],fees[i]);
                }
            }
        }
        return totalFee;
    ...
    function payReferrerCommission(address seller, address buyer, uint256 amount, address paymentToken, Relayer memory relayerInfo) internal returns(uint256) {
        ...
        if(referrer != address(0)){
            if(paymentToken == address(0))
                payable(referrer).transfer(referrerCommission);
            else
                IERC20(paymentToken).safeTransferFrom(buyer, referrer, referrerCommission);
        }
...
interface IAtomicMatchHandler {
    function onAtomicMatch(address asset, uint256 assetId, uint256 assetAmount, address seller, address buyer, uint256 price) external;
}
