...
contract WebBasedNFTs is ERC721Enumerable, Ownable, ChainlinkClient {
    using Chainlink for Chainlink.Request;
    using Strings for uint256;
    string baseURI;
    uint256 DIV_MULTIPLIER_PER_CLICK = 0.001 ether;
    mapping(uint256 => metadata) private _tokenInfo;
    ...
    function mintAdToken(
        ...
        _tokenInfo[_adCode] = metadata(_adCode, adOwner, 0, 0, webURL);
        require(IPaymentSplitter(_paymentSplitter).addAdAccount{value: msg.value}(adOwner, _adCode) == _adCode,"Error in creating account in payments");
        _safeMint(adOwner, _adCode);
        return _adCode;
    }
