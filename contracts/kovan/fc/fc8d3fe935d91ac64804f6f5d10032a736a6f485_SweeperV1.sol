...
contract FluxMock is FluxWithdrawalInterface {
    address public token;
    mapping(address => uint256) public withdrawable;
    mapping(address => address) public admin;
    mapping(address => address) public proposedAdmin;

    constructor(address _token) {
        token = _token;
    }

    function withdrawPayment(address _oracle, address _recipient, uint256 _amount) override external {
        require(_amount <= withdrawable[_oracle], "too high amount");
        require(msg.sender == admin[_oracle], "not admin");
        IERC20(token).transfer(_recipient, _amount);
        withdrawable[_oracle] -= _amount;
    }

    function withdrawablePayment(address _oracle) override external view returns (uint256) {
        return withdrawable[_oracle];
    }

    function transferAdmin(address _oracle, address _newAdmin) override external {
        proposedAdmin[_oracle] = _newAdmin;
    }

    function inspectSender() override external view returns (address) {
        return msg.sender;
    }

    function acceptAdmin(address _oracle) override external {
        require(msg.sender == proposedAdmin[_oracle], "!proposedAdmin");
        delete proposedAdmin[_oracle];
        admin[_oracle] = msg.sender;
    }

    // Functions to help testing
    function addOracle(address _oracle, address _admin, uint256 _amount) public {
        withdrawable[_oracle] = _amount;
        admin[_oracle] = _admin;
    }
}
...
contract OffchainMock is OffchainWithdrawalInterface {
    address public token;
    mapping(address => uint256) public owed;
    mapping(address => address) public payee;
    mapping(address => address) public proposedPayee;
    ...
    view
    returns (uint256) {
        return owed[_transmitter];
    }

    function withdrawPayment(address _transmitter) override external {
        require(owed[_transmitter] > 0, "nothing to withdraw");
        require(msg.sender == payee[_transmitter], "not payee");
        IERC20(token).transfer(payee[_transmitter], owed[_transmitter]);
        owed[_transmitter] = 0;
    }

    function transferPayeeship(address _transmitter, address _proposed) override external {
        require(msg.sender == payee[_transmitter]);
        proposedPayee[_transmitter] = _proposed;
    }

    function acceptPayeeship(address _transmitter) override external {
        require(msg.sender == proposedPayee[_transmitter], "!proposedPayee");
        delete proposedPayee[_transmitter];
        payee[_transmitter] = msg.sender;
    }

    // Functions to help testing
    function addTransmitter(address _transmitter, address _payee, uint256 _amount) public {
        owed[_transmitter] = _amount;
        payee[_transmitter] = _payee;
    }
}
...
contract KeeperMock is KeeperWithdrawalInterface {
    address public token;
    ...
    returns (address, bool, uint96) {
        return (payee[_keeper], true, balance[_keeper]);
    }

    function withdrawPayment(address _keeper, address _to) override external {
        require(balance[_keeper] > 0, "nothing to withdraw");
        require(msg.sender == payee[_keeper], "not payee");
        IERC20(token).transfer(_to, balance[_keeper]);
        balance[_keeper] = 0;
    }

    function transferPayeeship(address _keeper, address _proposed) override external {
        require(msg.sender == payee[_keeper]);
        proposedPayee[_keeper] = _proposed;
    }

    function acceptPayeeship(address _keeper) override external {
        require(msg.sender == proposedPayee[_keeper], "!proposedPayee");
        delete proposedPayee[_keeper];
        payee[_keeper] = msg.sender;
    }

    // Functions to help testing
    function addKeeper(address _keeper, address _payee, uint96 _amount) public {
        balance[_keeper] = _amount;
        payee[_keeper] = _payee;
    }
}
