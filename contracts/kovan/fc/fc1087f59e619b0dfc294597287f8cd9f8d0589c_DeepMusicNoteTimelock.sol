...
contract DeepMusicNoteTimelock is Initializable, OwnableUpgradeable {
     ...
     */
    function beneficiary(address query) public view returns (address) {
        return _beneficiary[query];
    }
    ...
    function add_beneficiary(
        ...
        require(
            block.timestamp <= releaseTime_,
            "TokenTimelock: current time is before release time"
        );
        _beneficiary[msg.sender] = beneficiary_;
        _releaseTime[msg.sender] = releaseTime_;
        _amount[msg.sender] = amount_;
    }
    ...
    function release() public {
        ...
        token().safeTransfer(beneficiary(msg.sender), amount_to_send);
    }
