...
abstract contract LeverageAcc is RoleAware, PriceMath {
    ...
    function borrow(
        ...
        if (!hasBorrowedToken(account, borrowToken)) {
            ...
            (uint256 oldBorrowed, uint256 accumulatorFP) =
                Lend(lending()).applyBorrowInterest(
    ...
    function yieldTokenInPeg(
        ...
        uint256 yieldFP =
            Lend(lending()).viewAccumulatedBorrowingYieldFP(token);
        // 1 * FP / FP = 1
        ...
        for (
            ...
            if (belowMaintenanceThreshold(account)) {
                ...
                for (uint256 buyIdx = 0; len > buyIdx; buyIdx++) {
                    ...
                    Lend(lending()).payOff(token, loanAmount);
...
contract LeverageTrading is LeverageMarginCall, IMarginTrading {
    ...
    function registerRawBorrow(
        ...
        account.borrowed[borrowToken] = borrowAmount;
        Lend(lending()).registerBorrow(borrowToken, borrowAmount);
    }
    ...
    function registerLiquidation(address trader) external override {
        require(isLeverageTrader(msg.sender), "Calling contr. not authorized");
        CrossMarginAccount storage account = marginAccounts[trader];
        require(loanInPeg(account) == 0, "Can't release: borrowing");
        ...
        returns (address[] memory borrowTokens, uint256[] memory borrowAmounts)
    {
        ...
        for (uint256 idx = 0; borrowTokens.length > idx; idx++) {
            ...
            borrowAmounts[idx] = Lend(lending()).viewWithBorrowInterest(
...
contract VaultRouter is RoleAware, BaseRouter {
        ...
        external
    {
        ...
        if (extinguishAmount > 0) {
            Lend(lending()).payOff(depositToken, extinguishAmount);
    ...
    function crossDepositETH() external payable {
        ...
        if (extinguishAmount > 0) {
            Lend(lending()).payOff(WETH, extinguishAmount);
            IncentiveReporter.subtractFromClaimAmount(
                WETH,
                msg.sender,
                extinguishAmount
    ...
    function crossWithdraw(address withdrawToken, uint256 withdrawAmount)
        external
    {
        IMarginTrading(leverageTrading()).registerWithdrawal(
            msg.sender,
            withdrawToken,
            withdrawAmount
        );
    ...
    function crossBorrow(address borrowToken, uint256 borrowAmount) external {
        Lend(lending()).registerBorrow(borrowToken, borrowAmount);
        IMarginTrading(leverageTrading()).registerBorrow(
            msg.sender,
            borrowToken,
            borrowAmount
        );
    ...
    function crossOvercollateralizedBorrow(
        ...
        Lend(lending()).registerBorrow(borrowToken, withdrawAmount);
    ...
    function registerTrade(
        ...
        if (extinguishAmount > 0) {
            Lend(lending()).payOff(outToken, extinguishAmount);
        ...
        if (borrowAmount > 0) {
            Lend(lending()).registerBorrow(inToken, borrowAmount);
