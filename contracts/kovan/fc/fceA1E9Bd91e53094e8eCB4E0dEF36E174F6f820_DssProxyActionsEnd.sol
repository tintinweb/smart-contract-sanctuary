...
interface GemJoinLike {
    ...
    function gem() external returns (GemLike_7);
...
interface dotBtcJoinLike {
    function vat() external returns (VatLike_14);
    function dai() external returns (GemLike_7);
    function join(address, uint) external payable;
    function exit(address, uint) external;
}
...
contract DssProxyActions is Common {
    ...
    function _getDrawDart(
        ...
        uint dai = VatLike_14(vat).dai(urn);
    ...
    function _getWipeDart(
        ...
        (, uint rate,,,) = VatLike_14(vat).ilks(ilk);
        // Gets actual art value of the urn
        (, uint art) = VatLike_14(vat).urns(ilk, urn);
    ...
    function _getWipeAllWad(
        ...
        uint dai = VatLike_14(vat).dai(usr);
    ...
    function transfer(address gem, address dst, uint amt) public {
        GemLike_7(gem).transfer(dst, amt);
    }
    ...
    function lockETH(
        ...
        VatLike_14(ManagerLike(manager).vat()).frob(
    ...
    function draw(
        ...
        if (VatLike_14(vat).can(address(this), address(daiJoin)) == 0) {
            VatLike_14(vat).hope(daiJoin);
        }
    ...
    function wipe(
        ...
        if (own == address(this) || ManagerLike(manager).cdpCan(own, cdp, address(this)) == 1) {
            ...
            frob(manager, cdp, 0, _getWipeDart(vat, VatLike_14(vat).dai(urn), urn, ilk));
        } else {
             // Joins DAI amount into the vat
    ...
    function openLockGNTAndDraw(
        ...
        GemLike_7(GemJoinLike(gntJoin).gem()).transfer(bag, amtC);
    ...
    function wipeAndFreeETH(
        ...
        frob(
            ...
            cdp,
            -toInt(wadC),
            _getWipeDart(ManagerLike(manager).vat(), VatLike_14(ManagerLike(manager).vat()).dai(urn), urn, ManagerLike(manager).ilks(cdp))
        );
        // Moves the amount from the CDP urn to proxy's address
...
contract DssProxyActionsEnd is Common {
    ...
    function _free(
        ...
        bytes32 ilk = ManagerLike(manager).ilks(cdp);
        address urn = ManagerLike(manager).urns(cdp);
        VatLike_14 vat = VatLike_14(ManagerLike(manager).vat());
        uint art;
        (ink, art) = vat.urns(ilk, urn);

        // If CDP still has debt, it needs to be paid
        if (art > 0) {
            EndLike_3(end).skim(ilk, urn);
    ...
    function pack(
        ...
        daiJoin_join(daiJoin, address(this), wad);
        VatLike_14 vat = dotBtcJoinLike(daiJoin).vat();
    ...
    function cashETH(
        ...
        uint wadC = mul(wad, EndLike_3(end).fix(ilk)) / RAY;
        // Exits WETH amount to proxy address as a token
        GemJoinLike(ethJoin).exit(address(this), wadC);
        // Converts WETH to ETH
        GemJoinLike(ethJoin).gem().withdraw(wadC);
    ...
    function cashGem(
        ...
        EndLike_3(end).cash(ilk, wad);
        // Exits token amount to the user's wallet as a token
        uint amt = mul(wad, EndLike_3(end).fix(ilk)) / RAY / 10 ** (18 - GemJoinLike(gemJoin).dec());
        GemJoinLike(gemJoin).exit(msg.sender, amt);
    }
}
...
contract DssProxyActionsDsr is Common {
    function join(
        ...
        PotLike_2(pot).join(mul(wad, RAY) / chi);
    }
    ...
    function exitAll(
        ...
        uint pie = PotLike_2(pot).pie(address(this));
        // Exits DAI from the pot
        PotLike_2(pot).exit(pie);
        // Allows adapter to access to proxy's DAI balance in the vat
        if (vat.can(address(this), address(daiJoin)) == 0) {
