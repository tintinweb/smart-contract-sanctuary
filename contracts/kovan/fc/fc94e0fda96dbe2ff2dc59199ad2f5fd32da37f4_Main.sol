...
contract Main is ERC721Full {
    ...
    function createTicket(string memory _tokenURI) public payable returns (bool) {
        ...
        emit TicketMinted(msg.sender, tokenCount, _tokenURI, msg.value);
        return true;
    }

    function buyTicket(uint256 tokenId) public payable returns (bool) {
        require(_exists(tokenId), "Invalid tokenId");
        require(msg.value > 0, "Invalid fee");
        require(ownerOf(tokenId) != msg.sender, "Can't buy own ticket");
        require(!valid[msg.sender][tokenId], "Already bought this ticket");
        ...
        returns (bool)
    {
        ...
        for (uint256 i = 0; i < tokenId.length; i++) {
            ...
            require(ownerOf(tokenId[i]) != msg.sender, "Can't buy own ticket");
            require(!valid[msg.sender][tokenId[i]], "Already bought this ticket");
            ...
            emit TicketPurchased(msg.sender, tokenId[i], msg.value.div(tokenId.length));
        }
