...
contract Lottery is Ownable, VRFConsumerBase {
    ...
    struct LotteryInfo {
        uint256 _lotteryID; // ID for lotto
        uint256 _prizePool; // The amount of cake for prize money
        uint256 _costPerTicket; // Cost per ticket in $cake
        uint256 _startingTimestamp; // Block timestamp for star of lotto
        uint256 _closingTimestamp; // Block timestamp for end of entries
        address _winner; // winner address
    }
    ...
    function enterLottery() public payable {
        ...
        require(msg.value >= costPerTicket);
        participants.push(payable(msg.sender));
        // 10% for admin
        uint256 _prizeAdmin = msg.value / 10;
        prizeAdmin += _prizeAdmin;
        // 90%
        prizePool += (msg.value - _prizeAdmin);
    }

    function closeLottery() public onlyOwner {
        require(lotteryStatus == Status.Open, "The lottery cannot end now!");
        lotteryStatus = Status.Closed;
        closingTimestamp = block.timestamp;
    }

    // add lottery informations
    function addLottery() private {
     ...
     * Requests randomness
     */
    function completeLottery() public onlyOwner {
     ...
     *
     * recieve random number - pick winner
     */
    function fulfillRandomness(bytes32 requestId, uint256 randomness)
        internal
        override
