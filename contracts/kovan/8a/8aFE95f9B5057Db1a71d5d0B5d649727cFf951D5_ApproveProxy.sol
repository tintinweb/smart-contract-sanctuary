...
// File: localhost/mint/implement/ApproveProxy.sol
...
contract ApproveProxy is IApproveProxy, ConfigBase {
    
    using SafeERC20 for IERC20;
    
    event ClaimAccess(address indexed spender, bool state);
    
    mapping (address => bool) public claimAccess;
    
    constructor (IConfig _config) ConfigBase(_config) {
    }
    
    function setClaimAccess(address spender, bool state) override external onlyOwner {
        claimAccess[spender] = state;
        emit ClaimAccess(spender, state);
    }
    
    function claim(address token, address owner, address spender, uint256 amount) override external {
        require(claimAccess[msg.sender], "publics:not_claim_access");
        if (0 < amount) {
