...
contract DIAGenesisMinter is Ownable {
	...
	function addGenesisNFT(address newAddress) public onlyOwner {
		genesisNFTs.push(newAddress);
	}

	function removeGenesisNFT(address removedAddress) external onlyOwner {
		for(uint256 i = 0; i < genesisNFTs.length; i++) {
			if(genesisNFTs[i] == removedAddress) {
	...
	function mintGenesisDataNFT() external {
...
library NFTUtil {
	...
	function randInt(uint256 max, uint256 seed) external view returns (uint) {
...
contract DIADataNFT is Ownable, ERC1155 {
	...
	constructor(address _paymentToken, uint256 _burnAmount, uint256 _mintingPoolAmount, address _mintingPool, uint256 _seed) ERC1155("https://api.diadata.org/v1/drmnft/data_{id}.json") {
		...
		DIASourceNFTAddr = address(new DIASourceNFT(msg.sender, address(this)));
	...
	function mintDataNFT() external {
		require(!genesisPhase);
		require(started);
		_mintDataNFT();
	}

	function _mintDataNFT() public {
		...
		uint256 newRank = numMinted;

		// Check for randomization (<10) and get unfilled rank
		if (numMinted < NUM_PRIVILEGED_RANKS) {
			newRank = NFTUtil.randInt(NUM_PRIVILEGED_RANKS, seed);
			while (rankMinter[newRank] != address(0)) {
				newRank = (newRank + 1) % NUM_PRIVILEGED_RANKS;
			}
		}

		// Check that the wallet is still allowed to mint
		require(mintsPerWallet[msg.sender] < 3);

		// // Transfer payment token to burn address
		require(ERC20(paymentToken).transferFrom(msg.sender, burnAddress, burnAmount), "Payment token transfer to burn address failed.");
		// // Transfer payment token to minting pool
		require(ERC20(paymentToken).transferFrom(msg.sender, mintingPool, mintingPoolAmount), "Payment token transfer to minting pool failed.");

		// Mint data NFT
		_mint(msg.sender, getErcRank(newRank), 1, "");

		// Update data struct
		rankMinter[newRank] = msg.sender;
		...
		require(ERC20(paymentToken).transferFrom(msg.sender, address(diaSourceNFTImpl), diaSourceNFTImpl.getSourcePoolAmount(currSourceNFTId)), "Payment token transfer to source pool failed.");

		mintsPerWallet[msg.sender] += 1;
		emit MintedDataNFT(msg.sender, numMinted, newRank);
	}
...
contract DIASourceNFT is Ownable, ERC1155 {
	...
	constructor(address newOwner, address _dataNFTContractAddress) ERC1155("https://api.diadata.org/v1/nft/source_{id}.json") {
		require(_dataNFTContractAddress != address(0), "_dataNFTContractAddress is 0.");
		transferOwnership(newOwner);

		dataNFTContractAddresses[_dataNFTContractAddress] = true;
	}
...

{
  ...
  "libraries": {
    "contracts/DIADataNFT.sol": {
      "NFTUtil": "0xc1f1adb5400069d08241b79c5e241d3d2c536988"
    }
  }
}