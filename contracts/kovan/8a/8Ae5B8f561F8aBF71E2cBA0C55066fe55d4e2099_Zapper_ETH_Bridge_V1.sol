...
// File: contracts/MaticBridge/Zapper_ETH_Bridge_V1.sol
...
contract Zapper_ETH_Bridge_V1 is ZapInBaseV1 {
    IUniswapV2Factory private constant quickswapFactory = IUniswapV2Factory(
        0x5757371414417b8C6CAad45bAeF941aBc7d3Ab32
    );
    IUniswapV2Router02 private constant quickswapRouter = IUniswapV2Router02(
    ...
    address
        private constant wmaticTokenAddress = 0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270;
    uint256
        ...
        if (fromTokenAddress == address(0)) {
            ...
            if (fromTokenAddress != wmaticTokenAddress) {
                if (_ToTokenContractAddress != wmaticTokenAddress) {
                    ...
                    address pairA = quickswapFactory.getPair(
                        fromTokenAddress,
                    ...
                    uint256 amtB = quickswapRouter.getAmountsOut(
                        tokens2Trade,
                        pathB
                    )[2];
    ...
    function _token2TokenSushi(
        ...
        if (fromTokenAddress == address(0)) {
            ...
            if (fromTokenAddress != wmaticTokenAddress) {
                if (_ToTokenContractAddress != wmaticTokenAddress) {
                    // check output via tokenA -> tokenB
                    address pairA = sushiswapFactory.getPair(
                        fromTokenAddress,
                        _ToTokenContractAddress
                    );
                    address[] memory pathA = new address[](2);
