...
contract ProsperSocial is Ownable, Initializable, ContextUpgradeSafe {
    ...
    event NewPool(uint id, address creator, address token, uint minBet, uint start, uint startPeriod, uint end, uint endPeriod, string poolQuestion, string[] poolOptions);
    ...
    function createPool(address token, uint minBet, uint start, uint startPeriod, uint end, uint endPeriod, string memory poolQuestion, string[] memory poolOptions) public {
        ...
        Pool memory newPool = Pool(msg.sender, token, poolQuestion, poolOptions.length, minBet, start, startPeriod, end, endPeriod);
        pools.push(newPool);
        ...
        emit NewPool(id, msg.sender, token, minBet, start, startPeriod, end, endPeriod, poolQuestion, poolOptions);
    }
