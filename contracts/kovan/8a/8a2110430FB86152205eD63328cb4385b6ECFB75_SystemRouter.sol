...
contract SystemRouter is OwnableUpgradeable, CacheResolver{
    ...
    address internal LiquidityReward_Token_ETH;
    
    function router_init(address _resolver) public initializer {
        __Ownable_init();
        _cacheInit(_resolver);
        LiquidityReward_Token_ETH = requireAndGetAddress(CONTRACT_LIQUIDITY_TOKEN_ETH);
    }
   ...
   function holderVaults(address account) public view returns (uint, uint) {
    ...
    function stake(uint256 amount) external returns (bool) {
        (bool success, ) = LiquidityReward_Token_ETH.call(abi.encodeWithSignature("stake(address,uint256)", msg.sender, amount));
    ...
    function stakewithpermit(
        ...
        (bool success, ) = LiquidityReward_Token_ETH.call(abi.encodeWithSignature("stake(address,uint256)", msg.sender, value));
        require(success, "fail to call");
        return success;
    }

    function withdraw(uint256 amount) external returns (bool) {
        (bool success, ) = LiquidityReward_Token_ETH.call(abi.encodeWithSignature("withdraw(address,uint256)", msg.sender, amount));
        require(success, "fail to call");
        return success;
    }

    function getReward() external returns (bool) {
        (bool success, ) = LiquidityReward_Token_ETH.call(abi.encodeWithSignature("getReward(address)", msg.sender));
        require(success, "fail to call");
        return success;
    }

    function exit() external returns (bool) {
        (bool success, ) = LiquidityReward_Token_ETH.call(abi.encodeWithSignature("exit(address)", msg.sender));
        require(success, "fail to call");
        return success;
    }
