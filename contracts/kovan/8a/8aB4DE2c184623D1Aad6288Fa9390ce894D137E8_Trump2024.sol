...
///https://trump2024token.net/
/// 
///https://t.me/trump2024tokenV2
...
contract Trump2024 is Context, IERC20, LockToken {
    using SafeMath for uint256;
    using Address for address;
    address payable public marketingWalletAddress = payable(0x404249207b5fD96F54D64593005bDaEe380238C0); // Marketing Wallet Address
    address payable public communityWalletAddress = payable(0x58E8C96A099F48Fcff7E15CD99ADd80148d577a8); // Community Wallet Address
    ...
    uint256 totalSwapableFee = _liquidityFee.add(_buybackFee).add(_marketingFee).add(_communityFee);

    uint256 _saleTaxFee = 3;
    uint256 _saleLiquidityFee = 8;
    uint256 _saleBuybackFee = 4;
    uint256 _saleMarketingFee = 3;
    uint256 _saleCommunityFee = 7;
    ...
    uint256 public communityTokensCollected = 0;
    ...
    {   
        ...
        uint256 walletsTotal = buybackTokensCollected.add(marketingTokensCollected).add(communityTokensCollected);

        uint256 ethForMarketing = newBalance.mul(marketingTokensCollected).div(walletsTotal);
        uint256 ethForCommunity = newBalance.mul(communityTokensCollected).div(walletsTotal);

        transferToAddressETH(marketingWalletAddress, ethForMarketing);
        transferToAddressETH(communityWalletAddress, ethForCommunity);

        liquidityTokensCollected = 0;
        buybackTokensCollected = 0;
        marketingTokensCollected = 0;
        communityTokensCollected = 0;
    ...
    function countUpFeeShare(uint256 amount) private
    {
        ...
        communityTokensCollected += amount.mul(_communityFee).div(100);
    }
    ...
    function restoreAllFee() private {
        ...
        totalSwapableFee = _liquidityFee.add(_buybackFee).add(_marketingFee).add(_communityFee);
    }
    ...
    function setSaleFee() private {
        ...
        _communityFee = _saleCommunityFee;
        totalSwapableFee = _liquidityFee.add(_buybackFee).add(_marketingFee).add(_communityFee);
    }
    ...
    function setAllFeePercent(uint256 taxFee, uint256 liquidityFee, uint256 buybackFee, uint256 marketingFee, uint256 communityFee) 
    external onlyOwner() 
    {
        _taxFee = taxFee;
    ...
    function setSaleFeePercent(uint256 taxFee, uint256 liquidityFee, uint256 buybackFee, uint256 marketingFee, uint256 communityFee) 
    external onlyOwner() 
    {
        _saleTaxFee = taxFee;
        ...
        _saleCommunityFee = communityFee;
    }
    ...
    function setCommunityWalletAddress(address _communitWallet) external onlyOwner() {
        communityWalletAddress = payable(_communitWallet);
    }
...
/////---BUYBACK----////    
    ...
    uint256 private minBalForBuyback = uint256(1 * 10**18);
    ...
    function setMinBalForBuyback(uint256 amount) public onlyOwner
    {
        minBalForBuyback = amount;
    }
