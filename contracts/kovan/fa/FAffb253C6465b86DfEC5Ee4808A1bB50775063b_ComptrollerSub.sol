 ...
 */
contract ComptrollerSub is ComptrollerV8Storage, ComptrollerErrorReporter, Exponential {
    ...
    event DistributedSupplierComp(CToken indexed cToken, address indexed supplier, uint256 compDelta, uint256 compSupplyIndex);

    /// @notice Emitted when COMP is distributed to a borrower
    event DistributedBorrowerComp(CToken indexed cToken, address indexed borrower, uint256 compDelta, uint256 compBorrowIndex);

    event NewCompAddress(address indexed comp, address indexed vault);
    ...
    function updateCompSupplyIndex(address cToken) internal {
        ...
        if (deltaBlocks > 0 && supplySpeed > 0) {
            ...
            uint compAccruedAmount = mul_(deltaBlocks, supplySpeed);
            Double memory ratio = supplyTokens > 0 ? fraction(compAccruedAmount, supplyTokens) : Double({mantissa: 0});
    ...
    function updateCompBorrowIndex(address cToken, Exp memory marketBorrowIndex) internal {
        ...
        if (deltaBlocks > 0 && borrowSpeed > 0) {
            ...
            uint compAccruedAmount = mul_(deltaBlocks, borrowSpeed);
            Double memory ratio = borrowAmount > 0 ? fraction(compAccruedAmount, borrowAmount) : Double({mantissa: 0});
     ...
     */
    function grantCompInternal(address user, uint256 userAccrued) internal returns (uint256) {
        if (userAccrued > 0) {
            IERC20 _comp = IERC20(comp);
            uint256 compRemaining = _comp.balanceOf(vault);
            uint256 allowance = _comp.allowance(vault, address(this));
            if (userAccrued <= compRemaining && userAccrued <= allowance) {
                _comp.safeTransferFrom(vault, user, userAccrued);
                return 0;
            }
        }
        return userAccrued;
    ...
    function setCompAddress(address _comp, address _vault) external {
        require(admin == msg.sender, "Restricted to the admin");
        require(_comp != address(0), "Comp address is invaild");
        require(_vault != address(0), "The vault address is invaild");

        comp = _comp;
        vault = _vault;
        emit NewCompAddress(comp, vault);
    }
}
