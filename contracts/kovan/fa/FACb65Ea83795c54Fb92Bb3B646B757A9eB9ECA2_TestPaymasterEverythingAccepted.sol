...
contract asd {
    ...
    event Gas(uint initialGas, uint gasPrice);
    event InitialGas(uint extgas, uint gasleft, uint diff);
    function testExtGas(uint extgas, string calldata buf) external returns (uint initialGas) {
       initialGas = gasleft();
       emit InitialGas(extgas, initialGas, extgas-initialGas);
    }

    function testgas() public returns (uint initialGas, uint gasPrice) {
        initialGas = gasleft();
    gasPrice = tx.gasprice;
        emit Gas(initialGas, gasPrice);
    }

    event Executed(bool success, bytes ret);
    function exec(address target,bytes calldata data) external returns (bool, bytes memory) {
