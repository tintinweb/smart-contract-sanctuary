...
contract UniV3Helper {
    ...
    function getData(IUniswapV3 pool, int24 startTick, int24 endTick) external view returns (Tick[] memory ticks) {
        
        int24 tickSpacing = pool.tickSpacing();
        
        int24[] memory initTicks = new int24[](uint(endTick - startTick));

        uint counter = 0;
        for (int24 tickNum = startTick; tickNum <= endTick; tickNum += 256) {
