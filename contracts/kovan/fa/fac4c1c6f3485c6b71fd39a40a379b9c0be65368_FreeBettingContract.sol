...
contract FreeBettingContract is AbstractBettingContract {
    using SafeERC20 for IERC20;

    uint256 public minEntrants;
    ...
    function setMinAndMaxEntrant(uint256 _minEntrant, uint256 _maxEntrant)
        ...
        onlyLock
    {
        require(_maxEntrant > 0 && _maxEntrant > minEntrants);
        maxEntrants = _maxEntrant;
        minEntrants = _minEntrant;
    }

    function setAward(uint256 _award) external onlyOwner onlyLock {
        require(_award > 0);
        upfrontLockedFunds = _award;
    }
    ...
    function distributeReward() external override onlyPriceClosed onlyOpen {
        ...
        uint256 reward = 0;
        if (winners.length >= minEntrants) {
            reward = upfrontLockedFunds / winners.length;
