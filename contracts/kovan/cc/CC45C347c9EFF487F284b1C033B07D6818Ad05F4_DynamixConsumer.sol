...
contract DynamixConsumer is ChainlinkClient {
    using Chainlink for Chainlink.Request;
    string private constant DYNAMIX_API_URL =
        "https://organicziplab.com/api/generatess";
    address private oracle;
    bytes32 private jobId;
    uint256 private fee;
    mapping(bytes32 => address) private requestIdToMinter;
    ...
    function mintNFT() external {
        bytes32 requestId = generateNFT();
        requestIdToMinter[requestId] = msg.sender;
    }

    function generateNFT() public returns (bytes32 requestId) {
        Chainlink.Request memory request = buildChainlinkRequest(
            jobId,
            address(this),
            this.fulfill.selector
        );
        request.add("get", DYNAMIX_API_URL);
        request.add("path", "URL");
    ...
    {
        address minter = requestIdToMinter[_requestId];
        // _mintDynamix(minter, _bytes32ToString(_response));
    }
...
// contract DynamixToken is ERC721URIStorage {
//     using Counters for Counters.Counter;

//     Counters.Counter private _tokenIds;
//     mapping(address => string[]) private addressToNFTs;

//     constructor() ERC721("DYNAMIX", "DYNM") {}

//     function _mintDynamix(address minter, string memory url) internal {
//         _tokenIds.increment();
...
//     // function myNFT() external view returns (string[] memory) {
