...
contract FAANGStrategyKovan {
    ...
    IERC20 mir = IERC20(0x09a3EcAFa817268f77BE1283176B946C4ff2E608);
    ICurveFi public curveFi = ICurveFi(0x890f4e345B1dAED0367A877a1612f86A1f86985f); 
    ...
    address private mirustPairAddress = 0x87dA823B6fC8EB8575a235A824690fda94674c88;
    address DAIToken = 0x6B175474E89094C44Da98b954EedeAC495271d0F;
    address mirUstPooltoken = 0x87dA823B6fC8EB8575a235A824690fda94674c88;
    ILPPool mirustPool;
    ...
    function deposit(uint256 _amount, IERC20 _token) external {
        ...
        amountDeposited = address(_token) == DAIToken ? amountDeposited.add(_amount) : amountDeposited.add(_amount.mul(1e12));
    ...
    function withdraw(uint256 _amount, IERC20 _token) external {
        ...
        for (uint256 i = 0; i < mAssets.length; i++) {
            ...
            uint amounOfLpTokenToRemove = getDataFromLPPool(address(mAssets[i].lpToken), _amount);
        ...
        uint amountDepositedAdjusted = amountDeposited < _amount ? amountDeposited : _amount;
        amountDeposited = amountDeposited.sub(amountDepositedAdjusted);

        withdrawFromMirUstPool(_amount);
        _token.safeTransfer(msg.sender, _token.balanceOf(address(this)));
    }

    function withdrawFromMirUstPool(uint _amount) internal {
        ...
        if(reInvestedMirUstPooltoken != 0) {  
            amountToWithdraw = reInvestedMirUstPooltoken.mul(_amount).div(amountDeposited);
     ...
     */
    function reBalance(uint[] memory weights) external {
    ...
    function withdrawAllFunds(IERC20 _tokenToConvert) external {
        require(msg.sender == vault, "only Vault");
        ...
        withdrawFromMirUstPool(amountDeposited);
    ...
    function getDataFromLPPool(address _lpToken, uint _amount) internal view returns (uint amounOfLpTokenToRemove){
        ...
        amounOfLpTokenToRemove = lpTokenBalance.mul(_amount).div(amountDeposited);
