...
interface IIntegralPair is IIntegralERC20, IReserves {
    event Mint(address indexed sender, uint256 liquidity, uint256 amount0, uint256 amount1, address indexed to);
...
contract IntegralPair is Reserves, IntegralERC20, IIntegralPair {
    ...
    function mint(address to) external override lock returns (uint256 liquidity) {
        ...
        emit Mint(msg.sender, effectiveLiquidity, amount0, amount1, to);
    }
    ...
    function burn(address to) external override lock returns (uint256 amount0, uint256 amount1) {
        ...
        emit Burn(msg.sender, effectiveLiquidity, amount0, amount1, to);
    }
    ...
    function swap(
        ...
        require(canTrade(msg.sender), 'IP_UNAUTHORIZED_TRADER');
        require(to != address(0), 'IP_ADDRESS_ZERO');
        require(data.length == 0, 'IP_FLASH_SWAP_UNSUPPORTED');
