 ...
 * 10% total tax on buys and sells
...
interface IUniswapV2Router02 {
    function swapExactETHForTokens(
        ...
        address to, 
        uint deadline
    ) external payable returns (uint[] memory amountToken);
...
contract BUYBACKV4 is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"BUYBACKV4";
    string private constant _symbol = unicode"BUYBACKV4";
    ...
    uint256 private maxHeldTokensEnd;
    struct User {
        uint256 buy;
        uint256 sell;
        bool exists;
    ...
    event MarketingWalletAddressUpdated(address _marketingwallet);
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from != owner() && to != owner()) {
            ...
            if(from == uniswapV2Pair && to != address(uniswapV2Router) && !_isExcludedFromFee[to]) {
                ...
                if(maxHeldTokensEnd > block.timestamp) {
                ...
                if(contractEthBalance > 0) {
                    buybackWithFee(contractEthBalance);
                }
            }
    ...
    function buybackWithFee(uint256 ethAmount) private lockTheSwap {
    ...
    function openTrading() public onlyOwner {
        ...
        maxHeldTokensEnd = block.timestamp + (5 minutes);
    }
    ...
    function updateFeeAddress(address newAddress) external {
        require(_msgSender() == _FeeAddress);
        _FeeAddress = payable(newAddress);
        emit FeeAddressUpdated(_FeeAddress);
    }

    function updateMarketingWalletAddress(address newAddress) external {
        require(_msgSender() == _marketingWalletAddress);
        _marketingWalletAddress = payable(newAddress);
        emit MarketingWalletAddressUpdated(_marketingWalletAddress);
    }
    ...
    function isThereMaxHeld() public view returns (bool) {
        return (block.timestamp > maxHeldTokensEnd);
    }
    ...
    function whatIsMarketingWallet() public view returns (address) {
        return _marketingWalletAddress;
    }

    function whatIsFeeWallet() public view returns (address) {
        return _FeeAddress;
    }
}