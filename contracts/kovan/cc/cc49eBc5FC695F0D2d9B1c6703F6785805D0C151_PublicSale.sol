...
contract PublicSale is Initializable, BumperAccessControl {
    ...
    mapping(address => bool) public privateSaleWhitelitedAddresses;
    ...
    modifier whenSaleIsActive(address sender) {
        ...
        if (
            ...
            if (!privateSaleWhitelitedAddresses[sender])
                revert("not in presale whitelist");
            else _;
        } else if (
            // otherwise check if regular sale period available
            block.timestamp >= salePeriod.saleStart &&
            block.timestamp < salePeriod.saleEnd
        ) {
            _;
            // else - sale is not active
        } else revert("sale is not active");
    }
    ...
    function initialize(
        ...
        address[] memory _privateSaleInitialWhitelist,
        ...
        for (uint256 i; i < _privateSaleInitialWhitelist.length; i++) {
            privateSaleWhitelitedAddresses[
                _privateSaleInitialWhitelist[i]
            ] = true;
        }
    ...
    function setStakingContractAddress(address _newStaking) external onlyGovernance { 
    ...
    function depositAmountWithPermit(
        ...
        bytes32 r,
        bytes32 s
    ) external whenSaleIsActive(msg.sender) {
    ...
    function depositAmount(uint256 _usdcAmount, uint256 _stakePeriodIndex, bool _isLocked)
        external
        whenSaleIsActive(msg.sender)
    {
    ...
    function _depositAmount(uint256 _usdcAmount, uint256 _stakePeriodIndex, bool _isLocked)
        private
    {
    ...
    function setPrivateSaleWhiteListAddresses(
        address[] calldata _whitelist,
        bool[] calldata _values
    ) external onlyGovernance {
        require(
            _whitelist.length == _values.length,
            "length are not identical"
        );
        for (uint256 i; i < _whitelist.length; i++)
            privateSaleWhitelitedAddresses[_whitelist[i]] = _values[i];
    }
...
contract StakeRewardFixed is ReentrancyGuardUpgradeable, PausableUpgradeable, BumperAccessControl {
  using SafeERC20Upgradeable for IERC20Upgradeable;

  mapping(address => StakeInfo[]) public usersStake; // users staking information
  ...
  address public publicSaleContactAddress;

  event Staked(uint256 indexed amount, uint256 indexed period, uint256 indexed futureRewards);
  event Unstaked(uint256 indexed amount);
  ...
  function initialize(
    ...
    stakeOptions[0] = StakeOption(14, 200);
    stakeOptions[1] = StakeOption(28, 500);
    stakeOptions[2] = StakeOption(56, 1000);
    stakeOptions[3] = StakeOption(84, 2000);
    stakeOptions[4] = StakeOption(180, 3000);

    console.log("Stake contract created for token: %s", _stakeToken);
  }

  /// -------------------  EXTERNAL, PUBLIC, VIEW, HELPERS  -------------------

  function calcUserStakeRewards(uint256 amount, uint256 bonusAPY) public pure returns (uint256 rewards) {
    rewards = (amount * bonusAPY) / 10000;
  }

  // returns how many tokens can owner withdraw
  function freeAmount() public view returns (uint256) {
    return IERC20Upgradeable(stakeToken).balanceOf(address(this)) - totalReservedAmount;
  }

  /// -------------------  EXTERNAL, PUBLIC, STATE CHANGE -------------------
  function stake(uint256 amount, uint16 periodIndex) external nonReentrant {
    require(stakeOptions[periodIndex].periodInDays > 0, "Incorrect period index");

    uint256 periodDays = stakeOptions[periodIndex].periodInDays;
    uint256 bonusPercentage = stakeOptions[periodIndex].bonusInPercentage;
    uint256 rewards = calcUserStakeRewards(amount, bonusPercentage);
    ...
    IERC20Upgradeable(stakeToken).safeTransferFrom(msg.sender, address(this), amount);

    totalReservedAmount += amount + rewards;

    StakeInfo memory newStake = StakeInfo({
      amountAndRewards: amount + rewards,
      end: block.timestamp + periodDays * 1 days
    });
    usersStake[msg.sender].push(newStake);
  ...
  function stakeFromPublicSale(
    ...
    require(msg.sender == publicSaleContactAddress, "Sender is not publicSale contract");
    require(stakeOptions[periodIndex].periodInDays > 0, "Incorrect period index");
  ...
  function unstake() external nonReentrant {
    ...
    for (uint256 i = 0; i < stakeInfoList.length; i++) {
      if (stakeInfoList[i].end != 0 && stakeInfoList[i].end < block.timestamp) {
        totalPurchaseAmount += stakeInfoList[i].amountAndRewards;
        stakeInfoList[i].end = 0;
        stakeInfoList[i].amountAndRewards = 0;
      }
    }
    require(totalPurchaseAmount > 0, "Staking pediods was not ended or havent stakes");

    IERC20Upgradeable(stakeToken).safeTransfer(msg.sender, totalPurchaseAmount);
    totalReservedAmount -= totalPurchaseAmount;

    console.log("User has been withdraw ", totalPurchaseAmount);

    emit Unstaked(totalPurchaseAmount);
  }

  /// ------------------- EXTERNAL OWNER FUNCTIONS -------------------
  function setStakeOptions(
    uint256[] memory _optionIndexes,
    uint16[] memory _periods,
    uint16[] memory _bonuses
  ) external onlyGovernance {
    require(
      _optionIndexes.length == _periods.length && _optionIndexes.length == _bonuses.length,
      "Values do not match, the length of the arrays must be the same"
    );
    for (uint256 i = 0; i < _optionIndexes.length; i++) {
      stakeOptions[i] = StakeOption(_periods[i], _bonuses[i]);
    }
  }

  function transferTokensToContractForRewards(uint256 _amount) external {
    require(_amount > 0, "Invalid amount");
    IERC20Upgradeable(stakeToken).safeTransferFrom(_msgSender(), address(this), _amount);
  }
