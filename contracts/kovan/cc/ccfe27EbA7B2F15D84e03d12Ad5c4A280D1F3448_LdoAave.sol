    ...
    ILdoAave
{
    ...
    function initialize(
        ...
        __FeeDistributor_init(_aave, _priceFeed, _lidoAgent, _delphiAgent, _developer);
 ...
 *   launch date.
 *
 * Among the fee attributed to Delphi, 10% is sent to the developer (0x_larry) while the
 * rest to a treasury account controlled by Delphi.
 *
...
abstract contract FeeDistributor is Initializable, IFeeDistributor {
     ...
     * @param _developer Address owned by the developer (0x_larry)
     */
    ...
    function distributeFees() external override {
        uint256 toLido = collectibleFeeLido;
        uint256 toDeveloper = collectibleFeeDelphi.div(10);
        uint256 toDelphi = collectibleFeeDelphi.sub(toDeveloper);

        _resetcollectibleFee();

        feeToken.safeTransfer(lidoAgent, toLido);
        feeToken.safeTransfer(developer, toDeveloper);
        feeToken.safeTransfer(delphiAgent, toDelphi);
    }
    ...
    function setDeveloper(address _developer) external override {
