...
// File: contracts/strategies/StabilizeStrategyTestStrat.sol
...
// This is a strategy takes reward token and sends it to staking vault

contract StabilizeStrategyTestStrat is Ownable {
    using SafeMath for uint256;
    ...
    function sendToVault() external onlyGovernance {
        // Sending staking rewards to token vault
        IERC20 tok = IERC20(rewardToken);
        uint256 bal = tok.balanceOf(address(this));
        tok.safeTransfer(zsTokenAddress, bal);
        StabilizeStakingPool(zsTokenAddress).notifyRewardAmount(bal);  
    }
