...
contract WebBasedNFTs is ERC721Enumerable, Ownable, ChainlinkClient {
    event WebAmountExhausted(uint256 tokenId);
    ...
    function mintWebToken(
        string memory webURL,
        uint256 _webCode
    ) external returns (uint256) {
        require(!_exists(_webCode));
        _tokenInfo[_webCode] = metadata(_webCode, msg.sender, 0, 0, webURL);
        require(IPaymentSplitter(_paymentSplitter).addWebAccount(msg.sender, _webCode) == _webCode,"Error in creating account in payments");
        _safeMint(msg.sender, _webCode);
        return _webCode;
    }
    ...
    recordChainlinkFulfillment(_requestId)
    {
        ...
        if(_tokenInfo[_requestedData[_requestId]].amount < payment){
            emit WebAmountExhausted(_requestedData[_requestId]);
        }
        require(_tokenInfo[_requestedData[_requestId]].amount < payment, "Amount exhausted for the Web token");
