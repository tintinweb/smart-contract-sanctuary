...
/// @notice period option(period in days and percentage assign with period )
    struct StakeOption {
        ...
        uint64 emission;          // calculated emission for each option
...
contract StakeRewards is PausableUpgradeable, BumperAccessControl {
    ...
    function calcRewards(StakeInfo memory s) public view returns (uint rewards, bool claimable, bool withdrawable, uint endOfLastPeriod) {
        ...
        if (opt.periodInDays == 0) { // flexible staking
            ...
            uint periods = (s.end - s.start) / (1 days * opt.periodInDays);
            endOfLastPeriod = s.start + periods * 1 days * opt.periodInDays;

            rewards = (index - s.lastCI) * s.amount;

            if (block.timestamp > endOfLastPeriod){
                extraRewards = (block.timestamp - endOfLastPeriod) * opt.emission * s.amount / opt.total;
            }
            withdrawable = block.timestamp > (s.start + (opt.periodInDays * 1 days));
        }
        console.log("Extra rewards: %s", extraRewards );

        rewards /= 10**18;
        if (rewards > s.claimed + extraRewards)
            rewards -= s.claimed + extraRewards;
        else
            rewards = 0;
        claimable = rewards > 0;
