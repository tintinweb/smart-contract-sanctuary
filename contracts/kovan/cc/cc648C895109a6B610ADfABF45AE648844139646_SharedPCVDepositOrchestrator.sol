...
contract SharedPCVDepositOrchestrator is ISharedPCVDepositOrchestrator {
    function init(
        address core,
        address pool,
        address nft,
        ...
        returns (address erc20UniswapPCVDeposit, address uniswapOracle)
    {
        uniswapOracle = address(
            new UniswapOracle(core, oraclePool, twapDuration, isPrice0)
        );
        erc20UniswapPCVDeposit = address(
            new ERC20UniswapPCVDeposit(core, pool, nft, router, uniswapOracle, tickLower, tickUpper)
        );
    }
}
...
contract ERC20UniswapPCVDeposit is UniswapPCVDeposit {
    ...
    /// @notice set vault address for collecting fee
    function setVaultAddress(address _vaultAddress) public override whenNotPaused onlyPCVController {
    ...
    function collect() public override whenNotPaused returns (uint256 amount0, uint256 amount1) {
        return _collect(vaultAddress);
    }

    function _collect(address to) internal returns (uint256 amount0, uint256 amount1) {
        ...
        emit Collect(to, amount0, amount1);
    }
