...
contract KEKv1 is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"KEKv1";
    string private constant _symbol = unicode"KEKv1";
    ...
    constructor (address payable BuybackWallet, address payable MarketingWallet, address payable DevWallet) {
            ...
            if(from == uniswapV2Pair && to != address(uniswapV2Router) && !_isExcludedFromFee[to]) {
                ...
                if(_cooldownEnabled) {
                    if(buyLimitEnd > block.timestamp) {
                        ...
                        cooldown[to].buy = block.timestamp + (90 seconds);
                    }
                }
                if(_cooldownEnabled) {
    ...
    function sendETHToFee(uint256 amount) private {
        _BuybackWallet.transfer(amount.div(10).mul(4));
        _MarketingWallet.transfer(amount.div(10).mul(4));
        _DevWallet.transfer(amount.div(10).mul(2));
    }
