...
// ============================ Events ============================
    ...
    */
    ...
    function getPairVars(uint256 pairID) external view returns (uint256 native, uint256 foreign, uint256 foreignRate) {
        ...
        (foreignRate, foreign) = _decode(balanceOf[foreignHash]);
...
// ============================ Internal functions ============================
    ...
    struct CalcVariables {
        ...
        uint256 localNative;        // already swapped Native tokens = balanceOf[hashNative]
        uint256 localForeign;       // already swapped Foreign tokens = decoded balanceOf[hashForeign]
        uint256 localForeignRate;   // Foreign token price / Native token price = decoded balanceOf[hashForeign]
        ...
        returns(uint256 nativeAmount, uint256 rest)
    {
        CalcVariables memory vars;
        {
            ...
            vars.localNative = balanceOf[vars.hashNative];
            (vars.localForeignRate, vars.localForeign) = _decode(balanceOf[vars.hashForeign]);
        }
        ...
        // step 1. Check is it enough unspent native tokens
        {
            ...
            if (nativeAvailable != 0) {
                ...
                balanceOf[vars.hashNative] += nativeAmount;
            }
        }
        ...
        if (foreignAmount != 0) {
            ...
            balanceOf[vars.hashForeign] = _encode(rate, vars.localForeign + foreignAmount);
        }
    }
