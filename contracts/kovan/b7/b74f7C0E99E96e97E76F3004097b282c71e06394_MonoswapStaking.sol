...
contract MonoswapStaking is Initializable, OwnableUpgradeable, ERC1155HolderUpgradeable {
        ...
        returns (uint256)
    {
        ...
        if (block.number >= pool.lastRewardBlock && stakedAmount != 0 && user.amount != 0) {
            if (pool.bActive == true) {
                ...
                uint256 monoReward = calcRewardWithBlock(pool.lastRewardBlock, block.number, ratio, pool.allocPoint);
                if (user.oldReward > 0) {
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 monoReward = calcRewardWithBlock(pool.lastRewardBlock, block.number, ratios[startPeriod], pool.allocPoint);
    ...
    function calcRewardWithBlock(uint256 _fromBlock, uint256 _toBlock, uint256 _ratio, uint256 _allocPoint)
        public
        view
        returns (uint256)
    {
