...
contract APIConsumer is ChainlinkClient {
     ...
     */
    function requestUintData(
        string memory _getURL,
        string memory _path,
        ...
        request.addUint("times", _timesAmount);

        // Sends the request
        return sendChainlinkRequestTo(oracle, request, fee);
    }

    function requestIntData(
        string memory _getURL,
        string memory _path,
        int256 _timesAmount
    ) public returns (bytes32 requestId) {
        Chainlink.Request memory request = buildChainlinkRequest(jobId, address(this), this.fulfillInt.selector);
        request.add("get", _getURL);
        request.add("path", _path);
        request.addInt("times", _timesAmount);
        return sendChainlinkRequestTo(oracle, request, fee);
    }

    function requestStringData(
        string memory _getURL,
        string memory _path
    ) public returns (bytes32 requestId) {
        Chainlink.Request memory request = buildChainlinkRequest(jobId, address(this), this.fulfillString.selector);
        request.add("get", _getURL);
        request.add("path", _path);
        return sendChainlinkRequestTo(oracle, request, fee);
    }
     ...
     */
    function fulfillUint(bytes32 _requestId, uint256 _value) public recordChainlinkFulfillment(_requestId) {
        uintValue = _value;
    }
     ...
     */
    function fulfillInt(bytes32 _requestId, int256 _value) public recordChainlinkFulfillment(_requestId) {
        intValue = _value;
    }
     ...
     */
    function fulfillString(bytes32 _requestId, string memory _value) public recordChainlinkFulfillment(_requestId) {
        stringValue = _value;
    }
