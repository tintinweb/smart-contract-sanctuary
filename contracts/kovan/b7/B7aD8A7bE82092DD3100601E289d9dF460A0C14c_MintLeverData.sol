...
contract MintLeverData is IMintLeverData, Owner {
    ...
    mapping(address => mapping(address => uint256)) public bondPTokenAmounts;//user-token-pamount
    ...
    function _sendPosition(address user, IOperateType.PositionOperateType operateType) internal {
        (address[] memory _bonds, uint256[] memory _amounts) = config.getMintRouter().getBond(address(this), user);
