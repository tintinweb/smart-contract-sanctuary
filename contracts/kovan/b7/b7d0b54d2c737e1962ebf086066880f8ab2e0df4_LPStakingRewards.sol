    ...
    ReentrancyGuardUpgradeable
{
    ...
    event LPRewardDurationUpdated(uint256 _rewardDuration);
    event RewardsEndUpdated(uint256 rewardsEndBlock);
    event LPAdded(address indexed lpToken);

    struct PoolInfo {
        IERC20Upgradeable lpToken; // Address of LP token contract.w
        uint256 totalStakes;
    ...
    mapping(address => bool) public isValidLP; // for querying if a LP is supported by Flurry
    ...
    address public flurryTokenAddr;

    address public stakingRewardsAddr;

    function initialize(address _flurryTokenAddr, address _stakingRewardsAddr) public initializer{
        AccessControlEnumerableUpgradeable.__AccessControlEnumerable_init();
        ReentrancyGuardUpgradeable.__ReentrancyGuard_init();

        flurryTokenAddr = _flurryTokenAddr;
    ...
    function setStakingRate(uint256 _stakingRate) public onlyRole(DEFAULT_ADMIN_ROLE){
        stakingRate = _stakingRate;
    }

    function getStakingRate() external view override returns (uint256) {
        return stakingRate;
    }
    ...
    function stake(address lpAddr, uint256 amount) external override {
        ...
        // TODO: add test case for isValidLP[lpAddr]: added lpAddr give true? Otherwise false?
        // require(isValidLP[lpAddr], "LP not in Flurry whitelist");
     ...
     */
    function lastBlockApplicable(address lpAddr) internal view returns (uint256){
        return _lastBlockApplicable(poolInfo[lpAddr].rewardsEndBlock);
    }

    function rewardOf(address user, address lpAddr) external view override returns (uint256){
        return _earned(user, lpAddr);
    }

    // TODO: check dynamic array behaviour. Do stress test
    // TODO: make test case for totalRewardOf
    ...
    function stakeOf(address user, address lpAddr) external view override returns (uint256){
     ...
     */
    function rewardsPerToken(address lpAddr) public view override returns (uint256){
    ...
    function _earned(address user, address lpAddr) internal view returns (uint256) {
        UserInfo storage _user = userInfo[lpAddr][user];
        return super._earned(_user.amount, rewardsPerToken(lpAddr).sub(_user.rewardPerTokenPaid), poolInfo[lpAddr].stakeTokenOne, _user.reward);
    }
    ...
    function _withdrawUser(address user, address lpAddr, uint256 amount) internal {
        // require(isValidLP[lpAddr], "LP not in Flurry whitelist");
        ...
        updateReward(user, lpAddr);
        userInfo[lpAddr][user].amount = userInfo[lpAddr][user].amount.sub(amount);
        poolInfo[lpAddr].totalStakes = poolInfo[lpAddr].totalStakes.sub(amount);
        poolInfo[lpAddr].lpToken.safeTransfer(user, amount);
    }

    // TODO: onlyRole(LP_TOKEN_REWARDS_ROLE)?
    function claimReward(address user, address lpAddr) external override {
        // require(isValidLP[lpAddr], "LP not in Flurry whitelist");
        updateReward(user, lpAddr);
        if (userInfo[lpAddr][user].reward > 0) {
            require(user != address(0), "claim reward on 0 address");
            userInfo[lpAddr][user].reward = IStakingRewards(stakingRewardsAddr).claimLPTokenReward(user, userInfo[lpAddr][user].reward);
        }
    }

    // TODO: onlyRole(LP_TOKEN_REWARDS_ROLE)?
    // TODO: check dynamic array behaviour. Do stress test
    // TODO: make test case for claimAllReward
    ...
    function addLP(uint256 _allocPoint, address _lpToken, uint256 _rewardsEndBlock) public onlyRole(DEFAULT_ADMIN_ROLE) {
        totalAllocPoint = totalAllocPoint.add(_allocPoint);

        uint256 currentBlock = block.number;
        uint256 _rewardDuration = _rewardsEndBlock - currentBlock;
        uint256 _stakeTokenOne = 10**IERC20MetadataUpgradeable(_lpToken).decimals();

        isValidLP[_lpToken] = true;
    ...
    function setLP(address lpAddr, uint256 _allocPoint) public onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function setRewardDuration(address lpAddr, uint256 _rewardDuration) external override onlyRole(DEFAULT_ADMIN_ROLE) {
        PoolInfo storage pool = poolInfo[lpAddr];
        require(
        ...
        pool.rewardsEndBlock = block.number.add(_rewardDuration);
        emit LPRewardDurationUpdated(pool.rewardsEndBlock);
    }

    function getRewardForDuration(address lpAddr) external view override returns (uint256) {
        // require(isValidLP[lpAddr], "LP not in Flurry whitelist");
        return _getRewardsForDuration(stakingRate, poolInfo[lpAddr].rewardDuration);
    }

    function shortenRewardsDuration(address lpAddr) external onlyRole(DEFAULT_ADMIN_ROLE){
        PoolInfo storage pool = poolInfo[lpAddr];
        uint256 currentBlock = block.number;
        if (pool.rewardsEndBlock > currentBlock) {
            pool.rewardsEndBlock = currentBlock;
            emit RewardsEndUpdated(currentBlock);
        }
    }
