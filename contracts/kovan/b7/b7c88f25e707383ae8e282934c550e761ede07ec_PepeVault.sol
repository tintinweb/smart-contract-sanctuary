    ...
    ReentrancyGuard
{
    ...
    function newBaseLiquidity(
        ...
        uint128 liquidity = _liquidityForAmounts(
            _baseLower,
            _baseUpper,
            balance0.mul(_percentage).div(100),
            balance1.mul(_percentage).div(100)
        );
        _mintLiquidity(_baseLower, _baseUpper, liquidity);
        (baseLower, baseUpper) = (_baseLower, _baseUpper);
    }

    function newRangeLiquidity(
        int24 _limitLower,
        int24 _limitUpper,
        uint8 _percentage
    ) external override nonReentrant {
        require(msg.sender == strategy, "strategy");
        require(_percentage >= 100, "percentage");
        _checkRange(_limitLower, _limitUpper);
        ...
        _mintLiquidity(_limitLower, _limitUpper, liquidity);
        (limitLower, limitUpper) = (_limitLower, _limitUpper);
    }

    function increaseLiquidity(bool base, uint8 _percentage) public override {
        require(msg.sender == strategy, "strategy");
        require(_percentage >= 100, "percentage");
        ...
        int24 upperTick;
        if (base) {
            (lowerTick, upperTick) = (baseLower, baseUpper);
        } else {
            (lowerTick, upperTick) = (limitLower, limitUpper);
        }
        _poke(lowerTick, upperTick);
        uint256 balance0 = getBalance0();
        uint256 balance1 = getBalance1();
        ...
        _mintLiquidity(lowerTick, upperTick, liquidity);
    }

    function decreaseLiquidity(bool base, uint8 _percentage) public override {
        require(msg.sender == strategy, "strategy");
        require(_percentage >= 100, "percentage");
        ...
        _burnAndCollect(lowerTick, upperTick, (liquidity * (_percentage)) / (100));
    }
        ...
        )
    {
        ...
        return (baseLower, baseUpper, liquidity);
    }

    function getRangeLiquidity()
        external
        view
        override
        returns (
        ...
        )
    {
        ...
        return (limitLower, limitUpper, liquidity);
    }
    ...
    // TODO Implement multisig onlyGovernance
    /**
...
interface IStrategy {
    // This will burn and remove previous base liquidity
    function newBaseLiquidity(
        int24 lower,
        int24 upper,
        uint8 percentage
    ...
    // This will burn and remove previous range liquidity
    function newRangeLiquidity(
        int24 lower,
        int24 upper,
        uint8 percentage
    ...
    // base - if true, increase in base position. If false, increase in range order
    function increaseLiquidity(bool base, uint8 percentage) external;

    // base - if true, decrease in base position. If false, decrease in range order
    function decreaseLiquidity(bool base, uint8 percentage) external;

    //implements swaps between both tokens in the pool.
    //if direction is true, swap happens from token0 -> token1. If false, swap occurs from token1 -> token0
    function swapTokensFromPool(
        bool direction,
        uint160 sqrtPriceLimitX96,
        int256 amountIn
    ...
    // returns current ranges for base position and liquidity locked inside it
    function getBaseLiquidity()
        external
        returns (
            int24 lower,
    ...
    // returns current ranges for range order and liquidity locked inside it
    function getRangeLiquidity()
        external
        returns (
            int24 lower,
    ...
    // returns available balance to use for token0
    function getBalance0() external returns (uint256);

    // returns available balance to use for token1
    function getBalance1() external returns (uint256);
