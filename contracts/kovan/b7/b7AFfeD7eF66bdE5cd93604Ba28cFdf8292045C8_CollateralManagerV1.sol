    ...
    UUPSUpgradeable
{
    ...
    function createReserve(string memory name, string memory description)
        ...
        onlyOwner
    {
        // QUESTION: Do we ensure each reserve's name is unique?
        // QUESTION: What are the best parameters to calculate a hash?

        bytes32 reserveId = keccak256(
            abi.encode(name, description, block.number)
        );
    ...
    function deleteReserve(bytes32 reserveId) public onlyOwner {
        ...
        for (uint256 i = 0; i < reserveVaultsLength; i++) {
            reserveVaults.pop();
        }
    ...
    /// @param vault Data contained to calculate value of collateral
        ...
        onlyOwner
    {
        ...
        for (uint256 i = 0; i < reservesLength; i++) {
            Vault[] storage vaults = reserveMap[reserves[i]].vaults;

            uint256 vaultsLength = vaults.length;
            for (uint256 j = 0; j < vaultsLength; j++) {
                if (vault.vault == vaults[j].vault) {
                    found = true;
        ...
        require(!found, "vault is already a member of a reserve");
        ...
        uint256 numerator = vault.numerator;
        uint256 denominator = vault.denominator;
        ...
        address calculator = vault.calculator;

        // TODO: Check calculator read call
        ...
        emit AddTokenReserveVault(reserveId, msg.sender, calculator);
    }
        ...
        onlyOwner
    {
        ...
        for (uint256 i = index; i < reserveVaultsLength - 1; i++) {
            reserveVaults[i] = reserveVaults[i + 1];
        }

        reserveVaults.pop();

        emit RemoveTokenReserveVault(reserveId, msg.sender, calculator);
    }
