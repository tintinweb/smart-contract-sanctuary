...
contract KabyHero is IKabyHero, ERC721, AcceptedToken, ReentrancyGuard {
    ...
    function _createHero() private returns (uint heroId) {
        ...
        emit HeroCreated(heroId, star);
    }

    function randomStar() private view returns (uint returnStar){
        uint randomStarIndex = uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp))) % 100;
        uint star = 0;
        while(star < 10 && starIndex[star] <= randomStarIndex){
            star++;
        }
        return star;
    }
...
interface IKabyHero {
    ...
    event HeroCreated(uint indexed heroId, uint star);
