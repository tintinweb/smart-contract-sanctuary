...
contract Lfi is ERC20Capped, AccessControl, ILfi {
    ...
    modifier onlyRole(bytes32 role) {
        require(hasRole(role, msg.sender), "LFI: sender not a minter");
        _;
    }
        ...
        onlyBy(governanceAccount)
    {
        ...
        grantRole(DEFAULT_ADMIN_ROLE, newGovernanceAccount);
        revokeRole(DEFAULT_ADMIN_ROLE, governanceAccount);
    ...
    function addMinter(address account) external onlyBy(governanceAccount) {
        ...
        require(
            ...
            "LFI: account is already a minter"
        );
    ...
    function removeMinter(address account) external onlyBy(governanceAccount) {
        ...
        require(
            hasRole(MINTER_ROLE, account),
            "LFI: account not an existing minter"
        );
