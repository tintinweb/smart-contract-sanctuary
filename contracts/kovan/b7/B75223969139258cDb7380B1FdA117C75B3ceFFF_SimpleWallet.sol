...
contract EntryPoint is StakeManager {
     ...
     */
    function getRequestId(UserOperation calldata userOp) external pure returns (bytes32) {
        return userOp.hash();
    }
...
contract SimpleWallet is IWallet {
    ...
    function _validateSignature(UserOperation calldata userOp, bytes32 requestId) internal view {
        bytes32 k = keccak256(abi.encode(requestId, entryPoint, block.chainid));
        bytes32 hash = k.toEthSignedMessageHash();
