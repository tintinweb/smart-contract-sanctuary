...
contract Greeter is Ownable, ChainlinkClient {
  ...
  event RequestFulfilled(bytes32 indexed requestId, bytes lastInfo);

  event RequestMultipleFulfilled(bytes32 indexed requestId, bytes32 thisName, bytes32 thisPosition, bytes32 thisTeam, bytes32 thisId);

  constructor(address _oracle) {
    setPublicChainlinkToken();
    setChainlinkOracle(_oracle);
    jobId = 0x6237363939326634613834333435323239383131303034356664373163383237;
    fee = 0.1 * (10 ** 18);
  }
  ...
  function requestInfo(uint256 _idx) public returns (bytes32 requestId_) {
    Chainlink.Request memory request = buildChainlinkRequest(jobId, address(this), this.fulfillMultiple.selector);
    request.addUint("idx", _idx);
    requestId_ = requestOracleData(request, fee);
  }

  function fulfillMultiple(bytes32 _requestId, bytes32 _name, bytes32 _position, bytes32 _team, bytes32 _id) public recordChainlinkFulfillment(_requestId) {
    emit RequestMultipleFulfilled(_requestId, _name, _position, _team, _id);
    name = _name;
    position = _position;
    team = _team;
    id = _id;
