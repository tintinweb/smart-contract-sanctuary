...
contract Catemoon is Context, IERC20, Ownable {
  ...
  uint256 private numTokensSellToAddToLiquidity = 1 * 10**7 * 10**18; // 0.0% of total supply
  ...
  function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeSellFee, bool takeDevFee) private {
    if(!takeSellFee) {
      removeSellFee();
    }
    ...
    (uint256 transferAmount, uint256 liquidityFee, uint256 devFee) = _getValues(amount);
  ...
  function _getValues(uint256 _amount) private view returns (uint256, uint256, uint256) {
    uint256 liquidityFee = calculateLiquidityFee(_amount);
    uint256 devFee = calculateDevFee(_amount);
    uint256 transferAmount = _amount - liquidityFee - devFee;
    return (transferAmount, liquidityFee, devFee);
  }
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal virtual {
    ...
    _tokenTransfer(sender, recipient, amount, takeSellFee, takeDevFee);
  }
  ...
  function swapAndToDev(uint256 contractTokenBalance) private lockTheSwap {
    // capture the contract's current ETH balance.
    // this is so that we can capture exactly the amount of ETH that the
    // swap creates, and not make the liquidity event include any ETH that
    // has been manually sent to the contract
    ...
    transferToAddressETH(devAddress, newBalance);
  }
