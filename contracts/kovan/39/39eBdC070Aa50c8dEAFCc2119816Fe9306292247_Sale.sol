...
contract Sale is ReentrancyGuard, ERC721TokenReceiver {
    ...
    function buyWithUSDC(uint256 _amount) public virtual nonReentrant returns(uint256){
     ...
     function buyWithUSDT(uint256 _amount) public virtual nonReentrant returns(uint256){
        require(resolveStableCoins(6) <= _amount, "Underflow Error");
        require(IERC20(contracts[bytes("USDT")]).balanceOf(msg.sender) >= _amount, "Insufficient Balance");
        require(IERC20(contracts[bytes("USDT")]).allowance(msg.sender,address(this)) >= _amount, "Allowance Error");
        uint256 tokenId = forSale[sold];
        sold+= 1;
        niftyPunks.safeTransferFrom(address(this), msg.sender, tokenId);
        IERC20(contracts[bytes("USDT")]).transferFrom(msg.sender, admin, _amount);
        return tokenId;
    }
