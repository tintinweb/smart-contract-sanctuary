    ...
    onlyController
  {
    ...
    for (uint256 i = 0; i < deployments.length; i++) {
      ...
      for (uint256 j = 0; j < deployments[i].tokens.length; j++) {
        ...
        int256 actualDeployAmount = deployAmount;
        if (
          deployAmount > 0 &&
          (getDesiredReserveTokenBalance(deployments[i].tokens[j]) <
            getReserveTokenBalance(deployments[i].tokens[j]))
        ) {
          actualDeployAmount -= int256(
