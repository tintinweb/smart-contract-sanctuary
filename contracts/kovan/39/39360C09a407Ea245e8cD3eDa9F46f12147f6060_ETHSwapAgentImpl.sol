...
contract ETHSwapAgentImpl is Context {
    ...
    event SwapPair711Register(address indexed sponsor,address indexed erc20Addr, string name, string symbol, string baseURI);
    event SwapStarted(address indexed erc20Addr, address indexed fromAddr, uint256 amount, uint256 feeAmount);
    event Swap711Started(address indexed erc20Addr, address indexed fromAddr, uint256 tokenId, uint256 feeAmount);
    event SwapFilled(address indexed erc20Addr, bytes32 indexed bscTxHash, address indexed toAddress, uint256 amount);
    event Swap711Filled(address indexed erc20Addr, bytes32 indexed bscTxHash, address indexed toAddress, uint256 tokenId);
    ...
    function register711SwapPairToBSC(address erc711Addr) external returns (bool) {
        require(!registeredERC[erc711Addr], "already registered");

        string memory name = IERC721Query(erc711Addr).name();
        string memory symbol = IERC721Query(erc711Addr).symbol();
        string memory baseURI = IERC721Query(erc711Addr).baseURI();

        require(bytes(name).length>0, "empty name");
        require(bytes(symbol).length>0, "empty symbol");

        registeredERC[erc711Addr] = true;

        emit SwapPair711Register(msg.sender, erc711Addr, name, symbol, baseURI);
        return true;
    }
    ...
    function swap711ETH2BSC(address erc721Addr, uint256 tokenId) payable external notContract returns (bool) {
        ...
        emit Swap711Started(erc721Addr, msg.sender, tokenId, msg.value);
        return true;
    }
}
