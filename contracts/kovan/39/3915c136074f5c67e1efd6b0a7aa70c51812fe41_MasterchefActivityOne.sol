...
contract MasterchefActivityOne is Ownable, ActivityBase{
    ...
    function emergencyWithdraw(uint256 _pid) public virtual{
        ...
        if(!withdrawnState){
            updateInvitorWeight(msg.sender, invitor, _pid, false, amount, isWithdrawn, withdrawnState);
        }
        updateUserWeight(msg.sender, _pid, false, amount, isWithdrawn);
        pool.lpTokenAmount = pool.lpTokenAmount.sub(amount);
        IERC20(pool.lpTokenSwap).safeTransfer(address(msg.sender), user.amount);
        user.amount = 0;
        user.rewardDebt = 0;
        user.revenue = 0;
        user.withdrawnState = isWithdrawn;
