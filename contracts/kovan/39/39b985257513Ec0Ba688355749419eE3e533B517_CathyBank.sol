...
contract CathyBank is Ownable {
    ...
    function borrowPHX(uint256 _PHXAmount) public payable {
        ...
        require(
            computeCollateralPercent(msg.value, _PHXAmount) >=
                minCollateralPercent, 'Provide more collateral'
        );
    ...
    function computeCollateralPercent(uint256 _ETHAmount, uint256 _PHXAmount)
        ...
        returns (uint256)
    {
        return ((_ETHAmount * ETHPHXConversionRate) / (10 ^ 18)) / _PHXAmount;
    }
    ...
    function getPHXBalance(address _owner) public view returns (uint256) {
        ...
        uint256 accruedInterest = (((timeElapsed * savingsInterestPercent) /
            secsYear) * PHXBalance[_owner]) / 1000000;
    ...
    function getLoanBalance(address _owner) public view returns (uint256) {
        ...
        uint secsYear = 60 * 60 * 24 * 360;
        uint256 accruedInterest = (((timeElapsed * loanInterestPercent) / secsYear) * loanBalance[_owner])/ 1000000;
