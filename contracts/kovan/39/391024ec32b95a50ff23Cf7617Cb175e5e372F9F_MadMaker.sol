...
// MadMaker is MasterChef's left hand and kinda a wizard. He can cook up Sushi from pretty much anything!
// This contract handles "serving up" rewards for xSushi holders by trading tokens collected from fees for Sushi.

// T1 - T4: OK
contract MadMaker is Ownable {
    ...
    function setBridge(address token, address bridge) external onlyOwner {
        // Checks
        require(
            token != mad && token != weth && token != bridge,
            "MadMaker: Invalid bridge"
        );
    ...
    modifier onlyEOA() {
        ...
        require(msg.sender == tx.origin, "MadMaker: must use EOA");
        _;
    }
    ...
    function _convert(address token0, address token1) internal {
        ...
        require(address(pair) != address(0), "MadMaker: Invalid pair");
        // balanceOf: S1 - S4: OK
        // transfer: X1 - X5: OK
    ...
    function _convertStep(
        ...
        if (token0 == token1) {
            ...
            if (token0 == mad) {
                IERC20(mad).safeTransfer(bar, amount);
                sushiOut = amount;
            } else if (token0 == weth) {
            ...
            // eg. SUSHI - ETH
            IERC20(mad).safeTransfer(bar, amount0);
            sushiOut = _toSUSHI(token1, amount1).add(amount0);
        } else if (token1 == mad) {
            // eg. USDT - SUSHI
            IERC20(mad).safeTransfer(bar, amount1);
    ...
    function _swap(
        ...
        require(address(pair) != address(0), "MadMaker: Cannot convert");
    ...
    {
        // X1 - X5: OK
        amountOut = _swap(token, mad, amountIn, bar);
    }
}
...
contract MadMakerExploitMock {
    MadMaker public immutable sushiMaker;
    constructor (address _sushiMaker) public{
        sushiMaker = MadMaker(_sushiMaker);
    } 
...
contract UniswapV2ERC20 {
    ...
    string public constant name = 'MadSwap LP Token';
...
contract MadSwapPairMock is UniswapV2Pair {
...
contract MadSwapFactoryMock is UniswapV2Factory {
