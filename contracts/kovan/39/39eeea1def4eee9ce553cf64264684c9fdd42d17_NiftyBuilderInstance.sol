...
contract NiftyBuilderInstance is ERC721Full {
    ...
    modifier onlyValidSender() {
        ...
        bool is_valid = nftg_registry.isValidNiftyManager(msg.sender);
    ...
    constructor(address registryContract, string memory _name, string memory _symbol, string memory base_uri, string memory name_of_creator) ERC721Full(_name, _symbol) public {
    ...
    function batchGiftNifty(address[] memory collector_address, uint niftyType) onlyValidSender public {
        for (uint i = 0; i < collector_address.length; i++) {
            giftNifty(collector_address[i], niftyType);
        }
    }
    ...
    function giftNifty(address collector_address, uint niftyType) onlyValidSender public {
        ...
        uint tokenId = ((niftyType * 10000) + specificTokenId);
        string memory tokenIdStr = uint2str(tokenId);
        string memory tokenURI = strConcat(baseURI, tokenIdStr);
        string memory ipfsHash = _niftyIPFSHashes[niftyType];
        require(bytes(ipfsHash).length > 0, "this type's ipfs has not been set yet");
        //mint token
...
library Counters {
    ...
    function decrement(Counter storage counter) internal {
        require(counter._value >= 1, "Counter is zero");
