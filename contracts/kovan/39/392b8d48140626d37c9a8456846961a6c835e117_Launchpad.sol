...
contract Launchpad  is Ownable {
    ...
    function buyTokenWithUSDT(uint256 _amount) external {
        ...
        require( userUpdatedBalance.add(usdt.balanceOf(address(this))) <= hardcap, "Hardcap for the tokens reached");

        require(userUpdatedBalance <= allowedUserBalance, "Exceeded allowed user balance");
        
        // usdt.transferFrom(msg.sender, address(this), _amount);
        
        doTransferIn(address(usdt), msg.sender, tokensPurchased);
