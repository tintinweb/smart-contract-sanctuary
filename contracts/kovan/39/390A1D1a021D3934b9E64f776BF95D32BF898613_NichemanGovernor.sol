...
contract NichemanGovernor is Arbitrable {
    ...
    uint constant NO_SHADOW_WINNER = type(uint).max; // The value that indicates that no one has successfully paid appeal fees in a current round. It's the largest integer and not 0, because 0 can be a valid submission index.
    ...
    modifier duringApprovalPeriod() {
        uint offset = sessions[sessions.length - 1].durationOffset;
        require(block.timestamp - lastApprovalTime > submissionTimeout.addCap(offset), "Approval time has not started yet.");
        _;
    }
     ...
     */
    function setMetaEvidence(string memory _metaEvidence) external {
     ...
     */
    function changeArbitrator(Arbitrator _arbitrator, bytes memory _arbitratorExtraData) public onlyByGovernor duringSubmissionPeriod {
        arbitrator = _arbitrator;
     ...
     */
    function changeMetaEvidence(string calldata _metaEvidence) public onlyByGovernor {
     ...
     */
    function submitList (address[] memory _target, uint[] memory _value, bytes memory _data, uint[] memory _dataSize, string memory _description) public payable duringSubmissionPeriod {
        ...
        for (uint i = 0; i < _target.length; i++) {
            ...
            submission.txs.push();
            Transaction storage transaction = submission.txs[submission.txs.length-1];
        ...
        if (session.submittedLists.length == 1)
            session.durationOffset = block.timestamp.subCap(lastApprovalTime);
    ...
    function withdrawTransactionList(uint _submissionID, bytes32 _listHash) public {
        ...
        require(block.timestamp - lastApprovalTime <= submissionTimeout / 2, "Lists can be withdrawn only in the first half of the period.");
    ...
    function executeSubmissions() public duringApprovalPeriod {
        ...
        if (session.submittedLists.length == 0) {
            ...
            payable(submission.submitter).transfer(sumDeposit);
            lastApprovalTime = block.timestamp;
            session.status = Status.Resolved;
            sessions.push();
    ...
    function withdrawFeesAndRewards(address _beneficiary, uint _session, uint _round, uint _submissionID) public {
        ...
        payable(_beneficiary).transfer(reward); // It is the user responsibility to accept ETH.
    ...
    function executeRuling(uint _disputeID, uint _ruling) internal override{
        ...
        if (_ruling != 0) {
            ...
            submission.approved = true;
            submission.approvalTime = block.timestamp;
            payable(submission.submitter).transfer(session.sumDeposit);
        }
    ...
    function executeTransactionList(uint _listID, uint _cursor, uint _count) public {
        ...
        require(block.timestamp - submission.approvalTime <= executionTimeout, "Time to execute the transaction list has passed.");
    ...
    ) public view returns(uint[] memory contributions) {
        Session storage session = sessions[_session];
        Round storage round = session.rounds[_round];
     ...
     */
    function getSubmittedLists(uint _session) public view returns (uint[] memory submittedLists) {
        Session storage session = sessions[_session];
