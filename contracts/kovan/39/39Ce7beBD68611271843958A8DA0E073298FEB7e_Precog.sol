...
contract Precog is IPrecog {
    ...
    ) internal view returns (uint256 profit, bool canBorrow, uint256 expiryDate, uint tradeAmount) {
        ...
        if (expiryDate > block.timestamp) {
            ...
            if (pairs[0] != address(0)) {
                if (amount > fee.feeTrading) {
                    tradeAmount = amount - fee.feeTrading;
    ...
    function invest(
        ...
        uint256 platformIndex,
        uint256 borrowAmount
    ) external returns (uint tradeAmount, uint calculatedTradeAmount) {
        ...
        (uint profit, bool canBorrow, uint expiryDate, uint calTradeAmount) = _calculateProfit(user, amount, fee, tradeInfo, loan);
        calculatedTradeAmount = calTradeAmount;
