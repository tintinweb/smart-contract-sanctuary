...
import "./ProjectERC20Factory.sol";


contract BatchCollection is ERC721Enumerable, Ownable, IBatchCollection {
    ...
    constructor(address _contractRegistry) ERC721("CO2KEN Batch Retirement Collection", "CO2KEN-BNFT") {
        contractRegistry = _contractRegistry;
    }

    /// @notice The verifier has the authority to confirm NFTs so ERC20's can be minted
    modifier onlyVerifier() {
        require(_verifier == _msgSender(), "Error: caller is not the verifier");
        _;
    }   
    ...
    function updateBatchWithData
        ...
        public
    {
        require(ownerOf(tokenId)==msg.sender || msg.sender==_verifier, "Error update only by owner or verifier");
        require(nftList[tokenId].confirmed == false, "Error: cannot change data after confirmation");
    ...
    function getProjectId(uint256 tokenId) public view override returns (string memory) {
        return nftList[tokenId].projectIdentifier;
    }

    function getVintage(uint256 tokenId) public view override returns (uint16) {
        return nftList[tokenId].vintage;
    }

    function getQuantity(uint256 tokenId) public view override returns (uint256) {
   ...
   function getBatchNFTData(uint256 tokenId) public view override returns (string memory, uint16, string memory, uint256, bool) {
        return (
    ...
    // Function that automatically coverts to ERC20s via corresponding contract
    // Submit the pvId string in order to save gas
    // pvId = projectId+vintage
    function fractionalize(uint256 tokenId, string memory pvId) public {      
        require(_isApprovedOrOwner(_msgSender(), tokenId), "ERC721: transfer caller is not owner nor approved");

        address ERC20Factory = IContractRegistry(contractRegistry).projectERC20FactoryAddress();
        address pvERC20 = ProjectERC20Factory(ERC20Factory).pvIdtoERC20(pvId);
...
interface IBatchCollection {
    ...
    function getVintage(uint256 tokenId) external view returns (uint16);   
    ...
    function getBatchNFTData(uint256 tokenId) external view returns (string memory, uint16, string memory, uint256, bool);
}
...
contract ProjectCollection is IProjectCollection, ERC721, Ownable {
    ...
    mapping (string => bool) public metaDataHashes;
    ...
    function updateController(uint256 tokenId, address _controller) public {
        ...
        returns (uint256)
    {
        ...
        require(metaDataHashes[metaDataHash]==false, "Error: metaDataHash already exists");
        projects[newItemId].metaDataHash = metaDataHash;
        metaDataHashes[metaDataHash]=true;
        ...
        public onlyOwner
    {
        ...
        projects[tokenId].standard = newStandard;
        projects[tokenId].methodology = newMethodology;
        projects[tokenId].region = newRegion; 

        require(metaDataHashes[newMetaDataHash]==false, "Error: metaDataHash already exists");    
        string memory oldHash = projects[tokenId].metaDataHash;

        projects[tokenId].metaDataHash = newMetaDataHash;
        metaDataHashes[newMetaDataHash]=true;
        metaDataHashes[oldHash]=false;

        emit ProjectUpdated(tokenId);
    }

    /// @dev Projects and their projectId's must be unique, changing them must be handled carefully
    function updateProjectId(uint256 tokenId, string memory newProjectId) public onlyOwner {
        require(_exists(tokenId), "Project not yet minted");
    ...
    function removeProject(uint256 tokenId) public onlyOwner {
    ...
    function getProjectDataByTokenId(uint256 tokenId) public view
        ...
        {
         uint256 tokenId = pidToTokenId[projectId];   
         return (
            projects[tokenId].standard,
            projects[tokenId].methodology,
            projects[tokenId].region
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        string memory metaDataHash = projects[tokenId].metaDataHash;
        ...
        if (bytes(metaDataHash).length > 0) {
            return string(abi.encodePacked(base, metaDataHash));
        }
...
contract ProjectERC20Factory {
    ...
    mapping (string => address) public pvIdtoERC20; 
    ...
    public {
        ...
        ProjectERC20 t = new ProjectERC20(pvId, pvId, projectId, vintage, _contractRegistry);
     ...
     function deployFromTemplate(uint256 tokenId) public {
        ...
        string memory pvId = projectVintageId(pid, Strings.toString(vintage));
    ...
    function checkExistence(string memory _pvId) internal view returns (bool) {
        if (pvIdtoERC20[_pvId] == address(0)) {
    ...
    // Currently only used in test suite, might be deprecated soon
...
import "./IContractRegistry.sol";
import "./IBatchCollection.sol";
import "./IRetirementBadgeCollection.sol";
import "./ProjectCollection.sol";
...
contract ProjectERC20 is Context, ERC20, IERC721Receiver {
    ...
    mapping (address => uint256) retiredAmount;
    ...
    // Function to get corresponding attributes from the ProjectCollection
    ...
    function onERC721Received(address operator, address from, uint256 tokenId, bytes calldata data)
        ...
        (string memory _projectId, uint16 _vintage, , uint256 quantity, bool confirmed) = IBatchCollection(msg.sender).getBatchNFTData(tokenId);
        require(checkMatchingAttributes(_projectId, _vintage), "Error: non-matching NFT");
        require(confirmed==true, "BatchNFT not yet confirmed");
     ...
     **/
    function checkMatchingAttributes(string memory _projectId, uint16 _vintage) internal view returns (bool) {
        // console.log("DEBUG sol: checkMatchingAttributes called");

        bytes32 pid721 = keccak256(abi.encodePacked(_projectId));
        bytes32 pid20 = keccak256(abi.encodePacked(projectId));

        if (pid20 == pid721 && vintage == _vintage) { 
            return true;
        }
        else {
            return false;
    ...
    function mintBadgeNFT(address to, uint256 amount) public {
        address badgeAddr = IContractRegistry(contractRegistry).retirementBadgeCollectionAddress();
        require(retiredAmount[msg.sender] >= amount, "Error: Cannot mint more than user has retired");
        
        IRetirementBadgeCollection(badgeAddr).mintBadge(to, projectId, vintage, amount);
...
interface IRetirementBadgeCollection {

    function mintBadge(address to, string memory projectId, uint16 vintage, uint256 amount) external;
