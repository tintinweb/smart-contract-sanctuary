...
inheritance, etc.) */
contract SimpleBankWAD { // CamelCase
    using SafeMath for uint256;
    ...
    constructor(address _wad) public {
    ...
    function deposit(uint amountDeposit) public payable returns (uint256) {
        ...
        balances[msg.sender] = balances[msg.sender].add(amountDeposit);
        // no "this." or "self." required with state variable
        // all values set to data type's initial value by default
        
        systemBalance = systemBalance.add(amountDeposit);
        
        require(IWAD(wad).transferFrom(msg.sender, address(this), amountDeposit), "transfer failed");

        emit DepositMade(msg.sender, amountDeposit); // fire event
    ...
    function withdraw(uint withdrawAmount) public returns (uint256 remainingBal) {
        ...
        systemBalance = systemBalance.sub(withdrawAmount);

        // Revert on failed
        require(IWAD(wad).transfer(msg.sender, withdrawAmount), "transfer failed");
    ...
    function increaseYear() public {
        for(uint256 i = 0; i < accounts.length; i++) {
            ...
            systemBalance = systemBalance.add(interest);
            
            IWAD(wad).mint(address(this), interest);
        }
    }
}