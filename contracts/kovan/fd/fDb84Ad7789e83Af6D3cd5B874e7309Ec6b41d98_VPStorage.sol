...
contract InTokenWrapper is ReentrancyGuard {
	...
	/**
	 * @dev Withdraw In Tokens (balance - _outAmount) & burn (_outAmount) after executeExchange
	 * @param _user User's address 
	 * @param _outAmount Amount of Out Tokens paid
	 */
	function _withdrawExecute(address _user, uint256 _outAmount)
		internal
		nonReentrant
	{
		uint256 _balance = _balances[_user];
		uint256 _returnAmount = _balance - _outAmount;
		_totalSupply -= _balance;
		_balances[_user] = 0;
		if(_balance > _outAmount)
			inToken.transfer(_user, _returnAmount);
		inToken.burnFrom(address(this), _outAmount);
		emit ExecutedExchange(_user, _outAmount, _returnAmount);
	}
...
contract Scheduller {
	...
	{
		_period = period;
		if(periodStartTime == 0 && _period != 0)
			return _period;
...
contract VPStorage is Scheduller, InTokenWrapper {
	...
	*/
		...
		returns(uint256 _outTokenForExhange)
	{
		...
		if(period == _period)
			...
			(_outTokenForExhange, ) = _getNextOutTokenForExchange(period);
	}
	...
	*/
	function _calculateIn(address _user)
		private
		view
		returns(uint256)
	{
		if(userPeriod(_user) != _currentPeriod()){
			return _getOutTokenForExchange();
		}else{
			return _getOutTokenForExchange() - balanceOf(_user);
		}
	}
	...
	* @param _user Address of the user
	*/
	function _calculateOut(address _user) 
		private
		view
		returns (uint256 _outAmount)
	{
		...
		if(_userPeriod != _period){
			...
			if(historyOutTokenForExchange[_userPeriod] >= _userPeriodTotalSupply){
				_outAmount = balanceOf(_user);
			}else{
				_outAmount = historyOutTokenForExchange[_userPeriod] * balanceOf(_user)
	...
	* @param _user Address of the user
	*/
	function _getInTokenBalanceForWithdraw(address _user)
		private
		view
		returns(uint256 _inBalance)
	{
		if(userPeriod(_user) == _currentPeriod())
			_inBalance = balanceOf(_user);
	}
	...
	****************************************/
	...
	function receiveOutToken(uint256 _amount)
		external
	...
	{
		_updatePeriod(true);
		require(_calculateIn(msg.sender) >= _amount, "amount greater than contract can accept this period");
		if(userPeriod(msg.sender) != period && balanceOf(msg.sender) != 0)
			executeExchange();
		_deposit(msg.sender, _amount, period);
	}
	...
	{
		_updatePeriod(false);
		require (_getInTokenBalanceForWithdraw(msg.sender) >= _amount, "amount greater balance for current period");
		_withdraw(msg.sender, _amount, period);
	}
	...
	*/
	function getInTokenBalanceForWithdraw(address _user)
		...
		returns(uint256)
	{
		return _getInTokenBalanceForWithdraw(_user);
	}
