...
contract Execute is IExecute {
    uint8 public isExecute;

	
	receive() external payable {}
    
    function execute() override external returns(bool) {
        isExecute = 1;
        TSC tsc =  TSC(msg.sender);
        
        tsc.partner().transfer(address(this).balance);
        
        return true;
    }
    
    function revert() override external returns(bool) {
        isExecute = 2;
        TSC tsc =  TSC(msg.sender);
        
        tsc.owner().transfer(address(this).balance);
    }
}
...
contract TSC is TerminateContractTemplate {
    ...
    function _closeNotCompleted() private {
        ...
        if (reward.tokens == address(0x0) && reward.value > 0) {
            address(uint160(address(msg.sender))).transfer(reward.value);
        }
        if (startTimmingRequired.tokens != address(0x0) && startTimmingRequired.value > 0) {
...
contract TscPool {
    event Created(address indexed _creator, address indexed _contracts);
    
    function create(
    ) public returns(address) {
        TSC contracts = new TSC();
