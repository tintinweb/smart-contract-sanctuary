...
interface IGovernancePharoToken {
    function mintStakerTokens(address to, uint256 amount) external;
    function transfer ( address recipient, uint256 amount ) external returns ( bool );
    function transferFrom ( address sender, address recipient, uint256 amount ) external returns ( bool );
    function approve ( address spender, uint256 amount ) external returns ( bool );
    function burnFrom ( address account, uint256 amount ) external;
...
/// @title Pharo Stake Pool Contract
/// @author jaxcoder
/// @notice Swaps PHRO for gPHRO and earn rewards on your PHRO deposit
...
contract PharoStakePool is AccessControl, ReentrancyGuard, TokenRecover {
    ...
    IPharoToken public phroToken;
    IGovernancePharoToken public gphroToken;
    ...
    struct StakerInfo {
        ...
        // pending reward = (user.amount * pool.accPhroPerShare) - user.rewardDebt
    ...
    struct PoolInfo {
        ...
        uint256 accPhroPerShare; // Accumulated PHRO per share, times 1e12. See below
    }
    ...
    mapping(uint256 => mapping(address => uint256)) public rewardsAccumulated;
    ...
    event UpdateEmissionRate(address indexed sender, uint256 pharoPerBlock);
        ...
        ReentrancyGuard()
    {
        ...
        phroToken = IPharoToken(_phroToken);
        gphroToken = IGovernancePharoToken(_gphroToken);
        ...
        returns (uint256)
    {
        ...
        uint256 accPhroPerShare = pool.accPhroPerShare;
        uint256 lpSupply = phroToken.balanceOf(address(this));
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accPhroPerShare = accPhroPerShare.add(
                phroReward.mul(1e12).div(lpSupply)
            );
        }

        return staker.amount.mul(accPhroPerShare).div(1e12).sub(staker.rewardDebt);
    }
    ...
    function updatePool(uint256 _poolId) public {
        ...
        uint256 phroReward = multiplier
            ...
            .mul(pool.allocationPoint)
            .div(totalAllocationPoint);
        phroToken.mintForContract(address(this), phroReward);
        pool.accPhroPerShare = pool.accPhroPerShare.add(
            phroReward.mul(1e12).div(lpSupply)
        );
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _poolId, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            uint256 pending = user
                .amount
                .mul(pool.accPhroPerShare)
                .div(1e12)
                .sub(user.rewardDebt);
            safePhroTransfer(msg.sender, pending);
        }
        phroToken.transferFrom(
            address(msg.sender),
            address(this),
            _amount
        );
        ...
        user.rewardDebt = user.amount.mul(pool.accPhroPerShare).div(1e12);
    ...
    function withdraw(uint256 _poolId, uint256 _amount) public {
        ...
        updatePool(_poolId);
        uint256 pending = user.amount.mul(pool.accPhroPerShare).div(1e12).sub(
            user.rewardDebt
        );
        safePhroTransfer(msg.sender, pending);
    ...
    function safePhroTransfer(address _to, uint256 _amount) internal {
        uint256 phroBal = phroToken.balanceOf(address(this));
        if (_amount > phroBal) {
            phroToken.transfer(_to, phroBal);
        } else {
            phroToken.mintForContract(msg.sender, _amount);
        }
    }
    ...
    // todo: only the DAO role can use this function todo:
