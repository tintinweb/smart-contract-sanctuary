...
contract SalePoolV1 is Ownable {
  ...
  // pid => referrer => sales
  mapping (uint256 => mapping(address => uint256)) public poolReferrerSales;

  // EVENTS
  event Purchase(uint256 indexed poolId, address indexed user,  
        uint256 amount, address indexed token, address currencyToken, uint256 cost);
  ...
  event WinnerAnnounced(uint256 indexed poolId, address indexed winner, 
        uint256 salesAmount, address token, 
        address currencyToken, uint256 rewardAmount);

  event PoolCreated(uint256 indexed poolId, address indexed token, 
    address indexed currencyToken, uint256 exchangeRate, 
    uint256 amount,
    uint256 weight, uint64 expiry);  
  ...
  function _newPool (address _poolOwner, IERC20 _token, 
    ...
    uint256 _reward, 
    uint256 _weight, uint64 _expiry) internal returns (uint256 _pid){
  ...
  function _openNewPool (address _poolOwner, IERC20 _token, 
    address _currencyToken, uint112 _exchangeRate,
    uint256 _reward, uint256 _weight, uint64 _expiry) internal returns (uint256 pid){
  ...
  function openNewPoolByOwner (IERC20 _token, address _currencyToken, uint112 _exchangeRate,
    uint256 _reward, uint256 _weight, uint64 _expiry) onlyOwner external returns (uint256 pid){
  ...
  function openNewPool (IERC20 _token, address _currencyToken, uint112 _exchangeRate,
    uint256 _reward, uint256 _weight, uint64 _expiry, bytes32 name) external returns (uint256 pid){

    // require (_weight >= minPoolCost, "Need more YouBetToken");

    address poolOwner = msg.sender;

    _reward = _safeTokenTransfer(poolOwner, address(_token), _reward);
  ...
  /**
    ...
    **/
  function buyTokenWithETH (uint256 _pid, uint256 _purchaseAmount, 
    ...
    uint256 cost = _purchaseAmount.mul(pools[_pid].exchangeRate)/1e18;
    ...
    if(referrer != address(0)) {
      uint256 newReferrarSales = _purchaseAmount.add(poolReferrerSales[_pid][referrer]);
  ...
  /**
    ...
    * @param _purchaseAmount amount in token to purchase
    **/
  function buyToken (uint256 _pid, uint256 _purchaseAmount, address referrer) external {
    address _customer = msg.sender;
    require (pools[_pid].status == PoolStatus.OPEN, "lottery pool not open");
    require (now < pools[_pid].expiry, "lottery pool expired");
    ...
    uint256 cost = _purchaseAmount.mul(pools[_pid].exchangeRate)/1e18;
    uint256 balanceDelta = _safeTokenTransfer(_customer, _poolCurrencyToken, cost);
  ...
  function processContestWinner (uint256 _pid, uint256 _contestReward) internal {
  ...
  /**
    ...
    **/
  function closePool (uint256 _pid) external {
    ...
    IERC20 _pooltoken = IERC20(pools[_pid].token);
    IERC20 _poolCurrencyToken = IERC20(pools[_pid].currencyToken);
    uint256 _poolReward = pools[_pid].reward;
    uint256 _poolExchangeRate = pools[_pid].exchangeRate;
    uint256 _poolBalance = pools[_pid].balance;

    uint256 _fees = _poolBalance/100;
    uint256 _contestReward = _poolBalance/200;
    uint256 _poolOwnerRevenue = _poolBalance - _fees - _contestReward;
