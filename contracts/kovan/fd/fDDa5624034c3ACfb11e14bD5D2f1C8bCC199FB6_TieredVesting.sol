...
contract TieredVesting {
    ...
    */
    ...
    function collect() public onlyClaimant returns(uint256) {
        ...
        ERC20 token = ERC20(tokenAddress);
        uint256 tiersCompleted = elapsed.div(tierLength);
        if (tiersCompleted == tierLength) {
                token.transfer(claimant, total);
                claimed = total;
        } else if (tiersCompleted > 0) {
            while (tiersCompleted > 0) {
                token.transfer(claimant, perTier);
                claimed = claimed.add(perTier);
                tiersCompleted = tiersCompleted.sub(1);
            }
        }
    }
}