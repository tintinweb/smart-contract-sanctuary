...
contract AlgebraFactory is IAlgebraFactory {
    ...
    bytes32 internal constant POOL_INIT_CODE_HASH = 0x9b5dde4d582d7ce86d30c3608954377de870b739113a39f3728c852cc255648b;
...
library DataStorage {
    uint32 public constant TIME_AGO = 24 * 60 * 60;
    struct Timepoint {
        ...
        if (lte(time, oldest.blockTimestamp, time - TIME_AGO)) {
            if (last.blockTimestamp + TIME_AGO > time) {
                (int56 bTick, , , ) = getSingleTimepoint(
                    ...
                    tick,
                    index,
                    liquidity //TODO:MB last - TIME_AGO?
                );
    ...
    function binarySearch(
        ...
        while (true) {
            ...
            if (lte(time, beforeOrAt.blockTimestamp, target)) {
                ...
                if (lte(time, target, atOrAfter.blockTimestamp)) break;
                l = i + 1;
            } else {
                r = i - 1;
            }
    ...
    function write(
        ...
        self[indexUpdated] = transform(last, blockTimestamp, tick, liquidity, avgTick, volumePerLiquidity);
    }
}
