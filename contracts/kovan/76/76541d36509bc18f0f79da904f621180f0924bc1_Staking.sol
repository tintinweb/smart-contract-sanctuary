...
contract Staking {
    ...
    function depositIntoPool(uint256 _depositAmount) public {
		...
		require(erc20.transferFrom(msg.sender, address(this), _depositAmount) == true, "transferFrom did not succeed. Are we approved?");
    ...
    function withdrawFromPool(uint256 _amount) public {
		...
		uint256 amountToSend = SafeMath.div(SafeMath.mul(contractBalance, amountToWithdraw), totalBalance);
	...
	function _unDebitedDrips(Provider memory user) internal view returns (uint256) {
	    ...
	    return SafeMath.div(SafeMath.mul(SafeMath.mul(SafeMath.sub(now, uint256(user.lastUpdateAt)), emissionRate), user.balance), 1e18);
	}
