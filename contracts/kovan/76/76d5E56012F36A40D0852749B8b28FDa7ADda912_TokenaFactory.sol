...
contract TokenaFactory is ITokenaFactory, Ownable, ReentrancyGuard {
	...
	function changePromotedPool(address pool, bool flag) external onlyOwner {
		...
		if (flag) {
			require(counter < 5, "Already has 5 promoted");
			require(!promotedPools[pool], "Already promoted");
			counter++;
			promotedPools[pool] = flag;
		} else {
			require(counter != 0, "There are no promoted");
			require(promotedPools[pool], "Already not promoted");
			counter--;
			promotedPools[pool] = flag;
		}
