...
abstract contract Declaration is Global {
    ...
    mapping(address => mapping(uint256 => uint256)) public burntAmount;
    mapping(uint256 => uint256) public dailyTotalBurntAmount;
...
abstract contract StakingToken is ReferralToken {
    ...
    */
    function _createStake(
        ...
        burntAmount[_staker][_startDay] = burntAmount[_staker][_startDay] + _stakedAmount;
        dailyTotalBurntAmount[_startDay] = dailyTotalBurntAmount[_startDay] + _stakedAmount;
    ...
    */
    ...
    function getBurntAmount(uint256 _startDay, uint256 _endDay) public view returns (uint256) {
        require(_startDay > 0 && _endDay > _startDay);
        uint256 burntAmount = 0;
        
        for (uint256 i = _startDay; i <= _endDay; i++) {
            burntAmount = burntAmount.add(dailyTotalBurntAmount[i]);
        }
...
contract SwappToken is StakingToken {
    ...
    function burnLiquidityTransfomer() external {
        require(transformerGateKeeper == msg.sender);
