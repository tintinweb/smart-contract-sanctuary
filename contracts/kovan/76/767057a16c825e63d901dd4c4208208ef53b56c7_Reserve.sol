...
contract Reserve is Ownable {
    ...
    event UniswapAmountsOut(uint256 _amount1, uint256 _amount2);
    ...
    function calculateSlippage(uint256 _amount) external onlyOwner {
        // calculate slippage
        uint256[] memory amountsOut = uniswapRouter.getAmountsOut(_amount, omsUsdcPath);
        emit UniswapAmountsOut(amountsOut[0], amountsOut[1]);
    }

    function transfer(uint256 _amount) external onlyOwner {
        usdc.safeTransfer(cascade, _amount);
        emit Transferred(address(cascade), address(usdc), _amount);
    }

    function transferReserve() external onlyOwner {
        oms.safeTransfer(reserveNg, minOmsReserveAmount);
        emit Transferred(address(reserveNg), address(oms), minOmsReserveAmount);

        omsx.safeTransfer(reserveNg, minOmsxReserveAmount);
