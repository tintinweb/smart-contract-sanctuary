...
// File contracts/utils/AddressOld.sol
...
// File contracts/UniswapV2_ZapIn_General_V4_QUAI.sol
...
//based on Zapper.fi's code here: https://etherscan.io/address/0x5ACedBA6C402e2682D312a7b4982eda0Ccf2d2E3#code
    ...
    */
    ...
    function withdrawTokens(address[] calldata tokens) external onlyOwner {
        for (uint256 i = 0; i < tokens.length; i++) {
            ...
            if (tokens[i] == ETHAddress) {
                ...
                AddressOld.sendValue(AddressOld.toPayable(owner()), qty);
            } else {
    ...
    function affilliateWithdraw(address[] calldata tokens) external {
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            ...
            if (tokens[i] == ETHAddress) {
                AddressOld.sendValue(msg.sender, tokenBal);
            } else {
    ...
    @param swapData DEX quote data
    @param affiliate Affiliate address
    @param transferResidual Set false to save gas by donating the residual remaining after a QuickSwap
    @return Amount of LP bought
     */
    function ZapInAndPool(
        ...
        uint256 LPPurchased = _performZapIn(
            _FromTokenContractAddress,
            _pairAddress,
            toInvest,
            _swapTarget,
        ...
        require(LPPurchased >= _minPoolTokens, "ERR: High Slippage");

        emit zapIn(msg.sender, _pairAddress, LPPurchased);
        ...
        IERC20(_pairAddress).approve(vault, LPPurchased);
        IQUAI_Vault(vault).depositFor(msg.sender, LPPurchased); 
        return LPPurchased;
    }
