...
contract USMDIA is USMTemplate{
    constructor(DiaOracleAdapter oracle_, address[] memory optOut_, string memory syntheticFeed_, string memory collateralFeed_) 
        USMTemplate(oracle_, optOut_, string(abi.encodePacked("DIA synthetic for ticker", syntheticFeed_)), string(abi.encodePacked("DIA-", syntheticFeed_)))
        {}
}
...
contract USMTemplate is IUSM, Oracle, ERC20Permit, WithOptOut, Delegable {
    ...
    constructor(Oracle oracle_, address[] memory optedOut_, string memory symbol, string memory name)
