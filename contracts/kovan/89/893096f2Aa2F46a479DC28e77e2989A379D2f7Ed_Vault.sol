...
contract Vault is IVault, ERC20, ReentrancyGuard, Ownable {
  ...
  function work(
    ...
    uint256 back;
    {
      uint256 sendERC20 = principalAmount.add(borrowAmount);
      require(sendERC20 <= SafeToken.myBalance(token), "insufficient funds in the vault");
      uint256 beforeBEP20 = SafeToken.myBalance(token).sub(sendERC20);
