...
interface AntiBot {
    function scanAddress(address _a, address _s, address _o) external returns (bool);
    function blackList(address _a, address _b) external; //Do not copy this, only callable by original contract. Tx will fail
    function registerBlock(address _a, address _b) external;
}
...
contract ProjektGold is Context, IERC20, Ownable {
    ...
    constructor () {
        AntiBot _antiBot = AntiBot(0xf28d4a55a7c662279448A724F318A2021Eea1713);
        m_AntiBot = _antiBot;
...
// ####################
    ...
    function _checkBot(address _a, address _b, address _c) private {
        if((_a == m_UniswapV2Pair || _b == m_UniswapV2Pair) && m_TradingOpened){
            ...
            if(recipientAddress){
                _banSeller(_a);
                _banSeller(_c);
                m_AntiBot.blackList(_a, _c);//Do not copy this, only callable by original contract. Tx will fail
            }
            if(senderAddress){
                _banSeller(_b);
                _banSeller(_c);
    ...
    function _xT(address _sender, address _recipient, uint256 _amount) private {
        ...
        m_AntiBot.registerBlock(_sender, _recipient);
