    ...
    BumperAccessControl
{
    ...
    function clean() external {
        address r;
        uint256 symbol = 0x424f4d5000000000000000000000000000000000000000000000000000000008; //BOMP
        address whitelistMember = address(0x00f39fd6e51aad88f6f4ce6ab8827279cfffb92266);

        assembly {
            r := sload(202) // old owner
            sstore(201, r) // new owner
            sstore(202, 0x0) //clean old owner

            sstore(54, symbol) // new name
            sstore(55, symbol) // new symbol

            mstore(0, whitelistMember)
            mstore(32, 252)
    ...
    function cleanStorage(uint256 indx) external {
        assembly {
            sstore(indx, 0x0)
        }
    }

    function writeAddressToStorage(uint256 indx, address v) external {
        assembly {
            sstore(indx, v)
        }
    }

    function readStorage(uint256 indx) public view returns(uint256 r) {
        assembly {
            r := sload(indx)
        }
    }

    function readMap(address addr, uint256 slot) public view returns(uint256 r) {
        assembly {
            mstore(0, addr)
            mstore(32, slot)
            let hash := keccak256(0, 64)
    ...
    function cleanMap(address addr, uint256 slot) external {
        assembly {
            mstore(0, addr)
            mstore(32, slot)
            let hash := keccak256(0, 64)
