...
contract CathyBank is Ownable {
    ...
    function getPHXRewardsShare(
        ...
        uint256 distributableRewards = PHXIncome;
        uint256 _rewardsShare = getShares(_shareholder);

        return _rewardsShare;
    }

    function addPHXIncome(uint256 _PHXIncome) external onlyOwner {
        PHXIncome = PHXIncome.add(_PHXIncome);
    }
    function computeShareRewards() external onlyOwner {
        ...
        PHXIncome = PHXIncome.sub(distributedRewards);
    ...
    function payLoanFullPHX() public {
        ...
        PHXReserves = PHXReserves.sub(accruedInterest);
    ...
    function computeCollateralPercent(uint256 _ETHAmount, uint256 _PHXAmount)
        ...
        returns (uint256)
    {
        return _ETHAmount.mul(ETHPHXConversionRate) / (10**16) / _PHXAmount;
    }
    ...
    function transferRewards(address _shareholder) internal {
        ...
        if (_shareHolderRewards <= 0) {
            _shareHolderRewards = getShares(_shareholder);
        }
    ...
    function getShares(address _shareholder) public view returns (uint256) {
        ...
        return ((balance.mul(PHXIncome)).div(totalSupply));
    }
