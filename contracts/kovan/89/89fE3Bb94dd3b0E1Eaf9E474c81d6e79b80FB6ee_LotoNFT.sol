...
contract LotoNFT is ERC721, ERC721Burnable, Ownable, VRFConsumerBase {
    ...
    uint public constant WINNERS_COUNT = 61;
    ...
    function allWinners() public view returns (uint[] memory winners) {
        uint _winnersCount = winners.length;
        winners = new uint[](_winnersCount);
    ...
    function fulfillRandomness(bytes32, uint randomness) internal override {
        for (uint i = 0; i < WINNERS_COUNT; i++) {
            _winners.push(nextWinner(uint(keccak256(abi.encode(randomness, i)))));
        }
    }
}
