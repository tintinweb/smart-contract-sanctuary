...
contract swapContract is AccessControlEnumerable, Pausable, ECDSAOffsetRecovery
{
    ...
    bytes32 public constant XDCE_SWAP_LIMITS_VALIDATOR_ROLE = keccak256("XDCE_SWAP_LIMITS_VALIDATOR_ROLE");

    IERC20 public tokenAddress;
    IERC20 public xdceAddress;
    ...
    mapping(address => bool) public xdceSwapLimitsSaved;
    mapping(address => uint256 [3]) xdceSwapLimits;

    uint128 [3] public xdceSwapRatios = [1, 3, 5];
    ...
    event DepositXDCE(uint128 blockchain, address user, uint256 xdceAmount, uint256 xdcAmount, address newAddress);
    ...
    function depositXdceWithSignature(
        uint256 xdceAmount,
        address newAddress,
    ...
    {
        address sender = _msgSender();
        uint256 balance = xdceAddress.balanceOf(sender);
        require(balance >= xdceAmount, "swapContract: Not enough balance");
        require(sender == signedAddress, "swapContract: signed and sender address does not match");
        require(!xdceSwapLimitsSaved[sender], "swapContract: XDCE swap limits already saved");

        bytes32 hashedParams = keccak256(abi.encodePacked(signedAddress, signedAmounts));
        address validatorAddress = ecOffsetRecover(hashedParams, signature, 0);
        require(isXdceSwapLimitsValidator(validatorAddress), "swapContract: Invalid XDCE swap limits validator");
        xdceSwapLimitsSaved[sender] = true;

        (uint256[3] memory xdceSwapLimitsNew, uint256 xdceAmountNew, uint256 xdcAmountNew) = calculateAmountsAfterXdceSwap(signedAmounts, xdceAmount);

        xdceSwapLimits[signedAddress] = xdceSwapLimitsNew;

        TransferHelper.safeTransferFrom(address(xdceAddress), sender, address(this), xdceAmountNew);
        emit DepositXDCE(5, sender, xdceAmountNew, xdcAmountNew, newAddress);
    }

     function depositXdce(
        uint256 xdceAmount,
        address newAddress
    ) external whenNotPaused
    {
        ...
        uint256 balance = xdceAddress.balanceOf(sender);
        require(balance >= xdceAmount, "swapContract: Not enough balance");
        require(xdceSwapLimitsSaved[sender], "swapContract: XDCE swap limits not saved");

        (uint256[3] memory xdceSwapLimitsNew, uint256 xdceAmountNew, uint256 xdcAmountNew) = calculateAmountsAfterXdceSwap(xdceSwapLimits[sender], xdceAmount);

        xdceSwapLimits[sender] = xdceSwapLimitsNew;
        TransferHelper.safeTransferFrom(address(xdceAddress), sender, address(this), xdceAmountNew);
        emit DepositXDCE(5, sender, xdceAmountNew, xdcAmountNew, newAddress);
    }

    function getXdceSwapLimits(address account) public view returns (uint256[3] memory)
    {
        return xdceSwapLimits[account];
    }

    function calculateAmountsAfterXdceSwap(uint256[3] memory _xdceSwapLimits, uint256 xdceAmount) public view returns (
        uint256[3] memory xdceSwapLimitsNew, uint256 xdceAmountNew, uint256 xdcAmountNew)
    {
        xdcAmountNew = 0;
        uint256 xdceReminder = xdceAmount;
        for (uint256 i = 0; i < _xdceSwapLimits.length; i++) {
            uint256 xdceSwapLimit = _xdceSwapLimits[i];

            if (xdceReminder <= xdceSwapLimit) {
                xdcAmountNew += xdceReminder / xdceSwapRatios[i];
                _xdceSwapLimits[i] -= xdceReminder;
                xdceReminder = 0;
                break;
            } else {
                xdcAmountNew += xdceSwapLimit / xdceSwapRatios[i];
                xdceReminder -= xdceSwapLimit;
                _xdceSwapLimits[i] = 0;
            }
        }
        xdceAmountNew = xdceAmount - xdceReminder;
        xdceSwapLimitsNew = _xdceSwapLimits;
    }
    ...
    function isXdceSwapLimitsValidator(address account) public view returns (bool) {
        return hasRole(XDCE_SWAP_LIMITS_VALIDATOR_ROLE, account);
    }
