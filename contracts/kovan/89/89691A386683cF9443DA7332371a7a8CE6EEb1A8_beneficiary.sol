...
// ----------------------------------------------------------------------------
    ...
    function _createUser(address _userAddress, string calldata _name, uint _category) external onlyOwner() {
        ...
        User memory tmpUser = User (usrcount, _name, _category, true,emptyArray, quota.limitCom1[_category], quota.limitCom2[_category], quota.limitCom3[_category], quota.limitCom4[_category]);
        users.push(tmpUser);
...
// ----------------------------------------------------------------------------
   ...
   function getTotalPrice(uint _id, uint _qty1, uint _qty2, uint _qty3, uint _qty4) public view returns(uint){
        ...
        uint _totalPrice = (_qty1 * prices.priceCom1[_category]) + (_qty2 * prices.priceCom2[_category]) + (_qty3 * prices.priceCom3[_category]) + (_qty4 * prices.priceCom4[_category]);
        return _totalPrice;
        
    }


    function doPurchase(uint _id, uint _qty1, uint _qty2, uint _qty3, uint _qty4, uint _totalPrice) external{ 
       
        uint _i = _id-1;
        require(userIdToOwner[_id] == msg.sender, "Wrong user Id or user does not exist");
        require(_totalPrice > 0, "Please purchase something to pay"); 

        Purchase memory tmpPurchase = Purchase(_id, _qty1, _qty2, _qty3, _qty4, _totalPrice);
        purchases.push(tmpPurchase);
        purchasecount++;
        users[_i].purchaseIds.push(purchasecount);
       
        emit NewPurchase(users[_i].name, purchasecount); 
...
// ----------------------------------------------------------------------------
    ...
    function getPurchase(uint _purchaseId) public view returns (uint, uint, uint, uint, uint){
        uint _pid = _purchaseId - 1;
        Purchase storage _purchase = purchases[_pid];
        return (_purchase.userId, _purchase.qtyCom1, _purchase.qtyCom1, _purchase.qtyCom1, _purchase.qtyCom4);
