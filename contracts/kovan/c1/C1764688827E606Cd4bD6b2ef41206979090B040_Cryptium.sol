...
contract Cryptium is CrptERC20, Control, ReentrancyGuard {
	...
	function rebase() external nonReentrant initialized {
		...
		if (isInflRebase) {
			...
			// _lastRebase = TimeLibrary.addMonths(_startTime, monthsPassAll);
	...
	function _getCurrentPrice() internal view returns (uint256) {
		...
		uint256 currentPrice = (uint256(currentPriceSqrt) * uint256(currentPriceSqrt) * 1000) >> (96 * 2);

		if (token0 != address(this)) {
			currentPrice = 1000000 / currentPrice;
		} 
	...
	function _getMonthsPass() private view returns (uint256, uint256) {
		// uint256 monthsPassedAll = TimeLibrary.diffMonths(_startTime, block.timestamp);
		// uint256 monthsPassedFromLastRebase = TimeLibrary.diffMonths(_lastRebase, block.timestamp);
		uint256 monthsPassedAll = (block.timestamp - _startTime) / (5 minutes);
		uint256 monthsPassedFromLastRebase = (block.timestamp - _lastRebase) / (5 minutes);
	 ...
	 */
	function _getAveragePrices() private returns (uint256, uint256) {
	 ...
	 * @return newPrice
	 */
	function _getTWAT() private returns (uint256 oldPrice, uint256 newPrice) {
		uint32 valueCount = 3;
		uint32[] memory secondAgos = new uint32[](valueCount);
		secondAgos[2] = 2 * period;
		secondAgos[1] = period;
		secondAgos[0] = 0;
	...
	function _getPrice(
		...
		uint32 period,
		uint8 decDiff
	) internal returns (uint256 oldPrice, uint256 newPrice) {
		...
		oldPrice = (uint256(oldSqrtPrice) * uint256(oldSqrtPrice) * 1000) >> (96 * 2);
		newPrice = (uint256(newSqrtPrice) * uint256(newSqrtPrice) * 1000) >> (96 * 2);
		address token0 = IUniswapV3Pool(_poolAddress).token0();
		emit Log("oldPrice newPrice from getPrice", oldPrice, newPrice, 0, 0);
		if (token0 != address(this)) {
			oldPrice = 1000000 / oldPrice;
