...
contract Redeployer {
    ...
    function redeploy(
        ...
        require(size >= argumentsOverride.length, "Invalid arguments overriding length");
        ...
        for (uint256 wordIndex = 0; wordIndex < words; wordIndex++) {
            assembly {
                ...
                mstore(add(add(bytecode, 0x20), sub(size, mul(0x20, sub(words, wordIndex)))), mload(argOffset))
            }
        }
