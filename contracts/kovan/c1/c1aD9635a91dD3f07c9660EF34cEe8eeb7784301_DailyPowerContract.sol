...
contract DailyPowerContract is Ownable, ChainlinkClient {
    ...
    function calculateFundingAmounts() public onlyOwner {
        ...
        //buyer price
        //uint256 buyerAmountCad = ((price * volume * 24)) * (10**13); // usign 24 hours for now, but need to account for daylight savings days with 23 or 25 hours
        buyerFundCadCentsAmount = ((price * volume * 24)) / 1000;
        uint256 buyerAmountCadAdjusted = buyerFundCadCentsAmount * (10**16); // 16 here instead of 18 because we are already in cents

        //seller price
        //uint256 sellerAmountCad = (((priceCap - price) * volume * 24)) * (10**13);
        sellerFundCadCentsAmount = (((priceCap - price) * volume * 24)) / 1000;
        ...
        //buyer cad to usd
        uint256 buyerUsdAmount = buyerAmountCadAdjusted * adjustedCadToUsdPrice;
        ...
        // //convert usd to cad
        // latestUsdToCadPrice = (1 * 10**16) / uint256(latestCadToUsdPrice); // invert the conversion rate
        // adjustedCadToUsdPrice = uint256(latestUsdToCadPrice) * 10**10; // it comes with 8 decimals already, so add 10 to get to 18 decimals

        // //buyer cad to usd
        // uint256 buyerUsdAmount = (buyerAmountCadAdjusted * 10**18) /
        //     adjustedCadToUsdPrice;

        // //seller cad to usd
        // uint256 sellerUsdAmount = (sellerAmountCadAdjusted * 10**18) /
        //     adjustedCadToUsdPrice;
    ...
    // function setsettledPriceCadCents(uint256 settled_price)
    //     public
    //     onlyOwner
    //     returns (bytes32 requestId)
    // {
    //     require(contract_state == CONTRACT_STATE.FULLY_FUNDED);
    //     settledPriceCadCents = settled_price;
    //     contract_state = CONTRACT_STATE.SETTLED_PRICE_ADDED;
    // }

    function requestsettledPriceCadCents()
        public
        onlyOwner
        returns (bytes32 requestId)
    {
        ...
        request.add(
            "get",
            "https://ar5jsismni.execute-api.ca-central-1.amazonaws.com/default/getAesoCurrentSupplyForSunburst"
        );

        request.add("path", "children.0.children.0.value");
    ...
    function calculateSettlementAmounts() public onlyOwner {
        ...
        if (settledPriceCadCents > price) {
            winner = buyer;
            buyerRefundEthAmount = buyerFundEthWeiAmount + winningsEthAmount;
            sellerRefundEthAmount = sellerFundEthWeiAmount - winningsEthAmount;
        }
        if (settledPriceCadCents < price) {
            winner = seller;
            buyerRefundEthAmount = buyerFundEthWeiAmount - winningsEthAmount;
