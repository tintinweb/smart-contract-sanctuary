...
contract Ko {
    ...
    mapping(uint32 => tournamentData) public tournaments;
    ...
    function declareWinner(uint32 participant, uint32 tournamentId) external {
        ...
        uint32 winnerId = ((participant < (currentTournament.realParticipantsCount*2 - currentTournament.evenCount))?
            (participant/(uint32(2)**currentTournament.round)):                                                                                         //standart
            ((participant - (currentTournament.realParticipantsCount*2 - currentTournament.evenCount)/2)/(uint32(2)**currentTournament.round-1))        //byes -> not halved in first round, rather offset
        )+ currentTournament.winnersOffset;
    ...
    function getWinner(uint32 id, uint32 tournamentId) public view returns (uint32, bool){
    ...
    function getPairing(uint32 id, uint32 tournamentId) public view returns (uint32){
