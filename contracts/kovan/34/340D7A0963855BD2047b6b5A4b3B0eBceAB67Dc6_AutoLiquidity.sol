...
contract AutoLiquidity is UniV3Liquidity {
    ...
    modifier onlyVault() {
        require(extAuthorize(), "UniV3Liquidity.onlyVault: !vault");
        _;
    }
    ...
    /// @param fromAmount token amount
    /// @param toAmount token amount
    function swapAfter(
        address fromToken,
        address toToken,
        uint256 fromAmount,
        uint256 toAmount) internal override {
        address feeToken = fromToken;
        uint256 swapAmount = fromAmount;
        if (toToken == address(ioToken)) {
            swapAmount = toAmount;
            feeToken = toToken;
        }
        uint256 fee = vault.calcRatioFee(SmartPoolStorage.FeeType.TURNOVER_FEE, swapAmount);
        if (feeToken != address(ioToken) && fee > 0) {
            fee = exactInput(feeToken, address(ioToken), fee, 0);
        }
        if (fee > 0) {
            ioToken.safeTransfer(getRewards(), fee);
        }
    }
    ...
    function collectAfter(
        ...
        if (token0 != address(ioToken) && fee0 > 0) {
            fee0 = exactInput(token0, address(ioToken), fee0, 0);
        }
        if (token1 != address(ioToken) && fee1 > 0) {
            fee1 = exactInput(token1, address(ioToken), fee1, 0);
        }
    ...
    /// @param tokens withdraw
    function withdraw(address[] calldata tokens) external onlyGovernance{
        for(uint256 i=0;i<tokens.length;i++){
            IERC20 token=IERC20(tokens[i]);
            uint256 balance = token.balanceOf(address(this));
            if (balance > 0) {
                token.safeTransfer(getGovernance(), balance);
            }
        }
    }
...
contract UniV3Liquidity is GovIdentity {
    ...
    modifier onlyAuthorize() {
        ...
        || isStrategist(msg.sender)
            || extAuthorize(), "UniV3Liquidity.onlyAuthorize: !authorize");
        _;
    }

    //Only position owner
    modifier onlyPositionOwner(uint256 tokenId) {
        require(positionOwners[tokenId] == msg.sender || extAuthorize(), "UniV3Liquidity.onlyPositionOwner: !position owner");
        _;
    }
    ...
    function exactInput(
        ...
        require(tokenLimit[msg.sender][tokenIn] >= amountIn, 'UniV3Liquidity.exactInput: available tokenIn limit');
        ...
        swapAfter(tokenIn,tokenOut,amountIn,amountOut);
    ...
    function exactOutput(
        ...
        require(tokenLimit[msg.sender][tokenIn] >= amountIn, 'UniV3Liquidity.exactOutput: available tokenIn limit');
        tokenLimit[msg.sender][tokenIn] -= amountIn;
        tokenLimit[msg.sender][tokenOut] += amountOut;
        swapAfter(tokenIn,tokenOut,amountIn,amountOut);
        emit Swap(msg.sender, tokenIn, tokenOut, amountIn, amountOut);
    ...
    {
        require(tokenLimit[msg.sender][token0] >= amount0Desired, 'UniV3Liquidity.mint: available token0 limit');
        require(tokenLimit[msg.sender][token1] >= amount1Desired, 'UniV3Liquidity.mint: available token1 limit');
        (
        uint256 tokenId,
        uint128 liquidity,
        uint256 amount0,
    ...
    function increaseLiquidity(
        ...
        uint256 amount0Min,
        uint256 amount1Min
    ) public onlyAuthorize onlyPositionOwner(tokenId) returns (
        uint128 liquidity,
        uint256 amount0,
        uint256 amount1
    ){
        ...
        require(tokenLimit[po][token0] >= amount0Desired, 'UniV3Liquidity.mint: available token0 limit');
        require(tokenLimit[po][token1] >= amount1Desired, 'UniV3Liquidity.mint: available token1 limit');
    ...
    function decreaseLiquidity(
        ...
        uint256 amount0Min,
        uint256 amount1Min
    ) public onlyAuthorize onlyPositionOwner(tokenId) returns (uint256 amount0, uint256 amount1){
