...
contract HODLIFE is ERC20, Ownable {
    ...
    function updateFees(uint8 _lFee, uint8 _rewardFee, uint8 _buybackFee, uint8 yieldFee) onlyOwner external{
        require(_lFee + _rewardFee + _buybackFee + yieldFee <= 20);
        liquidityFee = _lFee;
        BNBRewardsFee = _rewardFee;
        buyBackFees = _buybackFee;
        yfFees = yieldFee;
        totalFees = _lFee + _rewardFee + _buybackFee + yieldFee;
    ...
    function _transfer(
        ...
        if( 
            ...
            if(from == uniswapV2Pair){
                require(lastTransferTime[to].add(10 minutes) >= block.timestamp);
                lastTransferTime[to] = block.timestamp;
            }
            else{
                require(lastTransferTime[from].add(10 minutes) >= block.timestamp);
                lastTransferTime[to] = block.timestamp;
