...
contract AtomicSwap {
    ...
    )
  {
    ...
    inputTokenInstance.safeIncreaseAllowance(address(uniswapRouter), tokenAmountIn);

    collateralOut = (inputIsExact
    ...
    collateralInstance.safeIncreaseAllowance(address(synthereumPool), collateralOut);
    ...
    )
  {
    ...
    synthToken.safeIncreaseAllowance(address(synthereumPool), numTokens);

    redeemParams.recipient = address(this);
    (collateralRedeemed, ) = synthereumPool.redeem(redeemParams);

    collateralInstance.safeIncreaseAllowance(address(uniswapRouter), collateralRedeemed);

    outputTokenAmount = (inputIsExact
      ? uniswapRouter.swapExactTokensForTokens(
        collateralRedeemed,
        amountTokenOut,
        tokenSwapPath,
