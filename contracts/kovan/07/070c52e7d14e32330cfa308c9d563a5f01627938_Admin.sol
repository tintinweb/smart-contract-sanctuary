...
contract Admin{
     ...
     */
    function transferERC1155(address _tokenAddress, address _from, address _to, uint nftId, uint value, bytes _bytes)public returns (bool){
        require(_to != address(0), "_to is the zero address");
        bytes4 id=bytes4( keccak256( "safeTransferFrom(address,address,uint256,uint256,bytes)" ) );
        _tokenAddress.call(id, _from, _to, nftId, value, _bytes);
        return true;
    }
