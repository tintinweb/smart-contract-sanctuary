...
contract TestMLBGameConsumer is ChainlinkClient {
    ...
    struct Game {
        ...
        string awayTeam;
        string homeTeam;
        int256 awayTeamMoneyLine;
        int256 homeTeamMoneyLine;
    }
    ...
    function getGame() external view returns (Game memory _game) {
        (uint256 gameID, string memory status, string memory dateTime, string memory awayTeam, string memory homeTeam, int256 awayTeamMoneyLine, int256 homeTeamMoneyLine) = 
                abi.decode(gameBytes, (uint256, string, string, string, string, int256, int256));
        return Game({
            gameID: gameID,
            status: status,
            dateTime: dateTime,
