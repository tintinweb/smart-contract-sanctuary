...
contract CrypTOW {
    ...
    function startRound() private {
        require(roundDatas[currentRound + 1] > 0, "During time was not set.");
    ...
    function getRoundDetail(uint256 round) public view returns (address[] memory addrs, uint256[] memory times, uint256[] memory amounts, uint16[] memory teams, uint256[] memory fees) {
        require(round != 0, 'Invalid Address');
        uint256 count = 0;
        for (uint256 i = 0; i < betInfos.length; i++) {
            if (betInfos[i].round == round) {
                count++;
            }
        }
        ...
        for (uint256 i = 0; i < betInfos.length; i++) {
            if (betInfos[i].round == round) {
                addrs[index] = betInfos[i].addr;
                times[index] = betInfos[i].time;
        ...
        return (addrs, times, amounts, teams, fees);
    }
    
    function getHistory(address addr) public view returns (uint256[] memory rounds, uint256[] memory times, uint256[] memory amounts, uint16[] memory teams, uint256[] memory fees) {
        ...
        return (rounds, times, amounts, teams, fees);
    }
}