...
library SynthereumInterfaces {
  ...
  bytes32 public constant AtomicSwapProxy = 'AtomicSwapProxy';
}
...
// File: @jarvis-network/atomic-swap/contracts/v2/interfaces/IProxy.sol
...
// File: libs/legacy-currency-contracts/contracts/interfaces/IFixedRateCurrency.sol
...
interface IFixedRateCurrency {
  /** 
      @notice - Mints fixed rate synths against the deposited peg synth
      @param pegTokenAmount - the amount of peg synth to input
      @param recipient - recipient of minted tokens
   */
  function mintFromPegSynth(uint256 pegTokenAmount, address recipient)
    external
    returns (uint256 numTokensMinted);

  /** 
    @notice - Burns fixed rate synths and unlocks the deposited peg synth
    @param fixedRateAmount - the amount of fixed rate tokens to burn
    @param recipient - recipient of the unlocked collateral
   */
  function redeemToPegSynth(uint256 fixedRateAmount, address recipient)
    external
    returns (uint256 tokensRedeemed);

  /** 
    @notice - Mints fixed rate synths from synthereum collateral.
    @param synthereumPool - Synthereum Pool used to mint the peg token;
    @param mintParams - MintParams of the synthereum pool
  */
  function mintFromSynthereumCollateral(
    ISynthereumPoolOnChainPriceFeed synthereumPool,
    ISynthereumPoolOnChainPriceFeed.MintParams memory mintParams
  ) external;

  /** 
    @notice - Redeem synthereum collateral by burning fixed rate synths.
    @param synthereumPool - Synthereum Pool address used to burn the peg token and unlock collateral;
    @param redeemParams - RedeemParams of the synthereum pool
  */
  function redeemToSynthereumCollateral(
    ISynthereumPoolOnChainPriceFeed synthereumPool,
    ISynthereumPoolOnChainPriceFeed.RedeemParams memory redeemParams
  ) external;

  /** 
    @notice - Mints fixed rate currency from a synthereum synthetic token
    @param inputSynthAddress - Address of the input synthetic token
    @param synthereumPool - Address of the input synthereum pool for the exchange
    @param exchangeParams - ExchangeParams of the synthereum pool
  */
  ...
  /** 
    ...
    @param fixedRateAmount - Amount of fixed rate currency to input
    @param synthereumPool - Address of the input synthereum pool for the exchange
    @param exchangeParams - ExchangeParams of the pool
  */
  function swapForSynth(
    uint256 fixedRateAmount,
    ISynthereumPoolOnChainPriceFeed synthereumPool,
    ISynthereumPoolOnChainPriceFeed.ExchangeParams memory exchangeParams
  ) external;

  /**
    @notice Leverages AtomicSwapV2 to do ERC20/ETH -> synthereum collateral -> synthereum peg -> fixedSynth 
    @param dexId id of the dex selected for the trade within AtomicSwapProxy contract
    ...
    @param extraParams: dynamic-size bytes to encode extra parameters specific to an atomic swap instance
    @param tokenSwapPath: route token addresses 
    @param synthereumPool - Address of the input synthereum pool for the exchange
    @param mintParams - MintParams of the synthereum pool
   */
  function swapAndMint(
  ...
  /**
    @notice Leverages the AtomicSwapV2 to route fixedSynth -> synthetic peg token -> synthereum collateral -> ERC20/ETH
    @param dexId id of the dex selected for the trade within AtomicSwapProxy contract
    @param fixedRateAmountIn amount of fixed rate tokens to redeem
    @param amountOutMin minimum amount of output tokens accepted 
    @param tokenSwapPath: route token addresses 
    @param synthereumPool - Address of the input synthereum pool for the exchange
    @param redeemParams - RedeemParams of the synthereum pool
  */
  ...
  /**
    @notice Allows to swap between fixed rate currencies with the same peg token
    @param targetFixedRate Address of the output fixed rate token
    @param amountIn amount of input tokens
    @return numTokensMinted amount of target fixed rate tokens
   */
  function swapToFixedRateSamePeg(address targetFixedRate, uint256 amountIn)
    external
    returns (uint256 numTokensMinted);

  /**
    @notice Allows to swap between fixed rate currencies with a different peg token
    @param targetFixedRate Address of the output fixed rate token
    @param inputTokenSynthereumPool Address of the input synthereum pool for the exchange
    @param exchangeParams - ExchangeParams of the synthereum pool
    @param amountIn amount of input tokens
    @return numTokensMinted amount of target fixed rate tokens
   */
  function swapToFixedRateDifferentPeg(
    address targetFixedRate,
    ISynthereumPoolOnChainPriceFeed inputTokenSynthereumPool,
    ISynthereumPoolOnChainPriceFeed.ExchangeParams memory exchangeParams,
    uint256 amountIn
  ) external returns (uint256 numTokensMinted);

  function getRate() external view returns (uint256);

  function getPegToken() external view returns (address);
}
...
// File: @uma/core/contracts/common/implementation/Lockable.sol
...
// File: @jarvis-network/synthereum-contracts/contracts/tokens/interfaces/MintableBurnableIERC20.sol
...
// File: @jarvis-network/synthereum-contracts/contracts/tokens/MintableBurnableERC20.sol
...
// File: @jarvis-network/synthereum-contracts/contracts/tokens/MintableBurnableSyntheticToken.sol
...
contract FixedRateWrapper is ERC20Permit {
  using SafeMath for uint256;
...
contract FixedRateCurrency is FixedRateWrapper, IFixedRateCurrency {
  using SafeMath for uint256;
  using SafeERC20 for IERC20;
  ...
  // IAtomicSwapProxy public atomicSwapProxy;
  ...
  event SwapToFixedRate(
    address indexed targetFixedRate,
    uint256 amountIn,
    uint256 amountOut
  );
  ...
  event ProxyChange(address oldProxy, address newProxy);
  //----------------------------------------
  // Modifiers
  //----------------------------------------
  ...
  constructor(
    ...
    // IAtomicSwapProxy _atomicSwapProxyAddr,
    address _admin,
    uint256 _rate,
    string memory _name,
    string memory _symbol
    ...
    admin = _admin;
    // atomicSwapProxy = IAtomicSwapProxy(_atomicSwapProxyAddr);
  }
  ...
  function mintFromPegSynth(uint256 pegTokenAmount, address recipient)
    ...
    returns (uint256 numTokensMinted)
  {
    synth.safeTransferFrom(msg.sender, address(this), pegTokenAmount);
    numTokensMinted = super.wrap(pegTokenAmount, recipient);
  ...
  function redeemToPegSynth(uint256 fixedRateAmount, address recipient)
    ...
    returns (uint256 tokensRedeemed)
  {
    tokensRedeemed = super.unwrap(fixedRateAmount, recipient);
  ...
  function mintFromSynthereumCollateral(
    ...
    // pull synthereum collateral from user's wallet
    ...
    // mint jEUR (peg token) with synthereum collateral and deposit directly into this contract
    mintParams.recipient = address(this);
    (uint256 pegTokensMinted, ) = synthereumPool.mint(mintParams);
  ...
  function redeemToSynthereumCollateral(
    ...
    uint256 pegTokensUnlocked =
      super.unwrap(redeemParams.numTokens, address(this));
    ...
    // redeem synthereum collateral by burning jEur in broker contract and send them recipient in redeemParams
    redeemParams.numTokens = pegTokensUnlocked;
    (uint256 collateralRedeemed, ) = synthereumPool.redeem(redeemParams);
  ...
  function mintFromSynth(
    ...
    exchangeParams.recipient = address(this);
    (uint256 pegTokenAmount, ) = synthereumPool.exchange(exchangeParams);
  ...
  function swapForSynth(
    ...
    checkPoolRegistration(synthereumPool);

    // burn _synthAmount to get peg tokens jEur into this wallet
    uint256 pegTokensUnlocked = super.unwrap(fixedRateAmount, address(this));

    // allow the synthereum pool to transfer jEur
    synth.safeIncreaseAllowance(address(synthereumPool), pegTokensUnlocked);

    // exchange function in broker to get final asset into user wallet
    exchangeParams.numTokens = pegTokensUnlocked;
    (uint256 numTokensMinted, ) = synthereumPool.exchange(exchangeParams);
    emit SwapWithSynth(
      msg.sender,
      address(exchangeParams.destPool.syntheticToken()),
      address(this),
      numTokensMinted,
      'sell'
    );
  ...
  function swapAndMint(
    ...
    ISynthereumPoolOnChainPriceFeed synthereumPool,
    ISynthereumPoolOnChainPriceFeed.MintParams memory mintParams
  ) external payable override isActive {
    IERC20 collateralInstance = checkPoolRegistration(synthereumPool);
    IAtomicSwapProxy atomicSwapProxy =
      IAtomicSwapProxy(
        synthereumFinder.getImplementationAddress(
          SynthereumInterfaces.AtomicSwapProxy
        )
      );
    require(
    ...
    if (msg.value > 0) {
      isExactInput ? exactAmount = msg.value : minOutOrMaxIn = msg.value;
    } else {
      // deposit erc20 into this contract
      uint256 amountToPull = isExactInput ? exactAmount : minOutOrMaxIn;
      ...
      IERC20(tokenSwapPath[0]).safeIncreaseAllowance(
        address(atomicSwapProxy),
        amountToPull
      );
    }

    //erc20 -> synthereum collateral -> jEur to this wallet
    mintParams.recipient = address(this);
    uint256[2] memory amounts =
      atomicSwapProxy.swapAndMint{value: msg.value}(
        dexId,
        isExactInput,
        exactAmount,
        minOutOrMaxIn,
    ...
    // mint FixedRate to user wallet
    uint256 numTokensMinted = super.wrap(amounts[1], msg.sender);

    // eventual token/eth refund
    if (!isExactInput && minOutOrMaxIn > amounts[0]) {
      if (msg.value > 0) {
        // refund eth
        (bool success, ) =
          msg.sender.call{value: minOutOrMaxIn.sub(amounts[0])}('');
  ...
  function redeemCollateralAndSwap(
    ...
    IERC20 collateralInstance = checkPoolRegistration(synthereumPool);
    require(address(collateralInstance) == tokenSwapPath[0]);
    IAtomicSwapProxy atomicSwapProxy =
      IAtomicSwapProxy(
        synthereumFinder.getImplementationAddress(
          SynthereumInterfaces.AtomicSwapProxy
    ...
    // fixedRate -> pegSynth into this wallet
    uint256 pegSynthRedeemed = super.unwrap(fixedRateAmountIn, (address(this)));

    // allow AtomicSwap to pull pegSynth
    synth.safeIncreaseAllowance(address(atomicSwapProxy), pegSynthRedeemed);

    // pegSynth -> synthereum collateral -> ERC20 through AtomicSwap v2
    redeemParams.numTokens = pegSynthRedeemed;
    uint256 outputAmount =
      atomicSwapProxy.redeemCollateralAndSwap(
        dexId,
        true,
        0,
        amountOutMin,
  ...
  {
    require(
      IFixedRateCurrency(targetFixedRate).getPegToken() == address(synth),
      'The target token has a different peg token'
    );

    // unwrap input fixed rate to peg token
    uint256 pegTokensRedeemed = super.unwrap(amountIn, address(this));

    // approve target fixed rate to pull peg tokens
    synth.safeIncreaseAllowance(targetFixedRate, pegTokensRedeemed);

    // wrap into target fixed rate, minted to caller wallet
    numTokensMinted = IFixedRateCurrency(targetFixedRate).mintFromPegSynth(
      pegTokensRedeemed,
      msg.sender
    );

    emit SwapToFixedRate(targetFixedRate, amountIn, numTokensMinted);
  }
  ...
  function swapToFixedRateDifferentPeg(
    ...
    ISynthereumPoolOnChainPriceFeed.ExchangeParams memory exchangeParams,
    uint256 amountIn
  ) external override returns (uint256 numTokensMinted) {
    // unwrap input fixed rate to peg token
    uint256 pegTokensRedeemed = super.unwrap(amountIn, address(this));

    // allow the synthereum pool to transfer peg token
    synth.safeIncreaseAllowance(
      address(inputTokenSynthereumPool),
      pegTokensRedeemed
    );

    // input peg -> target peg Token into this wallet
    exchangeParams.recipient = address(this);
    exchangeParams.numTokens = pegTokensRedeemed;
    (uint256 targetPegTokenAmount, ) =
      inputTokenSynthereumPool.exchange(exchangeParams);

    // approve target fixed rate to pull target peg tokens
    IERC20(IFixedRateCurrency(targetFixedRate).getPegToken())
      .safeIncreaseAllowance(targetFixedRate, pegTokensRedeemed);

    // wrap target peg token to get target fixed rate
    numTokensMinted = IFixedRateCurrency(targetFixedRate).mintFromPegSynth(
      targetPegTokenAmount,
      msg.sender
    );
  ...
  function getPegToken() external view override returns (address) {
    ...
    returns (IERC20 collateralInstance)
  {
    ...
    require(
      synthereumPool.syntheticToken() == synth,
      'The pool does not contain the right peg token'
    );
  }
}