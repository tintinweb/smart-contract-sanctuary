...
contract BrightRiskToken is ERC20Upgradeable, AccessControlUpgradeable, IBrightRiskToken {
    ...
    uint256 internal _initialLiquidity;
    ...
    function __BrightRiskToken_init(
        ...
        minimumBaseDeposit = 1000 ether;
        priceFeed = IPriceFeed(_priceFeed);

        _mint(_msgSender(), _initialSupply);
        _initialLiquidity = _initialSupply;
    }
    ...
    // access: ANY
    function burn(uint256 _indexTokenAmount) external {
    ...
    function _mintAndDistributeIndex(address[] memory _stakers, uint256 _count) internal {
        for (uint256 i = 0; i < _count; i++) {
            ...
            uint256 _mintAmount = externalPoolByDepositor[_recipient].minting;
            if (_mintAmount > 0) {
                _mint(_recipient, _mintAmount);
            }
            //clean up
            delete externalPoolByDepositor[_recipient];
    ...
    */
    function _indexRatio() internal view returns (uint256 _ratio) {
        ...
        uint256 _currentTotalSupply = inCirculation();
    ...
    function _calculateStreamingFee() internal view returns (uint256) {
        ...
        uint256 amount = feePercentage.mul(inCirculation());
    ...
    function inCirculation() public view returns (uint256) {
        return totalSupply().sub(_initialLiquidity);
    }
