...
contract HAQQ is ERC20, Ownable {
    ...
    */
    event TierUpgrade(uint to);

    /*
    * Private function to upgrade tier and emit corresponding event
    */
    function _upgradeTier() private {
        if (currentTier < 3) {
            emit TierUpgrade(currentTier + 1);
    ...
    */
    function upgradeTier() public onlyOwner returns (bool success) {
        _upgradeTier();
        return true;
    }
    ...
    */
    function _buyTokens() private returns (bool success){
        ...
        //   uint tokensAmount = ((msg.value * price) / (10 ** 26) * tiers[currentTier].tokenPriceInUsd) * (10 ** decimals());
        uint tokensAmount = ((msg.value * price) / (10 ** 20) * tiers[currentTier].tokenPriceInUsd);

        // if amount is less than required in curren tier, function should revert
        require(tokensAmount >= tiers[currentTier].minPurchaseTokens * (10 ** decimals()), "The purchased amount ot tokens is less than the required minimum ");
        ...
        if (currentTier < 3 && totalSupply() >= (tiers[currentTier + 1].startsOnTokensSold * (10 ** decimals()))) {
            _upgradeTier();
        }
        return true;
    }
    ...
    */
    function withdrawAllToOwner() public onlyOwner returns (bool success) {
