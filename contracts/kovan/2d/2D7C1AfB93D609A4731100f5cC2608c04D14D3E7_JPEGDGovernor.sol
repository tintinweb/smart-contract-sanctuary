...
import "./GovernorMultiTokenVotes.sol";

contract JPEGDGovernor is
    GovernorUpgradeable,
    GovernorCountingSimpleUpgradeable,
    GovernorMultiTokenVotes,
    ...
    GovernorTimelockControlUpgradeable
{
    ...
    uint256 internal votingDelayBlocks;
    uint256 internal votingPeriodBlocks;
    Rate internal votingThresholdRate;

    function initialize(
        ERC20VotesUpgradeable _token,
        ERC20VotesUpgradeable[] memory _wrappedTokens,
        TimelockControllerUpgradeable _timelock,
        uint256 _quorumPercent,
        uint256 _votingDelay,
        uint256 _votingPeriod,
        Rate memory _thresholdRate
    ) external initializer {
        __Governor_init("JPEGDGovernor");
        __GovernorMultiTokenVotes_init(_token, _wrappedTokens);
        __GovernorVotesQuorumFraction_init(_quorumPercent);
        __GovernorTimelockControl_init(_timelock);
        votingDelayBlocks = _votingDelay;
        votingPeriodBlocks = _votingPeriod;
        votingThresholdRate = _thresholdRate;
    }
    ...
    function proposalThreshold() public view override returns (uint256) {
        return
            (token.totalSupply() * votingThresholdRate.numerator) /
            votingThresholdRate.denominator;
    }
...
/**
 * @dev Extension of {GovernorVotes} that supports multiple {ERC20Votes} backed by {_token}.
 *
 */
abstract contract GovernorMultiTokenVotes is GovernorVotesUpgradeable {
    uint256 public wrappedTokensLength;
    mapping(uint256 => ERC20VotesUpgradeable) public wrappedTokens;

    function __GovernorMultiTokenVotes_init(
        ERC20VotesUpgradeable _token,
        ERC20VotesUpgradeable[] memory _wrappedTokens
    ) internal initializer {
        __GovernorVotes_init(_token);
        for (uint256 i = 0; i < _wrappedTokens.length; i++) {
            wrappedTokens[i] = _wrappedTokens[i];
        }
        wrappedTokensLength = _wrappedTokens.length;
    }
    ...
    {
        votes = super.getVotes(account, blockNumber);
        for (uint256 i = 0; i < wrappedTokensLength; i++) {
            votes += wrappedTokens[i].getPastVotes(account, blockNumber);
        }
    }
