...
contract EonsAaveRouter is OwnableUpgradeable {
  ...
  event WithdrawSuccess(uint256 indexed pid, uint256 indexed vaule, uint256 indexed withdrawed);

  ILendingPoolAddressesProvider public _lendingPoolAddressesProvider;
  IEonsAaveVault public _eonsAaveVault;
  mapping(uint => AssetInfo) public assetInfo; // pid => aToken reserve address
  uint16 public _referralCode; 

  function initialize(address lendingPoolProvider) public initializer {
  ...
  function deposit(uint _amount, uint _pid, address _user) external {
    ...
    lendingPool.deposit(asset.reserve, _amount, address(this), _referralCode);
  }

  function withdraw(uint _pid, uint _amount, address _recipient) external {
    AssetInfo memory asset = assetInfo[_pid];
    ILendingPool lendingPool = ILendingPool(_lendingPoolAddressesProvider.getLendingPool());
    try IAToken(asset.aToken).approve(address(lendingPool), _amount) returns (bool) {

    } catch Error(string memory reason) {
      emit WithdrawError(_pid, reason);
    }
    if (_pid == 1) {  // if withdrawing eth. hard coded
      uint256 value = lendingPool.withdraw(asset.reserve, _amount, address(this));
      if (value == 0) {
        emit WithdrawError(_pid, 'Failed withdraw during the withdraw. Line 91');
      } else {
        uint256 balanceWithdrawed = IWETH(asset.reserve).balanceOf(address(this));
        emit WithdrawSuccess(_pid, value, balanceWithdrawed);
      }
      IWETH(asset.reserve).withdraw(_amount);
