...
contract BalancerUtils {
    ...
    function justSwap(address smartPoolAddress, address tokenIn, uint tokenAmountIn, address tokenOut, uint minAmountOut, uint maxPrice) public returns (uint, uint){
        ConfigurableRightsPool crpPool = ConfigurableRightsPool(smartPoolAddress);
        address bPool = address(crpPool.bPool());
        // sell the token by making a delegate call
        (, bytes memory result) =
            bPool.delegatecall(abi.encodeWithSelector(bytes4(keccak256("swapExactAmountIn(address,uint256,address,uint256,uint256)")), tokenIn, tokenAmountIn, tokenOut, minAmountOut, maxPrice));
