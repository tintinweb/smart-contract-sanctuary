...
abstract contract BondSubHour is LendMath {
    ...
    function updateHBond(address issuer, HourlyBond storage bond, address holder) internal 
    {
        ...
        if (yieldQuotientFP > 0) {
            payoutRewards(bond, meta, holder);
            uint256 oldAmount = bond.amount;

            bond.amount = setRate(bond.amount, yA.accumulatorFP, yieldQuotientFP);
    ...
    function viewHBond(address issuer, address holder)public view returns (uint256)
    {
        ...
        if (yieldQuotientFP > 0) {
            return setRate(bond.amount, cumulativeYield, yieldQuotientFP);
        } else {
            return bond.amount;
        }
    }
    ...
    function getUpdatedHAPY(address issuer, YieldAccumulator storage accumulator, uint256 window) internal returns (YieldAccumulator storage) 
    {
        ...
        if (timeDelta > window) 
        {
            ...
            accumulator.hourlyYieldFP = DepositRate(meta.totalLending, meta.totalBorrowed);
    ...
    function payoutRewards(HourlyBond storage bond, LendingMetadata storage meta,address holder) internal virtual;
}
...
contract Lend is RoleAware, BondSubHour {
    ...
    mapping(address => uint256) public clips;
    ...
    function activateDepositor(address issuer) external {
        activateDepositor(issuer, issuer);
    }

    /// Make issuer != token available for protocol (isol. margin)
    function activateDepositor(address issuer, address token)
        public
    ...
    function deactivateDepositor(address issuer) external onlyOwnerExecActivator {
        activeIssuers[issuer] = false;
    }

    /// Set lending cap
    function setDepositLimit(address issuer, uint256 cap)
        external
        onlyOwnerExecActivator
    {
    ...
    function setClaimDelay(uint256 delay) external onlyOwnerExec {
        withdrawDelay = delay;
    }

    function setLowestRate(uint256 rate) external onlyOwnerExec {
        normalRate = rate;
    }

    function setHighestRate(uint256 rate) external onlyOwnerExec {
        highRate = rate;
    }

    /// Set hourly yield APR for issuer
    function setHourlyAPY(address issuer, uint256 aprPercent)
        external
        onlyOwnerExecActivator
    {
    ...
    function viewLoanAPR(
        ...
        return setRate(balance, accumulatorFP, yieldQuotientFP);
    }

    /// @dev gets called by router to register if a trader borrows issuers
    function listLoan(address issuer, uint256 amount) external {
    ...
    function listDeposit(address issuer, uint256 amount) external {
    ...
    function listClaim(address issuer, uint256 amount) external {
        require(isLender(msg.sender), "Not an approved lender");
        require(activeIssuers[issuer], "Not approved issuer");
        LendingMetadata storage meta = lendingMeta[issuer];
    ...
    function payOut(address issuer, uint256 amount) external {
        require(isBorrower(msg.sender), "Not approved borrower");
        lendingMeta[issuer].totalBorrowed -= amount;
    }

    /// @dev get the credit yield for a specific issuer/token
    function viewTotalLoanAPR(address issuer)
        external
        view
        returns (uint256)
    {
    ...
    function viewAPR10k(YieldAccumulator storage yA)
        internal
        view
        returns (uint256)
    {
    ...
    function viewLoanAPR10k(address issuer)
        external
        view
        returns (uint256)
    {
        return viewAPR10k(borrowYieldAccumulators[issuer]);
    }

    /// @dev get current lending APR per 10k for a token / issuer
    function viewHBondAPR10k(address issuer)
        external
        view
        returns (uint256)
    {
        return viewAPR10k(hourlyBondYieldAccumulators[issuer]);
    }

    /// @dev In a liquidity crunch make a fallback bond until liquidity is good again
    function makeReserveBond(
        address issuer,
        address holder,
        uint256 amount
    ) external {
    ...
    function claimHBond(address issuer, uint256 amount) external {
    ...
    function closeHBondAcc(address issuer) external {
        HourlyBond storage bond = hourlyBondAccounts[issuer][msg.sender];
    ...
    function purchaseHBondTicket(address issuer, uint256 amount)
        external
    {
        require(activeIssuers[issuer], "Not approved issuer");

        collectAssets(issuer, msg.sender, amount);
    ...
    function initLoanTotalAPR(address issuer)
        external
        onlyOwnerExecActivator
    {
        YieldAccumulator storage yA = borrowYieldAccumulators[issuer];
    ...
    function setLoanForce(uint256 _borrowingFactor)
        external
        onlyOwnerExec
    {
    ...
    function outputBalance(address issuer)
        internal
        view
        override
        returns (uint256)
    {
        ...
        if (token == issuer) {
            ...
            return lendingMeta[issuer].totalLending - clips[issuer];
        }
    }
    ...
    function disburse(
        ...
        uint256 haircutAmount = clips[issuer];
    ...
    function clip(uint256 amount) external {
        clips[msg.sender] += amount;
    }
...
abstract contract LendMath {
    ...
    function DepositRate(uint256 totalLending, uint256 totalBorrowing)
        internal
        view
    ...
    function outputBalance(address issuance)
        internal
        view
        virtual
        returns (uint256);
