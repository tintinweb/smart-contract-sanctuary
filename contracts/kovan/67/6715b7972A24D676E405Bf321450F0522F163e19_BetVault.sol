...
contract BetVault is VRFConsumerBase {
    ...
    address public btcWithdrawAddress;
    address public ethWithdrawAddress;
    address public usdtWithdrawAddress;
    address public linkWithdrawAddress;
        ...
        )
    {
        ...
        btcWithdrawAddress = msg.sender;
        ethWithdrawAddress = 0xfFd303d36b7C1beE28a8AD74ee8B21b675519111;  
        usdtWithdrawAddress = 0x63d20dD9d8b7C95B18B5b282F247E1124eD79f3d;
        linkWithdrawAddress = msg.sender;
    }
    ...
    function setBtcWithdrawAddress(address _btcAddress) public onlyGov {
        btcWithdrawAddress = _btcAddress;
    }

    function setEthWithdrawAddress(address _ethAddress) public onlyGov {
        ethWithdrawAddress = _ethAddress;
    }

    function setUsdtWithdrawAddress(address _usdtAddress) public onlyGov {
        usdtWithdrawAddress = _usdtAddress;
    }

    function setLinkWithdrawAddress(address _linkAddress) public onlyGov {
        linkWithdrawAddress = _linkAddress;
    }
    ...
    function withdraw() external onlyGov {
        ...
        if (_btcBalance > 0) {
            BTCToken.safeTransfer(btcWithdrawAddress, _btcBalance);
            totalBTC = 0;
        }

        if (_usdtBalance > 0) {
            USDTToken.safeTransfer(usdtWithdrawAddress, _usdtBalance);
            totalUSDT = 0;
        }

        if (_ethBalance > 0) {
            payable(ethWithdrawAddress).transfer(_ethBalance);
            totalETH = 0;
        }

        if (_linkBalance > 0) {
            LINK.transfer(linkWithdrawAddress, _linkBalance);
        }
    }
}
