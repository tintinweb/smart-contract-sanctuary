    ...
    ReentrancyGuard
{
    ...
    function convertETHToBabyDoge() public onlyOwner {
        _swapTokens(
            babyDogeToken,
            _getQuote(
    ...
    function claimPool(uint256[] calldata nftIDs) external nonReentrant {
        ...
        require(claimable > 0);
        if (ethPayout) {
            payable(owner()).call{value: claimable};
        } else {
    ...
    function claimLotto(uint256 _id) external nonReentrant {
        ...
        for (uint256 i = 0; i < toClaimPrize.length; i++) {
            if (toClaimPrize[i] == _id) {
                uint256 prize = prizePool.div(toClaimPrize.length);
    ...
    function nextRewardNonce() public returns (bytes32 requestId) {
        ...
        if (ethPayout) {
            teamPortion = address(this).balance.mul(devTeamPercent).div(10000);
            payable(owner()).transfer(teamPortion);
            prizePool = address(this).balance.mul(lotoPercent).div(10000);
        } else {
            convertETHToBabyDoge();
            teamPortion = IERC20(babyDogeToken)
                .balanceOf(address(this))
                .mul(devTeamPercent)
                .div(10000);
            IERC20(babyDogeToken).transfer(owner(), teamPortion);
            prizePool = IERC20(babyDogeToken)
                .balanceOf(address(this))
                .mul(lotoPercent)
                .div(10000);
        }
