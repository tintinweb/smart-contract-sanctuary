...
contract DIAGenesisMinter is Ownable {
	...
	function postGenesisMint(uint mintIndex) external{
		DIADataNFT diaDataNFTImpl = DIADataNFT(postGenesisNFTs[mintIndex]);
		require(!diaDataNFTImpl.genesisPhase());
		require(diaDataNFTImpl.started());
		uint newRank = NFTUtil.randInt(diaDataNFTImpl.NUM_PRIVILEGED_RANKS(), seed);		
		diaDataNFTImpl._mintDataNFT(msg.sender, newRank);
		emit MintedDataNFT(msg.sender, diaDataNFTImpl.numMintedNFTs(), newRank );
	}
...
contract DIADataNFT is Ownable, ERC1155 {
	...
	// 	require(started);
	// 	uint newRank = NFTUtil.randInt(NUM_PRIVILEGED_RANKS, seed);
	// 	_mintDataNFT(msg.sender, newRank);
	// }

	function _mintDataNFT(address _origin, uint _newRank) public {
		if (genesisPhase) {
			require(msg.sender == DIAGenesisMinterAddr, "Only callable from the genesis minter contract");
		} else {
			require(msg.sender == address(this), "Only callable from mintDataNFT()");
		}
		...
		require(ERC20(paymentToken).transferFrom(_origin, burnAddress, burnAmount), "Payment token transfer to burn address failed.");
		// Transfer payment token to minting pool
		require(ERC20(paymentToken).transferFrom(_origin, mintingPool, mintingPoolAmount), "Payment token transfer to minting pool failed.");
		...
		require(ERC20(paymentToken).transferFrom(_origin, address(diaSourceNFTImpl), diaSourceNFTImpl.getSourcePoolAmount(currSourceNFTId)), "Payment token transfer to source pool failed.");
...

{
  ...
  "libraries": {
    "contracts/DIAGenesisMinter.sol": {
      "NFTUtil": "0x5225f56263687aed88222e717f0d64216c55751e"
    }
  }
}