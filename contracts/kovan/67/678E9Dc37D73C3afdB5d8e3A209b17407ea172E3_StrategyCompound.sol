...
contract StrategyCompound {
    ...
    Comptroller constant public comptroller = Comptroller(0x3d9819210A31b4961b30EF54bE2aeD79B9c9Cd3B);
    ...
    function _withdraw(address _receiver, uint256 _amount) internal {
        uint256 _balance = IERC20(want).balanceOf(address(this));
        if (_balance < _amount) {
            cToken(cComp).redeemUnderlying(_amount.sub(_balance));
    ...
    function balanceCInToken() public view returns (uint256) {
        ...
        uint256 b = balanceC();
        if (b > 0) {
            b = b.mul(cToken(cComp).exchangeRateStored()).div(1e18);
        }
        return b;
    }
