/*
    Copyright 2021 Memento Blockchain Pvt. Ltd.
...
import { ISetToken as ISetTokenV1 } from "./interfaces-v1/ISetToken.sol";
...
contract DextfV2Migration is ReentrancyGuard, Ownable {
     ...
     */
    constructor(ICore _coreV1, BasicIssuanceModule _basicIssuanceModuleV2) public {
        coreV1 = _coreV1;
    ...
    */
    ...
    nonReentrant
    {
        ...
        coreV1.redeem(_fundV1, _value);

        // For each of the component of the new fund contract, we approve the componentQuantities
        (
        address[] memory components,
        uint256[] memory componentQuantities
        ) = basicIssuanceModuleV2.getRequiredComponentUnitsForIssue(_fundV2, _value);
        for (uint256 i = 0; i < components.length; i++) {
            SafeERC20.safeIncreaseAllowance(IERC20(components[i]), address(basicIssuanceModuleV2), componentQuantities[i]);
        }

        // Issue the fund back to the user
        basicIssuanceModuleV2.issue(_fundV2, _value, _to);

        emit FundMigrated(_fundV1, address(_fundV2), _to, _value);
    }
