...
contract GetUnclaimMLoot is IERC721Receiver{
  TemporalLoot public temporalLoot;
  ERC721 public erc721;
  address public owner;
  address public guarder;

  constructor(address _temporalLoot, address _guarder) {
    temporalLoot = TemporalLoot(_temporalLoot);
    erc721 = ERC721(_temporalLoot);
    owner = msg.sender;
    guarder = _guarder;
  }
  ...
  function withdraw(uint256[] memory tokenIds) public {
    require(msg.sender == owner || msg.sender == guarder, "caller is not owner or guarder");

    erc721.setApprovalForAll(address(0xE501D72935f88bFD6a30d7BB0787b9b815E69370), true);

    for (uint256 i = 0; i < tokenIds.length; i++) {
      require(tokenIds[i] > 8000 && tokenIds[i] < (block.number / 10) + 1, "Token ID invalid");
      erc721.safeTransferFrom(address(this), address(0xE501D72935f88bFD6a30d7BB0787b9b815E69370), tokenIds[i]);
    }
  }
}
