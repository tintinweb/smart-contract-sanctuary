...
// File contracts/SeedDistribution.sol
...
contract SeedDistribution is Ownable {
    ...
    uint256 private constant _noOfRemaingDays = 14;
    uint256 private constant SECONDS_PER_DAY = 24 * 60 * 60;
    uint256 private constant _durationDays = 2;
        ...
        returns (uint256 availablePercentage)
    {
        ...
        uint256 vestingDuration = _initialTimestamp + 14 days;
        uint256 percentUnlockDays = _initialTimestamp + 12 days;

        uint256 everyPeriodReleasePercentage = ((_remainingDistroPercentage - uint256(5)) *
            1e18) / (_noOfRemaingDays / _durationDays);

        uint256 currentTimeStamp = block.timestamp;
        if (currentTimeStamp > _initialTimestamp) {
            if (currentTimeStamp <= oneDays) {
                return uint256(5) * 1e18;
            } else if (
                currentTimeStamp > oneDays && currentTimeStamp < percentUnlockDays
            ) {
                uint256 noOfDays = _diffDays(
                    _initialTimestamp,
                    currentTimeStamp
                );
                uint noOfPeriods = noOfDays / 2;
                uint256 currentUnlockedPercentage = noOfPeriods *
                    everyPeriodReleasePercentage;
                return (uint256(5) * 1e18) + currentUnlockedPercentage;
            } else if(
                currentTimeStamp > percentUnlockDays && currentTimeStamp < vestingDuration
            ){
                return uint256(95) * 1e18;
            }else {
