   ...
   // for temporary use till chainlink is available to use
    function updateEvents(
        ...
        for (uint256 i = 0; i < _event_ids.length; i++) {
            ...
            events[_event_ids[i]][affiliate_id] = SportEvent({
                ...
                moneyline_home: convertOddsFormat(_moneyline_homes[i]),
                moneyline_away: convertOddsFormat(_moneyline_aways[i]),
                moneyline_draw: convertOddsFormat(_moneyline_draws[i]),
                event_date: _dates[i]
            });
        }
    }
    ...
    function getTotalPayouts(bytes16[] memory _bet_event_ids) external {
        ...
        for (uint256 i = 0; i < _bet_event_ids.length; i++) {
            ...
            if (ev.winner == 1) odd = ev.moneyline_home;
            else if (ev.winner == 2) odd = ev.moneyline_away;
            else odd = ev.moneyline_draw;
    ...
    function getPayoutEst(
        ...
        if (winner == 1) {
            maxPayout = ev.moneyline_home.mul(amount).div(1e4);
        } else if (winner == 2) {
            maxPayout = ev.moneyline_away.mul(amount).div(1e4);
        } else {
            maxPayout = ev.moneyline_draw.mul(amount).div(1e4);
        }
        return maxPayout;
    }
    ...
    function updateMaxPoolPayout(uint256 amount) external {
        maxPoolPayout = maxPoolPayout.add(amount);
    }
