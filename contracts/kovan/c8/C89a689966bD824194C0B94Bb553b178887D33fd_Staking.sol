...
contract Staking is Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        require(_minStakeTime > 0 && _startTime > 0, "Error: wrong params");
        lessToken = _less;
        lpToken = _lp;
        ...
        onlyWhenOpen
    {
        ...
        if(participants == 1 && totalLessRewards + totalLpRewards > 0 && !_firstTransactionPerDay[today]){
        ...
        userStakes[sender].indexes[stakeIdLast] = userStakes[sender]
            .ids
            .length;
    ...
    function _unstake(uint256 id, bool isWithoutPenalty) internal nonReentrant {
        ...
        bool isUnstakedEarlier = (today - stakes[id].startTime) * 86400 <
            minStakeTime;
        ...
        emit Unstaked(staker, id, block.timestamp, isUnstakedEarlier);
    }
        ...
        returns (uint256 lpRewards, uint256 lessRewards)
    {
        ...
        for (uint256 i = 0; i < _depositDays.length; i++) {
        ...
        uint256 curDay;
        for (uint256 i = countStartIndex; i < countEndIndex; i++) {
    ...
    function _currentDay() private view returns (uint256) {
        return (block.timestamp - contractStart) / 86400;
    }
