...
contract TokenDispenserI25H75 is TokenDispenserBase {
    ...
    uint public constant p1VestRatio = 75e16;
    uint public constant p1rate = p1VestRatio / 1 hours;

    constructor (address kine_, uint startTime_) public {
        kine = IERC20(kine_);
        startTime = startTime_;
        transferPaused = false;
        phase1EndTime = startTime_.add(1 hours);
    }
    ...
    function vestedPerAllocation() public view returns (uint) {
        ...
        if (currentTime <= phase1EndTime) {
            return immediateVestRatio.add(currentTime.sub(startTime).mul(p1rate));
        }
        return 1e18;
    }
}