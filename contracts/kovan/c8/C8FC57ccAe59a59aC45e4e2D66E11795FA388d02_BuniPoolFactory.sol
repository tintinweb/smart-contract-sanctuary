...
contract BuniPool is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;

    // The address of the Buni Pool factory
    address public immutable BUNI_POOL_FACTORY;
    ...
    function initialize(
        ...
        require(msg.sender == BUNI_POOL_FACTORY, "Not factory");
...
contract BuniPoolFactory is Ownable {
    event NewBuniPoolContract(address indexed buniPool);
     ...
     * @param _poolLimitPerUser: pool limit per user in stakedToken (if any, else 0)
     * @param _admin: admin address with ownership
     * @return address of new Buni Pool contract
     */
    function deployPool(
        IBEP20 _stakedToken,
        IBEP20 _rewardToken,
        ...
        bytes memory bytecode = type(BuniPool).creationCode;
        bytes32 salt = keccak256(
            abi.encodePacked(_stakedToken, _rewardToken, _startBlock)
        );
        address buniPoolAddress;

        assembly {
            buniPoolAddress := create2(
                0,
                add(bytecode, 32),
                mload(bytecode),
                salt
        ...
        BuniPool(buniPoolAddress).initialize(
            _stakedToken,
            _rewardToken,
            _rewardPerBlock,
            _startBlock,
        ...
        emit NewBuniPoolContract(buniPoolAddress);
    }
}
