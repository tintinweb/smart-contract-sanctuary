...
contract assignmentToken {
    ...
    //specify event to be emitted on Mintership Transfer
    event MintershipTransfer(address indexed previousMinter, address indexed newMinter);

    //specify event to be emitted on burn
    event Burn(address indexed account, uint256 tokens);
    ...
    function mint(address receiver, uint256 amount) public returns (bool) {
        ...
        require(amount <= (MAXSUPPLY-supply), "exceeding supply limit");
        require(amount != 0);
        supply += amount;
    ...
    function transferMintership(address newMinter) public returns (bool) {
        ...
        require(msg.sender == minter, "only minter can transfer the mintership");
        minter = newMinter;
    ...
    function transfer(address _to, uint256 _value) public returns (bool) {
        ...
        require(_value + flatfee <= balances[msg.sender]);
        balances[msg.sender] -= _value + flatfee;
        balances[_to] += _value;
        balances[minter] += flatfee;
    ...
    function transferFrom(
        ...
        // NOTE: transfer value needs to be sufficient to cover fee
        require(_value + flatfee <= balances[_from], "not sufficient tokens");
        require(allowances[_from][msg.sender] >= _value, "not sufficient allowances");
        balances[_from] -= _value + flatfee;
        balances[_to] += _value;
        balances[minter] += flatfee;
