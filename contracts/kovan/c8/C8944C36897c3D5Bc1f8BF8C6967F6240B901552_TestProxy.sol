...
contract TestProxy {
    ...
    function createAuction(PoolConfig memory poolConfig) external {
        ...
        for (uint256 index = 0; index < poolConfig.tokens.length; index++) {
            console.log('1');
            TransferHelper.safeTransferFrom(
                poolConfig.tokens[index],
                msg.sender,
                address(this),
                uint256(poolConfig.amounts[index])
            );
            console.log('2');
    ...
    function testAuction(PoolConfig memory poolConfig) external {
        // 1: deposit tokens and approve vault
        for (uint256 index = 0; index < poolConfig.tokens.length; index++) {
            console.log('1');
            TransferHelper.safeTransferFrom(
