...
contract TONKATSUUU is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"Tonkatsuuu Inu TEST";
    string private constant _symbol = unicode"TONKATSUUU";
    ...
    uint private _maxSellAmount = _tTotal;
    uint private _maxBuyAmount = _tTotal;
        ...
        if (from != owner() && to != owner()) {
            ...
            if (from == uniswapV2Pair && to != address(uniswapV2Router) && ! _isExcludedFromFee[to]) {
                ...
                if(sellCooldownEnabled) {
                    lastBuy = block.timestamp + (7 minutes);
                }
            }
