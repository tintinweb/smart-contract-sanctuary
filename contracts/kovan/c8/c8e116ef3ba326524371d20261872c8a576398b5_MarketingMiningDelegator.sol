...
contract MarketingMining is ActivityBase{
    ...
    uint256 public SHDPerBlock = 124 * (1e15);
    ...
    function add(uint256 _allocPoint, IERC20 _tokenAddress, bool _withUpdate) external virtual {
        checkAdmin();
        if(_withUpdate){
            massUpdatePools();
        }
    ...
    function setAllocationPoint(uint256 _pid, uint256 _allocPoint, bool _withUpdate) external virtual {
        checkAdmin();
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function setIsDepositAvailable(bool _isDepositAvailable) external virtual onlyOwner {
        isDepositAvailable = _isDepositAvailable;
    }

    function setIsRevenueWithdrawable(bool _isRevenueWithdrawable) external virtual onlyOwner {
    ...
    function addAvailableDividend(uint256 _amount, bool _withUpdate) external virtual {
        if(_withUpdate){
            massUpdatePools();
        }
    ...
    function updateAfterwithdraw(uint256 _pid, uint256 _amount) internal {
        ...
        bool withdrawnState = user.withdrawnState;
        uint256 existedAmount = user.amount;
        require(existedAmount > 0 && existedAmount >= _amount, "withdraw: not good");
        updatePoolDividend(_pid);
    ...
    function getDepositWeight(uint256 _amount) private pure returns(uint256 weight){
        return _amount;
    }
    ...
    function getPoolInfo(uint256 _pid) external view virtual returns(uint256 _allocPoint, uint256 _accumulativeDividend, uint256 _usersTotalWeight, uint256 _tokenAmount, address _tokenAddress, uint256 _accs){
        PoolInfo storage pool = poolInfo[_pid];
    ...
    function getPagePoolInfo(uint256 _fromIndex, uint256 _toIndex) external view virtual
    ...
    function getUserInfoByPids(uint256[] memory _pids, address _user) external virtual view 
...
contract MarketingMiningDelegator is DelegatorInterface, MarketingMining {
    ...
    function setAllocationPoint(uint256 _pid, uint256 _allocPoint, bool _withUpdate) external override {
        delegateToImplementation(
            abi.encodeWithSignature(
    ...
    function setIsDepositAvailable(bool _isDepositAvailable) external override {
        delegateToImplementation(
            abi.encodeWithSignature(
    ...
    function setIsRevenueWithdrawable(bool _isRevenueWithdrawable) external override {
        delegateToImplementation(
            abi.encodeWithSignature(
    ...
    function addAvailableDividend(uint256 _amount, bool _isUpdate) external override {
        delegateToImplementation(
