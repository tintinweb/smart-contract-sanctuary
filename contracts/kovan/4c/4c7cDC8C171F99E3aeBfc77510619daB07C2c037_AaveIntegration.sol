    ...
    IIntegration
{
    ...
    function initialize(address[] memory controllers_,  address moduleMap_, address lendingPoolAddress_) public initializer {
    ...
    function withdraw(address tokenAddress, uint256 amount) public override onlyController {
        require(amount <= balances[tokenAddress], "AaveIntegration::withdraw: Withdraw amount exceeds balance");
    ...
    function deploy() external override onlyController {
        ...
        for(uint256 tokenId = 0; tokenId < tokenCount; tokenId++) {
            ...
            if(tokenAmount > 0) {
                try IAaveLendingPool(lendingPoolAddress).deposit(address(token), tokenAmount, address(this), 0) {} catch {}
            }
        }
    }
        ...
        for(uint256 tokenId = 0; tokenId < tokenCount; tokenId++) {
            ...
            if(aTokenAddress != address(0)){
                ...
                if(aTokenBalance > balances[tokenAddress]) {
                    try IAaveLendingPool(lendingPoolAddress).withdraw(tokenAddress, aTokenBalance - balances[tokenAddress], 
                        address(moduleMap.getModuleAddress(Modules.YieldManager))) {} catch {}
                }
            }
        }
    }
    ...
    function getATokenAddress(address underlyingTokenAddress) public view returns (address) {
        IAaveLendingPool.ReserveData memory reserveData = IAaveLendingPool(lendingPoolAddress).getReserveData(underlyingTokenAddress);
