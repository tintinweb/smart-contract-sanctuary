...
abstract contract Control is AccessControl {
	...
	uint32 internal period = 5 minutes;
	// uint32 internal period = 3 days; // DEV
	uint80 internal _initialPrice;
	// coeff with 2 decimals
	uint80 internal _coeffInflations = 1200;
...
contract Cryptium is CrptERC20, Control, ReentrancyGuard {
	...
	event Log2(string msg, address val1, address val2, uint256 val3, uint256 val4);
	...
	function rebase() external nonReentrant initialized {
		...
		if (isInflRebase) {
			...
			_lastRebase = TimeLibrary.addMonths(_lastRebase, monthsPassFromRebase); // DEV
			_lastRebase = _startTime + monthsPassAll * (5 minutes);
		}
	 ...
	 * @return
	 */
	function getAvailToClaim(address adr) external initialized returns (uint256) {
	...
	function _feeCount(
		...
		if (_isWhiteList[to]) {
			...
			emit Log2("from, to, limitAvail, fee", from, to, limitAvail, fee);
		}
	}
		...
		)
	{
		...
		if (newPrice == 0) {
			...
			price1 = (curPrice * oldPrice * 10000) / newPrice;
		}
		if (monthsPassFromRebase == 0) {
			price2 = 0;
		} else {
			price2 = ((_coeffInflations * monthsPassAll) / 12 + 10000) * _initialPrice;
		}

		if (price1 > price2 && price1 > curPrice) {
			supplyDelta = (_totalSupply * ((oldPrice * 10000) / newPrice)) / 10000;
		} else if (price2 > curPrice) {
			price2 = ((_coeffInflations * monthsPassFromRebase) / 12 + 10000) * _initialPrice;
			supplyDelta = (_totalSupply * (price2 / curPrice)) / 10000;
			isInflRebase = true;
		} else {
			isFall = false;
		}
	 ...
	 */
	function _getMonthsPass() private returns (uint256, uint256) {
		// uint256 monthsPassAll = TimeLibrary.diffMonths(_startTime, block.timestamp); // DEV
		// uint256 monthsPassFromRebase = TimeLibrary.diffMonths(_lastRebase, block.timestamp);
		uint256 monthsPassAll = (block.timestamp - _startTime) / (5 minutes);
		uint256 monthsPassFromRebase = (block.timestamp - _lastRebase) / (5 minutes);
		monthsPassAll = _checkMonths(_startTime, monthsPassAll);
		monthsPassFromRebase = _checkMonths(_lastRebase, monthsPassFromRebase);
		emit Log("startTime timestamp", _startTime, block.timestamp, 0, 0);
		emit Log("lastRebase timestamp", _lastRebase, block.timestamp, monthsPassAll, monthsPassFromRebase);
		if (monthsPassAll <= 12 && monthsPassFromRebase > 0) {
			return (monthsPassAll, monthsPassFromRebase);
		} else {
			return (0, 0);
		}
	}
	...
	/**
	 * @notice Checks that pass full months
	 * @param months: months pass from startTime
	 * @return months exect amount of full months
	 */
	function _checkMonths(uint256 fromTime, uint256 months) private view returns (uint256) {
		if (TimeLibrary.addMonths(fromTime, months) > block.timestamp) months--;
		return months;
	}
