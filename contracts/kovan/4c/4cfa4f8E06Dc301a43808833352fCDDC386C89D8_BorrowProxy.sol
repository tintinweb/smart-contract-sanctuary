    ...
    function borrowV1(address owner, uint256 id, address tokenA, uint256 amountA, address tokenB, uint256 amountB, address borrowToken, uint256 leverage, uint256 deadLine) override external returns (uint256, uint256, uint256) {
        ...
        if (0 < leverage) {
            ...
            if (tokenA == borrowToken) {
                ...
                IERC20(borrowToken).approve(address(exchange), leverage);
                amountA = amountA.add(exchange.swapExtractOut(borrowToken, tokenA, address(this), leverage, 1, deadLine));
            }
        }
    ...
    function trim(address tokenA, uint256 amountA, address tokenB, uint256 amountB, uint256 deadLine) internal returns (uint256, uint256) {
        ...
        if (valueA < valueB) {
            ...
            if (0 < swapB) {
                ...
                IERC20(tokenB).approve(address(exchange), swapB);
                price = exchange.swapExtractOut(tokenB, tokenA, address(this), swapB, 1, deadLine);
            ...
            if (0 < swapA) {
                ...
                IERC20(tokenA).approve(address(exchange), swapA);
                price = exchange.swapExtractOut(tokenA, tokenB, address(this), swapA, 1, deadLine);
