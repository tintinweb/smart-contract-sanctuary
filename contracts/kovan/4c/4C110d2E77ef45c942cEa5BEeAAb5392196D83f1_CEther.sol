...
contract CEtherProxy is TransparentUpgradeableProxy {
 ...
 * @author Cream
 */
abstract contract CTokenDeprecated is CTokenInterface, Exponential, TokenErrorReporter {
    ...
    function mintInternal(
        uint256 mintAmount
    ) internal nonReentrant nonMultiActivity(msg.sender) returns (uint256, uint256) {
    ...
    ) internal nonReentrant nonMultiActivity(msg.sender) returns (uint256) {
        uint256 error = accrueInterest();
        if (error != uint256(Error.NO_ERROR)) {
    ...
    ) external override nonReentrant nonMultiActivity(msg.sender) returns (uint256) {
        uint256 error = accrueInterest();
        if (error != uint256(Error.NO_ERROR)) {
            // accrueInterest emits logs on errors, but on top of that we want to log the fact that an attempted reduce reserves failed.
