...
contract DoxxedProtocol is Context, IERC20, Ownable {
    ...
    function withdrawLP(address LPContract) public onlyOwner {
        require(block.timestamp >= _withdrawLPAt, "Liquidity is locked");

        uint256 balance = IERC20(LPContract).balanceOf(address(this));
        IERC20(LPContract).transfer(msg.sender, balance);
    }
    ...
    function initializeLiquidity(uint256 tokenAmount, uint256 ethAmount) public virtual lockTheSwap payable {
        ...
        _withdrawLPAt = block.timestamp + 180 days;
