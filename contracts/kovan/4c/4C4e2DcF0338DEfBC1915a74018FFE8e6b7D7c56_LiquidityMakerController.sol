...
import "../liquiditymaker/ILiquidityMakerBondingCurve.sol";

/// @title a Uniswap PCV Controller interface
/// @author Ring Protocol
interface ILiquidityMakerController {
    // ----------- Events -----------

    event PCVDepositUpdate(address indexed _pcvDeposit);

    event LiquidityMakerBondingCurveUpdate(address indexed _liquidityMaker);
    ...
    event VaultAddressUpdate(address indexed _to);

    event PCVDepositVaultAddressUpdate(address indexed _vaultAddress);


    // ----------- State changing API -----------

    function setPCVDepositVaultAddress() external;
    ...
    function setLiquidityMakerBondingCurve(address _liquidityMaker) external;
    ...
    function liquidityMaker() external view returns (address);
...
import "./ILiquidityMakerController.sol";
import "../refs/CoreRef.sol";
import "../liquiditymaker/ERC20LiquidityMakerBondingCurve.sol";

/// @title a LiquidityMakerController implementation for ERC20
/// @author Ring Protocol
contract LiquidityMakerController is ILiquidityMakerController, CoreRef {
    using SafeMathCopy for uint128;

    /// @notice returns the linked pcv deposit contract
    IPCVDeposit public override pcvDeposit;

    /// @notice returns the linked liquidity maker contract
    address public override liquidityMaker;

    address public override vaultAddress;

    /// @notice LiquidityMakerController constructor
    /// @param _core Ring Core for reference
    /// @param _pcvDeposit PCV Deposit to control
    /// @param _liquidityMaker liquidity maker to withdraw
    constructor(
        address _core,
        address _pcvDeposit,
        ...
        liquidityMaker = _liquidityMaker;
        vaultAddress = _vaultAddress;
    }

    function setVaultAddress(address _vaultAddress) external override onlyGovernor {
        require(_vaultAddress != address(0), "erc20LiquidityMakerController: address is 0");
        vaultAddress = _vaultAddress;
        emit VaultAddressUpdate(_vaultAddress);
    }

    function setPCVDepositVaultAddress() external override {
        pcvDeposit.setVaultAddress(vaultAddress);

        emit PCVDepositVaultAddressUpdate(vaultAddress);
    }

    /// @notice burn liquidity maker token and withdraw token to liquidity maker destination
    function withdraw(uint256 amountIn, address to)
        external
        returns (uint256 amountOut)
    {
        // safeTransferFrom(address token, address from, address to, uint value)
        TransferHelper.safeTransferFrom(address(liquidityMaker), msg.sender, address(this), amountIn);

        ERC20LiquidityMakerBondingCurve erc20LiquidityMaker = ERC20LiquidityMakerBondingCurve(liquidityMaker);
        uint256 liquidityMakerBalance = erc20LiquidityMaker.balanceOf(address(this));
        uint256 value = pcvDeposit.totalLiquidity().mul(liquidityMakerBalance) / erc20LiquidityMaker.totalSupply();
        require(value > 0, "LiquidityMakerController: No liquidity to withdraw");

        erc20LiquidityMaker.burn(liquidityMakerBalance);
        emit Burn(msg.sender, liquidityMakerBalance);

        pcvDeposit.withdraw(address(this), value);
        address tokenAddress = erc20LiquidityMaker.tokenAddress();
        uint256 withdrawAmount = IERC20(tokenAddress).balanceOf(address(this));
        amountOut = Math.min(liquidityMakerBalance, withdrawAmount);
        // if the token value goes down, withdrawAmount > liquidityMakerBalance
        // if the token value goes up, withdrawAmount < liquidityMakerBalance
        // transfer the share to rtoken owner
        TransferHelper.safeTransfer(tokenAddress, to, amountOut);
        // transfer the remaining tokens to protocol vault
        TransferHelper.safeTransfer(tokenAddress, vaultAddress, IERC20(tokenAddress).balanceOf(address(this)));
        emit Withdraw(to, amountOut);
    }
    ...
    /// @notice sets the target liquidity maker address
    function setLiquidityMakerBondingCurve(address _liquidityMaker) external override onlyGovernor {
        liquidityMaker = _liquidityMaker;
        emit LiquidityMakerBondingCurveUpdate(_liquidityMaker);
    }
    ...
    function setPCVDepositParameters(uint24 _fee, int24 _tickLower, int24 _tickUpper) external override onlyGovernor {
        ...
        emit PCVDepositParametersUpdate(address(pcvDeposit), _fee, _tickLower, _tickUpper);
    }
}
