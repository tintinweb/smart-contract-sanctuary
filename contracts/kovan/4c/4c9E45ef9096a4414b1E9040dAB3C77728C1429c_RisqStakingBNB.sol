...
// File: contracts/Staking/StakingBNB.sol
...
contract RisqStakingBNB is RisqStaking, IRisqStakingERC20 {
    using SafeERC20 for IERC20;
    using SafeMath for uint;

    IERC20 public immutable BNB;

    constructor(ERC20 _token, ERC20 bnb) public
        RisqStaking(_token, "RISQ BNB Staking lot", "rlBNB") {
        BNB = bnb;
    }
    ...
    function sendProfit(uint amount) external override {
        ...
        if (_totalSupply > 0) {
            ...
            BNB.safeTransferFrom(msg.sender, address(this), amount);
            emit Profit(amount);
        } else {
            BNB.safeTransferFrom(msg.sender, FALLBACK_RECIPIENT, amount);
        }
    }

    function _transferProfit(uint amount) internal override {
        BNB.safeTransfer(msg.sender, amount);
    }
}