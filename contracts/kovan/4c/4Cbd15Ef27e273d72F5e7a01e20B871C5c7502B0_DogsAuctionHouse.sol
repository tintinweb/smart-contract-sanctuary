...
contract DogsAuctionHouse is IDogsAuctionHouse, PausableUpgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
     ...
     */
    function createBid(uint256 dogId, string memory tokenURI) external payable override nonReentrant {
        ...
        emit AuctionBid(_auction.dogId, msg.sender, tokenURI, msg.value, extended);
...
interface IDogsAuctionHouse {
    ...
    event AuctionBid(uint256 indexed dogId, address sender, string tokenURI, uint256 value, bool extended);
    ...
    function createBid(uint256 dogId, string memory tokenURI) external payable;
