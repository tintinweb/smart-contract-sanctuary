...
contract Core is ICore, Permissions, Initializable {
    ...
    function setRusd(address token) external override {
        _setRusd(token);
    }

    /// @notice sets Ring address to a new address
    /// @param token new ring address
    function setRing(address token) external override {
        _setRing(token);
    }
...
contract Permissions is IPermissions, AccessControl {
    ...
    function revokeBurner(address burner) external override {
    ...
    function revokeGovernor(address governor) external override {
        revokeRole(GOVERN_ROLE, governor);
    }

    /// @notice revokes guardian role from address
    /// @param guardian ex guardian
    function revokeGuardian(address guardian) external override {
