    ...
    ReentrancyGuardUpgradeable
{
        ...
        onlyNotSettled
    {
        ...
        uint256 protocolBalance = collateral.balanceOf(address(this));
        collateral.safeTransferFrom(msg.sender, address(this), _collateralQty);
        uint256 collateralQty = _collateralQty - (collateral.balanceOf(address(this)) - protocolBalance);

        uint256 fee;
        if (issuanceFees > 0) {
            fee = (collateralQty * issuanceFees) / 10000;
            collateralQty = collateralQty - fee;
            accumulatedFees = accumulatedFees + fee;
        }

        uint256 qtyToBeMinted = collateralQty / volatilityCapRatio;

        volatilityToken.mint(msg.sender, qtyToBeMinted);
        inverseVolatilityToken.mint(msg.sender, qtyToBeMinted);

        emit Collateralized(msg.sender, collateralQty, qtyToBeMinted, fee);
    }
