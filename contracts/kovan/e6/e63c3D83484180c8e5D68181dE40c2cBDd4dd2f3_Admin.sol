...
abstract contract LeverageMarginCall is LeverageAcc {
    ...
    function maintainerIsFailing() internal view returns (bool) {
        (address currentMaintainer, ) = Admin(manager()).viewCurrentMaintenanceStaker();
    ...
    {
        bool isAuthorized = Admin(manager()).isAuthorizedStaker(msg.sender);
        ...
        if (!canTakeNow) {
            ...
            loser = Admin(manager()).getUpdatedCurrentStaker();
        }
        ...
        address currentMaintainer = Admin(manager()).getUpdatedCurrentStaker();
        if (isAuthorized) {
