...
contract WeldTokensale is OwnableUpgradeable, PausableUpgradeable, AccessControlUpgradeable {
    ...
    mapping (uint=>mapping(address => uint256)) public purchasedAtStage;
    ...
    modifier isAdmin(address _addr){
        require(hasRole(ADMIN_ROLE, _addr), "Address is not an admin");
        _;
    }
    ...
    */
    ...
    function initialize(address _USDTToken, address _USDCToken, address _DAIToken, address WELD,
        uint s1_start, uint s1_end, uint s1_pool, uint withdrawal_period, 
         uint vest_duration, address _admin) public initializer {
     ...
     */
    function adminSetVestingStart(uint256 _vestingStart) virtual public isAdmin(_msgSender()){
        require(vestingStart == 0, "Vesting start is already set");
        require(_vestingStart > stages[stages_count-1].stage_end && block.timestamp < _vestingStart, "Incorrect time provided");
        vestingStart = _vestingStart;
    }

    function adminAddStageAndResetVestingStart(uint _start, uint _end, uint _pool, uint _vestingStart) public isAdmin(_msgSender()){
        require(stages.length<5, "All stages has been add");
        stages.push(Stage(_start, _end, _pool, 0));
        stages_count++;
        adminSetVestingStart(_vestingStart);
    }
     ...
     */
    function adminSetRates(address _token, uint256 _rate) external isAdmin(_msgSender()) {
        if (_token == address(0))
            ETHRate = _rate;
        else
            rates[_token] = _rate;
    ...
    */
    function adminSetTreasury(address treasury) external isAdmin(_msgSender()) {
        _treasury = treasury;
    }
    ...
    */
    function adminSetMaxWELD(uint256 _maxWELD) external isAdmin(_msgSender()) {
    ...
    */
    function adminSetMaxGasPrice(uint256 _maxGasPrice) external isAdmin(_msgSender()) {
     ...
     */
    function adminSetPool(uint256 _stage, uint256 _pool) external isAdmin(_msgSender()) {
        stages[_stage].stage_pool=_pool;
    }
    ...
    */
    function adminPause() external isAdmin(_msgSender()) {
        _pause();
    }
    ...
    */
    function adminUnpause() external isAdmin(_msgSender()) {
        _unpause();
    }

    
    function adminAddPurchase(address _receiver, uint256 _amount) virtual external isAdmin(_msgSender()) {
        purchased[_receiver] = purchased[_receiver].add(_amount);
    }
    
    function adminAddWhitelist(address _user) virtual external isAdmin(_msgSender()) {
        isWhitelisted[_user]=true;
    }

    function adminRemoveWhitelist(address _user) virtual external isAdmin(_msgSender()) {
     ...
     */
    receive() external virtual payable whenNotPaused {
     ...
     */
    function purchaseWELDwithERC20(address ERC20token, uint256 ERC20amount) external supportedCoin(ERC20token) whenNotPaused whitelistOnly correctGas {
        ...
        require(current_stage!=Stages.Ended && current_stage!=Stages.NotStarted, "Sale is over");
     ...
     */
    function purchaseWELDwithETH() external payable  whenNotPaused whitelistOnly {
    ...
    function adminWithdrawERC20(address ERC20token) external onlyOwner{
        ...
        require(ERC20token != WELDToken || _canWithdrawWELD(), "WELD withdrawal is forbidden");
    ...
    * @return Current stage
    ...
    function currentStage() public view returns(Stages){
        if(stages[0].stage_start>block.timestamp){
            return Stages.NotStarted;
        }
        for(uint i=0; i<stages.length; i++){
            if(block.timestamp>stages[i].stage_end){
                continue;
            }else{
                return Stages(i);
            }
     ...
     */
    function _canWithdrawWELD() virtual internal view returns (bool) {
