...
contract dNXMToken is IdNXMToken, ERC20PermitUpgradeable {
    ...
    function setDependencies(address _nxMaster, address _wNXMTokenAddress, uint _rewardsFee) external override onlyInjector {
        ...
        _fee = _rewardsFee.mul(PRECISION);
    }
    ...
    **/
    ...
    function withdraw() public override{
        ...
        // calculate rate right here, after fee is applied
        uint _amountNXM = convertDNXMToNXM(_amountDNXMRequested);

        uint256 _ownRewards = _withdrawnCumulativeRewards.mul(balanceOf(_sender).mul(PRECISION).div(totalSupply())).div(PRECISION);
        _withdrawnCumulativeRewards = _withdrawnCumulativeRewards.sub(_ownRewards);
    ...
    function collectFees(address _treasury) external override onlyInjector {
        _nxmToken.transfer(_treasury, _nxmToken.balanceOf(address(this)).sub(_withdrawnCumulativeRewards));
    }
...
interface IdNXMToken {
    ...
    function collectFees(address _treasury) external;
