...
import "./interfaces/IEquipmentVendor.sol";
...
contract Equipment is IEquipment, ERC1155, AcceptedToken {
    ...
    /**
     * @dev Owner set new EquipmentVendor contract
     * @param contractAddr New EquipmentVendor contract address
     */
    function setEquipmentVendor(IEquipmentVendor contractAddr) external onlyOwner {
        require(address(contractAddr) != address(0));
        vendorContract = contractAddr;
    }

    /**
     * @dev Owner set new URI
     * @param uri New URI
     */
    ...
    /**
     * @dev Owner set upgrade fee in token
     * @param fee New upgrade fee in token
     */
    ...
    /**
     * @dev Owner set mint fee in token
     * @param fee New upgrade fee in token
     */
    ...
    /**
     * @dev Owner create an item.
     * @param name Name of item
     * @param maxSupply Max supply of item
    ...
    function createItem(
        ...
        _items.push(Item(name, maxSupply, 0, 0, 1, 0, rarity));
        uint itemId = _items.length - 1;
        mint(msg.sender, itemId, 1);

        emit ItemCreated(itemId, name, maxSupply, rarity);
    }

    /**
     * @dev Owner add next tier item to existing one.
     * @param itemId Id of item
     * @param upgradeAmount Amount upgade of item
     */
    ...
    /**
     * @dev Burns the same items to upgrade its tier
     * @param itemId Id of Item
     */
    function upgradeItem(uint itemId) external override collectTokenAsFee(upgradeFeeInToken, owner()) {
        Item storage currentItem = _items[itemId];
        uint8 upgradeAmount = currentItem.upgradeAmount;
        uint nextTierItemId = nextTierItems[itemId];
        Item storage nextTierItem = _items[nextTierItemId];
        address sender = msg.sender;

        require(nextTierItemId != 0, "Equipment: not upgradable");
        require(nextTierItem.minted + 1 <= nextTierItem.maxSupply, "Equipment: max supply reached");
        ...
        nextTierItem.minted += 1;
        emit TransferSingle(sender, address(0), sender, nextTierItemId, 1);
    }

    /**
     * @dev Pays some fee to get random items
     * @param vendorId Id of vendor
     * @param amount Amount of random items
     */
    function rollEquipmentGacha(uint vendorId, uint amount) external override {
        ...
        acceptedToken.safeTransferFrom(sender, owner(), mintFeeInToken * totalReceivedItems);
    }

    /**
     * @dev Mints items and returns true if it's run out of stock.
     * @param account Wallet address need mint items
     * @param itemId Id of Item
     * @param amount Amount of random items
     */
    function mint(address account, uint itemId, uint16 amount) public override onlyOperator returns (bool) {
        Item storage item = _items[itemId];

        require(item.minted + amount <= item.maxSupply, "Equipment: max supply reached");
    ...
    /**
     * @dev Burns ERC1155 equipment since it is equipped to the hero.
     * @param account Wallet address of hero owner
     * @param itemIds Id of Item
     */
    ...
    /**
     * @dev Returns ERC1155 equipment back to the owner.
     * @param account Wallet address of hero owner
     * @param itemIds Id of Item
     */
    ...
    /** 
     * @dev Check if item is out of stock.
     * @param itemId Id of item
...
interface IEquipmentVendor {
     ...
     *
     * Requirements:
     * - Only `equipmentContract` can call this function.
     */
