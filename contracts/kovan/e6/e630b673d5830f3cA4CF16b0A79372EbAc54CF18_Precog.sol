...
contract Precog is IPrecog {
    ...
    function liquidate(address futureContract, address user) external returns(uint256 amount) {
        ...
        _repayLoan(user, futureContract, expiryDate);
    ...
    function _repayLoan(address user, address futureContract, uint expiryDate) internal {
        BorrowInfo[] memory loans = userBorrowInfo[user][futureContract];
        if (loans.length > 0) {
            for (uint i = 0; i < loans.length; i++) {
                uint repayAmount = _getDebtAmount(loans[i], expiryDate);
    ...
    function _calculateProfit(
        ...
        if (expiryDate > block.timestamp) {
            ...
            if (pairs[0] != address(0)) {
                uint revenue = _getRevenue(amount, expiryDate, pairs, tradeInfo);
                if (revenue > availableAmount[user]) {
                    profit = revenue - availableAmount[user];
                }
                
                if (loan.platformIndex > 0) {
                    revenue = _getRevenue(amount + loan.borrowAmount, expiryDate, pairs, tradeInfo);
                    uint debtAmount = _getDebtAmount(loan, expiryDate);
                    uint feeLending = _convertEthToUsdc(feeLendingByEth);
                    if (revenue > availableAmount[user] + debtAmount + feeLending) {
                        uint profitLoan = revenue - availableAmount[user] - debtAmount - feeLending;
    ...
    function invest(
        ...
        uint256 feeTrading = _swapFee(feeTradingByEth);
        BorrowInfo memory loan = _getBorrowForTrade(amount, borrowAmount, lendingPlatformIndex);
        amount -= feeTrading;
        (uint profit, bool canBorrow, uint expiryDate) = _calculateProfit(user, amount, tradeInfo, loan);
        require(profit > 0, "PrecogV2: NOT_PROFITABLE");
        
        uint tradeAmount = amount;
        if (canBorrow) {
        ...
        _updateTradingAmount(user, tradeInfo.futureContract, amount, profit, feeTrading);
        _removeTradingUser(user);
        
        emit Trade(user, 
            tradeInfo.futureContract, expiryDate, 
            amount, profit, feeTrading, 
