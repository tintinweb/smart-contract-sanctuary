...
contract PositionHelper {
    ...
    function getTraderPositons(IOpenLev openLev, uint16 marketId, address[] calldata traders, bool[] calldata longTokens, bytes calldata dexData) external view returns (Vars[] memory results){
        results = new Vars[](traders.length);
        ...
        for (uint i = 0; i < traders.length; i++) {
            ...
            (uint current,,,) = openLev.marginRatio(traders[i], marketId, longTokens[i], dexData);
            item.marginRatio = current;
