...
abstract contract Vanity is Context, IBEP20, Ownable {
    ...
    uint256 internal _tTotal = 10000000000 * _DECIMALFACTOR;
    ...
    uint256 private    _BURN_FEE = 200; // 2% BURNED WITH EVERY TRANSACTION
...
contract VanityPlus is Vanity{
    uint256 private _liquidityFee = 200;  //2% of each tx sent to LP
    bool inSwapAndLiquify;
    bool public swapAndLiquifyEnabled = true;
    uint256 private _maxTxAmount = totalSupply();
    address public immutable uniswapV2Pair;
    IUniswapV2Router02 public immutable uniswapV2Router;
    uint256 private numTokensSellToAddToLiquidity = 500000 * 10**6 * 10**9;
    address public immutable buyBackCoin;

    constructor(address _uinswapV2Router02) Vanity(){
        buyBackCoin = address(0);           //CHANGE THIS to be an arg after testing
        
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(_uinswapV2Router02);
    ...
    event SwapAndLiquify(uint,uint,uint);

    /// @dev this function sets the max tx as a percent of total supply. decimals limited to 10**2
    function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner() {
        _maxTxAmount = totalSupply() * (maxTxPercent) / (
            10**2
        );
    }

    ///@dev buyback function from original Vanity token
    ///@notice must approve tx first with old coin
    function buyback(uint amount)external{
        IERC20(buyBackCoin).transferFrom(msg.sender, address(this), amount);
    ...
    override {
        ...
        // Take lq on any transfer as it doesn't take from anyone
        _takeLiquidity(calculateLiquidityFee(amount));
