    ...
    function setCertifiedAddition(
        ...
        if (len > 0) {
            ...
            for (uint256 i = 0; i < len; i++) {
                if (
                    _whitelist[i] != generalInfo.creator /* && !whitelistTier[_whitelist[i]] */
...
contract PresaleFactoryCertified is ReentrancyGuard {
    ...
    constructor(address _bscsInfoAddress) {
        ...
        //transferOwnership(safeLibrary.owner());
        //owner = safeLibrary.owner();
    }
    ...
    function createPresaleCertified(
        ...
        require(msg.value >= feeEth && feeEth > 0, "5");
        ...
        /* uint8 decNT = ERC20(_addition.nativeToken).decimals();

        if(decNT != 18){
            if(decNT < 18)
                tokenAmounts[0] *= uint256(10)**uint256(18 - decNT);
            else
                tokenAmounts[0] *= uint256(10)**uint256(decNT - 18);
        } */
    ...
    function countAmountOfTokens(
        ...
        if (_liqPrice != 0 && _liqPerc != 0) {
            tokenAmounts[0] = ((_hardCap *
                _liqPerc *
                (uint256(10)**uint256(_decimalsToken))) / (_liqPrice * 100));
            if(_decimalsNativeToken != 18){
                if(_decimalsNativeToken < 18)
                    tokenAmounts[0] *= uint256(10)**uint256(18 - _decimalsNativeToken);
                else
                    tokenAmounts[0] /= uint256(10)**uint256(_decimalsNativeToken - 18);
            }
...

{
  ...
  "libraries": {
    ...
    "/Users/rita/Documents/less-contracts/contracts/libraries/Calculation1.sol": {
      "Calculation1": "0xA9cC76A913B7fFac6EE279ad987Df57a2b184d35"
    }
  },
  "outputSelection": {
    "*": {
