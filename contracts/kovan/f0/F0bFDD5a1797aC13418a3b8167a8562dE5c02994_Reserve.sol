...
contract Reserve is Ownable {
    ...
    event OmssTokenConfigured(address indexed _omssToken, uint256 _minReserveAmount);
    ...
    address[] omsUsdcPath;
    address[] omssUsdcPath;

    uint deadline = now + 600;

    uint256 initialUSDInvestmentAmount;
    uint256 minOmsReserveAmount;
    uint256 minOmssReserveAmount;
    ...
    function setOmssToken(address _omssToken, uint256 _minReserveAmount) external onlyOwner {
        omss = IERC20(_omssToken);
        minOmssReserveAmount = _minReserveAmount;
        omssUsdcPath = [ address(omss), address(usdc) ];
        emit OmssTokenConfigured(_omssToken, _minReserveAmount);
    }
    ...
    function transferReserve() external onlyOwner {
        ...
        omss.transfer(reserveNg, minOmssReserveAmount);
        emit Transferred(address(reserveNg), address(omss), minOmssReserveAmount);
    }
    ...
    function _sellExcessOmsTokens() internal {
        ...
        if (currentBalance > minOmsReserveAmount) {
            ...
            // approve Uniswap Router to be able to transace on our oms tokens.
    ...
    function _sellExcessOmssTokens() internal {
        // check balance
        uint256 currentBalance = omss.balanceOf(address(this));
        
        if (currentBalance > minOmssReserveAmount) {
            // determine how many tokens we want to sell off by getting balance and deducting minimum holding amount
            uint256 amount = currentBalance - minOmssReserveAmount;

            // approve Uniswap Router to be able to transace on our omss tokens.
            omss.approve(address(uniswapRouter), amount);
            ...
            emit TokenSwapped(address(omss), amount, address(usdc), this.usdcBalance());
        } else {
            emit TokenSwapped(address(omss), 0, address(usdc), this.usdcBalance());
        }
    }
}