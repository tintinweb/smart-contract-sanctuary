...
contract SwapContract is AccessControl, Pausable, ECDSAOffsetRecovery {
    ...
    uint256 private constant FixedPoint128 = 0x100000000000000000000000000000000;
    ...
    uint256 totalStackedAmount;

    struct StackingPosition{
        uint256 amount;
        uint256 feeGrowthLast;
    }

    mapping(address => StackingPosition) private positions;
        ...
        TransferFrom(params)
    {
        ...
        if (params.path.length == 1) {
            ...
            feeGrowthGlobal += FullMath.mulDiv(
    ...
    function setRouter(IUniswapV2Router02 _router)
        ...
        onlyOwnerAndManager
    {
        blockchainRouter = _router; //TODO: allowance
    ...
    function stake(uint256 _amount) external{
        IERC20(RubicAddresses[numOfThisBlockchain]).safeTransferFrom(
            msg.sender,
            address(this),
            _amount
        );
        positions[msg.sender] = StackingPosition(
            _amount,
            feeGrowthGlobal
        );
        totalStackedAmount += _amount;
    }

    function _claim() private{
        StackingPosition memory _position = positions[msg.sender];

        uint256 reward = FullMath.mulDiv(
                    feeGrowthGlobal - _position.feeGrowthLast,
                    _position.amount,
                    FixedPoint128
        );
        IERC20(RubicAddresses[numOfThisBlockchain]).safeTransfer(msg.sender, reward);
    }

    function claim() external{
        _claim();
        positions[msg.sender].feeGrowthLast = feeGrowthGlobal;
    }
