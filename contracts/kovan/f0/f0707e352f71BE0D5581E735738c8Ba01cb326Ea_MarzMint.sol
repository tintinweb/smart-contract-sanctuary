...
contract MarzMint {
    ...
    function claim(uint256[] calldata plotIds) external {
        MarzLand _marz = MarzLand(marz);

        for (uint256 i = 0; i < plotIds.length; i++) {
            if (!_marz.exists(plotIds[i])) {
                _marz.claimTo(msg.sender, plotIds[i]);
            }
        }
    }

    function claimBlock(uint256 start, uint256 width, uint256 height) external {
        MarzLand _marz = MarzLand(marz);
        ...
        for (uint256 i = 0; i < height; i++) {
            for (uint256 j = 0; j < width; j++) {
                ...
                if (!_marz.exists(plotId)) {
                    _marz.claimTo(msg.sender, plotId);
                }
            }
        }
    }
