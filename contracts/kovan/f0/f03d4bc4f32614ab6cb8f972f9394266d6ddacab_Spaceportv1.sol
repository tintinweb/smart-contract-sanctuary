...
contract Spaceportv1 is ReentrancyGuard {
  ...
  function userWithdrawTokens () external nonReentrant {
    ...
    if (STATUS.LP_GENERATION_COMPLETE_TIME + buyer.vestingEnd < block.timestamp) {
      ...
      uint256 tokensOwedCopy = tokensOwed;
      tokensOwed = tokensOwed.mul(block.timestamp - buyer.lastUpdate).div(buyer.vestingEnd - STATUS.LP_GENERATION_COMPLETE_TIME);
      
      uint256 tokensRatio = tokensOwedCopy.div(tokensOwed);
      buyer.tokensOwed = buyer.tokensOwed.div(tokensRatio);
