...
contract TokensFactory is Initializable, RegistryStorage {
    /* Events */
    event ApwibtDeployed(address _futureVault, address _apwibt);
    ...
    /**
     * @notice Deploy the apwibt of the future
     * @dev the caller must be a registered future vault
     */
    function deployAPWIBT(
        ...
        newToken = IProxyFactory(registry.getProxyFactoryAddress()).deployMinimal(
            registry.getAPWineIBTLogicAddress(),
            payload,
            keccak256(abi.encodePacked(ibtSymbol, msg.sender))
        );
        emit ApwibtDeployed(msg.sender, newToken);
    }
     ...
     * @dev the caller must be a registered future vault
     */
    function deployNextFutureYieldToken() external onlyRegisteredFutureVault returns (address newToken) {
        IFutureVault futureVault = IFutureVault(msg.sender);
        IERC20 apwibt = IERC20(futureVault.getAPWIBTAddress());
        IERC20 ibt = IERC20(futureVault.getIBTAddress());
        uint256 periodDuration = futureVault.PERIOD_DURATION();
        uint256 periodIndex = futureVault.getNextPeriodIndex();

        string memory tokenDenomination = _getFYTSymbol(apwibt.symbol(), periodDuration);
        bytes memory payload =
            abi.encodeWithSignature(
        ...
        newToken = IProxyFactory(registry.getProxyFactoryAddress()).deployMinimal(
            ...
            payload,
            keccak256(abi.encodePacked(tokenDenomination, msg.sender))
        );
     ...
     * @param _apwibtSymbol the APWine IBT symbol for this futureVault
     * @param _periodDuration the duration of the periods for this futureVault
     * @return the generated symbol of the FYT
     */
    function _getFYTSymbol(string memory _apwibtSymbol, uint256 _periodDuration) internal view returns (string memory) {
        return
...
interface IFutureVault {
    ...
    event APWIBTSet(IAPWineIBT _apwibt);
     ...
     */
    function getAPWIBTPerAmountDeposited(uint256 _amount) external view returns (uint256);
...
library APWineNaming {
    ...
    function uintToString(uint8 _i) internal pure returns (string memory) {
        if (_i == 0) {
            return "0";
        }
        uint j = _i;
