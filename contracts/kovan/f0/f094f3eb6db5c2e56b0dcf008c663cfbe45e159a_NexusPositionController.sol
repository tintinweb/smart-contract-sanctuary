...
contract NexusPositionController is AbstractController, ERC20Upgradeable {
    ...
    function canStake() public view override returns (bool) {
        return !_pooledStaking.hasPendingUnstakeRequests() && super.canStake();
    }
    ...
    function unstake() external override onlyIndex returns (uint256) {
        ...
        //swap wXNM into base
        uint256 _baseAmount = _sellWNXM(_amount);
    ...
    function _sellWNXM(uint256 _amount) internal returns (uint256 _baseAmount) {
        _checkApprovals(_wNXMToken, address(_uniswapRouter), _amount);
        _baseAmount = _sellWNXMforDAI(_amount);
