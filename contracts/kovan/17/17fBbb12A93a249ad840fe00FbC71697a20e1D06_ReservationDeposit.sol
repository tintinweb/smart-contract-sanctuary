...
contract ReservationDeposit {
    ...
    function getBalanceETH() public view onlyOwner returns (uint256) {
    ...
    function getAddressToUserWalletProjectAmountFunded(
        ...
        uint32 buildingId,
        uint32 propertyId
    ) public view onlyOwner returns (uint256, string memory) {
        string memory emailLower = _toLower(email);
        uint256 sumAmount = 0;
    ...
    function getFunders() public view onlyOwner returns (string[] memory) {
        return funders;
    }
    ...
    function refund(
        ...
        string memory emailLower = _toLower(email);
        uint32 countindexesToRemove = 0;
        uint32[] memory indexesToRemove = new uint32[](
            addressToUserWalletProjectAmountFunded[emailLower].length
        );
        for (uint32 i = 0; i < indexesToRemove.length; i++) {
            indexesToRemove[i] = 1000000000;
        }
        for (
            uint32 i = 0;
            i < addressToUserWalletProjectAmountFunded[emailLower].length;
            ...
            if (
                ...
                addressToUserWalletProjectAmountFunded[emailLower][i]
                    .amount = 0;
                indexesToRemove[countindexesToRemove] = i;
                countindexesToRemove++;
            }
        }

        for (uint32 i = 0; i < indexesToRemove.length; i++) {
            if (indexesToRemove[i] != 1000000000) {
