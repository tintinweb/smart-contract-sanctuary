...
contract ProsperSocial is Ownable, Initializable, ContextUpgradeSafe {
    ...
    mapping (uint => mapping (uint => string)) public questionStrings;
    ...
    event NewPool(uint id, address creator, address token, uint minBet, uint start, uint startPeriod, uint end, uint endPeriod, string[] questionStrings, string[] poolOptions);
    ...
    function createPool(address token, uint minBet, uint start, uint startPeriod, uint end, uint endPeriod, string[] memory poolQuestionStrings, string[] memory poolOptions) public {
