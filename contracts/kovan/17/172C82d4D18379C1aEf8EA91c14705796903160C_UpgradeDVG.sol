...
contract UpgradeDVG is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function addWhiteList(address[] memory _addresses, uint256[] memory _amounts) external onlyOwner {
        ...
        for (uint i = 0; i < _addresses.length; i ++) {
            ...
            uint256 admount = _amounts[i];
            Holder memory holder = dvgHolders[user];
            require(holder.balance == 0, "This address already added");

            holder.balance = admount;
            dvgHolders[user] = holder;

            totalBalance = totalBalance.add(admount);
        }

        dvgHolderCount = dvgHolderCount.add(_addresses.length);
    }

    function upgradeDVG(uint256 _amount) external payable onlyEOA nonReentrant returns(uint256 dvdAmount) {
        ...
        holder.swapped = holder.swapped.add(dvdAmount);
        dvgHolders[sender] = holder;
