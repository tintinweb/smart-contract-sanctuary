...
interface Setter {
    ...
    function setTotalAllowance(address, uint) external;
    function setPerBlockAllowance(address, uint) external;
}

contract DeployDebtFloorSetter {
    uint constant WAD  = 10 ** 18;
    uint constant RAY  = 10 ** 27;
    uint constant RAD  = 10 ** 45;
    uint constant GWEI = 10 ** 9;

    function execute(address safeEngine, address oracleRelayer, address treasury, address ethPriceOracle) public returns (address, address) {
        DSValue gasPriceOracle = new DSValue();
        gasPriceOracle.updateResult(200 * GWEI); // 200 gwei

        SingleDebtFloorAdjuster setter = new SingleDebtFloorAdjuster(
            safeEngine,
            oracleRelayer,
            treasury,
            address(gasPriceOracle),
            ethPriceOracle,
            bytes32("ETH-A"),   // collateralType
            0.001 ether,        // baseUpdateCallerReward
            0.001 ether,        // maxUpdateCallerReward
            RAY,                // perSecondCallerRewardIncrease
            7200,               // updateDelay
            400000,             // gasAmountForLiquidation
            1000000000 * RAD,   // maxDebtFloor
            RAD                 // minDebtFloor
        );

        Setter(safeEngine).addAuthorization(address(setter));
        setter.modifyParameters("maxRewardIncreaseDelay", 6 hours);

        // allowances
        Setter(treasury).setTotalAllowance(address(setter), uint(-1));
        Setter(treasury).setPerBlockAllowance(address(setter), 0.001 ether);

        return (address(setter), address(gasPriceOracle));
    }
}