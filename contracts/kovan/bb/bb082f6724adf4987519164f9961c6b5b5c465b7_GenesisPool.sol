...
contract GenesisPool is TokenWrapper, Ownable {
    ...
    //0xeef1922A8A32E50d0a97a40843b6A795E5F27b58
    //0xbab448813fA5CcAB2d814bEa1eCB44f0C6919306
  ...
  uint256 public division;
	...
	constructor(ERC1155Tradable _slvtnftAddress, IERC20 _erc20Address, uint256 _maxStake, uint256 _division) public TokenWrapper(_erc20Address) {
		...
		return
			points[account].add(
				blockTime.sub(lastUpdateTime[account]).mul(1e18).div(86400).mul(
					(balanceOf(account).div(division)).div(1e18)
				)
			);
	}
  ...
  function setDivisionAmount(uint256 _division) public onlyOwner {
    division = _division;
  }
