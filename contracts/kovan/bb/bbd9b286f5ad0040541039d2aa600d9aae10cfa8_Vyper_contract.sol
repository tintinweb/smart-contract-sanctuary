...
def updateRewards():
    ...
    # Which gives:
    totalToPaid: uint256 = 0
    if self.endBlock >= block.number:
        totalToPaid = (self.incentives * (block.number - self.lastBlock))/(self.endBlock - self.lastBlock)
    else:
        totalToPaid = self.incentives
...
def bal(user: address) -> (uint256, uint256):
    ...
    return (
        self.holdings[user],
        (self.paidPerToken + self.holdings[user] * totalToPaid/self.totalStaked) * self.holdings[user] - self.userRewards[user]
        )
