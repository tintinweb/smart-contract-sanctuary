...
contract Presale {
    using SafeMath for uint256;
    uint256 hardcap = 2500 * 1e18;
    uint256 maticRecieved;
    uint256 polarisPerMATIC = 240;
    Polaris polaris;

    address payable treasury;

    constructor(address payable polaris_, address payable treasury_) public {
        polaris = Polaris(polaris_);
        treasury = treasury_;
    }

    function () external payable {
        require(maticRecieved.add(msg.value) <= hardcap, "Hardcap reached.");
        require(msg.value <= 25 * 1 ether, "NO!");
        maticRecieved = maticRecieved.add(msg.value);
        polaris.transfer(msg.sender, msg.value.mul(polarisPerMATIC));
...
contract ERC20TransferLiquidityLock is ERC20 {
    ...
    constructor() public {
        ...
        liquidityLockDivisor = 60;
        callerRewardDivisor = 15;
        rebalanceDivisor = 50;
        rebalanceInterval = 1 hours;
        lpUnlocked = block.timestamp + 30 days;
        minRebalanceAmount = 1000 ether;
        treasury = msg.sender;
        Presale presale_ = new Presale(address(this), treasury);
        bounce = address(presale_);
    ...
    function rebalanceLiquidity() public {
        require(balanceOf(msg.sender) >= minRebalanceAmount, "No.");
    ...
    function rewardLiquidityProviders(uint256 liquidityRewards) internal {
        super._transfer(address(this), uniswapV2Pair, liquidityRewards);
        IUniswapV2Pair(uniswapV2Pair).sync();
        emit RewardLiquidityProviders(liquidityRewards);
    }

    function remLiquidity(uint256 lpAmount) internal returns(uint ETHAmount) {
    ...
    WhitelistAdminRole 
{
    ...
    function setLiquidityLockDivisor(uint256 _liquidityLockDivisor) public onlyWhitelistAdmin {
        if (_liquidityLockDivisor != 0) {
            require(_liquidityLockDivisor >= 10, "Polaris::setLiquidityLockDivisor: too small");
        }
    ...
    function setRebalanceDivisor(uint256 _rebalanceDivisor) public onlyWhitelistAdmin {
        require(_rebalanceDivisor >= 10, "Polaris::setRebalanceDivisor: too small");
    ...
    function setCallerRewardDivisior(uint256 _rewardDivisor) public onlyWhitelistAdmin {
        require(_rewardDivisor >= 10, "Polaris::setCallerRewardDivisor: too small");
    ...
    function splitLiquidity() public onlyWhitelistAdmin {
        require(block.timestamp > lastRebalance + rebalanceInterval, 'Too Soon.');
        lastRebalance = block.timestamp;
        // lockable supply is the token balance of this contract
        uint256 _lockableSupply = balanceOf(address(this));
    ...
    function finalizeSplit() public onlyWhitelistAdmin {
        uint _locked = balancer.rebalance(callerRewardDivisor);
        emit Rebalance(_locked);
    }
