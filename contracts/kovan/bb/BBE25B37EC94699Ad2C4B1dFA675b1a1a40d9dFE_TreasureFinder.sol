...
contract TreasureFinder is BoringOwnable {
    ...
    address public ETH_Destination;
    address public WBTC_USDC_Destination;
    address public DAI_USDT_Destination;
    ...
    constructor (
       ...
       ETH_Destination = _swapRewardDistributor;
       WBTC_USDC_Destination = _swapRewardDistributor;
       DAI_USDT_Destination = _swapRewardDistributor;
    }
    
    function destinationFor(address token) internal view returns (address) {
        if (token == weth) {
            return ETH_Destination;
        }
        if(token == wbtc || token == usdc) {
            return WBTC_USDC_Destination;
        }
        if(token == dai || token == usdt) {
            return DAI_USDT_Destination;
        }
        return address(this);
    }
    ...
    function _convert(address token0, address token1) internal returns(uint256) {
        ...
        uint amountBONE;
        if (!_convertTopCoins(token0, token1, amount0, amount1, ETH_Destination)) {
            if (!_convertTopCoins(token0, token1, amount0, amount1, WBTC_USDC_Destination)) {
                if (!_convertTopCoins(token0, token1, amount0, amount1, DAI_USDT_Destination)) {
    ...
    function _convertTopCoins(
        ...
        address destination0 = destinationFor(token0);
        address destination1 = destinationFor(token1);

        if (destination0 == destination && destination1 == destination) {
            IERC20(token0).safeTransfer(destination, amount0);
            IERC20(token1).safeTransfer(destination, amount1);
        }
        else if (destination0 == destination) {
            IERC20(token0).safeTransfer(destination, _swap(token1, token0, amount1, address(this)).add(amount0));
        } else if (destination1 == destination) {
            IERC20(token1).safeTransfer(destination, _swap(token0, token1, amount0, address(this)).add(amount1));
        } else {
            return false;
        }
        return true;
    ...
    function setETH_Destination(address _destination) external onlyOwner {
        ETH_Destination = _destination;
    }

    function setWBTC_USDC_Destination(address _destination) external onlyOwner {
        WBTC_USDC_Destination = _destination;
    }

    function setDAIUSDTDestination(address _destination) external onlyOwner {
        DAI_USDT_Destination = _destination;
    }
}
