...
contract ReceiptTokenFactory is  AccessControl {
    
    bytes32 public constant TOKEN_CREATOR_ROLE = keccak256("TOKEN.CREATOR");
    ...
    function createReceiptToken(address underlyingAddress, address strategy) external onlyTokenCreator returns(address) {
   ...
   /**
    * @dev grants token creator role to stragegy facory contracts
    * @param addr strategy factory contract address. e.g HarvestEthFactory.sol, HarvestSCFactory.sol, SushiSLPFactory.sol
    **/
    function grantTokenCreatorRole(address addr) external onlyAdmin() {
         grantRole(TOKEN_CREATOR_ROLE, addr);
    }

    modifier onlyTokenCreator() {
          require(hasRole(TOKEN_CREATOR_ROLE, _msgSender()), "!permission");
         _;
    }
