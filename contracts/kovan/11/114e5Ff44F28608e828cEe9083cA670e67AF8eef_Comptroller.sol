...
contract BUM is LibNote {
    // --- Auth ---
    mapping (address => uint) public wards;
    function rely(address guy) external note auth { wards[guy] = 1; }
    function deny(address guy) external note auth { wards[guy] = 0; }
    modifier auth {
        require(wards[msg.sender] == 1, "BUM/not-authorized");
        _;
    }
    ...
    function add(uint x, uint y) internal pure returns (uint z) {
        require((z = x + y) >= x, "BUM math error");
    }
    ...
    function transferFrom(address src, address dst, uint wad)
        public returns (bool)
    {
        require(balanceOf[src] >= wad, "BUM/insufficient-balance");
        if (src != msg.sender && allowance[src][msg.sender] != uint(-1)) {
            require(allowance[src][msg.sender] >= wad, "BUM/insufficient-allowance");
    ...
    function burn(address usr, uint wad) external {
        require(balanceOf[usr] >= wad, "BUM/insufficient-balance");
        if (usr != msg.sender && allowance[usr][msg.sender] != uint(-1)) {
            require(allowance[usr][msg.sender] >= wad, "BUM/insufficient-allowance");
                    ...
                    bool allowed, uint8 v, bytes32 r, bytes32 s) external
    {
        ...
        require(holder != address(0), "BUM/invalid-address-0");
        require(holder == ecrecover(digest, v, r, s), "BUM/invalid-permit");
        require(expiry == 0 || now <= expiry, "BUM/permit-expired");
        require(nonce == nonces[holder]++, "BUM/invalid-nonce");
...
/**
 * @title ChumHum's CToken Contract
...
contract CToken is CTokenInterface, Exponential, TokenErrorReporter {
    ...
    function borrowFresh(address payable borrower, uint borrowAmount) internal returns (uint) {
        ...
        if (getCashPrior() < borrowAmount) {
            return fail(Error.TOKEN_INSUFFICIENT_CASH, FailureInfo.BORROW_CASH_NOT_ABUMLABLE);
        }
    ...
    function _reduceReservesFresh(uint reduceAmount) internal returns (uint) {
        ...
        if (getCashPrior() < reduceAmount) {
            return fail(Error.TOKEN_INSUFFICIENT_CASH, FailureInfo.REDUCE_RESERVES_CASH_NOT_ABUMLABLE);
        }
...
contract Comptroller is ComptrollerV3Storage, ComptrollerInterface, ComptrollerErrorReporter, Exponential {
     ...
     */
    function distributeBUMMinterChumHum(address bumMinter, bool distributeAll) public {
        if (address(bumVaultAddress) != address(0)) {
            releaseToVault();
        }
     ...
     */
    function _setChumHumBUMVaultRate(uint chumhumBUMVaultRate_) public {
        require(msg.sender == admin, "only admin can");
     ...
     */
    function _setBUMVaultInfo(address vault_, uint256 releaseStartBlock_, uint256 minReleaseAmount_) public {
        require(msg.sender == admin, "only admin can");
...
contract TokenErrorReporter {
    ...
    enum FailureInfo {
        ...
        BUM_MINT_NEW_ACCOUNT_BALANCE_CALCULATION_FAILED
    }
...
contract BUMControllerErrorReporter {
    enum Error {
        NO_ERROR,
        UNAUTHORIZED,
        REJECTION,
...
contract CHUM is Tokenlock {
    ...
    /// @notice Total number of tokens in circulation
    uint public constant totalSupply = 30000000e18; // 30 million CHUM
...
/**
  * @title ChumHum's InterestRateModel Interface
  * @author ChumHum
  */
