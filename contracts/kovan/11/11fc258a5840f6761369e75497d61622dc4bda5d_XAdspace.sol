...
contract XAdspace {
   ...
   event BoughtAdspace(address spaceOwner, address token, uint256 tokens, string adURL, address renter);
   event CreatedAdProgram(address spaceOwner, address token, uint256 tokens, string adURL);
  ...
  function createAdProgram(address token, uint256 startPrice, string calldata initialUrl) public returns (bool) {
    ...
    emit CreatedAdProgram(from, token, startPrice, initialUrl);
  ...
  function buyAdspace(address spaceOwner, address token, uint256 tokens, string calldata adURL) public returns (bool) {
    ...
    emit BoughtAdspace(spaceOwner, token, remainingAdspaceValue + rentalPremium, adURL, from);
