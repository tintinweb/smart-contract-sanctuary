...
contract ERC20CollateralPayment is ERC20PaymentStandard, Ownable {
        ...
        )
    {
        ...
        uint256 ltv = ScoreOracleInterface(creditOracle).LTV(score);
...
interface ScoreOracleInterface {
  function requestUpdatedScoreBorrow(uint256, uint256, bytes32, bytes memory) external returns (uint16);
  function getCurrentScore(uint256) external view returns (uint16);

  // test function NOT ACTUALY IN INTERFACE
  function LTV(uint16) external view returns(uint256);
}
