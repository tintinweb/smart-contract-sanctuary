...
// File: contracts/tokens/xBuoy.sol
...
contract xBuoy is ERC721 {
    ...
    mapping (address => Stake) public activeStakes;
    ...
    struct NFT {
        ...
        uint256 nextClaim;    //the timestamp which allows the next payout
    ...
    function craftNFT(
        ...
        activeStakes[_sender].stakeList.push(_id);
        activeStakes[_sender].stakes++;
    ...
    function killNFT(uint id) public {
        ...
        activeStakes[ownerOf(id)].stakes--;
    }
    ...
    function viewActiveStakes(address x) public view returns(uint[] memory) {
        uint total = activeStakes[x].stakes;
        uint[] memory result = new uint[](total);
        uint counter;
        uint i;
        while(i < total) {
            uint id = activeStakes[x].stakeList[counter];
            if(nfts[id].active == true){
                result[i] = id;
                i++;
            }
            counter++;
