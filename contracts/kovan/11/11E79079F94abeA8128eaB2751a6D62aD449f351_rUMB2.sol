...
contract rUMB2 is rUMB {
    mapping (address => bool) public registeredPools;

    event PoolSetup(address pool, bool registered);
    ...
    function poolSetup(address _pool, bool _register) external onlyOwner() {
        registeredPools[_pool] = _register;
        emit PoolSetup(_pool, _register);
    }

    function mintReward(address _holder, uint256 _amount) external {
        require(registeredPools[msg.sender], "pool not registered");
