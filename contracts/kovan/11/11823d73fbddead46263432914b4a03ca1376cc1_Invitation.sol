...
contract ActivityBase is Ownable{
    ...
    function getPoolAmountLimit(uint256 _pid) public view virtual returns(uint256){
        return poolTokenAmountLimit[_pid];
    }
...
contract MasterchefActivityOne is ActivityBase{
    ...
    function pendingSHARD(uint256 _pid, address _user) external view virtual returns (uint256, uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        uint256 pending = user.modifiedWeight.mul(accShardPerWeight).div(1e12).sub(user.rewardDebt).add(user.revenue);
        (, uint256 userDividend) = calculateDividend(pending, _pid, user.amount, isUserContractSender[_pid][msg.sender]);
        return (userDividend, block.number);
    }
...
contract MasterchefActivityOneDelegator is MasterchefActivityOne, DelegatorInterface {
    ...
    function getPoolAmountLimit(uint256 _pid) public view override returns (uint256) {
        bytes memory data =
            delegateToViewImplementation(
                abi.encodeWithSignature("getPoolAmountLimit(uint256)", _pid)
            );
        return abi.decode(data, (uint256));
    }
...
contract Invitation is Ownable, IInvitation{
    ...
    MasterchefActivityOneDelegator public activity;
    ...
    function resetInvitationRelationship(address _user) public onlyOwner{
    ...
    function setActivityOne(MasterchefActivityOneDelegator _ac) public{
        activity = _ac;
    }
