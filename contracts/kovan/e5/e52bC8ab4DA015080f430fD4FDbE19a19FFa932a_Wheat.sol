...
/**
    * @dev Define the Equipment Traits that will be stored on chain
*/
...
contract FarmlandEquipment is ERC721, Ownable {
    using Counters for Counters.Counter;
    using SafeMath for uint256;
    Counters.Counter private _tokenIds;

    constructor() ERC721("Farmland Equipment", "EQUIP") public {
        _setBaseURI("https://ipfs.io/ipfs/"); // set the base URL to retrieve the NFT from IPFS, which means only the CID is stored on chain
    }

    /**
     * @dev PUBLIC: By making EquipmentTraits structs public we can read the attributes through the contract view
    */
    mapping(uint256 => EquipmentTraits) public equipmentTraits;
    ...
    */
      function mintEquipment(address receiver, string memory tokenURI, string memory equipmentName, uint256 maxBoostLevel, uint256 blocksToMaxBoost, uint256 expiryDate) external onlyOwner returns (uint256) {
        _tokenIds.increment();
        uint256 id = _tokenIds.current();

        // Store Key Equipment traits in on chain
        EquipmentTraits storage equipmentTrait = equipmentTraits[id];
        equipmentTrait.equipmentName = equipmentName;
        equipmentTrait.maxBoostLevel = maxBoostLevel;
        equipmentTrait.blocksToMaxBoost = blocksToMaxBoost;
        equipmentTrait.expiryDate = 0;
        if ( expiryDate > 0 ) {
            equipmentTrait.expiryDate = expiryDate;
        }
...
struct Farm {
    ...
    uint256 equipmentID;
    string equipmentName;
    uint256 equipmentmaxBoostLevel;
    uint256 equipmentblocksToMaxBoost;
    uint256 equipmentAddedBlockNumber;
    uint256 equipmentExpiry;
}

/**
 * @dev Farmland - Crop Smart Contract
 */
contract Wheat is ERC777, IERC777Recipient, ReentrancyGuard, IERC721Receiver, ERC165, ERC721Holder  {
    /**
    ...
    modifier preventSameBlock(address targetAddress) {
        require(
            ...
            "You can not perform these functions in the same block"
        );
    ...
    modifier requireFarm(address targetAddress, bool requiredState) {
        if (requiredState) {
            require(
                ...
                "You must have allocated land to your farm"
            );
        } else {
            require(
                farms[targetAddress].amount == 0,
    ...
    /**
     * @dev There must be a Equipment on this farm to execute this function
     */
    modifier requireEquipment(address targetAddress, bool requiredState) {
        if (requiredState) {
            require(
                farms[targetAddress].equipmentID != 0,
                "You must have equipmment on your farm"
            );
        } else {
            require(
                farms[targetAddress].equipmentID == 0,
                "You must not have equipment on your farm"
            );
        }
        _; // Call the actual code
    }
    ...
    /**
     * @dev This will be Farmland Equipment NFT token smart contract address
    */
    FarmlandEquipment private immutable nftToken;

    /**
     * @dev This is the LAND/DAI pair LP token used to calculate undelying LAND balance
     */
    IUniswapV2Pair private immutable lpToken20;

    /**
     * @dev This is the address for the treasury for the fees
     */
    ...
    constructor(
        ...
        token = IERC777(_token); // Land contract
        lpToken20 = IUniswapV2Pair(_lpToken20Address); //LP contract holding Land Pair
        _registerInterface(IERC721Receiver.onERC721Received.selector);
        nftToken = FarmlandEquipment(_nftTokenAddress); // NFT contract
     ...
     */
    uint256 private constant maxGrowthCycle = 302400;
     ...
     */
    uint256 private constant maturityBoostExtension = 20000;
     ...
     */
    uint256 private globalAllocatedAmount;

    /**
     * @dev PRIVATE: Store how much is crop has been composted globally (only from active farms on LAND addresses)
     */
    uint256 private globalCompostedAmount;

    /**
     * @dev PRIVATE: Store how many addresses currently have an active farm
     */
    uint256 private globalTotalFarms;

    /**
     * @dev PRIVATE: Store wLAND (Total Value Locked) in the crop contract
     */
    uint256 private globalAllocatedAmountLP;
    ...
    event EquipmentAllocated(
        address sender,
        uint256 blockNumber,
        uint256 TokenID,
        string equipmentName
    );
    event EquipmentReleased(
        address sender,
        uint256 blockNumber,
        uint256 TokenID,
        string equipmentName
    ...
    */
        ...
        preventSameBlock(_msgSender())
    {
        ...
        if (block.number < failsafeTargetBlock) {
            require(amount <= failsafeMaxAmount,"You can only allocate a maximum of 1000 LAND during failsafe.");
        }

        Farm storage senderFarm = farms[_msgSender()]; // Shortcut accessor
        (,,, uint256 amountUnderlying,) = getLiquidityPoolDetails(amount); // Returns the underlying LAND
        
        if (senderFarm.amount == 0) { // Check to see if there is LAND in the farm
            senderFarm.amountLP = amount; // Store the amount of LP token
            senderFarm.amount = amountUnderlying; // Stores the underlying LAND
            senderFarm.blockNumber = block.number; // Block when farm first setup
            senderFarm.harvesterAddress = farmerAddress; // Stores the farmers address
            globalCompostedAmount = globalCompostedAmount.add(senderFarm.compostedAmount); // retains any composted crops for returning farmers
            globalTotalFarms += 1; // Increment the total farms counter
        } else {
            senderFarm.amountLP = senderFarm.amountLP.add(amount); // Store the amount of LP token
            senderFarm.amount = senderFarm.amount.add(amountUnderlying); // Stores the underlying LAND
        }

        globalAllocatedAmount = globalAllocatedAmount.add(amountUnderlying);
        globalAllocatedAmountLP = globalAllocatedAmountLP.add(amount);
        senderFarm.lastHarvestedBlockNumber = block.number; // Reset the last harvest height whenever new LAND is allocated .. this means any unharvested crop will be lost
    ...
    /**
     * @dev PUBLIC: Releasing a farm returns LP Token to the owners
     */
    function release(uint256 amountLPToRelease)
        public
        nonReentrant()
        preventSameBlock(_msgSender())
    ...
    {
        Farm storage senderFarm = farms[_msgSender()]; // Shortcut accessor
        require(amountLPToRelease > 0, "You must provide a positive amount of LAND to release");
        require(amountLPToRelease <= senderFarm.amountLP, "You don't have this much LAND to release");

        // Calculate LAND based % of amountLP being released & update balances
        uint256 amount = senderFarm.amount.mul(amountLPToRelease).div(senderFarm.amountLP);
        senderFarm.amount = senderFarm.amount.sub(amount);
        senderFarm.amountLP = senderFarm.amountLP.sub(amountLPToRelease);
        globalAllocatedAmount = globalAllocatedAmount.sub(amount);
        globalAllocatedAmountLP = globalAllocatedAmountLP.sub(amountLPToRelease);

        // Check if the farm will be closed completely & adjust global balances
        if (senderFarm.amountLP == 0) {
            globalCompostedAmount = globalCompostedAmount.sub(senderFarm.compostedAmount); 
            globalTotalFarms = globalTotalFarms.sub(1);
        }

        uint256 feeAmountLP = fee.mul(amountLPToRelease).div(_percentMultiplier);
        uint256 amountLPLessFee = amountLPToRelease.sub(feeAmountLP);

        emit Released(_msgSender(), amountLPToRelease, senderFarm.compostedAmount);

        // Send back the LAND less the fee to person calling the method -- [RE-ENTRANCY WARNING] external call, must be at the end
        IERC777(token).send(_msgSender(), amountLPLessFee, ""); 
        // Send fee to the treasury on release -- [RE-ENTRANCY WARNING] external call, must be at the end
        IERC777(token).send(treasuryAddress, feeAmountLP, "");
    }
    ...
    function allocateEquipment(uint256 tokenID)
        public
        nonReentrant()
        preventSameBlock(_msgSender())
        requireFarm(_msgSender(), true) // You can't add equipment if you don't have a farm
        requireEquipment(_msgSender(), false) // You can't add equipment if you already have equipment
        {

        require(
            tokenID > 0,
            "You must provide a tokenID of the Equipment"
        );

        (string memory equipmentName, uint256 maxBoostLevel,uint256 blocksToMaxBoost,uint256 expiryDate) = nftToken.equipmentTraits(tokenID);

        // Store Key Equipment traits in on chain
        Farm storage farm = farms[_msgSender()];
        farm.equipmentID = tokenID;
        farm.equipmentAddedBlockNumber = block.number;
        farm.equipmentName = equipmentName;
        farm.equipmentmaxBoostLevel = maxBoostLevel;
        farm.equipmentblocksToMaxBoost = blocksToMaxBoost;
        farm.equipmentExpiry = expiryDate;

        emit EquipmentAllocated(_msgSender(),block.number,tokenID,equipmentName);
    ...
    function releaseEquipment()
        ...
        nonReentrant()
        preventSameBlock(_msgSender())
        requireEquipment(_msgSender(), true) // You can't remove equipment if you don't have any
        {

        Farm storage farm = farms[_msgSender()];
        uint256 tokenID = farm.equipmentID;
        string memory equipmentName = farm.equipmentName;
        
        // Wipe equipment traits
        farm.equipmentID = 0;
        farm.equipmentAddedBlockNumber = 0;
        farm.equipmentName = "";
        farm.equipmentmaxBoostLevel = 0;
        farm.equipmentblocksToMaxBoost = 0;
        farm.equipmentExpiry = 0;

        emit EquipmentReleased(_msgSender(),block.number,tokenID,equipmentName);
        ...
        returns (uint256 actualMinted)
    {
        ...
        uint256 lastBlockInGrowthCycle = targetFarm.lastHarvestedBlockNumber.add(maxGrowthCycle); // end of growth cycle last allowed block
        ...
        uint256 combinedMultipliers = getCombinedMultipliers(targetAddress).div(_percentMultiplier);
        uint256 afterMultiplier = blocksMintedByAmount.mul(combinedMultipliers);
        actualMinted = afterMultiplier.div(_harvestPerBlockDivisor);
    }

    /**
     * @dev PRIVATE: Returns the details associated with a liquidity pool position
     */
    function getLiquidityPoolDetails(uint256 amount)
        private
        view
        returns (
            ...
            uint256 underlyingToken0Balance,
            uint256 underlyingToken1Balance
        ) {
        require(amount > 0, "You must provide a positive amount to avoid divide by zero error");
        (uint112 _reserve0,uint112 _reserve1, ) = lpToken20.getReserves();
        token0Balance = uint256(_reserve0);
        token1Balance = uint256(_reserve1); 
        lpTotalSupply = lpToken20.totalSupply();
        require(amount <= lpTotalSupply, "You must provide a positive amount to avoid divide by zero error");
        
        // Calculates the underlying token balances for each of the pair
        underlyingToken0Balance = uint256(amount.mul(token0Balance).div(lpTotalSupply));
        underlyingToken1Balance = uint256(amount.mul(token1Balance).div(lpTotalSupply)); 
    }
        ...
        returns (uint256 timeMultiplier)
    {
        ...
        timeMultiplier = Math.min(maxMaturityBoost, blockDiff); // Min 1x, Max 3x
    }
    ...
    /**
     * @dev PUBLIC: Find out a farms Equipment boost for the current LAND address (Using 1 block = 13.5 sec formula)
     */
    function getAddressEquipmentMultiplier(address targetAddress)
        public
        view
        returns (uint256 equipmentMultiplier)
    {
        Farm storage targetFarm = farms[targetAddress]; // Shortcut accessor

        // Ensure this address has a Equipment on the LAND
        if (targetFarm.equipmentID == 0) {
            return _percentMultiplier;
        }

        // You don't get a boost until minimum blocks passed 
        uint256 targetBlockNumber = targetFarm.equipmentAddedBlockNumber; //.add(startMaturityBoost); TODO Decide if equipment boost starts immediately
        
        if (block.number < targetBlockNumber) {
            return _percentMultiplier;
        }

        // You don't get a boost if the MaxBoostLevel is 1x
        if (targetFarm.equipmentmaxBoostLevel <= _percentMultiplier) {
            return _percentMultiplier;
        }
        // You don't get a boost if the your equipment has expired; if Expiry equals zero then no expiry
        if (targetFarm.equipmentExpiry > 0 && block.timestamp >= targetFarm.equipmentExpiry) {
            return _percentMultiplier;
        }

        // Calculates the boost extension; i.e., the extension over and above 1x e.g., the 2x to get to a 3x boost
        uint256 equipmentBoostExtension = targetFarm.equipmentmaxBoostLevel.sub(_percentMultiplier);

        // 24 hours - min before starting to receive rewards
        // Max before multiplier stops growing depends on the equipment attributes 
        // The function returns PERCENT (10000x) the multiplier for 4 decimal accuracy
        uint256 blockDiff =
            block.number
            .sub(targetBlockNumber)
            .mul(equipmentBoostExtension) 
            .div(targetFarm.equipmentblocksToMaxBoost)
            .add(_percentMultiplier);

        equipmentMultiplier = Math.min(targetFarm.equipmentmaxBoostLevel, blockDiff); // Min 1x, Max depends on the equipment attributes 
    }
    /**
     * @dev PUBLIC: Get address specific underlying LP token details
     */
    function getCombinedMultipliers(address targetAddress)
        public
        view
        returns (uint256 totalMultiplier)
    {
        uint256 maturityMultiplier = getAddressMaturityMultiplier(targetAddress);
        uint256 compostMultiplier = getAddressCompostMultiplier(targetAddress);
        uint256 equipmentMultiplier = getAddressEquipmentMultiplier(targetAddress);
        totalMultiplier = compostMultiplier.mul(maturityMultiplier).mul(equipmentMultiplier).
        ...
        returns (uint256 globalRatio) 
    {
        ...
        // Note that we multiply both ratios by the ratio multiplier before dividing. For tiny CROP/LAND burn ratios.
        globalRatio = globalCompostedAmount.mul(_ratioMultiplier).div(globalAllocatedAmount);
    }
        ...
        returns (uint256 globalAverageRatio) 
    {
        ...
        // Compost/Maturity for both address & network
        // Note that we multiply both ratios by the ratio multiplier before dividing. For tiny CROP/LAND burn ratios.
        globalAverageRatio = globalCompostedAmount.mul(_ratioMultiplier).div(globalAllocatedAmount).div(globalTotalFarms);
    }
    ...
    function getAddressDetails(address targetAddress)
        ...
        returns (
            ...
            uint256 addressEquipmentMultiplier,
            uint256 addressCombinedMultipliers
        )
    {
        blockNumber = block.number;
        cropBalance = balanceOf(targetAddress);
        harvestAmount = getHarvestAmount(targetAddress, block.number);
        addressMaturityMultiplier = getAddressMaturityMultiplier(targetAddress);
        addressCompostMultiplier = getAddressCompostMultiplier(targetAddress);
        addressEquipmentMultiplier = getAddressEquipmentMultiplier(targetAddress);
        addressCombinedMultipliers = getCombinedMultipliers(targetAddress);
    }
    ...
    {
        blockNumber = block.number;
        isOperator = IERC777(token).isOperatorFor(address(this), targetAddress);
        lpLandBalance = IERC777(token).balanceOf(targetAddress);
        myRatio = getAddressRatio(targetAddress);
        globalRatio = getGlobalRatio();
        globalAverageRatio = getGlobalAverageRatio();
    }
        ...
        )
    {
        ...
        (,,, uint256 globalInFarmToken0Balance, uint256 globalInFarmToken1Balance) = getLiquidityPoolDetails(globalAllocatedAmountLP);
        ...
        return (
            ...
            globalAllocatedAmountLP,
            globalInFarmToken0Balance,
            globalInFarmToken1Balance
        );
    }
     ...
     * @dev PUBLIC: Get address specific underlying LP token details
     */
    function getAddressLiquidityPoolDetails(address targetAddress)
        public
        view
        returns (
            uint256 blockNumber,
            uint256 LandInFarmBalance,
            uint256 inFarmCurrentToken0Balance,
            uint256 inFarmCurrentToken1Balance,
            uint256 inWalletToken0Balance,
            uint256 inWalletToken1Balance
        )
    {
        blockNumber = block.number;
        Farm storage targetFarm = farms[targetAddress]; // Shortcut accessor
        LandInFarmBalance = targetFarm.amount;
        uint256 wLandInFarmBalance = targetFarm.amountLP;
        uint256 wLandWalletBalance = IERC777(token).balanceOf(targetAddress);
        (,,, inFarmCurrentToken0Balance, inFarmCurrentToken1Balance) = getLiquidityPoolDetails(wLandInFarmBalance);
        (,,, inWalletToken0Balance, inWalletToken1Balance) = getLiquidityPoolDetails(wLandWalletBalance);
    }
}
