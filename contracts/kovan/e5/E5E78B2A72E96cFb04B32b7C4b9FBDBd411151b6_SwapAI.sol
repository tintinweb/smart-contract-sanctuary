...
contract SwapAI is ISwapAI {
  ...
  function manualSwapUserBalance(bool toTUSD) external override {
    if (toTUSD) {
      ...
      _processAnalysisManualToTUSD(msg.sender, 6000, 105 * 10 ** 7, 95 * 10 ** 7);
    } else {
      // oracleMaster.executeAnalysis(address(this), this._processAnalysisManualToWBTC.selector);
      _processAnalysisManualToWBTC(msg.sender, 4000, 95 * 10 ** 7, 105 * 10 ** 7);
    }
  }
  ...
  function _processAnalysisManualToTUSD(
    ...
    require(
      IERC20(wbtcTokenAddr).transferFrom(address(this), address(tokenSwapper), currentUser.wbtcBalance),
      'Transfer of WBTC to TokenSwapper contract failed.'
    );

    // Then do the swap
    tokenSwapper.doManualSwap(currentUser, true);
  ...
  function _processAnalysisManualToWBTC(
    ...
    // First transfer the token amount to the token swapper contract
    require(
      IERC20(tusdTokenAddr).transferFrom(address(this), address(tokenSwapper), currentUser.tusdBalance),
      'Transfer of TUSD to TokenSwapper contract failed.'
    );

    // Then do the swap
    tokenSwapper.doManualSwap(currentUser, false);
