...
contract MoonMoney is TokenInterface {
    ...
    function _claimYields(address staker, uint amountStaked) internal returns (uint256 amt){
    ...
    function _mintFarmableToken(uint amount, address recipient) internal returns (bool)
    {
     ...
     /**
     * @dev Users can stake their tokens, requires preApproval.   
     * Tokens cannot be unstaked until a period of time elapses
     */
    function stakeTokens(uint amount) public returns (uint256 amt){
        ...
        uint newStakedTokensAmount = amount.add(stakedTokens[from].tokenAmount);
        
        //add these tokens to the valt and reset the timer 
        stakedTokens[from] = TokenVault( newStakedTokensAmount , blockNum );
        
        //forcibly claim yields here to reset the block number 
        return _claimYields(from, amount);
    ...
    function restakeTokens() public returns (uint256 amt){
        ...
        return _claimYields(from, amountStaked);
    }
