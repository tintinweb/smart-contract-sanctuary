...
// File: localhost/mint/implement/BorrowProxyTest.sol
...
contract BorrowProxyTest is IBorrowProxy, ConfigBase {
    ...
    function borrowV1(address owner, uint256 id, address tokenA, uint256 amountA, address tokenB, uint256 amountB, address borrowToken, uint256 leverage, uint256 deadLine) override external returns (uint256, uint256, uint256) {
        ...
        if (0 < leverage) {
            ...
            if (tokenA == borrowToken) {
                ...
                amountA = amountA.add(config.getExchange().swapExtractOut(borrowToken, tokenA, address(this), leverage, 1, deadLine));
            }
        }
    ...
    function trim(address tokenA, uint256 amountA, address tokenB, uint256 amountB, uint256 deadLine) internal returns (uint256, uint256) {
        ...
        if (valueA < valueB) {
            ...
            if (0 < swapB) {
                ...
                price = config.getExchange().swapExtractOut(tokenB, tokenA, address(this), swapB, 1, deadLine);
            ...
            if (0 < swapA) {
                ...
                price = config.getExchange().swapExtractOut(tokenA, tokenB, address(this), swapA, 1, deadLine);
    ...
    function trimV2(address tokenA, uint256 amountA, address tokenB, uint256 amountB, uint256 deadLine) internal returns (uint256, uint256) {
        uint256 oneA = 10 ** IERC20(tokenA).decimals();
        uint256 price = config.getExchange().swapEstimateOut(tokenA, tokenB, oneA);
        require(0 < price, "publics:swap_estimate_out_is_zero");
        uint256 valueA = amountA.mul(price);
