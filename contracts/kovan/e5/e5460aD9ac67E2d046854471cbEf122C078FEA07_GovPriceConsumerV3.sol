...
contract GovPriceConsumerV3 {
    // if(address(usdPriceAggrigators[tokenAddress])==address(0))){
    //     //get price from dex
    // }
    // else{
    ...
    event PriceFeedAdded(address indexed token, ChainlinkDataFeed _chainLinkDataFeed);
    event PriceFeedAddedBulk(address[] indexed tokens, ChainlinkDataFeed[] _chainLinkDataFeed);
     ...
     */
    constructor(address _govWorldAdminRegistry) 
    {
    ...
    *@param _chainLinkDataFeed The contract address of chainlink aggrigator.
    */
    function addUsdPriceAggrigator(address _tokenAddress, ChainlinkDataFeed memory _chainLinkDataFeed )
        external
        onlyAddTokenRole(msg.sender)
    {
        usdPriceAggrigators[_tokenAddress] = _chainLinkDataFeed;
        emit PriceFeedAdded( _tokenAddress, _chainLinkDataFeed);
    }
    ...
    @param _chainLinkDataFeed The contract address of the chainlink aggregator
     */

    function addUsdPriceAggrigatorBulk(address[] memory _tokenAddress, ChainlinkDataFeed[] memory _chainLinkDataFeed )
        external
        onlyAddTokenRole(msg.sender)
    {   
        
        for(uint256 i = 0 ; i < _tokenAddress.length; i++){
            this.addUsdPriceAggrigator(_tokenAddress[i], _chainLinkDataFeed[i]);
        }
        
         emit PriceFeedAddedBulk( _tokenAddress, _chainLinkDataFeed);
    }
    ...
    */
     ...
     */
    function getLatestUsdPrice(address priceFeedToken) 
        public 
        view 
        returns (int,uint8) 
    {
    ...
    function getLatestUsdPrices(address[] memory priceFeedToken) 
        public 
        view 
        returns (
    ...
    // calculate price based on pair reserves  via UniswapV2Factory
        ...
        returns(uint)
    {
        ...
        IDexPair pair = IDexPair(IDexFactory(_factory).getPair(_tokenA, _tokenB));
        IERC20Extras token1 = IERC20Extras(pair.token1());
        (uint resToken0, uint resToken1,) = pair.getReserves();

        // decimals
        uint res0 = resToken0*(10**token1.decimals());
        return((amount*res0)/resToken1); // return amount of token0 needed to buy token1
    }
    ...
    function getTokenPriceInBorrowed(
        ...
        if (pair.token0() == _stable) {
            ...
            uint256 altAmount = _amount.mul(resD.div(res1));
            return altAmount.div(token1Decimals);
            ...
            uint256 stableAmount = _amount.mul(resD.div(res0));
            return stableAmount.div(token0Decimals);

            // return (_altAmount.mul(resD.div(res0))); // return amount of stable needed to buy 1 altcoin
        }
    }
}
