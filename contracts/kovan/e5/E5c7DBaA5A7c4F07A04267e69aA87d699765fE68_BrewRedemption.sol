...
contract BrewRedemption is Ownable, ReentrancyGuard {
    ...
    function redeem(uint256[] memory orcIds ) external nonReentrant  {
        ...
        for (uint i = 0; i < orcIds.length; i++) {
            ...
            if(RedeemListInfo[orcIds[i]] > 0){
                amount = amount.add(block.timestamp.sub(RedeemListInfo[orcIds[i]]).div(60).mul(redemptionAmount));
            } else {
                if(orcIds[i] <= 100){
            ...
            emit RedeemedBrew(_msgSender(), orcIds[i], periodNumber, redemptionAmount);
        }
    ...
    function giveawayBrew(uint256 orcId, uint256 amount) external onlyOwner {
        brew.mint(orc.ownerOf(orcId), amount);

        emit RedeemedGiveawayBrew(orc.ownerOf(orcId), orcId, amount);
    }
