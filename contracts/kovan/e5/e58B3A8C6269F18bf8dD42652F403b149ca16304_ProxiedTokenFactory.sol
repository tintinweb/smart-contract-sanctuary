...
contract ProxiedSyntheticToken is SyntheticToken {
    ...
    function initialize(
        ...
        require(decimals() == 0, "Token must be uninitialized");
        _name = _tokenName;
...
import "./ProxiedSyntheticToken.sol";
...
contract ProxiedTokenFactory is Lockable {
    address public proxiedSyntheticTokenTemplate;

    constructor(address _proxiedSyntheticTokenTemplate) {
        proxiedSyntheticTokenTemplate = _proxiedSyntheticTokenTemplate;
    }
    ...
    function createToken(
        ...
        ProxiedSyntheticToken mintableToken = ProxiedSyntheticToken(Clones.clone(proxiedSyntheticTokenTemplate));

        mintableToken.initialize(tokenName, tokenSymbol, tokenDecimals);
