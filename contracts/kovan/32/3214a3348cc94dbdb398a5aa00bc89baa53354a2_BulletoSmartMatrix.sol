...
contract BulletoSmartMatrix {
        ...
        internal
    {
        ...
        users[userAddress] = User({
            ...
            refCount: 0,
            balanceBbond: 0,
            balanceBfi: users[userAddress].balanceBfi
        });
    ...
    function updateMatrixReferrer(
        ...
        if (users[referrerAddress].ActiveMatrix[level].firstLevelReferrals.length < 2 ) { // 1st and 2nd point
            ...
            if (referrerAddress == owner) {
                ...
                return sendBonds(referrerAddress, userAddress, level);
            }
            // Update user reffer
    ...
    function updateActiveReferrerSecondLevel(address userAddress, address referrerAddress, uint8 level) internal {
        if (
            ...
            if(users[referrerAddress].ActiveMatrix[level].reinvestCount == 1
               && users[referrerAddress].activePlanLevel[level+1] == false
            ){
                return sendBonds(owner, userAddress, level);
            }
    ...
    function setReflection(address userAddress, address referrerAddress, uint8 level) internal{
        ...
        while(true){   
            ...
            }else return sendBonds(owner, userAddress, level);
        }
    }
