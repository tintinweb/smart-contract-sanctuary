...
contract FeeManager is IFeeManager, Ownable
{
  ...
  function getBorrowFeeOnDebt (bool borrowerIsDiscounted, uint256 debtInCollateralToken) external view override returns (uint256, address)
  {
    // 25 = 0.25% in basis point; 1 - 0.25 = 0.75% -> 25% discount
    uint256 fee = borrowerIsDiscounted ? _rate.borrow - 25 : _rate.borrow;


    return (_calcPercentOf(debtInCollateralToken, fee), _burner);
  }
...
interface IFeeManager
{
  ...
  function getBorrowFeeOnDebt (bool borrowerIsDiscounted, uint256 debtInCollateralToken) external view returns (uint256, address);
}
