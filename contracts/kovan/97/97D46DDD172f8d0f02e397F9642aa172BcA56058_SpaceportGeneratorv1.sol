...
contract SpaceportGeneratorv1 is Ownable {
    ...
    struct SpaceportParams {
        uint256 amount;                 //  сколько проект хочет продать своих токенов
        uint256 tokenPrice;             //  цена за токен
        uint256 maxSpendPerBuyer;       //  сколько макс устанавливает проект - макс для юзера
        uint256 hardcap;                //  макс сколько проект хочет собрать в base токене
        uint256 softcap;                //  минимально
        uint256 liquidityPercent;       //  блокировка собранной части ликвидности
        uint256 listingRate;            // sale token listing price on plasmaswap
        uint256 startblock;             //  
        uint256 endblock;
        uint256 lockPeriod;             //  на сколько блок ликв
    }
    
    constructor(address factory, address settings) public {
        SPACEPORT_FACTORY = ISpaceportFactory(factory);
        SPACEPORT_SETTINGS = ISpaceportSettings(settings);
    }
    ...
    function createSpaceport (
      ...
      uint256[10] memory uint_params
        ...
        newSpaceport.init2(_baseToken, _spaceportToken, SPACEPORT_SETTINGS.getBaseFee(), SPACEPORT_SETTINGS.getTokenFee(), SPACEPORT_SETTINGS.getEthAddress(), SPACEPORT_SETTINGS.getTokenAddress());
...
contract Spaceportv1 is ReentrancyGuard {
  ...
  function userDeposit (uint256 _amount) external payable nonReentrant {
    ...
    buyer.vestingCliff = 5 minutes;
    buyer.vestingEnd = 20 minutes;
  ...
  function userWithdrawTokens () external nonReentrant {
    ...
    BuyerInfo storage buyer = BUYERS[msg.sender];
    require(STATUS.LP_GENERATION_COMPLETE_TIME + buyer.vestingCliff < block.timestamp, "vesting cliff : not time yet");
    ...
    if(STATUS.LP_GENERATION_COMPLETE_TIME + buyer.vestingEnd < block.timestamp) {
      STATUS.TOTAL_TOKENS_WITHDRAWN = STATUS.TOTAL_TOKENS_WITHDRAWN.add(buyer.tokensOwed);
      buyer.tokensOwed = 0;
    } 
    else {
      tokensOwed = buyer.vestingTokens.mul(block.timestamp - buyer.lastUpdate).div(buyer.vestingEnd);
      buyer.lastUpdate = block.timestamp;

      uint256 diff = buyer.vestingTokens.div(tokensOwed);
      STATUS.TOTAL_TOKENS_WITHDRAWN = STATUS.TOTAL_TOKENS_WITHDRAWN.add(buyer.vestingTokensOwed.div(diff));

      buyer.tokensOwed = buyer.tokensOwed.sub(buyer.vestingTokensOwed.div(diff));
      require(buyer.tokensOwed > 0, 'buyer.tokensOwed.sub(buyer.tokensOwed.div(diff))');
    }
...

{
  ...
  "libraries": {
    "/Users/pavelbulochkin/plasmapay/Launchpad-testdev/contracts/Spaceport-v1-periphery/contracts/SpaceportHelper.sol": {
      "SpaceportHelper": "0x3371243778872704db216C2dB704f14cd731C644"
    }
  },
  "outputSelection": {
    "*": {
