...
contract InfoAdapter {
    ...
    function getSLPInfo(address _lsp) internal view returns (LSPInfo memory lspInfo) {
        // slp properties
        ...
        TokenInfo memory collateralTokenInfo;
        collateralTokenInfo.name = collateralTokenInstance.name();
        collateralTokenInfo.symbol = collateralTokenInstance.symbol();
        collateralTokenInfo.decimals = collateralTokenInstance.decimals();
        collateralTokenInfo.tokenAddress = address(collateralTokenInstance);

        // save in the lspInfo
        lspInfo.collateralTokenInfo = collateralTokenInfo;

        // longTokenInstance
        TokenInfo memory longTokenInfo;
        longTokenInfo.name = longTokenInstance.name();
        longTokenInfo.symbol = longTokenInstance.symbol();
        longTokenInfo.decimals = longTokenInstance.decimals();
        longTokenInfo.tokenAddress = address(longTokenInstance);

        // save in the lspInfo
        lspInfo.longTokenInfo = longTokenInfo;

        // shortTokenInstance
        TokenInfo memory shortTokenInfo;
        shortTokenInfo.name = shortTokenInstance.name();
        shortTokenInfo.symbol = shortTokenInstance.symbol();
        shortTokenInfo.decimals = shortTokenInstance.decimals();
        shortTokenInfo.tokenAddress = address(shortTokenInstance);

        // save in the lspInfo
        lspInfo.shortTokenInfo = shortTokenInfo;
    }

    function getEMPInfo(address _emp) internal view returns (EMPInfo memory empInfo) {
        // emp properties
