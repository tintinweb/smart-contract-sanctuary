...
import './interfaces/IBaoziFactory.sol';
import './interfaces/IBaoziCallee.sol';
...
contract BaoziPair is IBaoziPair, BaoziTRC20 {
    ...
    event Log(string msg, address feeTo, uint256 amount, uint256 vr);
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        ...
        if (feeOn) {
            if (_kLast != 0) {
                ...
                if (rootK > rootKLast) {
                    uint liquidity = (totalSupply * (rootK - rootKLast)) / (rootK * 4 + rootKLast);
                    if (liquidity != 0) _mint(feeTo, liquidity);
                    emit Log("Fee to, liquidity", feeTo, liquidity, 0);
                }
                emit Log("Fee to, liquidity", address(0), rootKLast, rootK);
            }
        } else if (_kLast != 0) {
            kLast = 0;
        }
    ...
    function mint(address to) external override lock returns (uint liquidity) {
        ...
        _mint(to, liquidity);
        emit Log("liquidity", address(0), liquidity, 0);
...
import './interfaces/IBaoziTRC20.sol';
