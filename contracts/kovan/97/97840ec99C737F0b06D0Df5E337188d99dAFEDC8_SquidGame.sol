...
contract SquidGame is ERC721, ERC721Enumerable, Ownable, ERC721Burnable, ERC721Pausable, VRFConsumerBase {
  ...
  uint256 public lastRequestedBurn = 0; // Tracks the number of burns that have been requested so far
  string public baseTokenURI; // Base token URI
  GameState public gameState; // The current state of the game

  bytes32 private chainlinkRequestId; // The requestId for chainlink

  // Constants
  uint256 public constant NUM_ROUNDS = 3; // Number of rounds this game will last
  uint256 public constant MAX_ELEMENTS = 2**NUM_ROUNDS; // Max number of tokens minted
  uint256 public constant PRICE = 1 * 10**15; // Price to mint a single token
  uint256 public constant MAX_BY_MINT = 5; // Max number of tokens to be minted in a single transaction
  uint256 public constant CREATOR_PERCENTAGE = 30; // Percentage given to creators
  ...
  constructor(string memory baseURI)
    ERC721('Squid Game', 'SQUID')
    VRFConsumerBase(
      0xdD3782915140c8f3b190B5D67eAc6dc5760C46E9, // VRF Coordinator (Kovan)
      0xa36085F69e2889c224210F603D836748e7dC0088 // LINK Contract Token (Kovan)
    )
  {
  ...
  function setGameState(GameState newGameState) private onlyOwner {
  ...
  function startBurn() public onlyOwner {
    ...
    require(totalSupply() > 1, 'Game has finished');
    _getRandomNumber();
  }
  ...
  function claimWinnerReward() public {
    ...
    console.log('Winner address: %s', winnerAddress);
    require(_msgSender() == winnerAddress, 'Not winner');
    console.log('Winner reward: %s', address(this).balance);
  ...
  function claimCreatorReward() public onlyOwner {
    ...
    console.log('Creator reward: %s', creatorReward);
  ...
  function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
    ...
    _burnSupply(randomness % 2);
  }

  // uint256[] tokensToBurn;

  // TODO: ADD INTERNAL
  function _burnSupply(uint256 startingIndex) public {
