...
/**
 * @title DEMOSTBX | Stobox Technologies Security Token
 * @author Stobox Technologies Inc.
 * @dev DEMOSTBX ERC20 Token | This contract is opt for digital securities management.
 */
 
contract DEMOSTBXToken is IERC20, Roles {
    ...
    modifier onlyWhitelisted(address _account) {
        if (_isDisabledWitelist) {
            require(
                ...
                "DEMOSTBX: not whitelisted address."
            );
        }
        _;
    }
    ...
    modifier onlyWithUnfrozenFunds(address _account) {
        require(
            ...
            "DEMOSTBX: funds are frozen."
        );
        _;
    }

    /**
     * @notice DEMOSTBXToken simply implements a ERC20 token.
     */
    constructor() public Roles(msg.sender) {
        _name = "Stobox Technologies Security Token";
        _symbol = "DEMOSTBX";
        _decimals = 0;
        _kDecimals = 18;
        _k = 10**_kDecimals;
    ...
    function burn(address _account, uint256 _amount) external onlySuperAdmin {
        require(balanceOf(_account) >= _amount, "DEMOSTBX: balance too low");
    ...
    function transferFunds(
        ...
        require(balanceOf(_from) >= _amount, "DEMOSTBX: not enough tokens");
    ...
    function splitOrMerge(uint256 _x, uint256 _y) external onlySuperAdmin {
        require(_x != _y, "DEMOSTBX: _x must not be equal to _y");
        require(_x > 0, "DEMOSTBX: _x must be greater than 0");
        require(_y > 0, "DEMOSTBX: _y must be greater than 0");
    ...
    */
        ...
        returns (bool)
    {
        ...
        _approve(
            ...
            _allowances[_sender][msg.sender].sub(
                ...
                "DEMOSTBX: transfer amount exceeds allowance"
            )
        );
        ...
        returns (bool)
    {
        _approve(
            ...
            _allowances[msg.sender][_spender].sub(
                ...
                "DEMOSTBX: decreased allowance below zero"
            )
        );
    ...
    function _transfer(
        ...
        require(_sender != address(0), "DEMOSTBX: transfer from the zero address");
        require(_recipient != address(0), "DEMOSTBX: transfer to the zero address");

        _beforeTokenTransfer(_sender, _recipient, _amount);

        _balances[_sender] = _balances[_sender].sub(
            _amount,
            "DEMOSTBX: transfer amount exceeds balance"
        );
    ...
    function _mint(address _account, uint256 _amount) internal virtual {
        require(_account != address(0), "DEMOSTBX: mint to the zero address");
    ...
    function _burn(address _account, uint256 _amount) internal virtual {
        require(_account != address(0), "DEMOSTBX: burn from the zero address");

        _beforeTokenTransfer(_account, address(0), _amount);

        _balances[_account] = _balances[_account].sub(
            _amount,
            "DEMOSTBX: burn amount exceeds balance"
        );
    ...
    function _approve(
        ...
        require(_owner != address(0), "DEMOSTBX: approve from the zero address");
        require(_spender != address(0), "DEMOSTBX: approve to the zero address");
    ...
    function _updateTransferLimit(address _account, uint256 _amount) private {
        ...
        _transferLimits[_account].allowedToTransfer = _transferLimits[_account]
            .allowedToTransfer
            .sub(_amount, "DEMOSTBX: transfer exceeds your transfer limit");
    }
    ...
    function _updateTransactionCountLimit(address _account) private {
        if (_isEnabledTransactionCount) {
            ...
            _transactionCountLimits[_account]
                ...
                .sub(1, "DEMOSTBX: transfer exceeds your transaction count limit");
        }
    }
}
