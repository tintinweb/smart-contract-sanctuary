...
compactor: public(HashMap[address, int128])
...
def _totalToPaid() -> uint256:
    ...
    if self.endBlock == 0:
        return 0
    if self.endBlock > self.lastBlock:
...
def incentives_block(end: uint256):
    ...
    assert end > self.lastBlock  # This is essentially blocknumber, but potentially gives more flexibility when few are using the contract
...
def bal(user: address) -> (uint256, uint256):
    rewards: uint256 = convert(
        convert(
            (self.paidPerToken + self._totalToPaid() * max(self.totalStaked, 1)) * self.holdings[user]/10**6 - self.userRewardsPaid[user], int128)
