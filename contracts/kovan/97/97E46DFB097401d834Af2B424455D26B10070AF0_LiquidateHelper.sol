...
contract LiquidateHelper {
    ...
    function batchQuery(IOpenLev openLev, address[] calldata owners, uint16[] calldata marketIds, bool[] calldata longTokens) external view returns (Vars[] memory results){
        ...
        for (uint i = 0; i < owners.length; i++) {
            //            (uint current, uint32 marketLimit) = marginRatioInternal(openLev, priceOracle, owners[i], marketIds[i], longTokens[i]);
            //            results[i] = Vars(current, marketLimit);
            ...
            if (trade.marketValueOpen > marketValueCurrent) {// losing
                ...
                if (trade.depositFixedValue >= pnl) {
                    results[i] = Vars(trade.depositFixedValue.sub(pnl).mul(10000).div(borrowed), vars.marginRatio);
                    continue;
                    //                    return (trade.depositFixedValue.sub(pnl).mul(10000).div(borrowed), vars.marginRatio);
                } else {
                    results[i] = Vars(0, vars.marginRatio);
                    continue;
                    //                return (0, vars.marginRatio);
                }
            } else {// gaining
                uint pnl = marketValueCurrent.sub(trade.marketValueOpen);
                results[i] = Vars(trade.depositFixedValue.add(pnl).mul(10000).div(borrowed), vars.marginRatio);
                continue;
//                return (trade.depositFixedValue.add(pnl).mul(10000).div(borrowed), vars.marginRatio);
    ...
    function marginRatioInternal(IOpenLev openLev, PriceOracleInterface priceOracle, address owner, uint16 marketId, bool longToken)
