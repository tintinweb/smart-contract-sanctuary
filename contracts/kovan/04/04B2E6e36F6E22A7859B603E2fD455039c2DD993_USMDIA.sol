...
contract USMDIA is USMTemplate{
    constructor(Oracle oracle_, address[] memory optOut_, IERC20 collateral_,  string memory syntheticFeed_) 
        USMTemplate(
            oracle_,
            optOut_,
            collateral_,
...
/**
 * @dev This is a modified version of the original USM code. To avoid misunderstandings when comparing the codes, 
 * the name of the variables has not been modified, but at all times it should be understood that 'eth' means 
 * 'collateral' and 'usm' synthetic. 
 * e.g. the 'ethToUsm' function returns the price of the amount of collateral in the total of synthetic.
 * Thus, the collateral can be any ERC20 token and the synthetic any asset.
 */

contract USMTemplate is IUSM, Oracle, ERC20Permit, WithOptOut, Delegable {
    using WadMath for uint;

    uint256 private immutable fixedWAD;
    ...
    constructor(Oracle oracle_, address[] memory optedOut_, IERC20 _collateral, string memory name, string memory symbol, string memory fundingName, string memory fundingSymbol)
        ERC20Permit(name, symbol)
        WithOptOut(optedOut_)
    {
        fixedWAD = 10 ** (18 - uint8(ERC20(address(_collateral)).decimals()));
        oracle = oracle_;
     ...
     */
    function mint(address to, uint amount, uint minUsmOut) external override returns (uint usmOut) {
        usmOut = _mintUsm(to, amount, minUsmOut);
    }
     ...
     */
    function fund(address to, uint amount, uint minFumOut) external override returns (uint fumOut) {
        fumOut = _fundFum(to, amount, minFumOut);
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override noOptOut(recipient) returns (bool) {
        if (recipient == address(this) || recipient == address(fum) || recipient == address(0)) {
            _burnUsm(sender, sender, amount, MinOut.parseMinEthOut(amount));
        } else {
    ...
    function _mintUsm(address to, uint amount, uint minUsmOut) internal returns (uint usmOut)
    {
        ...
        (usmOut, adjShrinkFactor) = usmFromMint(ls, amount);
    ...
    function _burnUsm(address from, address to, uint usmToBurn, uint minEthOut) internal returns (uint ethOut)
    {
        // 1. Load the stored state:
        LoadedState memory ls = loadState();
        ...
        _storeState(ls);
        ethOut /= fixedWAD;
        collateral.transfer(to, ethOut);
    }

    function _fundFum(address to, uint amount, uint minFumOut) internal returns (uint fumOut)
    {
        // 1. Load the stored state:
        LoadedState memory ls = loadState();
        ...
        uint adjGrowthFactor;
        (fumOut, adjGrowthFactor) = fumFromFund(ls, fumSupply, amount, debtRatio_);
    ...
    function _defundFum(address from, address to, uint fumToBurn, uint minEthOut) internal returns (uint ethOut)
    {
        // 1. Load the stored state:
        LoadedState memory ls = loadState();
    ...
    function ethPool() public override view returns (uint pool) {
        pool = collateral.balanceOf(address(this)) * fixedWAD;
    }
