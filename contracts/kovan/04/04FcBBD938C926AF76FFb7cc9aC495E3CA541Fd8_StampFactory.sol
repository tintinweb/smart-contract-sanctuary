...
contract StampFactory is ERC721Enumerable, Ownable {
    ...
    uint internal _maxStamps;
    uint public constant MAX_MINCE_PER_WALLET = 5;
    uint public constant MINT_PRICE = 40000000000000000; // 0.04ETH
    
    bool internal _saleActive = false;    

    string public BASE_URI;

    constructor(string memory name, string memory symbol, uint maxSupply) ERC721(name, symbol) {
        _maxStamps = maxSupply;
    }
    ...
    function maxStampSupply() public view returns (uint) {
        return _maxStamps;
    }
    ...
    function mintStamp(uint count) public payable {
        require(msg.value >= (count * MINT_PRICE), "Not enough ETH");
        require(_saleActive, "Sale is not active");
        require((totalSupply() + count) <= _maxStamps, "All stamps minted");
        require((mincePerWallet[msg.sender] + count) <= MAX_MINCE_PER_WALLET, "You hit the limit");
