...
contract ChainlinkFiatOracle is IOracle, AccessControl {
    ...
    function getRate(address fxAsset) external view override returns(uint unitPrice){
        // Get prices
         (
            uint80 roundID,  
            int ethPrice,
        ...
        int ethIndirect = 1 ether;
        ethIndirect = ethIndirect.div(ethPrice);
        
        // Use that to calculate eth required per unit
        int finalQuote = ethIndirect.mul(ISOPrice);
        unitPrice = uint(finalQuote >=0 ? finalQuote : -finalQuote);
    }


    function setOracle(address fxAsset, address oracle) external override onlyOperator{
