...
contract Pool is Ownable {
  ...
  */
    ...
    function buyTickets(uint256 _count) public {
        ...
        deposit(totalDeposit, address(token), msgSender());
    }
...
*/
 ...
 @param  wallet : The address of the user's wallet to recieve the aTokens
 */

    function depositWithChecks(
        uint256 amount,
        address asset,
        address wallet
    ) external {
        ...
        require(
            IERC20(asset).allowance(wallet, lendingPool) >= amount,
            "Approval required for deposit"
        );
        require(!ILendingPool(lendingPool).paused(), "Lending Pool Locked");

        ILendingPool(lendingPool).deposit(asset, amount, wallet, 0);
    }

    function depositNoChecks(
        uint256 amount,
        address asset,
        address wallet
    ) external {
...
interface ILendingPool {
    ...
    function getAddressesProvider()
        external
        view
        returns (ILendingPoolAddressProvider);
