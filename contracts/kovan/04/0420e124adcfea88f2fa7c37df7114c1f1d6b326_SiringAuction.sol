...
abstract contract AuctionBase is IAuction {
        ...
        returns (uint256)
    {
        ...
        if(address(ticketToken) != address(0)){
            if(_buyer == seller){
                ...
                ticketToken.transferFrom(_buyer, feeAddr, price.mul(65).mul(30).div(10000));
            }
            else{
                ...
                ticketToken.transferFrom(_buyer, feeAddr, price.mul(15).div(100));
            }
        }
    ...
    function isOnAuction(uint256 _tokenId) external view override returns (bool) {
    ...
    function getPrice(uint256 _tokenId) external view override returns (uint256){
        Auction memory _auction = tokenIdToAuction[_tokenId];
        return _auction.price;
    }

    function getAuction(uint256 _tokenId) external view override returns (Auction memory) {
        Auction memory _auction = tokenIdToAuction[_tokenId];
        return _auction;
    }
}
...
contract SiringAuction is AuctionBase,Ownable {
    ...
    event Bid(address buyer, uint256 tokenId);

    constructor(address _nftAddress, address _ticketToken, address _prizePoolAddr, address _feeAddr) {
    ...
    function setPrizePoolAddr(address _prizePoolAddr) external onlyOwner {
        prizePoolAddr = _prizePoolAddr;
    }
        ...
        override
    {
        ...
        emit AuctionCreated(_tokenId, _price, auctionDuration);
    }
