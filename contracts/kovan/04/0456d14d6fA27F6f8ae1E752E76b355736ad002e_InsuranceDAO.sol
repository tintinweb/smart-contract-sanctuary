...
contract InsuranceDAO is Ownable{
    ...
    mapping (uint => mapping (address => uint)) public roundDeposit; // deposit for round
    mapping (uint => mapping (address => uint)) public levelsEntered;//level of each driver originally for payout(not cost!)
    ...
    function addToDao (uint256 level, uint256 tokenId, uint256 timeStamp, bytes32 r, bytes32 s, uint8 v) public payable {
        ...
        if(levelsEntered[_round.current()][_msgSender()] > 0){
            require(roundBalances[_round.current()][_msgSender()] == roundDeposit[_round.current()][_msgSender()], "cannot change level once a payout is made");
            ...
            roundDeposit[_round.current()][_msgSender()] = costSchedule[level-1];
