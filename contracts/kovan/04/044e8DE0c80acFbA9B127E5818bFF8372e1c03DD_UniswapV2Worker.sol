...
contract UniswapV2Worker is OwnableUpgradeSafe, ReentrancyGuardUpgradeSafe, IUniswapV2Worker{
  /// @notice Libraries
  ...
  function initialize(
    ...
    require(
      ...
      (baseToken == lpToken.token0() || baseToken == lpToken.token1()), "UniswapV2Worker::initialize:: LP underlying not match with farm & base token");
    
  }

  /// @dev Require that the caller must be an EOA account to avoid flash loans.
  modifier onlyEOA() {
    require(msg.sender == tx.origin, "UniswapV2Worker::onlyEOA:: not eoa");
    _;
  }

  /// @dev Require that the caller must be the operator.
  modifier onlyOperator() {
    require(msg.sender == operator, "UniswapV2Worker::onlyOperator:: not operator");
    _;
  }
    ...
    onlyOperator nonReentrant
  {
    ...
    console.log('UniswapV2Worker:share',share);
    (address strat, bytes memory ext) = abi.decode(data, (address, bytes));
    require(okStrats[strat], "UniswapV2Worker::work:: unapproved work strategy");
    uint256 beforeBEP20 = farmingToken.myBalance().sub(share);
    console.log('UniswapV2Worker:beforeBEP20',beforeBEP20);
    console.log('UniswapV2Worker:before:baseToken.myBalance()',baseToken.myBalance());
    ...
    console.log('UniswapV2Worker:fBalance',fBalance);
    console.log('UniswapV2Worker:baseToken.myBalance()',baseToken.myBalance());
  ...
  function getMktSellAmount(uint256 aIn, uint256 rIn, uint256 rOut) public view returns (uint256) {
    if (aIn == 0) return 0;
    require(rIn > 0 && rOut > 0, "UniswapV2Worker::getMktSellAmount:: bad reserve values");
  ...
  function _addShare(uint256 id, uint256 share) internal {
    ...
    require(farmingToken.myBalance() >= totalShare, 'UniswapV2Worker::addShare::Wrong Math');
  ...
  function _removeShare(uint256 id) internal returns(uint256){
    require(farmingToken.myBalance() >= totalShare, 'UniswapV2Worker::removeShare:Wrong Math');
