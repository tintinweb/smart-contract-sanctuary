...
contract NuPayNFT is ERC721URIStorage, Ownable, Pausable {
    ...
    constructor(address market_) ERC721("NuPay", "NPAY") {
        _contractAddress = payable(owner());
    ...
    function pause() external onlyMarket returns (bool) {
        _pause();
        return true;
    }

    function unpause() external onlyMarket returns (bool) {
        _unpause();
        return true;
    }

    function setContractAddress(address newContract) external onlyOwner returns (bool) {
        _contractAddress = payable(newContract);
        return true;
    }

    function setMarket(address newMarket) external onlyMarket returns (bool) {
    ...
    function _transfer(
        ...
        _market.resolveDeal(tokenId, from, to);
    }
    ...
    modifier onlyMarket() {
        require(address(_market) == _msgSender(), "Caller is not the market");
        _;
    }
}
