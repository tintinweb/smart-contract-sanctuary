...
import {Policy, PolicyInitArgs, LibPolicy} from "./libraries/LibPolicy.sol";
import {Product, LibProduct} from "./libraries/LibProduct.sol";
import {Benefit, LibBenefit, BenefitName} from "./benefits/LibBenefit.sol";
...
contract OmakaseCore is KeeperCompatibleInterface {
    ...
    function generatePolicyId(uint256 policyNo)
        external
        view
        returns (bytes32)
    {
        uint256 curr = block.timestamp;
        string memory strPolicyId = policyNo.toString();
    ...
    function generateUserHash(
        ...
        Gender gender,
        string calldata birthDate // yyyy-mm-dd
    ...
    function generateBenefitInitializeData(uint256 maximumAmount)
        external
        pure
        returns (bytes memory)
    {
        return LibBenefit.generateInitializeData(maximumAmount);
    }
    ...
    function getBenefitTripCancellation(bytes32 productId)
        external
        view
        returns (Benefit memory)
    {
        return s.products[productId].benefitTripCancellation;
    }
    ...
    function setPolicyFlights(
        ...
        for (uint256 i = 0; i < flightData.length; i++) {
            Flight storage flight = _createFlightIfNotExists(flightData[i]);
            require(
    ...
    function createProduct(
        bytes32 productId,
        BenefitName[] calldata benefits,
        bytes[] calldata benefitsData
    ) external onlyOracle {
        ...
        product.initialize(benefits, benefitsData);

        emit NewProduct(productId, benefits, msg.sender);
    }
    ...
    function _createFlightIfNotExists(FlightArgs calldata data)
        private
        returns (Flight storage flight)
    {
        require(data.id != 0, Errors.OC_INVALID_FLIGHT_ID);

        flight = s.flights[data.id];
        if (flight.isEmpty()) {
            flight.initialize(data.departureTime, data.arrivalTime);

            s.flightQueue.addFlightToQueue(data.id, data.arrivalTime);

            s.totalFlights++;

            emit NewFlight(data.id, msg.sender);
        }
    }
    ...
    function _evaluateFlightDeparture(
        ...
        for (uint256 i = 0; i < flight.policies.length; i++) {
            ...
            BenefitFlightDelay storage benefit = s
                .products[s.policies[policyId].productId]
                .benefitFlightDelay;

            if (!benefit.eligibleForClaim(hourDifference)) {
                continue;
            }

            uint256 indemnity = benefit.calculateIndemnity(hourDifference);
...
library LibBenefit {
    function benefitStorage(bytes32 position)
        internal
        pure
        returns (Benefit storage ds)
    {
        assembly {
            ds.slot := position
        }
    ...
    function initialize(Benefit storage self, bytes memory data) internal {
        uint256 maximumAmount = abi.decode(data, (uint256));
        self.maximumAmount = uint64(maximumAmount);
    }

    function isEmpty(Benefit storage self) internal view returns (bool) {
...
library LibBenefitFlightDelay {
    function benefitStorage(bytes32 position)
        internal
        pure
        returns (BenefitFlightDelay storage ds)
    {
        assembly {
            ds.slot := position
        }
    ...
    function eligibleForClaim(
        ...
        if (self.claimedAmount >= self.maximumAmount) {
            return false;
        }
    ...
    function calculateIndemnity(
        ...
        if (indemnity >= self.maximumAmount) {
            indemnity = self.maximumAmount;
        }
    }
}
...
struct Product {
    BenefitName[] benefits;
    Benefit benefitPersonalAccident;
    BenefitFlightDelay benefitFlightDelay;
    Benefit benefitBaggageDelay;
    Benefit benefitTripCancellation;
    Benefit benefitFlightPostponement;
}

library LibProduct {
    using LibBenefit for Benefit;
    using LibBenefitFlightDelay for BenefitFlightDelay;
    ...
    function initialize(
        ...
        for (uint256 i = 0; i < benefits.length; i++) {
            if (benefits[i] == BenefitName.PersonalAccident) {
                self.benefitPersonalAccident.initialize(benefitsData[i]);
            } else if (benefits[i] == BenefitName.FlightDelay) {
                self.benefitFlightDelay.initialize(benefitsData[i]);
            } else if (benefits[i] == BenefitName.BaggageDelay) {
                self.benefitBaggageDelay.initialize(benefitsData[i]);
            } else if (benefits[i] == BenefitName.TripCancellation) {
                self.benefitTripCancellation.initialize(benefitsData[i]);
            } else if (benefits[i] == BenefitName.FlightPostponement) {
                self.benefitFlightPostponement.initialize(benefitsData[i]);
            }
        }
    }
