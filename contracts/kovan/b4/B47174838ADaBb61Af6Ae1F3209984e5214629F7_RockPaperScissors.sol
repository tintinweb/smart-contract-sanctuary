...
contract RockPaperScissors {
    ...
    function placeGuess(bytes32 _saltedHashedGuess) payable public {
        require(msg.value > 0, "bet amount must be positive");
        if (placedNumber == 1) {
            require(msg.value == playerMapping[playerAddress[1]].betAmount, "bet amount of 2 players must be equal");
        }
        playerMapping[msg.sender].hashedGuess = _saltedHashedGuess;
        playerMapping[msg.sender].betAmount = msg.value;
    ...
    function payout() internal {
        ...
        uint256 winningAmount = playerMapping[playerAddress[1]].betAmount + playerMapping[playerAddress[2]].betAmount;
        bool success = false;
        if ( guessDifference == 1 || guessDifference == -2) {
            (success, ) = playerAddress[1].call{ value: winningAmount }("");
        } else if (guessDifference == -1 || guessDifference == 2 ) {
            (success, ) = playerAddress[2].call{ value: winningAmount }("");
        } else if (guessDifference == 0) {
            (success, ) = playerAddress[1].call{ value: playerMapping[playerAddress[1]].betAmount }("");
            (success, ) = playerAddress[2].call{ value: playerMapping[playerAddress[2]].betAmount }("");
        }
