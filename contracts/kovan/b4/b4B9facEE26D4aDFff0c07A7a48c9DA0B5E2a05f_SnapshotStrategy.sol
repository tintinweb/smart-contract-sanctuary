...
// File: SnapshotStrategy.sol

contract SnapshotStrategy {
    address public convController;
    constructor(address _convController) public {
    ...
    function balanceOfDToken(address _token, address _account) public returns (uint256 _balance) {
        ...
        address _dtoken = IConvController(convController).dtokens(_token);
        if (address(_dtoken) != address(0)) {
            _balance += IERC20(_dtoken).balanceOf(_account);
        }
    }
}