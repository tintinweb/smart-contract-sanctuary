...
contract TokenFarm is ChainlinkClient, Ownable {
    ...
    function add(address token/*, address priceFeed*/) public onlyOwner {
    ...
    function unstake(address token, uint256 _amount) public {
        require(_amount > 0, "amount cannot be 0");
        require(support(token) , "Token currently isn't allowed");
        uint256 balance = _balances[token][msg.sender];
        if (_amount > balance) _amount = balance;
        IERC20(token).transfer(msg.sender, _amount);
        _balances[token][msg.sender] -= _amount;
        emit Unstaked(token, msg.sender, _amount);
    }
}
