...
contract ExchangeApi is Ownable, ChainlinkClient, VRFConsumerBase {
    using Chainlink for Chainlink.Request;  

	address private LINK_TOKEN = 0xa36085F69e2889c224210F603D836748e7dC0088;
	address private ORACLE = 0xc57B33452b4F7BB189bB5AfaE9cc4aBa1f7a4FD8; //kovan;
    bytes32 private JOB_ID = "d5270d1c311941d0b08bead21fea7747";
    bytes32 internal keyHash = 0x6c3699283bda56ad74f6b855546325b68d482e983852a7a82979cc4807b641f4;

	uint private fee = 10 ** 17;
	...
	event OrderCreated(bytes32 requestIdPrice, bytes32 requestIdRandom);
	event TransactionCreated(address sender, uint256 fullAmount);
	event FillRandomness(uint256 random, bytes32 requestId);

	constructor(address _tokenContractAddress) VRFConsumerBase(
	...
	*/
	function orderTokens() payable public {
		require(IERC20(LINK_TOKEN).balanceOf(address(this)) >= fee*2, "Not enought LINK tokens");

        Chainlink.Request memory request = buildChainlinkRequest(JOB_ID, address(this), this.fulfillPrice.selector);

        // Set the URL to perform the GET request on
        request.add("get", "https://api.coinstats.app/public/v1/coins/ethereum?currency=USD");
        request.add("path", "coin.price");
        request.addInt("times", 10**18);

        
		bytes32 requestIdRandom = requestRandomness(keyHash, fee);
        bytes32 requestIdPrice = sendChainlinkRequestTo(ORACLE, request, fee);
        Order memory _transaction;
		...
		orderBook.push(_transaction);

		emit OrderCreated(requestIdPrice, requestIdRandom);
	}
	/**
     * Get order from Order book by requestId
     */
	function getOrderIndexByRequestId(bytes32 requestId) public view returns(uint) {
		for (uint i = 0; i < orderBook.length; i++) {
			if(orderBook[i].requestIdRandom == requestId || orderBook[i].requestIdPrice == requestId) {
				return i;
			}
		}
    ...
    /**
     * Receive the response for price request
     */ 
    function fulfillPrice(bytes32 _requestId, uint256 _price) public recordChainlinkFulfillment(_requestId)
    {
		//todo: add checkups
		uint orderIndex = getOrderIndexByRequestId(_requestId);
		orderBook[orderIndex].price = _price;

		if (orderBook[orderIndex].random == 0) return;
		
		Order memory _currentOrder = orderBook[orderIndex];
		createTransaction(
			_currentOrder.from,
			_currentOrder.amount,
			_currentOrder.price,
			_currentOrder.random
		);
    }
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        ...
        uint256 _random = 5;//(randomness % 26) + 5;
		uint orderIndex = 0;//getOrderIndexByRequestId(requestId);
		orderBook[orderIndex].random = _random;

		if (orderBook[orderIndex].price == 0) return;
	...
	function createTransaction(address sender, uint256 _amount, uint256 _price, uint256 _random) private
    {
        uint256 fullAmount = 0;
  
        fullAmount += (_amount * uint256(_price) * _random) / (10**19);

        require(fullAmount > tokenFDRT.balanceOf(address(this)), "Sorry, there is not enough tokens to buy!");

        tokenFDRT.transfer(sender, fullAmount);
		
		emit TransactionCreated(sender, fullAmount);
    }
