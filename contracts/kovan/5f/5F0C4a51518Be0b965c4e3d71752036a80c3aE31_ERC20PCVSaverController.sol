...
import "./IPCVSaverController.sol";
import "../refs/CoreRef.sol";
import "../saver/ERC20PCVSaver.sol";

/// @title a PCVSaverController implementation for ERC20
/// @author Ring Protocol
contract ERC20PCVSaverController is IPCVSaverController, CoreRef {
    using SafeMathCopy for uint128;

    /// @notice returns the linked pcv deposit contract
    IPCVDeposit public override pcvDeposit;

    /// @notice returns the linked pcv saver contract
    address public override pcvSaver;

    /// @notice ERC20PCVSaverController constructor
    /// @param _core Ring Core for reference
    /// @param _pcvDeposit PCV Deposit to control
    /// @param _pcvSaver PCV Saver to withdraw
    constructor(
        address _core,
        address _pcvDeposit,
        address _pcvSaver
    ...
    /// @notice burn Saver token and withdraw token to saver destination
    function withdraw(uint256 amountIn, address to)
        external
        returns (uint256 amountOut)
    {
        // safeTransferFrom(address token, address from, address to, uint value)
        TransferHelper.safeTransferFrom(address(pcvSaver), msg.sender, address(this), amountIn);

        ERC20PCVSaver erc20PCVSaver = ERC20PCVSaver(pcvSaver);
        uint256 pcvSaverBalance = erc20PCVSaver.balanceOf(address(this));
        uint256 value = pcvDeposit.totalLiquidity().mul(pcvSaverBalance) / erc20PCVSaver.totalSupply();
        require(value > 0, "ERC20PCVSaverController: No liquidity to withdraw");

        erc20PCVSaver.burn(pcvSaverBalance);
        emit Burn(msg.sender, pcvSaverBalance);

        pcvDeposit.withdraw(address(this), value);
        address tokenAddress = erc20PCVSaver.tokenAddress();
        uint256 withdrawAmount = IERC20(tokenAddress).balanceOf(address(this));
        amountOut = Math.min(pcvSaverBalance, withdrawAmount);
        TransferHelper.safeTransfer(tokenAddress, to, amountOut);
        TransferHelper.safeTransfer(tokenAddress, address(pcvDeposit), IERC20(tokenAddress).balanceOf(address(this)));
    ...
    /// @notice sets the target PCV Saver address
    function setPCVSaver(address _pcvSaver) external override onlyGovernor {
        pcvSaver = _pcvSaver;
        emit PCVSaverUpdate(_pcvSaver);
    }
...
import "../saver/IPCVSaver.sol";

/// @title a Uniswap PCV Controller interface
/// @author Ring Protocol
interface IPCVSaverController {
    // ----------- Events -----------
    ...
    event PCVSaverUpdate(address indexed _pcvSaver);
    ...
    function setPCVSaver(address _pcvSaver) external;
    ...
    function pcvSaver() external view returns (address);
}
