...
contract SperaxToken is ERC20, Pausable, MintPausable, Ownable {
    ...
    ISPAMintAdapter public SPAMintAdapter;
     ...
     */
    constructor(string memory _name, string memory _symbol, uint8 _decimals, uint256 _initialSupply, address SPAMintAdapterAddr) public {
        name = _name;
        symbol = _symbol;
        decimals = _decimals;
        SPAMintAdapter = ISPAMintAdapter(SPAMintAdapterAddr);
     ...
     */
    function mintForUSDs(address account, uint256 amount) whenMintNotPaused external {
        require(SPAMintAdapter.isAuthorized(_msgSender()) == true, "can only mint SPA from authorized address (e.g. USDs redeeming/burning)");
