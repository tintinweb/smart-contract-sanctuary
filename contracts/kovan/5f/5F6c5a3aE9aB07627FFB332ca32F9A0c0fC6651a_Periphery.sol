...
contract Periphery is IPeriphery, ReentrancyGuard {
    ...
    function newLimitLiquidity(
        ...
        vault.setRangeTicks(_limitLower, _limitUpper);
    }
        ...
        )
    {
        ...
        (liquidity, , , amount0, amount1) = vault.position(
            vault.baseLower(),
            vault.baseUpper()
        );
    }

    function getRangeAmounts(address _vault)
        public
        view
        override
        returns (
    ...
    function getRangeTicks(address _vault)
        external
        override
        returns (int24, int24)
    {
    ...
    //         "Periphery : onlyStrategy :: tx sender needs to be a valid strategy manager"
    //     );
    //     _;
    // }
    ...
    function _getVault(address _manager) internal view returns (IVault vault) {
        ...
        require(
            _vault != address(0),
            "Periphery : _getVault :: PANIC! SM has no valid vault"
        );
...
interface IVault is IERC20 {
    ...
    function setBaseTicks(int24 baseLower, int24 baseUpper) external;

    function setRangeTicks(int24 limitLower, int24 limitUpper) external;
...
interface IPeriphery {
    ...
    function getRangeAmounts(address vault)
        external
        returns (
            uint128 liquidity,
            uint256 amount0,
    ...
    function getBaseTicks(address vault) external returns (int24 lowerTick, int24 upperTick);
    function getRangeTicks(address vault) external returns (int24 lowerTick, int24 upperTick);
