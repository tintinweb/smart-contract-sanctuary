...
contract Borrower is Ownable, ReentrancyGuard {
	...
	****************************************/
		...
		onlyAmountGreaterThanZero(amount_)
	{
		...
		if(_user.debt != 0){
			uint256 _price = priceOracle.getAssetPrice(address(reserve));
			require(_price != 0, "oracle price error");
			require(amount_ <= _getAvailableWithdraw(_user.balance, _user.debt, _price, lender.borrowCoefficient()),
		...
		onlyAmountGreaterThanZero(amount_)
	{
		...
		uint256 _price = priceOracle.getAssetPrice(address(reserve));
		require(_price != 0, "oracle price error");
		uint256 _availableBorrow = _getAvailableBorrow(_user.balance, _user.debt, _price, lender.borrowCoefficient());
	...
	function depositAndBorrow(uint256 amountDeposit_, uint256 amountBorrow_, bool unwrap_)
		external
	{
		deposit(amountDeposit_);
		borrow(amountBorrow_, unwrap_);
	}
	...
	function repayAndWithdraw(uint256 amountRepay_, uint256 amontWithdraw_, bool wrap_)
		...
		payable
	{
		repay(amountRepay_, wrap_);
		withdraw(amontWithdraw_);
	}
		...
		nonReentrant
	{
		...
		uint256 _liquidationAmount = _getLiquidationAmount(_user.balance, _user.debt, _price);
		lendingPool.withdraw(address(reserve), _liquidationAmount, msg.sender);
		_user.balance -= _liquidationAmount; 
		_user.debt = 0;
		totalBalance -= _liquidationAmount;
		emit Liquidate(borrower_, _liquidationAmount, msg.sender);
	}	
	...
	****************************************/
		...
		returns(uint256 availableBorrow_)
	{
		...
		if(_user.balance + amountDeposit_ != 0){
			uint256 _price = priceOracle.getAssetPrice(address(reserve));
			require(_price != 0, "oracle price error");	
			availableBorrow_ = _getAvailableBorrow(_user.balance + amountDeposit_, _user.debt, _price, lender.borrowCoefficient());
		}
	}
		...
		returns(uint256 availableWithdraw_)
	{
		...
		if(_user.debt != 0){
			uint256 _price = priceOracle.getAssetPrice(address(reserve));
			require(_price != 0, "oracle price error");	
			availableWithdraw_ =  _getAvailableWithdraw(_user.balance, _user.debt - amountRepay_, _price, lender.borrowCoefficient());
		}else{
			availableWithdraw_ =  _user.balance;
		}
	}
