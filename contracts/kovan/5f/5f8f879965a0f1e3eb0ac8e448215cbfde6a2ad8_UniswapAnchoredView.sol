...
contract UniswapAnchoredView is UniswapConfig {
     ...
     */
    function fetchEthPrice() private returns (uint) {
    ...
    function fetchAnchorPrice(string memory symbol, TokenConfig memory config, uint conversionFactor) internal virtual returns (uint) {
        ...
        uint256 sqrtPriceX96 = mul(TickMath.getSqrtRatioAtTick(tick), 10**18).div(2**96);
        uint rawUniswapPriceMantissa = mul(sqrtPriceX96, sqrtPriceX96).div(10**18);
        ...
        if (config.isUniswapReversed) {
            ...
            anchorPrice = (unscaledPriceMantissa / config.baseUnit) / (10**18 / config.baseUnit);
        } else {
