...
contract SquidWarsPair is Ownable, Pausable, ISquidWarsRandomCallback {
        ...
        returns (bool)
    {
        ...
        if (curEndpoint == 0) {
            ...
            finishtime = blocktime_ + level.duration;
            level.lowerBound = 0;
            level.upperBound = 0;
        }
        //支持预售票数
        ...
        returns (bool)
    {
        ...
        //尝试开启下一关
        curEndpoint++;
        GameLevels storage nlevel = _gameLevels[curEndpoint];
    ...
    function getReward() public {
        ...
        balance = balance - (tip + bonus);
        if(!firstGetReward){
            (uint256 bonusFee_, uint256 tipFee_, uint256 amount_) = SquidWarsLib.getFee(balance, _bonusFee, _tipFee);
            tip += tipFee_;
            bonus += bonusFee_;
            firstGetReward = true;
            balance = amount_;
        ...
        balance = balance * usersInfo.tickets / numOfTikects_;
        numOfTikects_ -= usersInfo.tickets;
        (uint256 fee_, uint256 ramount_) = SquidWarsLib.getWithdrawFee( balance, finishtime );
        tip += fee_;
    ...
    function withdrawTip(address tipAddress_) public onlyOwner{
        uint tip_ = tip;
        tip = 0;
        _squidWars.transfer(tipAddress_, tip_);
    }
}
