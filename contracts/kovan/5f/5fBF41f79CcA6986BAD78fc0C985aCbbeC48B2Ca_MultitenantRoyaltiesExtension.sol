...
// File: LiveArt/Extension/MultitenantRoyaltiesExtension.sol
...
contract MultitenantRoyaltiesExtension is
    IMultitenantRoyaltiesExtension, OwnableUpgradeable {

    IERC721MultitenantStorage _erc721MultitenantStorage;

    // 100% == 100 * 100 BPS
    uint256 private constant maxBPS = 10000;

    mapping(
        string => mapping(uint256 => Royalties.RoyaltyReceiver[])
    ) private _royaltyReceivers;

    mapping(
        string => mapping(uint256 => uint256)
    ) private _tokenBuyoutPrice;

    mapping(
        string => mapping(uint256 => Royalties.BuyOutReceiver[])
    ) private _tokenBuyoutReceivers;

    /**
    * @dev record all token payments received by this extension
    */
    mapping(
        string => mapping(uint256 => uint256[])
    ) private _tokenPayments;

    mapping(
        string => mapping(uint256 => uint256)
    ) private _highestPrice;
    ...
    function registerToken(
        ...
        for (uint256 i = 0; i < royaltyReceivers.length; i++) {
            _royaltyReceivers[app][tokenId].push(royaltyReceivers[i]);
        }
    }
    function registerToken(
        string calldata app,
    ...
    function afterTokenTransfer(
        ...
        _tokenTransfers[app][tokenId]++;
    }

    function getBuyOutPrice(
        string calldata app,
        uint256 tokenId
    ) external override view onlyAdmin returns (uint256) {
        return _tokenBuyoutPrice[app][tokenId];
    }
    function buyOutToken(
        string calldata app,
        uint256 tokenId
        ...
        require(
            msg.value == _tokenBuyoutPrice[app][tokenId],
            "incorrect buyOut price"
        );

        Royalties.BuyOutReceiver[] memory buyOutReceivers =
            _tokenBuyoutReceivers[app][tokenId];
        for (uint i = 0; i < buyOutReceivers.length; i++) {
            uint256 payAmount = _calculatePercentage(
                msg.value,
                buyOutReceivers[i].percentage
            );

            AddressUpgradeable.sendValue(
                buyOutReceivers[i].wallet,
                payAmount
            );
        }
    }
    ...
    function payForRightsTransfer(
        ...
        Royalties.RoyaltyReceiver[] memory royaltyReceivers =
            _royaltyReceivers[app][tokenId];

        uint256 currentPaymentFunds = msg.value;
        uint256 CAPPS = 0;
        uint256 highestPrice = _highestPrice[app][tokenId];
        if (currentPaymentFunds > highestPrice && highestPrice > 0) {
            CAPPS = currentPaymentFunds - highestPrice;
        }
        uint256 released;
        uint256[] storage tokenPayments = _tokenPayments[app][tokenId];
        ...
        for (uint256 i = 0; i < royaltyReceivers.length; i++) {
            ...
            if (CAPPS > 0) {
                CAPPSShare = _calculatePercentage(
                    CAPPS,
                    royaltyReceivers[i].CAPPS
                );
            }

            uint256 payment;
            if (tokenPayments.length > 0) /* resale */ {
                payment = _calculatePayment(
                    currentPaymentFunds,
                    royaltyReceivers[i].resalePercentage,
                    royaltyReceivers[i].fixedCut,
                    CAPPSShare
                );
            } else /* first sale */ {
                payment = _calculatePayment(
                    currentPaymentFunds,
                    royaltyReceivers[i].percentage,
                    royaltyReceivers[i].fixedCut,
                    CAPPSShare
                );
            }
        ...
        if (currentPaymentFunds > highestPrice) {
            _highestPrice[app][tokenId] = currentPaymentFunds;
        }
        if (released < currentPaymentFunds) {
            AddressUpgradeable.sendValue(
                from,
                currentPaymentFunds - released
            );
        }
        tokenPayments.push(currentPaymentFunds);
    }

    function _saveBuyoutData(
        string calldata app,
        uint256 tokenId,
        uint256 buyOutPrice,
        Royalties.BuyOutReceiver[] memory buyOutReceivers
    ) internal {
        _tokenBuyoutPrice[app][tokenId] = buyOutPrice;

        uint256 totalBuyOut;
        for (uint32 i = 0; i < buyOutReceivers.length; i++) {
            totalBuyOut += buyOutReceivers[i].percentage;
            _tokenBuyoutReceivers[app][tokenId].push(
                buyOutReceivers[i]
            );
        }
        ...
        require(
            totalBuyOut == maxBPS,
            "combined buy out percentage should be exactly 100%"
        );
    }
    ...
    function _validateRoyaltyReceivers(
        ...
        (uint256 totalFirstSaleBPS, uint256 totalResaleBPS) =
            _calculateTotalRoyalties(royaltyReceivers);
        require(
            totalFirstSaleBPS <= maxBPS,
            "first sale percentage can't be more than 100%"
        );
        require(
            totalResaleBPS <= maxBPS,
            "resale percentage can't be more than 100%"
        );
    }
    ...
    function _calculateTotalRoyalties(
        ...
        uint256 totalFirstSaleBPS,
        uint256 totalResaleBPS
    ) {
        uint256 _totalFirstSaleBPS;
        uint256 _totalResaleBPS;
        for (uint256 i = 0; i < royaltyReceivers.length; i++) {
            _totalFirstSaleBPS += royaltyReceivers[i].percentage;
            _totalResaleBPS += royaltyReceivers[i].resalePercentage;
        }

        return (_totalFirstSaleBPS, _totalResaleBPS);
    }

    function getTokenRoyalties(
        string calldata app,
        uint256 tokenId
    ) external view override onlyAdmin returns (
        Royalties.RoyaltyReceiver[] memory
    ) {
        return _royaltyReceivers[app][tokenId];
    }
    ...
    function getRoyalties(
        string calldata app,
        uint256 tokenId
    ) public view override onlyAdmin returns (
        address payable[] memory,
        uint256[] memory
    ) {
        return (
            _getRoyaltyReceivers(app, tokenId),
            _getRoyaltyBPS(app, tokenId)
        );
    }
    ...
    function royaltyInfo(
        ...
        Royalties.RoyaltyReceiver[] memory royaltyReceivers =
            _royaltyReceivers[app][tokenId];
        require(
            royaltyReceivers.length <= 1,
            "More than 1 royalty receiver"
        );
        if (royaltyReceivers.length == 0) {
            return (address(this), 0);
        }

        if(_tokenTransfers[app][tokenId] == 0) {
            return (
                royaltyReceivers[0].wallet,
                _calculatePercentage(
                    salePrice,
                    royaltyReceivers[0].percentage
                )
            );
        }
        return (
            ...
            _calculatePercentage(
                salePrice,
                royaltyReceivers[0].resalePercentage
            )
        );
    }
    ...
    function getFees(
        ...
        return getRoyalties(app, tokenId);
    }
    ...
    function getFeeRecipients(
        ...
        return _getRoyaltyReceivers(app, tokenId);
    }
    function getFeeBps(
        string calldata app,
        uint256 tokenId
    ) external view override onlyAdmin returns (uint[] memory) {
        return _getRoyaltyBPS(app, tokenId);
    }
    ...
    */
    function _getRoyaltyBPS(
        ...
        Royalties.RoyaltyReceiver[] memory royaltyReceivers =
            _royaltyReceivers[app][tokenId];
        uint256[] memory royaltyBPS = new uint256[](
            royaltyReceivers.length
        );

        for (uint256 i = 0; i < royaltyReceivers.length; i++) {
            if (_tokenTransfers[app][tokenId] == 0) {
                royaltyBPS[i] = royaltyReceivers[i].percentage;
            } else {
                royaltyBPS[i] = royaltyReceivers[i].resalePercentage;
            }
        }
    ...
    function _calculatePayment(
        ...
        return _calculatePercentage(totalReceived, percentage) +
            fixedCut + CAPPS;
    }
