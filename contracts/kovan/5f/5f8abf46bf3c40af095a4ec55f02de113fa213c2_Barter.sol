...
contract Barter {
    ...
    uint256 private bartersNumber = 0;
    ...
    function createBarter(uint256 _myTokenId) external returns (uint256) {
        ...
        bartersNumber++;
        require(barters[msg.sender][bartersNumber] == 0);

        barters[msg.sender][bartersNumber] = _myTokenId;

        return bartersNumber;
    }

    function createOffer(uint256 _fromTokenId, uint256 _barterNumber) external {
        require(isApprovedForAll(msg.sender));
        require(balanceOf(msg.sender, _fromTokenId) > 0);
        offers[msg.sender][_barterNumber] = _fromTokenId;
    }
    ...
    function removeOffer(uint256 _barterNumber) external {
        offers[msg.sender][_barterNumber] = 0;
    }

    function acceptOffer(uint256 _myTokenId, address _from, uint256 _fromTokenId, uint256 _barterNumber) external {
        require(isApprovedForAll(_from));
        require(isApprovedForAll(msg.sender));
        require(balanceOf(_from, _fromTokenId) > 0);
        require(balanceOf(msg.sender, _myTokenId) > 0);

        require(barters[msg.sender][_barterNumber] == _myTokenId);
        require(offers[_from][_barterNumber] == _fromTokenId);

        barters[msg.sender][_barterNumber] = 0;
        offers[_from][_barterNumber] = 0;
