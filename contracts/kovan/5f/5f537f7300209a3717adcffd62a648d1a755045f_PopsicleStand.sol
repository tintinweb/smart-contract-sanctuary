...
contract PopsicleStand is Ownable {
    ...
    function pendingIce(address _user)
        external
        view
        returns (uint256 pendingIceReward, uint256 pendingRewardToken)
    {
        ...
        pendingIceReward = user.amount.mul(accIcePerShareTemp).div(1e12).sub(user.rewardDebtIce).add(user.remainingIceTokenReward);
