...
contract PharoStakePool is Context, AccessControl, Ownable, ReentrancyGuard {
    ...
    uint256 public PHRO_PER_BLOCK = 100 ether;
    ...
    struct StakerInfo {
        ...
        // pending reward = (staker.amount * pool.accumulatedgPharoPerShare) - staker.rewardDebt
        // Whenever a user deposits or withdraws tokens to a pool. Here's what happens:
        //   1. The pool's `accumulatedgPharoPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 accumulatedgPharoPerShare; // last block that PHRO dist occurred
    ...
    */
        ...
        returns(uint256)
    {
        ...
        return pool.accumulatedgPharoPerShare.div(100);
    }
    ...
    function poolDetails(uint256 poolId) external view returns(uint[3] memory) {
        ...
        return [
            ...
            pool.accumulatedgPharoPerShare,
        ...
        public
    {
        ...
        uint256 contractBeforeBalance = gphroToken.balanceOf(address(this));

        PoolInfo storage pool = poolInfo[_poolId];
        StakerInfo storage staker = stakerInfo[_poolId][msg.sender];
        updatePool(_poolId);
        if(staker.amount > 0) {
            uint256 pending = staker.amount.mul(pool.accumulatedgPharoPerShare).div(1e12).sub(staker.rewardDebt);
            if(pending > 0) {
        ...
        staker.rewardDebt = staker.amount.mul(pool.accumulatedgPharoPerShare).div(1e12);
        ...
        onlyOwner
    {
        ...
        phroToken.mintTokens(burnAddress, burnAmount);
        uint256 rewardAfterBurn = pharoReward - burnAmount;
        phroToken.mintTokens(address(this), rewardAfterBurn);

        // add the reward amount when its paid
        bonusSums[msg.sender] = bonusSums[msg.sender] + pharoReward;

        pool.accumulatedgPharoPerShare = pool.accumulatedgPharoPerShare.add(pharoReward.mul(1e18).div(poolTokenSupply));
        pool.lastRewardBlock = block.number;
    }
        ...
        returns(uint256) 
    {
        ...
        uint256 accumulatedgPharoPerShare = pool.accumulatedgPharoPerShare;
        uint256 stakedSupply = pool.asset.balanceOf(address(this));
        if(stakedSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 pharoReward = multiplier.mul(PHRO_PER_BLOCK).mul(pool.allocationPoint).div(totalAllocationPoint);
            accumulatedgPharoPerShare = accumulatedgPharoPerShare.add(pharoReward.mul(1e12).div(stakedSupply));
        }

        return staker.amount.mul(accumulatedgPharoPerShare).div(1e12).sub(staker.rewardDebt);
    }
