...
contract PCOG is Context, InterfacePCOG, Ownable {
    ...
    function removeBurner(address account) external override onlyOwner {
        ...
        emit RemoveBurner(msg.sender, account);
    }

    function getAllBurners() external override view returns (address[] memory) {
        return burners;
    }

    function getIsBurner(address account) external override view returns (bool) {
    ...
    function burn(uint256 amount) external virtual override {
        require(msg.sender == owner() || isBurner[msg.sender], "PCOG: NOT_A_BURNER_OR_OWNER");
