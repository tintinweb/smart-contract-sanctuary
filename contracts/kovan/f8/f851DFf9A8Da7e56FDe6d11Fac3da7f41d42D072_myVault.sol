...
contract myVault {
	...
	address public daiAddress = 0x4F96Fe3b7A6Cf9725f59d353F723c1bDb64CA6Aa; //0x6B175474E89094C44Da98b954EedeAC495271d0F;
	address public wethAddress = 0xd0A1E359811322d97991E03f863a0C30C2cF029C; //0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
	...
	function rebalance() public {
		...
		uint usdBalancePercentage = (usdBalance / totalBalance) * 100;
		emit logThis('usdBalancePercentage', usdBalancePercentage);
		if (usdBalancePercentage < usdTargetPercentage) {
			uint amountToSellUSD = totalBalance / 100 * (usdTargetPercentage - usdBalancePercentage);
			emit logThis('amountToSellUSD', amountToSellUSD);
			sellWeth(amountToSellUSD);
		} else {
			uint usdBalance4 = daiToken.balanceOf(address(this));
			uint amountToBuyUSD =  totalBalance / 100 * (usdBalancePercentage - usdTargetPercentage);
			uint usdBalance5 = daiToken.balanceOf(address(this));
			emit logThis('amountToBuyUSD', amountToBuyUSD);
			buyWeth(amountToBuyUSD);
		}
	}

	function buyWeth(uint amountToBuyUSD) internal {
		uint256 deadline = block.timestamp + 15;
		uint24 fee = 3000;
		address recipient = address(this);
		uint256 amountIn = amountToBuyUSD * 10**18;
	...
	function sellWeth(uint amountToSellUSD) internal {
		uint256 deadline = block.timestamp + 15;
		uint24 fee = 3000;
		address recipient = address(this);
		uint256 amountOut = amountToSellUSD * 10**18;
		uint256 amountInMaximum = 1;
		uint160 sqrtPriceLimitX96 = 0;
