...
//    address public constant BAT_ADDRESS = 0x2d12186Fbb9f9a8C28B3FfdD4c42920f8539D738;
    ...
    function instantiateExchangeB(address token) internal{
        // get Exchange B Address
        address exchangeB_address = uniswapFactoryB.getExchange(token);
        ...
        returns (bool)
    {
        ...
        IERC20 bat = IERC20(buyToken);
        

        // Buying ETH at Exchange A
        require(
            dai.approve(address(exchangeA), amounts[0]),
...
//       require(
//           dai.transfer(msg.sender, profit.div(2)),
...
//       );
    ...
    function myFlashLoanCall(uint256 amount,address token) public {
        instantiateExchangeB(token);
        ...
        assets[0] = address(DAI_ADDRESS); // Kovan DAI
        ...
        amounts[0] = amount.mul(1000000000000000000);
        //amounts[1] = 1 ether;
        // amounts[2] = 1 ether;
        // amounts[3] = 1 ether;
        // amounts[4] = 1 ether;
    ...
    function withDraw() external onlyOwner{
        ...
        require(
           ...
           "Could not transfer back"
        );
    }

    event TokenToEthInputPrice(address exchange,address token,uint amount);

    function hasOpportunity(address token,uint amount) public returns(bool){
        address exchange1_address = uniswapFactoryA.getExchange(token);
        IUniswapExchange exchange1 = IUniswapExchange(exchange1_address);
        uint amount1=exchange1.getTokenToEthInputPrice(amount);
        require(
            amount1>0,
            "Amount1 should more than zero"
        );
        emit TokenToEthInputPrice(UNISWAP_FACTORY_A,token,amount1);

        address exchange2_address = uniswapFactoryB.getExchange(token);
        IUniswapExchange exchange2 = IUniswapExchange(exchange2_address);
        uint amount2=exchange2.getTokenToEthInputPrice(amount);
        require(
            amount2>0,
            "Amount2 should more than zero"
        );
        emit TokenToEthInputPrice(UNISWAP_FACTORY_B,token,amount2);

        require(
            amount2.mul(100).div(amount1)>=105,
