...
contract Agic is ConstantAddresses, ERC20, Ownable {
    ...
    //pledge + reward
    function ethBalanceOf(address owner) public view notZeroAddress(owner) returns (uint256 userBalanceOf, uint256 supply){
    ...
    function withdraw(uint256 agic) public {
        (uint256 userBalanceOf,) = ethBalanceOf(msg.sender);
        uint256 balance = userBalanceOf.mul(4);
        ...
        require(userPledgeEth >= subPledgeEth, "Agic: Not so much subPledgeEth");
    ...
    function withdraw2(uint256 eth) public {
        _safeApprove(WETH, _aaveProvider.getLendingPool(), eth);
        ILendingPool(_aaveProvider.getLendingPool()).withdraw(WETH, eth, address(this));
    }
    
    function withdraw3(uint256 eth) public {
         _WETH.withdraw(eth);
    }
     ...
     */
    function _rewardAmount(uint256 amount) public view returns (uint256 subPledge, uint256 reward, uint256 fee){
        uint256 totalBalanceOf = _aWETH.balanceOf(address(this));
        uint256 supply;
        {
