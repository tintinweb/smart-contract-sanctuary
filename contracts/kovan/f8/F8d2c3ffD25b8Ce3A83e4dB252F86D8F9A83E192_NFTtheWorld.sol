...
contract NFTtheWorld {
    uint256 percentageLimit = 5;

    uint256 public totalNumberOfDrops = 0;
    ...
    struct GeneralDropInformation {
        uint256 maxNumberOfNFTsToBuy;
        uint256 reservedNFTsCount;
        string[] availableNFTs;
    }
    ...
    // Dictionary of form <dropHash>: GeneralDropInformation
    mapping(uint256 => GeneralDropInformation) private dropInformation;
    ...
    function createDrop(
        ...
        uint256 dropHash = totalNumberOfDrops;
        ...
        GeneralDropInformation memory generalDropInformation;
        generalDropInformation.availableNFTs = _uris;
        // We need to let users buy 1 NFT instead of 5% if there are less than 20
        if (_uris.length < 20) {
            generalDropInformation.maxNumberOfNFTsToBuy = 1;
        } else {
            generalDropInformation.maxNumberOfNFTsToBuy =
                (_uris.length * percentageLimit) /
                100;
        }
        generalDropInformation.reservedNFTsCount = 0;
        dropInformation[dropHash] = generalDropInformation;
    }
    ...
    function joinDrop(uint256 _numberOfNFTs, uint256 _dropHash) public {
        require(
            dropInformation[_dropHash].reservedNFTsCount !=
                dropInformation[_dropHash].availableNFTs.length,
            "You cannot join the drop anymore."
        );
        require(
            _numberOfNFTs <= dropInformation[_dropHash].maxNumberOfNFTsToBuy,
        ...
        dropInformation[_dropHash].reservedNFTsCount += _numberOfNFTs;
    ...
    function drop(uint256 _dropHash) public {
        ...
        for (uint256 i = 0; i < joinedUsers[_dropHash].length; i++) {
            ...
            for (
                uint256 j = dropInformation[_dropHash].availableNFTs.length;
                j >
                dropInformation[_dropHash].availableNFTs.length -
                    nftReservations[joinedUsers[_dropHash][i]][_dropHash];
                j--
            ) {
                uint256 nftElement = getNFTIndex(
                    dropInformation[_dropHash].availableNFTs[j],
                    _dropHash
                );
                ...
                dropInformation[_dropHash].availableNFTs.pop();
            }
        }
    }
    ...
    function shuffle(uint256 _dropHash) internal {
        for (
            uint256 i = 0;
            i < dropInformation[_dropHash].availableNFTs.length;
            i++
        ) {
            uint256 n = i +
                (uint256(keccak256(abi.encodePacked(block.timestamp))) %
                    (dropInformation[_dropHash].availableNFTs.length - i));
            string memory temp = dropInformation[_dropHash].availableNFTs[n];
            dropInformation[_dropHash].availableNFTs[n] = dropInformation[
                _dropHash
            ].availableNFTs[i];
            dropInformation[_dropHash].availableNFTs[i] = temp;
        }
    }
    ...
    {
        require(
            msg.sender != _addressToRemove,
            "You can't remove yourself from the list"
        );
