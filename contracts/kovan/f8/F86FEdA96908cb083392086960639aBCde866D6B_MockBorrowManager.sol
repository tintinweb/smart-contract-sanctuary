...
import { Status, Debt, DebtMetadata, DebtInfo, IBorrowManager } from "../../interfaces/IBorrowManager.sol";
...
contract MockBorrowManager is IBorrowManager, AccessControlEnumerable, ReentrancyGuard, Pausable
{
  ...
  function getDebtInfo (bytes32 debtID) external view override returns (DebtInfo memory)
  {
  ...
  function borrow (Borrow calldata borrowData) external nonReentrant
  {
    _isNotPaused();


    Contracts memory coordinators = _ADDRESSES.coordinators();

    uint256 debtInUSD = IOracle(coordinators.oracle).convertToUSD(borrowData.token, borrowData.amount);

    uint256 debtInCollateralToken = IOracle(coordinators.oracle).convert(borrowData.token, borrowData.collateralToken, borrowData.amount);

    (address depositVault, address collateralVault) = ICoordinator(coordinators.coordinator).increaseTokenTotalBorrowedAndGetPairVault(borrowData.token, borrowData.collateralToken, borrowData.amount);

    require(_isValidBorrow(debtInUSD, borrowData.interestRate, borrowData.durationInSecs), "!valid terms");
    require(IERC20(borrowData.token).balanceOf(depositVault) >= borrowData.amount, "!enough liq");
    require(ICollateralizationManager(coordinators.collateralizationManager).isSufficientInitialCollateral(coordinators.tokenRegistry, borrowData.token, borrowData.collateralToken, debtInCollateralToken, borrowData.collateral), "!adequate collateral");


    bytes32 id = keccak256(abi.encodePacked(_totalDebtCount, block.timestamp, msg.sender, borrowData.token, borrowData.amount, borrowData.collateralToken, borrowData.collateral));
    ...
    ({
      ...
      amount: uint128(borrowData.amount),
      collateralToken: borrowData.collateralToken,
      collateral: uint128(borrowData.collateral),
      interestRate: uint16(borrowData.interestRate),
      durationInSecs: uint24(borrowData.durationInSecs),
      startTimestamp: uint32(block.timestamp)
    });


    bool isDiscounted = IStakingManager(coordinators.stakingManager).isDiscountedBorrower(msg.sender);


    IStakingManager(coordinators.stakingManager).increaseBorrowerExpectedStake(msg.sender, borrowData.amount, debtInUSD, block.timestamp + borrowData.durationInSecs);

    IRewardManager(coordinators.rewardManager).registerDebt(id, debtInUSD, borrowData.interestRate, isDiscounted);


    (uint256 debtFee, address burner) = IFeeManager(coordinators.feeManager).getBorrowFeeOnDebt(debtInCollateralToken, isDiscounted);


    IERC20(borrowData.collateralToken).safeTransferFrom(msg.sender, burner, debtFee);
    IERC20(borrowData.collateralToken).safeTransferFrom(msg.sender, collateralVault, borrowData.collateral);
    IERC20(borrowData.token).safeTransferFrom(depositVault, msg.sender, borrowData.amount);


    emit NewBorrow(id, msg.sender, borrowData.token, borrowData.amount);
  }
  ...
  function _isOverdue (Debt memory debt, DebtMetadata memory debtMetadata) private view returns (bool)
  {
    ...
    uint256 dueTimestamp = debtMetadata.extensionTimestamp > 0 ? (debtMetadata.extensionTimestamp + _MAX_EXT_DURATION) : (debt.startTimestamp + debt.durationInSecs);
  ...
  function extend (bytes32 debtID) external nonReentrant
  {
    ...
    require(_stakingMgr().isDiscountedBorrower(msg.sender), "!enough stake");
    ...
    require(IERC20(debt.token).balanceOf(asset.vaults.deposit) >= _calcPercentOf(debt.amount, 15000), "!adequate liq");
    ...
    uint256 debtFee = _calcPercentOf(debt.amount, 75);
  ...
  {
    Asset memory debtAsset = _getAsset(debt.token);
    ...
    IERC20(debt.token).safeTransferFrom(msg.sender, debtAsset.vaults.interest, interest);
    IERC20(debt.token).safeTransferFrom(msg.sender, debtAsset.vaults.deposit, debtRepaid);


    if (collateralToWithdraw > 0)
    {
      IERC20(debt.collateralToken).safeTransferFrom(_getAsset(debt.collateralToken).vaults.collateral, msg.sender, collateralToWithdraw);
    }
  ...
  function _canRepay (Debt memory debt, DebtMetadata memory debtMetadata) private view
  {
    ...
    require(block.timestamp > debt.startTimestamp + 5 minutes, "recent");
  }
  ...
  {
    ICollateralizationManager colMgr = _colMgr();

    // TODO: should be + debtToRepay *important*
    (, uint256 postRepaymentCRatio) = colMgr.isSufficientCollateral(debt.borrower, debt.token, debt.amount - debtToRepay, debt.collateralToken, debt.collateral);


    if (postRepaymentCRatio <= colMgr.getTokenRatio(debt.collateralToken).init)
    {
      return 0;
    }
  ...
  function repayPartial (bytes32 debtID, uint256 amount, bool withdrawProportionalCollateral) external nonReentrant
  {
    ...
    {
      collateralToWithdraw = _calcProportionallyWithdrawableCollateral(debt, amount);

      _debt[debtID].collateral = debt.collateral - uint128(collateralToWithdraw);
    }


    _repay(debt, debtID, amount, collateralToWithdraw);
  }
  ...
  function liquidate (bytes32 debtID) external nonReentrant
  {
    ...
    (address depositVault, address interestVault, address collateralVault) = ICoordinator(_ADDRESSES.coordinator()).getPairVaultAll(debt.token, debt.collateralToken);

    (uint256 defaultFee, address burner) = _feeMgr().getDefaultFee(debt.collateral);

    uint256 interest = _calcPercentOf(debt.amount, debt.interestRate);


    IERC20(debt.collateralToken).safeTransferFrom(collateralVault, burner, defaultFee);
    ...
    IERC20(debt.token).safeTransferFrom(msg.sender, depositVault, debt.amount);
    IERC20(debt.token).safeTransferFrom(msg.sender, interestVault, interest);
    IERC20(debt.collateralToken).safeTransferFrom(collateralVault, msg.sender, compensation);


    if ((debt.collateral - compensation) > 0)
    {
      // TODO: should be + compensation *important*
      IERC20(debt.collateralToken).safeTransferFrom(collateralVault, burner, debt.collateral - compensation);
    }
  ...
  function _getLiquidationDetails (Debt memory debt, uint256 amount) private view returns (uint256, uint256, uint256)
  {
    ...
    uint256 maxLiquidatableCollateral = oracle.convert(debt.token, debt.collateralToken, _calcPercentOf(amount, currentCRatio));

    uint256 maxLiquidatorCompensation = oracle.convert(debt.token, debt.collateralToken, _calcPercentOf(amount + interest, _MAX_LIQ_COMPENSATION));



    return (interest, maxLiquidatorCompensation, maxLiquidatableCollateral);
  }
  ...
  function liquidatePartial (bytes32 debtID, uint256 amount) external nonReentrant
  {
    ...
    {
      _debtMetadata[debtID] = DebtMetadata({ status: Status.Active, initialAmount: debt.amount, extensionTimestamp: 0, weightedAvgRepayTimestamp: 0 });
    }


    (address depositVault, address interestVault, address collateralVault) = ICoordinator(_ADDRESSES.coordinator()).getPairVaultAll(debt.token, debt.collateralToken);

    (uint256 interest, uint256 maxLiquidatorCompensation, uint256 maxLiquidatableCollateral) = _getLiquidationDetails(debt, amount);

    (uint256 defaultFee, address burner) = _feeMgr().getDefaultFee(maxLiquidatableCollateral);


    IERC20(debt.collateralToken).safeTransferFrom(collateralVault, burner, defaultFee);


    uint256 compensation = Math.min(maxLiquidatorCompensation, maxLiquidatableCollateral - defaultFee);
    ...
    IERC20(debt.token).safeTransferFrom(msg.sender, depositVault, amount);
    IERC20(debt.token).safeTransferFrom(msg.sender, interestVault, interest);
    IERC20(debt.collateralToken).safeTransferFrom(collateralVault, msg.sender, compensation);
  ...
  function forceLiquidate (bytes32 debtID) external
  {
    ...
    IERC20(debt.collateralToken).safeTransferFrom(_getAsset(debt.collateralToken).vaults.collateral, _feeMgr().getBurner(), debt.collateral);
...
{
  // interestRate in basis point
  function registerDebt (bytes32 debtID, uint256 debtInUSD, uint256 interestRate, bool isDiscountedBorrower) external;
}
