...
contract InvestmentStrategy is IStrategy, Ownable {

    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    uint256 private constant FULL_ALLOC = 100000; // 100%

    /// @notice when investment was redeemed
    event InvestmentRedeemed(address indexed from, uint256 amount, address indexed to);

    /// @notice when investment was allocated
    event InvestmentAllocated(address indexed to, uint256 amount);

    /// @notice Weighted allocations per strategy. The index of the array determines the allocation strategy.
    ...
    /// @notice proposed strategy addresses
    address payable[] public proposedStrategies;

    /// @notice executed strategy addresses
    ...
    constructor(uint256[] memory _allocations, address payable[] memory _strategies, address _registry) {

        require(_allocations.length == _strategies.length, "ALLOCATION_INVALID");

        proposedStrategies = _strategies;
    ...
    /// @dev only vault
    modifier onlyVault() {
        require(msg.sender == contractRegistry
            .requireAndGetAddress("HoldingVault"), "NOT_VAULT");
        _;
    }

    /// @notice Set a new allocation for the investment. Only owner.
    /// @param  newAllocation array of protocol allocation.
    /// @param  newStrategies array of updated strategies.
    function setAllocation(uint256[] memory newAllocation,
            address payable[] memory newStrategies) external onlyOwner {
        ...
        require(newAllocation.length == newStrategies.length, "NO_LENGTH_MATCH");

        proposedAllocation = newAllocation;
        proposedStrategies = newStrategies;
    }

    function redeem(uint256 _redeemAmount, uint256 _totalSupply, address _account)
    external override onlyVault returns (uint256 redeemed) {

        redeemed = _redeemInternal(_redeemAmount, _totalSupply, _account);
    }

    function rebalance() external override onlyVault {

        uint256 balance = balanceInStrategy();
        if (balance > 0) {
            _allocateFunds(proposedAllocation, proposedStrategies, balance);
    ...
    /// @notice Get the invested balance.
    /// @return total ETH invested balance.
    function investedBalance() external override view returns (uint256 total) {
        if (executedAllocation.length == 0) {
            return balanceInStrategy();
        }
        ...
        for (uint256 i = 0; i < executedAllocation.length; i++) {
            ...
            total = total.add(IStrategy(strategy).investedBalance());
        }

        // add strategy balance
        total = total.add(balanceInStrategy());
    }
    ...
    /// @param  _account address to send redeemed to.
    ...
    private returns (uint256 redeemed) {
        ...
        for (uint256 i = 0; i < executedAllocation.length; i++) {
            strategy = strategies[i];
            require(strategy != address(0), "NO_STRATEGY");
            uint256 strategyBalance = IStrategy(strategy).investedBalance();
            if (strategyBalance > 0) {
                uint256 _strategyRedeemed = IStrategy(strategy).redeem(
                    _amount, _totalSupply, _account
                );
                redeemed = redeemed.add(_strategyRedeemed);
                emit InvestmentRedeemed(strategy, _strategyRedeemed, _account);
            }
        }
    }
    ...
    function _allocateFunds(uint256[] memory _allocations, address payable[] memory _strategies, uint256 _total) internal {
        ...
        for (uint256 i = 0; i < _allocations.length; i++) {
            ...
            if (currAmount != 0) {
                ...
                (bool sent,) = strategy.call{value: currAmount}("");
                require(sent, "Failed to send ETH");
                IStrategy(strategy).rebalance();
                emit InvestmentAllocated(strategy, currAmount);
            }
        }
    }
