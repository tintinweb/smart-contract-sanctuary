...
contract Vault is IVault, ReentrancyGuard, Ownable {
  ...
  constructor(IVaultConfig _config) public {
    config = _config;
  }

  /// @dev Request Funds from user through Vault
  function requestFunds(address user, address targetedToken, uint256 amount) external override nonReentrant {
  ...
  function work(
    ...
    uint256 back;
    {
      uint256 sendBEP20 = principalAmount;
      require(sendBEP20 <= SafeToken.myBalance(baseToken), "Vault::work:: insufficient funds in the vault");
      uint256 beforeBEP20 = SafeToken.myBalance(baseToken).sub(sendBEP20);
      SafeToken.safeTransfer(baseToken, strat, sendBEP20);
      IStrategy(strat).execute(msg.sender, ext);
      back = SafeToken.myBalance(baseToken).sub(beforeBEP20);
    }
        // 6. Return excess baseToken back.
    if (back > 0) {
      if (baseToken == config.wrappedNative()) {
        SafeToken.safeTransfer(baseToken, config.wNativeRelayer(), back);
        WNativeRelayer(uint160(config.wNativeRelayer())).withdraw(back);
        SafeToken.safeTransferETH(msg.sender, back);
      } else {
        SafeToken.safeTransfer(baseToken, msg.sender, back);
      }
    }
  }
