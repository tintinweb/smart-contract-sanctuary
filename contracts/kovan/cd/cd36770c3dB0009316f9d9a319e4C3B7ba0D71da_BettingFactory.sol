...
contract BettingContract is IBettingContract {
    ...
    uint public gapValidatePriceTime = 300;
    ...
    modifier decimalsLength(uint8 decimals) {
        require(
            ...
            "BETTING: Required range of decimals is (0;18]"
        );
        _;
    }
    ...
    function setBracketsPriceDecimals(uint8 decimals)
        external
        override
        onlyOwner
    ...
    function getPrice(uint8 _decimals) external payable onlyOpen {
        require(
            block.timestamp >= priceValidationTimestamp + gapValidatePriceTime,
            "BETTING: Only call price aftergapValidatePriceTime"
        );
    ...
    function close() external override onlyPriceClosed onlyOpen {
        ...
        uint256 reward = 0;
        if (winers.length > 0) {
            reward = getTotalToken() / winers.length;
            ...
            if (winers[0] != address(0x0)) {
                ...
                delete winers[0];
                IERC20(tokenAddress).safeTransfer(winner, getTotalToken());
            }
        } else {
            IERC20(tokenAddress).safeTransfer(owner, getTotalToken());
        }
        Address.sendValue(owner, (address(this).balance * 95) / 100);
    ...
    function _closeForce() private {
        if (listBuyer.length > 0) {
            uint256 reward = getTotalToken() / listBuyer.length;
            for (uint256 i = listBuyer.length - 1; i > 0; i--) {
                if (listBuyer[i] != address(0x0)) {
                    address buyer = listBuyer[i];
                    delete listBuyer[i];
                    IERC20(tokenAddress).safeTransfer(buyer, reward);
                    Address.sendValue(payable(buyer), ticketPrice);
                }
            }
            if (listBuyer[0] != address(0x0)) {
                address buyer = listBuyer[0];
                delete listBuyer[0];
                IERC20(tokenAddress).safeTransfer(buyer, getTotalToken());
                Address.sendValue(payable(buyer), address(this).balance);
            }
        } else {
            IERC20(tokenAddress).safeTransfer(owner, getTotalToken());
        }
        ...
        )
    {
        (uint256 _price, uint16 _decimals) = IPriceContract(priceContract)
            .getPrice(resultId);
        price = (_price * bracketsPriceDecimals) / _decimals;
        if (price == 0) {
...
interface IBettingContract {
    ...
    function setBracketsPriceDecimals(uint8 decimals) external;
