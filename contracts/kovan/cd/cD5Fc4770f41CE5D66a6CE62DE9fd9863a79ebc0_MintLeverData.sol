...
contract MintLeverData is IMintLeverData, Owner {
    ...
    event Position(address indexed mintLever, address indexed user, IOperateType.PositionOperateType operateType, address capitalToken, uint256 capitalAmount, uint256 amountA, uint256 amountB, address indexed borrowToken, uint256 borrowAmount, address[] bondTokens, uint256[] bondAmounts);
    ...
    function claimBondPTokenAmount(address user, address to, address token, uint256 amount) override external onlyOwner {
        ...
        _safeTransfer(address(config.getLoanPublics(token)), to, amount);
    }
    
    function claimAllBondPToken(address user, address to) override external onlyOwner returns (address[] memory, uint256[] memory) {
        address[] memory _bondTokens = bondTokens[user];
        uint256[] memory _amounts = new uint256[](_bondTokens.length);
        for (uint256 i = 0; i < _bondTokens.length; i++) {
            _amounts[i] = bondPTokenAmounts[user][_bondTokens[i]];
            bondPTokenAmounts[user][_bondTokens[i]] = 0;
            _safeTransfer(address(config.getLoanPublics(_bondTokens[i])), to, _amounts[i]);
        }
        return (_bondTokens, _amounts);
    }
    ...
    function addBond(address user, address token, uint256 amount) override external onlyOwner nonZeroAddress(token) nonZeroAmount(amount) {
        ...
        if (0 != _code) {
            revert(Strings.concat("publics:mint_lever_data_loan_mint_error_code:", Strings.toString(_code)));
        }
    ...
    function removeBond(address user, address token, uint256 amount) override external onlyOwner nonZeroAddress(token) {
        ...
        require(!config.getMintRouter().canClearing(msg.sender, user), "publics:risk_rate_too_low");
    ...
    function _extractBond(address user, uint256 deadLine) internal {
        ...
        for (uint256 i = 0; i < _bondTokens.length; i++) {
            ...
            if (0 < _amount) {
                ...
                _safeTransfer(_bondTokens[i], user, _amount);
            }
        }
    }
    ...
    function _repayV1(address user, uint256 amount) internal returns (uint256, uint256) {//剩余数量，还款数量
        ...
        if (0 != _code) {
            revert(Strings.concat("publics:mint_lever_data_loan_repay_error_code:", Strings.toString(_code)));
        }
    ...
    function _repayV2(address user, IOperateType.PositionOperateType operateType, address token, uint256 amount, uint256 deadLine) internal {
        ...
        _safeTransfer(borrowTokens[user], user, amount);
        _sendPosition(user, operateType);
    }
    ...
    function _redeem(address user, address token, uint256 amount) internal returns (uint256) {
        ...
        if (0 < amount) {
            ...
            if (0 != _code) {
                revert(Strings.concat("publics:mint_lever_data_loan_redeem_error_code:", Strings.toString(_code)));
            }
        }
        return amount;
    }
    ...
    function _redeemUnderlying(address user, address token, uint256 amount) internal returns (uint256) {
        if (0 < amount) {
            ...
            if (0 != _code) {
                revert(Strings.concat("publics:mint_lever_data_loan_redeem_underlying_error_code:", Strings.toString(_code)));
            }
    ...
    function _sendPosition(address user, IOperateType.PositionOperateType operateType) internal {
        address[] memory _bonds = bondTokens[user];
        uint256[] memory _amounts = new uint256[](_bonds.length);
        for (uint256 i = 0; i < _bonds.length; i++) {
            _amounts[i] = this.getBondTokenAmount(user, _bonds[i]);
        }
        emit Position(owner, user, operateType, capital, capitalAmounts[user], tokenAAmounts[user], tokenBAmounts[user], borrowTokens[user], this.getBorrowAmountV2(user), _bonds, _amounts);
    }
