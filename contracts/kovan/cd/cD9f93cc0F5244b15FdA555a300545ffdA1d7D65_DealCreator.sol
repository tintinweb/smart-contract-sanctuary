...
// import "./interfaces/IERC20.sol";
 ...
 contract Deal is Ownable, Pausable, ReentrancyGuard  { //Whitelist
    ...
    function pay(uint256 _amount, bytes memory _signature) payable external nonReentrant isActive whenNotPaused{ //onlyWhitelisted
        
        // bytes32 message = keccak256(abi.encodePacked(msg.sender, address(this)));
        //TODO: restore check signature
...
contract DealCreator is IDealCreator, Ownable {
    using SafeERC20 for IERC20;

    IVerifier public verifier;
    ILocker public locker;

    mapping(address => uint256) public lastParticipations;

    // mapping(address => DealInfo) public deals;

    mapping(address => bool) public deals;

    //TODO: contract size exceed
    // struct DealInfo {
    ...
    event DealCreated(address dealAddress,
        address paymentToken,
        uint256 tokenPrice,        
        address rewardToken,
        uint256 startTimestamp,
    ...
    function createDeal(
        ...
        uint256[] memory tiersAmount = new uint256[](allTiers.length); 
        
        for (uint256 i = 0; i < allTiers.length; i++) {
             tiersAmount[i] = allTiers[i].blpAmount;
        }
        ...
        require(_rewardToken.balanceOf(address(deal)) == _maxDistributedTokenAmount,  "Unsupported token");

        emit DealCreated(address(deal),
                        address(_paymentToken),
                        _tokenPrice,        
        ...
        // DealInfo storage info = deals[address(deal)];
        ...
        // info.minimumRaise = _minimumRaise;
        // info.maxDistributedTokenAmount = _maxDistributedTokenAmount;
        // info.allowRefund = _allowRefund;

        return address(deal);
    }

    function userParticipate(address user, uint256 tierIndex) external override{
        //Check that sender is our deal contract
        require(deals[msg.sender], "Declined");  
        TierLockups memory tier = allTiers[tierIndex];
        require(now - lastParticipations[user] > tier.timeLockups , "Lockup");  
        lastParticipations[user]= now;
    }

    function updateTier(uint256 index, uint256 blpAmount, uint256 timeLockups) external onlyOwner{
        require(index < allTiers.length , "Incorrect index");        
        TierLockups storage tier = allTiers[index];
        tier.blpAmount = blpAmount;
        tier.timeLockups = timeLockups;
    }

    function addTier(uint256 blpAmount, uint256 timeLockups) external onlyOwner{
        allTiers.push(TierLockups(blpAmount, timeLockups));
    }

    function getLockedBLP(address user) external override view returns (uint256)
    {
        return locker.getLockedBLP(user);
    }
}
