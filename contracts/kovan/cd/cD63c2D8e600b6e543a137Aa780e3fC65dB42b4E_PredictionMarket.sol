...
contract PredictionMarket {
    ...
    function placeBet(Side side) external payable {
        ...
        require(msg.sender != s_market, "market cannot place bet!");
        bets[side] += msg.value;
        betsPerGambler[msg.sender][side] += msg.value;
    }

    function Bets(Side side) public view returns (uint256) {
        return bets[side];
    }

    function MyBets(Side side) public view returns (uint256) {
        return betsPerGambler[msg.sender][side];
    }
