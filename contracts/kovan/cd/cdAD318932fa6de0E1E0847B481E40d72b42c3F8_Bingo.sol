...
contract Bingo is ERC721, VRFConsumerBase, KeeperCompatibleInterface {
    ...
    */
    ...
    function checkForWinners() internal {
        ...
        if(winIdx > 0) {
            for(uint256 i=0; i<winIdx; i++) {
                winners[_winners[i]] += prizePool / winIdx;
                winnerPool += prizePool / winIdx;
                emit BingoWinner(_gameIds.current(), ownerOf(_winners[i]), _winners[i]);
            }
            // Reset game
            prizePool = 0;
            _gameIds.increment();
