...
contract PropertyRegistry is Ownable {
    ...
    modifier onlyBlocksquareOrSpecialWallet {
        require(PropertyRegistryHelpers(_dataProxy).hasSystemAdminRights(msg.sender) || msg.sender == _specialWallet, "PropertiesFactory: You need to have system admin rights or special wallet!");
        _;
    }
    ...
    function setSpecialWallet(address specialWallet) public onlyOwner {
    ...
    function changePropertyValuation(address property, uint256 propertyValuation) public {
        require(PropertyRegistryHelpers(_dataProxy).hasSystemAdminRights(msg.sender) || msg.sender == _specialWallet ||
        PropertyRegistryHelpers(_dataProxy).getCPOfProperty(property) == msg.sender || PropertyRegistryHelpers(_dataProxy).isCPAdminOfProperty(msg.sender, property), "PropertyRegistry: You don't have permission");
    ...
    function changeTokenValuation(address property, uint256 tokenValuation) public {
        require(PropertyRegistryHelpers(_dataProxy).hasSystemAdminRights(msg.sender) || msg.sender == _specialWallet ||
        PropertyRegistryHelpers(_dataProxy).getCPOfProperty(property) == msg.sender || PropertyRegistryHelpers(_dataProxy).isCPAdminOfProperty(msg.sender, property) ||
            msg.sender == _exchange, "PropertyRegistry: You don't have permission");
        _basicInfos[property].tokenValuation = tokenValuation;
        emit PropertyValuationChange(property, tokenValuation);
    }

    function addPropertyInfo(address property, string memory propertyType, string memory kadastralMunicipality, string memory parcelNumber, string memory ID, uint64 buildingPart) public onlyBlocksquareOrSpecialWallet {
    ...
    function editPropertyInfo(address property, uint64 idProp, string memory propertyType, string memory kadastralMunicipality, string memory parcelNumber, string memory ID, uint64 buildingPart) public onlyBlocksquareOrSpecialWallet {
