...
@internal
def _testArrayRollup(_arrTest: uint256[5]):
    self.stateArray = _arrTest

@internal
def _testArrayRollupIntermediate(_arrTest: uint256[5]):
    self._testArrayRollup(_arrTest)

@external
def testArray(arrTest: uint256[5]):
    self._testArrayRollupIntermediate(arrTest)

@external
def checkDecode(arrTest: uint256[N_ALL_COINS]):
    self._testArrayRollup(arrTest)