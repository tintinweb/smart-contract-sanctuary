...
contract Treasury is DexCaller, Adminable {
    using SafeMath for uint;
    using SafeERC20 for IERC20;
    ...
    //
    uint public tranferedToAccount;
    ...
    function feeShareRatio() internal view returns (uint){
        return 100 - devFundRatio;

    }

    function setDevFundRatio(uint newRatio) external onlyAdmin {
        require(newRatio <= 100);
        devFundRatio = newRatio;
    }

    function exit() external {
        withdraw(stakeBalanceOf(msg.sender));
        getReward();
    }

    function stakeBalanceOf(address account) public view returns (uint) {
    ...
    function setDev(address newDev) external {
        require(dev == address(0), "Dev address can't change after set");
        dev = newDev;
    }

    function convertToSharingToken(address fromToken, uint amount, uint minBuyAmount) external {
        //sharing token increment
        uint sharingTokenIncrement = sharingToken.balanceOf(address(this)).add(tranferedToAccount).sub(totalToShared).sub(devFund);
        //newReward=rewardFromToken+sharingTokenIncrement
        uint newReward = flashSell(address(sharingToken), fromToken, amount, minBuyAmount).add(sharingTokenIncrement);
    ...
    function stake(uint amount) external updateReward(msg.sender) {
        ...
        lever.safeTransferFrom(msg.sender, address(this), amount);
    ...
    function withdraw(uint amount) public updateReward(msg.sender) {
        ...
        lever.safeTransfer(msg.sender, amount);
    ...
    function getReward() public updateReward(msg.sender) {
        ...
        if (reward > 0) {
            ...
            tranferedToAccount = tranferedToAccount.add(reward);
