...
interface IStakingRewards {
    ...
    function availableReward() external  returns (uint256);
...
contract StakingRewards is IStakingRewards, RewardsDistributionRecipient, ReentrancyGuard, NativeMetaTransaction {
    ...
    function availableReward() external override nonReentrant updateReward(_msgSender()) returns (uint256) {
        if (block.timestamp < periodFinish) 
            return 0;
        
        uint256 currentTime = block.timestamp;

        uint256 currentSplit = currentTime.sub(periodFinish).div(splitWindow);
        if (currentSplit > splits)
            currentSplit = splits;
        if (!hasClaimed[_msgSender()])
            return currentSplit.add(1).mul(claim).mul(earned(_msgSender())).div(100);
        return 
            currentSplit.sub(claimedSplits[_msgSender()])
                .mul(totalVestedRewardForUser[_msgSender()])
                .div(splits);
    }
