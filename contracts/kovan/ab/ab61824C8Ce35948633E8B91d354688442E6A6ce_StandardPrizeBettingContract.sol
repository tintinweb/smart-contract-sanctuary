...
abstract contract AbstractBettingContract is IBettingContract {
    ...
    mapping(address => bool) public betOrNot;
    ...
    function getPrice(uint256 _decimals)
        external
        payable
        override
        onlyOpen
    ...
    function getTicket() public view override returns (uint256) {
    ...
    {
        (uint256 _price, uint256 _decimals) = IPriceContract(priceContract)
            .getPrice(resultId);
...
contract StandardPrizeBettingContract is AbstractBettingContract {
    using SafeERC20 for IERC20;

    uint256 public maxEntrants;
    uint256 public ticketPrice;
    uint256 private upfrontLockedFunds;
    uint256 public rewardForWinner = 975;
    uint256 public rewardForCreator = 25;
    ...
    function setMaxEntrant(uint256 _maxEntrant) external onlyOwner onlyLock {
        require(_maxEntrant > 0);
        maxEntrants = _maxEntrant;
        upfrontLockedFunds =
            (_maxEntrant * ticketPrice * rewardForCreator) /
            10**(decimalOfRate + 2);
    }
        ...
        onlyLock
    {
        ...
        require(getTotalToken() == upfrontLockedFunds);
        priceContract = _priceContract;
        status = Status.Open;
        emit Ready(block.timestamp);
    }

    function distributeReward() external override onlyPriceClosed onlyOpen {
        ...
        if (winners.length > 0) {
            reward =
                ((getTotalToken() - upfrontLockedFunds) * rewardForWinner) /
            ...
            if (winners[0] != address(0x0)) {
                address winner = winners[0];
                delete winners[0];
                IERC20(tokenPool).safeTransfer(winner, getTotalToken());
            }
        }

        IERC20(tokenPool).safeTransfer(creator, getTotalToken());
        emit Close(block.timestamp, price, reward);
    }
        ...
        betable
    {
        ...
        IERC20(tokenPool).safeTransferFrom(
            msg.sender,
            owner,
            IBettingPool(owner).getFee()
        );

        if (_bracketIndex > bracketsPrice.length - 1) {
            _bracketIndex = bracketsPrice.length;
        }
        betOrNot[msg.sender] = true;
        buyers[msg.sender] = _bracketIndex;
        ticketSell[_bracketIndex].push(msg.sender);
        listBuyer.push(msg.sender);
        emit Ticket(msg.sender, _bracketIndex, IBettingPool(owner).getFee());
    }
}
