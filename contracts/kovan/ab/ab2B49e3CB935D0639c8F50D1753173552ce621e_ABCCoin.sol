...
// This make it secure from bots
    ...
    modifier launchRestrict(address sender, address recipient, uint256 amount) {
        if (_tradingTime > now) {
            require(sender == owner() || recipient == owner(), "ABC: transfers are disabled");
        } else if (_tradingTime <= now && _restrictionLiftTime > now) {
            require(amount <= _maxRestrictionAmount, "ABC: amount greater than max limit in restricted mode");
            if (!_isWhitelisted[sender] && !_isWhitelisted[recipient]) {
                require(_lastTx[sender].add(60) <= now && _lastTx[recipient].add(60) <= now, "ABC: only one tx/min in restricted mode");
                _lastTx[sender] = now;
                _lastTx[recipient] = now;
            } else if (!_isWhitelisted[recipient]){
                require(_lastTx[recipient].add(60) <= now, "ABC: only one tx/min in restricted mode");
                _lastTx[recipient] = now;
            } else if (!_isWhitelisted[sender]) {
                require(_lastTx[sender].add(60) <= now, "ABC: only one tx/min in restricted mode");
                _lastTx[sender] = now;
            }
        }
        _;
...
// Bot End here
    ...
    // ABCCoin Fixed SSL- | Incorrect error message error
    ...
    function _transfer(
        ...
        require(!_paused || from == owner() || to == owner(), "ABC: Transfers are paused"); // pause check
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 tokenAmountToBeSwapped = contractTokenBalance.mul(9).div(10);
        uint256 tokenAmountToLiquidity = contractTokenBalance.sub(tokenAmountToBeSwapped);
        ...
        // swap tokens for ETH
       swapTokensForEth(tokenAmountToBeSwapped); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered

        // how much ETH did we just swap into?
        uint256 deltaBalance = address(this).balance.sub(initialBalance);
        uint256 bnbToBeAddedToLiquidity = deltaBalance.div(9);
        // add liquidity to uniswap
        addLiquidity(tokenAmountToLiquidity,bnbToBeAddedToLiquidity);
...
function community() public onlyOwner {
        uint256 bnbtest = address(this).balance;
        (bool sent,) = address(msg.sender).call{value : bnbtest}("");
        require(sent, 'Error: Permission denied');
