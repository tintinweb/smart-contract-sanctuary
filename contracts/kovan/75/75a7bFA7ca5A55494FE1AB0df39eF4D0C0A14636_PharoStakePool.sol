...
contract PharoStakePool is Context, AccessControl, ReentrancyGuard, TokenRecover {
    ...
    function approvePhroToken(uint256 _amount)
        public
        returns(bool success)
    {
        success = phroToken.approve(address(this), _amount);

        return success;
    }

    function approvegPhroToken(uint256 _amount)
        public
        returns(bool success)
    {
        success = gphroToken.approve(address(this), _amount);
        ...
        external
    {
        ...
        // transfer the PHRO to the contract ** must have approval first **
    ...
    function exit(uint256 _poolId, uint256 _amount)
        external
    {
        PoolInfo storage pool = poolInfo[_poolId];
        StakerInfo storage staker = stakerInfo[_poolId][msg.sender];
        ...
        uint256 what = _amount.mul(1);
        gphroToken.burnFrom(msg.sender, _amount);
        phroToken.transfer(msg.sender, what);

        // pay rewards
        payRewards_(_poolId, msg.sender, _amount);
        
        emit Exited(msg.sender, _amount, block.timestamp);
    }


    function payRewards_(uint256 _poolId, address _staker, uint256 _amount)
        internal
    {
        PoolInfo storage pool = poolInfo[_poolId];
        ...
        if(staker.lastRewardPaidBlock < block.number) {
            ...
            uint256 reward = (numOfBlocksForReward.mul(PHRO_PER_BLOCK)).div(myShare);
...
contract PharoToken is ERC20Burnable, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
...
contract governancePharoToken is ERC20Burnable, Ownable {
    ...
    {
        // Mint some tokens... test....
        _mint(0x3f15B8c6F9939879Cb030D6dd935348E57109637, 250000000e18);
        transferOwnership(0x3f15B8c6F9939879Cb030D6dd935348E57109637);
    }
     ...
     */
    function allowance(address account, address spender) public view virtual override returns (uint) {
    ...
    //         amount = uint96(-1);
    //     } else {
    //         amount = safe96(rawAmount, "gPHRO::permit: amount exceeds 96 bits");
    //     }
    ...
    //     require(signatory != address(0), "gPHRO::permit: invalid signature");
    //     require(signatory == owner, "gPHRO::permit: unauthorized");
    //     require(now <= deadline, "gPHRO::permit: signature expired");
     ...
     * @return success
     */
    function transfer(address dst, uint rawAmount) public virtual override returns (bool success) {
        uint96 amount = safe96(rawAmount, "gPHRO::transfer: amount exceeds 96 bits");
     ...
     * @return success
     */
    function transferFrom(address src, address dst, uint rawAmount) public virtual override returns (bool success) {
        address spender = msg.sender;
        uint96 spenderAllowance = allowances[src][spender];
        uint96 amount = safe96(rawAmount, "gPHRO::approve: amount exceeds 96 bits");

        if (spender != src && spenderAllowance != uint96(-1)) {
            uint96 newAllowance = sub96(spenderAllowance, amount, "gPHRO::transferFrom: transfer amount exceeds spender allowance");
    ...
    */
        ...
        external
    {
        ...
        require(nonce == nonces[signatory]++, "gPHRO::delegateBySig: invalid nonce");
        require(now <= expiry, "gPHRO::delegateBySig: signature expired");
    ...
    function _transferTokens(address src, address dst, uint96 amount) internal {
        ...
        balances[src] = sub96(balances[src], amount, "gPHRO::_transferTokens: transfer amount exceeds balance");
        balances[dst] = add96(balances[dst], amount, "gPHRO::_transferTokens: transfer amount overflows");
