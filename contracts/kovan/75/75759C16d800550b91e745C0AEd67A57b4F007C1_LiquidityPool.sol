...
contract LiquidityPool is Initializable, OwnableUpgradeable, BlockLock, PausableUpgradeable, ILiquidityPool {
    ...
    /// @return (uint256) How much USDC the Liquidity Pool owns(1e18)
    function currentLiquidity() public view returns (uint256) {
        return convertTo18(IERC20(stableCoin).balanceOf(address(this)));
    }
    ...
    function repay(uint256 _amount) public notLocked(msg.sender) whenNotPaused {
        ...
        IERC20(stableCoin).safeTransferFrom(msg.sender, address(this), convertFrom18(parsedAmount));
    ...
    function getLPTValue() public view returns (uint256) {
        uint256 totalSupplyLiquidityPool = convertTo18(IERC20(liquidityPoolToken).totalSupply());
