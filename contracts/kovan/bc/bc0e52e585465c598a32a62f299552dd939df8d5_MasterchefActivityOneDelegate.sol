...
contract ActivityBase is Ownable{
    ...
    function setTokenAmountLimit(uint256 _pid, uint256 _tokenAmountLimit) public virtual onlyOwner {
        poolTokenAmountLimit[_pid] = _tokenAmountLimit;
    }

    function setTokenAmountLimitFeeRate(uint256 _feeRateNumerator, uint256 _feeRateDenominator) public virtual onlyOwner {
        require(
    ...
    function calculateDividend(uint256 _pending, uint256 _pid, uint256 _userAmount, bool _isContractSender) internal view returns (uint256 _devDividend, uint256 _userDividend){
        uint256 fee = 0;
...
contract MasterchefActivityOne is ActivityBase{
    using SafeMath for uint256;
    ...
    function initialize(
        ...
        amountfeeRateDenominator = 2;

        contractFeeRateNumerator = 1;
        contractFeeRateDenominator = 2;
    ...
    function add(uint256 _allocPoint, IERC20 _tokenAddress, bool _withUpdate) public virtual onlyOwner{
        if(_withUpdate){
            massUpdatePools();
        }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public virtual {
        ...
        if(isRevenueWithdrawable){
            ...
            (uint256 devDividend, uint256 userDividend) = calculateDividend(pending, _pid, existedAmount, isContractSender);
            if(devDividend > 0){
    ...
    returns (uint256 _pending, uint256 _potential, uint256 _blockNumber) {
        ...
        _potential = totalAvailableDividend > _potential? _potential: totalAvailableDividend;
    }
    ...
    uint256 _revenue, uint256 _rewardDebt, bool _withdrawnState, bool _isContractSender){
        ...
        _isContractSender = isUserContractSender[_pid][_user];
    }
...
// File: contracts/MasterchefActivityOneDelegate.sol

pragma solidity 0.6.12;



contract MasterchefActivityOneDelegate is MasterchefActivityOne, DelegateInterface {
    /**
     ...
     * @notice Called by the delegator on a delegate to forfeit its responsibility
     */
    function _resignImplementation() public override onlyOwner {
        // Shh -- we don't ever want this hook to be marked pure
        if (false) {
            implementation = address(0);
        }
