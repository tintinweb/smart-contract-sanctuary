...
contract EmiStaking02 is Ownable {
    ...
    event StakesClaimed(address indexed beneficiary, uint256 amount);
    ...
    constructor(
        ...
        require(_stakingEndDate > block.timestamp, "Staking end date should be in future");
        lockToken = _token;
        stakingEndDate = _stakingEndDate;
        lockPeriod = _lockPeriod;
        emiRouter = _router;
        stakingLastUnlock = _stakingEndDate+_lockPeriod;
        pathToStables = _path;
        maxUSDStakes = 100000; // 100000 by default
    ...
    function withdraw() external {
        ...
        emit StakesClaimed(msg.sender, _bal);
    }
     ...
     * @param staker Address of staker to pull data for
     */
    function unlockedBalanceOf(address staker) external view onlyOwner returns (uint256) {
        return _getUnlockedBalance(staker);
    }

    /**
     * @dev Return amount of unlocked tokens ready to be claimed by the callee
     */
    function myUnlockedBalance() external view returns (uint256) {
        return _getUnlockedBalance(msg.sender);
    }
     ...
     * @param staker Address of staker to pull data for
     */
    function _getUnlockedBalance(address staker) internal view returns (uint256) {
    ...
    function _checkMaxUSDCondition(address staker, uint256 amount) internal view returns (bool) {
       ...
       // calc price in usd
        ...
        if (tokenMode==0) { // straight token
          ...
          uint256 [] memory tokenAmounts = IEmiRouter(emiRouter).getAmountsOut(_bal.add(amount), pathToStables);
          stakesTotal = tokenAmounts[tokenAmounts.length-1].div(10**tokenDec);
        } else if (tokenMode==1) {
          stakesTotal = _getStakesForLPToken(_bal.add(amount));
        }

        return (stakesTotal <= maxUSDStakes);
    }
