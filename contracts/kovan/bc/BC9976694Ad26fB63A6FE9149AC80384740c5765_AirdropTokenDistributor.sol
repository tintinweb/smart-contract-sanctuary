...
interface IAirdropTokenDistributor {
    ...
    // Returns the merkle root of the merkle tree containing NFT id available to claim.
    function merkleRoot() external view returns (bytes32);
    // Returns true if the index has been marked claimed.
    function isClaimed(uint256 index) external view returns (bool);
    // Claim the given NFT token of a certain ID to the given address. Reverts if the inputs are invalid.
