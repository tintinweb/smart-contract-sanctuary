...
//    The order of view's address.
    ...
    // get lastest price from defferent views in order. stop lookup when get the gToken price
    function getUnderlyingPrice(address gToken) external view returns (uint){
        for (uint32 i = 0; i < views.length; i++) {
            uint underlyingPrice = uint256(PriceViewV1Interface(views[i]).getUnderlyingPrice(gToken));
