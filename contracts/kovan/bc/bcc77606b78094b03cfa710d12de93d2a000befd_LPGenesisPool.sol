...
contract LPGenesisPool is LPTokenWrapper, Ownable {
	ERC1155Tradable public slvtnft;

	uint256 public periodStart = 1616872998;
  ...
  uint256 public multiplier;
	...
	constructor(ERC1155Tradable _slvtnftAddress, IERC20 _erc20Address, uint256 _maxStake, uint256 _multiplier) public LPTokenWrapper(_erc20Address) {
		slvtnft = _slvtnftAddress;
        maxStake = _maxStake;
		...
		return
			points[account].add(
				blockTime.sub(lastUpdateTime[account]).mul(1e18).div(86400).mul(
					(balanceOf(account).mul(multiplier)).div(1e18)
				)
			);
	}
	...
	function stake(uint256 amount) public updateReward(msg.sender) {
		...
		require(amount.add(balanceOf(msg.sender)) <= maxStake, "Max stake exceeded.");
	...
	function redeem(uint256 card) public updateReward(msg.sender) {
		...
		require(slvtnft.totalSupply(card) < slvtnft.maxSupply(card), "Max cards minted");

		points[msg.sender] = points[msg.sender].sub(cards[card]);
		slvtnft.mint(msg.sender, card, 1, "");
		emit Redeemed(msg.sender, cards[card]);
	}

  function setMaxStakeAmount(uint256 _maxStake) public onlyOwner {
    maxStake = _maxStake;
  }
  
    function setMultiplierAmount(uint256 _multiplier) public onlyOwner {
    multiplier = _multiplier;
  }
