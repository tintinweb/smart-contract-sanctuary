...
contract LendingPair is ILendingPair, ReentrancyGuard, TransferHelper, ERC721Receivable {
  ...
  function _borrow(address _token, uint _amount) internal {
    ...
    _checkBorrowEnabled();
    _checkBorrowLimits(_token, _amount);
  ...
  function _checkDepositLimit(address _token, uint _amount) internal view {
    ...
    if (depositLimit > 0) {
      require(totalSupplyAmount[_token] + _amount <= depositLimit, "LendingPair: deposit limit reached");
    }
  }

  function _checkBorrowLimits(address _token, uint _amount) internal view {
