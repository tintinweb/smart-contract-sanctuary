...
contract PresaleBEP20 is Ownable {
    ...
    uint256 public maximumDepositEthAmount = 1 ether;
    uint256 rewardTokenCount;

    mapping(address => uint256) public deposits;

    constructor(DexniftyToken _token, address payable _presale) public {
        token = _token;
        presale = _presale;
        ...
        presaleEndTimestamp = now.add(14 days);
    }
    ...
    function deposit() public payable {
        ...
        uint256 tokenAmount = msg.value.mul(rewardTokenCount).div(1e8);
        ...
        require(token.balanceOf(address(token)) >= tokenAmount, "incorrect amount1");
    ...
    function burn() external onlyOwner {
        ...
        token.burn(address(token), token.balanceOf(address(token)).div(2));
    }
    ...
    function getLatestPrice() internal {
        ...
        rewardTokenCount = uint256(price / 35 * 100);
    }
