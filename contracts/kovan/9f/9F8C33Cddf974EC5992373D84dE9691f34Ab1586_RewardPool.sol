...
contract RewardPool is LPTokenWrapper, IRewardDistributionRecipient {
    ...
    function setEmiPriceData(address _emiPrice, address[3] memory _stables, uint8 _marketID, uint8 _id) public onlyOwner {
    ...
    function getStakedValuesinUSD() public view returns (uint256 senderStake, uint256 totalStake) {
        ...
        senderStake = balanceOf(msg.sender).mul(IEmiPrice2(emiPrice).getCoinPrices(tokens, stables, marketID)[id]);
        totalStake = totalSupply().mul(IEmiPrice2(emiPrice).getCoinPrices(tokens, stables, marketID)[id]);
    }
}
