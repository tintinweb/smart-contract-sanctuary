...
contract BaseContract {
    ...
    function getLastRequest() public view returns (string memory value){
        string  memory v = requests[requests.length-1].value;
        return v;
    }

    function getRequest(uint idx) public view returns (string memory value){
        string  memory v = requests[idx].value;
        return v;
    }
