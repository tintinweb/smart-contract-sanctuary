...
contract Reserve is Ownable {
    ...
    address[] private omsUsdcPath;
    address[] private omsxUsdcPath;

    uint256 private initialUSDInvestmentAmount;
    uint256 private minOmsReserveAmount;
    uint256 private minOmsxReserveAmount;
    ...
    constructor(address _cascade, address _usdc, address _aToken, address _aaveLendingPool, address _uniswapRouter) public Ownable() {
        require(_cascade != address(0), 'The address can not be a zero-address');
        require(_usdc != address(0), 'The address can not be a zero-address');
        require(_aToken != address(0), 'The address can not be a zero-address');
        require(_aaveLendingPool != address(0), 'The address can not be a zero-address');
        require(_uniswapRouter != address(0), 'The address can not be a zero-address');
    ...
    function setOmsxToken(address _omsxToken, uint256 _minReserveAmount) external onlyOwner {
        require(_omsxToken != address(0), 'The address can not be a zero-address');
        omsx = IERC20(_omsxToken);
        minOmsxReserveAmount = _minReserveAmount;
        omsxUsdcPath = [ _omsxToken, address(usdc) ];
    ...
    function updateReserveNg(address _reserveNg) external onlyOwner {
        require(_reserveNg != address(0), 'The address can not be a zero-address');
