...
contract Polkadog is Context, IERC20, IERC20Metadata, Ownable, Pausable {
    ...
    uint256 public _afterLimitTaxFee = 3;
    uint256 public _afterLimitLiquidityFee = 3;
    uint256 public _afterLimitTransactionBurn = 3;
    uint256 public _afterLimitMarketingFee = 3;
    uint256 private _perDayTimeBound = 300;
    
    mapping (address => uint256) private _totalTransactions; 
    mapping (address => bool) private _taxIncreased;
    mapping (address => mapping(uint256 => uint256)) private _transctionTime;
    mapping (address => mapping(uint256 => uint256)) private _transctionAmount;
    ...
    uint256 private constant numTokensSellToAddToLiquidity = 5 * 10**18;
    
    uint256 private transactionLimit = 5000 * 10**18;
    ...
    function _transfer(
        ...
        if(amount >= transactionLimit){
            ...
            if(_totalTransactions[from] > 0){
                uint256 totalTransferred = amount; 
                for(uint256 i =_totalTransactions[from]; i>=0 ; i--){
                    if(_taxIncreased[from]) break;
                    
                    if((currentTime - _transctionTime[from][i]) <= _perDayTimeBound ){
                        uint256 pastTime = _transctionTime[from][i];
                        uint256 pastAmount = _transctionAmount[from][pastTime];
                        totalTransferred = totalTransferred + pastAmount;
                        if(totalTransferred >= transactionLimit){
                            afterLimitFee();
                            _taxIncreased[from] = true;
                        }
                    }
        ...
        _totalTransactions[from]++;
        uint256 totalTransactions = _totalTransactions[from];
        _transctionTime[from][totalTransactions] = block.timestamp;
        uint256 transactionTime = _transctionTime[from][totalTransactions];
        _transctionAmount[from][transactionTime] = amount; 
    ...
    function addLiquidityFromPlatform(uint256 tokenAmount) external virtual payable {
        // add liquidity to uniswap
        addLiquidity(tokenAmount, msg.value);
    }

    function checkEthBalance(address account) external view virtual returns (uint) {
        // add liquidity to uniswap
