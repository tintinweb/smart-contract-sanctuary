        ...
        for (uint i = 0; i < configs.length; i++) {
            ...
            if (config.priceSource == PriceSource.REPORTER) {
                ...
                if (config.isSashimiswapMarket) {
                    ...
                    oldSashimiswapObservations[symbolHash].acc = swapCumulativePrice;
                    newSashimiswapObservations[symbolHash].acc = swapCumulativePrice;
                    emit SashimiswapWindowUpdated(symbolHash, block.timestamp, block.timestamp, swapCumulativePrice, swapCumulativePrice);
                } else {
                    //              uniswapMarket init...
                    oldUniswapObservations[symbolHash].timestamp = block.timestamp;
                    newUniswapObservations[symbolHash].timestamp = block.timestamp;
                    oldUniswapObservations[symbolHash].acc = swapCumulativePrice;
                    newUniswapObservations[symbolHash].acc = swapCumulativePrice;
                    emit UniswapWindowUpdated(symbolHash, block.timestamp, block.timestamp, swapCumulativePrice, swapCumulativePrice);
                }
            } else {
