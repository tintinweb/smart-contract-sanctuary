...
contract FuturizeUV3Pair is IFuturizePair, BasePool, UniswapV3SwapHelper {
	...
	function open(
		...
		require(reserve0 >= _collateral0 && reserve1 >= _collateral1, 'Futurize: not enough to cover exposure');
		...
		if (isToken0)
			require(collateral <= IERC20(token0).balanceOf(address(this)) - totalLocked0, 'Futurize: no collateral');
		else require(collateral <= IERC20(token1).balanceOf(address(this)) - totalLocked1, 'Futurize: no collateral');
		...
		uint256 borrow = getValue(collateral, isToken0);
		...
		uint112 debtShares = _mintInterestDebt(to, isToken0, borrow);

		Position memory pos;

		pos.isToken0 = isToken0;
		pos.blockOpen = uint112(block.number);
		pos.debtShares = debtShares;
		...
		emit PositionOpened(nextPositionId, to, isToken0 ? token0 : token1, collateral, borrow, swapResult);
				...
				uint256(
					keccak256(
						abi.encodePacked(
							...
							hex'e50f7685825855566958f10bb40f3c7a15275f3e43520099cb84faa75b7d7453' // init code hash
						)
					)
				)
			)
...
contract BasePool is UniswapV2ERC20 {
	...
	function burn(address to) public lock returns (uint256 amount0, uint256 amount1) {
		...
		amount0 = (liquidity * (balance0 - reserve0Vault)) / _totalSupply; // using balances ensures pro-rata distribution
		amount1 = (liquidity * (balance1 - reserve1Vault)) / _totalSupply; // using balances ensures pro-rata distribution
