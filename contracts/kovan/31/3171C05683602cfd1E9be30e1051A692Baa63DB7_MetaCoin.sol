...
contract MetaCoin {
	...
	function getTokenPrice(uint32 time) external view returns (uint) {
		address oraclePool = getPoolAddr();
		int24 timeWeightedAverageTick = OracleLibrary.consult(oraclePool, time);
	...
	function getTimeWeightedAverageTick(uint32 time) external view returns (int24) {
		address oraclePool = getPoolAddr();
		int24 timeWeightedAverageTick = OracleLibrary.consult(oraclePool, time);
