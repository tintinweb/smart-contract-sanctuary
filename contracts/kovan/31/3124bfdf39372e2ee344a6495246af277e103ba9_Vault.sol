...
//Receipt tokens owed has a lingering divide by zero error. If receipt tokens owed is nonzero while the vault value is zero, the RTO calculation will result in div by zero.
//ensure safe casting for getPriceOfTokenInWei and all other castings. dont want to overflow
...
//as of july 25 this contract works
...
contract Vault 
{
    ...
    uint public NEGLIGIBLE_AMOUNT_OF_WEI = 10;//1000000000000000000; //This is 10^18. 1 ETH, which is about $2500 at the time of this being written.
    ...
    function deposit() public payable {
        address RECEIPT_TOKEN_ADDRESS = address(0xCE89682a598BCFc1bfF2039055e21287cA4b2706);
        ...
        if (oldTokenSupply == 0) { 
            ...
            receiptTokensOwed = (depositValue * oldTokenSupply) / (newVaultValue - depositValue); //To fix div by zero, a possible solution may be to check if the vault value is zero before running. In that case, you can somehow shut down this contract and get people their funds back through some emergency method
        }
    ...
    function getVaultAmountOfToken(address tokenAddress) private view returns (uint256 vaultAmount) {
        //This method now functions properly for both deposit and rebalance as of August 2nd. If it's breaking something is most likely wrong elsewhere
    ...
    function getPriceOfTokenInWei(address oracleAddress) private view returns (uint256 price) {
        if(oracleAddress == POLYGON_ORACLE_ADDRESS) {
            return (uint256)(IOracle(oracleAddress).latestAnswer() / IOracle(ETH_USD_ORACLE).latestAnswer()); //MATIC is a special case because there isn't an MATIC-ETH oracle currently.
        }
        //Historically had problems with rebalance but not deposit
        return (uint256)(IOracle(oracleAddress).latestAnswer());
    }
