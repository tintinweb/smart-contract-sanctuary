...
contract OnePlanetCarbonOffset is ERC20Interface, Owned, SafeMath {
    ...
    event ApprovedDaiPurchase(address buyer, uint256 ApprovedAmount, bool success);
    ...
    function approveBuy1PLwithDai(uint256 daiAmount) public returns (bool success) {
        
        bytes memory data;
        
        (success, data) = daiAddress.delegatecall(abi.encodeWithSignature("approve(address,uint256)", address(this), daiAmount));
        
        emit ApprovedDaiPurchase(address(this), daiAmount, success);
        return success;
    }
