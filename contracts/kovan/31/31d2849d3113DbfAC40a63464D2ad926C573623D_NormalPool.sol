...
contract NormalPool is Ownable, ReentrancyGuard {
  using SafeMath for uint256;

  constructor(address _bando) public {
    BandoAddress = _bando;
  }

  address public BandoAddress;
  ...
  mapping(address => uint256) private bandoBalance;
  ...
  */
  function stake(uint256 amount) public updateReward(_msgSender()) nonReentrant {
    require(amount.add(balanceOf(_msgSender())) >= 2000000000000000000, "Cannot stake less than 2 BANDO");
    require(amount.add(balanceOf(_msgSender())) <= 10000000000000000000, "Cannot stake more than 10 BANDO");
    bandoBalance[_msgSender()] = bandoBalance[_msgSender()].add(amount);
    IERC20(BandoAddress).transferFrom(_msgSender(), address(this), amount);
  ...
  function withdraw(uint256 amount) public updateReward(_msgSender()) nonReentrant {
    ...
    IERC20(BandoAddress).transfer(_msgSender(), amount);
    bandoBalance[_msgSender()] = bandoBalance[_msgSender()].sub(amount);
  ...
  function redeem(uint256 _id) public updateReward(_msgSender()) nonReentrant {
    ...
    IBandoNFT(NFTAddress).mint(_msgSender(), _id, 1);
