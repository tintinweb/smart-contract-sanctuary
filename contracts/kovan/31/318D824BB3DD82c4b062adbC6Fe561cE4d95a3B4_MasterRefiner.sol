...
contract MasterRefiner is Ownable {
    ...
    function pendingRP1(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > _lastRewardBlock && lpSupply != 0) {
            ...
            uint256 propellantReward = (multiplier * propellantPerBlock * pool.allocPoint) / (totalAllocPoint);
            accRP1PerShare = accRP1PerShare + ((propellantReward*1e12) / lpSupply);
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accRP1PerShare = pool.accRP1PerShare + ((propellantReward * 1e12) / (lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (_userAmount != 0) {
            uint256 pending = (_userAmount * pool.accRP1PerShare) / (1e12) - user.rewardDebt;
            if(pending != 0) {
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        uint256 pending = (_userAmount * _accRP1PerShare) / (1e12) - user.rewardDebt;
        if(pending != 0) {
            safeRP1Transfer(msg.sender, pending);
        }
        if(_amount != 0) {
