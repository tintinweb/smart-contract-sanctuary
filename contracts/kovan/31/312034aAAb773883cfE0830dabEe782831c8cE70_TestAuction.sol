...
contract TestAuction is MintAuction, ERC1155Holder, VRFConsumerBase, KeeperCompatibleInterface {
        ...
        returns (bool upKeepNeeded, bytes memory performData)
    {
        ...
        for(int i = 0; int(i) < int(autoResolveQ.length-1); i++){
        ...
        upKeepNeeded = (hasLink && auctionEnded);
        performData = bytes("");
    }
    ...
    function performUpkeep(bytes calldata /*performData*/) external override {
        ...
        //resolve an auction.
        for(int i = 0; i < int(indexRef.length-1); i++){
            uint idx = indexRef[uint(i)];
            Auction memory auc = autoResolveQ[idx];
...
contract MintAuction  {
    ...
    mapping(address=>uint) pendingReturns;  // tracks users pending returns.
    ...
    function getAutoResolveQ(uint i) public view returns(
        bool,
        address,
        uint256
    )
    ...
    function _ownerOf (address _nft, uint256 _tokenId) internal view returns(bool){
        ERC1155 nft = ERC1155(_nft);
        return nft.balanceOf(msg.sender, _tokenId) != 0;
    }
    // transfers ownership over to the contract
    function _escrow(address _owner, address _nft, uint256 _tokenId) internal {
        ERC1155 nft = ERC1155(_nft);
        nft.safeTransferFrom(_owner, address(this), _tokenId, 1, '');
    }
    // transers an escrowed asset from the contract to a user
    function _transfer(address receiver, address _nft, uint256 _tokenId) internal {
        ERC1155 nft = ERC1155(_nft);
    ...
    function _onAuction(address _nft, uint256 _tokenId) internal view returns(bool){
        Auction storage auction = tokenAddressAndIdToAuction[_nft][_tokenId];
    ...
    function _cancelAuction(address seller, address _nft, uint256 _tokenId) internal {
    ...
    function _computeNetworkFee(uint256 _price) internal view returns(uint256){
    ...
    function _bid(address _nft, uint256 _tokenId) internal returns(uint256){
        ...
        emit Bid(uint128(auction.highestBid), address(auction.nft), uint256(_tokenId));
    ...
    function _End(address _nft, uint256 _tokenId) internal {
        ...
        // SUCCESS
        emit AuctionSuccessful(uint256(_tokenId), uint256(finalPrice), address(winner));
    }
