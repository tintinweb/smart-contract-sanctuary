...
import "./openzeppelin/GovernanceOwnable.sol";
...
contract ArgoPayments is Pausable, GovernanceOwnable {
     ...
     */
    function updateFeederTokenSymbol(string memory s) public onlyGovernanceAddress {
        require(
            bytes(s).length!= 0,
    ...
    function getUnderlyingPrice() public view returns (uint256) {
        ...
        ) = priceFeed.getCoinInfo(feederSymbol);
        return uint256(price) * (10**13);
    }
...
interface IDiaOracle {
	...
	function getCoinInfo(string calldata name) external view returns (uint256, uint256, uint256, string memory);
}
