...
contract Crowdsale is Context, ReentrancyGuard, Ownable {
    ...
    function initSale(
        ...
        require(address(crowdsaleToken) != address(0), "Crowdsale: token is the zero address");
    ...
    function startNewSale(
        ...
        require(closingTime > openingTime, "TimedCrowdsale: opening time is not before closing time");
        require(price > 0, "Crowdsale: price is 0");
        require(_token.balanceOf(address(this)) >= cap, 'Crowdsale: Cap is to big');
        _price = price;
        _cap = cap;
        _openingTime = openingTime;
        _closingTime = closingTime;
