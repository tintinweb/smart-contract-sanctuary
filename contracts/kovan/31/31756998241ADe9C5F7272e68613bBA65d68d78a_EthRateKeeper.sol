...
contract EthRateKeeper is KeeperCompatibleInterface {
    ...
    */
    ...
    constructor(uint updateInterval, address payable _crowdsale) {
    ...
    function checkUpkeep(bytes calldata /* checkData */) external view override returns (bool upkeepNeeded, bytes memory performData ) {
        ...
        //3-. Do conversion
        uint256 result = convert(ethPrice, _decimals);
    ...
    function convert(int _ethPrice, uint8 _decimals) internal view returns (uint256 result){
        //uint256 a =  uint256(_ethPrice)/(10**_decimals); //Get rid of the decimals

        //uint256 b = 
        /*
        _ethPrice($) -------- 1_000_000_000_000_000_000 wei ------ 1 eth
        100_000_000-------1$          --------- x
        */

        result = ONE_ETH_TIMES_ONE_DOLLAR / uint256(_ethPrice);
    }
...
contract PausableCrowdsale is Crowdsale, Pausable, Ownable {
     ...
     */
    function setKeeper(address __keeper) public onlyOwner {
        _keeper = __keeper;
    }
...
contract Crowdsale is Context, ReentrancyGuard {
    ...
    /**
     * @dev Sets a new rate of how many token units a buyer gets per wei.
     * @param __newEthRate New rate of how many token units a buyer gets per wei.
     */
    function _setNewEthRate(uint256 __newEthRate) internal virtual {
        _rate =  __newEthRate;
        emit  RateUpdated( __newEthRate);
    }
    ...
    function buyTokens(address beneficiary) public nonReentrant payable {
        ...
        emit GaufPurchased(_msgSender(), beneficiary, weiAmount, tokens);
