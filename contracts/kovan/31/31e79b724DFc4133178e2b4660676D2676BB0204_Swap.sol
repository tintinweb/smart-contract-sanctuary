...
contract Swap is OwnerPausableUpgradeable, ReentrancyGuardUpgradeable {
     ...
     *
     * @param _dopple dopple token
     * @param _pooledTokens an array of ERC20s this pool will accept
     * @param decimals the decimals to use for each pooled token,
     * eg 8 for WBTC. Cannot be larger than POOL_PRECISION_DECIMALS
     * @param lpTokenName the long-form name of the token to be deployed
     * @param lpTokenSymbol the short symbol for the token to be deployed
     * @param _reserver reserve address
     * @param _a the amplification coefficient * n * (n - 1). See the
     * @param _mintFee dolly mint fee in dopple
     * @param _burnFee dolly burn fee in dopple
    ...
    function initialize(
        ...
        swapStorage.dopple = _dopple;
        swapStorage.reserver = _reserver;
        swapStorage.futureATime = 0;
        swapStorage.mintFee = _mintFee;
        swapStorage.burnFee = _burnFee;
        swapStorage.swapFee = 0;
        swapStorage.adminFee = 0;
        swapStorage.defaultWithdrawFee = 0;
    }
    ...
    function setMintFee(uint256 newMintFee) external onlyOwner {
        swapStorage.setMintFee(newMintFee);
    }
    ...
    function setBurnFee(uint256 newBurnFee) external onlyOwner {
        swapStorage.setBurnFee(newBurnFee);
    }

    /**
     * @notice Update the reserver
     */
    function setReserver(address newReserver) external onlyOwner {
        swapStorage.setReserver(newReserver);
    }
