...
contract MarginSwapPool is IMSPInterface, Tools, Ownable, Exponential, ErrorReporter {
    ...
    function openPosition(
        ...
        if (vars.existId != 0) {
            return morePosition(vars.existId, _supplyAmount, vars.borrowAmount, _swapToken, _amountOutMin);
        }
        ...
        MSPStruct.supplyConfig memory scs = MSPStruct.supplyConfig({
            symbol: EIP20Interface(_swapToken).symbol(), 
            supplyToken: address(_swapToken),
        ...
        emit NewPositionEvent(msg.sender, vars.currentId, Operation.OperationType.OPEN_POSITION, vars.borrowAmount, address(_swapToken), bondTokens, bondAmounts);
    }
    ...
    function morePosition(
        ...
        // console.log("mspconfig.borrowAmount:", mspconfig.borrowAmount, "borrow:", borrow);
        emit MorePositionEvent(_id, _supplyAmount, _borrowAmount, address(_swapToken), acturallySwapAmount, _amountOutMin);
        emit NewPositionEvent(msg.sender, _id, Operation.OperationType.MORE_POSITION, mspconfig.borrowAmount, address(_swapToken), bondTokens, bondAmounts);
    }
    ...
    function addMargin(
        ...
        emit NewPositionEvent(msg.sender, _id, Operation.OperationType.ADD_MARGIN, borrow, address(mspconfig.swapToken), bondTokens, bondAmounts);
    }
    ...
    function redeemMargin(
        ...
        emit NewPositionEvent(msg.sender, _id, Operation.OperationType.REDEEM_MARGIN, mspconfig.borrowAmount, address(mspconfig.swapToken), bondTokens, bondAmounts);
    }
    ...
    function repayFromWallet(uint256 _id, address _repayToken, uint256 _repayAmount, uint256 _amountOutMin) public returns (uint256, uint256) {
        ...
        if (_repayToken != assetUnderlying) {
            ...
            if (_repayAmount == uint256(-1)) {
                ...
                // console.log("债务币种还款:", _repayToken, "tryToRepayAmount:", vars.tryToRepayAmount);
            } else {
        ...
        emit NewPositionEvent(msg.sender, vars.id, Operation.OperationType.REPAY_FROM_WALLET, mspconfig.borrowAmount, address(mspconfig.swapToken), bondTokens, bondAmounts);
    }
    ...
    function repay(uint256 _id, uint256 _repayAmount) public returns (uint256, uint256) {
        ...
        // console.log("mspconfig.borrowAmount:", mspconfig.borrowAmount, mspconfig.isFreeze);
        ...
        emit NewPositionEvent(msg.sender, _id, Operation.OperationType.REPAY_FROM_WALLET, mspconfig.borrowAmount, address(mspconfig.swapToken), bondTokens, bondAmounts);
    }
    ...
    function repayFromMargin(
        ...
        emit NewPositionEvent(msg.sender, vars.id, Operation.OperationType.REPAY_FROM_MARGIN, mspconfig.borrowAmount, address(mspconfig.swapToken), bondTokens, bondAmounts);
        return 0;
    }
    ...
    function closePosition(uint256 _id) public {
        ...
        emit NewPositionEvent(msg.sender, _id, Operation.OperationType.CLOSE_POSITION, 0, address(mspconfig.swapToken), zero1, zero2);
    }
    ...
    function enabledAndDoDeposit(uint256 _id) public {
        ...
        (uint256 borrow, address swapToken, address[] memory bondTokens, uint256[] memory bondAmounts) = getBondTokens(msg.sender, _id);
        emit NewPositionEvent(msg.sender, _id, Operation.OperationType.ENABLE_AND_DEPOSIT, borrow, swapToken, bondTokens, bondAmounts);
    }
     /**
    ...
    function disabledAndDoWithdraw(uint256 _id) public {
        ...
        (uint256 borrow, address swapToken, address[] memory bondTokens, uint256[] memory bondAmounts) = getBondTokens(msg.sender, _id);
        emit NewPositionEvent(msg.sender, _id, Operation.OperationType.DISABLE_AND_WITHDRAW, borrow, swapToken, bondTokens, bondAmounts);
    }
     ...
     */
    function getAccountConfigDetail(address _account, uint256 _id) public view returns ( uint256, uint256, uint256, uint256, address, uint256, bool) {
