...
contract ERC721Mintable is ERC721, MinterRole {
    ...
    mapping(address=>mapping(uint256=>Properties)) public token_properties;
    ...
    function mint(address to, 
        ...
        uint256 satashiId) public onlyMinter returns (bool) {
        _mint(to, tokenId);
		Properties memory p;
        ...
        p.satashi = satashiId;
        token_properties[to][tokenId] = p;
        userClaimIds[to].push(tokenId);
        return true;
    }

	function getProperty(address user, uint256 id) external view returns(Properties memory){
		return token_properties[user][id];
	}
