...
/// TODO 现在存的是address，比如oracle，config，考虑把合约调用优化成直接调用Instance
/// TODO consider collateral factor
contract Controller is IController, Initializable{
    using Address for address;
    using SafeERC20 for IERC20;

    uint256 private constant scalingConstant = 1e18;
    ...
    INonFungiblePosition public override positionToken;
    mapping(address => address) public override ibTokens; // mapping from underlying to bToken
    mapping(address => address) public override underlyings; // mapping from bToken to underlying token

    modifier onlySwapper(){
        /// require(msg.sender == swapper)
        _;
    }
    ...
    function initialize(
        ...
        address positionToken_) external initializer{
        oracle = oracle_;
        config = config_;
        interestModel = interestModel_;
        positionToken = INonFungiblePosition(positionToken_);
    }

    /// 开始前，用户要approve
    /// TODO 添加校验， 如msg.sender必须是swapper
    ...
    ) external override onlySwapper returns (uint256 amountBorrow, uint256 debtShare){
        /// require(msg.sender is swapper, "not swapper")

        /// transfer ibToken from trader
        IERC20(collateralIBToken).safeTransferFrom(trader, address(this), collateralIBTokenAmount);

        amountBorrow = _mulLeverage(
            IOracle(oracle).convertIBToken(
                collateralIBToken,
                debtUnderlying,
                collateralIBTokenAmount
            ),
            leverage
        );

        /// Controller borrow from ibToken with leverage and send to dexSwapper
        /// TODO borrow send token to user change to approve controller spending amount, or consider just allow controller max spending
        DebtToken debtIBToken = DebtToken(ibTokens[debtUnderlying]);
        debtShare = debtIBToken.borrow(msg.sender, amountBorrow);
    }

    /// TODO 目前钱是先到controller再到ibToken，可以考虑直接swap出来到ibToken
    ...
    function openPositionMint(
        ...
        address debtUnderlying,
        uint256 debtShare,
        uint256 leverage) external override onlySwapper returns(uint256 positionId) {
        // do the swap from debt to position in DEX
        address positionIBTokenAddress = ibTokens[positionUnderlying];
        DebtToken positionIBToken = DebtToken(positionIBTokenAddress);

        // TODO 目前钱是先到controller再到ibToken，可以考虑直接swap出来到ibToken，需要修改ibtoken里的transferFrom实现，改成存balance然后检查balance的改变
        _approve(positionUnderlying, positionIBTokenAddress, amount);
        // mint for position re-lending
        uint256 positionCredit = positionIBToken.mint(address(this), amount);
        // mint position NFT
        // TODO consider optimize the struct definitions，是否可以不需要两次才引用到MintParams
        INonFungiblePosition.MintParams memory mintParams =
        INonFungiblePosition.MintParams({
            recipient: recipient,
            collateralIBToken: collateralIBToken,
            debtUnderlying: debtUnderlying,
            debtShare: debtShare,
            leverage: leverage,
            positionIBToken: positionIBTokenAddress,
            totalPositionIBTokenAmount: positionCredit + collateralIBTokenAmount
        });

        // TODO consider add position mint event

        positionId = INonFungiblePosition(positionToken).mint(mintParams);
    }

    // TODO double check if this needs onlySwapper access
    function getClosePositionInfo(uint256 positionId) external override
        returns (
            address debtUnderlying,
        ...
        uint256 debtShare;
        // first get how much debt owed
        (, debtUnderlying, debtShare,, positionIBToken, positionIBTokenAmount)
            = positionToken.getPosition(positionId);

        // TODO consider optimize how we find and use DebtToken address
        DebtToken debtIBToken = DebtToken(ibTokens[debtUnderlying]);
        currentDebtUnderlyingAmount = debtIBToken.fetchDebtShareValue(debtShare);
        positionMarkToDebtAmount =
            IOracle(oracle).convertIBToken(
                positionIBToken,
                debtUnderlying,
                positionIBTokenAmount
            );
        // TODO 完善平仓的检查，包括可能的平仓手续费收取，以及margin rate的检查
    }

    // before closing a position, estimate how much position burn to repay debt
    function closePositionWithdraw(
        address debtUnderlying,
        uint256 currentDebtUnderlyingAmount,
        address positionIBToken,
        uint256 positionIBTokenAmount) external override onlySwapper returns (uint256 burnRequiredAmount){
        DebtToken positionIBTokenInstance = DebtToken(positionIBToken);

        uint256 underlyingNeeded =
            IOracle(oracle).convert(debtUnderlying, positionIBTokenInstance.underlying(), currentDebtUnderlyingAmount);
        // use some of the position ibtoken to withdraw some asset to swapper
        burnRequiredAmount = positionIBTokenInstance.fetchUnderlyingMarkToIBTokenAmount(underlyingNeeded);

        // TODO consider check position health and collateral factor and fee, etc
        require(positionIBTokenAmount >= burnRequiredAmount, "insufficient position");

        // burn and send to swapper
        positionIBTokenInstance.burn(msg.sender, burnRequiredAmount);
    }

    // use swapped debt underlying to repay debt
    // TODO 对swap出来的钱balance做检查
    function closePositionRepayAndBurn(
        ...
        address recipient) external override {
        // first repay the Debt, TODO remove approve here, need to change in DebtToken
        address debtIBToken = ibTokens[debtUnderlying];
        DebtToken debtIBTokenInstance = DebtToken(debtIBToken);
        _approve(debtUnderlying, debtIBToken, repayAmount);
        debtIBTokenInstance.repay(address(this), repayAmount);

        // then send the rest of the positionIBToken back to the recipient
        IERC20(underlyings[positionIBToken]).transfer(recipient, positionLeftAmount);

        // then close and burn the position
        positionToken.burn(closePositionId);
    }
    ...
    // TODO 考虑把这个地方搞美观点
    function _mulLeverage(uint256 amount, uint256 leverage) internal pure returns (uint256){
        return amount * leverage / scalingConstant;
    }

    // TODO will use create function to create ibtoken
    function setIBToken(address ibToken) public {
        DebtToken debtToken = DebtToken(ibToken);
        address underlying = debtToken.underlying();
        ibTokens[underlying] = ibToken;
        underlyings[ibToken] = underlying;
    }
}
...
contract DebtToken is ERC20Permit, ReentrancyGuard {
    using SafeERC20 for IERC20;
    ...
    address public immutable controller; // Controller address
    ...
    /// @dev Initializes the DebtToken contract.
    /// @param _controller Controller address.
    /// @param _underlying The underlying token address for the bToken.
    constructor(address _controller, address _underlying)
    ERC20Permit('Interest Bearing Token')
    ERC20('Interest Bearing Token', 'ibTOKEN')
    {
        require(_controller != address(0), 'constructor/zero-address');
        require(_underlying != address(0), 'constructor/underlying-zero-address');
        controller = _controller;
        underlying = _underlying;
//        interestRate = IInterestModel(IController(_controller).interestModel()).initialRate();
    ...
    function name() public view override returns (string memory) {
        try IERC20Metadata(underlying).name() returns (string memory data) {
            return string(abi.encodePacked('iB ', data));
        } catch (bytes memory) {
            return ERC20.name();
        }
    }
...
//        require(!Pausable(controller).paused(), 'Controller/paused');
...
//        IConfig config = IConfig(IController(controller).config());
//        IInterestModel model = IInterestModel(IController(controller).interestModel());
...
//        interestRate = model.getNextInterestRate(totalLoanable_, totalLoan_);
...
//        }
    ...
    /// @dev Borrows the funds for the given address. Must only be called by Controller.
    ...
    function borrow(address _to, uint _amount) external nonReentrant returns (uint debtShare) {
        require(msg.sender == controller, 'borrow/not-Controller');
        accrue();

        // TODO currently only allow controller to borrow
    ...
    /// @dev Repays the debt using funds from the given address. Must only be called by Controller.
    ...
    function repay(address _from, uint _amount) external nonReentrant returns (uint debtShare) {
        require(msg.sender == controller, 'repay/not-Controller');
        accrue();
        uint amount;
        {
            uint balBefore = IERC20(underlying).balanceOf(address(this));
    ...
    /// @dev Recovers tokens in this contract. EMERGENCY ONLY. Full trust in Controller.
    ...
    function recover(
        ...
        require(msg.sender == controller, 'recover/not-Controller');
    ...
    function fetchIBTokenValueAmount(uint256 credit) external nonReentrant returns(uint256 amount){
        accrue();
        uint supply = totalSupply();
        return (credit * (totalLoanable + totalLoan)) / supply;
    }

//    // TODO check the math, from amount => ibToken and from ibToken => amount
    // TODO might not need
    function fetchUnderlyingMarkToIBTokenAmount(uint256 amount) external nonReentrant returns(uint256 ibTokenAmount){
        accrue();
        ibTokenAmount = (amount * totalSupply()) / (totalLoanable + totalLoan);
    }
}
...
interface IOracle {
    ...
    function getAssetPrice(address token) external returns (uint256);
    ...
    function getAssetValue(address token, uint256 amount) external returns (uint256);
    ...
    function convertIBToken(address fromIBToken, address toToken, uint256 amount) external returns (uint256);
}
