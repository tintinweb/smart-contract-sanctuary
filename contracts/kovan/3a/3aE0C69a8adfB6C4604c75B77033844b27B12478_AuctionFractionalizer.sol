...
contract AuctionFractions
{
	fallback () external payable
	{
		assembly {
			...
			let result := delegatecall(gas(), 0xBa38f6ce8e11bb478FEB459f824771C947ad0336, 0, calldatasize(), 0, 0) // replace 2nd parameter by AuctionFractionsImpl address on deploy
...
contract AuctionFractionsImpl is ERC721Holder, ERC20, ReentrancyGuard
{
	...
	function bid(uint256 _newFractionPrice) external payable nonReentrant inAuction
	{
		...
		if (bidder == address(0)) {
			...
			{
				_transfer(address(this), bidder, balanceOf(address(this)));
				_safeTransfer(paymentToken, bidder, _balanceOf(paymentToken));
			}
			_fractionsCount = balanceOf(_from);
			uint256 _fractionsCount2 = _fractionsCount * _fractionsCount;
		...
		_transfer(_from, address(this), _fractionsCount);
...
contract AuctionFractionalizer is ReentrancyGuard
{
	...
	function fractionalize(address _target, uint256 _tokenId, string memory _name, string memory _symbol, uint8 _decimals, uint256 _fractionsCount, uint256 _fractionPrice, address _paymentToken, uint256 _kickoff, uint256 _duration, uint256 _fee) external nonReentrant
	{
		address _from = msg.sender;
