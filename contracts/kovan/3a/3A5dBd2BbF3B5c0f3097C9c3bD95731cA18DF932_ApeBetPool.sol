...
contract ApeBetPool is Ownable, ReentrancyGuard {
    using SafeMath for uint256;

    event CollectedReward(uint256 _payout);
    ...
    function sendRewards(address payable account, uint256 amount)
        external
        onlyBookMakerOwner
    {
        account.transfer(amount);
        emit CollectedReward(amount);
    }

    function depositUserETH() public payable {}

    function withdrawFunds() external onlyOwner {
        payable(owner()).transfer(bookMaker.maxPoolPayout());
        bookMaker.updateMaxPoolPayout(0, bookMaker.maxPoolPayout());
    }

    // Functions for suppliers.
    function depositETH() external nonReentrant payable {
    ...
    function depositBetToken(uint256 amount) external nonReentrant {
        ...
        deposits[msg.sender] = deposits[msg.sender].sub(removalETH);
        supplierDeposit = supplierDeposit.sub(removalETH);
        bookMaker.updateMaxPoolPayout(0, ethAmount);
    }

    function resetSupplierDeposit() external onlyOwner {
        supplierDeposit = 0;
    }
}