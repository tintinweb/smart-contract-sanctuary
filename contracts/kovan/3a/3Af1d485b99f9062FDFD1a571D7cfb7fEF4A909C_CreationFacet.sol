...
import {Modifiers} from "../base/Modifiers.sol";

import {Errors} from "../libraries/Errors.sol";
import {AppStorage, LibAppStorage} from "../libraries/LibAppStorage.sol";
import {LibOracle} from "../libraries/LibOracle.sol";
import {LibFlightQueue} from "../libraries/LibFlightQueue.sol";
...
contract CreationFacet is Modifiers {
    ...
    function addInsurancePackage(
        ...
        require(packageId != 0, Errors.CREA_INVALID_INSURANCE_PACKAGE_ID);

        InsurancePackage storage insPackage = s.getInsurancePackage(packageId);

        require(
            insPackage.isEmpty(),
            Errors.CREA_INSURANCE_PACKAGE_ALREADY_ADDED
        );
        require(delayInterval > 0, Errors.CREA_INVALID_DELAY_INTERVAL);
    ...
    function createPolicy(bytes32 policyId, PolicyArgs calldata args)
        external
        onlyOracle
    {
    ...
    function createPolicyWithFlight(
        bytes32 policyId,
        PolicyArgs calldata policyArgs,
        bytes32 flightId,
        FlightArgs calldata flightArgs
    ) external onlyOracle {
    ...
    function _createPolicy(bytes32 policyId, PolicyArgs calldata args) private {
        require(policyId != 0, Errors.CREA_INVALID_POLICY_ID);

        Policy storage policy = s.getPolicy(policyId);

        require(policy.isEmpty(), Errors.CREA_POLICY_ALREADY_ADDED);
        require(
            !s.insurancePackages[policy.packageId].isEmpty(),
            Errors.CREA_INVALID_INSURANCE_PACKAGE
        );

        policy.initialize(args.userHash, args.packageId);
    ...
    function _createFlightIfNotExists(
        bytes32 flightId,
        FlightArgs calldata args
    ) private {
        require(flightId != 0, Errors.CREA_INVALID_FLIGHT_ID);

        Flight storage flight = s.getFlight(flightId);
        if (!flight.isEmpty()) {
            return;
        }

        flight.initialize(args.departureTime, args.arrivalTime);
        LibFlightQueue.addFlightToQueue(s.flightQueue, flightId);
        s.totalFlights++;
    ...
    function _setPolicyFlight(bytes32 policyId, bytes32 flightId) private {
        ...
        require(!policy.isEmpty(), Errors.CREA_POLICY_EMPTY);
        require(policy.flightId == 0, Errors.CREA_POLICY_HAS_FLIGHT);

        policy.flightId = flightId;

        s.getFlight(flightId).addPolicy(
            policyId,
            s.getInsurancePackage(policy.packageId).delayInterval
        );
