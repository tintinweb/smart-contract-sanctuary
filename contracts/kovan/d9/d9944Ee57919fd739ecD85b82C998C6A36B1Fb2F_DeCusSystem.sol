...
contract DeCusSystem is Ownable, Pausable, IDeCusSystem, EIP712 {
        ...
        )
    {
        ...
        status = uint8(getGroupStatus(btcAddress));
    ...
    function listGroupStatus(string[] calldata btcAddressArray)
        external
        view
        returns (uint8[] memory statusArray)
    {
        statusArray = new uint8[](btcAddressArray.length);
    ...
    function getGroupStatus(string calldata btcAddress) public view returns (uint8 status) {
        Group storage group = groups[btcAddress];
        if (group.required == 0) {
            return uint8(GroupStatus.None);
        }
