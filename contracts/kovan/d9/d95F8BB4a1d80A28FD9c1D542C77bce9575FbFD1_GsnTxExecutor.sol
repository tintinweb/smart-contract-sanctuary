...
import "./interfaces/IGsnTxExecutor.sol";
import "./interfaces/IUniswapV2Router02.sol";

contract GsnTxExecutor is IGsnTxExecutor, Ownable {
    // Exchange router
    IUniswapV2Router02 private router;

    address private relay = address(0);


    string public constant name = 'GsnTxExecutor';
    string public constant version = '1';
    bytes32 public constant TX_REQUEST_TYPEHASH = keccak256("TxRequest(address from,address to,uint256 value,uint256 gas,uint256 nonce,bytes data,uint256 validUntil)");
    bytes32 public DOMAIN_SEPARATOR;
    mapping(address => uint) public nonces;

    constructor(address _router, address _relay) {
    ...
    function execute(TxRequest calldata req, TxSettings calldata settings, bytes calldata signature) external {
        (settings);
        require(msg.sender == relay, "Unknown relay address");
        require(msg.sender == tx.origin, "relay must sender of transaction");
        _verify(req, signature);

    }

    function txEstimateGas(address from, address to, uint256 value, bytes calldata data) external payable returns (EstimateGasResult memory result) {
        require(msg.sender == relay, "Unknown relay address");
        require(msg.sender == tx.origin, "relay must sender of transaction");
        ...
        (result.success, result.resultData) = to.call{gas : gasBefore, value : value}(callData);
    ...
    function _verify(TxRequest calldata req, bytes calldata signature) internal {
        ...
        require(signer != address(0) && signer == req.from, 'GsnTxExecutor: INVALID_SIGNATURE');
    }
