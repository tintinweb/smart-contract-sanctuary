...
contract GameOfTrustStorage{
    ...
    bool public redeemRewardTokenClaimed;
}
...
contract GameOfTrust is GameOfTrustBase, GameOfTrustStorage {
    ...
    function redeemGAN() external onlyAdmin {
        ...
        require(!redeemRewardTokenClaimed, "Have redeemed");
        uint256 totalSurplusAmount;
        for (uint i = 0; i < poolInfo.length; i++) {
            PoolInfo memory pool = poolInfo[i];
            totalSurplusAmount = totalSurplusAmount.add(calculateSurplusRewardToken(pool, pool.rewardRate));
        }
        redeemRewardTokenClaimed = true;
        IERC20(rewardToken).safeTransfer(msg.sender, totalSurplusAmount);
    ...
    function unlockedWithdraw(
        ...
        (uint256 fine,uint256 totalUnlock, uint256 finalQuantity) = unlockedWithdrawView(_pool, _user, endBlock);
    ...
    function unlockedWithdrawView(
        ...
        return (fine, totalUnlock, finalQuantity);
    }

    function lockedWithdraw(
        PoolInfo storage _pool,
        UserInfo storage _user,
        uint256 _amount
    ) internal returns (uint256, uint256){
        if (block.number <= stakeEndBlock || !redeemRewardTokenClaimed) {
            //        stake period
    ...
    function withdrawToken(
        ...
        uint256 fine;
        uint256 finalQuantity;
        (fine, finalQuantity) = calculateWithdrawalInternal(_amount, _fineRate);
    ...
    //    view withdraw result.
    function pendingWithdraw(uint256 _pid, address _user,uint256 _amount) external view returns (uint256 fine, uint256 reward, uint256 withdrawAmount){
        PoolInfo memory pool = poolInfo[_pid];
        UserInfo memory user = userInfo[_pid][_user];
        if (block.number >= startBlock) {
            if (pool.unlockBlock > 0) {
                uint256 endBlock = pool.unlockBlock.add(pool.unlockCycle);
                (,, withdrawAmount) = unlockedWithdrawView(pool, user, endBlock);
            } else {
    ...
    function harvest(uint256 _pid) external {
        ...
        uint256 unlockAmount = harvestUnlockReward(pool, user,false);
