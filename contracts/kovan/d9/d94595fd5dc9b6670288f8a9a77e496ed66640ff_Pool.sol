...
contract Pool {
    ...
    modifier updateWeight(address account) {
        _totalWeight = _totalWeight.add(_getEnd().sub(_timeRecord).mul(_totalSupply));
        _weights[account] = _weights[account].add(_getEnd().sub(_userTimeRecord[account]).mul(_balances[account]));
        _timeRecord = _getEnd();
        _userTimeRecord[account] = _getEnd();
        _;
    }
    ...
    function _getEnd() private view returns(uint256){
        return Math.min(block.timestamp,endTime);
    }

    function _periodReward() private view returns(uint256){
