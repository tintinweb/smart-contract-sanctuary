...
contract PriceOracle {
    ...
    address internal immutable _baseToken; // _weth
    ...
    modifier valueNotNullCoin(address tokenAddress) {
        require(
            lastUpdatedObservation[tokenAddress].timestamp != uint32(0),
            "key not present"
        );
        _;
    }
    ...
    function addInstancesOfCoin(address tokenAddress) public {
        require(
            tokenAddress != address(0),
            "Token address shoudn't not be equal to null"
        );
        updatePrice(tokenAddress);
    }
    ...
    {
        delete lastUpdatedObservation[tokenAddress];
    }

    function getLatestPriceOfCoin(address tokenAddress)
        public
        view
        valueNotNullCoin(tokenAddress)
        returns (int256)
    {
        FixedPoint.uq112x112 memory tokenPrice = _getTokenPrice(tokenAddress);
        return int256(tokenPrice.mul(1000000000000000000).decode144());
    }
    ...
    function observePrice(address tokenIn)
        internal
        view
