...
contract AirDrop is Context, Ownable {
  ...
  function initContest(address[] memory addresses, uint256[] memory ratios, uint256 endContest) public onlyOwner claimNotActive {
    ...
    for (uint256 i = 0; i < addresses.length; i++) {
      ...
      _amounts[addresses[i]] = tmpAmount;
      _flags[addresses[i]] = true;
  ...
  function claimBNBs() public payable claimActive {
    ...
    if (_addresses.length > 0) {
      require(_flags[attender], "AIRDROP: Current user is not a member of contest anymore!");

      _withdraw(attender, _amounts[attender]);
    } else {
      require(address(this).balance > 0, "AIRDROP: The contract has no money!");
  ...
  function _withdrawRemaining() private onlyOwner {
    ...
    for (uint256 i = 0; i < _addresses.length; i++) {
      if (_amounts[_addresses[i]] > 0) {
        totalRemaining = _amounts[_addresses[i]] + totalRemaining;
        _amounts[_addresses[i]] = 0;
  ...
  function getAttender(address hisAddress) public view returns(uint256, uint256)  {
    require(_flags[hisAddress], "AIRDROP: Not member of contest!");
    return (_ratios[hisAddress], _amounts[hisAddress]);
  }
