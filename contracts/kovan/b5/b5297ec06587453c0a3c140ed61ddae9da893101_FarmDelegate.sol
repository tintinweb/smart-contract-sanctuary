...
contract FarmDelegate is FarmStorageV1 {
    ...
    function reDeposit(uint256 _pid, uint256 amount, string memory channel) public payable {
    ...
    function _getEndBlock(uint256 blockHeightBegin, uint256 rewardPending)internal view returns(uint256 blockHeightEnd,uint256 rewardRest){
        ...
        if( _phase < 200 && (distributeTokenPerBlockConcentratedMining.div(2 ** _phase) == 0 || distributeTokenPerBlockContinuousMining.div(2 ** _phase) == 0)) return(blockHeightBegin,0);
