...
contract Treasury is DexCaller, Adminable {
    ...
    function setDev(address newDev) external onlyAdmin {
        dev = newDev;
    }

    function convertToSharingToken(address fromToken, uint amount, uint minBuyAmount) external {
        if (fromToken == address(lever)) {
            require(lever.balanceOf(address(this)).sub(totalStaked) >= amount, 'Exceed available balance');
        }
