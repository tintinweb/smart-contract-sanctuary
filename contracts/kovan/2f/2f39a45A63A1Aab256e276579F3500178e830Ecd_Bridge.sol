...
contract Bridge is Ownable, AccessControl {
    ...
    event ChargeFee(address indexed user, uint256 fee);
    ...
    function withdrawToken(address _token, uint256 _amount) external onlyOwner {
        IPowerERC20(_token).transfer(owner(), _amount);
    }
    ...
    function transferToOtherChain( uint256 chainId, uint256 amount, address receiver) external {
        require(amount > 0, "amount > 0");
        require(receiver != address(0), "receiver != address(0)");
        require(supportedChain[chainId] == true, "network is not supported");
        ...
        IPowerERC20(token).burnFrom(msg.sender, amount);
    ...
    function mintForTrasferCrossChain(uint256 fromChainId, bytes32 id, uint256 amount, address receiver) external onlyOperator {
        ...
        require(amount > 0, "amount > 0");
        require(receiver != address(0), "receiver != address(0)");
        require(supportedChain[fromChainId] == true, "network is not supported");
        minted[id] = true;
    ...
    function _chargeFee(address user, uint256 fee) private {
        IPowerERC20(token).safeTransferFrom(user, address(this), fee);
