    ...
    NFT721Mint
{
    ...
    /**
     * @notice Allows an admin to update NFT config variables.
     * @dev This must be called right after the initial call to `initialize`.
     */
    function adminUpdateConfig(address _nftMarket, string memory baseURI) public onlyAdmin {
...
abstract contract AccountMigration is OperatorRole {
    ...
    /**
     * @dev Confirms the msg.sender is a operator and that the signature provided is valid.
     * @param signature Message `I authorize to migrate my account to ${newAccount.address.toLowerCase()}`
    ...
    modifier onlyAuthorizedAccountMigration(
        ...
        bytes32 hash =
        _toEthSignedMessage(
            abi.encodePacked("I authorize to migrate my account to ", _toAsciiString(newAddress))
        );
        require(
...
abstract contract NFT721Creator is Initializable, AccountMigration, ERC721Upgradeable {
    ...
    /**
     * @notice Allows an NFT owner or creator and to work together in order to update the creator
    ...
    /**
     * @notice Allows a split recipient and to work together in order to update the payment address
     * to a new account.
     * @param signature Message `I authorize to migrate my account to ${newAccount.address.toLowerCase()}`
     * signed by the original account.
     */
...
/**
 * @notice Holds a reference to the  Market and communicates fees to 3rd party marketplaces.
 */
...
abstract contract NFT721Mint is Initializable, ERC721Upgradeable, NFT721Creator, NFT721Market, NFT721Metadata {
    ...
    /**
     * @notice Allows a creator to mint an NFT and set approval for the marketplace.
    ...
    /**
     * @notice Allows a creator to mint an NFT and have creator revenue/royalties sent to an alternate address.
     * Also sets approval for the marketplace.  This can be used by creators the first time they mint an NFT to
...
/**
 * @notice Allows a contract to leverage the operator role defined by the treasury.
 */
abstract contract OperatorRole is TreasuryNode {
    // This file uses 0 data slots (other than what's included viaTreasuryNode)
