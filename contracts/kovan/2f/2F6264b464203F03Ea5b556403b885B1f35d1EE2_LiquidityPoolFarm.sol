...
contract LiquidityPoolFarm is Ownable {
    ...
    function updatePool() public {
        ...
        accAdyPerShare = accAdyPerShare.add(adyReward.mul(1e18).div(daiSupply));
        lastRewardBlock = block.number;
    }
    ...
    function withdraw(uint256 _amount) public {
        ...
        if (pending > 0) {
            IAuditoryToken(adyToken).approve(address(this), pending);
            IAuditoryToken(adyToken).transfer(address(msg.sender), pending);
        }
        if (_amount > 0) {
            user.amount = user.amount.sub(_amount);
            poolToken.approve(address(msg.sender), pending);
