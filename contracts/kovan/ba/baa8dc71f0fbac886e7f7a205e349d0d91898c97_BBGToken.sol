...
contract BaseToken is Pausable {
    ...
    string constant public name = 'BIGBANG GAMEFI';
    string constant public symbol = 'BBG';
    uint8 constant public decimals = 18;
    uint256 public totalSupply =  268000000*10**uint256(decimals);
...
contract LockToken is BaseToken {
    ...
    function lock(address lockAddress, uint8 lockType, uint256[] endtimeList, uint256[] remainList) public onlyOwner returns (bool) {
        ...
        for (uint256 i = 1; i < length; i++) {
            require(endtimeList[i] > thisEndtime && remainList[i]*1e18 < thisRemain);
...
contract BBGToken is BaseToken, LockToken {
    constructor() public {
