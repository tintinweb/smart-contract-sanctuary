...
// Dependency file: contracts/exchange/LibOrderQueue.sol
...
// Dependency file: contracts/exchange/LibUnsettledTrade.sol
...
// Dependency file: contracts/exchange/ExchangeRoles.sol
...
abstract contract ExchangeRoles {
    ...
    function applyForMaker() external {
        ...
        uint256 expiration = IVotingEscrow(votingEscrow).getTimestampDropBelow(
            msg.sender,
            makerRequirement
        );
...
abstract contract CoreUtility {
    ...
    function _endOfWeek(uint256 timestamp) internal pure returns (uint256) {
        return
            ((timestamp.add(1 days) - (SETTLEMENT_TIME - 16 hours)) / 1 days) *
            1 days +
            SETTLEMENT_TIME -
            16 hours;
    }
...
// Dependency file: contracts/exchange/Staking.sol
...
// Root file: contracts/exchange/Exchange.sol
...
// import {Order, OrderQueue, LibOrderQueue} from "contracts/exchange/LibOrderQueue.sol";
// import {UnsettledBuyTrade, UnsettledSellTrade, UnsettledTrade, LibUnsettledBuyTrade, LibUnsettledSellTrade} from "contracts/exchange/LibUnsettledTrade.sol";

// import "contracts/exchange/ExchangeRoles.sol";
// import "contracts/exchange/Staking.sol";
...
contract Exchange is ExchangeRoles, Staking {
    ...
    address[] private _makersBuy;
    address[] private _makersSell;
    ...
    {
        uint256 price = fund.latestPriceOracle().getLatestPrice();
    ...
    function buyM(
        ...
        (uint256 estimatedNav, , ) = estimateNavs(block.timestamp);
    ...
    {
        (uint256 estimatedNavM, uint256 estimatedNavA, uint256 estimatedNavB) = estimateNavs(epoch);
    ...
    function _buy(
        ...
        // uint256 size = 0;
        // uint256 orderIndex0 = 0;
        // uint256 pdLevel0 = bestAsks[version][tranche];
        // for (; pdLevel0 <= maxPDLevel; pdLevel0++) {
        //     OrderQueue storage orderQueue = asks[version][tranche][pdLevel0];
        //     orderIndex0 = orderQueue.head;
        //     while (orderIndex0 != 0) {
        //         Order storage order = orderQueue.list[orderIndex0];
        //         if (!isMaker(order.maker)) {
        //             orderIndex0 = order.next;
        //             continue;
        //         }
        //         size = size + 1;
        //         orderIndex0 = order.next;
        ...
        // address[] memory makers = new address[](size);
        ...
        for (; pdLevel <= maxPDLevel; pdLevel++) {
            uint256 version_ = version;
            uint256 tranche_ = tranche;
            uint256 estimatedNav_ = estimatedNav;
            uint256 price = pdLevel.mul(PD_TICK).add(PD_START).multiplyDecimal(estimatedNav_);
            OrderQueue storage orderQueue = asks[version_][tranche_][pdLevel];
            ...
            while (orderIndex != 0) {
                ...
                if (currentTrade.reservedBase < order.fillable) {
                    ...
                    currentTrade.effectiveQuote = order.fillable.mul(estimatedNav_).div(
                        MAKER_RESERVE_RATIO
                    );
                ...
                unsettledTrades[order.maker][tranche_][epoch].makerSell.add(currentTrade);
                // makers[index] = order.maker;
                // index++;
                _makersBuy.push(order.maker);
                ...
                _tradeLocked(tranche_, order.maker, currentTrade.reservedBase);
            ...
            if (orderIndex != 0) {
                ...
                if (bestAsks[version_][tranche_] != pdLevel) {
                    bestAsks[version_][tranche_] = pdLevel;
                }
                break;
            }
        }
        ...
        uint256 i = 0;
        uint256 length = _makersBuy.length;
        for (; i < length; i++) {
            settleMaker(_makersBuy[i], epoch);
            _makersBuy.pop();
        }
        settleTaker(msg.sender, epoch);
    }
    ...
    function _sell(
        ...
        // uint256 orderIndex0 = 0;
        // uint256 pdLevel0 = bestAsks[version][tranche];
        // for (; pdLevel0 >= minPDLevel; pdLevel0--) {
        //     OrderQueue storage orderQueue = bids[version][tranche][pdLevel0];
        //     orderIndex0 = orderQueue.head;
        //     while (orderIndex0 != 0) {
        //         Order storage order = orderQueue.list[orderIndex0];
        //         if (!isMaker(order.maker)) {
        ...
        for (; pdLevel >= minPDLevel; pdLevel--) {
            uint256 version_ = version;
            uint256 tranche_ = tranche;
            uint256 baseAmount_ = baseAmount;
            uint256 estimatedNav_ = estimatedNav;
            uint256 price = pdLevel.mul(PD_TICK).add(PD_START).multiplyDecimal(estimatedNav_);
            OrderQueue storage orderQueue = bids[version_][tranche_][pdLevel];
            orderIndex = orderQueue.head;
            while (orderIndex != 0) {
                Order storage order = orderQueue.list[orderIndex];
                ...
                currentTrade.frozenBase = baseAmount_.sub(totalTrade.frozenBase);
                ...
                unsettledTrades[order.maker][tranche_][epoch].makerBuy.add(currentTrade);
                // makers[index] = order.maker;
                // index++;
                _makersSell.push(order.maker);
            ...
            if (orderIndex != 0) {
                // This premium-discount level is not completely filled. Matching ends here.
                if (bestBids[version_][tranche_] != pdLevel) {
                    bestBids[version_][tranche_] = pdLevel;
                }
                break;
            }
        }
        ...
        // uint256 i = 0;
        // for (; i < index; i++) {
        //     settleMaker(makers[i], epoch);
        // }
        uint256 i = 0;
        uint256 length = _makersSell.length;
        for (; i < length; i++) {
            settleMaker(_makersSell[i], epoch);
            _makersSell.pop();
        }
        settleTaker(msg.sender, epoch);
    }
    ...
    function _settleTaker(
        ...
        if (takerBuy.frozenQuote > 0) {
            (uint256 executionQuote, uint256 executionBase) = _buyTradeResult(
                takerBuy,
                estimatedNav
            );
        ...
        if (takerSell.frozenBase > 0) {
            (uint256 executionQuote, uint256 executionBase) = _sellTradeResult(
                takerSell,
                estimatedNav
            );
    ...
    function _transferQuoteFrom(address account, uint256 amount) private {
        uint256 amountToTransfer = amount.add(_quoteDecimalMultiplier - 1) /
