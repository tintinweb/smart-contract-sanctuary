...
contract PairFarmPool is Ownable, ReentrancyGuard {
    ...
    /* 用户每期快照信息表  地址 期数ID */
    mapping(address => mapping(uint256 => UserInfo)) private _userPeriodsInfo;
    ...
    /* 用户期数信息 */
    function userPeriodsInfo(address user, uint256 periodId)
        public
        view
        returns (UserInfo memory)
    {
        PeriodInfo storage periodInfo = periods[periodId]; //  期数信息
        UserInfo storage userInfo = _userPeriodsInfo[user][periodId];
        return
            UserInfo({
                balance: userInfo.balance,
                rewardReceived: userInfo.rewardReceived,
                totalReward: userInfo.balance.div(periodInfo.totalBalance).mul(
    ...
    function forceWithdaw(
        ...
        for (uint256 i = 0; i < periodIds.length; i++) {
            _userPeriodsInfo[user][periodIds[i]]
                .rewardReceived = _userPeriodsInfo[user][periodIds[i]]
                .rewardReceived
        ...
        if (amount > 0) {
            require(
                amount > depositTokenDeduction,
                "PairFarmPool: The number of tokens deducted exceeds the total amount"
            );
            /* 扣除本金并提取 */
    ...
    function _getReward(address user, uint256 periodId) private {
        ...
        UserInfo memory userInfo = userPeriodsInfo(user, periodId); // 用户信息
        uint256 rewardToBeReceived =
            userInfo.totalReward.sub(userInfo.rewardReceived); // 计算待领取奖励
        require(
            rewardToBeReceived > 0,
            "PairFarmPool: The reward has been withdrawn"
        );
        _userPeriodsInfo[user][periodId].rewardReceived = userInfo.totalReward; // 更新已领取奖励
    ...
    function _updateCurrentPeriodUserBalance(address user) private {
        _userPeriodsInfo[user][currentPeriodId()].balance = balances[user];
