...
// File: contracts/library/PriceRange.sol

pragma solidity ^0.7.1;

contract PriceRangeValue {
  enum PriceRange { UNDER_VALUE, STABLE, OVER_VALUE }
}
...
interface DogeV3CalculatorInterface {
  ...
  function getDifference(PriceRange priceRange, PriceRange priceRangeBefore)
    external
    view
    returns (int24 tickDifference);

  function getCurrentPrice(int24 currentTick, bool isReversed)
    external
    view
    returns (uint256 PriceE8);

  function getPriceRange(uint256 priceE8) external view returns (PriceRange);

  function getRatio(PriceRange range) external view returns (uint64 ratio);
}
...
*/
  function createAndInitializePool() external returns (address poolAddress);
  ...
  function depositFirstStage(uint256 amount) external;
  ...
  function currentTokenId()
    external
    view
    returns (uint128 tokenId, uint128 tokenId2);

  function currentPrice() external view returns (uint256 priceE8);
}

// File: contracts/Interfaces/ChainLinkAggregatorInterface.sol
...
contract DogeV3 is Ownable, DogeV3Interface, KeeperCompatibleInterface, IERC20 {
  ...
  uint160 public initialSQRTPrice160 = 50109238377568856638554065;
  uint160 public reversedInitialSQRTPrice160 =
    125268352476029512691171702188174;
  ...
  {
    require(address(swap) == address(0), "Already Initialized");
    if (isReversed) {
      poolAddress = LiquidityManager.createAndInitializePoolIfNecessary(
        address(pair),
        address(this),
        fee,
        reversedInitialSQRTPrice160
      );
    } else {
      poolAddress = LiquidityManager.createAndInitializePoolIfNecessary(
        address(this),
    ...
    swap = IUniswapV3Pool(poolAddress);
  }
  ...
  function depositFirstStage(uint256 amount) external override onlyOwner {
    uint96 beforeBalance = balance[address(this)];
    require(address(swap) != address(0), "Not initialized yet");

    (, int24 tick, , , , , ) = swap.slot0();
    (int24 upperTick, int24 lowerTick) =
      calcs.setInitialTicks(tick, swap.tickSpacing());
    require(
    ...
    LiquidityManager.mint(
      _getMintParams(887220, -887220, balance[address(this)], (amount * 1) / 10)
    );
    uint96 afterBalance = balance[address(this)];
  ...
  function incrementStage() public override {
    int24 tick;
    {
    ...
    PriceRange nextPriceRange = getPriceRangeFromTick(tick);
    ...
    currentStage += 1;
    currentPriceRange = nextPriceRange;
  }
  ...
  function performUpkeep(bytes calldata) external override {
    require(isUpgradable(), "Not upgradable");
    incrementStage();
  }
  ...
  function _transfer(
    ...
    balance[to] = balance[to].add(value.toUint96());
    balance[from] = balance[from].sub(value.toUint96());

    emit Transfer(from, to, value);
    return true;
  }

  function _issueLiquidityNFT(
    int24 tickUpper,
    int24 tickLower,
    int24 currentTick,
    PriceRange nextPriceRange
  ) internal returns (uint256 tokenId, uint256 tokenId2) {
    uint256 dogeAmount = balance[address(this)];
    uint256 ethAmount = pair.balanceOf(address(this));
    uint256 tokenAmount;
    int24 secondUpperTick;
    int24 secondLowerTick;
    if (
      (!isReversed && nextPriceRange == PriceRange.UNDER_VALUE) ||
      (isReversed && nextPriceRange == PriceRange.OVER_VALUE)
    ) {
      tokenAmount = getFirstLiquidity(ethAmount, nextPriceRange);
    ...
    if (nextPriceRange == PriceRange.STABLE) {
      (tokenId, , , ) = LiquidityManager.mint(
        _getMintParams(tickUpper, tickLower, dogeAmount, ethAmount)
      );
    } else {
      (tokenId, , , ) = LiquidityManager.mint(
    ...
    if (
      pair.balanceOf(address(this)) != 0 && nextPriceRange != PriceRange.STABLE
    ) {
    ...
    )
  {
    ...
    (uint160 price, int24 currentTick, , , , , ) = swap.slot0();
    PriceRange nextPriceRange = getPriceRangeFromTick(currentTick);
    if (isReversed) {
      lowerPrice = uint256(10**36)
        .div(
        calcs.getPriceE8FromTick(
          stage[currentStageId].upperTick -
            calcs.getDifference(currentPriceRange, nextPriceRange)
        )
      )
        .div(10**10);
      upperPrice = uint256(10**36)
        .div(
        calcs.getPriceE8FromTick(
          stage[currentStageId].lowerTick +
            calcs.getDifference(currentPriceRange, nextPriceRange)
        )
      )
        .div(10**10);
  ...
  function currentTokenId()
    ...
    returns (uint128 tokenId, uint128 tokenId2)
  {
    tokenId = stage[currentStage].tokenId;
    tokenId2 = stage[currentStage].tokenId2;
  }

  function getPriceRangeFromTick(int24 currentTick)
    internal
    view
    returns (PriceRange)
  {
    uint256 priceE8 = calcs.getCurrentPrice(currentTick, isReversed);
    return calcs.getPriceRange(priceE8);
  }

  function getFirstLiquidity(uint256 wethAmount, PriceRange nextPriceRange)
    public
    view
    returns (uint256)
  {
    uint64 ratioE8 = calcs.getRatio(nextPriceRange);
  ...
  function isUpgradable() public view override returns (bool upgradable) {
    (uint160 price, int24 currentTick, , , , , ) = swap.slot0();
  ...
  function currentPrice() external view override returns (uint256 priceE8) {
    (, int24 currentTick, , , , , ) = swap.slot0();
    return calcs.getCurrentPrice(currentTick, isReversed);
  }
}