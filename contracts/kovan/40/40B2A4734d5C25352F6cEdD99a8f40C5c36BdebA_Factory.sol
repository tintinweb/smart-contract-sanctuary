...
// 	function initialize(bytes32 bPoolOneId, bytes32 bPoolTwoId, address bIntermediateTokenAddress) external;
// }
contract Factory {

	bytes4 private constant INITIALIZE = bytes4(keccak256(bytes('initialize(bytes32,bytes32,address)')));
...

	{
		...
		bytecode = abi.encodePacked(elasticPoolBytecode_, params_);
		_deploy(bytecode, elasticPoolAddress_);

		params_ = abi.encode(owner, usdcDecimals, usdcAddress, reservePoolAddress_, vpStorageAddress_, vpCoefficient);
		bytecode = abi.encodePacked(volatilePoolBytecode_, params_);
		_deploy(bytecode, volatilePoolAddress_);

		params_ = abi.encode(usdcAddress, elasticPoolAddress_, volatilePoolAddress_);
		bytecode = abi.encodePacked(reservePoolBytecode_, params_);
		_deploy(bytecode, reservePoolAddress_);

		params_ = abi.encode(usdcAddress, volatilePoolAddress_, vpsPeriodDays);
		bytecode = abi.encodePacked(vpStorageBytecode_, params_);
		_deploy(bytecode, vpStorageAddress_);

		(bool success, ) = elasticPoolAddress_.call(
			abi.encodeWithSelector(INITIALIZE, bPoolOneId, bPoolTwoId, bIntermediateTokenAddress)
		);
		require(success, 'elastic init failed');
		// IElasticPool(elasticPoolAddress_).initialize(bPoolOneId, bPoolTwoId, bIntermediateTokenAddress);
