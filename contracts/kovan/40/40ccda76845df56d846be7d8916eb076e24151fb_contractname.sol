...
contract contractname is Context, IERC20, Ownable {
    ...
    address private _UtilityWalletAddress = 0x030C7cF295908192B5D2D9d6d326d6eAed3cCb90;
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        ...
        uint256 tburn = calculateutilityfee(tAmount);
    ...
    function _takeburn(uint256 tburn) private {
        uint256 currentRate =  _getRate();
        uint256 rburn = tburn.mul(currentRate);
        _rOwned[_UtilityWalletAddress] = _rOwned[_UtilityWalletAddress].add(rburn);
        if(_isExcluded[_UtilityWalletAddress])
            _tOwned[_UtilityWalletAddress] = _tOwned[_UtilityWalletAddress].add(tburn);
    }
