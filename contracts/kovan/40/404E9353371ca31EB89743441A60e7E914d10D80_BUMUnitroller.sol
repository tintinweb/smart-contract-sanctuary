...
contract BUMUnitroller is BUMUnitrollerAdminStorage, BUMControllerErrorReporter {

    /**
      * @notice Emitted when pendingBUMControllerImplementation is changed
      */
    event NewPendingImplementation(address oldPendingImplementation, address newPendingImplementation);

    /**
      * @notice Emitted when pendingBUMControllerImplementation is accepted, which means comptroller implementation is updated
      */
    ...
    function _setPendingImplementation(address newPendingImplementation) public returns (uint) {
        ...
        address oldPendingImplementation = pendingBUMControllerImplementation;

        pendingBUMControllerImplementation = newPendingImplementation;

        emit NewPendingImplementation(oldPendingImplementation, pendingBUMControllerImplementation);
    ...
    */
    function _acceptImplementation() public returns (uint) {
        ...
        if (msg.sender != pendingBUMControllerImplementation) {
            return fail(Error.UNAUTHORIZED, FailureInfo.ACCEPT_PENDING_IMPLEMENTATION_ADDRESS_CHECK);
        }

        // Save current values for inclusion in log
        address oldImplementation = bumControllerImplementation;
        address oldPendingImplementation = pendingBUMControllerImplementation;

        bumControllerImplementation = pendingBUMControllerImplementation;

        pendingBUMControllerImplementation = address(0);

        emit NewImplementation(oldImplementation, bumControllerImplementation);
        emit NewPendingImplementation(oldPendingImplementation, pendingBUMControllerImplementation);
    ...
    function () external payable {
        ...
        (bool success, ) = bumControllerImplementation.delegatecall(msg.data);
