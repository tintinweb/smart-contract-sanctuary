    ...
    StakingRewards
{
    ...
    function initialize(
        ...
        rewardsToken = IERC20(_rewardsToken); // Liquidity Mining token
        rewardEscrow = RewardEscrow(_rewardEscrow); // Address of vesting contract
        rewardsAreEscrowed = _rewardsAreEscrowed; // True if rewards are escrowed after unstaking
    }
     ...
     *  @param sender address of the minter
     */
    function mint(
        uint8 inputAsset,
        uint256 amount,
        address sender
    ) external onlyTerminal whenNotPaused {
        require(amount > 0);
        ...
        uint256 token0Balance = token0.balanceOf(sender);
        uint256 token1Balance = token1.balanceOf(sender);
        ...
        uint256 mintAmount = _mintInternal(liquidityAmount);
        // Stake CLR tokens
    ...
    /**
     *  @dev Burn *amount* of staked xAssetCLR tokens to receive proportional
     *  @dev amount of pool tokens
     *  @param amount amount of CLR tokens user wants to burn
     *  @param sender address which wants to burn tokens
     */
    function burn(uint256 amount, address sender) external onlyTerminal {
        require(amount > 0);
        ...
        // Check if address has enough balance
        uint256 addressBalance = stakedBalanceOf(sender);
        require(
            addressBalance >= proRataBalance,
            "Address doesn't have enough balance to burn"
        );

        // Burn receipt token
        stakedToken.burnFrom(sender, amount);
        // Unstake rewards
        unstakeRewards(proRataBalance, sender);
        // Burn Staked CLR token
        super._burn(address(this), amount);

        (uint256 amount0, uint256 amount1) = getAmountsForLiquidity(
            uint128(proRataBalance)
        );
        ...
        token0.safeTransfer(sender, amount0);
        token1.safeTransfer(sender, amount1);
    }
    ...
    function getBufferToken0Balance() public view returns (uint256 amount0) {
        if (address(token0) != address(rewardsToken)) {
    ...
    function getBufferToken1Balance() public view returns (uint256 amount1) {
        if (address(token1) != address(rewardsToken)) {
    ...
    function adminRebalance() external onlyOwnerOrManager {
        UniswapLibrary.adminRebalance(
            UniswapLibrary.TokenDetails({
                ...
                rewardToken: address(rewardsToken),
                rewardAmountRemaining: remainingRewardAmount
            }),
    ...
    function _mintInternal(uint256 _amount)
        private
        returns (uint256 mintAmount)
    {
        mintAmount = calculateMintAmount(_amount, totalSupply());
        ...
        onlyOwnerOrManager
    {
        ...
        require(token != address(this), "Cannot withdraw the staking token");
    ...
    /**
     * Configure whether rewards are escrowed or not
     * @param _rewardsAreEscrowed true if the rewards are escrowed in RewardEscrow
     */
    function setRewardsAreEscrowed(bool _rewardsAreEscrowed)
        public
        override
        onlyTerminal
    {
        super.setRewardsAreEscrowed(_rewardsAreEscrowed);
    }
     ...
     */
    function initializeRewards(uint256 reward) public override onlyTerminal {
        super.initializeRewards(reward);
    }
...
library UniswapLibrary {
    ...
    struct TokenDetails {
        ...
        // used to check if t0 or t1 matches reward token in buffer balance
        address rewardToken;
    ...
    function checkTwap(
        ...
        int128 deviation = _lastTwap > twap
            ? _lastTwap - twap
            : twap - _lastTwap;
        int128 maxDeviation = ABDKMath64x64.mul(
            twap,
    ...
    function swapToken0ForToken1(
        ...
        uint256 token0Balance = getBufferToken0Balance(
            ...
            tokenDetails.rewardAmountRemaining
        );
        require(
    ...
    function restoreTokenRatios(
        ...
        uint256 mul1 = amountsMinted.amount0ToMint.mul(
            amountsMinted.amount1Minted
        );
        uint256 mul2 = amountsMinted.amount1ToMint.mul(
            amountsMinted.amount0Minted
        );
        (uint256 balance0, uint256 balance1) = getBufferTokenBalance(
            tokenDetails
 ...
 * Addresses can claim their rewards at any time after initiation
...
contract StakingRewards is IStakingRewards, ReentrancyGuard {
    ...
    IERC20 public rewardsToken; // Address of reward token
    uint256 public override periodFinish = 0; // timestamp at which the rewards program ends
    uint256 public rewardRate = 0; // reward amount unlocked per second
    uint256 public override rewardsDuration = 7 days; // rewards program duration
    uint256 public lastUpdateTime; // last time the rewardPerTokenStored has been updated
    uint256 public rewardPerTokenStored; // reward token amount per staked token amount
    uint256 public totalRewardAmount; // total amount of rewards sent to contract
    uint256 public remainingRewardAmount; // remaining amount of rewards in contract

    mapping(address => uint256) public userRewardPerTokenPaid; // last stored user reward per staked token
    mapping(address => uint256) public rewards; // last stored rewards for user
    ...
    bool public rewardsAreEscrowed; // True if rewards are escrowed in RewardEscrow after unstaking

    RewardEscrow public rewardEscrow; // Vesting / Escrow contract address
    ...
    function stakeRewards(uint256 amount, address sender)
        ...
        updateReward(sender)
    {
        require(amount > 0, "Cannot stake 0 rewards");
        ...
        updateReward(sender)
    {
        ...
        require(
            _stakedBalances[sender] >= amount,
            "Sender does not have enough tokens to withdraw"
        );
     ...
     * @param sender address to claim rewards for
     */
    function claimReward(address sender) public override updateReward(sender) {
        uint256 rewardAmount = earned(sender);
        if (rewardAmount > 0 && remainingRewardAmount >= rewardAmount) {
            rewards[sender] = 0;
            ...
            if (rewardsAreEscrowed) {
                rewardsToken.safeTransfer(address(rewardEscrow), rewardAmount);
                ...
                rewardsToken.safeTransfer(sender, rewardAmount);
            }
            remainingRewardAmount = remainingRewardAmount.sub(rewardAmount);
        }
    }
     ...
     */
    function initializeRewards(uint256 rewardAmount)
        public
        virtual
        override
        updateReward(address(0))
    {
        if (block.timestamp >= periodFinish) {
            ...
            rewardRate = rewardAmount.add(leftover).div(rewardsDuration);
        }
    ...
    function setRewardsAreEscrowed(bool _rewardsAreEscrowed)
        ...
        override
    {
        rewardsAreEscrowed = _rewardsAreEscrowed;
    }

    /* ========== MODIFIERS ========== */

    /**
     * Update the reward per token stored and last update time
     * If an address stakes, unstakes or claims, update his accumulated rewards
...
library Utils {
    ...
    function calculateSwapAmount(
        ...
        uint256 add1 = ABDKMath64x64.mulu(
            liquidityRatio,
            amountsMinted.amount1ToMint
        );
        uint256 add2 = midPrice
            .mul(
        ...
        int128 nRatio = ABDKMath64x64.divu(
