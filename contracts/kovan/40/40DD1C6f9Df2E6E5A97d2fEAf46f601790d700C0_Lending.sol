...
contract Lending is ILending {
    ...
    modifier onlyAdmin() {
        require(msg.sender == admin, "Lending: NOT_ADMIN_ADDRESS");
        _;
    }
	...
	{  
        ...
        if (platformIndex == 1) {
            ...
            if (borrowToken == daiAAVE || borrowToken == usdcAAVE) return (0, 0);
            ...
            (,,uint availableBorrowAmountETH,,,) = aaveLendingPool.getUserAccountData(address(this));
    ...
    function sendCollateral(uint platformIndex, address collateralToken, uint256 amount) external override onlyAdmin {
    ...
    function withdrawCollateral(uint platformIndex, address collateralToken, uint256 amount) external override onlyAdmin {
        require(collateralToken != address(0), "Lending: Invalid borrow token!");
        ...
        if (platformIndex == 1) {
            ...
            infor.liquidationThreshold = (liquidationThreshold / priceReserveETH) * (10 ** decimalReserve);
            ...
            uint borrowedAmount = compoundErc20[collateralToken].borrowBalanceStored(address(this));
            ...
            if(borrowedAmount > 0){
                ...
                uint availableWithdrawAmount = liquidity / oracle.getUnderlyingPrice(collateralToken);
                require(amount < availableWithdrawAmount, "LENDING: COLLATERAL_NOT_ENOUGH");
                compoundErc20[collateralToken].redeemUnderlying(amount);
                IERC20(usdc).transfer(msg.sender, amount);
            }
            else{
                require(amount <= compoundErc20[collateralToken].balanceOfUnderlying(address(this)), "LENDING: COLLATERAL_NOT_ENOUGH");
                compoundErc20[collateralToken].redeemUnderlying(amount);
                IERC20(usdc).transfer(msg.sender, amount);
            }
