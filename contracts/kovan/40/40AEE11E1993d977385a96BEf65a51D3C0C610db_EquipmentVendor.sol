...
contract EquipmentVendor is IEquipmentVendor, Ownable {
    ...
    constructor(IEquipment _equipmentContract) {
        equipmentContract = _equipmentContract;
    }
    
    modifier onlyEquipmentContract() {
        require(address(equipmentContract) == msg.sender, "EquipmentVendor: not from Equipment");
        _;
    }
    
    function addItemsToVendor(uint vendorId, uint[] memory itemIds) external onlyOwner override {
        for(uint i = 0; i < itemIds.length; i++){
            require(!items[itemIds[i]], "EquipmentVendor: Item is existed");
            IEquipment.Item memory item = equipmentContract.getItem(itemIds[i]);
            require(item.tier == 1, "EquipmentVendor: cannot add");
            require(item.minted < item.maxSupply, "EquipmentVendor: Item is reached max supply");
    ...
    function removeItemsFromVendor(uint vendorId, uint[] memory itemIds) external onlyOwner override {
        for(uint i = 0; i < itemIds.length; i++){
            require(items[itemIds[i]], "EquipmentVendor: Item does not exist");
            _itemsOfVendor[vendorId].remove(itemIds[i]);
        }
        emit ItemsRemoved(vendorId, itemIds);
    }

    function mintRandomItems(address account, uint vendorId, uint amount) external override onlyEquipmentContract returns (uint) {
        ...
        for(uint i = 0; i < amount; i++){
            ...
            IEquipment.Item memory item = equipmentContract.getItem(_itemsOfVendor[vendorId].at(random));
            if (item.minted < item.maxSupply) {
                IEquipment(equipmentContract).mint(account, _itemsOfVendor[vendorId].at(random), 1);
                minted++;
                if (item.minted + 1 == item.maxSupply) {
                    _itemsOfVendor[vendorId].remove(_itemsOfVendor[vendorId].at(random));
                    length -= 1;
                }
            }
        }
