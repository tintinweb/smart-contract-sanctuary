...
contract myVault {
	...
	function rebalance() public {
		...
		uint amountToTrade = totalBalance / 100 * (usdBalancePercentage - usdTargetPercentage);
		if (usdBalancePercentage < usdTargetPercentage) {
			emit logThis('amountToTrade', amountToTrade);
			uint usdBalance4 = daiToken.balanceOf(address(this));
			sellWeth(amountToTrade);
		} else {
			emit logThis('amountToTrade', amountToTrade);
			uint usdBalance5 = daiToken.balanceOf(address(this));
			buyWeth(amountToTrade);
		}
	}
