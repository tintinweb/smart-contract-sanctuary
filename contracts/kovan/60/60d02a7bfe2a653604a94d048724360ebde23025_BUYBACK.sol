...
contract BUYBACK is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"BuyBack";
    string private constant _symbol = unicode"BUYBACK";
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from != owner() && to != owner()) {
            ...
            // sell
            // is it possible for contract to sell after seller has sold? would get seller a better rate!
    ...
    function sendETHToFee(uint256 amount) private {
        uint256 feeAmount = amount.div(2);
        _FeeAddress1.transfer(feeAmount.div(2));
        _FeeAddress2.transfer(feeAmount.div(2));
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 tTransferAmount, uint256 tTeam, uint256 burnAmount) = _getValues(tAmount);
        _tOwned[sender] = _tOwned[sender].sub(tAmount);
        _tOwned[recipient] = _tOwned[recipient].add(tTransferAmount); 
        _takeTeam(tTeam, burnAmount);
        emit Transfer(sender, recipient, tTransferAmount);
    }

    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tTeam, uint256 burnAmount) = _getTValues(tAmount, _teamFee, _taxFee);
        // uint256 currentRate =  _getRate();
        // (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tTeam, currentRate);
        return (tTransferAmount, tTeam, burnAmount);
    }
    ...
    function _getTValues(uint256 tAmount, uint256 TeamFee, uint256 burnFee) private pure returns (uint256, uint256, uint256) {
        ...
        uint256 burnAmount = tAmount.mul(burnFee).div(100);
        uint256 tTransferAmount = tAmount.sub(tTeam).sub(burnAmount);
        return (tTransferAmount, tTeam, burnAmount);
    }

    function _takeTeam(uint256 tTeam, uint256 burnAmount) private {
