...
contract ShardingDAOMining is IInvitation, ActivityBase {
    ...
    function withdraw(uint256 _pid) external virtual {
        ...
        WithdrawAmount memory withdrawAmount;  // Relieves stack pressure
        withdrawAmount.pending = poolAccs[_pid].mul(userWeight).div(1e12);
        withdrawAmount.pending = withdrawAmount.pending.sub(senderRevenueInfo.userRewardDebt);
        withdrawAmount.pending = withdrawAmount.pending.add(senderRevenueInfo.revenue);
        senderRevenueInfo.revenue = 0;
        if(withdrawAmount.pending > 0){
            pool.accumulativeDividend = pool.accumulativeDividend.sub(withdrawAmount.pending);
            (withdrawAmount.treasruyDividend, withdrawAmount.devDividend, withdrawAmount.pending) = calculateDividend(withdrawAmount.pending, _pid, amount, isUserContractSender[_pid][msg.sender]);
            if(withdrawAmount.treasruyDividend > 0){
                safeSHARDTransfer(marketingFund, withdrawAmount.treasruyDividend);
            }
            if(withdrawAmount.devDividend > 0){
                safeSHARDTransfer(developerDAOFund, withdrawAmount.devDividend);
            }
            if(withdrawAmount.pending > 0){
                safeSHARDTransfer(msg.sender, withdrawAmount.pending);
            }
        }
