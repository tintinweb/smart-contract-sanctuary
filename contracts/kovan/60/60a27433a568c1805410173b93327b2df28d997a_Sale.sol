...
contract Sale is Ownable {
    ...
    function buyToken(uint256 amountUSDT, string memory orderId) public onlyActive returns(bool) {
        uint256 amountSecurities = (amountUSDT / basePrice);
        Order memory order;
        require(IBEP20(USDT).transferFrom(_msgSender(), address(this), amountUSDT), "transferFrom: USDT error");
    ...
    function buyTokenView(uint256 amountUSDT) public view returns(uint256 USDT, uint256 securities) {
        uint256 amountSecurities = (amountUSDT / basePrice);
        return (
