...
contract AHFactory is ERC1155, ERC1155Holder {
    ...
    function setNFT(string memory _name, string memory _desc, string memory _link, uint256 _copies, uint256 _fee, uint256 _basePrice, string memory _metadata) public {
        ...
        super._mint(msg.sender, nextNFTIndex , _copies,"");     
        mappingNFT[nextNFTIndex].currentOwner = msg.sender;
        ownedNFT[msg.sender].push(nextNFTIndex);

        indexNFT.push(nextNFTIndex);
        nextNFTIndex += 1; 
    }

// how to get token ID from tx hash
    ...
    // check remove purchased nft from marketplace
    ...
    mapping (address => mapping (uint256 => StructAuction)) public mappingAuction;
    ...
    function setAuctionData(string memory tokenId, string memory price, string memory duration, string memory timestamp, address _user) public {
        string memory dataString = string(abi.encodePacked(tokenId,"|", price, "|", duration, "|", timestamp, "|", msg.sender));
    ...
    */  
    function bid(address _seller, uint256 _tokenId) public payable {
        ...
        require(!mappingAuction[_seller][_tokenId].finished, "This auction has ended!.");
    ...
    function finalizeAuction(address _seller, uint256 _tokenId) public {
        require(block.timestamp >= mappingAuction[_seller][_tokenId].auctionEndTime, "Auction not yet finished.");
        require(!mappingAuction[_seller][_tokenId].finished, "Auction already finished!.");
       
        mappingAuction[_seller][_tokenId].finished = true;
        address winner = mappingAuction[_seller][_tokenId].highestBidder;            
        safeTransferFrom(_seller, winner, _tokenId , 1, ""); // transfer from contract
