...
contract LootSpellbook is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function generateSpell(uint256 tokenId, uint256 position) internal view returns (string memory) {
        uint256 rand = random(string(abi.encodePacked("Spell", toString(tokenId), toString(position))));
        string memory spell = spells[rand % spells.length];

        uint256 randLevel = random(string(abi.encodePacked("Level", toString(tokenId), toString(position))));
        string memory level = toString((randLevel % 9)+1);

        return string(abi.encodePacked(spell, " (L", level, ")"));
    }
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        parts[1] = generateSpell(tokenId, 1);

        parts[2] = '</text><text x="10" y="40" class="base">';

        parts[1] = generateSpell(tokenId, 2);

        parts[4] = '</text><text x="10" y="60" class="base">';

        parts[1] = generateSpell(tokenId, 3);

        parts[6] = '</text></svg>';

        string memory output = string(abi.encodePacked(parts[0], parts[1], parts[2], parts[3], parts[4], parts[5], parts[6]));
        
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Spellbook #', toString(tokenId), '", "description": "Loot Spellbooks are randomized collections of seven spells generated and stored on chain. Feel free to use Loot Spellbooks in any way you want.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
    ...
    constructor() ERC721("Spellbooks (for Adventurers)", "LootSpellbook") Ownable() {}
}
