...
contract poop is Context, IERC20, Ownable {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        if ((now - fixedSaleStartTimestamp) <= 180000 && amount <= 10**11 * 10**9){
        ...
        else if ((now - fixedSaleStartTimestamp) > 180000 && (now - fixedSaleStartTimestamp) <= 360000 && amount <= 5 * 10**11 * 10**9){
            // send 4% tokens to developer wallet
            _transferStandard(sender, devWallet, (amount * _developerFee).div(1000));
            ...
            amount = amount - (amount * _developerFee).div(1000);
        ...
        else if ((now - fixedSaleStartTimestamp) > 360000){
            // send 4% tokens to developer wallet
            _transferStandard(sender, devWallet, (amount * _developerFee).div(1000));
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        if(m_AntiBot)                                                                           // Check if AntiBot is enabled
            AntiBot.registerBlock(sender, recipient);   
    }
