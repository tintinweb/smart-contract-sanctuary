...
contract TWSP is ERC20 , ERC20Detailed("TWS Protocol", "TWST", 8), Ownable {
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal override {
    ...
    if( stakeAddr == address(0)) {
      ...
      uint burnFee = amount* 2/100;
      super._transfer(sender, feeTo, fee);
      super._transfer(sender, recipient, amount.sub(fee).sub(burnFee));
      burn(burnFee);
      ...
      uint stakeFee = amount * 10 / 100;
      uint topFee = amount* 3/ 100;
      uint burnFee = amount* 2/100;
      super._transfer(sender, stakeAddr, stakeFee);
      super._transfer(sender, feeTo, topFee);
      super._transfer(sender, recipient, amount.sub(topFee).sub(stakeFee).sub(burnFee));
      burn(burnFee);
    }
  }
