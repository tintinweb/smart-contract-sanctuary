...
contract SHITCOINV3 is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"Shitcoin v3";
    string private constant _symbol = unicode"SHITCOINV3";
    uint8 private constant _decimals = 9;
    uint256 private _taxFee;
    uint256 private _teamFee;
    uint256 private _taxFeeBuy = 4;
    uint256 private _teamFeeBuy = 6;
    uint256 private _taxFeeSell = 4;
    uint256 private _teamFeeSell = 6;
    ...
    IERC20 friendlyToken = IERC20(address(0));
    uint256 friendlyTokenCount = 0;

    // needs to contain different data types (address & number)
    /*
    uint256[2][] friendlyTokenArray; 

    struct FriendlyToken {
        uint256 minamount;
        bool exists;
    }
    */
    ...
    event BuyFeesUpdated(uint _taxfee, uint _teamfee);
    event SellFeesUpdated(uint _taxfee, uint _teamfee);
    event FeeAddress1Updated(address _feewallet1);
    event FeeAddress2Updated(address _feewallet2);

    event FriendlyTokenUpdated(IERC20 _token);
    event FriendlyTokenCountUpdated(uint _amount);
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(address(friendlyToken) != address(0)) {
            if(getTokenBalanceOf(trader) >= friendlyTokenCount) {
                takeFee = false;
            }
        }
    ...
    // restrict to ONLY turning OFF the fee
    ...
    function setBuyFees(uint256 tax, uint256 team) external {
        require(_msgSender() == _feeAddress1);
        require(tax.add(team) <= 10, "Fee is too high");
        _taxFeeBuy = tax;
        _teamFeeBuy = team;
        emit BuyFeesUpdated(_taxFeeBuy, _teamFeeBuy);
    }

    function setSellFees(uint256 tax, uint256 team) external {
        require(_msgSender() == _feeAddress1);
        require(tax.add(team) <= 10, "Fee is too high");
        _taxFeeSell = tax;
        _teamFeeSell = team;
        emit SellFeesUpdated(_taxFeeSell, _teamFeeSell);
    }

    function setFriendlyToken(address tokenAddress) external onlyOwner {
        friendlyToken = IERC20(tokenAddress);
        emit FriendlyTokenUpdated(friendlyToken);
    }

    function setFriendlyTokenCount(uint amount) external onlyOwner {
        friendlyTokenCount = amount * 10**9;
        emit FriendlyTokenCountUpdated(friendlyTokenCount);
    }

    function getTokenBalanceOf(address holder) public view returns (uint balance) {
        return friendlyToken.balanceOf(holder);
    }

    function getFriendlyToken() public view returns (address) {
        return address(friendlyToken);
    }

    /*
    function setFriendlyToken(address tokenAddress, uint amount) external onlyOwner {
        friendlyTokenArray.push([tokenAddress,amount]);
    }

    function getTokenBalanceOfArray(address holder) public returns (uint balance) {
        for(uint i=0; i < friendlyTokenArray.length; i++) {
            IERC20 token = IERC20(friendlyTokenArray[i][1]);
            if (token.balanceOf(holder) >= friendlyTokenArray[i][2]) {
                return true;
            } else {
                return false;
            }
