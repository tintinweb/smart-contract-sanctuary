...
contract ApeBetBookMaker is Ownable, ReentrancyGuard {
        ...
        betsValidation(_bet_event_ids, _bet_winners, _bet_amounts)
    {
        for (uint256 i = 0; i < _bet_event_ids.length; i++) {
            ...
            if (maxReserved[_bet_event_ids[i]][affiliate_id] < reservedAmount) {
                maxPoolPayout = maxPoolPayout.add(maxReserved[_bet_event_ids[i]][affiliate_id]).sub(reservedAmount);
                maxReserved[_bet_event_ids[i]][affiliate_id] = reservedAmount;
            }
