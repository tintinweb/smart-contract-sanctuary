...
contract PlayTreks is Dai {
    GenNFTInterface genft;   /// add generated interface to main contract
   ...
   receive() external payable {
    ...
    function withdrawAllDaiFunds() public onlyCreator {
        ...
        for (uint i=0; i<ownersList.length; i++) {
            daitoken.transfer(ownersList[i], (ownersShares[i]*totalBalance)/100);
            emit Withdrawal(ownersList[i], (ownersShares[i]*totalBalance)/100);
        }
    }
    ...
    function withdrawDaiFunds(uint amount) public onlyCreator {
        ...
        for (uint i=0; i<ownersList.length; i++) {
            daitoken.transfer(ownersList[i], (ownersShares[i]*amounts)/100);
            emit Withdrawal(ownersList[i], (ownersShares[i]*amounts)/100);
        }
    }
    ...
    function withdrawAllEthFunds() public onlyCreator {
        ...
        for (uint i=0; i<ownersList.length; i++) {
           address payable to = payable(ownersList[i]);
            to.transfer((ownersShares[i]*totalBalance)/100);
            emit Withdrawal(ownersList[i], (ownersShares[i]*totalBalance)/100);
        }
    }
    ...
    function withdrawEthFunds(uint amount) public onlyCreator {
       ...
       for (uint i=0; i<ownersList.length; i++) {
           address payable to = payable(ownersList[i]);
            to.transfer((ownersShares[i]*amounts)/100);
            emit Withdrawal(ownersList[i], (ownersShares[i]*amounts)/100);
        }
    ...
    function createEscrow(
      ...
      //bytes32 _s // Signature value from web3.eth.accounts.sign()
        ...
        escrows[_tradeHash] = Escrow(true, _sellerCanCancelAfter, 0, _nft_address, gen);
        emit Created(_tradeHash);
    }
    
    function addNft(address _nft_address) public returns (GenNFTInterface){
        genft = GenNFTInterface(_nft_address);
        return genft;
    }
    ...
    function transferNft(address _to, uint256 _value, uint128 _totalGasFeesSpentByRelayer, uint16 _fee, Escrow memory _escrow) private {
        uint256 _totalFees = (_value * _fee / 10000) + _totalGasFeesSpentByRelayer;
        if(_value - _totalFees > _value) return; // Prevent underflow
        feesAvailableForWithdraw += _totalFees; // Add the the pot for playtreks to withdraw
        _escrow.genft.transfer(_to);
    }
    ...
    function doRelease(
      ...
      uint128 _additionalGas
        ...
        transferNft(_buyer, _value, _gasFees, _fee, escrows[_tradeHash]);
        delete escrows[_tradeHash];
    ...
    function release(string memory _tradeID, address _seller, address _buyer, uint256 _value, uint16 _fee) external returns (bool){
      require(msg.sender == _seller);
      return doRelease(_tradeID, _seller, _buyer, _value, _fee, 0);
    }
    function disableSellerCancel(string memory _tradeID, address _seller, address _buyer, uint256 _value, uint16 _fee) external returns (bool) {
      require(msg.sender == _buyer);
      return doDisableSellerCancel(_tradeID, _seller, _buyer, _value, _fee, 0);
    }
    function buyerCancel(string memory _tradeID, address _seller, address _buyer, uint256 _value, uint16 _fee) external returns (bool) {
      require(msg.sender == _buyer);
      return doBuyerCancel(_tradeID, _seller, _buyer, _value, _fee, 0);
    }
    function sellerCancel(string memory _tradeID, address _seller, address _buyer, uint256 _value, uint16 _fee) external returns (bool) {
      require(msg.sender == _seller);
      return doSellerCancel(_tradeID, _seller, _buyer, _value, _fee, 0);
    }
    function sellerRequestCancel(string memory _tradeID, address _seller, address _buyer, uint256 _value, uint16 _fee) external returns (bool) {
      require(msg.sender == _seller);
    ...
    modifier onlyCreator {
        ...
        require(yes = true, "Only the partners in this distribution can call this function");
        _;
    }
