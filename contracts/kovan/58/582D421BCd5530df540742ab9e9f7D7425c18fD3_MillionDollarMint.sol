...
contract MillionDollarMint is ERC1155, Ownable, RandomNumber {
    ...
    function mint(uint8 amount) public payable {
        require(amount <= 10, "Max 10 per txn");

        totalSupply += amount;
        uint32 ts = totalSupply;

        require(ts <= 10000, "More than total supply");

        uint256 price = amount * PRICE;
        if (ts <= 1000) {
          price = 0 ether;
        } else if(ts > 1000 && ts <= 1009) {
          price = (ts - 1000) * PRICE;
        } 
        ...
        for (uint256 i = ts-amount; i < ts; i++) {
