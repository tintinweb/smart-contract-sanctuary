...
/*
    ...
    */
    ...
    function deployBalancerPool(uint256 _swapFee) public returns (bool success){
      bPoolSTBL = bPool.newBPool();
      bPoolSTBL.setSwapFee(_swapFee);
      success = true;
    }

    function approveTokensForPool() public returns (bool success){
      for (uint256 s = 0; s < poolTokens.length; s += 1){
    ...
    function bindTokensToPool() public returns (bool success){
      for (uint256 s = 0; s < poolTokens.length; s += 1){
    ...
    function finalizePool() public returns (bool success){
      bPoolSTBL.finalize();
      success = true;
    }
