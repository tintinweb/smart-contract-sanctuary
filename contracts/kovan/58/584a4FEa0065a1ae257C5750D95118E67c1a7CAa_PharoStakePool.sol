...
contract PharoStakePool is AccessControl, ReentrancyGuard, TokenRecover {
        ...
        public
    {
        ...
        // todo: check if the contract has a balance of gPHRO
        // if so just transfer them instead of minting.
        if(gphroToken.balanceOf(address(this)) >= _amount) {
            // contract has enough gPHRO
            gphroToken.transferFrom(address(this), msg.sender, amount);
        } else {
            // we have to mint them
