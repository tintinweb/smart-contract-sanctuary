...
contract TestMWRConsumer is ChainlinkClient {
    using Chainlink for Chainlink.Request;
    ...
    function requestElectionWinner(string calldata _date, string calldata _raceId, string calldata _statePostal) public returns (bytes32 requestId) 
    {
        Chainlink.Request memory request = buildChainlinkRequest(jobId, address(this), this.fulfill.selector);
        request.add("date", _date);
        request.add("raceId", _raceId);
        request.add("statePostal", _statePostal);
        request.add("resultsType", "t");
