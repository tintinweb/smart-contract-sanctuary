...
contract Market is Initializable, OwnableUpgradeable, BlockLock, PausableUpgradeable, IMarket {
    ...
    function collateralize(uint256 _amount) external override notLocked(msg.sender) whenNotPaused {
        ...
        address underlyingAsset = Price(assetPriceAddress).underlyingAssetAddress();
        require(
            IERC20(underlyingAsset).balanceOf(address(this)).add(_amount) <= collateralCap,
    ...
    function sendCollateralToLiquidator(
        ...
        IERC20(Price(assetPriceAddress).underlyingAssetAddress()).safeTransfer(_liquidator, tokens);
    }
    ...
    function withdraw(uint256 _amount) external notLocked(msg.sender) whenNotPaused {
        ...
        require(collateralAmount >= _amount, "You have not collateralized that much");

        lock(msg.sender);
        collateralAmount = collateralAmount.sub(_amount);
        ...
        IERC20(Price(assetPriceAddress).underlyingAssetAddress()).safeTransfer(msg.sender, _amount);
    }
}
...
abstract contract Price {
    ...
    uint256 private constant FACTOR = 1e18;
    uint256 private constant PRICE_DECIMALS_CORRECTION = 1e6;
    ...
    function getPrice() external view returns (uint256) {
        ...
        uint256 usdcPrice = usdPrice.mul(FACTOR).div(uint256(usdcusdPrice)).div(usdcPriceDecimalMultiplier).div(
            PRICE_DECIMALS_CORRECTION
        );
        return usdcPrice;
    }
}
