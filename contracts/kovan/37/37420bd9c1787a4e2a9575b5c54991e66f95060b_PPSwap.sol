...
contract PPSwap is ERC20Interface, SafeMath{
     ...
     */
    function getSwapfee(uint offerID, address account) 
        public 
        view
        returns (uint)
    {
        
        if(offerID <= 3 || balances[account] >= 10*10**6*10**18) return 0;
        
        if(balances[account] >=  1*10**6*10**18) return 5*10**14; /* 0.0005 eth */
    ...
    function acceptOffer(uint offerID)
        ...
        uint swapfee = getSwapfee(offerID, msg.sender);
