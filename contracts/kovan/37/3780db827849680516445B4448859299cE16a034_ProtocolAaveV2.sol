...
contract ProtocolAaveV2 is DSMath, ProtocolWETH {
    ...
    function getLTV(address token) public view returns (uint16 lvt) {
        ...
        bytes memory config = aave.getConfiguration(_token);
        lvt = abi.decode(config,(uint16));
    }
}
...
interface AaveInterface {
    ...
    function getConfiguration(address asset) external view returns (bytes memory);
