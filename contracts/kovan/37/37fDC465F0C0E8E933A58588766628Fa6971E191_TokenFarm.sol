...
contract TokenFarm is Ownable {
    ...
    function issueTokens() public onlyOwner {
        ...
        for (uint256 i = 0; i < stakers.length; i++) {
            ...
            // send the recipients token reward
            // based on their total value staked by them.
            // first calculate userTotalValueStaked
            uint256 userTotalValueStaked = getUserTotalValue(recipient);
            dappToken.transfer(recipient, userTotalValueStaked);
        }
    }
    ...
    function getUserTotalValue(address _user) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < allowedTokens.length; i++) {
            totalValue += getUserSingleTokenTotalValue(_user, allowedTokens[i]);
        }
        return totalValue;
    }

    function getUserSingleTokenTotalValue(address _user, address _token)
        public
        view
        returns (uint256)
    {
    ...
    {
        // we can get this information from chain-link pricefeed
    ...
    function stakeTokens(uint256 _amount, address _token) public {
        // what tokens can users stake ? // how much amount can they stake?
