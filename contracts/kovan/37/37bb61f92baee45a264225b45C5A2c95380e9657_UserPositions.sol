    ...
    IUserPositions
{
    ...
    function initialize(address[] memory controllers_, address moduleMap_, uint32 biosRewardsDuration_) public initializer {
    ...
    function setBiosRewardsDuration(uint32 biosRewardsDuration_) external override onlyController {
        require(_biosRewardsDuration != biosRewardsDuration_, 
            "UserPositions::setBiosRewardsDuration: Duration must be set to a new value");
        require(biosRewardsDuration_ > 0, 
    ...
    function seedBiosRewards(address sender, uint256 biosAmount) external override onlyController {
        require(biosAmount > 0, "UserPositions::seedBiosRewards: BIOS amount must be greater than zero");

        IERC20MetadataUpgradeable bios = IERC20MetadataUpgradeable(IIntegrationMap(moduleMap
            .getModuleAddress(Modules.IntegrationMap)).getBiosTokenAddress());
    ...
    function deposit(
        ...
        for(uint256 tokenId; tokenId < tokens.length; tokenId++) {
            ...
            require(integrationMap.getTokenAcceptingDeposits(tokens[tokenId]),
            ...
            IERC20MetadataUpgradeable erc20 = IERC20MetadataUpgradeable(tokens[tokenId]);

            // Get the balance before the transfer
            uint256 beforeBalance = erc20.balanceOf(moduleMap.getModuleAddress(Modules.Kernel));

            // Transfer the tokens from the depositor to the Kernel
            erc20.safeTransferFrom(depositor, moduleMap.getModuleAddress(Modules.Kernel), amounts[tokenId]);

            // Get the balance after the transfer
            uint256 afterBalance = erc20.balanceOf(moduleMap.getModuleAddress(Modules.Kernel));
            uint256 actualAmount = afterBalance - beforeBalance;

            // Increase rewards
            IBiosRewards(moduleMap.getModuleAddress(Modules.BiosRewards)).increaseRewards(tokens[tokenId], depositor, actualAmount);
            IEtherRewards(moduleMap.getModuleAddress(Modules.EtherRewards)).updateUserRewards(tokens[tokenId], depositor);
        ...
        if(ethAmount > 0) {
            ...
            // Increase rewards
            IBiosRewards(moduleMap.getModuleAddress(Modules.BiosRewards)).increaseRewards(wethAddress, depositor, ethAmount);
            IEtherRewards(moduleMap.getModuleAddress(Modules.EtherRewards)).updateUserRewards(wethAddress, depositor);
    ...
    private returns (
        ...
        for(uint256 tokenId; tokenId < tokens.length; tokenId++) {
            ...
            require(integrationMap.getTokenAcceptingWithdrawals(tokens[tokenId]),
                "UserPositions::_withdraw: This token is not accepting withdrawals");
            require(amounts[tokenId] <= _balances[tokens[tokenId]][recipient], 
                "UserPositions::_withdraw: Withdraw amount exceeds user balance");

            if(IERC20MetadataUpgradeable(tokens[tokenId]).balanceOf(moduleMap.getModuleAddress(Modules.Kernel)) < amounts[tokenId]) {
                ...
                if(IERC20MetadataUpgradeable(tokens[tokenId]).balanceOf(moduleMap.getModuleAddress(Modules.Kernel)) < amounts[tokenId]) {
                    ...
                    amounts[tokenId] = IERC20MetadataUpgradeable(tokens[tokenId]).balanceOf(moduleMap.getModuleAddress(Modules.Kernel));
                }
            }
            ...
            if(tokens[tokenId] == wethAddress && withdrawWethAsEth) {
                ...
                IERC20MetadataUpgradeable(tokens[tokenId])
                    .safeTransferFrom(moduleMap.getModuleAddress(Modules.Kernel), recipient, amounts[tokenId]);
            }
    ...
    function closePositionsForWithdrawal(address token, uint256 amount) private {
        IIntegrationMap integrationMap = IIntegrationMap(moduleMap.getModuleAddress(Modules.IntegrationMap));
        uint256 tokenIntegrationWeightSum = integrationMap.getTokenIntegrationWeightSum(token);

        // Iterate through integrations and close positions in proportion to integration weights
        for(uint256 integrationId; integrationId < integrationMap.getIntegrationAddressesLength(); integrationId++) {
            address integrationAddress = integrationMap.getIntegrationAddress(integrationId);

            uint256 desiredWithdrawAmount = amount * integrationMap
                .getTokenIntegrationWeight(integrationAddress, token) / tokenIntegrationWeightSum;

            if(desiredWithdrawAmount > IIntegration(integrationAddress).getBalance(token)) {
                desiredWithdrawAmount = IIntegration(integrationAddress).getBalance(token);
            }

            IIntegration(integrationAddress).withdraw(token, desiredWithdrawAmount);
        }

        if(IERC20MetadataUpgradeable(token).balanceOf(moduleMap.getModuleAddress(Modules.Kernel)) < amount) {
    ...
    function fullyClosePositionsForWithdrawal(address token, uint256 amount) private {
        IIntegrationMap integrationMap = IIntegrationMap(moduleMap.getModuleAddress(Modules.IntegrationMap));
        uint256 integrationId;
        bool doneClosingPositions;

        while(!doneClosingPositions) {
            IIntegration integration = IIntegration(integrationMap.getIntegrationAddress(integrationId));

            // Withdraw the tokens full balance from the integration
            integration.withdraw(token, integration.getBalance(token));

            if(integrationId == integrationMap.getIntegrationAddressesLength() - 1 || 
                IERC20MetadataUpgradeable(token).balanceOf(moduleMap.getModuleAddress(Modules.Kernel)) >= amount) {
    ...
    function _increaseBiosRewards() private {
        ...
        uint256 kernelBiosBalance = IERC20MetadataUpgradeable(biosAddress).balanceOf(moduleMap.getModuleAddress(Modules.Kernel));

        require(kernelBiosBalance > biosRewards.getBiosRewards() + _totalSupply[biosAddress], 
            "UserPositions::increaseBiosRewards: No available BIOS to add to rewards");

        uint256 availableBiosRewards = kernelBiosBalance - biosRewards.getBiosRewards() - _totalSupply[biosAddress];
    ...
    function _increaseTokenBiosRewards(address token, uint256 biosReward) private {
        ...
        require(IERC20MetadataUpgradeable(IIntegrationMap(moduleMap.getModuleAddress(Modules.IntegrationMap))
            .getBiosTokenAddress()).balanceOf(moduleMap.getModuleAddress(Modules.Kernel)) >= 
    ...
    function claimEthRewards(address recipient) external override onlyController returns (uint256 ethClaimed) {
        ethClaimed = _claimEthRewards(recipient);
    }

    /// @param recipient The address of the user claiming BIOS rewards
    function _claimEthRewards(address recipient) private returns (uint256 ethClaimed) {
        ethClaimed = IEtherRewards(moduleMap.getModuleAddress(Modules.EtherRewards)).claimEthRewards(recipient);
    }
    ...
    function _claimBiosRewards(address recipient) private returns (uint256 biosClaimed) {
        ...
        for(uint256 tokenId; tokenId < tokenCount; tokenId++) {
            ...
            if(biosRewards.earned(token, recipient) > 0) {
                biosClaimed += IBiosRewards(moduleMap.getModuleAddress(Modules.BiosRewards)).claimReward(token, recipient);
            }
        }

        IERC20MetadataUpgradeable(integrationMap.getBiosTokenAddress())
            .safeTransferFrom(moduleMap.getModuleAddress(Modules.Kernel), recipient, biosClaimed);
    } 
