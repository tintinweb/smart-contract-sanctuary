 ...
 * @author Mudra, influenced by https://github.com/wbobeirne/eth-balance-checker/blob/master/contracts/BalanceChecker.sol
 * @notice Implements a logic of getting multiple tokens balance for one user address
 * @dev NOTE: THIS CONTRACT IS NOT USED WITHIN THE MUDRA PROTOCOL. It's an accessory contract used to reduce the number of calls
 * towards the blockchain from the Mudra backend.
 **/
...
interface ILendingPool {
   ...
   * For further details please visit https://developers.mudra.com
 ...
 * @author Mudra
 * @notice Defines the error messages emitted by the different contracts of the Mudra protocol
...
import {VersionedInitializable} from '../libraries/mudra-upgradeability/VersionedInitializable.sol';
  ...
  LendingPoolStorage
{
  ...
  function liquidationCall(
    ...
    vars.collateralAtoken = IMToken(collateralReserve.mTokenAddress);
    ...
    if (receiveMToken) {
      vars.liquidatorPreviousMTokenBalance = IERC20(vars.collateralAtoken).balanceOf(msg.sender);
      ...
      // Burn the equivalent amount of mToken, sending the underlying to the liquidator
...
import {IMudraIncentivesController} from './IMudraIncentivesController.sol';
...
interface IMToken is IERC20, IScaledBalanceToken, IInitializableMToken {
   ...
   **/
  function getIncentivesController() external view returns (IMudraIncentivesController);

  /**
   * @dev Returns the address of the underlying asset of this mToken (E.g. WETH for mWETH)
   **/
 ...
 * @notice Interface for the Mudra price oracle.
 **/
...
interface ILendingPoolCollateralManager {
   ...
   * @param receiveMToken true if the liquidators wants to receive the mTokens, false if
 ...
 *
 * @author Mudra, inspired by the OpenZeppelin Initializable contract
 */
    ...
    )
  {
    ...
    for (vars.i = 0; vars.i < reservesCount; vars.i++) {
      ...
      if (vars.liquidationThreshold != 0 && userConfig.isUsingAsCollateral(vars.i)) {
        vars.compoundedLiquidityBalance = IERC20(currentReserve.mTokenAddress).balanceOf(user);
...
library ValidationLogic {
    ...
    if (interestRateMode == uint256(DataTypes.InterestRateMode.STABLE)) {
      ...
      require(
        !userConfig.isUsingAsCollateral(reserve.id) ||
          ...
          amount > IERC20(reserve.mTokenAddress).balanceOf(userAddress),
        Errors.VL_COLLATERAL_SAME_AS_BORROWING_CURRENCY
      );

      vars.availableLiquidity = IERC20(asset).balanceOf(reserve.mTokenAddress);
  ...
  function validateSwapRateMode(
    ...
    if (currentRateMode == DataTypes.InterestRateMode.STABLE) {
      ...
      require(
        !userConfig.isUsingAsCollateral(reserve.id) ||
          reserve.configuration.getLtv() == 0 ||
          stableDebt.add(variableDebt) > IERC20(reserve.mTokenAddress).balanceOf(msg.sender),
        Errors.VL_COLLATERAL_SAME_AS_BORROWING_CURRENCY
      );
    } else {
  ...
  function validateRebalanceStableBorrowRate(
    ...
    uint256 availableLiquidity = IERC20(reserveAddress).balanceOf(mTokenAddress).wadToRay();
  ...
  function validateSetUseReserveAsCollateral(
    ...
    uint256 underlyingBalance = IERC20(reserve.mTokenAddress).balanceOf(msg.sender);
...
interface IInitializableMToken {
   ...
   * @param treasury The address of the Mudra treasury, receiving the fees on this mToken
   * @param underlyingAsset The address of the underlying asset of this mToken (E.g. WETH for mWETH)
  ...
  function initialize(
    ...
    IMudraIncentivesController incentivesController,
    uint8 mTokenDecimals,
...
interface IMudraIncentivesController {
...
library ReserveLogic {
  ...
  function init(
    ...
    require(reserve.mTokenAddress == address(0), Errors.RL_RESERVE_ALREADY_INITIALIZED);
...
import {ILendingPoolAddressesProvider} from '../interfaces/ILendingPoolAddressesProvider.sol';
import {IMudraIncentivesController} from '../interfaces/IMudraIncentivesController.sol';
...
contract UiPoolDataProvider is IUiPoolDataProvider {
  ...
  IMudraIncentivesController public immutable incentivesController;
  IPriceOracleGetter public immutable oracle;

  constructor(IMudraIncentivesController _incentivesController, IPriceOracleGetter _oracle) public {
    ...
    )
  {
    ...
    for (uint256 i = 0; i < reserves.length; i++) {
      ...
      reserveData.mTokenAddress = baseData.mTokenAddress;
      ...
      // we're getting this info from the mToken, because some of assets can be not compliant with ETC20Detailed
      reserveData.symbol = IERC20Detailed(reserveData.mTokenAddress).symbol();
      reserveData.name = '';
      ...
      if (address(0) != address(incentivesController)) {
        (
          reserveData.mTokenIncentivesIndex,
          reserveData.aEmissionPerSecond,
          reserveData.aIncentivesLastUpdateTimestamp
        ) = incentivesController.getAssetData(reserveData.mTokenAddress);
      ...
      if (user != address(0)) {
        // incentives
        if (address(0) != address(incentivesController)) {
          userReservesData[i].mTokenincentivesUserIndex = incentivesController.getUserAssetData(
            user,
            reserveData.mTokenAddress
          );
        ...
        userReservesData[i].scaledMTokenBalance = IMToken(reserveData.mTokenAddress)
...
interface IUiPoolDataProvider {
  ...
  struct UserReserveData {
    ...
    // incentives
    uint256 mTokenincentivesUserIndex;
 ...
 * - An instance of this same contract, can't be used across different Mudra markets, due to the caching
...
contract DefaultReserveInterestRateStrategy is IReserveInterestRateStrategy {
  ...
  {
    uint256 availableLiquidity = IERC20(reserve).balanceOf(mToken);
 ...
 * @notice Interface for the Mudra borrow rate oracle. Provides the average market borrow rate to be used as a base for the stable borrow rate calculations
 **/
...
contract WETHGateway is IWETHGateway, Ownable {
  ...
  function withdrawETH(
    ...
    IMToken mWETH = IMToken(ILendingPool(lendingPool).getReserveData(address(WETH)).mTokenAddress);
    uint256 userBalance = mWETH.balanceOf(msg.sender);
...
/// - Owned by the Mudra governance system, allowed to add sources for assets, replace them
///   and change the fallbackOracle
contract MudraOracle is IPriceOracleGetter, Ownable {
  using SafeERC20 for IERC20;
  ...
  /// @notice External function called by the Mudra governance to set or replace sources of assets
  /// @param assets The addresses of the assets
  /// @param sources The address of the source of each asset
  ...
  /// - Callable only by the Mudra governance
...
/**
 * @title Mudra ERC20 MToken
 * @dev Implementation of the interest bearing token for the Mudra protocol
 * @author Mudra
 */
contract MToken is
  VersionedInitializable,
  IncentivizedERC20('MTOKEN_IMPL', 'MTOKEN_IMPL', 0),
  IMToken
{
  using WadRayMath for uint256;
  using SafeERC20 for IERC20;
  ...
  IMudraIncentivesController internal _incentivesController;
  ...
  /**
   * @dev Returns the address of the Mudra treasury, receiving the fees on this mToken
   **/
   ...
   **/
  function _getIncentivesController() internal view override returns (IMudraIncentivesController) {
   ...
   * @dev Returns the address of the incentives controller contract
   **/
  function getIncentivesController() external view override returns (IMudraIncentivesController) {
 ...
 * @author Mudra, inspired by the Openzeppelin ERC20 implementation
 **/
abstract contract IncentivizedERC20 is Context, IERC20, IERC20Detailed {
   ...
   **/
  function _getIncentivesController() internal view virtual returns (IMudraIncentivesController);
...
/**
 * @title Mudra MToken enabled to delegate voting power of the underlying asset to a different address
 ...
 * @dev Main registry of LendingPoolAddressesProvider of multiple Mudra protocol's markets
 * - Used for indexing purposes of Mudra protocol's markets
 * - The id assigned to a LendingPoolAddressesProvider refers to the market it is connected with,
 *   for example with `0` for the Mudra main market and `1` for the next created
 * @author Mudra
 **/
...
import {
  VersionedInitializable
} from '../../libraries/mudra-upgradeability/VersionedInitializable.sol';
import {IncentivizedERC20} from '../IncentivizedERC20.sol';
import {Errors} from '../../libraries/helpers/Errors.sol';
...
import {MToken} from '../protocol/tokenization/MToken.sol';
import {
  DefaultReserveInterestRateStrategy
} from '../protocol/lendingpool/DefaultReserveInterestRateStrategy.sol';
import {Ownable} from '../dependencies/openzeppelin/contracts/Ownable.sol';
...
contract MTokensAndRatesHelper is Ownable {
  address payable private pool;
  address private addressesProvider;
  address private poolConfigurator;
  event deployedContracts(address mToken, address strategy);
 ...
 * @dev Main point of interaction with an Mudra protocol's market
 * - Users can:
 *   # Deposit
 *   # Withdraw
 *   # Borrow
...
contract LendingPool is VersionedInitializable, ILendingPool, LendingPoolStorage {
  ...
  function deposit(
    ...
    reserve.updateState();
    reserve.updateInterestRates(asset, mToken, amount, 0);

    IERC20(asset).safeTransferFrom(msg.sender, mToken, amount);

    bool isFirstDeposit = IMToken(mToken).mint(onBehalfOf, amount, reserve.liquidityIndex);
  ...
  function withdraw(
    ...
    uint256 userBalance = IMToken(mToken).balanceOf(msg.sender);
    ...
    reserve.updateInterestRates(asset, mToken, 0, amountToWithdraw);
    ...
    IMToken(mToken).burn(msg.sender, to, amountToWithdraw, reserve.liquidityIndex);
  ...
  function repay(
    ...
    address mToken = reserve.mTokenAddress;
    reserve.updateInterestRates(asset, mToken, paybackAmount, 0);

    if (stableDebt.add(variableDebt).sub(paybackAmount) == 0) {
      _usersConfig[onBehalfOf].setBorrowing(reserve.id, false);
    }

    IERC20(asset).safeTransferFrom(msg.sender, mToken, paybackAmount);

    IMToken(mToken).handleRepayment(msg.sender, paybackAmount);
  ...
  function swapBorrowRateMode(address asset, uint256 rateMode) external override whenNotPaused {
    ...
    reserve.updateInterestRates(asset, reserve.mTokenAddress, 0, 0);
  ...
  function rebalanceStableBorrowRate(address asset, address user) external override whenNotPaused {
    ...
    address mTokenAddress = reserve.mTokenAddress;
    ...
    reserve.updateInterestRates(asset, mTokenAddress, 0, 0);
  ...
  //     address[] memory mTokenAddresses = new address[](assets.length);
  //     uint256[] memory premiums = new uint256[](assets.length);

  //     vars.receiver = IFlashLoanReceiver(receiverAddress);

  //     for (vars.i = 0; vars.i < assets.length; vars.i++) {
  //       mTokenAddresses[vars.i] = _reserves[assets[vars.i]].mTokenAddress;

  //       premiums[vars.i] = amounts[vars.i].mul(_flashLoanPremiumTotal).div(10000);

  //       IMToken(mTokenAddresses[vars.i]).transferUnderlyingTo(receiverAddress, amounts[vars.i]);
  //     }
  ...
  //           IERC20(vars.currentMTokenAddress).totalSupply(),
  //           vars.currentPremium
  //         );
  ...
  function finalizeTransfer(
    ...
    require(msg.sender == _reserves[asset].mTokenAddress, Errors.LP_CALLER_MUST_BE_AN_ATOKEN);
  ...
  /**
   * @dev Initializes a reserve, activating it, assigning an mToken and debt tokens and an
   * interest rate strategy
   * - Only callable by the LendingPoolConfigurator contract
   * @param asset The address of the underlying asset of the reserve
   * @param mTokenAddress The address of the mToken that will be assigned to the reserve
   * @param stableDebtAddress The address of the StableDebtToken that will be assigned to the reserve
   * @param mTokenAddress The address of the VariableDebtToken that will be assigned to the reserve
   * @param interestRateStrategyAddress The address of the interest rate strategy contract
   **/
  function initReserve(
    address asset,
  ...
  function _executeBorrow(ExecuteBorrowParams memory vars) internal {
    ...
    if (vars.releaseUnderlying) {
      IMToken(vars.mTokenAddress).transferUnderlyingTo(vars.user, vars.amount);
    }
...
// prettier-ignore
import {InitializableImmutableAdminUpgradeabilityProxy} from '../libraries/mudra-upgradeability/InitializableImmutableAdminUpgradeabilityProxy.sol';
...
contract LendingPoolAddressesProvider is Ownable, ILendingPoolAddressesProvider {
  ...
  /**
   * @dev Returns the id of the Mudra market to which this contracts points to
...
} from '../libraries/mudra-upgradeability/InitializableImmutableAdminUpgradeabilityProxy.sol';
import {ReserveConfiguration} from '../libraries/configuration/ReserveConfiguration.sol';
import {ILendingPoolAddressesProvider} from '../../interfaces/ILendingPoolAddressesProvider.sol';
import {ILendingPool} from '../../interfaces/ILendingPool.sol';
import {IERC20Detailed} from '../../dependencies/openzeppelin/contracts/IERC20Detailed.sol';
 ...
 * @author Mudra
 * @dev Implements the configuration methods for the Mudra protocol
 **/
...
contract LendingPoolConfigurator is VersionedInitializable, ILendingPoolConfigurator {
  ...
  function _initReserve(ILendingPool pool, InitReserveInput calldata input) internal {
    address mTokenProxyAddress =
      _initTokenWithProxy(
        input.mTokenImpl,
        abi.encodeWithSelector(
          IInitializableMToken.initialize.selector,
          pool,
          input.treasury,
          input.underlyingAsset,
          IMudraIncentivesController(input.incentivesController),
   ...
   **/
  function updateMToken(UpdateMTokenInput calldata input) external onlyPoolAdmin {
    ILendingPool cachedPool = pool;
    ...
    _upgradeTokenImplementation(reserveData.mTokenAddress, input.implementation, encodedCall);
  ...
  function _checkNoLiquidity(address asset) internal view {
    ...
    uint256 availableLiquidity = IERC20Detailed(asset).balanceOf(reserveData.mTokenAddress);
 ...
 * @author Mudra, inspired by the OpenZeppelin upgradeability proxy pattern
...
contract MudraProtocolDataProvider {
  using ReserveConfiguration for DataTypes.ReserveConfigurationMap;
  using UserConfiguration for DataTypes.UserConfigurationMap;
  ...
  function getAllMTokens() external view returns (TokenData[] memory) {
    ...
    for (uint256 i = 0; i < reserves.length; i++) {
      ...
      mTokens[i] = TokenData({
        symbol: IERC20Detailed(reserveData.mTokenAddress).symbol(),
...

{
  ...
  "libraries": {
    "contracts/protocol/libraries/logic/ReserveLogic.sol": {
      "ReserveLogic": "0xa711838aafd9e9bc2526c360a100988048f07558"
    },
    "contracts/protocol/libraries/logic/ValidationLogic.sol": {
      "ValidationLogic": "0xedd538de7ea46296386974007be7b8511c795d20"
    }
  }
}