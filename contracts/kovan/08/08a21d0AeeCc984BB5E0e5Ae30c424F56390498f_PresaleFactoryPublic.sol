...
contract LessLibrary is Ownable {
    ...
    uint256 private votingTime = 15 minutes; //three days
    ...
    function createPresale(
        ...
        require(
            _info.openTime > block.timestamp &&
                _info.openVotingTime + safeLibrary.getVotingTime() + 900 <= 
                _info.openTime &&
                _info.closeTime - _info.openTime > 1200 &&
...
contract PresalePublic is ReentrancyGuard {
    ...
    mapping(address => bool) private whitelisTier;
    ...
    modifier votesPassed(uint256 totalStakedAmount) {
        require(intermediate.yesVotes >= intermediate.noVotes, "Votes against more than votes for");
        require(
            intermediate.yesVotes >= lessLib.getMinYesVotesThreshold(totalStakedAmount), 
            "Votes amount less than minimal threshold"
        );
        require(block.timestamp >= generalInfo.closeTimeVoting, "Voting is still open");
        _;
    }
    ...
    function init(
        ...
        require(
            _priceTokensForSaleLiquiditySoftHardOpenCloseFee[8] > 0,
            "No fee"
        );
    ...
    function register(
    ...
    ) external openRegister notCreator votesPassed(_totalStakedAmount) {
        ...
        require(!whitelisTier[msg.sender], "al. whitelisted");
        if (_tier < 3)
            tickets.push(
                TicketsInfo(msg.sender, _tokenAmount / (500 * tokenMagnitude))
            );
        whitelisTier[msg.sender] = true;
    ...
    function invest(
        ...
        notCreator
    {
        require(whitelisTier[msg.sender], "not whitelisted");
        ...
        if (nowTime < generalInfo.openTimePresale + 300) {
            require(
                ...
                "Staked amount does not meets requirements for TIER 5"
            );
            tokensLeft =
            ...
            require(
                ...
                "Staked amount does not meets requirements for TIER 4"
            );
            tokensLeft =
            ...
            require(
                ...
                "Staked amount does not meets requirements for TIER 3"
            );
            tokensLeft =
                (intermediate.beginingAmount - 
                    generalInfo.tokensForSaleLeft) +
            ...
            require(
                ...
                "Staked amount does not meets requirements for TIER 2"
            );
            tokensLeft =
                (intermediate.beginingAmount - 
                    generalInfo.tokensForSaleLeft) +
            ...
            require(
                ...
                "Staked amount does not meets requirements for TIER 1"
            );
...

{
  ...
  "evmVersion": "berlin",
  "libraries": {
    "/wsl-files/work/git/eth/less-contracts/contracts/libraries/Calculations.sol": {
