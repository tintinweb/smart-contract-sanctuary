...
contract TokenFarm is Ownable {
    ...
    // mapping token  address to user address to amounts
    ...
    function addAllowedTokens(address token) public onlyOwner returns (bool) {
    ...
    function getUserTotalValue(address user) public view returns (uint256) {
        ...
        if (uniqueTokensStaked[user] > 0) {
            for (
                ...
                totalValue =
                    totalValue +
                    getUsersStakingBalanceEthValue(
                        user,
    ...
    function getUsersStakingBalanceEthValue(address user, address token)
        public
        view
        returns (uint256)
    {
