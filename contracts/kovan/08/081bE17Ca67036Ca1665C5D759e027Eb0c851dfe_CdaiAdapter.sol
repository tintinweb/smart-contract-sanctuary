...
contract CdaiAdapter is AdapterBase {
    ...
    //RedeemCErc20Tokens: Number of CTokens to be redeemed
        ...
        returns (uint256, uint256)
    {
        ...
        uint256 receivedUnderlyingTokenQuantity =
            redeemTokens.mul(_cdai.exchangeRateCurrent()).div(10**18);
        require(
            _cdai.redeem(redeemTokens) == 0,
    ...
    //RedeemCErc20Tokens: Amount of underlying to be redeemed
    function redeemWrappedTokenInUnderlying(uint256 redeemAmount)
        external
        onlyBy(farmingPoolAddress)
        returns (uint256, uint256)
    {
        require(redeemAmount != 0, "CdaiAdapter: can't redeem 0");

        uint256 beforeBalance =
            _cdai.balanceOf(address(this)).mul(_cdai.exchangeRateCurrent()).div(
                10**18
            );
        uint256 receivedUnderlyingTokenQuantity = redeemAmount;
        require(
            _cdai.redeemUnderlying(redeemAmount) == 0,
            "CdaiAdapter: error redeeming underlying"
        );
        ...
        uint256 actualAmount = beforeBalance.sub(afterBalance);
        // slither-disable-next-line reentrancy-events
        emit RedeemWrappedTokenInUnderlying(
            underlyingAssetAddress,
            programAddress,
            redeemAmount,
            actualAmount,
    ...
    event RedeemWrappedTokenInUnderlying(
