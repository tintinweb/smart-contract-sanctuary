...
contract Bank is IBank, ERC20UpgradeSafe, ReentrancyGuardUpgradeSafe, OwnableUpgradeSafe {
  /// @notice Libraries
  ...
  uint256 public bankDebtShare;
  uint256 public bankDebtVal;
  ...
  modifier onlyEOA() {
    //
    require(msg.sender == tx.origin, "Bank::onlyEoa:: not eoa");
    _;
  }

  /// @dev Get token from msg.sender
  modifier transferTokenToBank(uint256 value) {
    if (msg.value != 0) {
      require(token == config.getWrappedNativeAddr(), "Bank::transferTokenToBank:: baseToken is not wNative");
      require(value == msg.value, "Bank::transferTokenToBank:: value != msg.value");
  ...
  modifier inExec() {
    require(POSITION_ID != _NO_ID, "Bank::inExec:: not within execution scope");
    require(STRATEGY == msg.sender, "Bank::inExec:: not from the strategy");
    require(_IN_EXEC_LOCK == _NOT_ENTERED, "Bank::inExec:: in exec lock");
  ...
  modifier accrue(uint256 value) {
    if (now > lastAccrueTime) {
      ...
      bankDebtVal = bankDebtVal.add(interest);
      lastAccrueTime = now;
    }
    _;
  }
  ...
  function initialize(
    ...
    masterChefID = uint256(-1);

    debtToken = _debtToken;

    SafeToken.safeApprove(debtToken, config.getMasterChefAddr(), uint256(-1));
  ...
  function pendingInterest(uint256 value) public view returns (uint256) {
    if (now > lastAccrueTime) {
      ...
      uint256 ratePerSec = config.getInterestRate(bankDebtVal, balance);
      return ratePerSec.mul(bankDebtVal).mul(timePast).div(1e18);
    } else {
      return 0;
    }
  }
  ...
  function debtShareToVal(uint256 debtShare) public view returns (uint256) {
    if (bankDebtShare == 0) return debtShare; // When there's no share, 1 share = 1 val.
    return debtShare.mul(bankDebtVal).div(bankDebtShare);
  }
  ...
  function debtValToShare(uint256 debtVal) public view returns (uint256) {
    if (bankDebtShare == 0) return debtVal; // When there's no share, 1 share = 1 val.
    return debtVal.mul(bankDebtShare).div(bankDebtVal);
  }
  ...
  function withdraw(uint256 share) external override accrue(0) nonReentrant {
    ...
    require(supply == 0 || supply >= 1e16, "Bank::withdraw:: no tiny shares");
  }

  /// @dev Request Funds from user through Bank
  ...
  function _masterChefDeposit(uint256 id, uint256 amount) internal {
    if (amount > 0) {
      IDebtToken(debtToken).mint(address(this), amount);
      IMasterChef(config.getMasterChefAddr()).deposit(positions[id].owner, masterChefID, amount);
    }
  }

  function _masterChefWithdraw(uint256 id) internal {
    if (positions[id].debtShare > 0) {
      // Note: Do this way because we don't want to fail open, close, or kill position
      // if cannot withdraw from  somehow. 0xb5c5f672 is a signature of withdraw(address,uint256,uint256)
      (bool success, ) = config.getMasterChefAddr().call(abi.encodeWithSelector(0xb5c5f672, positions[id].owner, masterChefID, positions[id].debtShare));
  ...
  function work(uint256 id, address worker, uint256 principalAmount, uint256 loan, uint256 maxReturn, bytes calldata data)
    external payable
    onlyEOA transferTokenToBank(principalAmount) accrue(principalAmount) nonReentrant
  {
    require(masterChefID != uint256(-1), "Bank::work:: poolId not set");
    ...
    if (id == 0) {
      ...
      pos = positions[id];
      require(id < nextPositionID, "Bank::work:: bad position id");
      require(pos.worker == worker, "Bank::work:: bad position worker");
      require(pos.owner == msg.sender, "Bank::work:: not position owner");
      _masterChefWithdraw(id);
    }
    emit Work(id, loan);
    ...
    require(config.isWorker(worker), "Bank::work:: not a worker");
    require(loan == 0 || config.acceptDebt(worker), "Bank::work:: worker not accept more debt");
    ...
    uint256 back;
    {
      ...
      require(sendBEP20 <= SafeToken.myBalance(token), "Bank::work:: insufficient funds in the bank");
    ...
    if (debt > 0) {
      require(debt >= config.minDebtSize(), "Bank::work:: too small debt size");
      uint256 health = IWorker(worker).health(id);
      uint256 workFactor = config.workFactor(worker, debt);
      require(health.mul(workFactor) >= debt.mul(10000), "Bank::work:: bad work factor");
      _addDebt(id, debt);
      _masterChefDeposit(id, pos.debtShare);
    }
  ...
  function kill(uint256 id) external onlyEOA accrue(0) nonReentrant {
    require(masterChefID != uint256(-1), "Bank::kill:: poolId not set");
    // 1. Verify that the position is eligible for liquidation.
    Position storage pos = positions[id];
    require(pos.debtShare > 0, "Bank::kill:: no debt");
    // 2. Distribute ALPACAs in masterChef to owner
    _masterChefWithdraw(id);
    ...
    require(health.mul(killFactor) < debt.mul(10000), "Bank::kill:: can't liquidate");
  ...
  function _addDebt(uint256 id, uint256 debtVal) internal {
    ...
    bankDebtShare = bankDebtShare.add(debtShare);
    bankDebtVal = bankDebtVal.add(debtVal);
  ...
  function _removeDebt(uint256 id) internal returns (uint256) {
    ...
    if (debtShare > 0) {
      ...
      pos.debtShare = 0;
      bankDebtShare = bankDebtShare.sub(debtShare);
      bankDebtVal = bankDebtVal.sub(debtVal);
  ...
  function updateDebtToken(address _debtToken, uint256 _newPid) external onlyOwner {
    require(_debtToken != token, "Bank::updateDebtToken:: _debtToken must not be the same as token");
    address[] memory okHolders = new address[](2);
    okHolders[0] = address(this);
    okHolders[1] = config.getMasterChefAddr();
    IDebtToken(_debtToken).setOkHolders(okHolders, true);
    debtToken = _debtToken;
    masterChefID = _newPid;
    SafeToken.safeApprove(debtToken, config.getMasterChefAddr(), uint256(-1));
  }

  function setMasterChefPoolId(uint256 _poolId) external onlyOwner {
    SafeToken.safeApprove(debtToken, config.getMasterChefAddr(), uint256(-1));
    masterChefID = _poolId;
  }
...
interface IBankConfig {
  ...
  /// @dev Return the address of masterChef contract.
  function getMasterChefAddr() external view returns (address);
