...
contract ChainlinkVRF is VRFConsumerBase, Ownable {
    ...
    mapping (uint => uint256) public chainlinkResults;//VRF return per nonce
    mapping (uint => uint256[]) internal randomResults; // 10 numbers first 5 under 100K, last 5 unique between 1 and 11,111
    mapping (uint => uint256) public blockNumberResults;//blockNumber so that can prove ran after certain time
    mapping (uint => mapping (uint => bool)) public chosenIndices;//indices per nonce (last 5 only)
    mapping (uint => bool) public nonceCalculated; //whether that nonce has been calculated or not
    uint public nonce;//tracks which roll (and also allows proof against re-roll accusations)
    ...
    */
    ...
    function calculateNumbers(bool useCurrNonce, uint _nonce) public onlyOwner{
        ...
        while(currentIndex < 10){
            target = (uint256(keccak256(abi.encode(randomNumber, counter))) % modulus)+1;
            while (chosenIndices[currNonce][target]){
                counter++;
                target = (uint256(keccak256(abi.encode(randomNumber, counter))) % modulus) + 1;
            }
            numbersArray[currentIndex] = target;
            chosenIndices[currNonce][target] = true;
            currentIndex++;
            counter++;
        }
