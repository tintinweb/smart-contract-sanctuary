...
contract Aggreator {
    ...
    struct pairRoundIdPrice {
        string pair;
        mapping (uint256 => mapping(address => bool)) roundIdPrice;
    }

    // mapping (string => mapping(address => bool)) public pairNode;
    mapping (address => bool) public approveList;
    mapping (address => bool) public setRequestList;
    ...
    mapping (uint256 => mapping(address => mapping(string => bool))) private roundIdPrice;
    mapping (string => int256) public latestPrice;
    mapping (string => uint256) finish;
    ...
    uint256 private minResponses = 1;
    ...
    function addSetRequestList(address _listener) public {
        require(msg.sender == teamAddress,"no admin");
        setRequestList[_listener] = true;
    }
    
    function removeSetRequest(address _listener) public {
        require(msg.sender == teamAddress,"no admin");
        setRequestList[_listener] = false;
    }

    function setRequest(uint256 _roundId,string memory _pair) public {
        require(setRequestList[msg.sender],"no admin");
        require(_roundId == roundIds[_pair].roundId,"roundId is wrong");
        require(block.timestamp - roundIds[_pair].timeStamp > 180 || finish[_pair] == 2,"roundId invalid");
        finish[_pair] = 1;
        delete pairPrice[_pair];
        uint256 temp = priceRequestFee.mul(priceRequestDestroy).div(100);
        destroyFee += temp;
        reward = reward + priceRequestFee - temp;
    ...
    function receiveData(string memory _pair, uint256 _roundId, int256 _price) external {
        ...
        require(!roundIdPrice[_roundId][msg.sender][_pair],"send price multy times");
        pairPrice[_pair].push(_price);
        roundIdPrice[_roundId][msg.sender][_pair] = true;
...
//设置喂价更新请求时销毁的比例，50%为50，
    function setPriceRequestDestroy(uint256 _priceRequestDestroy) external {
        require(msg.sender == teamAddress,"no admin");
        priceRequestDestroy = _priceRequestDestroy;
    }

//追加保证金
    function addDepoistFee(uint256 _depoistFee) external {
        require(nodes[msg.sender],"no node");
        uint256 tmp = _depoistFee * 10 ** decimals;
        depoistFeeBalance[msg.sender] += tmp;
        safeTransferFrom(token, msg.sender, address(this), tmp);
    }
...
//赎回担保金
    function withdraw() external {
        ...
        depoistFeeBalance[msg.sender] = 0;
        safeTransfer(token, msg.sender, depoistFeeBalance[msg.sender]);
    }

    function claim() external {
        // require(nodes[msg.sender],"no permission");
        require(nodeFeedPriceTimes[msg.sender] != 0, "no send data");
    ...
    function getBalance(address _nodeAddress) external view returns(uint256) {
        uint256 tmp = reward.mul(nodeFeedPriceTimes[_nodeAddress]).div(totalTimes);
        return tmp;
    }
