...
contract MasterChef is Ownable, ReentrancyGuardUpgradeable {
        ...
        if (vaultMonth == 12) {
            ...
            if (vaultMonth != 0) {
                daysOfStakingscore = daysOfStakingscore.sub(
                    daysByMonthConstant.mul(vaultMonth).div(month)
                );
            }
            stakingscorenett = amountstaked.mul(stakeIndays).div(daysecondss).div(
                daysOfStakingscore
            );
        }
    ...
    function distributeIndividualReward(uint256 _pid, uint256 _amount) private {
        ...
        for (uint256 i = 0; i < adds.length; i++) {
            ...
            uint256 sharePercentage =
                user.totalUserBaseMul.mul(10000);
            user.totalReward = user.totalReward.add(
                (_amount.mul(sharePercentage).div(poolBaseMul)).div(10000)
            );
        }
    }
        ...
        for (uint256 pid = 0; pid < poolInfo.length; ++pid) {
            ...
            for (uint256 i = 0; i < adds.length; i++) {
                ...
                uint256 sharePercentage =
                    user.totalUserBaseMul.mul(10000);
                user.totalReward = user.totalReward.add(
                    (_amount.mul(sharePercentage).div(allPoolBaseMul)).div(10000)
                );
            }
        }
    }
    ...
    */
    function distributeTvlAdjustedReward(uint256 _amount) private {
        ...
        for (uint256 pid = 0; pid < poolInfo.length; ++pid) {
            PoolInfo storage pool = poolInfo[pid];
            uint256 tvl = pool.lpToken.balanceOf(address(this));
            uint256 poolRewardShare = tvl.mul(10000);
            uint256 reward = (_amount.mul(poolRewardShare).div(totalTvl)).div(10000);
    ...
    */
    function updateCurBlockReward(
        ...
        uint256 sharePercentage = userBaseMul.mul(10000);
