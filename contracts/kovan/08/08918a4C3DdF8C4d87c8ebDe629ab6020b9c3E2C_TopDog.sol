...
contract TopDog is Ownable {
    ...
    // xTreat reward percent
    uint256 public treatPercent = 5;
    ...
    // Update Address
    function updateAddress(address _xShib, address _xLeash, address _tBone, address _xTreat) public onlyOwner {
        xShib = _xShib;
        xLeash = _xLeash;
        tBone = _tBone;
        xTreat = _xTreat;
    ...
    // Update Percent
    function updatePercent(uint256 _devPercent, uint256 _shibPercent, uint256 _leashPercent, uint256 _bonePercent, uint256 _treatPercent) public onlyOwner {
        massUpdatePools();
    ...
    function pendingBone(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 rewardPercent = devPercent.add(shibPercent).add(leashPercent).add(bonePercent).add(treatPercent);
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 treatReward = boneTotalReward.mul(treatPercent).div(1000);

        uint256 userReward = boneTotalReward.sub(devReward.add(shibReward).add(leashReward).add(boneReward).add(treatReward));
        
        bone.mint(devaddr, devReward);
        bone.mint(xShib, shibReward);
        bone.mint(xLeash, leashReward);
        bone.mint(tBone, boneReward);
        bone.mint(xTreat, treatReward);
