...
interface IApeBetBookMaker {
    ...
    function maxPoolReward() external view returns (uint256);
    ...
    function updateMaxPoolReward(uint256 amount) external;
...
contract ApeBetPool is Ownable {
    ...
    function changeBookMaker(address _bookMakerAddress) external onlyOwner {
        ...
        bookMaker.updateMaxPoolReward(address(this).balance);
    }

    receive() external payable {
        bookMaker.updateMaxPoolReward(msg.value);
        supplierDeposit = supplierDeposit.add(msg.value);
    }

    function claimReward(bytes16 _event_id, uint256 _affiliate_id) external {
        require(
            bookMaker.bets(_event_id, _affiliate_id, msg.sender).bet_reward !=
                0,
        ...
        bookMaker.removeReward(_event_id, _affiliate_id, msg.sender);
    }
    ...
    function depositBetToken() external {
        msg.sender.transfer(
            bookMaker
                .maxPoolReward()
        ...
        betToken.burn(msg.sender, betToken.balanceOf(msg.sender));
        deposits[msg.sender] = 0;
        supplierDeposit = supplierDeposit.sub(deposits[msg.sender]);
    }
}