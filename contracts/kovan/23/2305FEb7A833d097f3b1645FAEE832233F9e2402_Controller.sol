...
contract Controller is IController, Governable {
    ...
    function setConsumerAddress(address _address) external onlyGovernance {
    ...
    function onlyConsumerOrGovernance() private view {
        require((consumerContract != address(0) && msg.sender == consumerContract) || (store.isGovernance(msg.sender)), "C7");
    }
    ...
    function requestFutureWeights(address _vault) external onlyHardWorkerOrGovernance {
        require(_vault != address(0), "C10");
        require(vaults[_vault], "C11");
        require(!isRequestFutureVault[_vault], "C12");
        bytes memory data = abi.encode(_vault,IMainStrategy(IVault(_vault).strategy()).getStrategy());
        IApiConsumer(consumerContract).requestData(
            data,
