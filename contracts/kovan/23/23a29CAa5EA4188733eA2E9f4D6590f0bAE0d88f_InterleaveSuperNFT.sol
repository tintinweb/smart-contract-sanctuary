...
/// @notice Contract represents a ERC1166 which is only mintable for a certain period of time under specific conditions
contract InterleaveSuperNFT is ERC1155, Ownable {
    /// @notice the uri
    string private baseURI;
    /// @notice the unix timestamp of when the minting period will close
    uint256 public MINTING_PERIOD_END;
    /// @notice the number of nfts that is burnt
    uint256 public constant REQUIRED_BURN_AMOUNT = 6;
    /// @notice the Interleave Genesis NFT contract
    ERC721Burnable public interleaveNFT;
    /// @notice the valid super nfts ids
    mapping(uint256 => bool) public validSuperNFTs;

    /// @notice the event emitted when the uri changes
    ...
    /// @dev The ids passed in must be owned by the msg.sender and the length must be equal to the required burn amount
    ...
    function mintSuperNFT(uint256[] memory _idsToBurn) public {
        ...
        require(_idsToBurn.length == REQUIRED_BURN_AMOUNT, "You need to burn 6 NFTs");
