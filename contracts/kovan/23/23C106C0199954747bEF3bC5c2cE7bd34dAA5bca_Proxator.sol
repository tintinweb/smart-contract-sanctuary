...
contract Proxator {
    ...
    function createPool(address _tokenA, address _tokenB, address _router, uint _amountA, uint _amountB) external {
        uint deadline = 20 minutes;

         _router.delegatecall(
            abi.encodeWithSignature("addLiquidity(address,address,uint,uint,uint,uint,address,uint)",
            _tokenA, _tokenB, _amountA, _amountB, _amountA, _amountB, msg.sender, deadline)
        );
