...
contract Kyoko is Ownable, ERC721Holder {
    ...
    mapping(IERC721 => mapping(uint256 => NFT)) public NftMap; // Collaterals mapping

    mapping(IERC721 => mapping(uint256 => mapping(address => OFFER)))
        public OfferMap;
    ...
    // deposit collateral
    event Deposit(uint256 _nftTokenId, IERC721 _nftToken);

    // lend erc20
    event Lend(uint256 _nftTokenId, IERC721 _nftToken);

    // borrow erc20
    event Borrow(uint256 _nftTokenId, IERC721 _nftToken);
    // repay erc20
    event Repay(uint256 _nftTokenId, IERC721 _nftToken);

    // Claim collateral
    event ClaimCollateral(uint256 _nftTokenId, IERC721 _nftToken);

    // LToken claim erc20 token
    event ClaimERC20(uint256 _nftTokenId, IERC721 _nftToken);

    // execute emergency to warning borrower
    event ExecuteEmergency(uint256 _nftTokenId, IERC721 _nftToken);

    // Liquidate collateral
    event Liquidate(uint256 _nftTokenId, IERC721 _nftToken);

    event AddOffer(uint256 _nftTokenId, IERC721 _nftToken, address _maker);

    event CancelOffer(uint256 _nftTokenId, IERC721 _nftToken, address _maker);
    ...
    modifier checkCollateralStatus(uint256 _nftTokenId, IERC721 _nftToken) {
    ...
    function addOffer(
        ...
        OfferMap[_nftToken][_nftTokenId][msg.sender] = OFFER({
            apy: _apy,
            price: _price,
            period: _period,
        ...
        emit AddOffer(_nftTokenId, _nftToken, msg.sender);
    }

    function cancelOffer(IERC721 _nftToken, uint256 _nftTokenId)
        public
        isPause
    {
        ...
        _offer.accept = false;
        _offer.symbol = false;
        IERC20(_offer.erc20Token).safeTransfer(msg.sender, _offer.price);
        emit CancelOffer(_nftTokenId, _nftToken, msg.sender);
    }
    ...
    function acceptOffer(
        ...
        require(_nft.borrower == msg.sender && !_nft.marks.isBorrow);
        ...
        _nft.lender = _lender;
        IERC20(_offer.erc20Token).safeTransfer(
            address(msg.sender),
            _offer.price
        );
        emit AcceptOffer(_nftTokenId, _nftToken, msg.sender);
    }
    ...
    function lend(
        ...
        NFT storage _nft = NftMap[_nftToken][_nftTokenId];
        IERC20 _erc20Token = _nft.collateral.erc20Token;
        require(!_nft.marks.isBorrow, "This collateral already borrowed");
        // get lend amount
        ...
        emit Lend(_nftTokenId, _nftToken);
        // borrow action
        _borrow(_nftToken, _nftTokenId);
    }

    function _borrow(IERC721 _nftToken, uint256 _nftTokenId) internal {
        NFT storage _nft = NftMap[_nftToken][_nftTokenId];
        // change collateral status
        _nft.marks.isBorrow = true;
        _nft.borrowTimestamp = block.timestamp;
        // send erc20 token to collateral _nft.borrower
        IERC20(_nft.collateral.erc20Token).safeTransfer(
            address(_nft.borrower),
            _nft.collateral.price
        );
        emit Borrow(_nftTokenId, _nftToken);
    }
    ...
    function repay(
        ...
        emit Repay(_nftTokenId, _nftToken);
    }

    function claimCollateral(IERC721 _nftToken, uint256 _nftTokenId)
        public
        isPause
    {
        NFT storage _nft = NftMap[_nftToken][_nftTokenId];
        ...
        emit ClaimCollateral(_nftTokenId, _nftToken);
    }

    function claimERC20(IERC721 _nftToken, uint256 _nftTokenId) public isPause {
        ...
        emit ClaimERC20(_nftTokenId, _nftToken);
    }

    function executeEmergency(IERC721 _nftToken, uint256 _nftTokenId)
        public
        isPause
        checkCollateralStatus(_nftTokenId, _nftToken)
    {
        ...
        emit ExecuteEmergency(_nftTokenId, _nftToken);
    }

    function liquidate(IERC721 _nftToken, uint256 _nftTokenId)
        public
        isPause
        checkCollateralStatus(_nftTokenId, _nftToken)
    {
        ...
        emit Liquidate(_nftTokenId, _nftToken);
    }
    ...
    function _addMaker(address _sender) internal {
        uint256 len = OfferMaker.length;
        bool hasMaker = false;
        for (uint256 index = 0; index < len; index++) {
            if (_sender == OfferMaker[index]) {
                hasMaker = true;
                break;
            }
        }
        if (!hasMaker) OfferMaker.push(_sender);
    }
