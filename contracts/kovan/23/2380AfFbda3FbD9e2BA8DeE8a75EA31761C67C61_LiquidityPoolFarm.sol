...
contract LiquidityPoolFarm is Ownable {
    ...
    function deposit(uint256 _amount) public {
        ...
        updatePool();
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(accAdyPerShare).div(1e18).sub(
                user.rewardDebt
            );
            if (pending > 0) {
                // emit Test(accAdyPerShare, pending.mul(1e18), 0, 0, 0);
        ...
        user.rewardDebt = user.amount.mul(accAdyPerShare).div(1e18);
        // emit Test(accAdyPerShare, user.amount, user.rewardDebt, 0, 0);
    ...
    function updatePool() public {
        ...
        emit Test(0, adyReward, 0, 0, 0);
        uint256 currentReward = adyReward.mul(1e18);
        uint256 divided = currentReward.div(daiSupply);
        accAdyPerShare = accAdyPerShare.add(divided);
        emit Test(accAdyPerShare, adyReward, daiSupply, currentReward, divided);
    ...
    function withdraw(uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(accAdyPerShare).div(1e18);
        emit Withdraw(msg.sender, _amount.mul(1e18));
    }
    ...
    function pendingAdy(address _user) external view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && lpSupply != 0) {
            ...
            _accAdyPerShare = accAdyPerShare.add(
                adyReward.mul(1e18).div(lpSupply)
            );
        }
        return user.amount.mul(_accAdyPerShare).div(1e18).sub(user.rewardDebt);
    }

    // Safe cake transfer function, just in case if rounding error causes pool to not have enough ADYs.
    function safeAdyTransfer(address _to, uint256 _amount) internal {
...
interface IAuditoryToken is IERC20 {
