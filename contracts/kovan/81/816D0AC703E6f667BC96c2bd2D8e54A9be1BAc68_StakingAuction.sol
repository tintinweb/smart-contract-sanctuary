...
contract StakingAuction is VersionedInitializable, AuctionBase, ReentrancyGuard {
     ...
     */
    function getConfiguration() external view returns (DataTypes.StakingAuctionConfiguration memory) {
        return
    ...
    function _bid(
        ...
        if (_overtimeWindow > 0 && block.timestamp > endTimestamp - _overtimeWindow) {
            auction.endTimestamp = endTimestamp + _overtimeWindow;
        }
        ...
        emit BidSubmitted(nft, nftId, _nftData[nft][nftId].auctionId, onBehalfOf, spender, amount);
    }
 ...
 *
 * @dev A simple implementation that holds basic auction parameter functionality, common to both staking and
...
abstract contract AuctionBase is AdminPausableUpgradeSafe {
    ...
    event TreasuryFeeChanged(uint16 newTreasuryFee);
