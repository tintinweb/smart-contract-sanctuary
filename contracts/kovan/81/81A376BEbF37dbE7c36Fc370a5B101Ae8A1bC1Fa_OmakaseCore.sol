...
contract OmakaseCore is KeeperCompatibleInterface {
    ...
    event PolicyFlightAdded(bytes32 indexed policyId, bytes32 indexed flightId);
    event PolicyFlightRemoved(
    ...
    function addAdditionalPayout(
        ...
        if (benefitName == BenefitName.FlightDelay) {
            ...
            revert(Errors.OC_BENEFIT_CANT_ADD_ADDITIONAL_PAYOUT);
        }
        ...
        require(
            ...
            Errors.OC_INVALID_ADDITIONAL_PAYOUT_AMOUNT
        );
    ...
    function setPolicyFlights(
        ...
        require(!policy.hasFlight, Errors.OC_POLICY_HAS_FLIGHT_ALREADY);

        for (uint256 i = 0; i < flightData.length; i++) {
            addPolicyFlight(policyId, flightData[i]);
        }

        policy.hasFlight = true;
    }

    function updatePolicyFlights(
        bytes32 policyId,
        bytes32[] calldata prevFlightIds,
        FlightArgs[] calldata newFlightsData
    ) external onlyOracle {
        for (uint256 i = 0; i < prevFlightIds.length; i++) {
            updatePolicyFlight(policyId, prevFlightIds[i], newFlightsData[i]);
        }
    }

    function updatePolicyFlight(
        bytes32 policyId,
        bytes32 prevFlightId,
        FlightArgs calldata newFlightData
    ) private {
        Policy storage policy = s.policies[policyId];
        Flight storage prevFlight = s.flights[prevFlightId];
        uint256 idxInprevFlight = policy.idxInFlight[prevFlightId];

        require(!policy.isEmpty(), Errors.OC_POLICY_EMPTY);
        require(
            prevFlight.policies[idxInprevFlight] == policyId,
            Errors.OC_INVALID_PREVIOUS_FLIGHT
        );
        require(
            policy.flightBenefitStatuses[prevFlightId] == BenefitStatus.Active,
            Errors.OC_POLICY_FLIGHT_IS_ALREADY_IN_PROGRESS
        );

        prevFlight.removePolicy(idxInprevFlight);

        delete policy.idxInFlight[prevFlightId];
        emit PolicyFlightRemoved(policyId, prevFlightId);

        addPolicyFlight(policyId, newFlightData);
    }
    ...
    function completeClaim(
        ...
        if (
            ...
            policy.flightBenefitStatuses[flightId] = BenefitStatus.Expired;
        } else {
            policy.flightBenefitStatuses[flightId] = BenefitStatus.Claimed;
        }
    ...
    function _evaluateFlightDeparture(
        ...
        for (uint256 i = 0; i < flight.policies.length; i++) {
            ...
            if (
                policy.flightBenefitStatuses[flightId] !=
            ...
            policy.flightBenefitStatuses[flightId] = BenefitStatus
                .ClaimInProgress;
    ...
    function addPolicyFlight(bytes32 policyId, FlightArgs calldata flightInput)
        private
    {
        ...
        s.flightQueue.addIfNotExists(flightInput.id, flightInput.arrivalTime);

        s.policies[policyId].idxInFlight[flightInput.id] = flight.addPolicy(
            policyId
        );
        emit PolicyFlightAdded(policyId, flightInput.id);
    }
    ...
    {
        return s.policies[policyId].flightBenefitStatuses[flightId];
    }
...
library Errors {
    ...
    string constant OC_INVALID_ADDITIONAL_PAYOUT_AMOUNT =
        "OmakaseCore: Invalid additional payout amount";
    string constant OC_BENEFIT_CANT_ADD_ADDITIONAL_PAYOUT =
        "OmakaseCore: Benefit cannot add additional payout";
    string constant OC_POLICY_HAS_FLIGHT_ALREADY =
        "OmakaseCore: Policy has flight already";
    string constant OC_INVALID_PREVIOUS_FLIGHT =
        "OmakaseCore: Invalid previous flight";
    string constant OC_POLICY_FLIGHT_IS_ALREADY_IN_PROGRESS =
        "OmakaseCore: Policy flight is already in progress";
...
struct Policy {
    ...
    mapping(bytes32 => BenefitStatus) flightBenefitStatuses;
