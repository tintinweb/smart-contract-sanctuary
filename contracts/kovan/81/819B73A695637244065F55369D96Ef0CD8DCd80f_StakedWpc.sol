...
contract StakedToken is IStakedWpc, ERC20Snapshot, DistributionManager, VersionedInitializable {
    ...
    constructor(
        ...
        _wpcGovernance = ITransferHook(wpcGovernance);
        _decimals = decimals;
    }
    ...
    function setWpcGovernance(address wpcGovernance) external {
        require(msg.sender == EMISSION_MANAGER, 'ONLY_EMISSION_MANAGER');

        address old = address(_wpcGovernance);
        _wpcGovernance = ITransferHook(wpcGovernance);
        emit WpcGovernanceChanged(old, wpcGovernance);
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (address(_wpcGovernance) != address(0)) {
            _wpcGovernance.onTransfer(from, to, amount);
        }
    }
}
