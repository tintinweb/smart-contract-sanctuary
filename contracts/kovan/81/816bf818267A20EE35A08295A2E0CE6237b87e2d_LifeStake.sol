...
contract LifeStake {
    ...
    function initialize(
        ...
        StakeI.Supply =  maxStake.mul(10**TOKEN.decimals());
        StakeI.Remaining = StakeI.Supply;
        StakeI.StakePercent = 800;
        StakeI.StakePeriod = 30 minutes;

        StakeII.Supply = maxStake.mul(10**TOKEN.decimals()) ;
        StakeII.Remaining = StakeII.Supply;
        StakeII.StakePercent = 900;
        StakeII.StakePeriod = 180 minutes;

        StakeIII.Supply = maxStake.mul(10**TOKEN.decimals()) ;
        StakeIII.Remaining = StakeIII.Supply;
        StakeIII.StakePercent = 1000;
    ...
    function deposit(address rewardAddress,uint256 _depositId, uint256 _amount) public validDepositId(_depositId){
        ...
        require(StakeI.Sold.add(StakeI.Sold).add(StakeI.Sold) <= maxStake,"MaxStake limit reached");
        if(_depositId == 1)
        {
        StakeI.Remaining = StakeI.Remaining.sub(_amount);
        StakeI.Sold = StakeI.Sold.add(_amount);
        PlanI[rewardAddress].TokenPerDay = PlanI[rewardAddress].TokenPerDay.add((_amount.mul(StakeI.StakePercent).div(percentDivider)));
        ...
        {
        StakeII.Remaining = StakeII.Remaining.sub(_amount);
        StakeII.Sold = StakeII.Sold.add(_amount);
        PlanII[rewardAddress].TokenPerDay = PlanII[rewardAddress].TokenPerDay.add((_amount.mul(StakeII.StakePercent).div(percentDivider)));
        ...
        {
        StakeIII.Remaining = StakeIII.Remaining.sub(_amount);
        StakeIII.Sold = StakeIII.Sold.add(_amount);
        PlanIII[rewardAddress].TokenPerDay = PlanIII[rewardAddress].TokenPerDay.add((_amount.mul(StakeIII.StakePercent).div(percentDivider)));
    ...
    function _withdraw(address _user,uint256 _depositId) internal validDepositId(_depositId) {
        if(PlanI[_user].TokenPerDay > 0  && _depositId == 1)
        ...
        if(PlanII[_user].TokenPerDay > 0 && _depositId == 2)
        ...
        if(PlanIII[_user].TokenPerDay > 0 && _depositId == 3)
        {
