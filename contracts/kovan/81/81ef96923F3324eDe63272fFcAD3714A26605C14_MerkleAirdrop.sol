...
contract MerkleAirdrop is Ownable, ReentrancyGuard {
    ...
    function addRoot(bytes32 _merkleRoot, uint256 _height) external onlyOwner {
        merkleRoots.push(_merkleRoot);
        merkleHeights.push(_height);
    }
        ...
        returns (bool)
    {
        ...
        delete merkleRoots;
        delete merkleHeights;
        selfdestruct(payable(_owner));
        return true;
    }

    function claimTokens(IERC20 _token, uint256 amount) external onlyOwner nonReentrant {
        require(
            address(_token) != address(0),
            "MerkleAirdrop: Wrong token"
        );
        uint256 tokenBalance = _token.balanceOf(address(this));
        require(
            tokenBalance > 0,
            "MerkleAirdrop: Token balane equals zero"
        );
        require(
            amount < tokenBalance,
            "MerkleAirdrop: Wrong amount"
        );
    ...
    function getTokensByMerkleProof(
        ...
        require(
            _proof.length == merkleHeights[rootInd],
            "MerkleAirdrop: Wrong proof length"
        );
    ...
    function _uintToStr(uint256 i) internal pure returns (string memory) {
        if (i == 0) return "0";
        uint256 j = i;
        uint256 length;
        while (j != 0) {
        ...
        returns (bytes32)
    {
        ...
        bytes memory _bb = bytes(_addressToAsciiString(_a));
        bytes memory _bc = bytes(space);
        bytes memory _bd = bytes(_uintToStr(_n));
        string memory abcde =
