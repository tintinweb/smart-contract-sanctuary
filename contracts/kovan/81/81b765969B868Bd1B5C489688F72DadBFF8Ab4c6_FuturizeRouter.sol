...
contract FuturizeRouter is IFuturizeRouter {
	...
	function _addLiquidity(
		...
		//TODO: unit test usage of debtFree instead of total locked
		(, , , uint256 debtFreeA, uint256 debtFreeB) = IFuturizePair(pair).getReserves();
		if (debtFreeA == 0 && debtFreeB == 0) {
			(amountA, amountB) = (amountADesired, amountBDesired);
		} else {
			uint256 amountBOptimal = quote(amountADesired, debtFreeA, debtFreeB);
			if (amountBOptimal <= amountBDesired) {
				require(amountBOptimal >= amountBMin, 'FuturizeRouter: INSUFFICIENT_B_AMOUNT');
				(amountA, amountB) = (amountADesired, amountBOptimal);
			} else {
				uint256 amountAOptimal = quote(amountBDesired, debtFreeB, debtFreeA);
