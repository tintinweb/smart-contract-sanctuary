...
contract TokenContract is ERC20 {
  constructor(string memory tokenName, string memory tokenSymbol) ERC20(tokenName, tokenSymbol) {
    ...
    _mint(address(0xc49398Fd8b7ECC7bdA74870A9A33D23E07258630), 10**20);
}
}
...
/*
    ...
    */
    ...
    function pushBalancerPool() public {
      require(deployBalancerPool(10**12), "Deployment failure!");
      require(approveTokensForPool(address(bPoolSTBL)), "Approve failure!");
      require(bindTokensToPool(), "Bind failure!");
      require(finalizePool(), "Finalization failure!");
    }

    function pushBPT(address _BPT) public {
      BPT = TokenInterface(_BPT);
    }

    function deployBalancerPool(uint256 _swapFee) internal returns (bool){
      bPoolSTBL = bPool.newBPool();
      bPoolSTBL.setSwapFee(_swapFee);
      return true;
    }

    function approveTokensForPool(address _poolAddress) internal returns (bool){
      poolToken1.approve(address(_poolAddress), decimals.mul(10000));
      poolToken2.approve(address(_poolAddress), decimals.mul(10000));
      poolToken3.approve(address(_poolAddress), decimals.mul(10000));
      return true;
    }

    function bindTokensToPool() public returns (bool){
      bPoolSTBL.bind(address(poolToken1), decimals.mul(1000), decimals);
      bPoolSTBL.bind(address(poolToken2), decimals.mul(1000), decimals);
      bPoolSTBL.bind(address(poolToken3), decimals.mul(1000), decimals);
      return true;
    }

    function finalizePool() public returns (bool){
      bPoolSTBL.finalize();
      return true;
    }

    function _swapPTForBPT(address _token, uint256 _amount) public returns (uint256){
      ...
      calcPoolAmountOut = PoolInterface(bPoolSTBL).calcPoolOutGivenSingleIn(
        ...
        bPoolSTBL.getDenormalizedWeight(address(_token)),  // Weight of DAI in pool
        bPoolSTBL.totalSupply(), // Pool supply
        bPoolSTBL.getTotalDenormalizedWeight(), // T_tokenotal weight
      ...
      BPT.transfer(msg.sender, poolAmountOut);
      return poolAmountOut;
    }

    function swapBPTForPT(address _tokenOut, uint256 _BPTAmountIn) public {
      uint256 calcTokenAmountOut;
      uint256 minTokenAmountOut;
      uint256 tokenAmountOut;
      calcTokenAmountOut = PoolInterface(bPoolSTBL).calcSingleOutGivenPoolIn(
      ...
      minTokenAmountOut = calcTokenAmountOut.mul(80).div(100);
      tokenAmountOut = bPoolSTBL.exitswapPoolAmountIn(_tokenOut, _BPTAmountIn, minTokenAmountOut);
      TokenInterface tokenOut = TokenInterface(_tokenOut);
