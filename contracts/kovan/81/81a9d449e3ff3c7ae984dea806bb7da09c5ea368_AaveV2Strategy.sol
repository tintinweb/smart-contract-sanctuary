...
contract AaveV2Strategy is BaseRhoStrategy {
    ...
    function bonusRatePerBlock() external pure override returns (uint256) {
        return 0;
    }

    function bonusSupplyRate() external pure override returns (uint256) {
        return 0;
    }
    ...
    function updateBalanceOfUnderlying() external view override returns (uint256) {
    ...
    function withdrawUnderlyingInternal(uint256 _amount) internal override {
        ...
        ILendingPool lendingPool = ILendingPool(lpAddrProvider.getLendingPool());
        // `onBehalfOf` = _msgSender() to send underlying to the vault
        lendingPool.withdraw(address(underlying), _amount, _msgSender());
    }

    function withdrawAllInternal() internal override {
        withdrawUnderlyingInternal(type(uint256).max);
    }

    function collectRewardToken() external override onlyRole(VAULT_ROLE) whenNotPaused nonReentrant {}
    function shouldCollectReward(uint256) external override pure returns(bool){return false;}
    ...
    ReentrancyGuardUpgradeable
{
     ...
     */
    function withdrawAllInternal() internal virtual;

    function withdrawAll() external override onlyRole(VAULT_ROLE) whenNotPaused nonReentrant {
        withdrawAllInternal();
        emit WithdrawAll();
    }
