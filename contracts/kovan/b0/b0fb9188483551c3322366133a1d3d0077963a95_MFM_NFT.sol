...
contract ERC721MetadataMintable is ERC721, ERC721Metadata, MinterRole {
    ...
    mapping(uint256 => address payable) internal _tokenAuthors;
...
contract MFM_NFT is ERC721Full, ERC721MetadataMintable, Ownable {
  ...
  function setAddresses(address token_addr, address payable _platform, address payable _authorVault) onlyOwner public {
  ...
  function MintAndSellNFTBNB(address to, string memory tokenURI,uint256 quantity, bool flag, uint256 price, string memory ownerId) public {
   uint256 tokenId;
  ...
  function MintAndAuctionNFT(address to, string memory tokenURI,uint256 quantity, bool flag, address _contract, uint256 _minPrice, uint256 _endTime) public {
      uint256 _tokenId;
  ...
  function claimAuction(uint256 _tokenId) public {
    ...
    wrap.withdraw((_auctionTokens[_tokenId].bidAmount / 1000) * authorPercentage);
  ...
  function listNftAdmin(address payable owner,uint256 tokenId,uint256 price) public onlyAdminMinter {
  ...
  function buyNFT(address nft_a,uint256 tokenId, address buyer) public payable {
        ...
        //author fee
        initialBalance = address(this).balance;
        wrap.withdraw((_saleTokens[tokenId].price / 1000) * authorPercentage);
    	newBalance = address(this).balance.sub(initialBalance);
    	address payable author = address(uint160(_tokenAuthors[tokenId]));
        author.transfer(newBalance);
