    ...
    ReentrancyGuard
{
        ...
        uint256 available =
            getFreeCollateral(msg.sender, token).add(
                unitPrice
                    .mul(tokenAmount)
                    .div(1 ether)
                    .mul(getVaultRatio(msg.sender, token).sub(minMintDeviation))
                    .div(100)
            );
    }
    ...
    {
        uint256[] memory _ratios = new uint256[](collateralTokens.length);
        for (uint256 i = 0; i < _ratios.length; i++) {
            _ratios[i] = collateralDetails[collateralTokens[i]].mintCR;
        }
        ratios = _ratios;
    }
        ...
        returns (uint256 free)
    {
        ...
        for (uint256 i = 0; i < collateralTokens.length; i++) {
            ...
            if (balance > 0 && collateralTokens[i] != WETH) {
                ...
                depositTotal = depositTotal.add(balance);
                ratio = ratio.add(
                    collateralDetails[collateralTokens[i]].mintCR
                );
                count = count + 1;
        ...
        if (locked >= depositTotal) {
            free = 0;
        } else {
            free = depositTotal.sub(locked);
        }
    }

    function getVaultRatio(address account, address fxToken)
        public
        view
        override
...
contract Treasury is ITreasury, AccessControl, ReentrancyGuard, IValidator {
        ...
        notLiquidatingVault(user, fxToken)
    {
        ...
        if (interestLastUpdateDate[user][fxToken] == block.timestamp)
            return;
