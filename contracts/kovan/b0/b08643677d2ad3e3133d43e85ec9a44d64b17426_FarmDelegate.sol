...
contract FarmDelegate is FarmStorageV1 {
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (number > usdtStartBlock) {
            uint256 subtractor = pool.lastRewardBlock > usdtEndBlock
                ? usdtEndBlock
                : pool.lastRewardBlock;

            uint256 multiplier = number.sub(subtractor);
        ...
        returns (uint256)
    {
        ...
        lastAccLockGanPerShare = lastAccLockGanPerShare.add(
    ...
    function newReward(uint256 _usdtAmount, uint256 _newPerBlock) public {
    ...
    function reDeposit(uint256 _pid) public payable {
        ...
        if (pendingAmount > amountA)
            safeGanTransfer(msg.sender, pendingAmount.sub(amountA));
