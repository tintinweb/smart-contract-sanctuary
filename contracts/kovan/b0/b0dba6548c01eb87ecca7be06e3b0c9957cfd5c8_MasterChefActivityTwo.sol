...
// NTSToken with Governance.
contract NTSToken is ERC20("NTSToken", "NTS"), Ownable {
    // cross chain
    ...
    function mint(address _to, uint256 _amount) public {
        require(minters[msg.sender] == true, "NTS : You are not the miner");
        _mint(_to, _amount);
    }
...
// File: interfaces/IMasterchefStageOne.sol

pragma solidity 0.6.12;



interface IMasterChefStageOne{

    function initialize(IERC20 _NTS,uint256 _startBlock, address _token) external;

    function addWETHTokenPair(IUniswapV2Pair _lpTokenSwap) external;

    function add(IUniswapV2Pair _lpTokenSwap, IUniswapV2Pair _tokenToEthSwap) external;

    function deposit(uint256 _pid, address sender, uint256 _amount) external;

    function withdraw(uint256 _pid, address _sender) external;

    function setDeveloperFund(address _devAddress, uint256 _userDividendWeight, uint256 _devDividendWeight) external;

    function massUpdatePools() external;

    function updatePoolDividend(uint256 _pid) external;

    function pendingNTS(uint256 _pid, address _user) external view returns (uint256, uint256);

    function getDepositWeight(uint256 _lockAmount) external view returns(uint256 _currentWeight, uint256 _nextWeight);

    function getPoolLength() external view returns(uint256);

    function getPoolInfo(uint256 _pid) external view 
    returns(uint256 _accumulativeDividend, uint256 _usersTotalWeight, uint256 _lpTokenAmount, uint256 _oracleWeight
        , address _swapAddress, address _relatedSwapAddress);

    function getPagePoolInfo(uint256 _fromIndex, uint256 _toIndex) external view 
    returns(uint256[] memory _accumulativeDividend, uint256[] memory _usersTotalWeight, uint256[] memory _lpTokenAmount, 
    uint256[] memory _oracleWeight, address[] memory _swapAddress);

    function getTermInfo() external view returns(uint256 _currentBlockHeight, uint256 _nextTerm, uint256 _leftBlockToNextTerm);

    function getUserInfo(uint256 _pid, address _sender) external view returns(uint256 _amount, uint256 _originWeight, uint256 _modifiedWeight, uint256 _endBlock);
}
...
contract ActivityBase{
    ...
    // The block number when NTS mining starts.
...
contract MasterChefActivityTwo is Ownable, IInvitation, ActivityBase {
    ...
    // Bonus muliplier for early NTS makers.
    uint256 public constant INITIAL_BONUS_PER_BLOCK = 11052 * (1e14);
    // after each term, mine half NTS token
    ...
    // stageOne address
    IMasterChefStageOne public stageOne;
    ...
    // undividend nts
    uint256 public unDividendNts;

    // 20% nts => NTS - ETH pool
    uint256 public ntsPoolDividendWeight = 2;
    // 80% nts => NTS - ETH pool
    ...
    function dealEvilPoolDiviend(uint256 _pid) private {
        ...
        if (currentRankCount > 0) {
            ...
            unDividendNts = unDividendNts.add(undistributeDividend);
        }
    ...
    function setNtsPoolDividendWeight(
        uint256 _ntsPoolWeight,
        uint256 _otherPoolWeight
    ) public virtual onlyOwner {
        require(
            _ntsPoolWeight != 0 && _otherPoolWeight != 0,
            "invalid input"
        );
        massUpdatePools();
        ntsPoolDividendWeight = _ntsPoolWeight;
        otherPoolDividendWeight = _otherPoolWeight;
    }

    function massUpdatePools() private {
        uint256 poolCountInRank = rankPoolIndex.length;
        uint256 farmMintNts = mintNTS(address(this), block.number);
        updateNTSPoolAccumulativeDividend(block.number);
        if(poolCountInRank == 0){
            farmMintNts = farmMintNts.mul(otherPoolDividendWeight)
                                     .div(ntsPoolDividendWeight.add(otherPoolDividendWeight));
            if(farmMintNts > 0){
                unDividendNts = unDividendNts.add(farmMintNts);
            }
        }
    ...
    function mintNTS(address _address, uint256 _toBlock) private returns (uint256){
    ...
    function updateNTSPoolAccumulativeDividend (uint256 _toBlock) private{
        PoolInfo storage pool = poolInfo[0];
        if (pool.lastDividendHeight >= _toBlock) return;
        uint256 poolReward =
            getModifiedRewardToken(pool.lastDividendHeight.add(1), _toBlock);

        uint256 ntsPoolDividend = poolReward.mul(ntsPoolDividendWeight)
                                               .div(ntsPoolDividendWeight.add(otherPoolDividendWeight));                              
        pool.lastDividendHeight = _toBlock;
        uint256 existedDividend = pool.accumulativeDividend;
        pool.accumulativeDividend = existedDividend.add(ntsPoolDividend);
    }

    // deposit LP tokens to MasterChef for NTS allocation.
    ...
    function addToRank(
        ...
        if(unDividendNts > 0){
            _pool.accumulativeDividend = _pool.accumulativeDividend.add(unDividendNts);
            unDividendNts = 0;
        }
    ...
    function pendingNTS(uint256 _pid, address _user)
        external
        view
        virtual
        returns (uint256 _pending, uint256 _potential)
    {
        ...
        _potential = _potential    
            ...
            .div(totalWithdrawWeight);
        uint256 numerator;
        uint256 denominator = otherPoolDividendWeight.add(ntsPoolDividendWeight);
        if(_pid == 0){
            numerator = ntsPoolDividendWeight;
        }
        else{
