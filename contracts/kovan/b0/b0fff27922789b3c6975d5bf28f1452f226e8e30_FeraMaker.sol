...
contract FeraMaker {
    ...
    constructor(IUniswapV2Factory _factory, address _bar, address _fera, address _weth) public {
        factory = _factory;
        fera = _fera;
        bar = _bar;
        weth = _weth;
    ...
    function _toWETH(address token) internal returns (uint256) {
        ...
        if (token == weth) {
            uint amount = IERC20(token).balanceOf(address(this));
            _safeTransfer(token, factory.getPair(weth, fera), amount);
            return amount;
        }
        ...
        pair.swap(amount0Out, amount1Out, factory.getPair(weth, fera), new bytes(0));
        return amountOut;
    }

    function _toFERA(uint256 amountIn) internal {
        IUniswapV2Pair pair = IUniswapV2Pair(factory.getPair(weth, fera));
        (uint reserve0, uint reserve1,) = pair.getReserves();
        address token0 = pair.token0();
