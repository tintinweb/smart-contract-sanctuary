...
// File: StrategyAllOnly.sol

contract StrategyAllOnly is Ownable, ReentrancyGuard, IStrategy {
    ...
    /// @dev Execute worker strategy. Take LP tokens + ETH. Return LP tokens + ETH.
    /// @param data Extra calldata information passed along to this strategy.
    function execute(address /* user */, uint256 /* amount */, uint256 /* debt */, bytes calldata data)
        external
        nonReentrant
    {
        ...
        uint256 rIn = lpToken.token0() == token ? r0 : r1;
        ...
        (,, uint256 moreLPAmount) = router.addLiquidity(token, fToken,
            token.myBalance(), fToken.myBalance(), 0, 0, address(this), now
        );
