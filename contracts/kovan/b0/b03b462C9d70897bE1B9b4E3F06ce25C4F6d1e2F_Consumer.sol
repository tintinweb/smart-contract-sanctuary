...
contract Consumer is ChainlinkClient {
  ...
  mapping (bytes32 => address) internal requestIdToCaller;
  ...
  function validateName(string memory _name, address _callbackContract, bytes4 _selector) public returns (bytes32 requestId_) {
    string memory queryString = string(abi.encodePacked("name=", _name));
    Chainlink.Request memory request = buildChainlinkRequest(jobId, _callbackContract, _selector);
    request.add("get", "https://8ggiug5d6d.execute-api.us-east-1.amazonaws.com/dev/validate");
    request.add("queryParams", queryString);
