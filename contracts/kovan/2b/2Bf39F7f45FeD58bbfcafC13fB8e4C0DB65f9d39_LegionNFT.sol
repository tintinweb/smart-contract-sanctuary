...
contract LegionNFT is Context, IERC721, Ownable {
    ...
    // function execute(uint256 _tokenId, bool isBeast) external {
    //     uint256 mintingPrice = 0;
    //     if(isBeast) {
    //         require(msg.sender==beast.ownerOf(_tokenId), "Execute : you should own this beast");
    //         mintingPrice = beast.getMintingPrice();
    //     } else {
    //         require(msg.sender==warrior.ownerOf(_tokenId), "Execute : you should own this warrior");
    //         mintingPrice = warrior.getMintingPrice();
    //     }
    //     rewardpool.returnTokenToPlayer(msg.sender, mintingPrice.mul(10**18).mul(20).div(100));
    //     //isBeast ? beast.burn(_tokenId) : warrior.burn(_tokenId);
    // }
...
contract BeastNFT is Context, IBeastNFT, Ownable {
    ...
    function sendBackFromMarketplace(uint256 tokenId) external {
        require(ownerOf(tokenId)==msg.sender, "You should own this beast first");
        require(tokenData[tokenId].onMarket==true, "This beast is not on Marketplace");
        tokenData[tokenId].onMarket = false;
    }
...
contract WarriorNFT is Context, IWarriorNFT, Ownable {
    ...
    function sendBackFromMarketplace(uint256 tokenId) external {
        require(ownerOf(tokenId)==msg.sender, "You should own this warrrior first");
        require(tokenData[tokenId].onMarket==true, "This warrior is not on Marketplace");
        tokenData[tokenId].onMarket = false;
    }
