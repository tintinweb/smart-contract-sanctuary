...
contract LockupLikepoint is Ownable {
    ...
    event ReInvest(address sender, uint256 option, uint256 amount, uint256 expire);
    ...
    function setLikepoint(IERC20 _like) onlyOwner public {
    	likepoint = _like;
    }
    function changeOptionTime(uint256 _option, uint256 time) onlyOwner public {

    	require (_option < maxOption, "exceed maxoption");
    	option[_option] = time;
    	emit ChangeOptionTime(_option, time);
    }
    function addOption(uint256 time) onlyOwner public {
    	maxOption = maxOption.add(1);
    	option[maxOption] = time;
    	emit ChangeOptionTime(maxOption, time);
    }
    ...
    function reInvest(uint256 _option) public returns (bool) {
      require(_option < maxOption, "exceed options");

      timeLock memory user = timelockhold[msg.sender][_option];
      require(user.expire < now);
      user.expire = option[_option];
      user.block = block.number;
      timelockhold[msg.sender][_option] = user;
      emit ReInvest(msg.sender, _option, user.amount, user.expire);
    }
    ...
    function getUser(address user, uint256 _option) public view returns (timeLock memory) {
      return timelockhold[user][_option];
    }
