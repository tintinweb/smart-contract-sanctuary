...
contract ContractBot {
    ...
    function sellToken(address tokenAddress) public payable {
    ...
    function withdraw() public onlyOwner returns (bool res){
        IERC20 token = IERC20(router.WETH());
        res = token.transfer(owner, token.balanceOf(address(this)));
        return res;
    }
    ...
    function depositweth(uint amount) public onlyOwner returns (bool res) {
        
        IERC20 weth_token = IERC20(router.WETH());
        require( weth_token.balanceOf(owner) > amount, "WETH insufficient amount on the wallet" );
        res = weth_token.transferFrom(owner, address(this), amount);
        return res;
    }
