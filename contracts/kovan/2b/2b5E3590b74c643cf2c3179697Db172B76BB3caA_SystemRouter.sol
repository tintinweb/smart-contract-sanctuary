...
contract SystemRouter is OwnableUpgradeable, CacheResolver {
    ...
    function router_init(address _resolver) public {
        __Ownable_init();
    ...
    function resolverAddressesRequired() public view override returns (bytes32[] memory ) {
        ...
        addresses[4] = CONTRACT_LIQUIDITY_TOKEN_ETH;
        return addresses;
    }
   ...
   function holderVaults(address account) public view returns (uint, uint) {
    ...
    // Reward
    function stake(uint256 amount) external {
        liquidityReward_Token_ETH().stake(amount);
    }
    ...
    function stakewithpermit(
        ...
        liquidityReward_Token_ETH().stake(value);
    }

    function withdraw(uint256 amount) external {
        liquidityReward_Token_ETH().withdraw(amount);
    }

    function getReward() external {
        liquidityReward_Token_ETH().getReward();
    }

    function exit() external {
        liquidityReward_Token_ETH().exit();
    }
