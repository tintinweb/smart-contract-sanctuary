...
import './token/bases/staking/StakedToken.sol';
import './token/bases/staking/lib/DistributionTypes.sol';
import './utils/SafeMathUint128.sol';
import "hardhat/console.sol";

import '@openzeppelin/contracts-upgradeable/math/SafeMathUpgradeable.sol';

contract OriginatorStaking is StakedToken {
    ...
    struct Proposer {
        uint256 currentAmount; // proposer's current deposited amount
        uint256 objective;  // proposer's objective amount to be fulfilled
    }
    mapping(bytes32 => Proposer) public proposers;

    event InitialBalanceDeposited(address indexed contributor, uint256 amount);
    event StateChange(uint256 state);
    event Contribution(address indexed contributor, uint256 amount);
    event Withdraw(address indexed beneficiary, uint256 amount);
    event StartRewardsProjectFunded(uint256 previousEmissionsPerSecond, uint256 extraEmissionsPerSecond); 
    event EndRewardsProjectFunded(uint256 restoredEmissionsPerSecond, uint256 extraEmissionsPerSecond); 
    ...
    function initialize(
        ...
        state = OriginatorStakingState.PROPOSAL;
    }
    ...
    modifier onProposal() {
        require(state == OriginatorStakingState.PROPOSAL, 'ONLY_ON_PROPOSAL_STATE');
        _;
    }
    /**
     ...
     * @param _proposalRate uint256 (value * 100 e.g. 0.54 DAI/ETHIX == 54)
     * @param _collateralizationPercentGoal uint256 (value * 100 e.g. 150% == 15000)
     * @param _referenceGoal uint256 wei amount in exchange currency par (e.g 100k DAI)
     */
    function setUpProposal(address _auditor, address _originator, uint256 _auditorPercentage, uint256 _originatorPercentage, uint256 _proposalRate, uint256 _collateralizationPercentGoal, uint256 _referenceGoal) external onlyEmissionManager onProposal{
        require(_referenceGoal != 0, 'INVALID_ZERO_AMOUNT');
        require(_auditor != _originator, 'AUDITOR_AND_ORIGINATOR_CANNOT_BE_THE_SAME');
        ...
        uint256 collateralizationGoal = calculateCollateralizationGoal(_collateralizationPercentGoal, _referenceGoal, _proposalRate);
        proposers[AUDITOR_ROLE].objective = calculatePercentage(collateralizationGoal, _auditorPercentage);
        proposers[ORIGINATOR_ROLE].objective = calculatePercentage(collateralizationGoal, _originatorPercentage);
    }
    /**
     * @notice Function to participate (originator or auditor)
     * in proposal period.
     * If cap is reached, state change to staking period
     */
    function deposit(uint256 _amount) external payable onProposal{
        ...
        if (msg.sender == getRoleMember(ORIGINATOR_ROLE, 0)) {
            ...
            revert('DEPOSIT_PERMISSION_DENIED');
        }

        require(proposers[senderRole].currentAmount < proposers[senderRole].objective, 'CAN_NOT_DEPOSIT_OBJECTIVE_FULFILLED');

        uint256 depositAmount = calculateDepositAmount(senderRole, _amount);
        proposers[senderRole].currentAmount = proposers[senderRole].currentAmount.add(depositAmount);
        IERC20Upgradeable(STAKED_TOKEN).safeTransferFrom(msg.sender, address(this), depositAmount);
        emit Contribution(msg.sender, depositAmount);

        if (isInitialBalanceDeposited()) {
            uint256 totalObjective = proposers[ORIGINATOR_ROLE].objective.add(proposers[AUDITOR_ROLE].objective);
            changeState(OriginatorStakingState.STAKING);
            emit InitialBalanceDeposited(msg.sender, totalObjective);
        }
    }

    /**
     * @notice Calculate the correct amount to deposit
     * @param _role auditor or originator
     * @param _amount uint256 amount from deposit method
     */
    function calculateDepositAmount(bytes32 _role, uint256 _amount) internal returns (uint256) {
        uint256 depositAmount = 0;
        if (proposers[_role].objective.sub(proposers[_role].currentAmount) >= _amount) {
            depositAmount = _amount;
        } else {
            depositAmount = proposers[_role].objective.sub(proposers[_role].currentAmount);
        }
        return depositAmount;
    }

    /**
     * @notice Check if both roles have fulfilled the objective
     */
    function isInitialBalanceDeposited() internal returns (bool) {
        bool objectiveFulfilled = false;
        if ((proposers[ORIGINATOR_ROLE].objective.sub(proposers[ORIGINATOR_ROLE].currentAmount) == 0) && 
        (proposers[AUDITOR_ROLE].objective.sub(proposers[AUDITOR_ROLE].currentAmount) == 0)) {
     ...
     **/
    function changeState(OriginatorStakingState _newState) internal {
        state = _newState;
    ...
    function withdraw() external payable{
        bytes32 senderRole = 0x00;
        require(state == OriginatorStakingState.DEACTIVATED, 'ONLY_ON_DEACTIVATED_STATE');
        ...
        uint256 contribution = proposers[senderRole].currentAmount;
        require(contribution > 0, "Contribution is 0");

        uint256 contractBalance = IERC20Upgradeable(STAKED_TOKEN).balanceOf(address(this));
        uint256 reclaimAmount = (contractBalance < contribution) ? contractBalance : contribution;
        
        proposers[senderRole].currentAmount = proposers[senderRole].currentAmount.sub(contribution);
        IERC20Upgradeable(STAKED_TOKEN).safeTransfer(msg.sender, reclaimAmount);

        emit Withdraw(msg.sender, reclaimAmount);
    }
     ...
     **/
    function stake(address _to, uint256 _amount) public virtual override {
        require(!hasRole(ORIGINATOR_ROLE, _to), 'ORIGINATOR_STAKER_PERMISSION_DENIED');
        require(!hasRole(AUDITOR_ROLE, _to), 'AUDITOR_STAKER_PERMISSION_DENIED');
        require(state != OriginatorStakingState.PROPOSAL, 'WRONG_STATE_PROPOSAL');
        require(state != OriginatorStakingState.DEACTIVATED, 'WRONG_STATE_DEACTIVATED');
    ...
    /**
     * Function to calculate the collateralization amount from its percentual goal
     * @param _collateralizationPercent  Percentage to calculate the collateralization amount
     * @param _referenceGoal Amount in exchange currency par e.g 400k DAI
     * @param _rate (value * 100 e.g. 0.54 DAI/ETHIX == 54)
     * @return (amount)
     **/
    function calculateCollateralizationGoal(uint256 _collateralizationPercent, uint256 _referenceGoal, uint256 _rate) internal pure returns (uint256) {
        return uint256((_referenceGoal.mul(_collateralizationPercent).div(100)).div(_rate));
    }

    /**
     * @notice Function to declare contract on deactivated state
     * Only Emission Manager could change to this state
     **/
    function declareDeactivated() external onlyEmissionManager {
        changeState(OriginatorStakingState.DEACTIVATED);
    }

    /**
     * Function to add an extra emissions per second corresponding to staker rewards when a lending project by this originator
     * is funded.
     * @param extraEmissionsPerSecond  emissions per second to be added to current ones.
     */
    function startProjectFundedRewards(uint128 extraEmissionsPerSecond) external {
      ...
      uint128 newEmissionsPerSecond = currentDistribution.emissionPerSecond.add(extraEmissionsPerSecond);
      ...
      emit StartRewardsProjectFunded(currentDistribution.emissionPerSecond, extraEmissionsPerSecond);
     ...
     * @param extraEmissionsPerSecond  emissions per second to be added to current ones.
     */
    function endProjectFundedRewards(uint128 extraEmissionsPerSecond) external {
      require(state == OriginatorStakingState.STAKING, 'ONLY_ON_STAKING_STATE');
      AssetData storage currentDistribution = assets[address(this)];
      uint128 newEmissionsPerSecond = currentDistribution.emissionPerSecond.sub(extraEmissionsPerSecond);
      DistributionTypes.AssetConfigInput [] memory newAssetConfig = new DistributionTypes.AssetConfigInput[](1);
      newAssetConfig[0] = DistributionTypes.AssetConfigInput({
          emissionPerSecond: newEmissionsPerSecond,
          totalStaked: totalSupply(),
      ...
      emit EndRewardsProjectFunded(currentDistribution.emissionPerSecond, extraEmissionsPerSecond);

    }

    // TODO: Slashing
    // function sendFundsToCompensationSystem() internal {}
...
contract StakedToken is Initializable, IStakedEthix, EthixERC20Snapshot, StakingRewards {
     ...
     */
    function setCooldown(uint256 cooldownSeconds) public {
        require(hasRole(EMISSION_MANAGER_ROLE, msg.sender), 'ONLY_EMISSION_MANAGER');
        COOLDOWN_SECONDS = cooldownSeconds;
    }
     ...
     */
    function setUnstakeWindow(uint256 unstakeWindow) public {
        require(hasRole(EMISSION_MANAGER_ROLE, msg.sender), 'ONLY_EMISSION_MANAGER');
        UNSTAKE_WINDOW = unstakeWindow;
    }
}
