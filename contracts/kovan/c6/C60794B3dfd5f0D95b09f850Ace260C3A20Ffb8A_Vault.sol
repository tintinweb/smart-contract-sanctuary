...
contract Vault is IVault, ReentrancyGuardUpgradeable
{
  ...
  function __Vault_init (address token) external override initializer
  {
    require(msg.sender == _ADDRESSES.coordinator(), "!coordinator");


    __ReentrancyGuard_init();

    _token = IERC20(token);
    _token.safeApprove(_ADDRESSES.depositManager(), type(uint256).max);
    _token.safeApprove(_ADDRESSES.borrowManager(), type(uint256).max);
  }
  ...
  function updateAllowances () external override
  {
    require(msg.sender == _ADDRESSES.coordinator(), "!coordinator");


    _token.safeApprove(_ADDRESSES.depositManager(), 0);
    _token.safeApprove(_ADDRESSES.depositManager(), type(uint256).max);

    _token.safeApprove(_ADDRESSES.borrowManager(), 0);
    _token.safeApprove(_ADDRESSES.borrowManager(), type(uint256).max);
  }
}
