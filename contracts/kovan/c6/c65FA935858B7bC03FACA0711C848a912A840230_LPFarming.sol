...
/**
 * JPEGD LP farming (JPEGD/ETH, JPEGD/3CRV, ...)
 */
contract LPFarming is Ownable, ReentrancyGuard {
    ...
    event ClaimAll(address user, uint256 amount);
    ...
    mapping(address => uint256) private userRewards; // user's withdrawable rewards
    mapping(address => bool) public whitelistedContracts;

    constructor(address _jpegd)
    {
        jpegd = IERC20(_jpegd);
    }

    modifier noContract(address _account) {
        require(!_account.isContract() || whitelistedContracts[_account], "Contracts aren't allowed to farm");
        _;
    }

    function setContractWhitelisted(address _contract, bool _isWhitelisted) external onlyOwner {
        whitelistedContracts[_contract] = _isWhitelisted;
    }

    function newEpoch(uint256 _startBlock, uint256 _endBlock, uint256 _rewardPerBlock) external onlyOwner {
        require(block.number != _blockNumber(), "epoch ongoing");
        require(_startBlock > epoch.endBlock, "Invalid start block");
    ...
    function deposit(uint256 _pid, uint256 _amount) external noContract(msg.sender) {
    ...
    function withdraw(uint256 _pid, uint256 _amount) external noContract(msg.sender) {
        require(_amount > 0, "invalid_amount");
    ...
    function _normalizeBlockNumber(uint256 blockNumber) internal view returns (uint256) {
    ...
    function claim(uint256 _pid) external nonReentrant noContract(msg.sender) {
        ...
        jpegd.safeTransfer(msg.sender, rewards);
    ...
    function claimAll() external nonReentrant noContract(msg.sender){
        for (uint256 i = 0; i < poolInfo.length; i++) {
            ...
            userInfo[i][msg.sender].rewardDebt =
                (userInfo[i][msg.sender].amount *
                    poolInfo[i].accRewardPerShare) /
                (1e36);
        }
