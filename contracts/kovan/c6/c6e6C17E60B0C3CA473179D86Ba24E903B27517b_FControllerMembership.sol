...
abstract contract FRErc20Interface is FRErc20Storage {
    ...
    function redeem(uint redeemTokens, uint suppyId) virtual external returns (uint);
    function redeemUnderlying(uint redeemAmount, uint suppyId) virtual external returns (uint);
    function borrow(uint borrowAmount, uint period) virtual external returns (uint);
    function repayBorrow(uint repayAmount, uint borrowId) virtual external returns (uint);
    function repayBorrowBehalf(address borrower, uint repayAmount, uint borrowId) virtual external returns (uint);
...
abstract contract FRToken is IFRToken, Exponential, TokenErrorReporter {
    ...
    function borrowRatePerBlock() external override view returns (uint) {
        ...
        uint newSlope1 = mulUInt(fixedRateSlope1, 12e17);
        uint newSlope2 = mulUInt(fixedRateSlope2, 12e17);
        if(divUInt(totalBorrows,getCashPrior()) < optimalUtilisationRate)
        {
            return mulUInt(mulUInt(floatRate,newSlope1), 1e18);
        }
        else
        {
            return mulUInt(mulUInt(floatRate,newSlope2), 1e18);
        }
    }
    ...
    function supplyRatePerBlock() external override view returns (uint) {
        ...
        if(divUInt(totalBorrows,getCashPrior()) < optimalUtilisationRate)
        {
            return mulUInt(mulUInt(supplyRate,fixedRateSlope1), 1e18);
        }
        else
        {
            return mulUInt(mulUInt(supplyRate,fixedRateSlope2), 1e18);
        }
    }
    ...
    function mintInternal(uint mintAmount, uint period) internal nonReentrant returns (uint, uint) {
        ...
        if(divUInt(totalBorrows,getCashPrior()) < optimalUtilisationRate)
        {
            supplyRate = mulUInt(mulUInt(floatSupplyRate,fixedRateSlope1), 1e18);
        }
        else
        {
            supplyRate = mulUInt(mulUInt(floatSupplyRate,fixedRateSlope2), 1e18);
        }
     ...
     */
    function redeemInternal(uint redeemTokens, uint depositId) internal nonReentrant returns (uint) {
        uint error = accrueInterest();
        if (error != uint(Error.NO_ERROR)) {
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function redeemUnderlyingInternal(uint redeemAmount, uint depositId) internal nonReentrant returns (uint) {
        uint error = accrueInterest();
        if (error != uint(Error.NO_ERROR)) {
            // accrueInterest emits logs on errors, but we still want to log the fact that an attempted redeem failed
            return fail(Error(error), FailureInfo.REDEEM_ACCRUE_INTEREST_FAILED);
    ...
    function borrowInternal(uint borrowAmount, uint period) internal nonReentrant returns (uint) {
        ...
        if(divUInt(totalBorrows,getCashPrior()) < optimalUtilisationRate)
        {
            borrowRate = mulUInt(mulUInt(floatRate,newSlope1), 1e18);
        }
        else
        {
            borrowRate = mulUInt(mulUInt(floatRate,newSlope2), 1e18);
        }
     ...
     */
    function repayBorrowInternal(uint repayAmount, uint borrowId) internal nonReentrant returns (uint, uint) {
        uint error = accrueInterest();
        if (error != uint(Error.NO_ERROR)) {
            // accrueInterest emits logs on errors, but we still want to log the fact that an attempted borrow failed
     ...
     * @return (uint, uint) An error code (0=success, otherwise a failure, see ErrorReporter.sol), and the actual repayment amount.
     */
    function repayBorrowBehalfInternal(address borrower, uint repayAmount, uint borrowId) internal nonReentrant returns (uint, uint) {
        uint error = accrueInterest();
        if (error != uint(Error.NO_ERROR)) {
            // accrueInterest emits logs on errors, but we still want to log the fact that an attempted borrow failed
