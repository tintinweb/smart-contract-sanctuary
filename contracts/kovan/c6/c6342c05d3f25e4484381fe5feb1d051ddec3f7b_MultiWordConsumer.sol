...
contract MultiWordConsumer is ChainlinkClient {
   ...
   * @param _specId string jobId in the Oracle
  ...
  function requestMultipleParameters(
    ...
    public
  {
    bytes32 specId = stringToBytes32(_specId);
  ...
  function fulfillMultipleParameters(
    bytes32 _requestId,
    uint256 _usdResponse,
    uint256 _eurResponse,
    uint256 _jpyResponse
  )
    public
    recordChainlinkFulfillment(_requestId)
  {
    emit RequestMultipleFulfilled(_requestId, _usdResponse, _eurResponse, _jpyResponse);
    usd = _usdResponse;
    eur = _eurResponse;
    jpy = _jpyResponse;
  }
