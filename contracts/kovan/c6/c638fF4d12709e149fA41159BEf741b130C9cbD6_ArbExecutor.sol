...
contract ArbExecutor {
    ...
    function swapEthForTokenSFONTT(uint256 amountIn, address token, uint256 amountOutMin) public payable returns (uint256){
    ...
    function swapTokenForTokenSFONTT(uint256 amountIn, address tokenIn, address tokenOut, uint256 amountOutMin) public payable returns (uint256){
    ...
    function swapTokenForEthSFONTT(uint256 amountIn, address token, uint256 amountOutMin) public payable returns (uint256)  {
        assert(ERC20_Interface(token).transfer(UniswapV2Library.pairFor(Uniswap_Factory_Address, token, WETH_Address), amountIn));

        uint balanceBefore = ERC20_Interface(WETH_Address).balanceOf(address(this));
        _swapSupportingFeeOnTransferTokens(token, WETH_Address, address(this));
        uint256 amountOut = ERC20_Interface(WETH_Address).balanceOf(address(this)).sub(balanceBefore);
    ...
    function swapSFONTT(uint256 amountIn, address tokenIn, address tokenOut, uint256 amountOutMin) public payable returns (uint256) {

        if(tokenIn == WETH_Address) {
            return swapEthForTokenSFONTT(amountIn, tokenOut, amountOutMin);
        } else if(tokenOut == WETH_Address) {
            return swapTokenForEthSFONTT(amountIn, tokenIn, amountOutMin);
        } else {
            return swapTokenForTokenSFONTT(amountIn, tokenIn, tokenOut, amountOutMin);
        }
    }

    function Strategy_Pathfinding_NoCheck(Operation memory _operation, uint256 amountIn) public payable {

        uint256 loopIn = amountIn; 
        uint256 finalOut = 0;

        for(uint i = 0; i < _operation.tokens.length - 1; i++) {
            uint256 currentOut = swap(loopIn, _operation.tokens[i], _operation.tokens[i + 1], _operation.minOuts[i]);
            loopIn = currentOut;

            if(i == _operation.tokens.length - 2) {
                finalOut = currentOut;
            }
        }
    }

    function Strategy_Pathfinding(Operation memory _operation, uint256 amountIn) public payable {
        ...
        require(finalOut > amountIn);
    }

    function Strategy_Pathfinding_NoCheck_SFONTT(Operation memory _operation, uint256 amountIn) public payable {

        uint256 loopIn = amountIn; 
        uint256 finalOut = 0;

        for(uint i = 0; i < _operation.tokens.length - 1; i++) {
            uint256 currentOut = swapSFONTT(loopIn, _operation.tokens[i], _operation.tokens[i + 1], _operation.minOuts[i]);
            loopIn = currentOut;
    ...
    function Strategy_Pathfinding_SFONTT(Operation memory _operation, uint256 amountIn) public payable {
