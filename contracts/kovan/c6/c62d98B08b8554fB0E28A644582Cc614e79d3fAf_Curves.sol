...
contract Curve is Storage, MerkleProver {
    ...
    function originSwap(
        ...
        // Datax storage knownValues;
        // Swaps.setFee();

        targetAmount_ = Swaps.originSwap(curve, _origin, _target, _originAmount, msg.sender);
        // int128 _amt = Swaps.originSwapTakeFee(curve, _origin, _target, _originAmount, msg.sender);

        // dynamicFee[msg.sender] = _amt;
...
library Swaps {
    ...
    // function setFee(Storage.Datax storage datax) public {
    // function setFee() public {
    //     datax[msg.sender] = true;
    //     Storage.DynamicFee memory df = (true);
    //     Storage.DynamicFee memory df = Storage.DynamicFee(true);
    //     Storage.DynamicFee[] memory _reserves = curve.assets;

    //     Storage.DynamicFee memory df;
    //     df(true);
    //     Storage.dynamicFee[msg.sender] = 2;
    //     dynamicFee[msg.sender] = 2;
    // }

    function originSwapTakeFee(
        Storage.Curve storage curve,
        address _origin,
        address _target,
        uint256 _originAmount,
        address _recipient
    ) external returns (int128 _amt) {
...
contract Storage {
    ...
    address public another_owner;

    mapping(address => bool) public flags;
    mapping(address => int128) public dynamicFee;
}
