...
contract RemovePosition is FlashLoanReceiverBase {
    ...
    function lockGemAndDraw(
        ...
        console.log('collateralToLock', collateralToLock);
        console.log('daiToBorrow', daiToBorrow);
    ...
    function lockAndDrawOperation(bytes memory params) public payable{
        ...
        if (parameters.token1FromUser.add(token1FromDebtToken) > 0){
            ...
            console.log('parameters.token1FromUser.add(token1FromDebtToken).sub(token1Used)', parameters.token1FromUser, token1FromDebtToken, token1Used);
            console.log('IERC20(parameters.token1).balanceOf(address(this))', IERC20(parameters.token1).balanceOf(address(this)));
