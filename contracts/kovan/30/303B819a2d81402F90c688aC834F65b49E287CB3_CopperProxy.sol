...
contract CopperProxy is Ownable {
    ...
    mapping(address => PoolData) public _poolData;
    ...
    function exitPool(address pool, uint256[] calldata minAmountsOut) external onlyPoolOwner(pool) {
        ...
        TransferHelper.safeTransfer(
            poolTokens[0],
            msg.sender,
            IERC20(poolTokens[0]).balanceOf(address(this))
        );
        TransferHelper.safeTransfer(
            poolTokens[1],
            msg.sender,
            IERC20(poolTokens[1]).balanceOf(address(this))
        );
    }
    ...
    function skim(address token) external onlyOwner {
        ...
        TransferHelper.safeTransferFrom(token, msg.sender, VAULT, IERC20(token).balanceOf(address(this)));
    }
}
