 ...
 * @author Aave
 */
contract LendToAaveMigratorV2 is VersionedInitializable {
   ...
   * NOTE: The asset rescuer should be trusted (gov), since has access to the contract's funds.
   * 
   * @param assetRescuer the address of the rescuer of locked and stuck assets
   * @param lendAmount the amount of LEND to send to LendToken contract
   * @param lendAmountAsMigrated the amount of LEND to be considered as migrated
   */
  function initialize(address assetRescuer, uint256 lendAmount, uint256 lendAmountAsMigrated) public initializer {
    require(lendAmount <= lendAmountAsMigrated, 'INVALID_LEND_AMOUNTS');
    
    _assetRescuer = assetRescuer;

    // Take the LEND amount into account as migrated
    _totalLendMigrated = _totalLendMigrated.add(lendAmountAsMigrated);
    LEND.safeTransfer(address(LEND), lendAmount);
  }
  ...
  /**
   * @dev Redirect stuck tokens in this contract to the safe vault
   * @param asset the address of the asset
   * @param safeVault the address of a vault where funds should be transferred to
   * @param amount the amount of tokens to redirect
   */
  function retrieveStuckTokens(
    ...
    require(msg.sender == _assetRescuer, 'ONLY_ASSET_RESCUER');
    IERC20(asset).safeTransfer(safeVault, amount);
  }
}
