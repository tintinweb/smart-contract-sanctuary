...
contract MonoswapStaking is Ownable, ERC1155Holder {
    ...
    {
        if (_from <= _to) {
            return _to.sub(_from).div(blockPerPeriod);
        } else {
            return 0;
        } 
    }

    function calcReward(uint256 periods, uint256 allocPoint)
        public
        view
        returns (uint256)
    {
        ...
        while (periods > 0) {
            ...
            periods = periods.sub(1);
        } 
        return totalRatio.mul(allocPoint).div(totalAllocPoint);
    }
        ...
        returns (uint256)
    {
        ...
        uint256 lpSupply = pool.lpToken.balanceOf(address(this), pool.lpTokenId);
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 periods =
                getPeriods(pool.lastRewardBlock, block.number);
            uint256 monoReward =
                calcReward(periods, pool.allocPoint);
            accMonoPerShare = accMonoPerShare.add(
                monoReward.div(lpSupply)
            );
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 monoReward =
                calcReward(periods, pool.allocPoint);
        mono.mint(address(this), monoReward);
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            uint256 pending =
                user.amount.mul(pool.accMonoPerShare).div(1e12).sub(
                    user.rewardDebt
                );
