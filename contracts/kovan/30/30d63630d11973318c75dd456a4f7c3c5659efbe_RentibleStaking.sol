...
*/      
    ...
    function deposit(uint256 _depositLptAmount) public {
        ...
        dailyTotalLptAmount[dayNumber] = currentTotalLptAmount;
    ...
    function addToTheTheUsersAssignedReward() public returns(uint256) {
        ...
        uint256 rewardCountedUptoDayNextDay =  rewardCountedUptoDay.add(1);
        ...
        uint256 totalLptAmountPrev = 0;
        
        for (uint256 i = rewardCountedUptoDayNextDay; i <= dayNumberMinusOne; i++) {
            
            uint256 ddd = dailyTotalLptAmount[i];
            if (ddd == 0) {
                ddd = totalLptAmountPrev;
            } else {
                totalLptAmountPrev = ddd;
            }
            
           uint256 k = ddd.div(usersCurrentLtpAmount);
           uint256 t = dailyPlannedErc20RewardAmounts[i].div(k);

            if (dailyErc20RewardAmounts[i] == 0) {
                dailyErc20RewardAmounts[i] = dailyPlannedErc20RewardAmounts[i];
            }

            if (dailyErc20RewardAmounts[i] > t) {
                usersRewardRecently = usersRewardRecently.add(t);
                dailyErc20RewardAmounts[i] = dailyErc20RewardAmounts[i].sub(t);
            } else {
    ...
    function calculateCurrentTakeableRewardOfTheUser() public view returns(uint256) {
        ...
        for (uint256 i = rewardCountedUptoDayNextDay; i <= dayNumberMinusOne; i++) {
            ...
            uint256 x = dailyErc20RewardAmounts[i];

            if (x == 0) {
                x = dailyPlannedErc20RewardAmounts[i];
            }
            ...
            if (x > t) {
                ...
                // relevant at the and because of possible slight rounding issues
                usersRewardRecently = usersRewardRecently.add(x);
            }
