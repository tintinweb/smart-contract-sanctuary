...
contract SVaultNetValue is SVaultNetValueStorage {
   ...
   //@notice  The number of seconds in a year
    ...
    event NetValueFixed(address pool,uint256 tokenInUSDBefore,uint256 tokenInUSDAfter,uint256 tokenBefore,uint256 tokenAfter);
    modifier onlyAdmin() {
   ...
   function commissionFixed(uint256 fixedPoolLength,PoolInfo[] memory poolInfos,NetValue[] memory netValues)internal  returns(NetValue[] memory,uint256){
         ...
         for(uint256 i = 0; i < poolInfos.length; i++){
            ...
            for(uint256 j = 0; j < pools.length; j++)
            {
               if( netValue.pool == pools[j]){
                   ...
                   emit NetValueFixed(pools[j],tokenInUSDBefore,netValue.totalTokensInUSD,tokenBefore,netValue.totalTokens);
    ...
    function getApys(SVaultNetValue.NetValue[] memory netValues,uint256 fixedPoolsLength,uint256 flexiblePoolsLength) public view returns( bool[] memory isPositive,address[] memory pools,uint256[] memory apys,uint256[] memory amountInUSD){        
   ...
   function _culculate(uint256 amountInUSD,uint256 apyScale,uint256 rate)internal view returns(uint256){ 
    ...
    // function takeAllowed(uint256 excessCommission) public view returns(bool){
    //      uint256 totalAmount;
    ...
    //      require(msg.sender == admin, "!admin");
    //      bool allowed = takeAllowed(excessCommission);
    //      if(allowed){
    //             for(uint256 i = 0; i < excessCommission.length; i++){
