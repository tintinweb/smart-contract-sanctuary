...
contract UsdcOracle is IUniswapOracle, CoreRef {
    ...
    IUniswapV3Pool public override pool;
    bool public override isPrice0 = true;

    /// @notice the window over which the initial price will "thaw" to the true peg price
    uint32 public override duration;
    uint256 public override immutable decimalsMultiplier = 1e6;
...
import "../pcv/ERC20UniswapPCVDeposit.sol";
import "../oracle/UniswapOracle.sol";
import "../oracle/UsdcOracle.sol";
import "./IOrchestrator.sol";

contract USDCPCVDepositOrchestrator is IPCVDepositOrchestrator, Ownable {
    function init(
        address core,
        address pool,
        address nft,
        ...
        returns (address erc20UniswapPCVDeposit, address uniswapOracle)
    {
        ...
        erc20UniswapPCVDeposit = address(
            new ERC20UniswapPCVDeposit(core, pool, nft, router, uniswapOracle, IERC20(erc20TokenAddress))
        );
    }
...
contract ERC20UniswapPCVDeposit is UniswapPCVDeposit {
    ...
    IERC20 public immutable erc20TokenAddress;
    ...
    /// @param _erc20TokenAddress address of stable coin ERC20 token, such as DAI, USDT, USDC
    constructor(
        address _core,
        address _pool,
        address _nft,
    ...
    function deposit() external payable override whenNotPaused {
        uint256 erc20AmountBalance = erc20TokenAddress.balanceOf(address(this)); // include any ERC20 dust from prior LP
    ...
    function collect() external override whenNotPaused {
        (uint256 amount0, uint256 amount1) = nft.collect(
        ...
        returns (uint256)
    {
        ...
        return address(rusd()) < address(erc20TokenAddress) ? amountToken1 : amountToken0;
    }

    function _transferWithdrawn(address to, uint256 amount) internal override {
        erc20TokenAddress.transfer(to, amount);
    }
        ...
        if (tokenId == 0) {
            (tokenId, , ,) = nft.mint(
                INonfungiblePositionManager.MintParams({
                    token0: address(rusd()),
                    token1: address(erc20TokenAddress),
                    fee: fee,
                    tickLower: tickLower,
                    tickUpper: tickUpper,
                    amount0Desired: rusdAmount,
                    amount1Desired: erc20Amount,
                    amount0Min: 0,
                    amount1Min: 0,
                    recipient: address(this),
                    deadline: endOfTime
...
abstract contract UniswapPCVDeposit is IPCVDeposit, UniRef {
    ...
    int24 public override tickLower = -513; // (X/2**96)**2 = 0.95
    int24 public override tickUpper = 487; // (X/2**96)**2 = 1.05
        ...
        whenNotPaused
    {
        ...
        uint256 amountWithdrawn = _removeLiquidity(liquidityToWithdraw.toUint128());
    ...
    function totalValue() public view override returns (uint256) {
        (, , address token0, , , , , , , , uint128 tokensOwed0, uint128 tokensOwed1) = nft.positions(tokenId);
        return token() == token0 ? tokensOwed0 : tokensOwed1;
    }
    ...
    function _removeLiquidity(uint128 amount)
        internal
        virtual
        returns (uint256);
