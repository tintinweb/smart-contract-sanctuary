...
contract Orderbook{
    ...
    function createOrder (
        ...
        if (isMatching == true) 
            _matchingMarketOrder(msg.sender, userOrders[msg.sender].length - 1);
    ...
    function _startMatching(address _user, uint _index) internal {
    ...
    function startMatching(uint index) external {
        _startMatching(msg.sender, index);
    }
    ...
    function _matchingMarketOrder(address user, uint index) internal {
        OwnerOrderInfo memory ownerOrder = userOrders[user][index];
        ...
        if(ordersSuitable.length > 0) {
            ...
            if (amountARemaining == 0) {
                ...
                liquidity[ownerOrder.tokenB][ownerOrder.tokenA] -= price;
                emit MatchingMarketOrder (
                    tokenA, 
                    tokenB, 
                    nonce, 
                ...
                liquidity[ownerOrder.tokenA][ownerOrder.tokenB] -= (order.amountA - amountARemaining);
                liquidity[ownerOrder.tokenB][ownerOrder.tokenA] -= amountBRemaining;
                emit MatchingMarketOrder (
                    tokenA, 
                    tokenB, 
                    nonce, 
    ...
    function matchingMarketOrder(uint index) external {
        _matchingMarketOrder(msg.sender, index);
    }
    ...
    function cancelOrder(uint index) external {
        ...
        liquidity[ownerOrder.tokenA][ownerOrder.tokenB] -= order.amountA;
        order.amountA = 0;
        emit CancelOrder(ownerOrder.nonce, ownerOrder.tokenA, ownerOrder.tokenB, order.amountA, uint64(block.timestamp));
    }
    ...
    function closeOrder(uint index) external {
        ...
        order.amountA = 0;
        order.amountB = 0;
        emit CloseOrder(ownerOrder.nonce, ownerOrder.tokenA, ownerOrder.tokenB, order.amountA,order.amountB, uint64(block.timestamp));
    }
