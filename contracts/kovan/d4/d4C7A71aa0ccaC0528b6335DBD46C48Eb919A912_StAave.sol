...
abstract contract FeeDistributor is Initializable, IFeeDistributor {
    ...
    function distributeFees() external override {
        ...
        emit FeeDistributed(toLido, toDelphi, toDeveloper);
    }
    ...
    function setFeeRecipient(address newRecipient) external override {
        ...
        if (msg.sender == lidoAgent) {
            ...
            developer = newRecipient;
        } else {
            revert("Only current recipients can call");
        }

        emit FeeRecipientSet(lidoAgent, delphiAgent, developer);
    }
...
interface IFeeDistributor {
    ...
    event FeeDistributed(uint256 toLido, uint256 toDelphi, uint256 toDeveloper);
