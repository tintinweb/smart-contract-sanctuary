...
contract CathyBank is Ownable {
    ...
    function getETHRewardsShare(address payable _shareholder)
        internal
        returns (bool)
    {
        ICERC20 cbt = ICERC20(cathyBankContractAddress);
        ...
        uint256 ethShare = (contractETHBalance.mul(sharePercentage)).div(100);
    ...
    function distributeShareRewards() external onlyOwner {
        for (uint8 i = 0; i < cathyBankTokenHolders.length; i++) {
            getETHRewardsShare(cathyBankTokenHolders[i]);
        }
    }
}
