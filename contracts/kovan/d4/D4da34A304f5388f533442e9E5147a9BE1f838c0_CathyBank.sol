...
contract CathyBank is Ownable {
    ...
    // Settings
    uint256 public minCollateralPercent = 150000000; // 150000000 = 150%
    uint256 public loanInterestPercent = 7000000; // 7000000 = 7%
    uint256 public savingsInterestPercent = 4000000; // 4000000 = 4%
    ...
    function depositPHX(uint256 _PHXAmount) public {
        ...
        PHXBalance[msg.sender] = PHXBalance[msg.sender].add(_PHXAmount);
        PHXReserves = PHXReserves.add(_PHXAmount);
        emit PHXDepositTransaction(msg.sender, _PHXAmount, PHXBalance[msg.sender]);
    }

    function borrowPHX(uint256 _PHXAmount) payable public {
        require(_PHXAmount >= PHXReserves, 'Not enough reserves');
        require(computeCollateralPercent(msg.value, _PHXAmount) >= 
            minCollateralPercent);

        ETHCollateral[msg.sender] = ETHCollateral[msg.sender].add(msg.value);
        borrowedAmount[msg.sender] = borrowedAmount[msg.sender].add(_PHXAmount);

        IERC20 PHX = IERC20(phxToken);
        PHX.transfer(msg.sender, _PHXAmount);
        emit PHXBorrowTransaction(msg.sender, msg.value, _PHXAmount, loanInterestPercent);
    }

    function computeCollateralPercent(
        uint256 _ETHAmount, 
        uint256 _PHXAmount
    ) public view returns (uint256) {
        return ((_ETHAmount * ETHPHXConversionRate * 1000000) / (10^18) / 100) / _PHXAmount;
    }
}
