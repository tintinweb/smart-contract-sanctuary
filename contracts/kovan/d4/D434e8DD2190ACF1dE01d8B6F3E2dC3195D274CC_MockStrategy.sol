...
contract L2_NovaRegistry is Auth, CrossDomainEnabled, ReentrancyGuard {
    ...
    function requestExec(
        ...
        require(msg.value == gasLimit.mul(gasPrice).add(tip), "INCORRECT_VALUE");
    ...
    function withdrawTokens(bytes32 execHash) external nonReentrant requiresAuth {
        ...
        payable(creator).transfer(getRequestGasPrice[execHash].mul(getRequestGasLimit[execHash]).add(getRequestTip[execHash]));
    ...
    function speedUpRequest(bytes32 execHash, uint256 gasPrice) external payable requiresAuth returns (bytes32 newExecHash) {
        ...
        require(msg.value == gasPrice.sub(previousGasPrice).mul(previousGasLimit), "INCORRECT_VALUE");
    ...
    function execCompleted(
        ...
        payable(creator).transfer(gasLimit.mul(gasPrice).sub(gasPayment).add(reverted ? tip : 0));

        // Pay the recipient the gas payment + the tip (if execution succeeded).
        payable(rewardRecipient).transfer(gasPayment.add(reverted ? 0 : tip));
    }
