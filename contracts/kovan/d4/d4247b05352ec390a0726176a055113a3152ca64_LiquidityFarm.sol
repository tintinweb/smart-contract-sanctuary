...
// File: contracts/interfaces/ITrader.sol
...
// File: contracts/farming/LiquidityFarm.sol
...
contract LiquidityFarm is Ownable {
    ...
    uint constant TRADE_AMOUNT = 1 ether;
    ...
    function withdrawZapAndUnstake(address _pool, uint _amount) public poolExists(_pool) {
        require(pools[_pool].stakes[msg.sender].liquidity >= _amount, "You do not have that many liquidity tokens staked");
        // update StakeInfo
    ...
    function _calculateReward(
        address _pool,
        StakeInfo storage _stake
    ) private view returns (uint256 reward) {
        ILiquidityPool liquidityPool = ILiquidityPool(_pool);
        uint share = _stake.tokenStake * ONE_HUNDRED_PERCENT / liquidityPool.totalSupply();
        (uint t0, uint t1,) = liquidityPool.getReserves();
        ...
        uint amount = 
        trader.getAmountOut(rewardToken,depositToken, TRADE_AMOUNT) *
        ...
        uint duration = block.timestamp - _stake.updatedAt;
        uint apy = pools[_pool].apy;
