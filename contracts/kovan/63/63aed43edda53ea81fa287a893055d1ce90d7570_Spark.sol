...
// library containing math functions from BancorFormula.sol
library Math {
...
0.9.0:
    1. fix comments
    2. switch back to private
    3. safe math libraries clean up
    4. libraries/interfaces clean up
    5. simplify events
*/
...
contract Spark is Ownable {
    ...
    event Staked(address user, uint ID, uint amount, uint totalStakedLP);
    event Unstaked(address user, uint ID, uint amount, uint remainingLP);
    ...
    struct Deposit {
        ...
        bool hasFreeClaimRequest;
        // if there is a free-claim request on this deposit
        uint requestID;
    ...
    struct Profile {
        ...
        uint numDeposit;
        // the number of deposits held by the user
        uint numRequest;
        // the number of free-claim requests by the user
        uint numLP;
        // the number of LP tokens currently held by the user in the contract
        uint cumuReward;
        // cumulative reward tokens received by the user
    }
...
//TO-DO: switch back to private
    uint public _startRequestID = 1;
    uint public _nextRequestID = 1;
    ...
    function stake(uint amount) external returns (uint id) {
        // sanity check
        require(amount > 0, '_stake Error: stake amount is zero.');
        // check if the user is still allowed to join
        require(totalRewardGiven < threshold, "_stake Error: threshold has been met. No new deposit is allowed.");

        // update the user and global stats for this interaction
        // 1. update user stats
        id = _userProfiles[msg.sender].nextID;
        Deposit memory newDeposit = Deposit(amount, now, 0, false, 0);
        _userDeposits[msg.sender][id] = newDeposit;

        _userProfiles[msg.sender].nextID = id + 1;
        _userProfiles[msg.sender].numDeposit = _userProfiles[msg.sender].numDeposit + 1;
        _userProfiles[msg.sender].numLP = _userProfiles[msg.sender].numLP.add(amount);
        ...
        emit Staked(msg.sender, id, amount, _userProfiles[msg.sender].numLP);
    }
    ...
    function claim(uint index) public returns (uint) {
        ...
        if (_userDeposits[msg.sender][index].hasFreeClaimRequest) {
    ...
    function _claim(address staker, uint index) private returns (uint) {
        ...
        _userProfiles[staker].cumuReward = _userProfiles[staker].cumuReward.add(claimableAmt);
        // 2. update global stats
    ...
    function claimAndWithdraw(uint index) public returns (uint claimAmt, uint withdrawAmt) {
        ...
        // update user stats
        _userProfiles[msg.sender].numLP = _userProfiles[msg.sender].numLP.sub(withdrawAmt);
        _userProfiles[msg.sender].numDeposit = _userProfiles[msg.sender].numDeposit - 1;
        ...
        emit Unstaked(msg.sender, index, withdrawAmt, _userProfiles[msg.sender].numLP);
    }
    ...
    function requestFreeClaim(uint index) public {
        ...
        require(!_userDeposits[msg.sender][index].hasFreeClaimRequest, "requestFreeClaim: a request has already been placed.");

        // update global request records
        Request memory newRequest = Request(true, msg.sender, index);
        uint id = _nextRequestID;
        _requestMap[id] = newRequest;
        _nextRequestID++;
        totalRequest++;

        // update user record
        _userDeposits[msg.sender][index].hasFreeClaimRequest = true;
        _userDeposits[msg.sender][index].requestID = id;
        _userProfiles[msg.sender].numRequest = _userProfiles[msg.sender].numRequest + 1;
    ...
    function executeFreeClaim(uint numToExe) external onlyOwner {
        // sanity check
        require(numToExe <= totalRequest, "executeFreeClaim: there are not so many requests to execute.");

        // execute each free-claim request one by one
        uint numExecuted = 0;
        while (numExecuted < numToExe) {
            // check if the request is still active
            if (_requestMap[_startRequestID].isActive) {
                // get request info
                (address requester, uint index) = (_requestMap[_startRequestID].user, _requestMap[_startRequestID].index);
                // conduct reward claim for the requester for free
                _claim(requester, index);

                // update user record
                _userDeposits[requester][index].hasFreeClaimRequest = false;
                _userDeposits[requester][index].requestID = 0;
                _userProfiles[requester].numRequest--;
                // update global records
                delete _requestMap[_startRequestID];
                totalRequest--;
                _startRequestID++;
                // emit event and count
                emit FreeClaimExecuted(requester, index, _startRequestID-1);
                numExecuted++;
            } else {
    ...
    function _cancelFreeClaim(address staker, uint index) private {
        ...
        require(_userDeposits[staker][index].hasFreeClaimRequest, "_cancelFreeClaim: there is no request on this deposit.");
        // global record
        uint reqID = _userDeposits[staker][index].requestID;
        delete _requestMap[reqID];
        totalRequest--;
        // user record
        _userDeposits[msg.sender][index].hasFreeClaimRequest = false;
        _userDeposits[msg.sender][index].requestID = 0;
        _userProfiles[msg.sender].numRequest--;
        // emit event
    ...
    function computeLPValue(uint amountInLP, uint poolLP, uint priceRaw, uint k) public pure returns (uint) {
        //calculate sqrt(4 * k * price)
        //TO-DO: lower the precision of priceRaw and k based on power's max-num
        (uint sqrtRaw, uint powerPrecision) = Math.power(priceRaw, 1, 1, 2);
        uint sqrtP = sqrtRaw >> powerPrecision;
        uint kLowPrec = k.div(10**12);
        (sqrtRaw, powerPrecision) = Math.power(kLowPrec, 1, 1, 2);
        uint sqrtK = sqrtRaw >> powerPrecision;
        uint sqrtResult = sqrtP.mul(sqrtK).mul(2).mul(10**6);
        uint rawValue = amountInLP.mul(sqrtResult).div(poolLP);
        return uint(rawValue >> uint(112.div(2)));
    }
