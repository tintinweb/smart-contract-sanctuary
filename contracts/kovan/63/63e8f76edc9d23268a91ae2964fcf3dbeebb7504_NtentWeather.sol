...
contract NtentWeather is VRFConsumerBase, ChainlinkClient {
    using Chainlink for Chainlink.Request;
    ...
    function data(uint256 tokenId) public view returns(string memory weatherData) {
        weatherData = Strings.uint2str(tokenData[tokenId]);
    }
    ...
    function requestRandom() public onlyAdmin returns (bytes32 requestId) {
        require(LINK.balanceOf(address(this)) >= fee, "Not enough LINK to pay fee");
        requestId = requestRandomness(vrfKeyhash, fee);
