...
library Orders {
    ...
    event DepositEnqueued(uint256 indexed orderId, uint128 validAfterTimestamp, DepositOrder order);
    event WithdrawEnqueued(uint256 indexed orderId, uint128 validAfterTimestamp, WithdrawOrder order);
    event SellEnqueued(uint256 indexed orderId, uint128 validAfterTimestamp, SellOrder order);
    event BuyEnqueued(uint256 indexed orderId, uint128 validAfterTimestamp, BuyOrder order);
    ...
    uint256 private constant EXECUTE_PREPARATION_COST = 55000; // deque + getPair in execute
    ...
    function enqueueDepositOrder(Data storage data, DepositOrder memory depositOrder) internal {
        ...
        emit DepositEnqueued(data.newestOrderId, validAfterTimestamp, depositOrder);
    ...
    function enqueueWithdrawOrder(Data storage data, WithdrawOrder memory withdrawOrder) internal {
        data.newestOrderId++;
        uint128 validAfterTimestamp = uint128(block.timestamp + data.delay);
        emit WithdrawEnqueued(data.newestOrderId, validAfterTimestamp, withdrawOrder);
        data.orderQueue[data.newestOrderId] = StoredOrder(
            WITHDRAW_TYPE,
            timestampToUint32(validAfterTimestamp),
    ...
    function enqueueSellOrder(Data storage data, SellOrder memory sellOrder) internal {
        data.newestOrderId++;
        uint128 validAfterTimestamp = uint128(block.timestamp + data.delay);
        emit SellEnqueued(data.newestOrderId, validAfterTimestamp, sellOrder);
        data.orderQueue[data.newestOrderId] = StoredOrder(
    ...
    function enqueueBuyOrder(Data storage data, BuyOrder memory buyOrder) internal {
        data.newestOrderId++;
        uint128 validAfterTimestamp = uint128(block.timestamp + data.delay);
        emit BuyEnqueued(data.newestOrderId, validAfterTimestamp, buyOrder);
        data.orderQueue[data.newestOrderId] = StoredOrder(
...
interface IIntegralDelay {
    ...
    function deposit(Orders.DepositParams memory depositParams) external payable;

    function withdraw(Orders.WithdrawParams memory withdrawParams) external payable;

    function sell(Orders.SellParams memory sellParams) external payable;

    function buy(Orders.BuyParams memory buyParams) external payable;
...
contract IntegralDelay is IIntegralDelay {
    ...
    function disableDeposit(address pair) public {
        require(msg.sender == owner, 'ID_FORBIDDEN');
        orders.depositDisabled[pair] = true;
    }

    function disableWithdraw(address pair) public {
        require(msg.sender == owner, 'ID_FORBIDDEN');
        orders.withdrawDisabled[pair] = true;
    }

    function disableBuy(address pair) public {
        require(msg.sender == owner, 'ID_FORBIDDEN');
        orders.buyDisabled[pair] = true;
    }

    function disableSell(address pair) public {
        require(msg.sender == owner, 'ID_FORBIDDEN');
        orders.sellDisabled[pair] = true;
    }
    ...
    function deposit(Orders.DepositParams calldata depositParams) external payable override lock {
        orders.deposit(depositParams, tokenShares);
    }

    function withdraw(Orders.WithdrawParams calldata withdrawParams) external payable override lock {
        orders.withdraw(withdrawParams);
    }

    function sell(Orders.SellParams calldata sellParams) external payable override lock {
        orders.sell(sellParams, tokenShares);
    }

    function buy(Orders.BuyParams calldata buyParams) external payable override lock {
    ...
    function _executeDeposit(Orders.DepositOrder memory depositOrder) public {
        ...
        if (
            ...
            if (amount0Left != 0) {
                (amount0Left, amount1Left) = _swapDeposit0(pair, token0, amount0Left, depositOrder.minSwapPrice);
            } else if (amount1Left != 0) {
                (amount0Left, amount1Left) = _swapDeposit1(pair, token1, amount1Left, depositOrder.maxSwapPrice);
            }
        }
    ...
    function _addLiquidityAndMint(
        ...
        (uint256 amount0, uint256 amount1) = _addLiquidity(pair, amount0Desired, amount1Desired);
    ...
    function _swapDeposit0(
        ...
        require(minSwapPrice == 0 || price >= minSwapPrice, 'ID_PRICE_TOO_LOW');
    ...
    function _swapDeposit1(
        ...
        require(maxSwapPrice == 0 || price <= maxSwapPrice, 'ID_PRICE_TOO_HIGH');
    ...
    function _executeBuy(Orders.BuyOrder memory buyOrder) public {
        ...
        if (tokenOut == tokenShares.weth && buyOrder.unwrap) {
            ...
            payable(buyOrder.to).transfer(buyOrder.amountOut);
        } else {
            pair.swap(amount0Out, amount1Out, buyOrder.to, new bytes(0));
        }
    }
    ...
    function _executeSell(Orders.SellOrder memory sellOrder) public {
        ...
        if (tokenOut == tokenShares.weth && sellOrder.unwrap) {
            ...
            payable(sellOrder.to).transfer(amountOut);
        } else {
            pair.swap(amount0Out, amount1Out, sellOrder.to, new bytes(0));
        }
    }
    ...
    function _quote(
        ...
        require(amount0 > 0, 'ID_INSUFFICIENT_AMOUNT');
        require(reserve0 > 0 && reserve1 > 0, 'ID_INSUFFICIENT_LIQUIDITY');
