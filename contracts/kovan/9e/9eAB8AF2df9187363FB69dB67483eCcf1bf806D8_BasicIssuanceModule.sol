...
// File contracts/lib/ExactSafeErc20.sol
...
// File contracts/protocol/lib/PositionUtil.sol
...
abstract contract ModuleBase is IModule {
    ...
    IController public immutable _controller;
...
contract BasicIssuanceModule is ModuleBase, ReentrancyGuard {
    ...
    mapping(IMatrixToken => IManagerIssuanceHook) internal _managerIssuanceHook;
    ...
    function getManagerIssuanceHook(IMatrixToken matrixToken) external view returns (IManagerIssuanceHook) {
        return _managerIssuanceHook[matrixToken];
    }
        ...
        returns (address[] memory, uint256[] memory)
    {
        ...
        uint256[] memory notionalUnits = new uint256[](count);

        for (uint256 i = 0; i < count; i++) {
            require(!matrixToken.hasExternalPosition(components[i]), "BI0"); // "Only default positions are supported"
    ...
    {
        _managerIssuanceHook[matrixToken] = preIssueHook;
    ...
    function removeModule() external pure override {
        revert("BI3"); // "The BasicIssuanceModule module cannot be removed")
    }
    ...
    function _callPreIssueHooks(
        ...
        IManagerIssuanceHook preIssueHook = _managerIssuanceHook[matrixToken];
