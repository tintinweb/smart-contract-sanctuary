...
import "../pcv/ERC20UniswapPCVController.sol";
...
contract ERC20UniswapPCVController is IUniswapPCVController, UniRef, Timed {
    ...
    /// @param _erc20TokenAddress ERC20 token address
    constructor(
        address _core,
        address _pcvDeposit,
        address _oracle,
    ...
    function setPCVDepositParameters(uint24 fee, int24 tickLower, int24 tickUpper) external override onlyGovernor {
        pcvDeposit.collect();
        pcvDeposit.withdraw(address(pcvDeposit), pcvDeposit.totalValue());
        pcvDeposit.burnAndReset(fee, tickLower, tickUpper);
        pcvDeposit.deposit();

        emit PCVDepositParametersUpdate(address(pcvDeposit), fee, tickLower, tickUpper);
    }
    ...
    function _reweight() internal {
        ...
        uint256 balance = erc20TokenAddress.balanceOf(address(this));
        // transfer the entire pcv erc20 token balance to pcvDeposit, then run deposit
        erc20TokenAddress.transfer(address(pcvDeposit), balance);
        pcvDeposit.deposit();
    ...
    function _swapErc20() internal {
        ...
        router.exactInputSingle(
            ISwapRouter.ExactInputSingleParams({
                tokenIn: address(erc20TokenAddress),
                ...
                sqrtPriceLimitX96: 79228162514264337593543950336 // (X/2**96)**2 = 1
            })
        );
    }
...
interface IUniswapPCVController {
    ...
    function setPCVDepositParameters(uint24 fee, int24 tickLower, int24 tickUpper) external;
