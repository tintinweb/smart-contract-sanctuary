...
import "./IUniswapV2PCVController.sol";
import "../refs/UniV2Ref.sol";
import "../external/UniswapV2Library.sol";
import "../utils/Timed.sol";

/// @title a IUniswapV2PCVController implementation for ERC20
/// @author Ring Protocol
contract ERC20UniswapV2PCVController is IUniswapV2PCVController, UniV2Ref, Timed {
    ...
    IPCVDepositV2 public override pcvDeposit;
    ...
    /// @notice ERC20UniswapV2PCVController constructor
    ...
    constructor(
        ...
        address _pair,
        address _router
    ) UniV2Ref(_core, _pair, _router, _oracle) Timed(_reweightDuration) {
        pcvDeposit = IPCVDepositV2(_pcvDeposit);
    ...
    function reweight() external override whenNotPaused {
        ...
        require(
            reweightEligible(),
            "ERC20UniswapV2PCVController: Not passed reweight time or not at min distance"
        );
        _reweight();
        _incentivize();
    }
    ...
    function setReweightWithdrawBPs(uint256 _reweightWithdrawBPs)
        ...
        onlyGovernor
    {
        require(_reweightWithdrawBPs <= BASIS_POINTS_GRANULARITY, "ERC20UniswapV2PCVController: withdraw percent too high");
    ...
    function _returnToPeg() internal {
        (uint256 rusdReserves, uint256 ethReserves) = getReserves();
        if (rusdReserves == 0 || ethReserves == 0) {
            return;
        }
        ...
        require(
            _isBelowPeg(_peg),
            "ERC20UniswapV2PCVController: already at or above peg"
        );

        // calculate amount ETH needed to return to peg then swap
        uint256 amountEth = _getAmountToPegOther(rusdReserves, ethReserves, _peg);
        _swapErc20(amountEth, ethReserves, rusdReserves);
    }
    ...
    function _swapErc20(
        ...
        uint256 amountOut =
            UniswapV2Library.getAmountOut(amount, ethReserves, rusdReserves);

        TransferHelper.safeTransfer(token(), address(pair), amount);
...
/// @title a Uniswap V2 PCV Controller interface
/// @author Ring Protocol
interface IUniswapV2PCVController is IUniswapPCVController {
    // ----------- Getters -----------
