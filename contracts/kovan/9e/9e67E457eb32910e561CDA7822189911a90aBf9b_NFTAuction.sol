...
contract NFTAuction is Ownable {
    ...
    constructor(address _nftFactory, address _tokenToPay, address _pool, 
                address _nft, address _mining, address _canvasAuction) {
    ...
    function getNFTInAuction(uint8 _amountPerPage, uint32 _page) public view returns(Auction[] memory, uint) {
        Auction[] memory arr = new Auction[](_amountPerPage);
        uint startPoint = _amountPerPage * _page;
        // uninstanted amount.
        uint rawAmount = getUninstantedAmount();
        uint totalLength = nftInAuctionArr.length + rawAmount;
        if((totalLength - 1) >= startPoint) {
            for(uint i = 0; i < _amountPerPage; i++) {
                ...
                if((nftInAuctionArr.length - 1) >= (startPoint + i)) {
                    ...
                    // 已实例化的不够了，可以用未实例化的填充。
                    auction = Auction(0, pool, pool, 0, 0, 0, pool, false);
                    auction = getNoBidAuction(auction);
                }
                arr[i] = auction;
            }
    ...
    function getNoBidAuction(Auction memory _auction) private view returns(Auction memory) {
        ...
        auction.startedAt = nftFactory.getCurPeriod() * period;
        auction.endAt = auction.startedAt + period;
        auction.lastPrice = defaultPrice;
        auction.seller = auction.belongsTo;
        auction.lastBidder = auction.belongsTo;
        return auction;
    }

    function getUninstantedAmount() internal view returns(uint) {
        uint total = nftFactory.getTotalAmount();
        if(total >= nftArr.length) {
            return 0;
        }
        return total - nftArr.length;
    }
    ...
    function bidUninstanted(string memory _tokenURI, uint128 _price) public {
        ...
        uint tokenId = createNFT(_tokenURI);

        uint startedAt = nftFactory.getCurPeriod() * period;
        uint endAt = startedAt + period;
        Auction memory auction = Auction(tokenId, pool, pool, _price, endAt, startedAt, msg.sender, true);
    ...
    function createNFTByCanvas(string memory _tokenURI) public {
        ...
        uint startedAt = nftFactory.getCurPeriod() * period;
        uint endAt = startedAt + period;
        Auction memory auction = Auction(tokenId, msg.sender, msg.sender, defaultPrice, endAt, startedAt, address(0), true);
        nftIdToAuction[tokenId] = auction;
        nftInAuctionArr.push(tokenId);
        nftArr.push(tokenId);
    ...
    // 假的NFT实例化，创建后创建者是归平台
    function createNFT(string memory _tokenURI) private returns(uint) {
        uint id = _tokenIdCounter.current();
        nft.safeMint(address(this), id, _tokenURI);
    ...
    function auctionEnd(uint _tokenId) public {
        ...
        tokenToPay.safeTransfer(auction.seller, auction.lastPrice);
...
interface ICanvasAuction {
    ...
    function popOne(address _owner) external returns(bool);
    function getMyAmout(address _owner) external view returns(uint);
}
