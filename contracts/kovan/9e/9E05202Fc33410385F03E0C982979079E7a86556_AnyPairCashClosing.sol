...
contract AnyPairCashClosing {
    ...
    address public directPair;
    address public paymentPair;
    address public assetPair; 
    address payable public weth = 0xd0A1E359811322d97991E03f863a0C30C2cF029C; //wrapped kovan eth
    //uint public fee;
    ...
    constructor(address _asset, address _paymentCurrency) public {
        asset = _asset;
        paymentCurrency = _paymentCurrency;
        assetDecimals = IERC20(_asset).decimals();
        directPair = IUniswapV2Factory(uniFactory).getPair(asset, paymentCurrency);
        //if this direct pair exists - great all we need to do
        if (directPair != address(0x0)) {
            cashCloseOn = true;
        } else {
    ...
    function swapTo(uint amountOut, address to) public {
        ...
        swap(paymentPair, paymentCurrency, amountOut, wethIn, to); //swaps to send the just received wethIn and finally gets the USD Out
    }
    ...
    function swapFrom(uint amountIn, address to) public {
        ...
        swap(paymentPair, paymentCurrency, cashOut, wethOut, to); 
    }
    ...
    function getTo(uint amountOut) public view returns (uint amountIn, uint wethIn) {
        ...
        amountIn = estIn(wethIn, assetPair, asset);
    ...
    function getFrom(uint amountIn) public view returns (uint cashOut, uint wethOut) {
        ...
        wethOut = estCashOut(amountIn, assetPair, asset);
    ...
    function estCashOut(uint amountIn, address pair, address token) public view returns (uint amountOut) {
        ...
        address token0 = IUniswapV2Pair(pair).token0();
        amountOut = (token0 == token) ? UniswapV2Library.getAmountOut(amountIn, resA, resB) : UniswapV2Library.getAmountOut(amountIn, resB, resA);
    }

    function estIn(uint amountOut, address pair, address token) public view returns (uint amountIn) {
        (uint resA, uint resB, uint b) = IUniswapV2Pair(pair).getReserves();
        address token0 = IUniswapV2Pair(pair).token0();
        amountIn = (token0 == token) ? UniswapV2Library.getAmountIn(amountOut, resA, resB) : UniswapV2Library.getAmountIn(amountOut, resB, resA);
    }
