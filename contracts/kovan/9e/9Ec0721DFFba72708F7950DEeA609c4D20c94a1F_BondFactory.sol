...
/**
 * @dev Controller for a CAGS bond
 *
 ...
 * totalInterestRate = 1000
 * INTEREST_RATE_GRANULARITY = 1000
 *
 * Day 0:
 ...
 *
 * Day 15:
 * - AMPL supply stayed the same since Day 5
 * - tranche ratio is 20/30/50
...
contract BondController is IBondController, Initializable, AccessControl {
    ...
    uint256 private constant INTEREST_RATE_GRANULARITY = 1000;
    ...
    uint256 public interestSacrified;
    ...
    function init(
        ...
        // Create A' iou
        address iouTokenAddress = IIouFactory(_iouFactory).createIou("CAGS token", "IOU", address(_tranches[0].token));
        ious.push(iouTokenAddress);
        iouTokenAddresses[iouTokenAddress] = true;

        // Create Z' iou
        iouTokenAddress = IIouFactory(_iouFactory).createIou("CAGS token", "IOU", address(_tranches[1].token));
        ious.push(iouTokenAddress);
        iouTokenAddresses[iouTokenAddress] = true;
        ...
        dailyInterestRate = totalInterestRate / ((maturityDate - creationDate) / 1 days); // TODO this becomes zero if ((maturityDate - creationDate) / 1 days) > totalInterestRate
    }
    ...
    function deposit(uint256[] memory amounts) external override {
        ...
        // If always deposit with correct ratio, then can always transfer in 20 A and mint 20 A'
    ...
    function redeemMature(address iou, uint256 amount) external override {
        ...
        if (iou == _ious[0]) {
            totalInterest = getInterestOnRedeemMature(amount);
            ...
            TransferHelper.safeTransfer(
              ...
              _msgSender(),
              totalInterest + (amount * interestSacrified) / IIou(iou).totalSupply()
            );

            interestSacrified -= amount;

        } else if (iou == _ious[1]) {
            totalInterest = (amount * totalInterestRate) / INTEREST_RATE_GRANULARITY;
        ...
        // Burn after transferring so that Z tranche transferred is correct `totalInterest + (amount * interestSacrified) / IIou(iou).totalSupply()`
    ...
    function redeem(uint256[] memory amounts) external override {
        ...
        // If always redeem with correct ratio, then can always burn 20 A' and transfer out 20 A
        for (uint256 i = 0; i < amounts.length; i++) {
            require(
    ...
    function getInterestOnRedeemEmergency(uint256 amount) public override view returns (uint256) {
        return (amount * ((block.timestamp - creationDate) / 1 days) * dailyInterestRate * tranches[1].ratio) / (INTEREST_RATE_GRANULARITY * tranches[0].ratio);
    }
    ...
    function redeemEmergency(uint256 amount) external override {
        ...
        TransferHelper.safeTransferFrom(address(_tranches[0].token), address(this), _msgSender(), amount);
        TransferHelper.safeTransferFrom(address(_tranches[1].token), address(this), _msgSender(), totalInterestTillNow);

        interestSacrified += getInterestOnRedeemMature(amount) - totalInterestTillNow;
