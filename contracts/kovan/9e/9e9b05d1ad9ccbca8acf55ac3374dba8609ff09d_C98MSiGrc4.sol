...
contract C98MSiGrc4 {
    ...
    constructor(address[] memory owners_, uint16 requiredVote_) {
        ...
        if (requiredVote_ == 0 || requiredVote_ > _owners.length) {
            requirement.requiredVote = uint16(_owners.length);
        }
        else {
    ...
    function cancelRequest()
        ...
        VoteProgress memory progress = _voteProgress;
        require(progress.requestId > 0, "C98MSiG: No pending request");
        require(progress.timestamp < block.timestamp, "C98MSiG: Not enough timeout");
...
contract C98MSiGrc4Factory {
    function createMulitSig(address[] memory owners_, uint16 requiredVote_)
        public returns (C98MSiGrc4 wallet) {
        if (owners_.length == 0) {
            owners_ = new address[](1);
            owners_[0] = msg.sender;
        }
        wallet = new C98MSiGrc4(owners_, requiredVote_);
    }
}