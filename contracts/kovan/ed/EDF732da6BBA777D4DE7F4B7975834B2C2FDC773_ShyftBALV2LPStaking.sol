...
contract ShyftBALV2LPStaking is Ownable {
  ...
  function getPrice(address _tokenA, address _tokenB) public view returns (uint256 priceA, uint256 priceB) {
    address pair = UniswapV2Library.pairFor(factory, _tokenA, _tokenB);    
  ...
  function getTimeElapsed(address _tokenA, address _tokenB) external view returns (uint256) {
    address pair = UniswapV2Library.pairFor(factory, _tokenA, _tokenB);
    Observation storage firstObservation = getFirstObservationInWindow(pair);
