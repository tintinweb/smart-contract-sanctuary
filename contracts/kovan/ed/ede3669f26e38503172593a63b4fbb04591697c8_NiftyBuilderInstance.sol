...
contract NiftyBuilderInstance is ERC721Full, Owned {
    ...
    constructor(string memory _name, string memory _symbol, string memory base_uri, string memory name_of_creator) ERC721Full(_name, _symbol) public {
        baseURI = base_uri;
        nameOfCreator = name_of_creator;
    }

    function setNiftyIPFSHash(uint niftyType, string memory ipfs_hash) isOwner public {
    ...
    function closeContract() isOwner public {
        isClosed = true;
    }

    function batchGiftNifty(address[] memory collector_address, uint niftyType) isOwner public {
        for (uint i = 0; i < collector_address.length; i++) {
            giftNifty(collector_address[i], niftyType);
        }
    }

    function giftNifty(address collector_address, uint niftyType) isOwner public {
