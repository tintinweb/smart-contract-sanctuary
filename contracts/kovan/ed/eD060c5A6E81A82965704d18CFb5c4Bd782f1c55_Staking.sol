...
contract Staking is IERC721Receiver, Ownable, ReentrancyGuard {
    ...
    event Staked(address actionAddress, uint256 tokenId, uint256 stakeAmount, uint256 rewardPercent);
    event Withdrawn(address actionAddress, uint256 tokenId, uint256 stakeAmount, uint256 rewardPercent, uint256 stakeDuration, uint256 rewardAmount);

    event StakingStarted(address actionAddress);
    event StakingStopped(address actionAddress);

    event RewardPercentUpdated(address actionAddress, uint256 rewardPercent);
    event StakingNftAddressSet(address actionAddress, address stakingNftAddress);
    ...
    function stakingErc20Address() public view returns (address) {
        return _stakingErc20Address;
    }

    function rewardErc20Address() public view returns (address) {
        return _rewardErc20Address;
    }

    function stakingNftAddress() public view returns (address) {
        return _stakingNftAddress;
    }

    function stakingStarted() public view returns (bool) {
        return _stakingStarted;
    }

    function currentRewardPercent() public view returns (uint256) {
    ...
    function stats()
        ...
        returns (
            ...
            uint256 stakeErc20Balance,
            uint256 rewardErc20Balance
        )
    {
        stakeErc20Balance = IERC20(_stakingErc20Address).balanceOf(address(this));
        rewardErc20Balance = IERC20(_rewardErc20Address).balanceOf(address(this));
        return (
            _staked,
            _stakedAllTime,
            stakeErc20Balance,
        ...
        )
    {
        ...
        (startAt, stakeAmount, rewardPercent, ) = stakingNFT.getStakeTokenData(nftTokenId_);
        if (block.timestamp > startAt && stakeAmount != 0 && rewardPercent != 0) {
    ...
    function withdraw(uint256 nftTokenId_) public nonReentrant returns (bool) {
        ...
        ( , uint256 stakeAmount, uint256 rewardPercent, uint256 stakeDuration, uint256 rewardAmount) = calculateReward(nftTokenId_);
        ...
        IERC20 rewardERC20 = IERC20(_rewardErc20Address);
        rewardERC20.transfer(_msgSender(), rewardAmount);
    ...
    function startStaking() external onlyOwner {
        ...
        require(_stakingNftAddress != address(0), "Staking: stakingNftAddress is not set");
        _stakingStarted = true;
        emit StakingStarted(_msgSender());
    }

    function stopStaking() external onlyOwner {
        require(_stakingStarted, "Staking: Staking already stopped");
        _stakingStarted = false;
        emit StakingStopped(_msgSender());
    }

    function updateRewardPercent(uint256 rewardPercent_) public onlyOwner {
        require(rewardPercent_ > 0 && rewardPercent_ <= 10000, "Staking: Invalid rewardPercent");
        _rewardPercent = rewardPercent_;
        emit RewardPercentUpdated(_msgSender(), rewardPercent_);
    }

    function setStakingNftAddress(address stakingNftAddress_) external onlyOwner {
        require(stakingNftAddress_ != address(0), "Staking: Invalid stakingNftAddress");
        require(_stakingNftAddress == address(0), "Staking: StakingNftAddress has already been set");

        IStakingNFT stakingNFT = IStakingNFT(stakingNftAddress_);
        require(stakingNFT.minterAddress() == address(this), "Staking: Invalid minterAddress in stakingNFT");

        _stakingNftAddress = stakingNftAddress_;
        emit StakingNftAddressSet(_msgSender(), stakingNftAddress_);
    }
    ...
    function withdrawAnyErc20(
        ...
        returns (bool)
    {
        IERC20 erc20Token = IERC20(erc20Address_);
        return erc20Token.transfer(addressTo_, amount_);
    }
}
