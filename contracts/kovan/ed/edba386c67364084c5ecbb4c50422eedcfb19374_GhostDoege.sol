...
contract GhostDoege is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "GhostDoege";
    ...
    uint256 public _redistributionAtDay = 300;
    
    uint256 public _liquidityFeeAtDay = 200;
    uint256 public _marketingFeeAtDay = 200;
    uint256 public _buybackFeeAtDay = 200;

    uint256 public _redistributionAtNight = 500;
    uint256 public _liquidityFeeAtNight = 100;
    ...
    function collectFee(address account, uint256 amount, uint256 rate) private returns (uint256) {
        ...
        //@dev tax fee
        if(_taxFee != 0){
            isNight() ? _taxFee = 500 : _taxFee = 200;
        ...
        //@dev liquidity fee
        if(_liquidityFee != 0){
            isNight() ? _liquidityFee = 100 : _liquidityFee = 600;
        ...
        //@dev burn fee
        if(_burnFee != 0){
            isNight() ? _burnFee = 100 : _burnFee = 200;
    ...
    function isNight() private returns (bool) {
        uint hoursDate = DateTime.getHour(block.timestamp);
        if(hoursDate > 18 &&  hoursDate < 23 ){
            return false;
        } else {
            return true;
        }
