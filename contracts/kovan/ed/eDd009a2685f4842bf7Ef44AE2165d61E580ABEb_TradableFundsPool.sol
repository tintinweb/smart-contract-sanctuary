...
contract TradableFundsPool is Ownable {
    ...
    function updateInvestorsOwnership(int256 percentageIncrease, address sender)
        internal
    {
        // amountOwnership[msg.sender] = percentageIncrease;
        for (uint256 i = 0; i < investorList.length; i++) {
            address curInvestor = investorList[i];
            int256 curAmountOwnership = int256(amountOwnership[curInvestor]);
            int256 newAmountOwnership;
            newAmountOwnership =
                ((10**18 - percentageIncrease) * curAmountOwnership) /
                (10**18);
            if (curInvestor != sender) {
                amountOwnership[curInvestor] = uint256(newAmountOwnership);
            } else {
                newAmountOwnership += percentageIncrease;
                amountOwnership[curInvestor] = uint256(newAmountOwnership);
            }
        }
    }
    ...
    function withdraw() public {
        ...
        updateInvestorsOwnership(-1 * int256(senderOwnership), msg.sender);
    }
