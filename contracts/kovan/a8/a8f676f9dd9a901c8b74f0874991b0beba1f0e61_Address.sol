...
pragma solidity ^0.6.12;
    ...
    contract MuskV3 is Context, IERC20, Ownable {
        ...
        string private _name = 'MuskRat V3';
        string private _symbol = 'MuskV3';
        uint8 private _decimals = 9;

        // Tax and fees will start at 0 so we don't have a big impact when deploying to Uniswap
        // fees wallet address is null but the method to set the address is exposed
            ...
            if(sender != owner() && recipient != owner())
              ...
              if(sender != owner() && recipient != uniswapV2Pair) {
                  ...
                  if (!_isExcluded[sender]) {
                          ...
                          require(block.timestamp >= timestamp[sender], "Everyone is vested! You cannot sell more than 25% of your stack once per 90 mins");
                          ...
                          if (getSwapPercent(balanceOf(sender),_maxTxAmount) > 10 ){

                              //now we know user has higher than 10% of max tx allowed, otherwise we can just let them swap as amount is tiny
                              require(getSwapPercent(amount, balanceOf(sender)) <= 25, "You cannot trade more than 25% per swap");
                          }

                          //add a 90 minute timestamp to current block until next swap
                          timestamp[sender] = block.timestamp.add(90 minutes);
