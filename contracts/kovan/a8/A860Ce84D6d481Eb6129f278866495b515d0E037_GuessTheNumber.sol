...
contract GuessTheNumber is Ownable, VRFConsumerBase, KeeperCompatibleInterface {
    ...
    event winnerPayed(address indexed winner, uint256 amount, uint indexed roundNumber);
    event startingNewRound(uint indexed roundNumber, string message);

    uint public immutable interval;
    uint public lastTimeStamp;

    constructor(address vrfCoordinator, address link, bytes32 keyHash, uint256 fee, uint setInterval) VRFConsumerBase(vrfCoordinator, link) {
        s_keyHash = keyHash;
        s_fee = fee;
    ...
    function performUpkeep(bytes calldata /*performData*/) external override {
        ...
        //five minutes to pay winners and reset array of voters
        getRandomNumber();
        payWinners();
        resetLottery();
        emit startingNewRound(roundNumber + 1, "Starting a new round");   
    }

    function setUsersGuess(uint256 _guess) public payable {
        require(!users[msg.sender].hasGuessed, "User already guessed!");
        require(msg.value == 100 wei, "Amount should be equal to 100 Wei!");
    ...
    function getUsersGuess(address _id) public view returns (uint256){
        require(users[_id].hasGuessed, "User has not guessed!");
        return users[_id].guess;
    }
    ...
    function getNumberOfWinners() internal view returns(uint256) {
        ...
        for (uint256 i = 0; i < usersWhoHaveVoted.length; i++) {
            if (users[usersWhoHaveVoted[i]].guess == randomNumber){
                count++;
            }
        }
    ...
    function payWinners() private {
        ...
        for (uint256 i = 0; i < winners.length; ++i) {
            ...
            emit winnerPayed(winners[i], amountPerWinner, roundNumber);
        } 

    }

    function getRandomNumber() private onlyOwner returns (bytes32 requestId) {
        require(LINK.balanceOf(address(this)) >= s_fee, "Not enough LINK to pay fee");
        return requestRandomness(s_keyHash, s_fee);
    }

    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        randomNumber = (randomness % 10) + 1;
    }
}
