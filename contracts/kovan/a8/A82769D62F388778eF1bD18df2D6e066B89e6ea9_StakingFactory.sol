...
contract StakingFactory is AccessControl, ReentrancyGuard, IStakingFactory {
    ...
    function createStaking(address token) public onlyManagerOrAdmin isInitialize {
    ...
    function inputReward(Reward[] calldata rewards) external onlyManagerOrAdmin isInitialize nonReentrant {
        uint256 totalReward;
    ...
    function claimReward(uint256 index) external isInitialize nonReentrant {
        address sender = _msgSender();
        uint256 amount = _claimReward(index);
    ...
    function changeFreezeTime(uint256 _freezeTime) external {
    ...
    function _claimReward(uint256 index) private returns (uint256 amount) {
        address sender = _msgSender();
    ...
    function getAvailToClaim(address pool) external view returns (uint256) {
        address sender = _msgSender();
        try IStaking(pool).availDividends(sender) returns (uint256 dividends) {
            return dividends;
        } catch (bytes memory) {
            return 0;
        }
