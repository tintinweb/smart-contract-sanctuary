...
contract BasementStaking is Ownable {
	...
	*/
	...
	function upgradeDweller(address nftContract, uint256 tokenID, uint256 expAmount) external {
		...
		require(recieve20Payment(msg.sender, expAmount), "DwellersStaking: Need to spend EXP to upgrade");
	...
	function unstakeDweller(address nftContract, uint256 tokenID) external {
		...
		if(_stake.lockAccumlation < stakingProps.maxStakingS){
			require(recieve20Payment(msg.sender, calcUnstakingFee(_stake.lockAccumlation, stakingProps.tsForReward, stakingProps.tsReward)), "DwellersStaking: need to pay 33% of accumlated tokens to unstake early");
		}
