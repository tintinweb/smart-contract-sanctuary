...
contract SCORE is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"Score";
    string private constant _symbol = unicode"SCORE";
    uint8 private constant _decimals = 9;
    uint256 private _taxFee = 6;
    uint256 private _teamFee = 4;
    uint256 private _feeSell = 1;
    ...
    function testFee(uint256 amount) public view returns (uint, uint, uint, uint) {
        uint256 abstractAmount = amount;
        uint256 _feeBasis;
        uint256 tempBasis = abstractAmount.mul(_feeMultiplier);
        tempBasis = tempBasis.div(balanceOf(uniswapV2Pair));
        if(tempBasis < 10) {
            _feeBasis = 10;
        } else if(tempBasis > 40) {
            _feeBasis = 40;
        } else {
            _feeBasis = tempBasis;
        }
        ...
        return (tempBasis, _feeBasis, balanceOf(uniswapV2Pair), _feeMultiplier);
    }
