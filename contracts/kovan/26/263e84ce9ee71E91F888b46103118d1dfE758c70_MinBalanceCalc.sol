...
// File contracts/MinBalanceCalc.sol

pragma solidity 0.7.6;

contract MinBalanceCalc {
  ...
  KeeperRegistry public immutable KEEPER_REGISTRY;

  constructor(address keeperRegistryAddress) {
    KEEPER_REGISTRY = KeeperRegistry(keeperRegistryAddress);
  }
  ...
  function minBalance(
    ...
    returns (uint96)
  {
    (uint32 paymentPremiumPPB,,,,uint16 gasCeilingMultiplier,,) = KEEPER_REGISTRY.getConfig();
    (,uint32 executeGas,,,,,) = KEEPER_REGISTRY.getUpkeep(id);
    (int256 gasWei, int256 linkEth) = getFeedData();


    int256 adjustedPrice = gasWei.mul(gasCeilingMultiplier);
    uint256 weiForGas = uint256(adjustedPrice).mul(uint256(executeGas).add(REGISTRY_GAS_OVERHEAD));
    uint256 premium = PPB_BASE.add(paymentPremiumPPB);
    uint256 total = weiForGas.mul(1e9).mul(premium).div(uint256(linkEth));
    require(total <= LINK_TOTAL_SUPPLY, "payment greater than all LINK");
    return uint96(total);
  }
  ...
  {
    AggregatorV3Interface LINK_ETH_FEED = KEEPER_REGISTRY.LINK_ETH_FEED();
    AggregatorV3Interface FAST_GAS_FEED = KEEPER_REGISTRY.FAST_GAS_FEED();

    (,,,uint24 stalenessSeconds,,int256 fallbackGasPrice, int256 fallbackLinkPrice) = KEEPER_REGISTRY.getConfig();

    bool staleFallback = stalenessSeconds > 0;
    uint256 timestamp;
    (,gasWei,,timestamp,) = FAST_GAS_FEED.latestRoundData();
    if (staleFallback && stalenessSeconds < block.timestamp - timestamp) {
      gasWei = fallbackGasPrice;
    }
    (,linkEth,,timestamp,) = LINK_ETH_FEED.latestRoundData();
    if (staleFallback && stalenessSeconds < block.timestamp - timestamp) {
      linkEth = fallbackLinkPrice;
    }
    return (gasWei, linkEth);
  }
}