...
contract TokenFarm is Ownable{ 
    ...
    function getUserTVL(address _user) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < allowedTokens.length; i++) {
            tvl = tvl + getUserSingleTokenValue(_user, allowedTokens[i]);
        }
        return tvl;
    }
    ...
    function stakeTokens(uint256 _amount, address _token) public {
        ...
        require(isAllowed(_token), "Token is not allowed");
        IERC20(_token).transferFrom(msg.sender, address(this), _amount);
        addToUniqueTokensStaked(msg.sender, _token);
    ...
    function addToUniqueTokensStaked(address _staker, address _token) internal {
