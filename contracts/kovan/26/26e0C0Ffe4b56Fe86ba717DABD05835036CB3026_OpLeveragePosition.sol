...
contract OpLeveragePosition is OpCommon {
    ...
    function openLong(
        ...
        // uint256 _amt;
        // if (leverageToken != ethAddr) {
        //     IERC20 tokenContract = IERC20(leverageToken);
        //     _amt = amountLeverageToken == type(uint256).max
...
interface ProtocolAaveV2Interface {
    function depositToken(address token, uint256 amt) external payable returns (uint256 _amt);
    function withdrawToken(address token, uint256 amt) external payable returns (uint256 _amt);
    function borrowToken( address token, uint256 amt, uint256 rateMode ) external payable returns (uint256 _amt);
    function paybackToken( address token, uint256 amt, uint256 rateMode ) external payable returns (uint256 _amt);
    function enableTokenCollateral(address[] calldata tokens) external payable;
    function swapTokenBorrowRateMode(address token, uint256 rateMode) external payable;
}
