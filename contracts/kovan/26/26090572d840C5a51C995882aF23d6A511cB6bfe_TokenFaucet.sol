...
contract TokenFaucet is BaseRelayRecipient, OwnableUpgradeable, TokenListener {
  ...
  /// @notice Timestamp after which the users can claim their staking rewards.
  uint256 public rewardsUnlockTime;
  ...
  /// @notice Initializes a new TokenFaucet
  ...
  /// @param _forwarder The address of trusted forwarder
   function initialize (
  ...
  function deposit(uint256 amount) external {
    ...
    emit Deposited(msgSender(), amount);
  }
  ...
  function claim(address user) external returns (uint256) {
    require(block.timestamp >= rewardsUnlockTime, "TokenFaucet/rewards-locked");
    drip();
  ...
  /// @notice Allows the owner to set the staking rewards unlock time. 
  /// @param _newUnlockTime The new timestamp until which the rewards should be locked.
  function setRewardsUnlockTime(uint256 _newUnlockTime) public onlyOwner {
    require(_newUnlockTime > 0, "TokenFaucet/timestamp-gt-zero");

    rewardsUnlockTime = _newUnlockTime;

    emit UnlockTimeChanged(_newUnlockTime);
  }
