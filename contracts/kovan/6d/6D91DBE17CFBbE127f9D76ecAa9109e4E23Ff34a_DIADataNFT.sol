...
contract DIADataNFT is Ownable, ERC1155 {
	...
	uint256 public numCategoriesCreated = 0;

	struct DataNFTCategory {
		uint256 numMintedNFTs;
		mapping (uint256 => address) rankOwner;
	...
	mapping (uint256 => DataNFTCategory) public dataNFTCategories;
	bool public genesisPhase = true;
	uint256 private seed;

	constructor(address _paymentToken, uint256 _burnAmount, uint256 _mintingPoolAmount, address _mintingPool, uint256 _seed) ERC1155("https://api.diadata.org/v1/nft/data_{id}.json") {
	...
	event NewDataNFTCategory(uint256 dataCategoryId, uint256 sourceNFTId);
	event MintedDataNFT(uint256 dataCategoryId, address owner, uint256 numMinted, uint256 newRank);
	event ClaimedMiningPoolReward(uint256 dataCategoryId, uint256 rank, address claimer, uint lastClaimPayout);
	...
	function generateDataNFTCategory(uint256 sourceNFTId) external {
		require(diaSourceNFTImpl.balanceOf(msg.sender, sourceNFTId) > 0);
		uint256 dataCategoryId = numCategoriesCreated;
		dataNFTCategories[dataCategoryId].exists = true;
		dataNFTCategories[dataCategoryId].sourceNFTId = sourceNFTId;
		numCategoriesCreated = numCategoriesCreated + 1;
		emit NewDataNFTCategory(dataCategoryId, sourceNFTId);
	}

	function mintDataNFT(uint256 dataCategoryId) external {
		require(!genesisPhase);
		_mintDataNFT(dataCategoryId);
	}

	function mintGenesisDataNFT() external {
		require(genesisPhase);
		uint256 dataCategoryId = NFTUtil.randInt(numCategoriesCreated, seed);
		while(!dataNFTCategories[dataCategoryId].exists) {
			dataCategoryId = (dataCategoryId + 1) % (numCategoriesCreated);
		}
		_mintDataNFT(dataCategoryId);
	}

	function _mintDataNFT(uint256 dataCategoryId) internal {
		// Check that categoryId exists
		//DataNFTCategory storage currNFTCategory  = dataNFTCategories[dataCategoryId];
		require(dataNFTCategories[dataCategoryId].exists, "_mintDataNFT.Category must exist");

		// Get current number minted of the NFT
		uint256 numMinted = dataNFTCategories[dataCategoryId].numMintedNFTs;
		...
		if (numMinted < NUM_PRIVILEGED_RANKS) {
			...
			while (dataNFTCategories[dataCategoryId].rankOwner[newRank] != address(0)) {
		...
		// Mint data NFT
		_mint(msg.sender, dataCategoryId, 1, "");

		// Update data struct
		dataNFTCategories[dataCategoryId].rankOwner[newRank] = msg.sender;
		dataNFTCategories[dataCategoryId].numMintedNFTs = numMinted + 1;

		// Update Source NFT data
		uint256 currSourceNFTId = dataNFTCategories[dataCategoryId].sourceNFTId;
		...
		emit MintedDataNFT(dataCategoryId, msg.sender, numMinted, newRank);
	}

	function getRankOwner(uint256 dataCategoryId, uint256 rank) external view returns (address) {
	    return dataNFTCategories[dataCategoryId].rankOwner[rank];
	}
	function getLastClaimPayout(uint256 dataCategoryId, uint256 rank) external view returns (uint) {
	    return dataNFTCategories[dataCategoryId].lastClaimPayout[rank];
	}

	function claimMintingPoolReward(uint256 dataCategoryId, uint256 rank) public {
		address claimer = msg.sender;
		require(dataNFTCategories[dataCategoryId].rankOwner[rank] == claimer);

		uint256 numMinted = NUM_PRIVILEGED_RANKS;
		if (dataNFTCategories[dataCategoryId].numMintedNFTs > NUM_PRIVILEGED_RANKS) {
			numMinted = dataNFTCategories[dataCategoryId].numMintedNFTs;
		}
		uint256 reward = getRewardAmount(rank, numMinted, dataNFTCategories[dataCategoryId].lastClaimPayout[rank]);

		// transfer reward to claimer
		require(ERC20(paymentToken).transferFrom(mintingPool, claimer, reward));
		dataNFTCategories[dataCategoryId].lastClaimPayout[rank] = numMinted;
		emit ClaimedMiningPoolReward(dataCategoryId, rank, claimer, numMinted);
	}

	function getRewardAmount(uint256 position, uint256 numMinted, uint256 numMintedLastClaim) public view returns (uint) {
		require(!genesisPhase);
		// 1. Calculate reward from beginning
		// Never get tetraeder(0), because that is 0 and we don't want to give no reward
		uint256 maxNumMinted = Math.max(NUM_PRIVILEGED_RANKS, numMinted);
		uint256 ownRawWeight = getTetraederNumber(maxNumMinted - position);
		...
		uint256 ownRawWeightLastClaim = getTetraederNumber(numMintedLastClaim - position);
	...
	function getMintedNFTs(uint256 dataCategoryId) external view returns (uint) {
		return dataNFTCategories[dataCategoryId].numMintedNFTs;
	}
}
...
contract DIASourceNFT is Ownable, ERC1155 {
	...
	address public paymentToken;
	address public dataNFTContractAddress;
	...
	function updateDataNFTContractAddress(address newAddress) onlyOwner external {
	...
	function notifyDataNFTMint(uint256 sourceNftId) external {
		require(msg.sender == dataNFTContractAddress, "notifyDataNFTMint: Only data NFT contract can be used to mint data NFTs");
