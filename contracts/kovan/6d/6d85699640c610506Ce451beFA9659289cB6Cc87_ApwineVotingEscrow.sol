...
contract ApwineVotingEscrow is MerkleDistributor, OwnableUpgradeable {
    ...
    function getNextEpochStart() external view returns (uint256) {
        ...
        uint gracePeriodEnd = getGracePeriodEnd();
        return epoch == 0 ? gracePeriodEnd : gracePeriodEnd.add(epochDuration.mul(epoch));
    }

    function getTimeUntilNextEpoch() external view returns (uint256) {
        uint256 epoch = getCurrentEpoch();
        uint256 gracePeriodEnd = getGracePeriodEnd();
        return epoch == 0 ? gracePeriodEnd.sub(block.timestamp) :
        gracePeriodEnd.add(epochDuration.mul(epoch)).sub(block.timestamp);
    }
    ...
    function getCurrentRewardsRate() public view returns (uint256) {
        ...
        return epoch > finalEpoch ? 0 : MAX_BPS - (epoch.mul(rewardReductionPerEpoch));
    }
        ...
        if (claimable > 0) {
            ...
            if (tokensToLock > 0) {
                ...
                require(
                    ...
                    "ApwineVotingEscrow: Not enough allowance to veAPW."
                );
                require(
                    IERC20Upgradeable(token).transfer(account, tokensToLock),
                    "Lockable transfer to user failed."
                );
