...
contract nftToken is ERC721Enumerable, ReentrancyGuard, Ownable {
    constructor() ERC721("loot", "loot") {
        _nftAddress = 0x57dF2d9bFc8F4F8e69cC45c9445853cCB3c4Fb58;
        feeAmount = 0;
        _tokenId = 0;
    }
    ...
    function receiveToken() public onlyOwner nonReentrant {
    ...
    function discoverMap() public payable nonReentrant {
        address _msgSender = _msgSender();
        require(
            balanceOf(_msgSender) <= 20,
            "user have 20 tokens counld not mint"
        );
        require(_tokenId <= 10000, "user can mint 9000 tokens only");

        if (_tokenId > 6000) {
            require(
                msg.value >= 60000000000000000,
                "does not give enough eth to constract"
            );
            ...
            ERC721 tempToken = ERC721(_nftAddress);
            if (tempToken.balanceOf(_msgSender) > 0) {
                _safeMint(_msgSender, _tokenId++);
            } else {
                require(false, "user does not have map token");
            }
        }
        feeAmount += msg.value;
    }

    function ownerDiscoverMap() public nonReentrant onlyOwner {
