...
contract PriceOracleAggregator is PriceOracle {
    ...
    function getUnderlyingPrice(PToken pToken) public view returns (uint256) {
        ...
        (uint256 price,) = IAssetPrice(priceOracle).getPriceUSD(asset);
...
contract PErc20 is PToken, PErc20Interface {
    ...
    function initialize(
        ...
        super.initialize(comptroller_, interestRateModel_, pubMiningRateModel_, initialExchangeRateMantissa_, name_, symbol_, decimals_);
    ...
    function doTransferIn(address from, uint256 amount) internal returns (uint256) {
        ...
        //这个token是erc20，里面的transferFrom方法
        //这个方法与普通合约里面某一个地址的transfer方法不一样
        //这个是合约成员方法调用
        //没错，另外一种是以太坊的转移，直接addr.tansfer()，然后由合约支付
        //TODO 这里transferFrom的sucess没见到赋值啊？？
    ...
    function setWhiteList(address trustList) public returns (uint256){
        return setWhiteListInternal(trustList);
    }
    ...
    function doCreditLoanBorrow(
        ...
        return doCreditLoanBorrowInternal(_trust, _borrower, _borrowAmount);
    }
    ...
    function doCreditLoanRepay(
        ...
        return doCreditLoanRepayInternal(_trust, _payer, _repayAmount);
    }
}