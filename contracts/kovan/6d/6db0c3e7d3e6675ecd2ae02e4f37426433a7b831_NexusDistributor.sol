...
contract NexusDistributor is AbstractDistributor,
    ERC721Upgradeable,
    // OwnableUpgradeable,
    ...
    IDistributor
{
  ...
  */
    ...
    function getCoverCount(address _owner, bool _isActive) external view override returns (uint256){
    ...
    function getCover(
        ...
        if (_isActive == true) {
            cover = _nexusGetCoversBoughtFromNexusDirectly(_owner, _coverId, _loopLimit);
        } else {
            cover = _nexusGetCoversBoughtFromNexusDistributor(_owner, _coverId, _loopLimit);
        }
        return cover;
    }

    function _nexusGetCoversBoughtFromNexusDirectly(
        address _owner,
        uint256 _coverId,
        uint256 _loopLimit
    ) internal view returns (Cover memory) {
        uint[] memory userCovers = _nexusGetAllCoversOfUser(_owner);
        uint limit = userCovers.length.min(_loopLimit);

        Cover[] memory covers = new Cover[](userCovers.length);

        for (uint id = 0; id < limit; id++) {
            Cover memory cover;
            (uint8 status,
            uint256 sumAssured,
            uint16 coverPeriod,
            ...
            address memberAddress) = getCover(id);

            cover.status = status;
            cover.coverAmount = sumAssured;
            cover.productId = coverPeriod;
            cover.expiration = validUntil;
            cover.contractAddress = contractAddress;
            cover.currency = coverAsset;
            cover.premium = premiumInNXM;
            cover.refAddress = memberAddress;
    ...
    function _nexusGetCoversBoughtFromNexusDistributor(
        address _owner,
        uint256 _coverId,
        uint256 _loopLimit
    ) internal view returns (Cover memory) {
        uint ownerBalance = balanceOf(_owner);
        uint limit = ownerBalance.min(_loopLimit);

        Cover[] memory covers = new Cover[](ownerBalance);
        ...
        for (uint i = 0; i < limit; i++) {
            ...
            address coverAsset,
            uint256 premiumInNXM,
            address memberAddress) = getCover(tokenId);
    ...
    function _nexusGetAllCoversOfUser(address _add) internal view returns (uint[] memory) {
        address nexusQuotationDataAddress = master.getLatestAddress("QD");
        return IQuotationData(nexusQuotationDataAddress).getAllCoversOfUser(_add);
    }
    ...
    function getQuote(
        ...
        address nexusGatewayAddress = master.getLatestAddress("GW");
        IQuotationData quoteData = IQuotationData(master.getLatestAddress("QD"));


        uint256 coverPrice = IGateway(nexusGatewayAddress).getCoverPrice(
            _contractAddress,
            _coverAsset,
            _sumAssured,
            uint16(_coverPeriod),
            IGateway.CoverType(1),
            _data
        );

        CoverQuote memory coverQuote;
        coverQuote.prop1 = feePercentage;
        coverQuote.prop2 = coverQuote.prop1.mul(coverPrice)
        .div(10000)
        .add(coverPrice);
        coverQuote.prop3 = quoteData.getCoverLength();
        coverQuote.prop4 = quoteData.getTotalSumAssuredSC(_nexusCoverable, "DAI");
        coverQuote.prop5 = quoteData.getTotalSumAssuredSC(_nexusCoverable, "ETH");
        coverQuote.prop6 = quoteData.getTotalSumAssured("DAI");
        coverQuote.prop7 = quoteData.getTotalSumAssured("ETH");
   ...
   function buyCover(
        ...
        if (coverAsset == ETH) {
            ...
            (buyCoverValue, ) = _defineBuyingCoverValue(coverAsset,data);
        } else {
            ...
            (buyCoverValue,  coverAsset) = _defineBuyingCoverValue(coverAsset,data);
            IERC20Upgradeable(coverAsset).approve(address(gateway), buyCoverValue);
        }
    ...
    function buyCoverInsurace(IDistributor.BuyInsuraceQuote memory) external payable override nonReentrant {
        revert("Unsupported operation");
    }

    function supportsInterface(bytes4 interfaceId) public view override (IERC165Upgradeable,ERC165Upgradeable) returns (bool) {
        ...
        )
    {
        ...
        if (payoutToken == ETH) {
            ...
            ) = address(msg.sender).call{value: payoutAmount}("");
  ...
  */
        ...
        returns (bytes memory response, uint256 withheldAmount)
    {
        ...
        if (asset == ETH) {
            ...
            ) = address(msg.sender).call{value: ethRemainder}("");
            require(
                ok,
                "Distributor: Returning ETH remainder to sender failed."
            );
     ...
     */
    function approveNXM(address spender, uint256 amount) public  {
     ...
     */
    function withdrawNXM(address recipient, uint256 amount) public  {
     ...
     */
    function sellNXM(uint256 nxmIn, uint256 minEthOut) external  {
    ...
    function _defineBuyingCoverValue(
        address _coverAsset,
        bytes calldata _data
    ) internal returns(uint256 _buyingCoverValue,address _buyingCoverAsset){

        // Get cover price in "coverAsset" & NXM
        (uint256 _priceInCoverAsset, uint256 _priceInNXM ) = abi.decode(_data,(uint256, uint256 ));
       
        // If cover price in NXM is less than cover price in "coverAsset", then buy wNXM.
        if(_priceInNXM < _priceInCoverAsset){
            uint256 _priceInWNXM = _buyWNXM(_coverAsset,_priceInNXM);
            _unwrapWNXM(_priceInWNXM);
            uint256 _discountEarnings = IERC20Upgradeable(_coverAsset).balanceOf(address(this));
            _buyingCoverValue = _priceInCoverAsset;
            _buyingCoverAsset = address(nxmToken);
            transferToTreasury(_discountEarnings, _coverAsset);
        }
        _buyingCoverValue = _priceInCoverAsset;
        _buyingCoverAsset = _coverAsset;
    }

    function _abstractDistributor_init(
        address _uniswapRouterAddress,
        address _wethTokenAddress
    ) external  {
        __AbstractDistributor_init(_uniswapRouterAddress,_wethTokenAddress);
    }   

    function setwNXMToken(address _tokenAddress) external {
        wNXMToken = IWNXMToken(_tokenAddress);
    }
    function _buyWNXM(address _buyingAsset,uint256 _amount) internal returns (uint256) {
        _checkApprovals(address(_buyingAsset), _amount);
        return _swapTokenForToken(_amount, address(_buyingAsset), address(wNXMToken));
    }
    function _unwrapWNXM(uint256 _amount) internal {
        wNXMToken.unwrap(_amount);
    }
}
...
import "@openzeppelin/contracts-upgradeable/token/ERC20/IERC20Upgradeable.sol";
import "./nexus/utils/PreciseUnitMath.sol";
...
abstract contract AbstractDistributor {
    ...
    function __AbstractDistributor_init(
        address _uniswapRouterAddress,
        address _wethTokenAddress
    ) internal {
    ...
    function _checkApprovals(
        ...
        if (IERC20Upgradeable(_asset).allowance(address(this), address(_uniswapRouter)) < _amount) {
            IERC20Upgradeable(_asset).approve(address(_uniswapRouter), PreciseUnitMath.MAX_UINT_256);
        }
    }
}
