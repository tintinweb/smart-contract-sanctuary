...
contract PesaAdminStorage {
    ...
    */
    ...
    /**
    * @notice Active brains of Pesa
    */
    address public implementation;

    /**
    * @notice Pending brains of Pesa
    */
    address public pendingImplementation;
}

contract PControllerV1Storage is PesaAdminStorage {
...
contract PControllerV2Storage is PControllerV1Storage {
    struct Market {
        ...
        /// @notice Whether or not this market receives PESA
        bool isPesa;
    }
...
contract PControllerV3Storage is PControllerV2Storage {
    struct PesaMarketState {
        /// @notice The market's last updated pesaBorrowIndex or pesaSupplyIndex
        uint224 index;
    ...
    uint public pesaRate;

    /// @notice The portion of pesaRate that each market currently receives
    mapping(address => uint) public pesaSpeeds;

    /// @notice The COMP market supply state for each market
    mapping(address => PesaMarketState) public pesaSupplyState;

    /// @notice The COMP market borrow state for each market
    mapping(address => PesaMarketState) public pesaBorrowState;

    /// @notice The COMP borrow index for each market for each supplier as of the last time they accrued COMP
    mapping(address => mapping(address => uint)) public pesaSupplierIndex;

    /// @notice The COMP borrow index for each market for each borrower as of the last time they accrued COMP
    mapping(address => mapping(address => uint)) public pesaBorrowerIndex;

    /// @notice The COMP accrued but not yet transferred to each user
    mapping(address => uint) public pesaAccrued;
}

/**
 * @title PeasCore
 * @dev Storage for the pController is at this address, while execution is delegated to the `implementation`.
 * PTokens should reference this contract as their pController.
 */
contract PesaCore is PesaAdminStorage, PControllerErrorReporter {
    ...
    /**
      * @notice Emitted when pendingImplementation is accepted, which means pController implementation is updated
      */
    event NewImplementation(address oldImplementation, address newImplementation);
    ...
    /**
    * @notice Accepts new implementation of pController. msg.sender must be pendingImplementation
...
/**
 * @title Pesa's PController Contract
 */
contract PController is PControllerV3Storage, PControllerInterface, PControllerErrorReporter, Exponential {
    ...
    /// @notice Emitted when market pesa status is changed
    event MarketPesa(PToken pToken, bool isPesa);

     /// @notice Emitted when PESA address is changed
    event NewPesaAddress(address oldPesaAddress, address newPesaAddress);

    /// @notice Emitted when PESA rate is changed
    event NewPesaRate(uint oldPesaRate, uint newPesaRate);

    /// @notice Emitted when a new PESA speed is calculated for a market
    event PesaSpeedUpdated(PToken indexed pToken, uint newSpeed);

    /// @notice Emitted when PESA is distributed to a supplier
    event DistributedSupplierPesa(PToken indexed pToken, address indexed supplier, uint pesaDelta, uint pesaSupplyIndex);

    /// @notice Emitted when PESA is distributed to a borrower
    event DistributedBorrowerPesa(PToken indexed pToken, address indexed borrower, uint pesaDelta, uint pesaBorrowIndex);

    /// @notice The threshold above which the flywheel transfers PESA, in wei
    uint public constant pesaClaimThreshold = 0.001e18;

    /// @notice The initial PESA index for a market
    uint224 public constant pesaInitialIndex = 1e36;
    ...
    function mintAllowed(address pToken, address minter, uint mintAmount) external returns (uint) {
        ...
        updatePesaSupplyIndex(pToken);
        distributeSupplierPesa(pToken, minter, false);
    ...
    function redeemAllowed(address pToken, address redeemer, uint redeemTokens) external returns (uint) {
        ...
        // Keep the flywheel moving
        updatePesaSupplyIndex(pToken);
        distributeSupplierPesa(pToken, redeemer, false);
    ...
    function borrowAllowed(address pToken, address borrower, uint borrowAmount) external returns (uint) {
        ...
        updatePesaBorrowIndex(pToken, borrowIndex);
        distributeBorrowerPesa(pToken, borrower, borrowIndex, false);
    ...
    function seizeAllowed(
        ...
        if (PToken(pTokenCollateral).pController() != PToken(pTokenBorrowed).pController()) {
            return uint(Error.PCONTROLLER_MISMATCH);
        }

        // Keep the flywheel moving
        updatePesaSupplyIndex(pTokenCollateral);
        distributeSupplierPesa(pTokenCollateral, borrower, false);
        distributeSupplierPesa(pTokenCollateral, liquidator, false);
    ...
    function transferAllowed(address pToken, address src, address dst, uint transferTokens) external returns (uint) {
        ...
        // Keep the flywheel moving
        updatePesaSupplyIndex(pToken);
        distributeSupplierPesa(pToken, src, false);
        distributeSupplierPesa(pToken, dst, false);
    ...
    function getHypotheticalAccountLiquidityInternal(
        ...
        for (uint i = 0; i < assets.length; i++) {
            ...
            // Pre-pesaute a conversion factor from tokens -> ether (normalized price value)
    ...
    /**
      * @notice Sets a new price oracle for the pController
    ...
    function _setPriceOracle(PriceOracle newOracle) public returns (uint) {
        ...
        // Track the old oracle for the pController
        PriceOracle oldOracle = oracle;

        // Set pController's oracle to newOracle
        oracle = newOracle;
    ...
    function _supportMarket(PToken pToken) external returns (uint) {
        ...
        markets[address(pToken)] = Market({isListed: true, isPesa: false, collateralFactorMantissa: 0});
    ...
    function _become(PesaCore pesaCore) public {
        require(msg.sender == pesaCore.admin(), "only pesaCore admin can change brains");
        require(pesaCore._acceptImplementation() == 0, "change not authorized");
    }
    ...
    /**
     * @notice Recalculate and update PESA speeds for all PESA markets
     */
    function refreshPesaSpeeds() public {
        require(msg.sender == tx.origin, "only externally owned accounts may refresh speeds");
        refreshPesaSpeedsInternal();
    }

    function refreshPesaSpeedsInternal() internal {
        ...
        for (uint i = 0; i < allMarkets_.length; i++) {
            ...
            updatePesaSupplyIndex(address(pToken));
            updatePesaBorrowIndex(address(pToken), borrowIndex);
        }
        ...
        for (uint i = 0; i < allMarkets_.length; i++) {
            PToken pToken = allMarkets_[i];
            if (markets[address(pToken)].isPesa) {
        ...
        for (uint i = 0; i < allMarkets_.length; i++) {
            ...
            uint newSpeed = totalUtility.mantissa > 0 ? mul_(pesaRate, div_(utilities[i], totalUtility)) : 0;
            pesaSpeeds[address(pToken)] = newSpeed;
            emit PesaSpeedUpdated(pToken, newSpeed);
        }
    }

    /**
     * @notice Accrue PESA to the market by updating the supply index
     * @param pToken The market whose supply index to update
     */
    function updatePesaSupplyIndex(address pToken) internal {
        PesaMarketState storage supplyState = pesaSupplyState[pToken];
        uint supplySpeed = pesaSpeeds[pToken];
        uint blockNumber = getBlockNumber();
        uint deltaBlocks = sub_(blockNumber, uint(supplyState.block));
        if (deltaBlocks > 0 && supplySpeed > 0) {
            uint supplyTokens = PToken(pToken).totalSupply();
            uint pesaAccrued = mul_(deltaBlocks, supplySpeed);
            Double memory ratio = supplyTokens > 0 ? fraction(pesaAccrued, supplyTokens) : Double({mantissa: 0});
            Double memory index = add_(Double({mantissa: supplyState.index}), ratio);
            pesaSupplyState[pToken] = PesaMarketState({
    ...
    /**
     * @notice Accrue PESA to the market by updating the borrow index
     * @param pToken The market whose borrow index to update
     */
    function updatePesaBorrowIndex(address pToken, Exp memory marketBorrowIndex) internal {
        PesaMarketState storage borrowState = pesaBorrowState[pToken];
        uint borrowSpeed = pesaSpeeds[pToken];
        uint blockNumber = getBlockNumber();
        uint deltaBlocks = sub_(blockNumber, uint(borrowState.block));
        if (deltaBlocks > 0 && borrowSpeed > 0) {
            uint borrowAmount = div_(PToken(pToken).totalBorrows(), marketBorrowIndex);
            uint pesaAccrued = mul_(deltaBlocks, borrowSpeed);
            Double memory ratio = borrowAmount > 0 ? fraction(pesaAccrued, borrowAmount) : Double({mantissa: 0});
            Double memory index = add_(Double({mantissa: borrowState.index}), ratio);
            pesaBorrowState[pToken] = PesaMarketState({
                index: safe224(index.mantissa, "new index exceeds 224 bits"),
                block: safe32(blockNumber, "block number exceeds 32 bits")
            });
        } else if (deltaBlocks > 0) {
    ...
    /**
     * @notice Calculate PESA accrued by a supplier and possibly transfer it to them
     * @param pToken The market in which the supplier is interacting
     * @param supplier The address of the supplier to distribute PESA to
     */
    function distributeSupplierPesa(address pToken, address supplier, bool distributeAll) internal {
        PesaMarketState storage supplyState = pesaSupplyState[pToken];
        Double memory supplyIndex = Double({mantissa: supplyState.index});
        Double memory supplierIndex = Double({mantissa: pesaSupplierIndex[pToken][supplier]});
        pesaSupplierIndex[pToken][supplier] = supplyIndex.mantissa;

        if (supplierIndex.mantissa == 0 && supplyIndex.mantissa > 0) {
            supplierIndex.mantissa = pesaInitialIndex;
        }

        Double memory deltaIndex = sub_(supplyIndex, supplierIndex);
        uint supplierTokens = PToken(pToken).balanceOf(supplier);
        uint supplierDelta = mul_(supplierTokens, deltaIndex);
        uint supplierAccrued = add_(pesaAccrued[supplier], supplierDelta);
        pesaAccrued[supplier] = transferPesa(supplier, supplierAccrued, distributeAll ? 0 : pesaClaimThreshold);
        emit DistributedSupplierPesa(PToken(pToken), supplier, supplierDelta, supplyIndex.mantissa);
    }

    /**
     * @notice Calculate PESA accrued by a borrower and possibly transfer it to them
     * @dev Borrowers will not begin to accrue until after the first interaction with the protocol.
     * @param pToken The market in which the borrower is interacting
     * @param borrower The address of the borrower to distribute PESA to
     */
    function distributeBorrowerPesa(address pToken, address borrower, Exp memory marketBorrowIndex, bool distributeAll) internal {
        PesaMarketState storage borrowState = pesaBorrowState[pToken];
        Double memory borrowIndex = Double({mantissa: borrowState.index});
        Double memory borrowerIndex = Double({mantissa: pesaBorrowerIndex[pToken][borrower]});
        pesaBorrowerIndex[pToken][borrower] = borrowIndex.mantissa;

        if (borrowerIndex.mantissa > 0) {
            Double memory deltaIndex = sub_(borrowIndex, borrowerIndex);
            uint borrowerAmount = div_(PToken(pToken).borrowBalanceStored(borrower), marketBorrowIndex);
            uint borrowerDelta = mul_(borrowerAmount, deltaIndex);
            uint borrowerAccrued = add_(pesaAccrued[borrower], borrowerDelta);
            pesaAccrued[borrower] = transferPesa(borrower, borrowerAccrued, distributeAll ? 0 : pesaClaimThreshold);
            emit DistributedBorrowerPesa(PToken(pToken), borrower, borrowerDelta, borrowIndex.mantissa);
        }
    }

    /**
     * @notice Transfer PESA to the user, if they are above the threshold
     * @dev Note: If there is not enough PESA, we do not perform the transfer all.
     * @param user The address of the user to transfer PESA to
     * @param userAccrued The amount of PESA to (possibly) transfer
     * @return The amount of PESA which was NOT transferred to the user
     */
    function transferPesa(address user, uint userAccrued, uint threshold) internal returns (uint) {
        if (userAccrued >= threshold && userAccrued > 0) {
            Pesa pesa = Pesa(getPesaAddress());
            require(pesa != Pesa(0x0));
            uint pesaRemaining = pesa.balanceOf(address(this));
            if (userAccrued <= pesaRemaining) {
                pesa.transfer(user, userAccrued);
                return 0;
            }
        }
        return userAccrued;
    ...
    /**
     * @notice Claim all the pesa accrued by holder in all markets
     * @param holder The address to claim PESA for
     */
    function claimPesa(address holder) public {
        return claimPesa(holder, allMarkets);
    }

    /**
     * @notice Claim all the pesa accrued by holder in the specified markets
     * @param holder The address to claim PESA for
     * @param pTokens The list of markets to claim PESA in
     */
    function claimPesa(address holder, PToken[] memory pTokens) public {
        address[] memory holders = new address[](1);
        holders[0] = holder;
        claimPesa(holders, pTokens, true, true);
    }

    /**
     * @notice Claim all pesa accrued by the holders
     * @param holders The addresses to claim PESA for
     * @param pTokens The list of markets to claim PESA in
     * @param borrowers Whether or not to claim PESA earned by borrowing
     * @param suppliers Whether or not to claim PESA earned by supplying
     */
    function claimPesa(address[] memory holders, PToken[] memory pTokens, bool borrowers, bool suppliers) public {
        for (uint i = 0; i < pTokens.length; i++) {
            ...
            if (borrowers == true) {
                ...
                for (uint j = 0; j < holders.length; j++) {
                    distributeBorrowerPesa(address(pToken), holders[j], borrowIndex, true);
                }
            }
            if (suppliers == true) {
                updatePesaSupplyIndex(address(pToken));
                for (uint j = 0; j < holders.length; j++) {
                    distributeSupplierPesa(address(pToken), holders[j], true);
                }
            }
        }
    }

    /*** Pesa Distribution Admin ***/

    function _setPesaAddress(address pesaAddress_) public {
        require(adminOrInitializing(), "only admin can set pesa address");

        address oldAddress = pesaAddress;
        pesaAddress = pesaAddress_;

        emit NewPesaAddress(oldAddress, pesaAddress_);
    }

    /**
     * @notice Set the amount of PESA distributed per block
     * @param pesaRate_ The amount of PESA wei per block to distribute
     */
    function _setPesaRate(uint pesaRate_) public {
        require(adminOrInitializing(), "only admin can change pesa rate");

        uint oldRate = pesaRate;
        pesaRate = pesaRate_;
        emit NewPesaRate(oldRate, pesaRate_);

        refreshPesaSpeedsInternal();
    }

    /**
     * @notice Add markets to pesaMarkets, allowing them to earn PESA in the flywheel
     * @param pTokens The addresses of the markets to add
     */
    function _addPesaMarkets(address[] memory pTokens) public {
        require(adminOrInitializing(), "only admin can add pesa market");

        for (uint i = 0; i < pTokens.length; i++) {
            _addPesaMarketInternal(pTokens[i]);
        }

        refreshPesaSpeedsInternal();
    }

    function _addPesaMarketInternal(address pToken) internal {
        Market storage market = markets[pToken];
        require(market.isListed == true, "pesa market is not listed");
        require(market.isPesa == false, "pesa market already added");

        market.isPesa = true;
        emit MarketPesa(PToken(pToken), true);

        if (pesaSupplyState[pToken].index == 0 && pesaSupplyState[pToken].block == 0) {
            pesaSupplyState[pToken] = PesaMarketState({
                index: pesaInitialIndex,
                block: safe32(getBlockNumber(), "block number exceeds 32 bits")
            });
        }

        if (pesaBorrowState[pToken].index == 0 && pesaBorrowState[pToken].block == 0) {
            pesaBorrowState[pToken] = PesaMarketState({
                index: pesaInitialIndex,
                block: safe32(getBlockNumber(), "block number exceeds 32 bits")
            });
    ...
    /**
     * @notice Remove a market from pesaMarkets, preventing it from earning PESA in the flywheel
     * @param pToken The address of the market to drop
     */
    function _dropPesaMarket(address pToken) public {
        require(msg.sender == admin, "only admin can drop pesa market");

        Market storage market = markets[pToken];
        require(market.isPesa == true, "market is not a pesa market");

        market.isPesa = false;
        emit MarketPesa(PToken(pToken), false);
    ...
    /**
     * @notice Return the address of the PESA token
     * @return The address of PESA
     */
    function getPesaAddress() public view returns (address) {
        return pesaAddress;
    }
}