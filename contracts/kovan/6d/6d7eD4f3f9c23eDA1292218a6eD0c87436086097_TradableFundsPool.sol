...
contract TradableFundsPool is Ownable {
    ...
    using SafeMathChainlink for int256;
    ...
    function deposit(uint256 _amount) public {
        ...
        require(unitOfAccountToken.balanceOf(msg.sender) >= _amount);

        uint256 currentWorth = getCurrentPoolWorth();

        if (amountFunded[msg.sender] == 0) {
            investorList.push(msg.sender);
        }

        unitOfAccountToken.transferFrom(msg.sender, address(this), _amount);
        ...
        uint256 addedWorth = (_amount * uocPrice) / (10**18);
        uint256 percentageIncrease = (addedWorth * 10**18) /
            (addedWorth + currentWorth);
        updateInvestorsOwnershipAtDeposit(percentageIncrease, msg.sender);
    }

    function updateInvestorsOwnershipAtDeposit(
    ...
    function getCurrentPoolWorth() public view returns (uint256) {
        uint256 total = 0;
    ...
    function updateInvestorsOwnershipAtWithdrawal(
        uint256 percentageIncrease,
        address sender
    ) internal {
        for (uint256 i = 0; i < investorList.length; i++) {
            address curInvestor = investorList[i];
            uint256 curAmountOwnership = amountOwnership[curInvestor];
            int256 newAmountOwnership;

            if (curInvestor != sender) {
                amountOwnership[curInvestor] =
                    (curAmountOwnership * 10**18) /
                    (10**18 - percentageIncrease);
            } else {
                amountOwnership[curInvestor] = 0;
            }
        }
    }
    ...
    function withdraw() public {
        ...
        if (investorList.length != 1) {
            updateInvestorsOwnershipAtWithdrawal(senderOwnership, msg.sender);
        }
        removeInvestorFromList(msg.sender);
    }

    function removeInvestorFromList(address investorAddress) internal {
        for (uint256 i = 0; i < investorList.length; i++) {
            if (investorList[i] == investorAddress) {
