...
library Address {
    ...
    function functionCallWithValue(
        ...
        require(address(this).balance >= value, "Address: balance too low");
        require(isContract(target), "Address: non-contract");
...
abstract contract IERC721 is IERC165 {
     ...
     */
    function approve(address to, uint256 tokenId) virtual external;
     ...
     */
    function setApprovalForAll(address operator, bool _approved)virtual external;
...
contract Administerable is AccessControl {
    ...
    constructor(bytes32 role, address account) {
...
// Part: CharacterDetailsV1

contract CharacterDetailsV1 is Administerable {
    uint256 private raritySeed;
    mapping(address => uint256) private experience;
    mapping(address => bytes32) private levels;
    bool private _rarityLock = false;
    bool public showCharacterDetails = false;

    constructor(bytes32 role) Administerable(role, msg.sender) {}

    Rarity private rarity;
    mapping(int8 => Class) private class;
    mapping(int8 => Lands) private land;

    function setCharacter(uint256 _raritySeed, uint48 _rarityValue)
        public
        onlyOnce
    {
        raritySeed = _raritySeed;
        rarity = determineRarity(_rarityValue);
        class[0] = determineClass(_rarityValue);
        land[0] = determineLand(_rarityValue);
    }

    function isAfterReveal() public view returns (bool) {
        return showCharacterDetails || isAdmin(msg.sender);
    }
    ...
    function determineRarity(uint48 rareValue) public pure returns (Rarity) {
    ...
    function determineClass(uint48 value) internal view returns (Class) {
        return classList[(value % uint32(classList.length)) + 1];
    }

    function determineLand(uint48 value) internal view returns (Lands) {
     ...
     * To String Methods
     */
    function toRarityEnglish(Rarity value) public pure returns (string memory) {
    ...
    function toClassEnglish(Class value) public pure returns (string memory) {
    ...
    function toLandEnglish(Lands value) public pure returns (string memory) {
    ...
    function getRarity() public view onlyAfterRelease returns (Rarity) {
        return rarity;
    }

    function getRarityReadable()
        public
        view
        onlyAfterRelease
        returns (string memory)
    {
        return toRarityEnglish(rarity);
    }

    function getClass(int8 index) public view returns (Class) {
        return class[index];
    }

    function getClassReadable(int8 index) public view returns (string memory) {
        return toClassEnglish(class[index]);
    }

    function getLand(int8 index) public view returns (Lands) {
        return land[index];
    }

    function getLandReadable(int8 index) public view returns (string memory) {
        return toLandEnglish(land[index]);
    }

    function getExperience(address gameAddress)
        public
        view
        onlyAfterRelease
        returns (uint256)
    {
        return experience[gameAddress];
    }

    function getLevels(address gameAddress)
        public
        view
        onlyAfterRelease
        returns (bytes32)
    {
        return levels[gameAddress];
    }

    function addClass(Class newClass, int8 index)
        public
        onlyAdminOrGameControl
    {
    ...
    function setClass(Class newClass, int8 index)
        public
        onlyAdminOrGameControl
    {
        class[index] = newClass;
    ...
    function addLand(Lands newLand, int8 index) public onlyAdminOrGameControl {
        land[index] = newLand;
    }

    function setLand(Lands newLand, int8 index) public onlyAdminOrGameControl {
        land[index] = newLand;
    }
    ...
    function updateExperience(
        ...
        if (increase) {
            experience[changedExperience] =
                uint256(experience[changedExperience]) +
                amount;
        } else {
            experience[changedExperience] =
                uint256(experience[changedExperience]) -
    ...
    function updateLevel(
        ...
        if (increase) {
            levels[changedExperience] = bytes32(
                uint256(levels[changedExperience]) + amount
            );
        } else {
            levels[changedExperience] = bytes32(
                uint256(levels[changedExperience]) - amount
...
contract StakeWarsV1 is ERC721Enumerable, CharacterDetailsV1, VRFConsumerBase {
    ...
    mapping(bytes32 => User) public requestIdToUser;
    mapping(uint48 => uint256) public rarityRatio;

    mapping(address => CharacterDetailsV1[]) public stakeWarrior;
    ...
    constructor(
        ...
        ERC721(name, symbol)
        CharacterDetailsV1(DEFAULT_ADMIN_ROLE)
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness)
        ...
        override
    {
        uint48 rare48 = uint48(randomness);

        address sender = requestIdToUser[requestId].sender;
        uint256 stakeWarriorLength = requestIdToUser[requestId].tokenId;

        caught = totalSupply() + 100;
        // caught = address(stakeWarrior[sender][stakeWarriorLength]);
        // CharacterDetailsV1 warrior = CharacterDetailsV1(
        //     address(stakeWarrior[sender][stakeWarriorLength])
        // );
        // warrior.setCharacter(randomness, rare48);
        // rarityRatio[
        //     uint48(stakeWarrior[sender][tokenId].determineRarity(rare48))
        // ]++;
        _safeMint(sender, caught);
    }

    function reserve(uint256 num) public onlyAdmin {
    ...
    function mint(uint256 numberOfMints) public payable {
        ...
        require(numberOfMints > 0 && numberOfMints < 11);
        require(totalSupply() + numberOfMints <= maxSupply);
        require(price * numberOfMints == msg.value);
    ...
    function createMint(uint256 i) internal {
        bytes32 requestId = requestRandomness(keyhash, linkFee);
        stakeWarrior[msg.sender].push(new CharacterDetailsV1(USER_ROLE));
        requestIdToUser[requestId] = User(
            msg.sender,
            stakeWarrior[msg.sender].length
        );
    ...
    function toggleStatus(int8 status) public onlyAdmin {
        if (status == 0) saleActive = !saleActive;
        else if (status == 1) showCharacterDetails = true;
    }
    ...
    function setBaseURI(string memory uri) public onlyAdminOrGameControl {
        baseURI = uri;
    }
}