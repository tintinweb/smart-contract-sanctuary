...
contract BalancerUtils is Ownable{
    ...
    function justSwap(address smartPoolAddress, address tokenIn, uint tokenAmountIn, address tokenOut, uint minAmountOut, uint maxPrice) public onlyOwner returns (uint, uint){
        ConfigurableRightsPool crpPool = ConfigurableRightsPool(smartPoolAddress);
    ...
    function pokeSwap(address smartPoolAddress, address tokenIn, uint tokenAmountIn, address tokenOut) public onlyOwner returns(uint, uint){
        ConfigurableRightsPool crpPool = ConfigurableRightsPool(smartPoolAddress);
        crpPool.pokeWeights();
    ...
    function forceSwap(address smartPoolAddress, address tokenIn, uint tokenAmountIn, address tokenOut) public onlyOwner returns (uint,uint) {
        ConfigurableRightsPool crpPool = ConfigurableRightsPool(smartPoolAddress);
        BPool bPool = crpPool.bPool();
        TokenInterface TI = TokenInterface(tokenIn);
        TokenInterface TO = TokenInterface(tokenOut);
    ...
    function doPoke(address smartPoolAddress) onlyOwner public {
        ConfigurableRightsPool crpPool = ConfigurableRightsPool(smartPoolAddress);
        crpPool.pokeWeights();
    }

    // can use view function to calculate how much needed do gas optimization
    function swapPokeSwap(address smartPoolAddress, address tokenToSell, uint inAmount, address pairToken) public onlyOwner returns(uint){
        ConfigurableRightsPool crpPool = ConfigurableRightsPool(smartPoolAddress);
        BPool bPool = crpPool.bPool();
