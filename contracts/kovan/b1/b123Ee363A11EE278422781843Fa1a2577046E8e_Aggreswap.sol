...
contract Aggreswap is Ownerable, ISwap {
    ...
    function swapEstimateOutWithDEX(address tokenIn, address tokenOut, uint256 amountIn) external view returns (uint256 amountOut, string memory dexName) {
        Dex memory dex;
        (amountOut, dex) = _getBestOut(tokenIn, tokenOut, amountIn);
        dexName = dex.name;
    }

    function swapEstimateInWithDEX(address tokenIn, address tokenOut, uint256 amountOut) external view returns (uint256 amountIn, string memory dexName) {
        Dex memory dex;
