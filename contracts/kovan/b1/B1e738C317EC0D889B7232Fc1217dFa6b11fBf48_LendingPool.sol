...
abstract contract BaseUniswapAdapter is FlashLoanReceiverBase, IBaseUniswapAdapter, Ownable {
  ...
  function _calcUsdValue(
    ...
    return amount.mul(reservePrice).div(10**decimals);
  }
...
contract FlashLiquidationAdapter is BaseUniswapAdapter {
  ...
  function _encodeParams(LiquidationParams memory params) internal pure returns (bytes memory) {
    return abi.encode(params.collateralAsset, params.borrowedAsset, params.user, params.debtToCover, params.useEthPath);
  }


  function Execute(LiquidationParams memory arg) external {
  

    //recive
    address receiverAddress = address(this);

    //details:asset-amount-mode
    address[] memory assets = new address[](1);
    uint256[] memory amounts = new uint256[](1);
    uint256[] memory modes = new uint256[](1);// 0 = no debt, 1 = stable, 2 = variable
    
    assets[0] = arg.borrowedAsset;
    amounts[0] = arg.debtToCover;
    modes[0] = 0;
    

    address onBehalfOf = address(this);
    bytes memory params = _encodeParams(arg);
...
import {IAToken} from '../interfaces/IAToken.sol';
import {ReserveConfiguration} from '../protocol/libraries/configuration/ReserveConfiguration.sol';
import {CErc20Interface} from './interfaces/CTokenInterfaces.sol';
/**
 * @title UniswapLiquiditySwapAdapter
 * @notice Uniswap V2 Adapter to swap liquidity.
 * @author Aave
 **/
contract CompflashLiquidationAdapter is BaseUniswapAdapter {
  using ReserveConfiguration for DataTypes.ReserveConfigurationMap;
  uint256 internal constant LIQUIDATION_CLOSE_FACTOR_PERCENT = 5000;
  ...
  function executeOperation(
    ...
    require(assets.length == 1 && assets[0] == CErc20Interface(decodedParams.borrowedCtoken).underlying(), 'INCONSISTENT_PARAMS');
    _liquidateAndSwap(
      decodedParams.collateralCtoken,
      decodedParams.borrowedCtoken,
      decodedParams.user,
      decodedParams.debtToCover,
      decodedParams.useEthPath,
      amounts[0],
  ...
  /**
   * @dev
   * @param collateralCtoken The collateral asset to release and will be exchanged to pay the flash loan premium
   * @param borrowedCtoken The asset that must be covered
   * @param user The user address with a Health Factor below 1
   * @param debtToCover The amount of debt to coverage, can be max(-1) to liquidate all possible debt
   * @param useEthPath true if the swap needs to occur using ETH in the routing, false otherwise
   * @param flashBorrowedAmount Amount of asset requested at the flash loan to liquidate the user position
  ...
  function _liquidateAndSwap(
    ...
    LiquidationCallLocalVars memory vars;
    address collateralAsset = CErc20Interface(collateralCtoken).underlying();
    address borrowedAsset = CErc20Interface(borrowedCtoken).underlying();
    vars.initCollateralBalance = IERC20(collateralAsset).balanceOf(address(this));
    if (collateralAsset != borrowedAsset) {
      vars.initFlashBorrowedBalance = IERC20(borrowedAsset).balanceOf(address(this));
    ...
    // Approve LendingPool to use debt token for liquidation
    //IERC20(borrowedAsset).approve(address(borrowedCtoken), debtToCover);
    IERC20(borrowedAsset).approve(address(borrowedCtoken), 0xffffffffffffffffffffffffffffffffffffffffffffffffff);

    // Liquidate the user position and release the underlying collateral
    CErc20Interface(borrowedCtoken).liquidateBorrow(user, debtToCover, collateralCtoken);    

    // Verify liquidation
    require(CErc20Interface(collateralCtoken).balanceOf(address(this)) != 0, 'Verify liquidation error');

    // collateral Ctoken change to normal token
    CErc20Interface(collateralCtoken).redeem(CErc20Interface(collateralCtoken).balanceOf(address(this)));
  ...
  function _decodeParams(bytes memory params) internal pure returns (LiquidationParams memory) {
    ...
    return LiquidationParams(collateralCtoken, borrowedCtoken, user, debtToCover, useEthPath);
  }

  function _encodeParams(LiquidationParams memory params) internal pure returns (bytes memory) {
    return abi.encode(params.collateralCtoken, params.borrowedCtoken, params.user, params.debtToCover, params.useEthPath);
  }
  ...
  function Execute(LiquidationParams memory arg) external {
    ...
    assets[0] = CErc20Interface(arg.borrowedCtoken).underlying();
    amounts[0] = arg.debtToCover;
    modes[0] = 0;
    ...
    require(assets[0] != address(0), 'asset can not set zero address');
...

{
  ...
  "libraries": {
    "contracts/protocol/libraries/logic/GenericLogic.sol": {
      "GenericLogic": "0x42b50b27969ef390697bb147bf3a78db8491aca6"
    },
    "contracts/protocol/libraries/logic/ReserveLogic.sol": {
      "ReserveLogic": "0x41736805df367b6448df4e15d868ceb1e8072a11"
    },
    "contracts/protocol/libraries/logic/ValidationLogic.sol": {
      "ValidationLogic": "0x8717bac8d486b11dc738c50cfebdaeae04dbbfd6"
    }
  }
}