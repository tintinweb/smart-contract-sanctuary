...
contract HabibiProtocol is Context, IBEP20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "HabibiProtocol";
    ...
    uint256 internal _tokenTotal = 1000000000000000 *10**9;
    ...
    bool public liquidityEnalbed;
    
    uint256 public liquidityAddedAt = block.timestamp;
    
    uint256 public lastLiquidity = block.timestamp;
    uint256 public liquidityInterval = 30 minutes;
    
    IUniswapV2Router02 public uniswapV2Router;
    address public uniswapV2Pair;
    address public charityAddress = 0x7D16eF6A43515F0cDb6b202397bd500fAd47fBbf;
    address public developmentAddress = 0x7D16eF6A43515F0cDb6b202397bd500fAd47fBbf;
    
    event RewardsDistributed(uint256 amount);
    event SwapAndLiquifyEnabledUpdated(bool enabled);
    event SwapedTokenForEth(uint256 EthAmount, uint256 TokenAmount);event SwapAndLiquify(uint256 tokensSwapped, uint256 ethReceived, uint256 tokensIntoLiqudity);
    ...
    function excludeAccount(address account) external onlyOwner() {
        require(account != 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D, "Habibi: Uniswap router cannot be excluded.");
        require(account != address(this), 'Habibi: The contract it self cannot be excluded');
        require(!_isExcluded[account], "Habibi: Account is already excluded");
    ...
    function includeAccount(address account) external onlyOwner() {
        require(_isExcluded[account], "Habibi: Account is already included");
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if(!inSwapAndLiquify && sender != uniswapV2Pair) {
            ...
            //@dev Buy tokens & add liquidity
            if(block.timestamp > lastLiquidity + liquidityInterval 
                && liquidityEnalbed && contractTokenBalance > 0) {
        ...
        if(!isExcludedFromFee[sender] && !isExcludedFromFee[recipient] && !inSwapAndLiquify){
            ...
            if (_tokenTotal <= 1000000000000000 *10**9 && _tokenTotal > 250000000000000 *10**9) {
                transferAmount = collectFeeOnPhase_1(sender,amount,rate);
            } else
            
            if (_tokenTotal <= 250000000000000 *10**9 && _tokenTotal > 121000000000 *10**9) {
                transferAmount = collectFeeOnPhase_2(sender,amount,rate);
            } else
            
            transferAmount = collectFeeOnPhase_3(sender,amount,rate);
        }
    ...
    function collectFeeOnPhase_1(address account, uint256 amount, uint256 rate) private returns (uint256) {
        uint256 transferAmount = amount;
        uint256 _liquidityFee = 200;
        uint256 _burnFee = 300;
        uint256 _charityFee = 100;
        uint256 _developmentFee = 100;
    ...
    function collectFeeOnPhase_2(address account, uint256 amount, uint256 rate) private returns (uint256) {
        uint256 transferAmount = amount;
        uint256 _liquidityFee = 200;
        uint256 _burnFee = 150;
        uint256 _charityFee = 50;
        uint256 _developmentFee = 100;
    ...
    function collectFeeOnPhase_3(address account, uint256 amount, uint256 rate) private returns (uint256) {
        uint256 transferAmount = amount;
    ...
    function _burn(uint256 amount) public {
        require(amount > 0, "ERC20: You cannot transfer zero token.");
    ...
    function EnableSwapAndLiquify(bool) public onlyOwner {
        swapAndLiquifyEnabled = true;
    }
    
    function DisableSwapAndLiquify(bool) public onlyOwner {
    ...
    function setliquidityInterval(uint256 interval) public onlyOwner {
        liquidityInterval = interval;
    }
    
    function liquidityEnabled(bool) public onlyOwner {
        liquidityEnalbed = true;
    }
    
    function liquidityDisabled(bool) public onlyOwner {
        liquidityEnalbed = false;
    }
    ...
    function transferAnyBEP20Tokens(address tokenAddress, address recipient, uint256 amount) public onlyOwner {
        require(tokenAddress != address(this), "BEP20: You can't transfer HabibiProtocol.");
