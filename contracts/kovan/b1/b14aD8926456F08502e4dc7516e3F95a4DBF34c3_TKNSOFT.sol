...
contract TKNSOFT is ERC20, ERC20Burnable, Ownable {
    ...
    constructor() ERC20("TKNSOFT", "TKN") {
        ...
        preIcoStageOneEnd = preIcoStart + 1 minutes;
        preIcoStageTwoEnd = preIcoStageOneEnd + 1 minutes;
        weiRaised = 0;
        tokenRate = 25;
    ...
    function purchaseTokens(address _beneficiary) public payable {
        ...
        if(block.timestamp >= preIcoStart && block.timestamp < preIcoStageOneEnd && stageOneSupply > 0 && !preIcoStageOneEnded){
        ...
        }else if(block.timestamp >= preIcoStageOneEnd && block.timestamp < preIcoStageTwoEnd && stageTwoSupply > 0 && !preIcoStageTwoEnded){
            tokens = SafeMath.add(tokens, SafeMath.mul(weiAmount,tokenRate));
