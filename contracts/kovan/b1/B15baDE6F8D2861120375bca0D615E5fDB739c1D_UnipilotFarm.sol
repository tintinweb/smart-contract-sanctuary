...
contract UnipilotFarm is IUnipilotFarm,ReentrancyGuard, IERC721Receiver {
    ...
    function deletePools(address[] memory pools) external override onlyGovernance {
        for (uint256 i = 0; i < pools.length; i++) {
            poolWhitelist[pools[i]] = false;
            delete poolInfo[pools[i]];

            emit DeletePool(pools[i], poolWhitelist[pools[i]], block.timestamp);
        }
    }
    ...
    function withdrawNFT(uint256 tokenId) external override nonReentrant returns (bool isLeft) {
        address poolAddress = userInfo[tokenId].poolAddress;
    ...
    /// @param tokenId unstake tokenID
    function emergencyNFTWithdraw(uint256 tokenId) nonReentrant external {
...
interface IUnipilotFarm {
    ...
    event DeletePool(address poolAddress, bool status, uint256 time);
    ...
    function deletePools(address[] memory pools) external;
