...
contract AgreementContract is Ownable {
    ...
    mapping(string => mapping(string => bool)) public _approveContract;
    mapping(uint256 => Contract) public contractInfo;
    mapping(string => mapping (string => Milestone[])) public milestonesInfo;          // contratId => milestoneId => milestoneInfo
    ...
    function addMilestone(
        ...
        for (uint256 index = 0; index < _milestone.length; index++) {
            milestonesInfo[contractId][milestoneId].push(_milestone[index]);        
        }
    }

    function approveContract(string memory contractId, string memory employeeId , bool status)external {
