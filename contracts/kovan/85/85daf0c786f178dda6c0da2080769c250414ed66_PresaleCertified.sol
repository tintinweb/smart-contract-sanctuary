...
contract PresaleCertified is ReentrancyGuard {
        ...
        presaleIsNotCancelled
    {
        ...
        if (certifiedAddition.vesting == 0) {
            ...
            //uint256 oneMonth = 300; //PROD: 2592000
            uint256 beginingTime;// = uniswapInfo.unlockTime - uniswapInfo.lpTokensLockDurationInDays*lpDaySeconds;
            if(certifiedAddition.liquidity){
                beginingTime = uniswapInfo.unlockTime - uniswapInfo.lpTokensLockDurationInDays*lpDaySeconds;
            }
            else {
            ...
            uint256 earnedTokens = numOfParts * part - investments[msg.sender].amountClaimed;
            /* require(
                nowTime - claimed[msg.sender].lastTimeClaimed >=
                    oneMonth, //PROD: 2592000
                "TIME"
            ); */ //one month vesting period == 30 days
            require (earnedTokens > 0, "NOCLAIM");
            //investments[msg.sender].lastTimeClaimed = block.timestamp;
            if (
                earnedTokens <=
                investments[msg.sender].amountTokens -
                    investments[msg.sender].amountClaimed
                ...
                require(
                    IERC20(generalInfo.token).transfer(msg.sender, earnedTokens),
                    "T.TRANSFER"
                );
            } else {
                earnedTokens =
        ...
        liquidityAdded
    {
        ...
        require(!intermediate.withdrawedFunds && block.timestamp >= generalInfo.closeTimePresale && intermediate.raisedAmount >= generalInfo.softCapInWei, "ONCE/TIME/SOFT");
