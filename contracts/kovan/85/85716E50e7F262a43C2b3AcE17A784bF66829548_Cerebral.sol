...
contract Cerebral{
    ...
    modifier onlyAdmin() {
        require(msg.sender == admin, "Cerebral: NOT_ADMIN_ADDRESS");
        _;
    }
    ...
    function createOrder (
        ...
        require(amountA != 0, "Cerebral: AMOUNT_TOKEN_IS_NOT_AVAILABLE");
    ...
    function searchOrdersForMatching(address owner, uint ownerOrderId) external view returns (
        ...
        require(order.expiryDate > block.timestamp, "Cerebral: EXPIRYDATE_NOT_AVAILABLE");
        require(order.amountA != 0, "Cerebral: AMOUNT_TOKEN_IS_NOT_AVAILABLE");
    ...
    function _startMatching(address _owner, uint _ownerOrderId) internal {
        ...
        OrderInfo storage order = orders[ownerOrder.tokenA][ownerOrder.tokenB][ownerOrder.orderId];
        require(order.expiryDate > uint64(block.timestamp), "Cerebral: EXPIRYDATE_NOT_AVAILABLE");
        require(order.amountA != 0, "Cerebral: AMOUNT_TOKEN_IS_NOT_AVAILABLE");
    ...
    function calculatePrice(address tokenA, address tokenB, uint amountA) public view returns (uint price) {
        require(tokenA != address(0) && tokenB != address(0), "Cerebral: TOKEN_NOT_AVAILABLE");
        require(tokenA != tokenB, "Cerebral: TOKEN_IS_NOT_SAMPLE");
        require(amountA != 0, "Cerebral: AMOUNT_TOKEN_IS_NOT_AVAILABLE");
