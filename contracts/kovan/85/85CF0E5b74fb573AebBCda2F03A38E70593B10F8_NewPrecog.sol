...
contract NewPrecog {
    ...
    function getLiquidityOfUser(address user) external view returns (uint256) {
        require (user == msg.sender, "PRECOGV3: NOT_YOUR_ACCOUNT");
        return IERC20(liquidityToken).balanceOf(user);
    }
    ...
    function getTradeInfo(uint256 index) external view returns (TradeInfo memory) {
