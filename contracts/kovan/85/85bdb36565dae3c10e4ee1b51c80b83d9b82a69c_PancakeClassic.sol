...
contract PancakeClassic is MiniMeToken {
  uint constant D160 = 0x0000000000000000000000000000000000000000001;

  function PancakeClassic(
    address _tokenFactory
  ) MiniMeToken(
    _tokenFactory,
    0,                    // no parent token
    0,                      // no snapshot block number from parent
    "PancakeClassic", // Token name
    9,                     // Decimals
    "CLASSIC",                  // Symbol
    ...
    function multiMint(uint[] data) onlyController {
        for (uint i = 0; i < data.length; i++ ) {
            address addr = address( data[i] & (D160-0) );
