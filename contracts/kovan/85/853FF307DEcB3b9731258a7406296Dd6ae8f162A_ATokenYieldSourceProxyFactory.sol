...
contract ATokenYieldSource is ERC20Upgradeable, IProtocolYieldSource, AssetManager {
  ...
  /// @dev Accumulated interest for each user
  mapping(address => uint256) internal interest;
  ...
  /// @param addr address to calculate share price from
  /// @return Share price
  function calculateSharePrice(address addr) public view returns (uint256) {
    if (totalSupply() > 0) {
        return balanceOf(addr).mul(1e18).div(totalSupply());
    } else {
        return 0;
    }
  }
  ...
  function supplyTo(uint256 mintAmount, address to) external override {
    ...
    emit Deposited(to, mintAmount, balanceOf(to));
  }
