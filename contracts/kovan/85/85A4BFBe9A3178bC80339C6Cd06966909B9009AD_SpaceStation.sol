...
contract SpaceStation is ReentrancyGuard {
    ...
    function _payFees(uint256 _cid, Operation _op) private {
        ...
        uint256 total = _getGasAvgPerOp(_op) * uint(FASTGAS.latestAnswer()) + feeConf.platformFee;
    ...
    function _payFine(uint256 _cid, uint256 _mintBlock) private {
        ...
        if (block.number < _mintBlock + lockBlockNum) {
            ...
            uint256 total = _getGasAvgPerOp(Operation.StakeOut) * uint(FASTGAS.latestAnswer()) + campaignFeeConfigs[_cid][Operation.StakeOut].platformFee;
