...
contract Creators is Context{
    ...
    mapping(address => uint256) private _overallAllowances;
    address[3] private _creators;
    Jackpot private _j;
    uint256 private total_allowed;

    constructor(address[3] memory creators_){
        _creators = creators_;
    }
    ...
    modifier onlyCreators() {
        ...
        for(uint i=0; i<3; i++){
            if(_msgSender() == _creators[i]){
                isCreator = true;
                break;
            }
        }
        require(isCreator, "Ownable: caller is not a creator");
        _;
    }

    function creators() public view onlyCreators returns (address, address, address){
        return (_creators[0], _creators[1], _creators[2]);
    }

    function getTokenAddress() public view onlyCreators returns (address){
        return address(_j);
    }

    function setTokenAddress(address address_) public onlyCreators{
        _j = Jackpot(address_);
    }

    function getContractBalance() private view returns (uint256) {
        return _j.balanceOf(address(this));
    }

    function allowance(uint256 amount) public onlyCreators{
        require(possibleAllowance(_msgSender()) >= amount, 'Transfer amount exceeds balance');

        uint256 currentAccountAllowance = currentAllowance(_msgSender());
        _overallAllowances[_msgSender()] += amount - currentAccountAllowance;
        total_allowed += amount - currentAccountAllowance;

        require(_j.approve(_msgSender(), amount), 'approve failed');
    }

    function currentAllowance(address account) public view onlyCreators returns(uint256){
        require(account == _creators[0] || account == _creators[1] || account == _creators[2], "Account is not one of the creators address");
        return _j.allowance(address(this), account);
    }

    function possibleAllowance(address account) public view onlyCreators returns(uint256){
        require(account == _creators[0] || account == _creators[1] || account == _creators[2], "Account is not one of the creators address");
        uint256 balanceContract = getContractBalance();
        uint256 creatorsCurrentAllowance = currentAllowance(_creators[0]) + currentAllowance(_creators[1]) + currentAllowance(_creators[2]);
        uint256 currentAccountAllowance = currentAllowance(account);
        return (balanceContract + creatorsCurrentAllowance - total_allowed) / uint(3) - _overallAllowances[account] + currentAccountAllowance;
    }
}
...
contract Jackpot is Context, Ownable, IERC20, IERC20Metadata, VRFConsumerBase {
    ...
    function setUniswapV2Pair(address uniswapV2Pair) public onlyOwner {
        require(_tradingOpened, "trading is already opened");
