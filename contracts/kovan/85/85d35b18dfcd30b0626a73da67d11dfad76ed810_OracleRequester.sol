...
// File: contracts/OracleRequester.sol
 ...
 */
contract OracleRequester is ChainlinkClient {
    using Chainlink for Chainlink.Request;
    ...
    function setOracleAddress(address oracleAddress) public {
        oracle = oracleAddress;
    }

    function request(bytes32 requestJobId, uint256 someVal) public returns (bytes32 requestId) 
    {
        Chainlink.Request memory req = buildChainlinkRequest(requestJobId, address(this), this.fulfill.selector);
        req.addInt("result", 1);
        req.addUint("someVal", someVal);
     ...
     */ 
    function fulfill(bytes32 _requestId, uint256 fulfilledResult) public recordChainlinkFulfillment(_requestId)
    {
