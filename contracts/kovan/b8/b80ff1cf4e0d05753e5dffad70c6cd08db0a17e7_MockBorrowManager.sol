...
contract MockBorrowManager is IBorrowManager, AccessControlEnumerable, ReentrancyGuard, Pausable
{
  ...
  function _getBorrowMeta (Contracts memory coordinators, address debtToken, uint256 debt, address collateralToken) private view returns (bool, uint256, uint256)
  {
    return (IStakingManager(coordinators.stakingManager).isDiscountableBorrower(msg.sender), IOracle(coordinators.oracle).convertToUSD(debtToken, debt), IOracle(coordinators.oracle).convert(debtToken, collateralToken, debt));
  }

  function borrow (BorrowData calldata data) external nonReentrant
  {
    _isNotPaused();

    Contracts memory coordinators = _getCoordinatingContracts();
    (bool borrowerIsDiscounted, uint256 debtInUSD, uint256 debtInCollateralToken) = _getBorrowMeta(coordinators, data.token, data.amount, data.collateralToken);
    (address debtTokenVault, address collateralTokenVault) = ICoordinator(coordinators.coordinator).getBorrowVaults(data.token, data.collateralToken);

    require(_isValidBorrow(debtInUSD, data.interestRate, data.durationInSecs), "!valid terms");
    require(IERC20(data.token).balanceOf(debtTokenVault) >= data.amount, "!enough liq");
    require(ICollateralizationManager(coordinators.collateralizationManager).isSufficientInitialCollateral(coordinators.tokenRegistry, data.token, data.collateralToken, debtInCollateralToken, data.collateral), "!adequate collateral");


    bytes32 id = _generateDebtID(data.token, data.amount, data.collateralToken, data.collateral, data.interestRate, data.durationInSecs);
    ...
    ({
      ...
      collateral: uint128(data.collateral),
      interestRate: uint16(data.interestRate),
      durationInSecs: uint24(data.durationInSecs),
      startTimestamp: uint32(block.timestamp)
    });


    IStakingManager(coordinators.stakingManager).increaseBorrowerExpectedStake(coordinators.oracle, msg.sender, data.token, debtInUSD, block.timestamp + data.durationInSecs);

    IRewardManager(coordinators.rewardManager).registerDebt(id, debtInUSD, data.interestRate, borrowerIsDiscounted);


    uint256 debtFee = IFeeManager(coordinators.feeManager).getBorrowFeeOnDebt(borrowerIsDiscounted, debtInCollateralToken);


    IERC20(data.collateralToken).safeTransferFrom(msg.sender, IFeeManager(coordinators.feeManager).getBurner(), debtFee);
    IERC20(data.collateralToken).safeTransferFrom(msg.sender, collateralTokenVault, data.collateral);
    IVault(debtTokenVault).transferToken(msg.sender, data.amount);


    emit NewBorrow(id, msg.sender, data.token, data.amount);
  }
 
  
  function borrow2 (BorrowData calldata data) external nonReentrant
  {
    _isNotPaused();
  
    Contracts memory coordinators = _getCoordinatingContracts();
    BorrowMetadata memory metadata = _getBorrowMetadata(coordinators, data.token, data.amount, data.collateralToken);
    (address debtTokenVault, address collateralTokenVault) = ICoordinator(coordinators.coordinator).getBorrowVaults(data.token, data.collateralToken);
  
    require(_isValidBorrow(metadata.debtInUSD, data.interestRate, data.durationInSecs), "!valid terms");
    require(IERC20(data.token).balanceOf(debtTokenVault) >= data.amount, "!enough liq");
    require(ICollateralizationManager(coordinators.collateralizationManager).isSufficientInitialCollateral(coordinators.tokenRegistry, data.token, data.collateralToken, metadata.debtInCollateralToken, data.collateral), "!adequate collateral");
    ...
    IStakingManager(coordinators.stakingManager).increaseBorrowerExpectedStake(coordinators.oracle, msg.sender, data.token, metadata.debtInUSD, block.timestamp + data.durationInSecs);
  
    IRewardManager(coordinators.rewardManager).registerDebt(id, metadata.debtInUSD, data.interestRate, metadata.borrowerIsDiscounted);
  ...
  /*function borrow (address debtToken, uint256 amount, address collateralToken, uint256 collateral, uint256 interestRate, uint256 durationInSecs) external nonReentrant
  {
    ...
    IERC20(collateralToken).safeTransferFrom(msg.sender, IFeeManager(coordinators.feeManager).getBurner(), debtFee);
