...
contract Position is Initializable, ERC721EnumerableUpgradeable, IPosition {
  ...
  struct PositionDetails {
    ...
    uint256 rate;
    bytes32 borrower; // kept for later use, ie onchain svg generation
  ...
  // next position id
  uint256 public _nextId = 21;

  IBorrowPool public pool;
  ...
  function getPositionRepartition(uint256 tokenId) external view returns (uint256, uint256) {
    return
      pool.getAmountRepartition(
        _positions[tokenId].borrower,
  ...
  function deposit(
    ...
    (uint256 adjustedBalance, uint256 bondsIssuanceIndex) = pool.deposit(
      amount,
      rate,
      borrowerHash,
      underlyingToken,
    ...
    _positions[tokenId] = PositionDetails({
      adjustedBalance: adjustedBalance,
      rate: rate,
      borrower: borrowerHash,
    ...
    emit Deposit(to, tokenId, amount, rate, borrowerHash, address(underlyingToken));
  }

  function updateRate(uint256 tokenId, uint256 newRate) external override {
    require(ownerOf(tokenId) == _msgSender(), Errors.POS_MGMT_ONLY_OWNER);

    uint256 oldRate = _positions[tokenId].rate;

    (uint256 newAmount, uint256 newBondsIssuanceIndex) = pool.updateRate(
      _positions[tokenId].adjustedBalance,
      _positions[tokenId].borrower,
      oldRate,
      newRate,
    ...
    emit UpdateRate(
      ...
      newRate,
      _positions[tokenId].borrower,
      address(_positions[tokenId].underlyingToken)
    );
  }
  ...
  function withdraw(uint256 tokenId) external override {
    ...
    (uint256 withdrawnDeposit, uint256 remainingBonds, uint256 bondsMaturity) = pool.withdraw(
      _msgSender(),
      _positions[tokenId].adjustedBalance,
      _positions[tokenId].rate,
      _positions[tokenId].bondsIssuanceIndex,
    ...
    _positions[tokenId].adjustedBalance -= withdrawnDeposit;
    _positions[tokenId].remainingBonds = remainingBonds;
    ...
    if (_positions[tokenId].adjustedBalance <= 100 && _positions[tokenId].remainingBonds == 0) {
      _burn(tokenId);
      delete _positions[tokenId];
      emit BurnPosition(_msgSender(), tokenId);
    }
  }
}
...
interface IBorrowPool {
  ...
  event Borrow(bytes32 indexed borrower, uint256 amount, address underlyingToken);
  event Repay(bytes32 indexed borrower, uint256 amount, address underlyingToken);
  event LateRepay(bytes32 indexed borrower, uint256 amount, address underlyingToken);
  ...
  event UpdateRate(
    uint256 oldAdjustedAmount,
    uint256 oldRate,
    uint256 newAdjustedAmount,
    uint256 newRate,
    uint256 newBondsIssuanceIndex
  );
  event TopUpManagementFees(bytes32 borrower, uint256 amount);
   ...
   * @return Bonds Issuance Index for the newly created position
   **/
  function deposit(
   ...
   * @param bondsIssuanceIndex The index that determines when the position was created
   * @param borrower The identifier of the borrower
   ...
   * @return updated amount of tokens of the position, adjusted to the new tick's global liquidity ratio
   * @return new bonds issuance index
   **/
  function updateRate(
    uint256 adjustedAmount,
    bytes32 borrower,
  ...
  /**
   * @notice Top Up management fees
   * @dev Add amount to the management fees reserve
   * @param normalizedAmount Amount of token set aside for management fees use
   **/
  function topUpManagementFees(uint256 normalizedAmount) external;
  ...
  /**
   * @notice Collect yield provider fees as well as management fees for the target rate
   * @dev Updates liquidity ratio for the target rate
   **/
  function collectFeesForTick(bytes32 borrower, uint256 rate) external;

  /**
   * @notice Collect yield provider fees as well as management fees for the whole order book
   * @dev Updates liquidity ratio for the target rate
   * Iterates over all orderbook initialised ticks
   **/
...
/// @title IPosition
/// @notice Contains methods that can be called by anyone to create and manage their position
interface IPosition {
  /**
   * @notice Emitted when #mint is called and is a success
   * @param lender Address of the lender depositing token on the platform
   * @param tokenId Id of the new position
   * @param amount Amount of deposited token
   * @param rate Rate required by lender to lend their deposited token
   * @param borrower Target borrower
   * @param underlyingToken Asset/Token deposited for lending purpose
   **/
  event Deposit(
    address indexed lender,
    uint256 tokenId,
   ...
   * @param lender Address of the lender updating their position
   * @param tokenId Id of the position that has just been updated
   * @param amount Amount of deposited token plus their accrued interests
   * @param rate New rate required by lender to lend their deposited token
   * @param borrower Target borrower
   * @param underlyingToken Asset/Token deposited for lending purpose
   **/
  event UpdateRate(
   ...
   * @param lender Address of the lender withdrawing its token from the platform
   * @param tokenId Id of the position that has just been burnt
   * @param amount Amount of withdrawn token
   * @param rate Rate required by lender on the position that has just been burnt
   * @param borrower Target borrower
   * @param underlyingToken Asset/Token to be withdrawn
   **/
  event Withdraw(
    address indexed lender,
    uint256 tokenId,
  ...
  /**
   * @notice Emitted after #sell or #withdraw is a success and there are no bond left nor remaining deposit
   * @param lender Address of the lender withdrawing or selling their token from the platform
   * @param tokenId Id of the position that has just been burnt
   **/
  event BurnPosition(address indexed lender, uint256 tokenId);

  /**
   * @notice Creates a new position and deposits tokens into the borrower's orderbook
   * @dev Will call the deposit function from BorrowPool contract
   * @param to Address that is depositing the tokens.
   * @dev the Address is recorded as the owner of the position token that is minted
   * @param amount Amount of deposited token
   * @param rate Rate required by lender on the new position
   **/
  function deposit(
    address to,
    uint256 amount,
  ...
  /**
   * @notice Updates the rate of an existing position in the relevant order book
   * @dev Will call the updateRate function from BorrowPool contract
   * Shall only be called for non-borrowed position
   * @param tokenId Id of the position token the rate of which is getting updated
   * @param newRate New rate required by lender on their deposited token
   **/
  function updateRate(uint256 tokenId, uint256 newRate) external;

  /**
   * @notice Withdraws the portion of the position that is deposited with the yield provider
   * @dev Will call the withdraw function from BorrowPool contract
   * Will only withdraw the non loaned part of the position, the position will still
   * exist and consist only of all bonds purchased before
...
library Errors {
  ...
  string public constant BP_TOKEN_TRANSFER = "30"; // "An error occurred during the transfer of the underlying token from user to JellyFi BorrowPool";
  ...
  string public constant BP_WITHDRAW_ON_EMPTY_POOL = "34"; // "Cannot withdraw on an empty pool";
  string public constant BP_TARGET_BOND_ISSUANCE_INDEX_EMPTY = "35"; // "Target bond issuance index has no amount to withdraw";
  string public constant BP_BOND_ISSUANCE_ID_TOO_HIGH = "36"; // "Bond issuance id is too high";
  string public constant BP_LOAN_ALREADY_ONGOING = "37"; // "There's already a loan ongoing";
  ...
  string public constant PSM_POOL_DEFAULTED = "33"; // "Pool Defaulted";
  // Position
