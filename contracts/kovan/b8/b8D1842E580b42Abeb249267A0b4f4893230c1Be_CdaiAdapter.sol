...
import "./interfaces/external/compound/ICErc20.sol";
import "./AdapterBase.sol";

contract CdaiAdapter is AdapterBase {
    ...
    constructor(
        ...
        require(
            ...
            "CdaiAdapter: underlying asset address is the zero address"
        );
        require(
            programAddress_ != address(0),
            "CdaiAdapter: cDai address is the zero address"
        );
        require(
            farmingPoolAddress_ != address(0),
            "CdaiAdapter: farming pool address is the zero address"
        );
        ...
        _cdai = ICErc20(programAddress);
        _underlyingAsset = IERC20(underlyingAssetAddress);
    }

    modifier onlyBy(address account) {
        require(msg.sender == account, "CdaiAdapter: sender not authorized");
        _;
    }
    ...
    function getTotalWrappedTokenAmountCore()
        ...
        returns (uint256)
    {
        return (_cdai.balanceOf(address(this))).mul(10**10);
    }
    ...
    function getWrappedTokenPriceInUnderlyingCore()
        ...
        returns (uint256)
    {
        uint256 cTokenDecimals = _cdai.decimals();
        require(
            cTokenDecimals == 8,
            "CdaiAdapter: cdai only has 8 decimal places"
        );

        uint256 exchangeRateMantissa = _cdai.exchangeRateStored();

        return exchangeRateMantissa.div(10**10);
    }
    ...
    function depositUnderlyingToken(uint256 amount)
        ...
        returns (uint256)
    {
        require(amount != 0, "CdaiAdapter: can't add 0");

        _underlyingAsset.safeTransferFrom(msg.sender, address(this), amount);

        require(
            _underlyingAsset.balanceOf(address(this)) >= amount,
            "CdaiAdapter: insufficient balance"
        );

        _underlyingAsset.safeApprove(programAddress, amount);
        uint256 beforeBalance = _cdai.balanceOf(address(this));

        require(_cdai.mint(amount) == 0, "CdaiAdapter: error minting cTokens");

        uint256 afterBalance = _cdai.balanceOf(address(this));
        //receivedWrappedTokenQuantity is using 18 instead of 8 decimals
        uint256 receivedWrappedTokenQuantity =
            (afterBalance.sub(beforeBalance)).mul(10**10);
    ...
    //RedeemCErc20Tokens
    function redeemWrappedToken(uint256 redeemTokens)
        external
        override
        onlyBy(farmingPoolAddress)
        returns (uint256, uint256)
    {
        require(redeemTokens != 0, "CdaiAdapter: can't redeem 0");

        uint256 beforeBalance = _cdai.balanceOf(address(this));
        uint256 underlyingAssetBeforeBalance =
            _underlyingAsset.balanceOf(address(this));

        //redeemTokens uses 18 decimals and _redeemTokens uses 8 decimals
        uint256 _redeemTokens = redeemTokens.div(10**10);

        require(
            _cdai.redeem(_redeemTokens) == 0,
            "CdaiAdapter: error redeeming cTokens"
        );

        uint256 afterBalance = _cdai.balanceOf(address(this));
        uint256 underlyingAssetAfterBalance =
            _underlyingAsset.balanceOf(address(this));

        uint256 receivedUnderlyingTokenQuantity =
            underlyingAssetAfterBalance.sub(underlyingAssetBeforeBalance);
        //actualAmount is using 18 instead of 8 decimals
        uint256 actualAmount = (beforeBalance.sub(afterBalance)).mul(10**10);
        ...
        onlyBy(governanceAccount)
    {
        require(
            ...
            "CdaiAdapter: new governance account is the zero address"
        );
        ...
        onlyBy(governanceAccount)
    {
        require(
            ...
            "CdaiAdapter: new farming pool address is the zero address"
        );
