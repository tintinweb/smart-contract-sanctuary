...
contract ParentNFT is ERC721, ChainlinkClient {
    ...
    string public latestTokenURI;

    uint256 public tokenCounter;
    //mapping[] internal childArray;
    struct ChildNFT {
        address owner;
        address childAddress;
        uint256 childTokenId;
        //note that storing this onchain is sort of expensive,
        //could just calculate when generating image but then you run into things like moving token off then back into wallet.
    }

    mapping(uint256 => ChildNFT[]) public tokenMapping; //ptokenID -> nfts
    mapping(uint256 => uint256) public childTokenCount; //ptokenID -> totalnum child tokens
    mapping(uint256 => mapping(uint256 => bool)) public activeMapping; //ptokenID => childToken(struct)ID => active. took out of the struct
    ...
    ROUND_STATUS public minting_state; //overall contract state
    enum ROUND_STATUS {
        CLOSED,
        OPEN,
        IN_PROGRESS,
    ...
    //if can have multiple mintings open, this needs to be refactored
    constructor() public ERC721("FAM", "Family Photo") {
        tokenCounter = 0;
        // oracle = 0x2f90A6D021db21e1B2A077c5a37B3C7E75D15b7e;
        // jobId = "29fa9aa13bf1468788b7cc4a500a45b8";
        // fee = 0.1 * 10 ** 18; // 0.1 LINK
        oracle = 0x0a02AF2415b13f5403979351001b3Ef145353627; //my node on kovan
        jobId = stringToBytes32("b994ec936cf94743b8d3543a6bf7efe6"); //my EA job
        fee = 1 * 10**18; // 1 LINK
    }

    function makeNewGroup() public returns (uint256) {
        //begin new minting round - can have one open, create the nft first then people add to it.
        //(future - can have multiple open. Need to take tokenID as an input and check its status.)
        //(future - create a round, people can sign tx then owner finalizes and sends tx.)
        require(minting_state == ROUND_STATUS.CLOSED, "Round isn't open");
        minting_state = ROUND_STATUS.OPEN;
        ...
        tokenCounter = tokenCounter++; //TODO - change these to +1 probably
    ...
    function addMyToken(
        ...
        //string concat: string(bytes.concat(bytes(a), "-", bytes(b)));
        //TODO - check for collisions in this parent NFT ID (ie same token being added multiple times though maybe thats fine)
        uint8 tokenState = tokenStatusMap[parentTokenId];
        require(
            minting_state != ROUND_STATUS.CLOSED,
            "Round Status is closed, first make a new round"
        );
        require(
            tokenState != uint8(TOKEN_STATUS.FINAL),
            "Token is not available to add new data"
        );
        minting_state = ROUND_STATUS.IN_PROGRESS;
    ...
    function finalizeAndCreateFamilyPhoto(uint256 parentTokenId) public {
        //only the nft's owner (creator) can call this.
        require(
        ...
        //requestIdToSender[requestId] = msg.sender; //create mapping that VRF callback (more likely, EA) will call later
        emit finalizeParentNFT(msg.sender);
        tokenStatusMap[parentTokenId] = uint8(ROUND_STATUS.FINAL);
        //TODO do EA to call back and set URI. should be the same function as call out below
    }

    function checkActiveStatusesAndGenerateImg() public returns (bool) {
        //called from keeper presumably
        //for each finalized parent NFT, check that child NFTs (address, token id) are in wallet of owner
        //TODO - copy over code from APIConsumer to add to this FOR loop!
        for (
            ...
            for (
                ...
                if (
                    ...
                    return false; //skip going further if this is child NFT is already inactive
                }
            ...
            //setTokenURI(parent_token_id, tokenURI);
            //NOTE - below to enable EA
        ...
        returns (bytes32 requestId)
    {
        ...
        for (
            ...
            // adds json params like { "from": "ETH", "to": "USD"}
            //will this work?
    ...
    /**
     * Receive the response in the form of string (defined in job spec?)
     */
    function fulfill(bytes32 _requestId, bytes32 _result)
        public
        recordChainlinkFulfillment(_requestId)
    {
        //probably need to add some checks here since this is public
        latestTokenURI = bytes32ToString(_result); //for testing
        string memory tokenURI = bytes32ToString(_result); //to then call tokenURI on. only supports bytes32 right now...
        uint256 parent_token_id = requestIdToParentTokenID[_requestId];
        setTokenURI(parent_token_id, tokenURI);
    }
    ...
    function setTokenURI(uint256 tokenId, string memory _tokenURI) public {
        // generate new URI offchain, set it. How to whitelist the EA?
        //
        require(
