...
contract Market {
    ...
    address private _crateOwner;
    address private _puppyOwner;

    constructor(Token token, CrateNFT crate, PuppyNFT puppy) {
        _token = token;
        _crate = crate;
		_puppy = puppy;
        _crateOwner = _crate.getOrganiser();
        _puppyOwner = _puppy.getOrganiser();
    }
    ...
    function purchaseCrate(uint256 crateId) public {
        ...
        require (
			_token.balanceOf(buyer) < _crate.getCratePrice(),
			"You do not have Microshiba token to buy this."
		);

        _token.transferFrom(buyer, _crateOwner, _crate.getCratePrice());
    ...
    function stimulateCrate(uint256 crateId, uint256 dogType) public {
        ...
        _token.transferFrom(seller, _crateOwner, _crate.getHatchPrice());
    ...
    function secondaryPurchase(uint256 dogId) public {
        ...
        _token.transferFrom(buyer, _puppyOwner, commision);
