...
abstract contract Control is AccessControl {
	modifier isValidPercent(uint256 percent) {
		require(percent >= 0 && percent <= 100, "Contract must be initialized");
		_;
	}
	...
	bytes32 internal constant DEV = keccak256("dev");
	...
	// some var which is percent must set between [0,100]
	uint256 internal _transferFee = 50;
	uint256 internal _feeFree = 10;
	uint256 internal _operateFee = 10;
	uint256 internal _burnFee = 10;
	uint256 internal _feeDistribAmongOthers = 80;
	...
	// in percent
	uint256 internal _coeffInflations = 12;

	// in days
	uint56 internal _period = 3;

	function setOperationAddress(address adr) external onlyRole(DEFAULT_ADMIN_ROLE) {
		require(adr != address(0), "Address must not be '0x0'");
		address _operationAddressLocal = _operationAddress;
		if (_operationAddressLocal == address(0)) {
			grantRole(OPERATION, adr);
		} else {
			revokeRole(OPERATION, _operationAddressLocal);
			grantRole(OPERATION, adr);
		}
	...
	function setRebaseAddress(address adr) external onlyRole(DEFAULT_ADMIN_ROLE) {
		require(adr != address(0), "Address must not be '0x0'");
	...
	function setPoolAddress(address poolAddress) external isPermitToChange {
	...
	function setFeeAddress(address feeAddress) external isPermitToChange {
		require(feeAddress != address(0), "Address must not be 0");
		_feeAddress = feeAddress;
	}

	function setOwnerOldContractToken(address ownerOldContractToken) external isPermitToChange {
		require(ownerOldContractToken != address(0), "Address must not be 0");
		_ownerOldContractToken = ownerOldContractToken;
	}

	function addToWhiteList(address adr) external isPermitToChange {
		require(adr != address(0), "Address must not be '0x0'");
		require(_isWhiteList[adr] == false, "Address must not be in whitelist");
		_isWhiteList[adr] = true;
	}

	function removeFromWhiteList(address adr) external isPermitToChange {
		require(adr != address(0), "Address must not be '0x0'");
		require(_isWhiteList[adr] == true, "Address must be in whitelist");
		_isWhiteList[adr] = false;
	}

	function setTransferFee(uint256 transferFee) external isValidPercent(transferFee) isPermitToChange {
		_transferFee = transferFee;
	}

	function setFeeFree(uint256 feeFree) external isValidPercent(feeFree) isPermitToChange {
		_feeFree = feeFree;
	}

	function setOperateFee(uint256 operateFee) external isValidPercent(operateFee) isPermitToChange {
		_operateFee = operateFee;
	}

	function setBurnFee(uint256 burnFee) external isValidPercent(burnFee) isPermitToChange {
		_burnFee = burnFee;
	}

	function setFeeDistribAmongOthers(uint256 feeDistrib) external isValidPercent(feeDistrib) isPermitToChange {
		_feeDistribAmongOthers = feeDistrib;
	}

	function setInflactionKoeff(uint256 inflKoef) external isPermitToChange {
		require(inflKoef != 0, "Must be not 0");
		_coeffInflations = inflKoef;
	}

	function setPeriod(uint32 period) external isPermitToChange {
...
contract Cryptium is CrptERC20, Control, ReentrancyGuard {
	...
	event Log(string msg, uint256 val, uint256 val2, uint256 val3);
	...
	constructor(string memory _name, string memory _symbol) CrptERC20(_name, _symbol) {
	...
	function initializeContract(
		...
		require(owner != address(0), "Address must not be 0");
		_isInitialized = true;
		_crpto = crpto;
		_crptp = crptp;
		_isInitialized = true;
	 ...
	 */
	function getCurrentPrice() external view initialized returns (uint256) {
	 ...
	 */
	function getFreeToSeil() external view initialized returns (uint256 limitAvail) {
	...
	function _getCurrentPrice() private view returns (uint256) {
		...
		uint8 decDiff = getTokenDecimalsDiff();
	...
	function _getAveragePrices() private view returns (uint256, uint256) {
		(uint256 oldPrice, uint256 newPrice) = getTWAT();
	 ...
	 * @return newPrice
	 */
	function getTWAT() private view initialized returns (uint256 oldPrice, uint256 newPrice) {
		uint32 valueCount = 3;
		uint32 period = uint32(_period * (1 days));
		...
		uint8 decDiff = getTokenDecimalsDiff();
		return getPrice(tickCumulatives, period, decDiff);
	}
	...
	function getPrice(
		...
		uint32 period,
		uint8 decDiff
	) public view initialized returns (uint256 oldPrice, uint256 newPrice) {
	 ...
	 */
	function getTokenDecimalsDiff() private view returns (uint8 decDiffUnsign) {
	...
	function _transfer(
		...
		emit Log("value fee feeGon", value, fee, feeGon);
	...
	function _feeCount(
		...
		if (_isWhiteList[from]) {
			...
			UserLimit storage limit = _userLimit[from];
