...
contract HAQQ is ERC20, Ownable {
    ...
    */
    function _buyTokens() private returns (bool success){
        ...
        // uint msgValueInUsdCents = msg.value * ((price / 10 ** 8) * 100);
        // uint msgValueInUsdCents = msg.value * (price / (10 ** 10));
        uint msgValueInUsdCents = msg.value * price / (10 ** 10);
        ...
        if (
            (currentTier < 3) &&
            totalSupply() >= (tiers[currentTier + 1].startsOnTokensSold) * (10 ** decimals())
        ) {
            _upgradeTier();
        }
        return true;
