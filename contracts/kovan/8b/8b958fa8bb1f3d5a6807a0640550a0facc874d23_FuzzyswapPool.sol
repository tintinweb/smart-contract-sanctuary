...
contract FuzzyswapPool is PoolState, PoolImmutables, IFuzzyswapPool {
     ...
     */
    function _modifyPosition(address owner, int24 bottomTick, int24 topTick, int128 liquidityDelta)
        private
        returns (
        ...
        )
    {
        ...
        (uint256 feeGrowthInside0X128, uint256 feeGrowthInside1X128) =
            ticks.getInnerFeeGrowth(bottomTick, topTick, _globalState.tick, _totalFeeGrowth0Token, _totalFeeGrowth1Token);

        _recalculatePosition(position, liquidityDelta, feeGrowthInside0X128, feeGrowthInside1X128);

        // if liquidityDelta is negative, e.g. the liquidity was removed, and also the tick was flipped, it
...
//    }
    ...
    function burn(
        ...
        tickValidation(bottomTick, topTick);
        (Position storage position, int256 amount0Int, int256 amount1Int) =
            _modifyPosition(
                msg.sender,
                bottomTick,
                topTick,
    ...
    function _changeFee(
        ...
        fee = IDataStorageOperator(dataStorageOperator).getFee(
            _time,
            _tick,
            _index,
            _liquidity
    ...
    struct SwapCache {
        // the community fee for the input token
        uint8 communityFee;
    ...
    struct SwapState {
        ...
        // amount of input token paid as community fee
        uint128 communityFee;
    ...
    function swap(
        ...
        (amount0, amount1, blockTimestamp, state) = _calculateSwap(
            zeroForOne,
            amountSpecified,
            sqrtPriceLimitX96
        );
        ...
        _changeFee(blockTimestamp, state.tick, globalState.timepointIndex, liquidity);
    ...
    function _calculateSwap(        
        ...
        int256 amountSpecified,
        uint160 sqrtPriceLimitX96
        ) private returns(int256 amount0, int256 amount1, uint32 blockTimestamp, SwapState memory state) {
        ...
        SwapCache memory cache =
            SwapCache({
                ...
                communityFee: zeroForOne ? (globalStateStart.communityFee % 16) : (globalStateStart.communityFee >> 4),
        ...
        if(activeIncentive.virtualPool != address(0)){
            ...
            (,uint32 _time,,,,) = IDataStorageOperator(dataStorageOperator).timepoints(globalStateStart.timepointIndexSwap);
            if (_time != blockTimestamp){
                if(activeIncentive.endTimestamp > blockTimestamp && activeIncentive.startTimestamp < blockTimestamp){
        ...
        while (state.amountSpecifiedRemaining != 0 && state.sqrtPriceX96 != sqrtPriceLimitX96) {
            ...
            // if the community fee is on, calculate how much is owed, decrement feeAmount, and increment communityFee
            if (cache.communityFee > 0) {
                uint256 delta = step.feeAmount / cache.communityFee;
                step.feeAmount -= delta;
                state.communityFee += uint128(delta);
            }
            ...
            if (state.sqrtPriceX96 == step.sqrtPriceNextX96) {
                ...
                if (step.initialized) {
                    ...
                    if (!cache.computedLatestTimepoint) {
                        (cache.tickCumulative, cache.secondsPerLiquidityCumulative,,) = IDataStorageOperator(dataStorageOperator).getSingleTimepoint(
                            blockTimestamp,
                            0,
                            globalStateStart.tick,
                            globalStateStart.timepointIndex,
                            cache.liquidityStart
                        );
                        cache.computedLatestTimepoint = true;
                    }
                    if(hasActiveIncentive){
        ...
        if (state.tick != globalStateStart.tick) {
            ...
            (globalState.sqrtPriceX96, globalState.tick, globalState.timepointIndex, globalState.timepointIndexSwap) = (
                state.sqrtPriceX96,
                state.tick,
                timepointIndex,
                timepointIndex
        ...
        // update fee growth global and, if necessary, community fees
        // overflow is acceptable, protocol has to withdraw before it hits type(uint128).max fees
        if (zeroForOne) {
            totalFeeGrowth0Token = state.feeGrowthGlobalX128;
            if (state.communityFee > 0) communityFees.token0 += state.communityFee;
        } else {
            totalFeeGrowth1Token = state.feeGrowthGlobalX128;
            if (state.communityFee > 0) communityFees.token1 += state.communityFee;
        }

        return (amount0, amount1, blockTimestamp, state);
    }
    ...
    function flash(
        ...
        if (paid0 > 0) {
            uint8 communityFee0 = globalState.communityFee % 16;
            uint256 fees0 = communityFee0 == 0 ? 0 : paid0 / communityFee0;
        ...
        if (paid1 > 0) {
            uint8 communityFee1 = globalState.communityFee >> 4;
            uint256 fees1 = communityFee1 == 0 ? 0 : paid1 / communityFee1;
    ...
    /// @inheritdoc IFuzzyswapPoolPermissionedActions
    function setCommunityFee(uint8 communityFee0, uint8 communityFee1) external override lock onlyFactoryOwner {
        ...
        uint8 communityFeeOld = globalState.communityFee;
        globalState.communityFee = communityFee0 + (communityFee1 << 4);
        emit SetCommunityFee(communityFeeOld % 16, communityFeeOld >> 4, communityFee0, communityFee1);
    }
    ...
    function collectCommunityFee(
        ...
        if (amount0 > 0) {
            if (amount0 == communityFees.token0) amount0--; // ensure that the slot is not cleared, for gas savings
            communityFees.token0 -= amount0;
            TransferHelper.safeTransfer(token0, recipient, amount0);
        }
        if (amount1 > 0) {
            if (amount1 == communityFees.token1) amount1--; // ensure that the slot is not cleared, for gas savings
...
import './pool/IFuzzyswapPoolPermissionedActions.sol';
...
interface IFuzzyswapPool is
    ...
    IFuzzyswapPoolPermissionedActions,
...
import '../interfaces/pool/IFuzzyswapPoolState.sol';
import '../libraries/TickManager.sol';

abstract contract PoolState is IFuzzyswapPoolState {
    struct GlobalState {
    ...
    // @inheritdoc IFuzzyswapPoolState
    ProtocolFees public override communityFees;
...
import '../interfaces/pool/IFuzzyswapPoolImmutables.sol';
import '../interfaces/IFuzzyswapPoolDeployer.sol';

abstract contract PoolImmutables is IFuzzyswapPoolImmutables {
    ...
    constructor(address deployer) {
        (dataStorageOperator, factory, token0, token1) = IFuzzyswapPoolDeployer(deployer).parameters();
    }
}
...
library SqrtPriceMath {
    ...
    function getNextSqrtPriceFromInput(
        ...
        return getNextSqrtPrice(price, liquidity, amountIn, zeroForOne, true);
    }
    ...
    function getNextSqrtPriceFromOutput(
        ...
        return getNextSqrtPrice(price, liquidity, amountOut, zeroForOne, false);
    }
    ...
    function getNextSqrtPrice(
        ...
        if (zeroForOne == fromInput) { // rounding up or down
            ...
            if (fromInput) {
                ...
                if ((product = amount * price) / amount == price) {
                    ...
                    if (denominator >= numerator1)
                        ...
                        return uint160(FullMath.mulDivRoundingUp(numerator1, price, denominator));
                }

                return uint160(FullMath.divRoundingUp(numerator1, (numerator1 / price).add(amount)));
            } else {
                uint256 product;
                // if the product overflows, we know the denominator underflows
                // in addition, we must check that the denominator does not underflow
                require((product = amount * price) / amount == price && numerator1 > product);
                uint256 denominator = numerator1 - product;
                return FullMath.mulDivRoundingUp(numerator1, price, denominator).toUint160();
            }
        } else {
            ...
            if (fromInput) {
                ...
                return uint256(price).add(quotient).toUint160();
            } else {
                uint256 quotient =
                    (
                        amount <= type(uint160).max
    ...
    function getAmount0Delta(
        ...
        uint256 numerator1 = uint256(liquidity) << Constants.RESOLUTION;
        uint256 numerator2 = priceUpper - priceLower;
        ...
        return
            roundUp
                ? FullMath.divRoundingUp(
                    FullMath.mulDivRoundingUp(numerator1, numerator2, priceUpper),
                    priceLower
                )
                : FullMath.mulDiv(numerator1, numerator2, priceUpper) / priceLower;
    }
    ...
    function getAmount1Delta(
        ...
        return
            roundUp
                ? FullMath.mulDivRoundingUp(liquidity, priceUpper - priceLower, Constants.Q96)
                : FullMath.mulDiv(liquidity, priceUpper - priceLower, Constants.Q96);
    }
    ...
    function getAmount0Delta(
        ...
        return
            liquidity < 0
                ? -getAmount0Delta(priceLower, priceUpper, uint128(-liquidity), false).toInt256()
                : getAmount0Delta(priceLower, priceUpper, uint128(liquidity), true).toInt256();
    }
    ...
    function getAmount1Delta(
        ...
        return
            liquidity < 0
                ? -getAmount1Delta(priceLower, priceUpper, uint128(-liquidity), false).toInt256()
                : getAmount1Delta(priceLower, priceUpper, uint128(liquidity), true).toInt256();
    }
}
 ...
 */
interface IFuzzyswapPoolPermissionedActions {
    /**
