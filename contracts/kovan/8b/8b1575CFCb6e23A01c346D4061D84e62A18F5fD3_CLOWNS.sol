...
contract CLOWNS is IERC777Recipient {
    ...
    uint256 public immutable _minBondDuration; // TODO: should maybe be a governance parameter
    IERC1820Registry constant internal _ERC1820_REGISTRY =
        IERC1820Registry(0x1820a4B7618BdE71Dce8cdc73aAB6C95905faD24);

    event NewCLO(address clo, uint256 bond, int96 exitRate);

    constructor(ISuperfluid host, uint256 minBondDuration) {
        _host = ISuperfluid(host);
        _minBondDuration = minBondDuration;
        address cfaAddr = address(
            host.getAgreementClass(keccak256("org.superfluid-finance.agreements.ConstantFlowAgreement.v1"))
        );
    ...
    function _becomeCLO(ISuperToken token, address newCLO, uint256 amount, int96 exitRate) internal {
        ...
        require(uint256(exitRate) * _minBondDuration <= amount, "CLOWNS: below minimum bond duration");
        require(amount > token.balanceOf(address(this))-amount, "CLOWNS: bid too low");

        // close stream to current CLO if exists
        (uint256 timestamp, int96 curFlowRate,,) = _cfa.getFlow(token, address(this), currentCLO);
        if (curFlowRate > 0) {
