...
// File: @openzeppelin/contracts/token/ERC20/extensions/IERC20Metadata.so
...
contract PugFactory is Ownable {
    ...
    event RewardsPerBlockUpdated(uint256 updatedRewardsPerBlock);
    event FeeUpdated(uint256 updatedFee);
    event PugStakingUpdated(address indexed updatedPugStaking);

    constructor(address _admin, address _pugToken, uint256 _pugCreationReward, uint256 _fee, uint256 _rewardsPerBlock, address _pugStaking) Ownable() {
    ...
    function claimRewards(uint256 _lastRewardBlock) external returns(uint256) {
        uint256 allocatedReward = getRewardsPerBlock(msg.sender);
        uint256 reward = (block.number - _lastRewardBlock) * allocatedReward;
        if(reward != 0) {
    ...
    function getRewardsPerBlock(address pug) view public returns(uint256) {
        uint256 _totalRewardPoints = totalRewardPoints;
        return (_totalRewardPoints != 0 ? (rewardPoints[pug] * rewardsPerBlock / _totalRewardPoints): 0);
    }
    ...
    function updateRewardPerBlock(uint256 _updatedRewards) external onlyOwner {
        rewardsPerBlock = _updatedRewards;
        emit RewardsPerBlockUpdated(_updatedRewards);
    }
