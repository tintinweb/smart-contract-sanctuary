...
contract Payout {
    ...
    function payoutERC20Batch(IERC20[] calldata tokens, address[] calldata recipients, uint[] calldata amounts) external isOwnerOrManager {
        require(tokens.length == amounts.length && amounts.length == recipients.length);
