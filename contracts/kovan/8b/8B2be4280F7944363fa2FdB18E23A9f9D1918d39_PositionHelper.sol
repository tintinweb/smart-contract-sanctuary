...
contract PositionHelper {
    ...
    function batchQuery(IOpenLev openLev, uint16[] calldata marketIds, address[] calldata traders, bool[] calldata longTokens, bytes calldata dexData) external view returns (Vars[] memory results){
        results = new Vars[](traders.length);
        for (uint i = 0; i < traders.length; i++) {
            Vars memory item;
            Types.Trade memory trade = openLev.activeTrades(traders[i], marketIds[i], longTokens[i]);
            if (trade.held == 0) {
                results[i] = item;
                continue;
            }
            ...
            (uint current,,,) = openLev.marginRatio(traders[i], marketIds[i], longTokens[i], dexData);
            item.marginRatio = current;
            Types.Market memory market = openLev.markets(marketIds[i]);
            uint borrowed = longTokens[i] ? market.pool0.borrowBalanceCurrent(traders[i]) : market.pool1.borrowBalanceCurrent(traders[i]);
