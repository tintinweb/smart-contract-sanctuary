...
contract Loot is NPassCore {
            ...
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "Bag #',
                        toString(tokenId),
                        '", "description": "Ubiq Loot is randomized adventurer gear generated and stored on chain. Gimme the Loot.", "image": "data:image/svg+xml;base64,',
    ...
    /* ========== CONSTRUCTOR ========== */
    /**
     * @param _nContractAddress nCeption contract address
     * @param _priceForNHoldersInWei Mint price for nCeption holders
     * @param _priceForOpenMintInWei Mint price for public sale
     * @param _burnerAddress Burner smart contract address where claimed funds will be forwarded to
     */
    constructor(
        address _nContractAddress,
        uint256 _priceForNHoldersInWei,
        uint256 _priceForOpenMintInWei,
        address payable _burnerAddress
    ) NPassCore("Ubiq Loot", "LOOT", IN(_nContractAddress), true, 8888, 0, _priceForNHoldersInWei, _priceForOpenMintInWei) Ownable() {
        require(_burnerAddress != address(0), "NPass: burner address is the zero address");
    ...
    function mint(uint256 tokenId) public payable override nonReentrant {
        ...
        // Send UBQ mint value to Burner address
