 ...
 */
contract XanaAuction is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    using SafeMath for uint256;
    using AddressUpgradeable for address payable;
    using SafeERC20 for IERC20;

    /// @notice Event emitted only on construction. To be used by indexers
    event XanaAuctionContractDeployed();
    ...
    function initialize(address payable _platformFeeRecipient, IERC20 _RWT, address _rewardTokenOwner, IXanaAddressRegistry _addressRegistry, IERC20 _Zora)
        public
        initializer
    {
        require(
            _platformFeeRecipient != address(0),
            "XanaAuction: Invalid Platform Fee Recipient"
        );
        ...
        emit XanaAuctionContractDeployed();
 ...
 */
        ...
        nonReentrant
    {
        ...
        IXanaMarketplace marketplace = IXanaMarketplace(
        ...
        if (payAmount > 0) {
            if (auction.payToken == address(0)) {
                ...
                if (payAmount != 0) {
                    if(Zora.balanceOf(IERC721(_nftAddress).ownerOf(_tokenId)) > 0){
                        RWT.safeTransferFrom(rewardTokenOwner, IERC721(_nftAddress).ownerOf(_tokenId), payAmount.mul(5).div(100));
                    }
                    if(Zora.balanceOf(winner) > 0){
                        RWT.safeTransferFrom(rewardTokenOwner, winner, payAmount.mul(5).div(100));
                    }
                }
            }
        }
        ...
        emit AuctionResulted(
            ...
            winner,
            auction.payToken,
            IXanaMarketplace(addressRegistry.marketplace()).getPrice(
                auction.payToken
            ),
            winningBid
        );
    ...
    /// @notice Method for set Zora Token
    /// @param _zoraTokenAddr Zora Token Address
    function setBundleZoraAddr(address _zoraTokenAddr)
        external
        onlyOwner
    {
        Zora = IERC20(_zoraTokenAddr);
    }
