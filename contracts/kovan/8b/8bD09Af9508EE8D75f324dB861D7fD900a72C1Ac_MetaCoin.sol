...
contract MetaCoin {
	address token0Addr = 0xd0A1E359811322d97991E03f863a0C30C2cF029C;
	address token1Addr = 0x1f9840a85d5aF5bf1D1762F925BDADdC4201F984;

	function getPoolAddr() public view returns (address){
		PoolAddress.PoolKey memory key = PoolAddress.getPoolKey(token0Addr, token1Addr, 3000);
		return PoolAddress.computeAddress(0x1F98431c8aD98523631AE4a59f267346ea31F984, key);
	}
	function getTokenPrice() external view returns (uint) {
		address oraclePool = getPoolAddr();
		int24 timeWeightedAverageTick = OracleLibrary.consult(oraclePool,3600);
		//To-DO: confirm 1 or 10^18
		uint quoteAmount = OracleLibrary.getQuoteAtTick(timeWeightedAverageTick, 10**18, token0Addr, token1Addr);
		return quoteAmount;
	}
}