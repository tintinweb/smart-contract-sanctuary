...
contract TornadoProxy is EnsResolve {
  ...
  function deposit(address _tornado, bytes32 _commitment, bytes calldata _encryptedNote) external payable {
    ...
    ITornadoInstance(_tornado).deposit.value(msg.value)(_commitment);
    tornadoTrees.registerDeposit(_tornado, _commitment);
    emit EncryptedNote(msg.sender, _encryptedNote);
  }

  function updateInstance(address _instance, bool _update) external onlyGovernance {
  ...
  function withdraw(
    ...
    ITornadoInstance(_tornado).withdraw.value(msg.value)(_proof, _root, _nullifierHash, _recipient, _relayer, _fee, _refund);
    tornadoTrees.registerWithdrawal(_tornado, _nullifierHash);
  }
