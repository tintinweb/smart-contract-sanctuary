...
/**
 *
 * NameChangeToken Contract (The native token of testNFTs)
 * @dev Extends standard ERC20 contract
 */
contract TestERC is Context, IERC20 {
    ...
    /**
     * @dev When accumulated testERCs have last been claimed for a testNFT index
     */
    function lastClaim(uint256 tokenIndex) public view returns (uint256) {
        require(INFT(_NFTAddress).ownerOf(tokenIndex) != address(0), "Owner cannot be 0 address");
        require(tokenIndex < INFT(_NFTAddress).totalSupply(), "NFT at index has not been minted yet");
    ...
    /**
     * @dev Accumulated testERC tokens for a testNFT token index.
     */
    function accumulated(uint256 tokenIndex) public view returns (uint256) {
        ...
        if (lastClaimed == emissionStart) {
            uint256 initialAllotment = INFT(_NFTAddress).isMintedBeforeReveal(tokenIndex) == true ? INITIAL_ALLOTMENT.mul(PRE_REVEAL_MULTIPLIER) : INITIAL_ALLOTMENT;
    ...
    /**
     * @dev Claim mints testERCs and supports multiple testNFT token indices at once.
     */
    function claim(uint256[] memory tokenIndices) public returns (uint256) {
        ...
        for (uint i = 0; i < tokenIndices.length; i++) {
            ...
            require(tokenIndices[i] < INFT(_NFTAddress).totalSupply(), "NFT at index has not been minted yet");
            ...
            require(INFT(_NFTAddress).ownerOf(tokenIndex) == msg.sender, "Sender is not the owner");
        ...
        require(totalClaimQty != 0, "No accumulated testERC");
