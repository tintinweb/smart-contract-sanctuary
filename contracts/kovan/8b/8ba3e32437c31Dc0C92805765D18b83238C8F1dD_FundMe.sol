...
contract FundMe {
    ...
    function EthereumToUSD(uint256 ethAmount) public view returns (uint256) {
        return (getPrice() * ethAmount) / (10**getdecimals()); // / (10 ** _maxDec); // parametrs , uint8 _maxDec - 18
    }

    //getEntraceFee
    // USD To Ethereum
    function USDToEthereum(uint256 _usdAmount) public view returns (uint256) {
        uint256 usdAmount = _usdAmount * 10**getdecimals(); // becuase getprice() has 8 decimals, it has to has 8 digit by own
