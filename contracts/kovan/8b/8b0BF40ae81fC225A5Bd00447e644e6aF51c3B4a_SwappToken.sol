...
abstract contract Declaration is Global {
    ...
    uint32 constant SECONDS_IN_DAY = 18000 seconds; // for testing
    ...
    ILiquidityGuard public LIQUIDITY_GUARD;
    ...
    function _setLiquidityGuard(address liquidityGuard) internal {
        LIQUIDITY_GUARD = ILiquidityGuard(liquidityGuard);
    }
...
abstract contract LiquidityToken is StakingToken {
    ...
    function createLiquidityStake(
        ...
        liquidityStakeID = generateLiquidityStakeID(msg.sender);

        newLiquidityStake.startDay = nextSwappDay();
        newLiquidityStake.stakedAmount = _liquidityTokens;
        newLiquidityStake.isActive = true;

        globals.liquidityShares = globals.liquidityShares.add(_liquidityTokens);
    ...
    function endLiquidityStake(
    ...
    ) snapshotTrigger external returns (uint256) {
        LiquidityStake memory liquidityStake = liquidityStakes[msg.sender][_liquidityStakeID];

        require(liquidityStake.isActive);

        liquidityStake.isActive = false;
        liquidityStake.closeDay = _currentSwappDay();

        liquidityStake.rewardAmount = _calculateRewardAmount(liquidityStake);

        _mint(msg.sender, liquidityStake.rewardAmount);
        ...
        globals.liquidityShares = globals.liquidityShares.sub(liquidityStake.stakedAmount);
    ...
    function _calculateRewardAmount(
        ...
        uint256 calculationDay =
            globals.currentSwappDay < maxCalculationDay ?
            globals.currentSwappDay : maxCalculationDay;
...
contract SwappToken is LiquidityToken {
    ...
    modifier onlyLiquidityTransformer() {
        require(
            ...
            'SWAPP: Invalid liquidity transformer'
        );
        _;
    }

    constructor(address liquidityGuard) ERC20("Swapp Token", "SWAPP") {
        transformerGateKeeper = msg.sender;
        _setLiquidityGuard(liquidityGuard);
     ...
     */
    function setLiquidityTransfomer(address _immutableTransformer) external {
        require(transformerGateKeeper == msg.sender);
     ...
     */
    function mintSupply(address _investorAddress, uint256 _amount) external onlyLiquidityTransformer {
     ...
     */
    function giveStatus(address _referrer) external onlyLiquidityTransformer {
