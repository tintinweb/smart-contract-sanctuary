...
contract VNFTX is ERC721 {
  ...
  function mintCryptoNFTs(string memory _name, string memory _tokenURI, uint256 _price) external returns (uint256) {
...
contract VonderHospital is  Ownable, IERC721Receiver { // โรงพยาบาล
    ...
    address public _VNFTX;

    constructor(address VNFTX) public {
        _VNFTX = VNFTX; // 0x003B5f941aC979dC8f41acdB3cCe59EF6aFAdF2d
    ...
    function changeVNFTX(address VNFTX) public onlyOwner {
        _VNFTX = VNFTX;
    }
    ...
    function breed(address lotteryNFTdad, uint256 _dadId, address lotteryNFTmum, uint256 _mumId) external returns (uint256){
        ...
        return _createVonderNFT(_lotteryNFTmum, _mumId, _lotteryNFTdad, _dadId, _newGeneration, _newDna, msg.sender);
    }
    ...
    function _createVonderNFT(
        ...
        uint256 newBornTokenId = IVNFTX(_VNFTX).mintCryptoNFTs(getOffSpringName(_mumId,_dadId), "QmeWYjWYAdRJtCH4ZzTt8yYPN3GiWS6jcBKrJE8WvXY9nM",0 );
        
        IVNFTX(_VNFTX).safeTransferFrom(address(this),_owner,newBornTokenId);
    ...
    function getOffSpringName(uint _mumId,uint _dadId) public pure returns (string memory) {
        ...
        return string(abi.encodePacked("Offspring of", " #", uint2str(_mumId), " #", uint2str(_dadId)));
    }
    ...
    function burn(uint256 _tokenId) public { // Burn Gen2
