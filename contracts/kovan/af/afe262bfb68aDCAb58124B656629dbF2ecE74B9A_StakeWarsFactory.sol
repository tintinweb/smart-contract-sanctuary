...
interface IAccessControl {
     ...
     *
     * `_DEFAULT_ADMIN_ROLE` is the starting admin for all roles, despite
...
contract StakeWarsInternals {
    ...
    constructor(uint256 _token, uint256 _SEED) {
        SEED = _SEED;
        token = _token;
    }
    ...
    function determineRarity(uint48 rareValue, uint256 seed)
        ...
        returns (Rarity)
    {
        rareValue = rareValue % 2**12;
        if (rareValue > 2**11) return Rarity.COMMON;
        else if (rareValue >= 2**10) return Rarity.FAIRLY_COMMON;
        else if (rareValue > 2**9) return Rarity.KEEPER;
        else if (rareValue > 2**8) return Rarity.SHINY;
        else if (rareValue > 2**7) return Rarity.BRONZE;
        else if (rareValue > 2**6) return Rarity.SILVER;
        else if (rareValue > 2**5) return Rarity.GOLD;
        else if (rareValue > 2**4) return Rarity.PLATNIUM;
        else if (rareValue > 2**3) return Rarity.SUPER_RARE;
        else if (rareValue > 2**2) return Rarity.SECRET;
        else if (rareValue > 2**1) return Rarity.SINGLES;
...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    /**
     * @dev Base URI for computing {tokenURI}.Can be overriden
     * in child contracts.
     */
...
// File: StakeWarsFactory.sol

contract StakeWarsFactory is ERC721Enumerable, Administerable, VRFConsumerBase {
    ...
    mapping(bytes32 => User) public _requestIdToUser;
        ...
        VRFConsumerBase(_vrfCoordinator, _linkToken)
    {
        ...
        securityKey2 = _securityKey2;
        linkFee = _linkFee;
        keyhash = _keyhash;
        maxSupply = _maxSupply;
        _defaultFullURI = defaultFullURI;
    }
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness)
        ...
        override
    {
        address sender = _requestIdToUser[requestId].sender;
        uint256 tokenId = _requestIdToUser[requestId].tokenId;
        StakeWarsInternals warrior = tokenIdToStakeWarrior[tokenId];
        uint48 rare48 = uint48(randomness);
        warrior.setCharacteristics(randomness, rare48, securityKey2);
        rarityRatio[uint48(warrior.determineRarity(rare48, securityKey2))]++;

        _safeMint(sender, tokenId);
    }

    function reserve() public payable onlyAdmin {
        bytes32 requestId = requestRandomness(keyhash, linkFee);
        _requestIdToUser[requestId] = User(msg.sender, _tokenIdsGen);
    ...
    function toggleSaleActive() public onlyAdmin {
        saleActive = !saleActive;
    }

    function togglePreSaleActive() public onlyAdmin {
    ...
    function _withdraw() public onlyAdmin {
