...
contract Octavate is ERC721, Ownable, ReentrancyGuard{
    ...
    function setEarlyAccess(address[] calldata addresses, uint8 max_mint) public onlyOwner {
        for (uint256 i = 0; i < addresses.length; i++) {
            earlyAccess[addresses[i]] = max_mint;
        }
    }
    ...
    // function setRandomArray(randomValue) private {
    //     rArr = new uint16[](MAX_SUPPLY);
    //     for (uint16 i = 0; i < MAX_SUPPLY; i++){
    //         rArr[i] = i + 1; //our nfts are 1-indexed
    //     }
    //     for (uint16 i = 0; i < rArr.length; i++) {
    //         uint16 n = i + uint256(keccak256(abi.encode(randomValue,i))) % (rArr.length - i);
    //         uint16 temp = rArr[n];
    //         rArr[n] = rArr[i];
    //         rArr[i] = temp;
    //     }
