...
contract CryptoAthletes is ERC721Enumerable, Ownable, ERC721Burnable, ERC721Pausable {
    ...
    function rand(address payable _to) private view returns(uint256) {
      uint256 total = _totalSupply();
      uint256 maxRandom = MAX_ELEMENTS - total;

      uint256 seed = uint256(keccak256(abi.encodePacked(
        block.timestamp + block.difficulty +
        ((uint256(keccak256(abi.encodePacked(block.coinbase)))) / (block.timestamp)) + 
        block.gaslimit + 
        ((uint256(keccak256(abi.encodePacked(_to)))) / (block.timestamp)) + 
        block.number
      )));

      return (seed - (seed / (maxRandom)) * (maxRandom));
    }

    function generateIds() public onlyOwner {
        for (uint256 i = 0; i < MAX_ELEMENTS; i++) {
            ids.push(i + 1);
        }
    }

    function deleteByIndex(uint256 _index) internal returns(uint256[] memory) {
      if (_index >= ids.length) {
        return ids;
      }

      for(uint256 i = _index; i < ids.length - 1; i++) {
        ids[i] = ids[i + 1];
      }
    ...
    function _mintAnElement(address payable _to) private {
        uint256 randomIndex = rand(_to);
        uint256 randomId = ids[randomIndex];
        ids = deleteByIndex(randomIndex);

        _tokenIdTracker.increment();
        _safeMint(_to, randomId);

        emit CreateCryptoAthletes(randomId);
    }
