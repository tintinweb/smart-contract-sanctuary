...
contract PriceOracleProxy is Initializable, PriceOracle, Exponential, Denominations {
    ...
    function getUnderlyingPrice(address cToken) public override view returns (uint256) {
        ...
        if (pairsWithEth[underlying].pair != address(0)) {
            return getPriceFromEthPair(underlying);
        }
    ...
    event AggregatorUpdated(address indexed tokenAddress, address base, address quote, bool isUsed);
    event IsLPUpdated(address indexed tokenAddress, bool isLP);
    event SetYVaultToken(address indexed tokenAddress, YvTokenVersion version);
    event SetCurveToken(address indexed tokenAddress, CurvePoolType poolType, address swap);
    event SetDAOVaults(address indexed tokenAddress, DAOVaultId vaultId);
    event SetTokensPairedWithEth(address indexed tokenAddress, address pair);
    ...
    function _updatePair(address pairAddress) public {
        require(msg.sender == admin, "only the admin may update the pair");
        (uint price0Cumulative, uint price1Cumulative, uint32 blockTimestamp) = 
