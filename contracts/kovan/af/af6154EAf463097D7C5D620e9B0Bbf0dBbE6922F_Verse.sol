    ...
    Pausable
{
    ...
    function mintBanana(address account, uint256[] memory ids, uint256[] memory amount, bytes memory data)
    public
    onlyOwner
    {
        _mintBatch(account, ids, amount, data);
    }
        /* Overrides */

    function _mint(address account, uint256 id, uint256 amount, bytes memory data) internal override(ERC1155) {
        require(id!=0, "ID cant be zero");
        require(amount==1, "amount needs to be 1");
        super._mint(account, id, amount, data);
    }

    function _mintBatch(address account, uint256[] memory ids, uint256[] memory amounts, bytes memory data) internal onlyOwner override(ERC1155) {
        super._mintBatch(account, ids, amounts, data);
    }

    function _burn(address account, uint256 id, uint256 amount) internal onlyOwner override(ERC1155) {
        super._burn(account, id, amount);
    }

    function _burnBatch(address account, uint256[] memory ids, uint256[] memory amounts) internal onlyOwner override(ERC1155) {
