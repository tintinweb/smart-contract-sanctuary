...
contract Staking is Ownable, ReentrancyGuard {
    ...
    function emergencyWithdraw() external onlyOwner onlyWhenOpen nonReentrant {
        ...
        if(totalLessRewards > 0){
            require(lessToken.transfer(owner(), lessToken.balanceOf(address(this))), "Error: can't send tokens");
        }
        if(totalLpRewards > 0){
            require(lpToken.transfer(owner(), lpToken.balanceOf(address(this))), "Error: can't send tokens");
        }
    }
