...
contract BTAP_BNB_Pool is Ownable {
    ...
    address public token = 0xB6C38e49985749C02d6D2FAa9ff8e5454B6f4990;
    
    // BTAP/BNB LP token contract address
    address public LPtoken = 0xB6C38e49985749C02d6D2FAa9ff8e5454B6f4990;
    
    // Charity address
    address public charityAddress = 0x62f8bD7a8C5515Ca00501A3BBE35590Ab45B6bb9;
    
    // Burn address
    address public burnAddress = 0x0026477C2173FD4D90e16444bcDdb7EAC50Ee036;
    
    // reward rate 300 % per year
    uint256 public rewardRate = 4819316;
    
    uint256 public charityFee = 25;
    uint256 public burnFee = 25;
    
    // reward interval 365 days
    uint256 public rewardInterval = 365 days;
    
    uint256 public minimumAmount = 12449899598 *10**8;
    ...
    function updateAccount(address account) private {
        ...
        if (pendingDivs > 0) {
            uint256 BurnFee = pendingDivs.mul(burnFee).div(10000);
            uint256 CharityFee = pendingDivs.mul(charityFee).div(10000);
            uint256 afterFee = pendingDivs.sub(BurnFee).sub(CharityFee);
            
            require(Token(token).transfer(account, afterFee), "Could not transfer tokens.");
            require(Token(token).transfer(burnAddress, BurnFee), "Could not transfer tokens.");
            require(Token(token).transfer(charityAddress, CharityFee), "Could not transfer tokens.");
    ...
    function deposit(uint256 amountToStake) public {
        ...
        require(Token(LPtoken).transferFrom(msg.sender, address(this), amountToStake), "Insufficient Token Allowance");
    ...
    function withdraw(uint256 amountToWithdraw) public {
        ...
        require(Token(LPtoken).transfer(msg.sender, amountToWithdraw), "Could not transfer tokens.");
    ...
    function emergencyWithdrawal() public {
        ...
        require(Token(LPtoken).transfer(msg.sender, depositedTokens[msg.sender]), "Could not transfer tokens.");
    ...
    // function to allow admin to set charity fee rate..
    function setCharityFee(uint256 _percent) public onlyOwner {
        charityFee = _percent;
    }
    
    // function to allow admin to set burn fee rate..
    function setBurnFee(uint256 _percent) public onlyOwner {
        burnFee = _percent;
    }
    
    // function to allow admin to set charity address..
    function setCharityAddress(address _address) public onlyOwner {
        charityAddress = _address;
    }
    
    // function to allow admin to set burn address..
    ...
    // function to allow admin to set maximum amount of BTAP/BNB LP token..
    function setMaximumAmount(uint256 amount) public onlyOwner {
        maximumAmount = amount;
    }
    
    // function to allow admin to set minimum amount of BTAP/BNB LP token..
    ...
    function transferAnyERC20Tokens(address _token, address _to, uint256 _amount) public onlyOwner {
        require(_token != LPtoken);
