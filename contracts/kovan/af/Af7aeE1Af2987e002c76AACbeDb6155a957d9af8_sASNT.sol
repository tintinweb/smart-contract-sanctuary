...
contract sASNT is ERC20Permit, Ownable {
    ...
    uint256 public _gonsPerFragment; //TODO change back to private
    mapping(address => uint256) public _gonBalances; //TODO change back to private
    ...
    function initialize( address stakingContract_ ) external returns ( bool ) {
        ...
        require( stakingContract_ != msg.sender );
        stakingContract = stakingContract_;
         
        uint256 value = 1 * 10**9;
        uint256 gonValue = value.mul( _gonsPerFragment );
        _gonBalances[ stakingContract ] = TOTAL_GONS.sub( gonValue );
        _gonBalances[ msg.sender ] = gonValue;

        emit Transfer( address(0x0), stakingContract, _totalSupply );
        emit Transfer( address(0x0), initializer, 1000000000 ); // send 1 sASNT to msg.sender addresse to have a minimum circulating supply
