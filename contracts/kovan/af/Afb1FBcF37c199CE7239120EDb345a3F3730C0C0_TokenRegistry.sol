...
contract TokenRegistry is AccessControl {
    ...
    /// @dev reverts if no mapping found
    ...
    {
        string memory tokenSymbol = addressToSymbol[tokenAddress];
        require(bytes(tokenSymbol).length > 0, "no-mapping-found");
        return tokenSymbol;
    }
    ...
    function getAddress(string memory tokenSymbol)
        ...
        returns (address)
    {
        address tokenAddress = symbolToAddress[tokenSymbol];
        require(tokenAddress != address(0), "no-mapping-found");
        return tokenAddress;
    }
}