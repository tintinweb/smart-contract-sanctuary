...
contract AaveV1Strategy is BaseRhoStrategy, PausableUpgradeable {
    using SafeERC20Upgradeable for IERC20MetadataUpgradeable;

    IAaveV1AToken public aToken;
    IAaveV1LendingPoolAddressesProvider public provider;
    ...
    function initialize(
        ...
        aToken = IAaveV1AToken(_aTokenAddr);
        provider = IAaveV1LendingPoolAddressesProvider(_lpAddressesProviderAddr);
    }
    ...
    function effectiveSupplyRate(uint256 delta, bool isPositive) external view override returns (uint256) {
        ...
        IAaveV1LendingPoolCore lpCore = IAaveV1LendingPoolCore(lpCoreAddr);
        address _reserve = address(underlying);
        uint256 newAvailableLiquidity =
            isPositive
                ? lpCore.getReserveAvailableLiquidity(_reserve) + delta
                : lpCore.getReserveAvailableLiquidity(_reserve) - delta;
        IAaveV1ReserveInterestRateStrategy interestRateStrategy =
            IAaveV1ReserveInterestRateStrategy(lpCore.getReserveInterestRateStrategyAddress(address(underlying)));
        (uint256 rateInRay, , ) =
    ...
    function deploy(uint256 _amount) public override onlyRole(VAULT_ROLE) {
        require(_amount > 0, "AaveV1Strategy: deploy amount must be positive");
        underlying.safeIncreaseAllowance(provider.getLendingPoolCore(), _amount);
        IAaveV1LendingPool lendingPool = IAaveV1LendingPool(provider.getLendingPool());
    ...
    function withdrawUnderlying(uint256 _amount) public override onlyRole(VAULT_ROLE) {
        require(_amount > 0, "AaveV1Strategy: withdraw amount must be positive");
        require(aToken.balanceOf(address(this)) >= _amount, "AaveV1Strategy: cannot withdraw more than balance");
        require(aToken.isTransferAllowed(address(this), _amount), "AaveV1Strategy: transfer not allowed by Aave V1");
...
/**
 * @dev Interface for Aave V1 A Token
 * Documentation: https://developers.aave.com/#atokens
 */
interface IAaveV1AToken {
    /**
 ...
 */
interface IAaveV1ReserveInterestRateStrategy {
    /**
