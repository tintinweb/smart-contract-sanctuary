...
contract Auction is  AuctionCore {

    constructor(address factory_, address dataStore_) {
        initialize(factory_, dataStore_);
    } //end constructor
...
import "./IAuctionDataStore.sol";
import "@openzeppelin/contracts/utils/Context.sol";

contract AuctionCore is   Context, StructsDef, TransferBase, ReentrancyGuard {
    using SafeMath for uint256;

    event NewAuction(uint256 _id);
    event AuctionCancelled(uint256 _id);
    event AuctionEnded(uint256 _id, uint256 _winningBidId);
    event NewBid(address indexed _bidder, uint256 _id, uint256 _bidId, address _currency, uint256 _amount);
    event BidWithdrawn(address indexed _bidder, uint256 _id, uint256 _bidId);
    

    bool initialized;
    IAuctionDataStore   _dataStore;
    IFactory            _factory;
    ...
    function initialize(
        ...
        _dataStore = IAuctionDataStore(dataStore_);
     ...
     * @param  _assetType   - Asset Type  ERC721 or ERC1155
    ...
    function create(
        ...
        require(_assetType == ERC721_ASSET_TYPE 
            || _assetType == ERC1155_ASSET_TYPE, "Auction#create: INVALID_ASSET_TYPE");
        ...
        _item.bidType          = (_isLimited ? TIMED_BID_LISTING : OPEN_BID_LISTING);
        _item.assetType        = _assetType;
        ...
        transfer(_assetType, _assetContract, _tokenId, _msgSender(), address(this), 1);

        FeeConfig memory _feeConfig = _factory.getFeeConfig();
        
        // Check Listing fee
        if(_feeConfig.listingFeePercent > 0) {
            uint256 listingFee = _item.askingPrice.value.mul(_feeConfig.listingFeePercent).div(10_000);
            
            if(isNativeAsset(_paymentToken)){
                require(listingFee == msg.value, "Auction#create: INVALID_ETH_FEE");
                transfer(ETH_ASSET_TYPE, _item.askingPrice.paymentToken, 0, address(this), _feeConfig.adminFeeAddress, listingFee);
            } else {
                transfer(ERC20_ASSET_TYPE, _item.askingPrice.paymentToken, 0, _msgSender(), _feeConfig.adminFeeAddress, listingFee);
            }
        }   
    ...
    function cancelAuction(uint256 _id) public onlyAuctionOwner(_id) {
        ...
        transfer(item.assetType, item.assetContract, item.tokenId, address(this), _msgSender(), 1);
     ...
     */
    function finalizeAuction(uint256 _id, uint256 _bidId) public nonReentrant onlyAuctionOwner(_id) {
        MarketItem memory item = _dataStore.getMarketItem(_id);
        ...
        if(_dataStore.getBidsLengthForAuction(_id) == 0) {
            ...
            if(item.auctionData.isTimeLimted) {
                ...
                if(winningBid.value > 0) {
                    ...
                    _distributeBid(item.assetContract, item.tokenId, winningBid.currency, address(this), _msgSender(), winningBid.value);
                }
            } else {
                ...
                transfer(item.assetType, item.assetContract, item.tokenId, address(this), choosenBid.bidder, 1);
                
                _distributeBid(item.assetContract, item.tokenId, choosenBid.currency, address(this), _msgSender(), choosenBid.value);
                
                _dataStore.closeBid(_bidId);
            }

            _dataStore.delMarketItem(_id);
            emit AuctionEnded(_id, winnerBidId);
        }
    }

    function _distributeBid(address contractAddr, uint256 tokenId, address paymentToken, address from, address to, uint256 amount) internal {
    ...
    function placeBid(uint256 _id, address currency, uint256 amount) public payable {
        ...
        if(isNativeAsset(item.askingPrice.paymentToken)) {
            require(amount == msg.value, "Auction#placeBid: INVALID_ETH_VALUE");
        } else {
        ...
        if(oldBid.isActive) {
            ...
            emit NewBid(_msgSender(), _id, oldBid.id, currency, oldBid.value.add(amount));
        }
        else {
            // add bid to Store
            Bid memory newBid;
            ...
            emit NewBid(_msgSender(), _id, newBidId, currency, amount);
        }
    }
     ...
     * @param _id uint256 ID of the created auction
     */
    function withdrawBid(uint256 _id) public nonReentrant {
        MarketItem memory item = _dataStore.getMarketItem(_id);
...
pragma abicoder v2;
import "../StructsDef.sol";
import "../Market/IMarketDataStore.sol";

abstract contract IAuctionDataStore is StructsDef, IMarketDataStore {
