...
// File: contracts/interfaces/cake/ICakePool.sol
...
// File: contracts/strategy/StrategyPencakeLp.sol
...
contract StrategyForCakeLp is INEKUStrategy, Ownable {
    ...
    address constant public neku = address(0x34D7255167CC6e9e29e1ed73Fb012C2162C1FA21);
    address constant public weth = address(0xd0A1E359811322d97991E03f863a0C30C2cF029C);
    
    uint public burnfee = 0;
    uint public fee = 100;
    uint public foundationfee = 0;
    ...
    function deposit() public override {
        ...
        if (_want > 0) {
            IERC20(want).safeApprove(cakePool, 0);
            IERC20(want).safeApprove(cakePool, _want);
            ICakePool(cakePool).deposit(pid, _want);
        }
    }
    ...
    function _withdrawAll() internal {
        ...
        if(balance > 0){
            ICakePool(cakePool).withdraw(pid, balance);
        }
    }

    function _withdrawSome(uint256 _amount) internal returns (uint) {
        ICakePool(cakePool).withdraw(pid, _amount);
        return _amount;
    }
    
    function harvest() external checkStrategist {
        ICakePool(cakePool).withdraw(pid, 0);
        doswap();
        dosplit();
        deposit();
    }
    ...
    function balanceOfPool() public view returns (uint) {
       (uint256 amount, ) = ICakePool(cakePool).userInfo(pid, address(this));
        return amount;
    }
