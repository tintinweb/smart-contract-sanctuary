...
/// @title Aloe predictions market to run on Kovan during the hackathon
/// @author Aloe Capital LLC
contract AloePredictionsKovan is AloePredictions {
    address immutable owner;

    constructor(IERC20 _ALOE, IUniswapV3Pool _UNI_POOL) AloePredictions(_ALOE, _UNI_POOL) {
        owner = msg.sender;
    }

    Bounds groundTruth;
    bool shouldInvertPricesNext;

    /// @dev Allows the owner to set groundTruth (copy it from Mainnet so that Kovan
    /// is realistic)
    function set(Bounds calldata _groundTruth, bool _shouldInvertPricesNext) external {
        require(msg.sender == owner, "Aloe: Only owner can advance during hackathon");

        groundTruth = _groundTruth;
        shouldInvertPricesNext = _shouldInvertPricesNext;
    }

    /// @dev Same as base class `advance()`, but only owner can call, and uses stored
    /// groundTruth rather than fetching it
