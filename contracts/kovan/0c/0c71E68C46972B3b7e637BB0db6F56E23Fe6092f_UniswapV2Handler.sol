...
contract UniswapV2Handler is Ownerable, ISwap {
    ...
    Aggreswap public aggreswap;
    IUniswapV2Factory public factory;
    IUniswapV2Router02 public router;

    constructor(Aggreswap aggreswap_, IUniswapV2Factory factory_, IUniswapV2Router02 router_) {
        aggreswap = aggreswap_;
        factory = factory_;
        router = router_;
    ...
    function swapExtractOut(
        ...
        require(aggreswap.isTokenSupported(tokenIn), "UniswapV2Handler: tokenIn is not supported");
    ...
    // ) external override returns (uint256) {
    //     require(recipient != address(0), "UniswapV2Handler: recipient is the zero address");
    //     (uint256 amountEst, address[] memory path) = _getBestIn(tokenIn, tokenOut, amountOut);
    //     uint256 amountInMax = amountEst.add(amountEst.mul(slippage).div(10000));
    //     require(IERC20(tokenIn).approve(address(router), amountInMax), "UniswapV2Handler: approve failed");
    ...
    function _getBestOut(address tokenIn, address tokenOut, uint256 amountIn) internal view returns (uint256, address[] memory) {
        ...
        if (pair != address(0)) {
            ...
            uint256[] memory amounts = router.getAmountsOut(amountIn, resultPath);
        ...
        for (uint256 i = 0; i < baseTokens.length; i++) {
            ...
            if (factory.getPair(tokenOut, baseTokens[i]) == address(0)) {
                continue;
            }
            ...
            tempPath[2] = tokenOut;
            uint256[] memory amounts = router.getAmountsOut(amountIn, tempPath);
    ...
    function _getBestIn(address tokenIn, address tokenOut, uint256 amountOut) internal view returns (uint256, address[] memory) {
        ...
        if (pair != address(0)) {
            ...
            resultPath[0] = tokenIn;
            resultPath[1] = tokenOut;
            uint256[] memory amounts = router.getAmountsIn(amountOut, resultPath);
        ...
        for (uint256 i = 0; i < baseTokens.length; i++) {
            ...
            tempPath[1] = baseTokens[i];
            tempPath[2] = tokenOut;
            uint256[] memory amounts = router.getAmountsIn(amountOut, tempPath);
    ...
    function _isBaseToken(address token) internal view returns (bool) {
        for (uint256 i = 0; i < baseTokens.length; i++) {
            if (baseTokens[i] == token) {
                return true;
            }
