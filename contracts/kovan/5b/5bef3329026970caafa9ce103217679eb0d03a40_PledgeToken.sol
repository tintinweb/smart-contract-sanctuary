...
contract PledgeToken is Ownable {
    ...
    address public apple = 0x323A4Df3855b5F64fd11bAE71af1c4992eE23588; //收益币种

    address public masterChef = 0x76bAAB64424E8c7c50A9CD56da25eb6ac9adf1C0; //挖矿合约
    
    uint256 public period = 259200; //锁仓周期

    address public certificate; //存入凭证

    address public token; //本金币种

    uint256 public pid; //矿池ID

    address public taxAddress; //税务地址

    uint256 public taxRadio = 0; //税务比例

    struct UserInfo {
        uint256 amount;     // 用户本金量
        uint256 rewardDebt; // 用户投入时刻矿池总收益
        uint256 created_at; // 用户投入时间 
    }

    mapping (address => UserInfo) public userInfo; //用户属性表
    ...
    function pendingOre(address _user) external view returns (uint256) {
        (IERC20 _lp, uint256 _allocPoint, uint256 _lastRewardBlock, uint256 _accOrePerShare ) = IMasterChef(masterChef).poolInfo(pid);
    ...
    function deposit(uint256 _amount) public {
        ...
        (, , , uint256 _accOrePerShare ) = IMasterChef(masterChef).poolInfo(pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(_accOrePerShare).div(1e12).sub(user.rewardDebt);
            safeAppleTransfer(msg.sender, pending);
        }
        mintCertificate(_amount);
        IMasterChef(masterChef).deposit(pid, _amount);
        user.amount = user.amount.add(_amount);
        user.rewardDebt = user.amount.mul(_accOrePerShare).div(1e12);
