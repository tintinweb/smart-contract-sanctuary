...
contract Manager is IManager, Initializable, Ownable, Pausable, ReentrancyGuard {
    ...
    function registerController(bytes32 id, address controller) external override onlyOwner {
    ...
    function unRegisterController(bytes32 id) external override onlyOwner {
    ...
    function registerPool(address pool) external override onlyOwner {
        require(!pools.contains(pool), "POOL_EXISTS");
        pools.add(pool);
        emit PoolRegistered(pool);
    }

    function unRegisterPool(address pool) external override onlyOwner {
        require(pools.contains(pool), "INVALID_POOL");
        pools.remove(pool);
        emit PoolUnregistered(pool);
    }

    function setCycleDuration(uint256 duration) external override onlyOwner {
    ...
    function completeRollover(string calldata rewardsIpfsHash) external override onlyOwner {
        require(block.number > (currentCycle.add(cycleDuration)), "PREMATURE_EXECUTION");
        _completeRollover(rewardsIpfsHash);
    }

    function execute(CycleData calldata params) external override onlyOwner {
        require(block.number > (currentCycle.add(cycleDuration)), "PREMATURE_EXECUTION");
        ...
        for (uint256 x = 0; x < params.cycleSteps.length; x++) {
            address controllerAddress = registeredControllers[params.cycleSteps[x].controllerId];
            controllerAddress.functionDelegateCall(
                params.cycleSteps[x].data,
                "CYCLE_STEP_EXECUTE_FAILED"
            );
            emit DeploymentStepExecuted(
                params.cycleSteps[x].controllerId,
                controllerAddress,
                params.cycleSteps[x].data
            );
        }
    ...
    function startCycleRollover() external override onlyOwner {
...
interface IManager {
    ...
    function execute(CycleData calldata params) external;
...
contract Staking is IStaking, Initializable, Ownable {
...
contract Pool is ILiquidityPool, Initializable, ERC20, Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
...
contract EthPool is ILiquidityEthPool, Initializable, ERC20, Ownable 
{
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    using Address for address;
...
contract Toke is ERC20Pausable  {
