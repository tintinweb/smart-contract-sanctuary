...
event InsertOrderV3(uint amountIn, uint amountOut);

function insertOrderV3(uint amount, uint minout, address[] memory paths, uint24[] memory fees) external onlyTraders{
    ...
    emit InsertOrderV3(amount, amounts);
}
