...
contract OnePlanetOffset is ERC20Interface, Owned, SafeMath {
    ...
    constructor() public {
        ...
        checkGasFactor = 1;
        offsetSwitch = 1;
        offsetMsg = "These tokens are climate positive using 1PLANET. Visit www.1PLANET.app to verify Tx info. Burning one 1PLCO2 = offsetting 1 metric ton of CO2.";
    ...
    modifier estGas {
     ...
     modifier OnePlanetCarbonOffset {
        ...
        CO2Emitted = safeDiv(CO2Emitted, 1e15);
        CO2Emitted = safeMul(CO2Emitted, 1e15);
        require(OnePlanetInstance.balanceOf(address(this)) >= CO2Emitted, "Not enough 1PLANET tokens to retire"); //Require that the NFT minter has enough 1PLANET tokens
    ...
    function buy1PLwithDai(uint256 daiAmount) public returns (bool success) {
        ...
        require(address(daiAmount) == daiAddress, "Token is not DAI");
    ...
    function getLatestPrice() public {
        ...
        ethPrice1PL = safeDiv(tokenPrice, ethPrice);
    }
    ...
    function retireOnePL(uint tokens, string calldata message) external estGas returns (bool success) {
        tokens = safeDiv(tokens, 1e15);
        tokens = safeMul(tokens, 1e15); // only retire in kg
