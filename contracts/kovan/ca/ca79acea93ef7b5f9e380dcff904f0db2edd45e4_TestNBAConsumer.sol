...
contract TestNBAConsumer is ChainlinkClient {
    using Chainlink for Chainlink.Request;
  
    uint256 public twoPointersPercentage;
    uint256 public twoPointersMade;
    ...
    event PlayerStatsFound(uint256 twoPointersPercentage, uint256 twoPointersMade, uint256 points);
    ...
    function requestPlayerData(string calldata _date, uint256 _playerID) public returns (bytes32 requestId) 
    {
        Chainlink.Request memory request = buildChainlinkRequest(jobId, address(this), this.fulfill.selector);
        request.add("date", _date);
        request.addUint("playerID", _playerID);
        return sendChainlinkRequestTo(oracle, request, fee);
    }
    
    function fulfill(bytes32 _requestId, uint256 _twoPointsPercentage, uint256 _twoPointersMade, uint256 _points) public recordChainlinkFulfillment(_requestId)
    {
        twoPointersPercentage = _twoPointsPercentage;
        twoPointersMade = _twoPointersMade;
        points = _points;
        emit PlayerStatsFound(_twoPointsPercentage, _twoPointersMade, _points);
    }
}