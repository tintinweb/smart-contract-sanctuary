...
contract RewardDistributor is Ownable, Pausable {
    ...
    address constant public LON_TOKEN_ADDR = 0x0000000000095413afC295d19EDeb1Ad7B71c952;
    ...
    struct FeeToken {
        ...
        address[] path;
        uint256 LFactor; // Percentage of fee token reserved for feeTokenRecipient
        uint256 RFactor; // Percentage of buyback-ed lon token for treasury
        bool enable;
        uint256 minBuy;
        uint256 maxBuy;
        uint256 lastTimeBuyback;
    }
    ...
    *************************************************************/
    constructor(
        ...
        FeeToken storage feeToken = feeTokens[LON_TOKEN_ADDR];
        feeToken.RFactor = 40;
        feeToken.minBuy = 100;
    ...
    *************************************************************/
    ...
    function setFeeTokens(
        ...
        uint256[] memory _exchangeIndex,
        address[][] memory _path,
        uint256[] memory _LFactor,
        uint256[] memory _RFactor,
        bool[] memory _enable,
    ...
    function enableFeeTokens(address[] memory _feeTokenAddr, bool[] memory _enable) external only_Operator_or_Owner {
    ...
    function _validate(
        ...
        if (amountFeeTokenToSwap > 0) {
            ...
            require(block.timestamp > _feeToken.lastTimeBuyback.add(uint256(buybackInterval)), "already a buyback recently");
        }
    }
    ...
    function _buyback(
        ...
        if (_amountFeeTokenToSwap > 0) {
            ...
            _feeToken.lastTimeBuyback = block.timestamp;
    ...
    *************************************************************/
    function buyback(address _feeTokenAddr, uint256 _amount, uint256 _minLonAmount) external whenNotPaused only_EOA_or_Self {
        ...
        if (_feeTokenAddr == LON_TOKEN_ADDR) {
            ...
            feeToken.lastTimeBuyback = block.timestamp;
            return;
        }
