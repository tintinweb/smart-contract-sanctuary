...
interface PControllerLensInterface {
    function markets(address) external view returns (bool, uint);
    function oracle() external view returns (PriceOracle);
    function getAccountLiquidity(address) external view returns (uint, uint, uint);
    function getAssetsIn(address) external view returns (PToken[] memory);
    function claimPesa(address) external;
    function pesaAccrued(address) external view returns (uint);
}
...
contract PesaLens {
    ...
    function pTokenMetadata(PToken pToken) public returns (PTokenMetadata memory) {
        ...
        PControllerLensInterface pController = PControllerLensInterface(address(pToken.pController()));
        (bool isListed, uint collateralFactorMantissa) = pController.markets(address(pToken));
        address underlyingAssetAddress;
        uint underlyingDecimals;

        if (pesaareStrings(pToken.symbol(), "pETH")) {
    ...
    function pTokenUnderlyingPrice(PToken pToken) public returns (PTokenUnderlyingPrice memory) {
        PControllerLensInterface pController = PControllerLensInterface(address(pToken.pController()));
        PriceOracle priceOracle = pController.oracle();
    ...
    function getAccountLimits(PControllerLensInterface pController, address account) public returns (AccountLimits memory) {
        (uint errorCode, uint liquidity, uint shortfall) = pController.getAccountLiquidity(account);
        require(errorCode == 0);

        return AccountLimits({
            markets: pController.getAssetsIn(account),
            liquidity: liquidity,
            shortfall: shortfall
        });
    }

    struct PesaBalanceMetadata {
        uint balance;
    }

    function getPesaBalanceMetadata(Pesa pesa, address account) external view returns (PesaBalanceMetadata memory) {
        return PesaBalanceMetadata({
            balance: pesa.balanceOf(account)
        });
    }

    struct PesaBalanceMetadataExt {
        uint balance;
        uint allocated;
    }

    function getPesaBalanceMetadataExt(Pesa pesa, PControllerLensInterface pController, address account) external returns (PesaBalanceMetadataExt memory) {
        uint balance = pesa.balanceOf(account);
        pController.claimPesa(account);
        uint newBalance = pesa.balanceOf(account);
        uint accrued = pController.pesaAccrued(account);
        uint total = add(accrued, newBalance, "sum pesa total");
        uint allocated = sub(total, balance, "sub allocated");

        return PesaBalanceMetadataExt({
            balance: balance,
            allocated: allocated
        });
    }

    function pesaareStrings(string memory a, string memory b) internal pure returns (bool) {
