...
contract LockDeposits is Ownable {
    ...
    /// @dev finish timestamp for locking (deposit.finishDate must be < than that)
    uint32 public immutable finishTimestamp;
    ...
    constructor(address _rewardToken, uint32 _finishTimestamp) {
        if (_finishTimestamp >= block.timestamp + SECONDS_PER_YEAR) revert InvalidFinishTimestamp();
        if (RewardToken(_rewardToken).decimals() != 18) revert TokenNotSupported();
        ...
        onlyOwner
    {
        ...
        if (RewardToken(_token).decimals() != 18) revert TokenNotSupported();
    ...
    function lock(address _token, uint32 _period, uint256 _amount) external {
        unchecked {
            ...
            if (!RewardToken(_token).transferFrom(msg.sender, address(this), _amount)) revert TokenTransferFailed();
            ...
            if (now32 + _period > finishTimestamp) revert LockingNotPossible();

            deposit.amount = uint128(_amount);
            deposit.startDate = now32;
            deposit.finishDate = now32 + _period;
            deposit.APY = uint32(baseRate * multiplier / RATE_DECIMALS / RATE_DECIMALS);
    ...
    function withdrawMany(address[] calldata _tokens, uint256[] calldata _indexes) public {
        for (uint256 i; i < _tokens.length; i++) {
            withdraw(_tokens[i], _indexes[i]);
        }
    }

    function claim(address _token, uint256 _index) external virtual {
    ...
    // TODO
//    function exitAndCall(address _token, uint256 _index, address _contract, bytes memory _data) external {
//        claim();
//        withdraw();
//        // and call
//    }

    /// @return totalReward total amount of reward token that user earns during lock period
    ...
    function withdraw(address _token, uint256 _index) public {
        unchecked {
            ...
            uint32 now32 = uint32(block.timestamp);
            if (deposit.finishDate <= now32) revert DepositLocked();

            uint256 amount = deposit.amount;
            if (amount == 0) return;

            deposit.withdrawnAt = now32;

            emit LogWithdraw(_token, msg.sender, _index, amount);
            RewardToken(_token).transfer(msg.sender, amount);
        }
    }
    ...
    function balanceOf(address _token, address _address) public view returns (uint256 totalLocked, uint256 reward) {
        unchecked {
            ...
            for (uint256 i; i < lastIndex; i++) {
                ...
                reward += _calculateReward(deposit, timestamp);
            }
        }
    }
}
