...
contract Admin{
    ...
    /**
     * Transfer ERC1155 token and ERC20 token then update onSaleBalance
     */
    function transfer(address _ERC20Address, address _ERC20From, address _ERC20To, uint _ERC20Value, address _ERC1155Address, address _ERC1155From, address _ERC1155To, uint _ERC1155NftId, uint _ERC1155Value, bytes memory _ERC1155Bytes )public returns (bytes memory){
        ...
        (bool successOffSale, bytes memory returndataOffSale) = address(_ERC1155Address).call(abi.encodeWithSignature("offSale(address,uint256,uint256)", _ERC1155From, _ERC1155NftId, _ERC1155Value));
        
        if (!(successERC20 && successERC1155 && successOffSale) )
            revert();
