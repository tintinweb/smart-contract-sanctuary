...
/**
 * @title A consumer contract for Coinmetrics EA 'burned-eth' endpoint.
 * @author LinkPool
 * @notice Request Burned ETH since EIP-1559.
 * @dev EstimateGas functions only have to store the result in the storage.
 */
contract CoinmetricsBurnedETHConsumer is ChainlinkClient {
    ...
    /**
     * @notice Returns the burned amount of ETH since EIP-1559 release day.
     * @dev Result format is in wei as uint256.
     * @param _jobId the job ID.
     * @param _payment the LINK amount in Juels (i.e. 10^18 aka 1 LINK).
     */
    function requestBurnedEthHistorical(bytes32 _jobId, uint256 _payment) public {
        Chainlink.Request memory req = buildChainlinkRequest(_jobId, address(this), this.fulfillBurnedEth.selector);

        req.add("endpoint", "burned-eth");

        requestOracleData(req, _payment);
    }

    /**
     * @notice Returns the burned amount of ETH since start date until today.
     * @dev Result format is in wei as uint256.
     * @param _jobId the job ID.
     * @param _payment the LINK amount in Juels (i.e. 10^18 aka 1 LINK).
     * @param _startDate the start date (YYYY-MM-DD).
     */
    function requestBurnedEthStart(
        bytes32 _jobId,
        uint256 _payment,
    ...
    /**Copy_EnetpulseConsumerV2
     * @notice Returns the burned amount of ETH since EIP-1559 release day until end date.
     * @dev Result format is in wei as uint256.
     * @param _jobId the job ID.
     * @param _payment the LINK amount in Juels (i.e. 10^18 aka 1 LINK).
     * @param _endDate the end date (YYYY-MM-DD).
     */
    function requestBurnedEthEnd(
        bytes32 _jobId,
        uint256 _payment,
    ...
    /**
     * @notice Returns the burned amount of ETH from start date to end date.
     * @dev Result format is in wei as uint256.
     * @param _jobId the job ID.
     * @param _payment the LINK amount in Juels (i.e. 10^18 aka 1 LINK).
     * @param _startDate the start date (YYYY-MM-DD).
    ...
    function fulfillBurnedEth(bytes32 _requestId, uint256 _burnedETH) public recordChainlinkFulfillment(_requestId) {
        burnedETH = _burnedETH;
        requestIdData[_requestId] = _burnedETH;
    }
