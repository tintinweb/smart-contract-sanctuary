...
interface IMediaEyeSubRecieverMed {
    function subscribeLevelOneRelay(address account, uint256 startTimestamp, uint256 endTimestamp) external;
}
...
contract MediaEyeSubSenderMed {
  ...
  function subscribeLevelOne(address account, uint256 startTimestamp, uint256 endTimestamp) external onlySubscription {
    bytes4 methodSelector = IMediaEyeSubRecieverMed(address(0)).subscribeLevelOneRelay.selector;
