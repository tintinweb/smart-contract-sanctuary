...
contract Arb is Ownable, Initializable {
    ...
    function arb(uint256 inputAmount) external onlyOwner {
        ERC20(usdc).transferFrom(msg.sender, address(this), inputAmount);
        ExchangeRouter.Swap memory swap = ExchangeRouter.Swap({
            pool: pool,
            tokenIn: usdc,
            tokenOut: gyd,
            swapAmount: 1e18,
        ...
        ExchangeRouter.Swap[] memory array = new ExchangeRouter.Swap[](1);
        array[0] = swap;


            ExchangeRouter.Swap[][] memory swapSequences
         = new ExchangeRouter.Swap[][](1);
        swapSequences[0] = array;

        uint256 sammOutAmount = ExchangeRouter(router).multihopBatchSwapExactIn(
            swapSequences,
            TokenInterface(usdc),
            TokenInterface(gyd),
            1e18,
