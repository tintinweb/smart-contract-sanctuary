...
contract Oracle is AccessControl, IOracle {
    ...
    constructor(uint256 _maxTimeout) {
        maxTimeout = _maxTimeout;

        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _setupRole(DATA_PROVIDER, _msgSender());
    }

    function uploadData(address[] calldata tokens, uint256[] calldata values) external override {
        require(tokens.length == values.length, "Oracle: Error inputs");
        require(
            hasRole(DATA_PROVIDER, _msgSender()),
            "Not allowed"
        );
    ...
    function getData(address[] calldata tokens) external view override returns (bool[] memory isValidValue, uint256[] memory tokensPrices) {
