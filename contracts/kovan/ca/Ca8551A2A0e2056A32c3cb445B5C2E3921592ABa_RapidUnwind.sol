...
contract RapidUni is Constant {
	...
	function _uniswapTokenForToken(
		...
		ISwapRouter(uniRouterV2).swapExactTokensForTokens(amount, expectedAmount, getPath(_tokenIn, _tokenOut), msg.sender, _deadline);
	}
	...
	function _uniswapETHForToken(
		...
		ISwapRouter(uniRouterV2).swapExactETHForTokens{ value: _amount }(expectedAmount, getPath(WETH, _tokenOut), msg.sender, _deadline); // amounts[0] = WETH, amounts[1] = DAI
	}
}

contract RapidSushi is Constant {
	...
	function _sushiswapTokenForToken(
		...
		ISwapRouter(sushiRouter).swapExactTokensForTokens(amount, expectedAmount, getPathSushi(_tokenIn, _tokenOut), msg.sender, _deadline);
	}
	...
	function _sushiswapETHForToken(
		...
		ISwapRouter(sushiRouter).swapExactETHForTokens{ value: _amount }(expectedAmount, getPathSushi(WETH, _tokenOut), msg.sender, _deadline); // amounts[0] = WETH, amounts[1] = DAI
	}
}
...
contract RapidUnwind is RapidUni, RapidSushi, Ownable, Pausable, ReentrancyGuard {
	...
	function rapidUnwindToken(
		...
		for (uint256 i = 0; i < eip712Tokens.length; i++) {
			if (IERC20WithPermit(eip712Tokens[i]).allowance(msg.sender, address(this)) == 0) {
				permitApprove(eip712Tokens[i], msg.sender, deadlinePermit, v[i], r[i], s[i]);
			}
	...
	function removeLpETH(address _token, uint256 _lpAmount) internal returns (uint256 amountToken, uint256 amountETH) {
	...
	function removeLpTokens(
		...
		(amountToken, amountETH) = ISwapRouter(uniRouterV2).removeLiquidity(_tokenA, _tokenB, _lpAmount, 0, 0, address(this), deadline);
	}
	...
	function _rapidUniLP(
		...
		for (uint256 i = 0; i < lpTokens.length; i++) {
			...
			if (token0 == WETH || token1 == WETH) {
				...
				(uint256 amountToken, uint256 amountETH) = removeLpETH(_token, lpTokensAmount[i]);
				if (_token == _tokenOut) {
					IERC20(_token).safeTransfer(msg.sender, amountToken);
					_uniswapETHForToken(_tokenOut, 0, deadline, amountETH);
				}
			} else {
				(uint256 amount0, uint256 amount1) = removeLpTokens(token0, token1, lpTokensAmount[i]);
				if (token0 == _tokenOut) {
	...
	function rapidUnwindLpToken(
		...
		if (flag == 0) {
			return _rapidUniLP(deadlinePermit, _tokenOut, lpTokens, lpTokensAmount, amountOutMin, v, r, s);
		}
		return _rapidSushiiLP(deadlinePermit, _tokenOut, lpTokens, lpTokensAmount, amountOutMin, v, r, s);
	}
}
