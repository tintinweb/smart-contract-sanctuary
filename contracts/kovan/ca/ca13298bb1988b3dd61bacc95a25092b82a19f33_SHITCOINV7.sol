 ...
 * Functions for removing fees, and updating fee addresses
...
contract SHITCOINV7 is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"Shitcoin v7";
    string private constant _symbol = unicode"SHITCOINV7";
    uint8 private constant _decimals = 9;
    uint256 private _taxFee = 6;
    uint256 private _teamFee = 4;
    uint256 private _feeRate = 1;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from != owner() && to != owner()) {
            ...
            if(!inSwap && from != uniswapV2Pair && _tradingOpen) {
                // take fee on sells
                takeFee = true;
                // higher fee on sells within first 24 hours post-launch
    ...
    function toggleDevFee() external {
        require(_msgSender() == _feeAddress1);
        _useDevFee = !_useDevFee;
        emit UseDevFeeBooleanUpdated(_useDevFee);
    }

    function updateTaxFee(uint256 fee) external {
        require(_msgSender() == _feeAddress1);
    ...
    function updateTeamFee(uint256 fee) external {
        require(_msgSender() == _feeAddress1);
        require(fee <= 10, "Fee is too high");
        _teamFee = fee;
        emit TeamFeeUpdated(_teamFee);
    }
