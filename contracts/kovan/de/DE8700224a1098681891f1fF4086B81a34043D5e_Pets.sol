...
interface IPets {
    ...
    function isAlive(Pet memory pet) external returns(bool);

    function isSleeping(Pet memory pet) external returns(bool);

    function hasEnergy(Pet memory pet, uint minEnergy) external returns(bool);
