...
contract CompetitionPool is Ownable {
    ...
    IChainLinkOracleSportData public chainlinkOracleSportData;
    ...
    constructor(address _chainlinkOracleSportData, address _competitionFactory, address _tokenAddress)
    {
        chainlinkOracleSportData = IChainLinkOracleSportData(
            _chainlinkOracleSportData
        );
    ...
    function depositeLinkTokenForOracle(uint256 _amount) public {
        IERC20(linkTokenForOracle).safeTransfer(
            address(chainlinkOracleSportData),
            _amount
        );
    }
    ...
    function _setRateforCreator(address _pool)
        private
    {
        address _creator = creator[_pool];
        (uint256 _ownerRate, uint256 _creatorRate, uint256 _winnerRate) = getRate(_creator);
    ...
    function getRate(address _creator) public view returns(uint256 _ownerRate, uint256 _creatorRate, uint256 _winnerRate) {
        uint256 _index;
        uint256 balance = IERC20(tokenAddress).balanceOf(_creator);
        for (uint256 i = bracketsAmountToken.length-1; i > 0; i--) {
            if (
                balance >= bracketsAmountToken[i]
            ) {
                _index = i;
                break;
            }
        }
        return (rewardRate[_index][0], rewardRate[_index][1], rewardRate[_index][2]);
    }
...
interface IRegularCompetitionContract {
    ...
    function getPrivacy() external view returns (bool);
}
