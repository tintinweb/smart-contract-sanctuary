...
contract IDOCreator is Ownable {
    ...
    function createIDO(
        ...
        uint256 feeAmount = idoMaster.feeAmount();
        if(feeAmount > 0){
            uint256 burnPercent = idoMaster.burnPercent();
            uint256 burnAmount =  feeAmount.mul(burnPercent).div(idoMaster.divider());
            ERC20Burnable feeToken = idoMaster.feeToken();
        ...
        idoMaster.registrateIDO(msg.sender,
            address(idoPool),        
            _tokenPrice,
            address(_rewardToken),
            _startTimestamp,
...
import "./interfaces/IidoMaster.sol";
 ...
 contract IDOPool is Ownable, Pausable, Whitelist, ReentrancyGuard  {
  ...
  function setVesting(uint256 _vestingPercent,
        ...
        require(vestingPercent <= 100, "Vesting percent must be <= 100");
        if(vestingPercent > 0)
        {
            require(vestingInterval > 0 , "TokenVesting #createVesting: interval must be greater than 0");
            require(vestingDuration >= vestingInterval, "TokenVesting #createVesting: interval cannot be bigger than duration");
        }
    }
    ...
    function withdrawFunds() external onlyOwner nonReentrant{
        idoMaster.feeWallet().transfer(address(this).balance.mul(feeFundsPercent).div(100)); /* Fee Address */
