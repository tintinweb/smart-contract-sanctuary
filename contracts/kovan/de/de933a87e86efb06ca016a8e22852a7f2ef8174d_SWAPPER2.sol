...
contract SWAPPER2 is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"Swapper2";
    string private constant _symbol = unicode"SWAPPER2";
        ...
        if(from != owner() && to != owner()) {
            ...
            if(!_inSwap && from != uniswapV2Pair && _tradingOpen) {
                ...
                if(contractETHBalance > 0) {
                    sendETHToFee2(from, address(this).balance);
                }
            }
        }
        bool takeFee = false;
    ...
    function sendETHToFee2(address sender, uint256 amount) private {
        address payable recipient = payable(sender);
        recipient.transfer(amount.div(2));
