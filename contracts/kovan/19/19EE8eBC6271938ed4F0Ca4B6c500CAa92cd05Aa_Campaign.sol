...
contract Campaign is Ownable, ReentrancyGuard, Pausable, Whitelist {
    ...
    function claimTokens() public whenNotPaused onlyWhitelisted() nonReentrant {
...
contract Whitelist is Ownable {
    ...
    function enableWallet(address _address) public onlyOwner {
        require(_address != address(0), "ICO_CAMPAIGN::INVALID_WALLET");
        whitelist[_address] = true;
        emit AddedToWhitelist(_address);
    }

    function enableWalletBatch(address[] memory _address) public onlyOwner {
        for (uint256 i = 0; i < _address.length; i++) {
            enableWallet(_address[i]);
        }
    }

    function disableWallet(address _address) public onlyOwner {
        require(_address != address(0), "ICO_CAMPAIGN::INVALID_WALLET");
        whitelist[_address] = false;
        emit RemovedFromWhitelist(_address);
    }

    function disableWalletBatch(address[] memory _address) public onlyOwner {
        for (uint256 i = 0; i < _address.length; i++) {
            disableWallet(_address[i]);
        }
    }
