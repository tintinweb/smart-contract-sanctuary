    ...
    function mint(uint256 amount) external {
        ...
        for(uint i=0; i<amount; i++) {
            ...
            if (randNum==0&&randNum<5) {
                string[2] memory imgs = ["Qma4GioKvo7yEzyN8Mk5G5qGmQ4w7g7nrdXwiix8iugthK", "QmTyQ3Mab2711GWGMyvXqMVCQpK49WH1KQRzhUBJS96Q7W"];
                warrior = Warrior("Dragon", 6, 45000+genRand(5001, randNum), imgs, executeBackAmount);
            } else if (randNum>=5&&randNum<100) {
                string[2] memory imgs = ["QmPkQJkvoo4HRKniqwwNZynAs5wr381PUCdAWqjmKgZmPy", "QmcNEMp12fU7tNyTt8oRBKeW7QjXR9gLKwJoXPYAav86XV"];
                warrior = Warrior("Minotaur", 5, 4000+genRand(1501, randNum), imgs, executeBackAmount);
            } else if (randNum>=100&&randNum<800) {
                string[2] memory imgs = ["QmRWs3ToWngasDdZD9kKDwYKcnpK5saGYK2bCCaniL4ZtV", "QmZwioRnSrxDUcDK1e4LNkvGi7WW9eJXnHrEwXavathiwe"];
                warrior = Warrior("Dwarf", 4, 3000+genRand(1000, randNum), imgs, executeBackAmount);
            } else if (randNum>=800&&randNum<2200) {
                string[2] memory imgs = ["QmXJpBzqkR2BxRREGGcFPAuKvbuJwaLYWsSXDurKPZjF5p", "Qmbdyp864SwZZ2X3yBWKX1YeSax4BRj8F3yvLM9PcgAXYw"];
                warrior = Warrior("Satyr", 3, 2000+genRand(1000, randNum), imgs, executeBackAmount);
            } else if (randNum>=2200&&randNum<5000) {
                string[2] memory imgs = ["QmcrAjTFb7RsCEiAkqcy3rcr9CUCNX17PG3QpVnkQMQ4vd", "QmPbsQEreJNFVRkNkgyzDNTgkQ6Xt6GrPQXjz83FYTyQMi"];
                warrior = Warrior("Gnome", 2, 1000+genRand(1000, randNum), imgs, executeBackAmount);
            } else {
                string[2] memory imgs = ["QmT2kUH6H6vmNKrgGhcg34jaWY7RresHoyCocThZiA9Q5z", "QmRXAzXAA8oiuMxe9m8Tdp9oq4tsBXgDjkpNz3Tz5uuRj2"];
