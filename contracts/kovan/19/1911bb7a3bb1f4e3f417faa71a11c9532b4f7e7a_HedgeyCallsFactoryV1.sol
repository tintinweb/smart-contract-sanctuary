...
contract HedgeyCallsV1 is ReentrancyGuard {
    ...
    function getOptionDetails(uint _c) external view returns (
        address short, 
        uint assetAmt, 
        uint strike, 
        uint totalPurch, 
        ...
        short = call.short;
        assetAmt = call.assetAmt;
        strike = call.strike;
        totalPurch = call.totalPurch;
        price = call.price;
        expiry = call.expiry;
        open = call.open;
    ...
    function transferPymt(bool _isWETH, address _token, address from, address payable to, uint _amt) internal {
        if (_isWETH) {
            require(msg.value == _amt, "transfer issue: transferring wrong eth amount");
    ...
    function buyOptionFromNewShort(uint _c, uint _d) payable public nonReentrant {
        ...
        require(newAsk.short == newAsk.long, "d_put not newAsk"); //new Ask is always set such that the long == short
    ...
    event NewOptionOpened(uint _c);
    event OpenOptionSold(uint _c);
    event OpenShortRePurchased(uint _c);
    event OpenOptionPurchased(uint _c);
    event OptionChanged(uint _c);
...
contract HedgeyCallsFactoryV1 {
    ...
    function createContract(address asset, address pymtCurrency) public {
        ...
        HedgeyCallsV1 callContract = new HedgeyCallsV1(asset, pymtCurrency, collector, fee, collectorSet);
