...
// File: contracts/interfaces/IWethERC20.sol
...
// File: contracts/core/objects/LoanStruct.sol
...
// File: contracts/core/objects/LoanParamsStruct.sol
...
// File: contracts/core/objects/OrderStruct.sol
...
// File: contracts/core/objects/LenderInterestStruct.sol
...
// File: contracts/core/objects/LoanInterestStruct.sol
...
// File: contracts/core/Objects.sol
...
// File: contracts/mixins/EnumerableBytes32Set.sol
...
// File: contracts/swaps/ISwapsImpl.sol
...
interface ISwapsImpl {
    ...
    function dexExpectedRate(
        address sourceTokenAddress,
        address destTokenAddress,
        uint256 sourceTokenAmount) // sourceTokenAmount 变量没有用到
        external
        view
        returns (uint256);
...
// File: contracts/feeds/IPriceFeeds.sol
...
// File: contracts/testhelpers/TestToken.sol
...
// File: contracts/swaps/connectors/testnet/SwapsImplWootrade.sol
...
contract SwapsImplWootrade is State, ISwapsImpl {
        ...
        returns (uint256 destTokenAmountReceived, uint256 sourceTokenAmountUsed)
    {
        ...
        (uint256 tradeRate, uint256 precision) = IPriceFeeds(_priceFeeds).queryRate(
            sourceTokenAddress,
            destTokenAddress
        );
        ...
        // ????????????????????????????????????????????????????? 这个需要调整，实际操作应该不是burn和mint
        // TestToken(sourceTokenAddress).burn(sourceTokenAmountUsed); //销毁原token
        // TestToken(destTokenAddress).mint(address(this), destTokenAmountReceived); //mint目标token
        // if (returnToSenderAddress != address(this) && sourceTokenAmountUsed < maxSourceTokenAmount) {
        //     // send unused source token back 未消耗的token转回
        //     IERC20(sourceTokenAddress).safeTransfer(
        //         returnToSenderAddress,
        //         maxSourceTokenAmount-sourceTokenAmountUsed
        //     );
        // }
        // 只要保证合约的测试币种有足够的余额就行了
        // 模拟把 sourceToken 留在合约 targetToken 发给调用者
        ...
        // 将token 从合约中转移到 msg.sender
        IERC20 token = IERC20(destTokenAddress);
        uint256 balance = token.balanceOf(address(this));
        if (balance < destTokenAmountReceived)
            destTokenAmountReceived = balance;
