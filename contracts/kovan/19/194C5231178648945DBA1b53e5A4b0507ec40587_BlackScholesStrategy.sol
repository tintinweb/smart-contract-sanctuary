...
contract BlackScholesStrategy is Ownable {
    ...
    mapping(address => uint256) volatility;
    mapping(address => AggregatorInterface) oracles;
    mapping(address => uint256) lastRebalanced; 
    ...
    function getIdealPrice(address lsp) public view returns (uint256 collateralPrice, uint256 idealPrice) {
        ...
        uint256 secondsToExpiry = block.timestamp - expiryTimestamp;
        uint256 annualTime = FullMath.mulDiv(secondsToExpiry, 10 ** 18, 365 days);
