...
function initialize(
  ...
  cCOMP = CErc20(0x70e36f6BF80a52b3B46b3aF8e106CC0ed743E8e4);  
    ...
    function updateCompoundVault(address vaultAddress) public {
        ...
        uint256 currentSupply = CErc20(address(vault.collateral)).balanceOfUnderlying(address(vault.logic)); // total vault value in USD
        ...
        if (currentSupply == 0) {
         ...
         uint256 vaultAssetPerShare = currentSupply * vault.mentissa / IERC20Upgradeable(address(vault.logic)).totalSupply(); 
    ...
    AccessControlUpgradeable
{
    ...
    uint256 public accumulatedIvaultPerShare;
    ...
    function mint(
        ...
        _mint(msg.sender, getCurrentRate() * _amount );
        ...
        returns (uint256 price)
    {
        if(totalSupply() == 0){
          return  mentissa;
           ...
           return CErc20(collateral).balanceOfUnderlying(address(this)) / totalSupply() / mentissa;
        }
    ...
    function updateCompoundVault() internal {
  
        require(msg.sender == msg.sender,'iCOMP : Unauthorized');
        if (block.number <= lastClaimBlock) {
            return;
        }

        uint256 currentSupply = CErc20(collateral).balanceOfUnderlying(address(this)); // total vault value in USD
        ...
        if (currentSupply == 0) {
         ...
         uint256 vaultAssetPerShare = currentSupply * mentissa / totalSupply(); 
         //uint256 compClaimed = vault.logic.claimComp();
         //cCOMP.mint(compClaimed);
        ...
        //vault.accumulatedCompPerShare += compPerShare;
        accumulatedIvaultPerShare += vaultAssetPerShare; 
