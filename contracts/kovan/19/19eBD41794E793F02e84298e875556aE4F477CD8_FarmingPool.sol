...
contract FarmingPool is Pausable, ReentrancyGuard, IFarmingPool {
        ...
        nonReentrant
    {
        ...
        require(!paused(), "paused");
        uint256 farmerTotalTransferToAdapter =
            _totalTransferToAdapter[msg.sender];
        require(farmerTotalTransferToAdapter > 0, "no transfer");
        require(
            IBtoken(btokenAddress).balanceOf(msg.sender) >= requestedAmount,
            "insufficient BToken"
        );
        ...
        (uint256 actualAmount, uint256 receiveQuantity) =
            IAdapter(adapterAddress).redeemWrappedToken(requestedAmount);
        require(
        ...
        _totalTransferToAdapter[msg.sender] = farmerTotalTransferToAdapter.sub(
        ...
        if (
            interestEarning < repaymentDetails.payableInterest &&
            repaymentDetails.payableInterest.sub(interestEarning) <
            ROUNDING_TOLERANCE
        ) {
            previousNettInterestEarning = 0;
        } else {
...

        {
            ...
            if (
                ...
                totalUnderlyingAsset = totalUnderlyingAsset.sub(
                    repaymentDetails.depositPrincipal
                );
            }
        }

        uint256 actualAmountToReceive = repaymentDetails.amountToReceive;
        {
            // scope to avoid stack too deep errors
    ...
    function calculateRepaymentDetails(
        ...
        uint256 principal =
            (underlyingAssetQuantity >= underlyingAssetInvested)
                ? depositPrincipal
                : underlyingAssetQuantity.div(leverageFactor);
