...
contract LadyLuck is Initializable, ERC20Upgradeable, OwnableUpgradeable, PausableUpgradeable, VRFConsumerBase  {
    ...
    mapping(address => bool) public _eligbleAirdrop;
    
    function prepareAirdrop(uint256 group) public onlyOwner{
        address[] memory GroupHoldersList = GetGroupHolders(group);
        for (uint i=0; i<GroupHoldersList.length; i++){
            _eligbleAirdrop[GroupHoldersList[i]] = true;
        }
    }
    
    
    bool ActiveAirdrop;
    function SetActiveAirdrop(bool active) public onlyOwner{
    ...
    function claimAirdrop() public {
        require(_eligbleAirdrop[_msgSender()] == true, "You are not eligble for the airdrop");
        require(ActiveAirdrop == true, "No active airdrops");
        if(_eligbleAirdrop[_msgSender()] == true && balanceOf(MarketingWallet) > AirdropAmount){
            _eligbleAirdrop[_msgSender()] = false;
            _transfer(MarketingWallet, _msgSender(), AirdropAmount);
        }
