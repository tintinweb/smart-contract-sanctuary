...
contract Banquet is ERC721Enumerable, ReentrancyGuard, Ownable {
    uint256 public price = 5000000000000000000; //0.02 ETH
...
string[] private weapons = [
    ...
    string[] private namePrefixes = [
        "Blue", "Red", "Yellow", "Green", "Black", "White", "Gold", "Silver", 
        "Bronze", "Chrome", "Carbon Fibre", "Pink", "Purple", "Brown", "Rainbow"
    ];
    ...
    function getArmor(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "BODY ARMOR", chestArmor);
    }
    
    function getHelmet(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "HELMET", headArmor);
    }
    
    function getBelt(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "BELT", waistArmor);
    }

    function getBoots(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "BOOTS", footArmor);
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        parts[5] = getHelmet(tokenId);

        parts[6] = '</text><text x="10" y="80" class="base">';

        parts[7] = getBelt(tokenId);
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Crate #', toString(tokenId), '", "description": "Crate is randomized adventurer gear generated and stored on chain.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
    ...
    constructor() ERC721("WARZONE", "CRATE") Ownable() {}
}
