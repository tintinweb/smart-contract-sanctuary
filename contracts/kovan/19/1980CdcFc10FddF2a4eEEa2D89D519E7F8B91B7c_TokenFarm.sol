...
import { IERC20, IProtocolDataProvider, IStableDebtToken } from './Interfaces.sol';
import { SafeERC20 } from './Libraries.sol';

contract TokenFarm is ChainlinkClient, Ownable {
    using SafeERC20 for IERC20;

    string public name = "Social DeFi";
    ...
    address testholding;
    uint256 testHoldingUint;
    bool testHoldingBool;
    address sender;
    ...
    IERC20 aDai = IERC20(0x3B91257Fe5CA63b4114ac41A0d467D25E2F747F3);

    //from aave v2 examples
    ...
    constructor(address _dappTokenAddress) public {
        ...
        amount = 0;
        interestRateMode = 1;
        assetDai = address(0xFf795577d9AC8bD7D90Ee22b6C1703490b6512FD);
        referral = 0;
        user = address(0xc3690b5755bC5658f3f385Ec37131d2b7CB188c9);
        //owner = msg.sender;
    ...
    function setValuesTest(address _asset, uint256 _amount, bool _isPull) public {
        testHoldingUint = _amount;
        testholding = _asset;
        testHoldingBool = _isPull;
    }

    function getValuesTestUint() public view returns (uint256){
        return testHoldingUint;
    }

    function getValuesTestHolding() public view returns (address){
        return testholding;
    }
    ...
    function getValuesTestBool() public view returns (bool){
        return testHoldingBool;
    }


    function depositCollateral(address _asset, uint256 _amount, bool _isPull) public {
        /*if (_isPull) {
        ...
        //sender = msg.sender;
        IERC20(_asset).transferFrom(msg.sender, address(this), _amount); //testing with dapp token
        //chainshot beginning
    ...
    function depositDai(uint256 _amount, address _user) public {
        //function deposit(address asset, uint256 amount, address onBehalfOf, uint16 referralCode)
        //https://docs.aave.com/developers/the-core-protocol/lendingpool
        //lendingPool.deposit(_dappTokenAddress, amount, msg.sender, 0);
        //Retrieve LendingPool address
        //temp removal ILendingPoolAddressesProvider provider = ILendingPoolAddressesProvider(address(0x88757f2f99175387aB4C6a4b3067c77A695b0349)); // kovan address, for other addresses: https://docs.aave.com/developers/deployed-contracts/deployed-contract-instances 
        //ILendingPool lendingPool = ILendingPool(provider.getLendingPool());
        /*ILendingPool lendingPool = ILendingPool(0xE0fBa4Fc209b4948668006B2bE61711b7f465bAe);
        IERC20 dai = IERC20(0xFf795577d9AC8bD7D90Ee22b6C1703490b6512FD);
        IERC20 aDai = IERC20(0x3B91257Fe5CA63b4114ac41A0d467D25E2F747F3);*/

        //IERC20(assetDai).approve(lendingPool, _amount);
        //IERC20(assetDai).approve(provider.getLendingPool(), _amount);
        //lendingPool.deposit(assetDai, _amount, _user, referral);
        dai.approve(address(lendingPool), _amount);
		lendingPool.deposit(address(dai), _amount, address(this), 0);
        //lendingPool.deposit(assetDai, amount, msg.sender, referral);
        //lendingPool.swapBorrowRateMode(assetDai, interestRateMode);
    }
    ...
    function updateUniqueTokensStaked(address _user, address token) internal {
        if (stakingBalance[token][user] <= 0) {
            uniqueTokensStaked[user] = uniqueTokensStaked[user] + 1;
        }
    }

    function getUserStakingBalanceEthValue(address _user, address token)
        public
        view
        returns (uint256)
    {
    ...
    //friend management
    function addFriend(address _referrer, address _user) public {
        referralList.push([_referrer, _user]);
    }

    function getFriends() public view returns (uint count) {
