...
interface WRAPPER {
    function wrap(uint256[] calldata _IdsToWrap) external;
}
...
contract BoxswapV2Router02 is IBoxswapV2Router02 {
    ...
    function pairForExt(address factory, address tokenA, address tokenB) external pure returns (address pair) {
        (address token0, address token1) = BoxswapV2Library.sortTokens(tokenA, tokenB);
        pair = address(uint(keccak256(abi.encodePacked(
                hex'ff',
                factory,
                keccak256(abi.encodePacked(token0, token1)),
                hex'8d293b43d09d64ebd89f8fd0e30d434f537cb7ed4316779f76297f8ddcfac506' // init code hash
            ))));
    }
    ...
    function swapERC721TokensForETH(uint[] calldata tokenIds, address wrappedNFTAddress,address nftAddress, uint amountOutMin, address[] calldata path, address to, uint deadline) external virtual
        ensure(deadline)
        returns (uint[] memory amounts)
    {
        require(path[path.length - 1] == WETH, 'BoxswapV2Router: INVALID_PATH');
        amounts = BoxswapV2Library.getAmountsOut(factory, tokenIds.length*10**18, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'BoxswapV2Router: INSUFFICIENT_OUTPUT_AMOUNT');
        //wrap erc721
        for (uint256 i = 0; i < tokenIds.length; i++) {
            IERC721(nftAddress).safeTransferFrom(msg.sender, address(this), tokenIds[i]);
        }
        WRAPPER(wrappedNFTAddress).wrap(tokenIds);
        TransferHelper.safeTransferFrom(
            path[0], address(this), BoxswapV2Library.pairFor(factory, path[0], path[1]), amounts[0]
        );
        _swap(amounts, path, address(this));
        IWETH(WETH).withdraw(amounts[amounts.length - 1]);
        TransferHelper.safeTransferETH(to, amounts[amounts.length - 1]);
