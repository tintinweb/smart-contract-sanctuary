...
interface ILendingPool {
    ...
    function rollIn(address _borrower, uint256 _colDepositAmount) external;
}
...
// File contracts/utils/IOwnable.sol
...
// File contracts/utils/AggregatorV3Interface.sol
...
// File contracts/LendingPoolImplementation.sol
...
contract LendingPool is ILendingPool, Ownable {
    ...
    function initialize(
        ...
        allowUnderWater = _oracle == address(0);
    ...
    //Lend
    // Send the amount of tokens you wish to lend to this contract. 

    function borrow(uint256 _colDepositAmount) external {
        // uint256 lendAmount = _borrow(msg.sender, _colDepositAmount);
        _safeTransfer(lendToken, msg.sender, _borrow(msg.sender, _colDepositAmount));
    }

    function rollIn(address _borrower, uint256 _colDepositAmount) external {
        _borrow(_borrower, _colDepositAmount);
    }

    function _borrow(address _borrower, uint256 _colDepositAmount) internal returns (uint256){
        _validatePrice();
        ...
        uint256 rawPayoutAmount = computePayoutAmount(_colDepositAmount);
    ...
    function rollOver (address _newPool) external {
        ...
        if (newPool.mintRatio() <= mintRatio){
            uint256 diffToRepay = userReport.colAmount * (1e18 - (newPool.mintRatio()/mintRatio)); //Account for the difference in MR
            _pullTokensFrom(msg.sender, lendToken, diffToRepay + userReport.totalFees);
            //This probably needs to be address this and we need to approve the exact amount
            
            colToken.approve(_newPool, userReport.colAmount);
            newPool.rollIn(msg.sender, userReport.colAmount); //Send ALL collateral and record new debt
            userReport.colAmount = 0;
            userReport.borrowAmount = 0; //Clean users debdt in current pool
        }else{
            //Send the chunk of collateral back. 
            uint256 diffToReimburse = userReport.colAmount * ((newPool.mintRatio()/mintRatio)- 1e18);
            _pullTokensFrom(msg.sender, lendToken, userReport.totalFees); // Charge the fee
            _safeTransfer(colToken, msg.sender, diffToReimburse);
            newPool.rollIn(msg.sender, userReport.colAmount - diffToReimburse); // SHould come from debt map
        }
    }

    function _validateNewPool(ILendingPool pool) view private {
        require(address(pool.lendToken()) == address(lendToken), "Wrong lend token");
        require(address(pool.colToken()) == address(colToken), "Wrong col token");
        require(address(IOwnable(address(pool)).owner()) == address(owner()), "Different pool owner");
        require(pool.expiry() > expiry, "Pools does not last long enough");
    }
    ...
    function repay(uint256 _repayAmount) external {
        UserReport memory userReport = debt[msg.sender];
        require(block.timestamp <= expiry, "Pool closed");
        require(_repayAmount <= (userReport.borrowAmount + userReport.totalFees), "Acount debt is less");
        require(userReport.borrowAmount > 0, "No debt");
        uint256 repayRemainder = _repayAmount;

        console.log(userReport.totalFees);
    ...
    function collect() external onlyOwner {
        ...
        _safeTransfer(colToken, treasury, colToken.balanceOf(address(this)) * protocolFee / 10000 );

        // Send premium to lender
        _safeTransfer(lendToken, msg.sender, lendToken.balanceOf(address(this)));
        _safeTransfer(colToken, msg.sender, lendToken.balanceOf(address(this)));
        emit Collect();
    }

    function computePayoutAmount(uint256 colDepositAmount) public view returns (uint256) {
        uint8 colDecimals = colToken.decimals();
        uint8 lendDecimals = lendToken.decimals();
        return colDepositAmount*mintRatio * (10**lendDecimals) / (10**colDecimals) / 1e18;
    }

    function computeReturnAmount(uint256 repayAmount) public view returns (uint256) {
        uint8 colDecimals = colToken.decimals();
        uint8 lendDecimals = lendToken.decimals();
        return repayAmount * 1e18 * (10 ** colDecimals) / (10 ** lendDecimals) / mintRatio;
    }
    ...
    *******************/
    ...
    function _pullTokensFrom(address sender, IERC20 token, uint256 amount) private {
        uint256 initialBalance = token.balanceOf(address(this));
        token.safeTransferFrom(sender, address(this), amount);
        require(token.balanceOf(address(this)) == initialBalance + amount, "Transfer failed");
    }

    function _validatePrice() private view {
        if (allowUnderWater == false) {
            ( , int price, , , ) = priceFeed.latestRoundData();
            require(price >= int256(mintRatio), "LTV > 100%");        
        }
    }
...
contract PoolFactory {
    ...
    event DeployPool(uint256 _mintRatio,
        address _colToken,
        address _lendToken,
        address _oracle,
        uint256 _feeRate,
    ...
    function deployPool(
        ...
        emit DeployPool(_mintRatio, _colToken, _lendToken, _oracle, _feeRate, _protocolFee, _expiry, _treasury);
    }
}