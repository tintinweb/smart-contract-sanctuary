...
contract MyNFT is ERC721Enumerable, ERC721URIStorage, AccessControl {
    ...
    function mint(address to, string calldata _tokenURI) public onlyRole(MINTER_ROLE) {
        uint256 tokenId = totalSupply();
        _safeMint(to, tokenId);
        _setTokenURI(tokenId, _tokenURI);
    }

    function mintBatch(address[] calldata to, string[] calldata _tokenURI) external {
        require(to.length == _tokenURI.length, "Non-mathcing length");
        for (uint256 i; i < to.length; i++) {
            mint(to[i], _tokenURI[i]);
        }
    }
