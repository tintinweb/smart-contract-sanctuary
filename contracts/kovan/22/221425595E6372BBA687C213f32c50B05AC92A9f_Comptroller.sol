 ...
 * @author Publics
 */
contract PToken is PTokenInterface, Exponential, TokenErrorReporter, ICreditLoan {
    /**
        ...
        )
    {
        ...
        //返回的是account借币数量
        (mErr, borrowBalance) = borrowBalanceStoredInternal(account, loanType);
     ...
     */
    function borrowBalanceCurrent(address account, LoanType loanType) external nonReentrant returns (uint256) {
        //看到产生利息了：
        // borrowBalanceStored: 99082789,这是借的数量
        // borrowBalanceCurrent : 99095615,这是当前要还的总额
        // accrueInterest()
        require(accrueInterest() == uint256(Error.NO_ERROR), "accrue interest failed");
        return borrowBalanceStored(account, loanType);
    }
     ...
     */
    function borrowBalanceStored(address account, LoanType loanType) public view returns (uint256) {
        (MathError err, uint256 result) = borrowBalanceStoredInternal(account, loanType);
     ...
     */
    function borrowBalanceStoredInternal(address account, LoanType loanType) internal view returns (MathError, uint256) {
        ...
        // BorrowSnapshot storage borrowSnapshot = (_loanType == LoanType.NORMAL) ? accountBorrows[account] 
        // : (_loanType == LoanType.MARGIN_SWAP_PROTOCOL ? accountBorrowsMarginSP[account]:accountBorrowsMiningSP[account]);

        BorrowSnapshot storage borrowSnapshot = accountBorrows[account];
        // console.log("NORMAL!");
        
        if (LoanType.MARGIN_SWAP_PROTOCOL == loanType) {
            // console.log("MARGIN_SWAP_PROTOCOL!");
            borrowSnapshot = accountBorrowsMarginSP[account];
        } else if (LoanType.MINNING_SWAP_PROTOCOL == loanType) {
            // console.log("MINNING_SWAP_PROTOCOL!");
            borrowSnapshot = accountBorrowsMiningSP[account];
        }
    ...
    function accrueInterest() public returns (uint256) {
        ...
        //A = 利率*块数, 这几个块的累计利率，为什么用当前的借款利率来计算过去的块应该产生的总利率？
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function redeemInternal(uint256 redeemTokens) internal nonReentrant returns (uint256, uint256) {
        uint256 error = accrueInterest();
        if (error != uint256(Error.NO_ERROR)) {
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function redeemUnderlyingInternal(uint256 redeemAmount) internal nonReentrant returns (uint256, uint256) {
        uint256 error = accrueInterest();
        if (error != uint256(Error.NO_ERROR)) {
            // accrueInterest emits logs on errors, but we still want to log the fact that an attempted redeem failed
            return (fail(Error(error), FailureInfo.REDEEM_ACCRUE_INTEREST_FAILED), 0);
    ...
    function redeemFresh(
        ...
        (vars.mathErr, vars.exchangeRateMantissa) = exchangeRateStoredInternal();
        if (vars.mathErr != MathError.NO_ERROR) {
            return (failOpaque(Error.MATH_ERROR, FailureInfo.REDEEM_EXCHANGE_RATE_READ_FAILED, uint256(vars.mathErr)), 0);
        }
        ...
        if (redeemTokensIn > 0) {
            ...
            if (vars.mathErr != MathError.NO_ERROR) {
                return (failOpaque(Error.MATH_ERROR, FailureInfo.REDEEM_EXCHANGE_TOKENS_CALCULATION_FAILED, uint256(vars.mathErr)), 0);
            }
        } else {
            /*
            ...
            if (vars.mathErr != MathError.NO_ERROR) {
                return (failOpaque(Error.MATH_ERROR, FailureInfo.REDEEM_EXCHANGE_AMOUNT_CALCULATION_FAILED, uint256(vars.mathErr)), 0);
            }
        ...
        if (vars.mathErr != MathError.NO_ERROR) {
            return (failOpaque(Error.MATH_ERROR, FailureInfo.REDEEM_NEW_TOTAL_SUPPLY_CALCULATION_FAILED, uint256(vars.mathErr)), 0);
        }

        (vars.mathErr, vars.accountTokensNew) = subUInt(accountTokens[redeemer], vars.redeemTokens);
        if (vars.mathErr != MathError.NO_ERROR) {
            return (failOpaque(Error.MATH_ERROR, FailureInfo.REDEEM_NEW_ACCOUNT_BALANCE_CALCULATION_FAILED, uint256(vars.mathErr)), 0);
        }
        ...
        return (uint256(Error.NO_ERROR), vars.redeemAmount);
    }
    ...
    function borrowInternal(uint256 borrowAmount) internal nonReentrant returns (uint256) {
        ...
        return borrowFresh(msg.sender, borrowAmount, LoanType.NORMAL);
    }
    ...
    function borrowFresh(
        ...
        uint256 allowed = comptroller.borrowAllowed(address(this), borrower, borrowAmount, _loanType);
        if (allowed != 0) {
            //3, 14, 4
        ...
        (vars.mathErr, vars.accountBorrows) = borrowBalanceStoredInternal(borrower, _loanType);
        if (vars.mathErr != MathError.NO_ERROR) {
        ...
        //PErc20
        //CEth分别实现自己的方法
        if (LoanType.MARGIN_SWAP_PROTOCOL == _loanType) {
            //信用贷，转账到白名单合约，而不是具体账户，在合约中记录与账户的关系
            require(whiteList[msg.sender], "caller shoud be in white list");
            doTransferOut(msg.sender, borrowAmount);
            /* We write the previously calculated values into storage */
            accountBorrowsMarginSP[borrower].principal = vars.accountBorrowsNew; //当前这个borrower所借的全部数量
            accountBorrowsMarginSP[borrower].interestIndex = borrowIndex; //当前的索引值

        } else if (LoanType.MINNING_SWAP_PROTOCOL == _loanType){
            require(whiteList[msg.sender], "caller shoud be in white list");
            doTransferOut(msg.sender, borrowAmount);
            /* We write the previously calculated values into storage */
            accountBorrowsMiningSP[borrower].principal = vars.accountBorrowsNew; //当前这个borrower所借的全部数量
            accountBorrowsMiningSP[borrower].interestIndex = borrowIndex; //当前的索引值

        } else if (LoanType.NORMAL == _loanType){
        ...
        emit Borrow(borrower, borrowAmount, vars.accountBorrowsNew, vars.totalBorrowsNew, _loanType);
    ...
    function repayBorrowInternal(uint256 repayAmount) internal nonReentrant returns (uint256, uint256) {
        ...
        return repayBorrowFresh(msg.sender, msg.sender, repayAmount, LoanType.NORMAL);
    }
    ...
    function repayBorrowBehalfInternal(address borrower, uint256 repayAmount) internal nonReentrant returns (uint256, uint256) {
        ...
        // repayBorrowFresh emits repay-borrow-specific logs on errors, so we don't need to
        return repayBorrowFresh(msg.sender, borrower, repayAmount, LoanType.NORMAL);
    }
    ...
    function repayBorrowFresh(
        ...
        //console.log("repayBorrowFresh::start");
        ...
        if (LoanType.MARGIN_SWAP_PROTOCOL == loanType) {
            vars.borrowerIndex = accountBorrowsMarginSP[borrower].interestIndex;
        } else if (LoanType.MINNING_SWAP_PROTOCOL == loanType){
            vars.borrowerIndex = accountBorrowsMiningSP[borrower].interestIndex;
        } else if (LoanType.NORMAL == loanType){
            vars.borrowerIndex = accountBorrows[borrower].interestIndex;
        }

        /* We fetch the amount the borrower owes, with accumulated interest */
        (vars.mathErr, vars.accountBorrows) = borrowBalanceStoredInternal(borrower, loanType); //TODO 还款
        if (vars.mathErr != MathError.NO_ERROR) {
        ...
        if (repayAmount == uint256(-1)) {
            ...
            if (repayAmount > vars.accountBorrows) {
                ...
                console.log("更新为最大欠款额!");
            }

            vars.repayAmount = repayAmount;
        }

        //console.log("repayBorrowFresh::repayAmount:", repayAmount);
        ...
        if (LoanType.MARGIN_SWAP_PROTOCOL == loanType) {
            accountBorrowsMarginSP[borrower].principal = vars.accountBorrowsNew;
            accountBorrowsMarginSP[borrower].interestIndex = borrowIndex;

        } else if (LoanType.MINNING_SWAP_PROTOCOL == loanType){
            accountBorrowsMiningSP[borrower].principal = vars.accountBorrowsNew;
            accountBorrowsMiningSP[borrower].interestIndex = borrowIndex;
    ...
    function liquidateBorrowFresh(
        ...
        (uint256 repayBorrowError, uint256 actualRepayAmount) = repayBorrowFresh(liquidator, borrower, repayAmount, LoanType.NORMAL);
    ...
    function doCreditLoanBorrowInternal(
        ...
        require(whiteList[msg.sender], "doCreditLoanBorrowInternal address not in whitelist");
        ...
        error = borrowFresh(_borrower, _borrowAmount, _loanType);
        if (error != uint256(Error.NO_ERROR)) {
            return fail(Error(error), FailureInfo.BORROW_FRESHNESS_CHECK);
        }

        emit NewCreditLoanBorrowEvent(msg.sender, _loanType, _borrower, _borrowAmount, error);
        return uint256(Error.NO_ERROR);
    }
    ...
    function doCreditLoanRepayInternal(
        ...
        //console.log('msg.sender:', msg.sender);
        require(whiteList[msg.sender], "doCreditLoanRepayInternal address not in whitelist");
        ...
        (error,actualAmount) = repayBorrowFresh(msg.sender, _payer, _repayAmount, _loanType);
        if (error != uint256(Error.NO_ERROR)) {
            return (fail(Error(error), FailureInfo.REPAY_BORROW_FRESHNESS_CHECK), 0);
        }

        emit NewCreditLoanRepayEvent(address(this), _loanType, msg.sender, _repayAmount, actualAmount, error);
...
contract Comptroller is ComptrollerV5Storage, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError{
    ...
    function exitMarket(address pTokenAddress) external returns (uint256) {
        ...
        (uint256 oErr, uint256 tokensHeld, uint256 amountOwed, ) = pToken.getAccountSnapshot(msg.sender, LoanType.NORMAL);
    ...
    function liquidateBorrowAllowed(
        ...
        uint256 borrowBalance = PToken(pTokenBorrowed).borrowBalanceStored(borrower, LoanType.NORMAL);
        ...
        )
    {
        ...
        for (uint256 i = 0; i < assets.length; i++) {
            ...
            (oErr, vars.pTokenBalance, vars.borrowBalance, vars.exchangeRateMantissa) = asset.getAccountSnapshot(account, LoanType.NORMAL);
    ...
    function distributeBorrowerComp(
        ...
        if (borrowerIndex.mantissa > 0) {
            ...
            uint256 borrowerAmount = div_(PToken(pToken).borrowBalanceStored(borrower, LoanType.NORMAL), marketBorrowIndex);
    ...
    function getUnclaimedPub(address holder) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < allMarkets.length; i++) {
            ...
            uint256 borrowerAmount = div_(market.borrowBalanceStored(holder, LoanType.NORMAL), marketBorrowIndex);
            if (borrowerAmount > 0) {
