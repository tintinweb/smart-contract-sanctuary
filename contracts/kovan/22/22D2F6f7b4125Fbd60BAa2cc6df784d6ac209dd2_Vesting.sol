...
contract Vesting is PausableUpgradeable, BumperAccessControl {
    ...
    function claimToStaking(
        ...
        whenNotPaused
    { 
        uint256 _claimedAmount = _claim(account, vestingInfo, merkleProof);
        IERC20(tokenBUMP).approve(staking, _claimedAmount);
        StakeRewards(staking).stakeFor(_claimedAmount, stakingOption, account);
    }
    ...
    {
        uint256 _claimedAmount = _claim(account, vestingInfo, merkleProof);
        IERC20(tokenBUMP).safeTransfer(account, _claimedAmount);
    }
        ...
        returns (uint256)
    {
        ...
        uint256 amount = vestingInfo.previousAmount;

        if(block.timestamp > vestingInfo.start) { 
            amount = vestingInfo.totalAmount + amount -
        ...
        recipientClaimed[account] = totalClaimed + amount;
