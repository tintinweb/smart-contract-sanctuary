...
contract AccuweatherConsumer3 is ChainlinkClient {
    ...
    struct LocationCurrentConditions {
        uint256 locationKey;
        uint256 timestamp;
        ...
        int256 temperature;
        uint256 windSpeed;
        uint256 relativeHumidity;
        uint256 uvIndex;
        uint256 weatherIcon;
    }

    // Maps <RequestId, Result>
    mapping(bytes32 => LocationCurrentConditions) public requestIdLocationCurrentConditions;
    mapping(bytes32 => Coord) public requestIdCoord;
     ...
     * @param _units the measurement system
     */
    function requestGasPrice(
        ...
        coord.lat = _lat;
        coord.lon = _lon;
        requestIdCoord[requestId] = coord;
    }
    ...
    /**
     * Lacks:
     * - lat/lon -> how to relate it with the request on an easy way
     * - Location name, countryCode
     * - Other current conditions data
     * - units
     */
    function fulfillLocationCurrentConditions(
        ...
        uint256 locationKey,
        uint256 timestamp,
        string memory precipitationType,
        ...
        LocationCurrentConditions memory locationCurrentConditions;

        if (locationKey > 0) {
            locationCurrentConditions.locationKey = locationKey;
            locationCurrentConditions.timestamp = timestamp;
            locationCurrentConditions.precipitationType = precipitationType;
            locationCurrentConditions.windDirection = windDirection;
            locationCurrentConditions.temperature = temperature;
            locationCurrentConditions.windSpeed = windSpeed;
            locationCurrentConditions.relativeHumidity = relativeHumidity;
            locationCurrentConditions.uvIndex = uvIndex;
            locationCurrentConditions.weatherIcon = weatherIcon;
        }
        requestIdLocationCurrentConditions[_requestId] = locationCurrentConditions;
    }
