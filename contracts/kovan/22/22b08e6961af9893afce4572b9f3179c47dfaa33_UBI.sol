...
contract UBI is Initializable, ERC20BurnableUpgradeable, ERC20SnapshotUpgradeable {
  ...
  */
  function initialize(uint256 _initialSupply, uint256 _accruedPerSecond, IProofOfHumanity _proofOfHumanity) public initializer {
    __Context_init_unchained();
    __ERC20_init_unchained("Universal Basic Income", "EARTH");
  ...
  */
  function startAccruing(address _human) external isRegistered(_human, true) {
    require(lastMintedSecond[_human] == 0, "The submission is already accruing UBI.");
    lastMintedSecond[_human] = block.timestamp;
  }
  ...
  */
  function reportRemoval(address _human) external isRegistered(_human, false) {
    require(lastMintedSecond[_human] != 0, "The submission is not accruing UBI.");
    uint256 newSupply = getAccruedValue(_human);

    lastMintedSecond[_human] = 0;
    withdrawn[_human] = 0;
  ...
  */
  function getAccruedValue(address _human) public view returns (uint256 accrued) {
    uint256 totalAccrued = accruedPerSecond.mul(block.timestamp.sub(lastMintedSecond[_human]));

    // If this human does not have started to accrue, or current available balance to withdraw is negative, return 0.
    if (lastMintedSecond[_human] == 0 || withdrawn[_human] >= totalAccrued || proofOfHumanity.isRegistered(_human) == false) return 0;
