...
contract Orderbook{
    ...
    function depositOrder (OwnerOrderInfo memory ownerOrder, uint256 amountA) external {
        ...
        require (amountA != 0, "ORDERBOOK: AMOUNT_IS_NOT_AVAILABLE");
    ...
    function _searchOrdersForMatching(address _user, uint _index) internal view returns (
    ...
    function _matchingMarketOrder(address user, uint index) internal {
        ...
        (uint[] memory ordersSuitable, uint price, uint amountARemaining, uint amountBRemaining) = _searchOrdersForMatching(user, index);
        if(price == 0)
            return;
        uint amountAReceive;
