...
contract StAave is Initializable, OwnableUpgradeable, LSTokenGovPowerSnapshot, IStAave {
    ...
    function stake(uint256 amount) external override {
        ...
        _mintShares(msg.sender, shares, amount);
    }

    /**
     * @dev Redeem AAVE from staking contract, burn stAAVE, refund AAVE to user.
     * Note 1. This function takes shares (not amount!) as argument.
     * Note 2. In some rare situations, e.g. when one user owns the vast majority of the
     * TVL, his stAAVE balance may be greater than the staked amount (as stAAVE includes
     * unclaimed rewards). In this case, we simply return all the staked amount and ignore
     * the unclaimed reward part.
     */
    function redeem(uint256 shares) public override {
        ...
        uint256 stakedAmount = _getStaked();

        stkAave.redeem(msg.sender, amount > stakedAmount ? stakedAmount : amount);

        _burnShares(msg.sender, shares, amount);
    }
...
abstract contract LSToken is Initializable, ILSToken {
    ...
    function _burnShares(
        ...
        _shares[from] = _shares[from].sub(shares, "Burn amount exceeds shares!");
