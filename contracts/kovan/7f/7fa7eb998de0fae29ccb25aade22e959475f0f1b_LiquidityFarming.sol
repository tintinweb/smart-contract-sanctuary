...
import "ABDKMathQuad.sol";
 ...
 contract LiquidityFarming is Ownable {
  ...
  mapping(address => mapping (address => uint256)) internal timeLockingPeriod;
  ...
  mapping(uint256 => uint256) internal timeBonus;
  ...
  address LGE;
    ...
    constructor() {
        ...
        LGE = address(0xbB2b160792F28943febeEe62D56Ac14f00C7e5De);
        rewardTax = 8;
  ...
  {//uint256 reward = rewardOf(_token, _stakeholder);
    ...
    if(_permanent){
      ...
      userScore[_token][_stakeholder] = userScore[_token][_stakeholder].add(_amount.add(_amount.mul(timeBonus[timeLockingPeriod[_token][_stakeholder]]).div(100)));
      totalScore[_token] = totalScore[_token].add(_amount.add(_amount.mul(timeBonus[timeLockingPeriod[_token][_stakeholder]]).div(100)));
