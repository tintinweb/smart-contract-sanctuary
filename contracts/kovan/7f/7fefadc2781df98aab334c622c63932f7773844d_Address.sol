...
contract BoringBananasCo is ERC721, Ownable {
  ...
  uint256 public constant maxBananaPurchase = 20;

  uint256 public constant MAX_BANANAS = 10000;

  bool public saleIsActive = false;

  mapping(uint256 => string) public bananaNames;

  // Reserve 125 Bananas for team - Giveaways/Prizes etc
  uint256 public bananaReserve = 125;

  event bananaNameChange(address _by, uint256 _tokenId, string _name);
  ...
  function reserveBananas(address _to, uint256 _reserveAmount) public onlyOwner {
    ...
    require(
      _reserveAmount > 0 && _reserveAmount <= bananaReserve,
  ...
  function mintBoringBanana(uint256 numberOfTokens) public payable {
    require(saleIsActive, 'Sale must be active to mint Banana');
    require(
      numberOfTokens > 0 && numberOfTokens <= maxBananaPurchase,
      'Can only mint 20 tokens at a time'
    );
    require(
      totalSupply().add(numberOfTokens) <= MAX_BANANAS,
      'Purchase would exceed max supply of Bananas'
    );
  ...
  function changeBananaName(uint256 _tokenId, string memory _name) public {
    require(ownerOf(_tokenId) == msg.sender, "Hey, your wallet doesn't own this banana!");
    require(
      sha256(bytes(_name)) != sha256(bytes(bananaNames[_tokenId])),
  ...
  function viewBananaName(uint256 _tokenId) public view returns (string memory) {
