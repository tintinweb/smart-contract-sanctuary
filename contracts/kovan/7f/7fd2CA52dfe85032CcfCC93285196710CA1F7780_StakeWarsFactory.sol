...
contract StakeWarsFactory is ERC721Enumerable, Administerable, VRFConsumerBase {
    ...
    uint256 private _tokenIdsCharacterized;
    ...
    // mapping(uint256 => User) public _requestIdToUser;
    ...
    function mintPresale(uint256 numberOfMints) public payable {
        ...
        uint256 reserved = presaleWhitelist[sender];
        ...
        presaleWhitelist[sender] = reserved - numberOfMints;
        for (uint256 i; i < numberOfMints; i++) {
            _tokenIdsGen++;
            StakeWarsInternals warrior = new StakeWarsInternals(securityKey);
            tokenIdToStakeWarrior[_tokenIdsGen] = warrior;
            _safeMint(sender, _tokenIdsGen);
        }
    }
    ...
    function setCharacterisitics() public onlyAdmin {
        for (
            uint256 i = _tokenIdsCharacterized;
            i < _tokenIdsGen;
            _tokenIdsCharacterized++
        ) {
            address sender = _msgSender();
            bytes32 requestId = requestRandomness(keyhash, linkFee);
            _requestIdToUser[requestId] = User(sender, _tokenIdsGen);
        }
    }
    ...
    function Donate() public payable {
        ...
        patronsList[sender] += msg.value;
        patronArray.push(sender);
