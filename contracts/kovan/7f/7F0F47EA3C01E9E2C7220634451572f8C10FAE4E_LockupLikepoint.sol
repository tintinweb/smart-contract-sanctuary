...
contract LockupLikepoint is Ownable {
    ...
    function setInit() onlyOwner public {
      ...
      optionPrice[0] = 50000000000000000000000;
      optionPrice[1] = 40000000000000000000000;
      optionPrice[2] = 30000000000000000000000;
      optionPrice[3] = 20000000000000000000000;
    }
    ...
    function getAmountLottery(address user) public view returns (uint256) {
      uint256 numTicket = 0;
      for(uint i = 0; i<maxOption; i++) {
        timeLock memory userPoint = timelockhold[user][i];
        if(userPoint.amount >= optionPrice[i]){
          uint256 ticket = userPoint.amount/optionPrice[i];
          numTicket = numTicket.add(ticket);
        }
      }
      return numTicket;
