...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public dividendToken = 0x1f9840a85d5aF5bf1D1762F925BDADdC4201F984;
...
contract BabyUni is ERC20, Ownable {
    ...
    uint256 public constant ETH_REWARDS_FEE = 6;
    uint256 public constant LIQUIDITY_FEE = 3;
    uint256 public constant TREASURY_FEE = 3;
    uint256 public constant TOTAL_FEES = ETH_REWARDS_FEE + LIQUIDITY_FEE + TREASURY_FEE;
    ...
    address private _dividendToken = 0x1f9840a85d5aF5bf1D1762F925BDADdC4201F984;
    ...
    function activate() public onlyOwner {
        require(!tradingEnabled, "BabyUni: Trading is already enabled");
        _swapEnabled = true;
        tradingEnabled = true;
    }
    ...
    constructor() ERC20("BabyUni", "BUNI") {
    ...
    receive() external payable {
      ...
      function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BabyUni: The Uniswap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function excludeFromFees(address account) public onlyOwner {
        require(!_isExcludedFromFees[account], "BabyUni: Account is already excluded from fees");
    ...
    function _transfer(
        ...
        if (!tradingIsEnabled) {
            ...
            if(!openForPresale){
                require(canTransferBeforeTradingIsEnabled[from], "BabyUni: This account cannot send tokens until trading is enabled");
            }
        }
        ...
        if (tradingIsEnabled &&
            ...
            uint256 tokensForLp = contractTokenBalance.mul(LIQUIDITY_FEE).div(TOTAL_FEES);
            swapAndLiquify(tokensForLp);
                
            uint256 swapTokens = contractTokenBalance.mul(TREASURY_FEE).div(TOTAL_FEES);
...
contract BabyUniDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("BabyUni_Dividend_Tracker", "BabyUni_Dividend_Tracker") {
    	claimWait = 3600;
    ...
    function excludeFromDividends(address account) external onlyOwner {
      ...
      function updateGasForTransfer(uint256 newGasForTransfer) external onlyOwner {
        require(newGasForTransfer != gasForTransfer, "BabyUni_Dividend_Tracker: Cannot update gasForTransfer to same value");
