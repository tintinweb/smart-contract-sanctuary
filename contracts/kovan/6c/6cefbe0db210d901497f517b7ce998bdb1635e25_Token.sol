...
contract Token {
    ...
    // Keep track balances and allowances approved, returns the token balance of the address passed in
    mapping(address => uint256) public balanceOf;
    mapping(address => mapping(address => uint256)) public allowance;

    // Events - fire events on state changes etc.  for Logging, refer to the emit lines below
    event Transfer(address indexed from, address indexed to, uint256 value);
    event Approval(address indexed owner, address indexed spender, uint256 value);
    
    //this contructs the token based on the parameters passed during run
    ...
    //parent transfer function, does check and then calls the sub transfers based
    ...
    //basic function for normal p2p transfer
    ...
    //transfer for remote transfer, like an exchange transferrring for you. so from an external request
