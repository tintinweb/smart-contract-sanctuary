...
contract Swiss {
    ...
    function getStandings(uint32 position, uint32 tournamentId) external view tournamentExists(tournamentId)
        returns(uint32) {
        tournamentData storage currentTournament = tournaments[tournamentId];
        return currentTournament.standings[position];
    }

    function getParticipantNames(uint32 participantNr, uint32 tournamentId) external view tournamentExists(tournamentId)
        returns(string memory) {
        tournamentData storage currentTournament = tournaments[tournamentId];
        return currentTournament.participantList[participantNr].username;
    }

    function getParticipantAddresses(uint32 participantNr, uint32 tournamentId) external view tournamentExists(tournamentId)
        returns(address) {

        tournamentData storage currentTournament = tournaments[tournamentId];
        return currentTournament.participantList[participantNr].userAddress;
    }

    function getPairing(uint32 participantNr, uint32 round, uint32 tournamentId) external view tournamentExists(tournamentId)
        returns(uint32) {

        tournamentData storage currentTournament = tournaments[tournamentId];
        return currentTournament.pairingsHistory[round][participantNr];
    }
