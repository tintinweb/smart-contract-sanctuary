...
contract LessLibrary is Ownable {
    ...
    uint256 private votingTime = 10 minutes; //three days
    uint256 private registrationTime = 10 minutes; // one day
    uint256 private lpDaySeconds = 60; // one day
    ...
    function getLpDaySeconds() external view returns(uint256){
        return lpDaySeconds;
    }

    function getTiersTimes(uint256 _index) external view returns(uint256){
        return tiersTimes[_index];
    }
    ...
    function createPresale(
        ...
        require(
            _info.openTime > block.timestamp &&
                ...
                _info.openTime &&
                _info.closeTime - _info.openTime > safeLibrary.getTiersTimes(0) &&
...
contract PresalePublic is ReentrancyGuard {
    ...
    modifier votesPassed(uint256 totalStakedAmount) {
        ...
        require(block.timestamp >= generalInfo.closeTimeVoting, "Voting is open");
        _;
    }
        ...
        notCreator
    {
        ...
        uint256 tokensLeft;
        uint256 nowTime = block.timestamp;
        if (nowTime < generalInfo.openTimePresale + lessLib.getTiersTimes(3)) {
            require(
                _stakedAmount >= stakingTiers[0], 
                "Not enough staked for TIER 5"
            );
            tokensLeft =
                (intermediate.beginingAmount * poolPercentages[0]) /
                100;
        } else if (nowTime < generalInfo.openTimePresale + lessLib.getTiersTimes(2)) {
            require(
                    _stakedAmount >= stakingTiers[1],
                "Not enough staked for TIER 4"
            );
            tokensLeft =
                (intermediate.beginingAmount - 
                    generalInfo.tokensForSaleLeft) +
                ((intermediate.beginingAmount * poolPercentages[1]) / 100);
        } else if (nowTime < generalInfo.openTimePresale + lessLib.getTiersTimes(1)) {
            require(
                    _stakedAmount >= stakingTiers[2],
                "Not enough staked for TIER 3"
            );
            tokensLeft =
                (intermediate.beginingAmount - 
                    generalInfo.tokensForSaleLeft) +
                ((intermediate.beginingAmount * poolPercentages[2]) / 100);
        } else if (nowTime < generalInfo.openTimePresale + lessLib.getTiersTimes(0)) {
            require(
                    _stakedAmount >= stakingTiers[3],
                "Not enough staked for TIER 2"
            );
            tokensLeft =
                (intermediate.beginingAmount - 
                    generalInfo.tokensForSaleLeft) +
    ...
    function addLiquidity() external presaleIsNotCancelled nonReentrant {
        ...
        uniswapInfo.unlockTime =
            block.timestamp +
            (uniswapInfo.lpTokensLockDurationInDays * lessLib.getLpDaySeconds());
    }
...

{
  ...
  "libraries": {
    "/wsl-files/work/git/eth/less-contracts-for-tests/contracts/libraries/Calculations.sol": {
      "Calculations": "0x77375913a230CbC8F7369F6D80Bf264D29c1E3d3"
    }
  },
  "outputSelection": {
    "*": {
