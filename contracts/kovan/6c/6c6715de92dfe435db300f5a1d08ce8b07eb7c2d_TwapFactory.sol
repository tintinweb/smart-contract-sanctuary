...
contract TwapPair is Reserves, TwapLPToken, ITwapPair {
    ...
    function swap(
        ...
        require(amount0In > 0 || amount1In > 0, 'TP_INSUFFICIENT_INPUT_AMOUNT');
        ...
        if (amount0Out > 0) {
            ...
            uint256 balance0After = ITwapOracle(oracle).tradeY(balance1.sub(fee1), reference0, reference1);
            ...
            uint256 balance1After = ITwapOracle(oracle).tradeX(balance0.sub(fee0), reference0, reference1);
    ...
    function getSpotPrice() public view override returns (uint256 spotPrice) {
        ...
        return ITwapOracle(oracle).getSpotPrice(reserve0, reference0);
    }
    ...
    function getSwapAmount0In(uint256 amount1Out) external view override returns (uint256 swapAmount0In) {
        ...
        uint256 balance0After = ITwapOracle(oracle).tradeY(balance1After, reference0, reference1);
    ...
    function getSwapAmount1In(uint256 amount0Out) external view override returns (uint256 swapAmount1In) {
        ...
        uint256 balance1After = ITwapOracle(oracle).tradeX(balance0After, reference0, reference1);
