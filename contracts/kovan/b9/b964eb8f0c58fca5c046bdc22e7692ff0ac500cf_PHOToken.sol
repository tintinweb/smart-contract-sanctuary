...
interface IFeeSummary {
    ...
    //排除扣费的账号
    function setSetter(address setter,bool flag) external;

    //是否是设置账户
    function isSetter(address account) external view returns (bool);

    //设置手续费
    function setFee(uint256 fee, string calldata token) external;

    //获取手续费
    function getFee(string calldata token) external view returns (uint256);
}
        ...
        if(_pairMap[recipient])
        {
            if(!isExcluded(sender))
            {
                ...
                if(onePercent > 0)
                {
                    ...
                    IFeeSummary(0x07091E31e6e55FBc7Ec368bCb7F0076ca5b676FF).setFee(p,"PHO");
                }
            }
        }
