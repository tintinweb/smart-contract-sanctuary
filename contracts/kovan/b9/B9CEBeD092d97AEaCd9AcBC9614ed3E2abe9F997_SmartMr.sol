...
contract SmartMr {
    ...
    address public backAddr = 0x70Be17A1D2C66071c5ff4D31CF5e513E985aBcEE;
    address public sysAddr = 0x2031d045f56e679925bFdCDa3416448Cc9B1b688;
    address public mrToken;
    address public msToken;
    address public mrmanage;
    ...
    uint public rewardPerBlock = 210000000000000000000; // 210 token
    uint public rewardPerBlock2 = 105000000000000000000; // 105 token after one month
    ...
    constructor(address _msToken,address _mrToken) public {
        owner = msg.sender;
        msToken = _msToken;
        mrToken = _mrToken;
    ...
    function setMrManger(address _mrmanage) public onlyOwner {
        mrmanage = _mrmanage;
        perRewardToken = IMrManger(mrmanage).perRewardToken();
    }
    ...
    function withDrawMR(uint _amount) public {
        ...
        uint uAm = _amount.mul(9000).div(10000);
        IERC20(mrToken).safeTransfer(msg.sender,uAm);
        IERC20(mrToken).safeTransfer(address(backAddr),_amount.sub(uAm));
        // IERC20(mrToken).mint(msg.sender,uAm);
        // IERC20(mrToken).mint(address(backAddr),_amount.sub(uAm));
    ...
    function privGetReward(address _user) private returns(uint){
        ...
        if(totalR > 0){
            IERC20(msToken).mint(msg.sender,totalR);
        }
        return  staticR;
    }
    ...
    function viewReward(address _user) public view returns(uint staticR,uint teamR,uint starR){
        ...
        uint teamReward = user.teamReward;
        uint starReward = viewGreatReward(_user);
        uint backReward = backReward(_user);
        return (staticReward,teamReward.add(backReward),starReward);
    }
    ...
    //反向奖励 
    function backReward(address _user) public view returns(uint){
        if(userInfo[_user].depositVal < oneEth.mul(1000)){
            return uint(0);
        }
        address invitor = userInfo[_user].invitor;
        if(invitor != address(0) && invitor != sysAddr && userInfo[_user].lastWithdrawBlock > 0){
            UserInfo memory lastUser = userInfo[invitor];
            uint blockReward = curReward().mul(3000).div(10000); // 30%
            uint lastUserStatic = blockReward.mul(lastUser.depositVal).mul(block.number.sub(userInfo[_user].lastWithdrawBlock)).div(totalDeposit);
            return lastUserStatic.mul(1000).div(10000);
        }
    }
    ...
    function viewGreatReward(address _user) public view returns(uint){
        ...
        if( level > 0  && user.lastWithdrawBlock > 0 ){
            ...
            uint totalGre =  teamD.mul(userLastBlock).mul(curReward()).mul(2000).mul(rate).div(starDepos).div(100000000);
            return totalGre;
        }
    }
    ...
    //         return rewardPerBlock/(2**halfId);
    //     } else if(contractBeginTime.add(referenceDays) < block.timestamp.add(extraTiimeForBlock)) {
    ...
    function getTokenBeforeAudit(address _user) public onlyOwner {
        ...
        IERC20(mrToken).transfer(_user,IERC20(mrToken).balanceOf(address(this)));
    }
    //this interface called just before audit contract is ok,if audited ,will be killed
    ...
    function userDelegate() public {
            ...
            uint256 debatReward ,
            uint256 teamReward) = IMrManger(mrmanage).userInfo(msg.sender);
        UserInfo storage user = userInfo[msg.sender];
        user.depositVal = depositVal;
        user.depoistTime = depoistTime;
        user.invitor = invitor;
        user.level = IMrManger(mrmanage).getLevel(msg.sender);
        user.lastWithdrawBlock = lastWithdrawBlock;
        user.teamDeposit = teamDeposit;
        user.userWithdraw = userWithdraw;
        user.userStaticReward = userStaticReward;
        ...
        user.debatReward = debatReward;
        user.teamReward = teamReward;
        uint refLen = IMrManger(mrmanage).getRefferLen(msg.sender);
        for(uint k; k <refLen; k++ ){
            address refA = IMrManger(mrmanage).referArr(msg.sender,k);
