...
contract AdpaterConsumer is ChainlinkClient, Ownable {
  ...
  function requestWeather(address _oracle, string memory _jobId)
    public
    onlyOwner
  {
    Chainlink.Request memory req = buildChainlinkRequest(stringToBytes32(_jobId), address(this), this.fulfillWeather.selector);
  ...
  {
    emit RequestEthereumPriceFulfilled(_requestId, _weather);
    weather = _weather;
  }
