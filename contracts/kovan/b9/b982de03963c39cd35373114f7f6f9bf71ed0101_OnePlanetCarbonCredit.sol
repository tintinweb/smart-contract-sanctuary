...
abstract contract OnePlanetInterface {
     ...
     function gasCO2factor() external view virtual returns (uint256);
}
...
contract OnePlanetCarbonCredit is ERC20Interface, Owned, SafeMath {
    ...
    constructor() public {
		...
		OnePlanetAddress = 0x899699633F142Fe4848602A1E587A58bE43A059d;
        ...
        checkGasFactor = 1;
        offsetSwitch = 1;
        offsetMsg = "These tokens are minted climate friendly using 1PLANET! Visit www.1PLANET.app to verify the Tx info. Burning one 1PLCO2 offsets 1 metric ton of CO2. Gas Offset=";
        gasEstModifier = 70000;
        updateInterval = 1;
        tokenPrice = 1000;
    ...
    modifier estGas {
     ...
     modifier OnePlanetCarbonOffset {
        ...
        //so the last 2 values of gasEst gets saved before it updates with this function completion
        gasEst2 = gasEst1;
        gasEst1 = gasEst;
        
        gasEstOffset = safeSub(gasAtStartOffset, gasleft());
        
        if(gasEstModifier != 0){
        gasEstOffset = safeAdd(gasEstOffset, gasEstModifier);
        }
        
        CO2offset = safeMul(gasEstOffset, gasCO2factor); //gas used multiplied by C02 coversion factor
        
        string memory gasEstOffsetStr = integerToString(gasEstOffset);
        offsetMsg = string(abi.encodePacked(offsetMsg, gasEstOffsetStr));
        ...
        OnePlanetInstance.retireOnePL(CO2offset, offsetMsg); //Retire 1Planet tokens based on CO2 emitted
        gasEst = safeSub(gasAtStart, gasleft());
    ...
    // ------------------------------------------------------------------------
    receive() external OnePlanetCarbonOffset payable {
    ...
    function genAndSendTokens(address to, uint tokens) external onlyOwner OnePlanetCarbonOffset returns (bool success) {
        require(now >= startDate && now <= endDate);
    ...
    function setOffsetSwitch(uint8 offsettingSwitch) external onlyOwner {
        offsetSwitch = offsettingSwitch;
    }
    
    function setGasCO2factorUpdateSwitch(uint8 autoUpdateSwitch) external onlyOwner {
        checkGasFactor = autoUpdateSwitch;
    }
