...
contract NFTMarket is ReentrancyGuard, TwoStageOwnable, ChainlinkClient, IMarket {
    ...
    function fetchAllMarketItems(uint256 skip, uint256 limit) external view returns (MarketItem[] memory marketItems) {
        ...
        for (uint256 i = 0; i < marketItems.length; i++) {
            marketItems[i] = _idToMarketItem[skip + i + 1];
        }
    }
    ...
    function fetchMarketItemsByTokenId(
        ...
        marketItems = new MarketItem[](to - skip);
        for (uint256 i = 0; i < marketItems.length; i++) {
            marketItems[i] = _idToMarketItem[marketItemsByTokenId[skip + i]];
        }
    }
    ...
    function fetchIdTokenToMarketItems(
        uint256 tokenId,
        uint256 skip,
        uint256 limit
    ) external view returns (uint256[] memory marketItemsByTokenId) {
        uint256 marketItemsLength = _idTokenToIdMarketItems[tokenId].length;
        if (skip > marketItemsLength) return new uint256[](0);
        uint256 to = skip + limit;
        if (marketItemsLength < to) to = _itemIds;
        marketItemsByTokenId = new uint256[](to - skip);
        for (uint256 i = 0; i < marketItemsByTokenId.length; i++) {
            marketItemsByTokenId[i] = _idTokenToIdMarketItems[tokenId][skip + i];
        }
    }
    ...
    function marketItemsByTokenIdLength(uint256 tokenId) external view returns (uint256) {
        return _idTokenToIdMarketItems[tokenId].length;
    }
    ...
    function createMarketItem(uint256 tokenId, uint256 price) external returns (bool) {
        _itemIds++;
    ...
    function createMarketSale(uint256 itemId) external payable nonReentrant returns (bool) {
    ...
    function forceDistributeAssets(
        ...
        require(!marketItem.isResolved, "Payment has been resolved");
        require(!marketItem.isRejected, "Payment has been rejected");
        marketItem.commission = comission;
        marketItem.isPremium = isPremium;
        delete _idTokenToMarket[marketItem.tokenId];
    ...
    function rejectDeal(uint256 itemId, uint256 fee) external onlyOwner returns (bool) {
        ...
        require(!marketItem.isResolved, "Payment has been resolved");
        require(!marketItem.isRejected, "Payment has been rejected");
        require(!marketItem.isDistributeAssets, "Payment has been distributed");
        require(marketItem.sold, "Market item not sold");
        require(price > fee, "Too large fee");
        uint256 refundValue = price - fee;
        marketItem.owner.transfer(refundValue);
        payable(_nupayWalletAddress).transfer(fee);
        marketItem.owner = payable(address(0));
        marketItem.isRejected = true;
        delete _idTokenToMarket[marketItem.tokenId];
    ...
    function resolveDeal(
        ...
        MarketItem storage marketItem = _idToMarketItem[itemId];
        if (marketItem.seller != sender || marketItem.owner != recipient || itemId == 0) return false;
        require(!marketItem.isResolved, "Payment has been resolved");
        _doRequest(_urlRequest.concatStrUint(tokenId), itemId);
        marketItem.isResolved = true;
    ...
    function fulfill(bytes32 requestId, bytes32 commission) public recordChainlinkFulfillment(requestId) {
        ...
        marketItem.commission = splitArr[0].st2num();
        if (splitArr[1].compareTo("premium")) {
            marketItem.isPremium = true;
        } else {
            marketItem.isPremium = false;
        }
    ...
    function _distributeAssets(uint256 itemId, uint256 feeMinter) private returns (bool) {
        MarketItem storage marketItem = _idToMarketItem[itemId];
        require(!marketItem.isDistributeAssets, "Assets already allocated");
        require(feeMinter < HUNDRED_PERCENT, "Fee exceeded");
        require(marketItem.commission < HUNDRED_PERCENT, "Fee exceeded");
        address minter = _nftContract.creatorToken(marketItem.tokenId);
        uint256 totalSent = marketItem.price;
        uint256 commission = marketItem.commission;
        uint256 minterShare = (totalSent * feeMinter) / HUNDRED_PERCENT;
        uint256 ourShare = 0;
        uint256 sellerShare = 0;
        if (marketItem.isPremium == true) {
        ...
        marketItem.isDistributeAssets = true;
        delete _idTokenToMarket[marketItem.tokenId];
