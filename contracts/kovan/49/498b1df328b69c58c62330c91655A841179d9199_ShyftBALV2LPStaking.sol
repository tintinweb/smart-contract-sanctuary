...
contract ShyftBALV2LPStaking is Ownable {
  ...
  function getTwoTokensReward(
    ...
    // reward amount USD
    uint256 pendingUSD = pendingAmount.mul(shyftPrice);
    ...
    if (pendingUSD > 0 && priceA > 0 && priceB > 0) {
      ...
      if (totalValue > 0) {
        uint256 shareA = priceA.mul(balancePair.balanceA).div(totalValue.div(1e18));
        uint256 shareB = priceB.mul(balancePair.balanceB).div(totalValue.div(1e18));

        amountA = shareA.mul(pendingUSD).div(priceA); // div(1e18) for pendingUSD - on frontend, div(1e16) for shareA(:priceA)
        amountB = shareB.mul(pendingUSD).div(priceB); // div(1e18) for pendingUSD - on frontend, div(1e16) for shareB(:priceB)
      }
    }
  }

  // Get reward with several tokens
  function getTwoTokensReward2(
    uint256 _balPoolId, 
    address _tokenA, 
    address _tokenB
  ) external view returns (uint256 amountA, uint256 amountB) {
  ...
  function getTwoTokensReward3(
    ...
    if (pendingUSD > 0 && priceA > 0 && priceB > 0) {
      ...
      if (totalValue > 0) {
        ...
        amountA = shareA.div(1e18).mul(pendingUSD).div(priceA); // div(1e18) for pendingUSD - on frontend, div(1e16) for shareA(:priceA)
        amountB = shareB.div(1e18).mul(pendingUSD).div(priceB); // div(1e18) for pendingUSD - on frontend, div(1e16) for shareB(:priceB)
      }
    }
  }
