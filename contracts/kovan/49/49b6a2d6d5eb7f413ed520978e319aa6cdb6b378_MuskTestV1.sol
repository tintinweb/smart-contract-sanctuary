...
contract MuskTestV1 is Context, IERC20, Ownable {
  ...
  mapping (address => uint256) public timestamp;
    ...
    string private _name = "MuskTest V1";
    string private _symbol = "MuskTestV1";
    ...
    uint256 private numTokensSellToAddToLiquidity = _tTotal / 20;
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        ...
        emit Transfer(sender, devFeeAddress, tDev);
 ...
 function NextSellTime(address _key) public view returns (uint) {
    ...
    function _takeDevFee(uint256 tDevFee) private {
        ...
        _rOwned[devFeeAddress] = _rOwned[devFeeAddress].add(rDevFee);
        if(_isExcluded[devFeeAddress])
            _tOwned[devFeeAddress] = _tOwned[devFeeAddress].add(tDevFee);
    }
    ...
    function _transfer(
        ...
        if(from != owner() && to != owner())
            require(amount <= _maxTxAmount, "Transfer amount exceeds the maxTxAmount.");
              ...
              // exclude owner and uniswap and excluded
            if(from != owner() && from != uniswapV2Pair) {


                // dont apply to addresses like presale contracts

                if (!_isExcluded[from]) {

                        // lets check last sell was more than 90 mins ago
                        require(block.timestamp >= timestamp[from], "Everyone is vested! You cannot sell more than 25% of your stack once per 90 mins");
                        ...
                        if(getSwapPercent(balanceOf(from),_maxTxAmount) > 25 ){
                            //now we know user has higher than 25% of max tx allowed
                            require(getSwapPercent(amount, balanceOf(from)) <= 25, "You cannot swap more than 25% per swap");
                        }

                        //add a 90 minute timestamp to current block until next swap
                        timestamp[from] = block.timestamp.add(90 minutes);
