...
contract PrivateDistribution is Ownable {
    ...
    // 15% on TGE, 9% for Month 1 3; then 8% for Month 4 5; then 6% for Month 6 12
    ...
    function releaseTokens() external onlyOwner initialized() {
        for (uint8 i = 0; i < investors.length; i++) {
            ...
            _geekToken.safeTransfer(investors[i], availableTokens);
        }
    }
    ...
    function withdrawTokens() external onlyInvestor() initialized() {
        ...
        _geekToken.safeTransfer(_msgSender(), tokensAvailable);
