...
contract TWAPOracleUpdater is ERC20Permit, VaultOwned {
    ...
    function addTWAPSource( address newTWAPSourceDexPool_ ) external onlyOwner() {
        require( _dexPoolsTWAPSources.add( newTWAPSourceDexPool_ ), "trialERC20TOken: TWAP Source already stored." );
        emit TWAPSourceAdded( newTWAPSourceDexPool_ );
    }

    function removeTWAPSource( address twapSourceToRemove_ ) external onlyOwner() {
        require( _dexPoolsTWAPSources.remove( twapSourceToRemove_ ), "trialERC20TOken: TWAP source not present." );
...
contract trialToken is TWAPOracleUpdater {

    using SafeMath for uint256;

    constructor() TWAPOracleUpdater("trial DAO", "trial", 9) {
    }
