...
contract MagicShuffle is VRFConsumerBase, ConfirmedOwner(msg.sender) {
    ...
    mapping(string => bool) private s_roll_in_progress;
    ...
    function rollDice(string memory roller) public onlyOwner returns (bytes32 requestId) {
        ...
        require(!s_roll_in_progress[roller], "Roll in progress");

        requestId = requestRandomness(s_keyHash, s_fee);

        s_rollers[requestId] = roller;
        s_roll_in_progress[roller] = true;
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        ...
        s_roll_in_progress[s_rollers[requestId]] = false;
        emit DiceLanded(requestId, random);
    }

    function traits(string memory player) public view returns (uint8[] memory) {
        require(s_results[player].length > 0, "Not rolled");
        require(!s_roll_in_progress[player], "Roll in progress");
