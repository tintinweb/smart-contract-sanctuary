...
/**
 * @title Iron Bank Router for Iron Bank Hive Vault
 * @notice This contract is created for Easy XBE project
 * @author Oleg Bedrin
 * @dev Created on: 21.10.2021
 */
contract IronBankRouter is IRouterV2, SushiswapSwaps {

    using SafeERC20 for IERC20;

    IVaultTransfers public ibHiveVault; // Iron Bank Hive Router address
    IVotingStakingRewards public votingStakingRewards; // XBE Staking Contract
    ICurveIronBankStableSwap public curveIronBank; // Curve Iron Bank StableSwap pool - 0x2dded6Da1BF5DBdF597C45fcFaa3194e53EcfeAF
    IERC20 public curveIronBankLpToken; // Curve Iron Bank StableSwap LP Token
    ...
    struct ConstructorParams {
        ...
        address _usdt; // USDT
        address _xbe; // XBE
        address _ibHiveVault; // Iron Bank Hive Vault
        address _votingStakingRewards; // Voting Staking Rewards (XBE Staking Contract)
        address _curveIronBank; // Curve Iron Bank StableSwap Pool
        address[] _addresses; // addresses of tokens in Curve Iron Bank StableSwap Pool
        uint256[] _tokenIdxsInCurvePool; // indexes of tokens in Curve Iron Bank StableSwap Pool
    }

    /**
     * @notice Constructor for IronBankRouter
    ...
    {
        TransferHelper.safeApprove(_token, address(curveIronBank), _tokensReceived);
        uint256[3] memory amounts;
        amounts[tokenToIndex[_token]] = _tokensReceived;
        curveIronBankLpAmount = curveIronBank.add_liquidity(
            amounts,
            _minAmountLP,
            true
        );
    ...
    function _depositToHive(address _recipient, uint256 _curveIronBankLpAmount) internal {
        TransferHelper.safeApprove(
            address(curveIronBankLpToken),
            address(ibHiveVault),
            _curveIronBankLpAmount
        );
        ibHiveVault.depositFor(_curveIronBankLpAmount, _recipient);
    }



    /**
     * @notice This function is for deposit to Iron Bank Hive Vault.
     * It should take your either ETH or USDC, or DAI, or USDT, convert to USDC amount A of ETH if ETH provided,
     * convert amount B of provided token to XBE and auto-stake it for you.
     * The A amount of provided token goes to Curve Iron Bank StableSwap pool,
     * and all acquired LP of the curve pool then staked in Iron Bank Hive Vault
        ...
        returns(uint256 curveIronBankLpAmount)
    {
        ...
        curveIronBankLpAmount = _depositToCurve(params.token, tokensReceived, params.minAmountLP);

        _depositToVotingStakingRewards(swappedAmountBToXBE, params.recipient);
        _depositToHive(params.recipient, curveIronBankLpAmount);
    }

    /**
     * @notice This function is for withdraw funds from Iron Bank Hive Vault.
     * It should take your provided Iron Bank Hive Vault LP tokens, withdraw your Curve Iron Bank StableSwap
