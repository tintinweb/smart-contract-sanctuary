...
contract VolmexIndexFactory is OwnableUpgradeable {
    ...
    event PositionTokenImplementationUpdated(
        address indexed newPositionTokenImplementation
    );
    ...
    /**
     * @notice Update the `positionTokenImplementation`
     * @param _newPositionTokenImplementation address of the new position token implementation
     */
    function updatePositionTokenImplementation(
        address _newPositionTokenImplementation
    ) external onlyOwner {
        require(
            _newPositionTokenImplementation != address(0),
            "IndexFactory: Can't set the zero address"
        );

        positionTokenImplementation = _newPositionTokenImplementation;

        emit PositionTokenImplementationUpdated(
            _newPositionTokenImplementation
        );
    }
    ...
    function registerIndex(
        ...
        IERC20Modified volatilityToken = _volmexProtocolContract
        .volatilityToken();
        IERC20Modified inverseVolatilityToken = _volmexProtocolContract
        .inverseVolatilityToken();
        ...
        returns (address)
    {
        ...
        VolmexPositionToken newVolatilityToken = VolmexPositionToken(
 ...
 * @author volmex.finance [[email protected]]
 */
contract VolmexProtocol is BaseProtocol {
    using VolmexSafeERC20 for IERC20Modified;

    /**
     * @dev Call the base protocol initializer
     *
 ...
 * @author volmex.finance [[email protected]]
 *
 * This is a base protocol having all the common methods
 */
