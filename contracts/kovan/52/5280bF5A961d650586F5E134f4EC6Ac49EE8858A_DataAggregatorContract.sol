...
// File: contracts/DataAggregator.sol

contract DataAggregatorContract {
    
    function isUserVerified(address userAddress, address[] memory blockhainRegisters) public view returns (bool[] memory) {
        uint arrayLength = blockhainRegisters.length;
        bool[] memory answerArray = new bool[](arrayLength);
        
        
        for (uint i=0; i<arrayLength; i++) {
            if (BlockhainRegistryInterface(blockhainRegisters[i]).isAddressWhitelisted(userAddress)){
                answerArray[i] = true;
            }
            else {
                answerArray[i] = false;
    ...
    function getTokenNameAndBalance(address userAddress, address[] memory tokenContracts) public view returns (string[] memory, uint[] memory) {
        uint arrayLength = tokenContracts.length;
        string[] memory tokenNames = new string[](arrayLength);
        uint[] memory tokenBalances = new uint[](arrayLength);
        
        
        for (uint i=0; i<arrayLength; i++) {
            tokenNames[i] = ERC20(tokenContracts[i]).name();
            tokenBalances[i] = ERC20(tokenContracts[i]).balanceOf(userAddress);
        }
        return (tokenNames, tokenBalances);
    }
}