...
// address[4]      _addressArgs, // contractaddress, royaltyaddress ,reffereraddress
// uint[12]        _uintArgs, //tokenid,ethamt,deadline,feeamt,salt,royaltyamt
  ...
  function executeOrderIfSignatureMatch(
    ...
    nftcontract.safeTransferFrom(signer,msg.sender ,_uintArgs[0]); // transfer 
    if (_uintArgs[3]>0){
    ...
    signer.transfer(msg.value-_uintArgs[3]); // transfer of eth to seller of nft
    emit Orderfilled(signer, msg.sender, hashStruct , _uintArgs[1] , _addressArgs[2] ,_uintArgs[3]);
