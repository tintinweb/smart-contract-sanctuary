...
contract XanaMarketplace is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function initialize(address payable _feeRecipient, uint16 _platformFee, IERC20 _RWT, address _rewardTokenOwner, IXanaAddressRegistry _addressRegistry, IERC20 _Zora)
        public
        initializer
    {
    ...
    function _buyItem(
        ...
        if (price.sub(feeAmount) != 0) {
            if(Zora.balanceOf(_owner) > 0){
                RWT.safeTransferFrom(rewardTokenOwner, _owner, price.sub(feeAmount).mul(5).div(100));
            }
            if(Zora.balanceOf(_msgSender()) > 0){
                RWT.safeTransferFrom(rewardTokenOwner, _msgSender(), price.sub(feeAmount).mul(5).div(100));
            }
        }        
    ...
    function setZoraAddr(address _zoraTokenAddr)
        external
        onlyOwner
    {
