...
contract FreeBettingContract is AbstractBettingContract {
    ...
    function setMinEntrant(uint256 _minEntrant) external onlyOwner onlyLock {
    ...
    function close() external override onlyPriceClosed onlyOpen {
        ...
        if (winners.length > minEntrants) {
            ...
            IERC20(IBettingPool(owner).getTokenPool()).safeTransfer(creator, getTotalToken());
        }
