...
*/
contract FiftyFiftyAllocationStrategy is IAllocationStrategy, UniswapModule, Ownable, ReentryProtection {

    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    uint256 public constant MIN_COMP_CLAIMING = 10**10;
    ...
    */
    constructor(address _aToken, address _lendingPoolAddressesProvider, address _cToken, address _comptroller, address _compAddress, address _underlying, address _uniswapRouter) public {
        ...
        lendingPoolCore = IAaveLendingPoolCore(provider.getLendingPoolCore());
    ...
    */
    function investETH(uint256 _amountOutMin, uint256 _deadline) external payable override returns (uint256) {
        ...
        uint256 firstInvestAmount = investAmount.div(2);
        underlying.safeApprove(address(lendingPoolCore), firstInvestAmount);
        // TODO consider setting Aave ref id
        lendingPool.deposit(address(underlying), firstInvestAmount, 0);

        uint256 remainingInvestAmount = investAmount - firstInvestAmount;
        underlying.safeApprove(address(cToken), remainingInvestAmount);
    ...
    */
    function investUnderlying(uint256 _investAmount, uint256 _deadline) external override returns (uint256) {
    ...
    */
    function invest(address _tokenIn, uint256 _investAmount, uint256 _amountOutMin, uint256 _deadline) external override returns (uint256) {

        farmYield(_deadline);

        uint256[] memory swappedAmounts = swapExactTokensForTokens(_tokenIn, address(underlying), _investAmount, _amountOutMin, _deadline, uniswapRouter);
        uint256 investAmount = swappedAmounts[1];
        uint256 firstInvestAmount = investAmount.div(2);
        underlying.safeApprove(address(lendingPoolCore), firstInvestAmount);
        // TODO consider setting Aave ref id
    ...
    */
    function redeemUnderlying(uint256 _redeemAmount) external override onlyOwner returns(uint256) {
        ...
        aToken.redeem(redeemAmountAave);
        underlying.safeTransfer(msg.sender, redeemAmountAave);

        require(cToken.redeemUnderlying(redeemAmountCompound) == 0, "cToken.redeemUnderlying failed");
        underlying.safeTransfer(msg.sender, redeemAmountCompound);
    ...
    */
    ...
    /**
        @notice Claim yield from protocols
        @param _deadline Deadline for a swap
    */
    function farmYield(uint256 _deadline) public {
        if (getUnclaimedComp() < MIN_COMP_CLAIMING)
            return;
        comptroller.claimComp(address(this));
        uint256 compBalance = COMP.balanceOf(address(this));
        uint amountOut = getAmountsOut(address(COMP), address(underlying), compBalance, uniswapRouter);
        uint256[] memory swappedAmounts = swapExactTokensForTokens(address(COMP), address(underlying), compBalance, amountOut, _deadline, uniswapRouter);
        underlying.safeApprove(address(cToken), swappedAmounts[1]);
        require(cToken.mint(swappedAmounts[1]) == 0, "mint failed");
    }
...
contract UniswapModule {
    ...
    function swapExactTokensForTokens(
        ...
        path[0] = _tokenIn;
        path[1] = _tokenOut;
        IERC20(_tokenIn).safeApprove(_uniswapRouter, _amountIn);
