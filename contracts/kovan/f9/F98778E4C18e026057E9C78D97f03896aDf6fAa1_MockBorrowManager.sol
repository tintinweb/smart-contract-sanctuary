...
contract MockBorrowManager is IBorrowManager, AccessControlEnumerable, ReentrancyGuard, Pausable
{
  ...
  function _transferBorrowFee (BorrowMetadata memory metadata, address feeManager, address collateralToken) private
  {
    (uint256 debtFee, address burner) = IFeeManager(feeManager).getBorrowFeeOnDebt(metadata.debtInCollateralToken, metadata.isDiscounted);
  ...
  function borrow (address debtToken, uint256 amount, address collateralToken, uint256 collateral, uint256 interestRate, uint256 durationInSecs) external nonReentrant
  {
    ...
    BorrowMetadata memory metadata = BorrowMetadata
    ({
      isDiscounted: IStakingManager(coordinators.stakingManager).isDiscountedBorrower(msg.sender),
      debtInUSD: IOracle(coordinators.oracle).convertToUSD(debtToken, amount),
      debtInCollateralToken: IOracle(coordinators.oracle).convert(debtToken, collateralToken, amount)
    });

    (address depositVault, address collateralVault) = ICoordinator(coordinators.coordinator).increaseTokenTotalBorrowedAndGetPairVault(debtToken, collateralToken, amount);

    require(_isValidBorrow(metadata.debtInUSD, interestRate, durationInSecs), "!valid terms");
    require(IERC20(debtToken).balanceOf(depositVault) >= amount, "!enough liq");
    ...
    IStakingManager(coordinators.stakingManager).increaseBorrowerExpectedStake(msg.sender, amount, metadata.debtInUSD, block.timestamp + durationInSecs);

    IRewardManager(coordinators.rewardManager).registerDebt(id, metadata.debtInUSD, interestRate, metadata.isDiscounted);


    _transferBorrowFee(metadata, coordinators.feeManager, collateralToken);
    IERC20(collateralToken).safeTransferFrom(msg.sender, collateralVault, collateral);
    IERC20(debtToken).safeTransferFrom(depositVault, msg.sender, amount);
  ...
  {
    // TODO: bug; should be > 1 *important*
    require(debt.startTimestamp > 0 && debtMetadata.status == Status.Active, "!active");
