...
/**
 * @title Defi99's DErc20LINKDelegator Contract
 * @notice DTokens which wrap an EIP-20 underlying and delegate to an implementation
 */
contract DErc20LINKDelegator is DTokenInterface, DErc20Interface, DDelegatorInterface {
    /**
     ...
     */
    function _setImplementation(address implementation_, bool allowResign, bytes memory becomeImplementationData) public {
        require(msg.sender == admin, "DErc20LINKDelegator::_setImplementation: Caller must be admin");
     ...
     */
    function liquidateBorrow(address borrower, uint repayAmount, DTokenInterface dTokenCollateral) external returns (uint) {
        borrower; repayAmount; dTokenCollateral; // Shh
        delegateAndReturn();
    }
   ...
   /**
    ...
    function () external payable {
        require(msg.value == 0,"DErc20LINKDelegator:fallback: cannot send value to fallback");
