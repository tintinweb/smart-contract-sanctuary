...
contract MockCore {
    ...
    function cancel(uint256 _cryptoGameId) external onlyMaintainer {
        ...
        require(createdConditions[_cryptoGameId].isCancelled == false, "Azuro: condition was already cancelled");
    ...
    function resolveCondition(uint256 _cryptoGameId) external onlyMaintainer {
        require(createdConditions[_cryptoGameId].timestamp > 0, "Azuro: condition not exists");
        require(createdConditions[_cryptoGameId].isResolved == false, "Azuro: condition was already resolved");
