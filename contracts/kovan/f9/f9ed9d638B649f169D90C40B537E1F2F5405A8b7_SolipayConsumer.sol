...
/**
 * @title A consumer contract for Solipay
 * @author LinkPool
 * @notice Interact with the Solipay Location API for Chainlink.
 */
contract SolipayConsumer is ChainlinkClient {
    using Chainlink for Chainlink.Request;

    /* ========== CONSUMER STATE VARIABLES ========== */

    struct ContainsMostVisitedLocation {
    ...
    // Maps
    mapping(bytes32 => ContainsMostVisitedLocation) public requestIdContainsMostVisitedLocation;
    ...
    /**
     * @notice Returns wether the most visited location of a Solipay wallet is in the list of country codes.
     * @param _specId the jobID.
     * @param _payment the LINK amount in Juels (i.e. 10^18 aka 1 LINK).
     * @param _address the Solipay address.
     * @param _countryCodes the list of country codes (ISO 3166 alpha-2) dash-separated, e.g. ES-PT-IT.
     */
    function requestContainsMostVisitedLocation(
        bytes32 _specId,
        uint256 _payment,
        ...
        Chainlink.Request memory req = buildChainlinkRequest(
            _specId,
            address(this),
            this.fulfillRequestMostVisitedLocation.selector
        );

        req.add("endpoint", "contains-most-visited-location"); // Not required if it has been hardcoded in the jobspec

        req.add("address", _address);
        req.add("countryCodes", _countryCodes);
        req.addUint("days", 1); // Not required if it is not present in the jobspec

        bytes32 requestId = requestOracleData(req, _payment);

        ContainsMostVisitedLocation memory cmvl;
        cmvl.walletAddress = _address;
        cmvl.countryCodes = _countryCodes;
        cmvl.noDays = 1;

        requestIdContainsMostVisitedLocation[requestId] = cmvl;
    }
     ...
     * @param _payment the LINK amount in Juels (i.e. 10^18 aka 1 LINK).
     * @param _address the Solipay address.
     * @param _countryCodes the list of country codes (ISO 3166 alpha-2) dash-separated, e.g. PT-ES-FR-IT.
     * @param _days the minimum number of days the address has been in the most visited location (from 1 to 21).
     */
    function requestContainsMostVisitedLocation(
        bytes32 _specId,
        uint256 _payment,
    ...
    /**
     * @notice Returns the Solipay Location API response for the given wallet, country codes and days.
     * @param _requestId the request ID for fulfillment.
     * @param _isValid whether the address' most visited country is in the list of country codes.
     */
    function fulfillRequestMostVisitedLocation(bytes32 _requestId, bool _isValid)
        public
        recordChainlinkFulfillment(_requestId)
    {
        requestIdContainsMostVisitedLocation[_requestId].isValid = _isValid;
    }
