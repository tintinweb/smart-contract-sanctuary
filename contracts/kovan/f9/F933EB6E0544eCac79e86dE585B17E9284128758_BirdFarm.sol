...
contract BirdFarm is Ownable {
    ...
    mapping(address => bool) private canHarvestNextTime;
    ...
    function deposit(uint256 _pid, uint256 _amount) external {
        ...
        canHarvestNextTime[msg.sender] = true;
    ...
    function harvestPendingReward(uint256 _pid) external {
        require(
            canHarvestNextTime[msg.sender],
            "You have already harvested. Deposit some pool tokens to get reward and harvest reward."
        );
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(
        ...
        setPendingReward(msg.sender, _pid, 0);
        if (user.amount == 0) canHarvestNextTime[msg.sender] = false;
    ...
    function setPendingReward(
        ...
        pendingRewardOf[_user][_pid] = _amount;
    }
