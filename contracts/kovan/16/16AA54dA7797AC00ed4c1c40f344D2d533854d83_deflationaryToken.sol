...
contract deflationaryToken is ERC20, Ownable, ReentrancyGuard {
    ...
    uint16 private defMaxInterestRate = 2000; // deflationary max interest rate, 1% equal 100
    ...
    function mintAmount(address[] calldata _accounts, uint256 _amount) external onlyOwner nonReentrant {
    ...
    function mintAmounts(address[] calldata _accounts, uint256[] calldata _amounts) external onlyOwner nonReentrant {
        require(_accounts.length == _amounts.length, "DeflationaryToken: Invalid arrays length");
        for (uint i = 0; i < _accounts.length; ++i) {
    ...
    function updateDefAddress(address _defAddress) external onlyOwner {
        require(_defAddress != address(0), "DeflationaryToken: Invalid defAddress");
        defAddress = _defAddress;
        UpdateDefAddress(_defAddress);
    }

    function updateDefFixedFee(uint256 _defFixedFee) external onlyOwner {
        defFixedFee = _defFixedFee;
        UpdateDefFixedFee(_defFixedFee);
    }

    function updateDefInterestRate(uint16 _defInterestRate) external onlyOwner {
        require(_defInterestRate <= defMaxInterestRate, "DeflationaryToken: Invalid defInterestRate");
        defInterestRate = _defInterestRate;
        UpdateDefInterestRate(_defInterestRate);
    }

    function calculateFee(uint256 amount) public view returns (uint256) {
        return defFixedFee + amount * defInterestRate / 10000;
    }

    function transfer(address to, uint256 amount) public override returns (bool) {
        _transferFee(msg.sender, amount, false);
        return super.transfer(to, amount);
    }

    function transferFrom(address from, address to, uint256 amount) public override returns (bool) {
        _transferFee(from, amount, true);
        return super.transferFrom(from, to, amount);
    }

    function _transferFee(address from, uint256 amount, bool useTransferFrom) internal returns (bool) {
        uint256 calculatedFee = calculateFee(amount);
        if (calculatedFee > 0) {
            if (useTransferFrom) {
                super.transferFrom(from, defAddress, calculatedFee);
            } else {
