...
contract SafeSolAlpha is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "Safe Solidity Alpha";
    string private _symbol = "SAFESOL-A";
    ...
    //@dev The dev fee contains two decimal places so 350 = 3.5%
    uint256 public _feeDecimal = 2;
    uint256 public _devFee = 350;
        ...
        returns (uint256)
    {
        ...
        if (!deductTransferFee) {
            ...
            return
                tokenAmount.sub(tokenAmount.mul(_devFee).div(10** _feeDecimal + 2)).mul(
                    _getReflectionRate()
                );
        }
    }
    ...
    function collectFee(address account, uint256 amount, uint256 rate) private returns (uint256) {
        ...
        //@dev Tax fee
        if(_devFee != 0){
            uint256 taxFee = amount.mul(_devFee).div(10**(_feeDecimal + 2));
            transferAmount = transferAmount.sub(taxFee);
            _reflectionTotal = _reflectionTotal.sub(taxFee.mul(rate));
            _devFeeTotal = _devFeeTotal.add(taxFee);
