...
contract SperaxToken is ERC20, Pausable, MintPausable, Ownable {
    ...
    address public USDsVaultAddr;
    bool public USDsVaultAddrSet;
     ...
     */
    function setUSDsVaultAddr(address account) onlyOwner external {
        require(USDsVaultAddrSet == false, "address already set");
        USDsVaultAddr = account;
        USDsVaultAddrSet = true;
    }
     ...
     * @dev mint SPA when USDs is burnt
     */
    function mintForUSDs(uint256 amount, address account) whenMintNotPaused external {
        require(_msgSender() == USDsVaultAddr, "can only mint SPA for USDs redeeming/burning");
