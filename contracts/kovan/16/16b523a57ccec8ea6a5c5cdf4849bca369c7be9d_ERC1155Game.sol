...
contract ERC1155Game is  OwnableUpgradeable, ReentrancyGuardUpgradeable{
    ...
    mapping(uint8=>IERC20) public tokenNames;
    
    //tokenid-> tokenname-> Fix price of ingredient
    mapping(uint256=> mapping(uint8 => uint256)) public fixPriceMap;

    //tokenid-> tokenname-> Variable fix price of ingredient
    mapping(uint256=> mapping(uint8 => uint256)) public fixVarPriceMap;
    ...
    event LogCreateIngredient(uint256 tokenId, address to, string _Uri, bytes  _data, uint8 _class);

    event LogCreateDish(uint256 tokenId, address to, string _Uri, bytes  _data, uint256[]  _burnTokenIdRequired, uint256[] _burnTokensQtyRequired);
    ...
    /**
        * @dev Returns the sale prices of all ERC20 tokens 
        * @return result returns Array of sale prices of each erc20 token index
    */
    function getCurrentPrice(uint256 id) public view
            returns (uint256[] memory){
							uint256[] memory prices= new uint[](totalSaleTokens); 
            if(tokenType[id]._type==1){
                for(uint8 i=0;i<totalSaleTokens;i++){
                    prices[i] = fixPriceMap[id][i];
                }
                return prices;
            }else{
                ...
                for(uint8 i=0;i<totalSaleTokens;i++){
                    uint currPrice = fixVarPriceMap[id][i].mul(multiplyFactor).div(1000000);
                    prices[i] = currPrice;
                }
    ...
    function createIngredientNFT(address to, uint256 initialSupply, string calldata _Uri,
        ...
        emit LogCreateIngredient(id, to,  _Uri, _data, _class); 
    ...
    function createDishNFT(address to, uint256 initialSupply, string calldata _Uri,
        ...
        emit LogCreateDish(id, to,  _Uri, _data, _burnTokenIdRequired, _burnTokensQtyRequired);
