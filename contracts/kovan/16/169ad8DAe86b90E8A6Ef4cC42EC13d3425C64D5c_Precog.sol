...
contract Precog {
    ...
    function test( address[] memory pairs, TradeInfo memory trade) external view returns (uint reserveAExchange, uint reserveBExchange){
    ...
    function testfuture (uint expiryDate, address[] memory pairs, TradeInfo memory trade) external view returns (uint reserveAFutureExchange, uint reserveBFutureExchange){
    ...
    function testOptimize(uint reserveAExchange, uint reserveBExchange, uint reserveAFutureExchange, uint reserveBFutureExchange) external view returns (uint tradeAmount){
        uint sqrtK;
        unchecked {
