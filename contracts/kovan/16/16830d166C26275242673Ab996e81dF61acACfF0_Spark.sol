...
/*
    1.1.7
    1. changed currentAPR() to computeAPRatTime()
*/
...
contract Spark is Ownable, OwnableExt {
    ...
    address public constant REWARD_TOKEN_POOL = 0x90a5a0616e86A46D927dFF34386C8Bc969e42421;
    ...
    /**
     * @notice compute instantaneous APR at Time based on deposit creation time
     * @param stakedTimeStamp timestamp of deposit creation time
     * @param Time tiemsStamp at which the function compute the APR
     * @return APR (precision = APR_PREC, no %)
     */
    function computeAPRatTime(uint stakedTimeStamp, uint Time) public view returns (uint) {
        require(stakedTimeStamp < Time, "computeAPRatTime: the deposit made in the future.");
        require(stakedTimeStamp > startTimeStamp, "computeAPRatTime: the deposit made before the program started.");
        uint endTimeNoExt = startTimeStamp + 29 days;
        uint endTimeExt = endTimeNoExt + extDays * (1 days);
        require(stakedTimeStamp <= endTimeExt, "computeAPRatTime: the deposit made after the program has ended.");
        if (Time > endTimeExt) {
            return 0;
        }
        if (Time > endTimeNoExt) {
            return extAPR
                   .mul(APR_PREC);
        }
        uint tNotInExt = Time
     ...
     * @param k the last k in the pair pool (precison = 10 ** 36)
    ...
    function computeLPValue(uint amountInLP, uint poolLP, uint priceRaw, uint k) public pure returns (uint) {
        ...
        uint rawValue = amountInLP
                        ...
                        .mul(2)
                        .div(poolLP);
        uint valueWithPresion = rawValue.div(2**56);
