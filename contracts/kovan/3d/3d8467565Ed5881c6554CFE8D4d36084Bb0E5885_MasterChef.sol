...
import "./ShibaPalToken.sol";
import "./interfaces/IMasterChef.sol";

// FairLaunch is a smart contract for distributing shibapal by asking user to stake the ERC20-based token.
contract MasterChef is IMasterChef, Ownable, ReentrancyGuard {
  ...
  struct PoolInfo {
    ...
    uint256 accShibaPalPerShareTilBonusEnd; // Accumated ShibaPals per share until Bonus End.
  }
  ...
  function setshibapalPerBlock(uint256 _shibapalPerBlock) external onlyOwner {
    shibapalPerBlock = _shibapalPerBlock;
  }
  ...
  function addPool(
    ...
    poolInfo.push(
      PoolInfo({
        ...
        accShibaPalPerShareTilBonusEnd: 0
      })
    );
  }
  ...
  function manualMint(address _to, uint256 _amount) external onlyOwner {
    shibaPal.manualMint(_to, _amount);
  }
  ...
  function pendingShibaPal(uint256 _pid, address _user) external override view returns (uint256) {
  ...
  function updatePool(uint256 _pid) public override {
    ...
    // update accShibaPalPerShareTilBonusEnd
    if (block.number <= bonusEndBlock) {
      shibaPal.lock(devaddr, shibapalReward.mul(bonusLockUpBps).div(100000));
      pool.accShibaPalPerShareTilBonusEnd = pool.accShibaPalPerShare;
    }
    if(block.number > bonusEndBlock && pool.lastRewardBlock < bonusEndBlock) {
      uint256 shibapalBonusPortion = bonusEndBlock.sub(pool.lastRewardBlock).mul(bonusMultiplier).mul(shibapalPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
      shibaPal.lock(devaddr, shibapalBonusPortion.mul(bonusLockUpBps).div(100000));
      pool.accShibaPalPerShareTilBonusEnd = pool.accShibaPalPerShareTilBonusEnd.add(shibapalBonusPortion.mul(1e12).div(lpSupply));
    }
    pool.lastRewardBlock = block.number;
  }

  // Deposit Staking tokens to FairLaunchToken for SHIBAPAL allocation.
  function deposit(address _for, uint256 _pid, uint256 _amount) external override nonReentrant {
    ...
    user.bonusDebt = user.amount.mul(pool.accShibaPalPerShareTilBonusEnd).div(1e12);
  ...
  // Harvest ShibaPals earn from the pool.
  ...
  function _harvest(address _to, uint256 _pid) internal {
    ...
    require(pending <= shibaPal.balanceOf(address(this)), "wtf not enough shibaPal");
    uint256 bonus = user.amount.mul(pool.accShibaPalPerShareTilBonusEnd).div(1e12).sub(user.bonusDebt);
    safeShibaPalTransfer(_to, pending);
    shibaPal.lock(_to, bonus.mul(bonusLockUpBps).div(10000));
  }
  ...
  function safeShibaPalTransfer(address _to, uint256 _amount) internal {
    ...
    if (_amount > shibapalBal) {
      require(shibaPal.transfer(_to, shibapalBal), "failed to transfer SHIBAPAL");
    } else {
      require(shibaPal.transfer(_to, _amount), "failed to transfer SHIBAPAL");
    }
  }
