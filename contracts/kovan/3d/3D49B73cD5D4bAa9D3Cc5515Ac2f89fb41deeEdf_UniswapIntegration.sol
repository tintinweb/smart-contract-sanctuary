    ...
    IUniswapIntegration
{
    ...
    event LiquidityPositionConfigured(address token0, address token1, uint24 feeNumerator, int24 tickLower, int24 tickUpper, uint256 weight);
    event liquidityPositionWeightUpdated(uint256 liquidityPositionIndex, uint256 weight, bool triggerRebalance);
    ...
    /// @param uniswapIntegrationDeployerAddress The address of the Uniswap Integration Deployer contract
    function setUniswapIntegrationDeployer(address uniswapIntegrationDeployerAddress) external override onlyOwner {
        uniswapIntegrationDeployer = IUniswapIntegrationDeployer(uniswapIntegrationDeployerAddress);
    }

    /// @param baseStablecoinAddress_ The base stablecoin token that the Uniswap Integration uses for swaps
    function setBaseStablecoin(address baseStablecoinAddress_) external override onlyManager {
        require(IIntegrationMap(moduleMap.getModuleAddress(Modules.IntegrationMap)).getIsTokenAdded(baseStablecoinAddress_), 
            "UniswapIntegration::setBaseStablecoin: Token has not been added to IntegrationMap");
        require(baseStablecoinAddress_ != baseStablecoinAddress, 
    ...
    function configureLiquidityPosition(
        ...
        (address token0, address token1) = IUniswapTrader(moduleMap.getModuleAddress(Modules.UniswapTrader)).getTokensSorted(tokenA, tokenB);

        require(IUniswapTrader(moduleMap.getModuleAddress(Modules.UniswapTrader)).getPoolFeeNumerator(tokenA, tokenB, 0) != 0,
        ...
        emit LiquidityPositionConfigured(token0, token1, feeNumerator, tickLower, tickUpper, weight);
    }
    ...
    function updateLiquidityPositionWeight(uint256 liquidityPositionIndex, uint256 weight, bool triggerRebalance) external onlyManager {
        require(liquidityPositionIndex < liquidityPositions.length, "UniswapIntegration::updateLiquidityPositionWeight: Liquidity position does not exist");
        liquidityPositions[liquidityPositionIndex].weight = weight;
        if(triggerRebalance) {
            rebalance();
        }

        emit liquidityPositionWeightUpdated(liquidityPositionIndex, weight, triggerRebalance);
    }
    ...
    function withdraw(address tokenAddress, uint256 amount) external override onlyController {       
        require(amount <= balances[tokenAddress], "UniswapIntegration::withdraw: Withdraw amount is greater than balance");
        ...
        if(amount <= token.balanceOf(address(uniswapIntegrationDeployer))) {
            ...
            uint256 withdrawalAmountInBaseStablecoinValue = getTokenValueInBaseStablecoin(tokenAddress, amount);
            ...
            if(stablecoinReserveBalance > withdrawalAmountInBaseStablecoinValue) {
                ...
                uniswapIntegrationDeployer.swapReservesForWithdrawalToken(tokenAddress);
                if(amount <= token.balanceOf(address(uniswapIntegrationDeployer))) {
                    _withdraw(address(token), amount);
                } else {
                    uniswapIntegrationDeployer.closePositionsForWithdrawal(tokenAddress, amount);
                    if(token.balanceOf(address(uniswapIntegrationDeployer)) >= amount) {
                        _withdraw(address(token), amount);
                    } else {
                        _withdraw(address(token), token.balanceOf(address(uniswapIntegrationDeployer)));
                    }
                }
            } else {
    ...
    function _deploy() private {
        ...
        uint256[] memory liquidityPositionDesiredDeployAmounts = new uint256[](liquidityPositions.length);
        ...
        uint256 totalBaseStablecoinValue = getBaseStablecoinReserveBalance() + getAllPositionsBaseStablecoinValue();
                
        // Calculate desired token amounts
        for(uint256 liquidityPositionIndex; liquidityPositionIndex < liquidityPositions.length; liquidityPositionIndex++) {        
            uint256 positionDesiredBaseStablecoinValue = totalBaseStablecoinValue * liquidityPositions[liquidityPositionIndex].weight / liquidityPositionsWeightSum;
            uint256 positionActualBaseStablecoinValue = getPositionBaseStablecoinValue(liquidityPositionIndex);

            if(positionActualBaseStablecoinValue < positionDesiredBaseStablecoinValue) {
                liquidityPositionDesiredDeployAmounts[liquidityPositionIndex] = positionDesiredBaseStablecoinValue - positionActualBaseStablecoinValue;
            }
            
            uint256 token0Id = integrationMap.getTokenId(liquidityPositions[liquidityPositionIndex].token0);
            uint256 token1Id = integrationMap.getTokenId(liquidityPositions[liquidityPositionIndex].token1);
            (uint256 desiredAmount0, uint256 desiredAmount1) = getIncreaseLiquidityAmounts(liquidityPositionIndex, liquidityPositionDesiredDeployAmounts[liquidityPositionIndex]);
        ...
        uniswapIntegrationDeployer.swapExcessTokensForBaseStablecoin(tokenDesiredAmounts);

        // Swap excess base stablecoin for desired token amounts
        uniswapIntegrationDeployer.swapExcessBaseStablecoinForTokens(tokenDesiredAmounts);
    ...
    function _withdraw(address tokenAddress, uint256 amount) private {
        IERC20MetadataUpgradeable token = IERC20MetadataUpgradeable(tokenAddress);
        uint256 balanceBefore = token.balanceOf(address(uniswapIntegrationDeployer));

        IERC20MetadataUpgradeable(token).safeTransferFrom(address(uniswapIntegrationDeployer), 
            moduleMap.getModuleAddress(Modules.Kernel), amount);
                
        balances[tokenAddress] -= (balanceBefore - token.balanceOf(address(uniswapIntegrationDeployer)));
    }
    ...
    function transferTokensToUniswapIntegrationDeployer() private {
        ...
        for(uint256 tokenId; tokenId < tokenCount; tokenId++) {
            ...
            if(token.balanceOf(address(this)) > 0) {
                if(token.allowance(address(uniswapIntegrationDeployer), address(this)) == 0) {
                    uniswapIntegrationDeployer.tokenApprovals(address(token));
                }

                token.safeTransfer(address(uniswapIntegrationDeployer), token.balanceOf(address(this)));
            }
        }
    }
    ...
    function increaseLiquidityPositions(uint256[] memory liquidityPositionDesiredDeployAmounts) private {
        for(uint256 liquidityPositionIndex; liquidityPositionIndex < liquidityPositions.length; liquidityPositionIndex++) {
            if(liquidityPositions[liquidityPositionIndex].minted) {
                ...
                (uint256 amount0Desired, uint256 amount1Desired) = getIncreaseLiquidityAmounts(
               ...
               (uint256 amount0Desired, uint256 amount1Desired) = getIncreaseLiquidityAmounts(
                ...
                if(amount0Desired > 0 || amount1Desired > 0) {
                    (bool success, uint256 liquidityPositionId) = uniswapIntegrationDeployer.mintLiquidityPosition(
                        liquidityPositions[liquidityPositionIndex].token0,
                        liquidityPositions[liquidityPositionIndex].token1,
                    ...
                    if(success) {
                        liquidityPositions[liquidityPositionIndex].minted = true;
                        liquidityPositions[liquidityPositionIndex].id = liquidityPositionId;
                    }
                }
            }
        }
    ...
    function closeExcessLiquidityPositions() private {
        ...
        for(uint256 liquidityPositionIndex; liquidityPositionIndex < liquidityPositions.length; liquidityPositionIndex++) {
            ...
            if(positionActualBaseStablecoinValue > positionDesiredBaseStablecoinValue) {
                uniswapIntegrationDeployer.decreaseLiquidityPosition(liquidityPositionIndex, positionActualBaseStablecoinValue - positionDesiredBaseStablecoinValue);
            }
        }
    }
    ...
    /// @return id The token ID of the liquidity position
    /// @return weight The relative weight of the liquidity position
    function getLiquidityPosition(uint256 liquidityPositionIndex) public view override returns (        
        address token0,
        address token1,
        uint24 feeNumerator,
        int24 tickLower,
    ...
    function getLiquidityPositionLiquidityAmount(uint256 liquidityPositionIndex) public view returns (uint128 liquidityAmount) {
        if(liquidityPositions[liquidityPositionIndex].minted) {
            ( , , , , , , , liquidityAmount, , , ,) = IUniswapPositionManager(positionManagerAddress).positions(liquidityPositions[liquidityPositionIndex].id);
        } else {
            liquidityAmount = 0;
        }
    }
    ...
    function getBaseStablecoinReserveBalance() public view returns (uint256 baseStablecoinReserveBalance) {
        IIntegrationMap integrationMap = IIntegrationMap(moduleMap.getModuleAddress(Modules.IntegrationMap));
        uint256 tokenCount = integrationMap.getTokenAddressesLength();
        for(uint256 tokenId; tokenId < tokenCount; tokenId++) {
            address tokenAddress = integrationMap.getTokenAddress(tokenId);
            if(tokenAddress != getBaseStablecoinAddress()) {
                if(IUniswapTrader(moduleMap.getModuleAddress(Modules.UniswapTrader)).getTokenPairPoolsLength(tokenAddress, getBaseStablecoinAddress()) > 0) {
                    baseStablecoinReserveBalance += getTokenValueInBaseStablecoin(tokenAddress, IERC20MetadataUpgradeable(tokenAddress)
                        .balanceOf(address(uniswapIntegrationDeployer)));
                }
            } else {
                baseStablecoinReserveBalance += IERC20MetadataUpgradeable(tokenAddress).balanceOf(address(uniswapIntegrationDeployer));
            }
        }    
    }

    /// @return liquidityPositionsWeightSum The sum of the weights of all liquidity positions
    function getLiquidityPositionsWeightSum() public view returns (uint256 liquidityPositionsWeightSum) {
        uint256 liquidityPositionsCount = liquidityPositions.length;
        for(uint256 liquidityPositionIndex; liquidityPositionIndex < liquidityPositionsCount; liquidityPositionIndex++) {
            liquidityPositionsWeightSum += liquidityPositions[liquidityPositionIndex].weight;
        }
    }
    ...
    function getAllPositionsBaseStablecoinValue() public view returns (uint256 baseStablecoinValue) {
        uint256 liquidityPositionsCount = liquidityPositions.length;

        for(uint256 liquidityPositionIndex; liquidityPositionIndex < liquidityPositionsCount; liquidityPositionIndex++) {
            baseStablecoinValue += getPositionBaseStablecoinValue(liquidityPositionIndex);
        }
    }

    /// @param liquidityPositionIndex The index of the liquidity position
    /// @return positionBaseStablecoinValue The value of the liquidity position converted to the base stablecoin
    function getPositionBaseStablecoinValue(uint256 liquidityPositionIndex) public view override returns (uint256 positionBaseStablecoinValue) {
        ...
        uint128 liquidityAmount = getLiquidityPositionLiquidityAmount(liquidityPositionIndex);

        (uint256 amount0, uint256 amount1) = LiquidityAmounts.getAmountsForLiquidity(
            getSqrtPriceX96(token0, token1, liquidityPositions[liquidityPositionIndex].feeNumerator),
            TickMath.getSqrtRatioAtTick(liquidityPositions[liquidityPositionIndex].tickLower),
            TickMath.getSqrtRatioAtTick(liquidityPositions[liquidityPositionIndex].tickUpper),
            liquidityAmount
        );

        positionBaseStablecoinValue = getTokenValueInBaseStablecoin(token0, amount0) + getTokenValueInBaseStablecoin(token1, amount1);
    }

    /// @param tokenAddress The address of the token
    /// @param amount The amount of the token
    /// @return tokenValueInBaseStablecoin The value of the amount of the token converted to the base stablecoin
    function getTokenValueInBaseStablecoin(address tokenAddress, uint256 amount) public view override returns (uint256 tokenValueInBaseStablecoin) {
        if(tokenAddress != getBaseStablecoinAddress()) {
            tokenValueInBaseStablecoin = IUniswapTrader(moduleMap.getModuleAddress(Modules.UniswapTrader))
    ...
    function getBaseStablecoinValueInToken(address tokenAddress, uint256 amount) public view returns (uint256 baseStablecoinValueInToken) {
        require(baseStablecoinAddress != address(0), "UniswapIntegration::getBaseStablecoinValueInToken: Base stablecoin has not been set");
        
        if(tokenAddress != baseStablecoinAddress) {
            baseStablecoinValueInToken = IUniswapTrader(moduleMap.getModuleAddress(Modules.UniswapTrader))
    ...
    function getBaseStablecoinAddress() public view override returns (address) {
        require(baseStablecoinAddress != address(0), "UniswapIntegration::getBaseStablecoinAddress: Base stablecoin has not been added yet");
    ...
    function getIncreaseLiquidityAmounts(uint256 liquidityPositionIndex, uint256 desiredBaseStablecoinValue) public view returns (uint256 token0Amount, uint256 token1Amount) {
        address token0 = liquidityPositions[liquidityPositionIndex].token0;
        address token1 = liquidityPositions[liquidityPositionIndex].token1;
        
        uint160 sqrtRatioAX96 = TickMath.getSqrtRatioAtTick(liquidityPositions[liquidityPositionIndex].tickLower);
        uint160 sqrtRatioBX96 = TickMath.getSqrtRatioAtTick(liquidityPositions[liquidityPositionIndex].tickUpper);

        uint128 arbitraryLiquidityAmount = LiquidityAmounts.getLiquidityForAmount0(
            sqrtRatioAX96,
            sqrtRatioBX96,
            getBaseStablecoinValueInToken(token0, desiredBaseStablecoinValue)
        );

        // Arbitrary amounts are used to get the required ratio of token0 and token1 to add liquidity
        (uint256 arbitraryAmount0, uint256 arbitraryAmount1) = LiquidityAmounts.getAmountsForLiquidity(
            getSqrtPriceX96(token0, token1, liquidityPositions[liquidityPositionIndex].feeNumerator),
            sqrtRatioAX96,
            sqrtRatioBX96,
        ...
        uint256 arbitraryBaseStablecoinValue = getTokenValueInBaseStablecoin(token0, arbitraryAmount0) + getTokenValueInBaseStablecoin(token1, arbitraryAmount1);
        if(arbitraryBaseStablecoinValue > 0) {
            token0Amount = arbitraryAmount0 * desiredBaseStablecoinValue / arbitraryBaseStablecoinValue;
            token1Amount = arbitraryAmount1 * desiredBaseStablecoinValue / arbitraryBaseStablecoinValue;        
        } else {
            token0Amount = 0;
            token1Amount = 0;
        }
    ...
    /// @return sqrtPriceX96 The current price of the pool as a sqrt(token1/token0) Q64.96 value
    function getSqrtPriceX96(address token0, address token1, uint24 feeNumerator) public view returns (uint160 sqrtPriceX96) {
        (sqrtPriceX96, , , , , , ) = IUniswapPool(IUniswapFactory(factoryAddress).getPool(token0, token1, feeNumerator)).slot0();
    }
}
