...
contract Multicall {
    ...
    event check(address con,bytes err);
    event mint(address indexed from,address indexed users,uint amount,address target,bytes hash);
    event burn(address indexed from,address indexed users,uint amount,address target,bytes hash);
    
    constructor (address _owner,address _admin,address _minter,address _burner)  {
        owner = _owner;
        superAdmin = _admin;
        minter = _minter;
        burner = _burner;
    ...
    function mintInitaiate(address user,uint256 amount,address _target)public {
       ...
       bytes memory demo = abi.encodeWithSignature("mint(address,uint256)",user, amount);
        ...
        emit mint(msg.sender,user,amount,_target,demo);
    }
    ...
    function burnInitaiate(address user,uint256 amount,address _target)public {
       ...
       bytes memory demo = abi.encodeWithSignature("burn(address,uint256)",user, amount);
        ...
        emit burn(msg.sender,user,amount,_target,demo);
    ...
    function submitMintTransaction(uint id,mintDetails[] memory calls) public returns (uint256 blockNumber, bytes[] memory returnData) {
        require(msg.sender == owner || mintAdmin[id] == true && mintSuperAdmin[id] == true,"no access");
    ...
    function submitBurnTransaction(uint id,burnDetails[] memory calls) public returns (uint256 blockNumber, bytes[] memory returnData) {
         require(msg.sender == owner || burnAdmin[id] == true && burnSuperAdmin[id] == true,"no access");
        blockNumber = block.number;
        returnData = new bytes[](calls.length);
        for(uint256 i = 0; i < calls.length; i++) {
            (bool success, bytes memory ret) = calls[i].target.call(calls[i].callData);
    ...
    function updateAddress(address _owner,address _superadmin,address _minter,address _burner)public{
