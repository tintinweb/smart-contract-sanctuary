...
interface IPortal {
    ...
    function updateAccountEscrowedLockedAmount(address account, uint amount, bool add, bool sub) external;
}
...
contract RewardEscrowUpgradeable is OwnableUpgradeable, CacheResolverUpgradeable {
    ...
    function appendEscrowEntry(address account, uint amount, uint duration) external onlyInternalContract {
        ...
        uint accountLastEntryId = accountEscrowedEntries[account][accountEscrowedEntries[account].length - 1];
        require(block.timestamp > accountEscrowedEvent[account][accountLastEntryId].escrowedEndTime, "start time of new entry must excceed the last escrow entry of user");
