...
contract DexProxy is Ownable {
    ...
    /**
     * @dev Contract owner fee in range 1 to feeDivisor (1 means 1/feeDivisor), so range is 0.00001 (0.001%) to 1
     */
    uint providerFee;
    ...
    constructor(uint _promoterFee, uint _providerFee, address _providerFeeTarget, uint[] memory _availableFeeValues, address[] memory _dexes) {
        _setPromoterFee(_promoterFee);
        _setProviderFee(_providerFee);
    ...
    function sendReceivedTokens(address toToken, uint tokensReceived, FeeInfo calldata feeInfo) private {
        uint integratorAmount = tokensReceived * feeInfo.fee / feeDivisor;
        uint promoterAmount = tokensReceived * promoterFee / feeDivisor;
        uint providerAmount = tokensReceived * providerFee / feeDivisor;
        uint userAmount = tokensReceived = integratorAmount - promoterAmount - providerAmount;

        if (toToken == address(0)) {
            transferNative(integratorAmount, promoterAmount, providerAmount, userAmount, feeInfo);
        } else {
            transferErc20(IERC20(toToken), integratorAmount, promoterAmount, providerAmount, userAmount, feeInfo);
        }
    }

    function transferErc20(
        IERC20 toToken,
        uint integratorAmount,
        uint promoterAmount,
        uint providerAmount,
        uint userAmount,
        FeeInfo calldata feeInfo
    ) private {
        toToken.safeTransfer(feeInfo.feeTarget, integratorAmount);
        toToken.safeTransfer(feeInfo.promoter, promoterAmount);
        toToken.safeTransfer(providerFeeTarget, providerAmount);
        toToken.safeTransfer(_msgSender(), userAmount);
    }
    ...
    function transferNative(
        ...
        payable(feeInfo.feeTarget).transfer(integratorAmount);
        payable(feeInfo.promoter).transfer(promoterAmount);
        payable(providerFeeTarget).transfer(providerAmount);
        payable(_msgSender()).transfer(userAmount);
    }
    ...
    function setProviderFee(uint _providerFee) external onlyOwner {
        _setProviderFee(_providerFee);
    }

    function _setProviderFee(uint _providerFee) private {
        require(_providerFee <= feeDivisor, 'Fee can not be grow than feeDivisor');
