...
contract Oracle is IOracle, Ownable
{
  ...
  function getConversionRate (address from, address to) public view override returns (uint256)
  {
    if (from == to && to == _DAI)
    {
      return _DECIMALS;
    ...
    // fromToken rate / toToken rate
    return ( (from == _WETH ? _DECIMALS : _calcTokenETHRate(from)) * _DECIMALS ) / (to == _WETH ? _DECIMALS : _calcTokenETHRate(to));
  }

  function _convertToDestToken (uint256 amountInSrcToken, address srcToken, address destToken, uint256 conversionRate) private view returns (uint256)
  {
    uint256 srcDecimals = _decimal[srcToken];
    uint256 destDecimals = _decimal[destToken];


    if (destDecimals >= srcDecimals)
    {
      return (amountInSrcToken * conversionRate * (10 ** (destDecimals - srcDecimals))) / _DECIMALS;
    }
    else
    {
      return (amountInSrcToken * conversionRate) / (_DECIMALS * (10 ** (srcDecimals - destDecimals)));
    }
  }

  function convertFromUSD (address to, uint256 amount) external view override returns (uint256)
  {
    return _convertToDestToken(amount, _DAI, to, getConversionRate(_DAI, to));
  }

  function convertToUSD (address from, uint256 amount) external view override returns (uint256)
  {
    return _convertToDestToken(amount, from, _DAI, getConversionRate(from, _DAI));
  }

  function convert (address from, address to, uint256 amount) external view override returns (uint256)
  {
    return _convertToDestToken(amount, from, to, getConversionRate(from, to));
  }
}
