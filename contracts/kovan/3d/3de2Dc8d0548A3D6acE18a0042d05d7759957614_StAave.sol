 ...
 */
contract StAave is Initializable, OwnableUpgradeable, LSTokenGovPowerSnapshot, IStAave {
    ...
    function initialize(
        ...
        require(_aave != address(0), "aave cannot be zero");
        require(_stkAave != address(0), "stkAave cannot be zero");
        require(_aaveGovernance != address(0), "aaveGovernance cannot be zero");
        require(_lidoAgent != address(0), "lidoAgent cannot be zero");
        require(_delphiAgent != address(0), "delphiAgent cannot be zero");

        __Context_init_unchained();
        __Ownable_init_unchained();
        __LSTokenGovPowerSnapshot_init(_name, _symbol, 18);
    ...
    /**
     * @dev Receive AAVE from user, stake to Aave staking contract, mint stAAVE to user.
     * @dev User must have approved the contract to spend his AAVE.
     */
    ...
    /**
     * @dev Activate cooldown in order to unstake AAVE.
     */
    ...
    /**
     * @dev Redeem AAVE from staking contract, burn stAAVE, refund AAVE to user. Take in
     * amount as argument.
     */
    function redeem(uint256 amount) external override {
        require(amount > 0, "Amount must be greater than zero");
        require(amount <= balanceOf(msg.sender), "Cannot redeem more than user balance");

        _redeem(amount, _getSharesByUnderlying(amount));
    }
     ...
     * shares as argument.
     */
    function redeemShares(uint256 shares) public override {
        require(shares > 0, "Shares must be greater than zero");
        require(shares <= shareOf(msg.sender), "Cannot redeem more than user shares");

        _redeem(_getUnderlyingByShares(shares), shares);
    }
    ...
    /**
     * @dev Distribute fees to recipients. 20% goes to Delphi and the rest to Lido.
     */
    function withdrawFees() external override {
        aave.safeTransfer(lidoAgent, collectibleFeeLido);
        aave.safeTransfer(delphiAgent, collectibleFeeDelphi);
     ...
     * @dev Fee rate can be no more than 10% (1000 basis points)
    ...
    /**
     * @dev Update the address of a fee recipient
     */
    function setFeeRecipient(address newRecipient) external override {
        require(newRecipient != address(0), "Cannot set to zero address");

        if (msg.sender == lidoAgent) {
            lidoAgent = newRecipient;
        } else if (msg.sender == delphiAgent) {
            delphiAgent = newRecipient;
        } else {
            revert("Caller is not a current fee recipient");
        }

        require(lidoAgent != delphiAgent, "Fee recipients cannot be the same");

        emit FeeRecipientSet(lidoAgent, delphiAgent);
    }
    ...
    function _redeem(uint256 amount, uint256 shares) private {
        ...
        returns (uint256 rewardsAfterFee, uint256 fee)
    {
        ...
        rewardsAfterFee = rewards.sub(unsavedFee);
    }

    function _setFeeRate(uint256 _feeRate) private {
        require(_feeRate <= 1000, "Fee rate is too high");
    ...
    function _incrementFees(uint256 fee) private {
        uint256 toDelphi = fee.div(5);
        uint256 toLido = fee.sub(toDelphi);

        collectibleFeeLido = collectibleFeeLido.add(toLido);
        collectibleFeeDelphi = collectibleFeeDelphi.add(toDelphi);
    }
}
...
interface IStAave {
    ...
    event FeeWithdarwn(uint256 toLido, uint256 toDelphi);
    event FeeRateSet(uint256 feeRate);
    event FeeRecipientSet(address lidoAgent, address delphiAgent);
}
    ...
    IGovernancePowerDelegationToken
{
    ...
    /**
     * @dev Returns the governance power of a user at a certain block
     * @dev Note: the function takes `delegationType` argument as required by Aave's
     * `IGovernancePowerDelegationToken` interface. However, StAave does not in fact
     * implement delegation. Therefore, we return the same number regardless of which
     * delegation type is being queried.
    ...
    function getPowerAtBlock(
        ...
        // Use two underscores here so that the variable does not shadow the public
        // function that goes by the same name
        ...
        if (totalStaked == 0 || __totalShares == 0 || userShares == 0) {
            return 0;
        } else {
            return totalStaked.mul(userShares).div(__totalShares);
        }
    }
...
abstract contract LSToken is Initializable, ILSToken {
    ...
    function transferFrom(
        ...
        _approve(from, to, _allowances[from][msg.sender].sub(amount));
        return true;
    }

    /**
     * @dev In addition to the standard `approve` method, we implemented `increaseAllowance`
     * and `decreaesAllowance` here, as `approve` is known to be susceptible to an attack
     * which allows the spender to spend both the old and new allowances:
     *
     * https://docs.google.com/document/d/1YLPtQxZu1UAvO9cZ1O2RPXBbT0mooh4DYKjA_jp-RLM
     *
     * Instead of using `approve`, it is recommended to decrease allowance to zero, then
     * increase it to the desired amount.
     */
    ...
    function _getUnderlyingByShares(uint256 shares) internal view returns (uint256) {
        require(_totalShares > 0, "Contract has no outstanding shares");
     ...
     * underlying asset.
     * @dev For the first ever deposit, we initialize 1 underlying = 1e32 shares.
     */
