...
contract FarmPool is ReentrancyGuard {
    ...
    struct FarmPoolInfo {
        ...
        uint256 startBlock;
        uint256 endBlock;
        uint256 rewardsDuration; // duration = start - block + 1
        uint256 rewardRatio;
    ...
    /* ========== EVENTS ========== */
    event LogSetOwner(address indexed owner, address indexed _owner);
    ...
    event LogSetProposal(uint256 indexed farmId, string indexed proposalId);
    ...
    constructor(address _register) public {
        ...
        owner = msg.sender;
        emit LogSetOwner(address(0), owner);
    }
    ...
    function createPool(
        ...
        poolR.totalSupply = poolR.totalSupply.add(stakeAmount);
        poolR.balances[_creator] = stakeAmount;
        // poolR.stakingToken.safeTransferFrom(msg.sender, address(this), 100*10**18);

        emit LogStaked(poolId, _creator, stakeAmount);
    }
    ...
    function getFarmInfo(uint256 farmId)
        external
        view
        returns (
    ...
    {
        FarmPoolInfo memory pool = pools[farmId];
    ...
    function setOwner(address _addr) external onlyOwner {
        require(_addr != address(0), "ERR_ZERO_ADDR");
        emit LogSetOwner(owner, _addr);
        owner = _addr;
    }

    function setProposalId(uint256 farmId, string calldata proposalId)
        external
        onlyOwner
    {
        FarmPoolInfo storage pool = pools[farmId];
        pool.proposalId = proposalId;
        emit LogSetProposal(farmId, proposalId);
    }
}