 ...
 */
contract Fantopia is Ownable, SignerRole, IERC721, IERC721Metadata, ERC721Burnable, ERC721Base {
    uint256 baseRoyaltyFee;
    event RoyaltyFeeUpdated(uint256 fee);

    constructor (string memory name, string memory symbol, address newOwner, address signer, string memory contractURI, string memory tokenURIPrefix, uint256 royaltyFee) public ERC721Base(name, symbol, contractURI, tokenURIPrefix) {
        if(signer != address(0))_addSigner(signer);
        setBaseRoyaltyFee(royaltyFee);
        _registerInterface(bytes4(keccak256('MINT_WITH_ADDRESS')));
        transferOwnership(newOwner);
    }

    function mint(uint256 tokenId, uint8 v, bytes32 r, bytes32 s, Fee[] memory _fees, string memory tokenURI) public {
        require(isSigner(ecrecover(keccak256(abi.encodePacked(this, tokenId)), v, r, s)), "owner should sign tokenId & fees");
        if(baseRoyaltyFee != 0)
            _fees[_fees.length] = Fee(address(uint160(owner())),baseRoyaltyFee);
    ...
    function setBaseRoyaltyFee(uint256 fee) public onlyOwner {
        baseRoyaltyFee = fee;
        emit RoyaltyFeeUpdated(fee);
    }
}
