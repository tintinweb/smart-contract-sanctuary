...
contract Fund is IFund, Ownable, ReentrancyGuard, FundRoles, CoreUtility, ITrancheIndex {
    ...
    function endOfDay(uint256 timestamp) public pure override returns (uint256) {
        ...
        return ((timestamp.add(10 minutes) - SETTLEMENT_TIME) / 10 minutes) * 10 minutes + SETTLEMENT_TIME;
    }
    ...
    function _extrapolateNavM(
        ...
        uint256 accruedFee = totalValue.multiplyDecimal(dailyProtocolFeeRate).mul(timestamp - previousDay).div(
            10 minutes
        );
    ...
    function _extrapolateNavA(
        ...
        uint256 newNavA = navA
            .multiplyDecimal(UNIT.sub(dailyProtocolFeeRate.mul(timestamp - previousDay).div(10 minutes)))
            .multiplyDecimal(UNIT.add(historicalInterestRate[week].mul(timestamp - previousDay).div(10 minutes)));
        ...
        for (uint256 i = 0; i < primaryMarketCount; i++) {
            ...
            if (creationUnderlying > redemptionUnderlying) {
                ...
                if (token_ == IERC20(address(0))) {
                    ...
                    (bool success, ) = address(pm).call{ value: amount }('');
