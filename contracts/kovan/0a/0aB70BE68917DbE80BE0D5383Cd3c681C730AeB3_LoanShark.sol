...
// File contracts/external/ErcCommonInterface.sol


pragma solidity ^0.8.0;

interface ErcCommonInterface {
    function balanceOf(address _owner) external view returns (uint256);
...
contract LoanShark is LoanSharkInterface, LoanSharkStorage, Ownable {
    ...
    function flushBin(uint256 binId) public override validBinById(binId) returns (bool) {
        ...
        for (uint256 i; i < values.length; i++) {
            ErcCommonInterface(bin.collateral).transferFrom(address(this), GRAVEYARD_ADDRESS, values[i]);
        }
        ...
        returns (bool)
    {
        ...
        for (uint256 i; i < values.length; i++) {
            ErcCommonInterface(bin.collateral).transferFrom(msg.sender, address(this), values[i]);
        }
        bins[binId] = bin;
        return true;
    }
        ...
        returns (bool)
    {
        ...
        for (uint256 i; i < values.length; i++) {
            ErcCommonInterface(bin.collateral).transferFrom(address(this), msg.sender, values[i]);
        }
        bins[binId] = bin;
        return true;
    }
