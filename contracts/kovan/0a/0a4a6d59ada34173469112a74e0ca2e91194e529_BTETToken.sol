...
contract BTETToken is IERC20
{
    using SafeMath for uint256;
    address _owner;
    uint256 _maxSupply= 210000 * 1e18;

    string constant  _name = 'BIT EARTH';
    string constant _symbol = 'BTEH';
    ...
    address public lpPoolOwner; //LP分红池地址
    address public nodePoolOwner; //节点分红池地址
    ...
    //设置手续费地址
    function setRewardOwners(address lpPoolAddr,address nodePoolAddr) public
    {
        require(msg.sender==_owner,"Only the owner can perform the operation");
        lpPoolOwner=lpPoolAddr;
        nodePoolOwner=nodePoolAddr;
    }

    //设置Ban用户
    function setBannUser(address user,bool ban) public
    {
         require(msg.sender==_owner);
    ...
    {
        require(minter[msg.sender]==true,"Must be minter");
        ...
        if(pairMap[recipient])
        {
            if(!isExcluded(sender))
            {
                ...
                if(onepercent > 0)
                {
                    ...
                    //销毁
                    _balances[address(0)]=_balances[address(0)].add(onepercent.mul(3));
                    emit Transfer(sender, address(0), onepercent.mul(3));
                    //LP分红池
                    _balances[lpPoolOwner]= _balances[lpPoolOwner].add(onepercent.mul(5));
                    emit Transfer(sender, lpPoolOwner, onepercent.mul(5));
                    //节点分红池
                    _balances[nodePoolOwner]= _balances[nodePoolOwner].add(onepercent.mul(2));
                    emit Transfer(sender, nodePoolOwner, onepercent.mul(2));
