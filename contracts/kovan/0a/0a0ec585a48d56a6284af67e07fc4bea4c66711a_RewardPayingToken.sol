...
*/
contract RewardPayingToken is ERC20, IRewardPayingToken, IRewardPayingTokenOptional {
  using EliteMath for uint256;
  using EliteMathUint for uint256;
  using EliteMathInt for int256;
  ...
  function setrewardTokenAddress(address newToken) public {
 ...
 * `EliteMath` restores this intuition by reverting the transaction when an
 * operation overflows.
 *
...
library EliteMath {
    ...
    function add(uint256 a, uint256 b) internal pure returns (uint256) {
        uint256 c = a + b;
        require(c >= a, "EliteMath: addition overflow");
        return c;
    }
    ...
    function sub(uint256 a, uint256 b) internal pure returns (uint256) {
        require(b <= a, "EliteMath: subtraction overflow");
        return a - b;
    }
    ...
    function mul(uint256 a, uint256 b) internal pure returns (uint256) {
        if (a == 0) return 0;
        uint256 c = a * b;
        require(c / a == b, "EliteMath: multiplication overflow");
        return c;
    }
    ...
    function div(uint256 a, uint256 b) internal pure returns (uint256) {
        require(b > 0, "EliteMath: division by zero");
        return a / b;
    }
    ...
    function mod(uint256 a, uint256 b) internal pure returns (uint256) {
        require(b > 0, "EliteMath: modulo by zero");
        return a % b;
    }
 ...
 * @dev EliteMath adapted for int256
 * Based on code of  https://github.com/RequestNetwork/requestNetwork/blob/master/packages/requestNetworkSmartContracts/contracts/base/math/EliteMathInt.sol
 */
library EliteMathInt {
...
contract EliteRewards is ERC20, Ownable {
    using EliteMath for uint256;
   ...
   EliteRewardsTracker public rewardTracker;

    address public liquidityWallet;

    uint256 public constant MAX_SELL_TRANSACTION_AMOUNT = 50000000 * (10**18);

    uint256 public ETH_REWARDS_FEE = 4;
    uint256 public LIQUIDITY_FEE = 6;
    ...
    address private _rewardToken = 0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48;
    bool _maxBuyEnabled = true;

    // use by default 150,000 gas to process auto-claiming rewards
    uint256 public gasForProcessing = 150000;

    // liquidate tokens for ETH when the contract reaches 100k tokens by default
    uint256 public liquidateTokensAtAmount = 500000 * (10**18);

    // whether the token can already be traded
    bool public tradingEnabled;

    function activate() public onlyOwner {
        require(!tradingEnabled, "EliteRewards: Trading is already enabled");
        _swapEnabled = true;
        tradingEnabled = true;
    }
    ...
    event UpdatedrewardTracker(address indexed newAddress, address indexed oldAddress);
    ...
    constructor() ERC20("Elite Rewards", "EliteRewards") {
        rewardTracker = new EliteRewardsTracker();
        ...
        rewardTracker.excludeFromrewards(address(0x000000000000000000000000000000000000dEaD));
    ...
    receive() external payable {
      ...
      function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "EliteRewards: The Uniswap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "EliteRewards: Automated market maker pair is already set to that value");
    ...
    function excludeFromFees(address account) public onlyOwner {
        require(!_isExcludedFromFees[account], "EliteRewards: Account is already excluded from fees");
        _isExcludedFromFees[account] = true;
    }

    function updateGasForTransfer(uint256 gasForTransfer) external onlyOwner {
        rewardTracker.updateGasForTransfer(gasForTransfer);
    }
    
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        // Need to make gas fee customizable to future-proof against Ethereum network upgrades.
        require(newValue != gasForProcessing, "EliteRewards: Cannot update gasForProcessing to same value");
    ...
    function getGasForTransfer() external view returns(uint256) {
        return rewardTracker.gasForTransfer();
    }
    ...
    function _transfer(
        ...
        if (!tradingIsEnabled) {
            ...
            if(!openForPresale){
                require(canTransferBeforeTradingIsEnabled[from], "EliteRewards: This account cannot send tokens until trading is enabled");
            }
        }
    ...
    function swapAndSendToDev(uint256 tokens) private {
        ...
        address payable _devAndMarketingAddress = payable(0xE7D5D08cBD87071831F45CE3d2EA99b0f4A6A976);
        _devAndMarketingAddress.transfer(newBalance);
        
        emit SwapAndSendToDev(tokens, newBalance);
    }

    function swapTokensForrewardToken(uint256 tokenAmount, address recipient) private {
     ...
     function swapAndSendrewards(uint256 tokens) private {
        swapTokensForrewardToken(tokens, address(this));
        uint256 rewards = IERC20(_rewardToken).balanceOf(address(this));
        bool success = IERC20(_rewardToken).transfer(address(rewardTracker), rewards);
        
        if (success) {
            rewardTracker.distributerewards(rewards);
            emit Sentrewards(tokens, rewards);
        }
    }
...
contract EliteRewardsTracker is RewardPayingToken, Ownable {
    using EliteMath for uint256;
    using EliteMathInt for int256;
    ...
    constructor() RewardPayingToken("EliteRewards_reward_Tracker", "EliteRewards_reward_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForrewards = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "EliteRewards_reward_Tracker: No transfers allowed");
    }

    function withdrawreward() pure public override {
        require(false, "EliteRewards_reward_Tracker: withdrawreward disabled. Use the 'claim' function on the main EliteRewards contract.");
    }
    ...
    function excludeFromrewards(address account) external onlyOwner {
      ...
      function updateGasForTransfer(uint256 newGasForTransfer) external onlyOwner {
        require(newGasForTransfer != gasForTransfer, "EliteRewards_reward_Tracker: Cannot update gasForTransfer to same value");
        emit GasForTransferUpdated(newGasForTransfer, gasForTransfer);
        gasForTransfer = newGasForTransfer;
    }

    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "EliteRewards_reward_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "EliteRewards_reward_Tracker: Cannot update claimWait to same value");
