...
contract APIConsumer is ChainlinkClient, ConfirmedOwner {
    ...
    // A withdraw function to avoid locking LINK in the contract
    function withdrawLink() external {
        LinkTokenInterface token = LinkTokenInterface(chainlinkTokenAddress());
    ...
    function requestBytes32Data(
        ...
        Chainlink.Request memory request = buildChainlinkRequest(jobId, address(this), this.fulfillBytes32.selector);
        request.add("get", _getURL);
        request.add("path", _path);
        return sendChainlinkRequestTo(oracle, request, fee);
    }
    ...
    /// Receives the response in the form of int256
    function fulfillInt(bytes32 _requestId, int256 _value) public recordChainlinkFulfillment(_requestId) {
        intValue = _value;
    }

    /// Receives the response in the form of bytes32
    function fulfillBytes32(bytes32 _requestId, bytes32 _value) public recordChainlinkFulfillment(_requestId) {
        stringValue = bytes32ToString(_value);
    }
