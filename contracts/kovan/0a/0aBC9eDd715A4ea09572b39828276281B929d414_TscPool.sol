...
contract TSC is TerminateContractTemplate {
    ...
    function setExpiration() external virtual {
        revert();
    }
    ...
    receive() external payable onlyPartner isLive onlyStartTimming {
        ...
        while (total > 0 && i < listDepositETH.size) {
            if (
                ...
                uint256 remain = listDepositETH.list[i].value -
                    listDepositETH.list[i].deposited;
                if (total > remain) {
                    total -= remain;
                    listDepositETH.list[i].deposited = listDepositETH
                        .list[i]
                        .value;
                    passCount++;
