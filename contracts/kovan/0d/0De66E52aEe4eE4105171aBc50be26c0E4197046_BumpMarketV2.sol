    ...
    BumperAccessControl
{
    ...
    modifier lockWithdrawal {
        require(
            block.timestamp >= BUSDCV2(busdcTokenAddress).unlockTimestamp(),
        ...
        returns (bool)
    {
        ...
        BUSDCV2(busdcTokenAddress).burn(msg.sender, _amount);

        (bumpTokensAsRewards, bumpTokensPurchased) = getBumpAllocation(0, _amount);
    ...
    function depositAmount(
        ...
        _depositAmount(_amount, _amountForBumpPurchase, _coin);
    }
    ...
    function updateSwapAllowedPeriod(uint256 _start, uint256 _end)
        ...
        onlyGovernance
    {
        require(_end >= _start, "The end of the period must be larger than the start");
        swapAllowedPeriod = TimePeriod(_start, _end);
        emit UpdatedSwapAllowedPeriod(
            msg.sender,
            _start,
            _end
        );
    ...
    function estimateBumpRewards(
        ...
        if (block.timestamp >= BUSDCV2(busdcTokenAddress).unlockTimestamp()) {
            return 0;
        }
    ...
    {
        require(
            BUSDCV2(busdcTokenAddress).balanceOf(msg.sender) >= amount,
        ...
        returns (bool)
    {
        ...
        require(
            BUSDCV2(busdcTokenAddress).balanceOf(receiver) >= amount,
            "User doesn't have enough bUSDC to burn"
        );
        ...
        returns (bool)
    {
        ...
        BUSDCV2(busdcTokenAddress).burn(receiver, amount);
...
// SPDX-License-Identifier: AGPL-3.0-or-later
// This version is deprecated please refer to BUSDCV2.sol.
