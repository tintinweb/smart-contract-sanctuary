...
contract Loot is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function getCHA(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "CHA", charisma);
    }

    function getCON(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "CON", constitution);
    }
    ...
    function getWIS(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "WIS", wisdom);
    }
        ...
        returns (string memory)
    {
        ...
        parts[
            0
        ] = '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350"><style>.base {  font-family: serif; font-size: 14px; }</style><rect width="100%" height="100%" fill="black" /><text x="10" y="20" class="base">';

        parts[1] = getNameLabel(tokenId);

        parts[2] = '</text><text x="10" y="40" class="base" fill="white">';

        parts[3] = getRace(tokenId);

        parts[4] = '</text><text x="10" y="60" class="base" fill="white">';
            ...
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "',
                        getName(tokenId),
                        '", "description": "Mudverse is randomized adventurer gear generated and stored on chain. Stats, images, and other functionality are intentionally omitted for others to interpret. Feel free to use Mudverse in any way you want.", "image": "data:image/svg+xml;base64,',
