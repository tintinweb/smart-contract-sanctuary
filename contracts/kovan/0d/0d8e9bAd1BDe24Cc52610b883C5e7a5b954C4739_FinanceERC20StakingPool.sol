...
import "./lib/IERC20.sol";
import "./FinanceLPBalanceToken.sol";




contract FinanceERC20StakingPool is IFinancePool {
    ...
    address public DEK_address;          // DEK token
    ...
    uint256[] public DEK_earned;                // Total DEK earned (minted at wind_down_epoch)
    ...
    /**** SAFFRON LP TOKENS ****/
    // If we just have a token address then we can look up epoch and tranche balance tokens using a mapping(address=>FinanceV1dsecInfo)
    // LP tokens are dsec (redeemable for interest+DEK) and principal (redeemable for base asset) tokens
    struct FinanceLPTokenInfo {
        bool        exists;
        uint256     epoch;
        LPTokenType token_type;
    }

    mapping(address=>FinanceLPTokenInfo) public finance_LP_token_info;

    constructor(address _strategy, address _base_asset, address _DEK_address, bool epoch_cycle_reset) {
    ...
    function new_epoch(uint256 epoch, address finance_LP_dsec_token_address, address finance_LP_principal_token_address) public {
        ...
        dsec_token_addresses.push(finance_LP_dsec_token_address);
        principal_token_addresses.push(finance_LP_principal_token_address);

        // Token info for looking up epoch and tranche of dsec tokens by token contract address
        finance_LP_token_info[finance_LP_dsec_token_address] = FinanceLPTokenInfo({
        exists: true,
        epoch: epoch,
        ...
        finance_LP_token_info[finance_LP_principal_token_address] = FinanceLPTokenInfo({
        exists: true,
        epoch: epoch,
    ...
    function add_liquidity(uint256 amount, Tranche tranche) external {
        ...
        require(epoch == 0, "v1.0: must be epoch 0 only");
        ...
        // Mint Finance LP epoch 1 <base_asset_name> dsec tokens and transfer them to sender
        FinanceLPBalanceToken(dsec_token_addresses[epoch]).mint(msg.sender, dsec);

        // Mint Finance LP epoch 1 <base_asset_name> principal tokens and transfer them to sender
        FinanceLPBalanceToken(principal_token_addresses[epoch]).mint(msg.sender, amount);
    ...
    event WindDownEpochState(uint256 previous_epoch, uint256 DEK_earned, uint256 epoch_dsec);
    function wind_down_epoch(uint256 epoch, uint256 amount_dek) public override {
        ...
        DEK_earned[epoch] = amount_dek;

        // Total dsec
        uint256 epoch_dsec = total_dsec[epoch];
        epoch_wound_down[epoch] = true;
        emit WindDownEpochState(previous_epoch, DEK_earned[epoch], epoch_dsec);
    }

    event RemoveLiquidityDsec(uint256 dsec_percent, uint256 DEK_owned);
    event RemoveLiquidityPrincipal(uint256 principal);
    function remove_liquidity(address dsec_token_address, uint256 dsec_amount, address principal_token_address, uint256 principal_amount) external {
        ...
        if (dsec_token_address != address(0x0) && dsec_amount > 0) {
            ...
            FinanceLPTokenInfo memory token_info = finance_LP_token_info[dsec_token_address];
            require(token_info.exists, "balance token lookup failed");
            FinanceLPBalanceToken sbt = FinanceLPBalanceToken(dsec_token_address);
            ...
            require(token_epoch == 0, "v1.0: bal token epoch must be 0");
            require(epoch_wound_down[token_epoch], "can't remove from wound up epoch");

            // Dsec gives user claim over a tranche's earned DEK and interest
            dsec_percent = dsec_amount.mul(1 ether).div(total_dsec[token_epoch]);
            DEK_owned = DEK_earned[token_epoch].mul(dsec_percent) / 1 ether;
            DEK_earned[token_epoch] = DEK_earned[token_epoch].sub(DEK_owned);
        ...
        if (principal_token_address != address(0x0) && principal_amount > 0) {
            // Get info about the v1 dsec token from its address and check that it exists
            FinanceLPTokenInfo memory token_info = finance_LP_token_info[principal_token_address];
            require(token_info.exists, "balance token info lookup failed");
            FinanceLPBalanceToken sbt = FinanceLPBalanceToken(principal_token_address);
        ...
        if (dsec_token_address != address(0x0) && dsec_amount > 0) {
            ...
            IERC20(DEK_address).safeTransfer(msg.sender, DEK_owned);
            emit RemoveLiquidityDsec(dsec_percent, DEK_owned);
        }
        if (principal_token_address != address(0x0) && principal_amount > 0) {
            FinanceLPBalanceToken sbt = FinanceLPBalanceToken(principal_token_address);
            require(sbt.balanceOf(msg.sender) >= principal_amount, "insufficient principal balance");
...
contract FinanceLPBalanceToken is ERC20 {
    address public pool_address;

    constructor (string memory name, string memory symbol) ERC20(name, symbol) {
        // Set pool_address to Finance pool that created token
