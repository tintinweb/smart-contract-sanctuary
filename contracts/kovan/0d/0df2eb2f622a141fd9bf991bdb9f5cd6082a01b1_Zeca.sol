...
contract Zeca is BEP20, Owned {
    ...
    *
    ...
    function shareFile(uint _fileId, address _receiver, uint _amount, uint stakeType_) external returns(bool) {
    ...
    //Empty the members of a particular FileID
    function deleteFile(uint _fileId) external returns(bool) {
        //Require FileID is not 0 and it exists
        require(_fileId != 0, "Zeca: FileID Invalid");
        require(starred[_msgSender()][_fileId] || !starred[_msgSender()][_fileId],"Zeca: FileID does not exist");
