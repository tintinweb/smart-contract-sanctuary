...
contract Publ {
    ...
    string public constant name = "Publics";
    ...
    uint256 public constant totalSupply = 10000000e18; // 10 million Publ
    ...
    /**
     * @notice Construct a new Publ token
    ...
    function approve(address spender, uint256 rawAmount) external returns (bool) {
        ...
        if (rawAmount == uint256(-1)) {
            ...
            amount = safe96(rawAmount, "Publ::approve: amount exceeds 96 bits");
        }
    ...
    function transfer(address dst, uint256 rawAmount) external returns (bool) {
        uint96 amount = safe96(rawAmount, "Publ::transfer: amount exceeds 96 bits");
    ...
    function transferFrom(
        ...
        uint96 amount = safe96(rawAmount, "Publ::approve: amount exceeds 96 bits");

        if (spender != src && spenderAllowance != uint96(-1)) {
            uint96 newAllowance = sub96(spenderAllowance, amount, "Publ::transferFrom: transfer amount exceeds spender allowance");
    ...
    function delegateBySig(
        ...
        require(signatory != address(0), "Publ::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "Publ::delegateBySig: invalid nonce");
        require(now <= expiry, "Publ::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint256 blockNumber) public view returns (uint96) {
        require(blockNumber < block.number, "Publ::getPriorVotes: not yet determined");
    ...
    function _transferTokens(
        ...
        require(src != address(0), "Publ::_transferTokens: cannot transfer from the zero address");
        require(dst != address(0), "Publ::_transferTokens: cannot transfer to the zero address");

        balances[src] = sub96(balances[src], amount, "Publ::_transferTokens: transfer amount exceeds balance");
        balances[dst] = add96(balances[dst], amount, "Publ::_transferTokens: transfer amount overflows");
    ...
    function _moveDelegates(
        ...
        if (srcRep != dstRep && amount > 0) {
            if (srcRep != address(0)) {
                ...
                uint96 srcRepNew = sub96(srcRepOld, amount, "Publ::_moveVotes: vote amount underflows");
            ...
            if (dstRep != address(0)) {
                ...
                uint96 dstRepNew = add96(dstRepOld, amount, "Publ::_moveVotes: vote amount overflows");
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(block.number, "Publ::_writeCheckpoint: block number exceeds 32 bits");
...
contract PTokenStorage {
    ...
    mapping(address => BorrowSnapshot) internal accountBorrowsCreditLoan;
...
contract PTokenInterface is PTokenStorage {
     ...
     */
    event Borrow(address borrower, uint256 borrowAmount, uint256 accountBorrows, uint256 totalBorrows, bool isCreditLoan);

    /**
     * @notice Event emitted when a borrow is repaid
     */
    event RepayBorrow(address payer, address borrower, uint256 repayAmount, uint256 accountBorrows, uint256 totalBorrows, bool isCreditLoan);
    ...
    function getAccountSnapshot(address account, bool isCreditLoan)
        external
        view
        returns (
            uint256,
...
contract PToken is PTokenInterface, Exponential, TokenErrorReporter {
     ...
     * @return Whether or not the transfer succeeded
     */
    //spender是被授权者，可以花费src的钱，有allowance限额
     ...
     */
    //获取当前账户金额的快照，给comptroller使用，便于检查流动性的
    //TODO getAccountSnapshot 还需要看看
    function getAccountSnapshot(address account, bool isCreditLoan)
        external
        view
        returns (
        ...
        )
    {
        ...
        //返回的是account借币数量
        (mErr, borrowBalance) = borrowBalanceStoredInternal(account, isCreditLoan);
        ...
        //返回pToken的数量，借款数量，exchangeRate（这个数据是变化的，初始2000000000000.。。，现在200000103810.。。）
    ...
    function borrowBalanceCurrent(address account) external nonReentrant returns (uint256) {
        //看到产生利息了：
        // borrowBalanceStored: 99082789,这是借的数量
        // borrowBalanceCurrent : 99095615,这是当前要还的总额
        // accrueInterest()
        require(accrueInterest() == uint256(Error.NO_ERROR), "accrue interest failed");
     ...
     */
    function borrowBalanceStored(address account) public view returns (uint256) {
        (MathError err, uint256 result) = borrowBalanceStoredInternal(account, false);
     ...
     */
    function borrowBalanceStoredInternal(address account, bool isCreditLoan) internal view returns (MathError, uint256) {
        ...
        BorrowSnapshot storage borrowSnapshot = isCreditLoan ? accountBorrowsCreditLoan[account] : accountBorrows[account];
         ...
         */
        // TODO:这是什么意思 borrowSnapshot.principal * borrowIndex / borrowSnapshot.interestIndex
     ...
     */
    //根据当前的存储数量变动，计算PToken和underlying的兑换比率, PToken随着block的增加，会兑换更多的underlying
    ...
    function accrueInterest() public returns (uint256) {
        ...
        //accrualBlockNumber 是在部署的时候initialize时，通过block.number获取,
        //accrualBlockNumber 会在当前函数最后更新为 currentBlockNumber
    ...
    function redeemFresh(
        ...
        //如果当前pToken持有的underlying数量不足，则退出
    ...
    function borrowInternal(uint256 borrowAmount) internal nonReentrant returns (uint256) {
        ...
        return borrowFresh(msg.sender, borrowAmount, false);
    }
    ...
    function borrowFresh(
        ...
        uint256 allowed = comptroller.borrowAllowed(address(this), borrower, borrowAmount, isCreditLoan);
        if (allowed != 0) {
            //3, 14, 4
        ...
        (vars.mathErr, vars.accountBorrows) = borrowBalanceStoredInternal(borrower, isCreditLoan);
        if (vars.mathErr != MathError.NO_ERROR) {
        ...
        if (isCreditLoan) {
            /* We write the previously calculated values into storage */
            accountBorrowsCreditLoan[borrower].principal = vars.accountBorrowsNew; //当前这个borrower所借的全部数量
            accountBorrowsCreditLoan[borrower].interestIndex = borrowIndex; //当前的索引值
        } else {
            /* We write the previously calculated values into storage */
            accountBorrows[borrower].principal = vars.accountBorrowsNew; //当前这个borrower所借的全部数量
            accountBorrows[borrower].interestIndex = borrowIndex; //当前的索引值
        }

        totalBorrows = vars.totalBorrowsNew; //全局维护总量

        /* We emit a Borrow event */
        emit Borrow(borrower, borrowAmount, vars.accountBorrowsNew, vars.totalBorrowsNew, isCreditLoan);
    ...
    function repayBorrowFresh(
        ...
        vars.borrowerIndex = isCreditLoan ? 
            accountBorrowsCreditLoan[borrower].interestIndex : 
            accountBorrows[borrower].interestIndex; //最后一次借款的索引位置

        /* We fetch the amount the borrower owes, with accumulated interest */
        (vars.mathErr, vars.accountBorrows) = borrowBalanceStoredInternal(borrower, isCreditLoan); //TODO 还款
        if (vars.mathErr != MathError.NO_ERROR) {
         ...
         */
        //2. 清算人向pToken合约转钱，清算人还完钱，即borrower的欠款被还完了
        //清算人帮还了这么多acturalRepayAmount数量， 是address(this)在underlying中先后的数量相减得出的
        //返回的是还款前后合约的差价，所以是真正还款的数量
        ...
        if (isCreditLoan) {
            /* We write the previously calculated values into storage */
            accountBorrowsCreditLoan[borrower].principal = vars.accountBorrowsNew;
            accountBorrowsCreditLoan[borrower].interestIndex = borrowIndex;
        } else {
            /* We write the previously calculated values into storage */
        ...
        emit RepayBorrow(payer, borrower, vars.actualRepayAmount, vars.accountBorrowsNew, vars.totalBorrowsNew, isCreditLoan);
    ...
    function liquidateBorrowFresh(
        ...
        (uint256 repayBorrowError, uint256 actualRepayAmount) = repayBorrowFresh(liquidator, borrower, repayAmount, false);
        ...
        if (address(pTokenCollateral) == address(this)) {
            //TODO 抵押物就是当前要清算的token？，抵押BUSD，借出BUSD？？为何？
            //如果是相同的，则向清算人转入token
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    //TODO _setReserveFactorFresh 这个参数：reserveFactorMantissa待研究
    ...
    //测试专用，后期whiteList写死
    function setWhiteListInternal(address _trustList) public returns (uint256) {
        require(_trustList != address(0), "white list is invalid!");
        require(msg.sender == admin, "only admin can set white list");

        whiteList[_trustList] = true;
        return 0;
    }
    ...
    function doCreditLoanBorrowInternal(
        ...
        require(whiteList[_trust] == true, "doCreditLoanBorrowInternal address not in whitelist");
        ...
        return borrowFresh(_borrower, _borrowAmount, true);
    }
    ...
    function doCreditLoanRepayInternal(
        ...
        require(whiteList[_trust] == true, "doCreditLoanRepayInternal address not in whitelist");
        ...
        return repayBorrowFresh(_payer, _payer, _repayAmount, true);
    }
}
...
contract Comptroller is ComptrollerV5Storage, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError {
    ...
    event CompSpeedUpdated(PToken indexed pToken, uint256 newSpeed);
    ...
    event DistributedSupplierComp(PToken indexed pToken, address indexed supplier, uint256 compDelta, uint256 compSupplyIndex);

    /// @notice Emitted when PUBL is distributed to a borrower
    event DistributedBorrowerComp(PToken indexed pToken, address indexed borrower, uint256 compDelta, uint256 compBorrowIndex);
    ...
    function exitMarket(address pTokenAddress) external returns (uint256) {
        ...
        (uint256 oErr, uint256 tokensHeld, uint256 amountOwed, ) = pToken.getAccountSnapshot(msg.sender, false);
        ...
        //把最后一个持有的pToken移动到了刚刚删除的pToken的位置，保证数组的连续性
    ...
    function mintAllowed(
        ...
        //TODO:这两个是和Comp代币有关的操作，先不用看
        updatePubSupplyIndex(pToken);
        distributeSupplierComp(pToken, minter);
    ...
    function redeemAllowed(
        ...
        // Keep the flywheel moving
        updatePubSupplyIndex(pToken);
        distributeSupplierComp(pToken, redeemer);
    ...
    function liquidateBorrowAllowed(
        ...
        //repayAmount: 10000000
        // maxCLose: 49,541,392.5
        //这里是计算，清算人最大可以清算的数量
        //确实只能清算一部分，由closeFactor限制，publics是限定为0.5
    ...
    function transferAllowed(
        ...
        // Keep the flywheel moving
        updatePubSupplyIndex(pToken);
        distributeSupplierComp(pToken, src);
        distributeSupplierComp(pToken, dst);
    ...
    struct AccountLiquidityLocalVars {
        ...
        uint256 pTokenBalance; //持有的pToken
    ...
    {
        //这句可以理解为：如果不借，不还，当前状态下，能否被清算,所以传入的数据都是0,0,0
        ...
        )
    {
        ...
        for (uint256 i = 0; i < assets.length; i++) {
            ...
            (oErr, vars.pTokenBalance, vars.borrowBalance, vars.exchangeRateMantissa) = asset.getAccountSnapshot(account, false);
            ...
            //collateralFactorMantissa这个值是通过setCollateralFactor()函数设置的
            ...
            //这是一个for循环，遍历所有自己参与的pToken，包含其他的pToken，也包括当前操作的pToken
        ...
        /*
                0: uint256: 0
                1: uint256: 39992000000040970278999999353200
                2: uint256: 0 
        */
     ...
     */
    //需要手动调用: 80000000000000000， 表示8e18
     ...
     * @param compSpeed New PUBL speed for market
     */
    function setCompSpeedInternal(PToken pToken, uint256 compSpeed) internal {
        uint256 currentCompSpeed = compSpeeds[address(pToken)];
        if (currentCompSpeed != 0) {
            ...
            if (compSupplyState[address(pToken)].index == 0 && compSupplyState[address(pToken)].block == 0) {
                compSupplyState[address(pToken)] = CompMarketState({ index: compInitialIndex, block: safe32(getBlockNumber(), "block number exceeds 32 bits") });
            }

            if (compBorrowState[address(pToken)].index == 0 && compBorrowState[address(pToken)].block == 0) {
                compBorrowState[address(pToken)] = CompMarketState({ index: compInitialIndex, block: safe32(getBlockNumber(), "block number exceeds 32 bits") });
            }
        }

        if (currentCompSpeed != compSpeed) {
            compSpeeds[address(pToken)] = compSpeed;
            emit CompSpeedUpdated(pToken, compSpeed);
        }
    }
    ...
    function updatePubSupplyIndex(address pToken) internal {
        ...
        // uint256 supplySpeed = compSpeeds[pToken];
        ...
        if (deltaBlocks > 0 && supplySpeed > 0) {
            ...
            compSupplyState[pToken] = CompMarketState({ index: safe224(index.mantissa, "new index exceeds 224 bits"), block: safe32(blockNumber, "block number exceeds 32 bits") });
    ...
    function updatePubBorrowIndex(address pToken, Exp memory marketBorrowIndex) internal {
        ...
        // uint256 borrowSpeed = compSpeeds[pToken];
        ...
        if (deltaBlocks > 0 && borrowSpeed > 0) {
            ...
            compBorrowState[pToken] = CompMarketState({ index: safe224(index.mantissa, "new index exceeds 224 bits"), block: safe32(blockNumber, "block number exceeds 32 bits") });
        } else if (deltaBlocks > 0) {
     ...
     */
    function distributeSupplierComp(address pToken, address supplier) internal {
        CompMarketState storage supplyState = compSupplyState[pToken];
        Double memory supplyIndex = Double({ mantissa: supplyState.index });
        Double memory supplierIndex = Double({ mantissa: compSupplierIndex[pToken][supplier] });
        compSupplierIndex[pToken][supplier] = supplyIndex.mantissa;
        ...
        emit DistributedSupplierComp(PToken(pToken), supplier, supplierDelta, supplyIndex.mantissa);
    }
    ...
    function distributeBorrowerComp(
        ...
        Double memory borrowerIndex = Double({ mantissa: compBorrowerIndex[pToken][borrower] });
        compBorrowerIndex[pToken][borrower] = borrowIndex.mantissa;
        ...
        if (borrowerIndex.mantissa > 0) {
            ...
            emit DistributedBorrowerComp(PToken(pToken), borrower, borrowerDelta, borrowIndex.mantissa);
        }
    }
    ...
    function getUnclaimedPub(address holder) public view returns (uint256) {
        uint256 unclaimed = compAccrued[holder];
        
        for (uint256 i = 0; i < allMarkets.length; i++) {
            PToken market = allMarkets[i];
            
            uint256 supplierTokens = market.balanceOf(holder);
            if (supplierTokens > 0) {
            ...
            Exp memory marketBorrowIndex = Exp({ mantissa: market.borrowIndex() });
            uint256 borrowerAmount = div_(market.borrowBalanceStored(holder), marketBorrowIndex);
            if (borrowerAmount > 0) {
     ...
     */
    function claimComp(address holder, PToken[] memory pTokens) public {
        address[] memory holders = new address[](1);
        holders[0] = holder;
        claimComp(holders, pTokens, true, true);
    }
        ...
        for (uint256 i = 0; i < pTokens.length; i++) {
            ...
            if (borrowers == true) {
                ...
                for (uint256 j = 0; j < holders.length; j++) {
                    distributeBorrowerComp(address(pToken), holders[j], borrowIndex);
            ...
            if (suppliers == true) {
                updatePubSupplyIndex(address(pToken));
                for (uint256 j = 0; j < holders.length; j++) {
                    distributeSupplierComp(address(pToken), holders[j]);
                    compAccrued[holders[j]] = grantCompInternal(holders[j], compAccrued[holders[j]]);
                }
            }
        }
    ...
    function grantCompInternal(address user, uint256 amount) internal returns (uint256) {
        Publ publ = Publ(getCompAddress());
        uint256 compRemaining = publ.balanceOf(address(this));
