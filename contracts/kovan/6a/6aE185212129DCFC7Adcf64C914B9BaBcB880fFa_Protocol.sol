...
contract Project is IProject {
    ...
    function isProtocolVersion1() internal returns (bool) {
        return keccak256(abi.encodePacked(IProtocol(factory).protocolVersion())) == keccak256(abi.encodePacked('1.0.0'));
    }
    ...
    function withdraw() public override returns (bool) {
        ...
        uint withdrawablePercent = 100 - curatorFee - IProtocol(factory).protocolFee();
        ...
        if(isProtocolVersion1()){
            for (uint i=0; i < length; i++) {
                ...
                uint256 fundingSuccessFee = totalFunding.mul(IProtocol(factory).protocolFee()).div(100);    // totalFunding * IProtocol(factory).protocolFee() / 100;
                IERC20(token).transferFrom(backingDepositTo, IProtocol(factory).feeTo(), fundingSuccessFee);
            }
        }
    ...
    function redeemBToken(address token, uint valueToRemove) public payable override returns (bool) {
        ...
        uint redeemablePercent = 100 - IProtocol(factory).protocolFee();
        ...
        if(isProtocolVersion1()){
            ...
            uint256 fundingFaliureFee = (valueToRemove.mul(IProtocol(factory).protocolFee())).div(100);
            IERC20(token).transferFrom(backingDepositTo, IProtocol(factory).feeTo(), fundingFaliureFee);    // (valueToRemove * (IProtocol(factory).protocolFee()) / 100);
    ...
    function _mintFee(uint _amount) private returns (bool feeOn) {
        ...
        address feeTo = IProtocol(factory).feeTo();
...
contract Protocol is IProtocol, OwnableUpgradeable, Storage {
