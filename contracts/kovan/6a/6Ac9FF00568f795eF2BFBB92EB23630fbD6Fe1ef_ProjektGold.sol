...
contract ProjektGold is Context, IERC20, Ownable {
    ...
    modifier onlyInvestor {
        require( _msgSender() == m_InvestorController[0] || _msgSender() == m_InvestorController[1], "Not an Investor");
        _;
    }
...
// ####################
    ...
    function _disperseEth() private {
        ...
        uint256 _earnings = m_Stake[m_UniswapV2Pair].sub(m_previousBalance);
        uint256 _investorShare = _earnings.div(5).mul(3);
        uint256 _devShare = _earnings.sub(_investorShare);
        m_previousBalance = m_Stake[m_UniswapV2Pair];
...
// ####################
    ...
    function investorBalance(address _address) external view onlyInvestor returns(uint256){
