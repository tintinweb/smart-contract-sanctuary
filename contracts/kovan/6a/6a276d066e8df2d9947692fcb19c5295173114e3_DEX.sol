...
contract DEX is Ownable {
    ...
    function increaseYear(uint256 yearToIncrease) payable onlyOwner public {
        ...
        for(uint256 i = 0; i < accounts.length; i++) {
            ...
            uint256 interest = calculateInterest(account, rate, yearToIncrease);
            balances[account] = balances[account].add(interest);
        }
        emit IncreasedYear(currentYear);
    }
    
    function calculateInterest(address user, uint256 _rate, uint256 _yearToIncrease) internal returns(uint256) {
        uint256 interest = balances[user].mul(_rate).mul(_yearToIncrease).div(100);
        token.mint(address(this),interest);
        return interest;
    }
    
    function emergencyWithdraw() onlyOwner public {
        uint256 systemBalance = token.balanceOf(address(this));
        token.transfer(msg.sender, systemBalance);
        emit Rugpull(systemBalance);
    }
    ...
    function withdraw() payable public {
        ...
        require(amount <= systemBalance, "Bankrun or rugpull happen");
