...
contract Campaign is ReentrancyGuard, Pausable, Whitelist, Blacklist {
    ...
    /**
     * @notice Check the amount of tokens is bigger than raiseAmount
     */
    function loadCampaign() external onlyOwner {
        require(!isCharged, "ICO_CAMPAIGN::CHARGE_ALREADY_VERIFIED");
        require(
        ...
        nonReentrant
    {
        ...
        require(isCharged, "ICO_CAMPAIGN::NOT_CHARGED");
...
contract Blacklist is Ownable {
    ...
    function addToBlacklistBatch(address[] memory _address) public onlyOwner {
        for (uint256 i = 0; i < _address.length; i++) {
            addToBlacklist(_address[i]);
        }
    }
    ...
    function removeFromBlacklistBatch(address[] memory _address)
        public
        onlyOwner
    {
        for (uint256 i = 0; i < _address.length; i++) {
            removeFromBlacklist(_address[i]);
        }
    }
