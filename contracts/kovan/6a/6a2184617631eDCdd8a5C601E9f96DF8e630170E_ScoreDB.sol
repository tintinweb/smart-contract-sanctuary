...
contract ScoreDB is ChainlinkClient, Pausable, Ownable, ScoreDBInterface {
    ...
    // Mapping requestId to nature of caller (i.e., 0 for frontend, 1 for investor contract etc.). Requester supplies the nonce value along the request.
    mapping(bytes32 => uint8) private callerTracker;
        ...
        recordChainlinkFulfillment(_requestId)
    {
        ...
        // If the callerId id is of the requstUpdatedScoreBorrow function, execute the borrow fulfill logic
        if (callerTracker[_requestId] == 1) {
            LoanRequest memory c = loanRequestTracker[_requestId];
            // makes borrow fulfill optional
            if (c.loanId != 0) {
                IInvestor(c.investor).borrowFulfill(
                    c.loanId,
                    c.hash,
                    c.signature
                );
    ...
    function requestUpdatedScoreBorrow(
        ...
        console.log("requestUpdatedScoreBorrow");
        console.log(tokenId);
        // Ensure enough link
        require(
            IERC20(0xa36085F69e2889c224210F603D836748e7dC0088).balanceOf(
                address(this)
            ) >= fee,
            "Not enough LINK"
        );
        bytes32 reqId = requestScore(
            tokenId,
            callBackContract,
            functionSelector
        );
        callerTracker[reqId] = 1;
...
interface ScoreDBInterface {
    ...
    // Called by the lending contract, updates the score then calls  investor.borrowFulful()
    function requestUpdatedScoreBorrow(
        uint256 tokenId,
        uint256 loanId,
        bytes32 hash,
    ...
    /* Sets the baseURI of the off-chain API. In future, only owner can change it, for now, left it public
