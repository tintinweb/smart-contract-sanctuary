...
contract CreatorInvestmentPool {
    ...
    function createPool(string memory name,uint256 lockPeriod,uint256 depositFixedFee,uint256 referralDepositFee,uint256 anualPrecent,uint256 penaltyEarlyWithdraw,address superAdmin,address gPartner,address lPartner,address startupTeam) public returns (address) {
        require(msg.sender == _poolRegistry, "CreatorInvestmentPool: sender don't poolRegistry contract");
        InvestmentPool _poolContract = new InvestmentPool(name,lockPeriod,depositFixedFee,referralDepositFee,anualPrecent,penaltyEarlyWithdraw,superAdmin,gPartner,lPartner,startupTeam,msg.sender,_returnInvestmentLpartner,_assetsManageTeam);
