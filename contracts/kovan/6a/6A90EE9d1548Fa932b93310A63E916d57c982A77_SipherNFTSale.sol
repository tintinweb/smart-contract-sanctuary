...
contract SipherNFTSale is ISipherNFTSale, Whitelist {
    ...
    function getElapsedPublicSaleTime() internal view returns (uint256) {
        ...
        if (timestamp < config.publicTime) {
            return 0;
        } else if(timestamp < 16*REDUCE_PRICE_INTERVAL){
            return timestamp - config.publicTime;
        } else {
            return 16*REDUCE_PRICE_INTERVAL;
        }
            // return
            //     (config.publicTime <= timestamp) && (timestamp < 16*REDUCE_PRICE_INTERVAL)
            //         ? timestamp - config.publicTime
            //         : 16*REDUCE_PRICE_INTERVAL;
    ...
    function getPublicSaleCurrentPrice() public view returns (uint256) {
        ...
        if ((elapsed >= REDUCE_PRICE_INTERVAL) && (elapsed < 16*REDUCE_PRICE_INTERVAL )) {
            currentPrice = SALE_PUBLIC_STARTING_PRICE - (elapsed / REDUCE_PRICE_INTERVAL)*REDUCE_PRICE_LEVEL;
        } else if(elapsed >= 16*REDUCE_PRICE_INTERVAL) {
    ...
    function _validateAndUpdateWithBuyAmount(
        ...
        if (
            config.publicEndTime <= timestamp && timestamp < config.freeMintTime
        ) {
            require(
