...
contract MomentNft is Initializable,
 UUPSUpgradeable,
 ...
 NFT721Mint {
  ...
  /**
   * @dev Byte32 variable used to provide Admin access.
   */
  bytes32 public constant ADMIN_ROLE = keccak256("ADMIN_ROLE");

    function initialize(address minter, address admin) initializer public {
      //  __ERC721_init("Moment", "MOMENT");
       ERC721Upgradeable.__ERC721_init("Moment","MOMENT");
   ...
   /**
     ...
     */
    function supportsInterface(bytes4 interfaceId) public view virtual override(AccessControlUpgradeable,NFT721Mint) returns (bool) {
 ...
 */
abstract contract NFT721Mint is Initializable, NFT721Creator, NFT721Metadata {
  ...
  event Minted(
    ...
    string indexed indexedTokenURIPath,
    string tokenURIPath
  );
   ...
   */
  function mint(string memory tokenURIPath) public returns (uint256 tokenId) {
    nextTokenId.increment();
    tokenId = nextTokenId.current();
    _mint(msg.sender, tokenId);
    _updateTokenCreator(tokenId, address(msg.sender));
    _setTokenURIPath(tokenId, tokenURIPath);
    emit Minted(msg.sender, tokenId, tokenURIPath, tokenURIPath);
  }

  /**
   * @dev Explicit override to address compile errors.
   */
  function _burn(uint256 tokenId) internal virtual override( NFT721Creator, NFT721Metadata) {
    super._burn(tokenId);
  }

  function supportsInterface(bytes4 interfaceId) public view virtual override(NFT721Creator, NFT721Metadata) returns (bool) {
        return interfaceId == type(IERC721Upgradeable).interfaceId
            || interfaceId == type(IERC721MetadataUpgradeable).interfaceId
            || super.supportsInterface(interfaceId);
    }

  function _beforeTokenTransfer (address from, address to, uint256 tokenId) internal virtual override(ERC721Upgradeable,NFT721Metadata) {
       NFT721Metadata._beforeTokenTransfer (from,to,tokenId);
  }

  function tokenURI(uint256 tokenId) public view virtual override(ERC721Upgradeable, NFT721Metadata) returns (string memory) {
...
import "@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol";
import "@openzeppelin/contracts-upgradeable/token/ERC721/ERC721Upgradeable.sol";
// import "../utils/ERC721URIStorageUpgradeable.sol";
 ...
 */
abstract contract NFT721Creator is Initializable, ERC721Upgradeable,ERC165StorageUpgradeable {
   using Address for address;
 ...
 */
abstract contract NFT721Metadata is ERC721URIStorageUpgradeable,ERC721EnumerableUpgradeable {
  using StringsUpgradeable for uint256;

  /**
   * @dev Stores hashes minted by a creator to prevent duplicates.
   */
  mapping(address => mapping(string => bool)) private creatorToURIHashToMinted;

  // Base URI
  string private _BaseURI;
  event BaseURIUpdated(string baseURI);
  event TokenURIPathUpdated(uint256 indexed tokenId, string indexed indexedTokenURIPath, string tokenURIPath);
   ...
   */
  function getHasCreatorMintedURIHash(address creator, string memory tokenURIPath) public view returns (bool) {
    return creatorToURIHashToMinted[creator][tokenURIPath];
  }
  ...
  /**
   * @dev The URI path should be the CID + file.extension, e.g.
   * `QmfPsfGwLhiJrU8t9HpG4wuyjgPo9bk8go4aQqSu9Qg4h7/metadata.json`
   */
  function _setTokenURIPath(uint256 tokenId, string memory _tokenURIPath) internal {
    // 46 is the minimum length for an URI content hash, it may be longer if paths are used
    require(bytes(_tokenURIPath).length >= 46, "NFT721Metadata: Invalid URI path");
    require(!creatorToURIHashToMinted[msg.sender][_tokenURIPath], "NFT721Metadata: NFT was already minted");

    creatorToURIHashToMinted[msg.sender][_tokenURIPath] = true;
    _setTokenURI(tokenId, _tokenURIPath);
  }
  ...
  function _burn(uint256 tokenId) internal virtual override(ERC721URIStorageUpgradeable,ERC721Upgradeable) {
    delete creatorToURIHashToMinted[msg.sender][tokenURI(tokenId)];
    super._burn(tokenId);
  }
