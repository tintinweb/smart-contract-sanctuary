...
contract AureusV1 is ERC20Capped, Ownable {
...
contract EtruriaV1 is Ownable {
  using SafeMath for uint256;
  using SafeERC20 for AureusV1;
  ...
  struct PoolInfo {
    ...
    uint256 allocPoint; // Allocation points for said pool. RWDs to be distributed to pool participants per block
    uint256 lastRewardBlock; // Last block number that SUSHIs distribution occurs.
    uint256 accRwdPerShare; // Accumulated RWD rewards per share, times 1e18
  }


  // The Reward token
  AureusV1 public rwdToken;
  // Info of each pool.
  ...
  function deployAureus(
    ...
    require(
      ...
      'A reward token is already deployed'
    );
    rwdToken = new AureusV1(_name, _symbol, _cap);
