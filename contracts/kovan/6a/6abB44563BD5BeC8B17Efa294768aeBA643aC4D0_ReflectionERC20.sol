...
contract ReflectionERC20 is ERC20 , Ownable {
    ...
    constructor(string memory name_, string memory symbol_, address wavax_, address swapRouter_, address payable team_,address payable treasury_, address payable charity_) payable ERC20(name_, symbol_) {
        ...
        hodlRewardDistributor = new HODLRewardsDistributor(wavax_);
        
        hodlRewardDistributor.excludeFromRewards(wavaxPair);
        hodlRewardDistributor.excludeFromRewards(swapRouter_);
        hodlRewardDistributor.excludeFromRewards(team_);
        hodlRewardDistributor.excludeFromRewards(treasury_);
        hodlRewardDistributor.excludeFromRewards(charity_);
    ...
    function _customTransfer(address from_, address to_, uint256 amount_) internal returns (bool){
        ...
        if(whitlisted[to_].tax || whitlisted[to_].tax || _inSwap ){
            _transfer(from_ , to_ , amount_);
            return true;
        }
        require(amount_ <= maxTx, "ReflectionERC20: > maxTX");
        ...
        if(taxAmount > 0) {
            // make the swap to wavax todo: sfsa
            ...
            uint256 swapOut = IERC20(wavax).balanceOf(address(this)) - balanceBefore;
...
contract HODLRewardsDistributor is Ownable {
        ...
        function depositRewards(uint256 amount_) external {
            IWAVAX(wavax).transferFrom(owner(), address(this), amount_);
