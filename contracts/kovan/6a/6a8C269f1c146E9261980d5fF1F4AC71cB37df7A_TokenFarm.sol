...
contract TokenFarm is Ownable {
    ...
    returns(uint256) {
        ...
        uint256 tokensStaked = stakingBalance[_token][_user];
        (uint256 price, uint256 decimals) = getTokenValue(_token);

        return price * tokensStaked / (10**decimals) ;
    ...
    function stakeTokens(uint256 _amount, address _token) public {
        ...
        // how much
        require(_amount > 0, 'amount must be non 0');
    ...
    function unstakeTokens(address _token) public {
        ...
        require(balance > 0, 'balance must be non 0');
        require (tokenIsAllowed(_token), 'token not allowed');
        // updateUniqueTokensStaked(msg.sender, _token);
