...
contract PrecogV4 {
    ...
    mapping(address => mapping(address => uint256)) accountDebt;
    ...
    function getAccountDebt(address token, address account) external view returns (uint256) {
        return accountDebt[token][account];
    }
    ...
    function withdraw(address to, address token, uint256 amountIn, bool letPrecogOwe) public {
        ...
        if(convertedSendAmount > actualBalance) {
            if(letPrecogOwe) {
                ...
                accountDebt[token][to] += convertedSendAmount - actualBalance;
    ...
    function updateTradingStatus(
        ...
        //require(deadline > block.timestamp, "PrecogV4: INVALID_DEADLINE");
    ...
    function withdrawFee(address token) public onlyAdmin {
        ...
        emit WithdrawFee(admin, token, totalFeeWithdraw[token], totalFeeTrading[token], totalFeeLending[token]);
    ...
    function payAllDebtsAndBalanceLiquidity(address token) public {
        ...
        for(uint256 i = currentDebtNotPay[token]; i < paybook[token].length; i++) {
            ...
            accountDebt[token][paybook[token][i].account] -= paybook[token][i].debtAmount;
...
interface InterfacePCOG {
    ...
    event AddBurner(address owner, address burner);
    event RemoveBurner(address owner, address burner);
}
