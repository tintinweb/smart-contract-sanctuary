...
contract OmakaseCore is OmakaseStorage, BCAOwnable, KeeperCompatibleInterface {
    ...
    bytes32 public upkeepSignature;
    ...
    function checkUpkeep(
        ...
        if (upkeepNeeded) {
            upkeepSignature = keccak256(performData);
        }
    }
    ...
    function performUpkeep(bytes calldata performData) external override {
        require(
            upkeepSignature == keccak256(performData),
            "Function checkUpkeep should be called beforehand"
        );

        uint256 length = performData.length / 32;

        for (uint256 i = 0; i < length; i++) {
            bytes32 flightId = performData.toBytes32(i * 32);
            emit RequestStatusUpdate(flightId);
        }

        flightQueue.deleteHead(length);
        upkeepSignature = 0x0;
    }
...
library Impl_MinPriorityQueue {
    ...
    function deleteHead(LinkedList storage list, uint256 n) internal {
        bytes32 curr = list.head;
        for (uint256 i = 0; i < n; i++) {
            bytes32 next = list.next[curr];

            delete list.next[curr];
            delete list.priorities[curr];
