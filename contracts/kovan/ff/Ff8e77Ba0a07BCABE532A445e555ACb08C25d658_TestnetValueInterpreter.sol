...
contract TestnetValueInterpreter is IValueInterpreter, Ownable, TreasuryEnabledMixin {
    ...
    constructor(address _treasury, address _weth) TreasuryEnabledMixin(_treasury) public {
        WETH = _weth;
    }
    ...
    function isSupportedPrimitiveAsset(address _asset) external view override returns (bool isSupported_) {
        return __isSupportedAsset(_asset);
    }

    function isSupportedDerivativeAsset(address) external view override returns (bool isSupported_) {
        return false;
    }
