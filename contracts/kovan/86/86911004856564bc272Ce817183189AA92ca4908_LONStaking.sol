...
contract LONStaking is ERC20ForUpgradeable, OwnableForUpgradeable, ReentrancyGuard, Pausable {
    ...
    uint256 public COOLDOWN_IN_MINUTES;
    ...
    function initialize(
        address _owner,
        uint256 _COOLDOWN_IN_MINUTES,
        uint256 _BPS_RAGE_EXIT_PENALTY
    ) external {
        _initializeOwnable(_owner);
        _initializeERC20("Wrapped Tokenlon", "xLon");

        require(_COOLDOWN_IN_MINUTES >= 1, "COOLDOWN_IN_MINUTES less than 1 minute");
        require(_BPS_RAGE_EXIT_PENALTY <= BPS_MAX, "BPS_RAGE_EXIT_PENALTY larger than BPS_MAX");
        COOLDOWN_IN_MINUTES = _COOLDOWN_IN_MINUTES;
        COOLDOWN_SECONDS = _COOLDOWN_IN_MINUTES * 60;
    ...
    function setCooldownAndRageExitParam(uint256 _COOLDOWN_IN_MINUTES, uint256 _BPS_RAGE_EXIT_PENALTY) public onlyOwner {
        require(_COOLDOWN_IN_MINUTES >= 1, "COOLDOWN_IN_MINUTES less than 1 minute");
        require(_BPS_RAGE_EXIT_PENALTY <= BPS_MAX, "BPS_RAGE_EXIT_PENALTY larger than BPS_MAX");
        ...
        COOLDOWN_SECONDS = _COOLDOWN_IN_MINUTES * 60;
        BPS_RAGE_EXIT_PENALTY = _BPS_RAGE_EXIT_PENALTY;
        emit SetCooldownAndRageExitParam(_COOLDOWN_IN_MINUTES, _BPS_RAGE_EXIT_PENALTY);
    }
    ...
    function previewRageExit(address _account) external view returns (uint256 penalty, uint256 lonAmountReceive) {
        ...
        if (block.timestamp > cooldownEndTimestamp) {
            ...
            uint256 timeDiffInDays = Math.min(
                COOLDOWN_IN_MINUTES,
                (cooldownEndTimestamp.sub(block.timestamp)).div(60).add(1)
            );
            // Penalty = share * (number_of_days_to_cooldown_end / number_of_days_in_cooldown) * (BPS_RAGE_EXIT_PENALTY / BPS_MAX)
            penalty = share.mul(timeDiffInDays).mul(BPS_RAGE_EXIT_PENALTY).div(BPS_MAX).div(COOLDOWN_IN_MINUTES);
    ...
    function rageExit() public nonReentrant {
        ...
        if (block.timestamp > cooldownEndTimestamp) {
            ...
            // Penalty = share * (number_of_days_to_cooldown_end / number_of_days_in_cooldown) * (BPS_RAGE_EXIT_PENALTY / BPS_MAX)
            uint256 penalty = share.mul(timeDiffInDays).mul(BPS_RAGE_EXIT_PENALTY).div(BPS_MAX).div(COOLDOWN_IN_MINUTES);
