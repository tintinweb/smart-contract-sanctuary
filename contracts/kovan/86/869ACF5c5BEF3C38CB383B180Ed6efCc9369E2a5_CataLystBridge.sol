...
contract CataLystBridge is Manager {
    ...
    function userDeposit(uint256 _roundId) isValidRound(_roundId) external payable whenNotPaused() { 
        require(msg.value > 0, "Invalid-fund");
        uint256 fund = msg.value;
        RoundInfo memory round = rounds[_roundId];
        require(round.startTime <= block.timestamp && block.timestamp <= round.startTime.add(round.depositDuration),"Cam-not-deposit");
        round.totalDepositFund = round.totalDepositFund.add(fund);
        rounds[_roundId] = round;
        userFund[msg.sender][_roundId] = userFund[msg.sender][_roundId].add(fund);
    }
    ...
    function userWithDrawn(uint256 _roundId) isValidRound(_roundId) whenNotPaused external {
        ...
        require(block.timestamp >= round.startTime.add(round.depositDuration).add(round.stakeDuration) && round.totalWithDrawnFund > 0, "Can-not-withdrawn-now"); // deposit is closed and allow withdrawn
        uint256 amountToWithdrawn = fundOfUser.mul(round.totalWithDrawnFund).div(round.totalDepositFund);
    ...
    function adminCollectFund(uint256 _roundId) isValidRound(_roundId) external onlyOwner() whenNotPaused() {
        require(rounds[_roundId].startTime.add(rounds[_roundId].depositDuration) < block.timestamp, "Deposit-time-not-end-yet");
        RoundInfo memory round = rounds[_roundId];
    ...
    function adminDepositFund(uint256 _roundId) isValidRound(_roundId) external payable onlyOwner() whenNotPaused() {
        RoundInfo memory round = rounds[_roundId];
        require(round.startTime.add(round.depositDuration).add(round.stakeDuration) < block.timestamp, "Round-not-end-yet");
