...
// After the initial burst phase, emission will continue for perpetuity at an inflation rate at is offset by profit generated
...
contract EmissionsOperator is Ownable, ReentrancyGuard {
    ...
    // variables
    uint256 constant WEEKLY_DURATION = 15 minutes; // Each reward period lasts for one week
    ...
    uint256 public vestTime = 20 minutes; // Time it takes to reach 0% penalty
    ...
    function rebalancePoolRewards(bool recalculateFromProfit) internal {
        ...
        for(i = 0; i < length; i++){
            if(totalPools[activePools[i]].isLpPool == false){
                // Some zero profit pools will have no rewards
                if(totalPools[activePools[i]].poolWeight == 0 && totalPools[activePools[i]].excludedFromWeight == true){
