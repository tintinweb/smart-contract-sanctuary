...
contract CrowdPresale {
    using SafeMath for uint256;

    address payable internal richFactoryAddress; // address that creates the presale contracts
    address payable public richDevAddress; // address where dev fees will be transferred to
    ...
    bool public cakeLiquidityAdded = false; // if true, liquidity is added in PancakeSwap and lp tokens are locked //set fonksiyonu düzenlenecek
    ...
    constructor(
        address _richFactoryAddress,
        address _richDevAddress
    ) public {
        require(_richFactoryAddress != address(0));
        require(_richDevAddress != address(0));
        richFactoryAddress = payable(_richFactoryAddress);
        richDevAddress = payable(_richDevAddress);
    }

    modifier onlyRichDev() {
        require(
            richFactoryAddress == msg.sender || richDevAddress == msg.sender
        );
        _;
    }

    modifier onlyPresaleCreatorOrRichFactory() {
        require(
            presaleCreatorAddress == msg.sender ||
                richFactoryAddress == msg.sender,
            "Not presale creator or factory"
        );
        _;
    }

    modifier onlyPresaleCreatorOrRichDev() {
        require(
            presaleCreatorAddress == msg.sender || richDevAddress == msg.sender,
    ...
    ) external onlyPresaleCreatorOrRichFactory {
    ...
    //BNB toplanacağı için ve decimal 18 olduğu için hepsi 18 sıfıra göre ayarlanacak. 10000000 

    function setGeneralInfo(
        uint256 _totalTokens, // toplam nekadar token satılacak. 500000 için "500000000000000000000000" girdik. 
        uint256 _tokenPriceInWei, // token satış fiyatı 0.005 BNB için "5000000000000000"
        uint256 _hardCapInWei, // satış için en fazla toplanacak miktar. 2500 bnb için "2500000000000000000000" girdik.
        uint256 _softCapInWei, // satışın başarılı olması için enz toplanacak miktar. 1000 bnb için "1000000000000000000000" girdik.
        uint256 _maxInvestInWei,//her cüzdan için max alım miktarı. 40 bnb için "40000000000000000000" girdik. decimal kaç ise okadar sıfır ekledik
        uint256 _minInvestInWei,// her cüzdan en az nekadar yatırmalı 0.1 bnb için "100000000000000000"
        uint256 _openTime, // satış açılma zamanı
        uint256 _closeTime // satış kapanma vakti
    ) external onlyPresaleCreatorOrRichFactory {
    ...
    function addWhitelistedAddresses(address[] calldata _whitelistedAddresses)
        external
        onlyPresaleCreatorOrRichFactory
    {
    ...
    function cancelAndTransferTokensToPresaleCreator() external {
        if (
            !cakeLiquidityAdded &&
            presaleCreatorAddress != msg.sender &&
            richDevAddress != msg.sender
        ) {
            revert();
        }
        if (cakeLiquidityAdded && richDevAddress != msg.sender) {
            revert();
        }
    ...
    function collectFundsRaised() external onlyRichDev {
        require(!presaleCancelled);
        require(totalCollectedWei > softCapInWei, "Soft cap reached");

        if (address(this).balance > 0) {
            richDevAddress.transfer(address(this).balance);
        }
    }

    function burnUnsoldTokens() external onlyPresaleCreatorOrRichDev {
    ...
    function setCakeLiquidityAdded() external onlyRichDev {
        require(!cakeLiquidityAdded, "Liquidity has already been added");
        cakeLiquidityAdded = true;    
    }
    
    function setPresaleCancelled() external onlyPresaleCreatorOrRichDev {
        require(!cakeLiquidityAdded, "Presale has already been canceled");
        presaleCancelled = true;    
    }
}