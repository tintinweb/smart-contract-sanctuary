...
interface IRewardPool {
    function returnTokenToPlayer(address _address) external;
}
...
contract WarriorNFT is Context, IERC721, Ownable {
    ...
    function mint(uint256 amount) external {
        ...
        for(uint i=0; i<amount; i++) {
            ...
            if (randNum==0&&randNum<5) {
                warrior = Warrior("FFF", 6, 45000+genRand(5001, randNum), "QmTyQ3Mab2711GWGMyvXqMVCQpK49WH1KQRzhUBJS96Q7W");
            } else if (randNum>=5&&randNum<100) {
                warrior = Warrior("EEE", 5, 4000+genRand(1501, randNum), "QmcNEMp12fU7tNyTt8oRBKeW7QjXR9gLKwJoXPYAav86XV");
            } else if (randNum>=100&&randNum<800) {
                warrior = Warrior("DDD", 4, 3000+genRand(1000, randNum), "QmZwioRnSrxDUcDK1e4LNkvGi7WW9eJXnHrEwXavathiwe");
            } else if (randNum>=800&&randNum<2200) {
                warrior = Warrior("CCC", 3, 2000+genRand(1000, randNum), "Qmbdyp864SwZZ2X3yBWKX1YeSax4BRj8F3yvLM9PcgAXYw");
            } else if (randNum>=2200&&randNum<5000) {
                warrior = Warrior("BBB", 2, 1000+genRand(1000, randNum), "QmPbsQEreJNFVRkNkgyzDNTgkQ6Xt6GrPQXjz83FYTyQMi");
            } else {
                warrior = Warrior("AAA", 1, 400+genRand(600, randNum), "QmRXAzXAA8oiuMxe9m8Tdp9oq4tsBXgDjkpNz3Tz5uuRj2");
            }
    ...
    function executeWarrior(uint256 _tokenId) external {
        ...
        rewardPool.returnTokenToPlayer(msg.sender);
    }

    function setRewardPoolAddr(address _address) external onlyOwner {
        rewardPoolAddr = _address;
        rewardPool = IRewardPool(rewardPoolAddr);
        bloodstone.approve(address(rewardPoolAddr), 5000000*10**18);
    }
