...
contract Decentralearn is AccessControl, KeeperCompatibleInterface {
    ...
    modifier hasTokenBalance(address _tokenAddress) {
        ...
        require(ERC20(_tokenAddress).balanceOf(address(this)) >= campaign.totalNumOftokens, "Token balance not same as mentioned at the time of create campaign!");
        _;
    }

    modifier hasEnoughTokenBalance(address _tokenAddress) {
        CampaignCreator memory campaign = campaigns[_tokenAddress];        
        require(ERC20(_tokenAddress).balanceOf(address(this)) >= campaign.NumOftokensPerTrng, "Not enough tokens to incentivize!");
        _;
    }
 ...
 **/
    function startCampaign(address _tokenAddress) public hasTokenBalance(_tokenAddress) {
            ...
            ERC20 token = ERC20(_tokenAddress);
            emit CampaignStarted(_tokenAddress, token.name(), campaign.campaignId, IERC20(_tokenAddress).balanceOf(address(this)));
        }
