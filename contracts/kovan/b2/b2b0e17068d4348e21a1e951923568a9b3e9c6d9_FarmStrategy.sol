...
abstract contract StrategyStorage is StrategyAdminStorage, IStrategy {
    ...
    address public override token0; //token0地址
    address public override token1;//token1地址
...
contract FarmStrategy is StrategyStorage{
    ...
    function withdraw(uint256 amount) external override returns (uint256 amount0, uint256 amount1){
        ...
        (amount0, amount1) = IUniswapV2Router02(router).removeLiquidity(token0, token1, amount, 0, 0, controller, block.timestamp.add(1));
    }
    ...
    function getProfitTokens() external view returns (address[] memory tokens){
    ...
    function getTokenAmounts() external view override returns (address[] memory tokens, uint256[] memory amounts){
        ...
        amounts[0] = lpTokenAmount.mul(IUniswapV2Router02(router).getTokenInPair(lpToken,token0)).div(totalSupply);
        amounts[1] = lpTokenAmount.mul(IUniswapV2Router02(router).getTokenInPair(lpToken,token1)).div(totalSupply);
        amounts[0] = amounts[0].add(IERC20(token0).balanceOf(address(this)));
        amounts[1] = amounts[1].add(IERC20(token1).balanceOf(address(this)));
        amounts[2] = getProfitAmount();
    }
    ...
    function getProfitAmount() view public returns (uint256){
        ...
        return profitAmount.add(IMasterChef(lpToken).pendingSashimi(pid, address(this)));
    }
