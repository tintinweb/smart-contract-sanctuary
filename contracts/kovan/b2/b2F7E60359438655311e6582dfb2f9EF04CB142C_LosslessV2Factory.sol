...
import "./LosslessV2Storage.sol";
import "./ERC20.sol";

contract LosslessV2Factory is LosslessV2Storage, ILosslessV2Factory {
	address public override feeTo;
	address public override feeToSetter;
	address public override newFeeToSetter;
	...
	address[] public override allPools; // all pools info are here
	mapping(address => bool) public override isPoolActive; // check if a pool is active or not


    modifier onlyAdmin() {
        require(msg.sender == feeToSetter, "LosslessV2Factory: FORBIDDEN");
        _;
    }

	constructor(address _feeToSetter) public {
		feeToSetter = _feeToSetter; // default is admin
	...
	function createPool(
		...
		bytes32 salt = keccak256(abi.encodePacked(allPools.length, bidToken, principalToken, addressProvider)); // add allPools.length (avoid duplicate pool address)
		LosslessV2Pool newPool = new LosslessV2Pool{ salt: salt }(bidToken, principalToken, addressProvider, biddingDuration, gamingDuration);
		(address shortToken, address longToken, address pauseToken, address sponsorToken) =  _initializePool(tokenName, tokenSymbol, ERC20(principalToken).decimals(), address(newPool));
		newPool.initialize(shortToken, longToken, pauseToken, sponsorToken);
		// save pool address to pool related
		getPool[bidToken][principalToken] = address(newPool);
		allPools.push(address(newPool));
		// set this pool as an active pool
		isPoolActive[address(newPool)] = true;

		emit PoolCreated(bidToken, principalToken, address(newPool), allPools.length);
	}

	///@dev only _feeToSetter / admin can call this function
	function terminatePool(address pool) external onlyAdmin returns (bool) {
	...
	/**
	 * @dev get pool token address through pool address
	 * @param id	0 shortToken, 1 longToken, 2 pause Token, 3 sponsorToken
	 * @param pool	pool address
	 **/
	function getToken(uint256 id, address pool) external override view returns(address token) {
		require(id <= 3, "LosslessV2Factory: IVALID ID");
		if (id == 0) {
			token = getShortToken[pool];
		} else if (id == 1) {
			token = getLongToken[pool];
		} else if (id == 2) {
			token = getPauseToken[pool];
		} else if (id == 3) {
			token = getSponsorToken[pool];
		}
	}

	// below functions all limited to admin

	/**
	 * @dev	 The default FeeToSetter is admin but admin can assign this role to others by calling `setFeeToSetter`
	 * @param _feeTo	percentage of total interest as trading fee
	 **/
	...
	/**
	 * @dev	 only admin can set the feePercent (usage: fee = totalInterest.mul(feePercent).div(PRECISION))
	 * @param _feePercent	percentage of total interest as trading fee
	 **/
	function setFeePercent(uint256 _feePercent) external override onlyAdmin {
		feePercent = _feePercent;
	}

	/**
	 * @dev	feeToSetter can assign its role to others
	 * @param _newFeeToSetter	new fee setter address
	 **/
	function setFeeToSetter(address _newFeeToSetter) external override onlyAdmin {
		newFeeToSetter = _newFeeToSetter;
	}

	/**
	 * @dev a second chance to confirm the previously assigned address
	 **/
	function confirmFeeToSetter() external override onlyAdmin {
		require(newFeeToSetter != address(0), "LosslessV2Factory: NEWFEETOSETTER CANNOT BE ZERO ADDRESS");
...
interface ILosslessV2Factory {
	...
	function getToken(uint256 id, address pool) external view returns(address token);
...
contract LosslessV2Pool is ILosslessV2Pool{
	...
	uint256 public valuePerPauseToken = PRECISION; // the value of pause Share
	uint256 public valuePerSponsorToken = PRECISION; // the value of sponsor share should be fixed to PRECISION
	...
	ILosslessV2Token private _pauseToken;
	...
	function endGame() external override onlyAfter(status.lastUpdateTimestamp.add(status.durationOfGame)) {
		...
		uint256 totalPausePrincipal = _pauseToken.totalSupply().mul(valuePerPauseToken).div(PRECISION);
		...
		// update share value
		_updateTokenValue(totalShortPrincipal, totalLongPrincipal, totalPausePrincipal, totalPrincipal);
	 ...
	 **/
	function withdraw(uint256 shortTokenAmount, uint256 longTokenAmount) external override {
	 ...
	 **/
	function swap(bool fromLongToShort, uint256 swapTokenAmount) external override {
		require(status.currState == PoolStatus.Accepting, "LosslessV2Pool: WRONG STATUS");
	 ...
	 **/
	function sponsorWithdraw(uint256 sponsorTokenAmount) external override {
	...
	function pausePrediction() external override {
		...
		uint256 shortTokenBalance = _shortToken.balanceOf(msg.sender);
		uint256 longTokenBalance = _longToken.balanceOf(msg.sender);
		require(shortTokenBalance != 0 || longTokenBalance != 0, "LosslessV2Pool: INSUFFICIENT BALANCE");

		// convert user long/short share to pause share(fixed to precision)
		_convertTokenToPauseToken(msg.sender, shortTokenBalance, longTokenBalance);
	}
	...
	function resumePrediction(bool fromPauseToShort) external override {
		require(status.currState == PoolStatus.Accepting, "LosslessV2Pool: WRONG STATUS");
		require(_pauseToken.balanceOf(msg.sender) > 0, "LosslessV2Pool: NO USER PAUSE SHARES IN POOL");

		// load user principal amount in pause pool
		uint256 userPrincipalAmount = _pauseToken.balanceOf(msg.sender).mul(valuePerPauseToken).div(PRECISION);
		// burn user pause tokens
		_pauseToken.burn(msg.sender, _pauseToken.balanceOf(msg.sender));

		if (fromPauseToShort == true) {
			// if user choose to resume short
			_mintTokens(true, msg.sender, userPrincipalAmount, 0);
		} else {
			// if user choose to resume long
			_mintTokens(true, msg.sender, 0, userPrincipalAmount);
		}
	}
	 ...
	 * @param _totalPausePrincipal	the total amount of pause principal
	 * @param _totalPrincipal		the total amount of principal
	 **/
	function _updateTokenValue(
		uint256 _totalShortPrincipal,
		uint256 _totalLongPrincipal,
		uint256 _totalPausePrincipal,
		...
		// update valuePerPauseToken
		uint256 interestForPausePool = totalInterest.mul(_totalPausePrincipal).div(_totalPrincipal);
		totalInterest = totalInterest.sub(interestForPausePool);
		_totalPausePrincipal = _totalPausePrincipal.add(interestForPausePool);
		if (_pauseToken.totalSupply() != 0) {
			// if there are pause shares -> update share value
			valuePerPauseToken = _totalPausePrincipal.mul(PRECISION).div(_pauseToken.totalSupply());
		}
		...
		emit UpdateTokenValue(valuePerShortToken, valuePerLongToken, valuePerPauseToken);
	}

	/**
	 * @dev convert long / short token to pause token
	 * @param _to 	the destination address got pause token
	 * @param _shortTokenBalance	the amount of the short token required
	 * @param _longTokenBalance		the amount of the long token required
	 **/
	function _convertTokenToPauseToken(
		address _to,
		uint256 _shortTokenBalance,
		uint256 _longTokenBalance
	) private lock {
		uint256 pauseShortTokenAmount = _shortTokenBalance.mul(valuePerShortToken).div(valuePerPauseToken);
		uint256 pauseLongTokenAmount = _longTokenBalance.mul(valuePerLongToken).div(valuePerPauseToken);

		// burn short & long token
		if (pauseShortTokenAmount != 0){
			_shortToken.burn(_to, _shortToken.balanceOf(_to)); 
		}
		if (pauseLongTokenAmount != 0){
			_longToken.burn(_to, _longToken.balanceOf(_to));
		}
		
		// update pause share info
		_pauseToken.mint(_to, pauseShortTokenAmount.add(pauseLongTokenAmount));
	}
	...
	function _redeemFromAAVE(address _asset, uint256 _amount) private {
		...
		if (_amount == 0){
			_amount = IERC20(aTokenAddress).balanceOf(address(this));
		}
	...
	function _getPrice() private view returns (uint256 price) {
		...
		uint256 price0 = priceOracle.getAssetPrice(bidToken);
		uint256 price1 = priceOracle.getAssetPrice(principalToken);
		
		price = price0.mul(PRECISION).div(price1);
	}
}
...
contract LosslessV2Storage {
	// token related
	// sponsor Token (1:1)
	mapping(address => address) internal getSponsorToken; // pool address => sponsorToken address
	// short token
	mapping(address => address) internal getShortToken; // pool address => shortToken address
	// long token
	mapping(address => address) internal getLongToken; // pool address => longToken address
	// pause token
	mapping(address => address) internal getPauseToken; // pool address => pauseToken address
    ...
    function _initializePool(
		...
		uint8 _decimals,
		address _pool
	) internal returns (address shortToken, address longToken, address pauseToken, address sponsorToken) {
		require(_pool != address(0), "LosslessV2Factory: ADDRESS PROVIDER ZERO_ADDRESS");
		
		// create a list of tokens for the new pool
		shortToken 	= 	_createToken(string(abi.encodePacked("short ", _tokenName)),	string(abi.encodePacked("short-", _tokenSymbol)), 	_decimals,	_pool);
		longToken 	= 	_createToken(string(abi.encodePacked("long ", _tokenName)),	string(abi.encodePacked("long-", _tokenSymbol)), 	_decimals,	_pool);
		pauseToken 	= 	_createToken(string(abi.encodePacked("pause ", _tokenName)), string(abi.encodePacked("pause-", _tokenSymbol)), 	_decimals,	_pool);
		sponsorToken = 	_createToken(string(abi.encodePacked("sponsor ", _tokenName)),	string(abi.encodePacked("sponsor-", _tokenSymbol)),	_decimals,	_pool);

		getSponsorToken[_pool] = sponsorToken;
		getShortToken[_pool] = shortToken;
		getLongToken[_pool] = longToken;
		getPauseToken[_pool] = pauseToken;

		// LosslessV2Pool(_pool).initialize(shortToken, longToken, pauseToken, sponsorToken);
	}
...
interface ILosslessV2Pool {
    ...
    */
	...
	event UpdateTokenValue(uint256 valuePerShortToken, uint256 valuePerLongToken, uint256 valuePerPauseToken);
	event AnnounceWinner(bool isShortLastRoundWinner, uint256 initialPrice, uint256 endPrice);
    ...
    */
	...
	function withdraw(uint256 shortTokenAmount, uint256 longTokenAmount) external;
