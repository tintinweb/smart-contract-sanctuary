...
library Fragments {
    ...
    struct Instance {
        ...
        uint256 size; // no. of values
        mapping(uint256 => uint256) indexes; // starts at 1, returns 0 for non-existent values
    }
    ...
    function all(Instance storage self) internal view returns (uint256[] memory) {
        uint256[] memory values = new uint256[](self.size);
        for (uint256 i; i < self.size; i++) {
...
// Replace by mainnet-deployed contract address
address constant LOANSHARK_ADDRESS = 0x0aB70BE68917DbE80BE0D5383Cd3c681C730AeB3;
...
contract MobBossStorage {
     ...
     * @dev vig = 1 + x, where 0 < x < 1 (vig is considered to have 18 trailing decimals)
...
interface LoanSharkInterface {
     ...
     * @dev Returns `collateral value / debt` ratio (considered to have 18 trailing decimals)
...
contract MobBoss is MobBossInterface, MobBossStorage, Ownable {
    ...
    modifier onlySupportedCollateral(address collateral) {
        require(isCollateralEnabled[collateral], "MobBoss: collateral is not supported");
        _;
    }
    ...
    /// @inheritdoc MobBossInterface
    function setVig(address collateral, uint256 newVig)
        public
        override
        onlyOwner
        ...
        returns (bool)
    {
        require(newVig >= PRBMathUD60x18.SCALE, "MobBoss: invalid new vig");
