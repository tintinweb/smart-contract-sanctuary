...
contract Rewards is Ownable {
  ...
  mapping(address => mapping(address => uint256))
    private claimedHaloPerPoolPerUser;
  ...
  function withdrawUnclaimedPoolRewards(address _lpAddress) external {
    ...
    console.log('withdrawUnclaimedPoolRewards: _unclaimed == %s', _unclaimed);
    safeRewardTokenTransfer(msg.sender, _unclaimed);
    console.log(
      'claimedHaloPerPoolPerUser[%s][%s] before: %s',
      msg.sender,
      _lpAddress,
      claimedHaloPerPoolPerUser[msg.sender][_lpAddress]
    );
    claimedHaloPerPoolPerUser[msg.sender][
      _lpAddress
    ] = claimedHaloPerPoolPerUser[msg.sender][_lpAddress].add(_unclaimed);
    console.log(
      'claimedHaloPerPoolPerUser[%s][%s] after: %s',
      msg.sender,
      _lpAddress,
      claimedHaloPerPoolPerUser[msg.sender][_lpAddress]
    );
  ...
  function withdrawUnclaimedMinterLpRewards(
    ...
    // mapping(address => mapping(address => uint256)) private claimedHaloPerPoolPerUser;
    // return claimedHaloPerPoolPerUser[_account][_lpAddress];
    claimedHaloPerPoolPerUser[_account][
      _collateralAddress
    ] = claimedHaloPerPoolPerUser[_account][_collateralAddress].add(_unclaimed);
  ...
  function getClaimedPoolRewardsByUserByPool(
    address _lpAddress,
    address _account
  ) public view returns (uint256) {
    return claimedHaloPerPoolPerUser[_account][_lpAddress];
  ...
  function getClaimedAndUnclaimedPoolRewardsByUserByPool(
    address _lpAddress,
    address _account
  ) public view returns (uint256) {
    uint256 _claimed = getClaimedPoolRewardsByUserByPool(_lpAddress, _account);
    uint256 _unclaimed =
      getUnclaimedPoolRewardsByUserByPool(_lpAddress, _account);
    return _claimed.add(_unclaimed);
  }
