...
contract SatoshiVerse is ERC721Enumerable, Ownable, ReentrancyGuard {
  ...
  mapping(address => mapping(string => uint16)) public tokensCount;
  mapping(address => uint16) public purchaseLimit;
  ...
  function seedPresaleWhiteList(address[] calldata users, string calldata tokenType, uint16[] calldata counts) external onlyOwner {
  ...
  function seedPublicWhiteList(address[] calldata users, uint16[] calldata counts) external onlyOwner {
    require(msg.sender != address(0), "Invalid user address");
    for(uint256 i = 0; i < users.length; i++) {
  ...
  function claim(uint256 claimedCount) external nonReentrant {
    ...
    uint16 genesisTokenCount = tokensCount[msg.sender]['genesis'];
    uint16 platinumTokenCount = tokensCount[msg.sender]['platinum'];
    uint16 goldTokenCount = tokensCount[msg.sender]['gold'];
    uint16 silverTokenCount = tokensCount[msg.sender]['silver'];
  ...
  function purchase(uint16 count) external payable nonReentrant {
