...
contract BorrowPool is Initializable, IBorrowPool, PoolSettingsManager {
    ...
    )
  {
    ...
    require(
      bondsIssuanceIndex <= (pool.ticks[rate].currentBondsIssuanceIndex + 1),
      Errors.BP_BOND_ISSUANCE_ID_TOO_HIGH
    );
    require(
      ((bondsIssuanceIndex <= pool.ticks[rate].currentBondsIssuanceIndex) &&
        pool.ticks[rate].adjustedRemainingAmount > 0) ||
        ((bondsIssuanceIndex > pool.ticks[rate].currentBondsIssuanceIndex) &&
          pool.ticks[rate].adjustedPendingAmount > 0),
