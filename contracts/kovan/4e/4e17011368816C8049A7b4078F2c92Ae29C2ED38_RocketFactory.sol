...
contract RocketFactory is IRocketFactory {
    ...
    function createPair(address tokenA, address tokenB) external override returns (address pair) {
        ...
        pair = address(new RocketPair{salt: salt}(token0, token1));
...
contract RocketPair is IRocketPair, RocketERC20 {
    ...
    modifier lock() {
        require(unlocked == 1, 'Rocket: LOCKED');
        unlocked = 0;
        _;
        unlocked = 1;
    }
    ...
    function initialize(address _token0, address _token1) external override {
        require(msg.sender == factory, 'Rocket: FORBIDDEN'); // sufficient check
        //token0 = _token0;
        //token1 = _token1;
    }
    ...
    function _update(uint balance0, uint balance1, uint112 _reserve0, uint112 _reserve1) private {
        require(balance0 <= type(uint112).max, 'Rocket: OVERFLOW');
        require(balance1 <= type(uint112).max, 'Rocket: OVERFLOW');
    ...
    function mint(address to) external override lock returns (uint liquidity) {
        ...
        require(liquidity > 0, 'Rocket: INSUFFICIENT_LIQUIDITY_MINTED');
        _mint(to, liquidity);
    ...
    function burn(address to) external override lock returns (uint amount0, uint amount1) {
        ...
        require(amount0 > 0 && amount1 > 0, 'Rocket: INSUFFICIENT_LIQUIDITY_BURNED');
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external override lock {
        require(amount0Out > 0 || amount1Out > 0, 'Rocket: INSUFFICIENT_OUTPUT_AMOUNT');
        (uint112 _reserve0, uint112 _reserve1,) = getReserves(); // gas savings
        require(amount0Out < _reserve0, 'Rocket: INSUFFICIENT_LIQUIDITY'); // gas savings
        require(amount1Out < _reserve1, 'Rocket: INSUFFICIENT_LIQUIDITY');
        ...
        require(amount0In > 0 || amount1In > 0, 'Rocket: INSUFFICIENT_INPUT_AMOUNT');
        { // scope for reserve{0,1}Adjusted, avoids stack too deep errors
        uint balance0AdjustedMulbalance1Adjusted = (balance0 * 1000 - amount0In * 2) * (balance1 * 1000 - amount1In * 2);
        require(balance0AdjustedMulbalance1Adjusted >= uint(_reserve0) * _reserve1 * 1000**2, 'Rocket: K');
        }
...
contract RocketERC20 is IRocketERC20 {
    ...
    function permit(address owner, address spender, uint value, uint deadline, uint8 v, bytes32 r, bytes32 s) external override {
        ...
        require(recoveredAddress != address(0) && recoveredAddress == owner, 'Rocket: INVALID_SIGNATURE');
