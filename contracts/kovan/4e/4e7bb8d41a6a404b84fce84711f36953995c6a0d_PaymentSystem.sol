...
//        require(sent, "Failed to send Ether");
  ...
  returns(uint256, uint256)
  {
    ...
    require(checkPriceRange(_fromToken, _toToken), "[Transaction rejected due to price fluctuations]"); // Check if it matches with live prices from oracle
  ...
  function checkPriceRange(address _fromToken, address _toToken) internal view returns(bool) {
    bool check1 = false;
    bool check2 = false;
    uint256 fromPriceOracle = uint(getLatestPrice(_fromToken));
    uint256 toPriceOracle = uint(getLatestPrice(_toToken));
    (uint256 fromPriceAMM, uint256 toPriceAMM) = latestPricesOfAMM(_fromToken, _toToken);
    if(fromPriceOracle > fromPriceAMM) {
      uint256 difference = fromPriceOracle.sub(fromPriceAMM);
      if(fromPriceAMM.div(difference) >= 100) check1 = true; // If less than 1% difference
    }
    else check1 = true;
    if(toPriceOracle < toPriceAMM) {
      uint256 difference = toPriceAMM.sub(toPriceOracle);
      if(toPriceAMM.div(difference) >= 100) check2 = true; // If less than 1% difference
    }
    else check2 = true;
