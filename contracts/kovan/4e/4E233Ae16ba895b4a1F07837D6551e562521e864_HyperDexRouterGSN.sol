...
contract HyperDexRouterGSN is BaseRelayRecipient, Ownable {
    ...
    uint256 public feeBeneficiary = 5; // 0.5%
    uint256[4] public feeReferrals = [1, 2, 3, 4];  // 0.1%, 0.2%, 0.3%, 0.4%
    ...
    function multiRoute(
        ...
        _payFees(feeToken, _beneficiary, inputAmount / 1000 * feeBeneficiary);
        for (uint256 i = 0 ; i < referrals.length; i++) {
            if (referrals[i] != _NULL_ADDRESS && feeReferrals[i] != 0) {
                _payFees(feeToken, payable(referrals[i]), inputAmount / 1000 * feeReferrals[i]);
            }
        }
        ...
        if (inputToken == _ETH_ADDRESS) {
            if (feeToken == _ETH_ADDRESS) {
                uint256 inputAmountFee = inputAmount.add(feeBeneficiary);

                for (uint256 i = 0 ; i < referrals.length; i++) {
                    if (referrals[i] != _NULL_ADDRESS && feeReferrals[i] != 0) {
                        inputAmountFee = inputAmountFee.add(feeReferrals[i]);
                    }
                }
