...
import "./OnidexswapRouter.sol";
import "./libraries/OnidexERC20.sol";
import "./interfaces/IAggregationExecutor.sol";
import "./interfaces/IOnidex.sol";

contract AggregationRouter is Ownable, OnidexswapRouter {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    using OnidexERC20 for IERC20;
    ...
    function discountedSwap(
        ...
        external
        payable
        returns (uint256 returnAmount, uint256 gasLeft, uint256 oniSpent)
    {
        uint256 initialGas = gasleft();

        address oniSource = address(0);
        if (desc.flags & _BURN_FROM_MSG_SENDER != 0) {
            oniSource = msg.sender;
        } else if (desc.flags & _BURN_FROM_TX_ORIGIN != 0) {
            oniSource = tx.origin; // solhint-disable-line avoid-tx-origin
        } else {
            revert("Incorrect oni burn flags");
        }
        ...
        (IOnidex oni, uint256 amount) = caller.calculateGas(initialGas.sub(gasleft()), desc.flags, msg.data.length);
        if (amount > 0) {
            oniSpent = oni.freeFromUpTo(oniSource, amount);
        }
        gasLeft = gasleft();
    }
        ...
        returns (uint256 returnAmount, uint256 gasLeft)
    {
        ...
        uint256 initialSrcBalance = (flags & _PARTIAL_FILL != 0) ? srcToken.onidexBalanceOf(msg.sender) : 0;
        uint256 initialDstBalance = dstToken.onidexBalanceOf(dstReceiver);
        ...
        returnAmount = dstToken.onidexBalanceOf(dstReceiver).sub(initialDstBalance);

        if (flags & _PARTIAL_FILL != 0) {
            spentAmount = initialSrcBalance.add(desc.amount).sub(srcToken.onidexBalanceOf(msg.sender));
    ...
    function rescueFunds(IERC20 token, uint256 amount) external onlyOwner {
        token.onidexTransfer(payable(msg.sender), amount);
    }
...
contract OnidexswapRouter is Permitable {
    ...
    function onidexswapWithPermit(
        ...
        return onidexswap(srcToken, amount, minReturn, pools);
    }
...
library OnidexERC20 {
    ...
    function onidexBalanceOf(IERC20 token, address account) internal view returns (uint256) {
        if (isETH(token)) {
    ...
    function onidexTransfer(IERC20 token, address payable to, uint256 amount) internal {
        if (amount > 0) {
            if (isETH(token)) {
                to.transfer(amount);
            } else {
    ...
    function onidexApprove(IERC20 token, address to, uint256 amount) internal {
...
interface IGasDiscountExtension {
    function calculateGas(uint256 gasUsed, uint256 flags, uint256 calldataLength) external view returns (IOnidex, uint256);
}