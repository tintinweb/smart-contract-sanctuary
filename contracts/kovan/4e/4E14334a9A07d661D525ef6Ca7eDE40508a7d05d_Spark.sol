...
/*
    1.1.2 - to be tested 0x6d932f464bA83dD47d179d247eFFE4B989eAFe73
    1. seperated computeInterestWithPrecision to computeInterestWithPrecisionByStakedTime and computeInterestWithPrecisionByDays
    2. added tryUpdate to have user interactions call updated
    3. added onlyOwner function: 1. halt or resume stake or claim 2. switch Oracle address
*/
...
contract Spark is Ownable {
    ...
    modifier tryUpdate {
        ...
        if (timeElapsed >= _priceOracle.period()) {
            _priceOracle.update();
        }
		_;
	}
     ...
     */
    function switchOracle(address newOracle) external onlyOwner {
    ...
    function stake(uint amount) external tryUpdate returns (uint64 id) {
        // sanity check
        require(stakeAllowed, 'stake Error: stake halted');
    ...
    function claim(uint index) public tryUpdate returns (uint) {
        // sanity check
        require(claimAllowed, 'claim Error: claim halted');
