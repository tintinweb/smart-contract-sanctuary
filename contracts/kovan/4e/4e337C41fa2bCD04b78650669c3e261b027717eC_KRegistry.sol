...
interface IKFactory {
    function isKPool(address b) external view returns (bool);
}
...
contract KRegistry {
    ...
    function addPoolPair(address pool, address token1, address token2) public returns(uint256 listed) {
        ...
        require(bfactory.isKPool(pool), "ERR_NOT_BPOOL");

        uint256 swapFee = IKPool(pool).getSwapFee();
        ...
        _pools[key].pools.add(pool);
         ...
         if (token1 < token2) {
            _infos[pool][key] = PoolPairInfo({
                weight1: uint80(IKPool(pool).getDenormalizedWeight(token1)),
                weight2: uint80(IKPool(pool).getDenormalizedWeight(token2)),
                swapFee: uint80(swapFee),
                liq: uint256(0)
            });
        } else {
            _infos[pool][key] = PoolPairInfo({
                weight1: uint80(IKPool(pool).getDenormalizedWeight(token2)),
                weight2: uint80(IKPool(pool).getDenormalizedWeight(token1)),
                swapFee: uint80(swapFee),
                liq: uint256(0)
            });
        }
        ...
        internal view returns(uint256[] memory effectiveLiquidity)
    {
        ...
        for (uint i = 0; i < pools.length; i++) {
            ...
            if (token1 < token2) {
                ...
                effectiveLiquidity[i] = effectiveLiquidity[i].mul(IKPool(pools[i]).getBalance(token2));
        ...
        public returns(uint256[] memory effectiveLiquidity)
    {
        ...
        for (uint i = 0; i < pools.length; i++) {
            ...
            if (token1 < token2) {
                ...
                _infos[pools[i]][key].liq = _infos[pools[i]][key].liq.mul(IKPool(pools[i]).getBalance(token2));
