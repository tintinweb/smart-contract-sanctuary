...
// File contracts/interfaces/ICWProjectFactory.sol




interface ICWProjectFactory {
...
interface ICWProject {
    ...
    function isNominationed(address _nomination) external view returns (bool);
}


// File contracts/external/token/ERC20.sol
...
// File contracts/external/token/ERC20Burnable.sol
...
contract CWProject is ICWProject {
    ...
    /// @notice project ipfsHash
    bytes32 public override ipfsHash;

    /// @notice ??? what is the right description of this?
    ...
    /// @notice address to the projectFactory
    address public override factory;

    /// @notice address to beneficiary
    ...
    mapping(address => bool) public override isNominationed;
    ...
    function _syncFunding() internal {
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            if (totalSupply > 0) {
                totalFunding += (totalSupply
                        * ICWProjectFactory(factory).priceOralceAggregator().getPriceInUSD(token))
    ...
    function addFunding(address _token, uint256 amount) internal {
        if (block.timestamp - lastFundingSyncTime >= fundingEpoch) {
            ...
            totalFunding += (amount
                * ICWProjectFactory(factory).priceOralceAggregator().getPriceInUSD(_token))
    ...
    function back(
        address _token, uint256 _value
    ) external override onlyAcceptedToken(_token) onlyActiveProject returns (bool) {
    ...
    function withdraw() public override noRewithdraw onlyFinishedProject returns (bool) {
        require(msg.sender == beneficiary, "PJ: NOT_BENEFICIARY");
        require(funded, "PJ: NOT_FUNDED");

        uint256 length = acceptedTokens.length;
        uint256 protocolFee = ICWProjectFactory(factory).protocolFee();
        uint256 withdrawPercent = 10000 - curatorFee - protocolFee;
        address payable protocolFeeTo = ICWProjectFactory(factory).feeTo();
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            if (amount > 0) {
                ...
                if (token == ETH_ADDRESS) {
                    (bool sent, ) = beneficiary.call{value: withdrawAmount}("");
                    (bool sent_, ) = protocolFeeTo.call{value: protocolFeeAmount}("");
                    require(sent && sent_, "PJ: ETH_WITHDRAW_FAILED");
                } else {
                    IERC20(token).safeTransfer(beneficiary, withdrawAmount);
                    IERC20(token).safeTransfer(protocolFeeTo, protocolFeeAmount);
                }
            }
        }

        emit Withdraw(msg.sender, (totalFunding * withdrawPercent) / 10000);
        return true;
    }
    ...
    function redeemBToken(
        address _token,
        uint256 _amount
    ) public override onlyAcceptedToken(_token) onlyFinishedProject returns (bool) {
        ...
        uint256 redeemPercent = 10000 -
            (ICWProjectFactory(factory).protocolFee());
        uint256 backedAmount = (_amount * redeemPercent) / 10000;
        uint256 protocolFeeAmount = _amount - backedAmount;

        if (_token == ETH_ADDRESS) {
            (bool sent, ) = payable(msg.sender).call{value: backedAmount}("");
            (bool sent_, ) = protocolFeeTo.call{value: protocolFeeAmount}("");

            require(sent && sent_, "PJ: REDEEM_ETH_FAILED");
        } else {
            IERC20(_token).safeTransfer(msg.sender, backedAmount);
            IERC20(_token).safeTransfer(protocolFeeTo, protocolFeeAmount);
        }
    ...
    function redeemCToken(
        ...
        if (!lockedWithdraw) {
            curating = curating * curatorFee;
            restTotalCurating = restTotalCurating * 10000;
        }
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            if (token == ETH_ADDRESS) {
                uint256 bonusAmount = ((address(this).balance -
                    IERC20(getCToken[token]).totalSupply()) * curating) /
                    restTotalCurating;
                if (bonusAmount > 0) {
                    (bool sent, ) = payable(msg.sender).call{
                        value: bonusAmount
    ...
    function setIpfsHash(bytes32 _ipfsHash) external override onlyCreator {
        ipfsHash = _ipfsHash;
    }

    /// @notice set project CW url
    function setCwUrl(bytes32 _cwUrl) external override onlyCreator {
        cwUrl = _cwUrl;
    }

    /// @notice set project Beneficary
    function setBeneficiary(address _beneficiary) external override onlyCreator {
        beneficiary = _beneficiary;
    }
    ...
    function addNominations(
        ...
        //     emit Nominated(projectId, _nominations[i]); // @TODO: determine second param
        // }
    }

    /// @notice remove nominations
    function removeNominations(
        address[] memory _nominations
    ) external view override onlyCreator {
        ...
        onlyCreator
    {
        for (uint256 i = 0; i < _tokens.length; i++) {
            ...
            if (!isAcceptedToken[_tokens[i]]) {
                ...
                address bToken = ICWProjectFactory(factory)
                .cwTokenImp()
                ...
                address cToken = ICWProjectFactory(factory)
                .cwTokenImp()
