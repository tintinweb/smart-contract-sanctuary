...
contract RemitVaultFarmEth is Ownable {
    ...
    uint public cliffTime = 4 minutes;
    ...
    function updateAccount(address account) private {
        distributeDivs();
        uint pendingDivs = _getPendingDivs(account);
        if (pendingDivs > 0) {
    ...
    function _getPendingDivs(address _holder) internal view returns (uint) {
    ...
    function getPendingDivs(address _holder) external view returns(uint256){
        if (totalTokens == 0) return 0;
        uint amount = getPendingDisbursement();
        uint256 _totalDivPoints = totalDivPoints.add(amount.mul(pointMultiplier).div(totalTokens));
        
        uint newDivPoints = _totalDivPoints.sub(lastDivPoints[_holder]);
        uint depositedAmount = depositedTokens[_holder];
        uint pendingDivs = depositedAmount.mul(newDivPoints).div(pointMultiplier);
