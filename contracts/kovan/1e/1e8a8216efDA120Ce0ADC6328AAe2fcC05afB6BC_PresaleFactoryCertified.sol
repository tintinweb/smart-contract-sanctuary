...
contract PresaleCertified is ReentrancyGuard {
    ...
    uint256[4] private tiersTimes = [1200, 900, 600, 300]; // 1h55m-> 1h45m -> 1h30m -> 1h PROD: [6900, 6300, 5400, 3600]
    //uint256 private lpDaySeconds = 1 minutes; // one day PROD: 1 days
    ...
    function setCertifiedAddition(
        ...
        if (len > 0) {
            ...
            for (uint256 i = 0; i < len; i++) {
                if (_whitelist[i] != generalInfo.creator /* && !whitelistTier[_whitelist[i]] */)
    ...
    function invest(
        ...
        if (certifiedAddition.whitelist.length > 0) {
            ...
            if (nowTime < generalInfo.openTimePresale + tiersTimes[3]) {
                ...
                require(_stakedAmount >= stakingTiers[2], "TIER3");
                ...
                require(_stakedAmount >= stakingTiers[3], "TIER2");
                ...
                require(_stakedAmount >= stakingTiers[4], "TIER1");
                tokensLeft = generalInfo.tokensForSaleLeft;
            }
        }
        ...
        nonReentrant
    {
        ...
        if (certifiedAddition.nativeToken == address(0)) {
            ...
            TransferHelper.safeTransfer(certifiedAddition.nativeToken, to, amount);
        }
    }
    ...
    function addLiquidity() external presaleIsNotCancelled nonReentrant {
        ...
        uniswapInfo.unlockTime =
            block.timestamp +
            (uniswapInfo.lpTokensLockDurationInDays * 1 minutes); //PROD: 60*24*60
        ...
        liquidityAdded
    {
        ...
        if (address(0) == certifiedAddition.nativeToken) {
            ...
            collectedBalance = IERC20(certifiedAddition.nativeToken).balanceOf(address(this));
            TransferHelper.safeTransfer(certifiedAddition.nativeToken, generalInfo.creator, collectedBalance);
        }
        ...
        onlyPresaleCreator
    {
        ...
        if (certifiedAddition.whitelist.length == 0) {
            require(
                _newOpenTime > block.timestamp + lessLib.getRegistrationTime(),
                "W.PARAMS"
            );
        }
        if (certifiedAddition.liquidity) {
...
contract PresaleFactoryCertified is ReentrancyGuard {
    ...
    function createPresaleCertified(
        ...
        if (_addition.whitelist.length == 0) {
            require(block.timestamp + safeLibrary.getRegistrationTime() + 60*3 <= _info.openTime, "TIME"); //PROD: change 60*3 (approval time)
        } else {
            require(block.timestamp + 60*3 <= _info.openTime, "TIME"); //PROD: change 60*3 (approval time)
        }
        require(
            750 < _info.closeTime - _info.openTime && //PROD: 6900
        ...
        //IERC20 _token = IERC20(_info.tokenAddress);
        ...
        //uint8 decimals = (address(_addition.nativeToken) == address(0)) ? 18 : ERC20(_addition.nativeToken).decimals();
        ...
        require(
            IERC20(_info.tokenAddress).transferFrom(msg.sender, address(presale), tokenAmounts[2]),
            "T.TRANSFER"
        );
