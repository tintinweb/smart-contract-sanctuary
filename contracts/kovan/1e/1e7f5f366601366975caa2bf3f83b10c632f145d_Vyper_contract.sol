...
def updateRewards():
    ...
    self.paidPerToken += totalToPaid / max(self.totalStaked, 1) * 10**6
...
def withdraw(amount: uint256):
    ...
    rewards: uint256 = convert(convert(self.paidPerToken * self.holdings[msg.sender] - self.userRewardsPaid[msg.sender], int128) - self.compactor[msg.sender], uint256)/10**6
...
def bal(user: address) -> (uint256, uint256):
    ...
    return (
        self.holdings[user],
        convert(convert(totalToPaid / self.totalStaked * self.holdings[msg.sender] - self.userRewardsPaid[msg.sender], int128) - self.compactor[msg.sender], uint256)
        )
