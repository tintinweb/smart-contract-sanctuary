...
contract Staker {
    ...
    // Balances of users' stacked funds
    ...
    event Executed(uint256 balance);
    ...
    /**
     * @notice Modifier that require the contract balance to met threshold or not
     * @param requiredMet Check if the threshold has been met or not
     */
    modifier thresholdMet(bool requiredMet) {
        if (requiredMet) {
            require(
                address(this).balance >= threshold,
                "Threshold is not met yet"
            );
        } else {
            require(
                address(this).balance < threshold,
    ...
    /**
     * @notice Send staked ETH to contract and updates balance
     */
    function stake() public payable deadlineReached(false) {
        // Update user's fund balance
        balances[msg.sender] += msg.value;

        // Send event to indicate user's address and amount of funds staked
        emit Stake(msg.sender, msg.value);
    }

    /**
     * @notice Sends contract balance to external contract after checking deadline has reached,
     * threshold has been met, and external contract has not completed the staking.
     */
    function execute()
        public
        stakeNotCompleted
     ...
     */
    function withdraw() public deadlineReached(true) thresholdMet(false) {
        // Check user funds staked in contract
        uint256 userBalance = balances[msg.sender];

        require(userBalance > 0, "No balance to withdraw");

        // Set user balance to 0 to prevent repeated withdrawals
        balances[msg.sender] = 0;

        // Transfer balance back to the user
        (bool sent, ) = msg.sender.call{value: userBalance}("");

        // If transfer was unsuccessful, reinstate user balance
        if (!sent) {
    ...
    /**
     * @notice Resets the deadline
     * @param secsToEnd Time to end of staking in seconds
     */
    function resetDeadline(uint256 secsToEnd) public {
        deadline = block.timestamp + secsToEnd;
    }
}
