...
contract BuryShib is ERC20("BuryShib", "xSHIB"){
    ...
    constructor(IERC20 _shib, IERC20 _bone) public {
        shib = _shib;
        bone = _bone;
    }
	...
	require(((_amount / (1000000 * 10**18)) > 0) && ((_amount % (1000000 * 10**18) ) == 0), "Shib must be stake in millions");
        ...
        user.rewardBoneDebt = user.amount.mul(poolInfo[0].accRewardPerShare).div(1e12);
        user.rewardEthDebt = user.amount.mul(poolInfo[1].accRewardPerShare).div(1e12);
    }
    ...
    function sendRewards(uint256 _pid, uint256 _share) internal {
        ...
        uint256 what;
        if(_pid == 0) {
            what = user.amount.mul(pool.accRewardPerShare).div(1e12).sub(user.rewardBoneDebt);
	        pool.token.transfer(msg.sender, what);
	        pool.lastRewardBalance = pool.token.balanceOf(address(this));
	}
        else {
            what = user.amount.mul(pool.accRewardPerShare).div(1e12).sub(user.rewardEthDebt);
	    msg.sender.call{ value: what }("");
	    address(this).balance;
	}
        
        if(_pid == 0)
            user.rewardBoneDebt = user.amount.mul(pool.accRewardPerShare).div(1e12);
        else
            user.rewardEthDebt = user.amount.mul(pool.accRewardPerShare).div(1e12);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (_pid == 0)
            rewardBalance = pool.token.balanceOf(address(this));
        else
