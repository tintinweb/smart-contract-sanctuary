...
contract Swiss {
    ...
    function getParticipantNames(uint32 tournamentId) external view tournamentExists(tournamentId)
        returns(string[] memory) {
        tournamentData storage currentTournament = tournaments[tournamentId];
        string[] memory participantnames = new string[](currentTournament.participantCount);
        for(uint32 i; i < currentTournament.participantCount; i++) {
            participantnames[i] = currentTournament.participantList[i].username; 
        }
