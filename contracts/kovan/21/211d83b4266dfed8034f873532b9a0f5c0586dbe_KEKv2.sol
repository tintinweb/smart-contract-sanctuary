...
contract Ownable is Context {
    ...
    event UniswapPairUpdated(address indexed uniswapV2Pair, address indexed newPair);
...
contract KEKv2 is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"KEKv2";
    string private constant _symbol = unicode"KEKv2";
    ...
    constructor (address payable FeeWallet, address payable TeamWallet) {
        ...
        if(from != owner() && to != owner()) {
            ...
            if(!inSwap && from != uniswapV2Pair && tradingOpen) {
                ...
                if(contractETHBalance > 0) {
                    sendETHToFee(address(this).balance.div(10).mul(7));
                    swapAndLiquify(address(this).balance.div(10).mul(3));
                }
            }
        }
        bool takeFee = true;
    ...
    function sendETHToFee(uint256 amount) private {
        _FeeWallet.transfer(amount.div(10).mul(2));
        _TeamWallet.transfer(amount.div(10).mul(4));
    }
    ...
    function setPair(address newPair) public virtual onlyOwner() {
        emit UniswapPairUpdated(uniswapV2Pair, newPair);
