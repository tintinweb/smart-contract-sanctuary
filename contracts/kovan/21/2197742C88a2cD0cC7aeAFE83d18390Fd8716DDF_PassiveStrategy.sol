...
library FullMath {
    /// @notice Calculates floor(aГ—bГ·denominator) with full precision. Throws if result overflows a uint256 or denominator == 0
    ...
    /// @notice Calculates ceil(aГ—bГ·denominator) with full precision. Throws if result overflows a uint256 or denominator == 0
    /// @param a The multiplicand
    /// @param b The multiplier
    /// @param denominator The divisor
    /// @return result The 256-bit result
    ...
    ReentrancyGuard
{
    ...
    address NFT_TOKEN_ADDRESS; // Address, where InvitationalErc721 contract located
    //InvitationalErc721 public burnToken = InvitationalErc721(NFT_TOKEN_ADDRESS);

    mapping(address => bool) isNotFirstDeposit; // Check if it is first deposit (If True, start timer)
    mapping(address => uint256) startTime;

    //mapping (address => uint256) NftOwner; //
    //uint256 tokenNftId;

    mapping(address => bool) isGetGift; //

    uint256 daysAfter; // the period of time after which the user will be able to donate the token
    //uint256 startTime; // Countdown point after 1st deposit
    ...
    constructor(
        ...
        NFT_TOKEN_ADDRESS = _nft;

        // NftOwner[_msgSender()] = _NftId;
        //tokenNftId = _NftId;

        //isFirstDeposit[_msgSender()] = true;
        isGetGift[_msgSender()] = false;
    ...
    {
        //checkIfGotTokenAndBalance(_tokenNftId);
        require(
            (InvitationalErc721(NFT_TOKEN_ADDRESS).ownerOf(_tokenNftId) == _msgSender()),
            "You don't have invitational NFT token or current deposit"
        );

        //checkIfFirstDeposit();
        if (!isNotFirstDeposit[_msgSender()]) {
            startTime[_msgSender()] = block.timestamp;
            isNotFirstDeposit[_msgSender()] = true;
        }
    ...
    function withdraw(
        ...
        //require(!(block.timestamp >= startTime[_msgSender()] + daysAfter));
        if (balanceOf(_msgSender()) == 0 && !(block.timestamp >= startTime[_msgSender()] + daysAfter)) {
            InvitationalErc721 burnToken = InvitationalErc721(NFT_TOKEN_ADDRESS);
            burnToken.burn(_tokenNftId);
            isNotFirstDeposit[_msgSender()] = false;
            startTime[_msgSender()] = 0;
        }
    ...
    ///////////////
    //NOT CORRECT
    /* function checkIfFirstDeposit() private {
        if (isFirstDeposit[_msgSender()] == true) {
            startTime = block.timestamp;
            isFirstDeposit[_msgSender()] = false;
        }
    } */

    /* function checkIfOwnToken(uint256 _tokenNftId) private {
        require((InvitationalErc721(NFT_TOKEN_ADDRESS).ownerOf(_tokenNftId) == _msgSender()), "You don't have invitational NFT");
    } */

    /* function checkIfGotTokenAndBalance(uint256 _tokenNftId) private {
        require(
            (InvitationalErc721(NFT_TOKEN_ADDRESS).ownerOf(_tokenNftId) == _msgSender()) ||
                ((getBalance0() == 0) && (getBalance1() == 0)),
            "You don't have invitational NFT token or current deposit"
        );
    } */

    /* function checkIfEarlyCheckout(uint256 _tokenNftId) private {
        require(!(block.timestamp >= startTime + daysAfter));
        if ((getBalance0() == 0) && (getBalance1() == 0)) {
            InvitationalErc721 burnToken = InvitationalErc721(NFT_TOKEN_ADDRESS);
            burnToken.burn(_tokenNftId);
        }
    } */

    function setDaysAfter(uint256 setPeriodInDays) external onlyGovernance {
        daysAfter = setPeriodInDays;
    }
    ...
    function shareNftToken(string calldata _tokenURI, address _to)
        external
        
        nonReentrant
    {
        //require(_to != _msgSender(), "You can't send NFT to yourself");
        require(_to != address(0), "You can't send NFT to zero address");

        require(
            (block.timestamp >= startTime[_msgSender()] + daysAfter),
            "The period of time before receiving Invitational NFT has not passed"
        );

        require(isGetGift[_msgSender()] == false, "You already got Invitational NFT to share");

        InvitationalErc721(NFT_TOKEN_ADDRESS).mint(_tokenURI, _to);

        //mint(_tokenURI, _to);
        isGetGift[_msgSender()] = true;
    }
}
...
contract PassiveStrategy is IStrategy {    
    ...
    function shouldRebalance() public view override returns (bool) {
        ...
        if (
            tick < TickMath.MIN_TICK + baseThreshold + tickSpacing ||
            tick > TickMath.MAX_TICK - baseThreshold - tickSpacing
        ) {
            return false;
        }
