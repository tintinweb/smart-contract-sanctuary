...
contract LockPosition {
    ...
    modifier onlyFundInvestor(uint256 period) {
        ...
        require(isFundInvestor, "not fund investor");
        _;
    }
    ...
    function updateRecipe(address _recipient) external onlyOwner {
        recipient = _recipient;
    }
    ...
    function invest(uint256 period, uint256 amount) external onlyFundInvestor(period) {
        ...
        if (surplusVolume < amount) {
            amount = surplusVolume;
        }
        require(amount > 0, 'maximumSales');

        IERC20(salesPeriod.pay).safeTransferFrom(msg.sender, recipient, amount);
        salesPeriod.salesVolume = salesPeriod.salesVolume.add(amount);
