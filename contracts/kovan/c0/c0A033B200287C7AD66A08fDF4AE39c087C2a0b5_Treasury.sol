...
contract Treasury is ITreasury, AccessControl, ReentrancyGuard, IValidator {
    ...
    modifier notLiquidatingVault(address account, address fxToken) {
        require(!isLiquidatingVault(account, fxToken), "VL");
        _;
    }
    ...
    ReentrancyGuard
{
        ...
        nonReentrant
    {
        ...
        Treasury().increaseDebtPosition(msg.sender, tokenAmount, token);
        assert(
    ...
    function burn(
        ...
        Treasury().decreaseDebtPosition(msg.sender, amount, token);
        assert(
            Treasury().getDebt(msg.sender, token) ==
    ...
    /**
     * @notice redeems tokens from a vault at a 1:1 asset/collateral ratio
     * @dev token must have been pre-approved for transfer with input amount
     * @param amount the amount of fxTokens to redeem with
     * @param token the fxToken to redeem with
     * @param from the account to redeem from
    ...
    function redeem(
        ...
        // Vault CR must be below the max. for redeem.
        require(
            !VaultLibrary().doesMeetRatio(
                from,
                token,
                IVaultLibrary.CollateralRatioType.Redeem
            ),
            "CR"
        );
        ...
        (
            ...
            bool metAmount
        ) = VaultLibrary().getCollateralForAmount(from, token, amountEth);
        require(metAmount, "CA");
        for (uint256 i = 0; i < collateralTokens.length; i++) {
            // Reduce vault debt and withdraw collateral to user.
            Treasury().decreaseDebtPosition(from, amount, token);
            Treasury().forceWithdrawCollateral(
                from,
                collateralTokens[i],
                msg.sender,
        ...
        returns (uint256 feePrice)
    {
        ...
        uint256 available =
            VaultLibrary().getFreeCollateralAsEth(msg.sender, token).add(
                unitPrice
                    .mul(tokenAmount)
                    .mul(VaultLibrary().getVaultMinimumRatio(msg.sender, token))
                    .div(1 ether)
                    .div(100)
            );
    }
