...
contract LPFarming is Context, Ownable, ReentrancyGuard {
     ...
     */
    function manualEpochInit(uint128 epochId) public onlyOwner {
    ...
    function getEpochRewardAmount(uint128 epochId) public pure returns (uint256) {
        if (epochId <= 0) {
            return 0;
        }
    ...
    function claimByIndex(uint128 index) public returns (bool) {
        uint256 total;
        uint128 lastEpochId;
        (total, lastEpochId) = getClaimAmountByIndex(_msgSender() ,index);
