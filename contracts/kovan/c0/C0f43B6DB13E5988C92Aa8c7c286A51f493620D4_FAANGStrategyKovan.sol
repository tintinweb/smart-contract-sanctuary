...
contract FAANGStrategyKovan is Ownable {
    ...
    uint mirustLpTokenPrice;
    mapping(address => uint) lpTokenPrice;
    ...
    constructor(
        ...
        curveIds[0xb7a4F3E9097C08dA09517b5aB877F7a917224ede] = 2;
        curveIds[0x07de306FF27a2B630B1141956844eB1552B956B5] = 3;
        curveIds[0x4F96Fe3b7A6Cf9725f59d353F723c1bDb64CA6Aa] = 1;
...
/*         //DAI
        IERC20(0x4F96Fe3b7A6Cf9725f59d353F723c1bDb64CA6Aa).approve(address(router), type(uint256).max);
        IERC20(0x4F96Fe3b7A6Cf9725f59d353F723c1bDb64CA6Aa).approve(address(curveFi), type(uint256).max);
        //USDC
        IERC20(0xb7a4F3E9097C08dA09517b5aB877F7a917224ede).approve((address(router)), type(uint).max);
        IERC20(0xb7a4F3E9097C08dA09517b5aB877F7a917224ede).approve((address(curveFi)), type(uint).max);
        //USDT
        IERC20(0x07de306FF27a2B630B1141956844eB1552B956B5).safeApprove(address(router), type(uint).max);
        IERC20(0x07de306FF27a2B630B1141956844eB1552B956B5).safeApprove((address(curveFi)), type(uint).max); */
...
/*             mAssetsTokens[i].approve(address(router), type(uint).max);
    ...
    function withdraw(uint256 _amount, IERC20 _token) external onlyVault {
        ...
        for (uint256 i = 0; i < mAssets.length; i++) {
            updateLpTokenPrice(address(mAssets[i].lpToken));
    ...
    function withdrawFromMirUstPool(uint _amount, uint _valueInPool, bool _withdrawAll) internal {
        ...
        if(reInvestedMirUstPooltoken != 0) {  
            ...
            if(_withdrawAll == true) {
                ...
                lpTokenTotalValue = mirustLpTokenPrice.mul(reInvestedMirUstPooltoken).div(1e18);
                valueToRemove = lpTokenTotalValue.mul(_amount).div(_valueInPool);
                amountToWithdraw = reInvestedMirUstPooltoken.mul(valueToRemove).div(lpTokenTotalValue);
    ...
    function getDataFromLPPool(address _lpToken, uint _amount, uint _valueInPool) internal view returns (uint amounOfLpTokenToRemove){
        ...
        // Find how much percentage is the withdrawl amount to totalDepositedAmount
        // withdraw that percentage of LPTokens
        uint lpTokenBalance = userTotalLPToken[IERC20(_lpToken)];        
        uint lpTokenTotalValue = lpTokenPrice[address(_lpToken)].mul(lpTokenBalance).div(1e18);
        uint valueToRemove = lpTokenTotalValue.mul(_amount).div(_valueInPool);

        amounOfLpTokenToRemove = lpTokenBalance.mul(valueToRemove).div(lpTokenTotalValue);
    ...
    function updateLpTokenPrice(address _lpTokenAddress) internal {
        //get reserves
        (uint reserve0, uint reserve1,) = IUniswapV2Pair(_lpTokenAddress).getReserves();
        uint totalLpTokenSupply = IUniswapV2Pair(_lpTokenAddress).totalSupply();

        address _token0 = IUniswapV2Pair(_lpTokenAddress).token0();
        address _token1 = IUniswapV2Pair(_lpTokenAddress).token1();
        
        address[] memory path = new address[](2);
        path[0] = _token0 == address(ust) ? _token1 : _token0;
        path[1] = address(ust);
        
        uint[] memory priceInUst = router.getAmountsOut(1e18, path);

        uint valueOfPool =  _token0 == address(ust) ? (reserve1.mul(priceInUst[1])).add(reserve0) : (reserve0.mul(priceInUst[1])).add(reserve1);
        lpTokenPrice[_lpTokenAddress] = valueOfPool.div(totalLpTokenSupply);
    }

    function updateMirUstlpTokenPrice() internal {
        (uint reserve0, uint reserve1,) = IUniswapV2Pair(mirustPairAddress).getReserves();
        uint totalLpTOkenSupply = IUniswapV2Pair(mirustPairAddress).totalSupply();
        ...
        mirustLpTokenPrice = ((reserve0.mul(priceInUst[1])).add(reserve1)).div(totalLpTOkenSupply);
    }
