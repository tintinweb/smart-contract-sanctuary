...
contract BettingPool is Owned {
    ...
    function _start(address _pool) internal {
        IBettingContract(_pool).start(address(priceContract));
    }
...
interface IBettingContract {
  ...
  function start(address _priceContract) external;
...
interface IPriceContract {
  function updatePrice(uint256 _time, address _tokens) external returns(bytes32);
