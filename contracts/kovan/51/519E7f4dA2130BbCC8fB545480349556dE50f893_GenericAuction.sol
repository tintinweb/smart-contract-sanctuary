 ...
 */
contract GenericAuction is VersionedInitializable, AuctionBase, WETHBase, ReentrancyGuard {
     ...
     * @param auctionId The auction identifier.
     */
    event Reclaimed(address indexed nft, uint256 indexed nftId, uint256 auctionId);
    ...
    function claimWonNFT(address nft, uint256 nftId) external nonReentrant whenNotPaused {
        ...
        DataTypes.DistributionData[] memory distribution = _nftData[nft][nftId].distribution;
        ...
        emit WonNftClaimed(nft, nftId, auctionIdCached, winner);
    }
    ...
    function reclaimEndedAuction(address nft, uint256 nftId) external nonReentrant whenNotPaused {
        ...
        emit Reclaimed(nft, auctionIdCached, nftId);
    }
    ...
    function _bid(
        ...
        if (currentBidder != address(0)) {
            if (address(currency) == address(WETH)) {
                ...
                (bool success, ) = currentBidder.call{value: currentBid, gas: 0}(new bytes(0));
                if (!success) {
