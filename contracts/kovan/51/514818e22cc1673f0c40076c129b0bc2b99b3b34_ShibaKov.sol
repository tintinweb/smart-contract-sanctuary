...
contract ShibaKov is Context, IERC20, Ownable {
    using SafeMath for uint256;
    string private constant _name = "CowTest";
    string private constant _symbol = "COWTEST";
    ...
    uint256 private _maxTxPct = 3000000000 * 10**9; //0.3%
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            ...
            uint256 maxTxAmount = _tTotal.mul(_maxTxPct).div(10**2);
            ...
            if (!inSwap && from != uniswapV2Pair && swapEnabled) {
                require(amount <= balanceOf(uniswapV2Pair).mul(2).div(100) && amount <= maxTxAmount);
    ...
    function maxTxAmount() public view returns (uint256) {
       return _tTotal.mul(_maxTxPct).div(10**2);
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        _transferFees(tDynamic);
        _reflectFee(rFee, tDynamic);
        emit Transfer(sender, recipient, tTransferAmount);
    }

    function _transferFees(uint256 tDynamic) private {
        uint256 currentRate = _getRate();
        ...
        uint256 rPool = tDynamic.mul(currentRate);
    ...
    function revertLaunchTxPercent() external onlyOwner() {
        _maxTxPct = 3000000000 * 10**9; //.3%
    ...
    function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner() {
        ...
        _maxTxPct = maxTxPercent * 10000000000  * 10**9;
        emit MaxTxPctUpdated(_maxTxPct);
    }
}