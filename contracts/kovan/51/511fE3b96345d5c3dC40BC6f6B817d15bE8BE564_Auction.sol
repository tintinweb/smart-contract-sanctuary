...
// File contracts/interface/IKeeperImport.sol
...
contract Auction is Ownable {
    ...
    State state = State.Bid;
    uint256 stateChangedTime = block.timestamp;
    uint256 bidSpan;
    ...
    constructor(
        ...
        require(_bidSpan > 0 && _bidSpan <= 30 days, "invalid bid span");
    ...
    function cancel(uint256 _bidId) external inState(State.Bid) {
        require(_bidId < allBids.length, "invalid bid id");
    ...
    function refund() external inState(State.Refund) {
        ...
        for (uint256 i = 0; i < userBids.bidIds.length; i++) {
            ...
            if (bd.live && bd.amount > bd.selectedAmount) {
                bd.live = false;
                uint256 refundAmount = bd.amount - bd.selectedAmount;
                require(ERC20(bd.asset).transfer(msg.sender, refundAmount));
                emit Refund(msg.sender, bidId, bd.asset, refundAmount);
            }
        }
    ...
    function getUserAmount(address user) public view returns (uint256) {
        return allUserBids[user].uniAmount;
    }
    ...
    function checkDeadline() private {
        if (state == State.Bid && stateChangedTime + bidSpan < block.timestamp) {
            changeState(State.BidEnded);
        } else if (state == State.BidEnded && stateChangedTime + 7 days < block.timestamp) {
            changeState(State.Refund);
        } else if (state == State.Challenge && stateChangedTime + 1 days < block.timestamp) {
            changeState(State.ChallengeEnded);
        } else if (state == State.ChallengeEnded && stateChangedTime + 6 days < block.timestamp) {
            changeState(State.Refund);
        }
    }
    ...
    function changeState(State newState) public {
        state = newState;
        stateChangedTime = block.timestamp;
