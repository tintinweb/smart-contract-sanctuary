...
// import "@openzeppelin/contracts/utils/Counters.sol"; //tokenID_Vs_vaultID
import "@openzeppelin/contracts/utils/math/Math.sol";

/**
 * @title SmartVault (version 0.3.4)
...
contract SmartVault is ERC721PresetMinterPauserAutoId, Ownable {
    // using Counters for Counters.Counter;    //tokenID_Vs_vaultID
    using SafeERC20 for IERC20;
    using Math for uint256;
    // Counters.Counter private _tokenIds;     //tokenID_Vs_vaultID

    enum status{Created, Paused, Canceled, Executed}                     //cast to 0, 1, 2, 3
    event vaultEvent(uint256 _vaultId, address _owner, uint8 _status);
    ...
    struct smartVaultDef {
        ...
        mapping (uint16 => beneficiary) beneficiaries;                   //map to beneficiary struct
        mapping (address => bool) isBeneficiary;                         //whitelist Beneficiaries
        uint16 numBeneficiaries;
        mapping (uint16 => nonfungibleAsset) nonfungibleAssets;          //map to nonfungibleAsset struct
        uint16 numNonFungibleAssets;
        address[] trustees;
    ...
    uint256 constant private minVaultPrice = 10000000000000000;          //wei
    uint256 constant private processingFee = 10;
      
    constructor() ERC721PresetMinterPauserAutoId("Carapace Smart Vault", "CSV", "") {
        mint(msg.sender);
        burn(0);
    }
     ...
     *  - Minimum Vault Price
     */
    function createSmartVault(IERC20[] memory _fungibleAssets, beneficiary[] memory _beneficiaries, nonfungibleAsset[] memory _nonfungibleAssets, address[] memory _trustees) public payable {
        uint8 _totalPercentage;
        uint16 i;
        uint256 _vaultId = ERC721PresetMinterPauserAutoId.getCurrentTokenId(); //_tokenIds.current();   //tokenID_Vs_vaultID
        ...
        // _tokenIds.increment();  //tokenID_Vs_vaultID
        _setupRole(MINTER_ROLE, msg.sender);
        mint(msg.sender); //super.mint(msg.sender); //teste sem super.mint
        
        //revokeRole(MINTER_ROLE, msg.sender);
        emit vaultBalanceChanges(_vaultId, msg.sender, address(this), msg.value, vaultBalance[msg.sender][_vaultId]);
        emit vaultEvent(_vaultId, msg.sender, uint8(smartVaults[_vaultId].vaultState));
    }
    ...
    function vaultReset(uint256 _vaultId) private {
        ...
        for (i=0;i<smartVaults[_vaultId].trustees.length;i++){
            ...
            // vaultsOfTrustee[smartVaults[_vaultId].trustees[i]][vaultIsAtIndex[_vaultId]] = vaultsOfTrustee[smartVaults[_vaultId].trustees[i]][vaultsOfTrustee[smartVaults[_vaultId].trustees[i]].length-1];
            // vaultIsAtIndex[vaultsOfTrustee[smartVaults[_vaultId].trustees[i]].length-1] = vaultIsAtIndex[_vaultId];
            // vaultsOfTrustee[smartVaults[_vaultId].trustees[i]].pop();
        }
     ...
     *  - Total percentage must be equal to 100;
     */
    function modifySmartVault(uint256 _vaultId, IERC20[] memory _fungibleAssets, beneficiary[] memory _beneficiaries, nonfungibleAsset[] memory _nonfungibleAssets, address[] memory _trustees) public onlyVaultOwner(_vaultId) {
        uint8 _totalPercentage;
        uint16 i;
        
        require(smartVaults[_vaultId].vaultState == status.Created || smartVaults[_vaultId].vaultState == status.Canceled, "VC");
        require(_beneficiaries.length > 0 && _trustees.length > 0, "0BT");
    ...
    //     uint256 _deposit = vaultBalance[msg.sender][_vaultId];
    //     vaultBalance[msg.sender][_vaultId] -= _deposit;
    //     vaultReset(_vaultId);
    //     smartVaults[_vaultId].vaultState = status.Canceled;

    //     payable(smartVaults[_vaultId].owner).transfer(_deposit*(100-processingFee)/100);
    //     payable(owner()).transfer(_deposit*processingFee/100);

    //     emit vaultBalanceChanges(_vaultId, address(this), msg.sender, _deposit, vaultBalance[msg.sender][_vaultId]);
    //     emit vaultEvent(_vaultId, msg.sender, uint8(smartVaults[_vaultId].vaultState));
    // }
    
    // v2: transfer difference
    function cancelSmartVault(uint256 _vaultId) public onlyVaultOwner(_vaultId) onlyVaultReady(_vaultId) {        
        uint256 _returnDeposit = vaultBalance[msg.sender][_vaultId]*(100-processingFee)/100;
        uint256 _processingFee = vaultBalance[msg.sender][_vaultId]-_returnDeposit;
    ...
    /**
     * @dev Execute Smart Vault, verifies the minimum of "Allowance" and "BalanceOf" each token, and distributes the 
     * value for each Beneficiary based on their configured percentages. Uses same approach to distribute the vault's
     * Deposit Balance.
     * Also distributes each non-fungible asset to the specific Beneficiary configured.
     * @param _vaultId ID of the Vault
     *
     * Requirements:
     *  - Vault state must be Created; 
    ...
    function executeSmartVault(uint256 _vaultId) public onlyVaultReadyAndTrustees(_vaultId) {
        ...
        uint256 _totalBalanceToDistribute = vaultBalance[smartVaults[_vaultId].owner][_vaultId]*(100-processingFee)/100;
        ...
        for (j=0;j<smartVaults[_vaultId].numBeneficiaries;j++){    
            ...
            emit vaultBalanceChanges(_vaultId, address(this), smartVaults[_vaultId].beneficiaries[j].beneficiary, _partialBalanceToDistribute, vaultBalance[smartVaults[_vaultId].owner][_vaultId]);
        }
        
        payable(owner()).transfer(vaultBalance[smartVaults[_vaultId].owner][_vaultId]*processingFee/100);
    ...
    function getSmartVaultInfo(uint256 _vaultId) public view returns (
        ...
        return (
            ...
            uint8(smartVaults[_vaultId].vaultState));
    }

    /**
     * @dev Getter for the beneficiaries and respective percentages of a given Vault ID.
     */
    function getBeneficiaries(uint256 _vaultId) public view returns (address[] memory, uint8[] memory) {
        address[] memory _beneficiaries = new address[](smartVaults[_vaultId].numBeneficiaries);
        uint8[] memory _percentages = new uint8[](smartVaults[_vaultId].numBeneficiaries);
        uint16 i;
