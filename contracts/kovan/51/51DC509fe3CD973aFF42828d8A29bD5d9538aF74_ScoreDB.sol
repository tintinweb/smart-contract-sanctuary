...
contract ScoreDB is ChainlinkClient, Pausable, Ownable, ScoreDBInterface {
  ...
  function setTesting(bool _status) public override onlyOwner {
    testing = _status;
  }
  ...
  function setLtv(uint16 _score, uint256 _LTV) public override onlyOwner {
    LTV[_score] = _LTV;
  }

  function getLtv(uint16 _score) public view override returns (uint256) {
    return LTV[_score];
  } 
  ...
  function setOracle (address _oracle, bytes32 _jobId, uint256 _fee) public override onlyOwner {
    oracle = _oracle;
    jobId = _jobId;
    fee = _fee;
  }
...
interface ScoreDBInterface {
    ...
    // Sets the LTV for a given score [OWNER]
    function setLtv(uint16 _score, uint256 _LTV) external;
