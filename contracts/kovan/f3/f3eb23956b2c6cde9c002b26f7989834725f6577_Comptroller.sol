...
contract PToken is PTokenInterface, Exponential, TokenErrorReporter {
    ...
    function mintFresh(address minter, uint256 mintAmount) internal returns (uint256, uint256) {
        ...
        // console.log("mintAmount:", mintAmount); //TODO 重要
    ...
    function borrowFresh(
        ...
        (vars.mathErr, vars.accountBorrows) = borrowBalanceStoredInternal(borrower, id, _loanType);
        if (vars.mathErr != MathError.NO_ERROR) {
        ...
        if (LoanType.MARGIN_SWAP_PROTOCOL == _loanType) {
            ...
            /* We write the previously calculated values into storage */
            accountBorrowsMarginSP[borrower][id].principal = vars.accountBorrowsNew; //当前这个borrower所借的全部数量
            accountBorrowsMarginSP[borrower][id].interestIndex = borrowIndex; //当前的索引值

        } else if (LoanType.MINNING_SWAP_PROTOCOL == _loanType){
            require(whiteList[msg.sender], "caller shoud be in white list");
            doTransferOut(msg.sender, borrowAmount);
            /* We write the previously calculated values into storage */
            accountBorrowsMiningSP[borrower][id].principal = vars.accountBorrowsNew; //当前这个borrower所借的全部数量
            accountBorrowsMiningSP[borrower][id].interestIndex = borrowIndex; //当前的索引值
    ...
    function repayBorrowFresh(
        ...
        if (LoanType.MARGIN_SWAP_PROTOCOL == loanType) {
            vars.borrowerIndex = accountBorrowsMarginSP[borrower][id].interestIndex;
        } else if (LoanType.MINNING_SWAP_PROTOCOL == loanType){
            vars.borrowerIndex = accountBorrowsMiningSP[borrower][id].interestIndex;
        } else if (LoanType.NORMAL == loanType){
            vars.borrowerIndex = accountBorrows[borrower].interestIndex;
        }

        /* We fetch the amount the borrower owes, with accumulated interest */
        (vars.mathErr, vars.accountBorrows) = borrowBalanceStoredInternal(borrower, id, loanType); //TODO 还款
        if (vars.mathErr != MathError.NO_ERROR) {
    ...
    function doCreditLoanRepayInternal(
        ...
        (error,actualAmount) = repayBorrowFresh(msg.sender, _payer, _repayAmount, _id, _loanType);
        if (error != uint256(Error.NO_ERROR)) {
...
contract Comptroller is ComptrollerV5Storage, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError{
    ...
    function mintAllowed(
        ...
        //TODO getCashPrior函数对于不同的资产返回的数量级不同，导致出现问题，需要特殊处理
    ...
    function updatePubSupplyIndex(address pToken) internal {
        ...
        uint256 supplySpeed = compSpeeds[pToken];//先使用固定的 //TODO
    ...
    function updatePubBorrowIndex(address pToken, Exp memory marketBorrowIndex) internal {
        ...
        uint256 borrowSpeed = compSpeeds[pToken]; //先使用固定的 //TODO
    ...
    function distributeBorrowerComp(
        ...
        if (borrowerIndex.mantissa > 0) {
            ...
            uint256 borrowerAmount = div_(PToken(pToken).borrowBalanceStored(borrower, 0, LoanType.NORMAL), marketBorrowIndex);
    ...
    function getUnclaimedPub(address holder) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < allMarkets.length; i++) {
            ...
            uint256 borrowerAmount = div_(market.borrowBalanceStored(holder, 0, LoanType.NORMAL), marketBorrowIndex);
            if (borrowerAmount > 0) {
    ...
    // 杠杆交易（挖矿）获取平台币
    function claimPub(address holder, address[] memory pTokens) public returns (uint256){
        // address memory pTokens_ = new address
        //TODO
    }
