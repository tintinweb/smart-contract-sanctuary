...
contract dNXMToken is IdNXMToken, ERC20PermitUpgradeable {
    ...
    function setDependencies(address _nxMaster, address _wNXMTokenAddress) external override onlyInjector {
    ...
    **/
    function callWithdrawal(uint _amountDNXM) public override{
        ...
        _pooledStaking.requestUnstake(_contracts, _amount, 0);
        ...
        withdrawalsInfo[_msgSender()] = WithdrawalInfo(
            ...
            _amountDNXM.add(withdrawalsInfo[_msgSender()].amountDNXMRequested)
        );
        emit CallWithdraw(_msgSender(), _amountDNXM);
    }

    function withdraw(uint _amountDNXM) public {
    ...
    {
        _unlockPeriod = withdrawalsInfo[_userAddr].coolDownTimeEnd;
        _amountDNXMRequested = withdrawalsInfo[_userAddr].amountDNXMRequested;
        _amountNXMSRequested = convertDNXMToNXM(_amountNXMSRequested);
    }

    function maxDNXMCallWithdrawal(address _address) public view override returns (uint256){
        return balanceOf(_address).sub(withdrawalsInfo[_msgSender()].amountDNXMRequested);
    }
    ...
    function _getNXMtoDNXMRatio() internal view returns (uint) {
        ...
        uint _stakesAndRewards = _currentlyAtStake().add(_pooledStaking.stakerReward(address(this)));
...
interface IdNXMToken {
    ...
    function setDependencies(address _nxMaster, address _wNXMTokenAddress) external;
