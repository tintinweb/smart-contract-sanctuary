...
contract ERC1155Implementation is AccessControlUpgradeable, ERC1155Upgradeable, Signature {
    string public name;
    ...
    event Fees(address indexed creator, uint256 id, uint256 fee);
    ...
    function init(string memory _uri, string memory _name, string memory _symbol, string memory _version) external returns (bool) {
        __ERC1155_init(_uri);
        __Signature_init(_name, _version);

        _setupRole(OWNER_ROLE, msg.sender);
        _setRoleAdmin(OWNER_ROLE, OWNER_ROLE);
        _setRoleAdmin(SIGNER_ADMIN_ROLE, OWNER_ROLE);
    ...
    function mint(uint256 id, uint256 supply, uint256 fee, uint8 v, bytes32 r, bytes32 s) external {
        require(creators[id] == address(0), "Token is already minted");
        require(supply != 0, "Supply should be positive");
        require(
            hasRole(SIGNER_ADMIN_ROLE, _getSigner(id, supply, fee, msg.sender, v, r, s)),
    ...
    function _uint2str(uint i) internal pure returns (string memory _uintAsString) {
        if (i == 0) {
            return "0";
        }
        uint j = i;
...
contract Signature {
    ...
    struct SignData {
        uint256 id;
        uint256 supply;
        uint256 fee;        // percent, 2 decimals
        address creator;
    }
    ...
    bytes32 constant SIGNDATA_TYPEHASH = keccak256(
        "SignData(uint256 id,uint256 supply,uint256 fee,address creator)"
    );
    ...
    function _getSigner(uint256 id, uint256 supply, uint256 fee, address creator, uint8 v, bytes32 r, bytes32 s) internal view returns (address) {
