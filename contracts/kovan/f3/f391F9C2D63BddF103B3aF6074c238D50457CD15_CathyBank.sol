...
contract CathyBank is Ownable {
    ...
    function redeemETH() public returns (bool) {
        ICETH cETH = ICETH(CETHContractAddress);

        cETH.redeemUnderlying(100000000000000000);

        return true;
    }

    // function redeemETH(uint256 _amount, bool redeemType) public returns (bool) {
    //     ICETH cETH = ICETH(CETHContractAddress);
