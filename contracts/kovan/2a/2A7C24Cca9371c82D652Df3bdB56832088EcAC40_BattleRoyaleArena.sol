...
contract BattleRoyale is ERC721Tradable {
  ...
  function burn(uint256 _tokenId) public virtual {
    require(msg.sender == ownerOf(_tokenId) || supportLevelAccess(msg.sender));
   ...
   * Method to withdraw ETH
   */
   function withdraw(uint256 amount) external override virtual onlySupport {
     uint256 balance = address(this).balance;
     require(amount <= balance);
  ...
  * Get Current ETH Balance from contract
  */
  function getCurrentBalance() external override onlySupport returns (uint256) {
    uint256 balance = address(this).balance;
    return balance;
  }
  /**
   ...
   * @param {[type]} uint256 [description]
   */
  function setIntervalTime(uint256 _intervalTime) external payable onlySupport returns (uint256) {
   ...
   * set currentPrice
   */
  function setPrice(uint256 _price) external onlySupport payable {
    price = _price;
  }
  /*
   * Toggle auto-start on or off
   */
  function autoStartOn(bool _autoStart) external onlySupport payable {
    autoStart = _autoStart;
  }
  /*
   * Toggle auto-start on or off
   */
  function autoPayoutOn(bool _autoPayout) external onlySupport payable {
    autoPayout = _autoPayout;
  }
  /*
   * Set Fee Rate - aggreed rate the contract takes from the artist for initial sale
   */
  function setFeeRate(uint256 _feeRate) external onlySupport payable {
    feeRate = _feeRate;
  }
  /*
   * setUnitsPerTransaction
   */
  function setUnitsPerTransaction(uint256 _units) external onlySupport payable {
    unitsPerTransaction = _units;
  }
  /*
   * setMaxSupply
   */
  function setMaxSupply(uint256 supply) external onlySupport payable {
    maxSupply = supply;
  }
  /*
   * setMaxElimsPerCall
   */
  function setMaxElimsPerCall(uint256 _maxElimsPerCall) external onlySupport payable {
    maxElimsPerCall = _maxElimsPerCall;
  }
   ...
   */
  function setDefaultTokenURI(string memory _tokenUri) external onlySupport payable {
   ...
   * @param string IPFS meta-data uri
   */
  function setPrizeTokenURI(string memory _tokenUri) external onlySupport payable {
    prizeTokenURI = _tokenUri;
  }
  /*
   * set artist
   */
  function setArtist(address payable _artist) external onlySupport payable {
    artist = _artist;
  }
  /*
   ...
   */
  function beginBattle() external onlySupport {
    startBattle();
  }
   ...
   */
  function executeRandomElimination(uint256 _randomNumber) external payable onlySupport {
   ...
   * payout artist
   */
  function executePayout() public payable onlySupport {
    executeAutoPayout();
  }
 ...
 */
contract ERC721Tradable is ERC721, CustomAccessControl {
   ...
   * Get Current ETH Balance from contract
   */
  function getCurrentBalance() external virtual onlyAdmin returns (uint256) {
    uint256 balance = address(this).balance;
    return balance;
  }
  /**
   ...
   */
  function setTokenURI(uint256 _tokenId, string memory _tokenURI) public onlyAdmin payable {
