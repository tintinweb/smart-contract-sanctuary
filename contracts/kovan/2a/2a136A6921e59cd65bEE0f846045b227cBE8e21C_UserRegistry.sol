...
contract UserRegistry is Registry, IUserRegistry,IERC1643{
    ...
    // uint256 public set_redeemption_address_count;

    uint256 public REDEMPTION_ADDRESS_COUNT;
    ...
    constructor(
        ...
        uint256 _REDEMPTION_ADDRESS_COUNT
    ) public {
        require(_minBurnBound <= _maxBurnBound, "min bigger than max");
        token = _token;
        minBurnBound = _minBurnBound;
        maxBurnBound = _maxBurnBound;
        REDEMPTION_ADDRESS_COUNT = _REDEMPTION_ADDRESS_COUNT;
    }

    function set_redeemption_address_count(uint256 _REDEMPTION_ADDRESS_COUNT) public onlyOwner {
        REDEMPTION_ADDRESS_COUNT = _REDEMPTION_ADDRESS_COUNT;
    }
    ...
    function canBurn(address _from, uint256 _amount)
        ...
        onlyToken
    {
        require(_isKyced(_from), "user has not KYC");
        require(!_isBlocked(_from), "blocklisted");
