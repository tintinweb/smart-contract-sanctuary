...
contract RaffleWorld is Ownable, VRFConsumerBase {
    ...
    mapping(address => RaffleTypes) numberOfRefferences;
    ...
    function setRequiredRaffleLink(uint256 _requireRaffleLink)
        external
        onlyOwner
    {
        requiredRaffleLink = _requireRaffleLink;
        emit SetRequiredRaffleLink(_msgSender(), _requireRaffleLink);
    }
    ...
    function setRaffleTicketNumber(uint256 _raffleId, uint256 _ticketNumber)
        external
        beforeRaffleStart(_raffleId)
        onlyOwner
    {
        raffles[_raffleId].ticketNumber = _ticketNumber;
        emit SetRaffleTicketNumber(_msgSender(), _raffleId, _ticketNumber);
    }
        ...
        onlyOwner
    {
        require(
            ...
            "RaffleWorld: RaffleStartDate should be in the future!"
        );
    ...
    function setRaffleWinnerStructure(
        ...
        for (uint256 i = 0; i < winnerStructure[_raffleId].length; i++) {
            if (winnerStructure[_raffleId][i].index == _index) {
                if (_percentage == 0) {
                    raffles[_raffleId].totalPercentage = raffles[_raffleId].totalPercentage.sub(winnerStructure[_raffleId][i].percentage); 
    ...
    function _addRaffleWinner(
        ...
        internal
    {
        // winnerStructure[_raffleId][_winIndex].winnerAddress = raffleEntries[_raffleId][_winnerIndex];  
    ...
    function removeRaffleWinner(
        ...
        onlyOwner
    {
        winnerStructure[_raffleId][_winIndex] = winnerStructure[_raffleId][winnerStructure[_raffleId].length-1];
        winnerStructure[_raffleId].pop();
    }

    function _sendRaffleWinnerMoney(
        uint256 _raffleId,
        uint256 _winIndex
    )
        internal
    {
        ...
        uint256 amount = winnerStructure[_raffleId][_winIndex].percentage.mul(raffles[_raffleId].prizeAmount).div(raffles[_raffleId].totalPercentage);
        token.transfer(winnerStructure[_raffleId][_winIndex].winnerAddress, amount);
    }
    ...
    function setRaffleCanceledStatus(uint256 _raffleId, bool _canceled)
        ...
        onlyOwner
    {
        raffles[_raffleId].canceled = _canceled;
        ...
        if (entries != 0) {
            for (uint256 i = 0; i < winnerStructure[_raffleId].length; i++) {
                uint256 index = uint256(
                    keccak256(abi.encode(random, i))
                ).mod(raffleEntries[_raffleId].length);
                console.log(random);
    ...
    function subscribeToRaffle(
        ...
        for (uint256 i = 0; i < _ticketsNo; i++) {
          //  raffleEntriesPositionById[_raffleId][_msgSender()].push(raffleEntries[_raffleId].length);
            raffleEntries[_raffleId].push(_msgSender());
        }
        ...
        if (
            ...
            numberOfRefferences[_refferedBy].bronze = numberOfRefferences[
                _refferedBy
            ].bronze.add(1);
        } else if (
            keccak256(bytes(raffles[_raffleId].raffleType)) ==
            ...
            numberOfRefferences[_refferedBy].silver = numberOfRefferences[
                _refferedBy
            ].silver.add(1);
        } else if (
            keccak256(bytes(raffles[_raffleId].raffleType)) ==
            ...
            numberOfRefferences[_refferedBy].gold = numberOfRefferences[
                _refferedBy
            ].gold.add(1);
        } else if (
            keccak256(bytes(raffles[_raffleId].raffleType)) ==
            ...
            numberOfRefferences[_refferedBy].diamond = numberOfRefferences[
                _refferedBy
            ].diamond.add(1);
        }
    ...
    function withdrawOwnerFunds(uint256 _raffleId) external onlyOwner {
        ...
        IERC20 raffleToken = IERC20(raffles[_raffleId].tokenContract);
        uint256 ownerFunds = (raffles[_raffleId].ticketPrice *
            raffles[_raffleId].ticketNumber) - raffles[_raffleId].prizeAmount;
        require(
