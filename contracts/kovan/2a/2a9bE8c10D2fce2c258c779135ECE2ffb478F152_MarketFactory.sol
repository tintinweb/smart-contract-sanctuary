...
contract MarketFactory is IMarketFactory {
    ...
    function createMarket(address token) external override returns (address) {

        // create market contract
        Market newMarket = new Market(token,config.getStable(),address(config));
    ...
    function createETHMarket() external override returns (address) {
        // create market contract (address(0) means ETH)
        Market newMarket = new Market(address(0),config.getStable(),address(config));
        address addr = address(newMarket);
        // add market to the protocol config
...
interface IMarketFactory {
    ...
    function createETHMarket() external returns (address);
}
