...
contract UniswapV3StakerHelper {

    address public nonfungiblePositionManager;
    address public uniswapV3Staker;
    address public aelinToken;
        
    constructor(address _nonfungiblePositionManager, address _uniswapV3Staker, address _aelinToken) {
    ...
    function depositAndStake(IUniswapV3Staker.IncentiveKey memory _key, address _from, uint256 _tokenId) public {
        INonFungiblePositionManager(nonfungiblePositionManager).safeTransferFrom(_from, uniswapV3Staker, _tokenId);
        IUniswapV3Staker(uniswapV3Staker).stakeToken(_key, _tokenId);
    }

    function unstakeAndWithdraw(IUniswapV3Staker.IncentiveKey memory _key, address _to, uint256 _tokenId, bytes memory _data) public {
        IUniswapV3Staker(uniswapV3Staker).unstakeToken(_key, _tokenId);
        IUniswapV3Staker(uniswapV3Staker).withdrawToken(_tokenId, _to, _data);
    }

    function claim(IUniswapV3Staker.IncentiveKey memory _key, address _to, uint256 _tokenId, uint256 _amount) public {
        IUniswapV3Staker(uniswapV3Staker).unstakeToken(_key, _tokenId);
        IUniswapV3Staker(uniswapV3Staker).claimReward(IERC20Minimal(aelinToken), _to, _amount);
        IUniswapV3Staker(uniswapV3Staker).stakeToken(_key, _tokenId);
    }
}
