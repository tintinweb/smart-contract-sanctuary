...
contract MyV2FlashLoan is FlashLoanReceiverBase {
    ...
    function executeOperation(
      ...
      bytes calldata params
        ...
        // swap WETH -> DAI on uniswap v2
        address[] memory path = new address[](2);
        path[0] = address(weth);
        path[1] = address(dai);

        uint[] memory minOuts = uniswap.getAmountsOut(amounts[0], path); 
        ...
        minOuts = uniswap.getAmountsOut(balanceDai, path);
        ...
        // repay the Aave Flashloan
        uint256 repaymentAmount = amounts[0].add(premiums[0]);
        IERC20(assets[0]).approve(address(LENDING_POOL), repaymentAmount);
    ...
    function myFlashLoanCall() public {
        ...
        assets[0] = address(0xd0A1E359811322d97991E03f863a0C30C2cF029C); // Kovan WETH
