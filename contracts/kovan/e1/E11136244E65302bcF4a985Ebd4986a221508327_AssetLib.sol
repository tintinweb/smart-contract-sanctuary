...
library AssetLib {
    ...
    function buyTokensInAssetRebase(
        ...
        for (uint256 i = 0; i < tokensToBuy.length && tokenToBuyInfoGlobals[1] > 0; ++i) {
            ...
            outputAmounts[i] = safeSwap([weth, tokensToBuy[i]], wethToSpend, assetFactory);
    ...
    function transferTokenAndSwapToWeth(
        ...
        if (tokenToPay == weth) {
            ...
            totalWeth = safeSwap([tokenToPay, weth], amount, assetFactory);
        }
    ...
    function safeSwap(
        ...
        address defaultDexRouter = IAssetFactory(assetFactory).defaultDexRouter();
        address dexRouter = getTokenDexRouter(assetFactory, path[0], defaultDexRouter);
    ...
    function depositToY(
        ...
        for (uint256 i = 0; i < tokensOfDividends.length; ++i) {
            ...
            totalWeth += safeSwap([tokensOfDividends[i], weth], amountOfDividends[i], assetFactory);
        }
        return totalWeth;
    }
    ...
    function getTokenDexRouter(
        address factory,
        address token,
        address defaultDexRouter
