    ...
    Pausable
{
    ...
    uint256 public rewardsDuration = 8 minutes;
    ...
    function lastTimeRewardApplicable() public view override returns (uint256) {
        return
            Math.min(
                _numWeeksPassed(block.timestamp).mul(2 minutes).add(startEmission),
                periodFinish
            );
    }
    ...
    function _numWeeksPassed(uint256 time) internal view returns (uint256) {
        ...
        return time.sub(startEmission).div(2 minutes).add(1);
    }
