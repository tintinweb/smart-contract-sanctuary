...
contract ReservePool is Ownable, ReentrancyGuard {
	...
	****************************************/
	...
	function emegencyWithdraw()
		external
	{
		require(elasticPool.paused() && elasticPool.getBalanceBp() == 0, "emergency conditions not meet");
		uint256 _balance = outToken.balanceOf(address(this));
		require(_balance != 0, "zero out token balance");
		outToken.safeTransfer(vpStorage, _balance);
	}
