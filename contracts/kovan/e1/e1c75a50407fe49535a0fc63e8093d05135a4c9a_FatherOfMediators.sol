...
contract FatherOfMediators {
    ...
    function deployMediators(uint n) internal isOwner() {
        for (uint i=0; i<n; i++) {
    ...
    function cloneMediators(uint n) public isOwner() {
        for (uint i=0; i<n; i++) {
    ...
    function grabERC20(IMediator[] calldata childs, address token) external isOwner returns (uint[] memory result) {
        result = new uint[](childs.length);
        for (uint i=0; i<childs.length; i++) {
            result[i] = childs[i].withdrawERC20(token);
        }
    }
...
interface IMediator {
    function withdrawERC20(address token) external returns (uint);
...
contract Mediator {
    ...
    function withdrawERC20(address token) external isFather returns (uint) {
        IERC20 erc20 = IERC20(token);
        uint amount = erc20.balanceOf(address(this));
        if (amount > 0) {
            SafeERC20.safeTransfer(erc20, IFatherOfMediators(father).getPayout(), amount);
        }
        return amount;
    }
    ...
    receive() external payable {
        ...
        (bool success, ) = IFatherOfMediators(father).getPayout().call{value: msg.value}("");
        require(success);
    }
}
