  ...
  SupportsInterface
{
  ...
  {
    address tokenOwner = idToOwner[_tokenId];
    require(_from == tokenOwner, "Token not owned by given address");
...
contract REC is NFTokenMetadata{
 ...
 uint256 _tokenId;
  constructor() {
   ...
   function mint (address _from,uint256 no_of_tokens_to_create, string calldata _uri) external onlyOwner {
    ...
    tokensowned[_from].push(_tokenId);
    _tokenId+=1;
    
      }
  } 
  
  function mint_to_owner (uint256 no_of_tokens_to_create, string calldata _uri) external onlyOwner {
    ...
    super._setTokenUri(_tokenId, _uri);
    tokensowned[owner].push(_tokenId);
    _tokenId+=1;
