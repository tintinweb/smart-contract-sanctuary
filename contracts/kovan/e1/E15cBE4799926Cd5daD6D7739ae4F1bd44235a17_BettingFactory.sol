...
contract BettingContract is IBettingContract {
    ...
    uint256 public TIME_PRICE_CLOSE;
    ...
    modifier onlyPriceClose() {
        require(
            block.timestamp > expiration + TIME_PRICE_CLOSE,
            "BETTING: Please waiting for price close"
        );
        _;
    }

    function setTimePriceClose(uint256 _seconds)
        external
        override
        onlyOwner
        onlyLock
    {
        TIME_PRICE_CLOSE = _seconds;
    }
    ...
    //Setup expiration
    function setExpirationContract(uint256 unixtime)
        public
        override
        onlyOwner
        onlyLock
        ...
        onlyLock
    {
        ...
        resultId = IPriceContract(_priceContract).updatePrice{value: msg.value}(
            expiration - block.timestamp + TIME_PRICE_CLOSE,
            tokenAddress,
            creator
        );
        status = Status.Open;
