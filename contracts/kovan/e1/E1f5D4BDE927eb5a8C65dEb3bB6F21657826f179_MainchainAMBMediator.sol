...
contract ChainAssigner is Ownable {
    ...
    function unassignStarSystem(uint _sysId, uint _chainId, uint _sysMap) external {
        address mediator = chainMediators[_chainId];
...
contract MainchainAMBMediator is BaseAMBMediator {
    ...
    function assignStarSystem(uint _sysId, address _recipient, uint _sysMap) external returns (bytes32 _messageId) {
    ...
    function unassignStarSystem(uint _sysId, uint _sysMap) external onlyMediatorOnOtherSide {
...
contract SidechainAMBMediator is BaseAMBMediator {
    ...
    function assignStarSystem(uint _sysId, address _recipient, uint _sysMap) external onlyMediatorOnOtherSide {
...
contract StarSystemData is Ownable {
    ...
    event StarSystemMapSet(uint indexed _sysId, uint _newMap);
    ...
    function mapOf(uint _sysId) external view returns (uint) { return sysData[_sysId].map; }
    ...
    function setMap(uint _sysId, uint _sysMap) public onlyDataEditor {
        require(_sysMap > 0 && _sysMap < 2**253, "Invalid system map"); // _sysMap must be smaller than snark scalar field (=> have first 3 bits empty)
        sysData[_sysId].map = _sysMap;
        emit StarSystemMapSet(_sysId, _sysMap);
    }

    function setSystemData(uint _sysId, address _owner, uint _sysMap) external onlyDataEditor {
