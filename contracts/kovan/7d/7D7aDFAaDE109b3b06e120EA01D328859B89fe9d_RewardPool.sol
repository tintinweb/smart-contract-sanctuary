...
contract RewardPool {
    ...
    modifier restricted(){
        require(msg.sender == Storage(_storage).controller() || msg.sender == Storage(_storage).governance(), "OP0");
        _;
    }

    function transferOnlyGovernance(address _token, uint256 amount, address _to) restricted external {
        IERC20(_token).safeTransfer(_to, amount);
    }

    function withdrawEth(address payable _addressTo, uint256 _amount) restricted external payable {
