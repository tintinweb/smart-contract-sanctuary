...
/**
  * @title LeverDelegator
  * @author Lever
  */
contract LeverDelegator is LeverInterface, LeverStorage, IUniswapV2Callee, Adminable, DelegatorInterface {
    ...
    function pool0(uint16 marketId) external override view returns (LPoolInterface){
        bytes memory data = delegateToViewImplementation(abi.encodeWithSignature("pool0(uint16)", marketId));
        return abi.decode(data, (LPoolInterface));
    }

    function pool1(uint16 marketId) external override view returns (LPoolInterface){
        bytes memory data = delegateToViewImplementation(abi.encodeWithSignature("pool1(uint16)", marketId));
        return abi.decode(data, (LPoolInterface));
    }
    ...
    function insurance(uint16 marketId, bool pool) external override view returns (uint){
        bytes memory data = delegateToViewImplementation(abi.encodeWithSignature("insurance(uint16,bool)", marketId, pool));
        return abi.decode(data, (uint));
    }
    ...
    function marginTrade(
        ...
        delegateToImplementation(abi.encodeWithSignature("marginTrade(uint16,bool,bool,uint256,uint256,uint256)",
            marketId, longToken, depositToken, deposit, borrow, minBuyAmount));
    }
    ...
    function setPenaltyRate(uint8 newRate) external override {
        delegateToImplementation(abi.encodeWithSignature("setPenaltyRate(uint8)", newRate));
    }
    ...
    function setController(ControllerInterface newController) external override {
