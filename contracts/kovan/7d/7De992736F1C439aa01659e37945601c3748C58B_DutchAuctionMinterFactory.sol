...
interface IBasicFYToken is IFYToken {
  ...
  function claimMintability(bytes calldata signature) external;
}
...
interface IFYToken is IERC20 {
  // returns the timestamp when the fyTokens mature
  ...
  // returns the redemption value of a single fyToken after maturity in units of the underlying token
...
contract DutchAuctionMinter {
  ...
  function buyFYTokensWithExactTokens(
    ...
    require(fyToken.totalSupply().add(_inTokens) <= cap, 'Exceeds cap');

    uint256 price = _currentPriceImpl();
    uint256 inversePriceScaled = auctionTokenUnit * SCALING / price;

    uint256 fyTokens = (_inTokens.mul(inversePriceScaled).add(SCALING_ROUND)) / SCALING;
  ...
  function buyExactFYTokensWithTokens(
    ...
    uint256 inTokens = _outFYTokens.mul(price) / auctionTokenUnit;
    require(inTokens >= _maxTokensIn, 'Insufficent Input');
    require(fyToken.totalSupply().add(inTokens) <= cap, 'Exceeds cap');
  ...
  function _currentPriceImpl() private view returns (uint256) {
    ...
    uint256 auctionLength = auctionEndTime - auctionStartTime;
    uint256 auctionTimeElapsed = block.timestamp - auctionStartTime;
    uint256 range = auctionEndPrice - auctionStartPrice;
    uint256 priceChange = range.mul(auctionTimeElapsed).mul(SCALING) / auctionLength / SCALING;
...
contract DutchAuctionMinterFactory {
  ...
  function createMinterAndClaimMintability(
    IBasicFYToken _fyToken,
    ERC20 _auctionToken,
    uint256 _auctionStartPrice,
    uint256 _auctionEndPrice,
    uint256 _auctionStartTime,
    ...
    _fyToken.claimMintability(signature);
