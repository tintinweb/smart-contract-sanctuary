...
import {FlashLiquidationAdapter} from "./FlashLiquidationAdapter.sol";
 ...
 *
 *   call liquidate() to begin the liquidation
 *
 */
contract Liquidator is FlashLiquidationAdapter {
    constructor(
        ...
        public
        FlashLiquidationAdapter(addressesProvider, uniswapRouter, wethAddress)
    {}
     ...
     *  _borrowedAsset aka _assetToLiquidate - the token address of the asset that will be liquidated
     * _flashAmount - flash loan amount (number of tokens) which is exactly the amount that will be liquidated
     * _collateralAsset - the token address of the collateral. This is the token that will be received after liquidating loans
     * _user - user ID of the loan that will be liquidated
     * _useEthPath - when using uniswap this is used to make sure the swap returns a minimum number of tokens, or will revert
     *
     */
    function liquidate(
        ...
        bytes memory params = abi.encode(
            _collateralAsset,
            _borrowedAsset, // passed via flashloan call; borrowed Asset, can also be accessed by assets[0]
            _user,
            amounts, // _debtToCover; can also be accesed via amounts[]
            _useEthPath
        );
        ...
        uint256 remainingTokens = IERC20(_collateralAsset).balanceOf(
            address(this)
        );
        console.log(_collateralAsset, remainingTokens);
        IERC20(_collateralAsset).transfer(msg.sender, remainingTokens);
        uint256 userBalance = IERC20(_collateralAsset).balanceOf(msg.sender);
