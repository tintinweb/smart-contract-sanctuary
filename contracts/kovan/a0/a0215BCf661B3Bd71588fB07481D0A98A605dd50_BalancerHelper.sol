...
contract BalancerHelper {
	...
	constructor(bytes32 poolId_) public {
		_poolId = poolId_;
	}

	function depositFunds(address[] memory tokens, uint256[] memory amounts) external {
	...
	function joinPool(
		...
		vault.joinPool(_poolId, msg.sender, msg.sender, request);
		// vault.joinPool(_poolId, address(this), msg.sender, request);
	}
}
