...
contract CopperProxy is Ownable {
    ...
    event Skimmed(address token, address from, address to, uint256 balance);
    ...
    function exitPool(address pool, uint256[] calldata minAmountsOut) external onlyPoolOwner(pool) {
        // 1. Get pool data
        ...
        bytes memory userData = abi.encode(ExitKind.EXACT_BPT_IN_FOR_TOKENS_OUT, bptBalance);
    ...
    function skim(address token) external onlyOwner {
        ...
        TransferHelper.safeTransferFrom(token, msg.sender, VAULT, balance);
        emit Skimmed(token, msg.sender, VAULT, balance);
    }
}
