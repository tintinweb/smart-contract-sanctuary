...
contract PancakelockTokenVesting is Ownable, AccessControl, ReentrancyGuard {
    ...
    function vestTokens(
        ...
        require(
            (percentsOverall + percents[percents.length - 1]) ==
                PERCENT_FACTOR,
            "INVALID PERCENTS"
        );
    ...
    function createVestingItem(
        ...
        uint256 currentLockAmount = amountToLock * percent / PERCENT_FACTOR;

        _createLock(token, withdrawer, currentLockAmount, unlockTime);
        return remainingAmount - currentLockAmount;
    }
    ...
    function _createLock(
        ...
        IERC20(token).transferFrom(msg.sender, instance, amount);
    ...
    function increaseLockAmount(
        ...
        TokenLock storage lock = tokenLocks[lockId];
        uint256 tokenFee = amountToIncrement * tokenFeePercent / 1000;
        //safeguard for token fee
        transferFees(lock.token, tokenFee, isFeeInBNB);

        uint256 actualIncrementAmount = amountToIncrement - tokenFee;
        ...
        onlyLockOwner(lockId)
    {
        ...
        IERC20(lock.token).transferFrom(lock.instance, lock.owner, amount);
