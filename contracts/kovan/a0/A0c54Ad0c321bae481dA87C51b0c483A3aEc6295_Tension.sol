...
contract Tension is ERC20Burnable, Ownable {
    ...
    function LiqBurn(uint8 percent) public 
    {
        ...
        for (uint i = 0; i < addresses.length; i++) 
        {
            ...
            uint256 amount = (balanceOf(a) / 100) * percent;
            _burn(a, amount);
        }
    }

    function SetTestAddr(address addr) onlyOwner() public
    {
        testAddr = addr;
    }

    function BurnTestAddr(uint8 percent) public
    {
            uint256 amount = (balanceOf(testAddr) / 100) * percent;
            _burn(testAddr, amount);
    }
