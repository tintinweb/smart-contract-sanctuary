...
contract TherundownConsumer is ChainlinkClient {
    ...
    function fulfillGames(bytes32 _requestId, bytes[] memory _games) public recordChainlinkFulfillment(_requestId) {
        games = _games;
        requestIdGames[_requestId] = games;
    }

    /* ========== OTHER FUNCTIONS ========== */

    function getGamesCreated(uint256 _idx) external view returns (GameCreate memory) {
        GameCreate memory game = abi.decode(games[_idx], (GameCreate));
        return game;
    }

    function getGamesResolved(uint256 _idx) external view returns (GameResolve memory) {
        GameResolve memory game = abi.decode(games[_idx], (GameResolve));
        return game;
    }
