...
contract CitadelVaultKovan is ERC20("DAO Vault Citadel", "daoCDV"), Ownable, BaseRelayRecipient {
    ...
    event Deposit(address indexed tokenDeposit, address caller, uint256 amount);
    event Withdraw(address indexed tokenWithdraw, address caller, uint256 amount);
    ...
    function deposit(uint256 _amount, uint256 _tokenIndex) external {
        ...
        emit Deposit(address(Tokens[_tokenIndex].token), _sender, _amount);
    ...
    function withdraw(uint256 _shares, uint256 _tokenIndex) external {
        ...
        emit Withdraw(address(Tokens[_tokenIndex].token), msg.sender, _withdrawAmtInUSD);
    }
