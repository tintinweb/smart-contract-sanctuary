...
contract Fauceth {
    ...
    function refill() public {
        if (vsdc.balanceOf(_fauceth) < (1000000 * 1e18)) {
            vsdc.mint(_fauceth, ((1000000 * 1e18) - vsdc.balanceOf(_fauceth)));
        }
    }
    ...
    function deposit() public payable {
        ...
        require(vsdc.balanceOf(_fauceth) >= val, "No amount");
        require(_chest.send(fee), "No fee");
        vsdc.transferFrom(_fauceth, msg.sender, val);
    ...
    function withdraw(uint val) public {
        require(control());
        require(val > 0 && vsdc.balanceOf(msg.sender) >= val, "No amount");
        ...
        vsdc.transferFrom(msg.sender, _fauceth, val);
        _chest.transfer(fee);
        _to.transfer(wad);
