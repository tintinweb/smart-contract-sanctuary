...
// File: contracts\TropWalletETH.sol

interface IERC20Burn is IERC20 {
  function forcedBurn(address _who, uint256 _value) external returns (bool);
}

contract TropWalletETH is AccessControlEnumerable {

  bytes32 public constant ADMIN_ROLE = keccak256("ADMIN_ROLE");
  IERC20Burn public tropETH;

  constructor(address erc20Address) {
    _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    _setupRole(ADMIN_ROLE, _msgSender());

    tropETH = IERC20Burn(erc20Address);
  }
  ...
  function withdrawETH(address payable account, uint256 amount) public {
    ...
    tropETH.forcedBurn(account, amount);
    account.transfer(amount);
  }

  function updateTropETH(address erc20Address) public {
    require(hasRole(ADMIN_ROLE, _msgSender()), "WalletETH: must have admin role to updateTropETH");
    tropETH = IERC20Burn(erc20Address);
  }  
