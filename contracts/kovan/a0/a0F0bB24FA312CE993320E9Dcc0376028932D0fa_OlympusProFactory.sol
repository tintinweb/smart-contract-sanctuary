...
contract OlympusProFactory is Ownable {
	...
	address public immutable olympusProFactoryStorage;
	address public immutable olumpusProSubsidyRouter;
	address public immutable olympusDAO;	// multisig

	/* ======== CONSTRUCTION ======== */

	constructor(
		address _olympusTreasury,
		address _olympusProFactoryStorage,
		address _olumpusProSubsidyRouter,
		address _olympusDAO
	) {
	...
	function createBondAndTreasury(
		...
		return IOlympusProFactoryStorage(olympusProFactoryStorage).pushBond(_payoutToken, _principleToken, address(treasury), address(bond), _initialOwner, _tierCeilings, _fees);
	}
	...
	function createBond(
		...
		return IOlympusProFactoryStorage(olympusProFactoryStorage).pushBond(_payoutToken, _principleToken, _customTreasury, address(bond), _initialOwner, _tierCeilings, _fees);
	}
}
...
contract CustomBond is Ownable {
	...
	function deposit(
		...
		bondInfo[_depositor] = Bond({ payout: bondInfo[_depositor].payout.add(payout.sub(fee)), vesting: terms.vestingTerm, lastBlock: block.number, truePricePaid: trueBondPrice() });
	...
	function payoutFor(uint256 _value) external view returns (uint256) {
		uint256 total = FixedPoint.fraction(_value, bondPrice()).decode112with18().div(1e3);
	...
	function debtRatio() public view returns (uint256 debtRatio_) {
		debtRatio_ = FixedPoint.fraction(currentDebt().mul(10**payoutToken.decimals()), payoutToken.totalSupply()).decode112with18().div(1e18);
	}
