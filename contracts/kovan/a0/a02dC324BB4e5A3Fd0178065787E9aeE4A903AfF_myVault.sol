...
contract myVault {
	...
	function rebalance() public {
		...
		uint usdBalancePercentage = 100; //(usdBalance / totalBalance) * 100;
		emit logThis('usdBalancePercentage', usdBalancePercentage);
		if (usdBalancePercentage < usdTargetPercentage) {
			uint amountToSellUSD = ((usdBalancePercentage - usdTargetPercentage)  * totalBalance) / 100;
			emit logThis('amountToSellUSD', amountToSellUSD);
			sellWeth(amountToSellUSD);
		} else {
			uint amountToBuyUSD = ((usdTargetPercentage - usdBalancePercentage) * totalBalance) / 100;
