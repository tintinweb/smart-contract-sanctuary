...
contract LL420GreenPass is ERC1155, ReentrancyGuard, Ownable {
    ...
    uint256 public constant MINT_FEE = 0.0465 ether;
    uint256 public constant OG_TIME_LIMIT = 1 hours + 3 minutes;
    uint256 public constant GREEN_LIST_TIME_LIMIT = 1 hours + 3 minutes;
    bytes32 public immutable ogMerkleRoot;
    bytes32 public immutable greenListMerkleRoot;

    uint256 public startTimestamp;

    mapping(uint256 => bool) public validTokenTypes;
    ...
    constructor(string memory _baseURI, uint256 _startTimestamp, bytes32 _ogMerkleRoot, bytes32 _greenListMerkleRoot) ERC1155(_baseURI) {
        ...
        validTokenTypes[0] = true;
        validTokenTypes[1] = true;

        emit SetBaseURI(baseURI);
        emit SetStartTimestamp(startTimestamp);
    }

    function claim(bytes32[] calldata merkleProof) external payable nonReentrant {
        require(msg.value >= MINT_FEE, "LL420GreenPass: Not enough fee");
    ...
    function claimOG(bytes32[] calldata merkleProof) external nonReentrant {
        require(block.timestamp >= startTimestamp, "LL420GreenPass: Not started yet");
    ...
    {
        require(
            validTokenTypes[typeId],
            "URI requested for invalid token type"
        );
        return
            bytes(baseURI).length > 0
                ? string(abi.encodePacked(baseURI, typeId))
                : baseURI;
    }
