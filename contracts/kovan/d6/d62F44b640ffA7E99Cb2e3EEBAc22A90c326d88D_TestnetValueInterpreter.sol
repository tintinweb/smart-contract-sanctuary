...
contract TestnetValueInterpreter is IValueInterpreter, Ownable, TreasuryEnabledMixin {
    ...
    function calcCanonicalAssetValue(
        ...
        uint256 _amount,
        address _quoteAsset
    ) public override returns (uint256 value_) {
        ...
        return
            _amount.mul(baseAssetPrice).mul(quoteAssetDivider).div(quoteAssetPrice).div(
                baseAssetDivider
            );
    }
    ...
    function calcCanonicalAssetsTotalValue(
        ...
        for (uint256 i; i < _baseAssets.length; i++) {
            value_ += calcCanonicalAssetValue(_baseAssets[i], _amounts[i], _quoteAsset);
        }
