...
contract PancakeRouter is IPancakeRouter02, Ownable {
    ...
    modifier calculateTokenFee(address[] memory path, address to) {
        address pair = PancakeLibrary.pairFor(factory, path[0], path[1]);

        // check if token add fee
        if (_isReflectionToken[path[0]]) {
            // add this pair to
    ...
    function swapExactTokensForTokens(
        ...
        address to,
        uint256 deadline
    ) external virtual override ensure(deadline) calculateTokenFee(path, to) returns (uint256[] memory amounts) {
