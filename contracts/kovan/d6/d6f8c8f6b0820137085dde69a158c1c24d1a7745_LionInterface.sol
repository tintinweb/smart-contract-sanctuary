...
// File @openzeppelin/contracts-upgradeable/token/ERC721/IERC721Upgradea[email protected]
...
contract LionInterface {
    ...
    function tokensOfOwner(address _owner) external view returns(LionSimple[] memory) {
        
        uint256 tokenCount = nonFungibleContract.balanceOf(_owner);

        (uint256[] memory tokenIds,,uint256 marketCount) = nftMarket.viewAsksByCollectionAndSeller(address(nonFungibleContract), _owner, 0, 200);

        LionSimple[] memory result = new LionSimple[](tokenCount + marketCount);
        ...
        for (uint256 index = 0; index < marketCount; index++) {
            result[tokenCount + index] = getLionSimple(tokenIds[index]);
        }
