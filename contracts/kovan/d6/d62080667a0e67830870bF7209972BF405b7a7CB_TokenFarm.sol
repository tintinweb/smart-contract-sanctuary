...
contract TokenFarm is ChainlinkClient, Ownable {
    ...
    mapping(address => mapping(address => uint256)) public TokenAccountBalance;
    //mapping(address => uint256) public uniqueTokensStaked;
    mapping(address => address) public TokenPriceFeed;

    constructor(address _intTokenAddress) public {
        intToken = IERC20(_intTokenAddress);
    }

    function addTokens(address token, address priceFeed) public onlyOwner {
        Tokens.push(token);
        TokenPriceFeed[token] = priceFeed;
    }

    function inToken(address token) public view returns (bool) {
    ...
    function stakeTokens(uint256 _amount, address token) public {
        ...
        require(inToken(token), "Token currently isn't allowed");
        IERC20(token).transferFrom(msg.sender, address(this), _amount);
        TokenAccountBalance[token][msg.sender] += _amount;
    }

    // Unstaking Tokens (Withdraw)
    function unstakeTokens(address token, uint256 _amount) public {
        uint256 balance = TokenAccountBalance[token][msg.sender];
        ...
        returns (uint256, uint8)
    {
        ...
        return (
            uint256(price),
            AggregatorV3Interface(TokenPriceFeed[token]).decimals()
        );
    }
    /* 
    function getUserTokenStakingBalanceEthValue(address user, address token) internal returns (uint256)
    {
