...
contract BoardRoomMDXStrategy is BoardRoomMDXStrategyStorage{
    ...
    function getProfitAmount() view public returns (address[] memory tokens, uint256[] memory amounts){
        tokens = new address[](1);
        amounts = new uint256[](1);
        tokens[0] = profit;
        amounts[0] = profitAmount.sub(reinvestmentAmount).add(IFarm(farm).pending(pid, address(this)));
    }
