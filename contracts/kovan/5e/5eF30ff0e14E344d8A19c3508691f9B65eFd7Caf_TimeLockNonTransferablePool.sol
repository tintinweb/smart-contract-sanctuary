...
contract TimeLockPool is BasePool, ITimeLockPool {
	...
	function deposit(
		...
		require(_receiver == _msgSender(), "TimeLockPool.deposit: receiver should be msg.sender");
		...
		depositsOf[_receiver].push(Deposit({ amount: _amount, start: uint64(block.timestamp), end: uint64(block.timestamp) + uint64(duration) }));
	...
	function withdrawAll() external {
		uint64 end = getLastedUnlockTime(_msgSender());
		require(block.timestamp >= end, "TimeLockPool.withdrawAll: unable to withdraw all yet" );
		for (uint i = 0; i <  depositsOf[_msgSender()].length; i++) {
			withdraw(i, _msgSender());
		}
	}

	function withdraw(uint256 _depositId, address _receiver) public {
		require(_receiver == _msgSender(), "TimeLockPool.withdraw: receiver should be msg.sender");
	...
	function getLastedUnlockTime(address _acount) public view returns (uint64) {
		require(depositsOf[_acount].length > 0, "TimeLockPool.getLastedUnlockTime: No deposit");
		uint64 lastestUnlock = depositsOf[_acount][0].end;
		if(depositsOf[_acount].length == 1) {
			return lastestUnlock;
		}
		for(uint i = 1; i < depositsOf[_acount].length; i++) {
			uint64 end = depositsOf[_acount][i].end;
			if (end > lastestUnlock) {
				lastestUnlock = end;
			}
		}
		return lastestUnlock;
...
abstract contract BasePool is ERC20Votes, AbstractRewards, IBasePool, Ownable {
	using SafeERC20 for IERC20;
