 ...
 */
contract MIMRouter is IRouterV2, SushiswapSwaps, Ownable {

    using SafeERC20 for IERC20;

    IVaultTransfers public mimHiveVault; // MIM Hive Router address
    IVotingStakingRewards public votingStakingRewards; // XBE Staking Contract
    ICurve3PoolDepositZap public curve3PoolZap; // Curve 3 pool Zap Contract - 0xA79828DF1850E8a3A3064576f380D90aECDD3359

    address public curveMimMetaPool; // Curve MIM StableSwap Meta Pool- 0x5a6A4D54456819380173272A5E8E9B9904BdF41B

    address public xbe; // XBE token
    address public mim; // MIM token
    address public usdc; // USDC token
    address public dai; // DAI token
    ...
    struct ConstructorParams {
        address _easyXBESushiswapOracle; // SushiSwap oracle
        address _uniswapV2Router; // SushiSwap Router02
        address _xbe; // XBE
        address _mimHiveVault; // MIM Hive Vault
        address _votingStakingRewards; // Voting Staking Rewards (XBE Staking Contract)
        address _curve3PoolZap; // Curve 3pool Zap Depositor Contract
        address _curveMimMetaPool; // Curve MIM StableSwap Meta Pool
        address[] _addresses; // addresses of tokens in Curve MIM StableSwap Meta Pool
        uint256[] _tokenIdxsInCurvePool; // indexes of tokens in Curve MIM StableSwap Meta Pool
    }

    /**
     * @notice Constructor for MIMRouter
     * @param params Struct in memory for initializing router
     */
  ...
  /**
   * @notice This setter is to manage indicies of tokens in Curve MIM StableSwap Meta Pool
   * @param _token Token of the pool
   * @param _index Index of the token in the pool
   */
    ...
    function _depositToCurve(address _token, uint256 _tokensReceived, uint256 _minAmountLP)
        internal
        returns(uint256 curveIronBankLpAmount)
    {
        uint256[4] memory amounts = [uint256(0), uint256(0), uint256(0), uint256(0)];
        amounts[tokenToIndex[_token]] = _tokensReceived;
        TransferHelper.safeApprove(_token, address(curve3PoolZap), _tokensReceived);
    ...
    function _depositToHive(address _recipient, uint256 _curve3PoolZapLpAmount) internal {
        TransferHelper.safeApprove(curveMimMetaPool, address(mimHiveVault), _curve3PoolZapLpAmount);
        mimHiveVault.depositFor(_curve3PoolZapLpAmount, _recipient);
    }

    /**
     * @notice This function is for deposit to MIM Hive Vault.
     * It should take your either ETH or USDC, or DAI, or USDT, or MIM,
     * convert to USDC amount A of ETH if ETH provided,
     * convert amount B of provided token to XBE and auto-stake it for you.
     * The A amount of provided token goes to Curve MIM StableSwap Meta pool through
     * Curve 3pool Zap Depositor contract. Then all acquired LP of the curve pool staked in MIM Hive Vault
     * for you. You must approve amountA + amountB amount of the tokens to this contract if they are not ETH.
    ...
    function deposit(DepositParams calldata params)
        ...
        returns(uint256 curve3PoolZapLpAmount)
    {
        (uint256 tokensReceived, uint256 swappedAmountBToXBE) = _performSwaps(params);

        // equals to amount of LP Hive
        curve3PoolZapLpAmount = _depositToCurve(params.token, tokensReceived, params.minAmountLP);

        _depositToVotingStakingRewards(swappedAmountBToXBE, params.recipient);
        _depositToHive(params.recipient, curve3PoolZapLpAmount);
    }

    /**
     * @notice This function is for withdraw funds from MIM Hive Vault.
     * It should take your provided MIM Hive Vault LP tokens, withdraw your Curve MIM StableSwap Meta Pool
     * LP tokens, withdraw requested token from Curve and swap to ETH if needed, depending on your
     * desire. The result of the swap if one occurred then transfered to you or provided address of a recipient.
