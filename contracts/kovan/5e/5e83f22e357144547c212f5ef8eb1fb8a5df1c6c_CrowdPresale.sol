...
contract CrowdPresale {
        ...
        notYetClaimedOrRefunded
    {
        ...
        uint256 claimAmount = getTokenAmount(investments[msg.sender]);
        if(claimAmount > 0){
    ...
    function burnUnsoldTokens() external investorOnly {
        require(cakeLiquidityAdded);
        require(!presaleCancelled);
        require(block.timestamp >= closeTime + 1 minutes);

        uint256 unsoldTokensAmount = getTokenAmount(investments[msg.sender]);
