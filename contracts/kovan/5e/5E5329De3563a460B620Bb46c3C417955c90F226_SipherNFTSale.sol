...
contract SipherNFTSale is ISipherNFTSale, Whitelist {
    ...
    uint32 public constant MAX_PUBLIC_BOUGHT = 15;
    uint32 public constant PUBLIC_SALE_CAP_PER_ADDRESS = 5;
    uint32 public constant REDUCE_PRICE_INTERVAL = 60; //10 minutes
    ...
    function withdrawSaleFunds(address payable recipient, uint256 amount)
        external
        onlyOwner
    {
    ...
    function getElapsedPublicSaleTime() internal view returns (uint256) {
        SaleConfig memory config = _saleConfig;
        uint256 timestamp = _blockTimestamp();
        if (timestamp < config.publicTime) {
            return 0;
        } else {
            return
                (config.publicTime <= timestamp) && (timestamp < config.publicEndTime)
                    ? timestamp - config.publicTime
                    : config.publicEndTime - config.publicTime;
        }
    }
    ...
    function getPublicSaleCurrentPrice() public view returns (uint256) {
        ...
        if (elapsed < REDUCE_PRICE_INTERVAL) {
            ...
            return (currentPrice);
                ...
                return
                (elapsed < 16*REDUCE_PRICE_INTERVAL)
                ? currentPrice
                : SALE_BASE_PRICE; 
            }
    }
    ...
    function rollStartIndex() external override {
        require(
            _blockTimestamp() > _saleConfig.endTime,
            "SipherNFTSale: sale not ended"
        );
     ...
     */
    function updateSaleConfigTime(uint64 _endTime) 
        external 
        onlyOwner {
    ...
    function getSaleConfig()
        ...
        view
        override
        returns (SaleConfig memory config)
    {
        config = _saleConfig;
    }
    ...
    function getSaleRecord()
        ...
        view
        override
        returns (SaleRecord memory record)
    {
        record = _saleRecord;
    }
    ...
    function getUserRecord(address user)
        ...
        view
        override
        returns (UserRecord memory record)
    {
    ...
    function _validateAndUpdateWithBuyAmount(
        ...
        require((config.publicTime <= timestamp && timestamp < config.publicEndTime) || (config.privateTime <= timestamp && timestamp < config.endTime) , "SipherNFTSale: Not in Sale Time");
        ...
        if (
            ...
            require(
                totalPaid == amount * SALE_BASE_PRICE,
                "SipherNFTSale: invalid paid value"
            );
            require(
        ...
        if (config.freeMintTime <= timestamp && timestamp < config.endTime) {
            // only whitelisted can buy at this period 
            require(
                totalPaid == 0, "Invalid paid amount"
            );
            require(
                isWhitelistedAddress(buyer, privateCap, freeMintCap, proofs) &&
                    whitelistedMerkelRoot != bytes32(0),
