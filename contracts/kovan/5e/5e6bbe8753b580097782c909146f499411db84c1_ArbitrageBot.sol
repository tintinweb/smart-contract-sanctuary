...
contract ArbitrageBot is AccessControl {
     ...
     * @param percentage The incremental percentage
     */
    function _execute(address tokenIn, address tokenOut, uint256 tokenAmountIn, uint256 percentage)
        internal
    {
        uint maxPrice = bPool.getSpotPrice(tokenIn, tokenOut).mul(percentage).div(100);
    ...
    /**
     *@notice create a batch transaction to swap multiple pairs on the Balancer pool
     ...
     * @param percentages list of the incremental percentage
     */
    function multiTrade(
    	...
    	external
    {
        ...
        for (uint8 i = 0; i < tokensIn.length; i++) {
        	_execute(tokensIn[i], tokensOut[i], tokensAmountIn[i], percentages[i]);
        }
    }
