...
contract Chain is VRFConsumerBase, VRF {
    ...
    function eligibleToSubmit(uint mySlot, 
            bytes memory vrfProof) public view returns (uint256 result) {
        if (randomnessRequested)
            return 1;
        if (block.number - latestSeedBlock >= TIMEOUT_BLOCKS)
            return 2;
        if (members[mySlot] != msg.sender)
            return 3;
        ...
        if (preSeed != latestSeed)
            return 4;

        if (!(publicKey[0] == publicKeys[msg.sender][0] && publicKey[1] == publicKeys[msg.sender][1])) 
            return 5;

        if (!(randomValueFromVRFProof(vrfProof) % membersLengthAtSeed == mySlot))
            return 6;
    ...
    function submitBlock(BlockSubmission calldata submission) public {
        require(eligibleToSubmit(submission.mySlot, submission.vrfProof) == 0);
