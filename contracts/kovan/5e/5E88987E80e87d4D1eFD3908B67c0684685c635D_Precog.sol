...
contract Precog {
    ...
    function maxProfitable(address user) public view returns (
        uint profit, address exchange, address[] memory pairs, address futureExchange, address futureToken
    ) {
        if (availableAmount[user] > feeTrading) {
            for (uint k = 0; k < futureExchanges.length; k++) {
                ...
                for (uint j = 0; j < tradingTokens.length; j++) {
                    ...
                    (_pairs[0], _pairs[1]) = (usdc, tradingTokens[j]);
                    for (uint256 i = 0; i < exchanges.length; i++) {
                        IExchangeRouter _exchange = IExchangeRouter(exchanges[i]);
                        (uint256 _profit, address _futureToken) = _calculateProfit(user, _exchange, _pairs, _futureExchange);
                        if (_profit > profit) {
                            profit = _profit;
    ...
    ) view public returns(uint profit, address futureToken) {
        uint amount = availableAmount[user] - feeTrading;
        try exchange.getAmountsOut(amount, pairs) returns(uint[] memory amountsOut) {
            uint minPriceAmount = _getMinPriceAmount(futureExchange, amountsOut[1], pairs);
            uint maxPriceAmount = _getMaxPriceAmount(futureExchange, amountsOut[1], pairs);
    ...
    function test(
        ...
        uint256 amountIn, 
        address[] memory pairs
    ) external view returns (address minPriceToken, uint expiryDate, uint  amountOut) {
         minPriceToken = futureExchange.minPriceToken(pairs[1], pairs[0]);
         expiryDate = IFutureToken(minPriceToken).expiryDate();
         amountOut = futureExchange.getAmountsOutFuture(amountIn, pairs[1], pairs[0], expiryDate);
