...
contract DeepWatersLending is ReentrancyGuard {
    ...
    **/
        ...
        nonReentrant
    {
        ...
        bool isUserStableRateBorrow = vault.isUserStableRateBorrow(ETH_ADDRESS, msg.sender);
        
        if (newBorrowBalance == 0 && isUserStableRateBorrow) {
            vault.setAverageStableInterestRate(ETH_ADDRESS, msg.sender, 0);
            vault.setBorrowRateMode(ETH_ADDRESS, msg.sender, false);
        }
    ...
    **/
        ...
        internal
    {
        ...
        bool isUserStableRateBorrow = vault.isUserStableRateBorrow(_asset, _user);
    ...
    **/
    function liquidation(address _user) external onlyLiquidator {
        ...
        (, , , data.healthFactor, ) = dataAggregator.getUserData(_user);
        ...
        for (uint256 i = 0; i < assetsList.length; i++) {
            ...
            data.userDebtBalance = vault.getUserBorrowBalance(data.assetAddress, _user).
                add(vault.getUserBorrowCurrentLinearInterest(data.assetAddress, _user));
            
            if (data.userDebtBalance > 0) {
                vault.updateLiquidatorDebt(data.assetAddress, liquidator, data.userDebtBalance);
                vault.updateBorrowBalance(data.assetAddress, _user, 0);
                
                if (vault.isUserStableRateBorrow(data.assetAddress, _user)) {
                    vault.setAverageStableInterestRate(data.assetAddress, _user, 0);
                    vault.setBorrowRateMode(data.assetAddress, _user, false);
                }
            }
            
            data.dTokenBalance = vault.getUserAssetBalance(data.assetAddress, _user);
            
            if (data.dTokenBalance > 0) {
                data.interestOnDeposit = vault.getInterestOnDeposit(data.assetAddress, _user);
        
                data.dToken = IDToken(vault.getAssetDTokenAddress(data.assetAddress));
                data.dToken.burn(_user, data.dTokenBalance);
                data.dToken.mint(liquidator, data.dTokenBalance.add(data.interestOnDeposit));
            }
