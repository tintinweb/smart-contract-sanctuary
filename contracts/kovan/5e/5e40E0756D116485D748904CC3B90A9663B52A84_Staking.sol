...
contract Staking is ReentrancyGuard, Context, IStaking {
    ...
    function deposit(uint256 amount, address sender) external override onlyFactory returns (bool isNewStake) {
    ...
    function withdraw(uint256 amount, address sender) external override onlyFactory returns (bool isEndStake) {
        require(amount > 0, "Input error");
        users[sender].lastRewardId = _upgradeReward(sender);
        users[sender].amountStaked -= amount;
        if (users[sender].amountStaked == 0) {
    ...
    function _upgradeReward(address user) private returns (uint256) {
        ...
        uint256 i = info.lastRewardId;
        for (; i < rewards.length; i++) {
            if (rewards[i].timestamp < info.lastTimestamp || rewards[i].timestamp - info.lastTimestamp > IStakingFactory(FACTORY).getFreezeTime()) {
                info.amountOfDividends += (info.amountStaked * rewards[i].amountReward) / rewards[i].totalStaked;
            }
