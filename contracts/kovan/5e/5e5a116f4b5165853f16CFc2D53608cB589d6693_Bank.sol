...
contract Bank is IBank, ERC20UpgradeSafe, ReentrancyGuardUpgradeSafe, OwnableUpgradeSafe {
  ...
  function positionInfo(uint256 id) public view returns (uint256, uint256) {
    ...
    return (IUniswapV2Worker(pos.worker).health(id), debtShareToVal(pos.debtShare));
  }
  ...
  function deposit(uint256 amountToken)
    ...
    //fix me
    // require(totalSupply() > 1e5, "Bank::deposit:: no tiny shares");
  }
  ...
  function withdraw(uint256 share) external override accrue(0) nonReentrant {
    ...
    // require(supply == 0 || supply >= 1e5, "Bank::withdraw:: no tiny shares");
  }
  ...
  function _masterChefDeposit(uint256 id, uint256 amount) internal {
    if (amount > 0) {
      ...
      IMiniChefV2(config.getMasterChefAddr()).deposit(masterChefID, amount, positions[id].owner);
    }
  }

  function _masterChefWithdraw(uint256 id) internal {
    console.log('bank:_masterChefWithdraw:',id);
    if (positions[id].debtShare > 0) {
      // Note: Do this way because we don't want to fail open, close, or kill position
      // if cannot withdraw from  somehow. 0x0ad58d2f is a signature of withdraw(uint256,uint256,address)
      // IMiniChefV2(config.getMasterChefAddr()).withdraw(masterChefID, positions[id].debtShare, positions[id].owner);
      (bool success, ) = config.getMasterChefAddr().call(abi.encodeWithSelector(0x0ad58d2f, masterChefID, positions[id].debtShare, positions[id].owner));
  ...
  function kill(uint256 id) external onlyEOA accrue(0) nonReentrant {
    ...
    uint256 health = IUniswapV2Worker(pos.worker).health(id);
    ...
    IUniswapV2Worker(pos.worker).liquidate(id);
