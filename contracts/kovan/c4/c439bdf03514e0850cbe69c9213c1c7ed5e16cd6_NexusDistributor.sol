    ...
    IDistributor
{
  ...
  */
   ...
   function buyCover(
        ...
        (uint256 _coverPriceInBuyingAsset, uint256 _priceInNXM ) = abi.decode(data,(uint256, uint256 ));
        ...
        // (buyCoverValue,  coverAsset) = _defineBuyingCoverValue(coverAsset,coverPrice,_priceInNXM);

        uint256 _maxNXMinCoverPrice = _howManyTokensAinB(address(wNXMToken),coverAsset,coverPrice);

        if(_maxNXMinCoverPrice < _priceInNXM){
            uint256 _priceInWNXM = _buyWNXM(coverAsset,_priceInNXM);
            _unwrapWNXM(_priceInWNXM);
            uint256 _discountEarnings = IERC20Upgradeable(coverAsset).balanceOf(address(this));
            // _buyingCoverValue = _priceInWNXM;
            // _buyingCoverAsset = address(nxmToken);
            transferToTreasury(_discountEarnings, coverAsset);
        }
    ...
    //         (buyCoverValue, ) = _defineBuyingCoverValue(coverAsset,coverPrice,_priceInNXM);
    //     } else {
    ...
    //     if (coverPriceWithFee.sub(coverPrice) > 0) {
  ...
  */
    ...
    function _defineBuyingCoverValue(
        address _coverAsset,
        uint256 _priceInCoverAsset,
        uint256 _priceInNXM
    ) public returns(uint256 _buyingCoverValue,address _buyingCoverAsset){
       
       uint256 _maxNXMinCoverPrice = _howManyTokensAinB(address(wNXMToken),_coverAsset,_priceInCoverAsset);
