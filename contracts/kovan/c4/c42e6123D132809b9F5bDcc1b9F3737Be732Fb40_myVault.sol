...
contract myVault {
	...
	function getTotalBalance() public view returns(uint) {
		...
		uint wethBalance = 0; //wethToken.balanceOf(address(this));
	...
	function rebalance() public {
		...
		if (usdBalancePercentage < usdTargetPercentage) {
			uint amountToSellUSD = (usdBalancePercentage - usdTargetPercentage) / 100 * totalBalance;
			emit logThis('amountToSellUSD', amountToSellUSD);
			sellWeth(amountToSellUSD);
		} else {
			uint amountToBuyUSD = (usdTargetPercentage - usdBalancePercentage) / 100 * totalBalance;
