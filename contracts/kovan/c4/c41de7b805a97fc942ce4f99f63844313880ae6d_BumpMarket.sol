...
///@title BumpMarket contract represent reserve of Maker deposits
///@notice This contains all methods to interact with stablecoin reserve
    ...
    GovernanceAndOwner
{
    ...
    ///@dev interest rate is 0 for now, but will be set to appropriate value in the future release
    struct Deposit {
        uint256 interest;
        uint256 balance;
        uint256 timestamp;
    ...
    ///@dev This maps an address to cumulative details of deposit made by maker
    ...
    ///@dev This value represents maximum percentage of total deposit amount ,user can use to buy BUMP tokens.
    ///@dev Decimal precision will be upto 2 decimals.
    uint256 public maxBumpPercent;

    ///@dev This state variable stores number of BUMP tokens available to be distributed as rewards.
    uint256 public bumpRewardAllocation;

    ///@dev This state variable stores maximum number of BUMP tokens that can be purchased by the users.
    uint256 public bumpPurchaseAllocation;

    ///@dev This stores address of USDC yearn vault
    address public usdcVault;

    ///@dev This stores address of BUMP token
    address public bumpTokenAddress;

    ///@dev This stores address of bUSDC token
    address public busdcTokenAddress;

    ///@dev These will be constants used in formulas
    // BUMP_INITAL_PRICE is in cent.
    ...
    //This event is emitted after maker deposit is made
    event DepositMade(
    ...
    ///@dev This event is emitted when rewards are issued to the user at the time of deposit
    event RewardIssued(address indexed rewardee, uint256 amount, uint256 price);

    ///@dev This event is emitted when BUMP are purchased by user
    event BumpPurchased(
        address indexed depositor,
        uint256 amount,
        uint256 price
    ...
    ///@dev This method is called during deployement by open zeppelin and work like constructor
    ...
    ///@param _bumpTokenAddress This is the address of BUMP token
    ///@param _busdcTokenAddress This is the address of BUSDC token
    ///@param _maxBumpPercent This is the maximum percentage of deposit amount that can be used to buy BUMP tokens.
    ///@param _bumpRewardAllocation This stores maximum number of BUMP tokens that can be distributed as rewards.
    ///@param _bumpPurchaseAllocation This stores maximum number of BUMP tokens that can be purchased by the users.
    function initialize(
    ...
    ///@notice This method pauses BumpMarket contract.
    function pause() external virtual whenNotPaused onlyGovernance {
        _pause();
    }

    ///@notice This method un-pauses BumpMarket contract.
    ...
    function pauseBusdc() external virtual onlyGovernance {
        BUSDC(busdcTokenAddress).pause();
    }

    ///@notice This method un-pauses bUSDC token and can only be called by governance.
    function unpauseBusdc() external virtual onlyGovernance {
        BUSDC(busdcTokenAddress).unpause();
    }

    ///@notice This returns number of yUSDC tokens issued on the name of BumpMarket contract.
    ///@return amount returns amount of yUSDC issued to BumpMarket by yearn vault.
    ...
    ///@notice This acts like external onlyGovernance interface for internal method _approveUSDCToYearnVault.
    ...
    //////@notice This acts like external onlyGovernance interface for internal method _depositUSDCInYearnVault.
    ...
    ///@return Returns amount of USDC redeemed
    ...
    ///@notice This function updates lockTimeStamp variable of busdc
    ///@param _unlockTimeStamp New deadline for lock in period
    function updateBusdcUnLockTimestamp(uint256 _unlockTimeStamp)
        external
        virtual
        onlyGovernance
    {
        BUSDC(busdcTokenAddress).updateUnlockTimestamp(_unlockTimeStamp);
    }
    ...
    ///@notice This method estimates how much BUMP you will get as rewards if certain amount of deposit is made
    ...
    ///@return Amount of BUMP rewards you will get if certain deposit amount is made
    ...
    ///@notice This function returns predicted swap rate for BUMP/USDC after given deposit is made.
    ...
    function getBumpAllocation(
        ...
        require(
            ...
            "No more BUMP rewards can be issued"
        );
        ...
        require(
            ...
            "You can't purchase this much amount of BUMP Tokens"
        );
...
///@title  BUMPToken is the BUMP ERC20 Token
///@notice This contract is an erc20 contract hence contains all the functions of an erc20.
    ...
    GovernanceAndOwner
{
    ...
    ///@param _increaseInSupply Amount by which supply will increase.
    ///@dev So this basically mints new tokens in the name of protocol.
    function mint(uint256 _increaseInSupply) external virtual onlyGovernance {
        _mint(address(this), _increaseInSupply);
    }
    ...
    ///@notice This function is called when we want to distribute BUMP tokens from protocol to some address.
    ...
    ///@notice This function is called when we want to transfer tokens when it's not timelocked
    ...
    ///@notice This function is called when we want to approve tokens when it's not timelocked
    ...
    ///@notice This function is called when we want to transfer tokens from one account to another using allowance mechanism when it's not timelocked
