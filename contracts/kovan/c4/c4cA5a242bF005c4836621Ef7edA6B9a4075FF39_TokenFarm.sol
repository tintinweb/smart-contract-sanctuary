...
contract TokenFarm is Ownable {
    ...
    mapping(address => mapping(address => uint256)) public stakingBalance;    // mapping token_address -> staker address -> address
    ...
    function getUserSingleTokenValue(address _user, address _token) public view returns (uint256) {
        //1ETH == 2000USD return 2000, or 200DAI == 200USD return 200
        if (uniqueTokensStaked[_user] <=0) {
            return 0;
        }
        //price of the token * stakingBalance[_token][user]
        (uint256 price, uint256 decimals) =  getTokenValue(_token);
        // 10 ETH -> price is 100. 10*100=1000USD
