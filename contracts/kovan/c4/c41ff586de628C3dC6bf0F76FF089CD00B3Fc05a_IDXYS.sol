...
contract IDXYS is Ownable{
    ...
    uint256 vaultCount;                                                         // a counter for the vaults
    ...
    mapping(address => bool) userRegistered;                                    // when set to true the user is allowed to use the contract
    mapping(address => uint256) userTier;                                       // the user tier
    ...
    function deposit(uint256 assetId, uint256 assetAmount, uint256 vaultId) public payable {
        ...
        require(tier[msg.sender] >= vault.tier, "Wrong Tier!");                                                     // require that this user have acces to this vault
        Currentvault.deposit(asset.addr, assetAmount);
