...
//    }
    ...
    function getNetValue(address pool) view external returns(NetValue memory,uint256,uint256){
        (NetValue[] memory netValues,)= getNetValuesInView();
        for(uint256 i = 0; i < netValues.length; i++){
            if(netValues[i].pool == pool) return (netValues[i],block.timestamp,block.number);
        }
    }
   ...
   function culculate(uint256 apy,bool isPositive,CommissionRate[] storage cp,CommissionRate[] storage cn,uint256 amountInUSD) internal view returns (bool,uint256){ 
    ...
    //test function
     function getNetValueForTest(address pool) view external returns(NetValue memory,NetValue memory,uint256,uint256){
        (NetValue[] memory netValues,NetValue[] memory netValuesNew)= getNetValuesInViewForTest();
        for(uint256 i = 0; i < netValues.length; i++){
            if(netValues[i].pool == pool) return (netValues[i],netValuesNew[i],block.timestamp,block.number);
        }
    }
    function getNetValuesInViewForTest() view public returns(NetValue[] memory netValues,NetValue[] memory netValuesNew){
        address[] memory fixedPools = IController(controller).getFixedPools();
        address[] memory flexiblePools = IController(controller).getFlexiblePools();         
        TokenPrice[] memory tokenPrices = new TokenPrice[](tokenCount);
        // get all tokens in pool and strategy
        ...
        netValuesNew = new NetValue[](fixedPools.length.add(flexiblePools.length));
        //  commision calculalte 
        if(!isCommissionPaused) 
            (netValuesNew,) = commissionFixed(fixedPools.length,poolInfos,netValues);
