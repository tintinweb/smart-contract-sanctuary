...
contract Salvation is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    event ApprovalDelay(uint256 approvalDelay);
    event SetBurnFee(uint256 burnFee);
    event SetTaxFee(uint256 taxFee);
    ...
    constructor() public {
        ...
        _approvalDelay = 8400; // 1 day approval delay on new strategies
        _takeFee;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // swap tokens for ETH
        swapTokensForEth(half); // <- this breaks the ETH -> SLTVN swap when swap+liquify is triggered
    ...
    function setTakeFee(bool takeFee) external onlyOwner() {
        _takeFee = takeFee;
        emit TakeFeesUpdated(takeFee);
    }
    ...
    function _setApprovalDelay(uint256 approvalDelay) external onlyOwner() {
        ...
        emit ApprovalDelay(approvalDelay);
    }
