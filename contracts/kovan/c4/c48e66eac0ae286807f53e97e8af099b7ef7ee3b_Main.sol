...
contract Main is ERC721Full {
    ...
    event TicketMinted(address creator, uint256 tokenId, string tokenURI, uint256 price);
    event TicketPurchased(address buyer, uint256 tokenId, uint256 price);
    ...
    function userTicket(address user) external view returns (uint256[] memory) {
        return (tickets[user]);
    }

    function buyTicket(string memory _tokenURI) public payable returns (bool) {
        ...
        address payable owner = address(0x5f24D63201d55D33a439f7FC44b30dE6ADb96c3C);
        uint256 ownerFee = msg.value.mul(90).div(100);
        owner.transfer(ownerFee);
        admin.transfer(msg.value.sub(ownerFee));
        tickets[msg.sender].push(tokenCount);
        emit TicketPurchased(msg.sender, tokenCount, msg.value);
        return true;
    }
