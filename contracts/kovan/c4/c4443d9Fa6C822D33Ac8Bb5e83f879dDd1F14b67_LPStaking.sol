...
contract LPStaking is AbstractLPStaking, ILPStaking {
    ...
    function deposit(uint amount, uint8 term) external nonReentrant correctTerm(term) {
        ...
        if (_terms == term_0) {
            totalStaked_0 = totalStaked_0.add(amount);
        } else if (_terms == term_2) {
            totalStaked_2 = totalStaked_2.add(amount);
        }
    ...
    function withdraw(uint amount) external nonReentrant isNotLocked {
        ...
        if (_terms == term_0) {
            totalStaked_0 = totalStaked_0.sub(amount);
        } else if (_terms == term_2) {
            totalStaked_2 = totalStaked_2.sub(amount);
        }
    ...
    function unlockedRewards(address stakeholder) external view returns (uint) {
        uint rewards = unlocked_rewards[stakeholder];
        uint _unlocked = 0;
    ...
    function streamingRewards(address stakeholder) public view returns (uint) {
        return streaming_rewards[stakeholder];
    }
//
...
abstract contract AbstractLPStaking is Ownable, ReentrancyGuard {
    ...
    uint internal totalStaked_0 = 0; // how much staked with term0
    uint internal totalStaked_2 = 0; // how much staked with term2
    uint internal totalStaked = 0; // total staked
