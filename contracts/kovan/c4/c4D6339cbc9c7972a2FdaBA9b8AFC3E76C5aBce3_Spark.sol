...
TO DO:
0.9.2:
    1. remove user stats to backend [done]
final clean up:
    1. fix comments
    2. library cleanup [done]
    3. switch back to private
*/
...
contract Spark is Ownable {
    ...
    uint public constant APR_PREC = 100000;
    ...
    function get_userProf(address user) public view returns (uint){
        return (_userProf[user].nextID);
    }
    function get_userDepo(address user, uint index) public view returns (uint, uint, uint, uint){
        return (_userDepo[user][index].stakedLP, _userDepo[user][index].timestampSec, _userDepo[user][index].amountClaimed, _userDepo[user][index].requestID);
    }
    ...
    function claim(uint index) public returns (uint) {
        ...
        if (_userDepo[msg.sender][index].requestID != 0) {
    ...
    function requestFreeClaim(uint index) public {
        ...
        require(_userDepo[msg.sender][index].requestID == 0, "requestFreeClaim: a request has already been placed.");
    ...
    function _cancelFreeClaim(address staker, uint index) private {
        ...
        require(_userDepo[staker][index].requestID != 0, "_cancelFreeClaim: there is no request on this deposit.");
        // global record
     ...
     * @dev "interestRate" includes APR_PREC
    ...
    function computeUnitRewardFor(address staker, uint index) public view returns (uint) {
        ...
        uint interestRate = computeInterestWithPrecision(timeDay);
