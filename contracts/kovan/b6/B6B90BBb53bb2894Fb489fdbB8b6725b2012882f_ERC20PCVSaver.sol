...
/// @title a pcv saver for save token
/// @author Ring Protocol
contract ERC20PCVSaver is PCVSaver {
    ...
    /// @param to address to receive Saver token
    /// @param amountIn amount of underlying tokens input
    /// @return amountOut amount of Saver token received
    function purchase(address to, uint256 amountIn)
        external
        payable
        virtual
...
interface IPCVSaver is IBondingCurve, IERC20 {
    
    event PurchaseLimitUpdate(uint256 _saveLimit);
    event PcvTokenHolderAddressUpdate(address _pcvTokenHolder);

    // ----------- State changing Api -----------
    function setPurchaseLimit(uint256 _saveLimit) external;
    function setPcvTokenHolder(address _pcvTokenHolder) external;

    // ----------- Getters -----------

    function purchaseLimit() external view returns (uint256);
    function pcvTokenHolder() external view returns (address);
}
...
import "./IPCVSaver.sol";
import "../refs/CoreRef.sol";
import "../pcv/PCVSplitter.sol";
import "../utils/Timed.sol";

/// @title an abstract pcv saver for saving token
/// @author Ring Protocol
abstract contract PCVSaver is IPCVSaver, ERC20Burnable, CoreRef, PCVSplitter, Timed {
    using Decimal for Decimal.D256;
    using SafeMathCopy for uint256;

    // set who holds the pcv LP token
    address public override pcvTokenHolder;
    ...
    /// @notice the total amount of token purchased in saver.
    uint256 public override totalPurchased;

    /// @notice the buffer applied on top of save amount
    ...
    /// @notice the limit of token can be saved in saver.
    ...
    /// @param _purchaseLimit the limit of token can be saved in saver
    /// @param _tokenName the token name of saver token
    /// @param _tokenSymbol the token symbol of saver token
    constructor(
        address _core,
        address[] memory _pcvDeposits,
        uint256[] memory _ratios,
    ...
    function allocate() external override whenNotPaused {
        require((!Address.isContract(msg.sender)), "PCVSaver: Caller is a contract");
        uint256 amount = getTotalPCVHeld();
        require(amount != 0, "PCVSaver: No PCV held");
    ...
    /// @notice return amount of Saver token received after a successful save
    /// @param amountIn the amount of underlying used to save
    /// @return amountOut the amount of Saver token received
    function getAmountOut(uint256 amountIn)
        external
        view
        override
    ...
    /// @notice mint Saver token and send to saver destination
        ...
        returns (uint256 amountOut)
    {
        ...
        require(totalPurchased <= purchaseLimit, "PCVSaver: Save limit reached");
        _mint(to, amountOut);
        uint256 amountProtocol = amountIn - amountOut;
        if (amountProtocol > 0 && pcvTokenHolder != address(0)) {
            _mint(pcvTokenHolder, amountProtocol);
        }
    ...
    /// @notice returns the buffer on the saver
    ...
    /// @notice permit spending of Saver token
    /// @param owner the Saver token holder
    ...
    function permit(
        ...
        require(deadline >= block.timestamp, "PCVSaver: EXPIRED");
        bytes32 digest =
            keccak256(
                abi.encodePacked(
                    "\x19\x01",
        ...
        require(
            ...
            "PCVSaver: INVALID_SIGNATURE"
        );
        _approve(owner, spender, value);
    }

    function setPcvTokenHolder(address _pcvTokenHolder) external override onlyGovernor {
        pcvTokenHolder = _pcvTokenHolder;
        emit PcvTokenHolderAddressUpdate(_pcvTokenHolder);
    }

    /// @notice this function is not used in pcv saver
