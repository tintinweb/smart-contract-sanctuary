...
contract AtomicSwap {
    ...
    )
  {
    ...
    uint256[2] memory amounts = _getNeededAmountAndLeftover(collateralAmountOut, tokenSwapPath, tokenAmountIn);
    ...
    if (amounts[1] != 0) {
      inputTokenInstance.safeTransfer(msg.sender, amounts[1]);
    }
    ...
    )
  {
    ...
    uint256[2] memory amounts = _getNeededAmountAndLeftover(amountTokenOut, tokenSwapPath, collateralRedeemed);
    ...
    if (amounts[1] != 0) {
      collateralInstance.safeTransfer(msg.sender, amounts[1]);
    }
  ...
  function _getNeededAmountAndLeftover(uint256 tokenOuput, address[] memory path, uint256 readyAmount) internal view returns(uint256[2] memory amounts) {
