...
contract TKNSOFT is ERC20, ERC20Burnable, Ownable {
    ...
    modifier icoIsEnded(){
        require(preIcoStageOneEnded == true && preIcoStageTwoEnded == true);
        _;
    }
    ...
    function endPreIcoStageOne() external onlyOwner {
        preIcoStageOneEnded = true;
    }

    function endPreIcoStageTwo() external onlyOwner {
        preIcoStageTwoEnded = true;
    }

    /*Whilelisters*/

    modifier isWhitelister(address _beneficiary){
        require(whitelister[_beneficiary]);
        _;
    }

    function isWhiteLister(address _beneficiary) public view returns (bool) {
        return whitelister[_beneficiary];
    }
