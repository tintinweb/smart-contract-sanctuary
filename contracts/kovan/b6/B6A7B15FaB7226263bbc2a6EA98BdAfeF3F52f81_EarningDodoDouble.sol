...
// File: localhost/mint/tripartitePlatform/dodo/IDVM.sol
...
// File: localhost/mint/tripartitePlatform/dodo/EarningDodoDouble.sol
...
contract EarningDodoDouble is IEarning, Owner {
    ...
    SetAddress.Set private tokens;
    mapping(address => uint256) public rewardPerShares;//token->amount
    mapping(address => mapping(address => uint256)) public debts;//user->token->amount
    mapping(address => mapping(address => uint256)) public pendings;//user->token-amount
    IERC20Mine public mine;
    address private stakeToken;
    
    function init(address[] memory parameters) override external nonZeroAddress(parameters[0]) {
        address _mine = parameters[0];
        require(address(0) == owner, "publics:contract_is_already_initialized");
        owner = msg.sender;
        mine = IERC20Mine(_mine);
        stakeToken = mine._TOKEN_();
        config = IConfig(parameters[1]);
    }
    ...
    function stake(address user, uint256 amount) override external onlyOwner {
        if (0 == amount) {
            return;
        }
        _snapshot();
    ...
    function unStake(address user, uint256 amount) override external onlyOwner returns (uint256) {
        if (0 == amount) {
            return 0;
        }
        _snapshot();
    ...
    function _snapshot() internal {
        ...
        address token;
        uint256 reward;
        uint256 rewardCount = mine.getRewardNum();
        for (uint256 i = 0; i < rewardCount; i++) {
            token = mine.getRewardTokenById(i);
            SetAddress.add(tokens, token);
            reward = IERC20(token).balanceOf(address(this));
            mine.claimReward(i);
            reward = IERC20(token).balanceOf(address(this)).sub(reward);
            rewardPerShares[token] = rewardPerShares[token].add(reward.mul(1e24).div(totalStakeAmount));
            emit ClaimReward(token, reward);            
        }
    }
    
    function _setDebt(address user) internal {
        address token;
        uint256 amount = stakeAmounts[user];
        uint256 len = SetAddress.length(tokens);
        for (uint256 i = 0; i < len; i++) {
            token = SetAddress.at(tokens, i);
            debts[user][token] = amount.mul(rewardPerShares[token]);
        }
    }
    ...
    function _provideReward(address user) internal {
        ...
        for (uint256 i = 0; i < len; i++) {
            token = SetAddress.at(tokens, i);
            reward = stakeAmounts[user].mul(rewardPerShares[token]).sub(debts[user][token]).div(1e24);
            pendings[user][token] = pendings[user][token].add(reward);
        }
    }
    
    function provideReward(address user) external {
        _provideReward(user);
    }
    ...
    function pendingRewards(address user) override external view returns (address[] memory, uint256[] memory) {
        ...
        for (uint256 i = 0; i < len; i++) {
            token = SetAddress.at(tokens, i);
            _tokens[i] = token;
            _amounts[i] = pendings[user][token];
            reward = mine.getPendingRewardByToken(address(this), token);
            rewardPerShare = rewardPerShares[token].add(reward.mul(1e24).div(totalStakeAmount));
            reward = stakeAmounts[user].mul(rewardPerShare).sub(debts[user][token]);
            reward = reward.div(1e24);
            _amounts[i] = _amounts[i].add(reward);
        }
        return (_tokens, _amounts);
    }
    
    function claimRewardsV1(address user) override external onlyOwner returns (address[] memory, uint256[] memory) {
        return _claimRewards(user, msg.sender, false);
    }
    
    function claimRewardsV2() override external returns (address[] memory, uint256[] memory) {
    ...
    function _claimRewards(address user, address to, bool share) internal returns (address[] memory, uint256[] memory) {
        ...
        _provideReward(user);
        _setDebt(user);
        uint256 platformTake;
        uint256 platformTakeRate = config.getPlatformTakeRate(owner);
        address platformFee = config.getPlatformFee();
        uint256 len = SetAddress.length(tokens);
        address[] memory _tokens = new address[](len);
        uint256[] memory _amounts = new uint256[](len);
        for (uint256 i = 0; i < len; i++) {
            _tokens[i] = SetAddress.at(tokens, i);
            _amounts[i] = pendings[user][_tokens[i]];
            if (0 < _amounts[i]) {
                pendings[user][_tokens[i]] = 0;
                if (share) {
                    platformTake = _amounts[i].mul(platformTakeRate).div(10000);
                    if (0 < platformTake) {
                        _amounts[i] = _amounts[i].sub(platformTake);
                        IERC20(_tokens[i]).safeTransfer(platformFee, platformTake);
                    }
                }
                if (0 < _amounts[i]) {
                    IERC20(_tokens[i]).safeTransfer(to, _amounts[i]);
                }
            }
        }
        return (_tokens, _amounts);
    ...
    function getLPTokens() override external view returns (uint256, uint256, address[] memory, uint256[] memory) {
        IDVM dvm = IDVM(stakeToken);
        address[] memory _tokens = new address[](2);
        _tokens[0] = dvm._BASE_TOKEN_();
        _tokens[1] = dvm._QUOTE_TOKEN_();
        uint256[] memory _amounts = new uint256[](2);
        _amounts[0] = dvm._BASE_RESERVE_();
        _amounts[1] = dvm._QUOTE_RESERVE_();
        return (dvm.totalSupply(), mine.totalSupply(), _tokens, _amounts);
    }
    ...
    function getRewardTokenInfos() override external view returns (address[] memory rewardTokens, uint256[] memory rewardPerBlocks) {
        ...
        rewardPerBlocks = new uint256[](len);
        for (uint256 i = 0; i < len; i++) {
            (address rewardToken, , , , uint256 rewardPerBlock, , ) = mine.rewardTokenInfos(i);
            rewardTokens[i] = rewardToken;
            rewardPerBlocks[i] = rewardPerBlock;
        }
    }
}