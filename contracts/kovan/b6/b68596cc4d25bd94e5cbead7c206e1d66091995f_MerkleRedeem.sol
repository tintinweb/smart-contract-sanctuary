...
contract MerkleRedeem is Ownable {
    ...
    constructor(address token_, bytes32 merkleRoot_) public {
        ...
        finish = block.timestamp + 30*24*3600; // .TBD
    }
    ...
    function claim(address account, uint256 amount, bytes32[] calldata merkleProof) external {
        ...
        require(block.timestamp <= finish, 'MerkleDistributor: Drop is finished.');
    ...
    function withDrawErc20(address tokenAddress, uint256 tokenAmount) external onlyOwner {
        require(block.timestamp > finish, 'MerkleDistributor: Drop is not finished.');

        IERC20(tokenAddress).safeTransfer(_msgSender(), tokenAmount);
        emit WithDrawErc20(tokenAddress, tokenAmount);
    }
