...
contract PresalePublic is ReentrancyGuard {
    ...
    {
        require(
            intermediate.raisedAmountBeforeLiquidity >= generalInfo.softCapInWei,
            "sCap n riched"
        );
    ...
    function cancelPresale() external presaleIsNotCancelled {
        if(intermediate.raisedAmountBeforeLiquidity < generalInfo.softCapInWei && block.timestamp >= generalInfo.closeTimePresale) {
