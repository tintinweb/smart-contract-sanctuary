...
// File: contracts/interfaces/INEKUPool.sol
...
// File: contracts/interfaces/INEKUVaultMigrateable.sol

pragma solidity ^0.6.12;


interface INEKUVaultMigrateable {
    function depositBehalf(address account, uint _amount) external;
    function setMigrateDist(INEKUVaultMigrateable vault) external ;
    function migrate() external;
}

// File: contracts/NEKUVault.sol
...
contract NEKUVault is ERC20, Ownable, INEKUVaultMigrateable{
    ...
    address public nekuPool;

    uint256 public totalDeposit;
    uint256 public totalWithdraw;

    INEKUVaultMigrateable public newVault;
    ...
    constructor (
        ...
        string(abi.encodePacked("Neku ", ERC20(_token).name())),
        string(abi.encodePacked("N-V-", _symbol))
    ) {
        token = IERC20(_token);
    ...
    function balance() public view returns (uint) {
        return token.balanceOf(address(this))
                .add(INEKUController(controller).balanceOf(address(token)));
    }
    ...
    function depositInternal(address _account, uint _amount, bool _autoFarm) internal {
        ...
        if (nekuPool != address(0) && _autoFarm) {
            _mint(address(this), _shares);
            IERC20(address(this)).safeApprove(nekuPool, _shares);
            INEKUPool(nekuPool).stakeBehalf(_account, _shares);
        } else {
    ...
    function withdrawFromController(uint _shares) internal returns (uint){
        ...
        if (b < r) {
            ...
            INEKUController(controller).withdraw(address(token), _withdraw);
            uint _after = token.balanceOf(address(this));
    ...
    function earn() public {
        ...
        INEKUController(controller).earn(address(token), bal);
    }
    ...
    function accrueReward(address _account) public view returns (uint) {
        ...
        if (nekuPool != address(0)) {
            gb = gb.add(INEKUPool(nekuPool).balanceOf(_account));
        }
        uint amount = 0;
    ...
    function setMigrateDist(INEKUVaultMigrateable _newVault) external override onlyOwner {
        require(address(this) != address(_newVault), "Not self");
        newVault = _newVault;
    }

    function setNekuPool(address _nekuPool) external onlyOwner {
        require(address(this) != nekuPool, "Not self");
        nekuPool = _nekuPool;
    }
