...
contract Intermediate is Ownable {
    ...
    constructor (
        ...
        uint256 _feeAmount  // amount of processing fee that should be transferred to system wallet
    )
    {
        require(
    ...
    */
    ...
    function cancel(uint256 amount) external onlySystem {
        smartSwap.cancel(nativeToken, foreignToken, foreignTokenReceiver, amount);
    }
/*
...
*/
    function withdraw(uint256 amount) external onlyOwner {
        if (nativeToken < address(9))
            ...
            TransferHelper.safeTransfer(nativeToken, nativeTokenReceiver, amount);
    }

    // add liquidity to counterparty 
    function addLiquidityAndClaimBehalf(
        uint128 amount,    //amount of native token that has to be swapped
        uint128 currentRate,     // rate with 18 decimals: tokenB price / tokenA price
        uint256 foreignBalance,  // total tokens amount sent by user to pair on other chain
        address senderCounterparty, // correspondent value from SwapRequest event
        address receiverCounterparty    // correspondent value from SwapRequest event
    ) external onlySystem {
        ...
        if (nativeToken > address(9)) {
            ...
            smartSwap.addLiquidityAndClaimBehalf(
                nativeToken, 
                foreignToken, 
            ...
            smartSwap.addLiquidityAndClaimBehalf{value: uint256(amount) + fee}(
                nativeToken, 
                foreignToken, 
                foreignTokenReceiver, 
                false, 
