...
abstract contract BFactory {
    ...
    function isBPool(address b) external view virtual returns (bool);
}
...
contract BalancerUtils {
    function isBPool(address factoryAddress, address poolAddress) public view returns (bool){
        BFactory factory = BFactory(factoryAddress);
        return factory.isBPool(poolAddress);
    }

    function checkBPool(address smartPoolAddress) public view returns (address){
        ConfigurableRightsPool crpPool = ConfigurableRightsPool(smartPoolAddress);
        return address(crpPool.bPool());
    }

    function currentSpotPrice(address poolAddress, address tokenIn, address tokenOut) public view returns(uint) {
        BPool pool = BPool(poolAddress);
        return pool.getSpotPrice(tokenIn, tokenOut);
    }

    // todo change to use delegatecall
    function justSwap(address smartPoolAddress, address tokenIn, uint tokenAmountIn, address tokenOut, uint minAmountOut, uint maxPrice) public returns (uint, uint){
        ConfigurableRightsPool crpPool = ConfigurableRightsPool(smartPoolAddress);
        BPool bPool = crpPool.bPool();
        TokenInterface TI = TokenInterface(tokenIn);
        TokenInterface TO = TokenInterface(tokenOut);

        require(TI.transferFrom(msg.sender, address(this), tokenAmountIn), "ERR_TRANSFER_FAILED");

        if (TI.allowance(address(this), address(bPool)) < tokenAmountIn) {
            TI.approve(address(bPool), uint(-1));
        }
        (uint tokenAmountOut, uint spotPriceAfter) =
            bPool.swapExactAmountIn(tokenIn, tokenAmountIn, tokenOut, minAmountOut, maxPrice);
    ...
    function pokeSwap(address smartPoolAddress, address tokenIn, uint tokenAmountIn, address tokenOut) public returns(uint, uint){
        ConfigurableRightsPool crpPool = ConfigurableRightsPool(smartPoolAddress);
        crpPool.pokeWeights();
        ...
        (uint tokenAmountOut, uint spotPriceAfter) =
        bPool.swapExactAmountIn(tokenIn, tokenAmountIn, tokenOut, 1, uint(-1));
    ...
    function forceSwap(address smartPoolAddress, address tokenIn, uint tokenAmountIn, address tokenOut) public returns (uint,uint) {
        ConfigurableRightsPool crpPool = ConfigurableRightsPool(smartPoolAddress);
        BPool bPool = crpPool.bPool();
        TokenInterface TI = TokenInterface(tokenIn);
        TokenInterface TO = TokenInterface(tokenOut);
    ...
    function doPoke(address smartPoolAddress) public {
        ConfigurableRightsPool crpPool = ConfigurableRightsPool(smartPoolAddress);
        crpPool.pokeWeights();
    }

    // can use view function to calculate how much needed do gas optimization
    function swapPokeSwap(address smartPoolAddress, address tokenToSell, uint inAmount, address pairToken) public returns(uint){
        ConfigurableRightsPool crpPool = ConfigurableRightsPool(smartPoolAddress);
        BPool bPool = crpPool.bPool();

        // sell the token
        TokenInterface TI = TokenInterface(tokenToSell);
        TokenInterface TO = TokenInterface(pairToken);

        require(TI.transferFrom(msg.sender, address(this), inAmount), "ERR_TRANSFER_FAILED");

        if (TI.allowance(address(this), address(bPool)) < inAmount) {
            TI.approve(address(bPool), uint(-1));
        }

        (uint tokenAmountOut,) =
            bPool.swapExactAmountIn(tokenToSell, inAmount, pairToken, 1, uint(-1));

        // do the poke
        crpPool.pokeWeights();

        // buy the token back
        if (TO.allowance(address(this), address(bPool)) < tokenAmountOut) {
            TO.approve(address(bPool), uint(-1));
        }
        (uint tokenAmountOut2,) = bPool.swapExactAmountIn(pairToken, tokenAmountOut, tokenToSell, 1, uint(-1));

        require(TO.transfer(msg.sender, TO.balanceOf(address(this))), "ERR_TRANSFER_FAILED");
        require(TI.transfer(msg.sender, TI.balanceOf(address(this))), "ERR_TRANSFER_FAILED");

        uint amountsEarned = tokenAmountOut2 - inAmount;
