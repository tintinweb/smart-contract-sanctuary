...
// File: contracts\InvitationalNFT.sol
...
//import "./interfaces/IInvitationalNFT.sol";
...
contract InvitationalNFT is
    ...
    AccessControl
{
    bytes32 public MINTER_BURNER_ROLE = keccak256("MINTER_BURNER_ROLE");
    ...
    mapping(uint256 => bool) private usedIds; //save used token-ids
    //mapping(address => bool) private vaults; //save vaults to give access to funcs
    mapping(address => bool) private haveAGift; //save which user has minted invitational nft cause of it's once
    mapping(uint256 => address) private vaultById; //each nft give access only in current vault instance
    ...
    constructor(
        ...
        _setupRole(MINTER_BURNER_ROLE, _admin);
    }

    modifier onlyMinterBurner(address sender) {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, sender) ||
                hasRole(MINTER_BURNER_ROLE, sender) ||
                hasRole(VAULT_ROLE, sender),
            "Caller is not in owner or manager or vault role"
        );
        _;
    }

    /**
     * @dev When new vault instance has deployed, neccessary to give a vault role. ROLES: ADMIN
     * @param _contract vault address
     */
    function setVaultAddress(address _contract) external {
        require(
            address(0) != _contract &&
                hasRole(DEFAULT_ADMIN_ROLE, _msgSender()) &&
                !hasRole(VAULT_ROLE, _contract),
            "Wrong params"
        );
        /* if (vaultContract != address(0))
            revokeRole(MINTER_BURNER_ROLE, vaultContract); */
        //vaultContract = _contract;
        //vaults[_contract] = true;
        _setupRole(VAULT_ROLE, _contract);
        //_setupRole(MINTER_BURNER_ROLE, _contract);
    }

    /**
     * @dev Mint nft. ROLES: ADMIN/MINTER-BURNER/VAULT.
     * @param _tokenURI token URI
     * @param mintTo current address to transfer nft
     * @param vault vault-address in which the owner of the token will have an access
     */
    function mint(string calldata _tokenURI, address mintTo, address vault)
        external
        onlyMinterBurner(_msgSender())
    {
        require(!hasTokenWithURI[_tokenURI], "ERC721Main: URI already exists");
        require(mintTo != address(0) && mintTo != address(this) /* && hasRole(VAULT_ROLE, _msgSender()) */, "Wrong addresses");
    ...
    /**
     * @dev Set token statis: USED. ROLES: VAULT
     * @param _id token id.
     */
    function setUsedId(uint256 _id) external {
        require(hasRole(VAULT_ROLE, _msgSender()) && _exists(_id) && !usedIds[_id], "Wrong params");
        usedIds[_id] = true;
    }

    /**
     * @dev Set user status: HAVE A GIFT. ROLES: VAULT
     * @param user user address.
     */
    function setHaveAGift(address user) external {
        require(hasRole(VAULT_ROLE, _msgSender()) && !haveAGift[user], "Wrong params");
        haveAGift[user] = true;
    }

    /**
     * @dev View function to see token status: (NOT)USED.
     * @param _id token id.
     * @return true/fale
     */
    function getUsedId(uint256 _id) external view returns(bool){
        require(_exists(_id), "Not exist");
        return usedIds[_id];
    }

    /**
     * @dev View function to see user status: (NOT) HAVE A GIFT. (has minted nft or not)
     * @param user user address.
     * @return true/fale
     */
    function getHaveAGift(address user) external view returns(bool){
        return haveAGift[user];
    }

    /**
     * @dev View function to see token status: (NOT)USED.
     * @param _id token id.
     * @return address of vault in which token give an access
     */
    function getVaultById(uint256 _id) external view returns(address){
        require(_exists(_id), "Not exist");
        return vaultById[_id];
    }
    ...
    /**
     * @dev View function to see contarct status: (NOT)VAULT.
     * @param _contract vault address.
     * @return true/false
     */
    function isVault(address _contract) public view returns(bool){
        return hasRole(VAULT_ROLE, _contract);
    }
    ...
    AccessControl
{
    ...
    InvitationalNFT public NFT_TOKEN; // Address, where InvitationalErc721 contract located
    uint256 public daysAfter; // the period of time after which the user will be able to donate the token
    NftTumbler public nftTumbler;

    mapping (address => UserInfo) public userInfo;

    struct UserInfo {
        bool isNotFirstDeposit; //false means that deposit is first, true means that deposit isn't first
        //bool haveAGift;
    ...
    struct NftTumbler {
        /* 0 - nft disabled, 
         * 1 - invitate by nft disabled but increase limit with nft enabled, 
         * 2 - invite enabled
         */
        uint8 inviteEnable; 
        bool mintEnable;
     ...
     * @param _daysAfter  The period of time after which the user will be able to donate the token
     * @param _nftTumbler [0]-on/off NFT invite, [1]-on/off NFT-mining
     * @param _nft Address of NFT owned by the User
     * @param _admin address for setting an admin role
     */
    constructor(
        poolInfo[3] memory _poolInfos,
        uint256 _currentPoolIndex,
        uint256 _protocolFee,
        uint256 _maxTotalSupply,
        uint256 _daysAfter,
        NftTumbler memory _nftTumbler,
        address _nft,
        address _admin
    ) ERC20("Vault", "VT") {
        ...
        for (uint8 i=0; i<3; i++) 
            pools[i].pool = IUniswapV3Pool(_poolInfos[i].pool);
        ...
        _setupRole(DEFAULT_ADMIN_ROLE, _admin);
        daysAfter = _daysAfter * 1 days;
        if(_nft != address(0))
            NFT_TOKEN = InvitationalNFT(_nft);
        nftTumbler = _nftTumbler;
     ...
     * @param burnNft User's choice to burn/not burn invitational nft
    ...
    {
        require(nftTumbler.inviteEnable > 0, "Status 0");

        address sender = _msgSender();

        UserInfo memory info = userInfo[sender];
        if(nftTumbler.inviteEnable == 2){
            require(
                (NFT_TOKEN.ownerOf(tokenNftId) == sender 
                    && !NFT_TOKEN.getUsedId(tokenNftId)) 
                    && NFT_TOKEN.getVaultById(tokenNftId) == address(this),
                "Wrong Invitational NFT parameters"
            );
        }
        if(!info.isNotFirstDeposit){
            userInfo[sender].startTimeDeposit = block.timestamp;
            userInfo[sender].isNotFirstDeposit = true;
        }

        require(amount0Desired > 0 || amount1Desired > 0, "amount0Desired or amount1Desired");
        require(amount0Desired <= deposit0Max && amount1Desired <= deposit1Max, "Deposit operation limit");
        ...
        if ((nftTumbler.inviteEnable == 1 
            && (NFT_TOKEN.ownerOf(tokenNftId) != sender 
            || NFT_TOKEN.getUsedId(tokenNftId))) 
            || (nftTumbler.inviteEnable == 2))

            require(totalSupply() <= maxTotalSupply, "maxTotalSupply");

        if (NFT_TOKEN.ownerOf(tokenNftId) == sender 
            && NFT_TOKEN.getVaultById(tokenNftId) == address(this) 
            && !NFT_TOKEN.getUsedId(tokenNftId)) {
                NFT_TOKEN.setUsedId(tokenNftId);
                if (burnNft)
                    NFT_TOKEN.burn(tokenNftId);
        }
        /* NFT_TOKEN.setUsedId(tokenNftId);
        if(burnNft && NFT_TOKEN.ownerOf(tokenNftId) == sender) {
            NFT_TOKEN.burn(tokenNftId);
        } */
    ...
    function withdraw(
        ...
        // If User withdraw all Tokens from Balance, Burn his NFT
        // Works if NFT functionality is Enabled
        if (balanceOf(sender) == 0 /*&& block.timestamp < userInfo[sender].startTimeDeposit + daysAfter*/) {
            /* require(NFT_TOKEN.ownerOf(_tokenNftId) == sender, "Not your NFT token");
            NFT_TOKEN.burn(_tokenNftId); */
            //delete(userInfo[sender]);
            userInfo[sender].isNotFirstDeposit = false;
            userInfo[sender].startTimeDeposit = 0;
        }
        ...
        if (amount0 > 0) require(token0.transfer(to, amount0), "transfer failed");
        if (amount1 > 0) require(token1.transfer(to, amount1), "transfer failed");
     ...
     * @param _poolId pool to use
     */
    function rerange(int24 _tickRange, uint8 _poolId) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "onyGovernance");
        require(_poolId >= 0 && _poolId < 3, "w Pool");
        ...
        int24 _baseLower = (_tick - int24(deltaLower * _tickRange / (deltaLower + deltaUpper))) / _tickSpacing;
        int24 _baseUpper = (_tick + int24(deltaUpper * _tickRange / (deltaLower + deltaUpper))) / _tickSpacing;
        
        int128 _lowerCorrection = (_baseLower > 0) ? 
            _baseLower * 100 / _tickSpacing % 100 : -_baseLower * 100 / _tickSpacing % 100;
        int128 _upperCorrection = (_baseUpper > 0) ? 
            _baseUpper * 100 / _tickSpacing % 100 : -_baseUpper * 100 / _tickSpacing % 100;
        
        if (_lowerCorrection >= 50) _baseLower = (_baseLower > 0) ? (_baseLower + 1) : (_baseLower - 1);
        if (_upperCorrection >= 50) _baseUpper = (_baseUpper > 0) ? (_baseUpper + 1) : (_baseUpper - 1);
        ...
        {
            (uint128 baseLiquidity, , , , ) = _position(_baseLowerPrev, _baseUpperPrev);
        ...
        (currentPool.baseLower, currentPool.baseUpper) = (_baseLower, _baseUpper);
    ...
    function emergencyRebalance(
        ...
        // Withdraw all current liquidity from Uniswap pool
        {
            (uint128 baseLiquidity, , , , ) = _position(currentPool.baseLower, currentPool.baseUpper);
            _burnAndCollect(currentPool.baseLower, currentPool.baseUpper, baseLiquidity);
        }
    ...
    {
        if (liquidity > 0) {
            (burned0, burned1) = currentPool.pool.burn(tickLower, tickUpper, liquidity);
        }
    ...
    function _mintLiquidity(
        ...
        if (liquidity > 0) {
            currentPool.pool.mint(address(this), tickLower, tickUpper, liquidity, "");
        }
    }
    ...
    function getTotalAmounts() public view override returns (uint256 total0, uint256 total1) {
        (uint256 baseAmount0, uint256 baseAmount1) = getPositionAmounts(currentPool.baseLower, currentPool.baseUpper);
    ...
    function uniswapV3MintCallback(
        ...
        if (amount0 > 0) require(token0.transfer(msg.sender, amount0), "transfer failed");
        if (amount1 > 0) require(token1.transfer(msg.sender, amount1), "transfer failed");
    }
    ...
    function uniswapV3SwapCallback(
        ...
        require(msg.sender == address(currentPool.pool));
        if (amount0Delta > 0)
            require(token0.transfer(msg.sender, uint256(amount0Delta)), "transfer failed");
        if (amount1Delta > 0)
            require(token1.transfer(msg.sender, uint256(amount1Delta)), "transfer failed");
    }
    ...
    function emergencyBurn(
        ...
        currentPool.pool.collect(address(this), tickLower, tickUpper, type(uint128).max, type(uint128).max);
    }
    ...
    /**
     * @notice Set a period that sets how many days the user should keep the Deposit
     * @param _setPeriodInDays amount of min deposit days
     */ 
    function setDaysAfter(uint256 _setPeriodInDays) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "onyGovernance");
        daysAfter = _setPeriodInDays * 1 days;
    }

    // Show a period of days the user should keep the Deposit
    function showDaysAfter() public view returns (uint256) {
        return daysAfter;
    }

    // Function for Sharing NFT after period of holding deposit
    // Works if NFT functionality is Enabled
    function mintNftToken(string calldata _tokenURI, address _to)
        external
        nonReentrant
    {   
        require(nftTumbler.mintEnable, "Disabled");
        address sender = _msgSender();
        require(_to != address(0) && _to != address(this) && _to != sender, "You can't send NFT to invalid address");
        require(
            (block.timestamp >= userInfo[sender].startTimeDeposit + daysAfter) && userInfo[sender].isNotFirstDeposit,
            "The period of time before receiving Invitational NFT has not passed"
        );

        require(!NFT_TOKEN.getHaveAGift(sender), "You already got Invitational NFT to share");
        NFT_TOKEN.setHaveAGift(sender);
        NFT_TOKEN.mint(_tokenURI, _to, address(this));
    }

    // Change parameter which increase maxTotalSupply
    
    // Function for Enabling/Disabling Functionality connected with NFT
    function setStartStopNFT(NftTumbler memory _nftTumbler) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "onyGovernance");
        nftTumbler = _nftTumbler;
    } 

    // Show if Functionality connected with NFT is Enabled(True) or Disabled(False)
    function showNftFuncStatus() public view returns (uint8, bool) {
        return (nftTumbler.inviteEnable, nftTumbler.mintEnable);
    }
}