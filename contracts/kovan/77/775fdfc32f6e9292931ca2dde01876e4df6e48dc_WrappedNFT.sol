...
contract WrappedNFT is ERC20 {
    ...
    function unwrap(uint256 _amount) public {
        require(msg.sender == address(boxswapFoundry) || !isContract(msg.sender), "WrappedNFT: Address must not be a contract.");
        unwrapFor(_amount, msg.sender);
    }

    function unwrapFor(uint256 _amount, address _recipient) public {
        require(msg.sender == address(boxswapFoundry) || !isContract(_recipient), "WrappedNFT: Recipient must not be a contract.");
