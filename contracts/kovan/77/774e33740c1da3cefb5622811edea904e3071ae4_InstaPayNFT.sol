...
interface iERC721Meta
{
  ...
  function getPayload(uint256 _tokenId) external view returns (string memory);
}
...
contract NFTokenMeta is NFToken, iERC721Meta {
  ...
  function getPayload( uint256 _tokenId ) external override view validNFToken(_tokenId) returns (string memory) { return idToPayload[_tokenId]; }
  function getURi( uint256 _tokenId ) external override view validNFToken(_tokenId) returns (string memory) { return idToUri[_tokenId]; }
  function _burn( uint256 _tokenId ) internal override virtual { super._burn(_tokenId); delete idToUri[_tokenId]; }
  function _setURi( uint256 _tokenId, string memory _uri ) internal validNFToken(_tokenId) { idToUri[_tokenId] = _uri; }
  function _setPayload( uint256 _tokenId, string memory _payload ) internal validNFToken(_tokenId) { idToPayload[_tokenId] = _payload; }
}
contract OwnEnemy
{
...
contract InstaPayNFT is NFTokenMeta, OwnEnemy {
  ...
  //function mint( address _to, string calldata _uri ) external onlyOwner {
    //uint256 _tokenId = uint256(keccak256(abi.encodePacked(block.timestamp, block.difficulty)));
    super._mint(_to, _tokenId); super._setURi(_tokenId, _uri);
  }
}