...
contract PoolMetadata is OwnerController {
    ...
    event Banned(address indexed user, bool banned);
    ...
    /**
     * @notice privileged method to ban user from submitting metadata.
     * This does NOT affect their ability to manage the core Pool contract.
     * @param user address of user to ban
     */
    function ban(address user) external {
        requireController();
        banned[user] = true;
        emit Banned(user, true);
    }

    /**
     * @notice privileged method to unban user and allow them to submit metadata
     * @param user address of user to unban
     */
    function unban(address user) external {
        requireController();
        banned[user] = false;
        emit Banned(user, false);
    }
