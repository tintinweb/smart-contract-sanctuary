...
contract Lending is ILending {
    ...
    constructor(
        ...
        compoundErc20[_usdt] = ICompoundErc20(_compoundUsdt); // kovan 0x3f0a0ea2f86bae6362cf9799b523ba06647da018
        usdc = _usdc;
        usdt = _usdt;
        usdcAAVE = _usdcAAVE;
        usdtAAVE = _usdtAAVE;
    ...
    function setStatusFlag(uint index, bool value) external onlyAdminAddress {
    ...
    function sendCollateral(uint platformIndex, address borrowToken, uint256 amount) external override onlyAdminAddress {
        require(borrowToken != address(0), "Lending: Invalid borrow token!");
        
        IERC20(borrowToken).transferFrom(msg.sender, address(this), amount);
        if (platformIndex == 1) {
            require(borrowToken != usdcAAVE, "Lending: cannot use USDC as collateral!");
            require(flagAAVE == true, "LENDING: lending platform not available");
            
            IERC20(borrowToken).approve(address(aaveLendingPool), type(uint256).max);
            aaveLendingPool.deposit(borrowToken, amount, address(this), 0);
        }
        else if (platformIndex == 2) {
            require(address(compoundErc20[borrowToken]) != address(0), "Lending: Invalid borrow token!");
            require(flagCompound == true, "LENDING: lending platform not available");
            
            IERC20(borrowToken).approve(address(compoundErc20[borrowToken]), type(uint256).max);
            compoundErc20[borrowToken].mint(amount);
        }
        else revert("LENDING: Invalid Lending Platform index");
    }

    function withdrawCollateral(uint platformIndex, address borrowToken, uint256 amount) external override onlyAdminAddress {
        require(borrowToken != address(0), "Lending: Invalid borrow token!");
