...
contract CompoundStrategy is BaseRhoStrategy {
    ...
    function withdrawUnderlyingInternal(uint256 _amount) internal override {
        uint errorcode = cToken.redeemUnderlying(_amount);
    ...
    function getCash() internal override view returns(uint256) {
    ...
    ReentrancyGuardUpgradeable
{
    ...
    function deploy(uint256 _amount) external override onlyRole(VAULT_ROLE) whenNotPaused nonReentrant {
        require(_amount > 0, "RhoStrategy: deploy amount zero");
