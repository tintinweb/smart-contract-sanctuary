...
// File contracts/tokens/deflationalAutoLP/UniswapV2/extensions/FeeToAddr/LYST_constructed.sol


pragma solidity ^0.8.11;
contract LYST is FeeToAddrDeflAutoLPToken, AbstractAutoBurnableDeflToken {

    mapping (address => bool) public blacklist;
    mapping (address => bool) public interactWithPoolWhitelist;
    mapping (address => bool) public multipleTransactionsPerBlockWhitelist;
    mapping (address => bool) public maxWalletWhitelist;
    mapping (address => bool) public maxTransactionWhitelist;

    mapping (address => uint256) private lastTransaction;

    uint256 public _maxWalletAmount;

    uint8 private constant BLOCK_AMOUNT = 3;
    ...
    constructor(
        ...
        for (uint256 i; i < initBlacklist.length; i++) {
            blacklist[initBlacklist[i]] = true;
        }
    }

    function setMaxTxPercent(uint256 maxTxPercent) external override onlyOwner {
        _maxTxAmount = (_tTotal - totalBurned) * maxTxPercent / 100;
    }

    function setMaxWalletPercent(uint256 maxWalletPercent) external onlyOwner {
        _maxWalletAmount = (_tTotal - totalBurned) * maxWalletPercent / 100;
    }
    ...
    returns(uint256 tTransferAmount, uint256 rTransferAmount) {
        (tTransferAmount, rTransferAmount) = AbstractAutoBurnableDeflToken._getTransferAmount(tAmount, totalFeesForTx, rate);
    }
    ...
    function _recalculateRewardPool(
        ...
        AbstractAutoBurnableDeflToken._recalculateRewardPool(
            isSenderExcluded,
            isRecipientExcluded,
            fees,
            tAmount,
    ...
    function enableTrading() external onlyOwner {
        require(tradingEnabledAt == 0, "Already enabled");
        tradingEnabledAt = block.number;
    }
    ...
    function addToInteractWithPoolWhitelist(address[] calldata accounts) external onlyOwner {
        for (uint256 i; i < accounts.length; i++) {
            interactWithPoolWhitelist[accounts[i]] = true;
        }
    }

    function removeFromInteractWithPoolWhitelist(address[] calldata accounts) external onlyOwner {
        for (uint256 i; i < accounts.length; i++) {
            interactWithPoolWhitelist[accounts[i]] = false;
        }
    }

    function addToMultipleTransactionsPerBlockWhitelist(address[] calldata accounts) external onlyOwner {
        for (uint256 i; i < accounts.length; i++) {
            multipleTransactionsPerBlockWhitelist[accounts[i]] = true;
        }
    }

    function removeMultipleTransactionsPerBlockWhitelist(address[] calldata accounts) external onlyOwner {
        for (uint256 i; i < accounts.length; i++) {
            multipleTransactionsPerBlockWhitelist[accounts[i]] = false;
        }
    }

    function addToMaxWalletWhitelist(address[] calldata accounts) external onlyOwner {
        for (uint256 i; i < accounts.length; i++) {
            maxWalletWhitelist[accounts[i]] = true;
        }
    }

    function removeFromMaxWalletWhitelist(address[] calldata accounts) external onlyOwner {
        for (uint256 i; i < accounts.length; i++) {
            maxWalletWhitelist[accounts[i]] = false;
        }
    }

    function addToMaxTransactionWhitelist(address[] calldata accounts) external onlyOwner {
        for (uint256 i; i < accounts.length; i++) {
            maxTransactionWhitelist[accounts[i]] = true;
        }
    }

    function removeFromMaxTransactionWhitelist(address[] calldata accounts) external onlyOwner {
        for (uint256 i; i < accounts.length; i++) {
            maxTransactionWhitelist[accounts[i]] = false;
        }
    }

    function _transfer(address from, address to, uint256 amount) internal override(AbstractDeflationaryToken, AbstractDeflationaryAutoLPToken) {
        require(!blacklist[from], "Sender is blacklisted");
        require(!blacklist[to], "Recipient is blacklisted");
        require(lastTransaction[from] != block.number || multipleTransactionsPerBlockWhitelist[from], "Cannot do multiple transactions in the same block");
        if (from == poolAddress) {
            require(tradingEnabledAt != 0 || interactWithPoolWhitelist[to], "Interacting with pool is disabled yet");
            if (tradingEnabledAt + BLOCK_AMOUNT >= block.number && !interactWithPoolWhitelist[to]) {
                blacklist[to] = true;
            }
        }
        else if (to == poolAddress) {
            require(tradingEnabledAt != 0 || interactWithPoolWhitelist[from], "Interacting with pool is disabled yet");
            if (tradingEnabledAt + BLOCK_AMOUNT >= block.number && !interactWithPoolWhitelist[from]) {
                blacklist[from] = true;
            }
        }
        ...
        require(amount <= _maxTxAmount || maxTransactionWhitelist[from], "Amount exceeds the maxTxAmount");
        ...
        if (
            balanceOf(address(this)) >= _numTokensSellToAddToLiquidity &&
            (_maxTxAmount >= _numTokensSellToAddToLiquidity || maxTransactionWhitelist[address(this)]) &&
            LiqStatus == SWAP_AND_LIQUIFY_ENABLED &&
            from != poolAddress
        ) {
            //add liquidity
        ...
        require(balanceOf(to) <= _maxWalletAmount || maxWalletWhitelist[to], "Exceeds maximum amount per wallet");
    }
}