...
contract CHIMP is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    /* ========== CONSTRUCTOR ========== */
    /**
     * @param _burnerAddress Burner smart contract address where minted funds will be forwarded to
     * @param _mintPrice Mint price per token
     */
    constructor(
        address payable _burnerAddress,
        uint256 _mintPrice
    ) ERC721("CHIMP", "CHIMP") Ownable() {
        require(_burnerAddress != address(0), "CHIMP: burner address is the zero address");
        require(_mintPrice > 0, "CHIMP: Mint price is 0");
    ...
    function setMintPrice(uint256 _mintPrice) external onlyOwner {
        require(_mintPrice > 0, "CHIMP: Claim price is 0");
        mintPrice = _mintPrice;
    }
    ...
    function mint(uint256[PIXEL_CHUNKS] memory pixelChunks, uint8[PALETTE_SIZE] memory colors) public payable nonReentrant {
        require(msg.value == mintPrice, 'UBQ value sent is not correct');
