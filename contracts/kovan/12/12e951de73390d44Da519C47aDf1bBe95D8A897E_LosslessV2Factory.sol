...
contract LosslessV2Factory is LosslessV2Storage, ILosslessV2Factory {
	...
	function createPool(
		...
		(address shortToken, address longToken, address pauseToken, address sponsorToken) =  _initializePool(address(newPool), tokenName, tokenSymbol);
	...
	// function createPool(bidToken, principalToken, addressProvider, biddingDuration, gamingDuration) external override onlyAdmin{
	// 	_createPool();
	// }

	// id: 0 shortToken, 1 longToken, 2 pause Token, 3 sponsorToken 
...
contract LosslessV2Storage {
	...
	//         |                   |          |
	mapping(address => mapping(address => address)) internal getPool;
	mapping(address => bool) internal isPoolActive; // check if a pool is active or not
	address[] internal allPools; // all pools info are here
    ...
    function _initializePool(
		...
		shortToken = _createToken(_pool, string(abi.encodePacked("short", _tokenName)), string(abi.encodePacked("short", _tokenSymbol)));
		longToken = _createToken(_pool, string(abi.encodePacked("long", _tokenName)), string(abi.encodePacked("long", _tokenSymbol)));
		pauseToken = _createToken(_pool, string(abi.encodePacked("pause", _tokenName)), string(abi.encodePacked("pause", _tokenSymbol)));
		sponsorToken = _createToken(_pool, string(abi.encodePacked("sponsor", _tokenName)), string(abi.encodePacked("sponsor", _tokenSymbol)));
	...
	function _createToken(
		...
		bytes32 salt = keccak256(abi.encodePacked(_pool, _name, _symbol));
		LosslessV2Token newToken = new LosslessV2Token{ salt: salt }(_name, _symbol, _pool);
