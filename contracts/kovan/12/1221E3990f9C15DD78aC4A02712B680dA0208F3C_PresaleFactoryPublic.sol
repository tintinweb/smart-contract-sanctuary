...
contract PresalePublic is ReentrancyGuard {
        ...
        notCreator
    {
        ...
        uint256[4] memory poolAmounts;
        uint256 prevPoolsAmount;
        ...
        if (nowTime < generalInfo.openTimePresale + tiersTimes[3]) {
            ...
            tokensLeft = poolAmounts[0];
        } else if (nowTime < generalInfo.openTimePresale + tiersTimes[2]) {
            require(
                    _stakedAmount >= stakingTiers[1],
                "TIER 4"
            );
            prevPoolsAmount = poolAmounts[0] - tokensSold;
            if (prevPoolsAmount == 0) {
                require(_stakedAmount < stakingTiers[0], "prev pool sold");
            }
            tokensLeft = poolAmounts[1] + prevPoolsAmount;
        } else if (nowTime < generalInfo.openTimePresale + tiersTimes[1]) {
            require(
                    _stakedAmount >= stakingTiers[2],
                "TIER 3"
            );
            prevPoolsAmount = poolAmounts[0] + poolAmounts[1] - tokensSold;
            if (prevPoolsAmount == 0) {
                require(_stakedAmount < stakingTiers[1], "prev pool sold");
            }
            tokensLeft = poolAmounts[2] + prevPoolsAmount;
            ...
            prevPoolsAmount = poolAmounts[0] + poolAmounts[1] + poolAmounts[2] - tokensSold;
            if (prevPoolsAmount == 0) {
                require(_stakedAmount < stakingTiers[2], "prev pool sold");
            }
            tokensLeft = poolAmounts[3] + prevPoolsAmount;
        } else {
            require(
...

{
  ...
  "libraries": {
    "/wsl-files/work/git/eth/less-contracts-for-tests/contracts/libraries/Calculations.sol": {
      "Calculations": "0xc0Cfaf5587C73440126148A0b72E8A4531a2F007"
    }
  },
  "outputSelection": {
    "*": {
