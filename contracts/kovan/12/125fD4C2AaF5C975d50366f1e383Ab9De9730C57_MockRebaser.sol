...
contract BasicRebaser {
    ...
    function recordPrice() public {
        ...
        if (noPending) {
            ...
            averageXAU = averageXAU.mul(counter).add(pendingXAUPrice).div(
                counter.add(1)
            );
            averageAUSC = averageAUSC.mul(counter).add(pendingAUSCPrice).div(
                counter.add(1)
            );
            ...
            averageXAU = averageXAU
                .mul(WINDOW_SIZE)
                .sub(pricesXAU[index])
                .add(pendingXAUPrice)
                .div(WINDOW_SIZE);
            averageAUSC = averageAUSC
                .mul(WINDOW_SIZE)
                .sub(pricesAUSC[index])
                .add(pendingAUSCPrice)
                .div(WINDOW_SIZE);
    ...
    function calculateRealTimeRebase() public view returns (uint256, uint256) {
        ...
        if (averageAUSC > highThreshold) {
            ...
            return (
                delta,
                secondaryPool == address(0) ? 0 : secondaryPoolBudget
            );
        } else if (averageAUSC < lowThreshold) {
            // AUSC is too cheap, this is a negative rebase decreasing the supply
            uint256 currentSupply = IERC20(ausc).totalSupply();
...
// File: contracts/mocks/MockRebaser.sol
