...
contract Precog is IPrecog {
    ...
    function getLiquidateInfo(address user, address futureContract) external view override returns (uint liquidateAmount, bool isTradeFinish) {
        liquidateAmount = liquidatedAmount[user][futureContract];
        isTradeFinish = isTradeFinished[user][futureContract];
    }
...
interface IPrecog {
    ...
    function getLiquidateInfo(address user, address futureContract) external view returns (uint256, bool);
