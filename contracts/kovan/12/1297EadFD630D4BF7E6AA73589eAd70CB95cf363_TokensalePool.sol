    ...
    function _withdrawLPartner(address payable sender) external onlyPoolRegistry returns (bool, uint256, address) {
        ...
        if (totalAmountReturn > 0) {
            if (token == address(0)) {
                ...
                IERC20 tokenContract = IERC20(token);
                tokenContract.transfer(sender, totalAmountReturn);
                return (true,totalAmountReturn,token);
            }
        }
...
//    event DebugPurchaseTokens(address sendTo, uint256 amountTokens, uint256 balance, uint256 delta, uint256 newPrice, uint256 initialBalance, uint256 amount);

    function purchaseTokensQuantityFor(uint256 amount) public view returns (uint256,uint256) {
        IERC20 tokenContract = IERC20(_tokenForTokensale);
        address poolAddress = address (this);
        uint256 balance = tokenContract.balanceOf(poolAddress);
    ...
    function purchaseTokens() public payable returns (uint256) {
        ...
        IERC20 tokenContract = IERC20(_tokenForTokensale);
        tokenContract.transfer(msg.sender, amountTokens);
        return amountTokens;
    }
