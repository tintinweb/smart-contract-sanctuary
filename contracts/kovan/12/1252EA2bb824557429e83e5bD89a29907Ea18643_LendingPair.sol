...
contract LendingPair is ILendingPair, ReentrancyGuard, TransferHelper, ERC721Receivable {
  ...
  function _positionAmounts(
    ...
    _priceA = _priceA * 1e18 / 10 ** decimals[tokenA];
    _priceB = _priceB * 1e18 / 10 ** decimals[tokenB];

    return uniV3Helper.positionAmounts(_position, _priceA, _priceB);
  }
  ...
  function _convertTokenValues(
    ...
    _fromPrice = _fromPrice * 1e18 / 10 ** decimals[_fromToken];
    _toPrice   = _toPrice   * 1e18 / 10 ** decimals[_toToken];

    return _inputAmount * _fromPrice / _toPrice;
  }
