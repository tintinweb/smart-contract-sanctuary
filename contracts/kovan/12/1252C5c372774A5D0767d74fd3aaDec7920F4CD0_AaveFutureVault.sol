...
abstract contract HybridFutureVault is RateFutureVault {
    ...
    function deposit(address _user, uint256 _amount) external virtual override periodsActive depositsEnabled onlyController {
        ...
        if (currScaledTotals == 0) {
            ...
            scaledTotals[getCurrentPeriodIndex()] = currScaledTotals.add(
                convertUnderlyingtoIBT(getAPWIBTPerAmountDeposited(_amount))
            );
        }
     ...
     */
    function getUnrealisedYieldPerAPWIBT() public view override returns (uint256) {
        ...
        uint256 yieldForAllAPWIBT = convertUnderlyingtoIBT(totalUnderlyingNow.sub(totalUnderlyingAtStart));
        return yieldForAllAPWIBT.mul(IBT_UNIT).div(totalUnderlyingAtStart);
    }
     ...
     */
    function getAPWIBTPerAmountDeposited(uint256 _amount) public view override returns (uint256) {
        uint256 scaledAmount =
            APWineMaths.getScaledInput(_amount, scaledTotals[getCurrentPeriodIndex()], ibt.balanceOf(address(this)));
...
abstract contract RateFutureVault is RewardsFutureVault {
     ...
     * @return the amount of yield (in IBT) generated during the current period
     */
    function getUnrealisedYieldPerAPWIBT() public view virtual override returns (uint256) {
     ...
     * @return the number of apwibt that can be minted for that amount
     */
    function getAPWIBTPerAmountDeposited(uint256 _amount) public view virtual override returns (uint256) {
...
abstract contract FutureVault is Initializable, RegistryStorage, ReentrancyGuardUpgradeable {
    ...
    function initialize(
        ...
        apwibt = IAPWineIBT(
            ITokensFactory(IRegistry(controller.getRegistryAddress()).getTokensFactoryAddress()).deployAPWIBT(
                ibt.symbol(),
                ibt.decimals(),
                PLATFORM_NAME,
                PERIOD_DURATION
    ...
    function _switchPeriod() internal periodsActive {
        ...
        uint256 yield = getUnrealisedYieldPerAPWIBT().mul(totalUnderlyingDeposited) / IBT_UNIT;
        ...
        if (yield > 0) {
            ...
            uint256 collectedYield =
                remainingYield.mul(collectedFYTSByPeriod[currentPeriodIndex]).div(totalUnderlyingDeposited);
    ...
    function updateUserState(address _user) public {
        ...
        if (lastPeriodClaimedOfUser < currentPeriodIndex && lastPeriodClaimedOfUser != 0) {
            apwibt.mint(_user, _prepareAPWIBTClaim(_user));
        }
        if (lastPeriodClaimedOfUser != currentPeriodIndex) lastPeriodClaimed[_user] = currentPeriodIndex;
    }

    function _prepareAPWIBTClaim(address _user) internal virtual returns (uint256 claimableAPWIBT) {
        uint256 currentPeriodIndex = getCurrentPeriodIndex();
        if (lastPeriodClaimed[_user] < currentPeriodIndex) {
            claimableAPWIBT = getClaimableAPWIBT(_user);
            delete premiumToBeRedeemed[_user];
            delete FYTsOfUserPremium[_user];
            lastPeriodClaimed[_user] = currentPeriodIndex;
            claimableFYTByUser[_user] = apwibt.balanceOf(_user).add(totalDelegationsReceived[_user]).sub(
    ...
    function _deposit(address _user, uint256 _amount) internal {
        uint256 underlyingDeposited = getAPWIBTPerAmountDeposited(_amount);
        uint256 apwibtToMint = _prepareAPWIBTClaim(_user).add(underlyingDeposited);
        uint256 currentPeriodIndex = getCurrentPeriodIndex();
        ...
        FYTsOfUserPremium[_user] = FYTsOfUserPremium[_user].add(apwibtToMint);
        premiumsTotal[currentPeriodIndex] = premiumsTotal[currentPeriodIndex].add(redeemable);

        /* Update State and mint apwibt*/
        totalUnderlyingDeposited = totalUnderlyingDeposited.add(underlyingDeposited);
        claimableFYTByUser[_user] = claimableFYTByUser[_user].add(apwibtToMint);

        apwibt.mint(_user, apwibtToMint);
    }
    ...
    function withdraw(address _user, uint256 _amount) external virtual nonReentrant withdrawalsEnabled onlyController {
        require((_amount > 0) && (_amount <= apwibt.balanceOf(_user)), "FutureVault: ERR_AMOUNT");
    ...
    function _withdraw(address _user, uint256 _amount) internal {
        ...
        uint256 fundsToBeUnlocked = _amount.mul(getUnlockableFunds(_user)).div(apwibt.balanceOf(_user));
        uint256 yieldToBeUnlocked = _amount.mul(getUnrealisedYieldPerAPWIBT()) / IBT_UNIT;
    ...
    function _prepareUserEarlyPremiumUnlock(address _user, uint256 _apwibtShares)
        internal
    ...
    function createFYTDelegationTo(
        ...
        require(_amount > 0 && _amount <= apwibt.balanceOf(_delegator).sub(totalDelegated), "FutureVault: ERR_AMOUNT");
    ...
    function exitTerminatedFuture(address _user) external nonReentrant onlyController {
        ...
        uint256 amount = apwibt.balanceOf(_user);
        require(amount > 0, "FutureVault: ERR_APWIBT_BALANCE");
    ...
    function _deployNewFutureYieldToken() internal {
        IFutureYieldToken newToken =
            IFutureYieldToken(ITokensFactory(registry.getTokensFactoryAddress()).deployNextFutureYieldToken());
        fyts.push(newToken);
    }
    ...
    function getClaimableAPWIBT(address _user) public view virtual returns (uint256) {
        ...
        if (lastPeriodClaimed[_user] < currentPeriodIndex) {
            uint256 recordedBalance = apwibt.recordedBalanceOf(_user);
            uint256 mintableAPWIBT = (recordedBalance).add(premiumToBeRedeemed[_user]); // add premium
            mintableAPWIBT = mintableAPWIBT.add(totalDelegationsReceived[_user]).sub(getTotalDelegated(_user)); // add delegated FYTs
            uint256 userStackingGrowthFactor = yieldOfPeriod[lastPeriodClaimed[_user]];
            if (userStackingGrowthFactor > 0) {
                mintableAPWIBT = mintableAPWIBT.add(claimableFYTByUser[_user].mul(userStackingGrowthFactor) / IBT_UNIT); // add reinvested FYTs
            }
            for (uint256 i = lastPeriodClaimed[_user] + 1; i < currentPeriodIndex; i++) {
                mintableAPWIBT = mintableAPWIBT.add(yieldOfPeriod[i].mul(mintableAPWIBT) / IBT_UNIT);
            }
            return mintableAPWIBT.add(getTotalDelegated(_user)).sub(recordedBalance).sub(totalDelegationsReceived[_user]);
        } else {
            return 0;
        }
    }
    ...
    function getClaimableFYTForPeriod(address _user, uint256 _periodIndex) external view virtual returns (uint256) {
        ...
        if (_periodIndex != currentPeriodIndex || _user == address(this)) {
            ...
            return apwibt.balanceOf(_user).add(totalDelegationsReceived[_user]).sub(getTotalDelegated(_user));
        }
    }
     ...
     * @return the amount of yield (in IBT) generated during the current period
     */
    function getUnrealisedYieldPerAPWIBT() public view virtual returns (uint256);
     ...
     * @return the number of apwibt that can be minted for that amount
     */
    function getAPWIBTPerAmountDeposited(uint256 _amount) public view virtual returns (uint256);
    ...
    function getPremiumPerUnderlyingDeposited(uint256 _amount) public view virtual returns (uint256) {
        ...
        uint256 yieldPerFYT = getUnrealisedYieldPerAPWIBT();
...
interface ITokensFactory {
    function deployNextFutureYieldToken() external returns (address newToken);
