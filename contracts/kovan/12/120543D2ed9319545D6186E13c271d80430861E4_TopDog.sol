...
contract TopDog is Ownable {
    ...
    uint256 public shibPercent = 1;
    // xLeash reward percent
    uint256 public leashPercent = 1;
    // tBone reward percent
    uint256 public bonePercent = 1;
    // xTreat reward percent
    uint256 public treatPercent = 2;
    ...
    function pendingBone(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accBonePerShare = accBonePerShare.add(boneReward.sub(boneReward.mul(rewardPercent).div(100)).mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 devReward = boneTotalReward.mul(devPercent).div(100);
        uint256 shibReward = boneTotalReward.mul(shibPercent).div(100);
        uint256 leashReward = boneTotalReward.mul(leashPercent).div(100);
        uint256 boneReward= boneTotalReward.mul(bonePercent).div(100);
        uint256 treatReward = boneTotalReward.mul(treatPercent).div(100);
