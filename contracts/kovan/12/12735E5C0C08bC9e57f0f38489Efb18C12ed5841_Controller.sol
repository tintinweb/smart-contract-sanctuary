...
contract Governable {
    ...
    constructor(address _store) public {
        require(_store != address(0), "C0");
        store = Storage(_store);
    }

    modifier onlyGovernance() {
        require(store.isGovernance(msg.sender), "C1");
        _;
    }

    function setStorage(address _store) public onlyGovernance {
        require(_store != address(0), "C2");
        store = Storage(_store);
    }
...
contract Controller is IController, Governable {
    ...
    modifier onlyHardWorkerOrGovernance() {
        require(hardWorkers[msg.sender] || (msg.sender == governance()), "C4");
        _;
    }
    ...
    function onlyOracleOrGovernance() private view {
        require((oracleContract != address(0) && msg.sender == oracleContract) || (store.isGovernance(msg.sender)), "C7");
    }

    function salvage(address _token, uint256 _amount, address to) external onlyGovernance {
    ...
    function setNewWeight(bytes calldata data) external {
        ...
        require(_vault != address(0), "C8");
        require(vaults[_vault], "C9");
    ...
    function requestFutureWeights(address _vault, address _requestConsumer) external onlyHardWorkerOrGovernance {
        require(_vault != address(0), "C10");
        require(vaults[_vault], "C11");
        require(!isRequestFutureVault[_vault], "C12");
    ...
    function cancelRequestFutureStrategy(address _vault) external onlyHardWorkerOrGovernance {
        require(_vault != address(0), "C13");
        require(vaults[_vault], "C14");
        isRequestFutureVault[_vault] = false;
    }
    ...
    function addVaultAndStrategy(address _vault, address[] calldata _strategies, address[] calldata _bridgeStrategies, uint256[] calldata _weights) external onlyGovernance {
        require(_vault != address(0), "C15");
        require(!vaults[_vault], "C16");
...
pragma solidity 0.5.16;
interface IFeeRewardForwarderTest {
