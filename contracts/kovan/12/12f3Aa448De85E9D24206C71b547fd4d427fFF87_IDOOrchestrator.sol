...
contract IDO is IDOInterface, UniRef {
        ...
        onlyGenesisGroup
    {
        ...
        uint256 rusdAmount = ringAmount.div(10);
        ...
        (tokenId, , ,) = nft.mint(
            INonfungiblePositionManager.MintParams({
                ...
                tickLower: -887220,
                tickUpper: 887220,
                amount0Desired: rusdAmount,
                amount1Desired: ringAmount,
                amount0Min: rusdAmount,
                amount1Min: ringAmount,
    ...
    function collect(address to) external override onlyGovernor {
        nft.collect(
...
interface IOracle {
    ...
    function isPrice0() external view returns (bool);
...
contract IDOOrchestrator is IIDOOrchestrator, Ownable {
    ...
    {
        ido = address(
            new IDO(core, pool, nft, router)
        );
        timelockedDelegator = address(
            new TimelockedDelegator(ring, admin, releaseWindowDuration)
        );
    }
...
abstract contract UniRef is IUniRef, OracleRef {
    ...
    uint256 private constant RING_DECIMALS_MULTIPLIER = 1e18; // to normalize RUSD wei units
        ...
        returns (Decimal.D256 memory)
    {
        ...
        uint256 priceX192 = (sqrtPriceX96**2).mul(oracle.decimalsMultiplier()).div(RING_DECIMALS_MULTIPLIER);
        if (oracle.isPrice0()) {
            return Decimal.ratio(priceX192, 2**192);
        } else {
            return Decimal.ratio(2**192, priceX192);
        }
    }
