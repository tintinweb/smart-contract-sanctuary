...
contract PPSwap is ERC20Interface, SafeMath{
    ...
    event MakeOffer(uint indexed offerID, address indexed accountA, address indexed tokenA, uint amtA, uint price, uint maxbuy);
 ...
 function getSwapfee(address account) 
    ...
    function makeOffer(address tokenA,
         ...
         emit MakeOffer(lastOfferID, msg.sender, tokenA, amtA, price, maxbuy);
    ...
    function acceptOffer(uint offerID, uint buyAmt)
                        external
                        payable 
                        returns(bool)
                        {
        ...
        emit AcceptOffer(offerID, accountA, accountB, buyAmt);
    ...
    function acceptOfferImp(address accountA, 
        ...
        B.transferFrom(accountB, accountA, wethAmt);
        return true;
    }
