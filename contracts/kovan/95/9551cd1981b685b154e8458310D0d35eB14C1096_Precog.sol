...
contract Precog {
    ...
    function test(uint expiryDate, address[] memory pairs, TradeInfo memory trade)
        public view returns (uint reserveAExchange, uint reserveBExchange, uint reserveAFutureExchange, uint reserveBFutureExchange, uint k, uint sqrtK)
    {
        ...
        address futureContract = IFutureTokenFactory(futureFactory).getFutureContract(usdc, pairs[1], expiryDate);
        if (IFutureContract(futureContract).token0() == usdc) {
        ...
        unchecked {
            k = reserveAExchange * reserveBExchange * reserveAFutureExchange * reserveBFutureExchange;
            if (k / (reserveAExchange * reserveBExchange) != (reserveAFutureExchange * reserveBFutureExchange))
                sqrtK = sqrt(k);
        }
