    ...
    Pausable
{
    ...
    uint256 public rewardsDuration = 20 minutes;
    ...
    function lastTimeRewardApplicable() public view override returns (uint256) {
        return
            Math.min(
                _numWeeksPassed(block.timestamp).mul(5 minutes).add(startEmission),
                periodFinish
            );
    }
        ...
        updateReward(msg.sender)
    {
        ...
        if(block.timestamp < periodFinish.add(1 minutes)){
    ...
    function getReward() public override nonReentrant updateReward(msg.sender) {
        uint256 reward = block.timestamp >= periodFinish.add(1 minutes) ? rewards[msg.sender] : 0;
        if (reward > 0) {
            rewards[msg.sender] = 0;
    ...
    function _numWeeksPassed(uint256 time) internal view returns (uint256) {
        ...
        return time.sub(startEmission).div(5 minutes).add(1);
    }
