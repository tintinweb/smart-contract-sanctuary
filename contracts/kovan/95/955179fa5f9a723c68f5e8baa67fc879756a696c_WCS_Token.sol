...
// File: contracts/ICO_Nicolas.sol
...
contract WCS_ICO {
    ...
    function buyTokens() external payable whenSaleIsActive {
        uint tokensAmount = msg.value / 1 ether * RATE_ETH;
        ethRaised += msg.value;
    ...
    function withdraw() external whenSaleIsFinalized {
        ...
        uint contribution = contributorsToTokenAmount[msg.sender];
        contributorsToTokenAmount[msg.sender] = 0;
        
        if (isSuccessful) {
            token.transfer(msg.sender, contribution); // withdraw tokens 
            emit withdrawTokens(msg.sender, contribution);
        } else {
            payable(msg.sender).transfer(contribution / RATE_ETH);  // withdraw eth 
        }
    }
