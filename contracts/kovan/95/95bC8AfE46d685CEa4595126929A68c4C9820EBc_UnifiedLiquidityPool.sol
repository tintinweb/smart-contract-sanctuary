 ...
 */
contract UnifiedLiquidityPool is ERC20, Ownable {
    ...
    /// @notice Event emitted when prize sent to the winner
    ...
    modifier onlyApprovedGame() {
        require(approvedGames[msg.sender]);
        _;
    }

    /**
     * @dev Constructor function
     * @param gemBiteAddr Address of GemBites Token
     */
    constructor(address gemBiteAddr) ERC20("Stake GBTS", "sGBTS") {
        require(
            gemBiteAddr.isContract() == true,
            "ULP: GemBites Address is not contract address"
        );

        GBTS = IERC20(gemBiteAddr);
     ...
     * @param amount Amount of GBTS token
     */
    function stake(uint256 amount) external canStake {
        require(
            GBTS.balanceOf(msg.sender) > amount,
            "ULP: Caller has not enough balance"
        );
        ...
        uint256 toMint =
            (amount * totalSupply()) /
                (GBTS.balanceOf(address(this)) + feeAmount);

        require(
            GBTS.transferFrom(msg.sender, address(this), amount),
            "ULP: Transfer failed"
        );

        _mint(msg.sender, toMint);

        emit staked(msg.sender, amount, toMint);
    }

    /**
     * @dev External function for exit staking. Users can withdraw their funds.
     * @param amount Amount of sGBTS token
     */
    function exitStake(uint256 amount) external canStake {
        require(
        ...
        uint256 stakeValue =
            (amount * GBTS.balanceOf(address(this))) / totalSupply();
        ...
        emit stakeExit(msg.sender, toSend, amount);
    }

    /**
     * @dev External function for sending prize to winner. This is called by only approved games.
     * @param prizeAmount Amount of GBTS token
     */
    function sendPrize(address winner, uint256 prizeAmount)
        external
        onlyApprovedGame
    {
        require(
            GBTS.balanceOf(address(this)) >= prizeAmount,
            "ULP: There is no enough GBTS balance"
        );
        require(GBTS.transfer(winner, prizeAmount), "ULP: Transfer failed");

        emit prizeSent(msg.sender, winner, prizeAmount);
    }

    /**
     * @dev External function for approving games. This is called by only owner.
     * @param gameAddr Address of game
     */
    function approveGame(address gameAddr, bool approved) external onlyOwner {
        require(
            gameAddr.isContract() == true,
            "ULP: Address is not contract address"
        );
        approvedGames[gameAddr] = approved;

        emit gameApproved(gameAddr, approved);
    }

    /**
     * @dev External function for burning sGBTS token. Only called by owner.
     * @param amount Amount of sGBTS
     */
    function burnULPsGbts(uint256 amount) external onlyOwner {
        _burn(address(this), amount);

        emit sGBTSburnt(amount);
    }
}
