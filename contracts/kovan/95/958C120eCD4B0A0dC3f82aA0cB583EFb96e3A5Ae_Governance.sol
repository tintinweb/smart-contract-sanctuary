...
contract Governance is Initializable, AccessControlUpgradeable {
    ...
    struct Proposal {
        ...
        // Total weight of upvotes in this proposal
        uint256 weightUpvotes;

        // Total weight of downvotes in this proposal
        uint256 weightDownvotes;
    ...
    function setTrainer(ITrainer _trainer) external onlyGovernanceAdmin {
        TRAINER = _trainer;
    }

    function propose(uint256 _proposalId, uint8 _productId) public noEmergency {
    ...
    function vote(uint256 _proposalId, bool _isUpvote) public noEmergency {
        require(BUNI_TOKEN.balanceOf(msg.sender) >= amountToVote, "Insufficient balance");
    ...
    function voteForBunicornGame(uint256 _proposalId, bool _isUpvote, uint256[] memory _trainerIds) public noEmergency {
        require(TRAINER.balanceOf(msg.sender) == _trainerIds.length, "Incorrect quantity of trainers");
        require(TRAINER.balanceOf(msg.sender) > 0, "Insufficient balance");

        Proposal storage proposal = proposals[_proposalId];
        Receipt storage receipt = receipts[_proposalId][msg.sender];

        require(proposal.productId == 2, "Not bunicorn game");
        require(!(proposal.canceled || proposal.executed), "Voting is closed");
        require(receipt.hasVoted == false, "Voter already voted");

        uint256 totalWeight;
        for (uint256 i = 0; i < _trainerIds.length; i++) {
            require(TRAINER.ownerOf(_trainerIds[i]) == msg.sender, "Not the trainer owner");

            totalWeight += TRAINER.getVotingWeight(_trainerIds[i]);
            TRAINER.lockTranferAfterVote(_trainerIds[i]);
        }

        if (_isUpvote) {
            proposal.numUpvotes++;
            proposal.weightUpvotes += totalWeight;
        } else {
            proposal.numDownvotes++;
            proposal.weightDownvotes += totalWeight;
        }
