...
contract NFTPlayToEarnGame is EIP712MetaTransaction {
    ...
    address public NFT_ADDRESS; // address of the supported NFT token
    ...
    address public WETH = address(0xc778417E063141139Fce010982780140Aa0cD5Ab); // WETH address on Polygon network
    address LIQUIDITY_POOL_MANAGER_ADDRESS =
        address(0xF78765bd14B4E8527d9E4E5c5a5c11A44ad12F47); // LiquidityPoolManager address on Polygon network

    constructor(string memory name, string memory version, address _nftAddress) EIP712MetaTransaction(name, version) {
    ...
    function createGame(
        ...
        address nftOwner = IERC721(NFT_ADDRESS).ownerOf(_nftTokenId);
     ...
     * @param _nftTokenId the tokenId of the NFT that owner holds
     */
    function joinGame(uint256 _gameId, uint256 _nftTokenId) public onlyAdmin {
        ...
        require(
            playerFunds[msgSender()] >= games[_gameId].buyIn,
            "Insuffient funds."
        );
        Player memory player = Player({
            player: msgSender(),
        ...
        players[_gameId][games[_gameId].numPlayersJoined] = player;
        games[_gameId].numPlayersJoined = games[_gameId].numPlayersJoined.add(1);
        nftPlayer[_gameId][_nftTokenId] = msgSender();
     ...
     * @param _gameId id of the game
     */
    function exitGame(uint256 _gameId) public onlyAdmin {
        ...
        for (uint256 i = 0; i < games[_gameId].numPlayersJoined - 1; i++) {
            if (players[_gameId][i].player == msgSender()) {
                index = i;
                break;
            }
        }
        ...
        emit PlayerExitGame(_gameId, msgSender());
    }
     ...
     */
    function endGame(uint256 _gameId, address _winner) public onlyAdmin {
