...
/**
  ...
  * @author Lever
  */
contract ControllerV1 is ControllerInterface, ControllerStorage, Adminable {
    ...
    function mintAllowed(address lpool, address minter, uint mintAmount) external override {
        ...
        mintAmount;
        require(getPoolAllowed(lpool), "mint paused");
    ...
    function borrowAllowed(address lpool, address borrower, address payee, uint borrowAmount) external override {
        require(getPoolAllowed(lpool), "borrow paused");
        require(LPoolInterface(lpool).availableForBorrow() >= borrowAmount, "borrow out of range");
        require(address(lever) == payee || address(lever) == address(0), 'payee not lever');
    ...
    function liquidateAllowed(address lpool, address liqMarker, address liquidator, uint liquidateAmount) external override {
        ...
        lpool;
        liquidateAmount;
        require(address(lever) == msg.sender || address(lever) == address(0), 'liquidate sender not lever');
        //distribution
        if (leverTokenDistribution.liquidatorPer > 0
            && leverTokenDistribution.liquidatorBalance >= leverTokenDistribution.liquidatorPer) {
            if (liqMarker == liquidator) {
                if (transferOut(liqMarker, leverTokenDistribution.liquidatorPer)) {
                    leverTokenDistribution.liquidatorBalance = leverTokenDistribution.liquidatorBalance - leverTokenDistribution.liquidatorPer;
                }
            } else {
                uint128 tranferAmountAvg = leverTokenDistribution.liquidatorPer / 2;
                ...
                leverTokenDistribution.liquidatorBalance = leverTokenDistribution.liquidatorBalance - tranferAmountSucceed;
            }
        }
    }

    function getPoolAllowed(address lpool) public view returns (bool) {
        return !lpoolUnAlloweds[lpool];
    }
    /*** Admin Functions ***/
    ...
    function setLever(LeverInterface _lever) external override onlyAdmin {
        lever = _lever;
    }

    function setInterestRateModel(InterestRateModel _interestRateModel) external override onlyAdmin {
        interestRateModel = _interestRateModel;
    }
    ...
    function createLPoolPair(address token0, address token1, uint32 marginRatio) external override {
        ...
        require(lpoolPairs[token0][token1].lpool0 == address(0), 'pool pair exists');
        require(lpoolPairs[token1][token0].lpool0 == address(0), 'pool pair exists');

        LPoolDelegator pool0 = new LPoolDelegator(token0, this, interestRateModel, LPOOL_TOKEN_EXCHANGE_RATE_MANTISSA,
            string(abi.encodePacked(LPOOL_TOKEN_PREFIX, ERC20(token0).name())), string(abi.encodePacked(LPOOL_TOKEN_PREFIX,
            ERC20(token0).symbol())), LPOOL_TOKEN_DECIMALS, admin, lpoolImplementation);
        LPoolDelegator pool1 = new LPoolDelegator(token1, this, interestRateModel, LPOOL_TOKEN_EXCHANGE_RATE_MANTISSA,
            string(abi.encodePacked(LPOOL_TOKEN_PREFIX, ERC20(token1).name())), string(abi.encodePacked(LPOOL_TOKEN_PREFIX,
            ERC20(token1).symbol())), LPOOL_TOKEN_DECIMALS, admin, lpoolImplementation);
        ...
        uint16 marketId = lever.addMarket(LPoolInterface(pool0), LPoolInterface(pool1), marginRatio);
        emit LPoolPairCreated(token0, address(pool0), token1, address(pool1), marketId, marginRatio);
        //distribution
        if (leverTokenDistribution.poolCreatorPer > 0 && leverTokenDistribution.poolCreatorBalance >= leverTokenDistribution.poolCreatorPer) {
            bool succeed = transferOut(msg.sender, leverTokenDistribution.poolCreatorPer);
            if (succeed) {
                leverTokenDistribution.poolCreatorBalance = leverTokenDistribution.poolCreatorBalance - leverTokenDistribution.poolCreatorPer;
            }
        }
    }

    function setLeverTokenDistribution(uint128 moreCreatorBalance, uint128 poolCreatorPer, uint128 moreLiquidatorBalance, uint128 liquidatorPer, uint128 moreSupplyBorrowBalance) external override onlyAdmin {
        uint128 newPoolCreatorBalance = leverTokenDistribution.poolCreatorBalance + moreCreatorBalance;
        require(newPoolCreatorBalance >= moreCreatorBalance, 'creator balance overflow');

        uint128 newLiquidatorBalance = leverTokenDistribution.liquidatorBalance + moreLiquidatorBalance;
        require(newLiquidatorBalance >= moreLiquidatorBalance, 'liq balance overflow');

        uint128 newSupplyBorrowBalance = leverTokenDistribution.supplyBorrowBalance + moreSupplyBorrowBalance;
        require(newSupplyBorrowBalance >= moreSupplyBorrowBalance, 'supply balance overflow');

        uint128 totalCreatorAndLiquidator = newPoolCreatorBalance + newLiquidatorBalance;
        require(totalCreatorAndLiquidator >= newPoolCreatorBalance, 'creator liquidator overflow');

        uint128 totalAll = totalCreatorAndLiquidator + newSupplyBorrowBalance;
        require(totalAll >= totalCreatorAndLiquidator, 'total more overflow');
        require(leverToken.balanceOf(address(this)) >= totalAll, 'not enough balance');

        leverTokenDistribution.poolCreatorBalance = newPoolCreatorBalance;
        leverTokenDistribution.poolCreatorPer = poolCreatorPer;
        leverTokenDistribution.liquidatorBalance = newLiquidatorBalance;
        leverTokenDistribution.liquidatorPer = liquidatorPer;
        leverTokenDistribution.supplyBorrowBalance = newSupplyBorrowBalance;

    }

    function distributionToken(address pool, uint supplyAmount, uint borrowAmount, uint64 startTime, uint64 duration) external override onlyAdmin {
        ...
        if (supplyAmount > 0) {
            lpoolDistributions[LPoolInterface(pool)][false] = getDistribution(supplyAmount, startTime, duration);
        }
        if (borrowAmount > 0) {
            lpoolDistributions[LPoolInterface(pool)][true] = getDistribution(borrowAmount, startTime, duration);
        }
        uint128 subAmount = uint128(supplyAmount.add(borrowAmount));
        uint128 newTotalAmount = leverTokenDistribution.supplyBorrowBalance - subAmount;
        require(newTotalAmount <= leverTokenDistribution.supplyBorrowBalance, "totalAmount overflow");
        leverTokenDistribution.supplyBorrowBalance = newTotalAmount;
        emit Distribution2Pool(pool, supplyAmount, borrowAmount);
    }

    function distributionTokenMore(address pool, uint supplyAmount, uint borrowAmount) external override onlyAdmin {
    ...
    function getDistribution(uint totalAmount, uint64 startTime, uint64 duration) internal pure returns (ControllerStorage.LPoolDistribution memory distribution){
    ...
    function getSupplyRewards(LPoolInterface[] calldata lpools, address account) external override {
        uint rewards = earnedInternal(lpools[0], account, false);
        for (uint i = 1; i < lpools.length; i++) {
        ...
        require(leverToken.balanceOf(address(this)) >= rewards, 'balance<rewards');
        leverToken.transfer(account, rewards);
    }


    function transferOut(address to, uint amount) internal returns (bool){
        if (leverToken.balanceOf(address(this)) < amount) {
            return false;
        }
...
contract LPoolDelegator is LPoolInterface, Adminable, DelegatorInterface {
     ...
     * @param contoller_ The address of the Comptroller
