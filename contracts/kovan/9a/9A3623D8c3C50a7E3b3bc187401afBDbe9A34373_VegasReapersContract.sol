...
contract Fields {
    ...
    uint8 internal constant MAX_TOKENS_BEFORE_AUTO_WITHDRAW = 50;
    ...
    function _internalMint(
        ...
        for (uint8 i = 0; i < count_; i++) {
            ...
            if (addRoles) {
                startCount++;
                if (startCount == 1) {
                    tokenRoles[tokenId] = ROLE_LION;
                } else if (startCount > 1) {
                    tokenRoles[tokenId] = ROLE_INFERNAL;
                }
            }
...
abstract contract MintWhitelist is MintReserve, MerkleProof {
    ...
    function mintWhitelist(
        ...
        require(
            numberOfMintedWhitelistTokens + count <= MAX_WHITELIST_TOKENS,
            "Cant mint more"
        );
        require(msg.value == MINT_PRICE_WHITELIST.mul(count), WRONG_BALANCE);

        uint8 currentMinted = mintedWhitelist[msg.sender];
        require(currentMinted + count <= MAX_WHITELIST_PER_PURCHASE, CANT_MINT);
        numberOfMintedWhitelistTokens += count;
        mintedWhitelist[msg.sender] = currentMinted + count;
        for (uint8 i; i < count; i++) {
            _mintToken(to, randomId());
        }
        distributePayout(count);
    }
    ...
    function increaseAddressCount(uint256 count_) internal {
        ...
        if (currentMintedTokens == 1) {
            addressRoles[msg.sender] = ROLE_LION;
        } else {
            addressRoles[msg.sender] = ROLE_INFERNAL;
        }
    }
...
contract VegasReapersContract is MintWhitelist {
    ...
    constructor(
        ...
        // reserve first 10 tokens for the team
