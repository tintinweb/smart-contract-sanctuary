...
contract GsnTxExecutor is IGsnTxExecutor, Ownable {
    ...
    function estimateGas(address from, address to, uint256 value, bytes calldata data) external pure returns (uint256) {
        (from, to, value, data);
        return 123;
    }
...
interface IGsnTxExecutor {
    ...
    function execute(TxRequest calldata req, TxSettings calldata settings, bytes calldata signature) external;
    function estimateGas(address from, address to, uint256 value, bytes calldata data) external view returns (uint256);
}
