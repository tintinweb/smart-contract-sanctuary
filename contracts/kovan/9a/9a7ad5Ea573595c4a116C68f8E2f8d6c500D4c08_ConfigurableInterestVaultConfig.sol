...
contract ConfigurableInterestVaultConfig is IVaultConfig, Ownable {
  ...
  event SetStrategies(address indexed caller, address strategy, bool ok);
  ...
  constructor(
    ...
    setParams(_wrappedNative, _wNativeRelayer);
  }
  ...
  /// @dev Set the configuration for the given strategies. Must only be called by the owner.
  function setStrategies(address[] calldata addrs, bool ok) external onlyOwner {
    for (uint256 idx = 0; idx < addrs.length; idx++) {
      strategies[addrs[idx]] = ok;
      emit SetStrategies(_msgSender(), addrs[idx], ok);
    }
  }
