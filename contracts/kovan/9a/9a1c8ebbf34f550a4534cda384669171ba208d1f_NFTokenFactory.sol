...
contract NFTokenFactory {
    ...
    ) public returns (NFToken) {
        return new NFToken(owner_, name, symbol, tokenURI);
    }

    function setTokenURI(NFToken token, string calldata tokenURI) external {
        require(msg.sender == root, "auth-unauthorized");
