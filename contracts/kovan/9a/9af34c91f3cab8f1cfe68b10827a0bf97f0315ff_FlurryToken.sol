...
abstract contract VestedToken is AccessControlEnumerableUpgradeable, ERC20Upgradeable {
    ...
    function __intitialize(
        ...
        restrictionAmount = _restrictionAmount;
        restrictionGas = _restrictionGas;
    ...
    function configurePool(
        ...
        require(uniswapV2Factory != address(0), "factory address is 0");
        require(lp != address(0), "lauch pool address is 0");
        uniRouter = IUniswapV2Router02(uniswapV2Router02);
        uniFactory = IUniswapV2Factory(uniswapV2Factory);
        launchPool = lp;
        ...
        if (launchPool != address(0)) {
            if (tradingTime == 0) {
                ...
                if (sender == launchPool) {
                    require(
                        (isWhitelisted[recipient] || isWhitelisted[msg.sender]) && !tx.origin.isContract(),
                        "VTKN: only uniswap router allowed"
                    );
                    uint256 ethBal = IERC20Upgradeable(address(uniRouter.WETH())).balanceOf(launchPool);
                    uint256 tokenBal = balanceOf(launchPool);
                    uint256 curPriceMultiple = (((ethBal * 10**18) / tokenBal) * 10**3) / launchPrice;
                    uint256 lockFactor = curPriceMultiple / (10**3 * unlockMultiple);
                    if (lockFactor <= 1) {
                        lockTime[recipient] = block.timestamp + maxLock - maxLock * lockFactor;
                        lockedAmount[recipient] = amount - amount * lockFactor;
                    }
                }
            } else {
