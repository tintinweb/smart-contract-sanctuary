...
//ugly way to decode revert reason: must have a function with calldata to use "slice" operator.
// also, instead of haveing yet another method, we use the Error itself, which we need for the signature..
contract RevertDecoder {
    function Error(string calldata reason) external returns (string memory) {
        bytes calldata b = bytes(reason);
        if (bytes4(b[0 :]) == RevertDecoder.Error.selector) {
            console.log( "decode value of error");
            (string memory result) = abi.decode(bytes(b[4 :]), (string));
            return result;
        }
        //unable to decode.
        return "";
...
contract EntryPoint is StakeManager {
    ...
    RevertDecoder revertDecoder = new RevertDecoder();
    ...
    function _validateWalletPrepayment(uint opIndex, UserOperation calldata op, uint requiredPrefund, PaymentMode paymentMode) internal returns (uint gasUsedByPayForSelfOp, uint prefund) {
        ...
        (bool success, bytes memory result) = op.getSender().call{gas : op.verificationGas}(abi.encodeWithSelector(IWallet.verifyUserOp.selector, op, requiredEthPrefund));
        if (!success) {
            string memory reason = revertDecoder.Error(string(result));
            if (bytes(reason).length > 0) {
                revert FailedOp(opIndex, address(0), string(reason));
            } else {
