...
contract OriginatorStaking is Initializable, StakingRewards, BaseTokenUpgradeable, IStaking, IProjectFundedRewards, IOriginatorManager {
    ...
    function initialize(
        ...
        state = OriginatorStakingState.UNINITIALIZED;
    }
    /**
    ...
    function setUpTerms(
        ...
        uint256 _defaultDelay
            ...
            require(_stakingGoal != 0, 'INVALID_ZERO_AMOUNT');
    ...
    function redeem(address to, uint256 amount) external override {
        require(amount != 0, 'INVALID_ZERO_AMOUNT');
        require(block.timestamp > DISTRIBUTION_END, 'DISTRIBUTION_DURATION_NOT_FINISHED');
     ...
     * @param lendingContractAddress lending contract address is relationated with this rewards
     */
    function startProjectFundedRewards(uint128 extraEmissionsPerSecond, address lendingContractAddress) external override {
      ...
      emit StartRewardsProjectFunded(currentEmission, extraEmissionsPerSecond, lendingContractAddress);
     ...
     * @param lendingContractAddress lending contract address is relationated with this rewards.
     */
    function endProjectFundedRewards(uint128 extraEmissionsPerSecond, address lendingContractAddress) external override {
      // require(state == OriginatorStakingState.STAKING, 'ONLY_ON_STAKING_STATE');
      AssetData storage currentDistribution = assets[address(this)];
      uint128 currentEmission = currentDistribution.emissionPerSecond;
      ...
      configureAssets(newAssetConfig);
      emit EndRewardsProjectFunded(currentEmission, extraEmissionsPerSecond, lendingContractAddress);
    }
     ...
     */
    function hasReachedGoal() external override view returns (bool) {
        require(stakingGoal != 0, 'INVALID_ZERO_AMOUNT');
    ...
    /**
     * @notice Set new distribution duration
     */
    function changeDistributionDuration(uint256 _distributionDuration) external override {
        require(hasRole(EMISSION_MANAGER_ROLE, msg.sender), 'ONLY_EMISSION_MANAGER');
    ...
    function withdrawProponentStake(uint256 amount, bytes32 role) external payable override {}

    function declareDeactivated() external override {} // Not needed anymore?

    function declareDefault(uint256 defaultedAmount) external override {}

    function liquidateManager(uint256 amount, bytes32 role) external override {}
...
interface IOriginatorManager {
    ...
    function withdrawProponentStake(uint256 amount, bytes32 role) external payable;
    function declareDeactivated() external; // Not needed anymore?
    function declareDefault(uint256 defaultedAmount) external;
    function liquidateManager(uint256 amount, bytes32 role) external;
    function changeDistributionDuration(uint256 _distributionDuration) external;
