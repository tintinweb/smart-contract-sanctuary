...
contract Vesting is PausableUpgradeable, BumperAccessControl {
    ...
    // Depricated. This is a packed array of booleans.
    mapping(uint256 => uint256) private claimedBitMap;

    // Depricated. Pause list of investors
    ...
    /// @notice Claim amount of tokens and stake them.
    /// @dev Can only be invoked if the contract is NOT paused.
    /// @param account Investor address to which funds will be transferred
    /// @param vestingInfo Vesting schedule info
        ...
        verifyProofs(account, vestingInfo, merkleProof)
    {
        ...
        require(
            amount >= claimAmount,
            "Nothing to claim" // TODO:
        );

        require(
            stakingAmount >= claimAmount,
            "Nothing to claim" // TODO:
        );
        ...
        if (claimAmount - stakingAmount > 0) {
            IERC20(tokenBUMP).safeTransfer(account, claimAmount - stakingAmount);
        }
        
        emit Claimed(0, account, claimAmount, block.timestamp);
    }
    ...
    /// @param autorenew - auto-renewal staking when its finished
    /// @param merkleProof Proof of data accuracy
    function claimAndWalletToStakingWithPermit(
        address account,
        VestingInfo memory vestingInfo,
        uint16 stakingOption,
        bool autorenew,
        ...
        verifyProofs(account, vestingInfo, merkleProof)
    {
        ...
        _claimAndWalletToStaking(account, vestingInfo, stakingOption, autorenew, walletAmount);
    }
    ...
    /// @param autorenew - auto-renewal staking when its finished
    /// @param merkleProof Proof of data accuracy
    function claimAndWalletToStakingWithAprove(
        address account,
        VestingInfo memory vestingInfo,
        uint16 stakingOption,
        bool autorenew,
    ...
    function _claimAndWalletToStaking(
        address account,
        VestingInfo memory vestingInfo,
        uint16 stakingOption,
        bool autorenew,
        ...
        internal
    {
        ...
        IERC20(tokenBUMP).safeTransferFrom(account, address(this), walletAmount);

        StakeRewards(staking).stakeFor(walletAmount + amount, stakingOption, account, autorenew);
