...
contract NFTMarket is IMarket, ReentrancyGuard, TwoStageOwnable, ChainlinkClient {
    ...
    function forceDistributeAssets(
        ...
        uint256 shareMinter,
        bool isPremium
    ) external onlyOwnerOrOperator whenNotStopped returns (bool) {
        require(itemId > 0 && itemId <= _itemIds, "Market item not exist");
        MarketItem memory item = fetchSpecificMarketItem(itemId);
        require(item.isResolved, "Payment had to be");
    ...
    function rejectDeal(uint256 itemId, uint256 fee) external onlyOwnerOrOperator whenNotStopped nonReentrant returns (bool) {
    ...
    function _distributeAssets(uint256 itemId, uint256 feeMinter) private nonReentrant returns (bool) {
        MarketItem memory item = fetchSpecificMarketItem(itemId);
    ...
    modifier onlyOwnerOrOperator() {
        address caller = msg.sender;
        require(caller == _operator || caller == owner(), "Not owner or operator");
        _;
    }
}
