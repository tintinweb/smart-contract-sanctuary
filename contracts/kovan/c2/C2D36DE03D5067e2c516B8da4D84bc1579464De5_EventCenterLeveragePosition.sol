...
contract EventCenterLeveragePosition is Ownable {
    ...
    uint256 public epochInterval = 10 minutes; //for test only
    uint256 public epochRound;
    bool epochRoundAutoStart = false;
    ...
    event SetAssetWeight (
        address indexed token,
        uint256 indexed weight
    );

    event SetEpochInterval(
        uint256 epochInterval
    );

    event ToggleEpochAutoStart(
        address indexed owner,
        bool indexed autoEpoch
    );
    ...
    function setEpochInterval(uint256 _epochInterval) external onlyOwner notInEpoch {
    ...
    function startEpoch() public onlyOwner notInEpoch {
        epochRound = epochRound + 1; 
        epochStart = block.timestamp;
        epochEnd = epochStart + epochInterval;
        emit StartEpoch(msg.sender, epochRound);
    }

    function toggleEpochAutoStart() external onlyOwner {
        epochRoundAutoStart = !epochRoundAutoStart;
        emit ToggleEpochAutoStart(msg.sender, epochRoundAutoStart);
    }


    function setWeight(address _token, uint256 _weight) external onlyOwner notInEpoch {
    ...
    function emitAddMarginEvent(
        ...
        address EOA = AccountCenterInterface(accountCenter).getEOA(msg.sender);
        address account = msg.sender;
        emit AddMargin(EOA, account, leverageToken, amountLeverageToken,epochRound);
    }
    ...
    function emitRemoveMarginEvent(
        ...
        address EOA = AccountCenterInterface(accountCenter).getEOA(msg.sender);
        address account = msg.sender;
        emit RemoveMargin(EOA, account, leverageToken, amountLeverageToken,epochRound);
    }
    ...
    function autoStartEpoch() internal {
        if (epochEnd > block.timestamp){
            if(epochRoundAutoStart == true){
                epochRound = epochRound + 1; 
                epochStart = block.timestamp;
                epochEnd = epochStart + epochInterval;
                emit StartEpoch(msg.sender, epochRound);
