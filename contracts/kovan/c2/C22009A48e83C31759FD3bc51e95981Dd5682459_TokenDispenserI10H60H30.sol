...
contract TokenDispenserBase is Context, Ownable {
    ...
    function replaceAccountWith(address oldAddress, address newAddress) external onlyOwner {
        ...
        accountVestedDetails[newAddress].accruedVested = avd.accruedVested;
        accountVestedDetails[newAddress].vestedPerAllocationUpdated = avd.vestedPerAllocationUpdated;
        accountVestedDetails[newAddress].claimed = avd.claimed;
...
contract TokenDispenserI10H60H30 is TokenDispenserBase {
    uint public phase1EndTime;
    uint public phase2EndTime;
    uint public constant immediateVestRatio = 1e17;
    uint public constant p1VestRatio = 6e17;
    uint public constant p2VestRatio = 3e17;
    uint public constant d1rate = p1VestRatio / 30 minutes;
    uint public constant d2rate = p2VestRatio / 30 minutes;

    constructor (address kine_, uint startTime_) public {
        kine = IERC20(kine_);
        startTime = startTime_;
        transferPaused = false;
        phase1EndTime = startTime_.add(30 minutes);
        phase2EndTime = startTime_.add(1 hours);
    }
    ...
    function vestedPerAllocation() public view returns (uint) {
        ...
        if (currentTime <= phase2EndTime) {
            return immediateVestRatio.add(p1VestRatio).add(currentTime.sub(phase1EndTime).mul(d2rate));
        }
        return 1e18;
    }
}