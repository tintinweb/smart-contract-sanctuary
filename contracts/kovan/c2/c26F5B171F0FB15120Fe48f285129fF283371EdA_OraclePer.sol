...
contract OraclePer is ChainlinkRequestInterface, OracleInterfacePer, Ownable, LinkTokenReceiver, WithdrawalInterface {
   ...
   */
  function setRequestsPermission(address _consumer, bool _allowed)
    external
    override
    onlyOwner()
  {
  ...
  modifier onlyAuthorizedConsumer() {
    require(authorizedConsumers[msg.sender] || msg.sender == owner(), "Not an authorized consumer to request");
    _;
  }
  /**
...
interface OracleInterfacePer {
  ...
  function setRequestsPermission(address node, bool allowed) external;
