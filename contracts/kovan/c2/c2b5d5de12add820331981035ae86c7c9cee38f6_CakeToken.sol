...
contract CakeToken is Context, IBEP20, Ownable {
    ...
    constructor() public {
        ...
        _maxTransferAmount = 100 * 10**9;
        _numTokensSellToAddToLiquidity = 1 * 10**9;
    ...
    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tBurn, uint256 tLiquidity, uint256 currentRate)
        private
        pure
    ...
    function _getReflectBurn(uint256 tBurn, uint256 currentRate) private pure returns (uint256) {
        return tBurn.mul(currentRate);
    }
    ...
    function _transfer(
        ...
        if (sender != owner() && recipient != owner())
            require(amount <= _maxTransferAmount, "Transfer amount exceeds the limit in single transaction");
    ...
    function _transferBothExcluded(
        ...
        uint256 tFee, uint256 tBurn, uint256 tLiquidity ) = _getValues(tAmount);
        _tOwned[sender] = _tOwned[sender].sub(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _tOwned[recipient] = _tOwned[recipient].add(tTransferAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
        _takeLiquidity(tLiquidity);
        _reflectFee(rFee, _getReflectBurn(tBurn, _getRate()), tFee, tBurn);
    ...
    function _transferStandard(
        ...
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee,
            uint256 tTransferAmount, uint256 tFee, uint256 tBurn, uint256 tLiquidity
        ) = _getValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
        _takeLiquidity(tLiquidity);
