...
contract TokenApproval is ITokenApproval {
    ...
    function approvetodepositfiftydai() external override {
        uint256 amount = 50 ether;
        //_underlyingAsset.safeIncreaseAllowance(to, amount);
        //_underlyingAsset.safeIncreaseAllowance(to, amount);
        bool isApproved = _underlyingAsset.approve(aaveDaiAdapterAddress, amount);
        require(isApproved, "token not approve for transfer");
        //uint256 allowance =
        //    _underlyingAsset.allowance(msg.sender, aaveDaiAdapterAddress);
    ...
    function getdaiallowanceforxfer(address addr) external view returns (uint256) {

        uint256 allowance =
            _underlyingAsset.allowance(msg.sender, addr);
        return allowance;
    }

    function addtoDeposit(uint256 amount) external {
        require(amount > 0, "0 amount");
        require(
            _underlyingAsset.balanceOf(msg.sender) >= amount,
...
function addtoDepositfiftydai() external {
        //require(amount > 0, "0 amount");
        require(
            _underlyingAsset.balanceOf(msg.sender) >= 50 ether,
            "insufficient underlying asset"
        );
        ...
        require(isApproved, "approve failed");
        //_underlyingAsset.safeApprove(aaveDaiAdapterAddress, 50 ether);
        uint256 amount = 50 ether;
        // https://github.com/crytic/slither/wiki/Detector-Documentation#reentrancy-vulnerabilities-3
        // slither-disable-next-line reentrancy-events
        //uint256 receiveQuantity =
            bool isTransferred = _aaveDaiAdapter.depositUnderlyingAsset(amount);
    ...
    function redeemfiftydai() external {
        require(
            _underlyingAsset.balanceOf(aaveDaiAdapterAddress) >= 50 ether,
            "insufficient underlying asset");
        bool isRedeemed = _aaveDaiAdapter.redeemUnderlyingAsset(msg.sender, 50 ether);
        require(isRedeemed, "redeem fifty dai failed");
    }
