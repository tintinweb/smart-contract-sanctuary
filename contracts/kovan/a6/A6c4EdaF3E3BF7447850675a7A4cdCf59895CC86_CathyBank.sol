...
contract CathyBank is Ownable {
    ...
    mapping(address => uint256) private loanBalance;
    mapping(address => uint256) public loanTime;
    mapping(address => uint256) private PHXBalance;
    /* mapping(address => uint256) private PHXIncome; */
    mapping(address => uint256) public lastActionTime;
    mapping(address => uint256) public ShareHolderRewards;

    uint256 public PHXReserves = 0;
    uint256 public PHXIncome = 0;
    uint256 public RewardsShare = 80;
    uint256 public CBTRevenue = 0;

    // Settings
    uint256 public minCollateralPercent = 150; // 150 = 150%
    uint256 public loanInterestPercent = 7; // 7000000 = 7%
    uint256 public savingsInterestPercent = 4; // 4000000 = 4%
    uint256 public ETHPHXConversionRate = 10000000; // 100000000 = 100000
    ...
    event PHXDepositTransaction(
        address sender,
        uint256 amount,
        uint256 totalBalance
    );

    event PHXWithdrawTransaction(
        address sender,
        uint256 amount,
        uint256 totalBalance
    );

    event PHXBorrowTransaction(
        address borrower,
    ...
    event PHXLoanPaymentTransaction(
        address borrower,
        uint256 PHXPayment,
        uint256 balance
    );
    ...
    function getPHXRewardsShare(
        address _shareholder
    ) public view returns (uint256) {
        uint256 distributableRewards = PHXIncome.mul(RewardsShare).div(100);
        uint256 _rewardsShare = getShares(_shareholder, distributableRewards);

        return _rewardsShare;
    }

    function computeShareRewards() external onlyOwner {
        uint256 distributableRewards = PHXIncome.mul(RewardsShare).div(100); // 80% of revenue goes to shareholder

        for (uint8 i = 0; i < cathyBankTokenHolders.length; i++) {
            uint256 _rewardShare = getPHXRewardsShare(cathyBankTokenHolders[i]);
            ShareHolderRewards[cathyBankTokenHolders[i]] = _rewardShare;
            distributedRewards = distributedRewards.add(_rewardShare);
        }
        PHXIncome = PHXIncome.sub(distributableRewards);
        CBTRevenue = PHXIncome;
        PHXIncome = 0;
    }

    function claimRewards() public {
        transferRewards(msg.sender);
    }

    function payLoanFullPHX() public {
        uint256 borrowerLoanBalance = getLoanBalance(msg.sender);
        uint256 collateralBalance = ETHCollateral[msg.sender];
        uint256 accruedInterest = getaccruedInterest(msg.sender);
        address payable loaner = msg.sender;
        require(borrowerLoanBalance > 0, "Payment Insufficient");

        IERC20 PHX = IERC20(phxToken);
        borrowerLoanBalance = borrowerLoanBalance.sub(borrowerLoanBalance);
        PHXReserves = PHXReserves.add(borrowerLoanBalance);


        ETHCollateral[msg.sender] = 0;
        loanBalance[msg.sender] = 0;
        PHXIncome = PHXIncome.add(accruedInterest);

        loaner.transfer(collateralBalance);

        PHX.transferFrom(msg.sender, address(this), borrowerLoanBalance);

        emit PHXLoanPaymentTransaction(
          msg.sender,
          borrowerLoanBalance,
          borrowerLoanBalance
        );
    ...
    function depositPHX(uint256 _PHXAmount) public {
        uint256 newBalance = getPHXBalance(msg.sender).add(_PHXAmount);
        IERC20 PHX = IERC20(phxToken);
        PHX.transferFrom(msg.sender, address(this), _PHXAmount);
        PHXBalance[msg.sender] = newBalance;
        PHXReserves = PHXReserves.add(_PHXAmount);
        lastActionTime[msg.sender] = block.timestamp;

        emit PHXDepositTransaction(
            msg.sender,
            _PHXAmount,
            PHXBalance[msg.sender]
        );
    }

    /* loanpayment - principal - (savingsInterest) */
    function borrowPHX(uint256 _PHXAmount) public payable {
        require(_PHXAmount <= PHXReserves, "Not enough reserves");
        require(
            computeCollateralPercent(msg.value, _PHXAmount) >=
                minCollateralPercent,
            "Provide more collateral"
        );
        require(loanBalance[msg.sender] == 0, "Must not have existing loan");

        ETHCollateral[msg.sender] = ETHCollateral[msg.sender].add(msg.value);
        loanBalance[msg.sender] = loanBalance[msg.sender].add(_PHXAmount);
        loanTime[msg.sender] = block.timestamp;
        PHXReserves = PHXReserves.sub(_PHXAmount);

        IERC20 PHX = IERC20(phxToken);
        PHX.transfer(msg.sender, _PHXAmount);
        emit PHXBorrowTransaction(
            msg.sender,
            msg.value,
            _PHXAmount,
            loanInterestPercent
    ...
    function withdrawPHX(uint256 _PHXAmount) public {
        uint256 _PHXBalance = getPHXBalance(msg.sender);
        uint256 _remainingBalance = _PHXBalance.sub(_PHXAmount);
        require(_PHXBalance >= _PHXAmount, "Insufficient balance");

        IERC20 PHX = IERC20(phxToken);
        PHX.transfer(msg.sender, _PHXAmount);
        PHXBalance[msg.sender] = _remainingBalance;
        PHXReserves = PHXReserves.sub(_PHXAmount);
        lastActionTime[msg.sender] = block.timestamp;

        emit PHXWithdrawTransaction(
            msg.sender,
            _PHXAmount,
            _PHXBalance
        );
    ...
    function computeCollateralPercent(uint256 _ETHAmount, uint256 _PHXAmount)
        public
        view
        returns (uint256)
    {
        return _ETHAmount.mul(ETHPHXConversionRate) / (10**18) / _PHXAmount;
    }

    function getPHXBalance(address _owner) public view returns (uint256) {
        if (lastActionTime[_owner] == 0 || PHXBalance[_owner] == 0) return 0;
        uint256 timeElapsed = block.timestamp - lastActionTime[_owner];
        uint256 secsYear = 60 * 60 * 24 * 360;
        uint256 accruedInterest = (timeElapsed *
            savingsInterestPercent *
            PHXBalance[_owner] *
            1000000) /
            secsYear /
            100000000;
        return PHXBalance[_owner] + accruedInterest;
    }

    function transferRewards(address _shareholder) internal {
        IERC20 PHX = IERC20(phxToken);
        uint256 _shareHolderRewards = ShareHolderRewards[_shareholder];
        ShareHolderRewards[_shareholder] = 0;

        require(_shareHolderRewards <= 0, "No rewards");
        PHX.transfer(_shareholder, _shareHolderRewards);

        emit LogAmount(_shareHolderRewards);
    }

    function getShares(address _shareholder, uint256 distributableRewards) public view returns (uint256) {
        ICERC20 cbt = ICERC20(cathyBankContractAddress);
        ...
        return ((balance.mul(distributableRewards)).div(totalSupply));
    }

    function getLoanBalance(address _owner) public view returns (uint256) {
        if (loanBalance[_owner] == 0) return 0;
        uint256 timeElapsed = block.timestamp - loanTime[_owner];
        uint256 secsYear = 60 * 60 * 24 * 360;
        uint256 accruedInterest = (timeElapsed *
            loanInterestPercent *
            loanBalance[_owner] *
            ...
            secsYear /
            100000000;
        return loanBalance[_owner] + accruedInterest;
    }

    function getaccruedInterest(address _owner) public view returns (uint256) {
        if (loanBalance[_owner] == 0) return 0;
        uint256 timeElapsed = block.timestamp - loanTime[_owner];
        uint256 secsYear = 60 * 60 * 24 * 360;
        uint256 accruedInterest = (timeElapsed *
