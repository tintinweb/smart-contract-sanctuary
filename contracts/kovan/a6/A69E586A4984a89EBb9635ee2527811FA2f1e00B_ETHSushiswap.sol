...
contract ETHSushiswap {
    ...
    constructor(IUniswapV2Router02 _sushiSwapRouter, IUniswapV2Factory _sushiswapFactory/*, IMasterChef _masterChef*/) public {
    ...
    function swapTokenToToken(address _tokenA, address _tokenB, uint256 _amountAIn) external {
        ...
        require(IERC20(_tokenA).approve(address(sushiswapRouter), _amountAIn), 'approve failed.');

        address[] memory path = new address[](2);
        path[0] = _tokenA;
        path[1] = _tokenB;

        uint[] memory amounts = sushiswapRouter.swapExactTokensForTokens(_amountAIn, 0, path, msg.sender, block.timestamp);
    ...
    function swapTokenToEth(address _tokenA, uint256 _amountAIn) external {
        ...
        uint[] memory amounts = sushiswapRouter.swapExactTokensForETH(_amountAIn, 0, path, msg.sender, block.timestamp);
    ...
    function swapEthToToken(address _tokenB) external payable {
        ...
        uint[] memory amounts = sushiswapRouter.swapExactETHForTokens{value : msg.value}(0, path, msg.sender, block.timestamp);
    ...
    function addLiquidity(address _tokenA, address _tokenB, uint _amountADesired, uint _amountBDesired) external {
        ...
        require(IERC20(_tokenA).approve(address(sushiswapRouter), _amountADesired), 'approve failed.');
        
        require(IERC20(_tokenB).transferFrom(msg.sender, address(this), _amountBDesired), 'transferFrom failed.');
        require(IERC20(_tokenB).approve(address(sushiswapRouter), _amountBDesired), 'approve failed.');

        (uint amountA, uint amountB, uint liquidity) = sushiswapRouter.addLiquidity(_tokenA, _tokenB, _amountADesired, _amountBDesired, 0, 0, msg.sender, block.timestamp);
    ...
    function addLiquidityETH(address _token, uint _amountDesired) external payable {
        ...
        require(IERC20(_token).approve(address(sushiswapRouter), _amountDesired), 'approve failed.');

        (uint amountToken, uint amountETH, uint liquidity) = sushiswapRouter.addLiquidityETH{value: msg.value}(_token, _amountDesired, 0, 0, msg.sender, block.timestamp);
    ...
    function removeLiquidity(address _tokenA, address _tokenB, address _pair, uint _liquidity) external {
        ...
        require(IERC20(_pair).approve(address(sushiswapRouter), _liquidity), 'approve failed.');
        
        (uint amountA, uint amountB) = sushiswapRouter.removeLiquidity(_tokenA, _tokenB, _liquidity, 0, 0, msg.sender, block.timestamp);
    ...
    function removeLiquidityETH(address _token, address _pair, uint _amountDesired) external {
        ...
        require(IERC20(_pair).approve(address(sushiswapRouter), _amountDesired), 'approve failed.');
        
        (uint amountToken, uint amountETH) = sushiswapRouter.removeLiquidityETH(_token, _amountDesired, 0, 0, msg.sender, block.timestamp);
