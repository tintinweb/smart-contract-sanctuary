...
contract Precog {
    ...
    function maxProfitable(address user) public view returns (
        uint profit, address exchange, address[] memory pairs, address futureExchange
    ) {
        if (availableAmount[user] > feeTrading) {
            for (uint k = 0; k < futureExchanges.length; k++) {
                ...
                for (uint j = 0; j < tradingTokens.length; j++) {
                    ...
                    (pairs[0], pairs[1]) = (usdc, tradingTokens[j]);
                    for (uint256 i = 0; i < exchanges.length; i++) {
                        IExchangeRouter _exchange = IExchangeRouter(exchanges[i]);
                        //(uint256 _profit,) = _calculateProfit(user, _exchange, _pairs, _futureExchange);
                        //if (_profit > profit) {
                        //    profit = _profit;
    ...
    ) view internal returns(uint profit, address futureToken) {
    ...
    function _getMinPriceAmount(
        ...
        address minPriceToken = futureExchange.minPriceToken(pairs[1], pairs[0]);
    ...
    function invest(
        ...
        (uint256 profit, address futureToken) = _calculateProfit(user, exchange, pairs, futureExchange);
