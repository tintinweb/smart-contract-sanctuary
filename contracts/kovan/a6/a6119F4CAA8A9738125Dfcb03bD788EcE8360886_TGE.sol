...
contract TGE is Ownable, Signature {
     ...
     */
    function signTermsAndConditions(bytes memory signature) external {
     ...
     */
    function deposit(address referrer) external payable notAborted {
    ...
    /**
     * @dev Returns the forex price as established by the deposit amount once
     *      the TGE is closed.
    ...
    function emergencyWithdrawAllFunds() public onlyOwner {
        ...
        if (balance > 0) msg.sender.call{value: self.balance}("");
        // Transfer FOREX.
