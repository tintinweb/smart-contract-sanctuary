...
/**
 * @title DEMOMMINE | Mariana Mining Security Token
 * @author Stobox Technologies Inc.
 * @dev DEMOMMINE ERC20 Token | This contract is opt for digital securities management.
 */
...
contract DEMOMMINEToken is IERC20, Roles {
    ...
    modifier onlyWhitelisted(address _account) {
        if (_isDisabledWitelist) {
            require(
                ...
                "DEMOMMINE: not whitelisted address."
            );
        }
        _;
    }
    ...
    modifier onlyWithUnfrozenFunds(address _account) {
        require(
            ...
            "DEMOMMINE: funds are frozen."
        );
        _;
    }

    /**
     * @notice DEMOMMINEToken simply implements a ERC20 token.
     */
    ...
    function burn(address _account, uint256 _amount) external onlySuperAdmin {
        require(balanceOf(_account) >= _amount, "DEMOMMINE: balance too low");
    ...
    function transferFunds(
        ...
        require(balanceOf(_from) >= _amount, "DEMOMMINE: not enough tokens");
    ...
    function splitOrMerge(uint256 _x, uint256 _y) external onlySuperAdmin {
        require(_x != _y, "DEMOMMINE: _x must not be equal to _y");
        require(_x > 0, "DEMOMMINE: _x must be greater than 0");
        require(_y > 0, "DEMOMMINE: _y must be greater than 0");
    ...
    */
        ...
        returns (bool)
    {
        ...
        _approve(
            ...
            _allowances[_sender][msg.sender].sub(
                ...
                "DEMOMMINE: transfer amount exceeds allowance"
            )
        );
        ...
        returns (bool)
    {
        _approve(
            ...
            _allowances[msg.sender][_spender].sub(
                ...
                "DEMOMMINE: decreased allowance below zero"
            )
        );
    ...
    function _transfer(
        ...
        require(_sender != address(0), "DEMOMMINE: transfer from the zero address");
        require(_recipient != address(0), "DEMOMMINE: transfer to the zero address");

        _beforeTokenTransfer(_sender, _recipient, _amount);

        _balances[_sender] = _balances[_sender].sub(
            _amount,
            "DEMOMMINE: transfer amount exceeds balance"
        );
    ...
    function _mint(address _account, uint256 _amount) internal virtual {
        require(_account != address(0), "DEMOMMINE: mint to the zero address");
    ...
    function _burn(address _account, uint256 _amount) internal virtual {
        require(_account != address(0), "DEMOMMINE: burn from the zero address");

        _beforeTokenTransfer(_account, address(0), _amount);

        _balances[_account] = _balances[_account].sub(
            _amount,
            "DEMOMMINE: burn amount exceeds balance"
        );
    ...
    function _approve(
        ...
        require(_owner != address(0), "DEMOMMINE: approve from the zero address");
        require(_spender != address(0), "DEMOMMINE: approve to the zero address");
    ...
    function _updateTransferLimit(address _account, uint256 _amount) private {
        ...
        _transferLimits[_account].allowedToTransfer = _transferLimits[_account]
            .allowedToTransfer
            .sub(_amount, "DEMOMMINE: transfer exceeds your transfer limit");
    }
    ...
    function _updateTransactionCountLimit(address _account) private {
        if (_isEnabledTransactionCount) {
            ...
            _transactionCountLimits[_account]
                ...
                .sub(1, "DEMOMMINE: transfer exceeds your transaction count limit");
        }
    }
}
