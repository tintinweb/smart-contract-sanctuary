...
contract DefaultReserveInterestRateStrategy is IReserveInterestRateStrategy {
    ...
    )
  {
    ...
    vars.currentStableBorrowRate = 0; /*ILendingRateOracle(addressesProvider.getLendingRateOracle())
...
abstract contract FactoryStorage {
    ...
    event ReserveDataSet(address indexed underlyingAssetAddr);
    event ReserveConfigurationChanged(address indexed asset, uint256 ltv, uint256 liquidationThreshold, uint256 liquidationBonus);
    ...
    address[] public underlyingAssetAddrs; 

    function getReserveData(address underlyingAsset) public view returns (ReserveData memory) {
        return reserveData[underlyingAsset];
    }

    // TODO: add permissions (i.e. only admin can set reserve data)
    function setReserveData(address underlyingAsset, ReserveData memory newData) public {
        if (bytes(reserveData[underlyingAsset].underlyingAssetName).length == 0) {
            underlyingAssetAddrs.push(underlyingAsset);
        }
        reserveData[underlyingAsset] = newData;
        emit ReserveDataSet(underlyingAsset);
    }
  ...
  function configureReserve(
    ...
    uint256 liquidationThreshold,
    uint256 liquidationBonus,
    uint256 reserveFactor) external {

        DataTypes.ReserveConfigurationMap storage currentConfig = reserveData[asset].configuration;
...
contract Factory is FactoryStorage, UserActions {
    ...
    address public defaultAToken;
    address public defaultStableDebtToken;
    address public defaultVariableDebtToken;
    ...
    constructor(
        ...
        address _defaultAToken,
        address _defaultStableDebtToken,
        address _defaultVariableDebtToken,
        address[] memory _underlyingAssetAddrs,
        FactoryStorage.ReserveData[] memory _reserveData
    ) 
    public 
    {
        require(_underlyingAssetAddrs.length == _reserveData.length, "Factory: Bad reserve arr lengths");
        ...
        defaultAToken = _defaultAToken;
        defaultStableDebtToken = _defaultStableDebtToken;
        defaultVariableDebtToken = _defaultVariableDebtToken;

        for (uint256 i = 0; i < _underlyingAssetAddrs.length; i++) {
            setReserveData(_underlyingAssetAddrs[i], _reserveData[i]);
        }
    }

    function createMarket(string memory name) external {

        require(userMarkets[msg.sender] == address(0),"Market already exists for user");
        ...
        addDefaultReserves(address(lendingPool), address(this));
        userMarkets[msg.sender] = address(lendingPool);
    ...
    function addDefaultReserves(address lendingPool, address treasury) internal {
        require(lendingPool != address(0), "Factory: Invalid market");
        require(treasury != address(0), "Factory: Invalid treasury");
        ILendingPoolAddressesProvider addressesProvider = ILendingPool(lendingPool).getAddressesProvider();
        LendingPoolConfigurator poolConfigurator = LendingPoolConfigurator(addressesProvider.getLendingPoolConfigurator());

        ILendingPoolConfigurator.InitReserveInput[] memory input = new ILendingPoolConfigurator.InitReserveInput[](underlyingAssetAddrs.length);

        for (uint256 i = 0; i < underlyingAssetAddrs.length; i++) {
            
            ReserveData memory reserveData = getReserveData(underlyingAssetAddrs[i]);
            
            input[i] = ILendingPoolConfigurator.InitReserveInput({
                aTokenImpl: defaultAToken, 
                stableDebtTokenImpl: defaultStableDebtToken, 
                variableDebtTokenImpl: defaultVariableDebtToken,
                underlyingAssetDecimals: reserveData.underlyingAssetDecimals, 
                interestRateStrategyAddress: address(
                    new DefaultReserveInterestRateStrategy(
                        addressesProvider, 
                        reserveData.interestRateStrategy.optimalUtilizationRate,
                        reserveData.interestRateStrategy.baseVariableBorrowRate,
                        reserveData.interestRateStrategy.variableRateSlope1,
                        reserveData.interestRateStrategy.variableRateSlope2,
                        reserveData.interestRateStrategy.stableRateSlope1,
                        reserveData.interestRateStrategy.stableRateSlope2
                    )
                ), 
                underlyingAsset: underlyingAssetAddrs[i],
                treasury: treasury,
                incentivesController: address(0), 
                underlyingAssetName: reserveData.underlyingAssetName,
                aTokenName: reserveData.aTokenName,
                aTokenSymbol: reserveData.aTokenSymbol,
                variableDebtTokenName: reserveData.variableDebtTokenName,
                variableDebtTokenSymbol: reserveData.variableDebtTokenSymbol,
                stableDebtTokenName: reserveData.stableDebtTokenName,
                stableDebtTokenSymbol: reserveData.stableDebtTokenSymbol,
                params: "0x",
                data: reserveData.configuration.data
            });
        }

        poolConfigurator.batchInitReserve(input);
    }
...
contract LendingPoolConfigurator is VersionedInitializable, ILendingPoolConfigurator {
  ...
  function isPoolAdmin(address user) public view returns(bool){
    return user == addressesProvider.getPoolAdmin();
  }
   ...
   **/
  function batchInitReserve(InitReserveInput[] calldata input) external onlyOwner {
   ...
   **/
  function updateAToken(UpdateATokenInput calldata input) external onlyOwner {
    ILendingPool cachedPool = pool;
   ...
   **/
  function updateStableDebtToken(UpdateDebtTokenInput calldata input) external onlyOwner {
    ILendingPool cachedPool = pool;
   ...
   * @param asset The address of the underlying asset of the reserve
   **/
  function disableBorrowingOnReserve(address asset) external onlyOwner {
    DataTypes.ReserveConfigurationMap memory currentConfig = pool.getConfiguration(asset);
   ...
   * @param asset The address of the underlying asset of the reserve
   **/
  function enableReserveStableRate(address asset) external onlyOwner {
    DataTypes.ReserveConfigurationMap memory currentConfig = pool.getConfiguration(asset);
   ...
   * @param asset The address of the underlying asset of the reserve
   **/
  function disableReserveStableRate(address asset) external onlyOwner {
    DataTypes.ReserveConfigurationMap memory currentConfig = pool.getConfiguration(asset);
   ...
   * @param asset The address of the underlying asset of the reserve
   **/
  function activateReserve(address asset) external onlyOwner {
    DataTypes.ReserveConfigurationMap memory currentConfig = pool.getConfiguration(asset);
   ...
   * @param asset The address of the underlying asset of the reserve
   **/
  function deactivateReserve(address asset) external onlyOwner {
    _checkNoLiquidity(asset);
   ...
   * @param asset The address of the underlying asset of the reserve
   **/
  function unfreezeReserve(address asset) external onlyOwner {
    DataTypes.ReserveConfigurationMap memory currentConfig = pool.getConfiguration(asset);
   ...
   **/
  function setReserveFactor(address asset, uint256 reserveFactor) external onlyOwner {
    DataTypes.ReserveConfigurationMap memory currentConfig = pool.getConfiguration(asset);
...
abstract contract UserActions {

    modifier onlyPoolAdmin(address configurator){
        require(LendingPoolConfigurator(configurator).isPoolAdmin(msg.sender), "!admin");
        _;
    }

    function enableBorrowingOnReserve(address configurator, address asset, bool stableBorrowRateEnabled) external onlyPoolAdmin(configurator) {
        LendingPoolConfigurator(configurator).enableBorrowingOnReserve(asset, stableBorrowRateEnabled);
    }

    function disableBorrowingOnReserve(address configurator, address asset) external onlyPoolAdmin(configurator) {
        LendingPoolConfigurator(configurator).disableBorrowingOnReserve(asset);
    }
    ...
    function configureReserveAsCollateral(
        ...
        external onlyPoolAdmin(configurator) {

        LendingPoolConfigurator(configurator).configureReserveAsCollateral(asset, ltv, liquidationThreshold, liquidationBonus);
    }

    function enableReserveStableRate(address configurator, address asset) external onlyPoolAdmin(configurator) {
        LendingPoolConfigurator(configurator).enableReserveStableRate(asset);
    }

    function disableReserveStableRate(address configurator, address asset) external onlyPoolAdmin(configurator) {
        LendingPoolConfigurator(configurator).disableReserveStableRate(asset);
    }

    function activateReserve(address configurator, address asset) external onlyPoolAdmin(configurator) {
        LendingPoolConfigurator(configurator).activateReserve(asset);
    }

    function deactivateReserve(address configurator, address asset) external onlyPoolAdmin(configurator) {
        LendingPoolConfigurator(configurator).deactivateReserve(asset);
    }

    function freezeReserve(address configurator, address asset) external onlyPoolAdmin(configurator) {
        LendingPoolConfigurator(configurator).freezeReserve(asset);
    }

    function unfreezeReserve(address configurator, address asset) external onlyPoolAdmin(configurator) {
        LendingPoolConfigurator(configurator).unfreezeReserve(asset);
    }

    function setReserveFactor(address configurator, address asset, uint256 reserveFactor) external onlyPoolAdmin(configurator) {
        LendingPoolConfigurator(configurator).setReserveFactor(asset, reserveFactor);
    }

    function setReserveInterestRateStrategyAddress(address configurator, address asset, address rateStrategyAddress) external onlyPoolAdmin(configurator) {
        LendingPoolConfigurator(configurator).setReserveInterestRateStrategyAddress(asset, rateStrategyAddress);
    }
}
