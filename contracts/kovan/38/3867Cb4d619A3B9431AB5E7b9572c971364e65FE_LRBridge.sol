...
contract LRBridge {
    ...
    event Relay(string geohash, uint price, uint population);
    ...
    mapping(string => LRData) public data;
    ...
    function relay(bytes memory proof) external {
        ...
        data[params.geohash] = LRData(result.price, result.population);
        
        emit Relay(params.geohash, data[params.geohash].price, data[params.geohash].population);
    }
}