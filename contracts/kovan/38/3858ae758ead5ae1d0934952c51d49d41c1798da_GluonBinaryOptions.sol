...
contract GluonBinaryOptions {
    ...
    event OptionResolved(bytes32 identifier, uint blockNumber, int marketPrice);

    constructor (address payable owner_, address oracleAddr) {
        owner = owner_;
    ...
    function addBinaryOption(string calldata optionCode, string calldata description, uint price, uint durationInBlocks) public returns (bool success){
        bytes32 id = keccak256(abi.encodePacked(optionCode, description, durationInBlocks));
        return addBinaryOption(id, optionCode, description, price, durationInBlocks);
    }

    function addBinaryOption(bytes32 identifier, string calldata optionCode, string calldata description, uint price, uint durationInBlocks) private returns (bool success) {

        require(durationInBlocks > 0);
        require(Options[identifier].expiryBlock == 0);
        Options[identifier] = Option(optionCode, identifier, description, block.number + durationInBlocks, price, false, Result.Unresolved, 0);
    ...
    function setOptionResult(bytes32 identifier, Result result) internal returns (bool success) {
        Option storage option = Options[identifier];
    ...
    function resolveOption(bytes32 identifier) public returns (bool success){
        Option storage option = Options[identifier];
        require(option.expiryBlock < block.number, 'Not yet expired');
        int latestPrice;
        if( ! option.resolved){
            latestPrice = getLatestPrice();
            uint(latestPrice) >= option.price ? setOptionResult(option.identifier, Result.Yes) : setOptionResult(option.identifier, Result.No);
            option.resolved = true;
            emit OptionResolved(option.identifier, block.number, latestPrice);
        }
        return true;
    }
