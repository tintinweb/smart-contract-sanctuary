...
contract Owned {
  ...
  function __Ownable_init(address newOwner) internal {
    if (owner == address(0)) {
      owner = newOwner;
...
contract UpkeepRegistrationRequests is Owned {
    ...
    constructor(
        ...
        if (LINKAddress != address(0)) {
          initialize(LINKAddress, minimumLINKJuels, msg.sender);
        }
    }
  ...
  ) public 
  {
    ...
    require(LINKAddress != address(0),"link address cannot be 0");
