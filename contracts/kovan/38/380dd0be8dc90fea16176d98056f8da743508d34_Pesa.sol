...
contract Pesa {
    ...
    address public marketDistributionFund;  // PesaCore address: distributed the tokens on the market's user
    address public userDistributionFund;    // PUSD address: distributed the tokens whos burns the PUSD on the app
    
    address public protocalAddress;         // Protocal account that holds the reserved tokens
     ...
     * @param account The initial protocal account that hold the reserved tokens
     * @param marketDistributionFund_ The PesaCore address that hold the market distribution tokens
     * @param userDistributionFund_ The PUSD address that hold the user distribution tokens
     ...
     */
    constructor(address account, address marketDistributionFund_, address userDistributionFund_, address foundersFund_, address investorsFund_, address advisorsFund_, address employeesFund_, uint256 releaseTime_) public {
        admin = msg.sender;
        protocalAddress = account;

        marketDistributionFund = marketDistributionFund_;
        userDistributionFund = userDistributionFund_;
        ...
        balances[address(founderTimelock)] = totalSupply.mul(foundersPercentage).div(100);           // allocating 10M tokens
        balances[address(investorTimelock)] = totalSupply.mul(investorsPercentage).div(100);         // allocating 10M tokens
        balances[address(advisorTimelock)] = totalSupply.mul(advisorsPercentage).div(100);           // allocating 5M tokens
        balances[address(employeeTimelock)] = totalSupply.mul(employeesPercentage).div(100);         // allocating 5M tokens
        
        balances[marketDistributionFund] = totalSupply.mul(marketDistributionPercentage).div(100);   // allocating 30M tokens (15M each for suppliers and borrowers)
        balances[userDistributionFund] = totalSupply.mul(userDistributionPercentage).div(100);       // allocating 30M tokens
        balances[protocalAddress] = totalSupply.mul(reservedPercentage).div(100);                    // allocating 10M tokens
    }
