...
import "./interfaces/IidoMaster.sol";
 ...
 contract IDOPool is Pausable, Whitelist, ReentrancyGuard  {
    ...
    // function removeOtherERC20Tokens(ERC20 _erc20Token, uint256 amount, address _to) external onlyOwner {
    //     require(address(_erc20Token) != address(rewardToken), "Token Address has to be diff than the erc20 subject to sale"); // Confirm tokens addresses are different from main sale one
    //     require(_erc20Token.transfer(_to, amount), "ERC20 Token transfer failed");
    // } 
