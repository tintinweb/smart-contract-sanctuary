...
contract JCoinMintable is ERC20
{
    ...
    mapping(address => uint256) public assetPrices; // address(0) = ETH
    ...
    event GetUnlocked(address indexed buyer, uint256 amount);
    event Withdraw(address indexed msgSender, bool isEth, address indexed to, uint256 amount);
    ...
    constructor(address _owner,
                ...
                uint256[] memory _prices) public ERC20(_owner, _name, _symbol)
    {
    ...
    function withdrawEth(address payable _to, uint256 _amount) external onlyRevenueOwner
    {
        if(_amount == 0)
        {
            _amount = ethCollected;
        }
        
        _to.transfer(_amount);
        ethCollected = ethCollected.sub(_amount);
        
        emit Withdraw(_msgSender(), true, _to, _amount);
    }
    ...
    function withdrawLockedToken(address _to, uint256 _amount) external onlyRevenueOwner
    {
        if(_amount == 0)
        {
            _amount = lockedToken.balanceOf(address(this)) - totalSupply();
        }
        
        lockedToken.safeTransfer(_to, _amount);
        
        require(lockedToken.balanceOf(address(this)) >= totalSupply(), "ERR_INVALID_AMOUNT");
        
        emit Withdraw(_msgSender(), false, _to, _amount);
    }
    //--------------------------------------------------------------------------------------------------------------------------
    
    // Assets:
    //--------------------------------------------------------------------------------------------------------------------------
    // if asset == address(0): price in ETH
    function setPrices(address[] calldata _assets, uint256[] calldata _prices) external onlyOwner
    {
        require(_assets.length == _prices.length, "ERR_ARRAYS_LENGTHS_DONT_MATCH");
    ...
    // todo: kill in prod: !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    function setUnlockTimestamp(uint256 _newUnlockTimestamp) external onlyOwner
    {
    ...
    function calcPrice(address _fromAsset, uint256 _fromAmount) public view returns (uint256 toActualAmount_, uint256 fromActualAmount_)
    {
        ...
        uint256 fromAssetPrice = assetPrices[_fromAsset];
        require(fromAssetPrice > 0, "ERR_ASSET_NOT_SUPPORTED");
        
        uint256 totalSupply = totalSupply();
        uint256 lockedTokenBalance = lockedToken.balanceOf(address(this));
        
        if(lockedTokenBalance <= totalSupply) return (0, 0);
        
        uint256 toAvailableForSell = lockedTokenBalance - totalSupply;
    ...
    function _swap(address _fromAsset, uint256 _fromAmount, uint256 _toExpectedAmount, uint32 _refCode) private
    {
        ...
        require(toActualAmount > 0, "ERR_ZERO_TO_ACTUAL_AMOUNT");
    ...
    function getUnlocked(uint256 _amount) external
    {
        ...
        lockedToken.safeTransfer(_msgSender(), _amount);
        
        emit GetUnlocked(_msgSender(), _amount);
    }
    //--------------------------------------------------------------------------------------------------------------------------
}