...
contract AttraCampaign is KeeperCompatibleInterface, FundManager, Lottery {
    ...
    function _closeCampaign() private {
        ...
        if (successful) {
            ...
            _refundAll();
            state = campaignState.FAIL_SETTLED;
        }
    }

    function _onReceivedRandomness() internal virtual override {
        state = campaignState.SUCCESS_READY_FOR_LOTTERY;
    ...
    function _completeLottery() private {
        ...
        state = campaignState.SUCCESS_SETTLED_WINNERS;
    }
    ...
    function settleBeneficiaryAndTokens() public {
        ...
        _settleBenAndTokens();
        state = campaignState.SUCCESS_SETTLED_BENEFICIARY;
    }
    ...
    {
        // checking if the time has come (duration expired)
        if (!hasEnoughLink()) {
            upkeepNeeded = false;
        } else {
            upkeepNeeded =
                (state == campaignState.OPEN && _canClose()) || // close it - refunds on fail / VRF request on success
    ...
    // ------- DEBUGGING, HEALTHCHECK, PUBLIC INTEREST

    function timeLeftUntilClosing() public view returns (int256) {
        int256 diff = int256(start) +
            int256(duration) -
            int256(block.timestamp);
        return diff <= 0 ? int256(0) : diff;
    }
    ...
    function settledWinners() public view returns (bool) {
        return state == campaignState.SUCCESS_SETTLED_WINNERS;
    }

    function settledBeneficiary() public view returns (bool) {
        return state == campaignState.SUCCESS_SETTLED_BENEFICIARY;
    }
}
...
contract AttraFinance is Ownable {
    ...
    event CreateCampaign(uint256 timestamp, uint256 campaignId);
...
/**
 * A contract to gather contributions and keep track of them.
 *
 * THIS contract
 * Has no notion of campaigns.
 * Works on a per-round-basis.
 * Takes contributions and refunds
 * Distributes tokens rewards to contributors of successful campaigns.

 * The OWNER contract:
 *  - starts and finishes rounds
 *  - sets a minimal amount for each round
*   - pays beneficiaries / lottery winners
 */
...
/**
 * Manages fees, keeps track of collected fees and transfers them when needed.
 *
 ...
 *
 * Has a CrowdFund under its control.
 * Has a notion of rounds, these being equivalent to CrowdFund rounds.
 */
...
/**
 * Lottery
 * Base contract for an AttraCampaign.
 */
...
/**
 * Keep Attra balances cleaner and payments management less error-prown.
