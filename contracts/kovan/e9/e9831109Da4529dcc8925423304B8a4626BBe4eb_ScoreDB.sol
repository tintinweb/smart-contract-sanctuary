...
interface ScoreDBInterface {
    ...
    function requestUpdatedScoreBorrow(uint256 tokenId, uint256 loanId, bytes32 hash, bytes memory signature) external;
...
interface IInvestor{
  function borrowFulfill(uint256,bytes32,bytes memory) external;
}
