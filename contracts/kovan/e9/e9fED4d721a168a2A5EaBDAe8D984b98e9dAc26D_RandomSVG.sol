...
contract RandomSVG is ERC721URIStorage, VRFConsumerBase, Ownable {
    ...
    ERC721 ("RandomSVG", "rsNFT") 
    {
        ...
        tokenCounter = 0; 
        maxNumberOfPaths = 15;
        maxNumberOfPathCommands =15;
        size = 500;
        pathCommands = ["M", "L"];
        colors = ["black", "green", "blue", "purple", "yellow", "red"];
    }
    ...
    function finishMint(uint256 tokenId) public {
        // [X] Check if minted and random number is returned
        // [X] Generate some random SVG code
        // [X] Turn that into a image URI
        // [X] Use that imageURI to format into a tokenURI
        ...
        return string (
            abi.encodePacked(
                ...
                Base64.encode(
                    ...
                    (abi.encodePacked(
                    '{"name": "SVG NFT", ', 
                    '"description": "An NFT base on SVG!", ', 
                    '"attributes": "", ',
                    '"image": "', imageURI, '"}'
                )
            ))
    ...
    function generatePathCommand(uint256 _randomness) public view returns (string memory pathCommand) {
        ...
        uint256 parameterTwo = uint256(keccak256(abi.encode(_randomness, size * 2 + 2))) %size;
