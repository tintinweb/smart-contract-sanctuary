...
contract FAANGVaultKovan is ERC20("DAO Vault Stonks", "daoSTO") {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;
    IERC20 public USDT = IERC20(0x07de306FF27a2B630B1141956844eB1552B956B5);
    IERC20 public USDC = IERC20(0xb7a4F3E9097C08dA09517b5aB877F7a917224ede);
    ...
    constructor(address _treasuryWallet, address _admin, address _strategy) {
        admin = _admin;
    ...
    function setPendingStrategy(address _strategy) external {
    ...
    function setAmountToKeepInVaultPerc(uint _daiPercentage, uint _usdcPercentage, uint usdtPercentage) external {
    ...
    function deposit(uint256 _amount, IERC20 _token) external onlyEOA{
        ...
        if (_token == DAI) {
            ...
            shares = totalSupply() == 0
                ? _amount
                : _amount.mul(totalSupply()).div(getTotalValueInPool());
            ...
            depositedAmount[msg.sender] = depositedAmount[msg.sender].add(_amountMagnified);
        } else if (_token == USDT) {
            ...
            shares = totalSupply() == 0
                ? _amountMagnified
                : _amountMagnified.mul(totalSupply()).div(getTotalValueInPool());
    ...
    function withdraw(uint256 _shares, IERC20 _token) external onlyEOA {
        ...
        if (amountToWithdraw > amountDeposited) {
            ...
            uint256 _fee = _profit.mul(fee).div(100); //20% fee
            amountToWithdraw = amountToWithdraw.sub(_fee);

            if(_token != DAI) {
                _token.safeTransfer(treasuryWallet, _fee.div(1e12));    
            } else {
    ...
    function invest() external {
        ...
        if(usdcBalance > 0) {
            ...
            uint usdcAmountToKeep = (ValueInPool.div(1e12)).mul(usdcPercInVault).div(100);
        ...
        if(usdtBalance > 0) {
            uint usdtAmountToKeep = (ValueInPool.div(1e12)).mul(usdtPercInVault).div(100);
    ...
    function migrateFund(IERC20 _token) external {
        require(msg.sender == admin, "Not an admin");
    ...
    function emergencyWithdraw(IERC20 _token) external {
        ...
        DAI.safeTransfer(admin, DAI.balanceOf(address(this)));
        USDC.safeTransfer(admin, USDC.balanceOf(address(this)));
        USDT.safeTransfer(admin, USDT.balanceOf(address(this)));
    }
