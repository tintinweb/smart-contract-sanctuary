...
contract OriginatorStaking is Initializable, StakingRewards, BaseTokenUpgradeable, IStaking, IProjectFundedRewards, IOriginatorManager {
    ...
    event EndDistribution(uint256 distributionEnd);
     ...
     **/
    function stake(address _onBehalfOf, uint256 _amount) external override notZeroAmount(_amount) {
        require(state == OriginatorStakingState.STAKING, 'ONLY_ON_STAKING_STATE');
        require(block.timestamp < DISTRIBUTION_END, 'DISTRIBUTION_FINISHED');
    ...
    function redeem(address _to, uint256 _amount) external override notZeroAmount(_amount) {
        require(block.timestamp >= DISTRIBUTION_END, 'DISTRIBUTION_NOT_FINISHED');
     ...
     */
    function withdrawProponentStake() external override {
     ...
     */
    function startProjectFundedRewards(uint128 _extraEmissionsPerSecond, address _lendingContractAddress) external override {
      require(state == OriginatorStakingState.STAKING, 'ONLY_ON_STAKING_STATE');
     ...
     * @param _lendingContractAddress lending contract address is relationated with this rewards.
     */
    function endProjectFundedRewards(uint128 _extraEmissionsPerSecond, address _lendingContractAddress) external override {
      require(state == OriginatorStakingState.STAKING, 'ONLY_ON_STAKING_STATE');
      AssetData storage currentDistribution = assets[address(this)];
      uint128 currentEmission = currentDistribution.emissionPerSecond;
    ...
    /**
     * @notice Set new end distribution with the new distribution duration
     * @param _distributionDuration new duration on seconds
     */
    function changeDistributionDuration(uint256 _distributionDuration) external override onlyEmissionManager {
        DISTRIBUTION_END = block.timestamp.add(_distributionDuration);
        emit EndDistribution(DISTRIBUTION_END);
    }
     ...
     **/
    function declareStakingEnd() external override onlyGovernance {
        require(state == OriginatorStakingState.STAKING, 'ONLY_ON_STAKING');
     ...
     **/
    function declareDefault(uint256 _defaultedAmount) external override onlyGovernance {
        require(state == OriginatorStakingState.STAKING, 'ONLY_ON_STAKING_STATE');
...
interface IOriginatorManager {
    ...
    function changeDistributionDuration(uint256 distributionDuration) external;
...
interface IStaking {
  ...
  function withdrawProponentStake() external;
