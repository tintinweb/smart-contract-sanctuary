...
contract Paymaster is BasePaymaster {
    ...
    constructor(address _relayHub, address _forwarder, address _router) {
    ...
    function preRelayedCall(
        ...
        uint256 tokenPreCharge = router.getAmountsOut(maxFeeEth, _tokenAndWrappedPair(address(token)))[1];

        // Recipient should match the sender
        uint256 msgIdx = uint256(_readBytes32(reqData, 4));
        address recipient = address(uint160(uint256(_readBytes32(reqData, 4 + msgIdx + 20))));
        require(recipient == payer, "sender does not match recipient");

        // Taking the tokens from the user
        token.transferFrom(payer, address(this), tokenPreCharge);

        return (abi.encode(payer, tokenPreCharge, token, maxPossibleGas), true);
    }
    ...
    function postRelayedCall(
        ...
        (address payer, uint256 tokenPreCharge, IERC20 token, uint256 maxPossibleGas) = abi.decode(context, (address, uint256, IERC20, uint256));
        ...
        uint256 spentTokens = router.swapTokensForExactETH(ethActualCharge, tokenPreCharge, _tokenAndWrappedPair(address(token)), address(this), block.timestamp)[0];

        // Send rest of tokens to user
        if (spentTokens < tokenPreCharge) {
            require(token.transfer(payer, tokenPreCharge - spentTokens));
        }
    }


    function _readBytes32(bytes memory b, uint256 index) internal pure returns (bytes32 res) {
    ...
    function _tokenAndWrappedPair(address tokenAddress) internal returns (address[] memory) {
        address[] memory tokenAndWrappedPair = new address[](2);

        tokenAndWrappedPair[0] = tokenAddress;
        tokenAndWrappedPair[1] = router.WETH();

        return tokenAndWrappedPair;
    }
