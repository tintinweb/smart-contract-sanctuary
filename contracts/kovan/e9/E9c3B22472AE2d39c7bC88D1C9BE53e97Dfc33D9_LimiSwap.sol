...
contract LimiSwap is KeeperCompatibleInterface, KeeperBase, Ownable {
   ...
   */
  constructor(address keeperRegistery_, ISwapRouter swapRouter_) {
    keeperRegistery = keeperRegistery_;
    swapRouter = swapRouter_;

    _createOrder(0, 0, address(0), address(0), address(0), 0);
  }
  ...
  function createOrder(
    ...
    //Checks
    require(amountIn >= 1e9, "amountIn must be bigger than 1e9");

    //Effects
    address user = msg.sender;
    _createOrder(price, amountIn, tokenIn, tokenOut, user, poolFee);
    ...
    emit OrderCreated(orderIdCounter - 1, price, amountIn, tokenIn, tokenOut, user, poolFee);
  }
  ...
  function performUpkeep(bytes calldata performData) external override onlyKeeper {
    ...
    //Interactions
    _swapExactInputSingle(order.amountIn, order.tokenIn, order.tokenOut, order.user, order.poolFee);
  ...
  function _createOrder(
    ...
    Order memory newOrder = Order(orderIdCounter, price, amountIn, tokenIn, tokenOut, user, poolFee);
  ...
  function _getPrice(
    ...
    amountOut = _swapExactInputSingle(1e9, tokenIn, tokenOut, address(0), poolFee);
  }
