...
contract TimedEscrow {
    ...
    /**
     * @notice Transfers suppported tokens to the sender beneficiary
        ...
        returns (uint256 amountWithdrawn_, uint256 residualAmount_)
    {
        ...
        uint256 accrualDuration = _releaseTime.sub(_startAccrualTime);
        uint256 totalTokens = _beneficiary[beneficiary_].totalAmount;
        uint256 claimedTokens = _beneficiary[beneficiary_].claimedAmount;
        amountWithdrawn_ = totalTokens.mul(timePassed).div(accrualDuration).sub(
            claimedTokens
        );
