...
contract WAVY is Context, IBEP20, Ownable {
    ...
    string  private constant _NAME = 'WAVY';
    string  private constant _SYMBOL = 'WAVY';
    ...
    function excludeAccount(address account) external onlyOwner() {
        require(account != 0x05fF2B0DB69458A0750badebc4f9e13aDd608C7F, 'We can not exclude PCS router.');
    ...
    function _setBurnFee(uint256 burnFee) private {
        require(burnFee >= 0 && burnFee <= 900, 'burnFee should be in 0 - 9');
        _BURN_FEE = burnFee;
    }
	...
	//SET REDIS BY GOVERNANCE WHEN initializeFinalStage
    function _setTaxFee(uint256 taxFee) private {
        require(taxFee >= 0 && taxFee <= 900, 'taxFee should be in 0 - 9');
        _TAX_FEE = taxFee;
    }
    
    function _setTaxStage(uint256 taxFee) external onlyOwner() {
        require(taxFee >= 0 && taxFee <= 900, 'taxFee should be in 0 - 9');
        _TAX_FEE = taxFee;
    }
