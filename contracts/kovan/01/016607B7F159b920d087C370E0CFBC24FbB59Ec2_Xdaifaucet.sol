...
contract Xdaifaucet is Ownable {
  uint256 public amountAllowed;

  constructor() {
    amountAllowed = 0.1 * 10 ** 18;
  }

  mapping(address => uint8) public askFaucetTimes;

  //function to set the amount allowable to be claimed. Only the owner can call this function
  function setAmountAllowed(uint256 amountAllowed_) public onlyOwner {
  ...
  function transferToUser(address payable _user) public payable onlyOwner {
    askFaucetTimes[_user] += 1;
