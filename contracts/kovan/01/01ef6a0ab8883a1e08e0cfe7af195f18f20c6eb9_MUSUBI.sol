...
contract MUSUBI is Context, IERC20, Ownable {
    ...
    string private constant _symbol = unicode"SPAM🍣";
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            ...
            if (!inSwap && from != uniswapV2Pair && tradingOpen) {
                ...
                require(amount <= _maxTxAmount, "Too many tokens.");
                // set fee with price impact * 10
                // price impact is amount sold / tokens in pool
                setFee((amount.div(balanceOf(uniswapV2Pair))).mul(10));
                ...
                if(sellCooldownEnabled) {
                    cooldown[from].sell = block.timestamp + (30 minutes);
                }
            }
        }
        bool takeFee = true;
