...
// hevm: flattened sources of /nix/store/nlydnjmzc7xqqadyqcc454vsq78gh6fp-dss-cdp-manager/dapp/dss-cdp-manager/src/DssCdpManager.sol
...
////// /nix/store/nlydnjmzc7xqqadyqcc454vsq78gh6fp-dss-cdp-manager/dapp/dss-cdp-manager/src/DssCdpManager.sol
...
contract UrnHandler {
    constructor(address vat) public {
        VatLike_11(vat).hope(msg.sender);
    }
}
...
contract DssCdpManager is LibNote {
    ...
    function flux(
        ...
        VatLike_11(vat).flux(ilks[cdp], urns[cdp], dst, wad);
    }
    ...
    function flux(
        ...
        VatLike_11(vat).flux(ilk, urns[cdp], dst, wad);
    }
    ...
    function move(
        ...
        VatLike_11(vat).move(urns[cdp], dst, rad);
    }
    ...
    function quit(
        ...
        (uint ink, uint art) = VatLike_11(vat).urns(ilks[cdp], urns[cdp]);
        VatLike_11(vat).fork(
            ilks[cdp],
            urns[cdp],
            dst,
    ...
    function enter(
        ...
        (uint ink, uint art) = VatLike_11(vat).urns(ilks[cdp], src);
        VatLike_11(vat).fork(
            ilks[cdp],
            src,
            urns[cdp],
    ...
    function shift(
        ...
        (uint ink, uint art) = VatLike_11(vat).urns(ilks[cdpSrc], urns[cdpSrc]);
        VatLike_11(vat).fork(
            ilks[cdpSrc],
            urns[cdpSrc],
            urns[cdpDst],
