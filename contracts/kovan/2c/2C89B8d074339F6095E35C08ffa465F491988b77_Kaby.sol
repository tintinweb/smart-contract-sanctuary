...
contract Kaby is IKaby, ERC721, AcceptedToken, ReentrancyGuard {
    ...
    uint256 public marketFeeInBps = 22;
    uint256 public serviceFeeInToken = 1e20;
    uint256 public maxStar = 10;
    string private _uri;
    ...
    modifier onlyStakingPool() {
        require(getStakingPool() == msg.sender, "Kaby: not Staking pool");
        _;
    }
     ...
     */
    function setEquipmentContract(IEquipment equipmentAddress)
        external
        onlyOwner
    {
    ...
    function claimHero(uint256 versionId, uint256 amount) external override {
        ...
        if (
            version.startingIndex == 0 &&
            (version.currentSupply == version.maxSupply ||
                block.timestamp >= version.revealTime)
        ) {
    ...
    function getHero(uint256 heroId)
        ...
        view
        override
        returns (uint256 star, uint256[5] memory equipment)
    {
        Hero memory hero = _heros[heroId];
        star = hero.star;
        equipment = [hero.gem1, hero.gem2, hero.gem3, hero.gem4, hero.gem5];
    }
     ...
     */
    function _createHero() private returns (uint256 heroId) {
        _heros.push(Hero(1, 0, 0, 0, 0, 0));
        heroId = _heros.length - 1;
        emit HeroCreated(heroId);
    }

    function _setHeroEquipment(
        uint256 heroId,
        ...
        require(
            herosOnSale[heroId] == 0,
            "Kaby: cannot change items while on sale"
        );
        ...
        if (isRemove == false) {
            for (uint256 i = 0; i < itemIds.length; i++) {
                ...
                if (hero.gem1 == 0) {
                    ...
                    require(false, "Kaby: invalid space");
                }
            }
        } else {
            for (uint256 i = 0; i < itemIds.length; i++) {
                ...
                if (hero.gem1 == itemId) {
                    ...
                    require(false, "Kaby: invalid item for remove");
                }
            }
        }
    }

    function _finalizeStartingIndex(uint256 versionId, Version storage version)
        private
    {
        uint256 startingIndex = uint256(
