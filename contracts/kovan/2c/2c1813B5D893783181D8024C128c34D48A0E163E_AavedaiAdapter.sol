...
contract AavedaiAdapter is AdapterBase {
    ...
    function getdaiallowanceforxfer() external view returns (uint256) {
        //bool isApproved = _underlyingAsset.approve(address(this), 50 ether);
        //require(isApproved, "token not approve for transfer");
        uint256 allowance =
            _underlyingAsset.allowance(msg.sender, address(this));
    ...
    function redeemfiftydai() external {
        ...
        require(balance >= 50 ether, "not enough token to redeem");
        _underlyingAsset.safeTransfer(msg.sender, 50 ether);
    }

    function approveUnderlyingToken(uint256 amount) external {
        bool isApproved = _underlyingAsset.approve(address(this), amount);
        require(isApproved, "token not approve for transfer");
        uint256 allowance =
            _underlyingAsset.allowance(msg.sender, address(this));
        require(allowance >= amount, "Check the token allowance");
