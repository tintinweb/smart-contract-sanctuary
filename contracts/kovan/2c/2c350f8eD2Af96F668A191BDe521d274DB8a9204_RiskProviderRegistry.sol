 ...
 * they want that needs to be in the range [-10, 10].
 */
contract RiskProviderRegistry is IRiskProviderRegistry, Ownable {
    /* ========== STATE VARIABLES ========== */

    // Association of a risk provider to a strategy and finally to a risk score [-10, 10]
    mapping(address => mapping(address => int8)) private _risk;
    ...
    /**
     * @dev Returns whether a particular address is a risk provider.
     */
    ...
    /**
     * @dev Returns the risk score of a particular strategy as defined by
    ...
    /**
     * @dev Allows the risk score of a strategy to be set.
     *
     ...
     */
    function setRisk(address strategy, int8 risk) external {
        require(
            isProvider(msg.sender),
            "RiskProviderRegistry::setRisk: Insufficient Privileges"
        );
        _risk[msg.sender][strategy] = risk;

        emit RiskAssessed(msg.sender, strategy, risk);
    }

    /* ========== RESTRICTED FUNCTIONS ========== */

    /**
     * @dev Allows the inclusion of a new provider to the registry.
     *
    ...
    /**
     * @dev Allows the removal of an existing provider to the registry.
     *
