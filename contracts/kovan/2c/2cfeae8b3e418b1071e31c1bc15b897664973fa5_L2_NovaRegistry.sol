...
contract L2_NovaRegistry is ReentrancyGuard {
    ...
    /// @notice Maps execHashes to the address of the task associated with the request.
    mapping(bytes32 => address) public requestTasks;
    /// @notice Maps execHashes to the calldata associated with the request.
    mapping(bytes32 => bytes) public requestCalldatas;
    /// @notice Maps execHashes to the gas limit a bot should use to execute the request.
    mapping(bytes32 => uint256) public requestGasLimits;
    /// @notice Maps execHashes to the gas price a bot must use to execute the request.
    mapping(bytes32 => uint256) public requestGasPrices;
    /// @notice Maps execHashes to the input tokens a bot must have to execute the request.
    mapping(bytes32 => InputToken[]) public requestInputTokens;
    /// @notice Maps execHashes to the 'bounty' tokens a bot will recieve for executing the request.
    mapping(bytes32 => Bounty[]) public requestBounties;
    /// @notice Maps execHashes to the creator of each request.
    mapping(bytes32 => address) public requestCreators;

    /// @dev Maps execHashes to a boolean indicating whether the tokens (inputs/bounties) have been removed, either via a bot successfully executing the request, a withdrawal, or a bump.
    mapping(bytes32 => bool) internal requestTokensRemoved;

    /// @dev Maps execHashes to a timestamp representing when the request will be disabled/enabled.
    /// @dev If the timestamp is negative it means the request will be disabled at (the absolute value of) that timestamp.
    /// @dev If the timestamp is positive it means the request will be enabled at that timestamp.
    mapping(bytes32 => int256) internal requestDisableOrEnableTimestamps;

    /// @notice Emitted when `requestExec` is called.
    event RequestCreated(bytes32 indexed execHash, address indexed task);

    /// @notice Emitted when an update to `requestDisableOrEnableTimestamps` is made.
    event RequestEnableOrDisableTimeoutStarted(
        bytes32 indexed execHash,
        int256 timestamp
    );

    /// @notice Emitted when an update to `requestTokensRemoved` is made.
    event RequestTokensRemoved(bytes32 indexed execHash);
    ...
    function requestExec(
        ...
        emit RequestCreated(execHash, task);
    }
    ...
    function cancel(bytes32 execHash, uint256 withdrawDelaySeconds) public {
        ...
        require(
            requestDisableOrEnableTimestamps[execHash] == 0,
            "ALREADY_CANCELED"
        );

        require(withdrawDelaySeconds >= 300, "DELAY_TOO_SMALL");

        // Set the delay timestamp to int(current timestamp + the delay)
        int256 timestamp = int256(block.timestamp + withdrawDelaySeconds);
        requestDisableOrEnableTimestamps[execHash] = timestamp;

        emit RequestEnableOrDisableTimeoutStarted(execHash, timestamp);
    }

    function withdraw(bytes32 execHash) external nonReentrant {
        require(
            int256(block.timestamp) >
                requestDisableOrEnableTimestamps[execHash],
            "DELAY_NOT_PASSED"
        );
        ...
        require(!requestTokensRemoved[execHash], "TOKENS_ALREADY_REMOVED");
        ...
        emit RequestTokensRemoved(execHash);
    }
    ...
    function isDisabled(bytes32 execHash)
        external
        view
        returns (bool isDisabled, int256 disableTimestamp)
    {
        // TODO
    }
