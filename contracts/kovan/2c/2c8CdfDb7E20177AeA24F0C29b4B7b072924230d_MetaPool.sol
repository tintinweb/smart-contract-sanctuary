...
contract MetaPool is IUniswapV3MintCallback, IUniswapV3SwapCallback, ERC20 {
  ...
  uint128 public immutable desiredLiquidityRatio;
  ...
  /// @param _desiredLiquidityRatio Default ratio of wide liquidity to tight liquidity
  constructor(
    ...
    uint128 _desiredLiquidityRatio
  ) {
    pool = _pool;
    token0 = _pool.token0();
    token1 = _pool.token1();
    ...
    desiredLiquidityRatio = _desiredLiquidityRatio;
  ...
  /// @notice Returns the current liquidity ratio between the tight & wide pools (with 18 decimal points)
  /// @return ratio The current liquidity ratio
  function currentLiquidityRatio() external view returns (uint256 ratio) {
    (uint128 tightLiquidity, , , , ) = pool.positions(tightPositionID);
    (uint128 wideLiquidity, , , , ) = pool.positions(widePositionID);
    ratio = uint256(tightLiquidity).mul(1e18) / wideLiquidity;
  }
  ...
  function initialize() external {
    ...
    pool.mint(
      ...
      tightLowerTick,
      tightUpperTick,
      100 * desiredLiquidityRatio, // Won't overflow, since we assume reasonable desiredLiquidityRatio
      abi.encode(msg.sender) // Data field for uniswapV3MintCallback
    );
