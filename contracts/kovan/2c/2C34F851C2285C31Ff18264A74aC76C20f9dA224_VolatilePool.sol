...
// 
contract VolatilePool is Ownable, ReentrancyGuard, ERC20, Pausable {
	using SafeERC20 for IERC20;

	// Internal token decimals 
	uint8 private tokenDecimals;

	// ReservePool contract address where contract will send inTokens
	address public reservePool;
	// VPStorage contract address allowed to burn VPT without approval
	address public vpStorage;
	// Contract address of the Token contract accepts
	IERC20 public inToken;
	// Coefficients precision (parts per million, ppm)
	uint256 constant public COEFFICIENT_PRECISION = 1e6;
	// Coefficient in ppm (parts per million)
	uint256 public coefficient;

	/** 
	* @dev Lender constructor
	* @param decimals_ VPT Token decimals
	...
	* @param reservePool_ ReservePool contract address
	* @param vpStorage_ - VPStorage contract address
	*/
	function setReservePoolVPStorage (
		...
		onlyOwner
	{
		require (reservePool == address(0), "reservePool, vpStorage addresses are already set");
	...
	****************************************/
	...
	/**
	 * @dev Destroys tokens from account, if msg.sender is not VPStorage allowance checked
	 * @param account_ to burn from
	...
	****************************************/
	...
	/**
	 * @dev Overrides ERC20 allowance(), returns infinite allowance for VPStorage
	 */
