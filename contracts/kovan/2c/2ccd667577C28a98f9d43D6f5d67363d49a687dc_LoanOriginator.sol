...
contract LoanOriginator is ILoanOriginator {
    ...
    modifier onlyAdmins {
        require(IManagementCompany(MCBoard).isMCAdmin(msg.sender) == true, "ERR:LO001 ONLY ADMIN");
        _;
    }

    modifier onlyMC {
        require(msg.sender == MCBoard, "ERR:LO002 ONLY MCSC");
        _;
    }

    modifier onlySPV {
        require(msg.sender == IManagementCompany(MCBoard).SPVWalletAddress(), "ERR:LO003 ONLY SPV");
        _;
    }
    ...
    function createLoanPool(uint rate1, uint rate2, uint utilizationLimit, address currency, string calldata loanPoolName) public override onlyAdmins {
        require(currency != address(0), 'ERR:LO004 CURRENCY IS 0x0');
        (uint _minRate, uint _maxRate) = rate1 < rate2 ? (rate1, rate2) : (rate2, rate1);
        require(getLoanPoolIDByParams[_minRate][_maxRate][currency] == 0, 'ERR:LO005 LOANPOOL EXISTS');
        address _LRSCAddress = IManagementCompany(MCBoard).LRSCAddress();
        require(_LRSCAddress != address(0), "ERR:LO006 LRSC IS 0x0");
        ...
        ILoanPool(loanPool).initialize(_minRate, _maxRate, utilizationLimit, currency, MCBoard, nextLoanPoolID, loanPoolName);
    ...
    function closeLoanPool(uint loanPoolID) external override onlyAdmins {
        ...
        require(_pool != address(0), "ERR:LO007 LPSC IS 0x0");
    ...
    function deposit(uint amount, uint loanPoolID) external override {
        address _pool = getLoanPoolByID[loanPoolID];
        require(_pool != address(0), "ERR:LO007 LPSC IS 0x0D");
    ...
    // drawFund and payLoan function designed for MC Contract
    function payLoan(uint amount, uint loanPoolID, uint loanEntity) external onlyMC override {
        ...
        require(_LRSCAddress != address(0), "ERR:LO006 LRSC IS 0x0");
        require(
            ILandRegistration(_LRSCAddress).getLandAppraisalAmount(_landID) != 0,
            "ERR:LO008 LAND MUST NOT VOIDED"
        ...
        // update amount borrowed by developer both in land struct and developer struct
        address _currency = ILoanPool(_pool).currency();
        uint _decimals = ERC20(_currency).decimals();
        ILandRegistration(_LRSCAddress).updateAmountBorrowedByDeveloper(_landID, _loanDeduction, _decimals, false, false);
    }
    ...
    function drawFund(uint amount, uint loanPoolID, uint landID, uint closeDate, string calldata projectDescription) external onlySPV override {
        ...
        address _LRSCAddress = IManagementCompany(MCBoard).LRSCAddress();
        require(_pool != address(0), "ERR:LO007 LPSC IS 0x0");
        require(_spvwallet != address(0),   "ERR:LO009 SPV IS 0x0");
        require(_LRSCAddress != address(0), "ERR:LO006 LRSC IS 0x0");
        require(closeDate >= block.timestamp, "ERR:LO010 CLOSE DATE MUST BE FUTURE");
        require(bytes(projectDescription).length > 0, "ERR:LO011 DESCRIPTION MUST NOT EMPTY");
         // check land and developer validity
        uint _developerID = ILandRegistration(_LRSCAddress).getDeveloperIDByLandID(landID);
        require(
            ILandRegistration(_LRSCAddress).isDeveloperIDValid(_developerID) == true,
            "ERR:LO012 INVALID DEVELOPER ID"
        );
        require(
            ILandRegistration(_LRSCAddress).isLandIDValid(landID) == true,
            "ERR:LO013 INVALID LANDID"
        );

        // update amount borrowed by developer both in land struct and developer struct
        address _currency = ILoanPool(_pool).currency();
        uint _decimals = ERC20(_currency).decimals();
        ILandRegistration(_LRSCAddress).updateAmountBorrowedByDeveloper(landID, amount, _decimals, true, false);
        // spv draw fund from exisitng pool
        ILoanPool(_pool).drawFund(landID, _spvwallet, amount, closeDate, projectDescription);
        
        emit LandDrawFund(loanPoolID, landID, closeDate, amount, projectDescription);
    }

    // if SPV already know which loan entity to repay the debt
    function debtVoid(uint payableDebtAmount, uint loanPoolID, uint loanEntity) external onlyMC override {
        address _pool = getLoanPoolByID[loanPoolID];
        uint _landID = ILoanPool(_pool).getloanEntityLandID(loanEntity);
        address _LRSCAddress = IManagementCompany(MCBoard).LRSCAddress();
        require(_LRSCAddress != address(0), "ERR:LO006 LRSC IS 0x0");
        ...
        address _currency = ILoanPool(_pool).currency();
        uint _decimals = ERC20(_currency).decimals();
        ILandRegistration(_LRSCAddress).updateAmountBorrowedByDeveloper(_landID, _amountDeduction, _decimals, false, false);

        emit LoanDebtVoid(loanPoolID, loanEntity, payableDebtAmount);
    }
    ...
    function getLoanPoolInfoByID(uint _pool) external override view returns (string memory, uint, uint, uint, uint, uint, uint, address, ILoanPool.poolStatus) {
...
interface ILoanPool {
   ...
   function getLoanPoolInfo() external view returns (string memory, uint, uint, uint, uint, uint, uint, address, poolStatus);
...
interface ILoanOriginator {
   ...
   function getLoanPoolInfoByID(uint poolID) external view returns (string memory, uint, uint, uint, uint, uint, uint, address, ILoanPool.poolStatus);
}
...
interface ILandRegistration {
    ...
    function updateAmountBorrowedByDeveloper(uint _landID, uint _amount, uint _decimals, bool isAdd, bool isDebtVoid) external;
...
contract LoanPool is ILoanPool, IERC20 {
    ...
    uint public poolDebt;        // total withdrawed fund
    ...
    uint[] public activeLoanEntityIDs;     // current active loan entities
    mapping(uint => LoanEntity) getLoanEntityByID;  // from loanEntityID -> LoanEntity struct
    mapping(uint => uint) public indexOfLoanEntity;        // from loanEntityID -> index in activeLoanEntity array
    mapping(uint => bool) public isLoanEntityIDValid;      // check if loan is valid or not
    ...
    modifier onlyLOSC() {
        require(msg.sender == LOSCAddress, "ERR:LP001 ONLY LOSC");
        _;
    }

    modifier onlyPoolOpen() {
        require(status == poolStatus.Opening, "ERR:LP002 CLOSED");
        _;
    }
    ...
    function initialize(uint _minRate, uint _maxRate, uint _utilizationLimitRate, address _currency, address _MCBoard, uint _myId, string calldata _loanPoolName) external override onlyLOSC {
        minRate = _minRate;
        maxRate = _maxRate;
        ...
        // set pool ERC20
        _name = _loanPoolName;
        _symbol = _loanPoolName;
        _decimals = ERC20(_currency).decimals();

        emit LoanPoolInitialized(minRate, maxRate, utilizationLimitRate, currency, MCBoard, myId, loanPoolName);
    }
    ...
    function refreshAllLoansByLandID(uint landID) external override returns (uint totalDebt) {
        ...
        for (uint i = 1; i < length; i++){
            ...
            uint256 newInterestGenerated =  _loan.principal.mul(_loan.interestRateAPY)
                                            .mul(block.timestamp.div(1 days) - _loan.lastUpdateDate)
                                            .div(DAYSPERYEAR).div(DECIMALBASE);
    ...
    function deposit(address from, uint amount) external override onlyLOSC onlyPoolOpen {
        ...
        require(
            ...
            "ERR:LP004 INSUFFICIENT FUND"
        );
        require(
            IERC20(currency).allowance(from, address(this)) >= amount,
            "ERR:LP005 INSUFFICIENT ALLOWANCE"
        );
        ...
        rateToBorrow = minRate.add(currentUltilizationRate.mul(maxRate.sub(minRate)).div(DECIMALBASE)); // update rate to borrow
    ...
    function drawFund(uint landID, address spvwallet, uint amount, uint closeDate, string calldata projectDescription) external override onlyLOSC onlyPoolOpen {
        ...
        require(
            ...
            "ERR:LP008 UTILIZATION RATE LIMIT"
        );
    ...
    function payLoan(uint amount, uint loanEntityID, address managementCompany) external override onlyLOSC onlyValidLoanEntity(loanEntityID) returns (uint loanDeduction) {
        require(
            ...
            "ERR:LP009 INVALID LOANENTITYID"
        );
        ...
        // update update interest obligated
        uint256 newInterestGenerated =  _loan.principal.mul(_loan.interestRateAPY)
                                        .mul(block.timestamp.div(1 days) - _loan.lastUpdateDate).div(DAYSPERYEAR).div(DECIMALBASE);
        if(newInterestGenerated > 0) {
            _loan.lastUpdateDate = block.timestamp.div(1 days);
            _loan.interestObligated = _loan.interestObligated.add(newInterestGenerated);
        }
        ...
        if (isOverPaid == true){
            ...
            getLoanEntityIDsByLandID[_loan.landID][_indexOfRemovedLoanEntity] = _lastLoanEntityID;
            ...
            address _LRSCAddress = IManagementCompany(MCBoard).LRSCAddress();
            ILandRegistration(_LRSCAddress).removeUniqueLoanEntityId(_loan.developerID, myId,loanEntityID);
        } else {
            if (amount >= _loan.interestObligated) {
                ...
                _loan.interestObligated = 0;    // reset interest generated.
            } else {
        ...
        emit MCRepayed(address(this), managementCompany, currency, amount);
    }
    ...
    function debtVoid(uint loanEntityID, uint payableDebtAmount, address managementCompany) external override onlyLOSC onlyValidLoanEntity(loanEntityID) returns (uint loanDeduction){
        require(
            payableDebtAmount <= IERC20(currency).balanceOf(managementCompany), 
            "ERR:LP010 INSUFFICIENT ASSET"
        );
    ...
    function createLoanEntity(uint landID, uint developerID, uint principal, uint closeDate, string calldata projectDescription) internal{
        ...
        address _LRSCAddress = IManagementCompany(MCBoard).LRSCAddress();
        _loan.projectType = ILandRegistration(_LRSCAddress).getLandRezoningInfo(landID);
        ...
        ILandRegistration(_LRSCAddress).addUniqueLoanEntityId(_loan.developerID, myId, _loan.loanEntityID);
    }
    ...
    function _fromCurrencyToTokenCalculation(uint amountOfCurrency) internal returns (uint256) {
        ...
        for(uint i = 1; i < activeLoanEntityIDs.length; i++) {
            ...
            uint newInterestGenerated = 
                loanEntity.principal.mul(loanEntity.interestRateAPY)
                .mul(block.timestamp.div(1 days).sub(loanEntity.lastUpdateDate)).div(DAYSPERYEAR).div(DECIMALBASE);
            if(newInterestGenerated > 0) {    
    ...
    function getLoanPoolInfo() external view override returns (string memory, uint, uint, uint, uint, uint, uint, address, poolStatus) {
        return ( 
                loanPoolName, 
                poolDebt, 
                poolReserve, 
    ...
    function getLoanEntityViewByLoanEntityID(uint _loanID) external view override returns (
      ...
      string memory projectType
        ...
        loanPoolName_ = getLoanEntityByID[_loanID].loanPoolName;
        landID = getLoanEntityByID[_loanID].landID;
        address _LRSCAddress = IManagementCompany(MCBoard).LRSCAddress();
        uint _developerID = ILandRegistration(_LRSCAddress).getDeveloperIDByLandID(landID);
        borrower = ILandRegistration(_LRSCAddress).getDeveloperNameByID(_developerID);
        collateral = ILandRegistration(_LRSCAddress).getCollateralAddressByID(landID);
    ...
    function getLoanEntityPaymentInfoByLoanEntityID(uint _loanID) external view override returns (
        uint principal,
        uint interestObligated,
        uint totalPaid
    ) {
        principal = getLoanEntityByID[_loanID].principal;
        interestObligated = getLoanEntityByID[_loanID].interestObligated;
        totalPaid = getLoanEntityByID[_loanID].totalPaid;
    }

    function getLoanEntityStatus(uint _loanID) external view override returns (bool loanStatus) {
        loanStatus = getLoanEntityByID[_loanID].status;
    }   
     
    ///@notice necessary ActiveLoanEntityIDs getter function for iterable struct mapping
    function getActiveLoanEntityIDs() external view returns(uint[] memory result) {
        return activeLoanEntityIDs;
    }
}
