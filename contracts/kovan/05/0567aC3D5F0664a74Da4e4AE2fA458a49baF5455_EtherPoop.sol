    ...
    PaymentSplitterUpgradeable
{
    ...
    function buyPoop (uint poopNumber) external payable {
        ...
        safeTransferFrom(ownerOf(poopNumber), _msgSender(), poopNumber);
        payable(ownerOf(poopNumber)).transfer(poops[poopNumber].price);
    }
    
    function sellPoop (uint poopNumber, uint price) external {
    ...
    function dontSellPoop (uint poopNumber) external {
        require(msg.sender == ownerOf(poopNumber), "sellPoop: Only poop owner can cancel sell");
        poops[poopNumber].currentlyForSale = false;
    }
