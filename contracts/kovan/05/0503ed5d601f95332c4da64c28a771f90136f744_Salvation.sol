...
contract Salvation is Context, IERC20, Ownable {
    ...
    event NewStratBurnTax(uint tax);
    event NewStratHoldersTax(uint tax);
    event NewStratLiquidityTax(uint tax);
    event UpgradeStrat(uint tax);
    ...
    function proposeBurnTax(uint _tax) public {
        require(msg.sender == strategistOnly);
