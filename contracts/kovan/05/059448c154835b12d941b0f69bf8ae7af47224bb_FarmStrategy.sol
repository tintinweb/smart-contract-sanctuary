...
abstract contract FarmStrategyStorage is StrategyAdminStorage, IStrategy {
    ...
    bool public override isStrategy;
}
...
contract FarmStrategy is FarmStrategyStorage{
    ...
    function withdraw(address[] memory tokens, uint256 amount) external override returns (uint256, address[] memory, uint256[] memory){
        ...
        if(balance >= tokenAmount){
            ...
            (uint256 _caculatedLpAmount,uint256 _lpTokenAmount) = caculateLPTokenAmount(token, tokenAmount.sub(balance)); //733263988540527
            if(_lpTokenAmount == 0){
                withdrawTokens[0] = token;
                amounts[0] = balance;
                tokenAmount = balance;
            }else{
                lpTokenAmount = lpTokenAmount.sub(_lpTokenAmount,"wrong lpTokenAmount");
                ...
                if(token == token0){
                    ...
                    uint256 needAmount0;
                    uint256 needAmount1;
                    if(_caculatedLpAmount > _lpTokenAmount){
                        tokenAmount = amount0.mul(token0Price).add(amount1.mul(token1Price)).div(tokenPrice);
                        needAmount0 = amount0;
                        needAmount1 = amount1;
                    }else {
                        uint256 tokenAmountInUSD = tokenAmount.mul(tokenPrice);
                        needAmount0 = tokenAmountInUSD.div(token0Price).div(2);
                        needAmount1 = tokenAmountInUSD.div(token1Price).div(2);
                        require(needAmount0 > 0 && needAmount1 > 0 && needAmount0 <= amount0 && needAmount1 <= amount1, "Wrong amount");
                    }
            ...
            if(otherToken == profit) otherTokenBalance = otherTokenBalance.sub(profitAmount.sub(reinvestmentAmount));
            if(otherTokenAmount == 0){
                tokenAmount = 0;
            }
            else if(otherTokenAmount > otherTokenBalance){
                tokenAmount = otherTokenBalance.mul(tokenReserve).div(otherTokenReserve);
                otherTokenAmount = tokenAmount == 0? 0 : otherTokenBalance;
            } 
    ...
    function reinvestment(address[] memory pools, address[] memory tokens, uint256[] memory amounts) external override{
        ...
        require(reinvestmentAmount <= profitAmount,"Invalid amount");
    ...
    function getProfitAmount() view external override returns (address[] memory tokens, uint256[] memory amounts, uint256[] memory pendingAmounts){
        tokens = new address[](1);
        amounts = new uint256[](1);
        pendingAmounts = new uint256[](1);
        tokens[0] = profit;
        amounts[0] = profitAmount.sub(reinvestmentAmount);
        pendingAmounts[0] = IFarm(farm).pending(pid, address(this));
    }
    ...
    function caculateLPTokenAmount(address token, uint256 amount) internal view returns (uint256,uint256){
        ...
        uint256 caculatedLpAmount = amount.mul(totalSupply);
        caculatedLpAmount = caculatedLpAmount.div(tokenBalance).add(1);
        uint256 _lpTokenAmount= caculatedLpAmount;
        if(_lpTokenAmount > lpTokenAmount || lpTokenAmount.sub(_lpTokenAmount) < lpLimit) _lpTokenAmount = lpTokenAmount;
        if(_lpTokenAmount == 0) return (caculatedLpAmount,0);
        if(_lpTokenAmount < lpLimit) _lpTokenAmount = lpLimit;
        return (caculatedLpAmount,_lpTokenAmount);
    }
