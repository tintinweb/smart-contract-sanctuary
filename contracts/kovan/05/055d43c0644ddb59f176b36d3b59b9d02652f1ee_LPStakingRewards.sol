    ...
    ReentrancyGuardUpgradeable
{
    ...
    event LPRewardDurationUpdated(address indexed lpToken, uint256 rewardDuration);
    event LPRewardsEndUpdated(address indexed lpToken, uint256 rewardsEndBlock);
    ...
    function stake(address lpToken, uint256 amount) external override {
        ...
        require(
            poolInfo[lpToken].lpToken.balanceOf(user) >= amount,
            "Not Enough balance to stake"
        );
        // TODO: add test case for isValidLP[lpToken]: added lpToken give true? Otherwise false?
        // require(isValidLP[lpToken], "LP not in Flurry whitelist");
        updateReward(user, lpToken);
        userInfo[lpToken][user].amount = userInfo[lpToken][user].amount.add(
            amount
        );
        poolInfo[lpToken].totalStakes = poolInfo[lpToken].totalStakes.add(amount);
        poolInfo[lpToken].lpToken.safeTransferFrom(user, address(this), amount);

        emit Staked(_msgSender(), lpToken, amount);
    ...
    function rewardsPerToken(address lpToken) public view override returns (uint256){
        ...
        return
            poolInfo[lpToken].rewardPerToken.add(
                (
                    lastBlockApplicable(lpToken).sub(
                        poolInfo[lpToken].lastUpdateBlock
                    )
                )
                    .mul(stakingRate)
                    .mul(poolInfo[lpToken].stakeTokenOne)
                    .mul(poolInfo[lpToken].allocPoint)
                    .div(poolInfo[lpToken].totalStakes)
                    .div(totalAllocPoint)
            );
    }
    ...
    function _earned(address user, address lpToken) internal view returns (uint256) {
        ...
        return super._earned(_user.amount, rewardsPerToken(lpToken).sub(_user.rewardPerTokenPaid), poolInfo[lpToken].stakeTokenOne, _user.reward);
    }
    ...
    function _withdrawUser(address user, address lpToken, uint256 amount) internal {
        // require(isValidLP[lpToken], "LP not in Flurry whitelist");
        ...
        updateReward(user, lpToken);
        userInfo[lpToken][user].amount = userInfo[lpToken][user].amount.sub(amount);
        poolInfo[lpToken].totalStakes = poolInfo[lpToken].totalStakes.sub(amount);
    ...
    function claimReward(address user, address lpToken) external override {
        ...
        if (userInfo[lpToken][user].reward > 0) {
            ...
            userInfo[lpToken][user].reward = IStakingRewards(stakingRewardsAddr).claimLPTokenReward(user, userInfo[lpToken][user].reward);
        }
    }
    ...
    function addLP(address lpToken, uint256 allocPoint, uint256 rewardDuration) public onlyRole(DEFAULT_ADMIN_ROLE) {
        totalAllocPoint = totalAllocPoint.add(allocPoint);

        uint256 currentBlock = block.number;
        uint256 stakeTokenOne = 10 ** IERC20MetadataUpgradeable(lpToken).decimals();

        isValidLP[lpToken] = true;
        ...
        poolInfo[lpToken] = PoolInfo({
            ...
            rewardPerToken: 0,
            rewardsEndBlock: currentBlock.add(rewardDuration),
            rewardDuration: rewardDuration,
            stakeTokenOne: stakeTokenOne
        });
        emit LPAdded(lpToken);
    }

    // Update the given pool's allocation point. Can only be called by the owner.
    function setLP(address lpToken, uint256 allocPoint) public onlyRole(DEFAULT_ADMIN_ROLE) {
        totalAllocPoint = totalAllocPoint.sub(poolInfo[lpToken].allocPoint).add(allocPoint);
        poolInfo[lpToken].allocPoint = allocPoint;
    }

    function setRewardDuration(address lpToken, uint256 rewardDuration) external override onlyRole(DEFAULT_ADMIN_ROLE) {
        PoolInfo storage pool = poolInfo[lpToken];
        require(
        ...
        pool.rewardDuration = rewardDuration;
        pool.rewardsEndBlock = block.number.add(rewardDuration);
        emit LPRewardDurationUpdated(lpToken, pool.rewardDuration);
        emit LPRewardsEndUpdated(lpToken, pool.rewardsEndBlock);
    }

    function getRewardForDuration(address lpToken) external view override returns (uint256) {
        // require(isValidLP[lpToken], "LP not in Flurry whitelist");
        return _getRewardsForDuration(stakingRate, poolInfo[lpToken].rewardDuration);
    }

    function shortenRewardsDuration(address lpToken) external onlyRole(DEFAULT_ADMIN_ROLE){
        PoolInfo storage pool = poolInfo[lpToken];
        uint256 currentBlock = block.number;
