...
contract BondFactory is IBondFactory, Context {
    ...
    address public immutable iouFactory;
    mapping(bytes32 => address) public bonds;

    constructor(address _target, address _iouFactory) {
        target = _target;
        iouFactory = _iouFactory;
    }
    ...
    function createBond(
        ...
        address[] memory collateralTrancheAddresses = new address[](_tranches.length);
        for (uint256 i = 0; i < _tranches.length; i++) {
            collateralTrancheAddresses[i] = address(_tranches[i].token);
        }
        bytes32 bondHash = keccak256(abi.encodePacked(collateralTrancheAddresses, _maturityDate));
        require(bonds[bondHash] == address(0), "BondFactory: Bond already exists");

        address clone = Clones.clone(target);
        bonds[bondHash] = clone;
        BondController(clone).init(iouFactory, _tranches, _msgSender(), _maturityDate, _totalInterestRate);
