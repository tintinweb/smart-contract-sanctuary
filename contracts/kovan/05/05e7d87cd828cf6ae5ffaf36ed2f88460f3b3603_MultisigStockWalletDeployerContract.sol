...
// File: contracts/Deployers/MultisigStockWalletDeployerContract.sol
...
contract MultisigStockWalletDeployerContract is AccessControl { 
    
    bytes32 public constant MODERATOR_ROLE = keccak256("MODERATOR_ROLE");

    CompanyRegistryInterface private _companyRegistryInstance;
    
    event MultisigStockWalletDeployed(address indexed multisigAddress);
    ...
    // MODERATOR_ROLE
    //
    function deployNewMultisig(uint[] memory voteWeigth, address[] memory participants, address tokenAddress) public returns (address) {
        require (hasRole(MODERATOR_ROLE, msg.sender), "Sender is not moderator!");
        require (voteWeigth.length == participants.length, "Votes does not match the participants!");
        uint parentCompanyId = _companyRegistryInstance.getCompanyIdByToken(tokenAddress);
        BlockhainRegistryInterface registry = BlockhainRegistryInterface(_companyRegistryInstance.getCompaniesRegistry(parentCompanyId));
        
        for (uint i=0; i<participants.length; i++) {
            require(registry.isUserAddressWhitelisted(participants[i]), "One of the participants is not whitelisted!");
        }
        
        address newContractAddress = address(new MultisigStockWalletContract(voteWeigth, participants, tokenAddress));
        _companyRegistryInstance.whitelistContractAddress(address(registry), newContractAddress);

        emit MultisigStockWalletDeployed(newContractAddress);
    ...
    function sanityCheck() public view returns (bool companyRegistrySet, bool registryPermissionSet) {
        ...
        bool registryPermissionSetB = (_companyRegistryInstance.hasRole(keccak256("DEPLOYERS"), address(this)));
