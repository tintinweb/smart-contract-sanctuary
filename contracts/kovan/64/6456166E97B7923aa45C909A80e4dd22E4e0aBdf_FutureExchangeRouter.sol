...
contract FutureExchangeRouter is IFutureExchangeRouter {
    ...
    mapping(address => address[]) listTokensInPair;
    ...
    function getListTokensInPair(address token) external view override returns(address[] memory) {
        return listTokensInPair[token];
    }
    ...
    function addLiquidityFuture(
        ...
        if (futureToken == address(0)) {
            ...
            listTokensInPair[tokenA].push(tokenB);
            listTokensInPair[tokenB].push(tokenA);
        }
        
        uint256 reserveA = IERC20(tokenA).balanceOf(futureToken);
        uint256 reserveB = IERC20(tokenB).balanceOf(futureToken);
        if (reserveA != 0 && reserveB != 0) {
            require(reserveB != PrecogV2Library.quote(amountA, reserveA, reserveB), "Liquidity amount provided invalid");   
        }
