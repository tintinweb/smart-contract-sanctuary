...
contract KingOfWallStreet is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "KingOfWallStreet";
    string private _symbol = "KOWS";
    ...
    uint256 public _lotteryFee = 100;
    ...
    constructor() public {
        // IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x05fF2B0DB69458A0750badebc4f9e13aDd608C7F); // for BSC
    ...
    function _transfer(
        ...
        //if any account belongs to the excludedAccount transfer tokens
    ...
    function collectFee(address account, uint256 amount, uint256 rate) private returns (uint256) {
        ...
        //@dev Lottery fee
        if(_lotteryFee != 0){
            uint256 lotteryFee = amount.mul(_lotteryFee).div(10**(_feeDecimal + 2));
            transferAmount = transferAmount.sub(lotteryFee);
            _reflectionBalance[lotteryWallet] = _reflectionBalance[lotteryWallet].add(lotteryFee.mul(rate));
            if (_isExcluded[lotteryWallet]) {
                _tokenBalance[lotteryWallet] = _tokenBalance[lotteryWallet].add(lotteryFee);
            }
            _lotteryFeeTotal = _lotteryFeeTotal.add(lotteryFee);
            emit Transfer(account,lotteryWallet,lotteryFee);
        }
    ...
    function setMarketingWallet(address account) external onlyOwner {
     ...
     function setLotteryWallet(address account) external onlyOwner {
        lotteryWallet = account;
    }
