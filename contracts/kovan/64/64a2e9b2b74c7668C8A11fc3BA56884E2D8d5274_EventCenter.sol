...
contract EventCenter is Ownable {
    ...
    function addScore(
        ...
        (notOverflow, timeToEpochEnd) = SafeMath.trySub(epochEnd, block.timestamp);
        if(notOverflow == false){
            timeToEpochEnd = 0;
        }
        (notOverflow, postionScore) = SafeMath.tryMul(timeToEpochEnd, amount);
        require(notOverflow == true, "CHFRY: You are so rich!");
        (notOverflow, postionScore) = SafeMath.tryMul(postionScore, tokenWeight);
        require(notOverflow == true, "CHFRY: You are so rich!");
