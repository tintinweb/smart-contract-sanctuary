...
contract BettingPool is Ownable {
    ...
    uint256 private fee;
    uint256 private maxTimeWaitFulfill = 30 * 60;
    ...
    function setMaxTimeWaitFulfill(uint256 _time) external onlyOwner {
        require(_time > 300);
        emit MaxTimeWaitFulfill(maxTimeWaitFulfill, _time);
        maxTimeWaitFulfill = _time;
    }
        ...
        onlyOwner
    {
        for (uint256 i = 0; i < _rewardRateList.length; i++) {
            require(
                _rewardRateList[i]._decimal >= 0 &&
                    _rewardRateList[i]._decimal <= 18
            );
            require(
    ...
    function createNewStandardPrizeBetting(
        ...
        IStandardPrizeBettingContract(betting).setMaxEntrant(_maxEntrant);
    ...
    function createNewGuaranteedPrizeBetting(
        ...
        IGuaranteedPrizeBettingContract(betting).setMaxEntrant(_maxEntrant);
        IERC20(tokenPool).safeTransferFrom(
            msg.sender,
            betting,
    ...
    function setupBettingContract(
        ...
        uint256 _priceValidationTimestamp,
        uint256 _lastBetPlaced
    ) internal onlyBettingOwner(_pool) onlyExistedPool(_pool) returns (bool) {
    ...
    function checkRefund(address _betting)
        external
        view
        onlyExistedPool(_betting)
        returns (bool)
    {
        (bytes32 _resultId, uint256 _timestamp) = IBettingContract(_betting)
            .getResultID();
        if (_timestamp == 0 || _resultId == bytes32(0)) return false;
        if (
            block.timestamp > (_timestamp + maxTimeWaitFulfill) &&
...
interface IBettingContract {
    ...
    function getResultID() external view returns (bytes32, uint256);
...
interface IGuaranteedPrizeBettingContract {
    ...
    function setMaxEntrant(uint256 _maxEntrant)
        external;
...
interface IStandardPrizeBettingContract {
    ...
    function setMaxEntrant(uint256 _maxEntrant) external;
