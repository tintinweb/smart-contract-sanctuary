...
contract CommonsWhitelist is Ownable {
    ...
    function update(address _newDelegateAddress) public onlyWhitelisted {
        require(!whitelist[_newDelegateAddress].whitelisted, "cannot overwrite existing members");
        require(_newDelegateAddress != address(0), "invalid address");

        whitelist[_newDelegateAddress] = whitelist[msg.sender];
