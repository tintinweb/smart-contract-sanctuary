...
contract CryptoPricks is ERC721Enumerable, Ownable, ChainlinkClient {
  ...
  uint32 public secondsInDay = 86_400;
  ...
  */
  ...
  // only owner
  function getVolumeDate() public onlyOwner() {
    requestVolumeData();
  }
 ...
 function whitelistUser(address _user) public onlyOwner {
  ...
  function performUpkeep(bytes calldata /* performData */) external {
    require(_getDateBeforeTimestamp(block.timestamp) >= latestDateChecked + secondsInDay, "Upkeep already performed today");
    running = true;
    requestVolumeData();
  }
}