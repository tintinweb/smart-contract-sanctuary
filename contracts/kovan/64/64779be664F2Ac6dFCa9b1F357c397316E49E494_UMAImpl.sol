...
contract UMAImpl is IProtocol {
    ...
    string public constant _name = "UMA-Impl-v1";
    ...
    constructor(ILongShortPairCreator lspCreator_, ICoveredCallFinancialLibrary coveredCall_, IOptionRegistry optionRegistry_) {
        ...
        require(address(lspCreator_) != address(0x0));
        require(address(coveredCall_) != address(0x0));
    ...
    modifier onlyRegistry() {
        require(msg.sender == address(optionRegistry), "UMAImpl::not-authorized");
        _;
    }
    ...
    // @inheritdoc IProtocol
    function isSupported(bool isCall, address asset, address collateral) public pure override returns (bool) {
    ...
    function collateralBalanceOf(DataTypes.OptionData memory data, address account) external view override returns (uint256) {
        // TODO: modify it for diff collateral decimals
        if (data.isCall && data.collateral == data.underlying) {
            bytes32 lspKey = keccak256(abi.encode(data));
            ILongShortPair lsp = ILongShortPair(lsps[lspKey]);
            ...
            return shortToken.balanceOf(account);
        }
        return 0;
    }
    ...
    function redeem(
        ...
        require(shortTokenBal >= amt, "UMAImpl::insufficient-long-tokens");
    ...
    function settle(
...

        {
            ...
            collateralPayoutRatios[lspKey] = collateralReturned.wdiv(totalCollaterals[lspKey]);
        }
    ...
    function create(
        ...
        DataTypes.Asset memory asset = optionRegistry.assets(data.collateral);

        string memory syntheticName = string(abi.encodePacked(asset.name, " Call Option"));
        string memory syntheticSymbol = string(abi.encodePacked("o", asset.symbol, "-C"));
    ...
    function updatePriceId(address collateral, bytes32 id) external onlyAdmin {
        ...
        require(asset.isActive, "UMAImpl::invalid-collateral");
