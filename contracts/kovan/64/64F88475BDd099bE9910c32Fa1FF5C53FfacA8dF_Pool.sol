...
contract Pool is Ownable {
  ...
  function pickWinner() public returns (address) {
    if (totalAmount > 0) {
       return address(uint160(uint256(sortitionSumTrees.draw(SUM_TREE_KEY, randomNumber))));
    } else {
      return address(0);
    }
  }
 ...
 function initiateRandom(uint256 _seed) public {
      IRandomness(rContract).getRandomNumber(_seed);
  }
  ...
  function getRandomNumber(uint _limit) public returns (uint256) {
    ...
    function _setRandomNumber(uint256 _val) external {
     randomNumber = _val % entryCount+1;
  }
...

{
  ...
  "libraries": {
    "utils/SortitionSumTreeFactory.sol": {
      "SortitionSumTreeFactory": "0x16d0ae9793da018c9576b009a89a1307e4d25064"
    }
  }
}