...
contract TokenFarm is Ownable {
    // mappings: token_address -> staker_address -> amount
    // address -> num_of_unique_tokens
    // address -> chainlink_pricefeed
    ...
    function stakeToken(uint256 _amount, address _token) public {
        // validity checks
        require(_amount > 0, "Invalid staking amount. Must be more than 0.");
        require(_tokenIsAllowed(_token), "Token is currently not allowed.");
        // transfer tokens
    ...
    function unstakeToken(address _token) public {
        ...
        updateStakers(msg.sender);
    }

    function issueTokens() public onlyOwner {
        // Issue tokens to all staker
        for (uint256 stakerIndex; stakerIndex < stakers.length; stakerIndex++) {
            address recipient = stakers[stakerIndex];
            // send a token reward based on total values locked
            uint256 userTotalValue = getUserTotalValue(recipient);
            dappToken.transfer(recipient, userTotalValue);
        }
    }

    function _addAllowedToken(address _token) public onlyOwner {
        for (
    ...
    function _tokenIsAllowed(address _token) public returns (bool) {
        for (
            uint256 allowedTokensIndex;
            allowedTokensIndex < allowedTokens.length;
            allowedTokensIndex++
    ...
    function updateStakers(address _user) public {
    ...
    function setPriceFeedAddress(address _priceFeed, address _token)
        public
        onlyOwner
    {
