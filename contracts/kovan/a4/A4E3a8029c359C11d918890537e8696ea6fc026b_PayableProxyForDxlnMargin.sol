...
import "../../protocol/intf/IWETH.sol";
import "../../protocol/utils/ReentrancyGuard.sol";
import "../../protocol/DxlnMargin.sol";
import "../../protocol/lib/Account.sol";
import "../../protocol/lib/Actions.sol";
import "../../protocol/lib/Require.sol";
import "../helpers/OnlyDxln.sol";

// import "../../protocol/DxlnMargin.sol";

/**
 * Contract for wrapping/unwrapping ETH before/after interacting with DxlnMargin
 */

contract PayableProxyForDxlnMargin is OnlyDxln, ReentrancyGuard {
    // ============ Constants ============

    // bytes32 constant FILEDXLN = "PayableProxyForDxlnMargin";
    ...
    constructor(address dxlnMargin, address payable _WETH)
        OnlyDxln(dxlnMargin)
    {
        // DXLN_MARGIN = DxlnMargin(dxlnMargin);
        WETH = _WETH;
        IWETH(_WETH).approve(dxlnMargin, type(uint256).max);
    }
    ...
    fallback() external payable {
        require(msg.sender == address(WETH), "Cannot receive ETH"); // coverage-disable-line
    }
    ...
    function operate(
        ...
        for (uint256 i = 0; i < actions.length; i++) {
            ...
            require(owner1 == msg.sender, "Sender must be primary account");
        ...
        DXLN_MARGIN.operate(accounts, actions);

        // return all remaining WETH to the sendEthTo as ETH
        uint256 remainingWeth = IWETH(WETH).balanceOf(address(this));
        if (remainingWeth != 0) {
            require(sendEthTo != address(0), "Must set sendEthTo");
...
contract OnlyDxln {
    ...
    DxlnMargin public DXLN_MARGIN;

    // ============ Constructor ============

    constructor(address dxlnMargin) {
        DXLN_MARGIN = DxlnMargin(dxlnMargin);
    }

    // ============ Modifiers ============

    modifier onlyDxln(address from) {
        require(from == address(DXLN_MARGIN), "Only Solo can call function");
        _;
    }
}
