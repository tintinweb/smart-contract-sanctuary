...
import {FlightQueue, LibFlightQueue} from "../libraries/LibFlightQueue.sol";
...
contract KeeperFacet is KeeperCompatibleInterface {
        ...
        returns (bool upkeepNeeded, bytes memory performData)
    {
        ...
        uint256 count = 0;
        while (curr != 0 && block.timestamp >= s.getFlight(curr).nextCheck) {
            count++;
            curr = s.flightQueue.next[curr];
        }
    ...
    function _checkFlight(bytes32 flightId) private {
        require(
            block.timestamp >= s.getFlight(flightId).nextCheck,
            Errors.KEEP_INVALID_PERFORM_DATA
        );
...
import {FlightQueue} from "../libraries/LibFlightQueue.sol";
...
library LibAppStorage {
    ...
    function getPolicy(AppStorage storage s, bytes32 id)
        internal
        view
    ...
    function getFlight(AppStorage storage s, bytes32 id)
        internal
        view
    ...
    function getInsurancePackage(AppStorage storage s, bytes32 id)
        internal
        view
        returns (InsurancePackage storage)
    {
        return s.insurancePackages[id];
    }
}
...
library LibFlightQueue {
    ...
    function addFlightToQueue(FlightQueue storage self, bytes32 id) internal {
        AppStorage storage s = LibAppStorage.appStorage();

        require(!s.getFlight(id).isEmpty(), Errors.FQ_FLIGHT_NOT_INSERTED);
        Flight memory data = s.getFlight(id);
        ...
        if (curr == 0) {
            self.head = id;
            self.tail = id;
        } else if (data.nextCheck <= s.getFlight(curr).nextCheck) {
            self.head = id;
            self.next[id] = curr;

            self.prev[curr] = id;
        } else if (data.nextCheck >= s.getFlight(self.tail).nextCheck) {
            curr = self.tail;
            ...
            while (true) {
                next = self.next[curr];
                if (
                    next == 0 || s.getFlight(next).nextCheck >= data.nextCheck
                ) {
                    break;
                }
    ...
    function popHead(
        ...
        for (uint256 i = 0; i < count; i++) {
            ...
            if (curr == tail) {
                self.tail = curr = 0;
                require(i + 1 >= count, Errors.FQ_COUNT_EXCEEDS_TOTAL);
            } else {
                curr = next;
            }
        }
...
library LibFlight {
    function initialize(
        ...
        require(
            departureTime > block.timestamp && arrivalTime > block.timestamp,
            Errors.FL_INVALID_FLIGHT_TIME
        );

        self.nextCheck = arrivalTime;
    ...
    function addPolicy(
        ...
        require(!self.departed, Errors.FL_ALREADY_DEPARTED);
        require(self.nextCheck == self.arrivalTime, Errors.FL_ALREADY_CHECKED);

        if (self.checkInterval == 0 || delayInterval < self.checkInterval) {
...
import {InsurancePackage, LibInsurancePackage} from "./InsurancePackage.sol";
import {Errors} from "../libraries/Errors.sol";
