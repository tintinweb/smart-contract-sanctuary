...
contract LooksWorker is Ownable, ReentrancyGuard, IWorker {
  ...
  constructor(
    ...
    require(
      ...
      "Worker::initialize:: underlying not match with farm & base token"
    );
  }
  ...
  function getShares() internal view returns (uint256 _shares) {
  ...
  function calculateShares(uint256 amount) internal view returns (uint256) {
    ...
    (uint256 totalAmountStaked, ) = ITokenDistributor(feeSharingSystem.tokenDistributor()).userInfo(address(feeSharingSystem));
    uint256 currentShares;
    uint256 totalShares = feeSharingSystem.totalShares();
  ...
  function health(uint256 id) external view override returns (uint256) {
    // 1. Get the position's LOOKS balance and LOOKS total supply.
    ...
    (uint256 totalAmountStaked, ) = ITokenDistributor(feeSharingSystem.tokenDistributor()).userInfo(address(feeSharingSystem));
    uint256 totalShares = feeSharingSystem.totalShares();
    uint256 userFarmingToken = (totalAmountStaked * _shares) / totalShares;
    uint[] memory amounts = router.getAmountsOut(userFarmingToken, path);
