    ...
    ERC2771Context
{
    ...
    uint256 public constant _jobIntervalSeconds = 1 * 60; // 60 seconds
    ...
    uint256 public _totalRewardsLeft;
    ...
    constructor(
        ...
        for (uint256 i = 0; i < rewards.length; ++i) {
            _totalRewardsLeft += rewards[i];
        }
        require(_totalRewardsLeft == totalRewards, "Reward mismatch");
    }
    ...
    function withdraw() external onlyOwner {
        require(
            (_expirationTime < block.timestamp) || _jobCompleted,
            "Not expired"
        );
        ...
        returns (bool upkeepNeeded, bytes memory performData)
    {
        ...
        if (_daiInstance.balanceOf(address(this)) != _totalRewardsLeft)
            upkeepNeeded = false;
    }
