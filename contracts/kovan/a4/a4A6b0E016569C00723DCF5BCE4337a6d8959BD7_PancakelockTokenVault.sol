...
// File contracts/PancakelockTokenVault.sol

// SPDX-License-Identifier: UNLICENSED

pragma solidity ^0.8.0;

contract PancakelockTokenVault {
    ...
    function init(address _token) external returns (bool) {
        require(data.status == 0, "ALREADY INIT");
        data.factory = msg.sender;
        data.status = 1;
        IERC20(_token).approve(data.factory, ~uint256(0));
        return true;
    }
