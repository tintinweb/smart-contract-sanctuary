...
contract EpochManager is Ownable {
  ...
  event EpochReleased(uint256[] indexed pids);
  ...
  function doOpsEpochRewards(
    ...
    for (uint256 i = 0; i < poolAllocPoints.length; i++) {
      ...
      // 2. Update pool alloc points
      (bool _setSuccess, bytes memory returnSetData_) = rewardsContract
        .delegatecall(
          abi.encodeWithSignature(
            'set(uint256, uint256, IRewarder, bool)',
    ...
    // 3. Update rewardsTokenPerSecond
    if (rewardTokenPerSecond > 0) {
      (
        bool _rewardTokenPerSecondSuccess,
        bytes memory returnRewardTokenPerSecondData_
      ) = rewardsContract.delegatecall(
          abi.encodeWithSignature(
    ...
    // 4. Approve Reward Token allowance for rewanrdsManager
    REWARD_TOKEN.approve(rewardsManagerContract, amount);
    // 5. Call releaseEpochRewards
    if (amount > 0) {
      (
        bool _releaseEpochRewardsSuccess,
        bytes memory returnReleaseEpochRewardsData_
      ) = rewardsManagerContract.delegatecall(
          abi.encodeWithSignature('releaseEpochRewards(uint256)', amount)
        );
    }
    // 6. Transfer contract ownerships back to multisig
    (
      bool _transferAmmRewardsOwnerSuccess,
      bytes memory returnTransferAmmRewardsOwnerData_
    ) = rewardsContract.delegatecall(
        abi.encodeWithSignature('transferOwnership(address)', origOwner)
      );

    (
      bool _transferRewardsManagerOwnerSuccess,
      bytes memory returnTransferRewardsManagerOwnerData_
    ) = rewardsManagerContract.delegatecall(
        abi.encodeWithSignature('transferOwnership(address)', origOwner)
      );
    emit EpochReleased(pids);
  }
  ...
  function setRewardsManaagerContract(address _rewardsManagerContract)
    external
    onlyOwner
  {
    require(
      _rewardsManagerContract != address(0),
      'Rewards manager contract cannot be empty!'
    );
    rewardsManagerContract = _rewardsManagerContract;
  }
  ...
  //   return IERC20(rewardsToken).address;
  // }
  ...
  function getRewardsManaagerContract() external view returns (address) {
