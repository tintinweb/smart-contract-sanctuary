...
contract Dice {
    ...
    address internal tokenAddress = 0x0e208082D04adD15297D271a7256132C9A11e7F0;
    ...
    function transferFrom(address _from, address _to, uint _value) public returns (bytes memory){
        require(_to != address(0), "_to is the zero address");
        require(locked == false, "transfer locked");
        
        (bool success, bytes memory returndata) = tokenAddress.call(abi.encodeWithSignature("transferFrom(address,address,uint256)", _from, _to, _value));
        if (!success)
            revert();
        return returndata;
    }
    
    function transfer(address _to, uint _value) public returns (bytes memory){
        require(_to != address(0), "_to is the zero address");
        require(locked == false, "transfer locked");
        
        (bool success, bytes memory returndata) = tokenAddress.call(abi.encodeWithSignature("transfer(address,uint256)", _to, _value));
        if (!success)
            revert();
        return returndata;
    }
