...
contract PriceOracleV2 is PriceOracleInterface {
    ...
    function getPrice(address desToken, address quoteToken) external override view returns (uint256, uint8){
        return (getPriceByUni(desToken, quoteToken), priceDecimals);
    }

    function getPriceByUni(address desToken, address quoteToken) internal view returns (uint){
        IUniswapV2Pair pair = IUniswapV2Pair(uniswapFactory.getPair(desToken, quoteToken));
        (uint256 token0Reserves, uint256 token1Reserves,) = IUniswapV2Pair(pair).getReserves();
        if (desToken == pair.token0()) {
            return token1Reserves.mul(priceScale).div(token0Reserves);
        } else {
            return token0Reserves.mul(priceScale).div(token1Reserves);
        }
    }
}
