...
contract FatherOfMediators {
    ...
    function cloneMediators(uint n) external isOwner {
        for (uint i=0; i<n; i++) {
    ...
    function poolOf(address token) external view isOwner returns (address[] memory pool, uint amount, uint count) {
        IERC20 erc20 = IERC20(token);
    ...
    function grabERC20(IMediator[] calldata childs, address token) external isOwner returns (uint[] memory result) {
        ...
        for (uint i=0; i<childs.length; i++) {
            result[i] = childs[i].withdrawERC20(token, payout);
        }
    }
...
interface IMediator {
    function withdrawERC20(address token, address payout) external returns (uint);
...
contract Mediator {
    ...
    function withdrawERC20(address token, address payout) external isFather returns (uint) {
        IERC20 erc20 = IERC20(token);
