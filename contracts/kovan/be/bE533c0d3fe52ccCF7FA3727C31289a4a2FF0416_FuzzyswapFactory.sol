...
contract ExternalOracle {
    ...
    function getFee(
        ...
        return uint24(AdaptiveFee.getFee(TWVolatilityAverage, TWVolumePerLiqAverage));
...
library Oracle {
    ...
    function transform(
        ...
        return
            Observation({
                ...
                initialized: true,
                volumePerAvgLiquidity: (averageLiquidity > 0 ? last.volumePerAvgLiquidity + uint128(volume/averageLiquidity) : 0)
            });
    }
    ...
    function observeSingle(
        ...
        if (secondsAgo == 0) {
            ...
            return (last.tickCumulative, last.secondsPerLiquidityCumulative, last.volatilityCumulative, last.volumePerAvgLiquidity);
        }
        ...
        if (target == beforeOrAt.blockTimestamp) {
            ...
            return (beforeOrAt.tickCumulative, beforeOrAt.secondsPerLiquidityCumulative, beforeOrAt.volatilityCumulative, beforeOrAt.volumePerAvgLiquidity);
        } else if (target == atOrAfter.blockTimestamp) {
            // we're at the right boundary
            return (atOrAfter.tickCumulative, atOrAfter.secondsPerLiquidityCumulative, atOrAfter.volatilityCumulative, atOrAfter.volumePerAvgLiquidity);
        } else {
            // we're in the middle
            ...
            return (
                ...
                beforeOrAt.volatilityCumulative +
                    ...
                    targetDelta,
                beforeOrAt.volumePerAvgLiquidity +
                    ((atOrAfter.volumePerAvgLiquidity - beforeOrAt.volumePerAvgLiquidity) / observationTimeDelta) *
                    targetDelta
            );
        }
    }
        ...
        internal view returns (uint112 TWVolatilityAverage, uint256 TWVolumePerLiqAverage)
    {
        ...
        if (lte(time, oldest.blockTimestamp, time - TIME_AGO)){
            ...
            (,,uint112 volatilityBefore, uint256 volumePerLiqBefore) = observeSingle(
                self,
                time,
                TIME_AGO,
                tick,
            ...
            (,,uint112 volatilityAfter, uint256 volumePerLiqAfter) = observeSingle(
                self,
                time,
                0,
                tick,
            ...
            return ((volatilityAfter - volatilityBefore) / TIME_AGO, (volumePerLiqAfter - volumePerLiqBefore) / TIME_AGO);
        }
    }
...
library AdaptiveFee {
    uint32 constant alpha1 = 3000 - 500;
    uint32 constant alpha2 = 10000 - 3000;

    uint32 constant beta1 = 150;
    uint32 constant beta2 = 1500;

    uint32 constant gamma1 = 30;
    uint32 constant gamma2 = 100;

    uint32 constant volumeBeta = 30;
    uint32 constant volumeGamma = 10;

    // Maybe uint112
    function getFee(uint128 volatility, uint256 volumePerLiquidity) internal pure returns(uint256 fee) {
        uint256 sigm1 = sigmoid(volatility, gamma1, alpha1, beta1);
        uint256 sigm2 = sigmoid(volatility, gamma2, alpha2, beta2); 

        fee = 500 + sigmoid(volumePerLiquidity, volumeGamma, sigm1 + sigm2, volumeBeta);
    }
