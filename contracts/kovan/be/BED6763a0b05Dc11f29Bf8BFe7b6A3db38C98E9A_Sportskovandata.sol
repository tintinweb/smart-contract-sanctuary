...
contract Sportskovandata is ChainlinkClient {
    ...
    constructor() {
        ...
        fee = fee = 0.1 * 10 ** 18; // (Varies by network and job)
    }

  
    function requestSportsTeam() public {
        Chainlink.Request memory req = buildChainlinkRequest(jobId, address(this), this.fulfillSportsTeam.selector);
        req.add("date", "2021-06-12");
        req.add("teamName", "Sporting Kansas City");
        //req.add("city", _city);
        sendChainlinkRequestTo(oracle, req, fee);
    }

    
    function fulfillSportsTeam(bytes32 _requestId, uint256 _result) public recordChainlinkFulfillment(_requestId) {
        result = _result;
    }
