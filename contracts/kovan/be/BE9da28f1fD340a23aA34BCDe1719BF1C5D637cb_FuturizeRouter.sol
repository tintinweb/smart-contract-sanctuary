...
contract FuturizeRouter is IFuturizeRouter {
	...
	modifier ensure(uint256 deadline) {
		require(deadline >= block.timestamp, 'FuturizeRouter: EXPIRED');
		_;
	}
	...
	function openPos(
		...
		FuturizeUV3Pair(pair).open(collateral0, collateral1, to, minReceived, leverage);
	}
	...
	function increasePos(
		...
		FuturizeUV3Pair pair = FuturizeUV3Pair(pairFor(tokenA, tokenB));
		(, , , , , , , , bool isToken0) = pair.positions(positionId);
		isToken0
			? TransferHelper.safeTransferFrom(tokenA, msg.sender, address(pair), collateral)
			: TransferHelper.safeTransferFrom(tokenB, msg.sender, address(pair), collateral);
		pair.increase(positionId, collateral, minReceived, leverage);
	}
	...
	function liquidateAndBurn(
		...
		require(amountA >= amountAMin, 'FuturizeRouter: INSUFFICIENT_A_AMOUNT');
		require(amountB >= amountBMin, 'FuturizeRouter: INSUFFICIENT_B_AMOUNT');
	}
	...
	function _addLiquidity(
		...
		address pair = FuturizeFactory(factory).getPair(tokenA, tokenB);
		if (pair == address(0)) {
			pair = FuturizeFactory(factory).createPair(tokenA, tokenB);
		}
		//TODO: remove collateral from reserves
		(uint256 reserveA, uint256 reserveB, ) = IFuturizePair(pair).getReserves();
		if (reserveA == 0 && reserveB == 0) {
			(amountA, amountB) = (amountADesired, amountBDesired);
		} else {
			uint256 amountBOptimal = quote(amountADesired, reserveA, reserveB);
			if (amountBOptimal <= amountBDesired) {
				require(amountBOptimal >= amountBMin, 'FuturizeRouter: INSUFFICIENT_B_AMOUNT');
				(amountA, amountB) = (amountADesired, amountBOptimal);
			} else {
				uint256 amountAOptimal = quote(amountBDesired, reserveB, reserveA);
				assert(amountAOptimal <= amountADesired);
				require(amountAOptimal >= amountAMin, 'FuturizeRouter: INSUFFICIENT_A_AMOUNT');
		...
		)
	{
		...
		liquidity = IFuturizePair(pair).mint(to);
	}
	...
	function removeLiquidity(
		...
		address pair = pairFor(tokenA, tokenB);
		IFuturizePair(pair).transferFrom(msg.sender, pair, liquidity); // send liquidity to pair
		(uint256 amount0, uint256 amount1) = IFuturizePair(pair).burn(to);
		(address token0, ) = sortTokens(tokenA, tokenB);
	...
	function removeLiquidityWithPermit(
		...
		IFuturizePair(pair).permit(msg.sender, address(this), value, deadline, v, r, s);
	...
	function quote(
		...
		require(amountA > 0, 'FuturrizeRouter: INSUFFICIENT_AMOUNT');
		require(reserveA > 0 && reserveB > 0, 'FuturizeRouter: INSUFFICIENT_LIQUIDITY');
		amountB = (amountA * reserveB) / reserveA;
	}

	// returns sorted token addresses, used to handle return values from pairs sorted in this order
	function sortTokens(address tokenA, address tokenB) public pure returns (address token0, address token1) {
		require(tokenA != tokenB, 'FuturizeRouter: IDENTICAL_ADDRESSES');
		(token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
		require(token0 != address(0), 'FuturizeRouter: ZERO_ADDRESS');
	}
	...
	function pairFor(address tokenA, address tokenB) public view returns (address pair) {
		...
		return
			predictDeterministicAddress(
				FuturizeFactory(factory).futurizePairImpl(),
...
contract FuturizeUV3Pair is BasePool {
	...
	function _updatePos(
		...
		require(_oraclePriceInRange(), 'invalid oracle price');
	...
	function _oraclePriceInRange() internal returns (bool isInRange) {
		return true;
	}
...
contract BasePool is UniswapV2ERC20 {
	...
	function mint(address to) external lock returns (uint256 liquidity) {
		...
		(uint256 total0, uint256 total1, ) = getReserves();
		...
		if (_totalSupply == 0) {
			...
			liquidity = Math.min(
				(amount0 * _totalSupply) / (total0 - reserve0Collateral),
				(amount1 * _totalSupply) / (total1 - reserve1Collateral)
			); //dont include collateral in totals
		}
	...
	function skim(address to) external lock {
		address _token0 = token0; // gas savings
		address _token1 = token1; // gas savings
		(uint112 _total0, uint112 _total1, ) = getReserves();
...
library UniswapV3SwapHelper {
	...
	function getTwapPrice(IUniswapV3Pool pool, uint32 twapInterval)
		public
		view
		returns (uint256 token0Price, uint256 token1Price)
	{
		...
		return getTokenPricesFromSqrtPriceX96(sqrtPriceX96);
	}
