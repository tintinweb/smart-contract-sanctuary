    ...
    SignatureValidator
{
    ...
    *************************************************************/
        ...
        returns (uint256) 
    {
        ...
        // Replace fee factor and subsidy factor if transaction is not sent by relayer
        ...
        if(_isCurve(_makerAddr)) {
            // PermanetStorage can recognize `ETH_ADDRESS` but not `ZERO_ADDRESS`.
            // Convert it to `ETH_ADDRESS` as passed in `_takerAssetAddr` or `_makerAssetAddr` might be `ZERO_ADDRESS`.
            vars.takerAssetInternalAddr = vars.fromEth ? ETH_ADDRESS : _takerAssetAddr;
            vars.makerAssetInternalAddr = vars.toEth ? ETH_ADDRESS : _makerAssetAddr;
        } else {
     ...
     */
    function _isCurve(address _makerAddr) internal pure returns (bool) {
        if (
            _makerAddr == UNISWAP_V2_ROUTER_02_ADDRESS ||
            _makerAddr == SUSHISWAP_ROUTER_ADDRESS ||
            _makerAddr == BALANCER_EXCHANGE_PROXY_ADDRESS ||
            _makerAddr == KYBER_NETWORK_PROXY_ADDRESS
        ) return false;
        else return true;
    }
    ...
    function _verify(
        ...
        transactionHash = keccak256(
            abi.encode(
                ...
                _vars.makerAddr,
                _vars.takerAssetAddr,
                _vars.makerAssetAddr,
                _vars.takerAssetAmount,
                _vars.makerAssetAmount,
                _vars.userAddr,
                _vars.receiverAddr,
                _salt,
                _deadline
        ...
        require(isValidSignature(_vars.userAddr, EIP712SignDigest, bytes(""), _sig), "AMMWrapper: invalid user signature");
        // Validate that the transaction is not seen before
        require(! permStorage.isAMMTransactionSeen(transactionHash), "AMMWrapper: transaction seen before");
     ...
     */
    function _prepare(GroupedVars memory _vars) internal {
        // Transfer asset from user and deposit to weth if needed
        if (_vars.fromEth) {
            require(msg.value > 0, "AMMWrapper: msg.value is zero");
            require(_vars.takerAssetAmount == msg.value, "AMMWrapper: msg.value doesn't match");
            // Deposit ETH to WETH if internal asset is WETH instead of ETH
            if (! _isInternalAssetETH(_vars.takerAssetInternalAddr)) {
                weth.deposit{value: msg.value}();
            }
        } else {
            // other ERC20 tokens
            spender.spendFromUser(_vars.userAddr, _vars.takerAssetAddr, _vars.takerAssetAmount);
        }
    }
    ...
    function _swap(
        ...
        internal
        approveTakerAsset(_vars.takerAssetInternalAddr, _vars.makerAddr)
        returns (string memory source, uint256 receivedAmount)
    {
        ...
        if (_vars.makerAddr == UNISWAP_V2_ROUTER_02_ADDRESS ||
        ...
        } else if (_vars.makerAddr == BALANCER_EXCHANGE_PROXY_ADDRESS) {
            source = "Balancer V2";
            receivedAmount = _traderBalancerTokenToToken(
                _vars.takerAssetInternalAddr,
                _vars.makerAssetInternalAddr,
                _vars.takerAssetAmount,
                minAmount
            );
        } else if (_vars.makerAddr == KYBER_NETWORK_PROXY_ADDRESS) {
            source = "KyberSwap V2";
            receivedAmount = _traderKyberSwapTokenToToken(
                _vars.takerAssetInternalAddr,
                _vars.makerAssetInternalAddr,
                _vars.takerAssetAmount,
                minAmount
            ...
            if (fromTokenCurveIndex > 0 && toTokenCurveIndex > 0) {
                ...
                uint256 balanceBeforeTrade = _getSelfBalance(_vars.makerAssetInternalAddr);
                ...
                uint256 balanceAfterTrade = _getSelfBalance(_vars.makerAssetInternalAddr);
        ...
        returns (uint256 settleAmount)
    {
        ...
        uint256 _feeFactor = _vars.feeFactor;
        uint256 _subsidyFactor = _vars.subsidyFactor;

        if (_vars.receivedAmount == _vars.makerAssetAmount) {
            settleAmount = _vars.receivedAmount;
        } else if (_vars.receivedAmount > _vars.makerAssetAmount) {
            // shouldCollectFee = ((receivedAmount - makerAssetAmount) / receivedAmount) > (feeFactor / 10000)
            bool shouldCollectFee = _vars.receivedAmount.sub(_vars.makerAssetAmount).mul(BPS_MAX) > _feeFactor.mul(_vars.receivedAmount);
            if (shouldCollectFee) {
                // settleAmount = receivedAmount * (1 - feeFactor) / 10000
                settleAmount = _vars.receivedAmount.mul(BPS_MAX.sub(_feeFactor)).div(BPS_MAX);
            } else {
                settleAmount = _vars.makerAssetAmount;
            }
        } else {
            ...
            bool inSubsidyRange = _vars.makerAssetAmount.sub(_vars.receivedAmount).mul(BPS_MAX) <= actualSubsidyFactor.mul(_vars.receivedAmount);
            require(inSubsidyRange, "AMMWrapper: amount difference larger than subsidy amount");

            uint256 selfBalance = _getSelfBalance(_vars.makerAssetInternalAddr);
            bool hasEnoughToSubsidize = selfBalance >= _vars.makerAssetAmount;
            if (! hasEnoughToSubsidize && _isInternalAssetETH(_vars.makerAssetInternalAddr)) {
                // We treat ETH and WETH the same so we have to convert WETH to ETH if ETH balance is not enough.
                uint256 amountShort = _vars.makerAssetAmount.sub(selfBalance);
        ...
        if (_vars.toEth) {
            ...
            if (! _isInternalAssetETH(_vars.makerAssetInternalAddr)) {
                weth.withdraw(settleAmount);
            }
            _vars.receiverAddr.transfer(settleAmount);
        } else {
            // other ERC20 tokens
            IERC20(_vars.makerAssetAddr).safeTransfer(_vars.receiverAddr, settleAmount);
        }
    }
    ...
    function _traderBalancerTokenToToken(
        address _takerAssetAddr,
        address _makerAssetAddr,
        uint256 _takerAssetAmount,
        uint256 _minAmount
        ...
        returns (uint256)
    {
        ...
        uint256 totalAmountOut = balancerExchangeProxy.smartSwapExactIn(TokenInterface(_takerAssetAddr), TokenInterface(_makerAssetAddr), _takerAssetAmount, _minAmount, 5);
        return totalAmountOut;
    }

    function _traderKyberSwapTokenToToken(
        address _takerAssetAddr,
        address _makerAssetAddr,
        uint256 _takerAssetAmount,
        uint256 _minAmount
        ...
        returns (uint256)
    {
        ERC20 takerAsset = ERC20(_takerAssetAddr);
        ERC20 makerAsset = ERC20(_makerAssetAddr);
        /**
            takerAssetAmountFormatted * (expectedRate / 10 ** 18) = makerAssetAmountFormatted
            => expectedRate = makerAssetAmountFormatted * (10 ** 18) / takerAssetAmountFormatted
            => minConversionRateUint256 = minMakerAssetAmountFormatted * (10 ** 18) / takerAssetAmountFormatted
            => minConversionRateUint256 = (minAmount / (10 ** makerAssetDecimals)) * (10 ** 18) / (takerAssetAmount / (10 ** takerAssetDecimals))
            For uint256 precision problem, multiplying first
            => minConversionRateUint256 = minAmount * (10 ** 18) * (10 ** takerAssetDecimals) / (10 ** makerAssetDecimals) / takerAssetAmount
        */
        uint256 minConversionRateUint256 = _minAmount
            .mul(10 ** 18)
            .mul(10 ** uint256(takerAsset.decimals()))
            .div(10 ** uint256(makerAsset.decimals()))
            .div(_takerAssetAmount);

        IKyberNetworkProxy kyberProxy = IKyberNetworkProxy(KYBER_NETWORK_PROXY_ADDRESS);
        uint256 totalAmountOut = kyberProxy.trade(
            takerAsset,
            _takerAssetAmount,
            makerAsset,
            address(this),
