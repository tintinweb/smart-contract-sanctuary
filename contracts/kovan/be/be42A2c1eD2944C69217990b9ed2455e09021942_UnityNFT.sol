...
contract UnityNFT is ChainlinkClient, ERC721 {
    ...
    constructor(address _oracle, bytes32 _jobId, address _linkTokenAddress, uint256 _fee, string memory _baseURI, uint256 _interval) ERC721("UnityNFT", "UNFT") {
    ...
    onlyOwner
    public{
        require(!isRunning, "Cycle is already running.");
    ...
    function fulfill(bytes32 _requestId) public recordChainlinkFulfillment(_requestId) {
        URI_ID = (URI_ID + 1) % 7;
        if(LINK.balanceOf(address(this)) >= oraclePayment){
            Chainlink.Request memory req = buildChainlinkRequest(jobId, address(this), this.fulfill.selector);  
            req.addUint("until", block.timestamp + interval);
            sendChainlinkRequestTo(oracle, req, oraclePayment);
        }else{
