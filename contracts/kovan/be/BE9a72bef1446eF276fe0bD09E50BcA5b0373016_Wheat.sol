...
contract Wheat is ERC777, IERC777Recipient, ReentrancyGuard, IERC721Receiver, ERC165, ERC721Holder  {
    ...
    */
        ...
        requireFarm(_msgSender(), false) // Ensure LAND is not already in a farm
    {
        ...
        (,,, uint256 amountUnderlying) = getLPDetails(amount); // Calculates the underlying LAND
        ...
        requireFarm(_msgSender(), true) // Ensure the address you are releasing has a farm on the LAND
    {
        ...
        uint256 amountLP = senderFarm.amountLP;

        senderFarm.amount = 0; // Set LAND to zero
        senderFarm.amountLP = 0; // Set LP token to zero
        ...
        emit Released(_msgSender(), amountLP, senderFarm.compostedAmount);

        uint256 feeAmountLP = fee.mul(amountLP).div(100 * (10**18));
        uint256 amountLPLessFee = amountLP.sub(feeAmountLP);
    ...
    function allocateEquipment(uint256 tokenID) public
        nonReentrant()
        preventSameBlock(_msgSender())
        requireFarm(_msgSender(), true) // You can't add equipment if you don't have a farm
        requireEquipment(_msgSender(), false) // You can't add equipment if you already have equipment on the a farm
        {
        ...
        (string memory equipmentName, uint256 maxBoostLevel,uint256 blocksToMaxBoost) = nftToken.equipmentTraits(tokenID);

        // TODO check if it's possible to add a higher tokenID than is available
        ...
        farm.equipmentmaxBoostLevel = maxBoostLevel.mul(_percentMultiplier);
        farm.equipmentblocksToMaxBoost = blocksToMaxBoost;

        // TODO Emit Event

        emit EquipmentAllocated(
            _msgSender(),
            block.number,
            tokenID,
            equipmentName
    ...
    function releaseEquipment() public
        nonReentrant()
        preventSameBlock(_msgSender())
        requireEquipment(_msgSender(), true) // You can't remove equipment if you don't have any on a farm
        {
        ...
        returns (uint256)
    {
        ...
        uint256 equipmentMultipler = getAddressEquipmentMultiplier(targetAddress);
        uint256 afterMultiplier = blocksMintedByAmount.mul(compostMultiplier).div(_percentMultiplier);
        afterMultiplier = afterMultiplier.mul(maturityMultipler).div(_percentMultiplier);
        afterMultiplier = afterMultiplier.mul(equipmentMultipler).div(_percentMultiplier);
    ...
    function getLPDetails(uint256 amount)
        ...
        underlyingToken0Balance = uint256(amount.mul(token0Balance).div(lpTotalSupply)); // Calculates the underlying $Land = LPAmount / LPTotalSupply * LPreserve of LAND
    }
        ...
        returns (uint256)
    {
        ...
        // You don't get a boost until minimum blocks passed 
        uint256 targetBlockNumber = targetFarm.equipmentAddedBlockNumber.add(startMaturityBoost);
        ...
        uint256 blockDiff =
            block.number
            .sub(targetBlockNumber)
            .mul(targetFarm.equipmentmaxBoostLevel.sub(_percentMultiplier)) // calculates the extension over and above 1x 
            .div(targetFarm.equipmentblocksToMaxBoost)
            .add(_percentMultiplier);

        uint256 equipmentMultiplier = Math.min(targetFarm.equipmentmaxBoostLevel, blockDiff); // Min 1x, Max depends on the equipment attributes 
        ...
        )
    {
        ...
        uint256 addressEquipmentMultiplier = getAddressEquipmentMultiplier(targetAddress);
