...
contract RightClickNFT is ERC721URIStorage {
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory uri) {
        ...
        // copied-nft original owner vandal-nft, moved to tokenURI after copied-nft burn, zeroed data
        if (vandal[tokenId].status == 2) {
            return ftokenURI(tokenId);
        }
    }

    // user needs to call this when he has still original copy-nft
