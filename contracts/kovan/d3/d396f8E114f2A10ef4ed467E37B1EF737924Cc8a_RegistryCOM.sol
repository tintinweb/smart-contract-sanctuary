...
contract RegistryCOM is ERC721Burnable {
    ...
    constructor(address initialController) ERC721("Forever Domains (.com)", "FDCOM") public {
    ...
    modifier onlyApprovedMinter() {
        require(isApprovedMinter(msg.sender));
        _;
    }
    ...
    function approveMinter(address account) public onlyApprovedMinter {
        approvedMinters[account] = true;
    }

    function renounceMintingApproval() public {
    ...
    function mintSLD(address to, string memory label) public onlyApprovedMinter returns (uint256) {
    ...
    function setRecord(uint256 tokenId, string memory key, string memory value) public {
        require(_exists(tokenId));
    ...
    function destroyRecord(uint256 tokenId, string memory key) public {
        require(_exists(tokenId));
        require(_isApprovedOrOwner(msg.sender, tokenId), "ERC721: caller is not owner nor approved");
    ...
    function getRecordByKey(uint256 tokenId, string memory key) public view returns(string memory){
        require(_exists(tokenId));

        return (records[tokenId][key].value);
    }

    function isRecordDestroyed(uint256 tokenId, string memory key) public view returns(bool) {
        require(_exists(tokenId));
