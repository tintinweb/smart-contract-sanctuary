    ...
    ReentrancyGuard
{
    ...
    mapping(address => mapping(uint256 => DataTypes.RankedBidData)) internal _bids;
     ...
     */
    event BidsUpdated(uint256 indexed auctionId, address[] outbids, uint256 minimumPrice);
     ...
     */
    event FundsReceived(uint256 indexed auctionId, address indexed recipient, address[] bidders, uint256 amount);
     ...
     */
    event Whitelisted(address currency);

    /**
     * Emitted when a currency is removed from the whitelist.
     *
     * @param currency The newly unwhitelisted currency.
     */
    event Unwhitelisted(address currency);
    ...
    function updateBids(
        uint256 auctionId,
        uint256 newMinimum,
        address[] calldata toOutbid
    ) external nonReentrant onlyAdmin {
        for (uint256 i = 0; i < toOutbid.length; i++) {
            _bids[toOutbid[i]][auctionId].outbid = true;
        }
        _auctionsById[auctionId].minPrice = newMinimum;

        emit BidsUpdated(auctionId, toOutbid, newMinimum);
    }
    ...
    function withdrawBid(uint256 auctionId) external nonReentrant whenNotPaused {
        DataTypes.RankedBidData storage bidData = _bids[msg.sender][auctionId];
        require(bidData.outbid, Errors.RA_NOT_OUTBID);
        uint256 returnAmount = bidData.amount;
        IERC20Permit currency = IERC20Permit(_auctionsById[auctionId].currency);
        _bids[msg.sender][auctionId].amount = 0;

        if (address(currency) == address(WETH)) {
            WETH.withdraw(returnAmount);
            (bool success, ) = msg.sender.call{value: returnAmount, gas: 0}(new bytes(0));
            if (!success) {
        ...
        onlyAdmin
    {
        ...
        for (uint256 i = 0; i < toReceive.length; i++) {
            DataTypes.RankedBidData memory bid = _bids[toReceive[i]][auctionId];
            require(!bid.outbid, Errors.RA_OUTBID);
            require(!bid.claimed, Errors.RA_CLAIMED);
            amountToTransfer = amountToTransfer.add(bid.amount);
            _bids[toReceive[i]][auctionId].claimed = true;
        }
     ...
     */
    function getBid(address bidder, uint256 auctionId)
        external
        view
        returns (DataTypes.RankedBidData memory)
    {
    ...
    function _bid(
        ...
        DataTypes.RankedBidData storage bidData = _bids[onBehalfOf][auctionId];
