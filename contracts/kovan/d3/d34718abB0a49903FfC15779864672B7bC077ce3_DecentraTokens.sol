...
contract DecentraTokens is Context, IERC20, Ownable, RandomNumberConsumer {
    ...
    uint256 public minLottoBalance = 1 * 10**4 * 10**_decimals;
    ...
    uint256 public _maxTxAmount = 1 * 10**5 * 10**_decimals; //1%
    uint256 public _maxWalletAmount = 3 * 10**5 * 10**_decimals; //3%
    uint256 private numTokensSellToDistribute =  5 * 10**4 * 10**_decimals; //0.5%
    ...
    function getStats() external view returns(uint256, uint256, uint256, LotteryState, uint256) {
        return(lottoJackpotAmount, minLottoBalance, numWinners, state, totalWon);
    }
