...
contract VolatilePool is Ownable, ReentrancyGuard, ERC20 {
	...
	****************************************/
	...
	/**
	 * @dev Destroys tokens from account, if account is not msg.sender allowance
		...
		external 
	{
		if(_account != msg.sender)
			require(allowance(_account, msg.sender) >= _amount, "burn amount exceeds allowance");
		...
		nonReentrant
	{
		...
		inToken.safeTransferFrom(msg.sender, workerAddress, _inAmount);
		uint256 _outAmount = _inAmount * coefficient / COEFFICIENT_PRECISION;
