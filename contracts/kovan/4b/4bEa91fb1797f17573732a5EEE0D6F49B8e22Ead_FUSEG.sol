...
contract FUSEG is ERC20, Ownable {
  ...
  uint private _feesPercentage = 3;
  uint private _refFeesPercentage = 5;
  ...
  constructor(address payable teamAddress) ERC20("fuse.gold", "FUSEG")  {
 ...
 function updateRefFeePercentage(uint256 newPercentage) public onlyOwner {
  ...
  function getRefFeesPercentage() public view returns(uint) {
  ...
  function _collectFees(address payable client, uint amount) internal returns(uint) {
    ...
    // Start of internal functions
  function _collectRefFees(address payable client, uint amount) internal view returns(uint) {
    uint feeAmount = 0;
    bool isInWhiteList = _whiteList[client];
    if(!isInWhiteList && Members[client].refAddress != address(0)) {
      feeAmount = percentageCalculator(amount, _refFeesPercentage);
    }
    return feeAmount;
  }
    /**
    ...
    function percentageCalculator(uint amount, uint percentage) internal pure returns(uint256) {
  ...
  function buyGold(address payable client, address payable ref, uint256 initAmount) public onlyOwner {
    ...
    uint refAmount = _collectRefFees(client, totalAmount);
