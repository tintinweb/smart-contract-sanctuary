...
contract MinterVault {
    ...
    IChainLinkFeed public immutable FASTGAS;
    uint256 constant public MIN = 105;
    uint256 constant public BASE = 100;
    uint256 constant public TRANSFER_USED_GAS = 40000; // calc eth: 19000; transfer eth: 21000
    ...
    constructor(address _iChainLinkFeed){
    ...
    modifier onlyMinter() {
        ...
        _;
        // cost eth = (((mintUsedGas + TRANSFER_USED_GAS) * gas price) * 1.05
        uint256 gasPrice = uint256(FASTGAS.latestAnswer());
        uint256 cost = _gasUsed.sub(gasleft()).add(TRANSFER_USED_GAS).mul(gasPrice).mul(MIN).div(BASE);
