...
// File @openzeppelin/contracts/token/ERC721/extensions/IERC72[email protected]
...
contract RiseAngle is ERC721Enumerable, Ownable {
    ...
    uint256 public PRESALE_MAX_MINT = 1;
    uint256 public PUBLIC_SALE_MAX_MINT = 1;
    uint256 public MAX_RAMS_MINT = 2;
    ...
    constructor(string memory baseURI) ERC721("RiseAngle", "RAM") {
    ...
    function claimReserved(address recipient, uint256 amount) external onlyOwner {
        ...
        uint256 _nextTokenId = numRamsMinted + 1;
    ...
    function mintPresale(uint256 amountOfRams) external payable whenPresaleStarted {
        // console.log("MSG.VALUE", msg.value);
        require(_presaleEligible[msg.sender], "You are not eligible for the presale");
        require(totalSupply() < MAX_RAMS, "All tokens have been minted");
        require(amountOfRams <= PRESALE_MAX_MINT, "Cannot purchase this many tokens during presale");
        require(totalSupply() + amountOfRams <= MAX_RAMS, "Minting would exceed max supply");
        require(_totalClaimed[msg.sender] + amountOfRams <= PRESALE_MAX_MINT, "Purchase exceeds max allowed");
        require(amountOfRams > 0, "Must mint at least one dad");
        require(PRE_SALE_PRICE * amountOfRams == msg.value, "ETH amount is incorrect");
        ...
        emit PresaleMint(msg.sender, amountOfRams);
    }

    function mint(uint256 amountOfRams) external payable whenPublicSaleStarted {
        require(totalSupply() < MAX_RAMS, "All tokens have been minted");
        require(amountOfRams <= MAX_PER_MINT, "Cannot purchase this many tokens in a transaction");
        require(totalSupply() + amountOfRams <= MAX_RAMS, "Minting would exceed max supply");
        require(_totalClaimed[msg.sender] + amountOfRams <= MAX_RAMS_MINT, "Purchase exceeds max allowed per address");
        require(amountOfRams > 0, "Must mint at least one dad");
    ...
    function setMAX_PER_MINT(uint256 _amount) external onlyOwner {
        MAX_PER_MINT = _amount;
    }

    function setPRESALE_MAX_MINT(uint256 _amount) external onlyOwner {
        PRESALE_MAX_MINT = _amount;
    }

    function setPUBLIC_SALE_MAX_MINT(uint256 _amount) external onlyOwner {
        PUBLIC_SALE_MAX_MINT = _amount;
    }

    function setMAX_RAMS_MINT(uint256 _amount) external onlyOwner {
        MAX_RAMS_MINT = _amount;
    }
