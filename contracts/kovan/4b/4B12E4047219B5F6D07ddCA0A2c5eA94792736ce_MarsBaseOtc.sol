...
contract MarsBaseOtc is Ownable, IMarsBaseOtc, ReentrancyGuard {
    ...
    modifier onlyWhenOrderExists(bytes32 _id) {
        require(orders[_id].owner != address(0), "102");
        _;
    }

    modifier onlyOrderOwner(bytes32 _id) {
        require(orders[_id].owner == _msgSender(), "103");
        _;
    }

    modifier standartChecking(bytes32 _id) {
        ...
        onlyOwner
    {
        ...
        checkZeroBalance(ownerTokensInvested.length, ownerAmountsInvested);
        checkZeroBalance(usersTokensInvested.length, usersAmountsInvested);
     ...
     * @param distribution rules for transfer tokens
     */
    function makePartialSwap(bytes32 _id, OrdersBidInfo[] memory distribution)
        external
        override
        standartChecking(_id)
        onlyOwner
    {
        OrderInfo memory order = orders[_id];
        ...
        for (vars.i = 0; vars.i < distribution.length; vars.i = vars.i.add(1)) {
            require(
                ifAddressExists(_id, distribution[vars.i].investor),
                "no bid"
            );
        ...
        standartChecking(_id)
    {
        ...
        IntVars memory vars;
        vars.len = ordersBid[_id].length;

        require(vars.len > 0, "605");
        require(orderIndex < vars.len, "606");

        OrdersBidInfo memory bid = ordersBid[_id][orderIndex];
        address investor = bid.investor;
        BrokerInfo memory brInfo = ownerBroker[_id];
        ...
        vars.len = ordersOwnerBid[_id].length;
        brInfo = usersBroker[_id];

        for (vars.i = 0; vars.i < vars.len; vars.i = vars.i.add(1)) {
            bid = ordersOwnerBid[_id][vars.i];
            transferTokens(
                _id,
                bid.amountInvested,
                brInfo.percents,
     ...
     * @param _id uniqe id of order
     * @param orderIndex bid for transfer tokens
     * @param _amount array of amounts to user from owner`s bids
     */
    function makePartialSwapByOwner(
        bytes32 _id,
        uint256 orderIndex,
        uint256[] memory _amount
    ) external override standartChecking(_id) onlyOwner {
        OrderInfo memory order = orders[_id];
        ...
        vars.len = ordersOwnerBid[_id].length;
        require(_amount.length == vars.len, "607");

        OrdersBidInfo memory bid = ordersBid[_id][orderIndex];
        address investor = bid.investor;

        BrokerInfo memory brInfo = ownerBroker[_id];
        _setNewAmountInvested(_id, orderIndex, bid.amountInvested, false);
        transferTokens(
            _id,
            bid.amountInvested,
            brInfo.percents,
        ...
        for (vars.i = 0; vars.i < vars.len; vars.i = vars.i.add(1)) {
            bid = ordersOwnerBid[_id][vars.i];
            _setNewAmountInvested(_id, vars.i, _amount[vars.i], true);
            transferTokens(
                _id,
                _amount[vars.i],
                brInfo.percents,
    ...
    function createOrder(
        ...
        require(orders[_id].owner == address(0), "201");
     ...
     * @param _owner will be owner of order
     */
    ...
    {
        require(orders[_id].isCancelled == false, "401");
        require(orders[_id].isSwapped == false, "402");

        address caller = _msgSender();
        require(caller == orders[_id].owner || caller == owner(), "403");
    ...
    /**
     * close bid
     * @param _id will be owner of order
     ...
     * @param _id of order
     * @param _newExpirationDate new date
     */
    function setNewExpirationDate(bytes32 _id, uint256 _newExpirationDate)
        external
        onlyOrderOwner(_id)
        onlyWhenOrderExists(_id)
    {
        require(_newExpirationDate > block.timestamp, "205");
        orders[_id].expirationDate = _newExpirationDate;
    }
    ...
    function findBid(
        ...
        if (owner) array = ordersOwnerBid[_id];
        else array = ordersBid[_id];
    ...
    function _setNewAmountInvested(
        ...
        if (owner) {
            bid = ordersOwnerBid[_id][_orderIndex];
            require(bid.amountInvested >= _amount, "903");
            ordersOwnerBid[_id][_orderIndex].amountInvested =
                bid.amountInvested -
                _amount;
        } else {
            bid = ordersBid[_id][_orderIndex];
            require(bid.amountInvested >= _amount, "903");
            ordersBid[_id][_orderIndex].amountInvested =
                bid.amountInvested -
                _amount;
        }
        return bid;
    ...
    function _buyOrderDeposit(
        ...
        if (_from == orders[_id].owner) {
            require(isAddressInWhiteList[_token] == true, "308");
    ...
    function _sellOrderDeposit(
        ...
        if (_from == orders[_id].owner) {
            ...
            require(isAddressInWhiteList[_token] == true, "311");
            ordersBid[_id].push(ownersBid);
        }
    ...
    function transferTokens(
        ...
        if (_forTwo) {
            ...
            vars.toBroker = _amountInvested.mul(_percents).div(
                BROKERS_DENOMINATOR
            );
            vars.toUser = _amountInvested.sub(vars.toBroker);
            ...
            if (
                order.token == _investedToken &&
                ((order.orderType == OrderTypeInfo.buyType &&
                    order.owner == _investor) ||
                    (order.orderType == OrderTypeInfo.sellType &&
                        order.owner == _from))
            ) {
                if (
                    order.amountOfToken <= _amountInvested &&
                    orders[_id].isSwapped == false
                ) {
                    orders[_id].isSwapped = true;
                    orders[_id].amountOfToken = 0;
                } else {
                    orders[_id].amountOfToken =
                        order.amountOfToken -
                        _amountInvested;
                }
            }
        } else {
            vault.withdraw(_investedToken, _investor, _amountInvested);
        }
    }

    function checkZeroBalance(uint256 len, uint256[] memory array)
        private
        pure
    {
        for (uint256 i = 0; i < len; i = i.add(1)) {
    ...
    function ifAddressExists(bytes32 _id, address investor)
        private
        view
        returns (bool check)
    {
        ...
        OrderInfo memory order = orders[_id];
        OrdersBidInfo[] memory array;
        if (investor == order.owner) {
            array = ordersOwnerBid[_id];
        } else {
            array = ordersBid[_id];
        }
