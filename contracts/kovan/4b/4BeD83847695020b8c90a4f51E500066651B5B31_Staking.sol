...
contract Staking is ReentrancyGuard, Ownable {
    ...
    // reward finish time
    uint256 public stakingPeriodFinish = 0;
    // rewards per token rate
    uint256 public stakingRewardRate = 0;
    // total staking period
    uint256 public stakingPeriod = 30 days;
    uint256 public lastUpdateTime;
    uint256 public rewardPerTokenStored;
    // lockup time
    uint256 public lockupTime = 30 days;
    ...
    constructor(
        ...
        legacy = StakingToken(legacyContract);
    }

    /* ========== VIEWS ========== */

    function fullSupply() external view returns (uint256) {
        return _fullSupply;
    }
    ...
    function lastRewardTime() public view returns (uint256) {
        return Math.min(block.timestamp, stakingPeriodFinish);
    }

    function rewardPerAsset() public view returns (uint256) {
        if (_fullSupply == 0) {
    ...
    function viewRewardSum(address account) public view returns (uint256) {
        return
            _balances[account]
                .mul(rewardPerAsset().sub(userRewardPerTokenPaid[account]))
                .div(1e18)
                .add(rewards[account]);
    }

    function getRewardForPeriod() external view returns (uint256) {
        return stakingRewardRate.mul(stakingPeriod);
    }

    function rewardDifference(StakeAccount memory account)
        internal
        view
        returns (uint256)
    {
        uint256 totalReward = legacy.viewAggregatedReward();
    ...
    function staking(uint256 amount)
        external
        nonReentrant
        refreshRewards(msg.sender)
    {
        require(amount > 0, "Cannot stake 0");
        _fullSupply = _fullSupply.add(amount);
    ...
    function claimDeposit(uint256 amount)
        external
        nonReentrant
        refreshRewards(msg.sender)
    {
        require(amount > 0, "Cannot withdraw 0");
        require(block.timestamp >= stakeStart[msg.sender] + lockupTime);

        _fullSupply = _fullSupply.sub(amount);
        ...
        if (migrated[msg.sender]) {
            ...
            require(w < MAX_VALUE, "Migrate account first");

            uint256 rewardDiff = rewardDifference(legacyStakeAccounts[msg.sender]);
    ...
    function claimReward() public nonReentrant refreshRewards(msg.sender) {
    ...
    function notifyRewardSum(uint256 reward)
        external
        onlyOwner
        refreshRewards(address(0))
    {
        if (legacyCarry > 0) {
            reward -= legacyCarry;
            legacyCarry = 0;
        }
        if (block.timestamp >= stakingPeriodFinish) {
            stakingRewardRate = reward.div(stakingPeriod);
        } else {
            uint256 remaining = stakingPeriodFinish.sub(block.timestamp);
            uint256 leftover = remaining.mul(stakingRewardRate);
            stakingRewardRate = reward.add(leftover).div(stakingPeriod);
        }

        // Ensure the provided reward amount is not more than the balance in the contract.
        // This keeps the reward rate in the right range, preventing overflows due to
        // very high values of stakingRewardRate in the earned and rewardsPerToken functions;
        // Reward + leftover must be less than 2^256 / 10^18 to avoid overflow.
        uint256 balance = rewardsToken.balanceOf(address(this));
        require(
            stakingRewardRate <= balance.div(stakingPeriod),
            "Provided reward too high"
        );

        lastUpdateTime = block.timestamp;
        stakingPeriodFinish = block.timestamp.add(stakingPeriod);
        emit RewardAdded(reward);
    }

    // End rewards emission earlier
    function updateEndPeriod(uint256 timestamp)
        external
        onlyOwner
        refreshRewards(address(0))
    {
        stakingPeriodFinish = timestamp;
    }
    ...
    function setStakingTime(uint256 _stakingPeriod) external onlyOwner {
        require(
            block.timestamp > stakingPeriodFinish,
            "Previous staking period must be complete before changing the duration for the new period"
        );
        stakingPeriod = _stakingPeriod;
        emit RewardsDurationUpdated(stakingPeriod);
    }

    function setLockupPeriod(uint256 t) external onlyOwner {
        lockupTime = t;
    }

    function move(address[] calldata accounts) external onlyOwner {
        ...
        for (uint256 i; accounts.length > i; i++) {
            ...
            stakeStart[accountAddress] = account.lockEnd - lockupTime;
    ...
    modifier refreshRewards(address account) {
        rewardPerTokenStored = rewardPerAsset();
        lastUpdateTime = lastRewardTime();
        if (account != address(0)) {
            rewards[account] = viewRewardSum(account);
...
abstract contract StakingToken {
    ...
    function setMaxRewardPerBlock(uint256 rewardPerBlock) external {
        require(msg.sender == roles.owner() || msg.sender == roles.performer(), "Not authorized");
        updateAggregatedReward();
        totalCurrentRewardPerBlock = rewardPerBlock;
    }

    function supplyToRewardGoal(uint256 amount) external {
        MFI.safeTransferFrom(msg.sender, address(this), amount);
        updateAggregatedReward();
        rewardTarget += amount;
    }

    function removeFromRewardGoal(uint256 amount, address recipient) external {
        require(msg.sender == roles.owner() || msg.sender == roles.performer(), "Not authorized");
    ...
    function staking(uint256 amount, uint256 duration) external {
        ...
        account.cumulativeStart = updateAggregatedReward();

        account.lockEnd = max(block.timestamp + duration, account.lockEnd);
    }

    function claimDeposit(uint256 amount) external {
        StakeAccount storage account = stakeAccounts[msg.sender];
    ...
    function viewAggregatedReward() public view returns (uint256) {
        return
            min(
                rewardTarget,
                cumulativeReward +
    ...
    function updateAggregatedReward() public returns (uint256) {
        if (block.number > lastCumulativeUpdateBlock) {
            cumulativeReward = viewAggregatedReward();
            lastCumulativeUpdateBlock = block.number;
        }
        return cumulativeReward;
    }

    function _viewRewardSum(StakeAccount storage account)
        internal
        view
        returns (uint256)
    {
        uint256 totalReward = viewAggregatedReward();
        return
            ((totalReward - account.cumulativeStart) * account.stakeWeight) /
            (totalCurrentWeights + 1);
    }

    function viewRewardSum(address account) external view returns (uint256) {
        return _viewRewardSum(stakeAccounts[account]);
    }
        ...
        internal
    {
        if (account.stakeWeight > 0) {
            uint256 reward =
                min(_viewRewardSum(account), MFI.balanceOf(address(this)));
