...
contract Token is ERC20BurnableUpgradeSafe {
    ...
    function verifyBuySellConditions(address from, address to, uint256 amount) internal {

        if(pairs[from] || pairs[to]){
            require(startTime < block.number, "Token not available for trade");
            require(amount <= endBlock.sub(block.number).mul(tgeAmount).div(blocks), "Trade amount reached");
        }
