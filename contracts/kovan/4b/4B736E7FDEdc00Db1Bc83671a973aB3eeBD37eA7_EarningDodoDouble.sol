...
contract EarningDodoDouble is IEarning, Lock, AddressCheck {
    ...
    function claimRewards(address user) override external onlyOwner lock returns (address[] memory, uint256[] memory) {
        return _claimRewards(user, msg.sender);
    }
    
    function harvest() override lock external returns (address[] memory, uint256[] memory) {
        return _claimRewards(msg.sender, msg.sender);
    }

    function _claimRewards(address user, address to) internal returns (address[] memory, uint256[] memory) {
        _snapshot();
        _provideReward(user);
        _setDebt(user);
        uint256 len = SetAddress.length(tokens);
