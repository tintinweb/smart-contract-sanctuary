...
contract FarmDelegate is FarmStorageV1 {
    ...
    function migrateGToken(uint256[] calldata _pids) internal {
        ...
        pool.convertRate = calculationCovertRate(pool.totalOrigDep, pool.totalAmount);
        pool.accGanPerShare = accGanPerShareCoefficientBefore.div(pool.totalAmount);
        pool.accLockGanPerShare = accLockGanPerShareCoefficientBefore.div(pool.totalAmount);
        pool.accUsdtPerShare = accUsdtPerShareCoefficientBefore.div(pool.totalAmount);
    }
    ...
    function isLpToken(address _token) internal returns (bool){
    ...
    function checkRefundAndConvert(uint _pid) internal {
        ...
        if (pool.hasMigrate) {
            UserInfo storage user = userInfo[_pid][msg.sender];
            if (isLpToken(pool.lpToken) && pool.refundToken != address(0)) {
                //                lpToken to return
                if (!user.flag) {
    ...
    function calculationCovertRate(uint256 _before, uint256 _after) internal returns (uint){
