...
contract CoreBank is Ownable{
   ...
   //Event Divident
   event payDividendsPerYearsMade(address indexed admin,uint256 totalInterest);
  ...
  /* constructor() {
   ...
   function deposit() public payable returns(uint256){
        ...
        //Broadcast deposit evente
        emit DepositMade(msg.sender,msg.value);
        
        return balances[msg.sender];
   }
   
   function withdraw(uint256 withdrawAmount) public returns (uint256 remainBalance){
      ...
      // balances[msg.sender] -= withdrawAmount; 
       ...
       //Transfer to user , Revert on failed
       payable(msg.sender).transfer(withdrawAmount);
       
       
       //return balances[msg.sender]; 
       remainBalance =  balances[msg.sender];
   ...
   function systemWithDraw(uint256 withdrawAmount) public onlyOwner returns (uint256){
        ...
        require(withdrawAmount <= systemBalance(),"NOT enought");
        payable(msg.sender).transfer(withdrawAmount);
   ...
   function payDividendsPerYears() public onlyOwner payable {
       //require(owner() == msg.sender,"You not owner");
       uint256 totalInterest = 0;
       
       //แก้เป็น Batch รันรอบย่อยๆ จะได้ประหยัด gas block limit
       // หรือปรับเป็น BIGO 1 ให้ user รันแก๊สเอง จะได้ประหยัด
       ...
       require(msg.value == totalInterest,"Not enought interest to pay");
        ...
        emit payDividendsPerYearsMade(msg.sender,totalInterest);
       
   }
   
   
   // Kulap แนะนำให้ คำนวณยอดนี้ของแต่ละคน + balance private และแสดงยอดรวมให้ user ทราบอีกที แต่เงินจริงๆ ยังไม่ได้เติมให้
   function systemDividendsPerYear() external view onlyOwner returns (uint256){
