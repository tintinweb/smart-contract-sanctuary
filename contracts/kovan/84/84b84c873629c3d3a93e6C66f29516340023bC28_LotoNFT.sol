...
contract LotoNFT is ERC721, ERC721Burnable, Ownable, VRFConsumerBase, ReentrancyGuard {
    ...
    uint public tier1Share = 30;
    uint public tier2Share = 25;
    uint public tier3Share = 25;
    ...
    function selectWinners() public { // only owner
        ...
        for (uint i = 0; i < totalWinners; i++) {
            ...
            address winner = ownerOf(winnerToken);

            uint share = 0;
            if (i < tier1WinnersQuantity) {
                share = tier1Share;
            } else if (i < tier2WinnersQuantity) {
                share = tier2Share;
            } else if (i < tier3WinnersQuantity) {
                share = tier3Share;
            }

            accountRewards[winner] += finalAmount.mul(share).div(100);

            _winners.push(winnerToken);
        }
    ...
    function nextWinner(uint random) internal returns (uint) {
        uint maxIndex = totalSupply() - _winners.length;
    ...
    function allWinners() public view returns (uint[] memory winners) {
        uint _totalWinners = _winners.length;
