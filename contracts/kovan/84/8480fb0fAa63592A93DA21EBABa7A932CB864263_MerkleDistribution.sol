...
contract MerkleDistribution {
	...
	function isClaimed(address _addr, uint256 _id) public view returns(bool){
		bytes32 node = keccak256(abi.encodePacked(_addr, _id));
	...
	function claim(
		...
		setClaimed(node);
		require(token.mint(_addr, _id, _universeId, _earthId, _personId, _blockchainId, _sattoshiId), "MerkleDistribution: Mint Failed");
