...
contract HyperDexRouterGSN is BaseRelayRecipient, Ownable {
    ...
    uint256 public _feeBeneficiary = 5; // 0.5%
    uint256[4] public _feeReferrals = [5, 3, 2, 1];  // 0.5%, 0.3%, 0.2%, 0.1%
    ...
    function multiRoute(
        ...
        _payFees(feeToken, _beneficiary, inputAmount / 1000 * _feeBeneficiary);
        for (uint256 i = 0 ; i < referrals.length; i++) {
            if (referrals[i] != _NULL_ADDRESS && _feeReferrals[i] != 0) {
                _payFees(feeToken, payable(referrals[i]), inputAmount / 1000 * _feeReferrals[i]);
            }
        }
        ...
        if (inputToken == _ETH_ADDRESS) {
            if (feeToken == _ETH_ADDRESS) {
                uint256 inputAmountFee = inputAmount.add(_feeBeneficiary);

                for (uint256 i = 0 ; i < referrals.length; i++) {
                    if (referrals[i] != _NULL_ADDRESS && _feeReferrals[i] != 0) {
                        inputAmountFee = inputAmountFee.add(_feeReferrals[i]);
                    }
                }

                require(msg.value == inputAmountFee,"Insufficient value with fee");
            }
            else {
    ...
    /// @dev Set a new referrals fees 
    /// @param fees New referrals fees values
    function setReferralFees(uint256[4] memory fees) public onlyOwner {
        _feeReferrals = fees;
    }
    ...
    function _payFees(address token, address payable referral, uint256 amount) private {
        if (token == _ETH_ADDRESS) {
            return _sendETH(referral, amount);
        }
        return _sendERC20(IERC20(token), _msgSender(), referral, amount);
    }
...
abstract contract Context {
    function _msgSenderContext() internal view virtual returns (address payable) {
        return msg.sender;
    }
