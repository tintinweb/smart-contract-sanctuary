...
contract PoolStake is Owned {
    ...
    constructor(address _token, address _Owner) public Owned(_Owner) {
        ...
        uToken = 0x3D95E5625AA194BB291D31A9ced44F4e811C06ae;
        wallet = 0x538f14c190ba4B81A6f3CfBfD8dE470e5293ba3A;
        multiplier = Multiplier(0xE173A61B9814F67F03e5218068A88c52807Ee4eC);
     ...
     */ 
    function configure(address _multiplier, address _uToken, address _wallet) external returns(bool) {
        
        require(msg.sender == wallet, "only wallet can call");
        require(Address.isContract(_multiplier), "Multiplier must be a smart contract");
        require(_uToken != address(0) && _wallet != address(0), "won't accept the zero address");
        
        multiplier = Multiplier(_multiplier);
        uToken = _uToken;
        wallet = _wallet;
        
        emit NewConfig(msg.sender, _multiplier, _uToken, _wallet);
        return true;
    }
