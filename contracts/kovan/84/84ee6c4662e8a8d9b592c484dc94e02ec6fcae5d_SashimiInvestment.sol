...
// File: contracts/interfaces/IVaultBase.sol
...
contract SashimiInvestment is ISashimiInvestment, Ownable {
    ...
    function reBalance(address _token) notFrozen public {
        ...
        (uint256 beforeReserves, uint256 afterReserves, uint256 beforeDepositAmount) = getReservesIfReBalance(_token);
        ...
        if (beforeReserves < afterReserves)
        {
            ...
            // withdraw
            IVaultProvider(provider.vault).withdraw(amount);
        ...
        else if (beforeReserves > afterReserves)
        {
            ...
            // approve and deposit
            IVaultProvider(provider.vault).deposit(amount);
    ...
    function harvest(address _token) notFrozen public{
        ...
        uint256 profitAmount = IVaultProvider(provider.vault).harvest();
        if (profitAmount > 0)
        {
            address profit = IVaultProvider(provider.vault).profit();
    ...
    function withdraw(address _token, uint256 _amount) override onlyRouter notFrozen public {
        ...
        IVaultProvider(provider.vault).withdraw(_amount);
    ...
    function withdrawWithReBalance(address _token, uint256 _amount) override onlyRouter notFrozen public{
        (uint256 beforeReserves, uint256 afterReserves, ) = getReservesIfReBalance(_token);
        if (beforeReserves < afterReserves)
        {
    ...
    function addProvider(address _token, address _vault) onlyOwner public returns (uint256) {
        ...
        require(IVaultProvider(_vault).controller() == address (this), "Controller not matched.");
        require(IVaultProvider(_vault).token() == _token, "Token not matched.");
    ...
    function chooseProvider(address _token, uint256 _providerId) onlyAdmin public {
        ...
        if (chosen != 0) {
            ...
            // deposit on new
            uint256 withdrawAmount = IVaultProvider(oldProvider.vault).withdrawAll();
            uint256 depositAmount = deposits[_token];
            require(withdrawAmount >= depositAmount, "Insufficient withdraw amount.");
            IVaultProvider(provider.vault).deposit(depositAmount);
        }
    ...
    function emergenceWithdraw(address _token, uint256 providerId, uint256 _amount, address _to) onlyOwner public{
        Provider memory provider = providers[providerId];
        IVaultProvider(provider.vault).transfer(_token, _to, _amount);
    }

    // for token cannot withdraw with harvest
    function emergenceWithdrawETH(uint256 providerId, uint256 _amount, address _to) onlyOwner public{
        Provider memory provider = providers[providerId];
        IVaultProvider(provider.vault).transferETH(_to, _amount);
    }
    ...
    function emergenceTriggerProvider(uint256 providerId, address target, uint value, string memory signature, bytes memory data) onlyOwner public{
        Provider memory provider = providers[providerId];
        IVaultProvider(provider.vault).executeTransaction(target, value, signature, data);
    }
    ...
    function earned(address _token) view public returns(address profit, uint256 amount){
        uint256 providerId = chosenProviders[_token];
        Provider memory provider = providers[providerId];
        profit = IVaultProvider(provider.vault).profit();
        amount = IVaultProvider(provider.vault).earned();
    }
    ...
    function getReservesIfReBalance(address _token) private view returns (uint256 beforeReserves, uint256 afterReserves, uint256 beforeDepositAmount){
        uint256 reservesRatio = reservesRatios[_token];
        require(reservesRatio > 0, "ReservesRatios not exists.");

        beforeReserves = IERC20(_token).balanceOf(router);
        beforeDepositAmount = deposits[_token];
