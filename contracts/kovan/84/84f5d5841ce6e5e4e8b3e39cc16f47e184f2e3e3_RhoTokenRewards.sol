    ...
    ReentrancyGuardUpgradeable
{
    ...
    struct RhoTokenInfo {
        ...
        uint256 rewardDuration; // duration of the current rewards period in number of blocks
        uint256 rhoTokenOne; // multiplier for one unit of RhoToken
    }

    struct UserInfo{
        uint256 amount; // number of rhoTokens held by user
    ...
    mapping(address => address[]) public userRhoTokenRecord;

    /**
     * @notice address of rewards token contract
     */
    address private rewardsTokenAddress;
    ...
    function totalRewardOf(address user) external view override returns (uint256) {
        ...
        for (uint256 i = 0; i < rhoTokenList.length; i++){
            // totalReward += this.rewardOf(user, rhoTokenList[i]);
        }
        return totalReward;
    }
    ...
    function rewardsPerToken() public view override returns (uint256) {
        ...
        return rewardsPerTokenStored.add(
            (lastBlockApplicable().sub(lastUpdateBlock)).mul(rewardRate).mul(_rhoTokenOne).div(totalSupply)
        );
    }
    ...
    function _earned(address addr) internal view returns (uint256) {
        return super._earned(_rhoToken.balanceOf(addr), rewardsPerToken().sub(rewardsPerTokenPaid[addr]), _rhoTokenOne, rewards[addr]);
    }
    ...
    function updateReward(address addr) public override {
        ...
        console.log("rewardsPerTokenStored", rewardsPerTokenStored);
        ...
        console.log("rewards[addr]", rewards[addr]);
        console.log("rewardsPerTokenPaid[addr]", rewardsPerTokenPaid[addr]);
    }
    ...
    function setRewardAmount(uint256 reward) external override onlyOwner {
        ...
        if (block.number >= rewardsEndBlock) {
            ...
            uint256 leftover = blocksRemaining.mul(rewardRate);
        ...
        require(rewardRate <= balance.div(rewardsDuration), "Insufficient balance for rewards");
    ...
    function shortenRewardsDuration() external override onlyOwner {
        // RhoTokenInfo storage rhoTokenxxx =
        uint256 currentBlock = block.number;
        if (rewardsEndBlock > currentBlock) {
    ...
    function claimReward(address user) external override {
        console.log("claimRewards", user);
        updateReward(user);
        console.log("rewards[user]", rewards[user]);

        if (rewards[user] > 0) {
            rewards[user] = flurryRewards.claimRhoTokenReward(user, rewards[user]);
        }
    }
