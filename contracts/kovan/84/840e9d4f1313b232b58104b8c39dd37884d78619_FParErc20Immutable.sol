...
// File: contracts/FParErc20.sol

pragma solidity ^0.6.12;


/**
 * @title Parvalue's ParErc20 Contract
 * @notice RTokens which wrap an EIP-20 underlying
 * @author Blaize
 */
contract FParErc20 is FRToken, FRErc20Interface {
    /**
    ...
    function initialize(address underlying_,
        ...
        super.initialize(controller_, interestRateModel_, name_, symbol_, decimals_,slope1,slope2,optimalUtilisationRate_);

        // Set underlying and sanity check it
        underlying = underlying_;
        ERC20Burnable(underlying).totalSupply();
    }
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function mint(uint mintAmount, uint period) external override returns (uint) {
        (uint err,) = mintInternal(mintAmount, period);
        return err;
    }
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function redeem(uint redeemTokens, uint supplyId) external override returns (uint) {
        return redeemInternal(redeemTokens, supplyId);
    }
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function redeemUnderlying(uint redeemAmount, uint depositId) external override returns (uint) {
        return redeemUnderlyingInternal(redeemAmount, depositId);
    }
    ...
    /**
      ...
      * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
      */
    function borrow(uint borrowAmount, uint period) external override returns (uint) {
        return borrowInternal(borrowAmount, period);
    }
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function repayBorrow(uint repayAmount, uint borrowId) external override returns (uint) {
        (uint err,) = repayBorrowInternal(repayAmount, borrowId);
        return err;
    }
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function repayBorrowBehalf(address borrower, uint repayAmount, uint borrowId) external override returns (uint) {
        (uint err,) = repayBorrowBehalfInternal(borrower, repayAmount, borrowId);
        return err;
    }
    ...
    function getCashPrior() internal override view returns (uint) {
        ERC20Burnable token = ERC20Burnable(underlying);
        return token.balanceOf(address(this));
    }

    /**
     * @dev Similar to IERC20transfer, except it handles a False result from `transferFrom` and reverts in that case.
    ...
    function doTransferIn(address from, uint amount) internal override returns (uint) {
        IERC20NonStandardInterface token = IERC20NonStandardInterface(underlying);
        uint balanceBefore = ERC20Burnable(underlying).balanceOf(address(this));
        ...
        require(success, "TOKEN_TRANSFER_IN_FAILED_VIII");

        // Calculate the amount that was *actually* transferred
        uint balanceAfter = ERC20Burnable(underlying).balanceOf(address(this));
        require(balanceAfter >= balanceBefore, "TOKEN_TRANSFER_IN_OVERFLOW");
        return balanceAfter - balanceBefore;   // underflow already checked above, just subtract
    }

    /**
     * @dev Similar to IERC20transfer, except it handles a False success from `transfer` and returns an explanatory
    ...
    function doTransferOut(address payable to, uint amount) internal override {
        ...
        require(success, "TOKEN_TRANSFER_OUT_FAILED_IX");
    }
}

// File: contracts/FParErc20Immutable.sol

pragma solidity ^0.6.12;


/**
 * @title ParValue's CErc20Immutable Contract
 * @notice RTokens which wrap an IERC-20 underlying and are immutable
 * @author Compound
 */
contract FParErc20Immutable is FParErc20 {
    /**
    ...
    constructor(address underlying_,
        ...
        initialize(underlying_, controller_, interestRateModel_, name_, symbol_, decimals_, slope1, slope2, optimalUtilisationRate_);
