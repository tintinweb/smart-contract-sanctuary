...
contract PoolStake is Owned {
    ...
    //12 hours represented in seconds
    ...
    event NewConfig(address indexed sender, address newMultiplier, address NewFeeToken, address newWallet);
    ...
    constructor(address _token, address _Owner) public Owned(_Owner) {
        ...
        uToken = 0xf8d798E2A6F1550690a4E6b82B20B052096F46a3;
        platformWallet = 0xE06553a9E0B7d5A0A018965D7fc85E2aB2d6EE51;
        multiplier = Multiplier(0xca677801CE2bf600f5ff36F6D89970FC9A1241C9);
        uniswapRouter = UniswapV2Router(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);  
    }
    
    /*
     * @dev change the addresses of the multiplier, fee token and fee receiver (only fee receiver can call)
     * ---------------------------------------------------------------------------------------------------
     * @param _multiplier      --> the new multiplier address (must be a smart contract)
     * @param _uToken          --> the new fee token address
     * @param _wallet          --> the new fee receiver address
     * --------------------------------------------------------
     * returns whether successfully changed or not.
     */ 
    function configure(address _multiplier, address _uToken, address _platformWallet) external returns(bool) {
        
        require(msg.sender == platformWallet, "only wallet can call");
        require(Address.isContract(_multiplier), "Multiplier must be a smart contract");
        require(_uToken != address(0) && _platformWallet != address(0), "won't accept the zero address");
        
        multiplier = Multiplier(_multiplier);
        uToken = _uToken;
        platformWallet = _platformWallet;
        
        emit NewConfig(msg.sender, _multiplier, _uToken, _platformWallet);
        return true;
    }
     ...
     * -------------------------------------------------------------------------------------
     */
    function addLiquidity(uint _term, uint _multiplier) external payable {
        ...
        if (mul == 1) _withMultiplier(term, provided_ETH, provided_token, wethRP,  tokenRP);
    ...
    function _withdrawUserETHBonus(address payable _user, uint releasedAmount) internal returns(bool) {
        ...
        require(IERC20(uToken).transferFrom(_user, platformWallet, fee), "must approve fee");
     ...
     */ 
    function multiplierContract() external view returns(address) {
