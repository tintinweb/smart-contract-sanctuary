...
contract BuryShib is ERC20("BuryShib", "xSHIB"){
    using SafeMath for uint256;
    ...
    function enter(uint256 _amount) public {
        require(((_amount / (1000000 * 10**18)) > 0) && ((_amount % (1000000 * 10**18) ) == 0), "Shib must be stake in millions");
    ...
    // Leave the doghouse. Claim back your SHIB.
    ...
    function leave(uint256 _share) public {
        ...
        UserInfo storage user = userInfo[msg.sender];
        uint256 whatEth = user.amount.mul(accRewardPerShare).div(1e12).sub(user.rewardDebt);
        msg.sender.call{ value: whatEth }("");
        lastRewardBalance = address(this).balance;
    ...
    function updatePool() public {
        ...
        uint256 lpSupply = shib.balanceOf(address(this));
