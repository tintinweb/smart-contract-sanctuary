...
contract BalancerUtils {
    ...
    function justSwap(address smartPoolAddress, address tokenIn, uint tokenAmountIn, address tokenOut, uint minAmountOut, uint maxPrice)
    public returns (uint, uint){
        ConfigurableRightsPool crpPool = ConfigurableRightsPool(smartPoolAddress);
        BPool bPool = crpPool.bPool();
        // sell the token
        return bPool.swapExactAmountIn(tokenIn, tokenAmountIn, tokenOut, minAmountOut, maxPrice);
    }

//    function pokeSwap(address smartPoolAddress, address tokenIn, uint inAmount, address tokenOut, uint minAmountOut, uint maxPrice){
//
//    }
    function forceSwap(address smartPoolAddress, address tokenIn, uint inAmount, address tokenOut) public returns (uint,uint) {
        ConfigurableRightsPool crpPool = ConfigurableRightsPool(smartPoolAddress);
        BPool bPool = crpPool.bPool();
        // sell the token
        return bPool.swapExactAmountIn(tokenIn, inAmount, tokenOut, 1, uint(-1));
    }
    ...
    function swapPokeSwap(address smartPoolAddress, address tokenIn, uint inAmount, address tokenOut) public returns(uint){
        ConfigurableRightsPool crpPool = ConfigurableRightsPool(smartPoolAddress);
        BPool bPool = crpPool.bPool();
        // sell the token
        (uint tokenAmountOut,) = bPool.swapExactAmountIn(tokenIn, inAmount, tokenOut, 1, uint(-1));

        // do the poke
        crpPool.pokeWeights();

        // buy the token back
        (uint tokenAmountOut2,) = bPool.swapExactAmountIn(tokenOut, tokenAmountOut, tokenIn, 1, uint(-1));
