...
contract Precog is IPrecog {
    ...
    function invest(
        ...
        uint balance = IERC20(usdc).balanceOf(address(tradeInfo.futureExchange));
        ...
        uint tradeAmount = amount - fee.feeTrading;
        if (canBorrow) {
            loan.borrowAmount = _getDebtAmount(loan, block.timestamp, expiryDate);
            ILending(lendingContract).createLoan(platformIndex, usdc, borrowAmount);
            tradeAmount += borrowAmount;
            if (userBorrowInfo[user][tradeInfo.futureContract].length == 0) {
        ...
        //);
        return (profit, IERC20(usdc).balanceOf(address(tradeInfo.futureExchange)) - balance);
    }
