...
contract Bonds is ERC1155, IBonds {
    ...
    uint256 constant public override head = 0;
    ...
    * in that order
     */
    function getStakingAt(address _who, uint256 _index) external override view returns(uint, uint, uint256, uint256, uint256){
    ...
    *
     */
    function newLoan(address _paymentContractAddress, uint256 _id, bytes32 _hash, bytes memory _signature) external override{
        // compute the hash of the data and verify it matches the hash provided

        // require(keccak256(abi.encodePacked(_paymentContractAddress, _id)) == _hash, 
        ...
        address user = _hash.recover(_signature);

        IERC20PaymentStandard pc = IERC20PaymentStandard(_paymentContractAddress);
        uint256 amm;
        address creator;
        (amm, creator) = pc.issueBonds(_id);
        require(user == creator, "Data provided must be signed by the borrower");
    ...
    * @param _amm is the ammount to stake
     */
    function stake(uint256 _id, uint256 _amm) external override returns(uint){
    ...
    * @return if successful. May not be if loan has been completed since staking
    ...
    function unstake(uint256 _index) external override returns(bool){
        ...
        uint256 toMint = periodsStaked * (amm * pc.getInterest(id) / ONE_HUNDRED_PERCENT);
    ...
    * @return the number of periods
     */
    function getAccruances(address _who, uint256 _index) public override view returns(uint256) {
