...
contract Booking is Ownable {
    ...
    mapping(string => Database) History;
    ...
    function book_excursion(address _token, string memory _Excursion_ID)
        public
        payable
    {
        require(
            msg.value >= amount / getTokenPrice(_token),
            "Not enought funding"
        ); //Cost in ETH
        ...
        History[_Excursion_ID].Cost = msg.value; //amount;
    ...
    function get_Excursion(string memory _Excursion_ID)
        public
        view
        returns (Database memory)
    {
        return History[_Excursion_ID];
    }
    ...
    function Guide_Cancel_excursion(string memory _Excursion_ID)
        public
        payable
    {
        require(
        ...
        address_Buyer.transfer(History[_Excursion_ID].Cost);
        History[_Excursion_ID].Cost = 0;
        History[_Excursion_ID].Status = Payment_Status.Canceled_Guide;
    }

    function Buyer_Cancel_excursion(string memory _Excursion_ID)
        public
        payable
    {
        require(
        ...
        address_Guide.transfer(
            ((History[_Excursion_ID].Cost * Share_Guide) / 100) *
                (1 - Lati_Comision / 100)
        );
        address_Buyer.transfer(
    ...
    function Guide_Request_Payment(string memory _Excursion_ID) public payable {
        int256 days_toRelease = int256(History[_Excursion_ID].Date_Booked) +
            int256(History[_Excursion_ID].Delta_toExcursion) +
            int256(Days_forRelease * 1 days) -
            int256(block.timestamp);
    ...
    function Approve_transferFromERC20(IERC20 _token, uint256 _amount)
        public
        payable
    {
        uint256 erc20balance = _token.balanceOf(msg.sender);
        require(_amount <= erc20balance, "Balance is to low");
        _token.allowance(msg.sender, payable(address(this)));
        _token.approve(payable(address(this)), _amount + 10000);
    }
