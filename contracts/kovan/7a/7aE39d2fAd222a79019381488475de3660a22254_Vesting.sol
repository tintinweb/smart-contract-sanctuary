...
contract Vesting is Initializable, BumperAccessControl {
    ...
    */
    ...
    event ClaimAmount(address recipient, uint256 amount);
    ...
    function addRecipients(
        ...
        for (uint256 i = 0; i < _amounts.length; i++) {
            ...
            (_startTime, _endTime, _cliffDuration) = abi.decode(
                (_times[i]),
                (uint256, uint256, uint256)
            );
            (_amount, _onStartAmount) = abi.decode(
                (_amounts[i]),
                (uint256, uint256)
            );
        ...
        isNonZeroAddress(recipient)
    {
        ...
        uint256 _bal =
            recipients[recipient].totalVestingAmount -
    ...
    function claim(uint256 amount) external {
        ...
        emit ClaimAmount(msg.sender, amount);
    }
