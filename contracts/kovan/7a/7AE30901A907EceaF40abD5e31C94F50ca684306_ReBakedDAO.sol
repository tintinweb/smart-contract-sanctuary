...
// 
contract ReBakedDAO is Ownable {

	using SafeERC20 for IERC20;
	
	// Rebaked DAO wallet
	address public treasury;
	// Fee Precision PPM (parts per million)
	uint256 constant public FEE_PRECISION = 1e6;
	...
	event CreatedProject(bytes32 indexed projectId);
	...
	****************************************/
	...
	/**
	* @dev Creates project
	* @param initiator_ project initiator
	* @param token_ project token address, zero addres if project has not token yet (IOUT will be deployed on project approval) 
	...
	*/
		...
		returns(bytes32 projectId_)
	{
		...
		if(address(token_) == address(0))
			token_ = _deployIOUT(budget_);
		else
			token_.safeTransferFrom(initiator_, address(this), budget_);
		uint256 _feeDaoAmount = budget_ * feeDao / FEE_PRECISION;
		uint256 _feeObserversAmount = budget_ * feeObservers / FEE_PRECISION;
		token_.safeTransfer(treasury, _feeDaoAmount);
		Project storage _project = projectData[projectId_];
		_project.initiator = initiator_;
		_project.token = token_;
		_project.budget = budget_;
		_project.budgetAllocated = _feeDaoAmount + _feeObserversAmount;
		_project.timeCreated = block.timestamp;
		emit CreatedProject(projectId_);
	}
	...
	*/
		...
		returns(bytes32 projectId_)
	{
		...
		require(projectData[projectId_].initiator == address(0), "duplicate project id");
	}
	...
	*/
	function _deployIOUT(uint256 totalSupply_)
		private
		returns(IERC20 token_)
	{
		token_ = IERC20(new IOUToken(totalSupply_));
	}
	...
	*/
	function getProjectData(bytes32 projectId_)
		external
		view
		returns(Project memory projectData_)
	{
		projectData_ = projectData[projectId_];
	}
