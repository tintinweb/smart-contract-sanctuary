...
abstract contract AbstractBettingContract is IBettingContract {
    ...
    function getPrice(uint8 _decimals) external payable override onlyOpen {
        require(
...
contract CommunityBettingContract is AbstractBettingContract {
    using SafeERC20 for IERC20;

    uint256 public ticketPrice;

    constructor(address payable _owner, address payable _creator)
        AbstractBettingContract(_owner, _creator)
    {}
    ...
    function close() external override onlyPriceClosed onlyOpen {
        ...
        status = Status.End;
        address[] memory winers;
        if (!success) {
            winers = listBuyer;
        } else {
            winers = ticketSell[result];
        }
        uint256 reward = 0;
        if (winers.length > 0) {
        ...
        emit Close(block.timestamp, price, winers, reward);
        selfdestruct(owner);
    }

    // guess_value = real_value * 10**bracketsPriceDecimals
    function buyTicket(uint256 _bracketIndex) public override onlyOpen betable {
...
import "../CommunityBettingContract.sol";
import "../interface/IBettingFactory.sol";

contract CommunityBettingFactory is IBettingFactory {
    function createNewBettingContract(
        address payable _owner,
        address payable _creator
    ) public override returns (address) {
        CommunityBettingContract betting = new CommunityBettingContract(
            _owner,
            _creator
        );
