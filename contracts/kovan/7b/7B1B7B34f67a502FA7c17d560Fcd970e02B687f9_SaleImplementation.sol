...
contract SaleImplementation is AccessControlUpgradeable , ERC2771ContextUpgradeable {
    ...
    modifier isValidCancel ( uint256 nftId ){
        require(_nftSales[nftId].owner == _msgSender(), "Only owner/admin can cancel Sale");
        require(_nftSales[nftId].isActive, "NFT is not up for sale");
        _;
    }

    modifier isValidPurchase ( uint256 nftId ){
        require( _nftSales[nftId].isActive , "NFT is not for sale");
        require( _token.allowance(_msgSender() , address(this)) >= _nftSales[nftId].price, "Grant token approval to Sale Contract");
        _;
    }

    modifier isValidPurchaseParcel( uint256[] memory nftIds ){
        uint256 totalAmount = 0;
        for (uint256 i = 0; i < nftIds.length; i++) {
            uint256 nftId = nftIds[i];
            totalAmount = totalAmount.add(_nftSales[nftId].price);
        }
        require( _token.allowance(_msgSender(), address(this)) >= totalAmount, "Grant token approval to Sale Contract" );
        _;
    }
     ...
     */
    function sellProperty(uint256 nftId, uint256 price) isValidSell(nftId) external {
     ...
     * @param nftId - nftId of the property
     */
    function cancelSale(uint256 nftId) external isValidCancel(nftId) onlyRole(ADMIN) { // Instead of line 164
    ...
    function iPurchaseProperty (uint256 nftId , address buyer , address seller , uint256 price ) internal {
     ...
     * @param nftId - nftId of the property
     */
    function purchaseProperty(uint256 nftId , address buyer ) public isValidPurchase( nftId ) onlyRole(ADMIN) {
     ...
     */
    function purchaseParcel(uint256[] memory nftIds , address buyer ) external isValidPurchaseParcel( nftIds ) onlyRole(ADMIN){
        for (uint256 i = 0; i < nftIds.length; i++) {
            uint256 nftId = nftIds[i];
            iPurchaseProperty(nftId , buyer , _nftSales[nftId].owner , _nftSales[nftId].price);
        }
