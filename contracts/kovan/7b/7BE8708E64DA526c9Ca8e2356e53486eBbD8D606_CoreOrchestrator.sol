...
contract CoreOrchestrator is Ownable {
    ...
    address public constant USDC_USDT_UNI_POOL =
        address(0xDE1d29b49A004F723877b38215af82A5437c75d6); // kovan
    address public constant USDC_DAI_UNI_POOL =
        address(0x9D4804E59bEf28491A3993a5111C2390A167F1ED); // kovan
    address public constant NFT =
        address(0xC36442b4a4522E871399CD717aBDD847Ab11FE88); // kovan & mainnet
    address public constant ROUTER =
        address(0xE592427A0AEce92De3Edee1F18E0157C05861564); // kovan & mainnet


    address public constant USDC =
        address(0xb7a4F3E9097C08dA09517b5aB877F7a917224ede); // kovan
    address public constant USDT =
        address(0x07de306FF27a2B630B1141956844eB1552B956B5); // kovan
    ...
    // ----------- Params -----------
    uint256 public constant RING_KEEPER_INCENTIVE = 500e18;

    uint256 public constant MIN_REWEIGHT_DISTANCE_BPS = 100;

    bool public constant USDC_PER_USDT_IS_PRICE_0 = true; // for the USDT_USDC pair // kovan
    bool public constant USDC_PER_DAI_IS_PRICE_0 = true; // for the DAI_USDC pair // kovan

    uint256 public ringSupply;
    uint256 public constant IDO_RING_PERCENTAGE = 1;
    uint256 public constant GENESIS_RING_PERCENTAGE = 10;
    uint256 public constant STAKING_RING_PERCENTAGE = 10;

    uint256 public constant RING_GRANTS_AMT = 20_000_000e18;
    uint256[9] public RING_TIMELOCK_AMTS = [
    ...
    IPCVDepositOrchestrator private usdcPCVDepositOrchestrator;
    ...
    constructor(
        ...
        usdcPCVDepositOrchestrator = IPCVDepositOrchestrator(
    ...
    function initPoolUSDC() public onlyOwner {
        usdcRusdPool = INonfungiblePositionManager(NFT).createAndInitializePoolIfNecessary(
            rusd < USDC ? rusd : USDC, rusd < USDC ? USDC : rusd, 500, 79228162514264337593543950336 // (X/2**96)**2 = 1
        );
    }

    function initPoolUSDT() public onlyOwner {
        usdtRusdPool = INonfungiblePositionManager(NFT).createAndInitializePoolIfNecessary(
            rusd < USDT ? rusd : USDT, rusd < USDT ? USDT : rusd, 500, 79228162514264337593543950336 // (X/2**96)**2 = 1
        );
    }

     function initPoolDAI() public onlyOwner {
        daiRusdPool = INonfungiblePositionManager(NFT).createAndInitializePoolIfNecessary(
            rusd < DAI ? rusd : DAI, rusd < DAI ? DAI : rusd, 500, 79228162514264337593543950336 // (X/2**96)**2 = 1
        );
    }
    ...
    function initPoolGovernanceToken() public onlyOwner {
        ringRusdPool = INonfungiblePositionManager(NFT).createAndInitializePoolIfNecessary(
            ring < rusd ? ring : rusd, ring < rusd ? rusd : ring, 3000, 79228162514264337593543950336 // (X/2**96)**2 = 1
        );
    }
    ...
    function initPCVDepositUSDC() public onlyOwner() {
        (usdcUniswapPCVDeposit, usdcUniswapOracle) = usdcPCVDepositOrchestrator.init(
            ...
            NFT,
            ROUTER,
            address(0), // set address(0) to use usdc oracle
    ...
    function initGenesis() public onlyOwner {
        ...
        core.allocateRing(
            genesisGroup,
            (ringSupply * GENESIS_RING_PERCENTAGE) / 100
        );
