...
contract TokenFarm is Ownable {
    ...
    function getUserTotalValue(address _user) public view returns (uint256) {
        ...
        for (
            uint256 tokensAllowedIndex = 0;
            tokensAllowedIndex < tokensAllowed.length;
            tokensAllowedIndex++
        ) {
            userTotalValue =
                userTotalValue +
                getUserSingleTokenValue(
                    _user,
                    tokensAllowed[tokensAllowedIndex]
                );
        }
        ...
        returns (uint256)
    {
        ...
        (uint256 price, uint256 decimals) = getPriceFeed(_token);
    ...
    function unstaketTokens(address _token) public {
        uint256 balance = stakingBalance[_token][msg.sender];
        require(balance > 0, "no have staked this token");
    ...
    function setTokenAllowed(address _token) public onlyOwner {
    ...
    function tokenIsAllowed(address _token) public returns (bool) {
        for (
            ...
            if (tokensAllowed[tokensAllowedIndex] == _token) {
                return true;
            }
        }
        return false;
