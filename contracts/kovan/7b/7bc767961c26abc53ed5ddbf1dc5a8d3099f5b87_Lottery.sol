...
contract Lottery is AccessControl, VRFConsumerBase {
    ...
    event EthSwappedForLink(uint256 amount);
    ...
    uint256 public lotteryLength = 900;
    // lotto state
    bool public open;
    bool processing;

    uint256 public ticketPrice = 3; // USD
    ...
    */
    ...
    uint256 internal linkThreshold = 12 * 10**18;
    ...
    function getLinkEthBuyAmount() internal view returns (uint256) {
        ...
        ratio = ratio / 10**18;
        uint256 amount = linkThreshold / ratio;
        return amount;
    }
    ...
    function draw() public {
        ...
        if (IERC20(link).balanceOf(address(this)) <= linkThreshold) {
    ...
    function takeFees(uint256 amount, address affiliate)
        ...
        returns (uint256)
    {
        uint256 opsAmount = (amount * opsFee) / n;
        uint256 aFee = (amount * affiliateFee) / n;
        uint256 liquidityFee = (amount * lFee) / n;
        // uint divAmount = amount * divFee / n;
        // swap rewards amount and send to the staking contract
        // pay ops
        if (affiliate == ops) {
            if (balances[ops] == 0) {
                balances[ops] = opsAmount;
            } else {
                balances[ops] = balances[ops] + opsAmount;
            }
        }
        // pay affiliate
        ...
        if (shouldSwapBack()) {
            swapETHForTokens(vicex, deadAddress, liquidityFee);
        }

        return amount - opsAmount - aFee - liquidityFee;
    }
