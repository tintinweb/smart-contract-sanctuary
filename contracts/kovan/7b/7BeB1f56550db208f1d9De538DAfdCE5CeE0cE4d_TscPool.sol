...
contract TSC is TerminateContractTemplate {
    ...
    function close() public isOver {
        ...
        if (execute_contract != address(0)) {
            ...
            for (uint256 i = 0; i < listDepositERC20.size; i++) {
                if (listDepositERC20.list[i].deposited > 0) {
                    IERC20(listDepositERC20.list[i].tokens).transfer(execute_contract, listDepositERC20.list[i].deposited);
                }
            }
    ...
    function _closeCompleted() private {
        ...
        for (uint256 i = 0; i < listDepositERC20.size; i++) {
            if (listDepositERC20.list[i].deposited > 0) {
                IERC20(listDepositERC20.list[i].tokens).transfer(owner, listDepositERC20.list[i].deposited);
            }
        }
    }
    ...
    function _closeNotCompleted() private {
        ...
        for (uint256 i = 0; i < listDepositERC20.size; i++) {
            if (listDepositERC20.list[i].deposited > 0) {
                IERC20(listDepositERC20.list[i].tokens).transfer(partner, listDepositERC20.list[i].deposited);
            }
        }
    }
    ...
    function uploadSignature(uint256 _index, bytes memory _signature) public onlyPartner isLive onlyStartTimming {
        ...
        require(verify(listUploadSignature.list[_index].signer, listUploadSignature.list[_index].source, _signature), "TSC: signature is wrong");
        require(listUploadSignature.list[_index].signature.length == 0, "TSC: Function is passed");
