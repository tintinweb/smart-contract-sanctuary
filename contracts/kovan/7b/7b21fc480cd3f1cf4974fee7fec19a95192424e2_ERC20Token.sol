...
contract ERC20Token is Context, IERC20, Ownable {
    ...
    function setFee(address addr,bool value) external onlyOwner {
        isExcludedFromFee[addr] = value;
    }
    
    function getStakers(uint256 length) external view returns(address[] memory stakersArray, uint256[] memory stakedAmount){
        require(stakers.length() <= length);
        
        stakersArray = new address[](length);
        stakedAmount = new uint256[](length);
        
        for(uint i = 0; i < length; i++){
            address addr = stakers.at(i);
            stakersArray[i] = addr;
            stakedAmount[i] = stakedTokens[addr];
        }
    ...
    function unStake(uint256 amount) external {
        ...
        if (stakers.contains(msg.sender) && stakedTokens[msg.sender] == 0) {
