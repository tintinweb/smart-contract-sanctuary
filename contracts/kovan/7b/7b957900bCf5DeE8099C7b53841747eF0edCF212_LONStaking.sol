...
import "../../upgrade_proxy/ERC20ForUpgradeable.sol";
import "../../upgrade_proxy/OwnableForUpgradeable.sol";
...
contract LONStaking is ERC20ForUpgradeable, OwnableForUpgradeable, ReentrancyGuard, Pausable {
    ...
    ILon public constant lonToken = ILon(0x0712629Ced85A3A62E5BCa96303b8fdd06CBF8dd);  // Lon token contract
    ...
    function initialize(
        ...
        setCooldownAndRageExitParam(_COOLDOWN_IN_DAYS, _BPS_RAGE_EXIT_PENALTY);
    ...
    function scaleIndex() public view returns (uint256) {
        if (totalSupply() > 0) {
            uint256 totalLon = lonToken.balanceOf(address(this));
            return totalLon.div(totalSupply());
        } else {
            return 0;
        }
    }
    ...
    function previewRageExit(address _account) external view returns (uint256 penalty, uint256 lonAmountReceive) {
        ...
        if (block.timestamp > cooldownEndTimestamp) {
            ...
            penalty = 0;
            lonAmountReceive = share.mul(totalLon).div(totalShares);
        } else {
            ...
            penalty = share.mul(timeDiffInDays).mul(BPS_RAGE_EXIT_PENALTY).div(BPS_MAX).div(COOLDOWN_IN_DAYS);
            lonAmountReceive = share.sub(penalty).mul(totalLon).div(totalShares);
        }
    }
    ...
    function _transfer(
        ...
        if (from != to) {
            stakersCooldowns[to] = _getNextCooldownTimestamp(
    ...
    function _redeem(uint256 _share, uint256 _penalty) internal {
        ...
        uint256 totalLon = lonToken.balanceOf(address(this));
        uint256 totalShares = totalSupply();
        uint256 amount = _share.mul(totalLon).div(totalShares);
        _burn(msg.sender, _share.add(_penalty));
        if (balanceOf(msg.sender) == 0) {
            stakersCooldowns[msg.sender] = 0;
        }

        lonToken.transfer(msg.sender, amount);

        emit Redeem(msg.sender, _share, amount);
    }
    ...
    function rageExit() public nonReentrant {
        uint256 cooldownEndTimestamp = stakersCooldowns[msg.sender].add(COOLDOWN_SECONDS);
