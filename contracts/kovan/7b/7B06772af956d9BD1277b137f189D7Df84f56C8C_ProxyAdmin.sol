...
contract ProxyAdmin is Ownable {
    ...
    /**
     * @dev Returns the current version of `proxy`.
     * 
     * Requirements:
     * 
     * - This contract must be the admin of `proxy`.
     */
    function getVersion(TransparentUpgradeableProxy proxy) public view returns (address) {
        // We need to manually run the static call since the getter cannot be flagged as view
        // bytes4(keccak256("version()")) == 0x54fd4d50
        (bool success, bytes memory returndata) = address(proxy).staticcall(hex"54fd4d50");
        require(success);
        return abi.decode(returndata, (address));
    }
     ...
     * - This contract must be the admin of `proxy`.
     */
    function upgrade(TransparentUpgradeableProxy proxy,  uint256 version, address implementation) public onlyOwner {
     ...
     * - This contract must be the admin of `proxy`.
     */
    function upgradeAndCall(TransparentUpgradeableProxy proxy, uint256 version, address implementation, bytes memory data) public payable onlyOwner {
        proxy.upgradeToAndCall.value(msg.value)(version, implementation, data);
    }
}
...
contract TransparentUpgradeableProxy is UpgradeableProxy {
     ...
     */
    constructor(uint256 _initVersion, address _admin) public payable UpgradeableProxy(_initVersion) {
    ...
    function version() external ifAdmin returns (uint256) {
        return _version();
    }
     ...
     */
    function upgradeTo(uint256 newVersion, address newImplementation) external ifAdmin {
     ...
     */
    function upgradeToAndCall(uint256 newVersion, address newImplementation, bytes calldata data) external payable ifAdmin {
        _upgradeTo(newVersion, newImplementation);
