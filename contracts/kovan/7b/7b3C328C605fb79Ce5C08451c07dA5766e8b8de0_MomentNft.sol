...
contract MomentNft is Initializable,
 ...
 NFT721Mint {
    ...
    function initialize(address minter, address admin, address operator) initializer public {
      //  __ERC721_init("Moment", "MOMENT");
       ...
       MomentRole._initializeMomentRole(admin,minter,operator);
     }
    /**
     * @dev Update the configuration of contract and set new Base URI
     */
    function adminUpdateConfig(string memory baseURI) public onlyRole(ADMIN_ROLE) {
     ...
     */
    function supportsInterface(bytes4 interfaceId) public view virtual override(AccessControlUpgradeable,ERC721Upgradeable, NFT721Creator,NFT721Metadata,NFT721Mint) returns (bool) {
    ...
    /**
     * @dev Get Token URI. This will return complete URI for token.
     */
    function tokenURI(uint256 tokenId) public view virtual override(ERC721Upgradeable,NFT721Metadata,NFT721Mint) returns (string memory) {
      return super.tokenURI(tokenId);
    }

    /**
     * @dev Internal function called before transfering tokens.
     */
    function _beforeTokenTransfer (address from, address to, uint256 tokenId) internal virtual override(ERC721Upgradeable,NFT721Metadata,NFT721Mint) {
   ...
   */
    ...
    function _baseURI() internal view virtual override(ERC721Upgradeable,NFT721Metadata,NFT721Mint) returns (string memory) {
      return  super._baseURI();
    }

   /**
   * @dev Burn functionaity to burn tokens.Explicit overrided to address compile errors.
   */
    function _burn(uint256 tokenId) internal virtual override(ERC721Upgradeable,NFT721Creator,NFT721Metadata,NFT721Mint) {
       super._burn(tokenId);
    }
...
/**
 * @notice Allows a contract to leverage the role.
 */
abstract contract MomentRole is Initializable, AccessControlUpgradeable {
  /**
   * @dev Byte32 variable used to provide Minter access.
   */
  ...
  /**
   * @dev Called once after the initial deployment to assign the initial role.
   */
  function _initializeMomentRole(address _admin, address _minter, address _operator) internal initializer {
...
import "../utils/AccountMigration.sol";
import "./MomentRole.sol";

/**
 * @notice Allows each token to be associated with a creator.
 */
abstract contract NFT721Creator is Initializable, ERC721Upgradeable,ERC165StorageUpgradeable,MomentRole,AccountMigration {
...
//onlyAuthorizedAccountMigration(originalAddress, newAddress, signature)
    /**
   * @notice Allows an NFT owner or creator and Moment to work together in order to update the creator
   * to a new account and/or transfer NFTs to that account.
   * @param signature Message `I authorize Moment to migrate my account to ${newAccount.address.toLowerCase()}`
  ...
  ) public onlyRole(OPERATOR_ROLE) onlyAuthorizedAccountMigration(originalAddress,newAddress,signature){
  ...
  /**
     * @dev See {IERC165-supportsInterface}.
     */
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC721Upgradeable,ERC165StorageUpgradeable,AccessControlUpgradeable) returns (bool) {
        return interfaceId == type(IERC721Upgradeable).interfaceId
            || interfaceId == type(IERC721MetadataUpgradeable).interfaceId
            || super.supportsInterface(interfaceId);
    }
 ...
 */
abstract contract NFT721Metadata is ERC721URIStorageUpgradeable,ERC721EnumerableUpgradeable,NFT721Creator {
  using StringsUpgradeable for uint256;
   ...
   */
  function _burn(uint256 tokenId) internal virtual override(ERC721URIStorageUpgradeable,ERC721Upgradeable,NFT721Creator) {
     ...
     * @dev See {IERC165-supportsInterface}.
     */
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC721Upgradeable,ERC721EnumerableUpgradeable,NFT721Creator) returns (bool) {
        return interfaceId == type(IERC721Upgradeable).interfaceId
            || interfaceId == type(IERC721MetadataUpgradeable).interfaceId
            || super.supportsInterface(interfaceId);
    }
...
abstract contract NFT721Mint is Initializable, NFT721Creator, NFT721Metadata {
   ...
   * @dev Explicit override to address compile errors.
   */
  function _baseURI() internal view virtual override(ERC721Upgradeable,NFT721Metadata) returns (string memory) {
      return  super._baseURI();
  }
 ...
 */
abstract contract AccountMigration {
  ...
  /**
   * @dev Confirms the msg.sender is a Moment operator and that the signature provided is valid.
   * @param signature Message `I authorize Moment to migrate my account to ${newAccount.address.toLowerCase()}`
   * signed by the original account.
   */
  modifier onlyAuthorizedAccountMigration(
    ...
    bytes32 hash =
      _toEthSignedMessage(
        abi.encodePacked("I authorize Moment to migrate my account to ", _toAsciiString(newAddress))
      );
    require(
