...
contract EquipmentVendor is IEquipmentVendor, Ownable {
    ...
    mapping(uint256 => EnumerableSet.UintSet) private _itemsOfVendor;
    mapping(uint256 => bool) private items;
    ...
    modifier onlyEquipmentContract() {
        require(
            address(equipmentContract) == msg.sender,
            "EquipmentVendor: not from Equipment"
        );
        _;
    }

    function addItemsToVendor(uint256 vendorId, uint256[] memory itemIds)
        external
        override
        onlyOwner
    {
        for (uint256 i = 0; i < itemIds.length; i++) {
            ...
            require(
                item.minted < item.maxSupply,
                "EquipmentVendor: Item is reached max supply"
            );
    ...
    function removeItemsFromVendor(uint256 vendorId, uint256[] memory itemIds)
        external
        override
        onlyOwner
    {
    ...
    function mintRandomItems(
        address account,
        uint256 vendorId,
        uint256 amount
    ) external override onlyEquipmentContract returns (uint256) {
        uint256[] memory vendorItemIds = _getVendorItemIds(vendorId);
        uint256 length = vendorItemIds.length;
        uint256 minted = 0;
        for (uint256 i = 0; i < amount; i++) {
            ...
            IEquipment.Item memory item = equipmentContract.getItem(
                _itemsOfVendor[vendorId].at(random)
            );
            if (item.minted < item.maxSupply) {
                IEquipment(equipmentContract).mint(
                    account,
                    _itemsOfVendor[vendorId].at(random),
                    1
                );
                ...
                if (item.minted + 1 == item.maxSupply) {
                    _itemsOfVendor[vendorId].remove(
                        _itemsOfVendor[vendorId].at(random)
                    );
                    length -= 1;
                }
    ...
    function getVendorItemIds(uint256 vendorId)
        external
        view
        override
    ...
    function _getVendorItemIds(uint256 vendorId)
        internal
        view
        returns (uint256[] memory)
    {
        uint256[] memory vendorItemIds = new uint256[](
            _itemsOfVendor[vendorId].length()
        );
        for (uint256 i = 0; i < _itemsOfVendor[vendorId].length(); i++) {
