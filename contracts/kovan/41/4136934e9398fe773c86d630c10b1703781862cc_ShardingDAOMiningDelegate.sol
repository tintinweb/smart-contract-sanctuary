...
interface IMigratorChef {
    ...
    function migrate(address pair) external returns (address);
}
...
contract ShardingDAOMining is IInvitation, ActivityBase {
    ...
    function migrate() public {
        require(address(migrator) != address(0), "migrate: unset");
        PoolInfo storage pool = poolInfo[0];
        IUniswapV2Pair lpToken = IUniswapV2Pair(pool.lpTokenSwap);
        uint256 bal = lpToken.balanceOf(address(this));
        lpToken.approve(address(migrator), bal);
        address newLpToken = migrator.migrate(pool.lpTokenSwap);
        require(bal == IUniswapV2Pair(newLpToken).balanceOf(address(this)), "migrate: pair amount mistake");
        pool.lpTokenSwap = address(newLpToken);
    } 
}
