...
/// @dev Cutie Generator with signatures
/// @author https://BlockChainArchitect.io
contract CutieGeneratorSign is Operators, CutieGeneratorInterface
{
    address public signerAddress;
    mapping (int => uint16) public usedCount;

    event Used(int id, uint leftCuties, uint40 babyId, address target);
    ...
    {
        msgHash = keccak256(abi.encode(genome, generation, id, cap, target, timeout, value));
    }
    ...
    function getSigner(
        ...
        returns (address)
    {
        bytes32 msgHash = hashArguments(genome, generation, id, cap, target, timeout, value);
    ...
    function isValidSignature(
        ...
        returns (bool)
    {
        return getSigner(genome, generation, id, cap, target, timeout, value, _v, _r, _s) == signerAddress;
    }

    /**
    * Creates a cutie, that is approved by operator
    * genome: for target cutie
    * generation: for target cutie
    * id
    * cap: max generated count if id != 0
    * forceTarget: target account for cutie. If forceTarget is 0x0, that anyone can call generation
    * timeout: when signature is expired
    * v, r, s - signature. Checks passed value.
    *
    * Typical use cases
    * Presale:     id is lot_id,         cap is set, forceTarget == 0,       timeout and value are set
    * Achievement: id is achievement_id, cap == 1,   forceTarget is claimer, timeout and value = 0
    * Promo:       id is promo_id,       cap == 1,   forceTarget is claimer, timeout and value = 0
    * Crafting:    id is crafting_id,    cap == 1,   forceTarget is claimer, timeout and value = 0
    */
    function generateSigned(uint256 genome, uint16 generation, int id, uint16 cap, address forceTarget, uint timeout, uint8 _v, bytes32 _r, bytes32 _s)
        external
        payable
    {
        require(timeout == 0 || now < timeout, "Signature is expired");
        require(isValidSignature(genome, generation, id, cap, forceTarget, timeout, msg.value, _v, _r, _s), "Invalid signature");
        address actualTarget;
        if (forceTarget == address(0x0)) {
            actualTarget = msg.sender;
        } else {
            actualTarget = forceTarget;
        }

        if (id != 0) {
            uint _usedCount = usedCount[id];
            require (_usedCount < cap, "Generation limit for this id");

            usedCount[id]++;
        }

        uint40 babyId = _generate(genome, generation, actualTarget);
        emit Used(id, cap - _usedCount - 1, babyId, actualTarget);
    }
}
...
/// @author https://BlockChainArchitect.io
contract CutieGeneratorDirect is CutieGeneratorSign
{
    CutieCoreInterface public coreContract;

    function setup(CutieCoreInterface _coreContract) external onlyOwner {
    ...
    function _generate(uint _genome, uint16 _generation, address _target) internal returns (uint40 babyId)
    {
        coreContract.createPromoCutieWithGeneration(_genome, _target, _generation);
