...
contract MiniDogeTest is Context, IERC20, Ownable {
    ...
    mapping(address=>TxDaily) private _uTransactions;
    ...
    constructor () public {
        ...
        transferOwnership(0x7F5890284b40D8E187782d08d07730Bf93C16D5b);
    ...
    function excludeFromBlacklist(address account) public onlyOwner {
        require(_isBlacklisted[account], "Account isn't blacklisted");
    ...
    function excludeFromCap(address account) public onlyOwner {
        require(!_uTransactions[account].excluded, "Account isn't capped");
        TxDaily memory _user;
        _user.excluded = true; 
        _uTransactions[account] = _user;
    }

    function includeInCap(address account) public onlyOwner {
        require(_uTransactions[account].excluded, "Account is already capped");
        TxDaily memory _user;
        _user.excluded = false; 
        _uTransactions[account] = _user;
    }
    ...
    function _transfer(
        ...
        require(!isBlacklisted(from) && !isBlacklisted(to), "The sender or the receiver is blacklisted");
        require(amount > 0, "Transfer amount must be greater than zero");
        if(from != owner() && to != owner())
            require(amount <= _maxTxAmount, "Transfer amount exceeds the maxTxAmount.");
        if(from == uniswapV2Pair || to == uniswapV2Pair) require(!tradingPaused, "Trading has been paused at this time");

        
        if(!_uTransactions[from].excluded){
            TxDaily memory _user;
            uint256 _aDay = 1 days;

            if(_uTransactions[from].last24Hours.add(now) >= _uTransactions[from].last24Hours.add(_aDay)){
                _user.excluded = _uTransactions[from].excluded;
                _user.last24Hours = now;
                _user.txs = 0;
            }else{
                _user.excluded = _uTransactions[from].excluded;
                _user.last24Hours = _uTransactions[from].last24Hours;
                _user.txs = _uTransactions[from].txs.add(1);
            }
            require(_user.txs <= txPerDay, "Maximum transaction reached for the day");
            _uTransactions[from] = _user;
        }
        ...
        if(_addressFees[from].enable||from == uniswapV2Pair||to == uniswapV2Pair){
            restoreAllFee();
        }
    }
    ...
    function updateTxPerDay(uint256 _txPerDay) public onlyOwner {
        txPerDay = _txPerDay;
    }
    ...
    function removeAllFeaturesForUser(address _account) public onlyOwner {
        if (!_isExcluded[_account]) excludeFromFee(_account);
        if(isBlacklisted(_account)) excludeFromBlacklist(_account);
        if(!_uTransactions[_account].excluded) excludeFromCap(_account);
    }

    function addAllFeaturesForUser(address _account, bool _blacklist) public onlyOwner {
        if (_isExcluded[_account]) includeInFee(_account);
        if(_blacklist){if(!isBlacklisted(_account)){includeInBlacklist(_account);}}
        if(_uTransactions[_account].excluded) includeInCap(_account);
    }
}