...
contract Oasis is ERC721Enumerable, Ownable, ERC721Holder, KeeperCompatibleInterface, Accountable {
    ...
    uint256 public GIVEAWAY_DURATION = 79200;
        ...
        Accountable(_splits, _splitWeights)
    {
        ...
        require(_whitelistedContracts.length == GIVEAWAY_WINNERS, "Must whitelist the proper number of collections.");
    ...
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
        ...
        return string(abi.encodePacked('data:application/json;utf8,{"name":"Oasis Coin: #', uint2str(_tokenId), '","image":"', url, '"}'));
    }
    ...
    function setGiveawayDuration(uint256 duration) public onlyOwner {
        GIVEAWAY_DURATION = duration;
    }

    function checkUpkeep(bytes calldata) external view override returns (bool upkeepNeeded, bytes memory) {
        upkeepNeeded = block.timestamp >= startingTimeStamp + GIVEAWAY_DURATION && giveawayState == GIVEAWAY_STATE.OPEN;
    }

    function performUpkeep(bytes calldata) external override {
        require(giveawayState == GIVEAWAY_STATE.OPEN, "Giveaway does not need pulled.");
        require(block.timestamp >= startingTimeStamp + GIVEAWAY_DURATION, "Not ready to pick winners.");
