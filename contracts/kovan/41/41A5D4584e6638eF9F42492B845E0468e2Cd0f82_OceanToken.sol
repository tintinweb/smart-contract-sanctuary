...
contract OceanToken is ERC20, Ownable {
    ...
    uint256 constant TOTAL_SUPPLY = 1_000 * BASE;



    address public wfuse;
    address public swapRouter;
    address public wfusePair;

    bool public autoBatchProcess;

    uint256 public tax = 21;

    mapping(address => bool) public isLpPair;

    mapping(address => bool) public whitlisted;
    ...
    constructor(
        ...
        ).createPair(wfuse_, address(this));

        isLpPair[wfusePair] = true;

        swapHundler = new SwapHandler(wfuse_, swapRouter_);

        // whiteliste wallets
        whitlisted[address(this)] = true;

        whitlisted[address(swapHundler)] = true;
        whitlisted[owner_] = true;

        hodlRewardDistributor = new HODLRewardsDistributor(wfuse);

        hodlRewardDistributor.excludeFromRewards(wfusePair);
        ...
        whitlisted[address(hodlRewardDistributor)] = true;
    ...
    */
    function whitelist(
        ...
        whitlisted[wallet_] = whiteListed_;
    }
    ...
    */
    ...
    function _customTransfer(
        ...
        if (whitlisted[from_] || whitlisted[to_]) {
            ...
            if (reflectionEnabled) {
                ...
                if (taxReserved > 0 && from_ != wfusePair && to_ != wfusePair)
                    processReserves();                
            }
            // transfer 
...
interface IOceanToken {
    function hodlRewardDistributor() external returns(HODLRewardsDistributor);
}
...
contract SwapHandler is Ownable {
    ...
    constructor (
        ...
        swapRouter = swapRouter_;
        wfuse = wfuse_;
        erc20 = IOceanToken(msg.sender);
    }

    /**
        this will swap the amounts to avax/eth/bnb/matic and send them to the respective wallets
     */
    function swapToNativeWrappedToken() isInSwap onlyOwner external {
        uint256 amount = IERC20(owner()).balanceOf(address(this));
        IERC20(owner()).approve(swapRouter, amount);
...
contract HODLRewardsDistributor is Ownable {
    ...
    address immutable public wfuse;
    ...
    /**
        ...
        to all shareholders
        */
    function depositWfuse(
        uint256 amount_
    ) external {
        IWrappedNativeToken(wfuse).transferFrom(msg.sender, address(this), amount_);
        IWrappedNativeToken(wfuse).withdraw(IWrappedNativeToken(wfuse).balanceOf(address(this)));
    }
