...
contract LosslessV2Pool is ILosslessV2Pool, KeeperCompatibleInterface {
    ...
    modifier onlyAfter(uint256 _time) {
        require(
            ...
            "LosslessV2Pool: INVALID TIMESTAMP AFTER"
        );
        _;
    }
    ...
    function startFirstRound() external override {
        require(
            status.isFirstRound == true,
            "LosslessV2Pool: NOT FIRST ROUND!"
        );
        require(
            status.currState == PoolStatus.FirstGame,
            "LosslessV2Pool: WRONG STATUS"
        );
        // modify status variable
        // status.gameRound = 1;
        status.lastUpdateTimestamp = block.timestamp;
        ...
        onlyAfter(status.lastUpdateTimestamp.add(status.durationOfGame))
    {
        ...
        uint256 totalShortPrincipal = _shortToken
            .totalSupply()
            .mul(valuePerShortToken)
            .div(PRECISION);
        uint256 totalLongPrincipal = _longToken
            .totalSupply()
            .mul(valuePerLongToken)
            .div(PRECISION);
        uint256 totalSponsorPrincipal = _sponsorToken
            .totalSupply()
            .mul(valuePerSponsorToken)
            .div(PRECISION);
        uint256 totalPrincipal = totalShortPrincipal.add(
            totalLongPrincipal.add(totalSponsorPrincipal)
        );
        if (IERC20(principalToken).balanceOf(address(this)) < totalPrincipal) {
            ...
            totalInterest = IERC20(principalToken).balanceOf(address(this)).sub(
                    totalPrincipal
                );
        }
        ...
        returns (bool upkeepNeeded, bytes memory performData)
    {
        ...
        if (
            ...
            currState == PoolStatus.Locked &&
            block.timestamp > lastUpdateTimestamp.add(durationOfGame)
        ) {
            upkeepNeeded = true;
        } else {
            upkeepNeeded = false;
        ...
        lock
    {
        ...
        require(
            shortPrincipalAmount > 0 || longPrincipalAmount > 0,
            "LosslessV2Pool: INVALID AMOUNT"
        );
        ...
        if (
            _shortToken.balanceOf(msg.sender) == 0 &&
            _longToken.balanceOf(msg.sender) == 0
        ) {
    ...
    function withdraw(
        ...
        require(
            shortTokenAmount > 0 || longTokenAmount > 0,
            "LosslessV2Pool: INVALID AMOUNT"
        );
        ...
        require(
            userShortTokenBalance >= shortTokenAmount &&
                userLongTokenBalance >= longTokenAmount,
            "LosslessV2Pool: INSUFFICIENT BALANCE"
        );

        // calculate withdraw principal amount
        uint256 shortPrincipalAmount = shortTokenAmount
            .mul(valuePerShortToken)
            .div(PRECISION);
        uint256 longPrincipalAmount = longTokenAmount
            .mul(valuePerLongToken)
            .div(PRECISION);
     ...
     **/
    function swap(bool fromLongToShort, uint256 swapTokenAmount)
        external
        override
        lock
    {
        ...
        uint256 tokenBalanceOfTargetPosition = fromLongToShort
            ? longTokenBalance
            : shortTokenBalance;
        // check user balance
        require(
            swapTokenAmount > 0 &&
                swapTokenAmount <= tokenBalanceOfTargetPosition,
            "LosslessV2Pool: INSUFFICIENT BALANCE"
        );
    ...
    function sponsorDeposit(uint256 principalAmount) external override lock {
        require(
            status.currState != PoolStatus.Terminated,
            "LosslessV2Pool: POOL TERMINATED"
        );
     ...
     **/
    function sponsorWithdraw(uint256 sponsorTokenAmount)
        external
        override
        lock
    {
    ...
    function _updateTokenValue(
        ...
        if (status.isShortLastRoundWinner == true) {
            ...
            valuePerShortToken = _totalShortPrincipal.mul(PRECISION).div(
                _shortToken.totalSupply()
            );
        } else if (status.isShortLastRoundWinner == false) {
            // long win
            _totalLongPrincipal = _totalLongPrincipal.add(totalInterest);
            valuePerLongToken = _totalLongPrincipal.mul(PRECISION).div(
                _longToken.totalSupply()
            );
        }
    ...
    function _getATokenAddress(address _asset)
        private
        view
        returns (address _aToken)
    {
        // protocol data provider
        uint8 number = 1;
        bytes32 id = bytes32(bytes1(number));
        address dataProviderAddress = ILendingPoolAddressesProvider(
            addressProvider
        ).getAddress(id);
     ...
     */
    function claimAAVE(address stakedAAVEAddress_, uint256 amount_)
        external
        override
    {
        require(
            stakedAAVEAddress_ != address(0),
            "LosslessV2Pool: stakedAAVEAddress_ ZERO ADDRESS"
        );
        address feeTo = ILosslessV2Factory(factory).feeTo();
