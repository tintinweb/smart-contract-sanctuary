...
import 'fuzzyswap/contracts/libraries/Constants.sol';
...
/// @title NFT positions
/// @notice Wraps Fuzzyswap  positions in the ERC721 non-fungible token interface
    ...
    SelfPermit
{
    ...
    ) ERC721Permit('Fuzzyswap Positions NFT-V1', 'FUZ-POS', '1') PeripheryImmutableState(_factory, _WBNB, _poolDeployer) {
        ...
        returns (uint256 amount0, uint256 amount1)
    {
        ...
        PoolAddress.PoolKey memory poolKey = _poolIdToPoolKey[position.poolId];
        IFuzzyswapPool pool = IFuzzyswapPool(PoolAddress.computeAddress(poolDeployer, poolKey));
...
import 'fuzzyswap/contracts/interfaces/callback/IFuzzyswapMintCallback.sol';
import 'fuzzyswap/contracts/libraries/TickMath.sol';
...
abstract contract LiquidityManagement is IFuzzyswapMintCallback, PeripheryImmutableState, PeripheryPayments {
    struct MintCallbackData {
        PoolAddress.PoolKey poolKey;
        address payer;
    }

    /// @inheritdoc IFuzzyswapMintCallback
        ...
        )
    {
        ...
        pool = IFuzzyswapPool(PoolAddress.computeAddress(poolDeployer, poolKey));
...
abstract contract PeripheryImmutableState is IPeripheryImmutableState {
    ...
    constructor(address _factory, address _WBNB, address _poolDeployer) {
        factory = _factory;
...
abstract contract PoolInitializer is IPoolInitializer, PeripheryImmutableState {
    ...
    function createAndInitializePoolIfNecessary(
        ...
        pool = IFuzzyswapFactory(factory).getPool(token0, token1);

        if (pool == address(0)) {
            pool = IFuzzyswapFactory(factory).createPool(token0, token1);
            IFuzzyswapPool(pool).initialize(sqrtPriceX96);
        } else {
            (uint160 sqrtPriceX96Existing, , , , ,) = IFuzzyswapPool(pool).globalState();
...
library CallbackValidation {
    ...
    function verifyCallback(
        ...
        address tokenA,
        address tokenB
    ) internal view returns (IFuzzyswapPool pool) {
    ...
    function verifyCallback(address factory, PoolAddress.PoolKey memory poolKey)
        internal
        view
        returns (IFuzzyswapPool pool)
    {
        pool = IFuzzyswapPool(PoolAddress.computeAddress(factory, poolKey));
...
import '../interfaces/external/IWBNB.sol';
...
abstract contract PeripheryPayments is IPeripheryPayments, PeripheryImmutableState {
    ...
    function unwrapWBNB(uint256 amountMinimum, address recipient) external payable override {
        uint256 balanceWBNB = IWBNB(WBNB).balanceOf(address(this));
        require(balanceWBNB >= amountMinimum, 'Insufficient WBNB');

        if (balanceWBNB > 0) {
            IWBNB(WBNB).withdraw(balanceWBNB);
            TransferHelper.safeTransferBNB(recipient, balanceWBNB);
        }
    }
    ...
    /// @inheritdoc IPeripheryPayments
    function refundBNB() external payable override {
        if (address(this).balance > 0) TransferHelper.safeTransferBNB(msg.sender, address(this).balance);
    }
