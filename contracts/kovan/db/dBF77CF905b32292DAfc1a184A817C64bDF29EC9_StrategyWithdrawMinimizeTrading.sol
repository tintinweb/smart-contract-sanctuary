...
// File: StrategyWithdrawMinimizeTrading.sol

contract StrategyWithdrawMinimizeTrading is Ownable, ReentrancyGuard, IStrategy {
  ...
  constructor(address _token, IUniswapV2Router02 _router) public {
  ...
  /// @param data Extra calldata information passed along to this strategy.
  function execute(address user, uint256 /* amount */, uint256 debt, bytes calldata data) external  nonReentrant {
    // 1. Find out what farming token we are dealing with.
    (address fToken, uint256 minFToken) = abi.decode(data, (address, uint256));
    IUniswapV2Pair lpToken = IUniswapV2Pair(factory.getPair(fToken, token));
    // 2. Remove all liquidity back to ETH and farming tokens.
    lpToken.approve(address(router), uint256(-1));
    router.removeLiquidity(token, fToken, lpToken.balanceOf(address(this)), 0, 0, address(this), now);
    ...
    uint256 remainingBalance = token.myBalance();
