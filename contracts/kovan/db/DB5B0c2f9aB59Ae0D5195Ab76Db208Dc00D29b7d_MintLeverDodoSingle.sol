...
abstract contract MintLever is IMintLever, Lock, Owner {
    ...
    function _redeem(address user, address token, uint256 amount) internal returns (uint256) {
        uint256 _pAmount = data.getBondPTokenAmount(user, token);
        if (_pAmount < amount) {
            amount = _pAmount;
        }
        if (0 < amount) {
            data.updatePTokenAmount(user, token, amount, IOperateType.MathOperateType.Sub);
            uint256 _code;
...
contract MintLeverDodoSingle is MintLever {
    ...
    function directClearingPosition(address user, uint256 deadLine) override external lock {
        ...
        uint256 _boudAmount;
        address[] memory _bondTokens = data.getBondToken(user);
        for (uint256 i = 0; i < _bondTokens.length; i++) {
            _boudAmount = _redeem(user, _bondTokens[i], ~uint256(0));
