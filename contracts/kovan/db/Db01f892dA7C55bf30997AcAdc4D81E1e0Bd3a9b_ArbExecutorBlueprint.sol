...
contract ArbExecutorBlueprint {
    ...
    function ST3(address[] memory _operationPath, uint256[] memory _minOuts, uint256 amountIn) public {

        uint256 out1 = Uniswap_EthToToken(_operationPath[1], amountIn, _minOuts[0]);
        uint256 tempOut = out1;

        for(uint i=1; i<(_operationPath.length - 2); i++) {
            uint newOut = Uniswap_TokenToToken(_operationPath[i], _operationPath[i+1], tempOut, _minOuts[i]);
            tempOut = newOut;
        }

        uint256 finalOut = Uniswap_TokenToEth(_operationPath[_operationPath.length - 2], tempOut, _minOuts[_minOuts.length - 1]);
        emit FinalEthOut(finalOut);

    }

    function ST4(Operation memory _operation, uint256 amountIn) public {

        uint256 out1 = Uniswap_EthToToken(_operation.tokens[1], amountIn, _operation.minOuts[0]);
        uint256 tempOut = out1;

        for(uint i=1; i<(_operation.tokens.length - 2); i++) {
            uint newOut = Uniswap_TokenToToken(_operation.tokens[i], _operation.tokens[i+1], tempOut, _operation.minOuts[i]);
            tempOut = newOut;
        }

        uint256 finalOut = Uniswap_TokenToEth(_operation.tokens[_operation.tokens.length - 2], tempOut, _operation.minOuts[_operation.minOuts.length - 1]);
        emit FinalEthOut(finalOut);
