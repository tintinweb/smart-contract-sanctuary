...
contract ApeBetBookMaker is Ownable {
    ...
    function collectRewards() external {
        ...
        for (uint256 i = 0; i < userBetData[msg.sender].length; i++) {
            uint256 odd = 0;
            Bet storage bet = userBetData[msg.sender][i];
            SportEvent memory ev = bet.eventInfo;
        ...
        msg.sender.transfer(totalPayouts);
        emit CollectedReward(totalPayouts);
    }
        ...
        for (uint256 i = 0; i < _event_ids.length; i++) {
            ...
            if (events[_event_ids[i]][affiliate_id].event_status == 2) {
                ...
                for (
                    ...
                    if (
                        bets[event_id][affiliate_id][account].bet_winner !=
                        events[event_id][affiliate_id].winner
                    ) {
                        maxPoolPayout = maxPoolPayout.add(
                            bets[event_id][affiliate_id][account].bet_amount
                        );
                    }
                }
            }
