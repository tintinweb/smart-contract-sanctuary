...
contract HedgeyCallsV1 is ReentrancyGuard {
    using SafeMath for uint;
    ...
    function getOpenOptions() public view returns (int256[] memory _calls) {
        _calls = new int256[](c);
        for (uint i = 0; i < c; i++) {
            Call storage call = calls[i];
            _calls[i] = (call.open && call.expiry > now && !call.exercised) ? int(i) : -1;
        }
    }

    //gets all of the calls that someone is short
    function getShortOptions(address _short) public view returns (int[] memory _calls) {
        _calls = new int[](c);
        for (uint i = 0; i < c; i++) {
            Call storage call = calls[i];
            _calls[i] = (call.short == _short) ? int(i) : -1;
        }
    }


    function getLongOptions(address _long) public view returns (int[] memory _calls) {
        _calls = new int[](c); 
        for (uint i = 0; i < c; i++) {
            Call storage call = calls[i];
            _calls[i] = (call.long == _long) ? int(i) : -1;
        }
    }
    
    function getAllOptions(address _holder) public view returns (int[] memory _calls) {
        _calls = new int[](c);
        for (uint i = 0; i < c; i++) {
            Call storage call = calls[i];
            _calls[i] = (call.long == _holder || call.short == _holder) ? int(i) : -1;
        }
    }



    function getTradeableOptions() public view returns (int[] memory _calls) {
        _calls = new int[](c);
        for (uint i = 0; i < c; i++) {
            Call storage call = calls[i];
            _calls[i] = (call.tradeable && call.expiry > now && !call.exercised) ? int(i) :-1;
        }
    }
    ...
    //function to change an already submitted bid
    function changeNewBid(uint _c, uint _assetAmt, uint _strike, uint _price, uint _expiry) payable public nonReentrant {
        Call storage call = calls[_c];
        require(msg.sender == call.long, "only long can change a bid");
    ...
    function sellOpenOptionToNewBid(uint _c, uint _d) payable public nonReentrant {
        ...
        emit OpenOptionSold( _c, _d, openCall.assetAmt, openCall.strike, openCall.price, openCall.expiry);
    }

    //function for someone to write the call for the open bid
    function sellNewOption(uint _c) payable public nonReentrant {
        Call storage call = calls[_c];
        require(call.short == address(0x0));
        ...
        call.tradeable = false;
        call.open = true;
        emit NewOptionOpened(_c, call.assetAmt, call.strike, call.price, call.expiry);
    }
    ...
    //function to change a new ask
    function changeNewAsk(uint _c, uint _assetAmt, uint _strike, uint _price, uint _expiry) payable public nonReentrant {
        Call storage call = calls[_c];
    ...
    function buyOptionFromNewShort(uint _c, uint _d) payable public nonReentrant {
        ...
        emit OpenShortRePurchased( _c, _d, openShort.assetAmt, openShort.strike, openShort.price, openShort.expiry);
    }
    ...
    function setPrice(uint _c, uint _price, bool _tradeable) public {
        ...
        emit OptionChanged(_c, call.assetAmt, call.strike, call.price, call.expiry);
    }



    //use this function to sell existing calls
    function buyOpenOption(uint _c) payable public nonReentrant {
        Call storage call = calls[_c];
        ...
        call.tradeable = false;
        emit OpenOptionPurchased(_c, call.assetAmt, call.strike, call.price, call.expiry);
    }
    ...
    function rollExpired(uint _c, uint _assetAmt, uint _newStrike, uint _price, uint _newExpiry) payable public nonReentrant {
        ...
        emit OptionRolled(_c, _assetAmt, _newStrike, _price, _newExpiry);
    }
    ...
    //events
    event NewBid(uint _c, uint _assetAmt, uint _strike, uint _price, uint _expiry);
    event NewAsk(uint _c, uint _assetAmt, uint _strike, uint _price, uint _expiry);
    event NewOptionOpened(uint _c, uint _assetAmt, uint _strike, uint _price, uint _expiry);
    event OpenOptionSold(uint _c, uint _d, uint _assetAmt, uint _strike, uint _price, uint _expiry);
    event OpenShortRePurchased(uint _c, uint _d, uint _assetAmt, uint _strike, uint _price, uint _expiry);
    event OpenOptionPurchased(uint _c, uint _assetAmt, uint _strike, uint _price, uint _expiry);
    event OptionChanged(uint _c, uint _assetAmt, uint _strike, uint _price, uint _expiry);
    event OptionExercised(uint _c);
    event OptionRolled(uint _c, uint _assetAmt, uint _strike, uint _price, uint _expiry);
    event OptionReturned(uint _c);
    event OptionCancelled(uint _c);
}