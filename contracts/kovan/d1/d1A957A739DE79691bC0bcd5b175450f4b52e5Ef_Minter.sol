...
contract Minter is ERC721, Ownable {
    ...
    function mintEth(uint256 numTokens) public payable mintableSupply(numTokens) {
        require(msg.value > 0, "Zero ether value");

        uint256 mintPrice = _mintPriceEth.mul(numTokens);
        require(msg.value == mintPrice, "Invalid ether amount");

        mintTokens(numTokens);
    }

    function mintUsd(uint256 amount, uint256 numTokens) public mintableSupply(numTokens) {
        require(amount > 0, "Zero amount sent");

        uint256 mintPrice = _mintPriceUsd.mul(numTokens);
        require(amount == mintPrice, "Invalid usd amount");
