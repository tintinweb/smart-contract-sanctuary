...
contract FatherOfMediators {
    ...
    function poolERC20(address[] calldata tokens) external view returns (address[] memory pool, uint[] memory amounts) {
    ...
    function poolETH() external view returns (address[] memory pool, uint amount, uint count) {
        address[] memory tmp = new address[](mediators.length);
        for (uint i=0; i<mediators.length; i++) {
    ...
    //         result[i] = erc20.balanceOf(childs[i]);
    //         IMediator(childs[i]).withdrawERC20(token, payout, result[i]);
    //     }
    // }
    ...
    // function grabERC20List(address[] calldata tokens, address[] calldata childs) external isOwnerOrManager {
    //     require(tokens.length == childs.length, "Different arguments length");
    //     for (uint i=0; i<tokens.length; i++) {
    //         IMediator(childs[i]).withdrawERC20(tokens[i], payout, IERC20(tokens[i]).balanceOf(childs[i]));
    //     }
    // }
    
    function grabETH(address[] calldata childs) external isOwner returns (uint[] memory result) {
        result = new uint[](childs.length);
        for (uint i=0; i<childs.length; i++) {
            result[i] = IMediator(childs[i]).withdrawETH(payout);
        }
    }
