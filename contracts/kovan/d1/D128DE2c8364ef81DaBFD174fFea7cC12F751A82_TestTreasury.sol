...
contract TestTreasury is Context, ERC20, AdamOwned {
    ...
    event ExchangeEve(int256 evePrice, int256 coinPrice, string tokenName, uint256 quatityToExchange);
    ...
    function exchangeEVE(address to, ERC20 token, uint256 quantity) public payable returns (int256) {
        ...
        int256 coinPrice = getXUsdPrice(tokenName);

        emit ExchangeEve(evePrice, coinPrice, tokenName, quantity);

        //UNKNOW / USD / EDT / USD ==> UNKNOW / EDT
        int256 perEDT = _priceConverter.getDerivedPrice(
            coinPrice,
            8,
            evePrice,
            8,
        ...
        uint256 amount = uint256(perEDT) * quantity / (10 ** 8);
