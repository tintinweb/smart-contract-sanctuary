...
contract SportsDataIOChainlink is ChainlinkClient, IChainLinkOracleSportData {
    using Chainlink for Chainlink.Request;
    using String for string;
    event GetData(bytes32 _id);
    event GetInfoCompetition(bytes32 _id);
    event ReceiveData(bytes32 _id, uint256[] _data);

    mapping(bytes32 => bool) public isRequest;
    mapping(bytes32 => bool) public isResponse;
    mapping(bytes32 => uint256[]) public results;
    mapping(bytes32 => Competition) public data;
    ...
    //This contract requires 0.1 Link Token fee to execute
    function requestData(uint256 _competitionId)
        public
        override
        returns (bytes32)
    {
        ...
        // request.add("id", _competitionId);
        request.add("date", "2021-06-12");
        request.add("teamName", "Sporting Kansas City");
    ...
    function fulfill(bytes32 _requestId, uint256[] memory _data) public {
        require(
        ...
        returns (bytes32)
    {
        ...
        // request.add("id", _competitionId);
        bytes32 myId = sendChainlinkRequestTo(
            chainlinkOracleAddress(),
            request,
            payment
        );
        isRequest[myId] = true;
        emit GetInfoCompetition(myId);

        //mock data
        data[myId] = Competition(
            "12345",
            "1243",
            "soccer",
            "France vs China",
    ...
    function fulfillMultipleParameters(
        ...
        string memory leagueId,
        string memory sportTypeResponse,
        string memory matchNameResponse,
        string memory player1Response,
        string memory player2Response
