...
contract Cagnotte {
    ...
    constructor(address payable _creator, address _receiver, uint _delay, uint _minCap) {
        creator = _creator;
        receiver = _receiver;
...
contract CryptoLeetchi {
    ...
    mapping(string => address) public ipfsCagnotte;
    ...
    function startCagnotte(address payable _creator, address _receiver, string memory _multihash, uint _delay, uint _minCap) external returns (Cagnotte) {
        ...
        Cagnotte newCagnotte = new Cagnotte(_creator, _receiver, targetTime, _minCap); 
        ipfsCagnotte[_multihash] = address(newCagnotte);
        cagnottes.push(newCagnotte);
        
        emit cagnotteStarted(address(newCagnotte), _creator, _receiver, _multihash, targetTime, _minCap);
