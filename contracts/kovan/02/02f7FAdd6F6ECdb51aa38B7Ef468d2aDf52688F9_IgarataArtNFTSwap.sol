...
// File: contracts\IgarataArtNFTSwap.sol
...
contract IgarataArtNFTSwap is Ownable, ERC1155Holder {
    using SafeMath for uint256;
	using EnumerableSet for EnumerableSet.AddressSet;
    ...
    IIgarataArtNFT public igarataArtNFT;
	...
	uint256 public totalEarning; /* Total IgarataArt Token */
	...
	function initialize(address _tokenAddress, address _nftAddress, uint256 _fee, address _feeAddress, string memory _name, string memory _uri) external onlyOwner {
		...
		igarataArtNFT = IIgarataArtNFT(_nftAddress);
		feeAddress = _feeAddress;
		swapFee = _fee;
		collections.push(_nftAddress);
		collectionCreators[_nftAddress] = msg.sender;
		emit CollectionCreated(_nftAddress, msg.sender, _name, _uri, true);//"IgarataArt","https://nft.app/igarataArt/"
	}
    ...
    function setNFTAddress(address _address) external onlyOwner {
		require(_address != address(0x0), "invalid address");
        igarataArtNFT = IIgarataArtNFT(_address);
    }
	...
	function createCollection(string memory _name, string memory _uri, bool bPublic) public returns(address collection) {
		bytes memory bytecode = type(IgarataArtNFT).creationCode;
        ...
        IIgarataArtNFT(collection).initialize(_name, _uri, msg.sender, bPublic);
	...
	function delist(uint256 _id) external {
		...
		IIgarataArtNFT(items[_id].collection).safeTransferFrom(address(this), items[_id].owner, items[_id].token_id, items[_id].balance, "delist from IgarataArt Marketplace");
		items[_id].balance = 0;
	...
	function addItems(uint256 _id, uint256 _amount) external {
		...
		IIgarataArtNFT(items[_id].collection).safeTransferFrom(msg.sender, address(this), items[_id].token_id, _amount, "add items to IgarataArt Marketplace");
	...
	function removeItems(uint256 _id, uint256 _amount) external {
		...
		IIgarataArtNFT(items[_id].collection).safeTransferFrom(address(this), msg.sender, items[_id].token_id, _amount, "remove items from IgarataArt Marketplace");
    ...
    function buy(uint256 _id, uint256 _amount) external {
		...
		uint256 igarataArtAmount = item.price.mul(_amount);
		uint256 token_balance = igarataArtToken.balanceOf(msg.sender);
		require(token_balance >= igarataArtAmount, "insufficient token balance");

		// transfer IgarataArt token to admin
		if(swapFee > 0) {
			require(igarataArtToken.transferFrom(msg.sender, feeAddress, igarataArtAmount.mul(swapFee).div(PERCENTS_DIVIDER)), "failed to transfer admin fee");
		}
		// transfer IgarataArt token to creator
		if(item.creatorFee > 0) {
			require(igarataArtToken.transferFrom(msg.sender, item.creator, igarataArtAmount.mul(item.creatorFee).div(PERCENTS_DIVIDER)), "failed to transfer creator fee");
		}
		// transfer IgarataArt token to owner
		uint256 ownerPercent = PERCENTS_DIVIDER.sub(swapFee).sub(item.creatorFee);
		require(igarataArtToken.transferFrom(msg.sender, item.owner, igarataArtAmount.mul(ownerPercent).div(PERCENTS_DIVIDER)), "failed to transfer to owner");

		// transfer NFT token to buyer
		IIgarataArtNFT(items[_id].collection).safeTransferFrom(address(this), msg.sender, item.token_id, _amount, "buy from IgarataArt Marketplace");
		...
		totalEarning = totalEarning.add(igarataArtAmount);
