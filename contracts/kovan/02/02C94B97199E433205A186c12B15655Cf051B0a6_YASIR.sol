...
contract YASIR is ERC721Enumerable, AccessControl, Ownable {

    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;

    string _baseTokenURI = "https://testnet.api.marketplace.chainguardians.ar/token/";

    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");

    address public upgradedToAddress = address(0);
    uint256 internal _cap = 6666666;
    ...
    function mintBactchTokens(address _mintTo, uint8 _count) external returns (bool) {
        for(uint8 i = 0; i < _count ; i++) {
            _tokenIds.increment();
            mint(_mintTo, _tokenIds.current());
        }
