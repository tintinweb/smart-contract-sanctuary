...
contract RewardEscrow is Ownable {
     ...
     */
    function numVestingEntries(address pool, address token, address account)
        public
        view
        returns (uint256)
    {
    ...
    function getVestingQuantity(
        ...
        return getVestingScheduleEntry(pool, token, account, index)[QUANTITY_INDEX];
    }

    /**
     * @notice Obtain the index of the next schedule entry that will vest for a given user.
     */
    function getNextVestingIndex(address pool, address token, address account)
        public
        view
        returns (uint256)
    {
     ...
     * @return A pair of uints: (timestamp, token quantity). */
    function getNextVestingEntry(address pool, address token, address account)
        public
        view
     ...
     */
    function getNextVestingTime(address pool, address token, address account)
        external
        view
        returns (uint256)
    {
     ...
     */
    function getNextVestingQuantity(address pool, address token, address account)
        external
        view
        returns (uint256)
    {
     ...
     */
    function checkAccountSchedule(address pool, address token, address account)
        public
        view
        returns (uint256[520] memory)
    {
        ...
        for (uint256 i = 0; i < schedules; i++) {
            uint256[2] memory pair = getVestingScheduleEntry(pool, token, account, i);
            _result[i * 2] = pair[0];
    ...
    function appendVestingEntry(
        ...
        emit VestingEntryCreated(pool, token, account, block.timestamp, quantity);
    }
