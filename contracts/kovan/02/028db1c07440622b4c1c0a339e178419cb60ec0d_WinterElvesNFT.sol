...
contract WinterElvesNFT is ERC721URIStorageUpgradeable, OwnableUpgradeable {
    ...
    function preSale(address _to, uint256 _amount, bytes32[] memory _proof)
        ...
        totalSupplyCheck
    {
        require(_verify(_leaf(_to), rootWhitelisted, _proof), "Invalid merkle proof");

        require(_amount <= 3, "Wrong amount of NFTs");
        require(
            super.balanceOf(_to) + 1 <= 3,
            "You have maximum amount of NFTs"
        );
        require(totalSupply <= totalSupply + _amount, "Total supply does not have `_amount` of free tokens");
    ...
    function preSaleExclusive(address _to, uint256 _amount, bytes32[] memory _proof)
        public
        payable
        totalSupplyCheck
    {
        require(_verify(_leaf(_to), rootExclusive, _proof), "Invalid merkle proof");
        require(_amount <= 4, "Wrong amount of NFTs");
        require(
            super.balanceOf(_to) + 1 <= 4,
            "You have maximum amount of NFTs"
        );
