...
contract Precog {
    ...
    function setIsStaking(bool _isStaked) external {
        if(_isStaked){
            require(availableAmount[msg.sender] > 0, "PrecogV2: STAKING_AMOUNT_NOT_ENOUGH");
        }
    ...
    function withdrawLiquidate(address futureContract, address to, address user) external {
        ...
        if(isStaked[user] == true){
            ...
            isStaked[user] == false;
            amount = liquidatedAmount[user][futureContract] + returnStakingAmount - feeWithdrawByUSDC;
        } else {
    ...
    function reinvest(address futureContract, address user) external {
        ...
        if(isStaked[user] == true){
            ...
            uint returnStakingAmount = IStakingPool(stakingPool).withdraw(user);
            isStaked[user] == false;
            availableAmount[msg.sender] += (liquidatedAmount[user][futureContract] + returnStakingAmount);
        } else {
    ...
    function liquidate(address futureContract, address user) public {
        ...
        if(isStaked[user] == true){
            if(IStakingPool(stakingPool).getEndTime() > expiryDate){
                ...
                require(IERC20(usdc).balanceOf(address(this)) >= revenue, "PrecogV2: insufficient balance");
