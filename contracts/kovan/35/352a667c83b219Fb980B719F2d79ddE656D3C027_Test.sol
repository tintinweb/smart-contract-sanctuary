...
contract Test {
    ...
    event Sender(address indexed user);
    event Address(address indexed user);
    ...
    function deposit(address _erc20Contract,uint256 _amount) public returns(bool){
    ...
    function withdraw(address _erc20Contract, uint256 _amount) public returns(uint256){
        IERC20(_erc20Contract).approve(lendingPoolAddress,_amount);
        uint256 amountWithdrawn = iLendingPool.withdraw(_erc20Contract, _amount, msg.sender);
        return amountWithdrawn;
    }

    function checkCollateralValueInEth() public view returns(uint256){
