...
contract Controller is Initializable, IController, Permissions{
    ...
    // LIMIT is price deviation which is acceptable
    // solhint-disable var-name-mixedcase    
    uint256 public override LIMIT = 50;

    // solhint-disable var-name-mixedcase
    uint256 public override MINT_FEE = 80;
    ...
    function setLimit(uint256 amount) public override onlyGovernor {
        require(amount < BASIS_POINT);
        LIMIT = amount;
        emit LimitUpdate(amount);
    }

    function setMintFee(uint256 amount) public override onlyGovernor{
        require(amount < BASIS_POINT);        
        MINT_FEE = amount;
        emit MintFeeUpdate(amount);
    }
    ...
    function addPoolWrapper(address poolWrapper) public override onlyGovernor {
        ...
        emit PoolWrapperAdd(poolWrapper);
    }
...
/// @title Access control module for Controller
/// @author Artha Protocol
contract Permissions is IPermissions, AccessControl {
    ...
    bytes32 public constant POOL_WRAPPER_ROLE = keccak256("POOL_WRAPPER_ROLE");
    ...
    constructor() public {
        ...
        _setRoleAdmin(BALANCER_ROLE, GOVERN_ROLE);
        _setRoleAdmin(POOL_WRAPPER_ROLE, GOVERN_ROLE);
    }
    ...
    modifier onlyBalancer() {
        require(isBalancer(msg.sender), "Permissions: Caller is not a balancer");
        _;
    }
    ...
    /// @notice grants balancer role to address
    /// @param balancer new balancer
    function grantBalancer(address balancer) public override onlyGovernor {
        grantRole(BALANCER_ROLE, balancer);
    }

    /// @notice grants balancer role to address
    /// @param poolWrapper new poolWrapper
    function grantPoolWrapper(address poolWrapper) public override onlyGovernor {
        grantRole(POOL_WRAPPER_ROLE, poolWrapper);
    }
    ...
    /// @notice revokes balancer role from address
    /// @param balancer ex balancer
    function revokeBalancer(address balancer) public override onlyGovernor {
        revokeRole(BALANCER_ROLE, balancer);
    }

    /// @notice revokes balancer role from address
    /// @param poolWrapper ex poolWrapper
    function revokePoolWrapper(address poolWrapper) public override onlyGovernor {
        revokeRole(POOL_WRAPPER_ROLE, poolWrapper);
    }
    ...
    /// @notice checks if address is a balancer
    /// @param _address address to check
    /// @return true _address is a balancer
    function isBalancer(address _address) public view override returns (bool) {
        return hasRole(BALANCER_ROLE, _address);
    }

    /// @notice checks if address is a balancer
    /// @param _address address to check
    /// @return true _address is a balancer
    function isPoolWrapper(address _address) public view override returns (bool) {
        return hasRole(POOL_WRAPPER_ROLE, _address);
    }
...
contract Artha {
    ...
    string public constant name = "Artha";

    /// @notice EIP-20 token symbol for this token
    // solhint-disable-next-line const-name-snakecase
    string public constant symbol = "ARTHA";
    ...
    // solhint-disable-next-line const-name-snakecase
    uint public totalSupply = 1_000_000_000e18; // 1 billion ARTHA
    ...
    /**
     * @notice Construct a new ARTHA token
    ...
    function setMinter(address minter_) external {
        require(msg.sender == minter, "ARTHA: only the minter can change the minter address");
    ...
    function mint(address dst, uint rawAmount) external {
        require(msg.sender == minter, "ARTHA: only the minter can mint");
        require(dst != address(0), "ARTHA: cannot transfer to the zero address");

        // mint the amount
        uint96 amount = safe96(rawAmount, "ARTHA: amount exceeds 96 bits");
        uint96 safeSupply = safe96(totalSupply, "ARTHA: totalSupply exceeds 96 bits");
        totalSupply = add96(safeSupply, amount, "ARTHA: totalSupply exceeds 96 bits");

        // transfer the amount to the recipient
        balances[dst] = add96(balances[dst], amount, "ARTHA: transfer amount overflows");
    ...
    function approve(address spender, uint rawAmount) external returns (bool) {
        ...
        if (rawAmount == uint(-1)) {
            ...
            amount = safe96(rawAmount, "ARTHA: amount exceeds 96 bits");
        }
    ...
    external {
        ...
        require(signatory != address(0), "ARTHA: invalid signature");
        require(signatory == owner, "ARTHA: unauthorized");
        // solhint-disable-next-line not-rely-on-time
        require(block.timestamp <= deadline, "ARTHA: signature expired");
    ...
    function transferFrom(address src, address dst, uint rawAmount) external returns (bool) {
        ...
        if (spender != src && spenderAllowance != uint96(-1)) {
            uint96 newAllowance = sub96(spenderAllowance, amount, "ARTHA: transfer amount exceeds spender allowance");
    ...
    function delegateBySig(address delegatee, uint nonce, uint expiry, uint8 v, bytes32 r, bytes32 s) public {
        ...
        address signatory = ecrecover(digest, v, r, s);
        require(signatory != address(0), "ARTHA: invalid signature");
        require(nonce == nonces[signatory]++, "ARTHA: invalid nonce");
        // solhint-disable-next-line not-rely-on-time
        require(block.timestamp <= expiry, "ARTHA: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber) public view returns (uint96) {
        require(blockNumber < block.number, "ARTHA: not yet determined");
    ...
    function _transferTokens(address src, address dst, uint96 amount) internal {
        require(src != address(0), "ARTHA: cannot transfer from the zero address");
        require(dst != address(0), "ARTHA: cannot transfer to the zero address");

        balances[src] = sub96(balances[src], amount, "ARTHA: transfer amount exceeds balance");
        balances[dst] = add96(balances[dst], amount, "ARTHA: transfer amount overflows");
    ...
    function _moveDelegates(address srcRep, address dstRep, uint96 amount) internal {
        if (srcRep != dstRep && amount > 0) {
            if (srcRep != address(0)) {
                ...
                uint96 srcRepNew = sub96(srcRepOld, amount, "ARTHA: vote amount underflows");
            ...
            if (dstRep != address(0)) {
                ...
                uint96 dstRepNew = add96(dstRepOld, amount, "ARTHA: vote amount overflows");
    ...
    function _writeCheckpoint(address delegatee, uint32 nCheckpoints, uint96 oldVotes, uint96 newVotes) internal {
      uint32 blockNumber = safe32(block.number, "ARTHA: block number exceeds 32 bits");
...
contract AUSD is IAUSD, ERC20, ControllerRef{
    ...
    function permit(
        ...
        // solhint-disable-next-line not-rely-on-time
        require(deadline >= block.timestamp, "ArthaToken: EXPIRED");
        bytes32 digest =
            keccak256(
                abi.encodePacked(
                    "\x19\x01",
        ...
        require(
            ...
            "ArthaToken: INVALID_SIGNATURE"
        );
        _approve(owner, spender, value);
    }    

    /// @notice burn AUSD tokens from caller
    /// @param amount the amount to burn
    function burn(uint256 amount) public override(IAUSD) {
