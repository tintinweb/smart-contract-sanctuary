...
contract EuroMatch is ContractOwnable{
    ...
    uint256 public _kRatio  = 1000000;
    uint256 public _mintRaito = 100;

    event selectPositionActived(address indexed accountAddress,string  matchNo , uint8 position, uint256 amountBNB);
    event redeemRewardActived(address indexed accountAddress,string  matchNo , uint8 position, uint256 amountBNB, uint256 amountToken);
    ...
    function selectPosition(string memory matchNo , uint8 position ) public payable returns (uint256) {
        ...
        require (_userBalance.matchName(matchNo) == true ,"Incorect match name");
        require (position < 4 ,"Incorect Position");
        require (block.timestamp < _userBalance.matchLock(matchNo) , "Time Over! cannot deposit" );
        uint256 _newBalance =  _userBalance.deposit{value: msg.value }(matchNo,msg.sender,position,msg.value);   
        emit selectPositionActived (msg.sender,matchNo,position,msg.value);
        return _newBalance;
    ...
    function redeemReward(string memory matchNo,uint8 position , uint256 withdrawAmount) public  returns (uint256,uint256 ) {
        require (_userBalance.matchName(matchNo) == true ,"Incorect match name");
        require (position < 4 ,"Incorect Position");
        require ((block.timestamp < _userBalance.matchLock(matchNo) ) ||  (_userBalance.matchUnLock(matchNo) == true), "temporary close for withdraw " );
       ...
       uint256 _reward = 0;
        if(position == 1)
        {
            ...
            uint256 _khome = _userBalance.kHome(matchNo);
            if(_khome == 0)
            {
            ...
            {
               _withdrawReceive = (withdrawAmount * _khome ) / _kRatio;
               if(_khome == _kInitial)
               {
                   uint256 _tax = withdrawAmount - _withdrawReceive;
                   uint256 _depTax = _tax / 2 ;
                   uint256 _rewardTax = _tax - _depTax; 
                  _userBalance.depositTax(_rewardTax,_depTax);
               }
            }
        }
        else if (position == 2)
        {
            ...
            uint256 _kdraw = _userBalance.kDraw(matchNo);
            if(_kdraw == 0)
            {
             _reward = withdrawAmount * _mintRaito;
            }
            ...
            {
                 _withdrawReceive = (withdrawAmount * _kdraw ) / _kRatio;
                 if(_kdraw == _kInitial) {
                   uint256 _tax = withdrawAmount - _withdrawReceive;
                   uint256 _depTax = _tax / 2 ;
                   uint256 _rewardTax = _tax - _depTax; 
        ...
        else if (position==3)
        {
            ...
            uint256 _kaway = _userBalance.kAway(matchNo);
            if(_kaway== 0)
            {
               _reward = withdrawAmount * _mintRaito;
            }
            ...
            {
                 _withdrawReceive = (withdrawAmount * _kaway ) / _kRatio;
                if(_kaway == _kInitial){
                   uint256 _tax = withdrawAmount - _withdrawReceive;
                   uint256 _depTax = _tax / 2 ;
                   uint256 _rewardTax = _tax - _depTax; 
        ...
        emit redeemRewardActived(msg.sender , matchNo, position ,_withdrawReceive ,_reward);
        return (_withdrawReceive,_reward);
    }
    
    function createMatch(string memory matchNo ,uint256 lockTime ) public onlyOwner{
        require (_userBalance.matchName(matchNo) == false ,"Duplicate match name");
        _userBalance.setKMatch(matchNo,_kInitial,_kInitial,_kInitial);
        _userBalance.setMatchStart(matchNo,lockTime);
    } 
    
    function updateResult(string memory matchNo , uint8 winPosition) public onlyOwner{
        require (_userBalance.matchName(matchNo) == true ,"Match name Incorect");
        require (block.timestamp > (_userBalance.matchLock(matchNo) + 6000) , "Too eraly to update result" );
        ...
        if(_win == 0)
        {
            ...
            _kdraw = 0;
            _kaway = 0;
            _depFee =  (_total * _depPercent) / 100;
            _reward =  _total - _depFee;
            _userBalance.depositTax(_reward,_depFee);
        }
        else
        {
            ...
            if(_winRatio > 11){
            _depFee =  (_total * _depPercent) / 100;
            _reward =  (_total * _rewardPercent) / 100;
            _userBalance.depositTax(_reward,_depFee);
            _total = _total - _reward;
            _total = _total - _depFee;
            }
             if(winPosition==1) {
                 _khome = (_total * _kRatio) / _win;
                 _kdraw = 0;
                 _kaway = 0;
            }
            else if (winPosition==2){
                 _khome = 0;
                 _kdraw = (_total * _kRatio) / _win;
                 _kaway = 0;
            }
            else if (winPosition==3){
                 _khome = 0;
                 _kdraw = 0;
                 _kaway = (_total * _kRatio) / _win;
            }

            
        }
            _userBalance.setKMatch(matchNo,_khome,_kdraw,_kaway);
            _userBalance.setMatchUnlockStatus(matchNo,true);
    } 
   



    function matchInfo(string memory matchNo, address account) public view virtual  
        returns (uint256 homeTotal,uint256 drawTotal,uint256 awayTotal, uint256 userHomeBalance , uint256 userDrawBalance ,uint256 userAwayBalance , uint256 timelock ,bool isFinish)  {
       homeTotal = _userBalance.homeTotal(matchNo);
       drawTotal = _userBalance.drawTotal(matchNo);
       awayTotal = _userBalance.awayTotal(matchNo);
       userHomeBalance = _userBalance.homeBalance(matchNo,account);
       userDrawBalance = _userBalance.drawBalance(matchNo,account);
       userAwayBalance = _userBalance.awayBalance(matchNo,account);
       timelock = _userBalance.matchLock(matchNo);
       isFinish = _userBalance.matchUnLock(matchNo);
   }
   
    function rewardInfo(string memory matchNo, address account) public view virtual 
        returns(uint256 userHomeBalance, uint256 userDrawBalance, uint256 userAwayBalance , uint256 kHome , uint256 kDraw , uint256 kAway){
       userHomeBalance = _userBalance.homeBalance(matchNo,account);
       userDrawBalance = _userBalance.drawBalance(matchNo,account);
       userAwayBalance = _userBalance.awayBalance(matchNo,account);
       kHome =  _userBalance.kHome(matchNo);
       kDraw =  _userBalance.kDraw(matchNo);
       kAway =  _userBalance.kAway(matchNo);
    }
    
    function matchHistory(address account) public view virtual returns(string [] memory){
        return _userBalance.matchAccount(account);
    }
    
    function blockTime() public view virtual returns(uint256){
        return block.timestamp;
    }


    function checkMatchStatus(string memory matchNo ) public view virtual  returns (bool,bool){
        return (_userBalance.matchName(matchNo),_userBalance.matchUnLock(matchNo)) ;
    }
