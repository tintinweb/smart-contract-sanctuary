...
// File: contracts/interface/IYouswapFactoryV2.sol
...
interface BaseStruct {
    ...
    /**
    专区类型: You专区，单币挖矿区，lp挖矿区，联合多挖区 //TODO
    */
    enum PoolCategory {
        YOU_CATEGORY, //单币挖矿
        LP_TOKEN_CATEGORY, //lp挖矿
        SINGLE_TOKEN_CATEGORY, //单币定期挖矿
        MULTIPLE_TOKEN_CATEGORY //lp定期挖矿
    }
    ...
    /**
    矿池质押信息
    每个矿池都是一个这样的结构，所有的矿池是id=>PoolStakeInfo的map
     */
    struct PoolStakeInfo {
        ...
        // DistributionMode distributionMode; //固定发放，递减发放
        // uint256 rewardDecRate; //每日递减百分比, 10: 1%, 15: 1.5%, 100: 10%, 1000: 100%
    ...
    /**
    矿池奖励信息
    每个矿池都至少有一个挖矿奖励接口：PoolRewardInfo
    多挖时每个矿池有多个：PoolRewardInfo
    每个多挖的币种，都涉及到token地址，每个区块奖励多少个，一共奖励多少个，已经奖励了多少个，每个算力奖励多少个
    这个结构仅描述这个奖励币种的信息，和挖矿人没有关系
     */
    ...
    /**
    用户质押信息
    用户可以选择一个矿池进行stake操作，从那一刻起，就开始享受挖矿的收益，
    stake的币种（lp或单币）就会化作算力，进行挖矿
    由于算力总是从矿池开始哪一个时刻开始记录的，因此质押的时候，要把历史的算力当成负债
    待领取部分是给upper1，upper2级的，对于自己而言，这个字段用不到：自己是通过rewardPerShare动态计算收益的，而不是要在一个字段中
     */
...
interface IYouswapFactoryV2Core is BaseStruct {
    function initialize(address _owner, address _platform, uint256 _benefitRate, address _invite) external;
...
挖矿
 */
interface IYouswapFactoryV2 is BaseStruct {
    ////////////////////////////////////////////////////////////////////////////////////
    /**
    修改OWNER
     */
    ...
    */
    ...
    算力占比
     */
    function powerScale(uint256 poolId, address user)
        external
        view
    ...
    待领取的奖励
     */
    // function pendingRewardV2(uint256 poolId, address user)
    //     external
    //     view
    //     returns (address[] memory, uint256[] memory);

    function pendingRewardV3(uint256 poolId, address user)
        external
        view
        returns (
            address[] memory,
    ...
    通过token查询矿池编号
     */
    // function poolNumbers(address token)
    //     external
    //     view
    ...
    质押数量范围
     */
    function stakeRange(uint256 poolId)
        external
        view
        returns (uint256, uint256);
    ...
    */
    ...
    /*
    质押币种，总锁仓，地址数，结束时间，质押数量到算力系数
    */
    function getStakePoolInfo(uint256 poolId) external view returns (address, uint256, uint256, uint256, uint256);
    ...
    function addPool(
        ...
        uint256[] memory poolParams,
        PoolLockType poolType,
        // DistributionMode distributionType,
        address[] memory tokens,
        uint256[] memory rewardTotals,
        uint256[] memory rewardPerBlocks
    ) external;
...
// File: contracts/implement/YouswapFactoryV2.sol
...
contract YouswapFactoryV2 is IYouswapFactoryV2 {
    ...
    IYouswapFactoryV2Core public core;
    ...
    /**
    @notice clone YouSwapFactoryV2初始化
    @param _owner 项目方
    @param _platform FactoryCreator平台
    @param _benefitRate 抽成比例，直接透传
    @param _invite clone邀请合约，直接透传
    ...
    */
    function initialize(address _owner, address _platform, uint256 _benefitRate, address _invite, address _core) external {
        ...
        core = IYouswapFactoryV2Core(_core);
        core.initialize(address(this), _platform, _benefitRate, _invite);
        owner = _owner;
        platform = _platform;
        _setOperateOwner(_owner, true); //给owner授权运营权限
        initialized = true;
    }
    ...
    */
    function stake(uint256 poolId, uint256 amount) external override {
        ...
        require((ZERO != poolStakeInfo.token) && (poolStakeInfo.startTime <= block.timestamp), "YouSwap:POOL_NOT_EXIST_OR_MINT_NOT_START"); //是否开启挖矿
    ...
    function unStake(uint256 poolId, uint256 amount) external override {
        ...
        if (poolStakeInfo.poolType == PoolLockType.SINGLE_TOKEN_FIXED || poolStakeInfo.poolType == PoolLockType.LP_TOKEN_FIXED) {
            require(block.timestamp >= poolStakeInfo.lockUntil, "YouSwap:POOL_LOCKED_UNSTAKE_DENIED!");
            //TODO发送事件比较好
        }
        require((ZERO != poolStakeInfo.token) && (poolStakeInfo.startBlock <= block.number), "YouSwap:POOL_NOT_EXIST_OR_MINING_NOT_START");
        require((0 < amount) && (userStakeInfo.amount >= amount), "YouSwap:BALANCE_INSUFFICIENT");
    ...
    批量提取奖励
     */
    function withdrawRewards(uint256[] memory _poolIds) external override {
        require((0 < _poolIds.length) && (50 >= _poolIds.length), "YouSwap:PARAMETER_ERROR_TOO_SHORT_OR_LONG");
        for (uint256 i = 0; i < _poolIds.length; i++) {
            withdrawReward(_poolIds[i]);
        }
    }
    ...
    算力占比
     */
    function powerScale(uint256 poolId, address user) external view override returns (uint256, uint256) {
        BaseStruct.PoolStakeInfo memory poolStakeInfo = core.getPoolStakeInfo(poolId);
        // console.log("totalPower:", poolStakeInfo.totalPower);
                ...
                if (poolStakeInfo.startBlock <= block.number) {
                    ...
                    if (userStakeInfo.invitePendingRewards.length == _poolRewardInfos.length) {
                        if (0 < poolStakeInfo.totalPower) {
                            ...
                            // console.log("poolRewardInfo.rewardPerShare:", poolRewardInfo.rewardPerShare);
                        }
                        // console.log("userStakeInfo.stakeRewardDebts[i]:", userStakeInfo.stakeRewardDebts[i]);
    ...
    矿池ID
     */
    function poolIdsV2() external view override returns (uint256[] memory) {
        return core.poolIdsV2();
    }

    /**
    质押数量范围
     */
    function stakeRange(uint256 poolId) external view override returns (uint256, uint256) {
        BaseStruct.PoolStakeInfo memory poolStakeInfo = core.getPoolStakeInfo(poolId);
        if (ZERO == poolStakeInfo.token) {
            return (0, 0);
        }
    ...
    function getPowerRatio(uint256 poolId) override external view returns (uint256) {
        BaseStruct.PoolStakeInfo memory poolStakeInfo = core.getPoolStakeInfo(poolId);
        return poolStakeInfo.powerRatio;
    }

    /*
    质押币种，总锁仓，地址数，结束时间，质押数量到算力系数
    */
    function getStakePoolInfo(uint256 poolId) external view override returns (address, uint256, uint256, uint256, uint256) {
        BaseStruct.PoolStakeInfo memory poolStakeInfo = core.getPoolStakeInfo(poolId);
        BaseStruct.PoolRewardInfo[] memory poolRewardInfo = core.getPoolRewardInfo(poolId);
        // for (uint256 i = 0; i < poolStakeInfo.length; i++) {
        ...
        return (
            ...
            poolStakeInfo.lockUntil, 
            poolStakeInfo.powerRatio);
    }
    ...
    */
    ...
    function addPool(
        ...
        require((0 < tokens.length) && (10 >= tokens.length) && (tokens.length == rewardTotals.length) && (tokens.length == rewardPerBlocks.length), "YouSwap:PARAMETER_ERROR_REWARD");

        for (uint256 i = 0; i < tokens.length; i++) {
            //创建矿池时直接将奖励币种打入矿池
            IERC20(tokens[i]).safeTransferFrom(msg.sender, address(core), rewardTotals[i]);
        }
    ...
    function setRewardPerBlock(
        ...
        address token,
        uint256 rewardPerBlock
    ) external override onlyOperater {
        core.setRewardPerBlock(poolId, token, rewardPerBlock);
    }

    /**
    修改矿池总奖励 //TODO 得修改，token得打入进来才行
     */
    function setRewardTotal(
        uint256 poolId,
        address token,
        ...
        core.setRewardTotal(poolId, token, rewardTotal);
    }
