...
contract Liquidator is ILiquidator, Multicall, Manager {
    ...
    function getLendingPoolAddressProvider() public view returns(address) {
    ...
    function getBatchUserData(address[] calldata users) override public view returns (uint256[] memory hlfs) {
        ILendingPoolAddressesProvider addressProvider = ILendingPoolAddressesProvider(lendingPoolAddressProvider);
    ...
    function getBatchUserBalances(address[] calldata users, address[] calldata tokens) public view returns(UserBalances[] memory usersBalances){
        for(uint256 i = 0; i < users.length; i++) {
            usersBalances[i].user = users[i];
            for(uint256 j = 0; j < tokens.length; j++){
                uint256 balance = IERC20(tokens[j]).balanceOf(users[i]);
                usersBalances[i].balances[j] = Balance(tokens[j], balance);
            }
        }
        ...
        returns (address, address)
    {
        ...
        if (IERC20(_reserve).balanceOf(address(this)) < _purchaseAmount) {
            revert(
                string(
                    abi.encodePacked(
                        "multicall-skip: Not enough balance of ",
                        Strings.toHexString(uint160(_reserve), 20),
                        " to cover user debt ",
                            Strings.toString(_purchaseAmount)
                    )
                )
            );
        }
...
interface ILiquidator {
  function getBatchUserData(address[] calldata users) external returns (uint[] memory hlfs);
...
//                if (abi.decode(result[:14], (string)) == "multicall-skip") continue;
