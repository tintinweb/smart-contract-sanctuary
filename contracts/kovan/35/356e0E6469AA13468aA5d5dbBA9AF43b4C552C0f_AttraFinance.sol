...
contract AttraCampaign is KeeperCompatibleInterface, FundManager, Lottery {
    ...
    event Advanced(uint256 id, AttraLib.CampStatus newStatus);
    ...
    function _advanceStatus(AttraLib.CampStatus newStatus) private {
        status = newStatus;
        emit Advanced(_myId, status);
    }
        ...
        returns (AttraLib.CampStatus)
    {
        ...
        _advanceStatus(AttraLib.CampStatus.OPEN);
    ...
    function _closeCampaign() private {
        ...
        if (successful) {
            ...
            startLottery();
            _advanceStatus(AttraLib.CampStatus.SUCCESS_PREPARE_LOTTERY);
        } else {
            // finalize immediately with refunds
            //console.log(" * * * Refunding...");
            _refundAll();
            _advanceStatus(AttraLib.CampStatus.FAIL_SETTLED);
        }
    }

    function _onReceivedRandomness() internal virtual override {
        _advanceStatus(AttraLib.CampStatus.SUCCESS_READY_FOR_LOTTERY);
    ...
    function _completeLottery() private {
        ...
        _advanceStatus(AttraLib.CampStatus.SUCCESS_SETTLED_WINNERS);
    }
    ...
    function settleBeneficiaryAndTokens() external onlyOwner returns (uint256) {
        ...
        _advanceStatus(AttraLib.CampStatus.SUCCESS_SETTLED_BENEFICIARY);
        return amount;
    }
...
contract AttraFinance is Ownable {
    ...
    // function totalCollectedUSDInCampaign(uint256 _id)
    //     public
    //     view
    //     returns (uint256)
    // {
    //     AttraCampaign camp = AttraCampaign(campaigns[_id]);
    //     return camp.totalCollectedInUSD();
    // }
