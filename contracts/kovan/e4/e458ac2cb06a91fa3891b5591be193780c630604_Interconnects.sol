...
contract PolygonQuickSwapIntegration is Controlled, IAMMIntegration {
    ...
    function configureStaking(uint32 poolId, address stakingRewards)
        external
        onlyManager
    {
    ...
    function configureStakingOverwrite(uint32 poolId, address stakingRewards)
        external
        onlyManager
    {
    ...
    function _configureStaking(
        ...
        if (overwrite != true) {
            require(
                address(stakings[poolId]) == address(0),
                "Staking already configured"
            );
        }
        ...
        require(
            IStakingRewards(stakingRewards).stakingToken() == pairAddress,
            "Incorrect staking address"
        );

        stakings[poolId] = IStakingRewards(stakingRewards);
    ...
    function harvestYield() external override onlyController {
        ...
        if (yieldAmount > 0) {
            IERC20MetadataUpgradeable(wmaticAddress).safeTransfer(
                moduleMap.getModuleAddress(Modules.YieldManager),
                yieldAmount
            );
            emit YieldReceived(yieldAmount);
    ...
    function harvestYieldByPool(
        uint32 poolId,
        uint256 tokenRatioX1000, // price ratio for rewards token!
        bool convertToWmatic
    ) external onlyManager {
        stakings[poolId].getReward();

        if (convertToWmatic) {
            // check and convert
            address rewardsToken = stakings[poolId].rewardsToken();
            uint256 balance = IERC20MetadataUpgradeable(rewardsToken).balanceOf(
                address(this)
            );

            if (balance > 0) {
                checkPriceCeiling(rewardsToken, wmaticAddress, tokenRatioX1000);
                uint256[] memory amounts = swapExactInput(
                    rewardsToken,
                    wmaticAddress,
                    address(this),
                    balance,
                    getAmountOutMinimum(rewardsToken, wmaticAddress, balance)
                );
                yieldBalances[poolId] += amounts[1];
            }
        }
    ...
    function getLiquidityToWithdraw(
        ...
        address tokenAddress,
        uint256 amount
    ) internal returns (uint256 liquidityToWithdraw) {
        Pool memory pool = getPool(poolId);
        ...
        liquidityToWithdraw =
            (stakings[poolId].balanceOf(address(this)) * (liquidityPercent)) /
            100;
    }
    ...
    function swapExactInput(
        ...
        uint256 amountIn,
        uint256 amountOutMin
    ) public onlyController returns (uint256[] memory) {
        address[] memory path = new address[](2);
        path[0] = tokenIn;
        path[1] = tokenOut;
        uint256 deadline = block.timestamp;
        ...
        returns (uint256)
    {
        ...
        return stakings[poolId].earned(address(this));
    }
    ...
    function checkPriceCeiling(
        ...
        address pairAddress = address(pairFor(tokenA, tokenB));
        require(pairAddress != address(0), "QuickSwapIntegration:BadPair");
...
interface IPolygonQuickSwapIntegration is IIntegration {
    ...
    function getPairInfo(address pair)
        external
        view
        returns (QuickLiquidityPosition memory);
