    ...
    Ownable
{
    ...
    function reserve(string calldata _backerName)
        ...
        whenNotPaused
    {
        require(isGoing && _tokenIds.current() < MAX_SUPPLY, "Sale ended");
        ...
        returns (bool upkeepNeeded, bytes memory performData)
    {
        if (!isGoing) {
            upkeepNeeded =
                block.timestamp >= START_BLOCK_TIMESTAMP &&
                _tokenIds.current() < MAX_SUPPLY;
            performData = abi.encodeWithSelector(
                this.updateIsGoing.selector,
                true
            );
        } else {
            upkeepNeeded = _tokenIds.current() == MAX_SUPPLY;
            performData = abi.encodeWithSelector(
                this.updateIsGoing.selector,
                false
            );
