...
contract ElasticPool is Ownable, ReentrancyGuard, ERC20, Pausable {
	...
	*/
	...
	/** 
	* @dev Sets VPStorage contract address, can be called only once
	* @param _vpStorage - New worker contract address
	*/
	function setVPStorage (
		address _vpStorage
	)
		external
		onlyOwner
	{
		require (vpStorage == address(0), "vpStorage address is already set");
		inToken.safeApprove(_vpStorage,  type(uint256).max);
		vpStorage = _vpStorage;
	}
	...
	****************************************/
	...
	/**
	 * @dev Returns holdtime in seconds for given user address
	 * @param _address address of the user
	 */
	function getHoldTime(address _address)
		external
		view
		returns (uint256 _holdTime)
	{
		uint256 _releaseTime = userData[_address].lastTime + holdTime; 
		if(_releaseTime > block.timestamp)
			_holdTime = _releaseTime - block.timestamp;
	}
