...
contract NtentArt is CustomERC721Metadata {
    ...
    function mint(address _to, uint256 _projectId, address _by) external returns (uint256 _tokenId) {
        require(isMintWhitelisted[msg.sender], "Must mint from whitelisted minter");
    ...
    function burn(address ownerAddress, uint256 tokenId) external returns(uint256 _tokenId) {
        require(isMintWhitelisted[msg.sender], "Must burn from whitelisted minter");
    ...
    function toggleProjectIsPaused(uint256 _projectId) public onlyArtistOrWhitelisted(_projectId) {
        projects[_projectId].paused = !projects[_projectId].paused;
    }

    function addProject(string memory _projectName, address _artistAddress, uint256 _pricePerTokenInWei, address _purchaseContract, bool _acceptsMintPass, uint256 _mintPassProjectId, bool _dynamic) public onlyWhitelisted {
    ...
    function updateProjectPurchaseContractInfo(uint256 _projectId, address _projectPurchaseContract, bool _acceptsMintPass, uint256 _mintPassProjectId) onlyUnlocked(_projectId) onlyWhitelisted public {
    ...
    function updateProjectDataContractInfo(uint256 _projectId, address _projectDataContract) onlyUnlocked(_projectId) onlyWhitelisted public {
        projects[_projectId].purchaseContract = _projectDataContract;
    }
    ...
    function toggleProjectUseIpfsForStatic(uint256 _projectId) onlyArtistOrWhitelisted(_projectId) public {
    ...
    function overrideTokenDynamicImageWithIpfsLink(uint256 _tokenId, string memory _ipfsHash) onlyArtistOrWhitelisted(tokenIdToProjectId[_tokenId]) public {
        staticIpfsImageLink[_tokenId] = _ipfsHash;
    }

    function clearTokenIpfsImageUri(uint256 _tokenId) onlyArtistOrWhitelisted(tokenIdToProjectId[_tokenId]) public {
    ...
    function projectTokenInfo(uint256 _projectId) view public returns (address artistAddress, uint256 pricePerTokenInWei, uint256 invocations, uint256 maxInvocations, bool active, address purchaseContract, bool acceptsMintPass, uint256 mintPassProjectId) {
    ...
    function tokenURI(uint256 _tokenId) external view onlyValidTokenId(_tokenId) returns (string memory) {
        ...
        string memory data = "";
        if(projects[tokenIdToProjectId[_tokenId]].dataContract != address(0)){
            NtentData ntentData = NtentData(projects[tokenIdToProjectId[_tokenId]].dataContract);
            data = Strings.strConcat("|", ntentData.data(_tokenId));
        }
        
        if (bytes(staticIpfsImageLink[_tokenId]).length > 0) {
            return Strings.strConcat(projects[tokenIdToProjectId[_tokenId]].projectBaseIpfsURI, staticIpfsImageLink[_tokenId], data);
        }

        if (!projects[tokenIdToProjectId[_tokenId]].dynamic && projects[tokenIdToProjectId[_tokenId]].useIpfs) {
            return Strings.strConcat(projects[tokenIdToProjectId[_tokenId]].projectBaseIpfsURI, projects[tokenIdToProjectId[_tokenId]].ipfsHash, data);
        }

        return Strings.strConcat(projects[tokenIdToProjectId[_tokenId]].projectBaseURI, Strings.uint2str(_tokenId), data);
    }
}