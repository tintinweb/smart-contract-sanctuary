...
contract LimitOrderManager is
    ...
    uint32 public override gasUsageMonitor;
    ...
    mapping(address => uint56) private poolIds;

    /// @dev Pool keys by pool ID, to save on SSTOREs for position data
    mapping(uint56 => PoolAddress.PoolKey) private poolIdToPoolKey;

    /// @dev The ID of the next token that will be minted. Skips 0
    uint176 private nextId;

    /// @dev The ID of the next pool that is used for the first time. Skips 0
    uint56 private nextPoolId;

    /// @dev last monitor index + 1 ; always > 0
    uint16 public nextMonitor;
    ...
    function placeLimitOrder(LimitOrderParams calldata params)
        ...
        _mint(msg.sender, (_tokenId = nextId++));
            ...
            uint16 _selectedIndex = _selectMonitor();
            ...
            limitOrders[_tokenId] = LimitOrder({
                ...
                poolId: cachePoolKey(_poolAddress,_poolKey),
    ...
    function processLimitOrder(
        ...
        PoolAddress.PoolKey memory poolKey = poolIdToPoolKey[limitOrder.poolId];
    ...
    function cachePoolKey(address pool, PoolAddress.PoolKey memory poolKey) private returns (uint56 poolId) {
        poolId = poolIds[pool];
        if (poolId == 0) {
            poolIds[pool] = (poolId = nextPoolId++);
    ...
    function setGasUsageMonitor(uint32 _gasUsageMonitor) external {
        isAuthorizedController();
        gasUsageMonitor = _gasUsageMonitor;
    }
    ...
    returns (uint256 _serviceFee, uint256 _monitorFee) {
        ...
        _monitorFee = quoteKROM(
            _targetGasPrice.mul(gasUsageMonitor).mul(_noOrders)
        );
    ...
    function _selectMonitor() internal view returns (uint16 _selectedIndex) {

        uint16 monitorLength = uint16(monitors.length);
...
interface IOrderManager {
    ...
    function placeLimitOrder(LimitOrderParams calldata params) external payable returns (
            uint256 _tokenId,
            int24 _tickLower,
            int24 _tickUpper,
            uint128 _liquidity,
    ...
    function gasUsageMonitor() external view returns (uint32);
}
