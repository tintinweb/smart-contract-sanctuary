...
contract MintLeverRouter is IMintLeverRouter, ConfigBase {
    ...
    function getTVL(address mintLever, uint8 decimal) public view returns (uint256) {
        uint256 usd;
        uint256 _usd;
    ...
    function getAPRs(address[] memory mintLevers, uint8 decimal) external view returns (uint256[] memory) {
        uint256[] memory aprs = new uint256[](mintLevers.length);
        for (uint256 i = 0; i < mintLevers.length; i++) {
            aprs[i] = getAPRInternal(mintLevers[i], decimal);
        }
        return aprs;
    }

    function getAPRInternal(address mintLever_, uint8 decimal) internal view returns (uint256) {
        IEarning earning = IEarning(MintLever(mintLever_).earning());
        (address[] memory tokens, uint256[] memory rewardPerBlocks) = earning.getRewardTokenInfos();
        uint256 result = 0;
        uint256 temp = 0;
        uint256 price = 0;
        for (uint i = 0; i < tokens.length; i++) {
            (, price) = config.getAssetPrice().getPriceUSDV2(tokens[i], decimal);
            temp = rewardPerBlocks[i].mul(price).mul(2102400);
            result = result.add(temp);
        }
        
        uint256 tvl = getTVL(mintLever_, decimal);
        result = result.div(tvl);
        return result;
    }
}