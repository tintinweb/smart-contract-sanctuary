  ...
  ERC721HolderUpgradeable
{
  ...
  function addDomainsToSell(
    ...
    require(labels.length == metadataUris.length, "Arrays are not equal size");

    // Take each pair and create a domain to sell from it
    for (uint256 i = 0; i < labels.length; ++i) {
      domainsForSale.push(
        Domain({label: labels[i], metadataUri: metadataUris[i]})
      );
    }
  }
  ...
  function purchaseDomains(uint8 count) public payable virtual {
    ...
    require(
      domainsPurchasedByAccount[msg.sender] + count <= maxPurchasesPerAccount,
    ...
    for (uint8 i = 0; i < numPurchased; ++i) {
      ...
      uint256 domainId =
        controller.registerSubdomainExtended(
          parentDomainId,
          domainsForSale[purchasedIndices[i]].label,
          address(this),
