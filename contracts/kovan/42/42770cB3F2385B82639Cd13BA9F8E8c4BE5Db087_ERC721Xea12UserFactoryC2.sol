...
import "../erc-721/ERC721Xea12User.sol";
import "@openzeppelin/contracts/proxy/BeaconProxy.sol";
import "@openzeppelin/contracts/access/Ownable.sol";

/**
 * @dev This contract is for creating proxy to access ERC721Xea12User token.
 *
 * The beacon should be initialized before call ERC721Xea12UserFactory constructor.
 *
 */
contract ERC721Xea12UserFactoryC2 is Ownable {
    address public beacon;

    event Create721Xea12UserProxy(address proxy);
    ...
    function createToken(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI, address[] memory operators, uint salt) external {
        ...
        ERC721Xea12User token = ERC721Xea12User(address(beaconProxy));
        token.transferOwnership(_msgSender());
        emit Create721Xea12UserProxy(beaconProxy);
    }
    ...
    function getData(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI, address[] memory operators) pure internal returns(bytes memory){
        return abi.encodeWithSelector(ERC721Xea12User(0).__ERC721Xea12User_init.selector, _name, _symbol, baseURI, contractURI, operators);
    }
}
...
contract ERC721Xea12User is OwnableUpgradeable, ERC721BurnableUpgradeable, ERC721Lazy, HasContractURI {

    event CreateERC721Xea12User(address owner, string name, string symbol);

    function __ERC721Xea12User_init(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI, address[] memory operators) external initializer {
        _setBaseURI(baseURI);
        ...
        emit CreateERC721Xea12User(_msgSender(), _name, _symbol);
    }
