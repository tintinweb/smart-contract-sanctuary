...
contract LMTerminal is Initializable, OwnableUpgradeable {
    ...
    address manager;
    address xTokenFeeManager;
    address stakingRewardsImplementation;

    RewardEscrow rewardEscrow;
    CLRDeployer clrPoolDeployer;
    IxAssetCLR public clr;
    StakedCLRToken public stakedToken;
    ...
    function initialize(
        ...
        xTokenFeeManager = _xTokenFeeManager;
     ...
     * @param totalRewardsAmount total amount of rewards in the program
    ...
    function deployIncentivizedPool(
        ...
        // Deploy CLR
        IxAssetCLR clrPool = IxAssetCLR(clrPoolDeployer.deployCLRPool());
        StakedCLRToken _stakedToken = new StakedCLRToken(
            "StakedCLRToken",
            symbol,
            address(clrPool)
        );
        ...
        // If there's vesting after unstaking, add the rewards contract to the escrow contract
        // This should be set up in script for now
        // if (vestingPeriod > 0) {
        //     rewardEscrow.addRewardsContract(address(clrPool));
        //     rewardEscrow.addRewardsToken(rewardToken);
        // }
        // Setup rewards initializer and duration
        clrPool.setRewardsDistribution(address(this));
    ...
    function initiateRewardsProgram(
        ...
        clrPool.notifyRewardAmount(totalRewardAmount);
 ...
 * Deploys CLR proxies pointing to main CLR Beacon address
 * CLR Beacon points to implementation address
 */
contract CLRDeployer {
    address clrBeacon;

    constructor(address _clrBeacon) {
        clrBeacon = _clrBeacon;
    }

    function deployCLRPool() public returns (address pool) {
        xAssetCLRProxy clrInstance = new xAssetCLRProxy(clrBeacon);
