...
contract StakingPool is IStakingPool, PermissionGroup {
    ...
    constructor(
        ...
        kabyContract = kabyAddr;
        rewardDistributorContract = distributorAddr;
    }

    function setKabyContract(IKaby kabyAddr) external onlyOwner {
        require(address(kabyAddr) != address(0));
        kabyContract = kabyAddr;
    }
    ...
    function stake(uint heroId, uint amount, StakingType stakingType) external override {
        ...
        _stake(account, heroId, amount, stakingType);
    }
    ...
    function upgradeStarForHero(uint heroId) external override {
        ...
        kabyContract.upgradeStar(heroId, 1);
    ...
    function _stake(address account, uint heroId, uint amount, StakingType stakingType) private {
        StakingData storage stakingHero = stakingData[account][heroId];
        ...
        if (stakingType == StakingType.noLockTime) {
            ...
            uint currentStar = kabyContract.getHeroStar(heroId);
        ...
        emit Staked(heroId, account, amount, stakingHero.lockedTime);
    }
...
interface IStakingPool {
    event Staked(uint indexed heroId, address indexed account, uint amount, uint lockedTime);
    ...
    /**
     * @notice Stake KABY crystals for farming hero star upgrade & KABY token.
     */
    function stake(uint heroId, uint amount, StakingType stakingType) external;

    /**
     * @notice Unstake KABY crystals from a hero.
     */
