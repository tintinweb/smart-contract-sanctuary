...
/**
 * @title LiquidatorController
 * @author Kine
 */
contract LiquidatorController is Ownable {
    /// @notice liquidator list
    ...
    constructor(address[] memory initLiquidators) public {
        addLiquidators(initLiquidators);
    }

    /// @notice check if valid liquidator
    function isValid(address liquidatorToVerify) external view returns (bool){
    ...
    function addLiquidators(address[] memory liquidatorsToAdd) public {
        for(uint i = 0; i < liquidatorsToAdd.length; i++){
            address liquidationToAdd = liquidatorsToAdd[i];
            liquidators[liquidationToAdd] = true;
            emit LiquidatorAdded(liquidationToAdd);
        }
    }
