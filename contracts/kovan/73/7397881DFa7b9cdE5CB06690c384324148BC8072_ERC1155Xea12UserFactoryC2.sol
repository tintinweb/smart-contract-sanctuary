...
import "../erc-1155/ERC1155Xea12User.sol";
import "@openzeppelin/contracts/proxy/BeaconProxy.sol";
import "@openzeppelin/contracts/access/Ownable.sol";

/**
 * @dev This contract is for creating proxy to access ERC1155UserXea12 token.
 *
 * The beacon should be initialized before call ERC1155Xea12UserFactory constructor.
 *
 */
contract ERC1155Xea12UserFactoryC2 is Ownable {
    address public beacon;

    event Create1155Xea12UserProxy(address proxy);
    ...
    function createToken(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI, address[] memory operators, uint salt) external {
        ...
        ERC1155Xea12User token = ERC1155Xea12User(address(beaconProxy));
        token.transferOwnership(_msgSender());
        emit Create1155Xea12UserProxy(beaconProxy);
    }
    ...
    function getData(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI, address[] memory operators) pure internal returns(bytes memory){
        return abi.encodeWithSelector(ERC1155Xea12User(0).__ERC1155Xea12User_init.selector, _name, _symbol, baseURI, contractURI, operators);
    }
}
