...
contract MerkleAirdropDistributor is Ownable {
    ...
    event NewAirdrop(address indexed token, uint256 amount, uint256 index);
    ...
    function addAirdrop(AirdropMetadata memory _metadata) external {
        require(_metadata.totalAmount > 0, "MerkleAirdropistributor: amount must be > 0");

        airdropMetadata.push(_metadata);
        IERC20(_metadata.token).safeTransferFrom(msg.sender, address(this), _metadata.totalAmount);

        emit NewAirdrop(_metadata.token, _metadata.totalAmount, airdropMetadata.length - 1);
    }
