...
contract LiquidityPoolFarm is Ownable {
    ...
    event Test(uint256 accAdyPerShare, uint256 adyReward, uint256 daiSupply);
    ...
    function deposit(uint256 _amount) public {
        ...
        if (user.amount > 0) {
            ...
            if (pending > 0) {
                emit Test(accAdyPerShare, pending, 0);
    ...
    function updatePool() public {
        ...
        emit Test(accAdyPerShare, adyReward, daiSupply);
    ...
    function pendingAdy(address _user) external returns (uint256) {
        ...
        if (block.number > lastRewardBlock && lpSupply != 0) {
            ...
            _accAdyPerShare = accAdyPerShare.add(
                adyReward.mul(1e18).div(lpSupply).mul(1e12)
            );
        }
        emit Test(
            _accAdyPerShare,
            user.amount.mul(_accAdyPerShare).div(1e12),
            user.amount.mul(_accAdyPerShare).div(1e12).sub(user.rewardDebt)
        );
