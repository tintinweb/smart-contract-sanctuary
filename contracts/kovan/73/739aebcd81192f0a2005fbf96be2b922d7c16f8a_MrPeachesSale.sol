...
// File: contracts/PeachesToken.sol

pragma solidity 0.6.12;

contract StableToken is ERC20("Stable Token", "DAI"), Ownable {
   ...
   function burn(address _from, uint256 _amount) public onlyOwner {
    ...
    function setupDecimals(uint8 decimals_) public onlyOwner {
...
contract MrPeachesSale {
    ...
    uint public privateSaleRate;
    uint public publicSaleRate;
    uint public uniswapRate;
    ...
    constructor(uint _privateSaleRate, uint _publicSaleRate, uint _uniswapRate, uint _privateSaleCap, uint _publicSaleCap, uint _multiplier, IERC20 _peaches, IERC20 _stableCoin, address _oracle, address _uniswapRouter) public {
    ...
    function depositEth() public payable {
        ...
        if (privateSaleClosed == false) {
            ...
            require(tokens >= 500000000000000000000 && tokens <= 50000000000000000000000, "Private sale limit");
            privateSaleSold = privateSaleSold.add(tokens);
            require(privateSaleSold <= privateSaleCap, "Cap reached");
            ...
            require(block.timestamp >= publicSaleOpenedAt && block.timestamp <= publicSaleOpenedAt.add(14 days), 'Time was reached');
            
            //if (block.timestamp <= publicSaleOpenedAt.add(2 hours)) {
                //require(whiteListed[msg.sender], "Not whitelisted");  
            //}
            
            int price_ = 51502800000;
            
            uint amount = msg.value.mul(uint(price_)).div(multiplier);
            ...
            require(tokens >= 500000000000000000000 && tokens <= 50000000000000000000000, "Public sale limit");
            publicSaleSold = publicSaleSold.add(tokens);
            require(publicSaleSold <= publicSaleCap, "Cap reached");
    ...
    function closePublicSale() external {
        ...
        require(block.timestamp > publicSaleOpenedAt.add(14 days) || publicSaleSold == publicSaleCap, 'Too early');
    ...
    function generateLiquidity() external {
        ...
        require(block.timestamp > publicSaleClosedAt.add(5 minutes), 'Too early');
    ...
    function emergencyWithdrawCoins() external {
        ...
        require(publicSaleClosedAt != 0, 'Public sale open');
        require(liquidityGeneratedAt == 0, 'Liquidity generated');
        require(block.timestamp > publicSaleClosedAt.add(60 minutes).add(2 days), 'Too early');
    ...
    require(msg.sender == developer, "Developer only");
        ...
        require(publicSaleClosedAt != 0, 'Public sale open');
        require(block.timestamp > publicSaleClosedAt.add(60 minutes).add(30 days), 'Too early');
