...
contract FuzzyswapFactory is IFuzzyswapFactory {
    ...
    function createPool(
        ...
        IDataStorageOperator dataStorage = IDataStorageOperator(address(new DataStorageOperator()));

        pool = IFuzzyswapPoolDeployer(poolDeployer).deploy(address(dataStorage), address(this), token0, token1);

        dataStorage.setPool(address(pool));
...
contract DataStorageOperator {
    using DataStorage for DataStorage.Observation[65535];

    DataStorage.Observation[65535] public observations;
...
/// @dev Instances of stored dataStorage data, "observations", are collected in the dataStorage array
...
/// Observations are overwritten when the full length of the dataStorage array is populated.
/// The most recent observation is available, independent of the length of the dataStorage array, by passing 0 to observe()
library DataStorage {
    ...
    /// @notice Initialize the dataStorage array by writing the first slot. Called once for the lifecycle of the observations array
    /// @param self The stored dataStorage array
    ...
    /// @notice Writes an dataStorage observation to the array
