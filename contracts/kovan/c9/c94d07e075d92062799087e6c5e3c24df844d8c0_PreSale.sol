...
contract TANGI is ERC20, ERC20Detailed {
  using SafeERC20 for IERC20;
  using Address for address;
  using SafeMath for uint256;
  
  
  constructor () ERC20Detailed("TANGI", "TANGI", 18)
  {
    _totalSupply = 250000000 *(10**uint256(18));
...
contract PreSale is Owned{
  ...
  constructor(uint256 _rate, address payable _wallet, TANGI _token) {
    require(_rate > 0);
  ...
  function _forwardFunds() internal {
    ...
    function queryERC20Balance(address _tokenAddress, address _addressToQuery) view public onlyOwner returns (uint) {
        uint256 contractBalance = ERC20Interface(_tokenAddress).balanceOf(_addressToQuery);
