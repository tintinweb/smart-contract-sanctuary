...
// File: contracts/ERC20StockContract.sol
...
contract ERC20StockContract is ERC20, Pausable, AccessControl {
     ...
     */
    constructor(address defaultAdminAddress, string memory name, string memory symbol, address companyRegistryAddress ) ERC20(name, symbol) {
   ...
   //
    ...
    function multiMint(address[] memory to, uint[] memory amount) public returns(bool) {
        require(hasRole(MODERATOR_ROLE, msg.sender));
        require(to.length == amount.length, "Different lenght of arrays!");
    ...
    function transferFromFrozenAccount(address from, address to, uint amount) public returns (bool) {
        require(hasRole(MODERATOR_ROLE, msg.sender));
        
        require(BlockhainRegistryInterface(getBlockchainRegistryAddress()).isAddressWhitelisted(to), "Account is not whitelisted!");
    ...
    function checkSendingAllowance(address from, address to) public view returns (bool) {
        (bool combinedAllowance, , , , ,) = checkSendingAllowanceSteps(from, to);
        require(combinedAllowance, "Transfers are disabled!");
        return combinedAllowance;
    }
    
    function checkSendingAllowanceSteps(address from, address to) public view returns (bool, bool, bool, bool, bool, bool) {
        bool companyTxtAllownace = _companyRegistryInstance.getCompanyActivity(getCompanyId());
        bool tokenTxtAllowance = !paused();
        bool userWhitelisted = BlockhainRegistryInterface(getBlockchainRegistryAddress()).isAddressWhitelisted(to);
        bool userFrozen = !BlockhainRegistryInterface(getBlockchainRegistryAddress()).isAddressFrozen(from);
        bool accountFrozen = !_frozenAccounts[from];
        bool combinedAllowance = (companyTxtAllownace && tokenTxtAllowance && userWhitelisted && userFrozen && accountFrozen);
        return (combinedAllowance, companyTxtAllownace, tokenTxtAllowance, userWhitelisted, userFrozen, accountFrozen);
    }
    
    function getCompanyId() public view returns (uint) {
        return _companyRegistryInstance.getCompanyIdByToken(address(this));
    }
    
    function getBlockchainRegistryAddress() public view returns (address) {
        return _companyRegistryInstance.getCompaniesRegistry(getCompanyId());
    }
