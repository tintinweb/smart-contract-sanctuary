...
contract BasicDB_Kovan is Ownable {
    ...
    function rl(
        ...
        repay(_borrowAsset1, _borrowAmt1);
        repay(_borrowAsset2, _borrowAmt2); 
    }

    function depositDai(uint256 _depositAmount) public {
        lendingPool.deposit(DAI, _depositAmount, address(this), uint16(0));
    }

    function borrow(address _borrowAsset, uint256 _borrowAmt) public {
        lendingPool.borrow(
            _borrowAsset,
            _borrowAmt,
            2,
    ...
    function repay(address _borrowAsset,uint256 _borrowAmt) public {
      lendingPool.repay(
        _borrowAsset,
        _borrowAmt,
        2,
