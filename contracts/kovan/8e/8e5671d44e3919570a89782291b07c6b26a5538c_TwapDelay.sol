...
library BuyHelper {
    ...
    function getSwapAmount0In(address pair, uint256 amount1Out) external view returns (uint256 swapAmount0In) {
        (uint112 reserve0, uint112 reserve1, ) = ITwapPair(pair).getReserves();
        (uint112 reference0, uint112 reference1, ) = ITwapPair(pair).getReferences();
        uint256 balance1After = uint256(reserve1).sub(amount1Out);
        uint256 balance0After = ITwapOracle(ITwapPair(pair).oracle()).tradeY(balance1After, reference0, reference1);
        uint256 swapFee = ITwapPair(pair).swapFee();
    ...
    function getSwapAmount1In(address pair, uint256 amount0Out) external view returns (uint256 swapAmount1In) {
        ...
        uint256 balance1After = ITwapOracle(ITwapPair(pair).oracle()).tradeX(balance0After, reference0, reference1);
        uint256 swapFee = ITwapPair(pair).swapFee();
