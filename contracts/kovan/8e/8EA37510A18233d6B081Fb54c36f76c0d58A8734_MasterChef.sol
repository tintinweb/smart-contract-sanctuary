...
contract MasterChef is Ownable {
    ...
    function pendingSashimi(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 sashimiReward = multiplier
                .mul(sashimiPerBlock)
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
            accSashimiPerShare = accSashimiPerShare.add(
                sashimiReward.mul(1e12).div(lpSupply)
            );
        }
        return
            user.amount.mul(accSashimiPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accSashimiPerShare = pool.accSashimiPerShare.add(
            sashimiReward.mul(1e12).div(lpSupply)
        );
        pool.lastRewardBlock = block.number;
    }
