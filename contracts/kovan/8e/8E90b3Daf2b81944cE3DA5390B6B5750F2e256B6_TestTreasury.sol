...
import "../interface/token/IEve.sol";
...
contract TestTreasury {
    ...
    event ExchangeEve(int256 evePrice, int256 coinPrice, uint256 quatityToExchange);

    constructor(address eve, address priceConverter) {
        _eve = IEve(eve);
    ...
    function exchangeEVE(address to, address tokenAddress, uint256 value) public payable returns (int256) {
        address from = msg.sender;
        MultiToken _multiToken = MultiToken(tokenAddress);
        ...
        uint256 tokenId = _multiToken.addressToId(tokenAddress);
        string memory tokenName = _multiToken.idToName(tokenId);

        int256 evePrice = int256(getEVEPrice());
        int256 coinPrice = getXUsdPrice(tokenName);

        emit ExchangeEve(evePrice, coinPrice, value);
        ...
        uint256 amount = uint256(perEDT) * value / (10 ** 8);
        _eve.mint(to, amount);
        _multiToken.burn(from, tokenId, value);
