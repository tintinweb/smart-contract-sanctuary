...
// File: contracts\@openzeppelin\contracts\token\ERC721\extensions\IERC721Metadata.sol
...
// File: contracts\@openzeppelin\contracts\token\ERC721\ERC721.sol
...
// File: contracts\tokens\xBuoy.sol

pragma solidity ^0.8.0;

contract xBuoy is ERC721 {

    address index = 0xa5DAd54a3d640344d163722440e6865f2F71F56a;

    struct NFT {
        bool active;
        uint256 contributed;  //amount of liquidity ownership token locked
        uint256 allotment;    //amount of buoy locked
        uint256 rewards;      //how many tokens they will earn each period
        uint256 payouts;      //how many payouts the stake has left 
        uint256 nextClaim;
        address platform;     //uniswap or balancer
    }
    ...
    constructor() ERC721("xBuoy", "XBUOY") {
    ...
    function craftNFT(
        ...
        address _platform
            ...
            //require(msg.sender == buoyMine);
            uint _id = nfts.length;
            _safeMint(_sender, _id);
            nfts.push(NFT(true, _contributed, _allotment, _rewards, _payouts, _nextClaim, _platform));
    }
    
    //staking contract will need to transfer NFT to itself before this is called
    function burn(uint _id) public {
            //require(msg.sender == addressIndex.getMine());
            _burn(_id);
    }

    function setNFT(uint id, uint nextClaim, uint payouts) public {
        //require(msg.sender == addressIndex.getMine();)
        nfts[id].nextClaim = nextClaim;
        nfts[id].payouts = payouts;
    }

    function killNFT(uint id) public {
        //require(msg.sender == addressIndex.getMine();)
        nfts[id].active = false;
    }
    
    function viewNFT(uint id) public view returns(bool, uint, uint, uint, uint, uint, address) {
        NFT memory token = nfts[id];
        return(token.active, token.contributed, token.allotment, token.rewards, token.payouts, token.nextClaim, token.platform);
    }
}