...
contract Lfi is ERC20Capped, ILfi {
    ...
    modifier onlyMinter() {
        require(_minters.contains(msg.sender), "LFI: sender not a minter");
        _;
    }
    ...
    function removeMinter(address account) external onlyBy(governanceAccount) {
        ...
        uint256 index = _minters.indexOf(account);
        require(index > 0, "LFI: account not an existing minter");

        _minters.removeAt(index);
    }
}
