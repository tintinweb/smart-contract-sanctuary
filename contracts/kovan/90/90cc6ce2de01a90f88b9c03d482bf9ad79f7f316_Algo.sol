...
contract Algo is Context,Ownable,ALGOToken{
	...
	function balanceOf(address _user) public payable returns (uint256) {
	...
	function allUserBalances(address _user) public returns (uint256 initSupply, uint256 userTokenCirculation, uint256 userBalance, uint256 realUserBalance) {
	...
	function transfer(address to, uint256 _tokens) public returns (bool) {
		_transfer(msg.sender,to,_tokens);
		return true;
	}
	
	    
	function transferFrom(address from,address to, uint256 _tokens) public returns (bool) {
	...
	function _transfer(address from,address to,uint256 _tokens) private returns (uint256) {
