    ...
    KeeperCompatibleInterface
{
    ...
    mapping(address => uint256[]) public employeeToRecentData;
    mapping(address => uint256) public employeeToPaymentInterval;
    mapping(address => mapping(string => address)) public tokenToPriceFeed;

    /////////////// TIJDELIJK
    address public constant UNI = 0xFf795577d9AC8bD7D90Ee22b6C1703490b6512FD;
     ...
     */
    function addAllowedTokens(uint256 _tokenId, address _token)
        public
        onlyOwner 
    {
    ...
    /**
     *  Assign token address to Chainlink Price Feed contract address.
     *  DAI/USD, ETH/USD, DAI/ETH
     */
    function setPriceFeedContract(
        address _from, string memory _to, address _priceFeed
    )
        public
        onlyOwner
    {
        ...
        onlyOwner
    {
        ...
        // 0 = previousSalary 
        // 1 = previousUsedCPI
        // 2 = previousPaymentTimestamp
        // 3 = preferedPaymentToken
    ...
    function getMinimumFundAmount() public returns (uint256) {
        ...
        for (uint256 i = 0; i < employees.length; i++) {
            employee = payable(employees[i]);
            minimumFundAmountInUSD = employeeToRecentData[employee][0];
            minimumFundAmount += getConversionRate(
                0, "usd", minimumFundAmountInUSD
            );
        }
    }
     ...
     */
    function getPrice(uint256 _tokenId, string memory _inCurrency)
        public
        view
    ...
    function payEmployee() public returns (bool) {
        for (uint256 i = 0; i < employeesToPayLength; i++) {
            ...
            uint256 salaryInToken = getConversionRate(
                preferedPaymentToken, "usd", salaryInUSD
            );
            // Paying.
            (bool sent, bytes memory data) = payable(
                employee
            ).call{value: salaryInToken}("");
