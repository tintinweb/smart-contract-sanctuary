...
contract StakeAuction is VersionedInitializable, AuctionBase {
    using SafeERC20 for IERC20Permit;
    using SafeERC20 for IStakedAave;
    using SafeMath for uint256;

    uint256 public constant STAKEAUCTION_REVISION = 0x1;
    IERC20Permit internal constant AAVE = IERC20Permit(0xB597cd8D3217ea6477232F9217fa70837ff667Af);
    IStakedAave internal constant STKAAVE = IStakedAave(0xf2fbf9A6710AfDa1c4AaB2E922DE9D69E0C97fd2);

    mapping(address => mapping(uint256 => DataTypes.AuctionData)) internal _auctionsByNft;
    mapping(address => mapping(uint256 => DataTypes.DistributionData[]))
        internal _distributionsByNft;
     ...
     */
    event Bid(address indexed nft, uint256 indexed nftId, address indexed bidder, uint256 amount);
    ...
    function createAuction(
        ...
        require(_auctionsByNft[nft][nftId].owner == address(0), Errors.AUCTION_EXISTS);
        ...
        _auctionsByNft[nft][nftId] = DataTypes.AuctionData(
            startPrice,
            msg.sender,
            address(0),
            startTimestamp,
        ...
        for (uint256 i = 0; i < distribution.length; i++) {
            ...
            _distributionsByNft[nft][nftId].push(distribution[i]);
        }
    ...
    function claimWonNFT(address nft, uint256 nftId) external whenNotPaused {
        ...
        _stakeDataByNft[nft][nftId] = DataTypes.StakeData(clone, owner);
        ...
        //delete(_distributionsByNft[nft][nftId]);
    ...
    function redeem(address nft, uint256 nftId) external whenNotPaused {
        ...
        address owner = stakeData.owner;
        DataTypes.DistributionData[] memory distribution = _distributionsByNft[nft][nftId];
        ...
        // Distribute AAVE and STKAAVE rewards, rounding errors accounted for in _distribute.
    ...
    function claimRewards(address nft, uint256 nftId) external whenNotPaused {
        DataTypes.StakeData storage stakeData = _stakeDataByNft[nft][nftId];
        DataTypes.DistributionData[] storage distribution = _distributionsByNft[nft][nftId];
        address vault = stakeData.vault;
        require(vault != address(0), Errors.NONEXISTANT_VAULT);
