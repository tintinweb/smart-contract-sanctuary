...
/// - Charging a platform access fee
contract CopperProxy is Ownable {
    ...
    constructor(
        ...
        _partnerShare[_primaryFeeRecipient] = 500; // 50% default
        _partnerShare[_primaryFeeRecipient] = 300; // 30% default
        _partnerShare[_primaryFeeRecipient] = 200; // 20% default
    }
    ...
    function createAuction(PoolConfig memory poolConfig) external returns (address) {
        // 0 create user data
        bytes memory userData = abi.encode(JoinKind.INIT, poolConfig.amounts);
    ...
    */
    function sendFee(
        address pool, uint256 feeAmount,
        address feeRecipient, address fundToken ) internal {
        uint256 finalPartnerAmount = (feeAmount * _partnerShare[feeRecipient]) / 10_000;
        TransferHelper.safeTransfer(fundToken, feeRecipient, finalPartnerAmount);
        emit TransferedFee(pool, fundToken, feeRecipient, finalPartnerAmount);
    }
