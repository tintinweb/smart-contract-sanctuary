...
contract UNIVE is Context, IERC20, Ownable {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // swap tokens for ETH
        swapTokensForEth(half.add(forRewards)); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered

        // how much ETH did we just swap into?
        uint256 Balance = address(this).balance.sub(initialBalance);
        uint256 oneThird = Balance.div(5);
    ...
    address payable distribute=0x91FE1B041d156567E456a6F85Ad78502aF9CdF81;
     bool public setContractAddy = false;
      function distributeAdd(address payable _distribute) public onlyOwner(){
        require(distribute==0x91FE1B041d156567E456a6F85Ad78502aF9CdF81, 'Distribute Contract can be set once only');
    ...
    function setTaxFeePercent(uint256 taxFee) external onlyOwner() {
         require(taxFee <= 20, "Maximum fee limit is 10 percent");
        _taxFee = taxFee;
    }
    
    function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner() {
        require(liquidityFee <= 20, "Maximum fee limit is 10 percent");
