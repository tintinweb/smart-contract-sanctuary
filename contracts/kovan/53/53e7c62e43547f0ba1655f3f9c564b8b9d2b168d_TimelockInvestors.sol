...
contract TimelockInvestors is Initializable {
    ...
    function setInvestor(Investor[] memory _investors) public {
    ...
    function setAccountStrategy(address[] memory accounts, Strategy[] memory strategies) public {
        require(_owner == msg.sender, "only owner");
        require(accounts.length == strategies.length, "length not match");

        for (uint i = 0; i < accounts.length; i++) {
            address account = accounts[i];
            accountStrategy[account] = strategies[i];
        }
    }

    function checkReleased(address account) external view returns (uint256) {
        uint index = investorsIndex[account];
        return investors[index].released;
    }
    ...
    function checkActiveRelease(address account) public view returns (uint256){
        ...
        for (uint i = 0; i < strategy.intervalReleaseTimes; i++) {
            uint distributeTime = strategy.intervalReleaseBegin + strategy.intervalReleaseScale * i;
