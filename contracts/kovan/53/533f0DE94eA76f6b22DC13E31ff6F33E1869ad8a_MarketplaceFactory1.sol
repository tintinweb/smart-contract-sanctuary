...
contract MarketplaceFactory1 {
  ...
  event Cloned(address cloneAddress);
...
//   }
  ...
  function createClone(address _owner) public{
      require(msg.sender == admin, "Only admin can clone contract");
      address clone = Clones.clone(implementationContract); //create smart contract clone
      emit Cloned(clone);

      PlaytreksMarketplaceProxy1(clone).initialize(_owner);
  }
}