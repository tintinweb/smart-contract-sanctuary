...
contract Cryptium is CrptERC20, Control, ReentrancyGuard {
	...
	function rebase() external nonReentrant initialized {
		...
		if (isInflRebase) {
			...
			_lastRebase = _lastRebase + monthsPassFromRebase * (3 minutes);
		}
	...
	function _getMonthsPass() private returns (uint256, uint256) {
		...
		uint256 monthsPassAll = (block.timestamp - _startTime) / (3 minutes);
		uint256 monthsPassFromRebase = (block.timestamp - _lastRebase) / (3 minutes);
