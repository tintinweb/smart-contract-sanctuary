...
import "./interfaces/IEthSignSAExecutorContract.sol";
import "./interfaces/IEthSignSAModeratorContract.sol";
import "./interfaces/IEthSignSARegistryContract.sol";

contract EthSignSAExecutorContract is
    IEthSignSAExecutorContract,
    Ownable,
    Pausable
{
    using SafeERC20 for IERC20;

    // keccak256(abi.encodePacked("I approve releasing all pending funds back to the initiator."))
    bytes32 public constant REFUND_AUTH_MESSAGE_HASH =
        0x6540b8867d9462ff3693e08e436ce64fc22dfee726e58f778132c476448af48e;

    IEthSignSARegistryContract public immutable _registry;
    IEthSignSAModeratorContract public immutable _moderator;
    IERC20 public immutable _rewardTokenContract;
    ...
    constructor(
        ...
        _rewardTokenContract = rewardTokenContract;
        ...
        )
    {
        return (
            owner(),
            _beneficiary,
            keccak256(abi.encodePacked(_rewards)),
            _index
        );
    }

    function depositEscrow() external override whenPaused {
        require(
            _rewardTokenContract.balanceOf(address(this)) == 0,
            "AE: Funds already deposited"
        );
        uint256 totalRewards = 0;
        for (uint256 i = 0; i < _rewards.length; ++i) {
            totalRewards += _rewards[i];
        }
        _rewardTokenContract.safeTransferFrom(
            owner(),
            address(this),
            totalRewards
        );
        emit EthSignSAExecutorEscrowDepositReceived(
    ...
    function handleDispute() external override {
        require(
            _msgSender() == _registry.getExecutorRegisteredTrigger(this),
            "AE: Unauthorized call"
        );
        _rewardTokenContract.safeApprove(
            address(_moderator),
            _rewardTokenContract.balanceOf(address(this))
        );
        _moderator.receiveDisputedFunds(_rewardTokenContract);
        emit EthSignSAExecutorDisputeHandled();
        _pause();
    }

    function refundEscrow(bytes calldata beneficiarySignature)
        external
        override
        onlyOwner
        whenNotPaused
    {
        require(
            SignatureChecker.isValidSignatureNow(
                _beneficiary,
                ECDSA.toEthSignedMessageHash(REFUND_AUTH_MESSAGE_HASH),
                beneficiarySignature
            ),
            "AE: Bad signature"
        );
        uint256 amount = _rewardTokenContract.balanceOf(address(this));
        _rewardTokenContract.safeTransfer(owner(), amount);
        emit EthSignSAExecutorEscrowDepositRefunded(amount);
        _pause();
    }

    function execute() external override whenNotPaused returns (bool) {
        require(
            _registry.getExecutorRegisteredTrigger(this) == _msgSender(),
            "AE: Unauthorized caller"
        );
        uint256 payoutAmount = _rewards[_index];
        _rewardTokenContract.safeTransfer(_beneficiary, payoutAmount);
        ++_index;
        emit EthSignSAExecutorExecuted(_beneficiary, payoutAmount);
        return customExecute();
    }

    function customExecute() internal virtual returns (bool) {}
}
