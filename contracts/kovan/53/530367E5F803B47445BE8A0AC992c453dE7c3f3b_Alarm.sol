...
contract Alarm is Ownable, KeeperCompatibleInterface {
  ...
  function setAlarm(uint256 time) public payable {
    ...
    require(msg.value > 100000 wei, "You must stake at least 100000 wei.");
  ...
  function performUpkeep(bytes calldata /* performData */) external override {
    ...
    for(uint i = 0; i < alarmOwners.length; i++) {
      if(alarms[alarmOwners[i]].time < block.timestamp) {
        // Call the transfer contract to send the eth to a charity
        console.log("Sending their eth to charity!");
