...
contract EthPool is IEthPool, AccessControl {
  ...
  /// @dev total rewards put by team members
  uint256 public rewards;

  /// @dev total deposit put by users
  uint256 public totalDeposits;

  /**
   * @dev reward amount when user first put his deposit
   * and used to calculate user reward
   * based on gap between reward and this value
   */
  mapping(address => uint256) public rewardBeforeDeposit;

  /// @dev deposit amount for per user
  ...
  function deposit() payable external override {
    ...
    if (userDeposits[msg.sender] == 0) {
      rewardBeforeDeposit[msg.sender] = rewards;
    }
  ...
  function rewardBalanceOf(address user) public override view returns (uint256) {
    return (rewards - rewardBeforeDeposit[user]) * userDeposits[user] / totalDeposits;
  }

  /**
   * @dev withdraw user deposit and reward
   */
  function withdraw() external override {
    require(userDeposits[msg.sender] > 0, "You didn't make a deposit");

    uint256 amount = userDeposits[msg.sender] + rewardBalanceOf(msg.sender);
