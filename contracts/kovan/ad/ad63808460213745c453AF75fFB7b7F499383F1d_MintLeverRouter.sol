...
interface IEarning {
    ...
    function harvest() external returns (address[] memory, uint256[] memory);
...
abstract contract MintLever is IMintLever, Lock, AddressCheck {
    ...
    function updateBondsV3(address user, address token) internal {
        bondsV3[user][token] = bondsV4[user][token].mul(getLoadPublicsRate(user, token)).div(10 ** 18);
    }
...
contract MintLeverRouter is IMintLeverRouter, ConfigBase {
    ...
    function getBondUSD(address mintLever, address user, uint8 decimal) override external view returns (uint256) {
        uint256 usd;
        uint256 _usd;
        (address[] memory tokens, uint256[] memory amounts) = this.getBond(mintLever, user);
    ...
    function getBond(address mintLever, address user) override external view returns (address[] memory, uint256[] memory) {
        ...
        for (uint256 i = 0; i < bondsCount; i++) {
            ...
            amounts[i] = _mintLever.bondsV4(user, tokens[i]).mul(getLoadPublicsRate(user, tokens[i], _mintLever.id())).div(10 ** 18);
        }
    ...
    // leeqiang
    // function get(address mintLever, address user, uint256 percentage) external view returns (address[] memory, uint256[] memory) {
    //     require(0 < percentage && percentage <= 100, "publics:percentage_overflow");
    //     MintLever _mintLever = MintLever(mintLever);
    //     uint256 amount = _mintLever.capitalAmounts(user).mul(percentage).div(100);
    ...
    function getLoadPublicsRate(address user, address token, uint256 id) internal view returns (uint256) {
        (uint256 code, , , uint256 rate) = config.getLoanPublics(token).getAccountSnapshot(user, id, ILoanTypeBase.LoanType.MINNING_SWAP_PROTOCOL);
        require(0 == code, "publics:loan_publics_get_account_snapshot_error");
        return rate;
    }
