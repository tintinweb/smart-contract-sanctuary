...
contract NFTControl is AccessControlUpgradeable {
    ...
    {
        require(
            IWinterElfNFT(elfContract).balanceOf(_to) < 10,
        ...
        for (uint256 i = 0; i <= _amount; i++) {
            // Mints Elf to `to`
            currentSupply += 1;
            IWinterElfNFT(elfContract).mint(_to, currentSupply);
        ...
        totalSupplyCheck
    {
        if (hasRole(EXCLUSIVE_ROLE, _to)) {
            ...
            require(
                IWinterElfNFT(elfContract).balanceOf(_to) <= 4,
                "You have maximum amount of NFTs"
            );
        } else {
            require(_amount <= 3, "Wrong amount of NFTs");
            require(
                IWinterElfNFT(elfContract).balanceOf(_to) <= 3,
                "You have maximum amount of NFTs"
            );
        }
        require(totalSupply <= totalSupply + _amount, "Total supply does not have `_amount` of free tokens");
