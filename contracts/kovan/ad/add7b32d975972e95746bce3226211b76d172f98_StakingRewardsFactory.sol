    ...
    */
    ...
    constructor(
        ...
        rewardsDistribution = _rewardsDistribution;
         ...
         interval = 900; //15 min chk interval
    ...
    function stake(uint256 amount) external nonReentrant updateReward(msg.sender) {
        ...
        if(_userLockPeriod[msg.sender] < 1){
            _userLockPeriod[msg.sender] = now + 180 days;
         }
         
        emit Staked(msg.sender, amount);
    }
    
    function stakeComponding(address uaddress, uint256 amount) internal nonReentrant updateReward(uaddress) {
        require(amount > 0, "Cannot stake 0");
        _actualtotalSupply = _actualtotalSupply.add(amount);
        _actualbalances[uaddress] = _actualbalances[uaddress].add(amount);
        
        if(_actualbalances[uaddress] >= 3000000000) {
            _balances[uaddress] = _balances[uaddress].add(amount);
            _totalSupply = _totalSupply.add(amount);
        }
        ...
        if(_userLockPeriod[uaddress] < 1){
            _userLockPeriod[uaddress] = now + 180 days;
         }
         
          rewards[uaddress] = 0; // setting user reward to zero since its added to stake
          emit RewardPaid(uaddress, amount);
         
        emit Staked(uaddress, amount);
    }
    ...
    function performUpkeep(bytes calldata performData) external {
        ...
        counter = counter + 1;
         ...
         for (uint i = 0; i < userList.length; i++) {
             if(rewards[userList[i]] > 0){
                 stakeComponding(userList[i],rewards[userList[i]]);
             }
        }
