...
contract Precog {
    ...
    function test(uint expiryDate, address[] memory pairs, TradeInfo memory trade)
        public view returns (uint reserveAExchange, uint reserveBExchange, uint reserveAFutureExchange, uint reserveBFutureExchange, uint k)
    {
        address factory = IFutureExchangeRouter(trade.futureExchange).futureTokenFactory();
        reserveAExchange = IERC20(usdc).balanceOf(trade.exchange);
        reserveBExchange = IERC20(pairs[1]).balanceOf(trade.exchange);
        (reserveAFutureExchange, reserveBFutureExchange) = PrecogV2Library.getReservesFuture(factory, usdc, pairs[1], expiryDate);
