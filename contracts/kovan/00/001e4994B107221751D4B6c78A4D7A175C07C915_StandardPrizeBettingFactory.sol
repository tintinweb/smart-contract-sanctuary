...
abstract contract AbstractBettingContract is IBettingContract {
    ...
    function getTotalToken() public view override returns (uint256) {
        return
            IERC20(IBettingPool(owner).getTokenPool()).balanceOf(address(this));
    }
...
contract StandardPrizeBettingContract is AbstractBettingContract {
        ...
        if (winners.length > 0) {
            ...
            for (uint256 i = winners.length - 1; i >= 0; i--) {
                if (winners[i] != address(0x0)) {
                    ...
                    delete winners[i];
                    IERC20(IBettingPool(owner).getTokenPool()).safeTransfer(
                        winner,
                        reward
                    );
                }
        ...
        betable
    {
        ...
        IERC20(IBettingPool(owner).getTokenPool()).safeTransferFrom(
            msg.sender,
            address(this),
            ticketPrice
        );
...
import "../StandardPrizeBettingContract.sol";
import "../interface/IBettingFactory.sol";

contract StandardPrizeBettingFactory {
    function createNewBettingContract(
        ...
        StandardPrizeBettingContract betting = new StandardPrizeBettingContract(
            _owner,
            _creator,
            _rewardForWinner,
            _rewardForCreator,
