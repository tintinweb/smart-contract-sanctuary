...
contract LicensedNFT is ERC721Preset {
    ...
    address public tokenShareImplementation;
    ...
    mapping(uint256 => address) internal sharesToken;
    ...
    bool public sharesTokenDeployementPaused;

    /* Events */
    event AuthTokensImplementationSet(address _tokenImplementation);
    event AuthDeploymentPaused();
    event AuthDeploymentResumed();
    event AuthDeployed(uint256 _tokenId, address _sharesAddress);
    event SubLicensesDeploymentPaused();
    event SubLicensesDeploymentResumed();
    event SubLicenseDeployed(uint256 _parentLicense, uint256 _childLicense, address _recipient);
    ...
    function colorToken(uint256 _tokenId, string memory _stateDescription) public {
        require(hasRole(ISSUER_ROLE, msg.sender), "ERR_CALLER");
    ...
    function cancelToken(uint256 tokenId) public {
        require(hasRole(ISSUER_ROLE, msg.sender), "ERR_CALLER");
        _burn(tokenId);
    }

    function updateTokenLegalURI(uint256 _tokenId, string memory _legalURI) public {
        require(hasRole(ISSUER_ROLE, msg.sender), "ERR_CALLER");
    ...
    /* Sub Licenses */
    function deploySubLicense(uint256 _tokenId, address _recipient) public {
        require(hasRole(ISSUER_ROLE, msg.sender), "ERR_CALLER");
        mint(_recipient);
    ...
    function revokeSubLicense(uint _subLicenseId) public {
    ...
    function _deployAuthToken(
        ...
        address newAuth = ClonesUpgradeable.clone(tokenShareImplementation);
        IAuthToken(newAuth).initialize(_name, _symbol, _tokendId);
        sharesToken[_tokendId] = newAuth;
        emit AuthDeployed(_tokendId, newAuth);
        return newAuth;
    }

    function pauseAuthDeployement() public {
        require(hasRole(ADMIN_ROLE, msg.sender), "ERR_CALLER");
        sharesTokenDeployementPaused = true;
        emit AuthDeploymentPaused();
    }

    function resumeAuthDeployement() public {
        require(hasRole(ADMIN_ROLE, msg.sender), "ERR_CALLER");
        sharesTokenDeployementPaused = false;
        emit AuthDeploymentResumed();
    }

    function setTokenImplementation(address _tokenImplementation) public {
        require(hasRole(ADMIN_ROLE, msg.sender), "ERR_CALLER");
        tokenShareImplementation = _tokenImplementation;
    ...
    function getChildLicenses(uint256 _tokenId) public view returns (uint256[] memory) {
        require(_exists(_tokenId), "ERR_TOKEN_ID");
        uint256[] memory childLicensesList = new uint256[](childLicenses[_tokenId].length());
    ...
    function getTokenStateDescriptionRef(uint256 _tokenId) public view returns (string memory) {
        require(_exists(_tokenId), "ERR_TOKEN_ID");
    ...
    function mint(
        ...
        _deployAuthToken(tokenId, name, symbol);
        tokenLegalURI[tokenId] = legalURI;
    }
