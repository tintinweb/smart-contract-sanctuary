...
contract ManagementCompany is IManagementCompany {
    ...
    function proposeNewLRSCAddress(address newLRSCAddress) external override onlyAdmin {
        require(newLRSCAddress != LRSCAddress, "Invalid parameter: please propose a different LOSC address.");
        require(newLRSCAddress.isContract() == true, "Invalid address: address should be contract address");
    ...
    function voteNewLRSCAddress() external override onlyAdmin {
        require(pendingLRSCAddress != address(0), "Invalid request: no propose found.");
    ...
    function debtVoidRequest(uint payableDebtAmount, uint loanPoolID, uint loanEntity) external override onlyAdmin {
        ...
        require(
            _pool != address(0), 
            "Invalid request: pool address is address(0)");
