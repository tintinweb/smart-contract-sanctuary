...
contract ExchangeStorage is ExchangeAdminStorage{
    ...
    uint256 public constant secondsPerDay = 600;

    uint256 public constant confirmTimeLimit = 1200; //10 days

    uint256 public constant depositTimeLimit = 10800; // 90 days
...
contract Exchange is ExchangeStorage {
    ...
    function initialize(
        ...
        endTime = _startTime.add(secondsPerDay.mul(180));
        paused = false;
    }
    ...
    function batchAccSashimi(uint256[] memory pids) external whenNotPaused{
        require(block.timestamp >= startTime, "Invalid Time");
        for(uint256 i=0; i < pids.length; i++){
            accSashimiInternel(pids[i]);
        }
    }

    function accSashimi(uint256 pid) external whenNotPaused{
        require(block.timestamp >= startTime, "Invalid Time");
        accSashimiInternel(pid);
    }

    function accSashimiInternel(uint256 pid) internal{
        PoolInfo storage pool = poolInfo[pid];
        UserInfo storage user = userInfo[pid][msg.sender];
