...
contract Habibi_Token is Context, IERC20, Ownable {
    ...
    address public charityAddress = 0x35291C34DA7D48357a04FB9A9c9822325595e1b2;
    address public developmentAddress = 0xcA3B70f5970bffC491b2a58209eD61b0C4f6311E;
    ...
    function collectFee(address account, uint256 amount, uint256 rate) private returns (uint256) {
        ...
        uint256 liquidityFee = amount.mul(_taxFee).div(10000);
        ...
        if(devFee > 0){
            ...
            _reflectionBalance[developmentAddress] = _reflectionBalance[developmentAddress].add(devFee.mul(rate));
            _devFeeTotal = _devFeeTotal.add(devFee);
            emit Transfer(account,developmentAddress,devFee);
        }
        
        //@dev last burn fee
        if (_tokenTotal > 121000000000 *10**9 && _tokenTotal.add(burnFee) < 121000000000 *10**9) {
            uint256 lastBurnAmount = _tokenTotal.sub(121000000000 *10**9);
        ...
        //@dev burn fee
        if (_tokenTotal >= 121000000000 *10**9 + burnFee) {
    ...
    function setDevelopmentAddress(address _devAddress) public onlyOwner {
    ...
    function setDevelopmenFee(uint256 fee) public onlyOwner {
        _devFee = fee;
    }
