...
abstract contract UniswapV2PCVDeposit is IPCVDepositV2, UniV2Ref {
    ...
    /// @notice withdraw lp tokens from the PCV allocation
    /// @param amount of lp tokens withdrawn
    /// @param to the address to send PCV to
    function withdrawLP(address to, uint256 amount)
        external
        override
        onlyPCVController
        whenNotPaused
    {
        ...
        require(
            amount <= lpBalance,
            "UniswapV2PCVDeposit: Insufficient lp amount"
        );

        TransferHelper.safeTransfer(address(pair), to, amount);

        emit LPWithdrawal(msg.sender, to, amount);
    }
