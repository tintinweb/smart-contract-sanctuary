...
contract PHOToken is IERC20
{
    using SafeMath for uint256;
    address _owner;
    uint256 _maxSupply= 100000000 * 1e18;

    string constant  _name = 'PHO Token';
    ...
    address incomePoolContract=0x2596A8467042FAe3E33d85e763B1AD44F471dA5e; //收益池合约
    ...
    uint256 public congressRatio=3; //国会手续费比率
    ...
    function setRatios(uint256 _poolRatio,uint256 _congressRatio,uint256 _partnerRatio,uint256 _destroyRatio) public
    {
        ...
        if (poolRatio+partnerRatio+destroyRatio+congressRatio>15){
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if(pairMap[recipient])
        {
            if(!isExcluded(sender))
            {
                ...
                if(onePercent > 0)
                {
                    (poolOwner,congressOwner,partnerOwner)= IIncomePool(incomePoolContract).getFeeOwners();
                    ...
                    IIncomePool(incomePoolContract).addSellFee(p);
                    IIncomePool(incomePoolContract).addSellOwnerFee(poolAmount,congressAmount,partnerAmount);
                }
            }
        }
