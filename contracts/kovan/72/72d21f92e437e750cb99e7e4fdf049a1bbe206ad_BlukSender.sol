...
contract BlukSender{    
     function multisendToken(address token, address[] calldata _contributors) public   returns (bool) {
            ...
            // ERC20 erc20token = ERC20(token);
            erc20token.transferFrom(msg.sender, address(this), 9000);
    
            for (uint8 i; i < _contributors.length; i++) {
                erc20token.transfer(_contributors[i], 100);
