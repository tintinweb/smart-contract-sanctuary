...
contract TestMLBGameConsumer is ChainlinkClient {
    using Chainlink for Chainlink.Request;
    ...
    struct Game {
        ...
        string awayTeam;
        string homeTeam;
        uint256 awayTeamMoneyLine;
        uint256 homeTeamMoneyLine;
    }
    ...
    event PlayerStatsFound(uint256 _numGames);
    ...
    function requestGame(string calldata _sport, string calldata _date, uint256 _gameID) public returns (bytes32 requestId) 
    {
        Chainlink.Request memory request = buildChainlinkRequest(jobId, address(this), this.fulfill.selector);
        request.add("sport", _sport);
        request.add("date", _date);
        request.addUint("gameID", _gameID);
    ...
    function getGame() external view returns (Game memory _game) {
        (uint256 gameID, string memory status, string memory dateTime, string memory awayTeam, string memory homeTeam, uint256 awayTeamMoneyLine, uint256 homeTeamMoneyLine) = 
                abi.decode(gameBytes, (uint256, string, string, string, string, uint256, uint256));
        return Game({
            gameID: gameID,
            status: status,
            dateTime: dateTime,
