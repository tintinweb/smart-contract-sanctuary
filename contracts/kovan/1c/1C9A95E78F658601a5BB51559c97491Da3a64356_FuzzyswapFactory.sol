...
contract ExternalOracle {
    ...
    {
        return observations.getAverages(
            time,
            tick,
            index,
            liquidity
    ...
    function getFee(
        ...
        (uint112 TWVolatilityAverage, uint256 TWVolumePerLiqAverage) = observations.getAverages(
            _time,
            _tick,
            _index,
            _liquidity
...
library Oracle {
    uint32 constant public TIME_AGO = 15*60;
    struct Observation {
        ...
        if (target == beforeOrAt.blockTimestamp) {
            ...
            return (
                ...
                beforeOrAt.volumeGMCumulative+
                    ((atOrAfter.volumeGMCumulative- beforeOrAt.volumeGMCumulative)/ observationTimeDelta) *
                    targetDelta
            );
        }
    }
    ...
    function observe(
        ...
        for (uint256 i = 0; i < secondsAgos.length; i++) {
            (tickCumulatives[i], secondsPerLiquidityCumulatives[i], volatilityCumulatives[i], volumePerAvgLiquiditys[i]) = observeSingle(
                self,
                time,
                secondsAgos[i],
                tick,
        ...
        internal view returns (uint112 TWVolatilityAverage, uint256 TWVolumePerLiqAverage)
    {
        ...
        if (lte(time, oldest.blockTimestamp, time - TIME_AGO)){
            ...
            (, uint160 secondsPerLiquidityCumulativeBefore, uint112 volatilityBefore, uint256 volumeGMCumulativeBefore) = 
            observeSingle(
                self,
                time,
                TIME_AGO,
...
library AdaptiveFee {
    ...
    function getFee(uint112 volatility, uint256 volumePerLiquidity, Configuration memory config) internal pure returns(uint256 fee) {
        ...
        fee = 500 + sigmoid(volumePerLiquidity, config.volumeGamma, sigm1 + sigm2, config.volumeBeta);
    }
