...
contract GenesisSales {
    ...
    uint256 public tokenPriceCommon = 77 ether / 100; // in ETH 0.077 = USDT 326.76952
    ...
    constructor(
        ...
        for (uint256 i = 0; i < _ids.length; i++) {
            idExists[_ids[i]] = true;
        }
        priceFeed = AggregatorV3Interface(_usdt);
    }
    ...
    function buyTokens(
        ...
        require(_paymentToken == usdt, "GenesisToken: unsuportted token");
        require(
            idExists[_tokenId],
            "GenesisToken: this tokenId has not supported"
        );
        ...
        uint256 amountRarity = 0;
        if (_tokenId == 1) {
            amountRarity = (_amount * tokenPriceCommon) * uint256(price);
        }
        if (_tokenId == 2) {
            amountRarity = (_amount * tokenPriceRare) * uint256(price);
        }
        if (_tokenId == 3) {
            amountRarity = (_amount * tokenPriceGold) * uint256(price);
        }

        require(amountRarity == 0, "GenesisToken: insufficient amount rarity");
