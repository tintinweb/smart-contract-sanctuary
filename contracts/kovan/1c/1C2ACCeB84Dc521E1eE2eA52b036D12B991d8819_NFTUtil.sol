...
contract DIADataNFT is Ownable, ERC1155 {
	...
	function mintDataNFT() external {
		...
		require(started);
		_mintDataNFT(msg.sender);
	}
	function _mintDataNFT(address _origin) public {
		...
		require(mintsPerWallet[_origin] < 3, "Sender has used all its 3 mints");
		...
		// Mint data NFT
		_mint(_origin, getErcRank(newRank), 1, "");

		// Update data struct
		rankMinter[newRank] = _origin;
		...
		emit MintedDataNFT(_origin, numMinted, newRank);
	}
