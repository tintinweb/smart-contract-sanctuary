...
contract GammaCrowdsale is ReentrancyGuard, Ownable {
    ...
    mapping (address => uint32) public _preICOInvestors;
    address[] public _preICOInvestorsAddresses;
    ...
    event PreICOInvestorSet(address indexed preICOInvestor, uint32 percentage);
     ...
     * @param vesting address of GAMMA vesting contract
    ...
    /**
     * @dev Adds/Sets pre-ico investor address and percentage. Can override existing record.
     * Can be called by owner only.
     * @param preICOInvestor address of pre-ico investor
     * @param percentage percentage (in one millionth of whole format) of total supply that pre-ico investor receives
     */
    function setPreICOInvestors(address preICOInvestor, uint32 percentage) external onlyOwner onlyBeforeOpening {
        // NOTE: the sum of all pre-ico investor percentages should be 10%, which is 100000 of 1000000
        _preICOInvestors[preICOInvestor] = percentage;
        _preICOInvestorsAddresses.push(preICOInvestor);

        emit PreICOInvestorSet(preICOInvestor, percentage);
    }
    ...
    function finalize() public onlyAfterClosing {
        ...
        if (softCapReached()) {
            uint256 preICOInvestorTotalAmount = (_gammaTokensSold.mul(10)).div(30);
            ...
            for(uint8 i = 0; i < _preICOInvestorsAddresses.length; i++) {
                uint256 share = _preICOInvestors[_preICOInvestorsAddresses[i]];
                uint256 amount = (preICOInvestorTotalAmount.mul(share)).div(100000);
                _gammaToken.mint(_preICOInvestorsAddresses[i], amount);
            }
