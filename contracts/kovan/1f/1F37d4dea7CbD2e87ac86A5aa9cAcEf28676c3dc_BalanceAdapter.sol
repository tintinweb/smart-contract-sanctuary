...
contract BalanceAdapter {
    struct BalanceParams {
        address owner;
        address token;
    }

    struct EMPPositionParams {
        address owner;
        address emp;
    }

    struct LSPositionParams {
        address owner;
        address lsp;
    }
    ...
    /**
     * @notice It gets all balances according to the balance parameters.
     * @dev Easy way to get all balances of ERC20 tokens.
     * @param _balanceParams Parameter definition to get balance
     */
    function getBalances(BalanceParams[] calldata _balanceParams) external view returns (BalanceInfo[] memory balances) {
        uint256 allParamsLength = _balanceParams.length;
        balances = new BalanceInfo[](allParamsLength);

        for (uint256 i = 0; i < allParamsLength; i++) {
            balances[i] = getTokenBalance(_balanceParams[i].token, _balanceParams[i].owner);
        }
    }

    /**
     * @notice It gets all balances according to the position parameters.
     * @dev Easy way to get all balances of ERC20 tokens.
     */
    function getEMPPositions(EMPPositionParams[] memory _positionParams) external view returns (EMPPositionInfo[] memory empPosition) {
        uint256 allParamsLength = _positionParams.length;
        empPosition = new EMPPositionInfo[](allParamsLength);

        for (uint256 i = 0; i < allParamsLength; i++) {
            empPosition[i] = getEMPPosition(_positionParams[i].emp, _positionParams[i].owner);
        }
    }

    /**
     * @notice It gets all LSPPositions according to the LSP parameters.
     */
    function getLSPPositions(LSPositionParams[] memory _slpParams) external view returns (LSPPositionInfo[] memory lspPositionInfo) {
        uint256 allParamsLength = _slpParams.length;
        lspPositionInfo = new LSPPositionInfo[](allParamsLength);

        for (uint256 i = 0; i < allParamsLength; i++) {
            lspPositionInfo[i] = getLSPosition(_slpParams[i].lsp, _slpParams[i].owner);
        }
    }

    function getTokenBalance(address _token, address _account) internal view returns (BalanceInfo memory balanceInfo) {
        balanceInfo.owner = _account;
        balanceInfo.tokenName = IERC20(_token).name();
        balanceInfo.decimals = IERC20(_token).decimals();
        balanceInfo.tokenSymbol = IERC20(_token).symbol();
        balanceInfo.balance = IERC20(_token).balanceOf(_account);
    }

    function getEMPPosition(address _emp, address _account) internal view returns (EMPPositionInfo memory empPosition) {
        IEMP empInstance = IEMP(_emp);
        IERC20 collateral = IERC20(empInstance.collateralCurrency());
        IERC20 syntheticToken = IERC20(empInstance.tokenCurrency());
        IEMP.PositionData memory position = empInstance.positions(_account);

        // fill empPosition
        empPosition.owner = _account;
        empPosition.emp = _emp;

        // collateral
        empPosition.collateralName = collateral.name();
        empPosition.collateralSymbol = collateral.symbol();
        empPosition.collateralDecimals = collateral.decimals();

        // synthetic
        empPosition.tokenName = syntheticToken.name();
        empPosition.tokenSymbol = syntheticToken.symbol();
        empPosition.tokenDecimals = syntheticToken.decimals();

        // position
        empPosition.rawCollateral = position.rawCollateral;
        empPosition.tokensOutstanding = position.tokensOutstanding;
    }

    function getLSPosition(address _lsp, address _account) internal view returns (LSPPositionInfo memory lspPosition) {
        ...
        // fill lspPosition
        lspPosition.owner = _account;
        lspPosition.lsp = _lsp;

        // collateral
        TokenInfo memory collateralToken = getTokenInfo(address(collateralTokenInstance), _account, _lsp);
        lspPosition.collateralTokenInfo = collateralToken;

        // longToken
        TokenInfo memory longToken = getTokenInfo(address(longTokenInstance), _account, _lsp);
        lspPosition.longTokenInfo = longToken;

        // shortToken
        TokenInfo memory shortToken = getTokenInfo(address(shortTokenInstance), _account, _lsp);
        lspPosition.shortTokenInfo = shortToken;
    }
