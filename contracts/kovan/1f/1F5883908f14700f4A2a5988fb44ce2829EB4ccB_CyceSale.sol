...
contract CyceSale is Context, Ownable {
    ...
    function _buyToken(address beneficiary, uint256 weiAmount) internal {
     if(!investedList[beneficiary]){
        investedList[beneficiary] = true;
        totalParticipants++;
      }

      totalETHRaised += weiAmount;
      totalSold += weiAmount.div(getUSDValue());
      payable(owner()).transfer(weiAmount);
      IERC20(cyceToken).transfer(beneficiary, weiAmount.div(getUSDValue()));
      emit purchase(beneficiary, weiAmount.div(getUSDValue()));
    }
