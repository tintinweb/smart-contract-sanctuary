...
/**
 * @title Stake Generic Auction Contract
 *
 *
 * @dev Contract that manages interactions between NFT owners, patrons and owners.
 */
contract GenericAuction is VersionedInit, AuctionBase, WETHBase {
    ...
    function createAuction(
        ...
        require(_nftData[nft][nftId].owner == address(0), Errors.AUCTION_EXISTS);
        ...
        _nftData[nft][nftId].owner = msg.sender;
    ...
    function reclaimEndedAuction(address nft, uint256 nftId) external whenNotPaused {
        ...
        address owner = _nftData[nft][nftId].owner;
        uint256 endTimestamp = auction.endTimestamp;
    ...
    function _bid(
        ...
        if (currentBidder != address(0)) {
            if (address(currency) == address(WETH)) {
                ...
                _safeTransferETH(currentBidder, currentBid);
            } else {
