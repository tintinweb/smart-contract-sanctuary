...
contract PlayTreksSwapper is Uniswap, UniswapFactory, Treks, DAI{
   ...
   receive() external payable {
     ...
     /**
    Similar to `getAvgDaiTreksPrice`, but checks the ETH/TREKS pool and
    converts ETH to TREKS tokens.
    @param amount the amount in ETH requested to be converted to TREKS
    */
   ...
   /**
    Similar to `getAvgDaiTreksPrice`, but checks the TREKS/ETH pool and
    converts TREKS to ETH tokens.
    @param amount the amount in TREKS requested to be converted to ETH
    */
   function getAvgTreksEthPrice(uint amount) public view returns(uint){
    ...
    /**
   Checks the DAI/TREKS pool and connects it to uniswap interface, gets the 
   reserves for both tokens and returns the correspondent amount in TREKS by using
   formula (amount * TREKS_reserve) / DAI_reserve
   @param amount the amount in DAI requested to be converted to TREKS
    */
   function getAvgDaiTreksPrice(uint amount) public view returns(uint){
    ...
    /**
    Similar to `getAvgDaiTreksPrice`, but converts TREKS to DAI
    @param amount the amount in TREKS requested to be converted to DAI
    */
   function getAvgTreksDaiPrice(uint amount) public view returns(uint){
    ...
    @ params tokenA first token address, tokenB second token address
     */
    ...
    /**
    Get the pool exchange address of Dai/Treks
    */
    ...
    */
    ...
    /**
    Remove your share in the Dai/Treks pool
    @ params liquidity amount to remove, amountAMin: amount of first token, amountBMin: amount of second token
    */
    ...
    function fastSwapExactTokensForETH(
        uint amountOut,
        uint amountIn,
        address token,
        uint deadline
    ...
    function fastSwapExactTokensForTokens(
        uint amountOut,
        uint amountIn,
        address token1,
        address token2,
    ...
    function fastSwapExactTokenForTokens(
        ...
        deadline = block.timestamp  + deadline;
        if(fastSwapExactTokensForTokens(
            amountOut,
            amountIn,
            token1,
            token2,
        ...
        path[0] = uniswap.WETH();
        path[1] = token2;
        uniswap.swapExactETHForTokensSupportingFeeOnTransferTokens{value: address(this).balance}(
            amountOut,
            path,
            msg.sender,
            deadline
