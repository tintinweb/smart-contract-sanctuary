...
contract RefineryVault is Ownable, Pausable {
    ...
    function deposit(uint256 _amount) external whenNotPaused notContract {
        ...
        user.refineryAtLastUserAction = user.shares.mul(balanceOf()).div(totalShares);
    ...
    function calculateHarvestRefineryRewards() external view returns (uint256) {
        uint256 amount = IMasterRefiner(masterRefiner).pendingRP1(0, address(this));
     ...
     */
    function calculateTotalPendingRefineryRewards() external view returns (uint256) {
        uint256 amount = IMasterRefiner(masterRefiner).pendingRP1(0, address(this));
        amount = amount.add(available());
    ...
    function balanceOf() public view returns (uint256) {
        (uint256 amount, ) = IMasterRefiner(masterRefiner).userInfo(0, address(this));
