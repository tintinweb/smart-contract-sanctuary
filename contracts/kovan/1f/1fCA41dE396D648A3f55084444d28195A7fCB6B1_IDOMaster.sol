...
contract IDOMaster is Ownable {
    ...
    uint256 public feeFundsPercent = 0; /* Default Fee 0% */
    ...
    constructor(
        ...
        divider = 100;
        //setTierPools(_disStakingPool, _lpDisStakingPool, _lpUniswapV2Pair, _disReserve0);
    }
        ...
        returns (uint256)
    {
        ...
        if(_lpBalance > 0) {
           //(uint112 _reserve0, uint112 _reserve1) = idoMaster.lpUniswapV2Pair.getReserves();
...
import "./interfaces/IidoMaster.sol";
 ...
 contract IDOPool is Pausable, Whitelist, ReentrancyGuard  {
    using SafeMath for uint256;
    using SafeERC20 for ERC20;
    ...
    constructor(
        ...
        require( _startTimestamp < _finishTimestamp,  "Start timestamp must be less than finish timestamp");
        require( _finishTimestamp > now, "Finish timestamp must be more than current block");
    ...
    function pay() payable external nonReentrant whenNotPaused{ //onlyWhitelisted whenNotPaused  nonReentrant
        ...
        if(enableTierSystem){
            require(user.totalInvestedETH.add(msg.value) <= idoMaster.getMaxEthPayment(msg.sender, maxEthPayment), "More then max amount");
        }
        else{
    ...
    function withdrawFunds() external onlyOwner nonReentrant{ // nonReentrant whenNotPaused isSaleFinalized {
        // require(minimumRaiseAchieved(), "Minimum raise has to be reached");
        idoMaster.getFeeWallet().transfer(address(this).balance.mul(feeFundsPercent).div(100)); /* Fee Address */
        msg.sender.transfer(address(this).balance);

        //   (bool success1, ) = idoMaster.getFeeWallet().call{value:address(this).balance.mul(feeFundsPercent).div(100)}("");
        //   require(success1, "Transfer fee failed.");
    ...
    function removeOtherERC20Tokens(address _tokenAddress, address _to) external onlyOwner {//isSaleFinalized {
        require(_tokenAddress != address(rewardToken), "Token Address has to be diff than the erc20 subject to sale"); // Confirm tokens addresses are different from main sale one
        ERC20 erc20Token = ERC20(_tokenAddress);
        require(erc20Token.transfer(_to, erc20Token.balanceOf(address(this))), "ERC20 Token transfer failed");
    } 

     function withdrawNotSoldTokens() external onlyOwner nonReentrant{
        require(now > finishTimestamp, "Withdraw allowed after finish time");
...
interface IidoMaster {
    ...
    // function setFeeToken(address _newFeeToken) external;

    // function setFeeAmount(uint256 _newFeeAmount) external;

    // function setFeeWallet(address _newFeeWallet) external;

    // function setBurnPercent(uint256 _newBurnPercent, uint256 _newDivider) external;

    // function setTierPools(IStakingPool _disStakingPool, IStakingPool _lpDisStakingPool, IUniswapV2Pair _lpUniswapV2Pair, bool _disReserve0) external;
    // function setTier(uint256 _vipDisAmount, uint256 _vipPercent, uint256 _holdersDisAmount, uint256 _holdersPercent, uint256 _publicDisAmount, uint256 _publicPercent) external;
