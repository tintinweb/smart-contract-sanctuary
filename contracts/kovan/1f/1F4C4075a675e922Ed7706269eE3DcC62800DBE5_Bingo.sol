...
library BingoGenerate {
    ...
    function validateWinner(uint256 randomness,  mapping (uint256 => bool) storage Balls) internal view returns (bool found) {
...
contract Bingo is ERC721, VRFConsumerBase, KeeperCompatibleInterface {
    ...
    event BingoWinner(uint256 game, uint256 floor, address owner, uint256 tokenId);
    ...
    */
    function mintCard(uint256 amount) public payable {
        require(amount <= 10, "EP");
        require((_tokenIds.current().add(amount) - gameFloor) <= maxCards, "EG");
    ...
    */
    function checkUpkeep(bytes calldata checkData) external view override returns (bool upkeepNeeded, bytes memory performData) {
        upkeepNeeded = (_tokenIds.current() - gameFloor >= minCards) && (block.timestamp > lastBallTime+ballDrawTime && !outOfBalls);
        performData = checkData;
    }

    function performUpkeep(bytes calldata performData) external override {
        require(block.timestamp > lastBallTime+ballDrawTime, "MT");
        require(_tokenIds.current() - gameFloor >= minCards, "MP");
    ...
    */
    function claimBingo(uint256 cardId) public payable {
        require(ownerOf(cardId) == msg.sender, "NO");
        require(BingoGenerate.validateWinner(cardRandomness[cardId], Balls), "NW");
        require(cardId > gameFloor, "NC");

        payable(msg.sender).transfer(prizePool);
        emit BingoWinner(_gameIds.current(), gameFloor, msg.sender, cardId);
    ...
    */
    ...
    function validateCard(uint256 cardId) public view returns (bool) {
        return BingoGenerate.validateWinner(cardRandomness[cardId], Balls);
    }
    ...
    function getCardsByOwner(address _owner) view public returns(uint256[] memory result) {
    ...
    *   Setters.
    */
    function contractSettings(string memory _baseUri, uint256 _ballDrawTime, uint256 _minCardsPerGame, uint256 _maxCardsPerGame, uint256 _pricePerCard, uint256 _prizeSplit, address _owner) public onlyOwner {
        baseUri = _baseUri;
    ...
    */
    function withdraw(IERC20 _token, uint256 _amount) public payable onlyOwner {
        if(_amount > 0) {
            ...
            uint256 _each = address(this).balance.sub(prizePool).div(4);
