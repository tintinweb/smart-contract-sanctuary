...
//
// $ROBO - the first reflection token eco-system.
// BNB Rewards - Launchpad - Swap - more dApps coming soon!
// Hyper-Deflationary Buy-Back token that automatically rewards holders with BNB!
// It doesn’t stop there, the ROBO contract buys back any sells. 
// You will never see 2 sells at once Raising hands
// 
//
// 📱 Telegram: https://t.me/RoboTokenOfficial
// 🌎 Website: https://RoboToken.app
// 🌐 Twitter: https://twitter.com/TokenRobo
// 📷 Instagram: https://Instagram.com/TokenRobo
// 📖 Medium: https://medium.com/@RoboToken
//
...
import "./SafeMath.sol";
import "./ROBODividendTracker.sol";
...
contract Robo is ERC20, Ownable {
    ...
    ROBODividendTracker public dividendTracker;
    ...
    mapping (address => bool) private _isExcludedFromFees;
      // Exclude from fees and maxSellTransactionAmount amount
    mapping (address => bool) private _isExcludedPresaleContract;
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "ROBO: The dividend tracker already has that address");

        ROBODividendTracker newDividendTracker = ROBODividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "ROBO: The new dividend tracker must be owned by the ROBO token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "ROBO: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "ROBO: Account is already the value of 'excluded'");
        _isExcludedFromFees[account] = excluded;

        emit ExcludeFromFees(account, excluded);
    } 
    
    function excludePresaleContractFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedPresaleContract[account] != excluded, "ROBO: Account is already the value of 'excluded'");
        _isExcludedPresaleContract[account] = excluded;
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "ROBO: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "ROBO: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "ROBO: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "ROBO: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "ROBO: Cannot update gasForProcessing to same value");
    ...
    function isExcludedPresaleContractFromFees(address account) public view returns(bool) {
        return _isExcludedPresaleContract[account];
    } 
    ...
    function _transfer(
        ...
        if( 
            ...
            !_isExcludedPresaleContract[from] && 
            !_isExcludedPresaleContract[to]
        ) {
            require(amount <= maxSellTransactionAmount, "Sell transfer amount exceeds the maxSellTransactionAmount.");
        }

        uint256 timeSinceLaunch = block.timestamp.sub(tradingEnabledTimestamp);
        if(timeSinceLaunch <= 1 hours && from == uniswapV2Pair && !_isExcludedFromFees[to] && !_isExcludedPresaleContract[from] && !_isExcludedPresaleContract[to]){
